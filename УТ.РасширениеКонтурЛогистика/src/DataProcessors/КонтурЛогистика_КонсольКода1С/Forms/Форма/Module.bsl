...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("Текст", НачальныйКод);
	Параметры.Свойство("ДоступныРеквизитыТ1ЭТРН", ДоступныРеквизитыТ1ЭТРН);
	Параметры.Свойство("НомерСтрокиТоваровДокументовОснования", НомерСтрокиТоваровДокументовОснования);
	...
	Если Параметры.Свойство("Переменные") Тогда
		...
		Если ТипЗнч(Параметры.Переменные) = Тип("Массив") тогда
			
			Для каждого Переменная Из Параметры.Переменные Цикл
				
				НоваяСтрока = ПеременныеНаСервере.Добавить();
				
				Если Переменная = "ЭтотДокументОбъект" Тогда
					НоваяСтрока.Имя = "ЭтотОбъект";	
				Иначе
					НоваяСтрока.Имя = Переменная;
				КонецЕсли;
	...
	Если ПеременныеНаСервере.Количество() = 0 Тогда
		Элементы.ГруппаПеременныеНаСервере.Скрыть();	
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(НачальныйКод) Тогда 
		ПодключитьОбработчикОжидания("УстановитьНачальныйКод", 1);
	КонецЕсли;
		
КонецПроцедуры

&НаКлиенте
Процедура УстановитьНачальныйКод()
	
	Попытка
		УстановитьТекст(НачальныйКод, Неопределено, Ложь);
		ОтключитьОбработчикОжидания("УстановитьНачальныйКод");
	Исключение
...
Процедура ПередСтандартнымЗавершениемРаботы(Отказ)
	...
	Если Модифицированность И ЕстьКод() Тогда
		...
		Если ЗначениеЗаполнено(ИмяСохраненногоФайла) Тогда
			Оповещение = Новый ОписаниеОповещения("ВопросОбновленияСохраненияЗавершение", ЭтаФорма);
			ТекстВопроса = "Сохранить изменения перед завершением работы?";
		Иначе
			Оповещение = Новый ОписаниеОповещения("ВопросСохраненияЗавершение", ЭтаФорма);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность И Не ЗакрытиеФормыПодтверждено И Текст <> НачальныйКод Тогда
...
Процедура ОбработатьСобытиеРедактора(Событие)
	...
	Если Событие <> Неопределено Тогда
		...
		Если ИмяСобытия = "EVENT_ON_LINK_CLICK" Тогда
			Если 0 < Найти(Событие.params.href, "e1cib") Тогда
...
Функция ДополнитьСтроку_(Знач Результат, СимволовДополнить, Дополнение = " ", Слева = Истина)
...
Функция ЭтоСсылка_(Ссылка)
	...
	Если ОбъектМетаданных <> Неопределено Тогда
		...
		Массив = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.РазложитьСтрокуВМассивСлов(ПолноеИмя, ".");
		Массив[0] = Массив[0] + "Ссылка";
		ИмяТипа = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.СоединитьСтроку(Массив, ".");
...
Функция ПолучитьИмяОбъектаМетаданных(Ссылка)
	...
	Если ОбъектМетаданных <> Неопределено Тогда
		Возврат КонтурЛогистика_ОбщегоНазначенияКлиентСервер.РазложитьСтрокуВМассивСлов(ОбъектМетаданных.ПолноеИмя(), ".").Получить(0);
	КонецЕсли;
...
Функция ЭтоСтрокаТабличнойЧасти(Ссылка)
	...
	Если Результат <> Неопределено Тогда
		...
		Если ОбъектМетаданныхИмеетТЧ(ИмяРодителя) Тогда
			...
			ЧастиИмени = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.РазложитьСтрокуВМассивСлов(ИмяРодителя, ".");
			...
			Если ТабличнаяЧасть <> Неопределено Тогда
				...
				ИмяТипа = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("%1ТабличнаяЧастьСтрока.%2.%3", Имя1, Имя2, ТабличнаяЧасть.Имя);
...
Функция ЭтоЗаписьНабораЗаписей(Ссылка)
	...
	Если Результат <> Неопределено Тогда
		...
		Если 0 < Найти(ПолноеИмя, "Регистр") Или 0 < Найти(ПолноеИмя, "Register") Тогда
			
			ЧастиИмени = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.РазложитьСтрокуВМассивСлов(ПолноеИмя, ".");
			Имя1 = ЧастиИмени.Получить(0);
			Имя2 = ЧастиИмени.Получить(1);
			ИмяТипа = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("%1Запись.%2", Имя1, Имя2);
...
Функция ЭтоМенеджерЗаписиРегистра(Значение)
	...
	Если Результат <> Неопределено Тогда
		...
		Если Найти(Имя, "РегистрСведений") = 1 И Метаданные.РегистрыСведений.Содержит(Результат) Тогда
...
Функция ЭтоНаборЗаписейРегистра(Значение)
	...
	Если Результат <> Неопределено Тогда
		...
		ИначеЕсли Найти(Имя, "РегистрНакопления") = 1 И Метаданные.РегистрыНакопления.Содержит(Результат) Тогда
			ЭтоНаборЗаписей = (Тип("РегистрНакопленияНаборЗаписей." + Результат.Имя) = ТипЗнч(Значение));
		ИначеЕсли Найти(Имя, "РегистрРасчета") = 1 И Метаданные.РегистрыРасчета.Содержит(Результат) Тогда
			ЭтоНаборЗаписей = (Тип("РегистрРасчетаНаборЗаписей." + Результат.Имя) = ТипЗнч(Значение));
		ИначеЕсли Найти(Имя, "РегистрБухгалтери") = 1 И Метаданные.РегистрыБухгалтерии.Содержит(Результат) Тогда
...
Функция ЭтоПеречисление_(Значение)
	...
	Результат = Метаданные.НайтиПоТипу(ТипЗнч(Значение));
	Возврат Результат <> Неопределено И Найти(Результат.ПолноеИмя(), "Перечисление") = 1;
	...
	Если Представление = Неопределено Тогда
		...
		Попытка
			Если Не Значение.Пустая() Тогда
				...
				Если htmlTag Тогда
					Представление = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("<a href='%1'>%2</a>", Ссылка, Строка(Значение));
				Иначе
					Представление = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("[%1](%2)", Строка(Значение), Ссылка);
				КонецЕсли;
			Иначе
				Представление = "";
			КонецЕсли;
...
Функция ПолучитьПредставлениеТаблицыЗначений(Значение, Заголовок, Смещение, КешСсылок)
	...
	МассивСтрок.Добавить(КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("**%1 (%2):**", Заголовок, Значение.Количество()));
	Индекс = 0;
	
	Для Каждого СтрокаТЗ Из Значение Цикл
		
		Маркер = ДополнитьСтроку_("- ", Смещение * 4, " ");
		МассивСтрок.Добавить(Маркер + "Строка: " + Индекс);
		
		Для Каждого Колонка Из Значение.Колонки Цикл
			Маркер = ДополнитьСтроку_("- ", (Смещение + 1) * 4, " ");
	...
	Возврат КонтурЛогистика_ОбщегоНазначенияКлиентСервер.СоединитьСтроку(МассивСтрок, Символы.ПС);
...
Функция ПолучитьПредставлениеДереваЗначений(Дерево, Смещение, КешСсылок)
	...
	МассивСтрок = Новый Массив();
	МассивСтрок.Добавить(КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("**%1 (%2):**", "Дерево значений", Дерево.Строки.Количество()));
...
Функция ПолучитьПредставлениеМенеджераЗаписейРегистра(Значение, Смещение, КешСсылок)
	...
	МассивСтрок.Добавить(КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("**%1:**", ТипЗнч));
...
Функция ПолучитьПредставлениеСпискаЗначенийМассива(МассивСписок, Смещение, КешСсылок)
	...
	МассивСтрок.Добавить(КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("**%1 (%2)**:", ТипЗнч, МассивСписок.Количество()) + Символы.ПС);
	Индекс = 0;
	
	Для Каждого Значение Из МассивСписок Цикл
		
		Маркер = ДополнитьСтроку_("- (" + Индекс + ") ", Смещение * 4, " ");
...
Функция ПолучитьПредставлениеСтруктурыСоответствия(Значение, Смещение, КешСсылок)
	...
	МассивСтрок = Новый Массив();
	МассивСтрок.Добавить(КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("**%1 (%2)**:", ТипЗнч(Значение), Значение.Количество()));
...
Функция ПолучитьПредставлениеЗапроса(Запрос, Смещение, КешСсылок)
	...
	МаркерТекста = ДополнитьСтроку_("- ", (Смещение + 1) * 4, " ");
...
Функция ПолучитьПредставлениеПеременной(Значение, Знач Смещение, КешСсылок)
	...
	ИначеЕсли ЭтоПеречисление_(Значение) Тогда
		Представление = Строка(Значение);
	...
	ИначеЕсли ЭтоСсылка_(Значение) Тогда
...
Функция ПолучитьКраткуюСтруктуруПеременной(Значение, Имя, ПутьКДанным, КешСсылок)
	...
	ИначеЕсли ТипЗнч = Тип("Структура") Или
		ТипЗнч(Значение) = Тип("Соответствие") Тогда
		Представление = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("%1 (%2)", Строка(ТипЗнч), Значение.Количество());
		Картинка = "array";
	...
	ИначеЕсли ТипЗнч = Тип("СтрокаТаблицыЗначений") Тогда
		Картинка = "table";
	ИначеЕсли ТипЗнч = Тип("ДеревоЗначений") Тогда
		Представление = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("%1 (%2)", Строка(ТипЗнч), Значение.Строки.Количество());
		Картинка = "tree";
	...
	ИначеЕсли ТипЗнч = Тип("КоллекцияДвижений") Тогда
		Представление = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("Движения (%1)", Значение.Количество());
		Картинка = "table";
		Если Значение.Количество() = 0 Тогда
			Класс = "final";
		КонецЕсли;
	...
	ИначеЕсли ЭтоСтрокаТабличнойЧасти(Значение) Тогда
		...
		Картинка = "table";
	ИначеЕсли ЭтоТабличнаяЧасть(Значение) Тогда
		Представление = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("Табличная часть (%1)", Значение.Количество());
		Картинка = "table";
	ИначеЕсли ЭтоЗаписьНабораЗаписей(Значение) Тогда
		Картинка = "table";
	ИначеЕсли ЭтоНаборЗаписейРегистра(Значение) Тогда
		Представление = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("Набор записей (%1)", Значение.Количество());
		Картинка = "table";
		Если Значение.Количество() = 0 Тогда
			Класс = "final";
		КонецЕсли;
	...
	ИначеЕсли ЭтоСсылка_(Значение) Тогда
		...
		Если Найти(Представление, "</a>") Тогда
...
Функция ПолучитьОписаниеПеременной(ИдентификаторПеременной, ИмяПеременной, ПутьКДанным, АдресХраненияПеременных)
	...
	Попытка
		...
		ИначеЕсли ЭтоСсылка_(ЗначениеПеременной) Тогда
...
Функция ИзвлечьНомерСтрокиИзОписанияОшибки(ОписаниеОшибки)
	...
	НомерСтроки = 0;
	ОткрывающаяСкобка = Найти(ОписаниеОшибки, "(");	
	
	Если 0 < ОткрывающаяСкобка Тогда
		
		ЗакрывающаяСкобка = Найти(ОписаниеОшибки, ")");
		
		Если 0 < ЗакрывающаяСкобка Тогда
			
			Позиция = Сред(ОписаниеОшибки, ОткрывающаяСкобка + 1, ЗакрывающаяСкобка - ОткрывающаяСкобка - 1);
			Подстроки = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.РазложитьСтрокуВМассивСлов(Позиция, ",");
...
&НаСервереБезКонтекста
Функция ВыполнитьКод(Текст, ПоказыватьЗначенияПеременныхПослеВыполнения, ПеременныеСтруктура = Неопределено)
	
	стрИнициализацияПеременных = "";
	
	Для каждого Стр Из ПеременныеСтруктура Цикл
		стрИнициализацияПеременных = стрИнициализацияПеременных + Стр.Ключ + " = " + "ПеременныеСтруктура." + Стр.Ключ + ";" + Символы.ПС;
	КонецЦикла;
	
	Если ЗначениеЗаполнено(стрИнициализацияПеременных) Тогда
		стрИнициализацияПеременных = стрИнициализацияПеременных + ";" + Символы.ПС;
	КонецЕсли;
	...
	Попытка
		Выполнить(стрИнициализацияПеременных + Текст);
...
Функция ВыполнитьКодНаСервере(Текст, ПоказыватьЗначенияПеременныхПослеВыполнения)
	...
	Если НомерСтрокиТоваровДокументовОснования = 1 Тогда
		ДокументОбъект = Неопределено;
		НастройкаКонвертации = Неопределено;
	КонецЕсли;
		
	ПеременныеСтруктура = Новый Структура;
	Для каждого Стр Из ПеременныеНаСервере Цикл
		
		ПеременныеСтруктура.Вставить(Стр.Имя, Стр.Значение);
		
		Если ДоступныРеквизитыТ1ЭТРН 
			И ЗначениеЗаполнено(Стр.Значение) 
			И Стр.Имя = "ДокументОбъект" 
			И ТипЗнч(Стр.Значение) = Тип("ДокументСсылка.КонтурЛогистика_ТрН_Титул1") Тогда
			
			Для каждого Реквизит Из Метаданные.Документы.КонтурЛогистика_ТрН_Титул1.Реквизиты Цикл
				ПеременныеСтруктура.Вставить(Реквизит.Имя, Стр.Значение[Реквизит.Имя]);	
			КонецЦикла;
		...
		Если НомерСтрокиТоваровДокументовОснования = 1 Тогда
			...
			Если Стр.Имя = "ДокументОбъект" И ЗначениеЗаполнено(Стр.Значение) Тогда
				ДокументОбъект = Стр.Значение;
			ИначеЕсли Стр.Имя = "НастройкаКонвертации" И ЗначениеЗаполнено(Стр.Значение) Тогда
				НастройкаКонвертации = Стр.Значение;
			КонецЕсли; 
	...
	Если НомерСтрокиТоваровДокументовОснования = 1 
		И ЗначениеЗаполнено(ДокументОбъект) 
		И ЗначениеЗаполнено(ДокументОбъект.ДокументОснование) 
		И ЗначениеЗаполнено(НастройкаКонвертации) 
		И ЗначениеЗаполнено(НастройкаКонвертации.ИмяТабЧастьТовары)
		И Не ЗначениеЗаполнено(НастройкаКонвертации.ПутьКоллекцияТабЧастьТовары) Тогда 
		
		Попытка
			ПеременныеСтруктура.Вставить("СтрокаТоваровДокументаОснования", ДокументОбъект.ДокументОснование[НастройкаКонвертации.ИмяТабЧастьТовары][0]);
		Исключение
			Сообщить("Не удалось рассчитать переменную СтрокаТоваровДокументаОснования");
		КонецПопытки;
			
	КонецЕсли;
		
	Возврат ВыполнитьКод(Текст, ПоказыватьЗначенияПеременныхПослеВыполнения, ПеременныеСтруктура);
...
&НаКлиенте
Процедура КонструкторЗапросаСОбработкойРезультата(Команда)
	
	ВызватьКонструкторЗапросаСОбработкойРезультата(View().getQuery());
...
Процедура ЗагрузитьИзФайла(Команда)
	...
	Если ЕстьКод() Тогда
		Оповещение = Новый ОписаниеОповещения("ВопросЗаменыКодомИзФайла", ЭтаФорма);
...
&НаКлиенте
Процедура ВызватьКонструкторЗапросаСОбработкойРезультата(ПараметрыЗапроса)
	
	Оповещение = Новый ОписаниеОповещения("ВопросСоздатьНовыйЗапросСОбработкойРезультата", ЭтаФорма);
	ТекстВопроса = "Создать новый запрос с обработкой результата?";
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
			
КонецПроцедуры

&НаКлиенте
Процедура ВопросСоздатьНовыйЗапросСОбработкойРезультата(Ответ, ДопПараметры) Экспорт
	
	Если Ответ = КодВозвратаДиалога.Да Тогда
		Конструктор = Новый КонструкторЗапроса();
	
		Оповещение = Новый ОписаниеОповещения("ПриЗакрытииКонструктораЗапросовСОбработкой", ЭтаФорма, ДопПараметры);
		Конструктор.Показать(Оповещение);
	КонецЕсли;
...
Процедура ПриЗакрытииКонструктораЗапросовСОбработкой(Текст, ДопПараметры) Экспорт
	...
	Если Текст <> Неопределено И ЗначениеЗаполнено(Текст) Тогда
		...
		ТекстДляКонфигуратора = ТекстЗапросаСОбработкой(Текст);		
		УстановитьТекст(ТекстДляКонфигуратора, ДопПараметры, Истина);
...
&НаСервере
Функция ТекстЗапросаСОбработкой(Текст)

	ТекстДляКонфигуратора = ТекстЗапросаДляКонфигуратора(Текст);
	
	ТекстЗапроса = "Запрос = Новый Запрос;";
	ТекстЗапроса = ТекстЗапроса + Символы.ПС + "Запрос.Текст = " + ТекстДляКонфигуратора + ";";
	...
	Для каждого Отбор Из СхемаЗапроса.ПакетЗапросов[0].Операторы[0].Отбор Цикл
		СтрОтбор = Строка(Отбор);
		ИмяОтбора = Прав(СтрОтбор, СтрДлина(СтрОтбор) - Найти(СтрОтбор, "&"));		
		ТекстЗапроса = ТекстЗапроса + "Запрос.УстановитьПараметр(" + Символ(34) + ИмяОтбора + Символ(34) + ", " + ИмяОтбора + ");" + Символы.ПС;		
	КонецЦикла;
	
	ТекстЗапроса = ТекстЗапроса + Символы.ПС;
	
	ТекстЗапроса = ТекстЗапроса + "РезультатЗапроса = Запрос.Выполнить();";
	ТекстЗапроса = ТекстЗапроса + Символы.ПС;
	ТекстЗапроса = ТекстЗапроса + Символы.ПС;
	ТекстЗапроса = ТекстЗапроса + "Выборка = РезультатЗапроса.Выбрать();";
	ТекстЗапроса = ТекстЗапроса + Символы.ПС;
	ТекстЗапроса = ТекстЗапроса + "Пока Выборка.Следующий() Цикл";
	ТекстЗапроса = ТекстЗапроса + Символы.ПС;
	ТекстЗапроса = ТекстЗапроса + Символы.ПС;
	ТекстЗапроса = ТекстЗапроса + "КонецЦикла;";
...
&НаСервере
Функция ТекстЗапросаДляКонфигуратора(Текст)
	
	ТекстДляКонфигуратора = СтрЗаменить(Текст, Символы.ПС, Символы.ПС + "|");
	ТекстДляКонфигуратора = СтрЗаменить(ТекстДляКонфигуратора, """", """""");
	ТекстДляКонфигуратора = """" + ТекстДляКонфигуратора + """";
	
	Возврат ТекстДляКонфигуратора; 
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если ЗначениеЗаполнено(ИмяСохраненногоФайла) Тогда
		Оповещение = Новый ОписаниеОповещения("ВопросВосстановленияДанныхИзФайлаЗавершение", ЭтаФорма);
	...
	Если ОбъектМетаданныхИмеетПредопределенные(ПолноеИмя) Тогда
		...
		Если ИмяМетаданных(ПолноеИмя) = "ПланСчетов" Тогда
			...
			Пока Выборка.Следующий() Цикл 
				ОписаниеПредопределенных.Вставить(Выборка.Имя, КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("%1 (%2)", Выборка.Имя, Выборка.Код));
			КонецЦикла;
...
Функция ПолучитьОписаниеОбъектаМетаданных(ТипОбъекта, АдресОбновления)
	...
	Части = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.РазложитьСтрокуВМассивСлов(ТипОбъекта, ".");
	...
	Если ЗначениеЗаполнено(ИмяКоллекции) Тогда
		...
		Если 0 < Найти(ИмяМетаданных(ПолноеИмя), "ВнешнийИсточникДанных") Тогда
...
Процедура ОбработкаСобытияПолученияМетаданных(Событие)
	...
	Если 0 < Найти(ПараметрыЗапроса, ".") Тогда
		
		ЧастиЗапроса = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.РазложитьСтрокуВМассивСлов(ПараметрыЗапроса, ".");
...
Функция ПолучитьМодульИзИсходников(ПолноеИмяФайла)
	...
	Путь = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.РазложитьСтрокуВМассивСлов(ПолноеИмяФайла, РазделительПути);
...
Функция ЗагрузитьОбщийМодульПоИмени(КаталогИсходногоКода, ИмяМодуля, Глобальный)
	...
	ФайлМодуля = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("CommonModules%1%2%1Ext%1Module.bsl", РазделительПути, ИмяВКонфигураторе);
...
Функция ЗагрузитьМодульМенеджераИлиОбъектаПоИмени(КаталогИсходногоКода, СтруктураВыгрузки)
	...
	ФайлМодуля = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("%1%2Ext%2%3.bsl", СтруктураВыгрузки.ИмяКаталога, РазделительПути, ИмяТипа);
	ФайлМодуля = КаталогИсходногоКода + ФайлМодуля;
	...
	Если ФайлНаДиске.Существует() Тогда
		...
		АдресЗагрузки = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("%1.items.%2.%3", ИмяКоллекции, СтруктураВыгрузки.ИмяОбъектаМетаданных, ТипМодуля);
...
Процедура ДополнитьСтруктуруВыгрузкиИсходниковМетаданных(СтруктураВыгрузки, РазделительПути)
	...
	Имена = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.РазложитьСтрокуВМассивСлов(ПолноеИмя, ".");
	...
	Если Имена.Количество() = ЧастейМетаданных Тогда
		...
		Если ЗначениеЗаполнено(КаталогТипа) Тогда
			...
			СтруктураВыгрузки.ИмяКаталога = КонтурЛогистика_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку_("%1%2%3", КаталогТипа, РазделительПути, Имена[ИндексОбъекта]);
		КонецЕсли;
...
Процедура ЗакрытьКонсоль(СтандартноеЗакрытие)
	...
	Если ЗначениеЗаполнено(ИндексныйФайл) Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ПослеУдаленияИндексногоФайла", ЭтаФорма, ПараметрыОповещения);
