...
Перем ВерсияСовместимостиБольшеРавно8_3_6Лок; // Булево, Неопределено
Перем КоннекторСтдСтрЛок; // ОбработкаОбъект.КоннекторСтдСтр
Перем КоннекторСтдJSONЛок; // ОбработкаОбъект.КоннекторСтдJSON
Перем КоннекторHTTPСлужебный83Лок; // ОбработкаОбъект.КоннекторHTTPСлужебный83
Перем КоннекторHTTPДвоичныеДанные82СлужебныйЛок; // ОбработкаОбъект.КоннекторHTTPДвоичныеДанные82Служебный
Перем КоннекторHTTPХешMD5Лок; // ОбработкаОбъект.КоннекторHTTPХешMD5
Перем КоннекторСтдОбщегоНазначенияЛок; // ОбработкаОбъект.КоннекторСтдОбщегоНазначения
Перем КоннекторСтдОСЛок; // ОбработкаОбъект.КоннекторСтдОС
Перем КоннекторСтдКоллекцииЛок; // ОбработкаОбъект.КоннекторСтдКоллекции
...
Функция ПолучитьСтрокуИзДвоичныхДанныхЛок(ВходящиеДвоичныеДанные, Кодировка = Неопределено) Экспорт
	...
	ВерсияСовместимостиБольшеРавно8_3_6Лок() Тогда
		Возврат
		КоннекторHTTPСлужебный83Лок()
		.ПолучитьСтрокуИзДвоичныхДанныхЛок(ВходящиеДвоичныеДанные, Кодировка);
	Иначе
		Возврат
		КоннекторHTTPДвоичныеДанные82СлужебныйЛок()
		.ПолучитьСтрокуИзДвоичныхДанныхЛок(ВходящиеДвоичныеДанные, Кодировка);
	КонецЕсли;
КонецФункции
...
Функция ПроксиПоУмолчанию(URL)
	...
	Попытка
		Модуль =
		КоннекторСтдОбщегоНазначенияЛок()
		.ОбщийМодульЛок(
...
Функция КоннекторСтдСтрЛок()
	...
	Если КоннекторСтдСтрЛок = Неопределено Тогда
		...
		Если ПараметрыПроверки.ИспользуемоеИмяФайла = Неопределено Тогда
			КоннекторСтдСтрЛок = Новый("DataProcessorObject.КонтурЛогистика_СтдСтр");
		Иначе
			КоннекторСтдСтрЛок = Новый("ExternalDataProcessorObject.КонтурЛогистика_СтдСтр");
		КонецЕсли;
	КонецЕсли;

	Возврат КоннекторСтдСтрЛок;
КонецФункции
...
Функция КоннекторСтдJSONЛок()
	...
	Если КоннекторСтдJSONЛок = Неопределено Тогда
		ПараметрыПроверки = Новый Структура;
		ПараметрыПроверки.Вставить("ИспользуемоеИмяФайла");
		ЗаполнитьЗначенияСвойств(ПараметрыПроверки, ЭтотОбъект);
	
		Если ПараметрыПроверки.ИспользуемоеИмяФайла = Неопределено Тогда
			КоннекторСтдJSONЛок = Новый("DataProcessorObject.КонтурЛогистика_СтдJSON");
		Иначе
			КоннекторСтдJSONЛок = Новый("ExternalDataProcessorObject.КонтурЛогистика_СтдJSON");
		КонецЕсли;
	КонецЕсли;

	Возврат КоннекторСтдJSONЛок;
КонецФункции
...
Функция КоннекторHTTPСлужебный83Лок()
	...
	Если КоннекторHTTPСлужебный83Лок = Неопределено Тогда
		ПараметрыПроверки = Новый Структура;
		ПараметрыПроверки.Вставить("ИспользуемоеИмяФайла");
		ЗаполнитьЗначенияСвойств(ПараметрыПроверки, ЭтотОбъект);
	
		Если ПараметрыПроверки.ИспользуемоеИмяФайла = Неопределено Тогда
			КоннекторHTTPСлужебный83Лок = Новый("DataProcessorObject.КонтурЛогистика_КоннекторHTTPСлужебный83");
		Иначе
			КоннекторHTTPСлужебный83Лок = Новый("ExternalDataProcessorObject.КонтурЛогистика_КоннекторHTTPСлужебный83");
		КонецЕсли;
	КонецЕсли;

	Возврат КоннекторHTTPСлужебный83Лок;
КонецФункции
...
Функция КоннекторHTTPДвоичныеДанные82СлужебныйЛок()
	...
	Если КоннекторHTTPДвоичныеДанные82СлужебныйЛок = Неопределено Тогда
		ПараметрыПроверки = Новый Структура;
		ПараметрыПроверки.Вставить("ИспользуемоеИмяФайла");
		ЗаполнитьЗначенияСвойств(ПараметрыПроверки, ЭтотОбъект);
	
		Если ПараметрыПроверки.ИспользуемоеИмяФайла = Неопределено Тогда
			КоннекторHTTPДвоичныеДанные82СлужебныйЛок = Новый("DataProcessorObject.КонтурЛогистика_КоннекторHTTPДвоичныеДанные82Служебный");
		Иначе
			КоннекторHTTPДвоичныеДанные82СлужебныйЛок = Новый("ExternalDataProcessorObject.КонтурЛогистика_КоннекторHTTPДвоичныеДанные82Служебный");
		КонецЕсли;
	КонецЕсли;

	Возврат КоннекторHTTPДвоичныеДанные82СлужебныйЛок;
КонецФункции
...
Функция КоннекторHTTPХешMD5Лок()
	...
	Если КоннекторHTTPХешMD5Лок = Неопределено Тогда
		ПараметрыПроверки = Новый Структура;
		ПараметрыПроверки.Вставить("ИспользуемоеИмяФайла");
		ЗаполнитьЗначенияСвойств(ПараметрыПроверки, ЭтотОбъект);
	
		Если ПараметрыПроверки.ИспользуемоеИмяФайла = Неопределено Тогда
			КоннекторHTTPХешMD5Лок = Новый("DataProcessorObject.КонтурЛогистика_КоннекторHTTPХешMD5");
		Иначе
			КоннекторHTTPХешMD5Лок = Новый("ExternalDataProcessorObject.КонтурЛогистика_КоннекторHTTPХешMD5");
		КонецЕсли;
	КонецЕсли;

	Возврат КоннекторHTTPХешMD5Лок;
КонецФункции
...
Функция КоннекторСтдОбщегоНазначенияЛок()
	...
	Если КоннекторСтдОбщегоНазначенияЛок = Неопределено Тогда
		ПараметрыПроверки = Новый Структура;
		ПараметрыПроверки.Вставить("ИспользуемоеИмяФайла");
		ЗаполнитьЗначенияСвойств(ПараметрыПроверки, ЭтотОбъект);
	
		Если ПараметрыПроверки.ИспользуемоеИмяФайла = Неопределено Тогда
			КоннекторСтдОбщегоНазначенияЛок = Новый("DataProcessorObject.КонтурЛогистика_СтдОбщегоНазначения");
		Иначе
			КоннекторСтдОбщегоНазначенияЛок = Новый("ExternalDataProcessorObject.КонтурЛогистика_СтдОбщегоНазначения");
		КонецЕсли;
	КонецЕсли;

	Возврат КоннекторСтдОбщегоНазначенияЛок;
КонецФункции
...
Функция КоннекторСтдОСЛок()
	...
	Если КоннекторСтдОСЛок = Неопределено Тогда
		ПараметрыПроверки = Новый Структура;
		ПараметрыПроверки.Вставить("ИспользуемоеИмяФайла");
		ЗаполнитьЗначенияСвойств(ПараметрыПроверки, ЭтотОбъект);
	
		Если ПараметрыПроверки.ИспользуемоеИмяФайла = Неопределено Тогда
			КоннекторСтдОСЛок = Новый("DataProcessorObject.КонтурЛогистика_СтдОС");
		Иначе
			КоннекторСтдОСЛок = Новый("ExternalDataProcessorObject.КонтурЛогистика_СтдОС");
		КонецЕсли;
	КонецЕсли;

	Возврат КоннекторСтдОСЛок;
КонецФункции
...
Функция КоннекторСтдКоллекцииЛок()
	...
	Если КоннекторСтдКоллекцииЛок = Неопределено Тогда
		ПараметрыПроверки = Новый Структура;
		ПараметрыПроверки.Вставить("ИспользуемоеИмяФайла");
		ЗаполнитьЗначенияСвойств(ПараметрыПроверки, ЭтотОбъект);
	
		Если ПараметрыПроверки.ИспользуемоеИмяФайла = Неопределено Тогда
			КоннекторСтдКоллекцииЛок = Новый("DataProcessorObject.КонтурЛогистика_СтдКоллекции");
		Иначе
			КоннекторСтдКоллекцииЛок = Новый("ExternalDataProcessorObject.КонтурЛогистика_СтдКоллекции");
		КонецЕсли;
	КонецЕсли;

	Возврат КоннекторСтдКоллекцииЛок;
КонецФункции
...
Функция ВерсияСовместимостиБольшеРавно8_3_6Лок() Экспорт
	Если ВерсияСовместимостиБольшеРавно8_3_6Лок = Неопределено Тогда
		ВерсияСовместимостиБольшеРавно8_3_6Лок = ВерсияСовместимостиБольшеРавноЛок("8.3.6");
	КонецЕсли;

	Возврат ВерсияСовместимостиБольшеРавно8_3_6Лок;
КонецФункции
...
Функция ВерсияСовместимостиБольшеРавноЛок(ВерсияСтрокой) Экспорт
		...
		Если Метаданные.РежимСовместимости = Неопределено Или Метаданные.РежимСовместимости
			...
			ВерсияСовместимостиБольшеРавно = КоннекторСтдСтрЛок().СравнитьВерсииЛок(СистемнаяИнфо.ВерсияПриложения, ВерсияСтрокой) >= 0;
		Иначе
			...
			ВерсияСовместимостиБольшеРавно = КоннекторСтдСтрЛок().СравнитьВерсииЛок(РежимСовместимости, ВерсияСтрокой) >= 0;
		КонецЕсли;
