...
Функция ПараметрыИнициализацииМодуля()
	...
	Результат.Вставить("МаркерКонфигурации", Неопределено);
	Результат.Вставить("ТочностьЦены", ТочностьЦеныПрочитать());
	
	Возврат Результат;
	
КонецФункции

Функция НовыйПараметрыОкружения()
	
	Параметры = Новый Структура;
	Параметры.Вставить("АдресСервисаДиадок", АдресСервисаПоУмолчанию());
	Параметры.Вставить("КлючРазработчика", КлючРазработчикаПоУмолчанию());
	Параметры.Вставить("АдресDiadocAPI", АдресАпиПоУмолчанию());
	
	Параметры.Вставить("АдресСервисаОбновлений", АдресСервисаОбновленийПоУмолчанию());
	
	Параметры.Вставить("ПутьКФайлуКомпоненты", Неопределено);
...
Процедура ПроверитьЗаполнитьМаркерКонфигурации(ОбщиеПараметры)
	
	Если Не ЗначениеЗаполнено(ОбщиеПараметры.МаркерКонфигурации) Тогда
		ОбщиеПараметры.МаркерКонфигурации = ПолучитьМаркерКонфигурации();
	КонецЕсли;
...
Функция ИнициализироватьОбщийКонтекстКлиентСервер(УИД = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ПроверитьЗаполнитьМаркерКонфигурации(ПараметрыИнициализации);
	
	МетаданныеОбработки = Метаданные();
	
	ПараметрыОкружения = НовыйПараметрыОкружения();
	
	ПараметрыКлиентСервер = Новый Структура;
	
	ПараметрыКлиентСервер.Вставить("ПараметрыОкружения", ПараметрыОкружения);
	...
	ПараметрыКлиентСервер.Вставить("ПолноеИмяОбработки"		, МетаданныеОбработки.ПолноеИмя());
	...
	ПараметрыКлиентСервер.Вставить("ВерсияОбработки", ВерсияОбработки);
	
	ВерсияОбработкиБезНомераСборки = ВерсияОбработкиБезНомераСборки();
	ПараметрыКлиентСервер.Вставить("ВерсияОбработкиБезНомераСборки", ВерсияОбработкиБезНомераСборки);
	
	СохраненнаяВерсияМодуля = ЗначениеНастройкиСохраненнаяВерсияМодуля();
	ПараметрыКлиентСервер.Вставить("СохраненнаяВерсияМодуля", СохраненнаяВерсияМодуля);
	
	ПараметрыКлиентСервер.Вставить("ИдентификаторБазы", ИдентификаторБазы());
...
Функция СоздатьМодуль(ИмяМодуля)
	...
	КаталогОтладки = ПараметрыКлиентСервер.КаталогМодулейСервера;
	
	Если Не ЭтоВнешняяОбработка() Тогда
		
		ПрефиксОбработокРасширения = "КонтурДиадокСтандартУФ_";
		Результат = Обработки[ПрефиксОбработокРасширения + ИмяМодуля].Создать();
		
	ИначеЕсли ПараметрыКлиентСервер.РежимОтладкиСервера 
		И ЗначениеЗаполнено(КаталогОтладки) Тогда
		
		ИмяФайла		 = ИмяМодуля + ".epf";
		ПолноеИмяФайла	 = ОбъединитьПути(КаталогОтладки, ИмяФайла);
...
Функция МассивПараметров(Параметр0, Параметр1, Параметр2, Параметр3, Параметр4, 
	...
	Результат[5]= Параметр5;
	Результат[6]= Параметр6;
	Результат[7]= Параметр7;
	Результат[8]= Параметр8;
	Результат[9]= Параметр9;
...
Функция ПроверитьЧтоБезопасныйРежимОтключен()
	...
	ИспользуетсяПрофильБезопасности = ИспользуютсяПрофилиБезопасностиИнформационнойБазы();
	БезопасныйРежим 				= БезопасныйРежим();
	
	Если НЕ ИспользуетсяПрофильБезопасности И БезопасныйРежим <> Ложь Тогда
...
Функция ИспользуютсяПрофилиБезопасностиИнформационнойБазы()
	
	Результат = Ложь;
	
	Попытка
		Результат = ПолучитьФункциональнуюОпцию("ИспользуютсяПрофилиБезопасности");
	Исключение
	КонецПопытки;
...
Функция ФункцииМодуля()
	...
	ДобавитьФункциюВМанифест(Результат, "ЕстьРеквизитИлиСвойствоОбъектаЛок");
	...
	ДобавитьФункциюВМанифест(Результат, "ПолучитьСписокАссоциированныхОрганизацийДиадок");
	ДобавитьФункциюВМанифест(Результат, "ПолучитьСписокBoxIdАссоциированныхОрганизацийДиадок");
	...
	ДобавитьФункциюВМанифест(Результат, "НазначениеСвойстваСтрокой");
	...
	ДобавитьФункциюВМанифест(Результат, "КлючиНастроекОтправкиКонтрагентов");
	ДобавитьФункциюВМанифест(Результат, "ЗначениеНастройкиСохраненнаяВерсияМодуля");
	...
	ДобавитьФункциюВМанифест(Результат, "ПрочитатьСопоставлениеНоменклатуры");
	
	ДобавитьФункциюВМанифест(Результат, "НастройкиОтправкиДокументов");
	ДобавитьФункциюВМанифест(Результат, "ОрганизацииИКонтрагентыСУтаревшимиНастройкамиОтправки");
	ДобавитьФункциюВМанифест(Результат, "ЭтоУстаревшаяНастройкаОтправки");
	ДобавитьФункциюВМанифест(Результат, "ВариантыСопоставленияНоменклатуры");
	
	// Машиночитаемые доверенности
	ДобавитьФункциюВМанифест(Результат, "МЧД_ИмяСправочникаВИнформационнойБазе");
	ДобавитьФункциюВМанифест(Результат, "МЧД_ДанныеДоверенностиИнформационнойБазы");
	ДобавитьФункциюВМанифест(Результат, "МЧД_ЕстьСправочникМашиночитаемыхДоверенностей");
	ДобавитьФункциюВМанифест(Результат, "УровниКритичностиСтатусаДокумента");
...
Функция КонтентФормализованногоДокумента(СсылкаНаОбъект, ФорматОтправки, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ПроверитьЗаполнитьПараметрыФормированияXML(ПараметрыПолученияКонтента);
	...
	РежимСовместимости155и820 = ИспользоватьРежимСовместимостиФорматов155и820();
	
	Если Это820 И РежимСовместимости155и820 Тогда
		...
		МенеджерИнтеграции.ЗаполнитьСведенияОДокументахПодтверждающихОтгрузку820(Результат, ДокументыОснования);
...
//
Процедура ПодобратьНоменклатуруИнформационнойБазы(НоменклатураКонтрагента, НоменклатураИБ) Экспорт
	
	МенеджерИнтеграции = МодульМенеджерИнтеграции();
	
	МенеджерИнтеграции.ПодобратьНоменклатуруИнформационнойБазы(НоменклатураКонтрагента, НоменклатураИБ);
...
Функция МЧД_ДанныеДоверенностиИнформационнойБазы(Ссылка) Экспорт
	...
	МенеджерИнтеграции = МодульМенеджерИнтеграции();
	Результат = МенеджерИнтеграции.МЧД_ДанныеДоверенностиИнформационнойБазы(Ссылка);
...
Функция УровниКритичностиСтатусаДокумента() Экспорт
...
Процедура ПроверитьЗаполнитьПараметрыФормированияXML(ПараметрыПолученияКонтента)
	
	Если ЗначениеЗаполнено(ПараметрыПолученияКонтента.ПараметрыФормированияФайла) Тогда
		Возврат;
	КонецЕсли;
	
	ГенерацияXML = МодульГенерацияXML();
	ПараметрыПолученияКонтента.ПараметрыФормированияФайла = ГенерацияXML.XML_ПараметрыФормированияФайла();
	ПараметрыПолученияКонтента.ПараметрыФормированияФайла.ТочностьЦены = ТочностьЦеныТекущееЗначение();
	
КонецПроцедуры

Функция ИспользоватьРежимСовместимостиФорматов155и820()
	
	Результат = ПараметрыКлиентСервер.ПодключаемыйМодуль.РежимСовместимостиФорматов155и820;
	Возврат Результат;
...
//
Процедура ПМ_ПослеЗаполненияКонтента(Контент, ТипКонтента, СсылкаНаОбъект, ПараметрыФормирования) Экспорт
...
Функция ИдентификаторНастройкиРежимСовместимостиФорматов155и820()
	Возврат "РежимСовместимостиФорматов155и820";
КонецФункции

Функция ЗначениеНастройкиРежимСовместимостиФорматов155и820()
	
	СохраненноеЗначение = Неопределено;
	
	ВладелецОбщихНастроекМодуля = МетодСервера(, "ОбъектКонстант");
	ИдентификаторНастройки = ИдентификаторНастройкиРежимСовместимостиФорматов155и820();
...
Функция ЗначениеНастройкиСохраненнаяВерсияМодуля() Экспорт
	
	ИдентификаторНастройки = ИдентификаторСвойстваСохраненнаяВерсияМодуля();
	
	Результат = ПолучитьЗначениеКонстанты(ИдентификаторНастройки);
...
Функция МаркерКонфигурации()
	...
	Если Результат = "Розница" Тогда
		
		ВерсияКонфигурации = Метаданные.Версия;
		
		Если СравнитьВерсии(ВерсияКонфигурации, "3.0") >= 0 Тогда 
			Результат = "УНФ16";
		КонецЕсли;
...
Функция ДопустимыеМаркерыКонфигураций() Экспорт
	...
	СписокМаркеров.Добавить("АльфаАвто6"	, "Альфа-Авто 6");
...
Функция ВерсияОбработкиБезНомераСборки()
...
Функция КлючРазработчика() Экспорт
	...
	Возврат ПараметрыКлиентСервер.ПараметрыОкружения.КлючРазработчика;
	
КонецФункции

Функция АдресСервиса() Экспорт 
	
	Возврат ПараметрыКлиентСервер.ПараметрыОкружения.АдресСервисаДиадок;
	
КонецФункции

Функция АдресАпи() Экспорт 
	
	Возврат ПараметрыКлиентСервер.ПараметрыОкружения.АдресDiadocAPI;
...
Функция КлючРазработчикаПоУмолчанию()
	Возврат "1S83-5-57-1a80dffc-da54-484c-b318-840f4b521c49";
КонецФункции
...
Функция КонтекстКлиентСервер_ПодключаемыйМодуль()
	...
	РежимСовместимости155и820 = Ложь;
	...
	Если ЗначениеЗаполнено(ВладелецОбщихНастроекМодуля) Тогда
		...
		РежимСовместимости155и820 = ЗначениеНастройкиРежимСовместимостиФорматов155и820();
	...
	Результат.Вставить("РежимСовместимостиФорматов155и820", РежимСовместимости155и820);
...
Функция ВыбратьОсновнойДоговорКонтрагента(Контрагент, Организация, СписокВидовДоговоров) Экспорт
	...
	ИначеЕсли ПараметрыКлиентСервер.МаркерКонфигурации = "БП30" Тогда
		...
		//начиная с 3.0.43.29 реквизит удален, вместо него ввели регистр сведений
...
Функция ПолучитьСписокАссоциированныхОрганизацийДиадок() Экспорт
	
	ТаблицаЯщиков = ПолучитьТаблицуЯщиковДиадокОрганизации();
	
	ОрганизацииИБ = Новый Массив;
	Для каждого СтрокаЯщика Из ТаблицаЯщиков Цикл
		ОрганизацииИБ.Добавить(СтрокаЯщика.Организация);
	КонецЦикла; 
	
	Возврат ОрганизацииИБ;
	
КонецФункции

Функция ПолучитьСписокBoxIdАссоциированныхОрганизацийДиадок() Экспорт
	
	Результат = Новый Массив;
	
	ТаблицаЯщиков = ПолучитьТаблицуЯщиковДиадокОрганизации();
	
	Для Каждого Строка Из ТаблицаЯщиков Цикл
		Результат.Добавить(Строка.BoxId);
	КонецЦикла; 
...
//		Структура
//
Функция СформироватьПечатнуюФормуАктаСверки(ДокументСверкиСсылка, ДополнительныеПараметры) Экспорт
	
	МенеджерИнтеграции = МодульМенеджерИнтеграции();
	
	Результат = МенеджерИнтеграции.РезультатВизуализацииАктаСверки( 
	ДокументСверкиСсылка, 
	ДополнительныеПараметры.ПараметрыСогласования,
	ДополнительныеПараметры.ФИОПодписанта
	);
...
//		Структура
//
Функция СформироватьПечатнуюФормуСчетаНаОплату(ДокументСчетаНаОплатуСсылка, ДополнительныеПараметры) Экспорт
	
	МенеджерИнтеграции = МодульМенеджерИнтеграции();
	
	Результат = МенеджерИнтеграции.РезультатВизуализацииСчетаНаОплату( 
	ДокументСчетаНаОплатуСсылка, 
	ДополнительныеПараметры.ПараметрыСогласования,
	ДополнительныеПараметры.ФИОПодписанта
	);
...
// Актуальные настройки отправки документов
//
// Возвращаемое значение:
//  Массив - коллекция фиксированных структур:
//		* Ключ - Строка - ключ настройки
//		* Представление - Строка - представление настройки отправки документов
//		* Устарела - Булево - Истина если настройка отправки документов устарела
Функция НастройкиОтправкиДокументов() Экспорт
...
// Возвращает настройки отправки указанных контрагентов.
//
// Параметры:
//  СписокКонтрагентов - Массив - содержит ссылки на элементы справочника "Контрагенты".
// 
// Возвращаемое значение:
//  Соответствие:
//    * Ключ - СправочникСсылка - ссылка на организацию
//    * Значение - Строка - значение настройки отправки документов
//
Функция КлючиНастроекОтправкиКонтрагентов(СписокКонтрагентов) Экспорт
	
	ИмяСвойства = ИдентификаторСвойстваФормироватьУПДКонтрагент();
	Результат = ПолучитьЗначенияСвойстваОбъектов(СписокКонтрагентов, ИмяСвойства);
...
Процедура УстановитьНастройкуОтправкиДокументовДляКонтрагента(КонтрагентИБ, ЗначениеНастройки) Экспорт
	
	ИмяСвойства = ИдентификаторСвойстваФормироватьУПДКонтрагент();
	
	УстановитьЗначениеСвойства(КонтрагентИБ, ИмяСвойства, , ЗначениеНастройки);
...
//   * ЕстьУстаревшее - Булево - Истина если найдены контрагенты или организации с устаревшими настройками отправки
//   * ОрганизацииИБ     - Массив - организации ИБ с устаревшими настройками отправки;
//   * КонтрагентыИБ     - Массив - контрагенты ИБ с устаревшими настройками отправки;
Функция ОрганизацииИКонтрагентыСУтаревшимиНастройкамиОтправки() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ЕстьУстаревшее",	Ложь);
	Результат.Вставить("ОрганизацииИБ",		Новый Массив);
	Результат.Вставить("КонтрагентыИБ",		Новый Массив);
	
	ОписаниеНастроекОтправки = НастройкиОтправкиДокументов();
	
	ОрганизацииИБ = ПолучитьСписокАссоциированныхОрганизацийДиадок();
	НастройкиОтправки = КлючиНастроекОтправки(ОрганизацииИБ);
	
	Для Каждого Элемент Из НастройкиОтправки Цикл
		
		КлючНастройки = Элемент.Значение;
		
		Если ЭтоУстаревшаяНастройкаОтправки(КлючНастройки, ОписаниеНастроекОтправки) Тогда
			Результат.ОрганизацииИБ.Добавить(Элемент.Ключ);
		КонецЕсли;
		
	КонецЦикла;	
	
	КонтрагентыИБ = ПолучитьСписокАссоциированныхКонтрагентовДиадок();
	НастройкиОтправкиПоКонтрагентам = КлючиНастроекОтправкиКонтрагентов(КонтрагентыИБ);
	
	Для Каждого Элемент Из НастройкиОтправкиПоКонтрагентам Цикл
		
		КлючНастройки = Элемент.Значение;
		
		Если ЗначениеЗаполнено(КлючНастройки) И ЭтоУстаревшаяНастройкаОтправки(КлючНастройки, ОписаниеНастроекОтправки) Тогда
			Результат.КонтрагентыИБ.Добавить(Элемент.Ключ);
		КонецЕсли;
		
	КонецЦикла;	
	
	Результат.ЕстьУстаревшее = ЗначениеЗаполнено(Результат.ОрганизацииИБ)
	ИЛИ  ЗначениеЗаполнено(Результат.КонтрагентыИБ);
	
	Возврат Результат;
	
КонецФункции // ОрганизацииИКонтрагентыСУтаревшимиНастройкамиОтправки()

// Проверяет актуальность настройки отправки документов.
//
// Параметры:
//  КлючНастройки - Строка - значение настройки "ДиадокФормироватьУПД"
//  ОписаниеНастроекОтправки - Массив - см. метод сервера НастройкиОтправкиДокументов()
//  						 - Неопределено - настройки берутся из метода НастройкиОтправкиДокументов()
// 
// Возвращаемое значение:
//  Булево - Истина, если эта настройка позволяет отправлять документы в неактуальных форматах.
//
Функция ЭтоУстаревшаяНастройкаОтправки(КлючНастройки, ОписаниеНастроекОтправки = Неопределено) Экспорт
	
	Результат = Ложь;
	
	Если ОписаниеНастроекОтправки = Неопределено Тогда
		
		ОписаниеНастроекОтправки = НастройкиОтправкиДокументов();
		
	КонецЕсли; 
	
	Для Каждого ОписаниеНастройки Из ОписаниеНастроекОтправки Цикл
...
// Устанавливает рекомендуемые настройки отправки УПД для переданных организаций и контрагентов 
//
// Параметры:
//  ОрганизацииИБ - Массив - содержит ссылки на элементы справочника "Организации".
//  КонтрагентыИБ - Массив - содержит ссылки на элементы справочника "Контрагенты".
Процедура ВыполнитьУстановкуРекомендуемыхНастроекОтправки(ОрганизацииИБ, КонтрагентыИБ) Экспорт
	
	ВыполнитьУстановкуРекомендуемыхНастроекОтправкиДляОрганизаций(ОрганизацииИБ);
	
	ВыполнитьУстановкуРекомендуемыхНастроекОтправкиДляКонтрагентов(КонтрагентыИБ);
	
КонецПроцедуры

Процедура ВыполнитьУстановкуРекомендуемыхНастроекОтправкиДляОрганизаций(ОрганизацииИБ)
	
	НастройкиОрганизаций = КлючиНастроекОтправки(ОрганизацииИБ);
...
Процедура ВыполнитьУстановкуРекомендуемыхНастроекОтправкиДляКонтрагентов(КонтрагентыИБ)
	
	НастройкиКонтрагентов = КлючиНастроекОтправкиКонтрагентов(КонтрагентыИБ);
	ПравилаЗамены = ПравилаОбновленияНастроек();
	
	Для Каждого Элемент Из НастройкиКонтрагентов Цикл
		
		ТекущийКонтрагент = Элемент.Ключ;
		ТекущаяНастройка = Элемент.Значение;
		...
		Если ЗначениеЗаполнено(ТекущаяНастройка) Тогда
			...
			Если НоваяНастройка <> Неопределено Тогда
				МетодСервера(, "УстановитьНастройкуОтправкиДокументовДляКонтрагента"
				, ТекущийКонтрагент
				, НоваяНастройка);
			КонецЕсли;
		КонецЕсли; 
...
// Рекомендуемые настройки отправки УПД для переданных организаций и контрагентов с учетом устаревших форматов
//
// Возвращаемое значение:
//  Соответствие:
//    * Ключ - Строка - старый формат отправки
//    * Значение - Строка - новый формат отправки
//	
Функция ПравилаОбновленияНастроек()
	
	Результат = Новый Соответствие;
	
	Результат.Вставить("СЧФДОП_820"		, "СЧФДОП_820_736");
	Результат.Вставить("СЧФДОП"			, "СЧФДОП_820_736");
	Результат.Вставить("СЧФ_ДОП_820"	, "СЧФ_ДОП_820_736");				
	Результат.Вставить("СЧФ_ДОП"		, "СЧФ_ДОП_820_736");
	Результат.Вставить("СЧФ"			, "СЧФ_ДОП_820_736");
	Результат.Вставить("СФТОРГ12АКТ_820", "СФТОРГ12АКТ_820_736");	
	Результат.Вставить("СФТОРГ12АКТ"	, "СФТОРГ12АКТ_820_736");
	Результат.Вставить("СФ"				, "СФТОРГ12АКТ_820_736");
	Результат.Вставить("ДОП"			, "СФТОРГ12АКТ_820_736");
	Результат.Вставить("ТОРГ12АКТ"		, "СФТОРГ12АКТ_820_736");
	Результат.Вставить("НЕТ"			, "СФТОРГ12АКТ_820_736");
...
Процедура СохранитьНастройкиПрокси(НастройкиПрокси) Экспорт
	...
	Если ВариантИспользования = "ИспользоватьПроксиСервер" Тогда
		...
		АдресПорт = нРег(НастройкиПрокси.АдресПорт);
		АдресПорт = СтрЗаменить(АдресПорт,"http://","");
		АдресПорт = СтрЗаменить(АдресПорт,"https://","");
	КонецЕсли;
...
Функция НазначениеСвойстваСтрокой(СвойствоСтрокой)
...
Функция СвойстваОбъектов_НайтиОбъект(НаименованиеСвойства,
	...
	Если ЗначениеЗаполнено(НаименованиеСвойства2) Тогда 
		...
		НазначениеСвойстваСтрокой(НаименованиеСвойства),
		ЗначениеСвойства,
		НаименованиеСвойства2,
		НазначениеСвойстваСтрокой(НаименованиеСвойства2),
		ЗначениеСвойства2);
	Иначе
...
Функция СвойстваОбъектов_НайтиМассивОбъектов(НаименованиеСвойства, ЗначениеСвойства,
	НаименованиеСвойства2 = Неопределено, ЗначениеСвойства2 = Неопределено) Экспорт
	
	Если ЗначениеЗаполнено(НаименованиеСвойства2) Тогда
		Возврат НайтиМассивОбъектов(НаименованиеСвойства, НазначениеСвойстваСтрокой(НаименованиеСвойства),
			ЗначениеСвойства, НаименованиеСвойства2, НазначениеСвойстваСтрокой(НаименованиеСвойства2), ЗначениеСвойства2);
	Иначе 
		Возврат НайтиМассивОбъектов(НаименованиеСвойства, НазначениеСвойстваСтрокой(НаименованиеСвойства), ЗначениеСвойства);
	КонецЕсли;
...
Процедура ОчиститьНаборУСвойства(СвойствоСсылка, ЗафиксированаРеструктуризация)
	...
	Если ЗначениеЗаполнено(СвойствоСсылка)
		И ЕстьРеквизитИлиСвойствоОбъектаЛок(СвойствоСсылка, "НаборСвойств", НаборСвойств)
...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Информация"	   , ДополнительноеОписаниеМодуля());
...
Функция ДополнительноеОписаниеМодуля()
	Возврат "Модуль работы с электронными первичными документами через Диадок";
КонецФункции
...
Функция КомандыВнешнейОбработки()
	...
	// при добавлении новых / изменении текущих команд стоит учеть сценарий
	// по добавлению / обновлению команд при автоматическом обновлении модуля,
	// которые выполняются в методе ОбновитьМодульВСправочникеДополнительныхОбработок()
	
	Результат = НоваяТаблицаКомандВнешнейОбработки();
	
	НоваяКоманда = Результат.Добавить();
	НоваяКоманда.Использование = "ОткрытиеФормы";
	НоваяКоманда.Представление = ПредставлениеМодуля();
	НоваяКоманда.Идентификатор = "ОткрытьДиадокСтандарт83";
...
Функция ИдентификаторСвойстваСохраненнаяВерсияМодуля() Экспорт
	
	Возврат "ДиадокСохраненнаяВерсияМодуля";
...
Функция ПолучитьГуидИзЧисла16(Число16)
	...
	Возврат Сред(Число16, 1, 8) + "-" + Сред(Число16, 9, 4) + "-" + Сред(Число16, 13, 4) +
...
Функция БазовыеФорматы() Экспорт
	...
	// универсальный передаточный документ в формате 970@
	Результат.Вставить("utd970", "utd970");
	...
	// формализованный акт сверки 405@
	Результат.Вставить("reconciliationact", "reconciliationact");
...
Функция ТипыКонтента() Экспорт
    ...
    // УПД в формате 820@
	Результат.Вставить("Utd970SellerContent", "Utd970SellerContent");
	...
	// Акт сверки в формате 405@
	Результат.Вставить("ReconciliationActSenderTitle", "ReconciliationActSenderTitle");
...
//
// Параметры:
//  ТипКонтента - Строка - Тип контента
// 
// Возвращаемое значение:
...
// Получает значение настройки "Точность цены" из текущих параметров модуля.
// 
// Возвращаемое значение:
//  Число.
//
Функция ТочностьЦеныТекущееЗначение()
	
	Результат = ПараметрыКлиентСервер.ТочностьЦены;
	Возврат Результат;
	
КонецФункции

// Получает сохраненное значение настройки "Точность цены".
// 
// Возвращаемое значение:
//  Число - значение настройки (по умолчанию 2).
//
Функция ТочностьЦеныПрочитать()
	
	КлючОбъекта = КлючОбъектаОбщихНастроек();
	КлючНастройки = КлючНастройкиТочностьЦены();
	
	Результат = ХранилищеОбщихНастроек.Загрузить(КлючОбъекта, КлючНастройки);
...
// Сохраняет значение настройки "Точность цены".
//
// Параметры:
//  Точность - Число - значение настройки.
//
Процедура ТочностьЦеныЗаписать(Точность) Экспорт
	
	КлючОбъекта = КлючОбъектаОбщихНастроек();
	КлючНастройки = КлючНастройкиТочностьЦены();
	ХранилищеОбщихНастроек.Сохранить(КлючОбъекта, КлючНастройки, Точность);
	
КонецПроцедуры

Функция КлючОбъектаОбщихНастроек()
	Возврат "НастройкаОпций_Диадок";
КонецФункции

Функция КлючНастройкиТочностьЦены()
	Возврат "ТочностьЦеныСФ_Диадок";
КонецФункции
