...
Процедура ОбработатьРежимФормы()
	...
	ИначеЕсли Режим = РежимОтказатьВПриглашении() Тогда
		...
		Элементы.ПолеВвода.АвтоОтметкаНезаполненного = НЕ ВозможноПустоеЗначение;
		Элементы.ТекстСообщения.Заголовок = ЗаголовокСообщения;
	ИначеЕсли Режим = РежимНаписатьКонтрагенту() Тогда
		Заголовок = "Написать контрагенту";
		УстановитьЗаголовокКнопкиВыполнить("Отправить");
		ЗаголовокСообщения = НСтр("ru='Отправка информационного сообщения'");
		ВозможноПустоеЗначение = Ложь;
		Элементы.ПолеВвода.АвтоОтметкаНезаполненного = НЕ ВозможноПустоеЗначение;
		Элементы.ТекстСообщения.Заголовок = ЗаголовокСообщения;
		ДлинаСтроки = 10000;
		КвалификаторСтроки10000 = Новый КвалификаторыСтроки(ДлинаСтроки, ДопустимаяДлина.Переменная);
		ОписаниеТипаСтрока10000 = Новый ОписаниеТипов("Строка", , , , КвалификаторСтроки10000);
		Элементы.ПолеВвода.ОграничениеТипа = ОписаниеТипаСтрока10000;
	Иначе
...
&НаКлиентеНаСервереБезКонтекста
Функция РежимНаписатьКонтрагенту()
	
	Возврат "НаписатьКонтрагенту";
