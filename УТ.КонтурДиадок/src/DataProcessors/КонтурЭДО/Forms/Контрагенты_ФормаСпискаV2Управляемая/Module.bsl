...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если ОповеститьОЗакрытии = Истина И ВладелецФормы <> Неопределено Тогда
		Оповестить("Диадок_ЗакрытиеФормыСпискаКонтрагентов", , ОсновнаяФорма().ИдентификаторОсновнойФормы());
	КонецЕсли;
...
Процедура ПоказатьЗаглушкуДлительнойОперацииНаСервере()
	...
	Элементы.ГруппаСписокСтраницы.ТекущаяСтраница = Элементы.СтраницаЗагрузкаСписка;
...
Процедура ОтключитьЗаглушкуДлительнойОперации()
	...
	Элементы.ГруппаСписокСтраницы.ТекущаяСтраница = Элементы.СтраницаСписок;
...
Процедура ИнициализироватьПараметрыФормы(Параметры)
	...
	Параметры.Свойство("ОповеститьОЗакрытии", ОповеститьОЗакрытии);
...
Процедура УстановитьУсловноеОформление()
	...
	УстановитьУсловноеОформлениеСписка_ТипКонтрагента();
	
КонецПроцедуры

&НаСервере
Процедура УстановитьУсловноеОформлениеСписка_ТипКонтрагента()
	
	ПредставленияТипов = ПредставленияТиповОрганизаций();
	
	Для Каждого ЭлементСписка Из ПредставленияТипов Цикл
		
		ЭлементУО = УсловноеОформление.Элементы.Добавить();
		ЭлементОтбора = ЭлементУО.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СправочникСписок.Тип");
		ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ЭлементОтбора.ПравоеЗначение = ЭлементСписка.Значение;
		
		ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст", ЭлементСписка.Представление);
		
		ОформляемоеПоле = ЭлементУО.Поля.Элементы.Добавить();
		ОформляемоеПоле.Поле = Новый ПолеКомпоновкиДанных("СправочникСписокТип");
...
Процедура ИнициализироватьСхемыКомпоновкиДанных()
	...
	ДобавитьВычисляемоеПолеОбработкаПриглашений(Результат);
...
Процедура ДобавитьВычисляемоеПолеОбработкаПриглашений(СхемаКомпоновкиДанных)
	...
	ИмяКолонки = "ОбработкаПриглашений";
	КолонкиСписка = ПолучитьКолонкиТаблицы("СправочникСписок");

	Для Каждого ЭлементКолонки Из КолонкиСписка Цикл
		Если ЭлементКолонки.Имя = ИмяКолонки Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;

	ВычисляемоеПоле = СхемаКомпоновкиДанных.ВычисляемыеПоля.Добавить();
	ВычисляемоеПоле.ПутьКДанным = ИмяКолонки;
	ВычисляемоеПоле.Заголовок = ЭлементКолонки.Заголовок;
	ВычисляемоеПоле.ТипЗначения = Новый ОписаниеТипов(
		ЭлементКолонки.ТипЗначения);
	ВычисляемоеПоле.Выражение = ВыражениеПоля;
...
Процедура ДобавитьПолеКомпоновкиДанных(НаборДанных, Колонка)
	...
	Если Колонка.Имя = "Тип" Тогда
		НовоеПоле.УстановитьДоступныеЗначения(ПредставленияТиповОрганизаций());
	КонецЕсли;
	
	Если Колонка.Имя = "ОбработкаПриглашений" Тогда
		НовоеПоле.УстановитьДоступныеЗначения(ПредставленияТиповПриглашений());
	КонецЕсли;
...
Процедура ИнициализироватьКомпоновщикНастроекСКД()
	...
	ДоступныеПоля = ДоступныеПоляРасширенногоФильтра(ТекущийРежимСписка);
	
	ДоступныеПоляОтбора = Настройки.ДоступныеПоляОтбора;
	ОбработкаПриглашенийПолеОтбора = ДоступныеПоляОтбора.Элементы.Найти("ОбработкаПриглашений");
	ОбработкаПриглашенийПолеОтбора.ДоступныеЗначения = ПредставленияТиповПриглашений();
...
&НаКлиентеНаСервереБезКонтекста
Функция ДоступныеПоляРасширенногоФильтра(Знач РежимСписка)
	...
	НовоеПоле = Контракт_ПолеФильтра();
	НовоеПоле.Имя = "ОбработкаПриглашений";
	НовоеПоле.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	Результат.Добавить(НовоеПоле);
...
Функция СведенияОПриглашениях(ИдентификаторыСтрок)
	...
	Для Каждого Контрагент Из ВыбранныеКонтрагенты Цикл
		...
		Приглашение = МодульЯдро.Контрагенты_СведенияОПриглашении(BoxID, Контрагент.ID);
...
Процедура ОтправитьПриглашенияЗавершение(ДополнительныеПараметры)
	...
	Контрагенты_ОтправкаПриглашенийСообщитьОРезультатеПользователю(
		РезультатОтправкиПриглашений,
		ДополнительныеПараметры.ИдентификаторыСтрок);
		
	Если ЗначениеЗаполнено(РезультатОтправкиПриглашений.ПриглашенныеКонтрагенты) Тогда
		ОбработатьНеСопоставленныхСКонтрагентов1С(
			РезультатОтправкиПриглашений.ПриглашенныеКонтрагенты);
	КонецЕсли;
	
	ЗапрашиватьОтветнуюПодпись = Модуль_Ядро.СвойствоСтруктуры(
		ДополнительныеПараметры.ПодготовленноеПриглашение,
		"ЗапрашиватьОтветнуюПодпись",
		Ложь);
	
	ОбновитьКэшПослеОтправкиПриглашенийНаСервере(
		РезультатОтправкиПриглашений.ПриглашенныеКонтрагенты,
		ЗапрашиватьОтветнуюПодпись);
...
&НаСервере
Процедура ОбновитьКэшПослеОтправкиПриглашенийНаСервере(ДанныеКонтрагентов, ЗапрашиватьОтветнуюПодпись)
	
	Для Каждого Контрагент Из ДанныеКонтрагентов Цикл
		Если Контрагент.Экстраверт И Не ЗапрашиватьОтветнуюПодпись Тогда
			НовыйСтатус = Статус_МойКонтрагент();
		Иначе
			НовыйСтатус = Статус_ОжидаемОтветНаПриглашение();
		КонецЕсли;
		ОбновитьКэш(Контрагент.ID, НовыйСтатус);
	КонецЦикла;
...
Функция ОтправитьПриглашенияНаСервере(Знач ДополнительныеПараметры)
	...
	ДанныеКонтрагентов = ДанныеКонтрагентовИзИдентификаторовСтрок(
		ДополнительныеПараметры.ИдентификаторыСтрок);
	...
	Экстраверты = Новый Массив;
	Интроверты = Новый Массив;
	Для Каждого Контрагент Из Результат.ПриглашенныеКонтрагенты Цикл
		Если Контрагент.Экстраверт И Не ЗапрашиватьОтветнуюПодпись Тогда
			Экстраверты.Добавить(Контрагент);
		Иначе
			Интроверты.Добавить(Контрагент);
		КонецЕсли;
	КонецЦикла;
	...
	Модуль_Ядро.Контрагенты_ЗаписатьСтатусДляМассиваКонтрагентов(
		ДанныеОрганизации,
		Экстраверты,
		Перечисление_СтатусыКонтрагентов.ПартнерскиеОтношения);
	
	Модуль_Ядро.Контрагенты_ЗаписатьСтатусДляМассиваКонтрагентов(
		ДанныеОрганизации,
		Интроверты,
		Перечисление_СтатусыКонтрагентов.ОжидаетсяОтвет);
...
&НаКлиенте
// BSLLS:MissingParameterDescription-off
Процедура Контрагенты_ОтправкаПриглашенийСообщитьОРезультатеПользователю(
		РезультатОтправкиПриглашений,
		ИдентификаторыСтрок)
	// BSLLS:MissingParameterDescription-off
	...
	Если ЗначениеЗаполнено(РезультатОтправкиПриглашений) Тогда
		...
		Если ИдентификаторыСтрок.Количество() <> КонтрагентыДляОбработки.Количество() Тогда
	...
	Экстраверты = Новый Массив;
	Интроверты = Новый Массив;
	Для Каждого Контрагент Из РезультатОтправкиПриглашений.ПриглашенныеКонтрагенты Цикл
		Если Контрагент.Экстраверт Тогда
			Экстраверты.Добавить(Контрагент);
		Иначе
			Интроверты.Добавить(Контрагент);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(Экстраверты)
			И ЗначениеЗаполнено(Интроверты) Тогда
		
		ШаблонСообщенияПользователю = НСтр(
				"ru = 'Приглашения отправлены: %1
				| Контрагенты добавлены: %2.'");
		
		ТекстСообщенияПользователю = МодульЯдро.Общее_ПодставитьПараметрыВСтроку(
			ШаблонСообщенияПользователю,
			Интроверты.Количество(),
			Экстраверты.Количество());
		
	ИначеЕсли ЗначениеЗаполнено(Экстраверты) Тогда
		
		ШаблонСообщенияПользователю = НСтр(
				"ru = 'Контрагенты добавлены: %1.'");
	...
	ПоказатьОповещениеПользователя(ТекстСообщенияПользователю);
...
&НаСервере
Функция ПредставленияТиповОрганизаций()
	
	Ядро = Модуль_ЯдроНаСервере();
	Результат = Ядро.ТипыОрганизацийКакСписок();
...
&НаСервере
Функция ПредставленияТиповПриглашений()
	
	Результат = Новый СписокЗначений;
	Результат.Добавить("Автоматически", "Автоматически");
	Результат.Добавить("Контрагентом вручную", "Контрагентом вручную");
...
Процедура ПоказатьФормуПриглашения(Приглашение, РежимОткрытия, ИдентификаторыСтрок)
	...
	ПараметрыФормы.Вставить("ДанныеОрганизации", ДанныеОрганизации);
	ПараметрыФормы.Вставить("Партнеры", ДанныеКонтрагентовИзИдентификаторовСтрок(ИдентификаторыСтрок));
...
&НаСервере
Процедура Метрики_ЗаписатьСтатистику_ОтправитьПриглашение(ВсегоКА, ПриглашенныеКонтрагенты)
	...
	ТипыКА = Модуль_Ядро.Перечисление_ТипыОрганизаций();
	ОтправленоТипЮЛ = 0;
	ОтправленоТипФЛ = 0;
	ОтправленоТипИП = 0;
	ОтправленоТипИЛ = 0;
	ЯщикиПолучателей = Новый Массив;
	
	Для Каждого Запись Из ПриглашенныеКонтрагенты Цикл
		
		Если Запись.Тип = ТипыКА.ЮЛ Тогда
			ОтправленоТипЮЛ = ОтправленоТипЮЛ + 1;
		ИначеЕсли Запись.Тип = ТипыКА.ИП Тогда
			ОтправленоТипИП = ОтправленоТипИП + 1;
		ИначеЕсли Запись.Тип = ТипыКА.ФЛ Тогда
			ОтправленоТипФЛ = ОтправленоТипФЛ + 1;
		Иначе
			ОтправленоТипИЛ = ОтправленоТипИЛ + 1;
		КонецЕсли;
		
		ЯщикиПолучателей.Добавить(Запись.ID);
		
	КонецЦикла;
	
	Переменные = Новый Структура;
	Переменные.Вставить("ВсегоКА", ВсегоКА);
	Переменные.Вставить("ОтправленоКА", ПриглашенныеКонтрагенты.Количество());
	Переменные.Вставить("ОтправленоКАЮЛ", ОтправленоТипЮЛ);
	Переменные.Вставить("ОтправленоКАИП", ОтправленоТипИП);
	Переменные.Вставить("ОтправленоКАФЛ", ОтправленоТипФЛ);
	Переменные.Вставить("ОтправленоКАИЛ", ОтправленоТипИЛ);
	Переменные.Вставить("toBoxId", ЯщикиПолучателей);
