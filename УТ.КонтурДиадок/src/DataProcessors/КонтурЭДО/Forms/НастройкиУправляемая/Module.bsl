...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьОписаниеГруппыОбновление(ЗначенияФО.НастройкиМенеджерПлагиновV4);
...
&НаСервере
Процедура УстановитьОписаниеГруппыОбновление(ИспользуетсяМенеджерПлагиновV4)
	
	Элементы.ГруппаАвтообновлениеМодуль.Видимость = НЕ ИспользуетсяМенеджерПлагиновV4;
	Элементы.ГруппаАвтообновлениеМодульПлагины.Видимость = ИспользуетсяМенеджерПлагиновV4;
...
Процедура УстановитьДоступностьЭлементовФормы()
	...
	Элементы.РазрешитьДлительныеОперацииВФоне.Доступность = Модуль_Платформа().ДлительныеОперации_Доступны();
...
Процедура ОтключитьВключитьДекорацииТариф(ЕстьОграниченияПоТарифу)
	...
	Элементы.ПерейтиНаПолныйТарифАвтообновление.Видимость = ЕстьОграниченияПоТарифу;
...
Функция ЗначенияФункциональныхОпций()
	...
	Модель.Вставить("НастройкиМенеджерПлагиновV4", Ложь);
	...
	Значение = ЗначенияОпций.Получить("PluginManager.V4");
	Если ЗначениеЗаполнено(Значение) Тогда
		Модель.НастройкиМенеджерПлагиновV4 = XMLЗначение(ТипБулево, Значение);
	КонецЕсли;
...
Функция ПроверитьВозможностьЗапускаФоновогоЗадания()
	...
	Попытка
		...
		ПроверитьВозможностьЗапускаФоновогоЗаданияНаСервере();
	...
	Исключение
		...
		Модуль_ЯдроНаКлиенте().Ошибка_Обработать(ВидОперации, ТекстОшибки, ТекстСообщения);
...
&НаСервере
Процедура ПроверитьВозможностьЗапускаФоновогоЗаданияНаСервере()
	
	Модуль_ЯдроНаСервере().ОбщиеНастройки_ПроверитьВозможностьЗапускаФоновыхЗаданий();
