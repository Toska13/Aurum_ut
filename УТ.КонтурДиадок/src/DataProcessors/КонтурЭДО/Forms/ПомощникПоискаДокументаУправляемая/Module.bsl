...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("ДокументУчета", СсылкаНаОбъект);
	Параметры.Свойство("ВидПакетаЧерновик", ВидПакетаЧерновик);
	...
	ИначеЕсли Режим = РежимыФормы.ТолькоПроверка Тогда
	...
	ИначеЕсли Режим = РежимыФормы.ПроверкаЧерновика Тогда
...
Процедура ОткрытьЦентрПоддержки()
	...
	Категория = Метрика_КатегорияПоРежимуФормы(Режим);
...
Процедура ОбработатьСценарииПроверокНаСервере()
	...
	РежимФормыВыборИПроверка = (Режим = РежимыФормы().ВыборИПроверка);
	РежимФормыТолькоПроверка = (Режим = РежимыФормы().ТолькоПроверка);
	РежимФормыПроверкаЧерновика = (Режим = РежимыФормы().ПроверкаЧерновика);
	
	ФормироватьМетрикуСоСпискомОшибокПроверкиДокумента = (
		ФормироватьМетрикуСоСпискомОшибок
		И РежимФормыТолькоПроверка
		И ЗначениеЗаполнено(ИсточникОткрытия)
		И НЕ ДокументПрошелПроверку
	);
	
	Если ФормироватьМетрикуСоСпискомОшибокПроверкиДокумента Тогда
		СформироватьМетрикуСоСпискомОшибокПроверкиДокумента(РезультатПроверки);
	КонецЕсли;
	
	ФормироватьМетрикуСоСпискомОшибокПроверкиЧерновика = (
		РежимФормыПроверкаЧерновика
		И НЕ ДокументПрошелПроверку
	);
	
	Если ФормироватьМетрикуСоСпискомОшибокПроверкиЧерновика Тогда
		СформироватьМетрикуСоСпискомОшибокПроверкиЧерновика(РезультатПроверки);
	КонецЕсли;
	
	Если РежимФормыВыборИПроверка
		Или РежимФормыПроверкаЧерновика
...
Функция РежимыФормы()
	...
	Результат.Вставить("ПроверкаЧерновика", "ПроверкаЧерновика");
...
Процедура Метрика_ДобавитьСтатистику_НажатиеКнопки(Действие, СтатусКонтрагента)
	...
	ДопПеременные.Вставить("OneCDocumentId", СсылкаНаОбъект.УникальныйИдентификатор());
	
	Ядро = Модуль_ЯдроНаКлиенте();
	
	Ядро.Метрика_ДобавитьСтатистику_СУчетомЗаполненияКонтекста(
		Метрика_КатегорияПоРежимуФормы(Режим),
		Действие, ,
		ДопПеременные, ,
		ИдентификаторТрассировки
	);
...
Процедура Метрика_ДобавитьОшибку(ДействиеМетрики, ДанныеОшибки)
	...
	КатегорияМетрики = Метрика_КатегорияПоРежимуФормы(Режим);
...
&НаСервере
Процедура СформироватьМетрикуСоСпискомОшибокПроверкиЧерновика(СписокОшибочныхПроверок)
	
	КатегорияМетрики = Метрика_КатегорияДействияСЧерновиками();
	ДействиеМетрики = Метрика_ДействиеПоказНайтиНужныйДокумент();
	
	ДопПеременные = Новый Соответствие;
	ДопПеременные.Вставить("EDocumentType", ВидПакетаЧерновик);
	ДопПеременные.Вставить("OneCDocumentID", СокрЛП(СсылкаНаОбъект.УникальныйИдентификатор()));
	ДопПеременные.Вставить("OneCDocumentType", СокрЛП(ТипЗнч(СсылкаНаОбъект)));
	ДопПеременные.Вставить("ReasonsCount", СписокОшибочныхПроверок.Количество());
	ДопПеременные.Вставить("ReasonsTypes", Новый Массив);
	Для Каждого РезультатПроверки Из СписокОшибочныхПроверок Цикл
		ДопПеременные.Получить("ReasonsTypes").Добавить(РезультатПроверки.Представление);
	КонецЦикла;
	
	ПредставлениеМетрики = "ПроверкаЧерновика";
	TraceId = "РаботаИзИнтерфейса1С" + СсылкаНаОбъект.УникальныйИдентификатор();
	
	Ядро = Модуль_ЯдроНаСервере();
	
	ИдентификаторЯщика = Ядро.ДокументыУчета_ИдентификаторЯщикаОрганизацииДокумента(СсылкаНаОбъект);
...
&НаКлиентеНаСервереБезКонтекста
Функция Метрика_ДействиеПоказНайтиНужныйДокумент()
	Возврат "ПоказНайтиНужныйДокумент";
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция Метрика_КатегорияПоРежимуФормы(ТекущийРежим)
	
	РежимыФормы = РежимыФормы();
	
	Если ТекущийРежим = РежимыФормы.ПроверкаЧерновика Тогда
		Результат = Метрика_КатегорияДействияСЧерновиками();
	Иначе
		Результат = Метрика_КатегорияНайтиНужныйДокумент();
	КонецЕсли;
