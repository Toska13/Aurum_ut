...
Функция НовыйОписаниеОповещения(ИмяПроцедуры,
	...
	Возврат Платформа.НовыйОписаниеОповещения(
		ИмяПроцедуры, Модуль,
...
Процедура ПоказатьПредупреждениеПереопределенная(ОписаниеОповещения = Неопределено,
	...
	Платформа = Модуль_Платформа();
	Платформа.ПоказатьПредупреждениеЛок(ОписаниеОповещения, Текст, Таймаут, Заголовок);
...
Процедура ВыполнитьОбработкуОповещенияПереопределенная(ОписаниеОповещения, РезультатОповещения = Неопределено) Экспорт
	...
	Платформа = Модуль_Платформа();
	Платформа.ВыполнитьОбработкуОповещенияЛок(ОписаниеОповещения, РезультатОповещения);
...
Процедура ПоказатьВыборЭлемента(Список, ОписаниеОповещенияОЗавершении = Неопределено, Заголовок = "", Элемент = Неопределено) Экспорт
	...
	Платформа = Модуль_Платформа();
	Платформа.ПоказатьВыборЭлемента(Список, ОписаниеОповещенияОЗавершении, Заголовок, Элемент);
...
Процедура ПоказатьДиалогВыбораКаталога(ОписаниеОповещенияОЗавершении,
	...
	Платформа = Модуль_Платформа();
	Платформа.ПоказатьДиалогВыбораКаталога(ОписаниеОповещенияОЗавершении, Заголовок, Каталог, МножественныйВыбор);
...
Процедура ПоказатьДиалогВыбораФайла(ОписаниеОповещенияОЗавершении, РежимРаботы, ПараметрыВыбора) Экспорт 
	...
	Платформа = Модуль_Платформа();
	Платформа.ПоказатьДиалогВыбораФайла(ОписаниеОповещенияОЗавершении, РежимРаботы, ПараметрыВыбора);
...
Процедура ПоказатьДиалогВыбораИзСписка(СписокЗначений, ОписаниеОповещенияОЗавершении, Форма, ЭлементФормы, Значение = Неопределено) Экспорт
	...
	Платформа = Модуль_Платформа();
	Платформа.ПоказатьДиалогВыбораИзСписка(СписокЗначений, ОписаниеОповещенияОЗавершении, Форма, ЭлементФормы, Значение);
...
Процедура ПоказатьОтметкуЭлементов(СписокЗначений, ОписаниеОповещенияОЗакрытии, Заголовок) Экспорт 
	...
	Платформа = Модуль_Платформа();
	Платформа.ПоказатьОтметкуЭлементовСпискаЗначений(СписокЗначений, ОписаниеОповещенияОЗакрытии, Заголовок);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МодельСтрокаСписка = Новый ФиксированнаяСтруктура(Ядро.Контракт_СтрокаСпискаДокументов());
	...
	Если Параметры.Свойство("КонфигурационныйФайл", Конфиг) Тогда
		Ядро.ОбщийКонтекст_Поместить("__КонфигурационныйФайл", Конфиг);
	КонецЕсли;
...
Процедура ПослеОткрытия()
	...
	СисИнфо = Модуль_Платформа().СистемнаяИнформацияНаКлиенте();
...
Процедура ВыполнитьДействияНаСервереПослеОткрытияФормы()
	...
	Ядро.МетрикиКонтур_ЗаписатьСобытие_СборКонтекстаПоМодулю();
...
Процедура ЗаполнитьФормуИзСохраненныхНастроек()
	...
	Если ЗначениеЗаполнено(СохраненныеНастройки) Тогда
		...
		Если ТипЗнч(СохраненныеНастройки) = Тип("Соответствие") Тогда
			...
			Если ДоступнаНастройкаПоказыватьЧерновики() Тогда
				УстановитьЗначениеНастройкиПоказыватьЧерновики(СохраненныеНастройки);
			КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(СохраненныеНастройки) Тогда
		// BSLLS:UnusedLocalVariable-off
		ПоказыватьЧерновики = НастройкаПоказыватьЧерновикиПоУмолчанию();
		// BSLLS:UnusedLocalVariable-on
	КонецЕсли;
	
	ОбновитьПризнакОткрытияНовойФормыАвторизации();
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьПризнакОткрытияНовойФормыАвторизации()

	// BSLLS:UnusedLocalVariable-off
	БылаОткрытаНоваяФормаАвторизации = ОбработкаОбъект().ПрочитатьПризнакОткрытаНоваяФормаАвторизации();
	// BSLLS:UnusedLocalVariable-on
...
&НаСервере
Процедура УстановитьЗначениеНастройкиПоказыватьЧерновики(Настройки)
	
	// BSLLS:UnusedLocalVariable-off
	ПоказыватьЧерновики = НастройкаПредложенияНаОтправкуЗначение(Настройки);
	// BSLLS:UnusedLocalVariable-on
	
КонецПроцедуры

&НаСервере
Функция НастройкаПредложенияНаОтправкуЗначение(СохраненныеНастройки)
	
	Результат = СохраненныеНастройки.Получить("ПоказыватьЧерновики");
	Если Результат = Неопределено Тогда
		Результат = НастройкаПоказыватьЧерновикиПоУмолчанию();
	КонецЕсли;
...
Функция ЗначенияФункциональныхОпций()
	...
	Модель.Вставить("НастройкаЧерновики", Ложь);
	Модель.Вставить("ПоказыватьЧерновики", Ложь);
	...
	Значение = ЗначенияОпций.Получить("BaseForm.ShowDraftsMode");
	Если ЗначениеЗаполнено(Значение) Тогда
		Модель.НастройкаЧерновики = XMLЗначение(ТипБулево, Значение);
	КонецЕсли;
	
	Значение = ЗначенияОпций.Получить("BaseForm.DefaultDraftsCheckbox");
	Если ЗначениеЗаполнено(Значение) Тогда
		Модель.ПоказыватьЧерновики = XMLЗначение(ТипБулево, Значение);
	КонецЕсли;
...
Функция ДоступнаНастройкаПоказыватьЧерновики()
	...
	ЗначенияФО = ЗначенияФункциональныхОпций();
	Результат = ЗначенияФО.НастройкаЧерновики;
...
&НаСервере
Функция НастройкаПоказыватьЧерновикиПоУмолчанию()
	
	ЗначенияФО = ЗначенияФункциональныхОпций();
	Результат = ЗначенияФО.ПоказыватьЧерновики;
...
Процедура УстановитьВидимостьКнопок()
	...
	Элементы.НастройкаОтправкиПриглашений.Видимость = Модуль_Ядро.АвтоматическаяОтправкаПриглашенийДоступна();
...
Процедура ОжиданиеОплатыСервисаИОшибокМЧД()
	...
	ПодключитьОбработчикОжидания("НачатьОтправкуПриглашенийКонтрагентам", Интервал01(), Истина);
...
// {	ЧЕРНОВИКИ ДОКУМЕНТОВ НА ОТПРАВКУ

&НаКлиенте
Процедура ПоказыватьЧерновикиПриИзменении(Элемент)
	
	ОбновитьСписок();
	
	Метрики_ЗаписатьНажатие_ПоказыватьЧерновики();
	
КонецПроцедуры

&НаКлиенте
Процедура Метрики_ЗаписатьНажатие_ПоказыватьЧерновики()
	
	КатегорияМетрики = Метрика_КатегорияДействияСЧерновиками();
	ДействиеМетрики = Метрика_ДействиеПоказыватьЧерновики();
	
	ОтборПоПакету = Неопределено;
	РежимыСписка = РежимыОтображения();
	РежимСписка = РежимыСписка.ДляОтправки;
	
	Переменные = ПеременныеЗамераКоличествоДокументов(ОтборПоПакету, РежимСписка);
	
	Ядро = Модуль_ЯдроНаКлиенте();
	Ядро.Метрика_ДобавитьСтатистику_СУчетомЗаполненияКонтекста(КатегорияМетрики, ДействиеМетрики, , Переменные);
...
&НаКлиенте
Процедура ОткрытьНастройкиАвтоматическойОтправкиПриглашений()

	Модуль_Платформа = Модуль_Платформа();
	Модуль_Платформа.ОткрытьНастройкиАвтоматическойОтправкиПриглашений();
...
Процедура ВыполнитьДействияНаСервереПослеАвторизации()
	...
	// BSLLS:UnusedLocalVariable-off
	СводноеОписаниеПредложений = СводноеОписаниеПредложенийДляВсехЛицевыхСчетов();
	// BSLLS:UnusedLocalVariable-on
...
&НаКлиенте
Процедура НастройкаОтправкиПриглашений(Команда)
	
	ОткрытьНастройкиАвтоматическойОтправкиПриглашений();
...
Процедура СписокДокументовВыбор(Элемент, ВыбраннаяСтрокаЭлементФормы, Поле, СтандартнаяОбработка)
	...
	Если ВыбраннаяСтрока <> Неопределено Тогда
		...
		Если Поле.Имя = "СписокДокументовДокумент"
			...
			ОбработчикОткрытияФормыДокументаУчета(ВыбраннаяСтрока.Документ);
			
		ИначеЕсли ВыбраннаяСтрока.ЭтоЧерновик Тогда
			
			ОбработчикОткрытияПредложенияДляОтправки(ВыбраннаяСтрока.Документ, ВыбраннаяСтрока.ВидПакета);
			
		ИначеЕсли Поле.Имя = "СписокДокументовСтатусЭПД"
			И ЗначениеЗаполнено(ВыбраннаяСтрока.СтатусЭПД) Тогда
			
			СписокДокументов_ПриВыбореСтатусаЭПД(ВыбраннаяСтрока);
			
		ИначеЕсли РежимОтображенияДокументов = РежимыОтображения().Перевозочные
			И ЗначениеЗаполнено(ВыбраннаяСтрока.Документ) Тогда
...
&НаКлиенте
Процедура ОбработчикОткрытияФормыДокументаУчета(ДокументСсылка)
	
	Платформа = Модуль_Платформа();
	Платформа.ПоказатьЗначениеЛок(ДокументСсылка);
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработчикОткрытияПредложенияДляОтправки(ДокументСсылка, ВидПакета)
	
	ОписаниеЗакрытияФормы = НовыйОписаниеОповещения(
		"ОбработатьРезультатЗакрытияПредложенияНаОтправку",
		ЭтаФорма
	);
	
	Платформа = Модуль_Платформа();
	Платформа.ПоказатьРезультатПроверкиЧерновикаПакета(ОписаниеЗакрытияФормы, ДокументСсылка, ВидПакета);
...
Процедура ОбработчикОповеститьОВыполненииКоманды(Параметр1 = Неопределено, Параметр2 = Неопределено) Экспорт
	...
	ИспользуетсяМодульЛогистики = Модуль_ЯдроНаКлиенте().КонтурЛогистика_ИспользуетсяМодульЛогистики();
...
Процедура УстановитьОтборПоВидуПакета(РежимОтображения)
	...
	СписокЗначенийВидовПакета = ОбработкаОбъект().СписокЗначенийВидовПакета(РежимОтображения);
...
Процедура ЗаполнитьМенюДействий(РежимОтображенияДокументов)
	...
	ОтключитьАнализ = Ядро.ОбщиеНастройки_Прочитать("ОтключитьАнализДляВходящихДокументов");
	...
	Элементы.ВыполнитьПодписаниеИОтправку.Видимость		= РежимДляОтправки ИЛИ РежимИсходящие ИЛИ РежимВнутренние
		ИЛИ РежимПеревозочные;
	
	Если РежимДляОтправки Тогда
		ПодписаниеСертификат = Ядро.ЗначениеНастройки_ПодписаниеСертификат();
		ДоступноПодписаниеГосключ = Ядро.ДоступноПодписаниеГосключ();
		Элементы.ВыполнитьПодписаниеИОтправку.Видимость = ПодписаниеСертификат;
		Элементы.ВыполнитьПодписаниеИОтправкуГосключ.Видимость	= ДоступноПодписаниеГосключ;
	Иначе
		Элементы.ВыполнитьПодписаниеИОтправкуГосключ.Видимость	= Ложь;
	КонецЕсли;
		
	Элементы.ОткрытьПомощникПоискаДокумента.Видимость = РежимДляОтправки;
	Элементы.ГруппаПоказатьЧерновики.Видимость = РежимДляОтправки;
	Элементы.ПоказыватьЧерновики.Видимость = РежимДляОтправки И ДоступнаНастройкаПоказыватьЧерновики();
	
	Элементы.ГруппаСогласование.Видимость = НЕ РежимПеревозочные;
	Элементы.ВыполнитьДобавитьЭПД.Видимость = РежимПеревозочные;	
	...
	Элементы.ВыполнитьПередатьВПодразделение.Видимость	= НЕ РежимДляОтправки И НЕ РежимПеревозочные;
...
Процедура ВыполнитьСохранитьДокумент(Команда)
	...
	Действие = ДействияЭДО().СохранитьДокумент;
	
	Если РежимОтображенияДокументов = РежимыОтображения().Перевозочные Тогда
		
		Если НЕ ПеревозочныеДокументы_МожноВыполнитьДействиеДляВыбранныхДокументов(Действие) Тогда
			Возврат;
		КонецЕсли;
...
Процедура ВыполнитьПолучитьПечатнуюФорму(Команда)
	...
	Действие = ДействияЭДО().ПолучитьПечатнуюФорму;
	...
	ПараметрыОбработки = Новый_ПараметрыГрупповойОбработки(ДанныеДляОбработки, Действие, КатегорияМетрики,
		ДействиеМетрики);
...
Процедура ОбработатьПолучитьПечатнуюФорму(ПараметрОповещения = Неопределено, ПараметрыОбработки) Экспорт
	...
	Для Каждого Документ Из ДанныеДляОбработки Цикл
		...
		Модуль_Платформа.ПолучитьПечатнуюФормуСервераДиадок(
			Документ,
			,
...
Процедура ВыполнитьПодписаниеИОтправкуГосключ(Команда)
	...
	Если ДействиеДоступно Тогда
		...
		ПолучитьСогласиеНаГрупповуюОбработкуВыделенныхСтрок("ОбработатьПакетыНаОтправкуГосключ", ПараметрыОбработки);
...
&НаКлиенте
Процедура ОбработатьПакетыНаОтправкуГосключ(ПараметрОповещения, ПараметрыОбработки) Экспорт
	
	ПлатформаУправляемая = Модуль_Платформа();
	ПлатформаУправляемая.ОбработатьПакетыНаОтправкуГосключ(ПараметрОповещения, ПараметрыОбработки);
...
Процедура ПроверитьПараметрыГрупповойОперацииИВыполнитьОбработчик(ОписаниеГрупповойОперации)
	...
	Если ПараметрыГрупповойОбработки.Действие = "ПодписатьИОтправитьГосключ" Тогда
		Госключ_НачатьПроверкуДанныхГрупповойОперации(ОписаниеГрупповойОперации);
	Иначе
...
Процедура УстановитьВидГруппыКнопокПодписать()
	...
	Если Элементы.ВыполнитьПодписаниеИОтправку.Видимость
		И Элементы.ВыполнитьПодписаниеИОтправкуГосключ.Видимость Тогда
		
		Элементы.ВыполнитьПодписаниеИОтправку.Заголовок = "Сертификатом";
		
		Элементы.ГруппаКнопокПодписаниеИОтправка.Вид = ВидГруппыФормы.Подменю;
		
	Иначе
		
		Элементы.ВыполнитьПодписаниеИОтправку.Заголовок = "Подписать и отправить";
		Элементы.ГруппаКнопокПодписаниеИОтправка.Вид = ВидГруппыФормы.ГруппаКнопок;
...
Процедура ДлительныеОперации_ПолучитьСтатусыЭПДвФоне()
	...
	Если НЕ Модуль_ЯдроНаКлиенте().КонтурЛогистика_ИспользуетсяМодульЛогистики() Тогда
		Возврат;
	КонецЕсли;
...
Функция ПеременныеЗамераКоличествоДокументов(ОтборПоПакету, РежимСписка)
	...
	Если РежимСписка = РежимыОтображения().ДляОтправки Тогда
		Результат.Вставить("NotReadyDocumentsCount", 0);
		Результат.Вставить("ЧекбоксЧерновики", ПоказыватьЧерновики);
	КонецЕсли;
...
Процедура ЗаполнитьПеременныеМетрикиПоДаннымКэшаДокументов(АдресСпискаДокументов, РежимСписка, ДопПеременные)
	...
	Если ЗначениеЗаполнено(КэшСпискаДокументовЛокальный) Тогда
		...
		Если РежимСписка = РежимыОтображения().ДляОтправки Тогда
			...
			ПараметрыОтбора = Новый Структура("ЭтоЧерновик", Истина);
			ЧерновикиПакетовНаОтправку = КэшСпискаДокументовЛокальный.НайтиСтроки(ПараметрыОтбора);
			
			ДопПеременные.Вставить("NotReadyDocumentsCount", ЧерновикиПакетовНаОтправку.Количество());
...
Процедура ОбновитьСписокНаСервере(Знач ПараметрыОбновленияСписка)
	...
	УстановитьВидимостьКолонкиИндексИконкиСтроки();
	НастроитьДополнительныеКолонки();
	НастроитьФормуПриСменеРежима();
	
	МенеджерМодуля = ОбработкаОбъект();
	МенеджерМодуля.СписокДокументов_ОбновитьКэш(
		ПараметрыОбновленияСписка,
		СписокДокументов,
		СопоставленныеКонтрагенты1С.Выгрузить()
	);
...
&НаКлиенте
Процедура ДлительныеОперации_ОбновитьСтатусыЭПД(Результат, ДопПараметр = Неопределено) Экспорт
	
	Ядро = Модуль_ЯдроНаКлиенте();
	
	Если Результат.ЕстьОшибки Тогда
		
		ШаблонТекстаОшибки = НСтр("ru='Произошла ошибка при получении статусов ЭПД из Логистики!
		|%1
		|(Подробности см. в журнале регистрации)'");
		
		ТекстОшибкиПользователю = Ядро.Общее_ПодставитьПараметрыВСтроку(
			ШаблонТекстаОшибки,
			Результат.ОписаниеОшибки.Краткое);
		
		Ядро.Ошибка_СообщитьПользователю(ТекстОшибкиПользователю);
		
		Возврат;
		
	КонецЕсли;
	
	ДанныеЭПД = Модуль_Платформа().ДлительныеОперации_Результат(Результат.Данные);
...
Процедура УстановитьЦветКнопокПереключенияРежимов()
	...
	Элементы.ВыбратьРежимПеревозочные.ЦветФона	= ЦветНеактивнойКнопки;
	...
	ИначеЕсли РежимОтображенияДокументов = РежимыОтображения.Перевозочные Тогда
		...
		Элементы.ВыбратьРежимПеревозочные.ЦветФона = ЦветАктивнойКнопки;
...
&НаСервере
Процедура УстановитьВидимостьКолонкиИндексИконкиСтроки()
	
	КолонкаТаблицы = "ИндексИконкиСтроки";
	
	Для Каждого КолонкаФормы Из Элементы.СписокДокументов.ПодчиненныеЭлементы Цикл
		
		Если КолонкаФормы.Имя = "СписокДокументов" + КолонкаТаблицы Тогда
			
			КолонкаФормы.Видимость = ДоступнаНастройкаПоказыватьЧерновики();
			Прервать;
...
Процедура ОбновитьСтраницуСпискаДокументовНаСервере() Экспорт
	...
	ПараметрыОтбора = Новый Структура("ЭтоЧерновик", Ложь);
	СписокСтрокДляОбработки = СписокДокументов.Выгрузить(ПараметрыОтбора);
	
	// BSLLS:UnusedLocalVariable-off
	ИтогСписокДокументовСуммаДокумента = СписокСтрокДляОбработки.Итог("СуммаДокумента");
	// BSLLS:UnusedLocalVariable-on
...
Функция Новый_СтруктураДанныхСтрокиСписка(Источник)
	...
	Результат = Новый Структура(МодельСтрокаСписка);
...
Процедура ОбработатьВыделениеДокументов(УстановитьФлажки)
	...
	ПараметрыОтбора = Новый Структура("ЭтоЧерновик", Ложь);
	СписокСтрокДляОбработки = СписокДокументов.НайтиСтроки(ПараметрыОтбора);
	
	Для Каждого СтрокаСписка Из СписокСтрокДляОбработки Цикл
...
Процедура УстановитьПредставлениеТекущегоПользователяДиадок(Форма)
	...
	Форма.Элементы.АвторизацияПодСертификатом.Видимость = Истина;
...
&НаКлиенте
Функция Метрика_КатегорияДействияСЧерновиками()
	
	КатегорииМетрик = Метрика_НазваниеКатегории();
	Результат = КатегорииМетрик.ДействияСЧерновиками;
...
&НаКлиенте
Функция Метрика_ДействиеПоказыватьЧерновики()
	
	ДействияМетрик = Метрика_НазваниеДействий();
	Результат = ДействияМетрик.ЧекбоксПоказатьЧерновики;
...
Процедура ПоказатьВыборТипаДанныхЭлементаСоставногоТипа(ЭлементСоставногоТипа) Экспорт
	...
	Платформа = Модуль_Платформа();
	ОбработчикОповещения = Платформа.НовыйОписаниеОповещения("ОбработатьВыборТипаЭлементаСоставногоТипа", ЭтаФорма,
		ДопПараметры);
	Платформа.ПоказатьВыборЭлемента(СписокВыбора, ОбработчикОповещения, "Выбор типа данных", ЭлементСоставногоТипа);
...
Процедура УстановитьУсловноеОформлениеСпискаДокументов()
	...
	УстановитьУсловноеОформление_СписокДокументовПромежуточныйПолучатель();
	УстановитьУсловноеОформление_СписокДокументовОрганизация();
	
	УстановитьУсловноеОформление_СписокДокументовЧерновики();
...
Процедура УстановитьУсловноеОформление_СписокДокументовПодразделение()
	...
	ЭлементСписокДокументовПодразделение.Оформление.УстановитьЗначениеПараметра("ЦветТекста", Цвет_Подсказки());
...
&НаСервере
Процедура УстановитьУсловноеОформление_СписокДокументовОрганизация()
	
	ЭлементСписокДокументовОрганизация = УсловноеОформление.Элементы.Добавить();
	ЭлементОтбораСписокДокументовОрганизация =
		ЭлементСписокДокументовОрганизация.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбораСписокДокументовОрганизация.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СписокДокументов.Организация");
	ЭлементОтбораСписокДокументовОрганизация.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	
	ПолеПредставление = Новый ПолеКомпоновкиДанных("СписокДокументов.ПредставлениеОрганизацииЭДО");
	ЭлементСписокДокументовОрганизация.Оформление.УстановитьЗначениеПараметра("Текст", ПолеПредставление);
	ЭлементСписокДокументовОрганизация.Оформление.УстановитьЗначениеПараметра("ЦветТекста", Цвет_Подсказки());
	
	ОформляемоеПолеСписокДокументовОрганизация = ЭлементСписокДокументовОрганизация.Поля.Элементы.Добавить();
	ОформляемоеПолеСписокДокументовОрганизация.Поле = Новый ПолеКомпоновкиДанных("СписокДокументовОрганизация");
	
	ОформляемоеПолеСписокДокументовГрузоотправитель = ЭлементСписокДокументовОрганизация.Поля.Элементы.Добавить();
	ОформляемоеПолеСписокДокументовГрузоотправитель.Поле = Новый ПолеКомпоновкиДанных("СписокДокументовГрузоотправитель");
...
Процедура УстановитьУсловноеОформление_СписокДокументовКонтрагент()
	...
	ЭлементСписокДокументовКонтрагент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", Цвет_Подсказки());
	...
	ОформляемоеПолеСписокДокументовПеревозчик = ЭлементСписокДокументовКонтрагент.Поля.Элементы.Добавить();
	ОформляемоеПолеСписокДокументовПеревозчик.Поле = Новый ПолеКомпоновкиДанных("СписокДокументовПеревозчик");
	
КонецПроцедуры

&НаСервере
Процедура УстановитьУсловноеОформление_СписокДокументовПромежуточныйПолучатель()
	
	ЭлементСписокДокументовПромежуточныйПолучатель = УсловноеОформление.Элементы.Добавить();
	ЭлементОтбораСписокДокументовПромежуточныйПолучатель =
		ЭлементСписокДокументовПромежуточныйПолучатель.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбораСписокДокументовПромежуточныйПолучатель.ЛевоеЗначение =
		Новый ПолеКомпоновкиДанных("СписокДокументов.ПромежуточныйПолучатель");
	ЭлементОтбораСписокДокументовПромежуточныйПолучатель.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	
	ПолеПредставление = Новый ПолеКомпоновкиДанных("СписокДокументов.ПредставлениеПромежуточногоПолучателяЭДО");
	ЭлементСписокДокументовПромежуточныйПолучатель.Оформление.УстановитьЗначениеПараметра("Текст", ПолеПредставление);
	ЭлементСписокДокументовПромежуточныйПолучатель.Оформление.УстановитьЗначениеПараметра("ЦветТекста", Цвет_Подсказки());
	
	ОформляемоеПолеСписокДокументовПромежуточныйПолучатель =
		ЭлементСписокДокументовПромежуточныйПолучатель.Поля.Элементы.Добавить();
	ОформляемоеПолеСписокДокументовПромежуточныйПолучатель.Поле =
		Новый ПолеКомпоновкиДанных("СписокДокументовПромежуточныйПолучатель");
	
	ОформляемоеПолеСписокДокументовГрузополучатель =
		ЭлементСписокДокументовПромежуточныйПолучатель.Поля.Элементы.Добавить();
	ОформляемоеПолеСписокДокументовГрузополучатель.Поле = Новый ПолеКомпоновкиДанных("СписокДокументовГрузополучатель");
	
КонецПроцедуры

&НаСервере
Процедура УстановитьУсловноеОформление_СписокДокументовЧерновики()
	
	ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
	
	ЭлементОтбора = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СписокДокументов.ЭтоЧерновик");
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	...
	ОформляемоеПоле = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
	ОформляемоеПоле.Поле = Новый ПолеКомпоновкиДанных("СписокДокументовФлажок");
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция Цвет_Подсказки()
	// BSLLS:MagicNumber-off
	Возврат Новый Цвет(128, 128, 128);
	// BSLLS:MagicNumber-on
КонецФункции
...
// Для подписания данных групповой операции выполняет проверку контента
//
// Параметры:
//	ОписаниеГрупповойОперации - Структура - см. Новый_ОписаниеГрупповойОперации()
//
&НаКлиенте
Процедура Госключ_НачатьПроверкуДанныхГрупповойОперации(ОписаниеГрупповойОперации)
	
	Модуль_Платформа = Модуль_Платформа();
	
	ПараметрыГрупповойОбработки	 = ОписаниеГрупповойОперации.ПараметрыГрупповойОбработки;
	ДанныеДляОбработки = ПараметрыГрупповойОбработки.ДанныеДляОбработки;
	КоличествоДанныхДляОбработки = ДанныеДляОбработки.Количество();
	
	РезультатПроверкиГосключ = Модуль_Платформа.Госключ_ПроверитьДанныеГрупповойОперации(
		ДанныеДляОбработки, ПараметрыГрупповойОбработки.Действие,
		ЭтаФорма.УникальныйИдентификатор);
		
	ДанныеДляОбработкиБезОшибок = РезультатПроверкиГосключ.ДанныеДляОбработкиБезОшибок;
	КоличествоДанныхДляОбработкиБезОшибок = ДанныеДляОбработкиБезОшибок.Количество();
	
	Если КоличествоДанныхДляОбработкиБезОшибок > 0 Тогда
		Если КоличествоДанныхДляОбработки <> КоличествоДанныхДляОбработкиБезОшибок Тогда
			
			ПараметрыГрупповойОбработки.ДанныеДляОбработки = РезультатПроверкиГосключ.ДанныеДляОбработкиБезОшибок;
			
			ОписаниеГрупповойОперации.Вставить("РезультатПроверкиГосключ", РезультатПроверкиГосключ);
			
			ПоказатьВопрос_НеДляВсехДокументовМожноВыполнитьДействиеГосключ(ОписаниеГрупповойОперации);
			
		Иначе
			
			Госключ_НачатьПодписаниеИОтправку(РезультатПроверкиГосключ);
			
		КонецЕсли;
	Иначе
		
		Если РезультатПроверкиГосключ.ЕстьОшибкиПриПодготовке Тогда
			
			Модуль_Платформа.ОткрытьФормуСпискаОшибокЛок(ЭтаФорма);
		
		Иначе
			
			Модуль_Платформа.ПоказатьВопрос_ДляДокументаНевозможноВыполнитьДействиеГосключ(
				РезультатПроверкиГосключ.ОшибкиПроверки);
...
&НаКлиенте
Процедура Госключ_НачатьПодписаниеИОтправку(РезультатПроверкиГосключ)
	
	ПлатформаНаКлиенте = Модуль_Платформа();
	
	МассивПакетов = РезультатПроверкиГосключ.ДанныеДляОбработкиБезОшибок;
	КоличествоПакетов = РезультатПроверкиГосключ.КоличествоПакетов;
	КоличествоФайловПакетов = РезультатПроверкиГосключ.КоличествоФайловПакетов;
	РазмерФайловПакетов = РезультатПроверкиГосключ.РазмерФайловПакетов;
	
	ПервыйПакет = МассивПакетов[0];
	ИдентификаторЯщика = ПервыйПакет.Идентификаторы.BoxId;
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("МассивПакетов", МассивПакетов);
	ДополнительныеПараметры.Вставить("ИдентификаторЯщика", ИдентификаторЯщика);
	Метрика_НазваниеФормы = Метрика_НазваниеФормы();
	
	ПараметрыОткрытияФормы = ПлатформаНаКлиенте.ПараметрыФормыВводаСНИЛС_OID(ИдентификаторЯщика, КоличествоПакетов,
		КоличествоФайловПакетов, РазмерФайловПакетов, Метрика_НазваниеФормы);
...
Процедура ПослеВводаСНИЛС_OID(РезультатВводаСНИЛС_OID, ДополнительныеПараметры) Экспорт
	...
	МассивПакетов = ДополнительныеПараметры.МассивПакетов;
	ИдентификаторЯщика = ДополнительныеПараметры.ИдентификаторЯщика;
	...
	Если СпособПодписания = Модуль_Платформа.Перечисление_ВыбранныйСпособПодписанияГосключ().СНИЛС Тогда
		...
		ПродолжитьПодписатьИсходящиеПакетыГосключ(МассивПакетов, ИдентификаторЯщика,
			РезультатВводаСНИЛС_OID.ВведенныйСНИЛС, "");
		
	ИначеЕсли СпособПодписания = Модуль_Платформа.Перечисление_ВыбранныйСпособПодписанияГосключ().OID Тогда
		
		ПродолжитьПодписатьИсходящиеПакетыГосключ(МассивПакетов, ИдентификаторЯщика,
			"", РезультатВводаСНИЛС_OID.ВведенныйOID);
...
&НаКлиенте
Процедура ПродолжитьПодписатьИсходящиеПакетыГосключ(МассивПакетов, ИдентификаторЯщика, СНИЛС, OID)
	
	ЯдроНаКлиенте = Модуль_ЯдроНаКлиенте();
	ИдентификаторЗадания = ЯдроНаКлиенте.Пакеты_ОтправитьПакетыНаПодписаниеГосключ(МассивПакетов, ИдентификаторЯщика,
		СНИЛС, OID);
...
Процедура ПослеПодписанияИсходящийПакетГосключ(РезультатПодписанияГосключ, ДополнительныеПараметры) Экспорт
	...
	Если РезультатПодписанияГосключ.РазрешитьДействие
		...
		ПакетыОтправлены = ВыполнитьОтправкуИсходящихПакетовГосключ(РезультатПодписания, МассивПакетов);
		
	КонецЕсли;
	
	ОбработатьРезультатОтправкиПакетов(ПакетыОтправлены);
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработатьРезультатОтправкиПакетов(ПакетыОтправлены)
...
&НаКлиенте
Функция ВыполнитьОтправкуИсходящихПакетовГосключ(РезультатПодписания, МассивПакетов)
	
	ЯдроНаКлиенте = Модуль_ЯдроНаКлиенте();
	
	ЯдроНаКлиенте.Пакеты_ДобавитьПодписиГосключФайловПакетов(МассивПакетов, РезультатПодписания);
	
	Результат = ВыполнитьОтправкуИсходящихПакетовНаСервере(МассивПакетов);
...
&НаСервере
Функция ВыполнитьОтправкуИсходящихПакетовНаСервере(МассивПакетов)
	
	Результат = Ложь;
	
	Модуль_Ядро = Модуль_ЯдроНаСервере();
	ВосстановитьДвоичныеДанныеПакетовИзВременногоХранилища(МассивПакетов);
	Результат = Модуль_Ядро.Пакеты_ОтправитьИСохранитьПоСписку(МассивПакетов);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ВосстановитьДвоичныеДанныеПакетовИзВременногоХранилища(МассивПакетов)
...
// Показывает пользователям форму с визуализацией ошибок проверок по ГК
//
// Параметры:
//	ОписаниеГрупповойОперации - Структура - см. Новый_ОписаниеГрупповойОперации()
//
&НаКлиенте
Процедура ПоказатьВопрос_НеДляВсехДокументовМожноВыполнитьДействиеГосключ(ОписаниеГрупповойОперации)
	...
	ТекстВопроса = 
	"Не для всех документов пожно выполнить подписание Госключом.
	|
	|- Чтобы узнать о возможных причинах ошибки и посмотреть инструкции, нажмите на кнопку ""Подробнее"".
	|- Чтобы выполнить действие для доступных документов, нажмите на кнопку ""Продолжить""."; 
	
	ОбработчикЗакрытияВопроса = НовыйОписаниеОповещения(
		"ОбработатьРезультатВыбора_НеДляВсехДокументовМожноВыполнитьДействиеГосключ", ЭтаФорма, ОписаниеГрупповойОперации);
	
	ПоказатьВопросПереопределенная(ОбработчикЗакрытияВопроса, ТекстВопроса, КнопкиВопроса, ,
		ДействияНаФормеВопросаКЭП.Закрыть , "Не для всех документов в списке можно выполнить действие");
	
КонецПроцедуры
	
&НаКлиенте
Процедура ОбработатьРезультатВыбора_НеДляВсехДокументовМожноВыполнитьДействиеГосключ(ВыбранноеДействие,
	ОписаниеГрупповойОперации) Экспорт
	
	ДействияНаФормеВопросаГосключ = ДействияНаФормеВопросаКЭП();
	
	ПродолжитьДляДоступных	 = ВыбранноеДействие = ДействияНаФормеВопросаГосключ.Продолжить;
	ОткрытьСписокОшибок		 = ВыбранноеДействие = ДействияНаФормеВопросаГосключ.ОткрытьСписокОшибок;
	
	РезультатПроверкиГосключ = ОписаниеГрупповойОперации.РезультатПроверкиГосключ;
...
&НаКлиенте
Процедура ДлительныеОперации_ОбновитьПлагиныАвтоматическиРезультат(Результат, ДопПараметр = Неопределено) Экспорт

	Результат = Модуль_Платформа().ДлительныеОперации_Результат(Результат);
	...
	Пояснение = Модуль_ЯдроНаКлиенте()._СтдСтр().СтрСоединитьЛок(Результат, Символы.ПС);
	Если Количество = 1 Тогда
		ТекстСообщения = "Плагин обновлен";
	ИначеЕсли Количество > 0 Тогда
		ТекстСообщения = Модуль_ЯдроНаКлиенте()._СтдСтр().СтрШаблонЛок("Обновлены плагины (%1)", Количество);
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура НачатьОтправкуПриглашенийКонтрагентам()
	...
	Если Модуль_Платформа().ДлительныеОперации_Доступны() И ФоновыеОперацииРазрешеныНаСервере() Тогда
		// BSLLS:UnusedLocalVariable-off
		ОписаниеЗадания = Модуль_Платформа().ДлительныеОперации_ВыполнитьВФоне("ОтправитьПриглашенияКонтрагентам");
		// BSLLS:UnusedLocalVariable-on
	КонецЕсли;
...
Процедура ПоказатьПлашкуОтзываПоРаботеСАвторизацией()
	...
	Если Не БылаОткрытаНоваяФормаАвторизации Тогда
		Возврат;
	КонецЕсли;
	...
	НоваяФорма = Модуль_Платформа().ИмяФормыАвторизации() = "АвторизацияНоваяУправляемая";
...
&НаСервере
Функция СводноеОписаниеПредложенийДляВсехЛицевыхСчетов()
	...
	Результат = МодульЯдро.ЛичныйКабинет_ДоступныеТипыПредложений(ПредложенияДляВсехЛицевыхСчетов);
...
Процедура ОткрытьПодпискиИОплатаИзБаннера(Элемент)
	...
	Платформа = Модуль_Платформа();
	Платформа.ОткрытьЛичныйКабинет(СводноеОписаниеПредложений, Неопределено, ЭтаФорма);
...
Процедура УстановитьВидимостьЭлементовДляЛогистики()
	...
	Элементы.ВыбратьРежимПеревозочные.Видимость = ПоказатьЭлементыЛогистики;
...
&НаКлиенте
Процедура ВыбратьРежимПеревозочные(Команда)
...
Процедура ПеревозочныеДокументы_ПослеЗакрытияФормыТитула(Результат, ДопПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если ЗначениеЗаполнено(Результат.LetterId) Тогда
			...
			ОтборПоПакету.ИдентификаторПакета = Результат.LetterId;
...
&НаКлиенте
Процедура ПеревозочныеДокументы_ДобавитьЭПД(Команда)
	...
	ПараметрыФормы.Вставить("ОтборПоОрганизации", ?(ВключитьОтборПоОрганизации, ОтборПоОрганизации, Неопределено));
	ПараметрыФормы.Вставить("ОтборПоКонтрагенту", ?(ВключитьОтборПоКонтрагенту, ОтборПоКонтрагенту, Неопределено));
	
	ОповещениеПослеЗакрытия = НовыйОписаниеОповещения("ПеревозочныеДокументы_ПослеЗакрытияФормыДобавленияЭПД", ЭтаФорма);
	
	Модуль_Платформа.ПеревозочныеДокументы_ОткрытьФормуПодбораДокументов(
		ПараметрыФормы,
		ЭтаФорма,
...
Процедура ПеревозочныеДокументы_ПослеЗакрытияФормыДобавленияЭПД(Результат, ДопПараметры) Экспорт 
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ДокументыОснования", Результат.ДокументыОснования);
		ДополнительныеПараметры.Вставить("ВидПакетаID", Результат.ВидПакетаID);
		ДополнительныеПараметры.Вставить("СтрокаСписка", Новый_СтруктураДанныхСтрокиСписка(Неопределено));

		Если Результат.ВидПакетаID = "ID_Перевозочные_ЗаказЗаявка" Тогда
		...
		Иначе
			...
			//ОповещениеПослеЗакрытия = НовыйОписаниеОповещения("ПеревозочныеДокументы_ПослеВыбораСценария", ЭтаФорма, ДополнительныеПараметры);
			//ПараметрыФормы = Новый Структура;
...
Процедура ПеревозочныеДокументы_ПослеВыбораСценария(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ДокументыОснования", ДополнительныеПараметры.ДокументыОснования);
		ПараметрыФормы.Вставить("ВидПакетаID", ДополнительныеПараметры.ВидПакетаID);
		ПараметрыФормы.Вставить("СтрокаСписка", ДополнительныеПараметры.СтрокаСписка);
		ПараметрыФормы.Вставить("Сценарий", ТекущийСценарий);
...
&НаКлиенте
Функция ПеревозочныеДокументы_МожноВыполнитьДействиеДляВыбранныхДокументов(Действие)
	
	ДанныеДляОбработки = ДанныеВыделенныхСтрокТЧ(Истина);
	СтатусЧерновик = "Черновик";
	СтатусПодготовлен = "Подготовлен";
	ЕстьПодходящиеСтроки = Ложь;
	
	Для Каждого СтрокаСписка Из ДанныеДляОбработки Цикл
		Если СтрокаСписка.Статус <> СтатусЧерновик
			И СтрокаСписка.Статус <> СтатусПодготовлен Тогда
			ЕстьПодходящиеСтроки = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если ДанныеДляОбработки.Количество() > 0
		И НЕ ЕстьПодходящиеСтроки Тогда
		
		ПараметрыОбработки = Новый_ПараметрыГрупповойОбработки(Неопределено, Действие, "", "");
...
&НаКлиенте
Процедура ОбработатьРезультатЗакрытияПредложенияНаОтправку(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
...
Процедура ОбновитьКэшСопоставленныхКонтрагентов()
	...
	СопоставленныеКонтрагенты1С.Очистить();
	СопоставленныеКонтрагенты1С.Загрузить(Ядро.Контрагенты_СопоставленныеПартнеры());
