...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КартинкаВниманиеЖелтая = БиблиотекаКартинокЯдра.КартинкаВниманиеЖелтая;
	// BSLLS:UnusedLocalVariable-off
	АдресКартинкиПодсказкаКодИтога = ПоместитьВоВременноеХранилище(КартинкаВниманиеЖелтая, УникальныйИдентификатор);
	// BSLLS:UnusedLocalVariable-on
...
Процедура ДокументОРазногласиях_УправлениеДоступностью()
	...
	Элементы.ГруппаПодсказкаКодИтога.Видимость = НоваяДоступность;
	
	// BSLLS:LineLength-off
	Если РезультатПриемки = РезультатПриемкиДопустимыеЗначения.ПринятоСРазногласиями Тогда
		ЗаголовокНадписьРезультатПриемки = НСтр("ru = 'Требуется корректировка или исправление. Например, часть товара не принята, или пришел пересорт кодов по части продукции'");
	ИначеЕсли РезультатПриемки = РезультатПриемкиДопустимыеЗначения.НеПринято Тогда
		ЗаголовокНадписьРезультатПриемки = НСтр("ru = 'Отказ от поставки: весь товар фактически не принят, приемка не состоялась, коды маркировки остаются на продавце'");
	Иначе
		Элементы.ГруппаПодсказкаКодИтога.Видимость = Ложь;
		ЗаголовокНадписьРезультатПриемки = "";
	КонецЕсли;
	// BSLLS:LineLength-on
	
	Элементы.НадписьПодсказкаКодИтога.Заголовок = ЗаголовокНадписьРезультатПриемки;
