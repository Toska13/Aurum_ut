...
Процедура ИнициализироватьФорму(ПараметрыФормы) 
	...
	ВидТитулаПараметров = Неопределено;
	ПараметрыФормы.Свойство("ВидТитула", ВидТитулаПараметров);
	...
	ИначеЕсли ЭтоФормаПолномочийДляПеревозочных(ТипКонтента, ТипыКонтента) Тогда
		ВидТитула = ВидТитулаПараметров;
		Перевозочные_ЗаполнитьФормуПолномочиямиПодписанта();
	Иначе
...
Процедура ЗаполнитьФормуПолномочиямиПодписанта()
	...
	Если ЗначениеЗаполнено(СвидетельствоОРегистрацииИП) Тогда
		Элементы.ГруппаСвидетельствоОРегистрацииИП_Значение.ТекущаяСтраница =
			Элементы.СтраницаСвидетельствоОРегистрацииИП_Значение;
	КонецЕсли;
...
&НаСервере
Процедура Перевозочные_ЗаполнитьФормуПолномочиямиПодписанта()
...
Процедура ЗаполнитьОрганизациюПодписанта()
	...
	СтрокаКонтекста	= Модуль_Ядро.КонтекстСеанса_СтрокаКонтекста(АдресЯщика);
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоФормаПолномочийДляПеревозочных(ТипКонтента, ТипыКонтента)
	
	Результат = (ТипКонтента = ТипыКонтента.kl
		Или ТипКонтента = ТипыКонтента.zakzvper
		Или ТипКонтента = ТипыКонтента.kl_porek_wt3_05_02_01);
...
Процедура Сохранить(Команда)
	...
	ИначеЕсли ЭтоФормаПолномочийДляПеревозочных(ТипКонтента, ТипыКонтента) Тогда
		Перевозочные_ПроверитьЗаполнениеИСохранитьПолномочияПодписанта(Отказ);
	Иначе
...
&НаКлиенте
Процедура Перевозочные_ПроверитьЗаполнениеИСохранитьПолномочияПодписанта(Отказ)
	
	Модуль_Платформа = Модуль_Платформа();
	Модуль_Платформа.ПроверитьЗаполнениеПоляФормы(Отказ, ЭтаФорма, "Должность");
	
	Если Не Отказ Тогда
		Перевозочные_СохранитьПолномочияПодписантаНаСервере();
	КонецЕсли;
...
&НаСервере
Процедура Перевозочные_СохранитьПолномочияПодписантаНаСервере()
...
Процедура ЗаполнитьСписокВыбора_ОбластьПолномочий()
	...
	ОбластиПолномочийПодписанта = Модуль_Ядро.ПолномочияПодписанта_ОбластиПолномочий(ВидТитула, ПокупательПродавец);
	ПредставленияПолномочийПодписанта = Модуль_Ядро.ПолномочияПодписанта_ПредставленияПолномочий(ВидТитула);
...
Процедура ЗаполнитьСписокВыбора_СтатусРаботника()
	...
	СтатусыПодписанта = Модуль_Ядро.ПолномочияПодписанта_СтатусыПодписанта(ВидТитула, ПокупательПродавец);
	ПредставленияСтатусов = Модуль_Ядро.ПолномочияПодписанта_ПредставленияСтатусовПодписанта(ВидТитула,
		ПокупательПродавец);
...
Процедура УправлениеФормой()
	...
	ИначеЕсли ЭтоФормаПолномочийДляПеревозочных(ТипКонтента, ТипыКонтента) Тогда
		...
		Элементы.СотрудникИнойОрганизацииОснованиеПолномочийОрганизации.Видимость = Ложь;
		Элементы.ГруппаСвидетельствоОРегистрацииИП.Видимость = Ложь;
		Элементы.ГруппаИныеСведения.Видимость = Ложь;
	Иначе
		СкрытьДолжность();
		СкрытьПоказатьОснованиеПолномочийОрганизации();
	КонецЕсли;
