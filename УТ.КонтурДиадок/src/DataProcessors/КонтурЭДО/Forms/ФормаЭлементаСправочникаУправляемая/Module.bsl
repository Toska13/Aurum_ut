...
Процедура НастройкиФормыДляОрганизаций()
	...
	Ядро = Модуль_ЯдроНаСервере();
	ТипыОрганизаций = Ядро.Перечисление_ТипыОрганизаций();
	Элементы.ГруппаМеткаФизическоеЛицо.Видимость = (ДанныеОрганизации.Тип = ТипыОрганизаций.ФЛ);
	
	ЗаполнитьНастройкуПринятияПриглашений();
	
	ИспользоватьПодключаемыйМодуль = Ядро.ОбщиеНастройки_Прочитать("ИспользоватьПодключаемыйМодуль");
	...
	ПараметрыЗаполненияСвойствЭлементаСертификат = Новый Структура;
	ПараметрыЗаполненияСвойствЭлементаСертификат.Вставить(
		"Заголовок", 
		"Отпечаток сертификата на сервере");
	ПараметрыЗаполненияСвойствЭлементаСертификат.Вставить(
		"ПодсказкаВвода",
		"Отпечаток сертификата на сервере 1С");
	ЗаполнитьЗначенияСвойств(
		Элементы.ОтпечатокСертификатаНаСервере, 
		ПараметрыЗаполненияСвойствЭлементаСертификат);
	Элементы.ОтпечатокСертификатаНаСервере.УстановитьДействие(
		"НачалоВыбора",
		"ОтпечатокСертификатаНаСервереНачалоВыбора");
	
	Элементы.ГруппаСистемныеНастройкиПодсистемаДиадок.Видимость	= ИспользуетсяПодсистемаДиадок;
...
&НаСервере
Процедура ЗаполнитьНастройкуПринятияПриглашений()
	// BSLLS:UnusedLocalVariable-off
	Если ДанныеОрганизации.Экстраверт Тогда
...
Процедура УстановитьВидимостьПредупреждения(Форма, ПоказыватьПриОтсутствииСвязей = Истина)
	...
	Если ЭтоФормаКонтрагента Тогда
		...
		НетСвязанныхЭлементов = (Форма.ТаблицаСвязанныеСправочники.Количество() = 0);
		
		Если ПоказыватьПриОтсутствииСвязей И НетСвязанныхЭлементов Тогда
...
Процедура НастройкиФормыДляКонтрагентов()
	...
	Ядро = Модуль_ЯдроНаСервере();
	ТипыОрганизаций = Ядро.Перечисление_ТипыОрганизаций();
	Элементы.ГруппаМеткаФизическоеЛицо.Видимость = (ДанныеКонтрагента.Тип = ТипыОрганизаций.ФЛ);
...
Процедура НастройкиФормыДляПодразделенийОрганизации()
	...
	Элементы.ГруппаМеткаФизическоеЛицо.Видимость = Ложь;
...
Процедура УстановитьСвойстваФормыТакси()
	...
	СвойствоВертикальнаяПрокруткаПриСжатии = Новый Структура("ВертикальнаяПрокруткаПриСжатии", Истина);
	ЗаполнитьЗначенияСвойств(Элементы.ГруппаНастройкиДокументов, СвойствоВертикальнаяПрокруткаПриСжатии);
	
	Ядро = Модуль_ЯдроНаСервере();
	Если НЕ Ядро.ПриложениеСтаршеВерсии("8.5") Тогда
		Элементы.ТаблицаСвязанныеСправочникиСвязанныйСправочник.МногострочныйРежим = Ложь;
	КонецЕсли;
...
Процедура ТаблицаСвязанныеСправочникиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ЭтоНоваяСтрока = (Элемент.ТекущиеДанные.СвязанныйСправочник = Неопределено);
...
Процедура ТаблицаСвязанныеСправочникиДобавитьСтроку(Кнопка)
	...
	ПоказыватьПриОтсутствииСвязей = Ложь;
	УстановитьВидимостьПредупреждения(ЭтаФорма, ПоказыватьПриОтсутствииСвязей);
...
Процедура ДатаГраницыСинхронизацииНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Модуль_Платформа.ПоказатьВводДатыЛок(Оповещение, ВыбраннаяДата, Подсказка, ЧастьДаты);
...
&НаКлиенте
Процедура ОтпечатокСертификатаНаСервереНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	Если Элементы.ОтпечатокСертификатаНаСервере.СписокВыбора.Количество() = 0 Тогда
		
		СтандартнаяОбработка = Ложь;
		
		ТекстПредупреждения = "Нет доступных сертификатов на сервере";
...
Процедура ОтпечатокСертификатаНаСервереНастроить()
	...
	Исключение
		...
		ВидОперации = "Получение сертификатов на сервере";
		...
		Элементы.ДекорацияПодсказкаОтпечатокСертификата.Заголовок = 
			КраткоеПредставлениеОшибки;
		
		Модуль_Ядро = Модуль_ЯдроНаСервере();
		Модуль_Ядро.Ошибка_Обработать(
			ВидОперации,
...
Процедура ПереходПереключательВВебОбработкаНавигационнойСсылки(Элемент)
// BSLLS:MissingParameterDescription-on	
	ШаблонАдресСсылки = "https://diadoc.kontur.ru/%1/settings/company";
	
	ОсновнаяФорма = ОсновнаяФорма();
	Модуль_Ядро = Модуль_ЯдроНаКлиенте();
	КатегорияМетрики = Метрика_НазваниеКатегории().Настройки;
	ДействиеМетрики = "КликПоСсылкеВВеб";
	Представление = "ВходящиеНастройки";
	...
	BoxId = Модуль_Ядро.АдресЯщикаВИдентификатор(
			ДанныеОрганизации.ID);
	
	АдресСсылки = Модуль_Ядро.Общее_ПодставитьПараметрыВСтроку(
			ШаблонАдресСсылки,
			BoxId);
	...
	Переменные = Новый Соответствие;
	Переменные.Вставить("Sociability",	Sociability);
	Модуль_Ядро.Метрика_ДобавитьСтатистику_ДляОрганизации(
		ДанныеОрганизации.ID, 
		КатегорияМетрики, 
		ДействиеМетрики, 
