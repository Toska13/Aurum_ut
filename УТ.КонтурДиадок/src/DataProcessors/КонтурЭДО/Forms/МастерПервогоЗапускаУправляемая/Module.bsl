...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьУсловноеОформлениеКонтрагентыТипОрганизации();
...
&НаСервере
Процедура УстановитьУсловноеОформлениеКонтрагентыТипОрганизации()
	
	Ядро = Модуль_ЯдроНаСервере();
	ТипыКА = Ядро.Перечисление_ТипыОрганизаций();
	
	ПредставленияТипов = Новый Соответствие;
	ПредставленияТипов.Вставить(ТипыКА.ФЛ, "Физическое лицо");
	ПредставленияТипов.Вставить(ТипыКА.ЮЛ, "");
	ПредставленияТипов.Вставить(ТипыКА.ИП, "");
	ПредставленияТипов.Вставить(ТипыКА.ИЛ, "");
	
	ТипОтметки = Неопределено;
	ЕстьТипОтметки = Ядро.ВерсияПлатформыБольшеИлиРавнаЛок("8.5.1");
	Если ЕстьТипОтметки Тогда
		ТипыОтметок = Вычислить("ТипОтметкиЯчейкиТаблицы");
		ТипОтметки = ТипыОтметок.ФигураПодТекстомОвальная;
	КонецЕсли;
	
	Для Каждого КлючЗначение Из ПредставленияТипов Цикл
		
		ЭлементУО = УсловноеОформление.Элементы.Добавить();
		ЭлементОтбора = ЭлементУО.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("КонтрагентыПоРежимуОтображения.ТипОрганизации");
		ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ЭлементОтбора.ПравоеЗначение = КлючЗначение.Ключ;
		
		ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст", КлючЗначение.Значение);
		Если ЗначениеЗаполнено(КлючЗначение.Значение) Тогда
			Если ЕстьТипОтметки Тогда
				ЭлементУО.Оформление.УстановитьЗначениеПараметра("ТипОтметки", ТипОтметки);
			Иначе
		...
		ОформляемоеПоле.Поле = Новый ПолеКомпоновкиДанных("КонтрагентыПоРежимуОтображенияТипОрганизации");
...
Процедура ЗаполнитьСписокКонтрагентовНаСервере()
	...
	Для Каждого ТекКонтрагентДиадок Из КонтрагентыДиадок Цикл
		...
		СтрокаКонтрагенты.ТипОрганизации = ТекКонтрагентДиадок.Тип;
