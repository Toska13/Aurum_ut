...
&НаКлиентеНаСервереБезКонтекста
Функция ДанныеХранилищаДокумента(Документ)
	
    Результат = Документ.Документооборот.ХранилищеКонтента.Данные;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// BSLLS:UnusedLocalVariable-off
	Перечисление_ТипыОрганизаций = Ядро.Перечисление_ТипыОрганизаций();
	// BSLLS:UnusedLocalVariable-on
	...
	Если Ядро.ВерсияПлатформыБольшеИлиРавнаЛок("8.3.21") Тогда
	...
	ИначеЕсли РежимОтображенияДокументов = РежимыОтображения.Исходящие
		ИЛИ РежимОтображенияДокументов = РежимыОтображения.Перевозочные
	...
	ОбновитьСведенияОМаркировкеНаСервере();
	ОбновитьСостояниеПеревозочныхДокументов();
	
	ПроверитьЭкстренныеУведомленияФормы();
	
	Параметры.Свойство("СводноеОписаниеПредложений", СводноеОписаниеПредложений);
	
	Если ЗначениеЗаполнено(СводноеОписаниеПредложений) Тогда
...
Процедура ДобавитьРеквизитДокументPDF()
	...
	НовыйЭлемент = Элементы.Добавить("ДокументPDF", Тип("ПолеФормы"), Элементы.ГруппаДокументPDF);
...
Процедура ДокументPDFНажатиеНаНавигационнойСсылке(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Ядро = Модуль_ЯдроНаКлиенте();
	ДопПеременные = Ядро.Метрики_ПеременныеПоДокументуПодробно(ТекущийДокумент);
	
	Платформа = Модуль_Платформа();
	Платформа.НачатьПереходПоВнешнейСсылке(НавигационнаяСсылка, ЭтаФорма, ДопПеременные);
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(ВыбранныеФайлы.ИменаФайлов) Тогда
		...
		НужноЗаполнитьТекущийДокумент = Пакет.Документы.Количество() = 0;
		...
		Если НужноЗаполнитьТекущийДокумент
			И Пакет.Документы.Количество() > 0 Тогда
			
			ТекущийДокумент = Пакет.Документы[0];
			// BSLLS:UnusedLocalVariable-off
			ОбновитьФормуОтображения = Истина;
			// BSLLS:UnusedLocalVariable-on
...
Процедура ОтключитьВключитьПередатьНаСогласованиеПакет(ЕстьОграниченияПоТарифу)
	...
	Если ЕстьОграниченияПоТарифу Тогда
		Элементы.ПередатьНаСогласованиеПакет.Видимость = Ложь;
		Элементы.ПередатьНаСогласованиеПакетЗаглушка.Видимость = Истина;
	КонецЕсли;
...
Процедура ОтключитьВключитьПередатьПоМаршрутуПакет(ЕстьОграниченияПоТарифу)
	...
	Если ЕстьОграниченияПоТарифу Тогда
		Элементы.ПередатьПоМаршрутуПакет.Видимость = Ложь;
		Элементы.ПередатьПоМаршрутуПакетЗаглушка.Видимость = Истина;
	КонецЕсли;
...
Процедура УстановитьСвойстваЭлемента_ДокументыПакетаПерезаполнитьИсходящий()
	...
	ЭтоИсходящийПакет = (Направление = НаправлениеПакета.Исходящее И ПакетОтправлен);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "КонтурМаркировка_ОбновитьСведенияОМаркировке" Тогда
		
		ЭтоОбработкаОповещения = Истина;
		
		СписокДокументовДляОповещения = Параметр;
		
		ОбновитьСведенияОМаркировке(ЭтоОбработкаОповещения, СписокДокументовДляОповещения);
...
Функция ПолучитьСостояниеУведомлений()
	...
	Результат = Модуль_Ядро.УведомленияФормыПакета_ТекущееСостояние(Фильтр);
...
Процедура ОткрытьФормуУведомлений()
	...
	Платформа = Модуль_Платформа();
	Платформа.Уведомления_ФормаПакета(ПараметрыФормы, ЭтаФорма, Оповещение);
...
Процедура ДокументыПакетаПриАктивизацииСтроки(Элемент)
	...
	Если ОбновитьФормуОтображения Тогда
		
		ОбновитьФормуОтображения = Ложь;
...
Процедура ЗаполнитьФормуПоТекущемуДокументуНаСервере()
	...
	Если ЗначениеЗаполнено(СсылкаНаДокумент)
		...
		Модуль_ЯдроНаСервере().ПеревезочныеДокументы_ИдентифицироватьТитулДокумента(Документ);
...
Процедура ОбновитьОтображениеДокумента()
	...
	ДопПараметры.Вставить("КлючДокументаПакета", ТекущийДокумент.Ключ);
	ДопПараметры = Новый ФиксированнаяСтруктура(ДопПараметры);
	ПриПолученииПревью = Платформа.НовыйОписаниеОповещения("ПриПолученииПревьюДокумента", ЭтаФорма, ДопПараметры);
	
	Документ = ДокументСФайлом(ТекущийДокумент);
	Задание = Платформа.ДлительныеОперации_ВыполнитьВФоне("Документы_Превью", Документ);
	Платформа.ДлительныеОперации_ОжидатьЗавершение(Задание, ПриПолученииПревью);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ДокументСФайлом(Знач Документ)
...
&НаКлиенте
Процедура ПриПолученииПревьюДокумента(РезультатЗадания, ДопПараметры) Экспорт
	
	Если ДопПараметры.КлючДокументаПакета <> ТекущийДокумент.Ключ Тогда
		Возврат;
	КонецЕсли;
	...
	ДанныеПревью = Модуль_Платформа().ДлительныеОперации_Результат(РезультатЗадания.Данные);
	УдалитьИзВременногоХранилища(РезультатЗадания.Данные);
	
	Попытка
		
		ОтобразитьПревью(ДанныеПревью);
		
	Исключение
		
		ОшибкаИнфо = ИнформацияОбОшибке();
		ОбработатьОшибкуОтображенияПревью(ОшибкаИнфо);
		ОтобразитьПревьюЗаглушка();
...
&НаСервере
Процедура ОбработатьОшибкуОтображенияПревью(Знач ОшибкаИнфо)
	
	Ядро = Модуль_ЯдроНаСервере();
	
	Категория = Ядро.Метрика_НазваниеКатегории().ПревьюДокумента;
	Действие = "ОтобразитьФайлПревью";
	ДопПеременные = Ядро.Метрики_ПеременныеПоДокументуПодробно(ТекущийДокумент);
	
	Ядро.Метрика_ДобавитьОшибку(Категория, Действие, ОшибкаИнфо, ДопПеременные);
	
	ВидОперации = Ядро.СтдСтр().СтрШаблонЛок("%1.%2", Категория, Действие);
	Ядро.Ошибка_Обработать(ВидОперации, ОшибкаИнфо);
	
КонецПроцедуры

&НаКлиенте
Процедура ОтобразитьПревью(Знач ДанныеПревью)
	
	Если ДанныеПревью.Расширение = "pdf" И ПриложениеПоддерживаетПросмотрДокументовPDF Тогда
		ОтобразитьПревьюPDF(ДанныеПревью.Файл);
	ИначеЕсли ДанныеПревью.Расширение = "mxl" Тогда
		ОтобразитьПревьюMXL(ДанныеПревью.Файл);
	Иначе
		ОтобразитьПревьюЗаглушка();
	КонецЕсли;
...
&НаКлиенте
Процедура ОтобразитьПревьюMXL(ТабДок)
...
&НаКлиенте
Процедура ОтобразитьПревьюPDF(Файл)
	
	Элементы.ГруппаДанныеДокументов.ТекущаяСтраница = Элементы.ГруппаДокументPDF;
	// BSLLS:UnusedLocalVariable-off
	Поток = Файл.ОткрытьПотокДляЧтения();
	// BSLLS:UnusedLocalVariable-on
	ПолеДокументPDF = Вычислить("ДокументPDF");
	ПолеДокументPDF.Прочитать(Поток);
	
КонецПроцедуры

&НаКлиенте
Процедура ОтобразитьПревьюЗаглушка()
	Элементы.ГруппаДанныеДокументов.ТекущаяСтраница = Элементы.ГруппаБезПечатнойФормы;
КонецПроцедуры
...
Функция СохранитьФайлПакетаНаДиск() 
	...
	Если ХранилищеКонтента <> Неопределено Тогда
		...
		ДвоичныеДанные = ДанныеХранилищаДокумента(ТекущийДокумент);
...
Процедура ЗаполнитьИсториюПоДокументуВызовСервера()
	...
	Если ИспользуетсяПодсистемаДиадок Тогда
		Ключ_kl_id = "ВнешнийСтатус_kl_id";
	Иначе
		Ключ_kl_id = "kl_id";
	КонецЕсли;
	
	ВнешниеСтатусы = ТекущийДокумент.Статусы.ВнешниеСтатусы;
	ИдентификаторПеревозочного_kl_id = Ядро.СвойствоСтруктуры(ВнешниеСтатусы, Ключ_kl_id);
	
	ИсторияДО = Ядро.Документы_ИсторияДО(ТекущийДокумент.Идентификаторы, ИдентификаторПеревозочного_kl_id);
	// BSLLS:UnusedLocalVariable-off
...
Процедура УстановитьВидимостьЭлементовФормы_ДобавлениеДокументовСДиска()
	...
	Элементы.СтраницаИсторияПоДокументу.Видимость = Ложь;
...
Процедура УстановитьВидимостьЭлементовФормы_ОтображениеПакета()
	...
	Элементы.СтраницаИсторияПоДокументу.Видимость		= ПакетОтправлен;
	...
	Если Пакет.Шаблон Тогда
		...
		Элементы.СтраницаИсторияПоДокументу.Заголовок = "История по шаблону";
	КонецЕсли;
...
Процедура УстановитьВидимостьКартинкаОшибкиИсторияПоДокументу()
	...
	Если КартинкаОшибкиМЧД = Неопределено Тогда
		КартинкаОшибкиМЧД = Элементы.СтраницаИсторияПоДокументу.Картинка;
	КонецЕсли;
	...
	Если ЕстьОшибкаПроверкиМЧД
		...
		Элементы.СтраницаИсторияПоДокументу.Картинка = КартинкаОшибкиМЧД;
	Иначе
...
Процедура УстановитьВидимостьКнопокКоманднойПанелиДокумента()
	...
	Элементы.КнопкаНаписатьКонтрагенту.Видимость = ДоступныеДействия.НаписатьКонтрагенту;
...
Процедура УстановитьВидГруппыКнопокПодписать()
	...
	Иначе
		...
		Элементы.ГруппаКнопокПодписатьИОтправитьИсходящий.Вид = ВидГруппыФормы.ГруппаКнопок;
...
Процедура УстановитьВидимостьКнопокКоманднойПанелиДокументовПакета()
	...
	Если Направление = Направление().Внутреннее
		И ЗначениеЗаполнено(ДокументВ1С)
		И Не ПакетОтправлен Тогда
		Элементы.ДокументыПакетаПерезаполнитьИсходящий.Видимость = Истина;
	КонецЕсли;
...
Функция МожноОтправитьЗакрытыйПакет()
	...
	Результат = Истина;
	НельзяОтправить = Пакет.Получатель.ТипОрганизации = Перечисление_ТипыОрганизаций.ФЛ
		Или Пакет.Отправитель.ТипОрганизации = Перечисление_ТипыОрганизаций.ФЛ;
		
	Если ПакетОтправлен Или Пакет.Шаблон
		Или НельзяОтправить Тогда
		Результат = Ложь;
	Иначе
...
Процедура ОрганизацияНачалоВыбораЗавершение(ВыбраннаяОрганизация, ДополнительныеПараметры) Экспорт 
	...
	Если Не МожноОтправитьЗакрытыйПакет() Тогда
		// BSLLS:UnusedLocalVariable-off
		ЗаблокированныйПакет = Ложь;
		// BSLLS:UnusedLocalVariable-on
		Элементы.ЗаблокированныйПакет.Видимость = Ложь;
	КонецЕсли;
...
Процедура ИсторияДокументаHTMLПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если Платформа.ЭтоОповещение(АдресСсылки) Тогда
		
		ДопПараметры = Неопределено;
		Платформа.ВыполнитьОповещение(АдресСсылки, ДопПараметры, ЭтаФорма);
		
	Иначе
		
		Ядро = Модуль_ЯдроНаКлиенте();
		ДопПеременные = Ядро.Метрики_ПеременныеПоДокументуПодробно(ТекущийДокумент);
		Платформа.НачатьПереходПоВнешнейСсылке(АдресСсылки, ЭтаФорма, ДопПеременные);
...
&НаКлиенте
Процедура СкачатьУС4003(АдресВременногоХранилищаУС = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если НЕ ЗначениеЗаполнено(АдресВременногоХранилищаУС) Тогда
		Возврат;
	КонецЕсли;
	
	Метрика_ДобавитьПоведение_НажатиеСкачатьУС4003();
	Метрика_ДобавитьСтатистику_НажатиеСкачатьУС4003();
	
	Модуль_Платформа = Модуль_Платформа();

	// BSLLS:MissingTempStorageDeletion-off
	ДанныеУС = ПолучитьИзВременногоХранилища(АдресВременногоХранилищаУС);
	// BSLLS:MissingTempStorageDeletion-on
	ИмяФайла = ДанныеУС.ИмяФайла;

	ИмяВременногоФайла = СохранитьФайлУСНаДиск(ДанныеУС);
...
&НаКлиенте
Функция СохранитьФайлУСНаДиск(ДанныеУС)
	
	Результат = Неопределено;
	
	ДвоичныеДанные = ДанныеУС.ФайлУС;
	ИмяФайла = ДанныеУС.ИмяФайла;
...
&НаКлиенте
Процедура Метрика_ДобавитьПоведение_НажатиеСкачатьУС4003()
	
	КатегорияМетрики = Метрика_НазваниеКатегорииПоНаправлению();
	ДействиеМетрики = Метрика_ДействиеНажатиеСкачатьУС4003();
...
&НаКлиенте
Процедура Метрика_ДобавитьСтатистику_НажатиеСкачатьУС4003()
...
&НаКлиенте
Функция Метрика_ДействиеНажатиеСкачатьУС4003()
	
	Возврат "Скачать сообщение 4003";
...
&НаКлиенте
Процедура КомандаПоказатьФайл(Команда)
	
	Если ТекущийДокумент = Неопределено Тогда
		Модуль_Платформа = Модуль_Платформа();
		Модуль_Платформа.ПоказатьПредупреждениеЛок(, "Не выбран электронный документ");
		Возврат;
	КонецЕсли;
	
	Если Не ТекущийДокумент.Тип.Формализованный Тогда
		ТекстСообщения = "Отображение содержимого поддерживается только для формализованных документов";
		Модуль_Платформа = Модуль_Платформа();
		Модуль_Платформа.ПоказатьПредупреждениеЛок(, ТекстСообщения);
		Возврат;
	КонецЕсли;
	
	ИмяВременногоФайла = ПолучитьИмяВременногоФайла(".xml");
	
	ДанныеДокумента = ДанныеХранилищаДокумента(ТекущийДокумент);
	ДанныеДокумента.Записать(ИмяВременногоФайла);
	
	КодировкаФайла = Модуль_ЯдроНаКлиенте().КодировкаXml(ИмяВременногоФайла);
	Если КодировкаФайла = Неопределено Тогда
		КодировкаФайла = "windows-1251";
	КонецЕсли;
	
	Оповещение = Модуль_ЯдроНаКлиенте()._СтдОбщегоНазначения().ОписаниеОповещения("_ПослеУдаленияФайла", ЭтаФорма);
	Модуль_ЯдроНаКлиенте()._СтдФС().НачатьУдалениеФайловЛок(Оповещение, ИмяВременногоФайла);
	
	Текст = Модуль_ЯдроНаКлиенте().СтрокаИзДвоичныхДанных(ДанныеДокумента, КодировкаФайла);
		
	ТекстовыйДокумент = Новый ТекстовыйДокумент();
	ТекстовыйДокумент.ИспользуемоеИмяФайла = ТекущийДокумент.Документооборот.ХранилищеКонтента.ИмяФайла;
	ТекстовыйДокумент.ТолькоПросмотр = Истина;                       
	ТекстовыйДокумент.УстановитьТекст(Текст);
	ТекстовыйДокумент.Показать(ТекущийДокумент.ИмяФайла);
...
Процедура ПерейтиВДиадок(Элемент)
	...
	Модуль_Платформа = Модуль_Платформа();
	Модуль_Платформа.ПерейтиВДиадок(ТекущийДокумент);
...
Процедура СохранитьДокументНаДиск(Команда)
	...
	ПараметрыКоманды = Модуль_Платформа.ПараметрыКомандыСохранитьДокументНаДиск(ИмяФайла, ДанныеДокумента);
	ПараметрыКоманды.ИсточникОткрытия = ИсточникОткрытияДляМетрики();
	ПараметрыКоманды.НазваниеФормыДляМетрики = Метрика_НазваниеФормы();
	
	Модуль_Платформа.СохранитьДокументНаДиск(ПараметрыКоманды);
...
Процедура КнопкаСохранитьДокументооборот(Команда)
	...
	Модуль_Платформа = Модуль_Платформа();
	Модуль_Платформа.СохранитьДокументооборот(ТекущийДокумент);
...
Процедура ДобавитьФайлВПакет(ДвоичныеДанныеФайла, ИмяФайла, РазмерФайла)
	...
	Документ.СлужебнаяИнформация.ПараметрыОтправки.ЗашифрованныйПакет =
		Ядро.СвойствоСтруктуры(Пакет.СлужебнаяИнформация,
								"ЗашифрованныйПакет",
								Ложь);
	УстановитьДоступностьПодписанияИсходящего();
	УстановитьЗаголовокФормы();
...
Процедура ПодписатьИОтправитьИсходящийГосключ(Команда)
	...
	РезультатПроверкиГосключ = Новый Структура("ДанныеЗаполнены", Ложь);
	
	Если РежимОтображенияДокументов = РежимыОтображения.ДляОтправки Тогда
		РезультатПроверкиГосключ = Госключ_ДанныеИсходящегоПакетаЗаполнены();
	КонецЕсли;

	Если РезультатПроверкиГосключ.ДанныеЗаполнены Тогда
...
Процедура НастроитьПараметрыПодписания(Команда)
	...
	ПараметрыОткрытия.АдресЯщика = ТекущийДокумент.Идентификаторы.BoxId;
	ПараметрыОткрытия.ТипКонтента = ТекущийДокумент.Тип.ТипКонтента;
	ПараметрыОткрытия.ПокупательПродавец = "Продавец";
	ПараметрыОткрытия.ВидТитула = ТекущийДокумент.Тип.ТипДокумента;
...
Функция Госключ_ДанныеИсходящегоПакетаЗаполнены()
	...
	РезультатПроверкиГосключ = Новый Структура;
	РезультатПроверкиГосключ.Вставить("ДанныеЗаполнены", Истина);
	РезультатПроверкиГосключ.Вставить("КоличествоПакетов", 0);
	РезультатПроверкиГосключ.Вставить("КоличествоФайловПакетов", 0);
	РезультатПроверкиГосключ.Вставить("РазмерФайловПакетов", 0);
	
	Размер100МБ = 100 * 1024 * 1024;
	РазмерФайловПакетов = 0;
	КоличествоФайловПакетов = Пакет.Документы.Количество();
	ДанныеЗаполнены = Истина;
	
	Если КоличествоФайловПакетов = 0 Тогда
	...
	Для Каждого СтрокаТЧ Из ДокументыПакета Цикл
		...
		Если Не СвойстваФайла = Неопределено Тогда
			...
			РазмерФайловПакетов = РазмерФайловПакетов + СвойстваФайла.РазмерФайла;
		...
		ДанныеЗаполнены = ДанныеЗаполнены И НЕ СтрокаТЧ.ЕстьОшибки;
	...
	РезультатПроверкиГосключ.ДанныеЗаполнены = ДанныеЗаполнены;
	РезультатПроверкиГосключ.КоличествоПакетов = 1;
	РезультатПроверкиГосключ.КоличествоФайловПакетов = КоличествоФайловПакетов;
	РезультатПроверкиГосключ.РазмерФайловПакетов = РазмерФайловПакетов;
		
	Возврат РезультатПроверкиГосключ;
...
Процедура Подписать(Команда)
	...
	Если ДляПодписанияТребуетсяЭТрН_Титул3(ТекущийДокумент) Тогда
		ОткрытьФормуЭТрН_Титул3(Действие);
	Иначе
...
Процедура ОтказатьВПодписи(Команда)
	...
	Действие = Ядро.Перечисление_ДействияЭДО().ОтказатьВПодписи;
...
&НаКлиенте
Процедура НаписатьКонтрагенту(Команда)

	КатегорияМетрики = Метрика_НазваниеКатегорииПоНаправлению();
	ДействиеМетрики = "Написать контрагенту";
	Метрика_НажатиеКнопкиПакетнойОбработки(КатегорияМетрики, ДействиеМетрики);
	Метрика_СтатистикаДляОрганизации(КатегорияМетрики, ДействиеМетрики);
	
	Модуль_Платформа = Модуль_Платформа();
	
	ОписаниеЗакрытияФормы = Модуль_Платформа.НовыйОписаниеОповещения("ОбработчикЗавершенияСообщенияКонтрагенту", ЭтаФорма);
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Режим", "НаписатьКонтрагенту");
...
&НаКлиенте
Процедура ОбработчикЗавершенияСообщенияКонтрагенту(Результат, ДополнительныеПараметры = Неопределено) Экспорт 
	...
	КатегорияМетрики = Метрика_НазваниеКатегорииПоНаправлению();
	ДействиеМетрики = "Отправить сообщение";
	Метрика_НажатиеКнопкиПакетнойОбработки(КатегорияМетрики, ДействиеМетрики);
	Метрика_СтатистикаДляОрганизации(КатегорияМетрики, ДействиеМетрики);
	...
	Действие = Ядро.Перечисление_ДействияЭДО().НаписатьКонтрагенту;
	
	ТекущийДокумент.СлужебнаяИнформация.Вставить("КонтентОтвета", Комментарий);
	
	ДокументыДляОбработки = Новый Массив;
	ДокументыДляОбработки.Добавить(ТекущийДокумент);
	
	ОбработанныеДокументы = Ядро.Документы_ВыполнитьДействиеСДокументами(ДокументыДляОбработки, Действие);
	Если Элементы.ГруппаСодержаниеДокумента.ТекущаяСтраница.Имя = "СтраницаИсторияПоДокументу"
		И ЗначениеЗаполнено(ОбработанныеДокументы) Тогда
		ЗаполнитьИсториюПоДокументу();
	КонецЕсли;
...
&НаКлиенте
Процедура Метрика_ЗаписатьНажатие_ОткрытьФайлПревью()
	
	Категория = Метрика_НазваниеКатегории().ПревьюДокумента;
	Действие = Команды.ПосмотретьВДругойПрограмме.Имя;
	Метка = "";
	
	Ядро = Модуль_ЯдроНаКлиенте();
	
	Переменные = Ядро.Метрики_ПеременныеПоДокументу(ТекущийДокумент);
...
&НаКлиенте
Процедура ПосмотретьВДругойПрограмме(Команда)
	
	Метрика_ЗаписатьНажатие_ОткрытьФайлПревью();
...
&НаКлиенте
Функция ДляПодписанияТребуетсяЭТрН_Титул3(ТекущийДокумент)
	
	ЭтоЭТрН_Титул1 = Неопределено;
	
	ТекущийДокумент.СлужебнаяИнформация.Свойство("ЭтоЭТрН_Титул1", ЭтоЭТрН_Титул1);
	
	Результат = (ЭтоЭТрН_Титул1 = Истина);
...
&НаКлиенте
Процедура ОткрытьФормуЭТрН_Титул3(Действие)
	...
	Значение_kl_id = Ядро.СвойствоСтруктуры(ТекущийДокумент.Статусы.ВнешниеСтатусы, Ключ_kl_id);
	
	Если НЕ ЗначениеЗаполнено(Значение_kl_id) Тогда
		
		ТекстПредупреждения = "В документе отсутствует идентификатор Контур.Логистики (kl-id).";
		Модуль_Платформа().ПоказатьПредупреждениеЛок(, ТекстПредупреждения);
		Возврат;
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ДокументЭТрН_Титул1", ТекущийДокумент);
	ПараметрыФормы.Вставить("Действие", Действие);
	ПараметрыФормы.Вставить("kl_id", Значение_kl_id);
	
	Модуль_Платформа = Модуль_Платформа();
	
	ОповещениеПослеЗакрытия = Модуль_Платформа.НовыйОписаниеОповещения("ПослеЗакрытияФормыЭТрН_Титул3", ЭтаФорма);
	
	Модуль_Платформа.ПеревозочныеДокументы_ОткрытьФормуЭТрН_Титул3(
		ПараметрыФормы,
		ЭтаФорма,
...
&НаКлиенте
Процедура ПослеЗакрытияФормыЭТрН_Титул3(Результат, ДопПараметры) Экспорт
	
	Если ЗначениеЗаполнено(Результат) Тогда
		
		ОтборПоПакету = ПараметрыДляОбновленияСписка(ТекущийДокумент.Идентификаторы, Неопределено);
		
		ИмяСобытия = "Диадок_ОбновитьГлавныйСписок";
		ПараметрыОповещения = ОтборПоПакету;
		ИсточникОповещения = ИдентификаторОсновнойФормы;
		
		Оповестить(ИмяСобытия, ПараметрыОповещения, ИсточникОповещения);
...
&НаКлиенте
Процедура СведенияОМаркировке(Команда)
	
	ДокументыУчета = УчетныеДокументыПакетаСОснованиями();
	
	КонтурМаркировка_ОткрытьСостояниеМаркировки(ЭтаФорма, ДокументыУчета);
	
	Метрика_ОткрытиеСведенийОМаркировке();
...
Процедура КонтурМаркировка_ОткрытьСостояниеМаркировки(ФормаВызова, ДокументыУчета) Экспорт
	...
	РаботаСМаркировкой = Ядро.Модуль_РаботаСМаркировкой(ЭтаФорма);
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		КонтурМаркировка_ОбработатьОшибку(Действие, ТекстОшибки);
		
	КонецЕсли;
	
КонецПроцедуры

// Обрабатывает ошибку интеграции с Маркировкой
//
// Параметры:
...
//
&НаСервере
Процедура КонтурМаркировка_ОбработатьОшибку(Действие, ТекстОшибки)
	
	Ядро = Модуль_ЯдроНаСервере();
	
	Категория = Ядро.Метрика_НазваниеКатегории().ИнтеграцияСМаркировкой;
	
	ДопПеременные = Новый Соответствие;
	ДопПеременные.Вставить("KonturBaseID", Ядро.ОбщийКонтекст_КонтурИдентификаторИБ());
	
	Ядро.Метрика_ДобавитьОшибку(Категория, Действие, ТекстОшибки, ДопПеременные);
	
	ВидОперации = Категория + "." + Действие;
	Ядро.СписокОшибок_ДобавитьОшибку(ВидОперации, ТекстОшибки);
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьСведенияОМаркировке(ЭтоОбработкаОповещения = Ложь, СписокДокументовДляОповещения = Неопределено)
	
	ОбновитьСведенияОМаркировкеНаСервере();
	
	Если ЭтоОбработкаОповещения И ВОповещенииЕстьДокументыПакета(СписокДокументовДляОповещения) Тогда
		ЗадатьВопросПослеИзмененияДанныхОКодах();
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьСведенияОМаркировкеНаСервере()
	
	ИспользуетсяМодульМаркировки = КонтурМаркировка_ИспользуетсяМодульМаркировки();
	
	Если НЕ ИспользуетсяМодульМаркировки Тогда
		Возврат;
	КонецЕсли;
	
	СведенияОМаркировкеДокументов = СведенияОМаркировкеДокументов();
	
	ИмяКоманды = "СведенияОМаркировке";
	
	КнопкаСведенияОМаркировке = ЭтаФорма.Элементы.Найти(ИмяКоманды);
	КомандаСведенияОМаркировке = ЭтаФорма.Команды.Найти(ИмяКоманды);
	
	Если НЕ КонтурМаркировка_ПоказатьКнопкуСведенияОМаркировке() 
		ИЛИ НЕ СведенияОМаркировкеДокументов.ВидимостьКоманды
		ИЛИ ЭтоПакетСчетИзРТУ() Тогда
		
		Если КнопкаСведенияОМаркировке <> Неопределено Тогда
			КнопкаСведенияОМаркировке.Видимость = Ложь;
		КонецЕсли;
	...
	Если КомандаСведенияОМаркировке = Неопределено Тогда
		
		КомандаСведенияОМаркировке = ЭтаФорма.Команды.Добавить(ИмяКоманды);
		КомандаСведенияОМаркировке.Действие = ИмяКоманды;
	...
	Если КнопкаСведенияОМаркировке = Неопределено Тогда
		...
		КнопкаСведенияОМаркировке = ЭтаФорма.Элементы.Добавить(ИмяКоманды, Тип("КнопкаФормы"), РодительКнопки);
		КнопкаСведенияОМаркировке.ИмяКоманды = ИмяКоманды;
		КнопкаСведенияОМаркировке.Отображение = ОтображениеКнопки.КартинкаИТекст;
		
	КонецЕсли;
	
	КнопкаСведенияОМаркировке.Заголовок = СведенияОМаркировкеДокументов.Заголовок;
	КнопкаСведенияОМаркировке.Картинка = СведенияОМаркировкеДокументов.Картинка;
	КомандаСведенияОМаркировке.Подсказка = СведенияОМаркировкеДокументов.Подсказка;	
	
КонецПроцедуры

&НаСервере
Функция КонтурМаркировка_ПоказатьКнопкуСведенияОМаркировке()
	...
	Значение = ЗначенияОпций.Получить("Mark.Install");
...
//
&НаСервере
Функция КонтурМаркировка_ИспользуетсяМодульМаркировки()
	
	Ядро = Модуль_ЯдроНаСервере();
	
	Результат = Ядро.ЗначениеНастройки_ИспользоватьСервисКонтурМаркировка();
...
&НаКлиенте
Процедура ЗадатьВопросПослеИзмененияДанныхОКодах()
	
	Модуль_Платформа = Модуль_Платформа();
	
	Оповещение = Модуль_Платформа.НовыйОписаниеОповещения("ПослеВопросаОбИзмененииДанныхОКодах", ЭтаФорма);
	ТекстВопроса = "Данные по кодам маркировки были изменены. Перезаполнить пакет?";
...
&НаКлиенте
Процедура ПослеВопросаОбИзмененииДанныхОКодах(Результат, ДополнительныеПараметры) Экспорт
	
	Метрика_ПерезаполнитьПослеИзмененияДанныхОКодах(Результат);
...
&НаКлиенте
Процедура Метрика_ПерезаполнитьПослеИзмененияДанныхОКодах(ДействиеКлиента)
	
	КатегорияМетрики = Метрика_НазваниеКатегории().ИнтеграцияСМаркировкой;
	
	Если ДействиеКлиента = КодВозвратаДиалога.Да Тогда
		ДействиеМетрики = "Перезаполнение документа";
	Иначе
		ДействиеМетрики = "Отказ от перезаполнения документа";
	КонецЕсли;
...
&НаСервере
Функция СведенияОМаркировкеДокументов()
	...
	Результат = МодульРаботаСМаркировкой.СостояниеМаркировки(ДокументыУчета);
...
&НаКлиенте
Процедура Метрика_ОткрытиеСведенийОМаркировке()
	
	Модуль_Ядро = Модуль_ЯдроНаКлиенте();
	
	КатегорияМетрики = Метрика_НазваниеКатегории().ИнтеграцияСМаркировкой;
	ДействиеМетрики = Метрика_НазваниеДействий().ОткрытиеСведенийОМаркировке;
	
	ПредставлениеКнопки = ПредставлениеКнопкиСведенияОМаркировке();
...
&НаКлиенте
Функция ПредставлениеКнопкиСведенияОМаркировке()
	
	ИмяКоманды = "СведенияОМаркировке";
	КнопкаСведенияОМаркировке = ЭтаФорма.Элементы.Найти(ИмяКоманды);
	
	Если КнопкаСведенияОМаркировке <> Неопределено Тогда
		Результат = КнопкаСведенияОМаркировке.Заголовок;
	Иначе
		Результат = "";
	КонецЕсли;
...
&НаСервере
Функция ЭтоПакетСчетИзРТУ()
	// BSLLS:MagicNumber-off
	Результат = Ложь;
	
	Если ЭтаФорма.Пакет.ИдентификаторВида <> "ID_СчетФактура"
		И Лев(ЭтаФорма.Пакет.ИдентификаторВида, 7) = "ID_Счет" Тогда
...
&НаКлиенте
Функция ВОповещенииЕстьДокументыПакета(СписокДокументовДляОповещения)
	
	Результат = Ложь;
	
	Если Не ЗначениеЗаполнено(СписокДокументовДляОповещения) Тогда
		Возврат Результат;
	КонецЕсли;
	
	ДокументыУчета = УчетныеДокументыПакетаСОснованиями();
	
	Для Каждого ДокументДляОповещения Из СписокДокументовДляОповещения Цикл
		Если ДокументыУчета.Найти(ДокументДляОповещения) <> Неопределено Тогда
			Результат = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Функция ПараметрыДляОбновленияСписка(Идентификаторы, ДокументУчета)
	...
	ОтборПоПакету	= ЯдроНаКлиенте.Контракт_ОтборПоПакету();
