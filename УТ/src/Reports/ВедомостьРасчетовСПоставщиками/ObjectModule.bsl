...
Функция ТекстЗапросаРасчетыПоСрокам(МассивВыбранныхПолей, НастройкиОтчета, ВычисляемыеПоля)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&Поставлено", ВзаиморасчетыОтчеты.ШаблонПоляОтгружено());
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&Зачтено", ВзаиморасчетыОтчеты.ШаблонПоляЗачтено());
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&Оплачено", ВзаиморасчетыОтчеты.ШаблонПоляОплачено());
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&СписаниеПереоценка", ВзаиморасчетыОтчеты.ШаблонПоляСписаниеПереоценка());
	
	ВзаиморасчетыОтчеты.ДобавитьОтборыВыбранныхПолейВЗапрос(ТекстЗапроса, НастройкиОтчета, СхемаКомпоновкиДанных.ВычисляемыеПоля, "Расчеты");
...
Функция ТекстЗапросаРасчеты(МассивВыбранныхПолей)
	...
	|	АналитикаУчета.Контрагент                   КАК Контрагент,
	|	АналитикаУчета.Договор                      КАК Договор,
	|	ВЫБОР КОГДА НЕ РасчетыСПоставщиками.ОбъектРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности
	|		ТОГДА АналитикаУчета.НаправлениеДеятельностиАналитики
	|		ИНАЧЕ РасчетыСПоставщиками.ОбъектРасчетов.НаправлениеДеятельности
	|	КОНЕЦ                                       КАК НаправлениеДеятельности,
