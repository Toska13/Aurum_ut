...
Функция РезультатЗапросаПоДаннымОтчета(СтруктураФормированияОтчета)
	...
	ТекстЗапроса = ТекстЗапроса
		+ СтруктураФормированияОтчета.ТекстЗапросыВременныеТаблицы
		+ Символы.ПС
		+ ТекстЗапросаПоВидамСобытий;
...
Функция ТекстЗапросаЗаявкиНаВозврат(Запрос,СтруктураФормированияОтчета)
	...
	Если СтруктураФормированияОтчета.ИспользуетсяНоваяАрхитектураРасчетов Тогда
		...
		ПараметрыФормированияТаблицы.ТекстУсловия =
			"Распоряжение.ВозвращаемыеТовары.ДатаПоступления < &ДатаОкончания
			|			И Распоряжение.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.Квозврату)
			|		ИЛИ Распоряжение.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.Кобеспечению)
			|		ИЛИ Распоряжение.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КОтгрузке)
			|			И &ТекстУсловияПараметрыВТОтбор";
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_РаспоряженияНаОтгрузкуОборотыРазвернутая КАК ТаблицаОстатков
		|		ПО ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Ссылка = ТаблицаОстатков.Распоряжение
		|			И ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.КодСтроки = ТаблицаОстатков.КодСтроки
		|ГДЕ
		...
		|	И (ТаблицаОстатков.КОформлениюПоВозвратамКоличествоОборот -
		|		ТаблицаОстатков.ОформленоПоВозвратамКоличествоОборот) > 0
		|	И &ТекстУсловияОтборДокумент
		|	
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|	ЛЕВОЕ СОЕДИНЕНИЕ #РаспоряженияНаОтгрузкуОборотыРазвернутая КАК РаспоряженияОбороты
		|	ПО
		|		ДокументЗаявкаНаВозвратТоваровОтКлиента.Ссылка = РаспоряженияОбороты.Распоряжение
		|		И (РаспоряженияОбороты.КОформлениюКоличествоОборот -
		...
		|ГДЕ
		|	ЕСТЬNULL(РаспоряженияОбороты.КОформлениюКоличествоОборот - РаспоряженияОбороты.ОформленоКоличествоОборот, 0) <>
	...
	Иначе
		...
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаВозвратТоваровОтКлиента.ЗаменяющиеТовары КАК ЗаявкаНаВозвратТоваровОтКлиентаТовары
		|			ПО ДокументЗаявкаНаВозвратТоваровОтКлиента.Ссылка = ЗаявкаНаВозвратТоваровОтКлиентаТовары.Ссылка
		|	ЛЕВОЕ СОЕДИНЕНИЕ #РаспоряженияНаОтгрузкуОборотыРазвернутая КАК РаспоряженияНаОтгрузкуОбороты
		|	ПО
		|		ДокументЗаявкаНаВозвратТоваровОтКлиента.Ссылка = РаспоряженияНаОтгрузкуОбороты.Распоряжение
		|		И (РаспоряженияНаОтгрузкуОбороты.КОформлениюКоличествоОборот -
		...
		|		ДокументЗаявкаНаВозвратТоваровОтКлиента.Ссылка = ТоварыКОтгрузкеОстатки.ДокументОтгрузки
		|ГДЕ
		|	ЕСТЬNULL(РаспоряженияНаОтгрузкуОбороты.КОформлениюКоличествоОборот -
		|		РаспоряженияНаОтгрузкуОбороты.ОформленоКоличествоОборот, 0) <>
		|		ЕСТЬNULL(ТоварыКОтгрузкеОстатки.КОтгрузкеОстаток, 0)
		|	И &ТекстУсловияОтборДокумент
		|
	...
	Если НЕ ПустаяСтрока(СтруктураФормированияОтчета.ТекстУсловияПоПользователям) Тогда
		...
		ТекстУсловияПараметрыВТОтбор = "
		|И Распоряжение.Менеджер" + СтруктураФормированияОтчета.ТекстУсловияПоПользователям;
		ТекстУсловияОтбор ="
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"И &ТекстУсловияПараметрыВТОтбор", ТекстУсловияПараметрыВТОтбор);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"И &ТекстУсловияОтборДокументОтгрузки",ТекстУсловияОтборДокументОтгрузки);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"И &ТекстУсловияОтборДокумент",ТекстУсловияОтборДокумент);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"И &ТекстУсловияОтборПланОплат", ТекстУсловияОтборПланОплат);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"И &ТекстУсловияОтбор",ТекстУсловияОтбор);
...
Функция ТекстЗапросаЗаказыКлиентов(Запрос,СтруктураФормированияОтчета)
	...
	Если СтруктураФормированияОтчета.ИспользуетсяНоваяАрхитектураРасчетов Тогда
		...
		|ГДЕ
		|	ЕСТЬNULL(РаспоряженияНаОтгрузкуОбороты.КОформлениюКоличествоОборот -
		|		РаспоряженияНаОтгрузкуОбороты.ОформленоКоличествоОборот, 0) > 0 
		|	И ЕСТЬNULL(ТоварыКОтгрузкеОстатки.КОтгрузкеОстаток, 0) > 0 
		|	И ЕСТЬNULL(РаспоряженияНаОтгрузкуОбороты.КОформлениюКоличествоОборот -
		|		РаспоряженияНаОтгрузкуОбороты.ОформленоКоличествоОборот, 0) <> ЕСТЬNULL(ТоварыКОтгрузкеОстатки.КОтгрузкеОстаток, 0)
		|	И &ТекстУсловияОтборДокумент
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		ПараметрыФормированияТаблицы = РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат.НовыйПараметрыФормированияРазвернутойТаблицы();
		ПараметрыФормированияТаблицы.КонецПериода = "&ДатаОкончания";
		ПараметрыФормированияТаблицы.ТекстУсловия =
			...
			|			ИЛИ Распоряжение.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.КОбеспечению))
			|	И &ТекстУсловияОтборЗаказКлиента";
		ПараметрыФормированияТаблицы.Группировка = "Распоряжение";
		ПараметрыФормированияТаблицы.Показатели = "КОформлению, Оформлено";
		ПараметрыФормированияТаблицы.Ресурсы = "Количество";
