...
// 
// Параметры: 
//  ДокументСсылка - ДокументСсылка, СписокЗначений Из ДокументСсылка - массив документов.
// 
// Возвращаемое значение:
...
Функция ДанныеИнформационнойБазы(ДокументСсылка) Экспорт
	...
	ВидПродукции = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукции(ДанныеШапки.ТоварнаяГруппа);
					...
					Если ЗаполнитьДанныеДекларацииПоДеревуУпаковок Тогда
						...
						Если КешДанныхРазбора[ИсходнаяСтрока[ИменаКолонок.НормализованныйКодМаркировки]] = Неопределено Тогда
							КешДанныхРазбора.Вставить(
								...
								НормализованныеДанныеРазбора(СтрокаДанных, ИменаКолонок)
);
						КонецЕсли;
	...
	Иначе
		...
		Иначе
			...
			Маркировка = ЭлектронноеВзаимодействиеИСМП.ЧастичноеСодержимое(ТаблицаУпаковок,, Истина);
...
Функция НормализованныеДанныеРазбора(СтрокаДанных, ИменаКолонок)
	
	Результат = Новый Структура;
	Результат.Вставить("ВидУпаковки",               СтрокаДанных[ИменаКолонок.ВидУпаковки]);
	Результат.Вставить("ТипШтрихкода",              СтрокаДанных[ИменаКолонок.ТипШтрихкода]);
	Результат.Вставить("EAN",                       СтрокаДанных[ИменаКолонок.EAN]);
	Результат.Вставить("GTIN",                      СтрокаДанных[ИменаКолонок.GTIN]);
	Результат.Вставить("КоличествоВложенныхЕдиниц", СтрокаДанных[ИменаКолонок.КоличествоВложенныхЕдиниц]);
	Результат.Вставить("ДанныеРазбора",             СтрокаДанных[ИменаКолонок.ДанныеРазбора]);
	Результат.Вставить("КодДляПередачиИСМП",        СтрокаДанных[ИменаКолонок.КодДляПередачиИСМП]);
	Возврат Результат;
