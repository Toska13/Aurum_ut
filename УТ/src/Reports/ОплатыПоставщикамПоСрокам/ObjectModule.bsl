...
Процедура ПриСозданииНаСервере(ЭтаФорма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Параметры.Свойство("ПараметрКоманды") Тогда 
		...
		ОбъектыРасчетов = Документы.КорректировкаГрафикаВзаиморасчетов.ОбъектыРасчетовКорректировок(Параметры.ПараметрКоманды);
		ЗначениеОтбора = Новый СписокЗначений;
		ЗначениеОтбора.ЗагрузитьЗначения(ОбъектыРасчетов);
...
Функция ТекстЗапроса()
	...
	|	АналитикаУчета.Контрагент                КАК Контрагент,
	|	АналитикаУчета.Договор                   КАК Договор,
	|	ВЫБОР КОГДА НЕ РасчетыПланОплат.ОбъектРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности
	|		ТОГДА АналитикаУчета.НаправлениеДеятельностиАналитики
	|		ИНАЧЕ РасчетыПланОплат.ОбъектРасчетов.НаправлениеДеятельности
	|	КОНЕЦ КАК НаправлениеДеятельности,
	...
	|	АналитикаУчета.Контрагент                   КАК Контрагент,
	|	АналитикаУчета.Договор                      КАК Договор,
	|	ВЫБОР КОГДА НЕ РасчетыПланПоставок.ОбъектРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности
	|		ТОГДА АналитикаУчета.НаправлениеДеятельностиАналитики
	|		ИНАЧЕ РасчетыПланПоставок.ОбъектРасчетов.НаправлениеДеятельности
	|	КОНЕЦ КАК НаправлениеДеятельности,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ВЫРАЗИТЬ(&СтрокаСостояниеВзаиморасчетов КАК СТРОКА(150))    КАК ГруппировкаВсего,
	...
	|	
	|	СУММА(ТаблицаЗадолженностей.НашДолг)                        КАК НашДолг,
	|	СУММА(ТаблицаЗадолженностей.НашДолгПросрочено)              КАК НашДолгПросрочено,
	|	СУММА(ТаблицаЗадолженностей.ДолгПоставщика)                 КАК ДолгПоставщика,
	|	СУММА(ТаблицаЗадолженностей.ДолгПоставщикаПросрочено)       КАК ДолгПоставщикаПросрочено,
	|	СУММА(ТаблицаЗадолженностей.ДолгПоставщикаНеКонтролируется) КАК ДолгПоставщикаНеКонтролируется,
	|	СУММА(ТаблицаЗадолженностей.КПоступлению)                   КАК КПоступлению,
	|	
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА НЕ РазностиДат.КоличествоДней ЕСТЬ NULL 
	|			ТОГДА ВЫБОР
	|					КОГДА РазностиДат.КоличествоДней > 0
	|						И ТаблицаЗадолженностей.ДолгПоставщика > 0
	...
	|			ТОГДА ВЫРАЗИТЬ(&СтрокаДолгНеКонтролируется КАК СТРОКА(150))
	|		ИНАЧЕ ЕСТЬNULL(Интервалы.НаименованиеИнтервала, ВЫРАЗИТЬ(&СтрокаДолгНеПросрочен КАК СТРОКА(150))) 
	...
	|ПОМЕСТИТЬ ЗадолженностьПоИнтервалам
	|ИЗ
	|	ТаблицаЗадолженностей КАК ТаблицаЗадолженностей
	...
	|	Интервалы.НаименованиеИнтервала,
	|	Интервалы.НомерСтроки,
	|	Интервалы.НижняяГраницаИнтервала
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	Таблица.ГруппировкаВсего               КАК ГруппировкаВсего,
	...
	|
	|	Таблица.НашДолг                        КАК НашДолг,
	|	Таблица.НашДолгПросрочено              КАК НашДолгПросрочено,
	|	Таблица.ДолгПоставщика                 КАК ДолгПоставщика,
	|	Таблица.ДолгПоставщикаПросрочено       КАК ДолгПоставщикаПросрочено,
	|	Таблица.ДолгПоставщикаНеКонтролируется КАК ДолгПоставщикаНеКонтролируется,
	|	Таблица.КПоступлению                   КАК КПоступлению,
	|
	|	Таблица.КоличествоДней                 КАК КоличествоДней,
	|
	|	Таблица.НаименованиеИнтервала          КАК НаименованиеИнтервала,
	|	Таблица.НомерИнтервала                 КАК НомерИнтервала,
	|	Таблица.НижняяГраницаИнтервала         КАК НижняяГраницаИнтервала
	|{ВЫБРАТЬ
	|	ГруппировкаВсего,
	|	Организация.*,
	|	Партнер.*,
	...
	|	НашДолгПросрочено,
	|	ДолгПоставщика,
	|	ДолгПоставщикаПросрочено,
	|	ДолгПоставщикаНеКонтролируется,
	|	КПоступлению,
	|	КоличествоДней,
	|	НаименованиеИнтервала,
	|	НомерИнтервала,
	|	НижняяГраницаИнтервала}
	|ИЗ ЗадолженностьПоИнтервалам КАК Таблица
	|{ГДЕ
	|	ГруппировкаВсего               КАК ГруппировкаВсего,
	...
	|	
	|	ДатаПлановогоПогашения         КАК ДатаПлановогоПогашения,
	|	ДатаВозникновения              КАК ДатаВозникновения,
	|	
	|	НашДолг                        КАК НашДолг,
	|	НашДолгПросрочено              КАК НашДолгПросрочено,
	|	ДолгПоставщика                 КАК ДолгКлиента,
	|	ДолгПоставщикаПросрочено       КАК ДолгКлиентаПросрочено,
	|	ДолгПоставщикаНеКонтролируется КАК ДолгПоставщикаНеКонтролируется,
	|	КПоступлению                   КАК КПоступлению,
	|	КоличествоДней                 КАК КоличествоДней,
	|	
	|	НаименованиеИнтервала          КАК НаименованиеИнтервала,
	|	НомерИнтервала                 КАК НомерИнтервала,
	|	НижняяГраницаИнтервала         КАК НижняяГраницаИнтервала}
	|";
