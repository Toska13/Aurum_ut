...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ДополнитьСтроки(ДокументРезультат, ДанныеПоиска);
...
//  1. Если пользователь отключил все выводимые поля, то возвращается №2, поскольку в отчет всегда выводится порядковый
//  номер и картинка.
...
//  3. Если ни одна из указанных в п.2 колонок не найдена, то возвращается №3 - первая колонка после картинки и 
//  порядкового номера.
//
// Параметры:
...
Функция НомерКолонкиГиперссылка(ВыводимыеПоля)
	...
	Для Каждого ИмяПоля Из ИменаПолейДляПоиска Цикл
		...
		НомерПоля = НомерПоля + КолонкаНомерПП() + 1;
		Возврат НомерПоля;
		
	КонецЦикла;
	
	Возврат КолонкаНомерПП() + 1;
...
// Дополняет отчет информацией из прикладной логики. Заполнение дополнительной информации выполняется в методе
// ПоискКонтрагентовПереопределяемый.ПриНастройкеОтображенияОтчета.
// 
// Параметры:
//  ДокументРезультат - ТабличныйДокумент - результат формирования отчета.
//  ДанныеПоиска - см. ПоискКонтрагентовСлужебный.ВнешниеДанныеДляОтчета.
//
Процедура ДополнитьСтроки(ДокументРезультат, ДанныеПоиска)

	НомерКолонкиКартинка = 1;
	КолонкаНомерПП = КолонкаНомерПП();
	
	НастройкиОтчета = Новый Структура(
		"ИНН, ДополнительныеДанные",
		ДанныеПоиска.ИНН,
		Новый Соответствие);
	...
	Если НастройкиОтчета.ДополнительныеДанные.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для НомерСтроки = 2 По ДокументРезультат.ВысотаТаблицы Цикл
		...
		ИННСтроки = ДанныеПоиска.ИННПоСтрокам.Получить(
			КлючСтроки);
		ДополнительныеДанныеСтроки = НастройкиОтчета.ДополнительныеДанные.Получить(
			ИННСтроки);
		
		Если ТипЗнч(ДополнительныеДанныеСтроки) <> Тип("Структура") Тогда
			Продолжить;
		КонецЕсли;
		
		Эталон = ПоискКонтрагентов.НовыйНастройкиСтроки();
		ЗаполнитьЗначенияСвойств(Эталон, ДополнительныеДанныеСтроки);
		
		Ячейка = ДокументРезультат.Область(
			НомерСтроки,
			НомерКолонкиКартинка,
			НомерСтроки,
			НомерКолонкиКартинка);
		
		Если ЗначениеЗаполнено(Эталон.Картинка) Тогда
			Попытка
				Ячейка.Картинка = БиблиотекаКартинок[Эталон.Картинка];
			Исключение
				ПоискКонтрагентовСлужебный.ЗаписатьИнформациюВЖурналРегистрации(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Не удалось установить картинку для контрагента с ИНН %1:
						|
						|%2'"),
						ИННСтроки,
		...
		Если ЗначениеЗаполнено(Эталон.ПримечаниеКартинки) Тогда
			Ячейка.Примечание.Текст = Эталон.ПримечаниеКартинки;
		КонецЕсли;
...
// Возвращает номер колонки, в которую выводится порядковый номер строки.
//
// Возвращаемое значение:
//  Число - номер колонки.
//
Функция КолонкаНомерПП()
