...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыОтображения.Вставить("ЗаголовокОкна",             "");
	ПараметрыОтображения.Вставить("ПоложениеКнопкиПоиска",     "");
	ПараметрыОтображения.Вставить("ЗаголовокКнопкиПоиска",     "");
	ПараметрыОтображения.Вставить("КартинкаКнопкиПоиска",      "");
	...
	Если ПоискКонтрагентовСлужебныйКлиентСервер.ЭтоСтруктура(Параметры.ПараметрыПоиска) Тогда
		...
		ЗаполнитьЗначенияСвойств(ПараметрыОтображения, Параметры.ПараметрыПоиска);
	...
	НастроитьВнешнийВид(ПараметрыОтображения);
...
Процедура ПриОткрытии(Отказ)
	...
	Если НужноОбновитьПараметры Тогда
		НужноЗагрузитьСохраненныеНастройки = Истина;
...
Процедура Подключаемый_БыстрыйДоступПриИзменении(Элемент)
	...
	Если ИзмененияОбработаны Тогда
		ИзмененияОбработаны = Ложь;
		ЗначениеПредставленияИзСписка(Элемент);
		Возврат;
	КонецЕсли;
	...
	Если Элемент.Имя = ОписаниеПоля.РеквизитПометка Тогда
		ОбработкаИзмененияГалкиБыстрогоДоступа(ОписаниеПоля);
	Иначе
		ОбработкаИзмененияПоляБыстрогоДоступа(ОписаниеПоля, Элемент);
	КонецЕсли;
...
Процедура Подключаемый_БыстрыйДоступОбработкаВыбора(
	...
	Если ОписаниеПоля.ВыборИзСправочника
		...
		Иначе
			...
			Если ОписаниеПоля.ЭтоПолеОКВЭД 
				И ПравильноеЗначение.Данные = "ПустоеЗначение" Тогда
				СтандартнаяОбработка = Ложь;
				Возврат;
			КонецЕсли;
			...
			Если ОписаниеПоля.ВидСравнения = ВидыСравнений.ВСписке.Значение Тогда
				...
				ПоместитьЗначениеВРеквизитСписок(
					ПравильноеЗначение,
					ОписаниеПоля);
...
&НаКлиенте
Процедура Подключаемый_УдалениеМножественныхЗначений(Элемент, Идентификаторы, СтандартнаяОбработка)
	...
	ЕстьИзменения = Ложь;
	Если ТипЗнч(ТекущийОтбор) = Тип("Структура") Тогда
		ТекущийСписок = ТекущийОтбор.Значение;
		Если ТипЗнч(ТекущийСписок) = Тип("СписокЗначений") Тогда
			
			ЭлементыДляУдаления = Новый Массив;
			Для Каждого Идентификатор Из Идентификаторы Цикл
				
				ЭлементСписка = ЭтотОбъект[ОписаниеПоля.РеквизитСписок].НайтиПоИдентификатору(Идентификатор);
				Если ЭлементСписка = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				
				Для Каждого ЭлементТекущегоСписка Из ТекущийСписок Цикл
					Если ЭлементТекущегоСписка.Представление = ЭлементСписка.Значение Тогда
						ЭлементыДляУдаления.Добавить(ЭлементТекущегоСписка);
					КонецЕсли;
				КонецЦикла;
			...
			Для Каждого ЭлементДляУдаления Из ЭлементыДляУдаления Цикл
				ТекущийСписок.Удалить(ЭлементДляУдаления);
	...
	Если ЕстьИзменения Тогда
		...
		Для Каждого ЭлементСписка Из ТекущийСписок Цикл
			НовоеПредставление.Добавить(ЭлементСписка.Представление);
		КонецЦикла;
		ТекущийОтбор.ПредставлениеЗначения = НовоеПредставление;
	КонецЕсли;
...
Процедура РезультатОбработкаРасшифровки(
	...
	ИначеЕсли Расшифровка = "ПереходКПокупкеСервиса" Тогда
		...
		ИнтернетПоддержкаПользователейКлиент.ОткрытьСтраницуИнтегрированногоСайта(
			"https://its.1c.ru/db/aboutitsnew/content/23/hdoc?order[comment]=%D0%97%D0%B0%D0%BA%D0%B0%D0%B7%20%D0%BD%D0%B0%201%D0%A1:%D0%9A%D0%BE%D0%BD%D1%82%D1%80%D0%B0%D0%B3%D0%B5%D0%BD%D1%82)");
		
	ИначеЕсли Расшифровка = "ПодключитьТестовыйПериод" Тогда
...
Процедура БыстрыеНастройки(Команда)
	...
	ОткрытьФормуНастроек("БыстрыеНастройки");
...
Процедура Настройки(Команда)
	...
	ОткрытьФормуНастроек("ВсеНастройки");
	
КонецПроцедуры

&НаКлиенте
Процедура НастроитьКолонкиТаблицы(Команда)
	
	ОткрытьФормуНастроек("Колонки");
...
Процедура СтандартныеНастройки(Команда)
	...
	Если ВидКонтрагента = Неопределено Тогда
		ПоказатьПредупреждение(
			Неопределено,
			НСтр("ru = 'Не указан вид контрагента'"));
		Возврат;
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения(
		"НастройкиПоУмолчаниюОбработкаОтвета",
		ЭтотОбъект);
		
	ПоказатьВопрос(
		Оповещение,
		НСтр("ru = 'Все настройки отборов для указанного вида контрагентов будут очищены.
			|Продолжить?'"),
...
&НаКлиенте
Процедура ИзменениеОтборовИПолейЗавершение(
		РезультатОткрытия,
		ДополнительныеПараметры) Экспорт
	...
	ПравильнаяСтруктура = Новый Структура(
		"НастройкаОтборов, ВыбранныеПоля");
	
	ЗаполнитьЗначенияСвойств(ПравильнаяСтруктура, РезультатОткрытия);
	
	ИзменениеОтборовЗавершение(
		ПравильнаяСтруктура.НастройкаОтборов,
		ДополнительныеПараметры);
		
	ИзменениеПолейЗавершение(
		ПравильнаяСтруктура.ВыбранныеПоля,
		ДополнительныеПараметры);
...
Процедура ЗагрузитьНастройкиИзБазы(ЭтоСозданиеФормы)
	...
	Если Не НужноОбновитьПараметры Тогда
		ЗагрузитьНастройкиОтбораИПолей(ЭтоСозданиеФормы);
	КонецЕсли;
	
	ЕстьСохраненныеНастройки = (ПараметрыПоиска.ДоступныеОтборы.Количество() > 0);
	
КонецПроцедуры

&НаСервере
Процедура ЗагрузитьНастройкиОтбораИПолей(ЭтоСозданиеФормы, ВыводитьОшибки = Истина)
	...
	Если ВыводитьОшибки И СообщенияПользователю.Количество() > 0 Тогда
	...
	НужноЗагрузитьСохраненныеНастройки = Ложь;
...
Процедура СоздатьГруппыБыстрогоДоступа()
	...
	Колонки = СтрРазделить("Лево,Право", ",");
	...
	Для Каждого КлючЗначение Из ВидыКонтрагентов Цикл
		...
		Для Каждого Колонка Из Колонки Цикл
			...
			РодительЭлемента = "БыстрыйДоступ" + Колонка;
			
			НоваяГруппа = Элементы.Добавить(
				РодительЭлемента + ТекущийВидКонтрагента,
				Тип("ГруппаФормы"),
				Элементы[РодительЭлемента]);
			НоваяГруппа.Вид                       = ВидГруппыФормы.ОбычнаяГруппа;
			НоваяГруппа.Группировка               = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
			НоваяГруппа.Отображение               = ОтображениеОбычнойГруппы.Нет;
			НоваяГруппа.ОтображатьЗаголовок       = Ложь;
			НоваяГруппа.РазрешитьИзменениеСостава = Ложь;
...
Процедура ОбновитьБыстрыйДоступ()
	...
	Если НужноЗагрузитьСохраненныеНастройки Тогда
		ЗагрузитьНастройкиОтбораИПолей(Ложь, НужноОбновитьПараметры);
	КонецЕсли;
	...
	Для Каждого КлючЗначение Из ПараметрыПоиска.БыстрыйДоступ Цикл
		...
		УдаляемыеРеквизиты.Добавить(КлючЗначение.Значение.РеквизитПометка);
	КонецЦикла;
	...
	Для Каждого КлючЗначение Из ВидыКонтрагентов Цикл
		...
		Если НастройкиОтборовПоВиду = Неопределено
			...
			// Пробуем задать настройки отборов по умолчанию, для вывода на форму.
			НастройкиОтборовПоУмолчанию(ТекущийВидКонтрагента);
			
			// Еще раз проверяем, поскольку по определенному виду контрагента может вообще
			// не быть отборов.
			НастройкиОтборовПоВиду = ПараметрыПоиска.НастройкаОтборов[ТекущийВидКонтрагента];
			Если НастройкиОтборовПоВиду = Неопределено
				Или НастройкиОтборовПоВиду.Количество() = 0 Тогда
		...
		Для Каждого ДоступныйОтбор Из ВсеОтборы Цикл
			...
			ОтборыДляВывода.Добавить(
				ДоступныйОтбор,
				Формат(ПользовательскаяНастройка.Порядок, "ЧЦ=4; ЧВН=; ЧГ="));
		...
		Для Каждого ЭлементОтбора Из ОтборыДляВывода Цикл
			...
			ИмяРеквизитаПометка   = "БыстрыйДоступПометка" + НовыйИндекс;
			...
			ОписаниеПоля.РеквизитПометка       = ИмяРеквизитаПометка;
	...
	Для каждого ИндексПоля Из ПорядокЭлементов Цикл
		...
		ДобавитьГалкуБыстрыйДоступ(НоваяГруппа, ОписаниеПоля);
	...
	Для каждого КлючЗначение Из ВидыКонтрагентов Цикл
		ТекущийВидКонтрагента = КлючЗначение.Значение;
		ВидимостьЭлемента = ТекущийВидКонтрагента = ВидКонтрагента;
...
&НаСервере
Процедура НастройкиОтборовПоУмолчанию(ТекущийВидКонтрагента)
	
	ДоступныеОтборы = ПараметрыПоиска.ДоступныеОтборы.Получить(ТекущийВидКонтрагента);
	
	Если ДоступныеОтборы = Неопределено
		Или ДоступныеОтборы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	НоваяНастройкаОтборов = Новый Соответствие;
	ТипОКВЭД = ПоискКонтрагентовСлужебный.ТипыОбработкиПолей().ОКВЭД;
	
	НастройкиПоУмолчанию = ПоискКонтрагентовСлужебный.НастройкиПоУмолчанию().Получить(ТекущийВидКонтрагента);
	ТекущееКоличество = 0;
	...
	Для Каждого ДоступныйОтбор Из ДоступныеОтборы Цикл
		...
		// Подбор варианта сравнения
		СравнениеПоУмолчанию = ПоискКонтрагентовСлужебныйКлиентСервер.ВидСравненияПоУмолчанию(
			ДоступныйОтбор.ВариантыСравнения);
		Если СравнениеПоУмолчанию = "" Тогда
			// Если нет ни одного варианта сравнения - пропускаем.
			Продолжить;
		КонецЕсли;
		
		НовыйОтбор = ПоискКонтрагентовСлужебныйКлиентСервер.НовыйОписаниеПоляОтбора();
		ЗаполнитьЗначенияСвойств(НовыйОтбор, ДоступныйОтбор);

		НовыйОтбор.ЭтоПолеОКВЭД = (ДоступныйОтбор.ТипОбработки = ТипОКВЭД);
		НовыйОтбор.ВидСравнения = СравнениеПоУмолчанию;

		Если ТекущееКоличество < НастройкиПоУмолчанию.КоличествоКритериевВБыстромДоступе Тогда
			НовыйОтбор.БыстрыйДоступ = Истина;
			ТекущееКоличество = ТекущееКоличество + 1;
		КонецЕсли;
		
		НоваяНастройкаОтборов.Вставить(НовыйОтбор.Идентификатор, НовыйОтбор);
		
	КонецЦикла;
	
	ПараметрыПоиска.НастройкаОтборов.Вставить(ТекущийВидКонтрагента, НоваяНастройкаОтборов);
...
&НаСервере
Процедура ДобавитьГалкуБыстрыйДоступ(
		Положение,
		ОписаниеПоля)
		
	НовыйЭлемент = Элементы.Добавить(
		ОписаниеПоля.РеквизитПометка,
		Тип("ПолеФормы"),
		Положение);
	НовыйЭлемент.Вид                = ВидПоляФормы.ПолеФлажка;
	НовыйЭлемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	НовыйЭлемент.ПутьКДанным        = ОписаниеПоля.РеквизитПометка;
	
	НовыйЭлемент.УстановитьДействие("ПриИзменении", "Подключаемый_БыстрыйДоступПриИзменении");
	
	ЭтотОбъект[ОписаниеПоля.РеквизитПометка] = ОписаниеПоля.Пометка;
...
Процедура ДобавитьЭлементБыстрыйДоступ(
	...
	НовыйЭлемент.ОтображатьФлажкиВВыпадающемСпискеПриВводеМножественныхЗначений = ОтображатьФлажок;
	...
	НовыйЭлемент.УстановитьДействие("УдалениеМножественныхЗначений", "Подключаемый_УдалениеМножественныхЗначений");
...
Функция ОписаниеПоля(Элемент)
	...
	ИндексПоля = СтрЗаменить(ИндексПоля,  "БыстрыйДоступПометка", "");
...
&НаКлиенте
Функция ПолучитьЗначениеИзНастроек(ОписаниеПоля)
	
	НастройкаОтборов = ПараметрыПоиска.НастройкаОтборов[ОписаниеПоля.ВидКонтрагента];
	Возврат НастройкаОтборов[ОписаниеПоля.ИдентификаторКритерия];
...
&НаКлиенте
Функция ЗначениеДобавлено(ТекущийСписок, Представление)
	
	Для Каждого ЭлементТекущего Из ТекущийСписок Цикл
		Если ЭлементТекущего.Представление = Представление Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура ЗначениеПредставленияИзСписка(Элемент)
	...
	Если (ОписаниеПоля.ВыборИзСправочника
		...
		ПредставлениеОтбора = ЭтотОбъект[ОписаниеПоля.РеквизитСписок];
		ПредставлениеОтбора.Очистить();
		
		ТекущийОтбор = ПолучитьЗначениеИзНастроек(ОписаниеПоля);
		Эталон = ПоискКонтрагентовСлужебныйКлиентСервер.НовыйОписаниеПоляОтбора();
		ЗаполнитьЗначенияСвойств(Эталон, ТекущийОтбор);
		Если ТипЗнч(Эталон.Значение) = Тип("СписокЗначений") Тогда
			Для Каждого ЗначениеОтбора Из Эталон.Значение Цикл
				ПредставлениеОтбора.Добавить(ЗначениеОтбора.Представление);
			КонецЦикла;
		КонецЕсли;
		
		БыстрыйДоступПометка = ПредставлениеОтбора.Количество() > 0;
...
&НаКлиенте
Процедура ОбработкаИзмененияПоляБыстрогоДоступа(ОписаниеПоля, Элемент)
	...
	ЭтоСписок = ОписаниеПоля.ВидСравнения = ВидыСравнений.ВСписке.Значение;
	...
	ИсходноеЗначениеГалки = ЭтотОбъект[ОписаниеПоля.РеквизитПометка];
	...
	Иначе
		...
		ИначеЕсли (ОписаниеПоля.ВыборИзСправочника
			...
			// Ничего не делаем.
			// Обработка в событии Подключаемый_УдалениеМножественныхЗначений
			ЗначениеПредставленияИзСписка(Элемент);
	...
	Если ИсходноеЗначениеГалки <> БыстрыйДоступПометка Тогда
		...
		ЭтотОбъект[ОписаниеПоля.РеквизитПометка] = БыстрыйДоступПометка;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработкаИзмененияГалкиБыстрогоДоступа(ОписаниеПоля)
	...
	НовоеЗначение = ЭтотОбъект[ОписаниеПоля.РеквизитПометка];
	...
	НовыйОтбор.Пометка = НовоеЗначение;
...
&НаКлиенте
Процедура ПоместитьЗначениеВРеквизитСписок(НовоеЗначение, ОписаниеПоля)
	
	ТекущийОтбор = ПолучитьЗначениеИзНастроек(ОписаниеПоля);
	Эталон = ПоискКонтрагентовСлужебныйКлиентСервер.НовыйОписаниеПоляОтбора();
	ЗаполнитьЗначенияСвойств(Эталон, ТекущийОтбор);
	Если ТипЗнч(Эталон.Значение) = Тип("СписокЗначений") Тогда
		ТекущийСписок = Эталон.Значение;
	Иначе
		ТекущийСписок = Новый СписокЗначений;
	КонецЕсли;
	
	Если ЗначениеДобавлено(
		ТекущийСписок,
		НовоеЗначение.Представление) Тогда
		Возврат;
	КонецЕсли;
	
	ТекущийСписок.Добавить(
		НовоеЗначение,
		НовоеЗначение.Представление);
...
Процедура ВыполнитьПереходНаСтраницу(Знач НомерСтраницы)
	...
	Элементы.ПереходПоНомеру.Доступность = ВсегоСтраниц > 1;
...
Функция ПодготовитьПараметрыОтчета(
	...
	Если НовыйЗапрос Тогда
		ЗаполнитьПараметрыЗапроса(ПараметрыОтчета.ПараметрыЗапроса, Истина);
	Иначе
...
Процедура ЗаполнитьПараметрыЗапроса(
	...
	НезаполненныеКритерии = Новый Массив;
	...
	Для Каждого КлючЗначение Из ПараметрыПоиска.НастройкаОтборов[ВидКонтрагента] Цикл
		...
		Если ОписаниеОтбора.ВыборИзСправочника
			...
			Иначе
				...
				Если ПравильноеЗначение = Неопределено Тогда
					...
					Иначе
						НезаполненныеКритерии.Добавить(ОписаниеОтбора.Идентификатор);
						Продолжить;
					КонецЕсли;
				Иначе
		...
		Если Не ЗначениеЗаполнено(ОписаниеОтбора.Значение) Тогда
			НезаполненныеКритерии.Добавить(ОписаниеОтбора.Идентификатор);
		КонецЕсли;
		
	КонецЦикла;
	
	Если НезаполненныеКритерии.Количество() > 0 И ВызыватьОшибку Тогда
		...
		Для Каждого КлючЗначение Из ПараметрыПоиска.БыстрыйДоступ Цикл
			Если КлючЗначение.Значение.ВидКонтрагента <> ВидКонтрагента
				Или НезаполненныеКритерии.Найти(КлючЗначение.Значение.ИдентификаторКритерия) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Укажите значение отбора у критерия ""%1"" либо отключить его использование'"),
				КлючЗначение.Значение.Представление);
			Представления.Добавить(КлючЗначение.Значение.Представление);
			
			Если КлючЗначение.Значение.ВидСравнения = ВидыСравнений.ВСписке.Значение Тогда
				ПутьКДаннымСообщения = КлючЗначение.Значение.РеквизитСписок;
			Иначе
				ПутьКДаннымСообщения = КлючЗначение.Значение.Реквизит;
			КонецЕсли;
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				...
				,
				,
				ПутьКДаннымСообщения);
			
		КонецЦикла;
		
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Необходимо указать значение отбора у следующих критериев либо отключить их использование:
			|%1'"),
...
Процедура ВыбранныеПоляВПараметрыЗапроса(ПараметрыЗапроса)
	...
	ПоляВОтборах = Новый Массив;
	Для Каждого КлючЗначение Из ПараметрыПоиска.НастройкаОтборов[ВидКонтрагента] Цикл
		
		ОписаниеОтбора = ПоискКонтрагентовСлужебныйКлиентСервер.НовыйОписаниеПоляОтбора();
		ЗаполнитьЗначенияСвойств(ОписаниеОтбора, КлючЗначение.Значение);
		
		Если ОписаниеОтбора.Пометка Тогда
			Если ОписаниеОтбора.Идентификатор = "CORP_ANYNAME" Тогда
				ПоляВОтборах.Добавить("CORP_NAME");
				ПоляВОтборах.Добавить("CORP_FULLNAME");
			Иначе
				ПоляВОтборах.Добавить(ОписаниеОтбора.Идентификатор);
			КонецЕсли;
		КонецЕсли;
	...
	Если НастройкаПолей <> Неопределено Тогда
		Для Каждого НастройкаПоля Из НастройкаПолей Цикл
			...
			Идентификатор = НастройкаПоля.Идентификатор;
			
			Если НастройкаПоля.Пометка Тогда
				ПараметрыЗапроса.Поля.Добавить(Идентификатор);
			Иначе
				Если ПоляВОтборах.Найти(Идентификатор) <> Неопределено Тогда
					НастройкаПоля.Пометка = Истина;
					ПараметрыЗапроса.Поля.Добавить(Идентификатор);
				КонецЕсли;
			КонецЕсли;
...
Процедура ПроверитьНастройкиПослеИзменения()
	...
	Если НастройкиОтличаются() Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСостояниеПоляТабличногоДокумента(
			Элементы.Результат,
			"Неактуальность");
		Элементы.ПереходПоНомеру.Доступность = Ложь;
	Иначе
		Элементы.КоманднаяПанельНавигация.Видимость = Истина;
		Элементы.ФормаСохранить.Доступность         = Истина;
		Если Результат.ВысотаТаблицы <> 0 Тогда
			ОбщегоНазначенияКлиентСервер.УстановитьСостояниеПоляТабличногоДокумента(
				Элементы.Результат);
		КонецЕсли;
		Элементы.ПереходПоНомеру.Доступность = Отчет.ВсегоСтраниц > 1;
	КонецЕсли;
...
&НаКлиенте
Процедура ОткрытьФормуНастроек(РежимФормы)
	...
	ОбработкаЗакрытия = Новый ОписаниеОповещения(
		"ИзменениеОтборовИПолейЗавершение",
		ЭтотОбъект);
	...
	ОткрытьФорму(
		"Отчет.ПоискКонтрагентов.Форма.НастройкаОтбораИПолей",
		ПараметрыФормы,
		УникальныйИдентификатор,
		,
		,
...
&НаКлиенте
Процедура НастройкиПоУмолчаниюОбработкаОтвета(Ответ, ДополнительныеПараметры) Экспорт
...
Процедура НастройкиПоУмолчаниюНаСервере()
	...
	Если ТипЗнч(НастройкаОтборов) = Тип("Соответствие") Тогда
		...
		НастройкиПоУмолчанию = ПоискКонтрагентовСлужебный.НастройкиПоУмолчанию().Получить(ВидКонтрагента);
		ТекущееКоличество = 0;
		
		Для Каждого КлючЗначение Из НастройкаОтборов Цикл
			Отбор = КлючЗначение.Значение;
			Отбор.Значение              = Неопределено;
			Отбор.ПредставлениеЗначения = "";
			Отбор.Пометка               = Ложь;
			Отбор.БыстрыйДоступ         = (ТекущееКоличество < НастройкиПоУмолчанию.КоличествоКритериевВБыстромДоступе);
	...
	Если ТипЗнч(НастройкаПолей) = Тип("Массив") Тогда
		...
		ПоляПоУмолчанию = ПоискКонтрагентовСлужебныйКлиентСервер.ПоляПоУмолчанию(ВидКонтрагента);
		
		Для Каждого ОписаниеПоля Из НастройкаПолей Цикл
			НовоеЗначение = (ПоляПоУмолчанию.Найти(ОписаниеПоля.Идентификатор) <> Неопределено);
			ОписаниеПоля.Пометка = НовоеЗначение;
		КонецЦикла;
...
&НаСервере
Процедура НастроитьВнешнийВид(ПараметрыПоиска)
	
	Если ЗначениеЗаполнено(ПараметрыПоиска.ЗаголовокОкна) Тогда
		Заголовок = ПараметрыПоиска.ЗаголовокОкна;
		АвтоЗаголовок = Ложь;
	КонецЕсли;
	
	КнопкаСформировать = Элементы.ФормаСформировать;
	
	Если ЗначениеЗаполнено(ПараметрыПоиска.ПоложениеКнопкиПоиска) Тогда
		Если ПараметрыПоиска.ПоложениеКнопкиПоиска = "Классический" Тогда
			КнопкаСформировать = Элементы.ФормаСформироватьВерх;
			Элементы.ГруппаСформироватьНиз.Видимость = Ложь;
		Иначе
			Элементы.ГруппаСформироватьВерх.Видимость = Ложь;
			Если ПараметрыПоиска.ПоложениеКнопкиПоиска = "ВнизуСлева" Тогда
				Элементы.ГруппаСформироватьНиз.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Лево;
			КонецЕсли;
		КонецЕсли;
	Иначе
		Элементы.ГруппаСформироватьВерх.Видимость = Ложь;
	...
	КнопкаСформировать.КнопкаПоУмолчанию = Истина;
	Если ЗначениеЗаполнено(ПараметрыПоиска.ЗаголовокКнопкиПоиска) Тогда
		КнопкаСформировать.Заголовок = ПараметрыПоиска.ЗаголовокКнопкиПоиска;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПараметрыПоиска.КартинкаКнопкиПоиска) Тогда
		
		Если ПараметрыПоиска.КартинкаКнопкиПоиска = "0" Тогда
			
			КнопкаСформировать.Отображение = ОтображениеКнопки.Текст;
			
		Иначе
		
			Попытка
				
				КнопкаСформировать.Картинка = БиблиотекаКартинок[ПараметрыПоиска.КартинкаКнопкиПоиска];
				КнопкаСформировать.Отображение = ОтображениеКнопки.КартинкаИТекст;
				
			Исключение
				
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось установить картинку кнопки поиска ""%1"".
						|
						|Причина:
						|%2'"),
					ПараметрыПоиска.КартинкаКнопкиПоиска,
