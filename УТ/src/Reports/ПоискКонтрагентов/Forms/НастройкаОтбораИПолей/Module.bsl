...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.РежимФормы = "БыстрыеНастройки" Тогда
		Элементы.Страницы.ОтображениеСтраниц = ОтображениеСтраницФормы.Нет;
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаБыстрыеНастройки;
		Заголовок = НСтр("ru = 'Быстрый доступ'");
		
	ИначеЕсли Параметры.РежимФормы = "Колонки" Тогда
		Элементы.Страницы.ОтображениеСтраниц = ОтображениеСтраницФормы.ЗакладкиСверху;
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПоля;
		Элементы.СтраницаБыстрыеНастройки.Видимость = Ложь;
		Элементы.ВсеНастройки.Видимость = Ложь;
	...
	Элементы.НастройкиОтборовЗначение.РасширенноеРедактированиеМножественныхЗначений = Истина;
	Элементы.НастройкиОтборовЗначение.ОтображатьФлажкиВВыпадающемСпискеПриВводеМножественныхЗначений = Ложь;
...
Процедура НастройкиОтборовЗначениеОбработкаВыбора(
	...
	Если (ТекущиеДанные.ВыборИзСправочника
		...
		Иначе
			...
			Если ТекущиеДанные.ЭтоПолеОКВЭД
				И ПравильноеЗначение.Данные = "ПустоеЗначение" Тогда
				СтандартнаяОбработка = Ложь;
				Возврат;
			КонецЕсли;
			
			Если ТекущиеДанные.ВидСравнения = ВидыСравнений.ВСписке.Значение Тогда
				
				Если ТипЗнч(ТекущиеДанные.ЗначениеИзСправочника) = Тип("СписокЗначений") Тогда
					ТекущийСписок = ТекущиеДанные.ЗначениеИзСправочника;
				Иначе
				...
				Если ЗначениеДобавлено(
					ТекущийСписок,
					ПравильноеЗначение.Представление) Тогда
					Возврат;
				КонецЕсли;
...
Процедура НастройкиОтборовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "НастройкиОтборовВидСравнения" Тогда
		
		ИзменитьВидСравнения("НастройкиОтборов");
...
Процедура БыстрыеНастройкиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элемент.ТекущийЭлемент = Элементы.БыстрыеНастройкиВидСравнения Тогда 
		...
		ИзменитьВидСравнения("БыстрыеНастройки");
...
Процедура Ок(Команда)
	...
	ЭтоБыстрыеНастройки = Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаБыстрыеНастройки;
	
	Для Каждого Стр Из НастройкиОтборов Цикл
		
		Если ЭтоБыстрыеНастройки Тогда
			// Пользователь может поменять вид сравнения с "В списке" на "Равно", а затем обратно.
			// Если выполнять преобразование в момент изменения, как при полных настройках,
			// то пользователь не увидит, что значение изменилось и может потерять данные.
			// Поэтому обработка только при сохранении настроек и только если текущая страница -
			// быстрые настройки
			Если Стр.ВидСравнения = ВидыСравнений.ВСписке.Значение Тогда
				Стр.Значение = ЗначениеВСписок(Стр.Значение);
				Стр.ЗначениеИзСправочника = ЗначениеВСписок(Стр.ЗначениеИзСправочника);
			Иначе
				Стр.Значение = СписокВЗначение(Стр.Значение);
				Стр.ЗначениеИзСправочника = СписокВЗначение(Стр.ЗначениеИзСправочника);
			КонецЕсли;
		КонецЕсли;
		...
		Если Стр.ВыборИзСправочника Или Стр.ЭтоПолеОКВЭД Тогда
			...
			Если ТипЗнч(НовыйОтбор.Значение) = Тип("СписокЗначений") Тогда
				Для Каждого ЭлементСписка Из НовыйОтбор.Значение Цикл
					ЭлементСписка.Представление = ЭлементСписка.Значение.Представление;
				КонецЦикла;
			КонецЕсли;
		Иначе
...
Процедура ОтборыПриСоздании()
	...
	Для Каждого ДоступныйОтбор Из Параметры.ДоступныеОтборы Цикл
		...
		ИначеЕсли Не ЗначениеЗаполнено(НоваяСтрока.ВидСравнения) Тогда
			
			НоваяСтрока.ВидСравнения = ПоискКонтрагентовСлужебныйКлиентСервер.ВидСравненияПоУмолчанию(
...
Процедура ПоляПриСоздании()
	...
	Если ДополнитьПоля.Количество() > 0 Тогда
		...
		Для Каждого Поле Из ДополнитьПоля Цикл
			ПоляОтчета.Добавить(Поле.Значение, Поле.Представление);
		КонецЦикла;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ИменаТаблиц.Добавить("НастройкиОтборов");
	ИменаТаблиц.Добавить("БыстрыеНастройки");
	...
	Для Каждого ИмяТаблицы Из ИменаТаблиц Цикл
		...
		Поля = Новый Массив;
		Поля.Добавить(ИмяТаблицы + "ВидСравнения");
...
#Иначе
		Элементы.НастройкиОтборовЗначение.ОграничениеТипа = Новый ОписаниеТипов("Неопределено");
#КонецЕсли
		...
		ТекущиеДанные.Значение.ТипЗначения = ТипЗначенияПоСтроке.Получить(ТекущиеДанные.ТипЗначения);
...
Процедура ИзмененияСпискаЗавершение(
	...
	Если ТекущиеДанные.ВыборИзСправочника
		...
		ТекущиеДанные.Значение              = ПредставлениеСписка;
		ТекущиеДанные.ЗначениеИзСправочника = РезультатВыбора;
...
&НаКлиенте
Процедура ИзменитьВсеПометки(НовоеЗначение)
	
	Для Каждого Стр Из НастройкиОтборов Цикл
		Стр.БыстрыйДоступ = НовоеЗначение;
	КонецЦикла;
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработатьВыборИзМеню(ВыбранныйЭлемент, ПараметрыОповещения) Экспорт
	
	Если ПараметрыОповещения.ТекущиеДанные = Неопределено
		Или ВыбранныйЭлемент = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыОповещения.ТекущиеДанные.ВидСравнения = ВыбранныйЭлемент.Значение;
	
	Если ПараметрыОповещения.ИмяЭлемента = "НастройкиОтборов" Тогда
		УстановитьДоступныйТипДанныхЗначения(ПараметрыОповещения.ТекущиеДанные);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ЗначениеПредставленияИзСписка(ТекущиеДанные)
	
	Если (ТекущиеДанные.ВыборИзСправочника
			Или ТекущиеДанные.ЭтоПолеОКВЭД)
			И ТекущиеДанные.ВидСравнения = ВидыСравнений.ВСписке.Значение Тогда
		
		ПредставлениеОтбора = ТекущиеДанные.Значение;
		ПредставлениеОтбора.Очистить();
		
		Если ТипЗнч(ТекущиеДанные.ЗначениеИзСправочника) = Тип("СписокЗначений") Тогда
			Для Каждого ЗначениеОтбора Из ТекущиеДанные.ЗначениеИзСправочника Цикл
...
&НаКлиенте
Процедура ПроверкаУдаленияИзМножественногоЗначения(ТекущиеДанные)
	
	ТекущийСписок = ТекущиеДанные.ЗначениеИзСправочника;
	Если ТипЗнч(ТекущийСписок) = Тип("СписокЗначений")
		И ТипЗнч(ТекущиеДанные.Значение) = Тип("СписокЗначений") Тогда
		
		НовоеЗначение = ТекущиеДанные.Значение.ВыгрузитьЗначения();
		ЭлементыДляУдаления = Новый Массив;
		Для Каждого ЭлементТекущегоСписка Из ТекущийСписок Цикл
			Если НовоеЗначение.Найти(ЭлементТекущегоСписка.Представление) = Неопределено Тогда
...
Процедура ПриИзмененииЗначения()
	...
	Если ИзмененияОбработаны Тогда
		...
		ЗначениеПредставленияИзСписка(ТекущиеДанные);
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли (ТекущиеДанные.ВыборИзСправочника
		...
		ПроверкаУдаленияИзМножественногоЗначения(ТекущиеДанные);
		ЗначениеПредставленияИзСписка(ТекущиеДанные);
...
&НаКлиенте
Процедура ИзменитьВидСравнения(ИмяЭлемента)
	...
	ПоказатьВыборИзМеню(
		Оповещение,
		ДоступныеВидыСравнения(ТекущиеДанные));
	
КонецПроцедуры

&НаКлиенте
Функция ДоступныеВидыСравнения(ТекущиеДанные)
