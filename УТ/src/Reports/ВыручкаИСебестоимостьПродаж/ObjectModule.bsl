...
Процедура ПередЗагрузкойВариантаНаСервере(ЭтаФорма, НовыеНастройкиКД) Экспорт
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.УдалитьНеиспользуемыеГруппировкиОтчета(НовыеНастройкиКД, НеиспользуемыеПоляОтчета());
...
Процедура ПередЗагрузкойНастроекВКомпоновщик(Контекст, КлючСхемы, КлючВарианта, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД) Экспорт
	...
	НужноПодключитьСхему = Ложь;
	
	Если ПолучитьФункциональнуюОпцию("УправлениеТорговлей") Тогда
		НужноПодключитьСхему = Истина;
	...
	Если НужноПодключитьСхему Тогда
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	#Область ДетализироватьДоИсходныхЗатрат
	
	МассивИспользуемыхПолейКД = Новый Массив();
	РасчетСебестоимостиПрикладныеАлгоритмы.ПолучитьИспользуемыеПоляВсехГруппировок(НастройкиОтчета.Структура,
		МассивИспользуемыхПолейКД);
	
	МассивПроверяемыхПолейКД = Новый Массив();
	МассивПроверяемыхПолейКД.Добавить("СтатьяРасходов");
	МассивПроверяемыхПолейКД.Добавить("АналитикаРасходов");
	МассивПроверяемыхПолейКД.Добавить("НоменклатураИсходная");
	МассивПроверяемыхПолейКД.Добавить("ХарактеристикаИсходная");
	МассивПроверяемыхПолейКД.Добавить("ДокументПоступленияИсходный");
	ДетализироватьДоИсходныхЗатрат = Ложь;
	Для Каждого ПолеКД Из МассивИспользуемыхПолейКД Цикл 
		Если МассивПроверяемыхПолейКД.Найти(СтрРазделить(Строка(ПолеКД.Поле), ".", Ложь)[0]) <> Неопределено Тогда
			ДетализироватьДоИсходныхЗатрат = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ДетализироватьДоИсходныхЗатрат",
		ДетализироватьДоИсходныхЗатрат);
	...
	НаборДанных = СхемаКомпоновкиДанных.НаборыДанных.ВыручкаИСебестоимостьПродажСДетализацией;
	НаборДанных.Запрос = ТекстЗапроса;

	// Удалим неиспользуемые в данной конфигурации поля.
	МассивПолей = НеиспользуемыеПоляОтчета();
	
	Для Каждого ИмяПоля Из МассивПолей Цикл
		
		МассивПолейИтога = Новый Массив;
		УдаляемоеПоле = НаборДанных.Поля.Найти(ИмяПоля);
		
		Если УдаляемоеПоле <> Неопределено Тогда
			НаборДанных.Поля.Удалить(УдаляемоеПоле);
		КонецЕсли;
		
		Для Каждого ПолеИтога Из СхемаКомпоновкиДанных.ПоляИтога Цикл
			Если СтрНайти(НРег(ПолеИтога.Выражение), НРег(ИмяПоля)) > 0 Тогда
				МассивПолейИтога.Добавить(ПолеИтога);
			КонецЕсли;
		КонецЦикла;

		Для Каждого ПолеИтога Из МассивПолейИтога Цикл
			СхемаКомпоновкиДанных.ПоляИтога.Удалить(ПолеИтога);
		КонецЦикла;
		
	КонецЦикла;
	
	Если МассивПолей.Количество() > 0 Тогда
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(НастройкиОтчета, МассивПолей, Истина);
		РасчетСебестоимостиПрикладныеАлгоритмы.УдалитьНеиспользуемыеГруппировкиОтчета(НастройкиОтчета, МассивПолей);
	КонецЕсли;
...
Функция ТекстЗапроса(ПоказыватьПродажи)
	...
	|		Продажи.ПериодКорректировки              КАК ПериодКорректировки,
	|		
	...
	|		И (Продажи.Количество <> 0 ИЛИ Продажи.СуммаВыручки <> 0 ИЛИ Продажи.СуммаВыручкиРегл <> 0
	|			ИЛИ Продажи.СуммаВыручкиСНДСРегл <> 0 ИЛИ Продажи.СуммаВыручкиБезНДС <> 0
	|			ИЛИ Продажи.Стоимость <> 0 ИЛИ Продажи.СтоимостьБезНДС <> 0 ИЛИ Продажи.СтоимостьРегл <> 0 ИЛИ Продажи.СтоимостьУпр <> 0
	|			
	|			// Доп. расходы
	|			ИЛИ Продажи.ДопРасходы <> 0 ИЛИ Продажи.ДопРасходыУпр <> 0 ИЛИ Продажи.ДопРасходыБезНДС <> 0 ИЛИ Продажи.ДопРасходыРегл <> 0
	|			
	|			// Трудозатраты
	|			ИЛИ Продажи.Трудозатраты <> 0 ИЛИ Продажи.ТрудозатратыУпр <> 0 ИЛИ Продажи.ТрудозатратыРегл <> 0
	|			
	|			// Постоянные постательные
	|			ИЛИ Продажи.ПостатейныеПостоянныеСНДС <> 0 ИЛИ Продажи.ПостатейныеПостоянныеБезНДС <> 0
	|			ИЛИ Продажи.ПостатейныеПостоянныеУпр <> 0 ИЛИ Продажи.ПостатейныеПостоянныеРегл <> 0
	|			
	|			// Переменные постательные
	|			ИЛИ Продажи.ПостатейныеПеременныеСНДС <> 0 ИЛИ Продажи.ПостатейныеПеременныеБезНДС <> 0
	|			ИЛИ Продажи.ПостатейныеПеременныеУпр <> 0 ИЛИ Продажи.ПостатейныеПеременныеРегл <> 0
	|			ИЛИ Продажи.РасходыНаПродажуСНДС <> 0 ИЛИ Продажи.РасходыНаПродажуБезНДС <> 0 ИЛИ Продажи.РасходыНаПродажуРегл <> 0
	|			ИЛИ Продажи.РасходыНаПродажуУпр <> 0 ИЛИ Продажи.РезервПодОбесценениеРегл <> 0 ИЛИ Продажи.РезервПодОбесценениеУпр <> 0
	|			ИЛИ Продажи.СуммаРучнойСкидки <> 0 ИЛИ Продажи.СуммаАвтоматическойСкидки <> 0)
	|	{ГДЕ
	|		Продажи.АналитикаУчетаНоменклатуры.Номенклатура.* КАК Номенклатура,
	|		Продажи.АналитикаУчетаНоменклатуры.Характеристика.* КАК Характеристика,
	|		Продажи.АналитикаУчетаНоменклатуры.МестоХранения.* КАК Склад,
	...
	|	Продажи КАК Продажи
	|ГДЕ
	// Во временной таблице Продажи, уже есть отбор по колонке АналитикаУчетаПоПартнерам.
	|	Продажи.Количество <> 0
	// Смотри комментарий 1531.
	|	И Продажи.ЗаказКлиента <> НЕОПРЕДЕЛЕНО
	|	И &ДетализироватьДоИсходныхЗатрат
	|
	|ИНДЕКСИРОВАТЬ ПО НАБОРАМ
	|	(
	// Индексация под первый индекс таблицы оборотов.
	|		(АналитикаУчетаНоменклатуры, ЗаказКлиента),
	// Индексация под дополнительный индекс.
	// Селективность выше при большом количестве продажи идентичных аналитик учета номенклатуры.
	|		(ЗаказКлиента, АналитикаУчетаНоменклатуры)
	|	)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|			(АналитикаУчетаНоменклатуры, ЗаказКлиента) В
	|					(ВЫБРАТЬ
	...
	// Комментарий 1531.
	// Во временной таблице АналитикиДляОтбора исключен ЗаказКлиента = Неопределено, исходя из этого:
	//   1. Отбор по АналитикаУчетаПоПартнерам избыточен, так как достаточно отбора по ЗаказКлиента.
	//   2. Отбор по партиям так же исключен исходя из того, что обычно по каждому заказу клиента нужны все партии.
	//   Исключение данного отбора вызвано тем, что на PostgreSQL, может формироваться, не эффективный план запроса,
	//   со сканированием не селективного индекса по партиям.
	//   При установке отбора по партии, в настройках отчета, условие отбора пробрасывается в параметры виртуальной таблицы.
	|				) КАК ИсходныеПродажи
	|ГДЕ
	...
	|	ВЫБОР
	|		КОГДА Продажи.ЗаказКлиента = НЕОПРЕДЕЛЕНО
	|			ТОГДА ЗНАЧЕНИЕ(Документ.ЗаказКлиента.ПустаяСсылка)
	|		ИНАЧЕ Продажи.ЗаказКлиента
...
Функция НеиспользуемыеПоляОтчета()
	
	МассивПолей = Новый Массив;

	
	МассивПолей.Добавить("СтатьяКалькуляцииПродукции");
	МассивПолей.Добавить("ИсходнаяСтоимостьПродукции");
