...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	НастроитьОграничениеДоступностиПараметров(СхемаКомпоновкиДанных, ПараметрВариантОтчета);
	...
	Если ТипЗнч(Контекст) = Тип("ФормаКлиентскогоПриложения") Тогда
		...
		Если НовыеПользовательскиеНастройкиКД <> Неопределено Тогда
			...
			Если ПараметрДатаОтчета <> Неопределено
			   И НЕ ЗначениеЗаполнено(ПараметрДатаОтчета.Значение) Тогда
...
Процедура УстановитьДополнительныеПараметры(НастройкиОтчета, СтруктураПараметров)
	...
	Если ВариантОтчета = 2
	 ИЛИ ВариантОтчета = 5 Тогда
		...
		Выборка.Следующий(); 
		ИНН = Выборка.ИНН;
		ПартнерКонтрагента = Выборка.Партнер; // СправочникСсылка.Партнеры
	...
	Иначе
		...
		Партнер = ПараметрПартнер.Значение; // СправочникСсылка.Партнеры
...
Процедура НастроитьОграничениеДоступностиПараметров(СхемаКомпоновкиДанных, ПараметрВариантОтчета)
	...
	Если ПараметрВариантОтчета <> Неопределено
	   И ЗначениеЗаполнено(ПараметрВариантОтчета.Значение) Тогда
		...
		Если ВариантОтчета = 4 Тогда
			...
			СхемаКомпоновкиДанных.Параметры.Партнер.ОграничениеИспользования = Ложь;
			
		ИначеЕсли ВариантОтчета = 5 Тогда
			
			СхемаКомпоновкиДанных.Параметры.Контрагент.ОграничениеИспользования = Ложь;
