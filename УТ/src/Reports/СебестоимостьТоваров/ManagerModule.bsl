...
Функция ТекстЗапросаВыборки() Экспорт
	...
	|	ОстаткиИДвижения.ПериодОтклонения КАК ПериодОтклонения,
	...
	|	ОстаткиИДвижения.ПериодОтклонения,
...
Функция ТекстЗапросаВТСебестоимость()
	...
	|	НАЧАЛОПЕРИОДА(СебестоимостьТоваров.ПериодОтклонения, МЕСЯЦ) КАК ПериодОтклонения,
	...
	|			И СебестоимостьТоваров.АналитикаУчетаПоПартнерам <> ЗНАЧЕНИЕ(Справочник.КлючиАналитикиУчетаПоПартнерам.ПустаяСсылка)}
	...
	|	И СебестоимостьТоваров.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеОтклоненийКРаспределению)
	|{ГДЕ
	|	(СебестоимостьТоваров.Период МЕЖДУ &НачалоПериода И &КонецПериода),
...
Функция ТекстЗапросаОстаткиИДвижения(РежимОтладкиПоИсточникам)
	...
	|	СебестоимостьТоваров.ПериодОтклонения КАК ПериодОтклонения,
	|	СебестоимостьТоваров.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
