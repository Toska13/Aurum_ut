...
Функция ДанныеИнформационнойБазы(ДокументСсылка, ДетальныеЗаписи = Ложь) Экспорт
	...
	Для Каждого ЭлементКоллекции Из ДанныеПротокола Цикл
		...
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЭлементКоллекции.Документ, "Операция, ТоварнаяГруппа");
		ВидыПродукции = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(РеквизитыДокумента.ТоварнаяГруппа);
		
		Если ВидыПродукции.Количество() = 0 Тогда
			ВызватьИсключение СтрШаблон(
				НСтр("ru='Не удалось определить виды продукции по товарной группе %1'"),
				РеквизитыДокумента.ТоварнаяГруппа);
		Иначе
