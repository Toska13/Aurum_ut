...
Функция ТекстЗапросаОтчета()
	...
	|			ТОГДА &Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|		ИНАЧЕ Номенклатура.Ссылка = &Номенклатура
	|	КОНЕЦ
	|	И ВидыНоменклатурыПолитикиУчетаСерий.ПолитикаУчетаСерий.ТипПолитики В
	...
	|		КОГДА &Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|			ТОГДА Серия = &Серия
	|		КОГДА &Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|			ТОГДА Номенклатура = &Номенклатура
	|		ИНАЧЕ Номенклатура = &Номенклатура
	|		И Серия = &Серия
	|	КОНЕЦ
	|	И Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|	И &Характеристика В (Характеристика, ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка), НЕОПРЕДЕЛЕНО)) КАК
	|		ТоварыНаСкладах
	|{ГДЕ
	|	ТоварыНаСкладах.Регистратор.* КАК Регистратор,
	|	ТоварыНаСкладах.ПериодСекунда КАК ПериодСекунда,
	...
	|ИЗ
	|	РегистрНакопления.ДвиженияСерийТоваров.Обороты(,, Авто, ЭтоСкладскоеДвижение
	|	И ВЫБОР
	|		КОГДА &Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|			ТОГДА Серия = &Серия
	|		КОГДА &Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	...
	|	КОНЕЦ
	|	И Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|	И &Характеристика В (Характеристика, ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка), НЕОПРЕДЕЛЕНО)
	...
	|		ЗНАЧЕНИЕ(Перечисление.СкладскиеОперации.ОтгрузкаВРозницу)))) КАК ДвиженияСерийТоваров
	|{ГДЕ
	|	ДвиженияСерийТоваров.Регистратор.* КАК Регистратор,
	|	ДвиженияСерийТоваров.ПериодСекунда КАК ПериодСекунда,
	|	ДвиженияСерийТоваров.ПериодДень КАК ПериодДень,
	...
	|	И СкладскаяОперация В (ЗНАЧЕНИЕ(Перечисление.СкладскиеОперации.ПриемкаПоВозвратуОтКлиента)))) КАК
