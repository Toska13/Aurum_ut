...
Функция ТекстЗапросаРасходы()
	...
	|
	|	НАЧАЛОПЕРИОДА(ПрочиеРасходы.ПериодОтклонения, МЕСЯЦ) КАК ПериодОтклонения,
	|
	...
	|	И ПрочиеРасходы.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеОтклоненийКРаспределению)
	|{ГДЕ
	|	(ПрочиеРасходы.АналитикаРасходов В
	|		(ВЫБРАТЬ
	|			ОтборПоСегментуПартнеров.Партнер
	|		ИЗ
	|			ОтборПоСегментуПартнеров
	|		ГДЕ
	|			ОтборПоСегментуПартнеров.ИспользуетсяОтборПоСегментуПартнеров = &ИспользуетсяОтборПоСегментуПартнеров)
	|	ИЛИ ПрочиеРасходы.АналитикаРасходов.Партнер В
	|		(ВЫБРАТЬ
	|			ОтборПоСегментуПартнеров.Партнер
	|		ИЗ
	|			ОтборПоСегментуПартнеров
