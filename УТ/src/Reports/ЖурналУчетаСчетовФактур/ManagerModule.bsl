...
Процедура СформироватьОтчет(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Если ВерсияПостановленияНДС1137 >= 9 Тогда
		Макет = ПолучитьОбщийМакет("ЖурналУчетаСчетовФактур26");
	ИначеЕсли ВерсияПостановленияНДС1137 = 8 Тогда
		Макет = ПолучитьОбщийМакет("ЖурналУчетаСчетовФактур2026");
...
#КонецОбласти
	...
	Иначе
		...
		СведенияОЮрФизЛице = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(
			СтруктураПараметров.Организация, КонецПериодаОтчета);
		Свидетельство = БухгалтерскийУчетПереопределяемый.СвидетельствоОРегистрации(СведенияОЮрФизЛице, СтруктураПараметров.НалоговыйПериод);
	КонецЕсли;
