...
Функция СформироватьВтПараметровОбеспеченияТоваров(МенеджерВременныхТаблиц)
	...
	Запрос.Текст = Справочники.ФорматыМагазинов.ТекстЗапросаВтФорматыСкладов() +
		...
		|ПОМЕСТИТЬ ВтНоменклатураСкладыИСпособыОбеспечения
		|ИЗ
	...
	Если ИспользоватьКонтрольКачества Тогда
		...
		Текст =
			...
			|ИЗ
			|	ВтНоменклатураСкладыИСпособыОбеспечения КАК Товары
	...
	Текст =
		...
		|	ВЫБРАТЬ
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(ВЫРАЗИТЬ(Таблица.Склад КАК Справочник.Склады).Календарь,
		...
		|			ИНАЧЕ ВЫРАЗИТЬ(Таблица.Склад КАК Справочник.Склады).Календарь
		|		КОНЕЦ КАК Календарь,
		...
		|ИЗ
		|	ВтНоменклатураСкладыИСпособыОбеспечения КАК Таблица
		|		
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ БлижайшийЦиклПоСпособамОбеспечения КАК Способ
		|		ПО Способ.Ссылка = Таблица.СпособОбеспечения
	...
	Если ИспользоватьКонтрольКачества Тогда
		...
		"(Способ.Номенклатура = Таблица.Номенклатура
		|И Способ.Склад = Таблица.Склад)";
	...
	Запрос.Текст =
		...
		|		ПО ТаблицаСпособовОбеспечения.Ссылка = Т.СпособОбеспечения
		|		
	...
	Если ИспользоватьКонтрольКачества Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "БлижайшийЦиклПоСпособамОбеспечения", "БлижайшийЦиклПоСпособамОбеспеченияСУчетомКонтроляКачества");
	КонецЕсли;
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&СоединениеПоТоваруКонтрольКачества", СоединениеПоТоваруКонтрольКачества);
