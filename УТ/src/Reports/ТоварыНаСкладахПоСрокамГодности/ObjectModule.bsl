...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	СкладскиеОперацииПриемки.Добавить(Перечисления.СкладскиеОперации.ОтражениеИзлишков);
	...
	ОстаткиСерийСправочно.Колонки.Добавить("КонтролируемаяПартия",			Новый ОписаниеТипов("СправочникСсылка.КонтролируемыеПартии"));
	...
	ОтборПоНачалуПериодаОГ		= НачалоПериодаОГ <> Неопределено
									И ТипЗнч(НачалоПериодаОГ.Значение) = Тип("Дата")
									И ЗначениеЗаполнено(НачалоПериодаОГ.Значение);
	ОтборПоОкончаниюПериодаОГ	= ОкончаниеПериодаОГ <> Неопределено
									И ТипЗнч(ОкончаниеПериодаОГ.Значение) = Тип("Дата")
									И ЗначениеЗаполнено(ОкончаниеПериодаОГ.Значение);
	
	НачалоПериодаОГ		= ?(ОтборПоНачалуПериодаОГ, НачалоПериодаОГ.Значение, Дата(1,1,1));
	ОкончаниеПериодаОГ	= ?(ОтборПоОкончаниюПериодаОГ, ОкончаниеПериодаОГ.Значение, Дата(1,1,1));
	...
	Для Каждого СтрТовары Из ДеревоОбработки.Строки Цикл
		...
		Для Каждого СтрСрокаГодности Из СтрТовары.Строки Цикл
			...
			Если (ОтборПоНачалуПериодаОГ
					И СтрСрокаГодности.ГоденДоАвторасчет < НачалоПериодаОГ)
				Или (ОтборПоОкончаниюПериодаОГ
			...
			КоличествоОстатка = Мин(КоличествоТоваров, СтрСрокаГодности.КоличествоСерия);
			
			Если ТекущаяСтрока = Неопределено
				Или (ТекущаяСтрока <> Неопределено
					И ТекущаяСтрока.Серия <> СтрСрокаГодности.СерияАвторасчет) Тогда
				...
				НоваяСтрокаДанных.Остаток = КоличествоОстатка;
				...
				ТекущаяСтрока = НоваяСтрокаДанных;
				
			Иначе
				ТекущаяСтрока.Остаток = ТекущаяСтрока.Остаток + КоличествоОстатка;
			КонецЕсли;
			
			КоличествоТоваров = КоличествоТоваров - КоличествоОстатка;
		...
		Если КоличествоТоваров <> 0
			И Не ОтборПоНачалуПериодаОГ Тогда
