...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Движения = СхемаКомпоновкиДанных.НаборыДанных.Движения; // НаборДанныхОбъединениеСхемыКомпоновкиДанных - 
	Движения.Элементы.РасчетыПоОплатеТруда.Запрос = ТекстЗапросаРасчетыПоОплатеТруда();
...
Функция ТекстЗапросаРасчетыПоОплатеТруда()
	
	ТекстЗапроса = 
	 "ВЫБРАТЬ
	 |	ДАТАВРЕМЯ(1, 1, 1) КАК Период,
	 |	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаОстатка,
	 |	НЕОПРЕДЕЛЕНО КАК Регистратор,
	 |	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК Валюта,
	 |	ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка) КАК Организация,
	 |	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаРегламентированногоУчета,
	 ...
	 |	""""""РасчетыПоОплатеТрудаЗИК"""""" КАК РазделПереоценки,
	 |	1400 КАК ПорядокРазделаПереоценки
	 |ГДЕ
	 |	ЛОЖЬ
	 |
	 |УПОРЯДОЧИТЬ ПО
