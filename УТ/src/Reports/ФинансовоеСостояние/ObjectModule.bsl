...
Функция ТекстВыполненоЗакрытиеМесяца()
	...
	|	И Операция.Проведен
	|	И Операция.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ПустаяСсылка)
...
Функция ТекстЗапросаДенежныеСредства()
	...
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСтатейУправленческогоБаланса.Актив) КАК ТипПоказателя,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансДенежныеСредстваНаличные) КАК Показатель,
	|	
	...
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСтатейУправленческогоБаланса.Актив) КАК ТипПоказателя,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансДенежныеСредстваВПути) КАК Показатель,
	|	
	|	ДенежныеСредства.Организация,
	|	ВЫБОР КОГДА &ВалютаОтчета = ДенежныеСредства.Организация.ВалютаРегламентированногоУчета ТОГДА
	|		ДенежныеСредства.Организация.ВалютаРегламентированногоУчета
	...
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСтатейУправленческогоБаланса.Актив) КАК ТипПоказателя,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансДенежныеСредстваБезналичные) КАК Показатель,
	|	
	|	ДенежныеСредства.Организация,
	|	ВЫБОР КОГДА &ВалютаОтчета = ДенежныеСредства.Организация.ВалютаРегламентированногоУчета ТОГДА
	|		ДенежныеСредства.Организация.ВалютаРегламентированногоУчета
...
Функция ТекстЗапросаДенежныеСредстваУПодотчетныхЛиц()
	...
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансДенежныеСредстваУПодотчетныхЛиц) КАК Показатель,
	|	
	|	ДенежныеСредства.Организация,
	|	ВЫБОР КОГДА &ВалютаОтчета = ДенежныеСредства.Организация.ВалютаРегламентированногоУчета ТОГДА
	|		ДенежныеСредства.Организация.ВалютаРегламентированногоУчета
...
Функция ТекстЗапросаПрочиеРасходы()
	...
	|	ТОГДА
	|		ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансРасходыБудущихПериодов)
	|	ИНАЧЕ
	|		ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансРасходыТекущегоПериода)
	|	КОНЕЦ КАК Показатель,
	|	
	...
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСтатейУправленческогоБаланса.Актив) КАК ТипПоказателя,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансРасходыТекущегоПериода) КАК Показатель,
	|	
	|	ПрочиеРасходы.Организация КАК Организация,
	|	&ВалютаУправленческогоУчета КАК Валюта,
	|	NULL КАК Партнер,
...
Функция ТекстЗапросаРасчетыСКлиентами()
	...
	|		ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансЗадолженностьСобственныхОрганизаций)
	|	ИНАЧЕ
	...
	|	ВЫБОР КОГДА РегистрАналитикаУчетаПоПартнерам.Партнер = ЗНАЧЕНИЕ(Справочник.Партнеры.НашеПредприятие) ТОГДА
	|		ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансОбязательстваПередСобственнымиОрганизациями)
	|	ИНАЧЕ
	...
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСтатейУправленческогоБаланса.Актив) КАК ТипПоказателя,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансЗадолженностьКлиентов) КАК Показатель,
	|	
	|	РегистрАналитикаУчетаПоПартнерам.Организация,
	...
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСтатейУправленческогоБаланса.Пассив) КАК ТипПоказателя,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансПолученныеАвансы) КАК Показатель,
	|	
...
Функция ТекстЗапросаРасчетыСПоставщиками()
	...
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСтатейУправленческогоБаланса.Пассив) КАК ТипПоказателя,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансЗадолженностьПередПоставщиками) КАК Показатель,
	|	
	...
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСтатейУправленческогоБаланса.Актив) КАК ТипПоказателя,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансВыданныеАвансы) КАК Показатель,
	|	
	|	Расчеты.ВидЗапасов.Организация,
	|	Расчеты.ВидЗапасов.Валюта КАК Валюта,
	|	Расчеты.ВидЗапасов.ВладелецТовара КАК Партнер,
...
Функция ТекстЗапросаРасчетыПоФинансовымИнструментам()
	...
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСтатейУправленческогоБаланса.Пассив) КАК ТипПоказателя,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансЗадолженностьПоКредитам) КАК Показатель,
	...
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСтатейУправленческогоБаланса.Актив) КАК ТипПоказателя,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансДепозитыВБанках) КАК Показатель,
	|	Аналитика.Организация,
	|	ВЫБОР КОГДА &ВалютаОтчета = Аналитика.Организация.ВалютаРегламентированногоУчета ТОГДА
	|		Аналитика.Организация.ВалютаРегламентированногоУчета
	|	КОГДА &ВалютаОтчета = &ВалютаУправленческогоУчета ТОГДА
...
Функция ТекстЗапросаПрибыльУбыток()
	...
	|	
	|	0 КАК СуммаНачальныйОстаток,
	|	- ДанныеРегистра.СуммаОборот КАК СуммаКонечныйОстаток
	|ИЗ
	|	РегистрНакопления.ПрочиеДоходы.Обороты({&НачалоПериода}, {&КонецПериода}, Авто) КАК ДанныеРегистра
	|";
...
Процедура УстановитьОбязательныеНастройки(ПользовательскиеНастройкиМодифицированы)
	...
	Если ПараметрСтатьиПрочихАктивовПассивов <> Неопределено Тогда
		...
		СтатьиПрочихАктивовПассивов.Добавить(ПланыВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансЦелевоеФинансирование);
		СтатьиПрочихАктивовПассивов.Добавить(ПланыВидовХарактеристик.СтатьиАктивовПассивов.ПрибылиИУбытки);
		СтатьиПрочихАктивовПассивов.Добавить(ПланыВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансПрочиеПассивы);
		СтатьиПрочихАктивовПассивов.Добавить(ПланыВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансПрочиеАктивы);
		СтатьиПрочихАктивовПассивов.Добавить(ПланыВидовХарактеристик.СтатьиАктивовПассивов.Налоги);
		СтатьиПрочихАктивовПассивов.Добавить(ПланыВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансДепонированнаяЗарплата);
