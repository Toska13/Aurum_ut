...
Функция КраткаяИнформацияПоОбмену(ИдентификаторНастройки)
	...
	Если ИдентификаторНастройки = ИдентификаторНастройкиОбменаБП30() Тогда
	...
	ИначеЕсли ИдентификаторНастройки = ИдентификаторНастройкиОбменаБПКОРП30() Тогда
		ПоясняющийТекст = НСтр("ru = 'Данная настройка позволит синхронизировать данные между программами ""%1""
		|и ""Бухгалтерия предприятия КОРП, редакция 3.0"".  В синхронизации участвуют документы и нормативно-справочная информация.'");
	ИначеЕсли ИдентификаторНастройки = ИдентификаторНастройкиОбменаБазовойУТБП30() Тогда
	...
	ИначеЕсли ИдентификаторНастройки = ИдентификаторНастройкиОбменаДокументооборот() Тогда
		ПоясняющийТекст = НСтр("ru = 'Данная настройка позволит синхронизировать данные между программами ""%1""
		|и ""1С:Документооборот КОРП, редакция 3.0"".  В синхронизации участвует нормативно-справочная информация.'");
	Иначе
...
Функция ПодробнаяИнформацияПоОбмену(ИдентификаторНастройки)
	...
	Если ЭтоИдентификаторНастройкиСБП3(ИдентификаторНастройки) Тогда
...
Процедура ПриОпределенииПоддерживаемыхОбъектовФормата(ПоддерживаемыеОбъекты, Режим, УзелОбмена = Неопределено) Экспорт
	...
	РазрешитьЗагрузкуДанныхВБазовуюВерсию = ?(УзелОбмена <> Неопределено, УзелОбмена.РазрешитьЗагрузкуДанныхВБазовуюВерсию, Ложь);
	Если СтрНайти(Режим, "Получение") > 0 И СтандартныеПодсистемыСервер.ЭтоБазоваяВерсияКонфигурации()
		И НЕ РазрешитьЗагрузкуДанныхВБазовуюВерсию Тогда
...
Процедура ПриПолученииВариантовНастроекОбмена(ВариантыНастроекОбмена, ПараметрыКонтекста) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("УправлениеТорговлей")
		И СтандартныеПодсистемыСервер.ЭтоБазоваяВерсияКонфигурации() Тогда
			...
			Если ПараметрыКонтекста.ИмяКорреспондента = "БухгалтерияПредприятия"
				...
				ВариантНастройки.ИдентификаторНастройки        = ИдентификаторНастройкиОбменаБазовойУТБП30();
	...
	Если ПараметрыКонтекста.ИмяКорреспондента = "БухгалтерияПредприятия"
		Или Не ЗначениеЗаполнено(ПараметрыКонтекста.ИмяКорреспондента) Тогда
		ВариантНастройки = ВариантыНастроекОбмена.Добавить();
		ВариантНастройки.ИдентификаторНастройки        = ИдентификаторНастройкиОбменаБП30();
		ВариантНастройки.КорреспондентВМоделиСервиса   = Истина;
		ВариантНастройки.КорреспондентВЛокальномРежиме = Истина;
	КонецЕсли;
	...
	Если ПараметрыКонтекста.ИмяКорреспондента = "БухгалтерияПредприятияКОРП"
		Или Не ЗначениеЗаполнено(ПараметрыКонтекста.ИмяКорреспондента) Тогда
		ВариантНастройки = ВариантыНастроекОбмена.Добавить();
		ВариантНастройки.ИдентификаторНастройки        = ИдентификаторНастройкиОбменаБПКОРП30();
		ВариантНастройки.КорреспондентВМоделиСервиса   = Истина;
		ВариантНастройки.КорреспондентВЛокальномРежиме = Истина;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ПараметрыКонтекста.ИмяКорреспондента) Тогда
		...
		ВариантНастройки = ВариантыНастроекОбмена.Добавить();
		ВариантНастройки.ИдентификаторНастройки        = ИдентификаторНастройкиОбменаДокументооборот();
		ВариантНастройки.КорреспондентВМоделиСервиса   = Истина;
		ВариантНастройки.КорреспондентВЛокальномРежиме = Истина;
...
Процедура ПриПолученииОписанияВариантаНастройки(ОписаниеВарианта, ИдентификаторНастройки, ПараметрыКонтекста) Экспорт
	...
	Если ИдентификаторНастройки = ИдентификаторНастройкиОбменаДокументооборот() Тогда
	...
	ИначеЕсли ИдентификаторНастройки = "ОбменУПБП" 
		Или ИдентификаторНастройки = ИдентификаторНастройкиОбменаБП30()
		Или ИдентификаторНастройки = ИдентификаторНастройкиОбменаБазовойУТБП30()
		Тогда
...
Процедура ПереходСУправлениеТорговлей() Экспорт
	...
	Пока УзлыПланаОбмена.Следующий() Цикл
		...
		Если СтрНайти(УзлыПланаОбмена.ВариантНастройки, "ОбменБП") > 0 
			Или УзлыПланаОбмена.ВариантНастройки= "ОбменУТ_БП3" Тогда
			ОчереднойУзел = УзлыПланаОбмена.ПолучитьОбъект();
			ОчереднойУзел.ВариантНастройки = ИдентификаторНастройкиОбменаБП30();
...
Процедура АктуализацияРеквизитовУзлаПланаОбмена(УзелПланаОбмена) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("УправлениеТорговлей")
		И (ВариантНастройки = ИдентификаторНастройкиОбменаБП30()
		Или ВариантНастройки = ИдентификаторНастройкиОбменаБПКОРП30()
		Или ВариантНастройки = ИдентификаторНастройкиОбменаБазовойУТБП30()
		Или ВариантНастройки = ИдентификаторНастройкиОбменаУниверсальный()) Тогда
...
Функция ИспользоватьПомощникСозданияОбменаДанными()
	...
	Если ПолучитьФункциональнуюОпцию("УправлениеТорговлей")
		...
		Запрос.УстановитьПараметр("ОбменУТБП", ИдентификаторНастройкиОбменаБазовойУТБП30());
...
Функция ИспользуемыеТранспортыСообщенийОбмена(ИдентификаторНастройки="")
	...
	ИначеЕсли ИдентификаторНастройки = "ОбменУПРМК" Тогда
		...
		ЗаполнитьИспользуемыеТранспортыСообщенийОбмена(Результат, Истина);
	Иначе
		ЗаполнитьИспользуемыеТранспортыСообщенийОбмена(Результат, Ложь);
	КонецЕсли;
...
Функция ИмяПланаОбменаСтаройНастройки(ВариантНастройки) Экспорт
	...
	Если СокрЛП(ВариантНастройки) = ИдентификаторНастройкиОбменаБП30() Тогда
		
		Возврат "ОбменУправлениеТорговлейБухгалтерияПредприятия30";
		
	ИначеЕсли СокрЛП(ВариантНастройки) = ИдентификаторНастройкиОбменаБПКОРП30() Тогда
...
Функция ВыполнитьПереходНаСинхронизациюДанныхЧерезУниверсальныйФормат(НастройкаСинхронизацииДанных) Экспорт
	...
	Попытка
		...
		Если ИмяПланаОбменаСтаройНастройки = "ОбменУправлениеТорговлейБухгалтерияПредприятия30" Тогда
			...
			ВариантНастройки = ИдентификаторНастройкиОбменаБП30();
			
		ИначеЕсли ИмяПланаОбменаСтаройНастройки = "ОбменУправлениеТорговлейБухгалтерияПредприятияКОРП30" Тогда
			
			ВариантНастройки = ИдентификаторНастройкиОбменаБПКОРП30();
...
Процедура ЗарегистрироватьИзмененияНаУзлеПоОстаткамНаСкладе(Узел) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ИзмененныеШК.Номенклатура КАК Номенклатура,
	|	ИзмененныеШК.Характеристика КАК Характеристика,
	|	ИзмененныеШК.Серия КАК Серия,
	|	ИзмененныеШК.Упаковка КАК Упаковка,
	|	ИзмененныеШК.ШтрихКод КАК ШтрихКод
	|ПОМЕСТИТЬ ИзмененныеШКПоЗарегистрированнымКортежам
	|ИЗ
	|	РегистрСведений.ШтрихкодыНоменклатуры.Изменения КАК ИзмененныеШК
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ШтрихкодыНоменклатуры КАК ДанныеШК
	|		ПО ИзмененныеШК.ШтрихКод = ДанныеШК.ШтрихКод
	|			И ИзмененныеШК.Номенклатура = ДанныеШК.Номенклатура
	|			И ИзмененныеШК.Характеристика = ДанныеШК.Характеристика
	|			И ИзмененныеШК.Серия = ДанныеШК.Серия
	|			И ИзмененныеШК.Упаковка = ДанныеШК.Упаковка
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеЗарегистрированныхОбъектов КАК ОтправленнаяНоменклатура
	|			ПО ДанныеШК.Номенклатура = ОтправленнаяНоменклатура.СсылкаКРегистрации
	|ГДЕ
	|	ДанныеШК.Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
    |
	|ОБЪЕДИНИТЬ ВСЕ
    |
	|ВЫБРАТЬ
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеЗарегистрированныхОбъектов КАК ОтправленнаяНоменклатура
	|			ПО ДанныеШК.Номенклатура = ОтправленнаяНоменклатура.СсылкаКРегистрации
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеЗарегистрированныхОбъектов КАК ОтправленнаяХарактеристика
	|			ПО ДанныеШК.Характеристика = ОтправленнаяХарактеристика.СсылкаКРегистрации
	|ГДЕ
	|	ДанныеШК.Характеристика <> ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
	|ИНДЕКСИРОВАТЬ ПО
	|	ШтрихКод
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДанныеКРегистрации.Номенклатура КАК Номенклатура,
	|	ДанныеКРегистрации.Характеристика КАК Характеристика,
	|	ДанныеКРегистрации.Серия КАК Серия,
	|	ДанныеКРегистрации.ШтрихКод КАК ШтрихКод
	|ПОМЕСТИТЬ РегистрацияПоОстаткамШК
	|ИЗ
	|	ДанныеКРегистрацииПоОстаткам КАК ДанныеКРегистрации
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеЗарегистрированныхОбъектов КАК ДанныеЗарегистрированныхОбъектовНоменклатура
	|		ПО ДанныеКРегистрации.Номенклатура = ДанныеЗарегистрированныхОбъектовНоменклатура.СсылкаКРегистрации
	|ГДЕ
	|	ДанныеКРегистрации.Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
	|	И ДанныеЗарегистрированныхОбъектовНоменклатура.СсылкаКРегистрации ЕСТЬ NULL
	|
	|Объединить Все
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДанныеКРегистрации.Номенклатура КАК Номенклатура,
	|	ДанныеКРегистрации.Характеристика КАК Характеристика,
	|	ДанныеКРегистрации.Серия КАК СерияСерия,
	|	ДанныеКРегистрации.ШтрихКод КАК ШтрихКод
	|	
	|ИЗ
	|	ДанныеКРегистрацииПоОстаткам КАК ДанныеКРегистрации
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеЗарегистрированныхОбъектов КАК ДанныеЗарегистрированныхОбъектовНоменклатура
	|		ПО ДанныеКРегистрации.Номенклатура = ДанныеЗарегистрированныхОбъектовНоменклатура.СсылкаКРегистрации
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеЗарегистрированныхОбъектов КАК ДанныеЗарегистрированныхОбъектовХарактеристика
	|		ПО ДанныеКРегистрации.Характеристика = ДанныеЗарегистрированныхОбъектовХарактеристика.СсылкаКРегистрации
	|ГДЕ
	|	ДанныеКРегистрации.Характеристика <> ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
	|	И (ДанныеЗарегистрированныхОбъектовНоменклатура.СсылкаКРегистрации ЕСТЬ NULL
	|		ИЛИ ДанныеЗарегистрированныхОбъектовХарактеристика.СсылкаКРегистрации ЕСТЬ NULL)
	|ИНДЕКСИРОВАТЬ ПО 
	|	ШтрихКод 
	|;
	|
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДанныеКРегистрации.Номенклатура КАК Номенклатура
	|ПОМЕСТИТЬ НоменклатураДляВыгрузкиЦен
	|ИЗ
	|	ДанныеКРегистрацииПоОстаткам КАК ДанныеКРегистрации
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НоменклатураДляВыгрузкиЦен.Номенклатура КАК Номенклатура,
	|	Цены.Ссылка КАК ДокУстановкаЦен,
	|	Цены.Ссылка.Дата КАК ДатаДок
	|ПОМЕСТИТЬ УстановкиЦенПоНоменклатуре
	|ИЗ
	|	Документ.УстановкаЦенНоменклатуры.Товары КАК Цены
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ НоменклатураДляВыгрузкиЦен КАК НоменклатураДляВыгрузкиЦен
	|		ПО Цены.Номенклатура = НоменклатураДляВыгрузкиЦен.Номенклатура
	|ГДЕ
	|	НЕ &Ценообразование25
	|	И Цены.Ссылка.Согласован
	|	И Цены.Ссылка.Проведен
	|	И Цены.Ссылка.Дата >= &ДатаВыгрузкиДок
	|
	|ОБЪЕДИНИТЬ ВСЕ 
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НоменклатураДляВыгрузкиЦен.Номенклатура,
	|	Цены.Ссылка,
	|	Цены.Ссылка.Дата
	|ИЗ
	|	Документ.УстановкаЦенНоменклатуры.Товары2_5 КАК Цены
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ НоменклатураДляВыгрузкиЦен КАК НоменклатураДляВыгрузкиЦен
	|		ПО Цены.Номенклатура = НоменклатураДляВыгрузкиЦен.Номенклатура
	|ГДЕ
	|	&Ценообразование25
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	УстановкиЦенПоНоменклатуре.Номенклатура КАК Номенклатура,
	|	МАКСИМУМ(УстановкиЦенПоНоменклатуре.ДатаДок) КАК ДатаДок
	|ПОМЕСТИТЬ ДатаЦеныНоменклатуры
	|ИЗ
	|	УстановкиЦенПоНоменклатуре КАК УстановкиЦенПоНоменклатуре
	|
	|СГРУППИРОВАТЬ ПО
	|	УстановкиЦенПоНоменклатуре.Номенклатура
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ  РАЗЛИЧНЫЕ
	|	 ПоследняяЦенаПоНоменклатуре.ДокУстановкаЦен КАК СсылкаКРегистрации
	|ПОМЕСТИТЬ ПоздняяРегистрацияУстановкиЦен
	|ИЗ
	|(ВЫБРАТЬ
	|	УстановкиЦенПоНоменклатуре.Номенклатура КАК Номенклатура,
	|	МАКСИМУМ(УстановкиЦенПоНоменклатуре.ДокУстановкаЦен) КАК ДокУстановкаЦен
	|ИЗ
	|	УстановкиЦенПоНоменклатуре КАК УстановкиЦенПоНоменклатуре
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДатаЦеныНоменклатуры КАК ДатаЦеныНоменклатуры
	|		ПО УстановкиЦенПоНоменклатуре.Номенклатура = ДатаЦеныНоменклатуры.Номенклатура
	|		И УстановкиЦенПоНоменклатуре.ДатаДок = ДатаЦеныНоменклатуры.ДатаДок
	|
	|СГРУППИРОВАТЬ ПО
	|	УстановкиЦенПоНоменклатуре.Номенклатура) ПоследняяЦенаПоНоменклатуре
	|ИНДЕКСИРОВАТЬ ПО
	|	СсылкаКРегистрации
	|;
	//++ Локализация
	...
	|	ШтрихкодыНоменклатуры.Упаковка КАК Упаковка, 
	|	ШтрихкодыНоменклатуры.ШтрихКод КАК ШтрихКод
	|ИЗ РегистрацияПоОстаткамШК КАК РегистрацияПоОстаткамШК
	|	ЛЕВОЕ СОЕДИНЕНИЕ ШтрихкодыНоменклатурыИзменения КАК ИзмененныеШК
	|	ПО РегистрацияПоОстаткамШК.ШтрихКод = ИзмененныеШК.ШтрихКод
	|		И РегистрацияПоОстаткамШК.Номенклатура = ИзмененныеШК.Номенклатура
	|		И РегистрацияПоОстаткамШК.Характеристика = ИзмененныеШК.Характеристика
	|		И РегистрацияПоОстаткамШК.Серия = ИзмененныеШК.Серия
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
	|	ПО РегистрацияПоОстаткамШК.Номенклатура = ШтрихкодыНоменклатуры.Номенклатура
	|		И РегистрацияПоОстаткамШК.Характеристика = ШтрихкодыНоменклатуры.Характеристика
	|		И РегистрацияПоОстаткамШК.Серия = ШтрихкодыНоменклатуры.Серия
	|		И РегистрацияПоОстаткамШК.ШтрихКод = ШтрихкодыНоменклатуры.ШтрихКод
	|ГДЕ
	|	ИзмененныеШК.ШтрихКод ЕСТЬ NULL
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ШтрихкодыНоменклатуры.Номенклатура КАК Номенклатура,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ИзмененныеШКПоЗарегистрированнымКортежам КАК ОтправленныеРанееКортежиШК
	|		ПО ШтрихкодыНоменклатуры.ШтрихКод = ОтправленныеРанееКортежиШК.ШтрихКод
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрацияПоОстаткамШК КАК РегистрацияПоОстаткамШК
	|		ПО ШтрихкодыНоменклатуры.ШтрихКод = РегистрацияПоОстаткамШК.ШтрихКод
	|ГДЕ
	|	РегистрацияПоОстаткамШК.ШтрихКод ЕСТЬ NULL
	|	И ОтправленныеРанееКортежиШК.ШтрихКод ЕСТЬ NULL
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|
	|ВЫБРАТЬ
	|	ПоздняяРегистрацияУстановкиЦен.СсылкаКРегистрации
	|ИЗ
	|	ПоздняяРегистрацияУстановкиЦен КАК ПоздняяРегистрацияУстановкиЦен
	|	ЛЕВОЕ СОЕДИНЕНИЕ ДанныеЗарегистрированныхОбъектов КАК Регистрации
	|		ПО ПоздняяРегистрацияУстановкиЦен.СсылкаКРегистрации = Регистрации.СсылкаКРегистрации
	|ГДЕ
	|	Регистрации.СсылкаКРегистрации ЕСТЬ NULL
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Запрос.УстановитьПараметр("Ценообразование25", ПолучитьФункциональнуюОпцию("ИспользуетсяЦенообразование25"));
	Запрос.УстановитьПараметр("ДатаВыгрузкиДок", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Узел, "ДатаНачалаВыгрузкиДокументов"));
	...
	Для каждого ДанныеШтрихкода Из ИзмененияШтрихкодов Цикл
		...
		Набор.Отбор.Номенклатура.Установить(ДанныеШтрихкода.Номенклатура, Истина);
		Набор.Отбор.Характеристика.Установить(ДанныеШтрихкода.Характеристика, Истина);
		Набор.Отбор.Серия.Установить(ДанныеШтрихкода.Серия, Истина);
		Набор.Отбор.Упаковка.Установить(ДанныеШтрихкода.Упаковка, Истина);
		Набор.Прочитать();
		ПланыОбмена.УдалитьРегистрациюИзменений(Узел, Набор);
	КонецЦикла;
	
	// Добавляем позднюю регистрацию ШК в обмене
	ПоздняяПервичнаяВыгрузкаШК = РезультатЗапроса[РезультатЗапроса.ВГраница()-2].Выгрузить();
	Набор = РегистрыСведений.ШтрихкодыНоменклатуры.СоздатьНаборЗаписей();
	Для каждого ДанныеШтрихкода Из ПоздняяПервичнаяВыгрузкаШК Цикл
		Набор.Отбор.Штрихкод.Установить(ДанныеШтрихкода.Штрихкод, Истина);
		Набор.Отбор.Номенклатура.Установить(ДанныеШтрихкода.Номенклатура, Истина);
		Набор.Отбор.Характеристика.Установить(ДанныеШтрихкода.Характеристика, Истина);
		Набор.Отбор.Серия.Установить(ДанныеШтрихкода.Серия, Истина);
	...
	// Добавляем позднюю регистрацию ШК в обмене
	ПоздняяПервичнаяВыгрузкаЧастичногоВыбытия = РезультатЗапроса[РезультатЗапроса.ВГраница()-3].Выгрузить();
	Набор = РегистрыСведений.ОписаниеНоменклатурыИС.СоздатьНаборЗаписей();
	Для каждого ДанныеНоменклатурыЧастичногоВыбытия Из ПоздняяПервичнаяВыгрузкаЧастичногоВыбытия Цикл
...
Процедура ОбновитьЗависимыеОтборы(Объект) Экспорт
	...
	Объект.СкладыИнформативныхОстатков.Очистить();
	Объект.ИспользоватьОтборПоСкладам = Объект.ИспользоватьОтборПоКассамККМ;
	...
	Если Объект.КассыККМ.Количество() = 0 Тогда
		...
		Объект.ДоговорыПодключенияКПлатежнымСистемам.Очистить(); 
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДоговорыЭквайринга.Ссылка КАК ДоговорПодключения
	|ИЗ
	|	Справочник.ДоговорыЭквайринга КАК ДоговорыЭквайринга
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОрганизацииККМ КАК ОрганизацииККМ
	|		ПО ДоговорыЭквайринга.Организация = ОрганизацииККМ.ОрганизацияКассы
	|ГДЕ
	|	ДоговорыЭквайринга.Ссылка В(&ТекущиеЭквДоговоры)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	ЗапросКоллекций.Параметры.Вставить("ТекущиеЭквДоговоры", 
		Объект.ДоговорыПодключенияКПлатежнымСистемам.Выгрузить().ВыгрузитьКолонку("ДоговорПодключения"));
	...
	Если Объект.ВыгружатьИнформативныеОстатки Тогда
		Объект.СкладыИнформативныхОстатков.Загрузить(РезультатыЗапроса[РазмерПакета-2].Выгрузить());
	КонецЕсли;
	
	Объект.ВидыЦенНоменклатуры.Загрузить(РезультатыЗапроса[РазмерПакета-3].Выгрузить());
	Объект.ДоговорыПодключенияКПлатежнымСистемам.Загрузить(РезультатыЗапроса[РазмерПакета-4].Выгрузить());
	Объект.ЭквайринговыеТерминалы.Загрузить(РезультатыЗапроса[РазмерПакета-5].Выгрузить());
...
// Возвращает идентификатор настройки обменов с 1С:Документооборот 3.0
//
// Возвращаемое значение:
//   Строка - идентификатор настройки
//
Функция ИдентификаторНастройкиОбменаДокументооборот() Экспорт
	
	Возврат "ОбменДОУП";
	
КонецФункции

// Возвращает идентификатор настройки универсального обмена
//
// Возвращаемое значение:
//   Строка - идентификатор настройки
//
Функция ИдентификаторНастройкиОбменаУниверсальный() Экспорт
...
// Это идентификатор настройки с БП3.
// 
// Параметры:
...
// Возвращаемое значение:
//  Булево
Функция ЭтоИдентификаторНастройкиСБП3(ИдентификаторНастройки) Экспорт
	
	Если ПустаяСтрока(ИдентификаторНастройки) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ВариантыНастроекСБП3 = Новый Массив;
	
	ВариантыНастроекСБП3.Добавить("ОбменУПБП");
	ВариантыНастроекСБП3.Добавить(ПланыОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат.ИдентификаторНастройкиОбменаБП30());
	ВариантыНастроекСБП3.Добавить(ПланыОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат.ИдентификаторНастройкиОбменаБПКОРП30()); 
	
	ЭтоВариантНастройкиСБП3 = ВариантыНастроекСБП3.Найти(ИдентификаторНастройки) <> Неопределено; 
	
	Возврат ЭтоВариантНастройкиСБП3;
