...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	СобытияФормИСКлиентПереопределяемый.ОбработкаВыбораНоменклатуры(ОписаниеОповещения, ВыбранноеЗначение,
		ИсточникВыбора);
...
Процедура ТоварыНоменклатураСоздание(Элемент, СтандартнаяОбработка)
	...
	СобытияФормВЕТИСКлиент.ПриСозданииНоменклатуры(Элемент, Элементы.Товары.ТекущиеДанные, СтандартнаяОбработка);
...
Процедура ДополнитьСтрокуПредставленияСтатусаВЕТИС(ПараметрыСтатуса, ПараметрыДополненияСтатусаВЕТИС)
	...
	Если ДополнитьСтрокуСтатуса Тогда
		...
		Если ЕстьДополнительнаяСтрока Тогда
			...
			ПредставлениеПредметаМаршрутаВозврата = Сред(ПредставлениеПредметаМаршрутаВозврата, СтрНайти(ПредставлениеПредметаМаршрутаВозврата, "|") + 1);
...
Процедура ОткрытьПодборСерий(Текст, СтандартнаяОбработка)
	...
	Если ПараметрыУказанияСерий <> Неопределено Тогда
		...
		ИнтеграцияИСКлиент.ОткрытьПодборСерий(ЭтотОбъект, ПараметрыУказанияСерий.Товары, Текст, СтандартнаяОбработка);
...
// 
// Параметры:
// 	Основания - Массив Из СправочникСсылка.ВетеринарноСопроводительныйДокументВЕТИС, СправочникСсылка.ВетеринарноСопроводительныйДокументВЕТИС - ВСД источник заполнения
&НаСервере
	...
	Если СтруктураРеквизитовИзменяемыхНаСервере().Свойство(ИмяОбновляемогоПоля) Тогда
		...
		Если Результат <> Неопределено И ТипЗнч(Результат) = Тип("Строка") И ДополнительныеПараметры = "Маршрут" Тогда
			...
			Если ПоследнийПунктМаршрута <> Неопределено И ПоследнийПунктМаршрута.ЕстьОтличия Тогда
				Оповещение = Новый ОписаниеОповещения("ВопросОбновитьНомерТС", ЭтотОбъект, Новый Структура("Результат", Результат));
