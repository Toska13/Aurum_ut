...
// 	 КодыОпераций - Соответствие:
//   *Ключ - СправочникСсылка.Номенклатура
//   *Значение - Строка - код операции для номенклатуры.
//
// Возвращаемое значение:
// 	ТаблицаЗначений - Товары по документу-основанию.
//
Функция ТоварыПоДокументыОснованию(ДокументОснование, ДатаСчетаФактуры, КодыОпераций = Неопределено) Экспорт
	...
	ПараметрыЗаполнения.Вставить("РеализацияЧерезКомиссионера", 
		?(ОбщегоНазначения.ЕстьРеквизитОбъекта("ХозяйственнаяОперация",ДокументОснование.Метаданные()), 
			...
			Ложь));
	ЭтоНачислениеНДСПриОтгрузке = ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту");
	ПараметрыЗаполнения.Вставить("НачислениеНДСПриОтгрузке", ЭтоНачислениеНДСПриОтгрузке);
	ПараметрыЗаполнения.Вставить("ВалютаВзаиморасчетов", ?(ЭтоНачислениеНДСПриОтгрузке, 
															ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "Договор.ВалютаВзаиморасчетов"),
															ПараметрыЗаполнения.Валюта));
	Если ЭтоНачислениеНДСПриОтгрузке Тогда 
		Отбор = Новый Структура();
		Отбор.Вставить("Исправление", Ложь);
		Отбор.Вставить("Корректировочный", Ложь);
		Отбор.Вставить("Перевыставленный", Ложь);
		ТаблицаСчетаФактуры = Документы.СчетФактураВыданный.СчетаФактурыПоОснованию(ДокументОснование, Отбор, Неопределено, Истина);
		Если ТаблицаСчетаФактуры.Количество() > 0 Тогда 
			ПараметрыЗаполнения.Вставить("ПлатежноРасчетныеДокументы", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТаблицаСчетаФактуры[0].Ссылка, "ПлатежноРасчетныеДокументы").Выгрузить());
		КонецЕсли;
	КонецЕсли;
	...
	ТаблицаТоваров.Колонки.Добавить("КодОперации");
	...
	Для Каждого Строка Из ТаблицаТоваров Цикл
		...
		Если ЗначениеЗаполнено(Строка.Номенклатура) Тогда
			...
			Если КодыОпераций <> Неопределено Тогда
				Строка.КодОперации = КодыОпераций.Получить(Строка.Номенклатура);
			КонецЕсли;
	...
	ПараметрыЗаполненияСтавкиНДС.Организация = Организация; 
	ПараметрыЗаполненияСтавкиНДС.НалогообложениеНДС = ПараметрыУчетаОрганизации.ОсновноеНалогообложениеНДСПродажи;
...
Функция ЕстьСчетаФактурыКОформлению(Параметры) Экспорт
	...
	|	ГДЕ
	|		НДССостояниеРеализации0.Состояние В (ЗНАЧЕНИЕ(Перечисление.НДССостоянияРеализация0.ОжидаетсяПодтверждение),
	...
	|		И &УсловиеОтбора
	|		И ТИПЗНАЧЕНИЯ(НДССостояниеРеализации0.Регистратор) <> ТИП(Документ.ПодтверждениеНулевойСтавкиНДС)
	|	) КАК Подзапрос";
...
Функция ПолучитьДанныеДляПечатнойФормыСчетФактура(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ДанныеДокументов.СтрокаАвансовыеСчетаФактуры КАК СтрокаАвансовыеСчетаФактуры,
	...
	|	СчетаФактуры.СтрокаАвансовыеСчетаФактуры КАК ПредставлениеАвансовыеСчетаФактуры,
...
Функция ТекстЗапросаТаблицаНДСЗаписиКнигиПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|	Товары.КодОперации КАК КодОперации0
	|ИЗ
	|	Документ.СчетФактураНаНеподтвержденнуюРеализацию0.Товары КАК Товары
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПодтверждениеНулевойСтавкиНДС КАК ПодтверждениеНулевойСтавкиНДС
	|			ПО НДССостояниеРеализации0.Регистратор = ПодтверждениеНулевойСтавкиНДС.Ссылка
	|ГДЕ
	|	Товары.Ссылка = &Ссылка
	|	И Товары.ТипЗапасов В (ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Услуга),
	|						   ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар),
	|						   ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Работа))
	|	И ПодтверждениеНулевойСтавкиНДС.Ссылка ЕСТЬ NULL
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураНаНеподтвержденнуюРеализацию0.ПлатежноРасчетныеДокументы КАК ПлатежноРасчетныеДокументы
	|			ПО Товары.Ссылка = ПлатежноРасчетныеДокументы.Ссылка
	|	
	|ГДЕ
	|	Товары.Ссылка = &Ссылка
	|	И Товары.ТипЗапасов В (ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Услуга),
...
Функция ТекстЗапросаПолученияНомеровИДатПлатежноРасчетныхДокументовПриОбновлении()
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураНаНеподтвержденнуюРеализацию0.ПлатежноРасчетныеДокументы КАК ПлатежныеДокументы
	...
	|ИЗ
	|	ТаблицаЗаполненияПлатежноРасчетныхДокументов КАК ОбновляемыеДанные
	|		ЛЕВОЕ СОЕДИНЕНИЕ
	|		Документ.ОтгрузкаТоваровКлиенту КАК ОтгрузкаТоваровКлиенту
	|		ПО
	|			ОбновляемыеДанные.ДокументОснование = ОтгрузкаТоваровКлиенту.Ссылка
	|	
	|		ЛЕВОЕ СОЕДИНЕНИЕ
	...
	|ГДЕ
	|	ОтгрузкаТоваровКлиенту.Ссылка ЕСТЬ NULL
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|ВЫБРАТЬ
	...
	|	ЗНАЧЕНИЕ(Документ.СчетФактураВыданныйАванс.ПустаяСсылка) КАК СчетФактураАванс,
	|	ЗНАЧЕНИЕ(Документ.СчетФактураВыданныйАванс.ПустаяСсылка) КАК ИсправлениеСчетаФактурыАванс,
	...
	|		ПО
	|			ОбновляемыеДанные.ДокументОснование = СчетФактураДокументыОснования.ДокументОснование
	|			И НЕ СчетФактураДокументыОснования.Ссылка.Корректировочный
	|			И НЕ СчетФактураДокументыОснования.Ссылка.Исправление
	|	
	|		ЛЕВОЕ СОЕДИНЕНИЕ
	|		Документ.СчетФактураВыданный.ПлатежноРасчетныеДокументы КАК ТаблицаПлатежноРасчетныеДокументы
	|		ПО
	|			СчетФактураДокументыОснования.Ссылка = ТаблицаПлатежноРасчетныеДокументы.Ссылка
	|	
	|ИТОГИ ПО
	|	СчетФактура
	|;
...
Функция ИмяФормыСчетФактураНаНеподтвержденнуюРеализацию0() Экспорт
	...
	Возврат "Документ.ПодтверждениеНулевойСтавкиНДС.Форма.ФормаРабочееМесто";
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Документы.СчетФактураНаНеподтвержденнуюРеализацию0.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "2.5.26.80";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("de97bc49-9d7a-47a9-a0ec-a9afbf8c582a");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.СчетФактураНаНеподтвержденнуюРеализацию0.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Порядок = Перечисления.ПорядокОбработчиковОбновления.Некритичный;
	Обработчик.Комментарий = НСтр("ru = 'Заполнение табличной части Платежно-расчетные документы Счет-фактур на неподтвержденную реализацию по ставке 0%'");
	...
	Изменяемые.Добавить(Метаданные.Документы.СчетФактураНаНеподтвержденнуюРеализацию0.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.СчетФактураНаНеподтвержденнуюРеализацию0 КАК СчетФактураВыданный
	|ГДЕ
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураНаНеподтвержденнуюРеализацию0 КАК ТаблицаОснований
