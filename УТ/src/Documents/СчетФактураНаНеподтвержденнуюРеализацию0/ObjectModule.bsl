...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если Метаданные.Документы.СчетФактураНаНеподтвержденнуюРеализацию0.Реквизиты.ДокументОснование.Тип.СодержитТип(ТипЗнч(Основание)) Тогда
		ТаблицаТоваров = Документы.СчетФактураНаНеподтвержденнуюРеализацию0.ТоварыПоДокументыОснованию(
							Основание,
							ДанныеЗаполнения.Дата,
							?(ДанныеЗаполнения.Свойство("КодыОпераций0"), ДанныеЗаполнения.КодыОпераций0, Неопределено));
		Товары.Загрузить(ТаблицаТоваров);
	КонецЕсли;
	
	ПредставлениеСтроки5а = ПолучитьПредставлениеСтроки5а(Основание, РеквизитыОснования);
	...
	СформироватьСтрокуАвансовыеСчетаФактуры();
...
Процедура СформироватьСтрокуАвансовыеСчетаФактуры()
	...
	Если СтрокаАвансовыеСчетаФактуры <> СтрокаНомеровИДата Тогда
		СтрокаАвансовыеСчетаФактуры = СтрокаНомеровИДата;
	КонецЕсли;
	
КонецПроцедуры

Функция ПолучитьПредставлениеСтроки5а(Основание, РеквизитыОснования)
	
	НаименованияДокументовДанныеПечати = ОбщегоНазначенияУТ.НаименованияЭлектронныхДокументовИлиОсновныхПечатныхФорм(Основание);
	ВерсияПостановления1137 = УчетНДСПереопределяемый.ВерсияПостановленияНДС1137(РеквизитыОснования.Дата);
	
	ТекстТоварнаяНакладная = НСтр("ru = 'Товарная накладная'");
	ТекстАкт = НСтр("ru = 'Акт'");
	...
	Если Выборка.Следующий() Тогда 
		НаименованиеДокумента = ?(Выборка.ЕстьТовары, ТекстТоварнаяНакладная, ТекстАкт);
	КонецЕсли;
	...
	Если ВерсияПостановления1137 >= 7 Тогда
		ПредставлениеСтроки5а = 
			СтрШаблон(
				НСтр("ru = '%1, № %2 от %3'"),
				?(НаименованияДокументовДанныеПечати[Основание] <> Неопределено,
					НаименованияДокументовДанныеПечати[Основание], НаименованиеДокумента),
