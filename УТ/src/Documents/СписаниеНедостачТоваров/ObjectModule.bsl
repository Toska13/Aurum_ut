...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		ИначеЕсли ДанныеЗаполнения.Свойство("АктОРасхождениях")
			...
			ИначеЕсли ТипЗнч(ДанныеЗаполнения.АктОРасхождениях) = Тип("ДокументСсылка.АктОРасхожденияхПослеОтгрузки") Тогда
				ЗаполнитьДокументНаОснованииАктаОтгрузки(ДанныеЗаполнения);
...
Процедура ЗаполнитьПоАктуКонтроляКачества(ДанныеЗаполнения)
	...
	Документы.АктКонтроляКачестваТоваров.ТоварыДляСозданияНаОсновании(
		...
		Перечисления.ДействияПоТоварамВАктеКонтроляКачества.ОформитьСписание);
	...
	Если ТоварыОснования.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ru='Документ %Документ% не содержит товаров c действием ""Оформить списание"". Ввод на основании документа запрещен.'");
	...
	ТоварыОснования.Колонки.Добавить("КонтролируемаяПартия");
	Если Не СкладыСервер.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач(Склад) Тогда
		ТоварыОснования.ЗаполнитьЗначения(РеквизитыШапки.КонтролируемаяПартия, "КонтролируемаяПартия");
	КонецЕсли;
...
Процедура ЗаполнитьДокументНаОснованииАктаОтгрузки(ДанныеЗаполнения)
	...
	|	АктОРасхожденияхПослеОтгрузки.Подразделение КАК Подразделение,
	|	&Склад                                      КАК Склад,
	|	&Основание                                  КАК ОснованиеАктаОРасхождениях,
	|	АктОРасхожденияхПослеОтгрузки.Статус        КАК СтатусАкт,
	|	НЕ АктОРасхожденияхПослеОтгрузки.Проведен   КАК ЕстьОшибкиПроведенАкт,
	...
	|	И АктОРасхожденияхПослеОтгрузкиТовары.Склад = &Склад
	|	И АктОРасхожденияхПослеОтгрузкиТовары.ДействиеСТоваром = ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.Списать)
	|
	|УПОРЯДОЧИТЬ ПО
	...
	ПараметрыПроверки.Статус                   = ВыборкаШапка.СтатусАкт;
	ПараметрыПроверки.ЕстьОшибкиПроведен       = ВыборкаШапка.ЕстьОшибкиПроведенАкт;
	ПараметрыПроверки.ЕстьОшибкиСтатус         = ВыборкаШапка.ЕстьОшибкиСтатусАкт;
	ПараметрыПроверки.МассивДопустимыхСтатусов = РасхожденияСервер.МассивДопустимыхСтатусовАктовОРасхожденияхПриСозданииНаОсновании();
	ОбщегоНазначенияУТ.ПроверитьВозможностьВводаНаОснованииСПараметрами(ДанныеЗаполнения.АктОРасхождениях, ПараметрыПроверки); 
	
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, ВыборкаШапка);
	
	РезультатЗапросаТовары = ПакетЗапросов[1];
	Если Не РезультатЗапросаТовары.Пустой() Тогда
		
		Товары.Загрузить(РезультатЗапросаТовары.Выгрузить());
		
	ИначеЕсли ЗначениеЗаполнено(ДанныеЗаполнения.АктОРасхождениях) Тогда  
		
		ТекстСообщения = НСтр("ru = 'В документе ""%Реализация%"" отсутствуют товары, по которым необходимо оформить списание.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Реализация%", ДанныеЗаполнения.АктОРасхождениях);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект);
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	Если Не Проведен
		...
		ВидыЗапасов.Свернуть("АналитикаУчетаНоменклатуры, ВидЗапасов, НомерГТД", ИменаКолонокСуммирования);
