...
Процедура НадписьВалютыНажатиеЗавершениеСервер(РезультатЗакрытия)
	...
	Если ИзмененныеРеквизиты.Свойство("ВалютаВзаиморасчетов") Тогда
		...
		ЗаполнитьБанковскиеСчетаПоУмолчаниюСервер();
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ЦеныПредприятияЗаполнениеСервер.УстановитьУсловноеОформлениеВидовЦен(ЭтотОбъект, "ТоварыВидЦеныПартнера");
...
Процедура ОрганизацияПолучательПриИзмененииСервер()
	...
	ДенежныеСредстваСервер.ПроверитьЗаполнитьКассуОрганизацииПоВладельцу(Объект.ОрганизацияПолучатель,
...
Процедура ДоговорПриИзмененииСервер()
	...
	Если Объект.ОплатаВВалюте <> ОплатаВВалюте Тогда
		Объект.БанковскийСчетОрганизации = Справочники.АвансовыйОтчетПрисоединенныеФайлы.ПустаяСсылка();
		Объект.БанковскийСчетОрганизацииПолучателя = Справочники.БанковскиеСчетаОрганизаций.ПустаяСсылка();
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьБанковскиеСчетаПоУмолчаниюСервер()
	...
	ПараметрыСчета = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации(Объект.ОрганизацияПолучатель);
	ПараметрыСчета.НаправлениеДеятельности = Объект.НаправлениеДеятельности;
	ПараметрыСчета.ИсключитьСчетаВВалюте = Объект.ОплатаВВалюте;
	ПараметрыСчета.Валюта = ВалютаРегламентированногоУчета;
	Объект.БанковскийСчетОрганизацииПолучателя = ДенежныеСредстваСервер.ПолучитьБанковскийСчетОрганизацииПоУмолчанию(
		ПараметрыСчета, Объект.БанковскийСчетОрганизацииПолучателя);
