...
Функция ПараметрыВзаиморасчеты(ДанныеЗаполнения = Неопределено) Экспорт
	...
	//Используется для заполнения значений по умолчанию, заполнения графика плановых оплат и даты платежа.
	СтруктураПараметров.Соглашение                       = "";
	СтруктураПараметров.Касса                            = "Объект.КассаПолучатель";
...
Функция ПредставлениеОбъектаНаОсновномЯзыке() Экспорт
	...
	СинонимДляЗаписиВИнформационнуюБазу = НСтр(
		"ru = 'Передача товаров между организациями'",
		КодОсновногоЯзыка);
...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	|	КОНЕЦ 											КАК НДСРегл,
	|	ТаблицаВидыЗапасов.СуммаСНДСУпр - ТаблицаВидыЗапасов.СуммаНДСУпр КАК СтоимостьУпр,
	|	ВЫБОР КОГДА ТаблицаВидыЗапасов.ВидДеятельностиНДС <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОпределяетсяРаспределением)
...
Функция ДанныеДляПечатныхФормСчетаНаОплатуИзвещения(ПараметрыПечати, МассивОбъектов, КодЯзыка = Неопределено) Экспорт
	...
	|	ЕСТЬNULL(Документы.БанковскийСчетКонтрагента.СчетЦифровогоРубля, ЛОЖЬ) КАК ЭтоСчетЦифровогоРубля,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(Документы.БанковскийСчетКонтрагента.СчетЦифровогоРубля, ЛОЖЬ)
	|			ТОГДА Документы.БанковскийСчетКонтрагента.ИдентификаторСчетаЦифровогоРубля
