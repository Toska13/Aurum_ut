...
Процедура ПринятьТоварНаСклад(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ДанныеШапки.ДатаОтгрузки          КАК Дата,
	|	ДанныеШапки.Склад                 КАК Склад,
	|	ДанныеШапки.ЗонаПриемки           КАК Ячейка,
	|	ДанныеШапки.ПомещениеПолучатель   КАК Помещение,
	...
	|	И ДанныеШапки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОрдеровНаПеремещение.Принят)
