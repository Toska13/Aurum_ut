...
Процедура ЗаполнитьТаблицуРезультатов(Объект) Экспорт
	...
	Запрос.УстановитьПараметр("КонецПериода",			Новый Граница(КонецМесяца(Объект.Дата), ВидГраницы.Включая));
...
Функция ЗаполнитьВводОстатковТоваровНаОснованииСверки(ЗаполняемыйДокумент, ДанныеЗаполнения, ИнтерактивноеЗаполнение = Ложь) Экспорт
	...
	СтруктураПересчетаСуммы = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(ЗаполняемыйДокумент);
	
	РезультатЗапросаДанныеСверки = РезультатЗапроса[1]; // РезультатЗапроса
	ТаблицаСверки = РезультатЗапросаДанныеСверки.Выгрузить();
	...
	Для Каждого ДанныеСверки Из ТаблицаСверки Цикл
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		...
		СтруктураДействий,
		ЗаполняемыйДокумент.Товары);
	
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ПересчитатьНДСРегл", СтруктураПересчетаСуммы);
	Если ЗаполняемыйДокумент.ЦенаВключаетНДС Тогда
	...
	Для Каждого ТекущаяСтрока Из ЗаполняемыйДокумент.Товары Цикл
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ЗаполняемыйДокумент.Товары, СтруктураДействий);
