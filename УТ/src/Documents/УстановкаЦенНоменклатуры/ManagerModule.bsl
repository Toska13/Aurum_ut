		...
		Если ТипЗнч(УстанавливатьВременныеЦены) = Тип("Булево") Тогда
			...
			Для каждого СтрокаТЗ Из ТаблицаОбработкиПериодовЦен Цикл
				Если СтрокаТЗ.УдалитьКортеж Тогда
					...
					Если СтрокаТЗ.ЕстьСовпадающиеВидыЦен Тогда
...
Функция ПолучитьТаблицуОбработкиПериодов(УстанавливатьВременныеЦены,МассивДокументы,СохраненныеДокументыПериодическихЦен)
	...
	|ПОМЕСТИТЬ ВидыЦенНовыхДокументов
	|ИЗ
	|	Документ.УстановкаЦенНоменклатуры.ВидыЦен КАК ТаблицаВидовЦен
	|ГДЕ
	|	ТаблицаВидовЦен.Ссылка В
	|		(ВЫБРАТЬ
	|			ТекущиеДокументыНачалаПериода.Ссылка
	|		ИЗ
	|			ТекущиеДокументыНачалаПериода)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТаблицаВидовЦен.ВидЦены КАК ВидЦены
	|ПОМЕСТИТЬ ВидыЦенСтарыхДокументов
	|ИЗ
	|	Документ.УстановкаЦенНоменклатуры.ВидыЦен КАК ТаблицаВидовЦен
	|ГДЕ
	|	ТаблицаВидовЦен.Ссылка В
	|		(ВЫБРАТЬ
	|			СохраненныеПарыДокументовПериодов.ДокументНачалаПериодаДействияЦен
	|		ИЗ
	|			СохраненныеПарыДокументовПериодов)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ИСТИНА КАК ВидЦеныСовпадает
	|ПОМЕСТИТЬ ЕстьСовпадающиеВидыЦен
	|ИЗ
	|	ВидыЦенНовыхДокументов КАК ВидыЦенНовыхДокументов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВидыЦенСтарыхДокументов КАК ВидыЦенСтарыхДокументов
	|		ПО ВидыЦенНовыхДокументов.ВидЦены = ВидыЦенСтарыхДокументов.ВидЦены
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ИСТИНА В
	|		(ВЫБРАТЬ
	|			ЕстьСовпадающиеВидыЦен.ВидЦеныСовпадает
	|		ИЗ
	|			ЕстьСовпадающиеВидыЦен) КАК ЕстьСовпадающиеВидыЦен,
	|	ВЫБОР
...
Функция ТекстЗапросаТаблицаЦеныНоменклатуры2_5(Запрос, ТекстыЗапроса, Регистры)
	...
	|	КОНЕЦ КАК Валюта,
	|	ВЫБОР
	|		КОГДА (ВидыЦенСпр.ВариантУказанияСтавокНДС = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСтавокНДСВПрайсЛисте.УказыватьОбщуюСтавкуНДС)
	|				ИЛИ ВидыЦенСпр.ВариантУказанияСтавокНДС = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСтавокНДСВПрайсЛисте.УказыватьСтавкиНДСПострочно))
