...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не Отказ Тогда
		...
		ДоступноУказаниеПроизводственнойПлощадки = МаркировкаТоваровКлиентСервер.ДоступноПолеПроизводственнаяПлощадкаВЕТИС(
...
Процедура ТоварнаяГруппаПриИзменении(Элемент)
	...
	ПриИзмененииТоварнаяГруппаНаСервере(Ложь);
	ПриИзмененииТоварнаяГруппаНаКлиенте();
...
Процедура ТоварнаяГруппаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ИначеЕсли Объект.ДанныеОтчетаПроизводственнойЛинии.Количество() Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'При изменении вида продукции табличная часть ""Коды маркировки""
				|будет очищена. Продолжить?'"),
			РежимДиалога);
		
	ИначеЕсли Объект.Товары.Количество() Тогда
		...
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ВопросПриИзмененииТоварнойГруппыПриЗавершении", ЭтотОбъект, ВыбранноеЗначение),
			НСтр("ru = 'При изменении вида продукции табличная часть ""Товары"" будет очищена. Продолжить?'"),
			РежимДиалога);
		
	ИначеЕсли Объект.ДокументыСертификации.Количество() Тогда
		
		СтандартнаяОбработка = Ложь;
		ВопросПриИзмененииТоварнойГруппыПриЗавершении(КодВозвратаДиалога.Да, ВыбранноеЗначение);
...
&НаКлиенте
Процедура ТоварнаяГруппаНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ТоварныеГруппы", ДоступныеТоварныеГруппы);
	
	ОткрытьФорму("Справочник.ТоварныеГруппыГИСМТ.ФормаВыбора", ПараметрыФормы, Элемент);
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	Если Не Объект.Ссылка.Пустая() Тогда
		ДанныеДокумента = Новый Структура("ТоварнаяГруппа, Операция");
	...
	ИдентификаторПроисхожденияВЕТИССтрокой = Объект.ИдентификаторПроисхожденияВЕТИССтрокой;
...
Процедура ЗаполнитьДоступныеОперации()
	...
	ДоступныеОперации = УточнениеСведенийОКодахМаркировкиКлиентСервер.ДоступныеОперации(Объект.ТоварнаяГруппа);
	
	Элементы.Операция.СписокВыбора.ЗагрузитьЗначения(ДоступныеОперации);
	...
	Если ДанныеДокумента <> Неопределено
		И ДанныеДокумента.ТоварнаяГруппа = Объект.ТоварнаяГруппа
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если Инициализация Или СтруктураРеквизитов.Свойство("ТоварнаяГруппа") Тогда
		
		Если ЭтоПродукцияПодконтрольнаяВЕТИС Тогда
			
			Если Не Форма.ИспользуетсяПодсистемаВЕТИС Тогда
				Элементы.ОтчетПроизводственнойЛинии.ТолькоПросмотр = Истина;
	...
	Если Инициализация Или СтруктураРеквизитов.Свойство("Операция") Тогда
		...
		ДоступенОтчетПроизводственнойЛинии = МаркировкаТоваровКлиентСервер.ДоступенОтчетПроизводственнойЛинии(
			ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
		Если Не ДоступенОтчетПроизводственнойЛинии Тогда
		...
		РедактированиеСертификации = УточнениеСведенийОКодахМаркировкиКлиентСервер.ДоступноПолеСертификация(
			ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
		Элементы.СтраницаДокументыСертификации.Видимость = РедактированиеСертификации;
		// устаревший реквизит, для новых видов операций не используется
		Элементы.Сертификация.Видимость = Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийИСМП.СведенияОКодахИдентификацииДляВводаВОборот");
		
		РедактированиеТНВЭД = МаркировкаТоваровКлиентСервер.ДоступноПолеТНВЭД(ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
		Элементы.КодТНВЭД.Видимость       = РедактированиеТНВЭД;
		Элементы.ТоварыКодТНВЭД.Видимость = РедактированиеТНВЭД;
		
		ДоступнаДатаПроизводства = МаркировкаТоваровКлиентСервер.ДляТоварнойГруппыИспользуетсяДатаПроизводства(
			ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
		Элементы.ДатаПроизводства.Видимость       = ДоступнаДатаПроизводства;
		Элементы.ТоварыДатаПроизводства.Видимость = ДоступнаДатаПроизводства;
		
		Элементы.СпособВводаВОборот.Видимость     = УточнениеСведенийОКодахМаркировкиКлиентСервер.ДоступноПолеСпособВводаВОборот(
		...
		Элементы.ГруппаВЕТИС.Видимость                            = РедактированиеВСД И ОтчетПроизводственнойЛинии;
		...
		Элементы.ТоварыИдентификаторПроисхожденияВЕТИС.Видимость  = РедактированиеВСД И Не ОтчетПроизводственнойЛинии;
		
		Элементы.ГруппаМолочнаяПродукция.Видимость = МаркировкаТоваровКлиентСервер.ДоступноПолеСкоропортящаясяПродукция(
			ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
		...
		Элементы.ГруппаПроизводственнаяПлощадкаВЕТИС.Видимость = МаркировкаТоваровКлиентСервер.ДоступноПолеПроизводственнаяПлощадкаВЕТИС(
			ВидыПродукцииПоТоварнойГруппе, Операция);
		...
		ДоступноПолеЛицензияНаПользованиеНедрами = МаркировкаТоваровКлиентСервер.ДоступноПолеЛицензияНаПользованиеНедрами(
			ВидыПродукцииПоТоварнойГруппе, Операция);
		Элементы.ГруппаЛицензииНаПользованиеНедрами.Видимость = ДоступноПолеЛицензияНаПользованиеНедрами;
		Элементы.ГруппаРазрешительнаяДокументация.ОтображатьЗаголовок = ДоступноПолеЛицензияНаПользованиеНедрами;
...
Процедура ОбновитьСтатусИСМП()
	...
	Реквизиты = Новый Структура("ТоварнаяГруппа, Операция");
...
Процедура ВопросПриИзмененииТоварнойГруппыПриЗавершении(РезультатВопроса, ВыбраннаяТоварнаяГруппа) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Или РезультатВопроса = Истина Тогда
		...
		ПриИзмененииТоварнаяГруппаНаСервере(РезультатВопроса = Истина);
		ПриИзмененииТоварнаяГруппаНаКлиенте();
...
&НаКлиенте
Процедура ПриИзмененииТоварнаяГруппаНаКлиенте()
...
&НаСервере
Процедура ПриИзмененииТоварнаяГруппаНаСервере(ПерезаполнитьПоОснованию)
	
	ИнтеграцияИСМП.НастроитьВидимостьКолонкиКоличестваКодовМаркировки(ЭтотОбъект);
	
	ЗаполнитьДоступныеОперации();
	Если Элементы.Операция.СписокВыбора.НайтиПоЗначению(Объект.Операция) = Неопределено Тогда
		Если Элементы.Операция.СписокВыбора.Количество() Тогда
...
Процедура ЗаполнитьДоступныеВидыДокументовСертификации()
	...
	ИнтеграцияИСМП.ЗаполнитьДоступныеВидыДокументовСертификации(
		Элементы.ДокументыСертификацииВидДокументаСертификации, Объект.ТоварнаяГруппа);
...
Процедура ПослеОбработкиШтрихкодов(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		Если ДанныеВыбораПоМаркируемойПродукции<>Неопределено Тогда
			...
			Если ИнтеграцияИСМПВЕТИСКлиентСервер.ЭтоВидПродукцииИСМПВЕТИС(Объект.ТоварнаяГруппа)
...
Процедура ЗаполнитьДанныеВЕТИСПоСтроке(Форма, ДобавленныеСтроки)
	...
	Если Не ИнтеграцияИСМПВЕТИСКлиентСервер.ЭтоВидПродукцииИСМПВЕТИС(Форма.Объект.ТоварнаяГруппа) Тогда
		Возврат;
	КонецЕсли;
