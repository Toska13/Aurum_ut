...
Функция ТекстЗапросаТаблицаРасчетыСПоставщиками(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект = Неопределено)
	...
	|	Шапка.Партнер КАК Партнер,
	|	Шапка.Договор КАК Договор,
	|	ВЫБОР КОГДА ЕСТЬNULL(Шапка.НаправлениеДеятельности.УчетРасчетовСПоставщиками, ЛОЖЬ)
	|					И НЕ ЕСТЬNULL(Шапка.ОбъектРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности, ИСТИНА)
	|			ТОГДА Шапка.НаправлениеДеятельности
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
	|	КОНЕЦ КАК НаправлениеДеятельности,
	|	Шапка.ОбъектРасчетов КАК ОбъектРасчетов,
	...
	|			И ВЫБОР КОГДА ЕСТЬNULL(Шапка.НаправлениеДеятельности.УчетРасчетовСПоставщиками, ЛОЖЬ)
	|						И НЕ ЕСТЬNULL(Шапка.ОбъектРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности, ИСТИНА)
	|					ТОГДА Шапка.НаправлениеДеятельности
	|				ИНАЧЕ ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
	|			КОНЕЦ = РегистрАналитикаУчетаПоПартнерам.НаправлениеДеятельности
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтКурсыВалют КАК КурсВалюты
	|		ПО Шапка.ВалютаВзаиморасчетов = КурсВалюты.ВалютаВзаиморасчетов
	|			И Шапка.ОбъектРасчетов = КурсВалюты.ОбъектРасчетов
	|ГДЕ
	...
	|		ИЛИ ЕСТЬNULL(Шапка.ОбъектРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности, ИСТИНА))";
