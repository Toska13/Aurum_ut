...
Процедура ПриСозданииЧтенииНаСервере()
	...
	Если Не ПодключенаОбработкаКодовМаркировки Тогда
		...
		Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСоСрокамиГодности(Объект.ТоварнаяГруппа,, Объект.Операция) Тогда
...
Процедура НастроитьЭлементыФормы(Форма)
	...
	Элементы.СтранаНазначения.Видимость    = ВыводИзОборотаКлиентСервер.ДоступноПолеСтранаНазначения(ВидыПродукцииПоТоварнойГруппе, Операция);
	Элементы.ТоварыСертификация.Видимость  = МаркировкаТоваровКлиентСервер.ДоступноПолеСертификация(ВидыПродукцииПоТоварнойГруппе);
	Элементы.ОтгрузкаНеучастнику.Видимость = ОтгрузкаТоваровКлиентСервер.ДоступноПолеОтгрузкаНеучастнику(ВидыПродукцииПоТоварнойГруппе, Операция);
	
	Элементы.ЗаполнитьДокументыСертификации.Видимость = МаркировкаТоваровКлиентСервер.ДоступноПолеСертификация(ВидыПродукцииПоТоварнойГруппе, Операция);
	...
	Если МаркировкаТоваровКлиентСервер.ДляТоварнойГруппыИспользуетсяДатаПроизводства(ВидыПродукцииПоТоварнойГруппе, Операция) Тогда
		...
		Элементы.ТоварыДатаПроизводства.АвтоОтметкаНезаполненного =
			КонструкторИСМПКлиентСервер.ЭтоВидПродукцииСОбязательнымУказаниемДатыПроизводства(ВидыПродукцииПоТоварнойГруппе, Операция);
	Иначе
		Элементы.ТоварыДатаПроизводства.Видимость = Ложь;
	КонецЕсли;
	
	Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСоСрокамиГодности(ВидыПродукцииПоТоварнойГруппе,, Операция) Тогда
		...
		Элементы.ТоварыСрокГодности.АвтоОтметкаНезаполненного =
			КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСОбязательнымУказаниемСроковГодности(ВидыПродукцииПоТоварнойГруппе, Операция);
	...
	Элементы.ТоварыДанныеИмпорта.Видимость = ОтгрузкаТоваровКлиентСервер.ДоступноПолеДанныеИмпорта(ВидыПродукцииПоТоварнойГруппе, Операция);;
	Элементы.ТоварыПроизводитель.Видимость = ОтгрузкаТоваровКлиентСервер.ДоступноПолеПроизводитель(ВидыПродукцииПоТоварнойГруппе, Операция);;
	...
	ДоступноПолеGTIN = МаркировкаТоваровКлиентСервер.ДоступноПолеGTIN(ВидыПродукцииПоТоварнойГруппе, Операция);
	
	Элементы.ТоварыGTIN.Видимость                = ДоступноПолеGTIN;
	Элементы.ТоварыСтатусПроверкиГосИС.Видимость = Не ДоступноПолеGTIN;
	Элементы.ЗаполнитьДоступныеGTIN.Видимость    = ДоступноПолеGTIN;
...
Процедура ЗаполнитьИдентификаторыВСДНаФорме()
	...
	Если Не ОтгрузкаТоваровКлиентСервер.ДоступноПолеИдентификаторПроисхождения(Объект.ТоварнаяГруппа, Объект.Операция) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьИнформациюОткрытияФормыПроверкиПодбора()
	...
	Если МаркировкаТоваровКлиентСервер.ДоступноПолеGTIN(Объект.ТоварнаяГруппа, Объект.Операция) Тогда
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Элементы.GLNКонтрагента.Видимость = ОтгрузкаТоваровКлиентСервер.ДоступноПолеПолеGLNКонтрагента(Объект.ТоварнаяГруппа, Объект.Операция);
	
	ДоступноПолеGTIN           = МаркировкаТоваровКлиентСервер.ДоступноПолеGTIN(Объект.ТоварнаяГруппа, Объект.Операция);
...
Процедура ВопросПриИзмененииТоварнойГруппыПриЗавершении(РезультатВопроса, ВыбраннаяТоварнаяГруппа) Экспорт
	...
	ПриИзмененииТоварнойГруппы(Элементы.ТоварнаяГруппа);
...
&НаСервере
Процедура ПриИзмененииТоварнаяГруппаНаСервере()
	...
	Если Элементы.Операция.СписокВыбора.НайтиПоЗначению(Объект.Операция) = Неопределено Тогда
		...
		Иначе
			Объект.Операция = Перечисления.ВидыОперацийИСМП.ПустаяСсылка();
		КонецЕсли;
...
Процедура ЗаполнитьДоступныеОперации()
	...
	ДоступныеОперации = ОтгрузкаТоваровКлиентСервер.ДоступныеОперации(Объект.ТоварнаяГруппа);
...
Процедура ОчиститьЗависимыеРеквизиты()
	...
	Если МаркировкаТоваровКлиентСервер.ДоступноПолеСертификация(Объект.ТоварнаяГруппа, Объект.Операция) Тогда
		Для Каждого СтрокаТовары Из Объект.Товары Цикл
...
&НаКлиенте
Процедура ПриИзмененииТоварнойГруппы(Элемент)
	
	ПриИзмененииТоварнаяГруппаНаСервере();
