...
Процедура ОбновитьСтатусЕГАИС()
	...
	СтатусПроверкиИПодбораАктРасхождений = РегистрыСведений.СтатусыПроверкиИПодбораИС.ТекущийСтатус(Объект.Ссылка, 1);
	МенеджерОбъекта = Документы.ТТНИсходящаяЕГАИС;
...
Процедура ОбновитьИнформациюОткрытияФормыСканирования()
	...
	Если СтатусПроверкиИПодбораАктРасхождений = Перечисления.СтатусыПроверкиИПодбораИС.Выполняется Тогда
		Элементы.ОтсканироватьИПроверитьАлкогольнуюПродукциюВозврат.Видимость = Истина;
		ТекстНадписи = НСтр("ru = 'Продолжить проверку расхождений по акцизным маркам'");
	ИначеЕсли СтатусПроверкиИПодбораАктРасхождений = Перечисления.СтатусыПроверкиИПодбораИС.Завершено Тогда
		Элементы.ОтсканироватьИПроверитьАлкогольнуюПродукциюВозврат.Видимость = Истина;
		ТекстНадписи = НСтр("ru = 'Результат проверки расхождений по акцизным маркам'");
	ИначеЕсли (СтатусЕГАИС = Перечисления.СтатусыОбработкиТТНИсходящейЕГАИС.АктРасхожденийПринят) Тогда
		Элементы.ОтсканироватьИПроверитьАлкогольнуюПродукциюВозврат.Видимость = Истина;
		ТекстНадписи = НСтр("ru = 'Проверить акт расхождений по акцизным маркам'");
	КонецЕсли;
...
Процедура СформироватьТекстДокументаЕГАИС()
	...
	Иначе
		ТекстДокументаЕГАИС = Данные.ТекстНадписи;
	КонецЕсли;
