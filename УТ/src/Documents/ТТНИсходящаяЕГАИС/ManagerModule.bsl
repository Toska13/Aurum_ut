...
//  См. РегистрыСведений.СтатусыДокументовЕГАИС.ВозвращаемоеЗначениеДальнейшиеДействияСтатус - статус после получения данных.
Функция СтатусПослеПолученияДанных(ДокументСсылка, Операция, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли Операция = Перечисления.ВидыДокументовЕГАИС.УведомлениеОРегистрацииДвиженияТТН Тогда
		...
		// получение информации от УТМ в неправильном порядке
...
Функция ТТНИсходящаяЕГАИСXML(ДокументСсылка)
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		Иначе
			РасчетноеКоличество = Окр(СтрокаТЧ.Количество / КоличествоВПотребительскойУпаковке, 0);
			Если РасчетноеКоличество = 0 Тогда
				ОбменДаннымиЕГАИСКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр(
					"ru = 'Для номенклатуры в строке %1 некорректно заполнено количество в потребительских упаковках.'"),
					СтрокаТЧ.НомерСтроки));
				НоваяСтрока.Количество = СтрокаТЧ.Количество;
				НоваяСтрока.Цена       = СтрокаТЧ.Цена;
			Иначе
				НоваяСтрока.Количество = РасчетноеКоличество;
...
//  ДокументСсылка - ДокументСсылка.ТТНИсходящаяЕГАИС - Документ, для которого выполняется заполнение
...
Процедура ЗаполнитьПродукциюДокумента(ТаблицаМаркируемойПродукции, ДокументСсылка, Сценарий) Экспорт
	...
	Если Сценарий = 0 Тогда // сценарий: отгрузка алкогольной продукции
		...
		|ГДЕ
		|	ТТНИсходящаяЕГАИСТовары.Ссылка = &ДокументСсылка
		|	И ТТНИсходящаяЕГАИСТовары.АлкогольнаяПродукция.ВидПродукции.Маркируемый = Истина
		|СГРУППИРОВАТЬ ПО
		|	ТТНИсходящаяЕГАИСТовары.АлкогольнаяПродукция,
	...
	ИначеЕсли Сценарий = 1 Тогда // сценарий: проверка акта расхождений (недопоставленный товар)
		
		ЗаполнитьПродукциюДокумента(ТаблицаМаркируемойПродукции, ДокументСсылка, 0);
		ТаблицаМаркируемойПродукции.Колонки.Добавить("Удалить", Новый ОписаниеТипов("Булево"));
		ТаблицаМаркируемойПродукции.ЗаполнитьЗначения(Истина, "Удалить");
		ТаблицаМаркируемойПродукции.ЗаполнитьЗначения(0, "Количество");
		КлючПоиска = КлючПоискаСтрокиПродукции();
		
		ДанныеПроверяемогоДокумента = ШтрихкодыУпаковок(ДокументСсылка, Ложь);
		МаркиОставить = НеподтвержденныеАкцизныеМарки(ДокументСсылка);
		...
		Для Каждого СтрокаТЧ Из ДанныеПроверяемогоДокумента.МаркированныеТовары Цикл
			Если МаркиОставить.Получить(СтрокаТЧ.Штрихкод) <> Неопределено Тогда
				Если СтрокаТЧ.СтрокаДерева.Строки.Количество() = 0 Тогда
					ЗаполнитьЗначенияСвойств(КлючПоиска, СтрокаТЧ);
					СтрокаТовара = ТаблицаМаркируемойПродукции.НайтиСтроки(КлючПоиска);
					Если СтрокаТовара.Количество() > 0 Тогда
						СтрокаТовара = СтрокаТовара[0];
						СтрокаТовара.Количество = СтрокаТовара.Количество + СтрокаТЧ.Количество;
						СтрокаТовара.Удалить = Ложь;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		
		ТаблицаМаркируемойПродукции = ТаблицаМаркируемойПродукции.Скопировать(Новый Структура("Удалить", Ложь));
		ТаблицаМаркируемойПродукции.Колонки.Удалить("Удалить");
		
	КонецЕсли;
	
КонецПроцедуры

// Подготавливает штрихкоды упаковок для акта о расхождениях
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка.ТТНИсходящаяЕГАИС - Документ ссылка
// 
// Возвращаемое значение:
//   См. ШтрихкодированиеЕГАИС.ВложенныеШтрихкодыУпаковокПоДокументу
Функция ШтрихкодыУпаковокАктаРасхождений(ДокументСсылка) Экспорт
	...
	Возврат ДанныеПроверяемогоДокумента;
	
КонецФункции

Функция КлючПоискаСтрокиПродукции()
...
//   * Для отражения результатов проверки при отгрузке продукции, изменяет документ
//   * Для отражения результатов проверки акта расхождений, не изменяет документ
//
// Параметры:
...
Процедура ОтразитьРезультатыПроверкиИПодбора(ПараметрыОкончанияПроверки) Экспорт
	...
	Если ПараметрыОкончанияПроверки.Сценарий = 0 Тогда
		
		АкцизныеМаркиЕГАИС.ОтразитьРезультатыПроверкиИПодбора(ЭтотОбъект(), ПараметрыОкончанияПроверки);
		
	ИначеЕсли ПараметрыОкончанияПроверки.Сценарий = 1 Тогда
		
		РегистрыСведений.СтатусыПроверкиИПодбораИС.ОтразитьЗавершение(
			ПараметрыОкончанияПроверки.ПроверяемыйДокумент,
			ПараметрыОкончанияПроверки.ДанныеПроверкиИПодбора,
			ПараметрыОкончанияПроверки.Сценарий);
		
	КонецЕсли;
	
КонецПроцедуры

Процедура ОбработатьСтрокиТабличнойЧасти(ДокументОбъект, ПараметрыЗаполнения) Экспорт
	...
	Для Каждого СтрокаТЧ Из ПараметрыЗаполнения.ИзмененныеСтроки Цикл
		Если СтрокаТЧ.Количество <> СтрокаТЧ.КоличествоДо Тогда
			ИнтеграцияИСПереопределяемый.ОбработатьСтрокуТабличнойЧасти(СтрокаТЧ, СтруктураДействий, ДокументОбъект);
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ДанныеШапки.ЕстьРасхождения 
	|		И СтатусыДокументовЕГАИС.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиТТНИсходящейЕГАИС.ПодтвержденСРасхождениями) КАК ЕстьРасхождения,
	...
	Запрос.УстановитьПараметр("ДатаРегистрацииДвижений", ?(ЗначениеЗаполнено(Реквизиты.ДатаРегистрацииДвижений),Реквизиты.ДатаРегистрацииДвижений, Реквизиты.Период));
...
Функция ТекстЗапросаТаблицаДвиженияСерийТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса = "";
	ИнтеграцияЕГАИСПереопределяемый.ПриЗаполненииТекстаЗапросаДвижениеСерийТоваров(ТекстЗапроса, ЭтотОбъект(Истина).Имя);
...
Функция ИменаРеквизитовДляЗаполненияПараметровУказанияСерий() Экспорт
	...
	Возврат ИнтеграцияИС.ИменаРеквизитовДляЗаполненияПараметровУказанияСерий(ЭтотОбъект(Истина));
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	Возврат ИнтеграцияИС.ПараметрыУказанияСерий(ЭтотОбъект(Истина), Объект);
...
Функция ТекстЗапросаЗаполненияСтатусовУказанияСерий(ПараметрыУказанияСерий) Экспорт
	...
	Возврат ИнтеграцияИС.ТекстЗапросаЗаполненияСтатусовУказанияСерий(ЭтотОбъект(Истина), ПараметрыУказанияСерий);
...
Функция ЭтотОбъект(ЭтоМетаданные = Ложь)
	Если ЭтоМетаданные Тогда
		Возврат Метаданные.Документы.ТТНИсходящаяЕГАИС;
	Иначе
		Возврат Документы.ТТНИсходящаяЕГАИС;
	КонецЕсли;
КонецФункции
...
Процедура ПостроитьBoxInfo(СтрокаДереваРодитель, ИспользованныеУзлыДереваУпаковок, ДеревоКодов)
	...
	Если КоробкиБезИерархии.Количество() > 0 Тогда
		Штрихкод = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ДеревоКодов, "bl");
		Для Каждого СтрокаДерева Из КоробкиБезИерархии Цикл
			Штрихкод.boxnum.Добавить(СтрокаДерева.Штрихкод);
		КонецЦикла;
		ДеревоКодов.bl.Добавить(Штрихкод);
	КонецЕсли;
	...
	Если КоробкиСИерархией.Количество() > 0 Тогда
		Для Каждого СтрокаДерева Из КоробкиСИерархией Цикл
			Штрихкод = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ДеревоКодов, "bl");
			Штрихкод.boxnum.Добавить(СтрокаДерева.Штрихкод);
			ПостроитьBoxInfo(СтрокаДерева, ИспользованныеУзлыДереваУпаковок, Штрихкод);
			ДеревоКодов.bl.Добавить(Штрихкод);
		КонецЦикла;
	КонецЕсли;
