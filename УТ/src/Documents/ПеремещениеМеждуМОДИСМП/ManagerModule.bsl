...
// 
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.СтатусыОбработкиПеремещенийМеждуМОДИСМП - Статусы данные переданы.
Функция СтатусыДанныеПереданы() Экспорт
	...
	Статусы.Добавить(Перечисления.СтатусыОбработкиПеремещенийМеждуМОДИСМП.РазрешениеПолучено);
	Статусы.Добавить(Перечисления.СтатусыОбработкиПеремещенийМеждуМОДИСМП.ПеремещениеОформлено);
	Статусы.Добавить(Перечисления.СтатусыОбработкиПеремещенийМеждуМОДИСМП.РазрешениеОтменено);
...
Функция ДальнейшееДействиеПоУмолчанию(СтруктураПараметров = Неопределено) Экспорт
	...
	Если СтруктураПараметров = Неопределено Тогда
		ТоварнаяГруппа = ТоварнаяГруппаПоУмолчанию();
	ИначеЕсли СтруктураПараметров.Свойство("ОбъектРасчета") И ЗначениеЗаполнено(СтруктураПараметров.ОбъектРасчета) Тогда
		ТоварнаяГруппа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураПараметров.ОбъектРасчета, "ТоварнаяГруппа");
	ИначеЕсли СтруктураПараметров.Свойство("ТоварнаяГруппа") И ЗначениеЗаполнено(СтруктураПараметров.ТоварнаяГруппа) Тогда
		ТоварнаяГруппа = СтруктураПараметров.ТоварнаяГруппа;
	Иначе
		ТоварнаяГруппа = ТоварнаяГруппаПоУмолчанию();
	КонецЕсли;
	
	Если Документы.РазрешениеНаОтгрузкуИСМП.ТоварнаяГруппаПоддерживаетРазрешениеНаОтгрузку(ТоварнаяГруппа) Тогда
		Возврат Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ЗапроситеРазрешениеНаОтгрузкуПродукции;
	Иначе
		Возврат Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ОформитеПеремещение;
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ОбновлениеИнформационнойБазыГосИС.ДобавитьОбработчикОбновленияПереходаНаКонструкторНастроек(
		...
		Новый УникальныйИдентификатор("2c82dcf9-d9d8-4fc2-bcb4-a701e22b7888"));
...
// Обработка данных перехода на конструктор настроек видов продуции
// 
// Параметры:
//  Параметры - см. ОбновлениеИнформационнойБазы.ОсновныеПараметрыОтметкиКОбработке
//
...
Процедура ЗаполнитьПродукциюДокумента(ТаблицаМаркируемойПродукции, ДокументСсылка, Сценарий) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПеремещениеМеждуМОДИСМПТовары.Номенклатура,
	|	ПеремещениеМеждуМОДИСМПТовары.Характеристика,
	|	ПеремещениеМеждуМОДИСМПТовары.Серия,
	|	ПеремещениеМеждуМОДИСМПТовары.Количество
	|ИЗ
	|	Документ.ПеремещениеМеждуМОДИСМП.Товары КАК ПеремещениеМеждуМОДИСМПТовары
	|ГДЕ
	|	ПеремещениеМеждуМОДИСМПТовары.Ссылка = &ДокументСсылка";
...
Процедура ЗаполнитьДанныеУПД_5_02_ИнформацияПродавца(Основание, Настройки, ДеревоДанных, Описание, Отказ) Экспорт
	...
	ЭтоИндивидуальныйПредприниматель = ОбщегоНазначенияИСВызовСервера.ПартнерЭтоИндивидуальныйПредприниматель(ВыборкаШапки.Организация);
...
Функция ПолучитьДанныеДляЭД(ДокументСсылка) Экспорт
	...
	Запрос.Текст =
		...
		|	ПеремещениеМеждуМОДИСМП.ТоварнаяГруппа                                  КАК ТоварнаяГруппа,
...
Функция ТоварнаяГруппаПоУмолчанию(ВозвращатьВсеДоступные = Ложь) Экспорт
	...
	Для Каждого ВидПродукции Из ВидПродукцииПиво Цикл
		
		ТоварнаяГруппа = КонструкторИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппаПоВидуПродукции(ВидПродукции);
		
		Если ТоварныеГруппы.Найти(ТоварнаяГруппа) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
	...
	Если ВозвращатьВсеДоступные Тогда
		Возврат ТоварныеГруппы;
	ИначеЕсли ТоварныеГруппы.Количество() Тогда
...
Процедура ОбработатьДанныеШтрихкодаПотребительскойУпаковки(Форма, ДанныеШтрихкода, ПараметрыЗаполнения)
	...
	ЭтоШтрихкодНоменклатуры = ТипЗнч(ДанныеШтрихкода) = Тип("Структура")
		И ДанныеШтрихкода.Свойство("ЭтоШтрихкодНоменклатуры")
	...
	Если Не ОбработкаБезМаркировки И Не ЭтоШтрихкодНоменклатуры Тогда
...
Функция ЗапросРазрешенияНаОтгрузкуПродукции(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	|	ВЫБОР
	|		КОГДА Шапка.ТоварнаяГруппа = ЗНАЧЕНИЕ(Справочник.ТоварныеГруппыГИСМТ.ПустаяСсылка)
	|			ТОГДА &ТоварнаяГруппаПоУмолчанию
	|		ИНАЧЕ Шапка.ТоварнаяГруппа
	...
	Запрос.УстановитьПараметр("ТоварнаяГруппаПоУмолчанию", ТоварнаяГруппаПоУмолчанию());
	...
	ТоварнаяГруппа    = Шапка.ТоварнаяГруппа;
	...
	Если ВидыПродукции.Количество() = 0 Тогда
		ШаблонСообщения = НСтр("ru = 'В настройках интеграции с ГИС МТ выключены виды продукции, поддерживающие перемещение между МОД.'");
...
Функция ОтменаРазрешенияНаОтгрузкуПродукции(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	СообщениеJSON.ТоварнаяГруппа = КонструкторИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппаПоВидуПродукции(ВидыПродукции[0]);
