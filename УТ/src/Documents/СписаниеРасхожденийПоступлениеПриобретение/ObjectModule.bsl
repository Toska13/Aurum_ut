...
//  ТолькоРаботы - Булево - Отбор по номенклатуре типа работа.
// 
// Возвращаемое значение:
//  МенеджерВременныхТаблиц - менеджер временных таблиц с данными документа.
//
Функция ВременныеТаблицыДанныхДокумента(ВидРасхождений, ДляТоваровУПартнеров, ТолькоИмпортныеТовары, ТолькоРаботы = Неопределено)
	...
	Если Не ДляТоваровУПартнеров Тогда
		ТипЗапасов = Перечисления.ТипыЗапасов.Товар;
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	МенеджерВременныхТаблиц		= ВременныеТаблицыДанныхДокумента(ВидРасхожденийНедопоставка, Ложь, Истина);
	...
	Если Не Проведен
		...
		ЗаполнитьВидыЗапасовНеИмпортныхТоваров();
		...
		ЗаполнитьАналитикиСУчетомСерийИНомераГТД();
...
Процедура ЗаполнитьАналитикиУчетаНоменклатурыВТабличныхЧастяхТоваров(ТаблицыДокумента = Неопределено)
	...
	МестаУчетаСклад = РегистрыСведений.АналитикаУчетаНоменклатуры.МестаУчета(
						ОсновнаяХозОперация,
						Склад,
						Подразделение,
						Партнер,
...
Процедура ЗаполнитьВидЗапасовПолучателя()
	...
	ВидРасхождений		= Перечисления.ВидыРасхожденийПоступленийПриобретений.Недопоставка;
	ОтборСтрок			= Новый Структура("ВидРасхождений", ВидРасхождений);
	ВыгружаемыеКолонки	= "АналитикаУчетаНоменклатуры, ВидЗапасов";
	
	ТаблицаТовары = Расхождения.Выгрузить(ОтборСтрок, ВыгружаемыеКолонки);
	ТаблицаТовары.Свернуть(ВыгружаемыеКолонки);
	
	ОтборСтрок = Новый Структура("АналитикаУчетаНоменклатуры");
	
	Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
		ЗаполнитьЗначенияСвойств(ОтборСтрок, СтрокаТовары);
		
		СтрокиОтбора = ВидыЗапасов.НайтиСтроки(ОтборСтрок);
		
		Для Каждого СтрокаЗапасов Из СтрокиОтбора Цикл
			СтрокаЗапасов.ВидРасхождений		= ВидРасхождений;
			СтрокаЗапасов.ВидЗапасовПолучателя	= СтрокаТовары.ВидЗапасов;
		КонецЦикла;
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗаполнитьАналитикиСУчетомСерийИНомераГТД()
	...
	|	&ВидРасхождений							КАК ВидРасхождений,
	|	ВЫБОР
	|		КОГДА &ХозяйственнаяОперацияРаспоряжения В (
	...
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ									КАК ЭтоОперацияТоварыВПути
	|ПОМЕСТИТЬ ВидыЗапасовДляЗапроса
	|ИЗ
	...
	|		КОГДА ЕСТЬNULL(КлючиАналитики.Номенклатура.ВидНоменклатуры.ПолитикаУчетаСерий.УчетТоваровВПутиОтПоставщикаПоСериям, ЛОЖЬ)
	|				ИЛИ ЕСТЬNULL(КлючиАналитики.Номенклатура.ВидНоменклатуры.ПолитикаУчетаСерий.УчетСерийВНеотфактурованныхПоставкахТоваров, ЛОЖЬ)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ВидыЗапасов.АналитикаУчетаНоменклатуры
	|	КОНЕЦ									КАК АналитикаОстатка,
	|	ВидыЗапасов.АналитикаУчетаНоменклатуры	КАК АналитикаУчетаНоменклатуры,
	|	ВидыЗапасов.ВидЗапасов					КАК ВидЗапасов,
	|	ВидыЗапасов.НомерГТД					КАК НомерГТД,
	|	ВидыЗапасов.Количество					КАК Количество,
	|	ВидыЗапасов.КоличествоПоРНПТ			КАК КоличествоПоРНПТ,
	|	ВидыЗапасов.ВидЗапасовПолучателя		КАК ВидЗапасовПолучателя,
	|	ВидыЗапасов.ВидРасхождений				КАК ВидРасхождений,
	...
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ									КАК УчетСериийУПартнеров,
	|	ВЫБОР
	|		КОГДА НЕ ВидыЗапасов.ЭтоОперацияТоварыВПути
	|			ТОГДА ЛОЖЬ
	|		КОГДА ЕСТЬNULL(КлючиАналитики.Номенклатура.ВестиУчетПоГТД, ЛОЖЬ)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|				И ЕСТЬNULL(КлючиАналитики.Номенклатура.ВестиУчетПоГТД, ЛОЖЬ)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ									КАК ЗаполнитьГТД,
	|	ВидыЗапасов.ЭтоОперацияТоварыВПути		КАК ЭтоОперацияТоварыВПути
	|ПОМЕСТИТЬ ВидыЗапасовДляЗапросаРасширенная
	|ИЗ
	|	ВидыЗапасовДляЗапроса КАК ВидыЗапасов
	...
	|	ВидыЗапасов.МестоХранения				КАК МестоХранения,
	|	ВидыЗапасов.СтатьяКалькуляции			КАК СтатьяКалькуляции,
	|	ВидыЗапасов.УчетСериийУПартнеров		КАК УчетСериийУПартнеров,
	|	ВидыЗапасов.ВестиУчетПоГТД				КАК ВестиУчетПоГТД,
	|	ВидыЗапасов.ЭтоОперацияТоварыВПути		КАК ЭтоОперацияТоварыВПути
	|ПОМЕСТИТЬ ВтВидыЗапасов
	|ИЗ
	|	ВидыЗапасовДляЗапросаРасширенная КАК ВидыЗапасов
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	АналитикаУчетаНоменклатуры,
	|	ВидЗапасов,
	...
	|	ВидыЗапасов.ВестиУчетПоГТД			КАК ВестиУчетПоГТД,
	|	ВидыЗапасов.ЭтоОперацияТоварыВПути	КАК ЭтоОперацияТоварыВПути
	|ПОМЕСТИТЬ ВТОтборАналитикССериями
	|ИЗ
	...
	|			И Аналитика.МестоХранения = ВидыЗапасов.МестоХранения
	|			И Аналитика.Назначение = ВидыЗапасов.Назначение
	|			И Аналитика.СтатьяКалькуляции = ВидыЗапасов.СтатьяКалькуляции
	|ГДЕ
	|	ВидыЗапасов.УчетСериийУПартнеров
	|	И ВидыЗапасов.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Аналитика,
	|	ВидЗапасов,
	...
	|						(ВЫБРАТЬ
	|							ТаблицаОтбора.Аналитика,
	|							ТаблицаОтбора.ВидЗапасов
	|						ИЗ
	|							ВТОтборАналитикССериями КАК ТаблицаОтбора
	|						ГДЕ
	|							ТаблицаОтбора.ВестиУчетПоГТД
	|							И ТаблицаОтбора.ЭтоОперацияТоварыВПути)
	...
	|							ВТОтборАналитикССериями КАК ТаблицаОтбора
	|						ГДЕ
	|							НЕ ТаблицаОтбора.ВестиУчетПоГТД
	|							И ТаблицаОтбора.ЭтоОперацияТоварыВПути)
	|				И Организация = &Организация
	|		) КАК Остатки
	|	ГДЕ
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТОтборАналитикССериями КАК ТаблицаОтбора
	|			ПО Движения.АналитикаУчетаНоменклатуры = ТаблицаОтбора.Аналитика
	|				И Движения.ВидЗапасов = ТаблицаОтбора.ВидЗапасов
	|				И НЕ ТаблицаОтбора.ЭтоОперацияТоварыВПути
	|	ГДЕ
	|		Движения.Период <= &ДатаДокумента
	|		И Движения.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		И (Движения.Регистратор, ИСТИНА) В
	|				(ВЫБРАТЬ
	...
	|		И Движения.НомерГТД = ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		РегистрНакопления.ТоварыОрганизаций КАК Движения
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтВидыЗапасов КАК ВидыЗапасов
	|			ПО Движения.АналитикаУчетаНоменклатуры = ВидыЗапасов.АналитикаУчетаНоменклатуры
	|				И Движения.ВидЗапасов = ВидыЗапасов.ВидЗапасов
	|	ГДЕ
	|		Движения.Регистратор = &Ссылка
	|		И ВидыЗапасов.УчетСериийУПартнеров
	|		И ВидыЗапасов.ЭтоОперацияТоварыВПути
	|		И Движения.НомерГТД = ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
	|		И ВидыЗапасов.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|	
	|	) КАК ТаблицаОстатков
	...
	|	СУММА(ТаблицаОстатков.Количество)			КАК Количество,
	|	СУММА(ТаблицаОстатков.КоличествоПоРНПТ)		КАК КоличествоПоРНПТ
	|ПОМЕСТИТЬ ВТОстатковСГТД
	|ИЗ
	|	(ВЫБРАТЬ
	|		Остатки.АналитикаУчетаНоменклатуры	КАК АналитикаУчетаНоменклатуры,
	|		Остатки.ВидЗапасов					КАК ВидЗапасов,
	...
	|				(АналитикаУчетаНоменклатуры, ВидЗапасов) В
	|						(ВЫБРАТЬ
	|							ТаблицаОтбора.АналитикаУчетаНоменклатуры,
	|							ТаблицаОтбора.ВидЗапасов
	|						ИЗ
	|							ВтВидыЗапасов КАК ТаблицаОтбора
	|						ГДЕ
	|							ТаблицаОтбора.ВестиУчетПоГТД
	|							И ТаблицаОтбора.НомерГТД = ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка))
	|				И Организация = &Организация
	|				И НомерГТД <> ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
	|		) КАК Остатки
	|	ГДЕ
	|		Остатки.КоличествоОстаток > 0
	|		ИЛИ Остатки.КоличествоПоРНПТОстаток > 0
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|				ГДЕ
	|					ТоварыКПоступлению.ДокументПоступления = &Распоряжение)
	|		И (Движения.АналитикаУчетаНоменклатуры, Движения.ВидЗапасов) В
	|				(ВЫБРАТЬ
	|					ТаблицаОтбора.Аналитика,
	|					ТаблицаОтбора.ВидЗапасов
	|				ИЗ
	...
	|		Движения.НомерГТД					КАК НомерГТД,
	|		-Движения.Количество				КАК Количество,
	|		-Движения.КоличествоПоРНПТ			КАК КоличествоПоРНПТ
	|	ИЗ
	|		РегистрНакопления.ТоварыОрганизаций КАК Движения
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтВидыЗапасов КАК ВидыЗапасов
	|			ПО Движения.АналитикаУчетаНоменклатуры = ВидыЗапасов.АналитикаУчетаНоменклатуры
	|				И Движения.ВидЗапасов = ВидыЗапасов.ВидЗапасов
	|				И Движения.НомерГТД = ВидыЗапасов.НомерГТД
	|				И ВидыЗапасов.НомерГТД <> ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
	|	ГДЕ
	|		Движения.Регистратор = &Ссылка
	...
	|ИМЕЮЩИЕ
	|	СУММА(ТаблицаОстатков.Количество) > 0
	|	ИЛИ СУММА(ТаблицаОстатков.КоличествоПоРНПТ) > 0
	|
	|;
	|//////////////////////////////////////////////////////////////////////////////// 6
	|ВЫБРАТЬ
	|	ВидыЗапасов.АналитикаУчетаНоменклатуры	КАК АналитикаОстатка,
	...
	|	ВидыЗапасов.ВидЗапасов					КАК ВидЗапасов,
	|	ВЫБОР
	|		КОГДА ВидыЗапасовСГТД.АналитикаУчетаНоменклатуры ЕСТЬ NULL
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
	|		ИНАЧЕ ВидыЗапасовСГТД.НомерГТД
	|	КОНЕЦ									КАК НомерГТД,
	|	ВЫБОР
	|		КОГДА ВидыЗапасовСГТД.АналитикаУчетаНоменклатуры ЕСТЬ NULL
	|			ТОГДА ВидыЗапасов.Количество
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ВидыЗапасовСГТД.Количество >= ВидыЗапасов.Количество
	|					ТОГДА ВидыЗапасов.Количество
	|				ИНАЧЕ ВидыЗапасовСГТД.Количество
	|			КОНЕЦ
	|	КОНЕЦ									КАК КоличествоУчитываяГТД,
	|	ВЫБОР
	|		КОГДА ВидыЗапасовСГТД.АналитикаУчетаНоменклатуры ЕСТЬ NULL
	|			ТОГДА ВидыЗапасов.Количество
	|		ИНАЧЕ ВидыЗапасовСГТД.Количество
	|	КОНЕЦ									КАК КоличествоОстатокУчитываяГТД,
	|	ВЫБОР
	|		КОГДА ВидыЗапасовСГТД.АналитикаУчетаНоменклатуры ЕСТЬ NULL
	|			ТОГДА 0
	|		ИНАЧЕ ВидыЗапасовСГТД.КоличествоПоРНПТ
	|	КОНЕЦ									КАК КоличествоПоРНПТ
	|ИЗ
	|	ВТОстатковССериями КАК ВидыЗапасов
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК КлючиАналитики
	|		ПО ВидыЗапасов.АналитикаУчетаНоменклатуры = КлючиАналитики.Ссылка
	|		
	...
	|			И Аналитика.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|			И Аналитика.МестоХранения = КлючиАналитики.МестоХранения
	|			И Аналитика.Назначение = КлючиАналитики.Назначение
	|			И Аналитика.СтатьяКалькуляции = КлючиАналитики.СтатьяКалькуляции
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОстатковСГТД КАК ВидыЗапасовСГТД
	|		ПО ВидыЗапасов.АналитикаУчетаНоменклатуры = ВидыЗапасовСГТД.АналитикаУчетаНоменклатуры
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ВидыЗапасов.ВидЗапасов					КАК ВидЗапасов,
	|	ВидыЗапасов.НомерГТД					КАК НомерГТД,
	|	ВидыЗапасов.Количество					КАК КоличествоУчитываяГТД,
	|	ВидыЗапасов.Количество					КАК КоличествоОстатокУчитываяГТД,
	|	ВидыЗапасов.КоличествоПоРНПТ			КАК КоличествоПоРНПТ
	|ИЗ
	|	ВТОстатковСГТД КАК ВидыЗапасов
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК КлючиАналитики
	|		ПО ВидыЗапасов.АналитикаУчетаНоменклатуры = КлючиАналитики.Ссылка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОстатковССериями КАК ВидыЗапасовССериями
	|		ПО ВидыЗапасов.АналитикаУчетаНоменклатуры = ВидыЗапасовССериями.АналитикаУчетаНоменклатуры
	|ГДЕ
	|	ВидыЗапасовССериями.АналитикаУчетаНоменклатуры ЕСТЬ NULL";
	...
	Запрос.УстановитьПараметр("ВидРасхождений",	Перечисления.ВидыРасхожденийПоступленийПриобретений.Недопоставка);
	Запрос.УстановитьПараметр("ХозяйственнаяОперацияРаспоряжения",	РеквизитыОснования.ХозяйственнаяОперация);
	
	РезультатЗапроса = Запрос.ВыполнитьПакетСПромежуточнымиДанными();
	
	ТаблицаВидыЗапасов = РезультатЗапроса[1].Выгрузить(); // ТаблицаЗначений - 
	ТаблицаВидыЗапасов.Индексы.Добавить("ЗаполнитьГТД");
	ТаблицаВидыЗапасов.Индексы.Добавить("АналитикаОстатка");
	
	ТаблицаОстатков = РезультатЗапроса[РезультатЗапроса.ВГраница()].Выгрузить();
	ТаблицаОстатков.Индексы.Добавить("АналитикаУчетаНоменклатуры, ВидЗапасов, НомерГТД");
	
	Ключ			= "АналитикаУчетаНоменклатуры";
	Условие			= "ПО [Количество]";
	ЗаполняемыеПоля	= "АналитикаОстатка,НомерГТД";
	
	НакладныеСервер.РаспределитьКоличествоИЗаполнить(
		ТаблицаОстатков,
		ТаблицаВидыЗапасов,
		"КоличествоУчитываяГТД",
		Ключ,
		Условие,
		Ложь,
		ЗаполняемыеПоля);
	
	ОтборСтрок		= Новый Структура("ЗаполнитьГТД", Истина);
	СтрокиОтбора	= ТаблицаВидыЗапасов.НайтиСтроки(ОтборСтрок);
	
	ОтборПоГТД = Новый Структура("АналитикаУчетаНоменклатуры, ВидЗапасов, НомерГТД");
	
	Для Каждого СтрокаЗапасов Из СтрокиОтбора Цикл
		Если Не ЗначениеЗаполнено(СтрокаЗапасов.НомерГТД) Тогда
			Продолжить;
		КонецЕсли;
		
		Если СтрокаЗапасов.Количество > СтрокаЗапасов.КоличествоУчитываяГТД Тогда
			КоличествоИзлишек = СтрокаЗапасов.Количество - СтрокаЗапасов.КоличествоУчитываяГТД;
			
			НоваяСтрока = ТаблицаВидыЗапасов.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаЗапасов);
			
			НоваяСтрока.Количество	= КоличествоИзлишек;
			НоваяСтрока.НомерГТД	= Неопределено;
		КонецЕсли;
		
		СтрокаЗапасов.Количество = СтрокаЗапасов.КоличествоУчитываяГТД;
		
		ЗаполнитьЗначенияСвойств(ОтборПоГТД, СтрокаЗапасов);
		
		СтрокиОстатков = ТаблицаОстатков.НайтиСтроки(ОтборПоГТД);
		
		Для Каждого СтрокаСГТД Из СтрокиОстатков Цикл
			СтрокаЗапасов.КоличествоПоРНПТ = СтрокаЗапасов.Количество * СтрокаСГТД.КоличествоПоРНПТ
												/ СтрокаСГТД.КоличествоОстатокУчитываяГТД;
			
			Прервать;
		КонецЦикла;
	КонецЦикла;
	
	ОтборСтрок		= Новый Структура("АналитикаОстатка");
	СтрокиОтбора	= ТаблицаВидыЗапасов.НайтиСтроки(ОтборСтрок);
	
	Для Каждого СтрокаЗапасов Из СтрокиОтбора Цикл
		СтрокаЗапасов.АналитикаОстатка = СтрокаЗапасов.АналитикаУчетаНоменклатуры;
	КонецЦикла;
	
	ТаблицаВидыЗапасов.Колонки.Удалить("АналитикаУчетаНоменклатуры");
	ТаблицаВидыЗапасов.Колонки.АналитикаОстатка.Имя = "АналитикаУчетаНоменклатуры";
	
	ВидыЗапасов.Загрузить(ТаблицаВидыЗапасов);
	
КонецПроцедуры

Процедура ЗаполнитьВидыЗапасовНеИмпортныхТоваров()
	
	ВидРасхождений	= Перечисления.ВидыРасхожденийПоступленийПриобретений.Недопоставка;
	ОтборСтрок		= Новый Структура("ВидРасхождений", ВидРасхождений);
	
	ВыгружаемыеКолонки = "Номенклатура, АналитикаУчетаНоменклатурыТоварыУПартнеров, Количество, ВидРасхождений,
							|ВидЗапасов";
	
	ТаблицаТоваров = Расхождения.Выгрузить(ОтборСтрок, ВыгружаемыеКолонки);
	...
	|	ТаблицаТоваров.Количество					КАК Количество,
	|	ТаблицаТоваров.ВидРасхождений				КАК ВидРасхождений,
	|	ТаблицаТоваров.ВидЗапасов					КАК ВидЗапасовПолучателя
	|ИЗ
	|	ТаблицаТоваров КАК ТаблицаТоваров
	...
	|
	|ГДЕ
	|	НЕ СправочникНоменклатура.ВестиУчетПоГТД
	|	И ТаблицаТоваров.ВидРасхождений = ЗНАЧЕНИЕ(Перечисление.ВидыРасхожденийПоступленийПриобретений.Недопоставка)";
	...
	ВидыЗапасов.Загрузить(РезультатЗапроса.Выгрузить());
