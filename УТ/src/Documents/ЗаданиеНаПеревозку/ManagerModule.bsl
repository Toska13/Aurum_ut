...
Функция ТекстЗапросаПолученияСпискаНакладныхИзЗаданийНаПеревозку(ФормироватьВТРаспоряжения) Экспорт
	...
	Если ФормироватьВТРаспоряжения Тогда
		...
		|	ЗаданиеНаПеревозкуМаршрут.АдресЗначение КАК АдресДоставкиЗначение,
	...
	|ИЗ
	|	РаспоряженияМаршрут КАК Распоряжения
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтгрузкаТоваровКлиенту.Товары КАК ДокументТовары
	|		ПО Распоряжения.Распоряжение = ДокументТовары.ЗаказКлиента
	|			И Распоряжения.Склад = ДокументТовары.Склад
	|ГДЕ
	|	ДокументТовары.Ссылка.Проведен
	...
	|	РаспоряженияМаршрут.АдресДоставкиЗначение КАК АдресДоставкиЗначение,
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "КомплектДокументовНаПринтер";
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КомплектДокументовНаПринтер") Тогда
...
Процедура СформироватьКомплектПечатныхФорм(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати) Экспорт
	...
	СортироватьПечатныеФормыПоКонтрагенту(МассивНакладных, ОбъектыПечати);
	
КонецПроцедуры

Процедура СортироватьПечатныеФормыПоКонтрагенту(МассивНакладных, ОбъектыПечати)
	...
	Для Каждого ТекДокумент Из МассивНакладных Цикл
		...
		Попытка
			КонтрагентНакладной = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекДокумент, "Контрагент"); // СправочникСсылка.Контрагенты
	...
	Для Каждого Строка Из СкладИПолучательДляПереоформленияОрдеров Цикл
		...
		Если НаборЗаписейОчереди.Количество() = 0 Тогда
			...
			Исключение
				...
				ШаблонСообщения = НСтр("ru = 'Не удалось выполнить проведение документа: %1 по причине: %2'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонСообщения,
					Строка(ЗаданиеНаПеревозку),
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				
				ЗаписьЖурналаРегистрации(
					ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
					УровеньЖурналаРегистрации.Ошибка,
					ЗаданиеНаПеревозку.Метаданные(),
					ЗаданиеНаПеревозку,
					ТекстСообщения);
			...
			СтруктураПараметров.Вставить("Получатель", Строка.Получатель);
