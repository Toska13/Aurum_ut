	   ...
	   И (ЗначениеНовогоСтатуса = Перечисления.СтатусыЗаданийНаПеревозку.Отправлено
		...
		Если ДоставкаТоваров.ПерезаполнитьЗаданиеНаПеревозкуПоРасходнымОрдерам(ЭтотОбъект) Тогда // есть изменения после перезаполнения
			...
			Если Распоряжения.Количество() = 0 Тогда
				...
				ШаблонОшибки = НСтр("ru = 'Статус документа %1 не установлен, т.к. после перезаполнения по расходным ордерам в нем не остается распоряжений.'");
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25() Тогда
		...
		ТекстИсключения = НСтр("ru = 'Документ не поддерживается в доставке версии 2.5'");
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25() Тогда
		...
		// в версии доставки 2.5 запрет записи документ в целом, см. ПередЗаписью
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25() Тогда
		...
		ТекстСообщения = НСтр("ru = 'Изменение документа не поддерживается в доставке версии 2.5'");
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение
		...
		ДополнительныеСвойства.Вставить(
			"РаспоряженияДляЗаписиСостояния",
...
Процедура ПриЗаписи(Отказ)
	...
	ПроведенПередЗаписью = ДополнительныеСвойства.ПроведенПередЗаписью; // Булево
	СтатусПередЗаписью = ДополнительныеСвойства.СтатусПередЗаписью; // ПеречислениеСсылка.СтатусыЗаданийНаПеревозку
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если Операция = Перечисления.ВидыДоставки.СоСклада
		...
		Пока Выборка.Следующий() Цикл
			...
			ШаблонСообщения = НСтр("ru = 'Расходный ордер ""%1"" по заданию на перевозку не готов к отгрузке.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонСообщения, Строка(Выборка.Ссылка));			
	...
	ИначеЕсли Операция = Перечисления.ВидыДоставки.СоСклада
		...
		Документы.ЗаданиеНаПеревозку.СформироватьОчередьПереоформленияРасходныхОрдеров(
			Ссылка,
			Отказ,
...
Процедура КонтрольОформленныхОрдеровПоЗаданиюНаПеревозку(Ссылка, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		ШаблонСообщения = НСтр("ru = 'Не оформлены все расходные ордера по заданию на перевозку ""%1"" для распоряжения ""%2"" на  складе ""%3"". Оформите ордера и(или) перезаполните задание на перевозку по расходным ордерам.'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонСообщения,
			Строка(Ссылка),
			Строка(Выборка.ДокументОтгрузки),
