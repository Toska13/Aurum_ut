...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	Если Объект.ПометкаУдаления Тогда
		Оповестить("Запись_ЗаданиеНаПеревозку");
	КонецЕсли;
КонецПроцедуры
...
Процедура СтатусПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка)
		...
		Режим = Новый СписокЗначений; // СписокЗначений из КодВозвратаДиалога
...
Процедура ДекорацияОформитьЭПДНажатие(Элемент, СтандартнаяОбработка)
	...
	ОповещениеОЗакрытие = Новый ОписаниеОповещения(
...
Процедура РазнестиПоВремени(Команда)
	...
	Если РаботаСТабличнымиЧастямиКлиент.ВыбранаСтрокаДляВыполненияКоманды(Элементы.Маршрут)
		...
		ОткрытьФорму("Обработка.РабочееМестоМенеджераПоДоставке.Форма.ФормаВыбораРаспоряжений", ПараметрыФормы, ЭтотОбъект);
...
Процедура ОбъединитьПоВремени(Команда)
	...
	Если СтруктураВозврата.РазныеАдреса Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не удалось объединить выделенные строки: выберите строки с одинаковыми адресами.'"));
		Возврат;
...
Процедура ЗаполнитьПоРасходнымОрдерам(Команда)
	...
	Режим = Новый СписокЗначений; // Массив из КодВозвратаДиалога
	Режим.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Перезаполнить'"));
	Режим.Добавить(КодВозвратаДиалога.Нет, НСтр("ru = 'Не перезаполнять'"));
	Текст = НСтр("ru = 'Перезаполнить задание по связанным расходным ордерам на товары?';");
	ПоказатьВопрос(Новый ОписаниеОповещения("ПослеВопросаОПерезаполненииПоОрдерам", ЭтотОбъект), Текст, Режим);
...
Процедура ОформитьТранспортныеНакладные(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка)
		...
		СписокКнопок = Новый СписокЗначений; // Массив из КодВозвратаДиалога
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект,РеквизитыТС);
	ОбновитьИтоговыйВесОбъемЗаполненность(ЭтотОбъект);
	ОтобразитьЕдПользователяНаСервере(ЭтотОбъект);
...
Процедура ОбновитьСкладыПогрузки()
	...
	Элементы.РаспоряженияЗаполнитьПоРасходнымОрдерам.Видимость = 
		(ЕстьОрдерныеСклады
	...
	КонецЕсли;
	ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%СкладыКоличество%", Строка(Склады.Количество()));
...
Процедура ЗаполнитьСлужебныеРеквизитыМаршрута(ОбновляемыеСтроки = Неопределено)
	...
	|	Маршрут.АдресЗначение КАК АдресДоставкиЗначение,
...
Функция ПроверитьБлокировкуЗадания()
	...
	Иначе
		...
		Исключение
			...
			Текст = СтрЗаменить(Текст,"%Задание%", Строка(Объект.Ссылка));
			ОбщегоНазначения.СообщитьПользователю(Текст, Объект.Ссылка);
			Возврат Ложь;
...
Процедура ОформитьТранспортныеНакладныеЗавершение(РезультатПроверки = Истина)
	...
	Если ВыделенныеСтрокиИдентификаторы.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Объект.Маршрут");
		Возврат;
...
Процедура ОбновитьТекстЭПД()
	...
	Если Элементы.ГруппаСостояниеЭПД.Видимость Тогда 
		...
		Элементы.ДекорацияЭПД.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"ЭПД (%1)",
			Строка(КоличествоЭПД));
	КонецЕсли;
