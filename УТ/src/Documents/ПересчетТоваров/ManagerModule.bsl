...
Функция КонтрольКачестваПараметрыДействий(Объект) Экспорт
	...
	ПараметрыДействия.ПолноеИмяОбъекта = "Документ.ПересчетТоваров";
	ПараметрыДействия.ЧитаемыеПоля = "Номенклатура,КонтролируемаяПартия,КоличествоФакт,Количество";
...
Процедура ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества(ТекущаяСтрока, ДанныеДляОбработки, ПараметрыДействия, ЗаполнитьШтатно) Экспорт
	...
	// Штатное заполнение.
	Если ТекущаяСтрока.КоличествоФакт <= ТекущаяСтрока.Количество Тогда
		ЗаполнитьШтатно = Ложь;
		ТекущаяСтрока.ПодлежитРазблокировке = Ложь;
		ТекущаяСтрока.ДатаПодтвержденияКачества = '00010101';
	КонецЕсли;
...
Функция ТекстЗапросаТаблицаТоварыНаСкладах(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|	КОНЕЦ КАК Назначение,
	|	ПересчетТоваровТовары.КонтролируемаяПартия,
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|	И ЕСТЬNULL(ТабЧасть.Упаковка.ТипУпаковки,
	...
	ТекстТоварныеМестаВДокументе =
		"ВЫБРАТЬ
		|	ТабЧасть.Ссылка КАК Ссылка,
		|	МАКСИМУМ(ТабЧасть.Ссылка.Дата) КАК Дата,
		|	МАКСИМУМ(ТабЧасть.Ссылка.Склад) КАК Склад,
		|	ТабЧасть.Номенклатура КАК Номенклатура,
		|	ТабЧасть.КонтролируемаяПартия КАК КонтролируемаяПартия,
		|	МАКСИМУМ(ТабЧасть.ДатаПодтвержденияКачества) КАК ДатаПодтвержденияКачества,
		|	МАКСИМУМ(ТабЧасть.ПодлежитРазблокировке) КАК ПодлежитРазблокировке,
		|	ТабЧасть.Характеристика КАК Характеристика,
		|	ТабЧасть.Назначение КАК Назначение,
	...
	ТекстТоварныеМестаВНоменклатуре =
		...
		|	ТоварныеМестаОстатки.Характеристика КАК Характеристика,
		|	ТоварныеМестаОстатки.Назначение КАК Назначение,
		|	ТоварныеМестаОстатки.Серия КАК Серия,
		|	ТоварныеМестаОстатки.КонтролируемаяПартия КАК КонтролируемаяПартия,
	...
	ТекстЦелыеКомплекты =
		"ВЫБРАТЬ
		|	ТоварныеМестаВНоменклатуре.Ссылка КАК Ссылка,
		|	ТоварныеМестаВНоменклатуре.Номенклатура КАК Номенклатура,
		|	ТоварныеМестаВНоменклатуре.Характеристика КАК Характеристика,
		|	ТоварныеМестаВНоменклатуре.Назначение КАК Назначение,
		|	ТоварныеМестаВНоменклатуре.КонтролируемаяПартия КАК КонтролируемаяПартия,
		|	МАКСИМУМ(ТабЧасть.Склад) КАК Склад,
		|	МАКСИМУМ(ТабЧасть.Дата) КАК Дата,
		|	МАКСИМУМ(ТабЧасть.ДатаПодтвержденияКачества) КАК ДатаПодтвержденияКачества,
		|	МАКСИМУМ(ТабЧасть.ПодлежитРазблокировке) КАК ПодлежитРазблокировке,
		|	ТоварныеМестаВНоменклатуре.Серия КАК Серия,
		|	МИНИМУМ(ВЫБОР
		...
		|			И ТабЧасть.Характеристика = ТоварныеМестаВНоменклатуре.Характеристика
		|			И ТабЧасть.Назначение = ТоварныеМестаВНоменклатуре.Назначение
		|			И ТабЧасть.КонтролируемаяПартия = ТоварныеМестаВНоменклатуре.КонтролируемаяПартия
		|			И ТабЧасть.Серия = ТоварныеМестаВНоменклатуре.Серия
		|			И ТабЧасть.ТоварноеМесто = ТоварныеМестаВНоменклатуре.ТоварноеМесто
		|			И ТабЧасть.КоличествоОстаток > 0
		|		
		|СГРУППИРОВАТЬ ПО
		|	ТоварныеМестаВНоменклатуре.Ссылка,
		|	ТоварныеМестаВНоменклатуре.Номенклатура,
		|	ТоварныеМестаВНоменклатуре.Характеристика,
		|	ТоварныеМестаВНоменклатуре.Назначение,
		|	ТоварныеМестаВНоменклатуре.КонтролируемаяПартия,
		|	ТоварныеМестаВНоменклатуре.Серия
		|ИМЕЮЩИЕ
		|	МИНИМУМ(ВЫБОР
		|			КОГДА (ВЫРАЗИТЬ(ЕСТЬNULL(ТабЧасть.КоличествоОстаток, 0) / ТоварныеМестаВНоменклатуре.КоличествоМест КАК ЧИСЛО(12, 0)))
		|					- ЕСТЬNULL(ТабЧасть.КоличествоОстаток, 0) / ТоварныеМестаВНоменклатуре.КоличествоМест <= 0
		...
		|			КОГДА (ВЫРАЗИТЬ(ЕСТЬNULL(ТабЧасть.КоличествоОстаток, 0) / ТоварныеМестаВНоменклатуре.КоличествоМест КАК ЧИСЛО(12, 0)))
		|					- ЕСТЬNULL(ТабЧасть.КоличествоОстаток, 0) / ТоварныеМестаВНоменклатуре.КоличествоМест > 0
		|				ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(ТабЧасть.КоличествоОстаток, 0) / ТоварныеМестаВНоменклатуре.КоличествоМест КАК ЧИСЛО(12, 0))- 1
		|			ИНАЧЕ (ВЫРАЗИТЬ(ЕСТЬNULL(ТабЧасть.КоличествоОстаток, 0) / ТоварныеМестаВНоменклатуре.КоличествоМест КАК ЧИСЛО(12, 0))) 
		|		КОНЕЦ) КАК КоличествоКомплектов
		|ИЗ
		|	ТоварныеМестаВНоменклатуре КАК ТоварныеМестаВНоменклатуре
		|		
...
Функция ТекстЗапросаВТТоварныеМестаОстатки(Запрос, ТекстыЗапроса)
	...
	|	ПересчетТоваровТовары.КонтролируемаяПартия КАК КонтролируемаяПартия,
	|	ВЫБОР
...
Функция ТекстЗапросаВтТовары(Запрос, ТекстыЗапроса)
	...
	|	ВТКомплектыПоТоварнымМестаКОтражениюИзлишковНедостач.КонтролируемаяПартия КАК КонтролируемаяПартия,
