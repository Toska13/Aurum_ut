...
Процедура ТоварыПриОкончанииРедактированияСервер()
	ВзаиморасчетыРаботаСФормами.ОбновитьТекстГиперссылкиСостояниеРасчетов(ЭтаФорма);
КонецПроцедуры
...
Функция ЗаполнитьЦеныПоСоглашениюСервер(ПоВсемСтрокам = Ложь, ТекстОшибки = "")
	...
	ДействияСИзмененнымиСтроками.Вставить("ЗаполнитьСтавкуНДСПоВидуЦен", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДСПоВидуЦен(Объект));
