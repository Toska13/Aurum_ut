...
Функция ТекстЗапросаТаблицаНДСЗаписиКнигиПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	МассивТекстовЗапросов.Добавить(ТекстЗапросаСторноИсправление);
	...
	МассивТекстовЗапросов.Добавить(ТекстЗапросаВыставляемыйКомиссионеру);
	
	ТекстЗапросаПлатежныеДокменты = "
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА &Исправление
	|			ТОГДА &Период
	...
	|				ИЛИ ТаблицаСуммДокументов.ДатаСчетаФактурыКомиссионера = ДАТАВРЕМЯ(1,1,1))
	|		И ТаблицаСуммДокументов.ТипСчетаФактуры = &ИдентификаторСчетФактураКомиссионеру
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураКомиссионеру.ПлатежноРасчетныеДокументы КАК ПлатежноРасчетныеДокументы
	|			ПО Покупатели.Ссылка = ПлатежноРасчетныеДокументы.Ссылка
	|ГДЕ
	|	Покупатели.Ссылка = &Ссылка
	|	И ТаблицаСуммДокументов.ТипЗапасов В (&ТипыЗапасовСобственные)
	|	И &Организация <> ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация)
	...
	МассивТекстовЗапросов.Добавить(ТекстЗапросаПлатежныеДокменты);
	
	ТекстЗапроса = СтрСоединить(МассивТекстовЗапросов, ОбщегоНазначения.ТекстОбъединитьВсе());
...
Функция ПолучитьДанныеДляПечатнойФормыСчетФактура(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ЕСТЬNULL(ДанныеСчетаФактуры.СчетФактураОснование.Дата, ДанныеСчетаФактуры.Дата) КАК ДатаСчетаФактурыКомиссионера,
...
Функция ТекстЗапросаДанныхШапкиДляПечатиСчетаФактуры(ПараметрыПечати)
	...
	|			ИЛИ ТаблицаДокумента.ДатаСчетаФактурыКомиссионера = НАЧАЛОПЕРИОДА(СчетаФактурыОснования.ДатаСчетаФактурыКомиссионера, ДЕНЬ))
