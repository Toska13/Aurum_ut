...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РаспределениеПоСчетам.Количество() Тогда
		...
		Если ЭтоНовый() Тогда
			УстановленСтатусНеСогласована = (Статус = Перечисления.СтатусыЗаявокНаРасходованиеДенежныхСредств.НеСогласована);
		Иначе
			УстановленСтатусНеСогласована = (ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "Статус") <> Статус
				И Статус = Перечисления.СтатусыЗаявокНаРасходованиеДенежныхСредств.НеСогласована);
		КонецЕсли;
		
		Если УстановленСтатусНеСогласована Тогда
			РаспределениеПоСчетам.Очистить();
		Иначе
