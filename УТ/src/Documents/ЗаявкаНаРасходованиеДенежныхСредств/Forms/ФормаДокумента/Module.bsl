...
Процедура ХозяйственнаяОперацияПриИзмененииСервер()
	...
	ПроверитьИОчиститьОрганизациюИБанковскийСчет();
	ПроверитьИОчиститьОрганизациюПолучательИБанковскийСчетПолучатель();
...
Процедура ОрганизацияПриИзмененииСервер()
	...
	Если ЗначениеЗаполнено(Объект.Организация) И ЗначениеЗаполнено(Объект.ФормаОплатыЗаявки) Тогда
		...
		Объект.БанковскийСчет = ДенежныеСредстваСервер.ПолучитьБанковскийСчетОрганизацииПоУмолчанию(ПараметрыСчета,
			Объект.БанковскийСчет, Объект.ФормаОплатыЗаявки);
...
Процедура ФормаОплатыПриИзмененииФрагмент()
	...
	ИначеЕсли ФормаОплаты = "СБП" Тогда
		Объект.ФормаОплатыНаличная = Ложь;
		Объект.ФормаОплатыБезналичная = Истина;
		Объект.ФормаОплатыПлатежнаяКарта = Ложь;
		Объект.ФормаОплатыЗаявки = ПредопределенноеЗначение("Перечисление.ФормыОплаты.СистемаБыстрыхПлатежей");
...
Процедура ФормаОплатыЗаявкиПриИзмененииСервер()
	...
	УстановитьДоступностьПлатежнойСсылкиСБП();
...
Процедура ОрганизацияПолучательПриИзмененииСервер()
	...
	Если ЗначениеЗаполнено(Объект.ОрганизацияПолучатель) Тогда
		...
		Объект.БанковскийСчетПолучатель = Справочники.БанковскиеСчетаОрганизаций.ПолучитьБанковскийСчетПоУмолчанию(
			ПараметрыСчета);
		ЗаполнитьПодчиненныеСвойстваПоСтатистике("ОрганизацияПолучатель");
		УправлениеЭлементамиФормы();
	КонецЕсли;
...
Процедура ПодборПоОстаткамОбработка(ПроцедураЗавершения)
	...
	Если ОплатаВДругуюОрганизацию Тогда
		...
		ИначеЕсли Объект.ФормаОплатыЗаявки = ПредопределенноеЗначение("Перечисление.ФормыОплаты.Безналичная")
			Или Объект.ФормаОплатыЗаявки = ПредопределенноеЗначение("Перечисление.ФормыОплаты.СистемаБыстрыхПлатежей") Тогда
...
Процедура ПодборПоОстаткамЗавершениеСервер()
	...
	ЗаполнитьПараметрыОбъектаРасчетовВРасшифровке(Истина);
...
Функция РеквизитыФормы(Форма)
	...
	РеквизитыФормы.Вставить("ДоступнаГиперссылкаСБП");
...
Процедура НастроитьДоступныеФормыОплаты()
	...
	ЗаявкаНаРасходованиеДенежныхСредствЛокализация.ДополнитьДоступныеФормыОплаты(ЭтотОбъект);
...
Процедура ЗаполнитьРеквизитыДокументаПоФормеОплаты()
	...
	Если Не ЗначениеЗаполнено(Объект.БанковскийСчет) Тогда
		ПараметрыСчета = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации(Объект.Организация);
		ПараметрыСчета.Валюта = Объект.Валюта;
		Объект.БанковскийСчет = Справочники.БанковскиеСчетаОрганизаций.ПолучитьБанковскийСчетПоУмолчанию(ПараметрыСчета);
		ТекущийБанковскийСчет = Объект.БанковскийСчет;
		ВалютаСчетаПлательщика = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.БанковскийСчет, "ВалютаДенежныхСредств");
	КонецЕсли;
...
&НаСервере
Процедура ПроверитьИОчиститьОрганизациюИБанковскийСчет()
...
&НаСервере
Процедура ПроверитьИОчиститьОрганизациюПолучательИБанковскийСчетПолучатель()
...
// Заполнить "СтавкаНДС" в строке "РасшифровкаПлатежа", если она из "ОбъектРасчетов" не заполнилась ранее.
// 
// Параметры:
//  ИдентификаторСтрокиТаблицы - Число - Идентификатор для элемента коллекции "РасшифровкаПлатежа".
//
&НаСервере
Процедура ЗаполнитьСтавкуНДПоУмолчанию(ИдентификаторСтрокиТаблицы)
	
	СтрокаТаблицы = Объект.РасшифровкаПлатежа.НайтиПоИдентификатору(ИдентификаторСтрокиТаблицы);
	
	Если СтрокаТаблицы <> Неопределено Тогда
		УчетНДСУП.ЗаполнитьСтавкуНДСДляПлатежей(СтрокаТаблицы.СтавкаНДС, СтрокаТаблицы.НалогообложениеНДС,
...
&НаСервере
Процедура ПересчитатьСуммуНДС(ИдСтроки)
	
	ДенежныеСредстваСервер.ПересчитатьСуммуНДС(Объект, ИдСтроки);
...
Процедура ПроверитьСоответствиеВалют()
	...
	ИначеЕсли Объект.ФормаОплатыЗаявки = Перечисления.ФормыОплаты.Безналичная
		Или Объект.ФормаОплатыЗаявки = Перечисления.ФормыОплаты.СистемаБыстрыхПлатежей Тогда
...
Процедура УстановитьФормуОплаты()
	...
	ИначеЕсли Объект.ФормаОплатыБезналичная
...
Процедура УстановитьПризнакПлатежаСКонвертацией(ЗначениеПоУмолчанию = Ложь)
	...
	Если Объект.ПлатежСКонвертацией Тогда
		...
		Если Объект.ФормаОплатыЗаявки = Перечисления.ФормыОплаты.СистемаБыстрыхПлатежей Тогда
...
Процедура ЗаполнитьУИП(ИдентификаторСтрокиРасшифровки)
	...
	Если ЗначениеЗаполнено(Объект.ИдентификаторПлатежа) И ПереключательРасшифровки = 1 Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОпределитьУИПОтличается()
	...
	УИПОтличается = Объект.РасшифровкаПлатежа.Итог("ИндексУИПОтличается") <> 0;
...
Процедура Подключаемый_ПриСозданииНаСервереЛокализация(Отказ, СтандартнаяОбработка)
	...
	ЗаполнениеИзПомощника = Ложь;
	Если Параметры.Свойство("ЗначенияЗаполнения")
			И Параметры.ЗначенияЗаполнения.Свойство("ЗаполнениеИзПомощникаПоНалогуНаПрибыль") Тогда
				ЗаполнениеИзПомощника = Истина;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Если Не ЗначениеЗаполнено(Параметры.ЗначениеКопирования) 
				И Не ЗаполнениеИзПомощника Тогда
...
&НаКлиенте
Процедура Подключаемый_НадписьОплатаСБПОбработкаНавигационнойСсылкиЛокализация(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	ОчиститьСообщения();
	СтандартнаяОбработка = Ложь;
	
	Если НавигационнаяСсылкаФорматированнойСтроки = "ЗаполнениеПоПлатежнойСсылке"
		И (Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаявокНаРасходованиеДенежныхСредств.КОплате")
		Или Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаявокНаРасходованиеДенежныхСредств.Отклонена")) Тогда
		
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Добавление платежной ссылки недоступно в статусе ""%1"".'"), Объект.Статус);
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		
	Иначе
		
		ПараметрыВвода = СистемаБыстрыхПлатежейУТКлиент.НовыйПараметрыВводаПлатежнойСсылки();
		ПараметрыВвода.Форма = ЭтотОбъект;
		ПараметрыВвода.Документ = Объект.Ссылка;
		ПараметрыВвода.ПлатежнаяСсылка = Объект.ПлатежнаяСсылкаСБП;
		
		ПараметрыВвода.Организация = Объект.Организация;
		ПараметрыВвода.БанковскийСчетОрганизации = Объект.БанковскийСчет;
		ПараметрыВвода.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ЮрФизЛицо.ЮрЛицо");
		
		СистемаБыстрыхПлатежейУТКлиент.ОткрытьВводПлатежнойСсылки(ПараметрыВвода);
...
Процедура ПослеВводаПлатежнойСсылкиСБП(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		Модифицированность = Истина;
		ЗаполнитьПоПлатежнойСсылке(Результат);
	КонецЕсли;
...
&НаСервере
Процедура УстановитьДоступностьПлатежнойСсылкиСБП()

	ДоступнаГиперссылкаСБП = ДенежныеСредстваСерверЛокализация.ДоступнаГиперссылкаСБП(Объект.Ссылка, Объект.ФормаОплатыЗаявки, Истина)
		И Не ЗаявкаНаРасходованиеДенежныхСредствЛокализация.ЭтоОперацияОплатыБезСБП(
			Объект.ХозяйственнаяОперация, Объект.ПлатежСКонвертацией, КонтрагентЮрФизЛицо);
...
Процедура УстановитьНадписьОплатыСБП()
	...
	Если ЗначениеЗаполнено(Объект.ПлатежнаяСсылкаСБП) Тогда
		ТекстСсылки = СокрЛП(Объект.ПлатежнаяСсылкаСБП);
		ТекстЗаголовка = "<a href = ОткрытьПлатежнуюСсылку>" + ТекстСсылки + "</a>";
	Иначе
		ТекстСсылки = НСтр("ru = 'Ввести платежную ссылку'");
		ТекстЗаголовка = "<a href = ЗаполнениеПоПлатежнойСсылке>" + ТекстСсылки + "</a>";
	КонецЕсли;
	
	НадписьОплатаСБП = СтроковыеФункции.ФорматированнаяСтрока(ТекстЗаголовка);

КонецПроцедуры

&НаСервере
Процедура ЗаполнитьПоПлатежнойСсылке(ДанныеЗаполнения)
...
Если Не ПодключеныОбработчикиЛокализация И ПолучитьФункциональнуюОпцию("ПоддержкаПлатежейРФ") Тогда
	...
	ПодключаемыеОбработчикиСобытийЭлементов.Вставить("НадписьОплатаСБП",                "ОбработкаНавигационнойСсылки");
