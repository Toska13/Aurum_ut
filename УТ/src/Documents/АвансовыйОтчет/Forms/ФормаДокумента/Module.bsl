...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидимостьСозданияБилетов = ПравоДоступа("Добавление", Метаданные.Документы.ОперацияСЭлектроннымБилетом);
	...
	ПравоИзменения = ПравоДоступа("Изменение", Метаданные.Документы.АвансовыйОтчет);
	Элементы.СписокДокументовЗакупкиЗакупкиУдалить.Доступность = ПравоИзменения;
	Элементы.СписокДокументовЗакупкиЗакупкиПодобрать.Доступность = ПравоИзменения;
	Элементы.СписокДокументовЗакупкиГруппаСоздать.Доступность = ПравоИзменения;
	
	//++ Локализация
	Если Не ПустаяСтрока(Параметры.ТоварыКассовогоЧекаАдрес) Тогда
		ЗагрузитьКассовыеЧекиНаСервере(Параметры.ТоварыКассовогоЧекаАдрес);
	КонецЕсли;
	//-- Локализация
	...
	//++ Локализация
	ПрочиеРасходыЗагрузитьКассовыеЧекиУстановитьВидимость();
	//-- Локализация
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_ПриобретениеТоваровУслуг"
		Или ИмяСобытия = "Запись_БилетовИБронирований" Тогда
...
Процедура ОплатаПоставщикамПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если НЕ Объект.Мультивалютный И Не ТекущиеДанные = Неопределено Тогда
...
Процедура ПрочиеРасходыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ПоляБилета.Найти(Поле.Имя) <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(СтрокаТЧ.ЭлектронныйБилет) Тогда
			...
			ОткрытьФорму("Документ.ОперацияСЭлектроннымБилетом.ФормаОбъекта", ПараметрыФормы, ЭтотОбъект);
			СтандартнаяОбработка = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьБилет(Команда)
	...
	Отбор.Вставить("ЭлектронныйБилет", ПредопределенноеЗначение("Документ.ОперацияСЭлектроннымБилетом.ПустаяСсылка"));
	Если Не Объект.Мультивалютный Тогда
		Отбор.Вставить("Валюта", Объект.Валюта);
	КонецЕсли;
	Отбор.Вставить("БезВозврата", Истина);
	Отбор.Вставить("УжеВыбранныеБилеты",УжеВыбранныеБилеты());
	Отбор.Вставить("ДатаАвансовогоОтчета", Объект.Дата);
	
	ПараметрыВыбораБилета = Новый Структура;
	ПараметрыВыбораБилета.Вставить("Отбор", Отбор);
	
	ОткрытьФорму("Документ.ОперацияСЭлектроннымБилетом.Форма.ФормаВыбораБилета", ПараметрыВыбораБилета, ЭтотОбъект,,,,
...
Процедура ДобавитьСтрокуПоДаннымБилета(БилетСсылка)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ДанныеЭлектронногоБилета.СтавкаНДС КАК СтавкаНДС,
	|	СУММА(ДанныеЭлектронногоБилета.СуммаНДС) КАК СуммаНДС,
	|	СУММА(ДанныеЭлектронногоБилета.Сумма) КАК Сумма
	|ИЗ
	|	(ВЫБРАТЬ
	|		Операции.СтавкаНДС КАК СтавкаНДС,
	|		ВЫБОР
	|			КОГДА Операции.ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыОперацийСБилетами.Возврат)
	...
	|		Документ.ОперацияСЭлектроннымБилетом.Операции КАК Операции
	|	ГДЕ
	|		Операции.Ссылка.ЭлектронныйБилет = &Ссылка
	|		И НЕ Операции.Ссылка.ПометкаУдаления) КАК ДанныеЭлектронногоБилета
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеЭлектронногоБилета.СтавкаНДС
	|
	|ИМЕЮЩИЕ
	|	СУММА(ДанныеЭлектронногоБилета.Сумма) > 0";
...
Процедура ЗаполнитьПодразделение(Команда)
	...
	Если ОтсутствуютСтрокПрочихРасходов() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗакупкиСоздатьЭлектронныйБилет(Команда)
	...
	ПараметрыСоздания = Новый Структура;
	ПараметрыСоздания.Вставить("ИмяДокумента", "ОперацияСЭлектроннымБилетом");
	ПараметрыСоздания.Вставить("ЗначенияЗаполнения", ЗначенияЗаполнения);
...
Процедура ЗагрузитьКассовыеЧеки(Команда)
	...
	//++ Локализация
	ЗагрузитьКассовыеЧекиЛокализация();
	//-- Локализация
...
&НаКлиенте
Функция ОтсутствуютСтрокПрочихРасходов()
...
Процедура ЗаполнитьПолученныеАвансы()
	...
	Для Каждого СтрокаРасходов Из Объект.ПрочиеРасходы Цикл
		ДоходыИРасходыКлиентСервер.ПриДобавленииСтрокиВТаблицу(ЭтотОбъект, СтрокаРасходов, "Объект.ПрочиеРасходы");
	КонецЦикла;
...
Процедура ПоместитьРасшифровкуПлатежаВоВременноеХранилище()
	ВзаиморасчетыРаботаСФормами.ПоместитьРасшифровкуПлатежаВоВременноеХранилище(ЭтаФорма, "ПодборПоОстаткам");
КонецПроцедуры
...
Процедура ЦенаВключаетНДСПриИзмененииСервер(КэшированныеЗначения)
	...
	ПересчитываемыеСтроки =
		Объект.ПрочиеРасходы.НайтиСтроки(Новый Структура("ЭлектронныйБилет", Документы.ОперацияСЭлектроннымБилетом.ПустаяСсылка()));
...
&НаКлиенте
Процедура ЗагрузитьКассовыеЧекиЛокализация()
...
&НаСервере
Процедура ЗагрузитьКассовыеЧекиНаСервере(Знач Адрес)
	...
	РассчитатьСуммуДокумента(ЭтотОбъект);
	Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.ГруппаРасходы;
	Модифицированность = Истина;

КонецПроцедуры

&НаСервере
Процедура ПрочиеРасходыЗагрузитьКассовыеЧекиУстановитьВидимость()

	Элементы.ПрочиеРасходыЗагрузитьКассовыеЧеки.Видимость = Объект.ЦенаВключаетНДС
