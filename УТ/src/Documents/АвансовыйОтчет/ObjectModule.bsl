	...
	Если ДополнительныеСвойства.Свойство("ДокументыЗакупки") Тогда
		...
		Для каждого ДокументЗакупки Из ДокументыЗакупки Цикл
			Если (ТипЗнч(ДокументЗакупки) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг")
				Или ТипЗнч(ДокументЗакупки) = Тип("ДокументСсылка.ОперацияСЭлектроннымБилетом"))
...
Функция ПолучитьЭлектронныеБилеты(Знач ДанныеЗаполнения, ЦелиВыдачи = Неопределено, Заявка = Неопределено)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДанныеЭлектронногоБилета.Ссылка КАК Ссылка,
	|	ДанныеЭлектронногоБилета.НомерБилета КАК НомерБилета,
	|	ДанныеЭлектронногоБилета.Дата КАК Дата,
	|	ДанныеЭлектронногоБилета.Подразделение КАК Подразделение,
	|	ПРЕДСТАВЛЕНИЕ(ДанныеЭлектронногоБилета.ТипУслуги) КАК НаименованиеВходящегоДокумента,
	|	ДанныеЭлектронногоБилета.ДеталиПоездки КАК ДеталиПоездки,
	|	ДанныеЭлектронногоБилета.Валюта КАК Валюта,
	|	ДанныеЭлектронногоБилета.СтатьяДвиженияДенежныхСредств КАК СтатьяДвиженияДенежныхСредств,
	|	ДанныеЭлектронногоБилета.КонтрагентПеревозчик КАК КонтрагентПеревозчик
	|ПОМЕСТИТЬ ВТЭлектронныеБилеты
	|ИЗ
	|	Документ.ОперацияСЭлектроннымБилетом КАК ДанныеЭлектронногоБилета
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПредыдущийОтчет КАК ПредыдущийОтчет
	|		ПО (ИСТИНА)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.АвансовыйОтчет.ПрочиеРасходы КАК ДанныеОтчета
	|		ПО ДанныеЭлектронногоБилета.Ссылка = ДанныеОтчета.ЭлектронныйБилет
	|			И (НЕ ДанныеОтчета.Ссылка = &Ссылка)
	|ГДЕ
	|	ДанныеЭлектронногоБилета.Сотрудник = &ПодотчетноеЛицо
	|	И ДанныеЭлектронногоБилета.Подразделение = &Подразделение
	|	И ДанныеЭлектронногоБилета.Организация = &Организация
	|	И ДанныеЭлектронногоБилета.ЭлектронныйБилет = ЗНАЧЕНИЕ(Документ.ОперацияСЭлектроннымБилетом.ПустаяСсылка)
	|	И ДанныеЭлектронногоБилета.Проведен
	|	И НЕ ДанныеЭлектронногоБилета.СпособСписанияНапрямую
	|	И (ДанныеЭлектронногоБилета.СтатьяДвиженияДенежныхСредств В (&ЦелиВыдачи)
	|			ИЛИ &ЛюбыеЦели)
	|	И (ДанныеЭлектронногоБилета.Валюта = &Валюта
	|			ИЛИ &Мультивалютный)
	|	И (ДанныеЭлектронногоБилета.ДокументОснование В (&Заявка)
	|			ИЛИ &ЗаявкаНеУказана)
	|	И ДанныеЭлектронногоБилета.ДатаПрибытия > ЕСТЬNULL(ПредыдущийОтчет.Дата, ДАТАВРЕМЯ(1, 1, 1))
	|	И ДанныеЭлектронногоБилета.ДатаПрибытия <= &ДатаАвансовогоОтчета
	|	И ДанныеОтчета.Ссылка ЕСТЬ NULL
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		ДанныеБилетов.Ссылка КАК ЭлектронныйБилет,
	|		ДанныеБилетов.НомерБилета КАК НомерВходящегоДокумента,
	|		ДанныеБилетов.Дата КАК ДатаВходящегоДокумента,
	|		ДанныеБилетов.Подразделение КАК Подразделение,
	|		ДанныеБилетов.НаименованиеВходящегоДокумента КАК НаименованиеВходящегоДокумента,
	|		ДанныеБилетов.ДеталиПоездки КАК Содержание,
	|		ДанныеБилетов.Валюта КАК Валюта,
	|		ДанныеБилетов.СтатьяДвиженияДенежныхСредств КАК СтатьяДвиженияДенежныхСредств,
	|		ДанныеБилетов.КонтрагентПеревозчик КАК Контрагент,
	|		СоставОперации.СтавкаНДС КАК СтавкаНДС,
	|		ВЫБОР
	|			КОГДА СоставОперации.ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыОперацийСБилетами.Возврат)
	...
	|		КОНЕЦ КАК СуммаНДС,
	|		СоставОперации.СуммаНДС <> 0 КАК БланкСтрогойОтчетности
	|	ИЗ
	|		ВТЭлектронныеБилеты КАК ДанныеБилетов
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОперацияСЭлектроннымБилетом.Операции КАК СоставОперации
	|			ПО ДанныеБилетов.Ссылка = СоставОперации.Ссылка
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ДанныеБилетов.Ссылка,
	|		ДанныеБилетов.НомерБилета,
	|		ДанныеБилетов.Дата,
	|		ДанныеБилетов.Подразделение,
	|		ДанныеБилетов.НаименованиеВходящегоДокумента,
	|		ДанныеБилетов.ДеталиПоездки,
	|		ДанныеБилетов.Валюта,
	|		ДанныеБилетов.СтатьяДвиженияДенежныхСредств,
	|		ДанныеБилетов.КонтрагентПеревозчик,
	...
	|			ИНАЧЕ СоставОперации.СуммаНДС
	|		КОНЕЦ,
	|		СоставОперации.СуммаНДС <> 0
	|	ИЗ
	|		ВТЭлектронныеБилеты КАК ДанныеБилетов
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОперацияСЭлектроннымБилетом КАК ДанныеОперацийБилетов
	|			ПО ДанныеБилетов.Ссылка = ДанныеОперацийБилетов.ЭлектронныйБилет
	|				И (ДанныеОперацийБилетов.Проведен)
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОперацияСЭлектроннымБилетом.Операции КАК СоставОперации
	|			ПО (ДанныеОперацийБилетов.Ссылка = СоставОперации.Ссылка)) КАК Операции
	|
	|СГРУППИРОВАТЬ ПО
	...
	Запрос.УстановитьПараметр("ДатаАвансовогоОтчета", Дата);
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ПрочиеРасходы, СтруктураДействий, Неопределено);
...
Процедура СгенерироватьПервичныеДокументы(Отказ)
	...
	|	И ПрочиеРасходы.ЭлектронныйБилет = ЗНАЧЕНИЕ(Документ.ОперацияСЭлектроннымБилетом.ПустаяСсылка)
	|СГРУППИРОВАТЬ ПО
	|	ПрочиеРасходы.НомерВходящегоДокумента,
	|	ПрочиеРасходы.ДатаВходящегоДокумента,
	|	ПрочиеРасходы.Контрагент,
	...
	МассивСвойствДанныхДляЗаполнения = Новый Массив;
	МассивСвойствДанныхДляЗаполнения.Добавить("НомерВходящегоДокумента");
	МассивСвойствДанныхДляЗаполнения.Добавить("ДатаВходящегоДокумента");
	МассивСвойствДанныхДляЗаполнения.Добавить("Контрагент");
	МассивСвойствДанныхДляЗаполнения.Добавить("Валюта");
	МассивСвойствДанныхДляЗаполнения.Добавить("СуммаДокумента");
	МассивСвойствДанныхДляЗаполнения.Добавить("СуммаРегл");
	СвойстваДляЗаполненияПервичногоДокумента = СтрСоединить(МассивСвойствДанныхДляЗаполнения, ",");
	
	НачатьТранзакцию();
	
	Попытка
		
		ТекстСообщения = НСтр("ru = 'Не удалось сформировать первичные документы'");
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.СтатусОбработки = 1 И Не ПометкаУдаления Тогда
				...
				ДанныеАвансовогоОтчета = Новый Структура(СвойстваДляЗаполненияПервичногоДокумента);
				ЗаполнитьЗначенияСвойств(ДанныеАвансовогоОтчета, Выборка);
				
				ДанныеАвансовогоОтчета.Вставить("Организация", Организация);
				ДанныеАвансовогоОтчета.Вставить("Подразделение", Подразделение);
				ДанныеАвансовогоОтчета.Вставить("Дата", Дата);
				ДанныеАвансовогоОтчета.Вставить("НалогообложениеНДС", ЗакупкаПодДеятельность);
				ДанныеАвансовогоОтчета.Вставить("ТипПервичногоДокумента", Перечисления.ТипыПервичныхДокументов.ЗакупкаЧерезПодотчетноеЛицо);
				
				ДанныеДляЗаполнения = Новый Структура("ДанныеАвансовогоОтчета", ДанныеАвансовогоОтчета);
				ДокументОбъект.Заполнить(ДанныеДляЗаполнения);
				
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось записать %1'"), ДокументОбъект);
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ПодробностиОшибки = ОбработкаОшибок.СообщениеОбОшибкеДляПользователя(ИнформацияОбОшибке());
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(НСтр("ru = '%1. Подробности ошибки: %2'"), ТекстСообщения, ПодробностиОшибки),,,, Отказ);
		Возврат;
		...
		Попытка
			...
			Пока Выборка.Следующий() Цикл
				Если ЗначениеЗаполнено(Выборка.Ссылка) Тогда
					...
					ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось отменить %1'"), ДокументОбъект);
...
Процедура ПроверитьДублиУчетаНДСПервичныхДокументов(Отказ)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ДанныеДляПроверки.НомерВходящегоДокумента КАК НомерВходящегоДокумента,
	|	ДанныеДляПроверки.ДатаВходящегоДокумента КАК ДатаВходящегоДокумента,
	|	МАКСИМУМ(ДанныеДляПроверки.НомерСтроки) КАК НомерСтроки
	|ИЗ
	|	(ВЫБРАТЬ
	|		1 КАК Активность,
	...
	|		ДанныеДокумента.ДатаВходящегоДокумента,
	|		ДанныеДокумента.Контрагент,
	|		МАКСИМУМ(ДанныеДокумента.НомерСтроки)
	|	ИЗ
	|		ДанныеДокумента КАК ДанныеДокумента
	|	ГДЕ
	|		ДанныеДокумента.БланкСтрогойОтчетности
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ДанныеДокумента.НомерВходящегоДокумента,
	|		ДанныеДокумента.ДатаВходящегоДокумента,
	|		ДанныеДокумента.Контрагент) КАК ДанныеДляПроверки
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеДляПроверки.НомерВходящегоДокумента,
	|	ДанныеДляПроверки.ДатаВходящегоДокумента,
	|	ДанныеДляПроверки.Контрагент
	|
	|ИМЕЮЩИЕ
	|	СУММА(ДанныеДляПроверки.Активность) = 0";
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Для документа №%1 от %2 указано одновременно несколько вариантов принятия к вычету НДС'"),
