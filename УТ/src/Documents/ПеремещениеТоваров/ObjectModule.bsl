...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("Основание") Тогда
			...
			Если ТипЗнч(ДанныеЗаполнения.Основание) = Тип("ДокументСсылка.АктКонтроляКачестваТоваров") Тогда
				ЗаполнитьПоАктуКонтроляКачества(ДанныеЗаполнения);
			ИначеЕсли ТипЗнч(ДанныеЗаполнения.Основание) = Тип("ДокументСсылка.ПриемкаТоваровНаХранение") Тогда
				ЗаполнитьПоПриемкеТоваровНаХранение(ДанныеЗаполнения);
			ИначеЕсли ТипЗнч(ДанныеЗаполнения.Основание) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	КлючевыеРеквизиты = КонтрольКачества.РеквизитыОпределенияНеобходимостиПартииВДокументе(СкладПолучатель, ХозяйственнаяОперация);
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКонтрольКачестваТоваров") Тогда
		...
		// Заполнение КонтролируемаяПартия.
		Для Каждого СтрокаТаблицы Из Товары Цикл
			Если ЗначениеЗаполнено(СтрокаТаблицы.КонтролируемаяПартияОтправителя) Тогда
				СтрокаТаблицы.КонтролируемаяПартия = СтрокаТаблицы.КонтролируемаяПартияОтправителя;
			Иначе
				СтрокаТаблицы.КонтролируемаяПартия = КонтролируемаяПартия;
			КонецЕсли;
		КонецЦикла;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ПеремещениеЗаблокированныхТоваров Тогда
		МассивНепроверяемыхРеквизитов.Добавить("КонтролируемаяПартияОтправителя");
	КонецЕсли;
...
Процедура ЗаполнитьПоАктуКонтроляКачества(ДанныеЗаполнения)
	...
	Документы.АктКонтроляКачестваТоваров.ТоварыДляСозданияНаОсновании(
		...
		Перечисления.ДействияПоТоварамВАктеКонтроляКачества.ОформитьПеремещение);
	...
	Если ТоварыОснования.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ru='Документ %Документ% не содержит товаров c действием ""Оформить перемещение"". Ввод на основании документа запрещен.'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения.Основание);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, РеквизитыШапки, "Организация,Подразделение");
...
Процедура ЗаполнитьПоПриемкеТоваровНаХранение(ДанныеЗаполнения)
...
Процедура ЗаполнитьДокументНаОснованииАктаОРасхожденияхПослеПеремещения(Знач ДанныеЗаполнения)
	...
	|	ПеремещениеТоваров.ПеремещениеЗаблокированныхТоваров,
	|	ПеремещениеТоваров.КонтролируемаяПартияОтправителя,
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	Если Не ПеремещениеЗаблокированныхТоваров Тогда
		КонтролируемаяПартияОтправителя = Справочники.КонтролируемыеПартии.ПустаяСсылка();
	КонецЕсли;
...
Процедура ЗаполнитьДокументНаОснованииЗаявкиНаОбеспечение(ЗаявкаНаОбеспечение)
	...
	СкладПолучатель = СвойстваШапки.Склад;
