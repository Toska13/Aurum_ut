...
Процедура СкладПолучательПриИзменении(Элемент)
	...
	Если СкладПолучатель <> Объект.СкладПолучатель Тогда
		...
		Иначе
			СкладПолучательПриИзмененииСервер(ОтвязатьВсеСтрокиОтЗаказа, ОчищатьСтроки, КэшированныеЗначения, Истина);
		КонецЕсли;
...
&НаСервере
Процедура СкладПолучательПриИзмененииСервер(ОтвязатьВсеСтрокиОтЗаказа, ОчищатьСтроки, КэшированныеЗначения, НепосредственноеИзменениеПоляСклад = Ложь)
...
Процедура УстановитьВидимостьГруппыКонтрольКачества()
	...
	Элементы.ГруппаКонтрольКачестваОтступ.Видимость = Не Объект.ПеремещениеПоЗаказам
		И Объект.ПеремещениеЗаблокированныхТоваров;
	Элементы.ПеремещениеЗаблокированныхТоваров.Видимость = Не Объект.ПеремещениеПоЗаказам;
...
Процедура ОбработкаПодбораТоваровИзЗаказа(АдресВХранилище)
	...
	ПродажиСерверЛокализация.ПеренестиШтрихкодыУпаковок(ЭтотОбъект, СтруктураДанных.ШтрихкодыУпаковок);
...
Процедура ПроверитьКоличествоВДокументе(Команда)
	...
	ПараметрыУказанияСерийВрем.ИмяПоляСкладОтправитель = Неопределено;
	ПараметрыУказанияСерийВрем.ИмяПоляСкладПолучатель = Неопределено;
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьКонтролируемуюПартиюОтправителя", КонтрольКачестваКэш);
...
Процедура ПеремещениеЗаблокированныхТоваровПриИзмененииСервер()
	...
	УстановитьВидимостьГруппыКонтрольКачества();
	Если Не Объект.ПеремещениеЗаблокированныхТоваров Тогда
	...
	Если КонтрольКачества.ОбновитьПараметрыДействий(
		...
		СтруктураДействий.Вставить("ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества", КонтрольКачестваКэш);
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Объект.Товары, СтруктураДействий);
		Для Каждого СтрокаТаблицы Из Объект.Серии Цикл
			СтрокаТаблицы.КонтролируемаяПартияОтправителя = Объект.КонтролируемаяПартияОтправителя;
		КонецЦикла;
		НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(Объект, ПараметрыУказанияСерий);
...
Процедура ПриИзмененииПеремещенияПоНесколькимЗаказам(КэшированныеЗначения)
	...
	Если Объект.ПеремещениеПоЗаказам Тогда
		...
		КонецЕсли;
		Объект.ПеремещениеЗаблокированныхТоваров = Ложь;
		Объект.КонтролируемаяПартияОтправителя = Справочники.КонтролируемыеПартии.ПустаяСсылка();
	...
	КонтрольКачества.ОбновитьПараметрыДействий(Документы.ПеремещениеТоваров, Объект, КонтрольКачестваКэш);
	СтруктураДействий = Новый Структура();
	СтруктураДействий.Вставить("ЗаполнитьКонтролируемуюПартиюОтправителя", КонтрольКачестваКэш);
	СтруктураДействий.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", КонтрольКачестваКэш);
	СтруктураДействий.Вставить("ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества", КонтрольКачестваКэш);
...
Процедура ЗаполнитьМногооборотнуюТаруИзХранилищаСервер(АдресТарыВХранилище)
	...
	СтруктураДействийДобавленныеСтроки.Вставить("ЗаполнитьКонтролируемуюПартиюОтправителя", КонтрольКачестваКэш);
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	Колонки = "Номенклатура,Характеристика,Упаковка,КоличествоУпаковок,НоменклатураНабора,ХарактеристикаНабора,Назначение";
