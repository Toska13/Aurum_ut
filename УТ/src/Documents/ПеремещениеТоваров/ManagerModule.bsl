...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Команда = Документы.АктКонтроляКачестваТоваров.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
	Если Команда <> Неопределено Тогда
		
		ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(
			Команда,
			"ПеремещениеЗаблокированныхТоваров",
			Ложь,
			ВидСравненияКомпоновкиДанных.Равно);
		
	КонецЕсли;
	
	Команда = Документы.ЗаявкаНаКонтрольКачестваТоваров.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
	Если Команда <> Неопределено Тогда
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	ИспользоватьОрдернуюСхемуПолучатель  = СкладыСервер.ИспользоватьОрдернуюСхемуПриПоступлении(
...
// 
// Параметры:
//  ОбъектИлиСсылка - ДокументОбъект.ПеремещениеТоваров, ДокументСсылка.ПеремещениеТоваров - документ.
...
// 
// Параметры:
//  Объект - ДокументОбъект.ПеремещениеТоваров - объект.
// 
// Возвращаемое значение:
...
Функция КонтрольКачестваПараметрыДействий(Объект) Экспорт
	...
	ПараметрыДействия.ЧитаемыеПоля = "ЗаказНаПеремещение";
	...
	ПараметрыДействия.ПараметрыЗапроса.Вставить("КонтрольКачестваПеремещениеПоЗаказам", Объект.ПеремещениеПоЗаказам);
	ПараметрыДействия.ПараметрыЗапроса.Вставить("КонтрольКачестваПеремещениеЗаблокированныхТоваров",
		Объект.ПеремещениеЗаблокированныхТоваров);
	ПараметрыДействия.ПараметрыЗапроса.Вставить("КонтрольКачестваЗаказНаПеремещение", Объект.ЗаказНаПеремещение);
	ПараметрыДействия.ТекстЗапроса =
		"ВЫБРАТЬ
		|	Таблица.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
		|	ВЫБОР
		|		КОГДА &КонтрольКачестваПеремещениеЗаблокированныхТоваров
		|			ТОГДА &КонтрольКачестваКонтролируемаяПартия
		// Дублируется логика получения заказа из обработчика ПередЗаписью.
		|		КОГДА &КонтрольКачестваПеремещениеПоЗаказам
		|				И &КонтрольКачестваЗаказНаПеремещение <> ЗНАЧЕНИЕ(Документ.ЗаказНаПеремещение.ПустаяСсылка)
		|				И Таблица.ЗаказНаПеремещение = ЗНАЧЕНИЕ(Документ.ЗаказНаПеремещение.ПустаяСсылка)
		|				И ВЫРАЗИТЬ(&КонтрольКачестваЗаказНаПеремещение КАК Документ.ЗаказНаПеремещение).ПеремещениеЗаблокированныхТоваров
		|			ТОГДА
		|				ВЫРАЗИТЬ(&КонтрольКачестваЗаказНаПеремещение КАК Документ.ЗаказНаПеремещение).КонтролируемаяПартияОтправителя
		// Логика интерактивного заполнения.
		|		КОГДА ЕСТЬNULL(ВЫРАЗИТЬ(Таблица.ЗаказНаПеремещение КАК Документ.ЗаказНаПеремещение).ПеремещениеЗаблокированныхТоваров, ЛОЖЬ)
		|			ТОГДА ВЫРАЗИТЬ(Таблица.ЗаказНаПеремещение КАК Документ.ЗаказНаПеремещение).КонтролируемаяПартияОтправителя
		|		ИНАЧЕ
	...
	ПараметрыДействия = КонтрольКачества.НовыйПараметрыДействия(ИмяДействия);
	ПараметрыДействия.ПолноеИмяОбъекта = "Документ.ПеремещениеТоваров";
	...
	ПараметрыДействия.КлючевыеРеквизитыШапки.Вставить("КонтрольКачестваКонтролируемаяПартия", Объект.КонтролируемаяПартияОтправителя);
	ПараметрыДействия.КлючевыеРеквизитыШапки.Вставить("КонтрольКачестваПеремещениеЗаблокированныхТоваров",
		Объект.ПеремещениеЗаблокированныхТоваров);
	ПараметрыДействия.КлючевыеРеквизитыШапки.Вставить("КонтрольКачестваПеремещениеПоЗаказам", Объект.ПеремещениеПоЗаказам);
...
Функция РаспоряженияНакладной(Накладная, РеквизитыШапки) Экспорт
	...
	Запрос.Текст =
		...
		|	Документ.ЗаказНаПеремещение КАК ЗаказНаПеремещение
		|ГДЕ
		|	ЗаказНаПеремещение.ХозяйственнаяОперация   = &ХозяйственнаяОперация
...
Функция МакетФормыВыбораНазначений() Экспорт
	...
	ОписаниеКолонок.ПутиКДанным.КонтролируемаяПартия = "Объект.Товары.КонтролируемаяПартияОтправителя";
...
Процедура ТекстЗапросаДанныеДокументаШапка(СписокЗапросов)
	...
	ТекстЗапросаШапка = 
		...
		|	ДанныеДокумента.СкладОтправитель КАК СкладОтправитель,
		|	ДанныеДокумента.СкладПолучатель КАК СкладПолучатель,
		|	ДанныеДокумента.ПеремещениеЗаблокированныхТоваров КАК ПеремещениеЗаблокированныхТоваров,
...
Процедура ТекстЗапросаДанныеДокументаТаблицаТовары(СписокЗапросов)
	...
	ТекстЗапросаТаблицаТовары =
		...
		|	ТаблицаТовары.ПодлежитРазблокировке КАК ПодлежитРазблокировке,
		|	ТаблицаТовары.ДатаПодтвержденияКачества КАК ДатаПодтвержденияКачества,
		|	ТаблицаТовары.КонтролируемаяПартияОтправителя КАК КонтролируемаяПартияОтправителя,
...
Процедура ЗапланироватьОтгрузкуТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаДанныхДокумента = 
		...
		|	ТоварыДокумента.КонтролируемаяПартияОтправителя КАК КонтролируемаяПартия,
...
Процедура ОформитьОтгрузкуТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаДанныхДокумента = 
		...
		|	ТоварыДокумента.Характеристика КАК Характеристика,
		|	ТоварыДокумента.НазначениеОтправителя КАК Назначение,
		|	ТоварыДокумента.Назначение КАК НазначениеДляСоединения,
		|	ТоварыДокумента.КонтролируемаяПартияОтправителя КАК КонтролируемаяПартия,
		|	ТоварыДокумента.Серия КАК Серия,
		|	ТоварыДокумента.СтатусУказанияСерийОтправитель КАК СтатусУказанияСерий,
		|	ТоварыДокумента.Количество КАК Количество,
	...
	ПараметрыМодульногоПроведения.СинонимТаблицыДокумента = "ТоварыДокумента";
	ПараметрыМодульногоПроведения.ПереопределяемыеПоляСоединенияССериями = ПереопределяемыеПоляСоединенияССериями;
	ПараметрыМодульногоПроведения.ОбъектМетаданных = Метаданные.Документы.ПеремещениеТоваров;
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если ДокументОбъект = Неопределено
		...
		ТекстЗапросаТабЧасть =
			...
			|			ТОГДА ТабЧасть.КонтролируемаяПартияОтправителя
			|		ИНАЧЕ ТабЧасть.ЗаказНаПеремещение
...
//
// Параметры:
//	МассивДокументов - Массив Из ДокументСсылка.ПеремещениеТоваров - коллекция ссылок на документы, используемых
...
Функция ДанныеДляПечатиЭтикеток(МассивДокументов) Экспорт
	...
	|	ДанныеШапки.Организация							КАК Организация,
	|	ДанныеШапки.СкладПолучатель						КАК Склад,
	|	ДанныеШапки.СкладПолучатель.РозничныйВидЦены	КАК ВидЦены
	|ИЗ
	|	Документ.ПеремещениеТоваров КАК ДанныеШапки
	|ГДЕ
	|	ДанныеШапки.Ссылка В (&МассивДокументов)
	|	И НЕ ДанныеШапки.СкладПолучатель.РозничныйВидЦены ЕСТЬ NULL";
	...
	РеквизитыДокументов = РезультатЗапроса.Выгрузить();
	...
	|ИЗ
	|	Документ.ПеремещениеТоваров.Серии КАК ТаблицаСерий
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПеремещениеТоваров КАК ДанныеШапки
	|		ПО ТаблицаСерий.Ссылка = ДанныеШапки.Ссылка
	|
	|ГДЕ
	...
	|	ТаблицаСерий.Серия			КАК Серия,
	|	ТаблицаТоваров.Упаковка		КАК Упаковка,
	|	ТаблицаСерий.Количество / ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки, 1) КАК Количество
	...
	СтруктураРезультат.Вставить("Товары",					РезультатЗапроса[РезультатЗапроса.ВГраница() - 1].Выгрузить());
	СтруктураРезультат.Вставить("ТаблицаОшибокПоСериям",	РезультатЗапроса[РезультатЗапроса.ВГраница()].Выгрузить());
...
Функция ДанныеДляПечатиЦенников(МассивДокументов) Экспорт
	...
	|	ВЫБОР
	|		КОГДА НЕ ПеремещениеТоваров.СкладПолучатель = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
...
Функция ПараметрыСозданияПоГруппеСкладов(ПараметрКоманды) Экспорт
	...
	ИначеЕсли ТипЗнч(ПараметрКоманды) = Тип("ДокументСсылка.АктКонтроляКачестваТоваров") Тогда
		
		Запрос.Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		|	Товары.Склад.Ссылка КАК Склад
		|ИЗ
		|	Документ.АктКонтроляКачестваТоваров.Товары КАК Товары
		|ГДЕ
		|	Товары.Ссылка = &Заказ
		|	И Товары.Действие = ЗНАЧЕНИЕ(Перечисление.ДействияПоТоварамВАктеКонтроляКачества.ОформитьПеремещение)";
	...
	ИначеЕсли Склады.Количество() = 0 Тогда
		Если ТипЗнч(ПараметрКоманды) = Тип("ДокументСсылка.АктКонтроляКачестваТоваров") Тогда
			Склады.Добавить(Справочники.Склады.ПустаяСсылка()); // в обработке заполнения документа товары заполняться не будут, будет выдано сообщение об отсуствии строк с нужным действием
		Иначе
