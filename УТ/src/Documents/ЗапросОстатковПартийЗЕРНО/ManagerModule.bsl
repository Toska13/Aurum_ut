...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ОбновлениеИнформационнойБазыГосИС.ДобавитьОбработчикОбновленияПереходаНаКонструкторНастроек(
		...
		Новый УникальныйИдентификатор("1e4755fa-a1d6-48ca-b298-e80dc8c0d418"));
	...
	ИначеЕсли Операция = Перечисления.ВидыОперацийЗЕРНО.ЗапросПартий
		...
		Если ВсеСтраницыВсехСтатусовПолучены И ПараметрыЗапросаОснования.СозданныеПартии.Количество() Тогда
			...
			Для Каждого СтрокаТаблицы Из ПатрииТребующиеЗагрузки Цикл
				...
				Если ЗначениеЗаполнено(СтрокаТаблицы.РазделУчета) Тогда
					ПараметрыФормирования.РазделУчета = СтрокаТаблицы.РазделУчета;
				Иначе
					ПараметрыФормирования.РазделУчета = ПараметрыЗапроса.РазделУчета;
				КонецЕсли;
...
Функция СообщениеЗагрузкиПартий(ДокументСсылка, ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.ПараметрыОбработкиДокумента.Операция = Перечисления.ВидыОперацийЗЕРНО.ЗагрузкаОстатковПартий Тогда
		...
		Шапка.Вставить("РазделУчета",          ДополнительныеПараметры.ПараметрыОбработкиДокумента.РазделУчета);
...
Функция СообщенияЗагрузкиПартий(ПараметрыФормирования, ПараметрыОбмена, КешОшибок = Неопределено)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПараметрыФормирования, "РазделУчета") Тогда
		РазделУчета = ПараметрыФормирования.РазделУчета;
	Иначе
		РазделУчета = ОбновлениеИнформационнойБазыЗЕРНО.РазделУчетаПоВидуПродукции(ПараметрыФормирования.ВидПродукции);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПараметрыФормирования.НомерПартии) Тогда
		ИнтеграцияЗЕРНОСлужебный.ЗаполнитьСвойствоXDTO(
			ЗапросПартийXDTO,
			ИнтеграцияЗЕРНОСлужебный.ИмяПоляНомерПартии(РазделУчета),
