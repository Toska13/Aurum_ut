...
Процедура НадписьЗакрытьНажатие(Элемент)
	...
	ПоказатьИнформационнуюНадпись = Ложь;
	
	ОбновитьОтображениеИнформационнойНадписи();
...
Процедура НадписьОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ОтборСостояние = "НетДанныхПлатежноРасчетногоДокумента";
	УстановитьОтборСостояния(Элементы.ПолученныеАвансы, ОтборСостояние);
...
Процедура ЗаполнитьПолученныеАвансыНаСервере()
	...
	Иначе
		Если РегистрыСведений.ЗаданияКРаспределениюВзаиморасчетов.АктуальныйПериодРассчитанныхДанныхПоВзаиморасчетам() <> Неопределено Тогда
			ОбщегоНазначения.СообщитьПользователю(ВзаиморасчетыСервер.ТекстПредупрежденияЗаданияРаспределенияВзаиморасчетов());
		КонецЕсли;
	КонецЕсли;
...
Функция ЗаполнитьАвансы()
	...
	ЗаполнитьЗначенияСвойств(ОтборАвансов, ЭтотОбъект);
	...
	ПолученныеАвансы.Загрузить(ПолученныеАвансыТЗ);
	ПолученныеАвансы.Сортировать("ДатаВыписки");
	ЗаполнитьНеиспользуемыеСчетаФактуры();
	
	ПолученныеАвансыТЗ.Свернуть("ДокументОснование, СФСформирован, Исправление, СуммаСчетаФактуры", "Сумма");
	Для Каждого Строка Из ПолученныеАвансыТЗ Цикл
		Если Строка.СФСформирован = Истина
			И Строка.Сумма <> Строка.СуммаСчетаФактуры
			И НЕ Строка.Исправление = Истина Тогда
...
Функция СформироватьСчетаФактурыСервер(ДанныеСчетФактуры = Неопределено, ОбновитьБезусловно = Ложь)
	...
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаАвансы.ДокументОснование) = ТИП(Документ.Сторно)
	|			ТОГДА НЕОПРЕДЕЛЕНО
		...
		Пока ВыборкаНалогообложениеНДС.Следующий() Цикл
			...
			Пока ВыборкаКонтрагент.Следующий() Цикл
				...
				Попытка
					...
					Если ТипЗнч(СчетФактураОбъект.ДокументОснование) = Тип("ДокументСсылка.Сторно") Тогда
						ПоказатьИнформационнуюНадпись = Истина;
					КонецЕсли;
...
Процедура ОбновитьДанныеОСчетахФактурахВСтроке(Строка, ТаблицаОбработанных,  ТаблицаВозниклиОшибки)
	...
	Если РезультатПоискаОбработанных.Количество() <> 0 Тогда
		...
		Если ТипЗнч(Строка.ДокументОснование) = Тип("ДокументСсылка.Сторно") Тогда
			Строка.НетДанныхПлатежноРасчетногоДокумента = Истина;
		КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПолученныеАвансы.НетДанныхПлатежноРасчетногоДокумента");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
...
Процедура УстановитьСтавкуНДСНаСервере(СтавкаНДС)
	...
	ВыделенныеСтроки = РаботаСТабличнымиЧастямиКлиентСервер.ЭлементыКоллекции(
		ПолученныеАвансы, Элементы.ПолученныеАвансы.ВыделенныеСтроки);
...
Функция УстановитьОтборСостояния(ТаблицаПолученныеАвансы, ОтборСостояние)
	...
	ИначеЕсли ОтборСостояние = "ОснованиеСторно" Тогда
		ТаблицаПолученныеАвансы.ОтборСтрок = Новый ФиксированнаяСтруктура("ОснованиеСторно", Истина);
	ИначеЕсли ОтборСостояние = "НетДанныхПлатежноРасчетногоДокумента" Тогда
		ТаблицаПолученныеАвансы.ОтборСтрок = Новый ФиксированнаяСтруктура("НетДанныхПлатежноРасчетногоДокумента", Истина);
	Иначе
...
&НаКлиенте
Процедура ОбновитьОтображениеИнформационнойНадписи()
	
	Элементы.ГруппаИнформация.Видимость = ПоказатьИнформационнуюНадпись;
