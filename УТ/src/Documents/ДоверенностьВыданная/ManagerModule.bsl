...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если ВидФормы = "ФормаСписка" Тогда
		Если Не ПравоДоступа("Чтение", Метаданные.РегистрыСведений.СостоянияПоОбъектамУчетаЭДО) Тогда
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если ДоступностьРаспоряженийТовары Тогда
		...
		|
		|ВЫБРАТЬ
		|	ТаблицаРаспоряженийВозврат.Распоряжение
		|ИЗ
		|	#ЗаявкиНаВозвратОборотыРазвернутая КАК ТаблицаРаспоряженийВозврат
		|ГДЕ
		|	(ТаблицаРаспоряженийВозврат.КОформлениюПоВозвратамКоличествоОборот -
		|		ТаблицаРаспоряженийВозврат.ОформленоПоВозвратамКоличествоОборот) > 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		ПараметрыФормированияТаблицы.ТекстУсловия =
			"НЕ Распоряжение.ЭтоЗаказКакСчет
			|		И Распоряжение В
			|			(ВЫБРАТЬ
...
Функция ТекстЗапросаРаспоряженийНаТовары(НетДоверенности = Неопределено) Экспорт
	...
	КонецЕсли;
	Если ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат) Тогда
		
		ТекстЗапросаРаспоряженияНаОтгрузку = "
		|	ВЫБРАТЬ
		|		ЗаявкиНаВозврат.Распоряжение КАК Распоряжение
		|	ИЗ
		|		#ЗаявкиНаВозвратОборотыРазвернутая КАК ЗаявкиНаВозврат
		|	
		|	СГРУППИРОВАТЬ ПО
		|		ЗаявкиНаВозврат.Распоряжение
		|	
		|	ИМЕЮЩИЕ
		|		СУММА(ЗаявкиНаВозврат.КОформлениюПоВозвратамКоличествоОборот -
		|			ЗаявкиНаВозврат.ОформленоПоВозвратамКоличествоОборот) > 0";
		...
		ТекстЗапросаРаспоряженияНаОтгрузку = СтрЗаменить(ТекстЗапросаРаспоряженияНаОтгрузку,
			"#ЗаявкиНаВозвратОборотыРазвернутая",
			ЗаявкиНаВозвратОборотыРазвернутая);
		
		ТекстЗапросаПлановоеПоступление.Добавить(ТекстЗапросаРаспоряженияНаОтгрузку);
...
Функция СформироватьГиперссылкуСмТакжеВРаботе(Параметры) Экспорт
	...
	Иначе
		...
		Если Не (ПолучитьФункциональнуюОпцию("ИспользоватьДоверенностиНаПолучениеТМЦ")
				...
				И (ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.ЗаказыПоставщикам)
					Или ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат)
...
Функция ТекстЗапросаРаспоряженийНаДС() Экспорт
	...
	|	И ДокументРасчетов.ФормаОплаты <> ЗНАЧЕНИЕ(Перечисление.ФормыОплаты.СистемаБыстрыхПлатежей)";
