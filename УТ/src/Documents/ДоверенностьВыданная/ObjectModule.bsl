...
Процедура ЗаполнитьПоЗаявкеНаВозвратТоваров(Знач ДокументОснование)
	...
	ПараметрыФормированияТаблицы.ТекстУсловия =
		...
		|		И Распоряжение.ЭтоЗаказКакСчет
	...
	|	ЗаказТовары.Характеристика КАК Характеристика,
	|	ЗаказТовары.Упаковка КАК Упаковка,
	|	(ЗаявкаНаВозврат.КОформлениюПоВозвратамКоличествоОборот -
	|		ЗаявкаНаВозврат.ОформленоПоВозвратамКоличествоОборот) / ВЫБОР
	|		КОГДА ЗаказТовары.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|			ТОГДА 1
	|		ИНАЧЕ &ТекстЗапросаКоэффициентУпаковки
	|	КОНЕЦ КАК КоличествоУпаковок
	|ИЗ
	|	ВТ_ЗаявкиНаВозвратОборотыРазвернутая КАК ЗаявкаНаВозврат
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаВозвратТоваровОтКлиента.ВозвращаемыеТовары КАК ЗаказТовары
	|		ПО (ЗаказТовары.Ссылка = &ДокументОснование)
	|			И ЗаявкаНаВозврат.Распоряжение = ЗаказТовары.Ссылка
	...
	|ГДЕ
	|	(ЗаявкаНаВозврат.КОформлениюПоВозвратамКоличествоОборот -
	|		ЗаявкаНаВозврат.ОформленоПоВозвратамКоличествоОборот) > 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	ПоДокументу = СтрЗаменить(ПоДокументу, "%Номер%", НомерДокумента);
	ПоДокументу = СтрЗаменить(ПоДокументу, "%Дата%", Формат(ДатаДокумента, "ДЛФ=D"));
	Товары.Загрузить(РезультатыЗапроса[РезультатыЗапроса.ВГраница()].Выгрузить());
	ТипДоверенности = Перечисления.ТипыВыданныхДоверенностей.НаПолучениеТоваров;
...
Процедура ЗаполнитьПоРеализацииТоваровУслуг(Знач ДокументОснование, ДанныеЗаполнения)
	...
	Если Выборка.Следующий() Тогда
		...
		|	И ТИПЗНАЧЕНИЯ(ТаблицаТовары.ЗаказКлиента) <> ТИП(Документ.ОтгрузкаТоваровКлиенту)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|		(ВЫБРАТЬ
		|			ТаблицаТоваровРеализации.ЗаказКлиента
		|		ИЗ
		|			Документ.РеализацияТоваровУслуг.Товары КАК ТаблицаТоваровРеализации
		|		ГДЕ
		|			ТаблицаТоваровРеализации.Ссылка = &Ссылка
		|			И ТаблицаТоваровРеализации.Ссылка.РеализацияПоЗаказам)
