...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьПараметрыВыбораПартнераКонтрагентаИЗаголовкиПолей();
	УстановитьВидимость();
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ЗаполнитьСлужебныеРеквизитыТаблицыРасчетыСПартнерами();
...
Процедура УстановитьУсловноеОформление()
	...
	ХозОперации.Добавить(Перечисления.ХозяйственныеОперации.ВводОстатковПремийПоставщиков);
...
&НаСервере
Процедура ЗаполнитьСлужебныеРеквизитыТаблицыРасчетыСПартнерами()
...
Процедура УстановитьПараметрыВыбораРасчетногоДокумента(ИмяЭлементаРасчетныйДокумент, СтрокаТаблицы, СозданиеНового = Ложь)
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВводОстатковПремийКлиентам") Тогда
		...
		МассивПараметровРасчетногоДокумента.Добавить(Новый ПараметрВыбора("Отбор.ТипПервичногоДокумента", Новый ФиксированныйМассив(МассивТиповПервичныхДокументов)));
	ИначеЕсли Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВводОстатковПремийПоставщиков") Тогда
		МассивТиповПервичныхДокументов.Добавить(ПредопределенноеЗначение("Перечисление.ТипыПервичныхДокументов.АктПремииПоставщика"));
		МассивПараметровРасчетногоДокумента.Добавить(Новый ПараметрВыбора("Отбор.ТипПервичногоДокумента", Новый ФиксированныйМассив(МассивТиповПервичныхДокументов)));
	КонецЕсли;
...
&НаСервере
Процедура УстановитьПараметрыВыбораПартнераКонтрагентаИЗаголовкиПолей()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов") Тогда
		...
		Элементы.РасчетыСПартнерамиКонтрагент.СвязиПараметровВыбора = Новый ФиксированныйМассив(Новый Массив);
...
Процедура ОбъектРасчетовСозданиеОбработкаВыбораТипа(Результат, ДополнительныеПараметры)Экспорт 
	Если Результат <> Неопределено Тогда
		...
		КонецЕсли;
		ОткрытьФорму(ИмяОбъектаМетаданных + ".ФормаОбъекта", СтруктураПараметров, Элементы.РасчетыСПартнерамиОбъектРасчетовОтправитель);
	КонецЕсли;
КонецПроцедуры
...
Функция ОбъектСПорядкомРасчетов(Ссылка)
	...
	Возврат ТипЗнч(Ссылка) <> Тип("ДокументСсылка.АвансовыйОтчет") 
		И ТипЗнч(Ссылка) <> Тип("ДокументСсылка.ОперацияСЭлектроннымБилетом")
