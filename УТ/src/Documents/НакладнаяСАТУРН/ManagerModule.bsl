...
Функция ИсходящаяНакладнаяJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	Для Каждого СтрокаТаблицы Из ВсеТовары Цикл
		...
		Если ДобавленнаяСтрокаПоПартии <> Неопределено Тогда
			...
			ДобавленнаяСтрокаПоПартии.КоличествоСАТУРН = ДобавленнаяСтрокаПоПартии.КоличествоСАТУРН + СтрокаТаблицы.КоличествоСАТУРН;
			Продолжить;
		КонецЕсли;
	...
	Если ШтрихкодыEAN.Количество() Тогда
		...
		Для Каждого СтрокаТаблицы Из СтрокиКОпределениюНоменклатуры Цикл
			...
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				...
				Если Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияСАТУРН(НайденнаяСтрока.ВидПродукции) Тогда
					Продолжить;
				КонецЕсли;
				
				ЗаполнитьЗначенияСвойств(СтрокаТаблицы, НайденнаяСтрока, "Номенклатура, Характеристика");
...
Процедура ЗаполнитьДокументПоДаннымСАТУРН(ДокументОбъект, ЭлементДанных, ЭлементОчереди, ПараметрыОбмена, ИзмененныеОбъекты)
	...
	ИначеЕсли Не ЭтоНовый Тогда
		ДокументОбъект.ДатаВозврата = Дата(1,1,1);
	КонецЕсли;
