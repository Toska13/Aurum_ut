...
Функция ИменаРеквизитовДляЗаполненияПараметровУказанияСерий() Экспорт
	ИменаРеквизитов = "Склад,Помещение,Статус,СкладскаяОперация,ДвижениеРанееПоступившейПартии,Дата";
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	#Иначе
		ПараметрыУказанияСерий = МобильноеРабочееМестоКладовщика.НоменклатураКлиентСерверПараметрыУказанияСерий();
		
		ПараметрыУказанияСерий.ИспользоватьАдресноеХранение = МобильноеРабочееМестоКладовщика.ИспользоватьАдресноеХранение(Объект.Склад, Объект.Помещение, Объект.Дата);
	#КонецЕсли
	...
	Если Объект.ДвижениеРанееПоступившейПартии Тогда
...
// 
// Параметры:
//  Объект - ДокументОбъект.ПриходныйОрдерНаТовары - объект.
// 
// Возвращаемое значение:
...
Функция КонтрольКачестваПараметрыДействий(Объект) Экспорт
	...
	ПоддерживаетсяКонтроль = КонтрольКачества.ПоддерживаетсяКонтрольКачества(Объект.Распоряжение);
	...
	Если Не Объект.ДвижениеРанееПоступившейПартии И ПоддерживаетсяКонтроль И ТребуетсяДоговор Тогда
		...
		Если ЗначениеЗаполнено(Объект.КонтролируемаяПартия) Тогда
			...
			Запрос.Текст =
				"ВЫБРАТЬ
				|	КонтролируемаяПартия.Договор КАК Договор
				|ИЗ
				|Справочник.КонтролируемыеПартии КАК КонтролируемаяПартия
				|ГДЕ
				|	КонтролируемаяПартия.Ссылка = &КонтролируемаяПартия";
		...
		Иначе
			...
			МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоСсылке(Объект.Распоряжение);
			РеквизитыПартии = МенеджерОбъекта.РеквизитыКонтролируемойПартии(Объект.Распоряжение, "КонтролируемаяПартия");
			Договор = РеквизитыПартии.Договор;
	...
	ПараметрыДействия.ЧитаемыеПоля = "Номенклатура,ЭтоУпаковочныйЛист";
	ПараметрыДействия.ИзменяемыеПоля = "ПроконтролироватьКачество,ДатаПодтвержденияКачества";
	ПараметрыДействия.ПараметрыЗапроса.Вставить("КонтрольКачестваСклад", Объект.Склад);
	ПараметрыДействия.ПараметрыЗапроса.Вставить("КонтрольКачестваПоддерживаетсяВРаспоряжении", ПоддерживаетсяКонтроль);
	ПараметрыДействия.ПараметрыЗапроса.Вставить("КонтрольКачестваДвижениеРанееПоступившейПартии",
		Объект.ДвижениеРанееПоступившейПартии);
	ПараметрыДействия.ПараметрыЗапроса.КонтрольКачестваДоговор = Договор;
	...
	ПараметрыДействия.ТекстЗапросаДанныеДокумента =
		...
		|	&КонтрольКачестваДвижениеРанееПоступившейПартии КАК Установить,
		|	Таблица.ЭтоУпаковочныйЛист ИЛИ НЕ &КонтрольКачестваПоддерживаетсяВРаспоряжении КАК Сбросить
	...
	ПараметрыДействия = КонтрольКачества.НовыйПараметрыДействия(ИмяДействия);
	ПараметрыДействия.ПолноеИмяОбъекта = "Документ.ПриходныйОрдерНаТовары";
	ПараметрыДействия.ЧитаемыеПоля = "Номенклатура,ЭтоУпаковочныйЛист";
	ПараметрыДействия.ИзменяемыеПоля = "ДатаПодтвержденияКачества";
	ПараметрыДействия.ПараметрыЗапроса.КонтрольКачестваДатаРазблокировкиЕслиНетДатыПоступленияПартии = ДатаПоступления;
	ПараметрыДействия.ПараметрыЗапроса.Вставить("КонтрольКачестваКонтролируемаяПартия", Объект.КонтролируемаяПартия);
	ПараметрыДействия.КлючевыеРеквизитыШапки.Вставить("КонтрольКачестваДвижениеРанееПоступившейПартии",
		Объект.ДвижениеРанееПоступившейПартии);
	...
	ПараметрыДействия.ТекстЗапросаДанныеДокумента =
		...
		|	ЛОЖЬ КАК Установить,
		|	Таблица.ЭтоУпаковочныйЛист КАК Сбросить
...
Процедура ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества(ТекущаяСтрока, ДанныеДляОбработки, ПараметрыДействия, ЗаполнитьШтатно) Экспорт
	...
	Если ПараметрыДействия.КлючевыеРеквизитыШапки.КонтрольКачестваДвижениеРанееПоступившейПартии Тогда
		
		ТекущаяСтрока.ДатаПодтвержденияКачества = ДанныеДляОбработки.ДатаПодтвержденияКачества;
		// ПодлежитРазблокировке - нет в табличной части.
		// КонтролируемаяПартия - нет в табличной части.
		
	Иначе
		
		// Заполняется действием ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества.
...
Процедура ТекстЗапросаДанныеДокументаШапка(СписокЗапросов)
	...
	ТекстЗапросаШапка = 
		...
		|	ДанныеДокумента.ДвижениеРанееПоступившейПартии КАК ДвижениеРанееПоступившейПартии,
		|	ДанныеДокумента.ПодлежитРазблокировке КАК ПодлежитРазблокировке,
...
Процедура ТекстЗапросаДанныеДокументаТаблицаТовары(СписокЗапросов)
	...
	|	ТаблицаТовары.ДатаПодтвержденияКачества КАК ДатаПодтвержденияКачества,
...
Процедура ПринятьТоварНаСклад(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаДокумента = 
		...
		|	ДанныеДокументаШапка.Распоряжение КАК Распоряжение,
		|	ТаблицаТовары.ПроконтролироватьКачество КАК ПроконтролироватьКачество,
		|	ДанныеДокументаШапка.ДвижениеРанееПоступившейПартии КАК ДвижениеРанееПоступившейПартии,
		|	ДанныеДокументаШапка.КонтролируемаяПартия КАК КонтролируемаяПартия,
		|	ДанныеДокументаШапка.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
		|	ТаблицаТовары.Номенклатура КАК Номенклатура,
		|	ТаблицаТовары.Характеристика КАК Характеристика,
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	ТекстЗапросаТабЧасть =
		...
		|	ДанныеДокументаШапка.ПодлежитРазблокировке КАК ПодлежитРазблокировке,
		...
		|	ВЫБОР
		|		КОГДА ДанныеДокументаШапка.ДвижениеРанееПоступившейПартии
		|			ТОГДА ДанныеДокументаШапка.КонтролируемаяПартия
		|		ИНАЧЕ ДанныеДокументаШапка.Распоряжение
...
Функция ПечатьПриходногоОрдераНаТовары(МассивОбъектов, ОбъектыПечати)
	...
	НоменклатураЛокализация.ЗаполнитьТекстЗапросаРеквизитыНоменклатуры("ПриходныйОрдерНаТоварыТовары.Номенклатура", ТекстыЗапросов);
	Если ИспользуетсяМаркируемаяПродукция Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ЛОЖЬ КАК МаркируемаяПродукция", ТекстыЗапросов.РеквизитыНоменклатурыВыборка);
	КонецЕсли; 
	Запрос.Текст = СтрЗаменить(Запрос.Текст, ", ТаблицаСоединения", ТекстыЗапросов.РеквизитыНоменклатурыСоединения);
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Пока ВыборкаПоСтрокамТЧ.Следующий() Цикл
			...
			Если ВыводитьУпаковки Тогда
				...
				Если ВыборкаПоСтрокамТЧ.ЭтоУпаковочныйЛист Тогда
					ОбластьСтрокаТаблицыКолонкаУпаковок.Параметры.ПредставлениеЕдиницыИзмеренияУпаковки = НСтр("ru = 'место'", ОбщегоНазначения.КодОсновногоЯзыка());
				КонецЕсли;
...
Функция ПолучитьДанныеДляПечатнойФормыОтборРазмещениеТоваров(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	ТекстыЗапросов = НоменклатураЛокализация.ТекстыЗапросаРеквизитовНоменклатуры();
	НоменклатураЛокализация.ЗаполнитьТекстЗапросаРеквизитыНоменклатуры("ТоварыДокумента.Номенклатура", ТекстыЗапросов);
	Если ИспользуетсяМаркируемаяПродукция Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ЛОЖЬ КАК МаркируемаяПродукция", ТекстыЗапросов.РеквизитыНоменклатурыВыборка);
	КонецЕсли; 
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&РеквизитыНоменклатурыГруппировка", ТекстыЗапросов.РеквизитыНоменклатурыГруппировка);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ", ТаблицаСоединения", ТекстыЗапросов.РеквизитыНоменклатурыСоединения);
...
Функция ТаблицаСерийПоОтгрузкеПриПеремещении(Объект)
	...
	СкладОтправитель		= ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Распоряжение, "СкладОтправитель");
	ВыгружаемыеКолонки		= "Номенклатура, Характеристика, Номенклатура, Назначение, Упаковка, Количество,
								|КоличествоУпаковок, ЭтоУпаковочныйЛист";
	...
	Если ОдноНазначениеПоТовару
		...
		|	ТаблицаТовары.Серия				КАК Серия,
		|	СУММА(ТаблицаТовары.Количество)	КАК Количество
		|ПОМЕСТИТЬ ТоварыКОтгрузкеПоРаспоряжениям
		|ИЗ
		|	Документ.РасходныйОрдерНаТовары.ТоварыПоРаспоряжениям КАК ТаблицаТовары
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РасходныйОрдерНаТовары КАК ДанныеШапки
		|		ПО ТаблицаТовары.Ссылка = ДанныеШапки.Ссылка
		|ГДЕ
		|	ДанныеШапки.Проведен
		|	И ТаблицаТовары.Распоряжение В(&Распоряжение)
		|	И ДанныеШапки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыРасходныхОрдеров.Отгружен)
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаТовары.Ссылка,
		...
		|			ИЗ
		|				ТоварыКОтгрузкеПоРаспоряжениям КАК ТоварыПоРаспоряжениям)
		|	И НЕ ТаблицаТовары.ЭтоУпаковочныйЛист
		...
		|ИЗ
		|	ВТОтгружаемыеТовары КАК ТаблицаТовары
		|ГДЕ
		|	(ТаблицаТовары.Ссылка, ТаблицаТовары.Номенклатура, ТаблицаТовары.Характеристика) В
		|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
		|				ОтгружаемыеТовары.Ссылка			КАК Ссылка,
		|				ОтгружаемыеТовары.Номенклатура		КАК Номенклатура,
		|				ОтгружаемыеТовары.Характеристика	КАК Характеристика
		|			ИЗ
		|				ВТОтгружаемыеТовары КАК ОтгружаемыеТовары
		|					ЛЕВОЕ СОЕДИНЕНИЕ ТоварыКОтгрузкеПоРаспоряжениям КАК ТоварыПоРаспоряжениям
		...
		|			ГДЕ
		|				ЕСТЬNULL(ТоварыПоРаспоряжениям.Серия, ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка))
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ТаблицаТовары.Серия				КАК Серия,
		|	ВЫБОР
		|		КОГДА ТаблицаТовары.Количество > ТоварыПоРаспоряжениям.Количество
		|			ТОГДА ТоварыПоРаспоряжениям.Количество
		|		ИНАЧЕ ТаблицаТовары.Количество
		|	КОНЕЦ							КАК Количество
		|ИЗ
		|	ВТОтгружаемыеТовары КАК ТаблицаТовары
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТоварыКОтгрузкеПоРаспоряжениям КАК ТоварыПоРаспоряжениям
		|		ПО ТаблицаТовары.Ссылка = ТоварыПоРаспоряжениям.Ссылка
		|			И ТаблицаТовары.Номенклатура = ТоварыПоРаспоряжениям.Номенклатура
		|			И ТаблицаТовары.Характеристика = ТоварыПоРаспоряжениям.Характеристика
		|			И ТаблицаТовары.Серия = ТоварыПоРаспоряжениям.Серия
		|			И ТоварыПоРаспоряжениям.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Ссылка,
		|	Номенклатура,
		...
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТаблицаТовары.Распоряжение КАК Распоряжение
		|ПОМЕСТИТЬ РаспоряженияНаОтгрузку
		|ИЗ
		|	Документ.РасходныйОрдерНаТовары.ТоварыПоРаспоряжениям КАК ТаблицаТовары
		|ГДЕ
		|	(ТаблицаТовары.Ссылка, ТаблицаТовары.Номенклатура, ТаблицаТовары.Характеристика) В
		...
		|	И (ТаблицаТовары.Распоряжение ССЫЛКА Документ.ПеремещениеТоваров
		|		ИЛИ ТаблицаТовары.Распоряжение ССЫЛКА Документ.ЗаказНаПеремещение)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Распоряжение
		|;
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПриходныйОрдерНаТовары КАК ДанныеШапки
		|		ПО ТоварыОрдера.Ссылка = ДанныеШапки.Ссылка
		|ГДЕ
		|	ДанныеШапки.Проведен
		|	И ДанныеШапки.Ссылка <> &ТекущийПриходныйОрдер
		|	И ДанныеШапки.Распоряжение В
		|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	И (ТоварыОрдера.Номенклатура, ТоварыОрдера.Характеристика) В
		|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
		|				ОтгружаемыеТовары.Номенклатура КАК Номенклатура,
		|				ОтгружаемыеТовары.Характеристика КАК Характеристика
		|			ИЗ
		...
		|		ТоварыКПоступлению.Характеристика	КАК Характеристика,
		|		ТоварыКПоступлению.Серия			КАК Серия,
		|		ТоварыКПоступлению.КОформлениюПоступленийПоОрдерам КАК Количество
		|	ИЗ
		|		РегистрНакопления.ТоварыКПоступлению КАК ТоварыКПоступлению
		|	ГДЕ
		...
		|	ТоварыДокумента.Характеристика					КАК Характеристика,
		|	ВЫБОР
		|		КОГДА НазначенияТоваров.Назначение ЕСТЬ NULL
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|		ИНАЧЕ НазначенияТоваров.Назначение
		|	КОНЕЦ											КАК Назначение,
		|	ТоварыДокумента.Упаковка						КАК Упаковка,
		...
		|//////////////////////////////////////////////////////////////////////////////// 11
		|ВЫБРАТЬ
		|	ТаблицаСерии.ДатаОтгрузки	КАК ДатаОтгрузки,
		...
		|ИЗ
		|	ОтгружаемыеТовары КАК ТаблицаСерии
		|		ЛЕВОЕ СОЕДИНЕНИЕ НазначенияТоваров КАК НазначенияТоваров
		...
		|ГДЕ
		|	(ТаблицаСерии.Номенклатура, ТаблицаСерии.Характеристика, ТаблицаСерии.Серия) В
		|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
		|				ОтгружаемыеТовары.Номенклатура КАК Номенклатура,
		|				ОтгружаемыеТовары.Характеристика КАК Характеристика,
		|				ОтгружаемыеТовары.Серия КАК Серия
		|			ИЗ
		|				ОтгружаемыеТоварыССериями КАК ОтгружаемыеТовары)
		|
		|УПОРЯДОЧИТЬ ПО
		|	ДатаОтгрузки,
		...
		ТоварыДокумента = РезультатЗапроса[ГраницаРезультата - 3].Выгрузить();
		СерииПоРаспоряжениям = РезультатЗапроса[ГраницаРезультата - 2].Выгрузить();
		
		ОтгружаемыеСерииКРаспределению	= РезультатЗапроса[ГраницаРезультата - 1].Выгрузить();
		ОтгружаемыеСерииКВычитанию		= РезультатЗапроса[РезультатЗапроса.ВГраница()].Выгрузить();
		
		ТоварыДокумента.Индексы.Добавить("Номенклатура, Характеристика, Назначение");
		ОтгружаемыеСерииКВычитанию.Индексы.Добавить("Номенклатура, Характеристика, Назначение, Серия");
		
		Серии = ОтгружаемыеСерииКВычитанию.СкопироватьКолонки();
		ОтгружаемыеСерии = ТаблицаРаспределенныОтгружаемыхСерий(ОтгружаемыеСерииКРаспределению, ОтгружаемыеСерииКВычитанию);
		
		ЗаполнитьТаблицуСерийПеремещения(Серии, ОтгружаемыеСерии, ТоварыДокумента);
	...
	Иначе
		...
		ТоварыДокумента		= РезультатЗапроса[ГраницаРезультата - 1].Выгрузить();
		СерииПоРаспоряжению	= РезультатЗапроса[ГраницаРезультата].Выгрузить();
...
Функция ТаблицаРаспределенныОтгружаемыхСерий(ТаблицаКРаспределению, ТаблицаВычитания)
	
	ТаблицаСерий	= ТаблицаВычитания.СкопироватьКолонки();
	УдаляемыеСтроки	= Новый Массив;
	РеквизитыОтбора	= "Номенклатура, Характеристика, Назначение, Серия";
	ОтборСтрок		= Новый Структура(РеквизитыОтбора);
	
	Для Каждого РаспределяемаяСтрока Из ТаблицаКРаспределению Цикл
		УдаляемыеСтроки.Очистить();
		ЗаполнитьЗначенияСвойств(ОтборСтрок, РаспределяемаяСтрока);
		
		СтрокиКВычитанию = ТаблицаВычитания.НайтиСтроки(ОтборСтрок);
		
		Для Каждого ВычитаемаяСтрока Из СтрокиКВычитанию Цикл
			Количество = Мин(РаспределяемаяСтрока.Количество, ВычитаемаяСтрока.Количество);
			
			НоваяСтрока = ТаблицаСерий.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ВычитаемаяСтрока);
			НоваяСтрока.Количество			= Количество;
			НоваяСтрока.КоличествоУпаковок	= Количество;
			
			ВычитаемаяСтрока.Количество = ВычитаемаяСтрока.Количество - НоваяСтрока.Количество;
			
			Если ВычитаемаяСтрока.Количество = 0 Тогда
				УдаляемыеСтроки.Добавить(ВычитаемаяСтрока);
		...
		Для Каждого СтрокаКУдалению Из УдаляемыеСтроки Цикл
			ТаблицаВычитания.Удалить(СтрокаКУдалению);
		КонецЦикла;
	КонецЦикла;
	
	ТаблицаСерий.Свернуть("Номенклатура, Характеристика, Назначение, Серия", "Количество, КоличествоУпаковок");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "11.5.25.7";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("a07d2ef3-6091-4ea1-9e4e-8a145862fc1f");
	...
	СписокОписаний.Добавить(НСтр("ru = 'Обновление документов ""Приходный ордер на товары"":'"));
	...
	Изменяемые.Добавить(Метаданные.Документы.ПриходныйОрдерНаТовары.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.ПриходныйОрдерНаТовары";
	...
	|ИЗ
	|	Документ.ПриходныйОрдерНаТовары КАК РеквизитыДокумента
	|ГДЕ
	|	ЛОЖЬ
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриходныйОрдерНаТовары КАК РеквизитыДокумента
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			Если ПричинаИсключения = 2 Тогда
				ВызватьИсключение НСтр("ru = 'Не удалось обработать обновление документов ""Приходный ордер на товары"".'");
			КонецЕсли;
