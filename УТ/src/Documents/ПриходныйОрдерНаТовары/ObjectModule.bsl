...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		ДанныеЗаполнения.Свойство("КонтролируемаяПартия",       КонтролируемаяПартия);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	КонтрольКачества.ЗаполнитьКонтролируемуюПартиюВПриходномОрдере(ЭтотОбъект);
...
Процедура ПриЗаписи(Отказ)
	...
	// Механизм "Контролируемые партии".
	КонтрольКачества.СоздатьКонтролируемуюПартиюВПриходномОрдере(ЭтотОбъект);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ДвижениеРанееПоступившейПартии Тогда
		
		КонтрольКачества.ОбработкаПроверкиЗаполненияДокументаИзлишков(
			ЭтотОбъект,
			МассивНепроверяемыхРеквизитов,
			Отказ,
			"Товары",
			НСтр("ru = 'Товары'"),
			ПодлежитРазблокировке,
			Ложь);
...
Процедура ЗаполнитьТоварыПоТоварамКПоступлению(ВидЗаполнения, ДатаПоступления) Экспорт
	...
	Если СкладскаяОперация = Перечисления.СкладскиеОперации.ПриемкаПоПеремещению Тогда
		...
		|	И (ТоварыКПоступлению.КонтролируемаяПартия = &КонтролируемаяПартия
		|			ИЛИ ТоварыКПоступлению.КонтролируемаяПартия = ЗНАЧЕНИЕ(Справочник.КонтролируемыеПартии.ПустаяСсылка))
		...
		Если ОдноНазначениеПоТовару
			...
			|	И РасходныйОрдер.КонтролируемаяПартия = &КонтролируемаяПартия
			...
			|		И ПриходныйОрдер.КонтролируемаяПартия = &КонтролируемаяПартия
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	ДвижениеРанееПоступившейПартии = Ложь;
	ПодлежитРазблокировке = Истина;
	...
	Если ЗначениеЗаполнено(КонтролируемаяПартия) Тогда
		...
		РеквизитыПартии = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			...
			"Партия,ПланироватьРазблокировкуТовара");
		УстановитьПривилегированныйРежим(Ложь);
		Если РеквизитыПартии.ПланироватьРазблокировкуТовара <> Неопределено Тогда // не битая ссылка
			ДвижениеРанееПоступившейПартии = РеквизитыПартии.Партия <> Распоряжение; // распоряжение перемешение, а партия не перемещение заблокированных товаров
			ПодлежитРазблокировке = РеквизитыПартии.ПланироватьРазблокировкуТовара;
		КонецЕсли;
	КонецЕсли;
