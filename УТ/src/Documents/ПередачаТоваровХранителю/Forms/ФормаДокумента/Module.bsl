...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ Локализация
	ИнтеграцияСМаркетплейсамиСервер.РазместитьНаФормеДополнительныеКомандыЭДО(ПараметрыЭДОПриСоздании);
	//-- Локализация
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "СверкаДокументаСТорговойПлощадкой_Исправление" 
			И Источник = Объект.Ссылка Тогда
		ОбновитьДокументПоДаннымТорговойПлощадки(Параметр);
		Активизировать();
	КонецЕсли;
...
Процедура ПередачаПоЗаказамПриИзменении(Элемент)
	...
	Если Объект.ПередачаПоЗаказам Тогда
		...
		Иначе
			...
			ОткрытьФорму("Документ.ПередачаТоваровХранителю.Форма.ФормаВыбораРаспоряжения", ПараметрыОткрытия, ЭтотОбъект, , , ,
...
Процедура ПроверитьКоличествоВДокументеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		ОбщегоНазначенияУТКлиент.ВывестиСообщениеПревышенияПоКоличеству(Результат, ТекстОснования);
...
Процедура ХозяйственнаяОперацияПриИзмененииСервер()
	...
	ИспользоватьПередачуПоНесколькимЗаказамДоИзменения = ИспользоватьПередачуПоНесколькимЗаказам;
	...
	Если Объект.ПередачаПоЗаказам
		Или ИспользоватьПередачуПоНесколькимЗаказамДоИзменения <> ИспользоватьПередачуПоНесколькимЗаказам Тогда
		Объект.ПередачаПоЗаказам = Ложь;
		ПередачаПоЗаказамПриИзмененииСервер();
	КонецЕсли;
	...
	УстановитьВидимостьЭлементаФормыТоварыНазначение();
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	ПараметрыЗаполненияВидаЦен.ПараметрыОтбора.ЦенаВключаетНДС = Ложь;
...
Процедура ОбработкаПодбораТоваровИзЗаказа(АдресТоваровВХранилище)
	...
	Результат = ПолучитьИзВременногоХранилища(АдресТоваровВХранилище); // ДокументТабличнаяЧасть.ПередачаТоваровХранителю.Товары
	
	ОбрабатываемыеСтроки = Новый Массив();
	ПодобранныеТовары = Результат.Товары;
	...
	ПродажиСерверЛокализация.ПеренестиШтрихкодыУпаковок(ЭтотОбъект, Результат.ШтрихкодыУпаковок);
	СобытияФорм.ПриИзмененииЭлемента(ЭтотОбъект, "Товары");
...
Процедура ПодборТоваровИзЗаказа()
	...
	ОткрытьФорму("Документ.ПередачаТоваровХранителю.Форма.ФормаПодбораТоваровИзЗаказа", ПараметрыПодбора, ЭтотОбъект, , , ,
...
&НаСервере
Процедура УстановитьВидимостьЭлементаФормыТоварыНазначение()
	
	ОтобразитьЭлемент = Объект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию
		И Объект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ПередачаНаХранениеСПравомПродажи;
	
	Элементы.ТоварыНазначение.Видимость = ОтобразитьЭлемент;
...
Процедура ЗаполнитьПоЗаказуСервер()
	...
	Документы.ПередачаТоваровХранителю.ЗаполнитьПоОстаткамЗаказов(
		Объект, Объект.Товары, Склад, МассивЗаказов, ПараметрыЗаполнения, Объект.ШтрихкодыУпаковок);
...
Процедура ПривязатьСтрокиПередачиКЗаказам(ОтобратьПоЗаказу)
	...
	Для Каждого ТекСтрока Из Объект.Товары Цикл
		...
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ТекСтрока.ЗаказКлиента) И ОтобратьПоЗаказу Тогда
...
Процедура ЗакрытьЗаказФрагмент()
	...
	ОткрытьФорму("Обработка.ПомощникЗакрытияЗаказов.Форма.ФормаЗакрытия", СтруктураЗакрытия, ЭтотОбъект, , , , Неопределено,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура УстановитьВозможностьРедактированияНД()
	...
	ПараметрыДоступностиНД.НакладнаяПоЗаказу = Объект.ПередачаПоЗаказам;
...
// ИнтеграцияСМаркетплейсами
&НаСервере
Процедура ОбновитьДокументПоДаннымТорговойПлощадки(Знач АдресХранилищаРасхождений)
	
	Если Не ЭтоАдресВременногоХранилища(АдресХранилищаРасхождений) Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицыЗначений Из РасхожденияРезультат.ШтрихкодыУпаковокРасхождения Цикл
		Если СтрокаТаблицыЗначений.Количество > 0
			...
			СтрокаТабличнойЧасти = Объект.ШтрихкодыУпаковок.Добавить();
