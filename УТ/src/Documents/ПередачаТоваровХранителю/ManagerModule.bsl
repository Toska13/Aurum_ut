...
//
Процедура ЗаполнитьПоОстаткамЗаказов(ДанныеОтбора, Товары, СкладПередачи, МассивЗаказов, ПараметрыЗаполнения, ШтрихкодыУпаковок) Экспорт
	...
	Если РезультатЗапросаПоОстаткамЗаказов[9].Пустой()
		И РезультатЗапросаПоОстаткамЗаказов[10].Пустой()
	...
	ВыборкаТовары = РезультатЗапросаПоОстаткамЗаказов[9].Выбрать();
	...
	Если ЗначениеЗаполнено(ДанныеОтбора.Ссылка) Тогда
		МассивЗаказовКлиентов.Добавить(ДанныеОтбора.Ссылка);
	КонецЕсли;
	ПередачаТоваровХранителюЛокализация.СформироватьОтгруженныеМарки(МассивЗаказовКлиентов, ОтгруженныеМарки);
	ПередачаТоваровХранителюЛокализация.ИсключитьРеализованныеМарки(МассивЗаказовКлиентов, ОтгруженныеМарки);
	ШтрихкодыУпаковок.Очистить();
	Если ОтгруженныеМарки <> Неопределено Тогда
		Для Каждого СтрокаТаблицы Из ОтгруженныеМарки Цикл
			ЗаполнитьЗначенияСвойств(ШтрихкодыУпаковок.Добавить(), СтрокаТаблицы);
		КонецЦикла;
	КонецЕсли;
	
	ЗаполнитьЦенуПродажи = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияЦеныВСтрокеТЧ(ДанныеОтбора);
...
Процедура ОформитьОтгрузкуТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	ПараметрыМодульногоПроведения.ОбъектМетаданных = Метаданные.Документы.ПередачаТоваровХранителю;
	
	СкладыСервер.ОформитьОтгрузкуТоваровПоНастройкеПараметров(Запрос,
																ТекстыЗапроса,
																Регистры,
																ТекстЗапросаДанныхДокумента,
...
Процедура ПриОпределенииКомандыПечатиКомплектаДокументов(Команда) Экспорт
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, 
	...
	Иначе
		...
		ТипКоличества = ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля();
	...
	|		ВЫБОР
	|			КОГДА РаспоряженияДвижения.Показатель = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.КОформлению)
	|				ТОГДА -РаспоряженияДвижения.Сумма
	|			ИНАЧЕ РаспоряженияДвижения.Сумма
	|		КОНЕЦ КАК Сумма
	|	ИЗ
	...
	|						СписокЗаказов.Ссылка
	|					ИЗ
	|						ВТСписокЗаказов КАК СписокЗаказов)
	|
	...
	|		РаспоряженияОбороты.Характеристика     КАК Характеристика,
	|		РаспоряженияОбороты.Серия              КАК Серия,
	|		-РаспоряженияОбороты.ОформленоКоличествоОборот КАК Количество,
	...
	|	ГДЕ
	|		РаспоряженияОбороты.Регистратор <> &Регистратор
	|	) КАК ТаблицаЗаказы
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаЗаказы.ЗаказКлиента,
	...
	ПередачаТоваровХранителюЛокализация.ОтработатьОграниченияПоПередачеНаОтветственноеХранение(ТекстЗапроса);
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	КонецЕсли;
		...
		|	ИЗ
		|		РегистрСведений.РаспоряженияНаОтгрузкуИВозвратКВыполнению КАК ТаблицаДанных
		|		
		|	ГДЕ
...
Функция ТекстЗапросаОстаткиТоваровКОформлению(ИмяВременнойТаблицы, ОтборПоИзмерениям = Неопределено) Экспорт
	...
	ТекстЗапроса =
		РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат.ТекстЗапросаОстатки(
			ИмяВременнойТаблицы,
			ОтборПоИзмерениям,
			"КОформлению > 0");
...
Процедура ПривязатьСтрокиПередачиКЗаказам(Форма, Объект, ПараметрыУказанияСерий, ОтобратьПоЗаказу) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Форма.ВидЦенУчетный) Тогда
