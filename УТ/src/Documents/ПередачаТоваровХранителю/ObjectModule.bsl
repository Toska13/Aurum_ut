		...
		ИначеЕсли ДанныеЗаполнения.Свойство("АктОРасхождениях")
			...
			Если ТипЗнч(ДанныеЗаполнения.ОснованиеАкта) = Тип("ДокументСсылка.ПередачаТоваровХранителю") Тогда
				...
				Если ДанныеЗаполнения.Свойство("Исправление") Тогда
					...
					КорректировкаИсправленияДокументаНаОснованииАктаОРасхожденияхПослеОтгрузки(ДанныеЗаполнения);
				Иначе
...
Процедура ЗаполнитьДокументНаОснованииЗаказаКлиента(Знач ДокументОснование,
	...
	Если ЗаполнятьПоОстаткам Тогда
		...
		Документы.ПередачаТоваровХранителю.ЗаполнитьПоОстаткамЗаказов(
			ЭтотОбъект, Товары, СкладПередачи, МассивЗаказов, ПараметрыЗаполнения, ШтрихкодыУпаковок);
...
Процедура КорректировкаИсправленияДокументаНаОснованииАктаОРасхожденияхПослеОтгрузки(Знач ДанныеЗаполнения)
	...
	Если ПередачаПоЗаказам Тогда
		КолонкиОтбора = "КодСтроки, ЗаказКлиента, " + КолонкиОтбора;
	КонецЕсли;
...
Процедура ЗаполнитьДокументНаОснованииАктаОРасхожденияхПослеПриемки(Знач ДанныеЗаполнения)
	...
	|	И ПоступлениеОтХранителя.Ссылка = &ДокументОснованиеИсправлениеОснования
	|;
	|
	|//////////////////////////////////////////////////////////////////////////////// 1
	|ВЫБРАТЬ
...
Процедура ЗаполнитьДокументНаОснованииИндивидуальногоСоглашенияСКлиентом(Знач ДокументОснование)
	...
	|	СоглашениеСКлиентом.КомиссионныеПродажи25 КАК КомиссионныеПродажи25
	|ИЗ
	...
	ПараметрыПроверкиВозможностиВводаНаОсновании = ОбщегоНазначенияУТ.ПараметрыПроверкиВозможностиВводаНаОсновании();
	ПараметрыПроверкиВозможностиВводаНаОсновании.Статус = Выборка.СтатусДокумента;
	ПараметрыПроверкиВозможностиВводаНаОсновании.ЕстьОшибкиСтатус = Выборка.ЕстьОшибкиСтатус;
	ПараметрыПроверкиВозможностиВводаНаОсновании.МассивДопустимыхСтатусов = МассивДопустимыхСтатусов;
	
	ОбщегоНазначенияУТ.ПроверитьВозможностьВводаНаОснованииСПараметрами(Выборка.Соглашение, ПараметрыПроверкиВозможностиВводаНаОсновании);
	
	Если Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию
			И Не Выборка.КомиссионныеПродажи25 Тогда
		
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Нельзя выполнить ввод документа ""Передача на комиссию"" на основании соглашения с операцией ""%1"" по комиссионной схеме %2.
											|Необходимо использовать документ ""%3"".'"),
			Выборка.ХозяйственнаяОперация,
...
Процедура ЗаполнитьЦеныПоУсловиямПродаж()
	...
	Если ПродажиСервер.ЭтоСоглашениеПоНовойКомиссии(Соглашение) Тогда
		ПараметрыЗаполнения.Вставить("ВидЦены", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "ВидЦенУчетный"));
	КонецЕсли;
