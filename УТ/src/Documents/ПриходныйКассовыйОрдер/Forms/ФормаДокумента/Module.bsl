...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если КлиентскоеПриложение.ТекущийВариантИнтерфейса() = ВариантИнтерфейсаКлиентскогоПриложения.Версия8_2 Тогда
		Элементы.КассаОтправитель.КнопкаВыбора = Истина;
	КонецЕсли;
...
Процедура ХозяйственнаяОперацияПриИзмененииСервер()
	...
	Если Не ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ОчиститьДополнительныеСведенияПоступления();
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Объект.Касса) Тогда
		...
		Если ИзмениласьОрганизация Тогда
			...
			Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзКассыККМ Тогда
				Объект.КассаККМ = Справочники.КассыККМ.КассаККМПоУмолчанию(Объект.Организация);
			КонецЕсли;
...
Процедура КассаОтправительПриИзмененииСервер()
	...
	ИсходнаяОрганизацияПлательщик = ОрганизацияПлательщик;
	
	ЗаполнитьОрганизациюПлательщик();
	
	Если Не ЗначениеЗаполнено(Объект.КассаОтправитель)
		И ОрганизацияПлательщик <> ИсходнаяОрганизацияПлательщик Тогда
		ОчиститьДополнительныеСведенияПоступления();
	КонецЕсли;
...
Процедура РасшифровкаПлатежаОснованиеПлатежаОбработкаВыбораНаСервере(ВыбранноеЗначение)
	...
	РасшифровкаПлатежаОбъектРасчетовПриИзмененииСервер(ИдентификаторСтроки);
...
Процедура ДополнитьВыбранноеЗначение(ВыбранноеЗначение)
	...
	ВыбранноеЗначение.Вставить("СтатьяДвиженияДенежныхСредств",
		ДенежныеСредстваСервер.СтатьяДвиженияДенежныхСредствОбъектаРасчетов(ВыбранноеЗначение.ОбъектРасчетов,
			Объект.ХозяйственнаяОперация));
...
Процедура УправлениеЭлементамиФормы()
	...
	МассивРеквизитовОперации = Новый Массив;
	Документы.ПриходныйКассовыйОрдер.ЗаполнитьИменаРеквизитовПоХозяйственнойОперации(
		Объект.ХозяйственнаяОперация, Неопределено, МассивРеквизитовОперации);
	...
	Тогда
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитов.Организация) Тогда
			Если ЗначениеЗаполнено(ОрганизацияПлательщик)
				...
				ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтотОбъект, "КассаОтправитель.Владелец");
			КонецЕсли;
			ОрганизацияПлательщик = СтруктураРеквизитов.Организация;
		КонецЕсли;
	Иначе
		ОрганизацияПлательщик = Неопределено;
	КонецЕсли;
...
Процедура УстановитьВозможныеЗначенияКассыОтправителя(ИзменятьКассуОтправителя = Истина)
	...
	Если ИзменятьКассуОтправителя Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(Объект.КассаОтправитель)
...
&НаСервере
Процедура ОчиститьДополнительныеСведенияПоступления()

	Объект.ПринятоОт = "";
	Объект.Основание = "";
	Объект.Приложение = "";
	Объект.ДоверенностьВыданная = Документы.ДоверенностьВыданная.ПустаяСсылка();
