...
//
// Параметры:
//	ЧекККМ - ДокументСсылка.ЧекККМ - Чек ККМ, по которому нужно определить наличие чеков коррекциии
//	ТолькоИсправлениеОшибки - Булево - Истина, поиск только видов коррекции "Исправление ошибки"
// Возвращаемое значение:
//	Булево - Есть чеки коррекции по переданному чеку ККМ
Функция ЕстьКоррекцияПоЧекуККМ(ЧекККМ, ТолькоИсправлениеОшибки = Ложь) Экспорт
	...
	|   И (&ВсеВидыЧековКоррекции
	|		ИЛИ ЧекККМКоррекции.ВидКоррекции = ЗНАЧЕНИЕ(Перечисление.ВидыЧековКоррекции.ИсправлениеОшибки))
	|	И (ФискальныеОперации.НомерЧекаККМ <> &ПустаяСтрока)";
	Запрос.УстановитьПараметр("ЧекККМ", ЧекККМ);
	Запрос.УстановитьПараметр("ВсеВидыЧековКоррекции", Не ТолькоИсправлениеОшибки);
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Запрос.УстановитьПараметр("Ссылка",                                   ДокументСсылка);
	Запрос.УстановитьПараметр("ВидКоррекции",                             Реквизиты.ВидКоррекции);
...
Процедура ОформитьОтгрузкуВРозницу(Запрос, ТекстыЗапроса, Регистры)
	...
	|	И &ВидКоррекции = ЗНАЧЕНИЕ(Перечисление.ВидыЧековКоррекции.ИсправлениеОшибки)";
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|	ВЫБОР
		|		КОГДА ТоварыЧека.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
		|			ТОГДА &Подразделение
		|		ИНАЧЕ ТоварыЧека.Ссылка.Склад
		|	КОНЕЦ КАК Склад,
