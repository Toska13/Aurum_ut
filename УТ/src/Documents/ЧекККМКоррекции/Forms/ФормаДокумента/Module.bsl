...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НастроитьВидКоррекции();
	
	Элементы.Товары.ТолькоПросмотр = Не (Объект.ВидКоррекции = Перечисления.ВидыЧековКоррекции.ИсправлениеОшибки);
	Элементы.ТоварыПомещение.Видимость = СкладыСервер.ИспользоватьСкладскиеПомещения(Объект.Склад,Объект.Дата);
	Элементы.СтраницаРасхождения.Видимость = (Объект.ВидКоррекции = Перечисления.ВидыЧековКоррекции.ИсправлениеОшибки);
...
Процедура ВидКоррекцииПриИзменении(Элемент)
	...
	Если Объект.ВидКоррекции = ПредопределенноеЗначение("Перечисление.ВидыЧековКоррекции.НеприменениеККТ") Тогда
		Объект.ОписаниеКоррекции = НСтр("ru = 'Неприменение ККТ'"); 
	Иначе
		Объект.ОписаниеКоррекции = "";
	КонецЕсли;
	
	Элементы.Товары.ТолькоПросмотр =
		Не (Объект.ВидКоррекции = ПредопределенноеЗначение("Перечисление.ВидыЧековКоррекции.ИсправлениеОшибки"));
	Элементы.СтраницаРасхождения.Видимость =
		(Объект.ВидКоррекции = ПредопределенноеЗначение("Перечисление.ВидыЧековКоррекции.ИсправлениеОшибки"));
...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Объект.ВидКоррекции = ПредопределенноеЗначение("Перечисление.ВидыЧековКоррекции.ИсправлениеОшибки")
		И (Поле = Элементы.ТоварыСтатусУказанияСерий Или Поле = Элементы.ТоварыСерия) Тогда
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	Иначе	
		...
		ПакетнаяОбработкаТабличнойЧастиКлиентСерверЛокализация.ДополнитьСтруктуруДействийПриИзмененииЭлемента(ЭтотОбъект, Элементы.ТоварыНоменклатура.Имя, СтруктураДействий);
...
Процедура ТоварыСтавкаНДСПриИзменении(Элемент)
	...
	СтруктураПараметровПересчета = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(Объект);
...
&НаКлиенте
Процедура ТоварыКомитентПриИзменении(Элемент)
	...
	Иначе
		...
		ТоварыКомитентПриИзмененииЛокализация(Элемент);
...
Процедура СформироватьНачальноеЗаполнениеЧекККМТовары()
	...
	|	ЧекККМТовары.Комитент КАК Комитент,
...
Процедура РассчитатьВнесенныеРасхождения()
	...
	КолонкиГруппировки = "НомерСтроки, Комитент, Номенклатура, ТипНоменклатуры, Характеристика, ХарактеристикиИспользуются, СтатусУказанияСерий, Серия, Упаковка, НоменклатураЕдиницаИзмерения, СтавкаНДС";
...
Функция РазрешеноИзменятьНоменклатуру(Номенклатура)
	...
	Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(ОписаниеНоменклатуры.КлючВидовПродукции, Истина)
		Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияЕГАИС(ОписаниеНоменклатуры.КлючВидовПродукции) Тогда
...
Функция ИзменяетсяНоменклатура(ЭлементИмя)
	...
	МассивЭлементовНоменклатуры.Добавить("Комитент");
...
Функция ПараметрыОперацииФискализацииЧекаНаСервере()
	...
	ПараметрыОперацииФискализацииЧека = ФормированиеПараметровФискальногоЧекаСервер
		.ПараметрыОперацииФискализацииЧека(Объект.Ссылка, Объект.Организация);
	...
	ФормированиеПараметровФискальногоЧекаСервер.ПараметрыФискальногоЧекаЗаполнитьПараметрыКоррекции(
		ПараметрыОперацииФискализацииЧека,
		?(Объект.ВидКоррекции = Перечисления.ВидыЧековКоррекции.ИсправлениеОшибки, Объект.ЧекККМ, Неопределено),
		Объект.ВидКоррекции,
		ДанныеКоррекции
	);
...
Процедура ПечатьЧека_Завершение(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если Объект.ВидКоррекции = ПредопределенноеЗначение("Перечисление.ВидыЧековКоррекции.НеприменениеККТ") Тогда
			АрхивироватьОтложенныйЧекККМ(Объект.ЧекККМ);
		КонецЕсли;
...
Процедура ЗаполнитьСлужебныеРеквизитыПоНоменклатуре()
	...
	ПакетнаяОбработкаТабличнойЧастиКлиентСерверЛокализация.ДополнитьСтруктуруДействийПриИзмененииЭлемента(ЭтотОбъект, "НоменклатураПризнаки", ПараметрыЗаполненияРеквизитов);
...
&НаСервере
Процедура НастроитьВидКоррекции()

	// Настройка отображения поля на форме
	ФискальнаяОперация = ОборудованиеЧекопечатающиеУстройстваВызовСервера.ДанныеФискальнойОперации(Объект.ЧекККМ);

	Элементы.ВидКоррекции.СписокВЫбора.Добавить(Перечисления.ВидыЧековКоррекции.ИсправлениеОшибки);
	Элементы.ВидКоррекции.СписокВЫбора.Добавить(Перечисления.ВидыЧековКоррекции.НеприменениеККТ);
	Элементы.ВидКоррекции.СписокВЫбора.Добавить(Перечисления.ВидыЧековКоррекции.БлокировкаЧекаВОФД);
	
	Если ФискальнаяОперация = Неопределено Тогда
		Элементы.ВидКоррекции.Доступность = Ложь;
	Иначе
		Элементы.ВидКоррекции.СписокВЫбора.Очистить();
		Элементы.ВидКоррекции.СписокВЫбора.Добавить(Перечисления.ВидыЧековКоррекции.ИсправлениеОшибки);
		Элементы.ВидКоррекции.СписокВЫбора.Добавить(Перечисления.ВидыЧековКоррекции.БлокировкаЧекаВОФД);
	КонецЕсли;

	// Установка значения по умолчанию
	Если Не ЗначениеЗаполнено(Объект.ВидКоррекции) Тогда
		Объект.ВидКоррекции = Перечисления.ВидыЧековКоррекции.ИсправлениеОшибки;
		Если ФискальнаяОперация = Неопределено Тогда
			Объект.ВидКоррекции = Перечисления.ВидыЧековКоррекции.НеприменениеККТ;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура АрхивироватьОтложенныйЧекККМ(ЧекККМ)
	
	ЧекККМОбъект = Объект.ЧекККМ.ПолучитьОбъект();
	ЧекККМОбъект.Архивный      = Ложь;
	ЧекККМОбъект.КассоваяСмена = Объект.КассоваяСмена;
	ЧекККМОбъект.Статус        = ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.Пробит");
	ЧекККМОбъект.НомерЧекаККМ  = Объект.НомерЧекаККМ;
...
&НаКлиенте
Процедура ТоварыКомитентПриИзмененииЛокализация(Элемент)
	...
	СтруктураЗаполненияДанныхПриУСНКомитентов = ПакетнаяОбработкаТабличнойЧастиКлиентСерверЛокализация.ПараметрыЗаполненияСобственникаТовараИСтавкиНДСПриУСНКомитентов(Объект, СтруктураСостояниеКассовойСмены.Подразделение);
	СтруктураЗаполненияДанныхПриУСНКомитентов.ПерезаполнитьСобственникаТовара = Ложь;
	СтруктураДействий.Вставить("ЗаполнитьСобственникаТовараИСтавкуНДСПриУСНКомитентов", СтруктураЗаполненияДанныхПриУСНКомитентов);
