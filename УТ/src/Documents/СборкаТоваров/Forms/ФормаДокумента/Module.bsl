...
Процедура СкладПриИзменении(Элемент)
	...
	ИзменилсяСклад = Склад <> Объект.Склад;
...
Процедура ПроконтролироватьКачествоПриИзменении(Элемент)
	...
	Если Не Объект.ПроконтролироватьКачество Тогда
...
Процедура УстановитьУсловноеОформление()
	...
	// Шапка.
	Поля = КонтрольКачества.Поля(
		Элементы.ПроконтролироватьКачество,
		Элементы.ДатаПодтвержденияКачества);
	КонтрольКачества.УстановитьОформление(УсловноеОформление, Поля);
...
Процедура ПерезаполнитьПоОрдерам(МассивЗаказов)
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РазборкаТоваров И ЗаполнятьПоРасходномуОрдеру
		...
		Документы.СборкаТоваров.ЗаполнитьПоЗаказамОрдерам(ТаблицаНакладнаяПоШапке, Объект.Ссылка, ПараметрыЗаполнения, Объект.ШтрихкодыУпаковок);
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СборкаТоваров И ЗаполнятьПоРасходномуОрдеру
		...
		ПараметрыЗаполнения.Ресурсы = Ресурсы;
		Документы.СборкаТоваров.ЗаполнитьПоЗаказамОрдерам(ТаблицаНакладная, Объект.Ссылка, ПараметрыЗаполнения, Объект.ШтрихкодыУпаковок);
...
Процедура ИзменитьТабличнуюЧастьПоРезультатамПроверки(ВозвращаемыеПараметры, КэшированныеЗначения)
	...
	СтруктураДействийИзмененныеСтроки.Вставить("ЗаполнитьПризнакАртикул",
		Новый Структура("Номенклатура", "Артикул"));
	СтруктураДействийИзмененныеСтроки.Вставить("ЗаполнитьПризнакТипНоменклатуры",
		Новый Структура("Номенклатура", "ТипНоменклатуры"));
	СтруктураДействийИзмененныеСтроки.Вставить("ЗаполнитьПризнакВедетсяУчетПоГТД",
		Новый Структура("Номенклатура", "ВедетсяУчетПоГТД"));
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействийИзмененныеСтроки);
	...
	СтруктураДействийДобавленныеСтроки.Вставить("ЗаполнитьПризнакВедетсяУчетПоГТД",
		Новый Структура("Номенклатура", "ВедетсяУчетПоГТД"));
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействийДобавленныеСтроки);
	СтруктураДействийДобавленныеСтроки.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества",
		КонтрольКачестваКлиентСервер.КэшПоКлючу(КонтрольКачестваКэш, "Товары"));
...
&НаСервере
Процедура УправлениеЭлементамиФормы(ЕстьИзменениеНоменклатуры = Ложь)
	...
	Элементы.ОблагаетсяАкцизом.Видимость = ИспользоватьПодакцизныеТовары;
	Элементы.ПодразделениеПроизводства.Видимость = ИспользоватьПодакцизныеТовары;
	УстановитьОформлениеПолейКонтроляКачества();
