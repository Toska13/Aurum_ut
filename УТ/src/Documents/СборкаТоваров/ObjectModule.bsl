...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ПрослеживаемыйКомплект
		...
		Если КонтролироватьНомераГТД
			Или ИспользоватьУчетПрослеживаемыхИмпортныхТоваров Тогда
			
			РеквизитыОсновнойКомпоненты = ОбщегоНазначенияУТ.ЗначенияРеквизитовОбъектаПоУмолчанию(
											НоменклатураОсновногоКомпонента,
											ИменаРеквизитов);
			
		Иначе
			РеквизитыОсновнойКомпоненты = Новый Структура(ИменаРеквизитов, Ложь, Ложь);
		КонецЕсли;
		
		Если КонтролироватьНомераГТД
			И РеквизитыОсновнойКомпоненты.ПрослеживаемыйТовар Тогда
			ТекстСообщения = НСтр("ru = 'Для непрослеживаемого комплекта в качестве основного компонента указана прослеживаемая по РНПТ комплектующая `%НазваниеТовара%`.
				|Укажите непрослеживаемый основной компонент, по которому ведется учет по ГТД.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеТовара%", ПредставлениеОсновногоКомпонента);
		...
		Если КонтролироватьНомераГТД Тогда
			Если Не РеквизитыОсновнойКомпоненты.ВестиУчетПоГТД Тогда
				Если ИспользоватьУчетПрослеживаемыхИмпортныхТоваров Тогда
					ТекстРекомендации = НСтр("ru = 'Укажите непрослеживаемый основной компонент, по которому ведется учет по ГТД'");
				Иначе
					ТекстРекомендации = НСтр("ru = 'Укажите основной компонент, по которому ведется учет по ГТД'");
				КонецЕсли;
				
				ТекстСообщения = НСтр("ru = 'По основному компоненту `%НазваниеТовара%` учет по ГТД не ведется.
					|%ТекстРекомендации%.'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеТовара%", ПредставлениеОсновногоКомпонента);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТекстРекомендации%", ТекстРекомендации);
	...
	ИначеЕсли Не ПрослеживаемыйКомплект
		...
		ТекстСообщения = НСтр("ru = 'Требуется определять страну происхождения и номера ГТД комплекта.
			|%1.'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, ТекстРекомендации);
	...
	Иначе
		...
		КонтрольКачества.ОбработкаПроверкиКорректностиСоединенияССериями(
			ЭтотОбъект,
			ПараметрыУказанияСерий.ТЧ.ПоляСвязи,
			"Товары",
			НСтр("ru = 'Товары'"),
			Отказ);
...
Процедура ЗаполнитьПоЗаказу(СтруктураЗаполнения)
	...
	Документы.СборкаТоваров.ЗаполнитьПоЗаказамОрдерам(ТаблицаНакладнаяПоШапке, Ссылка, ПараметрыЗаполнения, ШтрихкодыУпаковок);
	
	Ресурсы[0] = "Комплектующие";
	ПараметрыЗаполнения.Ресурсы = Ресурсы;
	Документы.СборкаТоваров.ЗаполнитьПоЗаказамОрдерам(ТаблицаНакладная, Ссылка, ПараметрыЗаполнения, ШтрихкодыУпаковок);
...
Процедура РаспределитьНомераГТД(Отказ) Экспорт
	...
	Если ВидыЗапасовДляСозданияСоставныхНомеровГТД.Количество() > 0 Тогда
		...
		ПрослеживаемыйКомплект =
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Номенклатура, "ПрослеживаемыйТовар")
			И УчетПрослеживаемыхТоваровЛокализация.ИспользоватьУчетПрослеживаемыхИмпортныхТоваров(Дата);
		ЕстьПрослеживаемыеКомплектующие = 0;
		
		Если Не ЭтоСборка
			И ПрослеживаемыйКомплект Тогда
			
			Комплектующие = Товары.ВыгрузитьКолонку("Номенклатура");
			Комплектующие = ОбщегоНазначенияКлиентСервер.СвернутьМассив(Комплектующие);
			
			ДанныеКомплектующих = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(Комплектующие, "ПрослеживаемыйТовар");
			
			Для Каждого КлючИЗначение Из ДанныеКомплектующих Цикл
				Если КлючИЗначение.Значение Тогда
					ЕстьПрослеживаемыеКомплектующие = Истина;
		...
		ИначеЕсли ЭтоСборка
			И ПрослеживаемыйКомплект
			И ТипыНомеровГТДВТаблице.КоличествоСоставныхНомеровГТД = 1
			И ТипыНомеровГТДВТаблице.КоличествоПрослеживаемыхНомеровГТД = 0 Тогда
			РаспределитьНомераГТДПрослеживаемыеПеренос();
		ИначеЕсли ЭтоСборка
			И ПрослеживаемыйКомплект
			И ТипыНомеровГТДВТаблице.КоличествоСоставныхНомеровГТД = 0
			И ТипыНомеровГТДВТаблице.КоличествоПрослеживаемыхНомеровГТД = 1 Тогда
			РаспределитьНомераГТДПрослеживаемыеСборка(Отказ);
		ИначеЕсли Не ЭтоСборка
			И ПрослеживаемыйКомплект
			И ЕстьПрослеживаемыеКомплектующие
		...
		ИначеЕсли ПрослеживаемыйКомплект
			И Не ЕстьПрослеживаемыеКомплектующие Тогда
			ТекстСообщения = НСтр("ru = 'Отсутствуют прослеживаемые по РНПТ комплектующие при прослеживаемом комплекте.
				|Будет произведено выбытие прослеживаемого комплекта.'");
			// Отказ не прокидывается в данном случае, поскольку возможен сценарий выбытия прослеживаемого комплекта.
...
Процедура РаспределитьНомераГТДПрослеживаемыеСборка(Отказ)
	...
	ТаблицаПроселживаемости = ЗакупкиСервер.ПолучитьПрослеживаемыеНомераГТД(ВидыЗапасовСписание.Выгрузить(, ИменаКолонок));
	...
	СоставныеПрослеживаемыеНомераГТД = ЗакупкиСервер.НайтиИлиСоздатьСоставныеПрослеживаемыеНомераГТД(ТаблицаПроселживаемости,
		Количество,
		Дата,
		Отказ,
		ПараметрыСоздания);
