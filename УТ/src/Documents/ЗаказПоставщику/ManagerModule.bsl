...
Функция ТекстЗапросаДляРасчетаСостоянийЗаказов() Экспорт
	...
	ТекстЗапроса = "
		...
		|			ИЛИ ДокументЗаказПоставщику.СуммаДокумента = 0)
		|	ТОГДА
		...
		|ПО
		|	ДокументЗаказПоставщику.ОбъектРасчетов = ДатыАктуальностиРасчетов.ОбъектРасчетов
		|ЛЕВОЕ СОЕДИНЕНИЕ
...
Функция МакетФормыВыбораНазначений() Экспорт
	...
	ОписаниеКолонок.ПутиКДанным.Распоряжение     = "Распоряжение";
...
Функция ПараметрыВзаиморасчеты(ДанныеЗаполнения = Неопределено) Экспорт
	...
	СтруктураПараметров.ДатаПоступления                  = "Объект.Товары.ДатаПоступления";
...
// 
// Параметры:
//  ОбъектИлиСсылка - ДокументОбъект.ЗаказПоставщику, ДокументСсылка.ЗаказПоставщику - документ.
...
Функция КонтрольКачестваПараметрыДействий(Объект) Экспорт
	...
	ПараметрыДействия.ЧитаемыеПоля = "Номенклатура,Склад,ДатаПоступления,Отменено,СписатьНаРасходы";
	...
	ПараметрыДействия.КлючевыеРеквизитыШапки.Вставить("ДлительностьДоставки", Объект.ДлительностьДоставки);
	ПараметрыДействия.ТекстЗапросаДанныеДокумента =
		...
		|	ЛОЖЬ КАК Установить,
		|	Таблица.Отменено ИЛИ Таблица.СписатьНаРасходы КАК Сбросить
...
// Возвращает дату, которая отличается указанной даты начала на количество рабочих дней,
// входящих в основной календарь предприятия.
//
// Параметры:
// ДатаНачала - Дата - дата, от которой нужно рассчитать количество дней.
// КоличествоДней - Число - количество дней, на которые нужно увеличить дату начала.
//
// Возвращаемое значение:
//   Дата - дата, увеличенная на количество рабочих дней по основному календарю предприятия.
//          Если календарь не заполнен, дата увеличивается на количество календарных дней.
//
Функция ДатаПоКалендарюПредприятия(ДатаНачала, КоличествоДней) Экспорт
	...
	Подстановки = ОбеспечениеВДокументахСервер.ДобавитьКДатеПоКалендарюПредприятия(
		"Таблица.ДатаНачала", "Таблица.КоличествоДней");
	...
	ДатаПоКалендарю = Выборка.ДатаПоКалендарю;
	Если Не ЗначениеЗаполнено(ДатаПоКалендарю) Тогда
		ДатаПоКалендарю = ОбщегоНазначенияУТКлиентСервер.РассчитатьДатуОкончанияПериода(
			ДатаНачала, Перечисления.Периодичность.День, КоличествоДней) + 1;
	КонецЕсли;
...
// Определяет дату ближайшего дня, включенного в график.
//
// Параметры:
//  ДатаНачала - Дата - дата, относительно которой нужно найти ближайшую.
...
// 
// Возвращаемое значение:
//  Дата - ближайшая дата, включенная в график.
//
Функция БлижайшаяРабочаяДата(ДатаНачала, Календарь = Неопределено) Экспорт
	...
	НачальныеДаты = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДатаНачала);
	ДатыПоКалендарю = Неопределено;
	...
	Если ТипЗнч(Календарь) <> Тип("СправочникСсылка.ПроизводственныеКалендари") Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ГрафикиРаботы") Тогда
			...
			ДатыПоКалендарю = МодульГрафикиРаботы.БлижайшиеДатыВключенныеВГрафик(
				Календарь, НачальныеДаты, ПараметрыПолучения);
		КонецЕсли;
	...
	Иначе
		...
		ПараметрыПолучения.ВызыватьИсключение = Ложь;
		ДатыПоКалендарю = КалендарныеГрафики.БлижайшиеРабочиеДаты(Календарь, НачальныеДаты, ПараметрыПолучения);
		
	КонецЕсли;
	
	Если ДатыПоКалендарю = Неопределено Тогда
		Возврат ДатаНачала;
	КонецЕсли;
	
	ДатаПоКалендарю = ДатыПоКалендарю.Получить(ДатаНачала);
	Если ДатаПоКалендарю = Неопределено Тогда
		ДатаПоКалендарю = ДатаНачала;
	КонецЕсли;
...
// Определяет дату отгрузки по дате поступления и длительности доставки по количеству дней, входящих в календарь.
//
// Параметры:
//  ДатаЗаказа - Дата - минимально допустимая для отгрузки дата.
//  ДатаПоступления - Дата - дата, от которой нужно рассчитать дату отгрузки.
//  ДлительностьДоставки - Число - количество дней, на которые нужно уменьшить дату поступления.
//  Календарь - СправочникСсылка.Календари
//            - СправочникСсылка.ПроизводственныеКалендари - график или 
//              производственный календарь, который необходимо использовать для расчета.
//
...
//   Дата
//
Функция ДатаОтгрузкиПоКалендарю(ДатаЗаказа, ДатаПоступления, ДлительностьДоставки, Календарь = Неопределено) Экспорт
	
	Если Календарь = Неопределено Тогда
		Календарь = Константы.ОсновнойКалендарьПредприятия.Получить();
	КонецЕсли;
	
	МинимальнаяДатаОтгрузки = БлижайшаяРабочаяДата(ДатаЗаказа, Календарь);
	Если ЗначениеЗаполнено(ДатаПоступления) Тогда
		ДатаОтгрузки = ДатаПоКалендарюПредприятия(ДатаПоступления, -ДлительностьДоставки);
	Иначе
		ДатаОтгрузки = БлижайшаяРабочаяДата(ТекущаяДатаСеанса(), Календарь);
	КонецЕсли;
	
	Возврат Макс(МинимальнаяДатаОтгрузки, ДатаОтгрузки);
	
КонецФункции

// Определяет длительность доставки в днях по календарю.
//
// Параметры:
//  ДатаНачала - Дата - дата начала периода доставки.
//  ДатаОкончания - Дата - дата окончания периода доставки.
//  Календарь - СправочникСсылка.Календари
//            - СправочникСсылка.ПроизводственныеКалендари - график или 
//              производственный календарь, который необходимо использовать для расчета.
//
...
//   Число
//
Функция ДлительностьДоставкиПоКалендарю(ДатаНачала, ДатаОкончания, Календарь = Неопределено) Экспорт
	...
	ДлительностьДоставки = КалендарныеГрафики.РазностьДатПоКалендарю(Календарь, ДатаНачала, ДатаОкончания, Ложь);
	Если ЗначениеЗаполнено(ДлительностьДоставки) Тогда
		ДлительностьДоставки = Макс(ДлительностьДоставки - 1, 1);
	Иначе
		ДлительностьДня = 86400;
		ДлительностьДоставки = (ДатаОкончания - ДатаНачала) / ДлительностьДня;
	КонецЕсли;
	
	Возврат ДлительностьДоставки;
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	ПроведениеДокументов.УстановитьПараметрыЗапросаПоВыборке(Запрос, Реквизиты, "ДоговорПредставление");
	
	Запрос.УстановитьПараметр("ЗаявкаНаЗакупку", Документы.ЗаявкаНаЗакупку.ЗаявкаОснованиеЗаказаПоставщику(ДокументСсылка));
...
Процедура ТекстЗапросаДанныеДокументаШапка(СписокЗапросов)
	...
	|	ДанныеШапки.Договор.ВариантОформленияДокументовПоступленияПоУслугам КАК ДоговорВариантОформленияДокументовПоступленияПоУслугам,
...
Процедура ТекстЗапросаДанныеДокументаТаблицаЭтапыГрафикаОплаты(СписокЗапросов)
	...
	|	ТаблицаЭтапыГрафикаОплаты.ДатаОтгрузки        КАК ДатаОтгрузки,
...
Процедура ЗапланироватьПоступлениеТоваров(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	|	ШапкаДокумента.ВариантПриемкиУслуг           КАК ВариантПриемкиУслуг,
	...
	|   	И НЕ (ИсточникДанных.Номенклатура.ТипНоменклатуры В
	|			(ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга),
	|			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))
	|			И ШапкаДокумента.ДоговорВариантОформленияДокументовПоступленияПоУслугам = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется)))
	|		ИЛИ (ВЫРАЗИТЬ(ИсточникДанных.Номенклатура КАК Справочник.Номенклатура).ТипНоменклатуры В(
	|				ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга),
	|				ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))
	|			И ШапкаДокумента.ДоговорВариантОформленияДокументовПоступленияПоУслугам = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется)
	|			И ШапкаДокумента.ХозяйственнаяОперация В(
	...
	Если ДокументОбъект = Неопределено Тогда
		...
		ТекстЗапросаДокумента = СтрЗаменить(ТекстЗапросаДокумента, "ШапкаДокумента.ДоговорВариантОформленияДокументовПоступленияПоУслугам", "ШапкаДокумента.Договор.ВариантОформленияДокументовПоступленияПоУслугам");
	КонецЕсли;
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	ТекстЗапросаНаправленияДеятельности = ВзаиморасчетыПроведение.ПолучитьТаблицуНаправленийДеятельности(ТекстЗапросаНаправленияДеятельности);
	...
	ВзаиморасчетыПроведениеПоставщики.ПроведениеРасчетыСПоставщикамиПланПоставок(Запрос, ТекстыЗапроса, Регистры, ТекстПланыПоставок);
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|	
		|	&ЗаявкаНаЗакупку <> ЗНАЧЕНИЕ(Документ.ЗаявкаНаЗакупку.ПустаяСсылка) КАК ПоГрафику,
		|	&ЗаявкаНаЗакупку КАК РаспоряжениеВГрафике,
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("3344821c-a845-4238-a1d6-52a2ec46bcee");
	...
	СписокОписаний.Добавить(НСтр("ru = 'Обновление документов ""Заказ поставщику"":'"));
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.ЗаказПоставщику КАК РеквизитыДокумента
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПоставщику КАК РеквизитыДокумента
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			Если ПричинаИсключения = 2 Тогда
				ВызватьИсключение НСтр("ru = 'Не удалось обработать обновление документов ""Заказ поставщику"".'");
			КонецЕсли;
