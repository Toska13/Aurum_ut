...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВариантОформленияДокументовПоУслугам = ЗакупкиСервер.ПолучитьВариантОформленияДокументовПоступленияПоУслугам(Объект.Договор);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		МногооборотнаяТараКлиент.ПредложитьПодобратьМногооборотнуюТару(
			ЭтотОбъект,
			"Товары",
			"Номенклатура,Характеристика,Количество,Склад,ДатаОтгрузки,ДатаПоступления",
...
Процедура ТоварыПослеУдаления(Элемент)
	...
	ТоварыПослеУдаленияНаСервере(ЕстьИзменениеДатОтгрузок);
	МногооборотнаяТараКлиент.ОбновитьСостояниеЗаполненияМногооборотнойТары(Объект.СостояниеЗаполненияМногооборотнойТары);
	УстановитьПризнакЗаполненияСклада();
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ПараметрыЗаполненияСклада = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияСкладаВСтрокеТЧ(
		Объект, СкладГруппа, , ,"СписатьНаРасходы");
...
Процедура ТоварыДатаОтгрузкиПриИзменении(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		Если ЗначениеЗаполнено(ТекущиеДанные.ДатаОтгрузки) Тогда
			ТекущиеДанные.ДатаПоступления = ДатаПоКалендарюПредприятия(ТекущиеДанные.ДатаОтгрузки, Объект.ДлительностьДоставки);
		Иначе
...
Процедура УстановитьУсловноеОформление()
	...
	Статусы = Перечисления.СтатусыЗаказовПоставщикам;
	ТипыНоменклатуры = Перечисления.ТипыНоменклатуры;
	КонтрольКачества.ДобавитьОтбор(ФлагНедоступен, "Объект.Товары.Отменено", Истина);
	...
	КонтрольКачества.ДобавитьОтбор(ДатаНеОбязательна, "Объект.Статус", Статусы.НеСогласован);
	КонтрольКачества.ДобавитьОтбор(ДатаНеОбязательна, "Объект.Статус", Статусы.Согласован);
...
Процедура ОбновитьОбязательностьСкладаВТЧ()
	...
	ПараметрыЗаполнения = Новый Структура("ЕстьРаботы, ЕстьСписатьНаРасходы", Истина, Истина);
	СтруктураДействий = Новый Структура;
...
Процедура СоглашениеПриИзмененииСервер(ПересчитатьЦены = Истина)
	...
	ПерезаполнитьБанковскийСчет = Ложь;
	...
	Если ЗначениеЗаполнено(Объект.Соглашение) Тогда
		...
		Если Договор <> Объект.Договор Тогда
			Договор = Объект.Договор;
			ПерезаполнитьБанковскийСчет = Истина;
			Объект.ВариантПриемкиТоваров = ЗакупкиСервер.ПолучитьВариантПриемкиТоваров(Неопределено, Объект.Договор);
			Объект.ВариантПриемкиУслуг = ЗакупкиСервер.ПолучитьВариантПриемкиУслуг(Объект.Договор);
		КонецЕсли;
	...
	ИзмененныеРеквизиты = ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтаФорма, "Соглашение, Договор");
	...
	Если ПерезаполнитьБанковскийСчет Тогда
...
Процедура ДатаОтгрузкиПриИзмененииНаСервере()
	...
	Иначе
		Объект.ДатаПоступления = ДатаПоКалендарюПредприятия(Объект.ДатаОтгрузки, Объект.ДлительностьДоставки);
	КонецЕсли;
...
Процедура ДатаПоступленияПриИзмененииСервер()
	...
	Если ЗначениеЗаполнено(Объект.ДатаОтгрузки) Тогда
		Объект.ДлительностьДоставки = ДлительностьДоставкиПоКалендарю(
			Объект.ДатаОтгрузки, Объект.ДатаПоступления, КалендарьПредприятия);
	Иначе
		Объект.ДатаОтгрузки = ДатаПоКалендарюПредприятия(Объект.ДатаПоступления, -Объект.ДлительностьДоставки);
		Если Объект.ДатаОтгрузки < Объект.Дата Тогда
			Объект.ДатаОтгрузки = Мин(Объект.ДатаПоступления, ДатаОтгрузкиПоКалендарю(
				Объект.Дата, Объект.ДатаПоступления, Объект.ДлительностьДоставки, КалендарьПредприятия));
			Объект.ДлительностьДоставки = ДлительностьДоставкиПоКалендарю(
				Объект.ДатаОтгрузки, Объект.ДатаПоступления, КалендарьПредприятия);
		КонецЕсли; 
		ОбеспечениеВДокументахСервер.ЗаполнитьРеквизитВКоллекции(Объект.Товары, "ДатаОтгрузки", Объект.ДатаОтгрузки);
	...
	ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтотОбъект, СтрРазделить("Товары.ДатаПоступления,Товары.ДатаОтгрузки",","));
...
&НаСервере
Процедура ТоварыПослеУдаленияНаСервере(ЕстьИзменениеДатОтгрузок)
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	СтруктураДействийСДобавленнымиСтроками.Вставить("ПриИзмененииТипаНоменклатуры",
		Новый Структура("ЕстьРаботы, ЕстьСписатьНаРасходы", Истина, Истина));
	СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьПризнакСписатьНаРасходыИПодразделение");
	...
	СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", КонтрольКачестваКэш);
...
Функция ЗаполнитьСкладВВыделенныхСтрокахНаСервере(Знач МассивВыделенныхСтрок, Склад)
	...
	ЗаполненоСтрок = СкладыСервер.ЗаполнитьСкладыВВыделенныхСтроках(Объект.Товары, МассивВыделенныхСтрок, Склад,, "СписатьНаРасходы");
...
Процедура УстановитьДоступностьЭлементовПоСтатусуСервер()
	...
	МассивЭлементов.Добавить("ТоварыПодобратьАналоги");
...
Процедура ЗаполнитьПустыеДатыПоступленияСервер()
	...
	Если Объект.ПоступлениеОднойДатой Тогда
		Если Не ЗначениеЗаполнено(Объект.ДатаПоступления) Тогда
			Если Не ЗначениеЗаполнено(Объект.ДатаОтгрузки) Тогда
				Объект.ДатаОтгрузки = ДатаОтгрузкиПоКалендарю(
					Объект.Дата, Объект.ЖелаемаяДатаПоступления, Объект.ДлительностьДоставки, КалендарьПредприятия);
			КонецЕсли;
			Объект.ДатаПоступления = ДатаПоКалендарюПредприятия(Объект.ДатаОтгрузки, Объект.ДлительностьДоставки);
		КонецЕсли;
		ДатаОтгрузки = Объект.ДатаОтгрузки;
	Иначе
		ДатаОтгрузки = ?(ЗначениеЗаполнено(Объект.ДатаОтгрузки), Объект.ДатаОтгрузки, ДатаОтгрузкиПоКалендарю(
			Объект.Дата, Объект.ЖелаемаяДатаПоступления, Объект.ДлительностьДоставки, КалендарьПредприятия));
	КонецЕсли;
	
	ДатаПоступления = ДатаПоКалендарюПредприятия(ДатаОтгрузки, Объект.ДлительностьДоставки);
	
	КонтролируемыеПартииСтроки = Новый Массив();
	Для Каждого СтрокаТЧ Из Объект.Товары Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧ.ДатаПоступления) Тогда
			...
			КонтролируемыеПартииСтроки.Добавить(СтрокаТЧ);
		КонецЕсли;
	КонецЦикла;
		
	КонтрольКачества.ОбновитьПараметрыДействий(Документы.ЗаказПоставщику, Объект, КонтрольКачестваКэш);
	
	СтруктураДействий = Новый Структура();
	СтруктураДействий.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", КонтрольКачестваКэш);
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(КонтролируемыеПартииСтроки, СтруктураДействий, Объект.Товары);
...
Процедура ЗаполнитьДатуОтгрузки(МассивВыделенныхСтрок = Неопределено)
	...
	ДатаОтгрузки = ДатаОтгрузкиПоКалендарю(
		Объект.Дата, Объект.ЖелаемаяДатаПоступления, Объект.ДлительностьДоставки, КалендарьПредприятия);
...
Процедура ЗаполнитьДатуОтгрузкиЗавершение(ВыбраннаяДата, ДополнительныеПараметры) Экспорт
	...
	Если ВыбраннаяДата <> Неопределено И ЗначениеЗаполнено(ВыбраннаяДата) Тогда
		...
		КонецЕсли;
		ЗаполнитьДатуОтгрузкиЗавершениеСервер(МассивВыделенныхСтрок);
...
&НаСервере
Процедура ЗаполнитьДатуОтгрузкиЗавершениеСервер(ВыделенныеСтроки)
	...
	Иначе
		...
		Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
			Коллекция.Добавить(Объект.Товары.НайтиПоИдентификатору(ВыделеннаяСтрока));
		КонецЦикла;
		
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(Коллекция, СтруктураДействий, Объект.Товары);
...
Процедура ДлительностьДоставкиПриИзмененииСервер()
	Если НЕ Объект.ПоступлениеОднойДатой Тогда
		ДокументЗакупки = РеквизитФормыВЗначение("Объект");
		ДокументЗакупки.ПересчитатьДатыПоКалендарю(Объект.ДлительностьДоставки);
		ЗначениеВРеквизитФормы(ДокументЗакупки, "Объект");
...
Процедура ЗаполнитьМногооборотнуюТаруИзХранилищаСервер(АдресТарыВХранилище)
	...
	СтруктураДействийДобавленныеСтроки.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", КонтрольКачестваКэш);
	
	МногооборотнаяТараСервер.ЗаполнитьМногооборотнуюТаруИзХранилища(
		Объект,
		АдресТарыВХранилище,
		"Товары",
		"Номенклатура,Характеристика,Склад,ДатаОтгрузки,ДатаПоступления",
...
Процедура ПолучитьЗагруженныеТоварыИзХранилища(АдресТоваровВХранилище)
	...
	ДатаОтгрузки = ДатаПоКалендарюПредприятия(Объект.ДатаПоступления, -Объект.ДлительностьДоставки);
	Для Каждого СтрокаТоваров Из ТоварыИзХранилища Цикл
		...
		Если Объект.ПоступлениеОднойДатой Тогда
			...
			СтрокаТЧТовары.ДатаОтгрузки = ДатаОтгрузки;
		КонецЕсли;
	...
	Если Не ЗакупкиСервер.ЭтоНеотфактурованнаяПоставкаУслугРабот(Объект.ХозяйственнаяОперация, ВариантОформленияДокументовПоУслугам) Тогда
		ЗаполнитьПризнакСписанияНаРасходыДляУслуг();
	КонецЕсли;
...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеКоманды(Команда)
	...
	ИначеЕсли Команда.Имя = "ПодобратьАналоги" Тогда
...
Процедура СкорректироватьСтрокиЗаказа(СтруктураДополнительныхПараметров)
	...
	Если СтруктураРезультата.КоличествоСтрок <> 0 ИЛИ СтруктураРезультата.СуммаОтклонения > 0 Тогда
		...
		ЭтапыОплатыСервер.ПроверитьЗаполнитьЭтапыГрафикаОплаты(ДокументЗакупки);
...
Функция ПолучитьБанковскийСчетОрганизацииПоУмолчаниюСервер()
	...
	Возврат ДенежныеСредстваСервер.ПолучитьБанковскийСчетОрганизацииПоУмолчанию(ПараметрыСчета,
		Объект.БанковскийСчет, Объект.ФормаОплаты);
...
Процедура ОбновитьТекстДокументыНаОсновании()
	...
	ЭтоЗакупкаЕАЭС = ВидыОперацийВСтранахЕАЭС.Найти(Объект.ХозяйственнаяОперация) <> Неопределено;
	
	Если ЗначениеЗаполнено(Объект.Ссылка) И ЭтоЗакупкаЕАЭС Тогда
		ТекстДокументыНаОсновании = УчетНДСЛокализация.СобытиеДокументыОПредварительныхПоставках(Объект, СуммаЗаказаноСЗалоговойТарой);
...
&НаСервере
// Заполняет реквизит формы "Распоряжение" для работы связей параметров выбора справочника "Назначения."
// См. Документы.ЗаказПоставщику.МакетФормыВыбораНазначений
Процедура ЗаполнитьРаспоряжение()
	
	Распоряжение = Документы.ЗаявкаНаЗакупку.ЗаявкаОснованиеЗаказаПоставщику(Объект);
...
&НаСервереБезКонтекста
Функция ДатаПоКалендарюПредприятия(Знач ДатаНачала, Знач КоличествоДней)
	
	Возврат Документы.ЗаказПоставщику.ДатаПоКалендарюПредприятия(ДатаНачала, КоличествоДней);
	
КонецФункции

&НаСервереБезКонтекста
Функция ДатаОтгрузкиПоКалендарю(Знач ДатаЗаказа, Знач ДатаПоступления, Знач ДлительностьДоставки, Знач КалендарьПредприятия)
	
	Возврат Документы.ЗаказПоставщику.ДатаОтгрузкиПоКалендарю(ДатаЗаказа, ДатаПоступления, ДлительностьДоставки, КалендарьПредприятия);
	
КонецФункции

&НаСервереБезКонтекста
Функция ДлительностьДоставкиПоКалендарю(Знач ДатаНачала, Знач ДатаОкончания, Знач КалендарьПредприятия)
	
	Возврат Документы.ЗаказПоставщику.ДлительностьДоставкиПоКалендарю(ДатаНачала, ДатаОкончания, КалендарьПредприятия);
