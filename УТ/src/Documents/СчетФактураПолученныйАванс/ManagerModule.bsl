...
Процедура СформироватьЗаданияКФормированиюДвиженийПоНДС(ТаблицаИзменений) Экспорт
	...
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	РеестрДокументов.Ссылка       КАК СчетФактура,
	|	СчетФактура.Организация       КАК Организация,
	|	Расчеты.Период                КАК Месяц
	|ИЗ
	|	РегистрСведений.РеестрДокументов КАК РеестрДокументов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтРасчеты КАК Расчеты
	|		ПО РеестрДокументов.Ссылка = Расчеты.РасчетныйДокумент
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураПолученныйАванс КАК СчетФактура
	|		ПО РеестрДокументов.СторнируемыйДокумент = СчетФактура.ДокументОснование
	|ГДЕ
	|	СчетФактура.Проведен
...
//  ДополнительныеПараметры - Структура - см. ДополнительныеПараметрыАвансовВыданных()
// 
// Возвращаемое значение:
...
// * Организация - СправочникСсылка.Организации - организация выдавшая аванс
...
Функция ПолучитьВыданныеАвансы(Организация, НачалоПериода, КонецПериода, ДополнительныеПараметры) Экспорт
	...
	ВыданныеАвансы.Колонки.Добавить("Организация");
	...
	ВыданныеАвансы.Колонки.Добавить("Исправление");
	...
	ПоляГруппировки = "Поставщик, ДокументОснование, ТипДокументаОснования, НалогообложениеНДС,
						|ВалютаДокумента, СтавкаНДС, Организация,
						|Номер, Дата, НомерСчетаФактуры, ДатаСчетаФактуры, СчетФактура, СФСформирован, Исправление";
	ПоляСуммирования = "СуммаАвансаРегл, СуммаНДС, ВалютнаяСумма";

	Если ДополнительныеПараметры.ВРазрезеОР = Истина Тогда
	...
	Если ДополнительныеПараметры.ВРазрезеНП = Истина Тогда
		ПоляГруппировки = ПоляГруппировки + ", НаправлениеДеятельности";
	КонецЕсли;
	...
	Результат = РезультатЗапросаПоАвансам(
			...
			КонецПериода,
			Организация,
			ДополнительныеПараметры.РасчетныйДокумент,
			МенеджерВременныхТаблиц,
			ДополнительныеПараметры.ИсключитьИУДА);
	...
	Пока ВыборкаАванс.Следующий() Цикл
		...
		Для каждого Ставка Из МассивСтавок Цикл
			...
			НоваяСтрока.СтавкаНДС       = ?(ДополнительныеПараметры.ВРазрезеСтавокНДС, СтавкаНДСАванса, СтавкаНДСПустая);
...
Функция ЕстьСчетаФактурыКОформлению(Параметры) Экспорт
	...
	ДополнительныеПараметры = ДополнительныеПараметрыАвансовВыданных();
	ДополнительныеПараметры.ИсключитьИУДА = Истина;
	
	ПолученныеАвансы = ПолучитьВыданныеАвансы(Параметры.МассивОрганизаций, НачалоПериода, Параметры.КонецПериода, ДополнительныеПараметры);
...
// Дополнительные параметры авансов выданных.
// 
// Возвращаемое значение:
//  Структура - Дополнительные параметры авансов выданных:
// * РасчетныйДокумент - ДокументСсылка, Неопределено - Расчетый документ, по которому получаются авансы, Если Неопределено, то по всем расченым документам.
// * ВРазрезеСтавокНДС - Булево - Если Истина, будут заполнены колонки со ставкой и суммой НДС.
// * ВРазрезеОР - Булево - 
// * ИсключитьИУДА - Булево - 
// * ВРазрезеНП - Булево - 
Функция ДополнительныеПараметрыАвансовВыданных() Экспорт
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("РасчетныйДокумент", Неопределено);
	ДополнительныеПараметры.Вставить("ВРазрезеСтавокНДС", Ложь);
	ДополнительныеПараметры.Вставить("ВРазрезеОР",        Ложь);
	ДополнительныеПараметры.Вставить("ИсключитьИУДА",     Ложь);
	ДополнительныеПараметры.Вставить("ВРазрезеНП",        Истина);
...
Функция ТекстЗапросаТаблицаРеестрДокументов(Запрос, ТекстыЗапроса, Регистры)
	...
	|	&ДатаОтраженияВУчете                     КАК ДатаОтраженияВУчете,
...
Функция РезультатЗапросаПоАвансам(НачалоПериода, КонецПериода, Организация, РасчетныйДокумент = Неопределено, МенеджерВременныхТаблиц, ИсключитьИУДА = Ложь)
	...
	|	
	|	ВЫБОР КОГДА РеестрДокументовСторно.Ссылка ЕСТЬ NULL
	|		ТОГДА ЕСТЬNULL(СчетФактураПолученныйАванс.Номер, """")
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК НомерСчетаФактуры,
	|	ВЫБОР КОГДА РеестрДокументовСторно.Ссылка ЕСТЬ NULL
	|		ТОГДА ЕСТЬNULL(СчетФактураПолученныйАванс.ДатаСоставления, ДАТАВРЕМЯ(1,1,1))
	|		ИНАЧЕ ДАТАВРЕМЯ(1,1,1)
	|	КОНЕЦ КАК ДатаСчетаФактуры,
	|	ВЫБОР КОГДА РеестрДокументовСторно.Ссылка ЕСТЬ NULL
	|		ТОГДА ЕСТЬNULL(СчетФактураПолученныйАванс.Сумма, 0)
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаСчетаФактуры,
	|	ВЫБОР КОГДА РеестрДокументовСторно.Ссылка ЕСТЬ NULL
	|		ТОГДА ЕСТЬNULL(СчетФактураПолученныйАванс.Ссылка, ЗНАЧЕНИЕ(Документ.СчетФактураПолученныйАванс.ПустаяСсылка))
	|		ИНАЧЕ ЗНАЧЕНИЕ(Документ.СчетФактураПолученныйАванс.ПустаяСсылка)
	|	КОНЕЦ КАК СчетФактура,
	|	ВЫБОР КОГДА РеестрДокументовСторно.Ссылка ЕСТЬ NULL
	|		ТОГДА ЕСТЬNULL(СчетФактураПолученныйАванс.Исправление, ЛОЖЬ)
	|		ИНАЧЕ ЛОЖЬ
	...
	|	ПО
	|		РеестрДокументовСторно.ТипСсылки = &ТипСсылки
	|		И РеестрДокументовСторно.СторнируемыйДокумент = СчетФактураПолученныйАванс.Ссылка
	...
	Запрос.УстановитьПараметр("ТипСсылки", ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ПустаяСсылка().Метаданные()));
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "11.5.25.31";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("5b454481-1c01-4876-b2e8-a9786e862b1d");
	...
	СписокОписаний.Добавить(НСтр("ru = 'Обновление документов ""Счет-фактура полученный (аванс)"":'"));
	СписокОписаний.Добавить(НСтр("ru = '- заполнение реквизита ""Дата отражения в учете"" из даты получения, если не был заполнен ранее'"));
	...
	Изменяемые.Добавить(Метаданные.Документы.СчетФактураПолученныйАванс.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Документ.СчетФактураПолученныйАванс КАК СчетФактураПолученныйАванс
	|ГДЕ
	|	СчетФактураПолученныйАванс.ДатаОтраженияВУчете = ДАТАВРЕМЯ(1, 1, 1)
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст = 
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураПолученныйАванс КАК ДанныеДокумента
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Попытка 
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если Не ЗначениеЗаполнено(ДокументОбъект.ДатаОтраженияВУчете) Тогда
					ДокументОбъект.ДатаОтраженияВУчете = ВыборкаДокументов.Дата;
				КонецЕсли;
	...
	Иначе
		...
		ШаблонСообщения = НСтр("ru = 'Обработана порция документов ""Счет-фактура полученный (аванс)"": %1'");
