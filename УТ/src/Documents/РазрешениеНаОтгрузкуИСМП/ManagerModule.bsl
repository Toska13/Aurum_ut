...
// Функция возвращает проверку, что товарная группа требует оформления разрешения на отгрузку
// 
// Параметры:
//  ТоварнаяГруппа - СправочникСсылка.ТоварныеГруппыГИСМТ
// 
// Возвращаемое значение:
//  Булево - Истина, если требуется оформление на отгрузку
Функция ТоварнаяГруппаПоддерживаетРазрешениеНаОтгрузку(ТоварнаяГруппа) Экспорт
	...
	СтруктураПараметров.Вставить("Ссылка", ПустаяСсылка());
	
	ВидыПродукцииПоТоварнойГруппе    = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ТоварнаяГруппа);
	ДоступныеВидыПродукцииРазрешения = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступныеВидыПродукции(СтруктураПараметров);
	
	// если хотя бы по одному виду продукции товарной группы требуется разрешение - по товарной группе требуется разрешение
	Возврат ОбщегоНазначенияКлиентСервер.РазностьМассивов(ВидыПродукцииПоТоварнойГруппе, ДоступныеВидыПродукцииРазрешения).Количество() < ВидыПродукцииПоТоварнойГруппе.Количество();
...
Функция ЗапросРазрешенияНаОтгрузкуПродукции(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	ВидыПродукции = КонструкторИСМПКлиентСервер.ВидыПродукцииТребующиеРазрешенияНаОтгрузку();
	Если ВидыПродукции.Количество() = 0 Тогда
		ШаблонСообщения = НСтр("ru = 'В настройках интеграции с ГИС МТ выключены виды продукции, поддерживающие разрешение на отгрузку.'");
		ТекстОшибки = СтрШаблон(ШаблонСообщения, ДокументСсылка);
		ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(СообщениеJSON, ТекстОшибки);
		СообщенияJSON.Добавить(СообщениеJSON);
		Возврат СообщенияJSON;
