...
Функция ПараметрыФормированияТранспортныхНакладных() Экспорт
	...
	Реквизиты.Вставить("АдресДоставкиЗначение", "ОснованиеТранспортнойНакладной.АдресДоставкиЗначение");
...
Функция ТекстТТН(Форма) Экспорт
	...
	Если ПараметрыФормированияТранспортныхНакладных.ЕстьЗаказы Тогда
		Если Объект[ПараметрыФормированияТранспортныхНакладных.ИмяПоляНакладнаяПоЗаказу] Тогда
			...
			КонецЕсли;	
			Если ЗаказСсылка = Неопределено Или ТипЗнч(ЗаказСсылка) = Тип("ДокументСсылка.ПервичныйДокумент") Тогда
	...
	+ ОбщегоНазначения.РазделительПакетаЗапросов()
	+ ТекстЗапросаВыборкиТранспортныхНакладныхПоДокументуОснованию(
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ПоЗаданиюНаПеревозку
				...
				Если ВыдаватьСообщения Тогда		
					...
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СпособДоставки%",
						ПредставлениеСпособаДоставки(Выборка.СпособДоставки, Выборка.Ссылка));
...
Функция СоздатьТранспортныеНакладные(МассивОбъектов, 
	...
	Для Каждого КлючЗначение Из ТипыОбъектов Цикл
		...
		|		ЕСТЬNULL(СкладыКонтактнаяИнформация.Значение, """") КАК АдресПогрузкиЗначение,
		...
		Если ПараметрыФормированияТранспортныхНакладных.ЕстьЗаказы Тогда
			...
			ТекстВыборкиДанныхОснования = ТекстВыборкиДанныхОснования
				+ Символы.ПС
				+ СтрШаблон(
					"И (НЕ &ИспользоватьРасширенныеВозможностиЗаказа
					|	ИЛИ НЕ ОснованиеТранспортнойНакладной.%1)", // @query-part
					ПараметрыФормированияТранспортныхНакладных.ИмяПоляНакладнаяПоЗаказу);
	...
	|	ВнутренныйЗапрос.АдресПогрузкиЗначение КАК АдресПогрузкиЗначение,
	...
	|	ПараметрыТТН.АдресПогрузкиЗначение КАК АдресПогрузкиЗначение,
	...
	Запрос.УстановитьПараметр("СпособыДоставкиПоЗаданиюНаПеревозку", 
		ДоставкаТоваровКлиентСервер.СпособыДоставкиДоКлиентаСНашимУчастием(Ложь));
	...
	ПоляЗаполняемыеПоПервомуЗначению.Добавить("АдресПогрузкиЗначение");
...
//  УсловиеВидимости - Массив из Структура, Структура - поля:
//  		* Реквизит - Строка
...
Процедура ДобавитьКомандыПечати(КомандыПечати, Порядок = Неопределено, УсловиеВидимости = Неопределено) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьТТН")
		...
		КомандаПечати.МенеджерПечати = "Документ.ТранспортнаяНакладная.Печать";
		...
		КомандаПечати.ДополнительныеПараметры.Вставить("ИнтерактивноеДобавление", ИнтерактивноеДобавление);
		КомандаПечати.ДополнительныеПараметры.Вставить("МенеджерПечати", "Документ.ТранспортнаяНакладная.Печать");
		
		Если Порядок <> Неопределено Тогда
			КомандаПечати.Порядок = Порядок;
		КонецЕсли;
		
		Если ТипЗнч(УсловиеВидимости) = Тип("Структура") Тогда
			КомандаПечати.УсловияВидимости.Добавить(УсловиеВидимости);
		ИначеЕсли ТипЗнч(УсловиеВидимости) = Тип("Массив") Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(КомандаПечати.УсловияВидимости, УсловиеВидимости);
		КонецЕсли;
...
Функция СформироватьПечатнуюФормуТТН(МассивОбъектов, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	Для Каждого КлючЗначение Из ТипыОбъектов Цикл
		...
		ТекстыЗапросаУничтоженияТаблиц = Новый Массив;
		
		// ПредыдущиеДокументы
		Если МенеджерВременныхТаблиц.Таблицы.Найти("ПредыдущиеДокументы") <> Неопределено Тогда
			
			ТекстЗапроса = "
			|УНИЧТОЖИТЬ ПредыдущиеДокументы
			|";
			
			ТекстыЗапросаУничтоженияТаблиц.Добавить(ТекстЗапроса);
		...
		Запрос.Текст = СтрСоединить(ТекстыЗапросаУничтоженияТаблиц, ОбщегоНазначения.РазделительПакетаЗапросов());
...
Функция ПредставлениеСпособаДоставки(СпособДоставки, Документ)
	
	Представление = "";
	
	Если СпособДоставки = Перечисления.СпособыДоставки.Самовывоз Тогда
		Представление = НСтр("ru = 'Самовывоз (с нашего склада)'");
	ИначеЕсли СпособДоставки = Перечисления.СпособыДоставки.ДоКлиента Тогда
		...
		Если ТипДокумента = Тип("ДокументСсылка.ПеремещениеТоваров")
			...
			Представление = НСтр("ru = 'Наша транспортная служба до получателя'");
		ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ОтгрузкаТоваровСХранения") Тогда
			Представление = НСтр("ru = 'Наша транспортная служба до владельца'");
		
		Иначе
			Представление = НСтр("ru = 'Наша транспортная служба до клиента'");
		КонецЕсли;
		
	ИначеЕсли СпособДоставки = Перечисления.СпособыДоставки.СиламиПеревозчика Тогда
		Представление = НСтр("ru = 'Перевозчик (с нашего склада)'");
	ИначеЕсли СпособДоставки = Перечисления.СпособыДоставки.СиламиПеревозчикаПоАдресу Тогда
		Представление = НСтр("ru = 'Наша транспортная служба до перевозчика'");
	ИначеЕсли СпособДоставки = Перечисления.СпособыДоставки.КПолучателюОпределяетСлужбаДоставки Тогда
		Представление = НСтр("ru = 'На усмотрение транспортной службы'");
	ИначеЕсли СпособДоставки = Перечисления.СпособыДоставки.СиламиПоставщикаДоНашегоСклада Тогда
		Представление = НСтр("ru = 'Поставщик (на наш склад)'");
	ИначеЕсли СпособДоставки = Перечисления.СпособыДоставки.СиламиПеревозчикаДоНашегоСклада Тогда
		Представление = НСтр("ru = 'Перевозчик (на наш склад)'");
	ИначеЕсли СпособДоставки = Перечисления.СпособыДоставки.СиламиПеревозчикаДоПунктаПередачи Тогда
		Представление = НСтр("ru = 'Наша транспортная служба от перевозчика'");
	ИначеЕсли СпособДоставки = Перечисления.СпособыДоставки.НашимиСиламиСАдресаОтправителя Тогда
		Представление = НСтр("ru = 'Наша транспортная служба от поставщика'");
	ИначеЕсли СпособДоставки = Перечисления.СпособыДоставки.ОтОтправителяОпределяетСлужбаДоставки Тогда
		Представление = НСтр("ru = 'На усмотрение транспортной службы'");
	КонецЕсли;
...
Функция ТекстВыборкиПолейДокументаОснования(Реквизиты, ПараметрыФормированияТранспортныхНакладных, ПолучатьРеквизитыИзЗаказа, ПолучатьРеквизитыЗадания, ЕстьСпособДоставки)
	...
	Для Каждого ИмяРеквизита Из Реквизиты Цикл
		...
		Если (ИмяРеквизита = "АдресДоставки"
			Или ИмяРеквизита = "АдресДоставкиЗначение"
			...
			Если ПараметрыФормированияТранспортныхНакладных.ЕстьЗаказы Тогда
				ТекстПоля =
				"ВЫБОР КОГДА ОснованиеТранспортнойНакладной." + ПараметрыФормированияТранспортныхНакладных.ИмяПоляНакладнаяПоЗаказу + "
				|	И &ИспользоватьРасширенныеВозможностиЗаказа ТОГДА
				...
				|		КОНЕЦ 
				|КОНЕЦ КАК " + ИмяРеквизита; // @query-part
			Иначе
				ТекстПоля = 
				"ВЫБОР
				|	КОГДА ОснованиеТранспортнойНакладной.СпособДоставки В (&СпособыДоставкиПоЗаданиюНаПеревозку) ТОГДА
...
Функция ТекстЗапросаВыборкиТранспортныхНакладныхПоДокументуОснованию(
		ПараметрыФормированияТранспортныхНакладных,
		ПолноеИмяДокумента,
		ВернутьЗапросДляВычисленияПриЗаписи,
		ЕстьСпособДоставки,
		ЭтоФормированиеТекстаГиперссылки = Ложь)
	
	Если ЭтоФормированиеТекстаГиперссылки Тогда
		СвойстваКоторыеОбязательноДолжныСовпадать = "ПометкаУдаления,Проведен";
	Иначе
		
		СвойстваКоторыеОбязательноДолжныСовпадать =
			"Организация,Грузоотправитель,Грузополучатель,ЗаказчикПеревозки,Плательщик,ПометкаУдаления,Проведен";
	...
	ВсеВыбираемыеРеквизиты = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
			СвойстваКоторыеОбязательноДолжныСовпадать,,
			Истина,
			Истина);
