	...
	Если Не ЗначениеЗаполнено(Объект.БанковскийСчетПлательщика) Тогда	
		Если ЗначениеЗаполнено(Объект.Плательщик) Тогда
			...
			Иначе
				...
				ПараметрыСчета.Организация = Объект.Плательщик;
				Объект.БанковскийСчетПлательщика =
...
Процедура УстановитьБанковскийСчетЗаказчикаПеревозки()
	...
	Если Не ЗначениеЗаполнено(Объект.БанковскийСчетЗаказчикаПеревозки) Тогда	
		Если ЗначениеЗаполнено(Объект.ЗаказчикПеревозки) Тогда
			...
			Иначе
				ПараметрыСчета = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации();
				ПараметрыСчета.Организация = Объект.ЗаказчикПеревозки;
				Объект.БанковскийСчетЗаказчикаПеревозки =
					ДенежныеСредстваСервер.ПолучитьБанковскийСчетОрганизацииПоУмолчанию(ПараметрыСчета);
			КонецЕсли;
		КонецЕсли;		
	КонецЕсли;
