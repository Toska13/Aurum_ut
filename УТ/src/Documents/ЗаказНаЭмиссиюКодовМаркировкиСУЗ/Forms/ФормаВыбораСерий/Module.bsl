...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КоличествоСимволовСерии = ДлинаСерийногоНомера(Параметры.Шаблон) - 1;
...
Функция ДлинаСерийногоНомера(Шаблон)
	...
	Если ЗначениеЗаполнено(Шаблон) Тогда
		...
		"ВЫБРАТЬ
		|	МАКСИМУМ (ВЫБОР
		|		КОГДА ЕСТЬNULL(ШаблоныРазбораКодовМаркировкиИСМП.ДлинаСерийногоНомера, 0) > 0
		...
		|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ШаблоныКодовМаркировкиСУЗ.ШаблоныРазбораКодовМаркировки КАК ШаблоныКодовМаркировкиТаблица
		|		ПО ШаблоныКодовМаркировкиСУЗ.Ссылка = ШаблоныКодовМаркировкиТаблица.Ссылка
		|			И НЕ ШаблоныКодовМаркировкиТаблица.Импорт
		...
		|ГДЕ
		|	ШаблоныКодовМаркировкиСУЗ.Ссылка = &Шаблон";
		
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			Результат = Выборка.ДлинаСерийногоНомера;
		КонецЕсли;
	КонецЕсли;
