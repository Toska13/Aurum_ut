...
Процедура ОформленоОрганизацииПриИзменении(Элемент)
	...
	СформироватьГиперссылкуСмТакже();
...
Процедура НастроитьВидимостьДоступностьЭлементовСервер()
	...
	Для Каждого ВидПродукции Из УчитываемыеВидыМаркируемойПродукции Цикл
		...
		КонецЕсли;
		Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМЕХ(ВидПродукции) Тогда
...
Функция СтатусыДляВыбораВФильтре()
	...
	ВключеныТолькоМех = (ВестиУчетМаркируемойПродукцииМЕХ И ВидыПродукцииИСМП.Количество() = 1);
...
Функция СформироватьГиперссылкуСмТакже()
	...
	Если Организации.Количество() Тогда
		...
		Для Каждого ЭлементКоллекции Из Организации Цикл
			ОтборОрганизации.Добавить(ЭлементКоллекции.Значение);
		КонецЦикла;
	КонецЕсли;
	
	ДоступныеНастройки = РегистрыСведений.НастройкиОбменаСУЗ.ДоступныеНастройкиОбменаСУЗ(ОтборОрганизации, Ложь);
	
	ИмяФормыНастройкиОбменаСУЗ = "РегистрСведений.НастройкиОбменаСУЗ.ФормаСписка";
	Если ДоступныеНастройки.Количество() = 0 Тогда
		ТекстГиперссылки = НСтр("ru = 'Настройте параметры подключения к СУЗ'");
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(ТекстГиперссылки,,ЦветаСтиля.ЦветТекстаТребуетВниманияГосИС,,
			ИмяФормыНастройкиОбменаСУЗ));
	Иначе
		ТекстГиперссылки = НСтр("ru = 'Параметры подключения к СУЗ'");
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(ТекстГиперссылки,,,,
			ИмяФормыНастройкиОбменаСУЗ));
	КонецЕсли;
	МассивСтрок.Добавить("; ");
	
	Если ПравоДоступа("Просмотр", Метаданные.РегистрыСведений.ПулКодовМаркировкиСУЗ) Тогда
		ТекстГиперссылки = НСтр("ru = 'Пул кодов маркировки'");
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(ТекстГиперссылки,,,,"РегистрСведений.ПулКодовМаркировкиСУЗ.Форма.ФормаСписка"));
		МассивСтрок.Добавить("; ");
	КонецЕсли;
	
	ТекстГиперссылки = НСтр("ru = 'Активные заказы'");
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(ТекстГиперссылки,,,,
			"Обработка.АктивныеЗаказыСУЗ.Форма"));
