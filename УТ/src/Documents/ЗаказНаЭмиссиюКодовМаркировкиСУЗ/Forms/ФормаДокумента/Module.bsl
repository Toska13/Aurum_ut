...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ОбновитьГиперссылкуНастроитьПодключениеСУЗ" Тогда
		СформироватьПредставлениеСтатусНастройкиСУЗ();
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(Объект.ТипОплатыКодовМаркировки)
		И Не ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеТипОплатыКодовМаркировки(Объект.ТоварнаяГруппа) Тогда
	...
	Если ЭтоМаркировкаОстатков Тогда
		...
		ПолноеОписание = ИнтеграцияИСМПКлиентСервер.ИспользованиеСпособаОписанияОстатков(Объект.ТоварнаяГруппа);
...
Процедура СтатусПредставлениеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "НастроитьПодключениеСУЗ" Тогда
		
		ПараметрыФормы = Новый Структура("Организация, ПроизводственныйОбъект", Объект.Организация, Объект.ПроизводственныйОбъект);
		ОткрытьФорму("РегистрСведений.НастройкиОбменаСУЗ.ФормаЗаписи", ПараметрыФормы, ЭтотОбъект);
...
Процедура ПроизводственныйОбъектАдресСтрокойПриИзменении(Элемент)
	...
	Объект.ПроизводственныйОбъектАдрес = ОбщегоНазначенияИСВызовСервера.ЗначенияПолейКонтактнойИнформации(
	...
	Если ПредыдущийСпособВводаВОборот <> Объект.СпособВводаВОборот Тогда
		...
		Если Объект.БезУчетаНоменклатуры
			И (Не ЭтоМаркировкаОстатков
				Или Не ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеБезУчетаНоменклатуры(
					ВидыПродукцииПоТоварнойГруппе, Объект.СпособВводаВОборот)) Тогда
			Объект.БезУчетаНоменклатуры = Ложь;
		КонецЕсли;
		
		Если Объект.ВвезенПослеДатыОбязательнойМаркировки
			И (Не ЭтоМаркировкаОстатков
				Или Не ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеВвезенПослеДатыОбязательнойМаркировки(
					ВидыПродукцииПоТоварнойГруппе, Объект.СпособВводаВОборот)) Тогда
			Объект.ВвезенПослеДатыОбязательнойМаркировки = Ложь;
		КонецЕсли;
		
		Если Объект.ПриобретенПроизведенПослеДатыОбязательнойМаркировки
			И (Не ЭтоМаркировкаОстатков
				Или Не ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеПриобретенПроизведенПослеДатыОбязательнойМаркировки(
					ВидыПродукцииПоТоварнойГруппе, Объект.СпособВводаВОборот)) Тогда
			Объект.ПриобретенПроизведенПослеДатыОбязательнойМаркировки = Ложь;
		КонецЕсли;
		
		Если Не ЭтоМаркировкаОстатков
			И ПредыдущийСпособВводаВОборот = ПредопределенноеЗначение("Перечисление.СпособыВводаВОборотСУЗ.МаркировкаОстатков")
			Или ЭтоМаркировкаОстатков
...
Процедура СервисПровайдерОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ТекстОшибки = "";
	Если Не ПроверкаВыбранногоСервисПровайдера(ВыбранноеЗначение, Объект.ТоварнаяГруппа, Объект.СпособВводаВОборот, ТекстОшибки) Тогда
		СтандартнаяОбработка = Ложь;
	...
	ИначеЕсли Элемент.ТекущийЭлемент = Элементы.ТоварыШаблон Тогда
		...
		Если ТекущиеДанные <> Неопределено
			...
			Иначе
				Режим = 3;
			КонецЕсли;
			ДоступныеШаблоны = ИнтеграцияИСМПКлиентСервер.ШаблоныКодовПоВидуПродукции(Объект.ТоварнаяГруппа, Режим);
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Или Копирование Тогда
		...
		Если Копирование И ЗначениеЗаполнено(ТекущиеДанные.ИдентификаторСтрокиВладельца) Тогда
			...
			ТекущиеДанные.ПредставлениеШаблона         = "";
		КонецЕсли;
...
Процедура ЗаполнитьGLNОргранизацииПоСтатистике(ОчищатьПустое = Истина)
	...
	Если Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(Объект.ТоварнаяГруппа)
...
Процедура ЗаполнитьКоличествоЭмитированоВТоварах()
	...
	Для Каждого СтрокаТовары Из Объект.Товары Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТовары.СпособВводаВОборот) Тогда
...
Процедура НастроитьЭлементыФормыНаСервере()
	...
	Элементы.СпособВыпускаВОборот.СписокВыбора.ЗагрузитьЗначения(
		ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступныеСпособыВводаВОборот(Объект.ТоварнаяГруппа));
	
	Элементы.СброситьПризнакПечати.Видимость = ИнтеграцияИСМП.РазрешенаПовторнаяПечатьКодовМаркировки();
	
	Если ЗаказНаЭмиссиюСУЗКлиентСервер.ПроверятьЗаполнениеСпособаВводаВОборот(Объект.ТоварнаяГруппа) Тогда
		Элементы.СпособВыпускаВОборот.АвтоОтметкаНезаполненного = Истина;
		Элементы.СпособВыпускаВОборот.ОтметкаНезаполненного = Истина;
		Элементы.СпособВыпускаВОборот.ПодсказкаВвода = "";
	Иначе
		Элементы.СпособВыпускаВОборот.АвтоОтметкаНезаполненного = Ложь;
		Элементы.СпособВыпускаВОборот.ОтметкаНезаполненного = Ложь;
		Элементы.СпособВыпускаВОборот.ПодсказкаВвода = НСтр("ru = 'Производство или импорт'");
	КонецЕсли;
	
	ДатаОбязательнойМаркировки = ИнтеграцияИСМПКлиентСерверПовтИсп.ДатаОбязательнойМаркировкиПродукции(Объект.ТоварнаяГруппа);
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	Если ИспользуетсяПроизводственныйОбъект Тогда
	...
	Если Не ПодключенаОбработкаКодовМаркировки Тогда
		ПроверкаИПодборПродукцииИС.ПодключитьОбработкуКодовМаркировки(ЭтотОбъект, Ложь, КлючСвязиСтатусаСтрок());
...
Процедура ОбработкаРезультатаПодбораНоменклатуры(ВыбранноеЗначение, КэшированныеЗначения)
	...
	ПараметрыЗаполнения = ИнтеграцияИС.ПараметрыЗаполненияТабличнойЧастиТовары(ВидыПродукцииПоТоварнойГруппе);
...
Процедура ЗаполнитьТаблицуПредставленийНоменклатурыНаКлиенте()
	...
	Если ЭтоМаркировкаОстатков
		Или КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(Объект.ТоварнаяГруппа) Тогда
		ЗаполнитьТаблицуПредставленийНоменклатуры();
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуПредставленийНоменклатуры()
	...
	ИначеЕсли КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(Объект.ТоварнаяГруппа) Тогда
...
Процедура ЗаполнитьТаблицуПредставленийНоменклатурыТоварыИзНатуральногоМеха()
	...
	Для Каждого СтрокаТовары Из Объект.Товары Цикл
		...
		Иначе
			СтрокаТовары.ПредставлениеGTINОстатки =
				ИнтеграцияИСМПКлиентСервер.ПредставлениеПустойНоменклатурыПродукцииПоддерживающейИндивидуализациюКИЗ(
					СтрокаТовары);
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбновитьСтатусИСМП()
	...
	СтатусДействияПредставление = ОбменДаннымиИСМП.ПредставлениеСтатуса(
		Статус,
		ДальнейшееДействие,
		ДопустимыеДействия);
...
Процедура ОбновитьПредставлениеСтатуса()
	...
	Если ЗначениеЗаполнено(СтатусНастройкиСУЗ) Тогда
		МассивСтрок = Новый Массив;
		МассивСтрок.Добавить(СтатусДействияПредставление);
		МассивСтрок.Добавить(" ");
		МассивСтрок.Добавить(СтатусНастройкиСУЗ);
		СтатусПредставление = Новый ФорматированнаяСтрока(МассивСтрок);
	Иначе
		СтатусПредставление = СтатусДействияПредставление;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ЧастиКодовТНВЭД =  МаркировкаТоваровКлиентСервер.ЧастиКодовТНВЭДПоляВозрастнаяКатегорияДляМаркировкиОстатков(Объект.ТоварнаяГруппа);
	Для Каждого ЧастьКодаТНВЭД Из ЧастиКодовТНВЭД Цикл
		
		ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных(Элементы.ТоварыКодТНВЭД.ПутьКДанным);
		ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.НеНачинаетсяС;
		ОтборЭлемента.ПравоеЗначение = ЧастьКодаТНВЭД;
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", 
		Новый ПолеКомпоновкиДанных(Элементы.ТоварыПредставлениеШаблона.ПутьКДанным));
	...
	ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных(Элементы.ТоварнаяГруппа.ПутьКДанным);
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.ВСписке;
...
Процедура ОбновитьВидимостьДоступностьЭлементовФормы(Форма)
	...
	ЭтоМех = КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ВидыПродукцииПоТоварнойГруппе);
	
	Элементы.ТоварыМаксимальнаяРозничнаяЦена.Видимость =
		КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(ВидыПродукцииПоТоварнойГруппе);
	Элементы.ПроизводственныйОбъектИдентификатор.Видимость =
		ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеПроизводственныйОбъектИдентификатор(
			ВидыПродукцииПоТоварнойГруппе, Объект.СпособВводаВОборот);
	Элементы.ИдентификаторПроизводственнойЛинии.Видимость =
		ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеИдентификаторПроизводственнойЛинии(
			ВидыПродукцииПоТоварнойГруппе, Объект.СпособВводаВОборот);
	Элементы.ОжидаемаяДатаНачалаПроизводства.Видимость =
		ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеОжидаемаяДатаНачалаПроизводства(
			ВидыПродукцииПоТоварнойГруппе, Объект.СпособВводаВОборот);
	Элементы.ПредставлениеАдресаПроизводственногоОбъекта.Видимость =
		ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеПредставлениеАдресаПроизводственногоОбъекта(
			ВидыПродукцииПоТоварнойГруппе, Объект.СпособВводаВОборот)
		Или ЭтоМех И Объект.СпособПолучения = ПредопределенноеЗначение(
			"Перечисление.СпособыПолученияИзготовленныхКИЗГИСМ.ДоставкаЭмитентомДоУказанногоМеста");
	
	Форма.ИспользуютсяНаборы            = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетНаборы(ВидыПродукцииПоТоварнойГруппе);
	Форма.ИспользуютсяГрупповыеУпаковки = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетГрупповыеУпаковки(ВидыПродукцииПоТоварнойГруппе);
	Форма.ШаблоныГрупповойУпаковки      = ИнтеграцияИСМПКлиентСервер.ШаблоныКодовПоВидуПродукции(Объект.ТоварнаяГруппа, 4);
	Форма.ШаблоныНабора                 = ИнтеграцияИСМПКлиентСервер.ШаблоныКодовПоВидуПродукции(Объект.ТоварнаяГруппа, 5);
	...
	Форма.ИспользованиеСервисПровайдераЗапрещено = Не ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеСервисПровайдер(
		ВидыПродукцииПоТоварнойГруппе, Объект.СпособВводаВОборот);

	Элементы.ГруппаСервисПровайдер.Видимость = Не Форма.ИспользованиеСервисПровайдераЗапрещено;
	Элементы.СервисПровайдер.Доступность     = Форма.ПечатьКодовМаркировки = 1;
	...
	Элементы.БезУчетаНоменклатуры.Видимость =
		ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеБезУчетаНоменклатуры(
			ВидыПродукцииПоТоварнойГруппе, Объект.СпособВводаВОборот);
	
	Элементы.ВвезенПослеДатыОбязательнойМаркировки.Видимость =
		ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеВвезенПослеДатыОбязательнойМаркировки(
			ВидыПродукцииПоТоварнойГруппе, Объект.СпособВводаВОборот);
	
	Элементы.ПриобретенПроизведенПослеДатыОбязательнойМаркировки.Видимость =
		ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеПриобретенПроизведенПослеДатыОбязательнойМаркировки(
			ВидыПродукцииПоТоварнойГруппе, Объект.СпособВводаВОборот);
	...
	Если Форма.ЭтоМаркировкаОстатков Тогда
		...
		ПараметрыОписанияОстатков =
			ИнтеграцияИСМПКлиентСервер.ИспользованиеПараметровОписанияОстатковПоВидуПродукции(
				ВидыПродукцииПоТоварнойГруппе);
		
		Элементы.ЗаполнитьПараметрыОписанияОстатков.Видимость = ПолноеОписание <> Истина;
		
		Элементы.ТоварыПолноеОписаниеОстатков.Видимость = ПолноеОписание = Неопределено;
		Элементы.ТоварыКодТНВЭД.Видимость               = ПолноеОписание <> Истина;
		...
		Элементы.ТоварыТоварныйЗнак.Видимость           = ПараметрыОписанияОстатков.ИспользоватьТоварныйЗнак;
	...
	Иначе
		...
		Элементы.ТоварыТоварныйЗнак.Видимость =
			ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеТоварныйЗнак(ВидыПродукцииПоТоварнойГруппе);
		
	КонецЕсли;
	
	Элементы.ТоварыШаблон.СписокВыбора.Очистить();
	ДоступныеШаблоны = ИнтеграцияИСМПКлиентСервер.ШаблоныКодовПоВидуПродукции(Объект.ТоварнаяГруппа, 3);
	Для Каждого ЭлементСпискаЗначений Из ДоступныеШаблоны Цикл
		Элементы.ТоварыШаблон.СписокВыбора.Добавить(
			ЭлементСпискаЗначений.Значение, ЭлементСпискаЗначений.Представление);
	КонецЦикла;
	...
	Элементы.ТоварыСрокГодности.Видимость = ЕстьЗаполненныйСрокГодности(Форма);
	...
	ИспользуетсяТипОплатыКодовМаркировки =
		ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеТипОплатыКодовМаркировки(ВидыПродукцииПоТоварнойГруппе);
	
	Элементы.ТипОплатыКодовМаркировки.Видимость = ИспользуетсяТипОплатыКодовМаркировки;
	
	Если ИспользуетсяТипОплатыКодовМаркировки Тогда
		Если Объект.СпособВводаВОборот = ПредопределенноеЗначение("Перечисление.СпособыВводаВОборотСУЗ.МаркировкаВнеПроизводстваИмпорта") Тогда
			Объект.ТипОплатыКодовМаркировки = ПредопределенноеЗначение("Перечисление.ТипыОплатыКодовМаркировкиСУЗ.ОплатаПоЭмиссии");
			Элементы.ТипОплатыКодовМаркировки.Доступность = Ложь;
		Иначе
			Элементы.ТипОплатыКодовМаркировки.Доступность = Истина;
			Если Не ЗначениеЗаполнено(Объект.ТипОплатыКодовМаркировки) Тогда
	...
	Элементы.ТоварыВладелецGTINСтрокой.Видимость = ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеВладелецGTIN(
		ВидыПродукцииПоТоварнойГруппе, Объект.СпособВводаВОборот);
...
Функция ДопустимыйФорматШтрихкода(ДанныеШтрихкодов)
	...
	Если ОбработкаСтрокиXML.НайтиНедопустимыеСимволыXML(ДанныеШтрихкодов.Штрихкод) > 0 Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура ТоварнаяГруппаПриИзмененииНаКлиенте(ПерезаполнитьПоОснованию = Ложь)
	...
	ТоварнаяГруппаПриИзмененииНаСервере(ПерезаполнитьПоОснованию);
...
&НаСервере
Процедура ТоварнаяГруппаПриИзмененииНаСервере(ПерезаполнитьПоОснованию)
	...
	Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(Объект.ТоварнаяГруппа) Тогда
...
Процедура ТоварыНоменклатураХарактеристикаПриИзмененииСервер(
			...
			Если ЭтоМаркировкаОстатков Тогда
				...
				Иначе
					...
					ПараметрыОписанияОстатков = ИнтеграцияИСМПКлиентСервер.ИспользованиеПараметровОписанияОстатковПоВидуПродукции(
						Объект.ТоварнаяГруппа,
						ИнтеграцияИСМПКлиентСервер.КодТНВЭДДляПередачиВИСМП(ДанныеСтроки.КодТНВЭД, Объект.ТоварнаяГруппа));
	...
	Если ОбновлятьПредставления И ЭтоМаркировкаОстатков Тогда
		ОбновитьДанныеФормыМаркировкаОстатковСервер();
	КонецЕсли;
...
//  ДополнительныеПараметры - Неопределено, Произвольный - не используется.
&НаКлиенте
Процедура ЗаполнитьДоступныеGTINНаКлиенте(РезультатАвторизации = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ТребуетсяОбновитьКлючСессии
		...
		ПараметрыАвторизации.Параметры             = Объект.ТоварнаяГруппа;
...
&НаКлиентеНаСервереБезКонтекста
Функция ЕстьЗаполненныйСрокГодности(Форма)
...
Процедура ОбработатьКодМаркировки(ИсходныеДанные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИСКлиент.ПараметрыСканирования(ЭтотОбъект,, ВидыПродукцииПоТоварнойГруппе);
...
Процедура Подключаемый_ЗаполнениеПараметровОстатковЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ВыделенныеСтроки.Количество() > 0 Тогда
		ЗаполнениеПараметровОстатковЗавершениеСервер(ОбщегоНазначенияКлиент.СкопироватьРекурсивно(ВыделенныеСтроки));
	Иначе
		ОбновитьДанныеФормыМаркировкаОстатков();
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнениеПараметровОстатковЗавершениеСервер(ИдентификаторыСтрок)
	
	Для Каждого СтрокаСписка Из ИдентификаторыСтрок Цикл
		ТоварыНоменклатураХарактеристикаПриИзмененииСервер(
			Элементы.ЗаполнитьПараметрыОписанияОстатков.Имя,, СтрокаСписка, Ложь);
	КонецЦикла;
...
Процедура ЗаполнитьФлагЭтоМаркировкаОстатков(Форма)
	...
	Форма.ЭтоМаркировкаОстатков = Форма.Объект.СпособВводаВОборот = ПредопределенноеЗначение(
...
Процедура ПроверитьОчиститьСервисПровайдера(Форма)
	...
	Если Не ЗначениеЗаполнено(Объект.ТоварнаяГруппа) Или Не ЗначениеЗаполнено(Объект.СпособВводаВОборот) Тогда
		Объект.СервисПровайдер = Неопределено;
		Возврат;
	КонецЕсли;
	
	Если Не ПроверкаВыбранногоСервисПровайдера(Объект.СервисПровайдер, Объект.ТоварнаяГруппа, Объект.СпособВводаВОборот) Тогда
		Объект.СервисПровайдер = Неопределено;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПроверкаВыбранногоСервисПровайдера(СервисПровайдер, ТоварнаяГруппа, СпособВводаВОборот, ТекстОшибки = Неопределено)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СервисПровайдерыСУЗТоварныеГруппы.ТоварнаяГруппа КАК ТоварнаяГруппа
	|ИЗ
	|	Справочник.СервисПровайдерыСУЗ.ТоварныеГруппы КАК СервисПровайдерыСУЗТоварныеГруппы
	|ГДЕ
	|	СервисПровайдерыСУЗТоварныеГруппы.Ссылка = &СервисПровайдер
	|УПОРЯДОЧИТЬ ПО
	|	СервисПровайдерыСУЗТоварныеГруппы.НомерСтроки";
	...
	// проверим соответствие по товарной группе
	ДоступныеТоварныеГруппы = РезультатЗапроса[1].Выгрузить().ВыгрузитьКолонку("ТоварнаяГруппа");
	
	Если ДоступныеТоварныеГруппы.Найти(ТоварнаяГруппа) = Неопределено Тогда
		
		Результат = Ложь;
		
		Если ФормироватьТекстОшибки Тогда
			
			ТекстСообщения = СтрШаблон(НСтр("ru = ' Недоступна товарная группа <%1>'"), ТоварнаяГруппа);
			ТекстСообщения = ТекстСообщения + Символы.ПС + НСтр("ru = ' доступные товарные группы:'");
			
			Счетчик = 0;
			КоличествоВыводимыхСтрок = 3;
			КоличествоДоступныхТоварныхГрупп = ДоступныеТоварныеГруппы.Количество();
			
			Если КоличествоДоступныхТоварныхГрупп = КоличествоВыводимыхСтрок + 1 Тогда
				КоличествоВыводимыхСтрок = КоличествоВыводимыхСтрок + 1;
			КонецЕсли;
			
			Для Каждого ДоступнаяТоварнаяГруппа Из ДоступныеТоварныеГруппы Цикл
				...
				Если Счетчик = КоличествоВыводимыхСтрок Тогда
					ТекстСообщения = ТекстСообщения + Символы.ПС + 
						СтрШаблон(НСтр("ru = '     (+ еще %1) - можно просмотреть в карточке сервис-провайдера'"),
							КоличествоДоступныхТоварныхГрупп - Счетчик);
					Прервать;
				КонецЕсли;
				
				ТекстСообщения = ТекстСообщения + Символы.ПС + "     " + ДоступнаяТоварнаяГруппа;
...
//   Значения по сервис-провайдерам указываются для РФ (СправочникСсылка.СтраныМира.Россия) и прочих (ПустаяСсылка)
//
// Возвращаемое значение:
//  Соответствие Из ПеречислениеСсылка.ВидыСервисПровайдеровСУЗ - виды сервис провайдеров с указанием доступных способов ввода в оборот:
//   * Ключ - ПеречислениеСсылка.ВидыСервисПровайдеровСУЗ.
//   * Значение - Соответствие Из СправочникСсылка.СтраныМира:
//    ** Ключ - СправочникСсылка.СтраныМира - 
//    ** Значение - Массив Из ПеречислениеСсылка.СпособыВводаВОборотСУЗ - ПеречислениеСсылка.СпособыВводаВОборотСУЗ.
//
&НаКлиентеНаСервереБезКонтекста
...
Процедура УстановитьПризнакНабораИГрупповойУпаковки(Форма, МассивСтрок)
	...
	Для Каждого Строка Из МассивСтрок Цикл
		Если ЗначениеЗаполнено(Строка.ИдентификаторСтрокиВладельца) Тогда
			...
			КонецЕсли;
			ЗаказНаЭмиссиюСУЗКлиентСервер.УстановитьПредставлениеШаблонаВСтроке(Строка);
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьСоставляющиеНаборов(МассивСтрок, КэшированныеЗначения, ДобавленныеСтроки)
	...
	ДоступныеШаблоны = ИнтеграцияИСМПКлиентСервер.ШаблоныКодовПоВидуПродукции(Объект.ТоварнаяГруппа, 0);
	...
	Для Каждого КомплектующаяНабора Из КомплектующиеНаборов Цикл
		...
		ЗаказНаЭмиссиюСУЗКлиентСервер.УстановитьПредставлениеШаблонаВСтроке(ТекущаяСтрока);
	...
	Для Каждого Строка Из МассивСтрок Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТовары.ИдентификаторСтрокиВладельца)
			...
			Если ИспользуютсяНаборы И ШаблонЕстьВСписке(СтрокаТовары.Шаблон, ШаблоныНабора)
				...
				СтрокаТовары.ПредставлениеШаблона         = "";
			...
			ИначеЕсли ЗначениеЗаполнено(СтрокаТовары.ИдентификаторСтрокиВладельца) Тогда
				...
				Если СтрокаВладелец <> Неопределено Тогда
					...
					СтрокаВладелец.ПредставлениеШаблона         = "";
...
Процедура ВопросИзменениеГрупповойУпаковкиНаПотребительскуюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ДанныеСтроки.ПредставлениеШаблона         = "";
...
Процедура ДобавитьСоставляющиеГрупповойУпаковки(МассивСтрок)
	...
	Для Каждого Строка из МассивСтрок Цикл
		...
		ЗаказНаЭмиссиюСУЗКлиентСервер.УстановитьПредставлениеШаблонаВСтроке(НоваяСтрока);
...
Процедура ПроверитьОчиститьВладельцаGTIN(Форма, Данные = Неопределено)
	...
	ИспользуетсяВладелецGTIN = ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеВладелецGTIN(
		Форма.Объект.ТоварнаяГруппа, Форма.Объект.СпособВводаВОборот);
...
&НаСервере
Процедура СформироватьПредставлениеСтатусНастройкиСУЗ()
	
	СтатусНастройкиСУЗ = Неопределено;
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		
		ДоступныеНастройки = РегистрыСведений.НастройкиОбменаСУЗ.ДоступныеНастройкиОбменаСУЗ(Объект.Организация, Ложь);
		
		Если ДоступныеНастройки.Количество() = 0 Тогда
			ИмяФормыНастройкиОбменаСУЗ = "НастроитьПодключениеСУЗ";
			ТекстГиперссылки = НСтр("ru = 'Настроить параметры подключения к СУЗ'");
			СтатусНастройкиСУЗ = Новый ФорматированнаяСтрока(ТекстГиперссылки,,ЦветаСтиля.ЦветТекстаТребуетВниманияГосИС,,
				ИмяФормыНастройкиОбменаСУЗ);
		КонецЕсли;
