...
//  Массив Из ПеречислениеСсылка.СтатусыОбработкиПеремаркировкиТоваровИСМП - Конечные статусы.
...
//
// Возвращаемое значение:
//  Массив Из ПеречислениеСсылка.СтатусыОбработкиПеремаркировкиТоваровИСМП - Статусы ошибок.
...
Функция ДальнейшееДействиеПоУмолчанию(СтруктураПараметров=Неопределено) Экспорт
	...
	Если ТипЗнч(СтруктураПараметров) = Тип("Структура") Тогда
		...
		ВидыПродукции = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(Реквизиты.ТоварнаяГруппа);
		Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияПоддерживающаяИндивидуализациюКИЗ(ВидыПродукции)
...
// 
// Параметры:
//  Объект - ДокументОбъект.ПеремаркировкаТоваровИСМП - Объект
// 
// Возвращаемое значение:
...
Функция СтатусПослеПодготовкиКПередачеДанных(ДокументСсылка, Операция, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Операция = Перечисления.ВидыОперацийИСМП.УдалитьСписаниеВведенныхВОборотКодовМаркировки Тогда
...
Функция СтатусПослеПолученияДанных(ДокументСсылка, Операция, ДополнительныеПараметры) Экспорт
	...
	Если Операция = Перечисления.ВидыОперацийИСМП.ПолучениеРезультатаОбработкиДокумента
		И ДополнительныеПараметры.ОперацияКвитанции = Перечисления.ВидыОперацийИСМП.УдалитьСписаниеВведенныхВОборотКодовМаркировки Тогда
		
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			ДокументСсылка,
			"ТоварнаяГруппа, ТребуетсяИндивидуализацияКИЗ");
		
		Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияПоддерживающаяИндивидуализациюКИЗ(Реквизиты.ТоварнаяГруппа)
			И Реквизиты.ТребуетсяИндивидуализацияКИЗ Тогда
	...
	Для Каждого СтрокаТЧТовары Из Товары Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТЧТовары.КодМаркировки) Тогда
			РезультатРазбора = ШтрихкодированиеОбщегоНазначенияИС.НоваяСтруктураОбработкиШтрихкода(
				СтрокаТЧТовары.КодМаркировки, Шапка.ТоварнаяГруппа, ПользовательскиеПараметрыРазбораКодаМаркировки);
			СтрокаТЧ.Вставить(
				"last_uin",
		...
		РезультатРазбора = ШтрихкодированиеОбщегоНазначенияИС.НоваяСтруктураОбработкиШтрихкода(
			СтрокаТЧТовары.НовыйКодМаркировки, Шапка.ТоварнаяГруппа, ПользовательскиеПараметрыРазбораКодаМаркировки);
		СтрокаТЧ.Вставить(
			"new_uin",
			ОбменДаннымиИСМП.КодМаркировкиДляПередачиИСМП(РезультатРазбора, ПараметрыНормализацииПрочее));
		...
		// Сертификация
		Если ПеремаркировкаТоваровКлиентСервер.ДоступноПолеСертификация(Шапка.ТоварнаяГруппа)
			И ЗначениеЗаполнено(СтрокаТЧТовары.ВидДокументаСертификации) Тогда
			СтрокаТЧ.Вставить("certificate_document",   ИнтерфейсИСМПОбщегоНазначения.ВидДокументаСертификации(СтрокаТЧТовары.ВидДокументаСертификации));
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ОбновлениеИнформационнойБазыГосИС.ДобавитьОбработчикОбновленияПереходаНаКонструкторНастроек(
		...
		Новый УникальныйИдентификатор("20430277-54e9-4200-b295-d005ab50f7f9"));
...
Функция ЕстьКодыМаркировкиДляБлокировки(Объект)
	...
	Если ПеремаркировкаТоваровКлиентСервер.ДоступнаБлокировкаКодовМаркировки(ВидыПродукции) Тогда
		Для Каждого СтрокаТовары Из Объект.Товары Цикл
			Если ЗначениеЗаполнено(СтрокаТовары.КодМаркировки) И (СтрокаТовары.ПричинаПеремаркировки
				= Перечисления.ПричиныПеремаркировкиТоваровИСМП.ИспорченоУтрачено
				Или СтрокаТовары.ПричинаПеремаркировки
				= Перечисления.ПричиныПеремаркировкиТоваровИСМП.ОшибкиОписанияТовара) Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ОбновитьДанныеКэшаКодовМаркировки(Объект, ДанныеКэша) Экспорт
	
	МассивКодовМаркировки = Новый Массив;
	Для Каждого Строка Из Объект.Товары Цикл
		Если ЗначениеЗаполнено(Строка.КодМаркировки)
			И ДанныеКэша.ДанныеКодовМаркировки[Строка.КодМаркировки] = Неопределено Тогда
			МассивКодовМаркировки.Добавить(Строка.КодМаркировки);
		КонецЕсли;
		Если ЗначениеЗаполнено(Строка.НовыйКодМаркировки)
			И ДанныеКэша.ДанныеКодовМаркировки[Строка.НовыйКодМаркировки] = Неопределено Тогда
			МассивКодовМаркировки.Добавить(Строка.НовыйКодМаркировки);
		КонецЕсли;
	КонецЦикла;
	
	Если МассивКодовМаркировки.Количество() > 0 Тогда
		МассивШтрихкодов = Новый Массив;
		ЗначенияРеквизитовОбъектов = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(МассивКодовМаркировки, "ЗначениеШтрихкода");
		Для Каждого КлючИЗначение Из ЗначенияРеквизитовОбъектов Цикл
			Если ЗначениеЗаполнено(КлючИЗначение.Значение.ЗначениеШтрихкода) Тогда
				ДанныеШтрихкода = Новый Структура;
				ДанныеШтрихкода.Вставить("Штрихкод",     КлючИЗначение.Значение.ЗначениеШтрихкода);
		...
		Если МассивШтрихкодов.Количество() > 0 Тогда
			...
			РезультатОбработкиШтрихкодов = ШтрихкодированиеОбщегоНазначенияИС.ОбработатьШтрихкоды(
				МассивШтрихкодов, ПараметрыСканирования, Неопределено, Новый УникальныйИдентификатор);
			Для Каждого КлючИЗначение Из ЗначенияРеквизитовОбъектов Цикл
				Если ЗначениеЗаполнено(КлючИЗначение.Значение.ЗначениеШтрихкода) Тогда
					ДанныеКодаМаркировки = Новый Структура;
					ДанныеКодаМаркировки.Вставить("ЗначениеШтрихкода", КлючИЗначение.Значение.ЗначениеШтрихкода);
					ДанныеЗначенияШтрихкода = РезультатОбработкиШтрихкодов.РезультатыОбработки[КлючИЗначение.Значение.ЗначениеШтрихкода];
					Если ДанныеЗначенияШтрихкода = Неопределено
						Или ДанныеЗначенияШтрихкода.ДанныеШтрихкода = Неопределено Тогда
						ДанныеКодаМаркировки.Вставить("ВидУпаковки", Неопределено);
					Иначе
						ДанныеКодаМаркировки.Вставить("ВидУпаковки", ДанныеЗначенияШтрихкода.ДанныеШтрихкода.ВидУпаковки);
					КонецЕсли;
					ДанныеКэша.ДанныеКодовМаркировки.Вставить(КлючИЗначение.Ключ, ДанныеКодаМаркировки);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Функция БлокировкаКодовМаркировкиJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	Операция = Перечисления.ВидыОперацийИСМП.УдалитьСписаниеВведенныхВОборотКодовМаркировки;
	...
	Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(ВидыПродукции) Тогда
		...
		Для Каждого СтрокаТЧТовары Из Товары Цикл
			...
			СтрокаТЧ = Новый Структура;
			РезультатРазбора = ШтрихкодированиеОбщегоНазначенияИС.НоваяСтруктураОбработкиШтрихкода(
				СтрокаТЧТовары.Штрихкод, Шапка.ТоварнаяГруппа, ПользовательскиеПараметрыРазбораКодаМаркировки);
...
Функция ИнициализироватьРезультатОбработкиШтрихкода(ДанныеШтрихкода)
