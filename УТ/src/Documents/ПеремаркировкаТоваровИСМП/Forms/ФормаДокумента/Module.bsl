...
Процедура ТоварнаяГруппаПриИзменении(Элемент)
	...
	ПерезаполнитьПоОснованию = (Объект.Товары.Количество() = 0 И ЗначениеЗаполнено(Объект.ДокументОснование));
	
	ТоварнаяГруппаПриИзмененииНаКлиенте(Элементы.ТоварнаяГруппа, ПерезаполнитьПоОснованию);
...
Процедура ТоварыНоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СобытияФормИСКлиентПереопределяемый.ПриНачалеВыбораНоменклатуры(Элемент, Объект.ТоварнаяГруппа, СтандартнаяОбработка);
...
Процедура ОбновитьКэшМаркируемойПродукцииСервер()
	...
	ОбновитьДанныеФормыНаОснованииКэшаМаркируемойПродукции();
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьДанныеФормыНаОснованииКэшаМаркируемойПродукции()
	...
	Если ЭтоАдресВременногоХранилища(КэшМаркируемойПродукции) Тогда
		...
		Если ДанныеКэша <> Неопределено Тогда
			ОбновитьДанныеКэшаКодовМаркировки(ДанныеКэша);
			Для Каждого Строка Из Объект.Товары Цикл
				Если ЗначениеЗаполнено(Строка.НовыйКодМаркировки) Тогда
					ДанныеКодаМаркировки = ДанныеКэша.ДанныеКодовМаркировки[Строка.НовыйКодМаркировки];
				ИначеЕсли ЗначениеЗаполнено(Строка.КодМаркировки) Тогда
					ДанныеКодаМаркировки = ДанныеКэша.ДанныеКодовМаркировки[Строка.КодМаркировки];
				Иначе
					ДанныеКодаМаркировки = Неопределено;
				КонецЕсли;
				Если ДанныеКодаМаркировки = Неопределено Тогда
					Строка.ВидУпаковки = Перечисления.ВидыУпаковокИС.ПустаяСсылка();
				Иначе
					Строка.ВидУпаковки = ДанныеКодаМаркировки.ВидУпаковки;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьДанныеКэшаКодовМаркировки(ДанныеКэша)
	
	Если Не ДанныеКэша.Свойство("ДанныеКодовМаркировки") Тогда
		ДанныеКэша.Вставить("ДанныеКодовМаркировки", Новый Соответствие);
	КонецЕсли;
	
	КоличествоДанных = ДанныеКэша.ДанныеКодовМаркировки.Количество();
	
	Документы.ПеремаркировкаТоваровИСМП.ОбновитьДанныеКэшаКодовМаркировки(Объект, ДанныеКэша);
	
	Если ДанныеКэша.ДанныеКодовМаркировки.Количество() > КоличествоДанных Тогда
...
Процедура УстановитьУсловноеОформление()
	...
	ВидыУпаковокНаборИГрупповая = Новый СписокЗначений;
	ВидыУпаковокНаборИГрупповая.Добавить(Перечисления.ВидыУпаковокИС.Набор);
	ВидыУпаковокНаборИГрупповая.Добавить(Перечисления.ВидыУпаковокИС.Групповая);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
		ЭлементУсловногоОформления.Отбор,
		"Объект.Товары.ВидУпаковки",
		ВидыУпаковокНаборИГрупповая,
	...
	ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
	Поле = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
	Поле.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыКодТНВЭД.Имя);
	...
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных(Элементы.ТоварыМарка.ПутьКДанным);
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.Заполнено;
	
	ОтборЭлемента = ОтборГруппа.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Объект.Товары.ВидУпаковки");
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.ВСписке;
	ОтборЭлемента.ПравоеЗначение = ВидыУпаковокНаборИГрупповая;
...
Процедура ОбработатьКодМаркировки(ИсходныеДанные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ДанныеРазбора = РазборКодаМаркировкиИССлужебныйКлиент.РазобратьКодМаркировки(ИсходныеДанные.Штрихкод, Объект.ТоварнаяГруппа);
...
Функция ОбработатьВводШтрихкода(ШтрихкодКоличество, КэшированныеЗначения, ПараметрыСканирования = Неопределено)
	...
	Если РезультатОбработкиШтрихкода.ДополнительныеПараметры <> Неопределено
		...
		Для Каждого ВидПродукции Из РезультатОбработкиШтрихкода.ВидыПродукции Цикл
			РезультатОбработкиШтрихкода.Вставить("ТоварнаяГруппа", ВидПродукции);
		КонецЦикла;
...
Функция ПроверитьЗаполнениеПоВидуДействия(ВыполняемоеДействие)
	...
	Если ВыполняемоеДействие = "ВыполнитьИндивидуализациюКИЗ"
		...
		Для Каждого СтрокаТовары Из Объект.Товары Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаТовары.КодМаркировки)
				И (СтрокаТовары.ПричинаПеремаркировки = ПредопределенноеЗначение(
						"Перечисление.ПричиныПеремаркировкиТоваровИСМП.ОшибкиОписанияТовара")
					Или СтрокаТовары.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Набор")
					Или СтрокаТовары.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Групповая")) Тогда
				ТекстОшибки = СтрШаблон(
					НСтр("ru = 'В строке %1 табличной части ""Товары "" не заполнен старый код маркировки.'"),
					НомерСтроки);
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					ТекстОшибки,,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
						"Объект.Товары", НомерСтроки, "КодМаркировки"),,
					Отказ);
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(СтрокаТовары.НовыйКодМаркировки) Тогда
				ТекстОшибки = СтрШаблон(
			...
			Если Не ЗначениеЗаполнено(СтрокаТовары.КодТНВЭД)
				...
				И ЗначениеЗаполнено(СтрокаТовары.НовыйКодМаркировки)
				И СтрокаТовары.ВидУпаковки <> ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Набор")
				И СтрокаТовары.ВидУпаковки <> ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Групповая") Тогда
				ТекстОшибки = СтрШаблон(
					НСтр("ru = 'В строке %1 табличной части ""Товары "" не заполнено поле ""Код ТН ВЭД"".'"),
					НомерСтроки);
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					ТекстОшибки,,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
...
Процедура ОбработчикПерезаполненияПоОснованию(ЗадаватьВопрос = Истина)
	...
	Если Объект.Товары.Количество() > 0 И ЗадаватьВопрос Тогда
		...
		ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения(
			"ВопросОПерезаполненииПоОснованиюПриЗавершении", ЭтотОбъект);
...
Процедура ВопросПриИзмененииТоварнойГруппыПриЗавершении(РезультатВопроса, ВыбраннаяТоварнаяГруппа) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да
		...
		ТоварнаяГруппаПриИзмененииНаКлиенте(Элементы.ТоварнаяГруппа, РезультатВопроса = Истина);
...
&НаКлиенте
Процедура ТоварнаяГруппаПриИзмененииНаКлиенте(Элемент, ПерезаполнитьПоОснованию = Ложь)
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Элементы.ТребуетсяИндивидуализацияКИЗ.Видимость = КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияПоддерживающаяИндивидуализациюКИЗ(ВидыПродукции);
	Элементы.ТоварыСтранаПроизводства.Видимость     = ПеремаркировкаТоваровКлиентСервер.ДоступноПолеСтранаПроизводства(ВидыПродукции);
	Элементы.ТоварыЦвет.Видимость                   = ПеремаркировкаТоваровКлиентСервер.ДоступноПолеЦвет(ВидыПродукции);
	Элементы.ТоварыРазмер.Видимость                 = ПеремаркировкаТоваровКлиентСервер.ДоступноПолеРазмер(ВидыПродукции);
	Элементы.ТоварыСертификация.Видимость           = ПеремаркировкаТоваровКлиентСервер.ДоступноПолеСертификация(ВидыПродукции);
