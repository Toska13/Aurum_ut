...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	|	И КорНДС.ХозяйственнаяОперация = &ХозяйственнаяОперация
	...
	Если Не КонецМесяца(Дата) = КонецКвартала(Дата) И ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ПереносОстатковПодНовыйВидДеятельности Тогда
...
Процедура ЗаполнитьТаблицуТоваровКИзмененияНалогообложения(МенеджерВременныхТаблиц, Товары)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереносОстатковПодНовыйВидДеятельности Тогда
		ИсходныйВидДеятельности = Документы.КорректировкаНалогообложенияНДСПартийТоваров.ИсходноеОсновноеНалогообложениеОрганизации(Организация, Дата, ПеремещениеПодДеятельность);
		Если ИсходныйВидДеятельности = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС Тогда
			// Переход с облагаемого вида деятельности, в качестве исходных могут быть все типы налогообложения прихода:
			ИсходныеВидыДеятельности = УчетНДСУПКлиентСервер.ПрименяемыеТипыНалогообложенияНДС(Ложь, Истина, Истина).ВыгрузитьЗначения();
			Граница = Новый Граница(КонецМесяца(Дата), ВидГраницы.Включая);
		Иначе
			ИсходныеВидыДеятельности = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИсходныйВидДеятельности);
			Граница = Новый Граница(НачалоМесяца(Дата), ВидГраницы.Включая);
		КонецЕсли;
		Запрос.УстановитьПараметр("ИсходныеВидыДеятельности", ИсходныеВидыДеятельности);
		РазделыУчетаДляАнализаОстатков = Перечисления.РазделыУчетаСебестоимостиТоваров.БалансовыеРазделыУчета();
		ИсключаемыеРазделы = Новый Массив;
		ИсключаемыеРазделы.Добавить(Перечисления.РазделыУчетаСебестоимостиТоваров.НезавершенноеПроизводство);
		ИсключаемыеРазделы.Добавить(Перечисления.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки);
		РазделыУчетаДляАнализаОстатков = ОбщегоНазначенияКлиентСервер.РазностьМассивов(РазделыУчетаДляАнализаОстатков, ИсключаемыеРазделы);
		Запрос.УстановитьПараметр("РазделыУчета", РазделыУчетаДляАнализаОстатков);
		Запрос.УстановитьПараметр("Граница", Граница);
		Запрос.Текст = ТекстЗапросаОстатковПриСменеНалогообложения();
	КонецЕсли;
...
Процедура ЗаполнитьВидЗапасовПолучателя()
	...
	ВыгружаемыеКолонки = "НомерСтроки, АналитикаУчетаНоменклатуры, ВидЗапасов";
...
Процедура ЗаполнитьДопКолонкиВидовЗапасов(МенеджерВременныхТаблиц)
	...
	ИменаРесурсов = "Количество, Стоимость, СтоимостьБезНДС, СтоимостьРегл, НДСРегл, ПостояннаяРазница, ВременнаяРазница";
	
	СтруктураПоиска	= Новый Структура("АналитикаУчетаНоменклатуры, ВидЗапасов, Склад");
	Ресурсы			= Новый Структура(ИменаРесурсов);
...
Процедура ЗаполнитьСтрокуВидовЗапасов(СтрокаТоваров, СтруктураПоиска, Ресурсы) 
	...
	Для Каждого СтрокаЗапасов Из ВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
		...
		ИначеЕсли Ресурсы.Количество <> 0 Тогда
...
Функция ТекстЗапросаОстатковПриСменеНалогообложения()
	...
	|				&Граница, Организация = &Организация И РазделУчета В (&РазделыУчета)) КАК Остатки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК Аналитика
	|		ПО Остатки.АналитикаУчетаНоменклатуры = Аналитика.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Назначения КАК Назначения
	|		ПО Аналитика.Назначение = Назначения.Ссылка
	|	ГДЕ
	|		Остатки.КоличествоОстаток > 0) КАК тПартии
	|
	|СГРУППИРОВАТЬ ПО
	|	тПартии.АналитикаУчетаНоменклатуры.Номенклатура,
	|	тПартии.АналитикаУчетаНоменклатуры.Характеристика,
