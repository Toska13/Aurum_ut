...
Функция ДанныеПодписейДляШтампа(ПрисоединенныйФайл) Экспорт
	...
	Для каждого ЭлементКоллекции Из УстановленныеПодписи Цикл
		Попытка
			Если ТипЗнч(ЭлементКоллекции.Сертификат) = Тип("ДвоичныеДанные") Тогда
				Сертификат = Новый СертификатКриптографии(ЭлементКоллекции.Сертификат);
			ИначеЕсли ТипЗнч(ЭлементКоллекции.Сертификат) = Тип("ХранилищеЗначения") Тогда
				Сертификат = Новый СертификатКриптографии(ЭлементКоллекции.Сертификат.Получить());
			Иначе
	...
	Если ВыборкаСообщений.Следующий() Тогда
		...
		Если ВыборкаСообщений.ЧастичноПодписан Тогда
			...
			Если ЗначениеЗаполнено(ПрисоединенныйФайл) Тогда
				ВыборкаПодписей = ЭлектроннаяПодпись.ПодписиОбъекта(ПрисоединенныйФайл);
...
Функция СформироватьПечатнуюФормуЭД(СообщениеОбмена, СсылкаНаДвоичныеДанныеФайла, ИдентификаторПечати, ДляПечати)
	...
	Если ОбменСБанкамиСлужебный.ЭтоЗарплатныйПроект(РеквизитыСообщенияОбмена.ВидЭД) Тогда
		ТабличныйДокументЗП = ТабличныйДокументЗарплатногоПроекта(СсылкаНаДвоичныеДанныеФайла);
		Если ТабличныйДокументЗП = Неопределено 
			Или ТипЗнч(ТабличныйДокументЗП) = Тип("ТабличныйДокумент") И ТабличныйДокументЗП.ВысотаТаблицы = 0 Тогда
			
			ТабличныйДокументЗП = ПечатнаяФормаДокументаЗарплатногоПроекта(
				СообщениеОбмена, СсылкаНаДвоичныеДанныеФайла, РеквизитыСообщенияОбмена.ВидЭД);
		КонецЕсли;
		
		Возврат ТабличныйДокументЗП;
	КонецЕсли;
	...
	ИначеЕсли РеквизитыСообщенияОбмена.ВидЭД = Перечисления.ВидыЭДОбменСБанками.ЗаявкаНаОткрытиеДепозита Тогда
		ТабличныйДокумент = ПечатнаяФормаЗаявкаНаОткрытиеДепозита(СообщениеОбмена, СсылкаНаДвоичныеДанныеФайла);
	ИначеЕсли РеквизитыСообщенияОбмена.ВидЭД = Перечисления.ВидыЭДОбменСБанками.ЗапросСостоянияДепозита Тогда
		ТабличныйДокумент = ПечатнаяФормаЗапросСостоянияДепозита(СообщениеОбмена, СсылкаНаДвоичныеДанныеФайла);
	ИначеЕсли РеквизитыСообщенияОбмена.ВидЭД = Перечисления.ВидыЭДОбменСБанками.ИзвещениеОСостоянииДепозита Тогда
		ТабличныйДокумент = ПечатнаяФормаИзвещениеОСостоянииДепозита(СообщениеОбмена, СсылкаНаДвоичныеДанныеФайла);
	ИначеЕсли РеквизитыСообщенияОбмена.ВидЭД = Перечисления.ВидыЭДОбменСБанками.Письмо Тогда
		ТабличныйДокумент = ПечатнаяФормаПисьма(СообщениеОбмена);
	КонецЕсли;
...
Функция ДанныеДляПечатиПлатежногоПорученияИзJSON(ДанныеДокумента)
	...
	СтруктураДанных = ОбщегоНазначения.JSONВЗначение(ДанныеСтрокой, , Ложь);
...
Функция ПечатнаяФормаДокументаЗарплатногоПроекта(СообщениеОбмена, ДанныеФайлаСсылка, ВидЭД)
	...
	СтрокаОбъекта = Результат.СтрокаОбъекта;
	ДанныеЭДДляПечати = ДанныеДляПечати(СтрокаОбъекта);
	НазваниеМакета = СтрШаблон("ЭД_СчетаПК_%1", ОбщегоНазначения.КодОсновногоЯзыка());
	ЗаполнитьТабличныйДокумент_ЭД(ТабличныйДокумент, ДанныеЭДДляПечати, НазваниеМакета);
	Возврат ТабличныйДокумент;
	...
	Для Каждого Выписка Из СтрокаТаблицыВыписок.Строки Цикл
		...
		Если СтрокаОперацияВыписки.Значение <> Неопределено И СтрокаОперацияВыписки.Значение <> 0 Тогда
			Для Каждого Операция Из СтрокаОперацияВыписки.Строки Цикл
				...
				КонецЕсли;
			КонецЦикла;
		ИначеЕсли СтрокаОперацияВыписки.Значение = 0 Тогда
			...
			ШаблонСообщения = НСтр("ru = 'По счету %1 за период с %2 по %3 не было проведено ни одной операции'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, НомерСчета, НачалоПериода, КонецПериода);
...
Функция ПолучитьДанныеВыпискиДляПечати(ДеревоДанных)
	...
	Иначе
		...
		НаименованиеБанкаМеждународное = ЗначениеРеквизитаВДереве(Выписка,
			"Выписки.НомерСтроки.Банк.НаименованиеМеждународное");
		НаименованиеБанкаЛокальное = ЗначениеРеквизитаВДереве(Выписка, "Выписки.НомерСтроки.Банк.Наименование");
		НаименованиеБанка = ?(ЗначениеЗаполнено(НаименованиеБанкаЛокальное), НаименованиеБанкаЛокальное,
			НаименованиеБанкаМеждународное);
		СтруктураШтампа.Вставить("НаименованиеБанка", НаименованиеБанка);
		СтруктураШтампа.Вставить("ДатаСоставленияВыписки", ЗначениеРеквизитаВДереве(Выписка, "Выписки.НомерСтроки.ДатаФормирования"));
	КонецЕсли;
...
Процедура ЗаполнитьОбластьОтметок(Макет, ТабличныйДокумент, СообщениеОбмена, РеквизитыБанка)
	...
	Если ЕстьШтамп ИЛИ ПодписанЭП ИЛИ ДокументОтправленВБанк Тогда
		...
		Если ПодписанЭП Тогда 
			...
			КоличествоПодписей = ЭлектронныеПодписи.Количество();   
		КонецЕсли;
		...
		Если КоличествоПодписей <= 1 Тогда  
			...
			Если ЕстьШтамп Тогда
				...
				Если ЗначенияДополнительныхРеквизитов.Свойство("ДатаСписания") Тогда
					ОбластьШтамп.Параметры.ДатаОперации = ЗначенияДополнительныхРеквизитов.ДатаСписания;    
				КонецЕсли;
				ТабличныйДокумент.Присоединить(ОбластьШтамп);
		...
		Если ПодписанЭП Тогда  
			...
			ИначеЕсли КоличествоПодписей > 1 Тогда
				НазваниеМакета = СтрШаблон("ШтампБанка_%1", ОбщегоНазначения.КодОсновногоЯзыка());
				МакетШтампа = Документы.СообщениеОбменСБанками.ПолучитьМакет(НазваниеМакета);				
				ОбластьШтамп = МакетШтампа.ПолучитьОбласть("Отметки|Штамп"); 
				ЗаполнитьЗначенияСвойств(ОбластьШтамп.Параметры, РеквизитыБанка);
				ОбластьШтамп.Параметры.Статус = РеквизитыСообщенияОбмена.Статус;
				ОбластьШтамп.Параметры.ДатаОперации = РеквизитыСообщенияОбмена.ДатаИзмененияСтатуса;   		
...
Функция ДанныеДляПечатиРеестрВыплатСамозанятым(СообщениеОбмена, ДанныеФайла)
	...
	ДанныеДляОбъекта = Новый Структура;	
	ДанныеПоТипу = ОбменСБанкамиСлужебный.ПрочитатьXDTOИзДвоичныхДанных(ДанныеФайла);
	Если ДанныеПоТипу.ЕстьОшибка Тогда
		ТекстСообщения = НСтр("ru = 'При чтении данных электронного документа произошла ошибка.
			|%1'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, ДанныеПоТипу.ПредставлениеОшибки);
		ШаблонОшибки = НСтр("ru = 'Возникла ошибка при чтении данных электронного документа.
			|Текст ошибки: %1
			|Файл %2.'");
		ТекстОшибки = СтрШаблон(ШаблонОшибки, ДанныеПоТипу.ПредставлениеОшибки, НСтр("ru = 'Реестр выплат'"));
...
Функция ПечатнаяФормаЗаявкаНаОткрытиеДепозита(СообщениеОбмена, ДанныеФайлаСсылка)
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ДанныеФайла = ПолучитьИзВременногоХранилища(ДанныеФайлаСсылка);
	Результат = ОбменСБанкамиСлужебный.СформироватьДеревоРазбора(
		Перечисления.ВидыЭДОбменСБанками.ЗаявкаНаОткрытиеДепозита, ДанныеФайла);
	СтрокаОбъекта = Результат.СтрокаОбъекта;
	ДанныеЭДДляПечати = ДанныеДляПечати(СтрокаОбъекта);
	ЗаполнитьТабличныйДокументЗаявкаНаОткрытиеДепозита(ТабличныйДокумент, ДанныеЭДДляПечати);
	Возврат ТабличныйДокумент;
	
КонецФункции

Процедура ЗаполнитьТабличныйДокументЗаявкаНаОткрытиеДепозита(ТабличныйДокумент, Реквизиты)
	
	НазваниеМакета = СтрШаблон("ЭД_ЗаявкаНаОткрытиеДепозита_%1", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПечатнаяФормаЗапросСостоянияДепозита(СообщениеОбмена, ДанныеФайлаСсылка)
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ДанныеФайла = ПолучитьИзВременногоХранилища(ДанныеФайлаСсылка);
	Результат = ОбменСБанкамиСлужебный.СформироватьДеревоРазбора(
		Перечисления.ВидыЭДОбменСБанками.ЗапросСостоянияДепозита, ДанныеФайла);
	СтрокаОбъекта = Результат.СтрокаОбъекта;
	ДанныеЭДДляПечати = ДанныеДляПечати(СтрокаОбъекта);
	ЗаполнитьТабличныйДокументЗапросСостоянияДепозита(ТабличныйДокумент, ДанныеЭДДляПечати);
	Возврат ТабличныйДокумент;
	
КонецФункции

Процедура ЗаполнитьТабличныйДокументЗапросСостоянияДепозита(ТабличныйДокумент, Реквизиты)
	
	НазваниеМакета = СтрШаблон("ЭД_ЗапросСостоянияДепозита_%1", ОбщегоНазначения.КодОсновногоЯзыка());
	Макет = ПолучитьМакет(НазваниеМакета);
	Макет.КодЯзыка = Метаданные.Языки.Русский.КодЯзыка;
	ОбластьМакета = Макет.ПолучитьОбласть("Шапка");
	ЗаполнитьЗначенияСвойств(ОбластьМакета.Параметры, Реквизиты);
...
Функция ПечатнаяФормаИзвещениеОСостоянииДепозита(СообщениеОбмена, ДанныеФайлаСсылка)
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ДанныеФайла = ПолучитьИзВременногоХранилища(ДанныеФайлаСсылка);
	Результат = ОбменСБанкамиСлужебный.СформироватьДеревоРазбора(
		Перечисления.ВидыЭДОбменСБанками.ИзвещениеОСостоянииДепозита, ДанныеФайла);
	СтрокаОбъекта = Результат.СтрокаОбъекта;
	ДанныеЭДДляПечати = ДанныеДляПечати(СтрокаОбъекта);
	ЗаполнитьТабличныйДокументИзвещениеОСостоянииДепозита(СообщениеОбмена, ТабличныйДокумент, ДанныеЭДДляПечати);
	Возврат ТабличныйДокумент;
	
КонецФункции

Процедура ЗаполнитьТабличныйДокументИзвещениеОСостоянииДепозита(СообщениеОбмена, ТабличныйДокумент, Реквизиты)

	НазваниеМакета = СтрШаблон("ЭД_ИзвещениеОСостоянииДепозита_%1", ОбщегоНазначения.КодОсновногоЯзыка());
	Макет = ПолучитьМакет(НазваниеМакета);
	Макет.КодЯзыка = Метаданные.Языки.Русский.КодЯзыка;
	ОбластьМакета = Макет.ПолучитьОбласть("Шапка");
	ЗаполнитьЗначенияСвойств(ОбластьМакета.Параметры, Реквизиты);
	ТабличныйДокумент.Вывести(ОбластьМакета);
	ОбластьМакета = Макет.ПолучитьОбласть("ГрафикВыплатыПроцентов_Шапка");
	ТабличныйДокумент.Вывести(ОбластьМакета);
	
	ОбластьМакета = Макет.ПолучитьОбласть("СтрокаВыплата");
	ДанныеГрафика = Реквизиты.ГрафикВыплатыПроцентов;
	
	Для Каждого ТекСтрокаВыплаты Из ДанныеГрафика Цикл
		ЗаполнитьЗначенияСвойств(ОбластьМакета.Параметры, ТекСтрокаВыплаты);
		ТабличныйДокумент.Вывести(ОбластьМакета);
	КонецЦикла;
	
	ОбластьМакета = Макет.ПолучитьОбласть("ИнформацияОПродукте");
	ИнформацияОПродукте = Реквизиты.ИнформацияОПродукте;
	ЗаполнитьЗначенияСвойств(ОбластьМакета.Параметры, ИнформацияОПродукте);
	ТабличныйДокумент.Вывести(ОбластьМакета);
	
	ДанныеШтампа = Неопределено;
	Если Реквизиты.Свойство("Штамп", ДанныеШтампа) Тогда
		ЗаполнитьОбластьОтметок(Макет, ТабличныйДокумент, СообщениеОбмена, ДанныеШтампа)
	КонецЕсли;
	
КонецПроцедуры

Функция ПечатнаяФормаПисьма(СообщениеОбмена)
	
	ПисьмоСсылка = ОбменСБанкамиСлужебныйВызовСервера.ДокументУчета(СообщениеОбмена);
	Если НЕ ЗначениеЗаполнено(ПисьмоСсылка) Тогда
		Возврат Новый ТабличныйДокумент;
	КонецЕсли;
	
	Возврат Документы.ПисьмоОбменСБанками.ПечатнаяФормаПисьма(ПисьмоСсылка);
