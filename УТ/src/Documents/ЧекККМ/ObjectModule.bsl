...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Иначе
		...
		Если Справочники.КассыККМ.КассаККМСозданаВСистеме() Тогда
		...
		Иначе
			...
			ВызватьИсключение НСтр("ru = 'Для продолжения работы в РМК необходимо создать хотя бы одну кассу ККМ'");
		КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не ЗначениеЗаполнено(Статус) Тогда
		Статус = Перечисления.СтатусыЧековККМ.ОтложенБезРезервирования;
	КонецЕсли;
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	ПараметрыЗаполнения = Документы.ЧекККМ.ПараметрыЗаполненияНалогообложенияНДСПродажи(ЭтотОбъект);	
	Если Не ЗначениеЗаполнено(ПараметрыЗаполнения.Дата) Тогда
