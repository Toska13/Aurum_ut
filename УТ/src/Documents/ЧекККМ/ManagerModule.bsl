...
Функция ТекстЗапросаДляПолученияНевозвращенныхПозицийПоЧекуККМ(ПараметрыОтбора) Экспорт
	...
	|	ЕСТЬNULL(ПроданныеПозиции.Комитент, ВозвращенныеПозиции.Комитент) КАК Комитент,
...
Функция ТекстЗапросаДляПолученияНеотмененныхОплатПоЧекуККМ(ИспользоватьОтборПоВидуОплаты, ВсеВидыОплатКромеУказанных = Ложь) Экспорт
	...
	|	НеотмененныеОплатыПоЧекуККМСБП.ИдентификаторКорзины,
	...
	|		ОплатыПлатежнымиКартами.ИдентификаторКорзины,
	...
	|			ПО ОплатыПлатежнымиКартами.Ссылка = ЧекККМ.Ссылка
	|	ГДЕ
	|		ЧекККМ.Проведен
...
// Возвращает текст запроса для получения невозвращенных серий по чеку ККМ.
// Помещение для возвратов товаров от клиента вычисляется из данных чека ККМ.
// В запросе используются параметры: 
//  Ссылка - ДокументСсылка.ЧекККМ - ссылка на документ продажи
//  ТекущийВозвратТоваровОтКлиента - Неопределено, ДокументСсылка.ВозвратТоваровОтКлиента - ссылка на текущий возврат,
//  для которого вычисляется состав серий.
//
// Возвращаемое значение:
//  Строка - 
//
Функция ТекстЗапросаДляПолученияНевозвращенныхСерийПоЧекуККМ() Экспорт
	...
	|	Серии.Характеристика,
	|	Серии.Серия,
	|	МАКСИМУМ(ЧекККМСерии.Помещение),
	|	-СУММА(Серии.Количество)
	|ИЗ
	|	Документ.ВозвратТоваровОтКлиента.Серии КАК Серии
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЧекККМ.Серии КАК ЧекККМСерии
	|		ПО ЧекККМСерии.Ссылка = &Ссылка
	|			И ЧекККМСерии.Номенклатура = Серии.Номенклатура
	|			И ЧекККМСерии.Характеристика = Серии.Характеристика
	|			И ЧекККМСерии.Серия = Серии.Серия	
	|ГДЕ
	|	Серии.Ссылка.ЧекККМ = &Ссылка
	|	И Серии.Ссылка.Проведен
	|	И (Серии.Ссылка <> &ТекущийВозвратТоваровОтКлиента)
	|
	|СГРУППИРОВАТЬ ПО
	|	Серии.Номенклатура,
	|	Серии.Характеристика,
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка) Экспорт
	...
	Запрос.УстановитьПараметр("ТоварЗарезервирован",                      Реквизиты.Статус = Перечисления.СтатусыЧековККМ.ОтложенСРезервированием);
...
Функция ТекстЗапросаВтСтатьиДДС(ТекстыЗапроса)
	...
	|	ТаблицаПлатежей.НомерСтроки КАК НомерСтроки,
	|	ВЫБОР
	|		КОГДА НЕ ЕСТЬNULL(СведенияОДоговоре.СтатьяДвиженияДенежныхСредствПоступлениеОплаты, ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка)
	|			ТОГДА СведенияОДоговоре.СтатьяДвиженияДенежныхСредствПоступлениеОплаты
	|		КОГДА НЕ ЕСТЬNULL(СведенияОДоговореНаТерминал.СтатьяДвиженияДенежныхСредствПоступлениеОплаты, ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка)
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры)
	...
	ВзаиморасчетыПроведениеКлиенты.ПроведениеПродажи(Запрос, ТекстыЗапроса, Регистры, ТекстПродажа, ТекстПланированиеОплат, ТекстЗачетАванса);
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|			ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.ОтложенСРезервированием),
		|			ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.КОплате))
