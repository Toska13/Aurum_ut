...
Перем КэшКодовТРУ;
&НаКлиенте
Перем КэшТоваровТребующихПодтвержденияВозраста;
// -- Локализация
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Объект.Товары.Количество() > 0 Тогда
			Для Каждого СтрокаТЧ Из Объект.Товары Цикл
				СтрокаТЧ.ИспользованСертификатФСС = Ложь;
			КонецЦикла;
		КонецЕсли;
	...
	ПриСозданииНаСервереЛокализация(ЭтотОбъект, Отказ, СтандартнаяОбработка);
...
Процедура ПриОткрытии(Отказ)
	...
	Если ИспользоватьПодключаемоеОборудование Тогда
		...
		Иначе
			...
			ПроверитьЦеныТоваровЛокализация();
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Не ТолькоПросмотр
		...
		Если ЗначениеЗаполнено(КэшированныеЗначения)
			И КэшированныеЗначения.Свойство("Штрихкоды") Тогда
		...
		ОбработатьШтрихкоды(ДанныеШтрихкодов, Ложь, Истина);
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ОбновитьСтатусыИРазрешеннияТоваровЛокализация();
	ОбработатьРазрешенияНаПродажуТоваровЛокализация(ЭтотОбъект);

	//++ Локализация
	РозничныеПродажиЛокализация.НастроитьФормуДляПодтвержденияВозрастаПокупателя(ЭтотОбъект, ТекущийОбъект);
	//-- Локализация
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ЗаполнитьСлужебныеРеквизитыПоНоменклатуре();
	РозничныеПродажиКлиентСервер.ВосстановитьИзКэшаДанныеДинамическихРеквизитовТаблицФормы(ЭтотОбъект);
...
&НаКлиенте
Процедура Подключаемый_ЭлементФормыСостояниеПроверкиВозрастаПокупателяОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	РозничныеПродажиКлиентЛокализация.СостояниеПроверкиВозрастаПокупателяОбработкаНавигационнойСсылки(ЭтотОбъект, Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка);
КонецПроцедуры
...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	//++ Локализация
	Если Элементы.Найти("ТоварыРеквизитыРазрешенияНаПродажу") <> Неопределено
		И Поле = Элементы.ТоварыРеквизитыРазрешенияНаПродажу Тогда
		
		ТекущаяСтрока = Объект.Товары.НайтиПоИдентификатору(ВыбраннаяСтрока);
		
		Если ТекущаяСтрока.ПродаетсяПоРазрешению Тогда
		
			СтруктураПоиска = Новый Структура("ИдентификаторСтроки", ТекущаяСтрока.ИдентификаторСтроки);
			СтрокиРазрешенияНаПродажу = Объект.РазрешенияНаПродажу.НайтиСтроки(СтруктураПоиска);
			
			ДанныеРазрешения = Новый Структура;
			ДанныеРазрешения.Вставить("Номенклатура", ТекущаяСтрока.Номенклатура);
			ДанныеРазрешения.Вставить("ИдентификаторСтроки", ТекущаяСтрока.ИдентификаторСтроки);
			
			Если СтрокиРазрешенияНаПродажу.Количество() Тогда
				ДанныеРазрешения.Вставить("ТипДокумента", СтрокиРазрешенияНаПродажу[0].ТипДокумента);
				ДанныеРазрешения.Вставить("НомерДокумента", СтрокиРазрешенияНаПродажу[0].НомерДокумента);
				ДанныеРазрешения.Вставить("ДатаДокумента", СтрокиРазрешенияНаПродажу[0].ДатаДокумента);
			КонецЕсли;
			
			ОткрытьФорму(
				"Документ.ЧекККМ.Форма.ФормаРазрешениеНаПродажу",
				ДанныеРазрешения,
				ЭтотОбъект,,,,
				Новый ОписаниеОповещения("ОткрытьФормуУказанияДокументаОтпускаПрепаратаЗавершение", ЭтотОбъект),
	...
	ОтменитьИзменениеВПравахРедактированияКоличества();
	//-- Локализация
...
Процедура ТоварыПриАктивизацииЯчейки(Элемент)
	...
	ДанныеТовара.Вставить("Номенклатура",   ТекущиеДанные.Номенклатура);
	ДанныеТовара.Вставить("Характеристика", ТекущиеДанные.Характеристика);
	ДанныеТовара.Вставить("Упаковка",       ТекущиеДанные.Упаковка);
	ДанныеТовара.Вставить("Серия",          ТекущиеДанные.Серия);
	
	Элементы.ТоварыШтрихкод.СписокВыбора.ЗагрузитьЗначения(
		РозничныеПродажиВызовСервера.ПолучитьШтрихкодыНоменклатуры(ДанныеТовара));
...
Процедура ТоварыПриАктивизацииСтроки(Элемент)
	...
	ТоварыПриАктивизацииСтрокиЛокализация(Элемент);
...
Процедура ТоварыПриИзменении(Элемент)
	...
	ТоварыПриИзмененииЛокализация(Элемент);
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ТоварыПриНачалеРедактированияЛокализация(Элемент, НоваяСтрока, Копирование);
...
Процедура ТоварыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если ДополнительныеПараметры.Свойство("ТребуетсяСерверныйВызов") И ДополнительныеПараметры.ТребуетсяСерверныйВызов = Истина Тогда
...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	ПроверитьРазрешениеНаУдалениеСтрокиТабличнойЧасти("Товары", Отказ);
...
Процедура ТоварыПослеУдаления(Элемент)
	...
	ТоварыПослеУдаленияЛокализация(Элемент);
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ТоварыНоменклатураПриИзмененииЛокализация(Элемент);
...
Процедура ТоварыНоменклатураПриИзмененииСервер(ИдентификаторТекущейСтроки)
	...
	Если НЕ ПодбиратьКодыМаркировкиВскрытыхПотребительскихУпаковокПоFIFOЛокализация() Тогда
		ОбработатьВскрытыеПотребительскиеУпаковкиПоТаблицеТоваровЛокализация(Объект.Товары.Выгрузить());
	КонецЕсли;
...
Процедура ТоварыПомещениеПриИзменении(Элемент)
	РозничныеПродажиКлиент.ОбновитьПомещениеВСтрокахТабличнойЧастиСерии(ЭтотОбъект, Элементы.Товары.ТекущиеДанные, КэшированныеСтроки[Элементы.Товары.Имя].Помещение);
КонецПроцедуры
...
Процедура НазначитьАвтоматическиеСкидки(Команда)
	...
	РозничныеПродажиКлиент.ОчиститьОкноСообщений(ЭтотОбъект);
	//++ Локализация
	Если НЕ ОпределятьПринадлежностьТовараИСтавкуНДСПриПодборе
		И ИспользоватьКомиссиюПриЗакупках Тогда
		
		ОпределятьПринадлежностьТовараИСтавкуНДСПередОплатойЛокализация();
	КонецЕсли;
	//-- Локализация
...
Процедура НапечататьПоследнийСлипЧек(Команда)
	...
	Если Не МенеджерОборудованияКлиент.ПодключаемоеОборудование().Свойство("ПоследнийСлипЧек", СтрокаСлипЧека)
...
Процедура ОткрытьПодбор(Команда)
	...
	ПараметрыФормы.Вставить("ОбъектХраненияУсловийПродаж", ОбъектХраненияУсловийПродаж);
...
Процедура ПримененныеСкидки(Команда)
	...
	Если НЕ Объект.СкидкиРассчитаны Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Скидки не рассчитаны. Перед открытием отчета необходимо рассчитать скидки.'"));
		Возврат;
	КонецЕсли;
	
	ВыполнитьДействиеПослеЗаписи(
		Новый ОписаниеОповещения("ОткрытьФормуОтчетаПримененныхСкидкок", ЭтотОбъект),
		НСтр("ru = 'Перед открытием отчета примененных скидок не удалось записать документ.'"));
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьФормуОтчетаПримененныхСкидкок(ИзмененныеДанныеЗаписаны, ДополнительныеПараметры) Экспорт
	...
	ОткрытьФорму("Отчет.ПримененныеСкидкиВДокументе.Форма.ФормаОтчета", ПараметрыФормы, ЭтотОбъект);
...
&НаКлиенте
Процедура УказатьРазрешениеНаПродажу(Команда)
	
	РозничныеПродажиКлиент.ОчиститьОкноСообщений(ЭтотОбъект);
	УказатьРазрешениеНаПродажуЛокализация();
...
&НаКлиенте
Процедура Подключаемый_ПодтверждениеВозрастаПокупателяЧерезМакс(Команда)
	РозничныеПродажиКлиентЛокализация.ПодтверждениеВозрастаПокупателяЧерезМакс(ЭтотОбъект, Команда);
КонецПроцедуры
...
//++ Локализация
	...
	Если НЕ ОпределятьПринадлежностьТовараИСтавкуНДСПриПодборе
		И ИспользоватьКомиссиюПриЗакупках
...
Процедура ОбработатьСтрокуПриИзмененииСерии(ТекущаяСтрока)
	...
	Если КонтролироватьАссортимент Тогда
		СтруктураДействий.Вставить("ЗаполнитьЦенуПродажиПоАссортименту",
			ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияЦеныПоАссортиментуВСтрокеТЧ(Объект));
	ИначеЕсли ИспользуетсяЦенообразование25 Тогда 
		СтруктураДействий.Вставить("ЗаполнитьУсловияРозничныхПродаж",
			ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияУсловийРозничныхПродажВСтрокеТЧ(
				Объект, ОбъектХраненияУсловийПродаж));
	Иначе
		СтруктураДействий.Вставить("ЗаполнитьЦенуПродажи",
			ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияЦеныРозницаВСтрокеТЧ(Объект));
	КонецЕсли;
	СтруктураДействий.Вставить("ПересчитатьСумму");
	СтруктураДействий.Вставить("ПересчитатьСуммуСУчетомАвтоматическойСкидки", Новый Структура("Очищать", Истина));
	СтруктураДействий.Вставить("ПересчитатьСуммуСУчетомРучнойСкидки", Новый Структура("Очищать", Истина));
	...
	ПакетнаяОбработкаТабличнойЧастиКлиентСерверЛокализация.ДополнитьСтруктуруДействийПриИзмененииЭлемента(ЭтотОбъект, "Серия", СтруктураДействий);
...
Функция ДобавитьВКорзинуНаСервере(ПараметрыТовара, НовыеСтроки)
	...
	СтруктураДействийСуществующаяСтрока = Новый Структура;
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействийСуществующаяСтрока, Объект);
	Если КонтролироватьАссортимент Тогда
		ДобавитьПроверкуАссортиментаВСтруктуруДействий(СтруктураДействийСуществующаяСтрока);
	КонецЕсли;
	
	СтруктураДействийНоваяСтрока = Новый Структура;
	
	СтруктураДействийНоваяСтрока.Вставить("ПроверитьХарактеристикуПоВладельцу");
	СтруктураДействийНоваяСтрока.Вставить("ПроверитьЗаполнитьУпаковкуПоВладельцу");
	
	Если ИспользоватьСкладскиеПомещения Тогда
		СтруктураДействийНоваяСтрока.Вставить("ЗаполнитьПомещение", Новый Структура("Склад", Объект.Склад));
	КонецЕсли;
	
	СтруктураДействийНоваяСтрока.Вставить("ЗаполнитьПродавца", Новый Структура("Продавец", ТекущийПродавец));
	СтруктураДействийНоваяСтрока.Вставить("ЗаполнитьСтавкуНДС",
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект));
	Если КонтролироватьАссортимент Тогда
		ДобавитьПроверкуАссортиментаВСтруктуруДействий(СтруктураДействийНоваяСтрока);
	КонецЕсли;
	
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействийНоваяСтрока,Объект);
	ПакетнаяОбработкаТабличнойЧастиКлиентСерверЛокализация.ДополнитьСтруктуруДействийПриИзмененииЭлемента(
		ЭтотОбъект, "Номенклатура", СтруктураДействийНоваяСтрока);
	...
	Для Каждого СтрокаТовара Из НовыеСтроки Цикл
		...
		Если ЭтоМаркированныйТоварСЧастичнымВыбытиемЛокализация(СтрокаТовара.Номенклатура)
			...
			Результат = ВскрытыеПотребительскиеУпаковкиПоТоваруЛокализация(СтрокаТовара);
		...
		Если РезультатПоиска.Количество() = 0 ИЛИ СтрокаТовара.Погрешность <> 0 Тогда
			...
			Погрешности.Вставить(ТекущаяСтрока, СтрокаТовара.Погрешность);
			
			ДобавляемыеСтроки.Добавить(ТекущаяСтрока);
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		ОбрабатываемыеСтроки,
		СтруктураДействийСуществующаяСтрока,
		Объект.Товары);
		
	СтруктураДействийНоваяСтрока.Вставить("ПересчитатьСуммуСУчетомПогрешностиОкругления",
...
Процедура ИзменитьТабличнуюЧастьПоРезультатамПроверки(ВозвращаемыеПараметры, КэшированныеЗначения)
	...
	ИзмененныеСтрокиДляОбработки = Новый Массив;
	ДобавленныеСтрокиДляОбработки = Новый Массив;
	...
	Для Каждого СтрокаИсточник Из ТаблицаТовары Цикл
		...
		Для Каждого СтрокаТЧ Из НайденныеСтроки Цикл
			...
			Иначе
				...
				ИзмененныеСтрокиДляОбработки.Добавить(СтрокаТЧ);
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			ДобавленныеСтрокиДляОбработки.Добавить(СтрокаТЧ);
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		ИзмененныеСтрокиДляОбработки,
		СтруктураДействий,
		Объект.Товары,
		КэшированныеЗначения);
	
	СтруктураДействий = СтруктураДействийПриИзмененииНоменклатурыТабличнойЧастиТовары();
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействий, Объект);

	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		ДобавленныеСтрокиДляОбработки,
		СтруктураДействий,
		Объект.Товары,
		КэшированныеЗначения);
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов, ЗагрузкаИзТСД = Ложь, НеизвестныеШтрихкоды = Ложь)
	...
	СтруктураДействийСДобавленнымиСтроками = Новый Структура;
	Если КонтролироватьАссортимент Тогда
		СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьЦенуПродажиПоАссортименту", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияЦеныПоАссортиментуВСтрокеТЧ(Объект));
	ИначеЕсли ИспользуетсяЦенообразование25 Тогда 
		СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьУсловияРозничныхПродаж", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияУсловийРозничныхПродажВСтрокеТЧ(Объект, ОбъектХраненияУсловийПродаж));
	Иначе
		СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьЦенуПродажи", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияЦеныРозницаВСтрокеТЧ(Объект));
	КонецЕсли;
...
&НаКлиенте
Процедура НачатьПодключениеОборудованиеПриОткрытииФормыЗавершение(Результат, Параметры) Экспорт
...
Процедура Подключаемый_ПослеОбработкиШтрихкодов()
	...
	Если ШтрихкодированиеНоменклатурыКлиент.НужноОткрытьФормуУказанияСерийПослеОбработкиШтрихкодов(ДанныеДляОбработки()) Тогда
		...
		Если ТекущиеДанныеИдентификатор <> Неопределено Тогда
			...
			Элементы.Товары.ТекущаяСтрока = ТекущиеДанныеИдентификатор;
...
Процедура ОбработатьСтрокиТЧ(ДобавленныеСтроки, ИзмененныеСтроки, КэшированныеЗначения = Неопределено) Экспорт
	...
	МассивыДляРазнойПакетнойОбработкиСтрок = РазделитьМассивСтрокДляРазныхТиповПакетнойОбработки(ДобавленныеСтроки);
	...
	Если МассивыДляРазнойПакетнойОбработкиСтрок.НеТребуетсяЗаполнитьЦену.Количество() Тогда
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			МассивыДляРазнойПакетнойОбработкиСтрок.НеТребуетсяЗаполнитьЦену,
			СтруктураДействийСДобавленнымиСтроками,
			Объект.Товары);
	КонецЕсли;
	
	Если МассивыДляРазнойПакетнойОбработкиСтрок.ТребуетсяЗаполнитьЦену.Количество() Тогда
		Если КонтролироватьАссортимент Тогда
			СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьЦенуПродажиПоАссортименту",
				ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияЦеныПоАссортиментуВСтрокеТЧ(Объект));
		ИначеЕсли ИспользуетсяЦенообразование25 Тогда 
			СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьУсловияРозничныхПродаж",
				ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияУсловийРозничныхПродажВСтрокеТЧ(
					Объект, ОбъектХраненияУсловийПродаж));
		Иначе
		...
		КонецЕсли;
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			МассивыДляРазнойПакетнойОбработкиСтрок.ТребуетсяЗаполнитьЦену,
			СтруктураДействийСДобавленнымиСтроками,
			Объект.Товары);
	КонецЕсли;
	
	СтрокиСОкруглением = Новый Массив();
		...
		Если СтрокаТЧ.КоличествоУпаковок > СтрокаТЧ.Количество И СтрокаТЧ.КоличествоУпаковок <> Окр(СтрокаТЧ.КоличествоУпаковок) Тогда
			...
			Если Окр(Окр(СтрокаТЧ.КоличествоУпаковок) / КоэффициентУпаковки, 3) = СтрокаТЧ.Количество Тогда
				СтрокаТЧ.КоличествоУпаковок = Окр(СтрокаТЧ.КоличествоУпаковок);
					СтрокиСОкруглением.Добавить(СтрокаТЧ);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Функция ЗарезервироватьНаСервере(ТребуетсяПовторнаяПопыткаЗаписи, СтатусДокумента, ПередЗакрытием = Ложь)
	...
	Объект.Статус = Перечисления.СтатусыЧековККМ[СтатусДокумента];
...
Процедура ЗарезервироватьНаКлиенте(РезультатОтложенДо, ДополнительныеПараметры) Экспорт
	...
	ТребуетсяПовторнаяПопыткаЗаписи = Ложь;
	СтатусДокумента = "ОтложенСРезервированием";
	Результат = ЗарезервироватьНаСервере(ТребуетсяПовторнаяПопыткаЗаписи, СтатусДокумента, Истина);
...
Функция ВыполнитьЧастьРасчетаНаСервере(ВозвращенноеЗначениеУправляемыеСкидки, ТребуетсяПовторнаяПопыткаЗаписи)
	...
	Если Не КонтрольНаСкладеОтключен Тогда
		СтатусДокумента = "КОплате";
		ТоварЗарезервирован = ЗарезервироватьНаСервере(ТребуетсяПовторнаяПопыткаЗаписи, СтатусДокумента);
...
Процедура НовыйЧекНаСервере(Ссылка = Неопределено)
	...
	ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(НоменклатураСервер.ПараметрыУказанияСерий(Объект, Документы.ЧекККМ));
	//++ Локализация
	ЕстьОплатаСертификатамиФСС = Ложь;
...
Процедура ОткрытьФормуОплатыНаличными(ИзмененныеДанныеЗаписаны, ДополнительныеПараметры) Экспорт
	...
	// Команда оплаты наличными вызвана из формы РМК
...
Процедура ОтменитьОплату(ИзмененныеДанныеЗаписаны, ДополнительныеПараметры) Экспорт
	...
	Если Объект.СкидкиРассчитаны  И Объект.Товары.Количество() > 0 Тогда
...
Процедура ОшибкаПриПроведенииЧекаВопросЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> КодВозвратаДиалога.Отмена Тогда
		...
		ИначеЕсли ДополнительныеПараметры.ИмяПроцедуры = "ЗарезервироватьНаСервере" Тогда
			РезультатОперации = ЗарезервироватьНаСервере(ТребуетсяПовторнаяПопыткаЗаписи, "ОтложенСРезервированием");
...
Функция ЗаписатьНаСервере(ТребуетсяПовторнаяПопыткаЗаписи = Неопределено)
	...
	РозничныеПродажиКлиентСервер.СохранитьВКэшДанныеДинамическихРеквизитовТаблицФормы(ЭтотОбъект);
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	Если КонтролироватьАссортимент Тогда
		...
		СтруктураПроверкиАссортимента.ТекстСообщения 
			= НСтр("ru = 'Товар %1 не включен в ассортимент магазина или запрещен к продаже.'");
	...
	Если НЕ ПодбиратьКодыМаркировкиВскрытыхПотребительскихУпаковокПоFIFOЛокализация() Тогда
		ОбработатьВскрытыеПотребительскиеУпаковкиПоТаблицеТоваровЛокализация(ТаблицаТоваров);
	КонецЕсли;
...
Процедура ИзменитьПомещениеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	СтароеЗначениеПомещения = СтрокаТЧ.Помещение;	
	СтрокаТЧ.Помещение = Помещение;
	
	РозничныеПродажиКлиент.ОбновитьПомещениеВСтрокахТабличнойЧастиСерии(ЭтотОбъект, СтрокаТЧ, СтароеЗначениеПомещения);
	ОбновитьДанныеИнформационнойПанели(ЭтотОбъект, Истина = ТолькоПросмотр);
...
Процедура УстановитьЗначенияПоНастройкам()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ОтборПоККМОтложенногоЧека = Оборудование.НайтиСтроки(Новый Структура("Объект", Объект.КассаККМ));
		Если Объект.Статус <> Перечисления.СтатусыЧековККМ.Пробит
			И ОтборПоККМОтложенногоЧека.Количество() = 0
			И Оборудование.Количество() > 0 Тогда
			Объект.КассаККМ = Оборудование[0].Объект;
		КонецЕсли;
		...
		Если Объект.Статус <> Перечисления.СтатусыЧековККМ.Пробит Тогда
			...
			Если Объект.Организация <> СтруктураСостояниеКассовойСмены.Организация Тогда
				Объект.Организация = СтруктураСостояниеКассовойСмены.Организация;
			КонецЕсли;
...
Процедура УстановитьВидимостьЭлементовФормы()
	...
	Элементы.ОплатитьЭСФСС.Видимость = Ложь;
...
Процедура ПередЗакрытиемЧека(Отказ, ОписаниеОповещения = Неопределено)
	...
	Если Не ТолькоПросмотр И (Модифицированность ИЛИ ЗначениеЗаполнено(Объект.Ссылка)) Тогда
		...
		ЗаголовокКнопкиЗакрыть = НСтр("ru = 'Закрыть'");
		Если ПраваДоступа.КорректировкаСтрок И Модифицированность Тогда
			ЗаголовокКнопкиЗакрыть = НСтр("ru = 'Закрыть без сохранения'");
		КонецЕсли;
		Кнопки.Добавить(3, ЗаголовокКнопкиЗакрыть);
...
Процедура ПослеЗавершенияПечатиСлипЧека(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если Не РезультатВыполнения.Результат Тогда
		...
		РозничныеПродажиВызовСервера.ЗаписьВЖурналРегистрации(
			ТекстСообщения, "ПослеЗавершенияПечатиСлипЧека", Объект.Ссылка);
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
...
&НаСервере
Функция СтруктураДействийПриИзмененииНоменклатурыТабличнойЧастиТовары()
...
&НаКлиенте
Процедура ПроверитьРазрешениеНаУдалениеСтрокиТабличнойЧасти(ИмяТабличнойЧасти, Отказ)
	
	ПроверитьРазрешениеНаУдалениеСтрокиТабличнойЧастиЛокализация(ИмяТабличнойЧасти, Отказ);
...
&НаСервере
Процедура ПриСозданииНаСервереЛокализация(Форма, Отказ, СтандартнаяОбработка)
	
	//++ Локализация
	ОбновитьПризнакПродажиТабачнойПродукцииТолькоПоМРЦЛокализация(ЭтотОбъект);
	УстановитьФункциональнуюОпциюОплатаЭСФССЛокализация();
	УстановитьВозможностьИспользованиеОплатыСБПНаФормахРМКЛокализация();
	ОпределятьПринадлежностьТовараИСтавкуНДСПриПодборе
		= ПолучитьФункциональнуюОпцию("ОпределятьПринадлежностьТовараИСтавкуНДСПриПодборе");
	Если Не ОткрыватьФормуСПодключеннымОборудованием = Истина Тогда
		ПроверитьКорректностьЦеныПродажиИМРЦТабачнойПродукцииЛокализация();
	КонецЕсли;
	РозничныеПродажиЛокализация.НастроитьФормуДляПодтвержденияВозрастаПокупателя(ЭтотОбъект, Объект);
...
&НаКлиенте
Процедура ТоварыПриНачалеРедактированияЛокализация(Элемент, НоваяСтрока, Копирование)
	
	//++ Локализация
	НастроитьПравоРедактированияКоличества();
	ОбработатьПопыткуИзмененияКоличества();
	...
	Если Копирование = Истина Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущиеДанные, ИмяРеквизитаУстановленаМРЦ) Тогда
			ТекущиеДанные.УстановленаМРЦ = Ложь;
		КонецЕсли;
	КонецЕсли;
	//-- Локализация
...
&НаКлиенте
Процедура ТоварыПриАктивизацииСтрокиЛокализация(Элемент)
	//++ Локализация
	ОтменитьИзменениеВПравахРедактированияКоличества();
	КонтрольКодовТРУ(Истина);
	КонтрольТоваровТребующихПодтвержденияВозраста(Истина);
	//-- Локализация
КонецПроцедуры

&НаКлиенте
Процедура ТоварыПриИзмененииЛокализация(Элемент)
	//++ Локализация
	КонтрольКодовТРУ();
	КонтрольТоваровТребующихПодтвержденияВозраста();
	//-- Локализация
КонецПроцедуры

&НаКлиенте
Процедура ТоварыПослеУдаленияЛокализация(Элемент)
	//++ Локализация
	КонтрольКодовТРУ();
	КонтрольТоваровТребующихПодтвержденияВозраста();
	//-- Локализация
...
&НаКлиенте
Процедура ТоварыНоменклатураПриИзмененииЛокализация(Элемент)
	//++ Локализация
	КонтрольКодовТРУ();
	КонтрольТоваровТребующихПодтвержденияВозраста();
	//-- Локализация
...
&НаСервере
Функция РазделитьМассивСтрокДляРазныхТиповПакетнойОбработки(ИсходныеСтроки)
	
	Результат = Новый Структура;
	Результат.Вставить("ТребуетсяЗаполнитьЦену", ИсходныеСтроки);
	Результат.Вставить("НеТребуетсяЗаполнитьЦену", Новый Массив);
	
	//++ Локализация
	Результат.Вставить("ТребуетсяЗаполнитьЦену", Новый Массив);
	Для Каждого ИсходнаяСтрока Из ИсходныеСтроки Цикл
		Если ИсходнаяСтрока[РозничныеПродажиКлиентСерверЛокализация.ИмяРеквизитаУстановленаМРЦ()] = Истина Тогда
			Результат.НеТребуетсяЗаполнитьЦену.Добавить(ИсходнаяСтрока);
		Иначе	
			Результат.ТребуетсяЗаполнитьЦену.Добавить(ИсходнаяСтрока);
		КонецЕсли;
	КонецЦикла;
	//-- Локализация
	Возврат Результат;
...
Процедура УправлениеДоступностьюКнопокОплатыЛокализация(Форма, ИнформацияОбОплате)
	...
	ОплатаЭСФССНачатаЧекНеПробит = ((Форма.Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.ОтложенСРезервированием")
										Или Форма.Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.КОплате"))
...
Процедура ПересчитатьДокументНаКлиентеЛокализация(ИнформацияОбОплате)
	...
	ОплатаЭСФССНачатаЧекНеПробит = ((Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.ОтложенСРезервированием") 
									Или Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.КОплате"))
									И ИнформацияОбОплате.ЭСФСС <> 0);
...
Функция ПараметрыОперацииФискализацииЧекаНаСервереЛокализация()
	...
	//++ Локализация
	ПараметрыОперацииФискализацииЧека = ФормированиеПараметровФискальногоЧекаСервер
		.ПараметрыОперацииФискализацииЧека(Объект.Ссылка, Объект.Организация, Объект.Кассир);
	...
	ФормированиеПараметровФискальногоЧекаСервер.ОбновитьПараметрыФискальногоЧекаПозицийЧекаККМ(
		ПараметрыОперацииФискализацииЧека,
		Объект.Ссылка);
...
Процедура ПриИзмененииНоменклатурыХарактеристикиКоличестваЛокализация()
	...
	Если (ВедетсяУчетЕГАИС Или ВедетсяУчетИСМП)
		И ЭтотОбъект.ПараметрыИнтеграцииГосИС["ВедетсяУчетВидовПродукцииСЧастичнымВыбытиемИС"] = Истина Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Если РозничныеПродажиЛокализация.ВедетсяУчетВидовПродукцииПоВетеринарнымСертификатам() Тогда
		ОбработатьРазрешенияНаПродажуТоваровЛокализация(ЭтотОбъект);
	КонецЕсли;
...
Процедура ТоварыПриОкончанииРедактированияЛокализация(
	...
	Если НоваяСтрока = Истина И КонтролироватьЦенуПродажиТабачнойПродукцииЛокализация() Тогда
		КэшированныеЗначения.Вставить("ОбнулитьЦенуДляТабачнойПродукции", Истина);
		ДополнительныеПараметры.Вставить("ТребуетсяСерверныйВызов", Истина);
	КонецЕсли;
...
Процедура ТоварыПриОкончанииРедактированияНаСервереЛокализация(ИдентификаторСтроки, КэшированныеЗначения)
	...
	ТекущаяСтрокаТаблицыТовары = Элементы.Товары.ТекущаяСтрока;
	Если КэшированныеЗначения.Свойство("ЗаполнитьКодыМаркировкиВскрытыхПотребительскихУпаковокИС") Тогда
		Если ТекущаяСтрокаТаблицыТовары <> Неопределено Тогда
			ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(ТекущаяСтрокаТаблицыТовары);
	...
	Если КэшированныеЗначения.Свойство("ОбнулитьЦенуДляТабачнойПродукции") И КэшированныеЗначения.ОбнулитьЦенуДляТабачнойПродукции = Истина Тогда
		Если ТекущаяСтрокаТаблицыТовары <> Неопределено Тогда
			ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(ТекущаяСтрокаТаблицыТовары);
			ОписаниеНоменклатуры = ОбщегоНазначенияИС.ОписаниеНоменклатуры(ТекущиеДанные.Номенклатура)[ТекущиеДанные.Номенклатура];
			...
			КонецЕсли;
		КонецЕсли;
		КэшированныеЗначения.Удалить("ОбнулитьЦенуДляТабачнойПродукции");
	КонецЕсли;
	
	РозничныеПродажиЛокализация.УдалитьРазрешенияНаПродажуБезСвязанныхТоваров(ЭтотОбъект);
	// -- Локализация
...
&НаСервере
Функция ВскрытыеПотребительскиеУпаковкиПоТоваруЛокализация(ДанныеПоТовару)
...
&НаСервере
Процедура ОбработатьВскрытыеПотребительскиеУпаковкиПоТаблицеТоваровЛокализация(ТаблицаТоваров)
	
	//++ Локализация
	
	Описание = ОбщегоНазначенияИС.ОписаниеНоменклатуры(ТаблицаТоваров.ВыгрузитьКолонку("Номенклатура"));
	
	Поиск = РегистрыСведений.ВскрытыеПотребительскиеУпаковкиИС.НоваяТаблицаПоискаВскрытыхПотребительскихУпаковок();
	
	Для Каждого СтрокаТЧ Из ТаблицаТоваров Цикл
		Если ЗначениеЗаполнено(Описание[СтрокаТЧ.Номенклатура].ВариантЧастичногоВыбытия) Тогда
			НоваяСтрока = Поиск.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТЧ);
			НоваяСтрока.УчитыватьСерии = ЗначениеЗаполнено(СтрокаТЧ.Серия);
			НоваяСтрока.Количество = СтрокаТЧ.КоличествоУпаковок;
		КонецЕсли;
	КонецЦикла;
	
	Результат = РегистрыСведений.ВскрытыеПотребительскиеУпаковкиИС.ВскрытыеПотребительскиеУпаковкиПоТаблицеТоваров(
		Поиск, ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(ЭтотОбъект));
...
Процедура ПередЗакрытиемЧекаЛокализация(Отказ, Кнопки)
	...
	Если ЭтоОплатаЭСФСС
		И Кнопки.Количество()	
		И (Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.ОтложенСРезервированием")
			Или Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.КОплате"))
...
&НаКлиенте
Процедура УказатьРазрешениеНаПродажуЛокализация()
	
	//++ Локализация
	
	Если Элементы.Товары.ВыделенныеСтроки.Количество() > 1 Тогда
		УказатьРазрешениеНаПродажуВСценарииНесколькихСтрок();
	ИначеЕсли Элементы.Товары.ВыделенныеСтроки.Количество() = 1 Тогда
		УказатьРазрешениеНаПродажуВСценарииОднойСтроки();
	КонецЕсли;
...
&НаКлиенте
Процедура ПроверитьРазрешениеНаУдалениеСтрокиТабличнойЧастиЛокализация(ИмяТабличнойЧасти, Отказ)
	
	//++ Локализация
	Если ИмяТабличнойЧасти = "Товары" Тогда
		
		ИндексыСтрокКУдалению = Элементы.Товары.ВыделенныеСтроки;
		ИндексыСтрокКУдалению = ОбщегоНазначенияУТКлиентСервер.СортированныйМассив(ИндексыСтрокКУдалению);
		
		Для Каждого ИндексСтрокиКУдалению Из ИндексыСтрокКУдалению Цикл
			
			СтрокаТовары = Объект.Товары.НайтиПоИдентификатору(ИндексСтрокиКУдалению);
			
			ИмяРеквизитаУстановленаМРЦ = РозничныеПродажиКлиентСерверЛокализация.ИмяРеквизитаУстановленаМРЦ();
			Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТовары, ИмяРеквизитаУстановленаМРЦ) Тогда
				Продолжить;
			КонецЕсли;
			
			СтрокиСНоменклатурой = Объект.Товары.НайтиСтроки(Новый Структура("Номенклатура", СтрокаТовары.Номенклатура));
			Если СтрокиСНоменклатурой.Количество() < 2 Тогда
				Продолжить;
			КонецЕсли;
			
			Если СтрокаТовары[ИмяРеквизитаУстановленаМРЦ] = Истина Тогда
				
				Шаблон = Нстр("ru = 'Для строки №%1 с ""%2"" подобрана КМ и цена продажи ""%3"" равна МРЦ из КМ - строку запрещено удалять, используйте форму подбора и проверки табачной продукции'");
...
&НаКлиенте
Процедура ПроверитьЦеныТоваровЛокализация()
	
	ПроверитьИзмененияЦенПоПрайсЛисту = Истина;
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, ИмяРеквизитаЕстьОшибкаВЦенеПоМРЦ)
		И ЭтотОбъект[ИмяРеквизитаЕстьОшибкаВЦенеПоМРЦ] = Истина Тогда
		ПроверитьИзмененияЦенПоПрайсЛисту = Ложь;
		ПодключитьОбработчикОжидания("ЗадатьВопросИсправленияОшибкиВЦенахПоМРЦ", 0.1, Истина);
	КонецЕсли;
	//-- Локализация
	
	Если ПроверитьИзмененияЦенПоПрайсЛисту Тогда
		ПроверитьЦеныПоПрайсЛистуЛокализация();
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ПроверитьЦеныПоПрайсЛистуЛокализация()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ИначеЕсли ИспользуетсяЦенообразование25 Тогда 
			СтруктураДействий.Вставить(
				"ЗаполнитьУсловияРозничныхПродаж",
				ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияУсловийРозничныхПродажВСтрокеТЧ(Объект, ОбъектХраненияУсловийПродаж));
		Иначе
			СтруктураДействий.Вставить(
		...
		Для Каждого СтрокаСЦенойПрайса Из ОбрабатываемыеСтроки Цикл
			...
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, ИмяРеквизитаЕстьОшибкаВЦенеПоМРЦ)
				...
				Если Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(СтрокаСЦенойПрайса.ВидПродукцииИС) Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			//-- Локализация
			
			НомерСтроки = СтрокаСЦенойПрайса.НомерСтроки;
			СтрокаТЧТовары = Объект.Товары[НомерСтроки - 1];
			
			Если СтрокаСЦенойПрайса.Цена <> СтрокаТЧТовары.Цена Тогда
				
				ТекстСообщения = СтрШаблон(
					НСтр("ru = 'В строке №%1 для ""%2"" цена позиции чека - ""%3"" отличается от цены прайс-листа - ""%4""'"),
					НомерСтроки,
					СтрокаТЧТовары.Номенклатура,
					СтрокаТЧТовары.Цена,
					СтрокаСЦенойПрайса.Цена);
				
				РозничныеПродажиВызовСервера.ЗаписьВЖурналРегистрации(
					ТекстСообщения, "ОткрытиеОтложенногоЧека", Объект.Ссылка);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
...
Функция ТекстДополнительногоОписанияДляРазливногоПива(ИнформацияПоВскрытымУпаковкам, АкцизныеМарки, СтрокаТовары)
	...
	Для Каждого ДанныеПоВскрытойУпаковке Из НайденныеСтрокиТовары Цикл
		...
		Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ПоддерживаетсяПодключениеКОборудованиюРозлива(
			ДанныеПоВскрытойУпаковке.ВидПродукции) Тогда
...
Функция ДанныеДокументаТовары()
	...
	|	АкцизныеМарки.РазрешительныйРежимИдентификаторЭкземпляраПО КАК РазрешительныйРежимИдентификаторЭкземпляраПО,
	|	АкцизныеМарки.РазрешительныйРежимВерсияБазы                КАК РазрешительныйРежимВерсияБазы,
	|	АкцизныеМарки.РазрешительныйРежимМинимальнаяРозничнаяЦена  КАК РазрешительныйРежимМинимальнаяРозничнаяЦена
...
Функция ЦветаТекстаПодсказки()
	...
	ЦветаТекстаПодсказки.Вставить("Предупреждение", ЦветаСтиля["ЦветТекстаПредупреждение"]);
...
&НаКлиенте
Процедура ЗадатьВопросИсправленияОшибкиВЦенахПоМРЦ()
	
	ПоказатьВопрос(
		Новый ОписаниеОповещения("ЗадатьВопросИсправленияОшибкиВЦенахПоМРЦЗавершение", ЭтотОбъект),
		НСтр("ru = 'В случае отказа будут удалены все КМ табачной продукции, по которой есть некорректные цены продажи.'"),
		РежимДиалогаВопрос.ДаНет,,,
		НСтр("ru = 'Цена продажи табачной продукции не прошла проверку по МРЦ и ЕМРЦ. Исправить?'"));
	
КонецПроцедуры

&НаКлиенте
Процедура ЗадатьВопросИсправленияОшибкиВЦенахПоМРЦЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	ОчиститьСообщения();
	ЗадатьВопросИсправленияОшибкиВЦенахПоМРЦЗавершениеНаСервере(
...
&НаСервере
Процедура ЗадатьВопросИсправленияОшибкиВЦенахПоМРЦЗавершениеНаСервере(Результат, ДополнительныеПараметры)
	
	ПроверитьНаличиеМРЦНаСоответствующиеЕМРЦ();
	
	Если Результат Тогда
		
		ПроверитьИсправитьНаличиеСкидокОплатБонуснымиБалламиЛокализация();
		ПривестиЦеныПродажиКМРЦ();
		
	Иначе
		
		// Удалить все ШК для товаров
		УдалитьШтрихкодыУпаковокПоТоварамСНекорректнойЦеной();
		
	КонецЕсли;
	
	ИмяРеквизитаЕстьОшибкаВЦенеПоМРЦ = РозничныеПродажиКлиентСерверЛокализация.ИмяРеквизитаЕстьОшибкаВЦенеПоМРЦ();
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, ИмяРеквизитаЕстьОшибкаВЦенеПоМРЦ) Тогда
		ЭтотОбъект[ИмяРеквизитаЕстьОшибкаВЦенеПоМРЦ] = Ложь;
	КонецЕсли;
...
&НаСервере
Процедура ПроверитьНаличиеМРЦНаСоответствующиеЕМРЦ()
	
	ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(ЭтотОбъект);
	ШтрихкодыУпаковок = Объект.АкцизныеМарки.Выгрузить().ВыгрузитьКолонку("АкцизнаяМарка");
...
&НаСервере
Процедура ПривестиЦеныПродажиКМРЦ()
...
&НаСервере
Процедура УдалитьШтрихкодыУпаковокПоТоварамСНекорректнойЦеной()
	
	ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(ЭтотОбъект);
	
	ШтрихкодыУпаковокСМРЦ = РозничныеПродажиЛокализация.ПолучитьШтрихкодыУпаковокСМРЦ(
		Объект.АкцизныеМарки.Выгрузить().ВыгрузитьКолонку("ШтрихкодУпаковки"),
		ПараметрыСканирования);
	РозничныеПродажиЛокализация.УдалитьШтрихкодыУпаковокПоТоварамСНекорректнойЦеной(ЭтотОбъект, ШтрихкодыУпаковокСМРЦ);
...
&НаКлиенте
Процедура НастроитьПравоРедактированияКоличества()
...
&НаКлиенте
Процедура ОбработатьПопыткуИзмененияКоличества()
...
&НаКлиенте
Процедура ОтменитьИзменениеВПравахРедактированияКоличества()
...
&НаСервере
Процедура ПроверитьКорректностьЦеныПродажиИМРЦТабачнойПродукцииЛокализация()
	
	РозничныеПродажиЛокализация.ПроверитьКорректностьЦеныПродажиИМРЦТабачнойПродукции(
		ЭтотОбъект, "АкцизныеМарки", Истина, Ложь);
	
КонецПроцедуры

&НаСервере
Функция КонтролироватьЦенуПродажиТабачнойПродукцииЛокализация()
	
	Возврат РозничныеПродажиЛокализация.КонтролироватьЦенуПродажиТабачнойПродукции(ТекущаяДатаСеанса());
	
КонецФункции

&НаКлиенте
Процедура УказатьРазрешениеНаПродажуВСценарииНесколькихСтрок()
	
	ПозицииЧекаКЗаполнению.Очистить();
	
	ЕстьЗаполненныеРазрешения = Ложь;
	
	Для Каждого ИдентификаторВыбраннойСтроки Из Элементы.Товары.ВыделенныеСтроки Цикл
	
		ВыбраннаяСтрока = Объект.Товары.НайтиПоИдентификатору(ИдентификаторВыбраннойСтроки);
		
		Если ВыбраннаяСтрока.ПродаетсяПоРазрешению Тогда
			СтрокаЧека = ПозицииЧекаКЗаполнению.Добавить();
			СтрокаЧека.ИдентификаторСтроки = ВыбраннаяСтрока.ИдентификаторСтроки;
			СтрокаЧека.Номенклатура = ВыбраннаяСтрока.Номенклатура;
		КонецЕсли;
		
		СтруктураПоиска = Новый Структура("ИдентификаторСтроки", ВыбраннаяСтрока.ИдентификаторСтроки);
		СтрокиРазрешенияНаПродажу = Объект.РазрешенияНаПродажу.НайтиСтроки(СтруктураПоиска);
		
		Если СтрокиРазрешенияНаПродажу.Количество() > 0 Тогда
			Если ЗначениеЗаполнено(СтрокиРазрешенияНаПродажу[0].ТипДокумента) Тогда
				ЕстьЗаполненныеРазрешения = Истина;
			КонецЕсли;
		КонецЕсли;
		
	КонецЦикла;
	
	Если ЕстьЗаполненныеРазрешения Тогда
		ПоказатьВопрос(
			Новый ОписаниеОповещения("УказатьРазрешениеНаПродажуДляВыделенныхСтрокВопросЗавершение", ЭтотОбъект),
			НСтр("ru = 'В выделенных позициях чека есть номенклатура, для которой указаны разрешения на продажу. Заменить?'"),
			РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	ИначеЕсли ПозицииЧекаКЗаполнению.Количество() > 0 Тогда
		ОткрытьФормуУказанияРазрешенияНаПродажу();
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'В выделенных позициях чека нет номенклатуры, для которой требуется указать разрешение на продажу'"));
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура УказатьРазрешениеНаПродажуДляВыделенныхСтрокВопросЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ОткрытьФормуУказанияРазрешенияНаПродажу()
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ПозицииЧекаКЗаполнению", ПозицииЧекаКЗаполнению);
...
&НаКлиенте
Процедура УказатьРазрешениеНаПродажуВСценарииОднойСтроки()
	...
	Если ТекущаяСтрока = Неопределено Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не выбрана позиция чека, для которой нужно указать разрешение на продажу'"));
		Возврат;
	КонецЕсли;
	
	Если Не ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(ТекущаяСтрока, "ПродаетсяПоРазрешению") Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'В позициях чеках нет номенклатуры, для которой нужно указать разрешение на продажу'"));
		Возврат;
	КонецЕсли;
	...
	Если ТекущаяСтрока.ПродаетсяПоРазрешению Тогда
		...
		ПараметрыФормы.Вставить("ИдентификаторСтроки", ТекущаяСтрока.ИдентификаторСтроки);
		
		СтруктураПоиска = Новый Структура("ИдентификаторСтроки", ТекущаяСтрока.ИдентификаторСтроки);
		СтрокиРазрешенияНаПродажу = Объект.РазрешенияНаПродажу.НайтиСтроки(СтруктураПоиска);
		
		Если СтрокиРазрешенияНаПродажу.Количество() > 0 Тогда
			ПараметрыФормы.Вставить("ТипДокумента", СтрокиРазрешенияНаПродажу[0].ТипДокумента);
			ПараметрыФормы.Вставить("НомерДокумента", СтрокиРазрешенияНаПродажу[0].НомерДокумента);
			ПараметрыФормы.Вставить("ДатаДокумента", СтрокиРазрешенияНаПродажу[0].ДатаДокумента);
		КонецЕсли;
	...
	Для Каждого СтрокаТовары Из Объект.Товары Цикл
		...
		Если СтрокаТовары.ИдентификаторСтроки = ТекущаяСтрока.ИдентификаторСтроки
			Или СтрокаТовары.ПродаетсяПоРазрешению = Ложь Тогда
			Продолжить;
		КонецЕсли;
		...
		Если СтрокиРазрешенияНаПродажу.Количество() = 0
			Или Не ЗначениеЗаполнено(СтрокиРазрешенияНаПродажу[0].ТипДокумента) Тогда
			
			СтрокаЧека = ПозицииЧекаКЗаполнению.Добавить();
			СтрокаЧека.ИдентификаторСтроки = СтрокаТовары.ИдентификаторСтроки;
			СтрокаЧека.Номенклатура = СтрокаТовары.Номенклатура;
		КонецЕсли;
	...
	Если ТекущаяСтрока.ПродаетсяПоРазрешению Или ПозицииЧекаКЗаполнению.Количество() > 0 Тогда
	...
	Иначе
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'В позициях чеках нет номенклатуры, для которой не указано разрешение на продажу'"));
...
&НаКлиенте
Процедура ОткрытьФормуУказанияДокументаОтпускаПрепаратаЗавершение(ДанныеРазрешенияНаПродажу, ДополнительныеПараметры) Экспорт
	
	Если ДанныеРазрешенияНаПродажу <> Неопределено
		И ДанныеРазрешенияНаПродажу.Результат <> Ложь Тогда
		
		Если ЗначениеЗаполнено(ДанныеРазрешенияНаПродажу.ИдентификаторСтроки) Тогда
			
			ЗаполнитьРазрешениеНаПродажуВПозицииЧека(
				ДанныеРазрешенияНаПродажу.ИдентификаторСтроки,
				ДанныеРазрешенияНаПродажу);
			
		КонецЕсли;
		
		Если ДанныеРазрешенияНаПродажу.Свойство("ПозицииЧекаКЗаполнению")
			И ДанныеРазрешенияНаПродажу.ПозицииЧекаКЗаполнению.Количество() > 0 Тогда
			
			Для Каждого СтрокаПозицийЧекаКЗаполнению Из ДанныеРазрешенияНаПродажу.ПозицииЧекаКЗаполнению Цикл
				
				ЗаполнитьРазрешениеНаПродажуВПозицииЧека(
					СтрокаПозицийЧекаКЗаполнению.ИдентификаторСтроки,
					ДанныеРазрешенияНаПродажу);
...
&НаКлиенте
Процедура ЗаполнитьРазрешениеНаПродажуВПозицииЧека(ИдентификаторСтроки, РеквизитыРазрешенияНаПродажу)
	...
	Если СтрокиТовары.Количество() = 0 Тогда
		СтрокиТовары[0].РеквизитыРазрешенияНаПродажу = "";
		Возврат;
	КонецЕсли;
	
	СтруктураПоиска = Новый Структура("ИдентификаторСтроки", ИдентификаторСтроки);
	СтрокиРазрешениеНаПродажу = Объект.РазрешенияНаПродажу.НайтиСтроки(СтруктураПоиска);
	
	Если СтрокиРазрешениеНаПродажу.Количество() = 0 Тогда
		СтрокаРазрешенияНаПродажу = Объект.РазрешенияНаПродажу.Добавить();
	Иначе
		СтрокаРазрешенияНаПродажу = СтрокиРазрешениеНаПродажу[0];
	КонецЕсли;
	
	СтрокаРазрешенияНаПродажу.ИдентификаторСтроки = ИдентификаторСтроки;
	СтрокаРазрешенияНаПродажу.ТипДокумента   = РеквизитыРазрешенияНаПродажу.ТипДокумента;
	СтрокаРазрешенияНаПродажу.НомерДокумента = РеквизитыРазрешенияНаПродажу.НомерДокумента;
	СтрокаРазрешенияНаПродажу.ДатаДокумента = РеквизитыРазрешенияНаПродажу.ДатаДокумента;
	
	СтрокиТовары[0].РеквизитыРазрешенияНаПродажу = "" + СтрокаРазрешенияНаПродажу.ТипДокумента
		+ " №" + СтрокаРазрешенияНаПродажу.НомерДокумента + " от "
		+ Формат(СтрокаРазрешенияНаПродажу.ДатаДокумента, "ДФ=dd.MM.yyyy");
...
Процедура ОбработатьРезультатОплатыЭСФСС(Знач РезультатВыполнения = Неопределено, Знач ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если РезультатВыполнения.Свойство("СуммаЭлектронногоСертификата")
		...
		РаспределитьТоварыПоОплатамЭСФСС(РезультатВыполнения.ТоварыФСС);
		...
		ЕстьОплатаСертификатамиФСС = Истина;
		УстановитьВидимостьРеквизитаТаблицыТоварыИспользованияСертификатаФСС();
...
&НаСервере
Процедура РаспределитьТоварыПоОплатамЭСФСС(ТоварыФСС)
	
	ТоварыОплаченныеФСС = ТоварыФСС.Выгрузить();
	ТоварыОплаченныеФСС.Свернуть(
		"Номенклатура, КодТовараТРУ, ИдентификаторСтроки",
		"Количество, КоличествоФСС");
	
	МассивИзмененныхСтрок = Новый Массив;
	
	Для Каждого СтрокаТоварыОплаченныеФСС Из ТоварыОплаченныеФСС Цикл
		
		СтрокиНайденныхТоваров = Объект.Товары.НайтиСтроки(
			Новый Структура("ИдентификаторСтроки", СтрокаТоварыОплаченныеФСС.ИдентификаторСтроки));
		
		КоличествоФСС = СтрокаТоварыОплаченныеФСС.КоличествоФСС;
		
		Для Каждого СтрокаТовара Из СтрокиНайденныхТоваров Цикл
			
			Если КоличествоФСС = 0 Тогда
				Прервать;
			КонецЕсли;
			
			Если СтрокаТоварыОплаченныеФСС.Количество <= СтрокаТоварыОплаченныеФСС.КоличествоФСС Тогда
				
				СтрокаТовара.ИспользованСертификатФСС = Истина;
				
			ИначеЕсли СтрокаТоварыОплаченныеФСС.Количество > КоличествоФСС Тогда
				
				Если СтрокаТовара.Количество <= КоличествоФСС Тогда
					СтрокаТовара.ИспользованСертификатФСС = Истина;
					КоличествоФСС = КоличествоФСС - СтрокаТовара.Количество;
					Продолжить;
				КонецЕсли;
				
				КоэффициентПересчетаФСС = КоличествоФСС / СтрокаТовара.Количество;
				КоэффициентПересчетаУпаковки = СтрокаТовара.Количество /  СтрокаТовара.КоличествоУпаковок;
				
				СтрокаТовараКопия = Объект.Товары.Вставить(СтрокаТовара.НомерСтроки-1);
				ЗаполнитьЗначенияСвойств(СтрокаТовараКопия, СтрокаТовара);
				
				СтрокаТовараКопия.ИспользованСертификатФСС = Истина;
				СтрокаТовараКопия.ИдентификаторСтроки = Новый УникальныйИдентификатор();
				
				СтрокаТовараКопия.Количество = КоличествоФСС;
				СтрокаТовара.Количество = СтрокаТовара.Количество - КоличествоФСС;
				
				СтрокаТовараКопия.КоличествоУпаковок
					= СтрокаТовараКопия.Количество / КоэффициентПересчетаУпаковки;
				СтрокаТовара.КоличествоУпаковок
					= СтрокаТовара.КоличествоУпаковок - СтрокаТовараКопия.КоличествоУпаковок;
				
				СтрокаТовараКопия.СуммаРучнойСкидки
					= СтрокаТовараКопия.СуммаРучнойСкидки * КоэффициентПересчетаФСС;
				СтрокаТовара.СуммаРучнойСкидки
					= СтрокаТовара.СуммаРучнойСкидки - СтрокаТовараКопия.СуммаРучнойСкидки;
				
				СтрокаТовараКопия.СуммаАвтоматическойСкидки
					= СтрокаТовараКопия.СуммаАвтоматическойСкидки * КоэффициентПересчетаФСС;
				СтрокаТовара.СуммаАвтоматическойСкидки
					= СтрокаТовара.СуммаАвтоматическойСкидки - СтрокаТовараКопия.СуммаАвтоматическойСкидки;
				
				СтрокаТовараКопия.СуммаБонусныхБалловКСписанию
					= СтрокаТовараКопия.СуммаБонусныхБалловКСписанию * КоэффициентПересчетаФСС;
				СтрокаТовара.СуммаБонусныхБалловКСписанию
					= СтрокаТовара.СуммаБонусныхБалловКСписанию - СтрокаТовараКопия.СуммаБонусныхБалловКСписанию;
				
				СтрокаТовараКопия.СуммаНачисленныхБонусныхБалловВВалюте
					= СтрокаТовараКопия.СуммаНачисленныхБонусныхБалловВВалюте * КоэффициентПересчетаФСС;
				СтрокаТовара.СуммаНачисленныхБонусныхБалловВВалюте
					= СтрокаТовара.СуммаНачисленныхБонусныхБалловВВалюте - СтрокаТовараКопия.СуммаНачисленныхБонусныхБалловВВалюте;
				
				СтрокаТовараКопия.СуммаБонусныхБалловКСписаниюВВалюте
					= СтрокаТовараКопия.СуммаБонусныхБалловКСписаниюВВалюте * КоэффициентПересчетаФСС;
				СтрокаТовара.СуммаБонусныхБалловКСписаниюВВалюте
					= СтрокаТовара.СуммаБонусныхБалловКСписаниюВВалюте - СтрокаТовараКопия.СуммаБонусныхБалловКСписаниюВВалюте;
				
				МассивИзмененныхСтрок.Добавить(СтрокаТовара);
				МассивИзмененныхСтрок.Добавить(СтрокаТовараКопия);
	...
	Если МассивИзмененныхСтрок.Количество() > 0 Тогда
		...
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			МассивИзмененныхСтрок, СтруктураДействий, Объект.Товары);
...
Процедура ПередатьДанныеФискальногоЧекаНСПКЗавершение(РезультатВыполнения, ДополнительныеПараметры) Экспорт 
	...
	Если Не РезультатПередачи.Результат Тогда
		ТекстСообщения = СтрШаблон(
			...
			РозничныеПродажиВызовСервера.ЗаписьВЖурналРегистрации(
				ТекстСообщения, "ПередатьДанныеФискальногоЧекаНСПКЗавершение", Объект.Ссылка);
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура УстановитьДоступностьКнопкиОплатитьЭСФСС(Значение)
	...
	Если Значение И СуммаСБПВыполнена = 0 И СуммаСБПОжидание = 0 Тогда
		...
		Если (Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.ОтложенСРезервированием")
								Или Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.КОплате"))
							И ЭтоОплатаЭСФСС Тогда
		...
		ИначеЕсли (Не (Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.ОтложенСРезервированием")
						Или Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.КОплате"))
...
// Возвращаемое значение:
//	Структура:
// 	 *ВсеКодыТРУ - Соответствие из КлючИЗначение:
...
// Управление возможностью выполнения проверки возраста покупателя. 
&НаКлиенте
Процедура КонтрольТоваровТребующихПодтвержденияВозраста(Знач ТолькоПриИзмененииКоличестваСтрок = Ложь)
	
	Если ТипЗнч(КэшТоваровТребующихПодтвержденияВозраста) <> Тип("Структура") Тогда
		КэшТоваровТребующихПодтвержденияВозраста = НовыйКэшТоваровТребующихПодтвержденияВозраста();
	КонецЕсли;
	
	Если ТолькоПриИзмененииКоличестваСтрок
		И КэшТоваровТребующихПодтвержденияВозраста.КоличествоСтрокНаМоментОбработки = Объект.Товары.Количество() Тогда
		Возврат;
	КонецЕсли;
	КэшТоваровТребующихПодтвержденияВозраста.КоличествоСтрокНаМоментОбработки = Объект.Товары.Количество();
	
	ИмяРеквизитЕстьТоварыТребующиеПодтвержденияВозраста = 
		РозничныеПродажиКлиентСерверЛокализация.ИмяРеквизитаЕстьТоварыТребующиеПодтвержденияВозрастаПокупателя();

	ЭтотОбъект[ИмяРеквизитЕстьТоварыТребующиеПодтвержденияВозраста] = Ложь;
	ТоварыДляПроверки = Новый Массив;
	Для Каждого СтрокаТовары Из Объект.Товары Цикл
		Если Не ЗначениеЗаполнено(СтрокаТовары.Номенклатура) Тогда
			Продолжить;
		КонецЕсли;
		ТребуетсяПодтверждениеВозраста = КэшТоваровТребующихПодтвержденияВозраста.ВсеТовары.Получить(СтрокаТовары.Номенклатура);
		Если ТребуетсяПодтверждениеВозраста = Неопределено 
			И ТоварыДляПроверки.Найти(СтрокаТовары.Номенклатура) = Неопределено Тогда
			ТоварыДляПроверки.Добавить(СтрокаТовары.Номенклатура);
		ИначеЕсли ТребуетсяПодтверждениеВозраста = Истина Тогда
			ЭтотОбъект[ИмяРеквизитЕстьТоварыТребующиеПодтвержденияВозраста] = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	КонтрольТоваровТребующихПодтвержденияВозрастаНаСервере(ТоварыДляПроверки, КэшТоваровТребующихПодтвержденияВозраста);
КонецПроцедуры

// Управление возможностью выполнения проверки возраста покупателя. 
&НаСервере
Процедура КонтрольТоваровТребующихПодтвержденияВозрастаНаСервере(ТоварыДляПроверки, КэшТоваровТребующихПодтвержденияВозраста)
	ИмяРеквизитЕстьТоварыТребующиеПодтвержденияВозраста = 
		РозничныеПродажиКлиентСерверЛокализация.ИмяРеквизитаЕстьТоварыТребующиеПодтвержденияВозрастаПокупателя();
	Если ТоварыДляПроверки.Количество() Тогда
		КлючиВидовПродукцииИСНоменклатуры = РозничныеПродажиЛокализация.КлючиВидовПродукцииИСТоварныхПозицийДокументаПродажи(ТоварыДляПроверки);
		Для Каждого ТоварДляПроверки  Из ТоварыДляПроверки Цикл
			КлючВидаПродукцииИСТребуетПроверкиВозраста = Ложь;
			КлючВидаПродукцииИС = КлючиВидовПродукцииИСНоменклатуры[ТоварДляПроверки];
			Если ЗначениеЗаполнено(КлючВидаПродукцииИС) Тогда
				КлючВидаПродукцииИСТребуетПроверкиВозраста = КонструкторИСОбщегоНазначенияКлиентСервер.ПроверятьВозраст(КлючВидаПродукцииИС);
				ЭтотОбъект[ИмяРеквизитЕстьТоварыТребующиеПодтвержденияВозраста] = ЭтотОбъект[ИмяРеквизитЕстьТоварыТребующиеПодтвержденияВозраста]
																Или КлючВидаПродукцииИСТребуетПроверкиВозраста;
			КонецЕсли;
			КэшТоваровТребующихПодтвержденияВозраста.ВсеТовары.Вставить(ТоварДляПроверки, КлючВидаПродукцииИСТребуетПроверкиВозраста);
		КонецЦикла;
	КонецЕсли;
	РозничныеПродажиЛокализация.ОбновитьНастройкиФормыДляПроверкиВозрастаПокупателя(ЭтотОбъект);
КонецПроцедуры

// Возвращаемое значение:
//	Структура:
// 	 *ВсеТовары - Соответствие из КлючИЗначение:
//   **Ключ - СправочникСсылка.Номенклатура
//   **Значение - Булево
// 	 *КоличествоСтрокНаМоментОбработки - Число
&НаКлиентеНаСервереБезКонтекста
Функция НовыйКэшТоваровТребующихПодтвержденияВозраста()
	
	Результат = Новый Структура;
	Результат.Вставить("ВсеТовары", Новый Соответствие);
	Результат.Вставить("КоличествоСтрокНаМоментОбработки", 0);
...
&НаСервере
Процедура ОпределятьПринадлежностьТовараИСтавкуНДСПередОплатойЛокализация()
	...
	СтруктураДействий.Вставить("ЗаполнитьСтавкуНДС",
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект));
	СтруктураДействий.Вставить("ЗаполнитьСобственникаТовараИСтавкуНДСПриУСНКомитентов",
		ПакетнаяОбработкаТабличнойЧастиКлиентСерверЛокализация.ПараметрыЗаполненияСобственникаТовараИСтавкиНДСПриУСНКомитентов(Объект));
...
&НаКлиенте
Процедура Подключаемый_ПодтверждениеВозрастаПокупателяЧерезМаксЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Подключаемый_ПодтверждениеВозрастаПокупателяЧерезМаксЗавершениеНаСервере(Результат, ДополнительныеПараметры);
	
КонецПроцедуры

&НаСервере
Процедура Подключаемый_ПодтверждениеВозрастаПокупателяЧерезМаксЗавершениеНаСервере(Результат, ДополнительныеПараметры)
	
	РозничныеПродажиЛокализация.ОбработатьРезультатПодтвержденияВозрастаПокупателяЧерезМакс(ЭтотОбъект, Результат, ДополнительныеПараметры);
	
КонецПроцедуры

&НаСервере
Процедура УстановитьВозможностьИспользованиеОплатыСБПНаФормахРМКЛокализация()
	РозничныеПродажиЛокализация.УстановитьВозможностьИспользованиеОплатыСБПНаФормахРМК(ЭтотОбъект);
КонецПроцедуры

&НаСервере
Процедура УстановитьФункциональнуюОпциюОплатаЭСФССЛокализация()
	РозничныеПродажиЛокализация.УстановитьФункциональнуюОпциюОплатаЭСФСС(ЭтотОбъект);
КонецПроцедуры

//-- Локализация

&НаКлиентеНаСервереБезКонтекста
Процедура ОбработатьРазрешенияНаПродажуТоваровЛокализация(Форма)
	
	//++ Локализация
	РозничныеПродажиКлиентСерверЛокализация.ОбработатьРазрешенияНаПродажуТоваров(Форма);
	//-- Локализация
...
&НаСервере
Процедура ОбновитьСтатусыИРазрешеннияТоваровЛокализация()
	
	//++ Локализация
	РозничныеПродажиЛокализация.ОбновитьСтатусыИРазрешеннияТоваров(Объект);
	//-- Локализация
