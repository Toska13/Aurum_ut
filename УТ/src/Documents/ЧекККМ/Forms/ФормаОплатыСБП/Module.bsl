...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДокументОплаты) = Тип("ДокументСсылка.ЧекККМВозврат")
		...
		Если ЗначениеЗаполнено(ДокументОплаты) Тогда
			...
			НастройкаПодключения = РозничныеПродажиЛокализация.НастройкаПодключения(ДоговорПодключения, Магазин);
			КОплате = ИнформацияОбОплате.СБП;
			Если (ТипЗнч(ДокументВозврата) = Тип("ДокументСсылка.ЧекККМВозврат") 
			  ИЛИ ТипЗнч(ДокументВозврата) = Тип("ДокументСсылка.ВозвратПодарочныхСертификатов")) И ТипЗнч(Параметры.ПолученоНаличными) = Тип("Число") Тогда
				КОплате = Мин(ИнформацияОбОплате.СБП, ИнформацияОбОплате.СуммаКОплате - ИнформацияОбОплате.ИтогоОплачено - Параметры.ПолученоНаличными);
			КонецЕсли;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСписокДоговоровИПодключений
...
Процедура ПодтвердитьОплату(Команда)
	...
	Если ТекущийЭтапПроцессаОплатыСБП = ЭтапыПроцессаОплатыВозвратаСБП().ЭтапОжиданияВнесенияОплаты Тогда
		...
		СтруктураПараметров.Вставить("РучноеПодтверждениеОплаты", Истина);
		
		ОповещениеЗавершитьОплату = Новый ОписаниеОповещения("ЗавершитьОплатуВозвратПлатежнойСистемой", ЭтотОбъект, СтруктураПараметров);
...
Процедура ЗаполнитьСписокДоговоровИПодключений(Организация)
	...
	СписокДоговоровИПодключений.Загрузить(
		Справочники.НастройкиРМК.СписокДоговоровИПодключенийСБП(Организация, КассаККМ));
...
Функция ПараметрыНастройкиПодключения()
	...
	НастройкиВозвратаОплаты = ПереводыСБПc2b.НастройкиВозвратаОплаты(ДокументОплаты, НастройкаПодключения);
...
Процедура ЗавершитьОплатуВозвратПлатежнойСистемой(Действие, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Действие = КодВозвратаДиалога.Нет Тогда   
		Возврат;
	КонецЕсли;
	...
	РезультатВыполнения.Вставить("РучноеПодтверждениеОплаты", Ложь);
	
	Если ЗначениеЗаполнено(ДополнительныеПараметры) И ДополнительныеПараметры.Свойство("РучноеПодтверждениеОплаты")
			И ДополнительныеПараметры.РучноеПодтверждениеОплаты = Истина Тогда
			РезультатВыполнения.РучноеПодтверждениеОплаты = Истина; 
			ОтложитьЧекНаСервере();
		КонецЕсли;
...
Процедура ОтобразитьСтраницуОшибкиЗавершенияОперации(ОшибкаЭтапаПроцесса, ТекстОшибки = "")
	...
	Элементы.ДекорацияНадписьПриОшибкеПодтвердитьОплату.Видимость = ДоступныеКоманды.ФормаПодтвердитьОплату;
	Элементы.ДекорацияНадписьПриОшибкеОтложитьЧек.Видимость = ДоступныеКоманды.ФормаОтложитьЧек;
...
Процедура ОпределитьСтатусОплатыЗавершение(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыполнения.Статус = "Выполнено" Тогда
		...
		СтатусОперацииВыполняется = НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииВыполняется();
		СтатусОперацииВыполнена   = НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииВыполнена();
		СтатусОперацииОтменена    = НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииОтменена();
		СтатусОперацииОшибка      = НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииОшибка();
...
Процедура ОбработатьСтатусВозврата(РезультатОперации, ОповещениеЗавершитьОплату, ЗапускПроверкиСтатуса = Истина)
	...
	СтатусОперацииТребуетсяПодтверждение = НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииТребуетсяПодтверждение();
	СтатусОперацииВыполняется            = НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииВыполняется();
	СтатусОперацииВыполнена              = НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииВыполнена();
