...
Процедура КомандаТочка(Команда)
	...
	ИначеЕсли АктивноеПоле = Элементы.НомерЭлектронногоСертификатаФСС.Имя Тогда
		
		НомерЭлектронногоСертификатаФСС = "";
	...
	Если ЭтоОплатаЭСФСС Тогда
		...
		Иначе
			Для Каждого СтрокаТоваров Из ТоварыФСС Цикл
				...
				Если СтрДлина(НомерЭлектронногоСертификатаФСС) >= 7 Тогда
					ТоварнаяПозиция.Вставить("СертификатПоследниеЦифры", Сред(НомерЭлектронногоСертификатаФСС,2,6));
				КонецЕсли;
				ТоварнаяПозиция.Вставить("ИдентификаторСтроки", СтрокаТоваров.ИдентификаторСтроки);
...
&НаКлиенте
Процедура ПолучитьСписокЭлектронныхСертификатовФСС(Команда)
	
	ПолучитьСписокЭлектронныхСертификатовФССКлиент();
...
Процедура ОплатитьЭлектроннымСертификатом(СледующаяОперация, ИдентификаторУстройства)
	...
	ПараметрыОперации.ДокументОснование = Документ;
...
Процедура ОплатитьВернутьЭСФССЗавершение(ДополнительныеПараметры)
	...
	ДополнительныеПараметры.Вставить("ТоварыФСС", ТоварыФСС);
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если НЕ ДополнительныеПараметры = Неопределено И ДополнительныеПараметры.Свойство("СледующаяОперация") Тогда
			...
			ИначеЕсли ДополнительныеПараметры.СледующаяОперация = "ПолучениеСпискаСертификатов" Тогда
				ОбработатьПолучениеСпискаЭлектронныхСертификатовФСС(РезультатВЫполнения.ДоступныеСертификаты);
			Иначе
...
Процедура ПредварительноеОдобрениеЭСФССЗавершение(РезультатВыполнения, ДополнительныеПараметры) Экспорт 
	...
	Если РезультатВыполнения.Результат Тогда
		ПредварительноеОдобрениеПолучено = Истина;
	КонецЕсли;
	СкорректироватьСуммуОплатыЭСФСС(РезультатВыполнения);
	ОбработатьРезультатПроверкиЭСФССНаСервере(РезультатВыполнения);
	Элементы.ГруппаВыбораНомераЭСФСС.Доступность = Не ПредварительноеОдобрениеПолучено;
...
Процедура ОбработатьРезультатПроверкиЭСФССНаСервере(РезультатВыполнения);
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если РезультатВыполнения.Свойство("ТоварныеПозиции")
			И РезультатВыполнения.ТоварныеПозиции.Количество() > 0 Тогда
...
// Корректирует сумму оплаты ЭС ФСС для устранения расхождений округления
//
// Параметры:
//  РезультатВыполнения - структура - результата от НСПК,
//                        содержащая СуммаСертификатами и ТоварныеПозиции
//
// * Описание:
// * Если сумма сертификатов отличается от суммы товаров в чеке, выполняется корректировка:
// * - При превышении суммы сертификатов над суммой товаров - корректируется вниз
// * - При занижении суммы сертификатов - проверяется превышение цены товара над
//  *  максимальной ценой сертификата; если превышения нет, корректируется вверх
// * - Если есть превышение цены товара над максимальной ценой сертификата,
//    корректировка не выполняется (нужна доплата).
//
&НаСервере
Процедура СкорректироватьСуммуОплатыЭСФСС(РезультатВыполнения)
	
	Если РезультатВыполнения.Результат Тогда
		
		// Проверка наличия суммы сертификатов
		Если НЕ РезультатВыполнения.Свойство("СуммаСертификатами") Тогда
			Возврат;
		КонецЕсли;    
		
		СуммаСертификатов = Число(РезультатВыполнения.СуммаСертификатами);
		// Проверка заполнения суммы
		Если НЕ ЗначениеЗаполнено(СуммаСертификатов) Тогда
			Возврат;
		КонецЕсли;  
		
		ИтогСуммыТоваровФСС = ТоварыФСС.Итог("Сумма");
		
		Если СуммаСертификатов < ИтогСуммыТоваровФСС Тогда
			Если РезультатВыполнения.Свойство("ТоварныеПозиции")
				И РезультатВыполнения.ТоварныеПозиции.Количество() > 0 Тогда
				...
				Для Каждого ТоварнаяПозиция Из РезультатВыполнения.ТоварныеПозиции Цикл
					...
					Если ТоварнаяПозиция.Свойство("Сертификаты")
						...
						И НЕ ЭтоПревышение Тогда
						
						Для Каждого Сертификат Из ТоварнаяПозиция.Сертификаты Цикл
							Если СтрокаТовара.Цена > Сертификат.МаксимальнаяЦена Тогда
								ЭтоПревышение = Истина;
								Прервать;
							КонецЕсли;
						КонецЦикла;
					КонецЕсли;
				...
				Если Не ЭтоПревышение Тогда
					РезультатВыполнения.СуммаСертификатами = ИтогСуммыТоваровФСС;
				КонецЕсли;
				
			КонецЕсли;
		ИначеЕсли СуммаСертификатов > ИтогСуммыТоваровФСС Тогда
			РезультатВыполнения.СуммаСертификатами = ИтогСуммыТоваровФСС;
		КонецЕсли;
	КонецЕсли;
...
Функция КолонкиТоварыФСС()
	...
	Результат.Вставить("КодТовараТРУ");
	
	Результат.Вставить("Количество");
	Результат.Вставить("КоличествоФСС");
	
	Результат.Вставить("Цена");
	Результат.Вставить("ЦенаФСС");
	Результат.Вставить("МаксимальнаяЦена");
	
	Результат.Вставить("Сумма");
	Результат.Вставить("СуммаФСС");
...
&НаКлиенте
Процедура ПолучитьСписокЭлектронныхСертификатовФССКлиент()
	
	Если ПредварительноеОдобрениеПолучено = Ложь Тогда
		ИдентификаторУстройства = Неопределено;
		Если ПодключенныеЭквайринговыеТерминалы.Количество() = 1 Тогда
			ИдентификаторУстройства = ПодключенныеЭквайринговыеТерминалы[0].ПодключаемоеОборудование;
		КонецЕсли;
		...
		// Сначала получаем параметры карты, затем на основании них получаем список доступных сертификатов
		ОписаниеОповещения = Новый ОписаниеОповещения("ПолучитьСписокЭлектронныхСертификатовФССКлиентПродолжение", ЭтотОбъект, ДополнительныеПараметры);
		ОборудованиеПлатежныеСистемыКлиент.НачатьПолучениеПараметровКарты(
			ОписаниеОповещения,
			УникальныйИдентификатор,
			ИдентификаторУстройства);
...
&НаКлиенте
Процедура ПолучитьСписокЭлектронныхСертификатовФССКлиентПродолжение(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		ПараметрыОперации = ПараметрыОперацииНСПКПоКассеККМ(КассаККМ);
		ПараметрыОперации.ХешНомерКарты = РезультатВыполнения.ХешНомерКарты;
		ПараметрыОперации.СсылкаНаПлатежныйСчет = РезультатВыполнения.СсылкаНаПлатежныйСчет;

		ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьОперациюЭТЗавершение", ЭтотОбъект, ДополнительныеПараметры);

		ЭлектронныеСертификатыНСПККлиент.НачатьПолучениеСпискаСертификатов(ОписаниеОповещения, ПараметрыОперации);
...
&НаКлиенте
Процедура ОбработатьПолучениеСпискаЭлектронныхСертификатовФСС(ДоступныеСертификаты)
	
	НомерЭлектронногоСертификатаФСС = "";
	ЭлементЭлектронногоСертификатаФСС = Элементы.НомерЭлектронногоСертификатаФСС;
	ЭлементЭлектронногоСертификатаФСС.СписокВыбора.Очистить();
	ЭлементЭлектронногоСертификатаФСС.РежимВыбораИзСписка = Ложь;
	
	Если ДоступныеСертификаты.Количество() Тогда
		
		Для Каждого СертификатНСПК Из ДоступныеСертификаты Цикл //Структура
			ИдентификаторСертификатаНСПК = СертификатНСПК.Идентификатор;
			ПредставлениеНомера = НСтр("ru='*'") + Прав(ИдентификаторСертификатаНСПК, 6);
			ЭлементЭлектронногоСертификатаФСС.СписокВыбора.Вставить(0, ПредставлениеНомера, ПредставлениеНомера);
		КонецЦикла;
		
		Если ДоступныеСертификаты.Количество() = 1 Тогда
			НомерЭлектронногоСертификатаФСС = ЭлементЭлектронногоСертификатаФСС.СписокВыбора[0].Значение;
		КонецЕсли;
		
		ЭлементЭлектронногоСертификатаФСС.РежимВыбораИзСписка = Истина;
	КонецЕсли;
...
Процедура УстановитьДоступностьИзмененияОплатыНаличными(Форма)
	...
	Форма.Элементы.ГруппаСтраницаПраваяКлавиатура.Доступность = ПолеПолученоНаличнымиДоступно ИЛИ Форма.АктивноеПоле = Форма.Элементы.Телефон.Имя ИЛИ Форма.АктивноеПоле = Форма.Элементы.НомерЭлектронногоСертификатаФСС.Имя;
	Форма.Элементы.ГруппаСтраницаПрочее.Доступность = ПолеПолученоНаличнымиДоступно ИЛИ Форма.АктивноеПоле = Форма.Элементы.Телефон.Имя ИЛИ Форма.АктивноеПоле = Форма.Элементы.НомерЭлектронногоСертификатаФСС.Имя;
КонецПроцедуры
...
Процедура ДобавитьЦифру(ВведеннаяЦифраСтрокой)
	...
	ИначеЕсли АктивноеПоле = Элементы.НомерЭлектронногоСертификатаФСС.Имя Тогда
		...
		НомерЭлектронногоСертификатаФСС = НомерЭлектронногоСертификатаФСС + ВведеннаяЦифраСтрокой;
		
		Если СтрДлина(НомерЭлектронногоСертификатаФСС) > 6 Тогда
			ТекущийЭлемент = Элементы.КомандаПробитьЧек;
		КонецЕсли;
...
Процедура ЗаполнитьТоварыФСС(ТоварныеПозиции)
	...
	Для Каждого СтрокаТовара Из ТоварныеПозиции Цикл
		Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаТовара, "ИдентификаторСтроки")
			И ЗначениеЗаполнено(СтрокаТовара.ИдентификаторСтроки) Тогда
			ТоварныеСтроки = ТоварыФСС.НайтиСтроки(Новый Структура("ИдентификаторСтроки", СтрокаТовара.ИдентификаторСтроки));
		Иначе
...
Процедура НастроитьВидимостьЭлементов(ИнформацияОбОплате)
	...
	Элементы.ГруппаВыбораНомераЭСФСС.Видимость       = Ложь;
	Элементы.ГруппаВыбораНомераЭСФСС.Доступность     = Истина;
	Если ЭтоОплатаЭСФСС Тогда
		...
		Иначе
			...
			Элементы.ГруппаВыбораНомераЭСФСС.Видимость = Не ОплатаЭСФССПрошлаЧекНеПробит;
			Если ПредварительноеОдобрениеПолучено = Истина Тогда
				Элементы.ГруппаВыбораНомераЭСФСС.Доступность = Ложь;
			КонецЕсли;
		КонецЕсли;
		
		Элементы.КнопкаСтатусТоваровФСС.Видимость = Истина;
		Элементы.ГруппаВыбораНомераЭСФСС.Видимость = Истина;
	Иначе
