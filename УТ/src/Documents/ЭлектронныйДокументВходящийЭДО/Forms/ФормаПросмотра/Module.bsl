...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия =
		ИнтерфейсДокументовЭДОКлиент.ИмяСобытияИзменениеДанныхСопоставленияНоменклатурыДокумента()
		И Параметр.Найти(Объект.Ссылка) <> Неопределено Тогда
		ПослеИзмененияДанныхСопоставленияНоменклатуры();
	КонецЕсли;	
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ИзмененВидДокумента Тогда
		...
		НовыеПараметрыЗаписи.Вставить("ИзменениеВидаДокумента", Истина);
...
Процедура ОтказатьсяОтПодписания(Команда)
	...
	ОтказатьсяОтПодписанияОбъектов(ПакетнаяОбработка);
	
КонецПроцедуры

&НаКлиенте
Процедура ОтказатьсяОтПодписанияДокумент(Команда)
...
Процедура Аннулировать(Команда)
	...
	РезультатПроверки = ТрансграничныйЭДОВызовСервера.ПроверитьЗапретАннулирования(Объект.Ссылка);
	Если РезультатПроверки.АннулированиеЗапрещено Тогда
		ШаблонСообщения = НСтр("ru = 'Действие не выполнено по причине: %1'");
...
&НаКлиенте
Процедура ВключитьРедактированиеРеквизитовНеформализованныхДокументов(Команда)
	
	Элементы.ВключитьРедактированиеРеквизитовНеформализованныхДокументов.Пометка =
		Не Элементы.ВключитьРедактированиеРеквизитовНеформализованныхДокументов.Пометка;
	НастроитьВидимостьРеквизитовДокументаДляРедактирования(ЭтотОбъект);
	РедактированиеРеквизитовВключено = Элементы.ВключитьРедактированиеРеквизитовНеформализованныхДокументов.Пометка;
	
	ИнтерфейсДокументовЭДОВызовСервера.ВключитьРедактированиеРеквизитовВходящихНеформализованныхДокументов(
		РедактированиеРеквизитовВключено);
...
Процедура ПолучитьДанныеПакета()
	...
	ТребуетсяОднородностьОтвета = СинхронизацияЭДО.ТребуетсяОднородностьОтвета(Объект.ИдентификаторКонтрагента) 
		ИЛИ ПакетыДокументовЭДО.ЭтоЗакрытыйПакет(ИдентификаторПакета);
...
Процедура НастроитьОформлениеФормы(ЭлементСхемы)
	...
	Элементы.ВидДокумента.Видимость = Элементы.ВидДокумента.СписокВыбора.Количество() > 1;
	Элементы.ЗаписатьДокумент.Видимость = Элементы.ВидДокумента.Видимость;
	ИнтерфейсДокументовЭДО.НастроитьКомандуРедактированияРеквизитовВходящихНеформализованныхДокументов(
		Элементы.ВключитьРедактированиеРеквизитовНеформализованныхДокументов,
		ЕстьПравоОбработкиДокументов, Элементы.ВидДокумента.Видимость);
	НастроитьВидимостьРеквизитовДокументаДляРедактирования(ЭтотОбъект);
...
Процедура НастроитьВидимостьИНаименованиеДействийПоСостояниюЭДО()
	...
	Элементы.ОтказатьсяОтПодписания.Видимость = ЕстьДействиеПоЭДО(ДействияНаЭтапе, Перечисления.ДействияПоЭДО.Отклонить)
		И ИспользуютсяУниверсальныеСообщения;
	Элементы.ОтказатьсяОтПодписанияДокумент.Видимость = ЕстьДействиеПоЭДО(ДействияНаЭтапеДляДокумента, Перечисления.ДействияПоЭДО.Отклонить)
		И ИспользуютсяУниверсальныеСообщения;
...
&НаКлиенте
Процедура ОтказатьсяОтПодписанияОбъектов(ПакетнаяОбработка)
...
Процедура ПодготовитьОтражениеВУчете()
	...
	Если ЭлементСхемы.ТипДокумента = Перечисления.ТипыДокументовЭДО.УПД
		И Не ОтражениеВУчетеЭДО.ОтражатьУниверсальныеДокументыОднимОбъектомУчета() Тогда
	...
	ИначеЕсли ЭлементСхемы.ТипДокумента = Перечисления.ТипыДокументовЭДО.УКД
		И Не ОтражениеВУчетеЭДО.ОтражатьУниверсальныеДокументыОднимОбъектомУчета() Тогда
	...
	ОбщегоНазначенияБЭДКлиентСервер.УдалитьЗначенияПоНачалуСтроки(СписокТиповДокументовДоп, "Удалить");
	ОбщегоНазначенияБЭДКлиентСервер.УдалитьЗначенияПоНачалуСтроки(СписокТиповДокументов, "Удалить");
	
	СписокТиповДокументовДоп.СортироватьПоПредставлению();
	...
	СписокПодбораОбщий.СортироватьПоПредставлению();
	СписокПодбораОсновной.СортироватьПоЗначению();
	СписокПодбораДоп.СортироватьПоЗначению();
...
Процедура НастроитьЭлементыОтраженияВУчете()
	...
	ИспользуетсяДополнительныйДокумент = (ЭтоСЧФДОПУПД Или ЭтоКСЧФДИСУКД)
		И Не ОтражениеВУчетеЭДО.ОтражатьУниверсальныеДокументыОднимОбъектомУчета();
	...
	Если ЕстьПравоОбработкиДокумента Тогда
		Элементы.ГруппаСоздатьДокументУчета.Видимость = Не ЕстьОдинДУ
			И ИнтеграцияЭДО.ДоступноОтражениеВУчете(Объект.ФорматОсновногоТитула, ЭлементСхемы.ТипДокумента, СостояниеЭДО);
...
Процедура ОткрытьСопоставлениеНоменклатуры()
	...
	НастройкиСопоставления.Организация        = Объект.Организация;
	НастройкиСопоставления.ДоговорКонтрагента = Объект.ДоговорКонтрагента;
...
&НаСервере
Процедура ПослеИзмененияДанныхСопоставленияНоменклатуры()
...
Процедура ПоказатьВыборСозданияДокументаУчета()
	...
	Если СписокСоздания.Количество() = 0 Тогда
		
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не установлены способы отражения данного вида документа.
...
&НаСервере
Процедура СоздатьДокументыУчета(Знач СпособыОбработкиДокумента, КонтекстОперации)
	...
	ТипДокумента = ЭлементСхемыИнформацияОтправителя(ЭтотОбъект).ТипДокумента;
	Если ОтражениеВУчетеЭДО.ТребуетсяОтражениеДвумяОбъектамиУчета(ТипДокумента, СпособыОбработкиДокумента) Тогда
		СпособОбработки = ОтражениеВУчетеЭДО.НовыйСпособОбработкиУПДУКД();
...
&НаКлиентеНаСервереБезКонтекста
Процедура НастроитьВидимостьРеквизитовДокументаДляРедактирования(Форма)
	
	Форма.Элементы.ГруппаРеквизитыДокументаДляРедактирования.Видимость =
		Форма.Элементы.ВключитьРедактированиеРеквизитовНеформализованныхДокументов.Видимость
		И Форма.Элементы.ВключитьРедактированиеРеквизитовНеформализованныхДокументов.Пометка;
