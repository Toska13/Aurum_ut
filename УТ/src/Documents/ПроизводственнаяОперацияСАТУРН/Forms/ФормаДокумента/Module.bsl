...
&НаКлиенте
Перем ДанныеУдаляемыхСтрок; // Для удаления сырья по товарам
	...
	Если ИнтеграцияСАТУРНКлиентСервер.ЭтоВыборПартииСАТУРН(ВыбранноеЗначение) Тогда
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			Если ТекущиеДанныеТоваров <> Неопределено Тогда
				НоваяСтрока.ИдентификаторСтрокиТоваров = ТекущиеДанныеТоваров.Идентификатор;
			КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не РедактированиеФормыНедоступно Тогда
		...
		ЕстьУказаниеИдентификаторовВТоварах = (Объект.Товары.НайтиСтроки(СтруктураОтбора).Количество() <> Объект.Товары.Количество() И Объект.Товары.Количество());
		
		ШаблонСообщенияОбОшибке = НСтр("ru = 'Укажите продукцию, которая производится с использованием сырья %1 в строке №%2.'");
		
		Если Не ЕстьУказаниеИдентификаторовВТоварах Тогда
			Возврат;
		КонецЕсли;
		
		Для Каждого СтрокаСырья Из Объект.Сырье Цикл
			
			Если Не ЗначениеЗаполнено(СтрокаСырья.ИдентификаторСтрокиТоваров) Тогда
				
				ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(ШаблонСообщенияОбОшибке, СтрокаСырья.Номенклатура, СтрокаСырья.НомерСтроки),,
					"Объект.Сырье[" + (СтрокаСырья.НомерСтроки - 1) + "].ИдентификаторСтрокиТоваров",, Отказ);
				
				Если СырьеТекущейСтроки Тогда
					ПереключитьРежимОтображенияСырьяПоТекущейСтроке();
				КонецЕсли;
...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТЧ.Идентификатор) Тогда
			ДанныеУдаляемыхСтрок.ИдентификаторыСтрок.Добавить(СтрокаТЧ.Идентификатор);
		КонецЕсли;
...
Процедура СырьеНоменклатураСоздание(Элемент, СтандартнаяОбработка)
	...
	СобытияФормСАТУРНКлиент.ПриСозданииНоменклатуры(Элемент, Элементы.Сырье.ТекущиеДанные, СтандартнаяОбработка);
...
Процедура СырьеПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока И Не Копирование Тогда
		...
		Если ТоварыТекущиеДанные <> Неопределено Тогда
			СырьеТекущиеДанные.ИдентификаторСтрокиТоваров = ТоварыТекущиеДанные.Идентификатор;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура СырьеИдентификаторСтрокиТоваровОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, ВыборДобавлением, СтандартнаяОбработка)
	
	ТекущиеДанные = Элементы.Сырье.ТекущиеДанные;
	Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		ТекущиеДанные.ПредставлениеСтрокиТовары = Элементы.СырьеИдентификаторСтрокиТоваров.СписокВыбора.НайтиПоЗначению(ВыбранноеЗначение).Представление;
	Иначе
...
&НаКлиенте
Процедура СырьеИдентификаторСтрокиТоваровОчистка(Элемент, СтандартнаяОбработка)
...
Процедура СырьеПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	УстановитьВидимостьПодсказкиНеобходимостьРаспределенияСырья(ЭтотОбъект);
...
Процедура ТоварыОткрытьПодбор(Команда)
	...
	ДополнительныеПараметры = Новый Структура("ИмяТабличнойЧасти, ЗаполнитьИдентификаторСтроки", "Товары", Истина);
...
Процедура СырьеОткрытьПодбор(Команда)
	...
	ТекущиеДанныеТоваров = Элементы.Товары.ТекущиеДанные;
	Если ТекущиеДанныеТоваров <> Неопределено Тогда
		ДополнительныеПараметры.Вставить("ИдентификаторСтроки", ТекущиеДанныеТоваров.Идентификатор);
	КонецЕсли;
...
Процедура ОтобразитьСырьеПоТекущейСтрокеТовары(Команда)
	...
	ЗаполнитьПредставлениеСырья(ЭтотОбъект);
	ЗаполнитьСписокВыбораПродукции(ЭтотОбъект);
	ПереключитьРежимОтображенияСырьяПоТекущейСтроке();
...
Процедура УстановитьУсловноеОформление()
	...
	// Представление привязки в таблице сырья к товарам
	Элемент = УсловноеОформление.Элементы.Добавить();
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.СырьеИдентификаторСтрокиТоваров.Имя);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", Новый ПолеКомпоновкиДанных("Объект.Сырье.ПредставлениеСтрокиТовары"));
	
	// Автоотметка незаполненной привязки таблицы сырья к товарам по старым документам
	Элемент = УсловноеОформление.Элементы.Добавить();
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.СырьеИдентификаторСтрокиТоваров.Имя);
	
	ГруппировкаОтборов = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ГруппировкаОтборов.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ;
	
	ОтборЭлемента = ГруппировкаОтборов.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных(Элементы.СырьеИдентификаторСтрокиТоваров.ПутьКДанным);
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	
	СписокДоступныхСтатусовДляРедактирования = Новый СписокЗначений();
	СписокДоступныхСтатусовДляРедактирования.Добавить(Перечисления.СтатусыОбработкиПроизводственнойОперацииСАТУРН.Черновик);
	СписокДоступныхСтатусовДляРедактирования.Добавить(Перечисления.СтатусыОбработкиПроизводственнойОперацииСАТУРН.Ошибка);
	
	ОтборЭлемента = ГруппировкаОтборов.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("СтатусСАТУРН");
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.НеВСписке;
	ОтборЭлемента.ПравоеЗначение = СписокДоступныхСтатусовДляРедактирования;
...
Процедура НастроитьЭлементыФормы(Форма)
	...
	Элементы.СырьеИдентификаторСтрокиТоваров.Видимость = Не Форма.СырьеТекущейСтроки;
...
&НаКлиенте
Процедура ПереключитьРежимОтображенияСырьяПоТекущейСтроке()
	
	СырьеТекущейСтроки = Не СырьеТекущейСтроки;
	
	Элементы.СырьеОтобразитьСырьеПоТекущейСтрокеТовары.Пометка = СырьеТекущейСтроки;
	Элементы.СырьеИдентификаторСтрокиТоваров.Видимость = Не СырьеТекущейСтроки;
...
Процедура ОбработкаРезультатаПодбораНоменклатуры(ВыбранноеЗначение, КэшированныеЗначения, ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.Свойство("ИдентификаторСтроки") И ЗначениеЗаполнено(ДополнительныеПараметры.ИдентификаторСтроки) Тогда
		
		Для Каждого ДобавляемаяСтрока Из ДобавленныеСтроки Цикл
			
			Если ЗначениеЗаполнено(ДобавляемаяСтрока.ИдентификаторСтрокиТоваров) Тогда
				Продолжить;
			КонецЕсли;
			
			ДобавляемаяСтрока.ИдентификаторСтрокиТоваров = ДополнительныеПараметры.ИдентификаторСтроки;
			
		КонецЦикла;
		
	КонецЕсли;
	
	Если ДополнительныеПараметры.Свойство("ЗаполнитьИдентификаторСтроки") И ДополнительныеПараметры.ЗаполнитьИдентификаторСтроки Тогда
		
		Для Каждого ДобавляемаяСтрока Из ДобавленныеСтроки Цикл
			
			Если ЗначениеЗаполнено(ДобавляемаяСтрока.Идентификатор) Тогда
				Продолжить;
			КонецЕсли;
			
			ДобавляемаяСтрока.Идентификатор = Новый УникальныйИдентификатор();
...
Процедура ЗаполнитьСписокВыбораПродукции(Форма)
	...
	Форма.Элементы.СырьеИдентификаторСтрокиТоваров.СписокВыбора.Очистить();
	Для Каждого СтрокаТовары Из Форма.Объект.Товары Цикл
		Форма.Элементы.СырьеИдентификаторСтрокиТоваров.СписокВыбора.Добавить(СтрокаТовары.Идентификатор, СтрокаТовары.ПредставлениеСтрокиТовары);
	КонецЦикла;
...
Процедура ОбновитьПредставлениеСырья(ТекущиеДанные)
	...
	Для Каждого СтрокаСырье Из Объект.Сырье Цикл
		Если СтрокаСырье.ИдентификаторСтрокиТоваров = ТекущиеДанные.Идентификатор Тогда
...
Процедура ЗаполнитьПредставлениеСырья(Форма)
	...
	Для Каждого СтрокаТЧ Из Форма.Объект.Товары Цикл
		...
		ПредставленияСоответствие.Вставить(СтрокаТЧ.Идентификатор, ПредставлениеСтроки);
		
	КонецЦикла;
	
	Для Каждого СтрокаТЧ Из Форма.Объект.Сырье Цикл
		СтрокаТЧ.ПредставлениеСтрокиТовары = ПредставленияСоответствие.Получить(СтрокаТЧ.ИдентификаторСтрокиТоваров);
	КонецЦикла;
...
Процедура ПриВыбореНоменклатуры(Номенклатура, ДополнительныеПараметры) Экспорт
	...
	Если ТекущийЭлемент = Элементы.Товары Тогда
		ИмяТЧ = "Товары";
	ИначеЕсли ТекущийЭлемент = Элементы.Сырье Тогда
		ИмяТЧ = "Сырье";
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура УстановитьОтборСырьяТекущейСтрокиТовары()
	...
	Если ТекущиеДанные <> Неопределено Тогда
		Отбор.ИдентификаторСтрокиТоваров = ТекущиеДанные.Идентификатор;
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьВидимостьПодсказкиНеобходимостьРаспределенияСырья(Форма)
	
	Объект   = Форма.Объект;
	Элементы = Форма.Элементы;
	
	Если Форма.РедактированиеФормыНедоступно Тогда
		Элементы.ГруппаПодсказкаНеобходимостьРаспределенияСырья.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	
	ЕстьНераспределенноеСырье = Ложь;
	
	Для Каждого СтрокаСырья Из Объект.Сырье Цикл
		
		Если ЗначениеЗаполнено(СтрокаСырья.ИдентификаторСтрокиТоваров) Тогда
			Продолжить;
		КонецЕсли;
		
		ЕстьНераспределенноеСырье = Истина;
		Прервать;
	...
	Элементы.ГруппаПодсказкаНеобходимостьРаспределенияСырья.Видимость = ЕстьНераспределенноеСырье И ЕстьУказаниеИдентификаторовВТоварах;
...
Процедура УдалитьСырье(УдалениеТовары)
	...
	ТаблицаСырье.Индексы.Добавить("ИдентификаторСтрокиТоваров");
	
	Для Каждого Идентификатор Из УдалениеТовары.ИдентификаторыСтрок Цикл
		
		Для Каждого СтрокаСырье Из ТаблицаСырье.НайтиСтроки(Новый Структура("ИдентификаторСтрокиТоваров", Идентификатор)) Цикл
