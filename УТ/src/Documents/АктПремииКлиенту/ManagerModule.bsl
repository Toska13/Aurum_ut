...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Если НоваяКоманда <> Неопределено Тогда
		...
		ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(
			НоваяКоманда,
			"Статус",
			Перечисления.СтатусыАктовПремийКлиентам.Получен);
	...
	НоваяКоманда = Документы.КорректировкаГрафикаВзаиморасчетов.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
	Если НоваяКоманда <> Неопределено Тогда
...
Функция ПараметрыВзаиморасчеты(ДанныеЗаполнения = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ЗначениеСсылочногоТипа(ДанныеЗаполнения) Тогда
		...
		ИменаРеквизитов = "Договор, Статус";
	...
	// Пока акт не получен объекта расчетов не возникает
	СтруктураПараметров.МожетЯвлятьсяОбъектомРасчетов = (Статус = Перечисления.СтатусыАктовПремийКлиентам.Получен);
	...
	// так как сумма в валюте документа рассчитывается по условиям
...
//
Функция ТекстЗапросаДляЗаполненияНаОснованииЗаявкиНаРасходДС() Экспорт
	
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	&Ссылка КАК ДокументОснование,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОплатыКлиенту) КАК ХозяйственнаяОперация,
	|	АктПремииКлиенту.Организация КАК Организация,
	|	АктПремииКлиенту.Подразделение КАК Подразделение,
	|	АктПремииКлиенту.Договор КАК Договор,
	|	АктПремииКлиенту.ВалютаВзаиморасчетов КАК Валюта,
	|	АктПремииКлиенту.СуммаВзаиморасчетов КАК Сумма,
	|	АктПремииКлиенту.СуммаВзаиморасчетов КАК СуммаВзаиморасчетов,
	|	АктПремииКлиенту.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
	|	АктПремииКлиенту.Контрагент КАК Контрагент,
	|	АктПремииКлиенту.Партнер КАК Партнер,
	...
	|	АктПремииКлиенту.ИдентификаторПлатежа КАК ИдентификаторПлатежа,
	|	ЛОЖЬ КАК ПеречислениеВБюджет,
	|	ИСТИНА КАК ФормаОплатыНаличная,
	|	ИСТИНА КАК ФормаОплатыБезналичная,
	|	ЕСТЬNULL(АктПремииКлиенту.Договор.БанковскийСчетКонтрагента, ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаКонтрагентов.ПустаяСсылка)) КАК БанковскийСчетКонтрагента,
	|	ЕСТЬNULL(АктПремииКлиенту.Договор.БанковскийСчетКонтрагента.ТекстНазначения, """") КАК НазначениеПлатежа,
	|	ЕСТЬNULL(АктПремииКлиенту.Договор.СтатьяДвиженияДенежныхСредств, ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка)) КАК СтатьяДвиженияДенежныхСредств,
	|	АктПремииКлиенту.ОбъектРасчетов КАК ОбъектРасчетов,
	|	АктПремииКлиенту.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчета,
	|	ВЫБОР
	|		КОГДА АктПремииКлиенту.НаправлениеДеятельности.УчетДоходов
	|			ТОГДА АктПремииКлиенту.НаправлениеДеятельности
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
	|	КОНЕЦ КАК НаправлениеДеятельности,
	|	ПРЕДСТАВЛЕНИЕ(АктПремииКлиенту.Статус) КАК ПредставлениеСтатуса,
	|	АктПремииКлиенту.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыАктовПремийКлиентам.Получен) КАК ЗапрещенВводПоСтатусу,
	|	АктПремииКлиенту.Представление КАК ПредставлениеДокумента
	|ИЗ
	|	Документ.АктПремииКлиенту КАК АктПремииКлиенту
	|ГДЕ
	|	АктПремииКлиенту.Ссылка = &Ссылка";
...
//  Строка - Текст запроса для заполнения на основании
//
Функция ТекстЗапросаДляЗаполненияНаОснованииСписанияБезналичныхДС() Экспорт
...
// Параметры:
//  Документ - ДокументОбъект.ЗаявкаНаРасходованиеДенежныхСредств, ДокументОбъект.СписаниеБезналичныхДенежныхСредств - 
//  ВыборкаПоДокументу - ВыборкаИзРезультатаЗапроса:
// * Валюта - СправочникСсылка.Валюты -
// * ХозяйственнаяОперация - ПеречислениеСсылка.ХозяйственныеОперации -
// * СтатьяДвиженияДенежныхСредств - СправочникСсылка.СтатьиДвиженияДенежныхСредств -
// * ДокументОснование - ДокументСсылка.АктПремииКлиенту -
// * Организация - СправочникСсылка.Организации -
//
Процедура ЗаполнитьРасшифровкуПлатежаПриВводеНаОсновании(Документ, ВыборкаПоДокументу) Экспорт
	
	РасшифровкаПлатежа = Документ.РасшифровкаПлатежа;
	
	ВалютаДокумента = ВыборкаПоДокументу.Валюта;
	ХозяйственнаяОперация = ВыборкаПоДокументу.ХозяйственнаяОперация;
	СтатьяДвиженияДенежныхСредств = ВыборкаПоДокументу.СтатьяДвиженияДенежныхСредств;
	ДокументОснование = ВыборкаПоДокументу.ДокументОснование;
	Организация = ВыборкаПоДокументу.Организация;
	СтавкаБезНДС = ПредопределенноеЗначение("Справочник.СтавкиНДС.БезНДС");
	
	ВыборкаПоАктам = ВыборкаДанныхЗаполненияРасшифровкиПлатежаПриВводеНаОсновании(ДокументОснование, Организация);
	Пока ВыборкаПоАктам.Следующий() Цикл
		
		СуммаПоГрафикуВсего = 0;
		УчтеноСуммаОплаты = 0;
	
		Выборка = ВыборкаПоАктам.Выбрать();
	...
	Если РасшифровкаПлатежа.Количество() = 0 Тогда
		...
		ЗаполнитьЗначенияСвойств(СтрокаРасшифровки, ВыборкаПоДокументу);
	...
	Если ТипЗнч(Документ) = Тип("ДокументОбъект.СписаниеБезналичныхДенежныхСредств") Тогда
		...
		Для Каждого СтрокаТЧ Из Документ.РасшифровкаПлатежа Цикл
			...
			СтрокаТЧ.ЗаявкаНаРасходованиеДенежныхСредств = ПустаяЗаявка;
...
Функция ТекстЗапросаТаблицаДвиженияКонтрагентДоходыРасходы(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект = Неопределено)
	...
	|	Шапка.НаправлениеДеятельности КАК НаправлениеДеятельностиКонтрагента,
	|	Шапка.Партнер КАК Партнер,
	|	Шапка.Контрагент КАК Контрагент,
	|	Шапка.Договор КАК Договор,
	|	Шапка.ОбъектРасчетов КАК ОбъектРасчетов,
	|	Шапка.НаправлениеДеятельности КАК НаправлениеДеятельностиСтатьи,
...
Функция ТекстЗапросаТаблицаРасчетыСКлиентами(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект = Неопределено)
	...
	"ВЫБРАТЬ
	|	Шапка.ДатаПризнанияВУчете КАК Период,
	|	Шапка.ДатаПризнанияВУчете КАК ДатаРегистратора,
	...
	|	Шапка.ИдентификаторДокумента КАК ИдентификаторФинЗаписи,
	|	ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ОтражениеПремииКлиентаРетроБонус) КАК НастройкаХозяйственнойОперации,
	|	ВЫБОР КОГДА Шапка.ВалютаВзаиморасчетов = &ВалютаРегламентированногоУчета
	|			ТОГДА 0
	|		ИНАЧЕ ЕСТЬNULL(КурсВалюты.КурсЧислительВалютыВзаиморасчетов, 1)
	|	КОНЕЦ КАК КурсЧислительВалютыВзаиморасчетов,
	|	ВЫБОР КОГДА Шапка.ВалютаВзаиморасчетов = &ВалютаРегламентированногоУчета
	|			ТОГДА 0
	|		ИНАЧЕ ЕСТЬNULL(КурсВалюты.КурсЗнаменательВалютыВзаиморасчетов, 1)
	|	КОНЕЦ КАК КурсЗнаменательВалютыВзаиморасчетов,
	|	ВЫБОР КОГДА Шапка.ВалютаВзаиморасчетов = &ВалютаУправленческогоУчета
	|			ТОГДА 0
	|		ИНАЧЕ ЕСТЬNULL(КурсВалюты.КурсЧислительВалютыУправленческогоУчета, 1)
	|	КОНЕЦ КАК КурсЧислительВалютыУпр,
	|	ВЫБОР КОГДА Шапка.ВалютаВзаиморасчетов = &ВалютаУправленческогоУчета
	|			ТОГДА 0
	|		ИНАЧЕ ЕСТЬNULL(КурсВалюты.КурсЗнаменательВалютыУправленческогоУчета, 1)
...
Процедура ТекстЗапросаДанныеДокументаШапка(СписокЗапросов)
	...
	|	ЕСТЬNULL(ДанныеДокумента.НаправлениеДеятельности.УчетДоходов, ЛОЖЬ) КАК УчетДоходов,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеДокумента.НаправлениеДеятельности.УчетДоходов, ЛОЖЬ)
	|			ТОГДА ДанныеДокумента.НаправлениеДеятельности
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
	|	КОНЕЦ КАК НаправлениеДеятельности,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеДокумента.НаправлениеДеятельности.УчетДоходов, ЛОЖЬ)
	|				И НЕ ЕСТЬNULL(ДанныеДокумента.ОбъектРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности, ИСТИНА)
	|			ТОГДА ДанныеДокумента.НаправлениеДеятельности
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
...
// Выборка данных для заполнения расшифровки платежа в документах, создаваемых на основании
// 
// Параметры:
//  АктПремии - ДокументСсылка.АктПремииКлиенту - 
//  Организация - СправочникСсылка.Организации - 
//  
// Возвращаемое значение:
//  ВыборкаИзРезультатаЗапроса - Реквизиты документа:
// * ОснованиеПлатежа - ДокументСсылка.АктПремииКлиенту -
...
// * СуммаПоГрафику - Число
// * СуммаОплаты - Число -
//
Функция ВыборкаДанныхЗаполненияРасшифровкиПлатежаПриВводеНаОсновании(АктПремии, Организация)
	...
	|ГДЕ
	|	ОбъектыРасчетов.Объект = &АктПремии
	...
	|						ВтОбъектыРасчетов.ОбъектРасчетов
	|					ИЗ
	|						ВтОбъектыРасчетов)) КАК РасчетыСКлиентамиОбороты
	...
	|	РасчетыСКлиентами.ПредоплатаОстаток,
	|	ЕСТЬNULL(-РасчетыСКлиентамиОбороты.ОплачиваетсяОборот, 0)
	|ИЗ
	...
	Запрос.УстановитьПараметр("АктПремии", АктПремии);
