...
Процедура ПроверитьАктуальностьОперации()
	...
	ВидыОпераций = Документы.ФормированиеПартийЗЕРНО.ДоступныеВидыОпераций().Получить(РазделУчета);
	
	Если ВидыОпераций <> Неопределено
		И ВидыОпераций.Найти(Операция) = Неопределено Тогда
		Операция = ВидыОпераций[0];
	КонецЕсли;
