...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ОбновлениеИнформационнойБазыГосИС.ДобавитьОбработчикОбновленияПереходаНаКонструкторНастроек(
		...
		Новый УникальныйИдентификатор("1aea3786-ae0b-47cf-b4f0-642fa586adff"));
...
Функция ДоступныеВидыОпераций() Экспорт
	...
	ВидыОперацийПоВидуПродукцииИС.Вставить(Перечисления.РазделыУчетаЗЕРНО.Зерно, ВидыОперацийЗерно);
	ВидыОперацийПоВидуПродукцииИС.Вставить(Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна, ВидыОперацийППЗ);
	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийЗЕРНО.АннулированиеФормированияПартии Тогда
		...
		Попытка
			...
			Партия = Справочники.РеестрПартийЗЕРНО.ПартияЗерна(
				НомерПартии, ЭлементОчереди.Организация, ЭлементОчереди.Подразделение, ЭлементОчереди.РазделУчета, ПараметрыОбмена);
...
Функция ФормированиеПартииЗЕРНО(ДокументСсылка, ДополнительныеПараметры)
	...
	ВидыОпераций = ДоступныеВидыОпераций().Получить(Шапка.РазделУчета);
	
	Если ВидыОпераций = Неопределено
		Или ВидыОпераций.Найти(Шапка.Операция) = Неопределено Тогда
	...
	ЭтоСХТП         = Шапка.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.Зерно И Не Шапка.ПоставитьПартиюНаХранение;
	ЭтоЭлеватор     = Шапка.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.Зерно И Шапка.ПоставитьПартиюНаХранение;
	ЭтоПереработчик = Шапка.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна;
