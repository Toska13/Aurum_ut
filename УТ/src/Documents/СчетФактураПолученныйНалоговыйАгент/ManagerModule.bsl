...
Функция ТекстЗапросаТаблицаПрочиеАктивыПассивы(Запрос, ТекстыЗапроса, Регистры)
	...
	ИнициализироватьВтТаблицаСуммДокументов(Запрос, ТекстыЗапроса);
	Запрос.УстановитьПараметр("НДС_ОбратноеОбложение", Справочники.ВидыНалоговВзносов.ПолучитьСсылкуПредопределенногоЭлемента("НДС_ОбратноеОбложение"));
	
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ТаблицаСуммДокументов.ДатаПереходаПраваСобственности > &Период
	|			ТОГДА ТаблицаСуммДокументов.ДатаПереходаПраваСобственности
	|		ИНАЧЕ &Период
	|	КОНЕЦ                                                          КАК Период,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)                         КАК ВидДвижения,
	|	&Организация                                                   КАК Организация,
	|	ТаблицаСуммДокументов.Подразделение                            КАК Подразделение,
	|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)      КАК НаправлениеДеятельности,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.Налоги)  КАК Статья,
	|	&НДС_ОбратноеОбложение                                         КАК Аналитика,
	|
	|	СУММА(ТаблицаСуммДокументов.НДСУпр)                            КАК Сумма
	|
	|ИЗ
	|	ВтТаблицаСуммДокументов КАК ТаблицаСуммДокументов
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаСуммДокументов.ДатаПереходаПраваСобственности,
	|	ТаблицаСуммДокументов.Подразделение";
...
Функция ТекстЗапросаТаблицаНДСПредъявленный(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР
	|		КОГДА ЗаписиКнигиПродаж.ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ВосстановлениеПоКСФ)
	|			ТОГДА ЗаписиКнигиПродаж.ВидДеятельностиНДС
	...
	|	ВЫБОР 
	|		КОГДА ЗаписиКнигиПродаж.ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ВосстановлениеПоКСФ)
	|			ТОГДА ЗаписиКнигиПродаж.СуммаБезНДС
	|		ИНАЧЕ 0
...
Процедура ИнициализироватьВтПартииПрочихРасходов(Запрос, ТекстыЗапроса)
	...
	|	И НЕ ДокументыПоступления.ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ВосстановлениеПоКСФ)
	|";
...
Процедура ИнициализироватьКлючиАналитикиУчетаПартийДокумента(Запрос)
	...
	|	ТаблицаДокументыПоступления.ВидЦенности КАК ВидЦенности,
	|	0 КАК КодСтроки
