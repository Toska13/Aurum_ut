...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Иначе
		...
		ПроверитьДоступностьДокументовПродажи(Отказ);
...
Процедура ПроверитьУчастниковПоУсловиюРетроБонусовКонтрагентуКлиенту(РеквизитыУсловий, Отказ)
	...
	ИначеЕсли СоставУчастников = Перечисления.СоставыУчастниковРетроБонусов.Клиенты
	 ИЛИ СоставУчастников = Перечисления.СоставыУчастниковРетроБонусов.СегментыПартнеров Тогда
	...
	Если ОшибкиЗаполнения.ОшибкаЗаполненияШапки Тогда
		...
		Если ДетализацияРасчетаУчастников = Детализация.ПоКонтрагентуКлиенту Тогда
			...
			Если СоставУчастников = Перечисления.СоставыУчастниковРетроБонусов.Соглашения
			 ИЛИ СоставУчастников = Перечисления.СоставыУчастниковРетроБонусов.СегментыПартнеров Тогда
...
Процедура ПроверитьФиксациюСоставаСегментовУРБ(Отказ, УстановитьБлокировку, Знач РеквизитыУсловий = Неопределено)
	...
	Если УстановитьБлокировку Тогда
		...
		Исключение
			...
			ШаблонОшибки = НСтр("ru = 'Не удалось выполнить проверку фиксации состава документа %1 по причине ""%2"".
								|Попробуйте позже'");
			ОписаниеОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ПроверитьДоступностьДокументовПродажи(Отказ)
	
	Если Начисления.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ТекстОшибки = НСтр("ru = 'Документ продажи недоступен'");
	ПредставлениеПоля = НСтр("ru = 'Документ продажи'");
	...
	Для НомерСтроки = 1 По Начисления.Количество() Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаНачисления.ДокументПродажи) Тогда
			Продолжить;
		КонецЕсли;
		
		ЧтениеРазрешено = УправлениеДоступом.ЧтениеРазрешено(СтрокаНачисления.ДокументПродажи);
		Если НЕ ЧтениеРазрешено Тогда
			...
			Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Начисления", НомерСтроки, "ДокументПродажи");
