...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ВыборДокументовПриобретенияДляОтчетаКомитенту" И Источник.ВладелецФормы = ЭтотОбъект Тогда
...
Процедура СоглашениеПриИзмененииСервер()
	...
	ДенежныеСредстваСервер.ПроверитьЗаполнитьБанковскийСчетОрганизацииПоВладельцу(Объект.Организация,
		Объект.БанковскийСчет, Объект.ФормаОплаты, Объект.НаправлениеДеятельности, Объект.ОплатаВВалюте);
...
Процедура ПредставлениеПриобретенияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ВыбратьДокумент" Тогда
		...
		Если ОтчетПоЗаказу Тогда
			ПараметрыОтбора.Вставить("Заказы", СписокЗаказов);
		КонецЕсли;
		ОткрытьФорму(ИмяФормыВыбора, ПараметрыОтбора, ЭтотОбъект);
...
Процедура ТоварыДокументПриобретенияПриИзменении(Элемент)
	...
	Если ТекущаяСтрока <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.ДокументПриобретения) Тогда
			
			ДанныеПоТовару = Новый Структура();
			ДанныеПоТовару.Вставить("Номенклатура");
			ДанныеПоТовару.Вставить("Характеристика");
			ДанныеПоТовару.Вставить("ДокументПриобретения");
			ДанныеПоТовару.Вставить("Назначение");
			ДанныеПоТовару.Вставить("НомерГТД");
			
			ЗаполнитьЗначенияСвойств(ДанныеПоТовару, ТекущаяСтрока);
			
			СтавкаНДС = ПолучитьСтавкуПоПриобретению(ДанныеПоТовару);
		КонецЕсли;
		
		Если СтавкаНДС <> Неопределено И ТекущаяСтрока.СтавкаНДС <> СтавкаНДС Тогда
...
Процедура НалогообложениеНДСПриИзмененииСервер()
	...
	Если Объект.СпособРасчетаВознаграждения <> Перечисления.СпособыРасчетаКомиссионногоВознаграждения.НеРассчитывается Тогда	
	
		ТекущаяСтрока = Новый Структура("СтавкаНДС", Объект.СтавкаНДСВознаграждения);
...
&НаСервере
Функция ПолучитьСтавкуПоПриобретению(ДанныеПоТовару)
	Возврат Документы.ОтчетКомитентуОЗакупках.ПолучитьСтавкуНДСПоПриобретению(ДанныеПоТовару, Объект.Организация);
КонецФункции
