...
Функция ТекстЗапросаТоварыУслугиКОформлениюОтчетовКомитентуОЗакупкахОстаткиПоНазначениям(ИмяВременнойТаблицы = Неопределено) Экспорт
	...
	|	ТаблицаОстатков.ДокументПриобретения КАК ДокументПриобретения,
	|	ТаблицаОстатков.ДокументПриобретения.Контрагент КАК Поставщик,
	|	ТаблицаОстатков.НомерГТД КАК НомерГТД,
	|	ТаблицаОстатков.Валюта КАК Валюта,
	|	ТаблицаОстатков.КоличествоОстаток КАК Количество,
	|	ТаблицаОстатков.КоличествоПоРНПТОстаток КАК КоличествоПоРНПТ,
	|	ТаблицаОстатков.КоличествоОстаток / ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки, 1) КАК КоличествоУпаковок,
	|	ВЫБОР
	|		КОГДА ПриобретениеТоваровУслуг.ЦенаВключаетНДС
	|			ТОГДА ВЫРАЗИТЬ(ТаблицаОстатков.СуммаСНДСОстаток / ВЫБОР
	|						КОГДА ТаблицаОстатков.КоличествоОстаток = 0
	|							ТОГДА 1
	|						ИНАЧЕ ТаблицаОстатков.КоличествоОстаток
	|					КОНЕЦ КАК ЧИСЛО(31, 2))
	|		ИНАЧЕ ВЫРАЗИТЬ((ТаблицаОстатков.СуммаСНДСОстаток - ТаблицаОстатков.СуммаНДСОстаток) / ВЫБОР
	|					КОГДА ТаблицаОстатков.КоличествоОстаток = 0
	|						ТОГДА 1
	|					ИНАЧЕ ТаблицаОстатков.КоличествоОстаток
	|				КОНЕЦ КАК ЧИСЛО(31, 2))
	|	КОНЕЦ КАК Цена,
	|	ТаблицаОстатков.СуммаСНДСОстаток КАК СуммаСНДС,
	|	ТаблицаОстатков.СуммаНДСОстаток КАК СуммаНДС,
	|	ВЫБОР
	|		КОГДА ПриобретениеТоваровУслуг.ЦенаВключаетНДС
	|			ТОГДА ТаблицаОстатков.СуммаСНДСОстаток
	|		ИНАЧЕ ВЫРАЗИТЬ(ТаблицаОстатков.СуммаСНДСОстаток - ТаблицаОстатков.СуммаНДСОстаток КАК ЧИСЛО(31, 2))
	|	КОНЕЦ КАК Сумма,
	|	Назначения.Заказ КАК ЗаказКлиента,
	|	
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(Назначения.УчитыватьСтавкиНДС, ЛОЖЬ)
	|			ТОГДА ТаблицаОстатков.СтавкаНДС
	|		ИНАЧЕ ЕСТЬNULL(СтавкиНДСНоменклатуры.СтавкаНДС, ЕСТЬNULL(ОсновныеСтавкиНДС.СтавкаНДС,
	...
	|				И Валюта = &Валюта
	|				И Организация = &Организация) КАК ТаблицаОстатков
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриобретениеТоваровУслуг КАК ПриобретениеТоваровУслуг
	|		ПО (ПриобретениеТоваровУслуг.Ссылка = ТаблицаОстатков.ДокументПриобретения)
	...
	|		ПО ТаблицаОстатков.Номенклатура = СтавкиНДСНоменклатуры.Номенклатура
	...
	|	ПО
	|		ТаблицаОстатков.Назначение = Назначения.Ссылка
	|";
...
Функция ПроверитьНаличиеПриобретенияИИсправительнойКорректировкиВОдномОтчете(Объект) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |ГДЕ
		               |	ТоварыОтчета.СчетФактураПолученный <> ЗНАЧЕНИЕ(Документ.СчетФактураПолученный.ПустаяСсылка)
		               |;
		               |
		               |////////////////////////////////////////////////////////////////////////////////
		               |ВЫБРАТЬ
...
// Возвращает ставку НДС по указанному документу приобретения.
//
// Параметры:
//	ДанныеСтроки - структура:
//  * Номенклатура - СправочникСсылка.Номенклатура - номенклатура;
//  * Характеристика - СправочникСсылка.ХарактеристикиНоменклатуры - характеристика номенклатуры; 
//  * ДокументПриобретения - ДокументСсылка.ПриобретениеТоваровУслуг, ДокументСсылка.ВозвратТоваровПоставщику, ДокументСсылка.КорректировкаПриобретения - документ приобретения;
...
//	
// Возвращаемое значение:
//	Неопределено, СправочникСсылка.СтавкиНДС - ставка по документу приобретения.
//
Функция ПолучитьСтавкуНДСПоПриобретению(ДанныеСтроки, Организация) Экспорт
	
	ТекстЗапроса = "ВЫБРАТЬ ПЕРВЫЕ 1
		|	ТоварыУслугиКОформлениюОтчетовКомитентуОЗакупкахОбороты.СтавкаНДС КАК СтавкаНДС
		|ИЗ
		...
		|				И ДокументПриобретения = &ДокументПриобретения
		|				И Назначение = &Назначение
		|				И НомерГТД = &НомерГТД) КАК ТоварыУслугиКОформлениюОтчетовКомитентуОЗакупкахОбороты";
	
	Запрос = Новый Запрос();
	Запрос.Параметры.Вставить("Организация", Организация);
	Для Каждого ДанныеПараметра Из ДанныеСтроки Цикл
		Запрос.Параметры.Вставить(ДанныеПараметра.Ключ, ДанныеПараметра.Значение);
	КонецЦикла;
	Запрос.Текст = ТекстЗапроса;
	Результат = Запрос.Выполнить().Выбрать();
	
	Если Результат.Следующий() Тогда
		Возврат Результат.СтавкаНДС;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры)
	...
	ВзаиморасчетыПроведениеКлиенты.ПроведениеОплатыОтКлиента(Запрос, ТекстыЗапроса, Регистры, ТекстОплата,"2");
...
Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ОтчетКомитенту) КАК НастройкаХозяйственнойОперации,
	|	ДанныеДокументаТовары.СтавкаНДС           КАК СтавкаНДС
	|
	|ИЗ
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если НЕ ПраваПользователяПовтИсп.ЭтоПартнер() Тогда
		...
		КомандаПечати.НаименованиеПечатнойФормы = НСтр("ru = 'Отчет комитенту (принципалу) о закупках'");
	КонецЕсли;
...
Функция ПолучитьДанныеДляПечатнойФормыОтчетПоЗакупкам(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ТоварыДокумента.Ссылка.Дата                     КАК ДатаСчетаФактуры,
	|	ТоварыДокумента.Ссылка.ПредставлениеНомера      КАК НомерСчетаФактуры
	...
	|	И ОснованияДокументов.ДокументОснование В(&МассивДокументов)
	|	И ТоварыДокумента.Ссылка.Перевыставленный
	|;
	//-- Локализация
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|		КОНЕЦ										КАК НомерГТД,
	//++ Локализация
	|		ТоварыСчетаФактуры.ДатаСчетаФактуры			КАК СчетФактураВыданныйДата,
	|		ТоварыСчетаФактуры.НомерСчетаФактуры		КАК СчетФактураВыданныйПредставлениеНомера,
	//-- Локализация
...
// 
// Параметры:
//  СсылкаОбъект - ДокументСсылка.ОтчетКомитентуОЗакупках - Документ, по которому нужно получить данные.
...
Функция СуммыПоЗаказам(СсылкаОбъект, ЗначенияРеквизитов = Неопределено) Экспорт
	...
	Если ЗначенияРеквизитов = Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТаблицаЗначений", "Документ.ОтчетКомитентуОЗакупках.Товары");
