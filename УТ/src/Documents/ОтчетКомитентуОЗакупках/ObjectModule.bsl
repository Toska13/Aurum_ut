...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетДоходовПоНаправлениямДеятельности")
			И Не ЗначениеЗаполнено(НаправлениеДеятельности) Тогда
			НаправленияДеятельностиСервер.ЗаполнитьНаправлениеПоУмолчанию(НаправлениеДеятельности, Соглашение, Договор);
		КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ШаблонОшибкаЗаполненияНаправленияДеятельности = НСтр("ru = 'Направление деятельности в назначении в строке %1 списка ""%2"" отличается от направления деятельности в отчете.'");
	...
	Для каждого СтрокаТаб Из Товары Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаб.Назначение) Тогда
			...
			Иначе
				...
				НаправлениеДеятельностиНазначения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТаб.Назначение, "НаправлениеДеятельности");
			
				Если НаправлениеДеятельности <>  НаправлениеДеятельностиНазначения Тогда
					
					ТекстСообщения = СтрШаблон(ШаблонОшибкаЗаполненияНаправленияДеятельности, Строка(СтрокаТаб.НомерСтроки), ПредставлениеТЧ);
					Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", СтрокаТаб.НомерСтроки, "Назначение");
...
Процедура ПроверкиСоответствияРеквизитовВДокументахПриобретения(Отказ)
	...
	Для Каждого СтрокаТовары Из Товары Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТовары.ДокументПриобретения) Тогда
			...
			РеквизитыДокументаПриобретения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаТовары.ДокументПриобретения, "ЦенаВключаетНДС, Организация, Валюта");
