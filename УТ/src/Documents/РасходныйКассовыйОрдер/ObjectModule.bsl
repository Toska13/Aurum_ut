...
Процедура ПриКопировании(ОбъектКопирования)
	...
	ОплатаПоЗаявкам = (Не РазрешеныПлатежиБезУказанияЗаявок И ЗначениеЗаполнено(Касса))
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	НеПроверятьЗаполнениеЗаявки = ДенежныеСредстваСервер.РазрешеныПлатежиБезУказанияЗаявок(ЭтотОбъект)
		И Не ОплатаПоЗаявкам;
	
	Если НеПроверятьЗаполнениеЗаявки Тогда
	...
	Если Не (ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыРаботнику
		...
		ИЛИ НеПроверятьЗаполнениеЗаявки Тогда
	...
	Если Документы.РасходныйКассовыйОрдер.МассивОперацийБезРасшифровкиПлатежа().Найти(ХозяйственнаяОперация) <> Неопределено
		И НеПроверятьЗаполнениеЗаявки Тогда
...
Процедура ЗаполнитьПоЗаявкеНаОбеспечение(Знач ДокументОснование, ДанныеЗаполнения)
	...
	Выборка = ДанныеЗаявки.РезультатЗапросаШапки.Выбрать();
	Выборка.Следующий();
	
	ДанныеЗаполнения = Новый Структура;
	Для Каждого Колонка Из Выборка.Владелец().Колонки Цикл
		ДанныеЗаполнения.Вставить(Колонка.Имя, Выборка[Колонка.Имя]);
	КонецЦикла;
