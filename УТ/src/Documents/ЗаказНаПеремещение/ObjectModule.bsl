...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПеремещениеЗаблокированныхТоваров Тогда
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено, ЗаполнятьВариантОбеспечения = Истина)
	...
	Если ЗаполнятьВариантОбеспечения Тогда
		...
		ПараметрыДокумента = Документы.ЗаказНаПеремещение.ПараметрыДокументаДляДействийОбеспечения(ЭтотОбъект);
...
Процедура ЗаполнитьПоПриемкеТоваровНаХранение(ДанныеЗаполнения)
	...
	|	НЕ Шапка.Проведен             КАК ЕстьОшибкиПроведен
	|ИЗ
	|	Документ.ПриемкаТоваровНаХранение КАК Шапка
	|ГДЕ
	|	Шапка.Ссылка = &ДокументОснование
	|;
	|
