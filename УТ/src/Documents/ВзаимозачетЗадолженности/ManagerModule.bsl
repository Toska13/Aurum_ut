...
Функция ТекстЗапросаТаблицаАктивацияПодарочныхСертификатов(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса = 
		"ВЫБРАТЬ
		|	ТабличнаяЧасть.ОбъектРасчетов.Объект КАК ПодарочныйСертификат,
		...
		|ИЗ
		|	Документ.ВзаимозачетЗадолженности.ДебиторскаяЗадолженность КАК ТабличнаяЧасть
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыПодарочныхСертификатов КАК ВидыПодарочныхСертификатов
		|		ПО (ВидыПодарочныхСертификатов.Ссылка = ТабличнаяЧасть.ОбъектРасчетов.Объект.Владелец)
		|ГДЕ
		|	ТабличнаяЧасть.Ссылка = &Ссылка
		|	И &ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВзаимозачетаЗадолженности.ПереносАвансаКлиентаМеждуКонтрагентами)
		|	И ТабличнаяЧасть.ОбъектРасчетов.Объект ССЫЛКА Справочник.ПодарочныеСертификаты";
...
Функция ТекстЗапросаВтДебиторскаяЗадолженность()
	...
	|		ИНАЧЕ ОбъектыРасчетов.НаправлениеДеятельности
...
Функция ТекстЗапросаВтРасчетыСКлиентами()
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(Направления.УчетДоходов,ЛОЖЬ) И НЕ ЕСТЬNULL(ОбъектыРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности,ИСТИНА)
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(КорНаправления.УчетДоходов,ЛОЖЬ) И НЕ ЕСТЬNULL(КорОбъектыРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности,ИСТИНА)
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(НаправленияПриемник.УчетДоходов,ЛОЖЬ) И НЕ ЕСТЬNULL(ОбъектыРасчетовПриемник.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности,ИСТИНА)
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(НаправленияЗеркальный.УчетДоходов,ЛОЖЬ) И НЕ ЕСТЬNULL(ОбъектыРасчетовЗеркальный.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности,ИСТИНА)
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(НаправленияПриемникЗеркальный.УчетДоходов,ЛОЖЬ) И НЕ ЕСТЬNULL(ОбъектыРасчетовПриемникЗеркальный.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности,ИСТИНА)
...
Функция ТекстЗапросаВтРасчетыСПоставщиками()
	...
	|	ЗадолженностьПредварительная.Договор                 КАК Договор,
	|	ВЫБОР
	|		КОГДА Направления.УчетРасчетовСПоставщиками И НЕ ЕСТЬNULL(ОбъектыРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности,ИСТИНА)
	|			ТОГДА ЗадолженностьПредварительная.НаправлениеДеятельности
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) 
	|	КОНЕЦ                                                КАК НаправлениеДеятельности,
	|	ЗадолженностьПредварительная.ВалютаВзаиморасчетов    КАК ВалютаВзаиморасчетов,
	...
	|	КорОбъектыРасчетов.Контрагент                        КАК КорКонтрагент,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(КорНаправления.УчетРасчетовСПоставщиками,ЛОЖЬ) И НЕ ЕСТЬNULL(КорОбъектыРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности,ИСТИНА)
	|			ТОГДА КорОбъектыРасчетов.НаправлениеДеятельности
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) 
	|	КОНЕЦ                                                КАК КорНаправлениеДеятельности,
	|	ЗадолженностьПредварительная.ОбъектРасчетовПриемник  КАК ОбъектРасчетовПриемник,
	...
	|	ОбъектыРасчетовПриемник.Контрагент                   КАК КонтрагентПриемник,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(НаправленияПриемник.УчетРасчетовСПоставщиками,ЛОЖЬ) И НЕ ЕСТЬNULL(ОбъектыРасчетовПриемник.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности,ИСТИНА)
	|			ТОГДА ОбъектыРасчетовПриемник.НаправлениеДеятельности
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) 
	|	КОНЕЦ                                                КАК НаправлениеДеятельностиПриемник,
	|	ЗадолженностьПредварительная.ПоДаннымОбъектаРасчетовИсточника КАК ПоДаннымОбъектаРасчетовИсточника,
	...
	|	ОбъектыРасчетовЗеркальный.Договор                                         КАК Договор,
	|	ВЫБОР
	|		КОГДА НаправленияЗеркальный.УчетРасчетовСПоставщиками И НЕ ЕСТЬNULL(ОбъектыРасчетовЗеркальный.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности,ИСТИНА)
	|			ТОГДА ОбъектыРасчетовЗеркальный.НаправлениеДеятельности
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) 
	|	КОНЕЦ                                                                     КАК НаправлениеДеятельности,
	|	ОбъектыРасчетовЗеркальный.ВалютаВзаиморасчетов                            КАК ВалютаВзаиморасчетов,
	...
	|	КорОбъектыРасчетов.Контрагент                                             КАК КорКонтрагент,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(НаправленияПриемникЗеркальный.УчетДоходов,ЛОЖЬ) И НЕ ЕСТЬNULL(КорОбъектыРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности,ИСТИНА)
	|			ТОГДА КорОбъектыРасчетов.НаправлениеДеятельности
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) 
	|	КОНЕЦ                                                                     КАК КорНаправлениеДеятельности,
...
Функция ТекстЗапросаРасчетыСКлиентами()
	...
	|			ТОГДА 3
	|		ИНАЧЕ 1
	|	КОНЕЦ                                                                 КАК ВидОперацииДляСортировки,
	...
	|	
	|	ВЫБОР
	|		КОГДА ТабличнаяЧасть.СуммаВзаиморасчетов = 0
	|			ИЛИ ТабличнаяЧасть.ВалютаВзаиморасчетов = &ВалютаРегламентированногоУчета
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(ТабличнаяЧасть.СуммаРегл / ТабличнаяЧасть.СуммаВзаиморасчетов КАК ЧИСЛО(10, 4))
	...
	|	ВЫБОР
	|		КОГДА ТабличнаяЧасть.СуммаУпр = 0
	|			ИЛИ ТабличнаяЧасть.ВалютаВзаиморасчетов = &ВалютаУправленческогоУчета
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(ТабличнаяЧасть.СуммаРегл / ТабличнаяЧасть.СуммаУпр КАК ЧИСЛО(10, 4))
...
Процедура РаспределитьТабличныеЧасти(Запрос)
	...
	|	ДанныеДокумента.КонтрагентДебитор КАК Контрагент,
	|	ВЫБОР
	|		КОГДА НЕ &СуммыРеглУпрУказаныВДокументе
	|				И (КурсыВалют.ВариантКурсаДоговора <> ЗНАЧЕНИЕ(Перечисление.ВариантыКурсаДоговора.Переменный)
	...
	Для Каждого Отправитель Из ОтправительЗадолженности Цикл
		...
		Для Каждого Получатель Из ПолучательЗадолженности Цикл
			...
			Если Отправитель.ВалютаВзаиморасчетов = Получатель.ВалютаВзаиморасчетов 
					И Отправитель.СуммаВзаиморасчетов <= Получатель.СуммаВзаиморасчетов
				ИЛИ Отправитель.СуммаРегл < Получатель.СуммаРегл Тогда
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	ВзаимозачетЗадолженностиЛокализация.Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
КонецПроцедуры
...
Функция ПолучитьДанныеВзаимозачетЗадолженности(МассивДокументов)
	...
	|					ИЛИ (ДанныеДокумента.ВидОперации В (&ВидыОперацийМеждуОрганизациями) И ДанныеДокумента.Организация = ДанныеДокумента.ОрганизацияКредитор))
	|				И НЕ ДанныеДокумента.ВидОперации В (&ВидыОперацийПоКомиссии)
	|				ИЛИ ДанныеДокумента.ВидОперации = &ПроизвольныйВзаимозачет ТОГДА
	|		ИСТИНА
	|	ИНАЧЕ
	|		ЛОЖЬ
	...
	ОперацииКомиссионера = Новый Массив;
	ОперацииКомиссионера.Добавить(Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ОтражениеОплатыЧерезКомиссионера);
	ОперацииКомиссионера.Добавить(Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ОтражениеВозвратаОплатыЧерезКомиссионера);
	
	Запрос.УстановитьПараметр("МассивДокументов", МассивДокументов);
	Запрос.УстановитьПараметр("ВидыОперацийМеждуОрганизациями", МассивОпераций);
	Запрос.УстановитьПараметр("ВидыОперацийПоКомиссии", ОперацииКомиссионера);
	Запрос.УстановитьПараметр("ПроизвольныйВзаимозачет", Перечисления.ВидыОперацийВзаимозачетаЗадолженности.Произвольный);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("d307c19a-eb0b-4bc4-830d-1e2280e401b6");
	...
	СписокОписаний.Добавить(НСтр("ru = 'Обновление документов ""Взаимозачет задолженности"":'"));
	СписокОписаний.Добавить(НСтр("ru = '- заполнение реквизита ""Направление деятельности"" для операций ""Произвольный взаимозачет"".';"));
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТЧДебиторскаяЗадолженность.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ВзаимозачетЗадолженности.ДебиторскаяЗадолженность КАК ТЧДебиторскаяЗадолженность
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВзаимозачетЗадолженности КАК РеквизитыШапки
	|		ПО ТЧДебиторскаяЗадолженность.Ссылка = РеквизитыШапки.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ОбъектыРасчетов КАК ОбъектыРасчетов
	|		ПО ТЧДебиторскаяЗадолженность.ОбъектРасчетов = ОбъектыРасчетов.Ссылка
	|ГДЕ
	|	РеквизитыШапки.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВзаимозачетаЗадолженности.Произвольный)
	|	И ТЧДебиторскаяЗадолженность.НаправлениеДеятельности <> ОбъектыРасчетов.НаправлениеДеятельности
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТЧКредиторскаяЗадолженность.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ВзаимозачетЗадолженности.КредиторскаяЗадолженность КАК ТЧКредиторскаяЗадолженность
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВзаимозачетЗадолженности КАК РеквизитыШапки
	|		ПО ТЧКредиторскаяЗадолженность.Ссылка = РеквизитыШапки.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ОбъектыРасчетов КАК ОбъектыРасчетов
	|		ПО ТЧКредиторскаяЗадолженность.ОбъектРасчетов = ОбъектыРасчетов.Ссылка
	|ГДЕ
	|	РеквизитыШапки.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВзаимозачетаЗадолженности.Произвольный)
	|	И ТЧКредиторскаяЗадолженность.НаправлениеДеятельности <> ОбъектыРасчетов.НаправлениеДеятельности
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Для Каждого СтрокаДанных Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			ДокументОбъект = СтрокаДанных.Ссылка.ПолучитьОбъект();
			...
			// Заполнение направления деятельности для операции Произвольный взаимозачет
			Если ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.Произвольный Тогда
				МассивОбъектовРасчетов = ДокументОбъект.ДебиторскаяЗадолженность.ВыгрузитьКолонку("ОбъектРасчетов");
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивОбъектовРасчетов, ДокументОбъект.КредиторскаяЗадолженность.ВыгрузитьКолонку("ОбъектРасчетов"));
				СоответствиеНаправлений = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(МассивОбъектовРасчетов, "НаправлениеДеятельности");
				
				Для Каждого Стр Из ДокументОбъект.ДебиторскаяЗадолженность Цикл
					Если ЗначениеЗаполнено(Стр.ОбъектРасчетов) И Стр.НаправлениеДеятельности <> СоответствиеНаправлений[Стр.ОбъектРасчетов] Тогда
						Стр.НаправлениеДеятельности = СоответствиеНаправлений[Стр.ОбъектРасчетов];
					КонецЕсли;
				КонецЦикла;
				
				Для Каждого Стр Из ДокументОбъект.КредиторскаяЗадолженность Цикл
					Если ЗначениеЗаполнено(Стр.ОбъектРасчетов) И Стр.НаправлениеДеятельности <> СоответствиеНаправлений[Стр.ОбъектРасчетов] Тогда
						Стр.НаправлениеДеятельности = СоответствиеНаправлений[Стр.ОбъектРасчетов];
					КонецЕсли;
				КонецЦикла;
		...
		Исключение
			...
			Если ПричинаИсключения = "ПлохиеДанные" Тогда
				...
				ОписаниеПроблемы = ОбновлениеИнформационнойБазыУТ.ПроблемаСДанными(
					СтрокаДанных.Ссылка, Рекомендация, ИнформацияОбОшибке());
			...
			ИначеЕсли ПричинаИсключения = "Запись" Тогда
				...
				ВызватьИсключение СтрСоединить(НСтр("ru = 'Не удалось обработать документ ""Взаимозачет задолженности"".'"), Символы.ПС);
