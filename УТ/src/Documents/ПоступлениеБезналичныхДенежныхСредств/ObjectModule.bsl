...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ПараметрыВыбораСтатейИАналитик = Документы.ПоступлениеБезналичныхДенежныхСредств.ПараметрыВыбораСтатейИАналитик(
		ХозяйственнаяОперация, ВыполнятьПроверкуЗаполненияСтатьиКомисии);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьУчетПрочихАктивовПассивов") И НЕ ЗначениеЗаполнено(СтатьяАктивовПассивов) Тогда
		СтатьяАктивовПассивов = ПланыВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансПрочиеПассивы;
	КонецЕсли;
	...
	ПараметрыВыбораСтатейИАналитик = Документы.ПоступлениеБезналичныхДенежныхСредств.ПараметрыВыбораСтатейИАналитик(
		ХозяйственнаяОперация, ОтражатьКомиссию И Не ВестиУчетКомиссииВоВзаиморасчетах);
...
Процедура ЗаполнитьПоЗаказуКлиента(Знач ДокументОснование, ДанныеЗаполнения, СуммаКОплате = 0)
	...
	Иначе
		...
		ПараметрыЗаполненияРасшифровки.СтатьяДвиженияДенежныхСредств = Выборка.СтатьяДвиженияДенежныхСредств;
...
Процедура ЗаполнитьПоСчетуНаОплатуКлиенту(Знач ДокументОснование, ДанныеЗаполнения)
	...
	Если Выборка.Следующий() Тогда
		...
		ВзаиморасчетыЗаполнениеРасшифровкиПлатежа.ЗаполнитьРасшифровкуПлатежаПоСчетуНаОплату(
			ДокументОснование,
			Выборка.Организация,
...
Процедура ЗаполнитьПоСписаниюБезналичныхДенежныхСредств(Знач ДокументОснование, ДанныеЗаполнения)
	...
	Иначе
		...
		Если ДанныеЗаполнения.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.КонвертацияВалюты Тогда
			...
			ПараметрыКонвертации.СуммаДокумента = 0;
...
Процедура ЗаполнитьПоРеализацииТоваровУслуг(Знач ДокументОснование, ДанныеЗаполнения, СуммаКОплате = 0)
	...
	Если Выборка.Следующий() Тогда
		...
		Иначе
			...
			|	И НЕ (ТИПЗНАЧЕНИЯ(ТаблицаТовары.ЗаказКлиента) = ТИП(Документ.ОтгрузкаТоваровКлиенту)
			|		ИЛИ ТИПЗНАЧЕНИЯ(ТаблицаТовары.ЗаказКлиента) = ТИП(Документ.ПервичныйДокумент)
			|			И ВЫРАЗИТЬ(ТаблицаТовары.ЗаказКлиента КАК
			|			Документ.ПервичныйДокумент).ТипПервичногоДокумента =
...
Процедура ЗаполнитьПоОтчетуКомиссионера(Знач ДокументОснование, ДанныеЗаполнения, СуммаКОплате = 0)
	...
	Если Выборка.Следующий() Тогда
		...
		Иначе
			...
			ВзаиморасчетыЗаполнениеРасшифровкиПлатежа.ЗаполнитьРасшифровкуПлатежаПоВозвратуПоставщику(
...
Процедура ЗаполнитьПоРеализацииУслугПрочихАктивов(Знач Реализация, ДанныеЗаполнения, СуммаКОплате = 0)
	...
	Если Выборка.Следующий() Тогда
		...
		ВзаиморасчетыЗаполнениеРасшифровкиПлатежа.ЗаполнитьРасшифровкуПлатежаПоЗаказуКлиента(ПараметрыЗаполненияРасшифровки, РасшифровкаПлатежа);
...
Процедура ЗаполнитьПоОтчетуПоКомиссии(Знач ДокументОснование, ДанныеЗаполнения, КОплате = 0, Вознаграждение = Ложь)
	...
	Если Выборка.Следующий() Тогда
		...
		Если Не ЗначениеЗаполнено(ДанныеЗаполнения.БанковскийСчет) Тогда
			...
			ПараметрыСчета.Валюта = ДанныеЗаполнения.Валюта;
			ДанныеЗаполнения.БанковскийСчет = Справочники.БанковскиеСчетаОрганизаций.ПолучитьБанковскийСчетПоУмолчанию(
				ПараметрыСчета);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ДанныеЗаполнения.БанковскийСчетОтправитель) Тогда
			ПараметрыСчета = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации();
			ПараметрыСчета.Организация = ДанныеЗаполнения.ОрганизацияОтправитель;
			ПараметрыСчета.Валюта = ДанныеЗаполнения.Валюта;
			ПараметрыСчета.КорреспондирующийСчет = ДанныеЗаполнения.БанковскийСчет;
			ДанныеЗаполнения.БанковскийСчетОтправитель =
...
Процедура ЗаполнитьПоМассивуОснований(ДанныеЗаполнения, МассивОснований)
	...
	Если ДанныеЗаполнения.Свойство("ХозяйственнаяОперация")
		...
		ПараметрыСчета = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации();
		ПараметрыСчета.Организация = ДанныеЗаполнения.Контрагент;
		ПараметрыСчета.Валюта = ДанныеЗаполнения.Валюта;
		ПараметрыСчета.КорреспондирующийСчет = ДанныеЗаполнения.БанковскийСчет;
		БанковскийСчетОтправитель = Справочники.БанковскиеСчетаОрганизаций.ПолучитьБанковскийСчетПоУмолчанию(
			ПараметрыСчета);
	Иначе
		БанковскийСчетКонтрагента = Справочники.БанковскиеСчетаКонтрагентов.ПолучитьБанковскийСчетПоУмолчанию(
			ДанныеЗаполнения.Контрагент, ДанныеЗаполнения.Валюта, ДанныеЗаполнения.БанковскийСчет);
...
Процедура ЗаполнитьПоВыкупуТоваровХранителем(Знач ДокументОснование, ДанныеЗаполнения, СуммаКОплате = 0)
	...
	|	ДанныеДокумента.Договор КАК Договор,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеДокумента.Договор.ПорядокРасчетов, ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоДоговорамКонтрагентов)
	|			ТОГДА ДанныеДокумента.Договор.СтатьяДвиженияДенежныхСредств
	|		ИНАЧЕ
	|			ВЫБОР
	|				КОГДА ЕСТЬNULL(ДанныеДокумента.Договор.СтатьяДвиженияДенежныхСредств, ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка)
	|					ТОГДА ЕСТЬNULL(ДанныеДокумента.Соглашение.СтатьяДвиженияДенежныхСредств, ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка))
	|				ИНАЧЕ ДанныеДокумента.Договор.СтатьяДвиженияДенежныхСредств
	|			КОНЕЦ
	|	КОНЕЦ КАК СтатьяДвиженияДенежныхСредств,
	|	ДанныеДокумента.СуммаДокумента КАК Сумма,
	|	ДанныеДокумента.Ссылка КАК ДокументОснование,
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	Если ТипЗнч(ДанныеЗаполнения) <> Тип("Структура")
		...
		БанковскийСчет = ДенежныеСредстваСервер.ТекущийБанковскийСчетОрганизации();
		
		Если Не ЗначениеЗаполнено(БанковскийСчет) Тогда
			ПараметрыСчета = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации();
			ПараметрыСчета.Организация = ?(ЗначениеЗаполнено(Организация), Организация, Неопределено);
			ПараметрыСчета.Валюта = ?(ЗначениеЗаполнено(Валюта), Валюта, Неопределено);
	...
	ЕстьДанныеВалюты = ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		И ДанныеЗаполнения.Свойство("Валюта")
		И ЗначениеЗаполнено(ДанныеЗаполнения.Валюта);
	
	ЕстьДанныеПартнера = ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		И ДанныеЗаполнения.Свойство("Партнер")
		И ЗначениеЗаполнено(ДанныеЗаполнения.Партнер);
	...
	Для каждого СтрокаТЧ Из РасшифровкаПлатежа Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧ.Партнер)
			И ЕстьДанныеПартнера Тогда
			СтрокаТЧ.Партнер = ДанныеЗаполнения.Партнер;
		КонецЕсли;
		
		Если ЕстьДанныеВалюты Тогда
