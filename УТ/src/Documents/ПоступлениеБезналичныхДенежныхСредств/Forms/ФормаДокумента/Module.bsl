...
Процедура ДоговорЭквайрингаПриИзмененииНаСервере()
	...
	Если Объект.ОтражатьКомиссию Тогда
		...
		Если ЗначенияРеквизитов.ФиксированнаяСтавкаКомиссии Тогда
			...
			Объект.СуммаКомиссии = ДенежныеСредстваКлиентСервер.РассчитатьСуммуКомиссии(Объект.СуммаДокумента, СтавкаКомиссии, "Поступление");
		Иначе
			СтавкаКомиссии = 0;
		КонецЕсли;
...
Процедура СуммаКомиссииПриИзменении(Элемент)
	...
	Если Объект.СуммаДокумента <> 0 Тогда
		СтавкаКомиссии = ДенежныеСредстваКлиентСервер.РассчитатьСтавкуКомиссии(
			Объект.СуммаДокумента, Объект.СуммаКомиссии, "Поступление");
	Иначе
		СтавкаКомиссии = 0;
	КонецЕсли;
...
Процедура РасшифровкаОбъектРасчетовПриИзмененииСервер(ИдСтроки)
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы.ОбъектРасчетов) Тогда
		СтрокаТаблицы.СтатьяДвиженияДенежныхСредств = ДенежныеСредстваСервер.СтатьяДвиженияДенежныхСредствОбъектаРасчетов(
			СтрокаТаблицы.ОбъектРасчетов, Объект.ХозяйственнаяОперация);
	КонецЕсли;
...
&НаКлиенте
Процедура РасшифровкаПлатежаОбъектРасчетовОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные,
...
&НаКлиенте
Процедура РасшифровкаБезРазбиенияОбъектРасчетовОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные,
	ВыборДобавлением, СтандартнаяОбработка)
...
&НаКлиенте
Процедура РасшифровкаБезРазбиенияОснованиеПлатежаОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные,
	ВыборДобавлением, СтандартнаяОбработка)
...
Процедура РасшифровкаПлатежаОбъектРасчетовОбработкаВыбораНаСервере(ВыбранноеЗначение)
	...
	СтрокаТаблицы.Организация = ВыбранноеЗначение.Организация;
	...
	СтрокаТаблицы.НаправлениеДеятельности = ВыбранноеЗначение.НаправлениеДеятельности;
	...
	СтрокаТаблицы.ИдентификаторПлатежа = ВыбранноеЗначение.ИдентификаторПлатежа;
	...
	ДенежныеСредстваСервер.ЗаполнитьДанныеОбъектаРасчетовРасшифровкиПлатежа(Объект, ИдентификаторСтроки, Истина);
	УстановитьОграничениеТипаДоговораРасшифровкиПлатежа();
...
Процедура ГиперссылкаРеквизитыОбъектаРасчетовНажатиеЗавершениеНаСервере()
	...
	Для Каждого Индекс Из ИзмененныеСтроки Цикл
		...
		ОбработатьИзменениеУИП(СтрокаРасшифровки);
	КонецЦикла;
	...
	Тогда
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитов.Организация) Тогда
			Если ЗначениеЗаполнено(ОрганизацияПлательщик)
				...
				ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтотОбъект, "БанковскийСчетОтправитель.Владелец");
			КонецЕсли;
...
Процедура ИдентификаторПлатежаОбработкаВыбораНаСервере(ВыбранноеЗначение)
	...
	Иначе
		...
		Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
			...
			СтрокаТаблицы.ОбъектРасчетов = ОбъектыРасчетовСервер.ОбъектРасчетовПоОснованиюПлатежа(
...
Процедура ОпределитьДействиеПриказовПлатежейВБюджет()
	...
	ПрименениеПриказа58н = ПлатежиВБюджетКлиентСервер.ДействуетПриказ58н_202604(ТекущаяДата);
