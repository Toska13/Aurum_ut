...
Процедура ТоварнаяГруппаПриИзменении(Элемент)
	...
	ТоварнаяГруппаИзменение(Элемент);
...
Процедура ОткрытьФормуРедактированияДанныхПервичногоДокумента(ИдентификаторыСтрок)
	...
	ВозвратВОборотТовараПриобретавшегосяНеДляПродажи = ВозвратВОборотКлиентСервер.ВозвратВОборотТовараПриобретавшегосяНеДляПродажи(
		Объект.ТоварнаяГруппа, Объект.ВидВозврата);
	
	ПервичныйДокументДоступенЕслиОплачен = ВозвратВОборотКлиентСервер.ДоступноПолеОплачен(
		Объект.ТоварнаяГруппа, Объект.ВидВозврата);
	
	ДоступныеВидыПервичныхДокументов = ДоступныеВидыПервичныхДокументов(Объект.ВидВозврата);
	...
	Если ПервичныйДокументДоступенЕслиОплачен Тогда
...
Процедура ВопросПриИзмененииТоварнойГруппыПриЗавершении(РезультатВопроса, ВыбраннаяТоварнаяГруппа) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да
		...
		ТоварнаяГруппаИзменение(Элементы.ТоварнаяГруппа, РезультатВопроса = Истина);
		Модифицированность = Истина;
	КонецЕсли;
...
&НаКлиенте
Процедура ТоварнаяГруппаИзменение(Элемент, ПерезаполнитьПоОснованию = Ложь)
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	ВидыВозврата = ВозвратВОборотКлиентСервер.ДоступныеВидыВозврата(ВидыПродукцииПоТоварнойГруппе);
	
	Элементы.ВидВозврата.СписокВыбора.Очистить();
	Для Каждого ВидВозврата Из ВидыВозврата Цикл
		Элементы.ВидВозврата.СписокВыбора.Добавить(ВидВозврата);
	КонецЦикла;
	
	Элементы.ВидВозврата.СписокВыбора.СортироватьПоЗначению();
	
	Элементы.ТоварыПредставлениеПервичногоДокумента.АвтоОтметкаНезаполненного =
		МаркировкаТоваровКлиентСервер.ПроверятьЗаполнениеПоляВидПервичногоДокумента(
			ВидыПродукцииПоТоварнойГруппе, Объект.ВидВозврата)
		Или МаркировкаТоваровКлиентСервер.ПроверятьЗаполнениеПоляНомерДатаПервичногоДокумента(
			ВидыПродукцииПоТоварнойГруппе, Объект.ВидВозврата);
	
	Элементы.ТоварыПредставлениеПервичногоДокумента.Видимость = ВыводИзОборотаКлиентСервер.ДоступноПолеВидПервичногоДокумента(
		ВидыПродукцииПоТоварнойГруппе, Объект.ВидВозврата);
	
	Элементы.ТоварыСертификация.АвтоОтметкаНезаполненного = МаркировкаТоваровКлиентСервер.ПроверятьЗаполнениеПоляСертификация(
		ВидыПродукцииПоТоварнойГруппе, Объект.ВидВозврата);
	
	Элементы.ТоварыСертификация.Видимость = МаркировкаТоваровКлиентСервер.ДоступноПолеСертификация(
		ВидыПродукцииПоТоварнойГруппе, Объект.ВидВозврата);
	
	Элементы.ИдентификаторГосударственногоКонтракта.Видимость = ВыводИзОборотаКлиентСервер.ДоступноПолеИдентификатораГосударственногоКонтракта(
		ВидыПродукцииПоТоварнойГруппе, Объект.ВидВозврата);
...
Функция ДоступныеВидыПервичныхДокументов(Причина)
	...
	Возврат Справочники.ВидыВозвратаВОборотИСМП.ДоступныеВидыПервичныхДокументов(Причина);
