...
Функция РаспределитьРасходыНаФинансовыйРезультат(ПараметрыОбработчика, ДокументРаспределения = Неопределено) Экспорт
	...
	КоличествоФЗ = РасчетСебестоимостиПовтИсп.ЗначенияТехнологическихПараметров(Справочники.ОперацииЗакрытияМесяца.ПустаяСсылка()).МаксимальноеКоличествоФЗФормированияДокументов;
	...
	Пока ВыборкаПоОрганизациям.Следующий() Цикл
		...
		Пока ВыборкаРегистраторов.Следующий() Цикл 
			...
			Пока ВыборкаДетальная.Следующий() Цикл
				...
				Если ВыборкаДетальная.РасходыТекущегоПериода Тогда
				...
				Если Не ВыборкаДетальная.Расходы = 0 
					Или ВыборкаДетальная.РасходыТекущегоПериода Тогда
...
Процедура ПодготовитьТаблицыКРасчету(ПараметрыРасчета, Запрос = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|	Реквизиты.НазначениеНастройкиРаспределения = ЗНАЧЕНИЕ(Перечисление.НазначениеПравилРаспределенияРасходов.БезРаспределенияРасходов) КАК РасходыТекущегоПериода,
		...
		|			ЗНАЧЕНИЕ(Перечисление.НазначениеПравилРаспределенияРасходов.БезРаспределенияРасходов))
		|		ИЛИ (
		|			Реквизиты.НазначениеНастройкиРаспределения В (
		...
		|			И Реквизиты.СтатьяРасходов.ВариантРаспределенияРасходовНУ В (
...
Функция ТекстЗапросаПоступилоПрочихРасходов(ВариантРасчета = "ПредварительныйРасчет", ИнициализацияОВЗ = Истина) Экспорт
	...
	ТекстРасходыКРаспределениюСводно = 
		...
		ТекстыЗапроса.Добавить(ТекстЗапросаВТДвиженияПрочихРасходовПоСтатьямРасходов());
...
Функция ТекстЗапросаДанныеДляРаспределения(ВыбиратьПервые = 0, ПоместитьСостояниеРасходовВВТ = Ложь, ВызовДляСКД = Ложь) Экспорт
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|	СУММА(НеРаспределенные.СуммаОстаток) КАК СуммаОстаток,
		|	СУММА(НеРаспределенные.СуммаБезНДСОстаток) КАК СуммаБезНДСОстаток,
		|	СУММА(НеРаспределенные.СуммаУпрОстаток) КАК СуммаУпрОстаток,
		|	СУММА(НеРаспределенные.СуммаРеглОстаток) КАК СуммаРеглОстаток,
		|	СУММА(НеРаспределенные.СуммаНУОстаток) КАК СуммаНУОстаток,
		|	ВЫБОР
		...
		|	(ВЫБОР
		|		КОГДА НЕ ЕСТЬNULL(Остатки.РаспределеноВУУ, ИСТИНА)
		|			ТОГДА ЕСТЬNULL(Остатки.СуммаОстаток, 0)
		|		ИНАЧЕ 0 КОНЕЦ) КАК СуммаОстаток,
		|	(ВЫБОР
		|		КОГДА НЕ ЕСТЬNULL(Остатки.РаспределеноВУУ, ИСТИНА)
		|			ТОГДА ЕСТЬNULL(Остатки.СуммаБезНДСОстаток, 0)
		|		ИНАЧЕ 0 КОНЕЦ) КАК СуммаБезНДСОстаток,
		|	(ВЫБОР
		|		КОГДА НЕ ЕСТЬNULL(Остатки.РаспределеноВУУ, ИСТИНА)
		|			ТОГДА ЕСТЬNULL(Остатки.СуммаУпрОстаток, 0)
		|		ИНАЧЕ 0 КОНЕЦ) КАК СуммаУпрОстаток,
		|	(ВЫБОР
		|		КОГДА НЕ ЕСТЬNULL(Остатки.РаспределеноВБУ, ИСТИНА)
		|			ТОГДА ЕСТЬNULL(Остатки.СуммаРеглОстаток, 0)
		|		ИНАЧЕ 0 КОНЕЦ) КАК СуммаРеглОстаток,
		|	(ВЫБОР
		|		КОГДА НЕ ЕСТЬNULL(Остатки.РаспределеноВНУ, ИСТИНА)
		|			ТОГДА ЕСТЬNULL(Остатки.СуммаНУОстаток, 0)
		|		ИНАЧЕ 0 КОНЕЦ) КАК СуммаНУОстаток,
		...
		|		 И Источник.ВариантРаспределенияРасходовУпр <> ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода)
		...
		|		КОГДА Задания.Месяц > &КонецПериода И НЕ ЕСТЬNULL(Остатки.РаспределеноВУУ, ИСТИНА)
		|			И Источник.ВариантРаспределенияРасходовУпр В (ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаНаправленияДеятельности),
		|														ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода))
		...
		|		 И Источник.ВариантРаспределенияРасходовРегл <> ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СостоянияРаспределенияРасходов.НеРаспределено)
		...
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СостоянияРаспределенияРасходов.ОшибкаРаспределения)
		|		КОГДА Задания.Месяц > &КонецПериода И НЕ ЕСТЬNULL(Остатки.РаспределеноВБУ, ИСТИНА)
		|			И Источник.ВариантРаспределенияРасходовРегл В (ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаНаправленияДеятельности),
		|															ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода))
		...
		|	ВТДанныеДляЗагрузки.СуммаОстаток КАК СуммаОстаток,
		|	ВТДанныеДляЗагрузки.СуммаБезНДСОстаток КАК СуммаБезНДСОстаток,
		|	ВТДанныеДляЗагрузки.СуммаУпрОстаток КАК СуммаУпрОстаток,
		|	ВТДанныеДляЗагрузки.СуммаРеглОстаток КАК СуммаРеглОстаток,
		|
...
Процедура ИнициализироватьЗапросПолученияДанныхДляРаспределения(Запрос, ПараметрыЗапроса) Экспорт
	...
	Если ПараметрыЗапроса.ВариантРаспределения = Неопределено Тогда
		...
		ПоддерживаемыеВариантыРаспределения.Добавить(Перечисления.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода);
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	НаФРБезРаспределения = ПредопределенноеЗначение("Перечисление.НазначениеПравилРаспределенияРасходов.БезРаспределенияРасходов");
	...
	ФормыПоНазначению.Вставить(НаФРБезРаспределения, "ФормаНастроекНаФР");
	...
	ИначеЕсли Параметры.Свойство("Основание") Тогда
		...
		НазначенияПоВарианту.Вставить(
			...
			НаФР);
		НазначенияПоВарианту.Вставить(
			ПредопределенноеЗначение("Перечисление.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода"),
			НаФРБезРаспределения);
		НазначенияПоВарианту.Вставить(
...
Процедура ДобавитьЭтап_ОформлениеДокументовРаспределенияРасходовПоНаправлениямДеятельности(ТаблицаЭтапов, ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ОформлениеДокументовРаспределенияРасходовПоНаправлениямДеятельности);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Оформление документов распределения расходов'"; // @НСтр-1
...
Процедура Использование_ОформлениеДокументовРаспределенияРасходовПоНаправлениямДеятельности(ПараметрыОбработчика) Экспорт
	...
	ВариантыРаспределения.Добавить(Перечисления.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода);
...
Процедура ДобавитьЭтап_РаспределениеРасходовПоНаправлениямДеятельности(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.РаспределениеРасходовПоНаправлениямДеятельности);
		
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Распределение расходов по направлениям деятельности'"; // @НСтр-1
	НоваяСтрока.ПредшествующиеЭтапы.Добавить(Справочники.ОперацииЗакрытияМесяца.РасчетПартийИСебестоимости);
	НоваяСтрока.ПредшествующиеЭтапы.Добавить(Справочники.ОперацииЗакрытияМесяца.РаспределениеНДС);
	НоваяСтрока.ПредшествующиеЭтапы.Добавить(Справочники.ОперацииЗакрытияМесяца.ОформлениеДокументовРаспределенияРасходовПоНаправлениямДеятельности);
...
Процедура Использование_РаспределениеРасходовПоНаправлениямДеятельности(ПараметрыОбработчика) Экспорт
	...
	Запрос.Текст =
		...
		|			ИЛИ Т.ДокументУпр.НазначениеНастройкиРаспределения = ЗНАЧЕНИЕ(Перечисление.НазначениеПравилРаспределенияРасходов.БезРаспределенияРасходов))
		...
		|			ИЛИ Т.ДокументРегл.НазначениеНастройкиРаспределения = ЗНАЧЕНИЕ(Перечисление.НазначениеПравилРаспределенияРасходов.БезРаспределенияРасходов))
...
Процедура ПроверкаОстатковПрочихРасходов(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("СтатьяРасходов", 			НСтр("ru = 'Статья расходов'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("АналитикаРасходов", 		НСтр("ru = 'Аналитика расходов'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("СуммаОстаток", 			НСтр("ru = 'Остаток с НДС'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("СуммаБезНДСОстаток", 		НСтр("ru = 'Остаток без НДС'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("СуммаРеглОстаток", 		НСтр("ru = 'Остаток регл'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("СуммаНУОстаток", 			НСтр("ru = 'Остаток НУ'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("СуммаУпрОстаток", 		НСтр("ru = 'Остаток упр'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
// Формирует текст запроса для получения движений прочих расходов по статьям расходов.
//
// Возвращаемое значение:
//   Строка - пакет запросов формирования временных таблиц, движений прочих расходов, для различных типов учета.
//
Функция ТекстЗапросаВТДвиженияПрочихРасходовПоСтатьямРасходов()
	...
	|	СтатьиКРаспределению.ВариантРаспределенияРасходов КАК ВариантРаспределенияРасходов,
	|	СтатьиКРаспределению.Предопределенный КАК Предопределенный,
	|	Движения.СтатьяРасходов КАК СтатьяРасходов
	|ПОМЕСТИТЬ ВТДвиженияПрочихРасходовПоСтатьямРасходов
	|	ИЗ
	|		СтатьиРасходовКРаспределению КАК СтатьиКРаспределению
	...
	ТекстДвиженияПрочихРасходовУпр = ЗаменитьПоШаблону(ШаблонТекстаЗапроса, ШаблонЗамен, "ТекстДвиженияПрочихРасходовУпр");
	ТекстДвиженияПрочихРасходовРегл = ЗаменитьПоШаблону(ШаблонТекстаЗапроса, ШаблонЗамен, "ТекстДвиженияПрочихРасходовРегл");
	ТекстДвиженияПрочихРасходовНДД = ЗаменитьПоШаблону(ШаблонТекстаЗапроса, ШаблонЗамен, "ТекстДвиженияПрочихРасходовНДД");
...
Функция ТекстЗапросаПоступилоПрочихРасходовИсточники(ВариантРасчета) Экспорт
	...
	|	ИЗ
	|		ВТДвиженияПрочихРасходовПоСтатьямРасходов КАК ДвиженияПрочихРасходов
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходы КАК Движения
	|			ПО Движения.Период = ДвиженияПрочихРасходов.Период
	|				И Движения.Регистратор = ДвиженияПрочихРасходов.Регистратор
	|				И Движения.НомерСтроки = ДвиженияПрочихРасходов.НомерСтроки
	|				И Движения.СтатьяРасходов = ДвиженияПрочихРасходов.СтатьяРасходов 
	|			
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеПрочихЗатрат КАК ДД
	|			ПО ДвиженияПрочихРасходов.Регистратор = ДД.Ссылка
	...
	|		ИЛИ ДвиженияПрочихРасходов.ВариантРаспределенияРасходов = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаСебестоимостьТоваров)
	|		)
...
Функция ШаблонЗамен()
	...
	ТекстДвиженияПрочихРасходовУпр.Вставить("ВТДвиженияПрочихРасходовПоСтатьямРасходов",
		"ВТДвиженияПрочихРасходовПоСтатьямРасходовУпр");
	...
	ТекстДвиженияПрочихРасходовРегл.Вставить("ВТДвиженияПрочихРасходовПоСтатьямРасходов",
		"ВТДвиженияПрочихРасходовПоСтатьямРасходовРегл");
	...
	ТекстДвиженияПрочихРасходовНДД.Вставить("ВТДвиженияПрочихРасходовПоСтатьямРасходов",
		"ВТДвиженияПрочихРасходовПоСтатьямРасходовНДД");
...
Функция ТекстРасходыКРаспределению(РежимОтладкиПоИсточникам)
	...
	|	ЛОЖЬ КАК РасходыТекущегоПериода,
	...
	|			ИЛИ НЕ ИсточникНастроек.ВариантРаспределенияРасходовУпр В (ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаНаправленияДеятельности),
	|																		ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода))
	|			ТОГДА 0
	...
	|			ИЛИ НЕ ИсточникНастроек.ВариантРаспределенияРасходовРегл В (ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаНаправленияДеятельности),
	|																		ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода))
	|			ТОГДА 0
	|		КОГДА ЕСТЬNULL(ДолиСписания.Транспортные, ЛОЖЬ)
	|				И ИсточникНастроек.ВариантРаспределенияРасходовРегл <> ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода)
	|			ТОГДА ВЫРАЗИТЬ(ОстаткиРасходов.СуммаРегл * ВЫРАЗИТЬ(ДолиСписания.ДоляСписания КАК ЧИСЛО(10,6)) КАК ЧИСЛО(31, 2))
	...
	|	Регистраторы.РасходыТекущегоПериода КАК РасходыТекущегоПериода,
	...
	|		И ИсточникНастроек.ВариантРаспределенияРасходовУпр В 
	|			(ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаНаправленияДеятельности),
	|			ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода)))
	|	ИЛИ
	|	(НЕ ОстаткиРасходов.СуммаРегл = 0 И Регистраторы.РегламентированныйУчет 
	|		И ИсточникНастроек.ВариантРаспределенияРасходовРегл В
	|				(ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаНаправленияДеятельности),
	|				ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода)))";
	ТекстыВложенногоЗапроса.Добавить(ТекстЗапроса);
	#КонецОбласти
	...
	|	ВложенныйЗапрос.РасходыТекущегоПериода КАК РасходыТекущегоПериода,
	...
	|		ВложенныйЗапрос.РасходыТекущегоПериода,
...
Функция ТекстРаспределенныеРасходы()
	...
	|	РасходыКРаспределению.РасходыТекущегоПериода КАК РасходыТекущегоПериода,
...
Функция ТекстРезультатРаспределения(РежимОтладкиПоИсточникам)
	...
	|	РаспределенныеРасходы.РасходыТекущегоПериода КАК РасходыТекущегоПериода,
	|	РаспределенныеРасходы.СтатьяРасходов КАК СтатьяРасходов,
	|	РаспределенныеРасходы.АналитикаРасходов КАК АналитикаРасходов,
...
Функция ТекстВыборкаРезультатРаспределения()
	...
	|	РезультатРаспределения.РасходыТекущегоПериода КАК РасходыТекущегоПериода,
