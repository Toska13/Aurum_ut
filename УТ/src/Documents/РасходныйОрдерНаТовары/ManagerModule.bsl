...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		ТекстЗапросаДоставка(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект);
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	#Иначе
		ПараметрыСерийСклада = МобильноеРабочееМестоКладовщика.ИспользованиеСерийНаСкладе(Объект.Склад, Ложь);
		
		ОтгружаемыеТовары = МобильноеРабочееМестоКладовщика.НоменклатураКлиентСерверПараметрыУказанияСерий();
		
		ОтгружаемыеТовары.ИспользоватьАдресноеХранение = МобильноеРабочееМестоКладовщика.ИспользоватьАдресноеХранение(Объект.Склад, Объект.Помещение, Объект.ДатаОтгрузки);
	#КонецЕсли
	...
	#Иначе
		ОтгружаемыеТовары.РегистрироватьСерии = МобильноеРабочееМестоКладовщика.НеобходимоРегистрироватьСерии(ОтгружаемыеТовары);
	#КонецЕсли
	...
	ОтгружаемыеТовары.ЭтоОрдер = Истина;
	ОтгружаемыеТовары.ИмяПоляПомещение = "Помещение";
	ОтгружаемыеТовары.ИмяПоляКонтролируемаяПартия = "КонтролируемаяПартия";
	...
	#Иначе
		ТоварыПоРаспоряжениям = МобильноеРабочееМестоКладовщика.НоменклатураКлиентСерверПараметрыУказанияСерий();
		ТоварыПоРаспоряжениям.ИспользоватьАдресноеХранение = МобильноеРабочееМестоКладовщика.ИспользоватьАдресноеХранение(Объект.Склад, Объект.Помещение, Объект.ДатаОтгрузки);
	#КонецЕсли
	...
	ТоварыПоРаспоряжениям.ИмяПоляКонтролируемаяПартия = "КонтролируемаяПартия";
...
Функция ТекстЗапросаДоставка(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если НЕ ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25()
	 ИЛИ НЕ ДокументОбъект.ОтгрузкаПоЗаданиюНаПеревозку
	 ИЛИ ТипЗнч(ДокументОбъект.ЗаданиеНаПеревозку) <> Тип("ДокументСсылка.Рейс") Тогда
		Возврат "";
	КонецЕсли;
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ШапкаДокумента.ДатаОтгрузки КАК Период,
	...
	|	ШапкаДокумента.Получатель КАК Получатель,
	|	ТаблицаТовары.Распоряжение КАК Распоряжение,
	|	ШапкаДокумента.ЗаданиеНаПеревозку КАК Рейс,
	|	ТаблицаТовары.Номенклатура КАК Номенклатура,
	|	ТаблицаТовары.Характеристика КАК Характеристика,
	|	ТаблицаТовары.Серия КАК Серия,
	|	1 КАК Плечо,
	|	ВЫБОР
	|		КОГДА ШапкаДокумента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыРасходныхОрдеров.Подготовлен)
	|			ТОГДА ТаблицаТовары.Количество
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ЗапланированаСборка,
	|	ВЫБОР
	|		КОГДА ШапкаДокумента.Статус В (&СтатусыСобирается)
	|			ТОГДА ТаблицаТовары.Количество
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Собирается,
	|	ВЫБОР
	|		КОГДА ШапкаДокумента.Статус В (&СтатусыСобрано)
	|			ТОГДА ТаблицаТовары.Количество
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Собрано,
	|	ТаблицаТовары.Назначение КАК Назначение
	|ИЗ
	|	ДанныеДокументаТаблицаТоварыПоРаспоряжениям КАК ТаблицаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеДокументаШапка КАК ШапкаДокумента
	|		ПО ТаблицаТовары.Ссылка = ШапкаДокумента.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументов
	|		ПО ТаблицаТовары.Распоряжение = РеестрДокументов.Ссылка
	|			И (НЕ РеестрДокументов.ДополнительнаяЗапись)";
	
	СтатусыСобирается = Новый Массив; // Массив из ПеречислениеСсылка.СтатусыРасходныхОрдеров
	СтатусыСобирается.Добавить(Перечисления.СтатусыРасходныхОрдеров.КОтбору);
	СтатусыСобирается.Добавить(Перечисления.СтатусыРасходныхОрдеров.КПроверке);
	СтатусыСобирается.Добавить(Перечисления.СтатусыРасходныхОрдеров.ВПроцессеПроверки);
	СтатусыСобирается.Добавить(Перечисления.СтатусыРасходныхОрдеров.Проверен);
	Запрос.УстановитьПараметр("СтатусыСобирается", СтатусыСобирается);
	
	СтатусыСобрано = Новый Массив; // Массив из ПеречислениеСсылка.СтатусыРасходныхОрдеров
	СтатусыСобрано.Добавить(Перечисления.СтатусыРасходныхОрдеров.КОтгрузке);
	СтатусыСобрано.Добавить(Перечисления.СтатусыРасходныхОрдеров.Отгружен);
	Запрос.УстановитьПараметр("СтатусыСобрано", СтатусыСобрано);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	//++ Локализация
	ИнтеграцияСМаркетплейсамиСервер.ДобавитьКомандуПечатиЭтикеток(КомандыПечати);
	//-- Локализация
