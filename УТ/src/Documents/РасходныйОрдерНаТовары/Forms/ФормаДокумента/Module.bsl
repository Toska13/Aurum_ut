...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25() Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Редактирование поля ""Тип отгрузки"" не рекомендуется'");
		Элементы.ТипОтгрузки.ОтображениеПредупрежденияПриРедактировании = ОтображениеПредупрежденияПриРедактировании.Отображать;
		Элементы.ТипОтгрузки.ПредупреждениеПриРедактировании = ТекстПредупреждения;
		
		ТекстПредупреждения = НСтр("ru = 'Редактирование поля ""Рейс"" не рекомендуется'");
		Элементы.ЗаданиеНаПеревозку.ОтображениеПредупрежденияПриРедактировании = ОтображениеПредупрежденияПриРедактировании.Отображать;
		Элементы.ЗаданиеНаПеревозку.ПредупреждениеПриРедактировании = ТекстПредупреждения; 
...
Процедура ЗаданиеНаПеревозкуНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25() Тогда
		...
		ОтборРейсов = Новый Структура;
		ОтборРейсов.Вставить("СкладОтправления", Объект.Склад);
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ТекущаяСтрока", Объект.ЗаданиеНаПеревозку);
		ПараметрыФормы.Вставить("Отбор", ОтборРейсов);
		ОткрытьФорму("Документ.Рейс.ФормаВыбора",
			ПараметрыФормы,
			Элемент);
		
	Иначе
		
		ПараметрыФормы = Новый Структура("Ордер,СтатусОрдера", Объект.Ссылка, Объект.Статус);
...
Процедура ДекорацияЭДОНажатие(Элемент, СтандартнаяОбработка)
	...
	// ЭлектронноеВзаимодействие.ОбменСКонтрагентами
	РезультатыПроверки = ЭлектронноеВзаимодействиеУТВызовСервера.ПроверитьНаличиеПараметровЭДПоИсточнику(Объект.Ссылка);
	ДоступныКомандыЭДО = РезультатыПроверки.Получить("ЕстьЭДО");
	Если Не ДоступныКомандыЭДО Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Для использования внутреннего ЭДО необходимо указать распоряжения с одинаковой организацией.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПеренестиВНовыйОрдер(Команда)
	...
	Если Объект.ОтгрузкаПоЗаданиюНаПеревозку Тогда
		...
		ПараметрыФормы = Новый Структура("Ордер", Объект.Ссылка);
		ОткрытьФорму("Документ.РасходныйОрдерНаТовары.Форма.ФормаВыбораЗаданияНаПеревозку",
			ПараметрыФормы,
			ЭтотОбъект,
			,
...
Процедура УстановитьУсловноеОформление()
	...
	ИспользоватьСтатусыРасходныхОрдеров = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Склад, "ИспользоватьСтатусыРасходныхОрдеров");
	Если Не ИспользоватьСтатусыРасходныхОрдеров Тогда
		Элементы.Контролер.Видимость = Ложь;
		Элементы.Отгрузил.Видимость = Ложь;
	КонецЕсли;
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	УстановитьВидимостьУправленияДоставкой();
...
&НаСервере
Процедура УстановитьВидимостьУправленияДоставкой()
	
	УправлениеДоставкой = (ЕстьДоставкаПоСкладскойОперации Или Объект.ОтгрузкаПоЗаданиюНаПеревозку);
	
	Если ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25() Тогда
		
		УправлениеДоставкой = (УправлениеДоставкой И НачинатьОтгрузкуПослеФормированияЗаданияНаПеревозку);
		
	КонецЕсли;
	
	Элементы.ТипОтгрузки.Видимость = УправлениеДоставкой;
	Элементы.ЗаданиеНаПеревозку.Видимость = УправлениеДоставкой;
	Элементы.ПорядокДоставки.Видимость = УправлениеДоставкой;
...
Функция ПроверитьТоварыИПоместитьВоВременноеХранилище(АдресХранилища, ВыгруженаТолькоНеМаркируемаяПродукция = Ложь)
	...
	Если ПроверкаПройдена 
		...
		Пока НомерТекущейСтроки >= 0 Цикл
			...
			РасходныйОрдерНаТоварыЛокализация.ОпределениеПризнакаПоэкземплярногоУчетаПродукцииНаДату(ТабличнаяЧастьТовары[НомерТекущейСтроки].ВидПродукцииИС, Объект.Дата, ВедетсяПоштучныйУчетПродукции);
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	УпаковочныеЛистыКлиент.ОбновитьКешированныеЗначенияДляТЧСУпаковочнымиЛистами(
		Элементы.ОтгружаемыеТовары,
		КэшированныеЗначения,
...
Процедура ЗаполнитьСлужебныеРеквизитыТЧТовары(ИмяТЧ, Перенумеровать = Истина)
	...
	Если ИмяТЧ = "ОтгружаемыеТовары" Тогда
		...
		Действия.Вставить("ЗаполнитьЕдиницуИзмеренияВеса",Новый Структура("Номенклатура, Упаковка", "ЕдиницаИзмеренияВеса"));
		Действия.Вставить("ЗаполнитьЕдиницуИзмеренияОбъема",Новый Структура("Номенклатура, Упаковка", "ЕдиницаИзмеренияОбъема"));
		ПакетнаяОбработкаТабличнойЧастиКлиентСерверЛокализация.ДополнитьСтруктуруДействийПриИзмененииЭлемента(ЭтотОбъект, "Номенклатура", Действия);		
	...
	Если ИмяТЧ = "ОтгружаемыеТовары" Тогда
		...
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Объект.ОтгружаемыеТовары, СтруктураДействий);
...
Процедура УстановитьДоступность()
	...
	Элементы.Отгрузил.Доступность = 
		(Объект.Статус = Перечисления.СтатусыРасходныхОрдеров.Проверен);
