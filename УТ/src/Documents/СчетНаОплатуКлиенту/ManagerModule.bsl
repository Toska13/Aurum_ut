...
//	Дата - Дата - дата основания, если не указана берется по параметру ДокументОснование
//
// Возвращаемое значение:
//  Строка - текст назначения платежа.
//
Функция СформироватьНазначениеПлатежа(Знач Номер, Знач ДокументОснование, Знач Дата = Неопределено) Экспорт
	...
	НомерОснования = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Номер, Ложь, Истина);
	ДатаОснования = Формат(Дата, "ДЛФ=D");
		
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		
		ТекстНазначениеПлатежа = НСтр("ru='Оплата по заказу клиента №%Номер% от %Дата%'");
		
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("СправочникСсылка.СоглашенияСКлиентами") Тогда
		
		ТекстНазначениеПлатежа = НСтр("ru='Оплата по соглашению №%Номер% от %Дата%'");
		
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
		
		ТекстНазначениеПлатежа = НСтр("ru='Оплата по заявке на возврат №%Номер% от %Дата%'");
		
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетКомиссионераОСписании") Тогда
		
		ТекстНазначениеПлатежа = НСтр("ru='Оплата по отчету комиссионера о списании №%Номер% от %Дата%'");
		
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетКомитенту") Тогда
		
		ТекстНазначениеПлатежа = НСтр("ru='Оплата по отчету комитенту №%Номер% от %Дата%'");
		
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетКомиссионера") Тогда
		
		ТекстНазначениеПлатежа = НСтр("ru='Оплата по отчету комиссионера о продажах №%Номер% от %Дата%'");
		
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		
		ТекстНазначениеПлатежа = НСтр("ru='Оплата по реализации товаров и услуг №%Номер% от %Дата%'");
		
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ВыкупВозвратнойТарыКлиентом") Тогда
		
		ТекстНазначениеПлатежа = НСтр("ru='Оплата по выкупу возвратной тары клиентом №%Номер% от %Дата%'");
		
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.АктВыполненныхРабот") Тогда
		
		ТекстНазначениеПлатежа = НСтр("ru='Оплата по акту выполненных работ №%Номер% от %Дата%'");
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетКомитентуОЗакупках") Тогда
		...
		ТекстНазначениеПлатежа = НСтр("ru='Оплата по отчету комитенту о закупках №%Номер% от %Дата%'");
		
	КонецЕсли;
		
	ТекстНазначениеПлатежа = СтрЗаменить(ТекстНазначениеПлатежа, "%Номер%", НомерОснования);
	ТекстНазначениеПлатежа = СтрЗаменить(ТекстНазначениеПлатежа, "%Дата%", ДатаОснования);
		
	Возврат ТекстНазначениеПлатежа;
	
КонецФункции

// Возвращает пути к реквизитам счета на оплату.
// 
// Параметры:
//  ДанныеЗаполнения - ДокументОбъект, СправочникОбъект, ДокументСсылка, СправочникСсылка, Структура, ДанныеФормыСтруктура - Объект или коллекция для расчета параметров взаиморасчетов.
// 
// Возвращаемое значение:
//  См. ВзаиморасчетыСервер.ПараметрыМеханизма
Функция ПараметрыВзаиморасчеты(ДанныеЗаполнения = Неопределено) Экспорт

	СтруктураРеквизитов = ВзаиморасчетыСервер.ПараметрыМеханизма();
	
	СтруктураРеквизитов.ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСКлиентом;
	СтруктураРеквизитов.ИзменяетПланОтгрузкиПоставки = Ложь;
	
	СтруктураРеквизитов.НаправлениеДеятельности =   "";
	СтруктураРеквизитов.ПорядокРасчетов =           "";
	СтруктураРеквизитов.Соглашение =                "";
	СтруктураРеквизитов.БанковскийСчетОрганизации = "Объект.БанковскийСчет";
	СтруктураРеквизитов.БанковскийСчетКонтрагента = "";
	СтруктураРеквизитов.Касса =                     "";
	СтруктураРеквизитов.ОплатаВВалюте =             "";
	СтруктураРеквизитов.НалогообложениеНДС =        "";
	СтруктураРеквизитов.ГруппаФинансовогоУчета =    "";
	СтруктураРеквизитов.ПутьКДаннымТЧЭтапыОплаты =  "Объект.ЭтапыГрафикаОплаты";
	СтруктураРеквизитов.Подразделение =             "";
	СтруктураРеквизитов.ДатаОтгрузки =              "";
	
	СтруктураРеквизитов.ДокументОснование =         "Объект.ДокументОснование";
	СтруктураРеквизитов.НазначениеПлатежа =         "Объект.НазначениеПлатежа";
	СтруктураРеквизитов.ТСП =                       "";
