...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДенежныеСредстваСервер.ИсключитьНедоступныеФормыОплаты(Элементы.ФормаОплаты);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ДенежныеСредстваКлиентЛокализация.ОбработкаОповещенияСБП(ЭтотОбъект, Объект.Ссылка, ИмяСобытия);
...
Процедура НадписьОплатаСБПОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ЭтоТолькоПросмотр = НавигационнаяСсылкаФорматированнойСтроки = "ПодробностиИсходящейСсылки";
	
	Если Не Объект.Проведен
		И Не ЭтоТолькоПросмотр Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(Нстр("ru = 'Для оплаты через СБП необходимо провести документ.'"));
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Объект.Ссылка)
		...
		И Не ЭтоТолькоПросмотр Тогда
		ТекстВопроса = НСтр("ru = 'Данные изменены и будут записаны. Продолжить?'");
	Иначе
		
		ДенежныеСредстваКлиентЛокализация.ОбработкаНавигационнойСсылкиСБП(
			ЭтотОбъект, Объект.Ссылка, НавигационнаяСсылкаФорматированнойСтроки);
		Возврат;
	...
	ОповещенияОЗавершении = Новый ОписаниеОповещения(
		"НадписьОплатаСБПОбработкаНавигационнойСсылкиЗавершение",
		ЭтотОбъект,
...
Процедура УправлениеЭлементамиФормы()
	...
	ДоступностьСчета = ЛюбаяОплата 
						Или (Объект.ФормаОплаты = Перечисления.ФормыОплаты.Безналичная)
	...
	СчетНаОплатуКлиентуЛокализация.УстановитьДоступностьЭлементовПоФормеОплаты(
		Элементы.ЛокализацияГруппаПлатежнаяСсылкаСБП, Объект.Ссылка, Объект.ФормаОплаты);
	...
	СчетНаОплатуКлиентуЛокализация.УстановитьНадписьОплатыСБП(Объект.Ссылка, ЭтотОбъект.НадписьОплатаСБП);
...
Процедура ЗаполнитьНазначениеПлатежа()
	...
	Если НЕ ПустаяСтрока(ДокументОснование) Тогда
		РеквизитыОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОснование, "Номер, Дата");
...
&НаКлиенте
Процедура НадписьОплатаСБПОбработкаНавигационнойСсылкиЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт

	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		
		ПараметрыЗаписи = Новый Структура;
		ПараметрыЗаписи.Вставить("РежимЗаписи", РежимЗаписиДокумента.Проведение);
		УспешноЗаписан = Записать(ПараметрыЗаписи);
		
		Если УспешноЗаписан Тогда
			ДенежныеСредстваКлиентЛокализация.ОбработкаНавигационнойСсылкиСБП(
				ЭтотОбъект, Объект.Ссылка, ДополнительныеПараметры.НавигационнаяСсылка);
		КонецЕсли;
