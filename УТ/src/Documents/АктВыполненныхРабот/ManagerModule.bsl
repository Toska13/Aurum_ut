...
Функция СтруктураСостояниеВыполненияДокумента() Экспорт
	...
	СтруктураСостояние.ИмяРегистраОтгрузкаУслуг = "РаспоряженияНаОтгрузкуИВозврат";
...
// Формирует представление объекта на основном языке.
// 
// Возвращаемое значение:
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если НЕ ПраваПользователяПовтИсп.ЭтоПартнер()
		...
		КомандаПечати.Идентификатор = "АктВыполненныхРаботБезСкидок";
		КомандаПечати.Представление = НСтр("ru = 'Акт выполненных работ'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
		КомандаПечати.ОсновнаяПечатнаяФорма = Истина;
		КомандаПечати.НаименованиеПечатнойФормы = НСтр("ru = 'Акт выполненных работ'");
	КонецЕсли;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	СинонимМакетаАктаВыполненныхРабот = НСтр("ru = 'Акт выполненных работ'");
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыПечати, "ИспользоватьСинонимДляКомплекта", Ложь) Тогда
		СинонимМакетаАктаВыполненныхРабот = ПараметрыПечати.СинонимМакетаДляКомплекта;
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Акт") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "Акт", СинонимМакетаАктаВыполненныхРабот, СформироватьПечатнуюФормуАктОбОказанииУслуг(МассивОбъектов, ОбъектыПечати, ПараметрыПечати));
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктВыполненныхРабот") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "АктВыполненныхРабот", СинонимМакетаАктаВыполненныхРабот, СформироватьПечатнуюФормуАктОбОказанииУслуг(МассивОбъектов, ОбъектыПечати, ПараметрыПечати));
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктВыполненныхРаботБезСкидок") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "АктВыполненныхРаботБезСкидок", СинонимМакетаАктаВыполненныхРабот, СформироватьПечатнуюФормуАктОбОказанииУслуг(МассивОбъектов, ОбъектыПечати, ПараметрыПечати));
	КонецЕсли;
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктMicrosoftWord") Тогда
		ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "АктMicrosoftWord");
		Если ПечатнаяФорма <> Неопределено Тогда
			
			ИмяМакета = "ПФ_DOC_Акт" + "_" + ОбщегоНазначения.КодОсновногоЯзыка();
			МакетИДанныеОбъекта = УправлениеПечатью.МакетыИДанныеОбъектовДляПечати("Документ.АктВыполненныхРабот", 
				ИмяМакета, МассивОбъектов);
			
			ОфисныеДокументы = Новый Соответствие;
			
			Шаблон = НСтр("ru = '[Организация]-[Контрагент] Акт №[Номер] от [Дата]'");
			...
			Для Каждого Ссылка Из МассивОбъектов Цикл
				...
				АдресХранилищаОфисныйДокумент = УправлениеПечатьюУТСервер.ПечатьАктВыполненныхРаботMicrosoftWord(Ссылка, МакетИДанныеОбъекта, ИмяМакета);
...
Функция ПолучитьМакетыИОписанияСекций(знач МассивИменМакетов) Экспорт
	...
	ОписаниеСекций = Новый Соответствие;
	...
	Для Каждого ИмяМакета Из МассивИменМакетов Цикл
		...
		Если ИмяМакета = "ПФ_DOC_Акт" + "_" + ОбщегоНазначения.КодОсновногоЯзыка()  Тогда
...
Функция ДанныеДляПечатныхФормСчетаНаОплатуИзвещения(ПараметрыПечати, МассивОбъектов, КодЯзыка = Неопределено) Экспорт
	...
	|			ПРЕДСТАВЛЕНИЕ(Услуги.Номенклатура.ЕдиницаИзмерения)
	|		ИНАЧЕ Услуги.Номенклатура.ЕдиницаИзмерения.МеждународноеСокращение
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, ПараметрыЗаполнения, МассивЗаказов = Неопределено)
	...
	|		(РаспоряженияОбороты.КОформлениюСуммаОборот -
	...
	|		ИЛИ (РаспоряженияОбороты.КОформлениюСуммаОборот -
	...
	|		-РаспоряженияДвижения.КОформлениюСумма + РаспоряженияДвижения.ОформленоСумма
	|	ИЗ
	|		#РаспоряженияНаОтгрузкуДвиженияРазвернутая КАК РаспоряженияДвижения
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
...
Функция КомплектПечатныхФорм() Экспорт
	...
	Иначе
		РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "АктВыполненныхРаботБезСкидок", НСтр("ru = 'Акт выполненных работ'"),                     1);
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("НеИспользоватьСчетаНаОплатуНеВыбиратьВариантВыводаСкидок") Тогда
		РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "СчетНаОплатуБезСкидок", НСтр("ru = 'Счет на оплату'"), 1);
...
// 
// Параметры:
//  СсылкаОбъект - ДокументСсылка.АктВыполненныхРабот - Документ, по которому нужно получить данные.
...
Функция СуммыПоЗаказам(СсылкаОбъект, ЗначенияРеквизитов = Неопределено) Экспорт
	...
	Если ЗначенияРеквизитов = Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТаблицаЗначений", "Документ.АктВыполненныхРабот.Услуги");
