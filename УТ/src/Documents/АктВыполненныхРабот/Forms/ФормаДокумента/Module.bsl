...
Процедура ДоговорПриИзменении(Элемент)
	...
	Если Объект.АктПоЗаказам И Договор <> Объект.Договор И Не ИспользуютсяЗаказыКакСчета() Тогда
...
Процедура УслугиНоменклатураПриИзменении(Элемент)
	...
	ПроверкаОплатыБонуснымиБаллами();
	...
	Если ИспользоватьСоглашенияСКлиентами И ЗначениеЗаполнено(Объект.Соглашение) Тогда
		СтруктураДействий.Вставить("ЗаполнитьУсловияПродажУслуги", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияУсловийПродажВСтрокеТЧ(Объект));
	Иначе
...
Процедура УслугиСуммаПриИзменении(Элемент)
	...
	СтруктураПересчетаЦены  = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруПересчетаЦеныСкидкиВПродажахВТЧ(Объект, Ложь, "Количество");
	СтруктураПересчетаСуммы = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(Объект);
...
Процедура ПартнерПриИзмененииСервер()
	...
	ОтветственныеЛицаСервер.УстановитьМенеджера(
		Объект, ПродажиСервер.ДокументОснование(Объект, "АктПоЗаказам, ЗаказКлиента, Услуги.ЗаказКлиента"));
...
Процедура СоглашениеПриИзмененииСервер()
	...
	ОтветственныеЛицаСервер.УстановитьМенеджера(
		ДокументПродажи, ПродажиСервер.ДокументОснование(ДокументПродажи, "АктПоЗаказам, ЗаказКлиента, Услуги.ЗаказКлиента"));
...
Процедура СуммаДокументаПриИзмененииСервер(СуммаДокумента)
	...
	ПараметрыПересчета = Ценообразование.ПараметрыПересчетаСуммТаблицыПриИзмененииКурса(Объект.ЦенаВключаетНДС, СуммаДокумента, "Количество");
...
Процедура ДоговорПриИзмененииСервер()
	...
	Если ДанныеДоговора.ОплатаВВалюте <> Объект.ОплатаВВалюте Тогда
		Объект.БанковскийСчетОрганизации = Справочники.БанковскиеСчетаОрганизаций.ПустаяСсылка();
	КонецЕсли;
	Объект.ОплатаВВалюте = ДанныеДоговора.ОплатаВВалюте;
...
Процедура ПриИзмененииАктовВыполненныхРаботПоНесколькимЗаказам()
	...
	ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтаФорма, "АктПоЗаказам");
...
Процедура НалогообложениеНДСПриИзмененииСервер()
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Объект.Услуги, СтруктураДействий, КэшированныеЗначенияСлужебный);
...
&НаКлиенте
Процедура ПроверкаОплатыБонуснымиБаллами()
	
	Если Не ИспользоватьБонусныеПрограммыЛояльности
		Или Не ОплатаБонуснымиБаллами Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПрименитьОплатуБонуснымиБалламиКлиент(АдресВоВременномХранилище = Неопределено, ВыводитьСообщение = Ложь)
	...
	Если АдресВоВременномХранилище = Неопределено И (Не ОплатаБонуснымиБаллами Или Не Объект.Услуги.Количество()) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПрименитьОплатуБонуснымиБаллами(АдресВоВременномХранилище = Неопределено, ВыводитьСообщение = Ложь)
	...
	ОбрабатываемаяКоллекция = ?(УдалениеОплаты, ТаблицаТоваров, Объект.Услуги);
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	КэшированныеЗначения = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначенияУслуг();
	ПараметрыЗаполнитьСтавкуНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект);
	...
	НаборыСервер.ЗаполнитьСлужебныеРеквизиты(ЭтотОбъект, "Услуги");
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	Колонки = "Номенклатура,Характеристика,Упаковка,Количество,ВидЦены,Цена,
		|ПроцентРучнойСкидки,СуммаРучнойСкидки,Подразделение,НоменклатураНабора,ХарактеристикаНабора";
	...
	ДобавитьВСтруктуруДействияПриИзмененииКоличества(СтруктураДействий,Объект);
	Если ТаблицаУслуг.Колонки.Найти("СуммаРучнойСкидки") <> Неопределено Тогда
...
Процедура РассчитатьИтоговыеПоказателиАктовВыполненныхРабот(Форма)
	...
	Форма.СуммаНДС   = Форма.Объект.Услуги.Итог("СуммаНДС");
...
Процедура УстановитьДоступностьЭлементовПоСтатусуСервер()
	...
	МассивЭлементов.Добавить("УслугиСоставНабора");
	МассивЭлементов.Добавить("УслугиВставитьСтроки");
...
Процедура ПолучитьУслугиИзХранилища(АдресУслугВХранилище)
	...
	ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтотОбъект, "АктПоЗаказам");
...
Функция ПривязатьСтрокиАктовВыполненныхРаботКЗаказамКлиентов(ОтобратьПоЗаказу)
	...
	|		РаспоряженияДвижения.ОформленоКоличество
	|	ИЗ
...
Процедура ПолучитьЗагруженныеТоварыИзХранилища(АдресТоваровВХранилище)
	...
	Для Каждого СтрокаУслуга Из УслугиИзХранилища Цикл
		...
		ОбрабатываемыеСтроки.Добавить(СтрокаТЧУслуги);
	КонецЦикла;
