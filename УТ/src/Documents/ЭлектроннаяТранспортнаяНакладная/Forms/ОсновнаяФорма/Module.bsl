...
Процедура ЗаполнитьЗначенияПоУмолчаниюДляТекущегоТитула(ПриСозданииНаСервере = Ложь)
	...
	ИначеЕсли Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул3")
		...
		КонтейнерТитулГрузоотправителя = ОбменСГИСЭПДКлиентСервер.КонтейнерОтображаемойВерсииРеквизита(ЭтотОбъект, "ТитулГрузоотправителя");
		...
		ТитулГрузоотправителяДатаИВремяДоставки = ОбменСГИСЭПДКлиентСервер.ПолучитьЗначениеРеквизитаИзСтруктурыФормы(ЭтотОбъект,
														...
														ТекущаяДатаСеанса(),
														КонтейнерТитулГрузоотправителя);
		Если Не ЗначениеЗаполнено(ТитулГрузоотправителяДатаИВремяДоставки) Тогда
			ТитулГрузоотправителяДатаИВремяДоставки = ТекущаяДатаСеанса();
		КонецЕсли;
		...
		ТитулГрузоотправителяКодМетодаОпределенияМассыГруза = ОбменСГИСЭПДКлиентСервер.ПолучитьЗначениеРеквизитаИзСтруктурыФормы(ЭтотОбъект,
														"ТитулГрузоотправителяКодМетодаОпределенияМассыГруза",
														"03", КонтейнерТитулГрузоотправителя);
		
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(ЭтотОбъект, 
			ПрефиксДоп + "ТитулГрузополучателяКодМетодаОпределенияМассыГруза", 
			ТитулГрузоотправителяКодМетодаОпределенияМассыГруза, Ложь, КонтейнерОтображаемойВерсии);
			
			
		ТитулГрузоотправителяКоличествоГрузовыхМест = ОбменСГИСЭПДКлиентСервер.ПолучитьЗначениеРеквизитаИзСтруктурыФормы(ЭтотОбъект,
														"ТитулГрузоотправителяКоличествоГрузовыхМест",
														0, КонтейнерТитулГрузоотправителя);
		
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(ЭтотОбъект, 
			ПрефиксДоп + "ТитулГрузополучателяОбщееКоличествоПринятыхГрузовыхМест", 
			ТитулГрузоотправителяКоличествоГрузовыхМест, Ложь, КонтейнерОтображаемойВерсии);
			
		ТитулГрузоотправителяМассаБруттоГруза = ОбменСГИСЭПДКлиентСервер.ПолучитьЗначениеРеквизитаИзСтруктурыФормы(ЭтотОбъект,
														"ТитулГрузоотправителяМассаБруттоГруза",
														0, КонтейнерТитулГрузоотправителя);
		
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(ЭтотОбъект, 
			ПрефиксДоп + "ТитулГрузополучателяМассаБруттоПринятогоГруза", 
			ТитулГрузоотправителяМассаБруттоГруза, Ложь, КонтейнерОтображаемойВерсии);
		
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(ЭтотОбъект, 
			ПрефиксДоп + "ТитулГрузополучателяФактическоеСостояниеГруза", 
			"Груз принят в полном объёме, без повреждений", Ложь, КонтейнерОтображаемойВерсии, Истина);
...
Процедура ЗаполнитьТитулГрузоотправителяГрузы_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Объект.ЭтоКонтейнернаяПеревозка = ОбменСГИСЭПДКлиентСервер.ПолучитьЗначениеРеквизитаИзСтруктурыФормы(ЭтотОбъект, "ЭтоКонтейнернаяПеревозка", Ложь);
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("УникальныйИдентификаторОбновляемойФормы", ЭтотОбъект.УникальныйИдентификатор);
	ПараметрыОповещения.Вставить("ИмяКнопки", "ЗаполнитьТитулГрузоотправителяСопроводительныеДокументы");
