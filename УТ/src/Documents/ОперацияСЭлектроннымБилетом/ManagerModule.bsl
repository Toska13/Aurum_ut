...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Попытка 
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если ВыборкаДокументов.ТребуетсяСозданиеНовогоБронирования Тогда
					...
					ПричинаИсключения = "ПлохиеДанныеНовыйДокументБронирования";
					
					НовыйДокументБронирования = СоздатьНовыйДокументБронированияНаОснованииОперацииСЭлектроннымБилетом(ДокументОбъект);
		...
		Исключение
			...
			ИначеЕсли ПричинаИсключения = "ПлохиеДанныеНовыйДокументБронирования" Тогда
				
				НачатьТранзакцию();
				
				Попытка
					
					ДокументОбъект = ОбновлениеИнформационнойБазыУТ.ПроверитьПолучитьОбъект(ВыборкаДокументов.Ссылка,
						ВыборкаДокументов.ВерсияДанных, Параметры.Очередь);
					
					Если ДокументОбъект <> Неопределено Тогда 
						
						НовыйДокументБронирования = СоздатьНовыйДокументБронированияНаОснованииОперацииСЭлектроннымБилетом(ДокументОбъект, Ложь);
						
						ШаблонРекомендации = НСтр("ru = 'Перенесите вручную присоединенные файлы к документу ""%1"" (%2)
												|в документ ""%3"" (%4).
												|После чего документ ""%3"" (%4) необходимо будет провести,
												|а документу ""%1"" (%2) установить пометку удаления.'", ОбщегоНазначения.КодОсновногоЯзыка());
						Рекомендация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонРекомендации,
											ДокументОбъект.Ссылка, 
											ПолучитьНавигационнуюСсылку(ДокументОбъект.Ссылка),
											НовыйДокументБронирования.Ссылка,
											ПолучитьНавигационнуюСсылку(НовыйДокументБронирования.Ссылка));
						
						ОписаниеПроблемы = ОбновлениеИнформационнойБазыУТ.ПроблемаСДанными(ВыборкаДокументов.Ссылка,
							Рекомендация, ИнформацияОбОшибке());
							
						ОбновлениеИнформационнойБазыУТ.ЗаписатьПлохиеДанные(
							ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОписаниеПроблемы), ПроблемныхОбъектов, Параметры);
				...
				Исключение
					...
					ОбновлениеИнформационнойБазыУТ.ЗаписатьПлохиеДанные(
						ИсключенияПриОбновлении, ПроблемныхОбъектов, Параметры);
	...
	ОбновлениеИнформационнойБазыУТ.ЗаписатьПлохиеДанные(ИсключенияПриОбновлении, ПроблемныхОбъектов, Параметры);
...
// Создает новый документ бронирования на основании документа "Операция с электронным билетом".
// 
// Параметры:
//  ДокументОперацияСЭлектроннымБилетом - ДокументОбъект.ОперацияСЭлектроннымБилетом - Документ операции с электронным билетом
//  ВключитьБизнесЛогику - Булево - включает бизнес-логику при записи нового документа бронирования.
// 
// Возвращаемое значение:
//  ДокументОбъект.БронированиеSmartway - Документ бронирования Smartway
Функция СоздатьНовыйДокументБронированияНаОснованииОперацииСЭлектроннымБилетом(ДокументОперацияСЭлектроннымБилетом, ВключитьБизнесЛогику = Истина) 
	
	НовыйДокументБронирования = Документы.БронированиеSmartway.СоздатьДокумент();
	ЗаполнитьЗначенияСвойств(НовыйДокументБронирования, ДокументОперацияСЭлектроннымБилетом);
	
	НовыйДокументБронирования.НомерБронирования = ДокументОперацияСЭлектроннымБилетом.НомерБилета;
	НовыйДокументБронирования.ДатаНачала = ДокументОперацияСЭлектроннымБилетом.ДатаОтправления;
	НовыйДокументБронирования.ДатаОкончания = ДокументОперацияСЭлектроннымБилетом.ДатаПрибытия;
	НовыйДокументБронирования.ДеталиБронирования = ДокументОперацияСЭлектроннымБилетом.ДеталиПоездки;
	
	НовыйДокументБронирования.Операции.Загрузить(ДокументОперацияСЭлектроннымБилетом.Операции.Выгрузить());
	НовыйДокументБронирования.ОбменДанными.Загрузка = Не ВключитьБизнесЛогику;
	НовыйДокументБронирования.Записать(РежимЗаписиДокумента.Запись);
	
	Возврат НовыйДокументБронирования;
