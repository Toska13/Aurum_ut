...
Процедура ДоговорПриИзмененииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = Истина Тогда
		ЦеныРассчитаны = ДоговорПриИзмененииЗавершениеСервер();
...
Процедура ПодборПоОстаткам(Команда)
	...
	Если КомиссионныеПродажи25 Тогда
		...
		ПараметрыПроверки.Вставить("ПроверятьРаспроведенность", Ложь);
	КонецЕсли;
...
Процедура ЗаполнитьЦеныПоДоговору(Команда)
	...
	Если ЦеныПредприятияЗаполнениеКлиент.НеобходимоЗаполнениеЦенПоВиду(Объект, ВидЦеныКомиссионера)
		...
		ЦеныРассчитаны = ЗаполнитьЦеныПоВидуЦен("ВидЦенПродажи") Или ЦеныРассчитаны;
		ПродажиКлиент.ОповеститьОбОкончанииЗаполненияЦенПоСоглашению(ЦеныРассчитаны);
...
Процедура ОрганизацияПриИзмененииСервер()
	...
	ДенежныеСредстваСервер.ПроверитьЗаполнитьБанковскийСчетОрганизацииПоВладельцу(Объект.Организация,
		Объект.БанковскийСчет, Объект.ФормаОплаты, , Объект.ОплатаВВалюте);
...
Процедура НалогообложениеНДСПриИзмененииСервер()
	...
	УчетНДСУП.УстановитьВидимостьКомандыЗаполнитьСтавкуНДС(ЭтотОбъект);
	АктулизироватьСтавкаНДСВознаграждения();
	
КонецПроцедуры

&НаСервере
Процедура АктулизироватьСтавкаНДСВознаграждения() 
	...
	Объект.СуммаНДСВознаграждения = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(Объект.СуммаВознаграждения,
...
&НаСервере
Функция ДоговорПриИзмененииЗавершениеСервер()
...
Процедура СпособРасчетаВознагражденияПриИзмененииСервер()
	...
	УправлениеЭлементамиФормы();
	ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтотОбъект, "СпособРасчетаВознаграждения");
	ОбновитьТекстСчетаФактурыПолученные();
...
Функция ПоместитьТоварыВХранилищеССерией(Знач Товары, УникальныйИдентификатор)
	...
	АдресПлатежейВХранилище = ПоместитьВоВременноеХранилище(
		Товары.Выгрузить(,"НомерСтроки, Номенклатура, Характеристика, НомерГТД, Серия, Количество"),
		УникальныйИдентификатор);
...
Процедура ОбработкаВыбораПодборПереданныхТоваров(ВыбранноеЗначение, ЦеныРассчитаны)
	...
	СвернутьНомераГТД = Не ИспользоватьУчетПрослеживаемыхИмпортныхТоваров Или КомиссионерНеВедетУчетПоРНПТ;
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	ДополнитьОписаниеНастроекПересчетаРеквизитовТабличнойЧасти(ЭтотОбъект, СтруктураДействийСДобавленнымиСтроками);
	...
	ДополнитьОписаниеНастроекПересчетаРеквизитовТабличнойЧасти(ЭтотОбъект, СтруктураДействийСИзмененнымиСтроками);
...
Процедура ДобавитьКомандуОбновленияСтатусаОплатыСБП()
	...
	ПараметрыКоманды.МестоРасположения = Элементы.ЗачетОплатыФорма;
...
Процедура ПодборПоОстаткамЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если КомиссионныеПродажи25 Тогда
		...
		ОценкаПроизводительностиКлиент.ЗамерВремени(Истина,
			"Документ.ОтчетКомиссионера.ФормаДокумента.Команда.ПодобратьПереданныеТовары");
		...
		ПараметрыПодбора.Вставить("АдресТоваровНакладнойВХранилище", АдресПлатежейВХранилище);
...
Процедура УдержатьВознаграждениеПриИзмененииСервер()
	ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтотОбъект, "УдержатьВознаграждение");
	УправлениеЭлементамиФормы();
КонецПроцедуры
