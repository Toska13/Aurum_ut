...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Если Не УказанаСтатьяРасходовНаСебестоимостьПродаж() Тогда
...
Процедура ЗаполнитьУсловияПродажПоСоглашению(ПараметрОрганизация = Неопределено, КэшированныеЗначенияПараметров = Неопределено) Экспорт
	...
	// Иницииализация общих параметров для дальнейших расчетов цен:	
	ОбщиеПараметрыЗаполненияЦены = ЦеныПредприятияЗаполнениеСервер.НовыйПараметрыЗаполненияЗаполнитьЦены();
	ЗаполнитьЗначенияСвойств(ОбщиеПараметрыЗаполненияЦены, ЭтотОбъект);
	
	Если ЗначениеЗаполнено(КонецПериода) Тогда
		ОбщиеПараметрыЗаполненияЦены.Дата = КонецПериода;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПараметрОрганизация) Тогда
		ОбщиеПараметрыЗаполненияЦены.Организация = ПараметрОрганизация;
	КонецЕсли;
	
	СтруктураПересчетаСуммы = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(ЭтотОбъект);
	
	// Расчет цен передачи:
	ПараметрыЗаполнения = ОбщегоНазначения.СкопироватьРекурсивно(ОбщиеПараметрыЗаполненияЦены);
	ПараметрыЗаполнения.ПоляЗаполнения = "ВидЦены, Цена, СтавкаНДС";
	
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ПересчитатьСумму", "КоличествоУпаковок");
	СтруктураДействий.Вставить("ПересчитатьСуммуСНДС", СтруктураПересчетаСуммы);
	СтруктураДействий.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
	СтруктураДействий.Вставить("ЗаполнитьСтавкуНДСПоВидуЦен", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДСПоВидуЦен(ЭтотОбъект));
	
	Если УсловияПродаж.НоваяСхемаКомиссии Тогда
		// Вид цены в "Новой комиссии" используется из договора 
		ПараметрыЗаполнения.ВидЦены = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "ВидЦенУчетный");
	КонецЕсли;
	...
	// Расчет цен продажи:
	Если УсловияПродаж.НоваяСхемаКомиссии Тогда 
		ПараметрыЗаполнениеПродажи = ОбщегоНазначения.СкопироватьРекурсивно(ОбщиеПараметрыЗаполненияЦены);
		ПараметрыЗаполнениеПродажи.ПоляЗаполнения = "ВидЦеныПродажи, ЦенаПродажи";
		ПараметрыЗаполнениеПродажи.ПолеЦена = "ЦенаПродажи";
		ПараметрыЗаполнениеПродажи.ПолеВидЦены = "ВидЦеныПродажи";
		ПараметрыЗаполнениеПродажи.ВидЦены = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "ВидЦенПродажи");
...
Функция УказанаСтатьяРасходовНаСебестоимостьПродаж()
	
	Результат = Истина;
	
	Если ЗначениеЗаполнено(СтатьяРасходов) Тогда
		
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтатьяРасходов,
			"ВариантРаспределенияРасходовРегл, ВариантРаспределенияРасходовУпр, ВариантРаспределенияРасходовНУ, КонтролироватьЗаполнениеАналитики");
		
		НаСебестоимостьПродаж = Перечисления.ВариантыРаспределенияРасходов.НаСебестоимостьПродаж;
		
		Если Реквизиты.КонтролироватьЗаполнениеАналитики
			Или Реквизиты.ВариантРаспределенияРасходовРегл <> НаСебестоимостьПродаж
			Или Реквизиты.ВариантРаспределенияРасходовУпр <> НаСебестоимостьПродаж
			Или Реквизиты.ВариантРаспределенияРасходовНУ <> НаСебестоимостьПродаж Тогда
...
Процедура ЗаполнитьДопКолонкиВидовЗапасов() Экспорт
	...
	Для Каждого СтрокаТоваров Из ТаблицаТовары Цикл
		...
		Для Каждого СтрокаЗапасов Из ВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			...
			Если СтрокаЗапасов.СуммаСНДС <> 0
				...
				НоваяСтрока.СуммаНДСВознаграждения	= СтрокаЗапасов.СуммаНДСВознаграждения + НоваяСтрока.СуммаНДСВознаграждения;
	...
	ИменаКолонокКРаспределению		= "АналитикаУчетаНоменклатуры, СтавкаНДС, СуммаНДС, СуммаСНДС, СуммаВознаграждения,
										|СуммаНДСВознаграждения, Количество";
	СоответствиеСуммКРаспределению	= Новый Соответствие;
	...
	Для Каждого СтрокаТоваров Из ТаблицаТовары Цикл
		...
		Для Каждого СтрокаЗапасов Из ВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			...
			КлючКэша = КлючКэшаСуммКРаспределению(СтрокаЗапасов, Знак);
			СуммыКРаспределению = СоответствиеСуммКРаспределению.Получить(КлючКэша);
			
			Если СуммыКРаспределению <> Неопределено Тогда
				СтрокаЗапасов.СуммаНДС					= СтрокаЗапасов.СуммаНДС + СуммыКРаспределению.СуммаНДС;
				СтрокаЗапасов.СуммаСНДС					= СтрокаЗапасов.СуммаСНДС + СуммыКРаспределению.СуммаСНДС;
				СтрокаЗапасов.СуммаВознаграждения		= СтрокаЗапасов.СуммаВознаграждения + СуммыКРаспределению.СуммаВознаграждения;
				СтрокаЗапасов.СуммаНДСВознаграждения	= СтрокаЗапасов.СуммаНДСВознаграждения + СуммыКРаспределению.СуммаНДСВознаграждения;
				
				СоответствиеСуммКРаспределению.Удалить(КлючКэша);
			КонецЕсли;
			...
			Если СтрокаЗапасов.СуммаСНДС <> 0
				...
				Если СуммыКРаспределению = Неопределено Тогда
					СуммыКРаспределению = Новый Структура(ИменаКолонокКРаспределению);
					ЗаполнитьЗначенияСвойств(СуммыКРаспределению, СтрокаЗапасов);
					
					СуммыКРаспределению.Количество = Количество;
					
					СоответствиеСуммКРаспределению.Вставить(КлючКэша, СуммыКРаспределению);
				Иначе
					СуммыКРаспределению.СуммаНДС				= СтрокаЗапасов.СуммаНДС + СуммыКРаспределению.СуммаНДС;
					СуммыКРаспределению.СуммаСНДС				= СтрокаЗапасов.СуммаСНДС + СуммыКРаспределению.СуммаСНДС;
					СуммыКРаспределению.СуммаВознаграждения		= СтрокаЗапасов.СуммаВознаграждения + СуммыКРаспределению.СуммаВознаграждения;
					СуммыКРаспределению.СуммаНДСВознаграждения	= СтрокаЗапасов.СуммаНДСВознаграждения + СуммыКРаспределению.СуммаНДСВознаграждения;
				КонецЕсли;
	...
	Для Каждого КлючИЗначение Из СоответствиеСуммКРаспределению Цикл
		СуммыКРаспределению = КлючИЗначение.Значение;
		
		ЗаполнитьЗначенияСвойств(СтруктураПоиска, СуммыКРаспределению);
		
		НайденныеСтроки	= ВидыЗапасов.НайтиСтроки(СтруктураПоиска);
		Индекс			= НайденныеСтроки.ВГраница();
		
		Пока Индекс >= 0 Цикл
			СтрокаЗапасов = НайденныеСтроки[Индекс];
			
			Если (СтрокаЗапасов.Количество > 0
					И СуммыКРаспределению.Количество < 0)
				Или (СтрокаЗапасов.Количество < 0
					И СуммыКРаспределению.Количество > 0) Тогда
...
Функция КлючКэшаСуммКРаспределению(ТекущаяСтрока, Знак)
	
	КлючКэша		= "";
	ПустоеЗначение	= "ПустоеЗначение";
	
	КлючКэша = ?(ЗначениеЗаполнено(ТекущаяСтрока.АналитикаУчетаНоменклатуры),
				Строка(ТекущаяСтрока.АналитикаУчетаНоменклатуры.УникальныйИдентификатор()),
				ПустоеЗначение);
	КлючКэша = КлючКэша + ?(ЗначениеЗаполнено(ТекущаяСтрока.СтавкаНДС),
							Строка(ТекущаяСтрока.СтавкаНДС.УникальныйИдентификатор()),
							ПустоеЗначение);
	КлючКэша = КлючКэша + Строка(Знак);
