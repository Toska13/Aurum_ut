...
Функция ПараметрыВыбораСтатейИАналитик(Объект) Экспорт
	...
	ПараметрыВыбора.ДоступностьПоОперации = (НеобходимоРаспределениеНДСПоТоварам
											 И ЭтоКонецКвартала
											 ИЛИ НЕ Объект.СписатьНДСПоРасходамАктивамНаСтатьиОтражения);
	
	Если Объект.Исправление Тогда 
		
		ДатаИсправляемогоРаспределения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ИсправляемыйДокумент, "Дата");
		НеобходимоРаспределениеНДСПоТоварамВПрошломПериоде = Документы.РаспределениеНДС.НеобходимоРаспределениеНДСПоТоварам(Объект.Организация, ДатаИсправляемогоРаспределения); 
		ЭтоИсправлениеКонцаКвартала = (Месяц(ДатаИсправляемогоРаспределения)%3 = 0);
		ПараметрыВыбора.ДоступностьПоОперации = (НеобходимоРаспределениеНДСПоТоварамВПрошломПериоде
											 И ЭтоИсправлениеКонцаКвартала
											 ИЛИ НЕ Объект.СписатьНДСПоРасходамАктивамНаСтатьиОтражения);
...
//	Организация - Массив из СправочникСсылка.Организации, СправочникСсылка.Организации, Неопределено - отбор организаций, по которым создаются документы.
...
Функция РаспределитьНДС(Период, Организация, ИмяСобытияЖР = "", МенеджерВТКэшейРегистров = Неопределено, ИдетРасчетСебестоимости = Ложь) Экспорт
	...
	|ПОМЕСТИТЬ ДокументыРаспределенияИсходные
	|ИЗ
	...
	|			И НЕ РаспределениеНДСПроведен.Исправление
	...
	|			И НЕ РаспределениеНДССоздан.Исправление
	...
	|ПОМЕСТИТЬ ДокументыРаспределенияАктуальные
	|ИЗ
	...
	|					Т.Документ
	|				ИЗ
	|					ДокументыРаспределенияИсходные КАК Т)) КАК ИсправлениеДокументов
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДокументыРаспределенияИсходные.Организация КАК Организация,
	|	ВЫБОР
	|		КОГДА НЕ АктуальныеДокументы.ИсправляемыйДокумент ЕСТЬ NULL
	|			ТОГДА АктуальныеДокументы.АктуальныйДокумент
	|		ИНАЧЕ ДокументыРаспределенияИсходные.Документ
	|	КОНЕЦ КАК Документ
	|ИЗ
	|	ДокументыРаспределенияИсходные КАК ДокументыРаспределенияИсходные
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДокументыРаспределенияАктуальные КАК АктуальныеДокументы
	|		ПО ДокументыРаспределенияИсходные.Документ = АктуальныеДокументы.ИсправляемыйДокумент
	|			И ЕСТЬNULL(АктуальныеДокументы.СторнированДокумент, ЛОЖЬ) = ЛОЖЬ";
	...
	Пока Выборка.Следующий() Цикл 
		...
		Попытка
			...
			Если НЕ ЗначениеЗаполнено(Выборка.Документ) Тогда
				...
				СтруктураИсправления = Новый Структура;
				СтруктураИсправления.Вставить("Исправление", Ложь);
				СтруктураИсправления.Вставить("ИсправляемыйДокумент", Неопределено);
				Документы.РаспределениеНДС.УстановитьНачалоПериода(ДокументОбъект.НачалоПериода, Период, Выборка.Организация, СтруктураИсправления);
...
// 
// Возвращаемое значение:
//  Соответствие Из ПеречислениеСсылка.ТипыНалогообложенияНДС, Строка - соответствие реквизитов выручки типам налогообложения НДС:
//  * Ключ - ПеречислениеСсылка.ТипыНалогообложенияНДС - типы налогообложения НДС.
//  * Значение - Строка - наименование реквизита
//
...
//
// Возвращаемое значение:
//  Соответствие Из ПеречислениеСсылка.ТипыНалогообложенияНДС, Строка - соответствие статей аналитик расходов
//                                                                      налогообложениям НДС.
//
...
Функция РеквизитыДокумента() Экспорт
	...
	РеквизитыДокумента.Добавить("Исправление");
	РеквизитыДокумента.Добавить("СторнируемыйДокумент");
	РеквизитыДокумента.Добавить("ИсправляемыйДокумент");
...
Функция ПолучитьДанныеДокумента(ДокументСсылка) Экспорт
	...
	|	ДанныеДокумента.Дата							КАК ДатаРаспределения,
	|	ЕСТЬNULL(ИсправляемыйДокументСсылка.Дата, ДанныеДокумента.Дата)		КАК Период,
	|	ДанныеДокумента.НачалоПериода					КАК НачалоПериода,
	|	КОНЕЦПЕРИОДА(ЕСТЬNULL(ИсправляемыйДокументСсылка.Дата, ДанныеДокумента.Дата), МЕСЯЦ)	КАК ПериодБазы,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеНДС КАК ИсправляемыйДокументСсылка
	|		ПО ДанныеДокумента.ИсправляемыйДокумент = ИсправляемыйДокументСсылка.Ссылка
	...
	Реквизиты.Вставить("ИдентификаторМетаданных", ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Документ.РаспределениеНДС"));
...
Функция ДанныеДляРаспределения(Реквизиты, ДопПараметры)
	...
	Запрос.УстановитьПараметр("РаспределениеЗаПолныйПериод", Реквизиты.РаспределениеЗаПолныйПериод); // Условие конца квартала.
	...
	|	РегистрНакопления.ПартииНДСКРаспределению КАК ПартииНДСКРаспределению
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.ВнутреннееПотребление КАК КорректировкаПриобретения
	|	ПО
	|		ПартииНДСКРаспределению.Регистратор = КорректировкаПриобретения.Ссылка
	|		И КорректировкаПриобретения.Исправление
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеОснованияКорректировки
	|	ПО
	...
	|		И Т.Регистратор <> &ДокументСсылка
	|		И ВЫБОР
	|			КОГДА ТИПЗНАЧЕНИЯ(Т.Регистратор) <> ТИП(Документ.РаспределениеНДС)
	|				ТОГДА ИСТИНА
	|			КОГДА &Исправление
	|				ТОГДА Т.Регистратор <> &ИсправляемыйДокумент
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	...
	НастройкиУчетаНДС = НастройкиНалоговУчетныхПолитик.ДействующиеПараметрыНалоговУчетныхПолитикНаДату(
						"НастройкиУчетаНДС", Реквизиты.Организация, Реквизиты.Период);
	Запрос.УстановитьПараметр("СтатьяОтраженияРасходов", НастройкиУчетаНДС.СтатьяРасходовНеНДС);
	Запрос.УстановитьПараметр("АналитикаОтраженияРасходов", НастройкиУчетаНДС.АналитикаРасходовНеНДС);
	//-- Локализация
	
	Запрос.Текст =
	// Распределение НДС по экспортным операциям
	"ВЫБРАТЬ
	|	ТаблицаОстатков.Организация                      КАК Организация,
	|	ТаблицаОстатков.Подразделение                    КАК Подразделение,
	|	ТаблицаОстатков.НаправлениеДеятельности          КАК НаправлениеДеятельности,
	...
	|	ПО
	|		ТаблицаОстатков.АналитикаРасходов = ПартииНДСКРаспределениюКорректировка.АналитикаРасходов
	|		И ТаблицаОстатков.ДокументПоступленияРасходов = ПартииНДСКРаспределениюКорректировка.ДокументПоступленияРасходов
...
Функция СформироватьДвижения(Реквизиты, Регистры, ДопПараметры)
	...
	Если ВыполнятьРаспределение Тогда
		...
		РезультатРаспределения = РаспределитьПартииНДСПоБазе(ДанныеДляРаспределения, Реквизиты, ДопПараметры);
	...
	ТаблицыСторно = ПроведениеДокументов.ИнициализироватьДанныеДокументаДляПроведения(Запрос, ТекстыЗапроса, ДопПараметры);
	
	Для Каждого ТекТаблица Из ТаблицыСторно Цикл 
		
		Если ТекТаблица.Ключ = "ТаблицаНДСЗаписиКнигиПродаж"
			Или ТекТаблица.Ключ = "ТаблицаНДСЗаписиКнигиПокупок" Тогда 
			Продолжить;
		КонецЕсли;
		
		Если ТаблицыДляДвижений.Свойство(ТекТаблица.Ключ) Тогда 
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТекТаблица.Значение, ТаблицыДляДвижений[ТекТаблица.Ключ]);
		Иначе 
			ТаблицыДляДвижений.Вставить(ТекТаблица.Ключ, ТекТаблица.Значение);
		КонецЕсли;
...
Функция РаспределитьПартииНДСПоБазе(ДанныеДляРаспределения, Реквизиты, ДопПараметры)
	
	ПартииНДСКРаспределению = ДанныеДляРаспределения.ПартииНДСКРаспределению;
	...
	БазаРаспределения = БазаРаспределения(ПартииНДСКРаспределению, Реквизиты, ДопПараметры);
		...
		Иначе
			...
			Если Таблица = Неопределено Тогда
				ТекстСообщения = СтрШаблон(
					НСтр("ru = 'Не определено налогообложение по внеоборотному активу %1.'"), 
	...
	РаспределениеНДСЛокализация.РассчитатьВосстановлениеНДСИСписаниеНаРасходы(
		ТаблицыРаспределения, ДанныеДляРаспределения, Реквизиты);
	...
	Для Каждого КлючИЗначение Из ТаблицыРаспределения Цикл
		...
		Для Каждого Строка Из Таблица Цикл
			...
			Если ВидыДеятельностиНДСПринимаетсяКВозмещению.Найти(Строка.ВидДеятельностиНДС) <> Неопределено
				...
				Строка.НастройкаХозяйственнойОперации = Справочники.НастройкиХозяйственныхОпераций.ВключениеИсключениеНДСВСтоимости;
...
Функция БазаРаспределения(ПартииНДСКРаспределению, Реквизиты, ДопПараметры)
	...
	|
	|////////////////////////////////////////////////////////////////////
	// выбираем последнее исправление Распределения НДС за этот период, по которому не было сторно
	|ВЫБРАТЬ
	|	ПериодыБазы.ПериодБазы КАК ПериодБазы,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДокументыРаспределенияИсходные.ПериодБазы КАК ПериодБазы,
	|	ВЫБОР
	|		КОГДА НЕ АктуальныеДокументы.ИсправляемыйДокумент ЕСТЬ NULL
	|			ТОГДА АктуальныеДокументы.АктуальныйДокумент
	|		ИНАЧЕ ДокументыРаспределенияИсходные.ДокументРаспределения
	|	КОНЕЦ КАК Документ,
	|	ДокументыРаспределенияИсходные.ДокументРаспределения КАК ИсправляемыйДокумент
	|ПОМЕСТИТЬ ДокументыРаспределения
	|ИЗ
	|	ДокументыРаспределенияИсходные КАК ДокументыРаспределенияИсходные
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДокументыРаспределенияАктуальные КАК АктуальныеДокументы
	|		ПО ДокументыРаспределенияИсходные.ДокументРаспределения = АктуальныеДокументы.ИсправляемыйДокумент
	|ГДЕ
	...
	|	ПО
	|		ПериодыБазы.ПериодБазы = ДокументыРаспределения.ПериодБазы
	|		И ДокументыРаспределения.ПериодБазы <> &ПериодБазы
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		Документ.РаспределениеНДС КАК РаспределениеНДС
	|	ПО
	|		ДокументыРаспределения.Документ = РаспределениеНДС.Ссылка
	|";
	...
	Запрос.УстановитьПараметр("ВызовИзЗакрытияМесяца", ДопПараметры <> Неопределено И ДопПараметры.Свойство("РегламентныеОперации"));
...
Функция ТаблицаПартииНДСКРаспределению(РезультатРаспределения, Реквизиты)
	...
	ПартииНДСКРаспределению.ЗаполнитьЗначения(Реквизиты.ДатаРаспределения,   "Период");
...
Функция ТаблицаПрочиеРасходы(РезультатРаспределения, Реквизиты, ПартииНДСКРаспределению)
	...
	МассивТекстовЗапросов.Добавить(РегистрыНакопления.ПрочиеРасходы.ТекстЗапросаТаблицаВтПрочиеРасходы("Сторно"));
	МассивТекстовЗапросов.Добавить(РегистрыНакопления.ПрочиеРасходы.ТекстЗапросаТаблицаПрочиеРасходы("Сторно"));
...
Процедура ПрочиеРасходыДобавитьСтрокиВключениеВСтоимость(ПрочиеРасходы, ТаблицаИсточник, Реквизиты)
	...
	Для каждого Строка Из ТаблицаИсточник Цикл
		...
		Если ТипыНалогообложенияНДСНеоблагаемые.Найти(Строка.НалогообложениеНДСПартии) <> Неопределено
			Или Строка.КорВидДеятельностиНДС = Перечисления.ТипыНалогообложенияНДС.РеализацияРаботУслугНаЭкспорт Тогда
				// Уже в расходах
				Продолжить;
		КонецЕсли;
		...
		НоваяСтрока.Период					= Реквизиты.ДатаРаспределения;
...
Функция ТаблицаРеестрДокументов(Реквизиты)
	...
	Запрос.УстановитьПараметр("ИдентификаторМетаданных", Реквизиты.ИдентификаторМетаданных);
	...
	Если Таблица.Количество() > 0 Тогда
		...
		Таблица.ЗаполнитьЗначения(ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Таблица[0].НомерПервичногоДокумента),
...
//  * ВыручкаНДС - Число
//  * ВыручкаНДС0 - Число
//  * ВыручкаНДС0Космос -  Число
//  * ВыручкаНДС0НесырьевыеТовары - Число
//  * ВыручкаНДС0СырьевыеТоварыУслуги - Число
//  * ВыручкаНДСПроизводствоСДЦ - Число
//  * ВыручкаНеНДС - Число
//  * ВыручкаНеРФ - Число
//  * ДокументыЭкспорт - ТаблицаЗначений - документы отгрузки с суммой выручки и ее корректировки:
//    ** Документ - ДокументСсылка - документ отгрузки.
//    ** СуммаВыручки - Число - сумма выручки.
//    ** КорректировкаВыручки - Число - сумма корректировки выручки.
//
...
Процедура УстановитьНачалоПериода(НачалоПериода, Дата, Организация, СтруктураИсправления) Экспорт
	
	Если СтруктураИсправления.Исправление
