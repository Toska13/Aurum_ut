...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.РаспределениеНДС") Тогда 
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ БазаРаспределенияУстанавливаетсяВручную Тогда
		...
		ДополнительныеПараметрыПолученияВыручки.Вставить("ДатаРаспределения", Дата);
				
		Результат = Документы.РаспределениеНДС.ПолучитьБазуРаспределенияНДС(
						?(Исправление, ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИсправляемыйДокумент, "Дата"), Дата),
						НачалоПериода,
						Организация,
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	КонецЕсли;
	Если ДополнительныеСвойства.Свойство("РегламентныеОперации") Тогда 
		ДопПараметры.Вставить("РегламентныеОперации", ДополнительныеСвойства.РегламентныеОперации);
	КонецЕсли;
