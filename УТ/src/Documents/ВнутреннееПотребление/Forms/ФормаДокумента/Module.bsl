...
&НаКлиенте
Процедура НаправлениеДеятельностиРасходовУказаноВСпискеПриИзменении(Элемент)

	Объект.НаправлениеДеятельностиРасходовУказаноВСписке = (НаправлениеДеятельностиРасходовУказаноВСписке = 1);
...
&НаКлиенте
Процедура ЗаполнитьНаправлениеДеятельности(Команда)
	...
	Если СписокСтрок.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Необходимо выбрать строки, в которых требуется заполнить направление деятельности'"));
		Возврат;
	КонецЕсли;
	
	ПараметрыФормыВыбора = ОбщегоНазначенияУТКлиентСервер.ПараметрыВыбора(Элементы.ТоварыНаправлениеДеятельности, ЭтотОбъект);

	ОписаниеОповещения = Новый ОписаниеОповещения("ЗаполнитьНаправлениеДеятельностиЗавершение", ЭтотОбъект, СписокСтрок);
	
	ОткрытьФорму("Справочник.НаправленияДеятельности.ФормаВыбора", 
...
Процедура НастроитьЗависимыеЭлементыФормыНаКлиентеНаСервере(Форма, ИзмененныеРеквизиты = "")
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("НаправлениеДеятельностиРасходовУказаноВСписке")
		Или ОбновитьВсе Тогда
		
		Элементы.ТоварыНаправлениеДеятельности.Видимость = Объект.НаправлениеДеятельностиРасходовУказаноВСписке;
		Элементы.ТоварыЗаполнитьНаправлениеДеятельности.Видимость = Объект.НаправлениеДеятельностиРасходовУказаноВСписке;
...
Процедура ИзменитьТабличнуюЧастьПоРезультатамПроверки(ВозвращаемыеПараметры, КэшированныеЗначения)
	...
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействийНайденныеСтроки);
	СтруктураДействийНайденныеСтроки.Вставить("ЗаполнитьПризнакАртикул", Новый Структура("Номенклатура", "Артикул"));
	СтруктураДействийНайденныеСтроки.Вставить("ПриИзмененииТипаНоменклатуры", Новый Структура("ЕстьРаботы, ЕстьОтменено", Истина, Ложь));

	СтруктураДействийНовыеСтроки = Новый Структура;
	
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействийНовыеСтроки);
	СтруктураДействийНовыеСтроки.Вставить("ЗаполнитьПризнакАртикул", Новый Структура("Номенклатура", "Артикул"));
	СтруктураДействийНовыеСтроки.Вставить("ПриИзмененииТипаНоменклатуры", Новый Структура("ЕстьРаботы, ЕстьОтменено", Истина, Ложь));
	ПакетнаяОбработкаТабличнойЧастиКлиентСерверЛокализация.ДополнитьСтруктуруДействийПриИзмененииЭлемента(
		ЭтотОбъект, "Номенклатура", СтруктураДействийНовыеСтроки);
		
	ОбрабатываемыеНовыеСтроки = Новый Массив;
	ОбрабатываемыеНайденныеСтроки = Новый Массив;
	...
	Для Каждого СтрокаИсточник Из ТаблицаТовары Цикл
		...
		Для Каждого СтрокаТЧ Из НайденныеСтроки Цикл
			...
			Иначе
				ОбрабатываемыеНайденныеСтроки.Добавить(СтрокаТЧ);
			КонецЕсли;
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		...
		СтруктураДействийНовыеСтроки,
		Объект.Товары,
		КэшированныеЗначения);
		
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		ОбрабатываемыеНайденныеСтроки,
		СтруктураДействийНайденныеСтроки,
		Объект.Товары,
		КэшированныеЗначения);

	Для Каждого СтрокаТЧ Из ОбрабатываемыеНовыеСтроки Цикл
		ПриДобавлениСтрокиТоваровНаСервере(СтрокаТЧ);
	КонецЦикла;
	
	Для Каждого СтрокаТЧ Из ОбрабатываемыеНайденныеСтроки Цикл
		ПриДобавлениСтрокиТоваровНаСервере(СтрокаТЧ);
	КонецЦикла;
...
Процедура ПроверитьКоличествоВДокументеЗавершение(Результат, ДополнительныеПараметры) Экспорт
    ...
    Если ВозвращаемыеПараметры <> Неопределено Тогда
        ...
        ТекстОснования = НСтр("ru = 'введенного на основании распоряжения или недоступна роль ""Внутреннее потребление сверх заказа"".'");
...
Процедура ОбработкаПодбораТоваровИзЗаказа(АдресВХранилище)
	...
	МассивЗаказов = Таблица.ВыгрузитьКолонку("ЗаказНаВнутреннееПотребление");
	НаправлениеДеятельностиРасходовУказаноВСпискеДляЗаказов = Документы.ВнутреннееПотребление.ИнцициализироватьНаправленияДеятельностиРасходов(МассивЗаказов);
	
	Если Объект.Товары.Количество() = 0 Тогда
		Объект.НаправлениеДеятельностиРасходовУказаноВСписке = НаправлениеДеятельностиРасходовУказаноВСпискеДляЗаказов
	Иначе
		Объект.НаправлениеДеятельностиРасходовУказаноВСписке = НаправлениеДеятельностиРасходовУказаноВСпискеДляЗаказов Или НаправлениеДеятельностиРасходовУказаноВСписке = 1
	КонецЕсли;		
	НаправлениеДеятельностиРасходовУказаноВСписке = ?(Объект.НаправлениеДеятельностиРасходовУказаноВСписке, 1, 0);
	...
	Для Каждого СтрокаТаблицы Из Таблица Цикл
		...
		Если НаправлениеДеятельностиРасходовУказаноВСписке = 1 И Не НаправлениеДеятельностиРасходовУказаноВСпискеДляЗаказов Тогда
			СтрокаТаблицы.НаправлениеДеятельности = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТаблицы.ЗаказНаВнутреннееПотребление, "НаправлениеДеятельности");
		КонецЕсли;
	...
	НастроитьЗависимыеЭлементыФормыНаСервере("ХозяйственнаяОперация,Товары,НаправлениеДеятельностиРасходовУказаноВСписке");
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	НаправленияДеятельностиКлиентСервер.СтруктураДействийВставитьПриДобавленииСтроки(ЭтотОбъект, СтруктураДействийСИзмененнымиСтроками);
...
Процедура ОбработатьШтрихкодыСервер(СтруктураПараметровДействия, КэшированныеЗначения)
	...
	Для Каждого Строка Из Объект.Товары Цикл
		ПриДобавлениСтрокиТоваровНаСервере(Строка);
		ОбновитьОтклоненияОтЗаказаВСтроке(Строка, Объект.ПотреблениеПоЗаказам);
	КонецЦикла;
...
Функция ПривязатьСтрокиКЗаказамНаВнутреннееПотребление(ОтобратьПоЗаказу = Ложь)
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказНаВнутреннееПотребление) Тогда
		...
		Текст =
			...
			|			ТОГДА ТаблицаЗаказы.ЗаказНаВнутреннееПотребление.Назначение
			|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
			|	КОНЕЦ КАК Назначение,
			|	ЗаказТовары.ФизическоеЛицо КАК ФизическоеЛицо,
			|	ЗаказТовары.КатегорияЭксплуатации КАК КатегорияЭксплуатации,
			|	ВЫБОР 
			|		КОГДА ЗаказТовары.Ссылка.НаправлениеДеятельностиРасходовУказаноВСписке ТОГДА
			|			ЗаказТовары.НаправлениеДеятельности
			|		ИНАЧЕ
			|			ЗаказТовары.Ссылка.НаправлениеДеятельности
	...
	Текст =
		...
		|	ЗаказыНаВнутреннееПотребление.НаправлениеДеятельности КАК НаправлениеДеятельности
		...
		|	ЗаказыНаВнутреннееПотребление.НаправлениеДеятельности
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|			И Заказы.НаправлениеДеятельности = ДублиТоваров.НаправлениеДеятельности
		|
		|УПОРЯДОЧИТЬ ПО
		|	Номенклатура,
		|	Характеристика,
	...
	ТаблицаНераспределенныхТоваров.Колонки.Добавить("НаправлениеДеятельности");
	
	ТаблицаТоваров = Объект.Товары.Выгрузить();
	Если Не Объект.НаправлениеДеятельностиРасходовУказаноВСписке Тогда
		ТаблицаТоваров.Колонки.Добавить("НаправлениеДеятельностиЗаказа");
		ТаблицаТоваров.ЗаполнитьЗначения(Объект.НаправлениеДеятельности, "НаправлениеДеятельностиЗаказа");
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Объект.НаправлениеДеятельностиРасходовУказаноВСписке Тогда
			СтруктураПоиска.Вставить("НаправлениеДеятельности", Выборка.НаправлениеДеятельности);
		Иначе
			СтруктураПоиска.Вставить("НаправлениеДеятельностиЗаказа", Выборка.НаправлениеДеятельности);
		КонецЕсли;
	...
	Если ТаблицаНераспределенныхТоваров.Количество() > 0 Тогда
		...
		СтрокиТовары = ТаблицаТоваров.НайтиСтроки(СтруктураПоиска);
		...
		Пока Индекс < ВсегоСтрокДляОбработки Цикл
			...
			Если Объект.НаправлениеДеятельностиРасходовУказаноВСписке Тогда
				СтруктураПоиска.Вставить("НаправлениеДеятельности", СтрокаТовары.НаправлениеДеятельности);
			Иначе
				СтруктураПоиска.Вставить("НаправлениеДеятельностиЗаказа", СтрокаТовары.НаправлениеДеятельностиЗаказа);
			КонецЕсли;
	...
	// Удаляем служебную колонку, добавленную для поиска в случае, когда направление деяетельности расходов 
	// совпадает с направлением деяетельности запасов
	Если ТаблицаТоваров.Колонки.Найти("НаправлениеДеятельностиЗаказа") <> Неопределено Тогда
		ТаблицаТоваров.Колонки.Удалить("НаправлениеДеятельностиЗаказа");
	КонецЕсли;
...
Процедура ПерезаполнитьПоОрдерам(МассивЗаказов)
	...
	Документы.ВнутреннееПотребление.ЗаполнитьПоЗаказамОрдерам(ТаблицаНакладная, Объект.Ссылка, ПараметрыЗаполнения, Объект.ШтрихкодыУпаковок);
...
&НаКлиенте
Процедура ЗаполнитьНаправлениеДеятельностиЗавершение(Результат, СписокСтрок) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого ИдентификаторСтроки Из СписокСтрок Цикл
		ДанныеСтроки = Объект.Товары.НайтиПоИдентификатору(ИдентификаторСтроки);
		ДанныеСтроки.НаправлениеДеятельности = Результат;
	КонецЦикла;
...
Процедура ЗаполнитьПоЗаказуСервер()
	...
	ДанныеОтбора.Вставить("НаправлениеДеятельностиРасходовУказаноВСписке", ?(НаправлениеДеятельностиРасходовУказаноВСписке = 1, Истина, Ложь));
	...
	ЗаполнитьНаправлениеДеятельностиРасходовУказаноВСписке(РезультатЗапроса.ТаблицаОформить);
	ЗаполнитьСлужебныеРеквизитыТабличнойЧасти();
	ЗаполнитьСтатусыУказанияСерийСервер();
	
	НастроитьЗависимыеЭлементыФормыНаСервере("Товары, НаправлениеДеятельностиРасходовУказаноВСписке");
...
&НаСервере
Процедура ЗаполнитьНаправлениеДеятельностиРасходовУказаноВСписке(ТаблицаТоваров) Экспорт
	
	Если ТаблицаТоваров.Количество() = 0 Тогда
		Возврат;
	КонецЕсли; 
	
	НаправлениеДеятельностиРасходовУказаноВСписке = ?(ТаблицаТоваров[0].НаправлениеДеятельностиРасходовУказаноВСписке, 1, 0);
	Объект.НаправлениеДеятельностиРасходовУказаноВСписке = ТаблицаТоваров[0].НаправлениеДеятельностиРасходовУказаноВСписке;
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		ТекущаяСтрока.ЗаказНаВнутреннееПотребление = ЗаказВСтрокеСверхЗаказа(
			Объект.ПотреблениеПоЗаказам, Объект.ЗаказНаВнутреннееПотребление);
		ОбрабатываемыеСтроки.Добавить(ТекущаяСтрока);
