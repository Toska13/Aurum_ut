...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.КроссТаблица Тогда
		...
		Если Не Объект.ЗаполнятьПоДаннымСервиса Тогда
...
Процедура СоглашениеПриИзменении(Элемент)
	...
	Если Объект.ЗаполнятьПланОплат Или ЗаполнятьОрганизацию Тогда
...
Процедура ОбовитьСуммыПланаОплат()
	...
	Если Объект.КроссТаблица Тогда
		...
		ПланОплаты.Свернуть("ВариантОплаты, ДатаПлатежа, ДатаОтгрузки, Замещен, СтавкаНДС","СуммаПлатежа, СуммаНДС, СуммаЗадолженности, СуммаОтгрузок");
	...
	Иначе
		...
		СуммыПоДатам = Объект.Товары.Выгрузить(Новый Структура("Отменено", Ложь), "ДатаОтгрузки, Замещен, СтавкаНДС, СуммаНДС, СуммаСНДС");
		СуммыПоДатам.Свернуть("ДатаОтгрузки, Замещен, СтавкаНДС", "СуммаНДС, СуммаСНДС");
...
Процедура ОрганизацияПриИзмененииСервер()
	...
	ПараметрыЗаполнения.Дата = Объект.НачалоПериода;
...
Процедура СкопироватьИзКроссТаблицы(КэшированныеЗначения, ЭтотОбъект)
	...
	Если ЭтотОбъект.КроссТаблица 
		...
		Если ЗначениеЗаполнено(Объект.Организация) Тогда
			СтруктураПересчетаСуммы = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(Объект);
			СтруктураДействий.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
			Если ТипЗнч(ЭтотОбъект) <> Тип("ДокументОбъект.ПланПродаж") Тогда
				СтруктураДействий.Вставить("ПересчитатьСуммуСНДС", СтруктураПересчетаСуммы);
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьЭтапыОплатыПоШаблонуСервер(ГрафикСоглашенияЗаполнен, ГрафикЗаполнен)
	...
	Иначе
		...
		СуммыПоДатам = Объект.Товары.Выгрузить(Новый Структура("Отменено", Ложь), "ДатаОтгрузки, Замещен, СтавкаНДС, СуммаСНДС");
		СуммыПоДатам.Свернуть("ДатаОтгрузки, Замещен, СтавкаНДС", "СуммаСНДС");
		СуммыПоДатам.Сортировать("ДатаОтгрузки");
		...
		Для каждого СуммаНаДату Из СуммыПоДатам Цикл
			...
			Для каждого ЭтапОплаты Из ПланОплаты Цикл
				...
				ЭтапОплаты.СуммаНДС = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(ЭтапОплаты.СуммаПлатежа, ЭтапОплаты.СтавкаНДС,
					Истина, Объект.НалогообложениеНДС);
...
Процедура ДобавитьЭтапОплатыПоУмолчаниюСервер()
	...
	СуммыПоДатам = Объект.Товары.Выгрузить(Новый Структура("Отменено", Ложь),"ДатаОтгрузки, СтавкаНДС, СуммаНДС, СуммаСНДС, Замещен");
	СуммыПоДатам.Свернуть("ДатаОтгрузки, Замещен, СтавкаНДС", "СуммаНДС, СуммаСНДС");
	СуммыПоДатам.Сортировать("ДатаОтгрузки");
...
Процедура СоглашениеПриИзмененииСервер()
	...
	РеквизитыСоглашения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Соглашение, 
		"Контрагент, ИспользуютсяДоговорыКонтрагентов, Организация, ХозяйственнаяОперация, ГрафикОплаты, ЦенаВключаетНДС");
...
Функция ПараметрыОтбора(Форма)
	...
	ПараметрыОтбора = Новый Структура("Партнер, Соглашение, Склад, Подразделение, Организация, Менеджер, ФорматМагазина, ПланированиеПоНазначениям, Назначение");
