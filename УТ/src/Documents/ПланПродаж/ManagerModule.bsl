...
Функция ТекстЗапросаТаблицаПланыОплатКлиентов(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыКонтроляОплатыКлиентом.АвансДоОбеспечения)
	|			ИЛИ ТаблицаДокумента.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыКонтроляОплатыКлиентом.ПредоплатаДоОтгрузки)
	|			ТОГДА ТаблицаДокумента.СуммаПлатежа
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаПредоплаты,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыКонтроляОплатыКлиентом.КредитПослеОтгрузки)
	|			ИЛИ ТаблицаДокумента.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыКонтроляОплатыКлиентом.КредитСдвиг)
	|			ТОГДА ТаблицаДокумента.СуммаПлатежа
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаПостоплаты,
...
Функция ЗапросЗамещенныеИЗамещающиеПланы(Параметры) Экспорт
	...
	|	И ПланПродажЗамещающий.Организация = &Организация
	...
	|	И ПланПродажЗамещенный.Организация = &Организация
...
Функция ЗапросЗамещения(Параметры) Экспорт
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст,
		...
		?(ЗначениеЗаполнено(Параметры.Организация),"Организация = &Организация","Истина"));
	Запрос.Текст = СтрЗаменить(Запрос.Текст,
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПраваПользователяПовтИсп.ЭтоПартнер() Тогда
		...
		КомандаПечати.ДополнительныеПараметры.Вставить("ВыводитьОрганизацию", Ложь);
...
Функция СформироватьПечатнуюФормуПлана(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Если ПараметрыПечати <> Неопределено И НЕ ПараметрыПечати.Свойство("ВыводитьПодразделение", ВыводитьПодразделение) Тогда
		ВыводитьПодразделение 	= Истина;
	КонецЕсли;
	Если ПараметрыПечати <> Неопределено И НЕ ПараметрыПечати.Свойство("ВыводитьОрганизацию", ВыводитьОрганизацию) Тогда
		ВыводитьОрганизацию 	= Истина;
	КонецЕсли;
	...
	|	СпрВидыПланов.ЗаполнятьОрганизацию              КАК ЗаполнятьОрганизацию,
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ВыводитьШапку Тогда
			...
			Если ДанныеПечати.ЗаполнятьОрганизацию И ВыводитьОрганизацию Тогда
				ОбластьМакета                                       = Макет.ПолучитьОбласть("Организация");
				ПредставлениеОрганизации                            = Строка(ДанныеПечати.Организация);
