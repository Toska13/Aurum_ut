		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			Если НайденныеСтроки.Количество() > 0 Тогда
				Для Каждого СтрокаТЧ Из НайденныеСтроки Цикл
					...
					// Нормирование и проверка дат по точности даты производства
					Если ПроверятьДатуПроизводства Тогда
						
						Если СтрокаТЧ.ДатаПроизводстваНачалоПериода < ВСДОбъект.ДатаПроизводстваНачалоПериода Тогда
							Продолжить;
						КонецЕсли;
						
						Если ВСДОбъект.ДатаПроизводстваТочностьЗаполнения = Перечисления.ТочностьЗаполненияПериодаВЕТИС.ДДММГГГГЧЧ Тогда
							ОкончаниеПроизводстваТЧ = НачалоЧаса(ОкончаниеПроизводстваТЧ);
							ОкончаниеПроизводстваВСД = НачалоЧаса(ОкончаниеПроизводстваВСД);
						ИначеЕсли ВСДОбъект.ДатаПроизводстваТочностьЗаполнения = Перечисления.ТочностьЗаполненияПериодаВЕТИС.ДДММГГГГ Тогда
							ОкончаниеПроизводстваТЧ = НачалоДня(ОкончаниеПроизводстваТЧ);
							ОкончаниеПроизводстваВСД = НачалоДня(ОкончаниеПроизводстваВСД);
						ИначеЕсли ВСДОбъект.ДатаПроизводстваТочностьЗаполнения = Перечисления.ТочностьЗаполненияПериодаВЕТИС.ММГГГГ Тогда
							ОкончаниеПроизводстваТЧ = НачалоМесяца(ОкончаниеПроизводстваТЧ);
							ОкончаниеПроизводстваВСД = НачалоМесяца(ОкончаниеПроизводстваВСД);
						ИначеЕсли ВСДОбъект.ДатаПроизводстваТочностьЗаполнения = Перечисления.ТочностьЗаполненияПериодаВЕТИС.ГГГГ Тогда
							ОкончаниеПроизводстваТЧ = НачалоГода(ОкончаниеПроизводстваТЧ);
							ОкончаниеПроизводстваВСД = НачалоГода(ОкончаниеПроизводстваВСД);
						КонецЕсли;
						
						Если ОкончаниеПроизводстваТЧ > ОкончаниеПроизводстваВСД Тогда
							Продолжить;
						КонецЕсли;
						
					КонецЕсли;
					
					// Нормирование и проверка дат по точности срока годности
					Если ПроверятьСрокГодности Тогда
						
						Если СтрокаТЧ.СрокГодностиНачалоПериода < ВСДОбъект.СрокГодностиНачалоПериода Тогда
							Продолжить;
						КонецЕсли;
						
						Если ВСДОбъект.СрокГодностиТочностьЗаполнения = Перечисления.ТочностьЗаполненияПериодаВЕТИС.ДДММГГГГЧЧ Тогда
							ОкончаниеСрокаГодностиТЧ = НачалоЧаса(ОкончаниеСрокаГодностиТЧ);
							ОкончаниеСрокаГодностиВСД = НачалоЧаса(ОкончаниеСрокаГодностиВСД);
						ИначеЕсли ВСДОбъект.СрокГодностиТочностьЗаполнения = Перечисления.ТочностьЗаполненияПериодаВЕТИС.ДДММГГГГ Тогда
							ОкончаниеСрокаГодностиТЧ = НачалоДня(ОкончаниеСрокаГодностиТЧ);
							ОкончаниеСрокаГодностиВСД = НачалоДня(ОкончаниеСрокаГодностиВСД);
						ИначеЕсли ВСДОбъект.СрокГодностиТочностьЗаполнения = Перечисления.ТочностьЗаполненияПериодаВЕТИС.ММГГГГ Тогда
							ОкончаниеСрокаГодностиТЧ = НачалоМесяца(ОкончаниеСрокаГодностиТЧ);
							ОкончаниеСрокаГодностиВСД = НачалоМесяца(ОкончаниеСрокаГодностиВСД);
						ИначеЕсли ВСДОбъект.СрокГодностиТочностьЗаполнения = Перечисления.ТочностьЗаполненияПериодаВЕТИС.ГГГГ Тогда
							ОкончаниеСрокаГодностиТЧ = НачалоГода(ОкончаниеСрокаГодностиТЧ);
							ОкончаниеСрокаГодностиВСД = НачалоГода(ОкончаниеСрокаГодностиВСД);
						КонецЕсли;
						
						Если ОкончаниеСрокаГодностиТЧ > ОкончаниеСрокаГодностиВСД Тогда
							Продолжить;
						КонецЕсли;
