...
Процедура ЗадатьОписаниеЗаполняемыхРеквизитовПоСтатистике(ОписаниеРеквизитов) Экспорт
	...
	ПараметрыОписания = ПараметрыОписанияЗаполняемыхРеквизитовПоСтатистике();
	ДобавитьОписаниеЗаполняемыхРеквизитовПоСтатистике(ОписаниеРеквизитов, ПараметрыОписания);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Идентификатор = "РезультатОпросаБидзаар";
	КомандаПечати.Представление = НСтр("ru = 'Результат опроса'");
	КомандаПечати.Обработчик = "ИнтеграцияСЭлектроннымиТорговымиПлощадкамиКлиент.ОткрытьРезультатОпросаБидзаар";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ДокументЗКП.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ЗапросКоммерческихПредложенийПоставщиков КАК ДокументЗКП
	|ГДЕ
	|	ДокументЗКП.ЗапрашиватьПредложенияПоставщиков = &СпособРазмещенияБидзаар
	|	И ДокументЗКП.УчетнаяЗапись = ЗНАЧЕНИЕ(Справочник.УчетныеЗаписиЭлектронныхТорговыхПлощадок.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	...
	|	Документ.ЗапросКоммерческихПредложенийПоставщиков КАК ДокументЗКП
	|ГДЕ
	|	ДокументЗКП.Наименование = """"
	|	И ДокументЗКП.УдалитьКраткоеОписание <> """"";
	
	Запрос.УстановитьПараметр("СпособРазмещенияБидзаар", 2);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ВЫБРАТЬ
	|	ОбновляемыеДанные.Ссылка КАК Ссылка,
	|	ЕСТЬNULL(Пользователи.Наименование, """") КАК ФИОМенеджера
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗапросКоммерческихПредложенийПоставщиков КАК ЗапросКоммерческихПредложенийПоставщиков
	|		ПО ОбновляемыеДанные.Ссылка = ЗапросКоммерческихПредложенийПоставщиков.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Пользователи КАК Пользователи
	|		ПО ЗапросКоммерческихПредложенийПоставщиков.Менеджер = Пользователи.Ссылка";
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировки = Блокировка.Добавить("Документ.ЗапросКоммерческихПредложенийПоставщиков");
			ЭлементБлокировки.УстановитьЗначение("Ссылка", Выборка.Ссылка);
			Блокировка.Заблокировать();
			
			ДокументОбъект = Выборка.Ссылка.ПолучитьОбъект();	// ДокументОбъект.ЗапросКоммерческихПредложенийПоставщиков
			
			Если ДокументОбъект.ЗапрашиватьПредложенияПоставщиков = 2 Тогда
				
				Если Не ЗначениеЗаполнено(ДокументОбъект.УчетнаяЗапись) Тогда
					// Получение учетной записи
					УчетнаяЗапись = УчетныеЗаписиОрганизаций.Получить(ДокументОбъект.Организация);
					Если УчетнаяЗапись = Неопределено Тогда
						...
						УчетныеЗаписиОрганизаций.Вставить(ДокументОбъект.Организация, УчетнаяЗапись);
					КонецЕсли;
					
					ДокументОбъект.УчетнаяЗапись = УчетнаяЗапись;
				КонецЕсли;
				
				Если ПустаяСтрока(ДокументОбъект.ПредставлениеКИ) Тогда
					//Формирование представления КИ
					ЧастиКИ = Новый Массив;
					Если ЗначениеЗаполнено(Выборка.ФИОМенеджера) Тогда
						ЧастиКИ.Добавить(Выборка.ФИОМенеджера);
					КонецЕсли;
					Если ЗначениеЗаполнено(ДокументОбъект.МенеджерEmail) Тогда
						ЧастиКИ.Добавить(НСтр("ru ='email'") + ": " + ДокументОбъект.МенеджерEmail);
					КонецЕсли;
					Если ЗначениеЗаполнено(ДокументОбъект.МенеджерНомерТелефона) Тогда
						ЧастиКИ.Добавить(НСтр("ru ='телефон'") + ": " + ДокументОбъект.МенеджерНомерТелефона);
					КонецЕсли;
					ПредставлениеКИ = СтрСоединить(ЧастиКИ, "; ");
					
					ДокументОбъект.ПредставлениеКИ = ПредставлениеКИ;
				КонецЕсли;
				
			КонецЕсли;
			
			Если ДокументОбъект.Наименование = ""
				И ДокументОбъект.УдалитьКраткоеОписание <> "" Тогда
				
				ДокументОбъект.Наименование = ДокументОбъект.УдалитьКраткоеОписание;
			КонецЕсли;
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось обработать некоторые запросы коммерческих предложений (пропущены): %1'"),
				ПроблемныхОбъектов);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
    ...
    Методы.Вставить("ВыбратьПредложения", Истина);
    Методы.Вставить("СформироватьАнкеты", Истина);
    Методы.Вставить("ПолучитьПредложенияПоТовару", Истина);
    Методы.Вставить("ПолучитьПредложенияПоТоварам", Истина);
    Методы.Вставить("СравнениеПоступившихПредложений", Истина);
    Методы.Вставить("ПоискТорговыхПредложенийПоОтбору", Истина);
    Методы.Вставить("ПолучениеДанныхТорговогоПредложения", Истина);
    Методы.Вставить("СформироватьКоммерческиеПредложения", Истина);
    Методы.Вставить("ПроверкаВыбранныхПредложенийТорговойПлощадки", Истина);
    Методы.Вставить("ПредставлениеКатегорийПоИдентификаторамСервиса", Истина);
...
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.СостоянияЗапросаКоммерческихПредложений - Статус по умолчанию
...
Процедура ЗаполнитьДанныеПоЗапросуКоммерческихПредложений(Знач СсылкаНаОбъект, ДанныеДокумента, Отказ) Экспорт
	...
	Запрос.Текст = 
		...
		|	ДокЗапросКоммерческихПредложенийПоставщиков.ВариантУказанияСрокаПоставки КАК ВариантУказанияСрокаПоставки,
		|	ЗапросКоммерческихПредложенийПоставщиковТовары.Изображение
		|ИЗ
		|	Товары КАК ТЧТовары
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗапросКоммерческихПредложенийПоставщиков.Товары КАК
		|			ЗапросКоммерческихПредложенийПоставщиковТовары
		|		ПО ТЧТовары.ИдентификаторСтрокиЗапроса = ЗапросКоммерческихПредложенийПоставщиковТовары.ИдентификаторСтрокиЗапроса
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗапросКоммерческихПредложенийПоставщиков КАК
		|			ДокЗапросКоммерческихПредложенийПоставщиков
		|		ПО ТЧТовары.Ссылка = ДокЗапросКоммерческихПредложенийПоставщиков.Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ЗапросКоммерческихПредложенийПоставщиков.Наименование КАК Описание,
	...
	НеОтражатьВПрисоединенныхФайлах = Новый Массив;
	Пока ВыборкаТовары.Следующий() Цикл
		...
		Если ВыборкаТовары.ИсточникДобавленияТовара = 1 
			...
			Если ЗначениеЗаполнено(ВыборкаТовары.Изображение) Тогда
				НеОтражатьВПрисоединенныхФайлах.Добавить(ВыборкаТовары.Изображение);
			КонецЕсли;
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ВыборкаТовары.Изображение) 
				И ОбщегоНазначения.СсылкаСуществует(ВыборкаТовары.Изображение)
				И Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВыборкаТовары.Изображение, "ПометкаУдаления") Тогда
				ДвоичныеДанныеФайла = РаботаСФайлами.ДвоичныеДанныеФайла(ВыборкаТовары.Изображение);
				НоваяСтрока.Товар.Изображение = РаботаСФайламиБЭД.Base64СтрокаИзДвоичныхДанных(ДвоичныеДанныеФайла);
				НеОтражатьВПрисоединенныхФайлах.Добавить(ВыборкаТовары.Изображение);		
			КонецЕсли;
	...
	МассивФайлов = ОбщегоНазначенияКлиентСервер.РазностьМассивов(МассивФайлов, НеОтражатьВПрисоединенныхФайлах);
		
	ДанныеФайлов = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(МассивФайлов, 
		"Наименование, Расширение, Размер, ПометкаУдаления");
...
// Получение категорий и изобаржений по идентификаторам карточек сервиса 1С:Номенклатуры.
//
// Параметры:
//  ПараметрыЗапроса - Структура - параметры поиска категорий в сервисе. Содержит ключи:
//    * Идентификаторы - Массив, Строка - идентификаторы карточек 1С:Номенклатуры,
...
//  АдресРезультата - Строка - адрес хранилища с результатом.
//
Процедура КатегорииИИзображенияПоИдентификаторамСервиса(ПараметрыОперации, АдресРезультата) Экспорт
	...
	ДанныеКатегорийИзображений = КоммерческиеПредложенияДокументы.ДанныеКатегорийИзображенийСервиса(Идентификаторы);
	Для Каждого Элемент Из ДанныеКатегорийИзображений Цикл
		ДанныеСервиса = Элемент.Значение;
		Если ДанныеСервиса.Свойство("ВнешнийАдресИзображения") 
			И ЗначениеЗаполнено(ДанныеСервиса.ВнешнийАдресИзображения) Тогда
			
			АдресИзображенияВХранилище = КоммерческиеПредложенияСлужебный.АдресИзображенияВХранилище(
				ДанныеСервиса.ВнешнийАдресИзображения, Ложь);
			РасширениеИзображения = ОбщегоНазначенияКлиентСервер.ПолучитьРасширениеИмениФайла(ДанныеСервиса.ВнешнийАдресИзображения);
		
			Если ЗначениеЗаполнено(АдресИзображенияВХранилище) Тогда
				ДанныеСервиса.Вставить("ДанныеИзображения",
					Новый Структура("ИзображениеДвоичныеДанные, РасширениеИзображения",
					ПолучитьИзВременногоХранилища(АдресИзображенияВХранилище), РасширениеИзображения));
				ДанныеСервиса.Удалить("ВнешнийАдресИзображения");				
			КонецЕсли;	
		КонецЕсли;	
	КонецЦикла;
	
	ПоместитьВоВременноеХранилище(ДанныеКатегорийИзображений, АдресРезультата);
...
//  АдресРезультата - строка - Адрес результата. Содержит свойства:
...
Процедура СформироватьКоммерческиеПредложения(ПараметрыПроцедуры, АдресРезультата) Экспорт
	...
	|	ПО СправочникКонтрагенты.Ссылка = врКонтрагенты.Контрагент
	|";
	
	Запрос.УстановитьПараметр("Контрагенты", Контрагенты);
	РезультатЗапроса = Запрос.Выполнить();
	СтрокаКонтрагента = РезультатЗапроса.Выбрать();
	...
	Пока СтрокаКонтрагента.Следующий() Цикл

		ИдентификаторПредложенияВСервисе = СтрокаКонтрагента.ИдентификаторПредложенияСервиса;
		КоммерческоеПредложениеСсылка = КоммерческиеПредложенияПоставщиков.Получить(ИдентификаторПредложенияВСервисе);
		...
		// Создание/заполнение коммерческого предложения
		...
		ДанныеКП.Вставить("ХозяйственнаяОперация", РеквизитыДокументаОснования.ХозяйственнаяОперация);
		ДанныеКП.Вставить("Дата", ДатаКП);
		ДанныеКП.Вставить("Контрагент", СтрокаКонтрагента.Контрагент);
		ДанныеКП.Вставить("Поставщик", СтрокаКонтрагента.Партнер);
		...
		Если Не КоммерческоеПредложение.Ссылка.Пустая() Тогда
			Блокировка = Новый БлокировкаДанных();
			ЭлементБлокировки = Блокировка.Добавить("Документ.КоммерческоеПредложениеПоставщика");
			ЭлементБлокировки.УстановитьЗначение("Ссылка", КоммерческоеПредложение.Ссылка);
		...
		Исключение
			...
			Исключение
				...
				ТекстОшибкиСозданияКП = НСтр("ru = 'Не удалось создать коммерческое предложение поставщика.
					|%1'", ОбщегоНазначения.КодОсновногоЯзыка());
				ТекстОшибкиСозданияКП = СтрШаблон(ТекстОшибкиСозданияКП,
					ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке));	
				РезультатСоздания.ТекстОшибки = ТекстОшибкиСозданияКП;
...
Процедура СформироватьАнкеты(ПараметрыОперации, АдресРезультата) Экспорт
	...
	РеквизитыОпроса = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПараметрыОперации.Опрос, "ШаблонАнкеты");
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ОтветыНаВопросы.НомерСтроки КАК НомерСтроки,
	|	ОтветыНаВопросы.КоммерческоеПредложение КАК КоммерческоеПредложение,
	|	ОтветыНаВопросы.Вопрос КАК Вопрос
	|ПОМЕСТИТЬ врОтветыНаВопросы
	|ИЗ
	|	&ОтветыНаВопросы КАК ОтветыНаВопросы
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	врОтветыНаВопросы.НомерСтроки КАК НомерСтроки,
	|	врОтветыНаВопросы.КоммерческоеПредложение КАК КоммерческоеПредложение,
	|	врОтветыНаВопросы.Вопрос КАК Вопрос,
	|	ВопросыШаблонаАнкеты.ЭлементарныйВопрос КАК ЭлементарныйВопрос
	|ИЗ
	|	врОтветыНаВопросы КАК врОтветыНаВопросы
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВопросыШаблонаАнкеты КАК ВопросыШаблонаАнкеты
	|		ПО врОтветыНаВопросы.Вопрос = ВопросыШаблонаАнкеты.Ссылка
	|ИТОГИ ПО
	|	КоммерческоеПредложение";
	
	ОтветыНаВопросы = Новый ТаблицаЗначений;
	ОтветыНаВопросы.Колонки.Добавить("НомерСтроки", Новый ОписаниеТипов("Число"));
	ОтветыНаВопросы.Колонки.Добавить("КоммерческоеПредложение", Новый ОписаниеТипов("ДокументСсылка.КоммерческоеПредложениеПоставщика"));
	ОтветыНаВопросы.Колонки.Добавить("Вопрос", Новый ОписаниеТипов("СправочникСсылка.ВопросыШаблонаАнкеты"));
	
	ОтветыПоНомерамСтрок = Новый Соответствие;
	
	НомерСтроки = 0;
	Для Каждого Ответ Из ПараметрыОперации.ОтветыНаВопросы Цикл
		НоваяСтрока = ОтветыНаВопросы.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Ответ);
		НоваяСтрока.НомерСтроки = НомерСтроки;
		НоваяСтрока.Вопрос = Ответ.Вопрос;
		
		ОтветыПоНомерамСтрок.Вставить(НомерСтроки, Ответ.Ответы);
		
		НомерСтроки = НомерСтроки + 1;
	КонецЦикла;
	
	Запрос.УстановитьПараметр("ОтветыНаВопросы", ОтветыНаВопросы);
	...
	ВыборкаКП = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока ВыборкаКП.Следующий() Цикл
		
		РезультатСоздания = Новый Структура;
		РезультатСоздания.Вставить("КоммерческоеПредложение", ВыборкаКП.КоммерческоеПредложение);
		РезультатСоздания.Вставить("Анкета", Неопределено);
		РезультатСоздания.Вставить("ТекстОшибки","");
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("Документ.КоммерческоеПредложениеПоставщика");
		ЭлементБлокировки.УстановитьЗначение("Ссылка", ВыборкаКП.КоммерческоеПредложение);
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		...
		Попытка
			...
			КоммерческоеПредложение = ВыборкаКП.КоммерческоеПредложение.ПолучитьОбъект();
			КоммерческоеПредложение.Заблокировать();
			
			УстановитьПривилегированныйРежим(Истина);
			Если ЗначениеЗаполнено(КоммерческоеПредложение.Анкета) Тогда
				Анкета = КоммерческоеПредложение.Анкета.ПолучитьОбъект();
				Анкета.Состав.Очистить();
			Иначе
				Анкета = Документы.Анкета.СоздатьДокумент();
			КонецЕсли;
			
			Анкета.Дата = КоммерческоеПредложение.Дата;
			Анкета.Опрос = ПараметрыОперации.Опрос;
			Анкета.Респондент = КоммерческоеПредложение.Поставщик;
			Анкета.ДатаРедактирования = КоммерческоеПредложение.Дата;
			Анкета.ШаблонАнкеты = РеквизитыОпроса.ШаблонАнкеты;
			Анкета.РежимАнкетирования = Перечисления.РежимыАнкетирования.Анкета;
			
			Выборка = ВыборкаКП.Выбрать();
			Пока Выборка.Следующий() Цикл
				...
				НоваяСтрока.Ответ = ОтветыПоНомерамСтрок.Получить(Выборка.НомерСтроки);
				НоваяСтрока.БезОтвета = Не ЗначениеЗаполнено(НоваяСтрока.Ответ);
			КонецЦикла;

			Анкета.Записать(РежимЗаписиДокумента.Проведение);
			АнкетаСсылка = Анкета.Ссылка;
			УстановитьПривилегированныйРежим(Ложь);
			
			КоммерческоеПредложение.Анкета = АнкетаСсылка;
			КоммерческоеПредложение.ОбменДанными.Загрузка = Истина;
			КоммерческоеПредложение.Записать();
			РезультатСоздания.Анкета = АнкетаСсылка;
			
			ЗафиксироватьТранзакцию();
		Исключение
			ОтменитьТранзакцию();
			
			Текст = НСтр("ru = 'Создание анкеты'",
			ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрации(Текст, УровеньЖурналаРегистрации.Ошибка, Метаданные.Документы.Анкета,,
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
					
			РезультатСоздания.ТекстОшибки = НСтр("ru = 'Не удалось создать анкету'",
				ОбщегоНазначения.КодОсновногоЯзыка());
		КонецПопытки;
...
// Добавляет описание реквизитов объекта, заполняемых по статистике их использования.
//
// Параметры:
//  ОписаниеРеквизитов - Структура - описание реквизитов, для которых необходимо получить значения по статистике
//  ДополнительныеПараметры - см. ПараметрыОписанияЗаполняемыхРеквизитовПоСтатистике 
//
Процедура ДобавитьОписаниеЗаполняемыхРеквизитовПоСтатистике(ОписаниеРеквизитов, ДополнительныеПараметры) Экспорт
	...
	ЗаполнениеОбъектовПоСтатистике.ДобавитьОписаниеЗаполняемыхРеквизитов(ОписаниеРеквизитов,
		"МенеджерEmail, МенеджерНомерТелефона", Параметры);
	
	Если ДополнительныеПараметры.ЗаполнитьПредставлениеКИ Тогда
	
		ЗаполнениеОбъектовПоСтатистике.ДобавитьОписаниеЗаполняемыхРеквизитов(ОписаниеРеквизитов,
			"ПредставлениеКИ", Параметры);
		
	КонецЕсли;
		
КонецПроцедуры

// Формирует дополнительные параметры для задания описания реквизитов объекта, заполняемых по статистике их использования.
//
// Возвращаемое значение:
//   Структура - содержит:
//   	* ЗаполнитьПредставлениеКИ - Булево - указывает на необходимость заполнения реквизита "ПредставлениеКИ"
//
Функция ПараметрыОписанияЗаполняемыхРеквизитовПоСтатистике() Экспорт
	Результат = Новый Структура;
	Результат.Вставить("ЗаполнитьПредставлениеКИ", Истина);
	Возврат Результат;
КонецФункции
