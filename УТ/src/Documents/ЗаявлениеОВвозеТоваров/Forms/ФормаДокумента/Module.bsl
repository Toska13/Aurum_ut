...
Процедура СоздатьСтатформуВФТСНажатие(Элемент)
	...
	Возврат; // Не требуется в УТ. Скрыта видимость кнопки.
...
Процедура РасчитатьСуммыВТабличнойЧастиНаСервере(СтрокаТабличнойЧасти, ДанныеОбъекта)
	...
	Если ДанныеОбъекта.Валюта <> ДанныеОбъекта.ВалютаРегламентированногоУчета Тогда
		СтруктураКурса = РаботаСКурсамиВалютУТ.ПолучитьКурсВалюты(
			...
			СтрокаТабличнойЧасти.ДатаПринятияНаУчет,
	...
	Если Объект.ИспользоватьАкцизы Или ИспользоватьПодакцизныеТовары Тогда
		СтрокаТабличнойЧасти.НалоговаяБазаНДС =
			СтрокаТабличнойЧасти.НалоговаяБазаНДС + СтрокаТабличнойЧасти.СуммаАкциза;
	КонецЕсли;
...
Процедура ДобавитьСтрокиВручную(Команда)
	...
	НоваяСтрока.ВалютаРегл = ВалютаРегламентированногоУчета;
	НоваяСтрока.ВалютаЕдиницаСтавкиАкциза = ВалютаРегламентированногоУчета;
...
Процедура ОрганизацияПриИзмененииНаСервере()
	...
	ИспользоватьПодакцизныеТовары = Не Объект.ИспользоватьАкцизы
		И УчетПодакцизныхТоваров.ИспользуетсяИмпортПодакцизныхТоваров(Объект.Организация, Объект.Дата);
	ЗаполнитьДополнительныеПризнакиТоваров();
	УстановитьВидимостьАкцизы();
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыГруппаНалоговаяБазаАкциза.Имя);
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыНалоговаяБазаАкцизы.Имя);
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыЕдиницаИзмеренияАкциза.Имя);
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыГруппаТвердаяСтавкаАкциза.Имя);
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыТвердаяСтавкаАкциза.Имя);
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыВалютаЕдиницаСтавкиАкциза.Имя);
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыАдвалорнаяСтавкаАкциза.Имя);
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыГруппаСуммаАкциза.Имя);
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыСуммаАкциза.Имя);
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыВалютаРеглАкциз.Имя);
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Товары.ТипСтавки");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно;
	ОтборЭлемента.ПравоеЗначение = Перечисления.ТипыСтавокПодакцизнойКатегории.Твердая;
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Товары.ТипСтавки");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно;
	ОтборЭлемента.ПравоеЗначение = Перечисления.ТипыСтавокПодакцизнойКатегории.Адвалорная;
...
Процедура ОбновитьИтоги(Форма)
	...
	Объект = Форма.Объект;
	Если Форма.ЭтоЛокализацияРФ Тогда
		Форма.ИтогиВсегоНДС = Окр(Объект.Товары.Итог("СуммаНДС"));
		Форма.ИтогиВсегоАкциз = Окр(Объект.Товары.Итог("СуммаАкциза"));
	Иначе
...
Процедура УстановитьВидимостьЭлементовФормы()
	...
	ОкруглятьСуммыНДСиАкцизовДоЦелыхВЗаявленияхОВвозеТоваров = Ложь;
	
	//++ Локализация
	ОкруглятьСуммыНДСиАкцизовДоЦелыхВЗаявленияхОВвозеТоваров = ЭтоЛокализацияРФ
		И ПолучитьФункциональнуюОпцию("ОкруглятьСуммыНДСиАкцизовДоЦелыхВЗаявленияхОВвозеТоваров");
	//-- Локализация
	
	Элементы.ОкруглятьИтоговуюСуммуНДСДоРублей.Видимость = ОкруглятьСуммыНДСиАкцизовДоЦелыхВЗаявленияхОВвозеТоваров;
	Элементы.ОкруглятьИтоговуюСуммуАкцизовДоРублей.Видимость = ОкруглятьСуммыНДСиАкцизовДоЦелыхВЗаявленияхОВвозеТоваров;
...
Процедура УправлениеФормой(Форма)
	...
	Для каждого Строка Из Объект.Товары Цикл
		...
		Строка.ВалютаРегл = Форма.ВалютаРегламентированногоУчета;
		Если ЗначениеЗаполнено(Строка.ЕдиницаИзмеренияАкциза) Тогда
			Строка.ВалютаЕдиницаСтавкиАкциза = СтрШаблон("%1/%2", Строка.ВалютаАкциза, Строка.ЕдиницаИзмеренияАкциза);
		Иначе
			Строка.ВалютаЕдиницаСтавкиАкциза =
				?(ЗначениеЗаполнено(Строка.ВалютаАкциза), Строка.ВалютаАкциза, Строка.ВалютаРегл);
		КонецЕсли;
...
Процедура ОбработкаВыбораДобавитьСтрокиИзДокументаПартии(МассивДокументов)
	...
	Для каждого ЭлементМассива Из МассивДокументов Цикл
		...
		Для каждого СтрокаТаблицыТовары Из ТаблицаТовары Цикл
			...
			Если Не Объект.ИспользоватьАкцизы И Не ИспользоватьПодакцизныеТовары Тогда
...
Процедура РассчитатьСуммуАкцизов(ТекущаяСтрока)
	...
	ТвердаяСтавкаАкциза = ТекущаяСтрока.ТвердаяСтавкаАкциза; 
	Если ЗначениеЗаполнено(ТекущаяСтрока.ВалютаАкциза)
		И ТекущаяСтрока.ВалютаАкциза <> ТекущаяСтрока.ВалютаРегл Тогда
		ТвердаяСтавкаАкциза = Окр(ТекущаяСтрока.ТвердаяСтавкаАкциза
			* КоэффициентПересчетаВВалютуАкциза(
				ТекущаяСтрока.ВалютаАкциза,
				ТекущаяСтрока.ВалютаРегл,
				ТекущаяСтрока.ДатаПринятияНаУчет), 2);
	КонецЕсли;
	
	ТекущаяСтрока.СуммаАкциза = ТекущаяСтрока.НалоговаяБазаАкцизы * ТвердаяСтавкаАкциза 
...
Процедура ЗаполнитьДополнительныеПризнакиТоваров()
	...
	Если ИспользоватьПодакцизныеТовары Тогда
		УчетПодакцизныхТоваров.ЗаполнитьДополнительныеПризнакиТоваровВЗаявленииОВвозе(Объект);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура УстановитьВидимостьАкцизы()
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
		"ИтогВсегоАкциз",
		"Видимость",
		Объект.ИспользоватьАкцизы Или ИспользоватьПодакцизныеТовары);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
		"ТоварыГруппаНалоговаяБазаАкциза",
		"Видимость",
		Объект.ИспользоватьАкцизы Или ИспользоватьПодакцизныеТовары);
...
Функция КоэффициентПересчетаВВалютуАкциза(ВалютаАкциза, БазоваяВалюта, Период)
	
	СтруктураКурса = РаботаСКурсамиВалютУТ.ПолучитьКурсВалюты(ВалютаАкциза, Период, БазоваяВалюта);
	КоэффициентПересчета = СтруктураКурса.КурсЧислитель / СтруктураКурса.КурсЗнаменатель;
