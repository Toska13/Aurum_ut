...
#КонецОбласти
	...
	Товары.Колонки.Добавить("КодыМаркировки", Новый ОписаниеТипов("Массив"));
	...
	Если Не Шапка.ОтчетПроизводственнойЛинии Тогда
		...
		Для Каждого СтрокаТЧ Из ВложенныеШтрихкодыУпаковок.МаркированныеТовары Цикл
			...
			НайденнаяСтрока = Товары.НайтиСтроки(Поиск);
	...
	ОтчетXDTO = РаботаСXMLЕГАИС.ОбъектXDTO(ПространствоИмен, "RepProducedType_v6");
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		НомерСтроки = Формат(Товары.Индекс(СтрокаТЧ) + 1, "ЧГ=0");
		...
		РаботаСXMLЕГАИС.ЗаполнитьСвойствоXDTO(Position, "ProductCode", СтрокаТЧ.АлкогольнаяПродукция, СообщениеXML);
		РаботаСXMLЕГАИС.ЗаполнитьСвойствоXDTO(Position, "Comment1",    СтрокаТЧ.Комментарий1,         СообщениеXML);
		РаботаСXMLЕГАИС.ЗаполнитьСвойствоXDTO(Position, "Comment2",    СтрокаТЧ.Комментарий2,         СообщениеXML);
		РаботаСXMLЕГАИС.ЗаполнитьСвойствоXDTO(Position, "Comment3",    СтрокаТЧ.Комментарий3,         СообщениеXML);
		РаботаСXMLЕГАИС.ЗаполнитьСвойствоXDTO(Position, "Party",       СтрокаТЧ.НомерПартии,          СообщениеXML);
		РаботаСXMLЕГАИС.ЗаполнитьСвойствоXDTO(Position, "alcPercent",  СтрокаТЧ.Крепость,             СообщениеXML);
		...
		СырьеСтроки = Сырье.НайтиСтроки(Новый Структура("ИдентификаторСтрокиТовары", СтрокаТЧ.ИдентификаторСтроки));
		Если СырьеСтроки.Количество() Тогда
			...
			Для Каждого СтрокаТЧСырье Из СырьеСтроки Цикл
				...
				РаботаСXMLЕГАИС.ЗаполнитьСвойствоXDTO(Resource, "IDENTITYPRODUCT", НомерСтрокиСырье,            СообщениеXML);
...
Процедура ОтразитьРезультатыПроверкиИПодбора(ПараметрыОкончанияПроверки) Экспорт
	...
	Если ПараметрыОкончанияПроверки.Сценарий = 1 Тогда
		
		АкцизныеМаркиЕГАИС.ОтразитьРезультатыПроверкиИПодбора(ЭтотОбъект(), ПараметрыОкончанияПроверки,, "Сырье");
		
	ИначеЕсли ПараметрыОкончанияПроверки.Сценарий = 0 Тогда
...
Процедура ЗаполнитьПродукциюДокумента(ТаблицаМаркируемойПродукции, ДокументСсылка, Сценарий) Экспорт
	...
	ИначеЕсли Сценарий = 1 Тогда
		...
		|	ОтчетОПроизводствеЕГАИССырье.АлкогольнаяПродукция,
		|	СУММА(ОтчетОПроизводствеЕГАИССырье.Количество) КАК Количество,
		|	ОтчетОПроизводствеЕГАИССырье.Справка2
		|ИЗ
		|	Документ.ОтчетОПроизводствеЕГАИС.Сырье КАК ОтчетОПроизводствеЕГАИССырье
		|ГДЕ
		|	ОтчетОПроизводствеЕГАИССырье.Ссылка = &ДокументСсылка
		|	И ОтчетОПроизводствеЕГАИССырье.АлкогольнаяПродукция.ВидПродукции.Маркируемый = Истина
		|СГРУППИРОВАТЬ ПО
		|	ОтчетОПроизводствеЕГАИССырье.Номенклатура,
		|	ОтчетОПроизводствеЕГАИССырье.Характеристика,
		|	ОтчетОПроизводствеЕГАИССырье.Серия,
		...
		ПроверкаИПодборПродукцииИСПереопределяемый.ПриОпределенииПродукцииДокумента(
			ДокументСсылка, 1, ТаблицаМаркируемойПродукции);
...
Процедура ОбработатьСтрокиТабличнойЧасти(ДокументОбъект, ПараметрыЗаполнения) Экспорт
	...
	Если ПараметрыЗаполнения.ПараметрыСканирования.Сценарий = 0 Тогда
...
Функция ЭтотОбъект(ЭтоМетаданные = Ложь)
	Если ЭтоМетаданные Тогда
		Возврат Метаданные.Документы.ОтчетОПроизводствеЕГАИС;
	Иначе
		Возврат Документы.ОтчетОПроизводствеЕГАИС;
	КонецЕсли;
КонецФункции
