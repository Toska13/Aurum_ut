...
Процедура ПроверитьДублиСтрок(Отказ)
	...
	Для Каждого СтрокаТовары Из Товары Цикл
		КлючНоменклатура = КлючиСтрок[СтрокаТовары.Номенклатура];
		Если КлючНоменклатура = Неопределено Тогда
			КлючНоменклатура = Новый Соответствие;
			КлючиСтрок.Вставить(СтрокаТовары.Номенклатура, КлючНоменклатура);
		КонецЕсли;
		КлючХарактеристика = КлючНоменклатура[СтрокаТовары.Характеристика];
		Если КлючХарактеристика = Неопределено Тогда
			КлючХарактеристика = Новый Соответствие;
			КлючНоменклатура.Вставить(СтрокаТовары.Характеристика, КлючХарактеристика);
		КонецЕсли;
		КлючСерия = КлючХарактеристика[СтрокаТовары.Серия];
		Если КлючСерия = Неопределено Тогда
			КлючХарактеристика.Вставить(СтрокаТовары.Серия, СтрокаТовары.НомерСтроки);
		Иначе
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'В строках %1 и %2 табличной части Товары совпадают данные номенклатуры.
					|Коды маркировки между ними не могут быть распределены.'"),
				КлючСерия.Значение, СтрокаТовары.НомерСтроки);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,
				ЭтотОбъект,
				ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
