...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СобытияФормИСКлиентСерверПереопределяемый.УстановитьПараметрыВыбораНоменклатуры(ЭтотОбъект, ВидыПродукцииИСМПЕГАИС);
	
	ДопустимыеВидыПродукцииСырья = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЕГАИС();
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		ДопустимыеВидыПродукцииСырья,
...
Процедура ШапкаНоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СобытияФормИСКлиентПереопределяемый.ПриНачалеВыбораНоменклатуры(
		ЭтотОбъект, ВидыПродукции, СтандартнаяОбработка,,
...
&НаКлиенте
Процедура ОткрытьФормуПроверкиИПодбораТовары(Команда)
...
&НаКлиенте
Процедура ОткрытьФормуПроверкиИПодбораСырье(Команда)
	
	ПроверкаИПодборПродукцииИСКлиент.ОткрытьФормуПроверкиИПодбора(ЭтотОбъект, ПараметрыУказанияСерий, 1);
...
Процедура ОбновитьСтатус(ОбновитьСтатусПроверкиИПодбора = Истина)
	...
	Если ОбновитьСтатусПроверкиИПодбора Тогда
		...
		СтатусПроверкиИПодбораСырье = РегистрыСведений.СтатусыПроверкиИПодбораИС.ТекущийСтатус(Объект.Ссылка, 1);
	КонецЕсли;
	...
	Если СтатусПроверкиИПодбораСырье = Перечисления.СтатусыПроверкиИПодбораИС.Выполняется
...
Процедура НастроитьЭлементыФормы(Форма)
	...
	Элементы.СтраницаАкцизныеМарки.Видимость = Форма.ДоступенРежимОтладки И Не Объект.ОтчетПроизводственнойЛинии;
	Элементы.СтраницаОтчетПроизводственнойЛинии.Видимость = Объект.ОтчетПроизводственнойЛинии;
	Элементы.СырьеОтобразитьСырьеПоТекущейСтрокеТовары.Видимость = Не Объект.ОтчетПроизводственнойЛинии;
	Элементы.СырьеИдентификаторСтрокиТовары.Видимость = Не Форма.СырьеТекущейСтроки И Не Объект.ОтчетПроизводственнойЛинии;
	
	Если Объект.ОтчетПроизводственнойЛинии Тогда
		Элементы.СтраницыПереключательОтчетПроизводственнойЛинии.ТекущаяСтраница = Элементы.ГруппаТоварОтчетаПроизводственнойЛинии;
		Если Объект.Товары.Количество() > 0 Тогда
		...
		КонецЕсли;
	Иначе
		Элементы.СтраницыПереключательОтчетПроизводственнойЛинии.ТекущаяСтраница = Элементы.ГруппаТоварыСписком;
	КонецЕсли;
...
Процедура УправлениеДоступностьюЭлементовФормы(Форма)
	...
	ОграничениеПоСтатусуПроверкиИПодбора = Форма.СтатусПроверкиИПодбораСырье = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиИПодбораИС.Выполняется")
		Или Форма.СтатусПроверкиИПодбора = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиИПодбораИС.Выполняется");
	...
	ЗависимыеОтСтатусаЕГАИС.Добавить("ОтчетПроизводственнойЛинии");
...
Процедура ОбновитьИнформациюОткрытияФормыСканирования()
	...
	Элементы.ГруппаИнформацияОСканированииВДругойФормеПиво.Видимость = ПравоИзменения
		И СтатусПроверкиИПодбораСырье = Перечисления.СтатусыПроверкиИПодбораИС.Выполняется;
	
	ТекстНадписи = "";
	Если СтатусПроверкиИПодбора = Перечисления.СтатусыПроверкиИПодбораИС.Выполняется Тогда
		Если ПравоИзменения Тогда
			ТекстНадписи = НСтр("ru = 'Продолжить сканирование кодов маркировки'");
		Иначе
			ТекстНадписи = НСтр("ru = 'Промежуточные результаты сканирования кодов маркировки'");
		КонецЕсли;
	ИначеЕсли (СтатусЕГАИС = Перечисления.СтатусыОбработкиОтчетаОПроизводствеЕГАИС.Черновик
		...
		Если ПравоИзменения Тогда
			ТекстНадписи = НСтр("ru = 'Отсканировать коды маркировки'");
		ИначеЕсли СтатусПроверкиИПодбораСырье = Перечисления.СтатусыПроверкиИПодбораИС.Завершено Тогда
			ТекстНадписи = НСтр("ru = 'Результаты сканирования кодов маркировки'");
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстНадписи) Тогда
		Элементы.ТоварыОткрытьФормуПроверкиИПодбораТовары.Заголовок = ТекстНадписи;
		Элементы.ТоварыОткрытьФормуПроверкиИПодбораТовары.Видимость = Истина;
	Иначе
		Элементы.ТоварыОткрытьФормуПроверкиИПодбораТовары.Видимость = Ложь;
	КонецЕсли;
	...
	ТекстНадписи = "";
	Если СтатусПроверкиИПодбораСырье = Перечисления.СтатусыПроверкиИПодбораИС.Выполняется Тогда
		Если ПравоИзменения Тогда
	...
	ИначеЕсли (СтатусЕГАИС = Перечисления.СтатусыОбработкиОтчетаОПроизводствеЕГАИС.Черновик
		...
		КонецЕсли;
	ИначеЕсли СтатусПроверкиИПодбораСырье = Перечисления.СтатусыПроверкиИПодбораИС.ПустаяСсылка() Тогда
	Иначе
		ТекстНадписи = НСтр("ru = 'Результаты подбора сырья'");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ТекстНадписи) Тогда
		Элементы.СырьеОткрытьФормуПроверкиИПодбораСырье.Заголовок = ТекстНадписи;
		Элементы.СырьеОткрытьФормуПроверкиИПодбораСырье.Видимость = Истина;
	Иначе
		Элементы.СырьеОткрытьФормуПроверкиИПодбораСырье.Видимость = Ложь;
	КонецЕсли;
...
Процедура ОпределитьНоменклатуруОтчетаПроизводственнойЛинии(Штрихкоды)
	...
	Если Штрихкоды.Количество() Тогда
		...
		НастройкиРазбораКодаМаркировки  = РазборКодаМаркировкиИССлужебный.НастройкиРазбораКодаМаркировки(ВидыПродукцииИСМПЕГАИС);
		...
		ГрупповаяОбработкаШтрихкодовИС.ВидУпаковкиИПредставлениеШтрихкодаУпрощенныйРазбор(
			Штрихкод, ВидыПродукцииИСМПЕГАИС, КешДанныхРазбора,
		...
		Для Каждого ВидПродукции Из ВидыПродукцииИСМПЕГАИС Цикл
			
			СтруктураПоиска = Новый Структура;
			СтруктураПоиска.Вставить("ШтрихкодEAN", EAN);
			СтруктураПоиска.Вставить("ВидПродукции", ВидПродукции);
