...
Функция СтатусыОшибок() Экспорт
	...
	Статусы.Добавить(Перечисления.СтатусыОбработкиСписанияПартииЗЕРНО.Ошибка);
...
//
// Возвращаемое значение:
//  См. РегистрыСведений.СтатусыОбъектовСинхронизацииЗЕРНО.РассчитатьСтатусы
//
Функция СтатусПослеПодготовкиКПередачеДанных(ДокументСсылка, Операция, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если Операция = Перечисления.ВидыОперацийЗЕРНО.СписаниеПартии
		Или Операция = Перечисления.ВидыОперацийЗЕРНО.СписаниеПартиийРасчетСтатуса Тогда
...
Процедура ПриЗаполненииОграниченияДоступа(Списки) Экспорт
	...
	УправлениеДоступомИСПереопределяемый.ПриЗаполненииОграниченияДоступа(
		Метаданные.Документы.СписаниеПартийЗЕРНО, Списки);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ОбновлениеИнформационнойБазыГосИС.ДобавитьОбработчикОбновленияПереходаНаКонструкторНастроек(
		...
		Новый УникальныйИдентификатор("ff348043-5a4b-4a9a-92b3-4ab82e661aee"));
...
// 
// Параметры:
//  Объект - ДокументОбъект.СписаниеПартийЗЕРНО - Объект
//  СписокСтрок - Неопределено, Массив Из СтрокаТабличнойЧасти - Список строк
// 
// Возвращаемое значение:
//  Структура - структура для показа сообщения пользователю о результатах заполнения
...
Процедура ОбработкаЗагрузкиПолученныхДанных(ЭлементОчереди, ПараметрыОбмена, ПолученныеДанные, ИзмененныеОбъекты, ПрочитаноВерсиейAPI) Экспорт
	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийЗЕРНО.АннулированиеСписанияПартии Тогда
		...
		Если ВРег(ПолученныеДанные.resultOperation) = "DONE" Тогда
			...
			Если ЭлементОчереди.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна Тогда
				ИмяПоляНомерЗаявки = "gpbDebitId";
			Иначе
				ИмяПоляНомерЗаявки = "lotDebitId";
			КонецЕсли;
...
Функция РезультирующийСтатусДокумента(ДокументСсылка) Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ИначеЕсли ВыборкаДетальныеЗаписи.Статус = Перечисления.СтатусыОбработкиСписанияПартииЗЕРНО.СписаниеПартииОбрабатывается Тогда
			Результат.Статус = ВыборкаДетальныеЗаписи.Статус;
