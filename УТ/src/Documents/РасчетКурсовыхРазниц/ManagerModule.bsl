...
Процедура ДобавитьЭтап_ПереоценкаДенежныхСредствИФинансовыхИнструментов(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ПереоценкаДенежныхСредствИФинансовыхИнструментов);
		
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Переоценка денежных средств и финансовых инструментов'"; // @НСтр-1
...
Функция СледующийМесяцКРасчетуДенежныхСредств(ПериодРасчета, МассивОрганизаций)
	...
	|		И ЗаданияКПереоценке.Операция = ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.ПереоценкаДенежныхСредствИФинансовыхИнструментов)
	|
	|	) КАК ДенежныеСредства
	|ИМЕЮЩИЕ 
...
Процедура ДобавитьЭтап_ПереоценкаРасчетовСКомитентомКомиссионером(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ПереоценкаРасчетовСКомитентом);
		
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Переоценка расчетов с комитентом (принципалом)'"; // @НСтр-1
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
...
Процедура РассчитатьКурсовыеРазницыПоСрокам(Организация, Дата, Операция, ПереоценкаПоДням = Неопределено, КоличествоОбработанныхЭлементов = 0) Экспорт
	...
	ПараметрыПереоценки = ВзаиморасчетыПереоценкаСервер.ПараметрыПереоценки(РасчетыСКлиентами, ПереоценкаПоДням);
	ВзаиморасчетыПереоценкаСервер.ВыполнитьПереоценкуЗаПериод(МассивОрганизаций, НачалоМесяца(Дата), КонецМесяца(Дата), ПараметрыПереоценки);
...
Процедура ПровестиДокументПереоценки(Организация, Период, Операция, ПараметрыПереоценки = Неопределено)
	...
	Попытка
		...
		НачатПереходНаНовуюАрхитектуруВзаиморасчетов = Константы.НачатПереходНаНовуюАрхитектуруВзаиморасчетов.Получить();
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(Выборка.Документ) Тогда
				...
				Если НачатПереходНаНовуюАрхитектуруВзаиморасчетов Тогда
