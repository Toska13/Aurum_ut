...
Процедура ЗаполнитьПараметрыОтбора()
	...
	ПараметрыОтбора.ФормаОплаты = ДенежныеСредстваСервер.ЗначениеПараметраФормыОплатыЗаявки(ПараметрыОтбора.ФормаОплаты);
...
Функция НовыйПараметрыОтбораЗаявок()
	...
	Отбор.Вставить("ФормаОплаты", Неопределено);
...
Функция ТекстВыборкиЗаявок()
	...
	|ГДЕ
	|	ЗаявкаНаКомандировку.ФормаОплатыЗаявки В (&ФормаОплаты)
