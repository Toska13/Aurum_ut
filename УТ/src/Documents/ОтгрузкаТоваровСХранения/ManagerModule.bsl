...
Функция ДобавитьКомандуСоздатьНаОснованииПоГруппеСкладов(КомандыСозданияНаОсновании) Экспорт
	Если ПравоДоступа("Добавление", Метаданные.Документы.ОтгрузкаТоваровСХранения) Тогда
		КомандаСоздатьНаОсновании = КомандыСозданияНаОсновании.Добавить();
		КомандаСоздатьНаОсновании.Менеджер = Метаданные.Документы.ОтгрузкаТоваровСХранения.ПолноеИмя();
		КомандаСоздатьНаОсновании.Обработчик = "СозданиеНаОснованииУТКлиент.СоздатьОтгрузкуТоваровСХраненияНаОснованииПоГруппеСкладов";
...
Функция ПорядокЗаполненияНазначенийВДокументе(Форма) Экспорт
	...
	ПорядокОбработкиДокумента.ИмяРеквизитаПоЗаказу = "ПоЗаказу";
...
Функция ПараметрыСозданияПоГруппеСкладов(ПараметрКоманды) Экспорт
	...
	Если ТипЗнч(ПараметрКоманды) = Тип("ДокументСсылка.АктКонтроляКачестваТоваров") Тогда
		...
		|	И Товары.Действие = ЗНАЧЕНИЕ(Перечисление.ДействияПоТоварамВАктеКонтроляКачества.ОформитьВозврат)";
...
Функция ТекстЗапросаТаблицаРаспоряженияНаОтгрузку(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ПО ТаблицаПоказателей.Ссылка В (ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Оформлено),
...
Процедура ЗаполнитьСписокХозяйственныхОпераций(СписокОпераций) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОтветственноеХранениеВПроцессеЗакупки") Тогда
		СписокОпераций.Добавить(
			...
			НСтр("ru = 'Отгрузка поклажедателю'"));
	КонецЕсли;
...
Функция СтруктураОснования(Объект)
	...
	СтруктураОснование.Вставить("ОснованиеДата");
		...
		Если ЗначениеЗаполнено(Объект.Основание)
			...
			СтруктураОснование.ОснованиеДата = СтруктураРеквизитов.Дата;
			...
			СтруктураОснование.ОснованиеДляПечати = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				Формат(СтруктураОснование.ОснованиеДата, "ДЛФ=DD"));
		...
		ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтгрузкаПринятыхСПравомПродажиТоваровСХранения Тогда
			...
			ДокументыПоступления = Объект.Товары.Выгрузить(, "ДокументПоступления").ВыгрузитьКолонку("ДокументПоступления");
			
			Запрос = Новый Запрос(
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	ПриемкаТоваровНаХранение.НомерВходящегоДокумента КАК НомерВходящегоДокумента,
			|	ПриемкаТоваровНаХранение.ДатаВходящегоДокумента КАК ДатаВходящегоДокумента,
			...
			|ГДЕ
			|	ПриемкаТоваровНаХранение.Ссылка В (&ДокументыПоступления)");
			
			Запрос.УстановитьПараметр("ДокументыПоступления", ДокументыПоступления);
			Запрос.УстановитьПараметр("Синоним", НСтр("ru='Приемка товаров на хранение'"));
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ЗначениеЗаполнено(Выборка.НомерВходящегоДокумента) И ЗначениеЗаполнено(Выборка.ДатаВходящегоДокумента) Тогда
					...
					ИнформацияОПоступлении =
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '%1 № %2 от %3'"),
							Выборка.Синоним,
							Выборка.НомерВходящегоДокумента,
							Формат(Выборка.ДатаВходящегоДокумента, "ДЛФ=ДД"));
					
					ДатаОснования = Выборка.ДатаВходящегоДокумента;
					НомерОснования = Выборка.НомерВходящегоДокумента;
					
				Иначе
					
					ИнформацияОПоступлении = ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(Выборка, Выборка.Синоним);
					ДатаОснования = Выборка.Дата;
					НомерОснования = Выборка.Номер;
					
				КонецЕсли;
				
				ТекстПоПоступлениям = ТекстПоПоступлениям + ", " + ИнформацияОПоступлении;
				
			КонецЦикла;
			
			СтруктураОснование.ОснованиеДляПечати = СокрЛП(Сред(ТекстПоПоступлениям, 3));
			СтруктураОснование.ОснованиеДата = ?(ОдноОснование, ДатаОснования, "");
			СтруктураОснование.ОснованиеНомер = ?(ОдноОснование, ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(НомерОснования), "");
...
Функция СформироватьТаблицуОтгрузки()
	...
	ТоварыДляОтгрузки.Колонки.Добавить("Количество", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
	...
	ТоварыДляОтгрузки.Колонки.Добавить("КоличествоВЗаказе", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
	...
	ТоварыДляОтгрузки.Колонки.Добавить("КоличествоДокумента", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
	...
	ТоварыДляОтгрузки.Колонки.Добавить("КоличествоУпаковок", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
	ТоварыДляОтгрузки.Колонки.Добавить("КоличествоВОтчете", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
	ТоварыДляОтгрузки.Колонки.Добавить("КоличествоОстаток", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
	ТоварыДляОтгрузки.Колонки.Добавить("КоличествоСобирается", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
	ТоварыДляОтгрузки.Колонки.Добавить("КоличествоСобрано", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
	ТоварыДляОтгрузки.Колонки.Добавить("КоличествоВНакладной", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
	ТоварыДляОтгрузки.Колонки.Добавить("КоличествоПодобрано", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
...
Функция ПолучитьТаблицуОстатковПоОтгрузке(ПараметрыДанных) Экспорт
	...
	Для каждого СтрокаПоЗаказуИОтгрузки Из ДанныеПоЗаказуИОтгрузке Цикл
		ОтборПоТовару = ПолучитьСтруктуруПоиска();
		...
		ОтборПоТоваруБезСерии = ПолучитьСтруктуруПоиска(Ложь);
		ЗаполнитьЗначенияСвойств(ОтборПоТоваруБезСерии, ОтборПоТовару);
		
		СтрокиОстатка = ДанныеПоОстаткам.НайтиСтроки(ОтборПоТовару);
		
		НоваяСтрокаОтгрузки = ТоварыДляОтгрузки.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрокаОтгрузки, СтрокаПоЗаказуИОтгрузки, "Номенклатура, Характеристика, Серия, Назначение, ЗаказКлиента, Цена,
			|Упаковка, Коэффициент, КоличествоВНакладной, Склад, ОрдернаяСхемаПриОтгрузке, Сумма");
		...
		Если СтрокиСОстаткамиПоОрганизации.Количество() = 0 Тогда
			СтрокиСОстаткамиПоОрганизации = ДанныеПоОстаткамОрганизации.НайтиСтроки(ОтборПоТоваруБезСерии);
		КонецЕсли;
		Для каждого СтрокаОстаткаПоОрганизации Из СтрокиСОстаткамиПоОрганизации Цикл
			...
			Иначе
				КоэффициентПересчета = ?(СтрокаПоЗаказуИОтгрузки.Коэффициент > 0, СтрокаПоЗаказуИОтгрузки.Коэффициент, 1);
				НоваяСтрокаОтгрузки.Цена = ?(ЗначениеЗаполнено(НоваяСтрокаОтгрузки.Цена), НоваяСтрокаОтгрузки.Цена, СтрокаОстаткаПоОрганизации.Цена * КоэффициентПересчета);
				НоваяСтрокаОтгрузки.Сумма = НоваяСтрокаОтгрузки.Цена * (КоличествоОстаток / КоэффициентПересчета);
			КонецЕсли;
		...
		Если НоваяСтрокаОтгрузки.ОрдернаяСхемаПриОтгрузке Тогда
			...
			Для каждого СтрокаОрдера Из СтрокиОрдеров Цикл
				Если СтрокаОрдера.КоличествоСобирается <> 0 Тогда
					КоличествоСобирается = Мин((СтрокаОрдера.КоличествоСобирается/СтрокаПоЗаказуИОтгрузки.Коэффициент)*СтрокаПоЗаказуИОтгрузки.Коэффициент, НоваяСтрокаОтгрузки.КоличествоОстаток, НоваяСтрокаОтгрузки.КоличествоВЗаказе);
				...
				Если СтрокаОрдера.Количество <> 0 Тогда
					КоличествоСобрано = Мин((СтрокаОрдера.Количество/СтрокаПоЗаказуИОтгрузки.Коэффициент)*СтрокаПоЗаказуИОтгрузки.Коэффициент, НоваяСтрокаОтгрузки.КоличествоОстаток, НоваяСтрокаОтгрузки.КоличествоВЗаказе);
	...
	ТоварыДляОтгрузки.Сортировать("ЗаказКлиента, Номенклатура, Характеристика, Серия, Назначение, Упаковка, Склад");
...
Процедура ЗаполнитьТекстЗапросаПоЗаказу(Запрос)
	...
	|			- РаспоряженияНаОтгрузкуОбороты.ОформленоКоличествоОборот) КАК КОформлению,
	|		(РаспоряженияНаОтгрузкуОбороты.КОформлениюСуммаОборот
	|			- РаспоряженияНаОтгрузкуОбороты.ОформленоСуммаОборот) КАК Сумма
	|	ИЗ
	|		#РаспоряженияНаОтгрузкуОборотыРазвернутая1 КАК РаспоряженияНаОтгрузкуОбороты
	|	ГДЕ
	|		(РаспоряженияНаОтгрузкуОбороты.КОформлениюКоличествоОборот
	|			- РаспоряженияНаОтгрузкуОбороты.ОформленоКоличествоОборот) <> 0
	|		ИЛИ (РаспоряженияНаОтгрузкуОбороты.КОформлениюСуммаОборот
	|			- РаспоряженияНаОтгрузкуОбороты.ОформленоСуммаОборот) <> 0
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		ЗаказыОстатки.Серия,
	|		0,
	|		ЗаказыОстатки.ОформленоКоличествоОборот,
	|		ВЫРАЗИТЬ(ЗаказыОстатки.Распоряжение КАК Документ.ЗаказКлиента).Назначение
	|	ИЗ
	|		#РаспоряженияНаОтгрузкуОборотыРазвернутая2 КАК ЗаказыОстатки
	|	ГДЕ
	...
	ПараметрыФормированияТаблицы.ТекстУсловия =
		...
		|					ТаблицаАналитикНоменклатуры.Номенклатура
		|				ИЗ
		|					ТаблицаАналитикНоменклатуры)"; // @query-part
	ПараметрыФормированияТаблицы.Группировка = "Регистратор, Распоряжение, Номенклатура, Характеристика, Склад, Серия";
...
Функция ТекстЗапросаТаблицаЗаказы(ИмяВременнойТаблицы = "") Экспорт
	...
	|		ВЫБОР
	|			КОГДА ИСТИНА
	|				ТОГДА (РаспоряженияОбороты.КОформлениюКоличествоОборот
	...
	|		ИЛИ (РаспоряженияОбороты.КОформлениюКоличествоОборот
	...
	|		И
	|			ЗаказКлиентаТовары.Номенклатура.ВариантОформленияПродажи = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияПродажи.РеализацияТоваровУслуг)
	...
	|			КОГДА РаспоряженияДвижения.Показатель = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Оформлено)
	|				ТОГДА РаспоряженияДвижения.Количество
	|		КОНЕЦ КАК Количество
	|	ИЗ
	|		РегистрНакопления.РаспоряженияНаОтгрузкуИВозврат КАК РаспоряженияДвижения
	...
	ПараметрыФормированияТаблицы.Показатели = "КОформлению, Оформлено, КОтгрузке, Отгружено";
	ПараметрыФормированияТаблицы.Ресурсы = "Количество";
...
Функция ПолучитьСтруктуруПоиска(ПоСериям = Истина)
