...
Процедура ЗаполнитьТаблицуТовары()
	...
	ОбрабатываемыеСтрокиПересчетСумм = Новый Массив;
	...
	Для каждого СтрокаОтгрузки Из ДанныеПоОтгрузке Цикл
		...
		Если ПересчитыватьСумму Тогда
			ОбрабатываемыеСтрокиПересчетСумм.Добавить(СтрокаТовара);
		Иначе
	...
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействий, Истина);
	
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		ОбрабатываемыеСтрокиПересчетСумм,
		СтруктураДействий,
		ПодобранныеТовары,
		КэшированныеЗначения);
	...
	Если ЗаказыКлиентов.Количество() > 0 Тогда
		ОтгрузкаТоваровСХраненияЛокализация.ЗаполнитьШтрихКодыУпаковок(
			ЗаказыКлиентов, ПодобранныеТовары.Выгрузить(), ШтрихкодыУпаковокНакладной);
	КонецЕсли;
...
Функция АдресТоваровВХранилище()
	...
	Результат.Вставить("Товары", ОтобранныеТовары);

	// формирование данных по маркам (если они есть) для загрузки в документ
	ШтрихкодыУпаковок = ТаблицаШтрихкодыУпаковок.Выгрузить(Новый Массив);
