...
Процедура ХозяйственнаяОперацияПриИзменении(Элемент)
	...
	СтараяХозяйственнаяОперация = ХозяйственнаяОперацияДоИзменения;
	
	Если Объект.ХозяйственнаяОперация = СтараяХозяйственнаяОперация Тогда
		Возврат;
	КонецЕсли;
	
	НужнаОчисткаТоваров = Ложь;
	НужноОчиститьПартнера = Ложь;
	
	ПоставкаПодПринципала = ПредопределенноеЗначение(
		"Перечисление.ХозяйственныеОперации.ПоставкаПодПринципала");
	
	// Определяем необходимость очистки товаров для операций, не требующих товаров
	Если Объект.ХозяйственнаяОперация = ПоставкаПодПринципала И Объект.ПоЗаказу Тогда
		НужнаОчисткаТоваров = Истина;
	КонецЕсли;
	
	
	Если НужнаОчисткаТоваров И Объект.Товары.Количество() > 0 Тогда
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("СтараяХозяйственнаяОперация", СтараяХозяйственнаяОперация);
		ДополнительныеПараметры.Вставить("НужноОчиститьПартнера", НужноОчиститьПартнера);
		ДополнительныеПараметры.Вставить("НужнаОчиститьТовары", Истина);
		
		Оповещение = Новый ОписаниеОповещения(
			"ХозяйственнаяОперацияПриИзмененииЗавершение",
			ЭтотОбъект,
			ДополнительныеПараметры);
		
		СписокКнопок = Новый СписокЗначений;
		СписокКнопок.Добавить("ОчиститьТовары", НСтр("ru='Очистить товары'"));
		СписокКнопок.Добавить("Отмена", НСтр("ru='Отмена'"));
		
		ПоказатьВопрос(
			Оповещение,
			НСтр("ru='При изменении хозяйственной операции необходимо очистить табличную часть Товары. Очистить товары?'"),
			СписокКнопок,
			,
			"ОчиститьТовары");
	...
	ВыполнитьДействияПриИзмененииХозяйственнойОперации(НужноОчиститьПартнера);
...
Процедура ЗаполнитьДокументПоступления(Команда)
	...
	Если Элементы.Товары.ВыделенныеСтроки.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Необходимо выбрать строки, для которых необходимо заполнить документ поступления.'");
...
Процедура ОрганизацияПриИзмененииСервер()
	...
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Валюта) Тогда
			...
			Объект.Валюта = ЗначениеНастроекКлиентСерверПовтИсп.ВалютаРегламентированногоУчетаОрганизации(
				Объект.Организация);
...
Процедура ИзменитьТабличнуюЧастьПоРезультатамПроверки(ВозвращаемыеПараметры, КэшированныеЗначения)
	...
	Если Не ОбрабатываемыеСтрокиНовые.Количество() = 0 Тогда
		ОбновитьЗависимыеРеквизитыФормыПоЗаказуСервер();
	КонецЕсли;
...
Процедура ПолучитьЗагруженныеТоварыИзХранилища(АдресТоваровВХранилище)
	...
	Для Каждого СтрокаТоваров Из Объект.Товары Цикл
		ЗаполнитьЗаказКлиентаПоНазначению(СтрокаТЧТовары, Объект, СписокОснований);
	КонецЦикла;
...
Процедура ОбновитьЗависимыеРеквизитыФормыПоЗаказуСервер()
	...
	Если Объект.Товары.Количество() = 0
	 ...
	 Или Не ИспользоватьЗаказыДавальцев2_5 Тогда
		...
		Элементы.ГруппаНадписьРасхождениеЗаказ.Видимость = Ложь;
		Элементы.ГруппаНадписьРасхождениеЗаказы.Видимость = Ложь;
...
Процедура ОбновитьЗависимыеРеквизитыФормыПоЗаказу(Товары, НадписьРасхождениеЗаказ, ЭлементыФормы)
	...
	ЭлементыФормы.ГруппаНадписьРасхождениеЗаказ.Видимость = КоличествоРасхождений > 0;
	ЭлементыФормы.ГруппаНадписьРасхождениеЗаказы.Видимость = КоличествоРасхождений > 0;
...
&НаКлиенте
Процедура ХозяйственнаяОперацияПриИзмененииЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	
	Если Ответ = "ОчиститьТовары" Тогда
		ВыполнитьДействияПриИзмененииХозяйственнойОперации(
			ДополнительныеПараметры.НужноОчиститьПартнера,
			ДополнительныеПараметры.НужнаОчиститьТовары,
			Ложь);
	ИначеЕсли Ответ = "Отмена" Тогда
		Объект.ХозяйственнаяОперация = ДополнительныеПараметры.СтараяХозяйственнаяОперация;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьДействияПриИзмененииХозяйственнойОперации(НужноОчиститьПартнера, НужнаОчиститьТовары = Ложь, ПропуститьНастройкуХозОперации = Ложь)
	
	Если НужноОчиститьПартнера Тогда
	...
	Если НужнаОчиститьТовары И Объект.Товары.Количество() > 0 Тогда
...
Процедура УстановитьВозможностьРедактированияНД()
	...
	ПараметрыДоступностиНД.НакладнаяПоЗаказу = Объект.ПоЗаказу;
