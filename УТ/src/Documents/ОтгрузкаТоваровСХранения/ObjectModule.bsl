		...
		ИначеЕсли ДанныеЗаполнения.Свойство("АктОРасхождениях")
			...
			Если ТипЗнч(ДанныеЗаполнения.ОснованиеАкта) = Тип("ДокументСсылка.ОтгрузкаТоваровСХранения") Тогда
				Если ДанныеЗаполнения.Свойство("Исправление") Тогда
					...
					КорректировкаИсправленияДокументаНаОснованииАктаОРасхожденияхПоОтгрузкеТоваровСХранения(ДанныеЗаполнения);
				Иначе
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ПартнерыИКонтрагенты.ПроверитьЗаполнениеПартнера(ЭтотОбъект, МассивНепроверяемыхРеквизитов, Отказ, Истина);
	...
	Для ТекИндекс = 0 По Товары.Количество()-1 Цикл
		...
		Если ПоЗаказу
			...
			ТекстОшибки = НСтр("ru='Не заполнено поле ""%ПредставлениеЗаказа%"" в строке %НомерСтроки% списка ""Товары""'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ПредставлениеЗаказа%", ПредставлениеЗаказа);
...
Процедура ЗаполнитьНаОснованииАктОРасхожденияхПоОтгрузкеТоваровСХранения(Знач ДанныеЗаполнения)
	...
	Для Каждого ТекСтрока Из ТаблицаТоваров Цикл
		...
		Если Не ЗначениеЗаполнено(НоваяСтрока.ЗаказКлиента) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура КорректировкаИсправленияДокументаНаОснованииАктаОРасхожденияхПоОтгрузкеТоваровСХранения(Знач ДанныеЗаполнения)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТоварыАкта.КодСтроки             КАК КодСтроки,
	|	ТоварыАкта.ЗаполненоПоРеализации КАК ЗаполненоПоРеализации,
	...
	|	ВЫБОР
	|		КОГДА ТоварыАкта.ДокументПоступления = Неопределено
	|				ИЛИ ТоварыАкта.ДокументПоступления ССЫЛКА Документ.ПриобретениеТоваровУслуг
	|			ТОГДА ЗНАЧЕНИЕ(Документ.ПриемкаТоваровНаХранение.ПустаяСсылка)
	|		ИНАЧЕ ТоварыАкта.ДокументПоступления
	|	КОНЕЦ                     КАК ДокументПоступления,
	|	ТоварыАкта.Количество     КАК Количество,
	|	ТоварыАкта.Сумма          КАК Сумма
	|ИЗ
	...
	|	И ТоварыАкта.Действие В (ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ПокупкаПерепоставленного),
	|							ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ПустаяСсылка),
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СерииАкта.НомерСтроки    КАК НомерСтроки,
	...
	|	СерииАкта.Количество     КАК Количество
	|ИЗ
	...
	|	И СерииАкта.Действие В (ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ПокупкаПерепоставленного),
	|							ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ПустаяСсылка),
	|							ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ВозвратПерепоставленного),
	|							ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ДопоставкаНеТребуется),
	|							ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ТребуетсяДопоставка))
	|	И СерииАкта.Количество > 0
	|	И НЕ (НЕ СерииАкта.ЗаполненоПоРеализации
	|		И СерииАкта.Действие = ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ПустаяСсылка))";
	...
	ТаблицаТоварыАкта = ПакетЗапросов[0].Выгрузить();
	ТаблицаСерииАкта  = ПакетЗапросов[1].Выгрузить();
	
	ТаблицаТовары = Товары.Выгрузить();
	ТаблицаСерии  = Серии.Выгрузить();
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтгрузкаПринятыхСПравомПродажиТоваровСХранения Тогда
		ДополнительныеКолонки = "КодСтроки, ДокументПоступления";
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоставкаПодПринципала
		И ПоЗаказу Тогда
		ДополнительныеКолонки = "КодСтроки, ЗаказКлиента";
	КонецЕсли;
	КолонкиОтбора = КолонкиОтбора + "," + ДополнительныеКолонки;
	ПараметрыОтбора = Новый Структура(КолонкиОтбора);
	
	Для Каждого ТекСтрока Из ТаблицаТоварыАкта Цикл
		
		Если ТекСтрока.ЗаполненоПоРеализации Тогда
			ЗаполнитьЗначенияСвойств(ПараметрыОтбора, ТекСтрока);
			НайденныеСтроки = ТаблицаТовары.НайтиСтроки(ПараметрыОтбора);
			Если НайденныеСтроки.Количество() > 0 Тогда
				НайденнаяСтрока = НайденныеСтроки[0];
				НайденнаяСтрока.Количество = ТекСтрока.Количество;
				НайденнаяСтрока.Сумма      = ТекСтрока.Сумма;
			Иначе
				НоваяСтрока = ТаблицаТовары.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрока);
			КонецЕсли;
		Иначе // для добавления строк из акта сверх заказа
			НоваяСтрока = ТаблицаТовары.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрока);
		КонецЕсли;
	...
	Для Каждого ТекСтрока Из ТаблицаСерииАкта Цикл
		
		ЗаполнитьЗначенияСвойств(ПараметрыОтбора, ТекСтрока);
		
		НайденныеСтроки = ТаблицаСерии.НайтиСтроки(ПараметрыОтбора);
		Если НайденныеСтроки.Количество() > 0 Тогда
			
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				
				Если НайденнаяСтрока.Количество = ТекСтрока.Количество Тогда
					Продолжить;
				КонецЕсли;
				...
				Иначе
					...
					НоваяСтрока.Количество = ТекСтрока.Количество - НайденнаяСтрока.Количество;
				КонецЕсли;
...
Процедура ЗаполнитьНаОснованииАктОРасхожденияхПоПриемкеТоваровНаХранение(Знач ДанныеЗаполнения)
	...
	|	&ДокументОснованиеИсправлениеОснования                                КАК ДокументПоступления
	|ИЗ
...
Процедура ЗаполнитьПоАктуКонтроляКачества(Знач ДанныеЗаполнения)
	...
	Документы.АктКонтроляКачестваТоваров.ТоварыДляСозданияНаОсновании(
		...
		Перечисления.ДействияПоТоварамВАктеКонтроляКачества.ОформитьВозврат);
	
	Запрос.УстановитьПараметр("Ссылка", ДанныеЗаполнения.Основание);
	Запрос.Текст =
		"ВЫБРАТЬ
		|	Реквизиты.КонтролируемаяПартия.Организация КАК Организация,
		|	Реквизиты.КонтролируемаяПартия.Партнер КАК Партнер,
		|	Реквизиты.КонтролируемаяПартия.Контрагент КАК Контрагент,
		|	Реквизиты.КонтролируемаяПартия.Партия КАК Партия,
		|	Реквизиты.КонтролируемаяПартия.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
		|	Реквизиты.КонтролируемаяПартия.Договор КАК Договор,
		|	Реквизиты.КонтролируемаяПартия.НаправлениеДеятельности КАК НаправлениеДеятельности,
		|	Реквизиты.КонтролируемаяПартия.Подразделение КАК Подразделение,
		|	Реквизиты.Статус КАК Статус
		|ПОМЕСТИТЬ ШапкаАкта
		|ИЗ
		|	Документ.АктКонтроляКачестваТоваров КАК Реквизиты
		|ГДЕ
		...
		|ПОМЕСТИТЬ Шапка
		|ИЗ
		|	ШапкаАкта КАК Реквизиты
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриемкаТоваровНаХранение КАК ПриемкаТоваровНаХранение
		|		ПО ПриемкаТоваровНаХранение.Ссылка = Реквизиты.Партия
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	Реквизиты.Соглашение КАК Соглашение,
		|	Реквизиты.НаправлениеДеятельности КАК НаправлениеДеятельности,
		...
		|	Реквизиты.БанковскийСчетКонтрагента КАК БанковскийСчетКонтрагента,
		...
		|	МАКСИМУМ(Реквизиты.ДокументПоступления) КАК ДокументПоступления,
		|	ВЫБОР
		|		КОГДА Товары.СтатусУказанияСерий В(10,14)
		|			ТОГДА Товары.Серия
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК Серия,
		...
		|ИЗ
		|	Товары КАК Товары
		|		ЛЕВОЕ СОЕДИНЕНИЕ Шапка КАК Реквизиты
		|		ПО ИСТИНА
		|СГРУППИРОВАТЬ ПО
		|	Товары.Номенклатура,
		...
		|	Товары КАК Товары
		|ГДЕ
		|	НЕ Товары.СтатусУказанияСерий В(10,14)
		|	И Товары.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|	И НЕ (Товары.Склад.ИспользоватьОрдернуюСхемуПриОтгрузке
		|		И &ДатаДокумента <= Товары.Склад.ДатаНачалаОрдернойСхемыПриОтгрузке)
		|СГРУППИРОВАТЬ ПО
		|	Товары.Номенклатура,
		|	Товары.Характеристика,
		|	Товары.Назначение,
	...
	РеквизитыАкта = ПакетРезультатов[1].Выбрать();
	РеквизитыАкта.Следующий();
	РеквизитыШапки = ПакетРезультатов[3].Выбрать();
	РеквизитыШапки.Следующий();
	КонтрольКачества.ВызватьИсключениеПриСозданииНаОснованииАкта(РеквизитыАкта);
	ТоварыОснования = ПакетРезультатов[4].Выгрузить();
	СерииОснования = ПакетРезультатов[5].Выгрузить();
	
	Если ТоварыОснования.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ru='Документ %Документ% не содержит товаров c действием ""Оформить возврат"". Ввод на основании документа запрещен.'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения.Основание);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	
	ХозяйственнаяОперация = ЗакупкиСервер.ПолучитьХозяйственнуюОперациюВозвратаПоПоступлению(РеквизитыАкта.ХозяйственнаяОперация);
	...
	Если ВсеОперацииДокумента.НайтиПоЗначению(ХозяйственнаяОперация) = Неопределено Тогда
		ТекстОшибки = НСтр("ru='Для операции ""%1"", по которой поступил товар, не предполагается создание документа ""Отгрузка товаров с хранения"". Ввод на основании документа запрещен.'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, РеквизитыАкта.ХозяйственнаяОперация);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	Если ТипЗнч(РеквизитыШапки.Партия) <> Тип("СправочникСсылка.ДоговорыКонтрагентов")
			И ТипЗнч(РеквизитыШапки.Партия) <> Тип("ДокументСсылка.ЗаявкаНаКонтрольКачестваТоваров") Тогда
		Валюта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыШапки.Партия, "Валюта");
	КонецЕсли;
	Товары.Загрузить(ТоварыОснования);
	Серии.Загрузить(СерииОснования);
	ПараметрыУказанияСерий = НоменклатураСервер.ПараметрыУказанияСерий(ЭтотОбъект, Документы.ОтгрузкаТоваровСХранения);
...
Процедура ЗаполнитьДокументПоЗаказуКлиента(Знач ДанныеЗаполнения, ПоОрдерам = Ложь)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		Если РеквизитыШапки.ЗапрещеноВыбиратьГруппуСкладов Тогда
