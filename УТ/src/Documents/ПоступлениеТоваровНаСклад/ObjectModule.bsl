...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ПараметрыУказанияСерий = НоменклатураСервер.ПараметрыУказанияСерий(ЭтотОбъект, Документы.ПоступлениеТоваровНаСклад);
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипДанныхЗаполнения = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		ЗаполнитьДокументНаОснованииЗаказаПоставщику(ДанныеЗаполнения, ДанныеЗаполнения.Склад, 
...
Функция ТекстЗапросаЗаполненияНаОсновании(ПоДаннымАктаОРасхождениях = Ложь)
	...
	|	МАКСИМУМ(ТоварыПриобретения.НоменклатураПартнера) КАК НоменклатураПартнера
	|
	|ПОМЕСТИТЬ ВТНоменклатураПартнераПоПриобретению
	|ИЗ
	|	Документ.ПриобретениеТоваровУслуг.Товары КАК ТоварыПриобретения
	|
	|ГДЕ
	|	&ИспользоватьНоменклатуруПартнеров
	|	И ТоварыПриобретения.Ссылка = &ДокументОснование
	|
	|СГРУППИРОВАТЬ ПО
	...
	|
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТоварыПриобретения.НоменклатураПартнера) = 1
	|;
	|
	|//////////////////////////////////////////////////////////////////////////////// 7
	|ВЫБРАТЬ
	...
	|	НоменклатураПартнераПоПриобретению.НоменклатураПартнера КАК НоменклатураПартнера
	|ИЗ
	|	(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		ТоварыКПоступлению.ДокументПоступления	КАК ДокументПоступления,
	|		ТоварыКПоступлению.Склад				КАК Склад,
	...
	|	) КАК Товары
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТНоменклатураПартнераПоПриобретению КАК НоменклатураПартнераПоПриобретению
	|		ПО Товары.Номенклатура = НоменклатураПартнераПоПриобретению.Номенклатура
	|		И Товары.Характеристика = НоменклатураПартнераПоПриобретению.Характеристика
	|		И Товары.Упаковка = НоменклатураПартнераПоПриобретению.Упаковка
	|		И Товары.Назначение = НоменклатураПартнераПоПриобретению.Назначение
	|		И Товары.Серия = НоменклатураПартнераПоПриобретению.Серия
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	Товары.Цена,
	|	Товары.СтавкаНДС,
	|	НоменклатураПартнераПоПриобретению.НоменклатураПартнера
	|
	|УПОРЯДОЧИТЬ ПО
	|	Номенклатура,
	|	Характеристика,
...
Процедура ЗаполнитьДокументНаОснованииЗаказаПоставщику(ДокументОснование,
	...
	|	МАКСИМУМ(ЗаказПоставщикуТовары.НоменклатураПартнера) КАК НоменклатураПартнера
	|
	|ПОМЕСТИТЬ ВТНоменклатураПартнераПоОснованию
	|ИЗ
	...
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказПоставщикуТовары.НоменклатураПартнера) = 1
	|;
	|
	|//////////////////////////////////////////////////////////////////////////////// 7
	|ВЫБРАТЬ
	...
	|	Товары.Цена							КАК Цена,
	|	Товары.СтавкаНДС					КАК СтавкаНДС,
	|	НоменклатураПартнераПоОснованию.НоменклатураПартнера КАК НоменклатураПартнера
	|ИЗ
	|	(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		ТоварыКПоступлению.ДокументПоступления	КАК ДокументПоступления,
	|		ТоварыКПоступлению.Склад				КАК Склад,
	...
	|	) КАК Товары
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТНоменклатураПартнераПоОснованию КАК НоменклатураПартнераПоОснованию
	|		ПО Товары.Номенклатура = НоменклатураПартнераПоОснованию.Номенклатура
	|		И Товары.Характеристика = НоменклатураПартнераПоОснованию.Характеристика
	|		И Товары.Упаковка = НоменклатураПартнераПоОснованию.Упаковка
	|
	|СГРУППИРОВАТЬ ПО
	|	Товары.ДокументПоступления,
	|	Товары.Номенклатура,
	...
	|	Товары.Цена,
	|	Товары.СтавкаНДС,
	|	НоменклатураПартнераПоОснованию.НоменклатураПартнера
	|
	|УПОРЯДОЧИТЬ ПО
	|	Номенклатура,
	|	Характеристика,
	...
	ЗаполнитьУпаковкиЦеныНоменклатуруПартнера(ТЗТовары, ТЗУпаковки);
	ПересчитатьКоличествоУпаковокЗаполнитьСтавкуНДС();
...
Процедура ЗаполнитьДокументНаОснованииАктаПриемкиПослеПоступления(ДанныеЗаполнения)
	...
	АктОРасхожденияхПослеПриемкиОснование = АктОРасхождениях;
...
Процедура ЗаполнитьУпаковкиЦеныНоменклатуруПартнера(ТЗТовары, ТЗУпаковки)
	
	СвойстваУпаковок	= "Упаковка, Количество, КоличествоУпаковок, Цена, СтавкаНДС, НоменклатураПартнера";
...
Процедура ЗаполнитьДокументНаОснованииДоговора(ДокументОснование,
	...
	Запрос = Новый Запрос(
		...
		|			ТОГДА Склад = &СкладПоступления
		|			ИНАЧЕ Склад <> ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
...
Функция ВременныеТаблицыДанныхДокумента(ДляТоваровУПартнеров, УчетПоНомерамГТД = Ложь)
	...
	//++ Локализация
	|ГДЕ
	|	Аналитика.Номенклатура.ВестиУчетПоГТД = &УчетПоНомерамГТД
	//-- Локализация
	|
	|ИНДЕКСИРОВАТЬ ПО
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	//++ Локализация
	МенеджерВременныхТаблицТоварыСобственныеУчетПоГТД = ВременныеТаблицыДанныхДокумента(Ложь, Истина);
	
	ЗапасыСервер.ЗаполнитьВидыЗапасовПоУмолчанию(МенеджерВременныхТаблицТоварыСобственныеУчетПоГТД, Товары);
	...
	МенеджерВременныхТаблицУчетПоГТД = ВременныеТаблицыДанныхДокумента(Истина, Истина);
	
	Если Не Проведен
		Или ПерезаполнитьВидыЗапасов
		Или ПроверитьИзменениеРеквизитовДокумента(МенеджерВременныхТаблицУчетПоГТД)
		Или ЗапасыСервер.ПроверитьИзменениеТоваровПоКоличествуИСумме(МенеджерВременныхТаблицТоварыСобственныеУчетПоГТД)
		Или ЗапасыСервер.ПроверитьИзменениеТоваровПоКоличествуИСумме(МенеджерВременныхТаблицУчетПоГТД) Тогда
		...
		ЗапасыСервер.ЗаполнитьВидыЗапасовПоОстаткамКОформлению(ЭтотОбъект,
																МенеджерВременныхТаблицУчетПоГТД,
																Отказ,
																ПараметрыЗаполнения);
...
Процедура ЗаполнитьСуммовыеПоказателиСтрокиТоваров(СтрокаТоваров)
	...
	Если Не ЗначениеЗаполнено(СтрокаТоваров.СтавкаНДС) Тогда
		ДействияСоСтрокой.Вставить("ЗаполнитьСтавкуНДС", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(ЭтотОбъект));
	КонецЕсли;
...
Процедура СвернутьСтрокиТоваров()
	...
	Если ОперацияТоварыВПути Тогда
		КолонкиГруппировки	= "Номенклатура, НоменклатураПартнера, Характеристика, Назначение, Серия, Упаковка";
		КолонкиСуммирования	= "Количество, КоличествоУпаковок";
	Иначе
		КолонкиГруппировки	= "Номенклатура, НоменклатураПартнера, Характеристика, Назначение, Серия, Упаковка, Цена, СтавкаНДС";
