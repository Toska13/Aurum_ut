...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ Локализация
	ИспользуетсяСервисСверки = ПолучитьФункциональнуюОпцию("ИспользуетсяСервисСверкиРасчетов");
	//-- Локализация
	...
	УстановитьПереданныеПараметры(ЭтотОбъект);
...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	...
	Если НастройкиОтбора <> Неопределено Тогда
		Если НЕ Параметры.ЗаменитьНастройкиФормы Тогда
			КомпоновкаДанныхКлиентСервер.СкопироватьЭлементы(КомпоновщикОтбор.Настройки.Отбор, НастройкиОтбора.Отбор);
		КонецЕсли;
		УстановитьПереданныеПараметры(Настройки);
		КомпоновкаДанныхКлиентСервер.СкопироватьЭлементы(Настройки["КомпоновщикОтбор.Настройки.Отбор"], КомпоновщикОтбор.Настройки.Отбор);
	КонецЕсли;
...
&НаКлиенте
Процедура НадписьЕстьРасхожденияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	//++ Локализация
	
	ПараметрыОтчета = ПолучитьОтборыСверки();
...
Процедура РасчетыМеждуОрганизациями(Команда)
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		Если РасчетыСКлиентом Тогда
			...
			КонецЕсли;
		ИначеЕсли РасчетыСПоставщиком Тогда
...
Процедура ЗаполнитьТаблицуРасчетовСервер()
	...
	Если ТаблицаРезультатаСКД.Количество() > 0 Тогда
		...
		Если НЕ ИспользуютсяФинИнструменты И НоваяАрхитектураВзаиморасчетов Тогда
			...
			КонецЕсли;
			Группировки = Группировки + ", НаименованиеИсходящихДокументов";
		КонецЕсли;
		//-- Локализация
		...
		Для Каждого Строка Из ТаблицаРасчетов Цикл
			Если НЕ Строка.СуществуютДокументы Тогда
...
&НаКлиенте
Процедура ПереложитьДокументыВРезультат()
	...
	Для Каждого СтрокаРасчетов Из ТаблицаРасчетов Цикл
		Данные = Новый Структура("ДокументСверки,Действие", СтрокаРасчетов.ДокументСверки, "НеИзменять");
		ОтработанныеДокументы.Добавить(Данные);
		МассивВсехДокументовСверки.Добавить(СтрокаРасчетов.ДокументСверки);
	КонецЦикла;
	
	ДокументыСверки = Новый Структура;
	ДокументыСверки.Вставить("ОтработанныеДокументы", ОтработанныеДокументы);
	ДокументыСверки.Вставить("МассивВсехДокументовСверки", МассивВсехДокументовСверки);
	Адрес = ПоместитьВоВременноеХранилище(ДокументыСверки, УникальныйИдентификатор);
	
	Результат = Новый Структура("АдресРезультата", Адрес);
	СоздатьДокументыЗавершение(Результат, Неопределено);
...
Процедура УстановитьВидимостьЭлементовНаСервере()
	...
	Элементы.ГруппаДанныеСервисаСверка20.Видимость = ИспользуетсяСервисСверки;
	//++ Локализация
	МассивСтрок = Новый Массив;
	СтрокаГиперссылки = Новый ФорматированнаяСтрока(
		НСтр("ru = 'По данным сервиса ""Сверка 2.0"" обнаружены'") + " ",
		,
		ЦветаСтиля.ЦветТекстаОтрицательногоЗначения);
	...
	СтрокаГиперссылки = Новый ФорматированнаяСтрока(
		...
		"Расхождения");
	МассивСтрок.Добавить(СтрокаГиперссылки);
	Элементы.ОткрытьОтчетРасхождения.Заголовок = Новый ФорматированнаяСтрока(МассивСтрок);
...
Процедура ОтобразитьНаименованиеИсходящихДокументов()
	...
	ВыводитьНаименования = ПолучитьФункциональнуюОпцию("ВыводитьНаименованияИсходящихДокументовВАктахСверкиИСчетахФактурах");
	СтрокиПодсказки = Новый Массив;
	Если ВыводитьНаименования Тогда
		СтрокиПодсказки.Добавить("Указанное наименование будет использоваться только для документов," + Символы.ПС);
		СтрокиПодсказки.Добавить("по которым не формировалась основная печатная или электронная форма." + Символы.ПС);
		СтрокиПодсказки.Добавить("Иначе будет использоваться наименование ранее сформированной основной формы." + Символы.ПС);
	Иначе
		СтрокиПодсказки.Добавить("Рекомендуется включить функциональную опцию" + Символы.ПС);
		Строка = Новый ФорматированнаяСтрока("Выводить наименования электронных документов или основных печатных форм" + Символы.ПС,Новый Шрифт(,,Истина));
		СтрокиПодсказки.Добавить(Строка);
		СтрокиПодсказки.Добавить("(раздел НСИ и администрирование - Продажи - Печать документов)," + Символы.ПС);
		СтрокиПодсказки.Добавить("чтобы для каждого исходящего документа выводилось наименование основной печатной или электронной формы," + Символы.ПС);
		СтрокиПодсказки.Добавить("сформированной ранее по данному документу." + Символы.ПС);
	КонецЕсли;
	ТекстПодсказки = Новый ФорматированнаяСтрока(СтрокиПодсказки);
	Элементы.НаименованиеИсходящихДокументовРасширеннаяПодсказка.Заголовок = ТекстПодсказки;
...
Процедура ВыполнитьПереходПоСтраницам(Команда)
	...
	Если Команда.Имя = "Далее"
		...
		Если НЕ (РасчетыСКлиентом ИЛИ РасчетыСПоставщиком ИЛИ РасчетыСКредитором ИЛИ РасчетыСДебитором ИЛИ РасчетыСАрендодателем) Тогда
			ТекстСообщения = НСтр("ru = 'Не выбран тип расчетов для сверки.'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , "РасчетыСКлиентом");
			Возврат;
		КонецЕсли;
		...
		//++ Локализация
		Если ИспользуетсяСервисСверки Тогда
	...
	ИначеЕсли Команда.Имя = "Далее"
		...
		Если ПараметрыЗаполнены() И ТаблицаРасчетов.Количество() > 0 Тогда
			...
			ДокументыБезИзменений = ТаблицаРасчетов.НайтиСтроки(Новый Структура("ИзменятьДокумент", "НеИзменять"));
			Если ТаблицаРасчетов.Количество() = ДокументыБезИзменений.Количество() Тогда
				ПереложитьДокументыВРезультат();
			ИначеЕсли ТаблицаРасчетов.Количество() > 0 Тогда
...
Функция ПараметрыФормыРасширенногоОтбора()
	...
	ПараметрыОткрытия.Вставить("СкрыватьСлужебныеОтборы", Ложь);
	ПараметрыОткрытия.Вставить("ЭтоСверкаМеждуОрганизациями", ЭтоСверкаМеждуОрганизациями);
...
Процедура УстановитьОсновныеОтборы(КомпоновщикНастроек = Неопределено)
	...
	Если РасчетыСПоставщиком Тогда
...
Процедура УстановитьОтборКомпоновщика(ИмяОтбора, ЗначениеОтбора, КомпоновщикНастроек = Неопределено)
	...
	Если ИмяОтбора = "Контрагент" И ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов") Тогда
		Если ТипЗнч(ЗначениеОтбора) = Тип("СправочникСсылка.Организации") Тогда
			ЭлементОтбора = ФинансоваяОтчетностьСервер.НайтиЭлементОтбора(
				КомпоновщикНастроек.Настройки.Отбор, "ОрганизацияКонтрагент");
		Иначе
...
Процедура ОбновитьОтборыНаФорме()
	...
	ЭтоСверкаМеждуОрганизациями = ТипЗнч(Контрагент) = Тип("СправочникСсылка.Организации");
	Если ЭтоСверкаМеждуОрганизациями Тогда
		Элементы.Контрагент.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.Организации");
	Иначе
		Элементы.Контрагент.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.Контрагенты");
...
Процедура ОбновитьГиперссылкуОтбора(КнопкаРасширенногоОтбора, Отбор)
	...
	ВидимыеОтборы.Добавить("ОрганизацияКонтрагент");
...
Процедура УстановитьТипРасчетовНаФорме()
	...
	ФлагиОтборов = ФлагиТиповРасчетов();
...
&НаСервере
Процедура УстановитьПереданныеПараметры(Настройки)
	
	Если ЗначениеЗаполнено(Параметры.НачалоПериода) Тогда
		Настройки["ПериодСверки"].Вариант = ВариантСтандартногоПериода.ПроизвольныйПериод;
		Настройки["ПериодСверки"].ДатаНачала = Параметры.НачалоПериода;
		НачалоПериода = Параметры.НачалоПериода;
	КонецЕсли;
	Если ЗначениеЗаполнено(Параметры.КонецПериода) Тогда
		Настройки["ПериодСверки"].Вариант = ВариантСтандартногоПериода.ПроизвольныйПериод;
		Настройки["ПериодСверки"].ДатаОкончания = Параметры.КонецПериода;
		КонецПериода = Параметры.КонецПериода;
	КонецЕсли;
	Если Параметры.ДоговорыБезОборотов Тогда
		ОтборДоговорыБезОборотов = ФинансоваяОтчетностьСервер.НайтиЭлементОтбора(КомпоновщикОтбор.Настройки.Отбор, "ДоговорыБезОборотов");
		ОтборДоговорыБезОборотов.Использование = Истина;
	КонецЕсли;
	Если ЗначениеЗаполнено(Параметры.Организация) И ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций")Тогда
		ЭлементОтбора = ФинансоваяОтчетностьСервер.НайтиЭлементОтбора(КомпоновщикОтбор.Настройки.Отбор, "Организация");
		ЭлементОтбора.Использование = Истина;
		ЭлементОтбора.ПравоеЗначение = Параметры.Организация;
	КонецЕсли;
	Если ЗначениеЗаполнено(Параметры.Контрагент) Тогда
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов") Тогда
			Документы.СверкаВзаиморасчетов2_5_11.ЗаполнитьОтборПартнераПоКонтрагенту(КомпоновщикОтбор.Настройки.Отбор, Параметры.Контрагент);
		Иначе
			ЭлементОтбора = ФинансоваяОтчетностьСервер.НайтиЭлементОтбора(КомпоновщикОтбор.Настройки.Отбор, "Контрагент");
			ЭлементОтбора.Использование = Истина;
			Если ТипЗнч(Параметры.Контрагент) = Тип("СписокЗначений") Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Если ЗначениеЗаполнено(Параметры.ОсновнойМенеджер) Тогда
		ЭлементОтбора = ФинансоваяОтчетностьСервер.НайтиЭлементОтбора(КомпоновщикОтбор.Настройки.Отбор, "ОсновнойМенеджер");
		Если ЭлементОтбора <> Неопределено Тогда
			ЭлементОтбора.Использование = Истина;
			ЭлементОтбора.ПравоеЗначение = Параметры.ОсновнойМенеджер;
		КонецЕсли;
	КонецЕсли;
	Если Параметры.ТипыРасчетов.Количество() > 0 Тогда
		УстановитьПереданныеТипыРасчетов(Настройки);
	КонецЕсли;
...
&НаСервере
Процедура УстановитьПереданныеТипыРасчетов(Настройки)
	
	ЭлементОтбора = ФинансоваяОтчетностьСервер.НайтиЭлементОтбора(КомпоновщикОтбор.Настройки.Отбор, "ТипРасчетов");
	ЭлементОтбора.Использование = Ложь;
	ЭлементОтбора.ПравоеЗначение = Параметры.ТипыРасчетов;
	Для Каждого ФлагОтбора Из ФлагиТиповРасчетов() Цикл
		ЕстьТипРасчетов = Параметры.ТипыРасчетов.НайтиПоЗначению(ФлагОтбора.Ключ) <> Неопределено;
		Настройки[ФлагОтбора.Значение] = ЕстьТипРасчетов;
		ЭлементОтбора.Использование = ЭлементОтбора.Использование ИЛИ ЕстьТипРасчетов;
	КонецЦикла;
	
КонецПроцедуры

&НаСервере
Функция ФлагиТиповРасчетов()
	
	ФлагиОтборов = Новый Соответствие;
	ФлагиОтборов.Вставить(Перечисления.ТипыРасчетовСПартнерами.РасчетыСКлиентом, "РасчетыСКлиентом");
	ФлагиОтборов.Вставить(Перечисления.ТипыРасчетовСПартнерами.РасчетыСПоставщиком, "РасчетыСПоставщиком");
...
Процедура УстановитьОтборНаФорме(ИмяОтбора)
	...
	Если ЭлементОтбора <> Неопределено Тогда
		...
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке И ЭлементОтбора.Родитель = Неопределено Тогда
			Если ЭлементОтбора.Использование Тогда
				Если ИмяОтбора = "Контрагент" И ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов")
					И ТипЗнч(ЭлементОтбора.ПравоеЗначение) = Тип("СписокЗначений")
					И ТипЗнч(ЭлементОтбора.ПравоеЗначение[0]) = Тип("СправочникСсылка.Партнеры") Тогда
					Для Каждого ЭлементСписка Из ЭлементОтбора.ПравоеЗначение Цикл
						Если ТипЗнч(ЭлементСписка.Значение) = Тип("СправочникСсылка.Партнеры") Тогда
							Контрагент = ПартнерыИКонтрагенты.ПолучитьКонтрагентаПартнераПоУмолчанию(ЭлементСписка.Значение);
							ЭлементСписка.Значение = Контрагент;
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
				Элемент = Элементы[ИмяОтбора]; // ПолеФормы - 
...
#Область СервисСверки

&НаКлиенте
Процедура ЗапуститьОбменСсервисом()
	
	Если ВыполняютсяСервисныеЗадания() Тогда
		// Обмен с сервисом уже запущен, просто получим расхождения если требуется
		Если ПолучитьРасхожденияПослеОбмена Тогда
			УстановитьСтатусРасхождений();
		КонецЕсли;
		Возврат;
	КонецЕсли;
	
	ОбменССервисом = ОбменятьсяДаннымиССервисом(УникальныйИдентификатор, НачалоПериода);
...
&НаСервереБезКонтекста
Функция ОбменятьсяДаннымиССервисом(УникальныйИдентификатор, НачалоПериода)
	
	НаименованиеЗадания = СервисСверкиРасчетов.ИмяСервиса() + ". " + НСтр("ru = 'Обмен с сервисом'");
...
&НаКлиенте
Процедура УстановитьСтатусРасхождений()
	
	ОчиститьСообщения();
	
	ЕстьРасхождения = ЕстьРасхожденияПоДаннымСервиса();
	Элементы.ГруппаОбнаруженыРасхождения.Видимость = ЕстьРасхождения;
	Элементы.НадписьНетРасхождений.Видимость = НЕ ЕстьРасхождения;
	
КонецПроцедуры

&НаСервере
Функция ЕстьРасхожденияПоДаннымСервиса() Экспорт
	
	//Получить необходимые отборы из компоновщика
	ОтборыСверки = ПолучитьОтборыСверки();
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	РасхожденияСверки.Организация КАК Организация,
	|	РасхожденияСверки.Контрагент КАК Контрагент
	|ИЗ
	|	РегистрСведений.СервисСверкиРасчетовОбнаруженныеРасхождения КАК РасхожденияСверки
	|ГДЕ
	|	РасхожденияСверки.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|	И РасхожденияСверки.Организация В(&Организация)
	|	И (&ПоВсемКонтрагентам ИЛИ РасхожденияСверки.Контрагент В(&Контрагент))";
	
	Запрос.УстановитьПараметр("НачалоПериода", ОтборыСверки.НачалоПериода);
	Запрос.УстановитьПараметр("КонецПериода", ОтборыСверки.КонецПериода);
	Запрос.УстановитьПараметр("Организация", ОтборыСверки.Организация);
	Запрос.УстановитьПараметр("Контрагент", ОтборыСверки.Контрагент);
	Запрос.УстановитьПараметр("ПоВсемКонтрагентам", НЕ ЗначениеЗаполнено(ОтборыСверки.Контрагент));
...
&НаСервере
Функция ПолучитьОтборыСверки()
	
	Результат = Новый Структура;
	Результат.Вставить("НачалоПериода", ПериодСверки.ДатаНачала);
	Результат.Вставить("КонецПериода", КонецДня(ПериодСверки.ДатаОкончания));
	Результат.Вставить("Организация", Организация);
	Если ЗначениеЗаполнено(Контрагент) Тогда
		ЗначениОтбораКонтрагент = Контрагент;
	Иначе
		ЭлементОтбора = ФинансоваяОтчетностьСервер.НайтиЭлементОтбора(КомпоновщикОтбор.Настройки.Отбор, "Контрагент");
		Если ЭлементОтбора.Использование Тогда
			ЗначениОтбораКонтрагент = ЭлементОтбора.ПравоеЗначение;
		Иначе
			ЗначениОтбораКонтрагент = Неопределено;
		КонецЕсли;
	КонецЕсли;
	Результат.Вставить("Контрагент", ЗначениОтбораКонтрагент);
