...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(НастройкиОтбора) Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов") Тогда
			Документы.СверкаВзаиморасчетов2_5_11.ЗаполнитьОтборПартнераПоКонтрагенту(КомпоновщикОтбор.Настройки.Отбор, Объект.Контрагент);
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Объект.ИтоговыеЗаписи.Количество() > 0 Тогда
...
Процедура ДетальныеЗаписиРасчетныйДокументПриИзменении(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(ТекущаяСтрока.РасчетныйДокумент) Тогда
		Возврат;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ТекущаяСтрока.ТипРасчетов) Тогда
		ЗаполнитьЗначенияСвойств(ТекущаяСтрока, Элементы.ДетальныеЗаписи.ОтборСтрок);
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	УстановитьОформлениеПоясняющегоПоля("ИтоговыеЗаписи", "Организация", НСтр("ru = 'Организация'"));
	УстановитьОформлениеПоясняющегоПоля("ИтоговыеЗаписи", "Контрагент", НСтр("ru = 'Контрагент'"));
	УстановитьОформлениеПоясняющегоПоля("ДетальныеЗаписи", "Организация", НСтр("ru = 'Организация'"));
	УстановитьОформлениеПоясняющегоПоля("ДетальныеЗаписи", "Контрагент", НСтр("ru = 'Контрагент'"));
...
&НаСервере
Процедура УстановитьОформлениеПоясняющегоПоля(ТипЗаписей, ИмяПоля, ТекстПодсказки)
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", ТекстПодсказки + ":");
...
Функция ПараметрыПриИзмененииКлючевыхРеквизитов(ИмяРеквизита)
	...
	НужноРаспровести = НужноПерезаполнитьТЧ И Объект.Проведен И ЗначениеЗаполнено(Объект.Ссылка) И ЗначениеОбязательно;
	ТекстВопроса = "";
...
Процедура ОтобразитьНаименованиеИсходящихДокументов()
	...
	Элементы.ДекорацияВалюта.Видимость = Истина;
	Элементы.СверкаВВалюте.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
