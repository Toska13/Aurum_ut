...
Процедура ДобавитьКомандыОтчетов(КомандыОтчетов, Параметры) Экспорт
	...
	//++ Локализация
	СервисСверкиРасчетовУТ.СверкаРасчетовСКонтрагентами_ДобавитьКомандуОтчета(КомандыОтчетов);
	//-- Локализация
...
Функция ВывестиДетальныеЗаписи(ДанныеПоОбъектуГруппировки, ДанныеВывода)
	...
	Если НастройкиПечати.ДетализацияВзаиморасчетов = "ОбъектРасчетов"
		...
		ФрагментыЗаголовка.Добавить(НСтр("ru = 'по объекту расчетов'", КодОсновногоЯзыка));
	ИначеЕсли НастройкиПечати.ДетализацияВзаиморасчетов = "Договор" Тогда
		ФрагментыЗаголовка.Добавить(НСтр("ru = 'по договору'", КодОсновногоЯзыка));
	КонецЕсли;
	Если НастройкиПечати.ДетализацияВзаиморасчетов <> "Валюта" Тогда
		...
		Иначе
			ФрагментыЗаголовка.Добавить("<" + НСтр("ru = 'не указан'", КодОсновногоЯзыка) + ">");
		КонецЕсли;
	КонецЕсли;
	Валюта = НСтр("ru='в валюте'", КодОсновногоЯзыка) + " " + ДанныеПоОбъектуГруппировки.ПредставлениеВалюта;
	...
	ОбластьМакетаШапка.Параметры.Дебет = НСтр("ru = 'Дебет'", КодОсновногоЯзыка);
	ОбластьМакетаШапка.Параметры.Кредит = НСтр("ru = 'Кредит'", КодОсновногоЯзыка);
	ОбластьМакетаНачОстаток.Параметры.ИмяОстатка = НСтр("ru = 'Сальдо начальное'", КодОсновногоЯзыка);
	ОбластьМакетаКонОстаток.Параметры.ИмяОстатка = НСтр("ru = 'Сальдо конечное'", КодОсновногоЯзыка);
	Если НастройкиПечати.ДебетКакДолг Тогда
		ОбластьМакетаШапка.Параметры.Дебет = НСтр("ru = 'Долг'", КодОсновногоЯзыка);
		ОбластьМакетаШапка.Параметры.Кредит = НСтр("ru = 'Аванс'", КодОсновногоЯзыка);
		ОбластьМакетаНачОстаток.Параметры.ИмяОстатка = НСтр("ru = 'Начальный остаток'", КодОсновногоЯзыка);
		ОбластьМакетаКонОстаток.Параметры.ИмяОстатка = НСтр("ru = 'Конечный остаток'", КодОсновногоЯзыка);
	КонецЕсли;
	...
	Если ДанныеПоОбъектуГруппировки.ЕстьОбороты Тогда
		Для Каждого ДетальныеЗаписи Из ДанныеПоОбъектуГруппировки.Строки Цикл
			...
			Если (ДетальныеЗаписи.НаименованиеДокумента = ИмяДокументаСторно()
					ИЛИ ДетальныеЗаписи.НаименованиеДокумента = ИмяДокументаИсправления())
				И ЗначениеЗаполнено(ДетальныеЗаписи.СвязанныеДокументы) Тогда
				ШаблонДокумента = "%ИмяДокумента%";
			Иначе
			...
			КонецЕсли;
			Если ЗначениеЗаполнено(ДетальныеЗаписи.СвязанныеДокументы)
				И СтрНайти(ДетальныеЗаписи.СвязанныеДокументы, ДетальныеЗаписи.НаименованиеДокумента) = 0 Тогда
...
Функция ЗапросДанныеНаПечать(МассивОбъектов, НастройкиПечати) Экспорт
	...
	ЗапросДанныеДокумента.УстановитьПараметр("ТипРасчетов", НСтр("ru = 'Расчеты'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ТекстЗапросаДанныеДокумента(НастройкиПечати)
	...
	|	ЕСТЬNULL(Реализация.ДатаПереходаПраваСобственности, ДАТАВРЕМЯ(1,1,1)) КАК ДатаПереходаПраваСобственности,
	|	ЕСТЬNULL(Реализация.ДатаПереходаПраваСобственности, ДАТАВРЕМЯ(1,1,1)) <> ДАТАВРЕМЯ(1,1,1) КАК ДатаПереходаПраваЗаполнена,
	|	ЕСТЬNULL(Реализация.Статус, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.ВПути) КАК ТоварыВПути,
	|
	...
	|
	|ПОМЕСТИТЬ втДетальныеЗаписиПредварительная
	|ИЗ
	...
	|			ПО ДетальныеЗаписи.РасчетныйДокумент = Реализация.Ссылка
	|ГДЕ
	...
	|	ЕСТЬNULL(Реализация.ДатаПереходаПраваСобственности, ДАТАВРЕМЯ(1,1,1)),
	|	ЕСТЬNULL(Реализация.ДатаПереходаПраваСобственности, ДАТАВРЕМЯ(1,1,1)) <> ДАТАВРЕМЯ(1,1,1),
	|	ЕСТЬNULL(Реализация.Статус, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.ВПути)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ДетальныеЗаписи.СвязанныеДокументы КАК СвязанныеДокументы,
	|	ДетальныеЗаписи.ДатаПереходаПраваСобственности КАК ДатаПереходаПраваСобственности,
	|	ДетальныеЗаписи.ДатаПереходаПраваЗаполнена КАК ДатаПереходаПраваЗаполнена,
	|	ДетальныеЗаписи.ТоварыВПути КАК ТоварыВПути,
	|
	|	ДетальныеЗаписи.ВалютаДокумента КАК ВалютаДокумента,
	|	ДетальныеЗаписи.СуммаДокумента КАК СуммаДокумента,
	|
	|	ДетальныеЗаписи.СуммаДолг КАК СуммаДолг,
	|	ДетальныеЗаписи.СуммаАванс КАК СуммаАванс,
	|	ВЫБОР
	|		КОГДА ДетальныеЗаписи.ДатаПереходаПраваЗаполнена ИЛИ ДетальныеЗаписи.ТоварыВПути
	|			ТОГДА ДетальныеЗаписи.СуммаДебет - ДетальныеЗаписи.СуммаКредит
	|		ИНАЧЕ
	|			ДетальныеЗаписи.СуммаДебет
	|	КОНЕЦ КАК СуммаДебет,
	|	ВЫБОР
	|		КОГДА ДетальныеЗаписи.ДатаПереходаПраваЗаполнена ИЛИ ДетальныеЗаписи.ТоварыВПути
	|			ТОГДА ДетальныеЗаписи.СуммаКредит - ДетальныеЗаписи.СуммаКредит
	|		ИНАЧЕ
	|			ДетальныеЗаписи.СуммаКредит
	|	КОНЕЦ КАК СуммаКредит,
	|
	|	ВЫБОР
	|		КОГДА ДетальныеЗаписи.ДатаПереходаПраваЗаполнена ИЛИ ДетальныеЗаписи.ТоварыВПути
	|			ТОГДА ДетальныеЗаписи.СуммаДебетКонтрагент - ДетальныеЗаписи.СуммаКредитКонтрагент
	|		ИНАЧЕ
	|			ДетальныеЗаписи.СуммаДебетКонтрагент
	|	КОНЕЦ КАК СуммаДебетКонтрагент,
	|	ВЫБОР
	|		КОГДА ДетальныеЗаписи.ДатаПереходаПраваЗаполнена ИЛИ ДетальныеЗаписи.ТоварыВПути
	|			ТОГДА ДетальныеЗаписи.СуммаКредитКонтрагент - ДетальныеЗаписи.СуммаКредитКонтрагент
	|		ИНАЧЕ
	|			ДетальныеЗаписи.СуммаКредитКонтрагент
	...
	|ИЗ
	|	втДетальныеЗаписиПредварительная КАК ДетальныеЗаписи
	|ГДЕ
	|	НЕ ((ТИПЗНАЧЕНИЯ(ДетальныеЗаписи.РасчетныйДокумент) = ТИП(Документ.ВзаимозачетЗадолженности)
	|			ИЛИ ДетальныеЗаписи.ДатаПереходаПраваСобственности <> ДАТАВРЕМЯ(1,1,1)
	|			ИЛИ ДетальныеЗаписи.ТоварыВПути)
	|		И ДетальныеЗаписи.СуммаДебет - ДетальныеЗаписи.СуммаКредит = 0)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВЫБОР КОГДА ДетальныеЗаписи.ТипРасчетов В (
	|			ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСКлиентом),
	|			ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСПоставщиком))
	|		ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	...
	|	ЕСТЬNULL(ДетальныеЗаписи.ДатаПереходаПраваСобственности, ДАТАВРЕМЯ(1,1,1)) КАК ДатаПереходаПраваСобственности,
	|	ЕСТЬNULL(ДетальныеЗаписи.ДатаПереходаПраваЗаполнена, ЛОЖЬ) КАК ДатаПереходаПраваЗаполнена,
	|	ЕСТЬNULL(ДетальныеЗаписи.ТоварыВПути, ЛОЖЬ) КАК ТоварыВПути,
	|
	...
	|	
	|	ВЫБОР 
	|		КОГДА ДетальныеЗаписи.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСКлиентом)
	|			ТОГДА 10
	|		КОГДА ДетальныеЗаписи.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСПоставщиком)
	|			ТОГДА 20
	|		ИНАЧЕ
	|			0
	...
	|	ЕСТЬNULL(ДеталиДокумента.ДатаПереходаПраваСобственности, ДАТАВРЕМЯ(1,1,1)) КАК ДатаПереходаПраваСобственности,
	|	ЕСТЬNULL(ДеталиДокумента.ТоварыВПути, ЛОЖЬ) КАК ТоварыВПути,
	|	
	...
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ втДетальныеЗаписиРасчетов КАК ДеталиДокумента
	...
	|	НомерДокумента,
	|	ПорядокТипаСуммы,
	|	ЕСТЬNULL(ДеталиДокумента.СуммаДокумента, 0) УБЫВ
	|
	|ИТОГИ
	...
	|	МАКСИМУМ(ДатаПереходаПраваСобственности),
	|	МАКСИМУМ(ТоварыВПути),
	|
	|	СУММА(СуммаДебет),
	|	СУММА(СуммаКредит),
	|	
...
// 
Функция ДанныеДляПечати(Данные, НаименованияДляПечати, НаименованиеИсходящихДокументов)
	...
	ИначеЕсли Данные.РасчетныйДокумент <> Неопределено И НЕ Данные.РасчетныйДокумент.Пустая() Тогда
		...
		НаименованиеПечатнойФормы = НаименованияДляПечати[Данные.РасчетныйДокумент];
		...
		ИначеЕсли НаименованиеПечатнойФормы <> Неопределено Тогда
			Результат.НаименованиеДокумента = СокращенияПечатныхФорм(НаименованиеПечатнойФормы);
		//++ Локализация
		ИначеЕсли ИсходящиеНакладные.Свойство(ИмяДокумента) Тогда
			...
			Иначе
				Результат.НаименованиеДокумента = ?(Данные.ЭтоУКД, НСтр("ru = 'УКД'"), НСтр("ru = 'УПД'"));
			КонецЕсли;
		//-- Локализация
		Иначе
...
Функция ПредставленияДокументов() Экспорт
	...
	ВидыДокументовКонтрагента.Вставить("АвансовыйОтчет",                        НСтр("ru='Расходный кассовый ордер'"));
	ВидыДокументовКонтрагента.Вставить("АктВыполненныхРабот",                   НСтр("ru='Акт выполненных работ'"));
	ВидыДокументовКонтрагента.Вставить("ОперацияСЭлектроннымБилетом",           НСтр("ru='Электронный билет'"));
...
Функция СокращенияПечатныхФорм(ИмяПечатнойФормы)
	
	СокращенияПечатныхФорм = Новый Соответствие;
	
	СокращенияПечатныхФорм.Вставить(
	...
	Сокращение = СокращенияПечатныхФорм[ИмяПечатнойФормы];
	Если Сокращение = Неопределено Тогда
		Сокращение = ИмяПечатнойФормы;
	КонецЕсли;
...
Функция ВходящиеНакладные() Экспорт
	...
	ВидыДокументовКонтрагента.Вставить("ОперацияСЭлектроннымБилетом");
...
Процедура СвернутьРасчетыПоГруппировкам(ДанныеДокумента, ТаблицаВзаиморасчетов)
	...
	//-- Локализация
	ДанныеИсправляемыхДокументов = ДанныеИсправляемыхДокументов(ТаблицаВзаиморасчетов);
	...
	РасчетныеДокументы = ТаблицаВзаиморасчетов.ВыгрузитьКолонку("РасчетныйДокумент");
	Если ДанныеИсправляемыхДокументов.Количество() > 0 Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(РасчетныеДокументы,
			ДанныеИсправляемыхДокументов.ВыгрузитьКолонку("РасчетныйДокумент"));
	КонецЕсли;
	НаименованияДокументовПечати = ОбщегоНазначенияУТ.НаименованияЭлектронныхДокументовИлиОсновныхПечатныхФорм(
		РасчетныеДокументы);
	...
	Для Каждого СтрокаРасчетов Из ТаблицаВзаиморасчетов Цикл
		...
		Если СтрокаРасчетов.НачальноеСальдо = 0 И ДанныеСумм.ЕстьЗаполненные Тогда
			...
			КонецЕсли;
			Если СтрокаРасчетов.СторноИсправление Тогда
				ИсправляемыйДокумент = ДанныеИсправляемыхДокументов.Найти(СтрокаРасчетов.РасчетныйДокумент, "ДокументСторноИсправления");
				ДанныеДляПечати = ДанныеДляПечати(ИсправляемыйДокумент, НаименованияДокументовПечати, ДанныеДокумента.НаименованиеИсходящихДокументов);
				Если ТипЗнч(СтрокаРасчетов.РасчетныйДокумент) = Тип("ДокументСсылка.Сторно") Тогда
					НоваяСтрока.НаименованиеДокумента = ИмяДокументаСторно();
				Иначе
					НоваяСтрока.НаименованиеДокумента = ИмяДокументаИсправления();
					НоваяСтрока.СуммаДокумента = ИсправляемыйДокумент.СуммаДокумента;
				КонецЕсли;
				НоваяСтрока.СвязанныеДокументы = СтрШаблон(НСтр("ru = '%1 №%2 от %3'"), 
					ДанныеДляПечати.НаименованиеДокумента,
					ДанныеДляПечати.НомерДокумента,
					Формат(ДанныеДляПечати.ДатаДокумента, "ДЛФ=D"));
			Иначе
				ДанныеДляПечати = ДанныеДляПечати(СтрокаРасчетов, НаименованияДокументовПечати, ДанныеДокумента.НаименованиеИсходящихДокументов);
				...
				Для Каждого СФ Из СФДокумента Цикл
					...
					Иначе
						...
						СвязанныйДокумент = СтрШаблон(НСтр("ru = 'Счет-фактура №%1 от %2'"),
								СокрЛП(НомерСФНаПечать),
								Формат(СФ.Дата, "ДЛФ=D"));
						СвязанныеДокументы.Добавить(СвязанныйДокумент)
					КонецЕсли;
				КонецЦикла;
				НоваяСтрока.СвязанныеДокументы = СтрСоединить(СвязанныеДокументы, ", ");
				//-- Локализация
			КонецЕсли;
		КонецЕсли;
...
Функция ДанныеИсправляемыхДокументов(ТаблицаВзаиморасчетов)
	
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьИсправлениеДокументов") Тогда
		ТаблицаВзаиморасчетов.Колонки.Добавить("СторноИсправление", Новый ОписаниеТипов("Булево"));
		Возврат ТаблицаВзаиморасчетов.СкопироватьКолонки();
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ДанныеСверки.РасчетныйДокумент КАК ДокументИсправления,
	|	ДанныеСверки.СуммаДокумента КАК СуммаДокумента,
	|	ДанныеСверки.ИсправляемыйДокумент КАК Ссылка,
	|	ДанныеСверки.СторнируемыйДокумент КАК СторнируемыйДокумент,
	|	ДанныеСверки.СторноИсправление КАК СторноИсправление
	|ПОМЕСТИТЬ втИсправляемыеДокументы
	|ИЗ
	|	&ДанныеСверки КАК ДанныеСверки
	|ГДЕ
	|	ДанныеСверки.СторноИсправление
	|	И ДанныеСверки.РасчетныйДокумент <> НЕОПРЕДЕЛЕНО
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ РАЗРЕШЕННЫЕ
	|	ИсправляемыеДокументы.ДокументИсправления КАК ДокументСторноИсправления,
	|	РеестрДокументов.Ссылка КАК РасчетныйДокумент,
	//логика выбора ДатаДокумента и НомерДокумента должна быть как в СверкаВзаиморасчетов2_5_11.Макеты.ОтборРсчетов
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ИсправляемыеДокументы.Ссылка) В 
	|					(ТИП(Документ.ПоступлениеБезналичныхДенежныхСредств),
	|					ТИП(Документ.СписаниеБезналичныхДенежныхСредств))
	|			ТОГДА ВЫБОР КОГДА РеестрДокументов.ДатаПервичногоДокумента = ДАТАВРЕМЯ(1,1,1)
	|							ТОГДА РеестрДокументов.ДатаОтраженияВУчете
	|							ИНАЧЕ РеестрДокументов.ДатаПервичногоДокумента
	|					КОНЕЦ
	|		ИНАЧЕ
	|			ВЫБОР КОГДА РеестрДокументов.ДатаПервичногоДокумента = ДАТАВРЕМЯ(1,1,1)
	|					ТОГДА РеестрДокументов.ДатаДокументаИБ
	|					ИНАЧЕ РеестрДокументов.ДатаПервичногоДокумента
	|			КОНЕЦ
	|	КОНЕЦ КАК ДатаДокумента,
	|	ВЫБОР КОГДА РеестрДокументов.НомерПервичногоДокумента = """"
	...
	|	ИсправляемыеДокументы.СуммаДокумента КАК СуммаДокумента,
	|	РеестрДокументов.Сумма КАК СтараяСуммаДокумента,
	|	ЕСТЬNULL(КорректировкаРеализации.ВидКорректировки, НЕОПРЕДЕЛЕНО) 
	|		= ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон) КАК ЭтоУКД
	|ИЗ
	...
	|		ПО ИсправляемыеДокументы.Ссылка = РеестрДокументов.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.КорректировкаРеализации КАК КорректировкаРеализации
	|		ПО ИсправляемыеДокументы.Ссылка = КорректировкаРеализации.Ссылка";
	
	Запрос.УстановитьПараметр("ДанныеСверки", ТаблицаВзаиморасчетов);
	
	Результат = Запрос.Выполнить().Выгрузить();
	Результат.Индексы.Добавить("ДокументСторноИсправления");
	
	Возврат Результат;
	
КонецФункции

Функция ИмяДокументаСторно()
	
	Возврат НСтр("ru = 'Сторно'");
	
КонецФункции

Функция ИмяДокументаИсправления()
	
	Возврат НСтр("ru = 'Исправление'");
...
//  ФрагментыЗапросаСервиса - Структура из КлючИЗначение
// 
// Возвращаемое значение:
//  ТаблицаЗначений, ДеревоЗначений - Получить взаиморасчеты
Функция ПолучитьВзаиморасчеты(КомпоновщикОтбор, ФрагментыЗапросаСервиса = Неопределено) Экспорт
	...
	//++ Локализация
	Если ФрагментыЗапросаСервиса <> Неопределено Тогда
		СхемаКомпоновкиДанных.НаборыДанных.ОстаткиИОбороты.Запрос = 
			СтрЗаменить(СхемаКомпоновкиДанных.НаборыДанных.ОстаткиИОбороты.Запрос, "//#ОтборСервисаСверки", ФрагментыЗапросаСервиса.Отбор);
		СхемаКомпоновкиДанных.НаборыДанных.ОстаткиИОбороты.Запрос = 
			СтрЗаменить(СхемаКомпоновкиДанных.НаборыДанных.ОстаткиИОбороты.Запрос, "//#СоединениеСервисаСверки", ФрагментыЗапросаСервиса.Соединение);
	КонецЕсли;
	//-- Локализация
	...
	ТипыСуммАренды = ТипыСуммАрендыДляСверки();
...
Процедура ИнициализироватьКомпоновщикНастроек(КомпоновщикОтбор, УникальныйИдентификаторФормы = Неопределено, НастройкиОтбора = Неопределено) Экспорт
	...
	Если ПравоДоступа("Просмотр", Метаданные.Справочники.ДоговорыКредитовИДепозитов) ИЛИ ПривилегированныйРежим Тогда
...
Процедура ЗаполнитьОтборПартнераПоКонтрагенту(Отбор, Контрагент) Экспорт
	...
	Если ТипЗнч(Контрагент) = Тип("СправочникСсылка.Организации") Тогда
		ЭлементОтбора = ФинансоваяОтчетностьСервер.НайтиЭлементОтбора(Отбор,"ОрганизацияКонтрагент");
		Если ЭлементОтбора = Неопределено Тогда
			ЭлементОтбора = КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(Отбор, "ОрганизацияКонтрагент");
		КонецЕсли;
		ЭлементОтбора.Использование = Истина;
		ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ЭлементОтбора.ПравоеЗначение = Контрагент;
	Иначе
		ЭлементОтбора = ФинансоваяОтчетностьСервер.НайтиЭлементОтбора(Отбор,"Партнер");
		Если ЭлементОтбора <> Неопределено Тогда
			ЭлементОтбора.Использование = Истина;
			Если ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
				ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
				ЭлементОтбора.ПравоеЗначение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "Партнер");
			ИначеЕсли ТипЗнч(Контрагент) = Тип("Массив") ИЛИ ТипЗнч(Контрагент) = Тип("СписокЗначений") Тогда
				ЭлементОтбора.Использование = Истина;
				ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
				Если ТипЗнч(Контрагент) = Тип("СписокЗначений") Тогда
					Если Контрагент.Количество() > 0 И ТипЗнч(Контрагент[0].Значение) = Тип("СправочникСсылка.Контрагенты") Тогда
						Партнеры = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(Контрагент.ВыгрузитьЗначения(), "Партнер");
					ИначеЕсли Контрагент.Количество() > 0 И ТипЗнч(Контрагент[0].Значение) = Тип("СправочникСсылка.Организации") Тогда
					...
					КонецЕсли;
				Иначе
					Партнеры = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(Контрагент, "Партнер");
				КонецЕсли;
				Если Партнеры <> Неопределено Тогда
					ЭлементОтбора.ПравоеЗначение = Новый СписокЗначений;
					Для Каждого Партнер Из Партнеры Цикл
						ЭлементОтбора.ПравоеЗначение.Добавить(Партнер.Значение);
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция ТекстыЗапросовДоступныхТаблиц()
	...
	Если ПравоДоступа("Просмотр", Метаданные.РегистрыНакопления.РасчетыПоФинансовымИнструментам)
	...
	КонецЕсли;
	Если ПравоДоступа("Просмотр", Метаданные.Справочники.ДоговорыКредитовИДепозитов)
		ИЛИ ПривилегированныйРежим Тогда
		Результат.Вставить("#2", ТекстЗапросаДоговоровКредитовДепозитовЗаймов()); //СправочникДоговорыКредитовИДепозитов
	КонецЕсли;
	Если ПравоДоступа("Просмотр", Метаданные.Документы.ПервичныйДокумент)
		ИЛИ ПривилегированныйРежим Тогда
		Результат.Вставить("#4", ТекстЗапросаПервичныйДокумент()); //ДокументПервичныйДокумент
	КонецЕсли;
	Если ПравоДоступа("Просмотр", Метаданные.Документы.КорректировкаЗадолженности)
		ИЛИ ПривилегированныйРежим Тогда
...
Функция ТекстЗапросаРасчетыПоФинансовымИнструментамОстаткиНаКонецПериода()
	...
	|		АналитикаУчетаПоПартнерам.Контрагент В (&Контрагенты)}
	|	) КАК РасчетыОстатки
...
Функция ТекстЗапросаРасчетыПоФинансовымИнструментам()
	...
	|			АналитикаУчетаПоПартнерам.Организация В (&Организации),
	|			АналитикаУчетаПоПартнерам.Контрагент В (&Контрагенты),
	|			АналитикаУчетаПоПартнерам.Партнер В (&Партнеры)}
	|		) КАК РасчетыОстатки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ АналитикаПоПартнерам КАК Аналитика
	|		ПО РасчетыОстатки.АналитикаУчетаПоПартнерам = Аналитика.КлючАналитики
	...
	|	ГДЕ
	|		РасчетыОбороты.СуммаРеглОборот <> 0) КАК ВложенныйЗапрос
	|	
	|СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаДоговоровКредитовДепозитовЗаймов()
	...
	|	Договоры.Организация.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
...
// Типы сумм аренды для сверки. В УТ пустой массив.
// 
// Возвращаемое значение:
//  Массив Из ПеречислениеСсылка.ТипыПлатежейПоАренде 
//
Функция ТипыСуммАрендыДляСверки() Экспорт
	
	ТипыСуммАренды = Новый Массив;
	
	Возврат ТипыСуммАренды;
