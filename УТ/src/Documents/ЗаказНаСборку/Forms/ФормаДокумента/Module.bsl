...
Процедура ДлительностьСборкиРазборкиПриИзменении(Элемент)
	...
	ОбновитьРеквизитыКонтроляКачества();
...
Процедура НачалоСборкиРазборкиПриИзменении(Элемент)
	...
	НачалоСборкиРазборкиПриИзмененииСервер();
...
Процедура ОкончаниеСборкиРазборкиПриИзменении(Элемент)
	...
	ОкончаниеСборкиРазборкиПриИзмененииСервер();
...
&НаКлиенте
Процедура ПроконтролироватьКачествоПриИзменении(Элемент)
	
	Если Объект.ПроконтролироватьКачество Тогда
		Объект.ДатаПодтвержденияКачества = '00010101';
	КонецЕсли;
	ПроконтролироватьКачествоПриИзмененииСервер();
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ДобавитьДействияОбеспеченияНаКлиенте(СтруктураДействий, "ВариантОбеспечения,ДоступноВДругихСтроках");
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Копирование Тогда
		СтруктураДействий = Новый Структура;
		ДобавитьДействияОбеспеченияНаКлиенте(СтруктураДействий, "ВариантОбеспечения,Доступно");
		СтруктураДействий.Вставить("ПроверитьСериюРассчитатьСтатус", Новый Структура("Склад, ПараметрыУказанияСерий", Объект.Склад, ПараметрыУказанияСерий.ТЧ));
...
&НаСервере
Процедура ОбновитьРеквизитыКонтроляКачества()

	Если КонтрольКачества.ОбновитьПараметрыДействий(
			Документы.ЗаказНаСборку,
			Объект,
			КонтрольКачестваКлиентСервер.КэшПоКлючу(КонтрольКачестваКэш, "Товары")) Тогда
	...
	Если КонтрольКачества.ОбновитьПараметрыДействий(
			Документы.ЗаказНаСборку,
			Объект,
			КонтрольКачестваКлиентСервер.КэшПоКлючу(КонтрольКачестваКэш, "Шапка")) Тогда
			
		Действия = Новый Структура();
		Действия.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества",
		...
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(Шапка, Действия, Неопределено);
		ЗаполнитьЗначенияСвойств(Объект, Шапка);
		
	КонецЕсли;
	УстановитьОформлениеПолейКонтроляКачества();
...
Процедура ДобавитьДействияОбеспеченияКомплектаНаСервере(СтруктураДействий, Знач ПоляСтрокой, ДатаОтгрузки = Неопределено)
	...
	Если СтруктураДействий.Свойство("ЗаполнитьКолонкуДоступно") Тогда
		СтруктураДействий.ЗаполнитьКолонкуДоступно.Вставить("ИдентификаторСтрокиПолученный", Неопределено);
	КонецЕсли;
...
Процедура ВыполнитьДействияОбеспеченияКомплектаНаСервере(Поля)
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РазборкаТоваров Тогда
		...
		ПоляОбъекта = "Количество,Номенклатура,Характеристика,ВариантОбеспечения,Обособленно,Подразделение,
			|Склад,Назначение,Статус,НачалоСборкиРазборки,ХозяйственнаяОперация,КоличествоУпаковок,Упаковка,Серия,Дата,Приоритет";
		ПоляОбъектаИСсылка = ПоляОбъекта + ",Ссылка";
		ПоляФормы   = "Доступно,ПерераспределятьЗапасы,ОтгружатьЕслиДоступно,ПерераспределятьЗапасы";
		ПоляФикции  = "НомерСтроки";
		Шапка = Новый Структура(ПоляОбъектаИСсылка + "," + ПоляФормы + "," + ПоляФикции);
		Шапка.Вставить("ТипНоменклатуры", ТипНоменклатуры);
		ЗаполнитьЗначенияСвойств(Шапка, Объект, ПоляОбъектаИСсылка);
...
Процедура ВыполнитьДействияОбеспеченияКомплектаНаКлиенте(Поля)
	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.РазборкаТоваров") Тогда
		...
		ЗаполнитьЗначенияСвойств(Шапка, Объект, ПоляОбъектаИСсылка);
		ЗаполнитьЗначенияСвойств(Шапка, ЭтотОбъект, ПоляФормы);
		ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(Шапка, Действия, Неопределено);
		ЗаполнитьЗначенияСвойств(Объект, Шапка, ПоляОбъекта);
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Шапка, ПоляФормы);
...
&НаСервере
Процедура ПроконтролироватьКачествоПриИзмененииСервер()
...
&НаСервере
Процедура УстановитьОформлениеПолейКонтроляКачества()
	
	ЭтоСборка = (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СборкаТоваров);
	Элементы.ТоварыПроконтролироватьКачество.Видимость = Не ЭтоСборка;
	Элементы.ТоварыДатаПодтвержденияКачества.Видимость = Не ЭтоСборка;
	Элементы.ТоварыГруппаПроверитьКачествоДо.Видимость = Не ЭтоСборка;
	Элементы.ПроконтролироватьКачество.Видимость = ЭтоСборка;
	Элементы.ДатаПодтвержденияКачества.Видимость = ЭтоСборка;
	Элементы.ГруппаПроверитьКачествоДо.Видимость = ЭтоСборка;
	Элементы.ДатаПодтвержденияКачества.Доступность = Объект.ПроконтролироватьКачество;
	Элементы.ДатаПодтвержденияКачества.АвтоОтметкаНезаполненного = Объект.ПроконтролироватьКачество;
...
Процедура НоменклатураПриИзмененииСервер(КэшЗначений)
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(Шапка, Действия, КэшЗначений);
	
	ЗаполнитьЗначенияСвойств(Объект, Шапка);
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РазборкаТоваров Тогда
		ВыполнитьДействияОбеспеченияКомплектаНаСервере("ВариантОбеспечения,Доступно");
	КонецЕсли;
...
&НаСервере
Процедура НачалоСборкиРазборкиПриИзмененииСервер()
	...
	Иначе
		...
		Объект.ОкончаниеСборкиРазборки = '00010101';
...
&НаСервере
Процедура ОкончаниеСборкиРазборкиПриИзмененииСервер()
	...
	Иначе
		...
		Объект.НачалоСборкиРазборки = '00010101';
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	СтруктураДействий.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества",
		КонтрольКачестваКлиентСервер.КэшПоКлючу(КонтрольКачестваКэш, "Товары"));
	НазначенияКлиентСервер.СтруктураДействийВставитьПриДобавленииСтроки(ЭтотОбъект, СтруктураДействий, Истина);
	ДобавитьДействияОбеспеченияНаСервере(СтруктураДействий, "ВариантОбеспечения");
...
Процедура ОбработатьУказаниеСерийСервер(ПараметрыФормыУказанияСерий, ТоварВШапке)
	...
	Иначе
		...
		Действия.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества",
			КонтрольКачестваКлиентСервер.КэшПоКлючу(КонтрольКачестваКэш, "Товары"));
		НоменклатураСервер.ОбработатьУказаниеСерий(Объект,ПараметрыУказанияСерий.ТЧ,ПараметрыФормыУказанияСерий, Действия);
...
Процедура УстановитьСвойстваЭлементовФормыХозяйственнаяОперация()
	...
	Если ЭтоСборка Тогда
		...
		Команды.ОтменитьНеотработанные.Подсказка = НСтр("ru = 'Отмене подлежит несобранное количество комплектов и неиспользованные комплектующие'");
	...
	Иначе
		...
		Команды.ОтменитьНеотработанные.Подсказка = НСтр("ru = 'Отмене подлежит неиспользованное количество комплектов и неполученные комплектующие'");
