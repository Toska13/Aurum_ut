...
// 
// Параметры:
//  ОбъектИлиСсылка - ДокументОбъект.ЗаказНаСборку, ДокументСсылка.ЗаказНаСборку - документ.
...
// 
// Параметры:
//  Объект - ДокументОбъект.ЗаказНаСборку - объект.
// 
// Возвращаемое значение:
...
Функция КонтрольКачестваПараметрыДействий(Объект) Экспорт
	...
	ПараметрыДействия.ЧитаемыеПоля = "Номенклатура,Отменено";
	...
	ПараметрыДействия.ПараметрыЗапроса.КонтрольКачестваДатаПоступления = Объект.ОкончаниеСборкиРазборки;
	...
	ПараметрыДействия.ТекстЗапросаДанныеДокумента =
		...
		|	ЛОЖЬ КАК Установить,
		|	&КонтрольКачестваХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СборкаТоваров)
		|		ИЛИ Таблица.Отменено КАК Сбросить
...
Функция ПараметрыПроверкиКорректностиКомплекта(Объект) Экспорт
	...
	ПараметрыПроверки.ЭтоЗаказКомплектации = Истина;
...
// Функция встраивания в механизм обеспечения. Необходимо добавить эту функцию в модуль менеджера
// для встраивания команд установки действий заказе вида "Резервировать на складе", "К обеспечению"
// и т.д. в список документов. Если списке документов (заказов) одного типа необходимо использовать разные параметры
// встраивания, в зависимости от значений реквизитов конкретного документа.
// В этом случае в обработке команд используется
// метод ОбеспечениеВДокументахКлиент.ВыполнитьДействиеВСпискеЗаказовРазныхТипов.
// 
// Параметры:
//  ВсеЗаказы - Массив Из ДокументСсылка.ЗаказНаСборку
//  КодДействия - Строка - условное обозначение выполняемой команды: "ДЕЙСТВИЕ_ОТГРУЗИТЬ", "ДЕЙСТВИЕ_КОБЕСПЕЧЕНИЮ" и т.д.
// Возвращаемое значение:
//  см. ОбеспечениеВДокументахСервер.НовыеПараметрыВыполненияКомандУстановкиДействий
Функция ПараметрыВыполненияКомандУстановкиДействий(ВсеЗаказы, КодДействия) Экспорт
	...
	Результат = ОбеспечениеВДокументахСервер.СтруктураЗаказыИПараметрыВстраивания();
	Результат.Заказы = РезультатЗапроса[0].Выгрузить().ВыгрузитьКолонку("Ссылка");
	Результат.ПараметрыВстраивания = ДоступныеОстаткиПараметрыВстраиванияСборка();
	Результаты.Добавить(Результат);
	
	Результат = ОбеспечениеВДокументахСервер.СтруктураЗаказыИПараметрыВстраивания();
	Результат.Заказы = РезультатЗапроса[1].Выгрузить().ВыгрузитьКолонку("Ссылка");
	Результат.ПараметрыВстраивания = ДоступныеОстаткиПараметрыВстраиванияРазборка();
	Результаты.Добавить(Результат);
	
	Результат = ОбеспечениеВДокументахСервер.НовыеПараметрыВыполненияКомандУстановкиДействий();
	Результат.КодДействия = КодДействия;
	Результат.ПараметрыВыполнения = Результаты;
...
Процедура ЗапланироватьПоступлениеТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаТовары.ОкончаниеСборкиРазборки               КАК Дата,
	...
	|	НЕОПРЕДЕЛЕНО                                        КАК ИсправляемыйДокумент,
	|	ТаблицаТовары.Ссылка                                КАК Заказ,
	|	ТаблицаТовары.Ссылка.ОкончаниеСборкиРазборки        КАК Дата,
	|	ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка) КАК Договор,
	|	НЕОПРЕДЕЛЕНО                                        КАК Соглашение,
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|			ТОГДА ВЫБОР
		|				КОГДА ТабЧасть.Ссылка.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|				ИЛИ ТабЧасть.Ссылка.ОбосабливатьПоНазначениюЗаказа
		|					ТОГДА ТабЧасть.Ссылка.НазначениеТовары
		|				ИНАЧЕ ТабЧасть.Ссылка.Назначение
		|			КОНЕЦ
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|	КОНЕЦ КАК Назначение,
		|	ЗНАЧЕНИЕ(Справочник.КонтролируемыеПартии.ПустаяСсылка) КАК КонтролируемаяПартия,
	...
	Если СкорректироватьМерныеТовары Тогда
		...
		Если ДопустимыеОтклонения <> Неопределено Тогда
			...
			Если СтрокиКомплектов.Количество() <> 0 И Макс(Отклонение, -Отклонение) <= СтрокиКомплектов[0].ДопустимоеОтклонение Тогда
				...
				ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ДокументОбъект, СтруктураДействий, Неопределено);
