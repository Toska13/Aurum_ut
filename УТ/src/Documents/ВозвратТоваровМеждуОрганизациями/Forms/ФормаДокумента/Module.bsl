...
Процедура ТоварыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	УстановитьВидимостьЭлементовПоДокументамПередачи();
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ДействияЦены = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияЦеныВСтрокеТЧ(Объект);
	ДействияПересчетНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(Объект);
	Действия = Новый Структура;
	
	ДействиеЗаполнитьСтавкуНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект);
	ДействиеЗаполнитьСтавкуНДС.Организация = Объект.ОрганизацияПолучатель;
	Если ЗначениеЗаполнено(ТекущаяСтрока.ДокументПередачи) Тогда
		ДействиеЗаполнитьСтавкуНДС.ПоДатеИсправления = Истина;
		ДействиеЗаполнитьСтавкуНДС.ИсправляемыйДокумент = ТекущаяСтрока.ДокументПередачи;
	ИначеЕсли  ЗначениеЗаполнено(Объект.ДокументПередачи) Тогда
		ДействиеЗаполнитьСтавкуНДС.ПоДатеИсправления = Истина;
		ДействиеЗаполнитьСтавкуНДС.ИсправляемыйДокумент = Объект.ДокументПередачи;
	КонецЕсли;
	...
	Действия.Вставить("ЗаполнитьСтавкуНДСПоВидуЦен", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДСПоВидуЦен(Объект));
...
Процедура ОрганизацияПолучательПриИзмененииСервер()
	...
	Объект.БанковскийСчетОрганизацииПолучателя =
		ДенежныеСредстваСервер.ПолучитьБанковскийСчетОрганизацииПоУмолчанию(СтруктураПараметров);
	...
	ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтаФорма, "ОрганизацияПолучатель");
...
Процедура ВалютаПриИзмененииСервер(ВалютаНовая)
	...
	ВзаиморасчетыЗаполнениеРасшифровкиПлатежа.ПересчитатьСуммыВВалютуРасшифровкаПлатежа(Объект,Объект.Товары.Итог("СуммаСНДС"));
...
Процедура НалогообложениеНДСПриИзмененииСервер()
	...
	Если ЗначениеЗаполнено(Объект.ДокументПередачи) Тогда
		ДействиеЗаполнитьСтавкуНДС.Дата = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументПередачи, "Дата");
	КонецЕсли;
	...
	СтруктураДействий.Вставить("СкорректироватьСтавкуНДС", ДействиеЗаполнитьСтавкуНДС);
	...
	ДействияПересчетНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВТЧ(Объект);
	СтруктураДействий = Новый Структура();
	...
	ОбновитьТекстСчетаФактуры();
	НастроитьИтогиФормы(ЭтотОбъект);
...
Процедура ПартнерПриИзмененииСервер()
	...
	ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтаФорма, "Партнер");
...
Процедура ПоместитьРасшифровкуПлатежаВоВременноеХранилище()
	ВзаиморасчетыРаботаСФормами.ПоместитьРасшифровкуПлатежаВоВременноеХранилище(ЭтаФорма, "РасшифровкаПлатежа");
КонецПроцедуры
...
Функция ПоместитьТоварыИВидыЗапасовВХранилище()
	...
	ПараметрыРедактированияВидовЗапасов.Вставить("ОрганизацияПолучатель", Объект.ОрганизацияПолучатель);
	ПараметрыРедактированияВидовЗапасов.Вставить("ОтображатьВидЗапасовПолучателя", Истина);
...
Процедура ПриЧтенииСозданииНаСервере(ТекущийОбъект)
	...
	ОбновитьИнформациюПоПередачам(ОбновлятьДанные);
	ЗакупкиСервер.ЗаполнитьСписокВыбораНаименованиеВходящегоДокумента(ЭтотОбъект, Объект.Контрагент);
...
Процедура ПодборПоОстаткамЗавершениеСервер()
	ВзаиморасчетыРаботаСФормами.ЗагрузитьРасшифровкуПлатежаИзВременногоХранилища(ЭтаФорма);
КонецПроцедуры
...
Процедура ДоговорПриИзмененииСервер()
	...
	Объект.БанковскийСчетОрганизации = РеквизитыДоговора.БанковскийСчетПолучателя;
	Объект.БанковскийСчетОрганизацииПолучателя = РеквизитыДоговора.БанковскийСчет;
	
	Если ЗначениеЗаполнено(Объект.Организация) И Не ЗначениеЗаполнено(Объект.БанковскийСчетОрганизации) Тогда
	...
	Если ЗначениеЗаполнено(Объект.ОрганизацияПолучатель)
		И Не ЗначениеЗаполнено(Объект.БанковскийСчетОрганизацииПолучателя) Тогда
...
Процедура ДоговорПродажиПриИзмененииСервер()
	...
	ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтаФорма, "ДоговорПродажи");
	ВзаиморасчетыРаботаСФормами.ПриИзмененииПараметровМеханизма(ЭтаФорма);
...
Процедура ДоговорПокупкиПриИзмененииСервер()
	...
	ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтаФорма, "ДоговорПокупки");
	ВзаиморасчетыРаботаСФормами.ПриИзмененииПараметровМеханизма(ЭтаФорма);
...
&НаСервере
Процедура ОбновитьИнформациюПоПередачам(ОбновлятьДанные = Истина)
...
&НаСервере
Процедура УстановитьВидимостьЭлементовПоДокументамПередачи()
	
	ЗаполненДокументПередачи = Ложь;
	
	Для Каждого СтрокаТЧ Из Объект.Товары Цикл
		
		Если ЗначениеЗаполнено(СтрокаТЧ.ДокументПередачи)
			И СтрокаТЧ.СпособОпределенияСебестоимости =
				Перечисления.СпособыОпределенияСебестоимостиВозврата.ИзДокументаПередачи Тогда
			ЗаполненДокументПередачи = Истина;
			Прервать;
		КонецЕсли;
		
	КонецЦикла;
	
	Элементы.ОтражениеВУчете.Видимость = Не ЗаполненДокументПередачи;
