...
Функция ПараметрыЗаполненияНалогообложенияНДС(Документ) Экспорт
	...
	ЗаполняемыеПоля = "Дата, Склад, НаправлениеДеятельности, Подразделение";
	
	ВозвратТоваров =
		РеквизитыДокумента.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратТоваровМеждуОрганизациями;
	...
	ПараметрыЗаполнения.ВозвратТоваровОтКлиента = ВозвратТоваров;
	ПараметрыЗаполнения.ВозвратТоваровОтКомиссионера = Не ВозвратТоваров;
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|			ТОГДА NULL
	|		ИНАЧЕ ТаблицаЗапасов.ДокументПередачи
...
Функция ТекстЗапросаВтЗапасыПолучателя(Запрос, ТекстыЗапроса) Экспорт
	...
	|	ТаблицаВидыЗапасов.СуммаАкциза                                                              КАК СуммаАкциза,
	|	ЕСТЬNULL(Суммы.СуммаСНДСУпр, 0)                                                             КАК СуммаСНДСУпр,
	|	ЕСТЬNULL(Суммы.СуммаБезНДСУпр, 0)                                                           КАК СуммаБезНДСУпр,
	|	ЕСТЬNULL(Суммы.СуммаНДСУпр, 0)                                                              КАК СуммаНДСУпр,
	|	ЕСТЬNULL(Суммы.СуммаСНДСРегл, 0)                                                            КАК СуммаСНДСРегл,
...
Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР
	|		КОГДА ВидыЗапасов.СпособОпределенияСебестоимости = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимостиВозврата.ИзДокументаПередачи)
	|			И ВидыЗапасов.ПериодПродажи >= &НачалоМесяцаПериода
	|			И ВидыЗапасов.ТипЗапасов В (ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар),
	|				ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар))
	|			И (ВидыЗапасов.СкладОтгрузки <> ВидыЗапасов.Склад
...
Функция ОписаниеОперацийУчетаСебестоимости(ДокументОбъект) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ТаблицаВидыЗапасов.ВидЗапасовОтгрузкиУчетный <> ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)
	...
	|		ИЛИ ЕСТЬNULL(ТаблицаВидыЗапасов.СкладОтгрузки, ТаблицаДокумента.Склад) <> ЕСТЬNULL(ТаблицаВидыЗапасов.Склад, ТаблицаДокумента.Склад)
	...
	|		ИЛИ ТаблицаВидыЗапасов.Назначение <> ЕСТЬNULL(ТаблицаВидыЗапасов.НазначениеОтгрузки, ТаблицаВидыЗапасов.Назначение))
	|	И ТаблицаВидыЗапасов.СпособОпределенияСебестоимости = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимостиВозврата.ИзДокументаПередачи)
	|	И ТаблицаВидыЗапасов.ТипЗапасов В (
	|		ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар))
...
#Область Реализация_Товар_Сторно
	...
	|	И ЕСТЬNULL(ТаблицаВидыЗапасов.СкладОтгрузки, ТаблицаДокумента.Склад) = ЕСТЬNULL(ТаблицаВидыЗапасов.Склад, ТаблицаДокумента.Склад)
	|	И (ТаблицаВидыЗапасов.ВидЗапасовОтгрузкиУчетный = ТаблицаВидыЗапасов.ВидЗапасовУчетный
	|	ИЛИ ТаблицаВидыЗапасов.ВидЗапасовОтгрузкиУчетный = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка))
	|	И ЕСТЬNULL(ТаблицаВидыЗапасов.НазначениеОтгрузки, ТаблицаВидыЗапасов.Назначение) = ТаблицаВидыЗапасов.Назначение
	|";
...
Функция ДанныеДляПечатныхФормСчетаНаОплатуИзвещения(ПараметрыПечати, МассивОбъектов, КодЯзыка) Экспорт
	...
	|		КОГДА ЕСТЬNULL(Документы.БанковскийСчетОрганизации.СчетЦифровогоРубля, ЛОЖЬ)
	|			ТОГДА Документы.БанковскийСчетОрганизации.ИдентификаторСчетаЦифровогоРубля
	|		ИНАЧЕ Документы.БанковскийСчетКонтрагента.НомерСчета
	|	КОНЕЦ КАК НомерБанковскогоСчета,	
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(Документы.БанковскийСчетОрганизации.СчетЦифровогоРубля, ЛОЖЬ)
	|			ТОГДА """"
	...
	|	Документы.БанковскийСчетОрганизации.АдресБанкаДляРасчетов КАК АдресБанкаДляРасчетов,
	|	Документы.БанковскийСчетОрганизации.СчетВБанкеДляРасчетов КАК СчетВБанкеДляРасчетов,
	|	ЕСТЬNULL(Документы.БанковскийСчетОрганизации.СчетЦифровогоРубля, ЛОЖЬ) КАК СчетЦифровогоРубля,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(Документы.БанковскийСчетОрганизации.СчетЦифровогоРубля, ЛОЖЬ)
	|			ТОГДА Документы.БанковскийСчетОрганизации.ИдентификаторСчетаЦифровогоРубля
	|		ИНАЧЕ Документы.БанковскийСчетОрганизации.НомерСчета
