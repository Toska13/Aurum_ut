...
Процедура ИнициализироватьПараметрыЗаполненияВидовЗапасовДляПроведения(ДокументОбъект, РежимЗаписи) Экспорт
	...
	ИначеЕсли РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда
		...
		ПараметрыЗаполнения.Добавить(ПараметрыЗаполненияВидовЗапасов("Организация"));
		ПараметрыЗаполнения.Добавить(ПараметрыЗаполненияВидовЗапасов("ОрганизацияПолучатель"));
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Для Каждого СтрТабл Из Товары Цикл
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрТабл.ДокументПередачи) Тогда
			ЗаполненДокументПередачи = Истина;
		КонецЕсли;
	КонецЦикла;
	
	Если ЗаполненДокументПередачи Тогда
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ОрганизацияПолучатель) И Не ЗначениеЗаполнено(АдресДоставки) Тогда
		АдресДоставки = ФормированиеПечатныхФорм.ПолучитьАдресИзКонтактнойИнформации(
...
Процедура ЗаполнитьПоПередачеТоваров(Знач ДокументОснование)
	...
	Для Каждого СтрокаТоваров Из ОбрабатываемыеСтроки Цикл
		Ценообразование.ПересчитатьСуммыВСтрокеПоСуммеСНДС(СтрокаТоваров, ЦенаВключаетНДС, Ложь, Ложь, Истина);
	КонецЦикла;
...
Процедура ЗаполнитьНалогобложениеИЦеныПоУмолчанию()
	...
	ПараметрыЗаполнитьСтавкуНДС.Организация = ОрганизацияПолучатель;
	Если ЗначениеЗаполнено(ДокументПередачи) Тогда
		ПараметрыЗаполнитьСтавкуНДС.Дата = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументПередачи, "Дата");
	КонецЕсли;
	Действия = Новый Структура("ЗаполнитьСтавкуНДС", ПараметрыЗаполнитьСтавкуНДС);
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Товары, Действия, КэшированныеЗначения);
...
Процедура ОбработатьТабличнуюЧастьТовары()
	...
	СтруктураЗаполненияСтавкиНДС.Организация = ОрганизацияПолучатель;
	Если ЗначениеЗаполнено(ДокументПередачи) Тогда
		СтруктураЗаполненияСтавкиНДС.Дата = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументПередачи, "Дата");
	КонецЕсли;
...
Процедура ЗаполнитьБанковскиеСчета()
	...
	Если ЗначениеЗаполнено(БанковскийСчетОрганизации) И ЗначениеЗаполнено(БанковскийСчетОрганизацииПолучателя) Тогда
		Возврат;
	КонецЕсли;
	
	ПродажиСервер.ЗаполнитьБанковскиеСчетаПоДоговору(Договор,
		?(ЗначениеЗаполнено(БанковскийСчетОрганизацииПолучателя), Неопределено, БанковскийСчетОрганизацииПолучателя),
	...
	Если ЗначениеЗаполнено(ОрганизацияПолучатель) И Не ЗначениеЗаполнено(БанковскийСчетОрганизацииПолучателя) Тогда
		ПараметрыСчета = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации(ОрганизацияПолучатель);
		БанковскийСчетОрганизацииПолучателя = ДенежныеСредстваСервер.ПолучитьБанковскийСчетОрганизацииПоУмолчанию(
			ПараметрыСчета);
	КонецЕсли;
...
Процедура ЗаполнитьДопКолонкиВидовЗапасов(ПоТоварамКОформлению)
	...
	Если ВидыЗапасов.Найти(Справочники.ВидыЗапасов.ПустаяСсылка(), "ВидЗапасовПолучателя") <> Неопределено Тогда
		...
		|	ТаблицаВидыЗапасов.НомерСтроки КАК НомерСтроки,
		|	ТаблицаВидыЗапасов.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры
		|ПОМЕСТИТЬ ТаблицаВидыЗапасовДляЗапроса
		|ИЗ
		...
		|ИЗ
		|	ТаблицаВидыЗапасовДляЗапроса КАК ТаблицаВидыЗапасов
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ТаблицаВидыЗапасовДляЗапроса";
		...
		ИменаВыгружаемыхКолонок = "НомерСтроки, ВидЗапасовПолучателя, АналитикаУчетаНоменклатуры";
...
Функция ПроверитьИзменениеРеквизитовДокумента(МенеджерВременныхТаблиц)
	...
	ИменаРеквизитов =
	"Организация, ХозяйственнаяОперация, Склад, НалогообложениеНДС, ДокументПередачи, ВидыЗапасовУказаныВручную,
	|Партнер, ОрганизацияПолучатель,
	|Контрагент";
...
// Проверяет изменение состава таблицы Товары.
//
// Параметры:
//	МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - менеджер временных таблиц документа.
//
// Возвращаемое значение:
//	Булево - признак наличие изменений в таблице Товары.
//
