...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("АвтоТест") Тогда // Возврат при получении формы для анализа.
		Возврат;
	КонецЕсли;
	Элементы.ОтражениеВУчете.ЦветТекстаЗаголовка = Метаданные.ЭлементыСтиля.ГиперссылкаЦвет.Значение;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "РедактироватьНабор"
		...
		Если ЗначениеЗаполнено(Объект.ЧекККМ) Тогда
			ПараметрыОткрытия.Вставить("ВидЦены", ВидЦеныЧекаККМ);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение 
		И Не ЗначениеЗаполнено(Объект.ЗаявкаНаВозвратТоваровОтКлиента)
...
Процедура ПриИзмененииЧекаККМСервер(ПерезаполнитьТовары)
	...
	Если КонтрольКачества.ОбновитьПараметрыДействий(
			Документы.ВозвратТоваровОтКлиента,
			Объект,
	...
	УстановитьВидимостьЭлементовПоДокументамРеализации();
	ПорядокОтражения = ПорядокОтражения(Объект.ОтразитьВПрибылиУбыткахПрошлыхЛет);
	Элементы.ОтражениеВУчете.Заголовок = ЗаголовокОтраженияВУчете(Объект.ОтразитьВПрибылиУбыткахПрошлыхЛет);
...
Процедура ДокументРеализацииПриИзменении(Элемент)
	...
	ДокументРеализацииПриИзмененииСервер();
...
Процедура ДоговорПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.ЗаявкаНаВозвратТоваровОтКлиента) 
		И Договор <> Объект.Договор И ИспользоватьРасширенныеВозможностиЗаказаКлиента() Тогда
				
		ОтвязатьОтЗаявки = Ложь;
		ВариантыОтветов = Новый СписокЗначений;
		
		ВариантыОтветов.Добавить("ОтвязатьОтЗаявки", НСтр("ru = 'Отвязать от заявки'"));
		ВариантыОтветов.Добавить("Отмена", НСтр("ru = 'Отмена'"));
		ТекстВопроса = НСтр("ru='При изменении договора документ будет отвязан от заявки. Продолжить?'");
			
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ДоговорПриИзмененииЗавершение",
				ЭтотОбъект,
				Новый Структура("ОтвязатьОтЗаявки", ОтвязатьОтЗаявки)),
			ТекстВопроса,
			ВариантыОтветов);
...
Процедура ДоговорПриИзмененииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт 
	...
	ИначеЕсли РезультатВопроса = "ОтвязатьОтЗаявки" Тогда
		
		Объект.ЗаявкаНаВозвратТоваровОтКлиента = ПредопределенноеЗначение("Документ.ЗаявкаНаВозвратТоваровОтКлиента.ПустаяСсылка");
		УстановитьВидимостьЭлементовЗаявкиНаВозвратТоваров();
...
Процедура ТоварыДокументРеализацииПриИзменении(Элемент)
	...
	ИдентификаторТекущейСтроки = Элементы.Товары.ТекущиеДанные.ПолучитьИдентификатор();
	ТоварыДокументРеализацииПриИзмененииСервер(ИдентификаторТекущейСтроки);
...
Процедура УстановитьУсловноеОформление()
	...
	// Установка условного оформления номенклатуры другого качества.
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов, ЗагрузкаИзТСД = Ложь)
	...
	СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьСпособОпределенияСебестоимостиСтрокой", ПараметрыОпределенияСебестоимости(ЭтотОбъект));
	...
	СтруктураДействий.ИзменятьКоличество                     = Не ЗначениеЗаполнено(Объект.ЗаявкаНаВозвратТоваровОтКлиента);
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ОбновлятьДанные = Не ЗначениеЗаполнено(Объект.Ссылка);
	ОбновитьИнформациюПоРеализациям(ОбновлятьДанные);
...
Процедура ПолучитьРасшифровкуПлатежаИзХранилища(АдресПлатежейВХранилище)
	...
	ВзаиморасчетыРаботаСФормами.ОбновитьРаспределеннуюСуммаРасшифровки(ЭтаФорма);
...
Процедура ВыполнитьПересчетСуммДокументаСервер(СтараяВалюта, НоваяВалюта)
	...
	ВзаиморасчетыЗаполнениеРасшифровкиПлатежа.ПересчитатьСуммыВВалютуРасшифровкаПлатежа(Объект, СуммаТоваров);
...
Процедура ИзменитьТабличнуюЧастьПоРезультатамПроверки(ВозвращаемыеПараметры, КэшированныеЗначения)
	...
	СтруктураДействийДляНовыхСтрок.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества",
		КонтрольКачестваКэш);
...
&НаСервере
Процедура ОбновитьИнформациюПоРеализациям(ОбновлятьДанные = Истина)
	...
	УстановитьПредставлениеСебестоимости();
	УчетПодакцизныхТоваров.ОбновитьЭлементыУчетаПодакцизныхТоваров(ЭтотОбъект, ОбновлятьДанные);
...
&НаСервере
Процедура УстановитьВидимостьЭлементовПоДокументамРеализации()
	
	ЗаполненДокументРеализации = Ложь;
	
	Для Каждого СтрокаТЧ Из Объект.Товары Цикл
		
		Если ЗначениеЗаполнено(СтрокаТЧ.ДокументРеализации)
			И СтрокаТЧ.СпособОпределенияСебестоимости =
				Перечисления.СпособыОпределенияСебестоимостиВозврата.ИзДокументаПродажи Тогда
			ЗаполненДокументРеализации = Истина;
			Прервать;
		КонецЕсли;
		
	КонецЦикла;
	
	Элементы.ОтражениеВУчете.Видимость = Не ЗаполненДокументРеализации;
	
КонецПроцедуры

&НаСервере
Процедура УстановитьВидимостьЭлементовЗаявкиНаВозвратТоваров()
	...
	Иначе
		МассивЭлементов.Добавить("ГруппаЗаявка");
	...
	Элементы.ГруппаЗаявка.Видимость = ЗначениеЗаполнено(Объект.ЗаявкаНаВозвратТоваровОтКлиента) И ИспользоватьРасширенныеВозможностиЗаказаКлиента;
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	Колонки = "Номенклатура,Характеристика,Упаковка,КоличествоУпаковок,Цена,НомерГТД,СпособОпределенияСебестоимости,
		...
		|НоменклатураНабора,ХарактеристикаНабора,Склад";
...
Процедура ПоместитьРасшифровкуПлатежаВоВременноеХранилище()
	ВзаиморасчетыРаботаСФормами.ПоместитьРасшифровкуПлатежаВоВременноеХранилище(ЭтаФорма, "УменьшенДолгСтрокой");
КонецПроцедуры
...
Процедура ЗаполнитьСтавкуНДСНаСервере(СтавкаНДС, КэшированныеЗначения)
	...
	Для каждого СтрокаТабличнойЧасти Из ВыделенныеСтроки Цикл
		Если Объект.ВозвратПереданнойМногооборотнойТары
...
&НаСервере
Процедура ДокументРеализацииПриИзмененииСервер()
...
&НаСервере
Процедура ТоварыДокументРеализацииПриИзмененииСервер(ИдентификаторТекущейСтроки)
	...
	Если ТекущаяСтрока.ДокументРеализации <> Неопределено И Не ЗначениеЗаполнено(ТекущаяСтрока.ДокументРеализации)
	...
	ИначеЕсли ТекущаяСтрока.ДокументРеализации <> Неопределено И ЗначениеЗаполнено(ТекущаяСтрока.ДокументРеализации)
		И Не ЗначениеЗаполнено(Объект.ЧекККМ)
...
&НаКлиентеНаСервереБезКонтекста
Функция ПорядокОтражения(ОтразитьВПрибылиУбыткахПрошлыхЛет)
	
	Если ОтразитьВПрибылиУбыткахПрошлыхЛет Тогда
...
&НаКлиентеНаСервереБезКонтекста
Функция ЗаголовокОтраженияВУчете(ОтразитьВПрибылиУбыткахПрошлыхЛет)
	
	ЗаголовокОтражениеВУчете = НСтр("ru = 'Отражение в учете: %1'");
	
	Если ОтразитьВПрибылиУбыткахПрошлыхЛет Тогда
		ОтражениеБУ = НСтр("ru = 'прочие доходы/расходы'");
	Иначе
		ОтражениеБУ = НСтр("ru = 'уменьшение выручки и себестоимости продаж'");
	КонецЕсли;
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ЗаголовокОтражениеВУчете, ОтражениеБУ);
...
Процедура УстановитьСвойстваКомментарияИспользованияУстаревшейКомиссии()
	...
	Элементы.ГруппаКомментарийИспользованиеУстаревшейКомиссии.Видимость = Не ВозвратТоваровЧерезКомиссионера
