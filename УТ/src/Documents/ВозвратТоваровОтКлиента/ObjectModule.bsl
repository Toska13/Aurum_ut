...
Процедура ЗаполнитьУсловияПродажПоУмолчанию() Экспорт
	...
	Если ЗначениеЗаполнено (Партнер) ИЛИ НЕ ИспользоватьСоглашенияСКлиентами Тогда
		...
		КомиссионныеПродажи25 =
			(ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратТоваровЧерезКомиссионера);
		
		ПараметрыОтбора = Новый Структура();
		ПараметрыОтбора.Вставить("ВыбранноеСоглашение",   Соглашение);
		ПараметрыОтбора.Вставить("ПустаяСсылкаДокумента", Документы.ВозвратТоваровОтКлиента.ПустаяСсылка());
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ЕстьПорчаВДокументе = Ложь;
	ЗаполненДокументРеализации = Ложь;
	Для Каждого СтрТабл Из Товары Цикл
		
		Если СтрТабл.Порча Тогда
			ЕстьПорчаВДокументе = Истина;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СтрТабл.ДокументРеализации) Тогда
			ЗаполненДокументРеализации = Истина;
		КонецЕсли;
		
		Если ЕстьПорчаВДокументе И ЗаполненДокументРеализации Тогда
			Прервать;
		КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКачествоТоваров") Тогда
		ВозвратПорчи = ЕстьПорчаВДокументе;
	КонецЕсли;
	
	Если ЗаполненДокументРеализации Тогда
		ОтразитьВПрибылиУбыткахПрошлыхЛет = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьДокументНаОснованииЗаявкиНаВозвратТоваровОтКлиента(Знач ДокументОснование)
	...
	|	(ЗаявкиНаВозвратТоваровОтКлиентов.КОформлениюПоВозвратамКоличествоОборот -
	|		ЗаявкиНаВозвратТоваровОтКлиентов.ОформленоПоВозвратамКоличествоОборот) КАК Количество,
	|	ЗаявкаТовары.КоличествоУпаковок КАК КоличествоУпаковок,
	|	ЗаявкаТовары.Сумма КАК Сумма,
	|	ЗаявкаТовары.Склад КАК Склад,
	|	ЗаявкаТовары.ДокументРеализации КАК ДокументРеализации
	|ПОМЕСТИТЬ ЗаявкаТовары
	|ИЗ
	|	ВТ_ЗаявкиНаВозвратОборотыРазвернутая КАК ЗаявкиНаВозвратТоваровОтКлиентов
	...
	|ГДЕ
	|	(ЗаявкиНаВозвратТоваровОтКлиентов.КОформлениюПоВозвратамКоличествоОборот -
	|		ЗаявкиНаВозвратТоваровОтКлиентов.ОформленоПоВозвратамКоличествоОборот) > 0
	...
	Пока ВыборкаТовары.Следующий() Цикл
		...
		ЗаполнитьЗначенияСвойств(СтрокаТаб, ВыборкаТовары, "НоменклатураНабора, ХарактеристикаНабора,
			|Номенклатура, Характеристика, Серия, Склад, Назначение, Упаковка, КодСтроки, Цена, СтавкаНДС, ДокументРеализации");
		
		//@skip-check bsl-legacy-check-string-literal
		Если Не ЗначениеЗаполнено(СтрокаТаб.ДокументРеализации)
			И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "ХозяйственнаяОперация") =
				Перечисления.ХозяйственныеОперации.ВозвратОтРозничногоПокупателя Тогда
...
Процедура ЗаполнитьПоЗаказу(СтруктураЗаполнения)
	...
	Если СтруктураЗаполнения.РеквизитыШапки.СпособКомпенсации = Перечисления.СпособыКомпенсацииВозвратовТоваров.ЗаменитьТовары Тогда
		СтруктураЗаполнения.РеквизитыШапки.СпособКомпенсации = Перечисления.СпособыКомпенсацииВозвратовТоваров.ВернутьДенежныеСредства;
	КонецЕсли;
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	Автор                = Пользователи.ТекущийПользователь();
	ДокументОснование = ПродажиСервер.ДокументОснование(ЭтотОбъект,
	...
	СтруктураЗаполненияСтавкиНДС = 
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(
			ЭтотОбъект, Ложь, ВозвратПереданнойМногооборотнойТары);
	...
	Если ЗначениеЗаполнено(ДокументРеализации) Тогда
		СтруктураЗаполненияСтавкиНДС.Дата = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументРеализации, "Дата");
	КонецЕсли;
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	Иначе
		...
		Если Не Проведен
			Или ЗапасыСервер.ПроверитьНеобходимостьПерезаполненияВидовЗапасовДокумента(ЭтотОбъект)
			Или ПроверитьИзменениеРеквизитовДокумента(МенеджерВременныхТаблиц, "НеИзДокументаПродажи")
			Или ПроверитьИзменениеТоваров(МенеджерВременныхТаблиц) Тогда
	...
	Если ВидыЗапасовПерезаполнены Тогда
		...
		ЗаполнитьСебестоимостьВидовЗапасов(СоответствиеТиповНоменклатуры, ДокументСодержитОборотнуюТару, Отказ);
...
Процедура ЗаполнитьНоменклатуруОприходованиеВидовЗапасов(Отказ)
	...
	МассивТоваров	= Товары.НайтиСтроки(ОтборСтрок);
	...
	Для Каждого СтрокаТоваров Из МассивТоваров Цикл
		...
		Для Каждого СтрокаЗапасов Из ВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			...
			НоваяСтрока.СуммаСНДС					= Мин(СуммаСНДС,
														?(СтрокаЗапасов.Количество <> 0,
															Количество * СтрокаЗапасов.СуммаСНДС / СтрокаЗапасов.Количество,
															0));
			НоваяСтрока.СуммаНДС					= Мин(СуммаНДС,
														?(СтрокаЗапасов.Количество <> 0,
			...
			ПеренесенныеВидыЗапасов.Добавить(СтрокаЗапасов.ВидЗапасов);
...
Процедура СинхронизироватьСвойстваВидовЗапасовОприходования(ПеренесенныеВидыЗапасов, Отказ)
	...
	|	И СправочникНоменклатура.ГруппаФинансовогоУчета <> ВидыЗапасов.ГруппаФинансовогоУчета
	|	И ТаблицаТоваров.Номенклатура <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
	
	ИменаВыгружаемыхКолонок = "НомерСтроки, НоменклатураОприходование, ВидЗапасов";
	...
	Для Каждого СтрокаЗапасов Из ВидыЗапасов Цикл
		...
		СтрокаЗапасов.ВидЗапасов = ВидЗапасовОприходование;
	КонецЦикла;
...
Функция ПроверитьИзменениеРеквизитовДокумента(МенеджерВременныхТаблиц, ОтборПоСпособуОпределенияСебестоимости = "Все")
	
	КоллекцияРеквизитов = Новый Массив;
	КоллекцияРеквизитов.Добавить("Дата");
	КоллекцияРеквизитов.Добавить("Организация");
	КоллекцияРеквизитов.Добавить("ДокументРеализации");
	КоллекцияРеквизитов.Добавить("НалогообложениеНДС");
	
	
	ИменаРеквизитов = СтрСоединить(КоллекцияРеквизитов, ",");
...
//
// Параметры:
//	МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - временная таблица товаров с аналитикой обособленного учета.
//
...
Процедура ЗаполнитьСебестоимостьВидовЗапасов(СоответствиеТиповНоменклатуры, ДокументСодержитОборотнуюТару, Отказ)
...
Процедура ЗаполнитьНомераГТДПоРаспоряжениям(ЦенаВключаетНДС, ПересчитатьСуммовыеПоказатели = Истина)
	...
	Если Не ПересчитатьСуммовыеПоказатели Тогда
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Товары, ДействияСоСтрокой, Неопределено);
	КонецЕсли;
