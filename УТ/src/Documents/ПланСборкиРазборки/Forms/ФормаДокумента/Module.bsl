...
Процедура ОбновитьСтруктуруКроссТаблицы(ТаблицаПериоды, ПересчитыватьИтоговыеПоказатели = Истина)
	...
	Для каждого Период Из ТаблицаПериоды Цикл
		Если НЕ Период.Активная Тогда
			Продолжить;
		КонецЕсли; 
		Элементы.ТоварыДатаСборкиРазборки.СписокВыбора.Добавить(НачалоДня(Период.ДатаНачала), Период.Заголовок);
	КонецЦикла; 
	...
	Пока СписокИсходныеТовары.Количество() <> 0 Цикл
		...
		Для каждого ДанныеПодсборки Из СписокПодсборок Цикл
			...
			Иначе
				...
				ОбрабатываемыеСтроки.Добавить(СтрокаПодсборка);
...
Функция ПараметрыПреобразованияКроссТаблицы()
	...
	СтруктураРеквизитовПериода = Новый Структура("ДатаНачала","ДатаСборкиРазборки");
	...
	Иначе
		...
		Если Объект.ЗаполнятьПоФормуле
			...
			ПланированиеКлиентСервер.ОтобразитьЗначениеДополнительныхПараметров(
				ЭтотОбъект, ПолучитьДопРеквизиты(ЭтотОбъект), "ТоварыПоДатам");
		КонецЕсли;
