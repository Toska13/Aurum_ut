...
Функция ПараметрыВзаиморасчеты(ДанныеЗаполнения = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ЗначениеСсылочногоТипа(ДанныеЗаполнения) Тогда
		...
		ДатаДокумента         = СтруктураДанныеЗаполнения.Дата;
		ДатаПроведенияБанком  = СтруктураДанныеЗаполнения.ДатаПроведенияБанком;
	ИначеЕсли ДанныеЗаполнения = Неопределено Тогда
		...
		ДатаПроведенияБанком  = Дата('00010101');
	Иначе
		...
		ДатаПроведенияБанком  = ДанныеЗаполнения.ДатаПроведенияБанком;
	КонецЕсли;
	...
	СтруктураПараметров.СуммаВзаиморасчетов              = "Форма.СуммаВзаиморасчетов";
	СтруктураПараметров.КурсЧислитель                    = "Форма.КурсЧислительВзаиморасчетов";
	СтруктураПараметров.КурсЗнаменатель                  = "Форма.КурсЗнаменательВзаиморасчетов";
	...
	Если ПроведеноБанком
		И НачалоДня(ДатаДокумента) <> ДатаПроведенияБанком Тогда
	...
	СтруктураПараметров.ПодборТолькоБезусловнойЗадолженности = ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОплатыКлиенту
		ИЛИ ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратДенежныхСредствВДругуюОрганизацию;
	СтруктураПараметров.ОплатаПоЗаявкам                  = "Объект.ОплатаПоЗаявкам";
...
//
// Параметры:
//	Объект - ДокументОбъект.СписаниеБезналичныхДенежныхСредств, Структура - данные документа.
...
Процедура ЗаполнитьИменаРеквизитовПоХозяйственнойОперации(Объект, МассивВсехРеквизитов, МассивРеквизитовОперации) Экспорт
	...
	МассивВсехРеквизитов.Добавить("ВестиУчетКомиссииВоВзаиморасчетах");
	МассивВсехРеквизитов.Добавить("ОбъектРасчетовКомиссии");
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОплатыНаПлатежнуюКарту Тогда
		...
		МассивРеквизитовОперации.Добавить("ОплатаПоЗаявкам");
		МассивРеквизитовОперации.Добавить("СтатьяДвиженияДенежныхСредств");
		МассивРеквизитовОперации.Добавить("ВестиУчетКомиссииВоВзаиморасчетах");
		МассивРеквизитовОперации.Добавить("ОбъектРасчетовКомиссии");
...
Процедура ЗаполнитьНастройкиПолейФормы(Настройки) Экспорт
	...
	УсловиеЗаявкиНаРасходДС = Финансы.НовыйОтбор(ГруппаИли, , , Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	УсловиеЗаявкиНаРасходДС.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ;
	Финансы.НовыйОтбор(УсловиеЗаявкиНаРасходДС, "Дополнительно.ИспользоватьЗаявкиНаРасходованиеДенежныхСредств", Истина);
	
	ПодГруппаИли = Финансы.НовыйОтбор(УсловиеЗаявкиНаРасходДС,,, Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ПодГруппаИли.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
	
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.ОплатаПоставщику);
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.ВозвратОплатыКлиенту);
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.ВыдачаДенежныхСредствПодотчетнику);
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.ОплатаДенежныхСредствВДругуюОрганизацию);
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.ПрочаяВыдачаДенежныхСредств);
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.ПеречислениеТаможне);
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.ПеречислениеВБюджет);
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.ВозвратДенежныхСредствВДругуюОрганизацию);
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.ВнутренняяПередачаДенежныхСредств);
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.КонвертацияВалюты);
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.ПеречислениеНаДепозиты);
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.ВыдачаЗаймов);
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.ОплатаПоКредитам);
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.ВыдачаЗаймаСотруднику);
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.ВыплатаЗарплатыПоЗарплатномуПроекту);
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.ВыплатаЗарплатыНаЛицевыеСчета);
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.ВозвратОплатыНаПлатежнуюКарту);
	Финансы.СкопироватьОтбор(ОтборВыплатаСамозанятому, ПодГруппаИли);
	
	УсловиеЗаявкиНаКомандировку = Финансы.НовыйОтбор(ГруппаИли, , , Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	УсловиеЗаявкиНаКомандировку.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ;
	Финансы.НовыйОтбор(УсловиеЗаявкиНаКомандировку, "Дополнительно.ИспользоватьЗаявкиНаКомандировку", Истина);
	Финансы.НовыйОтбор(УсловиеЗаявкиНаКомандировку, "ХозяйственнаяОперация", Операции.ВыдачаДенежныхСредствПодотчетнику);
	...
	// ДоговорКомиссии
	Элемент = Настройки.Добавить();
	Элемент.Поля.Добавить("ДоговорКомиссии");
	Финансы.НовыйОтбор(Элемент.Условие, "ВестиУчетКомиссииВоВзаиморасчетах", Истина);
	Элемент.Свойства.Вставить("Видимость");
	
	// ВестиУчетКомиссииВоВзаиморасчетахПереключатель
	Элемент = Настройки.Добавить();
	Элемент.Поля.Добавить("ВестиУчетКомиссииВоВзаиморасчетахПереключатель");
	Финансы.НовыйОтбор(Элемент.Условие, "ХозяйственнаяОперация", Операции.ВозвратОплатыНаПлатежнуюКарту);
	Элемент.Свойства.Вставить("Видимость");
	...
	// Отличается УИП
	Элемент = Настройки.Добавить();
	Элемент.Поля.Добавить("ДекорацияУИПОтличается");
	Элемент.Поля.Добавить("НадписьУИПОтличается");
	Элемент.Поля.Добавить("ДекорацияУИПОтличаетсяСписком");
	Элемент.Поля.Добавить("НадписьУИПОтличаетсяСписком");
	Элемент.Поля.Добавить("ДекорацияУИПОтличаетсяБезРазбиения");
	Элемент.Поля.Добавить("РасшифровкаПлатежаДекорацияУИПОтличается");
	Финансы.НовыйОтбор(Элемент.Условие, "Дополнительно.УИПОтличается", Истина);
	Финансы.НовыйОтбор(Элемент.Условие, "Дополнительно.ВалютныйПлатеж", Ложь);
	Финансы.НовыйОтбор(Элемент.Условие, "ПеречислениеВБюджет", Ложь);
	Финансы.НовыйОтбор(Элемент.Условие, "Дополнительно.РазрешеныПлатежиБезУказанияЗаявок", Истина);
	Финансы.НовыйОтбор(Элемент.Условие, "ОплатаПоЗаявкам", Ложь);
	Элемент.Свойства.Вставить("Видимость");
	#КонецОбласти
	...
	Финансы.НовыйОтбор(ГруппаИ, "Дополнительно.КурсЧислительВзаиморасчетов", 0,, ВидСравненияКомпоновкиДанных.НеРавно);
	Элемент.Свойства.Вставить("Доступность");
	...
	ГруппаИли = Финансы.НовыйОтбор(Элемент.Условие,,, Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ГруппаИли.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
	Финансы.НовыйОтбор(ГруппаИли, "Дополнительно.ИспользоватьНачислениеЗарплаты", Истина);   
	...
	ГруппаИли2 = Финансы.НовыйОтбор(ГруппаИ,,, Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ГруппаИли2.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
	Финансы.НовыйОтбор(ГруппаИли2, "ТипНалога", Справочники.ВидыНалоговВзносов.НДФЛ);
	Финансы.НовыйОтбор(ГруппаИли2, "ТипНалога", Справочники.ВидыНалоговВзносов.НДФЛСПревышения);
	...
	Финансы.НовыйОтбор(ГруппаИли1, "ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыПоЗарплатномуПроекту);
	Финансы.НовыйОтбор(ГруппаИли1, "ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыНаЛицевыеСчета);
	//-- Локализация
	...
	// Точность курса
	ДенежныеСредстваСервер.ЗаполнитьУсловноеОформлениеТочностьКурса(Настройки);
...
Функция ПараметрыВыбораСтатейИАналитик(ХозяйственнаяОперация, ДополнительнаяДоступность = Ложь) Экспорт
	...
	ДоступностьПоОперацииВозвратОплатыНаПлатежнуюКарту =
		(ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОплатыНаПлатежнуюКарту) И ДополнительнаяДоступность;
	ДоступностьПоОперацииНеВозвратОплатыНаПлатежнуюКарту =
		(ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ВозвратОплатыНаПлатежнуюКарту) И ДополнительнаяДоступность;
	
	ПараметрыВыбора.ДоступностьПоОперации = ДоступностьПоОперацииВозвратОплатыНаПлатежнуюКарту
		Или ДоступностьПоОперацииНеВозвратОплатыНаПлатежнуюКарту;
...
// Возвращает признак того, не занят ли указанный номер документа за период - год.
//
// Параметры:
//   Дата - Дата - дата, по которой выполняется проверка уникальности номера
//   ПроверяемыйНомер - Строка - номер документа, проверяемый на уникальность
//
// Возвращаемое значение:
//   Булево - Истина, если номер уникален, Ложь, если номер уже существует
//
Функция ЭтоУникальныйНомерДокумента(Дата, ПроверяемыйНомер) Экспорт
	...
	Запрос.Текст = 
		...
		|	И СписаниеБезналичныхДенежныхСредств.Дата МЕЖДУ НАЧАЛОПЕРИОДА(&Дата, ГОД) И КОНЕЦПЕРИОДА(&Дата, ГОД)";
	
	Запрос.УстановитьПараметр("Дата", Дата);
	Запрос.УстановитьПараметр("Номер", ПроверяемыйНомер);
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	КОНЕЦ КАК Период,
	|	ВЫБОР КОГДА НЕ ДанныеДокумента.ПроведеноБанком
	|		ИЛИ НАЧАЛОПЕРИОДА(ДанныеДокумента.Дата, ДЕНЬ) = ДанныеДокумента.ДатаПроведенияБанком ТОГДА
	|		ДанныеДокумента.Дата
	|	ИНАЧЕ
	|		КОНЕЦПЕРИОДА(ДанныеДокумента.ДатаПроведенияБанком, ДЕНЬ)
	|	КОНЕЦ КАК ПериодКурсов,
	...
	СписаниеБезналичныхДенежныхСредствЛокализация.ДополнитьВидыНалогов(ВидыНалогов);
...
Процедура ТекстЗапросаДанныеДокументаШапка(СписокЗапросов)
	...
	|	ДанныеДокумента.ОбъектРасчетовКомиссии КАК ОбъектРасчетовКомиссии,
	|	ДанныеДокумента.ВестиУчетКомиссииВоВзаиморасчетах КАК ВестиУчетКомиссииВоВзаиморасчетах,
	...
	|	ДанныеДокумента.ДоговорЭквайринга.Партнер КАК ДоговорЭквайрингаПартнер,
...
Процедура ТекстЗапросаДанныеДокументаТаблицаРасшифровкаПлатежа(СписокЗапросов)
	...
	|	ТаблицаРасшифровкаПлатежа.СуммаВВалютеОтправителя КАК СуммаВВалютеОтправителя,
	|	ТаблицаРасшифровкаПлатежа.СуммаНДСВВалютеОтправителя КАК СуммаНДСВВалютеОтправителя
...
Функция ТекстЗапросаВременнаяТаблицаРасшифровкаПлатежа(Запрос, ТекстыЗапроса, ДокументОбъект = Неопределено) Экспорт
	...
	|		ТаблицаРасшифровкаПлатежа.СуммаВВалютеОтправителя
	|	ИНАЧЕ
	...
	|			ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.СуммаВВалютеОтправителя * &КоэффициентПересчетаДенежныхСредствВВалютуУпр КАК ЧИСЛО(31,2))
	...
	|			ТаблицаРасшифровкаПлатежа.Сумма
	|		КОГДА ДанныеДокумента.ПлатежСКонвертацией ТОГДА
	|			ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.СуммаВВалютеОтправителя * &КоэффициентПересчетаДенежныхСредствВВалютуРегл КАК ЧИСЛО(31,2))
	|		ИНАЧЕ ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.Сумма * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31,2))
	|	КОНЕЦ КАК СуммаРегл,
	|	
	|	ТаблицаРасшифровкаПлатежа.СтавкаНДС КАК СтавкаНДС,
	|	ВЫБОР КОГДА ДанныеДокумента.ПлатежСКонвертацией ТОГДА
	|		ТаблицаРасшифровкаПлатежа.СуммаНДСВВалютеОтправителя
	...
	|		КОГДА ДанныеДокумента.ПлатежСКонвертацией ТОГДА
	|			ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.СуммаНДСВВалютеОтправителя * &КоэффициентПересчетаДенежныхСредствВВалютуУпр КАК ЧИСЛО(31,2))
	...
	|		ТОГДА ТаблицаРасшифровкаПлатежа.СуммаНДС
	|		КОГДА ДанныеДокумента.ПлатежСКонвертацией ТОГДА
	|			ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.СуммаНДСВВалютеОтправителя * &КоэффициентПересчетаДенежныхСредствВВалютуРегл КАК ЧИСЛО(31,2))
	...
	Если ДокументОбъект = Неопределено Тогда
		...
		Если Запрос.МенеджерВременныхТаблиц.Таблицы.Найти("ДанныеДокументаТаблицаРасшифровкаПлатежа") = Неопределено 
			Или Запрос.МенеджерВременныхТаблиц.Таблицы.Найти("ДанныеДокументаШапка") = Неопределено Тогда
...
Функция ТекстЗапросаТаблицаДенежныеСредстваБезналичные(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если (ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаПоставщику
		...
		И НЕ ДокументОбъект.СписокФизЛиц
		И Не ДокументОбъект.ПлатежСКонвертацией Тогда
		...
		|	
		|	ВЫБОР
		|		КОГДА ДанныеДокумента.ПроведеноБанком ТОГДА
		|			ТаблицаРасшифровкаПлатежа.Сумма
		|	ИНАЧЕ
		|		0
		|	КОНЕЦ КАК Сумма,
		...
		|	
		|	ВЫБОР
		|		КОГДА НЕ ДанныеДокумента.ПроведеноБанком ТОГДА
		|			ТаблицаРасшифровкаПлатежа.Сумма
		|	ИНАЧЕ
		|		0
		|	КОНЕЦ КАК КСписанию,
		...
		|			ТОГДА Неопределено
		|		ИНАЧЕ ТаблицаРасшифровкаПлатежа.АналитикаУчетаПоПартнерам
	...
	Если ДокументОбъект.ПлатежСКонвертацией Тогда
		...
		|	ЛЕВОЕ СОЕДИНЕНИЕ
		|		ТаблицаРасшифровкаПлатежа КАК ТаблицаРасшифровкаПлатежа
		|	ПО ДанныеДокумента.ПроведеноБанком = ЛОЖЬ";
...
Функция ТекстЗапросаТаблицаДенежныеСредстваКВыплате(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект = Неопределено)
	...
	ТекстЗапроса = "
	|ВЫБРАТЬ // Заявка в расшифровке, оплата в пределах заявки.
	|	&Период КАК Период,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	...
	|		И ДенежныеСредства.Регистратор <> &ИсправляемыйДокумент
	|
	|ГДЕ
	|	ДанныеДокумента.Ссылка = &Ссылка
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|	
	|ВЫБРАТЬ // Заявка в расшифровке, оплата сверх заявки.
	|	&Период КАК Период,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	ЗаявкиРасшифровки.ЗаявкаНаРасходованиеДенежныхСредств КАК ЗаявкаНаРасходованиеДенежныхСредств,
...
Функция ТекстЗапросаТаблицаВтИсходныеПрочиеРасходы(Запрос, ТекстыЗапроса, ДокументОбъект = Неопределено)
	...
	|	И НЕ ДанныеДокумента.ВестиУчетКомиссииВоВзаиморасчетах
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Процедура СформироватьСуммыДокументаВВалютахУчета(Запрос, ТекстыЗапроса, Регистры = Неопределено, ДокументОбъект = Неопределено) Экспорт
	...
	|	ВЫБОР
	|		КОГДА НЕ ТаблицаДокумента.ПроведеноБанком
	|				ИЛИ НАЧАЛОПЕРИОДА(ТаблицаДокумента.Дата, ДЕНЬ) = ТаблицаДокумента.Ссылка.ДатаПроведенияБанком ТОГДА
	|			ТаблицаДокумента.Дата
	|		ИНАЧЕ
	|			КОНЕЦПЕРИОДА(ТаблицаДокумента.ДатаПроведенияБанком, ДЕНЬ)
	|	КОНЕЦ                   КАК ДатаКурса,
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ПлатежСКонвертацией ТОГДА
	|			Строки.СуммаВВалютеОтправителя - Строки.СуммаНДСВВалютеОтправителя
	|		ИНАЧЕ ЕСТЬNULL(Строки.Сумма - Строки.СуммаНДС, ТаблицаДокумента.СуммаДокумента)
	|	КОНЕЦ КАК СуммаБезНДС,
	|	ЕСТЬNULL(Строки.СтавкаНДС, НЕОПРЕДЕЛЕНО) КАК СтавкаНДС,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ПлатежСКонвертацией
	|			ТОГДА Строки.СуммаНДСВВалютеОтправителя
	|		ИНАЧЕ ЕСТЬNULL(Строки.СуммаНДС, 0)
	|	КОНЕЦ КАК СуммаНДС,
	...
	|	ВЫБОР
	|		КОГДА НЕ ТаблицаДокумента.ПроведеноБанком
	|				ИЛИ НАЧАЛОПЕРИОДА(ТаблицаДокумента.Дата, ДЕНЬ) = ТаблицаДокумента.ДатаПроведенияБанком ТОГДА
	|			ТаблицаДокумента.Дата
	|		ИНАЧЕ
	|			КОНЕЦПЕРИОДА(ТаблицаДокумента.ДатаПроведенияБанком, ДЕНЬ)
	|	КОНЕЦ                   КАК ДатаКурса,
	...
	|		СУММА(ВЫБОР
	|			КОГДА &ПлатежСКонвертацией ТОГДА
	|				ТаблицаСумм.СуммаВВалютеОтправителя
...
Функция ТекстЗапросаТаблицаПрочиеАктивыПассивы(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеВБюджет Тогда
		...
		|	&Организация КАК Организация,
		|	ВЫБОР
		|		КОГДА ДанныеДокумента.ТипНалога В (&ВидыНалогов)
		|		ТОГДА ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
		|	ИНАЧЕ ДанныеДокумента.Подразделение КОНЕЦ КАК Подразделение,
		|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) КАК НаправлениеДеятельности,
...
Функция ТекстЗапросаТаблицаДвиженияДенежныеСредстваКонтрагент(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОплатыНаПлатежнуюКарту
		И ДокументОбъект.СуммаКомиссии <> 0 И ДокументОбъект.ОтражатьКомиссию И ДокументОбъект.ПроведеноБанком 
		И ДокументОбъект.ВестиУчетКомиссииВоВзаиморасчетах Тогда
		
		ТекстПодзапросаЗапроса = "//Комиссия за эквайринг
		|	ВЫБРАТЬ
		|		&Период КАК Период,
		|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КомиссияПоЭквайрингу) КАК ХозяйственнаяОперация,
		|		ДанныеДокумента.Организация КАК Организация,
		...
		|
		|		ДанныеДокумента.ДоговорЭквайрингаПартнер КАК Партнер,
		|		ДанныеДокумента.ОбъектРасчетовКомиссии.Контрагент КАК Контрагент,
		|		ДанныеДокумента.ОбъектРасчетовКомиссии.НаправлениеДеятельности КАК НаправлениеДеятельности,
		...
		|		0 КАК СуммаПредоплатыВВалютеПлатежа,
		|
		|		ВЫРАЗИТЬ(ДанныеДокумента.СуммаКомиссии * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(31,2)) КАК СуммаКВыплатеСверхЛимита,
		|	
		|		0 КАК СуммаНДС,
		|		0 КАК СуммаНДСРегл,
		|		0 КАК СуммаНДСВВалютеПлатежа,
		...
		|
		|		ДанныеДокумента.БанковскийСчет КАК ИсточникГФУДенежныхСредств,
		|		ДанныеДокумента.ОбъектРасчетовКомиссии КАК ИсточникГФУРасчетов,
		|		НЕОПРЕДЕЛЕНО КАК СтавкаНДС,
		|		НЕОПРЕДЕЛЕНО КАК ЗаявкаНаРасходованиеДенежныхСредств
		|	ИЗ
		|		ДанныеДокументаШапка КАК ДанныеДокумента
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если НЕ (ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОплатыКлиенту
		ИЛИ ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратДенежныхСредствВДругуюОрганизацию
		ИЛИ ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаПоставщику
		ИЛИ ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОплатыНаПлатежнуюКарту
		ИЛИ ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаДенежныхСредствВДругуюОрганизацию
		ИЛИ ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеТаможне) Тогда
		Возврат;
	КонецЕсли;
	...
	ТекстЗапросаКурсыВалютДокумента = ВзаиморасчетыПроведение.ПолучитьТаблицуКурсовВалютНаправленийДеятельности(ТекстЗапросаКурсыВалютДокумента, Истина);
	...
	ТекстВозврат = "
		...
		|	
		|	Таблица.ВалютаВзаиморасчетов                                             КАК ВалютаВзаиморасчетов,
		|	ВЫБОР КОГДА Документ.ПроведеноБанком 
		...
		|	КОНЕЦ                                                                    КАК Сумма,
		|	ВЫБОР 
		|		КОГДА НЕ Документ.ПроведеноБанком
		|				И НЕ (Документ.ОплатаПоЗаявкам 
		|					И &ИспользоватьЗаявкиНаРасходованиеДенежныхСредств
		|					И Таблица.ЗаявкаНаРасходованиеДенежныхСредствКонтролироватьОплатуПоОбъектамРасчетов)
		|			ТОГДА Таблица.СуммаВзаиморасчетов 
		|		ИНАЧЕ 0 
	...
	#Область ТекстУменьшитьОплачиваетсяКлиенту
	
	ТекстУменьшитьОплачиваетсяКлиенту = "";
	...
	ТекстУменьшитьОплачиваетсяКлиенту = "
		...
		|			ПО Документ.Ссылка = Таблица.Ссылка
		|ГДЕ
		|	Документ.ПроведеноБанком
		|	И Документ.ОплатаПоЗаявкам 
		|	И &ИспользоватьЗаявкиНаРасходованиеДенежныхСредств 
		|	И Таблица.ЗаявкаНаРасходованиеДенежныхСредствКонтролироватьОплатуПоОбъектамРасчетов
		|";
	...
	ВзаиморасчетыПроведениеКлиенты.ПроведениеВозвратаОплатыКлиенту(Запрос, ТекстыЗапроса, Регистры, ТекстВозврат,
		ТекстПереносЗадолженности, ТекстУменьшитьОплачиваетсяКлиенту);
	...
	ТекстОплата = "
		...
		ВзаиморасчетыПроведениеПоставщики.ПроведениеОплатыПоставщику(Запрос, ТекстыЗапроса, Регистры, ТекстОплата);
...
Функция СформироватьРеквизитыПлатежа(МассивОбъектов, ОбъектыПечати)
	...
	ПоддержкаПлатежейРФ = ПолучитьФункциональнуюОпцию("ПоддержкаПлатежейРФ");
	Запрос.УстановитьПараметр("ПоддержкаПлатежейРФ", ПоддержкаПлатежейРФ);
...
//
// Параметры:
//	ПроверяемыйДокумент - ДокументСсылка.СписаниеБезналичныхДенежныхСредств, ДокументОбъект.СписаниеБезналичныхДенежныхСредств - 
//	                      Документ, для которого необходимо найти зеркальный документ ПоступлениеБДС.
//
// Возвращаемое значение:
...
//
Функция НайтиЗеркальныйПлатеж(ПроверяемыйДокумент) Экспорт
	
	ЗеркальныйПлатеж = Неопределено;
	
	Если ОбщегоНазначения.ЗначениеСсылочногоТипа(ПроверяемыйДокумент) Тогда
		...
		Запрос.УстановитьПараметр("ДокументСсылка", ПроверяемыйДокумент);
	...
	Иначе
		...
		Запрос.УстановитьПараметр("БанковскийСчет",				ПроверяемыйДокумент.БанковскийСчетПолучатель);
		Запрос.УстановитьПараметр("Валюта",						ПроверяемыйДокумент.Валюта);
		Запрос.УстановитьПараметр("ДатаВходящегоДокумента",		ПроверяемыйДокумент.ДатаВходящегоДокумента);
		Запрос.УстановитьПараметр("НомерВходящегоДокумента",	ПроверяемыйДокумент.НомерВходящегоДокумента);
		Запрос.УстановитьПараметр("БанковскийСчетОтправитель",	ПроверяемыйДокумент.БанковскийСчет);
		Запрос.УстановитьПараметр("СуммаДокумента",				ПроверяемыйДокумент.СуммаДокумента);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "11.5.27.33";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("daade3ef-05b0-4e5d-aa91-0015a85601fa");
	...
	ТекстОбработчика = НСтр("ru = 'Заполнение реквизитов ""ИНН плательщика"", ""КПП плательщика"" и ""Текст плательщика"" в документе ""Списание безналичных денежных средств""'");
	...
	Приоритет.Процедура = "Справочники.Организации.ОбработатьДанныеДляПереходаНаНовуюВерсию";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|			ИЛИ СписаниеБезналичныхДенежныхСредств.КПППлательщика = """"
	|			ИЛИ (ВЫРАЗИТЬ(СписаниеБезналичныхДенежныхСредств.ТекстПлательщика КАК СТРОКА(1024))) = """")
	|	И СписаниеБезналичныхДенежныхСредств.Дата >= ДОБАВИТЬКДАТЕ(&НачалоМесяца, МЕСЯЦ, -1)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ПустаяСтрока(Выборка.ИННПлательщика)
			И Не ПустаяСтрока(Выборка.ИНН) Тогда
			СписокДокументов.Добавить(Выборка.Ссылка);
		ИначеЕсли ПустаяСтрока(Выборка.КПППлательщика)
			И Не ПустаяСтрока(Выборка.КПП) Тогда
			СписокДокументов.Добавить(Выборка.Ссылка);
		ИначеЕсли ПустаяСтрока(Выборка.ТекстПлательщика)
			И Не ПустаяСтрока(Выборка.ТекстНаименования) Тогда
			СписокДокументов.Добавить(Выборка.Ссылка);
		КонецЕсли;
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Не удалось заполнить реквизиты документа ""Списание безналичных денежных средств"" по обработчику'"));
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ДокументОбъект = ОбновлениеИнформационнойБазыУТ.ПроверитьПолучитьОбъект(
				Выборка.Ссылка, Выборка.ВерсияДанных, Параметры.Очередь); // ДокументОбъект
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если ПустаяСтрока(ДокументОбъект.ИННПлательщика)
					И Не ПустаяСтрока(Выборка.ИНН) Тогда
					ДокументОбъект.ИННПлательщика = Выборка.ИНН;
				КонецЕсли;
				...
				Если ПустаяСтрока(ДокументОбъект.КПППлательщика)
					...
					Если ДействуетПриказ58н
						...
						ДокументОбъект.КПППлательщика = "0";
					Иначе
						ДокументОбъект.КПППлательщика = Выборка.КПП;
					КонецЕсли;
				...
				Если ПустаяСтрока(ДокументОбъект.ТекстПлательщика)
					И Не ПустаяСтрока(Выборка.ТекстНаименования) Тогда
					ДокументОбъект.ТекстПлательщика = Выборка.ТекстНаименования;
				КонецЕсли;
...
Функция ТекстЗапросаРеквизитовПлательщикаСГруппировкойПоДокументам()
	...
	Возврат 
		...
		|	ДанныеДокумента.Дата КАК Дата,
		|	ДанныеДокумента.Организация КАК Организация,
		|	ЕСТЬNULL(ДанныеДокумента.ИННПлательщика, """") КАК ИННПлательщика,
		|	ЕСТЬNULL(ДанныеДокумента.КПППлательщика, """") КАК КПППлательщика,
		|	ЕСТЬNULL(ДанныеДокумента.ТекстПлательщика, """") КАК ТекстПлательщика,
		|	ЕСТЬNULL(ДанныеДокумента.ВерсияДанных, """") КАК ВерсияДанных
		...
		|	ИсторияЗначенийРеквизитов.Документ КАК Документ,
		|	МАКСИМУМ(ВЫБОР
		|			КОГДА ИсторияЗначенийРеквизитов.Признак = 0
		|				ТОГДА ИсторияЗначенийРеквизитов.ЗначениеРеквизита
		|			ИНАЧЕ """"
		|		КОНЕЦ) КАК Наименование,
		|	МАКСИМУМ(ВЫБОР
		|			КОГДА ИсторияЗначенийРеквизитов.Признак = 1
		|				ТОГДА ИсторияЗначенийРеквизитов.ЗначениеРеквизита
		|			ИНАЧЕ """"
		|		КОНЕЦ) КАК КПП,
		|	МАКСИМУМ(ВЫБОР
		|			КОГДА ИсторияЗначенийРеквизитов.Признак = 2
		|				ТОГДА ИсторияЗначенийРеквизитов.ЗначениеРеквизита
		|			ИНАЧЕ """"
		|		КОНЕЦ) КАК ИНН
		|ПОМЕСТИТЬ НайденныеЗначенияПоДокументу
		|ИЗ
		|	ИсторияЗначенийРеквизитов КАК ИсторияЗначенийРеквизитов
		...
		|	ИсходныеДанные.ВерсияДанных КАК ВерсияДанных,
		|	ИсходныеДанные.ИННПлательщика КАК ИННПлательщика,
		|	ВЫБОР
		|		КОГДА ИсходныеДанные.ПеречислениеВБюджет
		|				И ЕСТЬNULL(НайденныеЗначенияПоДокументу.ИНН, """") = """"
		|			ТОГДА ""0""
		|		КОГДА ПОДСТРОКА(ЕСТЬNULL(НайденныеЗначенияПоДокументу.ИНН, """"), 1, 2) = ""00""
		|			ТОГДА ""0""
		|		ИНАЧЕ НайденныеЗначенияПоДокументу.ИНН
		|	КОНЕЦ КАК ИНН,
		|	ИсходныеДанные.КПППлательщика КАК КПППлательщика,
		|	ВЫБОР
		|		КОГДА ИсходныеДанные.ПеречислениеВБюджет
		|				И ЕСТЬNULL(НайденныеЗначенияПоДокументу.КПП, """") = """"
		|			ТОГДА ""0""
		|		КОГДА ПОДСТРОКА(ЕСТЬNULL(НайденныеЗначенияПоДокументу.КПП, """"), 1, 2) = ""00""
		|			ТОГДА ""0""
		|		ИНАЧЕ НайденныеЗначенияПоДокументу.КПП
		|	КОНЕЦ КАК КПП,
		|	ИсходныеДанные.ТекстПлательщика КАК ТекстПлательщика,
		|	ЕСТЬNULL(НайденныеЗначенияПоДокументу.Наименование, """") КАК ТекстНаименования
		|ИЗ
		|	ИсходныеДанные КАК ИсходныеДанные
		|		ЛЕВОЕ СОЕДИНЕНИЕ НайденныеЗначенияПоДокументу КАК НайденныеЗначенияПоДокументу
		|		ПО ИсходныеДанные.Ссылка = НайденныеЗначенияПоДокументу.Документ";
