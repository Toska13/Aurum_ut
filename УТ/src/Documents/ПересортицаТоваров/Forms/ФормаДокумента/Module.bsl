...
Процедура ОбработатьУказаниеНомераГТДСервер(ИдентификаторСтроки,
	...
	Если ДанныеПодбора.ОстаточныйНомерГТД <> Неопределено Тогда
		...
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ТекущиеДанные, Действия, Неопределено);
...
Функция ОписаниеСтрокиТабличнойЧасти(ТекущаяСтрока = Неопределено)
	...
	Ключи = "Номенклатура, Характеристика, Назначение, Серия, СтатусУказанияСерий, НоменклатураОприходование,
		...
		|ЕдиницаИзмеренияТНВЭДСписание, КонтролируемаяПартия, ДатаПодтвержденияКачества, ПодлежитРазблокировке";
