...
Функция ПолучитьДокументыПоОтбору(ОтборИзСписка) Экспорт
	...
	|	&НеСопоставлена + (ВЫРАЗИТЬ(СтрокаДатаДокумента.РаспознанныйТекст КАК СТРОКА(1024))) КАК ДатаДокументаРаспознано,
	...
	|	&НеСопоставлена + (ВЫРАЗИТЬ(СтрокаОрганизация.РаспознанныйТекст КАК СТРОКА(1024))) КАК ОрганизацияРаспознано,
	...
	|				ИЛИ ОсновнаяТаблица.Направление = ЗНАЧЕНИЕ(Перечисление.ТипыДокументовРаспознаваниеДокументов.КассовыйЧек)
	|				ИЛИ ОсновнаяТаблица.Направление = ЗНАЧЕНИЕ(Перечисление.ТипыДокументовРаспознаваниеДокументов.НеопределенныйДокумент)
	|			ТОГДА """"
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ЕСТЬNULL(СтрокаВидДокумента.Значение, """") = """"
	|					ТОГДА ВЫБОР
	|							КОГДА ОсновнаяТаблица.Направление = ЗНАЧЕНИЕ(Перечисление.НаправленияРаспознанногоДокумента.Исходящий)
	|								ТОГДА ""РТИУ""
	|							ИНАЧЕ ""ПТИУ""
	|						КОНЕЦ
	|				ИНАЧЕ СтрокаВидДокумента.Значение
	|			КОНЕЦ
...
Процедура ЗаписатьИзмененияРеквизитов(ДанныеИзменений, АдресРезультата = Неопределено) Экспорт
	...
	Для Каждого СтрокаИзменений Из ДанныеИзменений Цикл
		...
		ЭлементБлокировки.УстановитьЗначение("Ссылка", СтрокаИзменений.Ссылка);
		...
		Исключение
			...
			ИмяСобытия = НСтр("ru = 'Групповая обработка распознанных документов. Изменение основных реквизитов'");
			ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка, Метаданные.Документы.РаспознанныйДокумент,
				СтрокаИзменений.Ссылка, ТекстОшибки);
...
Функция СоздатьРаспознанныйДокументСОшибкой(ДанныеСервиса, ДанныеВыборки, РезультатыОбработки, ОписаниеОшибки = Неопределено, ДатаСоздания = Неопределено, ДанныеСтраниц = Неопределено) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			РаспознаваниеДокументов.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Ошибка, , ,
			НСтр("ru = 'Не удалось записать документ по причине:'") + Символы.ПС
				+ ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Неопределено;
	КонецПопытки;
...
Если ПараметрыУказанияСерий.Направление = Перечисления.НаправленияРаспознанногоДокумента.Исходящий Тогда
	...
	|	Товары.ПорядокСтроки КАК НомерСтроки
	|ПОМЕСТИТЬ Товары
	|ИЗ
	|	&Товары КАК Товары
	|;
