...
Процедура ОтображениеСозданныхДокументов(Команда)
	...
	Элементы.ГрупповойВводДокументов.Видимость = Не ВключитьФильтр;
	Элементы.ОбщаяКомандаМобильныеПриложенияРаспознаванияДокументов.Видимость = Не ВключитьФильтр;
...
Процедура ГрупповаяОбработкаДокументов(Команда)
	...
	КлючеваяОперация
		= "РаспознаваниеДокументов.ОткрытиеФормы.Обработка.ГрупповаяОбработкаРаспознаваниеДокументов.ГрупповаяОбработка";
	...
	Если ВыделенныеСтроки.Количество() > 1 Тогда
		Для Каждого СтрокаСписка Из ВыделенныеСтроки Цикл
			ТекущаяСтрока = Элементы.Список.ДанныеСтроки(СтрокаСписка);
			РаспознанныеДокументы.Добавить(ТекущаяСтрока.Ссылка);
			ИдентификаторыДокументов.Добавить(ТекущаяСтрока.Идентификатор);
		КонецЦикла;
	КонецЕсли;
	...
	КомментарийЗамера = НовыйКомментарийЗамера(ОтборИзСписка, ИдентификаторыДокументов);
...
Процедура Подключаемый_ОбновитьСписок()
	...
	Элементы.СписокФильтрСтатусОбработан.Заголовок = СтрШаблон(НСтр("ru = 'Созданные: %1'"),
		СостоянияПоСтатусам.ДокументовОбработано);
...
&НаКлиенте
Функция НовыйКомментарийЗамера(ОтборИзСписка, ИдентификаторыДокументов)
	
	КомментарийЗамера = Новый Структура;
	
	Статусы = Новый Массив;
	Если Элементы.СписокФильтрСтатусНовые.Пометка Тогда
		Статусы.Добавить("Новый");
	КонецЕсли;
	Если Элементы.СписокФильтрСтатусОбработан.Пометка Тогда
		Статусы.Добавить("Обработан");
	КонецЕсли;
	Если Элементы.СписокФильтрСтатусСОшибкой.Пометка Тогда
		Статусы.Добавить("Ошибка");
	КонецЕсли;
	Если Элементы.СписокФильтрСтатусУдаленные.Пометка Тогда
		Статусы.Добавить("Удален");
	КонецЕсли;
	КомментарийЗамера.Вставить("Статусы", Статусы);
		
	КомментарийЗамера.Вставить("РаспознанныеДокументы", ИдентификаторыДокументов);
	
	КомментарийЗамера.Вставить("ТекущаяДата", Строка(ТекущаяДата));
	КомментарийЗамера.Вставить("ТекущаяДатаПериод", ?(ТекущаяДатаПериод = Неопределено, "", ТекущаяДатаПериод));
	
	Возврат КомментарийЗамера;

КонецФункции // НовыйКомментарийЗамера()
