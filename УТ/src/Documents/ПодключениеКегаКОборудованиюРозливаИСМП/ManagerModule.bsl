...
Функция ПодключениеКОборудованиюРозливаJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	ВидыПродукции       = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииПоЭлементуКонструктора(
		ПредопределенноеЗначение(
		"Справочник.ЭлементыКонструктораВидовПродукцииИС.ПоддерживаетсяПодключениеКОборудованиюРозлива"));
	ТоварнаяГруппа      = КонструкторИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппаПоВидуПродукции(ВидыПродукции);
...
Функция ДанныеЗаполненияПоСтатистике(Организация, ПоТекущемуПользователю = Истина, Склад = Неопределено) Экспорт
	...
	|	И (ПодключениеКегаКОборудованиюРозливаИСМП.Склад = &Склад Или &БезУчетаСклада)
	...
	Запрос.УстановитьПараметр("БезУчетаСклада",         Не ЗначениеЗаполнено(Склад));
	...
	Если ДанныеПоследнихДокументов.Количество() = 0 И Не БезУчетаОтветственного Тогда
		...
		Возврат ДанныеЗаполненияПоСтатистике(Организация, Ложь, Склад);
