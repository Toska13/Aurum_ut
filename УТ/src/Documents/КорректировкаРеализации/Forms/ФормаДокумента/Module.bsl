...
&НаКлиенте
Перем ПредыдущиеРеквизитыСтрокиТЧТовары;

&НаКлиенте
Перем ПредыдущиеРеквизитыСтрокиТЧРасхождения;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(Объект, ПараметрыУказанияСерий.Расхождения);
	КонецЕсли;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	УчетПодакцизныхТоваров.ОбновитьЭлементыУчетаПодакцизныхТоваров(ЭтотОбъект, Ложь, ДатаОснования);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "РедактироватьНабор"
		...
		Если Параметр.Свойство("ЗаказКлиента") Тогда
			ПараметрыКомплекта.Вставить("ЗаказКлиента", Параметр.ЗаказКлиента);
		КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	ПредыдущиеРеквизитыСтрокиТЧРасхождения = Новый Структура;
	ПредыдущиеРеквизитыСтрокиТЧРасхождения.Вставить("Склад");

	ПредыдущиеРеквизитыСтрокиТЧТовары = Новый Структура;
	ПредыдущиеРеквизитыСтрокиТЧТовары.Вставить("Склад");
...
Процедура ПредставлениеДокументаОснованияНажатие(Элемент, СтандартнаяОбработка)
	...
	Иначе
		...
		ИначеЕсли Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПереходПраваСобственности") Тогда
			СписокТипов.Добавить(Тип("ДокументСсылка.РеализацияТоваровУслуг"));
		КонецЕсли;
...
Процедура ПредставлениеДокументаОснованияНажатиеЗавершение(ВыбранноеЗначение, Параметры) Экспорт
	...
	Если ВыбранноеЗначение.Значение = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		СписокХозяйственнаяОперация.Добавить(
...
&НаКлиенте
Процедура ОблагаетсяАкцизомПриИзменении(Элемент)
	Возврат; // Не используется в УТ
КонецПроцедуры

&НаКлиенте
Процедура ОсобенностиОтраженияПриИзменении(Элемент)
	ОсобенностиОтраженияПриИзмененииСервер();
КонецПроцедуры

&НаКлиенте
Процедура ПеревестиДокументНаНовыйПорядокОтраженияВУчетеНажатие(Элемент)
	ПеревестиДокументНаНовыйПорядокОтраженияВУчетеСервер();
КонецПроцедуры
...
Процедура ТоварыПриАктивизацииЯчейки(Элемент)
	...
	Если Элемент.ТекущийЭлемент <> Неопределено
		...
		Иначе
			...
			КонецЕсли;
			Если ДобавлятьВернутьВПуть Тогда
				СписокВыбора.Добавить(
					ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.ВернутьВПуть"));
			КонецЕсли;
		КонецЕсли;
...
Процедура ТоварыПередНачаломИзменения(Элемент, Отказ)
	...
	ЗаполнитьЗначенияСвойств(ПредыдущиеРеквизитыСтрокиТЧТовары, Элементы.Товары.ТекущиеДанные);
...
Процедура ТоварыКоличествоУпаковокПриИзменении(Элемент)
	...
	ТекущаяСтрока.Вставить("СуммаНДСРегл",                ТекущиеДанные.СуммаНДСРегл);
	ТекущаяСтрока.Вставить("СуммаНДСУпр",                 ТекущиеДанные.СуммаНДСУпр);
...
Процедура ТоварыЗаказКлиентаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекущаяСтрока <> Неопределено Тогда
		...
		Если Объект.ХозяйственнаяОперация =
			...
			ХозяйственнаяОперацияПараметр = Объект.ХозяйственнаяОперация;
		КонецЕсли;
		...
		ПараметрыОтбора.Вставить("ХозяйственнаяОперация", ХозяйственнаяОперацияПараметр);
...
Процедура ТоварыСерияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьПодборСерий(Элементы.Товары.Имя, Элемент.ТекстРедактирования);
...
Процедура ТоварыСерияПриИзменении(Элемент)
	...
	СерияПриИзменении(Элементы.Товары.Имя);
				...
				Если ДобавлятьВернутьВПуть Тогда
					СписокВыбора.Добавить(
						ПредопределенноеЗначение(
							"Перечисление.ВариантыОтраженияКорректировокРеализаций.ВернутьВПуть"));
				КонецЕсли;
			КонецЕсли;
...
Процедура РасхожденияПередНачаломИзменения(Элемент, Отказ)
	...
	ЗаполнитьЗначенияСвойств(ПредыдущиеРеквизитыСтрокиТЧРасхождения, Элементы.Расхождения.ТекущиеДанные);
...
Процедура РасхожденияСерияПриИзменении(Элемент)
	...
	СерияПриИзменении(Элементы.Расхождения.Имя);
	
КонецПроцедуры

&НаКлиенте
Процедура РасхожденияСерияНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ОткрытьПодборСерий(Элементы.Расхождения.Имя, Элемент.ТекстРедактирования);
...
Процедура РасхожденияСкладПриИзменении(Элемент)
	...
	ТекущиеДанные.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач =
		ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач(ТекущиеДанные.Склад, Объект.Дата);
	
	Если ТекущиеДанные.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач Тогда
		
		Если ТекущиеДанные.ВариантОтражения =
			ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализациюУвеличитьСкладскиеОстатки") Тогда
			
			ТекущиеДанные.ВариантОтражения =
				ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализациюУчестьПриИнвентаризации");
			
		ИначеЕсли ТекущиеДанные.ВариантОтражения =
			ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УвеличитьРеализациюУменьшитьСкладскиеОстатки") Тогда
			
			ТекущиеДанные.ВариантОтражения =
				ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УвеличитьРеализациюУчестьПриИнвентаризации");
	...
	Иначе
		...
		Если ТекущиеДанные.ВариантОтражения =
			ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализациюУчестьПриИнвентаризации") Тогда
			
			ТекущиеДанные.ВариантОтражения =
				ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализациюУвеличитьСкладскиеОстатки");
			
		ИначеЕсли ТекущиеДанные.ВариантОтражения =
			ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УвеличитьРеализациюУчестьПриИнвентаризации") Тогда
			
			ТекущиеДанные.ВариантОтражения =
				ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УвеличитьРеализациюУменьшитьСкладскиеОстатки");
	...
	Если ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5
		И (Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.РеализацияКлиенту")
			Или Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПереходПраваСобственности")) Тогда
		
		СкладУчетаСерий = ?(Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПереходПраваСобственности"),
							Объект.Договор,
							ТекущиеДанные.Склад);
		...
		СтруктураДействий.Вставить("ПроверитьСериюРассчитатьСтатус",
			Новый Структура("Склад, ПараметрыУказанияСерий",
				СкладУчетаСерий,
				ПараметрыУказанияСерий.Расхождения));
...
Процедура РасхожденияВариантОтраженияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5
		...
		ОбработатьВыборВариантаОтраженияРасхождения();
...
&НаКлиенте
Процедура РасхожденияВернутьПриИзменении(Элемент)
	
	ТекущаяСтрока = Элементы.Расхождения.ТекущиеДанные;
	
	Если ТекущаяСтрока.Вернуть Тогда
		ТекущаяСтрока.ВариантОтражения = ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.ВернутьВПуть");
	Иначе
		ТекущаяСтрока.ВариантОтражения = ?(ТекущаяСтрока.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач,
			ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализациюУчестьПриИнвентаризации"),
			ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализациюУвеличитьСкладскиеОстатки"));
	КонецЕсли;
...
&НаКлиенте
Процедура ТоварыРазбитьСтроку(Команда)
	
	РазбитьСтрокуТЧ(Элементы.Товары.Имя);
	
КонецПроцедуры

&НаКлиенте
Процедура РасхожденияРазбитьСтроку(Команда)
	
	РазбитьСтрокуТЧ(Элементы.Расхождения.Имя);
...
&НаКлиенте
Процедура РасхожденияЗаполнитьСкладВВыделенныхСтроках(Команда)
	
	ВыделенныеСтроки = Элементы.Расхождения.ВыделенныеСтроки;
	
	ВозможноЗаполнениеСклада = СкладыКлиент.ПроверитьВозможностьЗаполненияСкладовВТабличнойЧасти(
		Объект,
		Объект.Расхождения,
		НСтр("ru='Расхождения'"),
		ВыделенныеСтроки);
	
	Если ВозможноЗаполнениеСклада Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"РасхожденияЗаполнитьСкладВВыделенныхСтрокахЗавершение",
			ЭтотОбъект,
...
&НаКлиенте
Процедура РасхожденияЗаполнитьСкладВВыделенныхСтрокахЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ВыбранныйСклад) Тогда
		...
		ЗаполненоСтрок = ЗаполнитьСкладВВыделенныхСтрокахНаСервере("Расхождения", ВыделенныеСтроки, ВыбранныйСклад);
...
Процедура ОткрытьПодбор(Команда)
	...
	Иначе
		...
		ПараметрыФормы.Вставить("ПараметрыУказанияСерий", ПараметрыУказанияСерий.Товары);
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	УчетНДСУП.УстановитьУсловноеОформлениеКодаТНВЭДПоНалогообложениюПродажи(ЭтотОбъект, "Расхождения");
	...
	УчетНДСУП.УстановитьУсловноеОформлениеСуммНДСПоНалогообложениюПродажи(ЭтотОбъект,,
		"РасхожденияСуммаНДСУпр");
	УчетНДСУП.УстановитьУсловноеОформлениеСуммНДСПоНалогообложениюПродажи(ЭтотОбъект,,
		"РасхожденияСуммаНДСРегл");
	
	//
	ЦеныПредприятияЗаполнениеСервер.УстановитьУсловноеОформлениеЦенаВключаетНДС(ЭтотОбъект);
	ЦеныПредприятияЗаполнениеСервер.УстановитьУсловноеОформлениеЦенаВключаетНДС(ЭтотОбъект, "РасхожденияСуммаСНДС");
	...
	ПараметрыУсловногоОформления = СкладыСервер.ПараметрыУсловногоОформленияСкладаВТЧПоТипуНоменклатуры();
	ПараметрыУсловногоОформления.Форма = ЭтотОбъект;
	ПараметрыУсловногоОформления.ИмяПоляВводаСклада = Элементы.РасхожденияСклад.Имя;
	ПараметрыУсловногоОформления.ИмяПоляТипНоменклатуры = "Объект.Расхождения.ТипНоменклатуры";
	СкладыСервер.УстановитьУсловноеОформлениеСкладаВТЧПоТипуНоменклатуры(ПараметрыУсловногоОформления);
	
	//
	
	ВариантыОтраженийСОбязательнымСкладом = Документы.КорректировкаРеализации.ВариантыОтраженийСОбязательнымСкладом();
	...
	ОтборЭлемента.ПравоеЗначение = ВариантыОтраженийСОбязательнымСкладом;
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Расхождения.Склад");
	...
	// Для ТЧ Расхождения
	СкладыСервер.УстановитьУсловноеОформлениеПодразделенияВТЧ(ЭтотОбъект, "Расхождения");
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.КорректировкаРеализации2_5");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5") Тогда
		...
		ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.РасхожденияСерия.Имя);
		...
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ХозяйственнаяОперация");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПереходПраваСобственности");
		
		ЗначенияВариантовОтражения = Новый СписокЗначений;
		ЗначенияВариантовОтражения.Добавить(
			ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализациюУчестьПриИнвентаризации"));
		ЗначенияВариантовОтражения.Добавить(
			ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализациюУвеличитьСкладскиеОстатки"));
		ЗначенияВариантовОтражения.Добавить(
		...
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Расхождения.ВариантОтражения");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
		ОтборЭлемента.ПравоеЗначение = ЗначенияВариантовОтражения;
		
		ОтборЭлемента = ГруппаОтбораИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Расхождения.СтатусУказанияСерийНаСкладах");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Больше;
		ОтборЭлемента.ПравоеЗначение = 0;
		
		ОтборЭлемента = ГруппаОтбораИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Расхождения.СтатусУказанияСерийПереданныхТоваров");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = 0;
		...
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ХозяйственнаяОперация");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.РеализацияКлиенту");
		
		ОтборЭлемента = ГруппаОтбораИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Расхождения.ВариантОтражения");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.ВернутьВПуть");
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.РасхожденияВернуть.Имя);
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.РасхожденияСуммаНДСРегл.Имя);
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.РасхожденияСуммаНДСУпр.Имя);
...
Процедура ЗаполнитьСтавкуНДСНаСервере(СтавкаНДС, КэшированныеЗначения)
	...
	Для каждого СтрокаТабличнойЧасти Из ВыделенныеСтроки Цикл
		Если СтрокаТабличнойЧасти.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.МногооборотнаяТара И ВернутьМногооборотнуюТару Тогда
...
Процедура ДатаПриИзмененииСервер()
	...
	КорректироватьНалоговуюБазуПрошлогоПериода = Не КорректировкаРеализацииЛокализация.НеКорректироватьНалоговуюБазуПрошлогоПериода(Объект.Организация, ДатаДокумента);
	...
	Если Объект.КорректировкаРеализации2_5 Тогда
		
		КорректировкаРеализацииЛокализация.ЗаполнитьОтражениеВУчетеПоУмолчанию(ЭтотОбъект);
		ОбновитьОтражениеВУчетеПоРеквизитамДокумента();
		УстановитьСвойстваЭлементовПоУчетнойПолитике();
		ОбновитьЗаголовокОтраженияВУчете();
...
Процедура ОперацияПриИзмененииНаСервере()
	...
	УстановитьВидимостьГруппыПрочихДоходовРасходов();
...
&НаСервере
Процедура ПеревестиДокументНаНовыйПорядокОтраженияВУчетеСервер()
	
	Модифицированность = Истина;
	Объект.КорректировкаРеализации2_5 = Истина;
	
	УстановитьВидимостьГруппыПрочихДоходовРасходов();
	УстановитьВидимостьЭлементовПоКорректировкеРеализации2_5();
	ОбновитьВариантыОтраженияРасхождений2_5();
...
&НаСервере
Процедура ОсобенностиОтраженияПриИзмененииСервер()
	
	ОбновитьРеквизитыДокументаПоОтражениюВУчете();
	ОбновитьЗаголовокОтраженияВУчете();
...
Функция ЗаполнитьСкладВВыделенныхСтрокахНаСервере(ИмяТЧ, Знач МассивВыделенныхСтрок, Склад)
	...
	Если Объект.КорректировкаРеализации2_5 И ИмяТЧ = "Расхождения" Тогда
		ОбновитьВариантыОтраженияРасхождений2_5();
	КонецЕсли;
...
Процедура ЗаполнитьРасхожденияСервер()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
		...
		Если Объект.ВалютаВзаиморасчетов <> ВалютаРегламентированногоУчета Тогда
			МассивВалют.Добавить(НСтр("ru = 'регл.'"));
		КонецЕсли;
		
		Если Объект.ВалютаВзаиморасчетов <> ВалютаУправленческогоУчета Тогда
			МассивВалют.Добавить(НСтр("ru = 'упр.'"));
		КонецЕсли;
		
		Если МассивВалют.Количество() Тогда
			
			ОбщегоНазначения.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Суммы НДС %1 были рассчитаны приблизительно. Необходимо проверить корректность их заполнения.'"),
					СтрСоединить(МассивВалют, " " + НСтр("ru = 'и'") + " ")));
	...
	Объект.Товары.Загрузить(ДокументОбъект.Товары.Выгрузить());
...
Процедура ПриОкончанииРедактированияНабора(АдресВоВременномХранилище)
	...
	СтруктураДействийСИзмененнымиСтроками.Вставить("ПроверитьСериюРассчитатьСтатус",
													Новый Структура("ПараметрыУказанияСерий",
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	// Включенная ФО ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5 подразумевает включенную
	// ФО ИспользоватьДоговорыСКлиентами, и обязательное использование договора в соглашении.
	// Проверяем галки [используются договоры контрагентов] и [возможна реализация без перехода права собственности]
	// в соглашении на случай, если корректировку делаем на основании реализации с хозяйственной операцией Реализация клиенту.
	Если ИспользоватьСоглашенияСКлиентами Тогда
		
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			Объект.Соглашение,
			"ИспользуютсяДоговорыКонтрагентов, ВозможнаРеализацияБезПереходаПраваСобственности");
		
		ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5 =
			ПолучитьФункциональнуюОпцию("ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5")
				И ЗначенияРеквизитов.ИспользуютсяДоговорыКонтрагентов = Истина
				И ЗначенияРеквизитов.ВозможнаРеализацияБезПереходаПраваСобственности = Истина;
	...
	Элементы.РасхожденияСерия.Видимость           = ПараметрыУказанияСерий.Расхождения.ИспользоватьСерииНоменклатуры;
	
	НастроитьОтображениеЭлементовПоИтогамРасхождений();
	
	ПриИзмененииСкладаВТабличнойЧастиСервер();
	Элементы.ТоварыЗаполнитьСкладВВыделенныхСтроках.Доступность = СкладГруппа;
	Элементы.РасхожденияЗаполнитьСкладВВыделенныхСтроках.Доступность = СкладГруппа;
	
	УстановитьПараметрыВыбораТоварыСклад();
	УстановитьПараметрыВыбораНоменклатура();
...
Процедура УстановитьВидимостьЭлементовПоОснованию()
	...
	МассивИменЭлементов = Новый Массив;
	МассивИменЭлементов.Добавить("РасхожденияЗаполнитьСкладВВыделенныхСтроках");
	
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(Элементы, МассивИменЭлементов, "Видимость",
		Элементы.РасхожденияСклад.Видимость);
...
Процедура УстановитьДоступностьЭлементовПоСтатусуСервер()
	...
	МассивЭлементов.Добавить("ОблагаетсяАкцизом");
	...
	МассивЭлементов.Добавить("Расхождения;;РасхожденияВернуть,РасхожденияСтатьяДоходов,РасхожденияАналитикаДоходов");
	...
	МассивЭлементов.Добавить("ТоварыЗаполнитьПоАктуПриемки");
	МассивЭлементов.Добавить("ТоварыЗаполнитьСтавкуНДС");
	МассивЭлементов.Добавить("ТоварыСоставНабора");
	МассивЭлементов.Добавить("РасхожденияРазбитьСтроку");
	МассивЭлементов.Добавить("РасхожденияЗаполнитьРасхождения");
	МассивЭлементов.Добавить("РасхожденияУстановитьВариантыОтражения");
	МассивЭлементов.Добавить("РасхожденияЗаполнитьСкладВВыделенныхСтроках");
	МассивЭлементов.Добавить("КорректировкаЗадолженности");
	МассивЭлементов.Добавить("ЗаполнитьПричинуКорректировкиЗадолженности");
	МассивЭлементов.Добавить("КорректировкаЗадолженностиПодобратьДокументы");
	МассивЭлементов.Добавить("РаспределитьОбщуюСуммуКорректировки");
	МассивЭлементов.Добавить("РассчитатьСуммуКорректировкиПоПроценту");
	МассивЭлементов.Добавить("ИзменитьИсправляемыйДокумент");
...
Процедура ЗаполнитьСписокВыбораОпераций()
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование)
		И ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.РеализацияУслугПрочихАктивов")
		Или Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности
		Или Объект.КорректировкаРеализации2_5 Тогда
...
Функция ПараметрыДляВыбораСтатейИАналитик()
	...
	ПараметрыДляВыбора.КорректировкаРеализации2_5 = Объект.КорректировкаРеализации2_5;
	ПараметрыДляВыбора.ОтразитьВПрибылиУбыткахПрошлыхЛет = Объект.ОтразитьВПрибылиУбыткахПрошлыхЛет;
	ПараметрыДляВыбора.КорректироватьНалоговуюБазуПрошлогоПериода = Объект.КорректироватьНалоговуюБазуПрошлогоПериода;
...
Процедура НастроитьОтображениеЭлементовПоИтогамРасхождений()
	...
	УчетПодакцизныхТоваров.ОбновитьЭлементыУчетаПодакцизныхТоваров(ЭтотОбъект, , ДатаОснования);
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	НаборыСервер.ЗаполнитьСлужебныеРеквизиты(ЭтотОбъект, "Товары");
...
Процедура УстановитьВидимостьПоОперации()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
		...
		Элементы.Склад.ВыборГруппИЭлементов = ГруппыИЭлементы.ГруппыИЭлементы;
	...
	Элементы.РасхожденияРазбитьСтроку.Видимость = Элементы.РасхожденияСерия.Видимость
													И ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5
													И (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту
														Или Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности);
	Элементы.РасхожденияКонтекстноеМенюРазбитьСтроку.Видимость = Элементы.РасхожденияРазбитьСтроку.Видимость;
	Элементы.РасхожденияЗаполнитьСкладВВыделенныхСтроках.Видимость =
...
Процедура УстановитьВариантыОтражения(ИмяТЧ)
	...
	ДобавлятьВернутьВПуть =
		...
		И (Объект.ХозяйственнаяОперация =
			...
			Или Объект.ХозяйственнаяОперация = 
				ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПереходПраваСобственности"));
	ДобавлятьСписатьНаРасходы        = Не Объект.КорректировкаРеализации2_5;
	ДобавлятьОтразитьНаПрочихДоходах = Не Объект.КорректировкаРеализации2_5;
	...
	Если ЕстьУвеличениеКоличестваПоОрдерномуСкладу
		...
		Если ЕстьУвеличениеКоличестваПоОрдерномуСкладу Тогда
			...
			Если ДобавлятьОтразитьНаПрочихДоходах Тогда
		...
		Если ЕстьУменьшениеКоличестваПоОрдерномуСкладу Тогда
			СписокВариантовОтражения = Новый СписокЗначений;
			Если ДобавлятьСписатьНаРасходы Тогда
			...
			Если ДобавлятьВернутьВПуть Тогда
				СписокВариантовОтражения.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.ВернутьВПуть"));
			КонецЕсли;
...
Процедура УстановитьВариантыОтраженияЗавершение(ДополнительныеПараметры)
	...
	Если ЗначениеЗаполнено(ДополнительныеПараметры.ВариантОтраженияУвеличениеКоличестваПоОрдерномуСкладу)
		...
		Для Каждого СтрокаТЧ Из Объект[ДополнительныеПараметры.ИмяТЧ] Цикл
			...
			ИначеЕсли (СтрокаТЧ.Количество < 0 Или Объект.ВидКорректировки = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратНедопоставленногоТовара")) Тогда	
				Если СтрокаТЧ.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач И ЗначениеЗаполнено(ДополнительныеПараметры.ВариантОтраженияУменьшениеКоличестваПоОрдерномуСкладу) Тогда
					...
					СтрокаТЧ.Вернуть = СтрокаТЧ.ВариантОтражения = ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.ВернутьВПуть");
				ИначеЕсли СтрокаТЧ.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Работа")
					И Не СтрокаТЧ.Сумма = 0 Тогда
		...
		УстановитьВариантыОтраженияСервер(ДополнительныеПараметры.ИмяТЧ)
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура УстановитьВариантыОтраженияСервер(ИмяТЧ)
	
	Если ИмяТЧ = Элементы.Расхождения.Имя Тогда
		НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(Объект, ПараметрыУказанияСерий.Расхождения);
	КонецЕсли;
...
&НаСервере
Процедура ОбработатьВыборВариантаОтраженияРасхождения()
...
Процедура СкладПриИзмененииСервер()
	...
	Если СкладГруппа Тогда
		...
		Элементы.РасхожденияГруппаОтправитель.Заголовок = НСтр("ru = 'Отправитель'");
	Иначе
		Элементы.ТоварыГруппаОтправитель.Заголовок      = НСтр("ru = 'Подразделение-отправитель'");
		Элементы.РасхожденияГруппаОтправитель.Заголовок = НСтр("ru = 'Подразделение-отправитель'");
	КонецЕсли;
	...
	Если Не СкладГруппа Тогда
		...
		ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач =
			ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач(Объект.Склад, Объект.Дата);
		
		Для Каждого СтрокаТЧ Из Объект.Расхождения Цикл
			
			Если ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач Тогда
				
				Если СтрокаТЧ.ВариантОтражения =
					ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализациюУвеличитьСкладскиеОстатки") Тогда
					
					СтрокаТЧ.ВариантОтражения =
						ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализациюУчестьПриИнвентаризации");
					
				ИначеЕсли СтрокаТЧ.ВариантОтражения =
					ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УвеличитьРеализациюУменьшитьСкладскиеОстатки") Тогда
...
&НаСервереБезКонтекста
Функция ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач(Склад, НаДату)
	Возврат СкладыСервер.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач(Склад, НаДату);
КонецФункции
...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеПриИзменении(Элемент)
	...
	ИначеЕсли Элемент.Имя = "РасхожденияСклад" Тогда
		РасхожденияСкладПриИзменении(Элемент);
	ИначеЕсли Элемент.Имя = "РасхожденияСерия" Тогда
		РасхожденияСерияПриИзменении(Элемент);
	ИначеЕсли Элемент.Имя = Элементы.ОблагаетсяАкцизом.Имя Тогда
		ОблагаетсяАкцизомПриИзменении(Элемент);
	ИначеЕсли Элемент.Имя = Элементы.КорректировкаЗадолженностиСтавкаНДС.Имя Тогда
		КорректировкаЗадолженностиСтавкаНДСПриИзменении(Элемент);
	ИначеЕсли Элемент.Имя = Элементы.КорректировкаЗадолженностиСуммаКорректировки.Имя Тогда
		КорректировкаЗадолженностиСуммаКорректировкиПриИзменении(Элемент);
	Иначе
...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеКоманды(Команда)
	...
	ИначеЕсли Команда.Имя = "ТоварыРазбитьСтроку" Тогда
		ТоварыРазбитьСтроку(Команда);
	ИначеЕсли Команда.Имя = "ВставитьСтроки" Тогда
		ВставитьСтроки(Команда);
	ИначеЕсли Команда.Имя = "ЗаполнитьПоАктуПриемки" Тогда
		ЗаполнитьПоАктуПриемки(Команда);
	ИначеЕсли Команда.Имя = "ЗаполнитьСтавкуНДС" Тогда
	...
	ИначеЕсли Команда.Имя = "РасхожденияРазбитьСтроку" Тогда
		РасхожденияРазбитьСтроку(Команда);
	...
	ИначеЕсли Команда.Имя = "РасхожденияЗаполнитьСкладВВыделенныхСтроках" Тогда
		РасхожденияЗаполнитьСкладВВыделенныхСтроках(Команда);
	ИначеЕсли Команда.Имя = "ПодобратьДокументы" Тогда
		ПодобратьДокументы(Команда);
	ИначеЕсли Команда.Имя = "РассчитатьСуммуКорректировкиПоПроценту" Тогда
		РассчитатьСуммуКорректировкиПоПроценту(Команда);
	ИначеЕсли Команда.Имя = "РаспределитьОбщуюСуммуКорректировки" Тогда
		РаспределитьОбщуюСуммуКорректировки(Команда);
	ИначеЕсли Команда.Имя = "ЗаполнитьПричинуКорректировкиЗадолженности" Тогда
		ЗаполнитьПричинуКорректировкиЗадолженности(Команда);
	ИначеЕсли Команда.Имя = "ИзменитьИсправляемыйДокумент" Тогда
		ИзменитьИсправляемыйДокумент(Команда);
	Иначе
...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеПередНачаломИзменения(Элемент, Отказ)
	Если Элемент.Имя = "Расхождения" Тогда
		РасхожденияПередНачаломИзменения(Элемент, Отказ);
	ИначеЕсли Элемент.Имя = "Товары" Тогда
...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	ИначеЕсли Элемент.Имя = "КорректировкаЗадолженности" Тогда
		КорректировкаЗадолженностиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр);
	Иначе
		ОбщегоНазначенияУТКлиент.КонтрольНеСогласованныхИзмененийВызватьИсключение(ЭтаФорма, Элемент);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура СерияПриИзменении(ИмяТЧ)
...
Процедура ОткрытьПодборСерий(ИмяТЧ, Текст = "")
	...
	Если НоменклатураКлиент.ДляУказанияСерийНуженСерверныйВызов(ЭтотОбъект, ПараметрыУказанияСерий[ИмяТЧ], Текст) Тогда
...
Процедура РазбитьСтрокуТЧ(ИмяТЧ)
	...
	Если ИмяТЧ = Элементы.Расхождения.Имя Тогда
		...
		Если ТекущиеДанные <> Неопределено Тогда
			ДопустимыеВарианты = Новый Массив;
			
			Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПереходПраваСобственности") Тогда
				ДопустимыеВарианты.Добавить(
					ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализациюУчестьПриИнвентаризации"));
				ДопустимыеВарианты.Добавить(
					ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализациюУвеличитьСкладскиеОстатки"));
				ДопустимыеВарианты.Добавить(
				...
				ИмяПоляБазовыйСтатус	= "СтатусУказанияСерийПереданныхТоваров";
				ИмяПоляКорСтатус		= "СтатусУказанияСерийНаСкладах";
			Иначе
				ДопустимыеВарианты.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.ВернутьВПуть"));
				
				ИмяПоляБазовыйСтатус	= "СтатусУказанияСерийНаСкладах";
				ИмяПоляКорСтатус		= "СтатусУказанияСерийПереданныхТоваров";
			КонецЕсли;
			
			Если Не (ТекущиеДанные[ИмяПоляКорСтатус] > 0
				И ТекущиеДанные[ИмяПоляБазовыйСтатус] = 0
				И ДопустимыеВарианты.Найти(ТекущиеДанные.ВариантОтражения) <> Неопределено) Тогда
				
				ТекстПредупреждения = НСтр("ru='Невозможно разбить строку.
					|Действие допустимо для строк, в которых доступно указание серии.'");
	...
	Оповещение = Новый ОписаниеОповещения("РазбитьСтрокуЗавершение", ЭтотОбъект, ИмяТЧ);
...
Процедура РазбитьСтрокуЗавершение(НоваяСтрока, ИмяТЧ) Экспорт
	...
	Если ИмяТЧ = Элементы.Товары.Имя Тогда
	...
	Если ИмяТЧ = Элементы.Расхождения.Имя Тогда
		ИсходноеКоличество = ТекущаяСтрока.КоличествоУпаковок + НоваяСтрока.КоличествоУпаковок;
		
		Если ИсходноеКоличество <> 0 Тогда
			
			ИменаРеквизитов	= "Сумма, СуммаНДС, СуммаСНДС";
			КоллекцияПолей	= Новый Структура(ИменаРеквизитов);
			
			Для Каждого КлючИЗначение Из КоллекцияПолей Цикл
				
				ТекущаяСтрока[КлючИЗначение.Ключ] =
					ТекущаяСтрока.КоличествоУпаковок
						* ТекущаяСтрока[КлючИЗначение.Ключ]
						/ ИсходноеКоличество;
				
				НоваяСтрока[КлючИЗначение.Ключ] = НоваяСтрока[КлючИЗначение.Ключ] - ТекущаяСтрока[КлючИЗначение.Ключ];
...
&НаСервере
Процедура УстановитьПараметрыВыбораНоменклатура()
	
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
		
		Элементы.ТоварыНоменклатура.ПараметрыВыбора = Новый ФиксированныйМассив(Новый Массив);
		Элементы.РасхожденияНоменклатура.ПараметрыВыбора = Новый ФиксированныйМассив(Новый Массив);
		
		РазрешенныеТипыНоменклатуры = Новый Массив;
		РазрешенныеТипыНоменклатуры.Добавить(Перечисления.ТипыНоменклатуры.Товар);
		
		ОбщегоНазначенияУТКлиентСервер.ДобавитьПараметрВыбора(
			Элементы.ТоварыНоменклатура,
			"Отбор.ТипНоменклатуры",
			РазрешенныеТипыНоменклатуры);
		
		ОбщегоНазначенияУТКлиентСервер.ДобавитьПараметрВыбора(
			Элементы.РасхожденияНоменклатура,
			"Отбор.ТипНоменклатуры",
			РазрешенныеТипыНоменклатуры);
...
Процедура УстановитьОтметкуНезаполненногоСклада()
	...
	АвтоОтметкаНезаполненногоСклада = Ложь;
	ВариантыОтраженийСОбязательнымСкладом = Документы.КорректировкаРеализации.ВариантыОтраженийСОбязательнымСкладом();
	
	Если Объект.ВидКорректировки =
			ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.РеализацияПерепоставленногоТовара")
			Или Объект.ВидКорректировки =
				ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратНедопоставленногоТовара") Тогда
		
		ВариантыОтражения = Объект.Товары.Выгрузить(, "ВариантОтражения");
		ВариантыОтражения.Свернуть("ВариантОтражения");
		
		Для Каждого СтрокаТаблицы Из ВариантыОтражения Цикл
			
			Если ВариантыОтраженийСОбязательнымСкладом.НайтиПоЗначению(СтрокаТаблицы.ВариантОтражения) <>
				Неопределено Тогда
				АвтоОтметкаНезаполненногоСклада = Истина;
				Прервать;
			КонецЕсли;
	...
	Иначе
		...
		ВариантыОтражения = Объект.Расхождения.Выгрузить(, "ВариантОтражения");
		ВариантыОтражения.Свернуть("ВариантОтражения");
	...
	Элементы.Склад.АвтоОтметкаНезаполненного = АвтоОтметкаНезаполненногоСклада;
	Если Не ЗначениеЗаполнено(Объект.Склад) Тогда
		Элементы.Склад.ОтметкаНезаполненного = АвтоОтметкаНезаполненногоСклада;
	КонецЕсли;
...
&НаСервере
Процедура УстановитьВидимостьГруппыПрочихДоходовРасходов()
	
	Элементы.ГруппаПрочиеДоходыРасходы.Видимость =
		Объект.ВидКорректировки <> Перечисления.ХозяйственныеОперации.РеализацияПерепоставленногоТовара
		И Объект.ВидКорректировки <> Перечисления.ХозяйственныеОперации.ВозвратНедопоставленногоТовара
		И Объект.ВидКорректировки <> Перечисления.ХозяйственныеОперации.КорректировкаРеализацииУменьшениеЗадолженностиСводно
		И Объект.ВидКорректировки <> Перечисления.ХозяйственныеОперации.КорректировкаРеализацииУвеличениеЗадолженностиСводно
		И Не Объект.КорректировкаРеализации2_5;
	
КонецПроцедуры

&НаСервере
Процедура УстановитьВидимостьЭлементовПоКорректировкеРеализации2_5()
	
	Элементы.ПеревестиДокументНаНовыйПорядокОтраженияВУчете.Видимость = Не Объект.КорректировкаРеализации2_5;
	Элементы.ОтражениеВУчете.Видимость = Объект.КорректировкаРеализации2_5;
	
	Элементы.РасхожденияВариантОтражения.Видимость = Не Объект.КорректировкаРеализации2_5;
	Элементы.РасхожденияУстановитьВариантыОтражения.Видимость = Не Объект.КорректировкаРеализации2_5;
	
	Элементы.РасхожденияВернуть.Видимость =
		Объект.КорректировкаРеализации2_5
		И ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5
		И Не РеализацияЧерезКомиссионера
		И (Объект.ХозяйственнаяОперация =
			ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.РеализацияКлиенту")
...
&НаСервере
Процедура УстановитьСвойстваЭлементовПоУчетнойПолитике()

	Элементы.ОтразитьВПрибылиУбыткахПрошлыхЛет.Доступность = КорректировкаПрошлогоПериода;
	//++ Локализация
...
&НаСервере
Процедура ОбновитьВариантыОтраженияРасхождений2_5()
	
	ЕстьДоговор = ЗначениеЗаполнено(Объект.Договор);
	ТипыНоменклатурыБезУчетаОрдернойСхемы = Новый Массив;
	ТипыНоменклатурыБезУчетаОрдернойСхемы.Добавить(Перечисления.ТипыНоменклатуры.Услуга);
	ТипыНоменклатурыБезУчетаОрдернойСхемы.Добавить(Перечисления.ТипыНоменклатуры.Работа);
	
	Для Каждого СтрокаРасхождений Из Объект.Расхождения Цикл
		
		Если ЕстьДоговор
			И СтрокаРасхождений.ВариантОтражения = Перечисления.ВариантыОтраженияКорректировокРеализаций.ВернутьВПуть Тогда
			СтрокаРасхождений.Вернуть = Истина;
		Иначе
			Если СтрокаРасхождений.Количество > 0 Тогда
				Если ТипыНоменклатурыБезУчетаОрдернойСхемы.Найти(СтрокаРасхождений.ТипНоменклатуры) <> Неопределено
					Или Ложь Тогда
					СтрокаРасхождений.ВариантОтражения = Перечисления.ВариантыОтраженияКорректировокРеализаций.УвеличитьРеализацию;
				Иначе
					Если СтрокаРасхождений.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач Тогда
						СтрокаРасхождений.ВариантОтражения = Перечисления.ВариантыОтраженияКорректировокРеализаций.УвеличитьРеализациюУчестьПриИнвентаризации;
					Иначе
			...
			ИначеЕсли СтрокаРасхождений.Количество < 0 Тогда
				Если ТипыНоменклатурыБезУчетаОрдернойСхемы.Найти(СтрокаРасхождений.ТипНоменклатуры) <> Неопределено
					Или Ложь Тогда
					СтрокаРасхождений.ВариантОтражения = Перечисления.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализацию;
				Иначе
					Если СтрокаРасхождений.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач Тогда
						СтрокаРасхождений.ВариантОтражения = Перечисления.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализациюУчестьПриИнвентаризации;
					Иначе
				...
				КонецЕсли;
			КонецЕсли;
			СтрокаРасхождений.Вернуть = Ложь;
		КонецЕсли;
...
&НаСервере
Процедура ОбновитьРеквизитыДокументаПоОтражениюВУчете()
	
	Если ПорядокОтражения = "Выручка" И Объект.ОтразитьВПрибылиУбыткахПрошлыхЛет Тогда
		Объект.ОтразитьВПрибылиУбыткахПрошлыхЛет = Ложь;
	ИначеЕсли ПорядокОтражения = "ПрочиеДоходы" И Не Объект.ОтразитьВПрибылиУбыткахПрошлыхЛет Тогда
		Объект.ОтразитьВПрибылиУбыткахПрошлыхЛет = Истина;
	КонецЕсли;
	
	//++ Локализация
	Если ПорядокОтраженияНУ = "ПрочиеДоходы" И Объект.КорректироватьНалоговуюБазуПрошлогоПериода Тогда
		Объект.КорректироватьНалоговуюБазуПрошлогоПериода = Ложь;
	ИначеЕсли ПорядокОтраженияНУ = "Выручка" И Не Объект.КорректироватьНалоговуюБазуПрошлогоПериода Тогда
		Объект.КорректироватьНалоговуюБазуПрошлогоПериода = Истина;
	КонецЕсли;
	//-- Локализация
...
&НаСервере
Процедура ОбновитьОтражениеВУчетеПоРеквизитамДокумента()
	
	Если Объект.ОтразитьВПрибылиУбыткахПрошлыхЛет Тогда
		ПорядокОтражения = "ПрочиеДоходы";
	Иначе
		ПорядокОтражения = "Выручка";
	КонецЕсли;
	
	//++ Локализация
	Если Объект.КорректироватьНалоговуюБазуПрошлогоПериода Тогда
		ПорядокОтраженияНУ = "Выручка";
	Иначе
		ПорядокОтраженияНУ = "ПрочиеДоходы";
	КонецЕсли;
	//-- Локализация
...
&НаСервере
Процедура ОбновитьЗаголовокОтраженияВУчете()
	
	ЗаголовокОтражениеВУчете = НСтр("ru = 'Отражение в учете: %1%2'");
	
	Если Объект.ОтразитьВПрибылиУбыткахПрошлыхЛет Тогда
		ОтражениеБУ = НСтр("ru = 'БУ - прочие доходы'");
	Иначе
		ОтражениеБУ = НСтр("ru = 'БУ - выручка'");
	КонецЕсли;
	...
	Элементы.ОтражениеВУчете.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ЗаголовокОтражениеВУчете,
		ОтражениеБУ,
		ОтражениеНУ);
