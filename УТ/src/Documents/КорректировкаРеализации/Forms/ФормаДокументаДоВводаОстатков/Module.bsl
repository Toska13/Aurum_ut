...
Процедура ОсобенностиОтраженияПриИзменении(Элемент)
	ОсобенностиОтраженияПриИзмененииНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура ПеренестиДокументНаНовыйПорядокОтраженияВУчетеНажатие(Элемент)
	ПеренестиДокументНаНовыйПорядокОтраженияВУчетеНаСервере();
КонецПроцедуры
...
Процедура ТаблицаКорректировкиПриИзменении(Элемент)
	...
	Если ТекущиеДанные <> Неопределено
		...
		УстановитьВариантОтраженияДляСтроки(ТекущиеДанные,
			КорректировкаУслугПрочихАктивов, КорректировкаПрошлогоПериода, Объект.КорректировкаРеализации2_5);
...
Процедура ТаблицаКорректировкиНоменклатураПриИзменении(Элемент)
	...
	ПараметрыЗаполнитьСтавкуНДС.Суффикс = Суффикс;
	ПараметрыЗаполнитьСтавкуНДС.Партнер = Объект.Партнер;
	ПараметрыЗаполнитьСтавкуНДС.Соглашение = Объект.Соглашение;
	ПараметрыЗаполнитьСтавкуНДС.ЗаполнитьВидЦены = Истина;
	СтруктураДействий.Вставить("ЗаполнитьСтавкуНДСПоВидуЦен", ПараметрыЗаполнитьСтавкуНДС);
	СтруктураДействий.Вставить("ЗаполнитьСтавкуНДССуффиксПоВидуЦен", ПараметрыЗаполнитьСтавкуНДС);
...
Процедура ТаблицаКорректировкиКоличествоУпаковокПослеПриИзменении(Элемент)
	...
	ПараметрыЗаполнитьСтавкуНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДСПоВидуЦен(Объект);
	ПараметрыЗаполнитьСтавкуНДС.Суффикс = "После";
	ПараметрыЗаполнитьСтавкуНДС.Партнер = Объект.Партнер;
	ПараметрыЗаполнитьСтавкуНДС.Соглашение = Объект.Соглашение;
	ПараметрыЗаполнитьСтавкуНДС.ЗаполнитьВидЦены = Истина;
	СтруктураДействий.Вставить("ЗаполнитьСтавкуНДССуффиксПоВидуЦен", ПараметрыЗаполнитьСтавкуНДС);
...
Процедура УстановитьУсловноеОформление()
	...
	ПараметрыУсловногоОформления.ИмяПоляВводаСклада = Элементы.ТаблицаКорректировкиСклад.Имя;
	ПараметрыУсловногоОформления.ИмяПоляТипНоменклатуры = "ТаблицаКорректировки.ТипНоменклатуры";
...
Процедура ДатаПриИзмененииСервер()
	...
	Если Объект.КорректировкаРеализации2_5 Тогда
		...
		УстановитьВидимостьЭлементовПоУчетнойПолитике();
...
Функция ПолучитьДоступныВариантыОтраженияДляСтроки(СтрокаТаблицы, КорректировкаУслугПрочихАктивов, КорректировкаПрошлогоПериода, КорректировкаРеализации2_5)
	
	СписокВыбора = Новый Массив();
	
	ДобавлятьСписатьНаРасходы        = Не КорректировкаРеализации2_5;
	ДобавлятьОтразитьНаПрочихДоходах = Не КорректировкаРеализации2_5;
	...
	ИначеЕсли КорректировкаУслугПрочихАктивов Тогда
		...
		Если КорректировкаПрошлогоПериода Тогда
			...
			Если ДанныеПоРасхождению.СуммаСНДСРасхождение < 0 Или ДанныеПоРасхождению.СуммаНДСРасхождение < 0 Тогда
				СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.СписатьНаРасходы"));
			Иначе
				СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.ОтразитьНаПрочихДоходах"));
			КонецЕсли;
	...
	ИначеЕсли (СтрокаТаблицы.ТипНоменклатуры <> ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Товар")
		...
		Если ДанныеПоРасхождению.СуммаСНДСРасхождение < 0 Или ДанныеПоРасхождению.СуммаНДСРасхождение < 0 Тогда
			СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализацию"));
		ИначеЕсли ДанныеПоРасхождению.СуммаСНДСРасхождение > 0 Или ДанныеПоРасхождению.СуммаНДСРасхождение > 0
...
&НаКлиентеНаСервереБезКонтекста
Функция ПолучитьДоступныВариантыОтраженияСписанияДляСтроки(СтрокаТаблицы, КорректировкаУслугПрочихАктивов,
	КорректировкаПрошлогоПериода, КорректировкаРеализации2_5)
	...
	Если Не КорректировкаУслугПрочихАктивов И ДанныеПоРасхождению.РасхождениеСтавкиНДС Тогда
		...
		Если ДанныеПоРасхождению.КоличествоУпаковокРасхождение < 0 Тогда
			...
			ИначеЕсли СтрокаТаблицы.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Работа") Тогда
				СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализацию"));
				Если Не КорректировкаРеализации2_5 Тогда
					СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.СписатьНаРасходы"));
				КонецЕсли;
			Иначе
				Если Не КорректировкаРеализации2_5 Тогда
...
&НаКлиентеНаСервереБезКонтекста
Функция УстановитьВариантОтраженияДляСтроки(СтрокаТаблицы, КорректировкаУслугПрочихАктивов, КорректировкаПрошлогоПериода, КорректировкаРеализации2_5)
	...
	СписокВыбора = ПолучитьДоступныВариантыОтраженияДляСтроки(СтрокаТаблицы,
		КорректировкаУслугПрочихАктивов, КорректировкаПрошлогоПериода, КорректировкаРеализации2_5);
...
&НаСервере
Процедура ПеренестиДокументНаНовыйПорядокОтраженияВУчетеНаСервере()
...
&НаСервере
Процедура ОсобенностиОтраженияПриИзмененииНаСервере()
...
Функция ЗаполнитьЦеныВыделенныхСтрокПоВидуЦенСервер(ВидЦен)
	...
	ПараметрыЗаполнитьСтавкуНДС.Партнер = Объект.Партнер;
	ПараметрыЗаполнитьСтавкуНДС.Соглашение = Объект.Соглашение;
	ПараметрыЗаполнитьСтавкуНДС.ВидЦены = ВидЦен;
	ПараметрыЗаполнитьСтавкуНДС.ЗаполнитьВидЦены = Истина;
	СтруктураДействий.Вставить("ЗаполнитьСтавкуНДСПоВидуЦен", ПараметрыЗаполнитьСтавкуНДС);
	СтруктураДействий.Вставить("ЗаполнитьСтавкуНДССуффиксПоВидуЦен", ПараметрыЗаполнитьСтавкуНДС);
...
Процедура ПриОкончанииРедактированияНабора(АдресВоВременномХранилище)
	...
	Для каждого Строка Из СтрокиНабора Цикл
		УстановитьВариантОтраженияДляСтроки(Строка, КорректировкаУслугПрочихАктивов, КорректировкаПрошлогоПериода, Объект.КорректировкаРеализации2_5);
	КонецЦикла;
...
Процедура УстановитьДоступностьЭлементовПоСтатусуСервер()
	...
	МассивЭлементов.Добавить(Элементы.ОблагаетсяАкцизом.Имя);
	...
	МассивЭлементов.Добавить(Элементы.ТаблицаКорректировкиЗаполнитьДанныеКорректировки.Имя);
	МассивЭлементов.Добавить(Элементы.ТаблицаКорректировкиВставитьСтроки.Имя);
	МассивЭлементов.Добавить(Элементы.ТаблицаКорректировкиСоставНабора.Имя);
	МассивЭлементов.Добавить(Элементы.ИзменитьИсправляемыйДокумент.Имя);
...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеКоманды(Команда)
	...
	ИначеЕсли Команда.Имя = Команды.ЗаполнитьДанныеКорректировки.Имя Тогда
		ЗаполнитьДанныеКорректировки(Команда);
	ИначеЕсли Команда.Имя = Команды.ВставитьСтроки.Имя Тогда
		ВставитьСтроки(Команда);
	ИначеЕсли Команда.Имя = Команды.СоставНабора.Имя Тогда
		СоставНабора(Команда);
	ИначеЕсли Команда.Имя = Команды.ИзменитьИсправляемыйДокумент.Имя Тогда
...
Процедура ПодготовитьЗаполнитьУстановитьВидимостьСерий()
	...
	ПараметрыУказанияСерийДокумента = НоменклатураСервер.ПараметрыУказанияСерий(Объект, Документы.КорректировкаРеализации);
	
	ПараметрыУказанияСерий = ПараметрыУказанияСерийДокумента.Товары;
	ПараметрыУказанияСерий.ИмяТЧТовары = "ТаблицаКорректировки";
	ПараметрыУказанияСерий.ИмяТЧСерии = "ТаблицаКорректировки";
	НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(ЭтотОбъект, ПараметрыУказанияСерий);
...
Процедура ЗаполнитьУсловияПродаж(Знач УсловияПродаж)
	...
	Если Контрагент <> Объект.Контрагент Тогда
		ПараметрыСчета = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаКонтрагента(Объект.Контрагент);
		Объект.БанковскийСчетКонтрагента = ДенежныеСредстваСервер.ПолучитьБанковскийСчетКонтрагентаПоУмолчанию(
			ПараметрыСчета);
	КонецЕсли;
...
Процедура ЗаполнитьУсловияПродажПоСоглашению()
	...
	ПараметрыСчета = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаКонтрагента(Объект.Контрагент);
	Объект.БанковскийСчетКонтрагента = ДенежныеСредстваСервер.ПолучитьБанковскийСчетКонтрагентаПоУмолчанию(
		ПараметрыСчета, Объект.БанковскийСчетКонтрагента);
	
	ПараметрыСчета = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации(Объект.Организация);
	Объект.БанковскийСчетОрганизации = ДенежныеСредстваСервер.ПолучитьБанковскийСчетОрганизацииПоУмолчанию(
		ПараметрыСчета, Объект.БанковскийСчетОрганизации);
...
Процедура УстановитьВидимостьЭлементовПоКорректировкеРеализации2_5()
	...
	Элементы.ПеренестиДокументНаНовыйПорядокОтраженияВУчете.Видимость = Не Объект.КорректировкаРеализации2_5;
	Элементы.ОтражениеВУчете.Видимость = Объект.КорректировкаРеализации2_5;
	Элементы.ГруппаПрочиеДоходыРасходы.Видимость = Не Объект.КорректировкаРеализации2_5;
	
	Элементы.ТаблицаКорректировкиГруппаВариантовОтражения.Видимость = Не Объект.КорректировкаРеализации2_5;
	
КонецПроцедуры

&НаСервере
Процедура УстановитьВидимостьЭлементовПоУчетнойПолитике()
...
Процедура ОбновитьВариантыОтраженияРасхождений2_5()
	...
	Для Каждого ТекСтрока Из ТаблицаКорректировки Цикл
		...
		Если ТекСтрока.Количество < ТекСтрока.КоличествоПосле Тогда
			Если ТипыНоменклатурыБезУчетаОрдернойСхемы.Найти(ТекСтрока.ТипНоменклатуры) <> Неопределено
				Или Ложь Тогда
				ТекСтрока.ВариантОтражения = Перечисления.ВариантыОтраженияКорректировокРеализаций.УвеличитьРеализацию;
			Иначе
				Если ТекСтрока.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач Тогда
					ТекСтрока.ВариантОтражения = Перечисления.ВариантыОтраженияКорректировокРеализаций.УвеличитьРеализациюУчестьПриИнвентаризации;
				Иначе
					ТекСтрока.ВариантОтражения = Перечисления.ВариантыОтраженияКорректировокРеализаций.УвеличитьРеализациюУменьшитьСкладскиеОстатки;
				КонецЕсли;
			КонецЕсли;
		ИначеЕсли ТекСтрока.Количество > ТекСтрока.КоличествоПосле Тогда
			Если ТипыНоменклатурыБезУчетаОрдернойСхемы.Найти(ТекСтрока.ТипНоменклатуры) <> Неопределено
				Или Ложь Тогда
				ТекСтрока.ВариантОтражения = Перечисления.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализацию;
			Иначе
				Если ТекСтрока.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач Тогда
					ТекСтрока.ВариантОтражения = Перечисления.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализациюУчестьПриИнвентаризации;
				Иначе
					ТекСтрока.ВариантОтражения = Перечисления.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализациюУвеличитьСкладскиеОстатки;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
