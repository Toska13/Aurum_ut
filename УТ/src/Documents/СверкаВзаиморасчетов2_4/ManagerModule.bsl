...
Функция РеквизитыДокумента(ДокументСсылка) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СверкаВзаиморасчетов2_4.ГруппировкиРасчеты КАК СверкаВзаиморасчетовИтоговыеЗаписи
...
Функция ОписаниеРасчетногоДокумента(СтрокаРасчетов) Экспорт
	...
	ВходящиеДокументы = Документы.СверкаВзаиморасчетов2_5_11.ВходящиеНакладные();
		...
		Если СтрокаРасчетов.НачальноеСальдо = 0 И ДанныеСумм.ЕстьЗаполненные Тогда
			...
			Если СФДокумента.Количество() > 0 Тогда
				...
				ОписаниеСФ = СтрШаблон("(" + ШаблонДокумента + ")",
										НСтр("ru = 'Счет-фактура'"), СокрЛП(НомерНаПечать), Формат(СФ.Дата, "ДЛФ=D"));
				Если НЕ СтрНачинаетсяС(СокрЛП(ВРег(ДанныеДляПечати.НаименованиеДокумента)), НСтр("ru = 'УПД'")) Тогда
...
Функция ТекстЗапросаРасчетыПоФинансовымИнструментам()
	...
	|		АналитикаУчетаПоПартнерам.Договор.НаправлениеДеятельности.* КАК НаправлениеДеятельности}
	|	) КАК РасчетыОстатки
...
Функция ВыборкаДанныхДокументаНаПечать(МассивОбъектов) Экспорт
	...
	Иначе
		ОтветственныеЛицаСервер.СформироватьВременнуюТаблицуОтветственныхЛицДокументов(Документы.СверкаВзаиморасчетов2_4.ПустаяСсылка(), МенеджерВременныхТаблиц);
	КонецЕсли;
