...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ОбменСГИСЭПД.ЗаполнитьДополнительныеРеквизитыФормы(ЭтотОбъект, ИнформацияПоПрефиксамТитула.ВПрограмме);
...
Функция НавигацияПомощника()
	...
	ЕстьМедосмотрыПосле = Ложь;
	
	Если ЭтотОбъект.ВерсииТитулов <> Неопределено Тогда
		СтрокиМедосмотрПосле = ЭтотОбъект.ВерсииТитулов.НайтиСтроки(
			Новый Структура("Титул", ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул6"))); 
		ЕстьМедосмотрыПосле = СтрокиМедосмотрПосле.Количество() > 0; 
	КонецЕсли;
...
Функция СтруктураШагаМедосмотрПосле(НомерШага)
	...
	Если СтруктураШага.КоличествоВерсий > 1 Тогда
		СтруктураШага.ВысотаДляТекста = 4;
	КонецЕсли;
