...
Функция ПолучитьДанныеПоАкцизам(ДокументСсылка) Экспорт
	...
	|					ИЗ
	|						Документ.ТаможеннаяДекларацияИмпорт.Товары КАК Т
	|					ГДЕ
	...
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСПодакцизнымТоваром.Импорт) КАК ВидОперацииСПодакцизнымТоваром,
	...
	|	ЗНАЧЕНИЕ(Справочник.КатегорииПодакцизныхТоваров.ПустаяСсылка) КАК КатегорияПодакцизногоТовара,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСПодакцизнымТоваром.ПустаяСсылка) КАК ВидОперацииСПодакцизнымТоваром,
	|	0 КАК Количество,
	|	0 КАК КоличествоАкциза,
	|	СУММА(-ТоварыДокумента.СуммаАкциза) КАК СуммаАкцизаРегл,
	|	СУММА(-(ВЫРАЗИТЬ(&КоэффициентПересчетаВВалютуУпр * ТоварыДокумента.СуммаАкциза КАК ЧИСЛО(31, 2)))) КАК СуммаАкцизаУпр,
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеДокумента.ОбъектРасчетов.УникальныйИдентификатор
	|";
...
Функция СуммаДокумента(Объект) Экспорт
	...
	СуммаДокумента = Объект.ТаможенныйСбор + Объект.ТаможенныйШтраф + Объект.Товары.Итог("СуммаПошлины")
		+ Объект.Товары.Итог("СуммаНДС") + Объект.Товары.Итог("СуммаАкциза");
...
Процедура УстановитьПараметрыЗапросаКоэффициентыВалют(Запрос) Экспорт
	...
	Если Запрос.Параметры.Валюта = ВалютаРегламентированногоУчета Тогда
		Запрос.УстановитьПараметр("КоэффициентПересчетаИзРеглВУпр", Коэффициенты.КоэффициентПересчетаВВалютуУпр);
	Иначе
		ВалютаУправленческогоУчета = ЗначениеНастроекПовтИсп.ВалютаУправленческогоУчета();
		ПериодКурса = Запрос.Параметры.Период;
		
		КоэффициентПересчетаИзРеглВУпр = РаботаСКурсамиВалютУТ.ПолучитьКоэффициентПересчетаИзВалютыВВалюту(
			ВалютаРегламентированногоУчета, ВалютаУправленческогоУчета, ПериодКурса, ВалютаРегламентированногоУчета);
		
		Запрос.УстановитьПараметр("КоэффициентПересчетаИзРеглВУпр", КоэффициентПересчетаИзРеглВУпр);
	КонецЕсли;
...
Функция ТекстЗапросаВтТоварыДляСоединенияАналитик(Запрос, ТекстыЗапроса)
	...
	|	ТЧТовары.ВидЗапасов.Организация КАК ВидЗапасовОрганизация,
	|	ТЧТовары.ВидЗапасов.ВладелецТовара КАК ВидЗапасовВладелецТовара,
	|	ТЧТовары.ВидЗапасов.НалогообложениеНДС КАК ВидЗапасовНалогообложениеНДС,
	|	ТЧТовары.ВидЗапасов.ГруппаФинансовогоУчета КАК ВидЗапасовГруппаФинансовогоУчета,
	|	ТЧТовары.ВидЗапасов.ГруппаПродукции КАК ВидЗапасовГруппаПродукции,
	|	ТЧТовары.ВидЗапасов.ВидЦены КАК ВидЗапасовВидЦены,
...
Функция ТекстЗапросаТаблицаТоварыОрганизаций(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ПО ВидыЗапасовСкладские.Организация = ТоварыДокумента.ВидЗапасовОрганизация
	|		И ВидыЗапасовСкладские.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|		И ВидыЗапасовСкладские.ВладелецТовара = ТоварыДокумента.ВидЗапасовВладелецТовара
	|		И ВидыЗапасовСкладские.НалогообложениеНДС = ТоварыДокумента.ВидЗапасовНалогообложениеНДС
	|		И ВидыЗапасовСкладские.ГруппаФинансовогоУчета = ТоварыДокумента.ВидЗапасовГруппаФинансовогоУчета
	|		И ВидыЗапасовСкладские.ГруппаПродукции = ТоварыДокумента.ВидЗапасовГруппаПродукции
	|		И ВидыЗапасовСкладские.ВидЦены = ТоварыДокумента.ВидЗапасовВидЦены
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры)
	...
	ВзаиморасчетыПроведениеПоставщики.ПроведениеЗакупки(Запрос, ТекстыЗапроса, Регистры, ТекстЗакупка, ТекстПланОплаты, ТекстЗачетАванса);
...
Процедура ЗаполнитьПоУмолчанию(Объект) Экспорт
	...
	БанковскийСчетОрганизации = ?(Организация = Объект.Организация, Объект.БанковскийСчетОрганизации, Неопределено);
...
Процедура ЗаполнитьПоУсловиямЗакупок(Объект, УсловияЗакупок) Экспорт
	...
	Если УсловияЗакупок.ИспользуютсяДоговорыКонтрагентов <> Неопределено И УсловияЗакупок.ИспользуютсяДоговорыКонтрагентов Тогда
		...
		Если ЗначениеЗаполнено(Объект.Договор) Тогда
			ОплатаВВалюте = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Договор, "ОплатаВВалюте");
			Если ОплатаВВалюте <> Объект.ОплатаВВалюте Тогда
				Объект.ОплатаВВалюте = ОплатаВВалюте;
				Объект.БанковскийСчетОрганизации = Справочники.БанковскиеСчетаОрганизаций.ПустаяСсылка();
			КонецЕсли;
		КонецЕсли;
	...
	Иначе
		...
		Если Объект.ОплатаВВалюте <> УсловияЗакупок.ОплатаВВалюте Тогда
...
Функция СтруктураЗаполнения() Экспорт
	...
	Результат.Вставить("БанковскийСчетОрганизации", Справочники.БанковскиеСчетаОрганизаций.ПустаяСсылка());
...
Процедура ЗаполнитьПоДанным(Объект, Данные) Экспорт
	...
	Если Данные.Товары<>Неопределено Тогда
		...
		Если УчетПодакцизныхТоваров.ИспользуетсяИмпортПодакцизныхТоваров(Объект.Организация, Объект.Дата) Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьРазделыТаможеннойДекларацииИмпорт") Тогда
			...
			Для каждого СтрокаТовары Из Объект.Товары Цикл
				...
				Раздел.СуммаАкциза = Раздел.СуммаАкциза + СтрокаТовары.СуммаАкциза;
			КонецЦикла;
		Иначе
