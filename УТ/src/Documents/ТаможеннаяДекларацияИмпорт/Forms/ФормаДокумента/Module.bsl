...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	УстановитьИспользованиеПодакцизныхТоваров();
...
Процедура РазделыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	Если НоваяСтрока Тогда
		...
		Элемент.ТекущиеДанные.СуммаАкциза = 0;
	КонецЕсли;
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	Если ИспользоватьПодакцизныеТовары Тогда
		ПараметрыЗаполненияПризнакаПодакцизныйТовар =
			ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияПризнакаПодакцизныйТовар(Объект);
		Действия.Вставить("ЗаполнитьПризнакПодакцизныйТовар", ПараметрыЗаполненияПризнакаПодакцизныйТовар);
		ПараметрыПересчетаСуммыАкциза =
			ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыАкциза(Объект, Истина);
		Действия.Вставить("ПересчитатьСуммуАкциза", ПараметрыПересчетаСуммыАкциза);
	КонецЕсли;
...
Процедура ТоварыКоличествоУпаковокПриИзменении(Элемент)
	...
	Действия = Новый Структура;
	Действия.Вставить("ПересчитатьКоличествоЕдиниц");
	Если ИспользоватьПодакцизныеТовары И ТекущаяСтрока.ПодакцизныйТовар Тогда
		ПараметрыПересчетаСуммыАкциза =
			ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыАкциза(Объект, Истина);
		Действия.Вставить("ПересчитатьСуммуАкциза", ПараметрыПересчетаСуммыАкциза);
	КонецЕсли;
...
Процедура ТоварыНоменклатураЕдиницаИзмеренияПриИзменении(Элемент)
	...
	Если ИспользоватьПодакцизныеТовары И ТекущаяСтрока.ПодакцизныйТовар Тогда
		...
		Действия = Новый Структура;
		ПараметрыПересчетаСуммыАкциза = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыАкциза(
			Объект, Истина);
		Действия.Вставить("ПересчитатьСуммуАкциза", ПараметрыПересчетаСуммыАкциза);
...
Процедура УстановитьСклад(Команда)
	Если Элементы.Товары.ВыделенныеСтроки.Количество()>0 Тогда
		...
		ПараметрыОткрытия.Вставить("ВыборГруппыСкладов", Ложь);
		ОткрытьФорму("Справочник.Склады.ФормаВыбора", ПараметрыОткрытия,,,,, Новый ОписаниеОповещения("УстановитьСкладЗавершение", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
КонецПроцедуры
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.РазделыСуммаАкциза.Имя);
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыСуммаАкциза.Имя);

	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ИспользоватьПодакцизныеТовары");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Ложь;
...
Процедура ПересчетКурсаВалютыСервер()
	...
	Если Действия.Свойство("ПересчитатьКоличествоЕдиниц") Тогда	// При расчете таможенной стоимости не требуется это действие
   ...
   |	ПоступлениеТоваров.Количество";
	...
	Для Каждого Товар Из Объект.Товары Цикл
		...
		Если СтрокиПоступления.Количество() > 0 Тогда
			...
			ОбрабатываемыеСтроки.Добавить(Товар);
...
Процедура КонтрагентПриИзмененииСервер()
	Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ЗаполнитьДоговорПоУмолчанию();
		Если Не Справочники.БанковскиеСчетаКонтрагентов.ПроверитьВладельцаБанковскогоСчета(
...
Процедура РаспределитьНаТоварыСервер(РаспределятьТаможеннуюСтоимость = Истина)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Если ТоварыРаздела.Количество() > 0 Тогда
			...
			Если ИспользоватьПодакцизныеТовары И Раздел.СуммаАкциза <> 0 Тогда
				Для Каждого Товар Из ТоварыРаздела Цикл
					Если ЗначениеЗаполнено(Товар.СуммаАкциза) Тогда
						Коэффициенты[Товар.НомерСтроки-1] = Коэффициенты[Товар.НомерСтроки-1] + Товар.СуммаАкциза;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			Если РаспределятьТаможеннуюСтоимость Тогда
...
Процедура ОбработкаВыбораДобавитьТовары(ВыбранноеЗначение, ДанныеРаздела, УдалятьСтроки, ИмяАдреса = Неопределено, ИмяТабчасти=Неопределено, ЗаполнятьСтоимость = Ложь)
	...
	Если ИспользоватьПодакцизныеТовары Тогда
		ПараметрыПересчетаСуммыАкциза =
			ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыАкциза(Объект, Истина);
		СтруктураДействий.Вставить("ПересчитатьСуммуАкциза", ПараметрыПересчетаСуммыАкциза);
	КонецЕсли;
	...
	Для каждого Товар Из ОбрабатываемыеСтроки Цикл
		ПересчитатьПоля(Товар, "СуммаПошлины, СуммаНДС", ДанныеРаздела, Объект.ИспользоватьРазделы);
	КонецЦикла;
	...
	ПересчитатьСуммыВРазделахНаСервере(ДанныеРаздела);
...
&НаСервере
Процедура АктуализироватьСуммуНДС()
	...
	Если ИспользоватьПодакцизныеТовары Тогда
		...
		СтруктураДействий.Вставить("ЗаполнитьПризнакПодакцизныйТовар", ПараметрыЗаполненияПризнакаПодакцизныйТовар);
		СтруктураДействий.Вставить("ПересчитатьСуммуАкциза", ПараметрыПересчетаСуммыАкциза);
	...
	Если Объект.ИспользоватьРазделы Тогда
		...
		СуммыПоРазделам = Объект.Товары.Выгрузить(, "НомерРаздела, СуммаАкциза");
		СуммыПоРазделам.Свернуть("НомерРаздела","СуммаАкциза");
		СуммыПоРазделам.Индексы.Добавить("НомерРаздела");
		
		Для Каждого Раздел Из Объект.Разделы Цикл
			СтрокаРаздела = СуммыПоРазделам.Найти(Раздел.НомерРаздела, "НомерРаздела");
			Если СтрокаРаздела <> Неопределено Тогда
				Раздел.СуммаАкциза = СтрокаРаздела.СуммаАкциза;
			КонецЕсли;
		КонецЦикла;
...
&НаКлиенте
Процедура ПересчитатьСуммыВРазделе(ПоВсемРазделам = Ложь)
	
	Если Не Объект.ИспользоватьРазделы Или Не ИспользоватьПодакцизныеТовары Тогда
		Возврат;
	КонецЕсли;
	
	Если ПоВсемРазделам Тогда
		Разделы = Объект.Разделы;
	Иначе
		СписокСтрокРазделов = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Элементы.Разделы.ТекущаяСтрока);
		Разделы = РаботаСТабличнымиЧастямиКлиентСервер.ЭлементыКоллекции(Объект.Разделы, СписокСтрокРазделов);
	КонецЕсли;
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Отбор.НомерРаздела = Раздел.НомерРаздела;
		ТоварыРаздела = Объект.Товары.НайтиСтроки(Отбор);
		Для Каждого СтрокаРаздела Из ТоварыРаздела Цикл
			Раздел.СуммаАкциза = Раздел.СуммаАкциза + СтрокаРаздела.СуммаАкциза;
		КонецЦикла;
...
&НаСервере
Процедура ПересчитатьСуммыВРазделахНаСервере(ДанныеРаздела)
	
	Если Не Объект.ИспользоватьРазделы Или Не ИспользоватьПодакцизныеТовары Тогда
		Возврат;
	КонецЕсли;
	
	Отбор = Новый Структура("НомерРаздела", ДанныеРаздела.НомерРаздела);
	Разделы = Объект.Разделы.Выгрузить(Отбор, "НомерСтроки");
	Для Каждого СтрокаРаздела Из Разделы Цикл
		ТоварыРаздела = Объект.Товары.Выгрузить(Отбор, "СуммаАкциза");
		ТоварыРаздела.Свернуть(,"СуммаАкциза");
		Раздел = Объект.Разделы[СтрокаРаздела.НомерСтроки-1];
		Раздел.СуммаАкциза = ТоварыРаздела[0].СуммаАкциза;
	КонецЦикла;
...
Процедура ЗаполнитьДополнительныеПризнакиТоваров()
	Действия = Новый Структура;
	УстановитьДействияДополнительныхПризнаковТоваров(Действия);
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Объект.Товары, Действия, Неопределено);
КонецПроцедуры
...
Процедура НастроитьЭлементыФормы()
	...
	Элементы.БанковскийСчетКонтрагента.Заголовок = ?(РасчетыСТаможней,
		НСтр("ru='Счет контрагента таможни'"),
		НСтр("ru='Счет контрагента брокера'"));
	...
	ВалютыДокументаИРеглУчетаОтличаются = Объект.Валюта <> ВалютаРегламентированногоУчета;
	Если ВалютыДокументаИРеглУчетаОтличаются Тогда
		ТекстЗаголовкаСуммаАкциза = СтрШаблон(НСтр("ru = 'Сумма акциза, %1'"), ВалютаРегламентированногоУчета);
	Иначе
		ТекстЗаголовкаСуммаАкциза = НСтр("ru = 'Сумма акциза'");
	КонецЕсли;
	Элементы.РазделыСуммаАкциза.Заголовок = ТекстЗаголовкаСуммаАкциза;
	Элементы.ТоварыСуммаАкциза.Заголовок = ТекстЗаголовкаСуммаАкциза;
	Элементы.ВалютаРегламентированногоУчетаРазделы.Видимость = ВалютыДокументаИРеглУчетаОтличаются;
	Элементы.ВалютаРегламентированногоУчета.Видимость = ВалютыДокументаИРеглУчетаОтличаются;
	Элементы.ИтогиРазделыАкцизРазделы.Видимость = ИспользоватьПодакцизныеТовары;
	Элементы.ИтогиТоварыСуммаАкциза.Видимость = ИспользоватьПодакцизныеТовары;
...
&НаСервере
Процедура УстановитьИспользованиеПодакцизныхТоваров()
	
	ИспользоватьПодакцизныеТовары =
		УчетПодакцизныхТоваров.ИспользуетсяИмпортПодакцизныхТоваров(Объект.Организация, Объект.Дата);
