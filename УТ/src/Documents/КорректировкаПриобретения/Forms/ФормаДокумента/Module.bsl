...
Процедура ТоварыНоменклатураПриИзмененииВопросПользователюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	СтруктураДействий.Вставить("ЗаполнитьПризнакСписатьНаРасходыИПодразделение",
		ДанныеДляЗаполненияПризнакаСписатьНаРасходы(Объект.ХозяйственнаяОперация, ВариантОформленияДоговораЗакупок,
...
&НаКлиенте
Процедура РасхожденияПроконтролироватьКачествоПриИзменении(Элемент)
...
&НаКлиенте
Процедура РасхожденияДатаПодтвержденияКачестваПриИзменении(Элемент)
...
Функция ПараметрыРегистрацииУведомленийОВвозеПрослеживаемыхТоваровСервер(Знач Объект)
	...
	Возврат КорректировкаПриобретенияЛокализация.ПараметрыРегистрацииУведомленийОВвозеПрослеживаемыхТоваров(Объект);
...
Процедура УстановитьУсловноеОформление()
	...
	УчетПрослеживаемыхТоваровЛокализация.УстановитьУсловноеОформлениеНомераГТД(ЭтотОбъект, ПараметрыУсловногоОформления);
	УчетПрослеживаемыхТоваровЛокализация.УстановитьУсловноеОформлениеВДокументеПриобретениеТоваровУслуг(ЭтотОбъект, Истина);
	...
	Поля = КонтрольКачества.Поля(
		Элементы.РасхожденияПроконтролироватьКачество,
		Элементы.РасхожденияДатаПодтвержденияКачества);
	ФлагНедоступен = Новый Массив();
	ТипыНоменклатуры = Перечисления.ТипыНоменклатуры;
	КонтрольКачества.ДобавитьОтбор(ФлагНедоступен, "Объект.Расхождения.СписатьНаРасходы", Истина);
	КонтрольКачества.ДобавитьОтбор(ФлагНедоступен, "Объект.Расхождения.ТипНоменклатуры", ТипыНоменклатуры.Работа);
	КонтрольКачества.ДобавитьОтбор(ФлагНедоступен, "Объект.Расхождения.ТипНоменклатуры", ТипыНоменклатуры.Услуга);
...
Процедура ДатаПриИзмененииСервер()
	...
	Если КонтрольКачества.ОбновитьПараметрыДействий(Документы.КорректировкаПриобретения, Объект, КонтрольКачестваКэш) Тогда
		СтруктураДействий = Новый Структура();
		СтруктураДействий.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", КонтрольКачестваКэш);
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Объект.Расхождения, СтруктураДействий);
	КонецЕсли;
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование)
		И ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
		УстановитьПривилегированныйРежим(Истина); 
		ИменаСвойств = "ВернутьМногооборотнуюТару, ТребуетсяЗалогЗаТару, ХозяйственнаяОперация";
		ЗначенияРеквизитовОбъекта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ДокументОснование, ИменаСвойств);
		...
		ХозяйственнаяОперацияДокументаОснования = ЗначенияРеквизитовОбъекта.ХозяйственнаяОперация;
		УстановитьПривилегированныйРежим(Ложь);
	КонецЕсли;
...
Процедура УстановитьДоступностьЭлементовПоСтатусуСервер()
	...
	МассивЭлементов.Добавить("ТоварыЗаполнитьПоАктуОРасхождениях");
	...
	МассивЭлементов.Добавить("ПодобратьДокументыДляКорректировкиЗадолженности");
	МассивЭлементов.Добавить("РассчитатьСуммуКорректировкиЗадолженностиПоПроценту");
	МассивЭлементов.Добавить("РаспределитьОбщуюСуммуКорректировкиЗадолженностиНаДокументы");
...
Процедура ОбновитьТекстДокументыНаОсновании()
	...
	ПараметрыРегистрацииУведомленийОВвозе = КорректировкаПриобретенияЛокализация.ПараметрыРегистрацииУведомленийОВвозеПрослеживаемыхТоваров(Объект);
...
Процедура ОпределитьГруппыВариантовОтраженияДляВыбора(ТаблицаРасхождений, НастройкиВариантовОтражения)
	...
	Для Каждого СтрокаРасхождений Из ТаблицаРасхождений Цикл
		...
		Если (СтрокаРасхождений.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Услуга")
				...
				Или СтрокаРасхождений.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.ПустаяСсылка"))
...
Процедура ВыбратьИнтерактивноВариантыОтраженияКорректировокРеализацийФрагмент(ДополнительныеПараметры);
	...
	Иначе
		...
		ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачестваСервер();
...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеПриИзменении(Элемент)
	...
	ИначеЕсли Элемент.Имя = "РасхожденияПроконтролироватьКачество" Тогда
		РасхожденияПроконтролироватьКачествоПриИзменении(Элемент);
	ИначеЕсли Элемент.Имя = "РасхожденияДатаПодтвержденияКачества" Тогда
		РасхожденияДатаПодтвержденияКачестваПриИзменении(Элемент);
...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеКоманды(Команда)
	...
	ИначеЕсли Команда.Имя = "ЗаполнитьПоАктуОРасхождениях" Тогда
		ЗаполнитьПоАктуОРасхождениях(Команда);
	...
	ИначеЕсли Команда.Имя = "ПодобратьДокументыДляКорректировкиЗадолженности" Тогда
		ПодобратьДокументыДляКорректировкиЗадолженности(Команда);
	ИначеЕсли Команда.Имя = "РассчитатьСуммуКорректировкиЗадолженностиПоПроценту" Тогда
		РассчитатьСуммуКорректировкиЗадолженностиПоПроценту(Команда);
	ИначеЕсли Команда.Имя = "РаспределитьОбщуюСуммуКорректировкиЗадолженностиНаДокументы" Тогда
		РаспределитьОбщуюСуммуКорректировкиЗадолженностиНаДокументы(Команда);
...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	Если Элемент.Имя = Элементы.КорректировкаЗадолженности.Имя Тогда
		КорректировкаЗадолженностиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Неопределено);
	Иначе
...
&НаКлиентеНаСервереБезКонтекста
Функция ДанныеДляЗаполненияПризнакаСписатьНаРасходы(ХозяйственнаяОперация, ВариантОформленияДоговораЗакупок, ВариантОформленияДокументовПоступленияПоУслугам)
	...
	ЭтоФактуровкаПоУслугамИРаботам = ЭтоФактуровкаПоТоварам И
		(ВариантОформленияДоговораЗакупок = ПредопределенноеЗначение("Перечисление.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваровИУслуг") Или
		ВариантОформленияДокументовПоступленияПоУслугам = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется"));
...
&НаСервере
Процедура ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачестваСервер()
