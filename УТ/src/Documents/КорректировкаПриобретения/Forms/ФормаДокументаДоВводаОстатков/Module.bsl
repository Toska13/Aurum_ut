...
Процедура НалогообложениеНДСПриИзмененииСервер(КэшированныеЗначения)
	...
	ПараметрыЗаполнитьСтавкуНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект, Ложь, ВернутьМногооборотнуюТару);
	ПараметрыЗаполнитьСтавкуНДС.Дата = ДатаОснования;
...
Функция ЗаполнитьСкладВВыделенныхСтрокахНаСервере(Знач МассивВыделенныхСтрок, Склад)
	...
	ЗаполненоСтрок = СкладыСервер.ЗаполнитьСкладыВВыделенныхСтроках(ТаблицаКорректировки, МассивВыделенныхСтрок, Склад,, "СписатьНаРасходы");
...
Процедура УстановитьВидимостьЭлементовПоТипуКорректировкиСервер()
	...
	Элементы.ТаблицаКорректировкиСтатусУказанияСерий.Видимость = ЭтоКорректировкаТоваровИУслуг;
