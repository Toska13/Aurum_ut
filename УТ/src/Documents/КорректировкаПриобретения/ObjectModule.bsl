...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЗначениеЗаполнено(ДокументОснование) Тогда
		...
		Если ТипОснования = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
			...
			РеквизитыОснования = "ВариантПриемкиТоваров, ВариантПриемкиУслуг, ХозяйственнаяОперация";
			
		ИначеЕсли ТипОснования = Тип("ДокументСсылка.АктОРасхожденияхПослеПриемки") Тогда
			
			РеквизитыОснования = "ВариантПриемкиТоваров, ХозяйственнаяОперация";
			
		ИначеЕсли ТипОснования <> Тип("ДокументСсылка.ПервичныйДокумент")
			И ТипОснования <> Тип("Неопределено") Тогда
		...
		Если ЗначениеЗаполнено(РеквизитыОснования) Тогда
			...
			ЗаполнитьЗначенияСвойств(ЭтотОбъект, ЗначенияРеквизитов, РеквизитыОснования);
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКонтрольКачестваТоваров")
			И ТипКорректировки = Перечисления.ТипыКорректировки.ОбычнаяКорректировка
			// При раздельной закупке корректировка приобретения не делает движений по РН Товары на складах, партия не нужна.
			// Кроме того на неордерном складе заполнить ее невозможно, так как партией будет документ "Поступления товаров на склад",
			//	на который нет ссылок в "Корректировке приобретения". 
			И Не ЗакупкиСервер.ЭтоХозяйственнаяОперацияРаздельнойЗакупки(ХозяйственнаяОперация)
			И ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.КорректировкаОтчетаПереработчика
			И Не ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПриобретениеУслугПрочихАктивов") Тогда
		...
		Запрос.Текст =
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	ТабЧасть.ЗаказПоставщику КАК ЗаказПоставщику,
			|	ВЫРАЗИТЬ(ТабЧасть.Склад КАК Справочник.Склады) КАК Склад
			|ПОМЕСТИТЬ ТабЧасть
			|ИЗ
			...
			|			ТОГДА ЕСТЬNULL(Приобретение.ПодотчетноеЛицо, &Партнер)
			|		ИНАЧЕ &Партнер
		...
		Индексы = "ЗаказПоставщику,Склад";
		Запрос.УстановитьПараметр("ТабЧасть", Расхождения.Выгрузить(, Индексы));
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Иначе
		...
		Если (ЭтоПрослеживаемыйДокумент
			...
			ПараметрыПроверки.ХозяйственнаяОперация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "ХозяйственнаяОперация");
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	ПараметрыЗаполненияСтакиНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(ЭтотОбъект);
...
Процедура ПриЗаписи(Отказ)
	...
	// Механизм "Контролируемые партии".
	КонтрольКачества.СоздатьКонтролируемыеПартииВТабличнойЧасти(ЭтотОбъект);
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Расхождения, СтруктураДействий);
...
Процедура ЗаполнитьПоАктуОРасхождениях(Знач СтруктураПараметров, ДанныеЗаполнения)
	...
	|	ДанныеДокумента.ПорядокРасчетов         КАК ПорядокРасчетов,
	|	ДанныеДокумента.ВариантПриемкиТоваров   КАК ВариантПриемкиТоваров,
	|	ДанныеДокумента.Договор.ВариантПриемкиУслуг КАК ВариантПриемкиУслуг,
	|	ДанныеДокумента.ХозяйственнаяОперация   КАК ХозяйственнаяОперацияОснования,
	|	НЕ ДанныеДокумента.Проведен             КАК ЕстьОшибкиПроведен,
	|	ВЫБОР
	...
	ЗаполнитьЗначенияСвойств(ДанныеЗаполнения, Выборка);
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, Выборка, "Организация, Склад, Валюта, ВалютаВзаиморасчетов, ДокументОснование, АктОРасхожденияхПослеПриемкиОснование, ЦенаВключаетНДС, НалогообложениеНДС, ХозяйственнаяОперация, Договор");
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	ИначеЕсли Не ИспользоватьРаздельноеОформлениеЗакупок
		...
		РаспределитьЗапасыНаРасхождения(РасхожденияДокумента, ХозяйственнаяОперацияДокумента);
...
Процедура ЗаполнитьАналитикамиСУчетомСерий()
	...
	ДанныеПоВидамЗапасов = Запрос.ВыполнитьПакет();
	ТаблицаВидыЗапасовРасход = ДанныеПоВидамЗапасов[1].Выгрузить(); 
	ТаблицаВидыЗапасовПриход = ДанныеПоВидамЗапасов[2].Выгрузить();
	ТаблицаВидыЗапасовСумм   = ДанныеПоВидамЗапасов[3].Выгрузить();
	
	Ключ = "Номенклатура, Характеристика, Назначение, НомерГТД, Серия";
	
	Условие = "ПО [КоличествоПоМодулю]";
	НакладныеСервер.РаспределитьКоличествоИЗаполнить(Результат, ТаблицаВидыЗапасовРасход, 
		"КоличествоПоДаннымПриобретения", Ключ, Условие, Ложь, "АналитикаУчетаНоменклатурыПоДаннымПриобретения");
	
	Для Каждого Строка Из ТаблицаВидыЗапасовРасход Цикл
	...
	|////////////////////////////////////////////////////////////////////////////////
	|	ВЫБРАТЬ
	|		ТаблицаВидыЗапасовРасход.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
	|		-ТаблицаВидыЗапасовРасход.Количество КАК Количество
	|	ПОМЕСТИТЬ ОстаткиАналитикПриНедостаче
	|	ИЗ
	|		&ТаблицаВидыЗапасовРасход КАК ТаблицаВидыЗапасовРасход
	|	ГДЕ
	|		ТаблицаВидыЗапасовРасход.Количество < 0	
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////	
	|ВЫБРАТЬ
	...
	|	
	|	ВЫБРАТЬ
	|		ОстаткиАналитикПриНедостаче.АналитикаУчетаНоменклатуры,
	|		ОстаткиАналитикПриНедостаче.Количество,
	|		3 КАК Приоритет
	|	ИЗ
	|		ОстаткиАналитикПриНедостаче КАК ОстаткиАналитикПриНедостаче
	|	) КАК Набор
	|
	|СГРУППИРОВАТЬ ПО
	|	Набор.Приоритет,
	...
	Запрос.УстановитьПараметр("ТаблицаВидыЗапасовРасход", ТаблицаВидыЗапасовРасход);
	...
	Условие = "ПО [Количество]";
	НакладныеСервер.РаспределитьКоличествоИЗаполнить(Результат, ТаблицаВидыЗапасовПриход, 
		"КоличествоПоРегистру", Ключ, Условие, Ложь, "АналитикаУчетаНоменклатурыПоРегистру");
	
	Для Каждого Строка Из ТаблицаВидыЗапасовПриход Цикл
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаВидыЗапасовПриход, ТаблицаВидыЗапасовРасход);
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаВидыЗапасовСумм, ТаблицаВидыЗапасовРасход);
	
	ВидыЗапасов.Загрузить(ТаблицаВидыЗапасовРасход);
	...
	Если ТипЗнч(ДокументОснование) <> Тип("ДокументСсылка.ПриобретениеУслугПрочихАктивов")
		...
		Если ЗначениеЗаполнено(ДокументОснование) Тогда
			...
			Если ОперацииФактуровка.Найти(ОперацияОснования) <> Неопределено
				И (РеквизитыДоговора.ВариантОформленияЗакупок = Перечисления.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваровИУслуг
				Или РеквизитыДоговора.ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется) Тогда
		...
		Для Каждого СтрокаПоиска Из МассивСтрокПоиска Цикл
			...
			Для Каждого СтрокаЗапасов Из НайденныеСтроки Цикл
				...
				Если (СтруктураРесурсов.Количество > 0
					...
					Или (СтруктураРесурсов.Количество < 0
					И СтрокаЗапасов.Количество > 0) Тогда
					Продолжить;
				КонецЕсли;
					
				МодульКоличествоРесурсов = ?(СтруктураРесурсов.Количество > 0, СтруктураРесурсов.Количество, -СтруктураРесурсов.Количество);
				МодульКоличествоЗапасов = ?(СтрокаЗапасов.Количество > 0, СтрокаЗапасов.Количество, -СтрокаЗапасов.Количество);
				Если МодульКоличествоРесурсов < МодульКоличествоЗапасов Тогда
					КоличествоПоСтроке = СтруктураРесурсов.Количество;
				Иначе
					КоличествоПоСтроке = СтрокаЗапасов.Количество;
				КонецЕсли;
...
Процедура РаспределитьЗапасыНаРасхождения(РасхожденияДокумента, ХозяйственнаяОперацияДокумента)
	
	РасхожденияДокумента.Колонки.Добавить("КоличествоДляРаспределения", Новый ОписаниеТипов("Число"));
	
	РасхожденияВидыЗапасов = Расхождения.Выгрузить();
	РасхожденияВидыЗапасов.Колонки.Количество.Имя = "КоличествоДляРаспределения";
	
	РаспределятьГТД = ХозяйственнаяОперацияДокумента = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС
						И УчетПрослеживаемыхТоваровЛокализация.ИспользоватьУчетПрослеживаемыхИмпортныхТоваров(Дата);
	
	ПрослеживаемостьТоваров = Неопределено;
	
	Если РаспределятьГТД Тогда
		ТоварыРасхождений		= РасхожденияВидыЗапасов.ВыгрузитьКолонку("Номенклатура");
		ПрослеживаемостьТоваров	= ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ТоварыРасхождений, "ПрослеживаемыйТовар");
	КонецЕсли;
	
	КлючСГТД	= "Номенклатура, Характеристика, Назначение, Серия, НомерГТД";
	КлючБезГТД	= "Номенклатура, Характеристика, Назначение, Серия";
	
	ОтборСГТД	= Новый Структура(КлючСГТД);
	ОтборБезГТД	= Новый Структура(КлючБезГТД);
	
	Для Каждого СтрокаЗапасов Из РасхожденияВидыЗапасов Цикл
		// Для строк расхождений на увеличение приобретения виды запасов заполняются по умолчанию.
		Если СтрокаЗапасов.КоличествоДляРаспределения > 0 Тогда
			Продолжить;
		КонецЕсли;
		
		ПрослеживаемыйТовар = ?(ПрослеживаемостьТоваров <> Неопределено,
								ПрослеживаемостьТоваров.Получить(СтрокаЗапасов.Номенклатура),
								Ложь);
		
		Отбор = ?(ПрослеживаемыйТовар, ОтборБезГТД, ОтборСГТД);
		ЗаполнитьЗначенияСвойств(Отбор, СтрокаЗапасов);
		
		НайденныеСтроки = РасхожденияДокумента.НайтиСтроки(Отбор);
		...
		Для Каждого СтрокаТовары Из НайденныеСтроки Цикл
			...
			Количество	= СтрокаЗапасов.КоличествоДляРаспределения;
			Превышение	= СтрокаТовары.КоличествоДляРаспределения + Количество - СтрокаТовары.Количество;
			...
			ЗаполняемыеПоляРавны = СтрокаЗапасов.ВидЗапасов = СтрокаТовары.ВидЗапасов;
			
			Если ПрослеживаемыйТовар
				И ЗаполняемыеПоляРавны Тогда
				
				ЗаполняемыеПоляРавны = СтрокаЗапасов.НомерГТД = СтрокаТовары.НомерГТД;
				
			КонецЕсли;
			
			Если Не ЗаполняемыеПоляРавны
				И СтрокаТовары.КоличествоДляРаспределения = 0 Тогда
				
				СтрокаТовары.ВидЗапасов = СтрокаЗапасов.ВидЗапасов;
				
				Если ПрослеживаемыйТовар Тогда
					СтрокаТовары.НомерГТД = СтрокаЗапасов.НомерГТД;
				КонецЕсли;
				
				СтрокаТовары.КоличествоДляРаспределения		= СтрокаТовары.КоличествоДляРаспределения + Количество;
				СтрокаЗапасов.КоличествоДляРаспределения	= СтрокаЗапасов.КоличествоДляРаспределения - Количество;
				
			ИначеЕсли ЗаполняемыеПоляРавны Тогда
				СтрокаТовары.КоличествоДляРаспределения		= СтрокаТовары.КоличествоДляРаспределения + Количество;
				СтрокаЗапасов.КоличествоДляРаспределения	= СтрокаЗапасов.КоличествоДляРаспределения - Количество;
			КонецЕсли;
		КонецЦикла;
		...
		Если НайденныеСтроки.Количество() > 0
			И СтрокаЗапасов.КоличествоДляРаспределения < 0 Тогда
			
			СтрокаТовары = НайденныеСтроки[НайденныеСтроки.ВГраница()];
			СтрокаТовары.КоличествоДляРаспределения = СтрокаТовары.КоличествоДляРаспределения
														+ СтрокаЗапасов.КоличествоДляРаспределения;
			...
			СтрокаЗапасов.КоличествоДляРаспределения = 0;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьРасхождения(ИспользуетсяДокументПоступлениеТоваров) Экспорт
	...
	|	ВЫБОР
	|		КОГДА НовыеДанныеИзТаблицы.Упаковка = НовыеДанныеИзТаблицы.Номенклатура.ЕдиницаИзмерения
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|		ИНАЧЕ НовыеДанныеИзТаблицы.Упаковка
	|	КОНЕЦ КАК Упаковка,
