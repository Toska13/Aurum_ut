...
//     * ИмяРегистра - ТаблицаЗначений - таблица данных для отражения в регистр.
//
...
Функция ПредставлениеОбъектаНаОсновномЯзыке() Экспорт
	...
	СинонимДляЗаписиВИнформационнуюБазу = НСтр(
		"ru = 'Отчет о розничных возвратах'",
		КодОсновногоЯзыка);
...
Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|		КОГДА ТаблицаВидыЗапасов.АналитикаРеализации <> ТаблицаВидыЗапасов.АналитикаУчетаНоменклатуры
...
Функция ТекстЗапросаТаблицаДвиженияДенежныеСредстваКонтрагент(Запрос, ТекстыЗапроса, Регистры)
	...
	Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтСуммаПлатежнымиКартамиВРазрезеСтатейДДС",
		                                            ТекстыЗапроса,
		                                            Запрос.МенеджерВременныхТаблиц) Тогда
		СоздатьВтСуммаПлатежнымиКартамиВРазрезеСтатейДДС(Запрос);
	КонецЕсли;
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВтСуммаПлатежнымиКартами.СтатьяДвиженияДенежныхСредств, ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПоступлениеОплатыОтКлиента)
	|		ИНАЧЕ ВтСуммаПлатежнымиКартами.СтатьяДвиженияДенежныхСредств
	...
	|ИЗ
	|	Документ.ОтчетОРозничныхВозвратах КАК ДанныеДокумента
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтСуммаПлатежнымиКартамиВРазрезеСтатейДДС КАК ВтСуммаПлатежнымиКартами
	|		ПО (ИСТИНА)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтСуммаПлатежнымиКартами КАК ВтСуммаПлатежнымиКартамиКонтроль
	|		ПО (ИСТИНА)
	|ГДЕ
	|	ДанныеДокумента.СуммаДокумента 
	|		- ЕСТЬNULL(ВтСуммаПлатежнымиКартамиКонтроль.Сумма, 0) <> 0
	|	И ДанныеДокумента.Ссылка = &Ссылка
	|	И &ПолноеПроведение
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|
	|	ВЫБОР
	|		КОГДА ТаблицаОплатаПлатежнымиКартами.ВидОплаты = Значение(Перечисление.ТипыПлатежнойСистемыККТ.СистемаБыстрыхПлатежей)
	|			ТОГДА ТаблицаОплатаПлатежнымиКартами.ДоговорПодключения
	|		ИНАЧЕ ТаблицаОплатаПлатежнымиКартами.ЭквайринговыйТерминал.Владелец
	...
	|	ПО	ДанныеДокумента.Ссылка = ТаблицаОплатаПлатежнымиКартами.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтСтатьиДДС КАК СтатьиДДС
...
Функция ТекстЗапросаВтСтатьиДДС(ТекстыЗапроса)
	...
	|	ТаблицаПлатежей.НомерСтроки КАК НомерСтроки,
	|	ВЫБОР
	|		КОГДА НЕ ЕСТЬNULL(СведенияОДоговоре.СтатьяДвиженияДенежныхСредствВозврат,
	|						  ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка))
	|							= ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка)
	|	
	|			ТОГДА СведенияОДоговоре.СтатьяДвиженияДенежныхСредствВозврат
	|
	|		КОГДА НЕ ЕСТЬNULL(СведенияОДоговореНаТерминал.СтатьяДвиженияДенежныхСредствВозврат,
	|						  ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка))
	|							= ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка)
	
	|			ТОГДА СведенияОДоговореНаТерминал.СтатьяДвиженияДенежныхСредствВозврат
	|
...
Процедура СоздатьВтСуммаПлатежнымиКартамиВРазрезеСтатейДДС(Запрос)
	...
	|ПОМЕСТИТЬ ВтСуммаПлатежнымиКартамиВРазрезеСтатейДДС
	|ИЗ
	|	Документ.ОтчетОРозничныхВозвратах.ОплатаПлатежнымиКартами КАК ТабличнаяЧасть
...
Процедура СоздатьВтСуммыНДСВРазрезеСтатейДДС(Запрос)
	...
	|		0 КАК ОбщаяСумма
	|	ИЗ
	|		Документ.ОтчетОРозничныхВозвратах.ОплатаПлатежнымиКартами КАК ОплатаПлатежнымиКартами
	|	ГДЕ
	...
	|	ИЗ
	|		Документ.ОтчетОРозничныхВозвратах.Товары КАК ОтчетОРозничныхПродажахТовары
	|	ГДЕ
	...
	|	ИЗ
	|		Документ.ОтчетОРозничныхВозвратах КАК ДанныеДокумента
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетОРозничныхВозвратах.Товары КАК ОтчетОРозничныхПродажахТовары
	...
	МассивТекстов.Добавить(ТекстЗапросаСтавкиНДС);
	...
	СтатьиДДС =
		Запрос.МенеджерВременныхТаблиц.Таблицы.Найти("ВтСуммаПлатежнымиКартамиВРазрезеСтатейДДС").ПолучитьДанные().Выгрузить();
	...
	Для Каждого СтрокаСуммНДС Из СуммыНДС Цикл
		...
		Если ОписаниеКоэффициентаБезналичныхДС.СуммаБезналичных <> 0 Тогда
			...
			Для Каждого СтрокаСтатейДДС Из СтатьиДДС Цикл
				...
				Если РезультатРаспределения = 0 Тогда
					Продолжить;
				КонецЕсли;
				...
				СтрокаБезналичных.Сумма = РезультатРаспределения;
				РаспределяемаяСуммаНДС = РаспределяемаяСуммаНДС - РезультатРаспределения;
				
			КонецЦикла;
		
		КонецЕсли;
		
		Если РаспределяемаяСуммаНДС <> 0 Тогда
		
			Если ОписаниеКоэффициентаБезналичныхДС.КоэффициентОплатыБезналичными = 1 Тогда
				
				СтрокаБезналичных = ТаблицаДляРаспределения.Добавить();
				СтрокаБезналичных.СтавкаНДС   = СтрокаСуммНДС.СтавкаНДС;
				СтрокаБезналичных.Сумма       = РаспределяемаяСуммаНДС;
				СтрокаБезналичных.СтатьяДвиженияДенежныхСредств = СтрокаСтатейДДС.СтатьяДвиженияДенежныхСредств;
				СтрокаБезналичных.ТипДенежныхСредств =
					ПредопределенноеЗначение("Перечисление.ТипыДенежныхСредств.ДенежныеСредстваУЭквайера");
			...
			Иначе
				...
				СтрокаНаличных.Сумма     = РаспределяемаяСуммаНДС;
	...
	ТаблицаДляРаспределения.Свернуть("СтавкаНДС,СтатьяДвиженияДенежныхСредств,ТипДенежныхСредств", "Сумма");
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|	И (НЕ (ЕСТЬNULL(ТабЧасть.Ссылка.КассаККМ.ТипКассы,
		|		НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ТипыКассККМ.ФискальныйРегистратор)
		|	И ТабЧасть.Ссылка.КассоваяСмена <> ЗНАЧЕНИЕ(Документ.КассоваяСмена.ПустаяСсылка))
		|	ИЛИ (ЕСТЬNULL(ТабЧасть.Ссылка.КассоваяСмена.Статус,
		|		НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.СтатусыКассовойСмены.Закрыта)
		|	И ЕСТЬNULL(ТабЧасть.Ссылка.КассоваяСмена.СтатусРегламентныхОпераций,
		|		НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.СтатусыРегламентныхОперацийКассовойСмены.СозданОтчетЧекиЗаархивированы)))";
...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	|		ИНАЧЕ ТаблицаВидыЗапасов.ПодразделениеРеализации
	|	КОНЕЦ КАК Подразделение,
	...
	|	ВЫБОР
	|		КОГДА ТаблицаВидыЗапасов.АналитикаРеализации <> ТаблицаВидыЗапасов.АналитикаУчетаНоменклатуры
	|			ИЛИ ТаблицаВидыЗапасов.ПериодПродажи < &НачалоМесяцаПериода
