...
Функция ДобавитьКомандуСоздатьНаОснованииПоГруппеСкладов(КомандыСозданияНаОсновании) Экспорт
	Если ПравоДоступа("Добавление", Метаданные.Документы.ВозвратТоваровПоставщику) Тогда
		КомандаСоздатьНаОсновании = КомандыСозданияНаОсновании.Добавить();
		КомандаСоздатьНаОсновании.Менеджер = Метаданные.Документы.ВозвратТоваровПоставщику.ПолноеИмя();
		КомандаСоздатьНаОсновании.Обработчик = "СозданиеНаОснованииУТКлиент.СоздатьВозвратТоваровПоставщикуНаОснованииПоГруппеСкладов";
		КомандаСоздатьНаОсновании.Представление = ОбщегоНазначенияУТ.ПредставлениеОбъекта(Метаданные.Документы.ВозвратТоваровПоставщику);
		КомандаСоздатьНаОсновании.РежимЗаписи = "Проводить";
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратТоваровКомитенту Тогда
		...
		ПараметрыУказанияСерий.ПараметрыПолученияДанныхИзРегистров.Вставить("Организация", Объект.Организация);
		ПараметрыУказанияСерий.ПараметрыПолученияДанныхИзРегистров.Вставить("Партнер", Объект.Партнер);
	КонецЕсли;
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если ВидФормы = "ФормаСписка" Тогда
		Если Не ПравоДоступа("Чтение", Метаданные.РегистрыСведений.СостоянияПоОбъектамУчетаЭДО)
			Или Не ПравоДоступа("Чтение", Метаданные.РегистрыСведений.ДокументыСОшибкамиПроверкиКонтрагентов) Тогда
...
Функция ПараметрыСозданияПоГруппеСкладов(ПараметрКоманды) Экспорт
	...
	ИначеЕсли Склады.Количество() = 0 Тогда
		Склады.Добавить(Справочники.Склады.ПустаяСсылка()); // в обработке заполнения документа товары заполняться не будут, будет выдано сообщение об отсутствии строк с нужным действием
	КонецЕсли;
...
Процедура ЗаполнитьСписокХозяйственныхОпераций(СписокОпераций) Экспорт
	...
	СписокОпераций.Добавить(Перечисления.ХозяйственныеОперации.ВозвратТоваровПоставщику);
...
Функция ТекстЗапросаВтВидыЗапасов(Запрос, ТекстыЗапроса) Экспорт
	...
	|	(ВЫБОР
	|		КОГДА ЕСТЬNULL(Аналитика.Назначение.ВидДеятельностиНДС, ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)) 
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	ВозвратТоваровПоставщикуЛокализация.ДополнитьТекстЗапросаТаблицаТоварыКОформлениюДокументовИмпорта(ТекстТовары);
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры)
	...
	ВзаиморасчетыПроведениеПоставщики.ПроведениеВозвратаТоваровПоставщику(Запрос, ТекстыЗапроса, Регистры, ТекстВозврат, ТекстРасшифровкаПлатежа);
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Иначе
			...
			Если ВыводитьКоды Тогда
				ОбластьКолонкаТовар.ШиринаКолонки = ОбластьКолонкаТовар.ШиринаКолонки * 0.975;
				ОбластьКолонкаКоличество = Макет.Область("ДанныеБезСуммКоличество");
				ОбластьКолонкаКоличество.ШиринаКолонки = ОбластьКолонкаКоличество.ШиринаКолонки * 0.95;
			Иначе
		...
		Иначе
			...
			Иначе
				ОбластьДанныеБезСуммГраница = Макет.ПолучитьОбласть("ШапкаТаблицыБезСумм|ДанныеБезСуммГраница");
				ТабличныйДокумент.Присоединить(ОбластьДанныеБезСуммГраница);
			КонецЕсли;
		КонецЕсли;
		...
		Пока ВыборкаПоТоварам.Следующий() Цикл
			...
			Иначе
				...
				Если ЕстьРНПТ Тогда
					...
					ТабличныйДокумент.Присоединить(ОбластьДанныеБезСуммКоличествоПоРНПТ);
				Иначе
					ОбластьДанныеБезСуммГраница = Макет.ПолучитьОбласть("СтрокаТаблицыБезСумм|ДанныеБезСуммГраница");
					ТабличныйДокумент.Присоединить(ОбластьДанныеБезСуммГраница);
				КонецЕсли;
