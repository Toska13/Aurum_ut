...
Процедура ЗаполнитьУсловияЗакупок(Знач УсловияЗакупок) Экспорт
	...
	БанковскийСчетОрганизации = ДенежныеСредстваСервер.ПолучитьБанковскийСчетОрганизацииПоУмолчанию(
		ПараметрыСчетаОрганизацииПоУмолчанию(), БанковскийСчетОрганизации);
...
Функция ВременныеТаблицыДанныхДокумента(ПоДокументуПоступления = Неопределено) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ТаблицаТоваров.ДокументПоступления = ЗНАЧЕНИЕ(Документ.ПриобретениеТоваровУслуг.ПустаяСсылка)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ТаблицаТоваров.ДокументПоступления
	...
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ТаблицаВидыЗапасов.ДокументПоступления
	|	КОНЕЦ КАК ДокументПередачи,
	|	ТаблицаВидыЗапасов.ВидЗапасов КАК ВидЗапасов,
	|	ТаблицаВидыЗапасов.НомерГТД КАК НомерГТД,
	|	ТаблицаВидыЗапасов.Количество КАК Количество,
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипДанныхЗаполнения = Тип("Структура") Тогда
		...
		ИначеЕсли ДанныеЗаполнения.Свойство("ЗаполнитьПоПринятойТаре") Тогда
...
Процедура ЗаполнитьДокументНаОснованииПоступленияТоваровИУслуг(Знач ДокументОснование)
	...
	Запрос = Новый Запрос(
		...
		|				И ПриобретениеТоваровУслуг.Склад.ВыборГруппы = ЗНАЧЕНИЕ(Перечисление.ВыборГруппыСкладов.РазрешитьВЗаказах)
		...
		|	ТоварыДокументаПоступления.Склад КАК Склад
		...
		|	ТоварыДокументаПоступления.Склад
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ЕСТЬNULL(ТоварыПоступления.Склад, ТоварыВозврата.Склад) КАК Склад
		...
		|		И ТоварыПоступления.НоменклатураПартнера = ТоварыВозврата.НоменклатураПартнера
		...
		|		И ТоварыПоступления.Склад = ТоварыВозврата.Склад
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	ТаблицаТоваров = ПакетЗапросов[8].Выгрузить();
	ВыборкаКРаспределению = ПакетЗапросов[9].Выбрать();
	СтруктураПоиска = Новый Структура("Номенклатура, Склад, Характеристика,
			|Серия, Назначение, ТипНоменклатуры, СтавкаНДС");
	
	Пока ВыборкаКРаспределению.Следующий() Цикл
		
		Количество = -ВыборкаКРаспределению.Количество;
		Сумма      = -ВыборкаКРаспределению.Сумма;
		СуммаНДС   = -ВыборкаКРаспределению.СуммаНДС;
		СуммаСНДС  = -ВыборкаКРаспределению.СуммаСНДС;
		
		ЗаполнитьЗначенияСвойств(СтруктураПоиска, ВыборкаКРаспределению);
		...
		Для Каждого СтрокаЗакупки Из СтрокиЗакупки Цикл
			...
			Если Количество > СтрокаЗакупки.Количество Тогда
	...
	ТаблицаТары = ПакетЗапросов[11].Выгрузить();
...
Процедура ЗаполнитьПоАктуКонтроляКачества(Знач ДанныеЗаполнения)
	...
	Запрос.Текст =
		...
		|		ПО ПриобретениеТоваровУслуг.Ссылка = Реквизиты.Партия
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	Реквизиты.НаправлениеДеятельности КАК НаправлениеДеятельности,
		|	Реквизиты.ЦенаВключаетНДС КАК ЦенаВключаетНДС,
		|	Реквизиты.Менеджер КАК Менеджер,
		|	Реквизиты.НалогообложениеНДС КАК НалогообложениеНДС,
	...
	Если ВсеОперацииДокумента.НайтиПоЗначению(ХозяйственнаяОперация) = Неопределено Тогда
		ТекстОшибки = НСтр("ru='Для операции ""%1"", по которой поступил товар, не предполагается создание документа ""Возврат товаров поставщику"". Ввод на основании документа запрещен.'");
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	ОтветственныеЛицаСервер.УстановитьМенеджера(
		ЭтотОбъект, ЗакупкиСервер.ДокументОснование(ЭтотОбъект, "ДокументПоступления, Товары.ДокументПоступления"));
	...
	Если ЗначениеЗаполнено(ДокументПоступления) Тогда
			СтруктураЗаполненияСтавкиНДС.Дата = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументПоступления, "Дата");
	КонецЕсли;
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	Если Не Проведен
		...
		Если ИмеютсяЗаписиТаблицыТоваровВМенеджереВременныхТаблиц(МенеджерВременныхТаблицПоДокументуПоступления) Тогда
			...
			ВидыЗапасовДляЗаполнения.Колонки.Добавить("ДокументПередачи", 
			...
			ЗапасыСервер.ЗаполнитьВидыЗапасовПоТоварамОрганизаций(ЭтотОбъект,
																	...
																	Отказ,
																	ПараметрыЗаполнения);
			ВидыЗапасовДляЗаполнения.Колонки.ДокументПоступления.Имя = "УдалитьДокументПоступления";														
			ВидыЗапасовДляЗаполнения.Колонки.ДокументПередачи.Имя = "ДокументПоступления";
...
Процедура ЗаполнитьДопКолонкиВидовЗапасов() Экспорт
	...
	Для Каждого СтрокаТоваров Из ТаблицаТовары Цикл
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			СтруктураПоиска.ДокументПоступления = Документы.ПриобретениеТоваровУслуг.ПустаяСсылка();
			НайденныеСтроки = ВидыЗапасов.НайтиСтроки(СтруктураПоиска);
		КонецЕсли;
		
		ЗаполнитьДопКолонкиВидовЗапасовПоТаблицеТоваров(СтрокаТоваров, НайденныеСтроки, КоличествоТоваров, КоличествоУпаковок);
		
		Если КоличествоТоваров <> 0
			И СтруктураПоиска.ДокументПоступления <> Документы.ПриобретениеТоваровУслуг.ПустаяСсылка() Тогда
			СтруктураПоиска.ДокументПоступления = Документы.ПриобретениеТоваровУслуг.ПустаяСсылка();
			НайденныеСтроки = ВидыЗапасов.НайтиСтроки(СтруктураПоиска);
			ЗаполнитьДопКолонкиВидовЗапасовПоТаблицеТоваров(СтрокаТоваров, НайденныеСтроки, КоличествоТоваров, КоличествоУпаковок);
		КонецЕсли;
...
Процедура ЗаполнитьДопКолонкиВидовЗапасовПоТаблицеТоваров(СтрокаТоваров, НайденныеСтроки, КоличествоТоваров, КоличествоУпаковок)
