...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		ПриобретениеТоваровУслугЛокализация.ДополнитьТекстыЗапросовПроведенияУчетомНДС(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект);
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Документы.ОтгрузкаТоваровКлиенту.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
...
// 
// Параметры:
//  ОбъектИлиСсылка - ДокументОбъект.ПриобретениеТоваровУслуг, ДокументСсылка.ПриобретениеТоваровУслуг - документ.
...
Функция РеквизитыКонтролируемойПартии(ОбъектИлиСсылка, ИмяПоляКонтролируемаяПартия) Экспорт
	...
	Если РеквизитыОбъекта.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаЧерезПодотчетноеЛицо Тогда
		Реквизиты.Отправитель = РеквизитыОбъекта.ПодотчетноеЛицо;
	КонецЕсли;
...
// 
// Параметры:
//  Объект - ДокументОбъект.ПриобретениеТоваровУслуг - объект.
// 
// Возвращаемое значение:
...
Функция КонтрольКачестваПараметрыДействий(Объект) Экспорт
	...
	Сбросить = Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаРеглУчет;
	...
	ПараметрыДействия.ПараметрыЗапроса.Вставить("КонтрольКачестваСбросить", Сбросить);
	...
	ПараметрыДействия.ТекстЗапросаДанныеДокумента =
		...
		|	ЛОЖЬ КАК Установить,
		|	&КонтрольКачестваСбросить ИЛИ Таблица.СписатьНаРасходы КАК Сбросить
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("7344821c-a805-4238-a7d6-52a2ec45bcee");
	...
	СписокОписаний.Добавить(НСтр("ru = 'Обновление документов ""Приобретение товаров и услуг"":'"));
	СписокОписаний.Добавить(НСтр("ru = '- заполняет реквизит ""Вариант приемки услуг"" значением реквизита ""Вариант приемки товаров"".'"));
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.ПриобретениеТоваровУслуг КАК РеквизитыДокумента
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
	|		ПО РеквизитыДокумента.Договор = ДоговорыКонтрагентов.Ссылка
	|ГДЕ
	|	РеквизитыДокумента.ВариантПриемкиУслуг = ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПустаяСсылка)";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	РеквизитыДокумента.ВариантПриемкиТоваров КАК ВариантПриемкиТоваров
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриобретениеТоваровУслуг КАК РеквизитыДокумента
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				Если Не ЗначениеЗаполнено(ДокументОбъект.ВариантПриемкиУслуг) Тогда
					ДокументОбъект.ВариантПриемкиУслуг = Выборка.ВариантПриемкиТоваров;
		...
		Исключение
			...
			Если ПричинаИсключения = 2 Тогда
				ВызватьИсключение НСтр("ru = 'Не удалось обработать обновление документов ""Приобретение товаров и услуг"".'");
			КонецЕсли;
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ДанныеДокумента.ВариантПриемкиУслуг КАК ВариантПриемкиУслуг,
	...
	ДанныеОбъекта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Реквизиты.Договор, 
	...
	НеотфактурованныеПоставкиУслугРабот = ОперацииФактуровка.Найти(Реквизиты.ХозяйственнаяОперация) <> Неопределено
		И (ДанныеОбъекта.ВариантОформленияЗакупок = Перечисления.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваровИУслуг
		Или ДанныеОбъекта.ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется);
...
Процедура ТекстЗапросаДанныеДокументаШапка(СписокЗапросов)
	...
	|	ДанныеШапки.ВариантПриемкиУслуг КАК ВариантПриемкиУслуг,
	...
	|	ДанныеШапки.ФактическаяДатаОперации КАК ФактическаяДатаОперации
...
Процедура ЗапланироватьПоступлениеТоваров(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	|	ДанныеДокументаШапка.ВариантПриемкиУслуг    КАК ВариантПриемкиУслуг,
	|	
	...
	|	ДанныеДокументаШапка.КонтролируемаяПартия    КАК КонтролируемаяПартия,
...
// Проверка на не использование схемы двухходовки.
	...
	СкладыСервер.ОформитьПриобретениеТоваровПоДвухходовке(Запрос, ТекстыЗапроса, Регистры, ТекстЗапросаДокумента, ,Истина);
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|	ТабЧасть.Ссылка.КонтролируемаяПартия КАК КонтролируемаяПартия,
		...
		|						ВЫБОР
		|							КОГДА ТабЧасть.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
		|								ТОГДА ТабЧасть.Ссылка.ВариантПриемкиУслуг
		|							ИНАЧЕ ТабЧасть.Ссылка.ВариантПриемкиТоваров
		|						КОНЕЦ В (
		...
		|							ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки),
		|							ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки))
		|				И ЕСТЬNULL(ТабЧасть.Ссылка.Договор.ВариантОформленияДокументовПоступленияПоУслугам = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется),
		|							ЛОЖЬ)))";
	...
	ТекстЗапросаТабЧасть =
		...
		|						ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаПоИмпортуТоварыВПути)))
		|			ИЛИ ТабЧасть.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
		|				И ТабЧасть.Ссылка.ВариантПриемкиУслуг = ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.РазделенаТолькоПоНакладным)
	...
	РаспределениеЗапасовДвижения.ЗапланироватьПриходЗапаса(Запрос, ТекстыЗапроса, Регистры, ТекстЗапросаТабЧасть, Истина, Истина);
...
Функция ТекстЗапросаВтТоварыОрганизаций(Запрос, ТекстыЗапроса, ДокументОбъект = Неопределено)  Экспорт
	...
	|	ТаблицаТовары.СтавкаНДС,
	|	СУММА(ВЫБОР
	|			КОГДА ТоварыДокумента.Количество < ТаблицаТовары.Количество
	|				ТОГДА ТоварыДокумента.Количество
	|			ИНАЧЕ ТаблицаТовары.Количество
	|		КОНЕЦ),
	|	СУММА(ВЫБОР
	|			КОГДА ТоварыДокумента.КоличествоПоРНПТ < ТаблицаТовары.КоличествоПоРНПТ
	|				ТОГДА ТоварыДокумента.КоличествоПоРНПТ
	|			ИНАЧЕ ТаблицаТовары.КоличествоПоРНПТ
	|		КОНЕЦ) КАК КоличествоПоРНПТ,
	// На склад.
...
Функция ТекстЗапросаВтТаблицаАналитикУчетаПартий(Запрос, ТекстыЗапроса, ДокументОбъект = Неопределено)
	...
	|		 И НЕ &УпрощеннаяСистемаНалогообложения
...
Функция ТекстЗапросаВтТоварыДляСоединенияАналитик(Запрос, ТекстыЗапроса)
	...
	|	СУММА(ТЧТовары.СуммаВзаиморасчетов) КАК СуммаВзаиморасчетов,
	|	СУММА(ТЧТовары.СуммаСНДС) КАК СуммаСНДС,
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	ТекстЗапросаКурсыВалютДокумента = ВзаиморасчетыПроведение.ПолучитьТаблицуКурсовВалютНаправленийДеятельности(ТекстЗапросаКурсыВалютДокументаНаправленияДеятельности);
	...
	ТекстЗакупка = "
		...
		|	ВЫБОР КОГДА Таблица.СверхЗаказа
		|		ТОГДА Неопределено
		|		ИНАЧЕ Таблица.Заказ
	...
	ВзаиморасчетыПроведениеПоставщики.ПроведениеЗакупки(Запрос, ТекстыЗапроса, Регистры, ТекстЗакупка, ТекстПланОплаты, ТекстЗачетАванса, ТекстТовары);
...
// Возвращает адрес во временном хранилище на сформированную коллекцию данных, используемую для печати этикеток.
//
// Параметры:
//	МассивДокументов - Массив Из ДокументСсылка.ПриобретениеТоваровУслуг - коллекция ссылок на документы, используемых
//																			для подготовки данных печати этикеток.
//
// Возвращаемое значение:
//	Строка - адрес во временном хранилище на сформированную коллекцию данных, используемую для печати этикеток.
//
Функция ДанныеДляПечатиЭтикеток(МассивДокументов) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеСклада.ТипСклада, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ТипыСкладов.РозничныйМагазин)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ										КАК РозничныйМагазин,
	|	ВЫБОР
	|		КОГДА ДанныеСклада.РозничныйВидЦены ЕСТЬ NULL
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыЦен.ПустаяСсылка)
	|		ИНАЧЕ ДанныеСклада.РозничныйВидЦены
	|	КОНЕЦ										КАК РозничныйВидЦены,
	|	МИНИМУМ(ТаблицаТоваров.НомерСтроки)			КАК Порядок,
	|	ТаблицаТоваров.Номенклатура					КАК Номенклатура,
	|	ТаблицаТоваров.Характеристика				КАК Характеристика,
	|	ВЫБОР
	|		КОГДА ДанныеХарактеристики.Ссылка ЕСТЬ NULL
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатурыДляЦенообразования.ПустаяСсылка)
	|		ИНАЧЕ ДанныеХарактеристики.ХарактеристикаНоменклатурыДляЦенообразования
	|	КОНЕЦ										КАК ХарактеристикаЦО,
	|	ТаблицаТоваров.Серия						КАК Серия,
	|	ВЫБОР
	|		КОГДА ДанныеСерии.Ссылка ЕСТЬ NULL
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СерииНоменклатурыДляЦенообразования.ПустаяСсылка)
	|		ИНАЧЕ ДанныеСерии.СерияНоменклатурыДляЦенообразования
	|	КОНЕЦ										КАК СерияЦО,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриобретениеТоваровУслуг КАК ДанныеШапки
	|		ПО ТаблицаТоваров.Ссылка = ДанныеШапки.Ссылка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК ДанныеСклада
	|		ПО ДанныеШапки.Склад = ДанныеСклада.Ссылка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатуры КАК ДанныеХарактеристики
	|		ПО ТаблицаТоваров.Характеристика = ДанныеХарактеристики.Ссылка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СерииНоменклатуры КАК ДанныеСерии
	|		ПО ТаблицаТоваров.Серия = ДанныеСерии.Ссылка
	|
	|ГДЕ
	...
	|	ТаблицаТоваров.Номенклатура			КАК Номенклатура,
	|	ТаблицаТоваров.Характеристика		КАК Характеристика,
	|	КОЛИЧЕСТВО(ТаблицаТоваров.Упаковка)	КАК КоличествоУпаковок
	|ПОМЕСТИТЬ ТаблицаТоваровСгруппированная
	|ИЗ
	...
	|ПОМЕСТИТЬ ТаблицаСерийДокументов
	|ИЗ
	|	Документ.ПриобретениеТоваровУслуг.Серии КАК ТаблицаСерий
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СерииНоменклатуры КАК ДанныеСерии
	|		ПО ТаблицаСерий.Серия = ДанныеСерии.Ссылка
	|
	|ГДЕ
	|	ТаблицаСерий.Ссылка В (&МассивДокументов)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаСерий.Ссылка,
	...
	|	ТаблицаСерий.Номенклатура		КАК Номенклатура,
	|	ТаблицаСерий.Характеристика		КАК Характеристика,
	|	КОЛИЧЕСТВО(ТаблицаСерий.Серия)	КАК КоличествоСерий
	|ПОМЕСТИТЬ ТаблицаСерийСгруппированная
	|ИЗ
	|	ТаблицаСерийДокументов КАК ТаблицаСерий
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаСерий.Ссылка,
	|	ТаблицаСерий.Номенклатура,
	...
	|	ТаблицаСерий.Характеристика	КАК Характеристика,
	|	ТаблицаСерий.Серия			КАК Серия,
	|	ТаблицаСерий.СерияЦО		КАК СерияЦО,
	|	ТаблицаТоваров.Упаковка		КАК Упаковка,
	|	ТаблицаСерий.Количество / ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, 1) КАК Количество
	|ПОМЕСТИТЬ ТаблицаСерийДляЗапроса
	|ИЗ
	|	ТаблицаТоваровДляЗапроса КАК ТаблицаТоваров
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаСерийДокументов КАК ТаблицаСерий
	|		ПО ТаблицаТоваров.Ссылка = ТаблицаСерий.Ссылка
	|			И ТаблицаТоваров.Номенклатура = ТаблицаСерий.Номенклатура
	|			И ТаблицаТоваров.Характеристика = ТаблицаСерий.Характеристика
	|
	|ГДЕ
	|	(ТаблицаТоваров.Ссылка, ТаблицаТоваров.Номенклатура, ТаблицаТоваров.Характеристика) В
	|		(ВЫБРАТЬ
	|			ТаблицаГруппировки.Ссылка			КАК Ссылка,
	|			ТаблицаГруппировки.Номенклатура		КАК Номенклатура,
	|			ТаблицаГруппировки.Характеристика	КАК Характеристика
	|		ИЗ
	|			ТаблицаТоваровСгруппированная КАК ТаблицаГруппировки
	|		ГДЕ
	|			ТаблицаГруппировки.КоличествоУпаковок = 1)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		ГДЕ
	|			ТаблицаГруппировки.КоличествоУпаковок > 1)
	|	И (ТаблицаТоваров.Ссылка, ТаблицаТоваров.Номенклатура, ТаблицаТоваров.Характеристика) В
	|		(ВЫБРАТЬ
	|			ТаблицаГруппировки.Ссылка			КАК Ссылка,
	|			ТаблицаГруппировки.Номенклатура		КАК Номенклатура,
	|			ТаблицаГруппировки.Характеристика	КАК Характеристика
	|		ИЗ
	|			ТаблицаСерийСгруппированная КАК ТаблицаГруппировки
	|		ГДЕ
	|			ТаблицаГруппировки.КоличествоСерий = 1)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура,
	|	Характеристика,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаСерийДляЗапроса КАК ТаблицаСерий
	|		ПО ШтрихкодыНоменклатуры.Номенклатура = ТаблицаСерий.Номенклатура
	|			И ШтрихкодыНоменклатуры.Характеристика = ТаблицаСерий.Характеристика
	|			И ШтрихкодыНоменклатуры.Серия = ТаблицаСерий.Серия
	|			И ШтрихкодыНоменклатуры.Упаковка = ТаблицаСерий.Упаковка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		ПО ТаблицаСерий.Номенклатура = ШтрихкодыНоменклатуры.Номенклатура
	|			И ТаблицаСерий.Характеристика = ШтрихкодыНоменклатуры.Характеристика
	|			И ТаблицаСерий.Серия = ШтрихкодыНоменклатуры.Серия
	|			И ТаблицаСерий.Упаковка = ШтрихкодыНоменклатуры.Упаковка
	|
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МИНИМУМ(ТаблицаТоваров.Порядок)	КАК Порядок,
	|	ТаблицаТоваров.Номенклатура		КАК Номенклатура,
	|	ТаблицаТоваров.Характеристика	КАК Характеристика,
	|	ТаблицаТоваров.Серия			КАК Серия,
	|	ВЫБОР
	|		КОГДА ТаблицаТоваров.РозничныйМагазин
	|			ТОГДА ЕСТЬNULL(ЦеныНоменклатуры.Упаковка, ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка))
	|		ИНАЧЕ ТаблицаТоваров.Упаковка
	|	КОНЕЦ							КАК Упаковка,
	|	СУММА(ВЫБОР
	|		КОГДА ТаблицаТоваров.РозничныйМагазин
	|			ТОГДА ТаблицаТоваров.Количество * (ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, 1) / ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки2, 1))
	...
	|			И ТаблицаТоваров.РозничныйВидЦены = ЦеныНоменклатуры.ВидЦены
	|
	|ГДЕ
	|	(ТаблицаТоваров.Ссылка, ТаблицаТоваров.Номенклатура, ТаблицаТоваров.Характеристика, ТаблицаТоваров.Упаковка) НЕ В
	|		(ВЫБРАТЬ
	|			ТаблицаСерий.Ссылка			КАК Ссылка,
	|			ТаблицаСерий.Номенклатура	КАК Номенклатура,
	|			ТаблицаСерий.Характеристика	КАК Характеристика,
	...
	|	СУММА(ВЫБОР
	|		КОГДА ТаблицаТоваров.РозничныйМагазин
	|			ТОГДА ТаблицаСерий.Количество * (ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, 1) / ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки2, 1))
	|		ИНАЧЕ ТаблицаСерий.Количество
	|	КОНЕЦ)							КАК Количество
	|ИЗ
	|	ТаблицаТоваровДляЗапроса КАК ТаблицаТоваров
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаСерий КАК ТаблицаСерий
	|		ПО ТаблицаТоваров.Ссылка = ТаблицаСерий.Ссылка
	|			И ТаблицаТоваров.Номенклатура = ТаблицаСерий.Номенклатура
	|			И ТаблицаТоваров.Характеристика = ТаблицаСерий.Характеристика
	|			И ТаблицаТоваров.Упаковка = ТаблицаСерий.Упаковка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры25.СрезПоследних КАК ЦеныНоменклатуры
	|		ПО ТаблицаТоваров.Номенклатура = ЦеныНоменклатуры.Номенклатура
	|			И ТаблицаТоваров.ХарактеристикаЦО = ЦеныНоменклатуры.ХарактеристикаЦО
	|			И ТаблицаСерий.СерияЦО = ЦеныНоменклатуры.СерияЦО
	|			И ТаблицаТоваров.РозничныйВидЦены = ЦеныНоменклатуры.ВидЦены
	|			И ТаблицаТоваров.РозничныйМагазин
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаТоваров.Номенклатура,
	|	ТаблицаТоваров.Характеристика,
	...
	|
	|ГДЕ
	|	(ТаблицаСерий.Ссылка, ТаблицаСерий.Номенклатура, ТаблицаСерий.Характеристика) В
	|		(ВЫБРАТЬ
	|			ТаблицаГруппировки.Ссылка			КАК Ссылка,
	|			ТаблицаГруппировки.Номенклатура		КАК Номенклатура,
	|			ТаблицаГруппировки.Характеристика	КАК Характеристика
	...
	|		ГДЕ
	|			ТаблицаГруппировки.КоличествоУпаковок > 1)
	|	И (ТаблицаСерий.Ссылка, ТаблицаСерий.Номенклатура, ТаблицаСерий.Характеристика) В
	|		(ВЫБРАТЬ
	|			ТаблицаГруппировки.Ссылка			КАК Ссылка,
	|			ТаблицаГруппировки.Номенклатура		КАК Номенклатура,
	|			ТаблицаГруппировки.Характеристика	КАК Характеристика
	...
	|	ДанныеШапки.Организация				КАК Организация,
	|	ДанныеШапки.Склад					КАК Склад,
	|	ДанныеШапки.Склад.РозничныйВидЦены	КАК ВидЦены
	|ИЗ
	|	Документ.ПриобретениеТоваровУслуг КАК ДанныеШапки
	|ГДЕ
	|	ДанныеШапки.Ссылка В (&МассивДокументов)";
	...
	РеквизитыДокументов = РезультатЗапроса[РезультатЗапроса.ВГраница()].Выгрузить();
	
	Организации	= ОбщегоНазначенияУТ.УникальныеЗначенияИзКолонкиТаблицы(РеквизитыДокументов, "Организация");
	Склады		= ОбщегоНазначенияУТ.УникальныеЗначенияИзКолонкиТаблицы(РеквизитыДокументов, "Склад");
	ВидыЦен		= ОбщегоНазначенияУТ.УникальныеЗначенияИзКолонкиТаблицы(РеквизитыДокументов, "ВидЦены");
	
	// Подготовка структуры действий для обработки печати ценников и этикеток
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ЗаполнитьОрганизацию", ?(Организации.Количество() = 1, Организации[0], Неопределено));
	СтруктураДействий.Вставить("ЗаполнитьСклад", ?(Склады.Количество() = 1, Склады[0], Склады));
	СтруктураДействий.Вставить("ЗаполнитьВидЦены", ?(ВидыЦен.Количество() = 1, ВидыЦен[0], Неопределено));
	...
	СтруктураРезультат.Вставить("Товары",					РезультатЗапроса[РезультатЗапроса.ВГраница() - 2].Выгрузить());
	СтруктураРезультат.Вставить("ТаблицаОшибокПоСериям",	РезультатЗапроса[РезультатЗапроса.ВГраница() - 1].Выгрузить());
...
// 
// Параметры:
//  СсылкаОбъект - ДокументСсылка.ПриобретениеТоваровУслуг - Документ, по которому нужно получить данные.
...
Функция СуммыПоЗаказам(СсылкаОбъект, ЗначенияРеквизитов = Неопределено) Экспорт
	...
	Если ЗначенияРеквизитов = Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТаблицаЗначений", "Документ.ПриобретениеТоваровУслуг.Товары");
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ЗаказШапки", "Товары.Ссылка.ЗаказПоставщику");
...
Процедура ПроверитьТипыНоменклатуры(ХозяйственнаяОперация, Организация, Договор, СписокНоменклатуры, Отказ) Экспорт
	...
	РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Договор, "ВариантОформленияЗакупок, 
		|ВариантОформленияДокументовПоступленияПоУслугам");
	
	Если Не РеквизитыДоговора.ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется
		Или Не РеквизитыДоговора.ВариантОформленияЗакупок = Перечисления.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваровИУслуг Тогда
		Возврат;
	КонецЕсли;
	...
	|		КОНЕЦ) КАК Товары,
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))
	|				ТОГДА 1
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК РаботыУслуги
	|ИЗ
	...
	Если Выборка.Следующий() Тогда
		Если Выборка.РаботыУслуги > 0 И Выборка.Товары > 0 Тогда
			ШаблонСообщения = НСтр("ru='По организации ""%1"" в договоре ""%2"" установлены разные методы оформления закупки товаров, работ и услуг.
				|Не допускается одновременное оформление товаров по схеме ""Неотфактурованные поставки товаров"" и работ и услуг по схеме 
				|""Поступление услуг сторнируется (""метод начисления"")"".
				|Оформите документы ""Приобретение товаров и услуг"" отдельно по товарам, отдельно по работам и услугам.'");
			
			ТекстСообщения = СтрШаблон(ШаблонСообщения, Организация, Договор);	
			
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		ИначеЕсли Выборка.РаботыУслуги > 0 И Выборка.Товары = 0 Тогда
			ШаблонСообщения = НСтр("ru='По организации ""%1"" в договоре ""%2"" установлены разные методы оформления закупки товаров, работ и услуг.
				|Для оформления приобретения работ и услуг по схеме ""Поступление услуг сторнируется (""метод начисления"")"" 
				|необходимо использовать документ ""Приобретение товаров и услуг"" с хозяйственной операцией ""Закупка у поставщика"".'");
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, СкладПоступления = Неопределено,
	...
	Если ТипЗнч(ДанныеОтбора) = Тип("Структура") И ДанныеОтбора.Свойство("ТолькоРаботыУслуги") Тогда
		ТолькоРаботыУслуги = ДанныеОтбора.ТолькоРаботыУслуги;
	КонецЕсли;
	...
	Иначе
		РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеОтбора.Договор, "ВариантОформленияЗакупок, 
			|ВариантОформленияДокументовПоступленияПоУслугам");
		
		Если Не РеквизитыДоговора.ВариантОформленияЗакупок = Перечисления.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваровИУслуг
			И РеквизитыДоговора.ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется Тогда
			Если ДанныеОтбора.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС Тогда
				Запрос.УстановитьПараметр("ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки);
			ИначеЕсли ДанныеОтбора.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика Тогда
				Запрос.УстановитьПараметр("ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки);
			Иначе
	...
	Запрос.УстановитьПараметр("ТолькоРаботыУслуги", 	 ТолькоРаботыУслуги);
	...
	РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеОтбора.Договор,
		"ВариантОформленияЗакупок, ВариантОформленияДокументовПоступленияПоУслугам");
	...
	Иначе
		...
		ТоварыНакладной.Колонки.Добавить("Количество", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
		...
		ТоварыНакладной.Колонки.Добавить("КоличествоВЗаказе", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
	...
	|				КОНЕЦ
	|				И (НЕ &ТолькоРаботыУслуги ИЛИ Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)))
	|			) КАК ЗаказыОстатки
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
...
Функция ТекстЗапросаТаблицаТоварыУслугиКОформлениюОтчетовКомитентуОЗакупках(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	|ПО
	|	ТаблицаТовары.Назначение = Назначения.Ссылка
	|ГДЕ
