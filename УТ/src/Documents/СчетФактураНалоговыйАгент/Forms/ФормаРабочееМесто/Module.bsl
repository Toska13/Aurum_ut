...
&НаКлиенте
Процедура УстановитьСтавкуНДС5(Команда)
	
	УстановитьСтавкуНДС("5/105");
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьСтавкуНДС7(Команда)
	
	УстановитьСтавкуНДС("7/107");
...
Процедура УстановитьСтавкуНДС(ТекстСтавкаНДС)
	...
	ИначеЕсли ТекстСтавкаНДС = "5/105" Тогда
		ТекСтавкаНДС = УчетНДСРФКлиентСерверПовтИсп.СтавкаНДСПоЗначениюПеречисления(
							ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС5_105"));
	ИначеЕсли ТекстСтавкаНДС = "7/107" Тогда
		ТекСтавкаНДС = УчетНДСРФКлиентСерверПовтИсп.СтавкаНДСПоЗначениюПеречисления(
							ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС7_107"));
	Иначе
		ТекСтавкаНДС = УчетНДСРФКлиентСерверПовтИсп.СтавкаНДСПоЗначениюПеречисления(
	...
	Для каждого ИдентификаторСтроки Из Элементы.ОплатыПоставщикам.ВыделенныеСтроки Цикл
		...
		Если ДанныеСтроки.СтавкаНДС <> ТекСтавкаНДС Тогда
			...
			ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ДанныеСтроки, СтруктураДействий, Неопределено);
		КонецЕсли; 
	КонецЦикла; 
