...
Функция ЕстьСчетаФактурыКОформлению(Параметры) Экспорт
	...
	ОплатыПоставщикам.Колонки.Добавить("Организация");
	...
	Если Параметры.Свойство("ПоОрганизациям") Тогда
		ОплатыПоставщикамБезСФ = ОплатыПоставщикам.Скопировать(
			ОплатыПоставщикам.НайтиСтроки(Новый Структура("СФСформирован", Ложь)),"Организация");
		ОплатыПоставщикамБезСФ.Свернуть("Организация");
		Возврат ОплатыПоставщикамБезСФ.ВыгрузитьКолонку("Организация")
	Иначе
...
Функция ТекстЗапросаТаблицаНДСПредъявленный(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ВтДанныеДляТаблицыИсходныеПартииПрочихРасходов КАК Строки
	|	ПО
	|		ИСТИНА
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
...
Функция ТекстЗапросаТаблицаПрочиеАктивыПассивы(Запрос, ТекстыЗапроса, Регистры)
	...
	|
	|ИЗ
	|	ВтДанныеДляТаблицыИсходныеПартииПрочихРасходов КАК ПартииПрочихРасходов
	|
	|ГДЕ
	|	ВЫБОР
	|		КОГДА &СФПоАренде
...
Процедура ИнициализироватьВтПартииПрочихРасходов(Запрос)
	...
	|	ВтНачислениеНДС.ИдентификаторСтроки,
	...
	|	ПартииПрочихРасходов.ИдентификаторСтроки,
	|	ПартииПрочихРасходов.ИдентификаторФинЗаписи
	|ПОМЕСТИТЬ ВтДанныеДляТаблицыИсходныеПартииПрочихРасходов
	|ИЗ
	...
	|	ВЫБОР
	|		КОГДА &СФПоАренде
	|			ТОГДА ПартииПрочихРасходов.НДСУпр
	|		ИНАЧЕ
	|			ПартииПрочихРасходов.Стоимость
	|	КОНЕЦ КАК Стоимость,
	|	ПартииПрочихРасходов.СтоимостьБезНДС,
...
Функция ТекстЗапросаТаблицаВтИсходныеПрочиеРасходы(Запрос, ТекстыЗапроса)
	...
	|ИЗ
	|	ВтДанныеДляТаблицыИсходныеПартииПрочихРасходов КАК ТаблицаРасходов
	|
	|ГДЕ
	|	ВЫБОР
	|		КОГДА &СФПоАренде
...
Функция ВключитьНДСВПартииПрочихРасходов(ТаблицаОплаты, ТаблицаПоступления, Реквизиты)
	...
	Для Каждого СтрокаОплаты Из ТаблицаОплаты Цикл
		...
		Для Каждого СтрокаПоступления Из СтрокиПоступления Цикл
			...
			ЗаполнитьЗначенияСвойств(СтрокаРезультата, СтрокаОплаты, "КорНаправлениеДеятельности, ИдентификаторСтроки");
			...
			Если Реквизиты.СФПоАренде Тогда 
				ЗаполнитьЗначенияСвойств(СтрокаРезультата, СтрокаОплаты, "НастройкаХозяйственнойОперации");
			КонецЕсли;
			Если ЗначениеЗаполнено(СтрокаПоступления.АналитикаРасходов) Тогда 
				СтрокаРезультата.ИдентификаторФинЗаписи = СтрокаПоступления.АналитикаРасходов.УникальныйИдентификатор();
			Иначе
...
Функция РезультатЗапросаПоОплатамНалоговыйАгент(НачалоПериода, КонецПериода, Организация, РасчетныйДокумент)
	...
	|	КОГДА НЕ ОбъектРасчетовИсточникАванса.Объект ЕСТЬ NULL
	|		ТОГДА ОбъектРасчетовИсточникАванса.Объект
	|	ИНАЧЕ
	|		Расчеты.Регистратор
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ОбъектыРасчетов КАК ОбъектРасчетовИсточникАванса
	|		ПО Расчеты.КорОбъектРасчетов = ОбъектРасчетовИсточникАванса.Ссылка
	...
	|			И Расчеты.Сумма > 0
	|			И (ОбъектРасчетовИсточникАванса.Объект ССЫЛКА Документ.СписаниеБезналичныхДенежныхСредств
	|				ИЛИ ОбъектРасчетовИсточникАванса.Объект ССЫЛКА Документ.АвансовыйОтчет
	|				ИЛИ ОбъектРасчетовИсточникАванса.Объект ССЫЛКА Документ.РасходныйКассовыйОрдер)
	|ГДЕ
