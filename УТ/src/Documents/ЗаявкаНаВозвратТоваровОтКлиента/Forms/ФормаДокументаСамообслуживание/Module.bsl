...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Склад = Объект.Склад;
	ЗаполнитьСписокВыбораСкладовВТабличнойЧастиНаФорме();
...
Процедура СоглашениеНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыбора <> Неопределено И РезультатВыбора.Соглашение <> Объект.Соглашение Тогда
		...
		Если Объект.ВозвращаемыеТовары.Количество() > 0 Тогда
			...
			ТекстВопроса = НСтр("ru = 'Соглашение было изменено, табличная часть Возвращаемые товары будет очищена. Продолжить?'");
			ПоказатьВопрос(
...
Процедура КонтрагентОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение <> ТекущееЗначениеКонтрагента Тогда
		Если Объект.ВозвращаемыеТовары.Количество() > 0 Тогда
			ТекстВопроса = НСтр("ru = 'Контрагент был изменен, табличная часть Возвращаемые товары будет очищена. Продолжить?'");
...
&НаСервере
Процедура ЗаполнитьСписокВыбораСкладовВТабличнойЧастиНаФорме()  
	Если НЕ СкладГруппа Тогда
		Возврат;
	КонецЕсли;	
	
	Данные = Новый Структура("Склад,ЭтоГруппаСкладов", Склад, СкладГруппа);
	СамообслуживаниеСервер.ЗаполнитьСписокВыбораСклада(Данные, Элементы.ВозвращаемыеТоварыСклад, Истина);
