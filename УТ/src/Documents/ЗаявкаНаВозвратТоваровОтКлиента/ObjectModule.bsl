...
Процедура ЗаполнитьУсловияПродажПоУмолчанию(ПересчитатьЦены = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(Партнер) ИЛИ НЕ ИспользоватьСоглашенияСКлиентами Тогда
		...
		ХозяйственныеОперации = ПродажиСервер.ПолучитьХозяйственнуюОперациюСоглашенияПоВозврату(ХозяйственнаяОперация);
...
Процедура ЗаполнитьУсловияПродажПоСоглашению(ЗаполнитьЗаменяющиеЦены = Истина) Экспорт
	...
	УчетныйВидЦены = Справочники.ВидыЦен.ПустаяСсылка();
	Если УсловияПродаж.НоваяСхемаКомиссии Тогда
		// Вид цены в "Новой комиссии" используется из договора 
		УчетныйВидЦены = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "ВидЦенУчетный");
	КонецЕсли;
	...
	ПараметрыЗаполнения.Вставить("Соглашение", Соглашение);
	Если УсловияПродаж.НоваяСхемаКомиссии Тогда
		ПараметрыЗаполнения.Вставить("ВидЦены", УчетныйВидЦены);
	КонецЕсли;
	...
	ПараметрыЗаполнитьСтавкуНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДСПоВидуЦен(ЭтотОбъект);
	ПараметрыЗаполнитьСтавкуНДС.Дата = ДатаРеализации;
...
Функция УстановитьСтатус(НовыйСтатус, ДополнительныеПараметры) Экспорт
	...
	Если ЭтоЗаказСоСклада Тогда
		...
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ЗаменяющиеТовары, СтруктураДействий, Неопределено);
	КонецЕсли;
...
Функция СкорректироватьСтрокиЗаказа(СтруктураПараметров) Экспорт
	...
	ВозвратПоПоступлению         = ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтКомиссионера
		Или ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтХранителя;
	...
	Запрос = Новый Запрос;
	Если ВозвратПоПоступлению Тогда
		Запрос.Текст = ТекстЗапросаПоТоварамКПоступлению();
	Иначе
		Запрос.Текст = ТекстЗапросаПоЗаявкамНаВозвратТоваровОтКлиентов();
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ТаблицаТовары",
		ВозвращаемыеТовары.Выгрузить(,
			"НомерСтроки, КодСтроки, Склад, Номенклатура, Характеристика, Упаковка,
			|Порча, НоменклатураОприходование, ХарактеристикаОприходование,
			|Количество, СуммаСНДС, Отменено, Серия"));
	...
	Запрос.УстановитьПараметр("ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров",
		Константы.ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров.Получить() / 100);
	
	ГраницыОборотов = ОбщегоНазначенияУТ.ГраницыОборотовРегистра(
		"РаспоряженияНаОтгрузкуИВозврат",
		"Распоряжение = &ЗаявкаНаВозвратТоваровОтКлиента",
		Запрос.Параметры);
	...
	ПараметрыПоискаПоНоменклатуре = Новый Структура;
	ПараметрыПоискаПоНоменклатуреОприходования = Новый Структура;
	
	Пока Выборка.Следующий() Цикл
		
		ПараметрыПоискаПоНоменклатуре = Новый Структура;
		ПараметрыПоискаПоНоменклатуре.Вставить("Номенклатура", Выборка.Номенклатура);
		ПараметрыПоискаПоНоменклатуре.Вставить("Характеристика", Выборка.Характеристика);
		ПараметрыПоискаПоНоменклатуре.Вставить("Склад", Выборка.Склад);
		Если ВозвратПоПоступлению Тогда
			ПараметрыПоискаПоНоменклатуре.Вставить("Серия", Выборка.Серия);
		КонецЕсли;
		ПараметрыПоискаПоНоменклатуре.Вставить("Отменено", Ложь);
		ПараметрыПоискаПоНоменклатуре.Вставить("Порча", Ложь);
		
		СтрокиЗаказа = ТаблицаТовары.НайтиСтроки(ПараметрыПоискаПоНоменклатуре);
		
		ПараметрыПоискаПоНоменклатуреОприходования.Вставить("НоменклатураОприходование", Выборка.Номенклатура);
		ПараметрыПоискаПоНоменклатуреОприходования.Вставить("ХарактеристикаОприходование", Выборка.Характеристика);
		ПараметрыПоискаПоНоменклатуреОприходования.Вставить("Склад", Выборка.Склад);
		Если ВозвратПоПоступлению Тогда
			ПараметрыПоискаПоНоменклатуреОприходования.Вставить("Серия", Выборка.Серия);
		КонецЕсли;
		ПараметрыПоискаПоНоменклатуреОприходования.Вставить("Отменено", Ложь);
		ПараметрыПоискаПоНоменклатуреОприходования.Вставить("Порча", Истина);
		
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СтрокиЗаказа, ТаблицаТовары.НайтиСтроки(ПараметрыПоискаПоНоменклатуреОприходования));
		...
		Для Сч = 0 По СтрокиЗаказа.Количество()-1 Цикл
			...
			СтруктураПоискаОтклонений = Новый Структура;
			СтруктураПоискаОтклонений.Вставить("Номенклатура", Строка.Номенклатура);
			СтруктураПоискаОтклонений.Вставить("Характеристика", Строка.Характеристика);
			СтруктураПоискаОтклонений.Вставить("Склад", Строка.Склад);
			
			Если ВозвратПоПоступлению Тогда
				СтруктураПоискаОтклонений.Вставить("Серия", Строка.Серия);
			КонецЕсли;
			...
			КоэффициентУпаковки = ?(Строка.Количество > 0, Строка.Количество / Строка.КоличествоУпаковок, 1);
			...
			Если Отклонение > 0 
				...
				ИначеЕсли Строка.Количество > 0 Тогда
					...
					МинОтклонение = Мин(Отклонение, Строка.Количество);
					НовоеКоличество = Строка.Количество - МинОтклонение;
					
					Строка.Количество              = НовоеКоличество;
					Строка.КоличествоУпаковок      = НовоеКоличество / КоэффициентУпаковки;
					НоваяСтрока.Количество         = МинОтклонение;
					НоваяСтрока.КоличествоУпаковок = МинОтклонение / КоэффициентУпаковки;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ОтветственныеЛицаСервер.УстановитьМенеджера(
		ЭтотОбъект, ПродажиСервер.ДокументОснование(ЭтотОбъект, "ДокументРеализации, ВозвращаемыеТовары.ДокументРеализации"));
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение 
		...
		И Не ЗаполнятьЦеныЗаменяющихТоваровПоВозвращаемым
...
Процедура ПриЗаписи(Отказ)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтХранителя Тогда
		Движения.РаспоряженияНаОтгрузкуИВозврат.ДополнительныеСвойства.Вставить("ФормироватьСводнуюТаблицуЗаявкиНаВозврат");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для ТекИндекс = 0 По ВозвращаемыеТовары.Количество()-1 Цикл
		...
		Если ЗначениеЗаполнено(Склад) И НЕ ЗначениеЗаполнено(ТекущаяСтрокаВозвращаемыеТовары.Склад) Тогда
			
			ТекстОшибки = НСтр("ru = 'Не заполнена колонка ""Получатель"" в строке %НомерСтроки% списка ""%ИмяТЧ%""'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%",  ТекущаяСтрокаВозвращаемыеТовары.НомерСтроки);
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ИмяТЧ%",        НСтр("ru = 'Возвращаемые товары'"));
			
			ОбщегоНазначения.СообщитьПользователю(
				ТекстОшибки,
				ЭтотОбъект,
				ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ВозвращаемыеТовары", ТекущаяСтрокаВозвращаемыеТовары.НомерСтроки, "Склад"),
				,
				Отказ);
...
Процедура ЗаполнитьДокументНаОснованииРеализацииТоваровИУслуг(Знач ДокументОснование)
	...
	Запрос = Новый Запрос(
		...
		|			И ТоварыРеализации.Характеристика = ТоварыПредыдущихВозвратов.Характеристика
		|			И ТоварыРеализации.Серия = ТоварыПредыдущихВозвратов.Серия
		|			И ТоварыРеализации.Склад = ТоварыПредыдущихВозвратов.Склад
		|ГДЕ
		|	ТоварыРеализации.Количество - ЕСТЬNULL(ТоварыПредыдущихВозвратов.Количество, 0) > 0
...
Процедура ЗаполнитьДокументНаОснованииАктаПоРеализации(Знач ДанныеЗаполнения)
	...
	Если ВыборкаШапка.ХозяйственнаяОперацияРеализация = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию Тогда
...
Процедура ЗаполнитьДокументНаОснованииАктаПоПередаче(Знач ДанныеЗаполнения)
	...
	|				И ПередачаТоваров.Склад.ВыборГруппы = ЗНАЧЕНИЕ(Перечисление.ВыборГруппыСкладов.РазрешитьВЗаказах)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	...
	|ГДЕ
	|	ПередачаТоваров.Ссылка = &ДокументОснованиеИсправлениеОснования
	|;
	|
	|//////////////////////////////////////////////////////////////////////////////// 1
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	ТоварыДокумента.Ссылка = &ДокументОснованиеИсправлениеОснования
	|
	|СГРУППИРОВАТЬ ПО
	...
	ДокументРеализации = ДанныеЗаполнения.ОснованиеАкта;
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, ВыборкаШапка);
...
Процедура ЗаполнитьДокументНаОснованииПередачиТоваровХранителю(Знач ДанныеЗаполнения)
	...
	|	ТоварыПередачи.ТипНоменклатуры,
	|	ТоварыПередачи.Характеристика,
	|	ТоварыПередачи.Назначение,
	|	ТоварыПередачи.Склад,
	|	ТоварыПередачи.Упаковка,
	|	ТоварыПередачи.Серия";
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	Если ОбщегоНазначенияУТКлиентСервер.АвторизованВнешнийПользователь() Тогда
		...
		СкладыСервер.ЗаполнитьСкладыВТабличнойЧасти(Склад, СкладГруппа, ВозвращаемыеТовары, Ложь);
		
		СтруктураДействийВозвращаемые = Новый Структура();
		КонтрольКачестваКэш = КонтрольКачества.КэшДанныхМеханизма(ЭтотОбъект);
		СтруктураДействийВозвращаемые.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества",
			КонтрольКачестваКэш);
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ВозвращаемыеТовары, СтруктураДействийВозвращаемые);
	...
	КэшВозвращаемыеТовары = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
	КэшЗаменяющиеТовары = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
	
	Если Не ЗначениеЗаполнено(ДокументРеализации) Тогда
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ВозвращаемыеТовары, СтруктураДействий, КэшВозвращаемыеТовары);
	Конецесли;
		
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ЗаменяющиеТовары, СтруктураДействий, КэшЗаменяющиеТовары);

	СкладГруппа = Справочники.Склады.ЭтоГруппаИСкладыИспользуютсяВТЧДокументовПродажи(Склад);
	СкладыСервер.ЗаполнитьСкладыВТабличнойЧасти(Склад, СкладГруппа, ВозвращаемыеТовары, Ложь);
	
	СтруктураДействийЗаменяющие = Новый Структура;
	СтруктураПересчетаСуммы = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВТЧ(ЭтотОбъект);
	СтруктураДействийЗаменяющие.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
	СтруктураДействийЗаменяющие.Вставить("ПересчитатьСуммуСНДС", СтруктураПересчетаСуммы);

	СтруктураДействийВозвращаемые = Новый Структура;
	СтруктураПересчетаСуммы = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВТЧ(ЭтотОбъект);
	СтруктураДействийВозвращаемые.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
	СтруктураДействийВозвращаемые.Вставить("ПересчитатьСуммуСНДС", СтруктураПересчетаСуммы);
	
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		КэшВозвращаемыеТовары.ОбработанныеСтроки,
		СтруктураДействийВозвращаемые,
		ВозвращаемыеТовары,
		Неопределено);
		
	СтруктураДействийВозвращаемые = Новый Структура();
	КонтрольКачестваКэш = КонтрольКачества.КэшДанныхМеханизма(ЭтотОбъект);
	СтруктураДействийВозвращаемые.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", КонтрольКачестваКэш);
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ВозвращаемыеТовары, СтруктураДействийВозвращаемые);
	
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		КэшЗаменяющиеТовары.ОбработанныеСтроки,
		СтруктураДействийЗаменяющие,
		ЗаменяющиеТовары,
		Неопределено);
...
Процедура ИнициализироватьУсловияПродаж()
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьСоглашенияСКлиентами")
		Или ЗначениеЗаполнено(Партнер) И Не ЗначениеЗаполнено(Соглашение) Тогда
		ЗаполнитьУсловияПродажПоУмолчанию();
	КонецЕсли;
...
Процедура ЗаполнитьБанковскиеСчета()
	...
	Если ЗначениеЗаполнено(БанковскийСчет) И ЗначениеЗаполнено(БанковскийСчетКонтрагента) Тогда
		Возврат;
	КонецЕсли;
...
Функция ТекстЗапросаПоТоварамКПоступлению()
	...
	|			ТОГДА ВЫРАЗИТЬ(ТаблицаТовары.НоменклатураОприходование КАК Справочник.Номенклатура)
	|		ИНАЧЕ ВЫРАЗИТЬ(ТаблицаТовары.Номенклатура КАК Справочник.Номенклатура)
	|	КОНЕЦ КАК Номенклатура,
	|	ВЫБОР
	|		КОГДА ТаблицаТовары.Порча
	|			ТОГДА ВЫРАЗИТЬ(ТаблицаТовары.ХарактеристикаОприходование КАК Справочник.ХарактеристикиНоменклатуры)
	|		ИНАЧЕ ВЫРАЗИТЬ(ТаблицаТовары.Характеристика КАК Справочник.ХарактеристикиНоменклатуры)
	...
	|	СУММА(ТоварыКПоступлению.КОформлениюНакладныхПоРаспоряжениюПриход
	|		* &ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров) КАК ДопустимоеОтклонение,
	|	ТоварыКПоступлению.Склад          КАК Склад,
	|	ТоварыКПоступлению.Серия          КАК Серия
	|ИЗ
	|	РегистрНакопления.ТоварыКПоступлению.Обороты(&НачПериод,&КонПериод,,
	|		ДокументПоступления = &ЗаявкаНаВозвратТоваровОтКлиента) КАК ТоварыКПоступлению
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК ДанныеНоменклатуры
	|		ПО ДанныеНоменклатуры.Ссылка = ТоварыКПоступлению.Номенклатура
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК ДанныеЕдиницыИзмерения
	|		ПО ДанныеЕдиницыИзмерения.Ссылка = ДанныеНоменклатуры.ЕдиницаИзмерения
	|		И ДанныеЕдиницыИзмерения.ТипИзмеряемойВеличины В (&МерныеТипыЕдиницИзмерений)
	|			
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаТовары.Заявка                             КАК Заявка,
	|	ТаблицаТовары.Номенклатура                       КАК Номенклатура,
	|	ТаблицаТовары.Характеристика                     КАК Характеристика,
	|	ТаблицаТовары.Упаковка                           КАК Упаковка,
	|	ТаблицаТовары.Склад                              КАК Склад,
	...
	|ИЗ
	|	ТаблицаТовары КАК ТаблицаТовары
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаВозвратТоваровОтКлиента КАК ЗаявкаНаВозвратТоваровОтКлиента
	|		ПО ЗаявкаНаВозвратТоваровОтКлиента.Ссылка = ТаблицаТовары.Заявка
	|		И НЕ ЗаявкаНаВозвратТоваровОтКлиента.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.НеСогласована),
	|													ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.Отклонена))
	|
	|СГРУППИРОВАТЬ ПО 
	|	ТаблицаТовары.Заявка,
	|	ТаблицаТовары.Номенклатура,
	|	ТаблицаТовары.Характеристика,
	|	ТаблицаТовары.Упаковка,
	|	ТаблицаТовары.Склад,
	...
	|	Поступления.Серия            КАК Серия,
	|	СУММА(Поступления.Оформлено) КАК Оформлено,
	|	СУММА(Поступления.Принято)   КАК Принято,
	|	СУММА(Поступления.Заказано)  КАК Заказано,
	|	СУММА(Поступления.ПоОрдерам) КАК ПоОрдерам
	|
	|ПОМЕСТИТЬ ВтПринято
	|ИЗ (ВЫБРАТЬ
	|		Остатки.ДокументПоступления                             КАК Заказ,
	|		Остатки.Номенклатура                                    КАК Номенклатура,
	|		Остатки.Характеристика                                  КАК Характеристика,
	|		Остатки.КОформлениюНакладныхПоРаспоряжениюРасход        КАК Оформлено,
	|		0                                                       КАК Принято,
	|		Остатки.Склад                                           КАК Склад,
	|		Остатки.Серия                                           КАК Серия,
	|		0                                                       КАК Заказано,
	|		Остатки.КОформлениюПоступленийПоОрдерамОборот           КАК ПоОрдерам
	|	ИЗ
	|		РегистрНакопления.ТоварыКПоступлению.ОстаткиИОбороты(,,,,
	|			ДокументПоступления = &ЗаявкаНаВозвратТоваровОтКлиента) КАК Остатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ТаблицаТовары.Заявка                                 КАК Заказ,
	|		ТаблицаТовары.Номенклатура                           КАК Номенклатура,
	|		ТаблицаТовары.Характеристика                         КАК Характеристика,
	|		0                                                    КАК Оформлено,
	|		0                                                    КАК Принято,
	...
	|	КОНЕЦ - 
	|	ВЫБОР
	|		КОГДА Поступления.ПоОрдерам > 0
	|			ТОГДА Поступления.ПоОрдерам
	|		ИНАЧЕ 0
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаВозвратТоваровОтКлиента КАК ЗаявкаНаВозвратТоваровОтКлиента
	|		ПО ЗаявкаНаВозвратТоваровОтКлиента.Ссылка = ТаблицаТовары.Заявка
	|		И ЗаявкаНаВозвратТоваровОтКлиента.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.НеСогласована),
	|													ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.Отклонена))
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки
...
Функция ТекстЗапросаПоЗаявкамНаВозвратТоваровОтКлиентов()
	...
	|ИЗ
	|	#ЗаявкиНаВозвратОборотыРазвернутая1 КАК ТоварыКПоступлению
	...
	|		ПО ТоварыКПоступлению.Распоряжение = ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Ссылка
	...
	|ИЗ
	|	#ЗаявкиНаВозвратОборотыРазвернутая2 КАК ТоварыКПоступлению
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаВозвратТоваровОтКлиента КАК ЗаявкаНаВозвратТоваров
	|		ПО ТоварыКПоступлению.Распоряжение = ЗаявкаНаВозвратТоваров.Ссылка
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК ДанныеНоменклатуры
	|		ПО ДанныеНоменклатуры.Ссылка = ТоварыКПоступлению.Номенклатура
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК ДанныеЕдиницыИзмерения
	|		ПО ДанныеЕдиницыИзмерения.Ссылка = ДанныеНоменклатуры.ЕдиницаИзмерения
	...
	|		Остатки.Номенклатура                                    КАК Номенклатура,
	|		Остатки.Характеристика                                  КАК Характеристика,
	|		Остатки.ОформленоПоВозвратамКоличествоОборот            КАК Оформлено,
	|		0                                                       КАК Принято,
	|		ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Склад КАК Склад,
	|		0                                                       КАК Заказано
	|	ИЗ
	|		#ЗаявкиНаВозвратОборотыРазвернутая3 КАК Остатки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаВозвратТоваровОтКлиента.ВозвращаемыеТовары КАК ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары
	|			ПО Остатки.Распоряжение = ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Ссылка
	|				 И Остатки.КодСтроки = ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.КодСтроки
	|				
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|		0                                                    КАК Заказано
	|	ИЗ
	|		#ЗаявкиНаВозвратОборотыРазвернутая4 КАК Остатки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаВозвратТоваровОтКлиента КАК ЗаявкаНаВозвратТоваров
	|			ПО Остатки.Распоряжение = ЗаявкаНаВозвратТоваров.Ссылка
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	// ЗаявкиНаВозвратОборотыРазвернутая1
	...
	// ЗаявкиНаВозвратОборотыРазвернутая2
	ПараметрыФормированияТаблицы.ТекстУсловия =
		"Распоряжение = &ЗаявкаНаВозвратТоваровОтКлиента
		|	И КодСтроки = 0";
	...
	// ЗаявкиНаВозвратОборотыРазвернутая3
	ПараметрыФормированияТаблицы.НачалоПериода = "";
	ПараметрыФормированияТаблицы.КонецПериода = "";
	ПараметрыФормированияТаблицы.ТекстУсловия =
		"Распоряжение = &ЗаявкаНаВозвратТоваровОтКлиента
		|	И КодСтроки <> 0";
	ПараметрыФормированияТаблицы.Группировка = "Распоряжение, Номенклатура, Характеристика, КодСтроки";
	...
	// ЗаявкиНаВозвратОборотыРазвернутая4
	ПараметрыФормированияТаблицы.ТекстУсловия =
		"Распоряжение = &ЗаявкаНаВозвратТоваровОтКлиента
		|	И КодСтроки = 0";
	ПараметрыФормированияТаблицы.Группировка = "Распоряжение, Номенклатура, Характеристика";
