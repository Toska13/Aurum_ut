...
Процедура РазделУчетаПриИзменении(Элемент)
	...
	ОбновитьДанные(Объект.РазделУчета, Новый Структура("ИмяПоля", "РазделУчета"));
...
Процедура РазделУчетаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если (ЗначениеЗаполнено(ОКПД2) Или ЗначениеЗаполнено(Номенклатура))
		И ВыбранноеЗначение <> Объект.РазделУчета Тогда
		
		СтандартнаяОбработка = Ложь;
		ТекстВопроса = НСтр("ru = 'При изменении раздела учета подобранные партии
...
Процедура ОКПД2ОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	...
	СобытияФормЗерноКлиент.ОКПД2ОкончаниеВводаТекста(
		Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка, Объект.РазделУчета);
...
Процедура ОКПД2АвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	СобытияФормЗЕРНОКлиент.ОКПД2АвтоПодбор(
		Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка, Объект.РазделУчета);
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	ИнтеграцияЗЕРНОКлиентСервер.УстановитьПараметрыВыбораНоменклатуры(ЭтотОбъект, Объект.РазделУчета, "Номенклатура");
...
Процедура ОбновитьДанныеНаСервере(Результат, ДополнительныеПараметры, КэшированныеЗначения)
	...
	Если ИмяОбновляемогоПоля = "ПартииПартия"
		...
		ПараметрыОбработки.Вставить(
			...
			Объект.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна
