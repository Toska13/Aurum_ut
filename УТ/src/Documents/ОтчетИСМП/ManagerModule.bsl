...
Функция СтатусПослеПодготовкиКПередачеДанных(ДокументСсылка, Операция, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Операция = Перечисления.ВидыОперацийИСМП.ОтчетИСМПСписокКИНаБалансе
		Или Операция = Перечисления.ВидыОперацийИСМП.ОтчетИСМПСведенияОбОтклонениях
		Или Операция = Перечисления.ВидыОперацийИСМП.ОтчетИСМПОстаткиВиртуальногоСклада Тогда
...
Функция СтатусПослеПолученияДанных(ДокументСсылка, Операция, ДополнительныеПараметры) Экспорт
	...
	Если Операция = Перечисления.ВидыОперацийИСМП.ПолучениеРезультатаОбработкиДокумента Тогда
		...
		Если ДополнительныеПараметры.ОперацияКвитанции = Перечисления.ВидыОперацийИСМП.ОтчетИСМПСведенияОбОтклонениях Тогда
...
Функция СообщениеКПередачеJSON(ДокументСсылка, ДальнейшееДействие, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ДальнейшееДействие = Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ЗапроситеОтчет Тогда
		...
		Иначе
			...
			Если ДополнительныеПараметры.ПараметрыОбработкиДокумента.ДополнительныеПараметры.Свойство("ЗагрузкаОстатков") Тогда
				Возврат ЗагрузитьОстаткиПоВидамПродукцииJSON(ДокументСсылка, ДополнительныеПараметры);
			Иначе
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ОбновлениеИнформационнойБазыГосИС.ДобавитьОбработчикОбновленияПереходаНаКонструкторНастроек(
		...
		Новый УникальныйИдентификатор("32a14e17-c54f-4aa7-9fc4-4e11ba7a3852"));
...
// Обработка загрузки переданного документа.
// 
// Параметры:
...
//  РезультатОбработкиЗаявки - См. ИнтерфейсИСМПОбщегоНазначения.СтатусОбработкиДокументаПоИдентификатору.
//  ПараметрыОбновленияСтатуса - См. ОбменДаннымиИСМПКлиентСервер.ПараметрыОбновленияСтатуса.
Процедура ОбработкаЗагрузкиПереданногоДокумента(ЭлементОчереди, ПараметрыОбмена, РезультатОбработкиЗаявки, ПараметрыОбновленияСтатуса) Экспорт
	...
	Если ЗначениеЗаполнено(ЭлементОчереди.Документ) Тогда
		...
		ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ОтчетИСМПСписокКИНаБалансе
			Или ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ОтчетИСМПОстаткиВиртуальногоСклада Тогда
			...
			СоответствиеGTIN = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
				ПараметрыЗапроса, "СоответствиеGTIN", Неопределено);
			ЗагружатьНоменклатуруИзСервиса = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
				ПараметрыЗапроса, "ЗагружатьНоменклатуруИзСервиса", Ложь);
			
			Если СоответствиеGTIN = Неопределено Тогда
				
				Результат = ЗаполнитьНоменклатуруДокумента(ДокументОбъект, ЭлементОчереди, ЗагружатьНоменклатуруИзСервиса);
				Если Результат.ТребуетсяОбновлениеКлючаСессии Тогда
					ПараметрыОбновленияСтатуса.ТребуетсяОбновлениеКлючаСессии = Истина;
					Возврат;
	...
	Иначе
		...
		ЭтоЗагрузкаОстатков = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ПараметрыЗапроса, "ЭтоЗагрузкаОстатков", Ложь);
		...
		Если ЭтоЗагрузкаОстатков Тогда
			
			ДокументОбъект.Операция = ЭлементОчереди.Операция;
			Если ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ОтчетИСМПСписокКИНаБалансе Тогда
				ДокументОбъект.СтатусКодаМаркировки = ПараметрыЗапроса.СтатусКодаМаркировки;
			КонецЕсли;
			ДокументОбъект.Комментарий = НСтр("ru = 'Автоматическая загрузка остатков номенклатуры.'");
			...
			Если ДокументОбъект.КодыНаБалансе.Количество() Тогда
			...
			// При загрузке остатков сохранять пустой документ, чтобы не запрашивать повторно.
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	Представление = ИнтеграцияИСМПКлиентСервер.ПредставлениеДокументаОтчетИСМП(
...
// Формирование сообщения JSON по загрузке остатков для передачи в сервис ИС МП
//
// Параметры:
//  ДокументСсылка - ДокументСсылка.ОтчетИСМП - Ссылка на документ
...
// Возвращаемое значение:
//  Массив из см. ИнтеграцияИСМПСлужебный.СтруктураСообщенияJSON
Функция ЗагрузитьОстаткиПоВидамПродукцииJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	
	СообщенияJSON         = Новый Массив;
	
	ЗагружатьНоменклатуруИзСервиса = ИнтеграцияИС.УпрощеннаяИнтеграцияБЭД();
	ТоварныеГруппы = ДополнительныеПараметры.ПараметрыОбработкиДокумента.ДополнительныеПараметры.ТоварныеГруппы;
	Организация    = ДополнительныеПараметры.ПараметрыОбработкиДокумента.Организация;
	
	Если ТоварныеГруппы.Количество() Тогда
		
		Операция           = Перечисления.ВидыОперацийИСМП.ОтчетИСМПСписокКИНаБалансе;
		РеквизитыОрганизации = РаботаСКонтрагентамиИСВызовСервера.ИННКПППоОрганизацииКонтрагенту(Организация);
		СтатусКодаМаркировки = Перечисления.СтатусыКодовМаркировкиИСМП.ВведенВОборот;
		
		Для Каждого ТоварнаяГруппа Из ТоварныеГруппы Цикл
			
			Если Не КонструкторИСМПКлиентСервер.ВидПродукцииПоддерживаетЗагрузкуСпискаКИНаБалансе(ТоварнаяГруппа) Тогда
				Продолжить;
			КонецЕсли;
			
			СообщениеJSON = ЗаполнитьСообщениеJSON(
				Организация,
				ТоварнаяГруппа,
				Операция,
				ДокументСсылка);
			...
			ПараметрыJSON.Вставить("status", ИнтерфейсИСМПОбщегоНазначения.СтатусКодаМаркировкиИСМП(СтатусКодаМаркировки));
			
			ПараметрыОтбораВидУпаковки = Новый Массив;
			ПараметрыОтбораВидУпаковки.Добавить("UNIT");
			Если КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетГрупповыеУпаковки(ТоварнаяГруппа) Тогда
				ПараметрыОтбораВидУпаковки.Добавить("GROUP");
			КонецЕсли;
			Если КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетНаборы(ТоварнаяГруппа) Тогда
				ПараметрыОтбораВидУпаковки.Добавить("SET");
			КонецЕсли;
			
			ПараметрыJSON.Вставить("packageType", ПараметрыОтбораВидУпаковки);
			// параметры загрузки отчета
			...
			// параметры обработки загруженного отчета
			ПараметрыЗапроса.Вставить("СтатусКодаМаркировки",           СтатусКодаМаркировки);
			ПараметрыЗапроса.Вставить("ЗагружатьНоменклатуруИзСервиса", ЗагружатьНоменклатуруИзСервиса);
			ПараметрыЗапроса.Вставить("ЭтоЗагрузкаОстатков",            Истина);
	...
	Если ДополнительныеПараметры.ПараметрыОбработкиДокумента.ДополнительныеПараметры.Свойство("ТоварныеГруппыОСУ")
		И ДополнительныеПараметры.ПараметрыОбработкиДокумента.ДополнительныеПараметры.ТоварныеГруппыОСУ.Количество() > 0 Тогда
		...
		СообщениеJSON = ЗаполнитьСообщениеJSON(
			Организация,
			ДополнительныеПараметры.ПараметрыОбработкиДокумента.ДополнительныеПараметры.ТоварныеГруппыОСУ[0],
			Перечисления.ВидыОперацийИСМП.ОтчетИСМПОстаткиВиртуальногоСклада,
			ДокументСсылка);
...
Функция ЗагрузитьОтклоненияПоВидамПродукцииJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	ДоступныеТоварныеГруппы = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступныеТоварныеГруппы();
	...
	Для Каждого Организация Из Организации Цикл
		...
		Для Каждого ТоварнаяГруппа Из ДоступныеТоварныеГруппы Цикл
			...
			Если Не КонструкторИСМПКлиентСервер.ВидПродукцииПоддерживаетЗагрузкуСведенийОбОтклонениях(ТоварнаяГруппа) Тогда
				Продолжить;
			КонецЕсли;
			...
			ДатаСинхронизации = ДатыСинхронизации.Получить(ТоварнаяГруппа);
			Сутки = 86400;
...
Функция ЗапроситеОтчетJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	Если Шапка.Операция = Перечисления.ВидыОперацийИСМП.ОтчетИСМПСведенияОбОтклонениях Тогда
		...
		Если Не КонструкторИСМПКлиентСервер.ВидПродукцииПоддерживаетЗагрузкуСведенийОбОтклонениях(Шапка.ТоварнаяГруппа) Тогда
			СообщениеJSON.ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Товарная группа ""%1"" не поддерживает загрузку сведений об отклонениях.'"),
	...
	ИначеЕсли Шапка.Операция = Перечисления.ВидыОперацийИСМП.ОтчетИСМПСписокКИНаБалансе Тогда
		...
		Если Не КонструкторИСМПКлиентСервер.ВидПродукцииПоддерживаетЗагрузкуСпискаКИНаБалансе(Шапка.ТоварнаяГруппа) Тогда
			СообщениеJSON.ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Товарная группа ""%1"" не поддерживает загрузку списка кодов маркировки на балансе.'"),
				Шапка.ТоварнаяГруппа);
			СообщениеJSON.ТребуетсяПодписание = Ложь;
			СообщенияJSON.Добавить(СообщениеJSON);
			Возврат СообщенияJSON;
		...
		Если Товары.Количество() Тогда
			...
			ШтрихкодированиеОбщегоНазначенияИС.ЗаполнитьПроверяемыеGTIN(Товары, МассивGTIN, СоответствиеGTIN, Истина);
		...
		ПараметрыОтбораВидУпаковки = Новый Массив;
		ВидыПродукции = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(Шапка.ТоварнаяГруппа);
		
		Если ВидыУпаковок.Количество() = 0 Тогда			
			
			ПараметрыОтбораВидУпаковки.Добавить("UNIT");
			Если КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетГрупповыеУпаковки(ВидыПродукции) Тогда
				ПараметрыОтбораВидУпаковки.Добавить("GROUP");
			КонецЕсли;
			Если КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетНаборы(ВидыПродукции) Тогда
				ПараметрыОтбораВидУпаковки.Добавить("SET");
			КонецЕсли;
		...
		Иначе
			...
			Для Каждого ВидУпаковки Из ВидыУпаковок Цикл
				Если ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая Тогда
					ПараметрыОтбораВидУпаковки.Добавить("BOX");
				Иначе
					ПараметрыОтбораВидУпаковки.Добавить(ИнтерфейсИСМПОбщегоНазначения.ВидУпаковки(ВидУпаковки, ВидыПродукции));
				КонецЕсли;
			КонецЦикла;
	...
	ИначеЕсли Шапка.Операция = Перечисления.ВидыОперацийИСМП.ОтчетИСМПОстаткиВиртуальногоСклада Тогда
...
Функция ЗаполнитьНоменклатуруДокумента(ДокументОбъект, ЭлементОчереди, ЗагружатьНоменклатуруИзСервиса)
	...
	Для Каждого СтрокаТовары Из ДокументОбъект.КодыНаБалансе Цикл
		Если ЗначениеЗаполнено(СтрокаТовары.GTIN) Тогда
			ШтрихкодыEAN.Добавить(СтрокаТовары.GTIN);
		КонецЕсли;
	КонецЦикла;
	...
	ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(ДокументОбъект);
	ДанныеПоШтрихкодамEANРезультат = ШтрихкодированиеОбщегоНазначенияИС.ДанныеПоШтрихкодамEAN(
		ШтрихкодыEAN, ПараметрыСканирования,, ЗагружатьНоменклатуруИзСервиса);
	...
	ИначеЕсли ЗначениеЗаполнено(ДанныеПоШтрихкодамEANРезультат.ТекстОшибки) Тогда
		Результат.ТекстОшибки = ДанныеПоШтрихкодамEANРезультат.ТекстОшибки;
		Возврат Результат;
	КонецЕсли;
...
Функция ЗаполнитьСообщениеJSON(Организация, ТоварнаяГруппа,  Операция, ДокументСсылка)
