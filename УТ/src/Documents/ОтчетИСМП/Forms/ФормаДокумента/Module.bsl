...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.Операция <> Перечисления.ВидыОперацийИСМП.ОтчетИСМПСведенияОбОтклонениях Тогда
		...
		МассивНепроверяемыхРеквизитов.Добавить("ПериодДатаВыгрузки");
...
Процедура ТоварнаяГруппаНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	ЭлементКонструктораДляОтбора = Неопределено;
	Если Объект.Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийИСМП.ОтчетИСМПСведенияОбОтклонениях") Тогда
		ЭлементКонструктораДляОтбора = ПредопределенноеЗначение(
			"Справочник.ЭлементыКонструктораВидовПродукцииИС.ОперацияЗагрузкаСведенийОбОтклонениях");
	ИначеЕсли  Объект.Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийИСМП.ОтчетИСМПСписокКИНаБалансе") Тогда
		ЭлементКонструктораДляОтбора = ПредопределенноеЗначение(
			"Справочник.ЭлементыКонструктораВидовПродукцииИС.ОперацияЗагрузкаСпискаКИНаБалансе");
	КонецЕсли;
	
	Если ЭлементКонструктораДляОтбора <> Неопределено Тогда
		
		СтандартнаяОбработка = Ложь;
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ЭлементКонструктора", ЭлементКонструктораДляОтбора);
...
Процедура ТоварнаяГруппаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ИначеЕсли Объект.Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийИСМП.ОтчетИСМПСписокКИНаБалансе")
...
Процедура ОперацияПриИзменении(Элемент)
	...
	УстановитьПараметрыВыбораНоменклатуры(ЭтотОбъект, Объект.ТоварнаяГруппа, Объект.Операция);
...
Процедура КодыНаБалансеНоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ВидыПродукции = ВидыПродукцииДокументаПоВидуОперации(Объект.Операция, Объект.ТоварнаяГруппа);
	СобытияФормИСКлиентПереопределяемый.ПриНачалеВыбораНоменклатуры(Элемент, ВидыПродукции, СтандартнаяОбработка);
...
Процедура НастроитьЭлементыФормы(Форма)
	...
	Если Форма.Объект.Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийИСМП.ОтчетИСМПСписокКИНаБалансе")
		Или Форма.Объект.Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийИСМП.ОтчетИСМПОстаткиВиртуальногоСклада") Тогда
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если Инициализация Или СтруктураРеквизитов.Свойство("Операция") Тогда
		...
		ЭтоОстаткиКИ  = Объект.Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийИСМП.ОтчетИСМПСписокКИНаБалансе");
		
		// Шапка
		Элементы.ГруппаСведенияОбОтклонениях.Видимость    = ЭтоСведенияОбОтклонениях;
		Элементы.ГруппаОтборСтрокКодыНаБалансе.Видимость  = ЭтоОстаткиКИ;
		Элементы.ГруппаСведенияКИНаБалансе.Видимость      = ЭтоОстаткиКИ;
		
		// Страницы
		Элементы.СтраницаСведенияОбОтклонениях.Видимость = ЭтоСведенияОбОтклонениях;
		Элементы.СтраницаТовары.Видимость                = ЭтоОстаткиКИ;
		Элементы.СтраницаКодыНаБалансе.Видимость         = Не ЭтоСведенияОбОтклонениях;
		
		// Таблица КодыНаБалансе
		Элементы.КодыНаБалансеСтатусКодаМаркировки.Видимость = ЭтоОстаткиКИ;
		Элементы.КодыНаБалансеВидУпаковки.Видимость          = ЭтоОстаткиКИ;
		Элементы.КодыНаБалансеСпособВводаВОборот.Видимость   = ЭтоОстаткиКИ;
		
		Элементы.ТоварнаяГруппа.Видимость = ЭтоСведенияОбОтклонениях Или ЭтоОстаткиКИ;
		
		Если Форма.РежимОткрытияПоВидуОперации = ИнтеграцияИСМПКлиентСервер.РежимОткрытияСведенияОбОтклонениях()
			Или Форма.РежимОткрытияПоВидуОперации = ИнтеграцияИСМПКлиентСервер.РежимОткрытияСписокКИНаБалансе() Тогда
...
Процедура ЗаполнитьДоступныеОперации()
	...
	Если РежимОткрытияПоВидуОперации = ИнтеграцияИСМПКлиентСервер.РежимОткрытияОстаткиКодов() Тогда
		Элементы.Операция.СписокВыбора.Очистить();
		Элементы.Операция.СписокВыбора.Добавить(Перечисления.ВидыОперацийИСМП.ОтчетИСМПСписокКИНаБалансе);
		Элементы.Операция.СписокВыбора.Добавить(Перечисления.ВидыОперацийИСМП.ОтчетИСМПОстаткиВиртуальногоСклада);
	КонецЕсли;
...
Процедура ЗаполнитьДоступныеВидыУпаковок()
	...
	Если КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетГрупповыеУпаковки(Объект.ТоварнаяГруппа) Тогда
		Элементы.ВидУпаковки.СписокВыбора.Добавить(
			Перечисления.ВидыУпаковокИС.Групповая);
	КонецЕсли;
	Если КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетНаборы(Объект.ТоварнаяГруппа) Тогда
		Элементы.ВидУпаковки.СписокВыбора.Добавить(
...
Функция ДанныеФайлаВыгрузкиПоДокументуДляСохранения()
	...
	Для Каждого ЭлементКоллекции Из ДанныеПротокола Цикл
		...
		ДанныеФайла = РаботаСФайлами.ДанныеФайла(ЭлементКоллекции.Ссылка, ПараметрыДанныхФайла);
		Возврат ДанныеФайла;
	КонецЦикла;
...
&НаКлиенте
Процедура ТоварнаяГруппаПриИзмененииНаКлиенте()
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьПараметрыВыбораНоменклатуры(Форма, ТоварнаяГруппа, Операция)
	
	ВидыПродукции = ВидыПродукцииДокументаПоВидуОперации(Операция, ТоварнаяГруппа);
...
&НаКлиентеНаСервереБезКонтекста
Функция ВидыПродукцииДокументаПоВидуОперации(Операция, ТоварнаяГруппа)
	
	Если Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийИСМП.ОтчетИСМПОстаткиВиртуальногоСклада") Тогда
		ВидыПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииОбъемноСортовогоУчета();
	Иначе
...
Процедура УстановитьЗаголовокДокумента(Форма, ЭтоНовый)
	...
	Форма.Заголовок = ИнтеграцияИСМПКлиентСервер.ПредставлениеДокументаОтчетИСМП(
		ЭтоНовый,
		Объект.Операция,
		Объект.Номер,
		Объект.Дата);
