...
Функция РаспоряжениеНаВыполнениеСкладскойОперации(ЗначенияПолейДляОпределенияРаспоряжения) Экспорт
	...
	ВариантПриемкиУслуг = ЗначенияПолейДляОпределенияРаспоряжения.ВариантПриемкиУслуг;
	...
	ИначеЕсли Справочники.СоглашенияСПоставщиками.СоглашениеИспользуетсяПриПриемке(ВариантПриемкиУслуг) Тогда
		Возврат ЗначенияПолейДляОпределенияРаспоряжения.Соглашение;
	ИначеЕсли Справочники.ДоговорыКонтрагентов.ДоговорИспользуетсяПриПриемке(ВариантПриемкиУслуг) Тогда
...
Функция МакетФормыВыбораНазначений() Экспорт
	...
	ОписаниеКолонок = Справочники.Назначения.ДобавитьОписаниеКолонок(МакетФормы, "ОбеспечениеЗаказовРаботами", Истина, "Объект.Услуги.Назначение");
...
//
Функция ПараметрыВыбораСтатейИАналитик(ХозяйственнаяОперация, Договор) Экспорт
	...
	ПараметрыВыбора.ОтборСтатейРасходов.ВариантРаспределенияРасходов = Неопределено;
	...
	Если ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется Тогда
		СписокВариантовРаспределенияРасходов = Новый Массив;
		
		Для каждого ЗначениеПеречисления Из Перечисления.ВариантыРаспределенияРасходов Цикл
			Если ЗначениеПеречисления = Перечисления.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы Тогда
				Продолжить;
			КонецЕсли;
			
			СписокВариантовРаспределенияРасходов.Добавить(ЗначениеПеречисления);
		КонецЦикла;
		
		ПараметрыВыбора.ОтборСтатейРасходов.ВариантРаспределенияРасходов = СписокВариантовРаспределенияРасходов;
	КонецЕсли;
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	ВариантПриемкиУслуг = Реквизиты.ВариантПриемкиУслуг;
...
Процедура ОформитьПоступлениеТоваровПоДвухходовке(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ИсточникДанных.Ссылка.ВариантПриемкиУслуг КАК ВариантПриемкиУслуг,
	...
	|	И НЕ ИсточникДанных.Ссылка.Договор.ВариантОформленияДокументовПоступленияПоУслугам = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется)
	|
	|";
	...
	ПараметрыМетода.ЭтоПоступлениеУслугВПодразделение = Истина;
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	РаспределениеЗапасовДвижения.ПриходЗапаса(Запрос, ТекстыЗапроса, Регистры, ТекстЗапросаТабЧасть, Истина, Ложь);
...
Функция ТекстЗапросаВтТаблицаАналитикУчетаПартий(Запрос, ТекстыЗапроса)
	...
	ТекстНомераСтрок = ТекстЗапросаКодыСтрокДляАналитикиУчетаПартий();
...
Функция ТекстЗапросаКодыСтрокДляАналитикиУчетаПартий()
	
	// Запросы получения кодов строк должны быть аналогичны запросам 
	// функции РасчетСебестоимостиПроведениеДокументов.ТекстЗапросаКодыСтрокДляАналитикиУчетаПартий()
	// с учетом особенностей документа.  
	...
	|ИЗ
	|	Документ.ПоступлениеУслугВПодразделение.Услуги КАК Т
	|ГДЕ
	|	Т.Ссылка = &Ссылка
	|СГРУППИРОВАТЬ ПО
	|	Т.Номенклатура,
	...
	// но разными назначениями, подразделениями, статьями расходов или аналитиками расходов.
	// Для всех строк с одинаковой номенклатурой, характеристикой, подразделением, назначением, статьей и аналитикой заполняется один
	...
	// Выделение отдельных партий необходимо для корректного отражения отклонений при фактуровке поставки. 
	...
	|		ПО Т.Номенклатура = Итоги.Номенклатура
	|		И Т.Характеристика = Итоги.Характеристика
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТНоменклатураДокумента КАК ИтогиДетально
	|		ПО Т.Номенклатура = ИтогиДетально.Номенклатура
	|		И Т.Характеристика = ИтогиДетально.Характеристика
	|		И (Т.Назначение = ИтогиДетально.Назначение
	|			ИЛИ НЕ &УчитыватьСебестоимостьТоваровПоНазначениям)
	|		И Т.Подразделение = ИтогиДетально.Подразделение
	|		И Т.СтатьяРасходов = ИтогиДетально.СтатьяРасходов
	|		И Т.АналитикаРасходов = ИтогиДетально.АналитикаРасходов
	|ГДЕ
	|	Т.Ссылка = &Ссылка
	|";
	
	ТекстЗапроса = ШаблонЗапросаВыборки + "
		|;
		|" + ТекстЗапросаВыборкиСводно + "
		|;
		|" + ШаблонЗапросаРезультата + "
...
Функция ТекстЗапросаВтТаблицаУслуги(Запрос, ТекстыЗапроса)
	...
	|	&НаправлениеДеятельности КАК НаправлениеДеятельности,
	|
	|	ТаблицаУслугиРаботы.Ссылка.Договор.ВариантОформленияДокументовПоступленияПоУслугам КАК ВариантОформленияДокументовПоступленияПоУслугам,
	|	ТаблицаУслугиРаботы.Ссылка.ВидДеятельностиНДС КАК ВидДеятельностиНДСДокумента,
	...
	|	ВЫРАЗИТЬ((ТаблицаУслугиРаботы.СуммаСНДС - ТаблицаУслугиРаботы.СуммаНДС) * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(31, 2)) КАК СуммаБезНДСУпр,	
	|	ВЫРАЗИТЬ(ТаблицаУслугиРаботы.СуммаСНДС * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31,2)) КАК СуммаСНДСРегл,
	|	ВЫРАЗИТЬ((ТаблицаУслугиРаботы.СуммаСНДС - ТаблицаУслугиРаботы.СуммаНДС)	* &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31,2)) КАК СуммаБезНДСРегл,	
	...
	|		ТОГДА 0
	|		ИНАЧЕ ВЫБОР
	|			КОГДА ТаблицаУслугиРаботы.Ссылка.Договор.ВариантОформленияДокументовПоступленияПоУслугам
	|				= ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется) 
	|			ТОГДА ВЫБОР
	|						КОГДА ТаблицаАналитикУчетаПартий.НалогообложениеНДС в (&ТипыНалогообложенияНДСУчитываетсяВСтоимости)
	|							ТОГДА ВЫРАЗИТЬ(ТаблицаУслугиРаботы.СуммаСНДС * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(31, 2))
	|							ИНАЧЕ ВЫРАЗИТЬ((ТаблицаУслугиРаботы.СуммаСНДС - ТаблицаУслугиРаботы.СуммаНДС) * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31, 2))
	|					КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА (&ПорядокОтраженияНеотфактурованныхПоставокНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыОтраженияНеотфактурованныхПоставокНУ.ОтражатьПриФактуровке)
	|			ИЛИ ТаблицаУслугиРаботы.Ссылка.Договор.ВариантОформленияДокументовПоступленияПоУслугам
	|				= ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется))
	|		 И (НЕ ТаблицаУслугиРаботы.СписатьНаРасходы	ИЛИ ЕСТЬNULL(ТаблицаУслугиРаботы.СтатьяРасходов.ПринятиеКНалоговомуУчету, ЛОЖЬ))
	// Если в документе указана статья расходов, не принимаемая к налоговому учету по налогу на прибыль, то должна быть отражена постоянная разница.
...
Функция ТекстЗапросаТаблицаМатериалыИРаботыВПроизводстве(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ИЛИ ТаблицаУслуги.Назначение.ТипНазначения <> ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.ПоставкаПодПринципала))
	|";
...
Функция ТекстЗапросаТаблицаНДСПредъявленный(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаДокумента.Договор.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	СУММА(ВтТаблицаУслуги.СуммаБезНДСРегл) КАК СуммаБезНДС,
	|	СУММА(ВтТаблицаУслуги.СуммаНДСРегл) КАК НДС,
	|	СУММА(ВтТаблицаУслуги.СуммаНДСУпр) КАК НДСУпр,
	...
	|	ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ЗакупкаУПоставщикаНеотфактурованнаяПоставка) КАК
	...
	|ГДЕ
	|	ТаблицаДокумента.Ссылка = &Ссылка
	|	И ТаблицаДокумента.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
	|	И НЕ ТаблицаДокумента.ВидДеятельностиНДС В (&ТипыНалогообложенияНДСУчитываетсяВСтоимости)
	|	И ТаблицаДокумента.Договор.ВариантОформленияДокументовПоступленияПоУслугам = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется)
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаДокумента.Ссылка,
	|	ТаблицаДокумента.Дата,
	|	ТаблицаДокумента.Организация,
	|	ТаблицаДокумента.Контрагент,
	|	ВтТаблицаУслуги.СтавкаНДС,
	|	ТаблицаДокумента.Договор.НаправлениеДеятельности,
	...
	|		ИНАЧЕ ТаблицаДокумента.Ссылка
	|	КОНЕЦ,
	|	ВтТаблицаУслуги.ИдентификаторСтроки,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.НеотфактурованныйНДС),
	|	ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС),
	|	ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ЗакупкаУПоставщикаНеотфактурованнаяПоставка)";
...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ВтТаблицаУслуги.ВариантОформленияДокументовПоступленияПоУслугам 
	|			В (ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется))
	|		ТОГДА
	|			ВЫБОР
	|				КОГДА ВтТаблицаУслуги.ВидДеятельностиНДС В (&ТипыНалогообложенияНДСУчитываетсяВСтоимости)
	|			ТОГДА
	|				ВтТаблицаУслуги.СуммаСНДСУпр
	|			ИНАЧЕ
	|				ВтТаблицаУслуги.СуммаБезНДСУпр
	|			КОНЕЦ
	|		ИНАЧЕ 
	|			ВтТаблицаУслуги.СуммаБезНДСУпр
	|		КОНЕЦ
	...
	|				ИНАЧЕ 
	|					ВЫБОР
	|						КОГДА ВтТаблицаУслуги.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОпределяетсяРаспределением) 
	|							И ВтТаблицаУслуги.ВидДеятельностиНДСДокумента В (&ТипыНалогообложенияНДСУчитываетсяВСтоимости)
	|							ТОГДА ВтТаблицаУслуги.СуммаБезНДСРегл
	|						ИНАЧЕ ВтТаблицаУслуги.СуммаСНДСРегл
	|					КОНЕЦ
	|			КОНЕЦ
	|		ИНАЧЕ 
	|			ВЫБОР
	...
	|					ТОГДА ВтТаблицаУслуги.СуммаСНДСРегл
	|				ИНАЧЕ 
	|					ВтТаблицаУслуги.СуммаБезНДСРегл 
	|			КОНЕЦ
	...
	|						КОГДА ВтТаблицаУслуги.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОпределяетсяРаспределением) 
	|							И ВтТаблицаУслуги.ВидДеятельностиНДСДокумента В (&ТипыНалогообложенияНДСУчитываетсяВСтоимости)
	|							ТОГДА ВтТаблицаУслуги.СуммаБезНДСУпр
	|						ИНАЧЕ ВтТаблицаУслуги.СуммаСНДСУпр
	|					КОНЕЦ
	|			КОНЕЦ
	|		ИНАЧЕ 
	|			ВЫБОР
	...
	|	0 												КАК ДопРасходыУпр,
	// По 60.НП и в СебестоимостиТоваров никогда не должна формироваться ПР.
	// В зависимости от флага ПорядокОтраженияНеотфактурованныхПоставокНУ в договоре либо НУ, либо ВР.
	|	0												КАК ПостояннаяРазница,
	|	(ВЫБОР
	|		КОГДА ВтТаблицаУслуги.ВидДеятельностиНДС В (&ТипыНалогообложенияНДСУчитываетсяВСтоимости)
