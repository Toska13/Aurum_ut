...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетПрочихДоходовРасходов") Тогда
		ПараметрыВыбораСтатейИАналитик = Документы.ПоступлениеУслугВПодразделение.ПараметрыВыбораСтатейИАналитик(ХозяйственнаяОперация, Договор);
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") И ДанныеЗаполнения.Свойство("ДатаДокумента") Тогда
	...
	Если ТипДанныхЗаполнения = Тип("Структура")
		...
		Если ТипЗначенияОснования = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
	 		ДанныеЗаполненияДоговор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения.ДокументОснование, "Договор");
			
 			Если ОснованиеДоговор(ДанныеЗаполненияДоговор) Тогда
				ЗаполнитьДокументНаОснованииДоговора(ДанныеЗаполненияДоговор, ХозяйственнаяОперацияПриемки);
			Иначе
	...
	ИначеЕсли ТипДанныхЗаполнения = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
 		...
 		ДанныеЗаполненияДоговор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения, "Договор");
 		Если ОснованиеДоговор(ДанныеЗаполненияДоговор) Тогда
			ЗаполнитьДокументНаОснованииДоговора(ДанныеЗаполненияДоговор);
		Иначе
	...
	ОтветственныеЛицаСервер.УстановитьМенеджера(ЭтотОбъект, Распоряжение);
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	ПараметрыЗаполненияСтавкиНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(ЭтотОбъект);
...
Процедура ЗаполнитьДокументНаОснованииЗаказаПоставщику(ДокументОснование, ЗаполнятьПоНакладной = Ложь, ХозяйственнаяОперацияПриемки = Неопределено)
	...
	|	СУММА(ВЫБОР
	|			КОГДА ВЫРАЗИТЬ(УслугиКПоступлению.ДокументПоступления КАК Документ.ЗаказПоставщику).Договор.ВариантОформленияДокументовПоступленияПоУслугам = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется)
	|				И НЕ ВЫРАЗИТЬ(УслугиКПоступлению.ДокументПоступления КАК Документ.ЗаказПоставщику).Договор.ВариантПриемкиУслуг В (
	...
	|				ТОГДА УслугиКПоступлению.КОформлениюНакладныхПоРаспоряжениюОстаток
	|			ИНАЧЕ УслугиКПоступлению.КОформлениюПоступленийПоРаспоряжениюОстаток
	...
	|								ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПоДоговорамБезЗаказовИНакладных),
	|								ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПоДоговорамПослеЗаказовИлиНакладных))
	|				ТОГДА УслугиКПоступлению.КОформлениюНакладныхПоРаспоряжениюОстаток > 0
	|			ИНАЧЕ УслугиКПоступлению.КОформлениюПоступленийПоРаспоряжениюОстаток > 0
	|		КОНЕЦ
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	УслугиКПоступлению.Характеристика,
	|	УслугиКПоступлению.Назначение,
	|	СУММА(УслугиКПоступлению.КОформлениюПоступленийПоНакладнымОстаток)
	|ИЗ
	|	РегистрНакопления.ТоварыКПоступлению.Остатки(
	|			,
	|			ДокументПоступления = &ДокументОснование
	...
	|	СУММА(ЗаказПоставщикуТовары.КоличествоУпаковок)
	|ИЗ
	|	Документ.ЗаказПоставщику.Товары КАК ЗаказПоставщикуТовары
	|ГДЕ
	|	ЗаказПоставщикуТовары.Ссылка = &ДокументОснование
	|	И ЗаказПоставщикуТовары.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))
	|	И ЗаказПоставщикуТовары.Ссылка.Договор.ВариантОформленияДокументовПоступленияПоУслугам = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется)
	|	И НЕ ЗаказПоставщикуТовары.Ссылка.Договор.ВариантПриемкиУслуг В (
	|		ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПоДоговорамБезЗаказовИНакладных),
	|		ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПоДоговорамПослеЗаказовИлиНакладных))
	|
	|СГРУППИРОВАТЬ ПО
	...
	|			ПО ПоступлениеУслугВПодразделение.Ссылка = РеестрДокументовСторно.СторнируемыйДокумент
	|				И РеестрДокументовСторно.ТипСсылки = &ТипСсылки
	|				И РеестрДокументовСторно.Проведен
	|				И НЕ РеестрДокументовСторно.ДополнительнаяЗапись
	|				И НЕ ТИПЗНАЧЕНИЯ(РеестрДокументовСторно.Ссылка)= ТИП(Документ.Сторно)
	|	ГДЕ
	...
	|			ИНАЧЕ 0
	|		КОНЕЦ КАК Цена,
	|		УслугиКПоступлению.СтавкаНДС КАК СтавкаНДС,
	|		УслугиКПоступлению.НомерСтроки КАК НомерСтроки
	|	ИЗ
	|		ВТЦеныТоваров КАК УслугиКПоступлению
	...
	Запрос.УстановитьПараметр("ТипСсылки", ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Документ.ПоступлениеУслугВПодразделение"));
...
Процедура ЗаполнитьДокументНаОснованииДоговора(ДокументОснование, ХозяйственнаяОперацияПриемки = Неопределено)
	...
	|	ВЫБОР
	|		КОГДА Договор.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщика)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаНеотфактурованнаяПоставка)
	|		КОГДА Договор.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСНеотфактурованнаяПоставка)
	|		КОГДА Договор.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаПоИмпорту)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаНеотфактурованнаяПоставка)
	|		КОГДА Договор.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаТоварыВПути)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаНеотфактурованнаяПоставка)
	|		КОГДА Договор.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаПоИмпортуТоварыВПути)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаНеотфактурованнаяПоставка)
	|		КОГДА Договор.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСТоварыВПути)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСНеотфактурованнаяПоставка)
	|		ИНАЧЕ Договор.ХозяйственнаяОперация
	|	КОНЕЦ КАК ХозяйственнаяОперацияПриемки
	|ПОМЕСТИТЬ ВТРаспоряжение
	|ИЗ
	...
	|	И Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))
	|	И ВЫРАЗИТЬ(ДокументПоступления КАК
	|		Справочник.ДоговорыКонтрагентов).ВариантОформленияДокументовПоступленияПоУслугам = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется)) КАК
	|		УслугиКПоступлению
	|ГДЕ
	...
	|	ЗаказыПоставщикамОстатки.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
	|ИЗ
	|	РегистрНакопления.ЗаказыПоставщикам.Остатки(, ВЫРАЗИТЬ(ЗаказПоставщику КАК
	|		Документ.ЗаказПоставщику).Договор = &ДокументОснование
	|	И Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))
	|	И ВЫРАЗИТЬ(ЗаказПоставщику КАК Документ.ЗаказПоставщику).ХозяйственнаяОперация В
	...
	|	И ВЫРАЗИТЬ(ЗаказПоставщику КАК
	|		Документ.ЗаказПоставщику).Договор.ВариантОформленияДокументовПоступленияПоУслугам = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется)) КАК
	...
	|	ТаблицаРаспоряжение.ЕстьОшибкиСтатус КАК ЕстьОшибкиСтатус,
	|	ТаблицаРаспоряжение.ХозяйственнаяОперацияПриемки КАК ХозяйственнаяОперацияПриемки,
	|	ТаблицаРаспоряжение.Договор.ВариантОформленияДокументовПоступленияПоУслугам КАК
	|		ВариантОформленияДокументовПоступленияПоУслугам,
	|	ТаблицаРаспоряжение.Договор.ВариантПриемкиУслуг КАК ВариантПриемкиУслуг
	|ИЗ
	|	ВТРаспоряжение КАК ТаблицаРаспоряжение
	|;
	|
	...
	|	ТаблицаУслуги.Назначение,
	|	ТаблицаУслуги.СписатьНаРасходы
	|ИМЕЮЩИЕ
	...
	ПоступлениеУслугНеИспользуется = Не ЗначениеЗаполнено(Выборка.ВариантОформленияДокументовПоступленияПоУслугам)
		Или Выборка.ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугНеИспользуется;
	
	Если ПоступлениеУслугНеИспользуется Или ПоступлениеУслугИспользуетсяПоДоговорам(Выборка) Тогда
		Распоряжение = ДокументОснование;
	КонецЕсли;
...
Процедура ПроверитьВариантОформленияЗакупокДоговора(ДокументОснование)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ДоговорыКонтрагентов.ВариантОформленияДокументовПоступленияПоУслугам КАК ВариантОформленияДокументовПоступленияПоУслугам
	|ИЗ
	...
	Если Выборка.Следующий()
		И (Не ЗначениеЗаполнено(Выборка.ВариантОформленияДокументовПоступленияПоУслугам) 
		Или Выборка.ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугНеИспользуется) Тогда
		
		ВызватьИсключение НСтр("ru = 'Поступление услуг в подразделение можно создавать только если в договоре основания включена схема ""Раздельная закупка для работ и услуг"".'");
...
Функция ОснованиеДоговор(Договор)
	
	РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Договор, 
		"ВариантОформленияДокументовПоступленияПоУслугам, ВариантПриемкиУслуг");
	
	Возврат ПоступлениеУслугИспользуетсяПоДоговорам(РеквизитыДоговора);
	
КонецФункции

Функция ПоступлениеУслугИспользуетсяПоДоговорам(РеквизитыДоговора)
	
	Если (РеквизитыДоговора.ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется
		...
		И (РеквизитыДоговора.ВариантПриемкиУслуг = Перечисления.ВариантыПриемкиТоваров.ПоДоговорамБезЗаказовИНакладных 
		Или РеквизитыДоговора.ВариантПриемкиУслуг = Перечисления.ВариантыПриемкиТоваров.ПоДоговорамПослеЗаказовИлиНакладных) Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура ЗаполнитьСуммовыеПоказателиСтрокиТоваров(СтрокаТоваров)
	...
	Если ЗначениеЗаполнено(СтрокаТоваров.Цена) Тогда
		ПараметрыПересчетаСуммы = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(ЭтотОбъект);
	...
	Если Не ЗначениеЗаполнено(СтрокаТоваров.СтавкаНДС) Тогда
		...
		ПараметрыЗаполненияСтавкиНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(ЭтотОбъект);
		ДействияСоСтрокой.Вставить("ЗаполнитьСтавкуНДС", ПараметрыЗаполненияСтавкиНДС);
	КонецЕсли;
