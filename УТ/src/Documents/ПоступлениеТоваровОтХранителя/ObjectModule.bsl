...
//
// Параметры:
//	ДокументОбъект - ДокументОбъект.ПоступлениеТоваровОтХранителя - документ, для которого выполняется инициализация параметров.
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого ТекущиеДанные Из Товары Цикл
		...
		АдресОшибки =  СтрЗаменить(АдресОшибки, "%НомерСтроки%", ТекущиеДанные.НомерСтроки);
		
		ПолеНоменклатураОприходование = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары",
			ТекущиеДанные.НомерСтроки,
			"НоменклатураОприходование");
		
		Если ТекущиеДанные.Порча Тогда
			
			ТекстОшибки = "";
			
			Если Не ЗначениеЗаполнено(ТекущиеДанные.НоменклатураОприходование) Тогда
				ТекстОшибки = НСтр("ru = 'Не заполнена колонка ""Номенклатура (оприходование)""'");
			ИначеЕсли ТекущиеДанные.Номенклатура = ТекущиеДанные.НоменклатураОприходование Тогда
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипДанныхЗаполнения = Тип("Структура")
		...
		Если ДанныеЗаполнения.Свойство("Исправление") Тогда
			...
			КорректировкаИсправленияДокументаНаОснованииАктаОРасхожденияхПослеПриемки(ДанныеЗаполнения);
		Иначе
...
Процедура ЗаполнитьПоЗаказу(СтруктураЗаполнения)
	...
	Если ЗначениеЗаполнено(МассивЗаказов)
		...
		|	ВозвращаемыеСерии.Склад КАК Склад,
		...
		|	И ВозвращаемыеСерии.Склад В (&Склад)
		|	
		|СГРУППИРОВАТЬ ПО
		|	ВозвращаемыеСерии.Номенклатура,
		|	ВозвращаемыеСерии.Характеристика,
		|	ВозвращаемыеСерии.Серия,
		|	ВозвращаемыеСерии.Склад,
		|	ВозвращаемыеСерии.Назначение";
		
		Запрос.УстановитьПараметр("МассивЗаказов", МассивЗаказов);
		Запрос.УстановитьПараметр("Склад", СтруктураЗаполнения.РеквизитыШапки.Склад);
...
Процедура КорректировкаИсправленияДокументаНаОснованииАктаОРасхожденияхПослеПриемки(ДанныеЗаполнения)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТоварыАкта.КодСтроки            КАК КодСтроки,
	|	ТоварыАкта.ЗаполненоПоОснованию КАК ЗаполненоПоОснованию,
	...
	|	ТоварыАкта.ЗаказПоставщику      КАК Распоряжение,
	...
	|	И НЕ (НЕ СерииАкта.ЗаполненоПоОснованию
	|		И СерииАкта.Действие = ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеПриемки.ПустаяСсылка))";
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтХранителя Тогда
		Если ПоРаспоряжениям Тогда
			ДополнительныеКолонки = "КодСтроки, Распоряжение, НомерГТД";
		Иначе
			ДополнительныеКолонки = "НомерГТД";
		КонецЕсли;
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтКомиссионера
		И ПоРаспоряжениям Тогда
		ДополнительныеКолонки = "КодСтроки, Распоряжение";
	КонецЕсли;
	Если Не ПустаяСтрока(ДополнительныеКолонки) Тогда
	...
	Для Каждого ТекСтрока Из ТаблицаТоварыАкта Цикл
		...
		Если ТекСтрока.ЗаполненоПоОснованию Тогда
...
Процедура ЗаполнитьПоАктуОРасхожденияхПослеОтгрузки(СтруктураЗаполнения)
	...
	Если СтруктураЗаполнения.Свойство("ИсправлениеОснования") И ЗначениеЗаполнено(СтруктураЗаполнения.ИсправлениеОснования) Тогда
		Запрос.УстановитьПараметр("ДокументОснованиеИсправлениеОснования", СтруктураЗаполнения.ИсправлениеОснования);
	Иначе
		Запрос.УстановитьПараметр("ДокументОснованиеИсправлениеОснования", СтруктураЗаполнения.ОснованиеАкта);
	КонецЕсли;
	...
	ДокументПередачи = СтруктураЗаполнения.ОснованиеАкта;
...
Процедура ЗаполнитьНоменклатуруОприходованиеВидовЗапасов()
	...
	Если ВозвратПорчи Тогда
		...
		Для Каждого СтрокаТоваров Из СтрокиПорчи Цикл
			...
			ПоискСтрокПоКлючу = ВидыЗапасов.НайтиСтроки(СтруктураПоиска);
			
			Для Каждого СтрокаВидыЗапасов Из ПоискСтрокПоКлючу Цикл
				
				Если СтрокаВидыЗапасов.Количество = 0 Тогда
					Продолжить;
				КонецЕсли;
				
				Количество = Мин(КоличествоТоваров, СтрокаВидыЗапасов.Количество);
				
				НоваяСтрока = ВидыЗапасов.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаВидыЗапасов);
				...
				СтрокаВидыЗапасов.Количество = СтрокаВидыЗапасов.Количество - НоваяСтрока.Количество;
