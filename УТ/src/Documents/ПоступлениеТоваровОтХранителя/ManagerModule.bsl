...
// 
// Параметры:
//  ОбъектИлиСсылка - ДокументОбъект.ПоступлениеТоваровОтХранителя, ДокументСсылка.ПоступлениеТоваровОтХранителя - документ.
...
// 
// Параметры:
//  Объект - ДокументОбъект.ПоступлениеТоваровОтХранителя - объект.
// 
// Возвращаемое значение:
...
Функция КонтрольКачестваПараметрыДействий(Объект) Экспорт
	...
	ПараметрыДействия.ЧитаемыеПоля = "Номенклатура,НоменклатураОприходование,Порча";
...
Функция ДанныеДляПроверкиВводаНаОсновании(Распоряжения, ХозяйственнаяОперация = Неопределено) Экспорт
	...
	КОформлению = Результат[Результат.ВГраница()].Выгрузить();
...
//	* ХарактеристикаОприходование - СправочникСсылка.ХарактеристикиНоменклатуры - 
...
Процедура ЗаполнитьПоЗаказамОрдерам(Товары, Регистратор, ПараметрыЗаполнения, ШтрихкодыУпаковок = Неопределено) Экспорт
	...
	Если ОтборПоСкладу
		И (ПараметрыЗаполнения.МассивЗаказов <> Неопределено
							И ТипЗнч(ПараметрыЗаполнения.МассивЗаказов[0]) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента")) Тогда
		ОбщегоНазначенияУТ.ДобавитьЭлементОтбораВКоллекцию(Отбор, "Таблица.Склад", "&Склад");
	КонецЕсли;	
	...
	Если Не Обработчик.ИспользоватьКодСтроки() Тогда
		...
		ОбщегоНазначенияУТ.УдалитьСтрокиТаблицы(ТаблицаЗаказы, УдаляемыеСтроки);
	КонецЕсли;
	...
	Отбор = Новый Соответствие;
	Отбор.Вставить("ДанныеУчета.ДокументОснование", "ДокументПоступления");
	
	ТекстСоединения =
		" Таблица.Ссылка = ДанныеУчета.ДокументОснование
		| И Таблица.Распоряжение = ДанныеУчета.Распоряжение
		| И Таблица.НомерГТД = ДанныеУчета.НомерГТД
		|";
	...
	Если ТаблицаОрдера.Количество() > 0 И ЗначениеЗаполнено(Регистратор) Тогда
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Регистратор, "ПоРаспоряжениям, Распоряжение");
		Если РеквизитыДокумента.ПоРаспоряжениям
			И ЗначениеЗаполнено(РеквизитыДокумента.Распоряжение) Тогда
			Для Каждого СтрокаОрдера Из ТаблицаОрдера Цикл
				Если Не ЗначениеЗаполнено(СтрокаОрдера.Распоряжение) Тогда
					СтрокаОрдера.Распоряжение = РеквизитыДокумента.Распоряжение;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПривязатьСтрокиКЗаказам(Форма, Объект, ПараметрыУказанияСерий, ОтобратьПоЗаказу) Экспорт
	...
	Если РезультатЗапроса[РезультатЗапроса.ВГраница() - 2].Пустой() Тогда
		Возврат;
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		// Сначала распределяем с учетом серий, а потом без.
		// Дубли обрабатываем только после попытки распределения без учета серий.
...
Процедура ОбновитьЗависимыеРеквизитыТабличнойЧасти(Объект, ПараметрыЗаполнения) Экспорт
	...
	ОбрабатываемыеСтрокиСВидомЦены = Новый Массив;

	Для Каждого Строка Из Объект.Товары Цикл
		
			Если ЗначениеЗаполнено(Строка.Цена) Тогда

				ОбрабатываемыеСтрокиСЦеной.Добавить(Строка);
				Продолжить;

			ИначеЕсли ЗначениеЗаполнено(Объект.ВидЦены) Тогда

				ОбрабатываемыеСтрокиСВидомЦены.Добавить(Строка);
				Продолжить;
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		ОбрабатываемыеСтрокиСВидомЦены,
		СтруктураДействий,
		Объект.Товары,
		КэшированныеЗначения);
...
Функция ТекстЗапросаТаблицаРаспоряженияНаОтгрузку(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаТовары.Ссылка = &Ссылка
	|	И ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаТовары.Распоряжение) = ТИП(Документ.ЗаявкаНаВозвратТоваровОтКлиента)
	|			ТОГДА НЕ ВЫРАЗИТЬ(ТаблицаТовары.Распоряжение КАК Документ.ЗаявкаНаВозвратТоваровОтКлиента).ЭтоЗаказКакСчет
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ
	|	И &ПоРаспоряжениям
...
Процедура ЗапланироватьПоступлениеТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР
	|		КОГДА &ТекстВыраженияПоступлениеПоРаспоряжениям
	...
	|	&ТекстВыраженияПоступлениеПоРаспоряжениям    КАК ПоступлениеПоЗаказам
	|ИЗ
	...
	ТекстЗапросаДокумента = СтрЗаменить(ТекстЗапросаДокумента,
		"&ТекстВыраженияПоступлениеПоРаспоряжениям",
		ТекстВыраженияПоступлениеПоРаспоряжениям("ИсточникДанных"));
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	|	&ТекстВыраженияПоступлениеПоРаспоряжениям КАК ПоГрафику,
	...
	ТекстЗапросаТабЧасть = СтрЗаменить(ТекстЗапросаТабЧасть,
		"&ТекстВыраженияПоступлениеПоРаспоряжениям",
		ТекстВыраженияПоступлениеПоРаспоряжениям("ТабЧасть"));
	...
	|	ВЫБОР
	|		КОГДА &ТекстВыраженияПоступлениеПоРаспоряжениям
	|		И НЕ ТабЧасть.Ссылка.Распоряжение В (НЕОПРЕДЕЛЕНО,
	...
	|			ТОГДА ТабЧасть.Ссылка.Распоряжение
	|		ИНАЧЕ ВЫБОР
	...
	|	И ТабЧасть.Ссылка.Склад.ДатаНачалаОрдернойСхемыПриПоступлении <= ТабЧасть.Ссылка.Дата
	|	И (ТабЧасть.Ссылка.ВариантПриемкиТоваров = ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.РазделенаТолькоПоНакладным)
	|	ИЛИ НЕ &ТекстВыраженияПоступлениеПоРаспоряжениям)";
...
Процедура ОформитьПоступлениеТоваровПоОдноходовке(Запрос, ТекстыЗапроса, Регистры)
	...
	ПараметрыМетода.ПереопределяемыеПоляСоединенияССериями = ПереопределяемыеПоляСоединенияССериями;
...
Функция ТекстЗапросаТаблицаТоварыОрганизаций(Запрос, ТекстыЗапроса, Регистры)
	...
	|	Документ.ПоступлениеТоваровОтХранителя.ВидыЗапасов КАК ТаблицаВидыЗапасов
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК Аналитика
	|		ПО ТаблицаВидыЗапасов.НоменклатураОприходование = Аналитика.Номенклатура
	|		И ТаблицаВидыЗапасов.ХарактеристикаОприходование = Аналитика.Характеристика
...
Функция ТекстЗапросаВтВидыЗапасов(Запрос, ТекстыЗапроса)
	...
	|		ПО ТаблицаВидыЗапасов.НоменклатураОприходование = АналитикаОприходования.Номенклатура
	|			И ТаблицаВидыЗапасов.ХарактеристикаОприходование = АналитикаОприходования.Характеристика
	...
	|		ПО ТаблицаВидыЗапасов.НоменклатураОприходование = АналитикаОприходованияБезНазначения.Номенклатура
	|			И ТаблицаВидыЗапасов.ХарактеристикаОприходование = АналитикаОприходованияБезНазначения.Характеристика
...
Процедура ИнициализироватьКлючиАналитикиУчетаНоменклатуры(Запрос)
	...
	|		И Аналитика.Номенклатура ЕСТЬ NULL
	|		И НЕ &УчитыватьСебестоимостьТоваровПоНазначениям
	|		И Товары.НоменклатураОприходование <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)) КАК ТаблицаАналитик";
...
Функция ТекстВыраженияПоступлениеПоРаспоряжениям(ИмяТаблицы)
	
	Возврат СтрШаблон(
		"(%1.Ссылка.ПоРаспоряжениям
		|	И ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(%1.Распоряжение) = ТИП(Документ.ЗаявкаНаВозвратТоваровОтКлиента)
		|			ТОГДА НЕ ВЫРАЗИТЬ(%1.Распоряжение КАК Документ.ЗаявкаНаВозвратТоваровОтКлиента).ЭтоЗаказКакСчет
		|		ИНАЧЕ ИСТИНА
		|	КОНЕЦ)",
		ИмяТаблицы);
...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	|	ЕСТЬNULL(ТаблицаВидыЗапасов.АналитикаУчетаНоменклатурыОприходования,
...
Функция ТекстЗапросаТоварыДокумента(Отбор, ТекстСоединения, ИспользоватьКодСтроки = Ложь)
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ДанныеУчета.ДокументОснование КАК Ссылка,
	...
	|	ЕСТЬNULL(Таблица.Распоряжение, ДанныеУчета.Распоряжение) КАК Распоряжение,
	...
	|	ЕСТЬNULL(Таблица.Распоряжение, ДанныеУчета.Распоряжение),
	|	&КодСтроки";
...
Функция ТекстЗапросаОстаткиКОформлениюПоАктам(ИмяВременнойТаблицы = "", ИспользоватьКодСтроки = Ложь)
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	Набор.ДокументОснование  КАК ДокументОснование,
...
Процедура ДобавитьДополнительноеСвойствоДляКонтроляПриПроведении(Объект) Экспорт
	...
	Если Не Объект.ПоРаспоряжениям Тогда
...
Функция ТекстЗапросаДанныеЗаполненияНакладной() Экспорт
	...
	|	КОНЕЦ КАК ВидЦены,
	|	Заказ.Ссылка.ВозвратПорчи КАК ВозвратПорчи
	|ИЗ
	...
	|	КОНЕЦ,
	|	Заказ.Ссылка.Сделка,
	|	Заказ.Ссылка.Партнер,
	|	Заказ.Ссылка.Контрагент,
	|	Заказ.Ссылка.Соглашение,
	|	Заказ.Ссылка.Договор,
	|	Заказ.Ссылка.НаправлениеДеятельности,
	|	Заказ.Ссылка.ВернутьМногооборотнуюТару,
	|	Заказ.Ссылка.Валюта,
	|	Заказ.Ссылка.ВозвратПорчи
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|СГРУППИРОВАТЬ ПО
	|	Передача.Ссылка,
	|	Передача.Ссылка.Организация,
	|	Передача.Ссылка.Подразделение,
	|	ВЫБОР
	|		КОГДА Передача.Ссылка.Склад.ЭтоГруппа
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	|		ИНАЧЕ Передача.Ссылка.Склад
	|	КОНЕЦ,
	|	Передача.Ссылка.Сделка,
	|	Передача.Ссылка.Партнер,
	|	Передача.Ссылка.Контрагент,
	|	Передача.Ссылка.Соглашение,
	|	Передача.Ссылка.Договор,
	|	Передача.Ссылка.НаправлениеДеятельности,
	|	Передача.Ссылка.ВернутьМногооборотнуюТару,
	|	Передача.Ссылка.Валюта";
...
Функция ТекстЗапросаТоварыПереданные() Экспорт
	...
	|					И НЕ ВтПолитикиУчетаСерий.УчетСерийВПереданныхНаХранениеТоварах
	|				ИЛИ &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОтКомиссионера)
	|					И НЕ ВтПолитикиУчетаСерий.УчетСерийВПереданныхНаКомиссиюТоваров
	|				ИЛИ &ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5),
	|												ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОтПереработчика2_5))
	|					И НЕ ВтПолитикиУчетаСерий.УчетСерийПереданныхПереработчикуТоваров)
	...
	|	ТоварыПереданныеГруппировка.Номенклатура,
	|	ТоварыПереданныеГруппировка.Характеристика,
	|	ТоварыПереданныеГруппировка.Назначение,
	|	ТоварыПереданныеГруппировка.Серия
	|";
...
Функция ТекстСоединения(ИспользоватьНомерГТД = Истина) Экспорт
	...
	ТекстСоединения =
	"Таблица.Ссылка = ДанныеУчета.Распоряжение
	|		И Таблица.Назначение = ДанныеУчета.Назначение
	|		И ВЫБОР
	|			КОГДА Таблица.Порча
	|				ТОГДА Таблица.НоменклатураОприходование
	|			ИНАЧЕ Таблица.Номенклатура
	|		КОНЕЦ = ДанныеУчета.Номенклатура
	|		И ВЫБОР
	|			КОГДА Таблица.Порча
	|				ТОГДА Таблица.ХарактеристикаОприходование
	|			ИНАЧЕ Таблица.Характеристика
	|		КОНЕЦ = ДанныеУчета.Характеристика";
