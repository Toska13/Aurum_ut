...
Процедура ПриИзмененииСтатусаДокумента(ДокументСсылка, ПредыдущийСтатус, НовыйСтатус, ПараметрыОбновленияСтатуса) Экспорт
	...
	ПроверятьЧастичноеВыбытие = ИнтеграцияЕГАИС.ИспользуетсяЧастичноеВыбытие(Ложь);
...
//  ДокументСсылка - ДокументСсылка.АктСписанияЕГАИС - Документ, для которого выполняется заполнение
...
Процедура ЗаполнитьПродукциюДокумента(ТаблицаМаркируемойПродукции, ДокументСсылка, Сценарий) Экспорт
	...
	|	И АктСписанияЕГАИСТовары.АлкогольнаяПродукция.ВидПродукции.Маркируемый = Истина
	|СГРУППИРОВАТЬ ПО
...
Функция ЭтотОбъект(ЭтоМетаданные = Ложь)
	Если ЭтоМетаданные Тогда
		Возврат Метаданные.Документы.АктСписанияЕГАИС;
	Иначе
		Возврат Документы.АктСписанияЕГАИС;
	КонецЕсли;
КонецФункции
