...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	ПараметрыУказанияСерий.ПоляСвязи.Добавить("ДействиеСТоваром");
...
Функция ТекстЗапросаЗаполненияСтатусовУказанияСерий(ПараметрыУказанияСерий) Экспорт
	...
	|				И ТоварыДляЗапроса.ДействиеСТоваром = СерииДляЗапроса.ДействиеСТоваром
	...
	|			И Товары.ДействиеСТоваром = ТоварыДляЗапроса.ДействиеСТоваром
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	...
	Если ПравоДоступа("Добавление", Метаданные.Документы.АктОРасхожденияхПослеОтгрузки) Тогда
		...
		ФункциональныеОпции = "ИспользоватьАктыРасхожденийПослеОтгрузки";
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если РасхожденияКлиентСервер.ТипОснованияРеализацияТоваровУслуг(Данные.ТипОснованияАктаОРасхождении) Тогда
		...
		Если Данные.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
			ШаблонПредставления = НСтр("ru = 'Акт о расхождениях после отгрузки товаров клиенту и перехода права собственности %1 от %2'");
		Иначе
			ШаблонПредставления = НСтр("ru = 'Акт о расхождениях после реализации %1 от %2'");
		КонецЕсли;
	
	ИначеЕсли РасхожденияКлиентСервер.ТипОснованияОтгрузкаТоваровКлиенту(Данные.ТипОснованияАктаОРасхождении) Тогда
		ШаблонПредставления = НСтр("ru = 'Акт о расхождениях после отгрузки товаров клиенту и до перехода права собственности %1 от %2'");
...
Функция ТекстЗапросаВТСерии(Запрос, ТекстыЗапроса)
	...
	|	ТаблицаСерии.ДействиеСТоваром                            КАК ДействиеСТоваром,
	|	ВЫБОР
	...
	|	И ТаблицаСерии.ДействиеСТоваром В (
	...
	|	ТаблицаТовары.ДействиеСТоваром,
	|	ВЫБОР
	...
	|	И ТаблицаТовары.ДействиеСТоваром В (
	|			ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.УчестьПриИнвентаризации),
	|			ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.Списать)
	|			)
	|	И &Статус В(
...
Функция ТекстЗапросаТаблицаДвиженияСерийТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|	И ТаблицаТовары.ДействиеСТоваром = ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.УчестьПриИнвентаризации)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Процедура ЗапланироватьОтгрузкуТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|				ИЛИ ДанныеШапки.ТипОснованияАктаОРасхождении = ЗНАЧЕНИЕ(Перечисление.ТипыОснованияАктаОРасхождении.ОтгрузкаТоваровКлиенту))
	|			И ТоварыДокумента.Действие В(ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ТребуетсяДопоставка),
	|										ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ДопоставкаНеТребуется),
	|										ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ПокупкаПерепоставленного),
	|										ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ВозвратПерепоставленного))))
	|	И ТоварыДокумента.ДействиеСТоваром В (
	|			ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.УчестьПриИнвентаризации),
	|			ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.Списать)
	|			)
...
Процедура ОформитьОтгрузкуТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	ПараметрыМодульногоПроведения.ОбъектМетаданных = Метаданные.Документы.АктОРасхожденияхПослеОтгрузки;
	ПараметрыМодульногоПроведения.ИмяТЧСерии = "Серии";
	ПараметрыМодульногоПроведения.ЭтоАктОРасхождениях = Истина;
	...
	СкладыСервер.ОформитьОтгрузкуТоваровАкты(Запрос,
											ТекстыЗапроса,
											Регистры,
											ТекстЗапросаДанныхДокумента,
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|					ИЛИ ТабЧасть.Ссылка.ТипОснованияАктаОРасхождении = ЗНАЧЕНИЕ(Перечисление.ТипыОснованияАктаОРасхождении.ОтгрузкаТоваровКлиенту)))
		|		И ТабЧасть.ДействиеСТоваром В (
		|				ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.УчестьПриИнвентаризации),
		|				ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.Списать)
		|				)
		|";
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("4f28da79-079d-400e-a491-663c89ebbdcd");
	...
	СписокОписаний.Добавить(
		НСтр("ru = 'Обновление документов ""Акт о расхождениях после отгрузки"":'"));
	СписокОписаний.Добавить(
		НСтр("ru = '- заполнение реквизита ""Как отработать расхождение с товаром"" в табличной части ""Товары"".';"));
	СписокОписаний.Добавить(
		НСтр("ru = '- актуализация реквизита ""Как отработать расхождение"" в табличной части ""Товары"" для документов с операцией ""Переход права собственности"".';"));
	
	Обработчик.Комментарий = СтрСоединить(СписокОписаний, Символы.ПС);
	
	ПолноеИмя = Метаданные.Документы.АктОРасхожденияхПослеОтгрузки.ПолноеИмя();
	
	Изменяемые = Новый Массив;
	Изменяемые.Добавить(ПолноеИмя);
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(ПолноеИмя);
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ДанныеТоваров.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.АктОРасхожденияхПослеОтгрузки.Товары КАК ДанныеТоваров
	|ГДЕ
	|	ДанныеТоваров.Действие <> ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ПустаяСсылка)
	|	И ДанныеТоваров.ДействиеСТоваром = ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	|	ДанныеСерий.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.АктОРасхожденияхПослеОтгрузки.Серии КАК ДанныеСерий
	|ГДЕ
	|	ДанныеСерий.Действие <> ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ПустаяСсылка)
	|	И ДанныеСерий.ДействиеСТоваром = ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.ПустаяСсылка)
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний = Новый Массив;
	СписокОписаний.Добавить(
		НСтр("ru = 'Не удалось обработать документы ""Акт о расхождениях после отгрузки"" по обработчику:'"));
	СписокОписаний.Добавить(
		НСтр("ru = '- заполнение реквизита ""Как отработать расхождение с товаром"" в табличной части ""Товары"".';"));
	СписокОписаний.Добавить(
		НСтр("ru = '- актуализация реквизита ""Как отработать расхождение"" в табличной части ""Товары"" для документов с операцией ""Переход права собственности"".';"));
	...
	|ИЗ
	|	&ДанныеДляОбновления КАК ТаблицаДокументов
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.АктОРасхожденияхПослеОтгрузки КАК ДанныеДокумента
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Попытка
			...
			ДокументОбъект = ОбновлениеИнформационнойБазыУТ.ПроверитьПолучитьОбъект(
				ВыборкаДокументов.Ссылка, ВыборкаДокументов.ВерсияДанных, Параметры.Очередь); // ДокументОбъект.АктОРасхожденияхПослеОтгрузки - 
			...
			Для Каждого СтрокаТовара Из ДокументОбъект.Товары Цикл
				...
				Если Не ЗначениеЗаполнено(СтрокаТовара.Действие) Тогда
					Продолжить;
				КонецЕсли;
				
				НовыеВариантыДействий = НовыеВариантыДействийДляПереходаНаНовуюВерсию(СтрокаТовара.Действие);
				
				Если ЗначениеЗаполнено(НовыеВариантыДействий.Действие) Тогда
					СтрокаТовара.Действие = НовыеВариантыДействий.Действие;
				КонецЕсли;
				Если ЗначениеЗаполнено(НовыеВариантыДействий.ДействиеСТоваром) Тогда
					СтрокаТовара.ДействиеСТоваром = НовыеВариантыДействий.ДействиеСТоваром;
				КонецЕсли;
			...
			Для Каждого СтрокаСерии Из ДокументОбъект.Серии Цикл
				...
				Если Не ЗначениеЗаполнено(СтрокаСерии.Действие) Тогда
					Продолжить;
				КонецЕсли;
				
				НовыеВариантыДействий = НовыеВариантыДействийДляПереходаНаНовуюВерсию(СтрокаСерии.Действие);
				
				Если ЗначениеЗаполнено(НовыеВариантыДействий.Действие) Тогда
					СтрокаСерии.Действие = НовыеВариантыДействий.Действие;
				КонецЕсли;
				Если ЗначениеЗаполнено(НовыеВариантыДействий.ДействиеСТоваром) Тогда
					СтрокаСерии.ДействиеСТоваром = НовыеВариантыДействий.ДействиеСТоваром;
				КонецЕсли;
		...
		Исключение
			...
			ИначеЕсли ПричинаИсключения = "Запись" Тогда
				...
				ВызватьИсключение НСтр("ru = 'Не удалось обработать документ ""Акт о расхождениях после отгрузки"".'");
...
Функция НовыеВариантыДействийДляПереходаНаНовуюВерсию(ВариантДействия)
	
	НовыеВариантыДействий = Новый Структура;
	НовыеВариантыДействий.Вставить("Действие", Неопределено);
	НовыеВариантыДействий.Вставить("ДействиеСТоваром", Неопределено);
	
	// Недостачи
	Если ВариантДействия = Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.УдалитьУменьшитьПереходПраваСобственности Тогда
		НовыеВариантыДействий.Вставить("Действие", Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ДопоставкаНеТребуется); // Оформить
		НовыеВариантыДействий.Вставить("ДействиеСТоваром", Перечисления.ВариантыДействийПоТоварамВАктеПослеОтгрузки.Ожидать);
	ИначеЕсли ВариантДействия = Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.УдалитьТребуетсяСписание Тогда
		НовыеВариантыДействий.Вставить("Действие", Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ДопоставкаНеТребуется); // Оформить
		НовыеВариантыДействий.Вставить("ДействиеСТоваром", Перечисления.ВариантыДействийПоТоварамВАктеПослеОтгрузки.СписатьИзПути);
	ИначеЕсли ВариантДействия = Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.УдалитьТребуетсяСписаниеДопоставка Тогда
		НовыеВариантыДействий.Вставить("Действие", Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ТребуетсяДопоставка); // Оформить и допоставить
		НовыеВариантыДействий.Вставить("ДействиеСТоваром", Перечисления.ВариантыДействийПоТоварамВАктеПослеОтгрузки.СписатьИзПути);
	// Излишки
	ИначеЕсли ВариантДействия = Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.УдалитьУвеличитьПереходПраваСобственности Тогда
		НовыеВариантыДействий.Вставить("Действие", Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ПокупкаПерепоставленного); // Оформить
		НовыеВариантыДействий.Вставить("ДействиеСТоваром", Перечисления.ВариантыДействийПоТоварамВАктеПослеОтгрузки.УчестьПриИнвентаризации);
	ИначеЕсли ВариантДействия = Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.УдалитьТребуетсяОприходование Тогда
		НовыеВариантыДействий.Вставить("Действие", Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ПокупкаПерепоставленного); // Оформить
		НовыеВариантыДействий.Вставить("ДействиеСТоваром", Перечисления.ВариантыДействийПоТоварамВАктеПослеОтгрузки.ОприходоватьВПуть);
	ИначеЕсли ВариантДействия = Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.УдалитьТребуетсяОприходованиеВозврат Тогда
		НовыеВариантыДействий.Вставить("Действие", Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ВозвратПерепоставленного); // Оформить и вернуть
		НовыеВариантыДействий.Вставить("ДействиеСТоваром", Перечисления.ВариантыДействийПоТоварамВАктеПослеОтгрузки.ОприходоватьВПуть);
	Иначе
		НовыеВариантыДействий.Вставить("ДействиеСТоваром", Перечисления.ВариантыДействийПоТоварамВАктеПослеОтгрузки.УчестьПриИнвентаризации);
	КонецЕсли;
	
	Возврат НовыеВариантыДействий;
