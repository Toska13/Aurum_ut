...
Функция СтатусПослеПодготовкиКПередачеДанных(ДокументСсылка, Операция) Экспорт
	...
	Если Операция = ОтчетОбИмпорте() Тогда
		...
		ПараметрыОбновления = РегистрыСведений.СтатусыДокументовЕГАИС.РассчитатьСтатусыКПередаче(
			ДокументСсылка,
			Перечисления.СтатусыОбработкиОтчетаОбИмпортеЕГАИС.КПередаче);
		
	ИначеЕсли Операция = ЗапросОтмены() Тогда
		
		ПараметрыОбновления = РегистрыСведений.СтатусыДокументовЕГАИС.РассчитатьСтатусыКПередаче(
			ДокументСсылка,
			Перечисления.СтатусыОбработкиОтчетаОбИмпортеЕГАИС.ЗапросНаОтменуПроведенияКПередаче);
...
Функция ТекстЗапросаПанельОбменСЕГАИСОформите() Экспорт
	...
	Если Не ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОПланируемомИмпортеЕГАИС) Тогда
		ТекстЗапроса = "ВЫБРАТЬ 0 КАК КоличествоДокументов";
	Иначе
		ТекстЗапроса = "
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Статусы.Основание) КАК КоличествоДокументов
		|ИЗ
		|	РегистрСведений.СтатусыОформленияДокументовЕГАИС КАК Статусы
		|ГДЕ
		|	Статусы.Документ = ЗНАЧЕНИЕ(Документ.ОтчетОбИмпортеЕГАИС.ПустаяСсылка)
		...
		|		 ЗНАЧЕНИЕ(Перечисление.СтатусыОформленияДокументовЕГАИС.ЕстьОшибкиОформления))
...
Функция ОтчетОбИмпортеЕГАИСXML(ДокументСсылка)
	...
	ТекстыЗапроса.Добавить(
		...
		|
		|	Шапка.ТаможенныйСклад.Код           КАК ТаможенныйСкладКод,
		|
	...
	ТекстыЗапроса.Добавить(
		...
		|ГДЕ
		|	Производители.ТипОрганизации = ЗНАЧЕНИЕ(Перечисление.ТипыОрганизацийЕГАИС.ЮридическоеЛицоРФ)
		|	ИЛИ Производители.ТипОрганизации = ЗНАЧЕНИЕ(Перечисление.ТипыОрганизацийЕГАИС.ИндивидуальныйПредпринимательРФ)",
		"АлкогольнаяПродукцияПроизводимаяВРФ");
	...
	СообщениеXML.Описание = ОбменДаннымиЕГАИС.ОписаниеОперацииПередачиДанных(ОтчетОбИмпорте(), ДокументСсылка);
	
	//@skip-warning
	АлкогольнаяПродукцияПроизводимаяВРФ = РезультатыЗапроса["АлкогольнаяПродукцияПроизводимаяВРФ"].Выбрать();
	Пока АлкогольнаяПродукцияПроизводимаяВРФ.Следующий() Цикл
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Алкогольная продукция ""%1"", производимая в РФ не может быть импортирована.'"),
			АлкогольнаяПродукцияПроизводимаяВРФ.Представление);
		ОбменДаннымиЕГАИСКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, ТекстОшибки);
	КонецЦикла;
	...
	ВидыПродукцииИСМПЕГАИС = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииИСМПЕГАИС();
	ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(Шапка.Ссылка, Неопределено, ВидыПродукцииИСМПЕГАИС);
	...
	Если Не Шапка.ОтчетПроизводственнойЛинии = Истина Тогда
		...
		ВложенныеШтрихкодыУпаковок = ШтрихкодированиеИС.ВложенныеШтрихкодыУпаковок(ШтрихкодыУпаковок, ПараметрыСканирования);
	Иначе
		//@skip-warning
...
#Область РазборКодовМаркировки
		...
		РезультатОбработкиДанныхОтчета = РазборКодаМаркировкиИСОтчетПроизводственнойЛинии.НормализоватьДанныеОтчетаПроизводственнойЛинии(
			ШтрихкодыТаблица, ВидыПродукцииИСМПЕГАИС, ПараметрыРазбора);
...
#КонецОбласти
	...
	ОтчетXDTO = РаботаСXMLЕГАИС.ОбъектXDTO(ПространствоИмен, "RepImportedType_v6");
	...
	РаботаСXMLЕГАИС.ЗаполнитьСвойствоXDTO(Header, "UNFSM_FLAG",      Ложь,                     СообщениеXML);
	РаботаСXMLЕГАИС.ЗаполнитьСвойствоXDTO(Header, "BONDEDWAREHOUSE", Шапка.ТаможенныйСкладКод, СообщениеXML);
...
Процедура ОбработатьДанныеШтрихкодаПотребительскойУпаковки(Форма, ДанныеШтрихкода, ПараметрыЗаполнения)
	...
	Если ЗначениеЗаполнено(ПараметрыЗаполнения.РодительскийШтрихкод) Тогда
...
Процедура ЗаполнитьДанныеОбъектаФормыПоДеревуУпаковок(Форма, ДеревоУпаковок, ПараметрыЗаполнения,
	...
	Для Каждого СтрокаДерева Из ДеревоУпаковок.Строки Цикл
		...
		Если Не ЗначениеЗаполнено(УпаковкаВерхнегоУровня) Тогда
...
Процедура ЗаполнитьПродукциюДокумента(ТаблицаМаркируемойПродукции, ДокументСсылка, Сценарий) Экспорт
	...
	|	ОтчетОбИмпортеЕГАИСТовары.АлкогольнаяПродукция,
	...
	ПроверкаИПодборПродукцииИСПереопределяемый.ПриОпределенииПродукцииДокумента(
		ДокументСсылка, Сценарий, ТаблицаМаркируемойПродукции);
...
Процедура ОтразитьРезультатыПроверкиИПодбора(ПараметрыОкончанияПроверки) Экспорт
	...
	АкцизныеМаркиЕГАИС.ОтразитьРезультатыПроверкиИПодбора(ЭтотОбъект(), ПараметрыОкончанияПроверки,,, Ложь);
...
Процедура ОбработатьСтрокиТабличнойЧасти(ДокументОбъект, ПараметрыЗаполнения) Экспорт
	...
	ЗаполнитьКрепость = ПараметрыЗаполнения.ДобавленныеСтроки.Количество()
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПараметрыЗаполнения.ДобавленныеСтроки[0], "Крепость");
	Если ЗаполнитьКрепость Тогда
...
Функция ЭтотОбъект(ЭтоМетаданные = Ложь)
	Если ЭтоМетаданные Тогда
		Возврат Метаданные.Документы.ОтчетОбИмпортеЕГАИС;
	Иначе
		Возврат Документы.ОтчетОбИмпортеЕГАИС;
	КонецЕсли;
КонецФункции
