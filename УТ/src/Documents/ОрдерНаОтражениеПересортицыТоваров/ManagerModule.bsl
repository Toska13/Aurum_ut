...
Функция ПечатьОтраженияПересортицыТоваровНаСкладе(МассивОбъектов, ОбъектыПечати)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&КолонкаКодов1", ?(ВыводитьКоды, "ТоварыОрдера.Номенклатура."
		+ ИмяКолонкиКодов, """"""));

	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&КолонкаКодов2", ?(ВыводитьКоды,
		"ТоварыОрдера.НоменклатураОприходование." + ИмяКолонкиКодов, """"""));

	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстЗапросаКоэффициентУпаковки1", Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки(
			"ТоварыОрдера.Упаковка", "ТоварыОрдера.Номенклатура"));

	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстЗапросаКоэффициентУпаковки2", Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки(
			"ТоварыОрдера.УпаковкаОприходование", "ТоварыОрдера.НоменклатураОприходование"));

	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстЗапросаНаименованиеЕдиницыИзмерения",
		Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаЗначениеРеквизитаЕдиницыИзмерения(
			"Наименование", "ТоварыОрдера.Упаковка", "ТоварыОрдера.Номенклатура"));
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Если ВыводитьЯчейкиИУпаковки Тогда
			...
			ОбластьШапкаТаблицыКолонкаКодовСписания  = Макет.ПолучитьОбласть(
				"ШапкаТаблицы|КолонкаКодовСписываемогоТовара");
			ОбластьСтрокаТаблицыКолонкаКодовСписания = Макет.ПолучитьОбласть(
				"СтрокаТаблицы|КолонкаКодовСписываемогоТовара");
			ОбластьПодвалТаблицыКолонкаКодовСписания = Макет.ПолучитьОбласть(
				"ПодвалТаблицы|КолонкаКодовСписываемогоТовара");
			...
			Если ВыводитьКоды Тогда
				ОбластьШапкаТаблицыКолонкаКодовПриходования 	= Макет.ПолучитьОбласть(
					"ШапкаТаблицы|КолонкаКодовПриходуемогоТовара");
				ОбластьСтрокаТаблицыКолонкаКодовПриходования 	= Макет.ПолучитьОбласть(
					"СтрокаТаблицы|КолонкаКодовПриходуемогоТовара");
				ОбластьПодвалТаблицыКолонкаКодовПриходования 	= Макет.ПолучитьОбласть(
					"ПодвалТаблицы|КолонкаКодовПриходуемогоТовара");
			КонецЕсли;
		...
		Иначе
			...
			ОбластьШапкаТаблицыКолонкаКодовСписания 	= Макет.ПолучитьОбласть(
				"ШапкаТаблицыБезЯчУп|КолонкаКодовБезЯчейкиСписываемогоТовара");
			ОбластьСтрокаТаблицыКолонкаКодовСписания 	= Макет.ПолучитьОбласть(
				"СтрокаТаблицыБезЯчУп|КолонкаКодовБезЯчейкиСписываемогоТовара");
			ОбластьПодвалТаблицыКолонкаКодовСписания 	= Макет.ПолучитьОбласть(
				"ПодвалТаблицы|КолонкаКодовБезЯчейкиСписываемогоТовара");

			ОбластьШапкаТаблицыКолонкаТоваровСписания 	= Макет.ПолучитьОбласть(
				"ШапкаТаблицыБезЯчУп|СписываемыйТоварБезЯчеек");
			ОбластьСтрокаТаблицыКолонкаТоваровСписания 	= Макет.ПолучитьОбласть(
				"СтрокаТаблицыБезЯчУп|СписываемыйТоварБезЯчеек");
			ОбластьПодвалТаблицыКолонкаТоваровСписания 	= Макет.ПолучитьОбласть(
				"ПодвалТаблицы|СписываемыйТоварБезЯчеек");

			Если ВыводитьКоды Тогда
				ОбластьШапкаТаблицыКолонкаКодовПриходования 	= Макет.ПолучитьОбласть(
					"ШапкаТаблицыБезЯчУп|КолонкаКодовПриходуемогоТовара");
				ОбластьСтрокаТаблицыКолонкаКодовПриходования 	= Макет.ПолучитьОбласть(
					"СтрокаТаблицыБезЯчУп|КолонкаКодовПриходуемогоТовара");
				ОбластьПодвалТаблицыКолонкаКодовПриходования 	= Макет.ПолучитьОбласть(
					"ПодвалТаблицы|КолонкаКодовПриходуемогоТовара");
			КонецЕсли;

			ОбластьШапкаТаблицыКолонкаТоваровПриходования 	= Макет.ПолучитьОбласть(
				"ШапкаТаблицыБезЯчУп|ПриходуемыйТоварБезУпаковок");
			ОбластьСтрокаТаблицыКолонкаТоваровПриходования 	= Макет.ПолучитьОбласть(
				"СтрокаТаблицыБезЯчУп|ПриходуемыйТоварБезУпаковок");
			ОбластьПодвалТаблицыКолонкаТоваровПриходования 	= Макет.ПолучитьОбласть(
				"ПодвалТаблицы|ПриходуемыйТоварБезУпаковок");
		...
		ОбластьКолонкаТоваровСписания = ?(ВыводитьЯчейкиИУпаковки, Макет.Область("СписываемыйТовар"), Макет.Область(
			"СписываемыйТоварБезЯчеек"));

		ОбластьКолонкаТоваровПриходования = ?(ВыводитьЯчейкиИУпаковки, Макет.Область("ПриходуемыйТовар"), Макет.Область(
			"ПриходуемыйТоварБезУпаковок"));

		Если Не ВыводитьКоды Тогда
			ОбластьКолонкаТоваровСписания.ШиринаКолонки = ОбластьКолонкаТоваровСписания.ШиринаКолонки + ?(
				ВыводитьЯчейкиИУпаковки, Макет.Область("КолонкаКодовСписываемогоТовара").ШиринаКолонки, Макет.Область(
				"КолонкаКодовБезЯчейкиСписываемогоТовара").ШиринаКолонки);
		...
		ОбластьШапка.Параметры.СкладПредставление = СкладыСервер.ПолучитьПредставлениеСклада(
			ВыборкаПоДокументам.СкладПредставление, ВыборкаПоДокументам.ПомещениеПредставление);
		...
		Пока ВыборкаПоСтрокамТЧ.Следующий() Цикл
			...
			ОбластьСтрокаТаблицыКолонкаТоваровСписания.Параметры.Товар = НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати(
				ВыборкаПоСтрокамТЧ.ПредставлениеНоменклатуры, ВыборкаПоСтрокамТЧ.ПредставлениеХарактеристики, ,
				ВыборкаПоСтрокамТЧ.ПредставлениеСерии, ДопПараметрыПредставлениеНоменклатуры);
			...
			ОбластьСтрокаТаблицыКолонкаТоваровПриходования.Параметры.ИспорченныйТовар = НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати(
				...
				ВыборкаПоСтрокамТЧ.ПредставлениеСерииИспорченнойНоменклатуры, ДопПараметрыПредставлениеНоменклатуры);
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	|		И ТабЧасть.КонтролируемаяПартия = ТоварныеМеста.КонтролируемаяПартия
