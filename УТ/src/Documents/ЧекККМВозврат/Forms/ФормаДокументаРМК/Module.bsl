...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолучитьСтатусОплатыСБППоЧекуКММ();
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов, ЗагрузкаИзТСД = Ложь)
	...
	СтруктураДействийСДобавленнымиСтроками.Вставить("ПересчитатьСуммуНДС", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(Объект));
...
Процедура ОбработатьСтрокиТЧ(ДобавленныеСтроки, ИзмененныеСтроки, КэшированныеЗначения = Неопределено, ДействияПриИзмененииКоличества = Неопределено) Экспорт
	...
	МассивыДляРазнойПакетнойОбработкиСтрок = РозничныеПродажи.РазделитьМассивСтрокДляРазныхТиповПакетнойОбработки(ЭтотОбъект, ДобавленныеСтроки);
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(СтруктураДействийСДобавленнымиСтроками, 
		ДействияПриИзмененииКоличества, Истина);
	ПакетнаяОбработкаТабличнойЧастиКлиентСерверЛокализация.ДополнитьСтруктуруДействийПриИзмененииЭлемента(ЭтотОбъект,
		"Номенклатура", СтруктураДействийСДобавленнымиСтроками);

	СтруктураДействийСИзмененнымиСтроками = Новый Структура;
	ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(СтруктураДействийСИзмененнымиСтроками,
		ДействияПриИзмененииКоличества, Истина);
...
Процедура ПриОкончанииРедактированияНабора(АдресВоВременномХранилище)
	...
	Если ЗначениеЗаполнено(Объект.ЧекККМ) Тогда
		...
		Для Каждого СтрокаКомплектующего Из Данные.Комплектующие Цикл
			...
			Если СтрокиТоваров.Количество() 
				И СтрокаКомплектующего.Упаковка <> СтрокиТоваров[0].Упаковка Тогда
				МассивСтрокДляОбработки.Добавить(СтрокиТоваров[0]);
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого СтрокаКомплектующего Из Данные.Комплектующие Цикл
			...
			Если СтрокиТоваров.Количество() Тогда
				...
				Если Не МассивСтрокДляОбработки.Найти(НайденнаяСтрокаТоваров) = Неопределено Тогда
					...
					ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(
						ДанныеДляПересчета, СтруктураДействийДляПересчета, КэшированныеЗначения);
...
Процедура ТоварыУпаковкаПриИзмененииПодтверждениеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Если ЗначениеЗаполнено(Объект.ЧекККМ) Тогда
			...
			ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ДанныеДляПересчета, СтруктураДействий, КэшированныеЗначения);
...
Процедура ИзменитьТабличнуюЧастьПоРезультатамПроверки(ВозвращаемыеПараметры, КэшированныеЗначения)
	...
	КэшированныеЗначенияДляПакетнойОбработки = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ПересчитатьКоличествоЕдиниц");
	СтруктураДействий.Вставить("ПересчитатьСуммуНДС", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(Объект));
	СтруктураДействий.Вставить("ПересчитатьСумму");
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		...
		СтруктураДействий,
		Объект.Товары,
		КэшированныеЗначенияДляПакетнойОбработки);
...
Процедура ЗаполнитьСлужебныеРеквизитыПоЧеку()
	...
	ОписанияЧисловогоТипа = Новый ОписаниеТипов("Число",,,
		Новый КвалификаторыЧисла(15, 3, ДопустимыйЗнак.Неотрицательный));
	
	ТаблицаДанныхДляПересчета = Новый ТаблицаЗначений();
	ТаблицаДанныхДляПересчета.Колонки.Добавить("Упаковка", Новый ОписаниеТипов("СправочникСсылка.УпаковкиЕдиницыИзмерения"));
	ТаблицаДанныхДляПересчета.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТаблицаДанныхДляПересчета.Колонки.Добавить("Количество", ОписанияЧисловогоТипа);
	ТаблицаДанныхДляПересчета.Колонки.Добавить("КоличествоУпаковок", ОписанияЧисловогоТипа);
	ТаблицаДанныхДляПересчета.Колонки.Добавить("ИдентификаторСтроки", Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(10)));
	...
	Если Не НевозвращенныеПозицииПоЧеку = Неопределено Тогда
		...
		Для Каждого СтрокаТовары Из Объект.Товары Цикл
			Если СтрокаТовары.КоличествоУпаковок > 0 Тогда
				...
				Если Не ДанныеНевозвращеннойПозиции = Неопределено 
					И ДанныеНевозвращеннойПозиции.Упаковка <> СтрокаТовары.Упаковка Тогда
					ДанныеДляПересчета = ТаблицаДанныхДляПересчета.Добавить();
					ДанныеДляПересчета.Упаковка = СтрокаТовары.Упаковка;
					ДанныеДляПересчета.Номенклатура = СтрокаТовары.Номенклатура;
					ДанныеДляПересчета.Количество = ДанныеНевозвращеннойПозиции.Количество;
					ДанныеДляПересчета.КоличествоУпаковок = ДанныеНевозвращеннойПозиции.КоличествоУпаковок;
					ДанныеДляПересчета.ИдентификаторСтроки = СтрокаТовары.ПолучитьИдентификатор();
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		
		Если ТаблицаДанныхДляПересчета.Количество() Тогда
			ТаблицаДанныхДляПересчета.Индексы.Добавить("ИдентификаторСтроки");
			...
			ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ТаблицаДанныхДляПересчета, СтруктураДействий, КэшированныеЗначения);
		КонецЕсли;
	...
	Для Каждого СтрокаТовары Из Объект.Товары Цикл
		...
		Если Не НевозвращенныеПозицииПоЧеку = Неопределено Тогда
			...
			ДанныеНевозвращеннойПозиции = НевозвращенныеПозицииПоЧеку[ИдентификаторСтроки];
			Если Не ДанныеНевозвращеннойПозиции = Неопределено Тогда
				...
				НайденныеДанныеДляПересчета = ТаблицаДанныхДляПересчета.НайтиСтроки(Новый Структура("ИдентификаторСтроки", ИдентификаторСтроки));
				Если НайденныеДанныеДляПересчета.Количество() > 0 Тогда
					НовоеЗначениеКоличествоУпаковокПоЧеку = НайденныеДанныеДляПересчета[0].КоличествоУпаковок;
				КонецЕсли;
			КонецЕсли;
...
&НаСервере 
Процедура ПолучитьСтатусОплатыСБППоЧекуКММ()
	...
	Если ЗначениеЗаполнено(Объект.ЧекККМ) Тогда
		...
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ЧекККМОплатаПлатежнымиКартами.СтатусОплатыСБП КАК СтатусОплатыСБП
		|ИЗ
		|	Документ.ЧекККМ.ОплатаПлатежнымиКартами КАК ЧекККМОплатаПлатежнымиКартами
		|ГДЕ
		|	ЧекККМОплатаПлатежнымиКартами.Ссылка = &Ссылка
		|	И ЧекККМОплатаПлатежнымиКартами.ВидОплаты = &ВидОплаты";
...
Процедура ЗаполнитьДанныеПоЧекуККМ()
	...
	Если ЗначениеЗаполнено(Объект.ЧекККМ) Тогда
		...
		СтатусКассовойСмены = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеПоЧекуККМ.КассоваяСмена, "Статус");
		ДанныеПоЧекуККМ.СтатусКассовойСмены = СтатусКассовойСмены;
			...
			Если ЭтоОтложенныйЧекККМВозврат Тогда
				...
				Если СтрокиНевозвращенныхТоваров.Количество() Тогда
					...
					Если СтрокиНевозвращенныхТоваров[0].Упаковка <> СтрокаТаблицы.Упаковка Тогда
						МассивСтрокДляОбработки.Добавить(СтрокаТаблицы);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого СтрокаТаблицы Из Объект.Товары Цикл
			Если Не МассивСтрокДляОбработки.Найти(СтрокаТаблицы) = Неопределено Тогда
				ДанныеДляПересчета = Новый Структура;
				...
				ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(
					ДанныеДляПересчета, СтруктураДействий, КэшированныеЗначения);
...
Процедура ДобавитьВКорзинуНаСервере(ПараметрыТовара, НовыеСтроки)
	...
	// Ограничивает строки, используемые для подготовки исходных данных, при выполнении пакетной обработки строк табличной части.
	ДобавленныеСтрокиДляОбработки = Новый Массив;
	// Используется для получения данных о погрешности вычисления сумм из параметра НовыеСтроки в привязке к добавляемым строкам табличной части Товары.
	СвязьДобавленныхИНовыхСтрок = Новый Соответствие;
	...
	Для Каждого СтрокаТовара Из НовыеСтроки Цикл
		...
		Если РезультатПоиска.Количество() = 0 ИЛИ СтрокаТовара.Погрешность <> 0 Тогда
			...
			ДобавленныеСтрокиДляОбработки.Добавить(ТекущаяСтрока);
			Погрешности.Вставить(ТекущаяСтрока, СтрокаТовара.Погрешность);
			СвязьДобавленныхИНовыхСтрок.Вставить(ТекущаяСтрока, СтрокаТовара);
		Иначе
			...
			ИзмененныеСтрокиДляОбработки.Добавить(ТекущаяСтрока);
			
		КонецЕсли;
		
	КонецЦикла;
	
	Если ДобавленныеСтрокиДляОбработки.Количество() Или	ИзмененныеСтрокиДляОбработки.Количество() Тогда
		Если ДобавленныеСтрокиДляОбработки.Количество() Тогда
			КэшированныеЗначения = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
		...
		Если ИзмененныеСтрокиДляОбработки.Количество() Тогда
			КэшированныеЗначения = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
...
Процедура ОткрытьФормуОплатыСБП(ИзмененныеДанныеЗаписаны, ПараметрыЗавершения) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.ЧекККМ) Тогда
		Если Не (ИнформацияОбОплате.СБП = 0 И ИнформацияОбОплате.СБПОтменено = 0) Тогда
			...
			ПараметрыОткрытияФормы.Вставить("Магазин", СкладКассыККМ(Объект.КассаККМ));
...
&НаСервере
Функция СкладКассыККМ(КассаККМ)
	РеквизитСклада = "Склад";
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КассаККМ, РеквизитСклада);
КонецФункции
