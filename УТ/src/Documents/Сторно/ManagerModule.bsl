...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Запрос.УстановитьПараметр("СторнируемыйДокумент", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "СторнируемыйДокумент"));
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылка);
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	НАЧАЛОПЕРИОДА(РеестрДокументов.ДатаДокументаИБ, ДЕНЬ) КАК ПериодКорректировки
	|ПОМЕСТИТЬ ИсходныйДокумент
	|ИЗ
	|	КритерийОтбора.СвязанныеДокументы(&СторнируемыйДокумент) КАК СвязанныеДокументы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументов
	|		ПО СвязанныеДокументы.Ссылка = РеестрДокументов.Ссылка
	|ГДЕ
	...
	|	ИсходныйДокумент.ПериодКорректировки КАК ПериодКорректировки,
	|	ИсходныйДокумент.ПериодКорректировки КАК ПериодОтклонения
	|ИЗ
	|	Документ.Сторно КАК ДанныеДокумента
	|		ЛЕВОЕ СОЕДИНЕНИЕ ИсходныйДокумент КАК ИсходныйДокумент
	|		ПО (ИСТИНА)
	|ГДЕ
	...
	Для каждого Колонка Из Результат.Колонки Цикл
		...
		СтруктураПараметров.Вставить(Колонка.Имя, Реквизиты[Колонка.Имя]);
	КонецЦикла;
...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	|	ТаблицаВидыЗапасов.Стоимость 				КАК СтоимостьБезНДС,
	|	ТаблицаВидыЗапасов.Стоимость 				КАК СтоимостьРегл,
	|	ТаблицаВидыЗапасов.Стоимость				КАК СтоимостьУпр,
	|	ТаблицаВидыЗапасов.Стоимость	 			КАК КорСтоимость,
	|
	// Прочие поля
	...
	|
	|ИЗ
	|	Документ.Сторно КАК ТаблицаДокумента
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СебестоимостьТоваров КАК ТаблицаВидыЗапасов
	|		ПО ЛОЖЬ
	|ГДЕ
