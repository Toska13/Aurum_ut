...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		Если Запрос.Параметры.ЕстьПрочиеРасходы Тогда
...
Процедура ДобавитьЭтап_РаспределениеДоходовПоНаправлениямДеятельности(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Распределение доходов по направлениям деятельности'"; // @НСтр-1
	...
	ПредшествующиеЭтапы.Добавить(Справочники.ОперацииЗакрытияМесяца.ОформлениеДокументовРаспределенияРасходовПоНаправлениямДеятельности);
...
Процедура Использование_РаспределениеДоходовПоНаправлениямДеятельности(ПараметрыОбработчика) Экспорт
	...
	|ГДЕ
	|	Доходы.СуммаОборот <> 0
	|	ИЛИ Доходы.СуммаУпрОборот <> 0
	|	ИЛИ Доходы.СуммаРеглОборот <> 0";
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ЗаданияКЗакрытию.Организация КАК Организация
		|ПОМЕСТИТЬ ВТЗаданияКРаспределениюДоходовПоНД
		|ИЗ
		|	РегистрСведений.ЗаданияКЗакрытиюМесяца КАК ЗаданияКЗакрытию
		|ГДЕ
		|	ЗаданияКЗакрытию.Организация В (&МассивОрганизаций)
		|	И ЗаданияКЗакрытию.Месяц = &КонецПериода
		|	И ЗаданияКЗакрытию.Операция = ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.РаспределениеДоходовПоНаправлениямДеятельности)
		|;
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Задания.Организация
		|ИЗ
		|	ВТЗаданияКРаспределениюДоходовПоНД КАК Задания";
...
Процедура ПроверкаОстатковПрочихДоходов(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		"ВТЗаданияКРаспределениюДоходовПоНД",
		НСтр("ru='По организации ""%1"" на конец периода %2 не выполнено распределение прочих доходов по направлениям деятельности.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		,
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка, ДопПараметры)
	...
	|ПОМЕСТИТЬ ДанныеДокументаРасходы
	|ИЗ
	|	Документ.РаспределениеДоходовПоНаправлениямДеятельности.Расходы КАК ДанныеДокумента
	|ГДЕ
	...
	|	ЕСТЬNULL(ДанныеДокументаРасходы.ЕстьПрочиеРасходы, ЛОЖЬ) КАК ЕстьПрочиеРасходы
	|ИЗ
	|	Документ.РаспределениеДоходовПоНаправлениямДеятельности КАК ДанныеДокумента
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеДокументаРасходы КАК ДанныеДокументаРасходы
	|		ПО ДанныеДокументаРасходы.Ссылка = ДанныеДокумента.Ссылка
	|ГДЕ
	|	ДанныеДокумента.Ссылка = &Ссылка
	|";
	...
	Запрос.УстановитьПараметр("ЕстьПрочиеРасходы", Реквизиты.ЕстьПрочиеРасходы);
...
Функция ТекстЗапросаТаблицаПрочиеДоходы(Запрос, ТекстыЗапроса, Регистры)
	...
	|	И ТаблицаДоходы.ИсходноеНаправлениеДеятельности <> ТаблицаДоходы.НаправлениеДеятельности
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ТаблицаДоходы.АналитикаДоходов           КАК АналитикаДоходов,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РаспределениеДоходовПоНаправлениямДеятельности) КАК ХозяйственнаяОперация,
	|	ТаблицаДоходы.ИсходноеНаправлениеДеятельности    КАК КорНаправлениеДеятельности,
	|
	|	СУММА(ТаблицаДоходы.Доходы)              КАК Сумма,
	|	СУММА(ТаблицаДоходы.ДоходыУпр)           КАК СуммаУпр,
	|	СУММА(ТаблицаДоходы.ДоходыРегл)          КАК СуммаРегл,
