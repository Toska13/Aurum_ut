...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.КроссТаблица Тогда
		...
		Если ЗаполнятьСкладВТЧ И Не ЗаполнятьПоДаннымСервиса Тогда
...
Функция ПолучитьПараметрыВидаПлана(Знач ВидПлана, Знач ОбновлятьПравило, АдресПравилаЗаполнения, АдресПользовательскихНастроек)
	...
	Реквизиты = "КоличествоПериодов, ЗаполнятьПодразделение, ЗаполнятьСклад, ЗаполнятьСкладВТЧ, 
		|ЗапретитьРедактированиеПравила, ЗаполнятьПоФормуле, ЗаполнятьПоДаннымСервиса, Замещающий, ЗаполнятьФорматМагазина,
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Если Объект.ЗаполнятьПоФормуле
			...
			ПланированиеКлиентСервер.ОтобразитьЗначениеДополнительныхПараметров(
				ЭтотОбъект, ПолучитьДопРеквизиты(ЭтотОбъект), "ТоварныеКатегорииПоПериодам");
		КонецЕсли;
