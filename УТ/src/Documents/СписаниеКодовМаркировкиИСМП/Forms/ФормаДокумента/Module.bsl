...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если Не Объект.Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийИСМП.СписаниеЧерезСУЗМех") Тогда
		...
		Если ТекущиеДанные <> Неопределено Тогда
			...
			ТребуетсяОбновитьКэшУпаковок = ПроверкаИПодборПродукцииИСКлиент.ПрименитьКешПоСтроке(
				...
				ДанныеКешаСтроки,
				Истина,
				КлючСвязиСтатусаСтрок(Объект.ТоварнаяГруппа, ТекущиеДанные.Номенклатура));
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Не РедактированиеФормыНедоступно Тогда
		ПроверкаИПодборПродукцииИС.ПрименитьКешШтрихкодовУпаковок(ЭтотОбъект,,,КлючСвязиСтатусаСтрок(Объект.ТоварнаяГруппа));
	КонецЕсли;
...
Процедура ТоварнаяГруппаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ИначеЕсли Объект.ОтчетПроизводственнойЛинии
		...
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ВопросПриИзмененииТоварнойГруппыПриЗавершении", ЭтотОбъект, ВыбранноеЗначение),
			НСтр("ru = 'При изменении товарной группы документ-основание, табличная часть ""Коды маркировки""
...
Процедура ВидПервичногоДокументаПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.ВидПервичногоДокумента)
		...
		И МаркировкаТоваровКлиентСервер.ДоступноПолеВидПервичногоДокумента(Объект.ТоварнаяГруппа, Объект.Операция) Тогда
...
Процедура ЗагрузитьИзВнешнегоФайла(Команда)
	...
	ДоступнаИерархия = Объект.ОтчетПроизводственнойЛинии
		И КонструкторИСМПКлиентСервер.ВидПродукцииИспользуетПередачуОтчетаОбОтбраковкеЧерезСУЗ(Объект.ТоварнаяГруппа);
...
Процедура ПослеОбработкиШтрихкодовСервер(РезультатОбработкиШтрихкода, КэшированныеЗначения)
	...
	Если Объект.Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийИСМП.СписаниеЧерезСУЗМех") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПрименитьКешШтрихкодовУпаковок()
	...
	Если Не РедактированиеФормыНедоступно Тогда
		ПроверкаИПодборПродукцииИС.ПрименитьКешШтрихкодовУпаковок(ЭтотОбъект, , Истина, КлючСвязиСтатусаСтрок(Объект.ТоварнаяГруппа));
	КонецЕсли;
...
Процедура ОбновитьСтатусИСМП()
	...
	СтатусДействияПредставление = ОбменДаннымиИСМП.ПредставлениеСтатуса(СтатусИСМП, ДальнейшееДействие, ДопустимыеДействия);
...
Процедура СформироватьПредставлениеСтатусНастройкиСУЗ()
	...
	Если ЗначениеЗаполнено(Объект.Организация) 
		И (Объект.Операция = Перечисления.ВидыОперацийИСМП.СписаниеЧерезСУЗ
		Или Объект.Операция = Перечисления.ВидыОперацийИСМП.СписаниеЧерезСУЗМех) Тогда
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	Если Не Объект.Ссылка.Пустая() Тогда
		ДанныеДокумента = СтруктураИсходныхДанныхДокумента();
...
Процедура ВопросПриИзмененииТоварнойГруппыПриЗавершении(РезультатВопроса, ВыбраннаяТоварнаяГруппа) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да
		...
		Если Не МаркировкаТоваровКлиентСервер.ДоступенОтчетПроизводственнойЛинии(Объект.ТоварнаяГруппа, Объект.Операция) Тогда
...
Процедура НастроитьЭлементыФормы(Форма)
	...
	ПричиныСписания = СписаниеКодовМаркировкиКлиентСервер.ДоступныеПричиныСписания(ВидыПродукцииПоТоварнойГруппе);
	
	Элементы.ПричинаСписания.СписокВыбора.Очистить();
	Элементы.ПричинаСписания.СписокВыбора.ЗагрузитьЗначения(ПричиныСписания);
	Элементы.ПричинаСписания.СписокВыбора.СортироватьПоЗначению();
	
	ДоступныеОперацииСписания = СписаниеКодовмаркировкиКлиентСервер.ДоступныеОперацииСписания(ВидыПродукцииПоТоварнойГруппе, Объект.ПричинаСписания);

	Элементы.Операция.СписокВыбора.Очистить();
	Элементы.Операция.СписокВыбора.ЗагрузитьЗначения(ДоступныеОперацииСписания);
	Элементы.Операция.СписокВыбора.СортироватьПоЗначению();
	
	Если ДоступныеОперацииСписания.Количество() = 1 Тогда
		Объект.Операция = ДоступныеОперацииСписания[0];
	КонецЕсли;
	
	Элементы.Операция.ТолькоПросмотр = ДоступныеОперацииСписания.Количество() = 1;
	
	Элементы.ВидПервичногоДокумента.СписокВыбора.Очистить();

	ДоступноПолеВидПервичногоДокумента = МаркировкаТоваровКлиентСервер.ДоступноПолеВидПервичногоДокумента(
		ВидыПродукцииПоТоварнойГруппе, Объект.Операция);

	Если ДоступноПолеВидПервичногоДокумента Тогда
		ВидыПервичногоДокумента = ДоступныеВидыПервичныхДокументов(Объект.ПричинаСписания, Объект.Операция);
	...
	ПередачаОтчетаЧерезСУЗ = КонструкторИСМПКлиентСервер.ВидПродукцииИспользуетПередачуОтчетаОбОтбраковкеЧерезСУЗ(ВидыПродукцииПоТоварнойГруппе);

	ЭтоСписаниеЧерезСУЗМех = Объект.Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийИСМП.СписаниеЧерезСУЗМех");
	
	Элементы.ОтчетПроизводственнойЛинии.Видимость = МаркировкаТоваровКлиентСервер.ДоступенОтчетПроизводственнойЛинии(ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
	Элементы.СтраницаТовары.Видимость             = Не Объект.ОтчетПроизводственнойЛинии И Не ЭтоСписаниеЧерезСУЗМех;
	Элементы.СтраницаОтчетПроизводственнойЛинии.Видимость = Объект.ОтчетПроизводственнойЛинии;
	Элементы.СтраницаПредставлениеКИЗ.Видимость           = ЭтоСписаниеЧерезСУЗМех;

	ОтображатьИсходныеЗначенияКодов = Объект.ОтчетПроизводственнойЛинии И Не Форма.РедактированиеФормыНедоступно;
	...
	Элементы.GLNОрганизации.Видимость = МаркировкаТоваровКлиентСервер.ДоступноПолеGLNОрганизации(
		ВидыПродукцииПоТоварнойГруппе, Объект.Операция);

	ДоступноПолеПричинаСписанияВТабличнойЧасти = СписаниеКодовМаркировкиКлиентСервер.ДоступноПолеПричинаСписанияВТабличнойЧасти(
		ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
	
	Элементы.ТоварыПричинаСписания.Видимость = ДоступноПолеПричинаСписанияВТабличнойЧасти;
	Если ДоступноПолеПричинаСписанияВТабличнойЧасти Тогда
		Элементы.ТоварыПричинаСписания.СписокВыбора.Очистить();
		ПричиныСписания = СписаниеКодовМаркировкиКлиентСервер.ДоступныеПричиныСписания(ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
		Элементы.ТоварыПричинаСписания.СписокВыбора.ЗагрузитьЗначения(ПричиныСписания);
		Элементы.ТоварыПричинаСписания.СписокВыбора.СортироватьПоЗначению();
	КонецЕсли;

	Элементы.ПроизводственныйОбъект.Видимость = МаркировкаТоваровКлиентСервер.ДоступноПолеПроизводственныйОбъект(
		ВидыПродукцииПоТоварнойГруппе, Объект.ПричинаСписания);

	Элементы.ГруппаПроизводственныйОбъектЛиния.Видимость = МаркировкаТоваровКлиентСервер.ДоступноПолеИдентификаторПроизводственнойЛинии(
		ВидыПродукцииПоТоварнойГруппе, Объект.ПричинаСписания);

	Элементы.ПредставлениеАдресаПроизводственногоОбъекта.Видимость = СписаниеКодовМаркировкиКлиентСервер.ДоступноПредставлениеАдресаПроизводственногоОбъекта(
		ВидыПродукцииПоТоварнойГруппе, Объект.ПричинаСписания);

	Элементы.ГруппаКодФИАС.Видимость = СписаниеКодовМаркировкиКлиентСервер.ДоступноПолеКодФИАС(
		ВидыПродукцииПоТоварнойГруппе, Объект.ПричинаСписания);
	...
	ОбщегоНазначенияИСКлиентСервер.УстановитьДоступностьЭлементовФормы(Форма, ЗависимыеОтСтатусаИСМП,
	...
	ОбщегоНазначенияИСКлиентСервер.УстановитьДоступностьЭлементовФормы(Форма, ЗависимыеОтСтатусаПроверкиИПодбора,
...
Процедура ДополнитьДоступныеПричиныВывода()
	...
	Если ЭтоАдресВременногоХранилища(ДанныеДокументаАдрес) Тогда
		...
		Если ДанныеДокумента.ТоварнаяГруппа = Объект.ТоварнаяГруппа
				И ЗначениеЗаполнено(ДанныеДокумента.ПричинаСписания)
				И Элементы.ПричинаСписания.СписокВыбора.НайтиПоЗначению(ДанныеДокумента.ПричинаСписания) = Неопределено Тогда
			Элементы.ПричинаСписания.СписокВыбора.Добавить(ДанныеДокумента.ПричинаСписания);
		КонецЕсли;
	КонецЕсли;
...
Процедура ДополнитьДоступныеОперации()
	...
	Если ЭтоАдресВременногоХранилища(ДанныеДокументаАдрес) Тогда
		ДанныеДокумента = ПолучитьИзВременногоХранилища(ДанныеДокументаАдрес);
		Если ДанныеДокумента.ТоварнаяГруппа = Объект.ТоварнаяГруппа
			И ДанныеДокумента.ПричинаСписания = Объект.ПричинаСписания
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	ТребуетсяУказаниеНомераИДатыПервичногоДокумента = МаркировкаТоваровКлиентСервер.ПроверятьЗаполнениеПоляНомерДатаПервичногоДокумента(
		Объект.ТоварнаяГруппа, Объект.Операция);
		
	ТребуетсяУказаниеВидаПервичногоДокумента = МаркировкаТоваровКлиентСервер.ПроверятьЗаполнениеПоляВидПервичногоДокумента(
		Объект.ТоварнаяГруппа, Объект.Операция);
	
	Элементы.ВидПервичногоДокумента.АвтоОтметкаНезаполненного = ТребуетсяУказаниеВидаПервичногоДокумента;
	Элементы.ДатаПервичногоДокумента.АвтоОтметкаНезаполненного  = ТребуетсяУказаниеНомераИДатыПервичногоДокумента;
	Элементы.НомерПервичногоДокумента.АвтоОтметкаНезаполненного = ТребуетсяУказаниеНомераИДатыПервичногоДокумента;
	...
	Элементы.НаименованиеПервичногоДокумента.АвтоОтметкаНезаполненного = Элементы.НаименованиеПервичногоДокумента.Видимость
		И (ТребуетсяУказаниеНомераИДатыПервичногоДокумента Или ТребуетсяУказаниеВидаПервичногоДокумента);
...
&НаКлиентеНаСервереБезКонтекста
Функция КлючСвязиСтатусаСтрок(ТоварнаяГруппа = Неопределено, Номенклатура = Неопределено)
...
Процедура ЗаполнитьGLNОргранизацииПоСтатистике()
	...
	Если Не МаркировкаТоваровКлиентСервер.ДоступноПолеGLNОрганизации(Объект.ТоварнаяГруппа)
...
&НаСервере
Процедура ПричинаСписанияПриИзмененииНаСервере()
...
Процедура ТоварыНоменклатураПриИзмененииСервер()
	...
	СобытияФормИС.КлючВидовПродукцииЗаполнитьДанные(ЭтотОбъект, ТекущиеДанные);
	// Конец РаботаСКлючомВидовПродукции
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ДоступныеВидыПервичныхДокументов(ПричинаСписания, Операция)
	
	Возврат Справочники.ПричиныВыводаИзОборотаИСМП.ДоступныеВидыПервичныхДокументов(ПричинаСписания, Операция);
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция СтруктураИсходныхДанныхДокумента()
	
	Результат = Новый Структура();
	Результат.Вставить("ТоварнаяГруппа");
	Результат.Вставить("ПричинаСписания");
