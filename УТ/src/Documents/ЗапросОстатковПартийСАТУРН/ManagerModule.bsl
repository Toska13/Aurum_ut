...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ОбновлениеИнформационнойБазыСАТУРН.ДобавитьОбработчикПриПереходеНаНовуюВерсиюAPI(
		...
		Новый УникальныйИдентификатор("1f3d3e45-c235-491a-b011-4e2434c681b1"),
		Обработчики);
...
Процедура ОбработкаЗагрузкиПолученныхДанных(ЭлементОчереди, ПараметрыОбмена, ПолученныеДанные, ИзмененныеОбъекты) Экспорт
	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийСАТУРН.ЗапросОстатковПартий Тогда
		...
		Для Каждого ЭлементДанных Из ВходящиеДанные Цикл
			...
			Если ОписаниеТипаИдентификатор.ПривестиЗначение(ЭлементДанных.ContractorId) <> ПараметрыЗапросаОснования.ОрганизацияСАТУРНИдентификатор Тогда
				Продолжить;
			КонецЕсли;
			
			ИдентификаторПартии = Формат(ЭлементДанных.batchId, "ЧГ=0;");
			
			КоличествоСАТУРН = ОписаниеТипаКоличествоСАТУРН.ПривестиЗначение(ЭлементДанных.countNetto);
			...
			Если Не ЗначениеЗаполнено(Партия) Тогда
				Партия = Справочники.ПартииСАТУРН.Партия(
					ИдентификаторПартии, ПараметрыОбмена, ЭлементОчереди.ОрганизацияСАТУРН, ПараметрыПолученияПартии);
			КонецЕсли;
			
			СтрокаДанныхПартии = Новый Структура();
			СтрокаДанныхПартии.Вставить("Партия",           Партия);
			СтрокаДанныхПартии.Вставить("КоличествоСАТУРН", ЭлементДанных.countNetto);
			...
			СтрокаДанныхПартии.Вставить(
				...
				Справочники.КлассификаторПАТСАТУРН.ПАТ(
					Формат(ЭлементДанных.patId, "ЧГ=0;"),
					ПараметрыОбмена,
					ЭлементОчереди.ОрганизацияСАТУРН));
...
Функция ЗапросОстатковПартийJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	СообщениеJSON = СообщенияЗагрузкиОстатковПартий(ПараметрыФормирования, ДополнительныеПараметры.ПараметрыОбмена, АбстрактноеСообщениеJSON);
...
Функция СообщенияЗагрузкиОстатковПартий(ПараметрыФормирования, ПараметрыОбмена, РеквизитыИсходящегоСообщения)
	...
	Если ЗначениеЗаполнено(ПараметрыФормирования.МестоХраненияИдентификатор) Тогда
		...
		МестоХраненияИдентификатор = ОписаниеТипаЧисло.ПривестиЗначение(ПараметрыФормирования.МестоХраненияИдентификатор);
		ИнтерфейсСАТУРН.ДобавитьАргументtheCard(АргументыОперации, "warehouseId", МестоХраненияИдентификатор);
	Иначе
		ИнтерфейсСАТУРН.ДобавитьАргументtheCard(АргументыОперации, "warehouseId", Неопределено);
	КонецЕсли;
...
Процедура СоздатьСообщенияЗагрузкиРодительскихПартий(ПараметрыЗапросаОснования, РеквизитыИсходящегоСообщения, ПараметрыОбмена, ЭлементОчереди)
	...
	Для Каждого ГруппаИдентификаторовПартий Из ГруппыИдентификаторовПартий Цикл
		...
		АргументыОперации = СообщениеJSON.АргументыОперации;
		АргументыОперации.size = ПараметрыЗапросаОснования.КоличествоНаСтранице;
		
		ИнтерфейсСАТУРН.ДобавитьУсловиеОтбора(АргументыОперации, "id", ГруппаИдентификаторовПартий);
