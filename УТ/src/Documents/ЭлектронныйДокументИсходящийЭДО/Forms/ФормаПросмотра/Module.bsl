...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		УстанавливатьПрочтениеСообщений = Истина;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	ОбновитьДанныеКонтрагентаИзСервисаНастроек();
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ЭтоНовыйДокумент") Тогда
		ЭтоНовыйДокумент = ПараметрыЗаписи.ЭтоНовыйДокумент;
	Иначе
		ЭтоНовыйДокумент = Не ЗначениеЗаполнено(Объект.Ссылка);
		ПараметрыЗаписи.Вставить("ЭтоНовыйДокумент", ЭтоНовыйДокумент);
	КонецЕсли;
	
	ЭлементыСхемыРегламента = Неопределено;
	Если Не ПараметрыЗаписи.Свойство("ЭлементыСхемыРегламента", ЭлементыСхемыРегламента) Тогда
		ЭлементыСхемыРегламента = ОбщегоНазначенияБЭДКлиентСервер.ПолучитьВсеЭлементыДереваРекурсивно(СхемаРегламента);
	КонецЕсли;
	...
	Если Не ПараметрыЗаписи.Свойство("КонтекстДиагностики") Тогда
	...
	КонецЕсли;
	Если Не ЭтоНовыйДокумент И ИзмененМаршрутПодписания Тогда
		ПараметрыЗаписи.Вставить("ИзменениеМаршрутаПодписания");
	КонецЕсли;
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	КонецЕсли;
	Если ПараметрыЗаписи.Свойство("ИзменениеМаршрутаПодписания") Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ВыборТекущегоДокументаПакета"
		И ЗначениеЗаполнено(ИдентификаторПакета) И Источник = ИдентификаторПакета Тогда
...
Процедура Подключаемый_СоставПакета_Нажатие(Элемент, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		СтандартнаяОбработка = Ложь;
		ОписаниеОповещения = Новый ОписаниеОповещения("СоставПакета_НажатиеПослеЗаписиПослеЗаписи",
			ЭтотОбъект, Элемент);
		НачатьЗаписьДокумента(ОписаниеОповещения);
	Иначе
...
Процедура ГруппаСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница.Имя = "СтраницаСообщения" И УстанавливатьПрочтениеСообщений Тогда
		УстановитьПрочтениеСообщенийДокумента(Объект.Ссылка);
		УстанавливатьПрочтениеСообщений = Ложь;
	КонецЕсли;
...
Процедура ТранслитироватьИмяФайла(Команда)
	...
	ОбновитьИмяФайлаСохранитьНастройкиТранслитерации();
...
Процедура ПоказатьКомментарийСостояния(Команда)
	...
	ИнтерфейсДокументовЭДОКлиент.ПоказатьКомментарийКСостояниюДокумента(КомментарийСостояния);
	...
	Иначе
		...
		Если Не ОкноОтправкиВФормализованномВидеПоказано
			...
			Обработчик = Новый ОписаниеОповещения(
				"ПоказатьОкноПредложенияОтправкиВФормализованномВидеПослеОтветаНаВопрос",
...
Процедура ЗапроситьУточнение(Команда)
	...
	ПакетнаяОбработка = СостоянияПакетаОднородно И ЗначениеЗаполнено(ИдентификаторПакета);
	ЗапроситьУточнениеПоОбъектам(ПакетнаяОбработка);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗапроситьУточнениеДокумент(Команда)
...
Процедура ПоказатьПротоколОбмена(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ТекстВопроса = НСтр("ru = 'Открытие протокола возможно только после записи документа.
									|Записать документ?'");
		Оповещение = Новый ОписаниеОповещения("ОткрытьПротоколОбменаПослеВопроса", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура ОтправитьСообщение(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВыполненияДействийПоЭДО", ЭтотОбъект, Новый Соответствие);
	ЭлектронныеДокументыЭДОКлиент.ОтправитьДополнительнуюИнформацию(ОбъектыДействий, ОписаниеОповещения, ТекстСообщения);
КонецПроцедуры
...
Процедура ВключитьПолеПредпросмотраPDFФайлов(Команда)
	ВключитьПолеПредпросмотраPDFФайловНаСервере();
	ОжидатьФормированияПредставленияДокумента();
КонецПроцедуры
...
&НаСервере
Процедура ОбновитьИмяФайлаСохранитьНастройкиТранслитерации()
	
	ИмяФайла = РаботаСФайламиБЭД.ПреобразоватьИмяФайлаСУчетомТранслитерации(ИсходноеИмяФайла, ОтключитьТранслитерацию);
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Подписи.ПодписьПоДоверенностиВерна");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Ложь;
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	ЗаполнитьЗначенияСвойств(ПараметрыОпределенияИспользованияУС, Объект);
	ИспользуютсяУниверсальныеСообщения = РегламентыЭДО.ИспользоватьУниверсальныеСообщения(ПараметрыОпределенияИспользованияУС);
	...
	ИдентификаторТекущегоЭлемента = ИдентификаторЭлементаСхемыРегламента(СхемаРегламента, Сообщение);
	...
	ИнтерфейсДокументовЭДО.НастроитьСтраницуСообщений(ЭтотОбъект, ЭлементСхемы);
	
	ИнтерфейсДокументовЭДО.НастроитьКомандуПодключенияИспользованияПредпросмотраФайловPDF(
		Элементы.ВключитьПолеПредпросмотраPDFФайлов, ЕстьПравоАдминистрирования);
	...
	ИначеЕсли ЗначениеЗаполнено(Параметры.ПросмотрПодготовленныхДанных) Тогда
		...
		Иначе
			ЗаполнитьДанныеПредварительногоЭлектронногоДокумента(НастройкиФормирования.НастройкиОтправки,
				ДанныеПредварительногоПросмотра);
		КонецЕсли;
		Если Параметры.ПросмотрПодготовленныхДанных.Свойство("АдресПредупреждений")
			И ЗначениеЗаполнено(Параметры.ПросмотрПодготовленныхДанных.АдресПредупреждений) Тогда
			МассивУведомленийПользователю = ПолучитьИзВременногоХранилища(Параметры.ПросмотрПодготовленныхДанных.АдресПредупреждений);
...
Процедура ЗаполнитьДанныеТекущегоЭлектронногоДокумента()
	...
	ЭлектронныйДокумент = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ЭлектронныйДокумент");
	ИдентификаторТекущегоЭлемента = ИдентификаторЭлементаСхемыРегламента(СхемаРегламента, ЭлектронныйДокумент);
...
Процедура ЗаполнитьРеквизитыПоФормализованномуДокументу(ДвоичныеДанные, РезультатРаспознания, ЭлементСхемы)
	...
	Если Не РезультатРаспознания.Свойство("ИдентификаторДокумента") 
		Или Не ЗначениеЗаполнено(РезультатРаспознания.ИдентификаторДокумента)
	...
	Если Не ЭлектронныеДокументыЭДО.ТипДокументаПоддерживаетЗагрузкуИзФайлаВФормализованномВиде(
		ТипДокумента, Объект.ТипРегламента) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ФайлСформированВ1С = Ложь Тогда
		...
		Если Не ЗначениеЗаполнено(Подписант.ИдентификаторОрганизации) Тогда
			ЗагружаемыеДанные = ФорматыЭДО.ДанныеФайлаЭД(ОписаниеФайла, ОписаниеОшибки);		
			...
			Если ЧастиИмениФайлаЗагружаемыеДанные.Количество() >= 3 Тогда
				Подписант.ИдентификаторОрганизации = ЧастиИмениФайлаЗагружаемыеДанные[3];
			КонецЕсли;
		КонецЕсли;
...
Процедура НастроитьВидимостьИНаименованиеДействийПоСостояниюЭДО()
	...
	Элементы.Отклонить.Видимость = ЕстьДействиеПоЭДО(ДействияНаЭтапе, Перечисления.ДействияПоЭДО.Отклонить) 
		И Не ИспользуютсяУниверсальныеСообщения;
	Элементы.ОтклонитьДокумент.Видимость = ЕстьДействиеПоЭДО(ДействияНаЭтапеДляДокумента,
		Перечисления.ДействияПоЭДО.Отклонить) 
		И Не ИспользуютсяУниверсальныеСообщения;
	
	Элементы.ЗапроситьУточнение.Видимость = ЕстьДействиеПоЭДО(ДействияНаЭтапе, Перечисления.ДействияПоЭДО.ЗапроситьУточнение)
		И ИспользуютсяУниверсальныеСообщения;
	Элементы.ЗапроситьУточнениеДокумент.Видимость = ЕстьДействиеПоЭДО(ДействияНаЭтапеДляДокумента, Перечисления.ДействияПоЭДО.ЗапроситьУточнение)
		И ИспользуютсяУниверсальныеСообщения;
...
Процедура ПоказатьПредставлениеДокумента(ЭлементСхемыРегламента)
	...
	Элементы.СодержимоеPdfДокумента.Видимость = Ложь;
...
Процедура ПодготовитьПредставлениеДокумента(ЭлементСхемыРегламента)
	...
	ИначеЕсли РезультатФормирования.ПредставлениеДокумента = Неопределено И Не ЭлементСхемыРегламента.Распознан Тогда
		ПодготовитьПредставлениеФайла(ЭлементСхемыРегламента);
		Если РезультатФормирования.Свойство("ТекстОшибки") И ЗначениеЗаполнено(РезультатФормирования.ТекстОшибки) Тогда
			ПодготовитьПредставлениеОшибки(ЭлементСхемыРегламента);
	...
	Если ТипЗнч(ПредставлениеДокумента) = Тип("ДвоичныеДанные") Тогда
		...
		Иначе
			ЭлементСхемыРегламента.ИмяРеквизита = "Файл";
			Элементы.СодержимоеPdfДокумента.Видимость = Истина;
			Элементы.ДекорацияПустая.Видимость    = Ложь;
		КонецЕсли;
		
	Иначе
		
		Если ТипЗнч(ПредставлениеДокумента) = Тип("Строка") Тогда
...
Функция ИдентификаторЭлементаСхемыРегламента(ЭлементСхемыРодитель, Сообщение = Неопределено)

	НайденныйИдентификатор = ИнтерфейсДокументовЭДОКлиентСервер.ИдентификаторЭлементаСхемыРегламента(ЭлементСхемыРодитель,
		Сообщение);
	...
	Если НайденныйИдентификатор = Неопределено Тогда
		...
		Если КоллекцияЭлементовСхемы.Количество() > 0 Тогда
			НайденныйИдентификатор = КоллекцияЭлементовСхемы[0].ПолучитьИдентификатор();
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьПодписи(ЭлементСхемы)
	...
	Для Каждого СтрокаПодпись Из УстановленныеПодписи Цикл
		...
		НоваяСтрока.ПодписьПоДоверенностиВерна = Не КриптографияБЭД.ПриПроверкеПодписиВозниклиОшибки(СтрокаПодпись);
		Если Не СтрокаПодпись.ЭтоПодписьПоДоверенности Тогда
			НоваяСтрока.ПодписьПоДоверенностиВерна = НоваяСтрока.ПодписьВерна;
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьВПакетФайлСКомпьютера()
	...
	ПараметрыДобавленияВПакет = ПараметрыДобавленияВПакет();
...
// Обход ошибки 70122180
// 
// Возвращаемое значение:
//  Булево
&НаКлиенте
Функция ИдентификаторИнвалидирован(ИдентификаторСтроки)
	Возврат СхемаРегламента.НайтиПоИдентификатору(ИдентификаторСтроки) = Неопределено;
КонецФункции
...
#Если ВебКлиент Тогда
		...
		Если ИдентификаторИнвалидирован(ИдентификаторЭлементаСхемы) Тогда
			Возврат;
		КонецЕсли;
...
Процедура ПоказатьПредставлениеДанныхДокументаПослеФормирования(Результат, Контекст) Экспорт
	...
	Если ИдентификаторТекущегоЭлемента <> Контекст.ИдентификаторТекущегоЭлемента Тогда
		Возврат;
	КонецЕсли;
	...
	ПредставлениеДокумента = РезультатФормирования.ПредставлениеДокумента;
	Если ТипЗнч(ПредставлениеДокумента) = Тип("ДвоичныеДанные") Тогда
		ИнтерфейсДокументовЭДОКлиент.ВывестиPDFДокументНаФорму(ЭтотОбъект, СведенияОPdfДокументе, ПредставлениеДокумента);
	КонецЕсли;
...
Процедура ПослеВыполненияДействийПоЭДО(Результат, НаборДействий) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(Результат)
		И ЗначениеЗаполнено(Результат.ОбработаноПоДействиям[ПредопределенноеЗначение("Перечисление.ДействияПоЭДО.СформироватьДополнительнуюИнформацию")]) Тогда
		ТекстСообщения = "";
	КонецЕсли;
...
Процедура ОбновитьДанныеЭлектронногоДокументаНаСервере()
	...
	ИдентификаторТекущегоЭлемента = ИдентификаторЭлементаСхемыРегламента(СхемаРегламента);
...
Процедура ПриИзмененииКлючевыхРеквизитов()
	...
	Если ЗначениеЗаполнено(НастройкиОтправки)
		И УчетныеЗаписиЭДО.ИспользоватьЦентрЭДО(НастройкиОтправки.ИдентификаторОтправителя) Тогда
		ИнтерфейсДокументовЭДО.ПроверитьВозможностьИспользованияУчетнойЗаписиЦентраЭДО(
...
Процедура ДобавитьВложениеИзТабличногоДокумента(ТабличныйДокумент, НаименованиеФайла)
	...
	Расширение = "pdf";
	ИсходноеИмяФайла = СтрШаблон("%1.%2", Наименование, Расширение);
...
Процедура ДобавитьВложениеИзДвоичныхДанных(ДвоичныеДанные, НаименованиеФайла, Расширение)
	...
	ИсходноеИмяФайла = СтрШаблон("%1.%2", НаименованиеФайла, Расширение);
...
&НаКлиенте
Процедура ОткрытьПротоколОбменаПослеВопроса(Ответ, ДополнительныеДанные) Экспорт
	...
	НачатьЗаписьДокумента(Новый ОписаниеОповещения("ПоказатьПротоколОбменаПослеЗаписи", ЭтотОбъект));
КонецПроцедуры
...
Процедура УстановитьПрочтениеСообщенийДокумента(ЭлектронныйДокумент)
	РегистрыСведений.ТекстовыеСообщенияДокументовЭДО.УстановитьПрочтениеСообщенийДокумента(ЭлектронныйДокумент);
КонецПроцедуры

&НаКлиенте
Процедура ЗапроситьУточнениеПоОбъектам(ПакетнаяОбработка)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВыполненияДействийПоЭДО", ЭтотОбъект, Новый Соответствие);
	ЭлектронныеДокументыЭДОКлиент.ЗапроситьУточнение(ОбъектыДействий, ОписаниеОповещения);
...
Процедура УстановитьНовыйНомерДокумента(ТекущийОбъект)
	...
	Если Не ТекущийОбъект.ЭтоНовый()
		ИЛИ ЗначениеЗаполнено(ТекущийОбъект.НомерДокумента)
		ИЛИ ЭлементСхемыИнформацияОтправителя(ЭтотОбъект).ФормированиеПоОбъектуУчета Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьВложениеИзФайлаНаДискеЗавершить(ОписаниеПомещенногоФайла, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РаботаСФайламиБЭДКлиент.ФайлМожноОтправитьПоЭДО(ОписаниеПомещенногоФайла.Имя) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьИВложитьФайл(ПолноеИмяФайла, АдресВХранилище)
	...
	Если НЕ РаботаСФайламиБЭДКлиент.ФайлМожноОтправитьПоЭДО(ПолноеИмяФайла) Тогда
		Возврат;
	КонецЕсли;
	...
	РезультатыПроверкиИмени = 
		РаботаСФайламиБЭДКлиент.ОписаниеФайлаПослеПроверкиКорректностиИмениПоИмениИАдресуВоВременномХранилище(
			ИмяФайла, АдресВХранилище, ТранслитерацияОтключена);
	...
	ДобавитьВложениеИзФайлаНаСервере(РезультатыПроверкиИмени.ОписаниеФайла.ИмяФайла, АдресВХранилище);
...
&НаКлиенте
Процедура ЗагрузитьИзФайлаЗавершить(РезультатВыполнения, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура СоставПакета_НажатиеПослеЗаписиПослеЗаписи(Результат, Элемент) Экспорт
	
	Если Результат <> Истина Тогда
		Возврат;
	КонецЕсли;

	ПакетыДокументовЭДОКлиент.ЭлементУправленияПакета_Нажатие(ЭтотОбъект, Элемент, Ложь);
...
#Область ОбновлениеДанныхКонтрагентаИзСервисаНастроек

&НаКлиенте
Процедура ОбновитьДанныеКонтрагентаИзСервисаНастроек()
	
	Если Не ТребуетсяОбновлениеДанныхКонтрагентаИзСервисаНастроек Тогда
		Возврат;
	КонецЕсли;
	
	ОбработчикЗавершения = Новый ОписаниеОповещения("ОбновитьДанныеКонтрагентаИзСервисаНастроекПослеОбновления",
		ЭтотОбъект);
	РаботаСАбонентамиЭДОКлиент.ОбновитьДанныеАбонентаПоИдентификатору(Объект.ИдентификаторКонтрагента,
		ОбработчикЗавершения);
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьДанныеКонтрагентаИзСервисаНастроекПослеОбновления(РезультатОбновления, ПараметрыОповещения) Экспорт
	
	ОбработкаНеисправностейБЭДКлиент.ОбработатьОшибки(РезультатОбновления.КонтекстДиагностики);
	
	СформироватьПредупреждения();
	
	ТребуетсяОбновлениеДанныхКонтрагентаИзСервисаНастроек = Ложь;
	
КонецПроцедуры

#КонецОбласти // ОбновлениеДанныхКонтрагентаИзСервисаНастроек

&НаСервере
Процедура ВключитьПолеПредпросмотраPDFФайловНаСервере()
	НастройкиЭДО.УстановитьИспользованиеПросмотраPDFДокументовЭДО(Не Элементы.ВключитьПолеПредпросмотраPDFФайлов.Пометка);
	ИнтерфейсДокументовЭДО.НастроитьКомандуПодключенияИспользованияПредпросмотраФайловPDF(
		Элементы.ВключитьПолеПредпросмотраPDFФайлов, ЕстьПравоАдминистрирования);
	ОбновитьПредставлениеЭлектронногоДокумента();
КонецПроцедуры
