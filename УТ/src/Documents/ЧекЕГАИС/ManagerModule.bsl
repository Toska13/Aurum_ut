...
Процедура ПриИзмененииСтатусаЧека(ДокументСсылка, ПредыдущийСтатус, НовыйСтатус, ПараметрыОбновленияСтатуса) Экспорт
	...
	ПроверятьЧастичноеВыбытие = ИнтеграцияЕГАИС.ИспользуетсяЧастичноеВыбытие();
...
//
// Параметры:
//  ДокументСсылка - ДокументСсылка.ЧекЕГАИСВозврат - Документ.
//  ПредыдущийСтатус - ПеречислениеСсылка.СтатусыИнформированияЕГАИС - Предыдущий статус.
//  НовыйСтатус - ПеречислениеСсылка.СтатусыИнформированияЕГАИС - Предыдущий статус.
//  ПараметрыОбновленияСтатуса - Структура - см. функцию ОбменДаннымиЕГАИС.ПараметрыОбновленияСтатуса().
//
...
//  ДокументСсылка - ДокументСсылка.ЧекЕГАИС - Документ, для которого выполняется заполнение
...
// Обработать строки табличной части.
// 
// Параметры:
//  ДокументОбъект - ДокументОбъект.ЧекЕГАИС - заполняемый объект
//  ПараметрыЗаполнения - См. АкцизныеМаркиЕГАИС.ПараметрыЗаполненияТоваровИАкцизныхМарок
...
Функция СообщенияЧекЕГАИСXML(ДокументСсылка, РезультатыЗапроса, МенеджерВременныхТаблиц, Операция = Неопределено) Экспорт
	...
	ФорматОбменаМенееV3 = Не Перечисления.ФорматыОбменаЕГАИС.ФорматОбменаНеМенееV3(ФорматОбмена);
	...
	Для Каждого СтрокаТЧ Из ВложенныеШтрихкодыУпаковок.МаркированныеТовары Цикл
		...
		Если СтрокаТЧ.ЧастичноеВыбытие Тогда
			...
			КонецЕсли;
			Если ФорматОбменаМенееV3 Тогда
				ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(
					СообщениеXML,
					СтрШаблон(
	...
	ИначеЕсли Не ФорматОбменаМенееV3 Тогда
...
// Таблица алкогольной продукции к определению справок2.
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка.ЧекЕГАИС - Документ ссылка
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Таблица алкогольной продукции к определению справок2
...
//  ТаблицаНеМаркируемойПродукции - ТаблицаЗначений - таблица продукции.
...
//  Массив Из ПеречислениеСсылка.СтатусыИнформированияЕГАИС - Статусы.
//
Функция СтатусыДвиженийАкцизныхМарокСвободныйОстаток()
...
//  Массив Из ПеречислениеСсылка.СтатусыИнформированияЕГАИС - Статусы.
//
Функция СтатусыДвиженийАкцизныхМарокКоличество()
...
Функция ЭтотОбъект(ЭтоМетаданные = Ложь)
	Если ЭтоМетаданные Тогда
		Возврат Метаданные.Документы.ЧекЕГАИС;
	Иначе
		Возврат Документы.ЧекЕГАИС;
	КонецЕсли;
КонецФункции
