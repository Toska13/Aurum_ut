...
Процедура ДобавитьКомандыОтчетов(КомандыОтчетов, Параметры) Экспорт
	...
	Отчеты.СоставРетроБонусовПоставщиков.ДобавитьКомандуОтчета(
		КомандыОтчетов, Параметры, "СоставРетроБонусовПоставщиковКонтекст");
	
	Отчеты.СоставРетроБонусовПоставщиков.ДобавитьКомандуОтчета(
		КомандыОтчетов, Параметры, "СоставРетроБонусовПоставщиковПоДаннымДокументаКонтекст");
...
//	ДополнительныеПараметры - Структура -Произвольный - Любые дополнительные сведения
//
...
Функция АктуальныеДанные(ДокументУсловий) Экспорт
	...
	|	Данные.ФиксацияВыполнена КАК ФиксацияВыполнена,
	...
	|	Данные.Вид.СвойствоНоменклатуры КАК СвойствоНоменклатуры,
...
Функция ТекстЗапросаТаблицаСвойстваНоменклатуры(ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	ИмяРегистра = "РетроБонусыПоставщиковСвойстваНоменклатуры";
...
Процедура ТекстЗапросаДанныеТаблицаСвойстваНоменклатуры(СписокЗапросов)
	...
	|ИЗ
	|	Документ.УсловияРетроБонусовПоставщика.СвойстваНоменклатуры КАК ТаблицаДанных
	|ГДЕ
	|	ТаблицаДанных.Ссылка = &Ссылка
	|
	|ИНДЕКСИРОВАТЬ ПО
...
Процедура НастройкаПоляТоварыКоличествоПлан(Настройки, МодульНастройки)
	...
	МодульНастройки.НовыйОтбор(Элемент.Условие, "ОтборТоваров", СоставыСписков.Выбранные);
...
Процедура НастройкаКомандФиксацииСегментов(Настройки, МодульНастройки)
	...
	Элемент.Поля.Добавить("ДекорацияЗафиксироватьСостав");
	...
	МодульНастройки.НовыйОтбор(ГруппаИЛИ, "Дополнительно.ДоступнаФиксацияСостава", Ложь);
	...
	Элемент.Поля.Добавить("ДекорацияОтменитьФиксациюСостава");
...
Функция ТекстЗапросаАктуальныеДанные()
	...
	|		ПО УсловияРетроБонусовПоставщика.ВидРетроБонуса = ВидыРетроБонусовПоставщиков.Ссылка
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	УсловияРетроБонусовПоставщикаСвойстваНоменклатуры.НомерСтроки КАК НомерСтроки,
	|	УсловияРетроБонусовПоставщикаСвойстваНоменклатуры.Ссылка КАК Ссылка,
	|	УсловияРетроБонусовПоставщикаСвойстваНоменклатуры.ЗначениеСвойства КАК ЗначениеСвойства,
	|	УсловияРетроБонусовПоставщикаСвойстваНоменклатуры.КоличествоПлан КАК КоличествоПлан,
	|	УсловияРетроБонусовПоставщикаСвойстваНоменклатуры.КоличествоПланГраницаДиапазона КАК КоличествоПланГраницаДиапазона,
	|	УсловияРетроБонусовПоставщикаСвойстваНоменклатуры.БазоваяЦена КАК БазоваяЦена,
	|	УсловияРетроБонусовПоставщикаСвойстваНоменклатуры.Процент КАК Процент,
	|	УсловияРетроБонусовПоставщикаСвойстваНоменклатуры.НачалоДействия КАК НачалоДействия,
	|	УсловияРетроБонусовПоставщикаСвойстваНоменклатуры.ОкончаниеДействия КАК ОкончаниеДействия,
	|	УсловияРетроБонусовПоставщикаСвойстваНоменклатуры.Отменено КАК Отменено
	|ПОМЕСТИТЬ ВТ_СвойстваНоменклатуры
	|ИЗ
	|	Документ.УсловияРетроБонусовПоставщика.СвойстваНоменклатуры КАК УсловияРетроБонусовПоставщикаСвойстваНоменклатуры
	|ГДЕ
	|	УсловияРетроБонусовПоставщикаСвойстваНоменклатуры.Ссылка В (&МассивОбъектов)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РетроБонусыПоставщиковСвойстваНоменклатуры.НачалоДействия КАК НачалоДействия,
	|	РетроБонусыПоставщиковСвойстваНоменклатуры.ОкончаниеДействия КАК ОкончаниеДействия,
	|	РетроБонусыПоставщиковСвойстваНоменклатуры.ДокументУсловий КАК ДокументУсловий,
	|	РетроБонусыПоставщиковСвойстваНоменклатуры.ЗначениеСвойства КАК ЗначениеСвойства,
	|	РетроБонусыПоставщиковСвойстваНоменклатуры.КоличествоПлан КАК КоличествоПлан,
	|	РетроБонусыПоставщиковСвойстваНоменклатуры.БонусПроцент КАК БонусПроцент,
	|	РетроБонусыПоставщиковСвойстваНоменклатуры.БазоваяЦена КАК БазоваяЦена,
	|	РетроБонусыПоставщиковСвойстваНоменклатуры.КоличествоПланГраницаДиапазона КАК КоличествоПланГраницаДиапазона,
	|	РетроБонусыПоставщиковСвойстваНоменклатуры.НомерСтрокиДокумента КАК НомерСтрокиДокумента,
	|	ВТ_Документы.Ссылка КАК Ссылка
	|ИЗ
	|	РегистрСведений.РетроБонусыПоставщиковСвойстваНоменклатуры КАК РетроБонусыПоставщиковСвойстваНоменклатуры
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Документы КАК ВТ_Документы
	|		ПО РетроБонусыПоставщиковСвойстваНоменклатуры.ДокументУсловий = ВТ_Документы.ДокументУсловий
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ДанныеПереопределяемыхПолей.СвойствоНоменклатуры КАК СвойствоНоменклатуры,
...
Процедура ОчиститьНеиспользуемыеРеквизитыТаблицыТоваров(Объект, Параметры, Знач ТаблицаДанных)
	...
	ЭтоТоварыПоДиапазонам = РетроБонусыКлиентСервер.ЭтоКоличествоПоДиапазонам(
	...
	Для Каждого СтрокаТЧ Из ТаблицаДанных Цикл
		...
		Если Параметры.СоставТоваров <> Перечисления.СоставыТоваровРетроБонусов.СвойствоНоменклатуры
		   И (Параметры.ТипБонуса <> Перечисления.ТипыРетроБонусовПоставщиков.Остатки
...
Функция ЗначениеСвойства(СведенияОбИсточнике, ЗагружаемыеДанные)
	...
	Если СведенияОбИсточнике.ЭтоПеречисление Тогда
		...
		Для Каждого ЗначениеПеречисления Из Перечисления[СведенияОбИсточнике.Имя] Цикл
			...
			НоваяСтрока.Значение = ЗначениеПеречисления;
	...
	ИначеЕсли СведенияОбИсточнике.ЭтоДокумент Тогда
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.УсловияРетроБонусовПоставщика КАК Документ
		|		ПО (Документ.Дата МЕЖДУ НАЧАЛОПЕРИОДА(ВТ_ТаблицаДанных.Дата, ДЕНЬ) И КОНЕЦПЕРИОДА(ВТ_ТаблицаДанных.Дата, ДЕНЬ))";
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Документ.УсловияРетроБонусовПоставщика", СведенияОбИсточнике.ПолноеИмя);
