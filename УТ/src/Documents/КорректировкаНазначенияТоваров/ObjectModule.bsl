...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	Если ЕстьПоПустомуНазначению Тогда
		...
		ЗаполнитьПризнакПустоеНазначениеВидовЗапасов(СтрокиСПустымНазначением);
...
Процедура ЗаполнитьПризнакПустоеНазначениеВидовЗапасов(ТоварыСПустымНазначением)
	...
	Для Каждого СтрокаТовары Из ТоварыСПустымНазначением Цикл
		
		КоличествоТоваров = СтрокаТовары.Количество;
		
		СтруктураПоиска.АналитикаУчетаНоменклатуры = СтрокаТовары.АналитикаУчетаНоменклатуры;
		...
		Для Каждого СтрокаЗапасов Из ТаблицаВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			...
			НоваяСтрока.НовоеНазначениеПустое	= Истина;
...
Процедура ЗаполнитьАналитикуОприходованиеВидовЗапасов()
	...
	|ПОМЕСТИТЬ ВТТаблицаЗапасов
	|ИЗ
	|	&ВидыЗапасов КАК ТаблицаЗапасов
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ВидЗапасов
	|;
	...
	|	ТаблицаЗапасов.АналитикаУчетаНоменклатуры	КАК АналитикаУчетаНоменклатуры,
	|	ТаблицаЗапасов.ВидЗапасов					КАК ВидЗапасов,
	|	ДанныеЗапасов.Организация					КАК Организация,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)	КАК ТипЗапасов,
	|	ДанныеНоменклатуры.ГруппаФинансовогоУчета	КАК ГруппаФинансовогоУчета
	|ИЗ
	|	ВТТаблицаЗапасов КАК ТаблицаЗапасов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыЗапасов КАК ДанныеЗапасов
	|		ПО ТаблицаЗапасов.ВидЗапасов = ДанныеЗапасов.Ссылка
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК Ключи
	|		ПО ТаблицаЗапасов.АналитикаУчетаНоменклатуры = Ключи.Ссылка
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК ДанныеНоменклатуры
	|		ПО Ключи.Номенклатура = ДанныеНоменклатуры.Ссылка
	|
	|ГДЕ
	|	ДанныеЗапасов.РеализацияЗапасовДругойОрганизации = ИСТИНА";
	
	ИменаКолонок = "АналитикаУчетаНоменклатуры, ВидЗапасов";
	
	Запрос.УстановитьПараметр("ВидыЗапасов", ВидыЗапасов.Выгрузить(, ИменаКолонок));
	
	РезультатЗапроса = Запрос.Выполнить();
	
	Если Не РезультатЗапроса.Пустой() Тогда
		ИменаКолонокПоУмолчанию = ИменаКолонок + ", " + "ВидЗапасовОприходование";
		
		ВидыЗапасовПоУмолчанию = ТаблицаВидыЗапасов.СкопироватьКолонки(ИменаКолонокПоУмолчанию);
		ВидыЗапасовПоУмолчанию.Индексы.Добавить(ИменаКолонок);
		...
		Пока Выборка.Следующий() Цикл
			...
			НоваяСтрока.ВидЗапасовОприходование = Справочники.ВидыЗапасов.ВидЗапасовДокумента(
													Выборка.Организация,
													,
													Выборка);
		КонецЦикла;
	Иначе
		ВидыЗапасовПоУмолчанию = Неопределено;
	КонецЕсли;
	...
	Для Каждого СтрокаТоваров Из Товары Цикл
		...
		Для Каждого СтрокаЗапасов Из ТаблицаВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			...
			Если ВидыЗапасовПоУмолчанию = Неопределено Тогда
				НоваяСтрока.ВидЗапасовОприходование = СтрокаЗапасов.ВидЗапасов;
			Иначе
				ЗаполнитьЗначенияСвойств(ОтборСтрок, НоваяСтрока);
				
				ДанныеОприходования = ВидыЗапасовПоУмолчанию.Найти(ОтборСтрок);
				
				Если ДанныеОприходования <> Неопределено Тогда
					НоваяСтрока.ВидЗапасовОприходование = ДанныеОприходования.ВидЗапасовОприходование;
				КонецЕсли;
			КонецЕсли;
