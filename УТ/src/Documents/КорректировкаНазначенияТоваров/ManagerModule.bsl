...
Функция МакетФормыВыбораНазначений() Экспорт
	...
	ШаблонНазначения.ТипыНазначений.Удалить(ШаблонНазначения.ТипыНазначений.Найти(Перечисления.ТипыНазначений.ДавальческоеПродукция22));
	ШаблонНазначения.Договор = "&Договор";
	ШаблонНазначения.УсловиеИспользования = "Объект.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийКорректировкиНазначения.РезервироватьИКорректировать)";
	
	ШаблонНазначения = Справочники.Назначения.ДобавитьШаблонНазначений(МакетФормы);
	ШаблонНазначения.ТипыНазначений.Удалить(ШаблонНазначения.ТипыНазначений.Найти(Перечисления.ТипыНазначений.ДавальческоеПродукция22));
	ШаблонНазначения.УсловиеИспользования = "Объект.ВидОперации В(
		|ЗНАЧЕНИЕ(Перечисление.ВидыОперацийКорректировкиНазначения.СнятьРезервПоМногимНазначениям),
		|ЗНАЧЕНИЕ(Перечисление.ВидыОперацийКорректировкиНазначения.Резервировать),
		|ЗНАЧЕНИЕ(Перечисление.ВидыОперацийКорректировкиНазначения.ПроизвольнаяКорректировкаНазначений),
		|ЗНАЧЕНИЕ(Перечисление.ВидыОперацийКорректировкиНазначения.ВстречнаяКорректировка),
		|ЗНАЧЕНИЕ(Перечисление.ВидыОперацийКорректировкиНазначения.ПустаяСсылка))";
	...
	ОписаниеКолонок.ПутиКДанным.ИсключаемыеНазначения = "Объект.Назначение";
...
// Параметры документа для механизма заполнения реквизита ПодлежитРазблокировке и ДатаПодтвержденияКачества
// в документе.
// 
// Параметры:
//  Объект - ДокументОбъект.ЗаказПоставщику - объект.
// 
// Возвращаемое значение:
...
Функция ТаблицаСнятияРезерва(Назначение, Товары, Регистратор) Экспорт
	...
	ТекстНазначения = "ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)"; // @query-part-1
	Запрос.Текст = ТекстЗапросаСвободныеОстатки(Истина, ТекстНазначения, "СвободныеОстатки"); 
	Запрос.Выполнить();
	...
	Запрос.Текст =
		...
		|	Таблица.НаКонтроле                                                     КАК НаКонтроле,
	...
	ТаблицаСкорректировано = ТаблицаТоваров.Скопировать(Новый Массив(), Ключ);
	ТаблицаСкорректировано.Колонки.Добавить("Количество",
		ОбщегоНазначения.ОписаниеТипаЧисло(15, 3, ДопустимыйЗнак.Неотрицательный));
	
	
	Запрос.УстановитьПараметр("ТаблицаСкорректировано", ТаблицаСкорректировано);
	...
	Текст =
		...
		|ПОМЕСТИТЬ ТаблицаСкорректировано
		|ИЗ
		|	&ТаблицаСкорректировано КАК Таблица
		|ИНДЕКСИРОВАТЬ ПО
		...
		|	Таблица.Склад КАК Склад,
		|	ВЫБОР
		|		КОГДА Таблица.Запас - ЕСТЬNULL(ТаблицаСкорректировано.Количество, 0) > 0
		|			ТОГДА Таблица.Запас - ЕСТЬNULL(ТаблицаСкорректировано.Количество, 0)
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК ВНаличии,
		|	ВЫБОР
		|		КОГДА Таблица.Обеспечено - ЕСТЬNULL(ТаблицаСкорректировано.Количество, 0) > 0
		|			ТОГДА Таблица.Обеспечено - ЕСТЬNULL(ТаблицаСкорректировано.Количество, 0)
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК Обеспечено,
		|	ВЫБОР
		|		КОГДА Таблица.Потребность - ЕСТЬNULL(ТаблицаСкорректировано.Количество, 0) > 0
		|			ТОГДА Таблица.Потребность - ЕСТЬNULL(ТаблицаСкорректировано.Количество, 0)
		|		ИНАЧЕ 0
		...
		|	ИсточникДанных КАК Таблица
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаСкорректировано КАК ТаблицаСкорректировано
		|		ПО ТаблицаСкорректировано.Номенклатура = Таблица.Номенклатура
		|		 И ТаблицаСкорректировано.Характеристика = Таблица.Характеристика
		|		 И ТаблицаСкорректировано.Склад = Таблица.Склад
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ СкладыОтгрузки КАК ТаблицаСкладовОтгрузки
		|		ПО ТаблицаСкладовОтгрузки.Склад          = Таблица.Склад
		|		 И ТаблицаСкладовОтгрузки.Номенклатура   = Таблица.Номенклатура
...
Функция ТаблицаРезервирования(Назначение, Источники, ТолькоСкладОтгрузки, Организация, Товары, Регистратор) Экспорт
	...
	ТекстНазначения = "ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)"; // @query-part-1
	Запрос.Текст = ТекстЗапросаСвободныеОстатки(Источники.ОстатокНаСкладе, ТекстНазначения, "СвободныеОстатки"); 
	Запрос.Выполнить();
	...
	Запрос.Текст = ТекстЗапросаСвободныеОстатки(Истина, ТекстНазначения, "ОбеспечениеЗаказов"); // @query-part-1
	Запрос.Выполнить();
	...
	Запрос.Текст =
		...
		|	ТаблицаНазначенийИсточников.НаКонтроле      КАК НаКонтроле
		|ИЗ
...
Процедура ДополнитьТаблицуКонтролируемымиПартиями(Таблица, Регистратор) Экспорт
	...
	Товары = Таблица.Скопировать(, "Номенклатура,Характеристика,Склад,ИсходноеНазначение,Помещение,Серия,Количество");
	...
	Текст =
		...
		|	Таблица.Характеристика КАК Характеристика,
		|	Таблица.Склад КАК Склад,
		|	Таблица.ИсходноеНазначение КАК Назначение,
		...
		|	Таблица.Серия
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура, Характеристика, Склад, Назначение, Помещение, Серия
		|;
		|
		|/////////////////////////////////////////////
		// Ограничение по остаткам с учетом серий
		|ВЫБРАТЬ
		|	Набор.Номенклатура КАК Номенклатура,
		|	Набор.Характеристика КАК Характеристика,
		|	Набор.Склад КАК Склад,
		|	Набор.Назначение КАК ИсходноеНазначение,
		...
		|		ТоварыНаСкладах.ВНаличииОстаток - ТоварыНаСкладах.КОтгрузкеОстаток КАК Количество
		|	ИЗ
		...
		|						Отбор.Назначение КАК Назначение,
		|						Отбор.Помещение КАК Помещение,
		|						Отбор.Серия КАК Серия
		|					ИЗ
		|						Товары КАК Отбор)) КАК ТоварыНаСкладах
		|		
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|		+ ВЫБОР
		|			КОГДА Движения.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		|				ТОГДА Движения.КОтгрузке
		|			ИНАЧЕ - Движения.КОтгрузке
		...
		|		И (Движения.Номенклатура, Движения.Характеристика, Движения.Склад, Движения.Назначение, Движения.Помещение, Движения.Серия)
		|			В(
		|				ВЫБРАТЬ
		|					Отбор.Номенклатура КАК Номенклатура,
		|					Отбор.Характеристика КАК Характеристика,
		...
		|
		|///////////////////////////////////////////////
		// Ограничение по свободным остаткам.
		|ВЫБРАТЬ
		|	Таблица.Номенклатура КАК Номенклатура,
		|	Таблица.Характеристика КАК Характеристика,
		|	Таблица.Склад КАК Склад,
		...
		|ПОМЕСТИТЬ ЗапасыИПотребностиСторно
		|ИЗ
		|	РегистрНакопления.ЗапасыИПотребности КАК Движения
		|ГДЕ
		|	Движения.Активность
		|		И Движения.Регистратор = &Регистратор
		|		И (Движения.Номенклатура, Движения.Характеристика, Движения.Склад, Движения.Назначение)
		|			В(
		|				ВЫБРАТЬ
		|					Отбор.Номенклатура КАК Номенклатура,
		|					Отбор.Характеристика КАК Характеристика,
		...
		|	Набор.Назначение КАК ИсходноеНазначение,
		|	ВЫБОР
		|		КОГДА Набор.КонтролируемаяПартия = ЗНАЧЕНИЕ(Справочник.КонтролируемыеПартии.ПустаяСсылка)
		|			ТОГДА СУММА(Набор.Свободно)
		|		КОГДА СУММА(Набор.Свободно) < СУММА(Набор.КРазблокировке)
		|			ТОГДА СУММА(Набор.Свободно)
		|		ИНАЧЕ СУММА(Набор.КРазблокировке)
		|	КОНЕЦ КАК Количество
		...
		|						Отбор.Назначение КАК Назначение
		|					ИЗ
		|						ТоварыБезСерий КАК Отбор)) КАК Запасы
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|		0 КАК КРазблокировке,
		|		- (Движения.ВНаличии
		|			- Движения.ЗаблокированоВНаличии) КАК Свободно
		|		ИЗ
		|			ЗапасыИПотребностиСторно КАК Движения
		|	ГДЕ
		|		Движения.ВНаличии - Движения.ЗаблокированоВНаличии <> 0
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|			- Запасы.РезервироватьПоМереПоступленияИзЗаблокированныхОстаток КАК Свободно
		|	ИЗ
		|		РегистрНакопления.ЗапасыИПотребности.Остатки(,
		|			(Номенклатура, Характеристика, Склад, Назначение)
		|				В(
		...
		|		Движения.Склад КАК Склад,
		|		Движения.Назначение КАК Назначение,
		|		Движения.Заказ КАК КонтролируемаяПартия,
		|		0 КАК КРазблокировке,
		|		- Движения.ЗаблокированоВНаличии КАК Свободно
		|	ИЗ
		|		ЗапасыИПотребностиСторно КАК Движения
		...
		|	Таблица.Количество КАК Количество,
		|	ЕСТЬNULL(Таблица.КонтролируемаяПартия.ДатаПодтвержденияКачества, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаПодтвержденияКачества,
		|	ЕСТЬNULL(Таблица.КонтролируемаяПартия.ДатаПоступления, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаПоступления,
		|	Таблица.Номенклатура.СрокКонтроляКачества КАК СрокКонтроляКачества
		|ПОМЕСТИТЬ ТоварыНаСкладахРасчетУпорядочивания
		|ИЗ
		|	ТоварыНаСкладахОстатки КАК Таблица
		|;
		|
		|///////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ЕСТЬNULL(Таблица.КонтролируемаяПартия.ДатаПоступления, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаПоступления,
		|	Таблица.Номенклатура.СрокКонтроляКачества КАК СрокКонтроляКачества
		|ПОМЕСТИТЬ ЗапасыИПотребностиОстаткиРасчетУпорядочивания
		|ИЗ
		|	ЗапасыИПотребностиОстатки КАК Таблица
		|;
		|
		|///////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|			И Таблица.ДатаПодтвержденияКачества = ДАТАВРЕМЯ(1, 1, 1)
		|			ТОГДА 0
		|		ИНАЧЕ 1
		|	КОНЕЦ КАК ПриоритетПартии,
		|	ВЫБОР
		|		КОГДА Таблица.ДатаПоступления = ДАТАВРЕМЯ(1, 1, 1)
		|				И Таблица.ДатаПодтвержденияКачества = ДАТАВРЕМЯ(1, 1, 1)
		|			ТОГДА ДАТАВРЕМЯ(1, 1, 1)
		|		КОГДА Таблица.ДатаПодтвержденияКачества <> ДАТАВРЕМЯ(1, 1, 1)
		|			ТОГДА Таблица.ДатаПодтвержденияКачества
		...
		|		ИНАЧЕ ДОБАВИТЬКДАТЕ(Таблица.ДатаПоступления, ДЕНЬ, Таблица.СрокКонтроляКачества)
		|	КОНЕЦ КАК ДатаПодтвержденияКачества
		|ИЗ
		|	ТоварыНаСкладахРасчетУпорядочивания КАК Таблица
		|		ЛЕВОЕ СОЕДИНЕНИЕ РасчетСрокаКонтроляКачестваПереопределяемый ПО ИСТИНА
		|УПОРЯДОЧИТЬ ПО
		|	Номенклатура,Характеристика,Склад,ИсходноеНазначение,Помещение,Серия,ПриоритетПартии,ДатаПодтвержденияКачества УБЫВ,КонтролируемаяПартия
		|;
		|
		|///////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	КОНЕЦ КАК ДатаПодтвержденияКачества
		|ИЗ
		|	ЗапасыИПотребностиОстаткиРасчетУпорядочивания КАК Таблица
		|		ЛЕВОЕ СОЕДИНЕНИЕ РасчетСрокаКонтроляКачестваПереопределяемый ПО ИСТИНА
		|УПОРЯДОЧИТЬ ПО
		|	Номенклатура,Характеристика,Склад,ИсходноеНазначение,ПриоритетПартии,ДатаПодтвержденияКачества УБЫВ,КонтролируемаяПартия";
	...
	ТоварыНаСкладах = ПакетЗапросов[ПакетЗапросов.ВГраница() - 1].Выгрузить();
	ЗапасыИПотребности = ПакетЗапросов[ПакетЗапросов.ВГраница()].Выгрузить();
	ПоляКлючаТоварыНаСкладах = "Номенклатура,Характеристика,Склад,ИсходноеНазначение,Помещение,Серия";
	ПоляКлючаЗапасыИПотребности = "Номенклатура,Характеристика,Склад,ИсходноеНазначение,КонтролируемаяПартия";
	ТоварыНаСкладах.Индексы.Добавить(ПоляКлючаТоварыНаСкладах);
	ЗапасыИПотребности.Индексы.Добавить(ПоляКлючаЗапасыИПотребности);
	КлючТоварыНаСкладах = Новый Структура(ПоляКлючаТоварыНаСкладах);
	КлючЗапасыИПотребности = Новый Структура(ПоляКлючаЗапасыИПотребности);
	ВсегоСтрок = Таблица.Количество();
	ВставленоСтрок = 0;
	Для Индекс = 0 По ВсегоСтрок - 1 Цикл
		
		НоваяСтрокаСПустойПартией = Неопределено;
		
		СтрокаТаблицы = Таблица[Индекс + ВставленоСтрок];
		Количество = СтрокаТаблицы.Количество;
		
		ЗаполнитьЗначенияСвойств(КлючТоварыНаСкладах, СтрокаТаблицы);
		НайденныеТовары = ТоварыНаСкладах.НайтиСтроки(КлючТоварыНаСкладах);
		
		Для ИндексТовары = 0 По НайденныеТовары.Количество() - 1 Цикл
			
			НайденныйТовар = НайденныеТовары[ИндексТовары];
			
			Распределить = Мин(НайденныйТовар.Количество, Количество);
			ЗаполнитьЗначенияСвойств(КлючЗапасыИПотребности, НайденныйТовар);
			НайденныеЗапасы = ЗапасыИПотребности.НайтиСтроки(КлючЗапасыИПотребности);
			
			Свободно = 0;
			Если НайденныеЗапасы.Количество() > 0 Тогда
				НайденныйЗапас = НайденныеЗапасы[0];
				Свободно = Мин(НайденныйЗапас.Количество, Распределить);
			КонецЕсли;
			Распределить = Свободно;
			Если Распределить > 0 Тогда
				НоваяСтрока = Таблица.Вставить(Индекс + ВставленоСтрок);
				ВставленоСтрок = ВставленоСтрок + 1;
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
				НоваяСтрока.КонтролируемаяПартия = НайденныйТовар.КонтролируемаяПартия;
				НоваяСтрока.Количество = Распределить;
				НайденныйЗапас.Количество = НайденныйЗапас.Количество - Распределить;
				НайденныйТовар.Количество = НайденныйТовар.Количество - Распределить;
				Количество = Количество - Распределить;
				Если Не ЗначениеЗаполнено(НоваяСтрока.КонтролируемаяПартия) Тогда
					НоваяСтрокаСПустойПартией = НоваяСтрока;
				КонецЕсли;
			КонецЕсли;
			
		КонецЦикла;
		
		// Если по данным запросов остатков не хватило, то добавляем остальное с пустой партией.
		Если Количество > 0 Тогда
			Если НоваяСтрокаСПустойПартией = Неопределено Тогда
				НоваяСтрока = Таблица.Вставить(Индекс + ВставленоСтрок);
				ВставленоСтрок = ВставленоСтрок + 1;
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
				...
				НоваяСтрокаСПустойПартией = НоваяСтрока;
			КонецЕсли;
			НоваяСтрокаСПустойПартией.Количество = НоваяСтрокаСПустойПартией.Количество + Количество;
		КонецЕсли;
...
Процедура УстановитьРезервы(Таблица, ТаблицаИтоги, РассчитыватьПотребностьПоВсемСкладам) Экспорт
	...
	Для Каждого Строка Из Таблица Цикл
		...
		Если Не ЗначениеЗаполнено(Строка.ИсходноеНазначение) Тогда
			...
			Строка.Количество = Макс(Мин(Потребность, Строка.Запас - Строка.Резерв - Строка.НаКонтроле), 0);
...
Функция ТекстЗапросаТаблицаТоварыНаСкладах(Запрос, ТекстыЗапроса, Регистры)
	...
	|	Подзапрос.Серия,
	|	Подзапрос.КонтролируемаяПартия,
	...
	|		ТаблицаТовары.Серия,
	|		ТаблицаТовары.КонтролируемаяПартия,
	|		ВЫБОР
	|			КОГДА ЕСТЬNULL(ТаблицаТовары.НовоеНазначение.ДвиженияПоСкладскимРегистрам, ЛОЖЬ)
	|				ТОГДА ТаблицаТовары.НовоеНазначение
	|			ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаТоваров.ПодлежитРазблокировке КАК ПодлежитРазблокировке,
	|	ТаблицаТоваров.ДатаПодтвержденияКачества КАК ДатаПодтвержденияКачества,
	...
	|	Подзапрос.КонтролируемаяПартия КАК КонтролируемаяПартия,
	|	Подзапрос.ПодлежитРазблокировке КАК ПодлежитРазблокировке,
	|	Подзапрос.ДатаПодтвержденияКачества КАК ДатаПодтвержденияКачества,
	...
	|		ТаблицаТоварныхМест.КонтролируемаяПартия КАК КонтролируемаяПартия,
	|		ТаблицаТоварныхМест.ПодлежитРазблокировке КАК ПодлежитРазблокировке,
	|		ТаблицаТоварныхМест.ДатаПодтвержденияКачества КАК ДатаПодтвержденияКачества,
	...
	|		ТаблицаТоварныхМест.КонтролируемаяПартия,
	|		ТаблицаТоварныхМест.ПодлежитРазблокировке,
	|		ТаблицаТоварныхМест.ДатаПодтвержденияКачества,
	...
	|	Подзапрос.ИсходноеНазначение,
	|	Подзапрос.КонтролируемаяПартия,
	|	Подзапрос.ПодлежитРазблокировке,
	|	Подзапрос.ДатаПодтвержденияКачества,
	|	Подзапрос.НовоеНазначение";
	...
	|	ТабЧасть.КонтролируемаяПартия КАК КонтролируемаяПартия,
	|	ТабЧасть.КонтролируемаяПартия <> ЗНАЧЕНИЕ(Справочник.КонтролируемыеПартии.ПустаяСсылка) КАК ПроконтролироватьКачество,
...
Функция ТекстЗапросаИсточникДанных(ВТалицеОтбораЕстьНазначение)
	...
	ТекстЗапроса =
		...
		|		И ТаблицаОстатков.Склад ССЫЛКА Справочник.Склады
		...
		|		И (&ВсеТовары
		|				ИЛИ (ТаблицаОстатков.Номенклатура, ТаблицаОстатков.Характеристика, &ПолеНазначениеТаблицыОстатков)
		|						В(
		|							ВЫБРАТЬ
		|								Отбор.Номенклатура КАК Номенклатура,
		|								Отбор.Характеристика КАК Характеристика,
		...
		|	ТаблицаОстатков.ЗаблокированоВНаличииОстаток КАК ЗаблокированоВНаличииОстаток,
		|	ТаблицаОстатков.ЗаблокированоПоступитОстаток КАК ЗаблокированоПоступитОстаток,
		|	ТаблицаОстатков.КРазблокировкеОстаток КАК КРазблокировкеОстаток,
		|	ТаблицаОстатков.РезервироватьНаСкладеИзЗаблокированныхОстаток КАК РезервироватьНаСкладеИзЗаблокированныхОстаток,
		|	ТаблицаОстатков.РезервироватьПоМереПоступленияИзЗаблокированныхОстаток КАК РезервироватьПоМереПоступленияИзЗаблокированныхОстаток
		...
		|					ОтборНазначений КАК Отбор)
		|			И (&ВсеТовары
		|					ИЛИ (Номенклатура, Характеристика, &ПолеНазначениеПараметровВиртуальныхТаблиц)
		|							В(
		|								ВЫБРАТЬ
		|									Отбор.Номенклатура КАК Номенклатура,
		|									Отбор.Характеристика КАК Характеристика,
		...
		|	ТаблицаОстатков.Склад КАК Склад,
		|	ТаблицаОстатков.Назначение КАК Назначение,
		|	ТаблицаОстатков.ВНаличииОстаток КАК ВНаличииОстаток,
		|	ТаблицаОстатков.ЗаблокированоВНаличииОстаток КАК ЗаблокированоВНаличииОстаток,
		|	ТаблицаОстатков.ЗаказаноОстаток КАК ЗаказаноОстаток,
		|	ТаблицаОстатков.ПоступитОстаток КАК ПоступитОстаток,
		|	ТаблицаОстатков.ЗаблокированоПоступитОстаток КАК ЗаблокированоПоступитОстаток,
		|	ТаблицаОстатков.РезервироватьНаСкладеОстаток КАК РезервироватьНаСкладеОстаток,
		|	ТаблицаОстатков.РезервироватьПоМереПоступленияОстаток КАК РезервироватьПоМереПоступленияОстаток,
		|	ТаблицаОстатков.ОтложитьРезервированиеОстаток КАК ОтложитьРезервированиеОстаток,
		|	ТаблицаОстатков.КОбеспечениюОстаток КАК КОбеспечениюОстаток,
		|	ТаблицаОстатков.НеОбеспечиватьОстаток КАК НеОбеспечиватьОстаток
		|ПОМЕСТИТЬ ЗапасыИПотребностиОстатки
		|ИЗ
		|	РегистрНакопления.ЗапасыИПотребности.Остатки(,
		|		Склад ССЫЛКА Справочник.Склады
		...
		|			+ Расчет.ВНаличииСБлокировкой_РезервироватьПоМере_ИзЗаблокированных КАК Резерв,
		|		Расчет.ВНаличии
		|			+ Расчет.ВНаличииСБлокировкой
		|			+ Расчет.Ожидается
		|			+ Расчет.ОжидаетсяСБлокировкой
		|			+ Расчет.НаСогласовании КАК Обеспечено,
		|		Расчет.Резервировать
		|			+ Расчет.РезервироватьПоМере
		|			+ Расчет.Отложить
		|			+ Расчет.КОбеспечению
		|			+ Расчет.ВНаличииСБлокировкой_Резервировать_ИзЗаблокированных
		|			+ Расчет.ВНаличииСБлокировкой_РезервироватьПоМере_ИзЗаблокированных КАК Потребность,
		|		Расчет.ВНаличииСБлокировкой_Излишек
		|			+ Расчет.ВНаличииСБлокировкой_Отложить
		|			+ Расчет.ВНаличииСБлокировкой_КОбеспечению КАК НаКонтроле
		|	ИЗ
		|		ПодстановкаРасчетРезервов КАК Расчет
		...
		|	Набор.Склад
		|;
		|УНИЧТОЖИТЬ ЗапасыИПотребностиСторно;
	...
	ИменаТаблиц.Сторно = "ЗапасыИПотребностиСторно"; // @query-part-1
	ИменаТаблиц.Результат = "ПодстановкаРасчетРезервов"; // @query-part-1
...
Функция ТекстЗапросаСвободныеОстатки(Получать, Назначения, ИмяТаблицыРезультат)
	
	Если Не Получать Тогда
		...
		ТекстЗапроса =
			...
			|ГДЕ
			|	ЛОЖЬ";
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ИмяТаблицыПереопределяемый", ИмяТаблицыРезультат); // @query-part-1
		Возврат ТекстЗапроса;
	...
	ТекстЗапроса =
		...
		|ГДЕ
		|	(ДвиженияДокумента.Номенклатура, ДвиженияДокумента.Характеристика) В(
		|		ВЫБРАТЬ
		...
		|	И ДвиженияДокумента.Назначение В (&НазначенияПереопределяемый)
		...
		|			ИЗ
		|				ОтборТоваровИсточникаДанных КАК ОтборТоваров)
		|		И Назначение В (&НазначенияПереопределяемый)) КАК Запасы
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура, Характеристика, Склад, Назначение
		|;
		|
		...
		|			+ Расчет.ВНаличииСБлокировкой_Излишек
		|			+ Расчет.ВНаличииСБлокировкой_Отложить
		|			+ Расчет.ВНаличииСБлокировкой_КОбеспечению КАК Свободно,
		|		Расчет.ВНаличииСБлокировкой_Излишек
		|			+ Расчет.ВНаличииСБлокировкой_Отложить
		|			+ Расчет.ВНаличииСБлокировкой_КОбеспечению КАК НаКонтроле
		|	ИЗ
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ИмяТаблицыПереопределяемый", ИмяТаблицыРезультат); // @query-part-1
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&НазначенияПереопределяемый", Назначения); // @query-part-1
...
//		* ИспользованиеСерий		 - Булево - признак использования серий,
//		* ИспользованиеПомещенийЯчеек- Булево - признак использования помещений или ячеек,
//		* Запас						 - Число - остаток товара на складе
//		* Резерв					 - Число - количество зарезервированного товара
//		* НаКонтроле				 - Число - количество не зарезервированного, но заблокированного товара
//		* Количество				 - Число - Количество товара, назначение которого необходимо скорректировать.
...
//		* СвободныйОстатокНаКонтроле - Число -
//		* Количество - Число -
//
Функция ТаблицаПомещенияЯчейкиПоТоварам(ТаблицаТовары, Ссылка, ТолькоОтмеченные = Истина) Экспорт
	...
	|	таб.НаКонтроле КАК СвободныйОстатокНаКонтроле,
	...
	|	ВЫБОР
	|		КОГДА Остатки.КонтролируемаяПартия = ЗНАЧЕНИЕ(Справочник.КонтролируемыеПартии.ПустаяСсылка)
	|			ТОГДА 0
	|		ИНАЧЕ Остатки.ВНаличииОстаток - Остатки.КОтгрузкеОстаток
	|	КОНЕЦ КАК ВНаличииНаКонтроле
	...
	|	КОНЕЦ КАК ВНаличии,
	|	ВЫБОР
	|		КОГДА Движения.КонтролируемаяПартия = ЗНАЧЕНИЕ(Справочник.КонтролируемыеПартии.ПустаяСсылка)
	|			ТОГДА 0
	|		КОГДА Движения.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|			ТОГДА -Движения.ВНаличии
	|		ИНАЧЕ Движения.ВНаличии
	...
	|	СУММА(Остатки.КОтгрузкеОстаток) КАК КОтгрузке,
	|	СУММА(ВЫБОР
	|		КОГДА Остатки.КонтролируемаяПартия = ЗНАЧЕНИЕ(Справочник.КонтролируемыеПартии.ПустаяСсылка)
	|			ТОГДА 0
	|		ИНАЧЕ Остатки.ВРезервеОстаток
	|	КОНЕЦ) КАК ВРезервеНаКонтроле,
	|	СУММА(ВЫБОР
	|		КОГДА Остатки.КонтролируемаяПартия = ЗНАЧЕНИЕ(Справочник.КонтролируемыеПартии.ПустаяСсылка)
	|			ТОГДА 0
	|		ИНАЧЕ Остатки.КОтгрузкеОстаток
	|	КОНЕЦ) КАК КОтгрузкеНаКонтроле 
	...
	|	СУММА(ТоварыНаСкладах.ВНаличииНаКонтроле) КАК ВНаличииНаКонтроле,
	|	МАКСИМУМ(ЕСТЬNULL(ТоварыКОтгрузке.ВРезерве + ТоварыКОтгрузке.КОтгрузке, 0)) КАК СерииКОтгрузке,
	|	МАКСИМУМ(ЕСТЬNULL(ТоварыКОтгрузке.ВРезервеНаКонтроле + ТоварыКОтгрузке.КОтгрузкеНаКонтроле, 0)) КАК СерииКОтгрузкеНаКонтроле
	|ИЗ
	...
	|	Остатки.ВНаличииНаКонтроле КАК ОбеспеченоНаКонтроле,
	...
	|	Остатки.ВНаличииНаКонтроле * ЕСТЬNULL(&ТекстЗапросаКоэффициентаУпаковки, 1) КАК ОбеспеченоЕдиницНаКонтроле,
	|	Остатки.СерииКОтгрузке КАК СерииКОтгрузке,
	|	Остатки.СерииКОтгрузкеНаКонтроле КАК СерииКОтгрузкеНаКонтроле,
	...
	|	Товары.СвободныйОстатокНаКонтроле КАК СвободныйОстатокНаКонтроле,
...
Процедура ЗаполнитьПоОснованию(Назначение, Организация, КорректировкаНазначения, ВидОперации, Товары, ТоварыОтбор = Неопределено) Экспорт
	...
	Таблица.Колонки.Добавить("Серия", Новый ОписаниеТипов("СправочникСсылка.СерииНоменклатуры"));
	Таблица.Колонки.Добавить("Помещение", Новый ОписаниеТипов("СправочникСсылка.СкладскиеПомещения"));
	Таблица.Колонки.Добавить("Ячейка", Новый ОписаниеТипов("СправочникСсылка.СкладскиеЯчейки"));
	...
	Если ВидОперации = Перечисления.ВидыОперацийКорректировкиНазначения.СнятьРезервПоМногимНазначениям
			...
			ТоварыКопия = Таблица.Скопировать(, "Номенклатура,ИсходноеНазначение,НовоеНазначение,Склад");
			Выборка = УказаниеПомещенийЯчеекСерийДляСтроки(ТоварыКопия);
			Пока Выборка.Следующий() Цикл
	...
	ДополнитьТаблицуКонтролируемымиПартиями(
		Таблица,
		КорректировкаНазначения);
