...
//  Массив Из ПеречислениеСсылка.СтатусыОбработкиПриемкиТоваровИСМП - Конечные статусы.
...
//
// Возвращаемое значение:
//  Массив Из ПеречислениеСсылка.СтатусыОбработкиПриемкиТоваровИСМП - Статусы ошибок.
//
...
// 
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.ДальнейшиеДействияПоВзаимодействиюИСМП - Дальнейшие действия по-умолчанию.
...
Функция СтатусПослеПолученияДанных(ДокументСсылка, Операция, ДополнительныеПараметры) Экспорт
	...
	Если Операция = Перечисления.ВидыОперацийИСМП.ПолучениеРезультатаОбработкиДокумента Тогда
		...
		Если ДополнительныеПараметры.Свойство("ОтражениеРезультатовПроверки") Тогда
			...
			СтатусПроверкиИПодбора = РегистрыСведений.СтатусыПроверкиИПодбораИС.ТекущийСтатус(ДокументСсылка, 0);
...
Процедура ОтразитьРезультатыПроверкиИПодбора(ПараметрыОкончанияПроверки) Экспорт
	...
	ЗаполнитьЦенуПоДаннымШтрихкодов = МаркировкаТоваровКлиентСервер.ДоступенОтчетОНанесении(ДокументОбъект.ТоварнаяГруппа);
	...
	Для Каждого СтрокаТаблицы Из ПараметрыОкончанияПроверки.ТаблицаПодобраннойПровереннойПродукции Цикл
		...
		Если ДоступенСрокГодности
			И ЗначениеЗаполнено(СтрокаТаблицы.СрокГодности) Тогда
	...
	ВидыПродукцииПоТоварнойГруппе = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ДокументОбъект.ТоварнаяГруппа);
	...
	Для Каждого СтрокаШтрихкодаВерхнегоУровня из ПараметрыОкончанияПроверки.ТаблицаШтрихкодовВерхнегоУровня Цикл
		...
		РезультатРазбора = ШтрихкодированиеОбщегоНазначенияИС.НоваяСтруктураОбработкиШтрихкода(
			СтрокаШтрихкодаВерхнегоУровня.Штрихкод, ВидыПродукцииПоТоварнойГруппе, ПользовательскиеПараметрыРазбораКодаМаркировки);
...
//  ДокументСсылка - ДокументСсылка.ПриемкаТоваровИСМП - Документ, для которого выполняется заполнение
...
// Значения штрихкодов из документа приемки.
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка.ПриемкаТоваровИСМП - Документ, для которого выполняется заполнение
// 
// Возвращаемое значение:
//  Массив Из Структура - Значения штрихкодов
Функция ЗначенияШтрихкодов(ДокументСсылка) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПриемкаТоваровИСМП.ЗначениеШтрихкода   КАК Штрихкод
	|ИЗ
	|	Документ.ПриемкаТоваровИСМП.ШтрихкодыУпаковок КАК ПриемкаТоваровИСМП
	|ГДЕ
	|	ПриемкаТоваровИСМП.Ссылка = &ДокументСсылка";
	Запрос.УстановитьПараметр("ДокументСсылка", ДокументСсылка);
	...
	Пока Выборка.Следующий() Цикл
		Результат.Добавить(Новый Структура("Штрихкод,Количество", Выборка.Штрихкод, 1));
	КонецЦикла;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ОбновлениеИнформационнойБазыГосИС.ДобавитьОбработчикОбновленияПереходаНаКонструкторНастроек(
		...
		Новый УникальныйИдентификатор("0a49077e-dfad-4b4a-99ba-9d619d33769f"));
...
Функция ПриемкаТоваровJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	ЕстьДанныеШапки = Шапка.Следующий();
	ВидыПродукции = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукции(Шапка.ТоварнаяГруппа);
	
	ПрисутствуетПродукцияИСМП = Ложь;
	...
	Если Не ЕстьДанныеШапки
		Или ШтрихкодыУпаковок.Количество() = 0
		Или Не ПрисутствуетПродукцияИСМП Тогда
