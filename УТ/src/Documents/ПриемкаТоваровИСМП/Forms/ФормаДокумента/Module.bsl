...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Объект.Товары.ВидПродукцииВЕТИС");
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Объект.Товары.ДоляСпирта");
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыДоляСпирта.Имя);
...
Процедура НастроитьЭлементыФормы(Форма)
	...
	Элементы.ТоварыКодТНВЭД.Видимость                 = МаркировкаТоваровКлиентСервер.ДоступноПолеТНВЭД(ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
	Элементы.ТоварыКодТНВЭД.АвтоОтметкаНезаполненного = МаркировкаТоваровКлиентСервер.ПроверятьЗаполнениеПоляКодТНВЭД(ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
	Элементы.ТоварыСертификация.Видимость             = МаркировкаТоваровКлиентСервер.ДоступноПолеСертификация(ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
	
	ДоступнаЦена = ВыводИзОборотаКлиентСервер.ДоступноПолеЦена(ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
	
	Элементы.ТоварыЦена.Видимость      = ДоступнаЦена;
	Элементы.ТоварыСуммаНДС.Видимость  = ДоступнаЦена;
	Элементы.ТоварыСумма.Видимость     = ДоступнаЦена;
	Элементы.ТоварыСтавкаНДС.Видимость = ДоступнаЦена;
	Элементы.ТоварыСуммаСНДС.Видимость = ДоступнаЦена;
	
	Элементы.ТоварыЦена.АвтоОтметкаНезаполненного = ВыводИзОборотаКлиентСервер.ПроверятьЗаполнениеПоляЦена(ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
	
	Элементы.ЗаполнитьДокументыСертификации.Видимость = МаркировкаТоваровКлиентСервер.ДоступноПолеСертификация(
		ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
	
	ФорматДаты = "ДФ=dd.MM.yyyy;";
	
	Если МаркировкаТоваровКлиентСервер.ДляТоварнойГруппыИспользуетсяДатаПроизводства(ВидыПродукцииПоТоварнойГруппе, Объект.Операция) Тогда
		...
		Элементы.ТоварыДатаПроизводства.АвтоОтметкаНезаполненного =
			КонструкторИСМПКлиентСервер.ЭтоВидПродукцииСОбязательнымУказаниемДатыПроизводства(ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
	Иначе
		Элементы.ТоварыДатаПроизводства.Видимость = Ложь;
	КонецЕсли;
	
	Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСоСрокамиГодности(ВидыПродукцииПоТоварнойГруппе,, Объект.Операция) Тогда
		...
		Элементы.ТоварыСрокГодности.АвтоОтметкаНезаполненного =
			КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСОбязательнымУказаниемСроковГодности(ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
	...
	Элементы.ТоварыСуммаАкциза.Видимость =
		МаркировкаТоваровКлиентСервер.ДоступноПолеСуммаАкциза(ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
	Элементы.ТоварыДоляСпирта.Видимость =
		МаркировкаТоваровКлиентСервер.ДоступноПолеСодержаниеЭтиловогоСпирта(ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
	Элементы.ТоварыНомерСерии.АвтоОтметкаНезаполненного =
		МаркировкаТоваровКлиентСервер.ПроверятьЗаполнениеПоляНомерСерии(ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
	
	ДоступенИдентификаторПроисхождения = ОтгрузкаТоваровКлиентСервер.ДоступноПолеИдентификаторПроисхождения(ВидыПродукцииПоТоварнойГруппе, Объект.Операция);
	
	Элементы.ТоварыИдентификаторПроисхожденияВЕТИС.Видимость = (ДоступенИдентификаторПроисхождения И Форма.ИспользуетсяПодсистемаВЕТИС);
	Элементы.ТоварыИдентификаторПроисхожденияВЕТИССтрокойНаФорме.Видимость = (ДоступенИдентификаторПроисхождения И Не Форма.ИспользуетсяПодсистемаВЕТИС);
	...
	ВыполняетсяИлиЗавершенаПроверка = (Форма.СтатусПроверкиИПодбора = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиИПодбораИС.Выполняется")
		Или Форма.СтатусПроверкиИПодбора = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиИПодбораИС.Завершено"));
	
	ВыполняетсяПроверка = (Форма.СтатусПроверкиИПодбора = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиИПодбораИС.Выполняется"));
	...
	ОбщегоНазначенияИСКлиентСервер.УстановитьДоступностьЭлементовФормы(Форма,
		...
		Не (Форма.РедактированиеФормыНеДоступно Или ВыполняетсяИлиЗавершенаПроверка));
	...
	ОбщегоНазначенияИСКлиентСервер.УстановитьДоступностьЭлементовФормы(Форма,
		ЗависимыеОтСтатусаПроверкиИПодбора,
		Не (Форма.РедактированиеФормыНеДоступно Или ВыполняетсяПроверка));
...
Процедура ОбновитьПредставлениеНоменклатуры()
	...
	ДанныеПоШтрихкодамEANРезультат = ШтрихкодированиеОбщегоНазначенияИС.ДанныеПоШтрихкодамEAN(МассивEAN);
