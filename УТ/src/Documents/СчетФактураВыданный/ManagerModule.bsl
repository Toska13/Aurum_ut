...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		ТекстЗапросаТаблицаЗачетыАвансов(Запрос, ТекстыЗапроса, Регистры);
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	Иначе
		...
		Если Объект.Корректировочный
			...
			ПараметрыУказанияСерий.ИспользоватьСерииНоменклатуры = УчитыватьСебестоимостьПоСериям;
		КонецЕсли;
	КонецЕсли;
...
Функция ТекстЗапросаЗаполненияСтатусовУказанияСерий(ПараметрыУказанияСерий) Экспорт
	...
	|		ПО Товары.ВидНоменклатуры.ПолитикаУчетаСерий = ПолитикиУчетаСерий.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
...
//
Функция ПолучитьПредставленияСтроки5а(Знач Таблица, МенеджерВременныхТаблиц) Экспорт
	...
	Результат.Колонки.Добавить("НаименованиеДокумента", Новый ОписаниеТипов("Строка"));
	Результат.Колонки.Добавить("НомерДокумента", Новый ОписаниеТипов("Строка"));
	Результат.Колонки.Добавить("ДатаДокумента", Новый ОписаниеТипов("Дата"));
	Результат.Колонки.Добавить("ПредставлениеСтроки5аДокументаОснования", Новый ОписаниеТипов("Строка"));
	
	РезультатПромежуточный = Результат.СкопироватьКолонки();
	...
	Запрос.УстановитьПараметр("ТекстТоварнаяНакладная", НСтр("ru = 'Товарная накладная'"));
	Запрос.УстановитьПараметр("ТекстАкт", НСтр("ru = 'Акт'"));
	...
	|	ПорядковыеНомера.ДокументОснование,
	|	ПорядковыеНомера.НаименованиеДокумента
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|		ВЫБОР
	|			КОГДА НоменклатураДокументов.ЕстьТовары
	|				ТОГДА &ТекстТоварнаяНакладная
	|			ИНАЧЕ &ТекстАкт
	...
	|			ПО ПорядковыеНомера.СчетФактура = НоменклатураДокументов.Ссылка
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ НоменклатураДокументов КАК НоменклатураДокументов
	|			ПО СчетФактураВыданныйДокументыОснования.Ссылка = НоменклатураДокументов.Ссылка
	|	ГДЕ
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ НоменклатураДокументов";
	
	ВерсияПостановления1137 = 1;
	РезультатЗапроса = Запрос.Выполнить();
	
	ДокументыОснования = РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("ДокументОснование");
	НаименованияДокументовДанныеПечати = ОбщегоНазначенияУТ.НаименованияЭлектронныхДокументовИлиОсновныхПечатныхФорм(ДокументыОснования);
	
	ВыборкаПоСФ = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	Пока ВыборкаПоСФ.Следующий() Цикл
		
		РезультатПромежуточный.Очистить();
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ВерсияПостановления1137 >= 7 Тогда
				ПредставлениеДокумента = 
					СтрШаблон(
						НСтр("ru = '%1, № %2 от %3'"),
						?(НаименованияДокументовДанныеПечати[Выборка.ДокументОснование] <> Неопределено,
							НаименованияДокументовДанныеПечати[Выборка.ДокументОснование], Выборка.НаименованиеДокумента),
						Выборка.Номер,
			...
			НоваяСтрока = РезультатПромежуточный.Добавить();
			НоваяСтрока.СчетФактура           = ВыборкаПоСФ.СчетФактура;
			НоваяСтрока.НаименованиеДокумента = Выборка.НаименованиеДокумента;
			НоваяСтрока.НомерДокумента        = Выборка.Номер;
			НоваяСтрока.ДатаДокумента         = Выборка.Дата;
			НоваяСтрока.ПредставлениеСтроки5аДокументаОснования = ПредставлениеДокумента;
		...
		РезультатПромежуточный.ЗаполнитьЗначения(ПредставлениеСтроки5а, "ПредставлениеСтроки5а");
		
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РезультатПромежуточный, Результат);
	КонецЦикла;
	
	Результат.Свернуть("СчетФактура, ПредставлениеСтроки5а, НаименованиеДокумента, НомерДокумента, ДатаДокумента, ПредставлениеСтроки5аДокументаОснования");
...
//	ДатаСФ - Дата - Дата СФ, у которой заполняется ТЧ ПлатежноРасчетныеДокументы. 
//	АктулизироватьРасчеты - Булево - В момент создания нового документа актулизовать не будем. В ОФФлан расчетах актулизация занимает много времени.
//
Процедура ЗаполнитьПлатежноРасчетныеДокументы(ПлатежноРасчетныеДокументы, МассивОснований, Организация, ДатаСФ, АктулизироватьРасчеты = Истина) Экспорт
	...
	ТипДата = Новый Массив();
	ТипДата.Добавить(Тип("Дата"));
	...
	Для каждого ДокументОснования Из МассивОснований Цикл
		...
		Если ТипЗнч(ДокументОснования) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
			МассивКорректировок.Добавить(ДокументОснования);
		КонецЕсли;
	...
	Если МассивКорректировок.Количество() > 0 Тогда
		...
		ОбщегоНазначения.ЗначениеРеквизитаОбъектов(МассивКорректировок, "ДокументОснование");
		Для каждого Документ Из МассивКорректировок Цикл
		
			НоваяСтрока = ТаблицаЗаполненияПлатежноРасчетныхДокументов.Добавить();
			НоваяСтрока.Ссылка = Документы.СчетФактураВыданный.ПустаяСсылка();
			НоваяСтрока.Организация = Организация;
			НоваяСтрока.ДокументОснование = Документ.ДокументОснование;
	...
	Запрос.УстановитьПараметр("ДатаСФ", ?(ЗначениеЗаполнено(ДатаСФ), КонецДня(ДатаСФ), ТекущаяДатаСеанса()));
	Запрос.Выполнить();
	...
	Если МассивАналитикУчетаПоПартнерам.Количество() > 0 И АктулизироватьРасчеты Тогда 
		Если Не ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
			...
			КонецПопытки;
		Иначе
			Если РегистрыСведений.ЗаданияКРаспределениюВзаиморасчетов.АктуальныйПериодРассчитанныхДанныхПоВзаиморасчетам(МассивАналитикУчетаПоПартнерам,,КонецРасчета) <> Неопределено Тогда
				ТекстСообщения = НСтр("ru = 'Печатная форма сформирована по неактуальным данным.'") + Символы.ПС;
				ТекстСообщения = ТекстСообщения + ВзаиморасчетыСервер.ТекстПредупрежденияЗаданияРаспределенияВзаиморасчетов();
	...
	ЗапросПоставщики.МенеджерВременныхТаблиц = Запрос.МенеджерВременныхТаблиц;
	...
	ЗапросПоставщики.Выполнить();
		
	Запрос.Текст = ТекстЗапросавтДанныеПлатежноРасчетныхДокументов();
	Запрос.Выполнить();
	
	Запрос.Текст = ТекстЗапросаВтСчетаФактурыАванс();
	Запрос.Выполнить();
...
//	ДатаСФ - Дата - Дата СФ, у которой заполняется ТЧ ПлатежноРасчетныеДокументы. 
//
Процедура ЗаполнитьДанныеСФНаАвансПоОтгрузке(ПлатежноРасчетныеДокументы, Организация, ДатаСФ) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("ДанныеПлатежноРасчетныхДокументов", ПлатежноРасчетныеДокументы.Выгрузить());
	Запрос.УстановитьПараметр("ДатаСФ", ?(ЗначениеЗаполнено(ДатаСФ), КонецДня(ДатаСФ), ТекущаяДатаСеанса()));
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("ТипыДенежныхДокументов", УчетНДСРФ.ТипыДенежныхДокументов());
	
	Запрос.Текст = ТекстЗапросаВтДанныеПлатежноРасчетныхДокументовПоОтгрузке();
	Запрос.Выполнить();
	...
	ВыборкаСчетФактур = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Если ВыборкаСчетФактур.Следующий() Тогда
		
		Выборка = ВыборкаСчетФактур.Выбрать();
		Пока Выборка.Следующий() Цикл
			Строка = ПлатежноРасчетныеДокументы.Найти(Выборка.РасчетныйДокумент, "РасчетныйДокумент");
			Если Строка <> Неопределено Тогда
				ЗаполнитьЗначенияСвойств(Строка, Выборка, "РасчетныйДокумент, НомерСчетаФактурыНаАванс, ДатаСчетаФактурыНаАванс, НомерИсправленияСчетаФактурыНаАванс, ДатаИсправленияСчетаФактурыНаАванс");
			КонецЕсли;
		КонецЦикла;
...
// Заполняет номера и даты платежно-расчетных документов по документам отгрузки.
//
// Параметры:
//	ПлатежноРасчетныеДокументы - Табличная часть - заполняемая табличная часть документа
//	МассивОснований - Массив Из ДокументСсылка.ОтгрузкаТоваровКлиенту - Массив документов-оснований счета-фактуры.
//
Процедура ЗаполнитьПлатежноРасчетныеДокументыПоОтгрузке(ПлатежноРасчетныеДокументы, МассивОснований) Экспорт
	
	СчетаФактуры = Документы.СчетФактураВыданный.СчетаФактурыПоОснованию(МассивОснований[0]);
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СчетФактура", СчетаФактуры);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ТаблицаПлатежноРасчетныеДокументы.ДатаПлатежноРасчетногоДокумента     КАК ДатаВходящегоДокумента,
	|	ТаблицаПлатежноРасчетныеДокументы.НомерПлатежноРасчетногоДокумента    КАК НомерВходящегоДокумента,
	|	ТаблицаПлатежноРасчетныеДокументы.НомерСчетаФактурыНаАванс            КАК НомерСчетаФактурыНаАванс,
	|	ТаблицаПлатежноРасчетныеДокументы.ДатаСчетаФактурыНаАванс             КАК ДатаСчетаФактурыНаАванс,
	|	ТаблицаПлатежноРасчетныеДокументы.РасчетныйДокумент                   КАК РасчетныйДокумент,
	|	ТаблицаПлатежноРасчетныеДокументы.НомерИсправленияСчетаФактурыНаАванс КАК НомерИсправленияСчетаФактурыНаАванс,
	|	ТаблицаПлатежноРасчетныеДокументы.ДатаИсправленияСчетаФактурыНаАванс  КАК ДатаИсправленияСчетаФактурыНаАванс,
	|	ТаблицаПлатежноРасчетныеДокументы.Ссылка КАК СчетФактура
	|ИЗ
	|	Документ.СчетФактураВыданный.ПлатежноРасчетныеДокументы КАК ТаблицаПлатежноРасчетныеДокументы
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.СчетФактураВыданный КАК Операция
	|	ПО
	|		ТаблицаПлатежноРасчетныеДокументы.Ссылка = Операция.Ссылка
	|ГДЕ
	|	ТаблицаПлатежноРасчетныеДокументы.Ссылка В (&СчетФактура)
...
Функция СтрокаРасчетноПлатежныхДокументов(ПлатежноРасчетныеДокументы) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ПлатежноРасчетныеДокументы Цикл
		Если ПустаяСтрока(СтрокаТаблицы.НомерПлатежноРасчетногоДокумента)
				И СтрокаТаблицы.ДатаПлатежноРасчетногоДокумента = Дата("00010101") Тогда
			Продолжить;
		КонецЕсли;
...
// Определяет строковое представление счетов-фактур на аванс по таблице платежно-расчетных документов счета-фактуры.
//
//	Параметры:
//		ПлатежноРасчетныеДокументы - ТаблицаЗначений - таблица платежно-расчетных документов
//			(см. ТЧ "ПлатежноРасчетныеДокументы" документа "СчетФактураВыданный").
//
//	Возвращаемое значение:
//		Строка - представление счетов-фактур на аванс.
//
Функция СтрокаАвансовыеСчетаФактуры(ПлатежноРасчетныеДокументы) Экспорт
	
	СтрокаНомеровИДата = "";
	Для Каждого СтрокаТаблицы Из ПлатежноРасчетныеДокументы Цикл
			
		Если ЗначениеЗаполнено(СтрокаТаблицы.НомерСчетаФактурыНаАванс) Тогда
			АвансовыеСчетаФактурыНомерДата = СтрШаблон(НСтр("ru='№ %1 от %2, исправление № %3 от %4'"),
				СтрокаТаблицы.НомерСчетаФактурыНаАванс, Формат(СтрокаТаблицы.ДатаСчетаФактурыНаАванс, "ДЛФ=D"),
				?(ЗначениеЗаполнено(СтрокаТаблицы.НомерИсправленияСчетаФактурыНаАванс), СтрокаТаблицы.НомерИсправленияСчетаФактурыНаАванс, "--"), ?(ЗначениеЗаполнено(СтрокаТаблицы.ДатаИсправленияСчетаФактурыНаАванс), Формат(СтрокаТаблицы.ДатаИсправленияСчетаФактурыНаАванс, "ДЛФ=D"), "--"));
			СтрокаНомеровИДата = СтрокаНомеровИДата 
				+ ?(Не ЗначениеЗаполнено(СтрокаНомеровИДата), "", "; ") + АвансовыеСчетаФактурыНомерДата;
		КонецЕсли;
...
Функция ЕстьСчетаФактурыКОформлению(Параметры) Экспорт
	...
	ИначеЕсли Параметры.Свойство("БезОграниченияПериода") Тогда
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
// Возвращает количество счетов-фактур по отгрузкам к оформлению.
//
// Параметры:
//	Параметры - Структура :
//	* МассивОрганизаций - Массив из СправочникСсылка.Организации - массив организаций.
//	* МассивРаспоряжений - Массив Из ДокументСсылка.ЗаказКлиента, ДокументСсылка.ОтгрузкаТоваровКлиенту - массив распоряжений, по которым выставлены счета-фактуры выданные.
//	* КонецПериода - Дата - ограничение сверху на дату документа-основания.
//	* БезОграниченияПериода - Булево - не ограничивать по дате документа-основания
//
// Возвращаемое значение:
//	Число - количество счетов-фактур к оформлению.
//
Функция ЕстьСчетаФактурыПоОтгрузкамКОформлению(Параметры) Экспорт
	...
	ПараметрыФормированияТаблицы.ТекстУсловия =
			"	(Распоряжение.Организация В (&Организация)
	...
	Если Не Параметры.Свойство("БезОграниченияПериода") Тогда 
	...
	|
	|ГДЕ 
	|	(ДанныеУчета.Распоряжение В (&Распоряжения)
	|	ИЛИ &БезОтбораПоРаспоряжениям)
	|	И РеестрДокументов.ТипСсылки ЕСТЬ NULL
	|
	|СГРУППИРОВАТЬ ПО
	...
	Запрос.УстановитьПараметр("ПоВсемОрганизациям", Не ЗначениеЗаполнено(Параметры.МассивОрганизаций));
	Если Не Параметры.Свойство("БезОграниченияПериода") Тогда
		Запрос.УстановитьПараметр("Месяц", ?(ЗначениеЗаполнено(Параметры.КонецПериода), КонецМесяца(Параметры.КонецПериода), Параметры.КонецПериода));
	КонецЕсли;
	Запрос.УстановитьПараметр("БезОтбораПоРаспоряжениям", Не Параметры.Свойство("Распоряжения"));
	Запрос.УстановитьПараметр("Распоряжения", ?(Параметры.Свойство("Распоряжения"), Параметры.Распоряжения, Неопределено));
	
	Выборка = Запрос.Выполнить().Выгрузить();
	
	Выборка.Свернуть("Распоряжение");
...
Функция КорректировкиДокументаОснования(ДокументРеализации, ДатаПодтверждения) Экспорт
	...
	|	ИЛИ &ДатаПодтверждения = ДАТАВРЕМЯ(1,1,1))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ДанныеДокумента.ОплатаВВалюте                                                     КАК ОплатаВВалюте,
	...
	|	ДанныеДокумента.НачислениеНДСПриОтгрузке                                          КАК НачислениеНДСПриОтгрузке,
	|	ДанныеДокумента.Дата                                                              КАК ДатаОтгрузки,
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.НачислениеНДСПриОтгрузке
	|			ТОГДА ДанныеДокумента.ДокументОснование
	|		ИНАЧЕ
	|			НЕОПРЕДЕЛЕНО
	|	КОНЕЦ                                                                             КАК ДокументОтгрузки,
	|	НЕОПРЕДЕЛЕНО                                                                      КАК ДокументОтгрузкиИсправление
	|ИЗ
	...
	КоэффициентПересчетаВВалютуВзаиморасчетовРегл = РаботаСКурсамиВалютУТ.ПолучитьКоэффициентПересчетаИзВалютыВВалюту(
										Запрос.Параметры.ВалютаВзаиморасчетов,
										Запрос.Параметры.ВалютаРегламентированногоУчета,
										Запрос.Параметры.Период);
										
	Запрос.УстановитьПараметр("КоэффициентПересчетаВВалютуВзаиморасчетовРегл", КоэффициентПересчетаВВалютуВзаиморасчетовРегл);
	...
	КоэффициентПересчетаВВалютуУпрРегл = РаботаСКурсамиВалютУТ.ПолучитьКоэффициентПересчетаИзВалютыВВалюту(
										Запрос.Параметры.ВалютаРегламентированногоУчета,
										ЗначениеНастроекПовтИсп.ВалютаУправленческогоУчета(),
										Запрос.Параметры.Период);
										
	Запрос.УстановитьПараметр("КоэффициентПересчетаВВалютуУпрРегл", КоэффициентПересчетаВВалютуУпрРегл);
	...
	Запрос.УстановитьПараметр("ИсходныеСчетаФактуры", ДокументСсылка.Товары.ВыгрузитьКолонку("ИсходныйСчетФактура"));
	Если Реквизиты.НачислениеНДСПриОтгрузке
		И Реквизиты.ДокументОтгрузки <> Неопределено Тогда 
		Если ТипЗнч(Реквизиты.ДокументОтгрузки) <> Тип("ДокументСсылка.ПервичныйДокумент") Тогда
			РеквизитыОтгрузки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Реквизиты.ДокументОтгрузки, "Исправление,ИсправляемыйДокумент");
			Если РеквизитыОтгрузки.Исправление Тогда
				Запрос.УстановитьПараметр("ДокументОтгрузкиИсправление", Реквизиты.ДокументОтгрузки);
				Запрос.УстановитьПараметр("ДокументОтгрузки", РеквизитыОтгрузки.ИсправляемыйДокумент);
			КонецЕсли;
		КонецЕсли;
		Запрос.УстановитьПараметр("ДатаОтгрузки", ОпределитьДатуОтгрузки(Запрос.Параметры, Истина));
	КонецЕсли;
	
	Запрос.УстановитьПараметр("ВыводитьСчетаФактурыНаАванс", УчетНДСПереопределяемый.ВерсияПостановленияНДС1137(Запрос.Параметры.Период) >= 8);
	
	Если Не Реквизиты.НачислениеНДСПриОтгрузке
		И ДокументСсылка.ДокументыОснования.Количество() > 0 Тогда 
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Договор", ДокументСсылка.ДокументыОснования[0].ДокументОснование.Метаданные()) Тогда 
			Запрос.УстановитьПараметр("Договор", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка.ДокументыОснования[0].ДокументОснование, "Договор"));
		КонецЕсли;
		Запрос.УстановитьПараметр("ДатаОтгрузки", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка.ДокументыОснования[0].ДокументОснование, "Дата"));
	КонецЕсли;
...
Функция ТекстЗапросаВтТаблицаТоваров(Запрос, ТекстыЗапроса)
	...
	ИмяРегистра = "ВТДокументыОснованияДаты";
	
	СформироватьСуммыДокументаВВалютахУчета(Запрос, ТекстыЗапроса);
	
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ДатыДокументовОснования.Ссылка КАК Ссылка,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДатыДокументовОснования.ДокументОснование) = ТИП(Документ.КорректировкаРеализации)
	|			ТОГДА ВЫРАЗИТЬ(ДатыДокументовОснования.ДокументОснование КАК Документ.КорректировкаРеализации).ДокументОснование
	|		ИНАЧЕ ДатыДокументовОснования.ДокументОснование
	|	КОНЕЦ КАК ДокументОснование
	|ПОМЕСТИТЬ ВТДокументыОснованияДаты
	|ИЗ
	|	Документ.СчетФактураВыданный.ДокументыОснования КАК ДатыДокументовОснования
	|	
	|ГДЕ
	|	ДатыДокументовОснования.Ссылка = &Ссылка
	|	И НЕ &ВводОстатков";
	...
	|	ЕСТЬNULL(Суммы.БазаНДСРегл, 0) КАК СуммаРегл,
	...
	|	МИНИМУМ(ВЫБОР
	|			КОГДА РеестрДокументов.ДатаДокументаИБ ЕСТЬ NULL
	|				ТОГДА &Период
	...
	|		ПО СчетФактураТовары.Ссылка = Суммы.Ссылка
	|			И СчетФактураТовары.ИдентификаторСтроки = Суммы.ИдентификаторСтроки
	|			И &НачислениеНДСПриОтгрузке
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДокументыОснованияДаты КАК ДатыДокументовОснования
	|			ПО СчетФактураТовары.Ссылка = ДатыДокументовОснования.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументов
	|			ПО (ДатыДокументовОснования.ДокументОснование = РеестрДокументов.Ссылка)
	|ГДЕ
	|	СчетФактураТовары.Ссылка = &Ссылка
	|	И НЕ &ВводОстатков
	|
	|СГРУППИРОВАТЬ ПО
	|	СчетФактураТовары.СчетФактураПолученныйОтПродавца,
	|	СчетФактураТовары.СуммаНДСУвеличение,
	|	СчетФактураТовары.СуммаНДС,
	|	СчетФактураТовары.МежценоваяРазница,
	|	СчетФактураТовары.СуммаУвеличение,
	|	СчетФактураТовары.СуммаУменьшение,
	|	СчетФактураТовары.Ссылка,
	|	СчетФактураТовары.ТипЗапасов,
	|	СчетФактураТовары.Сумма,
	|	СчетФактураТовары.СуммаНДСУменьшение,
	|	СчетФактураТовары.ИсходныйСчетФактура,
	|	СчетФактураТовары.МежценоваяРазницаУменьшение,
	|	ВЫБОР
	|		КОГДА СчетФактураДокументыОснования.ДокументОснование ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ПрочиеРаботыИУслуги)
	|	КОНЕЦ,
	|	СчетФактураТовары.МежценоваяРазницаУвеличение,
	|	СчетФактураТовары.СтавкаНДС,
	|	СчетФактураТовары.ПредыдущийСчетФактура,
	|	СчетФактураТовары.Ссылка.СчетФактураОснование,
	|	СчетФактураТовары.СтавкаНДС.ПеречислениеСтавкаНДС,
	|	ЕСТЬNULL(Суммы.БазаНДСРегл, 0),
	|	ЕСТЬNULL(Суммы.СуммаНДСРегл, 0),
	|	СчетФактураТовары.НомерСтроки";
...
Функция ТекстЗапросаВтТаблицаДанныхДляЖурнала(Запрос, ТекстыЗапроса)
	...
	|	НЕОПРЕДЕЛЕНО,
	|	ВЫБОР
	|		КОГДА &НачислениеНДСПриОтгрузке И НЕ (&Исправление ИЛИ &Корректировочный)
	|			ТОГДА 0
	|		ИНАЧЕ
	...
	|	И (НЕ &НДССМежценовойРазницы ИЛИ &Исправление ИЛИ &Корректировочный ИЛИ &НачислениеНДСПриОтгрузке)
	|
	|СГРУППИРОВАТЬ ПО
	|	СчетФактураТовары.Ссылка,
	|	ЕСТЬNULL(СчетФактураТовары.ПредыдущийСчетФактура.СчетФактураОснование.Дата, СчетФактураТовары.ПредыдущийСчетФактура.Дата),
	...
	|	КОНЕЦ),
	|	ИСТИНА,
	|	ЕСТЬNULL(Продавцы.Продавец, ЕСТЬNULL(СчетФактураТовары.СчетФактураПолученныйОтПродавца.Контрагент, НЕОПРЕДЕЛЕНО)),
	|	ЕСТЬNULL(Продавцы.ИННПродавца, ЕСТЬNULL(СчетФактураТовары.СчетФактураПолученныйОтПродавца.ИННКонтрагента, НЕОПРЕДЕЛЕНО)),
	|	ЕСТЬNULL(Продавцы.КПППродавца, ЕСТЬNULL(СчетФактураТовары.СчетФактураПолученныйОтПродавца.КППКонтрагента, НЕОПРЕДЕЛЕНО)),
	...
	|	ПО
	|		СчетФактураТовары.СчетФактураПолученныйОтПродавца.Ссылка = Продавцы.Ссылка
	|		И СчетФактураТовары.СчетФактураПолученныйОтПродавца.СоставленКомиссионеромОтИмениПродавца
	|		И НЕ СчетФактураТовары.СчетФактураПолученныйОтПродавца.СводныйКомиссионный
	|ГДЕ
	...
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	МАКСИМУМ(СчетФактураТовары.Ссылка) КАК Ссылка,
	|	НЕОПРЕДЕЛЕНО КАК ДокументОснование,
	|	МАКСИМУМ(СчетФактураТовары.ПредыдущийСчетФактура.ПредставлениеНомера),
	|	МАКСИМУМ(СчетФактураТовары.ПредыдущийСчетФактура.СчетФактураОснование.Дата),
	|	МАКСИМУМ(СчетФактураТовары.ПредыдущийСчетФактура.НомерИсправления),
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА СчетФактураТовары.ПредыдущийСчетФактура.Исправление
	|			ТОГДА СчетФактураТовары.ПредыдущийСчетФактура.Дата
	|		ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	|	КОНЕЦ),
	|	0,
	|	0,
	...
	|	МАКСИМУМ(СчетФактураТовары.СчетФактураПолученныйОтПродавца.Номер),
	|	МАКСИМУМ(СчетФактураТовары.СчетФактураПолученныйОтПродавца.ДатаСоставления),
	|	Продавцы.НомерСтроки + МИНИМУМ(СчетФактураТовары.НомерСтроки)
	|ИЗ
	|	ВТТаблицаТоваров КАК СчетФактураТовары
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.СчетФактураПолученный.Продавцы КАК Продавцы
	...
	|		СчетФактураТовары.СчетФактураПолученныйОтПродавца.Ссылка = Продавцы.Ссылка
	|		И СчетФактураТовары.СчетФактураПолученныйОтПродавца.СоставленКомиссионеромОтИмениПродавца
	|		И СчетФактураТовары.СчетФактураПолученныйОтПродавца.СводныйКомиссионный
	|ГДЕ
	|	НЕ СчетФактураТовары.ТипЗапасов В (&ТипыЗапасовСобственные)
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаВтТаблицаКорректировки(Запрос, ТекстыЗапроса, ПрошлыеКорректировки = Ложь)
	...
	|			КОГДА &НачислениеНДСПриОтгрузке
	|				ТОГДА СчетФактураТовары.СуммаРегл + СчетФактураТовары.СуммаНДСРегл
	...
	|			КОГДА &НачислениеНДСПриОтгрузке
	|				ТОГДА СчетФактураТовары.СуммаНДСРегл
	...
	|	ВЫБОР
	|		КОГДА &Исправление
	|			ТОГДА ЕСТЬNULL(СчетФактураТовары.ПериодЗаписи, ДАТАВРЕМЯ(1, 1, 1))
	|		ИНАЧЕ &Период
...
Функция ТекстЗапросаВтТаблицаПодтвержденияСтавки0(Запрос, ТекстыЗапроса)
	...
	|	ВложенныйЗапрос.ДатаПодтверждения КАК ДатаПодтверждения
	|ПОМЕСТИТЬ ВтТаблицаПодтвержденияСтавки0Предварительная
	|ИЗ
	|	(ВЫБРАТЬ
	|		НДССостояниеРеализации0.Регистратор КАК ДокументРегистратор,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|			РегистрСведений.НДССостояниеРеализации0.СрезПоследних КАК НДССостояниеРеализации0
	|		ПО
	...
	|	ИЗ
	|		РегистрСведений.НДССостояниеРеализации0.СрезПоследних(, ДокументРеализации = &ДокументОснование) КАК НДССостояниеРеализации0
	|	ГДЕ
	|		&НачислениеНДСПриОтгрузке И НЕ &Исправление И НЕ &Корректировочный
	|		И &НалогообложениеНДС В (ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт),
	|							ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортСырьевыхТоваровУслуг),
	|							ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортНесырьевыхТоваров))) КАК ВложенныйЗапрос
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ВтТаблицаПодтвержденияСтавки0.ДокументРегистратор КАК ДокументРегистратор,
	|	ВтТаблицаПодтвержденияСтавки0.ДокументРеализации КАК ДокументРеализации,
	|	ВтТаблицаПодтвержденияСтавки0.Состояние КАК Состояние,
	|	ВтТаблицаПодтвержденияСтавки0.ДатаПодтверждения КАК ДатаПодтверждения
	|ПОМЕСТИТЬ ВтТаблицаПодтвержденияСтавки0
	|ИЗ
	|	ВтТаблицаПодтвержденияСтавки0Предварительная КАК ВтТаблицаПодтвержденияСтавки0
	|УПОРЯДОЧИТЬ ПО
	|	ДатаПодтверждения УБЫВ
	|";
...
Функция ТекстЗапросаВтНДСЗаписиКнигиПродажДокументаОснования(Запрос, ТекстыЗапроса)
	...
	|		КОГДА &НачислениеНДСПриОтгрузке
	|			И НЕ &Корректировочный
	|			ТОГДА &ДокументОтгрузки
	|		КОГДА &Исправление
	|			ТОГДА &СчетФактураОснование
	|		КОГДА &ВозвратПоЭкспорту
	|			ТОГДА ПодтверждениеСтавки0.ДокументРеализации
	|		ИНАЧЕ
	|			&Ссылка
	|	КОНЕЦ,
	|	ВЫБОР
...
#Область ТекстКорректировка
	...
	|	ВЫБОР
	|		КОГДА &Исправление
	|			И НАЧАЛОПЕРИОДА(ТаблицаДанных.ПериодКорректировки, КВАРТАЛ) 
...
Функция ТекстЗапросаТаблицаНДСЗаписиКнигиПродажПредварительная(Запрос, ТекстыЗапроса)
	
	ИмяРегистра = "НДСЗаписиКнигиПродажПредварительная";
...
#Область ТекстКорректировка
	...
	|			ТОГДА &Ссылка
	|	КОНЕЦ КАК ИсправленныйСчетФактура,
	|	(&Исправление ИЛИ &ВозвратПоЭкспорту) КАК Исправление,
	...
	|	СУММА(ТаблицаДанных.СуммаНДС * &КоэффициентПересчетаВВалютуУпрРегл) КАК НДСУпр
	|ПОМЕСТИТЬ НДСЗаписиКнигиПродажПредварительная
	|ИЗ
	|	ВтТаблицаКорректировки КАК ТаблицаДанных
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтТаблицаПодтвержденияСтавки0 КАК ПодтверждениеСтавки0
	|	ПО ПодтверждениеСтавки0.Состояние = ЗНАЧЕНИЕ(Перечисление.НДССостоянияРеализация0.ПодтвержденаРеализация0)
	...
	|	ТаблицаДанных.Сумма + ТаблицаДанных.СуммаУменьшение > 0
	|	И НЕ ТаблицаДанных.Комиссия
	|	И (&Корректировочный ИЛИ &Исправление ИЛИ &НДССМежценовойРазницы ИЛИ &НачислениеНДСПриОтгрузке)
	|	И &Организация <> ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация)
	|	И (НЕ &НалогообложениеНДС В (ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортСырьевыхТоваровУслуг),
	|								ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортНесырьевыхТоваров),
	|								ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт))
	|		ИЛИ ТаблицаДанных.СтавкаНДС <> ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС0)
	|		ИЛИ НЕ ПодтверждениеСтавки0.Состояние ЕСТЬ NULL
	|			И НЕ ПодтверждениеСтавки0.ДокументРегистратор ССЫЛКА Документ.ПодтверждениеНулевойСтавкиНДС)
	|	И НЕ ТаблицаДанных.СчетФактураСОсобымОтражением
	|
	|СГРУППИРОВАТЬ ПО
	|	&Период,
...
#КонецОбласти
	...
	ТекстЗапроса = ТекстЗапросаКорректировка
...
Функция ТекстЗапросаТаблицаНДСЗаписиКнигиПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса("НДСЗаписиКнигиПродажПредварительная", ТекстыЗапроса) Тогда
		ТекстЗапросаТаблицаНДСЗаписиКнигиПродажПредварительная(Запрос, ТекстыЗапроса);
	КонецЕсли;
...
#Область ТекстСторноИсправление
	...
	|	И НЕ НДСЗаписиКнигиПродаж.Регистратор ССЫЛКА Документ.ПодтверждениеНулевойСтавкиНДС
	|	И (ВЫБОР 
	...
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ)
	|	И ПодтверждениеСтавки0.ДатаПодтверждения ЕСТЬ NULL
	|
	|СГРУППИРОВАТЬ ПО
	|	&Период,
	|	&Организация,
	...
	|	НДСЗаписиКнигиПродаж.НомерСчетаФактурыНаАванс,
	|	НДСЗаписиКнигиПродаж.ДатаСчетаФактурыНаАванс,
	|	НДСЗаписиКнигиПродаж.НомерИсправленияСчетаФактурыНаАванс,
	|	НДСЗаписиКнигиПродаж.ДатаИсправленияСчетаФактурыНаАванс,
	|	ВЫБОР
	|		КОГДА НДСЗаписиКнигиПродаж.ЗаписьДополнительногоЛиста
	|			ТОГДА ИСТИНА
	|		КОГДА НАЧАЛОПЕРИОДА(НДСЗаписиКнигиПродаж.Период,КВАРТАЛ) 
	...
	ТекстыПодзапроса.Добавить(ТекстЗапросаСторноИсправление);
	
#КонецОбласти
	
#Область ОсновныеЗаписи
	ТекстЗапросаОсновныеЗаписи = "
	|ВЫБРАТЬ
	...
	|ИЗ
	|	НДСЗаписиКнигиПродажПредварительная КАК НДСЗаписиКнигиПродаж
	|";
	
	ТекстыПодзапроса.Добавить(ТекстЗапросаОсновныеЗаписи);
...
#Область ТекстДатаИНомерДокументаОплаты
	...
	|	ЕСТЬNULL(ВложенныйЗапрос.ДатаОплаты, ДАТАВРЕМЯ(1,1,1)) КАК ДатаОплаты,
	...
	|	ВложенныйЗапрос.ЗаписьДополнительногоЛиста КАК ЗаписьДополнительногоЛиста,
	|	ЕСТЬNULL(ВложенныйЗапрос.КорректируемыйПериод, ДАТАВРЕМЯ(1,1,1)) КАК КорректируемыйПериод,
	|	ВложенныйЗапрос.СторнирующаяЗаписьДопЛиста КАК СторнирующаяЗаписьДопЛиста,
	|	ЕСТЬNULL(ВложенныйЗапрос.ДоговорКонтрагента, НЕОПРЕДЕЛЕНО) КАК ДоговорКонтрагента,
	|	ВложенныйЗапрос.ИсправленныйСчетФактура КАК ИсправленныйСчетФактура,
	|	ЛОЖЬ КАК Исправление,
	|	ЕСТЬNULL(ВложенныйЗапрос.ДатаСчетаФактурыКомиссионера, ДАТАВРЕМЯ(1,1,1)) КАК ДатаСчетаФактурыКомиссионера,
	|	ВложенныйЗапрос.СуммаБезНДС КАК СуммаБезНДС,
	|	ВложенныйЗапрос.НДС КАК НДС,
	|	ВложенныйЗапрос.НомерДокументаОплаты КАК НомерДокументаОплаты,
	|	ВложенныйЗапрос.ДатаДокументаОплаты КАК ДатаДокументаОплаты,
	|	ВложенныйЗапрос.НомерСчетаФактурыНаАванс КАК НомерСчетаФактурыНаАванс,
	|	ВложенныйЗапрос.ДатаСчетаФактурыНаАванс КАК ДатаСчетаФактурыНаАванс,
	|	ВложенныйЗапрос.НомерИсправленияСчетаФактурыНаАванс КАК НомерИсправленияСчетаФактурыНаАванс,
	|	ВложенныйЗапрос.ДатаИсправленияСчетаФактурыНаАванс КАК ДатаИсправленияСчетаФактурыНаАванс,
	|	ВложенныйЗапрос.НДСУпр КАК НДСУпр
	|	ИЗ(
	|	ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА &Исправление
	...
	|	НДСЗаписиКнигиПродаж.ВидЦенности КАК ВидЦенности,
	|	НДСЗаписиКнигиПродаж.СтавкаНДС КАК СтавкаНДС,
	|	ЕСТЬNULL(НДСЗаписиКнигиПродаж.ДатаОплаты, ДАТАВРЕМЯ(1,1,1)) КАК ДатаОплаты,
	|	ЕСТЬNULL(НДСЗаписиКнигиПродаж.ДокументОплаты, НЕОПРЕДЕЛЕНО) КАК ДокументОплаты,
	|	НДСЗаписиКнигиПродаж.Событие КАК Событие,
	|	ВЫБОР
	|		КОГДА &Исправление
	|			ТОГДА &Период
	...
	|	КОНЕЦ КАК ДатаСобытия,
	|	НДСЗаписиКнигиПродаж.ЗаписьДополнительногоЛиста КАК ЗаписьДополнительногоЛиста,
	|	ЕСТЬNULL(НДСЗаписиКнигиПродаж.КорректируемыйПериод, ДАТАВРЕМЯ(1,1,1)) КАК КорректируемыйПериод,
	|	НДСЗаписиКнигиПродаж.СторнирующаяЗаписьДопЛиста КАК СторнирующаяЗаписьДопЛиста,
	|	ЕСТЬNULL(НДСЗаписиКнигиПродаж.ДоговорКонтрагента, НЕОПРЕДЕЛЕНО) КАК ДоговорКонтрагента,
	|	ВЫБОР
	|		КОГДА &Исправление
	|			ТОГДА &Ссылка
	|	КОНЕЦ КАК ИсправленныйСчетФактура,
	|	ЕСТЬNULL(НДСЗаписиКнигиПродаж.ДатаСчетаФактурыКомиссионера, ДАТАВРЕМЯ(1,1,1)) КАК ДатаСчетаФактурыКомиссионера,
	|	0 КАК СуммаБезНДС,
	|	0 КАК НДС,
	|	ПлатежноРасчетныеДокументы.НомерПлатежноРасчетногоДокумента КАК НомерДокументаОплаты,
	|	ПлатежноРасчетныеДокументы.ДатаПлатежноРасчетногоДокумента КАК ДатаДокументаОплаты,
	|	ВЫБОР
	|		КОГДА &ВыводитьСчетаФактурыНаАванс
	|			ТОГДА ПлатежноРасчетныеДокументы.НомерСчетаФактурыНаАванс
	|		ИНАЧЕ
	|			NULL
	|	КОНЕЦ КАК НомерСчетаФактурыНаАванс,
	|	ВЫБОР
	|		КОГДА &ВыводитьСчетаФактурыНаАванс
	|			ТОГДА ПлатежноРасчетныеДокументы.ДатаСчетаФактурыНаАванс
	|		ИНАЧЕ
	|			NULL
	|	КОНЕЦ КАК ДатаСчетаФактурыНаАванс,
	|	ВЫБОР
	|		КОГДА &ВыводитьСчетаФактурыНаАванс
	|			ТОГДА ПлатежноРасчетныеДокументы.НомерИсправленияСчетаФактурыНаАванс
	|		ИНАЧЕ
	|			NULL
	|	КОНЕЦ КАК НомерИсправленияСчетаФактурыНаАванс,
	|	ВЫБОР
	|		КОГДА &ВыводитьСчетаФактурыНаАванс
	|			ТОГДА ПлатежноРасчетныеДокументы.ДатаИсправленияСчетаФактурыНаАванс
	|		ИНАЧЕ
	|			NULL
	...
	ТекстыПодзапроса.Добавить(ТекстЗапросаДатаИНомерДокументаОплаты);
	
#КонецОбласти
	
	ТекстЗапроса = СтрСоединить(ТекстыПодзапроса, ОбщегоНазначения.ТекстОбъединитьВсе());
	ТекстыЗапроса.Добавить(ТекстЗапроса, ИмяРегистра);
	Возврат ТекстЗапроса
...
Функция ТекстЗапросаВтТаблицаКорректировкиЗачетаАвансов(Запрос, ТекстыЗапроса)
	
	ИмяРегистра = "ВтТаблицаКорректировкиЗачетаАвансов";
	
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	&Ссылка КАК Ссылка,
	|	ЗачетыАвансовСчетамиФактурами.Ссылка КАК ИсправляемыйСчетФактура,
	|	ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов КАК ОбъектРасчетов,
	|	ЗачетыАвансовСчетамиФактурами.РасчетныйДокумент КАК РасчетныйДокумент,
	|	СУММА(ЗачетыАвансовСчетамиФактурами.Сумма) КАК Сумма,
	|	СУММА(ЗачетыАвансовСчетамиФактурами.СуммаРегл) КАК СуммаРегл
	|ПОМЕСТИТЬ втЗачтенныеАвансыИсправления
	|ИЗ
	|	Документ.СчетФактураВыданный.ПлатежноРасчетныеДокументы КАК ЗачетыАвансовСчетамиФактурами
	|ГДЕ
	|	ЗачетыАвансовСчетамиФактурами.Ссылка В (&ИсправляемыйСчетФактура)
	|	И ЗачетыАвансовСчетамиФактурами.Ссылка.Проведен
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗачетыАвансовСчетамиФактурами.Ссылка,
	|	ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов,
	|	ЗачетыАвансовСчетамиФактурами.РасчетныйДокумент
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СчетФактураВыданныйПлатежноРасчетныеДокументы.Ссылка КАК Ссылка,
	|	СчетФактураВыданныйПлатежноРасчетныеДокументы.ОбъектРасчетов КАК ОбъектРасчетов,
	|	СчетФактураВыданныйПлатежноРасчетныеДокументы.РасчетныйДокумент КАК РасчетныйДокумент,
	|	СУММА(СчетФактураВыданныйПлатежноРасчетныеДокументы.Сумма) КАК Сумма,
	|	СУММА(СчетФактураВыданныйПлатежноРасчетныеДокументы.СуммаРегл) КАК СуммаРегл
	|ПОМЕСТИТЬ втАвансыСчетаФактуры
	|ИЗ
	...
	|	СчетФактураВыданныйПлатежноРасчетныеДокументы.ОбъектРасчетов,
	|	СчетФактураВыданныйПлатежноРасчетныеДокументы.РасчетныйДокумент
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаТовары.Ссылка КАК Ссылка,
	|	ЛОЖЬ КАК ЭтоРегистратор,
	|	СУММА(ТаблицаТовары.СуммаБезНДСВал) КАК Сумма,
	|	СУММА(Суммы.СуммаБезНДСРегл + Суммы.СуммаНДСРегл) КАК СуммаРегл
	|ПОМЕСТИТЬ втСуммыСчетовФактур
	|ИЗ
	...
	|
	|ГДЕ
	|	ТаблицаТовары.Ссылка В (&ИсправляемыйСчетФактура)
	|	И НЕ &Корректировочный
	|
	|СГРУППИРОВАТЬ ПО
	...
	|			И ТаблицаТовары.ИдентификаторСтроки = Суммы.ИдентификаторСтроки
	|ГДЕ
	|	ТаблицаТовары.Ссылка В (&ИсходныеСчетаФактуры)
	|	И &Корректировочный
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаТовары.Ссылка
	...
	|ВЫБРАТЬ
	|	ТаблицаТовары.Ссылка КАК Ссылка,
	|	ИСТИНА КАК ЭтоРегистратор,
	|	СУММА(ТаблицаТовары.СуммаБезНДСВалУвеличение
	|		- ТаблицаТовары.СуммаБезНДСВалУменьшение
	|		+ ТаблицаТовары.СуммаБезНДСВал
	|			* ВЫБОР
	|				КОГДА &Корректировочный
	|					ТОГДА 0
	|				ИНАЧЕ 1
	...
	|		ПО ТаблицаТовары.ИдентификаторСтроки = Суммы.ИдентификаторСтроки
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	втЗачтенныеАвансыИсправления.Ссылка КАК Ссылка,
	|	втЗачтенныеАвансыИсправления.ИсправляемыйСчетФактура КАК ИсправляемыйСчетФактура,
	|	СУММА(втЗачтенныеАвансыИсправления.Сумма) КАК Сумма,
	|	СУММА(втЗачтенныеАвансыИсправления.СуммаРегл) КАК СуммаРегл
	|ПОМЕСТИТЬ втЗачтенныеАвансыИсправленияПоДокументу
	|ИЗ
	|	втЗачтенныеАвансыИсправления КАК втЗачтенныеАвансыИсправления
	|
	|СГРУППИРОВАТЬ ПО
	|	втЗачтенныеАвансыИсправления.Ссылка,
	|	втЗачтенныеАвансыИсправления.ИсправляемыйСчетФактура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	&Ссылка КАК Ссылка,
	|	ЗачетыАвансовСчетамиФактурами.Регистратор КАК ИсходныйСчетФактура,
	|	СУММА(ЗачетыАвансовСчетамиФактурами.Сумма) КАК Сумма,
	|	СУММА(ЗачетыАвансовСчетамиФактурами.СуммаРегл) КАК СуммаРегл
	|ПОМЕСТИТЬ втЗачтенныеАвансыИсходныхСчетовФактурПоДокументу
	|ИЗ
	|	РегистрНакопления.ЗачетыАвансовСчетамиФактурами КАК ЗачетыАвансовСчетамиФактурами
	|ГДЕ
	|	ЗачетыАвансовСчетамиФактурами.Регистратор В (&ИсходныеСчетаФактуры)
	|	И &Корректировочный
	|	И ЗачетыАвансовСчетамиФактурами.Активность
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗачетыАвансовСчетамиФактурами.Регистратор
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	&Ссылка КАК Ссылка,
	|	СУММА(втАвансыСчетаФактуры.Сумма) КАК Сумма,
	|	СУММА(втАвансыСчетаФактуры.СуммаРегл) КАК СуммаРегл
	|ПОМЕСТИТЬ втАвансыСчетаФактурыПоДокументу
	|ИЗ
	|	втАвансыСчетаФактуры КАК втАвансыСчетаФактуры
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ВложенныйЗапрос.ОбъектРасчетов КАК ОбъектРасчетов
	|ПОМЕСТИТЬ втОбъектРасчетовДляВосстановленияАванса
	|ИЗ	
	|	(ВЫБРАТЬ ПЕРВЫЕ 1
	|		ОбъектыРасчетов.Ссылка КАК ОбъектРасчетов,
	|		ОбъектыРасчетов.Дата КАК ДатаОбъектаРасчетов,
	|		ИСТИНА КАК ОтборПоОбъекту
	|	ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	втАвансыСчетаФактуры.Ссылка КАК Ссылка,
	|	втАвансыСчетаФактуры.ОбъектРасчетов КАК ОбъектРасчетов,
	|	втАвансыСчетаФактуры.РасчетныйДокумент КАК РасчетныйДокумент,
	|	втАвансыСчетаФактуры.Сумма - ЕСТЬNULL(втЗачтенныеАвансы.Сумма, 0) КАК Сумма,
	|	втАвансыСчетаФактуры.СуммаРегл - ЕСТЬNULL(втЗачтенныеАвансы.СуммаРегл, 0) КАК СуммаРегл,
	|	ЛОЖЬ КАК КорректировкаНаУменьшение
	|ПОМЕСТИТЬ ВтТаблицаКорректировкиЗачетаАвансов
	|ИЗ
	|	втАвансыСчетаФактуры КАК втАвансыСчетаФактуры
	|		ЛЕВОЕ СОЕДИНЕНИЕ втЗачтенныеАвансыИсправления КАК втЗачтенныеАвансы
	|		ПО втАвансыСчетаФактуры.Ссылка = втЗачтенныеАвансы.ссылка
	|			И втАвансыСчетаФактуры.ОбъектРасчетов = втЗачтенныеАвансы.ОбъектРасчетов
	|			И втАвансыСчетаФактуры.РасчетныйДокумент = втЗачтенныеАвансы.РасчетныйДокумент
	|ГДЕ
	|	втАвансыСчетаФактуры.Сумма - ЕСТЬNULL(втЗачтенныеАвансы.Сумма, 0) > 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	&Ссылка,
	|	втЗачтенныеАвансы.ОбъектРасчетов,
	|	втЗачтенныеАвансы.РасчетныйДокумент,
	|	-(втЗачтенныеАвансы.Сумма - ЕСТЬNULL(втАвансыСчетаФактуры.Сумма, 0)),
	|	-(втЗачтенныеАвансы.Сумма - ЕСТЬNULL(втАвансыСчетаФактуры.Сумма, 0)) * &КоэффициентПересчетаВВалютуВзаиморасчетовРегл,
	|	ИСТИНА
	|ИЗ
	|	втЗачтенныеАвансыИсправления КАК втЗачтенныеАвансы
	|		ЛЕВОЕ СОЕДИНЕНИЕ втАвансыСчетаФактуры КАК втАвансыСчетаФактуры
	|		ПО втЗачтенныеАвансы.Ссылка = втАвансыСчетаФактуры.ссылка
	|			И втЗачтенныеАвансы.ОбъектРасчетов = втАвансыСчетаФактуры.ОбъектРасчетов
	|			И втЗачтенныеАвансы.РасчетныйДокумент = втАвансыСчетаФактуры.РасчетныйДокумент
	|		ЛЕВОЕ СОЕДИНЕНИЕ втОбъектРасчетовДляВосстановленияАванса КАК втОбъектРасчетовДляВосстановленияАванса
	|		ПО ИСТИНА
	|
	|ГДЕ
	|	втЗачтенныеАвансы.Сумма - ЕСТЬNULL(втАвансыСчетаФактуры.Сумма, 0) > 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	&Ссылка,
	|	втОбъектРасчетовДляВосстановленияАванса.ОбъектРасчетов,
	|	&Ссылка,
	|	втСуммыСчетовФактурОснований.Сумма - втЗачтенныеАвансыИсходныхСчетовФактурПоДокументу.Сумма + втСуммыСчетовФактур.Сумма,
	|	(втСуммыСчетовФактурОснований.Сумма - втЗачтенныеАвансыИсходныхСчетовФактурПоДокументу.Сумма + втСуммыСчетовФактур.Сумма) * &КоэффициентПересчетаВВалютуВзаиморасчетовРегл,
	|	ИСТИНА
	|ИЗ
	|	втСуммыСчетовФактур КАК втСуммыСчетовФактурОснований
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втЗачтенныеАвансыИсходныхСчетовФактурПоДокументу КАК втЗачтенныеАвансыИсходныхСчетовФактурПоДокументу
	|		ПО втСуммыСчетовФактурОснований.Ссылка = втЗачтенныеАвансыИсходныхСчетовФактурПоДокументу.ИсходныйСчетФактура
	|			И НЕ втСуммыСчетовФактурОснований.ЭтоРегистратор
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втСуммыСчетовФактур КАК втСуммыСчетовФактур
	|		ПО втСуммыСчетовФактур.Ссылка = втЗачтенныеАвансыИсходныхСчетовФактурПоДокументу.Ссылка
	|			И втСуммыСчетовФактур.ЭтоРегистратор
	|		ЛЕВОЕ СОЕДИНЕНИЕ втОбъектРасчетовДляВосстановленияАванса КАК втОбъектРасчетовДляВосстановленияАванса
	|		ПО ИСТИНА
	|
	|ГДЕ
	|	(втСуммыСчетовФактурОснований.Сумма - втЗачтенныеАвансыИсходныхСчетовФактурПоДокументу.Сумма + втСуммыСчетовФактур.Сумма) < 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	втОбъектРасчетовДляВосстановленияАванса.ОбъектРасчетов,
	|	&Ссылка,
	|	-Операция.СуммаРучногоВосстановленияАванса,
	|	-Операция.СуммаРучногоВосстановленияАванса * &КоэффициентПересчетаВВалютуВзаиморасчетовРегл,
	|	ИСТИНА
	|ИЗ
	|	Документ.СчетФактураВыданный КАК Операция
	|		ЛЕВОЕ СОЕДИНЕНИЕ втОбъектРасчетовДляВосстановленияАванса КАК втОбъектРасчетовДляВосстановленияАванса
	...
	|	И Операция.СуммаРучногоВосстановленияАванса > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ВтТаблицаКорректировки.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ВтЕстьИзменениеЗачетаАвансов
	|ИЗ
	|	ВтТаблицаКорректировкиЗачетаАвансов КАК ВтТаблицаКорректировки";
...
Функция ТекстЗапросаТаблицаЗачетыАвансов(Запрос, ТекстыЗапроса, Регистры)

	ИмяРегистра = "ЗачетыАвансовСчетамиФактурами";
	
	Если НЕ ПроведениеДокументов.ТребуетсяТаблицаДляДвижений(ИмяРегистра, Регистры) Тогда
		Возврат "";
	КонецЕсли;
	
	Если Не Запрос.Параметры.НачислениеНДСПриОтгрузке Тогда 
		Возврат "";
	КонецЕсли;
	
	Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтТаблицаКорректировкиЗачетаАвансов", ТекстыЗапроса) Тогда
		ТекстЗапросаВтТаблицаКорректировкиЗачетаАвансов(Запрос, ТекстыЗапроса);
	КонецЕсли;
	...
	|	&Контрагент     КАК Контрагент,
	|	&Договор        КАК Договор,
	|	ВтТаблицаКорректировкиЗачетаАвансов.ОбъектРасчетов                       КАК ОбъектРасчетов,
	|	ВЫБОР
	|		КОГДА ВтТаблицаКорректировкиЗачетаАвансов.КорректировкаНаУменьшение
	|			ТОГДА &Ссылка
	|		ИНАЧЕ
	|			ВтТаблицаКорректировкиЗачетаАвансов.РасчетныйДокумент
	|	КОНЕЦ                                                                   КАК РасчетныйДокумент,
	|	СУММА(ВтТаблицаКорректировкиЗачетаАвансов.Сумма)                        КАК Сумма,
	|	СУММА(ВтТаблицаКорректировкиЗачетаАвансов.СуммаРегл)                    КАК СуммаРегл,
	|	ВтТаблицаКорректировкиЗачетаАвансов.КорректировкаНаУменьшение           КАК КорректировкаНаУменьшение
	|ИЗ
	|	ВтТаблицаКорректировкиЗачетаАвансов КАК ВтТаблицаКорректировкиЗачетаАвансов
	|ГДЕ
	|	НЕ &ВводОстатков
	|
	|СГРУППИРОВАТЬ ПО
	|	ВтТаблицаКорректировкиЗачетаАвансов.ОбъектРасчетов,
	|	ВЫБОР
	|		КОГДА ВтТаблицаКорректировкиЗачетаАвансов.КорректировкаНаУменьшение
	|			ТОГДА &Ссылка
	|		ИНАЧЕ
	|			ВтТаблицаКорректировкиЗачетаАвансов.РасчетныйДокумент
	|	КОНЕЦ,
	|	ВтТаблицаКорректировкиЗачетаАвансов.КорректировкаНаУменьшение";
...
Функция ТекстЗапросаТаблицаРаспоряженияНаОтгрузку(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	РаспоряженияНаОтгрузкуСторно.НомерСтроки			КАК НомерСтроки,
	|	&Период												КАК Период,
	|	РаспоряженияНаОтгрузкуСторно.Распоряжение			КАК Распоряжение,
	|	РаспоряженияНаОтгрузкуСторно.Номенклатура			КАК Номенклатура,
	|	РаспоряженияНаОтгрузкуСторно.Характеристика			КАК Характеристика,
	|	РаспоряженияНаОтгрузкуСторно.КодСтроки				КАК КодСтроки,
	|	ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)			КАК Склад,
	|	ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)	КАК Серия,
	|	ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.ОформленоСчетовФактур) КАК Показатель,
	|	-РаспоряженияНаОтгрузкуСторно.Количество			КАК Количество,
	|	-РаспоряженияНаОтгрузкуСторно.СуммаНДС				КАК СуммаНДС,
	|	-РаспоряженияНаОтгрузкуСторно.СуммаНДСУпр			КАК СуммаНДСУпр,
	|	-РаспоряженияНаОтгрузкуСторно.СуммаНДСРегл			КАК СуммаНДСРегл,
	|	-РаспоряженияНаОтгрузкуСторно.Сумма					КАК Сумма,
	|	ИСТИНА												КАК Сторно
	|ИЗ
	|	РегистрНакопления.РаспоряженияНаОтгрузкуИВозврат КАК РаспоряженияНаОтгрузкуСторно
	|ГДЕ
	|	РаспоряженияНаОтгрузкуСторно.Регистратор = &ИсправляемыйСчетФактура
	|	И &Исправление
	|	И НЕ &ВводОстатков
	|	И НЕ РаспоряженияНаОтгрузкуСторно.Сторно
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)	КАК Серия,
	|	ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.ОформленоСчетовФактур) КАК Показатель,
	|	ТаблицаТовары.КоличествоУвеличение
	|		- ТаблицаТовары.КоличествоУменьшение
	...
	|	ЕСТЬNULL(Суммы.СуммаНДС, 0)							КАК СуммаНДС,
	|	ЕСТЬNULL(Суммы.СуммаНДСУпр, 0)						КАК СуммаНДСУпр,
	|	ЕСТЬNULL(Суммы.СуммаНДСРегл, 0)						КАК СуммаНДСРегл,
	|	ТаблицаТовары.СуммаБезНДСВалУвеличение
	|		- ТаблицаТовары.СуммаБезНДСВалУменьшение
	|		+ ТаблицаТовары.СуммаБезНДСВал
	|			* ВЫБОР
	|				КОГДА &Корректировочный
	...
	|	И НЕ ТаблицаТовары.Распоряжение = НЕОПРЕДЕЛЕНО
	|	И НЕ &ВводОстатков
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки,
...
Процедура РассчитатьСуммуНДСРеглПриОтгрузке(ТаблицаТовары, ДополнительныеПараметры) Экспорт
	
	ВалютаРегламентированногоУчета = ДополнительныеПараметры.ВалютаРегламентированногоУчета;
	ЭтоКорректировочный = ДополнительныеПараметры.ЭтоКорректировочный;
	
	Если ДополнительныеПараметры.ВалютаВзаиморасчетов = ВалютаРегламентированногоУчета
		Или ДополнительныеПараметры.ВалютаВзаиморасчетов = ДополнительныеПараметры.ВалютаСчетаФактуры Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыЗаполнения = Новый Структура("ДокументОснование, Исправление, Корректировочный", 
							ДополнительныеПараметры.ДокументОснование, ДополнительныеПараметры.ЭтоИсправление, ДополнительныеПараметры.ЭтоКорректировочный);
	ПараметрыЗаполнения.Вставить("СчетФактураОснование", ?(ДополнительныеПараметры.Свойство("СчетФактураОснование"), ДополнительныеПараметры.СчетФактураОснование, Неопределено));
	ПараметрыЗаполнения.Вставить("Период", ДополнительныеПараметры.Период);
	ДатаОтгрузки = ОпределитьДатуОтгрузки(ПараметрыЗаполнения, Истина);
	
	Если Не ДополнительныеПараметры.ОплатаВВалюте Тогда // у.е. , зачет по курсу аванса
		
		Если ДополнительныеПараметры.ЭтоКорректировочный Тогда 
			
			// определим общую сумму увеличения с НДС в валюте
			РазницаСНДСВал = 0;
			СуммаСНДСВалУвеличение = 0;
			МассивКоэффициентов = Новый Массив;
			Для Каждого СтрокаТовары Из ТаблицаТовары Цикл 
				РазницаСНДСВал = РазницаСНДСВал + СтрокаТовары.СуммаБезНДСВалУвеличение - СтрокаТовары.СуммаБезНДСВалУменьшение;
				СуммаСНДСВалУвеличение = СуммаСНДСВалУвеличение + СтрокаТовары.СуммаБезНДСВалУвеличение;
				МассивКоэффициентов.Добавить(СтрокаТовары.СуммаБезНДСВалУвеличение);
			КонецЦикла;
			
			// определим общую дельту с НДС в руб.
			СуммаАвансаРегл = ?(РазницаСНДСВал < 0, 0, ?(ДополнительныеПараметры.ПлатежноРасчетныеДокументы = Неопределено, 0, ДополнительныеПараметры.ПлатежноРасчетныеДокументы.Итог("СуммаРегл")));
			СуммаАвансаВал = ?(РазницаСНДСВал < 0, 0, ?(ДополнительныеПараметры.ПлатежноРасчетныеДокументы = Неопределено, 0, ДополнительныеПараметры.ПлатежноРасчетныеДокументы.Итог("Сумма")));
			
			СуммаЗадолженностиВал = СуммаСНДСВалУвеличение - СуммаАвансаВал;
			СуммаСНДСУвеличениеРегл = СуммаАвансаРегл + РаботаСКурсамиВалютУТ.ПересчитатьВВалюту(
					СуммаЗадолженностиВал,
					ВалютаРегламентированногоУчета,
					ДополнительныеПараметры.ВалютаВзаиморасчетов,
					ВалютаРегламентированногоУчета,
					ДатаОтгрузки);
					
			МассивСуммСНДСРегл = ОбщегоНазначенияКлиентСервер.РаспределитьСуммуПропорциональноКоэффициентам(СуммаСНДСУвеличениеРегл, МассивКоэффициентов);
		
			СтруктураПоиска = Новый Структура("Распоряжение, Номенклатура, КодСтроки");
			
			Если МассивСуммСНДСРегл <> Неопределено Тогда // сумма увеличения <> сумме уменьшения
				
				ИндексМассиваСумм = 0;
				Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
					
					ЗначениеСтавкиНДС = УчетНДСУПКлиентСервер.ЗначениеСтавкиНДС(СтрокаТовары.СтавкаНДС);
					СтрокаТовары.СуммаНДСУвеличение = МассивСуммСНДСРегл[ИндексМассиваСумм] * ЗначениеСтавкиНДС / (ЗначениеСтавкиНДС + 100);
					СтрокаТовары.СуммаУвеличение = МассивСуммСНДСРегл[ИндексМассиваСумм];
					ИндексМассиваСумм = ИндексМассиваСумм + 1;
					
					СтрокаТовары.СуммаУменьшение = РаботаСКурсамиВалютУТ.ПересчитатьВВалюту(
							СтрокаТовары.СуммаБезНДСВалУменьшение,
							ВалютаРегламентированногоУчета,
							ДополнительныеПараметры.ВалютаВзаиморасчетов,
							ВалютаРегламентированногоУчета,
							ДатаОтгрузки);
					СтрокаТовары.СуммаНДСУменьшение = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(СтрокаТовары.СуммаУменьшение, СтрокаТовары.СтавкаНДС);
					
					ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаТовары);
					НайденныеСтроки = СтрокаТовары.ПредыдущийСчетФактура.Товары.НайтиСтроки(СтруктураПоиска);
					СтрокаТовары.Сумма = ?(НайденныеСтроки.Количество() = 0, 0, НайденныеСтроки[0].Сумма) + СтрокаТовары.СуммаУвеличение - СтрокаТовары.СуммаУменьшение;
					СтрокаТовары.СуммаНДС = ?(НайденныеСтроки.Количество() = 0, 0, НайденныеСтроки[0].СуммаНДС) + СтрокаТовары.СуммаНДСУвеличение - СтрокаТовары.СуммаНДСУменьшение;
					
				КонецЦикла;
				
			Иначе
				
				РассчитатьСуммуНДСРеглПриОтгрузкеБезАвансов(ТаблицаТовары, Новый Структура("ДатаОтгрузки, ВалютаСчетаФактуры, ВалютаРегламентированногоУчета, ВалютаВзаиморасчетов, ОплатаВВалюте", ДатаОтгрузки, ДополнительныеПараметры.ВалютаСчетаФактуры, ВалютаРегламентированногоУчета, ДополнительныеПараметры.ВалютаВзаиморасчетов, Ложь));
				
			КонецЕсли;
			
		Иначе
			
			//определим общую сумму с НДС в валюте
			СуммаСНДСВал = 0;
			МассивКоэффициентов = Новый Массив;
			Для Каждого СтрокаТовары Из ТаблицаТовары Цикл 
				СуммаСНДСВал = СуммаСНДСВал + СтрокаТовары.СуммаБезНДСВал;
				МассивКоэффициентов.Добавить(СтрокаТовары.СуммаБезНДСВал);
			КонецЦикла;
			
			// определим общую сумму с НДС в руб.
			СуммаАвансаРегл = ?(ДополнительныеПараметры.ПлатежноРасчетныеДокументы = Неопределено, 0, ДополнительныеПараметры.ПлатежноРасчетныеДокументы.Итог("СуммаРегл"));
			СуммаАвансаВал = ?(ДополнительныеПараметры.ПлатежноРасчетныеДокументы = Неопределено, 0, ДополнительныеПараметры.ПлатежноРасчетныеДокументы.Итог("Сумма"));
			
			СуммаЗадолженностиВал = СуммаСНДСВал - СуммаАвансаВал;
			СуммаСНДСРегл = СуммаАвансаРегл + РаботаСКурсамиВалютУТ.ПересчитатьВВалюту(
					СуммаЗадолженностиВал,
					ВалютаРегламентированногоУчета,
					ДополнительныеПараметры.ВалютаВзаиморасчетов,
					ВалютаРегламентированногоУчета,
			...
			МассивСуммСНДСРегл = ОбщегоНазначенияКлиентСервер.РаспределитьСуммуПропорциональноКоэффициентам(СуммаСНДСРегл, МассивКоэффициентов);
			
			СтруктураПоиска = Новый Структура("Распоряжение, Номенклатура, КодСтроки");
			
			Если МассивСуммСНДСРегл <> Неопределено Тогда 
			
				ИндексМассиваСумм = 0;
				Для Каждого СтрокаТовары Из ТаблицаТовары Цикл 
					ЗначениеСтавкиНДС = УчетНДСУПКлиентСервер.ЗначениеСтавкиНДС(СтрокаТовары.СтавкаНДС);
					СтрокаТовары.СуммаНДС = МассивСуммСНДСРегл[ИндексМассиваСумм] * ЗначениеСтавкиНДС / (ЗначениеСтавкиНДС + 100);
					СтрокаТовары.Сумма = МассивСуммСНДСРегл[ИндексМассиваСумм];
					ИндексМассиваСумм = ИндексМассиваСумм + 1;
				КонецЦикла;
	...
	Иначе 
		...
		Для Каждого СтрокаТовары Из ТаблицаТовары Цикл 
			...
			Если ДополнительныеПараметры.ЭтоКорректировочный Тогда
				РассчитатьСуммуНДСРеглПриОтгрузкеБезАвансов(ТаблицаТовары, Новый Структура("ДатаОтгрузки, ВалютаСчетаФактуры, ВалютаРегламентированногоУчета, ВалютаВзаиморасчетов, ОплатаВВалюте", ДатаОтгрузки, ДополнительныеПараметры.ВалютаСчетаФактуры, ВалютаРегламентированногоУчета, ДополнительныеПараметры.ВалютаВзаиморасчетов, Истина));
			Иначе
				СуммаНДСВал = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(СтрокаТовары.СуммаБезНДСВал, СтрокаТовары.СтавкаНДС);
				СуммаБезНДСВал = Окр(СтрокаТовары.СуммаБезНДСВал - СуммаНДСВал, 2, РежимОкругления.Окр15как20);
				СуммаБезНДСРегл = РаботаСКурсамиВалютУТ.ПересчитатьВВалюту(
									СуммаБезНДСВал,
									ВалютаРегламентированногоУчета,
									ДополнительныеПараметры.ВалютаВзаиморасчетов,
									ВалютаРегламентированногоУчета,
									ДатаОтгрузки);
				СтрокаТовары.СуммаНДС = ?(ДополнительныеПараметры.ВалютаСчетаФактуры = ВалютаРегламентированногоУчета, 
									УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(СуммаБезНДСРегл, СтрокаТовары.СтавкаНДС, Ложь),
									УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(СуммаБезНДСВал, СтрокаТовары.СтавкаНДС, Ложь));
				СтрокаТовары.Сумма = ?(ДополнительныеПараметры.ВалютаСчетаФактуры = ВалютаРегламентированногоУчета, СуммаБезНДСРегл + СтрокаТовары.СуммаНДС, СтрокаТовары.СуммаБезНДСВал);
			КонецЕсли;
...
Процедура РассчитатьСуммуНДСРеглПриОтгрузкеБезАвансов(ТаблицаТовары, ДополнительныеПараметры)
	
	СтруктураПоиска = Новый Структура("Распоряжение, Номенклатура, КодСтроки");
	
	Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
		
		Если ДополнительныеПараметры.ОплатаВВалюте Тогда
	
			СуммаНДСВалУвеличение = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(СтрокаТовары.СуммаБезНДСВалУвеличение, СтрокаТовары.СтавкаНДС);
			СуммаБезНДСВалУвеличение = Окр(СтрокаТовары.СуммаБезНДСВалУвеличение - СуммаНДСВалУвеличение, 2, РежимОкругления.Окр15как20);
			СуммаБезНДСРеглУвеличение = РаботаСКурсамиВалютУТ.ПересчитатьВВалюту(
								СуммаБезНДСВалУвеличение,
								ДополнительныеПараметры.ВалютаРегламентированногоУчета,
								ДополнительныеПараметры.ВалютаВзаиморасчетов,
								ДополнительныеПараметры.ВалютаРегламентированногоУчета,
								ДополнительныеПараметры.ДатаОтгрузки);
			СтрокаТовары.СуммаНДСУвеличение = ?(ДополнительныеПараметры.ВалютаСчетаФактуры = ДополнительныеПараметры.ВалютаРегламентированногоУчета, 
								УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(СуммаБезНДСРеглУвеличение, СтрокаТовары.СтавкаНДС, Ложь),
								УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(СуммаБезНДСВалУвеличение, СтрокаТовары.СтавкаНДС, Ложь));
			СтрокаТовары.СуммаУвеличение = ?(ДополнительныеПараметры.ВалютаСчетаФактуры = ДополнительныеПараметры.ВалютаРегламентированногоУчета, СуммаБезНДСРеглУвеличение + СтрокаТовары.СуммаНДСУвеличение, СтрокаТовары.СуммаБезНДСВалУвеличение);
				
			СуммаНДСВалУменьшение = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(СтрокаТовары.СуммаБезНДСВалУменьшение, СтрокаТовары.СтавкаНДС);
			СуммаБезНДСВалУменьшение = Окр(СтрокаТовары.СуммаБезНДСВалУменьшение - СуммаНДСВалУменьшение, 2, РежимОкругления.Окр15как20);
			СуммаБезНДСРеглУменьшение = РаботаСКурсамиВалютУТ.ПересчитатьВВалюту(
								СуммаБезНДСВалУменьшение,
								ДополнительныеПараметры.ВалютаРегламентированногоУчета,
								ДополнительныеПараметры.ВалютаВзаиморасчетов,
								ДополнительныеПараметры.ВалютаРегламентированногоУчета,
								ДополнительныеПараметры.ДатаОтгрузки);
			СтрокаТовары.СуммаНДСУменьшение = ?(ДополнительныеПараметры.ВалютаСчетаФактуры = ДополнительныеПараметры.ВалютаРегламентированногоУчета, 
								УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(СуммаБезНДСРеглУменьшение, СтрокаТовары.СтавкаНДС, Ложь),
								УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(СуммаБезНДСВалУменьшение, СтрокаТовары.СтавкаНДС, Ложь));
			СтрокаТовары.СуммаУменьшение = ?(ДополнительныеПараметры.ВалютаСчетаФактуры = ДополнительныеПараметры.ВалютаРегламентированногоУчета, СуммаБезНДСРеглУменьшение + СтрокаТовары.СуммаНДСУменьшение, СтрокаТовары.СуммаБезНДСВалУменьшение);
		...
		Иначе
			...
			СуммаСНДСРеглУвеличение = РаботаСКурсамиВалютУТ.ПересчитатьВВалюту(
							СтрокаТовары.СуммаБезНДСВалУвеличение,
							ДополнительныеПараметры.ВалютаРегламентированногоУчета,
							ДополнительныеПараметры.ВалютаВзаиморасчетов,
							ДополнительныеПараметры.ВалютаРегламентированногоУчета,
							ДополнительныеПараметры.ДатаОтгрузки);
			СтрокаТовары.СуммаНДСУвеличение = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(СуммаСНДСРеглУвеличение, СтрокаТовары.СтавкаНДС);
			СтрокаТовары.СуммаУвеличение = ?(ДополнительныеПараметры.ВалютаСчетаФактуры = ДополнительныеПараметры.ВалютаРегламентированногоУчета, СуммаСНДСРеглУвеличение, СтрокаТовары.СуммаБезНДСВалУвеличение);
			
			СуммаСНДСРеглУменьшение = РаботаСКурсамиВалютУТ.ПересчитатьВВалюту(
							СтрокаТовары.СуммаБезНДСВалУменьшение,
							ДополнительныеПараметры.ВалютаРегламентированногоУчета,
							ДополнительныеПараметры.ВалютаВзаиморасчетов,
							ДополнительныеПараметры.ВалютаРегламентированногоУчета,
							ДополнительныеПараметры.ДатаОтгрузки);
			СтрокаТовары.СуммаНДСУменьшение = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(СуммаСНДСРеглУменьшение, СтрокаТовары.СтавкаНДС);
			СтрокаТовары.СуммаУменьшение = ?(ДополнительныеПараметры.ВалютаСчетаФактуры = ДополнительныеПараметры.ВалютаРегламентированногоУчета, СуммаСНДСРеглУменьшение, СтрокаТовары.СуммаБезНДСВалУменьшение);
...
Процедура ТекстЗапросаВтНДСПоОтгрузкеТоваров(Запрос, ТекстыЗапроса) Экспорт
	
	ИмяРегистра = "ВтНДСПоОтгрузкеТоваров";
	...
	|	ТаблицаТовары.Распоряжение КАК СчетФактура,
	|
	|	ТаблицаТовары.СтавкаНДС КАК СтавкаНДС,
	|	ЕСТЬNULL(Суммы.СуммаНДСРегл, 0) КАК СуммаНДСРегл,
	|	ЕСТЬNULL(Суммы.СуммаНДСУпр, 0) КАК СуммаНДСУпр,
	|	ЕСТЬNULL(Суммы.СуммаБезНДС, 0) + ЕСТЬNULL(Суммы.СуммаНДС, 0) КАК СуммаСНДС,
	...
	|ПОМЕСТИТЬ ВтНДСПоОтгрузкеТоваров
	|ИЗ
	|	Документ.СчетФактураВыданный.Товары КАК ТаблицаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтСуммыДокументовВВалютахУчета КАК Суммы
	|		ПО ТаблицаТовары.Ссылка = Суммы.Ссылка
	...
	|	ТаблицаТовары.Ссылка = &Ссылка
	|	И &НачислениеНДСПриОтгрузке
	|	И ТаблицаТовары.ТипЗапасов В (&ТипыЗапасовСобственные)
	|	И ЕСТЬNULL(Суммы.СуммаНДСРегл, 0) <> 0
	|	И НЕ &ВводОстатков
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &СчетФактураОснование
	|	И &НачислениеНДСПриОтгрузке
	|	И ТаблицаТовары.ТипЗапасов В (&ТипыЗапасовСобственные)
	|	И &Исправление
	|	И ЕСТЬNULL(Суммы.СуммаНДСРегл, 0) <> 0
...
Функция ТекстЗапросаТаблицаДвиженияПоПрочимАктивамПассивам(Запрос, ТекстыЗапроса, Регистры)
	...
	Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтНДСПоОтгрузкеТоваров", ТекстыЗапроса) Тогда
		ТекстЗапросаВтНДСПоОтгрузкеТоваров(Запрос, ТекстыЗапроса)
	КонецЕсли;
	
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	НДСПоОтгрузкеТоваров.Период КАК Период,
	|	НДСПоОтгрузкеТоваров.Организация КАК Организация,
	|	НДСПоОтгрузкеТоваров.Подразделение КАК Подразделение,
	|	НДСПоОтгрузкеТоваров.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НДСПоОтгрузкамБезПереходаПраваСобственности) КАК Статья,
	|	НДСПоОтгрузкеТоваров.Аналитика КАК Аналитика,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийПрочихАктивовПассивов.Дебет) КАК ДебетКредит,
	|
	|	НДСПоОтгрузкеТоваров.СуммаНДСРегл КАК СуммаРегл,
	|	НДСПоОтгрузкеТоваров.СуммаНДСУпр КАК СуммаУпр,
	|	НДСПоОтгрузкеТоваров.СуммаСНДСУпр КАК СуммаСНДС,
	|	НДСПоОтгрузкеТоваров.СуммаБезНДСУпр КАК СуммаБезНДС,
	|
	|	НЕОПРЕДЕЛЕНО КАК ОбъектРасчетов,
	|	НДСПоОтгрузкеТоваров.ИдентификаторФинЗаписи КАК ИдентификаторФинЗаписи,
	|	НДСПоОтгрузкеТоваров.НастройкаХозяйственнойОперации КАК НастройкаХозяйственнойОперации
	|ИЗ
	|	ВтНДСПоОтгрузкеТоваров КАК НДСПоОтгрузкеТоваров";
...
Функция ТекстЗапросаТаблицаДвиженияПоНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|	НДСПоОтгрузкеТоваров.Аналитика КАК Контрагент,
	|	НДСПоОтгрузкеТоваров.СчетФактура КАК СчетФактура,
	|	НДСПоОтгрузкеТоваров.СтавкаНДС КАК СтавкаНДС,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.Товары) КАК ВидЦенности,
	|	НДСПоОтгрузкеТоваров.СуммаБезНДСРегл КАК СуммаБезНДС,
	|	НДСПоОтгрузкеТоваров.СуммаНДСРегл КАК НДС,
	|	НДСПоОтгрузкеТоваров.СуммаНДСУпр КАК НДСУпр,
...
Функция ТекстЗапросаТаблицаНДССостояниеРеализации0(Запрос, ТекстыЗапроса, Регистры)
	...
	|	&ДатаОтгрузки КАК ДатаРеализации,
	|	&Организация КАК Организация,
	|	&Контрагент КАК Контрагент,
	...
	|	СчетФактураТовары.СпрСтавкаНДС КАК СтавкаНДС,
	|	НДССостояниеРеализации0.ДатаПодтверждения КАК ДатаПодтверждения,
	...
	|		ПО НДССостояниеРеализации0.ДокументРеализации = &ДокументОснование
	...
	|	И НЕ &Исправление
	|	И НЕ &Корректировочный
	|	И &НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт)
	|	И СчетФактураТовары.СпрСтавкаНДС = &СтавкаНДСНаЭкспорт
	|	И &НачислениеНДСПриОтгрузке
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	&Период КАК ДатаПодтверждения,
	|	ВЫРАЗИТЬ(НДССостояниеРеализации0.Комментарий КАК СТРОКА(500)) КАК Комментарий
	|ИЗ
	|	Документ.СчетФактураВыданный КАК СчетФактураВозврата
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.Товары КАК СчетФактураТоварыВозврата
	|		ПО СчетФактураВозврата.Ссылка = СчетФактураТоварыВозврата.Ссылка
	|			И СчетФактураТоварыВозврата.НомерСтроки = 1
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.НДССостояниеРеализации0 КАК НДССостояниеРеализации0
	|		ПО НДССостояниеРеализации0.ДокументРеализации = &ДокументОснование
	|			И НЕ ТИПЗНАЧЕНИЯ(НДССостояниеРеализации0.Регистратор) = ТИП(Документ.ПодтверждениеНулевойСтавкиНДС)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НДССостояниеРеализации0 КАК НДССостояниеРеализации0ПодтверждениеСтавки
	|		ПО НДССостояниеРеализации0ПодтверждениеСтавки.ДокументРеализации = &ДокументОснование
	|			И ТИПЗНАЧЕНИЯ(НДССостояниеРеализации0ПодтверждениеСтавки.Регистратор) = ТИП(Документ.ПодтверждениеНулевойСтавкиНДС)
	|ГДЕ
	|	&Организация <> ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация)
	|	И &Исправление
	|	И СчетФактураВозврата.Ссылка В (&Ссылка)
	|	И СчетФактураТоварыВозврата.Ссылка ЕСТЬ NULL
...
Процедура ТекстЗапросаВтОбъектыРасчетов(Запрос, ТекстыЗапроса)
...
Функция ТаблицаТоваров(ПараметрыЗаполнения) Экспорт
	...
	ЭтоКорректировкаНаОснованииСФ = Ложь;
	ЭтоНачислениеНДСПриОтгрузке = ПараметрыЗаполнения.НачислениеНДСПриОтгрузке;
	...
	Если НЕ ЭтоКорректировочный И НЕ ЭтоИсправление И НЕ ЭтоПеревыставление
		И НЕ НДССМежценовойРазницы И Не ЭтоНачислениеНДСПриОтгрузке Тогда
		Возврат Результат;
	КонецЕсли;
	...
	ДополнительныеПараметры.Вставить("НачислениеНДСПриОтгрузке", ЭтоНачислениеНДСПриОтгрузке);
	...
	ЭтоВводОстатковПоОтгрузке = ЭтоНачислениеНДСПриОтгрузке И ТипЗнч(ДополнительныеПараметры.ДокументОснование) = Тип("ДокументСсылка.ПервичныйДокумент");
	
	Если ЭтоНачислениеНДСПриОтгрузке
		И Не ЭтоВводОстатковПоОтгрузке
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДополнительныеПараметры.ДокументОснование, "Исправление") Тогда
		МассивСторнируемыхДокументовОснований = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДополнительныеПараметры.ДокументОснование, "СторнируемыйДокумент"));
	Иначе
		МассивСторнируемыхДокументовОснований = Новый Массив;
	КонецЕсли;
	ДополнительныеПараметры.Вставить("ЭтоИсправлениеОтгрузки", МассивСторнируемыхДокументовОснований.Количество()>0);
	ПоместитьТаблицуТоваровТорговыхДокументов(МассивСторнируемыхДокументовОснований,
		МенеджерВременныхТаблиц, "вт_ТаблицаТоваровСторнируемыхДокументовОснований", ДополнительныеПараметры);
	...
	Если ЭтоПеревыставление И Не ЭтоКорректировочный И Не ЭтоИсправление Тогда
		...
		Возврат Результат;
	ИначеЕсли ТипЗнч(ДополнительныеПараметры.ДокументОснование) = Тип("ДокументСсылка.ВозвратТоваровОтКлиента")
		И Документы.ВозвратТоваровОтКлиента.ЭтоВозвратПослеПереходаПраваСобственности(ДополнительныеПараметры.ДокументОснование) Тогда 
		Результат.Товары = ТаблицаТоваровВозвратовПослеПереходаПраваСобственности(МенеджерВременныхТаблиц);
		Возврат Результат;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("НачислениеНДСПриОтгрузке", ЭтоНачислениеНДСПриОтгрузке);
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|	ВложенныйЗапрос.СчетФактураПолученныйОтПродавца КАК СчетФактураПолученныйОтПродавца,
	|	ВложенныйЗапрос.ДокументРеализации КАК ДокументРеализации,
	...
	|	МАКСИМУМ(ВложенныйЗапрос.НомерСтрокиПриПечати) КАК НомерСтрокиПриПечати,
	|	СУММА(ВложенныйЗапрос.Количество) КАК Количество,
	|	СУММА(ВложенныйЗапрос.КоличествоПоРНПТ) КАК КоличествоПоРНПТ,
	|	СУММА(ВложенныйЗапрос.Сумма) КАК Сумма,
	|	СУММА(ВложенныйЗапрос.СуммаНДС) КАК СуммаНДС,
	|	СУММА(ВложенныйЗапрос.СуммаБезНДСВал) КАК СуммаБезНДСВал
	|ПОМЕСТИТЬ вт_ТаблицаТоваровДокументовОснованийСИсправлениями
	|ИЗ
	|	(ВЫБРАТЬ
	|		ТаблицаТоваровДО.ИсходныйСчетФактура КАК ИсходныйСчетФактура,
	|		ТаблицаТоваровДО.ДокументРеализации КАК ДокументРеализации,
	|		ТаблицаТоваровДО.Распоряжение КАК Распоряжение,
	...
	|		ТаблицаТоваровДО.КодСтроки КАК КодСтроки,
	...
	|		ТаблицаТоваровДО.СуммаБезНДСВал КАК СуммаБезНДСВал
	|	ИЗ
	...
	|		-ТаблицаТоваровДО.СуммаБезНДСВал КАК СуммаБезНДСВал
	|	ИЗ
	|		вт_ТаблицаТоваровСторнируемыхДокументовОснований КАК ТаблицаТоваровДО
	|	ГДЕ
	|		&НачислениеНДСПриОтгрузке) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.ИсходныйСчетФактура,
	|	ВложенныйЗапрос.СчетФактураПолученныйОтПродавца,
	...
	|	ИЛИ СУММА(ВложенныйЗапрос.КоличествоПоРНПТ) <> 0
	|	ИЛИ СУММА(ВложенныйЗапрос.Сумма) <> 0
	|	ИЛИ СУММА(ВложенныйЗапрос.СуммаБезНДСВал) <> 0)
	|	ИЛИ &ЭтоИсправление
	|	ИЛИ НЕ &НачислениеНДСПриОтгрузке
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|ИЗ
	|	вт_ТаблицаТоваровДокументовОснованийСИсправлениями КАК вт_ТаблицаТоваров
	...
	|			И вт_ТаблицаТоваров.ИсходныйСчетФактура <> ЗНАЧЕНИЕ(Документ.СчетФактураВыданный.ПустаяСсылка)
	...
	|			И вт_ТаблицаТоваров.КодСтроки = СчетФактураВыданныйТовары.КодСтроки
	...
	|	СчетФактураВыданныйТовары.Ссылка.МоментВремени < &МоментВремени
	|	И СчетФактураВыданныйТовары.Ссылка <> &Ссылка
	|	И (&НДССМежценовойРазницы ИЛИ &НачислениеНДСПриОтгрузке)
	|
	|СГРУППИРОВАТЬ ПО
	|	вт_ТаблицаТоваров.ИсходныйСчетФактура,
	|	вт_ТаблицаТоваров.Номенклатура,
	...
	|	вт_КрайниеПериоды.КодСтроки КАК КодСтроки,
	...
	|			И вт_КрайниеПериоды.КодСтроки = СчетФактураВыданныйТовары.КодСтроки
	...
	|	вт_ТаблицаТоваров.Распоряжение КАК Распоряжение,
	|	вт_ТаблицаТоваров.ИсходныйСчетФактура КАК ИсходныйСчетФактура,
	...
	|			И вт_ТаблицаТоваров.КодСтроки = вт_КрайниеСчетаФактуры.КодСтроки
	...
	|
	|ГДЕ
	|	&НДССМежценовойРазницы ИЛИ &НачислениеНДСПриОтгрузке
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Запрос.УстановитьПараметр("ЭтоИсправление", ЭтоИсправление);
	
	МассивРезультатов = Запрос.ВыполнитьПакет();
	
	ТаблицаСчетаФактурыОснования = МассивРезультатов[5].Выгрузить();
	...
	ТаблицаПредыдущиеСчетаФактуры = МассивРезультатов[6].Выгрузить();
	Если ЭтоПеревыставление Тогда
		ДокументыРеализации = Новый Массив;
	Иначе
		ДокументыРеализации = МассивРезультатов[7].Выгрузить().ВыгрузитьКолонку("ДокументРеализации");
	КонецЕсли;
	...
	Иначе
		...
		Если ЭтоНачислениеНДСПриОтгрузке Тогда
			ПредыдущиеДокументыОснования = Новый Массив;
			СоответствиеДокументовОснованийПредыдущихСчетовФактур = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ПредыдущиеСчетаФактуры, "ДокументОснование");
			Для Каждого КлючИЗначение Из СоответствиеДокументовОснованийПредыдущихСчетовФактур Цикл
				Если ПредыдущиеДокументыОснования.Найти(КлючИЗначение.Значение.ДокументОснование) = Неопределено Тогда 
					ПредыдущиеДокументыОснования.Добавить(КлючИЗначение.Значение.ДокументОснование);
				КонецЕсли;
			КонецЦикла;
			ЭтоКорректировкаНаОснованииСФ = ПараметрыЗаполнения.Корректировочный
											И ПредыдущиеДокументыОснования.Найти(ДополнительныеПараметры.ДокументОснование) <> Неопределено;
		КонецЕсли;
	КонецЕсли;
	...
	ДополнительныеПараметры.Вставить("ЭтоКорректировкаНаОснованииСФ", ЭтоКорректировкаНаОснованииСФ);
	...
	Если ЭтоИсправление Тогда
		...
		Если ЭтоНачислениеНДСПриОтгрузке
			И Результат.Корректировочный Тогда 
			ДополнительныеПараметры.Вставить("ЭтоКорректировочный", Результат.Корректировочный);
		КонецЕсли;
	...
	Если ЭтоНачислениеНДСПриОтгрузке
		И Результат.Товары.Количество() Тогда
		ДополнительныеПараметры.Вставить("ВалютаВзаиморасчетов", ПараметрыЗаполнения.ВалютаВзаиморасчетов);
		ДополнительныеПараметры.Вставить("Ссылка", ПараметрыЗаполнения.МоментВремени.Ссылка);
		ДополнительныеПараметры.Вставить("Период", ПараметрыЗаполнения.МоментВремени.Дата);
		ДополнительныеПараметры.Вставить("ВалютаРегламентированногоУчета", ЗначениеНастроекПовтИсп.ВалютаРегламентированногоУчетаОрганизации(
																			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДополнительныеПараметры.ДокументОснование, "Организация")));
		ДополнительныеПараметры.Вставить("ВалютаУправленческогоУчета", ЗначениеНастроекПовтИсп.ВалютаУправленческогоУчета());
		ДополнительныеПараметры.Вставить("ОплатаВВалюте", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДополнительныеПараметры.ДокументОснование, "Договор.ОплатаВВалюте"));
		ДополнительныеПараметры.Вставить("ПлатежноРасчетныеДокументы", ?(ПараметрыЗаполнения.Свойство("ПлатежноРасчетныеДокументы"), ПараметрыЗаполнения.ПлатежноРасчетныеДокументы, Неопределено));
		РассчитатьСуммуНДСПриОтгрузке(Результат.Товары, ДополнительныеПараметры);
		РассчитатьСуммуНДСРеглПриОтгрузке(Результат.Товары, ДополнительныеПараметры);
	КонецЕсли;
...
Процедура ПоместитьТаблицуТоваровТорговыхДокументов(МассивТорговыхДокументов, МенеджерВременныхТаблиц, ИмяВременнойТаблицы, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Основание Из МассивТорговыхДокументов Цикл
		...
		ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПервичныйДокумент") Тогда
			...
			Если ТипЗнч(ДополнительныеПараметры.ДокументОснование) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
				...
				|	ДанныеКорректировки.КоличествоУпаковок КАК КоличествоУпаковок,
				...
				|	ВЫБОР
				|		КОГДА &ВыводитьБазовыеЕдиницыИзмерения ИЛИ вт_ПервичногоДокумента.КоличествоУпаковок = 0
				|			ТОГДА вт_ПервичногоДокумента.Количество
				|		ИНАЧЕ вт_ПервичногоДокумента.КоличествоУпаковок
				|	КОНЕЦ КАК Количество,
			...
			ИначеЕсли ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Основание, "ТипПервичногоДокумента") = Перечисления.ТипыПервичныхДокументов.ОтгрузкаТоваровКлиенту Тогда 
				...
				Запрос.Текст =
				"ВЫБРАТЬ ПЕРВЫЕ 1
				|	ТаблицаРегистра.Регистратор КАК ДокументВводаОстатков
				|ПОМЕСТИТЬ втДокументыВводаОстатков
				|ИЗ
				|	РегистрНакопления.РаспоряженияНаОтгрузкуИВозврат КАК ТаблицаРегистра
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВводОстатковТоваров КАК Операция
				|		ПО ТаблицаРегистра.Регистратор = Операция.Ссылка
				|ГДЕ
				|	ТаблицаРегистра.Распоряжение = &ПервичныйДокумент
				|	И НЕ Операция.Исправление
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ
				|	ВЫРАЗИТЬ(&ПервичныйДокумент КАК Документ.ПервичныйДокумент) КАК ДокументРеализации,
				|	ВЫРАЗИТЬ(&ПервичныйДокумент КАК Документ.ПервичныйДокумент) КАК Распоряжение,
				|	ВводОстатковТоваровТовары.Номенклатура КАК Номенклатура,
				|	ВводОстатковТоваровТовары.Характеристика КАК Характеристика,
				|	НЕОПРЕДЕЛЕНО КАК НоменклатураНабора,
				|	НЕОПРЕДЕЛЕНО КАК ХарактеристикаНабора,
				|	НЕОПРЕДЕЛЕНО КАК НоменклатураПартнера,
				|	ВводОстатковТоваровТовары.Серия КАК Серия,
				|	ВводОстатковТоваровТовары.СтавкаНДС КАК СтавкаНДС,
				|	ВводОстатковТоваровТовары.Упаковка КАК Упаковка,
				|	ВводОстатковТоваровТовары.НомерГТД КАК НомерГТД,
				|	НЕОПРЕДЕЛЕНО КАК КодТНВЭД,
				|	ВводОстатковТоваровТовары.КодСтроки КАК КодСтроки,
				|	"""" КАК Содержание,
				|	ВводОстатковТоваровТовары.Количество КАК Количество,
				|	ВводОстатковТоваровТовары.КоличествоПоРНПТ КАК КоличествоПоРНПТ,
				|	ВводОстатковТоваровТовары.СуммаСНДС КАК Сумма,
				|	ВводОстатковТоваровТовары.СуммаНДС КАК СуммаНДС
				|ПОМЕСТИТЬ вт_ПервичногоДокумента
				|ИЗ
				|	Документ.ВводОстатковТоваров.Товары КАК ВводОстатковТоваровТовары
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втДокументыВводаОстатков КАК втДокументыВводаОстатков
				|		ПО ВводОстатковТоваровТовары.Ссылка = втДокументыВводаОстатков.ДокументВводаОстатков
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ
				...
				|	КОНЕЦ КАК ТипЗапасов,
				|	СФ.Ссылка КАК ИсходныйСчетФактура,
				|	вт_ПервичногоДокумента.Распоряжение КАК Распоряжение,
				|	вт_ПервичногоДокумента.КодСтроки КАК КодСтроки,
				|	0 КАК НомерСтрокиПриПечати,
				|	НЕОПРЕДЕЛЕНО КАК СчетФактураПолученныйОтПродавца
				|ИЗ
				|	вт_ПервичногоДокумента КАК вт_ПервичногоДокумента
	...
	|	КОНЕЦ КАК Сумма,
	|	ТаблицаТоваров.Сумма КАК СуммаБезНДСВал,
	|	ВЫБОР
	|		КОГДА &ВалютаСчетаФактуры = ТаблицаТоваров.Валюта
	...
	|	ВЫБОР
	|		КОГДА &НачислениеНДСПриОтгрузке
	|			ТОГДА ТаблицаТоваров.Распоряжение
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	Если ВыводитьСерии
		И Не ДополнительныеПараметры.РеализацияЧерезКомиссионера
		И ТипЗнч(ДополнительныеПараметры.ДокументОснование) = Тип("ДокументСсылка.ВозвратТоваровОтКлиента") Тогда
		
		Склад = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДополнительныеПараметры.ДокументОснование, "Склад");
		ПараметрыУказанияСерий = НоменклатураСервер.ПараметрыУказанияСерий(ДополнительныеПараметры.ДокументОснование, Документы.ВозвратТоваровОтКлиента);
		ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.СчетФактураВыданный";
		
		Выборка = НоменклатураСервер.ВыборкаИзЗапросаРасчетаСтатусовУказанияСерий(ПараметрыУказанияСерий,
													ТаблицаТоваров,
													ТаблицаТоваров,
													Склад);
		
		Пока Выборка.Следующий() Цикл
			СтрТабл = ТаблицаТоваров.Найти(Выборка.НомерСтроки, "НомерСтроки");
			Если СтрТабл.Серия <> Справочники.СерииНоменклатуры.ПустаяСсылка() Тогда
				Если ПараметрыУказанияСерий.ИменаПолейСтатусУказанияСерий.Количество() = 0 Тогда
					Если Выборка.СтатусУказанияСерий = 0 Тогда
				...
				Иначе
					...
					Для Каждого ИмяПоляСтатус Из ПараметрыУказанияСерий.ИменаПолейСтатусУказанияСерий Цикл
						Если Выборка[ИмяПоляСтатус] <> 0 Тогда
							УдалятьСерию = Ложь;
							Прервать;
						КонецЕсли;
					КонецЦикла;
					Если УдалятьСерию Тогда
						СтрТабл.Серия = Справочники.СерииНоменклатуры.ПустаяСсылка();
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
	...
	Если Не ВыводитьСерии Тогда
		...
		|	ТаблицаТоваров.СуммаБезНДСВал КАК СуммаБезНДСВал,
		...
		|	СУММА(ТаблицаТоваров.СуммаБезНДСВал) КАК СуммаБезНДСВал,
...
Процедура ПоместитьТаблицуТоваровСчетовФактур(МассивСчетовФактур, МенеджерВременныхТаблиц, ИмяВременнойТаблицы, ДополнительныеПараметры)
	...
	|	ВЫБОР
	|		КОГДА НЕ СчетФактураВыданныйТовары.Ссылка.Корректировочный
	|		И НЕ СчетФактураВыданныйТовары.Ссылка.Исправление
	|		И СчетФактураВыданныйТовары.Ссылка.НачислениеНДСПриОтгрузке
	|			ИЛИ СчетФактураВыданныйТовары.ПредыдущийСчетФактура = ЗНАЧЕНИЕ(Документ.СчетФактураВыданный.ПустаяСсылка)
	|			ТОГДА СчетФактураВыданныйТовары.Ссылка
	|		ИНАЧЕ
	|			СчетФактураВыданныйТовары.ПредыдущийСчетФактура
	|	КОНЕЦ КАК ПредыдущийСчетФактура,
	|
	...
	|	СчетФактураВыданныйТовары.Распоряжение КАК Распоряжение,
	|
	|	ВЫБОР КОГДА СчетФактураВыданныйТовары.Ссылка.Корректировочный ТОГДА
	...
	|	СчетФактураВыданныйТовары.КодСтроки КАК КодСтроки,
	...
	|			ТОГДА СчетФактураВыданныйТовары.Сумма
	|		ИНАЧЕ
	|			ЕСТЬNULL(СуммыДокументов.БазаНДСРегл, СчетФактураВыданныйТовары.Сумма) + ЕСТЬNULL(СуммыДокументов.СуммаНДСРегл, 0)
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР
	|		КОГДА &ВалютаСчетаФактуры = &ВалютаОснования
	|			ТОГДА СчетФактураВыданныйТовары.СуммаНДС
	|		ИНАЧЕ
	|			ЕСТЬNULL(СуммыДокументов.СуммаНДСРегл, СчетФактураВыданныйТовары.СуммаНДС)
	|	КОНЕЦ КАК СуммаНДС,
	|	СчетФактураВыданныйТовары.СуммаБезНДСВал КАК СуммаБезНДСВал,
	|
	...
	|	СчетФактураВыданныйТовары.СуммаБезНДСВалУменьшение КАК СуммаБезНДСВалУменьшение,
	|	СчетФактураВыданныйТовары.СуммаБезНДСВалУвеличение КАК СуммаБезНДСВалУвеличение,
	...
	|			И СчетФактураВыданныйТовары.КодСтроки = ЦепочкиТоваровСФ.КодСтроки
	...
	|		ПО СчетФактураВыданныйТовары.Ссылка = СчетФактураВыданныйДокумент.Ссылка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютахУчета КАК СуммыДокументов
	|		ПО СчетФактураВыданныйТовары.Ссылка = СуммыДокументов.Регистратор
	|			И СчетФактураВыданныйТовары.ИдентификаторСтроки = СуммыДокументов.ИдентификаторСтроки
	|			И (СчетФактураВыданныйДокумент.Исправление ИЛИ СчетФактураВыданныйДокумент.Корректировочный)
	|
	|ГДЕ
	...
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ДокументРеализации,
	|	ТаблицаТоваровСФ.Распоряжение КАК Распоряжение,
	...
	|	ТаблицаТоваровСФ.КодСтроки КАК КодСтроки,
	...
	|	ТаблицаТоваровСФ.СуммаБезНДСВал КАК СуммаБезНДСВал,
	|
	...
	|	ТаблицаТоваровСФ.СуммаБезНДСВалУменьшение КАК СуммаБезНДСВалУменьшение,
	|	ТаблицаТоваровСФ.СуммаБезНДСВалУвеличение КАК СуммаБезНДСВалУвеличение,
	...
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ДокументРеализации,
	|	ТаблицаТоваровТД.Распоряжение КАК Распоряжение,
	...
	|	ТаблицаТоваровТД.КодСтроки КАК КодСтроки,
	...
	|	СУММА(ТаблицаТоваровТД.СуммаБезНДСВал) КАК СуммаБезНДСВал,
	|
	...
	|	0 КАК СуммаБезНДСВалУменьшение,
	|	0 КАК СуммаБезНДСВалУвеличение,
	...
	|			И ТаблицаТоваровТД.КодСтроки = ТаблицаТоваровСФ.КодСтроки
	...
	Запрос.УстановитьПараметр("ВалютаОснования", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДополнительныеПараметры.ДокументОснование, "Валюта"));
	УстановитьПривилегированныйРежим(Истина);
...
Процедура СвернутьТаблицуТоваровТорговыхДокументов(МенеджерВременныхТаблиц, ДополнительныеПараметры)
	...
	Запрос.УстановитьПараметр("ЭтоКорректировкаНаОснованииСФ", ДополнительныеПараметры.ЭтоКорректировкаНаОснованииСФ);
	Запрос.УстановитьПараметр("РеализацияЧерезКомиссионера", ДополнительныеПараметры.РеализацияЧерезКомиссионера);
	Запрос.УстановитьПараметр("НачислениеНДСПриОтгрузке", ДополнительныеПараметры.НачислениеНДСПриОтгрузке);
	Запрос.УстановитьПараметр("ЭтоКорректировочный", ДополнительныеПараметры.ЭтоКорректировочный);
	Запрос.УстановитьПараметр("ЭтоИсправление", ДополнительныеПараметры.ЭтоИсправление);
	Запрос.УстановитьПараметр("ЭтоИсправлениеОтгрузки", ДополнительныеПараметры.ЭтоИсправлениеОтгрузки);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВложенныйЗапрос.ИсходныйСчетФактура КАК ИсходныйСчетФактура,
	|	ВложенныйЗапрос.ДокументРеализации КАК ДокументРеализации,
	...
	|		ТаблицаТоваровДО.СуммаБезНДСВал КАК СуммаБезНДСВал
	|	ИЗ
	|		вт_ТаблицаТоваровДокументовОснованийСИсправлениями КАК ТаблицаТоваровДО
	|
	|	ГДЕ
	...
	|	ТаблицаТоваровДО.НоменклатураПартнера КАК НоменклатураПартнера,
	|	ТаблицаТоваровДО.НомерСтрокиПриПечати КАК НомерСтрокиПриПечати,
	|	-ЕСТЬNULL(ТаблицаТоваровТД.Количество, 0) КАК Количество,
	|	-ЕСТЬNULL(ТаблицаТоваровТД.КоличествоПоРНПТ, 0) КАК КоличествоПоРНПТ,
	|	-ЕСТЬNULL(ТаблицаТоваровТД.Сумма, 0) КАК Сумма,
	|	-ЕСТЬNULL(ТаблицаТоваровТД.СуммаНДС, 0) КАК СуммаНДС,
	|	-ЕСТЬNULL(ТаблицаТоваровТД.СуммаБезНДСВал, 0) КАК СуммаБезНДСВал
	|ИЗ
	|	вт_ТаблицаТоваровДокументовОснованийСИсправлениями КАК ТаблицаТоваровДО
	|		ЛЕВОЕ СОЕДИНЕНИЕ вт_ТаблицаТоваровИсходныхТорговыхДокументов КАК ТаблицаТоваровТД
	|		ПО ТаблицаТоваровДО.ИсходныйСчетФактура = ТаблицаТоваровТД.ИсходныйСчетФактура
	|			И ТаблицаТоваровДО.Номенклатура = ТаблицаТоваровТД.Номенклатура
	|			И ТаблицаТоваровДО.Характеристика = ТаблицаТоваровТД.Характеристика
	|			И ТаблицаТоваровДО.Серия = ТаблицаТоваровТД.Серия
	|			И ТаблицаТоваровДО.НоменклатураПартнера = ТаблицаТоваровТД.НоменклатураПартнера
	|			И ТаблицаТоваровДО.НоменклатураНабора = ТаблицаТоваровТД.НоменклатураНабора
	|			И ТаблицаТоваровДО.ХарактеристикаНабора = ТаблицаТоваровТД.ХарактеристикаНабора
	|			И ТаблицаТоваровДО.Содержание = ТаблицаТоваровТД.Содержание
	|			И ТаблицаТоваровДО.ТипЗапасов = ТаблицаТоваровТД.ТипЗапасов
	|			И ТаблицаТоваровДО.КодТНВЭД = ТаблицаТоваровТД.КодТНВЭД
	|			И ТаблицаТоваровДО.НомерГТД = ТаблицаТоваровТД.НомерГТД
	|			И ТаблицаТоваровДО.КодСтроки = ТаблицаТоваровТД.КодСтроки
	|			И ТаблицаТоваровДО.ЕдиницаИзмерения = ТаблицаТоваровТД.ЕдиницаИзмерения
	|			И ТаблицаТоваровДО.СтавкаНДС = ТаблицаТоваровТД.СтавкаНДС
	|ГДЕ
	|	НЕ &ЭтоИсправлениеНаОснованииСФ
	|	И &НачислениеНДСПриОтгрузке
	|	И НЕ &ЭтоИсправлениеОтгрузки
	|	И &ЭтоКорректировочный) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.ИсходныйСчетФактура,
	|	ВложенныйЗапрос.ДокументРеализации,
	...
	|		ИЛИ СУММА(ВложенныйЗапрос.СуммаБезНДСВал) <> 0
	|		ИЛИ &ЭтоКорректировкаНаОснованииСФ
	|";
...
Функция ТаблицаТоваровИсправление(МенеджерВременныхТаблиц, ДополнительныеПараметры)
	...
	|
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ТаблицаТоваровСФ.ПредыдущийСчетФактура, ТаблицаТоваровДО.ИсходныйСчетФактура) КАК ПредыдущийСчетФактура,
	...
	|	ТаблицаТоваровСФ.СуммаБезНДСВал + ЕСТЬNULL(ТаблицаТоваровДО.СуммаБезНДСВал, 0) КАК СуммаБезНДСВал,
	|
	...
	|	(ТаблицаТоваровСФ.СуммаБезНДСВалУвеличение - ТаблицаТоваровСФ.СуммаБезНДСВалУменьшение + ЕСТЬNULL(ТаблицаТоваровДО.СуммаБезНДСВал, 0)) * ВЫБОР
	|		КОГДА НЕ &ЭтоКорректировочный
	|			ТОГДА 0
	|		КОГДА ТаблицаТоваровСФ.СуммаБезНДСВалУвеличение - ТаблицаТоваровСФ.СуммаБезНДСВалУменьшение + ЕСТЬNULL(ТаблицаТоваровДО.СуммаБезНДСВал, 0) > 0
	|			ТОГДА 0
	|		ИНАЧЕ -1
	|	КОНЕЦ КАК СуммаБезНДСВалУменьшение,
	|	(ТаблицаТоваровСФ.СуммаБезНДСВалУвеличение - ТаблицаТоваровСФ.СуммаБезНДСВалУменьшение + ЕСТЬNULL(ТаблицаТоваровДО.СуммаБезНДСВал, 0)) * ВЫБОР
	|		КОГДА НЕ &ЭтоКорректировочный
	|			ТОГДА 0
	|		КОГДА ТаблицаТоваровСФ.СуммаБезНДСВалУвеличение - ТаблицаТоваровСФ.СуммаБезНДСВалУменьшение + ЕСТЬNULL(ТаблицаТоваровДО.СуммаБезНДСВал, 0) > 0
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаБезНДСВалУвеличение
	|ИЗ
	...
	|			И ТаблицаТоваровСФ.КодСтроки = ТаблицаТоваровДО.КодСтроки
	|			И ТаблицаТоваровСФ.Распоряжение = ТаблицаТоваровДО.Распоряжение
	...
	|	ТаблицаТоваровСФ.КоличествоПоРНПТ + ЕСТЬNULL(ТаблицаТоваровДО.КоличествоПоРНПТ, 0) КАК КоличествоПоРНПТ,
	|	ВЫБОР
	|		КОГДА &ВалютаСчетаФактуры = &ВалютаОснования ТОГДА
	|			ТаблицаТоваровСФ.Сумма + ЕСТЬNULL(ТаблицаТоваровДО.Сумма, 0)
	|		ИНАЧЕ
	|			0
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР
	|		КОГДА &ВалютаСчетаФактуры = &ВалютаОснования ТОГДА
	|			ТаблицаТоваровСФ.СуммаНДС + ЕСТЬNULL(ТаблицаТоваровДО.СуммаНДС, 0)
	|		ИНАЧЕ
	|			0
	|	КОНЕЦ КАК СуммаНДС,
	|	ТаблицаТоваровСФ.СуммаБезНДСВал + ЕСТЬNULL(ТаблицаТоваровДО.СуммаБезНДСВал, 0) КАК СуммаБезНДСВал,
	...
	|		ИНАЧЕ
	|			0
	|	КОНЕЦ + ТаблицаТоваровСФ.СуммаУменьшение + ТаблицаТоваровСФ.СуммаУвеличение = 0
	|	ИЛИ НЕ ВЫБОР
	|		КОГДА &ВалютаСчетаФактуры = &ВалютаОснования 
	|			И ТИПЗНАЧЕНИЯ(ТаблицаТоваровСФ.Распоряжение) = ТИП(Документ.ЗаказКлиента) ТОГДА
	|			ТаблицаТоваровСФ.СуммаНДС + ЕСТЬNULL(ТаблицаТоваровДО.СуммаНДС, 0)
	|		ИНАЧЕ
	|			0
	|	КОНЕЦ + ТаблицаТоваровСФ.СуммаНДСУменьшение + ТаблицаТоваровСФ.СуммаНДСУвеличение = 0)
	|	И (&НачислениеНДСПриОтгрузке ИЛИ &НДССМежценовойРазницы)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|		КОГДА НЕ &ЭтоКорректировочный
	|			ТОГДА 0
	|		ИНАЧЕ ТаблицаТоваровДО.СуммаБезНДСВал
	|	КОНЕЦ КАК СуммаБезНДСВалУвеличение
	|ИЗ
	...
	|			И ТаблицаТоваровДО.КодСтроки = ТаблицаТоваровСФ.КодСтроки
	|			И ТаблицаТоваровДО.Распоряжение = ТаблицаТоваровСФ.Распоряжение
...
Функция ТаблицаТоваровКорректировка(МенеджерВременныхТаблиц, ДополнительныеПараметры)
	...
	|			ТОГДА ЕСТЬNULL(ТаблицаТоваровСФ.НомерСтроки, 99999)
	|		ИНАЧЕ
	|			0
	|	КОНЕЦ КАК НомерСтрокиИсходногоСФ,
	|	ТаблицаТоваровДО.Распоряжение КАК Распоряжение,
	|	ТаблицаТоваровДО.Номенклатура КАК Номенклатура,
	|	ТаблицаТоваровДО.Характеристика КАК Характеристика,
	|	ТаблицаТоваровДО.Серия КАК Серия,
	...
	|	ТаблицаТоваровДО.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	ВЫБОР
	|		КОГДА ТаблицаТоваровДО.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	|			ТОГДА ТаблицаТоваровСФ.СтавкаНДС
	|		ИНАЧЕ
	...
	|	ТаблицаТоваровДО.Сумма + ЕСТЬNULL(ТаблицаТоваровСФ.Сумма, 0) КАК Сумма,
	|	ТаблицаТоваровДО.СуммаНДС + ЕСТЬNULL(ТаблицаТоваровСФ.СуммаНДС, 0) КАК СуммаНДС,
	|	ТаблицаТоваровДО.СуммаБезНДСВал + ЕСТЬNULL(ТаблицаТоваровСФ.СуммаБезНДСВал, 0) КАК СуммаБезНДСВал,
	|
	|	ТаблицаТоваровДО.Количество * ВЫБОР
	...
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаНДСУвеличение,
	|	ТаблицаТоваровДО.СуммаБезНДСВал * ВЫБОР
	|		КОГДА ТаблицаТоваровДО.СуммаБезНДСВал > 0 ИЛИ НЕ &ЭтоКорректировочный
	|			ТОГДА 0
	|		ИНАЧЕ -1
	|	КОНЕЦ КАК СуммаБезНДСВалУменьшение,
	|	ТаблицаТоваровДО.СуммаБезНДСВал * ВЫБОР
	|		КОГДА ТаблицаТоваровДО.СуммаБезНДСВал > 0 И &ЭтоКорректировочный
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаБезНДСВалУвеличение
	|ИЗ
	...
	|		ПО ТаблицаТоваровСФ.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|		
	|ГДЕ
	|	ТаблицаТоваровДО.ИсходныйСчетФактура ЕСТЬ NULL
	|	И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидПродукции)
	|	И НЕ &НДССМежценовойРазницы
	|	И НЕ &НачислениеНДСПриОтгрузке
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Запрос.УстановитьПараметр("ВидПродукции", ИнтеграцияИСМП.ИспользуемыеВидыМаркируемойПродукцииИСМП());
...
Функция ТаблицаТоваровВозвратовПослеПереходаПраваСобственности(МенеджерВременныхТаблиц)
	...
	|	0 КАК СуммаНДС,
	|
	|	-ТаблицаТоваров.Количество КАК КоличествоУменьшение,
	|	0 КАК КоличествоУвеличение,
	|	-ТаблицаТоваров.КоличествоПоРНПТ КАК КоличествоПоРНПТУменьшение,
	|	0 КАК КоличествоПоРНПТУвеличение,
	|	-ТаблицаТоваров.Сумма КАК СуммаУменьшение,
	|	0 КАК СуммаУвеличение,
	|	-ТаблицаТоваров.СуммаНДС КАК СуммаНДСУменьшение,
	|	0 КАК СуммаНДСУвеличение
	|ИЗ
	|	вт_ТаблицаТоваровДокументовОснований КАК ТаблицаТоваров
	|
...
Процедура РассчитатьСуммуНДСПриОтгрузке(ТаблицаТовары, ДополнительныеПараметры)
		
	СтруктураДействий = Новый Структура;
	КэшированныеЗначения = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
		
	СтруктураЗаполненияСтавкиНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(ДополнительныеПараметры.ДокументОснование);
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ТаблицаТовары, СтруктураДействий, КэшированныеЗначения);
	
	МассивСуффиксов = Новый Массив;
	СтруктураПересчетаСуммыНДС = Новый Структура;
	
	СтруктураДействий = Новый Структура;
	СтруктураПересчетаСуммыНДС.Вставить("ЦенаВключаетНДС", Истина);
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДополнительныеПараметры.ДокументОснование,"НалогообложениеНДС") Тогда
		НалогообложениеНДС =
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДополнительныеПараметры.ДокументОснование, "НалогообложениеНДС");
		СтруктураПересчетаСуммыНДС.Вставить("НалогообложениеНДС", НалогообложениеНДС);
			
	КонецЕсли;
	
	МассивСуффиксов.Добавить("");
	Если ДополнительныеПараметры.ЭтоКорректировочный Тогда
		МассивСуффиксов.Добавить("Уменьшение");
		МассивСуффиксов.Добавить("Увеличение");
	КонецЕсли;
	СтруктураПересчетаСуммыНДС.Вставить("Суффикс", МассивСуффиксов);
	СтруктураДействий.Вставить("ПересчитатьСуммуНДСОтгрузки", СтруктураПересчетаСуммыНДС);
	
	Если ДополнительныеПараметры.ЭтоКорректировочный
		Или ДополнительныеПараметры.ЭтоИсправление
		Или ТипЗнч(ТаблицаТовары[0].Распоряжение) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда 
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ТаблицаТовары, СтруктураДействий, КэшированныеЗначения);
	Иначе
...
Функция ПолучитьДанныеДляПечатнойФормыСчетФактура(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	Если (ЗаполнятьПлатежноРасчетныеДокументы ИЛИ Не ПараметрыПечати.ПечатьВВалюте)
		...
		Если Не ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
			...
			КонецПопытки;
		Иначе
			Если РегистрыСведений.ЗаданияКРаспределениюВзаиморасчетов.АктуальныйПериодРассчитанныхДанныхПоВзаиморасчетам(МассивАналитикУчетаПоПартнерам,,ОкончаниеПериодаРасчета) <> Неопределено Тогда
				ТекстСообщения = НСтр("ru = 'Печатная форма сформирована по неактуальным данным.'") + Символы.ПС;
				ТекстСообщения = ТекстСообщения + ВзаиморасчетыСервер.ТекстПредупрежденияЗаданияРаспределенияВзаиморасчетов();
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
			КонецЕсли;
	...
	Иначе
		...
		|	NULL КАК СтрокаАвансовыеСчетаФактуры
	...
	Если ЗаполнитьДанныеШтрихкодовДляУКДДо Тогда
		РезультатПоКонтрагентам         = МассивРезультатов[КоличествоРезультатов - 12];
		РезультатПоШапке                = МассивРезультатов[КоличествоРезультатов - 11];
		РезультатПоИсходнымДанным       = МассивРезультатов[КоличествоРезультатов - 10];
		РезультатПоТабличнойЧасти       = МассивРезультатов[КоличествоРезультатов - 9];
		...
		РезультатПоТабличнойЧастиТЗ.Колонки.Добавить("ЕдиницаИзмеренияКод", ОбщегоНазначения.ОписаниеТипаСтрока(4));
		...
		МаркировкаДо = МассивРезультатов[КоличествоРезультатов - 7].Выгрузить();
		
		ПараметрыСканирования = Неопределено;
		Основание = Неопределено;
		Если ПараметрыПечати.Свойство("ЗаполнениеЭлектронногоДокумента") Тогда
			Если РезультатПоТабличнойЧастиТЗ.Количество() Тогда
				Основание = РезультатПоТабличнойЧастиТЗ[0].Ссылка;
			ИначеЕсли МассивШапокОснований.Количество() Тогда
				Основание = МассивШапокОснований[0];
			КонецЕсли;
			...
			Если ТипЗнч(Основание) = Тип("ДокументСсылка.ВозвратТоваровОтКлиента") Тогда
				Для Каждого СтрокаТЧ Из РезультатПоТабличнойЧастиТЗ Цикл
					СтрокаТЧ.КоличествоДо = СтрокаТЧ.КоличествоДо - СтрокаТЧ.Количество;
			...
			ДанныеШапки = ШтрихкодированиеИС.ПолучитьСтруктуруРеквизитовДокумента(Основание);
		...
		Иначе
			Маркировка = МассивРезультатов[КоличествоРезультатов - 8].Выгрузить();
			Если ПараметрыПечати.Свойство("ЗаполнениеЭлектронногоДокумента") Тогда
			...
			Если ТипЗнч(Основание) = Тип("ДокументСсылка.ВозвратТоваровОтКлиента") Тогда
				// Отдельная логика формирования: штрихкоды после = штрихкоды до минус штрихкоды возврат
				ДанныеПосле = Маркировка.Скопировать();
				Маркировка = МаркировкаДо.Скопировать();
				Маркировка.Колонки.Добавить("Удалить", Новый ОписаниеТипов("Булево"));
				Для Каждого СтрокаТЧ Из ДанныеПосле Цикл
					Если ЗначениеЗаполнено(СтрокаТЧ.Штрихкод) Тогда
						Для Каждого СтрокаПосле Из Маркировка.НайтиСтроки(Новый Структура("Штрихкод", СтрокаТЧ.Штрихкод)) Цикл
							СтрокаПосле.Удалить = Истина;
						КонецЦикла;
					Иначе
						Распределить = СтрокаТЧ.Количество;
						Для Каждого СтрокаПосле Из Маркировка.НайтиСтроки(Новый Структура("Удалить, Штрихкод", Ложь, СтрокаТЧ.Штрихкод)) Цикл
							ПоСтроке = Мин(Распределить, СтрокаПосле.Количество);
							Распределить = Распределить - ПоСтроке;
							СтрокаПосле.КоличествоПотребительскихУпаковок = Окр(СтрокаПосле.КоличествоПотребительскихУпаковок * ПоСтроке / СтрокаПосле.Количество);
							СтрокаПосле.Количество = СтрокаПосле.Количество - ПоСтроке;
							СтрокаПосле.ЗначениеШтрихкода = Лев(СтрокаПосле.ЗначениеШтрихкода, 18) +  Формат(СтрокаПосле.КоличествоПотребительскихУпаковок, "ЧДЦ=0; ЧН=0; ЧГ=0;");
							СтрокаПосле.Удалить = СтрокаПосле.КоличествоПотребительскихУпаковок = 0;
						КонецЦикла;
					КонецЕсли;
				КонецЦикла;
				Маркировка = Маркировка.Скопировать(Новый Структура("Удалить", Ложь));
				Маркировка.Колонки.Удалить("Удалить");
			КонецЕсли;
			
		КонецЕсли;
		
		РезультатПоПоставщикам    = МассивРезультатов[КоличествоРезультатов - 6];
		
		Прослеживаемость = МассивРезультатов[КоличествоРезультатов - 3];
	...
	Иначе
		РезультатПоКонтрагентам   = МассивРезультатов[КоличествоРезультатов - 11];
		РезультатПоШапке          = МассивРезультатов[КоличествоРезультатов - 10];
		РезультатПоИсходнымДанным = МассивРезультатов[КоличествоРезультатов - 9];
		РезультатПоТабличнойЧасти = МассивРезультатов[КоличествоРезультатов - 8];
		
		Маркировка = МассивРезультатов[КоличествоРезультатов - 7].Выгрузить();
		...
		Если ПараметрыПечати.Свойство("ЗаполнениеЭлектронногоДокумента") Тогда
			...
			Если Выборка.Следующий() И
				(ТипЗнч(Выборка.ДокументОснование) = Тип("ДокументСсылка.КорректировкаРеализации")
					...
					Или ТипЗнч(Выборка.ДокументОснование) = Тип("ДокументСсылка.ПередачаТоваровХранителю")
					Или ТипЗнч(Выборка.ДокументОснование) = Тип("ДокументСсылка.ОтгрузкаТоваровСХранения")
					Или ТипЗнч(Выборка.ДокументОснование) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту")
					Или ТипЗнч(Выборка.ДокументОснование) = Тип("ДокументСсылка.ПередачаТоваровМеждуОрганизациями")
					Или ТипЗнч(Выборка.ДокументОснование) = Тип("ДокументСсылка.ВозвратТоваровМеждуОрганизациями"))
					Тогда
				...
				ДанныеШапки = ШтрихкодированиеИС.ПолучитьСтруктуруРеквизитовДокумента(ДокументОснование);
				ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(ДанныеШапки);
				Маркировка = ЭлектронноеВзаимодействиеИСМП.ЧастичноеСодержимоеИКодыОСУ(
					Маркировка, Товары, ПараметрыСканирования);
			Иначе
				Маркировка = ЭлектронноеВзаимодействиеИСМП.ЧастичноеСодержимое(Маркировка);
			КонецЕсли;
		Иначе
...
Процедура ПоместитьВременнуюТаблицуДанныхОснований(МенеджерВременныхТаблиц, МассивОснований)
	...
	Если Запрос.МенеджерВременныхТаблиц.Таблицы.Найти("СчетаФактурыОснования") = Неопределено
		...
		|				И НЕ ТаблицаОснований.Ссылка.НачислениеНДСПриОтгрузке 
	...
	|	И ТИПЗНАЧЕНИЯ(ДанныеОснований.Ссылка) <> ТИП(Документ.ОтчетКомиссионера)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
...
Функция ПоместитьВременнуюТаблицуПлатежноРасчетныхДокументов(МенеджерВременныхТаблиц, МассивАналитикУчетаПоПартнерам)
	...
	ПлатежноРасчетныеДокументыСводно.Колонки.Добавить("СтрокаАвансовыеСчетаФактуры",
		Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(1000)));
	...
	ПлатежноРасчетныеДокументы.Колонки.Добавить("РасчетныйДокумент");
	ПлатежноРасчетныеДокументы.Колонки.Добавить("НомерСчетаФактурыНаАванс",
		Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(50)));
	ПлатежноРасчетныеДокументы.Колонки.Добавить("ДатаСчетаФактурыНаАванс",
		Новый ОписаниеТипов("Дата",,, Новый КвалификаторыДаты(ЧастиДаты.Дата)));
	ПлатежноРасчетныеДокументы.Колонки.Добавить("НомерИсправленияСчетаФактурыНаАванс",
		Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(10)));
	ПлатежноРасчетныеДокументы.Колонки.Добавить("ДатаИсправленияСчетаФактурыНаАванс",
		Новый ОписаниеТипов("Дата",,, Новый КвалификаторыДаты(ЧастиДаты.Дата)));
	...
	Пока ВыборкаСчетФактур.Следующий() Цикл
		...
		НоваяСтрока.СтрокаАвансовыеСчетаФактуры = СтрокаАвансовыеСчетаФактуры(ПлатежноРасчетныеДокументы);
	...
	|	Таблица.СтрокаАвансовыеСчетаФактуры
	|ПОМЕСТИТЬ ТаблицаПлатежноРасчетныеДокументы
	|ИЗ
...
Процедура СформироватьВтСчетаФактурыПолученные(МенеджерВременныхТаблиц, ПараметрыПечати)
	...
	|	ТаблицаОснований.ДокументОснование КАК ДокументОснованиеДляПечати,
	|	ТаблицаОснований.Ссылка            КАК Ссылка
	|
...
Процедура СформироватьВТПорядковыеНомераТаблицыТоваровИПредставления5а(МенеджерВременныхТаблиц, ПараметрыПечати)
	...
	|	СчетаФактурыОснования.ДокументОснованиеДляПечати КАК ДокументОснование,
	...
	|СГРУППИРОВАТЬ ПО
	|	СчетаФактурыОснования.Ссылка,
	|	СчетаФактурыОснования.ДокументОснованиеДляПечати,
	...
	ПредставленияСтрок5а = ПолучитьПредставленияСтроки5а(ТаблицаТоваров, МенеджерВременныхТаблиц);
...
Функция ТекстЗапросаДанныхШапкиДляПечатиСчетаФактуры(ПараметрыПечати)
	...
	|	СчетаФактурыОснования.Ссылка КАК Ссылка,
	|	МИНИМУМ(СчетаФактурыОснования.ДокументОснование) КАК ДокументОснование,
	|	МИНИМУМ(СчетаФактурыОснования.ДокументОснование) КАК ДокументОснованиеДляПечати
	...
	|	ДанныеОснований.АдресДоставки,
	|	ВЫБОР КОГДА СчетФактураВыданный.РеализацияЧерезКомиссионера
	|		И НЕ ПокупателиКомиссия.Покупатель ЕСТЬ NULL
	|		И ЕСТЬNULL(ДанныеПокупателя.СтранаРегистрации, ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка) ТОГДА
	|			ВЫБОР КОГДА ЕСТЬNULL(ДанныеПокупателя.ОбособленноеПодразделение, ЛОЖЬ) ТОГДА
	|				ЕСТЬNULL(ДанныеПокупателя.ГоловнойКонтрагент.СтранаРегистрации, ДанныеКонтрагента.СтранаРегистрации)
	|			ИНАЧЕ
	|				ЕСТЬNULL(ДанныеПокупателя.СтранаРегистрации, ДанныеКонтрагента.СтранаРегистрации)
	|			КОНЕЦ
	|	ИНАЧЕ
	|		ДанныеКонтрагента.СтранаРегистрации
	...
	|	КОНЕЦ КАК Грузоотправитель,
	|
	|	ВЫБОР КОГДА ДанныеОснований.Грузополучатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|		И НЕ СчетФактураВыданный.РеализацияЧерезКомиссионера ТОГДА
	|		СчетФактураВыданный.Контрагент
	|	ИНАЧЕ
	|		ДанныеОснований.Грузополучатель
	|	КОНЕЦ КАК Грузополучатель,
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.Покупатели КАК ПокупателиКомиссия
	|	ПО СчетФактураВыданный.Ссылка = ПокупателиКомиссия.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК ДанныеПокупателя
	|	ПО ДанныеПокупателя.Ссылка = ПокупателиКомиссия.Покупатель
	|
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	...
	|	ПО СчетаФактуры.Ссылка = ДанныеОснований.Ссылка
	...
	|	ЕСТЬNULL(ТаблицаПлатежноРасчетныеДокументы.СтрокаАвансовыеСчетаФактуры,
	...
	Если ПечататьСчетаФактурыПолученные Тогда
		...
		|	""""                                                  КАК ПредставлениеАвансовыеСчетаФактуры,
		|	ДанныеДокумента.Валюта                                КАК ВалютаСчетаФактуры,
		|	ДанныеОснований.Контрагент                            КАК Контрагент,
		|	ДанныеОснований.КПППокупателя                         КАК КПППокупателя,
		|	ДанныеОснований.ИННПокупателя                         КАК ИННПокупателя,
	...
	Если ПараметрыПечати.Свойство("ДополнитьДаннымиУПД") И ПараметрыПечати.ДополнитьДаннымиУПД Тогда
		...
		|	ДанныеОснований.Кладовщик           КАК Кладовщик,
		|	ДанныеОснований.ДолжностьКладовщика КАК ДолжностьКладовщика,
		|	ВЫРАЗИТЬ(ДанныеОснований.АдресДоставкиЗначенияПолей КАК СТРОКА(1000)) КАК АдресДоставкиЗначенияПолей,
		|	ВЫРАЗИТЬ(ДанныеОснований.АдресДоставкиЗначение КАК СТРОКА(1000))      КАК АдресДоставкиЗначение,
		|	ДанныеОснований.Склад                       КАК Склад,
		|	ДанныеОснований.ДанныеТранспортнаяНакладная КАК ДанныеТранспортнаяНакладная,
		|	ДанныеОснований.СопроводительныеДокументы   КАК СопроводительныеДокументы,
		|");
...
Функция ТекстЗапросаДанныхМаркировки(ЗаполнитьДанныеШтрихкодовДляУКДДо = Ложь)
	...
	|	СчетаФактурыОснования КАК СчетаФактурыОснования
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.ДокументыОснования КАК ДокументыОснования
	|		ПО ДокументыОснования.Ссылка = СчетаФактурыОснования.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ШтрихкодыУпаковок КАК ТаблицаШтрихкодыУпаковок
	|		ПО ДокументыОснования.ДокументОснование = ТаблицаШтрихкодыУпаковок.Ссылка
	...
	|		Или СчетаФактурыОснования.Исправление";
...
Процедура ЗаполнитьПлатежноРасчетныеДокументыПоОтчетуКомитенту(ДокументОтчета, СчетаФактурыПолученные, ПлатежноРасчетныеДокументы, ДатаСФ) Экспорт
...
Функция ТекстЗапросавтДанныеПлатежноРасчетныхДокументов()
	...
	|	ТаблицаДокументов.ДатаСФ КАК ДатаСФ
	...
	|		И ПлатежныеДокументы.Организация = ТаблицаДокументов.Организация
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстЗапросаВтДанныеПлатежноРасчетныхДокументовПоОтгрузке()
	
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПлатежныеДокументы.ДатаПлатежноРасчетногоДокумента КАК ДатаВходящегоДокумента,
	|	ПлатежныеДокументы.НомерПлатежноРасчетногоДокумента КАК НомерВходящегоДокумента,
	|	ЗНАЧЕНИЕ(Документ.СчетФактураВыданный.ПустаяСсылка) КАК СчетФактура,
	|	&Организация КАК Организация,
	|	ПлатежныеДокументы.РасчетныйДокумент КАК РасчетныйДокумент,
	|	&ДатаСФ КАК ДатаСФ
	|ПОМЕСТИТЬ втДанныеПлатежноРасчетныхДокументов
	|ИЗ
	|	&ДанныеПлатежноРасчетныхДокументов КАК ПлатежныеДокументы
	|	
	|ИНДЕКСИРОВАТЬ ПО
	|	РасчетныйДокумент
	|";
...
Функция ТекстЗапросаВтСчетаФактурыАванс()
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	втДанныеПлатежноРасчетныхДокументов.РасчетныйДокумент КАК РасчетныйДокумент,
	|	втДанныеПлатежноРасчетныхДокументов.СчетФактура КАК СчетФактура,
	...
	|	ПО
	|		втДанныеПлатежноРасчетныхДокументов.РасчетныйДокумент = СчетФактураАванс.ДокументОснование
	|		И втДанныеПлатежноРасчетныхДокументов.ДатаСФ >= СчетФактураАванс.Дата
	...
	|		И втДанныеПлатежноРасчетныхДокументов.ДатаСФ >= ИсправленныйСчетФактураАванс.Дата
	|		И СчетФактураАванс.Исправление
	|		И НЕ ИсправленныйСчетФактураАванс.Корректировочный
	|		И ИсправленныйСчетФактураАванс.Проведен
	|
	|СГРУППИРОВАТЬ ПО
	|	втДанныеПлатежноРасчетныхДокументов.РасчетныйДокумент,
	|	втДанныеПлатежноРасчетныхДокументов.СчетФактура,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СчетаФактурыАванс.РасчетныйДокумент КАК РасчетныйДокумент,
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		НомераИсправлений.РасчетныйДокумент КАК РасчетныйДокумент,
	...
	|		ПО
	|			НомераИсправлений.СчетФактураАванс = СчетФактураВыданныйАванс.Ссылка
	|			И НомераИсправлений.НомерИсправления = СчетФактураВыданныйАванс.НомерИсправления
	...
	|			Документ.СчетФактураВыданныйАванс КАК СчетФактураВыданныйАванс
	|		ПО
	|			НомераИсправлений.СчетФактураАванс = СчетФактураВыданныйАванс.СчетФактураОснование
	|			И НомераИсправлений.НомерИсправления = СчетФактураВыданныйАванс.НомерИсправления
	|			И СчетФактураВыданныйАванс.Проведен
	...
	|	СчетФактураАванс),
	|	(ОрганизацияАванса,
	|	ИсправлениеСчетаФактурыАванс))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ втНомераИсправленийСчетовФактурНаАванс
	|	
	|";
...
Функция ТекстЗапросаПолученияНомеровИДатПлатежноРасчетныхДокументов()
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ДанныеПлатежноРасчетныхДокументов.РасчетныйДокумент КАК РасчетныйДокумент,
	|	ДанныеПлатежноРасчетныхДокументов.СчетФактура КАК СчетФактура,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДанныеПлатежноРасчетныхДокументов.РасчетныйДокумент) В (&ТипыДенежныхДокументов)
	|			ТОГДА ДанныеПлатежноРасчетныхДокументов.ДатаВходящегоДокумента
	|		ИНАЧЕ ДАТАВРЕМЯ(1,1,1,0,0,0)
	|	КОНЕЦ КАК ДатаВходящегоДокумента,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДанныеПлатежноРасчетныхДокументов.РасчетныйДокумент) В (&ТипыДенежныхДокументов)
	|			ТОГДА ДанныеПлатежноРасчетныхДокументов.НомерВходящегоДокумента
	|		ИНАЧЕ """"
	...
	|		ПО
	|			ДанныеПлатежноРасчетныхДокументов.РасчетныйДокумент = СчетаФактурыАванс.РасчетныйДокумент
	|	
	|		ЛЕВОЕ СОЕДИНЕНИЕ
	...
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(ДанныеПлатежноРасчетныхДокументов.РасчетныйДокумент) В (&ТипыДенежныхДокументов)
	|		ИЛИ(НЕ ДанныеСчетаФактурыАванс.СчетФактура ЕСТЬ NULL
	|		ИЛИ НЕ СчетаФактурыАванс.РасчетныйДокумент ЕСТЬ NULL
	...
	|	
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ втДанныеПлатежноРасчетныхДокументов
	|;
	|	
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ втСчетаФактурыАванс";
	Возврат ТекстЗапроса;
...
Процедура ЗаполнитьТаблицуПлатежноРасчетныхДокументовПоВыборке(ПлатежноРасчетныеДокументы, ДанныеВыборки)
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка, "РасчетныйДокумент, НомерСчетаФактурыНаАванс, ДатаСчетаФактурыНаАванс, НомерИсправленияСчетаФактурыНаАванс, ДатаИсправленияСчетаФактурыНаАванс");
		
	КонецЦикла;
	ПлатежноРасчетныеДокументы.Свернуть("ДатаПлатежноРасчетногоДокумента, НомерПлатежноРасчетногоДокумента, РасчетныйДокумент, НомерСчетаФактурыНаАванс, ДатаСчетаФактурыНаАванс, НомерИсправленияСчетаФактурыНаАванс, ДатаИсправленияСчетаФактурыНаАванс", "");
...
Процедура СформироватьСуммыДокументаВВалютахУчета(Запрос, ТекстыЗапроса, Регистры = Неопределено) Экспорт
	...
	Если ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтСуммыДокументовВВалютахУчета", ТекстыЗапроса, Запрос.МенеджерВременныхТаблиц) Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтОбъектыРасчетов", ТекстыЗапроса, Запрос.МенеджерВременныхТаблиц) Тогда
		ТекстЗапросаВтОбъектыРасчетов(Запрос, ТекстыЗапроса);
	КонецЕсли;
	
	Если Запрос.Параметры.НачислениеНДСПриОтгрузке Тогда 
		...
		|	&ДатаОтгрузки КАК ПериодБазыНДС,
		|	&ДатаОтгрузки КАК ДатаКурса,
		|
		|	ТаблицаДокумента.НомерСтроки КАК НомерСтроки,
		|	ТаблицаДокумента.ИдентификаторСтроки КАК ИдентификаторСтроки,
		|	ТаблицаДокумента.СуммаУвеличение - ТаблицаДокумента.СуммаУменьшение + ТаблицаДокумента.Сумма * ВЫБОР
		|			КОГДА &Корректировочный ТОГДА
		|				0
		|			ИНАЧЕ
		|				1
		|		КОНЕЦ - (ТаблицаДокумента.СуммаНДСУвеличение - ТаблицаДокумента.СуммаНДСУменьшение + ТаблицаДокумента.СуммаНДС * ВЫБОР
		|			КОГДА &Корректировочный ТОГДА
		|				0
		|			ИНАЧЕ
		|				1
		|		КОНЕЦ) КАК СуммаБезНДС,
		|	ТаблицаДокумента.СтавкаНДС КАК СтавкаНДС,
		|	ТаблицаДокумента.СуммаНДСУвеличение - ТаблицаДокумента.СуммаНДСУменьшение + ТаблицаДокумента.СуммаНДС * ВЫБОР
		|			КОГДА &Корректировочный ТОГДА
		|				0
		|			ИНАЧЕ
		|				1
		...
		|			1
		|	КОНЕЦ КАК СуммаНДСРегл,
		|	&КоэффициентПересчетаВВалютуУпрРегл * (ТаблицаДокумента.СуммаНДСУвеличение - ТаблицаДокумента.СуммаНДСУменьшение + ТаблицаДокумента.СуммаНДС * ВЫБОР
		|		КОГДА &Корректировочный ТОГДА
		|			0
		|		ИНАЧЕ
		|			1
		...
		|	ЕСТЬNULL(ОбъектыРасчетов.ОбъектРасчетов, НЕОПРЕДЕЛЕНО) КАК ОбъектРасчетов,
		|	ЛОЖЬ КАК ПересчитыватьПоДаннымРасчетов
		|ИЗ
		|	Документ.СчетФактураВыданный.Товары КАК ТаблицаДокумента
		...
		НеПересчитыватьСуммыНДС = Запрос.Параметры.НачислениеНДСПриОтгрузке
									И НЕ (Запрос.Параметры.ВалютаСчетаФактуры <> Запрос.Параметры.ВалютаРегламентированногоУчета
									И Запрос.Параметры.ОплатаВВалюте);
	
		РегистрыСведений.СуммыДокументовВВалютахУчета.СформироватьПоДаннымДокумента(
			Запрос, ТекстыЗапроса, Регистры, ТекстЗапросаДанных, НеПересчитыватьСуммыНДС);
			
	ИначеЕсли Запрос.Параметры.Исправление
		Или Запрос.Параметры.Корректировочный Тогда 
	...
	Иначе
		...
		|	НЕОПРЕДЕЛЕНО       КАК ОбъектРасчетов,
		|	ЛОЖЬ               КАК ПересчитыватьПоДаннымРасчетов,
		|	ИСТИНА             КАК НеПересчитыватьСуммыНДС,
		|	0                  КАК СуммаСНДСУпр,
		|	0                  КАК СуммаБезНДСУпр,
		|	0                  КАК СуммаНДСУпр,
		|	0                  КАК СуммаБезНДСРегл,
		...
		|	0                  КАК СуммаСНДСРегл,
		|	0                  КАК БазаНДСРегл,
		|	0                  КАК СуммаПриведенная,
		|	0                  КАК СуммаПриведеннаяРегл,
		|	0                  КАК СуммаПриведеннаяУпр,
		|	ЛОЖЬ               КАК ИзРегистра
...
Функция ТекстЗапросаТаблицаОснований()
	...
	|			ТОГДА ТаблицаОснований.ДокументОснование
	|		ИНАЧЕ ТаблицаОснований.Ссылка
	|	КОНЕЦ КАК ДокументОснованиеДляПечати,
	|	ТаблицаОснований.Ссылка.Корректировочный КАК Корректировочный,
	|	ТаблицаОснований.Ссылка.Исправление КАК Исправление,
	|	ТаблицаОснований.Ссылка.Дата КАК Дата,
	|	ТаблицаОснований.Ссылка.РеализацияЧерезКомиссионера КАК РеализацияЧерезКомиссионера
...
Процедура ПеревыставитьСчетаФактурыПолученные(ПараметрыСчетовФактурДляПеревыставления, АдресРезультата) Экспорт
	...
	Для Каждого ПараметрыДляЗаполнения Из ПараметрыСчетовФактурДляПеревыставления Цикл
		...
		Если ПараметрыДляЗаполнения.СводныйКомиссионный Тогда
			СтруктураДанных.Вставить("КодВидаОперации", "27");
		КонецЕсли;
		...
		Попытка
			...
			КонецЦикла;
 			...
 			Документы.СчетФактураВыданный.ЗаполнитьПлатежноРасчетныеДокументыПоОтчетуКомитенту(СчетФактураОбъект.ДокументОснование, СчетаФактурыПолученные,
																								СчетФактураОбъект.ПлатежноРасчетныеДокументы, СчетФактураОбъект.Дата);
			...
			СчетФактураОбъект.СтрокаАвансовыеСчетаФактуры =
				Документы.СчетФактураВыданный.СтрокаАвансовыеСчетаФактуры(СчетФактураОбъект.ПлатежноРасчетныеДокументы);
		...
		Исключение
			...
			Если СписокСообщений.Количество() > 0 Тогда
				ПоместитьВоВременноеХранилище(СписокСообщений, АдресРезультата);
			КонецЕсли;
			...
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ОписаниеОшибки%", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
// Возвращает дату отгрузки для подбора авансов и определения курса валют.
//
// Параметры:
//  ПараметрыСчетаФактуры - Структура - Описание:
//   * ДокументОснование - ДокументСсылка.ОтгрузкаТоваровКлиенту, ДокументСсылка.ПервичныйДокумент - документ-основание счета-фактуры
//   * Исправление - Булево - признак исправления счета-фактуры
//   * Корректировочный - Булево - признак корректировки счета-фактуры
//   * СчетФактураОснование - ДокументСсылка.СчетФактураВыданный - исправляемый счет-фактура
//   * Период - Дата - дата счета-фактуры
//  ДатаДляПолученияКурса - Булево - флаг, показывающий, для чего получается дата отгрузки - для получения курса или для получения остатков авансов.
//
// Возвращаемое значение:
//  Дата - дата отгрузки без перехода права собственности
//
Функция ОпределитьДатуОтгрузки(ПараметрыСчетаФактуры, ДатаДляПолученияКурса = Ложь) Экспорт
	
	Если ТипЗнч(ПараметрыСчетаФактуры.ДокументОснование) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда 
		РеквизитыДокументаОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПараметрыСчетаФактуры.ДокументОснование, "Дата, ОтгрузкаПоЗаказам, ЗаказКлиента, Исправление, ИсправляемыйДокумент");
	Иначе 
		РеквизитыДокументаОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПараметрыСчетаФактуры.ДокументОснование, "Дата");
	КонецЕсли;
	
	Если ПараметрыСчетаФактуры.Исправление
		И ПараметрыСчетаФактуры.Корректировочный
		И ЗначениеЗаполнено(ПараметрыСчетаФактуры.СчетФактураОснование) Тогда 
		Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыСчетаФактуры.СчетФактураОснование, "Дата");
	ИначеЕсли ПараметрыСчетаФактуры.Исправление
		И ТипЗнч(ПараметрыСчетаФактуры.ДокументОснование) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту")
		И РеквизитыДокументаОснования.Исправление Тогда
		Возврат НачалоДня(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыДокументаОснования.ИсправляемыйДокумент, "Дата")) - ?(ДатаДляПолученияКурса, 0, 1);
	ИначеЕсли ПараметрыСчетаФактуры.Корректировочный Тогда 
		Возврат ПараметрыСчетаФактуры.Период;
	Иначе
		Возврат НачалоДня(РеквизитыДокументаОснования.Дата) - ?(ДатаДляПолученияКурса, 0, 1);
	КонецЕсли;
	
КонецФункции

Функция ТекстЗапросаПолученияНомеровИДатПлатежноРасчетныхДокументовПриОбновлении()
	...
	|	ПлатежныеДокументы.ДатаПлатежноРасчетногоДокумента КАК ДатаВходящегоДокумента,
	|	ПлатежныеДокументы.НомерПлатежноРасчетногоДокумента КАК НомерВходящегоДокумента,
	|	ПлатежныеДокументы.Ссылка.Организация КАК Организация,
	|	ПлатежныеДокументы.РасчетныйДокумент КАК РасчетныйДокумент,
	|	ОбновляемыеДанные.Ссылка КАК СчетФактура,
	|	ОбновляемыеДанные.Ссылка.Дата КАК ДатаСФ
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.ПлатежноРасчетныеДокументы КАК ПлатежныеДокументы
	|		ПО ПлатежныеДокументы.Ссылка = ОбновляемыеДанные.Ссылка
	|	
	|ИНДЕКСИРОВАТЬ ПО НАБОРАМ (
	|	(РасчетныйДокумент),
	|	(СчетФактура))
	...
	|ИНДЕКСИРОВАТЬ ПО НАБОРАМ (
	|	(РасчетныйДокумент),
	|	(ОрганизацияАванса, СчетФактураАванс),
	|	(ОрганизацияАванса, ИсправлениеСчетаФактурыАванс))
	|;
	|	
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДанныеПлатежноРасчетныхДокументов.РасчетныйДокумент КАК РасчетныйДокумент,
	|	ОбновляемыеДанные.Ссылка КАК СчетФактура,
	|	ДанныеПлатежноРасчетныхДокументов.ДатаВходящегоДокумента КАК ДатаВходящегоДокумента,
	|	ДанныеПлатежноРасчетныхДокументов.НомерВходящегоДокумента КАК НомерВходящегоДокумента,
	|	ЕСТЬNULL(СчетаФактурыАванс.СчетФактураАванс, ЗНАЧЕНИЕ(Документ.СчетФактураВыданныйАванс.ПустаяСсылка)) КАК СчетФактураАванс,
	|	ЕСТЬNULL(СчетаФактурыАванс.ИсправлениеСчетаФактурыАванс, ЗНАЧЕНИЕ(Документ.СчетФактураВыданныйАванс.ПустаяСсылка)) КАК ИсправлениеСчетаФактурыАванс,
	|	ДанныеСчетаФактурыАванс.ДатаСчетаФактуры КАК ДатаСчетаФактурыНаАванс,
	|	ДанныеСчетаФактурыАванс.НомерСчетаФактуры КАК НомерСчетаФактурыНаАванс,
	...
	|		втДанныеПлатежноРасчетныхДокументов КАК ДанныеПлатежноРасчетныхДокументов
	|		ПО
	|			ОбновляемыеДанные.Ссылка = ДанныеПлатежноРасчетныхДокументов.СчетФактура
	|	
	|		ЛЕВОЕ СОЕДИНЕНИЕ
	|			втСчетаФактурыАванс КАК СчетаФактурыАванс
	|		ПО
...
Функция СформироватьГиперссылкуКОформлению(Параметры) Экспорт
	...
	МассивГиперссылок = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Гиперссылка);
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5") Тогда
		
		ТекстГиперссылки = НСтр("ru = 'ИСФ/КСФ по отгрузкам'", ОбщегоНазначения.КодОсновногоЯзыка());
	
		Если ЕстьСчетаФактурыПоОтгрузкамКОформлению(Параметры) Тогда
			Гиперссылка = Новый ФорматированнаяСтрока(ТекстГиперссылки,,,,
				ИмяФормыОтгрузкаТоваров());
		Иначе
			Гиперссылка = Новый ФорматированнаяСтрока(ТекстГиперссылки,,ЦветаСтиля.НезаполненноеПолеТаблицы,,
				ИмяФормыОтгрузкаТоваров());
		КонецЕсли;
		МассивГиперссылок.Добавить(Гиперссылка);
...
Функция ИмяФормыОтгрузкаТоваров() Экспорт
	
	Возврат "Документ.СчетФактураВыданный.Форма.ФормаРабочееМестоДляОформленияИсправленныхСчетовФактурПоОтгрузкам";
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("de97bc49-9d7a-47a9-a0ec-a9afbf8c581a");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение табличной части Платежно-расчетные документы Счет-фактур выданных'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Документ.СчетФактураВыданный КАК СчетФактураВыданный
	|ГДЕ
	|	СчетФактураВыданный.Дата >= &Период
	|	И СчетФактураВыданный.Проведен
	...
	|		ГДЕ
	|			СчетФактураВыданный.ДокументОснование = ОтгрузкаТоваровКлиенту.Ссылка)
	|	И (НЕ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	...
	|		ГДЕ
	|			ПлатежныеДокументы.Ссылка = СчетФактураВыданный.Ссылка)
	|	ИЛИ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	...
	|		ГДЕ
	|			ПлатежныеДокументы.Ссылка = СчетФактураВыданный.Ссылка
	|			И ПлатежныеДокументы.НомерСчетаФактурыНаАванс = """"))";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Обновление документов ""Счет-фактура выданный"":'"));
	СписокОписаний.Добавить(НСтр("ru = '- заполнение ТЧ ""Платежно-расчетные документы"" по данным взаиморасчетов'")); 
	...
	|	ОбновляемыеДанные.Ссылка.Дата КАК ДатаСФ,
	|	ОбновляемыеДанные.Ссылка.Организация КАК Организация,
	|	ТаблицаОснований.ДокументОснование КАК ДокументОснование
	|ПОМЕСТИТЬ ТаблицаЗаполненияПлатежноРасчетныхДокументов
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.ДокументыОснования КАК ТаблицаОснований
	|		ПО ТаблицаОснований.Ссылка = ОбновляемыеДанные.Ссылка
	|
	|ИНДЕКСИРОВАТЬ ПО НАБОРАМ (
	...
	Запрос.Текст = ТекстЗапросаПолученияНомеровИДатПлатежноРасчетныхДокументовПриОбновлении();
	...
	Пока ВыборкаСчетФактур.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировки.УстановитьЗначение("Ссылка", ВыборкаСчетФактур.СчетФактура);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
			Блокировка.Заблокировать();
			
			ПричинаИсключения = 2; // ПлохиеДанные
			Рекомендация = НСтр("ru = 'Заполните ТЧ Платежно-расчетные документы вручную или по кнопке Заполнить над ТЧ.'");
			
			ДокументОбъект = ВыборкаСчетФактур.СчетФактура.ПолучитьОбъект();
			
			Если ДокументОбъект = Неопределено Тогда
				ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки(ВыборкаСчетФактур.СчетФактура);
			Иначе
				ПлатежноРасчетныеДокументы = ДокументОбъект.ПлатежноРасчетныеДокументы;
				Выборка = ВыборкаСчетФактур.Выбрать();
				ЕстьИзменения = Ложь;
				ЗаполнятьТолькоНомераСФ = ПлатежноРасчетныеДокументы.Количество() > 0;
				Пока Выборка.Следующий() Цикл
					
					Если Не ЗначениеЗаполнено(Выборка.НомерВходящегоДокумента) И Не ЗначениеЗаполнено(Выборка.НомерСчетаФактурыНаАванс) Тогда
						Продолжить;
					КонецЕсли;
					
					Если ЗаполнятьТолькоНомераСФ Тогда
						Строка = ПлатежноРасчетныеДокументы.Найти(Выборка.РасчетныйДокумент, "РасчетныйДокумент");
					Иначе
						Строка = ПлатежноРасчетныеДокументы.Добавить();
						Строка.НомерПлатежноРасчетногоДокумента = Выборка.НомерВходящегоДокумента;
						Строка.ДатаПлатежноРасчетногоДокумента = Выборка.ДатаВходящегоДокумента;
					КонецЕсли;
					
					Если Строка <> Неопределено Тогда
						Если Не ЗначениеЗаполнено(Строка.НомерСчетаФактурыНаАванс) Тогда
							ЗаполнитьЗначенияСвойств(Строка, Выборка, "РасчетныйДокумент, НомерСчетаФактурыНаАванс, ДатаСчетаФактурыНаАванс, НомерИсправленияСчетаФактурыНаАванс, ДатаИсправленияСчетаФактурыНаАванс");
				...
				Если ЕстьИзменения Тогда
					...
					ДокументОбъект.СтрокаПлатежноРасчетныеДокументы =
					Документы.СчетФактураВыданный.СтрокаРасчетноПлатежныхДокументов(ПлатежноРасчетныеДокументы);
					ДокументОбъект.СтрокаАвансовыеСчетаФактуры =
					Документы.СчетФактураВыданный.СтрокаАвансовыеСчетаФактуры(ПлатежноРасчетныеДокументы);
				КонецЕсли;
		...
		Исключение
			...
			ОбновлениеИнформационнойБазыУТ.СообщитьОНеудачнойОбработке(ИнформацияОбОшибке(), ВыборкаСчетФактур.СчетФактура);
			
			Если ПричинаИсключения = 2 Тогда
				
				ОписаниеПроблемы = ОбновлениеИнформационнойБазыУТ.ПроблемаСДанными(
					ВыборкаСчетФактур.Ссылка, Рекомендация, ИнформацияОбОшибке());
	...
	Иначе
		...
		ШаблонСообщения = НСтр("ru = 'Обработана порция документов ""Счет-фактура выданный"": %1'");
