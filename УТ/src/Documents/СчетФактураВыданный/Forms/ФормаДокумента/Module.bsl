...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Объект.НачислениеНДСПриОтгрузке
			И ТипЗнч(Параметры.Основание) = Тип("Структура")
			И Параметры.Основание.Свойство("ОформлениеСчетаФактурыПоОтгрузкеИзРабочегоМеста") Тогда 
			ЗаполнитьТоварыПоОтгрузке(Параметры.Основание.ОформлениеСчетаФактурыПоОтгрузкеИзРабочегоМеста, Параметры.Основание.Свойство("ЭтоКорректировочныйСчетФактураПоОтгрузке"));
		КонецЕсли;
...
Процедура ПлатежноРасчетныеДокументыСтрокойНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("Перевыставленный", Объект.Перевыставленный);
	
	НовыйАдресВХранилище = Неопределено;
	
	Если Объект.НачислениеНДСПриОтгрузке Тогда
		
		Обработчик = Новый ОписаниеОповещения("ПлатежноРасчетныеДокументыПриЗаписиЗавершение", ЭтотОбъект, Новый Структура);
	
		Если Модифицированность
			Или Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
			ТекстВопроса = НСтр("ru = 'Данные модифицированы.
				|Зачет авансов доступен только после записи данных.
...
Процедура ВалютаПриИзменении(Элемент)
	...
	ЗаполнитьТаблицуТоваровНаКлиенте();
...
Процедура КорректировочныйПриИзменении(Элемент)
	...
	Если Объект.НачислениеНДСПриОтгрузке Тогда 
		Объект.Исправление = Не Объект.Корректировочный
								Или ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(Объект.СчетФактураОснование, "Исправление") = Истина;
		Если Объект.Корректировочный Тогда
			КорректировочныйПриИзмененииНаСервере();
		КонецЕсли;
		ЗаполнитьТаблицуТоваровНаКлиенте();
	КонецЕсли;
...
Процедура ТоварыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, ЭтоГруппа, Параметр)
	...
	Если Объект.НачислениеНДСПриОтгрузке
		И Объект.Корректировочный Тогда 
		Отказ = Истина;
	КонецЕсли;
...
Процедура ТоварыСтавкаНДСПриИзменении(Элемент)
	...
	Если Объект.НДССМежценовойРазницы Тогда 
		...
		Если Не ЗначениеЗаполнено(ТекущаяСтрока.СтавкаНДС) Тогда
			...
			ТекущаяСтрока.СуммаНДСУвеличение = 0;
			ТекущаяСтрока.СуммаНДСУменьшение = 0;
		Иначе
	...
	Если Объект.НачислениеНДСПриОтгрузке Тогда 
		...
		Если Объект.Корректировочный Тогда 
			ИсходныеДанныеСуммаНДС = ДанныеКорректировкиСФ(ТекущаяСтрока.ИсходныйСчетФактура, 
														Новый Структура("Распоряжение, Номенклатура, КодСтроки", ТекущаяСтрока.Распоряжение, ТекущаяСтрока.Номенклатура, ТекущаяСтрока.КодСтроки), 
														"СуммаНДС");
			...
			ТекущаяСтрока.СуммаНДС = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(
				...
				Истина);
			УменьшениеУвеличениеСуммаНДС = ТекущаяСтрока.СуммаНДС - ИсходныеДанныеСуммаНДС;
			
			ТекущаяСтрока.СуммаНДСУвеличение = ?(УменьшениеУвеличениеСуммаНДС > 0, ТекущаяСтрока.СуммаНДСУвеличение+УменьшениеУвеличениеСуммаНДС, ТекущаяСтрока.СуммаНДСУвеличение);
			ТекущаяСтрока.СуммаНДСУменьшение = ?(УменьшениеУвеличениеСуммаНДС < 0, ТекущаяСтрока.СуммаНДСУменьшение-УменьшениеУвеличениеСуммаНДС, ТекущаяСтрока.СуммаНДСУменьшение);
		Иначе
			ТекущаяСтрока.СуммаНДС = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(
				ТекущаяСтрока.Сумма,
				ТекущаяСтрока.СтавкаНДС,
...
&НаКлиенте
Процедура ТоварыОблагаетсяАкцизомПриИзменении(Элемент)
...
Процедура ТоварыКоличествоПриИзменении(Элемент)
	...
	Если Объект.НачислениеНДСПриОтгрузке
		...
		ИсходныеДанные = ДанныеКорректировкиСФ(ТекущаяСтрока.ИсходныйСчетФактура, 
											Новый Структура("Распоряжение, Номенклатура, КодСтроки", ТекущаяСтрока.Распоряжение, ТекущаяСтрока.Номенклатура, ТекущаяСтрока.КодСтроки), 
											"Количество");
		УменьшениеУвеличение = ТекущаяСтрока.Количество - ИсходныеДанные;
		ТекущаяСтрока.КоличествоУвеличение = ?(УменьшениеУвеличение > 0, УменьшениеУвеличение, 0);
		ТекущаяСтрока.КоличествоУменьшение = ?(УменьшениеУвеличение < 0, -УменьшениеУвеличение, 0);
...
Процедура ТоварыСуммаПриИзменении(Элемент)
	...
	Если Объект.НачислениеНДСПриОтгрузке
		...
		ИсходныеДанные = ДанныеКорректировкиСФ(ТекущаяСтрока.ИсходныйСчетФактура, 
											Новый Структура("Распоряжение, Номенклатура, КодСтроки", ТекущаяСтрока.Распоряжение, ТекущаяСтрока.Номенклатура, ТекущаяСтрока.КодСтроки), 
											"Сумма");
		УменьшениеУвеличение = ТекущаяСтрока.Сумма - ИсходныеДанные;
		ТекущаяСтрока.СуммаУвеличение = ?(УменьшениеУвеличение > 0, УменьшениеУвеличение, 0);
		ТекущаяСтрока.СуммаУменьшение = ?(УменьшениеУвеличение < 0, -УменьшениеУвеличение, 0);
...
Процедура ТоварыСуммаНДСПриИзменении(Элемент)
	...
	Если Объект.НачислениеНДСПриОтгрузке
		...
		ИсходныеДанные = ДанныеКорректировкиСФ(ТекущаяСтрока.ИсходныйСчетФактура, 
											Новый Структура("Распоряжение, Номенклатура, КодСтроки", ТекущаяСтрока.Распоряжение, ТекущаяСтрока.Номенклатура, ТекущаяСтрока.КодСтроки), 
											"СуммаНДС");
		УменьшениеУвеличение = ТекущаяСтрока.СуммаНДС - ИсходныеДанные;
		ТекущаяСтрока.СуммаНДСУвеличение = ?(УменьшениеУвеличение > 0, УменьшениеУвеличение, 0);
		ТекущаяСтрока.СуммаНДСУменьшение = ?(УменьшениеУвеличение < 0, -УменьшениеУвеличение, 0);
...
&НаКлиенте
Процедура ТоварыСуммаБезНДСВалПриИзменении(Элемент)
	...
	Если Объект.Корректировочный Тогда
		ИсходныеДанные = ДанныеКорректировкиСФ(ТекущаяСтрока.ИсходныйСчетФактура, 
											Новый Структура("Распоряжение, Номенклатура, КодСтроки", ТекущаяСтрока.Распоряжение, ТекущаяСтрока.Номенклатура, ТекущаяСтрока.КодСтроки), 
											"СуммаБезНДСВал");
		УменьшениеУвеличение = ТекущаяСтрока.СуммаБезНДСВал - ИсходныеДанные;
		ТекущаяСтрока.СуммаБезНДСВалУвеличение = ?(УменьшениеУвеличение > 0, УменьшениеУвеличение, 0);
		ТекущаяСтрока.СуммаБезНДСВалУменьшение = ?(УменьшениеУвеличение < 0, -УменьшениеУвеличение, 0);
	КонецЕсли;
	...
	СтруктураПересчетаСуммыНДС = ОбщегоНазначенияУТВызовСервера.ЗначенияРеквизитовОбъекта(Объект.ДокументОснование, "НалогообложениеНДС");
...
Процедура УстановитьУсловноеОформление()
	...
	ДобавляемыеЭлементы.Добавить(Элементы.ТоварыСуммаБезНДСВалУвеличение);
	ДобавляемыеЭлементы.Добавить(Элементы.ТоварыСуммаБезНДСВалУменьшение);
	...
	// Сумма без НДС (вал.) видна только для счетов-фактур, оформленных по отгрузкам без перехода права собственности
	
	Элемент = УсловноеОформление.Элементы.Добавить();
	
	ДобавляемыеЭлементы = Новый Массив;
	ДобавляемыеЭлементы.Добавить(Элементы.ТоварыСуммаБезНДСВал);
	ДобавляемыеЭлементы.Добавить(Элементы.ТоварыСуммаБезНДСВалУвеличение);
	ДобавляемыеЭлементы.Добавить(Элементы.ТоварыСуммаБезНДСВалУменьшение);
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.НачислениеНДСПриОтгрузке");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Ложь;
...
Процедура ОбновитьИнформациюПоДокументамОснованиям()
	...
	РазрешеноИзменение = ЕстьПравоНаРедактирование
		...
		И Не Объект.НачислениеНДСПриОтгрузке;
...
Функция ПараметрыПодборДокументовОснований()
	...
	КонецЕсли;
	СтруктураПараметров.Вставить("НачислениеНДСПриОтгрузке",     Объект.НачислениеНДСПриОтгрузке);
...
Процедура ЗаполнитьЗависимыеОтДокументовОснованийРеквизитыФормы()
	...
	Если Объект.НачислениеНДСПриОтгрузке
		И Не (Объект.Корректировочный Или Объект.Исправление) Тогда
...
Процедура ОчиститьДокументыОснования()
	...
	Объект.СтрокаАвансовыеСчетаФактуры =
		Документы.СчетФактураВыданный.СтрокаАвансовыеСчетаФактуры(Объект.ПлатежноРасчетныеДокументы);
	УстановитьТекстПлатежноРасчетныхДокументов();
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьТаблицуТоваровНаКлиенте()
...
Функция ЗаполнитьТаблицуТоваровНаСервере()
	...
	ПараметрыЗаполнения.Вставить("НачислениеНДСПриОтгрузке", Объект.НачислениеНДСПриОтгрузке);
	ПараметрыЗаполнения.Вставить("Договор", Объект.Договор);
	ПараметрыЗаполнения.Вставить("ПлатежноРасчетныеДокументы", Объект.ПлатежноРасчетныеДокументы.Выгрузить());
...
&НаСервере
Процедура РассчитатьНДСРеглПриОтгрузке()
	...
	ПараметрыЗаполнения.Вставить("ВалютаСчетаФактуры", Объект.Валюта);
	ПараметрыЗаполнения.Вставить("ЭтоКорректировочный", Объект.Корректировочный);
	ПараметрыЗаполнения.Вставить("ЭтоИсправление", Объект.Исправление);
	ПараметрыЗаполнения.Вставить("ДокументОснование", Объект.ДокументОснование);
	ПараметрыЗаполнения.Вставить("ВалютаВзаиморасчетов", Объект.ВалютаВзаиморасчетов);
	ПараметрыЗаполнения.Вставить("ОплатаВВалюте", Объект.ОплатаВВалюте);
	ПараметрыЗаполнения.Вставить("ПлатежноРасчетныеДокументы", Объект.ПлатежноРасчетныеДокументы.Выгрузить());
	ПараметрыЗаполнения.Вставить("СчетФактураОснование", Объект.СчетФактураОснование);
	ПараметрыЗаполнения.Вставить("Период", Объект.Дата);

	Документы.СчетФактураВыданный.РассчитатьСуммуНДСРеглПриОтгрузке(Объект.Товары, ПараметрыЗаполнения);
	
КонецПроцедуры

&НаСервере
Процедура КорректировочныйПриИзмененииНаСервере()
...
Процедура УправлениеЭлементамиФормы()
	...
	НачислениеНДСПриОтгрузке = Объект.НачислениеНДСПриОтгрузке;
	
	Элементы.Корректировочный.Видимость = ВводОстатков
											Или НачислениеНДСПриОтгрузке И (ЭтоИсправление Или ЭтоКорректировочный);
	Элементы.Корректировочный.Доступность = ВводОстатков
											Или НачислениеНДСПриОтгрузке
											И (ЭтоИсправление И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.СчетФактураОснование, "Корректировочный") <> Истина
											Или ЭтоКорректировочный);
	...
	Элементы.ГруппаПлатежноРасчетныеДокументы.Видимость = Не ЭтоКорректировочный И Не ВводОстатков Или НачислениеНДСПриОтгрузке И Объект.ДокументОснование <> Неопределено;
	Если НачислениеНДСПриОтгрузке Тогда 
		Элементы.СтрокаПлатежноРасчетныеДокументы.Доступность = НЕ (Объект.НалогообложениеНДС <> Перечисления.ТипыНалогообложенияНДС.ПродажаНаЭкспорт
																	И Объект.Валюта <> ВалютаРегламентированногоУчета);
	КонецЕсли;
	...
	Элементы.ГруппаТовары.Видимость = ЭтоИсправление Или ЭтоКорректировочный Или ЭтоПеревыставление Или НДССМежценовойРазницы Или НачислениеНДСПриОтгрузке;
	...
	Элементы.ТоварыРаспоряжение.Видимость = НачислениеНДСПриОтгрузке И ЭтоИсправление;
	НастроитьВидимостьРучнойКорректировки(ЭтотОбъект);
	...
	Элементы.Договор.Видимость = НачислениеНДСПриОтгрузке;
	
	НастроитьВидимостьВалютыКодаОперации();
	
	Элементы.ТоварыОблагаетсяАкцизом.Видимость     = ИспользоватьПодакцизныеТовары;
	Элементы.ТоварыСуммаАкциза.Видимость           = ИспользоватьПодакцизныеТовары;
	Элементы.ТоварыСуммаАкцизаУвеличение.Видимость = ИспользоватьПодакцизныеТовары;
	Элементы.ТоварыСуммаАкцизаУменьшение.Видимость = ИспользоватьПодакцизныеТовары;
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Если Объект.НачислениеНДСПриОтгрузке Тогда 
		...
		МассивПараметровВыбораРаспоряжения = Новый Массив;
		МассивПараметровВыбораРаспоряжения.Добавить(Новый ПараметрВыбора("Отбор.Ссылка", ОбщегоНазначенияУТ.УдалитьПовторяющиесяЭлементыМассива(МассивРаспоряжений(Объект.ДокументОснование))));
		Элементы.ТоварыРаспоряжение.ПараметрыВыбора = Новый ФиксированныйМассив(МассивПараметровВыбораРаспоряжения);
...
Процедура ОбновитьИнформациюПоПокупателям()
	...
	Иначе
		ТекстПокупателей = ОбщегоНазначенияУТКлиентСервер.СклонениеСлова(
				...
				НСтр("ru = 'клиент, клиента, клиентов, м'"));
...
&НаСервере
Функция ДанныеКорректировкиСФ(ИсходныйСчетФактура, СтруктураПоиска, ИмяПоля)
	
	Если Не ЗначениеЗаполнено(ИсходныйСчетФактура) Тогда 
		Возврат 0;
	КонецЕсли;
	
	ТоварыИсходногоСчетаФактуры = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИсходныйСчетФактура, "Товары").Выгрузить();
	ПоказательДоИзменения = 0;
	НайденныеСтроки = ТоварыИсходногоСчетаФактуры.НайтиСтроки(СтруктураПоиска);
	Если НайденныеСтроки.Количество() > 0 Тогда 
		ПоказательДоИзменения = НайденныеСтроки[0][ИмяПоля];
	КонецЕсли;
	
	Возврат ПоказательДоИзменения;
	
КонецФункции

&НаКлиенте
Процедура ПлатежноРасчетныеДокументыПриЗаписиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Отмена ИЛИ Отказ Тогда
		Возврат;
	КонецЕсли;
	...
	СуммаВалСНДС = СуммаДокументаСНДС();
	ПараметрыФормы.Вставить("СуммаКОплате",          СуммаВалСНДС);
	...
	ПараметрыФормы.Вставить("СчетФактура",           Объект.Ссылка);
	ПараметрыФормы.Вставить("СчетФактураОснование",  Объект.СчетФактураОснование);
	ПараметрыФормы.Вставить("Исправление",           Объект.Исправление);
	ПараметрыФормы.Вставить("НомерИсправления",           Объект.НомерИсправления);
	ПараметрыФормы.Вставить("СуммаРучногоВосстановленияАванса",  Объект.СуммаРучногоВосстановленияАванса);
	ПараметрыФормы.Вставить("Период",                Объект.Дата);
		
	ОткрытьФорму(
		"Документ.СчетФактураВыданный.Форма.ФормаЗачетаАвансов",
		ПараметрыФормы,,,,, Новый ОписаниеОповещения("ПлатежноРасчетныеДокументыСтрокойНажатиеЗавершение", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПлатежноРасчетныеДокументыСтрокойНажатиеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда 
		НовыйАдресВХранилище = Результат.ПлатежноРасчетныеДокументы;
		Объект.СуммаРучногоВосстановленияАванса = Результат.СуммаРучногоВосстановленияАванса;
	Иначе
	...
	Если Объект.НачислениеНДСПриОтгрузке
		И ЗначениеЗаполнено(НовыйАдресВХранилище) Тогда
		РассчитатьНДСРеглПриОтгрузке();
...
&НаСервере
Функция СуммаДокументаСНДС()

	Если Объект.Корректировочный Тогда 
		Возврат Объект.Товары.Итог("СуммаБезНДСВалУвеличение") - Объект.Товары.Итог("СуммаБезНДСВалУменьшение");
	Иначе
		Возврат Объект.Товары.Итог("СуммаБезНДСВал");
	КонецЕсли;
	
КонецФункции

&НаСервереБезКонтекста
Функция МассивРаспоряжений(ДокументОснование)
	
	Если Не ЗначениеЗаполнено(ДокументОснование) Тогда 
		Возврат Новый Массив;
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПервичныйДокумент") Тогда 
		Возврат ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДокументОснование);
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда 
		Запрос = Новый Запрос;
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтгрузкаТоваровКлиенту КАК ДокументОтгрузкаТоваров
		|		ПО ТаблицаТовары.Ссылка = ДокументОтгрузкаТоваров.Ссылка
		|ГДЕ
		|	ТаблицаТовары.Ссылка = &Ссылка
		|	И ДокументОтгрузкаТоваров.ОтгрузкаПоЗаказам
		...
		|	ТаблицаТовары.Ссылка = &Ссылка
		|	И ТаблицаТовары.КодСтроки <> 0
		|	И (НЕ ДокументОтгрузкаТоваров.ОтгрузкаПоЗаказам
		...
		|				)
		|		ИЛИ
		|		ДокументОтгрузкаТоваров.ОтгрузкаПоЗаказам
		|		И НЕ ТаблицаТовары.ЗаказКлиента В (
		|				ЗНАЧЕНИЕ(Документ.ЗаказКлиента.ПустаяСсылка),
		|				ЗНАЧЕНИЕ(Документ.ЗаявкаНаВозвратТоваровОтКлиента.ПустаяСсылка),
		|				НЕОПРЕДЕЛЕНО
		...
		Возврат Выборка.ВыгрузитьКолонку("ЗаказКлиента");
	Иначе
		Возврат Новый Массив;
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьТоварыПоОтгрузке(МассивСтрок, ЭтоКорректировочный)
	...
	Если ЭтоКорректировочный Тогда 
		...
		Объект.СчетФактураОснование = Документы.СчетФактураВыданный.ПустаяСсылка();
	КонецЕсли;
