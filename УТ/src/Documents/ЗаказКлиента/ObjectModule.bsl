...
Процедура ЗаполнитьУсловияПродаж(Знач УсловияПродаж) Экспорт
	...
	БанковскийСчет = ДенежныеСредстваСервер.ПолучитьБанковскийСчетОрганизацииПоУмолчанию(
		ПараметрыСчетаОрганизацииПоУмолчанию(),
		БанковскийСчет);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25() Тогда
		...
		Если Не Партнер.Пустая()
			И ДоставкаТоваровКлиентСервер.ХозяйственнаяОперацияУчаствуетВДоставке(ХозяйственнаяОперация)
	...
	Иначе
		...
		ДоставкаТоваров.ПроверитьЗаполнениеРеквизитовДоставки(
			ЭтотОбъект,
			МассивНепроверяемыхРеквизитов,
			Отказ);
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	ПараметрыСчета = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаКонтрагента(Контрагент);
	ПараметрыСчета.КорреспондирующийСчет = БанковскийСчет;
	БанковскийСчетКонтрагента = ДенежныеСредстваСервер.ПолучитьБанковскийСчетКонтрагентаПоУмолчанию(ПараметрыСчета,
...
Функция СкладыИспользуемыеДляДоставки()
	...
	ТаблицаТоваров = Товары.Выгрузить(ПараметрыПоиска, "Номенклатура, Склад");
...
Функция НастройкиПроверкиДоставкиДокумента()
	...
	Если ТребуетсяДоставка
		И СпособУказанияДоставки = Перечисления.СпособыУказанияДоставки.ПоСтрокам Тогда
		
		ПараметрыОтбора = Новый Структура;
		ПараметрыОтбора.Вставить("Отменено", Ложь);
		ИменаПолейТоваров = "НомерСтроки, Номенклатура, Характеристика, Серия, КодСтроки";
		ТаблицаТоваров = Товары.Выгрузить(ПараметрыОтбора, ИменаПолейТоваров);
		
		ТаблицаДоставки = ТоварыКДоставке.Выгрузить();
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТоварыКДоставке.НомерСтроки КАК НомерСтроки,
		|	ТоварыКДоставке.КодСтроки КАК КодСтроки,
		|	ТоварыКДоставке.СпособДоставки КАК СпособДоставки,
		|	ТоварыКДоставке.ТочкаДоставки КАК ТочкаДоставки,
		|	ТоварыКДоставке.ЗонаДоставки КАК ЗонаДоставки,
		|	ТоварыКДоставке.ДоставкаДоПеревозчика КАК ДоставкаДоПеревозчика,
		|	ТоварыКДоставке.Перевозчик КАК Перевозчик,
		|	ТоварыКДоставке.ТочкаДоставкиПеревозчика КАК ТочкаДоставкиПеревозчика,
		|	ТоварыКДоставке.ЗонаДоставкиПеревозчика КАК ЗонаДоставкиПеревозчика,
		|	ТоварыКДоставке.ВремяДоставкиС КАК ВремяДоставкиС,
		|	ТоварыКДоставке.ВремяДоставкиПо КАК ВремяДоставкиПо,
		|	ТоварыКДоставке.ДополнительнаяИнформацияПоДоставке КАК ДополнительнаяИнформацияПоДоставке,
		|	ТоварыКДоставке.ОсобыеУсловияПеревозки КАК ОсобыеУсловияПеревозки,
		|	ТоварыКДоставке.ОсобыеУсловияПеревозкиОписание КАК ОсобыеУсловияПеревозкиОписание
		|ПОМЕСТИТЬ ВтТаблицаДоставки
		|ИЗ
		|	&ТаблицаДоставки КАК ТоварыКДоставке
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВтТаблицаТоваров.НомерСтроки КАК НомерСтрокиДокумента,
		|	ВтТаблицаТоваров.КодСтроки КАК КодСтроки,
		...
		|	ЕСТЬNULL(ВтТаблицаДоставки.СпособДоставки, ЗНАЧЕНИЕ(Перечисление.СпособыДоставки25.НашимиСилами)) КАК СпособДоставки,
		|	ЕСТЬNULL(ВтТаблицаДоставки.ТочкаДоставки, ЗНАЧЕНИЕ(Справочник.ТочкиДоставки.ПустаяСсылка)) КАК ТочкаДоставки,
		|	ЕСТЬNULL(ВтТаблицаДоставки.ЗонаДоставки, ЗНАЧЕНИЕ(Справочник.ЗоныДоставки.ПустаяСсылка)) КАК ЗонаДоставки,
		|	ЕСТЬNULL(ВтТаблицаДоставки.ДоставкаДоПеревозчика, ЛОЖЬ) КАК ДоставкаДоПеревозчика,
		|	ЕСТЬNULL(ВтТаблицаДоставки.Перевозчик, ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)) КАК Перевозчик,
		|	ЕСТЬNULL(ВтТаблицаДоставки.ТочкаДоставкиПеревозчика, ЗНАЧЕНИЕ(Справочник.ТочкиДоставки.ПустаяСсылка)) КАК ТочкаДоставкиПеревозчика,
		|	ЕСТЬNULL(ВтТаблицаДоставки.ЗонаДоставкиПеревозчика, ЗНАЧЕНИЕ(Справочник.ЗоныДоставки.ПустаяСсылка)) КАК ЗонаДоставкиПеревозчика,
		|	ЕСТЬNULL(ВтТаблицаДоставки.ВремяДоставкиС, ДАТАВРЕМЯ(1, 1, 1)) КАК ВремяДоставкиС,
		|	ЕСТЬNULL(ВтТаблицаДоставки.ВремяДоставкиПо, ДАТАВРЕМЯ(1, 1, 1)) КАК ВремяДоставкиПо,
		|	ЕСТЬNULL(ВтТаблицаДоставки.ДополнительнаяИнформацияПоДоставке, """") КАК ДополнительнаяИнформацияПоДоставке,
		|	ЕСТЬNULL(ВтТаблицаДоставки.ОсобыеУсловияПеревозки, ЛОЖЬ) КАК ОсобыеУсловияПеревозки,
		|	ЕСТЬNULL(ВтТаблицаДоставки.ОсобыеУсловияПеревозкиОписание, """") КАК ОсобыеУсловияПеревозкиОписание
		|ИЗ
		|	ВтТаблицаТоваров КАК ВтТаблицаТоваров
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтТаблицаДоставки КАК ВтТаблицаДоставки
		|		ПО ВтТаблицаТоваров.КодСтроки = ВтТаблицаДоставки.КодСтроки
		|ГДЕ
		|	ВЫРАЗИТЬ(ВтТаблицаТоваров.Номенклатура КАК Справочник.Номенклатура).ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар)
		|
		|УПОРЯДОЧИТЬ ПО
		...
		НастройкиПроверкиДоставки.ТаблицаТоваровДоставки = Таблица;
...
Процедура ДоставкаПередЗаписью(Отказ, Знач РежимЗаписи)
	...
	Если Не ДоставкаПрименима Тогда
