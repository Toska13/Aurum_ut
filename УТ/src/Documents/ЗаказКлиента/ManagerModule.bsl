...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	КомандаСозданияНаОснованииПереходПраваСобственности =
	...
	Если КомандаСозданияНаОснованииПереходПраваСобственности <> Неопределено Тогда
		ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(КомандаСозданияНаОснованииПереходПраваСобственности,
			"ЭтоЗаказКакСчет", 
			Ложь,
			ВидСравненияКомпоновкиДанных.Равно);
	КонецЕсли;
	...
	Если КомандаСоздания <> Неопределено Тогда
		...
		ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(
			...
			Перечисления.СпособыУказанияДоставки.ПоСтрокам,
			ВидСравненияКомпоновкиДанных.НеРавно);
...
Функция ТекстЗапросаДляРасчетаСостоянийЗаказов() Экспорт
	...
	ТекстЗапроса ="
		...
		|// ОЖИДАЕТСЯ ПЕРЕХОД ПРАВА СОБСТВЕННОСТИ ////////////////////////////////////
		|	КОГДА
		...
		|		И ЕСТЬNULL(ЗаказыКлиентовОстатки.КОтгрузкеКонечныйОстаток, 0) = 0
		|		И ЕСТЬNULL(ЗаказыКлиентовОстатки.КОформлениюКонечныйОстаток, 0) > 0
		|		И ЕСТЬNULL(ТоварыКОтгрузкеОстатки.КОтгрузкеОстаток, 0) = 0
		|	ТОГДА
		|		ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовКлиентов.ОжидаетсяПереходПраваСобственности)
		|// В ПРОЦЕССЕ ОТГРУЗКИ //////////////////////////////////////////////////////
		|	КОГДА
		|		(ДокументЗаказКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.КОтгрузке)
		|			ИЛИ ДокументЗаказКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.КОбеспечению))
...
Функция СтруктураСостояниеВыполненияДокумента() Экспорт
	...
	СтруктураСостояние.Вставить("ВыводитьТаблицыДоставка", 5);
	СтруктураСостояние.Вставить("ИмяРегистраОтгрузкаУслуг", "РаспоряженияНаОтгрузкуИВозврат");
	СтруктураСостояние.Вставить("ВыводитьТаблицуППС", 6);
...
Функция ПараметрыВзаиморасчеты(ДанныеЗаполнения = Неопределено) Экспорт
	...
	СтруктураПараметров.НазначениеПлатежа                    = "Объект.НазначениеПлатежа";
	СтруктураПараметров.ТСП                                  = "Объект.Склад";
	СтруктураПараметров.ДополнительнаяИнформация             = "Объект.ДополнительнаяИнформация";
...
Функция ХозяйственныеОперацииДокумента(ПроверятьФункциональныеОпции) Экспорт
	...
	ИспользоватьОтгрузкуБезПереходаПраваСобственности2_0 = Истина;
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	ИначеЕсли ВидФормы = "ФормаСписка" Тогда
		...
		ИначеЕсли Не ПравоДоступа("Чтение", Метаданные.РегистрыСведений.СостоянияЗаказовКлиентов)
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ДанныеШапки.ТребуетсяДоставка КАК ТребуетсяДоставка,
	|	ДанныеШапки.ВидПланированияДоставки КАК ВидПланированияДоставки,
	|	ДанныеШапки.СпособУказанияДоставки КАК СпособУказанияДоставки,
	|	ДанныеШапки.СпособДоставки25 КАК СпособДоставки25,
	...
	Запрос.УстановитьПараметр("ТребуетсяДоставка", Реквизиты.ТребуетсяДоставка);
	Запрос.УстановитьПараметр("СпособУказанияДоставки", Реквизиты.СпособУказанияДоставки);
	Запрос.УстановитьПараметр("СпособДоставки25", Реквизиты.СпособДоставки25);
...
Процедура ТекстЗапросаДанныеДокументаШапка(СписокЗапросов)
	...
	|	ДанныеШапки.СпособУказанияДоставки КАК СпособУказанияДоставки,
	|	ДанныеШапки.СпособДоставки25 КАК СпособДоставки25,
	|	ДанныеШапки.ТочкаДоставки КАК ТочкаДоставки,
	|	ДанныеШапки.ЗонаДоставки КАК ЗонаДоставки,
	|	ДанныеШапки.ДоставкаДоПеревозчика КАК ДоставкаДоПеревозчика,
	|	ДанныеШапки.ПеревозчикПартнер КАК ПеревозчикПартнер,
	|	ДанныеШапки.ТочкаДоставкиПеревозчика КАК ТочкаДоставкиПеревозчика,
	|	ДанныеШапки.ЗонаДоставкиПеревозчика КАК ЗонаДоставкиПеревозчика,
...
Функция ТекстЗапросаДоставка(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект, ДопПараметры)
	...
	Если ДокументОбъект.Статус = Перечисления.СтатусыЗаказовКлиентов.НеСогласован
	...
	Если ДокументОбъект.СпособУказанияДоставки = Перечисления.СпособыУказанияДоставки.ПоСтрокам Тогда
		
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ШапкаДокумента.ВидПланированияДоставки КАК ВидПланированияДоставки,
		|	ТаблицаКДоставке.НомерСтроки КАК НомерСтроки,
		|	НАЧАЛОПЕРИОДА(&Период, ДЕНЬ) КАК Период,
		|	ШапкаДокумента.Организация КАК Организация,
		|	ЗНАЧЕНИЕ(Перечисление.ТипыДоставки.Исходящая) КАК ТипДоставки,
		|	ТаблицаТовары.ДатаОтгрузки КАК ДатаОтгрузки,
		|	ТаблицаКДоставке.СпособДоставки КАК СпособДоставки,
		|	ВЫБОР
		|		КОГДА ТаблицаКДоставке.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки25.СиламиПеревозчика)
		|			ТОГДА ТаблицаКДоставке.Перевозчик
		...
		|	ВЫБОР
		|		КОГДА ТаблицаКДоставке.ДоставкаДоПеревозчика
		|			ТОГДА ТаблицаКДоставке.Перевозчик
		|		ИНАЧЕ ШапкаДокумента.Партнер
		|	КОНЕЦ КАК Получатель,
		|	ВЫБОР
		|		КОГДА ТаблицаКДоставке.ДоставкаДоПеревозчика
		|			ТОГДА ТаблицаКДоставке.ТочкаДоставкиПеревозчика
		|		ИНАЧЕ ТаблицаКДоставке.ТочкаДоставки
		|	КОНЕЦ КАК ТочкаПолучателя,
		|	ВЫБОР
		|		КОГДА ТаблицаКДоставке.ДоставкаДоПеревозчика
		|			ТОГДА ТаблицаКДоставке.ЗонаДоставкиПеревозчика
		|		ИНАЧЕ ТаблицаКДоставке.ЗонаДоставки
		...
		|	ТаблицаТовары.Характеристика КАК Характеристика,
		|	ТаблицаТовары.Серия КАК Серия,
		|	ТаблицаКДоставке.КодСтроки КАК КодСтроки,
		|	ВЫБОР
		...
		|	НЕ ТаблицаКДоставке.ДоставкаДоПеревозчика КАК ЭтоКонечноеПлечо,
		|	1 КАК НомерМинимальногоПлеча,
		|	ИСТИНА КАК ПервичнаяЗапись,
		|	ТаблицаТовары.ДатаОтгрузки КАК ПервичнаяДатаОтгрузки,
		|	ВЫБОР
		|		КОГДА ТаблицаТовары.Обособленно
		|			ТОГДА ШапкаДокумента.Назначение
		|		ИНАЧЕ &НазначениеПоУмолчанию
		|	КОНЕЦ КАК Назначение
		|ИЗ
		|	Документ.ЗаказКлиента.ТоварыКДоставке КАК ТаблицаКДоставке
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента.Товары КАК ТаблицаТовары
		|		ПО ТаблицаКДоставке.Ссылка = ТаблицаТовары.Ссылка
		|			И ТаблицаКДоставке.КодСтроки = ТаблицаТовары.КодСтроки
		|			И (ТаблицаТовары.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить))
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента КАК ШапкаДокумента
		|		ПО ТаблицаКДоставке.Ссылка = ШапкаДокумента.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента.СкладыСведенияДоставки КАК ТаблицаСкладыСведенияДоставки
		|		ПО (ТаблицаТовары.Ссылка = ТаблицаСкладыСведенияДоставки.Ссылка)
		|			И (ТаблицаТовары.Склад = ТаблицаСкладыСведенияДоставки.Склад)
		|ГДЕ
		|	ТаблицаКДоставке.Ссылка = &Ссылка
		|	И &ТребуетсяДоставка
		|	И &СпособУказанияДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыУказанияДоставки.ПоСтрокам)
		|	И ТаблицаКДоставке.СпособДоставки <> ЗНАЧЕНИЕ(Перечисление.СпособыДоставки25.Самовывоз)
		|	И ТаблицаКДоставке.СпособДоставки <> ЗНАЧЕНИЕ(Перечисление.СпособыДоставки25.НашКурьер)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ШапкаДокумента.ВидПланированияДоставки,
		|	ТаблицаКДоставке.НомерСтроки,
		...
		|	ТаблицаКДоставке.Перевозчик,
		|	ТаблицаКДоставке.Перевозчик,
		|	ТаблицаКДоставке.ТочкаДоставкиПеревозчика,
		|	ТаблицаКДоставке.ЗонаДоставкиПеревозчика,
		|	ШапкаДокумента.Партнер,
		|	ТаблицаКДоставке.ТочкаДоставки,
		|	ТаблицаКДоставке.ЗонаДоставки,
		|	&Ссылка,
		|	НЕОПРЕДЕЛЕНО,
		|	ТаблицаТовары.Номенклатура,
		|	ТаблицаТовары.Характеристика,
		|	ТаблицаТовары.Серия,
		|	ТаблицаКДоставке.КодСтроки,
		|	ВЫБОР
		|		КОГДА ШапкаДокумента.ВидПланированияДоставки = ЗНАЧЕНИЕ(Перечисление.ВидыПланированияДоставки.ТекущимДокументом)
		|			ТОГДА &Ссылка
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		...
		|ГДЕ
		|	ТаблицаКДоставке.Ссылка = &Ссылка
		|	И ТаблицаКДоставке.ДоставкаДоПеревозчика
		|	И &ТребуетсяДоставка
		|	И &СпособУказанияДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыУказанияДоставки.ПоСтрокам)
		|	И ТаблицаКДоставке.СпособДоставки <> ЗНАЧЕНИЕ(Перечисление.СпособыДоставки25.Самовывоз)
		|	И ТаблицаКДоставке.СпособДоставки <> ЗНАЧЕНИЕ(Перечисление.СпособыДоставки25.НашКурьер)";
	...
	Если ДокументОбъект.СпособУказанияДоставки = Перечисления.СпособыУказанияДоставки.НаВесьДокумент Тогда
		...
		|	ТаблицаДанных.Плечо = 1 КАК ПервичнаяЗапись,
		|	ВЫБОР
		|		КОГДА ТаблицаДанных.Плечо = 1
		|			ТОГДА ТаблицаДанных.ДатаОтгрузки
		|		ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
		|	КОНЕЦ КАК ПервичнаяДатаОтгрузки,
		...
		|		ТаблицаТовары.Ссылка = &Ссылка
		|		И &ТребуетсяДоставка
		|		И &СпособУказанияДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыУказанияДоставки.НаВесьДокумент)
		...
		|		И НЕ ТаблицаТовары.Отменено
		|		И ТаблицаТовары.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить)
		|		И ШапкаДокумента.ДоставкаДоПеревозчика) КАК ТаблицаДанных";
		
	КонецЕсли;
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Документ.ЗаказКлиента.Товары КАК", "ДанныеДокументаТаблицаТовары КАК");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Документ.ЗаказКлиента КАК ШапкаДокумента", "ДанныеДокументаШапка КАК ШапкаДокумента");
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&НазначениеПоУмолчанию", "ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)");
	
	СинонимТаблицыДокумента = "ТаблицаДоставки";
...
Процедура ДобавитьКомандыОтчетов(КомандыОтчетов, Параметры) Экспорт
	...
	Если КомандаОтчет <> Неопределено Тогда
		КомандаОтчет.ВидимостьВФормах = "ФормаДокумента, ФормаДокументаСамообслуживание";
	...
	Если КомандаОтчет <> Неопределено Тогда
		...
		ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(
			КомандаОтчет,
			Метаданные.Документы.ЗаказКлиента.Реквизиты.ТребуетсяДоставка.Имя,
			Истина);
		
		ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(
			КомандаОтчет,
			Метаданные.Документы.ЗаказКлиента.Реквизиты.Организация.Имя,
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	|					ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовКлиентов.ГотовКОтгрузке),
	|					ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовКлиентов.ВПроцессеОтгрузки),
	|					ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовКлиентов.ОжидаетсяПереходПраваСобственности)) ТОГДА
	|			СостоянияЗаказовКлиентов.Заказ
