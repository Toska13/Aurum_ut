...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ИспользоватьУправлениеДоставкой25 = ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25();
	
	Если ИспользоватьУправлениеДоставкой Тогда
		
		Если ИспользоватьУправлениеДоставкой25 Тогда
			
			ЗаполнитьРеквизитыДоставкиДляОтображения();
			НастроитьСтраницуДоставки();
		...
		Иначе
			...
			ИначеЕсли Объект.СпособДоставки = Перечисления.СпособыДоставки.ДоКлиентаКурьером Тогда
...
Процедура СоглашениеПриИзмененииСервер(РезультатВыбора)
	...
	СтруктураДействийСоСкидками.Вставить("ЗаполнитьСтавкуНДС", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект, Истина));
	СтруктураДействийСоСкидками.Вставить("ЗаполнитьСтавкуНДСПоВидуЦен", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДСПоВидуЦен(Объект, Истина));
...
&НаСервере
Процедура НастроитьСтраницуДоставки()
	
	Если НЕ Объект.ТребуетсяДоставка Тогда
		
		СтраницаДоставки = Элементы.СтраницаСамовывоз;
		СпособДоставкиДляОтображения = Перечисления.СпособыДоставки25.Самовывоз;
		
	ИначеЕсли Объект.СпособУказанияДоставки = Перечисления.СпособыУказанияДоставки.ПоСтрокам
		  ИЛИ Объект.СпособДоставки25 = Перечисления.СпособыДоставки25.НашимиСилами
		  ИЛИ Объект.СпособДоставки25 = Перечисления.СпособыДоставки25.НашКурьер Тогда
		
		СтраницаДоставки = Элементы.СтраницаДоставкаДоПолучателя25;
		СпособДоставкиДляОтображения = Перечисления.СпособыДоставки25.НашимиСилами;
		
	ИначеЕсли Объект.СпособДоставки25 = Перечисления.СпособыДоставки25.ОпределяетСлужбаДоставки Тогда
		
		СтраницаДоставки = Элементы.СтраницаДоставкаДоПолучателя25;
		СпособДоставкиДляОтображения = Перечисления.СпособыДоставки25.ОпределяетСлужбаДоставки;
	...
	Элементы.СтраницыДоставки.ТекущаяСтраница = СтраницаДоставки;
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьРеквизитыДоставкиДляОтображения()
	...
	Если Объект.СпособУказанияДоставки = Перечисления.СпособыУказанияДоставки.НаВесьДокумент Тогда
		...
		ТочкаДоставкиДляОтображения = Объект.ТочкаДоставки;
		ВремяДоставкиСДляОтображения = Объект.ВремяДоставкиС;
		ВремяДоставкиПоДляОтображения = Объект.ВремяДоставкиПо;
		ДополнительнаяИнформацияДляОтображения = Объект.ДополнительнаяИнформацияПоДоставке;
		
	ИначеЕсли Объект.СпособУказанияДоставки = Перечисления.СпособыУказанияДоставки.ПоСтрокам Тогда 
		
		СтрокаИсточник = Неопределено;
		
		Для Каждого СтрокаДоставки Из Объект.ТоварыКДоставке Цикл
			
			Если ЗначениеЗаполнено(СтрокаДоставки.ТочкаДоставки) Тогда
				
				СтрокаИсточник = СтрокаДоставки;
				Прервать;
		...
		Если СтрокаИсточник <> Неопределено Тогда
			...
			ТочкаДоставкиДляОтображения = СтрокаИсточник.ТочкаДоставки;
			ВремяДоставкиСДляОтображения = СтрокаИсточник.ВремяДоставкиС;
			ВремяДоставкиПоДляОтображения = СтрокаИсточник.ВремяДоставкиПо;
			ДополнительнаяИнформацияДляОтображения = СтрокаИсточник.ДополнительнаяИнформацияПоДоставке;
