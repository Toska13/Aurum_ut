...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Склад) Тогда
		УстановитьДоступностьКнопокДобавления(ЭтотОбъект, Истина);
		РассчитатьРекомендацииПоддержанияЗапаса();
	Иначе
		УстановитьДоступностьКнопокДобавления(ЭтотОбъект, Ложь);
	КонецЕсли;
...
Процедура СкладПриИзменении(Элемент)
	...
	// Если при изменении склада не были досчитаны рекомендации для предыдущего значения склада, данные о расчете удаляются.
	СтрокаАктуальности = ПолучитьАктуальностьРекомендацийПоСкладу(АктуальностьРекомендаций, СтарыйСклад);
	Если СтрокаАктуальности <> Неопределено И ЗначениеЗаполнено(СтрокаАктуальности.ИдентификаторЗадания) Тогда
		АктуальностьРекомендаций.Удалить(СтрокаАктуальности);
	КонецЕсли;
...
Процедура РассчитатьРекомендацииИСтатистикуПотребления(Команда)
	...
	Иначе
		...
		СтрокаАктуальности.РезультатФоновогоЗадания = РезультатРасчета;
		ОжидатьРасчетаРекомендацийПоддержанияЗапасовСклада();
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	УстановитьОтборОформленияНеДляГруппы(Элемент);
	...
	Элемент = ДобавитьЭлементОформленияСУсловием(УсловноеОформлениеСписка, "ОбеспечиваемыйПериод",
		"ВариантОбеспечиваемогоПериода", Перечисления.ВариантыОбеспечиваемогоПериода.НеОграничен);
	УстановитьОтборОформленияНеДляГруппы(Элемент);
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СпособОбеспечения");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Заполнено;
	...
	Элемент = ДобавитьЭлементОформленияСУсловием(УсловноеОформлениеСписка, "ОбеспечиваемыйПериод",
		"ВариантОбеспечиваемогоПериода", Перечисления.ВариантыОбеспечиваемогоПериода.ДоДатыСледующейПоставкиПоРасписанию);
	УстановитьОтборОформленияНеДляГруппы(Элемент);
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СпособОбеспечения");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Заполнено;
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'До даты следующей поставки'"));
...
Процедура УстановитьОтборОформленияНеДляГруппы(Элемент)
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ЭтоГруппа");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Ложь;
...
Процедура ОжидатьРасчетаРекомендацийПоддержанияЗапасовСклада()
	...
	Если СтрокаАктуальности <> Неопределено И ЗначениеЗаполнено(СтрокаАктуальности.ИдентификаторЗадания) Тогда
		...
		ОповещениеОЗавершении = Новый ОписаниеОповещения(
			"РасчетРекомендацийПоддержанияЗапасовСкладаОповещение", ЭтотОбъект);
	
		ДлительныеОперацииКлиент.ОжидатьЗавершение(
			СтрокаАктуальности.РезультатФоновогоЗадания, ОповещениеОЗавершении, ПараметрыОжидания);
...
&НаКлиенте
Процедура РасчетРекомендацийПоддержанияЗапасовСкладаОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено"Тогда
		...
		Если СтрокаАктуальности <> Неопределено Тогда
			...
			СтрокаАктуальности.РекомендацииАктуальны = Истина;
			СтрокаАктуальности.ИдентификаторЗадания = Неопределено;
			СтрокаАктуальности.РезультатФоновогоЗадания = Неопределено;
			СостояниеРекомендаций = НСтр("ru = 'Рекомендации актуальны'");
			Элементы.РежимРекомендаций.Доступность = Истина;
			Если Элементы.СписокПоддерживаемыхЗапасовСреднедневноеПотребление.Видимость
				ИЛИ Элементы.СписокПоддерживаемыхЗапасовОтклонениеПотребления.Видимость Тогда
