...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Обработки.РаботаСНоменклатурой.СоздатьКомандыСостояния(ЭтотОбъект, 
		Элементы.ГруппаРезультатыНациональныйКаталог.Имя, 
		ВидКнопкиФормы.Гиперссылка);
	
	ПодсчетСтатистикиРезультатов   = РаботаСНоменклатуройСлужебный.ПолучитьСтатистикуРезультатов(УникальныйИдентификатор, 
										Организация);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ВРег(ИмяСобытия) = ВРег("Запись_НаборКонстант")
		...
		ИспользоватьСервисРаботаСНоменклатурой = РаботаСНоменклатуройСлужебныйВызовСервера.ИспользоватьСервисРаботаСНоменклатурой();
	...
	ИначеЕсли ВРег(ИмяСобытия) = ВРег("Запись_НаборКонстант")
				...
				И Элементы.ГруппаНациональныйКаталог.Видимость Тогда
		...
		КонецЕсли;
		ПодписыватьЭлектроннойПодписьюПриИзмененииНаКлиенте();
	...
	ИначеЕсли ВРег(ИмяСобытия) = ВРег("Обработка.РаботаСНоменклатурой.ОбновитьСтатистикуРезультатов")
		...
		ОбновитьСтатистикуИПроблемыБыстро(Истина, Истина, Ложь);
	...
	ИначеЕсли ВРег(ИмяСобытия) = ВРег("ВыгрузкаНоменклатурыЗавершена")
...
Процедура ЭлементВыгружатьВНациональныйКаталогПриИзменении(Элемент)
	...
	Оповестить("Обработка.РаботаСНоменклатурой.ПодписыватьЭлектроннойПодписью",
		?(ВыгружатьВНациональныйКаталог И ИспользоватьЭлектронныеПодписи, ПодписыватьЭлектроннойПодписью, Ложь),
		Организация);
	
КонецПроцедуры

&НаКлиенте
Процедура ПодписыватьЭлектроннойПодписьюПриИзменении(Элемент)
	
	ОбновляемыеПараметры = Новый Структура("ПодписыватьЭлектроннойПодписью", ПодписыватьЭлектроннойПодписью);
	РаботаСНоменклатуройСлужебныйВызовСервера.ОбновитьПараметрыНастройкиВыгрузки(Организация, ОбновляемыеПараметры);
...
&НаКлиенте
Процедура СопоставлениеНоменклатурыГотовоСтатистикаОбработкаНавигационнойСсылки(Элемент, 
...
Процедура Подключаемый_ДействиеКомандыСостояние(Команда)
	...
	ПараметрыОткрытия.Вставить("ПодписыватьЭлектроннойПодписью", ПодписыватьЭлектроннойПодписью);
	...
	ОткрытьФорму("Обработка.РаботаСНоменклатурой.Форма.РезультатыВыгрузкиНоменклатуры", 
		ПараметрыОткрытия, 
		ЭтотОбъект, 
		Организация);
...
Процедура ОрганизацияПриИзмененииНаКлиенте()
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, 
		ПараметрыФормы(Организация, ЛимитЗаписей, УникальныйИдентификатор),, "Организация");
	ОбновитьПараметрыФормыПриИзмененииОрганизации();
	
	Оповестить("Обработка.РаботаСНоменклатурой.ПриИзмененииОрганизации", 
		?(ВыгружатьВНациональныйКаталог И ИспользоватьЭлектронныеПодписи, ПодписыватьЭлектроннойПодписью, Ложь),
		Организация);
...
Функция ЗапуститьПроверкуАктивностиПартнера(Знач УникальныйИдентификатор, Знач КодПартера)
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, 
		"РаботаСНоменклатуройСлужебный.ПартнерАктивен", 
		КодПартера);
...
Процедура ПроверкаАктивностиПартнераЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекСохранить("РаботаСНоменклатурой",
		"ПомощникВыгрузки\НациональныйКаталогВидимость", 
		НациональныйКаталогВидимость);
...
Функция НоменклатураКВыгрузке(Знач Организация, Знач ЛимитЗаписей)
	...
	Возврат ВыборкаДанных.Количество();
...
&НаКлиенте
Процедура ВыбратьНоменклатуруНаКлиенте()
...
Функция ЗапуститьВыгрузкуНоменклатурыВФоне(Знач РазделениеВключено, Знач Организация)
	...
	ФоновоеЗадание = ФоновыеЗадания.Выполнить("РаботаСНоменклатуройСлужебный.ВыгрузитьДанныеНоменклатуры", 
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(НастройкаВыгрузки),
		КлючЗадания, 
		НСтр("ru = 'Выгрузка номенклатуры'"));
	УстановитьПривилегированныйРежим(Ложь);
...
Процедура ВыгружатьВНациональныйКаталогПриИзменении()
	...
	Элементы.ПодписыватьЭлектроннойПодписью.Доступность             = ВыгружатьВНациональныйКаталог;
...
&НаКлиенте
Процедура ПодписыватьЭлектроннойПодписьюПриИзмененииНаКлиенте()
	
	Элементы.ГруппаНациональныйКаталогЭлектроннаяПодпись.Видимость = ИспользоватьЭлектронныеПодписи;
	РаботаСНоменклатуройСлужебныйКлиент.ОбновитьЗаголовкиКнопокСостояние(ЭтотОбъект);
	
	Если ПодписыватьЭлектроннойПодписью 
			И ИспользоватьЭлектронныеПодписи Тогда
		Элементы.ГруппаРезультаты.Заголовок = НСтр("ru = 'Ознакомиться с результатами, подписать'");
	Иначе
		Элементы.ГруппаРезультаты.Заголовок = НСтр("ru = 'Ознакомиться с результатами'");
	КонецЕсли;
...
Процедура ПроверитьВыгрузкуНоменклатурыВФорме()
	Если ЗначениеЗаполнено(ИдентификаторЗадания) Тогда
		...
		Если Не ЗначениеЗаполнено(ИдентификаторЗадания) Тогда
			...
			Если ПрогрессВыгрузки.Свойство("ДатаПредставление") 
					Или ПрогрессВыгрузки.Свойство("ДатаВыгрузки") Тогда
...
Функция ДлительнаяОперацияЕстьПроблемыБыстро(Знач УникальныйИдентификатор, Знач Организация)
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, 
		"РаботаСНоменклатуройСлужебный.ЕстьПроблемыБыстро", 
		Организация);
...
Функция ОбновитьПараметрыНастройкиВыгрузки(Знач УникальныйИдентификатор, Знач Организация, Знач ОбновляемыеПараметры, Знач ОбработатьОтклоненные, Знач ИдентификаторЗадания)
	...
	Если ОбработатьОтклоненные Тогда
		ДлительныеОперации.ВыполнитьПроцедуру(, 
			"Обработки.РаботаСНоменклатурой.СброситьСостоянияВыгрузкиНоменклатуры", 
			Организация, 
			Перечисления.СостоянияВыгрузкиНоменклатуры.Отклонена);
	КонецЕсли;
...
Процедура ОбновитьСтатистикуИПроблемыБыстро(ОбновитьКоличествоКВыгрузке = Истина, ОбновитьСтатусПроблем = Истина, 
			ОбновитьСтатистикуРезультатов = Истина)
	
	ЗапроситьНаСервере = Новый Структура;
	Если ВыгружатьВНациональныйКаталог И ОбновитьСтатистикуРезультатов Тогда
...
Процедура ЗаполнитьСписокВыбораОрганизаций()
	...
	Если ОбновитьКлючи Тогда
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("РаботаСНоменклатурой", 
			"ПомощникВыгрузки\КлючиВыгрузки",
			КлючиВыгрузки);
	КонецЕсли;
...
Процедура ПроверитьДоступностьФункционалаНаСервере(Отказ)
	...
	Если НастройкиВыгрузки.ИспользоватьВыгрузкуВНациональныйКаталог Тогда
		НациональныйКаталогВидимость = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("РаботаСНоменклатурой", 
			"ПомощникВыгрузки\НациональныйКаталогВидимость");
		НациональныйКаталогВидимость = (НациональныйКаталогВидимость = Истина);
		Элементы.ГруппаНациональныйКаталог.Видимость = НациональныйКаталогВидимость;
...
Процедура ПроверитьДоступностьФункционалаНаКлиенте()
	...
	Иначе
		...
		ПараметрыОткрытия.ОписаниеРаздела = НСтр("ru = 'Для возможности выгрузки номенклатуры необходимо включить использование сервиса 1С:Номенклатура.'");
		
		РаботаСНоменклатуройКлиент.ОткрытьФормуПанелиАдминистрирования(ПараметрыОткрытия, 
			ЭтотОбъект,
...
Процедура ОбновитьФормуПослеОткрытия()
	...
	Если (Не Элементы.ГруппаНациональныйКаталог.Видимость
		...
		Иначе
			...
			ДлительныеОперацииКлиент.ОжидатьЗавершение(ПроверкаАктивностиПартнера, 
	...
	Если Не ПустаяСтрока(АдресТоваровВХранилище) 
			И ЭтоАдресВременногоХранилища(АдресТоваровВХранилище) Тогда
		ВыбратьНоменклатуруНаКлиенте();
	КонецЕсли;
...
Процедура ПослеЗакрытияПанелиАдминистрирования(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если Не РезультатЗакрытия = Неопределено 
			И РезультатЗакрытия.ИспользоватьСервисРаботаСНоменклатурой Тогда
		ИспользоватьСервисРаботаСНоменклатурой = Истина;
		ОбновитьФормуПослеОткрытия();
	Иначе
		Закрыть();
...
Процедура УстановитьВидимостьГруппыРезультаты()
	...
	ВидимостьГруппы = ВыгружатьВНациональныйКаталог И ЗначениеЗаполнено(Организация);
...
Процедура УстановитьЗаголовокКомандыВыбораРежимаПолученияНовыхШтрихкодов()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, 
		"ВыбратьРежимПолученияНовыхШтрихкодов",
		"Заголовок", 
		ЗаголовокРежим);
