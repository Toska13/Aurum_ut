...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ВидНоменклатуры) Тогда
		...
		Шаблон              = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1%2Отображаются только категории, сопоставленные с'") + " ",
			ТекстПредупреждения, 
			Разделитель) + "<a href='%1'>%2</a>";
		ТекстПредупреждения = СтроковыеФункции.ФорматированнаяСтрока(Шаблон, НавигационнаяСсылка, ВидНоменклатуры);
	...
	Если СсылкаНаОбъект <> Неопределено И НЕ ЗначениеЗаполнено(ВидНоменклатуры) Тогда
		...
		ВычислениеКатегорийОграничения = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, 
			"РаботаСНоменклатуройСлужебный.ДоступныеКатегорииСервиса", 
			СсылкаНаОбъект);
...
Процедура ПриОткрытии(Отказ)
	...
	Если ИнтернетПоддержкаПодключена Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(СтрокаПоиска) Тогда
			КоличествоЗаданий = 0;
			ПостроитьДеревоКатегорий(Неопределено,, СтрокаПоиска, Истина);
...
Процедура СнятьФлажки(Команда)
	...
	ИдентификаторыВыбранныхКатегорий.Очистить();
...
Функция ПолучитьОперациюДляЗапросаВСервисе(ИдентификаторКатегории, ПолучитьДанныеИзКэша,
	...
	ИначеЕсли ЗначениеЗаполнено(СтрокаПоискаКатегорий) И ИдентификаторКатегории = Неопределено Тогда
		...
		ДлительнаяОперация = РаботаСНоменклатуройСлужебныйКлиент.ПоискКатегорийПоСтроке(СтрокаПоискаКатегорий,
			УникальныйИдентификатор, Неопределено, Истина);
...
Функция ДлительнаяОперацияВычислениеКэшаПутей(Знач ИдентификаторыВыбранных, Знач УникальныйИдентификатор)
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, 
		"РаботаСНоменклатуройСлужебный.КэшПутейКатегорийСервиса",
		РаботаСНоменклатуройСлужебныйПовтИсп.КэшПутейКатегорий(), 
...
Функция ВосстановлениеДереваКатегорийИзКэшаНаСервере(ИдентификаторыПомеченных, РежимОтборПоСтроке, РежимТолькоВыбранные)
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, 
		"РаботаСНоменклатуройСлужебный.КэшПутейКатегорийСервиса", 
		РаботаСНоменклатуройСлужебныйПовтИсп.КэшПутейКатегорий(), 
		ИдентификаторыРодителей, 
		РежимТолькоВыбранные);
