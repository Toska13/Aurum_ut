...
Процедура КатегорияСервисаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если РежимСопоставления = "ПоНоменклатуре" Тогда
		КодТНВЭД = КатегорииНоменклатуры["КодТНВЭД"];
	КонецЕсли;
	
	ВыбратьКатегориюСервиса(ИдентификаторКатегорииСервиса, ТекущаяСсылка(), КодТНВЭД, ВидНоменклатуры, Оповещение);
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Найти(Поле.Имя, "Категория")
		...
		КодТНВЭД = "";
		Если РежимСопоставления = "ПоНоменклатуре" Тогда
			КодТНВЭД = Элемент.ТекущиеДанные.КодТНВЭД;
		КонецЕсли;
		
		ВыбратьКатегориюСервиса(Элемент.ТекущиеДанные.ИдентификаторКатегории, Элемент.ТекущиеДанные.Ссылка, КодТНВЭД, 
			ВидНоменклатуры);
...
Процедура ВыбратьКатегорию(Команда)
	...
	ИначеЕсли РежимСопоставления = "ПоНоменклатуре" И ТекущийСписок.ТекущиеДанные.ИдентификаторКатегорииПоВидам = "МножественнаяПривязкаКатегорий" Тогда
		...
		КодТНВЭД        = ТекущийСписок.ТекущиеДанные.КодТНВЭД;
	КонецЕсли;
	...
	ВыбратьКатегориюСервиса(ИдентификаторКатегории, СтрокиДляОбработки, КодТНВЭД, ВидНоменклатуры,, ТекстПредупреждения);
...
Процедура ОбновитьРеквизитыСервиса()
	...
	Задание = Новый Структура("Наименование, ПараметрыПроцедуры");
...
&НаСервереБезКонтекста
Функция ВыполнитьЗаданиеВФоне(Знач Задание, Знач УникальныйИдентификатор)
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = Задание.Наименование;
	ПараметрыВыполнения.ЗапуститьВФоне              = Истина;
	
	Возврат ДлительныеОперации.ВыполнитьВФоне("РаботаСНоменклатурой.ПолучитьРеквизитыРубрикатора", 
...
&НаКлиенте
Процедура ВыбратьКатегориюСервиса(ИдентификаторКатегории, СсылкаНаОбъект, КодТНВЭД, ВидНоменклатуры = Неопределено, 
			Оповещение = Неопределено, ТекстПредупреждения = "")
	...
	ИначеЕсли СценарийИспользования = "ВыгрузкаНоменклатуры" 
			Или СценарийИспользования = "УстранениеПроблемЗаполнения" Тогда
		ПараметрыФормыВыбора.СтрокаПоиска = КодТНВЭД;
	КонецЕсли;
...
Процедура СохранитьСопоставлениеРеквизитов(ТекущаяСтрока, Знач ВидНоменклатуры)
	...
	Если ЗначениеЗаполнено(РеквизитОбъекта) Тогда
		...
		ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(РеквизитОбъекта, "ТипЗначения") Тогда 
			ТипЗначения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитОбъекта, "ТипЗначения");
		КонецЕсли;
	КонецЕсли;
...
Процедура СохранитьИзмененияРеквизитовНаСервере(ИзмененныеСтроки, Знач СоответствиеРеквизитов, Знач ПараметрВидНоменклатуры, Знач ПараметрИдентификаторКатегории)
	...
	Если СоответствиеРеквизитов.Количество() Тогда
		...
		ДлительнаяОперация = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, 
			"РаботаСНоменклатуройСлужебный.СопоставитьЗначенияРеквизитовАвтоматически", 
			СоответствиеРеквизитов, 
			ПараметрВидНоменклатуры, 
			ПараметрИдентификаторКатегории);
	КонецЕсли;
...
Функция ОбновлениеПроблемЗаполнения(Знач УникальныйИдентификатор, Знач НастройкаВыгрузки)
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, 
		"РаботаСНоменклатуройСлужебный.ПроверитьГотовностьНоменклатурыКВыгрузке", 
		НастройкаВыгрузки);
...
Функция ДлительнаяОперацияОбязательныеРеквизиты(Знач УникальныйИдентификатор)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = 
		НСтр("ru = 'Работа с номенклатурой. Получение обязательных реквизитов Национального каталога.'");
	
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, 
		"РаботаСНоменклатуройСлужебный.ОбязательныеРеквизитыНациональногоКаталога");
...
&НаСервереБезКонтекста
Функция ШаблонКодТНВЭД(ПараметрПодстановки = """""")
	Возврат СтрШаблон("ВЫРАЗИТЬ(%1 КАК СТРОКА(10))", ПараметрПодстановки);
КонецФункции
...
Процедура ИнициализацияДинамическихСписков(ТекстЗапроса, ТаблицаНоменклатура, ИспользоватьИерархию, ИспользоватьВидыНоменклатуры)
	...
	КолонкаКодТНВЭД = ОсновнойЗапрос.Колонки.Найти("КодТНВЭД");
	Если КолонкаКодТНВЭД = Неопределено Тогда
		ОператорыЗапроса.ВыбираемыеПоля.Добавить(ШаблонКодТНВЭД());
		ОсновнойЗапрос.Колонки[ОсновнойЗапрос.Колонки.Количество() - 1].Псевдоним = "КодТНВЭД";
		Элементы.СписокПоНоменклатуреКодТНВЭД.Видимость = Ложь;
	Иначе
		Элементы.СписокПоНоменклатуреКодТНВЭД.Видимость = СценарийИспользования = "ВыгрузкаНоменклатуры" 
															Или СценарийИспользования = "УстранениеПроблемЗаполнения";
	КонецЕсли;
