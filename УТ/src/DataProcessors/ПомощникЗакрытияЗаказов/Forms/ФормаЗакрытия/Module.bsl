...
Процедура ЗаполнитьОтклоненияПоЗаказамКлиентов(ТаблицаТоваровРасход)
	...
	|		СУММА(РаспоряженияНаОтгрузкуОбороты.ОтгруженоКоличествоОборот) КАК КОформлениюКонечныйОстаток,
	|	СУММА(РаспоряженияНаОтгрузкуОбороты.КОтгрузкеКоличествоОборот) КАК КОформлениюПриход
	...
	|	ИЛИ СУММА(РаспоряженияНаОтгрузкуОбороты.КОтгрузкеКоличествоОборот) <> 0
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ЗаказКлиента,
	|	Номенклатура,
	...
	|		(ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))) КАК
	|			ТоварыКОтгрузкеОстатки
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|		0 КАК Количество,
	|		-ОстаткиКОформлению.КОтгрузкеКоличествоОборот КАК КОтгрузкеСверхЗаказа,
	|		0 КАК ДопустимоеОтклонение
	|	ИЗ
	|		#РаспоряженияНаОтгрузкуОборотыРазвернутая2 КАК ОстаткиКОформлению) КАК ТоварыКОтгрузкеОстатки
	|СГРУППИРОВАТЬ ПО
	...
	|	ЗаказыКлиентовОстатки.ОтгруженоКоличествоОборот КАК Количество,
	|	0 КАК ДопустимоеОтклонение
	|ИЗ
	|	#РаспоряженияНаОтгрузкуОборотыРазвернутая3 КАК ЗаказыКлиентовОстатки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	0,
	|	СУММА(ТоварыКОтгрузке.КОтгрузкеКоличествоОборот * (&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100))
	|ИЗ
	|	#РаспоряженияНаОтгрузкуОборотыРазвернутая4 КАК ТоварыКОтгрузке
	|СГРУППИРОВАТЬ ПО
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыУслугиКОформлениюОтчетовКомитентуОЗакупках.Остатки КАК
	|			ТоварыУслугиКОформлениюОтчетовКомитентуОЗакупкахОстатки
	...
	|	И
	|		ЗаказКлиентаТовары.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоставкаПодПринципала)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА ТаблицаТовары.Количество > ЗаказыКлиентовОстатки.КОформлениюПриход
	|			ТОГДА ТаблицаТовары.Количество - ЗаказыКлиентовОстатки.КОформлениюПриход
	|		ИНАЧЕ 0
	...
	// подзапрос #РаспоряженияНаОтгрузкуОборотыРазвернутая1
	...
	// подзапрос #РаспоряженияНаОтгрузкуОборотыРазвернутая2
	ПараметрыФормированияТаблицы = РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат.НовыйПараметрыФормированияРазвернутойТаблицы();
	ПараметрыФормированияТаблицы.НачалоПериода = "&НачПериодЗаказыКлиентов";
	ПараметрыФормированияТаблицы.КонецПериода = "&КонПериодЗаказыКлиентов";
	ПараметрыФормированияТаблицы.ТекстУсловия =
	...
	// подзапрос #РаспоряженияНаОтгрузкуОборотыРазвернутая3
	ПараметрыФормированияТаблицы = РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат.НовыйПараметрыФормированияРазвернутойТаблицы();
	ПараметрыФормированияТаблицы.ТекстУсловия =
		"Распоряжение В (&ЗаказыКлиента)
	...
	// подзапрос #РаспоряженияНаОтгрузкуОборотыРазвернутая4
	ПараметрыФормированияТаблицы = РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат.НовыйПараметрыФормированияРазвернутойТаблицы();
	ПараметрыФормированияТаблицы.НачалоПериода = "&НачПериодЗаказыКлиентов";
	ПараметрыФормированияТаблицы.КонецПериода = "&КонПериодЗаказыКлиентов";
	ПараметрыФормированияТаблицы.ТекстУсловия =
...
Процедура ЗаполнитьОтклоненияПоЗаявкамНаВозвратТоваровОтКлиента(ТаблицаТоваровПриход, ТаблицаТоваровРасход)
	...
	|	ТоварыКОтгрузке.Склад КАК Склад,
	|	ИСТИНА КАК ЭтоОтгрузка,
	|	СУММА(ТоварыКОтгрузке.КОтгрузкеКоличествоОборот
	|		* (&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100)) КАК ДопустимоеОтклонение
	|ИЗ
	|	#РаспоряженияНаОтгрузкуОборотыРазвернутая1 КАК ТоварыКОтгрузке
	|
	|СГРУППИРОВАТЬ ПО
	|	ТоварыКОтгрузке.Номенклатура,
	|	ТоварыКОтгрузке.Характеристика,
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА Таблица.НоменклатураОприходование = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|			ТОГДА Таблица.Номенклатура
	|		ИНАЧЕ Таблица.НоменклатураОприходование
	|	КОНЕЦ                  КАК Номенклатура,
	|	ВЫБОР
	|		КОГДА Таблица.ХарактеристикаОприходование = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
	|			ТОГДА Таблица.Характеристика
	|		ИНАЧЕ Таблица.ХарактеристикаОприходование
	...
	|		Остатки.Характеристика КАК Характеристика,
	|		Остатки.Склад КАК Склад,
	|		СУММА(Остатки.ОтгруженоКоличествоОборот) КАК Оформлено,
	|		СУММА(0) КАК Отгружено,
	|		СУММА(ВЫБОР
	|			КОГДА &ПоДокументу
	|				ТОГДА 0
	|			ИНАЧЕ
	|				Остатки.ЗаказаноКоличествоОборот
	|		КОНЕЦ) КАК Заказано
	|	ИЗ
	|		#РаспоряженияНаОтгрузкуОборотыРазвернутая2 КАК Остатки
	|	СГРУППИРОВАТЬ ПО
	...
	|			КОГДА &ПоДокументу
	|				ТОГДА 0
	|			ИНАЧЕ Остатки.ЗаявленоНаВозвратКоличествоОборот
	|		КОНЕЦ КАК Заказано
	|	ИЗ
	|		#ЗаявкиНаВозвратОборотыРазвернутая2 КАК Остатки
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	// подзапрос #ЗаявкиНаВозвратОборотыРазвернутая1
	ПараметрыФормированияТаблицы = РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат.НовыйПараметрыФормированияРазвернутойТаблицы();
	ПараметрыФормированияТаблицы.НачалоПериода = "&НачПериодЗаявки";
	ПараметрыФормированияТаблицы.КонецПериода = "&КонПериодЗаявки";
	ПараметрыФормированияТаблицы.ТекстУсловия =
		"Распоряжение В (&ЗаявкиНаВозвратТоваровОтКлиента)
		|		И Номенклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины В (&МерныеТипыЕдиницИзмерений)";
	ПараметрыФормированияТаблицы.Группировка = "Распоряжение, Номенклатура, Характеристика, Склад";
	...
	// подзапрос #ЗаявкиНаВозвратОборотыРазвернутая2
	ПараметрыФормированияТаблицы = РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат.НовыйПараметрыФормированияРазвернутойТаблицы();
	ПараметрыФормированияТаблицы.НачалоПериода = "&НачПериодЗаявки";
	ПараметрыФормированияТаблицы.КонецПериода = "&КонПериодЗаявки";
	ПараметрыФормированияТаблицы.ТекстУсловия =
...
Функция НоваяТаблицаТоваровПриходРасход()
	...
	Результат.Колонки.Добавить("НоменклатураОприходование",   Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	Результат.Колонки.Добавить("ХарактеристикаОприходование", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
