...
Процедура СопоставитьКлассификаторы(Команда)
	...
	ВыполнитьДействиеСопоставитьКлассификаторы();
...
Процедура СтраницаНачальныеДанные_ПриПереходеДалее(Отказ)
	...
	Иначе
		Если НЕ ЗначениеЗаполнено(Контрагент) Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Объект.ОрганизацияЕГАИС",, Отказ);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(ТорговыйОбъект)
			И Не УпрощеннаяИнтеграцияБЭД Тогда
...
Процедура ОткрытьФормуСканированияАлкогольнойПродукции()
	...
	ОткрытьФорму("Обработка.ПроверкаИПодборПродукцииИС.Форма.ПроверкаИПодбор", ПараметрыОткрытия, ЭтотОбъект, УникальныйИдентификатор,,
...
&НаКлиенте
Процедура ВыполнитьДействиеСопоставитьКлассификаторы()
...
Функция НайтиПодготовитьПроведенныеДокументыДляВыгрузки(МассивДокументов)
	...
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(МассивДокументов, "Проведен");
	
	Для Каждого ДокументСсылка Из МассивДокументов Цикл
		
		Если ЗначенияРеквизитов[ДокументСсылка] Тогда
...
Процедура ЗаполнитьСлужебныеРеквизитыТабличныхЧастей()
	...
	ИнтеграцияИС.ЗаполнитьСлужебныеРеквизитыВКоллекции(ЭтотОбъект, Объект.Корректировки);
	ИнтеграцияИС.ЗаполнитьСлужебныеРеквизитыВКоллекции(ЭтотОбъект, Объект.Остатки);
