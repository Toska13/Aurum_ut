...
Процедура ЗагрузитьСамозанятыхИзФайла(Команда)
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ЗагрузитьСамозанятыхИзФайлаПослеПроверки", ЭтотОбъект);
	ЗагрузитьИзФайла(ОповещениеОЗавершении);
...
Процедура ЗагрузитьЧекиСамозанятыхИзФайла(Команда)
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ЗагрузитьЧекиСамозанятыхИзФайлаПослеПроверки", ЭтотОбъект);
	ЗагрузитьИзФайла(ОповещениеОЗавершении);
...
Процедура ЗаполнитьДанныеПоУмолчанию()
	...
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		СтруктураПараметров = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации(Объект.Организация);
		СтруктураПараметров.Валюта = Валюта;
		БанковскийСчет = Справочники.БанковскиеСчетаОрганизаций.ПолучитьБанковскийСчетПоУмолчанию(СтруктураПараметров);
...
Процедура ДокументыОплатыОбъектРасчетовПриИзмененииНаСервере(ИдентификаторСтроки)
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы.Договор) Тогда
		...
		СтатьяДвиженияДенежныхСредств = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТаблицы.Договор, "СтатьяДвиженияДенежныхСредств");
...
Процедура ЗаполнитьКонтрагентовИзФайла(АдресЗагруженныхДанных)
	...
	ПараметрыОтбора.Вставить("Партнер", Справочники.Партнеры.ПустаяСсылка());
	НайденныеСтроки = СопоставленныеДанныеКонтрагентов.НайтиСтроки(ПараметрыОтбора);
	...
	Если НайденныеСтроки.Количество() <> 0 Тогда
		...
		Запрос.Текст = 
			...
			|	ИсходныеДанные.БанковскийСчетКонтрагента КАК БанковскийСчетКонтрагента,
			|	ИсходныеДанные.БанковскийСчетОрганизации КАК БанковскийСчетОрганизации,
			...
			|	ИсходныеДанные.Идентификатор КАК Идентификатор,
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	СопоставленныеДанные.Контрагент КАК Контрагент,
			|	ВЫБОР
			|		КОГДА СопоставленныеДанные.Партнер = ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)
			|				И СопоставленныеДанные.Договор <> ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
			|			ТОГДА ДоговорыКонтрагентов.Партнер
			|		КОГДА СопоставленныеДанные.Партнер = ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)
			|				И СопоставленныеДанные.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
			|			ТОГДА Контрагенты.Партнер
			|		ИНАЧЕ СопоставленныеДанные.Партнер
			|	КОНЕЦ КАК Партнер,
			|	СопоставленныеДанные.Договор КАК Договор,
			|	ВЫБОР
			|		КОГДА СопоставленныеДанные.БанковскийСчетКонтрагента = ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаКонтрагентов.ПустаяСсылка)
			|				И СопоставленныеДанные.Договор <> ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
			|				И НЕ ДоговорыКонтрагентов.БанковскийСчетКонтрагента ЕСТЬ NULL
			|			ТОГДА ДоговорыКонтрагентов.БанковскийСчетКонтрагента
			|		ИНАЧЕ СопоставленныеДанные.БанковскийСчетКонтрагента
			|	КОНЕЦ КАК БанковскийСчетКонтрагента,
			|	ВЫБОР
			|		КОГДА &БанковскийСчет <> ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка)
			|			ТОГДА &БанковскийСчет
			|		ИНАЧЕ СопоставленныеДанные.БанковскийСчетОрганизации
			|	КОНЕЦ КАК БанковскийСчетОрганизации,
			|	ВЫБОР
			|		КОГДА СопоставленныеДанные.СтатьяДвиженияДенежныхСредств = ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка)
			|				И СопоставленныеДанные.Договор <> ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
			|				И НЕ ДоговорыКонтрагентов.СтатьяДвиженияДенежныхСредств ЕСТЬ NULL
			|			ТОГДА ДоговорыКонтрагентов.СтатьяДвиженияДенежныхСредств
			|		ИНАЧЕ СопоставленныеДанные.СтатьяДвиженияДенежныхСредств
			|	КОНЕЦ КАК СтатьяДвиженияДенежныхСредств,
			|	СопоставленныеДанные.ОбъектРасчетов КАК ОбъектРасчетов,
			|	СопоставленныеДанные.ДатаПлатежа КАК ДатаПлатежа,
			|	СопоставленныеДанные.Идентификатор КАК Идентификатор,
			|	СопоставленныеДанные.Сумма КАК Сумма
			|ИЗ
			|	СопоставленныеДанные КАК СопоставленныеДанные
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Контрагенты
			|		ПО СопоставленныеДанные.Контрагент = Контрагенты.Ссылка
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
			|		ПО СопоставленныеДанные.Договор = ДоговорыКонтрагентов.Ссылка";
		
		Запрос.УстановитьПараметр("ИсходныеДанные", СопоставленныеДанныеКонтрагентов);
		Запрос.УстановитьПараметр("БанковскийСчет", БанковскийСчет);
		
		СопоставленныеДанныеКонтрагентов = Запрос.Выполнить().Выгрузить();
	...
	Для Каждого СтрокаТаблицы Из ПодобранныеСтроки Цикл
		...
		Если ВыборкаДоговораПоКонтрагенту.НайтиСледующий(Отбор) Тогда
			...
			Если ВыборкаДоговора.Следующий() Тогда
				...
				СтрокаТаблицы.Договор = ВыборкаДоговора.Договор;
				СтрокаТаблицы.ОбъектРасчетов = ВыборкаДоговора.ОбъектРасчетов;
				
				Если ЗначениеЗаполнено(ВыборкаДоговора.СтатьяДвиженияДенежныхСредств) Тогда
					СтрокаТаблицы.СтатьяДвиженияДенежныхСредств = ВыборкаДоговора.СтатьяДвиженияДенежныхСредств;
				КонецЕсли;
...
Процедура СоздатьДокументыОплатыПослеПолученияПараметров(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗакрытия <> Неопределено
		...
		ФоновоеЗадание = ЗаданиеСозданияДокументовОплаты(ПараметрыПроцедуры);
...
&НаСервере
Функция ЗаданиеСозданияДокументовОплаты(Знач ПараметрыПроцедуры)
	...
	ФоновоеЗадание = ДлительныеОперации.ВыполнитьВФоне(
		"Обработки.ПомощникРаботыССамозанятыми.СоздатьДокументыОплаты", ПараметрыПроцедуры, ПараметрыЗадания);
...
Процедура СоздатьДокументыПриобретенияПослеПолученияПараметров(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗакрытия <> Неопределено
		...
		ФоновоеЗадание = ЗаданиеСозданияДокументовПриобретения(ПараметрыПроцедуры);
...
&НаСервере
Функция ЗаданиеСозданияДокументовПриобретения(Знач ПараметрыПроцедуры)

	ПараметрыПроцедуры.Вставить("ТаблицаДанных", Объект.ДокументыПриобретения.Выгрузить());
	ПараметрыЗадания = ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор);
	ФоновоеЗадание = ДлительныеОперации.ВыполнитьВФоне(
		"Обработки.ПомощникРаботыССамозанятыми.СоздатьДокументыПриобретения", ПараметрыПроцедуры, ПараметрыЗадания);
...
&НаКлиенте
Процедура ЗагрузитьИзФайла(ОповещениеОЗавершении)
