...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	ДобавитьВСтруктуруДействияСНовымиСтроками(ЭтотОбъект, СтруктураДействийСДобавленнымиСтроками);
...
Функция ОбработатьШтрихкодыСервер(СтруктураПараметровДействия, КэшированныеЗначения)
	...
	Если СтруктураПараметровДействия.ТекущаяСтрока <> Неопределено Тогда
		...
		ПоляПоиска = Новый Структура("Номенклатура, Характеристика, Назначение, Серия, Упаковка");
...
Процедура УстановитьУсловноеОформление()
	...
	ПараметрыУсловногоОформления.ПутьКПолюТипИзмеряемойВеличины = "Объект.Неотгружаемые.ТипИзмеряемойВеличины";
	НоменклатураСервер.УстановитьУсловноеОформлениеЕдиницИзмерения(ЭтотОбъект, ПараметрыУсловногоОформления);
	...
	ПараметрыУсловногоОформления.ПутьКПолюТипИзмеряемойВеличины = "Объект.Недобор.ТипИзмеряемойВеличины";
	НоменклатураСервер.УстановитьУсловноеОформлениеЕдиницИзмерения(ЭтотОбъект, ПараметрыУсловногоОформления);
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.НеотгружаемыеУпаковка.Имя);

	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Неотгружаемые.Упаковка");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.НезаполненноеПолеТаблицы);
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.НедоборУпаковка.Имя);

	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Недобор.Упаковка");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.НезаполненноеПолеТаблицы);
...
Функция ЗавершитьПроверкуСервер(ПереноситьРезультатВОрдер = Истина)
	...
	Если Объект.Неотгружаемые.Количество() > 0 Тогда
		НоменклатураСервер.ЗаполнитьСлужебныеРеквизитыПоНоменклатуреВКоллекции(Объект.Неотгружаемые, ПараметрыЗаполненияРеквизитов);
	КонецЕсли;

	Если Объект.Недобор.Количество() > 0 Тогда
		НоменклатураСервер.ЗаполнитьСлужебныеРеквизитыПоНоменклатуреВКоллекции(Объект.Недобор, ПараметрыЗаполненияРеквизитов);
	КонецЕсли;
...
Функция РазбитьПоНеотгружаемым(КешированныеЗначения)
	...
	УчитываемыеПоля = СкладыКлиентСервер.УчитываемыеПриПроверкеРасходногоОрдераПоля(ЭтотОбъект, Неопределено);
	
	ИспользоватьУпаковкиНоменклатуры = ПолучитьФункциональнуюОпцию("ИспользоватьУпаковкиНоменклатуры");
	
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	УчитываемыеПоля.УчитыватьУпаковки = ИспользоватьУпаковкиНоменклатуры;
	...
	Пока ВыборкаПревышений.Следующий() Цикл
		...
		КоличествоУпаковокВДокументе = ВыборкаПревышений.КоличествоУпаковокВДокументе;
		...
		Если ВыборкаПревышений.КоличествоУпаковокВДокументе > 0 Тогда
			...
			Для Каждого СтрокаТЧ Из СтрокиСИзлишками Цикл
				...
				НеОтгружаемаяСтрока = НеОтгружаемаяСтрока(СтрокаТЧ, НеОтгружаемаяСтрока, ИспользоватьУпаковкиНоменклатуры);
				...
				Если НакопленноеКоличество + СтрокаТЧ.КоличествоУпаковок >= КоличествоУпаковокВДокументе Тогда
					...
					ОбрабатываемыеСтроки.Добавить(НеОтгружаемаяСтрока);
		...
		Иначе
			...
			ЛишнееКоличество = ВыборкаПревышений.Количество / КоэффициентУпаковки;
...
&НаСервере
Функция НеОтгружаемаяСтрока(СтрокаИзлишков, ТекущаяНеотгружаемаяСтрока, ИспользоватьУпаковкиНоменклатуры)
	
	ПоляПоиска = "Номенклатура,Характеристика,Назначение,Серия";
	Если ИспользоватьУпаковкиНоменклатуры Тогда
		ПоляПоиска = ПоляПоиска + ",Упаковка";
	КонецЕсли;
