...
Функция ПреобразоватьРасширеннуюНастройкуВУпрощенную(Форма)
	...
	Для Каждого ЭлементОтбора Из Форма.КомпоновщикНастроекОтбор.Настройки.Отбор.Элементы Цикл
		...
		Если Не Форма.ИспользоватьНоменклатуруПартнеров
			И Строка(ЭлементОтбора.ЛевоеЗначение) = Строка(ПолеНоменклатураРодитель)
		...
		Если Форма.ИспользоватьНоменклатуруПартнеров
			И Строка(ЭлементОтбора.ЛевоеЗначение) = Строка(ПолеНоменклатураПартнераРодитель)
			И ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке Тогда
