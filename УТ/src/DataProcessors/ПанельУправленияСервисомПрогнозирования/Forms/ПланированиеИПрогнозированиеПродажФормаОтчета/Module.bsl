...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВыполненоПодключение Тогда
		ВариантПерехода = СервисПрогнозирования.ВариантВыполненногоПерехода();
	КонецЕсли;
...
#КонецЕсли
	...
	// Сервис прогнозирования
	Если Не ПустаяСтрока(ВариантПерехода)
		И ВариантПерехода <> "ПереходаНеБыло" Тогда
		Отказ = Истина;
		Если ДоступныПраваАдминистратораСервисаПрогнозированияПродаж Тогда
			ОткрытьФорму("Обработка.ПанельУправленияСервисомПрогнозирования.Форма.БлокировкаСервиса",
				Новый Структура("ВариантПерехода", ВариантПерехода));
		Иначе
			ТекстОшибки = НСтр("ru = 'Необходимо заново подключиться в сервисе прогнозирования.
				|Текущий пользователь не имеет прав для выполнения переподключения. Обратитесь к администратору сервиса.'");
	...
	Элементы.ГруппаБиллингПодробно.Заголовок = НСтр("ru='Идет расчет баланса...'");
	...
	// Сервис прогнозирования.
	ОбновитьВидимостьТестовыхПериодов();
	Подключаемый_ИндикаторВыгрузки();
	ПодключитьОбработчикОжидания("Подключаемый_ОбновитьСтатусПодключенияИБаланс", 1, Истина);
	ПодключитьОбработчикОжидания("Подключаемый_Периодический_ОбновитьИнформацию", 60);
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если ЗначениеЗаполнено(ИдентификаторЗаданияЗагрузитьПрогноз) Тогда
		ИдентификаторыЗаданий.Добавить(ИдентификаторЗаданияЗагрузитьПрогноз);
	КонецЕсли;
	Если ЗначениеЗаполнено(ИдентификаторЗаданияОбновитьПрогноз) Тогда
		ИдентификаторыЗаданий.Добавить(ИдентификаторЗаданияОбновитьПрогноз);
	КонецЕсли;
	Если ЗначениеЗаполнено(ИдентификаторЗаданияОбновитьБаланс) Тогда
		ИдентификаторыЗаданий.Добавить(ИдентификаторЗаданияОбновитьБаланс);
	КонецЕсли;
	Если ЗначениеЗаполнено(ИдентификаторЗаданияОбновитьДоступностьТестовыхПериодов) Тогда
		ИдентификаторыЗаданий.Добавить(ИдентификаторЗаданияОбновитьДоступностьТестовыхПериодов);
	КонецЕсли;
	// Формирование отчета
	Если ЗначениеЗаполнено(ФоновоеЗаданиеИдентификатор) Тогда
		ИдентификаторыЗаданий.Добавить(ФоновоеЗаданиеИдентификатор);
	КонецЕсли;
	
	ОтменитьВыполнениеЗаданий(ИдентификаторыЗаданий);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "СнятиеБлокировки" Тогда
		// Сервис прогнозирования.
		ТребуетсяЗановоПодключитьсяВСервисе = Ложь;
	ИначеЕсли ИмяСобытия = "ЗапуститьОбучение" Тогда
		// Сервис прогнозирования.
		ПолучитьНастройкиСервиса();
		Если Параметр = "БезРасчетаЗатрат"
			И ТекущаяОшибка.Запрос = "ПолучениеСтоимости" Тогда
			ОбновитьВидимостьОшибокНаСервере(, Истина);
		Иначе
			ОбновитьВидимостьОшибокНаКлиенте();
		КонецЕсли;
		Если ЗначениеЗаполнено(ВидПлана)
			И (ПустаяСтрока(Параметр)
				Или Параметр = "БезРасчетаЗатрат") Тогда
			ОбновитьПрогнозНаСервере();
			Если Не ПустаяСтрока(ИдентификаторЗаданияОбновитьПрогноз) Тогда
				ПодключитьОбработчикОжиданияОбновитьПрогноз();
			Иначе
				Подключаемый_ИндикаторВыгрузки();
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли ИмяСобытия = "ВыборМоделиПрогнозирования" Тогда
		...
		Если Параметр.Свойство("ИмяВСервисе")
			...
			Если Не ПустаяСтрока(ИдентификаторЗаданияЗагрузитьПрогноз) Тогда
				ПодключитьОбработчикОжиданияЗагрузитьПрогноз();
			КонецЕсли;
			
		КонецЕсли;
		
	ИначеЕсли ИмяСобытия = "ПрерываниеОбученияМодели" Тогда
		// Сервис прогнозирования.
		Если ЗначениеЗаполнено(ВидПлана) Тогда
			ОбновитьИнформациюПоСценарию(Истина);
		КонецЕсли;
		
	ИначеЕсли ИмяСобытия = "ОчиститьИнформациюОТехническойОшибке" Тогда
		// Сервис прогнозирования.
		ОбновитьВидимостьОшибокНаСервере(, Истина);
	...
	ИначеЕсли ИмяСобытия = "СозданиеСценарияПрогнозированияИзМастера" Тогда
		СценарийПрогнозирования = Параметр.НовыйСценарий;
	ИначеЕсли ИмяСобытия = "СозданиеВидаПланаИзМастера" Тогда
...
#Область СервисПрогнозированияОбработчикиСобытийЭлементовШапкиФормы
...
Процедура ОшибкиОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ПользовательскиеОшибки" Тогда
		
		РезультатПроверкиНастроек = РезультатПроверкиКорректностиНастроекСервисаНаСервере(Ложь);
		Если РезультатПроверкиНастроек.ЕстьПользовательскиеОшибки Тогда
			
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("ПереданныеОшибки", РезультатПроверкиНастроек);
			ПараметрыФормы.Вставить("Сценарий",         СценарийПрогнозирования);
			ПараметрыФормы.Вставить("ВидПлана",         ВидПлана);
			ОткрытьФорму("Обработка.ПанельУправленияСервисомПрогнозирования.Форма.ОтчетПоПроблемам",
				ПараметрыФормы,
				ЭтотОбъект,
				,
				,
		...
		Иначе
			ОчиститьСообщения();
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Настройки сервиса были исправлены'"));
			ОбновитьВидимостьОшибокНаКлиенте(Ложь);
		КонецЕсли;
		
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ТехническаяОшибка" Тогда
		
		Если ПустаяСтрока(ТекущаяОшибка.Запрос) Тогда
			ОчиститьСообщения();
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Ошибка запроса была исправлена'"));
			ОбновитьВидимостьОшибокНаКлиенте();
			Возврат;
		КонецЕсли;
		
		ОткрытьФорму("Обработка.ПанельУправленияСервисомПрогнозирования.Форма.ОтчетПоПроблемам",
			Новый Структура("ПереданныеОшибки", ТекущаяОшибка),
			ЭтотОбъект,
			,
			,
			,
...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ОбработатьДанныеРасшифровки(Расшифровка, СтандартнаяОбработка); // Сервис прогнозирования.
...
Процедура СтатусОбученияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ЗапуститьОбучение" Тогда
		...
		Если ИдетВыгрузкаДанных Тогда
			ТекстОшибки = НСтр("ru='Невозможно запустить обучение поскольку в данный момент идет выгрузка данных на сервер.'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки);
			Возврат;
		КонецЕсли;
		...
		РезультатПроверкиНастроек = РезультатПроверкиКорректностиНастроекСервисаНаСервере(Ложь);
		Если РезультатПроверкиНастроек.ЕстьПользовательскиеОшибки Тогда
			ОбновитьВидимостьОшибокНаКлиенте(Истина);
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("ПереданныеОшибки", РезультатПроверкиНастроек);
			ПараметрыФормы.Вставить("Сценарий",         СценарийПрогнозирования);
			ПараметрыФормы.Вставить("ВидПлана",         ВидПлана);
		...
		ОткрытьФорму("Обработка.ПанельУправленияСервисомПрогнозирования.Форма.РасчетЗатратНаПостроениеПрогноза",
			ПараметрыФормы,
			ЭтотОбъект,
			,
			,
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ПрерватьОбучение" Тогда
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Сценарий", СценарийПрогнозирования);
		ПараметрыФормы.Вставить("ВидПлана", ВидПлана);
		ОткрытьФорму("Обработка.ПанельУправленияСервисомПрогнозирования.Форма.ВопросПрерыванияОбученияМодели",
			ПараметрыФормы,
			ЭтотОбъект,
			,
			,
...
#Область СервисПрогнозированияОбработчикиКомандФормы
...
Процедура Подключаемый_ИндикаторВыгрузки()
	...
	Иначе
		ПодключитьОбработчикОжидания("Подключаемый_ИндикаторВыгрузкиЗавершение", 2, Истина);
	КонецЕсли;
	УстановитьВидимостьДоступность();
...
&НаКлиенте
Процедура Подключаемый_ИндикаторВыгрузкиЗавершение()
	ВидПланаПриИзмененииНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_Периодический_ОбновитьИнформацию()
	
	// Метод-обертка чтобы периодический вызов работал независимо всегда
	// и для него не сбрасывался таймер запуска при попытке запуска принудительно 1 раз.
	Подключаемый_ОбновитьИнформацию();
...
Процедура Подключаемый_ОбновитьИнформацию()
	...
	ПодключитьОбработчикОжидания("Подключаемый_ОбновитьСтатусПодключенияИБаланс", 0.1, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ОбновитьСтатусПодключенияИБаланс()
	
	ДлительнаяОперация = НачатьПолучениеСтатусаПодключенияИБаланса(УникальныйИдентификатор);
	Если ДлительнаяОперация = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ДлительнаяОперация.Статус <> "Ошибка" Тогда
		ИдентификаторЗаданияОбновитьБаланс = ДлительнаяОперация.ИдентификаторЗадания;
		
		НастройкиОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
		НастройкиОжидания.ВыводитьОкноОжидания = Ложь;
		Обработчик = Новый ОписаниеОповещения("ПослеПолученияСтатусаПодключенияИБаланса", ЭтотОбъект);
...
Асинх Процедура Сформировать()
	...
	// Сервис прогнозирования.
	Если Не АктивноПодключениеИУстановленыПараметрыОтчета() Тогда
		Возврат;
	КонецЕсли;
...
#Область СервисПрогнозированияСлужебныеПроцедурыИФункцииСерверные
...
&НаСервере
Функция АктивноПодключениеКСервису()
...
&НаСервере
Функция АктивноПодключениеИУстановленыПараметрыОтчета()
	
	Если Не АктивноПодключениеКСервису() Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура УстановитьПараметрыОтчета()
	...
	КлючиВариантовДляВыводаФактовПродаж = Новый Массив();
	КлючиВариантовДляВыводаФактовПродаж.Добавить("ФактПрогноз");
	КлючиВариантовДляВыводаФактовПродаж.Добавить("ФактПрогнозРаздельно");
	КлючиВариантовДляВыводаФактовПродаж.Добавить("ФактПрогнозПоКатегориям");
	КлючиВариантовДляВыводаФактовПродаж.Добавить("ФактПрогнозПоКатегориямРаздельно");
	
	ИспользоватьПериодыПродаж = КлючиВариантовДляВыводаФактовПродаж.Найти(КлючТекущегоВарианта) <> Неопределено;
	...
	Если ЗначениеЗаполнено(ВидПлана) Тогда
		...
		УстановитьПараметрДанныхКомпоновки("ДатаОкончанияВыводаФактаПродаж", ДатаНачалаПрогнозирования, ИспользоватьПериодыПродаж);
		УстановитьПараметрДанныхКомпоновки("ДатаНачалаВыводаФактаПродаж",    ДобавитьМесяц(ДатаНачалаПрогнозирования, -1), ИспользоватьПериодыПродаж);
...
&НаСервере
Процедура ОбновитьВидимостьОшибокНаСервере(ЕстьПользовательскиеОшибки = Неопределено, ОчиститьТехническуюОшибку = Ложь)
	
	// Неопределено при обновлении по технической ошибке.
	Если ЕстьПользовательскиеОшибки <> Неопределено Тогда
		Если ЕстьПользовательскиеОшибки Тогда
			Элементы.ПользовательскиеОшибки.Видимость = Истина;
			ШаблонСтроки = "<a href=""ПользовательскиеОшибки"">%1</a>";
			ТекстОшибки  = НСтр("ru='Обнаружены некорректные настройки'");
			ПользовательскиеОшибки = СтроковыеФункции.ФорматированнаяСтрока(ШаблонСтроки, ТекстОшибки);
		Иначе
			Элементы.ПользовательскиеОшибки.Видимость = Ложь;
		КонецЕсли;
	КонецЕсли;
	
	Если ОчиститьТехническуюОшибку Тогда
		СервисПрогнозирования.ОчиститьТехническуюОшибку();
		Элементы.ТехническаяОшибка.Видимость = Ложь;
	Иначе
		ЕстьТехническаяОшибка = Не ПустаяСтрока(ТекущаяОшибка.Запрос);
		Если ЕстьТехническаяОшибка Тогда
			ШаблонСтроки = "<a href=""ТехническаяОшибка"">%1</a>";
			ТекстОшибки  = НСтр("ru='Произошла ошибка'");
			ТехническаяОшибка = СтроковыеФункции.ФорматированнаяСтрока(ШаблонСтроки, ТекстОшибки);
			
			Если ЗначениеЗаполнено(ТекущаяОшибка.ВидПлана) Тогда
				// Ошибка по выбранному виду плана.
				Элементы.ТехническаяОшибка.Видимость = ТекущаяОшибка.ВидПлана = ВидПлана;
			Иначе
	...
	Элементы.ГруппаОшибок.Видимость = Элементы.ПользовательскиеОшибки.Видимость Или Элементы.ТехническаяОшибка.Видимость;
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьИнформациюПоСценарию(ПрерватьОбучение = Ложь)
	
	Если Не АктивноПодключениеКСервису() Тогда
		Возврат;
	КонецЕсли;
	
	Статусы = СервисПрогнозирования.СтатусОбученияМодели(СценарийПрогнозирования, ВидПлана, ПрерватьОбучение,
...
&НаКлиенте
Процедура ПодключитьОбработчикОжиданияЗагрузитьПрогноз()
	
	ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
	ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ЗагрузитьПрогнозОповещение", ЭтотОбъект);

	ДлительныеОперацииКлиент.ОжидатьЗавершение(
		РезультатЗаданияЗагрузкаДанныхИзСервиса, ОповещениеОЗавершении, ПараметрыОжидания);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузитьПрогнозОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		ЗагрузитьПрогнозНаСервереЗавершение();
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ЗагрузитьПрогнозНаСервереЗавершение()
	
	ИдентификаторЗаданияЗагрузитьПрогноз = "";
	ОбновитьИнформациюПоСценарию();
	ОбновитьВидимостьОшибокНаСервере();
...
Процедура ЗагрузитьДанныеИзСервисаНаСервере(ИмяМодели = Неопределено)
	...
	Если ЗначениеЗаполнено(ВидПлана) Тогда
		...
		ИначеЕсли РезультатРасчета.Статус = "Выполнено" Тогда
			АдресХранилищаЗагрузитьПрогноз = РезультатРасчета.АдресРезультата;
			ЗагрузитьПрогнозНаСервереЗавершение();
		ИначеЕсли РезультатРасчета.Статус = "Ошибка" Тогда
			// Возникла платформенная ошибка или фильтры на загрузку исключили все данные прогноза.
			ВызватьИсключение РезультатРасчета.КраткоеПредставлениеОшибки;
		КонецЕсли;
		
		РезультатЗаданияЗагрузкаДанныхИзСервиса = РезультатРасчета;
...
Процедура ОбновитьПрогнозНаСервере()
	...
	Если ЗначениеЗаполнено(ВидПлана) Тогда
		...
		РезультатЗаданияОбновлениеПрогноза = РезультатРасчета;
		...
		ИначеЕсли РезультатРасчета.Статус = "Выполнено" Тогда
			АдресХранилищаОбновитьПрогноз = РезультатРасчета.АдресРезультата;
			ОбновитьПрогнозНаСервереЗавершение();
		КонецЕсли;
		ОбновитьИнформациюПоСценарию();
...
&НаКлиенте
Процедура ПодключитьОбработчикОжиданияОбновитьПрогноз()
	
	ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
	ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбновитьПрогнозОповещение", ЭтотОбъект);

	ДлительныеОперацииКлиент.ОжидатьЗавершение(
		РезультатЗаданияОбновлениеПрогноза, ОповещениеОЗавершении, ПараметрыОжидания);
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьПрогнозОповещение(Результат, ДополнительныеПараметры) Экспорт
...
&НаСервере
Процедура ОбновитьПрогнозНаСервереЗавершение()
...
Процедура ВидПланаПриИзмененииНаСервере()
	...
	РезультатПроверкиНастроек = РезультатПроверкиКорректностиНастроекСервисаНаСервере(Истина);
	ОбновитьВидимостьОшибокНаСервере(РезультатПроверкиНастроек.ЕстьПользовательскиеОшибки);
...
&НаСервере
Функция РезультатПроверкиКорректностиНастроекСервисаНаСервере(ПрерватьНаПервойОшибке)
	
	// Функция-прослойка.
	Возврат СервисПрогнозирования.РезультатПроверкиКорректностиНастроекСервиса(
		ВидПлана, ТипПлана, СценарийПрогнозирования, ПрерватьНаПервойОшибке);
...
Функция ОтладочнаяИнформацияПоВидуПлана()
	...
	ТаблицаМоделейСтрокой = СервисПрогнозирования.ПредставитьТаблицуСтрокой(ИнформацияОПоследнихМоделях, Истина);
	ПараметрыШаблона.Добавить(ТаблицаМоделейСтрокой);
	
	// Общая информация.
	ДатаОкончанияПродаж = СервисПрогнозированияПереопределяемый.ПолучитьДатыНачалаОкончанияПродаж(НастройкиСервиса, Ложь).ДатаОкончанияПродаж;
...
&НаСервере
Процедура ОбработатьДанныеРасшифровки(Расшифровка, СтандартнаяОбработка)
	
	Данные = ПолучитьИзВременногоХранилища(ОтчетДанныеРасшифровки); // ДанныеРасшифровкиКомпоновкиДанных.
	ЭлементыРасшифровки = Данные.Элементы; // ЭлементыРасшифровкиКомпоновкиДанных.
	ЭлементРасшифровки = ЭлементыРасшифровки.Получить(Расшифровка);
	Поля = ЭлементРасшифровки.ПолучитьПоля();
	Если Поля.Количество() > 0 Тогда
		Если Не ЗначениеЗаполнено(Поля[0].Значение)
			Или ТипЗнч(Поля[0].Значение) = Тип("Число") Тогда
			СтандартнаяОбработка = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьИнформациюОВыгрузкеКоллекций()
	...
	Иначе
		ИндикаторВыгрузки = 0;
	КонецЕсли;
	ПодсказкаПоУмолчанию = НСтр("ru = 'Начинается выгрузка данных...'");
	ШаблонПодсказки = НСтр("ru = 'Выгружается коллекция: %1, %2 из %3 записей.'");
	ИндикаторВыгрузкиПодсказкаВыгружаетсяКоллекция = СтроковыеФункции.ФорматированнаяСтрока(ШаблонПодсказки,
		ВыгружаемаяСейчасКоллекцияПредставление,
		СводкаОбменаДанными.ВыгруженоОбъектовКоллекции,
		СводкаОбменаДанными.ЗапланированоВыгрузитьОбъектовКоллекции);
	Если ПустаяСтрока(ВыгружаемаяСейчасКоллекцияПредставление) Тогда
		Элементы.ИндикаторВыгрузки.Подсказка = ПодсказкаПоУмолчанию;
	Иначе
		Элементы.ИндикаторВыгрузки.Подсказка = ИндикаторВыгрузкиПодсказкаВыгружаетсяКоллекция;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция НачатьПолучениеТестовогоПериодаНаСервера(УникальныйИдентификатор)
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияФункции(УникальныйИдентификатор);
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, "Обработки.ПанельУправленияСервисомПрогнозирования.ПодключениеТестовогоПериодаДоступно");
	
КонецФункции

&НаСервереБезКонтекста
Функция НачатьПолучениеСтатусаПодключенияИБаланса(УникальныйИдентификатор)
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияФункции(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение статуса подключения и баланса в фоне'");
	ПараметрыВыполнения.ОжидатьЗавершение = 0;
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, "СервисПрогнозирования.ПолучитьСтатусПодключенияКСервису");
...
#Область СервисПрогнозированияСлужебныеПроцедурыИФункцииКлиентские

&НаКлиенте
Процедура ПослеПолученияСтатусаПодключенияИБаланса(ДлительнаяОперация, Контекст) Экспорт
	...
	Если ДлительнаяОперация.Статус <> "Выполнено" Тогда
		Возврат;
	КонецЕсли;
	ИдентификаторЗаданияОбновитьБаланс = "";
	
	ДанныеОтвета = ПолучитьИзВременногоХранилища(ДлительнаяОперация.АдресРезультата);
	
	ПолучитьНастройкиСервиса();
	ОбновитьВидимостьОшибокНаКлиенте();
	Если ПустаяСтрока(ДанныеОтвета.ТекстОшибки) Тогда
		ОбновитьПредставлениеБалансаНаКлиенте(ДанныеОтвета.ДесериализованноеЗначение["status"]);
	Иначе
		ОбновитьПредставлениеБалансаНаКлиенте("Ошибка");
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьПредставлениеБалансаНаКлиенте(СтатусПодключения)
	
	// Расчет состояния в фоне, используя функцию СервисПрогнозирования.ПолучитьСтатусПодключенияКСервису().
	...
	Если СтатусПодключения = Неопределено Тогда
		...
		ПоказатьСтраницуБиллинга(Элементы.ГруппаПроверкаСтатусаПодключения);
		Возврат;
	КонецЕсли;
	...
	Если СтатусПодключения = "Ошибка" Тогда
		Элементы.Биллинг.Картинка = БиблиотекаКартинок.Предупреждение;
		Элементы.ГруппаБиллингПодробно.ЦветТекстаЗаголовка = WebЦвета.Красный;
		БалансЗаголовок = НСтр("ru='Не удалось обновить баланс'");
		ПоказатьСтраницуБиллинга(Элементы.ГруппаОшибка);
		Возврат;
	КонецЕсли;
	
	Если СтатусПодключения = "pending" Тогда
		Элементы.Биллинг.Картинка = БиблиотекаКартинок.ОжидаетВыполнения;
		БалансЗаголовок = НСтр("ru = 'Заявка отправлена'");
		ПоказатьСтраницуБиллинга(Элементы.ГруппаНеАктивирован);
	ИначеЕсли СтатусПодключения = "active" Тогда
		
		Если Баланс > 0 Тогда
			Элементы.Биллинг.Картинка = БиблиотекаКартинок.Информация16;
			БалансЗаголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Баланс: %1'"),
				Баланс);
			
			Если ЗначениеЗаполнено(ДатаОкончанияМаксимальногоТарифа) Тогда
				ШаблонСтроки = НСтр("ru = 'Доступно по тарифу: <b>%1</b> единиц вычислительной мощности до <b>%2</b>.'");
				Элементы.ТекстБалансПоложительный.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
					ШаблонСтроки,
					Баланс,
					Формат(ДатаОкончанияМаксимальногоТарифа, "ДЛФ=D"));
			Иначе
				ШаблонСтроки = НСтр("ru = 'Доступно по тарифу: <b>%1</b> единиц вычислительной мощности.'");
				Элементы.ТекстБалансПоложительный.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
					ШаблонСтроки,
					Баланс);
			КонецЕсли;
			ПоказатьСтраницуБиллинга(Элементы.ГруппаБалансПоложительный);
		ИначеЕсли Баланс < 0 Тогда
			Элементы.Биллинг.Картинка = БиблиотекаКартинок.Предупреждение;
			Элементы.ГруппаБиллингПодробно.ЦветТекстаЗаголовка = WebЦвета.Красный;
			БалансЗаголовок = НСтр("ru = 'Предоставлен кредит'");
			
			ШаблонСтроки = НСтр("ru = '<a style=""color: ЦветТекстаОтрицательныхЗначений"">Оплаченные тарифы закончились.</a>
				|Предоставлен кредит до <b>%1</b>.
				|Пожалуйста, приобретите тарифы для пополнения баланса.'");
			Элементы.ТекстБалансОтрицательный.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
				ШаблонСтроки,
				Формат(ДатаОкончанияМаксимальногоТарифа, "ДЛФ=D"));
			ПоказатьСтраницуБиллинга(Элементы.ГруппаБалансОтрицательный);
		Иначе // Нулевой баланс.
			Элементы.Биллинг.Картинка = БиблиотекаКартинок.Информация16;
			БалансЗаголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Баланс: %1'"),
				Баланс);
			
			ПоказатьСтраницуБиллинга(Элементы.ГруппаБалансНулевой);
		КонецЕсли;
		
	ИначеЕсли СтатусПодключения = "nomoney" Тогда
		Элементы.Биллинг.Картинка = БиблиотекаКартинок.Предупреждение;
		Элементы.ГруппаБиллингПодробно.ЦветТекстаЗаголовка = WebЦвета.Красный;
		БалансЗаголовок = НСтр("ru = 'Сервис отключен'");
		
		ШаблонСтроки = НСтр("ru = '<a style=""color: ЦветТекстаОтрицательныхЗначений"">Возможность прогнозирования приостановлена.</a>
			|Долг: <b>%1</b> единиц.
			|Пожалуйста, приобретите тарифы для пополнения баланса.'");
		Элементы.ТекстСервисОтключен.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			ШаблонСтроки,
			650);
		ПоказатьСтраницуБиллинга(Элементы.ГруппаСервисОтключен);
	КонецЕсли;
	
	Элементы.ГруппаБиллингПодробно.Заголовок = БалансЗаголовок;
...
&НаКлиенте
Процедура ОбновитьВидимостьОшибокНаКлиенте(ЕстьПользовательскиеОшибки = Неопределено)
	...
	Если ЕстьПользовательскиеОшибки <> Неопределено Тогда
		Если ЕстьПользовательскиеОшибки Тогда
			...
			ШаблонСтроки = "<a href=""ПользовательскиеОшибки"">%1</a>";
			ТекстОшибки  = НСтр("ru='Обнаружены некорректные настройки'");
			ПользовательскиеОшибки = СтроковыеФункцииКлиент.ФорматированнаяСтрока(ШаблонСтроки, ТекстОшибки);
		Иначе
			Элементы.ПользовательскиеОшибки.Видимость = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьТехническаяОшибка Тогда
		ШаблонСтроки = "<a href=""ТехническаяОшибка"">%1</a>";
		ТекстОшибки  = НСтр("ru='Произошла ошибка'");
		ТехническаяОшибка = СтроковыеФункцииКлиент.ФорматированнаяСтрока(ШаблонСтроки, ТекстОшибки);
...
&НаКлиенте
Процедура ОбновитьВидимостьТестовыхПериодов()
	
	// Недоступно подключение тестовых периодов.
	Если Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПодключениеСервисовСопровождения") Тогда
		Элементы.ПодключитьТестовыйПериод.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	
	ДлительнаяОперация = НачатьПолучениеТестовогоПериодаНаСервера(УникальныйИдентификатор);
	Если ДлительнаяОперация = Неопределено
		Или ДлительнаяОперация.Статус = "Ошибка" Тогда
		Возврат;
	КонецЕсли;
	ИдентификаторЗаданияОбновитьДоступностьТестовыхПериодов = ДлительнаяОперация.ИдентификаторЗадания;
	
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ПослеОбновленияДоступностиТестовыхПериодов", ЭтотОбъект);
	ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
...
&НаКлиенте
Процедура ПослеОбновленияДоступностиТестовыхПериодов(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не ТипЗнч(Результат) = Тип("Структура")
		...
		Возврат;
	КонецЕсли;
	ИдентификаторЗаданияОбновитьДоступностьТестовыхПериодов = "";
	
	// ДанныеРезультата["1C-Forecast"] - Булево.
	ДанныеРезультата = ПолучитьИзВременногоХранилища(Результат.АдресРезультата);
	
	Элементы.ПодключитьТестовыйПериод.Видимость = 
		ДанныеРезультата.Получить(СервисПрогнозированияПереопределяемыйКлиентСервер.ИдентификаторСервиса());
...
#Область СервисПрогнозированияСлужебныеПроцедурыИФункцииТексты
