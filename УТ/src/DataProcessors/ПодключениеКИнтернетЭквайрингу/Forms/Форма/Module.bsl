...
Функция НоваяНастройкаПодключения()
	...
	Иначе
		...
		ПроверитьНеобходимостьПривязкиОнлайнЗаказов(Настройки);
	КонецЕсли;
...
&НаСервере
Процедура ПроверитьНеобходимостьПривязкиОнлайнЗаказов(Настройки)
	...
	ОнлайнЗаказыОрганизация = ОрганизацияНастройки(Настройки);
	Если Не ЗначениеЗаполнено(ОнлайнЗаказыОрганизация) Тогда
		...
		Возврат;
	Иначе
		МодульНастройкиПодключенияКОнлайнЗаказам = ОбщегоНазначения.ОбщийМодуль("Справочники.НастройкиПодключенияКОнлайнЗаказам");
		НастройкиОрганизации = МодульНастройкиПодключенияКОнлайнЗаказам.НастройкиОрганизации(ОнлайнЗаказыОрганизация);
...
&НаСервере
Функция ОрганизацияНастройки(Настройки)
	
	ПоискОрганизации = Неопределено;
	ОтключитьЛогикуБИП = Ложь;
	
	ИнтеграцияПодсистемБИП.ПриОпределенииОрганизацииЭквайринг(
		ПоискОрганизации,
		Настройки.ПрикладныеНастройки,
		ОтключитьЛогикуБИП);
	ИнтернетЭквайрингПереопределяемый.ПриОпределенииОрганизации(
		ПоискОрганизации,
		Настройки.ПрикладныеНастройки,
		ОтключитьЛогикуБИП);
...
Процедура СоздатьНастройкуОнлайнЗаказы()
	...
	НастройкиОплат.Вставить(
		МодульОнлайнЗаказыКлиент.СпособОплатыИнтернетЭквайринг(),
		НастройкаПодключения);
...
Процедура СоздатьНастройкуОнлайнЗаказыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Статус = "Выполнено" Тогда
		...
		РезультатОперации = ПолучитьИзВременногоХранилища(Результат.АдресРезультата); // см. Справочники.НастройкиПодключенияКОнлайнЗаказам.НоваяНастройкаПодключения
...
Процедура ОбработкаНавигационныхСсылок(
	...
	Иначе
		НастройкиПлатежныхСистемКлиентСобытия.ПриОбработкеНавигационнойСсылки(
			НавигационнаяСсылкаФорматированнойСтроки,
			СтандартнаяОбработка,
			НастройкаПодключения);
...
&НаСервереБезКонтекста
Процедура ЗаписатьИнформациюВЖурналРегистрации(Знач Сообщение, Знач Ошибка = Истина)
