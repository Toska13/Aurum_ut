...
Функция ОбработатьШтрихкодыСервер(ДанныеШтрихкодов,КэшированныеЗначения, ИдентификаторТекущейСтроки)
	...
	Если ШтрихкодыПоТипам.БезТипа.Количество() > 0 Тогда
		...
		ТаблицаШтрихКодов.Колонки.Добавить("Количество",ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля(ДопустимыйЗнак.Неотрицательный));
		Для Каждого СтрМас Из ШтрихкодыПоТипам.БезТипа Цикл
			...
			ПоляИзШтрихкода = ШтрихкодированиеНоменклатурыСервер.ИнформацияОСерииИзШтрихкода(СтрМас.Штрихкод, 
		...
		Если Не ЕстьОшибки Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если НайденныеСтроки.Количество() = 0 Тогда
					...
					ОбрабатываемыеСтроки.Добавить(ОбрабатываемаяСтрока);
					Модифицированность   = Истина;
...
Процедура ВыполнитьЗапросЗаполненияТаблицыОстатков()
	...
	ПараметрыЗапроса.Вставить("КонтролируемаяПартия", КонтролируемаяПартия);
	...
	Для Каждого ДополнительныйПараметр Из ПараметрыУказанияСерий.ПараметрыПолученияДанныхИзРегистров Цикл
		ПараметрыЗапроса.Вставить(ДополнительныйПараметр.Ключ, ДополнительныйПараметр.Значение);
	КонецЦикла;
...
Функция ДобавитьСтрокиНаСервере(Результат)
	...
	ПараметрыПересчетаКоличества = Новый Структура("Упаковка, Номенклатура", Упаковка, ЗначенияПолейСвязи.Номенклатура);
	СтруктураДействий = Новый Структура("ПересчитатьКоличествоЕдиниц", ПараметрыПересчетаКоличества);
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ПараметрыСтроки, СтруктураДействий, Неопределено);
