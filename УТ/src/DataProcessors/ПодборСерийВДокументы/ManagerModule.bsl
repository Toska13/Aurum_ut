...
Функция ТекстЗапросаФормированияТаблицыДанныеРегистров(ВариантПолучениеДанныхИзРегистров, СкладИлиПодразделение = Неопределено) Экспорт
	...
	Если ВариантПолучениеДанныхИзРегистров = "ЗаказНакладнаяСерииИзОстатка" Тогда
		...
		Если СкладИлиПодразделение = Неопределено ИЛИ ТипЗнч(СкладИлиПодразделение) <> Тип("СправочникСсылка.СтруктураПредприятия") Тогда
			...
			|					И Склад = &Склад
			|					И КонтролируемаяПартия = &КонтролируемаяПартия) КАК ТоварыНаСкладахОстатки
			|	
			|	ОБЪЕДИНИТЬ ВСЕ
			|	
			|	ВЫБРАТЬ
			...
			|					И КонтролируемаяПартия = &КонтролируемаяПартия) КАК ТоварыКОтгрузкеОстатки
			|	
			|	ОБЪЕДИНИТЬ ВСЕ
			|	
			|	ВЫБРАТЬ
			...
			|		И ТоварыНаСкладах.КонтролируемаяПартия = &КонтролируемаяПартия
			...
			|		И РезервыСерий.КонтролируемаяПартия = &КонтролируемаяПартия
	...
	ИначеЕсли ВариантПолучениеДанныхИзРегистров = "НакладнаяСерииИзЗаказа" Тогда
		...
		|		И РезервыСерий.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|		И РезервыСерий.КонтролируемаяПартия = &КонтролируемаяПартия) КАК ДанныеРегистров
		|
		|СГРУППИРОВАТЬ ПО
		|	ДанныеРегистров.Серия";
	...
	ИначеЕсли ВариантПолучениеДанныхИзРегистров = "ТоварыКОформлениюПоступления" Тогда
		...
		|					И Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|					И КонтролируемаяПартия = &КонтролируемаяПартия) КАК ТоварыКОформлениюПоступленияОстатки
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|		И ТоварыКОформлениюПоступления.КонтролируемаяПартия = &КонтролируемаяПартия
		|) КАК ДанныеРегистров
		|
		|СГРУППИРОВАТЬ ПО
		|	ДанныеРегистров.Серия";
	...
	ИначеЕсли ВариантПолучениеДанныхИзРегистров = "ЗаказНакладнаяСерииПоОстаткамКомитента" Тогда
		...
		|ГДЕ
		|	Аналитики.Номенклатура = &Номенклатура
		|	И Аналитики.Характеристика = &Характеристика
		|	И Аналитики.МестоХранения = &Склад
		|	И (Аналитики.Назначение = &Назначение
		|	ИЛИ Аналитики.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|	И НЕ ВЫРАЗИТЬ(&Назначение КАК Справочник.Назначения).ДвиженияПоСкладскимРегистрам)
		|	И Аналитики.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|		ТоварыОрганизацийОстатки.КоличествоОстаток КАК СвободныйОстаток
		|	ИЗ
		|		РегистрНакопления.ТоварыОрганизаций.Остатки(, (АналитикаУчетаНоменклатуры, Организация, ВидЗапасов) В
		|			(ВЫБРАТЬ
		|				Аналитика.Ссылка,
		|				Аналитика.Организация,
		|				Аналитика.ВидЗапасов
		|			ИЗ
		|				ВТАналитикиУчетаНоменклатуры КАК Аналитика)) КАК ТоварыОрганизацийОстатки
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		|	ВЫБРАТЬ
		...
		|			ЛЕВОЕ СОЕДИНЕНИЕ ВТАналитикиУчетаНоменклатуры КАК Аналитика
		|			ПО ТоварыОрганизацийОстатки.АналитикаУчетаНоменклатуры = Аналитика.Ссылка
		|			И ТоварыОрганизацийОстатки.Организация = Аналитика.Организация
		|			И ТоварыОрганизацийОстатки.ВидЗапасов = Аналитика.ВидЗапасов
		|	ГДЕ
		|		НЕ Аналитика.Ссылка ЕСТЬ NULL
		|		И ТоварыОрганизацийОстатки.Регистратор = &Регистратор
		|		И ТоварыОрганизацийОстатки.АналитикаУчетаНоменклатуры.Серия
		|			<> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)) КАК
		|		ДанныеРегистров
		|СГРУППИРОВАТЬ ПО
		|	ДанныеРегистров.Серия";
...
Функция ВариантПолучениеДанныхИзРегистровПоПараметрамФормы(ПараметрыУказанияСерий, Распоряжение, Склад, ВидНоменклатуры) Экспорт
	...
	ИначеЕсли ПараметрыУказанияСерий.СкладскиеОперации.Найти(Перечисления.СкладскиеОперации.ПриемкаПоВозвратуОтКлиента) <> Неопределено
		И ПараметрыУказанияСерий.ПолноеИмяОбъекта <> Метаданные.Документы.ПоступлениеТоваровОтКлиента.ПолноеИмя()
		И ПараметрыУказанияСерий.ПолноеИмяОбъекта <> Метаданные.Документы.ПоступлениеТоваровОтХранителя.ПолноеИмя() Тогда
	...
	ИначеЕсли ПараметрыУказанияСерий.ПолноеИмяОбъекта = Метаданные.Документы.ВыкупТоваровХранителем.ПолноеИмя()
		Или ПараметрыУказанияСерий.ПолноеИмяОбъекта = Метаданные.Документы.ПоступлениеТоваровОтКлиента.ПолноеИмя()
		Или (ПараметрыУказанияСерий.ПолноеИмяОбъекта = Метаданные.Документы.РеализацияТоваровУслуг.ПолноеИмя()
			И ПараметрыУказанияСерий.ОперацияДокумента = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности)
		...
		Или (ПараметрыУказанияСерий.ПолноеИмяОбъекта = Метаданные.Документы.СписаниеОтгруженныхТоваров.ПолноеИмя()
			И ПараметрыУказанияСерий.ОперацияДокумента = Перечисления.ХозяйственныеОперации.СписаниеНедостачЗаСчетПоклажедателя
				Или ПараметрыУказанияСерий.ОперацияДокумента = Перечисления.ХозяйственныеОперации.СписаниеОтгруженныхТоваров) Тогда
	...
	ИначеЕсли (ПараметрыУказанияСерий.ПолноеИмяОбъекта = Метаданные.Документы.ВозвратТоваровПоставщику.ПолноеИмя()
			И ПараметрыУказанияСерий.ОперацияДокумента = Перечисления.ХозяйственныеОперации.ВозвратТоваровКомитенту) Тогда
		
		ВариантПолучениеДанныхИзРегистров = "ЗаказНакладнаяСерииПоОстаткамКомитента";
...
Функция ЗаголовкиПоВариантуПолучениеДанныхИзРегистров(ВариантПолучениеДанныхИзРегистров) Экспорт
	...
	ИначеЕсли ВариантПолучениеДанныхИзРегистров = "ЗаказНакладнаяСерииПоОстаткамУХранителя" Тогда
		ЗаголовокКнопки            = НСтр("ru = 'Переданные клиенту'");
		ЗаголовокСвободногоОстатка = НСтр("ru = 'Переданные клиенту, %ЕдиницаИзмерения%'");
	...
	ИначеЕсли ВариантПолучениеДанныхИзРегистров = "ЗаказНакладнаяСерииПоОстаткамКомитента" Тогда
		ЗаголовокКнопки            = НСтр("ru = 'У комитента'");
		ЗаголовокСвободногоОстатка = НСтр("ru = 'У комитента, %ЕдиницаИзмерения%'");
	ИначеЕсли ВариантПолучениеДанныхИзРегистров = "ОрдерСерииИзНакладной" Тогда
