...
Функция ДокументыВводаОстатковПодлежащиеОбработке(Действие)
	...
	// Добавление документов СФПолуч, СФВыданный и СФ на аванс, введенных на основании ввода остатков либо Первичных документов сформированных сверткой
	Если Действие = "проведение" Тогда
...
Процедура УдалитьДвиженияРегистровЗадания()
	...
	|ГДЕ
	|	Задания.Документ.Дата <= &ДатаОст";
	ЗаполнитьПараметрыЗапросаДатаСвертки(Запрос);
	ЗаданияОчереди = Запрос.Выполнить().Выгрузить();
	Если ЗаданияОчереди.Количество() > 0 Тогда
		НаборКУдалению = РегистрыСведений.ЗаданияКРаспределениюВзаиморасчетов.СоздатьНаборЗаписей();
		НаборКУдалению.Загрузить(ЗаданияОчереди);
		НаборКУДалению.ОбменДанными.Загрузка = Истина;
	...
	ЗаданияОчереди = Запрос.Выполнить().Выгрузить();
	Если ЗаданияОчереди.Количество() > 0 Тогда
		НаборКУдалению = РегистрыСведений.ЗаданияНеоперативногоДопроведенияДокументов.СоздатьНаборЗаписей();
		НаборКУдалению.Загрузить(ЗаданияОчереди);
		НаборКУДалению.ОбменДанными.Загрузка = Истина;
		НаборКУДалению.Записать(РежимЗамещения.Удаление);
	КонецЕсли;
...
Процедура СформироватьДокументыВводаОстатков(ПовторныйЗапуск = Ложь) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5") Тогда
		СформироватьДокументыВводаОстатковТоварыОтгруженные();
	КонецЕсли;
	...
	// 2.7 Ордерная схема
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5") Тогда
...
Процедура СформироватьДокументыВводаОстатковРаспоряженияНаОтгрузку()
	...
	ПараметрыФормированияТаблицы.КонецПериода = "&ГраницаОст";
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5") Тогда
		...
		|	СУММА(ОстаткиИОбороты.ЗаказаноКоличествоОборот -
		|		ОстаткиИОбороты.ОтгруженоКоличествоОборот -
		|		ОстаткиИОбороты.СписаноДоППСКоличествоОборот) КАК ЗаказаноКонечныйОстаток,
		|	СУММА(ОстаткиИОбороты.ЗаказаноСуммаОборот -
		|		ОстаткиИОбороты.ОтгруженоСуммаОборот-
		|		ОстаткиИОбороты.СписаноДоППССуммаОборот) КАК СуммаКонечныйОстаток,
		|	СУММА(ОстаткиИОбороты.ОтгруженоКоличествоОборот) КАК ЗаказаноРасход
		|ПОМЕСТИТЬ ВТОстаткиИОбороты
		|ИЗ
		|	#РаспоряженияНаОтгрузкуОборотыРазвернутая КАК ОстаткиИОбороты
		|СГРУППИРОВАТЬ ПО
		|	ОстаткиИОбороты.Распоряжение,
		|	ОстаткиИОбороты.Номенклатура,
		|	ОстаткиИОбороты.Характеристика,
		|	ОстаткиИОбороты.КодСтроки
		|ИМЕЮЩИЕ
		|	СУММА(ОстаткиИОбороты.ЗаказаноКоличествоОборот -
		|		ОстаткиИОбороты.ОтгруженоКоличествоОборот -
		|		ОстаткиИОбороты.СписаноДоППСКоличествоОборот) <> 0
		|	ИЛИ СУММА(ОстаткиИОбороты.ЗаказаноСуммаОборот -
		|		ОстаткиИОбороты.ОтгруженоСуммаОборот -
		|		ОстаткиИОбороты.СписаноДоППССуммаОборот) <> 0
		|	ИЛИ СУММА(ОстаткиИОбороты.ОтгруженоКоличествоОборот) <> 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|";
		ПараметрыФормированияТаблицы.Показатели = "Заказано, Отгружено, СписаноДоППС";
	Иначе
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ОстаткиИОбороты.Распоряжение КАК Распоряжение,
		...
		|	ОстаткиИОбороты.КодСтроки КАК КодСтроки,
		|	СУММА(ОстаткиИОбороты.ЗаказаноКоличествоОборот -
		|		ОстаткиИОбороты.ОформленоКоличествоОборот) КАК ЗаказаноКонечныйОстаток,
		|	СУММА(ОстаткиИОбороты.ЗаказаноСуммаОборот -
		|		ОстаткиИОбороты.ОформленоСуммаОборот) КАК СуммаКонечныйОстаток,
		|	СУММА(ОстаткиИОбороты.ОформленоКоличествоОборот) КАК ЗаказаноРасход
		|ПОМЕСТИТЬ ВТОстаткиИОбороты
		|ИЗ
		|	#РаспоряженияНаОтгрузкуОборотыРазвернутая КАК ОстаткиИОбороты
		|СГРУППИРОВАТЬ ПО
		...
		|ИМЕЮЩИЕ
		|	СУММА(ОстаткиИОбороты.ЗаказаноКоличествоОборот -
		|		ОстаткиИОбороты.ОформленоКоличествоОборот) <> 0
		|	ИЛИ СУММА(ОстаткиИОбороты.ЗаказаноСуммаОборот -
		|		ОстаткиИОбороты.ОформленоСуммаОборот) <> 0
		|	ИЛИ СУММА(ОстаткиИОбороты.ОформленоКоличествоОборот) <> 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|";
	...
	|	ТабРег.ЗаказаноРасход КАК ЗаказаноРасход
	|ИЗ
	|	ВТОстаткиИОбороты КАК ТабРег
	|ГДЕ
	|	ТабРег.ЗаказаноКонечныйОстаток > 0
	|	ИЛИ ТабРег.ЗаказаноРасход > 0
	|ИТОГИ
	|	МАКСИМУМ(ТабРег.ЗаказаноКонечныйОстаток),
	|	МАКСИМУМ(ТабРег.СуммаКонечныйОстаток),
	|	МАКСИМУМ(ТабРег.ЗаказаноРасход),
	|	МАКСИМУМ(КодСтроки),
	...
	Пока ВыборкаЗаказ.Следующий() Цикл
		...
		УдалитьДвиженияПоЗаказу("РаспоряженияНаОтгрузкуИВозврат", "Распоряжение", ВыборкаЗаказ.Заказ);
...
Процедура СформироватьДокументыВводаОстатковЗаявкиНаВозврат()
	...
	|	ТабРег.Характеристика КАК Характеристика,
	|	ТабРег.КодСтроки КАК КодСтроки,
	|	ТабРег.Распоряжение.Комментарий КАК Комментарий,
	|	(ТабРег.ЗаявленоНаВозвратКоличествоОборот - ТабРег.ОформленоПоВозвратамКоличествоОборот) КАК ЗаявленоКонечныйОстаток,
	|	ТабРег.ОформленоПоВозвратамКоличествоОборот КАК ЗаявленоРасход
	|ИЗ
	|	#ЗаявкиНаВозвратОборотыРазвернутая КАК ТабРег
	|ГДЕ
	|	(ТабРег.ЗаявленоНаВозвратКоличествоОборот - ТабРег.ОформленоПоВозвратамКоличествоОборот) > 0
	|	ИЛИ ТабРег.ОформленоПоВозвратамКоличествоОборот > 0
	|ИТОГИ
	|	СУММА(ЗаявленоКонечныйОстаток),
	|	СУММА(ЗаявленоРасход)
	|ПО
	|	Заказ";
	...
	ПараметрыФормированияТаблицы.ТекстУсловия =
		"Распоряжение В
		|		(ВЫБРАТЬ
		|				ОткрытыеРаспоряжения.Распоряжение КАК Распоряжение
		|			ИЗ
		|				РегистрСведений.РаспоряженияНаОтгрузкуИВозвратКВыполнению КАК ОткрытыеРаспоряжения)";
	ПараметрыФормированияТаблицы.Группировка = "Распоряжение, Номенклатура, Характеристика, КодСтроки";
	ПараметрыФормированияТаблицы.Показатели = "ЗаявленоНаВозврат, ОформленоПоВозвратам";
	...
	|	СУММА(ОстаткиИОбороты.ЗаказаноКоличествоОборот -
	|		ОстаткиИОбороты.ОтгруженоКоличествоОборот) КАК ЗаказаноКонечныйОстаток,
	|	СУММА(ОстаткиИОбороты.ЗаказаноСуммаОборот -
	|		ОстаткиИОбороты.ОтгруженоСуммаОборот) КАК СуммаКонечныйОстаток,
	|	СУММА(ОстаткиИОбороты.ОтгруженоКоличествоОборот) КАК ЗаказаноРасход
	|ПОМЕСТИТЬ ВТОстаткиИОбороты
	|ИЗ
	|	#РаспоряженияНаОтгрузкуОборотыРазвернутая КАК ОстаткиИОбороты
	...
	|ИМЕЮЩИЕ
	|	СУММА(ОстаткиИОбороты.ЗаказаноКоличествоОборот -
	|		ОстаткиИОбороты.ОтгруженоКоличествоОборот) <> 0
	|	ИЛИ СУММА(ОстаткиИОбороты.ЗаказаноСуммаОборот -
	|		ОстаткиИОбороты.ОтгруженоСуммаОборот) <> 0
	|	ИЛИ СУММА(ОстаткиИОбороты.ОтгруженоКоличествоОборот) <> 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	Пока Выборка.СледующийПоЗначениюПоля("ДокументВозврат") Цикл
		...
		Если ДокументИзменен Тогда
			...
			Исключение
				...
				ТекстСообщения = СтрШаблон("Не удалось записать документ:
										   |Документ: %1
										   |Причина: %2", ДокОст.Ссылка, 
															ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				
				ЗаписьЖурналаРегистрации("Заполнение себестоимости в документе 'ВозвратТоваровОтКлиента'",
...
Процедура СформироватьДокументыВводаОстатковТоварыОтгруженные()
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	ОстаткиИОбороты.Распоряжение КАК Распоряжение,
	|	ОстаткиИОбороты.Номенклатура КАК Номенклатура,
	|	ОстаткиИОбороты.Характеристика КАК Характеристика,
	|	ОстаткиИОбороты.КодСтроки КАК КодСтроки,
	|	СУММА(ОстаткиИОбороты.ОтгруженоКоличествоОборот -
	|		ОстаткиИОбороты.ОформленоКоличествоОборот) КАК ОтгруженоКонечныйОстаток
	|ПОМЕСТИТЬ ВТОстаткиИОбороты
	|ИЗ #РаспоряженияНаОтгрузкуОборотыРазвернутая КАК ОстаткиИОбороты
	|ГДЕ ОстаткиИОбороты.КодСтроки <> 0
	|СГРУППИРОВАТЬ ПО
	|	ОстаткиИОбороты.Распоряжение,
	|	ОстаткиИОбороты.Номенклатура,
	|	ОстаткиИОбороты.Характеристика,
	...
	|	ОстаткиИОбороты.Характеристика КАК Характеристика,
	|	ОстаткиИОбороты.КодСтроки КАК КодСтроки,
	|	РаспоряжениеОтгрузка.Склад КАК Склад, 
	|	РаспоряжениеОтгрузка.Серия КАК Серия,
	|	ВЫБОР
	|		КОГДА РаспоряжениеОтгрузка.КоличествоОборот < 0
	|			ТОГДА ВЫРАЗИТЬ(РаспоряжениеОтгрузка.Регистратор КАК Документ.ОтгрузкаТоваровКлиенту).ИсправляемыйДокумент
	|		ИНАЧЕ РаспоряжениеОтгрузка.Регистратор
	|	КОНЕЦ КАК Партия,
	|	МАКСИМУМ(ДокСФ.Ссылка) КАК СчетФактураВыданный,
	|	МАКСИМУМ(ОстаткиИОбороты.ОтгруженоКонечныйОстаток) КАК ОтгруженоКонечныйОстаток,
	|	СУММА(РаспоряжениеОтгрузка.КоличествоОборот) КАК КоличествоОтгруженоПоПартии,
	|	СУММА(РаспоряжениеОтгрузка.СуммаОборот) КАК СуммаОтгруженоПоПартии
	|ПОМЕСТИТЬ ВТОтгруженоПоПартии
	|ИЗ РегистрНакопления.РаспоряженияНаОтгрузкуИВозврат.Обороты(,&ГраницаОст,Регистратор, Распоряжение ССЫЛКА Документ.ЗаказКлиента 
	|		И Показатель = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Отгружено)) КАК РаспоряжениеОтгрузка
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	ВТОстаткиИОбороты КАК ОстаткиИОбороты
	|ПО РаспоряжениеОтгрузка.Распоряжение = ОстаткиИОбороты.Распоряжение
	|	И РаспоряжениеОтгрузка.Номенклатура = ОстаткиИОбороты.Номенклатура
	|	И РаспоряжениеОтгрузка.Характеристика = ОстаткиИОбороты.Характеристика
	|	И РаспоряжениеОтгрузка.КодСтроки = ОстаткиИОбороты.КодСтроки
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	Документ.СчетФактураВыданный.ДокументыОснования КАК ДокСФ
	|ПО ДокСФ.ДокументОснование = РаспоряжениеОтгрузка.Регистратор
	|ГДЕ (РаспоряжениеОтгрузка.Регистратор ССЫЛКА Документ.ОтгрузкаТоваровКлиенту
	|		ИЛИ РаспоряжениеОтгрузка.Регистратор ССЫЛКА Документ.ВводОстатковТоваров)
	|	И ОстаткиИОбороты.КодСтроки IS NOT NULL
	|СГРУППИРОВАТЬ ПО
	|	ОстаткиИОбороты.Номенклатура,
	|	ОстаткиИОбороты.Характеристика,
	|	ОстаткиИОбороты.КодСтроки,
	|	РаспоряжениеОтгрузка.Склад, 
	|	РаспоряжениеОтгрузка.Серия,
	|	ВЫБОР
	|		КОГДА РаспоряжениеОтгрузка.КоличествоОборот < 0
	|			ТОГДА ВЫРАЗИТЬ(РаспоряжениеОтгрузка.Регистратор КАК Документ.ОтгрузкаТоваровКлиенту).ИсправляемыйДокумент
	|		ИНАЧЕ РаспоряжениеОтгрузка.Регистратор
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВЫБОР КОГДА ПартияОтгрузкаШапка.Организация IS NULL ТОГДА ПартияВводОстатковШапка.Организация
	|	ИНАЧЕ ПартияОтгрузкаШапка.Организация КОНЕЦ КАК Организация,
	|	ВЫБОР КОГДА ПартияОтгрузкаШапка.Контрагент IS NULL ТОГДА ПартияВводОстатковШапка.Контрагент
	|	ИНАЧЕ ПартияОтгрузкаШапка.Контрагент КОНЕЦ КАК Контрагент,
	|	ВЫБОР КОГДА ПартияОтгрузкаШапка.Партнер IS NULL ТОГДА ПартияВводОстатковШапка.Партнер
	|	ИНАЧЕ ПартияОтгрузкаШапка.Партнер КОНЕЦ КАК Партнер,
	|	ВЫБОР КОГДА ПартияОтгрузкаШапка.Соглашение IS NULL ТОГДА ПартияВводОстатковШапка.СоглашениеСКлиентом
	|	ИНАЧЕ ПартияОтгрузкаШапка.Соглашение КОНЕЦ КАК СоглашениеСКлиентом,
	|	ВЫБОР КОГДА ПартияОтгрузкаШапка.Договор IS NULL ТОГДА ПартияВводОстатковШапка.Договор
	|	ИНАЧЕ ПартияОтгрузкаШапка.Договор КОНЕЦ КАК Договор,
	|	ВЫБОР КОГДА ПартияОтгрузкаШапка.Валюта IS NULL ТОГДА ПартияВводОстатковШапка.Валюта
	|	ИНАЧЕ ПартияОтгрузкаШапка.Валюта КОНЕЦ КАК Валюта,
	|	ВЫБОР КОГДА ПартияОтгрузкаШапка.НалогообложениеНДС IS NULL ТОГДА ПартияВводОстатковШапка.НалогообложениеНДС
	|	ИНАЧЕ ПартияОтгрузкаШапка.НалогообложениеНДС КОНЕЦ КАК НалогообложениеНДС,
	|	ВТОтгруженоПоПартии.Партия КАК Партия,
	|	ПартияОтгрузкаШапка.Дата КАК ПартияДата,
	|	ПартияОтгрузкаШапка.Номер КАК ПартияНомер,
	|	ВТОтгруженоПоПартии.СчетФактураВыданный КАК СчетФактураВыданный,
	|	ВТОтгруженоПоПартии.Склад КАК Склад,
	|	ВТОтгруженоПоПартии.Номенклатура КАК Номенклатура,
	|	ВТОтгруженоПоПартии.Характеристика КАК Характеристика,
	|	ВТОтгруженоПоПартии.Серия КАК Серия,
	|	ВТОтгруженоПоПартии.КодСтроки КАК КодСтроки,
	|	ВТОтгруженоПоПартии.КоличествоОтгруженоПоПартии КАК Количество,
	|	ВТОтгруженоПоПартии.КоличествоОтгруженоПоПартии КАК КоличествоУпаковок,
	|	ВТОтгруженоПоПартии.СуммаОтгруженоПоПартии КАК Сумма,
	|	ЕСТЬNULL(СтавкиНДСНоменклатуры.СтавкаНДС, ЕСТЬNULL(ОсновныеСтавкиНДС.СтавкаНДС, ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка))) КАК СтавкаНДС
	|ИЗ ВТОтгруженоПоПартии
	|ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтгрузкаТоваровКлиенту КАК ПартияОтгрузкаШапка
	|ПО ПартияОтгрузкаШапка.Ссылка = ВТОтгруженоПоПартии.Партия
	|ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВводОстатковТоваров КАК ПартияВводОстатковШапка
	|ПО ПартияВводОстатковШапка.Ссылка = ВТОтгруженоПоПартии.Партия
	|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтавкиНДСНоменклатуры.СрезПоследних(&ДатаОст) КАК СтавкиНДСНоменклатуры
	|ПО ВТОтгруженоПоПартии.Номенклатура = СтавкиНДСНоменклатуры.Номенклатура
	|	И ПартияОтгрузкаШапка.Организация.СтранаРегистрации = СтавкиНДСНоменклатуры.Страна
	|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОсновныеСтавкиНДС.СрезПоследних(&ДатаОст) КАК ОсновныеСтавкиНДС
	|ПО ПартияОтгрузкаШапка.Организация.СтранаРегистрации = ОсновныеСтавкиНДС.Страна
	|ИТОГИ 
	|	МАКСИМУМ(ПартияДата),
	|	МАКСИМУМ(ПартияНомер),
	|	МАКСИМУМ(СчетФактураВыданный),
	|	МАКСИМУМ(КодСтроки)
	|ПО
	|	ВЫБОР КОГДА ПартияОтгрузкаШапка.Организация IS NULL ТОГДА ПартияВводОстатковШапка.Организация
	|	ИНАЧЕ ПартияОтгрузкаШапка.Организация КОНЕЦ,
	|	ВЫБОР КОГДА ПартияОтгрузкаШапка.Контрагент IS NULL ТОГДА ПартияВводОстатковШапка.Контрагент
	|	ИНАЧЕ ПартияОтгрузкаШапка.Контрагент КОНЕЦ,
	|	ВЫБОР КОГДА ПартияОтгрузкаШапка.Партнер IS NULL ТОГДА ПартияВводОстатковШапка.Партнер
	|	ИНАЧЕ ПартияОтгрузкаШапка.Партнер КОНЕЦ,
	|	ВЫБОР КОГДА ПартияОтгрузкаШапка.Соглашение IS NULL ТОГДА ПартияВводОстатковШапка.СоглашениеСКлиентом
	|	ИНАЧЕ ПартияОтгрузкаШапка.Соглашение КОНЕЦ,
	|	ВЫБОР КОГДА ПартияОтгрузкаШапка.Договор IS NULL ТОГДА ПартияВводОстатковШапка.Договор
	|	ИНАЧЕ ПартияОтгрузкаШапка.Договор КОНЕЦ,
	|	ВЫБОР КОГДА ПартияОтгрузкаШапка.Валюта IS NULL ТОГДА ПартияВводОстатковШапка.Валюта
	|	ИНАЧЕ ПартияОтгрузкаШапка.Валюта КОНЕЦ,
	|	ВЫБОР КОГДА ПартияОтгрузкаШапка.НалогообложениеНДС IS NULL ТОГДА ПартияВводОстатковШапка.НалогообложениеНДС
	|	ИНАЧЕ ПартияОтгрузкаШапка.НалогообложениеНДС КОНЕЦ,
	|	ВТОтгруженоПоПартии.Склад,
	|	ВТОтгруженоПоПартии.Партия
	|";
	...
	ПараметрыФормированияТаблицы.ТекстУсловия =
		"ТИПЗНАЧЕНИЯ(Распоряжение) = ТИП(Документ.ЗаказКлиента)";
	ПараметрыФормированияТаблицы.Группировка = "Распоряжение, Номенклатура, Характеристика, КодСтроки";
	ПараметрыФормированияТаблицы.Показатели = "Отгружено, Оформлено";
	ПараметрыФормированияТаблицы.Ресурсы = "Количество, Сумма";
		...
		Пока ВыборкаВалюта.Следующий() Цикл
			...
			Пока ВыборкаНалогообложение.Следующий() Цикл
				...
				Пока ВыборкаКонтрагент.Следующий() Цикл
					...
					Пока ВыборкаПартнер.Следующий() Цикл
						ВыборкаСоглашение = ВыборкаПартнер.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
						Пока ВыборкаСоглашение.Следующий() Цикл
							ВыборкаДоговор = ВыборкаСоглашение.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
							Пока ВыборкаДоговор.Следующий() Цикл
								ВыборкаСклад = ВыборкаДоговор.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
								Пока ВыборкаСклад.Следующий() Цикл
									ВыборкаПартия = ВыборкаСклад.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
									Пока ВыборкаПартия.Следующий() Цикл
										ДокОст = Документы.ВводОстатковТоваров.СоздатьДокумент();
										ДокОст.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковОтгруженныхТоваров;
										ЗаполнитьШапкуДокументаВводаОстатков(ДокОст, КоличествоСформированныхДокументов, ВыборкаПартия);
										ДокОст.ОтражатьСебестоимость = Истина;
										ДокОст.ОтражатьВУУ = Ложь;
										ДокОст.МаксимальныйКодСтроки = ВыборкаПартия.КодСтроки;
										ДокОст.ЦенаВключаетНДС = Истина;
										Если ТипЗнч(ВыборкаПартия.Партия) = Тип("ДокументСсылка.ВводОстатковТоваров") Тогда
											ДокОст.Партия = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВыборкаПартия.Партия, "Партия");
											ДокПартия = ДокОст.Партия.ПолучитьОбъект(); 
											ДополнитьКомментарийДокумента(ДокПартия, Комментарий_НеУдалятьПриСверткеБазы);
											ДокПартия.Записать(РежимЗаписиДокумента.Запись); 
										Иначе
											ДокОст.Партия = ПреобразоватьСсылкуВПервичныйДокумент(Ложь, ВыборкаПартия.Партия, ВыборкаПартия.ПартияДата, ВыборкаПартия.ПартияНомер, Перечисления.ТипыПервичныхДокументов.ОтгрузкаТоваровКлиенту, ВыборкаПартия.Сумма);
											// Если есть СФвыд надо переназначить документ основание.
											Если ЗначениеЗаполнено(ВыборкаПартия.СчетФактураВыданный) Тогда
												СФОбъект = ВыборкаПартия.СчетФактураВыданный.ПолучитьОбъект();
												СФОбъект.ОбменДанными.Загрузка = Истина;
												Для Каждого ДокОсн Из СФОбъект.ДокументыОснования Цикл
													Если ДокОсн.ДокументОснование = ВыборкаПартия.Партия Тогда
														ДокОсн.ДокументОснование = ДокОст.Партия;
													КонецЕсли;
												КонецЦикла;
												СФОбъект.ДокументОснование = ДокОст.Партия;
												ДополнитьКомментарийДокумента(СФОбъект, Комментарий_НеУдалятьПриСверткеБазы);
												Попытка
										...
										Пока Выборка.Следующий() Цикл
											...
											СтрокаТЧ.КоличествоПоРНПТ = СтрокаТЧ.Количество;
										КонецЦикла;
										ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(НоменклатураСервер.ПараметрыУказанияСерий(ДокОст, Документы.ВводОстатковТоваров));
										...
										Для Каждого СтрокаТовары Из ДокОст.Товары Цикл
											...
											Если СтрокаТовары.СтатусУказанияСерий = 0 Тогда
												СтрокаТовары.СтатусУказанияСерий = 14;
												Продолжить;
											КонецЕсли;
											...
											Если СерияВТЧТовары Тогда
												...
												Продолжить;
											КонецЕсли;
											СтрокаТЧСерия = ДокОст.Серии.Добавить();
											ЗаполнитьЗначенияСвойств(СтрокаТЧСерия, СтрокаТовары);
											СтрокаТовары.Серия = "";
											ДобавленыСерии = Истина;
										КонецЦикла;
										Если ДобавленыСерии Тогда
											НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(ДокОст,ПараметрыУказанияСерий);
										КонецЕсли;
										ДокОст.Записать(РежимЗаписиДокумента.Запись);
										КоличествоСформированныхДокументов = КоличествоСформированныхДокументов + 1;
									КонецЦикла; // Пока ВыборкаПартия.Следующий() 
								КонецЦикла; //Пока ВыборкаСклад.Следующий() Цикл
							КонецЦикла; //Пока ВыборкаДоговор.Следующий() Цикл
						КонецЦикла; // Пока ВыборкаСоглашение.Следующий()
					КонецЦикла; // Пока ВыборкаПартнер.Следующий()
				КонецЦикла; // Пока ВыборкаКонтрагент.Следующий()
			КонецЦикла; // Пока ВыборкаНалогообложение.Следующий() Цикл
		КонецЦикла; //Пока ВыборкаВалюта.Следующий() Цикл
	КонецЦикла; // Пока ВыборкаОрганизация.Следующий()
	Если КоличествоСформированныхДокументов > 1 Тогда
		ВывестиСообщениеСформированыДокументы("ВводОстатковТоваров", Перечисления.ХозяйственныеОперации.ВводОстатковОтгруженныхТоваров);
	КонецЕсли;
КонецПроцедуры
...
Функция СоздатьПервичныйДокументДляСтрокиРасчетов(ДокОст, ОбъектРасчетов, СтрокаРасчетов)
	...
	ИначеЕсли ТипОбъектаЗаказ Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ОбъектРасчетов, "НалогообложениеНДС") Тогда
		    ДанныеЗаказа = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОбъектРасчетов, "НалогообложениеНДС");
			ПервичныйДок.НалогообложениеНДС = ДанныеЗаказа.НалогообложениеНДС;
		КонецЕсли;
		ДанныеЗаказа = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОбъектРасчетов, "Подразделение");
...
Процедура ЗаполнитьСтавкуНДСВСтрокеДокумента(СтрокаДокумента)
	Если НЕ ЗначениеЗаполнено(СтрокаДокумента.СтавкаНДС) Тогда
		...
		Если СтрокаДокумента.СуммаНДС > 0 
			...
			Если КНДС > 1.2 Тогда
				СтрокаДокумента.СтавкаНДС = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС22);
			ИначеЕсли КНДС > 1.19 Тогда
				СтрокаДокумента.СтавкаНДС = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС20);
			ИначеЕсли КНДС > 1.18 Тогда  
				СтрокаДокумента.СтавкаНДС = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС22_122);
			...
			ИначеЕсли КНДС > 1.08 Тогда  
				СтрокаДокумента.СтавкаНДС = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС10_110);
			ИначеЕсли КНДС > 1.05 Тогда
				СтрокаДокумента.СтавкаНДС = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС7);
			ИначеЕсли КНДС > 1.04 Тогда  
				СтрокаДокумента.СтавкаНДС = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС7_107);
			ИначеЕсли КНДС > 1.03 Тогда
				СтрокаДокумента.СтавкаНДС = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС5);
			ИначеЕсли КНДС > 1 Тогда  
				СтрокаДокумента.СтавкаНДС = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС5_105);
			КонецЕсли;			
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
