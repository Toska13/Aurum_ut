...
Процедура ОбновитьДвиженияПоРасчетамСПартнерами(РегистраторыКОтражению, ПараметрыОтладки = Неопределено) Экспорт
	...
	ДобавитьЗамер(Замеры, "ВыполнениеЗапросаДвижений", НачалоЗамера);
	ТипыРегистраторов = Метаданные.РегистрыНакопления.ПрочиеАктивыПассивы.СтандартныеРеквизиты.Регистратор.Тип;
	
	НачалоЗамера = ТекущаяУниверсальнаяДатаВМиллисекундах();
	Пока ВыборкаПоДокументам.Следующий() Цикл
		Если Не ТипыРегистраторов.СодержитТип(ТипЗнч(ВыборкаПоДокументам.Регистратор)) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОтразитьДокументыВУправленческомБалансе(ДокументыКотражению, ТаблицыДляДвижений = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ДанныеДвижений = ДанныеДвиженийОтраженияДокументовВУправленческомБалансе(ДокументыКотражению, ТаблицыДляДвижений,
		ДополнительныеПараметры);
	
	Если ДанныеДвижений <> Неопределено Тогда
		ВыборкаПоДокументам = ДанныеДвижений.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаПоДокументам.Следующий() Цикл
			ЗаписатьДвиженияПрочиеАктивыПассивы(ВыборкаПоДокументам);
		КонецЦикла;
	КонецЕсли;
	
КонецПроцедуры

// Данные движений отражения документов в управленческом балансе.
// 
// Параметры:
//  ДокументыКОтражению - ДокументСсылка, Массив из ДокументСсылка, МенеджерВременныхТаблиц - документы к отражению в упр. балансе
//                        Если передан менеджер временных таблиц, 
...
//                       Если не передана, тогда для переданных документов будет сформирован запрос выборки ко всем
//                       балансовым регистрам в системе. Список балансовых регистров см. Перечисление.ИсточникиУправленческогоБаланса
//  ДополнительныеПараметры - Неопределено, Структура - Дополнительные параметры формирования движений
// 
// Возвращаемое значение:
//  Неопределено, РезультатЗапроса - Данные движений для отражения документов в управленческом балансе
//
Функция ДанныеДвиженийОтраженияДокументовВУправленческомБалансе(ДокументыКотражению, ТаблицыДляДвижений = Неопределено,
...
//
Процедура ЗаписатьДвиженияПрочиеАктивыПассивы(ВыборкаПоДокументам) Экспорт
	...
	Если НаборЗаписей.Количество() > 0 Тогда
		...
		НаборЗаписей.ДополнительныеСвойства.Вставить("НеРегистрироватьЗаданияНеоперативнойОчереди");
...
Функция ЗапросДвиженийНаправлений(Параметры)
	...
	Для Каждого Движения Из ПервичныеДвижения Цикл
		...
		ИначеЕсли ИмяТаблицы = "ДвиженияПоАкцизам" Тогда
			ДобавитьТекстЗапросаДвиженияПоАкцизам(Запрос, Движения.Значение);
...
Процедура ДобавитьТекстЗапросаРасчетыСПартнерамиПоДокументам(Запрос, ТаблицаДвижений, ТипРасчетов)
	...
	|	Т.ЗаказКлиента.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ВЫБОР
	...
	|	Т.ЗаказКлиента.НаправлениеДеятельности,
	|	ВЫБОР
	|		КОГДА АналитикаПартнеров.Партнер = ЗНАЧЕНИЕ(Справочник.Партнеры.НашеПредприятие)
	|			ТОГДА &СтатьяДолгаСобственныхОрганизаций
	|		ИНАЧЕ
...
Процедура ДобавитьТекстЗапросаПрочиеДоходы(Запрос, ТаблицаДвижений)
	...
	|	НЕОПРЕДЕЛЕНО КАК Аналитика,
	|	СУММА(Т.Сумма) КАК Сумма,
	|	СУММА(-Т.Сумма) КАК СуммаБаланса,
...
Процедура ДобавитьТекстЗапросаПрочиеРасходы(Запрос, ТаблицаДвижений)
	...
	|	ВложенныйЗапрос.РасчетПартий КАК РасчетПартий,
	|	ВложенныйЗапрос.Сторно КАК Сторно
	|
	|ПОМЕСТИТЬ ИмяТаблицы
	|ИЗ
...
Процедура ДобавитьТекстЗапросаТоварыУслугиКОформлениюОтчетовКомитентуОЗакупках(Запрос, ТаблицаДвижений)
	...
	|	СУММА(ВЫБОР КОГДА Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -Т.СуммаСНДС ИНАЧЕ Т.СуммаСНДС КОНЕЦ) КАК СуммаБалансаВал,
	|	СУММА(ВЫБОР КОГДА Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -Т.СуммаУпр ИНАЧЕ Т.СуммаУпр КОНЕЦ) КАК СуммаБалансаУпр
	...
	ДобавитьЗначениеВПараметр(Запрос, "ВсеТаблицы", "втДанныеРегистраТоварыУслугиКОформлениюОтчетовКомитенту");
...
//ДвиженияПоАкцизам
Процедура ДобавитьТекстЗапросаДвиженияПоАкцизам(Запрос, ТаблицаДвижений)

	ИмяТаблицы = Метаданные.РегистрыНакопления.ДвиженияПоАкцизам.Имя;
	ПервичныеДвижения = "РегистрНакопления."+ИмяТаблицы;
	...
	ИначеЕсли ТипЗнч(ТаблицаДвижений) = Тип("ТаблицаЗначений") Тогда
		...
		|	Т.Период КАК Период,
		|	Т.Организация КАК Организация,
		|	Т.ПодразделениеУплатыАкциза КАК ПодразделениеУплатыАкциза,
		|	Т.НаправлениеДеятельности КАК НаправлениеДеятельности,
		|	Т.Сторно КАК Сторно,
		|	Т.СуммаАкцизаУпр КАК СуммаАкцизаУпр
		|ПОМЕСТИТЬ ПервичныеДвижения
		|ИЗ
		|	&ИмяТаблицы КАК Т
		|ГДЕ
	...
	|	СУММА(Т.СуммаАкцизаУпр) КАК Сумма,
	|	СУММА(-Т.СуммаАкцизаУпр) КАК СуммаБаланса,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыИсточниковДвижений.ДвиженияБаланса) КАК ВидИсточника,
	|	&Источник КАК Источник,
	...
	|	Т.Период,
	|	Т.Организация,
	|	Т.ПодразделениеУплатыАкциза,
	|	Т.НаправлениеДеятельности,
	|	Т.Сторно
	|ИМЕЮЩИЕ
	|	СУММА(Т.СуммаАкцизаУпр) <> 0";
