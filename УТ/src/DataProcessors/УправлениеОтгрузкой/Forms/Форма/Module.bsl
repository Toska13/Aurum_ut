...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ПараметрыОповещенияЭДО.ИмяДинамическогоСписка = "ОрдераВРаботе";
...
Процедура РаспоряженияНаОтгрузкуВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если (ТипЗнч(Элемент.ТекущиеДанные.Распоряжение) = Тип("ДокументСсылка.ЗаданиеНаПеревозку")
			ИЛИ ТипЗнч(Элемент.ТекущиеДанные.Распоряжение) = Тип("ДокументСсылка.Рейс"))
	...
	ИначеЕсли Поле = Элементы.РаспоряженияНаОтгрузкуСостояние Тогда
		...
		ИначеЕсли ТипЗнч(Элемент.ТекущиеДанные.Распоряжение) = Тип("ДокументСсылка.Рейс") Тогда
			
			Рейс = Элемент.ТекущиеДанные.Распоряжение;
			ВариантОтчета = ВариантОтчетаСборкаПоРаспоряжениямРейса(Рейс);
			
			ПараметрыОткрытия = Новый Структура;
			ПараметрыОткрытия.Вставить("ПараметрКоманды", Рейс);
...
Процедура РаспоряженияНаОтборВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле = Элементы.РаспоряженияНаОтборСостояние Тогда
		...
		ИначеЕсли ТипЗнч(Элемент.ТекущиеДанные.Распоряжение) = Тип("ДокументСсылка.Рейс") Тогда
			...
			Рейс = Элемент.ТекущиеДанные.Распоряжение;
			РасходныеОрдера = РасходныеОрдераПоРейсу(Рейс, Склад);
			
			Отбор = Новый Структура("Распоряжение", РасходныеОрдера);
			...
			ОткрытьФорму("Отчет.ТоварыКОтбору.Форма", ПараметрыФормы, ЭтотОбъект);
...
Процедура НазначитьЗаданиеНаПеревозку(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"НазначитьЗаданиеНаПеревозкуЗавершение",
		ЭтотОбъект,
...
Процедура Подключаемый_ВыполнитьКомандуЭДО(Команда)
	...
	Если Не ЭтоКомандаТекущиеДелаЭДО И Не ЭтоКомандаПроизвольныйДокумент Тогда
		Если Элементы.ОрдераВРаботе.ТекущиеДанные <> Неопределено Тогда
			ТекущийДокумент = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Элементы.ОрдераВРаботе.ТекущиеДанные, "Ссылка");
	...
	Если Не ДоступныКомандыЭДО Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Для данного типа документов ЭДО не предусмотрен.'"));
		Возврат;
	КонецЕсли;
	
	ЭлектронноеВзаимодействиеКлиент.ВыполнитьПодключаемуюКомандуЭДО(Команда, ЭтотОбъект, Элементы.ОрдераВРаботе);
...
Процедура Подключаемый_ОбновитьКомандыЭДО()
	...
	ОбменСКонтрагентамиКлиент.ОбновитьКоманды(ЭтотОбъект, Элементы.ОрдераВРаботе);
...
Процедура УстановитьВидимостьНеДоступноСозданиеРасходныхОрдеров()
	...
	Если Не ДоступноСозданиеРасходныхОрдеров Тогда	
		...
		Элементы.АвтоматическоеФормированиеРасходныхОрдеров.Видимость = АвтоматическоеФормированиеРасходныхОрдеров;
	...
	ИначеЕсли ФильтрЗаданий = "ПоТекущемуРаспоряжению" Тогда
		...
		Если Не ТекущиеРаспоряжения = Неопределено Тогда
			...
			Иначе
				...
				Если ТипЗнч(ТекущиеРаспоряжения.РаспоряжениеНаОтбор) = Тип("ДокументСсылка.ЗаданиеНаПеревозку")
					Или ТипЗнч(ТекущиеРаспоряжения.РаспоряжениеНаОтбор) = Тип("ДокументСсылка.Рейс") Тогда
					ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ЗаданияВРаботе, "Распоряжение", Неопределено,,,Ложь);
	...
	ИначеЕсли ФильтрОрдеров = "ПоТекущемуРаспоряжению" Тогда
		...
		Если Не ТекущиеРаспоряжения = Неопределено Тогда
			...
			Иначе
				ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ОрдераВРаботе, "Статус",   Неопределено,,,Ложь);
				ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ОрдераВРаботе, "Проведен", Истина,,,Ложь);
				Если ТипЗнч(ТекущиеРаспоряжения.РаспоряжениеНаОтгрузку) = Тип("ДокументСсылка.ЗаданиеНаПеревозку")
					ИЛИ ТипЗнч(ТекущиеРаспоряжения.РаспоряжениеНаОтгрузку) = Тип("ДокументСсылка.Рейс") Тогда
...
Процедура ЗаполнитьРаспоряженияНаОтгрузку(ПроверятьИзменениеПараметров = Ложь)
	...
	Запрос.УстановитьПараметр("ДатаОтгрузкиДоставки", НачалоДня(ДатаОтгрузки));
	Запрос.УстановитьПараметр("ДатаОтгрузкиЗаполнена", ЗначениеЗаполнено(ДатаОтгрузки));
	...
	Если ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров <> 0
		И Не (ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25()
				И СкладыСервер.НачинатьОтгрузкуПослеФормированияЗаданияНаПеревозку(Склад)) Тогда
	...
	Если НЕ ОтображениеДеталейОтгрузка Тогда
		...
		Если СкладыСервер.НачинатьОтгрузкуПослеФормированияЗаданияНаПеревозку(Склад) Тогда
			...
			ДоставкаТоваров.ДополнитьТекстЗапросаУправленияОтгрузкойДаннымиДоставки(Запрос, ТекстЗапроса);
	...
	Иначе // ОтображениеДеталейОтгрузка
		...
		ДоставкаТоваров.ДополнитьТекстЗапросаУправленияОтгрузкойДаннымиДоставкиДеталиОтгрузки(ТекстЗапроса);
...
Процедура ОбработкаОповещенияСервер(ИмяСобытия, СтруктураРаспоряжений)
	...
	ИначеЕсли ИмяСобытия = "ИзменениеСписка_ЗаданияВРаботе"
				Или ИмяСобытия = "Запись_Рейс" Тогда
...
Процедура СоздатьОрдераСервер()
		...
		Для Каждого СтрИД Из Элементы.РаспоряженияНаОтгрузку.ВыделенныеСтроки Цикл
			...
			Если ТипЗнч(СтрТабл.Распоряжение) = Тип("ДокументСсылка.ЗаданиеНаПеревозку") Тогда
				...
				Пока Выборка.СледующийПоЗначениюПоля("Получатель") Цикл
					...
					СтруктураПараметровСозданияРасходныхОрдеров.РаспоряженияНаОтгрузку.Очистить();
					Пока Выборка.Следующий() Цикл
			...
			ИначеЕсли ТипЗнч(СтрТабл.Распоряжение) = Тип("ДокументСсылка.Рейс") Тогда
				...
				|	ДоставкаОбороты.ДокументПланирования КАК ДокументПланирования,
				|	ДоставкаОбороты.Получатель КАК Получатель,
				|	МИНИМУМ(ДоставкаОбороты.КодСтроки) КАК КодСтроки
				|ПОМЕСТИТЬ ВтДанныеРейса
				|ИЗ
				...
				|	ДоставкаОбороты.ДокументПланирования
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ РАЗЛИЧНЫЕ
				|	ВтДанныеРейса.Распоряжение КАК Распоряжение,
				|	ЕСТЬNULL(РеестрДокументов.Партнер, ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)) КАК Получатель
				|ИЗ
				|	ВтДанныеРейса КАК ВтДанныеРейса
				|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументов
				|		ПО ВтДанныеРейса.Распоряжение = РеестрДокументов.Ссылка
				|			И (НЕ РеестрДокументов.ДополнительнаяЗапись)
				|
				|УПОРЯДОЧИТЬ ПО
				|	Получатель";
				Запрос.УстановитьПараметр("Рейс", СтрТабл.Распоряжение);
	...
	Если ФильтрОрдеров = "ПоТекущемуРаспоряжению" Тогда
		...
		Если ТипЗнч(ТекущееРаспоряжениеНаОтгрузку) = Тип("ДокументСсылка.ЗаданиеНаПеревозку")
			Или ТипЗнч(ТекущееРаспоряжениеНаОтгрузку) = Тип("ДокументСсылка.Рейс") Тогда
...
Процедура КонтрольРаботыФоновыхЗаданийФормированияОчереди()
	...
	Если Не АвтоматическоеФормированиеРасходныхОрдеров Тогда
		Возврат;
	КонецЕсли;
...
Процедура КонтрольРаботыФоновыхЗаданийФормированияОчередиСервер()
	...
	ОчередьОбновилась = Ложь;
	
	Получатели = ПолучателиПоКоторымНетФоновогоЗаданияПереоформленияОрдеров(Склад, ОчередьОбновилась);
	
	Если ОчередьОбновилась Тогда
		Элементы.АвтоматическоеФормированиеРасходныхОрдеров.ТекущаяСтраница = Элементы.СтраницаТребуетсяРучнойЗапуск;
	ИначеЕсли Получатели.Количество() > 0 Тогда
		Элементы.АвтоматическоеФормированиеРасходныхОрдеров.ТекущаяСтраница = Элементы.СтраницаПроблема;
	Иначе
		Элементы.АвтоматическоеФормированиеРасходныхОрдеров.ТекущаяСтраница = Элементы.СтраницаНетПроблем;
	КонецЕсли;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПолучателиПоКоторымНетФоновогоЗаданияПереоформленияОрдеров(Склад, ОчередьОбновилась = Ложь)
	
	Возврат СкладыСервер.ПолучателиПоКоторымНетФоновогоЗаданияПереоформленияОрдеров(Склад, ОчередьОбновилась);
...
&НаСервереБезКонтекста
Функция РасходныеОрдераПоРейсу(Знач Рейс, Знач Склад)
	
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТоварыКОтгрузке.Регистратор КАК РасходныйОрдер
	|ИЗ
	|	РегистрНакопления.ТоварыКОтгрузке КАК ТоварыКОтгрузке
	|ГДЕ
	|	ТоварыКОтгрузке.Склад = &Склад
	|	И ТоварыКОтгрузке.ЗаданиеНаПеревозку = &Рейс
	...
	Ордера = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("РасходныйОрдер");
