	...
	Если СтруктураДанных.Свойство("Товары") Тогда
		...
		Если ЗначениеЗаполнено(СтруктураДанных.СтруктураДействий) Тогда
			...
			Если СтруктураДанных.СтруктураДействий.Свойство("ЗаполнитьТаблицуТоваров", ПараметрДействия) Тогда
				...
				Если СтруктураДанных.Свойство("ТаблицаОшибокПоСериям")
					И СтруктураДанных.ТаблицаОшибокПоСериям.Количество() > 0 Тогда
					
					СообщитьОПроблемахЗаполненияСерийНоменклатуры(СтруктураДанных.ТаблицаОшибокПоСериям);
...
Процедура УстановитьШаблонЭтикеток(Команда)
	...
	ОткрытьФорму("Справочник.ШаблоныЭтикетокИЦенников.ФормаВыбора",
		ПараметрыФормыВыбора, ЭтотОбъект,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура УстановитьНовыйШтрихкодEAN13(Команда)
	...
	Для Каждого ИдентификаторСтроки Из Элементы.Товары.ВыделенныеСтроки Цикл
		...
		Если Не ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда
			ТекстСообщения = НСтр("ru = 'Не заполнена колонка ""Номенклатура"" в строке %1.'");
			ТекстСообщения = СтрШаблон(ТекстСообщения, НомерСтроки);
			
			Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти( "Товары", НомерСтроки, "Номенклатура");
			
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, Неопределено, Поле, "Объект", ЕстьОшибки);
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура)
			...
			И ТекущаяСтрока.ХарактеристикиИспользуются Тогда
			
			ТекстСообщения = НСтр("ru = 'Не заполнена колонка ""Характеристика"" в строке %1.'");
			ТекстСообщения = СтрШаблон(ТекстСообщения, НомерСтроки);
			
			Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", НомерСтроки, "Характеристика");
	...
	ОписаниеОшибки	= "";
	Результат		= УстановитьНовыеШтрихкодыEAN13НаСервере(МассивСтрок, ОписаниеОшибки);
	
	Если Результат = 0
		И Не ПустаяСтрока(ОписаниеОшибки) Тогда
...
&НаСервере
Функция УстановитьНовыеШтрихкодыEAN13НаСервере(ВыделенныеСтроки, ОписаниеОшибки = "")
		...
		Для Каждого НомерСтроки Из ВыделенныеСтроки Цикл
			...
			Если Не СтрокаТЧ.Весовой Тогда
				Для Каждого СтрокаТЧМаксимальныеЗначения Из МаксимальныеЗначенияКодовШтучныхШтрихкодов Цикл
					Если СтрокаТЧМаксимальныеЗначения.Код >= МаксимальныйКодШтучногоТовара Тогда
						Продолжить;
					КонецЕсли;
			...
			КонецЕсли;
			Если ИспользоватьСерииНоменклатуры Тогда
				НовыйШтрихкод.Серия = СтрокаТЧ.Серия;
			КонецЕсли;
...
Процедура ЗагрузитьНастройкиОтбораПоУмолчанию()
	...
	ТекстЗамены = ?(ИспользоватьСерииНоменклатуры,
					"ИсходныеДанные.Серия",
					"ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)");
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ДанныеПоСериямПереопределяемый", ТекстЗамены);
...
&НаСервере
Функция ДобавитьНоменклатуруПоШтрихкодам(ДанныеШтрихкодов, КэшированныеЗначения)
	...
	Для Каждого ТекШтрихкод Из ДанныеШтрихкодов Цикл
		...
		Иначе
			НоваяСтрока = Таблица.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ДобавляемыеДанные, ИменаКолонок);
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	НеизвестныеШтрихкоды = ДобавитьНоменклатуруПоШтрихкодам(МассивШтрихкодов, КэшированныеЗначения);
...
Процедура РазбитьПоУпаковкамНаСервере(ВыбранныеСтроки, Базовыми = Ложь, СкладскаяГруппа = Неопределено)
	...
	Для Каждого ИдентификаторСтроки Из ВыбранныеСтроки Цикл
		...
		СтрокаТовар.КоличествоВДокументе = ?(СтрокаТовар.КоличествоВДокументе,
											СтрокаТовар.КоличествоЭтикеток,
											СтрокаТовар.КоличествоВДокументе);
		...
		КоэффициентУпаковки	= Справочники.УпаковкиЕдиницыИзмерения.КоэффициентУпаковки(
								СтрокаТовар.Упаковка,
								СтрокаТовар.Номенклатура);
		
		ТребуемоеКоличество = СтрокаТовар.КоличествоВДокументе * ?(КоэффициентУпаковки, КоэффициентУпаковки, 1);
		ТребуемоеКоличество = ?(ТребуемоеКоличество > 0,
								ТребуемоеКоличество,
								СтрокаТовар.КоличествоЭтикеток * ?(КоэффициентУпаковки, КоэффициентУпаковки, 1));
		...
		Для Каждого СтрокаУпаковка Из ТаблицаУпаковок Цикл
			Если СтрокаУпаковка.Количество = 0 Тогда
				Продолжить
			КонецЕсли;
			...
			Штрихкоды = РозничныеПродажиВызовСервера.ПолучитьШтрихкодыНоменклатуры(НоваяСтрока);
	...
	Если ЭтоАдресВременногоХранилища(СохраненныеНастройки) Тогда
		...
		Если СтрокаНастройки <> Неопределено Тогда
			СтрокаНастройки.Значение = Объект.Вариант;
		КонецЕсли;
		
		СтрокаНастройки = ТаблицаСохраненныеНастройки.Найти("Объект.СкладскаяГруппа", "Ключ");
		Если СтрокаНастройки <> Неопределено Тогда
			СтрокаНастройки.Значение = Объект.СкладскаяГруппа;
		КонецЕсли;
...
//
&НаСервере
Процедура СообщитьОПроблемахЗаполненияСерийНоменклатуры(ТаблицаОшибок)
	
	ТаблицаОшибок.Индексы.Добавить("Номенклатура, Характеристика");
	
	ОтборСтрок = Новый Структура("Номенклатура, Характеристика");
	ШаблонСообщения = НСтр("ru = 'По данным документа %1 не удалось сопоставить сведения о сериях для товара ""%2"", учитываемого в различных упаковках.'");
	...
	Для Каждого СтрокаТоваров Из Объект.Товары Цикл
		...
		СтрокиОшибок = ТаблицаОшибок.НайтиСтроки(ОтборСтрок);
		
		Для Каждого ДанныеОшибки Из СтрокиОшибок Цикл
			ПредставлениеТовара = НоменклатураКлиентСервер.ПредставлениеНоменклатуры(
									ДанныеОшибки.Номенклатура,
									ДанныеОшибки.Характеристика);
			
			ТекстСообщения= СтрШаблон(ШаблонСообщения, Строка(ДанныеОшибки.Ссылка), ПредставлениеТовара);
			Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
					"Товары",
					СтрокаТоваров.НомерСтроки,
					"Номенклатура");
			
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Неопределено, Поле, "Объект");
		КонецЦикла;
	КонецЦикла;
