...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("Артикул", Артикул);
...
&НаКлиенте
Процедура ОтборыОчиститьХарактеристику(Команда)
...
&НаКлиенте
Процедура ОтборыОчиститьСерию(Команда)
...
Процедура ЗаполнитьОстаткиТоваровНаАдресномСкладе()
	...
	Если ЗначениеЗаполнено(Серия) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И &УсловиеСерия", "И Серия = &Серия");
		Запрос.УстановитьПараметр("Серия", Серия);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И &УсловиеСерия", "");
	КонецЕсли;
...
Процедура ЗаполнитьОстатки()
	...
	Элементы.ГруппаХарактеристика.Видимость  = ЗначениеЗаполнено(Характеристика); 
	Элементы.ГруппаСерия.Видимость           = ЗначениеЗаполнено(Серия); 
	...
	ИспользоватьПомещения                   = СвойстваСклада.ИспользоватьСкладскиеПомещения;
