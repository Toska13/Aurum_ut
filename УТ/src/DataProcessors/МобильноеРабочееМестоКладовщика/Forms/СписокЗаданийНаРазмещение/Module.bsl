...
Процедура НайтиПоШтрихкоду(Команда)
	...
	Описание = Новый ОписаниеОповещения("РезультатСканированиеДокумента", ЭтотОбъект);
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ТипПоиска", "ПоискДокумента");
	ПараметрыФормы.Вставить("Склад", Склад);
	ПараметрыФормы.Вставить("Помещение", Помещение);
	ПараметрыФормы.Вставить("Контекст", "ЗаданиеНаРазмещение");
...
Процедура ОбработатьШтрихкод(ДанныеШтрихкода)
	...
	Если ЗначениеЗаполнено(ДанныеПоШтрихкоду.Ссылки) Тогда
		РезультатСканированиеДокумента(ДанныеПоШтрихкоду);
	Иначе 
...
Функция ДанныеДокументаПоШтрихкоду(Знач Штрихкод)
	...
	Возврат Обработки.МобильноеРабочееМестоКладовщика.ДанныеСкладскогоДокументаПоШК(Штрихкод, "ЗаданиеНаРазмещение");
	
КонецФункции

&НаКлиенте
Процедура РезультатСканированиеДокумента(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
