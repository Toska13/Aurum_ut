...
Процедура ЗаписатьДанныеОрганизацииСервер()
	...
	Если Не ПустаяСтрока(ДанныеОрганизации.Организация.НомерСчета) Тогда
		...
		СтруктураПараметров = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации(ТекущаяОрганизация);
		ТекущийСчет = Справочники.БанковскиеСчетаОрганизаций.ПолучитьБанковскийСчетПоУмолчанию(СтруктураПараметров);
