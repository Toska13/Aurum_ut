...
Функция КЗаказуДляПоддержанияЗапаса(Состояние, ЗаСчетЗапасов) Экспорт
	...
	НаКонтролеКачества = ?(ЗаСчетЗапасов, Состояние.НаКонтролеКачестваОстаток, Состояние.НаКонтролеКачестваСвободныйОстаток);
	...
	Если Не ТочкаЗаказаДостигнута Тогда
		КЗаказу = Макс(0, Состояние.МаксимальныйОстаток - (Остаток - Состояние.МинимальныйОстаток)
			- (ЗаказыКПоступлению + НаКонтролеКачества));
	Иначе
		КЗаказу = Состояние.МаксимальныйОстаток - (ЗаказыКПоступлению + НаКонтролеКачества);
	КонецЕсли;
...
Функция СоздатьЗаказы(Объект) Экспорт
	...
	Таблица.Колонки.Добавить("КЗаказу",                  ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
	...
	Текст =
		...
		|////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ЕСТЬNULL(ВсеСоглашения.СрокОтгрузки + ВсеСоглашения.СрокДоставки, 0) КАК СрокПоставки
	...
	Подстановки = ОбеспечениеВДокументахСервер.ДобавитьКДатеПоКалендарюПредприятия("&НачалоДня", "Таблица.СрокПоставки");
	...
	Текст =
		...
		|	ДатыГрафика.ДатаПоступления КАК ДатаПоступления,
		...
		|		ПО ЕСТЬNULL(ВсеСоглашения.СрокОтгрузки + ВсеСоглашения.СрокДоставки, 0) = ДатыГрафика.СрокПоставки
		|
		|СГРУППИРОВАТЬ ПО
...
Процедура ПолучитьДанныеОЗапасах(Таблица, ВременнаяТаблицаТовары, Запрос, Параметры)
	...
	Описание = РаспределениеЗапасов();
	КоличествоТаблиц = Описание.КоличествоТаблиц;
	ТекстыЗапросов.Добавить(Описание.ТекстЗапроса);
	...
	ВсегоТаблиц = РезультатыЗапроса.Количество();
	
	ВыборкаТоварныеОграничения   = РезультатыЗапроса[ВсегоТаблиц - КоличествоТаблиц - 5].Выбрать();
	ТаблицаСтатистикиПотребления = РезультатыЗапроса[ВсегоТаблиц - КоличествоТаблиц - 4].Выгрузить();
	ТаблицаОстатки               = РезультатыЗапроса[ВсегоТаблиц - 3].Выгрузить();
	ТаблицаЗаказыКПоступлению    = РезультатыЗапроса[ВсегоТаблиц - 2].Выгрузить();
	ТаблицаУсловияЗакупок        = РезультатыЗапроса[ВсегоТаблиц - 1].Выгрузить();
...
Функция РаспределениеЗапасов()
	...
	Тексты = ОбеспечениеВДокументахСервер.РаспределениеЗапасов("", ИменаТаблиц);
	Текст = СтрСоединить(Тексты, ОбщегоНазначения.РазделительПакетаЗапросов());
	ВсегоТаблиц = Тексты.Количество() + 2;
	...
	Возврат Новый Структура("КоличествоТаблиц, ТекстЗапроса", ВсегоТаблиц, ТекстЗапроса);
...
Функция ТаблицаОстатки()
	...
	Текст =
		...
		|	Запасы.Номенклатура КАК Номенклатура,
		|	Запасы.Характеристика КАК Характеристика,
		|	Запасы.ВНаличии КАК Остаток,
		|	Запасы.ВНаличии_Излишек КАК СвободныйОстаток,
		|	Запасы.ВНаличииСБлокировкой КАК НаКонтролеКачестваОстаток,
		|	Запасы.ВНаличииСБлокировкой_Излишек КАК НаКонтролеКачестваСвободныйОстаток
		|ИЗ
		|	РасчетРаспределенияЗапасов КАК Запасы
		|ГДЕ
		|	Запасы.ВНаличии <> 0
		|		ИЛИ Запасы.ВНаличии_Излишек <> 0
		|		ИЛИ Запасы.ВНаличииСБлокировкой <> 0
		|		ИЛИ Запасы.ВНаличииСБлокировкой_Излишек <> 0
		|УПОРЯДОЧИТЬ ПО
		|	Номенклатура, Характеристика, Склад";
...
Функция ТаблицаЗаказыКПоступлению()
	...
	Текст =
		...
		|	Запасы.Характеристика КАК Характеристика,
		|	Запасы.Ожидается
		|		+ Запасы.ОжидаетсяСБлокировкой
		|		+ Запасы.НаСогласовании КАК ЗаказыКПоступлению,
		|	Запасы.Ожидается_Излишек
		|		+ Запасы.ОжидаетсяСБлокировкой_Излишек
		|		+ Запасы.НаСогласовании_Излишек КАК ЗаказыКПоступлениюСвободные
		|ИЗ
		|	РасчетРаспределенияЗапасов КАК Запасы
		|ГДЕ
		|	Запасы.Ожидается
		|		+ Запасы.ОжидаетсяСБлокировкой
		|		+ Запасы.НаСогласовании > 0
		|УПОРЯДОЧИТЬ ПО
		|	Номенклатура, Характеристика, Склад";
...
Функция ВременнаяТаблицаЗаказы()
	...
	Текст =
		...
		|	Потребности.Количество КАК Требуется,
		|	ВЫБОР
		|		КОГДА
		|			Потребности.СостояниеЗапаса В(
		...
		|			И Потребности.ХарактеристикаПотребности В(
