...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаписатьУпаковки", Истина);
	Методы.Вставить("ПолучитьДанныеЗаполнения", Истина);
	Методы.Вставить("УдалитьУпаковки", Истина);
...
Процедура ПолучитьДанныеЗаполнения(Параметры, АдресРезультата) Экспорт
	...
	РегистрыСведений.ПулКодовМаркировкиСУЗ.ЗаполнитьДеревоСуществующихУпаковокПоДокументу(
		ДанныеЗаполнения.СуществующиеУпаковки, Параметры.Документ, Параметры.ТоварнаяГруппа, Ложь);
...
Процедура ЗаписатьУпаковки(Параметры, АдресРезультата) Экспорт
	...
	Попытка
		...
		ШтрихкодВключаетМРЦ = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(Параметры.ТоварнаяГруппа);
			...
			Пока Выборка.Следующий() Цикл
				Если ШтрихкодВключаетМРЦ И Выборка.МаксимальнаяРозничнаяЦена > 0 Тогда
					...
					ДанныеРазбораКодаМаркировки = ГрупповаяОбработкаШтрихкодовИС.ВидУпаковкиИПредставлениеШтрихкодаУпрощенныйРазбор(
						КодМаркировки, Параметры.ТоварнаяГруппа, Неопределено,
		...
		ПараметрыВыполнения.Вставить("ТипШтрихкодаМаркированногоТовара",
			ТипШтрихкодаМаркированногоТовараПоТоварнойГруппе(Параметры.ТоварнаяГруппа));
...
Функция НовоеДеревоУпаковок() Экспорт
	...
	ДеревоУпаковок.Колонки.Добавить("Количество",                ОбщегоНазначения.ОписаниеТипаЧисло(18, 8));
	...
	ДеревоУпаковок.Колонки.Добавить("Шаблон",                    Новый ОписаниеТипов("СправочникСсылка.ШаблоныКодовМаркировкиСУЗ"));
	...
	ДеревоУпаковок.Колонки.Добавить("ВидПродукции",              Новый ОписаниеТипов("СправочникСсылка.ВидыПродукцииИС"));
...
Функция НоваяСоставУпаковки() Экспорт
	...
	СоставУпаковки.Колонки.Добавить("Шаблон",                    Новый ОписаниеТипов("СправочникСсылка.ШаблоныКодовМаркировкиСУЗ"));
...
Функция НоваяВложенныеЕдиницы() Экспорт
	...
	ВложенныеЕдиницы.Колонки.Добавить("Шаблон",         Новый ОписаниеТипов("СправочникСсылка.ШаблоныКодовМаркировкиСУЗ"));
...
Процедура ЗаполнитьВложенныеЕдиницы(Параметры, ОстаткиПродукции)
	...
	ВидыПродукцииПоТоварнойГруппе = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(Параметры.ТоварнаяГруппа);
	
	Если КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетНаборы(ВидыПродукцииПоТоварнойГруппе) Тогда
		ЗаполнитьНаборы(Параметры, ОстаткиПродукции);
	КонецЕсли;
	
	Если КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетГрупповыеУпаковки(ВидыПродукцииПоТоварнойГруппе) Тогда
...
Процедура ЗаполнитьНаборы(Параметры, ОстаткиПродукции)
	...
	ШаблоныНабора = ИнтеграцияИСМПКлиентСервер.ШаблоныКодовПоВидуПродукции(Параметры.ТоварнаяГруппа, 5);
...
Процедура ЗаполнитьГрупповыеУпаковки(Параметры, ОстаткиПродукции)
	...
	ШаблоныГрупповойУпаковки = ИнтеграцияИСМПКлиентСервер.ШаблоныКодовПоВидуПродукции(Параметры.ТоварнаяГруппа, 4);
...
Процедура ЗаполнитьСоставИсторическихУпаковок(Параметры, ДанныеЗаполнения)
	...
	|	РегистрСведений.ПулКодовМаркировкиСУЗ КАК ПулКодовМаркировки
	|ГДЕ
	|	ПулКодовМаркировки.ТоварнаяГруппа = &ТоварнаяГруппа
	|	И ПулКодовМаркировки.ШтрихкодУпаковки <> ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка)
	...
	|	ПулКодовМаркировки.Характеристика,
	|	ПулКодовМаркировки.Шаблон,
	|	ПулКодовМаркировки.ТоварнаяГруппа,
	|	ВЫБОР
	|		КОГДА ПулКодовМаркировки.Номенклатура = &ПустаяНоменклатура
	|			ТОГДА ПулКодовМаркировки.GTIN
	|		ИНАЧЕ """"
	...
	ШаблонПоУмолчанию = ИнтеграцияИСМПКлиентСервер.ШаблонКодаМаркировкиПоВидуПродукции(Параметры.ТоварнаяГруппа);
...
Функция НовыйШтрихкод(ТипШтрихкода, КлючНумератора, ОбразецШтрихкода, СерийныеНомераНумератор) Экспорт
	...
	Если ТипШтрихкода <> Перечисления.ТипыШтрихкодов.SSCC
		И ТипШтрихкода <> Перечисления.ТипыШтрихкодов.SSCC18
		И ТипШтрихкода <> Перечисления.ТипыШтрихкодов.SSCC20
	...
	Если ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC
		Или ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC18
		Или ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC20 Тогда
...
Функция НоваяОстаткиПродукции()
	...
	ОстаткиПродукции.Колонки.Добавить("Шаблон",                     Новый ОписаниеТипов("СправочникСсылка.ШаблоныКодовМаркировкиСУЗ"));
...
Функция НовоеСоставИсторическихУпаковок()
	...
	СоставИсторическихУпаковок.Колонки.Добавить("Шаблон",           Новый ОписаниеТипов("СправочникСсылка.ШаблоныКодовМаркировкиСУЗ"));
...
Функция ТипШтрихкодаМаркированногоТовараПоТоварнойГруппе(ТоварнаяГруппа)
	
	Результат = Перечисления.ТипыШтрихкодов.GS1_DataMatrix;
	
	Шаблон = ИнтеграцияИСМПКлиентСервер.ШаблонКодаМаркировкиПоВидуПродукции(ТоварнаяГруппа);
	Если ЗначениеЗаполнено(Шаблон) Тогда
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Ссылка", Шаблон);
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ШаблоныКодовМаркировкиСУЗ.ТипШтрихкода
		|ИЗ
		|	Справочник.ШаблоныКодовМаркировкиСУЗ КАК ШаблоныКодовМаркировкиСУЗ
		|ГДЕ
		|	ШаблоныКодовМаркировкиСУЗ.Ссылка = &Ссылка";
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			Результат = Выборка.ТипШтрихкода;
		КонецЕсли;
	КонецЕсли;
...
Функция ТекстЗапросаДляЗаписиУпаковок(ЭтоЗаказНаЭмиссию)
	...
	|		ИЛИ ЕСТЬNULL(ОписаниеНоменклатурыИС.ЕмкостьПотребительскойУпаковки, 1) > 1 КАК ПроизвольнаяЕдиницаУчета
	|ИЗ
