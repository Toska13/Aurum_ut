...
Процедура СформироватьГиперссылкуКОформлениюФоновоеЗадание(Параметры, АдресХранилища) Экспорт
	...
	Результат.КОформлению = СформироватьГиперссылкуКОформлениюСписокДокументов(Параметры[0], Параметры[1]);
...
// Возвращает текст гиперссылки перехода из журнала документов в рабочие места оформления накладных,
// по выбранным хозяйственным операциям и типам документов.
//
// Параметры:
//	ХозяйственныеОперацииИДокументы - ТаблицаЗначений - выбранные хозяйственные операции и типы документов
//	Параметры - Структура - параметры формирования текста гиперссылки.
//
// Возвращаемое значение:
//	ФорматированнаяСтрока, Неопределено - текст гиперссылки перехода в выбранные рабочие места оформления.
//
Функция СформироватьГиперссылкуКОформлениюСписокДокументов(ХозяйственныеОперацииИДокументы, ПараметрыФормирования) Экспорт
	...
	Для Каждого МенеджерРасчетаГиперссылок Из МенеджерыРасчетаГиперссылок Цикл
		...
		Для Каждого ТекущаяГиперссылка Из Гиперссылка Цикл
			ДобавитьГиперссылкуВСписок(ТекущаяГиперссылка, МассивСтрок);
		КонецЦикла;
...
Функция ТекстЗапросаЗаказыКОформлению(Организация = Неопределено, Склад = Неопределено)
	...
	|			ИСТИНА
	|		ИЗ
	|			#РаспоряженияНаОтгрузкуОборотыРазвернутая1 КАК ЗаказыКлиентов
	|		ГДЕ
	|			(ЗаказыКлиентов.КОформлениюКоличествоОборот -
	|				ЗаказыКлиентов.ОформленоКоличествоОборот) > 0)
	|	ИЛИ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	|			#РаспоряженияНаОтгрузкуОборотыРазвернутая2 КАК ЗаказыКлиентов
	|		ГДЕ
	|			(ЗаказыКлиентов.КОформлениюКоличествоОборот -
	|				ЗаказыКлиентов.ОформленоКоличествоОборот) > 0))";
