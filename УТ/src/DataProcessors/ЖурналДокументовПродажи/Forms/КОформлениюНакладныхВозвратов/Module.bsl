...
Процедура ОформитьПоРаспоряжениям(Команда)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ЖурналДокументовПродажи.КОформлениюНакладныхВозвратов.Команда.ОформитьПоРаспоряжениям");
...
Процедура СостояниеВыполнения(Команда)
	...
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		...
		Если	ТипВыделенногоДокумента = Тип("ДокументСсылка.ВозвратТоваровОтКлиента")
				ИЛИ ТипВыделенногоДокумента = Тип("ДокументСсылка.КорректировкаРеализации")
				ИЛИ ТипВыделенногоДокумента = Тип("ДокументСсылка.ПоступлениеТоваровОтХранителя") Тогда
...
Процедура ОформитьНакладныеНаКлиентеЗавершение(Результат, ДопПараметры) Экспорт
	...
	Если НЕ ПараметрыСозданныхДокументов.СписокОшибок = Неопределено Тогда
		...
		Если ТипЗнч(ПараметрыСозданныхДокументов.СписокОшибок) = Тип("Массив") Тогда
			
			ОшибкиПользователю = Неопределено;
		
			Для Каждого Ошибка Из ПараметрыСозданныхДокументов.СписокОшибок Цикл
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(ОшибкиПользователю, Ошибка.Поле, Ошибка.Текст, "");
			КонецЦикла;
		...
		Если ПоОрдерам Тогда
			...
			ИначеЕсли ДокументыПоТипамНакладных.Количество() = 1 Тогда
				...
				Иначе
					...
					Если ЕстьНакладная И Не ЕстьОрдер Тогда
	...
	Для Каждого Строка Из СписокРаспоряжений Цикл
		...
		Если ПоОрдерам Тогда
			...
			ИначеЕсли Строка.СостояниеОрдера = 4 Тогда
				...
				ТекстОшибки = НСтр("ru = 'Для распоряжения %1 невозможно оформить накладную по приемке, т.к. склад %2 не использует ордерную схему.'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Строка.Ссылка, Строка.Склад);
...
Функция ОформитьРядНакладныхПоТаблицам(СтруктураПараметров)
	...
	Если ЗначениеЗаполнено(ДокументыПоТипамНакладных) Тогда
		...
		ПараметрыФормирования.ИмяНовогоДокумента = МетаданныеНакладной.Имя;
		ПараметрыФормирования.СоздатьНовыйДокумент = Истина;
	КонецЕсли;
	СтруктураПараметровНовая.Вставить("ПараметрыФормирования", ПараметрыФормирования);
...
Функция МетаданныеНакладнойПоОперации(ПараметрыОперации)
	...
	Если	ОперацияРаспоряжения = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности
			ИЛИ ОперацияРаспоряжения = Перечисления.ХозяйственныеОперации.РеализацияКлиенту Тогда
		МетаданныеНакладной = Метаданные.Документы.ПоступлениеТоваровОтКлиента;
	КонецЕсли;
...
Функция ПараметрыФормированияДокументов()
	...
	ПараметрыФормирования.Вставить("ОформитьОтгрузку", Ложь);
	ПараметрыФормирования.Вставить("СоздатьНовыйДокумент", Ложь);
	ПараметрыФормирования.Вставить("ИмяКомандыОтгрузки", "");
	ПараметрыФормирования.Вставить("ИмяНовогоДокумента", "");
