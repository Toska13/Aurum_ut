...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДекорацияНастройкаКодовОпераций.Видимость		= НЕ УправлениеТорговлей;
	...
	Элементы.ГруппаДокументыОПредстоящейПоставке.Видимость = ПолучитьФункциональнуюОпцию("ИспользоватьИмпортныеЗакупки") И НЕ УправлениеТорговлей;
	Элементы.ГруппаЗаявлениеОВвозеТоваров.Видимость        = ПолучитьФункциональнуюОпцию("ИспользоватьИмпортныеЗакупки");
	Элементы.ГруппаЗаявлениеОВвозеТоваровПолученное.Видимость = ПолучитьФункциональнуюОпцию("ИспользоватьПродажиНаЭкспортНесырьевыхТоваров")
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если    ИмяСобытия = "Запись_СчетФактураПолученный"
		...
		Или ИмяСобытия = "Запись_ЗаявлениеОВвозеТоваровПолученное"
...
&НаКлиенте
Процедура ДекорацияСчетФактураВыданныйПоОтгрузкамНажатие(Элемент)
	
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ПомощникПоУчетуНДС.Форма.Элемент.ДекорацияСчетФактураВыданныйПоОтгрузкам.Нажатие");
...
&НаКлиенте
Процедура ДекорацияДОППНажатие(Элемент)
...
&НаКлиенте
Процедура ДекорацияЗаявлениеОВвозеТоваровПолученноеНажатие(Элемент)
...
Процедура ДекорацияНДССостояниеРеализации0Нажатие(Элемент)
	...
	СтруктураБыстрогоОтбора = Новый Структура;
	СтруктураБыстрогоОтбора.Вставить("ОрганизацияОтбор", Объект.Организация);
	СтруктураБыстрогоОтбора.Вставить("НачалоПериода", НачалоКвартала(Объект.КонецПериода));
	СтруктураБыстрогоОтбора.Вставить("КонецПериода", Объект.КонецПериода);
	Если СтрЗаканчиваетсяНа(Элемент.Заголовок, ")") Тогда
		СтруктураБыстрогоОтбора.Вставить("Состояние", ПредопределенноеЗначение("Перечисление.НДССостоянияРеализация0.ОжидаетсяПодтверждение"));
	КонецЕсли;
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("СтруктураБыстрогоОтбора", СтруктураБыстрогоОтбора);
	
	ОткрытьФорму("Документ.ПодтверждениеНулевойСтавкиНДС.Форма.ФормаРабочееМесто", ПараметрыФормы);
...
&НаКлиенте
Процедура ДекорацияНастройкаКодовОперацийНажатие(Элемент)
...
Процедура УстановитьВидимостьОформлениеЭлементов(ПроверятьРасширенныйНалоговыйПериод = Истина)
	...
	ДанныеИсточниковПоОтгрузкам = ДанныеИсточников.Добавить();
	ДанныеИсточниковПоОтгрузкам.Документ = "СчетФактураВыданныйПоОтгрузкам";
	ДанныеИсточниковПоОтгрузкам.КоличествоКОформлению = Документы.СчетФактураВыданный.ЕстьСчетаФактурыПоОтгрузкамКОформлению(ПараметрыДанных);
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("МассивОрганизаций", ПараметрыДанных.МассивОрганизаций);
	...
	|	И СчетФактура.НачислениеНДСПриОтгрузке
	|";
	
	Выборка = Запрос.Выполнить().Выбрать();
	ДанныеИсточниковПоОтгрузкам.КоличествоОформленных = ?(Выборка.Следующий(), Выборка.КоличествоОформленных, 0);
	...
	Для Каждого Источник Из ДанныеИсточников Цикл
		...
		Если Источник.КоличествоОформленных = 0 И Источник.КоличествоКОформлению = 0 Тогда
			...
			Если ИмяЭлемента = "ДекорацияСчетФактураНаНеподтвержденнуюРеализацию0" Тогда
				Элементы.ДекорацияНастройкаКодовОпераций.Видимость = Ложь;
			КонецЕсли;
		...
		Иначе
			...
			Если ИмяЭлемента = "ДекорацияСчетФактураНаНеподтвержденнуюРеализацию0" И Не УправлениеТорговлей Тогда
				Элементы.ДекорацияНастройкаКодовОпераций.Видимость = Истина;
			КонецЕсли;
		КонецЕсли;
...
Процедура УстановитьСтатусыРегламентныхОпераций()
	...
	Если ЗначениеЗаполнено(Объект.Организация) 
		...
		СостояниеПартииТоваров = СостояниеЭтапов[Справочники.ОперацииЗакрытияМесяца.РасчетПартийИСебестоимости];
		СостояниеРаспределениеНДС = СостояниеЭтапов[Справочники.ОперацииЗакрытияМесяца.РаспределениеНДС];
...
Процедура ОбновитьСостоянияОперацийНаСервере()
	...
	ЭтапыЗакрытияМесяца.Добавить(Справочники.ОперацииЗакрытияМесяца.РасчетПартийИСебестоимости);
	ЭтапыЗакрытияМесяца.Добавить(Справочники.ОперацииЗакрытияМесяца.РаспределениеНДС);
...
Процедура ПодключитьОбработчикОжиданияСтатусРегламентныхЗаданий()
	...
	Если ЗначениеЗаполнено(ИдентификаторЗаданияСтатусРегламентныхЗаданий) Тогда
		...
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбработкаСтатусаРегламентныхЗаданийОповещение", ЭтотОбъект);
...
&НаКлиенте
Процедура ОбработкаСтатусаРегламентныхЗаданийОповещение(Результат, ДополнительныеПараметры) Экспорт
...
Функция ОписаниеЗаголовковЭлементов()
	...
	ЗаголовкиЭлементов.Вставить("СчетФактураВыданныйПоОтгрузкам", 
			Новый Структура("ЕстьДанныеОбработано, ЕстьДанныеКОбработке, НетДанных",
					НСтр("ru = 'Все ИСФ/КСФ по отгрузкам выставлены'"),
					НСтр("ru = 'Выставить ИСФ/КСФ по отгрузкам (%1)'"),
					НСтр("ru = 'Нет отгрузок клиентам'")));
	...
	ЗаголовкиЭлементов.Вставить("ЗаявлениеОВвозеТоваровПолученное", 
			Новый Структура("ЕстьДанныеОбработано, ЕстьДанныеКОбработке, НетДанных",
					НСтр("ru = 'Все заявления о ввозе товаров от покупателей из стран Таможенного союза оформлены'"),
					НСтр("ru = 'Оформить заявления о ввозе товаров от покупателей из стран Таможенного союза (%1)'"),
					НСтр("ru = 'Нет продаж в страны Таможенного союза'")));
	...
	ЗаголовкиЭлементов.Вставить("СчетФактураНаНеподтвержденнуюРеализацию0", 
			Новый Структура("ЕстьДанныеОбработано, ЕстьДанныеКОбработке, НетДанных",
					НСтр("ru = 'Все подтверждения ставки НДС 0% реализаций установлены'"),
					НСтр("ru = 'Оформить подтверждение ставки НДС 0% (%1)'"),
					НСтр("ru = 'Нет реализаций по ставке НДС 0%'")));
