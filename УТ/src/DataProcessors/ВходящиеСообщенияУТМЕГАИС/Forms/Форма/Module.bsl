...
Процедура УдалитьСообщение(Команда)
	...
	ДанныеДляВыполненияОбменаНаКлиенте = ОбменДаннымиЕГАИСВызовСервера.УдалитьВходящиеДокументы(ДокументыКУдалению);
	
	Контекст = Новый Структура;
	Контекст.Вставить("ОповещениеПриЗавершении", ОбменДаннымиЕГАИСКлиент.ОповещениеПослеЗавершенииОбмена(Неопределено, Неопределено, Неопределено, УникальныйИдентификатор));
	Контекст.Вставить("НастройкиОбменаЕГАИС",    ДанныеДляВыполненияОбменаНаКлиенте.НастройкиОбменаЕГАИС);
	Контекст.Вставить("ИдентификаторВладельца",  УникальныйИдентификатор);
...
&НаКлиенте
Процедура ПрочитатьВходящиеСообщения()
...
Процедура НачатьПолучениеСпискаВходящихДокументов(ОрганизацииЕГАИС = Неопределено, ОповещениеПриЗавершении = Неопределено)
	...
	ДанныеДляВыполненияОбменаНаКлиенте = ОбменДаннымиЕГАИСВызовСервера.ВходящиеСообщения(ОрганизацииЕГАИС);
	
	Контекст = Новый Структура;
	Контекст.Вставить("ОповещениеПриЗавершении", ОбменДаннымиЕГАИСКлиент.ОповещениеПослеЗавершенииОбмена(Неопределено, Неопределено, ОповещениеПриЗавершении, УникальныйИдентификатор));
	Контекст.Вставить("НастройкиОбменаЕГАИС",    ДанныеДляВыполненияОбменаНаКлиенте.НастройкиОбменаЕГАИС);
	Контекст.Вставить("ИдентификаторВладельца",  УникальныйИдентификатор);
	Контекст.Вставить("ВОсновнойФорме",          Ложь);
	
	ПараметрыОбработкиСпискаВходящихДокументов = ОбменДаннымиЕГАИССлужебныйКлиент.ОбщиеПараметрыОбменаНаКлиенте(Контекст);
	...
	ДополнительныеПараметры = ОбменДаннымиЕГАИССлужебныйКлиент.ОбщиеПараметрыОбменаНаКлиенте(Контекст);
	...
	ОбменДаннымиЕГАИССлужебныйКлиент.ПолучитьСпискиВходящихСообщений(
		ОбменДаннымиЕГАИСКлиентСервер.ОрганизацииЕГАИС(ДанныеДляВыполненияОбменаНаКлиенте.НастройкиОбменаЕГАИС),
...
Процедура ПолучитьСообщения(ДанныеДокументовКПолучению, Контекст)
	...
	Если ОбменДаннымиЕГАИССлужебныйКлиент.ОбменУжеЗапущен(Контекст) Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыПриложения.Вставить("ЕГАИС.ДанныеСеансаОбмена",
		ОбменДаннымиЕГАИССлужебныйКлиент.СохраняемыеДанныеОбмена(
...
Процедура ПослеПолученияСообщений(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	ОбменДаннымиЕГАИССлужебныйКлиент.ЗакрытьФормуВыполненияОбмена(ДанныеОбменаЕГАИС.Контекст.Форма);
