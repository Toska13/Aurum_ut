...
Процедура ЗаполнитьПериоды()
	...
	Если Не ДокументыОснования.Количество() = 0 Тогда
		...
		ИначеЕсли РасхожденияКлиентСервер.ТипОснованияОтгрузкаТоваровКлиенту(ТипОснованияАктаОРасхождении) Тогда
			...
			ИмяТаблицы = "Документ.ОтгрузкаТоваровКлиенту";
...
Процедура ЗаполнитьТаблицуДокументовОснований()
	...
	ИначеЕсли РасхожденияКлиентСервер.ТипОснованияОтгрузкаТоваровКлиенту(ТипОснованияАктаОРасхождении) Тогда
		...
		ТаблицаОснованийДокумента.Колонки.Добавить("Реализация",
			Новый ОписаниеТипов("ДокументСсылка.ОтгрузкаТоваровКлиенту"));
		...
		|		ИНАЧЕ ИСТИНА
		|	КОНЕЦ КАК Выбран,
		|	ОтгрузкаТоваровКлиенту.Номер,
		|	ОтгрузкаТоваровКлиенту.Дата,
		|	ОтгрузкаТоваровКлиенту.СуммаДокумента,
		|	ОтгрузкаТоваровКлиенту.Ссылка
		|ИЗ
		|	Документ.ОтгрузкаТоваровКлиенту КАК ОтгрузкаТоваровКлиенту
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВыбранныеДокументы КАК ВыбранныеДокументы
		|		ПО ОтгрузкаТоваровКлиенту.Ссылка = ВыбранныеДокументы.Реализация
		|ГДЕ
		|	ВЫБОР 
		|		КОГДА НЕ &ИспользоватьСоглашенияСКлиентами
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ОтгрузкаТоваровКлиенту.Соглашение = &Соглашение
		|	КОНЕЦ
		|	И ОтгрузкаТоваровКлиенту.Контрагент = &Контрагент
		|	И ОтгрузкаТоваровКлиенту.Организация = &Организация
		|	И ОтгрузкаТоваровКлиенту.Партнер = &Партнер
		|	И ОтгрузкаТоваровКлиенту.Валюта = &Валюта
		|	И ОтгрузкаТоваровКлиенту.НалогообложениеНДС = &НалогообложениеНДС
		|	И ОтгрузкаТоваровКлиенту.Договор = &Договор
		...
		|				ТОГДА ИСТИНА
		|			КОГДА &ДатаНачала = &ПустаяДата
		|				ТОГДА ОтгрузкаТоваровКлиенту.Дата <= &ДатаОкончания
		|			КОГДА &ДатаОкончания = &ПустаяДата
		|				ТОГДА ОтгрузкаТоваровКлиенту.Дата >= &ДатаНачала
		|			ИНАЧЕ ОтгрузкаТоваровКлиенту.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
		|		КОНЕЦ
		|
		|УПОРЯДОЧИТЬ ПО
		|	ОтгрузкаТоваровКлиенту.Дата УБЫВ";
...
Процедура СформироватьИнформационнуюНадписьОтборы()
	...
	Если НЕ ОбщегоНазначенияУТКлиентСервер.АвторизованВнешнийПользователь() Тогда
		...
		Если РасхожденияКлиентСервер.ТипОснованияОтгрузкаТоваровКлиенту(ТипОснованияАктаОРасхождении) Тогда
			ИнформационнаяНадписьОтборы = НСтр("ru='Отбор по: %Организация%, %Партнер%,%Контрагент% %Валюта%, %Соглашение%, %НалогообложениеНДС%,%Договор%%ХозяйственнаяОперация%'");
		Иначе
