...
Функция ПротоколыОбменаПоДокументам(ЭлектронныеДокументы) Экспорт
	...
	Попытка
		...
		Для Каждого ЭлектронныйДокумент Из ЭлектронныеДокументы Цикл
			...
			ДанныеФормированияПротокола = ДанныеПротоколовОбменаПоДокументам.Получить(ЭлектронныйДокумент);
			Если ДанныеФормированияПротокола = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			ПротоколыОбменаПоДокументам = Создать();
			ТабличныйДокумент = ПротоколыОбменаПоДокументам.ПредставлениеПротокола(ДанныеФормированияПротокола);
...
// * Ключ - ДокументСсылка.ЭлектронныйДокументВходящийЭДО, 
// 		- ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
// * Значение - см. ОбработкаОбъект.ПротоколОбменаПоДокументуЭДО.НовыеДанныеФормированияПротокола 
Функция ДанныеПротоколовОбменаПоДокументам(ЭлектронныеДокументы)
	...
	Пока ВыборкаДанныеЭД.Следующий() Цикл
		...
		ПротоколыОбменаПоДокументам = Создать();
		ДанныеФормированияПротокола = ПротоколыОбменаПоДокументам.ДанныеФормированияПротокола(ПараметрыФормированияПротокола);
		
		Результат.Вставить(ЭлектронныйДокумент, ДанныеФормированияПротокола);
...
Функция СообщенияДокументовДляПротокола(ЭлектронныеДокументы)
	...
	Для Каждого СтрокаСПустымВидом Из СтрокиСПустымВидом Цикл
		СообщенияДокументов.Удалить(СтрокаСПустымВидом);
	КонецЦикла;
...
Функция ПредставлениеСообщенияДляПротокола(ТипЭлементаРегламента, ТипСлужебногоДокумента)
	...
	ИначеЕсли ЗначениеЗаполнено(ТипСлужебногоДокумента) Тогда
