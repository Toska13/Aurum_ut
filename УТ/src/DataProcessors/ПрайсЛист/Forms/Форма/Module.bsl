...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ЗаполнитьСтавкуНДСПросмотр.Видимость                 = ИспользуетсяЦенообразование25 И ИзменениеУстановкиЦенНоменклатуры И ПоказыватьСтавкуНДС;
	Элементы.ЗаполнитьСтавкуНДСРедактирование.Видимость           = ИспользуетсяЦенообразование25 И ИзменениеУстановкиЦенНоменклатуры И ПоказыватьСтавкуНДС;
	...
	Элементы.ЗначениеИзмененияСтрока.РедактированиеТекста = Ложь;
	...
	Элементы.ДатаДействияЦен1.Подсказка = ТекстПодсказкиПериод;
	Элементы.ДатаДействияЦен1.ОтображениеПодсказки = ОтображениеПодсказки.Кнопка;
	...
	ЦенообразованиеВызовСервера.УстановитьСвойстваДекорацииИнформированиеОбУдаленииЦенообразованияВерсии20(Элементы);
...
Процедура ЗаполнитьСтавкуНДС(Команда)
	...
	УстановкаЦенКлиент.ОткрытьФормуЗаполненияСтавокНДС(ЭтотОбъект, Дата);
...
Процедура ЗаполнитьНастройкиОтображения(СохраненныеНастройки)
	...
	ПоказыватьСтавкуНДС        = СохраненныеНастройки.ПоказыватьСтавкуНДС;
...
Процедура ПолучитьНастройки(НовыеНастройки)
	...
	Для Каждого ЭлементОтбора Из НовыеНастройки.Отбор.Элементы Цикл
		
		Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных")
			И ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТолькоВНаличии") Тогда
			...
			Иначе
				...
				НовыеНастройки.Отбор.Элементы.Удалить(ЭлементОтбора);
...
Процедура СохранитьНастройкиФормыНаСервере()
	...
	СтруктураДанных.ПоказыватьСтавкуНДС 					= ПоказыватьСтавкуНДС;
...
Процедура ПрименитьНастройки(АдресВоВременномХранилище = Неопределено, ДанныеРедактированияПрайсЛиста = Неопределено)
	...
	Если ЗначениеЗаполнено(ДанныеРедактированияПрайсЛиста) Тогда
		Если ДанныеРедактированияПрайсЛиста.Свойство("ИмяМодуля") Тогда
			ОбщегоНазначения.ОбщийМодуль(ДанныеРедактированияПрайсЛиста.ИмяМодуля).НастройкиПрайсЛиста(ЭтотОбъект, ДанныеРедактированияПрайсЛиста);
		Иначе
...
Процедура ЗаписатьИзмененияЦенЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ИспользуетсяЦенообразование25 Тогда
		Отказ = Ложь;
		УстановкаЦенКлиентСервер.ПроверитьЗаполнениеСтавкиНДСДереваЦен(ЭтотОбъект, Отказ, Истина);
		Если Отказ Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьВидимостьКолонокРедактирования(Форма, ИспользоватьРежимРедактирования)
		...
		Если СтрНайти(ТекЭлемент.Имя, "ГруппаЦены" + "ВидЦены") Тогда
			...
			Для Каждого ВложенныйЭлемент Из ТекЭлемент.ПодчиненныеЭлементы Цикл
				...
				Если СтрНайти(ВложенныйЭлемент.Имя, "СтавкаНДС") Тогда
					ВложенныйЭлемент.Видимость = Форма.ПоказыватьСтавкуНДС;
				КонецЕсли;
	...
	Если Форма.ИспользуетсяЦенообразование25 Тогда
		...
		Форма.Элементы.ЗаполнитьСтавкуНДСПросмотр.Видимость = Форма.ПоказыватьСтавкуНДС;
		Форма.Элементы.ЗаполнитьСтавкуНДСРедактирование.Видимость = Форма.ПоказыватьСтавкуНДС;
	Иначе
...
&НаКлиенте
Процедура ОткрытьОкноОжиданияИзмененияСтавкиНДСВФоне(Знач ИмяПроцедуры)
	
	Если ЗначениеЗаполнено(ИдентификаторЗадания) Тогда
		ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
		ПараметрыОжидания.ВыводитьОкноОжидания  = Истина;
		ПараметрыОжидания.ВыводитьСообщения     = Истина;
		ПараметрыОжидания.ТекстСообщения     	= НСтр("ru='Идет изменение ставки НДС прайс-листа'");
...
Процедура НастройкиЗаполненияСтавокНДСЗавершение(Результат, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИзменитьСтавкиНДС(Результат, ДополнительныеПараметры);
	
КонецПроцедуры

&НаКлиенте
Процедура ИзменитьСтавкиНДС(Результат, ДополнительныеПараметры = Неопределено)
	...
	Если ЗапуститьАсинхронно(ПараметрыРасчета) Тогда
		...
		РезультатРасчетаДлительнойОперации = ИзменитьСтавкиНДСНаСервереВФоне(ПараметрыРасчета);
		ИдентификаторЗадания						= РезультатРасчетаДлительнойОперации.ИдентификаторЗадания;
		АдресХранилищаРезультатаВыполненияЗадания 	= РезультатРасчетаДлительнойОперации.АдресРезультата;
		...
		Если РезультатРасчетаДлительнойОперации.Статус = "Выполнено" Тогда
			ИзменитьЦеныНаКлиентеЗавершениеОбщая(РезультатРасчетаДлительнойОперации);
		иначе	
			ОткрытьОкноОжиданияИзмененияСтавкиНДСВФоне("ИзменитьЦеныНаКлиентеЗавершениеОбщая")
		КонецЕсли;
			
	Иначе
			
		ИзменитьСтавкиНДСНаСервере(ПараметрыРасчета);
		ИзменитьЦеныНаКлиентеЗавершениеФрагментОбщая();
...
&НаСервере
Функция ИзменитьСтавкиНДСНаСервереВФоне(Знач ПараметрыРасчета)
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("Обработки.ПрайсЛист.ИзменитьСтавкиНДСНаСервере",
											ПараметрыЗадания,
											ПараметрыВыполнения);
