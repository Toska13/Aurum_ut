...
Процедура НаложенныйПлатежВидОплатыПриИзменении(Элемент)
	УстановитьВидимостьДоступностьНаложенныйПлатеж(ЭтотОбъект);
	УстановитьВидимостьДоступностьКодМаркировки(ЭтотОбъект);
КонецПроцедуры
...
Процедура УстановитьУсловноеОформление()
	...
	Для Каждого ЭлементРазмещениеГруза Из Элементы.РазмещениеГруза.ПодчиненныеЭлементы Цикл
		...
		Если Не ЭлементРазмещениеГруза = Элементы.РазмещениеГрузаНаименование
			И Не ЭлементРазмещениеГруза = Элементы.РазмещениеГрузаАртикул
			И Не ЭлементРазмещениеГруза = Элементы.РазмещениеГрузаКодМаркировки Тогда
	...
	// Скрытие колонки "КодМаркировки"
	ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь);
	
	ПолеЭлемента = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.РазмещениеГрузаКодМаркировки.Имя);
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ГрузВидРасшифровки");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно;
	ОтборЭлемента.ПравоеЗначение = "ПоТоварам";
	
	ОтборЭлемента = ГруппаОтборов.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТипЗаказа");
	ОтборЭлемента.ПравоеЗначение = Кэш.ТипыЗаказов.Доставка;
...
Процедура УстановитьКэшированныеЗначения()
	...
	ДанныеПоУмолчанию = СервисДоставки.ПараметрыПоУмолчанию(ТипГрузоперевозки);
	Кэш.Вставить("ТипыЗаказов", СервисДоставкиКлиентСервер.ТипыЗаказовСДЭК());
	Кэш.Вставить(ИмяКэшаГрузовогоМестаПоУмолчанию(), ЗначенияГрузовогоМестаПоУмолчанию(ДанныеПоУмолчанию));
	Кэш.Вставить(ИмяКэшаВыгружатьКодыМаркировки(), ДанныеПоУмолчанию.ВыгружатьКодыМаркировки);
...
Процедура УстановитьВидимостьДоступностьНаСтраницеПараметрыГруза(Знач Форма)
	...
	УстановитьВидимостьДоступностьКодМаркировки(Форма);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьВидимостьДоступностьКодМаркировки(Знач Форма)
	
	Форма.Элементы.РазмещениеГрузаКодМаркировки.Видимость = Форма.Кэш[ИмяКэшаВыгружатьКодыМаркировки()]
		И Форма.Элементы.ГруппаНаложенныйПлатежПраво.Видимость
		И Форма.НаложенныйПлатежВидОплаты <> 0;
...
&НаСервереБезКонтекста
Функция ЗначенияГрузовогоМестаПоУмолчанию(ДанныеПоУмолчанию)
	
	ИменаПараметров = Новый Структура;
	ИменаПараметров.Вставить("ВесГрузовогоМеста");
	ИменаПараметров.Вставить("ДлинаГрузовогоМеста");
	ИменаПараметров.Вставить("ШиринаГрузовогоМеста");
	ИменаПараметров.Вставить("ВысотаГрузовогоМеста");
	ЗаполнитьЗначенияСвойств(ИменаПараметров, ДанныеПоУмолчанию);
...
&НаКлиентеНаСервереБезКонтекста
Функция ИмяКэшаВыгружатьКодыМаркировки()
	Возврат "ВыгружатьКодыМаркировки";
КонецФункции
...
Процедура ПроверитьПоказателиГрузовыхМест(ЕстьОшибки, ВыводитьПредупреждения)
	...
	Для Каждого ГрузовоеМесто Из РазмещениеГрузаЭлементы Цикл
		...
		Если ТипЗаказа = Кэш.ТипыЗаказов.ИнтернетМагазин Тогда // Интернет-магазин
			ТоварныйСостав = ГрузовоеМесто.ПолучитьЭлементы();
			Если ТоварныйСостав.Количество() = 0 Тогда
				...
				КонецЕсли;
			КонецЕсли;
			Если Элементы.РазмещениеГрузаКодМаркировки.Видимость Тогда
				Для Каждого Строка Из ТоварныйСостав Цикл
					Если ЗначениеЗаполнено(Строка.КодМаркировки) И Строка.Количество > 1 Тогда
						ЕстьОшибки = Истина;
						Если ВыводитьПредупреждения Тогда
							ТекстОшибки = НСтр("ru = 'Для товара ""%1"" указано Количество больше 1.
								|Для маркируемого товара Количество не может быть больше 1'");
							ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
								Строка.Наименование);
							ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
						КонецЕсли;
	...
	Для Каждого ГрузовоеМесто Из РазмещениеГруза.ПолучитьЭлементы() Цикл
		...
		Если Товары <> Неопределено Тогда
			Для Каждого ТекущаяПозиция Из ГрузовоеМесто.ПолучитьЭлементы() Цикл
				...
				Если Не Элементы.РазмещениеГрузаКодМаркировки.Видимость Тогда
					НоваяПозиция.КодМаркировки = "";
				КонецЕсли;
