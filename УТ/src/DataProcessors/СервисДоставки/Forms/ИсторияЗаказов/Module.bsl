...
Процедура ОткрытьФормуПросмотраЗаказа(ДанныеЗаказа)
	...
	ОткрытьФорму("Обработка.СервисДоставки.Форма.ПросмотрЗаказа",
		ПараметрыОткрытияФормы,
		ЭтотОбъект,,,,,
...
&НаКлиенте
Процедура ВыполнитьЗапросПолучитьСписокСостоянийЗаказов()
...
Функция ВыполнитьЗапросВФоне(ИнтернетПоддержкаПодключена, ПараметрыОперации)
	...
	Если ПараметрыОперации.ИмяПроцедуры = СервисДоставкиКлиентСервер.ИмяПроцедурыПолучитьСписокСостоянийЗаказовИзСервиса() Тогда
		Возврат ДлительныеОперации.ВыполнитьВФоне("СервисДоставки.ПолучитьСписокСостоянийЗаказовИзСервиса",
			Задание.ПараметрыПроцедуры, ПараметрыВыполнения);
	ИначеЕсли ПараметрыОперации.ИмяПроцедуры = СервисДоставкиКлиентСервер.ИмяПроцедурыПолучитьЖурналЗаказовНаДоставкуИзСервиса() Тогда
		Возврат ДлительныеОперации.ВыполнитьВФоне("СервисДоставки.ПолучитьЖурналЗаказовНаДоставкуИзСервиса",
			Задание.ПараметрыПроцедуры, ПараметрыВыполнения);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
