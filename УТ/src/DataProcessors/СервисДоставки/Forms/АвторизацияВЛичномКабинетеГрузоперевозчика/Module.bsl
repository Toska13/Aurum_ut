...
&НаКлиенте
Процедура НаименованиеОрганизации1Очистка(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
КонецПроцедуры

&НаКлиенте
Процедура ИдентификаторОрганизации1Очистка(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
КонецПроцедуры
#КонецОбласти
...
Функция ПараметрыЗапросаЗаписатьНастройкиАвторизации(ПараметрыОперации, Отказ)
	...
	ЗаполнитьЗначенияСвойств(ПараметрыЗапроса, ДанныеОрганизации);
	ПараметрыЗапроса.Конфигурация = Константы.ЗаголовокСистемы.Получить();
	Если ЗначениеЗаполнено(ПараметрыЗапроса.Конфигурация) Тогда
		ПараметрыЗапроса.Конфигурация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 / %2",
			ПараметрыЗапроса.Конфигурация, Метаданные.КраткаяИнформация);
	Иначе
		ПараметрыЗапроса.Конфигурация = Метаданные.КраткаяИнформация;
	КонецЕсли;
...
Функция ПараметрыЗапросаОтправитьЗапросНаПолучениеРегистрационныхДанных(ПараметрыОперации, Отказ)
	...
	ПараметрыЗапроса.ИдентификаторПунктаВыдачи = Строка(Новый УникальныйИдентификатор());
