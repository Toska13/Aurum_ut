...
Процедура ДекорацияУдалитьСпособОплатыНажатие(Элемент)
	...
	СброситьРеквизитыГруппыОплатаДополнительная();
	ПриИзмененииРеквизита("КОплатеДополнительно");
	УстановитьВидимостьГруппаОплатаДополнительная();
...
Процедура ЗагрузитьРезультатПолученияЗаказаНаДоставку(Результат, ДанныеОбновлены = Истина)
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если Результат.Свойство("Данные") Тогда
			...
			// Заполнение реквизитов формы и документа
			...
			// Пересчет значений в зависимых реквизитах шапки
			ОбработатьИзменениеРеквизитаФормыНаСервере("СуммаДокумента");
...
Процедура ЗагрузитьРезультатПолученияСпискаПунктовВыдачиКлиента(Результат)
	...
	Если ЗначениеЗаполнено(Склад) Тогда
		ПунктВыдачиКлиентаИдентификатор = 
			СервисДоставки.ПолучитьПунктВыдачиКлиента(ОрганизацияБизнесСетиСсылка, ТипГрузоперевозки, Склад);
	КонецЕсли;
...
&НаКлиенте
Процедура УстановитьВидимостьГруппаОплатаДополнительная()
	Элементы.ГруппаОплатаДополнительная.Видимость = Ложь;
	Элементы.ДекорацияДобавитьСпособОплаты.Видимость = Истина;
КонецПроцедуры

&НаКлиенте
Процедура СброситьРеквизитыГруппыОплатаДополнительная()
...
Процедура ПроверитьЗаполнениеЗаказаПередОтправкой(Отказ)
	...
	ПроверитьНомерТелефона("ПолучательКонтактноеЛицоТелефон", Отказ);
...
Процедура УстановитьДоступностьЭлементовПоЗначениюОпции()
	...
	Элементы.ГруппаИнформацияСостояние.Доступность = ОпцияДоступнаКурьерика;
	Элементы.ГруппаКоманднаяПанельДействия.Доступность = ОпцияДоступнаКурьерика;
	Элементы.ГруппаСтраницы.ТолькоПросмотр = Не ОпцияДоступнаКурьерика;
	Элементы.ДекорацияДобавитьСпособОплаты.Доступность = ОпцияДоступнаКурьерика;
...
Процедура УстановитьОформлениеИсполнителя(Данные)
	...
	Если ЗначениеЗаполнено(Данные.ВодительНаименованиеДоставки) Тогда
		Элементы.Курьер.ЦветТекста = ЦветаСтиля.ЦветТекстаПоля;
		Курьер = СтрШаблон("%1%2", СокрЛП(Данные.ВодительНаименованиеДоставки),
			?(ЗначениеЗаполнено(Данные.ВодительНомерТелефонаДоставки), ", " + СокрЛП(Данные.ВодительНомерТелефонаДоставки), ""));
	Иначе
		Элементы.Курьер.ЦветТекста = ЦветТекстаПодсказки;
		Курьер = НСтр("ru = '<не назначен>'");
	КонецЕсли;
...
Процедура ОбработатьИзменениеКонтактнойИнформации(Элемент, ТипКонтактнойИнформацииСтрока)
	...
	Модифицированность = Истина;
	ЕстьОшибки = Ложь;
	ПроверитьПредставлениеКонтактнойИнформации(ИмяРеквизита, ЕстьОшибки);
