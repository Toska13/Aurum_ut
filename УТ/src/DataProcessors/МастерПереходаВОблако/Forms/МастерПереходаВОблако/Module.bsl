...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РазделениеВключено Тогда
		Элементы.ПараметрыСравненияШапка.Заголовок = НСтр("ru = 'Выберите вариант
														  |переноса:'");
		Элементы.Параметр1.Заголовок = НСтр("ru = 'Можно продолжать работу в текущем приложении в процессе переноса'");
		Элементы.Параметр2.Заголовок = НСтр(
			"ru = 'Данные автоматически обновятся до нужной версии в процессе переноса'");
		Элементы.СпособПереходаФайл.СписокВыбора[0].Представление =	НСтр(
			"ru = 'Получите файл выгрузки для ручного переноса.'");
		Элементы.СпособПереходаОблако.СписокВыбора[0].Представление = НСтр("ru = 'Введите адрес сервиса
	...
	Если ПараметрыПараллельнойЗагрузки.ДоступноИспользование Тогда
	...
	ПроверкаНаличияРасширений1СПлюс();
КонецПроцедуры
...
Процедура РасширенияДляВосстановленияИспользоватьРасширениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ИзменяетСтруктуруДанных",
		Элементы.РасширенияДляВосстановления.ТекущиеДанные.ИзменяетСтруктуруДанных);
	Оповещение = Новый ОписаниеОповещения("ОбработатьВыборРасширения", ЭтотОбъект);
	ОткрытьФорму(
		"Обработка.МастерПереходаВОблако.Форма.ВыборРасширений", 
...
Функция ПроверитьВозможностьАвтоматическогоПереходаСРасширениями()	
	...
	СтрокиВопроса.Добавить(НСтр("ru = 'Для продолжения необходимо удалить все расширения, изменяющие структуру данных, или использовать вариант ручного перехода, или порционную выгрузку данных.'"));
...
Процедура ЗапуститьВыгрузкуДанныхНаСервере()
	...
	Попытка
		...
		ПараметрыВыгрузки.Вставить(
			"ВидЗадания",
			Перечисления.ВидыЗаданийВыгрузкиЗагрузкиДанных.ИнтерактивнаяВыгрузкаДанных);
		...
		ПараметрыВыгрузки.Вставить("ВыгружатьДанныеРасширений1СПлюс", Истина);
		ПараметрыВыгрузки.Вставить("НомерКлиентскогоСеанса", НомерСеансаИнформационнойБазы());
		...
		Задание = ФоновыеЗадания.Выполнить(
			"ВыгрузкаЗагрузкаОбластейДанных.ВыгрузитьТекущуюОбластьВАрхив", ПараметрыЗадания,,
...
Асинх Процедура ПроверитьГотовностьВыгрузки() 
	...
	Если ЗначениеЗаполнено(ТекстПредупрежденийВыгрузки) Тогда
		...
		ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения(
			"ПодтверждениеСохраненияФайлаВыгрузкиЗавершение",
			ЭтотОбъект);
...
&НаКлиенте
Асинх Процедура ПодтверждениеСохраненияФайлаВыгрузкиЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Функция ПолучитьИмяФайлаВыгрузкиДанных(Знач ЭтоРежимВыгрузкиДляТехническойПоддержки)
 	...
 	ИмяФайла = СтрСоединить(ЧастиИмениФайла, "_") + ".zip";
 	
 	Возврат СтроковыеФункции.СтрокаЛатиницей(ИмяФайла);
...
&НаСервере
Процедура ПроверкаНаличияРасширений1СПлюс()
	Расширения1СПлюс = Новый Соответствие();
	ВыгрузкаЗагрузкаДанныхСлужебный.ЗаполнитьУстановленныеРасширения1СПлюс(Расширения1СПлюс);
	УстановленыРасширения1СПлюс = Расширения1СПлюс.Количество() > 0;
КонецПроцедуры
...
Процедура ЗапуститьПроверкуВыгружаемыхДанныхНаСервере()
	...
	Попытка
		...
		Задание = ФоновыеЗадания.Выполнить(
			"ВыгрузкаЗагрузкаДанных.ПоместитьОшибкиВыгружаемыхДанныхВоВременноеХранилище",
			ПараметрыЗадания,, НСтр("ru = 'Проверка выгружаемых данных'"));
...
&НаКлиенте
Процедура ПроверитьРасширения1СПлюс()
	ПараметрыЗапроса = Новый Структура;
	ПараметрыЗапроса.Вставить("ОбработчикРезультата", "Подключаемый_ПроверкаРасширений1СПлюс");
	ПараметрыЗапроса.Вставить("Ключ", УникальныйИдентификатор);
	ОбработчикРезультата = ПараметрыЗапроса.ОбработчикРезультата;
	
	НачатьПроверкуРасширений1СПлюс(ПараметрыЗапроса, АдресХранилища, ИдентификаторЗадания);
	
	ПараметрыОжидания = ШаблонПараметровОжидания();
	ПараметрыОжидания.ЗаголовокСтраницы = НСтр("ru = 'Проверка расширений'");
	ПараметрыОжидания.ОписаниеСостояния = НСтр("ru = 'Выполняется проверка контрольных сумм расширений 1С:Плюс...'");
	ПараметрыОжидания.ДопустимоеОжидание = 5;
	УстановитьСтраницуОжидание(Элементы.Страницы.ТекущаяСтраница, ПараметрыОжидания);
КонецПроцедуры

&НаСервереБезКонтекста
Процедура НачатьПроверкуРасширений1СПлюс(ПараметрыЗапроса, АдресХранилища, ИдентификаторЗадания)
	
	НачатьВыполнениеФоновогоЗаданияНаСервере("ПроверитьРасширения1СПлюс", 
		ПараметрыЗапроса, АдресХранилища, ИдентификаторЗадания);
	
КонецПроцедуры

&НаКлиенте
Асинх Процедура Подключаемый_ПроверкаРасширений1СПлюс(Результат, ДополнительныеПараметры) Экспорт	
	Если Результат.Свойство("Данные") И ТипЗнч(Результат.Данные) = Тип("Массив") И Результат.Данные.Количество() > 0 Тогда
		СтрокиВопроса = Новый Массив;
		СтрокиВопроса.Добавить(НСтр("ru = 'Обнаружены отличия в расширениях 1С:Плюс от исходных:'"));
		Для Каждого Расширение Из Результат.Данные Цикл
			СтрокиВопроса.Добавить(СтрШаблон(НСтр("ru = '● %1 версия %2'"), Расширение.Имя, Расширение.Версия));
		КонецЦикла;
		СтрокиВопроса.Добавить("");
		СтрокиВопроса.Добавить(НСтр("ru = 'При загрузке в сервис эти расширения будут заменены на исходные версии'"));
		
		Ждать ВопросАсинх(СтрСоединить(СтрокиВопроса, Символы.ПС), РежимДиалогаВопрос.ОК);
	КонецЕсли;
	
	Проверка1СПлюсВыполнена = Истина;
	
	УстановитьСтраницуПриветствие(Элементы.Страницы.ТекущаяСтраница);
	ПереходДалее(Элементы.Страницы.ТекущаяСтраница);
КонецПроцедуры
...
Процедура ПереходДалее(ТекущаяСтраница)
	...
	Если ТекущаяСтраница = Элементы.СтраницаПриветствие И ПроверитьЗаполнение() Тогда
		Если Не Проверка1СПлюсВыполнена И УстановленыРасширения1СПлюс Тогда
			ПроверитьРасширения1СПлюс();
...
Процедура УстановитьСтраницуСпособПерехода(ТекущаяСтраница)
	...
	Заголовок = СтрШаблон(ШаблонЗаголовка, ВсегоШаговМастера());
	УстановитьКонтролыПоУмолчанию();
	Если МиграцияДоступна Или ВыгрузкаДоступна Тогда
		Элементы.Далее.Доступность = Истина;
		Если СпособПерехода = -1 Тогда
			СпособПерехода = СпособВыгрузка();
		КонецЕсли;
	Иначе 
...
Процедура ПерейтиВФормуМиграцииПриложения()
	...
	ФормаМиграции.КоличествоШагов = ВсегоШаговМастера();
...
Функция ЗаголовокСопоставлениеПользователей()
	...
	Возврат СтрШаблон(НСтр("ru = 'Шаг %1 из %2: Сопоставление пользователей'"), ?(РасширенияДляВосстановления.Количество() = 0, 3, 4), ВсегоШаговМастера());
...
Функция ЗаголовокВыгрузкаДанных()
	...
	Возврат ?(ВыполняетсяСохранениеФайла, 
		НСтр("ru = 'Выгрузка данных'"), 
			СтрШаблон(НСтр("ru = 'Шаг %1 из %1: Выгрузка данных'"), ?(ПользователиИБ.Количество() = 0, 4, ВсегоШаговМастера())));
...
Процедура НачатьЗапросПоНастройкам(ПараметрыЗапроса, ОбработчикРезультата, АдресХранилища, ИдентификаторЗадания)
	...
	ИмяМетода = "ПолучитьИнформациюОВозможностяхПерехода";
    ...
    НачатьВыполнениеФоновогоЗаданияНаСервере(ИмяМетода, ПараметрыЗапроса, АдресХранилища, ИдентификаторЗадания);
...
Процедура ОбработатьВыборРасширенияНаСервере(ИмяРасширения, ТекущаяСтрока)
	...
	Если ТекСтрока.ИзменяетСтруктуруДанных <> СтрокаРасширения.ИзменяетСтруктуруДанных Тогда
		Если СтрокаРасширения.ИзменяетСтруктуруДанных Тогда
			Текст = НСтр("ru = 'Обратите внимание: Расширение ""%1"" не изменяет структуру данных,
				|но выбрано расширение изменяющее структуру данных.'");
		КонецЕсли;
...
Процедура ЗаполнитьРасширенияИБ()
	...
	Для Каждого РасширениеКонфигурации Из РасширенияКонфигурации.Получить(, ИсточникРасширенийКонфигурации.СеансАктивные) Цикл
		Если Не РасширениеКонфигурации.Активно 
			Или Расширения1СПлюс.Получить(РасширениеКонфигурации.УникальныйИдентификатор) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
	...
	Если Результат <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(Результат.Идентификатор) Тогда
			...
			Если Поиск.Количество() > 0 Тогда
				...
				НоваяСтрока.Пользователь = Поиск[0].Пользователь;
				ДополнитьСписокНеСопоставленнымиПользователями();
				ОбновитьСтатусСопоставленияПользователей();
			КонецЕсли; 
		КонецЕсли; 
...
Процедура СопоставитьПользователей()
	...
	Для Каждого Строка Из ПраваПользователей Цикл
		...
		Если Поиск.Количество() > 0 И Не ЗначениеЗаполнено(Поиск[0].ЛогинПользователяСервиса) Тогда
			...
			Строка.Пользователь = Поиск[0].Пользователь;
...
Процедура НачатьВыполнениеФоновогоЗаданияНаСервере(ИмяМетода, ПараметрыЗапроса, АдресХранилища, ИдентификаторЗадания)
	...
	ИначеЕсли ИмяМетода = "ВыполнитьМетодВнешнегоИнтерфейса" Тогда
		...
		ПараметрыВыполненияОбработки.Вставить("Метод", ПараметрыЗапроса.Метод);
		ПараметрыВыполненияОбработки.Вставить("Данные", ПараметрыЗапроса.Данные);
	ИначеЕсли ИмяМетода = "ПередатьФайл" Или ИмяМетода = "ПроверитьРасширения1СПлюс" Тогда
	...
	Результат = ДлительныеОперации.ВыполнитьВФоне(
		"ДлительныеОперации.ВыполнитьПроцедуруМодуляОбъектаОбработки", ПараметрыЗадания, ПараметрыВыполнения);
