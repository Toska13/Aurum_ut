...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НастроитьПоддержкуБанковскогоИКазначейскогоСопровождения();
...
Процедура Выгрузить(Команда)
	...
	ПараметрыОтбора.Вставить("ФорматОбмена", ПредопределенноеЗначение("Перечисление.ФорматОбменаСБанком.ФедеральноеКазначействоРФ"));
	НайденныеОшибки = Объект.БанковскиеСчета.НайтиСтроки(ПараметрыОтбора);
	
	Если НайденныеОшибки.Количество() > 0
		И Не ПоддержкаБанковскогоИКазначейскогоСопровождения Тогда
		
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Выбраны счета, для которых установлен формат обмена ""Федеральное казначейство РФ"". Требуется включить настройку ""Поддержка банковского и казначейского сопровождения госконтрактов"".'"));
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		Возврат;
		
	КонецЕсли;
	
	ПараметрыОтбора.Вставить("КодУБП", "");
	НайденныеОшибки = Объект.БанковскиеСчета.НайтиСтроки(ПараметрыОтбора);
	
	Для Каждого Счет Из НайденныеОшибки Цикл
		
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Не заполнено поле ""Код УБП"" для владельца счета %1.'"), Счет.Ссылка);
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , Элементы.БанковскиеСчетаНастройкаОбмена.Имя);
	...
	ПодключитьОжиданиеФоновыхЗаданий();
...
Процедура ОбработатьЗавершениеВыгрузки()
	...
	ВыгруженныеПлатежи = Новый Массив;
	
	Для каждого Счет Из Объект.БанковскиеСчета Цикл
		Если Счет.СохранитьФайл Тогда
			ЕстьФайлыДляСохранения = Истина;
			ВыгруженныеПлатежи.Добавить(Счет.АдресХранилищаДокументов);
	...
	ОбработатьЗавершениеВыгрузкиНаСервере(ЕстьНеобходимостьОбновленияСпискаПлатежей, ВсегоФайлов, ВыгруженныеПлатежи);
...
&НаКлиенте
Процедура ПодключитьОжиданиеФоновыхЗаданий();
	
	ПотокиВыгрузки = Новый Соответствие;
	
	Для Каждого Счет Из Объект.БанковскиеСчета Цикл
		
		Если ЗначениеЗаполнено(Счет.ИдентификаторВыгрузки) Тогда
			
			Поток = ПотокиВыгрузки.Получить(Счет.ИдентификаторВыгрузки); // Массив
			
			Если Поток = Неопределено Тогда
				
				СписокСчетов = Новый Массив;
				СписокСчетов.Добавить(Счет);
				ПотокиВыгрузки.Вставить(Счет.ИдентификаторВыгрузки, СписокСчетов);
	...
	Для Каждого ТекущийПоток Из ПотокиВыгрузки Цикл
		
		Счет = ТекущийПоток.Значение[0];
		
		Если ЗначениеЗаполнено(Счет.ДанныеВыгрузки)
			И ЭтоАдресВременногоХранилища(Счет.ДанныеВыгрузки) Тогда
			
			ФоновоеЗадание = ПолучитьИзВременногоХранилища(Счет.ДанныеВыгрузки);
			ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
			ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
			ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбработатьЗавершениеВыгрузкиОповещение", ЭтотОбъект, ТекущийПоток.Значение);
...
&НаКлиенте
Процедура ОбработатьЗавершениеВыгрузкиОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Для Каждого Счет Из ДополнительныеПараметры Цикл
...
&НаСервере
Процедура ОбработатьЗавершениеВыгрузкиНаСервере(ЕстьНеобходимостьОбновленияСпискаПлатежей, ВсегоФайлов, ВыгруженныеПлатежи)
	...
	ВывестиСообщенияПоВыгруженнымПлатежам(ВыгруженныеПлатежи);
...
&НаСервере
Процедура НастроитьПоддержкуБанковскогоИКазначейскогоСопровождения()

	ПоддержкаБанковскогоИКазначейскогоСопровождения = ПолучитьФункциональнуюОпцию("ПоддержкаБанковскогоИКазначейскогоСопровожденияГосконтрактов");
	Элементы.ГруппаКаталогВыгрузкиПодтверждающихДокументов.Видимость = ПоддержкаБанковскогоИКазначейскогоСопровождения;
...
&НаСервере
Процедура ВывестиСообщенияПоВыгруженнымПлатежам(ВыгруженныеПлатежи)

	Для каждого ТекущийФайл Из ВыгруженныеПлатежи Цикл
		
		Платежи = ПолучитьИзВременногоХранилища(ТекущийФайл);
		...
		Для Каждого ТекущийПлатеж Из Платежи Цикл
			...
			Если ТекущийПлатеж.ОшибкиВыгрузки <> "" Тогда
				
				ТекстОшибки = СтрШаблон(НСтр("ru = '%1:
							|%2%3'"), ТекущийПлатеж.Ссылка, Символы.Таб, ТекущийПлатеж.ОшибкиВыгрузки);
