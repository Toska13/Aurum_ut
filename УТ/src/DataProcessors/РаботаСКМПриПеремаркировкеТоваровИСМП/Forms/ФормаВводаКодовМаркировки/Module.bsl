...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнтеграцияИСМПКлиентСервер.УстановитьКартинкуСканированияКодаПоВидуПродукции(КартинкаСканирования, ВидыПродукции);
	ИнтеграцияИСМПКлиентСервер.УстановитьКартинкуСканированияКодаПоВидуПродукции(КартинкаСканированияНовая, ВидыПродукции);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	ДанныеНовогоКодаМаркировки = Неопределено;
	...
	Если Не ЗначениеЗаполнено(Объект.КодМаркировки) Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(Объект.НовыйКодМаркировки) Тогда
			Если ДанныеНовогоКодаМаркировки = Неопределено Тогда
			...
			КонецЕсли;
			Если ДанныеНовогоКодаМаркировки.ВидУпаковки = Перечисления.ВидыУпаковокИС.Набор
				Или ДанныеНовогоКодаМаркировки.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая Тогда
				ОшибкаЗаполненияСтарогоКода = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОшибкаЗаполненияСтарогоКода Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , "Объект.КодМаркировки", , Отказ);
	...
	ИначеЕсли Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(Объект.ТоварнаяГруппа)
	...
	Если ЗначениеЗаполнено(Объект.НовыйКодМаркировки)
		И Не ЗначениеЗаполнено(Объект.КодМаркировки)
		...
		КонецЕсли;
		Если ДанныеНовогоКодаМаркировки.ВидУпаковки <> Перечисления.ВидыУпаковокИС.Набор
			И ДанныеНовогоКодаМаркировки.ВидУпаковки <> Перечисления.ВидыУпаковокИС.Групповая Тогда
			ТекстСообщения = НСтр("ru = 'Поле ""Код ТНВЭД"" не заполнено'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , "Объект.КодТНВЭД", , Отказ);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗавершитьВвод(Отказ=Ложь, ЗакрытьФорму=Ложь)
	...
	Если Объект.РезультатВзятьИзПула Тогда
		...
		ВидПродукции = КлючВидаПродукцииНоменклатуры(Объект.НоваяНоменклатура);
		...
		СтруктураПечатиЭтикетки.ВидПродукции       = ВидПродукции;
		...
		СтруктураПечатиЭтикетки.СпособВводаВОборот = ПеремаркировкаТоваровКлиентСервер.СпособВводаВОборотПриПеремаркировке(ВидПродукции);
		...
		СтруктураПараметров.ВидПродукции          = ВидПродукции;
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Элементы.СтранаПроизводства.Видимость = ПеремаркировкаТоваровКлиентСервер.ДоступноПолеСтранаПроизводства(ВидыПродукции);
	Элементы.Цвет.Видимость               = ПеремаркировкаТоваровКлиентСервер.ДоступноПолеЦвет(ВидыПродукции);
	Элементы.Размер.Видимость             = ПеремаркировкаТоваровКлиентСервер.ДоступноПолеРазмер(ВидыПродукции);
	Элементы.Сертификация.Видимость       = ПеремаркировкаТоваровКлиентСервер.ДоступноПолеСертификация(ВидыПродукции);
	
	ДоступныеПричиныПеремаркировки = ПеремаркировкаТоваровКлиентСервер.ДопустимыеПричиныПеремаркировкиКодовМаркировки(ВидыПродукции);
	
	Если ДоступныеПричиныПеремаркировки.Найти(ПредопределенноеЗначение(
		"Перечисление.ПричиныПеремаркировкиТоваровИСМП.ОшибкиОписанияТовара")) = Неопределено Тогда
		Элементы.ПричинаПеремаркировкиОшибкиОписанияТовара.Видимость = Ложь;
	КонецЕсли;
	
	Если ДоступныеПричиныПеремаркировки.Найти(ПредопределенноеЗначение(
		"Перечисление.ПричиныПеремаркировкиТоваровИСМП.РеализацияТоваровПриобретенныхНеДляПродажи")) = Неопределено Тогда
	...
	Если ДоступныеПричиныПеремаркировки.Найти(ПредопределенноеЗначение(
		"Перечисление.ПричиныПеремаркировкиТоваровИСМП.ИспорченоУтрачено")) <> Неопределено Тогда
		Элементы.ПричинаПеремаркировкиИспорченоУтрачено.СписокВыбора.Добавить(
			ПредопределенноеЗначение("Перечисление.ПричиныПеремаркировкиТоваровИСМП.ИспорченоУтрачено"), НСтр(
			"ru = 'продаже или на складе'"));
	КонецЕсли;
	
	Если ДоступныеПричиныПеремаркировки.Найти(ПредопределенноеЗначение(
		"Перечисление.ПричиныПеремаркировкиТоваровИСМП.ВозвратОтРозничногоПокупателя")) <> Неопределено Тогда
		Элементы.ПричинаПеремаркировкиИспорченоУтрачено.СписокВыбора.Добавить(
			ПредопределенноеЗначение("Перечисление.ПричиныПеремаркировкиТоваровИСМП.ВозвратОтРозничногоПокупателя"), НСтр(
			"ru = 'возврате от розничного покупателя'"));
	КонецЕсли;
	
	Если ДоступныеПричиныПеремаркировки.Найти(ПредопределенноеЗначение(
		"Перечисление.ПричиныПеремаркировкиТоваровИСМП.ВозвратВСлучаеДистанционнойПродажи")) <> Неопределено Тогда
		Элементы.ПричинаПеремаркировкиИспорченоУтрачено.СписокВыбора.Добавить(
			ПредопределенноеЗначение("Перечисление.ПричиныПеремаркировкиТоваровИСМП.ВозвратВСлучаеДистанционнойПродажи"), НСтр(
			"ru = 'возврате в случае дистанционной продажи'"));
	КонецЕсли;
	
	Если ДоступныеПричиныПеремаркировки.Найти(ПредопределенноеЗначение(
		"Перечисление.ПричиныПеремаркировкиТоваровИСМП.ВозвратОтЮридическогоЛица")) <> Неопределено Тогда
		Элементы.ПричинаПеремаркировкиИспорченоУтрачено.СписокВыбора.Добавить(
			ПредопределенноеЗначение("Перечисление.ПричиныПеремаркировкиТоваровИСМП.ВозвратОтЮридическогоЛица"), НСтр(
			"ru = 'возврате от юридического лица или ИП'"));
	КонецЕсли;
	
	Если ДоступныеПричиныПеремаркировки.Найти(ПредопределенноеЗначение(
		"Перечисление.ПричиныПеремаркировкиТоваровИСМП.ВозвратИзЕАЭС")) <> Неопределено Тогда
		Элементы.ПричинаПеремаркировкиИспорченоУтрачено.СписокВыбора.Добавить(
			ПредопределенноеЗначение("Перечисление.ПричиныПеремаркировкиТоваровИСМП.ВозвратИзЕАЭС"), НСтр(
			"ru = 'возврате из ЕАЭС'"));
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.ПричинаПеремаркировки) Тогда
		Если Объект.ПричинаПеремаркировки = ПредопределенноеЗначение(
			"Перечисление.ПричиныПеремаркировкиТоваровИСМП.РеализацияТоваровПриобретенныхНеДляПродажи") Тогда

			Элементы.ПричинаПеремаркировкиРеализацияТоваров.Видимость = Истина;

		ИначеЕсли Объект.ПричинаПеремаркировки = ПредопределенноеЗначение(
			"Перечисление.ПричиныПеремаркировкиТоваровИСМП.ВозвратВСлучаеДистанционнойПродажи") Тогда

			Элементы.ПричинаПеремаркировкиИспорченоУтрачено.СписокВыбора.Добавить(
				ПредопределенноеЗначение(
				"Перечисление.ПричиныПеремаркировкиТоваровИСМП.ВозвратВСлучаеДистанционнойПродажи"), НСтр(
				"ru = 'возврате в случае дистанционной продажи'"));

		ИначеЕсли Объект.ПричинаПеремаркировки = ПредопределенноеЗначение(
			"Перечисление.ПричиныПеремаркировкиТоваровИСМП.ВозвратИзЕАЭС") Тогда
		...
		ИначеЕсли Объект.ПричинаПеремаркировки = ПредопределенноеЗначение(
			"Перечисление.ПричиныПеремаркировкиТоваровИСМП.ВозвратОтРозничногоПокупателя") Тогда
		...
		ИначеЕсли Объект.ПричинаПеремаркировки = ПредопределенноеЗначение(
			"Перечисление.ПричиныПеремаркировкиТоваровИСМП.ВозвратОтЮридическогоЛица") Тогда
...
Процедура ОбработкаНавигационнойСсылкиНоменклатуры(НавигационнаяСсылка, СтруктураПолей, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "ВыбратьНоменклатуру" Тогда
		...
		СобытияФормИСКлиентПереопределяемый.ПриНачалеВыбораНоменклатуры(
			ЭтотОбъект, ВидыПродукции, СтандартнаяОбработка, ОписаниеВыборЗначения);
...
Функция КлючВидаПродукцииНоменклатуры(Номенклатура)
	...
	Если Выборка.Следующий() Тогда
		Возврат Выборка.КлючВидовПродукции;
	КонецЕсли;
	
	Возврат Справочники.КлючиВидовПродукцииИС.ПустаяСсылка();
...
Процедура РучнойВводШтрихкодаЗавершение(ДанныеШтрихкода, ДополнительныеПараметры) Экспорт
	...
	ДанныеРазбора = РазборКодаМаркировкиИССлужебныйКлиент.РазобратьКодМаркировки(ДанныеШтрихкода.Штрихкод, Объект.ТоварнаяГруппа);
	
	ВидыПродукции = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(Объект.ТоварнаяГруппа);
	
	Если ДанныеРазбора <> Неопределено Тогда
		ОшибкаВводаШтрихкода = Истина;

		Для Каждого ВидПродукции Из ВидыПродукции Цикл
			ВидыУпаковокПоВидуПродукции = ДанныеРазбора.ВидыУпаковокПоВидамПродукции[ВидПродукции];

			Если ВидыУпаковокПоВидуПродукции <> Неопределено Тогда
				Если ВидыУпаковокПоВидуПродукции.Найти(
					ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Потребительская")) <> Неопределено
					Или ВидыУпаковокПоВидуПродукции.Найти(
		...
		Если ОшибкаВводаШтрихкода Тогда
...
Функция ПараметрыСканированияШтрихкода(ТекущаяСтраница)
	...
	Если ТекущаяСтраница = Элементы.СтраницаНовыйКодМаркировки Тогда
		...
		ПараметрыСканирования.ДопустимыйСпособВводаВОборот = ПеремаркировкаТоваровКлиентСервер.СпособВводаВОборотПриПеремаркировке(Объект.ТоварнаяГруппа);
...
Функция ОбработатьШтрихкодКодаМаркировки(ДанныеШтрихкода)
	...
	Если Элементы.СтраницыДанных.ТекущаяСтраница = Элементы.СтраницаКодМаркировки Тогда
		...
		Если ДанныеШтрихкода.Статус = ПредопределенноеЗначение(
			...
			Если ДанныеШтрихкода.СпособВводаВОборот = ПредопределенноеЗначение(
				...
				Или ДанныеШтрихкода.СпособВводаВОборот = ПредопределенноеЗначение(
				"Перечисление.СпособыВводаВОборотСУЗ.МаркировкаВнеПроизводстваИмпорта") Тогда
				...
				Если Не ЗначениеЗаполнено(Объект.КодМаркировки) Тогда
					...
					ИначеЕсли ДанныеШтрихкода.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Набор")
						Или ДанныеШтрихкода.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Групповая") Тогда
						ТекстОшибки = СтрШаблон(
							НСтр("ru = 'Введенный код является новым кодом маркировки вида ""%1"".
								       |Для кода данного вида, необходимо ввести старый код маркировки.
								       |Введите старый код маркировки.'"),
								ДанныеШтрихкода.ВидУпаковки);
					КонецЕсли;
					...
					Если ТекстОшибки <> Неопределено Тогда
						...
						ДанныеШтрихкода.ОбработанСОшибками = Истина;
						ДанныеШтрихкода.ТекстОшибки = ТекстОшибки;
						ДанныеШтрихкода.АдресДереваУпаковок = "";
	...
	Если СтруктураПолей.ЭтоНовыйКод Тогда
		...
		ИначеЕсли ДанныеШтрихкода.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Набор")
			...
			ВывестиСообщениеНаСтраницеВводаКодаМаркировки(ЭтотОбъект,
				СтрШаблон(НСтр(
				"ru = 'Для кода с видом упаковки ""%1"", необходимо ввести старый код маркировки.'"),
				ДанныеШтрихкода.ВидУпаковки));
			Возврат Ложь;
...
Процедура ПоказатьОшибкуВводаШтрихкода(ДанныеШтрихкода)
	...
	ОткрытьФорму("Обработка.ПроверкаИПодборПродукцииИС.Форма.ИнформацияОНевозможностиДобавленияОтсканированного", ПараметрыОткрытия, ЭтотОбъект);
...
Функция ДанныеКодаМаркировки(Форма, ШтрихкодУпаковки, ПараметрыСканирования)
	...
	Если Форма.КэшКодовМаркировки.ДанныеКодовМаркировки[ШтрихкодУпаковки] = Неопределено Тогда
		...
		Если РезультатОбработкиШтрихкода = Неопределено
			Или РезультатОбработкиШтрихкода.ДанныеШтрихкода = Неопределено Тогда
			ДанныеКодаМаркировки = Новый Структура;
...
Процедура КодМаркировкиПриИзмененииЗначения()
	...
	Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		...
		Если ЗначенияРеквизитов.ВидПродукции <> Неопределено
			И ВидыПродукции.Найти(ЗначенияРеквизитов.ВидПродукции) = Неопределено Тогда
...
Функция ПолучитьВидПродукцииНоменклатуры(Номенклатура)
	...
	Возврат КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(
		ОбщегоНазначенияИС.ОписаниеНоменклатуры(Номенклатура)[Номенклатура].КлючВидовПродукции,
...
// 
// Параметры:
//  ЭтоНовыйКод - Булево - Это новый код
//  ФлагИзмененияНоменклатуры - Число - Флаг изменения номенклатуры
//  ПроверитьКод - Булево - Проверить код
&НаКлиенте
Процедура ОбработатьИзменениеНоменклатуры(ЭтоНовыйКод, Знач ФлагИзмененияНоменклатуры = 0, ПроверитьКод = Истина)
	...
	ОбработатьЗависимыеОтКодаМаркировкиДанные(ЭтотОбъект, ПараметрыСканирования);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОбработатьЗависимыеОтКодаМаркировкиДанные(Форма, ПараметрыСканирования)
	
	УказанСтарыйКод = ЗначениеЗаполнено(Форма.Объект.КодМаркировки);
	
	Если ЗначениеЗаполнено(Форма.Объект.НовыйКодМаркировки) Тогда
		КодМаркировки = Форма.Объект.НовыйКодМаркировки;
	ИначеЕсли УказанСтарыйКод Тогда
		КодМаркировки = Форма.Объект.КодМаркировки;
	Иначе
	...
	Если КодМаркировки = Неопределено Тогда
		ЭтоНабор = Ложь;
	Иначе
		ДанныеКодаМаркировки = ДанныеКодаМаркировки(Форма, КодМаркировки, ПараметрыСканирования);
		ЭтоНабор = ДанныеКодаМаркировки.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Набор");
	КонецЕсли;
	
	Если ЭтоНабор И ЗначениеЗаполнено(Форма.Объект.КодТНВЭД) Тогда
		Форма.Объект.КодТНВЭД = Неопределено;
	КонецЕсли;
	
	Форма.Элементы.КодТНВЭД.Видимость = Не ЭтоНабор;
	Форма.Элементы.КодТНВЭД.АвтоОтметкаНезаполненного = Не УказанСтарыйКод;
	Форма.Элементы.КодТНВЭД.ОтметкаНезаполненного = Не УказанСтарыйКод;
