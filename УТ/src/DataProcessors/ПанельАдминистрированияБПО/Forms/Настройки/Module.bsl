...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ВерсияБиблиотеки.Заголовок = СтрШаблон(
			НСтр("ru = 'Версия библиотеки подключаемого оборудования: %1'"), 
			МенеджерОборудования.ВерсияБиблиотеки());
...
Процедура ПриОткрытии(Отказ)
	...
	Если РежимОткрытия = "ХранениеУспешныхОпераций" Тогда
		ПодключитьОбработчикОжидания("Подключаемый_ПриИзмененииРежимаОткрытия", 0.1, Истина);
	КонецЕсли;
...
Процедура ОбновитьСостояниеИнтернетПоддержки()
	...
	Если ОбщегоНазначенияБПО.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПолучениеВнешнихКомпонент") Тогда
		Элементы.ДекорацияОбновлениеВнешнихКомпонентНеВыполняется.Видимость =
			...
			И ВариантОбновленияВнешнихКомпонент = 1);
	Иначе
...
Процедура ОбработчикОповещений(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_НаборКонстант" Тогда
		УстановитьДоступность();
	ИначеЕсли ИмяСобытия = "ИнтернетПоддержкаОтключена" Или ИмяСобытия = "ИнтернетПоддержкаПодключена" Тогда
		ОбновитьСостояниеИнтернетПоддержки();
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ОчиститьИсториюОперацийОчередиЧеков()
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяБСП() Тогда
		...
		Возврат МодульДлительныеОперации.ВыполнитьПроцедуру(
			ПараметрыВыполненияПроцедуры, 
			"ЛогированиеОперацийБПО.ОчисткаИсторииОперацийОчередиЧеков");
		// Конец Вызов БСП
	Иначе
...
&НаСервереБезКонтекста
Функция ОчиститьИсториюПлатежныхОпераций()
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяБСП() Тогда
		...
		Возврат МодульДлительныеОперации.ВыполнитьПроцедуру(
			ПараметрыВыполненияПроцедуры, 
			"ЛогированиеОперацийБПО.ОчисткаИсторииПлатежныхОпераций");
		// Конец Вызов БСП
	Иначе
		Задание = ФоновыеЗадания.Выполнить(
			"ЛогированиеОперацийБПО.ОчисткаИсторииПлатежныхОпераций",,,
...
&НаСервереБезКонтекста
Функция ОчиститьИсториюФискальныхОпераций()
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяБСП() Тогда
		...
		Возврат МодульДлительныеОперации.ВыполнитьПроцедуру(
			ПараметрыВыполненияПроцедуры, 
			"ЛогированиеОперацийБПО.ОчисткаИсторииФискальныхОпераций");
		// Конец Вызов БСП
	Иначе
		Задание = ФоновыеЗадания.Выполнить(
			"ЛогированиеОперацийБПО.ОчисткаИсторииФискальныхОпераций",,,
...
&НаСервереБезКонтекста
Функция ОчиститьИсториюОперацийПроверкиКМ()
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяБСП() Тогда
		...
		Возврат МодульДлительныеОперации.ВыполнитьПроцедуру(
			ПараметрыВыполненияПроцедуры, 
			"ЛогированиеОперацийБПО.ОчисткаИсторииОперацийПроверкиКМ");
		// Конец Вызов БСП
	Иначе
		Задание = ФоновыеЗадания.Выполнить(
			"ЛогированиеОперацийБПО.ОчисткаИсторииОперацийПроверкиКМ",,,
...
&НаСервереБезКонтекста
Функция ОчиститьИсториюОперацийСОборудованием()
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяБСП() Тогда
		...
		Возврат МодульДлительныеОперации.ВыполнитьПроцедуру(
			ПараметрыВыполненияПроцедуры, 
			"ЛогированиеОперацийБПО.ОчисткаИсторииОперацийСОборудованием");
		// Конец Вызов БСП
	Иначе
		Задание = ФоновыеЗадания.Выполнить(
			"ЛогированиеОперацийБПО.ОчисткаИсторииОперацийСОборудованием",,,
...
Процедура ИзменитьИспользованиеОчисткиОпераций(ИмяЗадания)
	...
	Если СрокХранения = Перечисления.ПериодХраненияИсторииОпераций.ВсеВремя 
...
Процедура УстановитьЗначенияРеквизитов()
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяРаспределеннаяФискализация() Тогда
		...
		Если Не ЗначениеЗаполнено(СрокХраненияОперацийОчередиЧеков) Тогда
			СрокХраненияОперацийОчередиЧеков = Перечисления.ПериодХраненияИсторииОпераций.ВсеВремя;
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяМаркировка() Тогда
		...
		Если Не ЗначениеЗаполнено(СрокХраненияОперацийПроверкиКМ) Тогда
			СрокХраненияОперацийПроверкиКМ = Перечисления.ПериодХраненияИсторииОпераций.ВсеВремя;
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяПлатежныеСистемы() Тогда
		...
		Если Не ЗначениеЗаполнено(СрокХраненияПлатежныхОпераций) Тогда
			СрокХраненияПлатежныхОпераций = Перечисления.ПериодХраненияИсторииОпераций.ВсеВремя;
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяЧекопечатающиеУстройства() Тогда
		...
		Если Не ЗначениеЗаполнено(СрокХраненияФискальныхОпераций) Тогда
			СрокХраненияФискальныхОпераций = Перечисления.ПериодХраненияИсторииОпераций.ВсеВремя;
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяПодсистемаЛогирования() Тогда
		...
		Если Не ЗначениеЗаполнено(СрокХраненияОперацийСОборудованием) Тогда
			СрокХраненияОперацийСОборудованием = Перечисления.ПериодХраненияИсторииОпераций.ВсеВремя;
...
Функция РегламентноеЗаданиеПоИмени(ИмяЗадания)
	...
	Если ОбщегоНазначенияБПО.ПодсистемаСуществует("СтандартныеПодсистемы.РегламентныеЗадания") Тогда
		...
		Иначе
			...
			Комментарий = СтрШаблон(НСтр("ru = 'Ошибка получения регламентного задания %1'"), ИмяЗадания);
...
&НаКлиенте
Процедура Подключаемый_ПриИзмененииРежимаОткрытия()
	ТекущийЭлемент = Элементы.ХранитьУспешныеОперацииРазрешения;
КонецПроцедуры
