...
Процедура ДополнитьИмяПредметаПоТипуОбъекта(ИмяПредмета, ТипОбъекта)
	...
	Если Лев(ТипОбъекта, 2) <> "DM" Тогда
		КлассТип = СтрРазделить(ТипОбъекта, ".");
		Если СтрНайти(КлассТип[0], "Документ") > 0 Тогда
			КлассТип[0] = "Документы";
		ИначеЕсли СтрНайти(КлассТип[0], "Справочник") > 0 Тогда
			КлассТип[0] = "Справочники";
		ИначеЕсли СтрНайти(КлассТип[0], "БизнесПроцесс") > 0 Тогда
			КлассТип[0] = "БизнесПроцессы";
		ИначеЕсли СтрНайти(КлассТип[0], "Задача") > 0 Тогда
			КлассТип[0] = "Задачи";
		КонецЕсли;
		ИмяТипаПредмета = " (" + Метаданные[КлассТип[0]][КлассТип[1]].Синоним + ")";
	Иначе
		ИмяТипаПредмета = ИнтеграцияС1СДокументооборотКлиентСервер.ПредставлениеТипаОбъектаXDTO(ТипОбъекта);
	КонецЕсли;
...
Процедура ЗаполнитьСвойстваОбъектаПоЗадаче(Задача, Прокси)
	...
	ИначеЕсли ИмяТипа = "DMBusinessProcessConsiderationTaskAcquaint" Тогда
		Задача.resolution = РезультатВыполнения;
		...
		Если ЗначениеЗаполнено(ОбъектXDTO.dueDate) Тогда
			...
			ПолноеОписаниеЗадачи = ПолноеОписаниеЗадачи
				+ СтрШаблон(НСтр("ru = 'Срок: %1'"),
					Формат(ОбъектXDTO.dueDate, НСтр("ru = 'ДФ=''dd.MM.yyyy HH:mm'''")));
		КонецЕсли;
