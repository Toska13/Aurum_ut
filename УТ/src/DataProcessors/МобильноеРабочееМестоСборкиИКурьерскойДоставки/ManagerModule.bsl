...
// Выполняет подготовку таблицы товаров по данным распоряжения на доставку.
//
// Параметры:
//	Заказ - ДокументСсылка.ЗаказКлиента - распоряжение на доставку.
...
Процедура ТаблицаТовары(Заказ, АдресРезультата) Экспорт
	...
	|	&НоменклатураГруппировкаТоваров КАК Группа,
	...
	|	ВЫБОР
	|		КОГДА ВТТоварыЗаказа.Упаковка = ШтрихкодыНоменклатуры.Упаковка
	|			ТОГДА 0
	...
	|			И ВТТоварыЗаказа.Серия = ШтрихкодыНоменклатуры.Серия
	...
	|ИЗ
	|	ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ШтрихкодыПриоритет КАК ШтрихкодыПриоритет
	|		ПО ШтрихкодыНоменклатуры.Номенклатура = ШтрихкодыПриоритет.Номенклатура
	|			И ШтрихкодыНоменклатуры.Характеристика = ШтрихкодыПриоритет.Характеристика
	|			И ШтрихкодыНоменклатуры.Серия = ШтрихкодыПриоритет.Серия
	|			И ШтрихкодыНоменклатуры.Упаковка = ШтрихкодыПриоритет.Упаковка
	|			И ШтрихкодыНоменклатуры.Приоритет = ШтрихкодыПриоритет.Приоритет
	|
	|СГРУППИРОВАТЬ ПО
	|	ШтрихкодыНоменклатуры.Номенклатура,
	|	ШтрихкодыНоменклатуры.Характеристика,
	...
	|ИЗ
	|	ВТТоварыЗаказа КАК ВТТоварыЗаказа
	|		ЛЕВОЕ СОЕДИНЕНИЕ Штрихкоды КАК ШтрихкодыНоменклатуры
	|		ПО ВТТоварыЗаказа.Номенклатура = ШтрихкодыНоменклатуры.Номенклатура
	|			И ВТТоварыЗаказа.Характеристика = ШтрихкодыНоменклатуры.Характеристика
	|			И ВТТоварыЗаказа.Серия = ШтрихкодыНоменклатуры.Серия
	|			И ВТТоварыЗаказа.Упаковка = ШтрихкодыНоменклатуры.Упаковка";
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&НоменклатураГруппировкаТоваров",
		"Товары.Номенклатура." + ГруппировкаТоваров);
...
// Возвращает сведения о товаре, распознанные при сканировании штрихкода.
//
// Параметры:
//	Штрихкод - Строка - штрихкод номенклатуры, по которому выполняется поиск товара.
...
//		* Группа - СправочникСсылка.Номенклатура -
//		* КоличествоСтрок - Строка -
//		* ВесЕдиницаИзмерения - СправочникСсылка.УпаковкиЕдиницыИзмерения -
//		* СрокГодности - Число -
//		* ЕдиницаИзмеренияСрокаГодности - СправочникСсылка.УпаковкиЕдиницыИзмерения -
...
Функция НайтиТоварПоШК(Штрихкод, ВидЦены) Экспорт
	...
	|	1											КАК КоличествоСтрок,
	|	СправочникНоменклатура.СрокГодности			КАК СрокГодности,
	|	СправочникНоменклатура.ЕдиницаИзмеренияСрокаГодности КАК ЕдиницаИзмеренияСрокаГодности,
	|	ВЫБОР
	|		КОГДА ШтрихкодыНоменклатуры.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|			ТОГДА СправочникНоменклатура.ВесЕдиницаИзмерения
	|		ИНАЧЕ ШтрихкодыНоменклатуры.Упаковка.ВесЕдиницаИзмерения
	|	КОНЕЦ										КАК ВесЕдиницаИзмерения,
	|	&ВидЦены									КАК ВидЦены
	|ИЗ
	|	РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(, ВидЦены = &ВидЦены) КАК ЦеныНоменклатуры
	|		ПО ШтрихкодыНоменклатуры.Номенклатура = ЦеныНоменклатуры.Номенклатура
	|			И ШтрихкодыНоменклатуры.Характеристика = ЦеныНоменклатуры.Характеристика
	|			И ШтрихкодыНоменклатуры.Упаковка = ЦеныНоменклатуры.Упаковка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
...
Процедура УстановитьСтатусРаспоряженияСобран(Распоряжение, СтруктураЗаказа) Экспорт
	...
	СтруктураПересчетаСуммы = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(РаспоряжениеОбъект);
		...
		Для Каждого СтрокаРаспоряжение Из РаспоряжениеОбъект.Товары Цикл
			...
			Если СтрокаРаспоряжение.Номенклатура = СтрокаТовараНет.Номенклатура И СтрокаРаспоряжение.Характеристика = СтрокаТовараНет.Характеристика Тогда
				...
				Иначе
					...
					ОбрабатываемыеСтроки.Добавить(СтрокаРаспоряжение);
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		...
		СтруктураДействий,
		РаспоряжениеОбъект.Товары,
		Неопределено);
	
	// Собрано
	ОбрабатываемыеСтроки = Новый Массив;
	ПараметрыЗаполнитьСтавкуНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(РаспоряжениеОбъект);
		...
		Иначе 
			...
			Пока ИДНайденнойСтроки < МассивНайденных.Количество() Цикл
				...
				Если Сч = 0 Тогда
					...
					ОбрабатываемыеСтроки.Добавить(СтрокаРаспоряжения);
