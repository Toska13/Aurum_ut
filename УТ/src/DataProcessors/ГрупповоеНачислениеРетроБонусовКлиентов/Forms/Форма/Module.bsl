...
Процедура ЗаполнитьРеквизитыПоУсловию()
	...
	Если ЗначениеЗаполнено(Объект.ДокументУсловий) Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.Валюта)
		   И РеквизитыУсловий.БазаРасчетаПродаж = Перечисления.БазыРасчетаРетроБонусов.ВыручкаВзаиморасчеты
...
&НаСервере
Процедура ПроверитьВидРетроБонусаДокументаУсловий()
	...
	Если ЗначениеЗаполнено(Объект.ДокументУсловий) Тогда
		...
		Если СоставТоваров = Перечисления.СоставыТоваровРетроБонусов.СвойствоНоменклатуры Тогда
			...
			РазрешеноИспользовать = РезультатПроверки.РазрешеноИспользовать;
			Если НЕ РазрешеноИспользовать Тогда
				
				ИмяПоля = "ДокументУсловий";
				РетроБонусыСервер.СообщитьОбОшибкеСвойстваНоменклатуры(РезультатПроверки, ИмяПоля);
...
Функция ПараметрыФормированияДокументов()
	...
	ПараметрыФормирования.ПроводитьБезКонтроляОстатковРетроБонусовКлиентов =
		ПараметрыСеанса.ПроводитьБезКонтроляОстатковРетроБонусовКлиентов;
	
	Если БазаРасчетаПродаж = Перечисления.БазыРасчетаРетроБонусов.ВыручкаВзаиморасчеты
	 ИЛИ БазаРасчетаПродаж = Перечисления.БазыРасчетаРетроБонусов.ВыручкаРеглУчет
	 ИЛИ БазаРасчетаПродаж = Перечисления.БазыРасчетаРетроБонусов.ВыручкаУпрУчет Тогда
...
Процедура ПроверитьЗаполнениеШапки(Отказ)
	...
	ПредставлениеНачалоПериода = НСтр("ru = 'Начало периода'");
	СписокПроверяемыхРеквизитов.Добавить("НачалоПериода", ПредставлениеНачалоПериода);
	ПредставлениеОкончаниеПериода = НСтр("ru = 'Окончание периода'");
	СписокПроверяемыхРеквизитов.Добавить("ОкончаниеПериода", ПредставлениеОкончаниеПериода);
...
Процедура ДокументУсловийПриИзмененииСервер()
	...
	УстановитьСвязиПараметровВыбораДокументаУсловий();
	НастроитьВидимостьДоступностьФормы();
	ПроверитьВидРетроБонусаДокументаУсловий();
...
Процедура ВыполняютсяОперацииПоФиксацииСоставаДокументаУсловий(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекстСообщения =
		НСтр("ru = 'По документу условий уже выполняются операции по фиксации состава. Попробуйте позже'");
...
Функция ДанныеДокументовКФормированию()
	...
	ДанныеКФормированиюБезПроведения =
		ДанныеКФормированиюДокументовБезПроведения(ДанныеДокументов, МенеджерВременныхТаблиц);
	ДанныеКФормированиюСПроведением =
		ДанныеКФормированиюДокументовСПроведением(ДанныеДокументов, МенеджерВременныхТаблиц);
	
	ДанныеКФормированию = Новый Структура;
	ДанныеКФормированию.Вставить("ДанныеКФормированиюБезПроведения", ДанныеКФормированиюБезПроведения);
	ДанныеКФормированию.Вставить("ДанныеКФормированиюСПроведением", ДанныеКФормированиюСПроведением);
...
&НаСервере
Функция ДанныеКФормированиюДокументовБезПроведения(ДанныеДокументов, МенеджерВременныхТаблиц)
	
	ДанныеКФормированию =
		Обработки.ГрупповоеНачислениеРетроБонусовКлиентов.НовыеДанныеДляФормированияДокументовНачисленияБезПроведения();
		
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.Текст = ТекстЗапросаФормированияДокументовБезПроведения();
	Запрос.УстановитьПараметр("ДанныеДокументов", ДанныеДокументов);
	
	РезультатыЗапроса = Запрос.ВыполнитьПакет();
	ИндексРезультата = 2;
	Для Каждого КлючИЗначение Из ДанныеКФормированию Цикл
		
		РезультатЗапроса = РезультатыЗапроса[ИндексРезультата];
		ТаблицаДанных = КлючИЗначение.Значение; // см. Обработки.ГрупповоеНачислениеРетроБонусовКлиентов.НоваяТаблицаДокументовНачисления
...
&НаСервере
Функция ДанныеКФормированиюДокументовСПроведением(ДанныеДокументов, МенеджерВременныхТаблиц)
	
	ДанныеКФормированию =
		Обработки.ГрупповоеНачислениеРетроБонусовКлиентов.НовыеДанныеДляФормированияДокументовНачисленияСПроведением();
		
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.Текст = ТекстЗапросаФормированияДокументовСПроведением();
	Запрос.УстановитьПараметр("ДанныеДокументов", ДанныеДокументов);
...
&НаСервере
Функция ТекстЗапросаФормированияДокументовБезПроведения()
	...
	|ПОМЕСТИТЬ ВТ_ДанныеКФормированиюПодготовка
	|ИЗ
	...
	|	ДанныеДокументов.ДокументНачисления КАК ДокументНачисления,
	|	ВЫБОР
	|		КОГДА ДанныеДокументов.ДокументНачисления = ЗНАЧЕНИЕ(Документ.НачислениеРетроБонусовКлиента.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ВЫРАЗИТЬ(ДанныеДокументов.ДокументНачисления КАК Документ.НачислениеРетроБонусовКлиента).Проведен
	|	КОНЕЦ КАК Проведен,
	|	ДанныеДокументов.СуммаКНачислению КАК СуммаКНачислению,
	|	ДанныеДокументов.СуммаНачисленная КАК СуммаНачисленная,
	|	ДанныеДокументов.СуммаРасчетная КАК СуммаРасчетная
	...
	|ИЗ
	|	ВТ_ДанныеКФормированиюПодготовка КАК ДанныеДокументов
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	НЕ ДанныеКФормированию.Проведен
	|	И ДанныеКФормированию.СуммаРасчетная <> 0
...
&НаСервере
Функция ТекстЗапросаФормированияДокументовСПроведением()
