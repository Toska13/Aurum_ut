...
Процедура АдресКартинкиНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(АдресКартинки) Тогда
		ФайловаяСистемаКлиент.ОткрытьФайл(АдресКартинки, , "jpeg");	
	КонецЕсли;
...
Процедура ТоварыПриАктивизацииСтроки(Элемент)
	...
	НайденныеСтрокиКЭШаИзображений = КэшИзображенийТоваров.НайтиСтроки(Отбор);
	
	КЭШИзображенийТекущейСтроки.Очистить();
	
	Для Каждого СтрокаКЭШаИзображений Из НайденныеСтрокиКЭШаИзображений Цикл
		КЭШИзображенийТекущейСтроки.Добавить(СтрокаКЭШаИзображений.АдресИзображения);
	КонецЦикла;
		
	КоличествоИзображенийТекущейСтроки = КЭШИзображенийТекущейСтроки.Количество();
...
Процедура ПанельИзображение(Команда)
	...
	НастройкаПанелиИзображенийНаСервере(ИмяФормы, ПанельОтображается);	
...
Процедура СледующееИзображение(Команда)
	...
	Если ИндексТекущегоИзображенияСтроки = КЭШИзображенийТекущейСтроки.Количество() - 1 Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьНаОснованииДерева(ДанныеВходящегоДокумента)
	...
	Для Каждого СведенияОТоваре Из СведенияОТоварах.Строки Цикл
		...
		ЗагрузкаШтрихкодов(НоваяСтрока, СведенияОТоваре);
				
		ЗагрузкаИзображений(НоваяСтрока, СведенияОТоваре);
...
&НаСервере
Процедура ЗагрузкаИзображений(НоваяСтрока, СведенияОТоваре)

	АдресаИзображений = ЭлектронноеВзаимодействие.ЗначениеРеквизитаВДереве(
		СведенияОТоваре, "Товары.НомерСтроки.Товар.Изображения");
	
	Если ТипЗнч(АдресаИзображений) = Тип("Массив") Тогда
		
		Для Каждого АдресИзображения Из АдресаИзображений Цикл
			
			ОтносительныйАдрес = СтрНайти(АдресИзображения, "http") = 0;

			АдресИзображенияВХранилище = 
				КоммерческиеПредложенияСлужебный.АдресИзображенияВХранилище(АдресИзображения, ОтносительныйАдрес);
			Если Не ЭтоАдресВременногоХранилища(АдресИзображенияВХранилище) Тогда
				Продолжить;
			КонецЕсли;
			
			НоваяСтрокаАдресаИзображения = КэшИзображенийТоваров.Добавить();
			НоваяСтрокаАдресаИзображения.ИдентификаторСтрокиЗапроса = НоваяСтрока.ИдентификаторСтрокиЗапроса;
			НоваяСтрокаАдресаИзображения.АдресИзображения = АдресИзображенияВХранилище;	
			
			НоваяСтрока.ИзображениеВыбрано = Истина;					
...
&НаСервере
Процедура ЗагрузкаШтрихкодов(НоваяСтрока, СведенияОТоваре)

	ШтрихкодыТовара = ЭлектронноеВзаимодействие.ЗначениеРеквизитаВДереве(
		СведенияОТоваре, "Товары.НомерСтроки.Товар.Штрихкоды");
		
	Если ТипЗнч(ШтрихкодыТовара) = Тип("Массив") Тогда
		Для Каждого Штрихкод Из ШтрихкодыТовара Цикл
			НоваяСтрокаШтрихкодов = Штрихкоды.Добавить();
			НоваяСтрокаШтрихкодов.ИдентификаторСтрокиЗапроса =	НоваяСтрока.ИдентификаторСтрокиЗапроса;
			НоваяСтрокаШтрихкодов.Штрихкод = Штрихкод;		
		КонецЦикла;
	КонецЕсли;
...
Функция ВыполнитьСохранениеДокумента()
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("Обработки.ЗапросыКоммерческихПредложений.СохранитьЗапросУКлиента",
...
&НаСервереБезКонтекста
Процедура НастройкаПанелиИзображенийНаСервере(ИмяФормы, ПанельОтображается)
...
Процедура УстановитьАдресКартинки()
	...
	АдресКартинки = "";	
	Если ПанельОтображается И КоличествоИзображенийТекущейСтроки > 0 Тогда
		АдресКартинки = КЭШИзображенийТекущейСтроки[ИндексТекущегоИзображенияСтроки].Значение;
	КонецЕсли;
