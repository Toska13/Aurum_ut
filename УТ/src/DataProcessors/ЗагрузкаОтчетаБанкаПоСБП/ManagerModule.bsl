...
Функция ЗагрузитьОтчетыБанка(ДатаНач, ДатаКон, ДоговорПодключения, НастройкиПодключения, ПроводитьДокументыПриЗагрузке) Экспорт
	...
	Для Каждого СтрокаТЧ Из НастройкиПодключения Цикл
		...
		Если РезультатЗапросаОтчета.СтатусОперации <> НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииВыполнена() Тогда
			...
			НастройкиВызова = НастройкиПлатежныхСистемСлужебный.НовыйИтеративныйВызовОперации(180);
			...
			Пока СистемаБыстрыхПлатежейСлужебный.ВозможенВызовОперации(НастройкиВызова) Цикл
				...
				Если РезультатЗапросаСтатуса.СтатусОперации <> НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииВыполняется() Тогда
					Прервать;
				КонецЕсли;
