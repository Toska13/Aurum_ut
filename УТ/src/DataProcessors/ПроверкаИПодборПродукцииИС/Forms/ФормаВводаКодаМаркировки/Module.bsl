...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Только для прямого сканирования.
	ЭтоРозничныйВозвратПрямоеСканирование = Не ПараметрыСканирования.ИспользуетсяСоответствиеШтрихкодовСтрокДерева
		И ПараметрыСканирования.ВидОперацииИСМП = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ВидОперацииРозничныйВозврат();
	
	ДобавлятьБезКодаМаркировки = Ложь;

	Если ПоддерживаетсяОбъемноСортовойУчет Тогда
		ДобавлятьБезКодаМаркировки = Истина;
	ИначеЕсли ЭтоРозничныйВозвратПрямоеСканирование Тогда
		ДобавлятьБезКодаМаркировки = Истина;
	...
	ВидПродукцииКартинки = Неопределено;
	Если ТипЗнч(ВидМаркируемойПродукции) = Тип("СправочникСсылка.ВидыПродукцииИС") Тогда
		ВидПродукцииКартинки = ВидМаркируемойПродукции;
	Иначе
		Если ПараметрыСканирования.ИСМП Тогда
			ВидПродукцииКартинки = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(ВидМаркируемойПродукции, "ГИСМТ");
		КонецЕсли;
		Если ВидПродукцииКартинки = Неопределено И ПараметрыСканирования.ЕГАИС Тогда
			ВидПродукцииКартинки = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(ВидМаркируемойПродукции, "ЕГАИС");
		КонецЕсли;
	КонецЕсли;

	Если ВидПродукцииКартинки <> Неопределено Тогда
		ИнтеграцияИСМПКлиентСервер.УстановитьКартинкуСканированияКодаПоВидуПродукции(КартинкаСканирования,
			ВидПродукцииКартинки);
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаСканированиеКодаМаркировки Тогда
		ТекущийЭлемент = Элементы.СтраницаСканированиеКодаМаркировки;
	КонецЕсли;
	...
	Если ВладелецФормы <> Неопределено
		И ВладелецФормы.ИмяФормы = "Обработка.ПроверкаИПодборПродукцииИС.Форма.ПроверкаИПодбор" Тогда
...
Процедура НазначитьЗаголовокКомандыДобавитьБезМаркировкиСервер()
	...
	Клавиши = Команды.ДобавитьБезКодаМаркировки.СочетаниеКлавиш;
...
Процедура СоздатьНовыйКодМаркировкиИВывестиНаПечать()
	...
	СтруктураПечатиЭтикетки.ВидПродукции       = ВидМаркируемойПродукции;
	...
	СтруктураПараметров.ВидПродукции          = ВидМаркируемойПродукции;
