...
Процедура КомиссионерПриИзменении(Элемент)
	...
	УстановитьОтборДинамическихСписков();
	ОбновитьГиперссылкуКОформлению();
	ОбновитьДоступностьСозданияОтчетаКомиссионера();
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСписокНаОформление
...
Процедура ОформитьОтчетКомиссионераНаКлиенте()
	...
	Для Каждого Строка Из ВыделенныеСтроки Цикл
		...
		Если НайденнаяСтрока.ВестиРасчетыЧерезКонечныхПокупателей Тогда 
			НомерОшибки = 1;
			Прервать;
		ИначеЕсли НайденнаяСтрока.ПорядокРасчетов = ПредопределенноеЗначение("Перечисление.ПорядокРасчетов.ПоДоговорамНакладным") Тогда
			НомерОшибки = 2;
			Прервать;
		КонецЕсли;
	...
	ИначеЕсли НомерОшибки = 1 Тогда
		ТекстОшибки = НСтр("ru = 'Выбрана строка, по которой нельзя оформить отчет комиссионера, т.к. ведутся расчеты через комиссионера.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки);
	ИначеЕсли НомерОшибки = 2 Тогда
		ТекстОшибки = НСтр("ru = 'Выбрана строка, по которой нельзя оформить отчет комиссионера, т.к. выбран порядок расчетов ""Аванс по договорам, долг по накладным (только реализация)"".'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки);
	КонецЕсли;
...
&НаКлиенте
Процедура ОбновитьДоступностьСозданияОтчетаКомиссионера()
