...
Процедура ОрганизацияПриИзмененииСервер(ИзменитьОтбор = Истина)
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоРасчетныхСчетов") Тогда
		СтруктураПараметров = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации(Организация);
		БанковскийСчетОтбор = Справочники.БанковскиеСчетаОрганизаций.ПолучитьБанковскийСчетПоУмолчанию(СтруктураПараметров);
	КонецЕсли;
...
Процедура СписокПровести(Команда)
	...
	ПараметрыОбновления = РаботаСЖурналамиДокументовКлиент.ПараметрыОбновленияСпискаДокументов(ЭтотОбъект, "СписокПлатежей");
	ОбщегоНазначенияУТКлиент.ПровестиДокументы(Элементы.СписокПлатежей, Заголовок, ПараметрыОбновления);
	
	Если ПараметрыОбновления.ОбновитьСписок Тогда
		Оповестить("Запись_ДокументОплаты");
	КонецЕсли;
...
Процедура СписокОтменаПроведения(Команда)
	...
	ПараметрыОбновления = РаботаСЖурналамиДокументовКлиент.ПараметрыОбновленияСпискаДокументов(ЭтотОбъект, "СписокПлатежей");
	ОбщегоНазначенияУТКлиент.ОтменаПроведения(Элементы.СписокПлатежей, Заголовок, ПараметрыОбновления);
...
Процедура Подключаемый_ВыполнитьКомандуИнтеграции(Команда)
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыполнитьПодключаемуюКомандуИнтеграции(Команда, ЭтотОбъект, Элементы.СписокПлатежей);
...
Процедура ЗаполнитьКомандыСозданияДокументов()
	...
	Операции = Новый СписокЗначений;
	Если ПравоДоступа("Добавление", Метаданные.Документы.ПоступлениеБезналичныхДенежныхСредств) Тогда
		Операции.Добавить(Перечисления.ХозяйственныеОперации.ВозвратДенежныхСредствОтПоставщика);
...
Процедура ЗагрузитьОплатыИзЯндексКассыЛокализация(Команда)
	...
	ОчиститьСообщения();
	ДополнительныеПараметры = ДенежныеСредстваКлиентЛокализация.НовыеПараметрыЗагрузкиИзСервиса();
	ДополнительныеПараметры.ЭтоЗагрузкаИзЯндексКассы = Истина;
	ОповещениеПолученияОперацийПоЯндексКассе = Новый ОписаниеОповещения("ОбработкаПолученияОперацийЗавершение",
		ЭтотОбъект,
...
&НаКлиенте
Процедура Подключаемый_ЗагрузитьОплаченныеСсылкиСБПЛокализация(Команда)

	ОчиститьСообщения();
	
	ДополнительныеПараметры = ДенежныеСредстваКлиентЛокализация.НовыеПараметрыЗагрузкиИзСервиса();
	ДополнительныеПараметры.ЭтоЗагрузкаИзСБП = Истина;
	
	ОповещениеПослеПроверки = Новый ОписаниеОповещения(
		"НачатьЗагрузкуСтатусовОпераций",
		СистемаБыстрыхПлатежейУТКлиент,
		ДополнительныеПараметры);
	
	ДенежныеСредстваКлиентЛокализация.ПроверитьФоновоеЗаданиеЗагрузкиОплатИзСервиса(
		ЭтотОбъект, ДополнительныеПараметры, ОповещениеПослеПроверки);

КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ЗагрузитьОплаченныеСсылкиОнлайнЗаказовЛокализация(Команда)

	ОчиститьСообщения();
	
	ДополнительныеПараметры = ДенежныеСредстваКлиентЛокализация.НовыеПараметрыЗагрузкиИзСервиса();
	ДополнительныеПараметры.ЭтоЗагрузкаИзОнлайнЗаказов = Истина;
	
	ОповещениеПослеПроверки = Новый ОписаниеОповещения(
		"НачатьЗагрузкуСтатусовОпераций",
		ОнлайнЗаказыУТКлиент,
		ДополнительныеПараметры);
...
Процедура УстановитьВидимостьЛокализация()
	...
	ЕстьПравоЗагрузкиОплатИзСБП = ПереводыСБПc2b.ПереводыСБПДоступны()
		Или ПереводыСБПb2b.ПереводыСБПДоступны()
		Или Пользователи.ЭтоПолноправныйПользователь();
	ВидимостьЭлементов.Вставить(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("ЗагрузитьОплаченныеСсылкиСБП"), ЕстьПравоЗагрузкиОплатИзСБП);
	
	ЕстьПравоЗагрузкиОплатОнлайнЗаказов = ОнлайнЗаказы.ОнлайнЗаказыДоступны()
		Или Пользователи.ЭтоПолноправныйПользователь();
	ВидимостьЭлементов.Вставить(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("ЗагрузитьОплаченныеСсылкиОнлайнЗаказов"),
		ЕстьПравоЗагрузкиОплатОнлайнЗаказов);
...
&НаКлиенте
Процедура ОбработкаПолученияОперацийЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ЭтоЗагрузкаИзЯндексКассы Тогда
		...
		ИмяСобытия = "ЗавершенаЗагрузкаОперацииПоЯндексКассе";
		ДействиеПриНажатии = "e1cib/list/Документ.ОперацияПоЯндексКассе";
		
	ИначеЕсли ДополнительныеПараметры.ЭтоЗагрузкаИзСБП Тогда
		
		ТекстСообщения = ?(СчетчикДокументов > 0, НСтр("ru = 'Загружено платежей из СБП: %1'"),
			НСтр("ru = 'Нет оплаченных операций СБП'"));
		ИмяСобытия = "";
		ДействиеПриНажатии = "e1cib/list/Документ.ПоступлениеБезналичныхДенежныхСредств";
		
	Иначе
		
		ТекстСообщения = ?(СчетчикДокументов > 0, НСтр("ru = 'Загружено платежей из 1С:Онлайн-заказы: %1'"),
			НСтр("ru = 'Нет оплаченных заказов из 1С:Онлайн-заказы'"));
		ИмяСобытия = "";
		ДействиеПриНажатии = "e1cib/list/Документ.ПоступлениеБезналичныхДенежныхСредств";
...
Если Не ПодключеныОбработчикиЛокализация И ПолучитьФункциональнуюОпцию("ПоддержкаПлатежейРФ") Тогда
	...
	КомандыЛокализация.Добавить("ЗагрузитьОплаченныеСсылкиСБП");
	КомандыЛокализация.Добавить("ЗагрузитьОплаченныеСсылкиОнлайнЗаказов");
