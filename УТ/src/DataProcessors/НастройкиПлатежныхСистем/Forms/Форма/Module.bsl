...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	ДоступностьКонтекстноеМеню = Неопределено;
	ДоступностьКоманд = Неопределено;
	
	Если (ТекущиеДанные <> Неопределено
		И ТекущиеДанные.ТипЭлемента <> НастройкиПлатежныхСистемКлиентСервер.ИмяКорень()) Тогда
		ДоступностьКонтекстноеМеню = КешПараметров.ДоступностьНастроек.Получить(ТекущиеДанные.Подсистема);
		ДоступностьКоманд = КешПараметров.ДоступностьКоманд.Получить(ТекущиеДанные.Подсистема);
	КонецЕсли;
	
	Если ДоступностьКонтекстноеМеню = Неопределено Тогда
		ДоступностьКонтекстноеМеню = Ложь;
	КонецЕсли;
	
	Если ДоступностьКоманд = Неопределено Тогда
		ДоступностьКоманд = Новый Массив;
	КонецЕсли;
	
	Элементы.СписокКонтекстноеМенюСкопировать.Доступность = ДоступностьКонтекстноеМеню;
	Элементы.СписокКонтекстноеМенюИзменить.Доступность = ДоступностьКонтекстноеМеню;
	
	Для Каждого Кнопка Из Элементы.ГруппаДополнительно.ПодчиненныеЭлементы Цикл
		Если ТипЗнч(Кнопка) <> Тип("КнопкаФормы") Тогда
			Продолжить;
		КонецЕсли;
		Кнопка.Доступность = (ДоступностьКоманд.Найти(Кнопка.ИмяКоманды) <> Неопределено);
	КонецЦикла;
...
Процедура Скопировать(Команда)
	...
	НастройкиПлатежныхСистемКлиентСобытия.ПриКопированииНастройки(
		ТекущиеДанные,
		ОписаниеОповещения,
		Параметры.НеПредлагатьПодключениеКОнлайнЗаказам);
...
Процедура СоздатьДерево()
	...
	ИсходныеСсылки = Новый Соответствие;
...
&НаКлиенте
Процедура ЗаполнитьРазвернутыеСтроки(
		Ветка,
		РазвернутыеСтроки,
		ИсходныеСсылки)
		
	Если ЗначениеЗаполнено(Ветка.Ссылка)
		И ТипЗнч(Ветка.Ссылка) <> Тип("Строка") Тогда
		ИсходныеСсылки.Вставить(Ветка.Ссылка, Истина);
	КонецЕсли;
...
Процедура РазвернутьСтрокиПослеОбновления(
	...
	// Разворачиваем ранее развернутые ветки.
	Если ИсходныйПустой
	...
	// Разворачиваем ветки новых элементов.
	Если Не ИсходныйПустой Тогда
		Если ТипЗнч(Ветка.Ссылка) <> Тип("Строка")
			И ИсходныеСсылки.Получить(Ветка.Ссылка) = Неопределено Тогда
			ТекущийРодитель = Ветка.ПолучитьРодителя();
			Пока ТекущийРодитель <> Неопределено Цикл
				Идентификатор = ТекущийРодитель.ПолучитьИдентификатор();
				Если Не Элементы.Список.Развернут(Идентификатор) Тогда
					Элементы.Список.Развернуть(ТекущийРодитель.ПолучитьИдентификатор(), Ложь);
				КонецЕсли;
	...
	Попытка
		...
		Если Подсистема = Неопределено Тогда
			Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПлатежныеСистемы.БазоваяФункциональностьПлатежныхСистем") Тогда
				...
				Подсистема = МодульПлатежныеСистемыСлужебный.ПодсистемаПоИмени(ИмяПодсистемы);
			КонецЕсли;
		КонецЕсли;
...
Процедура УстановитьВидимостьДоступность()
	...
	НесколькоПодсистем = (Параметры.Подсистема = "Все"
		Или Параметры.Подсистема = НастройкиПлатежныхСистемКлиентСервер.ИмяПодсистемыПлатежныеСистемы())
