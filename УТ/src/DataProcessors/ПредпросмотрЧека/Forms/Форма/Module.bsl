...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЕдиныйЧек И ФормированиеФискальныхЧековСервер.ДокументПоставкиСВозможностьюПробитияЧеков(ДокументСсылка) Тогда
		...
		Иначе
			ЕдиныйЧек = Ложь;
			Элементы.ГруппаЕдиныйЧек.Доступность = Ложь;
			
			ТекстСообщения = НСтр("ru = 'Единый чек может быть только с одним документом оплаты платежной картой.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
		
	ИначеЕсли ЕдиныйЧек Тогда
		
		ПроверитьКорректностьРозничнойЦеныТабачнойПродукции();
		
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		
		Если Не ПроверитьКорректностьРозничнойЦеныТабачнойПродукцииВКорректировкеРеализации() Тогда
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	...
	СуммаПредоплатыВнеОдногоДня = ПараметрыПредпросмотраЧека.СуммаПредоплатыВнеОдногоДня;
	...
	ПолучитьОбработатьДанныеОплатыВИнтернетМагазинеИзДокумента();
	
	НастроитьСписокЗначенийВыбораТипаФискальногоДокумента();
	НастроитьСписокЗначенийВыбораЧасовойЗоныПередачиТовара();
	...
	//++ Локализация
	РозничныеПродажиЛокализация.НастроитьФормуДляПодтвержденияВозрастаПокупателя(ЭтотОбъект);
	//-- Локализация
	...
	УстановитьВидимостьРеквизитовОплатыВИнтернетМагазине(ЭтотОбъект);
	УстановитьДоступностьРеквизитовОплатыВИнтернетМагазине(ЭтотОбъект);
...
Процедура ТипФискальногоДокументаПриИзменении(Элемент)
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОперацияПоПлатежнойКарте") Тогда
		ТекстСообщения = НСтр("ru='Перед пробитием чека коррекции ""Неприменение ККТ"" убедитесь, что оплата банковской картой была ранее проведена успешно.'");
...
&НаКлиенте
Процедура ОплатаВИнтернетМагазинеПриИзменении(Элемент)
	
	Если ОплатаВИнтернетМагазине
		И ВариантОтправкиЭлектронногоЧека = ПредопределенноеЗначение("Перечисление.ВариантыОтправкиЭлектронногоЧекаПокупателю.НеОтправлять") Тогда
	...
	Если ОплатаВИнтернетМагазине Тогда
		АдресИнтернетМагазина = АдресИнтернетМагазинаВДоговореПодключения;
	Иначе
...
&НаКлиенте
Процедура АдресИнтернетМагазинаПриИзменении(Элемент)
...
&НаКлиенте
Процедура ЧасоваяЗонаПередачиТовараПриИзменении(Элемент)
	
	ОчиститьСообщения();
	
	ЧасоваяЗонаПередачиТовараПриИзмененииНаСервере();
...
Процедура ПробитьЧек(Команда)
	...
	Если Не ОплатыВИнтернетМагазинеЗаполненыКорректно() Тогда
	...
	ИначеЕсли ЗначениеЗаполнено(ОборудованиеТерминал)
		И ТипФискальногоДокумента = ПредопределенноеЗначение("Перечисление.ТипыФискальныхДокументовККТ.КассовыйЧек") Тогда
		...
		Иначе
			...
			Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОперацияПоПлатежнойКарте") Тогда
				...
				ПараметрыОткрытияФормы.Вставить("ДокументОснование", ДокументСсылка);
			КонецЕсли;
...
&НаКлиенте
Процедура ОтобразитьРеквизитыОплатыВИнтернетМагазине(Команда)
	
	ДоступныРеквизитыОплатыВИнтернетМагазине = Не ДоступныРеквизитыОплатыВИнтернетМагазине;
	Элементы.ОтобразитьРеквизитыОплатыВИнтернетМагазине.Пометка = ДоступныРеквизитыОплатыВИнтернетМагазине;
	
	Если Не ДоступныРеквизитыОплатыВИнтернетМагазине Тогда
...
Процедура ТипФискальногоДокументаПриИзмененииНаСервере()
	...
	НастроитьВидимостьПредставленияТерминала();
	УстановитьВидимостьДоступностьЧасовойЗоныПередачиТовара();
...
Процедура ТелефонПриИзмененииНаСервере()
	...
	Если Элементы.ВариантОтправкиЭлектронногоЧекаНеОтправлять.Доступность Тогда
...
&НаСервере
Процедура ЧасоваяЗонаПередачиТовараПриИзмененииНаСервере()
...
Процедура НастроитьВидимостьПредставленияТерминала()
	...
	Элементы.ПредставлениеТерминала.Видимость = ЗначениеЗаполнено(Терминал)
		И ТипФискальногоДокумента = Перечисления.ТипыФискальныхДокументовККТ.КассовыйЧек;
...
// Установить видимость доступность реквизитов интернет расчетов.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - Форма
//
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьВидимостьРеквизитовОплатыВИнтернетМагазине(Форма)
	
	ЭлементыПоказать = ТипЗнч(Форма.ДокументСсылка) = Тип("ДокументСсылка.ОперацияПоПлатежнойКарте")
		И (Форма.СпособПроведенияПлатежа = ПредопределенноеЗначение("Перечисление.СпособыПроведенияПлатежей.ИнтернетЭквайринг")
			Или Форма.СпособПроведенияПлатежа = ПредопределенноеЗначение("Перечисление.СпособыПроведенияПлатежей.СистемаБыстрыхПлатежей"));
	//++ Локализация
	ЭлементыПоказать = ЭлементыПоказать Или ТипЗнч(Форма.ДокументСсылка) = Тип("ДокументСсылка.ОперацияПоЯндексКассе");
	//-- Локализация
	ЭлементыПоказать = ЭлементыПоказать Или Форма.ДоступныРеквизитыОплатыВИнтернетМагазине;
	
	Если ЭлементыПоказать Тогда
		Форма.Элементы.ГруппаОплатаВИнтернетМагазине.Видимость = Истина;
	Иначе
		Форма.Элементы.ГруппаОплатаВИнтернетМагазине.Видимость = Ложь;
	КонецЕсли;
	
	Если ТипЗнч(Форма.ДокументСсылка) = Тип("ДокументСсылка.АктВыполненныхРабот")
		Или ТипЗнч(Форма.ДокументСсылка) = Тип("ДокументСсылка.КорректировкаРеализации")
		...
		Форма.Элементы.ОтобразитьРеквизитыОплатыВИнтернетМагазине.Видимость = Истина;
	Иначе
		Форма.Элементы.ОтобразитьРеквизитыОплатыВИнтернетМагазине.Видимость = Ложь;
	КонецЕсли;
...
//
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьДоступностьРеквизитовОплатыВИнтернетМагазине(Форма)
	
	Если Форма.ОплатаВИнтернетМагазинеВДоговореПодключения Тогда
		Форма.Элементы.ОплатаВИнтернетМагазине.Доступность = Ложь;
		Форма.Элементы.АдресИнтернетМагазина.Доступность = Ложь;
		Форма.Элементы.ВариантОтправкиЭлектронногоЧекаНеОтправлять.Доступность = Ложь;
	ИначеЕсли Форма.ОплатаВИнтернетМагазине Тогда
		Форма.Элементы.ОплатаВИнтернетМагазине.Доступность = Истина;
		Форма.Элементы.АдресИнтернетМагазина.Доступность = Истина;
		Форма.Элементы.ВариантОтправкиЭлектронногоЧекаНеОтправлять.Доступность = Ложь;
	Иначе
		Форма.Элементы.ОплатаВИнтернетМагазине.Доступность = Истина;
		Форма.Элементы.АдресИнтернетМагазина.Доступность = Ложь;
		Форма.Элементы.ВариантОтправкиЭлектронногоЧекаНеОтправлять.Доступность = Истина;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Форма.АдресИнтернетМагазинаВДоговореПодключения) Тогда
		Форма.Элементы.АдресИнтернетМагазина.Доступность = Ложь;
	КонецЕсли;
...
&НаСервере
Процедура УстановитьВидимостьДоступностьЧасовойЗоныПередачиТовара()
	Элементы.ЧасоваяЗонаПередачиТовара.Видимость = Ложь;
	//++ Локализация
	
	Элементы.ЧасоваяЗонаПередачиТовара.Видимость = ДоступноУказаниеЧасовойЗоныПередачиТовараДляДокумента(ДокументСсылка, ТипФискальногоДокумента, ЕдиныйЧек)
													И РозничныеПродажиКлиентСерверЛокализация.ТребуетсяПроверкаКодовМаркировкиСредствамиККТ(ПараметрыОперацииФискализацииЧека);
...
&НаСервере
Процедура НастроитьСписокЗначенийВыбораЧасовойЗоныПередачиТовара()
	
	Элементы.ЧасоваяЗонаПередачиТовара.СписокВыбора.Очистить();
	ЧасоваяЗонаПередачиТовара = 0;
	
	//++ Локализация
	ИсточникАдресов = ?(ЕдиныйЧек = Истина, ДокументыЕдиногоЧека, ДокументСсылка);
	Если ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25() Тогда
		ЧасовыеЗоныРФПоАдресамПокупателя = ПолучитьЧасовыеЗоныРФПоАдресамПокупателяДоставка25(ИсточникАдресов);
	Иначе
		ЧасовыеЗоныРФПоАдресамПокупателя = ПолучитьЧасовыеЗоныРФПоАдресамПокупателяДоставка20(ИсточникАдресов);
	КонецЕсли;
	
	Если ЧасовыеЗоныРФПоАдресамПокупателя.ЧасоваяЗонаПоУмолчанию > 0 Тогда
		ЧасоваяЗонаПередачиТовара = ЧасовыеЗоныРФПоАдресамПокупателя.ЧасоваяЗонаПоУмолчанию;
	КонецЕсли;	
	Если ЗначениеЗаполнено(ЧасовыеЗоныРФПоАдресамПокупателя.ОписаниеОшибки) Тогда
		ОбщегоНазначения.СообщитьПользователю(ЧасовыеЗоныРФПоАдресамПокупателя.ОписаниеОшибки);	
	КонецЕсли;
	
	Для Каждого ЧасоваяЗонаРФ Из НазванияГородовЧасовыхЗонРФПоУмолчанию() Цикл
		НазваниеГорода = "";
		Если ЧасоваяЗонаРФ.Ключ > 0 Тогда
			НазванияГородовЧасовойЗоны = ЧасовыеЗоныРФПоАдресамПокупателя.ЧасовыеЗоныНаселенныхПунктов[ЧасоваяЗонаРФ.Ключ];
			Если ЗначениеЗаполнено(НазванияГородовЧасовойЗоны) И НазванияГородовЧасовойЗоны.Количество() Тогда
				// Представление может состоять не более чем из 3х названий городов, первым выводится представление по умолчанию.
				НазванияГородов = Новый Массив;
				Для Индекс=0 По Мин(2,НазванияГородовЧасовойЗоны.ВГраница()) Цикл
					НазванияГородов.Добавить(НазванияГородовЧасовойЗоны[Индекс]);
				КонецЦикла;
				НазваниеГорода = СтрСоединить(НазванияГородов, ", ");
			КонецЕсли;
		КонецЕсли;
		Элементы.ЧасоваяЗонаПередачиТовара.СписокВыбора.Добавить(ЧасоваяЗонаРФ.Ключ, ЗаполнитьШаблонПредставленияЧасовойЗоны(ЧасоваяЗонаРФ.Ключ, НазваниеГорода));
	КонецЦикла;
	Элементы.ЧасоваяЗонаПередачиТовара.СписокВыбора.СортироватьПоЗначению();
	//-- Локализация
...
Процедура ОплатитьКартойЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОперацияПоПлатежнойКарте") Тогда
		ПараметрыОперации.ДокументОснование = ДокументСсылка;
	КонецЕсли;
...
Процедура ОбновитьПараметрыФискальногоЧека(ОбновитьМакетЧека = Истина)
	...
	Если ОплатаВИнтернетМагазине Тогда
		ПараметрыОперацииЧека.Вставить("АдресИнтернетМагазина", АдресИнтернетМагазина);
	КонецЕсли;
	...
	ПараметрыОперацииЧека.Вставить("ПризнакиСпособаРасчетаАвто", ПризнакиСпособаРасчетаАвто.Выгрузить());
	...
	ПараметрыОперацииЧека.Вставить("СуммаПредоплатыВнеОдногоДня", СуммаПредоплатыВнеОдногоДня);
	...
	ДополнитьПараметрыОперацииЧекаЛокализация(ПараметрыОперацииЧека, ВерсияФФД);
...
&НаСервере
Процедура ПолучитьОбработатьДанныеОплатыВИнтернетМагазинеИзДокумента()
	
	ЭтоДокументыИнтернетОплаты = ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОперацияПоПлатежнойКарте");
	//++ Локализация
	ЭтоДокументыИнтернетОплаты = ЭтоДокументыИнтернетОплаты Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОперацияПоЯндексКассе");
	//-- Локализация
	
	Если Не ЭтоДокументыИнтернетОплаты Тогда
		Возврат;
	КонецЕсли;
	...
	|	ДоговорыЭквайринга.СпособПроведенияПлатежа КАК СпособПроведенияПлатежа,
	|	ДоговорыЭквайринга.ОплатаВИнтернетМагазине КАК ОплатаВИнтернетМагазине,
	|	ДоговорыЭквайринга.АдресИнтернетМагазина КАК АдресИнтернетМагазина
	|ИЗ
	...
	|		ПО ОперацияПоПлатежнойКарте.ДоговорЭквайринга = ДоговорыЭквайринга.Ссылка
	|ГДЕ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДоговорыЭквайринга КАК ДоговорыЭквайринга
	|		ПО ОперацияПоЯндексКассе.ДоговорЭквайринга = ДоговорыЭквайринга.Ссылка
	|ГДЕ
	...
	Если Выборка.Следующий() Тогда
		ДоговорПодключенияКПлатежнымСистемам = Выборка.Ссылка;
		СпособПроведенияПлатежа = Выборка.СпособПроведенияПлатежа;
		
		ОплатаВИнтернетМагазинеВДоговореПодключения = Выборка.ОплатаВИнтернетМагазине;
		ОплатаВИнтернетМагазине = ОплатаВИнтернетМагазинеВДоговореПодключения;
		
		АдресИнтернетМагазинаВДоговореПодключения = Выборка.АдресИнтернетМагазина;
		Если ОплатаВИнтернетМагазинеВДоговореПодключения Тогда
			АдресИнтернетМагазина = АдресИнтернетМагазинаВДоговореПодключения;
		КонецЕсли;
	КонецЕсли;
	
	Если ОплатаВИнтернетМагазинеВДоговореПодключения
		И ВариантОтправкиЭлектронногоЧека = Перечисления.ВариантыОтправкиЭлектронногоЧекаПокупателю.НеОтправлять Тогда
...
&НаКлиенте
Функция ОплатыВИнтернетМагазинеЗаполненыКорректно()
	
	ОплатыВИнтернетМагазинеЗаполненыКорректно = Истина;
	
	Если ОплатаВИнтернетМагазине И НЕ ЗначениеЗаполнено(АдресИнтернетМагазина) Тогда
		ОплатыВИнтернетМагазинеЗаполненыКорректно = Ложь;
		ТекстСообщения = НСтр("ru = 'Не заполнен адрес интернет-магазина';");
		ОбщегоНазначенияКлиент.СообщитьПользователю(
				ТекстСообщения,
				,"Элемент.АдресИнтернетМагазина",,);
	КонецЕсли;
	
	Возврат ОплатыВИнтернетМагазинеЗаполненыКорректно;
...
Процедура ПослеОбработкиСостоянияСмены(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.КорректировкаРеализации")
		И РозничныеПродажиКлиент.ТребуетсяПроверкаКодовМаркировкиСредствамиККТ(ПараметрыОперацииФискализацииЧека) Тогда
...
&НаСервере
Процедура ДополнитьПараметрыОперацииЧекаЛокализация(ПараметрыОперацииЧека, ВерсияФФД)
	//++ Локализация	

	Если ДоступноУказаниеЧасовойЗоныПередачиТовараДляДокумента(ДокументСсылка, ТипФискальногоДокумента, ЕдиныйЧек) 
		И РозничныеПродажиКлиентСерверЛокализация.ТребуетсяПроверкаКодовМаркировкиСредствамиККТ(ПараметрыОперацииФискализацииЧека) Тогда
		Если ЧасоваяЗонаПередачиТовара > 0 Тогда
			ПараметрыОперацииЧека.Вставить("ЧасоваяЗона", ЧасоваяЗонаПередачиТовара);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПечатьЧекаЗавершениеЛокализация()
	...
	Если Не (ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг")) Тогда
		Возврат;
	КонецЕсли;
	
	ОбработатьДействияИСМППослеПечатиЧека(ДокументСсылка);
	Оповестить(СобытияФормКлиентЛокализация.ИмяСобытияПослеПробитияЧекаДляОбновленияПроверкиГИСМТ(),
		ДокументСсылка, ЭтотОбъект);
...
&НаСервереБезКонтекста
Процедура ОбработатьДействияИСМППослеПечатиЧека(ДокументСсылка)
...
// Заполняет представление часовой зоны РФ для списка выбора элемента
//
// Параметры:
//  ЧасоваяЗонаРФ - Число - 0, значит по данным регистрации ККТ, значения изменяются от 1 до 11, см. АдресныйКлассификатор.НомерЧасовойЗоны
//  НазваниеГорода - Строка - Название города, который должен представлять часовую зону, если не указано, то заполняется значением по умолчанию.
//
// Возвращаемое значение:
//  Строка -  
&НаСервере
Функция ЗаполнитьШаблонПредставленияЧасовойЗоны(ЧасоваяЗонаРФ = 0, Знач НазваниеГорода = "")
	
	СтрокаПредставления = "";
	Если Не ЗначениеЗаполнено(НазваниеГорода) Тогда
		НазваниеГорода = НазванияГородовЧасовыхЗонРФПоУмолчанию()[ЧасоваяЗонаРФ];
	КонецЕсли;
	Если ЧасоваяЗонаРФ = 0 Тогда
		СтрокаПредставления = НазваниеГорода;
	ИначеЕсли ЗначениеЗаполнено(НазваниеГорода) Тогда
		ШаблонПредставления = НСтр("ru = '(MSK%1%2) %3'");
		СмещениеОтМск = ЧасоваяЗонаРФ-2;
		СтрокаПредставления = СтрШаблон(ШаблонПредставления, ?(СмещениеОтМск >= 0, "+", ""), Формат(СмещениеОтМск, "ЧЦ=1; ЧН='0'; ЧГ=0"), НазваниеГорода);
	КонецЕсли;
...
// Возвращает соответствие названий городов часовым зонам РФ по умолчанию, ключом выступает номер часовой зоны от 1 до 11 см. АдресныйКлассификатор.НомерЧасовойЗоны.
//
// Возвращаемое значение:
//  Соответствие Из КлючИЗначение -
//
&НаСервере
Функция НазванияГородовЧасовыхЗонРФПоУмолчанию()
	Результат = Новый Соответствие;
	Результат.Вставить(0, НСтр("ru = 'По данным регистрации ККТ'"));
	Результат.Вставить(1, НСтр("ru = 'Калининградская область'"));
	Результат.Вставить(2, НСтр("ru = 'Москва'"));
	Результат.Вставить(3, НСтр("ru = 'Самара'"));
	Результат.Вставить(4, НСтр("ru = 'Екатеринбург'"));
	Результат.Вставить(5, НСтр("ru = 'Омск'"));
	Результат.Вставить(6, НСтр("ru = 'Красноярск'"));
	Результат.Вставить(7, НСтр("ru = 'Иркутск'"));
	Результат.Вставить(8, НСтр("ru = 'Якутск'"));
	Результат.Вставить(9, НСтр("ru = 'Владивосток'"));
	Результат.Вставить(10, НСтр("ru = 'Магадан'"));
	Результат.Вставить(11, НСтр("ru = 'Петропавловск-Камчатский'"));
    Возврат Результат;
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ДоступноУказаниеЧасовойЗоныПередачиТовараДляДокумента(ДокументСсылка, ТипФискальногоДокумента, ЕдиныйЧек)
	Результат = ТипФискальногоДокумента =  ПредопределенноеЗначение("Перечисление.ТипыФискальныхДокументовККТ.КассовыйЧек")
				И (ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
				Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ВозвратТоваровОтКлиента")
				Или (ЕдиныйЧек = Истина И (ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПриходныйКассовыйОрдер")
...
// Вычисляет часовые зоны РФ по доступным адресам покупателя. Функция использует адресные данные:
// Партнера, Контрагента, Грузополучателя, Склада, АдресаДоставки из документа.
//
// Параметры:
//  ДокументИсточникАдресов - ДокументСсылка, ТаблицаЗначений - 
//
// Возвращаемое значение:
//	Структура - Результат вычисления содержит следующие свойства:
//    *ЧасоваяЗонаПоУмолчанию - Число - Часовая зона передачи товара по умолчанию 0-11, если не удалось определить, то 0.
//    *ОписаниеОшибки - Строка - Заполняется описанием причины, по которой не удалось определить часовую зону по умолчанию.
//    *ЧасовыеЗоныНаселенныхПунктов - Соответствие из КлючИЗначение:
//											*Ключ - Число - Номер часовой зоны от 1 до 11.
//											*Значение - Массив Из Строка - Массив названий населенных пунктов.
//
&НаСервере
Функция ПолучитьЧасовыеЗоныРФПоАдресамПокупателяДоставка20(ДокументИсточникАдресов)
	Результат = Новый Структура;
	Результат.Вставить("ЧасоваяЗонаПоУмолчанию", 0);
	Результат.Вставить("ОписаниеОшибки", "");
	Результат.Вставить("ЧасовыеЗоныНаселенныхПунктов", Новый Соответствие);	
	
	РазрешенныеТипыСсылок = Новый Массив;
	РазрешенныеТипыСсылок.Добавить(Тип("ДокументСсылка.РеализацияТоваровУслуг"));
	РазрешенныеТипыСсылок.Добавить(Тип("ДокументСсылка.ВозвратТоваровОтКлиента"));
	
	ДокументыИсточники = Новый Массив;
	Если РазрешенныеТипыСсылок.Найти(ТипЗнч(ДокументИсточникАдресов)) <> Неопределено Тогда
		
		ДокументыИсточники.Добавить(ДокументИсточникАдресов);
		
	ИначеЕсли ТипЗнч(ДокументИсточникАдресов) = Тип("ТаблицаЗначений") 
			Или ТипЗнч(ДокументИсточникАдресов) = Тип("ДанныеФормыКоллекция") Тогда
			
		Для Каждого СтрокаИсточника Из ДокументИсточникАдресов Цикл
			Если РазрешенныеТипыСсылок.Найти(ТипЗнч(СтрокаИсточника.Документ)) <> Неопределено Тогда
				ДокументыИсточники.Добавить(СтрокаИсточника.Документ);
			КонецЕсли;
		КонецЦикла;
		
	КонецЕсли;
	
	Если ДокументыИсточники.Количество() Тогда		

		// Реквизит АдресДоставки имеет тип Строка(Неограниченной длины), поэтому он обрабатывается отдельно.
		МассивИсточниковАдресов = Новый Массив;
		МассивИсточниковАдресов.Добавить("ПартнерСсылка");
		МассивИсточниковАдресов.Добавить("КонтрагентСсылка");
		МассивИсточниковАдресов.Добавить("ГрузополучательСсылка");
		МассивИсточниковАдресов.Добавить("СкладСсылка");
		...
		ТекстЗапроса = "ВЫБРАТЬ
		               ...
		               |	ВЫБОР
		               |		КОГДА Документ.Партнер = ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)
		               |			ТОГДА НЕОПРЕДЕЛЕНО
		               |		ИНАЧЕ Документ.Партнер
		               |	КОНЕЦ КАК ПартнерСсылка,
		               |	ВЫБОР
		               ...
		               |	ВЫБОР
		               |		КОГДА Документ.Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		               |			ТОГДА НЕОПРЕДЕЛЕНО
		               |		ИНАЧЕ ВЫБОР
		               |				КОГДА Документ.Склад.ЭтоГруппа = ИСТИНА
		               |					ТОГДА НЕОПРЕДЕЛЕНО
		               |				ИНАЧЕ Документ.Склад
		               |			КОНЕЦ
		               |	КОНЕЦ КАК СкладСсылка,
		               |	Документ.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.Самовывоз) КАК ЭтоСамовывоз,
		               |	ВЫБОР
		               |		КОГДА Документ.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.Самовывоз)
		               |			ТОГДА """"
		               |		ИНАЧЕ Документ.АдресДоставкиЗначение
		               |	КОНЕЦ КАК АдресДоставкиЗначение
		               |ПОМЕСТИТЬ #ВтИсточникиАдресов
		               |ИЗ
		...
		ШаблонТекстЗапроса = "ВЫБРАТЬ
		                     |	ВтИсточникиАдресов.Дата КАК Дата,
		                     |	ВтИсточникиАдресов.ПолеИсточникАдреса КАК ИсточникАдреса,
		                     |	ВЫБОР
		                     |		КОГДА ТИПЗНАЧЕНИЯ(ВтИсточникиАдресов.ПолеИсточникАдреса) = ТИП(Справочник.Склады)
		                     |			ТОГДА ВтИсточникиАдресов.ЭтоСамовывоз
		                     |		ИНАЧЕ ЛОЖЬ
		                     |	КОНЕЦ КАК ЭтоСамовывоз
		                     |ИЗ
		                     |	#ВтИсточникиАдресов КАК ВтИсточникиАдресов
		                     |ГДЕ
		                     |	ВтИсточникиАдресов.ПолеИсточникАдреса <> НЕОПРЕДЕЛЕНО";
		
		ТекстЗапросовОбъединения = Новый Массив;
		Для Каждого ИсточникАдреса Из МассивИсточниковАдресов Цикл			
			ТекстЗапроса = СтрЗаменить(ШаблонТекстЗапроса, "ПолеИсточникАдреса", ИсточникАдреса);
			ТекстЗапросовОбъединения.Добавить(ТекстЗапроса);
		КонецЦикла;
		ТекстЗапросаОбъединения = СтрСоединить(ТекстЗапросовОбъединения, Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС);
		
		ТекстЗапроса = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               |	ВтИсточникиАдресов.Дата КАК Дата,
		               |	ВтИсточникиАдресов.ИсточникАдреса КАК ИсточникАдреса,
		               |	ВтИсточникиАдресов.ЭтоСамовывоз КАК ЭтоСамовывоз
		               |ИЗ
		               |	#ОбъединениеИсточниковАдресов КАК ВтИсточникиАдресов";
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#ОбъединениеИсточниковАдресов", "(" + ТекстЗапросаОбъединения + ")");		
		ТекстыЗапросов.Добавить(ТекстЗапроса);
	
		ТекстЗапроса = "ВЫБРАТЬ ПЕРВЫЕ 1
		               |	ВтИсточникиАдресов.АдресДоставкиЗначение КАК ИсточникАдреса
		               |ИЗ
		               |	#ВтИсточникиАдресов КАК ВтИсточникиАдресов
		               |ГДЕ
		               |	ВЫРАЗИТЬ(ВтИсточникиАдресов.АдресДоставкиЗначение КАК СТРОКА(1)) <> """"
		               |УПОРЯДОЧИТЬ ПО 
		               |	ВтИсточникиАдресов.Дата УБЫВ";
		
		ТекстыЗапросов.Добавить(ТекстЗапроса);

		Запрос.УстановитьПараметр("МассивСсылок", ДокументыИсточники);
		ТекстЗапроса = СтрСоединить(ТекстыЗапросов, ОбщегоНазначения.РазделительПакетаЗапросов());
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#ВтИсточникиАдресов", "ВтИсточникиАдресов");		
		Запрос.Текст = ТекстЗапроса;

		ИсточникиАдресов = Запрос.ВыполнитьПакет()[1].Выгрузить();
		ИсточникиАдресов.Колонки.Добавить("АдресДоставки");
		ИсточникиАдресов.Колонки.Добавить("ЭтоАдресДоставки", Новый ОписаниеТипов("Булево"));
		
		АдресаДоставки = Запрос.ВыполнитьПакет()[2].Выбрать();
		Пока АдресаДоставки.Следующий() Цикл
			СтрокаИсточника = ИсточникиАдресов.Добавить();
			СтрокаИсточника.АдресДоставки = АдресаДоставки.ИсточникАдреса;
			СтрокаИсточника.ЭтоАдресДоставки = Истина;
		КонецЦикла;
				
		УстановитьПривилегированныйРежим(Истина);
		
		ВычислитьЗначениеПоУмолчанию = Истина;
		Для Каждого ИсточникАдреса Из ИсточникиАдресов Цикл
			ЗначенияАдресов = Новый Массив;
			Если ИсточникАдреса.ЭтоАдресДоставки Тогда
				ЗначенияАдресов.Добавить(ИсточникАдреса.АдресДоставки);
			ИначеЕсли ЗначениеЗаполнено(ИсточникАдреса.ИсточникАдреса) Тогда
				Дата = ?(ЗначениеЗаполнено(ИсточникАдреса.Дата),ИсточникАдреса.Дата, ТекущаяДатаСеанса());
				АдресаИсточника = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(
					ИсточникАдреса.ИсточникАдреса, Перечисления.ТипыКонтактнойИнформации.Адрес, Дата, Ложь);
				ЗначенияАдресов = АдресаИсточника.ВыгрузитьКолонку("Значение");
			КонецЕсли;
			Для Каждого ЗначениеАдреса Из ЗначенияАдресов Цикл
				НомерЧасовойЗоны = АдресныйКлассификатор.НомерЧасовойЗоны(ЗначениеАдреса);
				Если ЗначениеЗаполнено(НомерЧасовойЗоны) Тогда
					ПоляАдреса = АдресныйКлассификаторСлужебный.КонтактнаяИнформацияВСтруктуруJSON(ЗначениеАдреса);
					
					// Наименование населенного пункта (город, деревня, село, поселок)
					НазваниеНаселенногоПункта = ?(ЗначениеЗаполнено(ПоляАдреса.locality), ПоляАдреса.locality,
													?(ЗначениеЗаполнено(ПоляАдреса.settlement), ПоляАдреса.settlement, ПоляАдреса.city));
					
					Если ЗначениеЗаполнено(НазваниеНаселенногоПункта) Тогда
						НазванияГородов = Результат.ЧасовыеЗоныНаселенныхПунктов[НомерЧасовойЗоны];
						Если НазванияГородов = Неопределено Тогда
							НазванияГородов = Новый Массив;
							НазванияГородов.Добавить(НазванияГородовЧасовыхЗонРФПоУмолчанию()[НомерЧасовойЗоны]);
							Результат.ЧасовыеЗоныНаселенныхПунктов.Вставить(НомерЧасовойЗоны, НазванияГородов);
						КонецЕсли;
						Если НазванияГородов.Найти(НазваниеНаселенногоПункта) = Неопределено Тогда
							НазванияГородов.Добавить(НазваниеНаселенногоПункта);	
						КонецЕсли;
					КонецЕсли;
					
					Если ВычислитьЗначениеПоУмолчанию 
						И (ИсточникАдреса.ЭтоАдресДоставки ИЛИ ИсточникАдреса.ЭтоСамовывоз) Тогда
						
						Если Результат.ЧасоваяЗонаПоУмолчанию = 0 Тогда
							Результат.ЧасоваяЗонаПоУмолчанию = НомерЧасовойЗоны;
						ИначеЕсли Результат.ЧасоваяЗонаПоУмолчанию <> НомерЧасовойЗоны Тогда
							Результат.ЧасоваяЗонаПоУмолчанию = 0;
							Результат.ОписаниеОшибки = НСтр("ru = 'В документах отгрузки указаны адреса доставки/самовывоза в разных часовых зонах. Выберите часовую зону вручную.'");
							ВычислитьЗначениеПоУмолчанию = Ложь;
						КонецЕсли;
					КонецЕсли;
...
// Вычисляет часовые зоны РФ по доступным адресам покупателя при использовании "Доставки 2.5".
//
// Параметры:
//  ДокументИсточникАдресов - ДокументСсылка, ТаблицаЗначений - 
//
...
//
&НаСервере
Функция ПолучитьЧасовыеЗоныРФПоАдресамПокупателяДоставка25(ДокументИсточникАдресов)
	...
	АдресаДоставкиРеализации = Новый ТаблицаЗначений;
	АдресаДоставкиРеализации.Колонки.Добавить("ДокументРеализации", Новый ОписаниеТипов("ДокументСсылка.РеализацияТоваровУслуг"));
	АдресаДоставкиРеализации.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	АдресаДоставкиРеализации.Колонки.Добавить("Характеристика", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
	АдресаДоставкиРеализации.Колонки.Добавить("Серия", Новый ОписаниеТипов("СправочникСсылка.СерииНоменклатуры"));
	АдресаДоставкиРеализации.Колонки.Добавить("Количество", ОбщегоНазначения.ОписаниеТипаЧисло(15, 3));
	АдресаДоставкиРеализации.Колонки.Добавить("КонечнаяТочкаДоставки", Новый ОписаниеТипов("СправочникСсылка.ТочкиДоставки"));
	...
	|ИТОГИ ПО
	|	Документ";
	Запрос.УстановитьПараметр("Ссылки", ДокументыИсточники);
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		ДокументСсылка = ВыборкаДокументов.Документ;
		
		ТоварыДокумента = Новый Массив;
		ВыборкаТоваров = ВыборкаДокументов.Выбрать();
		Пока ВыборкаТоваров.Следующий() Цикл
			ТоварыДокумента.Добавить(ВыборкаТоваров.Номенклатура);
		КонецЦикла;
		
		ОписаниеНоменклатуры = ОбщегоНазначенияИС.ОписаниеНоменклатуры(ТоварыДокумента);
		
		МаркированныеТовары = Новый Массив;
		Для Каждого Номенклатура Из ТоварыДокумента Цикл
			
			КлючВидовПродукции = ОписаниеНоменклатуры.Получить(Номенклатура).КлючВидовПродукции;
			ЭтоПродукцияИСМП = КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(
				КлючВидовПродукции, Истина);
			
			Если ЭтоПродукцияИСМП Тогда
				МаркированныеТовары.Добавить(Номенклатура);
			КонецЕсли;
			
		КонецЦикла;
		
		Если МаркированныеТовары.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ОтборНоменклатуры", МаркированныеТовары);
		
		АдресаДоставкиДокумента = ДоставкаТоваров.ДанныеРаспоряженияНаДоставку(
			ДокументСсылка, ДополнительныеПараметры);
		Для Каждого АдресДоставкиТовара Из АдресаДоставкиДокумента Цикл
			НовыйАдресДоставки = АдресаДоставкиРеализации.Добавить();
			ЗаполнитьЗначенияСвойств(НовыйАдресДоставки, АдресДоставкиТовара);
			НовыйАдресДоставки.ДокументРеализации = ДокументСсылка;
		КонецЦикла;
		
	КонецЦикла;
	
	АдресаДоставкиРеализации.Свернуть(
		"ДокументРеализации, Номенклатура, Характеристика, Серия, КонечнаяТочкаДоставки",
		"Количество");
	АдресаДоставки = АдресаДоставкиРеализации.Скопировать();
	АдресаДоставки.Свернуть("КонечнаяТочкаДоставки");
	
	УстановитьПривилегированныйРежим(Истина);
	
	МассивСсылокТочекДоставки = АдресаДоставки.ВыгрузитьКолонку("КонечнаяТочкаДоставки");
	
	АдресаТочекДоставки = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(
		МассивСсылокТочекДоставки, "АдресЗначенияПолей, Адрес");

	ЧасовыеЗоныТочек = Новый Соответствие;
	УникальныеЧасовыеЗоныЗначения = Новый Массив;

	Для Каждого ТочкаДоставки Из МассивСсылокТочекДоставки Цикл
		
		ТочкаДоставкиРеквизиты = АдресаТочекДоставки.Получить(ТочкаДоставки);
		Если ТочкаДоставкиРеквизиты = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		АдресЗначенияПолей = ТочкаДоставкиРеквизиты.АдресЗначенияПолей;
		
		НомерЧасовойЗоны = АдресныйКлассификатор.НомерЧасовойЗоны(АдресЗначенияПолей);
		Если ЗначениеЗаполнено(НомерЧасовойЗоны) Тогда
			ЧасовыеЗоныТочек.Вставить(ТочкаДоставки, НомерЧасовойЗоны);
			Если УникальныеЧасовыеЗоныЗначения.Найти(НомерЧасовойЗоны) = Неопределено Тогда
				УникальныеЧасовыеЗоныЗначения.Добавить(НомерЧасовойЗоны);
			КонецЕсли;
			
			ПоляАдреса = АдресныйКлассификаторСлужебный.КонтактнаяИнформацияВСтруктуруJSON(АдресЗначенияПолей);
	...
	Если УникальныеЧасовыеЗоныЗначения.Количество() > 1 Тогда
		
		МассивСообщений = Новый Массив;
		
		Для Каждого СтрокаАдреса Из АдресаДоставкиРеализации Цикл
			
			НомерЧасовойЗоны = ЧасовыеЗоныТочек.Получить(СтрокаАдреса.КонечнаяТочкаДоставки);
			Если ЗначениеЗаполнено(НомерЧасовойЗоны) Тогда
				
				ТекстТовара = НоменклатураКлиентСервер.ПредставлениеНоменклатуры(
					СтрокаАдреса.Номенклатура,
					?(ЗначениеЗаполнено(СтрокаАдреса.Характеристика), ЗначениеЗаполнено(СтрокаАдреса.Характеристика), ""),
					?(ЗначениеЗаполнено(СтрокаАдреса.Серия), ЗначениеЗаполнено(СтрокаАдреса.Серия), ""));
				
				АдресДоставки = АдресаТочекДоставки.Получить(СтрокаАдреса.КонечнаяТочкаДоставки);
				Если АдресДоставки = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				
				ИмяЧасовойЗоны = НазванияГородовЧасовыхЗонРФПоУмолчанию().Получить(НомерЧасовойЗоны);
				Если ЗначениеЗаполнено(ИмяЧасовойЗоны) Тогда
					ИмяЧасовойЗоны = ЗаполнитьШаблонПредставленияЧасовойЗоны(НомерЧасовойЗоны, ИмяЧасовойЗоны);
				Иначе
					ИмяЧасовойЗоны = Строка(НомерЧасовойЗоны);
				КонецЕсли;
				
				Шаблон = Нстр("ru = 'Товар ""%1"" в количестве ""%2"" - адрес доставки ""%3"" - часовая зона ""%4""'");
				
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					Шаблон,
					СокрЛП(ТекстТовара),
					СтрокаАдреса.Количество,
					СокрЛП(АдресДоставки.Адрес),
					СокрЛП(ИмяЧасовойЗоны));
		...
		Если МассивСообщений.Количество() > 1 Тогда
			...
			ТекстСообщения = Нстр("ru = 'В документе маркированный товар реализуется более чем в одной часовой зоне:'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
...
// Проверить наличие скидок и оплат бонусными баллами в документе реализации.
// 
// Параметры:
//  ДокументыДляПроверки - Массив Из ДокументСсылка.РеализацияТоваровУслуг - Документ для проверки.
// 
// Возвращаемое значение:
//  Булево - Реузльтат проверки наличия скидок оплат бонусными баллами
&НаСервере
Функция ПроверитьНаличиеСкидокОплатБонуснымиБалламиВРеализацииТоваровУслуг(ДокументыДляПроверки)
	...
	Запрос.УстановитьПараметр("МассивСсылок", ДокументыДляПроверки);
	...
	|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ ВидПродукцииИС,
	...
	|	ПО ОписаниеНоменклатурыИС.Номенклатура = ТоварыДокумента.Номенклатура
	|ГДЕ
	|	ТоварыДокумента.Ссылка В (&МассивСсылок)
	...
	|ГДЕ
	|	ТоварыДокумента.ВидПродукцииИС В (&ВидыПродукцииИС)
	...
	Пока ВыборкаЗапроса.Следующий() Цикл
		...
		Шаблон = Нстр("ru = 'В документе %1 заполнены скидки:'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			Шаблон,
			ВыборкаЗапроса.Ссылка) + Символы.ПС;

		ВыбораДетальныхЗаписей = ВыборкаЗапроса.Выбрать();
		Пока ВыбораДетальныхЗаписей.Следующий() Цикл
			НаименованиеТовара = НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати(
				ВыбораДетальныхЗаписей.НоменклатураНаименование,
				ВыбораДетальныхЗаписей.ХарактеристикаНаименование,,
				ВыбораДетальныхЗаписей.СерияНаименование);
			
			Шаблон = Нстр("ru = 'В строке №%1 для товара ""%2"" есть:'");
			
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				Шаблон,
				ВыбораДетальныхЗаписей.НомерСтроки,
				НаименованиеТовара);
			
			Если ЗначениеЗаполнено(ВыбораДетальныхЗаписей.СуммаАвтоматическойСкидки) Тогда
				...
				ТекстСообщения = ТекстСообщения + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					Шаблон,
					ВыбораДетальныхЗаписей.ПроцентАвтоматическойСкидки);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ВыбораДетальныхЗаписей.СуммаРучнойСкидки) Тогда
				Шаблон = Нстр("ru = 'ручная скидка ""%1%"";'");
				
				ТекстСообщения = ТекстСообщения + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					Шаблон,
					ВыбораДетальныхЗаписей.ПроцентРучнойСкидки);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ВыбораДетальныхЗаписей.СуммаБонусныхБалловКСписанию) Тогда
				Шаблон = Нстр("ru = 'оплата бонусными баллами ""%1"";'");
				
				ТекстСообщения = ТекстСообщения + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					Шаблон,
					ВыбораДетальныхЗаписей.СуммаБонусныхБалловКСписаниюВВалюте);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ВыбораДетальныхЗаписей.СуммаАвтоматическойСкидки)
				Или ЗначениеЗаполнено(ВыбораДетальныхЗаписей.СуммаРучнойСкидки)
				Или ЗначениеЗаполнено(ВыбораДетальныхЗаписей.СуммаБонусныхБалловКСписанию) Тогда
...
&НаСервере
Процедура ПроверитьКорректностьРозничнойЦеныТабачнойПродукции()
	
	//++ Локализация
	Если Не РозничныеПродажиЛокализация.КонтролироватьЦенуПродажиТабачнойПродукции(ТекущаяДатаСеанса()) Тогда
		Возврат;
	КонецЕсли;
	
	ТекЮрФизЛицо = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "ЮрФизЛицо");
	Если ТекЮрФизЛицо <> Перечисления.ЮрФизЛицо.ФизЛицо Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрокаДокументыЕдиногоЧека Из ДокументыЕдиногоЧека Цикл
		Если ТипЗнч(СтрокаДокументыЕдиногоЧека.Документ) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			ДокументыДляПроверки.Добавить(СтрокаДокументыЕдиногоЧека.Документ);
		КонецЕсли;
	КонецЦикла;
	
	Если ДокументыДляПроверки.Количество() Тогда
		РезультатПроверки = ПроверитьНаличиеСкидокОплатБонуснымиБалламиВРеализацииТоваровУслуг(ДокументыДляПроверки);
		
		// Подготовим данные необходимые для проверки
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
		|	РеализацияТоваровУслуг.Ссылка КАК Ссылка,
		|	РеализацияТоваровУслуг.ЦенаВключаетНДС КАК ЦенаВключаетНДС
		|ПОМЕСТИТЬ ДокументыДляПроверки
		|ИЗ
		...
		|	ДокументыДляПроверки.ЦенаВключаетНДС КАК ЦенаВключаетНДС
		|ИЗ ДокументыДляПроверки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДокументыДляПроверки КАК ДокументыДляПроверки
		|		ПО ДокументыДляПроверки.Ссылка = ШтрихкодыУпаковок.Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВЫРАЗИТЬ(ВЫБОР
		|		КОГДА ТоварыДокумента.Ссылка.ЦенаВключаетНДС
		...
		|		ЗНАЧЕНИЕ(Справочник.КлючиВидовПродукцииИС.ПустаяСсылка)) ВидПродукцииИС,
		...
		|	ТоварыДокумента.СуммаБонусныхБалловКСписанию КАК СуммаБонусныхБалловКСписанию,
		|	ТоварыДокумента.СуммаБонусныхБалловКСписаниюВВалюте КАК СуммаБонусныхБалловКСписаниюВВалюте,
		|	ТоварыДокумента.СуммаСНДС
		|ИЗ
		|	Документ.РеализацияТоваровУслуг.Товары КАК ТоварыДокумента
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДокументыДляПроверки КАК ДокументыДляПроверки
		|		ПО ДокументыДляПроверки.Ссылка = ТоварыДокумента.Ссылка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		|		ПО ОписаниеНоменклатурыИС.Номенклатура = ТоварыДокумента.Номенклатура
		|ГДЕ
		|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ОтборПоВидуПродукцииИС)";
		
		Запрос.УстановитьПараметр("МассивСсылок", ДокументыДляПроверки);
		Запрос.УстановитьПараметр("ОтборПоВидуПродукцииИС", КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииКоторымДоступноПолеМРЦ());
		РезультатыЗапроса = Запрос.ВыполнитьПакет();
		
		РеквизитыДокументов = РезультатыЗапроса[1].Выгрузить();
		РеквизитыДокументов.Индексы.Добавить("Ссылка");
		ШтрихкодыУпаковокДокументов = РезультатыЗапроса[2].Выгрузить();
		ШтрихкодыУпаковокДокументов.Индексы.Добавить("Ссылка");
		ТоварыДокументов = РезультатыЗапроса[3].Выгрузить();
		ТоварыДокументов.Индексы.Добавить("Ссылка");
		
		// Выполним проверку найденных документов
		Для Каждого ДокументДляПроверки Из РеквизитыДокументов Цикл
			
			ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(
				СтрокаДокументыЕдиногоЧека.Документ,,
				КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииКоторымДоступноПолеМРЦ(),
				Новый Структура);
			
			ШтрихкодыУпаковок = Новый Массив;
			Для Каждого СтрокаШтрихкодовУпаковки Из ШтрихкодыУпаковокДокументов.НайтиСтроки(Новый Структура("Ссылка", ДокументДляПроверки.Ссылка)) Цикл
				ШтрихкодыУпаковок.Добавить(СтрокаШтрихкодовУпаковки.ШтрихкодУпаковки);
			КонецЦикла;
			...
			РезультатПроверкиМинимальнойЦены = РозничныеПродажиЛокализация.ПроверитьШтрихкодыУпаковокНаМинимальнуюЦену(
				ДокументДляПроверки.Ссылка,
				ШтрихкодыУпаковок,
				ПараметрыСканирования,
				Истина);
			
			ТоварыДокумента = ТоварыДокументов.Скопировать(ТоварыДокументов.НайтиСтроки(Новый Структура("Ссылка", ДокументДляПроверки.Ссылка)));
			
			РезультатПроверкиРозничнойЦены = РозничныеПродажиЛокализация.ПроверитьКорректностьРозничнойЦеныПоМРЦ(
				ТоварыДокумента,
				ШтрихкодыУпаковокСМРЦ,
				ДокументДляПроверки.ЦенаВключаетНДС);
				
			РезультатПроверки = РезультатПроверки
				И РезультатПроверкиМинимальнойЦены
				И РезультатПроверкиРозничнойЦены;
	...
	Если РезультатПроверки = Ложь Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для пробития единого чека нужно привести цены продажи табачной продукции к значениями МРЦ из КМ.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	//-- Локализация
...
&НаСервере
Функция ПроверитьКорректностьРозничнойЦеныТабачнойПродукцииВКорректировкеРеализации()
	РезультатПроверки = Истина;
	//++ Локализация
	РеквизитыДляПроверки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "ЦенаВключаетНДС,ДокументОснование,ДокументОснование.Дата,ДокументОснование.Контрагент.ЮрФизЛицо");
	Если ТипЗнч(РеквизитыДляПроверки.ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		И РеквизитыДляПроверки.ДокументОснованиеКонтрагентЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо 
		И РозничныеПродажиЛокализация.КонтролироватьЦенуПродажиТабачнойПродукции(РеквизитыДляПроверки.ДокументОснованиеДата) Тогда
		
		ТабличныеЧастиДляПроверки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "Товары,ДокументОснование.ШтрихкодыУпаковок");
		ОтборПоВидуПродукцииИС = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииКоторымДоступноПолеМРЦ();
		ПозицииРеализацииТабака = РозничныеПродажиЛокализация.ТоварыДокументаРеализацииСВидамиПродукцииИС(РеквизитыДляПроверки.ДокументОснование, ОтборПоВидуПродукцииИС);
		Если ПозицииРеализацииТабака.Количество() Тогда

			СтрокиКорректировкиДляПроверки = Новый Массив;
			ТоварыКорректировки = ТабличныеЧастиДляПроверки.Товары.Выгрузить(); // ТаблицаЗначений
			ТоварыКорректировки.Колонки.Добавить("ВидПродукцииИС", Новый ОписаниеТипов("СправочникСсылка.ВидыПродукцииИС")); 
			ТоварыКорректировки.Индексы.Добавить("Номенклатура,Характеристика,Серия");
			Отбор = Новый Структура("Номенклатура,Характеристика,Серия");
			Для Каждого ПозицияРеализации Из ПозицииРеализацииТабака Цикл
				ЗаполнитьЗначенияСвойств(Отбор, ПозицияРеализации);
				СтрокиКорректировки = ТоварыКорректировки.НайтиСтроки(Отбор);
				Для Каждого СтрокаКорректировки Из СтрокиКорректировки Цикл
					Если ПозицияРеализации.Цена <> СтрокаКорректировки.Цена Тогда
						СтрокаКорректировки.ВидПродукцииИС = ПозицияРеализации.ВидПродукцииИС;
						СтрокиКорректировкиДляПроверки.Добавить(СтрокаКорректировки);
					КонецЕсли;
				КонецЦикла;
			КонецЦикла;
			...
			// Штрикоды упаковок используются из документа основания, т.к. есть проверка в РозничныеПродажиЛокализация.ПроверкаДанныхПередФискальнойОперацией
			ШтрихкодыУпаковок = ТабличныеЧастиДляПроверки.ДокументОснованиеШтрихкодыУпаковок.Выгрузить().ВыгрузитьКолонку("ШтрихкодУпаковки");
			ШтрихкодыУпаковокСМРЦ = РозничныеПродажиЛокализация.ПолучитьШтрихкодыУпаковокСМРЦ(
				ШтрихкодыУпаковок,
				ПараметрыСканирования,
				Истина);
				
			ТаблицаСтрокКорректировкиДляПроверки = ТоварыКорректировки.Скопировать(СтрокиКорректировкиДляПроверки);
			РезультатПроверки = РозничныеПродажиЛокализация.ПроверитьКорректностьРозничнойЦеныПоМРЦ(
				ТаблицаСтрокКорректировкиДляПроверки,
				ШтрихкодыУпаковокСМРЦ,
				РеквизитыДляПроверки.ЦенаВключаетНДС);
