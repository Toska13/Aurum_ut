...
Функция ТекстЗапросаДокументыПродажиКОформлению()
	...
	ТекстЗапроса = 
		...
		|ИЗ
		|	РегистрНакопления.РаспоряженияНаОтгрузкуИВозврат КАК РаспоряженияНаОтгрузкуПереопределяемый
		...
		|	И РаспоряженияНаОтгрузкуПереопределяемый.Количество > 0
		|	И РаспоряженияНаОтгрузкуПереопределяемый.Показатель = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Отгружено)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЗаказКлиента,
		|	Склад
