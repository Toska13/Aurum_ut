...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия <> "Запись_НаборКонстант" 
		И ИмяСобытия <> "ОбсужденияПодключены" Тогда
		Возврат;
	КонецЕсли;
	
	Если Источник = "ИспользоватьВнешнихПользователей"
		Или ИмяСобытия = "ОбсужденияПодключены"
		Или Источник = "ИспользоватьОповещенияПользователейОСобытиях" Тогда
...
&НаКлиенте
Процедура ИспользоватьОповещенияПользователейОСобытияхПриИзменении(Элемент)
	Подключаемый_ПриИзмененииРеквизита(Элемент);
КонецПроцедуры

&НаКлиенте
Процедура ИспользоватьОповещенияПользователейВTelegramПриИзменении(Элемент)
	Подключаемый_ПриИзмененииРеквизита(Элемент);
КонецПроцедуры

&НаКлиенте
Процедура ИспользоватьОповещенияПользователейПоПочтеПриИзменении(Элемент)
	Подключаемый_ПриИзмененииРеквизита(Элемент);
КонецПроцедуры
...
&НаКлиенте
Процедура ОткрытьВидыОповещенийПользователей(Команда)
	ОткрытьФорму("Справочник.ВидыОповещенийПользователей.Форма.ФормаСписка",, ЭтотОбъект);
КонецПроцедуры

&НаКлиенте
Процедура НастроитьИнтеграциюСTelegram(Команда)
	ОткрытьФорму("Обработка.ПодключениеКБотуОповещенийПользователей.Форма.СозданиеБотаTelegram",,ЭтотОбъект);
КонецПроцедуры
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если РеквизитПутьКДанным = "" 
		Или РеквизитПутьКДанным = "НаборКонстант.ИспользоватьОповещенияПользователейОСобытиях" Тогда
		...
		Элементы.ИспользоватьОповещенияПользователейОСобытиях.Доступность = СистемаВзаимодействийПодключена;
		Элементы.НастройкаВидовОповещенийПользователей.Доступность = СистемаВзаимодействийПодключена
			И НаборКонстант.ИспользоватьОповещенияПользователейОСобытиях;
		Элементы.КомментарийИспользоватьОповещенияПользователей.Видимость = Не СистемаВзаимодействийПодключена;
		Элементы.ДекорацияИспользоватьОповещенияПользователей.Видимость = Не СистемаВзаимодействийПодключена;
		
		Элементы.ИспользоватьОповещенияПользователейВTelegram.Доступность = НаборКонстант.ИспользоватьОповещенияПользователейОСобытиях И СистемаВзаимодействийПодключена;
		Элементы.ИспользоватьОповещенияПользователейПоПочте.Доступность = НаборКонстант.ИспользоватьОповещенияПользователейОСобытиях И СистемаВзаимодействийПодключена;
		
		Если НЕ СистемаВзаимодействийПодключена Тогда
			ТекстПодключить = НСтр("ru = 'Для использования оповещений пользователей о событиях системы необходимо подключение к сервису обсуждений.'");
			
			МассивСтрок = Новый Массив;
			МассивСтрок.Добавить(ТекстПодключить);
			Если ПодсистемаОбсужденияСуществует Тогда
				МассивСтрок.Добавить(Символы.ПС);
				МассивСтрок.Добавить(Новый ФорматированнаяСтрока(
					НСтр("ru = 'Подключиться'"), ,
					ЦветаСтиля.ГиперссылкаЦвет,
					Неопределено,
					"e1cib/app/" + Метаданные.Обработки.ПодключениеОбсуждений.ПолноеИмя()));
			КонецЕсли;
				
			Элементы.КомментарийИспользоватьОповещенияПользователей.Заголовок = Новый ФорматированнаяСтрока(МассивСтрок);
		КонецЕсли;
	КонецЕсли;
	Если РеквизитПутьКДанным = "" 
		Или РеквизитПутьКДанным = "НаборКонстант.ИспользоватьОповещенияПользователейВTelegram" Тогда
			Элементы.НастроитьИнтеграциюСTelegram.Доступность = НаборКонстант.ИспользоватьОповещенияПользователейОСобытиях
				И НаборКонстант.ИспользоватьОповещенияПользователейВTelegram;
	КонецЕсли;
КонецПроцедуры
