...
Процедура АссортиментИсточникаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.АссортиментИсточникаПометкаСнята Или Поле = Элементы.АссортиментИсточникаПометкаУстановлена Тогда
		...
		УстановитьПометку(ТекущиеДанные.Пометка, ТекущиеДанные.ИдентификаторПредложения);
...
Процедура Добавить(Команда)
	...
	Если Не ЭтоУстановкаПометокДляВсехПозиций И ИдентификаторыПредложенийВыбранные.Количество() = 0 Тогда
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("АссортиментИсточника.ИдентификаторПредложения");
...
Процедура ОбновитьПараметрыАссортиментаИсточника()
	...
	АссортиментИсточника.Параметры.УстановитьЗначениеПараметра("ИдентификаторыПредложенийВыбранные", ИдентификаторыПредложенийВыбранные.ВыгрузитьЗначения());
	АссортиментИсточника.Параметры.УстановитьЗначениеПараметра("ИдентификаторыПредложенийИсключенные", ИдентификаторыПредложенийИсключенные.ВыгрузитьЗначения());

КонецПроцедуры

&НаКлиенте
Процедура УстановитьПометку(Пометка, ИдентификаторПредложения = "")

	ОчиститьСообщения();

	Если ПустаяСтрока(ИдентификаторПредложения) Тогда
		ИдентификаторыПредложенийВыбранные.Очистить();
		ИдентификаторыПредложенийИсключенные.Очистить();
		ЭтоУстановкаПометокДляВсехПозиций = Пометка;

		ОбновитьПараметрыАссортиментаИсточника();
		Элементы.АссортиментИсточника.Обновить();
	Иначе
		ПараметрыИдентификаторыПредложенийВыбранные   = АссортиментИсточника.Параметры.Элементы.Найти("ИдентификаторыПредложенийВыбранные").Значение;
		ПараметрыИдентификаторыПредложенийИсключенные = АссортиментИсточника.Параметры.Элементы.Найти("ИдентификаторыПредложенийИсключенные").Значение;
		ПараметрыИндексПозицииУдаления   = ПараметрыИдентификаторыПредложенийИсключенные.Найти(ИдентификаторПредложения);

		ИндексПозицииУдаления   = ИдентификаторыПредложенийИсключенные.НайтиПоЗначению(ИдентификаторПредложения);

		Если Пометка Тогда
			Если ИндексПозицииУдаления <> Неопределено Тогда
				ПараметрыИдентификаторыПредложенийИсключенные.Удалить(ПараметрыИндексПозицииУдаления);
				ИдентификаторыПредложенийИсключенные.Удалить(ИндексПозицииУдаления);
			Иначе
				ПараметрыИдентификаторыПредложенийВыбранные.Добавить(ИдентификаторПредложения);
				ИдентификаторыПредложенийВыбранные.Добавить(ИдентификаторПредложения);
			КонецЕсли;
		Иначе
			Если ИндексПозицииУдаления <> Неопределено Тогда
				ПараметрыИдентификаторыПредложенийВыбранные.Удалить(ПараметрыИндексПозицииУдаления);
				ИдентификаторыПредложенийВыбранные.Удалить(ИндексПозицииУдаления);
			Иначе
				ПараметрыИдентификаторыПредложенийИсключенные.Добавить(ИдентификаторПредложения);
				ИдентификаторыПредложенийИсключенные.Добавить(ИдентификаторПредложения);
			КонецЕсли;
		КонецЕсли;

		Если ИдентификаторыПредложенийВыбранные.Количество() = 0 И ИдентификаторыПредложенийИсключенные.Количество() = 0 Тогда
			ЭтоУстановкаПометокДляВсехПозиций = Истина;
		КонецЕсли;
	КонецЕсли;
...
Функция ВызовЗаданияДобавленияАссортиментаНаСервере()
	...
	Если ЭтоУстановкаПометокДляВсехПозиций Тогда
		ПараметрыКомпоновщикаНастроек.Вставить("ИдентификаторыПредложенийИсключенные", ИдентификаторыПредложенийИсключенные);
	Иначе
		ПараметрыКомпоновщикаНастроек.Вставить("ИдентификаторыПредложенийВыбранные", ИдентификаторыПредложенийВыбранные);
	КонецЕсли;
	...
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(ПараметрыКомпоновщикаНастроек.НастройкиКомпоновкиДанных, "ИдентификаторыПредложенийВыбранные",
		ИдентификаторыПредложенийВыбранные);
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(ПараметрыКомпоновщикаНастроек.НастройкиКомпоновкиДанных, "ИдентификаторыПредложенийИсключенные",
		ИдентификаторыПредложенийИсключенные);
