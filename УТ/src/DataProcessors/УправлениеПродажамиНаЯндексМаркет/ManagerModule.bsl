...
Процедура ПрочитатьАктСверки(Знач ДанныеФайла, Параметры) Экспорт
	...
	Параметры.ТипЭлектронногоДокумента = Неопределено;
...
Процедура УстановитьПараметрыЗагрузкиИзФайлаВТабличнуюЧасть(Параметры) Экспорт
	...
	Если Параметры.ИмяТабличнойЧасти = "ДанныеОРеализованныхТоварах" Тогда
		...
		ОсновныеКолонки.Добавить("ИдентификаторПредложения");
		...
		ПустыеВИтогахКолонки.Добавить("ИдентификаторПредложения");
		ПустыеВИтогахКолонки.Добавить("ПредставлениеТовара");
		
		ЗаполняемыеПриЗагрузкеКолонки.Вставить("КонтрагентСозданПриЗагрузке", НСтр("ru = 'Контрагент создан при загрузке'"));
		ТипыДанныхКолонок.Вставить("КонтрагентСозданПриЗагрузке", ТипБулево);
		
		ЗаполняемыеПриЗагрузкеКолонки.Вставить("Партнер", НСтр("ru = 'Партнер'"));
		ТипыДанныхКолонок.Вставить("Партнер", Новый ОписаниеТипов("СправочникСсылка.Партнеры"));
		...
		ОбработчикиСобытий.Вставить("ИдентификаторПредложенияПриИзменении",
			"Обработки.УправлениеПродажамиНаЯндексМаркет.Подключаемый_ПолеПриИзменении");
		...
		ОбработчикиСобытий.Вставить("СуммаСкидкиПриИзменении",
			"Обработки.УправлениеПродажамиНаЯндексМаркет.Подключаемый_ПолеПриИзменении");
		ОбработчикиСобытий.Вставить("КонтрагентПриИзменении",
			"Обработки.УправлениеПродажамиНаЯндексМаркет.Подключаемый_ПолеПриИзменении");
	КонецЕсли;
...
Процедура ДополнитьУсловноеОформлениеТаблицыСопоставленияДанных(Форма, ИмяТабличнойЧасти) Экспорт
	...
	Если ИмяТабличнойЧасти = "ДанныеОРеализованныхТоварах" Тогда
		...
		ПолеОформления = ЭлементОформления.Поля.Элементы.Добавить();
		ПолеОформления.Поле = Новый ПолеКомпоновкиДанных(ИмяТаблицы + "_" + "Контрагент");
		
		ЭлементОтбора = ЭлементОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных(ИмяТаблицы + "." + "КонтрагентСозданПриЗагрузке");
		ЭлементОтбора.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
		ЭлементОтбора.ПравоеЗначение = Истина;
	КонецЕсли;
...
Процедура СопоставитьДанныеОРеализованныхТоварах(ТаблицаСопоставленияДанных, ДанныеИзФайла, СписокНеоднозначностей,
	...
	НастройкиУчетнойЗаписи = Параметры.ЗначенияПараметровСсылкиНаОбъект;
	
	КонстантыСервиса = ИнтеграцияСЯндексМаркетКлиентСервер.КонстантыСервиса();
	ВыполнятьПоискИСозданиеКонтрагентов =
		(НастройкиУчетнойЗаписи.СпособОтраженияПродаж <> КонстантыСервиса["СпособОтраженияПродажКомиссия"]);
	...
	Запрос.Текст =
		...
		|	ЗагружаемыеДанные.ИдентификаторПредложения КАК ИдентификаторПредложения,
		...
		|	ЗагружаемыеДанные.ЮридическийАдресКонтрагента КАК ЮридическийАдресКонтрагента,
	...
	Если ВыполнятьПоискИСозданиеКонтрагентов Тогда
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
		Запрос.Текст = ИнтеграцияСМаркетплейсамиСервер.ТекстЗапросаПоискаКонтрагентовИПартнеров("ВТ_ЗагружаемыеДанные");
		Запрос.Выполнить();
	КонецЕсли;
	...
	Запрос.Текст =
		...
		|			И ЗагружаемыеДанные.ИдентификаторПредложения = СтатусыПубликации.ИдентификаторПредложения
		|ГДЕ
	...
	Если ВыполнятьПоискИСозданиеКонтрагентов Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст,
			"ЗагружаемыеДанные.Контрагент КАК Контрагент",
			"ЕСТЬNULL(НайденныеКонтрагентыИПартнеры.Контрагент, ЗагружаемыеДанные.Контрагент) КАК Контрагент");
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст,
			...
			|		ПО ЗагружаемыеДанные.НаименованиеКонтрагента = НайденныеКонтрагентыИПартнеры.НаименованиеКонтрагента
			|			И ЗагружаемыеДанные.ИННКонтрагента = НайденныеКонтрагентыИПартнеры.ИННКонтрагента
			|			И ЗагружаемыеДанные.КППКонтрагента = НайденныеКонтрагентыИПартнеры.КППКонтрагента");
	...
	Пока ВыборкаИдентификатор.Следующий() Цикл
		...
		Пока ВыборкаДанных.Следующий() Цикл
			...
			Если Не ЗначениеЗаполнено(НоваяСтрока.Номенклатура) Тогда
				Если РезультатПоиска = Неопределено Тогда
					РезультатПоиска = ОпределитьНоменклатуруХарактеристикуУпаковку(ВыборкаПоиска, 
						ВыборкаИдентификатор.ИдентификаторПредложения, 
						"ИдентификаторПредложения");
				КонецЕсли;
			...
			Если ВыполнятьПоискИСозданиеКонтрагентов
					И Не ЗначениеЗаполнено(НоваяСтрока.Контрагент)
					И Не ПустаяСтрока(ВыборкаДанных.НаименованиеКонтрагента) Тогда
					
					КонтрагентСоздавался = Ложь;	
					ДанныеКонтрагента = Новый Структура();
					ДанныеКонтрагента.Вставить("НаименованиеКонтрагента", ВыборкаДанных.НаименованиеКонтрагента);
					ДанныеКонтрагента.Вставить("ИННКонтрагента", ВыборкаДанных.ИННКонтрагента);
					ДанныеКонтрагента.Вставить("КППКонтрагента", ВыборкаДанных.КППКонтрагента);
					ДанныеКонтрагента.Вставить("ЮридическийАдресКонтрагента", ВыборкаДанных.ЮридическийАдресКонтрагента);
					
					СтрокиТаблицыСопоставления = ТаблицаСопоставленияДанных.НайтиСтроки(ДанныеКонтрагента);
					
					Если СтрокиТаблицыСопоставления.Количество() > 0 Тогда
						Для каждого СтрокаТаблицыСопоставления Из СтрокиТаблицыСопоставления Цикл
							Если ЗначениеЗаполнено(СтрокаТаблицыСопоставления.Контрагент) Тогда
								КонтрагентСоздавался = Истина;
								НоваяСтрока.Контрагент = СтрокаТаблицыСопоставления.Контрагент;
								НоваяСтрока.Партнер = СтрокаТаблицыСопоставления.Партнер;
								Прервать;
							КонецЕсли;
						КонецЦикла;
					КонецЕсли;
					
					Если НЕ КонтрагентСоздавался Тогда
						ИнтеграцияСМаркетплейсамиСервер.ЗаполнитьДанныеПокупателя(
						НоваяСтрока,
						ВыборкаДанных,
						Параметры.УчетнаяЗапись,
					...
					Если ЗначениеЗаполнено(НоваяСтрока.Контрагент) Тогда
						НоваяСтрока.КонтрагентСозданПриЗагрузке = Истина;
					КонецЕсли;
			КонецЕсли;
...
//     * ИдентификаторПредложения    - Строка - внутренний идентификатор товара в системе продавца;
...
Функция НайтиНоменклатурыХарактеристикиУпаковки(ИсходныеДанные, РазделителиСловПоиска = Неопределено, 
	...
	Пока Истина Цикл
		...
		Если ЕстьШтрихкод
			...
			Для Каждого ЭлементКоллекции Из СтрРазделить(ВыборкаДанных.Штрихкод, ";", Ложь) Цикл
				...
				НоваяСтрока.ИдентификаторПредложения    = ВыборкаДанных.ИдентификаторПредложения;
		...
		НоваяСтрока.ЗначениеПоиска              = ВыборкаДанных.ИдентификаторПредложения;
		НоваяСтрока.ПоискХарактеристики         = Ложь;
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТаблицаПоиска.ИдентификаторПредложения КАК ИдентификаторПредложения,
		...
		|
		|ВЫБРАТЬ
		|	ТаблицаПоиска.ИдентификаторПредложения,
		...
		|
		|ВЫБРАТЬ
		|	Номенклатура.ИдентификаторПредложения,
		...
		|			И Номенклатура.ИдентификаторПредложения = ХарактеристикиНоменклатуры.ИдентификаторПредложения
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВложенныйЗапрос.ИдентификаторПредложения КАК ИдентификаторПредложения,
...
Функция НоваяТаблицаПоискаНоменклатурыХарактеристикиУпаковки() 
	...
	ОписаниеТипаСтрока255 = Новый ОписаниеТипов("Строка",,,, Новый КвалификаторыСтроки(255));
	...
	Результат.Колонки.Добавить("ИдентификаторПредложения",    ОписаниеТипаСтрока255);
...
Функция ТекстЗапросаПродажиСОтложеннымПереходомПраваСобственности(Знач ДокументыНеПомеченныеНаУдаление = Ложь)
	...
	ТекстЗапроса =
		...
		|	ТаблицаДанных.ИдентификаторПредложения КАК ИдентификаторПредложения,
		...
		|	ИтоговаяТаблицаДанных.ИдентификаторПредложения КАК ИдентификаторПредложения,
		...
		|	ИтоговаяТаблицаДанных.Контрагент КАК Контрагент,
		|	ИтоговаяТаблицаДанных.Партнер КАК Партнер,
...
Процедура Подключаемый_ПолеПриИзменении(Форма, ИмяПоля, ИдентификаторСтроки) Экспорт
	...
	Если Форма.ИмяТабличнойЧасти = "ДанныеОРеализованныхТоварах" Тогда
		...
		ИначеЕсли ИмяПоля = "ИдентификаторПредложения" Тогда
			Запрос = Новый Запрос;
			Запрос.Текст =
				"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
			...
			Запрос.УстановитьПараметр("УчетнаяЗапись",            Форма.УчетнаяЗапись);
			Запрос.УстановитьПараметр("ИдентификаторПредложения", ТекущиеДанные[ИмяПоля]);
		...
		ИначеЕсли ИмяПоля = "Контрагент" Тогда
			ТекущиеДанные.КонтрагентСозданПриЗагрузке = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура НоменклатураПриИзменении(Форма, ТекущиеДанные)
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ТекущиеДанные, 
		СтруктураДействий, 
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ИдентификаторПредложения)
		...
		Для Каждого СтрокаТаблицыЗначений Из Форма.ТаблицаСопоставленияДанных Цикл
			Если СтрокаТаблицыЗначений.ИдентификаторПредложения = ТекущиеДанные.ИдентификаторПредложения
