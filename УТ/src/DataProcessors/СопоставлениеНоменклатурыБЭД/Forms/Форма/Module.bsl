...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НастроитьФормуДляРаботыСПодсистемойОбменСКонтрагентами();
	...
	УстановитьСвойстваПереопределяемыхЭлементовФормы(Параметры.ОграничениеТипаНоменклатуры);
	...
	Если Не ЗначениеЗаполнено(ХранилищеСистемныхНастроек.Загрузить(ЭтотОбъект.ИмяФормы+"/ТекущиеДанные")) Тогда
		// Если нет сохраненных настроек, то обработчик ПриЗагрузкеДанныхИзНастроекНаСервере не вызывается, поэтому
		//  нужно вызвать поиск вариантов сразу при создании формы.
...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	Если ЗначениеЗаполнено(Справочник) Тогда
		// Это означает, что ограничение типа установлено вызывающим кодом и менять его на пользовательские настройки не нужно
		Настройки.Вставить("Справочник", Справочник);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Если ЗначениеЗаполнено(Справочник) 
		И Не Элементы.СопоставлениеНоменклатура.ОграничениеТипа.СодержитТип(Справочник) Тогда
		Справочник = Неопределено;
	КонецЕсли;
	
	УстановитьОграничениеТипаНоменклатуры();
	ЗапуститьПоискВариантов();
...
Процедура ТочностьПоискаПриИзменении(Элемент)
	...
	ЗапуститьПоискВариантовНаКлиенте();
...
&НаКлиенте
Процедура Подключаемый_ПараметрыПоискаПриИзменении(Элемент)
	ЗапуститьПоискВариантовНаКлиенте();
КонецПроцедуры
...
Процедура ОбработатьПараметрыФормы()
	...
	Если ЗначениеЗаполнено(Параметры.ДополнительныеРеквизитыСопоставления) Тогда
		СоздатьДополнительныеРеквизитыСопоставления(Параметры.ДополнительныеРеквизитыСопоставления);
	КонецЕсли;
	Если ЗначениеЗаполнено(Параметры.НоменклатураКонтрагентов) Тогда
		НоменклатураКонтрагентов = Параметры.НоменклатураКонтрагентов;
		ЗаполнитьНоменклатуруКонтрагентов(НоменклатураКонтрагентов);
	...
	Если ЗначениеЗаполнено(Параметры.ВидимостьКолонокСопоставления) Тогда
	...
	ТочностьПоиска            = Параметры.ТочностьПоискаПоУмолчанию;
	ТочностьПоискаДоИзменения = Параметры.ТочностьПоискаПоУмолчанию;
	
	Если Не СуществуетПодсистемаОбменСКонтрагентами Тогда
		Возврат;
	КонецЕсли;
	
	ЭтотОбъект[ИмяРеквизитаДоговор()] = Параметры.ДоговорКонтрагента;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Организация", "Видимость",
		Параметры.ОрганизацияВидимость);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, ИмяРеквизитаДоговор(), "Видимость",
		Параметры.ДоговорВидимость);
	
	МодульИнтерфейсДокументовЭДО = ОбщегоНазначения.ОбщийМодуль("ИнтерфейсДокументовЭДО");
	МодульИнтерфейсДокументовЭДО.УстановитьСвязиПараметровВыбораДоговорКонтрагента(Элементы[ИмяРеквизитаДоговор()],
		"Организация",
		"ВладелецОтбор");
...
&НаСервере
Процедура УстановитьСвойстваПереопределяемыхЭлементовФормы(ОграничениеТипаНоменклатуры)
	...
	СопоставлениеНоменклатураЗаголовокПоУмолчанию  = Элементы.СопоставлениеНоменклатура.Заголовок;
	...
	ИспользуемыеТипыНоменклатурыБЭД = СопоставлениеНоменклатурыКонтрагентовПовтИсп.ИспользуемыеТипыНоменклатурыБЭД();
	ЕстьПравоДобавленияНоменклатуры = ЕстьПравоДобавленияНоменклатуры(ИспользуемыеТипыНоменклатурыБЭД);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КомандаСоздатьНоменклатуру", "Видимость",
		ЕстьПравоДобавленияНоменклатуры);
	НастроитьФормуДляРаботыССоставнымТипомДанныхНоменклатуры(ИспользуемыеТипыНоменклатурыБЭД,
...
Процедура УстановитьСвойстваЭлементаОтбораПоВладельцу()
	...
	Если ВсеВладельцы.Количество() = 1 Тогда
		...
		Заголовок = Заголовок + " " + Строка(ВладелецОтбор);
	КонецЕсли;
...
Процедура СоздатьДополнительныеРеквизитыСопоставления(ДополнительныеРеквизитыСопоставления)
	...
	Если ТипЗнч(ДополнительныеРеквизитыСопоставления) <> Тип("Массив") 
		Или ДополнительныеРеквизитыСопоставления.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура НачатьПоискВариантов(НаборНоменклатурыКонтрагентов, Знач ТребуетсяПоискНоменклатуры = Истина, Знач ПолныйПоиск = Истина, Знач ОтборПоиска = Неопределено)
	...
	Если СуществуетПодсистемаОбменСКонтрагентами Тогда
		ПараметрыМетода.Вставить("Договор", ЭтотОбъект[ИмяРеквизитаДоговор()]);
	КонецЕсли;
	
	УстановитьРежимПоискаВариантовСопоставления(Истина);
	
	ОперацияПоискаВариантов = ДлительныеОперации.ВыполнитьВФоне(
		"Обработка.СопоставлениеНоменклатурыБЭД.МодульОбъекта.НайтиВариантыСопоставленияНоменклатуры",
		ПараметрыМетода,
		ПараметрыВыполнения);
...
&НаКлиенте
Процедура ЗапуститьПоискВариантовНаКлиенте()
...
&НаСервереБезКонтекста
Функция ЕстьПравоДобавленияНоменклатуры(Знач ИспользуемыеТипыНоменклатурыБЭД)
	
	ЕстьПраво = Истина;
	
	Для каждого ТипНоменклатуры Из ИспользуемыеТипыНоменклатурыБЭД Цикл
...
Процедура ОткрытьФормуПравилаПоиска(ДействиеСПравиломПоиска)
	...
	ПараметрыОткрытияФормы = 
		СопоставлениеНоменклатурыКонтрагентовСлужебныйКлиент.ПараметрыОткрытияФормыПравилоПоискаПоЧастиНаименования();
	ЗаполнитьЗначенияСвойств(ПараметрыОткрытияФормы, ТекущиеДанные);
	ПараметрыОткрытияФормы.ОбязательноеЗаполнениеХарактеристики = ТекущиеДанные.ОбязательноеЗаполнениеХарактеристики;
	...
	Иначе
		...
		ПараметрыОткрытияФормы.Ключ = Новый("РегистрСведенийКлючЗаписи.ПравилаПоискаПоЧастиНаименованияБЭД", 
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ЗначениеКлюча));
	КонецЕсли;
	
	ПараметрыОткрытияФормы.ОграничениеТипаНоменклатуры = Элементы.СопоставлениеНоменклатура.ОграничениеТипа;
...
#Область РаботаССоставнымТипомДанныхНоменклатуры

&НаСервере
Процедура НастроитьФормуДляРаботыССоставнымТипомДанныхНоменклатуры(ИспользуемыеТипыНоменклатурыБЭД, 
	ОграничениеТипаНоменклатуры)
	
	Если ЗначениеЗаполнено(ОграничениеТипаНоменклатуры) Тогда
		Элементы.СопоставлениеНоменклатура.ОграничениеТипа = ОграничениеТипаНоменклатуры;
		Элементы.Справочник.СписокВыбора.ЗагрузитьЗначения(ОграничениеТипаНоменклатуры.Типы());
	Иначе
		Элементы.СопоставлениеНоменклатура.ОграничениеТипа = Новый ОписаниеТипов(ИспользуемыеТипыНоменклатурыБЭД);
		Элементы.Справочник.СписокВыбора.ЗагрузитьЗначения(ИспользуемыеТипыНоменклатурыБЭД);
	КонецЕсли;
	
	ВсегоТипов = Элементы.СопоставлениеНоменклатура.ОграничениеТипа.Типы().Количество();
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Справочник", "Видимость", ВсегоТипов > 1);
	Если ВсегоТипов = 1 Тогда
		Справочник = Элементы.СопоставлениеНоменклатура.ОграничениеТипа.Типы()[0];
	КонецЕсли;
...
&НаСервере
Процедура УстановитьОграничениеТипаНоменклатуры()
	
	Если ТипЗнч(Справочник) = Тип("Тип") И ЗначениеЗаполнено(Справочник) Тогда
		НовоеОграничениеТипа = Новый ОписаниеТипов(
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочник));
	Иначе
		НовоеОграничениеТипа = ОписаниеТипаИспользуемыеТипы;
	КонецЕсли;
	
	Если Элементы.СопоставлениеНоменклатура.ОграничениеТипа <> НовоеОграничениеТипа Тогда
		Элементы.СопоставлениеНоменклатура.ОграничениеТипа = НовоеОграничениеТипа;
		УстановитьЗаголовокЭлементаСопоставлениеНоменклатураПоТипу();
	КонецЕсли;
...
Процедура УстановитьЗаголовокЭлементаСопоставлениеНоменклатураПоТипу()
	...
	ЭлементНоменклатура = Элементы.СопоставлениеНоменклатура;
	ЭлементНоменклатура.Заголовок = СопоставлениеНоменклатурыКонтрагентовСлужебный.ПредставлениеОбъектаПоТипу(
		...
		СопоставлениеНоменклатураЗаголовокПоУмолчанию);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция ИмяРеквизитаДоговор()
	Возврат "ДоговорКонтрагента";
КонецФункции

&НаСервере
Процедура НастроитьФормуДляРаботыСПодсистемойОбменСКонтрагентами()
	
	СуществуетПодсистемаОбменСКонтрагентами = ОбщегоНазначения.ПодсистемаСуществует(
	...
	ДобавляемыеРеквизиты = Новый Массив;
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(ИмяРеквизитаДоговор(),
		Метаданные.ОпределяемыеТипы.ДоговорСКонтрагентомЭДО.Тип,, НСтр("ru = 'Договор'")));
	
	ИзменитьРеквизиты(ДобавляемыеРеквизиты);
	
	Элемент = Элементы.Вставить(ИмяРеквизитаДоговор(), Тип("ПолеФормы"), Элементы.ГруппаПараметрыПоиска,
		Элементы.ГруппаТочностьПоиска);
	Элемент.ПутьКДанным = ИмяРеквизитаДоговор();
	Элемент.Вид         = ВидПоляФормы.ПолеВвода;
	Элемент.УстановитьДействие("ПриИзменении", "Подключаемый_ПараметрыПоискаПриИзменении");
