...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ТорговыеПредложения_УдалитьИзЗаказа"
		...
		УдалитьПредложенияИзЭлементовКорзины(Параметр.ИдентификаторыПредложений);
		ОбновитьКоличествоТоваровЭлементаКорзины();
...
&НаКлиенте
Процедура УдалитьПредложенияИзЭлементовКорзины(ИдентификаторыПредложений)
	
	Для Каждого Идентификатор Из ИдентификаторыПредложений Цикл
...
&НаКлиенте
Процедура ИзменитьКоличествоПредложенияВЭлементахКорзины(Идентификатор, Количество)
	
	НайденныеСтрокиТоваров = Товары.НайтиСтроки(Новый Структура("Идентификатор", Идентификатор));
	Если НайденныеСтрокиТоваров.Количество() > 0 Тогда
		НайденныеСтрокиТоваров[0].Количество = Количество;
	КонецЕсли;
	
	НайденныеСтрокиСписка = Список.НайтиСтроки(Новый Структура("Идентификатор", Идентификатор));
	Если НайденныеСтрокиСписка.Количество() > 0 Тогда
		НайденныеСтрокиСписка[0].ГиперссылкаДобавить = Количество;
	КонецЕсли;
...
Процедура НайтиТорговыеПредложенияВФоне(
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = '1С:Бизнес-сеть. Поиск торговых предложений по отборам.'");
	ПараметрыВыполнения.ЗапуститьВФоне              = Истина;
	ПараметрыВыполнения.ОжидатьЗавершение           = 0;
	
	ФоновоеЗаданиеПоиска = ДлительныеОперации.ВыполнитьВФоне(
		"ТорговыеПредложенияСлужебный.ПоискТорговыхПредложенийПоОтборуВФоне",
		ПараметрыЗапроса, 
		ПараметрыВыполнения);
...
Процедура ЗаполнитьТаблицуТорговыхПредложений(Знач АдресРезультата)
	...
	Если ЗначениеЗаполнено(ДанныеПоискаИзСервиса.ДеревоКатегорий) Тогда
		НовыйДеревоКатегорийКонтрольнаяСуммаСтрокой = 
			ОбщегоНазначения.КонтрольнаяСуммаСтрокой(ДанныеПоискаИзСервиса.ДеревоКатегорий);
		Если НовыйДеревоКатегорийКонтрольнаяСуммаСтрокой <> ДеревоКатегорийКонтрольнаяСуммаСтрокой Тогда
			ДеревоКатегорийКонтрольнаяСуммаСтрокой = НовыйДеревоКатегорийКонтрольнаяСуммаСтрокой;
	...
	Если ЗначениеЗаполнено(ДанныеПоискаИзСервиса.ДанныеКорзины) Тогда
		НовыйДанныеКорзиныКонтрольнаяСуммаСтрокой = 
			ОбщегоНазначения.КонтрольнаяСуммаСтрокой(ДанныеПоискаИзСервиса.ДанныеКорзины);
		Если НовыйДанныеКорзиныКонтрольнаяСуммаСтрокой <> ДанныеКорзиныКонтрольнаяСуммаСтрокой Тогда
			ДанныеКорзиныКонтрольнаяСуммаСтрокой = НовыйДанныеКорзиныКонтрольнаяСуммаСтрокой;
...
Процедура ПослеВыполненияДействияНадТоваром(Результат, ДополнительныеПараметры) Экспорт
	...
	РезультатыДействияНадТоваром = РезультатыДействияНадТоваром(Результат.АдресРезультата);
	
	Если РезультатыДействияНадТоваром.Отказ Тогда
		
		КоличествоИзЗагруженныхДанныхКорзины = КоличествоТовараИзЗагруженныхДанныхКорзины(
			ДополнительныеПараметры.Идентификатор, РезультатыДействияНадТоваром.ДанныеКорзины);
			
		Если КоличествоИзЗагруженныхДанныхКорзины = 0 Тогда
			ИдентификаторыПредложенийДляУдаления = 
				ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДополнительныеПараметры.Идентификатор);
			УдалитьПредложенияИзЭлементовКорзины(ИдентификаторыПредложенийДляУдаления);
		Иначе
			ИзменитьКоличествоПредложенияВЭлементахКорзины(
				ДополнительныеПараметры.Идентификатор, КоличествоИзЗагруженныхДанныхКорзины);
		КонецЕсли;

	Иначе
		
		Если ЗначениеЗаполнено(РезультатыДействияНадТоваром.ДанныеКорзины) Тогда
			ЗаполнитьСсылкиКоманд(ДополнительныеПараметры.Идентификатор, РезультатыДействияНадТоваром.ДанныеКорзины);
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция РезультатыДействияНадТоваром(Знач АдресРезультата)
		
	Результат = Новый Структура;
	Результат.Вставить("Отказ", Ложь);
	Результат.Вставить("ДанныеКорзины");
	
	РезультатыДействияНадТоваром = ПолучитьИзВременногоХранилища(АдресРезультата);
	Ошибка = РезультатыДействияНадТоваром.Ошибка;
	Если Ошибка <> Неопределено Тогда
		Если Не ПустаяСтрока(Ошибка.ТекстОшибкиСервиса) Тогда
			ОбщегоНазначения.СообщитьПользователю(Ошибка.ТекстОшибкиСервиса);
		ИначеЕсли Не ПустаяСтрока(Ошибка.ТекстОшибки) Тогда
			ОбщегоНазначения.СообщитьПользователю(Ошибка.ТекстОшибки);
		КонецЕсли;
	КонецЕсли;
	
	Результат.Отказ = РезультатыДействияНадТоваром.Отказ;
	
	Если ЗначениеЗаполнено(РезультатыДействияНадТоваром.ДанныеКорзины) Тогда
		
		ДанныеКорзиныДляКлиента = Новый Структура("Заказы", Новый Массив); // Массив Из Структура
	
		Для Каждого Заказ Из РезультатыДействияНадТоваром.ДанныеКорзины.Заказы Цикл
			ТоварыЗаказа = Новый Структура("Товары", Новый Массив); // Массив Из Структура
			ТоварыЗаказа.Товары = ОбщегоНазначения.ТаблицаЗначенийВМассив(Заказ.Товары);
			ДанныеКорзиныДляКлиента.Заказы.Добавить(ТоварыЗаказа);
		КонецЦикла;
		
		Результат.ДанныеКорзины = ДанныеКорзиныДляКлиента;
...
&НаКлиенте
Функция КоличествоТовараИзЗагруженныхДанныхКорзины(ИдентификаторПредложения, ДанныеКорзины)
	
	Результат = 0;
	
	Если ДанныеКорзины.Заказы = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;

	Для Каждого Заказ Из ДанныеКорзины.Заказы Цикл
		Для Каждого СтрокаТовара Из Заказ.Товары Цикл
			
			Если СтрокаТовара.Идентификатор <> ИдентификаторПредложения Тогда
				Продолжить;
			КонецЕсли;
