...
#Область СтатистикаСопоставления
	...
	Если ПараметрыВыполнения.ИспользоватьСтатистикуСопоставления Тогда
		...
		Если Не ПустаяСтрока(АдресРезультатаПодсчетаСтатистики) Тогда
			...
			Элементы.СопоставлениеРазноеЗаголовок.Заголовок = СтрШаблон(НСтр("ru = 'Не сопоставлено %1%% (%2)'"),
...
#Область РегистрацияОрганизаций
	...
	Если ПараметрыВыполнения.ИспользоватьРегистрациюОрганизации Тогда
		...
		ОрганизацииТребуетсяПовторноеПодключение = 
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
				Статистика, "ОрганизацииТребуетсяПовторноеПодключение", Новый Массив);
		
		КоличествоОрганизацийТребуетсяПовторноеПодключение = ОрганизацииТребуетсяПовторноеПодключение.Количество();

		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы, "ОрганизацииТребуетсяПовторноеПодключение", "Видимость",
				КоличествоОрганизацийТребуетсяПовторноеПодключение > 0);
				
		ШаблонТребуетсяПовторноеПодключение = НСтр("ru = 'Требуется повторное подключение (%1)'", 
			ОбщегоНазначения.КодОсновногоЯзыка());
			
		ТекстТребуетсяПовторноеПодключение = 
			СтрШаблон(ШаблонТребуетсяПовторноеПодключение, КоличествоОрганизацийТребуетсяПовторноеПодключение);

		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы, "ОрганизацииТребуетсяПовторноеПодключениеНадпись", "Заголовок", 
				ТекстТребуетсяПовторноеПодключение);
...
Процедура ОбновитьСтатистикуПубликации(ИспользоватьСтатистикуСопоставления, ИспользоватьСтатистикуПубликации,
	...
	Если ИспользоватьРегистрациюОрганизации Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы, "ОрганизацииТребуетсяПовторноеПодключение", "Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
...
Процедура ПоказатьСкрытьПояснения(ПоказыватьПояснения, Элементы)
	...
	ЗаголовокПоказыватьСкрытьПояснения = ?(ПоказыватьПояснения, 
		НСтр("ru = 'Скрыть пояснения'", КодОсновногоЯзыка),
		НСтр("ru = 'Показать пояснения'", КодОсновногоЯзыка));
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ПоказатьСкрытьПояснения", "Заголовок", ЗаголовокПоказыватьСкрытьПояснения);
...
&НаСервере
Процедура УсловноеГруппыОформлениеСинхронизации(РегламентноеЗаданиеВключено = Неопределено)
...
Процедура ПослеПодключенияОрганизации(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗакрытия <> Неопределено 
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "РезультатНадпись", "Видимость", Ложь);
		ПараметрыСинхронизации = ПараметрыСинхронизации();
		ТорговыеПредложенияКлиент.ВыполнитьСинхронизациюТорговыхПредложений(
			ПараметрыСинхронизации, 
			ДлительнаяОперация);
