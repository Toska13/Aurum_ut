...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗагружатьДокументы = Истина;
	ПодключениеАвтоВыписки = Ложь;
...
Процедура Подключаемый_ПолучитьВыпискуАсинхронныйОбмен()
	...
	Если ВидОперации = "ПробноеПолучениеВыписки" Тогда
		ПараметрыЗапроса.Вставить("ПодключениеАвтоВыписки", ПодключениеАвтоВыписки);
		ПараметрыЗапроса.Вставить("ЗагружатьДокументы", ЗагружатьДокументы);
	КонецЕсли;
...
Процедура ПослеСохраненияПолученныхПодписейВыписки(ЕстьОшибка, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ВыпискиБанка.Количество() Тогда
		...
		ПоказатьОповещениеПользователя(ЗаголовокОповещения, , ТекстОповещения);
		Если ВидОперации = "ПробноеПолучениеВыписки" Тогда
			Если ЗначениеЗаполнено(СтруктураВозврата.ТестОшибкиРазбораВыписки) Тогда
				РезультатПолученияВыписки = Новый Структура("ЕстьОшибкаРазбораВыписки,ТестОшибкиРазбораВыписки");
				ЗаполнитьЗначенияСвойств(РезультатПолученияВыписки, СтруктураВозврата);
				ЗакрытьФормуНаКлиенте(Ложь, РезультатПолученияВыписки);
			Иначе
				Закрыть(Истина);
			КонецЕсли;
		Иначе
...
Процедура ПослеОтправкиОчередногоЗапросаВыпискиСбербанк(Результат,
	...
	Если Результат = Неопределено ИЛИ Результат = "ОшибкаУПШ103" Тогда
		ПриОшибкеТекущейОперацииСбербанк();
		ЗакрытьФормуНаКлиенте(Истина);
		Возврат;
	КонецЕсли;
...
Функция ВыпискиПолученыСбербанк(Знач НастройкаОбмена, Знач МассивЗапросов,
	...
	Если ВсегоЗапросов > 0 И КоличествоОтклоненныхЗапросовВыписки = ВсегоЗапросов Тогда
		// Из банка уже ничего не получим, нужно прервать бесконечное ожидание и вывести ошибку.
...
Процедура ПослеОтправкиСообщенияОбменаВСбербанк(Тикет,
	...
	Если НЕ ЗначениеЗаполнено(Тикет) Тогда
		...
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеОтправкиСообщений, Результат);
		Возврат;
	ИначеЕсли Тикет = "ОшибкаУПШ103" Тогда
		Результат = Новый Структура;
		Результат.Вставить("Успех", Ложь);
		Результат.Вставить("ОшибкаУПШ103", Истина);
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеОтправкиСообщений, Результат);
		Возврат;
	КонецЕсли;
...
Процедура ПослеОтправкиСообщенийОбменаСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	ОшибкаУПШ103 = Неопределено;
	Если Не Результат.Успех И Не Результат.Свойство("ОшибкаУПШ103", ОшибкаУПШ103) Тогда

		ПриОшибкеТекущейОперацииСбербанк();
		ЗакрытьФормуНаКлиенте(, Результат);
		Возврат;
		
	ИначеЕсли ОшибкаУПШ103 = Истина Тогда
...
Процедура ПослеОтправкиЗапросовВыписокПоЛогинуСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		...
		Если ЭтоОшибкаУПШ Тогда
			...
			ОбменСБанкамиСлужебныйКлиент.ВывестиСообщениеОбОшибкеУПШ103();
...
Функция ЗапускЗаданияПробнойОтправкиЗапросаВыпискиВБанк(Знач ПараметрыЗапроса, Знач УникальныйИдентификатор)
	...
	Если ПравоДоступа("Редактирование", Метаданные.Справочники.НастройкиОбменСБанками) Тогда
		...
		ДанныеАвторизацииНастройкаОбмена = ОбменСБанкамиСлужебный.ДанныеАвторизацииНастройкаОбмена(
			ПараметрыЗапроса.НастройкаОбмена);
		Пароль = ДанныеАвторизацииНастройкаОбмена.ПарольАвтовыписка;
