...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(Параметры.СтатистикаПотребления) = Тип("Структура") Тогда
		...
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Параметры.СтатистикаПотребления);
	...
	Если Параметры.РежимОткрытия = "Полный" Или Параметры.РежимОткрытия = "ТолькоСпособ" Тогда
		...
		Если ЗначениеЗаполнено(СпособУнаследованный) Тогда
			ПараметрыЦиклаПоставкиПоГрафикуРаботыУнаследованные = ПараметрыЦиклаПоставкиПоГрафикуРаботы(
				Параметры.КлючПотребности, СпособУнаследованный, НачалоПериода);
		Иначе
		...
		Если ЗначениеЗаполнено(СпособИндивидуальный) Тогда
			ПараметрыЦиклаПоставкиПоГрафикуРаботыИндивидуальные = ПараметрыЦиклаПоставкиПоГрафикуРаботы(
				Параметры.КлючПотребности, СпособИндивидуальный, НачалоПериода);
		Иначе
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(ДлительнаяОперация) Тогда
		...
		ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация,
			Новый ОписаниеОповещения("ПолучитьСтатистикуПотребленияВФонеЗавершение", ЭтотОбъект),
			ПараметрыОжидания);
	КонецЕсли;
...
Процедура Ок(Команда)
	...
	Если НЕ ЗначениеЗаполнено(МетодОбеспечения) ИЛИ
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,,"МетодОбеспечения");
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСтатистикуПотребленияНаСервере(Результат)
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, СтатистикаПотребления);
...
Функция ПараметрыЦиклаПоставкиПоГрафикуРаботы(КлючПотребности, СпособОбеспечения, НачалоПериода)
	
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц();
	Запрос = Новый Запрос();
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("НачалоТекущегоДня" , НачалоПериода);
	...
	ОбеспечениеВДокументахСервер.ВременнаяТаблицаБлижайшийЦиклПоСпособамОбеспечения(МенеджерВременныхТаблиц);
	
	ТекстыЗапросов = Новый Массив();
	
	ИспользоватьКонтрольКачества = ПолучитьФункциональнуюОпцию("ИспользоватьКонтрольКачестваТоваров")
		И ТипЗнч(КлючПотребности.Склад) = Тип("СправочникСсылка.Склады");
	...
	Если ИспользоватьКонтрольКачества Тогда
		...
		Текст =
			...
			Подстановки = ОбеспечениеВДокументахСервер.ДобавитьКДатеПоКалендарюПредприятия(
				...
				"ВЫБОР
				|	КОГДА ВЫРАЗИТЬ(&Номенклатура КАК Справочник.Номенклатура).КонтролироватьКачествоПоступившегоТовара
				|			И ВЫРАЗИТЬ(&Склад КАК Справочник.Склады).КонтролироватьКачествоПоступившегоТовара
				|		ТОГДА ВЫРАЗИТЬ(&Номенклатура КАК Справочник.Номенклатура).СрокКонтроляКачества
				|	ИНАЧЕ 0
				|КОНЕЦ");
	...
	Текст =
		...
		|			ИНАЧЕ ВЫРАЗИТЬ(&Склад КАК Справочник.Склады).Календарь
		|		КОНЕЦ КАК Календарь,
		...
		|ИЗ
		|	БлижайшийЦиклПоСпособамОбеспечения КАК Способ
	...
	Текст =
		...
		|	
		|	ТаблицаСпособовОбеспечения.ДатаПоставки = ДАТАВРЕМЯ(1, 1, 1) КАК ОшибкаУстановкиИнтервалаДоДатыПоставки,
		|	
		|	ТаблицаСпособовОбеспечения.ДатаПоставки > ДАТАВРЕМЯ(1, 1, 1)
		...
		|	ТаблицаСпособовОбеспечения.ДатаПоставки > ДАТАВРЕМЯ(1, 1, 1)
		|		И ТаблицаСпособовОбеспечения.ГраницаПериодаОбеспечения > ДАТАВРЕМЯ(1, 1, 1)
		|		И ИнтервалМеждуПоставками.КоличествоДней ЕСТЬ NULL КАК ОшибкаРасчетаИнтервалаМеждуПоставками,
		|	ТаблицаСпособовОбеспечения.ДатаПоставки = ДАТАВРЕМЯ(1, 1, 1)
		|			ИЛИ ТаблицаСпособовОбеспечения.ГраницаПериодаОбеспечения = ДАТАВРЕМЯ(1, 1, 1) КАК ОшибкаУстановкиИнтервалаМеждуПоставками
		|ИЗ
		|	БлижайшийЦиклПоСпособамОбеспечения КАК ТаблицаСпособовОбеспечения
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ Константа.ОсновнойКалендарьПредприятия КАК ОсновнойКалендарьПредприятия
		|		ПО ИСТИНА
		|		
	...
	Если Выборка.Следующий() Тогда
		ЗаполнитьЗначенияСвойств(Результат, Выборка, "СрокИсполненияЗаказа,ОбеспечиваемыйПериод");
		Результат.НеЗаполненКалендарьПредприятия = Выборка.ОшибкаУстановкиИнтервалаДоДатыПоставки
			Или Выборка.ОшибкаУстановкиИнтервалаМеждуПоставками;
		Результат.НеЗаполненКалендарьСклада = Выборка.ОшибкаРасчетаИнтервалаДоДатыПоставки
			Или Выборка.ОшибкаРасчетаИнтервалаМеждуПоставками;
...
//  см. СтруктураПараметровЦиклаПоставкиПоГрафикуРаботы
...
Функция СтруктураПараметровЦиклаПоставкиПоГрафикуРаботы()
	...
	СтруктураПараметров.Вставить("НеЗаполненКалендарьПредприятия", Ложь);
	СтруктураПараметров.Вставить("НеЗаполненКалендарьСклада", Ложь);
...
Функция ЕстьОшибкиЦиклаПоставки(ПараметрыЦиклаПоставки)
	...
	Возврат ПараметрыЦиклаПоставки.НеЗаполненКалендарьПредприятия
			Или ПараметрыЦиклаПоставки.НеЗаполненКалендарьСклада;
...
Функция ТекстОшибкиЦиклаПоставки(ПараметрыЦиклаПоставки)
	...
	Текст = "";
	Если ПараметрыЦиклаПоставки.НеЗаполненКалендарьПредприятия Тогда
		
		Текст = НСтр("ru = 'Ошибка расчета даты поставки и/или границы обеспечиваемого периода по способу обеспечения. Заполните календарь предприятия'");
		
	ИначеЕсли ПараметрыЦиклаПоставки.НеЗаполненКалендарьСклада Тогда
		
		Текст = НСтр("ru = 'Ошибка расчета интервала до даты поставки и / или между поставками. Заполните календарь работы склада'");
...
Функция ФормаЗаполненаВерно()
	...
	Если ПереключательСпособа = 1 И Не ЗначениеЗаполнено(СпособИндивидуальный) Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Укажите способ обеспечения'"),,, "СпособИндивидуальный",);
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;
...
Процедура РассчитатьЗапасы()
	...
	ОшибкаРасчетаЦиклПоставки   = НСтр("ru = 'ошибка расчета (нет данных графика работы)'");
	...
	Если МетодОбеспечения = ПредопределенноеЗначение("Перечисление.МетодыОбеспеченияПотребностей.ПоддержаниеЗапасаНаСрокПоНорме") Тогда
		...
		МаксимальныйЗапасСтрокой =
			...
			?(Сроки.ОбеспечиваемыйПериод = 0, СтрШаблон(
				НСтр("ru = '0.000 %1 (на 0 дн)'"),
				ЕдиницаИзмерения),
...
&НаСервереБезКонтекста
Функция ПараметрыЦиклаПоставкиПоГрафикуРаботыВызовСервера(КлючПотребности, СпособОбеспечения, НачалоПериода)
	Возврат ПараметрыЦиклаПоставкиПоГрафикуРаботы(КлючПотребности, СпособОбеспечения, НачалоПериода);
КонецФункции
