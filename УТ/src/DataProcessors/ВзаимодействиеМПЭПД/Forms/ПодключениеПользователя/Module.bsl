...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОформитьУведомлениеОбИспользованииЭПДляВнутреннегоЭДО(ЭтотОбъект);
	ОформитьВидПодписи(ЭтотОбъект, Неопределено);
...
Процедура ВидОбменаПриИзменении(Элемент)
	...
	ВыбранноеЗначение = Новый Структура("ПрямойОбмен", ПрямойОбмен);
	ПриПолучениДанныхОтФормыВыбораВидаОбмена(ВыбранноеЗначение);
...
&НаКлиенте
Процедура ОформитьВидПодписиНаКлиенте()

	ПредупредитьОВидеПодписиНеИспользуется = Ложь;
	ОформитьВидПодписи(ЭтотОбъект, ПредупредитьОВидеПодписиНеИспользуется);
	
	Если ПредупредитьОВидеПодписиНеИспользуется Тогда 
		ТекстПредупреждения = НСтр("ru='При оформлении электронного путевого листа водитель не участвует в подписании документа. Вид подписи изменен на ""Не используется"".'");
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОформитьВидПодписи(Форма, ПредупредитьОВидеПодписиНеИспользуется)
	
 	АтомарныеРолиМП = СервисВзаимодействияМПЭПДКлиентСервер.АтомарныеРолиМП();
	ИндексВодитель = АтомарныеРолиМП.Найти(НСтр("ru='Водитель'"));
	ВыбранаТолькоРольВодитель = Форма.Роль = Pow(2, ИндексВодитель);
	Элементы = Форма.Элементы;
		
	АтомарныеВидыДокументовМП = СервисВзаимодействияМПЭПДКлиентСервер.АтомарныеВидыДокументовМП();
	ИндексЭПЛ = АтомарныеВидыДокументовМП.Найти(НСтр("ru='Путевой лист'"));
	ВыбранТолькоЭПЛ = Форма.ВидыДокументов = Pow(2, ИндексЭПЛ); 
	
	// Вид подписи доступен во всех случая кроме случая, когда выбран ЭПЛ и роль Водитель.
	ВидПодписиДоступен = Не (ВыбранаТолькоРольВодитель И ВыбранТолькоЭПЛ);
	
	Если ВидПодписиДоступен Тогда
		Если Форма.ВидЭП = ПредопределенноеЗначение("Перечисление.ВидыПодписиМПЭПД.НеИспользуется") Тогда
			Форма.ВидЭП = ПредопределенноеЗначение("Перечисление.ВидыПодписиМПЭПД.ПростаяПодпись"); 
		КонецЕсли;
	Иначе
		Если Форма.ВидЭП <> ПредопределенноеЗначение("Перечисление.ВидыПодписиМПЭПД.НеИспользуется") Тогда 
			ПредупредитьОВидеПодписиНеИспользуется = Истина;
		КонецЕсли;
		Форма.ВидЭП = ПредопределенноеЗначение("Перечисление.ВидыПодписиМПЭПД.НеИспользуется");
	КонецЕсли;
	
	Элементы.ВидЭП.Доступность = ВидПодписиДоступен;
...
Процедура УстановитьВидимостьЛицензииНаМедосмотр(Форма)
	...
	ВыбранаРольМедосмотр = СервисВзаимодействияМПЭПДКлиентСервер.РольПользователяБезУчетаДокументовПоИндексуУстановлена(
		Форма.Роль, ИндексМедосмотра);
...
#Область УведомлениеОбИспользованииЭПДляВнутреннегоЭДО

&НаКлиентеНаСервереБезКонтекста
Процедура ОформитьУведомлениеОбИспользованииЭПДляВнутреннегоЭДО(Форма)
	
	ВидЭП = Форма.ВидЭП;
	ПрямойОбмен = Форма.ПрямойОбмен;
	Элементы = Форма.Элементы; 
	
	ПЭПИлиУНЭПДляВнутреннегоЭДО = 
	(ВидЭП = ПредопределенноеЗначение("Перечисление.ВидыПодписиМПЭПД.УНЭПГосключ")
		Или ВидЭП = ПредопределенноеЗначение("Перечисление.ВидыПодписиМПЭПД.ПростаяПодпись"))
	И Не ПрямойОбмен;
	
	Элементы.ФормаСформироватьУведомление.Видимость = ПЭПИлиУНЭПДляВнутреннегоЭДО;
	Элементы.ГруппаОписание3.Видимость = ПЭПИлиУНЭПДляВнутреннегоЭДО;
