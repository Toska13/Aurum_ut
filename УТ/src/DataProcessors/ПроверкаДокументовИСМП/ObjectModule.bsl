...
Функция РезультатПроверкиДокументаНаСоответствиеТребованиямГИСМТ(ПараметрыПроверки) Экспорт
	...
	Если Не ТипДокументаПоддерживается(Документ)
		И Не ИнтеграцияИСМП.ЭтоИсходящийЭлектронныйДокумент(Документ)  Тогда
...
Процедура РезультатПроверкиДокументаНаСоответствиеТребованиямГИСМТИзФоновогоЗадания(ПараметрыФоновогоЗадания, АдресРезультата) Экспорт
	...
	ВозвращаемоеЗначение = РезультатПроверкиДокументаНаСоответствиеТребованиямГИСМТ(ПараметрыПроверки);
	Если ВозвращаемоеЗначение.РезультатПроверки <> Неопределено
		И ВозвращаемоеЗначение.РезультатПроверки.Свойство("ДанныеИсходногоДокумента") Тогда
		ВозвращаемоеЗначение.РезультатПроверки.ДанныеИсходногоДокумента = Неопределено;
	КонецЕсли;
...
Функция ФорматЭдоПоТипуДокумента(ТипДокумента, ДатаДокумента, ПередачаПивнойПродукции)
	...
	Если СведенияОФорматахЭДО.Колонки.Найти("ТипЭлектронногоДокумента") = Неопределено Тогда
		// в кеше оказалась таблица, свернутая до одной колонки ИдентификаторФормата
...
Функция РезультатПроверкиНаСоответствиеТребованиямГИСМТ(ПараметрыПроверки)
	...
	Если РезультатПроверкиПоДокументу.Статус = Перечисления.СтатусыОбработкиДокументовИСМП.НеВыполнялось
		...
		ИначеЕсли (РезультатПроверкиПоДокументу.СтатусОтправки = Перечисления.СтатусОтправкиГИСМТ.НеОтправлен
			...
			Если ПредварительнаяПроверкаВозможна Тогда
				...
				Если (ПараметрыОбработки.ОтправленНаПредварительнуюПроверку
					...
					Если ЗапуститьПовторнуюПроверку Тогда
						СоответствиеТребованиямГИСМТ.ЗапроситьИОбработатьДанныеВходящегоЭлектронногоДокумента(
							ПараметрыОбработки, РезультатПроверкиПоДокументу);
					КонецЕсли;
				...
				Если ПараметрыУказанияДанныхПоМОДВЭлектронномДокументе.ПрисутствуетПрослеживаемаяПоМОДПродукция
					И Не ПараметрыПроверки.ЭтоДокументПриобретения Тогда
					
					ПроверятьТоварныеГруппыОрганизации = Ложь;
					ПроверятьТоварныеГруппыКонтрагента = Ложь;
					
					Для Каждого ПараметрыУказанияМОД Из ПараметрыУказанияДанныхПоМОДВЭлектронномДокументе.ПараметрыУказанияМОДПоВидамПродукции Цикл
						
						Если ПараметрыУказанияМОД.Значение.ПроверятьМОДОтправителяТолькоПоПодконтрольнымОбъектамВЕТИС Тогда
							ПроверятьТоварныеГруппыОрганизации = Истина;
						КонецЕсли;
						
						Если ПараметрыУказанияМОД.Значение.ПроверятьМОДПолучателяТолькоПоПодконтрольнымОбъектамВЕТИС Тогда
							ПроверятьТоварныеГруппыКонтрагента = Истина;
						КонецЕсли;
						
						Если ПроверятьТоварныеГруппыОрганизации И ПроверятьТоварныеГруппыКонтрагента Тогда
							Прервать;
						КонецЕсли;
						
					КонецЦикла;
					
					Если ПроверятьТоварныеГруппыОрганизации Тогда
						ТоварныеГруппыИНаличиеРегистрацииОрганизацииВГИСМТ(ПараметрыОбработки);
					КонецЕсли;
					
					Если ПроверятьТоварныеГруппыКонтрагента Тогда
		...
		РезультатПроверкиПоДокументу.Вставить("ДанныеИсходногоДокумента",           СуществующийРезультат.ДанныеИсходногоДокумента);
		РезультатПроверкиПоДокументу.Вставить("НоменклатураСоздана",                СуществующийРезультат.НоменклатураСоздана);
		...
		Если ПараметрыОбработки.ДанныеШтрихкодов.ВидыПродукцииВДокументе.Количество() > 0 Тогда
			...
			Для Каждого ВидПродукции Из ПараметрыОбработки.ДанныеШтрихкодов.ВидыПродукцииВДокументе Цикл
		...
		РезультатПроверкиПоДокументу.ДанныеИсходногоДокумента = Неопределено;
		РезультатОбработки.РезультатПроверки = РезультатПроверкиПоДокументу;
...
Функция ЗапроситьСтатусыКодовМаркировки(ПараметрыОбработки)
	...
	Для Каждого ДанныеШтрихкода Из СтрокиСРазобраннымиШтрихкодами Цикл
		...
		Если Не ЕстьПродукцияПроверкиПоМОД
			И КонструкторИСМПКлиентСервер.ДоступноПеремещениеМеждуМОД(ДанныеШтрихкода.ВидПродукции) Тогда
		...
		Если ДанныеСервиса.ВложенныеУпаковки <> Неопределено Тогда
			...
			Для Каждого ВложеннаяУпаковка Из ДанныеСервиса.ВложенныеУпаковки Цикл
				...
				Если КонструкторИСМПКлиентСервер.ДоступноПеремещениеМеждуМОД(ДанныеВложеннойУпаковки.ВидПродукции) Тогда
					ЕстьПродукцияПроверкиПоМОД = Истина;
				Иначе
					Продолжить;
				КонецЕсли;
...
Процедура НаличиеКорректногоСтатусаУКодовМаркировкиДоОтправкиВГИСМТ(ПараметрыОбработки, СтрокиСоСтатусами)
	...
	Для Каждого ДанныеШтрихкода Из СтрокиСоСтатусами Цикл
		...
		Если ДанныеСервиса.ИННВладельца = ПараметрыОбработки.РеквизитыДокумента.ОрганизацияИНН
			Или ШтрихкодированиеОбщегоНазначенияИСМП.ВладелецМожетОтличатьсяПоДатеПроизводства(ДанныеСервиса, ПараметрыОбработки.ПараметрыСканирования, Истина) Тогда
			
			Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(ДанныеСервиса.ВидПродукции) Тогда
			...
			Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(ДанныеСервиса.ВидПродукции) Тогда
				...
				Если ДанныеСервиса.Статус = Перечисления.СтатусыКодовМаркировкиИСМП.Нанесен Тогда
...
Функция ТоварныеГруппыИзДанныхСервиса(ВидыПродукцииВДокументе, ТоварныеГруппыПоДаннымСервиса)
	...
	Для Каждого ВидПродукцииУточнение Из ВидыПродукцииВДокументе Цикл
		...
		Для Каждого ТоварнаяГруппа Из ТоварныеГруппыПоДаннымСервиса Цикл
			...
			ДанныеДляУточненияТоварнойГруппы.ВидПродукции = ВидПродукцииУточнение;
			ТоварнаяГруппаСсылка = ИнтерфейсИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппа(ТоварнаяГруппа, ДанныеДляУточненияТоварнойГруппы);
...
Функция ТоварныеГруппыИНаличиеРегистрацииОрганизацииВГИСМТ(ПараметрыОбработки)
	...
	РезультатЗапроса = ЗапроситьСтатусРегистрацииУчастникаИТоварныеГруппы(ПараметрыОбработки.Организация,
		ПараметрыОбработки.РеквизитыДокумента.ОрганизацияИНН);
	...
	Иначе
		...
		ПараметрыОбработки.Вставить("ТоварныеГруппыОрганизацииПодконтрольностьВЕТИС", РезультатЗапроса.ПодконтрольностьВЕТИС);
...
Функция ТоварныеГруппыИНаличиеРегистрацииКонтрагентаВГИСМТ(ПараметрыОбработки)
	...
	Иначе
		...
		ПараметрыОбработки.Вставить("ТоварныеГруппыКонтрагентаПодконтрольностьВЕТИС", РезультатЗапроса.ПодконтрольностьВЕТИС);
...
Процедура ЗаполнитьРезультатПроверкиФорматаЭД(ПараметрыОбработки, ФорматЭД, РезультатПроверки)
	...
	Если ФорматыЭдоПоТипуДокумента = Неопределено Тогда
		
		РезультатПроверки.ЕстьОшибки  = Истина;
		РезультатПроверки.ТекстОшибки = НСтр("ru = 'Настройки обмена ЭД получить не удалось, попробуйте повторно'");
		
	ИначеЕсли ФорматыЭдоПоТипуДокумента[ФорматЭД] = Неопределено Тогда
...
Функция НаличиеОшибокВКодахМаркировки(ПараметрыОбработки)
	...
	Для Каждого ДанныеШтрихкода Из СтрокиСРазобраннымиШтрихкодами Цикл
		...
		Если КонструкторИСМПКлиентСервер.ПоддерживаетсяСераяЗона(ДанныеШтрихкода.ВидПродукции)
		...
		Если ЗначениеЗаполнено(ДанныеШтрихкода.ВидПродукции) Тогда
			Если ТипЗнч(ДанныеШтрихкода.ВидПродукции) = Тип("СправочникСсылка.КлючиВидовПродукцииИС") Тогда
				ВидПродукцииСтроки = ВидыПродукцииГИСМТПоКлючам[ДанныеШтрихкода.ВидПродукции];
				Если ВидПродукцииСтроки = Неопределено Тогда
					ВидПродукцииСтроки = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииПоПодсистеме(ДанныеШтрихкода.ВидПродукции, "ГИСМТ");
					ВидыПродукцииГИСМТПоКлючам[ДанныеШтрихкода.ВидПродукции] = ВидПродукцииСтроки;
				КонецЕсли;
			Иначе
				ВидПродукцииСтроки = ДанныеШтрихкода.ВидПродукции;
			КонецЕсли;
			Если ЗначениеЗаполнено(ВидПродукцииСтроки)
				И УникальныеВидыПродукции[ВидПродукцииСтроки] = Неопределено Тогда
				УникальныеВидыПродукции[ВидПродукцииСтроки] = Истина;
				ВидыПродукцииВДокументе.Добавить(ВидПродукцииСтроки);
			КонецЕсли;
		КонецЕсли;
...
Функция НаличиеЗарегистрированногоМОДаОрганизации(ПараметрыОбработки, ПараметрыУказанияДанныхПоМОДВЭлектронномДокументе)
	...
	ПараметрыУказанияМОД = ЭлектронноеВзаимодействиеИСМП.КонструкторПараметрыУказанияМОДПоВидуПродукции();
	...
	Для Каждого ДанныеПоВидуПродукции Из ПараметрыУказанияДанныхПоМОДВЭлектронномДокументе.ПараметрыУказанияМОДПоВидамПродукции Цикл
		...
		Если ДанныеПоВидуПродукции.Значение.ПроверятьМОДОтправителяТолькоПоПодконтрольнымОбъектамВЕТИС И ПараметрыОбработки.Свойство("ТоварныеГруппыОрганизацииПодконтрольностьВЕТИС") Тогда
			
			ПодконтрольностьВЕТИС = ПараметрыОбработки.ТоварныеГруппыОрганизацииПодконтрольностьВЕТИС.Получить(ИнтерфейсИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппа(ДанныеПоВидуПродукции.Ключ));
			
			Если ПодконтрольностьВЕТИС = Неопределено Тогда
				ПодконтрольностьВЕТИС = Ложь;
			КонецЕсли;
			
			ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителя                               = ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителя Или ПодконтрольностьВЕТИС И ДанныеПоВидуПродукции.Значение.ТребуетсяУказаниеКодаФИАСПоМОДОтправителя;
			ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяИндивидуальныхПредпринимателей = ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяИндивидуальныхПредпринимателей Или ПодконтрольностьВЕТИС И ДанныеПоВидуПродукции.Значение.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяИндивидуальныхПредпринимателей;
			ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяЮридическихЛиц                 = ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяЮридическихЛиц Или ПодконтрольностьВЕТИС И ДанныеПоВидуПродукции.Значение.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяЮридическихЛиц;
	...
	Если Не ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителя Тогда
		Возврат РезультатПроверки;
	КонецЕсли;
	...
	ТребуетсяУказаниеКодаФИАС = ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяЮридическихЛиц И Не РеквизитыДокумента.ОтправительИндивидуальныйПредприниматель
		Или ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяИндивидуальныхПредпринимателей И РеквизитыДокумента.ОтправительИндивидуальныйПредприниматель;
...
Функция НаличиеЗарегистрированногоМОДаКонтрагента(ПараметрыОбработки, ПараметрыУказанияДанныхПоМОДВЭлектронномДокументе)
	...
	Для Каждого ДанныеПоВидуПродукции Из ПараметрыУказанияДанныхПоМОДВЭлектронномДокументе.ПараметрыУказанияМОДПоВидамПродукции Цикл
		...
		Если ДанныеПоВидуПродукции.Значение.ПроверятьМОДПолучателяТолькоПоПодконтрольнымОбъектамВЕТИС И ПараметрыОбработки.Свойство("ТоварныеГруппыКонтрагентаПодконтрольностьВЕТИС") Тогда
			
			ПодконтрольностьВЕТИС = ПараметрыОбработки.ТоварныеГруппыКонтрагентаПодконтрольностьВЕТИС.Получить(ИнтерфейсИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппа(ДанныеПоВидуПродукции.Ключ));
			
			Если ПодконтрольностьВЕТИС = Неопределено Тогда
				ПодконтрольностьВЕТИС = Ложь;
			КонецЕсли;
			
			ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателя                               = ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателя Или ПодконтрольностьВЕТИС И ДанныеПоВидуПродукции.Значение.ТребуетсяУказаниеКодаФИАСПоМОДПолучателя;
			ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяИндивидуальныхПредпринимателей = ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяИндивидуальныхПредпринимателей Или ПодконтрольностьВЕТИС И ДанныеПоВидуПродукции.Значение.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяИндивидуальныхПредпринимателей;
			ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяЮридическихЛиц                 = ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяЮридическихЛиц Или ПодконтрольностьВЕТИС И ДанныеПоВидуПродукции.Значение.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяЮридическихЛиц;
	...
	Если Не ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателя Тогда
		Возврат РезультатПроверки;
	КонецЕсли;
	...
	ТребуетсяУказаниеКодаФИАС = ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяЮридическихЛиц И Не РеквизитыДокумента.ПолучательИндивидуальныйПредприниматель
		Или ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяИндивидуальныхПредпринимателей И РеквизитыДокумента.ПолучательИндивидуальныйПредприниматель;
...
Процедура ЗаполнитьДанныеАдресовДляПроверкиМОД(ПараметрыОбработки, ПараметрыУказанияДанныхПоМОДВЭлектронномДокументе)
	...
	ЗаполнитьДанныеАдресаОтправителяПоПараметрамУказанияДанныхПоМОД(ПараметрыУказанияМОД, ПараметрыОбработки);
	ЗаполнитьДанныеАдресаПолучателяПоПараметрамУказанияДанныхПоМОД(ПараметрыУказанияМОД, ПараметрыОбработки);
	
КонецПроцедуры

Процедура ЗаполнитьДанныеАдресаОтправителяПоПараметрамУказанияДанныхПоМОД(ПараметрыУказанияМОД, ПараметрыОбработки)
	...
	ТребуетсяУказаниеФИАС = РеквизитыДокумента.ОтправительИндивидуальныйПредприниматель И ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяИндивидуальныхПредпринимателей
		Или Не РеквизитыДокумента.ОтправительИндивидуальныйПредприниматель И ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяЮридическихЛиц;
...
Процедура ЗаполнитьДанныеАдресаПолучателяПоПараметрамУказанияДанныхПоМОД(ПараметрыУказанияМОД, ПараметрыОбработки)
	...
	ТребуетсяУказаниеФИАС = РеквизитыДокумента.ПолучательИндивидуальныйПредприниматель И ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяИндивидуальныхПредпринимателей
		Или Не РеквизитыДокумента.ПолучательИндивидуальныйПредприниматель И ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяЮридическихЛиц;
...
Функция ЗапроситьСтатусРегистрацииУчастникаИТоварныеГруппы(Организация, ИНН)
	...
	Возврат ИнтерфейсИСМПОбщегоНазначения.ЗапроситьСтатусРегистрацииУчастникаИТоварныеГруппы(Организация, ИНН);
...
Функция СтатусОбработкиУниверсальногоДокументаПоИдентификатору(Организация, ИдентификаторЭлектронногоДокумента) Экспорт
	...
	Возврат ИнтерфейсИСМП.СтатусОбработкиУниверсальногоДокументаПоИдентификатору(Организация, ИдентификаторЭлектронногоДокумента);
