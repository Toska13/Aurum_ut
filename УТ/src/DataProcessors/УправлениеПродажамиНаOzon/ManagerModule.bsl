	...
	ИначеЕсли Параметры.ИмяТабличнойЧасти = "ДанныеОРеализованныхТоварах" Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Параметры.ДополнительныеПараметры, "ДанныеТорговойПлощадки") Тогда
			...
			ИспользуетсяПозаказныйОтчетОРеализации = ИнтеграцияСМаркетплейсамиКлиентСервер.ИспользуетсяПозаказныйОтчетОРеализации(
				ДанныеТорговойПлощадки.СпособОтраженияПродажFBO,
				ДанныеТорговойПлощадки.СпособОтраженияПродажFBS,
				ДанныеТорговойПлощадки.ПродажиРазделяютсяПоДоговорам);
			...
			Если ИспользуетсяПозаказныйОтчетОРеализации Тогда
				...
				ЗаполняемыеПриЗагрузкеКолонки.Вставить("НомерЗаказа", НСтр("ru = 'Номер заказа'"));
				ТипыДанныхКолонок.Вставить("НомерЗаказа", ТипСтрока_50);
			КонецЕсли;
...
Функция ПолучитьДанныеИнтеграционнымиМетодами(ТаблицаДляЗагрузкиДанных, Параметры) Экспорт
	...
	Если Параметры.ИмяТабличнойЧасти = "ДанныеОРеализованныхТоварах" Тогда
		Результат = ПолучитьДанныеОРеализованныхТоварах(ТаблицаДляЗагрузкиДанных, Параметры, ПараметрыПрогресса);
	Иначе
...
Функция СопоставитьЗагружаемыеДанные(ТаблицаСопоставленияДанных, ДанныеИзФайла, Знач Параметры) Экспорт
	...
	ПараметрыПрогресса.ПрогрессВыполнения    = 50; // Часть прогресса выполнена (сопоставление данных).
...
Функция ПолучитьДанныеОРеализованныхТоварах(ТаблицаДляЗагрузкиДанных, Параметры, ПараметрыПрогресса)
	...
	ВсегоСтрок = ТаблицаОборотовТоваров.Количество();
	...
	ПараметрыПрогресса.ШагИтерацииВПроцентах = 20;
	ИнтеграцияСМаркетплейсамиСервер.УстановитьПроцентПрогресса(1, 1, 0, ПараметрыПрогресса);
...
Процедура СопоставитьДанныеОРеализованныхТоварах(ТаблицаСопоставленияДанных, ДанныеИзФайла, СписокНеоднозначностей,
	...
	Запрос.Текст =
		...
		|	&СхемаДоставки КАК СхемаДоставки,
	...
	УчитыватьДанныеОтправлений = ИнтеграцияСМаркетплейсамиКлиентСервер.ИспользуетсяПозаказныйОтчетОРеализации(
	...
	Если УчитыватьДанныеОтправлений Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&НомерЗаказа", "ЗагружаемыеДанные.НомерЗаказа");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&СхемаДоставки", "ЗагружаемыеДанные.СхемаДоставки");
		Запрос.Выполнить();
		
		ПараметрыПрогресса.ШагИтерацииВПроцентах = Окр(ШагИтерацииВПроцентах / 5, 0) * 3;
		
		ДополнитьДанныеОРеализованныхТоварахДаннымиОтправлений(Запрос, ПараметрыПрогресса);
		
		ПараметрыПрогресса.ПрогрессВыполнения = ПараметрыПрогресса.ПрогрессВыполнения
			+ ПараметрыПрогресса.ШагИтерацииВПроцентах; // Часть прогресса выполнена.
		
		УчитыватьДанныеОтправлений =
			(Запрос.МенеджерВременныхТаблиц.Таблицы.Найти("ВТ_ДанныеОтправлений") <> Неопределено);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&НомерЗаказа", """""");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&СхемаДоставки", "ЗНАЧЕНИЕ(Перечисление.СхемыРаботыТорговыхПлощадок.ПустаяСсылка)");
		Запрос.Выполнить();
	КонецЕсли;
	...
	Шаблон =
		...
		|	ЗагружаемыеДанные.ДатаСчетаФактуры КАК ДатаСчетаФактуры,
		|	&СхемаДоставки КАК СхемаДоставки,
		|	&ДатаПередачиВДоставку КАК ДатаПередачиВДоставку,
		|	&ДатаДоставки КАК ДатаДоставки,
		|	&ИдентификаторСклада КАК ИдентификаторСклада,
		|	&НаименованиеСклада КАК НаименованиеСклада,
		|	&СкладОтгрузки КАК СкладОтгрузки,
		|	&СкладОтгрузкиЭтоГруппа КАК СкладОтгрузкиЭтоГруппа,
	...
	Если УчитыватьДанныеОтправлений Тогда
		Шаблон = Шаблон + "
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ДанныеОтправлений КАК ДанныеОтправлений
			|		ПО ЗагружаемыеДанные.НомерДокумента = ДанныеОтправлений.НомерОтправления
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК СправочникСклады
			|		ПО ДанныеОтправлений.СкладОтгрузки = СправочникСклады.Ссылка";
		
		ПолеСхемаДоставки = "ЕСТЬNULL(ДанныеОтправлений.СхемаДоставки, ЗагружаемыеДанные.СхемаДоставки)";
		ПолеДатаПередачиВДоставку = "ЕСТЬNULL(ДанныеОтправлений.ДатаПередачиВДоставку, ДАТАВРЕМЯ(1, 1, 1))";
		ПолеДатаДоставки = "ЕСТЬNULL(ДанныеОтправлений.ДатаДоставки, ДАТАВРЕМЯ(1, 1, 1))";
		ПолеНаименованиеСклада = "ЕСТЬNULL(ДанныеОтправлений.НаименованиеСклада, """")";
		ПолеИдентификаторСклада = "ЕСТЬNULL(ДанныеОтправлений.ИдентификаторСклада, """")";
		ПолеСкладОтгрузкиЭтоГруппа = "ЕСТЬNULL(СправочникСклады.ЭтоГруппа, ЛОЖЬ)";
		ПолеСкладОтгрузки = "ЕСТЬNULL(ДанныеОтправлений.СкладОтгрузки, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка))";
	Иначе
		ПолеСхемаДоставки = "ЗагружаемыеДанные.СхемаДоставки";
		ПолеДатаПередачиВДоставку = "ДАТАВРЕМЯ(1, 1, 1)";
		ПолеДатаДоставки = "ДАТАВРЕМЯ(1, 1, 1)";
		ПолеНаименованиеСклада = """""";
		ПолеИдентификаторСклада = """""";
		ПолеСкладОтгрузкиЭтоГруппа = "ЛОЖЬ";
		ПолеСкладОтгрузки = "ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)";
	КонецЕсли;
	
	Шаблон = СтрЗаменить(Шаблон, "&СхемаДоставки", ПолеСхемаДоставки);
	Шаблон = СтрЗаменить(Шаблон, "&ДатаПередачиВДоставку", ПолеДатаПередачиВДоставку);
	Шаблон = СтрЗаменить(Шаблон, "&ДатаДоставки", ПолеДатаДоставки);
	Шаблон = СтрЗаменить(Шаблон, "&НаименованиеСклада", ПолеНаименованиеСклада);
	Шаблон = СтрЗаменить(Шаблон, "&ИдентификаторСклада", ПолеИдентификаторСклада);
	Шаблон = СтрЗаменить(Шаблон, "&СкладОтгрузкиЭтоГруппа", ПолеСкладОтгрузкиЭтоГруппа);
	Шаблон = СтрЗаменить(Шаблон, "&СкладОтгрузки", ПолеСкладОтгрузки);
	...
	Пока ВыборкаИдентификаторов.Следующий() Цикл
		...
		Если ВыборкаДанных.Следующий() Тогда
			...
			Если Не ЗначениеЗаполнено(ВыборкаДанных.СхемаДоставки)
					И УчитыватьДанныеОтправлений Тогда
			...
			Если (ВыборкаДанных.СхемаДоставки <> Перечисления.СхемыРаботыТорговыхПлощадок.FBO
						Или ЗначениеЗаполнено(НоваяСтрока.СкладОтгрузки))
					И (ЗначениеЗаполнено(ВыборкаДанных.НаименованиеСклада)
						Или ЗначениеЗаполнено(ВыборкаДанных.ИдентификаторСклада)) Тогда
				НоваяСтрока.ФЛ_СкладОтгрузки =
					?(ЗначениеЗаполнено(ВыборкаДанных.НаименованиеСклада), СокрЛП(ВыборкаДанных.НаименованиеСклада), НСтр("ru = 'Склад Ozon'"))
			...
			ИначеЕсли НоваяСтрока.СхемаДоставки = Перечисления.СхемыРаботыТорговыхПлощадок.FBS Тогда
				...
				ИначеЕсли ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоСпособОтраженияКомиссия(НастройкиУчетнойЗаписи.СпособОтраженияПродажFBS) Тогда
					НоваяСтрока.ДоговорДляОтраженияПродажи = НастройкиУчетнойЗаписи.ДоговорПродажиЧерезСкладыСобственные;
				ИначеЕсли ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоСпособОтраженияРеализацияВПути(НастройкиУчетнойЗаписи.СпособОтраженияПродажFBS) Тогда
					НоваяСтрока.ДоговорДляОтраженияПродажи = НастройкиУчетнойЗаписи.ПокупательДоговор;
				КонецЕсли;
			КонецЕсли;
...
Процедура ДополнитьДанныеОРеализованныхТоварахДаннымиОтправлений(Запрос, ПараметрыПрогресса)
	
	УчетнаяЗапись = Запрос.Параметры.УчетнаяЗапись;
	
	ДоступныеРоли = ИнтеграцияСМаркетплейсомOzonСервер.СписокДоступныхРолейУчетнойЗаписи(УчетнаяЗапись);
	
	НомераОтправлений = Новый Массив;
	НомераЗаказов = Новый Массив;
	
	ПериодСозданияЗаказов = Новый СтандартныйПериод;
	
	Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВходящиеДанные.НомерДокумента КАК НомерОтправления,
		|	ВходящиеДанные.ДатаДокумента КАК ДатаОтправления,
		|	ЕСТЬNULL(ЗаказыТорговыхПлощадок.НомерЗаказа, ВходящиеДанные.НомерЗаказа) КАК НомерЗаказа
		|ИЗ
		|	ЗагружаемыеДанные КАК ВходящиеДанные
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗаказыТорговыхПлощадок КАК ЗаказыТорговыхПлощадок
		|		ПО ВходящиеДанные.НомерДокумента = ЗаказыТорговыхПлощадок.НомерОтправления
		|ГДЕ
		|	ВходящиеДанные.НомерДокумента <> """"
		|	И ВходящиеДанные.СхемаДоставки = ЗНАЧЕНИЕ(Перечисление.СхемыРаботыТорговыхПлощадок.ПустаяСсылка)";
	
	ВыборкаНомеровОтправлений = Запрос.Выполнить().Выбрать();
	Пока ВыборкаНомеровОтправлений.Следующий() Цикл
		НомераОтправлений.Добавить(ВыборкаНомеровОтправлений.НомерОтправления);
		
		Если ПустаяСтрока(ВыборкаНомеровОтправлений.НомерЗаказа) Тогда
			// Для незагруженных отправлений номер заказа формируем из номера отправления.
			ЧастиНомераОтправления = СтрРазделить(ВыборкаНомеровОтправлений.НомерОтправления, "-");
			ЧастиНомераОтправления.Удалить(ЧастиНомераОтправления.Количество() - 1);
			
			НомераЗаказов.Добавить(СтрСоединить(ЧастиНомераОтправления, "-"));
		Иначе
			НомераЗаказов.Добавить(ВыборкаНомеровОтправлений.НомерЗаказа);
		КонецЕсли;
		
		ДеньОтправленияНачало = НачалоДня(ВыборкаНомеровОтправлений.ДатаОтправления);
		ДеньОтправленияОкончание = КонецДня(ВыборкаНомеровОтправлений.ДатаОтправления);
		
		Если Не ЗначениеЗаполнено(ПериодСозданияЗаказов.ДатаНачала) Тогда
			ПериодСозданияЗаказов.ДатаНачала = ДеньОтправленияНачало;
			ПериодСозданияЗаказов.ДатаОкончания = ДеньОтправленияОкончание;
		Иначе
			ПериодСозданияЗаказов.ДатаНачала =
				Мин(ПериодСозданияЗаказов.ДатаНачала, ДеньОтправленияНачало);
			ПериодСозданияЗаказов.ДатаОкончания =
				Макс(ПериодСозданияЗаказов.ДатаОкончания, ДеньОтправленияОкончание);
		КонецЕсли;
	КонецЦикла;
	
	Если ДоступныеРоли.ДоступноЧтениеИнформацииПоЗаказам Тогда
		НомераЗаказов = ОбщегоНазначенияКлиентСервер.СвернутьМассив(НомераЗаказов);
		
		ДанныеОтправлений = ИнтеграцияСМаркетплейсомOzonСервер.ДанныеПоСпискуОтправлений(
			УчетнаяЗапись,
			ПериодСозданияЗаказов,
			НомераЗаказов,
			НомераОтправлений,
			ПараметрыПрогресса);
	ИначеЕсли ДоступныеРоли.ДоступноЧтениеСпискаТранзакций Тогда
		ДанныеОтправлений = ИнтеграцияСМаркетплейсомOzonСервер.ДанныеПоСпискуОтправленийИзТранзакций(
			УчетнаяЗапись,
			НомераОтправлений,
			ПараметрыПрогресса);
	Иначе
	...
	Если ДанныеОтправлений.Количество() > 0 Тогда
		Запрос.Текст =
			"ВЫБРАТЬ
			|	ДанныеОтправлений.НомерОтправления КАК НомерОтправления,
			|	ДанныеОтправлений.ДатаПередачиВДоставку КАК ДатаПередачиВДоставку,
			|	ДанныеОтправлений.ДатаДоставки КАК ДатаДоставки,
			|	ДанныеОтправлений.СхемаДоставки КАК СхемаДоставки,
			|	ВЫРАЗИТЬ(ДанныеОтправлений.НаименованиеСклада КАК СТРОКА(250)) КАК НаименованиеСклада,
			|	ДанныеОтправлений.ИдентификаторСклада КАК ИдентификаторСклада
			|ПОМЕСТИТЬ ВТ_ВходящиеДанные
			|ИЗ
			|	&ДанныеОтправлений КАК ДанныеОтправлений
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	ИдентификаторСклада
			|;
			...
			|	ДанныеОтправлений.ДатаДоставки КАК ДатаДоставки,
			|	ДанныеОтправлений.СхемаДоставки КАК СхемаДоставки,
			|	ДанныеОтправлений.НаименованиеСклада КАК НаименованиеСклада,
			|	ДанныеОтправлений.ИдентификаторСклада КАК ИдентификаторСклада,
			...
			|ИЗ
			|	ВТ_ВходящиеДанные КАК ДанныеОтправлений
			...
			|			И ДанныеОтправлений.ИдентификаторСклада = СоответствияОбъектовМаркетплейсов.ИдентификаторОбъектаМаркетплейса
			...
			|
			|////////////////////////////////////////////////////////////////////////////////
			|УНИЧТОЖИТЬ ВТ_ВходящиеДанные";
		Запрос.УстановитьПараметр("ДанныеОтправлений", ДанныеОтправлений);
		Запрос.Выполнить();
	КонецЕсли;
...
Функция ЗаписатьДанныеПоставок(ТаблицаСопоставленияДанных, Параметры, СсылкаДляОбновленияДанных,
	...
	ДанныеЗаполнения.Вставить("ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию);
	ДанныеЗаполнения.Вставить("СпособДоставки",        Перечисления.СпособыДоставки.Самовывоз);
...
Функция ЗаполнитьДанныеПоТоварамВДокументыПоставки(ЗаказКлиентаОбъект, ПодчиненныеДокументы,
	...
	Для Каждого СтрокаТовара Из ТаблицаСопоставленияДанных Цикл
		...
		Если Не СтрокаДокумента.ПерезаполненаШапка Тогда
			...
			ДанныеЗаполнения = ПередачаТоваровХранителюДанныеЗаполнения(
				НастройкиУчетнойЗаписи,
				ЗначенияОсновныхНастроек,
				ЗаказКлиентаОбъект);
			СтрокаДокумента.ДокументОбъект.Заполнить(ДанныеЗаполнения);
...
//  ЗначенияОсновныхНастроек - Структура:
//   * ИспользоватьСоглашенияСКлиентами - Булево
//  ЗаказКлиента - ДокументОбъект.ЗаказКлиента
// 
// Возвращаемое значение:
//  - СправочникСсылка.СоглашенияСКлиентами - если в информационной базе используются соглашения с клиентами.
//  - Структура - если в информационной базе не используются соглашения с клиентами:
//     * РеквизитыШапки - Структура:
//        ** Партнер - СправочникСсылка.Партнеры
//        ** ХозяйственнаяОперация - ПеречислениеСсылка.ХозяйственныеОперации
//
Функция ПередачаТоваровХранителюДанныеЗаполнения(НастройкиУчетнойЗаписи, ЗначенияОсновныхНастроек, ЗаказКлиента)
	
	Результат = Неопределено; // Неопределено, Структура, СправочникСсылка.СоглашенияСКлиентами
	
	Если ЗначенияОсновныхНастроек.ИспользоватьСоглашенияСКлиентами
			И ЗначениеЗаполнено(НастройкиУчетнойЗаписи.Соглашение) Тогда
		
		Результат = НастройкиУчетнойЗаписи.Соглашение;
	Иначе
		РеквизитыШапки = Новый Структура;
		РеквизитыШапки.Вставить("Партнер", ЗаказКлиента.Партнер);
		РеквизитыШапки.Вставить("ХозяйственнаяОперация", ЗаказКлиента.ХозяйственнаяОперация);
	...
	Если Форма.ИмяТабличнойЧасти = "ДанныеПоставок"
	  ...
	  Или Форма.ИмяТабличнойЧасти = "ДанныеОРеализованныхТоварах" Тогда
		...
		Если (ИмяПоля = "Номенклатура" Или ИмяПоля = "Характеристика" Или ИмяПоля = "Упаковка") Тогда
			...
			Отбор = Новый Структура("ИмяКолонки", "НоменклатураПартнера");
			РезультатПоиска = Форма.ИнформацияПоКолонкам.НайтиСтроки(Отбор);
			Если РезультатПоиска.Количество() > 0 Тогда
				Запрос.УстановитьПараметр("НоменклатураПартнера", ТекущиеДанные.НоменклатураПартнера);
				Запрос.Текст = Запрос.Текст + "
					|	И СтатусыПубликации.НоменклатураПартнера = &НоменклатураПартнера";
			КонецЕсли;
