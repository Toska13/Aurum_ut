...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДанныеТорговойПлощадки Тогда
		Если Параметры.ПланПоСкладамSellmonitor <> Неопределено Тогда
			ЗаполнитьДанныеПоПланированиюSellmonitor();
		Иначе
...
Процедура Запланировать(Команда)
	...
	Результат.Вставить("НоменклатураПартнера",             НоменклатураПартнера);
...
Процедура ЗаполнитьДанныеПоПланированию()
	...
	Запрос.Текст =
		...
		|	ВЫРАЗИТЬ(&НоменклатураПартнера КАК Справочник.НоменклатураКонтрагентов) КАК НоменклатураПартнера,
	...
	Запрос.УстановитьПараметр("НоменклатураПартнера",    Параметры.НоменклатураПартнера);
...
&НаСервере
Процедура ЗаполнитьДанныеПоПланированиюSellmonitor()
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СкладыSellmonitor.ИдентификаторОбъектаМаркетплейса КАК ИдентификаторОбъектаМаркетплейса,
	|	СкладыSellmonitor.НаименованиеОбъектаМаркетплейса КАК НаименованиеОбъектаМаркетплейса
	|ИЗ
	...
	|	И СкладыSellmonitor.ИдентификаторОбъектаМаркетплейса В(&ИдентификаторыСкладов)";
	
	Запрос.УстановитьПараметр("УчетнаяЗаписьSellmonitor", ИнтеграцияСSellmonitorСервер.ОсновнаяУчетнаяЗапись());
	Запрос.УстановитьПараметр("ИдентификаторУчетнойЗаписиТП", XMLСтрока(УчетнаяЗаписьТорговойПлощадки));
	
	ИдентификаторыСкладов = Новый Массив;
	Для Каждого СтрокаПлана Из Параметры.ПланПоСкладамSellmonitor Цикл
		ИдентификаторыСкладов.Добавить(СтрокаПлана.Ключ);
	КонецЦикла;
	...
	ПредставленияСкладов = Новый Соответствие;
	Пока Выборка.Следующий() Цикл
		ПредставленияСкладов.Вставить(Выборка.ИдентификаторОбъектаМаркетплейса, Выборка.НаименованиеОбъектаМаркетплейса);
	КонецЦикла;
	...
	Для Каждого СтрокаПлана Из Параметры.ПланПоСкладамSellmonitor Цикл
		...
		СтрокаТаблицыЗначений                                    = ДанныеТорговойПлощадки.Добавить();
		СтрокаТаблицыЗначений.НаименованиеСкладаТорговойПлощадки = ПредставленияСкладов.Получить(СтрокаПлана.Ключ);
		СтрокаТаблицыЗначений.Упаковка                           = СтрокаПлана.Значение.ВНаличии;
		СтрокаТаблицыЗначений.ВНаличии                           = СтрокаПлана.Значение.ВНаличии;
		
		Если Параметры.АнализироватьОтгрузки Тогда
			
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыЗначений, СтрокаПлана.Значение);
