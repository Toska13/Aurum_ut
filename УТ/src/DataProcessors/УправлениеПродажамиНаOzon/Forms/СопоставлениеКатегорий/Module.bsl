...
Процедура ПеренестиАтрибутыИЗначенияИзУстаревшейКатегории(УчетнаяЗаписьМаркетплейса, Категория1С, 
	...
	ИдентификаторВладельцаОбъекта = ИнтеграцияСМаркетплейсомOzonСервер.ИдентификаторВладельцаОбъектаМаркетплейса(
		ИдентификаторКатегории1С, УстаревшийИдентификаторКатегорииМаркетплейса, "");
	
	НовыйИдентификаторВладельцаОбъекта = ИнтеграцияСМаркетплейсомOzonСервер.ИдентификаторВладельцаОбъектаМаркетплейса(
			...
			Пока ВыборкаДанных.Следующий() Цикл
				...
				Если Запись.Выбран() Тогда
					Запись.ИдентификаторВладельцаОбъектаМаркетплейса = СтрЗаменить(
						...
						НовыйИдентификаторВладельцаОбъекта);
...
Процедура ЗаписьВРегистрСоответствияАтрибутов(ДанныеСтроки, ЭтоУдалениеЗаписи = Ложь)
	...
	Если ЭтоУдалениеЗаписи Тогда
		// Для реквизитов по умолчанию нужно не удалить запись, а записать пустую
		ПутьКРеквизитамПоУмолчанию = ИнтеграцияСМаркетплейсомOzonСервер.ПутьКРеквизитам1СПоУмолчанию();
		...
		Если ПутьКРеквизитамПоУмолчанию.Свойство(КлючАтрибута) Тогда
			ЭтоУдалениеЗаписи = Ложь;
		КонецЕсли;
	КонецЕсли;
