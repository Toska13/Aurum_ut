...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.СтраницыПросмотра.Видимость = Предпросмотр;
...
Асинх Процедура ФайлыПеретаскиваниеЗавершение(Знач Результат, Знач ПараметрыОповещения) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		// Владелец файла на стороне ДО мог быть создан непосредственно в момент перетаскивания.
		// В таком случае данные созданного объекта ДО будут содержаться в структуре "Результат".
		ID = Результат.ID;
		Тип = Результат.type;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ID) Тогда
...
Процедура ВосстановитьПоложениеВСписке()
	...
	Если Не ТекущаяСтрокаУстановлена Тогда
		...
		Если СтрокиКРаскрытию.Количество() > 0 Тогда
			СтрокаКРаскрытию = ИнтеграцияС1СДокументооборот3Клиент.СтрокаДереваФайловПоID(
				ДеревоФайлов,
				ИспользоватьРолиФайлов,
				СтрокиКРаскрытию[0]);
		КонецЕсли;
...
Процедура ПоказатьФайлыИСПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Значение = КодВозвратаДиалога.Да Тогда
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ОткрытьФормуСпискаФайловБСП(Владелец);
	КонецЕсли;
...
Функция ПрочитатьИОбновитьСписокФайлов(РезультатДлительнойОперации)
	...
	Для Каждого ФайлXDTO Из СписокФайлов Цикл
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ФайлXDTO, "htmlView")
			...
			HTMLПредпросмотрОбъекта =
				ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ПараметрыОповещенияПолученHTMLПредпросмотрОбъекта();
			HTMLПредпросмотрОбъекта.ID = ФайлXDTO.ObjectID.ID;
			HTMLПредпросмотрОбъекта.Тип = ФайлXDTO.ObjectID.type;
			HTMLПредпросмотрОбъекта.Расширение = ФайлXDTO.extension;
	...
	Если ИспользоватьЭлектронныеПодписи Тогда
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ОбновитьСписокПодписейФайлов(
			СписокФайлов,
			Подписи,
			УникальныйИдентификатор);
	КонецЕсли;
