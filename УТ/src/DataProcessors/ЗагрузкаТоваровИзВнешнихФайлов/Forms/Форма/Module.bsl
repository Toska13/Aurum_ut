...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПроверятьЗаполнениеНоменклатуры =
		Не Параметры.ПропускатьПроверкуНоменклатуры И Не Параметры.ЗагружатьУпаковочныеЛисты;
	...
	Для Каждого Отбор Из Параметры.ПараметрыОтбора Цикл
		Если Отбор.Ключ = "СегментНоменклатуры" Тогда
			...
			КонецЕсли;
		ИначеЕсли Отбор.Ключ = "ТипНоменклатуры" Тогда
			// Переопределение типов номенклатуры.
			// При указании штрихкода набора, должна быть возможность перенести в документ комплектующие, которые
			// соответствуют допустимым типам номенклатуры документа, даже если в составе допустимых типов явно
			// не определен тип номенклатуры - Набор.
			ДопустимыеТипыНоменклатуры = ОбщегоНазначения.СкопироватьРекурсивно(Отбор.Значение, Ложь);
			
			Если ДопустимыеТипыНоменклатуры.Найти(Перечисления.ТипыНоменклатуры.Набор) = Неопределено Тогда
				ОтборПоСоставуНабора = Истина;
				ДопустимыеТипыНоменклатуры.Добавить(Перечисления.ТипыНоменклатуры.Набор);
			КонецЕсли;
			
			ОтборПоНоменклатуре.Вставить(Отбор.Ключ, ДопустимыеТипыНоменклатуры);
			МассивПараметров.Добавить(Новый ПараметрВыбора("Отбор." + Отбор.Ключ, ДопустимыеТипыНоменклатуры));
		Иначе
	...
	Если ОтборПоНоменклатуре.Свойство("ТипНоменклатуры") Тогда
		ТекстОтбора = НСтр("ru = 'Для выбора доступна номенклатура с типом: %1.'");
		
		Типы				= Новый Массив;
		ПредставлениеТипов	= Новый Массив;
		
		Для Каждого ТипОтбора Из ОтборПоНоменклатуре.ТипНоменклатуры Цикл
			...
			Если ОтборПоСоставуНабора
				И ТипОтбора = Перечисления.ТипыНоменклатуры.Набор Тогда
				
				СодержитУслуги = ОтборПоНоменклатуре.ТипНоменклатуры.Найти(Перечисления.ТипыНоменклатуры.Услуга) <> Неопределено
					Или ОтборПоНоменклатуре.ТипНоменклатуры.Найти(Перечисления.ТипыНоменклатуры.Работа) <> Неопределено;
				СодержитТовары = ОтборПоНоменклатуре.ТипНоменклатуры.Найти(Перечисления.ТипыНоменклатуры.МногооборотнаяТара) <> Неопределено
					Или ОтборПоНоменклатуре.ТипНоменклатуры.Найти(Перечисления.ТипыНоменклатуры.Товар) <> Неопределено;
				
				Если Не СодержитУслуги
					И СодержитТовары Тогда
					
					ПредставлениеНабора =  НСтр("ru = 'Набор, содержащий товары'");
					
				ИначеЕсли СодержитУслуги
					И Не СодержитТовары Тогда
					
					ПредставлениеНабора =  НСтр("ru = 'Набор, содержащий услуги'");
					
				Иначе
					
					ПредставлениеНабора = ТипОтбора;
					
				КонецЕсли;
				
				ПредставлениеТипов.Добавить(ПредставлениеНабора);
				
			Иначе
				ПредставлениеТипов.Добавить(ТипОтбора);
			КонецЕсли;
		КонецЦикла;
		Элементы.ОтборНадпись.Заголовок = СтрШаблон(ТекстОтбора, СтрСоединить(ПредставлениеТипов, ", "));
		
		ПараметрыХарактеристик = Новый Массив;
		ПараметрыХарактеристик.Добавить(Новый ПараметрВыбора("Отбор.ТипНоменклатуры", Типы));
		Элементы.ТоварыХарактеристика.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыХарактеристик);
		Элементы.ТоварыНоменклатураПартнера.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыХарактеристик);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ПроверятьЗаполнениеНоменклатуры Тогда
		МассивНепроверяемыхРеквизитов = Новый Массив;
		МассивНепроверяемыхРеквизитов.Добавить("Объект");
		ОбщегоНазначения.УдалитьНепроверяемыеРеквизитыИзМассива(ПроверяемыеРеквизиты, МассивНепроверяемыхРеквизитов);
		Если Объект.Товары.Количество() = 0 Тогда
			ТекстОшибки = СтрШаблон(НСтр("ru='Не введено ни одной строки в список ""%1""'"),
				Метаданные.Обработки.ЗагрузкаТоваровИзВнешнихФайлов.ТабличныеЧасти.Товары.Синоним);
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ПараметрыЗаполнитьСтавкуНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС();
...
&НаСервере
Процедура ДобавитьПодобраннуюНоменклатуру(ТаблицаПодобраннойНоменклатуры, КодСтроки, НайденнаяНоменклатура, НайденнаяХарактеристика = Неопределено, НайденнаяНоменклатураПартнера = Неопределено)
	...
	ПодобранныеРанееТовары = ТаблицаПодобраннойНоменклатуры.НайтиСтроки(СтруктураОтбора);
	Если ПодобранныеРанееТовары.Количество() = 0 Тогда
		ПодобраннаяСтрока = ТаблицаПодобраннойНоменклатуры.Добавить();
...
Процедура СопоставитьДанныеТабличногоДокументаСДаннымиИБ()
	...
	ПодобраннаяНоменклатураИндексированная = ПодобраннаяНоменклатура.Выгрузить();
	ПодобраннаяНоменклатураИндексированная.Индексы.Добавить("КодСтроки");
	
	НайденныеСтроки = ПодобраннаяНоменклатураИндексированная.Скопировать();
	...
	Пока ЗаполненыНоменклатураКодАртикулШтрихкод Цикл
		...
		Если ЗначениеЗаполнено(НоваяСтрока.Штрихкод) Тогда
			...
			Если ВыборкаНайденныхНоменклатур.Количество() > 0 Тогда
				Пока ВыборкаНайденныхНоменклатур.Следующий() Цикл
					ПодобраннаяСтрока = ПодобраннаяНоменклатураИндексированная.Добавить();
		...
		ИначеЕсли ЗначениеЗаполнено(НоваяСтрока.Код) И ЗначениеЗаполнено(НоваяСтрока.ХарактеристикаНаименование) Тогда
			...
			Для каждого НайденнаяСтрока из НайденныеСтроки Цикл
				ДобавитьПодобраннуюНоменклатуру(ПодобраннаяНоменклатураИндексированная, НомерСтроки, НайденнаяСтрока.Номенклатура, НайденнаяСтрока.Характеристика);
			КонецЦикла;	
		...
		ИначеЕсли ЗначениеЗаполнено(НоваяСтрока.Артикул) И ЗначениеЗаполнено(НоваяСтрока.НоменклатураНаименование) И ЗначениеЗаполнено(НоваяСтрока.ХарактеристикаНаименование) Тогда
			...
			Для каждого НайденнаяСтрока из НайденныеСтроки Цикл
				ДобавитьПодобраннуюНоменклатуру(ПодобраннаяНоменклатураИндексированная, НомерСтроки, НайденнаяСтрока.Номенклатура, НайденнаяСтрока.Характеристика, ?(НашлиНоменклатуруПоставщиков, НайденнаяСтрока.НоменклатураПартнера, Неопределено));
			КонецЦикла;	
		...
		ИначеЕсли ЗначениеЗаполнено(НоваяСтрока.Код) И НЕ ЗначениеЗаполнено(НоваяСтрока.ХарактеристикаНаименование) Тогда
			...
			Для каждого НоменклатураПоКлючу из НайденнаяНоменклатураПоКлючу Цикл
				ДобавитьПодобраннуюНоменклатуру(ПодобраннаяНоменклатураИндексированная, НомерСтроки, НоменклатураПоКлючу.Номенклатура);
			КонецЦикла;			
		...
		ИначеЕсли ЗначениеЗаполнено(НоваяСтрока.Артикул) И ЗначениеЗаполнено(НоваяСтрока.НоменклатураНаименование) И НЕ ЗначениеЗаполнено(НоваяСтрока.ХарактеристикаНаименование) Тогда
			...
			Для каждого НоменклатураПоКлючу из НайденнаяНоменклатураПоКлючу Цикл
				Если НашлиНоменклатуруПоставщиков Тогда
					ДобавитьПодобраннуюНоменклатуру(ПодобраннаяНоменклатураИндексированная, НомерСтроки, НоменклатураПоКлючу.Номенклатура, НоменклатураПоКлючу.Характеристика, НоменклатураПоКлючу.НоменклатураПартнера);
				Иначе
					ДобавитьПодобраннуюНоменклатуру(ПодобраннаяНоменклатураИндексированная, НомерСтроки, НоменклатураПоКлючу.Номенклатура);
				КонецЕсли;	
			КонецЦикла;
				...
				Для каждого НоменклатураПоКлючу из НайденнаяНоменклатураПоКлючу Цикл
					...
					Иначе
						...
						Для каждого ХарактеристикаПоКлючу из НайденныеХарактеристикиПоКлючу Цикл
							ДобавитьПодобраннуюНоменклатуру(ПодобраннаяНоменклатураИндексированная, НомерСтроки, НоменклатураПоКлючу.Номенклатура, ХарактеристикаПоКлючу.Характеристика);
						КонецЦикла;
					КонецЕсли;
				КонецЦикла;
			Иначе
		...
		МассивПодобраннойНоменклатуры = ПодобраннаяНоменклатураИндексированная.НайтиСтроки(Новый Структура("КодСтроки", НомерСтроки));
	...
	Если ПересчитыватьСуммы Тогда
		...
		СуммаРучнойСкидкиЗаполнена = Объект.Товары.Итог("СуммаРучнойСкидки") <> 0;
		ПроцентРучнойСкидкиЗаполнен = Объект.Товары.Итог("ПроцентРучнойСкидки") <> 0;
		ЕстьСкидки = СуммаРучнойСкидкиЗаполнена Или ПроцентРучнойСкидкиЗаполнен;
		
		СтруктураПересчетаСуммы = Новый Структура;
		СтруктураПересчетаСуммы.Вставить("ЦенаВключаетНДС", ЦенаВключаетНДС);
		
		Если Не ЗначениеЗаполнено(Объект.Товары.Итог("СуммаНДС")) Или ЕстьСкидки Тогда
			СтруктураДействий.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Объект.Товары.Итог("СуммаСНДС")) Или ЕстьСкидки Тогда
			СтруктураДействий.Вставить("ПересчитатьСуммуСНДС", СтруктураПересчетаСуммы);
		КонецЕсли; 
		Если Не ЗначениеЗаполнено(Объект.Товары.Итог("Сумма")) Или ЕстьСкидки Тогда
			СтруктураДействий.Вставить("ПересчитатьСумму");
		КонецЕсли;
		
		Если СуммаРучнойСкидкиЗаполнена И Не ПроцентРучнойСкидкиЗаполнен Тогда
			СтруктураДействий.Вставить("ПересчитатьПроцентРучнойСкидки");
		ИначеЕсли Не СуммаРучнойСкидкиЗаполнена И ПроцентРучнойСкидкиЗаполнен Тогда
			СтруктураДействий.Вставить("ПересчитатьСуммуРучнойСкидки");
		КонецЕсли;
	...
	ПодобраннаяНоменклатура.Загрузить(ПодобраннаяНоменклатураИндексированная);
	ПодобраннаяНоменклатураИндексированная = Неопределено;
...
Функция ПолучитьДанныеИБПоНоменклатуреИХарактеристикам()
	...
	Запрос.Текст = 
		...
		|	Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Набор) КАК ЭтоНабор,
		...
		|	Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Набор),
		|	Номенклатура.ИспользованиеХарактеристик,
		|	ВЫБОР
		|		КОГДА Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Набор)
		|			И Номенклатура.ИспользованиеХарактеристик <> ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.НеИспользовать) 
		...
		|	ТаблицаНоменклатуры.ЭтоНабор КАК ЭтоНабор
		...
		|	И НЕ ХарактеристикиНоменклатуры.ПометкаУдаления
		|	И НЕ ПараметрыОтбора.ЭтоНабор
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ПараметрыОтбора.ЗначениеДляОтбораХарактеристик,
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПараметрыОтбораХарактеристик КАК ПараметрыОтбора
		|		ПО ВариантыКомплектацииНоменклатуры.Владелец = ПараметрыОтбора.ЗначениеДляОтбораХарактеристик
		|		И ВариантыКомплектацииНоменклатуры.Основной
		|ГДЕ
		|	НЕ ПараметрыОтбора.ЗначениеДляОтбораХарактеристик = НЕОПРЕДЕЛЕНО
		|	И НЕ ВариантыКомплектацииНоменклатуры.ПометкаУдаления
		|	И ПараметрыОтбора.ЭтоНабор
		|	И &ТекстУсловияОтборПоХарактеристике
		|
		|ИНДЕКСИРОВАТЬ ПО
	...
	ТекстУсловияОтбораПоНоменклатуре = "ИСТИНА";
	ТекстУсловияОтбораПоХарактеристике = "ИСТИНА";
	Если ЗначениеЗаполнено(ОтборПоНоменклатуре) Тогда
		...
		Если ОтборПоНоменклатуре.Свойство("ТипНоменклатуры") Тогда
			...
			Если Не СодержитУслуги
				...
				ТекстУсловияОтбораПоНоменклатуре = ТекстУсловияОтбораПоНоменклатуре + "
					|	И НЕ ЕСТЬNULL(ВариантыКомплектацииНоменклатуры.СодержитУслуги, Ложь)";
				ТекстУсловияОтбораПоХарактеристике = ТекстУсловияОтбораПоХарактеристике + "
					|	И НЕ ВариантыКомплектацииНоменклатуры.СодержитУслуги";
			...
			Если СодержитУслуги
				...
				ТекстУсловияОтбораПоНоменклатуре = ТекстУсловияОтбораПоНоменклатуре + "
					|	И НЕ ЕСТЬNULL(ВариантыКомплектацииНоменклатуры.СодержитТовары, Ложь)";
				ТекстУсловияОтбораПоХарактеристике = ТекстУсловияОтбораПоХарактеристике + "
					|	И НЕ ВариантыКомплектацииНоменклатуры.СодержитТовары";
				
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстУсловияОтборПоХарактеристике", ТекстУсловияОтбораПоХарактеристике);
...
Процедура ЗаполнитьСписокВыбораНоменклатурыХарактеристик(КодТекущейСтроки, СписокХарактеристик = Ложь)
	...
	Для каждого ЭлементМассива Из МассивПодобраннойНоменклатуры Цикл
		Если СписокХарактеристик Тогда
			Если СписокВыбораХарактеристка.НайтиПоЗначению(ЭлементМассива.Характеристика) = Неопределено
				И ЗначениеЗаполнено(ЭлементМассива.Характеристика) Тогда
...
Функция ПоместитьТоварыВХранилище()
	...
	ДействияНеНабор = Новый Структура("ЗаполнитьСтрануПроисхожденияДляНомераГТД");
	
	ОбрабатываемыеСтроки = Новый Массив();
	ОбрабатываемыеСтрокиНабор = Новый Массив();
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		Иначе
			...
			Для Каждого ПодобранныйТовар Из ПодобранныеТовары Цикл
				...
				ОбрабатываемыеСтрокиНабор.Добавить(НоваяСтрока);
