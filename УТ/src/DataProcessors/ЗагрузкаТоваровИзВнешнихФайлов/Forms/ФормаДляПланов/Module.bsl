...
Процедура СопоставитьДанныеТабличногоДокументаСДаннымиИБ()
	...
	Если Не КроссТаблица И ПланироватьПоСумме И Не ЗначениеЗаполнено(Объект.Товары.Итог("Сумма")) Тогда
		СтруктураДействий.Вставить("ПересчитатьСумму");
	КонецЕсли;
...
Процедура СопоставитьЗагружаемыеДанныеСДаннымиИБ()
	...
	ОбрабатываемыеСтрокиПересчетЕдиниц = Новый Массив; 
	ОбрабатываемыеСтрокиПересчетСуммы = Новый Массив;
	...
	Для каждого СтрокаТЧ Из Объект.Товары Цикл
		...
		Если СтрокаТЧ.Количество > 0 Тогда
			ОбрабатываемыеСтрокиПересчетЕдиниц.Добавить(СтрокаТЧ);
		КонецЕсли;
		
		Если НЕ ЭтоПланПродажПоКатегориям И НЕ КроссТаблица И ПланироватьПоСумме 
			И НЕ ЗначениеЗаполнено(СтрокаТЧ.Сумма) Тогда
			ОбрабатываемыеСтрокиПересчетСуммы.Добавить(СтрокаТЧ);
		КонецЕсли;
		
		ПроверитьСопоставлениеДанныхСтрокиЗагрузки(СтрокаТЧ, ЭтотОбъект);
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		ОбрабатываемыеСтрокиПересчетЕдиниц,
		СтруктураДействий,
		Объект.Товары);
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		ОбрабатываемыеСтрокиПересчетСуммы,
		СтруктураДействий,
		Объект.Товары);
...
Процедура ВыполнитьПоискПоШтрихКодам()
	...
	Для Каждого НайденныйШтрихКод Из НайденныеШтрихКоды Цикл
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			...
			ПроверитьСопоставлениеДанныхСтрокиЗагрузки(НайденныеСтроки[0], ЭтотОбъект);
		КонецЕсли;
	КонецЦикла;
