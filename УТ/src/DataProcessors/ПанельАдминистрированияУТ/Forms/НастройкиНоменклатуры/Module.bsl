...
&НаКлиенте
Процедура ИспользоватьАкцизыПриИзменении(Элемент)
	Подключаемый_ПриИзмененииРеквизита(Элемент, Истина);
КонецПроцедуры
...
&НаКлиенте
Процедура ИспользоватьНеуникальныеШтрихкодыНоменклатурыПриИзменении(Элемент)
	
	Подключаемый_ПриИзмененииРеквизита(Элемент);
	ПодключитьОбработчикОповещенияДлительнойОперации(РезультатОбновленияНастроекУникальностиШтрихкода,
													"РезультатОбновленияНастроекУникальностиШтрихкода");
...
&НаКлиенте
Процедура ПодключитьОбработчикОповещенияДлительнойОперации(РезультатВыполненияОперации, ИмяРезультатаОперации)
	
	Если РезультатВыполненияОперации <> Неопределено Тогда
		ПараметрыОповещения		= Новый Структура("ИмяРезультатаОперации", ИмяРезультатаОперации);
		ОповещениеОЗавершении	= Новый ОписаниеОповещения("ОбработчикОповещенияДлительнойОперации",
															ЭтотОбъект,
															ПараметрыОповещения);
		ПараметрыОжидания		= ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
		
		ДлительныеОперацииКлиент.ОжидатьЗавершение(РезультатВыполненияОперации, ОповещениеОЗавершении, ПараметрыОжидания);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработчикОповещенияДлительнойОперации(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		ИмяРезультатаОперации		= ДополнительныеПараметры.ИмяРезультатаОперации;
		РезультатВыполненияОперации	= ЭтотОбъект[ИмяРезультатаОперации];
		
		Если РезультатВыполненияОперации = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		Если ИмяРезультатаОперации = "РезультатОбновленияНастроекУникальностиШтрихкода" Тогда
			РезультатВыполненияОперации = Неопределено;
		КонецЕсли;
	КонецЕсли;
...
Функция СохранитьЗначениеРеквизита(РеквизитПутьКДанным)
	...
	Если КонстантаИмя <> "" Тогда
		...
		Если КонстантаИмя = "ИспользоватьНеуникальныеШтрихкодыНоменклатуры" Тогда
			
			Если Справочники.ВидыНоменклатуры.ЕстьУстановленныеНастройкиУникальностиШтрихкода(КонстантаЗначение) Тогда
				ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
				ПараметрыВыполнения.ЗапуститьВФоне = Истина;
				ПараметрыВыполнения.ОжидатьЗавершение = 0;
				
				РезультатРасчета = ДлительныеОперации.ВыполнитьПроцедуру(
									ПараметрыВыполнения,
									"Справочники.ВидыНоменклатуры.УстановитьНастройкиУникальностиШтрихкодаПоУмолчанию",
									КонстантаЗначение);
				
				Если РезультатРасчета.Статус = "Выполняется" Тогда
					РезультатОбновленияНастроекУникальностиШтрихкода = РезультатРасчета;
				ИначеЕсли РезультатРасчета.Статус = "Выполнено" Тогда
					РезультатОбновленияНастроекУникальностиШтрихкода = Неопределено;
				КонецЕсли;
			КонецЕсли;
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьПодакцизныеТовары" ИЛИ РеквизитПутьКДанным = "" Тогда
		ЗначениеКонстанты = НаборКонстант.ИспользоватьПодакцизныеТовары;
		
		ОбщегоНазначенияУТКлиентСервер.ОтображениеПредупрежденияПриРедактировании(
			Элементы.ИспользоватьАкцизы, ЗначениеКонстанты);
...
Процедура ОтображениеПредупрежденияПриРедактировании(РеквизитПутьКДанным)
	...
	СтруктураКонстант = Новый Структура("ИспользоватьНеуникальныеШтрихкодыНоменклатуры");
