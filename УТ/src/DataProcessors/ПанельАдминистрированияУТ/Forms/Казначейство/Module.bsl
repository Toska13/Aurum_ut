...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзмененВариантИспользованияЛимитовБДР" Тогда
		УстановитьСписокВыбораВариантаИспользованияЛимитов();
		Возврат;
...
Процедура Подключаемый_ПриИзмененииРеквизита(Элемент, ОбновлятьИнтерфейс = Истина, ВнешнееИзменение = Ложь)
	...
	Если КонстантаИмя = "КонтрольЛимитовПоДаннымБюджетирования2_4" Тогда
		
		ДополнительныеПараметры = Новый Структура(?(ВариантИспользованияЛимитов = "ЛимитыБюджетирования", "КонтрольЛимитовПоДаннымБюджетирования2_5", "КонтрольЛимитовПоДаннымБюджетирования2_4"));
		КонстантаИмя = "";
		Результат = УстановитьВариантКонтроляЛимитовПоДаннымБюджетирования(УникальныйИдентификатор, ДополнительныеПараметры);
		...
		Если Результат.Статус = "Выполняется" Тогда
			...
			ОповещениеОЗакрытии = Новый ОписаниеОповещения("УстановкаВариантаКонтроляЛимитовБюджетированияЗавершено", ЭтотОбъект);
			
			ОткрытьФорму("ОбщаяФорма.ДлительнаяОперация", ПараметрыФормаДлительнойОперации, ЭтотОбъект, , , , ОповещениеОЗакрытии);
			
		Иначе
			
			УстановкаВариантаКонтроляЛимитовБюджетированияЗавершено(Результат, Неопределено);
	...
	ИначеЕсли Элемент.Имя = "ВариантИспользованияЛимитов" Тогда
		Оповестить("ИзмененВариантИспользованияЛимитовБДДС");
	КонецЕсли;
...
&НаКлиенте
Процедура УстановкаВариантаКонтроляЛимитовБюджетированияЗавершено(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если ЗначениеЗаполнено(Результат.АдресРезультата) 
			...
			ПоказатьОповещениеПользователя(НСтр("ru = 'Установлен контроль лимитов по данным бюджетирования (2.5).'"));
		...
		Иначе
			...
			ВариантИспользованияЛимитов = "ЛимитыБюджетирования2_4";
			Подключаемый_ПриИзмененииРеквизита(Элементы.ВариантИспользованияЛимитов, Ложь);
...
Функция УстановитьВариантКонтроляЛимитовПоДаннымБюджетирования(УникальныйИдентификатор, ПараметрыЗапуска)
