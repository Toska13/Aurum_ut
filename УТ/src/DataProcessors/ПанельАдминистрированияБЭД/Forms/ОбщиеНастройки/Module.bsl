...
&НаКлиенте
Перем ВариантПроверкиСертификатаДоИзменения;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЕстьИнтеграцияЦентраЭДО = ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСКонтрагентами.ИнтеграцияЦентраЭДО");
	...
	Элементы.ГруппаНастройкиИнтеграцииЦентраЭДО.Видимость = ЕстьИнтеграцияЦентраЭДО;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ЗавершениеПроверкиПодписейМЧД" Тогда
		ОбновитьЭлементыФормы();
	КонецЕсли;
...
&НаКлиенте
Процедура Константа_ИспользоватьИнтеграциюЦентраЭДОПриИзменении(Элемент)
	Подключаемый_ПриИзмененииРеквизита(Элемент);
КонецПроцедуры
...
&НаКлиенте
Процедура РежимСтрогойПроверкиДоверенностейПриИзменении(Элемент)
	
	Если РежимСтрогойПроверкиДоверенностей = 1 Тогда
		
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ОбновляемыйЭлементФормы", Элемент);
		
		ОписаниеОповещения = 
			Новый ОписаниеОповещения("ОбновитьСведенияОРезультатахПроверкиПодписейПоМЧД",
		...
		ПараметрыВопросаПользователю.Заголовок =  НСтр("ru = 'Отключение строгой проверки доверенностей'");
		ПараметрыВопросаПользователю.Картинка = БиблиотекаКартинок.ДиалогВосклицание;
		
		ТекстВопроса = 
			НСтр("ru = 'Для перехода на упрощенную проверку подписей по доверенностям 
				|программе потребуется выполнить повторную проверку правильности подписания всех накопленных МЧД.'")
			+ Символы.ПС
			+ НСтр("ru = 'Это может занять продолжительное время.'") 
			+ Символы.ПС;
...
&НаКлиенте
Процедура Константа_ИспользоватьЗаполнениеРеквизитовВходящегоДокументаПоНаименованиюФайлаПриИзменении(Элемент)
...
&НаКлиенте
Процедура Константа_ВариантПроверкиСертификатовМЧДОчистка(Элемент, СтандартнаяОбработка)
...
&НаКлиенте
Процедура Константа_ВариантПроверкиСертификатовМЧДПриИзменении(Элемент)
	
	Если Константа_ВариантПроверкиСертификатовМЧДЭДО = ВариантПроверкиСертификатаДоИзменения Тогда
		Возврат;
	КонецЕсли;
	...
	ОписаниеОповещения = 
		Новый ОписаниеОповещения("ПослеСменыВариантаПроверкиСертификатаДоверителяМЧД",
			ЭтотОбъект, ДополнительныеПараметры);
	...
	ПараметрыВопросаПользователю = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
	ПараметрыВопросаПользователю.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
	ПараметрыВопросаПользователю.Заголовок =  НСтр("ru = 'Смена варианта проверки сертификата доверителя'");
	ПараметрыВопросаПользователю.Картинка = БиблиотекаКартинок.ДиалогВосклицание;
	
	ТекстВопроса = 
		НСтр("ru = 'Для смены варианта проверки сертификата доверителя доверенности
			|программе потребуется выполнить повторную проверку подписей
			|всех накопленных МЧД.'")
		+ Символы.ПС
		+ НСтр("ru = 'Это может занять продолжительное время.'");
...
&НаКлиенте
Процедура Константа_ВариантПроверкиСертификатовМЧДОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, ВыборДобавлением, СтандартнаяОбработка)
	ВариантПроверкиСертификатаДоИзменения = Константа_ВариантПроверкиСертификатовМЧДЭДО;
КонецПроцедуры
...
&НаКлиенте
Процедура ОткрытьУчетныеЗаписиЦентровЭДО(Команда)
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСКонтрагентами.ИнтеграцияЦентраЭДО") Тогда
		МодульИнтеграцияЦентраЭДОКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ИнтеграцияЦентраЭДОКлиент");
		МодульИнтеграцияЦентраЭДОКлиент.ОткрытьСписокУчетныхЗаписейЦентровЭДО();
	КонецЕсли;
...
Процедура СохранитьЗначениеРеквизита(Знач РеквизитПутьКДанным, Результат, ЗначениеРеквизита = "ЗначениеРеквизита")
	...
	Иначе
		...
		Если РеквизитПутьКДанным = "РежимСтрогойПроверкиДоверенностей" Тогда
			КонстантаИмя = "РежимСтрогойПроверкиДоверенностейОтключенЭДО";
			ЗначениеРеквизита = ?(РежимСтрогойПроверкиДоверенностей = 0, Ложь, Истина);
		КонецЕсли;
...
Процедура ПрочитатьУстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если ЕстьОбменСКонтрагентами Тогда
		...
		РежимСтрогойПроверкиДоверенностей = 
			?(МодульМашиночитаемыеДоверенности.РежимСтрогойПроверкиДоверенностейОтключен(), 1, 0);
		
		Константа_ВариантПроверкиСертификатовМЧДЭДО = 
			МодульМашиночитаемыеДоверенности.ВариантПроверкиСертификатаДоверителя();
	...
	Если ЕстьОбменСКонтрагентами
		...
		Константа_ИспользоватьЗаполнениеРеквизитовВходящегоДокументаПоНаименованиюФайла = 
			Константы["ИспользоватьЗаполнениеРеквизитовВходящегоДокументаПоНаименованиюФайла"].Получить();
		...
		МодульАвтоматическоеПолучениеЭДО = ОбщегоНазначения.ОбщийМодуль("АвтоматическоеПолучениеЭДО");
		ВидимостьАвтоматическогоПолученияПоТокенам = МодульАвтоматическоеПолучениеЭДО.ДоступноПолучениеПоТокенам();
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ГруппаОтправкаЭД", "Доступность", Константа_ИспользоватьОбменЭД И ДоступноАдминистрирование);
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ГруппаНастройкаРегламентногоЗаданияПолучение", "Доступность", Константа_ИспользоватьОбменЭД И ДоступноАдминистрирование);
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ГруппаНастройкиИспользоватьЗаполнениеРеквизитовПоНаименованиюФайла", "Доступность",
			Константа_ИспользоватьОбменЭД);
	...
	Если РеквизитПутьКДанным = "Константа_ИспользоватьЭлектронныеПодписи" ИЛИ РеквизитПутьКДанным = "" Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"Константа_ВариантПроверкиСертификатовМЧДЭДО", "Видимость", ЕстьОбменСКонтрагентами);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"Константа_ВариантПроверкиСертификатовМЧДЭДО", "Доступность", ИспользоватьЭлектронныеПодписи);
	КонецЕсли;
	
	Если ЕстьОбменСКонтрагентами
		И (РеквизитПутьКДанным = "Константа_ИспользоватьИнтеграциюЦентраЭДО" 
			ИЛИ РеквизитПутьКДанным = "") Тогда
		Константа_ИспользоватьИнтеграциюЦентраЭДО = Константы["ИспользоватьИнтеграциюЦентраЭДО"].Получить();
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ГруппаУчетныеЗаписиЦентровЭДО", "Доступность", Константа_ИспользоватьИнтеграциюЦентраЭДО);
	КонецЕсли;
	...
	Если ЕстьИнтеграцияЦентраЭДО
		И (РеквизитПутьКДанным = "Константа_ИспользоватьИнтеграциюЦентраЭДО" ИЛИ РеквизитПутьКДанным = "") Тогда
...
&НаКлиенте
Процедура ОбновитьСведенияОРезультатахПроверкиПодписейПоМЧД(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Или Результат.Значение = КодВозвратаДиалога.Нет Тогда
		РежимСтрогойПроверкиДоверенностей = 0;
		Возврат;
	КонецЕсли;
	
	ПроверитьПодписиМЧД(ДополнительныеПараметры.ОбновляемыйЭлементФормы);
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеСменыВариантаПроверкиСертификатаДоверителяМЧД(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Или Результат.Значение = КодВозвратаДиалога.Нет Тогда
		Константа_ВариантПроверкиСертификатовМЧДЭДО = ВариантПроверкиСертификатаДоИзменения;
		Возврат;
	КонецЕсли;
	
	Подключаемый_ПриИзмененииРеквизита(ДополнительныеПараметры.ОбновляемыйЭлементФормы);
...
&НаКлиенте
Процедура ПроверитьПодписиМЧД(ОбновляемыйЭлементФормы)
	
	КонтекстДиагностики = ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики();
	
	ДлительнаяОперация = НачатьПроверкуПодписейМЧДНаСервере();
	
	ПараметрыОжиданияБЭД = ПараметыОжиданияОперацииПроверкиПодписейМЧД();
	...
	ДополнительныеПараметры.Вставить("ИдентификаторОперации", КонтекстОжиданияОперации.ИдентификаторОперации);
	ДополнительныеПараметры.Вставить("ЭтоПрерываниеНаКлиенте", Ложь);
	ДополнительныеПараметры.Вставить("ОбновляемыйЭлементФормы", ОбновляемыйЭлементФормы);
	
	ОповещениеЗавершениеПроверкиПодписей = 
		Новый ОписаниеОповещения("ОбработатьЗавершениеПроверкиПодписейМЧД", ЭтотОбъект, ДополнительныеПараметры);
	...
	ОповещениеПослеПроверкиНаСервере = 
		Новый ОписаниеОповещения("ОбработатьРезультатПроверкиПодписейМЧДНаСервере", ЭтотОбъект, ДополнительныеПараметры);
	...
	ОповещениеОПрогрессеВыполнения = 
		...
		МодульМашиночитаемыеДоверенностиКлиент, КонтекстОжиданияОперации);
...
&НаКлиенте
Функция ПараметыОжиданияОперацииПроверкиПодписейМЧД()

	ПараметрыОжидания = ОжиданиеОперацийБЭДКлиент.НовыеПараметры();
	ПараметрыОжидания.ПроцентПрогресса = 0;
	ПараметрыОжидания.ОтображатьПроценты = Истина;
	ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Выполняется проверка подписей доверенностей.
...
&НаСервере
Функция НачатьПроверкуПодписейМЧДНаСервере()
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияФункции(Новый УникальныйИдентификатор);
	ПараметрыВыполнения.ЗапуститьВФоне = Истина;
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = 
		НСтр("ru = 'Обновление результатов проверок подписей МЧД на сервере'");
		
	// См. МашиночитаемыеДоверенности.ПроверитьПодписиМЧД
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
		"МашиночитаемыеДоверенности.ПроверитьПодписиМЧД");
	
КонецФункции

&НаКлиенте
Процедура ОбработатьЗавершениеПроверкиПодписейМЧД(РезультатОперации, ДополнительныеПараметры) Экспорт
	...
	Оповестить("ЗавершениеПроверкиПодписейМЧД");
	ОжиданиеОперацийБЭДКлиент.ЗакрытьФормуОжиданияПоИдентификаторуОперации(
		ДополнительныеПараметры.ИдентификаторОперации);
	
	Если ДополнительныеПараметры.ЭтоПрерываниеНаКлиенте Тогда
		
		ТекстСообщения = НСтр("ru = 'Перепроверка доверенностей не выполнена.
									|Операция была прервана пользователем.'") + Символы.ПС;
									
		ОбщегоНазначенияБЭДКлиент.ПоказатьФорматированнуюСтроку(
			Новый ФорматированнаяСтрока(ТекстСообщения), 
				НСтр("ru = 'Проверка подписей МЧД'"), НСтр("ru = 'Понятно'"));
	...
	Если РезультатОперации = Истина Тогда
		...
		Если ДополнительныеПараметры.ОбновляемыйЭлементФормы.Имя = "РежимСтрогойПроверкиДоверенностей" Тогда
	
			ТекстСообщения = НСтр("ru = 'Выполнена перепроверка всех МЧД.
							|Включена упрощенная проверка подписей документов по МЧД.'") + Символы.ПС;
		
			ОбщегоНазначенияБЭДКлиент.ПоказатьФорматированнуюСтроку(
				Новый ФорматированнаяСтрока(ТекстСообщения), 
					НСтр("ru = 'Проверка подписей МЧД'"), НСтр("ru = 'Понятно'"));
			
			РежимСтрогойПроверкиДоверенностей = 1;
			Подключаемый_ПриИзмененииРеквизита(ДополнительныеПараметры.ОбновляемыйЭлементФормы, Истина);
			
		КонецЕсли;
		
		Если ДополнительныеПараметры.ОбновляемыйЭлементФормы.Имя = "Константа_ВариантПроверкиСертификатовМЧДЭДО" Тогда
	
			ТекстСообщения = НСтр("ru = 'Выполнена перепроверка всех МЧД.'") + Символы.ПС;
	...
	Если Не ДополнительныеПараметры.ЭтоПрерываниеНаКлиенте Тогда
		
		ТекстСообщения = НСтр("ru = 'Перепроверка всех доверенностей не выполнена.
									|Подробнее см. Журнал регистрации.'") + Символы.ПС;
...
&НаКлиенте
Процедура ОбработатьРезультатПроверкиПодписейМЧДНаСервере(РезультатВыполненияФоновогоЗадания, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Неопределено Тогда
		ВыполнитьОбработкуОповещения(ОповещениеЗавершениеПроверкиПодписей, Ложь);
		Возврат;
	КонецЕсли;
	
	Если Результат.ПроверкаВыполнена = Истина Тогда
		ВыполнитьОбработкуОповещения(ОповещениеЗавершениеПроверкиПодписей, Истина);
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Результат.ДоверенностиКПроверкеНаКлиенте) Тогда
		
		ПараметрыОжидания = ПараметыОжиданияОперацииПроверкиПодписейМЧД();
		КонтекстОжиданияОперации = ОжиданиеОперацийБЭДКлиент.НовыйКонтекст(ПараметрыОжидания);
		КонтекстОжиданияОперации.ИдентификаторОперации = ДополнительныеПараметры.ИдентификаторОперации;
		
		ПараметрыПроверки = Новый Структура();
		ПараметрыПроверки.Вставить("Доверенности", Результат.ДоверенностиКПроверкеНаКлиенте);
		ПараметрыПроверки.Вставить("ДоверенностиПроверенные", Новый Массив());
		ПараметрыПроверки.Вставить("ПроверяемаяМЧД", Неопределено);
		ПараметрыПроверки.Вставить("ДанныеДоверенности", Неопределено);
		ПараметрыПроверки.Вставить("ОповещениеОЗавершении", ОповещениеЗавершениеПроверкиПодписей);
		ПараметрыПроверки.Вставить("КонтекстДиагностики", ДополнительныеПараметры.КонтекстДиагностики);
		ПараметрыПроверки.Вставить("КонтекстОжиданияОперации", КонтекстОжиданияОперации);
		
		МодульМашиночитаемыеДоверенностиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("МашиночитаемыеДоверенностиКлиент");
		
		МодульМашиночитаемыеДоверенностиКлиент.ПроверитьПодписиДоверенностей(
			Результат.ДоверенностиКПроверкеНаКлиенте, ПараметрыПроверки);
	КонецЕсли;
