...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЕстьПравоОтраженияДокументовВРеглУчете = ПолучитьФункциональнуюОпцию("УправлениеТорговлей");
	
	ЕстьПравоОтраженияДокументовВМеждународномУчете = Ложь;
	...
	РежимРаботы.ДоступностьВидовУчета.Вставить("РегламентированныйУчет", ЕстьПравоОтраженияДокументовВРеглУчете);
	РежимРаботы.ДоступностьВидовУчета.Вставить("МеждународныйУчет", ЕстьПравоОтраженияДокументовВМеждународномУчете);
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Если НЕ РежимРаботы.КонтекстноеОткрытиеФормы Тогда
		...
		ПервыйНезаблокированныйПериод = РегистрыСведений.НастройкаБлокировкиОтИзменений.НезаблокированныйПериод(Объект.МассивОрганизаций);
...
Процедура ПредставлениеОрганизацийНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ОбновитьДанныеФормыВФонеНаКлиенте(НЕ ОписаниеОрганизацийСформировано, Ложь);
...
Процедура ДеревоЭтаповВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элемент.ТекущийЭлемент = Элементы.ДеревоЭтаповГиперссылкаВыполнить И НЕ ИзмененияЗапрещены Тогда
		...
		Если СтрокаДереваЭтапов.ВыполнениеНаСервере Тогда
		
			СтандартнаяОбработка = Ложь;
			ОповещениеОбОкончанииВыбора =
				Новый ОписаниеОповещения("ДеревоЭтаповВыборЗавершение", ЭтотОбъект);
			
			СписокТиповПредметов = Новый СписокЗначений;
			СписокТиповПредметов.Добавить("ЭтотИПредыдущие",
				НСтр("ru='Выполнить этот и все предыдущие этапы (рекомендуется)'"),,
				БиблиотекаКартинок.СтартБизнесПроцесса);
				
			СписокТиповПредметов.Добавить("ТолькоЭтот",
				НСтр("ru='Выполнить только этот этап'"),,БиблиотекаКартинок.ПерейтиВперед);
			
			ПоказатьВыборИзМеню(ОповещениеОбОкончанииВыбора, СписокТиповПредметов);
...
&НаКлиенте
Процедура ДеревоЭтаповВыборЗавершение(ВыбранныйСпособ, ДопПараметры) Экспорт
	
	Если ВыбранныйСпособ = Неопределено Тогда
		Возврат;
	КонецЕсли;

	Если ВыбранныйСпособ.Значение = "ЭтотИПредыдущие" Тогда
		ОбработатьНажатиеГиперссылки(
			ДеревоЭтапов.НайтиПоИдентификатору(Элементы.ДеревоЭтапов.ТекущаяСтрока), Истина, Ложь);
	ИначеЕсли ВыбранныйСпособ.Значение = "ТолькоЭтот" Тогда
		ОбработатьНажатиеГиперссылки(
			ДеревоЭтапов.НайтиПоИдентификатору(Элементы.ДеревоЭтапов.ТекущаяСтрока), Истина, Истина);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура НастройкиРегламентногоЗаданияНеоперативногоДопроведенияДокументовНажатие(Элемент)

	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Действие", "Изменить");
	ПараметрыФормы.Вставить("Идентификатор",
		ПараметрыРегламентногоЗаданияНеоперативнойОчереди.ИдентификаторРегламентногоЗадания);
...
Процедура ОбновитьФорму(Команда)
	...
	ОбновитьДанныеФормыВФонеНаКлиенте(, Истина);
...
&НаКлиенте
Процедура ОткрытьВидыНеоперативныхЗаданий(Команда)
	
	ОткрытьФорму(
		"Справочник.ВидыНеоперативныхЗаданий.ФормаСписка",
		ЗакрытиеМесяцаКлиентЛокализация.ПараметрыФормыРегистраЗаданий(ЭтотОбъект));

КонецПроцедуры

&НаКлиенте
Процедура ОткрытьЗаданияНеоперативногоДопроведенияДокументов(Команда)
	
	ОткрытьФорму(
		"РегистрСведений.ЗаданияНеоперативногоДопроведенияДокументов.ФормаСписка",
		ЗакрытиеМесяцаКлиентЛокализация.ПараметрыФормыРегистраЗаданий(ЭтотОбъект));
		
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьЗаданияКРаспределениюВзаиморасчетов(Команда)
	
	ОткрытьФорму(
		"РегистрСведений.ЗаданияКРаспределениюВзаиморасчетов.ФормаСписка",
		ЗакрытиеМесяцаКлиентЛокализация.ПараметрыФормыРегистраЗаданий(ЭтотОбъект));
		
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьФормуЗакрытиеМесяца24(Команда)
	
	ОбновитьОсновнуюФормуСервер();
	Закрыть();
	ОткрытьФорму("Обработка.ОперацииЗакрытияМесяца.Форма",,,,,,,РежимОткрытияОкнаФормы.Независимый);
...
Процедура НастроитьЭлементыПриИнициализацииФормы(ВыполняетсяЗагрузкаНастроек = Ложь)
	...
	Если НЕ ВыполняетсяЗагрузкаНастроек Тогда
		...
		Элементы.ФормаОткрытьВидыНеоперативныхЗаданий.Видимость = ПравоДоступа("Просмотр",
			Метаданные.Справочники.ВидыНеоперативныхЗаданий);
		Элементы.ФормаОткрытьЗаданияНеоперативногоДопроведенияДокументов.Видимость = ПравоДоступа("Просмотр",
			Метаданные.РегистрыСведений.ЗаданияНеоперативногоДопроведенияДокументов);
		Элементы.ФормаОткрытьЗаданияКРаспределениюВзаиморасчетов.Видимость = ПравоДоступа("Просмотр",
			Метаданные.РегистрыСведений.ЗаданияКРаспределениюВзаиморасчетов);
		Элементы.ГруппаНеоперативнаяОчередьЗаданий.Видимость = Элементы.ФормаОткрытьВидыНеоперативныхЗаданий.Видимость
			Или Элементы.ФормаОткрытьЗаданияНеоперативногоДопроведенияДокументов.Видимость
			Или Элементы.ГруппаНеоперативнаяОчередьЗаданий.Видимость;
...
Процедура НастроитьЭлементыВЗависимостиОтНаличияФоновыхЗаданий()
	...
	Иначе
		...
		Если ИдетОбновление Тогда
			ПредставлениеРассчитываемогоЭтапа =
				Новый ФорматированнаяСтрока(СокрЛП(СостояниеЗадания.Наименование) + ": ");
				
		КонецЕсли;
		
	КонецЕсли;
	
	Элементы.ДекорацияБублик.Видимость 							  = ИдетРасчет ИЛИ ИдетОбновление;
	...
	Элементы.ГруппаОтменаРасчетаВФоне.Видимость 	  			  = ИдетРасчет;
	Элементы.ГруппаВыполнениеРасчетаВФоне.Видимость   			  = Истина;
	...
	Элементы.ГруппаДеревоЭтапов.Доступность 				   	  = НЕ ИдетОбновление Или ФоновоеОбновлениеСостояния;
	Элементы.ФормаПосмотретьПериодыЗаданий.Доступность			  = НЕ ИдетРасчет И НЕ ИдетОбновление;
	
	Элементы.ДеревоЭтаповГиперссылкаВыполнить.Доступность		  = НЕ ИдетРасчет 
		И (НЕ ИдетОбновление Или ФоновоеОбновлениеСостояния) И НЕ ИзмененияЗапрещены;
	Элементы.ДеревоЭтаповГиперссылкаПодробнее.Доступность		  = (НЕ ИдетОбновление Или ФоновоеОбновлениеСостояния);
	...
	Элементы.КнопкаНастроитьБлокировкиОтИзменений.Доступность	  = НЕ ИдетРасчет
		И (НЕ ИдетОбновление Или ФоновоеОбновлениеСостояния);
...
&НаКлиенте
Процедура ОбновитьДанныеФормыВФонеНаКлиенте(ФормироватьОписаниеОрганизаций = Истина, ОбновитьКэшСтатусовОпераций = Истина)
	
	ОтключитьОбработчикОжидания("ПроверитьАктивностьФоновогоЗаданияОбновленияФормы");
	
	ЗапуститьОбновлениеДанныхФормыВФоне(ФормироватьОписаниеОрганизаций, ОбновитьКэшСтатусовОпераций);
	
	Если ФоновоеЗаданиеФормированияСписка <> Неопределено Тогда
		
		ОповещениеОПрогрессе = Новый ОписаниеОповещения(
			"ОбновитьПрогрессФормированияЭтапов", ЭтотОбъект);
		
		ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
		ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
		ПараметрыОжидания.ОповещениеОПрогрессеВыполнения = ОповещениеОПрогрессе;
		 
		ОповещениеОЗавершение = Новый ОписаниеОповещения("ОбновитьДанныеФормыВФонеНаКлиентеЗавершение", ЭтотОбъект);
		ДлительныеОперацииКлиент.ОжидатьЗавершение(
			ФоновоеЗаданиеФормированияСписка, ОповещениеОЗавершение, ПараметрыОжидания);
...
&НаКлиенте
Процедура ОбновитьДанныеФормыВФонеНаКлиентеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Статус = "Ошибка" Тогда
		...
		ДатаДанных = ДатаАктуальностиДанных();
		ОтобразитьДатуСостоянияЭтапов(ДатаДанных);
...
&НаКлиенте
Процедура ОбновитьПрогрессФормированияЭтапов(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат <> Неопределено
		И Результат.Статус = "Выполняется"
		И Результат.Прогресс <> Неопределено Тогда
			
			Прогресс = Результат.Прогресс; // см. ДлительныеОперацииКлиент.НовоеСостояниеДлительнойОперации
			ДополнительныеПараметры = Прогресс.ДополнительныеПараметры; // см. ДлительныеОперации.СообщитьПрогресс.ДополнительныеПараметры
			
			Если ДополнительныеПараметры.ОбновитьКэшСтатусовОпераций Тогда
				
				Статус = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Статус операций закрытия месяца актуален на %1. Идет обновление %2%.'"),
					Формат(ДополнительныеПараметры.ДатаАктуальностиДанных, "ДЛФ=DDT"),
					Прогресс.Процент);
					
				ПредставлениеРассчитываемогоЭтапа = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
					Статус);
			Иначе
				ПредставлениеРассчитываемогоЭтапа = Новый ФорматированнаяСтрока(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Формирование списка операций закрытия месяца %1: %2'"),
						ЗакрытиеМесяцаКлиентСервер.ПредставлениеПериодаМесяцГод(Объект.ПериодРегистрации),
						Строка(Прогресс.Процент) + "%"));
			КонецЕсли;
...
&НаСервере
Функция ЗапуститьОбновлениеДанныхФормыВФоне(ФормироватьОписаниеОрганизаций = Истина, ОбновитьКэшСтатусовОпераций = Истина)
	...
	ПараметрыЗапускаОбновленияСостояния.ОбновитьКэшСтатусовОпераций = ОбновитьКэшСтатусовОпераций;
	
	ФоновоеОбновлениеСостояния = ОбновитьКэшСтатусовОпераций;
	...
	ФоновоеЗаданиеФормированияСписка = ДлительныеОперации.ВыполнитьВФоне(
		"ЗакрытиеМесяцаСервер.ЗаполнитьСостоянияЭтаповВФоновомЗадании",
		ПараметрыЗапускаОбновленияСостояния,
		ПараметрыВыполнения);

	ИдентификаторЗаданияОбновленияСостоянийЭтапов = ФоновоеЗаданиеФормированияСписка.ИдентификаторЗадания;
	
	// Настроим элементы формы.
	НастроитьЭлементыВЗависимостиОтНаличияФоновыхЗаданий();
	
	УстановитьПараметрыРегламентногоЗаданияНеоперативнойОчереди();
...
Процедура ЗаполнитьФормуОбновленнымиДаннымиНаКлиенте(ПолучитьДанныеИзФоновогоЗадания = Ложь)
	...
	Результат = ЗаполнитьФормуОбновленнымиДаннымиНаСервере(ПолучитьДанныеИзФоновогоЗадания);
	...
	Если Результат.ТребуетсяПерезапуск Тогда
		ОбновитьДанныеФормыВФонеНаКлиенте(,Истина);
	КонецЕсли;
...
Функция ЗаполнитьФормуОбновленнымиДаннымиНаСервере(ПолучитьДанныеИзФоновогоЗадания = Ложь)
	...
	Результат = Новый Структура("ТекстОшибки, ТребуетсяПерезапуск", "", Ложь);
	Результат.ТребуетсяПерезапуск = Ложь;
	
	ДеревоЭтапов.ПолучитьЭлементы().Очистить();
	
	Если ТипЗнч(СтруктураДанных) = Тип("Структура") Тогда // фоновое задание выполнено успешно
		Результат.ТребуетсяПерезапуск = СтруктураДанных.Свойство("СостоянияЭтаповИзКэша");
		...
		Если Результат.ТребуетсяПерезапуск Тогда
			...
			ПредставлениеРассчитываемогоЭтапа = СтроковыеФункции.ФорматированнаяСтрока(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Статус операций закрытия месяца актуален на %1. Идет обновление 1%.'"),
					Формат(СтруктураДанных.ДатаАктуальностиДанных, "ДЛФ=DDT")));
			
		Иначе

			ПредставлениеРассчитываемогоЭтапа = СтроковыеФункции.ФорматированнаяСтрока(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Статус операций закрытия месяца актуален на %1.'"),
	...
	Если ТипЗнч(СтруктураДанных) <> Тип("Структура") Тогда
		...
		Иначе
			...
			Пояснение = "";
			Если СостояниеЗадания.Отменено Тогда
				Пояснение = НСтр("ru = 'отменено администратором'");
			ИначеЕсли СостояниеЗадания.ЗавершеноАварийно Тогда
				Пояснение = НСтр("ru = 'завершено аварийно'");
			КонецЕсли;
			
			Если ЗначениеЗаполнено(Пояснение) Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При формировании списка операций закрытия месяца возникла ошибка:
						|фоновое задание %1.'"), Пояснение);
			КонецЕсли;
	...
	СтруктураДанных.Удалить("СостоянияЭтаповИзКэша");
...
Процедура ЗаполнитьПромежуточныеПоясненияЭтапа(РассчитываемыйЭтап, СтруктураДанных, РасчетВыполняется = Ложь)
	...
	ПараметрыОбработчика = Обработки.ОперацииЗакрытияМесяца.ИнициализироватьПараметрыОбработчикаЭтапа(
		...
		СтрокаТаблицыЭтапов,
		ПараметрыЗапускаОбновленияСостояния,
		"ЗакрытиеМесяца.ОбновлениеСостояния."
			+ Справочники.ОперацииЗакрытияМесяца.ПолучитьИмяПредопределенного(СтрокаТаблицыЭтапов.Код));
...
Процедура ПроверитьАктивностьФоновогоЗаданияРасчета()
	...
	Если СостояниеЗадания.Активно // еще выполняется расчет этапа
	 ИЛИ ЗапуститьРасчетСледующегоЭтапаВФоне() Тогда // запущен расчет следующего этапа
		...
		ПредставлениеРассчитываемогоЭтапа = 
			Новый ФорматированнаяСтрока(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Если ПрерватьРасчет Тогда
		...
		// Расчет прерван.
		ОбновитьДанныеФормыВФонеНаКлиенте(Ложь, Ложь);
...
&НаСервере
Процедура УстановитьПараметрыРегламентногоЗаданияНеоперативнойОчереди()

	ПараметрыРегламентногоЗаданияНеоперативнойОчереди = НеоперативнаяОчередьЗаданий.ПараметрыРегламентногоЗаданияНеоперативнойОчереди();
	
	ОтображатьГиперссылку = Ложь;
	
	Если НЕ ОбщегоНазначения.РазделениеВключено()
	 И НЕ ПараметрыРегламентногоЗаданияНеоперативнойОчереди.ИдентификаторРегламентногоЗадания = Неопределено Тогда
		ОтображатьГиперссылку = Не ПараметрыРегламентногоЗаданияНеоперативнойОчереди.РегламентноеЗаданиеИспользуется;
	КонецЕсли;
	
	Элементы.НастройкиРегламентногоЗаданияНеоперативногоДопроведенияДокументов.Видимость = ОтображатьГиперссылку;

КонецПроцедуры

&НаСервереБезКонтекста
Процедура ОбновитьОсновнуюФормуСервер()

	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
		"Обработка.ОперацииЗакрытияМесяца",
		"ОсновнаяФорма",
		"Обработка.ОперацииЗакрытияМесяца.Форма.ЗакрытиеМесяца");
...
&НаКлиенте
Процедура ОбработатьНажатиеГиперссылки(СтрокаДереваЭтапов, НажатоДействиеВыполнить = Истина, ТолькоОдинЭтап = Ложь)
	...
	Если ОписаниеДействия.ВидДействия = ПредопределенноеЗначение("Перечисление.ВидыДействийРасшифровкиОперацийЗакрытияМесяца.ОткрытьФорму") Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПараметрыФормы, "Отбор") Тогда
		...
		Если Не (ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПараметрыФормы, "Организация")
	...
	ИначеЕсли ОписаниеДействия.ВидДействия = ПредопределенноеЗначение("Перечисление.ВидыДействийРасшифровкиОперацийЗакрытияМесяца.ВыполнитьПроцедуруНаКлиенте") Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
			ПараметрыФормы, "ДополнительныеПараметры") Тогда
...
&НаСервере
Функция ПолучитьОписаниеВыбранногоДействияДляВыполненияНаКлиенте(ЭтоЭтап, КодСтроки, НажатоДействиеВыполнить = Истина, ТолькоОдинЭтап = Ложь)
	...
	Если ОписаниеДействия.ВидДействия = Перечисления.ВидыДействийРасшифровкиОперацийЗакрытияМесяца.ВыполнитьРасчет Тогда
		...
		Если ЗапуститьВыполнениеРасчета(КодСтроки, ТолькоОдинЭтап) Тогда
...
&НаСервере
Функция ЗапуститьВыполнениеРасчета(РассчитатьПоЭтап = Неопределено, ТолькоОдинЭтап = Ложь)
	...
	Если ТолькоОдинЭтап Тогда
		СостояниеРасчета.ЭтапыКРасчету.Очистить();
		СостояниеРасчета.ЭтапыКРасчету.Добавить(РассчитатьПоЭтап);
		СостояниеРасчета.КоличествоЭтапов = СостояниеРасчета.ЭтапыКРасчету.Количество();
		СостояниеРасчета.НомерТекущегоЭтапа = СостояниеРасчета.НомерПредыдущегоЭтапа + 1;
		РасчетЗапущен = ЗапуститьРасчетОдногоЭтапаВФоне();
	Иначе
...
&НаСервере
Функция ЗапуститьРасчетОдногоЭтапаВФоне()
	...
	Если СостояниеРасчета.НомерТекущегоЭтапа < СостояниеРасчета.КоличествоЭтапов - 1 Тогда
		...
		Если АктивныеРасчеты.ЕстьАктивныеРасчеты Тогда
			ОбщегоНазначения.СообщитьПользователю(АктивныеРасчеты.ТекстОшибки,,,, ПрерватьРасчет);
		КонецЕсли;
...
Процедура ДобавитьСтрокиДереваИзТаблицы(ТаблицаСтрок, ТаблицаСтрокРодителей, ИмяКолонкиПризнака, ПараметрыОтбора = Неопределено)
	...
	Для Каждого ТекущаяСтрока Из ИсточникДанных Цикл
		...
		Если СтрокаРодителя <> Неопределено Тогда // родитель может не выводиться - не надо выводить и подчиненные
			...
			НоваяСтрока.ВыполнениеНаСервере =
				ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущаяСтрока,"ДействиеВыполнить")
				И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущаяСтрока.ДействиеВыполнить,"НаСервере")
				И ТекущаяСтрока.ДействиеВыполнить.НаСервере;
...
&НаКлиенте
Процедура ОтобразитьДатуСостоянияЭтапов(ДатаСостояния)
	
	Статус = "";
	
	Если ДатаСостояния <> Дата(1,1,1) Тогда
		Статус = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Статус операций закрытия месяца актуален на %1.'"), Формат(ДатаСостояния, "ДЛФ=DDT"));
	КонецЕсли;
	
	ПредставлениеРассчитываемогоЭтапа = СтроковыеФункцииКлиент.ФорматированнаяСтрока(Статус);
...
&НаСервере
Функция ДатаАктуальностиДанных()
	...
	Если СтруктураДанных = Неопределено Тогда
		Возврат Дата(1,1,1);
	Иначе
		Возврат СтруктураДанных.ДатаАктуальностиДанных;
	КонецЕсли;
