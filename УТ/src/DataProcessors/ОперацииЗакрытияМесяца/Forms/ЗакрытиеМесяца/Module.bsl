...
&НаКлиенте
Процедура ОткрытьФормуЗакрытиеМесяца25(Команда)
...
Процедура УстановитьУсловноеОформление()
	...
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("Шрифт", ШрифтыСтиля.ГруппаВариантовОтчетовВажнаяШрифт);
...
Процедура ЗаполнитьПромежуточныеПоясненияЭтапа(РассчитываемыйЭтап, СтруктураДанных, РасчетВыполняется = Ложь)
	...
	ПараметрыОбработчика = Обработки.ОперацииЗакрытияМесяца.ИнициализироватьПараметрыОбработчикаЭтапа(
		...
		"ЗакрытиеМесяца.ОбновлениеСостояния." + Строка(СтрокаТаблицыЭтапов.Код));
