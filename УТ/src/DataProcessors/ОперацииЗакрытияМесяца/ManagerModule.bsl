...
// Инициализирует структуру параметров обработчика этапа.
// 
// Параметры:
...
// * РежимЗакрытияМесяца - Произвольный, Неопределено -
...
//
// Параметры:
//  ОбновлениеИБ - Булево - При передаче значения Истина вернет все операции, вне зависимости от установленных значений
//                          функциональных опций. Использвется для обновления справочника ОперацииЗакрытияМесяца.
// 
// Возвращаемое значение:
// см. Обработки.ОперацииЗакрытияМесяца.ИнициализироватьТаблицуОписанияЭтапов
//
Функция ЗаполнитьОписаниеЭтаповЗакрытияМесяца(ОбновлениеИБ = Ложь) Экспорт
	...
	Если Не ОбновлениеИБ Тогда
		Для Каждого Этап Из ТаблицаЭтапов Цикл
			Если Этап.ОтключеноПоФО Тогда
				ТаблицаЭтапов.Удалить(Этап);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
// * Код - СправочникСсылка.ОперацииЗакрытияМесяца -
// * Наименование - Строка -
// * МультиязычноеНаименование - Строка -
// * Родитель - Строка -
// * НомерЭтапа - Число -
// * ВыполняетсяВручную - Булево -
// * Информационный - Булево -
// * ОтображатьВыполненныеУспешно - Булево -
// * ПредшествующиеЭтапы - Массив из СправочникСсылка.ОперацииЗакрытияМесяца -
// * ЗависитОтЭтапов - Массив из СправочникСсылка.ОперацииЗакрытияМесяца -
// * ВлияетНаЭтапы - Массив из СправочникСсылка.ОперацииЗакрытияМесяца -
// * ОбновитьСостояниеЭтапов - Массив из СправочникСсылка.ОперацииЗакрытияМесяца -
...
// * МеждународныйУчет - Булево -
// * Отключено - Булево -
// * ОтключеноПоФО - Булево - Этап скрыт функциональной опцией
// * Видимость - Булево -
...
Функция ИнициализироватьТаблицуОписанияЭтапов() Экспорт
	...
	ТаблицаЭтапов.Колонки.Добавить("Код",		   		      Новый ОписаниеТипов("СправочникСсылка.ОперацииЗакрытияМесяца"));
	ТаблицаЭтапов.Колонки.Добавить("Наименование", 		      Новый ОписаниеТипов("Строка"));
	ТаблицаЭтапов.Колонки.Добавить("МультиязычноеНаименование", Новый ОписаниеТипов("Строка"));
	ТаблицаЭтапов.Колонки.Добавить("МультиязычныйКомментарий", Новый ОписаниеТипов("Строка"));
	...
	ТаблицаЭтапов.Колонки.Добавить("СледуетЗаЭтапом",		  Новый ОписаниеТипов("СправочникСсылка.ОперацииЗакрытияМесяца")); // для упорядочивания - предыдущий этап в дереве этапов
	...
	ТаблицаЭтапов.Колонки.Добавить("СостояниеИзКеша",
		Новый ОписаниеТипов("ПеречислениеСсылка.СостоянияОперацийЗакрытияМесяца")); // состояние операции закрытия месяца из регистра сведений "СостоянияЭтаповЗакрытияМесяца"
	...
	ТаблицаЭтапов.Колонки.Добавить("ОтключеноПоФО",           Новый ОписаниеТипов("Булево"));
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если ВидФормы = "Форма" Тогда
		ФормаИзНастроек = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
			"Обработка.ОперацииЗакрытияМесяца",
			"ОсновнаяФорма",
			"Обработка.ОперацииЗакрытияМесяца.Форма.ЗакрытиеМесяца25");

		Если Не ФормаИзНастроек = Неопределено Тогда
			ВыбраннаяФорма = ФормаИзНастроек;
...
Функция ИнициализироватьТаблицуОписанияГруппЭтапов() Экспорт
	...
	ТаблицаГруппЭтапов.Колонки.Добавить("МеждународноеНаименование", Новый ОписаниеТипов("Строка"));
	...
	Для Каждого СтрокаГруппы Из ТаблицаГруппЭтапов Цикл
		...
		Пока КоличествоЭтапов > 0 Цикл
			...
			Если ЭтапыГруппыСПорядком.Количество() = КоличествоЭтапов Тогда
				...
				Для Каждого ТекущийЭтап Из ЭтапыГруппыСПорядком Цикл
					ТекстОшибки = ТекстОшибки + Символы.ПС + Справочники.ОперацииЗакрытияМесяца.ПолучитьИмяПредопределенного(ТекущийЭтап.Код)
				КонецЦикла;
...
// * Состояние - СправочникСсылка.ОперацииЗакрытияМесяца -
// * Период - Дата -
...
Функция ИнициализироватьТаблицуПоясненийКЭтапам() Экспорт
	...
	ТаблицаПояснений.Колонки.Добавить("Родитель", 	   		  Новый ОписаниеТипов("СправочникСсылка.ОперацииЗакрытияМесяца")); // этап-родитель
...
Функция ИнициализироватьСтруктуруПараметрыРасчета(Период, КонецПериода = Неопределено, МассивОрганизаций = Неопределено, РежимЗакрытияМесяца = Неопределено) Экспорт
	...
	ПараметрыРасчета.Вставить("ТолькоОдинЭтап", 	       Ложь);
...
// ОбновитьТолькоОдинЭтап - СправочникСсылка.ОперацииЗакрытияМесяца -
...
// * ОбновляемыеЭтапы - Массив из СправочникСсылка.ОперацииЗакрытияМесяца -
// * ОбновитьТолькоОдинЭтап - СправочникСсылка.ОперацииЗакрытияМесяца -
...
Функция ИнициализироватьПараметрыЗапускаДляЗаполненияСостоянияЭтапов(СтруктураДанных, АдресХранилища = "",
	...
	ПараметрыЗапуска.Вставить("ОбновитьКэшСтатусовОпераций", Истина);
...
Процедура ЗаполнитьСостоянияЭтапов(ПараметрыЗапуска) Экспорт
	...
	// Последовательный запуск проверок этапов и проверок учета в системе
	
	ВсегоЭтапов = СтрокиЭтапов.Количество();
	Счетчик = 1;
	
	// Получение данных предыдущего анализа состояния закрытия месяца
	
	СостоянияЭтаповИзКеша = Новый Структура;
	СостоянияЭтаповИзКеша.Вставить("СостоянияЭтапов", Новый Соответствие());
	СостоянияЭтаповИзКеша.Вставить("ДатаАктуальностиДанных", ПараметрыЗапуска.СтруктураДанных.ДатаНачалаРасчета);
		
	ПрочитатьРезультатАнализаЭтапов(ПараметрыЗапуска, СостоянияЭтаповИзКеша);
	
	ТребуетсяОбновлениеКеша = Ложь;
	
	Если ПараметрыЗапуска.ОбновитьКэшСтатусовОпераций Тогда
		СостоянияЭтаповИзКеша.Вставить("СостоянияЭтапов", Новый Соответствие());
		ТребуетсяОбновлениеКеша = Истина;
	Иначе
		Для Каждого ТекущаяСтрока Из СтрокиЭтапов Цикл
			Если СостоянияЭтаповИзКеша.СостоянияЭтапов.Получить(ТекущаяСтрока.Код) = Неопределено Тогда
				ТребуетсяОбновлениеКеша = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	...
	Для Каждого ТекущаяСтрока Из СтрокиЭтапов Цикл
		...
		Если Не ТребуетсяОбновлениеКеша Тогда
			ТекущаяСтрока.СостояниеИзКеша = СостоянияЭтаповИзКеша.СостоянияЭтапов.Получить(ТекущаяСтрока.Код).Состояние;
		КонецЕсли;
		...
		ДлительныеОперации.СообщитьПрогресс(Цел((Счетчик/ВсегоЭтапов)*100),,
			Новый Структура("ОбновитьКэшСтатусовОпераций, ДатаАктуальностиДанных",
				ПараметрыЗапуска.ОбновитьКэшСтатусовОпераций,
				СостоянияЭтаповИзКеша.ДатаАктуальностиДанных));
		Счетчик = Счетчик + 1;
		
	КонецЦикла;
	
	Если ТребуетсяОбновлениеКеша Тогда
		СохранитьРезультатАнализаЭтапов(ПараметрыЗапуска);
		СтруктураДанных.Удалить("СостоянияЭтаповИзКэша");
	ИначеЕсли СтрокиЭтапов.Количество() Тогда
		СтруктураДанных.Вставить("СостоянияЭтаповИзКэша");
	КонецЕсли;
		
	СтруктураДанных.Вставить("ДатаАктуальностиДанных", СостоянияЭтаповИзКеша.ДатаАктуальностиДанных);
...
Процедура ОбработатьЭтап(ЭлементКоллекции, ПараметрыЗапуска, СтруктураДанных, ОписаниеОбщегоЗамераПроизводительности, ТекстДлительностьЭтапов) Экспорт
	...
	ПараметрыОбработчика = ИнициализироватьПараметрыОбработчикаЭтапа(
		...
		ТекущаяСтрока,
		ПараметрыЗапуска,
		"ЗакрытиеМесяца.ОбновлениеСостояния." + Справочники.ОперацииЗакрытияМесяца.ПолучитьИмяПредопределенного(ТекущаяСтрока.Код));
	...
	Иначе
		...
		// Заполнение состояния этапа из кэша.		
		Если ЗначениеЗаполнено(ТекущаяСтрока.СостояниеИзКеша) Тогда
			ТекущаяСтрока.Состояние = ТекущаяСтрока.СостояниеИзКеша;
			ЗаполнитьЗначенияСвойств(ТекущаяСтрока, ПараметрыОбработчика.ДанныеЭтапа,
				"ДатаНачалаРасчета, МассивОрганизаций, ДополнительныеСвойства");
		Иначе
			ПроверитьИспользованиеЭтапа(ПараметрыОбработчика);
			ЗаполнитьЗначенияСвойств(ТекущаяСтрока, ПараметрыОбработчика.ДанныеЭтапа,
				"Состояние, ДатаНачалаРасчета, МассивОрганизаций, ДополнительныеСвойства");
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.СостояниеИзКеша) Тогда
			ТекущаяСтрока.СостояниеИзКеша = Перечисления.СостоянияОперацийЗакрытияМесяца.ПустаяСсылка();
		Иначе
			Если ПараметрыЗапуска.УчетСостоянияПредшествующихЭтапов Тогда
...
Процедура ЗаполнитьСправочникПроверокСостоянияСистемы(ПараметрыЗапуска, ОписаниеОбщегоЗамераПроизводительности)
	...
	ТипКонтекстаВыполнения = Тип("СправочникСсылка.ОперацииЗакрытияМесяца");
...
Процедура ВыполнитьПроверкиЭтапа(СтруктураДанных, ТекущаяСтрока, ПараметрыОбработчика, ВыполнятьПроверкиДоЭтапа, ВыполнятьПроверкиПослеЭтапа)
	...
	ДанныеИзКэша = ЗначениеЗаполнено(ТекущаяСтрока.СостояниеИзКеша);
	
	Для НомерПопытки = 1 По ПараметрыЗакрытияМесяца.КоличествоПопытокВыполненияПроверки Цикл
		
		Попытка
			
			Если Не ДанныеИзКэша Тогда
...
Процедура СохранитьРезультатАнализаЭтапов(ПараметрыЗапуска)
	
	Организации = ПараметрыЗапуска.СтруктураДанных.ПараметрыРасчета.МассивОрганизаций;
	Месяц = ПараметрыЗапуска.СтруктураДанных.ДатаНачалаРасчета;
	
	ТаблицаЭтапы = ПараметрыЗапуска.СтруктураДанных.ТаблицаЭтапы;
	
	Для Каждого Организация Из Организации Цикл 
		Блокировка = Новый БлокировкаДанных();
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.СостоянияОперацийЗакрытияМесяца");
		ЭлементБлокировки.УстановитьЗначение("Организация", Организация);

		НачатьТранзакцию();
		Попытка
			Блокировка.Заблокировать();
			Записи = РегистрыСведений.СостоянияОперацийЗакрытияМесяца.СоздатьНаборЗаписей();
			Записи.Отбор.Организация.Установить(Организация);
			Записи.Отбор.Период.Установить(Месяц);
			Для Каждого Этап Из ТаблицаЭтапы Цикл
				Если Этап.Состояние <> Перечисления.СостоянияОперацийЗакрытияМесяца.Неопределено Тогда
					Запись = Записи.Добавить();
					Запись.ОперацияЗакрытияМесяца = Этап.Код;
					Запись.Организация = Организация;
					Запись.Период = Месяц;
					Запись.ДатаАнализа = ТекущаяДатаСеанса();
					Запись.СостояниеЭтапа = Этап.Состояние;
				КонецЕсли;
			КонецЦикла;
			Записи.Записать(РежимЗамещения.Замещение);
...
Процедура ПрочитатьРезультатАнализаЭтапов(ПараметрыЗапуска, СостоянияЭтаповИзКеша)
	
	ДатаАктуальностиДанных = ТекущаяДатаСеанса();
	
	Организации = ПараметрыЗапуска.СтруктураДанных.ПараметрыРасчета.МассивОрганизаций;;
	Период = ПараметрыЗапуска.СтруктураДанных.ДатаНачалаРасчета;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СостоянияОперацийЗакрытияМесяца.ОперацияЗакрытияМесяца КАК ОперацияЗакрытияМесяца,
		|	СостоянияОперацийЗакрытияМесяца.СостояниеЭтапа КАК СостояниеЭтапа,
		|	СостоянияОперацийЗакрытияМесяца.ДатаАнализа КАК ДатаАнализа,
		|	СостоянияОперацийЗакрытияМесяца.Организация КАК Организация
		|ИЗ
		|	РегистрСведений.СостоянияОперацийЗакрытияМесяца КАК СостоянияОперацийЗакрытияМесяца
		|ГДЕ
		|	СостоянияОперацийЗакрытияМесяца.Период = &Период
		|	И СостоянияОперацийЗакрытияМесяца.Организация В(&Организации)";
	...
	Пока РезультатЗапроса.Следующий() Цикл
		...
		Этап = СостоянияЭтапов.Получить(РезультатЗапроса.ОперацияЗакрытияМесяца);

		Если Этап = Неопределено Тогда
			Этап = Новый Структура("Состояние, ДатаАнализа",
				РезультатЗапроса.СостояниеЭтапа, РезультатЗапроса.ДатаАнализа);
				
			СостоянияЭтапов.Вставить(РезультатЗапроса.ОперацияЗакрытияМесяца, Этап);
		Иначе
			УстановитьСостояниеЭтапа(Этап.Состояние, РезультатЗапроса.СостояниеЭтапа);
		КонецЕсли;
		
		ДатаАктуальностиДанных = Мин(ДатаАктуальностиДанных, РезультатЗапроса.ДатаАнализа);
		
	КонецЦикла;
		
	СостоянияЭтаповИзКеша.Вставить("СостоянияЭтапов", СостоянияЭтапов);
	СостоянияЭтаповИзКеша.Вставить("ДатаАктуальностиДанных", ДатаАктуальностиДанных);

КонецПроцедуры

Процедура УстановитьСостояниеЭтапа(ТекущееСостояние, НовоеСостояние)
	
	ПриоритетыСостояний = Новый Соответствие;
	ПриоритетыСостояний.Вставить(Перечисления.СостоянияОперацийЗакрытияМесяца.Неопределено, 1);
	ПриоритетыСостояний.Вставить(Перечисления.СостоянияОперацийЗакрытияМесяца.НеТребуется, 2);
	ПриоритетыСостояний.Вставить(Перечисления.СостоянияОперацийЗакрытияМесяца.ВыполненоУспешно, 3);
	ПриоритетыСостояний.Вставить(Перечисления.СостоянияОперацийЗакрытияМесяца.ОжидаетВыполненияПредыдущихОпераций, 4);
	ПриоритетыСостояний.Вставить(Перечисления.СостоянияОперацийЗакрытияМесяца.НеВыполнено, 5);
	ПриоритетыСостояний.Вставить(Перечисления.СостоянияОперацийЗакрытияМесяца.ВыполненоСОшибками, 6);
	
	Если ПриоритетыСостояний.Получить(ТекущееСостояние) < ПриоритетыСостояний.Получить(НовоеСостояние) Тогда
