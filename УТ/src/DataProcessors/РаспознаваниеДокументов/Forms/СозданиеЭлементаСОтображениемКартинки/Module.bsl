...
Процедура СтавкаНДСНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОтправитьОповещениеВОсновнуюФорму("СтавкаНДС");
...
Процедура Создать(Команда)
	...
	ЗаполнитьРеквизитыСоздаваемогоОбъектаЗначениямиРеквизитовФормы();
...
Процедура ПоказатьВсеРеквизиты(Команда)
	...
	Если Не ЗначениеЗаполнено(ПараметрыОткрытияФормыОбъекта) Тогда
		Возврат;
	КонецЕсли;
	
	ЗаполнитьРеквизитыСоздаваемогоОбъектаЗначениямиРеквизитовФормы(Истина);
	
	ПараметрыСоздаваемогоОбъекта = Новый Структура;
	Для Каждого Строка Из РеквизитыСоздаваемогоОбъекта Цикл
    	ПараметрыСоздаваемогоОбъекта.Вставить(Строка.Имя, Строка.Значение);
	КонецЦикла;
	ПараметрыОткрытияФормыОбъекта.ДополнительныеДанные.Вставить("ПараметрыОбъекта", ПараметрыСоздаваемогоОбъекта);
	
	Закрыть();
	
	ИмяФормыСозданияЭлемента = "";
	РаспознаваниеДокументовКлиент.ИмяФормыСозданияЭлемента(ПараметрыОткрытияФормыОбъекта.ИмяЭлемента, 
																			ИмяФормыСозданияЭлемента, 
																			ПараметрыОткрытияФормыОбъекта);
	ОткрытьФорму(ИмяФормыСозданияЭлемента, ПараметрыОткрытияФормыОбъекта, ВладелецФормы, , , , ,
...
Процедура ОтобразитьРеквизиты()
	...
	ИначеЕсли ИмяСправочника = "БанковскиеСчетаКонтрагентов" Или ИмяСправочника = "БанковскиеСчетаОрганизаций"
		Или ИмяСправочника = "КлассификаторБанков" Тогда
	...
	Для Каждого РеквизитСоздаваемогоОбъекта Из РеквизитыСоздаваемогоОбъекта Цикл
		...
		Если ИмяСправочника = "Контрагенты"
			...
			Или ИмяСправочника = "КлассификаторБанков"
...
&НаСервере
Процедура УстановитьБанкПоБИК(БИК)
	
	Если ЗначениеЗаполнено(БИК) Тогда
		Банк = СсылкаНаБанк(БИК);
	КонецЕсли;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция СсылкаНаБанк(БИК)
...
Функция РеквизитыСоздаваемогоОбъекта(ПараметрыОткрытия)
	...
	Для Каждого РеквизитЭлемента Из СвязанныеКолонки Цикл
		...
		ИначеЕсли ПараметрыРеквизита.Имя = "ХозяйственнаяОперация" Тогда
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДополнительныеДанные, "ХозяйственнаяОперация") Тогда
				ПараметрыРеквизита.Значение = ДополнительныеДанные.ХозяйственнаяОперация;
			Иначе
				ПараметрыРеквизита.Значение = Неопределено;
			КонецЕсли;
		КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьРеквизитыСоздаваемогоОбъектаЗначениямиРеквизитовФормы(ВидКонтрагентаСтрокой = Ложь)
	...
	Для Каждого РеквизитСоздаваемогоОбъекта Из РеквизитыСоздаваемогоОбъекта Цикл
		...
		ИначеЕсли РеквизитСоздаваемогоОбъекта.Имя = "ЮридическоеФизическоеЛицо" Тогда
			...
			Если ВидКонтрагентаСтрокой Тогда
				Значение = ВидКонтрагента;	
			Иначе
				Значение = ?(ВидКонтрагента = "ФизическоеЛицо",
			...
			РеквизитСоздаваемогоОбъекта.Значение = Значение;
...
КонецПроцедуры // ЗаполнитьРеквизитыСоздаваемогоОбъектаЗначениямиРеквизитовФормы()
