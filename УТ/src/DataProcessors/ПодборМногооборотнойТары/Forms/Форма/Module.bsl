...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоАдресВременногоХранилища(Параметры.АдресТоваровВоВременномХранилище) Тогда
		...
		ПоказыватьДатуПоступления = Параметры.ПоказыватьДатуПоступления;
		
		Если ПоказыватьДатуПоступления Тогда
			Элементы.ТараДата.Заголовок = НСтр("ru = 'Дата отгрузки'");
		КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТараДатаПоступления.Имя);

	ГруппаОтбора1 = Элемент.Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ГруппаОтбора1.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;

	ОтборЭлемента = ГруппаОтбора1.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПоказыватьДатуПоступления");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Ложь;

	ОтборЭлемента = ГруппаОтбора1.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Тара.Дата");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Новый ПолеКомпоновкиДанных("Тара.ДатаПоступления");
...
Процедура ЗаполнитьДеревоМногооборотнойТары()
	...
	|	ТоварыДокумента.ДатаПоступления    КАК ДатаПоступления
	|ПОМЕСТИТЬ
	|	ВтТоварыДокумента
	|ИЗ
	...
	|	ВтТоварыДокумента.ДатаПоступления                               КАК ДатаПоступления,
	...
	|	втТара.ДатаПоступления                                                     КАК ДатаПоступления,
	...
	|	ВтТоварыДокумента.ДатаПоступления
	|УПОРЯДОЧИТЬ ПО
	...
	Товары.Свернуть("Дата, ДатаПоступления, Номенклатура, Характеристика, Склад", "Количество");
	ТоварыСУпаковкойВсей = Новый ТаблицаЗначений();
	ТоварыСУпаковкойВсей = 	ТоварыДокумента.Выгрузить();
	ТоварыСУпаковкойВсей.Свернуть("Дата, ДатаПоступления, Номенклатура, Характеристика,Упаковка,Склад", "Количество");	
	ТоварыСУпаковкой = Новый ТаблицаЗначений(); 
	ТоварыСУпаковкой.Колонки.Добавить("Дата");
	ТоварыСУпаковкой.Колонки.Добавить("ДатаПоступления");
	...
	Если Не РезультатЗапроса[2].Пустой() Тогда
			...
			Пока ВыборкаХарактеристики.Следующий() Цикл
				...
				Пока ВыборкаСклады.Следующий() Цикл
					...
					Пока ВыборкаДаты.Следующий() Цикл
						...
						ВыборкаДатыПоступления = ВыборкаДаты.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
						
						Пока ВыборкаДатыПоступления.Следующий() Цикл
						
							ВыборкаУпаковки = ВыборкаДатыПоступления.Выбрать();
							...
							ТаблицаУпаковок.Колонки.Добавить("ДатаПоступления");
		...
		Для Каждого ТекСтрока Из ТараДокумента Цикл
			...
			Если ТекСтрока.НачальноеКоличествоТары = ТекСтрока.КоличествоТары Тогда
				...
				НоваяСтрока.ДатаПоступления = ТекСтрока.ДатаПоступления;
...
Процедура ЗаполнитьКоличествоТарыИзДокумента(ТараДокумента, СтрокаДерева, ВсяТараПодобрана, ТоварыСУпаковкой)
	...
	ПараметрыОтбора.Вставить("ДатаПоступления", СтрокаДерева.ДатаПоступления);
...
Функция ПоместитьТаруВХранилище()
	...
	ТаблицаТары.Колонки.Добавить("ДатаПоступления");
		
	ДобавитьПодобраннуюТаруВТаблицу(ДеревоТара.Строки, ТаблицаТары);
		
	ТаблицаТары.Свернуть("Номенклатура,Характеристика,Склад,Дата,ДатаПоступления", "Количество");
