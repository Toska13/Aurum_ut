...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ Локализация
	ТекстЗапросаТТН = ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении() +
	"ВЫБРАТЬ
	|	ЕСТЬNULL(СостоянияОригиналов.ОбщееСостояние, НЕОПРЕДЕЛЕНО) КАК ОбщееСостояние,
	|	ЕСТЬNULL(СостоянияОригиналов.АвторИзменения, """") КАК АвторИзменения,
	|	ЕСТЬNULL(СостоянияОригиналов.ДатаПоследнегоИзменения, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаПоследнегоИзменения,
	|	ТранспортнаяНакладная.Дата КАК ДатаДокументаИБ,
	|	ТранспортнаяНакладная.Номер КАК НомерДокументаИБ,
	|	ТранспортнаяНакладная.Организация КАК Организация,
	|	ТранспортнаяНакладная.Грузополучатель КАК Контрагент,
	|	NULL КАК ДатаПервичногоДокумента,
	|	NULL КАК НомерПервичногоДокумента,
	|	ТранспортнаяНакладная.Ссылка КАК Ссылка,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СостоянияОригиналов.ОбщееСостояние, ИСТИНА)
	|			ТОГДА ТИПЗНАЧЕНИЯ(ТранспортнаяНакладная.Ссылка)
	|		ИНАЧЕ СТРОКА("""")
	|	КОНЕЦ КАК ВидДокумента,
	|	ТаблицаИдентификаторов.Ссылка КАК ТипСсылки,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СостоянияОригиналов.ОбщееСостояние, ИСТИНА)
	|			ТОГДА 0
	|		КОГДА ЕСТЬNULL(СостоянияОригиналов.ОбщееСостояние, ЛОЖЬ) = ЛОЖЬ
	|		И ЕСТЬNULL(СостоянияОригиналов.ФормаИзвне, ЛОЖЬ) = ИСТИНА
	|			ТОГДА 2
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК ПолеКартинки,
	|	ЕСТЬNULL(СостоянияОригиналов.ПервичныйДокумент, """") КАК ПервичныйДокумент,
	|	ТранспортнаяНакладная.Грузополучатель КАК Партнер,
	|	ЕСТЬNULL(СостоянияОригиналов.ФормаИзвне, ЛОЖЬ) КАК Извне,
	|	NULL КАК Сумма,
	|	NULL КАК Валюта,
	|	ТранспортнаяНакладная.Грузополучатель.ИНН КАК КонтрагентИНН,
	|	ЕСТЬNULL(СостоянияОригиналов.Состояние, ЗНАЧЕНИЕ(Справочник.СостоянияОригиналовПервичныхДокументов.ПустаяСсылка)) КАК
	|		СостояниеОригиналаПервичногоДокумента,
	|	0 КАК ПолеОтступа,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СостоянияОригиналов.Состояние,
	|			НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Справочник.СостоянияОригиналовПервичныхДокументов.ОригиналПолучен)
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СостояниеОригиналПолучен,
	|	ЕСТЬNULL(СостоянияОригиналов.ПервичныйДокументПредставление, """") КАК ПервичныйДокументПредставление
	|ИЗ
	|	Документ.ТранспортнаяНакладная КАК ТранспортнаяНакладная
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияОригиналовПервичныхДокументов КАК СостоянияОригиналов
	|		ПО ТранспортнаяНакладная.Ссылка = СостоянияОригиналов.Владелец
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ИдентификаторыОбъектовМетаданных КАК ТаблицаИдентификаторов
	|		ПО ТаблицаИдентификаторов.ПолноеИмя = &ИмяДокументаТранспортнаяНакладная";
	
	СписокДокументов.ТекстЗапроса = СписокДокументов.ТекстЗапроса + ТекстЗапросаТТН;
	
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(
		СписокДокументов.КомпоновщикНастроек.Настройки,
		"ИмяДокументаТранспортнаяНакладная",
		Метаданные.Документы.ТранспортнаяНакладная.ПолноеИмя());
	//-- Локализация
	
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(
		СписокДокументов.КомпоновщикНастроек.Настройки,
		"ТекстДоговорСКонтрагентом",
...
Процедура ОтборСостояниеПриИзменении(Элемент)
	...
	УстановитьОтборПоСостояниюОригинала(ОтборСостояние);
