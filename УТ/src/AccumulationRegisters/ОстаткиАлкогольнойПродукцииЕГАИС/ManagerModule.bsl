...
//     ** ТаблицаОстаткиАлкогольнойПродукцииЕГАИС - ТаблицаЗначений - таблица движений по регистру
...
Функция Справки2ВНаличии(ОрганизацияЕГАИС, АлкогольнаяПродукция) Экспорт
	...
	Если ЗначениеЗаполнено(ОрганизацияЕГАИС) Тогда
		...
		|		, ОрганизацияЕГАИС = &ОрганизацияЕГАИС И АлкогольнаяПродукция = &АлкогольнаяПродукция) КАК Остатки
		|ГДЕ
		|	Остатки.СвободныйОстатокОстаток > 0";
	Иначе
		Запрос.Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	Остатки.Справка2 КАК Справка2
		|ИЗ
		|	РегистрНакопления.ОстаткиАлкогольнойПродукцииЕГАИС.Остатки(
		|		, АлкогольнаяПродукция = &АлкогольнаяПродукция) КАК Остатки
		|ГДЕ
		|	Остатки.СвободныйОстатокОстаток > 0";
	КонецЕсли;

	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Справка2");
