...
Процедура ПриОкончанииРедактированияНабора(Форма, ИмяТЧ = "Товары", Параметры) Экспорт
	...
	Если Колонки.Найти("ДокументРеализации") <> Неопределено Тогда
	...
	ТЧДляПроверки = ТЧ.Выгрузить(Новый Массив);	//ТаблицаЗначений

	//++ Локализация
	ЕстьКоличествоПоРНПТ = ТЧДляПроверки.Колонки.Найти("КоличествоПоРНПТ") <> Неопределено;
	//-- Локализация

	ЕстьЗаказКлиента = ТЧДляПроверки.Колонки.Найти("ЗаказКлиента") <> Неопределено;
	Если ЕстьЗаказКлиента Тогда
		Отбор.Вставить("ЗаказКлиента", Данные.ЗаказКлиента);
	КонецЕсли;
	СтрокиКомплекта = ТЧ.НайтиСтроки(Отбор);

	ТребуетсяОпределениеПараметровСерийИзмененныхСтрок = Ложь;
	...
	Если СтруктураДействийСИзмененнымиСтроками.Свойство("ПроверитьСериюРассчитатьСтатус", ПараметрыСерий) Тогда
		ТребуетсяОпределениеПараметровСерийИзмененныхСтрок = ПараметрыСерий = Неопределено;
	КонецЕсли;
	ТребуетсяОпределениеПараметровСерийДобавленныхСтрок = Ложь;
	ПараметрыСерий = Неопределено;
	Если СтруктураДействийСДобавленнымиСтроками.Свойство("ПроверитьСериюРассчитатьСтатус", ПараметрыСерий) Тогда
		ТребуетсяОпределениеПараметровСерийДобавленныхСтрок = ПараметрыСерий = Неопределено;
	КонецЕсли;
	ДобавляемыеСтрокиЗаказ = Новый Массив;
	ДобавляемыеСтрокиРеализация = Новый Массив;	
	ИзменяемыеСтрокиЗаказ = Новый Массив;
	ИзменяемыеСтрокиЗаказСПересчетомСкидок = Новый Массив;
	ИзменяемыеСтрокиРеализация = Новый Массив;	
	ИзменяемыеСтрокиРеализацияСПересчетомСкидок = Новый Массив;
	...
	Для Каждого СтрокаТЧ Из Данные.Комплектующие Цикл
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			...
			ЦелевойМассив = ИзменяемыеСтрокиЗаказ;
			
			Если ТребуетсяОпределениеПараметровСерийИзмененныхСтрок Тогда
				ПараметрыУказанияСерий = ОпределитьПараметрыСерийПомощникаПродаж(Форма.ПараметрыУказанияСерий, ТекущаяСтрока);
				Если ПараметрыУказанияСерий = Форма.ПараметрыУказанияСерий.Реализация Тогда
					ЦелевойМассив = ИзменяемыеСтрокиРеализация;
				КонецЕсли;
			КонецЕсли;
			
			Если СтруктураДействийСИзмененнымиСтроками.Свойство("ПересчитатьСуммуСУчетомАвтоматическойСкидки")
				И ОчищатьАвтоматическиеСкидки Тогда
				Если ЦелевойМассив = ИзменяемыеСтрокиЗаказ Тогда
					ЦелевойМассив = ИзменяемыеСтрокиЗаказСПересчетомСкидок;
				ИначеЕсли ЦелевойМассив = ИзменяемыеСтрокиРеализация Тогда
					ЦелевойМассив = ИзменяемыеСтрокиРеализацияСПересчетомСкидок;
				КонецЕсли;
			КонецЕсли;
		...
		Иначе
			...
			ЦелевойМассив = ДобавляемыеСтрокиЗаказ;

			Если ТребуетсяОпределениеПараметровСерийДобавленныхСтрок Тогда
				ПараметрыУказанияСерий = ОпределитьПараметрыСерийПомощникаПродаж(Форма.ПараметрыУказанияСерий, НоваяСтрока);
				Если ПараметрыУказанияСерий = Форма.ПараметрыУказанияСерий.Реализация Тогда
					ЦелевойМассив = ДобавляемыеСтрокиРеализация;
				КонецЕсли;
			...
			ЦелевойМассив.Добавить(НоваяСтрока);
			
		КонецЕсли;
		
	КонецЦикла;

	Если ИзменяемыеСтрокиЗаказ.Количество() Тогда
		Если ТребуетсяОпределениеПараметровСерийИзмененныхСтрок Тогда
			ПараметрыУказанияСерий = ОпределитьПараметрыСерийПомощникаПродаж(Форма.ПараметрыУказанияСерий, ИзменяемыеСтрокиЗаказ[0]);
		...
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			ИзменяемыеСтрокиЗаказ,
			СтруктураДействийСИзмененнымиСтроками,
			ТЧ,
			КэшированныеЗначения);
	КонецЕсли;

	Если ИзменяемыеСтрокиРеализация.Количество() Тогда
		Если ТребуетсяОпределениеПараметровСерийИзмененныхСтрок Тогда
			ПараметрыУказанияСерий = ОпределитьПараметрыСерийПомощникаПродаж(Форма.ПараметрыУказанияСерий, ИзменяемыеСтрокиРеализация[0]);
			СтруктураДействийСИзмененнымиСтроками.ПроверитьСериюРассчитатьСтатус = Новый Структура("ПараметрыУказанияСерий", ПараметрыУказанияСерий);
		КонецЕсли;
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			ИзменяемыеСтрокиРеализация,
			СтруктураДействийСИзмененнымиСтроками,
			ТЧ,
			КэшированныеЗначения);
	КонецЕсли;
	...
	Если ИзменяемыеСтрокиЗаказ.Количество() Тогда
		...
		Если СтруктураДействийСИзмененнымиСтроками.Свойство("ПересчитатьСуммуСУчетомАвтоматическойСкидки") Тогда
			СтруктураДействийСИзмененнымиСтроками.ПересчитатьСуммуСУчетомАвтоматическойСкидки.Вставить("Очищать", Истина);
		КонецЕсли;
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			ИзменяемыеСтрокиЗаказ,
			СтруктураДействийСИзмененнымиСтроками,
	...
	Если ДобавляемыеСтрокиЗаказ.Количество() Тогда
		Если ТребуетсяОпределениеПараметровСерийДобавленныхСтрок Тогда
			ПараметрыУказанияСерий = ОпределитьПараметрыСерийПомощникаПродаж(Форма.ПараметрыУказанияСерий, ДобавляемыеСтрокиЗаказ[0]);
			СтруктураДействийСДобавленнымиСтроками.ПроверитьСериюРассчитатьСтатус = Новый Структура("ПараметрыУказанияСерий", ПараметрыУказанияСерий);
		КонецЕсли;
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			ДобавляемыеСтрокиЗаказ,
			СтруктураДействийСДобавленнымиСтроками,
			ТЧ,
			КэшированныеЗначения);
	КонецЕсли;

	Если ДобавляемыеСтрокиРеализация.Количество() Тогда
		Если ТребуетсяОпределениеПараметровСерийДобавленныхСтрок Тогда
			ПараметрыУказанияСерий = ОпределитьПараметрыСерийПомощникаПродаж(Форма.ПараметрыУказанияСерий, ДобавляемыеСтрокиРеализация[0]);
			СтруктураДействийСДобавленнымиСтроками.ПроверитьСериюРассчитатьСтатус = Новый Структура("ПараметрыУказанияСерий", ПараметрыУказанияСерий);
		КонецЕсли;
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			ДобавляемыеСтрокиРеализация,
			СтруктураДействийСДобавленнымиСтроками,
			ТЧ,
			КэшированныеЗначения);
	КонецЕсли;
...
Функция АдресНабораВоВременномХранилище(Форма, Параметры, ИмяТЧ = "Товары") Экспорт
	...
	КонецЕсли;
	Если Параметры.Свойство("ЗаказКлиента") = Истина Тогда
		ПараметрыОтбора.Вставить("ЗаказКлиента", Параметры.ЗаказКлиента);
	КонецЕсли;
...
Функция ОпределитьПараметрыСерийПомощникаПродаж(ПараметрыУказанияСерий, ТекущиеДанные)
