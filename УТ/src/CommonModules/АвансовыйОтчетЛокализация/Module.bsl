...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АвансовыйОтчет1")
		...
		ЕстьПечатьАвансовогоОтчетаСтаройВерсии = Ложь;
		...
		Для Инд = 0 По РезультатЗапроса.ВГраница() Цикл
			...
			Если СтарыеАвансовыеОтчеты.Количество() Тогда
				УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
					...
					ЕстьПечатьАвансовогоОтчетаСтаройВерсии = Истина;
			КонецЕсли;
		КонецЦикла;
...
Процедура СгенерироватьСчетаФактуры(Объект, Отказ)
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПрочиеРасходы.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ВЫБОР КОГДА ПрочиеРасходы.ЭлектронныйБилет <> ЗНАЧЕНИЕ(Документ.ОперацияСЭлектроннымБилетом.ПустаяСсылка) ТОГДА
...
Функция ТекстЗапросаПечати(Запрос, ИспользуетсяРеглУчет)
	...
	|	
	|ВЫБРАТЬ
	|	ДанныеЭлектронногоБилета.АвансовыйОтчет,
	|	ДанныеЭлектронногоБилета.Ссылка,
	|	ДанныеЭлектронногоБилета.ВалютаРеглУчета,
	|	ПРЕДСТАВЛЕНИЕ(ДанныеЭлектронногоБилета.ТипУслуги) КАК ТипУслуги,
	...
	|	NULL КАК Контрагент,
	|	NULL КАК КонтрагентОплаты,
	|	ДанныеЭлектронногоБилета.Валюта,
	|	НЕОПРЕДЕЛЕНО КАК СчетУчета,
	|	СУММА(ДанныеЭлектронногоБилета.ПоУчету) КАК ПоУчету,
	|	СУММА(ДанныеЭлектронногоБилета.ПоУчетуВВалюте) КАК ПоУчетуВВалюте,
	|	СУММА(ДанныеЭлектронногоБилета.ПоОтчету) КАК ПоОтчету,
	|	СУММА(ДанныеЭлектронногоБилета.ПоОтчетуВВалюте) КАК ПоОтчетуВВалюте,
	|	0 КАК СуммаНДС,
	|	НЕОПРЕДЕЛЕНО КАК ПервичныйДокумент,
	|	0 КАК НомерРаздела,
	|	ДанныеЭлектронногоБилета.Номер КАК НомерСтроки,
	|	ДанныеЭлектронногоБилета.Номер КАК НомерСтрокиУпорядочивания
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ДенежныеСредстваУПодотчетныхЛиц КАК ДенежныеСредства
	|		ПО ДенежныеСредства.Регистратор = ОперацииБилета.Ссылка
	|		И ДанныеАвансовогоОтчета.ПодотчетноеЛицо = ДенежныеСредства.ПодотчетноеЛицо
	|	ГДЕ
	|		ДанныеДокумента.АвансовыйОтчет В (&МассивДокументов)
	|		И ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.БронированиеЧерезПодотчетноеЛицо)) КАК ДанныеЭлектронногоБилета
	|СГРУППИРОВАТЬ ПО
	|	ДанныеЭлектронногоБилета.АвансовыйОтчет,
	|	ДанныеЭлектронногоБилета.Ссылка,
	|	ДанныеЭлектронногоБилета.ВалютаРеглУчета,
	|	Представление(ДанныеЭлектронногоБилета.ТипУслуги),
	|	ДанныеЭлектронногоБилета.НомерБилета,
	|	ДанныеЭлектронногоБилета.Дата,
	|	ДанныеЭлектронногоБилета.Валюта,
	|	ДанныеЭлектронногоБилета.Номер
	|	
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерРаздела,
	...
	Если Не ИспользуетсяРеглУчет Тогда
		...
		|	ВЫБОР
		|		КОГДА НАЧАЛОПЕРИОДА(ОплатаПоставщикам.Ссылка.Дата, ДЕНЬ) = ОплатаПоставщикам.Ссылка.ДатаУтверждения
		|			ТОГДА ОплатаПоставщикам.Ссылка.Дата
		|		ИНАЧЕ ОплатаПоставщикам.Ссылка.ДатаУтверждения
		|	КОНЕЦ КАК Период,
		|	""60"" КАК СчетДт,
		...
		|	ПрочиеРасходы.Ссылка КАК ДокументДвижения,
		|	ВЫБОР
		|		КОГДА НАЧАЛОПЕРИОДА(ПрочиеРасходы.Ссылка.Дата, ДЕНЬ) = ПрочиеРасходы.Ссылка.ДатаУтверждения
		|			ТОГДА ПрочиеРасходы.Ссылка.Дата
		|		ИНАЧЕ ПрочиеРасходы.Ссылка.ДатаУтверждения
		|	КОНЕЦ КАК Период,
