...
Функция ПараметрыУчетногоМеханизма() Экспорт
	...
	Параметры.Вставить("ВыполнениеЗаданийНеоперативнойОчереди", Ложь);
...
Процедура ПередЗаписьюДокумента(Документ, РежимЗаписи, РежимПроведения) Экспорт
	...
	СвойстваДокумента = Новый ФиксированнаяСтруктура("ЭтоНовый, РежимЗаписи, РежимПроведения, Проведен, Дата",
		Документ.ЭтоНовый(),
		РежимЗаписи,
		РежимПроведения,
		Документ.Проведен,
	...
	Документ.ДополнительныеСвойства.ПроведениеДокументов.Вставить(
		"ТаблицаОчередейЗаданий",
		ОтложенныеЗадания.ИнициализироватьТаблицуЗаданий());
	
	// Ключом структуры "СвойстваДвиженийМеханизмов" должно быть имя учетного механизма.
	// Инициализация значения должна происходить в модуле менеджера механизма.
	Документ.ДополнительныеСвойства.ПроведениеДокументов.Вставить("СвойстваДвиженийМеханизмов",Новый Структура);
...
//
Функция ЕстьТаблицаЗапроса(ИмяТаблицы, ТекстыЗапроса, МенеджерВременныхТаблиц = Неопределено) Экспорт
	
	Если ТекстыЗапроса = Неопределено И МенеджерВременныхТаблиц = Неопределено Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если МенеджерВременныхТаблиц <> Неопределено
	   И МенеджерВременныхТаблиц.Таблицы.Найти(ИмяТаблицы) <> Неопределено Тогда
		Возврат Истина;
	КонецЕсли;
...
// Удаляет из текста запроса временные таблицы, которые уже есть в менеджере временных таблиц.
// 
// Параметры:
//  ТекстыЗапроса           - см. ЕстьТаблицаЗапроса.ТекстыЗапроса
//  МенеджерВременныхТаблиц - см. ЕстьТаблицаЗапроса.МенеджерВременныхТаблиц
//
Процедура УдалитьИзТекстаЗапросаСозданиеВременныхТаблиц(ТекстыЗапроса, МенеджерВременныхТаблиц) Экспорт
	
	Если ТекстыЗапроса = Неопределено Или МенеджерВременныхТаблиц = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ВременныеТаблицы = МенеджерВременныхТаблиц.Таблицы;
	
	Индекс = ТекстыЗапроса.Количество() - 1;
	Пока Индекс > -1 Цикл
		
		ТекстЗапроса = ТекстыЗапроса[Индекс];
		Если ВременныеТаблицы.Найти(ТекстЗапроса.Представление) <> Неопределено Тогда
...
Процедура ДополнитьЗапросОтраженияДокумента(Запрос, ТекстыЗапроса, ТекстыШаблоновОтражения, ТекстыЗапросовДанныхДокумента, ТекстыШаблоновВременныхТаблиц = Неопределено) Экспорт
	...
	ХэшТекстовЗапросов = ХэшСуммаТекстовЗапросов(ТекстыШаблоновОтражения, ТекстыЗапросовДанныхДокумента, ТекстыШаблоновВременныхТаблиц);
	СохраненныеТекстыЗапросов = ТекстыЗапросовПроведенияИзКэша(ХэшТекстовЗапросов);
	Если СохраненныеТекстыЗапросов <> Неопределено Тогда
		ДобавитьТекстыЗапросовПроведенияВПакет(ТекстыЗапроса, СохраненныеТекстыЗапросов);
		Возврат;
	КонецЕсли;
	
	РезультатСборокиТекстовЗапросов = ТекстыЗапросовПроведения();
	...
	Для каждого КлючИЗначение Из ТекстыЗапросовДанныхДокумента Цикл
		...
		Если ПозицияКлючевогоСлова("ОБЪЕДИНИТЬ", ТекстЗапроса) <> Неопределено Тогда
			...
			РезультатСборокиТекстовЗапросов.ТекстыСозданияВременныхТаблиц.Добавить(
				ТекстСозданияВременнойТаблицы,
				ИмяВременнойТаблицы,
				Истина);
	...
	Для каждого КлючИЗначение Из ТекстыШаблоновОтражения Цикл
		...
		РезультатСборокиТекстовЗапросов.ТекстыПроведенияПоРегистру.Добавить(
				ТекстОтражения,
				ИмяРегистра);
	КонецЦикла;
	
	СохранитьТекстыЗапросовПроведенияВКэш(ХэшТекстовЗапросов, РезультатСборокиТекстовЗапросов);
	ДобавитьТекстыЗапросовПроведенияВПакет(ТекстыЗапроса, РезультатСборокиТекстовЗапросов);
...
//	ОбщиеВременныеТаблицы - Массив Из Строка - временные таблицы, используемые во всех выборках пакета запроса.
//	ДополнительныеПоляСуммирования - Строка - если в регистре есть реквизиты числового типа, можно передать их через запятую, чтобы учитывать расхождения в таких реквизитах как в ресурсах.
//	ДополнительныеОтборыЗаписейРегистра - Строка - отбор на записи регистра, если проведение формирует только часть записей под регистратором.
//                                                 Отбор передается как часть запроса. Псевдоним таблицы регистра "ТаблицаРегистра".
//                                                 Например: "ТаблицаРегистра.Поле1 = 0".
//
// Возвращаемое значение:
//	Массив из ДокументСсылка - ссылки на документы, по которым нужно переформировать движения по регистру.
//
Функция РегистраторыДляПерепроведения(ТекстыЗапросов, ПолноеИмяРегистра, ПолноеИмяДокумента,
			ОбщиеВременныеТаблицы = Неопределено, ДополнительныеПоляСуммирования = "", ДополнительныеОтборыЗаписейРегистра = "ИСТИНА") Экспорт
	...
	Если ОбщиеВременныеТаблицы <> Неопределено Тогда
		
		Для Каждого ТекстВыборки Из ОбщиеВременныеТаблицы Цикл
	...
	Для Каждого ТекстЗапроса Из ТекстыЗапросов Цикл
		...
		Если ПозицияКлючевогоСлова("УНИЧТОЖИТЬ", ТекстВыборки) <> Неопределено Тогда //@query-part
			Продолжить;
		Иначе
			...
			Если ПозицияКлючевогоСлова("ПОМЕСТИТЬ", ТекстВыборки) <> Неопределено Тогда //@query-part
	...
	Для Каждого ПоляРегистра Из СоставПолейРегистра Цикл
		...
		Для Каждого ПолеРегистра Из МетаданныеРегистра[ПоляРегистра] Цикл
			...
			Если (ПоляРегистра = "Ресурсы" Или СтрНайти(ДополнительныеПоляСуммирования, ИмяПоля) > 0)
				И ТипРегистра = "РегистрНакопления" Тогда
	...
	|	И (&ДополнительныеОтборыЗаписейРегистра)
	|";
	...
	ТекстЗапросаРегистра = СтрЗаменить(ТекстЗапросаРегистра, "&ДополнительныеОтборыЗаписейРегистра", ДополнительныеОтборыЗаписейРегистра);
...
// Проверяет наличие дополнительной таблицы данных документа.
//
// Параметры:
//  Документ   - см. ДобавитьТаблицуДанныхДокумента.Документ
//  ИмяТаблицы - см. ДобавитьТаблицуДанныхДокумента.ИмяТаблицы
//
// Возвращаемое значение:
//  Булево - Истина, если есть дополнительная таблица.
//
Функция ЕстьДополнительнаяТаблица(Документ, ИмяТаблицы) Экспорт
	
	СвойствоПроведениеДокументов                 = Неопределено;
	СвойствоДополнительныеТаблицыДанныхДокумента = Неопределено;
	
	Возврат Документ.ДополнительныеСвойства.Свойство("ПроведениеДокументов", СвойствоПроведениеДокументов)
			И СвойствоПроведениеДокументов.Свойство(
				"ДополнительныеТаблицыДанныхДокумента", СвойствоДополнительныеТаблицыДанныхДокумента)
			И СвойствоДополнительныеТаблицыДанныхДокумента.Свойство(ИмяТаблицы);
...
Функция ЭмуляцияДокумента(Ссылка, Дата) Экспорт
	...
	СвойстваДокумента = Новый ФиксированнаяСтруктура("ЭтоНовый, РежимЗаписи, РежимПроведения, ЭтоПерепроведение, Дата",
		Ложь,
...
//  МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - менеджер временных таблиц,
//      используемый для хранения таблиц контроля изменений регистров.
//  Запустить - Булево - признак запуска заданий очередей.
//                       По умолчанию ИСТИНА.
//
Процедура СформироватьЗаданияНаОтложенныеДвижения(Документ, МенеджерВременныхТаблиц, Запустить = Истина) Экспорт
	...
	СостоянияДокументов.ОтразитьСостоянияДокументов(Документ, МенеджерВременныхТаблиц, Запустить);
...
// Вызывается ПередЗаписью набора записей регистра накопления - сохраняет движения, сформированные отложенно.
...
//	Замещение - Булево, РежимЗамещения - режим записи набора (параметр события ПередЗаписью).
...
Процедура СохранитьДвиженияСформированныеОтложенно(НаборЗаписей, Замещение, ВидИсточника, ИсключитьНастройкиХО = Неопределено) Экспорт
	...
	Если ТипЗнч(Замещение) = Тип("Булево") Тогда
		РежимЗамещенияЗаписей = ?(Замещение, РежимЗамещения.Замещение, РежимЗамещения.Добавление);
	Иначе
		РежимЗамещенияЗаписей = Замещение;
	КонецЕсли;
	
	Если РежимЗамещенияЗаписей = РежимЗамещения.Слияние Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru = 'В процедуре не поддерживается режим записи ""%1""'"), РежимЗамещенияЗаписей);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ВидИсточника)
		ИЛИ РежимЗамещенияЗаписей = РежимЗамещения.Добавление
		ИЛИ РежимЗамещенияЗаписей = РежимЗамещения.Обновление
		ИЛИ РежимЗамещенияЗаписей = РежимЗамещения.Удаление Тогда
		Возврат;
	КонецЕсли;
...
// Возвращает таблицу очередей заданий проводимого документа.
//
// Параметры:
//  Документ - ДокументОбъект - документ, по которому необходимо получить таблицу очередей заданий.
//  Очередь  - Строка, Неопределено - имя регистра сведений очереди заданий.
//                                    По умолчанию Неопределено.
//
// Возвращаемое значение:
//  см. ОтложенныеЗадания.ИнициализироватьТаблицуЗаданий
//
Функция ТаблицаОчередейЗаданий(Документ, Очередь = Неопределено) Экспорт
	
	Таблица = Документ.ДополнительныеСвойства.ПроведениеДокументов.ТаблицаОчередейЗаданий;
	
	Если ЗначениеЗаполнено(Очередь) Тогда
		ОтложенныеЗадания.ПроверитьДобавитьТаблицуЗаданий(Очередь, Таблица);
	КонецЕсли;
...
// Устанавливает параметры запроса по выборке из результата запроса
// 
// Параметры:
//  ЗапросПриемник - Запрос - в который необходимо добавить параметры из выборки.
//  ВыборкаИсточник - ВыборкаИзРезультатаЗапроса - источник для установки параметров. Где имя колонки - имя параметра,
//  												а значение - значение параметра.
//  ИсключаемыеПоля - Строка - Имена колонок выборки, перечисленные через запятую, которые не нужно устанавливать
//													в качестве параметров.
Процедура УстановитьПараметрыЗапросаПоВыборке(ЗапросПриемник, ВыборкаИсточник, Знач ИсключаемыеПоля = "") Экспорт

	ИсключаемыеПоля = Новый Структура(ИсключаемыеПоля);

	Для Каждого Колонка Из ВыборкаИсточник.Владелец().Колонки Цикл
		
		ИмяПараметра = Колонка.Имя;
		
		Если ИсключаемыеПоля.Свойство(ИмяПараметра) Тогда
			Продолжить;
		КонецЕсли;
		
		ЗапросПриемник.УстановитьПараметр(ИмяПараметра, ВыборкаИсточник[ИмяПараметра]);
...
Функция УчетныеМеханизмыКонфигурации() Экспорт
	...
	МеханизмыКонфигурации.Вставить("Взаиморасчеты", "ВзаиморасчетыПроведение");
	...
	МеханизмыКонфигурации.Вставить("ДоставкаТоваров", "ДоставкаТоваров");
...
Процедура ЗапуститьВыполнениеЗаданийОперативнойОчереди(Документ, Отказ)
	
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
	
	ТаблицаОчередейЗаданий = ТаблицаОчередейЗаданий(Документ);
	
	КоличествоСтрок = ТаблицаОчередейЗаданий.Количество();
	...
	Пока Индекс > -1 Цикл
		...
		Если ТаблицаОчередейЗаданий[Индекс].Задания.Количество() = 0 Тогда
			ТаблицаОчередейЗаданий.Удалить(Индекс);
		КонецЕсли;
	...
	Если ТаблицаОчередейЗаданий.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ОтложенныеЗадания.Запустить(ТаблицаОчередейЗаданий);
	
КонецПроцедуры

// В учетном механизме проверяются условия и запускается выполнение заданий 
// неоперативной очереди в транзакции проведения документа.
// 
// Параметры:
//  Механизмы - СписокЗначений Из см. ПроведениеДокументов.ПараметрыУчетногоМеханизма
//  Документ - ДокументОбъект - Записываемый документ
//  Отказ - Булево - признак отказа от записи
Процедура ПроверитьВыполнитьЗаданияНеоперативнойОчереди(Механизмы, Документ, Отказ)
	...
	Для каждого Механизм Из Механизмы Цикл
		Если Механизм.Значение.ВыполнениеЗаданийНеоперативнойОчереди Тогда
			Механизм.Значение.МодульМеханизма.ПроверитьВыполнитьЗаданияНеоперативнойОчереди(Документ);
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

Функция УчетныеМеханизмыСторно(Документ)
	...
	Если МеханизмыДокумента.Найти("ИсправлениеДокументов") = Неопределено Тогда
...
Функция ПодготовитьНаборыЗаписейКРегистрацииДвижений(Документ, Свойства, Регистры = Неопределено)
	...
	Для Каждого НаборЗаписей Из НаборыЗаписейКОбработке Цикл
		...
		// Пробросим свойства движений механизмов.
		Если Документ.ДополнительныеСвойства.Свойство("ПроведениеДокументов")
			И Документ.ДополнительныеСвойства.ПроведениеДокументов.Свойство("СвойстваДвиженийМеханизмов") Тогда
			НаборЗаписей.ДополнительныеСвойства.Вставить("СвойстваДвиженийМеханизмов",
				Документ.ДополнительныеСвойства.ПроведениеДокументов.СвойстваДвиженийМеханизмов);
		КонецЕсли;
	КонецЦикла;
...
// Дополняет текст запроса механизма контроля дат запрета.
// 
// Параметры:
...
// 	Механизмы - СписокЗначений из См. ПараметрыУчетногоМеханизма - список механизмов.
Процедура ДополнитьТекстЗапросаКонтроля(Запрос, ТекстЗапроса, ТекстЗапросаВременныхТаблиц, ИменаВременныхТаблиц, Механизмы)
	Для каждого Механизм Из Механизмы Цикл
		
		Если Не ЗначениеЗаполнено(Механизм.Значение.КонтрольныеРегистрыДатаЗапрета) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОбработатьДокументПослеЗаписиДвижений(Механизмы, Документ, МенеджерВременныхТаблиц, Отказ);
	...
	НеоперативнаяОчередьЗаданий.СнятьПризнакВыполненоСОшибкойПоДокументам(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Документ.Ссылка));
...
Функция ХэшСуммаТекстовЗапросов(ТекстыШаблоновОтражения, ТекстыЗапросовДанныхДокумента, ТекстыШаблоновВременныхТаблиц)
	
	Хеширование = Новый ХешированиеДанных(ХешФункция.MD5);
	Для каждого КлючИЗначение Из ТекстыШаблоновОтражения Цикл 
		Хеширование.Добавить(КлючИЗначение.Значение);
	КонецЦикла;
	Для каждого КлючИЗначение Из ТекстыЗапросовДанныхДокумента Цикл 
		Хеширование.Добавить(КлючИЗначение.Значение);
	КонецЦикла;
...
Функция ТекстыЗапросовПроведенияИзКэша(ХэшСумма)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	КэшТекстовЗапросовПроведения.ТекстыЗапросов КАК ТекстыЗапросов
	|ИЗ
	|	РегистрСведений.КэшТекстовЗапросовПроведения  КАК КэшТекстовЗапросовПроведения 	
	|ГДЕ
	|	КэшТекстовЗапросовПроведения.ХэшСумма = &ХэшСумма 
	|";
	
	Запрос.УстановитьПараметр("ХэшСумма", ХэшСумма); 
	
	УстановитьПривилегированныйРежим(Истина);
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Выборка.ТекстыЗапросов.Получить();	
	КонецЕсли;
...
Процедура СохранитьТекстыЗапросовПроведенияВКэш(ХэшСумма, ТекстыЗапросов)
	
	УстановитьПривилегированныйРежим(Истина);
	Запись = РегистрыСведений.КэшТекстовЗапросовПроведения.СоздатьМенеджерЗаписи();
	Запись.ХэшСумма = ХэшСумма;
	Запись.ТекстыЗапросов =  Новый ХранилищеЗначения(ТекстыЗапросов);
	Запись.Записать();
	
КонецПроцедуры

Функция ТекстыЗапросовПроведения()
	
	ЗапросПроведения = Новый Структура();
	ЗапросПроведения.Вставить("ТекстыСозданияВременныхТаблиц", Новый СписокЗначений);
	ЗапросПроведения.Вставить("ТекстыПроведенияПоРегистру", Новый СписокЗначений);
	
	Возврат ЗапросПроведения;
	
КонецФункции

Процедура ДобавитьТекстыЗапросовПроведенияВПакет(ТекстыЗапросов, ЗапросыПроведения)
	
	Для каждого ЭлементСписка Из ЗапросыПроведения.ТекстыСозданияВременныхТаблиц Цикл
		ТекстыЗапросов.Добавить(ЭлементСписка.Значение, ЭлементСписка.Представление, ЭлементСписка.Пометка);	
	КонецЦикла;

	Для каждого ЭлементСписка Из ЗапросыПроведения.ТекстыПроведенияПоРегистру Цикл
		ТекстыЗапросов.Добавить(ЭлементСписка.Значение, ЭлементСписка.Представление, ЭлементСписка.Пометка);	
	КонецЦикла;
		
	Для каждого ЭлементСписка Из ЗапросыПроведения.ТекстыСозданияВременныхТаблиц Цикл
		Если ПозицияКлючевогоСлова("ДОБАВИТЬ", ЭлементСписка.Значение) <> Неопределено Тогда //@Query-part
			Продолжить;
		КонецЕсли;
		ТекстУдаления = СтрШаблон("УНИЧТОЖИТЬ %1", ЭлементСписка.Представление);
		ТекстыЗапросов.Добавить(ТекстУдаления);
	КонецЦикла;
...
Процедура ВставитьПомещениеВоВременнуюТаблицу(ТекстЗапроса, Знач ИмяВременнойТаблицы)
	...
	Если ПозицияКлючевогоСлова("ПОМЕСТИТЬ", ТекстЗапроса) <> Неопределено Тогда //@Query-part
		Возврат;
	КонецЕсли;
	Если ПозицияКлючевогоСлова("ДОБАВИТЬ", ТекстЗапроса) <> Неопределено Тогда //@Query-part
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьЗаменуПоПолномуСоответствию(Текст, Знач СтрокаПоиска, Знач СтрокаЗамены, ТолькоПервый = Ложь)
	
	Если ТолькоПервый Тогда
		ВыполнитьЗаменуПервогоПоПолномуСоответствию(Текст, СтрокаПоиска, СтрокаЗамены);
		Возврат;
	КонецЕсли;
	
	Если СтрНачинаетсяС(СтрокаПоиска, "#") Тогда
		РегулярноеВыражение = "\B\%1\b";
	Иначе
		РегулярноеВыражение = "\b%1\b";
	КонецЕсли;                         
	РегулярноеВыражение = СтрШаблон(РегулярноеВыражение, СтрокаПоиска); 
	Текст = СтрЗаменитьПоРегулярномуВыражению(Текст, РегулярноеВыражение, СтрокаЗамены);
	
КонецПроцедуры

Процедура ВыполнитьЗаменуПервогоПоПолномуСоответствию(Текст, Знач СтрокаПоиска, Знач СтрокаЗамены)
...
Функция СтруктураПолейВыборки(ТекстВыборки)
    ...
    ДлинаСловаКАК = СтрДлина("КАК"); //@query-part
	...
	Для каждого Подстрока Из ПодстрокиВыборкиДляАнализа Цикл
		...
		ПодстрокаВыборки = ПодстрокаВыборки + ?(ПодстрокаВыборки = "", "", ",") + Подстрока;	
		
		ЧислоОткрытийЗакрытийСкобок = СтрЧислоВхождений(ПодстрокаВыборки, "(") - СтрЧислоВхождений(ПодстрокаВыборки, ")");
		ЧислоДвойныхКавычек = СтрЧислоВхождений(ПодстрокаВыборки, """");
		
		Если ЧислоОткрытийЗакрытийСкобок = 0 И ЧислоДвойныхКавычек%2 = 0 Тогда
			
			ПозицияКАК = СтрНайти(ПодстрокаВыборки, "КАК", НаправлениеПоиска.СКонца); //@query-part
			Если ПозицияКАК = 0 Тогда
				ВызватьИсключение НСтр("ru = 'Для всех полей выборки должен быть задан псевдоним'");
			КонецЕсли;  
			
			Выражение = СтрШаблон("(%1)", СокрЛП(Лев(ПодстрокаВыборки, ПозицияКАК - 1)));
			Псевдоним = СокрЛП(Прав(ПодстрокаВыборки, СтрДлина(ПодстрокаВыборки) - (ПозицияКАК + ДлинаСловаКАК)));
...
Функция ПозицияКлючевогоСлова(КлючевоеСлово, ТекстЗапроса, Знач НачальнаяПозицияПоиска = 1)
	
	РегулярноеВыражение = СтрШаблон("\b%1\b", КлючевоеСлово);
	РезультатПоиска = СтрНайтиПоРегулярномуВыражению(ТекстЗапроса, РегулярноеВыражение,,НачальнаяПозицияПоиска);
	
	Если РезультатПоиска.НачальнаяПозиция <> 0 Тогда
		Возврат Новый Структура(
						"НачальнаяПозиция, КонечнаяПозиция", 
						РезультатПоиска.НачальнаяПозиция, 
...
// * ДвиженияТолькоВТекущемПериоде - Булево -
...
Функция ИнициализироватьПараметрыВосстановленияДвижений(ИсточникДанных, РасшифровкаКодовОшибок,
	...
	ПараметрыПроведения.Вставить("ДвиженияТолькоВТекущемПериоде", 		Ложь); // для документов, имеющих движения в нескольких периодах
...
Функция ВосстановитьДвиженияДокументов(ПараметрыПроведения) Экспорт
	...
	СуществующиеВТ = РасчетСебестоимостиПрикладныеАлгоритмы.ПолучитьПереченьСуществующихВременныхТаблиц(Запрос, СуществующиеВТ)
		 + ?(ЗначениеЗаполнено(ПараметрыПроведения.ИмяВременнойТаблицыПолноеПроведение), "," + ПараметрыПроведения.ИмяВременнойТаблицыПолноеПроведение, "");
	...
	Если НЕ РасчетСебестоимостиПрикладныеАлгоритмы.ВременнаяТаблицаСуществует(Запрос, "ВТМоментыВремени") Тогда
		...
		Если ЗначениеЗаполнено(ПараметрыПроведения.ИмяВременнойТаблицыПолноеПроведение)
		 И РасчетСебестоимостиПрикладныеАлгоритмы.ВременнаяТаблицаСуществует(Запрос, ПараметрыПроведения.ИмяВременнойТаблицыПолноеПроведение) Тогда // таблица может формироваться позже, в ТекстДополнительногоЗапросаПодготовкиДанных
...
Функция ПерепровестиДокументыПоРегиструНакопления(ПараметрыПроведения, ВыборкаДокументов, ИмяРегистраНакопления) Экспорт
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Если ПараметрыПроведения.ДвиженияТолькоВТекущемПериоде Тогда
		...
		Попытка
			...
			Если ПараметрыПроведения.ВызываетсяИзРасчетаСебестоимости Тогда
				...
				Если ПараметрыПроведения.ДвиженияТолькоВТекущемПериоде Тогда
					...
					ДокументДляПроведения.ДополнительныеСвойства.Вставить(
						"ПериодПерезаписиДвижений",
						ПараметрыПроведения.ПериодРасчета);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "ПроведениеДокументов.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "*";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("f82bd3b9-6546-42a6-a1e7-2b09d915a495");
	Обработчик.Многопоточный = Ложь;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "ПроведениеДокументов.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.Комментарий = НСтр("ru = 'Очищает кэш запросов проведения документов.'");
	
	Изменяемые = Новый Массив;
	Изменяемые.Добавить(Метаданные.РегистрыСведений.КэшТекстовЗапросовПроведения.ПолноеИмя());
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	НаборЗаписей = РегистрыСведений.КэшТекстовЗапросовПроведения.СоздатьНаборЗаписей();
