...
Процедура УправлениеВидимостьюКомандНакладнаяСАТУРН(Форма, Команды) Экспорт
	...
	Для Каждого КлючИЗначение Из Форма.НастройкиВыполненияПодключаемыхКомандИС.Команды Цикл
		...
		ИначеЕсли ИмяМетаданных = "РеализацияТоваровУслуг"
			...
			Или ИмяМетаданных = "ОтгрузкаТоваровКлиенту" Тогда
			Элемент.Видимость = 
...
Процедура КомандыАктИнвентаризацииСАТУРН(Команды) Экспорт 
	...
	ПодключаемыеКомандыИСКлиентСервер.ДобавитьКомандуВыбрать(Команды, "ОприходованиеИзлишковОтгруженныхТоваров", НСтр("ru = 'Оприходование излишков отгруженных товаров'"), "ИспользоватьПередачуНаОтветственноеХранениеСПравомПродажи");
	ПодключаемыеКомандыИСКлиентСервер.ДобавитьКомандуВыбрать(Команды, "СписаниеОтгруженныхТоваров",        НСтр("ru = 'Списание отгруженных товаров'"), "ИспользоватьПередачуНаОтветственноеХранениеСПравомПродажи" + ИнтеграцияИСУТКлиентСервер.ИмяФункицональнойОпцииИспользоватьПроизводствоНаСтороне25());
...
Процедура УправлениеВидимостьюКомандАктИнвентаризацииСАТУРН(Форма, Команды) Экспорт
	...
	Для Каждого КлючИЗначение Из Форма.НастройкиВыполненияПодключаемыхКомандИС.Команды Цикл
		...
		Если ИмяМетаданных = "ОприходованиеИзлишковТоваров"
			...
			Или ИмяМетаданных = "ОприходованиеИзлишковОтгруженныхТоваров" Тогда
			Форма.Элементы[КлючИЗначение.Ключ].Видимость = Приход;
		ИначеЕсли ИмяМетаданных = "СписаниеНедостачТоваров"
			Или ИмяМетаданных = "СписаниеОтгруженныхТоваров" Тогда
...
Процедура КомандыИмпортПродукцииСАТУРН(Команды) Экспорт 
	...
	ПодключаемыеКомандыИСКлиентСервер.ДобавитьКомандуОформить(Команды, "ПриобретениеТоваровУслуг",   НСтр("ru = 'Приобретение товаров и услуг'"));
	ПодключаемыеКомандыИСКлиентСервер.ДобавитьКомандуОформить(Команды, "ЗаказПоставщику",            НСтр("ru = 'Заказ поставщику'"));
...
// 	Команды - Структура - структура команд динамически формируемых для создания документов на основании.
//
Процедура УправлениеВидимостьюКомандИмпортПродукцииСАТУРН(Форма, Команды) Экспорт
	
	ЭтоОперацияИмпортИзЕАЭС = (Форма.Объект.Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийСАТУРН.ИмпортПродукцииИзЕАЭС"));
	ЭтоОперацияИмпортИзПрочихСтран = (Форма.Объект.Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийСАТУРН.ИмпортПродукцииПредварительноеУведомление"));
	ЭтоОперацияКонечныйИмпорт = Не ЭтоОперацияИмпортИзЕАЭС И Не ЭтоОперацияИмпортИзПрочихСтран;
	
	Для Каждого КлючИЗначение Из Форма.НастройкиВыполненияПодключаемыхКомандИС.Команды Цикл
		ИмяМетаданных = КлючИЗначение.Значение.ИмяМетаданных;
		Если ИмяМетаданных = "ЗаказПоставщику" Тогда
			Форма.Элементы[КлючИЗначение.Ключ].Видимость = ЭтоОперацияИмпортИзПрочихСтран;
		ИначеЕсли ИмяМетаданных = "ПриобретениеТоваровУслуг" Тогда
			Форма.Элементы[КлючИЗначение.Ключ].Видимость = ЭтоОперацияИмпортИзЕАЭС Или ЭтоОперацияКонечныйИмпорт;
		КонецЕсли;
	КонецЦикла;
