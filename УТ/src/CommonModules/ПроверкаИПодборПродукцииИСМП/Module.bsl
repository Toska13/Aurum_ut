...
Процедура ЗаполнитьПризнакАвтоматическийОСУИСВСтроке(СтрокаТовары, ПараметрыСканирования = Неопределено, НаДату = Неопределено) Экспорт
	...
	Если КонструкторИСМПКлиентСервер.ЭтоСырьеГИСМТ(СтрокаТовары.ВидПродукцииИС) Тогда
		...
		Таблица = РегистрыСведений.СоответствиеНоменклатурыСырьяГИСМТ.ТаблицаИдентификаторовСырья();
		СтрокаТаблицы = Таблица.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаТаблицы, СтрокаТовары);
		
		РегистрыСведений.СоответствиеНоменклатурыСырьяГИСМТ.ЗаполнитьИдентификационныеКоды(Таблица, ПараметрыСканирования);
		
		СтрокаТовары.АвтоматическийОСУИС = (СтрокаТаблицы.ПодходитИдентификационныйКод = 1)
...
Процедура ЗаполнитьПризнакАвтоматическийОСУИСВТаблице(ТаблицаТовары, ЗаполнитьКолонкуGTIN = Ложь, ПараметрыСканирования = Неопределено, НаДату = Неопределено) Экспорт
	...
	ТаблицаСырье = РегистрыСведений.СоответствиеНоменклатурыСырьяГИСМТ.ТаблицаИдентификаторовСырья();
	...
	Для Каждого СтрокаТаблицы Из ТаблицаТовары Цикл
		...
		Если ВидПродукцииПоддерживаетОСУ Тогда
			Если КонструкторИСМПКлиентСервер.ЭтоСырьеГИСМТ(СтрокаТаблицы.ВидПродукцииИС) Тогда
				ЗаполнитьЗначенияСвойств(ТаблицаСырье.Добавить(), СтрокаТаблицы);
			Иначе
	...
	Если Таблица.Количество() = 0 И ТаблицаСырье.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	ТаблицаСырье.Свернуть(Поля + ",ПодходитИдентификационныйКод,ИдентификационныйКод");
	ТаблицаСырье.Индексы.Добавить(Поля);
	...
	Если ТаблицаСырье.Количество() Тогда
		РегистрыСведений.СоответствиеНоменклатурыСырьяГИСМТ.ЗаполнитьИдентификационныеКоды(ТаблицаСырье, ПараметрыСканирования);
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из ТаблицаТовары Цикл
		...
		Если ВидПродукцииПоддерживаетОСУ Тогда
			...
			Если КонструкторИСМПКлиентСервер.ЭтоСырьеГИСМТ(СтрокаТаблицы.ВидПродукцииИС) Тогда
				СтрокаТаблицыПоиска = ТаблицаСырье.НайтиСтроки(Поиск)[0];
				СтрокаТаблицы.АвтоматическийОСУИС = (СтрокаТаблицыПоиска.ПодходитИдентификационныйКод = 1) И ДействуетОСУПоДатам И ВидОперацииПоддерживаетОСУ;
				Если ЗаполнитьКолонкуGTIN Тогда
					СтрокаТаблицы.GTIN = СтрокаТаблицыПоиска.ИдентификационныйКод;
				КонецЕсли;
			Иначе
	...
	Для Каждого СтрокаПоУПД Из ТоварыПоУПД Цикл
		...
		Если СтрокаПоУПД.МаркируемаяПродукция
			Или (ЭтоЭлектронныйДокумент
				...
				И ЗначениеЗаполнено(СтрокаПоУПД.Штрихкод)
				И Не КонструкторИСМПКлиентСервер.ЭтоСырьеГИСМТ(СтрокаПоУПД.ВидПродукции)) Тогда
