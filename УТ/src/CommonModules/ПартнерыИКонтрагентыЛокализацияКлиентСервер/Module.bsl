...
// Возвращает список доступных регистров налогов учетных политик контрагентов.
//
// Возвращаемое значение:
//  Массив - список имен регистров
//
Функция ДоступныеРегистрыНалоговУчетныхПолитикКонтрагентов() Экспорт
	
	МассивРегистров = Новый Массив();

	//++ Локализация
	МассивРегистров.Добавить("НастройкиУчетаНДСПриУСНКомитентов");
	//-- Локализация
...
// Параметры:
// 	Форма - Форма
// 		Справочник.Партнеры.Форма.ФормаЭлементаРеквизитыКонтрагента
// 		Справочник.Контрагенты.Форма.ФормаЭлемента.
//
Процедура УправлениеВидимостьюНастройкиУчетаНДСКомитента(Форма) Экспорт

	//++ Локализация
	Форма.Элементы.ЛокализацияГруппаНастройкиУчетаНДСКомитента.Видимость =
		НЕ Форма.Объект.ЮрФизЛицо = ПредопределенноеЗначение("Перечисление.ЮрФизЛицо.ФизЛицо");
	//-- Локализация
...
ТекстУстановленИзначально = НСтр("ru = 'установлено изначально'");
	
	ТекстКомандыИсторииНаименования = "";
	
	Если ИсторияНаименований.Количество() = 0 Тогда
		
		ТекстКомандыИсторииНаименования = ТекстУстановленИзначально;
		
	Иначе
		
		ИсторияНаименований.Сортировать("Период Убыв");
		ДатаПоследнегоПериода = ИсторияНаименований[0].Период;
		
		Если ЗначениеЗаполнено(ДатаПоследнегоПериода) Тогда
			
			ТекстКомандыИсторииНаименования = СтрШаблон(НСтр("ru = 'установлено с %1'"), Формат(ДатаПоследнегоПериода,"ДЛФ=D")); 
