...
Процедура ПриОкончанииИзмененияРеквизита(ИмяЭлемента, Форма, ПараметрыОбработки) Экспорт
	...
	КонецЕсли;
	Если ИмяЭлемента = "ИспользоватьОформлениеДокументовРаздельнойЗакупкиТоваров" Тогда
		УстановитьДоступностьПорядкаОтраженияНеотфактурованныхПоставокНУ(Форма);
	КонецЕсли;
	Если ИмяЭлемента = "ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслуг" Тогда
		УстановитьДоступностьПорядкаОтраженияНеотфактурованныхПоставокНУ(Форма);
	КонецЕсли;
	Если ИмяЭлемента = "ВариантОформленияЗакупок" Тогда
		УстановитьДоступностьПорядкаОтраженияНеотфактурованныхПоставокНУ(Форма);
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, СправочникОбъект) Экспорт
	...
	Если ЗначениеЗаполнено(СправочникОбъект.Дата) И ЗначениеЗаполнено(СправочникОбъект.Номер) И Не СправочникОбъект.ПлатежиПо275ФЗ 
		И Не ЗначениеЗаполнено(СправочникОбъект.ИдентификаторПлатежа)
		И Справочники.ДоговорыКонтрагентов.ЭтоДоговорСКлиентом(СправочникОбъект.ТипДоговора) Тогда
...
Процедура УстановитьВидимостьИдентификатораВалютногоКонтракта(Форма)
	...
	Элементы = Форма.Элементы;
	ИспользоватьУНК = ИспользоватьУникальныйНомерВалютногоКонтроля(Форма.Объект, Форма.КонтрагентЮрФизЛицо);
	
	Если ТипЗнч(Форма.Объект.Ссылка) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		Элементы.ЛокализацияГруппаВалютногоКонтроля.Видимость = ИспользоватьУНК;
	Иначе
		Элементы.УникальныйНомерВалютногоКонтроля.Видимость = ИспользоватьУНК;
	КонецЕсли;
