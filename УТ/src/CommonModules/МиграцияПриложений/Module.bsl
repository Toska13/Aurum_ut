...
Процедура ПриОпределенииПсевдонимовОбработчиков(СоответствиеИменПсевдонимам) Экспорт
	...
	СоответствиеИменПсевдонимам.Вставить(ИмяМетодаЗаданиеВыгрузка());
	СоответствиеИменПсевдонимам.Вставить(ИмяМетодаЗаданиеЗагрузка());
	СоответствиеИменПсевдонимам.Вставить(ИмяМетодаУдалитьВременногоПользователя());
...
Процедура ЗаданиеВыгрузка(ЗавершитьМиграцию = Ложь) Экспорт
	...
	Исключение
		...
		Иначе
			...
			РегистрацияОшибки(НСтр("ru = 'Выгрузка'", ОбщегоНазначения.КодОсновногоЯзыка()), 
				ТехнологияСервиса.ПодробныйТекстОшибки(ИнформацияОбОшибке));
	...
	Исключение
		...
		Если СостояниеЗагрузки.КоличествоПопыток >= 3 Тогда
			ЗавершитьЗагрузкуСОшибкой(СостояниеЗагрузки, 
				...
				ТехнологияСервиса.КраткийТекстОшибки(ИнформацияОбОшибке));
		Иначе 
			РегистрацияОшибки(НСтр("ru = 'Загрузка'", ОбщегоНазначения.КодОсновногоЯзыка()), 
				ТехнологияСервиса.ПодробныйТекстОшибки(ИнформацияОбОшибке));
...
Функция СоздатьПриложениеДляМиграции(ИсточникПараметровДоступа, Наименование, ЧасовойПояс, ПраваПользователей, 
	...
	МиграцияСРасширениями1СПлюсПоддерживаетсяСВерсии = 28;
	Если ПараметрыДоступа.ВерсияПрограммногоИнтерфейса < МиграцияСРасширениями1СПлюсПоддерживаетсяСВерсии 
		И Расширения1СПлюс.Количество() > 0 Тогда
		ИменаРасширений = Новый Массив;
		Для Каждого КлючЗначение Из Расширения1СПлюс Цикл
			ИменаРасширений.Добавить(КлючЗначение.Значение.ИмяРасширения);
		КонецЦикла;
		
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Менеджер сервиса не поддерживает миграцию с расширениями 1С:Плюс
			|Установлены расширения: 
			|%1'"), СтрСоединить(ИменаРасширений, Символы.ПС));
	КонецЕсли;
	
	Для Каждого КлючЗначение Из Расширения1СПлюс Цикл
		ДанныеРасширения = КлючЗначение.Значение;
		ИнфоОРасширении = НовоеРасширениеДляВосстановления(
		...
		Расширения.Добавить(ИнфоОРасширении);
	КонецЦикла;
...
Функция Описание(ИдентификаторХранилища, ИдентификаторДанных) Экспорт
	...
	ИначеЕсли ИдентификаторДанных = "remove-user" Тогда
		ПараметрыЗадания = Новый Структура("ИмяМетода", ИмяМетодаУдалитьВременногоПользователя());
...
Функция Загрузить(ОписаниеДанных) Экспорт
	...
	Если ОписаниеДанных.ИмяФайла = "message" Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			РегистрацияОшибки(НСтр("ru = 'Загрузка сообщения'", ОбщегоНазначения.КодОсновногоЯзыка()), 
				ТехнологияСервиса.ПодробныйТекстОшибки(ИнформацияОбОшибке));
			Результат.Успешно = Ложь;
...
Процедура УдалитьВременногоПользователя(Имя) Экспорт
	...
	Отбор = Новый Структура("ИмяМетода", ИмяМетодаУдалитьВременногоПользователя());
...
Функция ИмяМетодаЗаданиеВыгрузка() Экспорт
	
	Возврат "МиграцияПриложений.ЗаданиеВыгрузка";
	
КонецФункции

Функция ИмяМетодаЗаданиеЗагрузка() Экспорт
	
	Возврат "МиграцияПриложений.ЗаданиеЗагрузка";

КонецФункции

Функция ИмяМетодаУдалитьВременногоПользователя() Экспорт
	
	Возврат "МиграцияПриложений.УдалитьВременногоПользователя";
...
Функция СостояниеЗагрузки()
	...
	Если СостояниеЗагрузки.Выбран() Тогда
		...
		Если Не Статус.ВыполняетсяЗагрузка Тогда
			...
			ПараметрыМетода.Добавить(ПользователиИнформационнойБазы.ТекущийПользователь().Имя);
			ПараметрыЗадания = Новый Структура;
			ПараметрыЗадания.Вставить("ИмяМетода", ИмяМетодаУдалитьВременногоПользователя());
			ПараметрыЗадания.Вставить("Использование", Истина);
			ПараметрыЗадания.Вставить("Параметры", ПараметрыМетода);
...
Процедура ЗавершитьЗагрузку(СостояниеЗагрузки)
	...
	РаботаВМоделиСервисаБТС.ОбновитьГруппыДоступаПоставляемыхПрофилей();
	...
	Если РаботаВМоделиСервиса.ПоддерживаетсяНастройкаПравПользователейВМоделиСервисаБСП() Тогда
		МодульУправлениеДоступомСлужебныйВМоделиСервиса = 
			ОбщегоНазначения.ОбщийМодуль("УправлениеДоступомСлужебныйВМоделиСервиса");
		
		МодульУправлениеДоступомСлужебныйВМоделиСервиса.ОтправитьГруппыДоступаВМенеджерСервиса();
	КонецЕсли;

	КодОбластиДанных = РаботаВМоделиСервиса.ЗначениеРазделителяСеанса();
...
Функция НовоеРасширениеДляВосстановления(Имя, Версия, Ид, ИдВерсии) Экспорт
	Результат = Новый Структура();
	Результат.Вставить("id", Имя); 
	Результат.Вставить("version", Версия);
	Результат.Вставить("PlusExtId", Ид);
	Результат.Вставить("PlusExtVersionId", ИдВерсии);
	Возврат Результат;
КонецФункции
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаданиеВыгрузка", Истина);
	Методы.Вставить("ЗаданиеЗагрузка", Истина);
