...
Функция ПолучитьКоэффициентыПересчетаПоОбъектамРасчетов(ТекущаяВалюта, НоваяВалюта, Дата, Знач БазоваяВалюта = Неопределено, ОбъектыРасчетов) Экспорт
	...
	ТаблицаКурсов = РаботаСКурсамиВалютУТ.ПолучитьКурсВалютыПоОбъектамРасчетов(
		...
		БазоваяВалюта,
		ОбъектыРасчетов);
	// Для пустого объекта расчетов нужно добавить 2 строки таблицы курсов
	Если ОбъектыРасчетов.Найти(Справочники.ОбъектыРасчетов.ПустаяСсылка()) <> Неопределено Тогда
		СтрокаКурса = ТаблицаКурсов.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаКурса, ПолучитьКурсВалюты(ТекущаяВалюта, Дата, БазоваяВалюта));
		СтрокаКурса.ОбъектРасчетов = Справочники.ОбъектыРасчетов.ПустаяСсылка();
		СтрокаКурса = ТаблицаКурсов.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаКурса, ПолучитьКурсВалюты(НоваяВалюта, Дата, БазоваяВалюта));
		СтрокаКурса.ОбъектРасчетов = Справочники.ОбъектыРасчетов.ПустаяСсылка();
	КонецЕсли;
...
//	Точность - Число - число знаков после запятой, до которого нужно округлить числитель.
//
// Возвращаемое значение: 
...
//
Функция ПолучитьКроссКурсВалют(Валюта1, Валюта2, Точность = Неопределено) Экспорт
...
// Возвращает список базовых валют, который включает
// - валюты регламентированного учета организаций
// - валюту из константы "БазоваяВалютаПоУмолчанию".
//
// Возвращаемое значение:
//  Массив из СправочникСсылка.Валюты -
Функция БазовыеВалюты() Экспорт
	
	БазовыеВалюты = Новый Массив;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	БазовыеВалюты.Валюта КАК Валюта
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|	ИЗ
	|		Константа.БазоваяВалютаПоУмолчанию КАК БазоваяВалютаПоУмолчанию) КАК БазовыеВалюты";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		БазовыеВалюты.Добавить(Выборка.Валюта);
	КонецЦикла;
	
	Возврат БазовыеВалюты;
...
Процедура СинхронизоватьКурсыВалют(Источник) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("РегистрСведенийНаборЗаписей.КурсыВалют") Тогда
		...
		Если Источник.ДополнительныеСвойства.Свойство("ЗагрузкаКурсовВалютВОбластьЗаВесьПериод") Тогда
			Приемник.ДополнительныеСвойства.Вставить("ЗагрузкаКурсовВалютВОбластьЗаВесьПериод");
		КонецЕсли;
...
Процедура ВыполнитьЗапись(Приемник)
	...
	Приемник.ДополнительныеСвойства.Вставить("ОтключитьКонтрольПодчиненныхВалют");
