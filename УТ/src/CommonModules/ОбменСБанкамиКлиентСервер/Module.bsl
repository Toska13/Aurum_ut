...
Функция ТекстСсылкиПерепискаСБанками() Экспорт
	...
	СостояниеОбменаСБанками = ОбменСБанкамиСлужебныйВызовСервера.ТекущееСостояниеОбменаСБанкамиИДанныеОНепрочитанныхПисьмах();
	ТекущееСостояниеПисемСБанками = СостояниеОбменаСБанками.ТекущееСостояниеПисем;
	ДанныеОНепрочитанныхПисьмах = ТекущееСостояниеПисемСБанками.ДанныеОНепрочитанныхПисьмах;
	
	Если Не ТекущееСостояниеПисемСБанками.ЕстьНастройка Тогда
		Возврат Неопределено;
	ИначеЕсли ДанныеОНепрочитанныхПисьмах.КоличествоНепрочитанных = 0 Тогда
		Возврат НСтр("ru = 'Переписка с банками'");
	Иначе
		Шаблон = НСтр("ru = 'Переписка с банками (%1)'");
		Возврат СтрШаблон(Шаблон, ДанныеОНепрочитанныхПисьмах.КоличествоНепрочитанных);
	КонецЕсли;
...
//   * ТребуетсяПовторнаяАутентификация - Булево - если вернулось значение Истина, то сессия закончилась и 
...
//   * ПодключениеАвтоВыписки - Булево - признак того, что происходит подключение автоматической загрузки выписки.
//   * ЗагружатьДокументы - Булево - признак загрузки учетных документов.
//   * ЕстьОшибкаРазбораВыписки - Булево - признак наличия ошибки при разборе выписки банка.
//   * ТестОшибкиРазбораВыписки - Строка - текст ошибки разбора выписки банка.
//
Функция ПараметрыПолученияНовыхДокументовАсинхронныйОбмен() Экспорт
	...
	СтруктураВозврата.Вставить("УниверсальнаяДатаОбновленияСессии", 0);
	СтруктураВозврата.Вставить("ПодключениеАвтоВыписки", Ложь);
	СтруктураВозврата.Вставить("ЗагружатьДокументы", Истина);
	СтруктураВозврата.Вставить("ЕстьОшибкаРазбораВыписки", Ложь);
	СтруктураВозврата.Вставить("ТестОшибкиРазбораВыписки", "");
...
// Преобразовывает строку в число. Обрабатывает строки с произвольными разделителем дробной части и разделителем групп 
// разрядов. В случае неудачи генерируется исключение.
//
// Параметры:
...
//
Функция ЧислоИзСтроки(Знач Значение, РазделительДробнойЧасти = "") Экспорт
	...
	Если ПраваяГраница < ЛеваяГраница Тогда
		...
		ТекстСообщения = СтрШаблон(ШаблонСообщения, Значение);
...
Функция ЕстьПоддержкаОбменаПисьмами(ВерсияФормата) Экспорт
	...
	МинимальнаяВерсия = "2.3.1";
	Возврат ОбменСБанкамиСлужебныйКлиентСервер.СравнитьВерсиюФормата(ВерсияФормата, МинимальнаяВерсия);
	
КонецФункции

// Определяет возможность открытия депозита по формату обмена
//
// Параметры:
//  ВерсияФормата - Строка - версия формата обмена.
// 
// Возвращаемое значение:
//  Булево - Истина, если в текущем формате есть поддержка открытия депозита.
//
Функция ЕстьПоддержкаОткрытияДепозитов(ВерсияФормата) Экспорт
	
	МинимальнаяВерсия = ОбменСБанкамиДепозитыКлиентСервер.МинимальнаяВерсияФорматаОбменСБанкамиДепозиты();
	Возврат ОбменСБанкамиСлужебныйКлиентСервер.СравнитьВерсиюФормата(ВерсияФормата, МинимальнаяВерсия);
