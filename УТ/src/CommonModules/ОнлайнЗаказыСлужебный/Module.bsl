...
// Возвращает идентификатор способа оплаты СБП(c2b).
//
// Возвращаемое значение:
//  Строка - идентификатор операции.
//
Функция ИдентификаторВариантаСБПC2B() Экспорт
...
Функция ПереопределитьСообщениеПользователю(
	...
	ИначеЕсли КодОшибки = КодОшибкиНеверныйФорматЗапроса()
		И КодОшибкиСервиса = "EXPIRATION_TIMESTAMP_TOO_FAR" Тогда
		Возврат НСтр("ru = 'Превышен максимальный срок жизни онлайн-заказа (30 дней).'");
	ИначеЕсли КодОшибки = КодОшибкиНеверныйФорматЗапроса()
	...
	ИначеЕсли КодОшибки = КодОшибкиНеверныйФорматЗапроса()
		И КодОшибкиСервиса = "BAD_PAYMENT_GATEWAY_ACQUIRING_TOKEN_CHECK" Тогда
		Возврат НСтр("ru = 'Не удалось проверить корректность токена интернет-эквайринга.'");
	ИначеЕсли КодОшибки = КодОшибкиНеверныйФорматЗапроса()
		И КодОшибкиСервиса = "BAD_PAYMENT_GATEWAY_OZON_PAY_TOKEN_CHECK" Тогда
		Возврат НСтр("ru = 'Не удалось проверить корректность токена Озон Пэй.'");
	ИначеЕсли КодОшибки = КодОшибкиНеверныйФорматЗапроса()
		И КодОшибкиСервиса = "BAD_PAYMENT_GATEWAY_YANDEX_PAY_TOKEN_CHECK" Тогда
		Возврат НСтр("ru = 'Не удалось проверить корректность токена Озон Пэй.'");
	ИначеЕсли КодОшибки = КодОшибкиНеверныйФорматЗапроса()
	...
	Если КодОшибки = "ПревышеноКоличествоПопыток"
		И КодОшибкиСервиса = "ORDER_CREATION_LIMIT_EXCEEDED" Тогда
		Возврат НСтр("ru = 'Превышен суточный лимит количества регистрации новых онлайн-заказов.'");
	КонецЕсли;
	...
	ИначеЕсли КодОшибки = "ОтсутствуетДоступКСервису"
		И КодОшибкиСервиса = "CLOUD_SALES_REGISTER_TOKEN_OWNER_MISMATCH" Тогда
		Возврат НСтр("ru = 'Не удалось проверить корректность токена облачной кассы. Токен принадлежит другому пользователю интернет-поддержки.'");
	ИначеЕсли КодОшибки = "ОтсутствуетДоступКСервису"
		И КодОшибкиСервиса = "PAYMENT_GATEWAY_ACQUIRING_TOKEN_OWNER_MISMATCH" Тогда
		Возврат НСтр("ru = 'Не удалось проверить корректность токена интернет-эквайринга. Токен принадлежит другому пользователю интернет-поддержки.'");
	ИначеЕсли КодОшибки = "ОтсутствуетДоступКСервису"
		И КодОшибкиСервиса = "PAYMENT_GATEWAY_OZON_PAY_TOKEN_OWNER_MISMATCH" Тогда
		Возврат НСтр("ru = 'Не удалось проверить корректность токена Озон Пэй. Токен принадлежит другому пользователю интернет-поддержки.'");
	ИначеЕсли КодОшибки = "ОтсутствуетДоступКСервису"
		И КодОшибкиСервиса = "PAYMENT_GATEWAY_YANDEX_PAY_TOKEN_OWNER_MISMATCH" Тогда
		Возврат НСтр("ru = 'Не удалось проверить корректность токена Яндекс Пэй. Токен принадлежит другому пользователю интернет-поддержки.'");
	КонецЕсли;
...
//    * ТокенЯндексПэй - Строка - Токен подключения, используемый для оплаты через Яндекс Пэй;
...
Функция ПараметрыОперацииОбновленияНастройки(
	...
	ПараметрыОперации.Вставить("ТокенЯндексПэй", "");
...
Процедура ЗаполнитьТокен(
	...
	ИначеЕсли СпособОплаты = Перечисления.СпособыОплатыОнлайнЗаказов.ЯндексПэй Тогда
		ПараметрыОперации.ТокенЯндексПэй = РезультатОперации.Токен;
	КонецЕсли;
...
//
// Возвращаемое значение:
//  ФиксированныйМассив - параметры выбора настройки
//
...
Функция НовыйЗаказ(
	...
	Иначе
		...
		ПараметрыОперации.Вставить("ЗаказНаОплату", ЗаказНаОплату);
...
Функция ОбновитьЗаказ(
	...
	РезультатОперации = НовыйРезультатДанныеЗаказа();
	РезультатОперации.Вставить("ОшибкиВалидацииЗаказа", Новый Массив);
	РезультатОперации.Вставить("ЗаказНаОплату", Неопределено);
	...
	РезультатОперации.ЗаказНаОплату = ЗаказНаОплату;
...
Функция ОбновитьЗаказСВложениями(ПараметрыОперации) Экспорт
	...
	ПараметрыОперации.Вставить("ЗаказНаОплату", РезультатОбновления.ЗаказНаОплату);
...
Функция НовыйОписаниеТаблицыТовары(ОписаниеПредставленияТоваров)
	...
	Товары.Колонки.Добавить("ИдентификаторСтроки", ОбщегоНазначения.ОписаниеТипаСтрока(2048));
...
Функция НовоеОписаниеПредставленияТоваров()
	...
	КолонкаИдентификаторСтроки = НоваяКолонкаТовары();
	КолонкаИдентификаторСтроки.Представление = НСтр("ru = 'Идентификатор строки'");
	КолонкаИдентификаторСтроки.Идентификатор = "ИдентификаторСтроки";
	КолонкаИдентификаторСтроки.Видимость = Ложь;
	КолонкаИдентификаторСтроки.ПорядокОтображения = 6;
	...
	ВизуализацияТоваров.Добавить(КолонкаИдентификаторСтроки);
...
//      ** ШаблоныНазначения - см. НастройкиПлатежныхСистемСлужебный.НовыйШаблоныНазначений.
//      ** НазначениеПлатежа - Строка - Назначение платежа.
//    * ЯндексПэй - Структура, Неопределено - Параметры оплат через Яндекс Пэй,
//        Неопределено, если подсистема не внедрена, сценарий не используется
//        или не выбрана настройка подключения:
//      ** НастройкаПодключения - СправочникСсылка.НастройкиПодключенияКПлатежнымСистемам -
//           настройка подключения к Яндекс Пэй, выбранная для онлайн-заказов.
//      ** ШаблоныНазначения - см. НастройкиПлатежныхСистемСлужебный.НовыйШаблоныНазначений.
//      ** НазначениеПлатежа - Строка - Комментарий к платежу, доступный для просмотра в личном кабинете мерчанта.
//
Функция ПараметрыОплатЗаказа(ДокументЗаказа, НастройкиОплат)
	...
	ПараметрыОплат.Вставить("ЯндексПэй", Неопределено);
...
Функция СформироватьОписаниеЗаказаНаОплату(
	...
	Если ЗаказНаОплату.ПараметрыОплат.ЯндексПэй <> Неопределено Тогда
		ЗаказНаОплату.ПараметрыОплат.ЯндексПэй.Удалить("ШаблоныНазначения");
	КонецЕсли;
...
Процедура ПроверитьЗаказНаОплату(
	...
	Иначе
		...
		СрокЖизниЗаказа = УниверсальноеВремя(ЗаказНаОплату.ОбщиеПараметрыЗаказа.СрокЖизни, ЧасовойПоясСеанса());
		ТекущееВремяСеанса = УниверсальноеВремя(ТекущаяДатаСеанса(), ЧасовойПоясСеанса());
		
		Если (СрокЖизниЗаказа - ТекущееВремяСеанса) / 60 > 43200 Тогда
			
			РезультатОперации.КодОшибки = КодОшибкиНеверныйФорматЗапроса();
			РезультатОперации.СообщениеОбОшибке =
				НСтр("ru = 'Превышен максимальный срок жизни онлайн-заказа (30 дней).'");
			РезультатОперации.ИнформацияОбОшибке = НСтр("ru = 'При проверке онлайн-заказа возникли ошибки.'");
...
Процедура ЗаписатьПараметрыЗаказа(
	...
	ПараметрыЯндексПэй = НовыйПараметрыЯндексПэй(НастройкиОплаты);
	...
	Для Каждого Товар Из ДанныеЗаказа.Товары Цикл
		...
		ПроверитьСтрокуТоваровДляЯндексПэй(ПараметрыЯндексПэй, Товар);
	...
	Если НастройкиОплаты.СБП <> Неопределено Тогда
		...
		Для Каждого ВариантСБП Из НастройкиОплаты.СБП Цикл
			
			Если ВариантСБП.Ключ = ИдентификаторВариантаСБПC2B() Тогда
				
				ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("c2bPaymentPurpose");
				ЗаписьДанныхСообщения.ЗаписатьЗначение(ВариантСБП.Значение.НазначениеПлатежа);
				
			ИначеЕсли ВариантСБП.Ключ = ИдентификаторВариантаСБПB2B() Тогда
				
				ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("b2bPaymentPurpose");
				ЗаписьДанныхСообщения.ЗаписатьЗначение(ВариантСБП.Значение.НазначениеПлатежа);
				
				ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("b2bAccount");
				ЗаписьДанныхСообщения.ЗаписатьЗначение(ВариантСБП.Значение.РасчетныйСчет);
				
				ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("b2bTakeTax");
				ЗаписьДанныхСообщения.ЗаписатьЗначение(ВариантСБП.Значение.ОблагаетсяНДС);
				
				ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("b2bTotalTaxAmount");
				ЗаписьДанныхСообщения.ЗаписатьЗначение(ВариантСБП.Значение.СуммаНДС);
				
				ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("b2bUip");
				ЗаписьДанныхСообщения.ЗаписатьЗначение(ВариантСБП.Значение.ИдентификаторПлатежа);
	...
	// Параметры оплаты через Яндекс Пэй
	Если НеобходимоОтправитьЯндекс(ПараметрыЯндексПэй) Тогда
		
		ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("yandexPayParams");
		
		ЗаписьДанныхСообщения.ЗаписатьНачалоОбъекта();
		
		ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("comment");
		ЗаписьДанныхСообщения.ЗаписатьЗначение(НастройкиОплаты.ЯндексПэй.НазначениеПлатежа);
		
		// Идентификаторы номенклатуры
		ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("cartItems");
		
		ЗаписьДанныхСообщения.ЗаписатьНачалоМассива();
		
		Для Каждого Идентификатор Из ПараметрыЯндексПэй.Идентификаторы Цикл
			
			ЗаписьДанныхСообщения.ЗаписатьНачалоОбъекта();
			
			ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("productId");
		...
		ЗаписьДанныхСообщения.ЗаписатьКонецМассива();
		// Конец Идентификаторы номенклатуры
...
//    * ЯндексПэй - Структура, Неопределено - Параметры оплаты через Яндекс Пэй,
//        Неопределено в случае если настройка для оплат через Яндекс Пэй не выбрана.
// 
Функция НастройкиОплаты(
	...
	НастройкиОплатСервиса.Вставить("ЯндексПэй", Неопределено);
...
//            Может принимать следующие значения: "СБПc2b", "СБПb2b", "Наличные", "ПоРеквизитам",
//            "Эквайринг", "ОзонПэй", "ЯндексПэй", "Прочее".
...
// Возвращаемое значение:
//  Структура - результат получения данных онлайн-заказа:
//    * ДанныеЗаказов - Соответствие Из КлючИЗначение - Содержит публичные данные онлайн-заказов:
...
// Определяет строковый идентификатор для указанного способа оплаты онлайн-заказа.
//
// Параметры:
//   СпособОплаты - ПеречислениеСсылка.СпособыОплатыОнлайнЗаказов - Способ оплаты, для которого
//     требуется получить идентификатор
//
// Возвращаемое значение:
//   Строка - Строковый идентификатор способа оплаты
//
...
//    значения: "СБПc2b", "СБПb2b", "ИнтернетЭквайринг", "Эквайринг", "ОзонПэй", "ЯндексПэй".
//  
//  Возвращаемое значение:
...
Функция СпособОплатыПоИдентификатору(ИдентификаторСпособаОплаты) Экспорт
	...
	ИначеЕсли ИдентификаторСпособаОплаты = "ЯндексПэй" Тогда
		Возврат Перечисления.СпособыОплатыОнлайнЗаказов.ЯндексПэй;
	КонецЕсли;
...
Функция ОтправитьВложения(ПараметрыОперации) Экспорт
	...
	РезультатАктивации = АктивироватьЗаказ(
		...
		ДанныеВложений,
		ПараметрыОперации.ЗаказНаОплату);
	ЗаполнитьЗначенияСвойств(РезультатОперации, РезультатАктивации);
...
// Формирует параметры необходимые для контроля списка товаров на ограничения, накладываемые Яндекс Пэй
// 
// Параметры:
//  НастройкиОплаты - Структура - настройки оплат онлайн-заказа.
// 
// Возвращаемое значение:
//  Структура - Новый параметры яндекс пэй:
//   * ЕстьНастройкаЯндексПэй - Булево - признак того, что в настройке оплаты включено
//       использование оплаты через Яндекс Пэй.
//   * ЗаполненыИдентификаторыЯндекс - Булево - Истина, если во всех строках заполнены
//       идентификаторы для Яндекс Пэй
//   * Идентификаторы - Массив Из Строка - идентификаторы номенклатуры, которые
//       необходимо использовать при формировании платежной ссылки через Яндекс Пэй.
//   * Ошибки - Массив Из Строка - ошибки при проверке списка товаров.
//   * МодульЯндексПэйСлужебный - Неопределено, ОбщийМодуль - общий модуль, если ЕстьНастройкаЯндексПэй.
//   * ОбщаяСуммаЗаказа - Число - общая сумма заказа для проверки
// 
Функция НовыйПараметрыЯндексПэй(НастройкиОплаты)
	
	Результат = Новый Структура;

	Результат.Вставить(
		"ЕстьНастройкаЯндексПэй",
		НастройкиОплаты.ЯндексПэй <> Неопределено);
	Результат.Вставить("ЗаполненыИдентификаторыЯндекс", Истина);
	Результат.Вставить("Идентификаторы", Новый Массив);
	Результат.Вставить("Ошибки", Новый Массив);
	Результат.Вставить("МодульЯндексПэйСлужебный", Неопределено);
	Результат.Вставить("ОбщаяСуммаЗаказа", 0);
	
	Если Результат.ЕстьНастройкаЯндексПэй Тогда
		Результат.МодульЯндексПэйСлужебный = ОбщегоНазначения.ОбщийМодуль("ЯндексПэйСлужебный");
	КонецЕсли;
...
// Выполняет проверку строки товаров на соответствие требованиям Яндекс Пэй.
// 
// Параметры:
//  ПараметрыЯндексПэй - см. НовыйПараметрыЯндексПэй.
//  Товар - СтрокаТаблицыЗначений - проверяемая строка.
// 
Процедура ПроверитьСтрокуТоваровДляЯндексПэй(ПараметрыЯндексПэй, Товар)
	
	Если Не ПараметрыЯндексПэй.ЕстьНастройкаЯндексПэй Тогда
		// Если оплата через Яндекс Пэй не включена в настройке, то нет необходимости
		// выполнять какие-либо проверки.
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Товар.ИдентификаторСтроки) Тогда
		ОписаниеОшибки = ПараметрыЯндексПэй.МодульЯндексПэйСлужебный.ОшибкаСтрокиКорзиныОнлайнЗаказы(Товар);
		Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
			ПараметрыЯндексПэй.Ошибки.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1: %2'"),
					Товар.ИдентификаторСтроки,
					ОписаниеОшибки));
		Иначе
			ПараметрыЯндексПэй.Идентификаторы.Добавить(Товар.ИдентификаторСтроки);
		КонецЕсли;
	Иначе
		ПараметрыЯндексПэй.ЗаполненыИдентификаторыЯндекс = Ложь;
	КонецЕсли;
	
	ПараметрыЯндексПэй.ОбщаяСуммаЗаказа = ПараметрыЯндексПэй.ОбщаяСуммаЗаказа + Товар.СуммаСоСкидкой;
			
КонецПроцедуры

// Определяет, нужно ли выполнять отправку параметров, необходимых для оплаты через Яндекс Пэй.
// Проверяется включение способа оплаты в настройке и правильность заполнения товаров.
// Если все условия выполнены, то возвращается Истина.
// Если использование Яндекс Пэй не включено, то просто возвращается Ложь.
// Если использование Яндекс Пэй включено, но есть ошибки проверки введенных данных,
//  то дополнительно в журнал регистрации заносится описание ошибок.
// 
// Параметры:
//  ПараметрыЯндексПэй - см. НовыйПараметрыЯндексПэй.
// 
// Возвращаемое значение:
//  Булево - Истина, если требуется отправить параметры оплаты через Яндекс Пэй.
// 
Функция НеобходимоОтправитьЯндекс(ПараметрыЯндексПэй)
	
	Если Не ПараметрыЯндексПэй.ЕстьНастройкаЯндексПэй Тогда
		// Если оплата через Яндекс Пэй не включена в настройке, то нет необходимости
		// дальнейшей проверки и занесения информации в журнал.
		Возврат Ложь;
	КонецЕсли;
	
	Ошибки = Новый Массив;
	Если Не ПараметрыЯндексПэй.ЗаполненыИдентификаторыЯндекс
		Или ПараметрыЯндексПэй.Идентификаторы.Количество() = 0 Тогда
		Ошибки.Добавить(
			НСтр("ru = 'Заполнены не все идентификаторы номенклатуры для Яндекс Пэй'"));
	КонецЕсли;

	ОшибкаСуммы = ПараметрыЯндексПэй.МодульЯндексПэйСлужебный.ОшибкаСуммаКорзиныВнешний(
		ПараметрыЯндексПэй.ОбщаяСуммаЗаказа);
	Если ЗначениеЗаполнено(ОшибкаСуммы) Тогда
		Ошибки.Добавить(ОшибкаСуммы);
	КонецЕсли;
	
	Если ПараметрыЯндексПэй.Ошибки.Количество() > 0 Тогда
		Ошибки.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Строки списка товаров с ошибками:
				|%1'"),
				СтрСоединить(ПараметрыЯндексПэй.Ошибки, Символы.ПС)));
	КонецЕсли;
	
	Если Ошибки.Количество() > 0 Тогда
		
		ТекстДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В списке товаров есть ошибки, не позволяющие использовать оплату через Яндекс Пэй:'"),
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПлатежныеСистемы.БазоваяФункциональностьПлатежныхСистем") Тогда
			...
			Если МодульПлатежныеСистемыСлужебный.ЭтоНастройкаПодключения(НастройкаПодключения)
				...
				Если ИдентификаторПлатежнойСистемы = МодульНастройкиПлатежныхСистемКлиентСервер.ИмяПодсистемыОзонПэй() Тогда
					Возврат Перечисления.СпособыОплатыОнлайнЗаказов.ОзонПэй;
				ИначеЕсли ИдентификаторПлатежнойСистемы = НастройкиПлатежныхСистемКлиентСервер.ИмяПодсистемыЯндексПэй() Тогда
					Возврат Перечисления.СпособыОплатыОнлайнЗаказов.ЯндексПэй;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
