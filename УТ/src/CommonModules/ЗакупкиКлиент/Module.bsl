...
Процедура ОбработатьУказаниеНомераГТД(Форма,
	...
	Если ДействияОбработки.Количество() > 0 Тогда
		ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ТекущиеДанные, ДействияОбработки, Неопределено);
	КонецЕсли;
...
Процедура ОткрытьФормуСозданияДоговора(Форма, Элемент, ПутьКДоговору, Партнер, СтандартнаяОбработка, ИмяСобытияОповещения = "ПриИзмененииДоговора") Экспорт
	...
	ИменаДополнительныхПараметровВыбора = Новый Массив;
	ИменаДополнительныхПараметровВыбора.Добавить("Соглашение");
	
	Для Каждого ПараметрВыбора Из Элемент.ПараметрыВыбора Цикл
		Если СтрНайти(ПараметрВыбора.Имя,"Отбор.") <> 0
			Или ИменаДополнительныхПараметровВыбора.Найти(ПараметрВыбора.Имя) <> Неопределено Тогда
