...
Функция ПолучитьКонтрагентаПартнераПоУмолчанию(Партнер) Экспорт
	...
	ДанныеСтроки.Вставить("Контрагент", Справочники.Контрагенты.ПустаяСсылка());
	
	Действия = Новый Структура;
	Действия.Вставить("ЗаполнитьКонтрагентаПартнераПоУмолчанию");
	...
	Возврат ДанныеСтроки.Контрагент;
...
Процедура ЗаполнитьСписокПартнераСоВсехИерархией(Партнер, СписокПартнераСРодителями) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов") Тогда
		...
		Контрагент = ПолучитьКонтрагентаПартнераПоУмолчанию(Партнер);
		ГоловнойКонтрагент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "ГоловнойКонтрагент");
		Контрагент = ?(ЗначениеЗаполнено(ГоловнойКонтрагент), ГоловнойКонтрагент, Контрагент);
		
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	Контрагенты.Партнер КАК Партнер,
		|	ПРЕДСТАВЛЕНИЕ(Контрагенты.Партнер) КАК Представление
		|ИЗ
...
//  ОбратнаяОперация  - Булево - признак того, что хозяйственная операция используется как обратная.
//
// Возвращаемое значение:
//   Строка   - сформированный заголовок.
//
Функция ЗаголовокРеквизитаПартнерВЗависимостиОтХозяйственнойОперации(ХозяйственнаяОперация, ОбратнаяОперация = Ложь) Экспорт
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтКомиссионера Тогда
		ЗаголовокПартнер = НСтр("ru = 'Комиссионер'");
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОприходованиеИзлишковТоваровВПользуКомитента Тогда
		ЗаголовокПартнер = НСтр("ru = 'Комиссионер'");
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеНедостачЗаСчетКомитента Тогда
		ЗаголовокПартнер = НСтр("ru = 'Комиссионер'");
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаХранениеСПравомПродажи Тогда
		ЗаголовокПартнер = ?(Не ОбратнаяОперация, НСтр("ru = 'Хранитель'"), НСтр("ru = 'Клиент'"));
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПересортицаТоваровУХранителя Тогда
		ЗаголовокПартнер = НСтр("ru = 'Клиент'");
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПорчаТоваровУХранителя Тогда
		ЗаголовокПартнер = НСтр("ru = 'Клиент'");
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоставкаПодПринципала Тогда
		ЗаголовокПартнер = ?(Не ОбратнаяОперация, НСтр("ru = 'Клиент'"), НСтр("ru = 'Принципал'"));
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОприходованиеИзлишковТоваровВПользуПоклажедателя Тогда
		ЗаголовокПартнер = НСтр("ru = 'Клиент'");
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеНедостачЗаСчетПоклажедателя Тогда
		ЗаголовокПартнер = НСтр("ru = 'Клиент'");
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыкупПринятыхНаХранениеТоваров Тогда
		ЗаголовокПартнер = НСтр("ru = 'Поставщик'");
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтгрузкаПринятыхСПравомПродажиТоваровСХранения Тогда
		ЗаголовокПартнер = НСтр("ru = 'Поставщик'");
	Иначе
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов") Тогда
...
// Проверяет заполнение партнера. Вызывается из процедуры "ОбработкаПроверкиЗаполнения".
//
// Параметры:
//  Объект - ДокументОбъект - объект, в котором необходимо проверить партнера.
//  МассивНепроверяемыхРеквизитов - Массив из Строка - коллекция имен реквизитов объекта, не требующих проверки.
//  Отказ - Булево - флаг отказа в процедуре проверки заполнения.
//  ОбратнаяОперация  - Булево - признак того, что хозяйственная операция используется в документе как обратная.
//
Процедура ПроверитьЗаполнениеПартнера(Объект, МассивНепроверяемыхРеквизитов, Отказ, ОбратнаяОперация = Ложь) Экспорт
	
	Если Не ЗначениеЗаполнено(Объект.Партнер) Тогда
		
		МассивНепроверяемыхРеквизитов.Добавить("Партнер");
		
		ЗаголовокПартнер = ЗаголовокРеквизитаПартнерВЗависимостиОтХозяйственнойОперации(
			Объект.ХозяйственнаяОперация, ОбратнаяОперация);
		ТекстОшибки = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, , ЗаголовокПартнер);
...
//
Процедура ДополнитьТекстЗапросаЗаполнитьКонтрагентаПартнераПоУмолчанию(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов") Тогда
		...
		|		ПО ИсточникДанных.Партнер = Контрагенты.Партнер";
		ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ЗаполнитьКонтрагентаПартнераПоУмолчанию");
	Иначе
		
		ТекстЗапроса = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ИсточникДанных.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
		|	ИсточникДанных.Контрагент КАК ТекущийКонтрагент,
		|	&КонтрагентИзСоглашения КАК КонтрагентИзСоглашения,
		|	ЕСТЬNULL(ИерархияПартнеров.Уровень, 0) КАК УровеньВИерархии,
		|	ЕСТЬNULL(ИерархияПартнеров.Родитель, ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)) КАК ПартнерИерархии
		|ПОМЕСТИТЬ ПараметрыПоискаКонтрагентаПартнераПоУмолчанию
		|ИЗ
		|	ВтИсточникДанных КАК ИсточникДанных
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИерархияПартнеров КАК ИерархияПартнеров
		|		ПО ИсточникДанных.Партнер = ИерархияПартнеров.Партнер";
		
		Если ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("Соглашение", КэшированныеЗначения) Тогда 
		
			// Необходимо проверять использование договора по соглашению
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
				"&КонтрагентИзСоглашения",
				"ЕСТЬNULL(
				|	ВЫРАЗИТЬ(ИсточникДанных.Соглашение КАК Справочник.СоглашенияСПоставщиками).Контрагент,
		...
		ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ПараметрыПоискаКонтрагентаПартнераПоУмолчанию");
		
		ТекстЗапроса = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ПараметрыПоиска.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
		|	ПараметрыПоиска.УровеньВИерархии КАК УровеньВИерархии,
		|	ПараметрыПоиска.КонтрагентИзСоглашения КАК КонтрагентИзСоглашения,
		|	Контрагенты.Ссылка КАК Контрагент, // Не обрабатываем null, чтобы не усложнять расчет количества контрагентов на каждом уровне иерархии.
		|	ЕСТЬNULL(ПараметрыПоиска.ТекущийКонтрагент = Контрагенты.Ссылка, ЛОЖЬ) КАК ЭтоТекущийКонтрагент
		|ПОМЕСТИТЬ ДоступныеКонтрагентыПартнера
		|ИЗ
		|	ПараметрыПоискаКонтрагентаПартнераПоУмолчанию КАК ПараметрыПоиска
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Контрагенты
		|		ПО ПараметрыПоиска.ПартнерИерархии = Контрагенты.Партнер
		|		И НЕ Контрагенты.ПометкаУдаления";
		ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ДоступныеКонтрагентыПартнера");
		
		ТекстЗапроса = 
		"ВЫБРАТЬ
		|	// Выбор наиболее близкого ""родителя"", с контрагентами при условии, что количество его контрагентов = 1
		|	ОписаниеУровнейИерархии.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
		|	&МаксимальныйУровеньСЕдинственнымКонтрагентом КАК УровеньВИерархии
		|ПОМЕСТИТЬ ОписаниеУровнейИсточников
		|ИЗ
		|	(ВЫБРАТЬ
		|		ДоступныеКонтрагентыПартнера.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
		|		ДоступныеКонтрагентыПартнера.УровеньВИерархии КАК УровеньВИерархии,
		|		КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ДоступныеКонтрагентыПартнера.Контрагент) = 1 КАК ЕдинственныйКонтрагент,
		|		КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ДоступныеКонтрагентыПартнера.Контрагент) > 1 КАК МножествоКонтрагентов
		|	ИЗ
		|		ДоступныеКонтрагентыПартнера КАК ДоступныеКонтрагентыПартнера
		|	СГРУППИРОВАТЬ ПО
		|		ДоступныеКонтрагентыПартнера.ИдентификаторСтрокиВТ,
		|		ДоступныеКонтрагентыПартнера.УровеньВИерархии) КАК ОписаниеУровнейИерархии
		|СГРУППИРОВАТЬ ПО
		|	ОписаниеУровнейИерархии.ИдентификаторСтрокиВТ
		|ИМЕЮЩИЕ
		|	&МаксимальныйУровеньСЕдинственнымКонтрагентом > &МаксимальныйУровеньСМножествомКонтрагентов";
		
		ШаблонПолученияУровня =  
		"МАКСИМУМ(
		|	ВЫБОР КОГДА ОписаниеУровнейИерархии.%1 ТОГДА 
		|		ОписаниеУровнейИерархии.УровеньВИерархии
		|	ИНАЧЕ // Значение отсутствует. Выбираем значение, меньшее, чем возможное:
		|		-1
		|	КОНЕЦ)";
		
		ТекстЗапроса = СтрЗаменить(
			ТекстЗапроса,
			"&МаксимальныйУровеньСЕдинственнымКонтрагентом",
			СтрШаблон(ШаблонПолученияУровня, "ЕдинственныйКонтрагент"));
		
		ТекстЗапроса = СтрЗаменить(
			ТекстЗапроса,
			"&МаксимальныйУровеньСМножествомКонтрагентов",
			СтрШаблон(ШаблонПолученияУровня, "МножествоКонтрагентов"));
			
		ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ОписаниеУровнейИсточников");
		
		ТекстЗапроса = 
		"ВЫБРАТЬ
		|	ДоступныеКонтрагентыПартнера.ИдентификаторСтрокиВТ,
		|	ЕСТЬNULL( // 1. Выбор текущего указанного контрагента:
		|		МАКСИМУМ(ВЫБОР КОГДА ДоступныеКонтрагентыПартнера.ЭтоТекущийКонтрагент ТОГДА
		|			 ДоступныеКонтрагентыПартнера.Контрагент
		|		ИНАЧЕ NULL
		|	КОНЕЦ),
		|		// 2. Выбор контрагента из соглашения, если заполнено соглашение и контрагент в нем:
		|		ВЫБОР КОГДА МАКСИМУМ(ДоступныеКонтрагентыПартнера.КонтрагентИзСоглашения) <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка) ТОГДА
		|			МАКСИМУМ(ДоступныеКонтрагентыПартнера.КонтрагентИзСоглашения)
		|		ИНАЧЕ
		|			МАКСИМУМ(ВЫБОР КОГДА ОписаниеУровнейИсточников.УровеньВИерархии ЕСТЬ НЕ NULL ТОГДА
		|				// 3. Контрагент из ближайшего ""родителя"" с единственным контрагентом:
		|				ДоступныеКонтрагентыПартнера.Контрагент
		|			ИНАЧЕ // 4. Нет возможности автоматически выбрать значение:
		|				ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
		|			КОНЕЦ)
		|	КОНЕЦ) КАК Контрагент
		|ИЗ
		|	ДоступныеКонтрагентыПартнера КАК ДоступныеКонтрагентыПартнера
		|		ЛЕВОЕ СОЕДИНЕНИЕ ОписаниеУровнейИсточников КАК ОписаниеУровнейИсточников
		|		ПО ДоступныеКонтрагентыПартнера.ИдентификаторСтрокиВТ = ОписаниеУровнейИсточников.ИдентификаторСтрокиВТ
		|		И ДоступныеКонтрагентыПартнера.УровеньВИерархии = ОписаниеУровнейИсточников.УровеньВИерархии
		|СГРУППИРОВАТЬ ПО
		|	ДоступныеКонтрагентыПартнера.ИдентификаторСтрокиВТ";
		ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ЗаполнитьКонтрагентаПартнераПоУмолчанию");
	КонецЕсли;
