...
Процедура ЗаполнитьПоказательКоличествоПоОрдерам(ТаблицаПриемник, ТаблицаИсточник, ПоляКлюча) Экспорт
	...
	Если ТаблицаПриемник.Колонки.Найти("КоличествоВОрдере") = Неопределено Тогда
		ТаблицаПриемник.Колонки.Добавить("КоличествоВОрдере", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
	КонецЕсли;
	Если ТаблицаПриемник.Колонки.Найти("Собирается") = Неопределено Тогда
		ТаблицаПриемник.Колонки.Добавить("Собирается", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
	КонецЕсли;
...
// Возвращает дату поступления по накладной.
// 
// Параметры:
//  ДатаПоступления - Дата - поле ДатаПоступления в документе,
//  ДатаДокумента - Дата - поле Дата в документе,
//  ЗаменятьПустуюДатуДокументаТекущей - Булево - Истина, если пустую дата документа нужно заменять на текущую,
//                                                это нужно, например, когда метод вызывается в обработке заполнения
//                                                и дата еще не назначена платформой. 
// Возвращаемое значение:
//  Дата - Дата поступления.
Функция ДатаПоступления(ДатаПоступления, ДатаДокумента, ЗаменятьПустуюДатуДокументаТекущей = Ложь) Экспорт
	
	ДатаДокументаТекущая = ?(ДатаДокумента <> '00010101' Или Не ЗаменятьПустуюДатуДокументаТекущей,
		НачалоДня(ДатаДокумента), НачалоДня(ТекущаяДатаСеанса()));
	Возврат ?(ЗначениеЗаполнено(ДатаПоступления), НачалоДня(ДатаПоступления), ДатаДокументаТекущая);
...
Функция ТекстЗапросаСостоянияРасходныхОрдеров(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению, ПараметрыЗамены = Неопределено)
	...
	ДопУсловие = "ИСТИНА";
	Если ПараметрыОтбора.Подразделение Тогда
		ДопУсловие = ДопУсловие +
		" И (Получатель В (&Подразделение)
		|	ИЛИ (ВЫРАЗИТЬ(Получатель КАК Справочник.Склады).ЦеховаяКладовая
		|		И ВЫРАЗИТЬ(Получатель КАК Справочник.Склады).Подразделение В (&Подразделение)))";
	КонецЕсли;
	
	ДополнитьТекстОтборомПоОстаткам(ТекстЗапроса, "ДокументОтгрузки", ТипыРаспоряжений, ПараметрыОтбора.Склад, ДопУсловие, Ложь, Истина);
...
Функция ТекстЗапросаСостоянияПриходныхОрдеров(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению, ПараметрыЗамены = Неопределено)
	...
	Если ПараметрыОтбора.ХозОперация Тогда
		ДопУсловие = ДопУсловие +
		" И ХозяйственнаяОперация В (&ХозОперация)";
	КонецЕсли;
	
	Если ПараметрыОтбора.Подразделение Тогда
		ДопУсловие = ДопУсловие +
		" И (Отправитель В (&Подразделение)
		|	ИЛИ (ВЫРАЗИТЬ(Отправитель КАК Справочник.Склады).ЦеховаяКладовая
		|		И ВЫРАЗИТЬ(Отправитель КАК Справочник.Склады).Подразделение В (&Подразделение)))";
	КонецЕсли;
...
Функция ТекстЗапросаСостоянияОтгрузок(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению)
	...
	Если Не ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат) Тогда
	...
	|		Остатки.Серия КАК Серия
	|	ИЗ
	|		#РаспоряженияНаОтгрузкуОборотыРазвернутая КАК Остатки
	|	ГДЕ
	|		(Остатки.КОтгрузкеКоличествоОборот -
	|			Остатки.ОтгруженоКоличествоОборот) <> 0
	...
	|				И (НЕ &ОтбиратьПоХозОперации
	|				ИЛИ РеестрДокументов.ХозяйственнаяОперация В (&ХозОперация))
	|				И (НЕ &ОтбиратьПоОрганизации
	|				ИЛИ РеестрДокументов.Организация В (&Организация))
	|				И (НЕ &ОтбиратьПоПодразделению
	|				ИЛИ РеестрДокументов.Подразделение В (&Подразделение))
	|				И (НЕ &ОтбиратьПоМенеджеру
	|				ИЛИ РеестрДокументов.Ответственный В (&Менеджер)))) КАК ВложенныйЗапрос
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|			И РаспоряженияНаОтгрузку.Показатель = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Отгружено)
	|		ИМЕЮЩИЕ
	|			СУММА(РаспоряженияНаОтгрузку.Количество) > 0)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ЗаказКлиента,
	|	Склад
