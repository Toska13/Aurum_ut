...
Функция ТекстЗапросаПартииНДСКРаспределению(Запрос)
	...
	"ВЫБРАТЬ
	|	ТаблицаИзмененийПартииНДСКРаспределению.Организация КАК Организация,
	|	ВЫБОР 
	|		КОГДА Организации.ДатаРегистрации = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ НАЧАЛОПЕРИОДА(Организации.ДатаРегистрации, МЕСЯЦ) = НАЧАЛОПЕРИОДА(ТаблицаИзмененийПартииНДСКРаспределению.Период, МЕСЯЦ)
	|			И РАЗНОСТЬДАТ(КОНЕЦПЕРИОДА(ТаблицаИзмененийПартииНДСКРаспределению.Период, КВАРТАЛ), Организации.ДатаРегистрации, ДЕНЬ) <= 10 
	|	КОНЕЦ КАК ЭтоРасширенныйПериод
	|ПОМЕСТИТЬ ВтОперацияНаходитсяВРасширенномПериоде
	|ИЗ
	|	ТаблицаИзмененийПартииНДСКРаспределению КАК ТаблицаИзмененийПартииНДСКРаспределению
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Организации КАК Организации
	|		ПО (Организации.Ссылка = ТаблицаИзмененийПартииНДСКРаспределению.Организация)
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА ВтОперацияНаходитсяВРасширенномПериоде.Организация ЕСТЬ NULL
	|			ТОГДА ИзменениеПартий.Период
	|		ИНАЧЕ ДОБАВИТЬКДАТЕ(КОНЕЦПЕРИОДА(ИзменениеПартий.Период, КВАРТАЛ), СЕКУНДА, 1)
	|	КОНЕЦ КАК Период,
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтОперацияНаходитсяВРасширенномПериоде КАК ВтОперацияНаходитсяВРасширенномПериоде
	|		
	|	ПО ИзменениеПартий.Организация = ВтОперацияНаходитсяВРасширенномПериоде.Организация
	|		И (ВтОперацияНаходитсяВРасширенномПериоде.ЭтоРасширенныйПериод)
	|ГДЕ
	...
	ТекстУничтожитьВт = "ПериодыНастройкиУчетаНДСПартииНДСКРаспределению, ВтОперацияНаходитсяВРасширенномПериоде";
	...
	|		ИНАЧЕ КОНЕЦПЕРИОДА(ДОБАВИТЬКДАТЕ(КОНЕЦПЕРИОДА(Таблица.Период, МЕСЯЦ), СЕКУНДА, 1), МЕСЯЦ)
	|	КОНЕЦ КАК Месяц,
	|	ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.РаспределениеНДС) КАК Операция,
	|	Таблица.Организация КАК Организация,
	|	Таблица.Регистратор КАК Документ
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтОперацияНаходитсяВРасширенномПериоде КАК ВтОперацияНаходитсяВРасширенномПериоде
	|	
	|	ПО Таблица.Организация = ВтОперацияНаходитсяВРасширенномПериоде.Организация
	|		И (ВтОперацияНаходитсяВРасширенномПериоде.ЭтоРасширенныйПериод)
	|ГДЕ
...
// Определяет, находится Операция в расширенном налоговом периоде.
//
// Параметры:
...
//
// Возвращаемое значение:
//   Булево - Истина, если организация находится в расширенном периоде
//
Функция ОперацияНаходитсяВРасширенномПериоде(Организация, ДатаОперации, ДатаРегистрации = '00010101') Экспорт
	
	ЭтоРасширенныйПериод = Ложь;
...
Процедура ВыполнитьРасчет(ПараметрыРасчета)
	...
	СформироватьТаблицуРасширенияДокументовОплат(ПараметрыРасчета);
...
Процедура ПолучитьДокументыСторно(ПараметрыРасчета)
	...
	|			ИЛИ НДСПредъявленный.СчетФактура ССЫЛКА Документ.ОперацияСЭлектроннымБилетом)
	|ГДЕ
	|	РеестрДокументов.ДатаДокументаИБ МЕЖДУ &НачалоПериода И &КонецПериода
	|	И РеестрДокументов.СторноИсправление
	|	И НЕ РеестрДокументов.ДополнительнаяЗапись
...
Процедура СформироватьТаблицуРасширенияДокументовОплат(ПараметрыРасчета)
	...
	|	ИсправленияДокументов.ИсправляемыйДокумент КАК ИсходныйДокументОплаты
	|ПОМЕСТИТЬ ИсходныеДокументыОплаты
	|ИЗ
	|	Задания КАК Задания
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИсправленияДокументов КАК ИсправленияДокументов
	|		ПО Задания.СчетФактура = ИсправленияДокументов.Регистратор
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументОплаты
	|;
	...
	|	Задания.СчетФактура КАК ДокументОплаты,
	|	ВЫБОР
	|		КОГДА СчетФактураВыданныйАванс.Ссылка ЕСТЬ NULL
	|		И СчетФактураПолученныйАванс.Ссылка ЕСТЬ NULL
	|			ТОГДА ЕСТЬNULL(ИсходныеДокументыОплаты.ИсходныйДокументОплаты, Задания.СчетФактура)
	|		ИНАЧЕ Задания.СчетФактура
	|	КОНЕЦ КАК ИсходныйДокументОплаты
	|ПОМЕСТИТЬ РасширениеТаблицыДокументовОплат
	|ИЗ
	|	Задания КАК Задания
	|		ЛЕВОЕ СОЕДИНЕНИЕ ИсходныеДокументыОплаты КАК ИсходныеДокументыОплаты
	|		ПО Задания.СчетФактура = ИсходныеДокументыОплаты.ДокументОплаты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураПолученныйАванс КАК СчетФактураПолученныйАванс
	|		ПО Задания.СчетФактура = СчетФактураПолученныйАванс.ДокументОснование
	|		И СчетФактураПолученныйАванс.Проведен = ИСТИНА
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданныйАванс КАК СчетФактураВыданныйАванс
	|		ПО Задания.СчетФактура = СчетФактураВыданныйАванс.ДокументОснование
	|		И СчетФактураВыданныйАванс.Проведен = ИСТИНА
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	ДокументОплаты,
	|	ИсходныйДокументОплаты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ИсходныеДокументыОплаты";
...
Процедура ЗафиксироватьРезультат(ПараметрыРасчета)
	...
	|	&НомерЗадания                                  КАК НомерЗадания,
	|	Задания.Организация                            КАК Организация,
	|	РеестрДокументов.Регистратор                  КАК СчетФактура
	|ИЗ
	|	ИсходныеЗадания КАК Задания
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИсправленияДокументов КАК РеестрДокументов
	|	ПО Задания.СчетФактура = РеестрДокументов.ИсправляемыйДокумент
	|		И (РеестрДокументов.Период > &КонецПериода)
	|		И (РеестрДокументов.Активность)
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСКлиентамиПоСрокам КАК РасчетыСКлиентами
	|	ПО РасчетыСКлиентами.Период > &КонецПериода
	|		И РасчетыСКлиентами.РасчетныйДокумент = РеестрДокументов.ИсправляемыйДокумент
	|		И РасчетыСКлиентами.Активность
	|		И РасчетыСКлиентами.ПредоплатаРегл <> 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСПоставщикамиПоСрокам КАК РасчетыСПоставщиками
	|	ПО РасчетыСПоставщиками.Период > &КонецПериода
	|		И РасчетыСПоставщиками.РасчетныйДокумент = РеестрДокументов.Регистратор
	|		И РасчетыСПоставщиками.Активность
	|		И РасчетыСПоставщиками.ПредоплатаРегл <> 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
...
Процедура СформироватьНДСПредъявленный(ПараметрыРасчета)
	...
	ПолучитьНалоговуюБазуРеализации0(ПараметрыРасчета);
...
Процедура ПолучитьРазрезыВычетаНДС(ПараметрыРасчета)
	...
	// Для организаций с льготным налогообложением вычет не предоставляется.
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("НачалоПериода", ПараметрыРасчета.НачалоПериода);
	Запрос.УстановитьПараметр("КонецПериода",  ПараметрыРасчета.КонецПериода);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	КешРазрезыВычетаНДС.Организация КАК Организация,
	|	КешРазрезыВычетаНДС.СчетФактура КАК СчетФактура,
	|	КешРазрезыВычетаНДС.ВидДеятельностиНДС КАК ВидДеятельностиНДС,
	|	КешРазрезыВычетаНДС.ВидЦенности КАК ВидЦенности,
	|	КешРазрезыВычетаНДС.РеализацияЭкспорт КАК РеализацияЭкспорт,
	|	КешРазрезыВычетаНДС.ИсправленныйСчетФактура КАК ИсправленныйСчетФактура,
	|	КешРазрезыВычетаНДС.Событие КАК Событие,
	|	КешРазрезыВычетаНДС.Период КАК Период,
	|	КешРазрезыВычетаНДС.Регистратор КАК Регистратор
	|ПОМЕСТИТЬ КешРазрезыВычетаНДС
	|ИЗ
	|	РазрезыВычетаНДС КАК КешРазрезыВычетаНДС
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНастройкиУчетаНДСПриУСН КАК НастройкиУчетаНДСПриУСН
	|		ПО КешРазрезыВычетаНДС.Организация = НастройкиУчетаНДСПриУСН.Организация
	|			И (КешРазрезыВычетаНДС.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияНДСПредъявленный.НДСПринятКВычету))
	|			И НастройкиУчетаНДСПриУСН.Период МЕЖДУ &НачалоПериода И &КонецПериода
	...
	|ПОМЕСТИТЬ РазрезыВычетаНДС
	|ИЗ
	|	КешРазрезыВычетаНДС КАК КешРазрезыВычетаНДС
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ КешРазрезыВычетаНДС";
...
Функция РассчитатьЗаписиНДСПредъявленныйСторно(ПараметрыРасчета)
	...
	|		И НДСПредъявленный.ИсправленныйСчетФактура = ПрошлыеСторноИсправительногоСчетаФактуры.СторнируемыйДокумент
	...
	|	И НЕ (ТИПЗНАЧЕНИЯ(НДСПредъявленный.Регистратор) = ТИП(Документ.РаспределениеНДС)
	...
	|	ЕСТЬNULL(ИсправительныеCФ.ИсправленныйСчетФактура, НДСПредъявленный.ИсправленныйСчетФактура)    КАК ИсправленныйСчетФактура,
	...
	|	НДСПредъявленный.Партия КАК Партия,
	|	НДСПредъявленный.АналитикаУчетаПартий КАК АналитикаУчетаПартий,
	...
	|		ПО
	|			НДСПредъявленный.Организация = ИсправительныеCФ.Организация
	|			И НДСПредъявленный.СчетФактура = ИсправительныеCФ.СчетФактура
	|
	|ГДЕ
...
Процедура ПолучитьНалоговуюБазуРеализации0(ПараметрыРасчета)
	
	УчетНДСЛокализация.ПолучитьНалоговуюБазуРеализации0(ПараметрыРасчета);
	
	МенеджерВременныхТаблиц = ПараметрыРасчета.МенеджерВременныхТаблиц;
	Если МенеджерВременныхТаблиц.Таблицы.Найти("НалоговаяБазаРеализации0") = Неопределено Тогда
...
Функция РассчитатьЗаписиНДСПредъявленныйИзменениеВидаДеятельности(ПараметрыРасчета)
	...
	|		ИНАЧЕ
	|			ИСТИНА
	|	КОНЕЦ                              КАК ОтнестиКПредыдущемуКварталу,
	|	ВЫБОР
	|		КОГДА ИсправлениеРаспределениеНДС.Ссылка ЕСТЬ NULL 
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ
	...
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ
	|			ИсправлениеРаспределениеНДС.Дата
	|	КОНЕЦ                              КАК ДатаИсправленияРаспределенияНДС,
	...
	|	ПО
	|		ДД.Регистратор = ИсправлениеРаспределениеНДС.Ссылка
...
Функция РассчитатьЗаписиНДСПредъявленныйИзменениеНаправленияДеятельности(ПараметрыРасчета)
	...
	|		И Партии.ИдентификаторФинЗаписи = ИзменениеВидаДеятельности.ИдентификаторФинЗаписи
	|ГДЕ
...
Функция РассчитатьЗаписиНДСПредъявленныйПоИСФ(ПараметрыРасчета)
	...
	|	НДСПредъявленный.АналитикаРасходов,
	|	НДСПредъявленный.ИдентификаторФинЗаписи,
	|	НДСПредъявленный.Партия,
	|	НДСПредъявленный.АналитикаУчетаПартий,
	|	НДСПредъявленный.НастройкаХозяйственнойОперации
	|
	|ИМЕЮЩИЕ 
...
Функция РассчитатьЗаписиНДСПредъявленныйВосстановлениеПодВидДеятельности(ПараметрыРасчета)
	...
	УчетНДСЛокализация.РассчитатьЗаписиНДСПредъявленныйВосстановлениеПодВидДеятельности(ПараметрыРасчета);
	
	МенеджерВременныхТаблиц = ПараметрыРасчета.МенеджерВременныхТаблиц;
	Если МенеджерВременныхТаблиц.Таблицы.Найти("НДСПредъявленныйВосстановлениеПодВидДеятельности") = Неопределено Тогда
		...
		|	ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка) КАК ВидЦенности,
		|	ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка) КАК СтавкаНДС,
		|	ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ПустаяСсылка) КАК ВидДеятельностиНДС,
		|	НЕОПРЕДЕЛЕНО КАК ИсправленныйСчетФактура,
		|	НЕОПРЕДЕЛЕНО КАК РеализацияЭкспорт,
		|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) КАК НаправлениеДеятельности,
		|	ЛОЖЬ КАК ОтнестиКПредыдущемуКварталу,
		|	ДАТАВРЕМЯ(1, 1, 1) КАК ИсправляемыйПериод,
		|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаИсправленияРаспределенияНДС,
		|	0 КАК СуммаБезНДС,
		|	0 КАК НДС,
		|	0 КАК НДСУпр,
...
Процедура ПолучитьОстаткиНДСПредъявленныйНаКонецПериода(ПараметрыРасчета)
	...
	|	НДСПредъявленный.РеализацияЭкспорт,
	|	НДСПредъявленный.НаправлениеДеятельности,
	|	МАКСИМУМ(НДСПредъявленный.ОтнестиКПредыдущемуКварталу)      КАК ОтнестиКПредыдущемуКварталу,
	|	МАКСИМУМ(НДСПредъявленный.ИсправляемыйПериод)               КАК ИсправляемыйПериод,
	|	МАКСИМУМ(НДСПредъявленный.ДатаИсправленияРаспределенияНДС)  КАК ДатаИсправленияРаспределенияНДС,
	|	СУММА(НДСПредъявленный.СуммаБезНДС) КАК СуммаБезНДС,
	|	СУММА(НДСПредъявленный.НДС) КАК НДС,
	|	СУММА(НДСПредъявленный.НДСУпр) КАК НДСУпр
	...
	|		ДД.РеализацияЭкспорт,
	|		ДД.НаправлениеДеятельности,
	|		ДД.ОтнестиКПредыдущемуКварталу,
	|		ДД.ИсправляемыйПериод,
	|		ДД.ДатаИсправленияРаспределенияНДС,
	|		ВЫБОР
	|			КОГДА ДД.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|				ТОГДА ДД.СуммаБезНДС
	|			ИНАЧЕ -ДД.СуммаБезНДС
...
Функция РассчитатьЗаписиНДСПредъявленныйПринятиеКВычету(ПараметрыРасчета)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	БлокировкаВычетаНДСРеализацияЭкспорт.Организация КАК Организация,
	|	БлокировкаВычетаНДСРеализацияЭкспорт.СчетФактура КАК СчетФактура,
	|	БлокировкаВычетаНДСРеализацияЭкспорт.РеализацияЭкспорт КАК РеализацияЭкспорт,
	|	БлокировкаВычетаНДСРеализацияЭкспорт.НДС КАК НДС
	|ИЗ
	|	БлокировкаВычетаНДСРеализацияЭкспорт КАК БлокировкаВычетаНДСРеализацияЭкспорт
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		НДСПредъявленный.НДСУпр
	|	ИЗ
	|		НДСПредъявленныйВосстановлениеПоКСФ КАК НДСПредъявленный) КАК НДСПредъявленный
	|	
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(НДСПредъявленный.НДС) <> 0 ИЛИ
	|	СУММА(НДСПредъявленный.СуммаБезНДС) <> 0
	|
	|УПОРЯДОЧИТЬ ПО
	|	НДС УБЫВ
	...
	|	НалоговаяБазаРеализации0ВРазрезеКодовОпераций.НДСПриобретения КАК НДСПриобретения,
	|	НалоговаяБазаРеализации0ВРазрезеКодовОпераций.ДокументОтгрузки КАК РеализацияЭкспорт,
	|	НалоговаяБазаРеализации0ВРазрезеКодовОпераций.КодОперации КАК КодОперации,
	|	НалоговаяБазаРеализации0ВРазрезеКодовОпераций.СуммаПодтверждения КАК СуммаПодтверждения,
	|	НЕОПРЕДЕЛЕНО КАК Поставщик,
	|	НЕОПРЕДЕЛЕНО КАК ВидЦенности,
	|	НЕОПРЕДЕЛЕНО КАК СтавкаНДС,
	...
	|ГДЕ
	|	НалоговаяБазаРеализации0ВРазрезеКодовОпераций.КодОперации <> """"";
	...
	БлокировкиНДСРеализацияЭкспорт = РезультатЗапроса[1].Выгрузить();
	ВосстановленныйНДС = РезультатЗапроса[2].Выгрузить();
	ВосстановленныйНДС.Индексы.Добавить("Организация, СчетФактура, Поставщик, ВидЦенности, СтавкаНДС, НаправлениеДеятельности"); 
	ВыборкаОстатковНДСПредъявленныйПоСФ = РезультатЗапроса[3].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	НалоговаяБазаРеализации0ДляРаспределенияВычетаНДС = РезультатЗапроса[4].Выгрузить();
	НалоговаяБазаРеализации0ДляРаспределенияВычетаНДС.Индексы.Добавить("Организация, СчетФактура, РеализацияЭкспорт");
	...
	ТаблицаНДСПредъявленныйПринятиеКВычету.Колонки.Добавить("КодОперации", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(7)));
	ТаблицаНДСПредъявленныйПринятиеКВычету.Колонки.Добавить("ЗаписьДополнительногоЛиста", Новый ОписаниеТипов("Булево"));
	ТаблицаНДСПредъявленныйПринятиеКВычету.Колонки.Добавить("КорректируемыйПериод", Новый ОписаниеТипов("Дата"));
	ТаблицаНДСПредъявленныйПринятиеКВычету.Колонки.Добавить("ДатаИсправленияРаспределенияНДС", Новый ОписаниеТипов("Дата"));
	...
	Пока ВыборкаОстатковНДСПредъявленныйПоСФ.Следующий() Цикл
		...
		Пока ВыборкаВРазрезеИзмеренийСФ.Следующий() Цикл
			...
			СтруктураОтбораНДСВосстановлен = Новый Структура();
			СтруктураОтбораНДСВосстановлен.Вставить("Организация");
			СтруктураОтбораНДСВосстановлен.Вставить("СчетФактура");
			СтруктураОтбораНДСВосстановлен.Вставить("Поставщик");
			СтруктураОтбораНДСВосстановлен.Вставить("ВидЦенности");
			СтруктураОтбораНДСВосстановлен.Вставить("СтавкаНДС");
			СтруктураОтбораНДСВосстановлен.Вставить("НаправлениеДеятельности");

			ЗаполнитьЗначенияСвойств(СтруктураОтбораНДСВосстановлен, ВыборкаВРазрезеИзмеренийСФ);
			
			ВосстановленныйНДСПоСФ = ВосстановленныйНДС.НайтиСтроки(СтруктураОтбораНДСВосстановлен);
			...
			Если ВыборкаВРазрезеИзмеренийСФ.НДС > 0 Тогда
				Если ЗначениеЗаполнено(ВыборкаВРазрезеИзмеренийСФ.РеализацияЭкспорт) Тогда
					СтруктураОтбораРеализацияЭкспорт = Новый Структура();
					СтруктураОтбораРеализацияЭкспорт.Вставить("СчетФактура");
					СтруктураОтбораРеализацияЭкспорт.Вставить("РеализацияЭкспорт");
					ЗаполнитьЗначенияСвойств(СтруктураОтбораРеализацияЭкспорт, ВыборкаВРазрезеИзмеренийСФ);
					СуммаБлокировкиРеализацияЭкспорт = БлокировкиНДСРеализацияЭкспорт.НайтиСтроки(СтруктураОтбораРеализацияЭкспорт);
					Если СуммаБлокировкиРеализацияЭкспорт.Количество() > 0 Тогда
						СуммаБлокировкиПоИзмерениюСФ = Мин(ВыборкаВРазрезеИзмеренийСФ.НДС, Мин(СуммаБлокировки, СуммаБлокировкиРеализацияЭкспорт[0].НДС));
					Иначе
						СуммаБлокировкиПоИзмерениюСФ = 0;
					КонецЕсли;
				Иначе
			...
			СтруктураОтбораКодыОперации0 = Новый Структура();
			СтруктураОтбораКодыОперации0.Вставить("Организация");
			СтруктураОтбораКодыОперации0.Вставить("СчетФактура");
			СтруктураОтбораКодыОперации0.Вставить("РеализацияЭкспорт");
			ЗаполнитьЗначенияСвойств(СтруктураОтбораКодыОперации0, ВыборкаВРазрезеИзмеренийСФ);
			СтрокиНалоговаяБазаРеализации0 = НалоговаяБазаРеализации0ДляРаспределенияВычетаНДС.НайтиСтроки(СтруктураОтбораКодыОперации0);
			Коэффициенты = Новый Массив;
			Для Каждого СтрокаНалоговаяБазаРеализации0ДляРаспределенияВычетаНДС Из СтрокиНалоговаяБазаРеализации0 Цикл
				Коэффициенты.Добавить(СтрокаНалоговаяБазаРеализации0ДляРаспределенияВычетаНДС.СуммаПодтверждения);
				ЗаполнитьЗначенияСвойств(СтрокаНалоговаяБазаРеализации0ДляРаспределенияВычетаНДС, ВыборкаВРазрезеИзмеренийСФ);
			КонецЦикла;
			
			// Весь вычет относим на счет-фактуру.
			Если Суммы.НДСКВычету * Знак > 0 Тогда
				Если НДСВосстановлен > 0 Тогда
					...
					Если Коэффициенты.Количество() = 0 Тогда
						...
						ДобавитьСтрокуВычетаНДС(ТаблицаНДСПредъявленныйПринятиеКВычету, Суммы, ДоляДоступногоНДС, ВыборкаВРазрезеИзмеренийСФ, Истина);
					Иначе
						РаспределениеСуммыКВычету = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(НДС, Коэффициенты);
						Для Инд = 0 По СтрокиНалоговаяБазаРеализации0.Количество() - 1 Цикл
							ДоляДоступногоНДС = РаспределениеСуммыКВычету[Инд] / Суммы.НДС;
							ДобавитьСтрокуВычетаНДС(ТаблицаНДСПредъявленныйПринятиеКВычету, Суммы, ДоляДоступногоНДС, СтрокиНалоговаяБазаРеализации0[Инд], Истина);
						КонецЦикла;
					КонецЕсли;
				...
				Если Суммы.НДСКВычету * Знак > 0 Тогда
					Если Коэффициенты.Количество() = 0 Тогда
						Если Суммы.НДСКВычету * Знак <= ПараметрыРасчета.ПорогСписанияПогрешностейОкругления Тогда
							Продолжить;
						Иначе
							ДоляДоступногоНДС = Суммы.НДСКВычету / Суммы.НДС;
							ДобавитьСтрокуВычетаНДС(ТаблицаНДСПредъявленныйПринятиеКВычету, Суммы, ДоляДоступногоНДС, ВыборкаВРазрезеИзмеренийСФ, Ложь);
						КонецЕсли;
					Иначе
						РаспределениеСуммыКВычету = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(Суммы.НДСКВычету, Коэффициенты);
						Для Инд = 0 По СтрокиНалоговаяБазаРеализации0.Количество() - 1 Цикл
							ДоляДоступногоНДС = ?(Суммы.НДСКВычету <> 0, РаспределениеСуммыКВычету[Инд] / Суммы.НДС, 0);
							ДобавитьСтрокуВычетаНДС(ТаблицаНДСПредъявленныйПринятиеКВычету, Суммы, ДоляДоступногоНДС, СтрокиНалоговаяБазаРеализации0[Инд], Ложь);
						КонецЦикла;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
	...
	|	ТаблицаНДСПредъявленныйПринятиеКВычету.КодОперации КАК КодОперации,
	|	ЛОЖЬ КАК Сторно,
	|	ТаблицаНДСПредъявленныйПринятиеКВычету.Партия КАК Партия,
	|	ТаблицаНДСПредъявленныйПринятиеКВычету.АналитикаУчетаПартий КАК АналитикаУчетаПартий,
...
Процедура ДобавитьСтрокуВычетаНДС(ТаблицаПринятиеКВычету,
	...
	НоваяСтрока.ЗаписьДополнительногоЛиста = ВыборкаВРазрезеИзмеренийСФ.ОтнестиКПредыдущемуКварталу;
	НоваяСтрока.КорректируемыйПериод = ВыборкаВРазрезеИзмеренийСФ.ИсправляемыйПериод;
	НоваяСтрока.Период = ?(ВыборкаВРазрезеИзмеренийСФ.ОтнестиКПредыдущемуКварталу, ВыборкаВРазрезеИзмеренийСФ.ДатаИсправленияРаспределенияНДС, НоваяСтрока.Период);
...
Процедура СформироватьНДСАвансыПолученные(ПараметрыРасчета)
	...
	|					ВНУТРЕННЕЕ СОЕДИНЕНИЕ РасширениеТаблицыДокументовОплат КАК ВТ_ДокументовОплат
	|					ПО ВТ_ДокументовОплат.ДокументОплаты = Задания.СчетФактура
	|				ГДЕ
	|					СФ.ДокументОснование = ВТ_ДокументовОплат.ИсходныйДокументОплаты))";
	РезультатЗапроса = Запрос.Выполнить();
	...
	ИменаТаблицЗаписейНДСАвансыПолученные.Добавить(РассчитатьНДСАвансыПолученныеСторноЗачетов(ПараметрыРасчета));
...
Процедура ПолучитьСчетаФактурыВыданныеАванс(ПараметрыРасчета)
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РасширениеТаблицыДокументовОплат КАК ВТ_ДокументыОплат
	|	ПО
	|		ВТ_ДокументыОплат.Организация = СчетФактураВыданныйАванс.Организация
	|			И ВТ_ДокументыОплат.ИсходныйДокументОплаты = СчетФактураВыданныйАванс.ДокументОснование
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.ИсправленияДокументов КАК ИсправленияДокументов
	|	ПО
	|		ИсправленияДокументов.ИсправляемыйДокумент = СчетФактураВыданныйАванс.Ссылка
	|			И ИсправленияДокументов.Период <= &КонецПериода
...
Процедура ПолучитьИсправительныеСчетаФактурыВыданныеАванс(ПараметрыРасчета)
	...
	|	ПО
	|		ВТ_ДокументыОплат.Организация = ИсправленныйСчетФактура.Организация
	|		И ВТ_ДокументыОплат.ИсходныйДокументОплаты = ИсправленныйСчетФактура.ДокументОснование
	...
	|		РегистрСведений.ИсправленияДокументов КАК ИсправленияДокументов
	|	ПО
	|		ИсправленияДокументов.ИсправляемыйДокумент = ИсправленныйСчетФактура.Ссылка
	|			И ИсправленияДокументов.Период <= &КонецПериода
	|			И ИсправленияДокументов.СторнированИсправляемыйДокумент
	|
	|ГДЕ
	...
	Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.Дата) Тогда
		...
		ПараметрыРасчета.ИСФКонецПериода = КонецМесяца(ДобавитьМесяц(ПараметрыРасчета.КонецПериода, -1));
	КонецЕсли;
...
Функция АвансыПолученныеПоДокументамОплаты(ПараметрыРасчета)
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВТ_ДокументыОплат.ИсходныйДокументОплаты КАК ДокументОплаты,
	|		Расчеты.Контрагент КАК Контрагент,
	|		Расчеты.СуммаАванса КАК Аванс,
	|		Расчеты.СуммаАвансаУпр КАК АвансУпр
	|	ИЗ
	|		АвансыПолученные_Возникновения КАК Расчеты
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РасширениеТаблицыДокументовОплат КАК ВТ_ДокументыОплат
	|			ПО Расчеты.Организация = ВТ_ДокументыОплат.Организация
	|				И Расчеты.РасчетныйДокумент = ВТ_ДокументыОплат.ДокументОплаты
	|	) КАК Расчеты
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ПолучитьЗачетыАвансовПолученных(ПараметрыРасчета)
	...
	|		РасширениеТаблицыДокументовОплат КАК ВТ_ДокументыОплат
	|	ПО
	|		ВТ_ДокументыОплат.Организация = Зачеты.Организация
	|			И ВТ_ДокументыОплат.ДокументОплаты = Зачеты.РасчетныйДокумент
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	Зачеты.Контрагент,
	|	ВТ_ДокументыОплат.ИсходныйДокументОплаты,
	...
	Если ЕстьИСФ Тогда
		...
		Запрос.Параметры.Вставить("ИСФКонецПериода",ПараметрыРасчета.ИСФКонецПериода);
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(Зачеты.ДокументРегистратор) = ТИП(Документ.Сторно) И Зачеты.Сторно ТОГДА
		|			ВЫРАЗИТЬ(Зачеты.ДокументРегистратор КАК Документ.Сторно).СторнируемыйДокумент
		|		ИНАЧЕ Зачеты.ДокументРегистратор
		|	КОНЕЦ                                    КАК Регистратор,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(Зачеты.ДокументРегистратор) = ТИП(Документ.Сторно) И Зачеты.Сторно ТОГДА
		|			ВЫРАЗИТЬ(Зачеты.ДокументРегистратор КАК Документ.Сторно).СторнируемыйДокумент.Дата
		|		ИНАЧЕ Зачеты.ДатаПогашения
		|	КОНЕЦ                                    КАК Период,
		|	Зачеты.Организация                       КАК Организация,
		|	Зачеты.НаправлениеДеятельности           КАК НаправлениеДеятельности,
		|	Зачеты.Контрагент                        КАК Контрагент,
		...
		|			ВЫРАЗИТЬ(Зачеты.ДокументРегистратор КАК Документ.Сторно).СторнируемыйДокумент
		|		ИНАЧЕ Зачеты.ДокументРегистратор
		|	КОНЕЦ                                    КАК ДокументЗачетаАванса,
		|	Зачеты.ОбъектРасчетов                    КАК ОбъектРасчетов,
		|	ЛОЖЬ                                     КАК Сторно,
		|	СУММА(Зачеты.СобытиеЗнак)                КАК СобытиеЗнак,
		|	СУММА(Зачеты.СуммаПогашения)             КАК СуммаЗачета
...
Процедура ПолучитьОстаткиАвансовПолученныхДоПереноса(ПараметрыРасчета)
	...
	Если ЕстьИСФ Тогда
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ВТ_ТаблицаОплат.ИсходныйДокументОплаты КАК ДокументОплаты
		|ПОМЕСТИТЬ ДокументыОплаты
		|ИЗ
		|	РасширениеТаблицыДокументовОплат КАК ВТ_ТаблицаОплат
		|		ЛЕВОЕ СОЕДИНЕНИЕ ИсправленныеСчетаФактурыВыданныеАвансПерерасчет КАК ИсправленныеСчетаФактурыВыданныеАванс
		|		ПО ВТ_ТаблицаОплат.Организация = ИсправленныеСчетаФактурыВыданныеАванс.Организация
		|			И ВТ_ТаблицаОплат.ИсходныйДокументОплаты = ИсправленныеСчетаФактурыВыданныеАванс.ДокументОплаты
		|ГДЕ
	...
	Иначе
		...
		ПараметрыРасчетаАвансов.Организации                                           = ПараметрыРасчета.МассивОрганизаций;
		ПараметрыРасчетаАвансов.ВключитьДанныеПодарочныхСертификатов                  = Истина;
		ПараметрыРасчетаАвансов.ПараметрыОтбораПоРасчетномуДокументу.ИмяТаблицыОтбора = "РасширениеТаблицыДокументовОплат";
		ПараметрыРасчетаАвансов.ПараметрыОтбораПоРасчетномуДокументу.ИмяПоля          = "ИсходныйДокументОплаты"; 
...
Функция РассчитатьНДСАвансыПолученныеПереносыПриход(ПараметрыРасчета)
	...
	|	ДД.ИдентификаторФинЗаписи                 КАК ИдентификаторФинЗаписи,
	|	ДД.ОбъектРасчетов                         КАК ОбъектРасчетов,
	|	ВТ_ДокументыОплаты.ИсходныйДокументОплаты КАК ДокументОплаты,
	...
	|	РасчетыСКлиентами_ПереносыАвансов КАК ДД
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РасширениеТаблицыДокументовОплат КАК ВТ_ДокументыОплаты
	|	ПО
	|		ДД.Организация = ВТ_ДокументыОплаты.Организация
	|		И ДД.ДокументОплаты = ВТ_ДокументыОплаты.ДокументОплаты
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		СчетаФактурыВыданныеАванс КАК СчетаФактурыВыданныеАванс
	|	ПО
	|		ДД.Организация = СчетаФактурыВыданныеАванс.Организация
	|		И ВТ_ДокументыОплаты.ИсходныйДокументОплаты = СчетаФактурыВыданныеАванс.ДокументОплаты
	|		И (ВЫБОР
	|				КОГДА ТИПЗНАЧЕНИЯ(ВТ_ДокументыОплаты.ИсходныйДокументОплаты) = ТИП(Документ.ВводОстатков)
	|					ТОГДА ДД.Контрагент = СчетаФактурыВыданныеАванс.Контрагент
	|				ИНАЧЕ ИСТИНА
	|			КОНЕЦ)
	|
	...
	|	ПО
	|		ДД.Организация = ИсправленныеСчетаФактурыВыданныеАванс.Организация
	|		И ВТ_ДокументыОплаты.ИсходныйДокументОплаты = ИсправленныеСчетаФактурыВыданныеАванс.ДокументОплаты
	|		И (ВЫБОР
	|				КОГДА ТИПЗНАЧЕНИЯ(ВТ_ДокументыОплаты.ИсходныйДокументОплаты) = ТИП(Документ.ВводОстатков)
	|					ТОГДА ДД.Контрагент = ИсправленныеСчетаФактурыВыданныеАванс.Контрагент
	|				ИНАЧЕ ИСТИНА
	...
	Результат = Запрос.ВыполнитьПакет();
	ПереносыАвансов = Результат[1].Выгрузить();
	...
	Если ПереносыАвансов.Количество()>0 Тогда 
		...
		Для Каждого СтруктураОтбораЗачета Из ЭтапыЗачета Цикл
			Для Каждого Строка Из НачисленияСПолученногоАвансаИтоги Цикл
				...
				СтрокиПереносаАванса = ПереносыАвансов.НайтиСтроки(СтруктураОтбораЗачета);
				
				ВыполнитьЗачетАвансаПолученного(ПараметрыРасчета, СтрокаАванса, СтрокиНачислениеНДС, СтрокиПереносаАванса, НДСАвансыПолученные, "Приход");
...
Функция РассчитатьНДСАвансыПолученныеПереносыРасход(ПараметрыРасчета)
	...
	Результат = Запрос.ВыполнитьПакет();
	ПереносыАвансов = Результат[2].Выгрузить();
	...
	Если ПереносыАвансов.Количество()>0 Тогда 
		...
		Для Каждого СтруктураОтбораЗачета Из ЭтапыЗачета Цикл
			Для Каждого Строка Из НачисленияСПолученногоАвансаИтоги Цикл
				...
				ВыполнитьЗачетАвансаПолученного(ПараметрыРасчета, СтрокаАванса, СтрокиНачислениеНДС, СтрокиПереносаАванса, НДСАвансыПолученные, "Расход");
...
Функция НачислениеНДСАвансыПолученные(ПараметрыРасчета)
	...
	|		-ДД.НДСУпр                 КАК НДСУпр
	|	ИЗ
	|		НДСАвансыПолученныеПереносыРасход КАК ДД) КАК НачислениеСПолученногоАванса
	|
	|СГРУППИРОВАТЬ ПО
	|	НачислениеСПолученногоАванса.Организация,
	|	НачислениеСПолученногоАванса.ДокументОплаты,
...
Функция РассчитатьНДСАвансыПолученныеЗачеты(ПараметрыРасчета)
	...
	|			ПО ЗачетыАвансовПолученных.Регистратор = ВыручкаИСебестоимостьПродаж.Регистратор
	|	ГДЕ
	|		ЗачетыАвансовПолученных.Сторно <> ИСТИНА
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ЗачетыАвансовПолученных.Регистратор,
	|		ВЫБОР
	...
	|			ПО ЗачетыАвансовПолученных.Регистратор = ПрочаяВыручка.Регистратор
	|	ГДЕ
	|		ЗачетыАвансовПолученных.Сторно <> ИСТИНА) КАК Выручка
	|
	|СГРУППИРОВАТЬ ПО
	|	Выручка.Регистратор
	|
	...
	|	ЗачетыАвансовПолученных.Сторно КАК Сторно,
	|	ВЫБОР
	...
	|ИЗ
	|	ЗачетыАвансовПолученных КАК ЗачетыАвансовПолученных
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеПоВыручке КАК ДанныеПоВыручке
	|		ПО ЗачетыАвансовПолученных.Регистратор = ДанныеПоВыручке.Регистратор
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДокументыЗачетаАвансовПолученныхСторно КАК ДокументыЗачетаСторно
	|		ПО ЗачетыАвансовПолученных.ДокументОплаты = ДокументыЗачетаСторно.ДокументОплаты
	|			И ЗачетыАвансовПолученных.ОбъектРасчетов = ДокументыЗачетаСторно.ОбъектРасчетов
	|			И ЗачетыАвансовПолученных.ДокументЗачетаАванса = ДокументыЗачетаСторно.Документ
	|			И (НАЧАЛОПЕРИОДА(ЗачетыАвансовПолученных.Период, МЕСЯЦ) = НАЧАЛОПЕРИОДА(ДокументыЗачетаСторно.Период, МЕСЯЦ))
	|ГДЕ
	|	ЗачетыАвансовПолученных.Сторно <> ИСТИНА
	|	И ДокументыЗачетаСторно.Документ ЕСТЬ NULL
	|
	|УПОРЯДОЧИТЬ ПО
	|	Период";
...
Процедура ВыполнитьЗачетАвансаПолученного(ПараметрыРасчета, СтрокаАвансаПоДокументуОплаты, СтрокиАвансаНачислениеНДС, СтрокиЗачетаАванса, ТаблицаНДСАвансыПолученные, ВариантРаспределения)
	...
	Для Каждого ПогашенныйАвансПоСФ Из СтрокиЗачетаАванса Цикл
		...
		МножительЗнака = ?(ПогашенныйАвансПоСФ.Сторно, -1, 1);
		
		Если МножительЗнака * СуммаБезНДС > 0 Тогда
			МассивСуммаБезНДС = ОбщегоНазначенияУТКлиентСервер.РаспределитьПропорционально(СуммаБезНДС, АвансМассивСуммаБезНДС);
		КонецЕсли;
		
		Если МножительЗнака * СуммаНДС > 0 Тогда
			МассивСуммаНДС = ОбщегоНазначенияУТКлиентСервер.РаспределитьПропорционально(СуммаНДС, АвансМассивСуммаНДС);
		КонецЕсли; 
		
		Счетчик = -1;
		
		СтрокаРезультатаМакс = Неопределено; // Максимальная сумма зачета аванса
		Для Каждого СтрокаАванса Из СтрокиАвансаНачислениеНДС Цикл
			
			Счетчик = Счетчик + 1;
			
			Результат_СуммаБезНДС = ?(МножительЗнака * СуммаБезНДС > 0, МассивСуммаБезНДС[Счетчик], 0);
			Результат_НДС = ?(МножительЗнака * СуммаНДС > 0, МассивСуммаНДС[Счетчик], 0);
			...
			СтрокаРезультата.Сторно = ПогашенныйАвансПоСФ.Сторно;
			...
			ИначеЕсли ТипЗнч(ПогашенныйАвансПоСФ.ДокументЗачетаАванса) = Тип("ДокументСсылка.Сторно") Тогда
				 СтрокаРезультата.Событие = Перечисления.СобытияНДСАвансы.ВычетПриЗачетеАванса;
			...
			Если СтрокаРезультатаМакс = Неопределено
				ИЛИ СтрокаРезультата.СуммаБезНДС >= СтрокаРезультатаМакс.СуммаБезНДС Тогда
				СтрокаРезультатаМакс = СтрокаРезультата;
			КонецЕсли;
			...
			Если СтрокаАвансаПоДокументуОплаты.Аванс = 0 Тогда
				СтрокаРезультатаМакс.СуммаБезНДС = СтрокаРезультатаМакс.СуммаБезНДС + СтрокаАванса.СуммаБезНДС;
				СтрокаРезультатаМакс.НДС = СтрокаРезультатаМакс.НДС + СтрокаАванса.НДС;
				СтрокаРезультатаМакс.НДСУпр = СтрокаРезультатаМакс.НДСУпр + СтрокаАванса.НДСУпр;
			КонецЕсли;
...
Функция РассчитатьНДСАвансыПолученныеСторноЗачетов(ПараметрыРасчета)
	...
	|	ЗачетыАвансовПолученных.ДокументОплаты КАК ДокументОплаты,
	|	ЗачетыАвансовПолученных.ОбъектРасчетов КАК ОбъектРасчетов,
	|	ВЫРАЗИТЬ(ЗачетыАвансовПолученных.ДокументЗачетаАванса КАК Документ.Сторно).СторнируемыйДокумент КАК Документ,
	|	ЗачетыАвансовПолученных.ДокументЗачетаАванса КАК ДокументЗачетаАванса
	|ПОМЕСТИТЬ ДокументыЗачетаАвансовПолученныхСторно
	|ИЗ
	|	ЗачетыАвансовПолученных КАК ЗачетыАвансовПолученных
	|ГДЕ
	|	ЗачетыАвансовПолученных.Сторно = ИСТИНА
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗачетыАвансовПолученных.Период,
	|	ЗачетыАвансовПолученных.ДокументОплаты,
	|	ЗачетыАвансовПолученных.ОбъектРасчетов,
	|	ВЫРАЗИТЬ(ЗачетыАвансовПолученных.ДокументЗачетаАванса КАК Документ.Сторно).СторнируемыйДокумент,
	|	ЗачетыАвансовПолученных.ДокументЗачетаАванса
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументОплаты,
	|	ОбъектРасчетов,
	...
	|ВЫБРАТЬ
	|	2 КАК Приоритет,
	|	ДокументыЗачетаСторно.Период КАК Период,
	|	НДСАвансыПолученные.Регистратор КАК Регистратор,
	|	НДСАвансыПолученные.ВидДвижения КАК ВидДвижения,
	|	НДСАвансыПолученные.Организация КАК Организация,
	|	НДСАвансыПолученные.ДокументОплаты КАК ДокументОплаты,
	...
	|	-НДСАвансыПолученные.НДСУпр КАК НДСУпр,
	|	НДСАвансыПолученные.Событие КАК Событие,
	|	ДокументыЗачетаСторно.ДокументЗачетаАванса КАК ДокументЗачетаАванса,
	|	ЕСТЬNULL(ДокументыЗачетаСторно.Документ.Дата, НДСАвансыПолученные.Период) КАК ДатаЗачетаАванса,
	|	НДСАвансыПолученные.ИдентификаторФинЗаписи КАК ИдентификаторФинЗаписи,
	|	НДСАвансыПолученные.НастройкаХозяйственнойОперации КАК НастройкаХозяйственнойОперации,
	|	ИСТИНА КАК РегламентнаяОперация,
	|	ЛОЖЬ КАК Сторно
	|ПОМЕСТИТЬ НДСАвансыПолученныеСторноЗачетов
	|ИЗ
	|	РегистрНакопления.НДСАвансыПолученные КАК НДСАвансыПолученные
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДокументыЗачетаАвансовПолученныхСторно КАК ДокументыЗачетаСторно
	|		ПО НДСАвансыПолученные.ДокументЗачетаАванса = ДокументыЗачетаСторно.Документ
	|			И НДСАвансыПолученные.ДокументОплаты = ДокументыЗачетаСторно.ДокументОплаты
	|			И НДСАвансыПолученные.ОбъектРасчетов = ДокументыЗачетаСторно.ОбъектРасчетов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СчетаФактурыВыданныеАванс КАК СчетаФактурыВыданныеАванс
	|		ПО НДСАвансыПолученные.Регистратор = СчетаФактурыВыданныеАванс.Ссылка
	|ГДЕ
	|	НДСАвансыПолученные.РегламентнаяОперация
	|	И НЕ НДСАвансыПолученные.Сторно
	|	И НДСАвансыПолученные.Период < &НачалоПериода
	|	И НЕ НДСАвансыПолученные.ДокументОплаты В 
	|				(ВЫБРАТЬ
	|					ИСФ.ДокументОплаты КАК Ссылка
	|				ИЗ
	|					ИсправленныеСчетаФактурыВыданныеАванс КАК ИСФ)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|			И НДСАвансыПолученные.ДокументОплаты = ДокументыЗачетаСторно.ДокументОплаты
	|			И НДСАвансыПолученные.ОбъектРасчетов = ДокументыЗачетаСторно.ОбъектРасчетов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ИсправленныеСчетаФактурыВыданныеАванс КАК ИсправленныеСчетаФактурыВыданныеАванс
	|		ПО НДСАвансыПолученные.Регистратор = ИсправленныеСчетаФактурыВыданныеАванс.Ссылка
	|			И НДСАвансыПолученные.ИсправленныйСчетФактура = ИсправленныеСчетаФактурыВыданныеАванс.Ссылка
	|ГДЕ
	|	НДСАвансыПолученные.РегламентнаяОперация
	|	И НЕ НДСАвансыПолученные.Сторно
	|	И НДСАвансыПолученные.Период < &НачалоПериода";
	...
	Возврат "НДСАвансыПолученныеСторноЗачетов";
...
Процедура СформироватьНДСАвансыВыданные(ПараметрыРасчета)
	...
	ИменаТаблицЗаписейНДСАвансыВыданные.Добавить(РассчитатьНДСАвансыВыданныеСторноЗачетов(ПараметрыРасчета));
...
Процедура ПолучитьСчетаФактурыПолученныеАванс(ПараметрыРасчета)
	...
	|		РасширениеТаблицыДокументовОплат КАК ВТ_ДокументыОплат
	|	ПО
	|		ВТ_ДокументыОплат.Организация = СчетФактураПолученныйАванс.Организация
	|			И ВТ_ДокументыОплат.ИсходныйДокументОплаты = СчетФактураПолученныйАванс.ДокументОснование
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.ИсправленияДокументов КАК ИсправленияДокументов
	|	ПО
	|		ИсправленияДокументов.ИсправляемыйДокумент = СчетФактураПолученныйАванс.Ссылка
	|			И ИсправленияДокументов.Период <= &КонецПериода
	|			И ИсправленияДокументов.СторнированИсправляемыйДокумент
	|
	|ГДЕ
...
Функция ВычетНДСАвансыВыданные(ПараметрыРасчета)
	...
	|		-ДД.НДСУпр                 КАК НДСУпр
	|	ИЗ
	|		НДСАвансыВыданныеПереносыПриход КАК ДД
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|		-ДД.НДСУпр                  КАК НДСУпр
	|	ИЗ
	|		НДСАвансыВыданныеСторноЗачетов КАК ДД
	|	ГДЕ
	|		ДД.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|) КАК ВычетСВыданногоАванса
	|
	|СГРУППИРОВАТЬ ПО
...
Функция РассчитатьНДСАвансыВыданныеЗачеты(ПараметрыРасчета)
	...
	|	ЗачетыАвансовВыданных.Сторно КАК Сторно
	|ИЗ
	|	ЗачетыАвансовВыданных КАК ЗачетыАвансовВыданных
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДокументыЗачетаАвансовВыданныхСторно КАК ДокументыЗачетаСторно
	|		ПО ЗачетыАвансовВыданных.ДокументОплаты = ДокументыЗачетаСторно.ДокументОплаты
	|			И ЗачетыАвансовВыданных.ОбъектРасчетов = ДокументыЗачетаСторно.ОбъектРасчетов
	|			И ЗачетыАвансовВыданных.ДокументЗачетаАванса = ДокументыЗачетаСторно.Документ
	|			И (НАЧАЛОПЕРИОДА(ЗачетыАвансовВыданных.Период, МЕСЯЦ) = НАЧАЛОПЕРИОДА(ДокументыЗачетаСторно.Период, МЕСЯЦ))
	|ГДЕ
	|	ЗачетыАвансовВыданных.Сторно <> ИСТИНА
	|	И ДокументыЗачетаСторно.Документ ЕСТЬ NULL";
...
Процедура ВыполнитьЗачетАвансаВыданного(ПараметрыРасчета, СтрокаАвансаПоДокументуОплаты, СтрокиАвансаНачислениеНДС, СтрокиЗачетаАванса, ТаблицаНДСАвансыВыданные, ВариантРаспределения)
	...
	Для Каждого ПогашенныйАвансПоСФ Из СтрокиЗачетаАванса Цикл
		...
		Если ВариантРаспределения <> "Зачет" И
			НачалоДня(ДатаЗачетаАванса) = НачалоДня(ДатаНачисления) Тогда
			// Переносы аванса в день начисления не учитываем, т.к. они уже учтены в СФ на аванс.
			Продолжить;
		КонецЕсли;
		...
		Для Каждого СтрокаАванса Из СтрокиАвансаНачислениеНДС Цикл
			...
			Если ВариантРаспределения <> "Зачет" Тогда
				Если ВариантРаспределения = "Расход" Тогда
					СтрокаРезультата.НаправлениеДеятельности = ПогашенныйАвансПоСФ.НаправлениеДеятельности;
					СтрокаРезультата.Подразделение = ПогашенныйАвансПоСФ.Подразделение;
					СтрокаРезультата.ОбъектРасчетов = ПогашенныйАвансПоСФ.ОбъектРасчетов;
					СтрокаРезультата.Контрагент = ПогашенныйАвансПоСФ.Контрагент;
...
Функция РассчитатьНДСАвансыВыданныеПереносыПриход(ПараметрыРасчета)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	МИНИМУМ(НачислениеСВыданногоАванса.Приоритет) КАК Приоритет,
...
Функция РассчитатьНДСАвансыВыданныеСторноЗачетов(ПараметрыРасчета)
	...
	|	ЗачетыАвансовВыданных.ДокументОплаты КАК ДокументОплаты,
	|	ЗачетыАвансовВыданных.ОбъектРасчетов КАК ОбъектРасчетов,
	|	ВЫРАЗИТЬ(ЗачетыАвансовВыданных.ДокументЗачетаАванса КАК Документ.Сторно).СторнируемыйДокумент КАК Документ,
	|	ЗачетыАвансовВыданных.ДокументЗачетаАванса КАК ДокументЗачетаАванса
	|ПОМЕСТИТЬ ДокументыЗачетаАвансовВыданныхСторно
	|ИЗ
	|	ЗачетыАвансовВыданных КАК ЗачетыАвансовВыданных
	|ГДЕ
	|	ЗачетыАвансовВыданных.Сторно = ИСТИНА
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗачетыАвансовВыданных.Период,
	|	ЗачетыАвансовВыданных.ДокументОплаты,
	|	ЗачетыАвансовВыданных.ОбъектРасчетов,
	|	ВЫРАЗИТЬ(ЗачетыАвансовВыданных.ДокументЗачетаАванса КАК Документ.Сторно).СторнируемыйДокумент,
	|	ЗачетыАвансовВыданных.ДокументЗачетаАванса
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументОплаты,
	|	ОбъектРасчетов,
	...
	|	НДСАвансыВыданные.Событие КАК Событие,
	|	ДокументыЗачетаСторно.ДокументЗачетаАванса КАК ДокументЗачетаАванса,
	|	ЕСТЬNULL(ДокументыЗачетаСторно.Документ.Дата, НДСАвансыВыданные.Период) КАК ДатаЗачетаАванса,
	|	НДСАвансыВыданные.ИдентификаторФинЗаписи КАК ИдентификаторФинЗаписи,
	|	НДСАвансыВыданные.НастройкаХозяйственнойОперации КАК НастройкаХозяйственнойОперации,
	|	ИСТИНА КАК РегламентнаяОперация,
	|	ЛОЖЬ КАК Сторно
	|ПОМЕСТИТЬ НДСАвансыВыданныеСторноЗачетов
	|ИЗ
	|	РегистрНакопления.НДСАвансыВыданные КАК НДСАвансыВыданные
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДокументыЗачетаАвансовВыданныхСторно КАК ДокументыЗачетаСторно
	|		ПО НДСАвансыВыданные.ДокументЗачетаАванса = ДокументыЗачетаСторно.Документ
	|			И НДСАвансыВыданные.ДокументОплаты = ДокументыЗачетаСторно.ДокументОплаты
	|			И НДСАвансыВыданные.ОбъектРасчетов = ДокументыЗачетаСторно.ОбъектРасчетов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СчетаФактурыПолученныеАванс КАК СчетаФактурыПолученныеАванс
	|		ПО НДСАвансыВыданные.Регистратор = СчетаФактурыПолученныеАванс.Ссылка
	|ГДЕ
	|	НДСАвансыВыданные.РегламентнаяОперация
	|	И НЕ НДСАвансыВыданные.Сторно
	|	И НДСАвансыВыданные.Период < &НачалоПериода
	|	И НЕ НДСАвансыВыданные.ДокументОплаты В 
	|				(ВЫБРАТЬ
	|					ИСФ.ДокументОплаты КАК Ссылка
	|				ИЗ
	|					ИсправленныеСчетаФактурыПолученныеАванс КАК ИСФ)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|			И НДСАвансыВыданные.ДокументОплаты = ДокументыЗачетаСторно.ДокументОплаты
	|			И НДСАвансыВыданные.ОбъектРасчетов = ДокументыЗачетаСторно.ОбъектРасчетов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ИсправленныеСчетаФактурыПолученныеАванс КАК ИсправленныеСчетаФактурыПолученныеАванс
	|		ПО НДСАвансыВыданные.Регистратор = ИсправленныеСчетаФактурыПолученныеАванс.Ссылка
	|			И НДСАвансыВыданные.ИсправленныйСчетФактура = ИсправленныеСчетаФактурыПолученныеАванс.Ссылка
	|ГДЕ
	|	НДСАвансыВыданные.РегламентнаяОперация
	|	И НЕ НДСАвансыВыданные.Сторно
	|	И НДСАвансыВыданные.Период < &НачалоПериода";
	...
	Возврат "НДСАвансыВыданныеСторноЗачетов";
...
Процедура РассчитатьКорректировкуСписанияНДСНаРасходы(ПараметрыРасчета)
	...
	|	И ТИПЗНАЧЕНИЯ(НДСПредъявленныйСторно.Регистратор) <> ТИП(Документ.РаспределениеНДС)
	|	И ТИПЗНАЧЕНИЯ(СписанияНДСНаРасходы.СтатьяРасходов) = ТИП(ПланВидовХарактеристик.СтатьиАктивовПассивов)
	|
	|";
...
Процедура ПодготовитьВТ_АвансыПолученные_Возникновения(МенеджерВременныхТаблиц, ПараметрыРасчета, ТаблицаНастроек = Неопределено) Экспорт
	...
	ПараметрыРасчетаПоВзаиморасчетам =
		ВзаиморасчетыНДС.ИнициализироватьПараметрыПодготовкиРасчетовАвансов(ПараметрыРасчета);
	...
	ВзаиморасчетыНДС.ПодготовитьВТ_РасчетыСКлиентами_Авансы(
			МенеджерВременныхТаблиц,
			ПараметрыРасчетаПоВзаиморасчетам);
		
	ПодготовитьВТ_ЗачетыАвансовПолученныхСчетамиФактурамиКорректировкиНаУменьшение(
			МенеджерВременныхТаблиц,
			ПараметрыРасчетаПоВзаиморасчетам);
	...
	// ++ Локализация
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		Документ.КорректировкаРеализации КАК КорректировкаРеализацииПереходПраваСобственности
	|	ПО
	|		Расчеты.ДокументРегистратор = КорректировкаРеализацииПереходПраваСобственности.Ссылка
	|		И КорректировкаРеализацииПереходПраваСобственности.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	// -- Локализация
	|ГДЕ
	...
	// ++ Локализация
 	|	И КорректировкаРеализацииПереходПраваСобственности.Ссылка ЕСТЬ NULL
	// -- Локализация
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	Зачеты.Организация                                                 КАК Организация,
	|	Зачеты.ДокументОплаты                                              КАК РасчетныйДокумент,
	|	Зачеты.ДокументРегистратор                                         КАК ДокументРегистратор,
	|	Зачеты.ОбъектРасчетов                                              КАК ОбъектРасчетов,
	|	ЗНАЧЕНИЕ(Справочник.ПодарочныеСертификаты.ПустаяСсылка)            КАК ПодарочныйСертификат,
	|	Зачеты.НаправлениеДеятельности                                     КАК НаправлениеДеятельности,
	|	Зачеты.Контрагент                                                  КАК Контрагент,
	|	Зачеты.Договор                                                     КАК Договор,
	|	Зачеты.НалогообложениеНДС                                          КАК ТипНалогообложения,
	|	Зачеты.Дата                                                        КАК ДатаАванса,
	|	Зачеты.Валюта                                                      КАК ВалютаДокумента,
	|	Зачеты.СуммаРегл                                                   КАК СуммаАванса,
	|	Зачеты.Сумма                                                       КАК СуммаАвансаВал,
	|	Зачеты.Сумма                                                       КАК СуммаАвансаУпр
	|ИЗ
	|	ЗачетыАвансовПолученныхСчетамиФактурамиКорректировкиНаУменьшение КАК Зачеты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ЗачетыАвансовПолученныхСчетамиФактурамиКорректировкиНаУменьшение;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура ПодготовитьВТ_АвансыПолученные_Погашения(МенеджерВременныхТаблиц, ПараметрыРасчета, ТаблицаНастроек = Неопределено) Экспорт
	...
	ВзаиморасчетыНДС.ПодготовитьВТ_РасчетыСКлиентами_ПогашенияАвансов(
		МенеджерВременныхТаблиц,
		ПараметрыРасчетаПоВзаиморасчетам);
		
	ПодготовитьВТ_ЗачетыАвансовПолученныхСчетамиФактурами(
		МенеджерВременныхТаблиц,
		ПараметрыРасчетаПоВзаиморасчетам);
	...
	// ++ Локализация
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслугПереходПраваСобственности
	|	ПО
	|		Погашения.ДокументРегистратор = РеализацияТоваровУслугПереходПраваСобственности.Ссылка
	|		И РеализацияТоваровУслугПереходПраваСобственности.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		Документ.КорректировкаРеализации КАК КорректировкаРеализацииПереходПраваСобственности
	|	ПО
	|		Погашения.ДокументРегистратор = КорректировкаРеализацииПереходПраваСобственности.Ссылка
	|		И КорректировкаРеализацииПереходПраваСобственности.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	// -- Локализация
	|ГДЕ
	...
	|	И &ТекстОпределенияНалогообложения <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПроизводствоСДЦ)
	// ++ Локализация
	|	И РеализацияТоваровУслугПереходПраваСобственности.Ссылка ЕСТЬ NULL
	|	И КорректировкаРеализацииПереходПраваСобственности.Ссылка ЕСТЬ NULL
	// -- Локализация
	|
	|СГРУППИРОВАТЬ ПО
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(Погашения.СуммаПогашенияРегл * ВЫБОР
	|			КОГДА Погашения.Сторно = ИСТИНА
	|				ТОГДА -1
	|			ИНАЧЕ 1
	|		КОНЕЦ) > 0
	|	
	...
	|	Зачеты.ДокументРегистратор                                         КАК ДокументРегистратор,
	|	ЗНАЧЕНИЕ(Справочник.ПодарочныеСертификаты.ПустаяСсылка)            КАК ПодарочныйСертификат,
	|	ДАТАВРЕМЯ(1, 1, 1)                                                 КАК ДатаАванса,
	|	Зачеты.Дата                                                        КАК ДатаПогашения,
	|	Зачеты.НаправлениеДеятельности                                     КАК НаправлениеДеятельности,
	|	Зачеты.Контрагент                                                  КАК Контрагент,
	|	Зачеты.Договор                                                     КАК Договор,
	|	Зачеты.НалогообложениеНДС                                          КАК ТипНалогообложения,
	...
	|	Зачеты.Валюта                                                      КАК ВалютаДокумента,
	|	0                                                                  КАК СобытиеЗнак,
	|	Зачеты.СуммаРегл                                                   КАК СуммаПогашения,
	|	Зачеты.Сумма                                                       КАК СуммаПогашенияВал,
	|	Зачеты.Сумма                                                       КАК СуммаПогашенияУпр
	|ИЗ
	|	ЗачетыАвансовПолученныхСчетамиФактурами КАК Зачеты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ЗачетыАвансовПолученныхСчетамиФактурами;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура ПодготовитьВТ_АвансыПолученные(МенеджерВременныхТаблиц, ПараметрыРасчета, ТаблицаНастроек = Неопределено) Экспорт
	...
	ВзаиморасчетыНДС.ПодготовитьВТ_ПолученныеАвансыВРазрезеПогашенийИОстатков(
			МенеджерВременныхТаблиц,
			ПараметрыРасчетаПоВзаиморасчетам);
	...
	|		Документ.КорректировкаРеализации КАК КорректировкаРеализацииПереходПраваСобственности
	|	ПО
	|		Авансы.РасчетныйДокумент = КорректировкаРеализацииПереходПраваСобственности.Ссылка
	|		И КорректировкаРеализацииПереходПраваСобственности.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|ГДЕ
	|	КорректировкаРеализацииПереходПраваСобственности.Ссылка ЕСТЬ NULL
	// -- Локализация
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|		Зачеты.ОбъектРасчетов                                              КАК ОбъектРасчетов,
	|		Зачеты.Дата                                                        КАК ДатаАванса,
	|		&ПустаяДата                                                        КАК ДатаПогашения,
	|		Зачеты.ДокументРегистратор                                         КАК ДокументРегистратор,
	|		Зачеты.Валюта                                                      КАК ВалютаДокумента,
	|		Зачеты.СуммаРегл                                                   КАК Сумма,
	|		Зачеты.Сумма                                                       КАК СуммаВал,
	|		Зачеты.Сумма                                                       КАК СуммаУпр
	|	ИЗ
	|		ЗачетыАвансовПолученныхСчетамиФактурамиКорректировкиНаУменьшение КАК Зачеты
	|
	|	ОБЪЕДИНИТЬ ВСЕ
...
// Создает временную таблицу с колонками.
// см. ВзаиморасчетыНДС.ПодготовитьВТ_РасчетыСКлиентами_ПереносыАвансов.
//
// Параметры:
//  МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - 
//  ПараметрыРасчета        - см. ИнициализироватьПараметрыПодготовкиРасчетовАвансовВЦеляхНДС
...
Процедура ПодготовитьВТ_РасчетыСКлиентами_ПереносыАвансов(МенеджерВременныхТаблиц, ПараметрыРасчета, ТаблицаНастроек = Неопределено) Экспорт
	...
	ВзаиморасчетыНДС.ПодготовитьВТ_РасчетыСКлиентами_ПереносыАвансов(
		МенеджерВременныхТаблиц,
		ПараметрыРасчетаПоВзаиморасчетам);
...
Процедура ПодготовитьВТ_ОстаткиАвансыПолученныеДоПереноса(МенеджерВременныхТаблиц, ПараметрыРасчета, ТаблицаНастроек = Неопределено) Экспорт
	...
	|		КОНЕЦ
	|	ИЗ
	|		НДСАвансыПолученныеСторноЗачетов КАК ДД
	|	ГДЕ
	|		&ТекстОтборПоПараметрам
	|
	|	ОБЪЕДИНИТЬ ВСЕ
...
Процедура ПодготовитьВТ_ЗачетыАвансовПолученныхСчетамиФактурами(МенеджерВременныхТаблиц, ПараметрыРасчета)
	
	УчетНДСЛокализация.ПолучитьЗачетыАвансовПолученныхСчетамиФактурами(МенеджерВременныхТаблиц, ПараметрыРасчета);
	
	Если МенеджерВременныхТаблиц.Таблицы.Найти("ЗачетыАвансовПолученныхСчетамиФактурами") = Неопределено Тогда
...
Процедура ПодготовитьВТ_ЗачетыАвансовПолученныхСчетамиФактурамиКорректировкиНаУменьшение(МенеджерВременныхТаблиц, ПараметрыРасчета)
	
	УчетНДСЛокализация.ПолучитьЗачетыАвансовПолученныхСчетамиФактурамиКорректировкиНаУменьшение(МенеджерВременныхТаблиц, ПараметрыРасчета);
	
	Если МенеджерВременныхТаблиц.Таблицы.Найти("ЗачетыАвансовПолученныхСчетамиФактурамиКорректировкиНаУменьшение") = Неопределено Тогда
...
Процедура ПодготовитьВТ_АвансыВыданные_Возникновения(МенеджерВременныхТаблиц, ПараметрыРасчета, ТаблицаНастроек = Неопределено) Экспорт
	...
	ВзаиморасчетыНДС.ПодготовитьВТ_РасчетыСПоставщиками_Авансы(
			МенеджерВременныхТаблиц,
			ПараметрыРасчетаПоВзаиморасчетам);
...
Процедура ПодготовитьВТ_АвансыВыданные_Погашения(МенеджерВременныхТаблиц, ПараметрыРасчета, ТаблицаНастроек = Неопределено) Экспорт
	...
	ВзаиморасчетыНДС.ПодготовитьВТ_РасчетыСПоставщиками_ПогашенияАвансов(
		МенеджерВременныхТаблиц,
		ПараметрыРасчетаПоВзаиморасчетам);
...
// Создает временную таблицу с колонками.
// см. ВзаиморасчетыНДС.ПодготовитьВТ_РасчетыСПоставщиками_ПереносыАвансов
//
// Параметры:
//  МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - 
//  ПараметрыРасчета        - см. ИнициализироватьПараметрыПодготовкиРасчетовАвансовВЦеляхНДС
...
Процедура ПодготовитьВТ_ПереносыАвансовВыданных(МенеджерВременныхТаблиц, ПараметрыРасчета, ТаблицаНастроек = Неопределено) Экспорт
	...
	ВзаиморасчетыНДС.ПодготовитьВТ_РасчетыСПоставщиками_ПереносыАвансов(
		МенеджерВременныхТаблиц,
		ПараметрыРасчетаПоВзаиморасчетам);
