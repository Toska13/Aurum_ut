...
Функция ДоступныйСписокСоставТоваров(Параметры) Экспорт
	...
	ИначеЕсли ТипБонуса = ПредопределенноеЗначение("Перечисление.ТипыРетроБонусовПоставщиков.Остатки") Тогда
	...
	ИначеЕсли ПоказательТоваров = ПредопределенноеЗначение("Перечисление.ЦелевыеПоказателиРетроБонусовПоТоварам.Количество")
		...
		Значения.Добавить(ПредопределенноеЗначение("Перечисление.СоставыТоваровРетроБонусов.Номенклатура"));
		Значения.Добавить(ПредопределенноеЗначение("Перечисление.СоставыТоваровРетроБонусов.СвойствоНоменклатуры"));
...
// Доступный список состава отборов товаров по виду ретро-бонуса
// 
// Параметры:
...
Функция ДоступныйСписокОтборТоваров(Параметры) Экспорт
	...
	ИначеЕсли БазаРасчета = ПредопределенноеЗначение("Перечисление.БазыРасчетаРетроБонусов.ВыручкаБазовыеЦены")
	      ИЛИ БазаРасчета = ПредопределенноеЗначение("Перечисление.БазыРасчетаРетроБонусов.СуммаЗакупкиБазовыеЦены")
	...
	Иначе
		...
		Если СоставТоваров <> ПредопределенноеЗначение("Перечисление.СоставыТоваровРетроБонусов.СвойствоНоменклатуры") Тогда
...
// Параметры для формирования списка доступных составов участников
// 
// Возвращаемое значение:
//  Структура - Новые параметры доступных отборов товаров:
//	* БезРасчета - Булево - 
...
Функция ЗаголовокСтраницыПоНастройкам(СоставСписка, ОтборСписка, БонусПоставщиков = Ложь) Экспорт
	...
	#Область СоставыТоваровРетроБонусов_СвойствоНоменклатуры
	Если СоставСписка = ПредопределенноеЗначение("Перечисление.СоставыТоваровРетроБонусов.СвойствоНоменклатуры") Тогда
		
		Если ОтборСписка = ВыбранныеЗначения Тогда
			
			ЗаголовокСтраницы = НСтр("ru = 'Выбранные значения свойства'");
			
		ИначеЕсли ОтборСписка = КромеВыбранных Тогда
			
			ЗаголовокСтраницы = НСтр("ru = 'Кроме значений свойств'");
			
		Иначе
			
			ЗаголовокСтраницы = НСтр("ru = 'Значения свойств'");
...
// Возвращаемое значение:
//  Булево
Функция ЭтоКоличествоПоДиапазонам(ВидШкалы, ПоказательТоваров) Экспорт
...
Процедура СохранитьАналитикуСтрокиТоваров(Аналитика, Знач ПараметрыЗаполнения) Экспорт
	...
	ЭтоТоварыПоДиапазонам = ЭтоКоличествоПоДиапазонам(
...
//  ДопустимыеТипыСвойства - ОписаниеТипов
//
Процедура УстановитьФорматЗначенияСвойстваПоУмолчанию(Элемент, ДопустимыеТипыСвойства) Экспорт
	
	МассивТипов = ДопустимыеТипыСвойства.Типы();
	Если МассивТипов.Количество() = 1 Тогда
		
		Если МассивТипов[0] = Тип("Дата") Тогда
			
			УстановитьФорматДатыСвойства(Элемент, ДопустимыеТипыСвойства);
			
		ИначеЕсли МассивТипов[0] = Тип("Число") Тогда
			
			УстановитьФорматЧислоСвойства(Элемент, ДопустимыеТипыСвойства, Ложь)
...
//  ДопустимыеТипыСвойства - ОписаниеТипов
//
Процедура УстановитьФорматДатыСвойства(Элемент, ДопустимыеТипыСвойства) Экспорт
	
	КвалификаторДата = Новый КвалификаторыДаты(ЧастиДаты.Дата);
	КвалификаторВремя = Новый КвалификаторыДаты(ЧастиДаты.Время);
	
	Если ДопустимыеТипыСвойства.КвалификаторыДаты = КвалификаторДата Тогда
		
		Формат = "ДЛФ=D;";
		
	ИначеЕсли ДопустимыеТипыСвойства.КвалификаторыДаты = КвалификаторВремя Тогда
	...
	Элемент.Формат = Формат;
	Элемент.ФорматРедактирования = Формат;
...
//  Элемент - ПолеФормы - для которого выполняются настройки
//  ДопустимыеТипыСвойства - ОписаниеТипов
//  ТолькоФорматРедактирования - Булево
//
Процедура УстановитьФорматЧислоСвойства(Элемент, ДопустимыеТипыСвойства, ТолькоФорматРедактирования = Истина) Экспорт
	
	КвалификаторыЧисла = ДопустимыеТипыСвойства.КвалификаторыЧисла;
	Разрядность = КвалификаторыЧисла.Разрядность;
	РазрядностьДробнойЧасти = КвалификаторыЧисла.РазрядностьДробнойЧасти;
	ШаблонЧисла = "ЧЦ=%1; ЧДЦ=%2;";
	Формат = СтрШаблон(ШаблонЧисла, Разрядность, РазрядностьДробнойЧасти);
	Элемент.ФорматРедактирования = Формат;
