...
// Параметры:
//  ПараметрыПолученияСтавки - см. УчетНДСУП.ПараметрыПолученияСтавкиНДС
//	СтавкаНДС - СправочникСсылка.СтавкиНДС
Процедура ЗаполнитьСтавкуНДСПоНоменклатуреИНалогообложению(ПараметрыПолученияСтавки, СтавкаНДС) Экспорт
	//++ Локализация
	Если ЗначениеЗаполнено(ПараметрыПолученияСтавки.Номенклатура) Тогда
		РеквизитыНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПараметрыПолученияСтавки.Номенклатура, "СтавкаНДС, ТипНоменклатуры");
		ЭтоПодакцизныйТовар = УчетПодакцизныхТоваров.ЭтоПодакцизныйТовар(ПараметрыПолученияСтавки.Номенклатура, ПараметрыПолученияСтавки.Организация, ПараметрыПолученияСтавки.Дата);
	...
	Если ПараметрыПолученияСтавки.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПроизводствоСДЦ Тогда
		
		СтавкаНДС = УчетНДСУП.СтавкаНДСНоменклатуры(ПараметрыПолученияСтавки.Номенклатура, ПараметрыПолученияСтавки.Организация, ПараметрыПолученияСтавки.Дата);
		
	ИначеЕсли ПараметрыПолученияСтавки.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД Тогда
	...
	ИначеЕсли ПараметрыПолученияСтавки.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДСУПокупателя Тогда
		
		Если Не ПараметрыПолученияСтавки.ИсходящийДокумент Тогда
			СтавкаНДС = УчетНДСУП.СтавкаНДСПоУмолчанию(ПараметрыПолученияСтавки.Организация, ПараметрыПолученияСтавки.Дата, Истина, ПараметрыПолученияСтавки.НалогообложениеНДС);  
		Иначе
			СтавкаНДС = Справочники.СтавкиНДС.БезНДС;
		КонецЕсли;
		
	ИначеЕсли ПараметрыПолученияСтавки.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ЛьготноеНалогообложениеНДСНаУСН Тогда 
		
		НастройкиУчетаНДСПриУСН = НастройкиНалоговУчетныхПолитик.ДействующиеПараметрыНалоговУчетныхПолитикНаДату("НастройкиУчетаНДСПриУСН",
			ПараметрыПолученияСтавки.Организация,
			ПараметрыПолученияСтавки.Дата);
	...
	ИначеЕсли ПараметрыПолученияСтавки.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДССМежценовойРазницы Тогда
		
		СтавкаНДС = Справочники.СтавкиНДС.БезНДС;
		
	ИначеЕсли ПараметрыПолученияСтавки.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ЭлектронныеУслуги Тогда
	...
	ИначеЕсли ПараметрыПолученияСтавки.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаПоПатенту Тогда
	...
	ИначеЕсли ЗначениеЗаполнено(ПараметрыПолученияСтавки.Номенклатура)
		И ПараметрыПолученияСтавки.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС 
		И НЕ ЗначениеЗаполнено(ПараметрыПолученияСтавки.СтавкаНДС)
		...
		СтавкаНДСПеречислением = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыПолученияСтавки.Номенклатура, "УдалитьСтавкаНДС");
		СтавкаНДС = СтавкаНДСПоПеречислению(СтавкаНДСПеречислением, ПараметрыПолученияСтавки.НалогообложениеНДС);
	...
	Если ЗначениеЗаполнено(ПараметрыПолученияСтавки.УказаннаяСтавкаНДС) И СписокСтавокКИзменению.НайтиПоЗначению(ПараметрыПолученияСтавки.УказаннаяСтавкаНДС) <> Неопределено Тогда
		Если СтавкаНДС <> Справочники.СтавкиНДС.БезНДС Тогда
			 СтавкаНДС = ПараметрыПолученияСтавки.УказаннаяСтавкаНДС;
		КонецЕсли;
	КонецЕсли;
	//-- Локализация
...
//   СтавкаНДС - СправочникСсылка.СтавкиНДС
//
Процедура ЗаполнитьСтавкуНДСПоНоменклатуреИНалогообложениюПакетно(ПараметрыПолученияСтавки, КэшированныеЗначения, СтавкаНДС) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыПолученияСтавки.НалогообложениеНДС) И Перечисления.ТипыНалогообложенияНДС.ТипыБезНДС().Найти(ПараметрыПолученияСтавки.НалогообложениеНДС) <> Неопределено Тогда
		СтавкаНДС =  Справочники.СтавкиНДС.БезНДС;
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПараметрыПолученияСтавки.Номенклатура)
		И ЗначениеЗаполнено(ПараметрыПолученияСтавки.НалогообложениеНДС)
		И ПараметрыПолученияСтавки.НалогообложениеНДС <> Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС Тогда
	...
	ИначеЕсли ПараметрыПолученияСтавки.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДСУПокупателя Тогда
		...
		Если Не ПараметрыПолученияСтавки.ИсходящийДокумент Тогда
			СтавкаНДС = УчетНДСУП.СтавкаНДСПоУмолчаниюПакетно(КэшированныеЗначения, Истина, ПараметрыПолученияСтавки.НалогообложениеНДС);
		Иначе
			СтавкаНДС = Справочники.СтавкиНДС.БезНДС;
		КонецЕсли;
	...
	ИначеЕсли ПараметрыПолученияСтавки.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ЛьготноеНалогообложениеНДСНаУСН Тогда 
		...
		НастройкиУчетаНДСПриУСН = НастройкиНалоговУчетныхПолитик.ДействующиеПараметрыНалоговУчетныхПолитикНаДату("НастройкиУчетаНДСПриУСН",
			...
			ПараметрыПолученияСтавки.Дата);
		СтавкаНДС = УчетНДСУП.СтавкаНДСНоменклатуры(ПараметрыПолученияСтавки.Номенклатура, КэшированныеЗначения.ОрганизацияДляСтавкиНДС, ПараметрыПолученияСтавки.Дата);
...
Процедура ОпределитьОбъектыСКомандамиОтчетов(Объекты) Экспорт
	...
	Объекты.Добавить(Метаданные.Документы.ДокументОПредстоящейПоставке);
...
Процедура ДополнитьОписаниеОперацийИТиповДокументовДляЖурналаДокументовЗакупки(ХозяйственныеОперацииИДокументы) Экспорт
	...
	// ДокументОПредстоящейПоставке
	
	СтрокаДокументОПредстоящейПоставке = ХозяйственныеОперацииИДокументы.Добавить();
	Строка = СтрокаДокументОПредстоящейПоставке;
	Строка.ХозяйственнаяОперация        = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС;
	Строка.ДобавитьКнопкуСоздать        = Истина;
	Строка.ТипДокумента                 = Тип("ДокументСсылка.ДокументОПредстоящейПоставке");
	Строка.ПолноеИмяДокумента           = "Документ.ДокументОПредстоящейПоставке";
	Строка.ИспользуютсяСтатусы          = Ложь;
	Строка.КлючНазначенияИспользования  = "ДокументОПредстоящейПоставке";
	Строка.ЗаголовокРабочегоМеста = НСтр("ru = 'Документ о предстоящей поставке (ДОПП)'");
...
Функция СобытиеДокументыОПредварительныхПоставках(Объект, СуммаПоДокументу = 0) Экспорт
	...
	ЕстьПравоЧтенияДОПП = ПравоДоступа("Чтение", Метаданные.Документы.ДокументОПредстоящейПоставке);
	
	Если Не ЕстьПравоЧтенияДОПП Тогда
		Возврат Представление;
	КонецЕсли;
	
	ДанныеДОПП = Документы.ДокументОПредстоящейПоставке.НайтиДОПППоОснованию(Объект.Ссылка);
	
	МассивДОПП = ДанныеДОПП.МассивДОПП;
	НавигационнаяСсылкаДОПП = "";
	ЕстьПравоДобавленияДОПП = ПравоДоступа("ИнтерактивноеДобавление", Метаданные.Документы.ДокументОПредстоящейПоставке);
	...
	ИначеЕсли МассивДОПП.Количество() > 0 Тогда
		...
		Если МассивДОПП.Количество() = 1 Тогда
			...
			ТекстДОПП  = СтрШаблон(НСтр("ru = 'от %1'"),Формат(МассивДОПП[0].Дата, "ДЛФ=D"));
			Если ЗначениеЗаполнено(ЗначениеЗаполнено(МассивДОПП[0].НомерДОПП)) Тогда
				ТекстДОПП = СтрШаблон(НСтр("ru = ' № %1 '"), МассивДОПП[0].НомерДОПП) + ТекстДОПП;
			КонецЕсли;
			
			ТекстРаспоряжения = НСтр("ru = 'Документ о предстоящей поставке'")
								+ ТекстДОПП;
