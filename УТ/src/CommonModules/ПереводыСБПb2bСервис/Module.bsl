...
Функция ОпределитьСтатусОплаты(
	...
	Если Не ЗначениеЗаполнено(ДанныеОперации.КодОшибки) Тогда
		...
		ЗаполнитьЗначенияСвойств(
			...
			|РасчетныйСчет,
			|СуммаОперации");
		Если РезультатОперации.СтатусОперации = НастройкиПлатежныхСистемСлужебный.ИдентификаторСтатусаВПроцессе() Тогда
...
Функция АнализИсторическихДанныхОплаты(
	...
	Если ИсторическиеДанные <> Неопределено
		...
		И ИсторическиеДанные.ОблагаетсяНДС = ЗаказНаОплату.ОблагаетсяНДС
...
Процедура ПриЗагрузкеСтатусаОперации(
	...
	ИначеЕсли СтатусОперацииСервис = НастройкиПлатежныхСистемСлужебный.ИдентификаторСтатусаОтклонена() Тогда
	...
	Попытка
		...
		ДанныеОперации = РегистрыСведений.ИдентификаторыОперацийСБПb2b.ДанныеОперации(ДокументОперации);
		
		ПараметрыОперации.Вставить("РасчетныйСчет", ДанныеОперации.РасчетныйСчет);
		ПараметрыОперации.Вставить("ОблагаетсяНДС", ДанныеОперации.ОблагаетсяНДС);
		ПараметрыОперации.Вставить("СуммаНДС", ДанныеОперации.СуммаНДС);
	...
	Для Каждого КлючЗначение Из ОтложенныеОперации Цикл
		...
		Для Каждого РезультатОперация Из РезультатПроверки Цикл
			...
			Если ЗначениеЗаполнено(ДанныеОперации.КодОшибки) Тогда
				...
				Если ОтложеннаяОперация.КоличествоПопыток >= НастройкиПлатежныхСистемСлужебный.МаксимальноеКоличествоПопытокЗапросаСтатуса(ИмяПодсистемыСБП, Истина) Тогда
					ПриЗагрузкеСтатусаОперации(
						ОтложеннаяОперация.ДокументОперации,
						КлючЗначение.Ключ,
						ПереводыСБПb2bСлужебный.НовыйОписаниеПараметровОперации(),
			...
			Если ДанныеОперации.СтатусОплаты = НастройкиПлатежныхСистемСлужебный.ИдентификаторСтатусаВПроцессе()
				И (ОтложеннаяОперация.ПериодИспользования + 600) < ТекущаяДатаСеанса() Тогда
				ДанныеОперации.СтатусОплаты = НастройкиПлатежныхСистемСлужебный.ИдентификаторСтатусаОтклонена();
			КонецЕсли;
			...
			Если ДанныеОперации.СтатусОплаты = НастройкиПлатежныхСистемСлужебный.ИдентификаторСтатусаВыполнена()
				Или ДанныеОперации.СтатусОплаты = НастройкиПлатежныхСистемСлужебный.ИдентификаторСтатусаОтклонена() Тогда
			...
			Для Каждого РезультатОперация Из РезультатПроверки Цикл
				...
				Если ЗначениеЗаполнено(ДанныеОперации.КодОшибки) Тогда
					СистемаБыстрыхПлатежейСлужебный.УвеличитьКоличествоПопытокЗапросаСтатуса(
						...
						Метаданные.РегистрыСведений.ИдентификаторыОперацийСБПb2b.Имя);
					ИмяПодсистемыСБП = НастройкиПлатежныхСистемКлиентСервер.ИмяПодсистемыСБП();
					Если Операция.КоличествоПопыток >= НастройкиПлатежныхСистемСлужебный.МаксимальноеКоличествоПопытокЗапросаСтатуса(ИмяПодсистемыСБП, Истина) Тогда
						ПриЗагрузкеСтатусаОперации(
							Операция.ДокументОперации,
							КлючЗначение.Ключ,
							ПереводыСБПb2bСлужебный.НовыйОписаниеПараметровОперации(),
...
Функция ОперацияРегистрацияСтатическойСсылки(
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		РезультатОперации.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось зарегистрировать статическую ссылку.
				|
				|%1
				|
				|%2'"),
	...
	Если ЗапросНаличияСтатуса Тогда
		...
		Для Каждого ИдентификаторОплаты Из ИдентификаторыОплаты Цикл
			Если РезультатПроверки.Идентификаторы.Найти(ИдентификаторОплаты.Идентификатор) = Неопределено Тогда
				...
				ОписаниеСтатуса.СтатусОплаты = НастройкиПлатежныхСистемСлужебный.ИдентификаторСтатусаВПроцессе();
...
Функция ОперацииСТерминальнымСтатусом(
	...
	Если ИдентификаторыОплаты.Количество() = 0 Тогда
		РезультатПроверки.КодОшибки = НастройкиПлатежныхСистемСлужебный.КодОшибкиНеверныйФорматЗапроса();
...
Процедура ПолучитьСтатусыОплатыДинамическойСсылки(
	...
	Если РезультатИПП.Ошибка Тогда
		ЗаполнитьРезультатОперации(
			ИдентификаторыЗапроса,
			НастройкиПлатежныхСистемСлужебный.КодОшибкиНеверныйЛогинИлиПароль(),
