...
Процедура НачатьВыполнениеДействийПоЭДО(ОповещениеОЗавершении, ПараметрыВыполнения, КонтекстДиагностики = Неопределено) Экспорт
	...
	КонтекстДействийПоЭДО.ИдентификаторОперации = КонтекстОжиданияОперации.ИдентификаторОперации;
	КонтекстДействийПоЭДО.ДействияВыполняются = Истина;
	...
	Если Не МобильныйЭДОКлиент.ЭтоМобильныйКлиент()
		И ЭлектроннаяПодписьКлиент.ИспользоватьЭлектронныеПодписи()
...
Процедура ВывестиПредупреждениеОДокументеВНеПоддерживаемомФормате(КонтекстДиагностики, ПараметрыОбработкиОшибок) Экспорт
	
	Ошибка = КонтекстДиагностики.Диагностика.Ошибки[0];
	ТекстЗаголовка = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Ошибка.ВидОшибки, "ЗаголовокПроблемы", 
		НСтр("ru='Формат документа не поддерживается.'")); // Строка
...
Функция НовыйКонтекстДействийПоЭДО()
	...
	Контекст.Вставить("ИдентификаторОперации", Неопределено);
	Контекст.Вставить("ДействияВыполняются", Ложь);
	...
	Контекст.Вставить("ВыполнениеПоТакском", Ложь);
...
Асинх Процедура ВыполнитьДействияПоЭДО(КонтекстДействийПоЭДО)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ЭлектронноеАктированиеЕИС") Тогда
		...
		КонецЕсли;
	КонецЕсли;
	КлючЗамера = ОбщегоНазначенияБЭДКлиент.НачатьЗамер("ОтправитьПолучитьЭДО");
	ОповещениеОЗавершенииЗамера = ОбщегоНазначенияБЭДКлиент.ОповещениеОЗавершенииЗамера(КлючЗамера,
		КонтекстДействийПоЭДО.ОповещениеОЗавершении);
	КонтекстДействийПоЭДО.ОповещениеОЗавершении = ОповещениеОЗавершенииЗамера;
...
Процедура ПродолжитьВыполнениеДействийПоЭДОПослеДлительнойОперации(ДлительнаяОперация, КонтекстДействийПоЭДО) Экспорт
	...
	Если КонтекстДействийПоЭДО.ИдентификаторОперации <> Неопределено
		И Не КонтекстДействийПоЭДО.ДействияВыполняются Тогда
		Возврат;
	КонецЕсли;
	...
	РезультатыПоИнтеграциямЭДО = ЭлектронныеДокументыЭДОВызовСервера.РезультатыВыполненияДействийПоИнтеграциямЭДОДляКлиента(
		АдресРезультата, ДлительнаяОперация.Сообщения);
...
Процедура ПродолжитьВыполнениеДействийПоЭДОПослеОбработкиРезультата(РезультатОбработки, КонтекстДействийПоЭДО) Экспорт
	...
	ИначеЕсли Не КонтекстДействийПоЭДО.ВыполнениеПоТакском Тогда
		
		Если ЭлектронныеДокументыЭДОВызовСервера.ТребуетсяВыполнениеДействийПоAPIТакском() Тогда
			ВыполнитьДействияЭДОПриПереходеСТакском(КонтекстДействийПоЭДО);	
		Иначе
			ЗавершитьВыполнениеДействийПоЭДО(КонтекстДействийПоЭДО);
		КонецЕсли;
...
Процедура ВыполнитьДействияПоИзвещениямДокументовЭДО(КонтекстДействийПоЭДО)
	...
	ПараметрыВыполнения.ВыполнитьТолькоПоТакском = КонтекстДействийПоЭДО.ВыполнениеПоТакском; 
...
// Параметры:
//  КонтекстДействийПоЭДО - См. НовыйКонтекстДействийПоЭДО
Процедура ВыполнитьДействияЭДОПриПереходеСТакском(КонтекстДействийПоЭДО)
	
	ИдентификаторыОрганизацийПереводимыеИзТакском = ОбщегоНазначенияБЭДКлиентСервер.ПересечениеМассивов(
		КонтекстДействийПоЭДО.ПараметрыВыполненияДействий.ОбъектыДействий.ИдентификаторыОрганизаций,
		ЭлектронныеДокументыЭДОВызовСервера.СписокУчетныхЗаписейПереведенныхИзТакском());
		
	ПараметрыВыполнения = ИнтерфейсДокументовЭДОКлиентСервер.НовыеПараметрыВыполненияДействийПоЭДО(); 
	ПараметрыВыполнения.ВыполнитьТолькоПоТакском = Истина;
	ПараметрыВыполнения.ОбъектыДействий.ИдентификаторыОрганизаций = ИдентификаторыОрганизацийПереводимыеИзТакском;
	ПараметрыВыполнения.ОбъектыДействий.ЭлектронныеДокументы = КонтекстДействийПоЭДО.ПараметрыВыполненияДействий.ОбъектыДействий.ЭлектронныеДокументы;
	ПараметрыВыполнения.ОбъектыДействий.ПакетыДокументов = КонтекстДействийПоЭДО.ПараметрыВыполненияДействий.ОбъектыДействий.ПакетыДокументов;
	ПараметрыВыполнения.ОбъектыДействий.ТранспортныеКонтейнеры = КонтекстДействийПоЭДО.ПараметрыВыполненияДействий.ОбъектыДействий.ТранспортныеКонтейнеры;
	ПараметрыВыполнения.ОтпечаткиСертификатов = КонтекстДействийПоЭДО.ПараметрыВыполненияДействий.ОтпечаткиСертификатов;
	...
	Если ЭлектронныеДокументыЭДОКлиентСервер.ЕстьДействие(ТекущийНаборДействий, ДействиеЗагрузить) Тогда
		ИнтерфейсДокументовЭДОКлиентСервер.ДобавитьДействие(НаборДействий, ДействиеЗагрузить);
	КонецЕсли;
	...
	КонтекстДействийПоЭДО.ПараметрыВыполненияДействий = ПараметрыВыполнения;
	КонтекстДействийПоЭДО.СостоянияВыполненияДействий = НовыеСостоянияВыполненияДействийПоИнтеграциямЭДО();
	КонтекстДействийПоЭДО.ВыполнениеПоТакском = Истина;
...
Процедура ЗавершитьВыполнениеДействийПоЭДО(КонтекстДействийПоЭДО, РезультатДействийПоЭДО = Неопределено)
	...
	Если КонтекстДействийПоЭДО.ИдентификаторОперации <> Неопределено Тогда
		ОжиданиеОперацийБЭДКлиент.ЗакрытьФормуОжиданияПоИдентификаторуОперации(
			КонтекстДействийПоЭДО.ИдентификаторОперации);
	КонецЕсли;
	...
	Результат.Вставить("ПаролиСертификатов", КонтекстДействийПоЭДО.ПаролиСертификатов);
...
Процедура ЗавершитьВыполнениеДействийПоЭДОПослеПрерывания(РезультатПрерывания, КонтекстДействийПоЭДО) Экспорт
	...
	КонтекстДействийПоЭДО.ДействияВыполняются = Ложь;
...
Процедура ОбработатьРезультатПроверкиПодписейСервера(ОповещениеОЗавершении, РезультатПроверки, КонтекстДиагностики)
	...
	ПараметрыПроверки.ОбработкаПроверкиПодписи = Новый ОписаниеОповещения(
		"ОбработатьРезультатПроверкиПодписиДокумента", ЭлектронныеДокументыЭДОКлиент,
		ОбщегоНазначенияКлиент.СкопироватьРекурсивно(ПараметрыПроверки));
