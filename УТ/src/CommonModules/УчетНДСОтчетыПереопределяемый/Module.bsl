...
#Область ОтражениеДОППВДекларацииПоКосвеннымНалогам

//  КБК НДС при ввозе товаров.
// 
// Возвращаемое значение:
//  Строка - КБК НДС при ввозе товаров
Функция КБКНДСПриВвозеТоваров() Экспорт
	
	Возврат "18210401000011000110";
	
КонецФункции

// Нулевой КБК при ввозе товаров.
// 
// Возвращаемое значение:
//  Строка - Нулевой КБК при ввозе товаров
Функция НулевойКБКПриВвозеТоваров() Экспорт
	
	Возврат "00000000000000000000";
...
Функция ТекстЗапросаРеквизитыСчетовФактурПостановление981КнигаПродаж() Экспорт
	...
	|	И Операция.Организация В (&Организация)
	|	И НЕ Операция.Исправление
	|	И НЕ (Операция.Корректировочный И Операция.НачислениеНДСПриОтгрузке)
	...
	|	Операция.Организация  КАК Организация,
	|	Операция.Ссылка КАК СчетФактура,
	|	Операция.НомерБилета КАК НомерСчетаФактуры,
	|	Операция.Дата КАК ДатаСчетаФактуры,
	|	Операция.Ссылка КАК СчетФактураДокумент,
	|	НЕОПРЕДЕЛЕНО КАК СтавкаНДСАванса,
	|	НЕОПРЕДЕЛЕНО КАК ДоговорАванса,
