...
Процедура НачатьПроверкуКодовМаркировкиСредствамиККТ(ПараметрыНачалаПроверки) Экспорт
	...
	Для Каждого ЭлементПроверки Из ПараметрыПроверки.ЭлементыПроверки Цикл
		...
		Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(ЭлементПроверки.ВидПродукции, Истина)
			...
			РезультатПроверки.ТребуетсяРазрешительныйЗапросГИСМТ = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ПродукцияПодлежитОбязательнойОнлайнПроверкеПередРозничнойПродажей(
				...
				ПараметрыПроверки.ПараметрыСканирования.ТребуетсяПроверкаСредствамиККТ Или ПараметрыПроверки.ПараметрыСканирования.ЭтоОблачнаяККТИСМП);
			...
			КонецЕсли;
			Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(ЭлементПроверки.ВидПродукции) Тогда
				ПараметрыПроверки.ЕстьДанныеДляПроверкиМОТП = Истина;
			КонецЕсли;
		ИначеЕсли КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(ЭлементПроверки.ВидПродукции) Тогда
...
Процедура ВыполнениеШагаПроверкиСредствамиККТ(Форма, ПараметрыПроверки) Экспорт
	...
	ИначеЕсли ПараметрыПроверки.ТекущаяОперация = ВыполняемыеОперации.ПроверкаМРЦ Тогда
		
		ПроверкаМРЦПоЦенеПродажи(ПараметрыПроверки);
...
Процедура АвторизацияПользователяЗавершение(Результат, ПараметрыЗавершенияОбработкиШтрихкода) Экспорт
	...
	ИначеЕсли ТипЗнч(Результат[Организация]) = Тип("Строка") Тогда
		...
		Иначе
			...
			ОткрытьФорму("Обработка.ПроверкаИПодборПродукцииИС.Форма.ИнформацияОНевозможностиДобавленияОтсканированного", ПараметрыОткрытияФормы);
		КонецЕсли;
...
//   * РезультатыОбработкиШтрихкода - Соответствие, см. ШтрихкодированиеОбщегоНазначенияИС.ИнициализироватьРезультатОбработкиШтрихкода
Асинх Функция ОбработатьАсинхронноШтрихкодыНаЛокальномМодулеЧестныйЗнакПриПробитии(ПараметрыПроверки, ДанныеОбработки, СписокЛокальныхМодулейДляОбработки = Неопределено) Экспорт
	...
	ЗаполнитьДанныеКодаМаркировкиПоПолученнымДаннымРазрешительногоРежима(МассивСтрокДляОбработки, РезультатОффлайнЗапроса.СтатусыКодовМаркировкиГИСМТ);
	
	ВозвращаемоеЗначение.РезультатыОбработкиШтрихкода = ШтрихкодированиеОбщегоНазначенияИСМПВызовСервера.ОбработатьСписокШтрихкодовПослеУточненияНаТСПИоТ(
		ДанныеОбработки,
		ПараметрыСканирования,
		ФормаБлокировкиПоПараметрамПроверки(ПараметрыПроверки).УникальныйИдентификатор,
		РезультатОффлайнЗапроса.АварийныйРежим);
...
Асинх Функция ОбработатьАсинхронноШтрихкодНаЛокальномМодулеЧестныйЗнак(ПараметрыЗавершенияВводаШтрихкода) Экспорт
	...
	Иначе
		...
		ВозвращаемоеЗначение.Отказ                             = ЗначениеЗаполнено(РезультатОбработки.ТекстОшибки);
...
#Область ПроверкаШтрихкодовВЛокальномМодулеЧестногоЗнака
...
//  ОписаниеПослеЗавершения               - ОписаниеОповещения - процедура, выполняемая после обработки штрихкода на ЛМ ЧЗ
//  ОперацияПробития                      - Булево - Истина, если обработка штрихкода на ЛМ ЧЗ осуществляется при пробитии чека в рамках обновления идентификаторов РР
// Возвращаемое значение:
//  Булево - Истина, если обработка штрихкода на ЛМ ЧЗ проведена успешно.
Асинх Функция ОбработатьАсинхронноШтрихкод(ПараметрыЗавершенияОбработкиШтрихкода, ОписаниеПослеЗавершения = Неопределено, ОперацияПробития = Ложь) Экспорт
	...
	Если ШтрихкодированиеОбщегоНазначенияИСКлиент.ПрисутствуетПродукцияИСМП(ПараметрыСканирования.ДопустимыеВидыПродукции, Истина)
		...
		ПроверкаЗавершена = ЗавершитьПроверкуШтрихкодаПослеПолученияДанныхЛокальногоМодуляИСМП(Результат, ОписаниеПослеЗавершения, ОперацияПробития);
...
Функция ЭлементПроверкиСредствамиКТТПроверяетсяНаОборудовании(ЭлементПроверки) Экспорт
	...
	Возврат Не (КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ЭлементПроверки.ВидПродукции)
		Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(ЭлементПроверки.ВидПродукции));
...
Функция ДанныеДляПроверкиСредствамиККТСодержатОшибки(ПараметрыПроверки)
	...
	Для Каждого СтрокаПроверки Из ПараметрыПроверки.Результат.ЭлементыПроверки Цикл
		...
		Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(СтрокаПроверки.ВидПродукции) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(СтрокаПроверки.ПолныйКодМаркировки) Тогда
			...
			Если ПараметрыПроверки.ЭтоОблачнаяККТ
				И (СтрокаПроверки.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Потребительская")
				 Или СтрокаПроверки.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Групповая")
				 Или СтрокаПроверки.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Набор")) Тогда
			...
			ИначеЕсли Не ПараметрыСканирования.ТребуетсяПроверкаСредствамиККТ
				И СтрокаПроверки.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Потребительская")
				И КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(СтрокаПроверки.ВидПродукции) Тогда
		...
		ИначеЕсли ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ЗапрещенаРаботаРазрешительногоРежимаСФФД_1_05(
			...
			СтрокаПроверки.ЗапрещеноИгнорироватьОшибку = Истина;
...
// * ЕстьДанныеДляПроверкиИСМП - Булево - наличие продукции ИСМП (включая МОТП)
// * ЕстьДанныеДляПроверкиЕГАИС - Булево - наличие продукции ЕГАИС
// * ЕстьДанныеДляПроверкиМОТП - Булево - наличие продукции МОТП
...
Функция НовыеПараметрыПроверкиНаККТ(ПараметрыНачалаПроверки)
	...
	ВозвращаемоеЗначение.Вставить("ЕстьДанныеДляПроверкиМОТП",   Ложь);
...
Процедура ДополнитьРезультатПроверкиСредствамиККТДаннымиОстаткаЧастичногоВыбытия(ПараметрыПроверки, ЭлементПроверки, РезультатПроверки)
	...
	ОперацияИС = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ОперацияИС(
...
Процедура ПроверкаКМСредствамиККТЕдинымМетодомБПО(ПараметрыПроверки)
	...
	Для Каждого ЭлементПроверки Из ПараметрыПроверки.ЭлементыПроверки Цикл
		...
		ИначеЕсли ЗначениеЗаполнено(ЭлементПроверки.ЧастичноеВыбытиеКоличество) Тогда
			...
			Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ИспользоватьЧислительИЗнаменательПриЧастичномВыбытииНаККТ(ЭлементПроверки.ВидПродукции) Тогда
			...
			Иначе
				...
				ПараметрыЗапросаКМ.МераКоличестваПредметаРасчета = КонструкторИСМПОбщегоНазначенияКлиентСервер.МераКоличестваПредметаРасчетаПриЧастичномВыбытии(ЭлементПроверки.ВидПродукции);
...
Процедура ЛокальнаяПроверкаИОтправкаЗапроса(ПараметрыПроверки)
	...
	ИначеЕсли КонструкторИСМПОбщегоНазначенияКлиентСервер.ИспользоватьЧислительИЗнаменательПриЧастичномВыбытииНаККТ(ЭлементПроверки.ВидПродукции) Тогда
	...
	ИначеЕсли КонструкторИСМПОбщегоНазначенияКлиентСервер.ПоддерживаетсяПодключениеКОборудованиюРозлива(ЭлементПроверки.ВидПродукции) Тогда
...
Процедура ОбновлениеCDNПлощадокГИСМТ(ПараметрыПроверки)
	...
	ПараметрыЗапроса.Параметры               = Новый Структура("НеВыводитьОкноОжидания, НомерФискальногоНакопителя", Истина, ПараметрыПроверки.ПараметрыСканирования.НомерФискальногоНакопителя);
	
	ИнтерфейсИСМПОбщегоНазначенияКлиент.АктуализацияСпискаCDNПлощадок(ПараметрыЗапроса);
	
КонецПроцедуры

Процедура ПроверкаМРЦПоЦенеПродажи(ПараметрыПроверки)
	
	ЭлементыПроверки        = ПараметрыПроверки.ЭлементыПроверки;
	
	Для Каждого ЭлементПроверки Из ЭлементыПроверки Цикл
		
		Если ПараметрыПроверки.ЭтоСканирование И Не ЗначениеЗаполнено(ЭлементПроверки.Сумма) Тогда
			Продолжить;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ЭлементПроверки.МинимальнаяРозничнаяЦена) Тогда
			Продолжить;
		КонецЕсли;
		
		Если Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноУказаниеМинимальнойРозничнойЦены(ЭлементПроверки.ВидПродукции) Тогда
			Продолжить;
		КонецЕсли;
		
		ЦенаПродажиЕдиницы        = Окр(ЭлементПроверки.Сумма / ЭлементПроверки.Количество, 2);
		ШаблонОшибкиПроверкаПоМРЦ = НСтр("ru = 'Минимальная розничная цена по данным ГИС МТ - %1 руб.
										|Продажа по цене %2 руб. запрещена.'");
		ТекстОшибкиПроверкаПоМРЦ  = СтрШаблон(ШаблонОшибкиПроверкаПоМРЦ, ЭлементПроверки.МинимальнаяРозничнаяЦена, ЦенаПродажиЕдиницы);
		
		Если ПараметрыПроверки.ВыполняетсяЛогирование Тогда
			
			ШаблонПроверки = НСтр("ru = 'МРЦ - %1 рублей.
								  |Цена продажи - %2 рублей.'");
			ТекстПроверки  = СтрШаблон(ШаблонПроверки, ЭлементПроверки.МинимальнаяРозничнаяЦена, ЦенаПродажиЕдиницы);
			
			ТекстЛога = ТекстДляЗаписиВЛогЗапросов(
				НСтр("ru = 'Проверка МРЦ'"),
				ПараметрыПроверки,
		...
		Если ЭлементПроверки.МинимальнаяРозничнаяЦена > ЦенаПродажиЕдиницы Тогда
			
			РезультатЭлементаПроверки = ПараметрыПроверки.Результат.ДанныеПроверки.Получить(ЭлементПроверки.ИдентификаторЭлемента);
			
			РезультатЭлементаПроверки.ТекстОшибки    = ТекстОшибкиПроверкаПоМРЦ;
			ПараметрыПроверки.Результат.ТекстОшибки  = ТекстОшибкиПроверкаПоМРЦ;
			
			ПараметрыПроверки.Результат.ЕстьОшибки        = Истина;
			ПараметрыПроверки.ЗапрещеноИгнорироватьОшибку = Ложь;
...
Процедура ПроверкаИдентификатораГИСМТПоКодуМаркировки(ПараметрыПроверки)
	...
	Для Каждого ЭлементПроверки Из ЭлементыПроверки Цикл
		...
		Если Не ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ПродукцияПодлежитОбязательнойОнлайнПроверкеПередРозничнойПродажей(ЭлементПроверки.ВидПродукции, ПараметрыСканирования.ВидОперацииИСМП, ПараметрыСканирования.ТребуетсяПроверкаСредствамиККТ Или ПараметрыСканирования.ЭтоОблачнаяККТИСМП) Тогда
	...
	Если ДанныеДляЗапроса.Количество() = 0 Тогда
		...
		Если ПараметрыПроверки.ЕстьДанныеДляПроверкиМОТП Тогда
		
			ПараметрыПроверки.ТекущаяОперация = ВыполняемыеОперации.ПроверкаМРЦ;
...
Асинх Процедура ПолучениеИдентификатораГИСМТЗавершение(Результат, ПараметрыПроверки) Экспорт
	...
	Если ДанныеПроверкиНаТСПИоТ.Количество() Тогда
		...
		Если ПараметрыПроверки.ВыполняетсяЛогирование Тогда
			...
			ТекстЛога = ТекстДляЗаписиВЛогЗапросов(
				НСтр("ru = 'Запрос контрольной марки на ТС ПИоТ'"),
				ПараметрыПроверки);
		...
		ДополнительныеПараметрыЗапросаТСПИоТ = Новый Структура();
		
		ДополнительныеПараметрыЗапросаТСПИоТ.Вставить("ПараметрыПроверки",                  ПараметрыПроверки);
		ДополнительныеПараметрыЗапросаТСПИоТ.Вставить("СоответствиеСтрокЭлементовПроверки", СоответствиеСтрокЭлементовПроверки);
		ДополнительныеПараметрыЗапросаТСПИоТ.Вставить("ДанныеПроверкиНаТСПИоТ",             ДанныеПроверкиНаТСПИоТ);
		ДополнительныеПараметрыЗапросаТСПИоТ.Вставить("ДанныеНастройкиТСПИоТ",              ДанныеНастройкиТСПИоТ);
		
		ОповещениеПослеВыполненияПроверкиНаТСПИоТ = Новый ОписаниеОповещения("ПолучениеДанныхТСПИоТЗавершение", ЭтотОбъект, ДополнительныеПараметрыЗапросаТСПИоТ);
		
		МодульОбработки = ОбщегоНазначенияКлиент.ОбщийМодуль("ИнтеграцияТСПИоТКлиент");
		РезультатОбработкиНаТСПИоТ = Ждать МодульОбработки.ОбработатьАсинхронноШтрихкодыНаТСПИоТПриПробитии(ДанныеПроверкиНаТСПИоТ,
			ПараметрыПроверки,
			ДанныеНастройкиТСПИоТ,
			ОповещениеПослеВыполненияПроверкиНаТСПИоТ);
		...
		Если РезультатОбработкиНаТСПИоТ.НачатаПроверкаНаВнутреннемТСПИоТ Тогда
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли ДанныеПроверкиНаЛокальномМодуле.Количество() Тогда
		...
		РезультатОффлайнРежима = Ждать ОбработатьАсинхронноШтрихкодыНаЛокальномМодулеЧестныйЗнакПриПробитии(
			ПараметрыПроверки,
		...
		Для Каждого РезультатОбработкиШтрихкода Из РезультатОффлайнРежима.РезультатыОбработкиШтрихкода Цикл
	...
	Для Каждого КлючИЗначение Из ДанныеОтвета.ДанныеИдентификаторов Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеПоКМ, "КонтролиРазрешительногоРежима") Тогда
			...
			ИначеЕсли Не ДействуетАварийныйРежим Тогда
				...
				Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноУказаниеМинимальнойРозничнойЦены(ДанныеПоКМ.ВидПродукции) Тогда
					ЭлементПроверки.МинимальнаяРозничнаяЦена                 = ДанныеПоКМ.МинимальнаяРозничнаяЦена;
				КонецЕсли;
...
Процедура ПолучениеДанныхТСПИоТЗавершение(РезультатОбработкиНаТСПИоТ, ДополнительныеПараметры) Экспорт
	
	ДействуетАварийныйРежим            = Ложь;
	ПараметрыПроверки                  = ДополнительныеПараметры.ПараметрыПроверки;
	СоответствиеСтрокЭлементовПроверки = ДополнительныеПараметры.СоответствиеСтрокЭлементовПроверки;
	
	Если РезультатОбработкиНаТСПИоТ.АварийныйРежим Тогда
		ДействуетАварийныйРежим = Истина;
	КонецЕсли;
	
	Если РезультатОбработкиНаТСПИоТ.Отказ Тогда
		
		Если Не РезультатОбработкиНаТСПИоТ.СтатусыТСПИоТ = Неопределено
			И Не ИнтеграцияТСПИоТКлиентСервер.ПроверкаГотовностиТСПИоТаКПроверкеКМ(РезультатОбработкиНаТСПИоТ.СтатусыТСПИоТ) Тогда
			
			ПараметрыОткрытия = Новый Структура();
			ПараметрыОткрытия.Вставить("ПодключаемоеОборудование", РезультатОбработкиНаТСПИоТ.ПодключаемоеОборудование);
			ПараметрыОткрытия.Вставить("ДанныеСтатусаТСПИоТ",      РезультатОбработкиНаТСПИоТ.СтатусыТСПИоТ);
			...
			ОткрытьФорму("ОбщаяФорма.ФормаПроверкиСтатусовГотовностиТСПИоТ", ПараметрыОткрытия, ДополнительныеПараметры.ПараметрыПроверки.ФормаОсновногоОбъекта,,,, ПослеЗакрытияПроверкиСтатусов);
			Возврат;
	...
	Для Каждого РезультатОбработкиШтрихкода Из РезультатОбработкиНаТСПИоТ.ОбработанныеСтрокиПроверки Цикл
		
		ЭлементПроверки = СоответствиеСтрокЭлементовПроверки.Получить(РезультатОбработкиШтрихкода.Штрихкод);
		
		Если ЭлементПроверки = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ДанныеИдентификаторов.Вставить(ПараметрыПроверки.ЭлементыПроверки.Найти(ЭлементПроверки), РезультатОбработкиШтрихкода);
		
	КонецЦикла;
	
	Для Каждого КлючИЗначение Из ДанныеИдентификаторов Цикл
		
		ЭлементПроверки = ПараметрыПроверки.ЭлементыПроверки[КлючИЗначение.Ключ];
		ДанныеПоКМ      = КлючИЗначение.Значение;
...
Функция ЗавершитьПроверкуШтрихкодаПослеПолученияДанныхЛокальногоМодуляИСМП(Результат, ОписаниеПослеЗавершения, ОперацияПробития = Ложь) Экспорт
	...
	РезультатОбработкиШтрихкода = ШтрихкодированиеОбщегоНазначенияИСМПВызовСервера.ОбработатьШтрихкодыПослеУточненияНаЛокальномМодулеЧестногоЗнака(ДанныеШтрихкода,
		ПараметрыСканирования,
		Форма.УникальныйИдентификатор,
		Результат.АварийныйРежим,
		Результат.ЗапросыДляЛогирования);
	
	РезультатОбработкиШтрихкода.ТребуетсяПроверкаРазрешительногоРежимаНаЛокальномМодуле = Ложь;
...
Функция ЗавершитьОбработкуКодаМаркировкиПоУточнениямИПроверкам(ПараметрыЗавершенияВводаШтрихкода, РезультатОбработкиШтрихкода, ВыполнятьОбработчикОповещения)
	...
	Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(
...
Асинх Процедура ПослеЗавершенияПроверкиГотовностиСтатусаТСПИоТ(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ОповещениеПослеВыполненияПроверкиНаТСПИоТ = Новый ОписаниеОповещения("ПолучениеДанныхТСПИоТЗавершение", ЭтотОбъект, ДополнительныеПараметры);
	
	МодульОбработки = ОбщегоНазначенияКлиент.ОбщийМодуль("ИнтеграцияТСПИоТКлиент");
	РезультатОбработкиНаТСПИоТ = Ждать МодульОбработки.ОбработатьАсинхронноШтрихкодыНаТСПИоТПриПробитии(ДополнительныеПараметры.ДанныеПроверкиНаТСПИоТ,
		ДополнительныеПараметры.ПараметрыПроверки,
		ДополнительныеПараметры.ДанныеНастройкиТСПИоТ,
		ОповещениеПослеВыполненияПроверкиНаТСПИоТ);
...
Процедура ЗаполнитьДанныеКодаМаркировкиПоПолученнымДаннымРазрешительногоРежима(ДанныеКодовМаркировки, СтатусыКодовМаркировки) Экспорт
