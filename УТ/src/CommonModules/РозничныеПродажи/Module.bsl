...
// Возвращает склады касс ККМ с иерархией
// 
// Параметры:
//  КассыККМ - Массив Из СправочникСсылка.КассыККМ - Кассы ККМ
// 
// Возвращаемое значение:
//  Массив Из СправочникСсылка.Склады
Функция СкладыКассККМВИерархии(КассыККМ) Экспорт
	...
	|		И КассыККМ.Ссылка В (&КассыККМ)
	|		И КассыККМ.ТипКассы = &ТипКассыФискальныйРегистратор";
	...
	Возврат СкладыКассСРодителямиВИерархии(Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Склад"));
...
// Возвращаемое значение:
//  Массив Из СправочникСсылка.Склады
Функция СкладыКассСРодителямиВИерархии(Склады) Экспорт
	
	СкладыКассККМ = Склады;
	
	Родители = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(СкладыКассККМ, "Родитель");
	
	МассивРодителейКОбработке = Новый Массив;
	Для Каждого ЗначениеРодителя Из Родители Цикл
		Если ЗначениеЗаполнено(ЗначениеРодителя.Значение.Родитель) Тогда
			МассивРодителейКОбработке.Добавить(ЗначениеРодителя.Значение.Родитель);
			СкладыКассККМ.Добавить(ЗначениеРодителя.Значение.Родитель);
		КонецЕсли;
	КонецЦикла;
	
	Пока МассивРодителейКОбработке.Количество() > 0 Цикл
		
		Родители = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(МассивРодителейКОбработке, "Родитель");
		МассивРодителейКОбработке.Очистить();
	...
	Возврат СкладыКассККМ;
	
КонецФункции

// Возвращает иерархию склада
// 
// Параметры:
...
// Возвращаемое значение:
//  Массив Из СправочникСсылка.Склады
Функция СкладВИерархии(Склад) Экспорт
	...
	Склады = СкладыКассСРодителямиВИерархии(Склады);
	...
	Пока Выборка.Следующий() Цикл
		Если Склады.Найти(Выборка.Ссылка) = Неопределено Тогда
...
Процедура НастроитьБыстрыеТоварыИГорячиеКлавиши(Форма, Выборка) Экспорт
	...
	Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(Форма, "БыстрыеТовары")
		...
		БыстрыеТовары = ВыборкаБыстрыеТовары.Получить(); // ТаблицаЗначений - см. Справочники.ГорячиеКлавиши.НоваяКоманда.Таблица
	...
	КомандыФормы = ВыборкаКомандыФормы.Получить(); // ТаблицаЗначений - см. Справочники.ГорячиеКлавиши.НоваяКоманда.Таблица
...
// Обработчик регламентного задания Метаданные.РегламентныеЗадания.УдалениеНеиспользуемыхДанныхОПринадлежностиЗапасовТорговыхТочек
//
Процедура ВыполнитьУдалениеНеиспользуемыхДанныхОПринадлежностиЗапасовТорговыхТочек() Экспорт
	
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.УдалениеНеиспользуемыхДанныхОПринадлежностиЗапасовТорговыхТочек);
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление неиспользуемых данных о принадлежности запасов торговых точек'", ОбщегоНазначения.КодОсновногоЯзыка()),
	                         УровеньЖурналаРегистрации.Информация, , ,
	                         НСтр("ru = 'Начато регламентное удаление неиспользуемых данных о принадлежности запасов торговых точек.'"));

	РегистрыСведений.УточненияПринадлежностиЗапасовТорговыхТочек.ВыполнитьУдалениеНеиспользуемыхДанныхОПринадлежностиЗапасовТорговыхТочек();
	
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление неиспользуемых данных о принадлежности запасов торговых точек'", ОбщегоНазначения.КодОсновногоЯзыка()),
	                         УровеньЖурналаРегистрации.Информация, , ,
	                         НСтр("ru = 'Закончено регламентное удаление неиспользуемых данных о принадлежности запасов торговых точек.'"));
...
//	НалогообложениеНДС - ПеречислениеСсылка - Тип налогообложения НДС в соответствии с котором необходимо заполнить Отчеты о розничных продажх/возвратах.
//	МенеджерВременныхТаблиц - МенеджерВременныхТаблиц -  
//	СтрокиВозвратаТовараВСмену - Массив Из СтрокаТабличнойЧасти - 
//  ОписаниеОшибки - Строка - Возвращаемое описание ошибки.
//
// Возвращаемое значение:
//  ДокументСсылка.ОтчетОРозничныхВозвратах - Ссылка на отчет, если операция выполнена успешно. Иначе - Неопределено.
//
Функция ЗаполнитьОтчетОРозничныхВозвратахПоКассовойСмене(СостояниеКассовойСмены, НалогообложениеНДС, МенеджерВременныхТаблиц, СтрокиВозвратаТовараВСмену, ОписаниеОшибки = "") Экспорт
	...
	Если ОбновлятьОтчеты Тогда
		...
		Возврат ЗаполнитьОтчетОРозничныхВозвратах(
			ОтчетОРозничныхВозвратахОбъект,
			СостояниеКассовойСмены,
			НалогообложениеНДС,
			МенеджерВременныхТаблиц,
			СтрокиВозвратаТовараВСмену,
			,
			ОписаниеОшибки);
	Иначе
...
//	НалогообложениеНДС - ПеречислениеСсылка - Тип налогообложения НДс в соответствии с котором необходимо заполнить Отчеты о розничных продажх/возвратах.
//	МенеджерВременныхТаблиц - МенеджерВременныхТаблиц -
//	СтрокиВозвратаТовараВСмену - Неопределено, Массив Из СтрокаТабличнойЧасти - Строки возврата товара в смену продажи
//	ТолькоЗаполнить - Булево - Признак, определяющий будет ли выполнена попытка проведения/записи после заполнения.
//  ОписаниеОшибки - Строка - Возвращаемое описание ошибки.
//
// Возвращаемое значение:
//  ДокументСсылка.ОтчетОРозничныхВозвратах - Ссылка на отчет, если операция выполнена успешно. Иначе - Неопределено.
//
Функция ЗаполнитьОтчетОРозничныхВозвратах(
	ОтчетОРозничныхВозвратахОбъект,
	СостояниеКассовойСмены,
	НалогообложениеНДС,
	МенеджерВременныхТаблиц,
	СтрокиВозвратаТовараВСмену = Неопределено,
	ТолькоЗаполнить = Ложь,
	...
	Попытка
		...
		|	СписокТоваров.ЧекККМНалогообложениеНДС КАК ЧекККМНалогообложениеНДС,
		|	СписокТоваров.ЧекККМКассоваяСмена КАК ЧекККМКассоваяСмена,
		|	СписокТоваров.ВозвратВСмену КАК ВозвратВСмену
		|ИЗ
		|	(ВЫБРАТЬ
		|		СписокТоваров.ПоЧекуКоррекции КАК ПоЧекуКоррекции,
		|		СписокТоваров.НоменклатураНабора КАК НоменклатураНабора,
		...
		|		СписокТоваров.ЧекККМНалогообложениеНДС КАК ЧекККМНалогообложениеНДС,
		|		СписокТоваров.ЧекККМКассоваяСмена КАК ЧекККМКассоваяСмена,
		|		НЕ (ЧекиККМВозвратПоКоторымОтражаетсяВОРВ.ЧекККМ ЕСТЬ NULL) КАК ВозвратВСмену
		|	ИЗ
		|		СписокТоваров КАК СписокТоваров
		|			ЛЕВОЕ СОЕДИНЕНИЕ ЧекиККМВозвратПоКоторымОтражаетсяВОРВ КАК ЧекиККМВозвратПоКоторымОтражаетсяВОРВ
		|			ПО ЧекиККМВозвратПоКоторымОтражаетсяВОРВ.ЧекККМ = СписокТоваров.ЧекККМСсылка
		|	) КАК СписокТоваров
		|		ЛЕВОЕ СОЕДИНЕНИЕ ИндексыПоАналитике КАК ИндексыПоАналитике
		|		ПО СписокТоваров.Склад = ИндексыПоАналитике.Склад
		|			И СписокТоваров.Валюта = ИндексыПоАналитике.Валюта
		...
		СтрокиВозвратаТовараВСмену = Новый Массив;
		...
		Если ВыборкаПоАналитике.Следующий() Тогда
			...
			Пока ВыборкаПоАналитикеНоменклатуры.Следующий() Цикл
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					...
					Если ВыборкаДетальныеЗаписи.Количество <> 0 Тогда
						...
						ЗаполнитьЗначенияСвойств(СтрокаТабличнойЧастиТовары, ВыборкаДетальныеЗаписи);
						СтрокаТабличнойЧастиТоварыКоличествоУпаковок = ?(
							СтрокаТабличнойЧастиТовары.КоличествоУпаковок = 0,
							ВыборкаДетальныеЗаписи.Количество,
							СтрокаТабличнойЧастиТовары.КоличествоУпаковок);
						СтрокаТабличнойЧастиТовары.Цена = Окр(
							СтрокаТабличнойЧастиТовары.Сумма / СтрокаТабличнойЧастиТоварыКоличествоУпаковок, 15, 2);
						
						ПодобратьЗаполнитьДокументРеализацииВСтроке(
							СтрокаТабличнойЧастиТовары,
							ТаблицаДокументовРеализации,
							СостояниеКассовойСмены,
							ВыборкаДетальныеЗаписи,
							Истина);
						
						Если ВыборкаДетальныеЗаписи.ВозвратВСмену
							И Не ТолькоЗаполнить Тогда
							СтрокаТабличнойЧастиТовары.СпособОпределенияСебестоимости
								 = Перечисления.СпособыОпределенияСебестоимостиВозврата.ИзТекущегоДокумента;
							СтрокиВозвратаТовараВСмену.Добавить(СтрокаТабличнойЧастиТовары);
						КонецЕсли;
			...
			// Заполнение табличной части "Оплата платежными картами".
			МассивСтрок = ОплатаПлатежнымиКартами.НайтиСтроки(
				Новый Структура("ИндексПоАналитике", ВыборкаПоАналитике.ИндексПоАналитике));
		...
		ОтчетОРозничныхВозвратахЛокализация.ОбработкаЗаполнения(ОтчетОРозничныхВозвратахОбъект, Неопределено, Истина);
...
Функция ПодготовитьДанныеПоЧекамКассовойСмены(СостояниеКассовойСмены, МенеджерВременныхТаблиц, ВключатьНепроведенныеЧеки = Ложь) Экспорт
	...
	Запрос.Текст = "
		...
		|	ЧекиККМ.СуммаДокумента КАК СуммаДокумента,
		|	ЧекиККМ.ПолученоНаличными КАК ПолученоНаличными
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТоварыРозничныхПродаж.Ссылка КАК ЧекККМВозврат,
		|	ТоварыРозничныхПродаж.ЧекККМ КАК ЧекККМ,
		|	ТоварыРозничныхПродаж.Ссылка.ВыданоНаличными КАК ВыданоНаличными
		|ПОМЕСТИТЬ ЧекиККМВозвратОформленныеВСмену
		|ИЗ
		|	ТоварыРозничныхВозвратовВСменуПродажи КАК ТоварыРозничныхПродаж
		|
		|ИНДЕКСИРОВАТЬ ПО
		...
		|		МАКСИМУМ(СписокПродажВСмену.Помещение) КАК Помещение,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ОплатыЗаВычетомВозвратовВСмену.ЧекККМ КАК ЧекККМ
		|ПОМЕСТИТЬ ЧекиККМВозвратПоКоторымОтражаетсяВОРВСмешаннаяОплата
		|ИЗ
		|	(ВЫБРАТЬ
		|		ЧекиККМПоКоторымОформленВозвратВСмену.ЧекККМ КАК ЧекККМ,
		|		ЧекККМОплатаПлатежнымиКартами.ВидОплаты КАК ВидОплаты,
		|		ЧекККМОплатаПлатежнымиКартами.ДоговорПодключения КАК ДоговорПодключения,
		|		ЧекККМОплатаПлатежнымиКартами.ЭквайринговыйТерминал КАК ЭквайринговыйТерминал,
		|		ЧекККМОплатаПлатежнымиКартами.Сумма КАК Сумма
		|	ИЗ
		|		ЧекиККМПоКоторымОформленВозвратВСмену КАК ЧекиККМПоКоторымОформленВозвратВСмену
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЧекККМ.ОплатаПлатежнымиКартами КАК ЧекККМОплатаПлатежнымиКартами
		|			ПО ЧекККМОплатаПлатежнымиКартами.Ссылка = ЧекиККМПоКоторымОформленВозвратВСмену.ЧекККМ
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|		НЕОПРЕДЕЛЕНО,
		|		НЕОПРЕДЕЛЕНО,
		|		ЧекиККМПоКоторымОформленВозвратВСмену.ПолученоНаличными
		|	ИЗ
		|		ЧекиККМПоКоторымОформленВозвратВСмену КАК ЧекиККМПоКоторымОформленВозвратВСмену
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		ЧекиККМВозвратОформленныеВСмену.ЧекККМ,
		|		ЧекККМВозвратОплатаПлатежнымиКартами.ВидОплаты,
		|		ЧекККМВозвратОплатаПлатежнымиКартами.ДоговорПодключения,
		|		ЧекККМВозвратОплатаПлатежнымиКартами.ЭквайринговыйТерминал,
		|		-ЧекККМВозвратОплатаПлатежнымиКартами.Сумма
		|	ИЗ
		|		ЧекиККМВозвратОформленныеВСмену КАК ЧекиККМВозвратОформленныеВСмену
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЧекККМВозврат.ОплатаПлатежнымиКартами КАК ЧекККМВозвратОплатаПлатежнымиКартами
		|			ПО ЧекККМВозвратОплатаПлатежнымиКартами.Ссылка = ЧекиККМВозвратОформленныеВСмену.ЧекККМВозврат
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|		НЕОПРЕДЕЛЕНО,
		|		НЕОПРЕДЕЛЕНО,
		|		-ЧекиККМВозвратОформленныеВСмену.ВыданоНаличными
		|	ИЗ
		|		ЧекиККМВозвратОформленныеВСмену КАК ЧекиККМВозвратОформленныеВСмену
		|	) КАК ОплатыЗаВычетомВозвратовВСмену
		|	
		|	СГРУППИРОВАТЬ ПО
		|		ОплатыЗаВычетомВозвратовВСмену.ЧекККМ,
		|		ОплатыЗаВычетомВозвратовВСмену.ВидОплаты,
		|		ОплатыЗаВычетомВозвратовВСмену.ДоговорПодключения,
		|		ОплатыЗаВычетомВозвратовВСмену.ЭквайринговыйТерминал
		|
		|ИМЕЮЩИЕ 
		|	СУММА(ОплатыЗаВычетомВозвратовВСмену.Сумма) < 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ЧекиККМВозвратПоКоторымОтражаетсяВОРВ.ЧекККМ КАК ЧекККМ
		|ПОМЕСТИТЬ ЧекиККМВозвратПоКоторымОтражаетсяВОРВ
		|ИЗ
		|	(ВЫБРАТЬ
		|		ЧекиККМВозвратПоКоторымОтражаетсяВОРВСмешаннаяОплата.ЧекККМ КАК ЧекККМ
		|	ИЗ ЧекиККМВозвратПоКоторымОтражаетсяВОРВСмешаннаяОплата КАК ЧекиККМВозвратПоКоторымОтражаетсяВОРВСмешаннаяОплата
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		ЧекиККМПоКоторымОформленыПроблемныеВозвратыВСмену.ЧекККМ КАК ЧекККМ
		|	ИЗ ЧекиККМПоКоторымОформленыПроблемныеВозвратыВСмену КАК ЧекиККМПоКоторымОформленыПроблемныеВозвратыВСмену
		|	) КАК ЧекиККМВозвратПоКоторымОтражаетсяВОРВ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	НЕ (ЧекиККМПоКоторымОформленыПроблемныеВозвратыВСмену.ЧекККМ ЕСТЬ NULL) КАК ЧекККМПоКоторомуОформленПроблемныйВозврат
		...
		|		ПО ЧекККМТовары.Ссылка = ЧекиККМПоКоторымОформленыПроблемныеВозвратыВСмену.ЧекККМ
		|ГДЕ
		|	ЧекККМТовары.Ссылка.КассоваяСмена = &КассоваяСмена
		|	И (ЧекККМТовары.Ссылка.Проведен
		|			ИЛИ &ВключатьНепроведенныеЧеки И НЕ ЧекККМТовары.Ссылка.ПометкаУдаления)
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ЧекиККМВозвратПоКоторымОтражаетсяВОРВ КАК ЧекиККМВозвратПоКоторымОтражаетсяВОРВ
		|		ПО ЧекККМТовары.Ссылка.ЧекККМ = ЧекиККМВозвратПоКоторымОтражаетсяВОРВ.ЧекККМ
		|ГДЕ
		|	ЧекиККМВозвратПоКоторымОтражаетсяВОРВ.ЧекККМ ЕСТЬ NULL
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	И (ЧекККМКоррекции.Проведен
		|		ИЛИ &ВключатьНепроведенныеЧеки И НЕ ЧекККМТовары.Ссылка.ПометкаУдаления)
		|	И ЧекККМКоррекции.Ссылка.ВидКоррекции = ЗНАЧЕНИЕ(Перечисление.ВидыЧековКоррекции.ИсправлениеОшибки)
		|	И ЧекККМКоррекции.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Пробит)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|	И (ЧекККМТовары.Ссылка.Проведен
		|			ИЛИ &ВключатьНепроведенныеЧеки И НЕ ЧекККМТовары.Ссылка.ПометкаУдаления)
		|	И ЧекККМТовары.Ссылка.ВидКоррекции = ЗНАЧЕНИЕ(Перечисление.ВидыЧековКоррекции.ИсправлениеОшибки)
		|	И ЧекККМТовары.Ссылка.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Пробит)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		...
		|	ЛОЖЬ КАК ПоЧекуКоррекции,
		|	ЧекККМТовары.Ссылка КАК Ссылка,
		|	ЧекККМТовары.Ссылка.ЧекККМ КАК ЧекККМСсылка,
		|	ЧекККМТовары.НоменклатураНабора КАК НоменклатураНабора,
		|	ЧекККМТовары.ХарактеристикаНабора КАК ХарактеристикаНабора,
		|	ЧекККМТовары.Номенклатура КАК Номенклатура,
		|	ЧекККМТовары.Характеристика КАК Характеристика,
		...
		|	ЧекККМТовары.Ссылка.ЧекККМ.НалогообложениеНДС КАК ЧекККМНалогообложениеНДС,
		|	ЧекККМТовары.Ссылка.ЧекККМ.КассоваяСмена КАК ЧекККМКассоваяСмена
		|ИЗ
		|	Документ.ЧекККМВозврат.Товары КАК ЧекККМТовары
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ЧекиККМВозвратПоКоторымОтражаетсяВОРВ КАК ЧекиККМВозвратПоКоторымОтражаетсяВОРВ
		|		ПО ЧекККМТовары.Ссылка.ЧекККМ = ЧекиККМВозвратПоКоторымОтражаетсяВОРВ.ЧекККМ
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|			ИЛИ ЧекККМКоррекции.ВидКоррекции = ЗНАЧЕНИЕ(Перечисление.ВидыЧековКоррекции.НеприменениеККТ)
		...
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА ЧекККМКоррекции.ВидКоррекции = ЗНАЧЕНИЕ(Перечисление.ВидыЧековКоррекции.НеприменениеККТ)
		|			ТОГДА 2
		|		ИНАЧЕ 1
		|	КОНЕЦ
		|ИЗ
		...
		|	И ЧекККМКоррекции.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Пробит)
		|	И ЧекККМКоррекции.КассоваяСмена = &КассоваяСмена
		|	И ЧекККМКоррекции.ВидКоррекции = ЗНАЧЕНИЕ(Перечисление.ВидыЧековКоррекции.ИсправлениеОшибки)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Функция ЗаполнитьОтчетыОРозничныхПродажахИВозвратах(СостояниеКассовойСмены, ОписаниеОшибки, ВключатьНепроведенныеЧеки = Ложь) Экспорт
	...
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	ТипыНалогообложенияНДСПоКассовойСмене = ПодготовитьДанныеПоЧекамКассовойСмены(
		СостояниеКассовойСмены,
		МенеджерВременныхТаблиц,
		ВключатьНепроведенныеЧеки);
	...
	Для Каждого ТипНалогообложенияНДС Из ТипыНалогообложенияНДСПоКассовойСмене Цикл
		...
		// Заполнение отчета о розничных возвратах
		ОтчетОРозничныхВозвратах = ЗаполнитьОтчетОРозничныхВозвратахПоКассовойСмене(
			СостояниеКассовойСмены,
			ТипНалогообложенияНДС.НалогообложениеНДС,
			МенеджерВременныхТаблиц,
			СтрокиВозвратаТовараВСмену,
			ОписаниеОшибкиЗаполнения);
		...
		ОписаниеОшибкиЗаполнения = "";
		// Заполнение отчета о розничных продажах
		ОтчетОРозничныхПродажах = ЗаполнитьОтчетОРозничныхПродажахПоКассовойСмене(
			СостояниеКассовойСмены,
			ТипНалогообложенияНДС.НалогообложениеНДС,
			МенеджерВременныхТаблиц,
			ОписаниеОшибкиЗаполнения);
		...
		Если ОтчетОРозничныхПродажах <> Неопределено
			И ОтчетОРозничныхВозвратах <> Неопределено
			И СтрокиВозвратаТовараВСмену <> Неопределено
			И СтрокиВозвратаТовараВСмену.Количество() > 0 Тогда
			
			ОтчетОРозничныхВозвратахОбъект = ОтчетОРозничныхВозвратах.ПолучитьОбъект();
			Для Каждого СтрокаВозвратаТовара Из СтрокиВозвратаТовараВСмену Цикл
				СтрокаТЧТоварыОРП = ОтчетОРозничныхВозвратахОбъект.Товары[СтрокаВозвратаТовара.НомерСтроки - 1];
				
				СтрокаТЧТоварыОРП.СпособОпределенияСебестоимости
					= Перечисления.СпособыОпределенияСебестоимостиВозврата.ИзДокументаПродажи;
				СтрокаТЧТоварыОРП.ДокументРеализации = ОтчетОРозничныхПродажах;
			КонецЦикла;
			
			Попытка
				
				ОтчетОРозничныхВозвратахОбъект.Записать(РежимЗаписиДокумента.Запись);
				Если ОтчетОРозничныхВозвратахОбъект.Проведен Тогда
					ОтчетОРозничныхВозвратахОбъект.Записать(
						РежимЗаписиДокумента.Проведение,
						РежимПроведенияДокумента.Неоперативный);
				КонецЕсли;
				
				ОтчетОРозничныхПродажахПроведен = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					ОтчетОРозничныхПродажах,
					"Проведен");
				Если ОтчетОРозничныхВозвратахОбъект.Проведен И ОтчетОРозничныхПродажахПроведен Тогда
					ОтчетОРозничныхПродажахОбъект = ОтчетОРозничныхПродажах.ПолучитьОбъект();
					ОтчетОРозничныхПродажахОбъект.Записать(
						РежимЗаписиДокумента.Проведение,
						РежимПроведенияДокумента.Неоперативный);
				КонецЕсли;
			...
			Исключение
				...
				ОписаниеОшибки = НСтр("ru = 'При дозаполнении в ОРВ и ОРП данными возврата в смену с измененным видом оплаты по кассовой смене %КассоваяСмена% произошла ошибка.
		                                     |Дополнительное описание:
		                                     |%ДополнительноеОписание%'");
				ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%КассоваяСмена%", СостояниеКассовойСмены.КассоваяСмена);
				ОписаниеОшибки = СтрЗаменить(
		...
		Если ОтчетОРозничныхВозвратах = Неопределено
			Или ОтчетОРозничныхПродажах = Неопределено
			Или ОписаниеОшибок.Количество() > 0 Тогда
...
Функция ОписаниеКассовойСмены() Экспорт
	...
	СостояниеКассовойСмены.Вставить("НалогообложениеНДС", Перечисления.ТипыНалогообложенияНДС.ПустаяСсылка());
...
Функция ПолучитьДанныеПродажЗаПериод(ПараметрыРасчета) Экспорт
	...
	|		И (ЕСТЬNULL(ОтчетОРозничныхПродажахТовары.Характеристика.Принципал, ОтчетОРозничныхПродажахТовары.Номенклатура.Принципал) = НЕОПРЕДЕЛЕНО
	|			ИЛИ ЕСТЬNULL(ОтчетОРозничныхПродажахТовары.Характеристика.Принципал, ОтчетОРозничныхПродажахТовары.Номенклатура.Принципал) = ОтчетОРозничныхПродажахТовары.Ссылка.Организация)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|			ИЛИ ОтчетКомиссионераТовары.Покупатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка))
	|		И ОтчетКомиссионераТовары.НомерСчетаФактурыКомиссионера = """"
	|		И (ЕСТЬNULL(ОтчетКомиссионераТовары.Характеристика.Принципал, ОтчетКомиссионераТовары.Номенклатура.Принципал) = НЕОПРЕДЕЛЕНО
	|			ИЛИ ЕСТЬNULL(ОтчетКомиссионераТовары.Характеристика.Принципал, ОтчетКомиссионераТовары.Номенклатура.Принципал) = ОтчетКомиссионераТовары.Ссылка.Организация)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		И (ЕСТЬNULL(ОтчетОРозничныхВозвратахТовары.Характеристика.Принципал, ОтчетОРозничныхВозвратахТовары.Номенклатура.Принципал) = НЕОПРЕДЕЛЕНО
	|			ИЛИ ЕСТЬNULL(ОтчетОРозничныхВозвратахТовары.Характеристика.Принципал, ОтчетОРозничныхВозвратахТовары.Номенклатура.Принципал) = ОтчетОРозничныхВозвратахТовары.Ссылка.Организация)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
...
//
Функция РазделитьМассивСтрокДляРазныхТиповПакетнойОбработки(Форма, ИсходныеСтроки) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ТребуетсяЗаполнитьЦену", ОбщегоНазначения.СкопироватьРекурсивно(ИсходныеСтроки));
	Результат.Вставить("НеТребуетсяЗаполнитьЦену", Новый Массив);

	РозничныеПродажиЛокализация.РазделитьМассивСтрокДляРазныхТиповПакетнойОбработки(Форма, ИсходныеСтроки, Результат);
...
Процедура ЗаполнитьОписаниеКассовойСменыПоКассеККМ(ОписаниеКассовойСмены, КассаККМ)
	...
	ПараметрыЗаполнения.Организация = Выборка.Организация;
	ПараметрыЗаполнения.Дата = ТекущаяДатаСеанса();
	ПараметрыЗаполнения.Склад = Выборка.Склад;
	Если ЗначениеЗаполнено(Выборка.Подразделение) Тогда
		ПараметрыЗаполнения.Подразделение = Выборка.Подразделение;
	КонецЕсли;
	ПараметрыЗаполнения.РозничнаяПродажа = Истина;
	УчетНДСУП.ЗаполнитьНалогообложениеНДСПродажи(ОписаниеКассовойСмены.НалогообложениеНДС, ПараметрыЗаполнения);
...
Функция ТекстыЗапросовУсловияРозничныхПродажПакетнаяОбработка(Знач ЗаполнятьПоФорматуМагазинов) Экспорт
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	КурсыВалютыЦены.Валюта КАК Валюта,
	|	КурсыВалютыЦены.КурсЧислитель КАК КурсЧислитель,
	|	КурсыВалютыЦены.КурсЗнаменатель КАК КурсЗнаменатель
	...
	|			Т.ВидЦены
	|		ИЗ
	|			ТаблицаОтбора КАК Т)) КАК ТаблицаЦен
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура,
	|	ХарактеристикаЦО,
	...
	Результат.Добавить(ТекстЗапроса, "ЦеныНоменклатурыСрезПоследних");
	...
	|		И ТаблицаОтбора.ХарактеристикаЦО = ЦеныНоменклатурыСрезПоследних.ХарактеристикаЦО
	|		И ТаблицаОтбора.СерияЦО = ЦеныНоменклатурыСрезПоследних.СерияЦО
	|		И ТаблицаОтбора.УпаковкаЦО = ЦеныНоменклатурыСрезПоследних.УпаковкаЦО
	...
	|ПОМЕСТИТЬ ЗаполнитьУсловияРозничныхПродажПредварительная
	|ИЗ
	...
	Результат.Добавить(ТекстЗапроса, "ЗаполнитьУсловияРозничныхПродажПредварительная");
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ВтИсточникДанных.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|	ЕСТЬNULL(ТаблицаПредварительная.ВидЦены, ЗНАЧЕНИЕ(Справочник.ВидыЦен.ПустаяСсылка)) КАК ВидЦены,
	|	ЕСТЬNULL(ТаблицаПредварительная.Цена, 0) КАК Цена
	|ИЗ
	|	ВтИсточникДанных КАК ВтИсточникДанных
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗаполнитьУсловияРозничныхПродажПредварительная КАК ТаблицаПредварительная
	|		ПО ВтИсточникДанных.ИдентификаторСтрокиВТ = ТаблицаПредварительная.ИдентификаторСтрокиВТ";
