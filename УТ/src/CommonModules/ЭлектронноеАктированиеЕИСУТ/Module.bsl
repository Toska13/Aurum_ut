...
Процедура ПередЗаписьюНаСервереГосударственногоКонтракта(
	...
	Если ТекущийОбъект.ТипСопровождения = Перечисления.ТипыСопровожденияГосударственныхКонтрактов.Банковское Тогда
...
Процедура ПриЗаписиДоговора(Госконтракт, Договор) Экспорт
	...
	Если ВладелецКонтракта <> ДоговорСсылка И Не ЗначениеЗаполнено(Госконтракт.Контрагент) Тогда
...
Процедура ЗаполнитьДанныеДереваДляЭлектронногоАктированияЕИС(ДеревоДанных,
	...
	Пока ВыборкаПодписантов.Следующий() Цикл
		...
		Если НЕ ПодписантДобавлен И Не ЭтоУКД Тогда
...
Процедура ЗаполнитьМестаПоставкиПриложенияЕИСПоАдресуДоставки(АдресДоставки, ИдентификаторМестаПоставкиЕИС, ДанныеПриложенияЕИС) Экспорт
	...
	Если ЗначениеЗаполнено(АдресДоставкиСтруктура) Тогда
		...
		Попытка
			...
			Если ДанныеПриложенияЕИС.Свойство("Контрагент") Тогда
				СведенияОбУчастнике.Вставить("Ссылка", ДанныеПриложенияЕИС.Контрагент);
			Иначе
				СведенияОбУчастнике.Вставить("Ссылка", ПредопределенноеЗначение("Справочник.Контрагенты.ПустаяСсылка"));
			КонецЕсли;
			ПолучитьСведенияАдресаПоГАР	= ПолучитьСведенияАдресаПоГАР(СведенияОбУчастнике, "Факт");
			МестоПоставки.КодГАР		= ПолучитьСведенияАдресаПоГАР.КодГар;
			МестоПоставки.КодСтраны		= ПолучитьСведенияАдресаПоГАР.КодСтраны;
...
// Возвращает модифицированный текст запроса с логикой получения идентификатора госконтракта.
// 
// Параметры:
...
// 	Строка - Текст запроса
//  
Функция ТекстЗапросаИдентификаторГосКонтракта(ТекстЗапроса) Экспорт
	
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИдентификаторГосКонтракта", "ДанныеДокумента.Договор.ИдентификаторГосКонтракта");
