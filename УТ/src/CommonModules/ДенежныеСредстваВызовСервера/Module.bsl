...
Функция ТекстЗапросаВозможностьГруппировкиЗаявок()
	...
	|					ИЛИ Заявка.ФормаОплатыЗаявки = ЗНАЧЕНИЕ(Перечисление.ФормыОплаты.СистемаБыстрыхПлатежей)
	...
	|
	|	ВЫБОР 
	|		КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Заявка.ИдентификаторОперацииСБП) > 1
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК НесколькоИдентификаторовСБП
	|
	|	
	|ИЗ
	|	Документ.ЗаявкаНаРасходованиеДенежныхСредств КАК Заявка
...
Функция СообщитьОбОшибкахФормированияДанныхЗаполненияОплаты(ВыборкаРеквизитыШапки)
	...
	Если ВыборкаРеквизитыШапки.НесколькоИдентификаторовСБП Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрЗаменить(ТекстСообщения, "%ПредставлениеПоля%", НСтр("ru='Идентификатор операции СБП'")),,,, Отказ);
	КонецЕсли;
...
// 
// Параметры:
//  Документ - ДокументСсылка - Документ, в котором используется СБП.
//  ТипРасчетов - ПеречислениеСсылка.ТипыРасчетовСПартнерами - Тип расчетов с партнером.
//  ВыбранныеРеквизиты - Соответствие из КлючИЗначение - реквизиты, которые необходимо получить.
//  ПолучитьСуммыВзаиморасчетов - Булево - признак, получения суммы взаиморасчетов по документу.
// 
// Возвращаемое значение:
//  см. ДенежныеСредстваСервер.ДанныеДокумента
//
Функция ДанныеДокумента(Документ, ТипРасчетов, ВыбранныеРеквизиты, ПолучитьСуммыВзаиморасчетов = Ложь) Экспорт

	Возврат ДенежныеСредстваСервер.ДанныеДокумента(Документ, ТипРасчетов, ВыбранныеРеквизиты, ПолучитьСуммыВзаиморасчетов);
