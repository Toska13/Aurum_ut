	...
	Если ЗначениеЗаполнено(ФизическоеЛицо) Тогда
		...
		Иначе
			...
			ЗаполнитьЗначенияСвойств(
				ФИОФизическогоЛица,
				ФизическиеЛицаКлиентСервер.ЧастиИмени(СокрЛП(ФизическоеЛицо), "Фамилия,Имя"));
...
Функция ФамилияИнициалыФизЛица(ФизическоеЛицо, Дата = Неопределено) Экспорт
	...
	Если ФамилияИнициалы = "" Тогда
		ФамилияИнициалы = СокрЛП(ФизическоеЛицо);
	КонецЕсли;
...
// Создает временную таблицу ВТФИОФизическихЛиц. Состав таблицы:
//  ФизическоеЛицо     - СправочникСсылка.ФизическиеЛица.
//  Период             - Дата - дата, на которую получено ФИО физического лица.
//  Фамилия            - Строка.
//  Имя                - Строка.
//  Отчество           - Строка.
//  ФамилияИмяОтчество - Строка.
//  ФамилияИнициалы    - Строка.
//
// Параметры:
//  МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - менеджер, содержащий входящую таблицу-фильтр "ВТПериодыФизическихЛиц":
//   *ФизическоеЛицо - СправочникСсылка.ФизическиеЛица - физическое лицо, для которого требуется получение ФИО.
//   *Период - Дата  - дата, на которую требуется получить ФИО.
//  ИмяВходящейТаблицы  - Строка - имя входящей таблицы-фильтра, которая содержится в менеджере временных таблиц,
//                                 значение по умолчанию - "ВТПериодыФизическихЛиц".
//  ИмяИсходящейТаблицы - Строка - имя результирующей таблицы, которая будет содержать ФИО физических лиц,
//                                 значение по умолчанию - "ВТФИОФизическихЛиц".
//
Процедура СоздатьВТФИОФизическихЛиц(МенеджерВременныхТаблиц, ИмяВходящейТаблицы = "ВТПериодыФизическихЛиц", ИмяИсходящейТаблицы = "ВТФИОФизическихЛиц") Экспорт
	...
	|	МАКСИМУМ(ФИОФизическихЛиц.Период) КАК ДатаСреза
	|ПОМЕСТИТЬ ВТФильтрСрезаПоследнихФИО
	|ИЗ
	...
	|		ПО ПериодыФизическихЛиц.ФизическоеЛицо = ФИОФизическихЛиц.ФизическоеЛицо
	|			И ПериодыФизическихЛиц.Период >= ФИОФизическихЛиц.Период
	|ГДЕ
	|	ФИОФизическихЛиц.ФизическоеЛицо ССЫЛКА Справочник.ФизическиеЛица
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ФильтрСрезаПоследнихФИО.ФизическоеЛицо КАК ФизическоеЛицо,
	|	ФильтрСрезаПоследнихФИО.Период КАК Период,
	|	ФИОФизическихЛиц.Фамилия КАК Фамилия,
	|	ФИОФизическихЛиц.Имя КАК Имя,
	|	ФИОФизическихЛиц.Отчество КАК Отчество,
	...
	|	КОНЕЦ + ВЫБОР
	|		КОГДА ФИОФизическихЛиц.Фамилия = """"
	|			ТОГДА """"
	...
	|ИЗ
	|	ВТФильтрСрезаПоследнихФИО КАК ФильтрСрезаПоследнихФИО
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизическихЛиц КАК ФИОФизическихЛиц
	|		ПО ФильтрСрезаПоследнихФИО.ФизическоеЛицо = ФИОФизическихЛиц.ФизическоеЛицо
	|			И ФильтрСрезаПоследнихФИО.ДатаСреза = ФИОФизическихЛиц.Период
	|ГДЕ
	|	ФИОФизическихЛиц.ФизическоеЛицо ССЫЛКА Справочник.ФизическиеЛица
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТФильтрСрезаПоследнихФИО";
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТПериодыФизическихЛиц", ИмяВходящейТаблицы);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТФИОФизическихЛиц", ИмяИсходящейТаблицы);
