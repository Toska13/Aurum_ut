...
Процедура ЗаполнитьИсправление(ДокументОбъект, ИсправляемыйДокумент) Экспорт
	...
	Если Не РегистрацияСторноРазрешена(ИсправляемыйДокумент) Тогда
		ВызватьИсключение НСтр("ru = 'Регистрация исправления недоступна'");
	КонецЕсли;
...
Функция РегистрацияСторноРазрешена(СторнируемыйДокумент) Экспорт
	...
	ПроверитьЗавершениеОбновления(СторнируемыйДокумент, Отказ);
...
Процедура ПроверитьЗаполнениеИсправительногоДокумента(ДокументОбъект, Отказ)
	...
	ЭтоСторно = ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.Сторно");
	Если ЭтоСторно Тогда
		ЭтоПервоеИсправление = Ложь;
	Иначе
	...
	|////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(Взаиморасчеты.Период) КАК Период
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|	ГДЕ
	|		РасчетыСКлиентами.Регистратор = &СторнируемыйДокумент
	...
	|	ГДЕ
	|		РасчетыСПоставщиками.Регистратор = &СторнируемыйДокумент
	|		И РасчетыСПоставщиками.Сумма <> 0 
	|	) КАК Взаиморасчеты
	|";
	
	Запрос.УстановитьПараметр("ТипСсылки", ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ДокументОбъект.СторнируемыйДокумент.Метаданные().ПолноеИмя()));
	...
	Иначе
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДокументОбъект, "Организация")
			И РеквизитыСторнируемогоДокумента.Организация <> ДокументОбъект.Организация Тогда
			ТекстСообщения = НСтр("ru = 'При исправлении организация не может быть изменена, следует выполнить сторнирование и регистрацию нового документа.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ДокументОбъект, "Организация", , Отказ);
		КонецЕсли;
	КонецЕсли;
	...
	Если ДругиеИсправления.Следующий() Тогда
		Если ТипЗнч(ДругиеИсправления.Ссылка) = Тип("ДокументСсылка.Сторно") Тогда
			Если ЭтоСторно Тогда
				ТекстСообщения =СтрШаблон(НСтр("ru = 'На основании ""%1"" уже введен документ ""Сторно"".'"),
		...
		Иначе
			Если ЭтоСторно Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'На основании ""%1"" введен исправительный документ.
				                       |Ввод сторно возможен только на основании последнего исправительного документа.'"),
				                       ДокументОбъект.СторнируемыйДокумент);
			Иначе
	...
	Если Не Отказ И Не РеквизитыСторнируемогоДокумента.Проведен Тогда
		Если ЭтоСторно Тогда
			ТекстСообщения = НСтр("ru = 'Сторнируемый документ не проведен.'");
		ИначеЕсли ЭтоПервоеИсправление Тогда
	...
	Если Не Отказ И РеквизитыСторнируемогоДокумента.Дата >= ДатаВремяИсправленияДляПроверки Тогда
		Если ЭтоСторно Тогда
			ТекстСообщения = НСтр("ru = 'Дата и время сторно должны быть больше, чем у сторнируемого документа'");
		ИначеЕсли ЭтоПервоеИсправление Тогда
			ТекстСообщения = НСтр("ru = 'Дата и время исправления должны быть больше, чем у исправляемого документа'");
		Иначе
			ТекстСообщения = НСтр("ru = 'Дата и время исправления должны быть больше, чем у предыдущего исправления документа'");
		КонецЕсли;
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ДокументОбъект, "Дата", , Отказ);
	КонецЕсли;
	
	ДвиженияВзаиморасчетов = РезультатыЗапроса[2].Выбрать();
	Если ДвиженияВзаиморасчетов.Следующий() И ЗначениеЗаполнено(ДвиженияВзаиморасчетов.Период) Тогда
		Если Не Отказ И ДвиженияВзаиморасчетов.Период >= ДатаВремяИсправленияДляПроверки Тогда
			Если ЭтоСторно Тогда
				ТекстСообщения = НСтр("ru = 'Дата и время сторно должны быть больше, чем период движения взаиморасчетов сторнируемого документа.'");
			ИначеЕсли ЭтоПервоеИсправление Тогда
				ТекстСообщения = НСтр("ru = 'Дата и время исправления должны быть больше, чем период движения взаиморасчетов исправляемого документа.'");
			Иначе
				ТекстСообщения = НСтр("ru = 'Дата и время исправления должны быть больше, чем период движения взаиморасчетов предыдущего исправления документа.'");
			КонецЕсли;
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ДокументОбъект, "Дата", , Отказ);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриЧтенииСозданииНаСервере(Форма, ЭлементИсправление)
	...
	ВстроеноИсправление = 
		ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "Исправление")
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "ИсправляемыйДокумент");
	...
	ИначеЕсли ВстроеноИсправление И Объект.Исправление Тогда
		...
		// В исправлении блокируем изменение организации.
		// При необходимости изменить организацию, следует выполнить сторнирование и регистрацию нового документа.
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма.Элементы, "Организация") Тогда
...
Функция ПоследнийИсправительныйДокумент(ИсправляемыйДокумент) Экспорт
	...
	|ГДЕ
	|	РеестрДокументов.ТипСсылки = &ТипСсылки
	|	И РеестрДокументов.СторнируемыйДокумент = &ИсправляемыйДокумент
	|	И РеестрДокументовПоследующиеИсправления.Ссылка ЕСТЬ NULL
	|	И НЕ РеестрДокументов.ДополнительнаяЗапись
	|	И РеестрДокументов.Проведен
	|";
...
Процедура ПроверитьЗавершениеОбновления(СторнируемыйДокумент, Отказ)
	
	Если ОбновлениеИнформационнойБазы.ОтложенноеОбновлениеЗавершено() Тогда
		Возврат;
	КонецЕсли;
	
	РезультатПроверки = ОбновлениеИнформационнойБазы.ОбъектОбработан(СторнируемыйДокумент);
	
	Если Не РезультатПроверки.Обработан Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Обновление документа еще не завершено.'"),,,,Отказ);
		Возврат;
	КонецЕсли;
	
	ТипДокумента = ТипЗнч(СторнируемыйДокумент);
	ДокументМетаданные = Метаданные.НайтиПоТипу(ТипДокумента);
	
	Для Каждого РегистрМетаданные Из ДокументМетаданные.Движения Цикл
		
		НаборЗаписей = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(РегистрМетаданные.ПолноеИмя()).СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Регистратор.Установить(СторнируемыйДокумент);
		
		РезультатПроверкиДвижения = ОбновлениеИнформационнойБазы.ОбъектОбработан(НаборЗаписей);
		
		Если Не РезультатПроверкиДвижения.Обработан Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Обновление движений документа по регистру ""%1"" еще не завершено.'"),
				РегистрМетаданные.Представление());
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
