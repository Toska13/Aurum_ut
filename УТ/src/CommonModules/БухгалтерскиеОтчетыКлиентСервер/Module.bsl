...
Функция ПолучитьПредставлениеПериода(НачалоПериода = '00010101', КонецПериода = '00010101', ТолькоДаты  = Ложь, ВидПериода = 0) Экспорт
	...
	БухгалтерскиеОтчетыКлиентСерверПереопределяемый.ПереопределитьПредставлениеПериода(ТекстПериод, НачалоПериода, КонецПериода, ВидПериода);
...
Функция ИнициализироватьПараметрыПроверкиАктуальности(Форма) Экспорт
	...
	ЭтапыЗакрытияМесяца.Добавить(ПредопределенноеЗначение("Справочник.ОперацииЗакрытияМесяца.РасчетПартийИСебестоимости"));
	ЭтапыЗакрытияМесяца.Добавить(ПредопределенноеЗначение("Справочник.ОперацииЗакрытияМесяца.РасчетДетализацииПродукции"));
	ЭтапыЗакрытияМесяца.Добавить(ПредопределенноеЗначение("Справочник.ОперацииЗакрытияМесяца.ФормированиеДвиженийПоРасчетамСПартнерамиИПереоценкаРасчетов"));
