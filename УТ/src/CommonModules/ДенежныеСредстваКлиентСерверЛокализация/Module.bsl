...
// Возвращает параметры, необходимые для проверки получателя в документе с входящей платежной ссылкой СБП.
// 
// Возвращаемое значение:
//  Структура - Параметры проверки получателя в документе:
// * ИНН - Строка - ИНН получателя в документе.
// * НомерСчета - Строка - Номер счета получателя в документе.
//
Функция ПараметрыПроверкиПолучателяПлатежнойСсылки() Экспорт
...
// Возвращает признак соответствия получателя входящей платежной ссылки СБП с получателем, указанным в документе.
// 
// Параметры:
//  ПроверяемыеДанные - см. ПараметрыПроверкиПолучателяПлатежнойСсылки
//  ДанныеПлатежнойСсылки -Структура - Данные получателя, указанные в платежной ссылке:
// * ИНН - Строка - ИНН получателя в платежной ссылке.
// * НомерСчета - Строка - Номер счета получателя в платежной ссылке.
// 
// Возвращаемое значение:
//  Булево - Получатель платежной ссылки соответствует документу
Функция ПолучательПлатежнойСсылкиСоответствуетДокументу(ПроверяемыеДанные, ДанныеПлатежнойСсылки) Экспорт

	ЕстьОшибки = (ДанныеПлатежнойСсылки.ИНН <> ПроверяемыеДанные.ИНН
		И Не ПустаяСтрока(ПроверяемыеДанные.ИНН))
		Или (ДанныеПлатежнойСсылки.СчетПолучателя <> ПроверяемыеДанные.НомерСчета
		И Не ПустаяСтрока(ПроверяемыеДанные.НомерСчета));
...
// Формы оплаты заявок на безналичную оплату.
// 
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.ФормыОплаты - Формы оплаты заявок на безналичную оплату
Функция ФормыОплатыЗаявокНаБезналичнуюОплату() Экспорт

	ФормыОплаты = Новый Массив;
	
	//++ Локализация
	ФормыОплаты.Добавить(ПредопределенноеЗначение("Перечисление.ФормыОплаты.Безналичная"));
	ФормыОплаты.Добавить(ПредопределенноеЗначение("Перечисление.ФормыОплаты.СистемаБыстрыхПлатежей"));
