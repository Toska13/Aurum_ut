...
Процедура ЗаполнитьДанныеПлатежныхПоручений(МассивСсылок, ДанныеДляЗаполнения) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ПрямыеРасчеты = Не ЗначениеЗаполнено(Выборка.КонтрагентБанкДляРасчетов);
		
		НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
		ПоляПлатежа["Номер"] = НомерДокумента;
		
		Если Не ЗначениеЗаполнено(ПоляПлатежа.РеквизитыПолучателя_Наименование) Тогда
			...
			ПоляПлатежа.РеквизитыПолучателя_Наименование = ДенежныеСредстваСервер.НаименованиеПлательщикаПолучателяПлатежа(
				ПоляПлатежа.РеквизитыПолучателя_Наименование,
				ПрямыеРасчеты,
				Выборка.КонтрагентРасчСчет,
				Выборка.КонтрагентБанк,
				Выборка.КонтрагентГородБанка);
		...
		Если Не ПоддержкаПлатежейРФ
			Или Выборка.ВидДокумента = Перечисления.ТипыПлатежныхДокументов.ПлатежноеТребование Тогда
		...
		Если Не ПрямыеРасчеты Тогда
			...
			Если ПустаяСтрока(Выборка.КонтрагентКоррСчетБанка) Тогда
				ПоляПлатежа.РеквизитыПолучателя_РасчСчет = Выборка.КонтрагентСчетВБанкеДляРасчетов;
			Иначе
				ПоляПлатежа.РеквизитыПолучателя_РасчСчет = Выборка.КонтрагентКоррСчетБанка;
			КонецЕсли;
			
			ПоляПлатежа.РеквизитыПолучателя_Банк_БИК = Выборка.КонтрагентБИКБанкаДляРасчетов;
			ПоляПлатежа.РеквизитыПолучателя_Банк_Город = Выборка.КонтрагентГородБанкаДляРасчетов;
			ПоляПлатежа.РеквизитыПолучателя_Банк_КоррСчет = Выборка.КонтрагентКоррСчетБанкаДляРасчетов;
			ПоляПлатежа.РеквизитыПолучателя_Банк_Наименование = КлиентБанкСервер.НаименованиеБанкаПолучателяПлатежа(
				Выборка.КонтрагентБанкДляРасчетов, Выборка.КонтрагентБанкДляРасчетовНаименованиеРКЦ);
			
		Иначе
			
			ПоляПлатежа.РеквизитыПолучателя_РасчСчет = Выборка.КонтрагентРасчСчет;
			ПоляПлатежа.РеквизитыПолучателя_Банк_БИК = Выборка.КонтрагентБИКБанка;
			ПоляПлатежа.РеквизитыПолучателя_Банк_Город = Выборка.КонтрагентГородБанка;
			ПоляПлатежа.РеквизитыПолучателя_Банк_КоррСчет = Выборка.КонтрагентКоррСчетБанка;
			ПоляПлатежа.РеквизитыПолучателя_Банк_Наименование = КлиентБанкСервер.НаименованиеБанкаПолучателяПлатежа(
				Выборка.КонтрагентБанк, Выборка.КонтрагентБанкНаименованиеРКЦ);
		...
		Если Выборка.БанковскийСчетКонтрагента.РучноеИзменениеРеквизитовБанка
			И ПрямыеРасчеты Тогда		
			ПараметрыОбработкиОшибок = ЭлектронноеВзаимодействиеКлиентСервер.НовыеПараметрыОбработкиОшибки(
		...
		ИначеЕсли Выборка.БанковскийСчетКонтрагента.РучноеИзменениеРеквизитовБанка
			И Не ПрямыеРасчеты Тогда
			ПараметрыОбработкиОшибок = ЭлектронноеВзаимодействиеКлиентСервер.НовыеПараметрыОбработкиОшибки(
				Выборка.БанковскийСчетКонтрагента, "ГородБанкаДляРасчетов");
			ЭлектронноеВзаимодействие.ДобавитьВРеквизитОбработкуОшибки(
				ДанныеДляЗаполнения[ИндексДокумента], "РеквизитыПолучателя.Банк.Город", ПараметрыОбработкиОшибок);
		ИначеЕсли Не ПрямыеРасчеты Тогда
			ПараметрыОбработкиОшибок = ЭлектронноеВзаимодействиеКлиентСервер.НовыеПараметрыОбработкиОшибки(
				Выборка.БанковскийСчетКонтрагента.БанкДляРасчетов, "Город");
			ЭлектронноеВзаимодействие.ДобавитьВРеквизитОбработкуОшибки(
				ДанныеДляЗаполнения[ИндексДокумента], "РеквизитыПолучателя.Банк.Город", ПараметрыОбработкиОшибок);
		Иначе
			ПараметрыОбработкиОшибок = ЭлектронноеВзаимодействиеКлиентСервер.НовыеПараметрыОбработкиОшибки(
...
Процедура ЗаписатьУзелXML(ЧтениеXML, ЗаписьXML, ДесятичныеСвойства, ИмяЭлемента) Экспорт
...
Функция ИнициализироватьПоляПлатежногоПоручения()
	...
	Поля.Вставить("РеквизитыПолучателя_Банк_Город");
	Поля.Вставить("РеквизитыПолучателя_Банк_КоррСчет");
...
//  ТекстОшибки - Строка - текст сообщения об ошибке, если значение не соответствует фасетам типа. Параметр изменяется в процессе работы
//
	...
	ИначеЕсли СвойствоXDTO.Тип.Имя = "string"
		...
		Для Каждого Фасет Из Фасеты Цикл
			...
			ИначеЕсли Фасет.Вид = ВидФасетаXDTO.МинДлина Тогда
				...
				Если Число(Фасет.Значение) <> 0
					И ПустаяСтрока(ЗначениеПоля)
					И Не СвойствоXDTO.ВозможноПустое Тогда
					ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка установки значения свойства %1
											|Устанавливаемое значение: %2
											|Минимально допустимое количество символов: %3'"),
											СвойствоXDTO,
											ЗначениеПоля,
											Число(Фасет.Значение));
				ИначеЕсли Число(Фасет.Значение) > СтрДлина(ЗначениеПоля) Тогда
			...
			ИначеЕсли Фасет.Вид = ВидФасетаXDTO.Образец Тогда
				
				ЗначениеПодобно = СтрПодобнаПоРегулярномуВыражению(ЗначениеПоля, Фасет.Значение);
				
				Если Не ЗначениеПодобно Тогда
					ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка установки значения свойства %1
											|Устанавливаемое значение: %2
											|Значение не соответствует регулярному выражению: %3'"),
											СвойствоXDTO,
											ЗначениеПоля,
											Фасет.Значение);
				КонецЕсли;
			...
			ИначеЕсли Фасет.Вид = ВидФасетаXDTO.Перечисление Тогда
				ДоступныеЗначения.Добавить(Фасет.Значение);
			КонецЕсли;
		...
		Если ДоступныеЗначения.Количество() > 0 Тогда
			...
			Если Не ПустаяСтрока(ЗначениеПоля)
				И Не СвойствоXDTO.ВозможноПустое
				И ДоступныеЗначения.Найти(ЗначениеПоля) = Неопределено Тогда
				ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка установки значения свойства %1
											|Устанавливаемое значение: %2
	...
	Для Каждого СвойствоXDTO Из ОбъектXDTO.Свойства() Цикл
		...
		Иначе
			...
			Если СвойствоXDTO.Тип.Имя = "decimal"
				...
				ФорматнаяСтрока = СтрШаблон("ЧЦ=%1; ЧДЦ=%2; ЧРД=%3; ЧН=; ЧГ=0",
...
Функция ИзменитьФорматОтображенияДесятичныхСвойствXML(ТекстXML, ДесятичныеСвойства, Кодировка)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		ЗаписатьУзелXML(ЧтениеXML, ЗаписьXML, ДесятичныеСвойства, ИмяЭлемента);
	КонецЦикла;
