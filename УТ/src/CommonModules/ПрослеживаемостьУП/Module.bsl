...
Процедура СоздатьУведомленияВФоне(ПараметрыСоздания, АдресРезультата) Экспорт     
	...
	Для каждого ТекущиеДанные из ДанныеУведомлений Цикл
		...
		Если ИмяТипаУведомления = "УведомлениеОВвозеПрослеживаемыхТоваров" Тогда
			...
			Если ТекущиеДанные.КорректировочноеУведомление Тогда
				ТаблицаУведомленийОВвозе = Документы.УведомлениеОВвозеПрослеживаемыхТоваров.УведомленияОВвозеПрослеживаемыхТоваровПоОснованию(ТекущиеДанные.Основание, Истина, ТекущиеДанные.КодТНВЭД);
				Если ТаблицаУведомленийОВвозе.Количество() Тогда
					ТаблицаУведомленийОВвозе.Сортировать("НомерКорректировки Убыв");
					ИсправляемыйДокумент = ТаблицаУведомленийОВвозе[0].Ссылка;
					НомерКорректировки = ТаблицаУведомленийОВвозе[0].НомерКорректировки + 1;
					ТаблицаДанныхДляУведомлений = Документы.УведомлениеОВвозеПрослеживаемыхТоваров.ДанныеТЧТоварыПоЗаявлению(ТекущиеДанные.Основание, ИсправляемыйДокумент, ТекущиеДанные.КодТНВЭД);
				Иначе
					...
					ТекущиеДанные.КорректировочноеУведомление = Ложь;
				КонецЕсли;
			Иначе
				ТаблицаДанныхДляУведомлений = Документы.УведомлениеОВвозеПрослеживаемыхТоваров.ДанныеТЧТоварыПоОснованию(ТекущиеДанные.Основание, ТекущиеДанные.КодТНВЭД);
			КонецЕсли;
			...
			Для Каждого СтрокаТаблицы Из ТаблицаЕдиницИзмерения Цикл
				...
				Если ТекущиеДанные.КорректировочноеУведомление Тогда
					ДокументОбъект.НомерКорректировки = НомерКорректировки;
					ДокументОбъект.Исправление = Истина;
					ДокументОбъект.ИсправляемыйДокумент = ИсправляемыйДокумент;
					ДокументОбъект.СторнируемыйДокумент = ИсправляемыйДокумент;
				КонецЕсли;
