...
Процедура ОчиститьЛогЗапросов(ПараметрыЛогирования) Экспорт
	...
	ОбщегоНазначенияИС.ПроверитьРасположениеФайлаВоВременномКаталоге(ИмяФайлаЛога);
...
// * ВнутреннийТСПИоТККТ       - Неопределено, СправочникСсылка.ПодключаемоеОборудование - передается ККТ с ТС ПИоТ, если логируется ответ от внутреннего ТС ПИоТа
...
Функция НоваяСтруктураДанныхЗаписи() Экспорт
	...
	ПараметрыЗаписиЛога.Вставить("ВнутреннийТСПИоТККТ",       Неопределено);
...
Процедура Вывести(ДанныеЗаписи, ПараметрыЛогирования) Экспорт
	...
	ВнутреннийТСПИоТККТ       = ДанныеЗаписи.ВнутреннийТСПИоТККТ;
	...
	Если HTTPЗапросОтвет <> Неопределено Тогда
		...
		Для Каждого КлючИЗначение Из HTTPЗапросОтвет.Заголовки Цикл
			Если ЭтоHTTPЗапрос И НРег(КлючИЗначение.Ключ) = НРег("Authorization") Тогда
	...
	ИначеЕсли ДанныеЗаписи.ЛогироватьHTTPЗапрос И Не ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		Если ДанныеЗаписи.HTTPЗапросОтветЗаголовки <> Неопределено Тогда
	...
	ИначеЕсли ЗначениеЗаполнено(ВнутреннийТСПИоТККТ) Тогда
		
		ЗаписьТекста.ЗаписатьСтроку("");
		ЗаписьТекста.ЗаписатьСтроку(СтрШаблон(НСтр("ru = 'Данные ответа 1С:ТС ПИоТ: %1'"), ВнутреннийТСПИоТККТ));
...
Процедура ДополнитьИнформациюОбОкруженииШапка(ДанныеОкружения) Экспорт
	...
	ИдентификаторыКлассификаторов = ОбщегоНазначенияИС.ИдентификаторыКлассификаторов(Истина);
	Для Каждого КлючИЗначение Из ИдентификаторыКлассификаторов Цикл
		
		ВерсияКлассификатора = РаботаСКлассификаторами.ВерсияКлассификатора(КлючИЗначение.Ключ);
		Если ЗначениеЗаполнено(ВерсияКлассификатора) Тогда
			ДанныеОкружения.Добавить(
			СтрШаблон(
				"%1: %2",
				КлючИЗначение.Значение.Наименование,
