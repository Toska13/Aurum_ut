...
//		ДокументОснование - ДокументСсылка.СписаниеОтгруженныхТоваров - ссылка на документ, который нужно распечатать.
//
...
Функция ПолучитьДанныеДляПечатнойФормыТОРГ16(ПараметрыПечати, ДокументОснование) Экспорт
	...
	|	""""                    КАК НомерОснования
	|ИЗ
	|	Документ.СписаниеОтгруженныхТоваров КАК ДокументСписания
	|ГДЕ
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеНедостачЗаСчетКомитента Тогда
		...
		|ИЗ
		|	Документ.СписаниеОтгруженныхТоваров.Товары КАК ТоварыСписания
		|ГДЕ
	...
	ИначеЕсли ИсточникИнформацииОЦенахДляПечати = Перечисления.ИсточникиИнформацииОЦенахДляПечати.ПоСебестоимости Тогда
		...
		Если ПредварительныйРасчет Тогда
			...
			|ИЗ
			|	Документ.СписаниеОтгруженныхТоваров.ВидыЗапасов КАК СписываемыеВидыЗапасов
...
Функция ТаблицаКурсовВалют(МассивДокументов)
	...
	ТаблицаКурсовВалют.Колонки.Добавить("Ссылка",    Новый ОписаниеТипов("ДокументСсылка.СписаниеОтгруженныхТоваров"));
