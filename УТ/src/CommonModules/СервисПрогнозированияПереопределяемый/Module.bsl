...
Процедура ЗаписатьПрогнозПродаж(СтруктураОтвета, ВидПлана, ИмяМодели, ИдОбучения, КачествоМодели) Экспорт
	...
	КоличествоДанныхТекущегоПрогноза = ТаблицаДляФильтрации.Количество();
	Если КоличествоДанныхТекущегоПрогноза = 0 Тогда
		Возврат;
	КонецЕсли;
	
	// 1 шаг фильтрации - применение общего фильтра на загрузку.
	ОбщийФильтрИсключилЧастьДанных = Ложь;
	...
	Если ЗначениеЗаполнено(ЗначениеКонстанты) Тогда
		...
		Если ЗначениеЗаполнено(ФильтрНаЗагрузку)
			...
			// Поиск и удаление группировочной строки с пустыми значениями, которая может быть добавлена после прохождения фильтрации.
			...
			// Проверка исключения фильтром данных прогноза.
			Если ТаблицаДляФильтрации.Количество() = 0 Тогда
				ВызватьИсключение ТекстФильтрыЗагрузкиИсключаютВсеДанныеПрогноза("Общий");
			КонецЕсли;
			ОбщийФильтрИсключилЧастьДанных = КоличествоДанныхТекущегоПрогноза <> ТаблицаДляФильтрации.Количество();
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтруктураНастроек) Тогда
		...
		// Проверка исключения фильтром данных прогноза.
		Если ТаблицаДляФильтрации.Количество() = 0 Тогда
			Если ОбщийФильтрИсключилЧастьДанных Тогда
				ТекстОшибки = ТекстФильтрыЗагрузкиИсключаютВсеДанныеПрогноза("ОбщийИВидаПлана");
			Иначе
				ТекстОшибки = ТекстФильтрыЗагрузкиИсключаютВсеДанныеПрогноза("ВидаПлана");
			КонецЕсли;
...
Функция ПолучитьКоллекциюПродаж(НастройкиСервиса, ДиапазонДатДляВыгрузки,
	...
	Если УчитыватьПотерянныеЗаказы Тогда
		...
		Запрос.УстановитьПараметр("ТипыРегистраторовЗаказов", РегистраторыНевыполненныхЗаказов());
	КонецЕсли;
...
Функция ПолучитьКоллекциюПромоАкций(НастройкиСервиса, ДиапазонДатДляВыгрузки = Неопределено)
	...
	ДобавляемаяДатаДиапазона = ?(ДиапазонДатДляВыгрузки.ДатаНачала > ДатаНачалаПромо,
		ДиапазонДатДляВыгрузки.ДатаНачала,
		ДатаНачалаПромо);
	ДатаОкончанияПромо       = ?(ДиапазонДатДляВыгрузки.ДатаОкончания < ДатыНачалаОкончания.ДатаОкончанияПромо,
		ДиапазонДатДляВыгрузки.ДатаОкончания,
		ДатыНачалаОкончания.ДатаОкончанияПромо);
	...
	Пока ДобавляемаяДатаДиапазона < ДатаОкончанияПромо Цикл
...
Функция ТекстЗапросаКоллекцииПродажВТНевыполненныхЗаказов()
	...
	|	СУММА(ВЫБОР
	|		КОГДА Обороты.Показатель = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Заказано)
	|			ТОГДА Обороты.КоличествоОборот
	|			ИНАЧЕ 0
	|	КОНЕЦ)                              КАК ЗаказаноПриход,
	|	СУММА(ВЫБОР
	|		КОГДА Обороты.Показатель = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Отгружено)
	|			ТОГДА Обороты.КоличествоОборот
	|			ИНАЧЕ 0
	...
	|ПОМЕСТИТЬ Обороты
	|ИЗ
	|	РегистрНакопления.РаспоряженияНаОтгрузкуИВозврат.Обороты(&ДатаНачала, &ДатаОкончания, Запись,
	|		Показатель В (ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Заказано),
	...
	|	И ТИПЗНАЧЕНИЯ(Обороты.Регистратор) В (&ТипыРегистраторовЗаказов)
	|СГРУППИРОВАТЬ ПО
...
Функция РегистраторыНевыполненныхЗаказов()
	
	Регистраторы = Новый Массив;
	Регистраторы.Добавить(Тип("ДокументСсылка.АктВыполненныхРабот"));
	Регистраторы.Добавить(Тип("ДокументСсылка.ЗаказКлиента"));
	Регистраторы.Добавить(Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента"));
	Регистраторы.Добавить(Тип("ДокументСсылка.КорректировкаРегистров"));
	Регистраторы.Добавить(Тип("ДокументСсылка.ОтгрузкаТоваровСХранения"));
	Регистраторы.Добавить(Тип("ДокументСсылка.ОтчетКомитентуОЗакупках"));
	Регистраторы.Добавить(Тип("ДокументСсылка.ПередачаТоваровХранителю"));
	Регистраторы.Добавить(Тип("ДокументСсылка.РеализацияТоваровУслуг"));
	Регистраторы.Добавить(Тип("ДокументСсылка.Сторно"));
	...
	Иначе
		...
		Для Каждого ПолеОтбора Из ОтборыНастроек.Элементы Цикл
			...
			Иначе
				...
				КонецЦикла;
				Если Не ПолеНайдено Или Не ЗначениеЗаполнено(ПолеОтбора.ПравоеЗначение) Тогда
					КУдалению.Добавить(ПолеОтбора);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
Функция ПолучитьРезультатСКД(СхемаКомпоновкиДанных, НастройкиОтборов,
	...
	СоединитьОтборыПоПолюВЕдиныйОтбор(НастройкиСхемы.Отбор.Элементы, Ложь);
...
Процедура СоединитьОтборыПоПолюВЕдиныйОтбор(КоллекцияЭлементовОтбора, ЭтоГруппа)
	
	КоличествоОтборов = КоллекцияЭлементовОтбора.Количество();
	Если КоличествоОтборов < 2
		И ЭтоГруппа Тогда
		Возврат;
	КонецЕсли;
	...
	Номенклатуры       = Новый СписокЗначений;
	Склады             = Новый СписокЗначений;
	Партнеры           = Новый СписокЗначений;
	Характеристики     = Новый СписокЗначений;
	
	ГруппыОтбора = Новый Массив;
	
	Индекс = 0;
	Пока КоличествоОтборов > 0 Цикл
		...
		Если СервисПрогнозированияПереопределяемыйКлиентСервер.ЭтоГруппировкаКомпоновкиДанных(ЭлементОтбора) Тогда
			...
			Если ЭлементОтбора.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ Тогда
				ГруппыОтбора.Добавить(ЭлементОтбора);
			КонецЕсли;
			Индекс = Индекс + 1;
		...
		Иначе
			...
			Если ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно Тогда
				...
				Если ЭлементОтбора.ЛевоеЗначение = ПолеНоменклатура Тогда
					Номенклатуры.Добавить(ЭлементОтбора.ПравоеЗначение);
					КоллекцияЭлементовОтбора.Удалить(ЭлементОтбора);
				ИначеЕсли ЭлементОтбора.ЛевоеЗначение = ПолеСклад Тогда
					Склады.Добавить(ЭлементОтбора.ПравоеЗначение);
					КоллекцияЭлементовОтбора.Удалить(ЭлементОтбора);
				ИначеЕсли ЭлементОтбора.ЛевоеЗначение = ПолеПартнер Тогда
					Партнеры.Добавить(ЭлементОтбора.ПравоеЗначение);
					КоллекцияЭлементовОтбора.Удалить(ЭлементОтбора);
				ИначеЕсли ЭлементОтбора.ЛевоеЗначение = ПолеХарактеристика Тогда
					Характеристики.Добавить(ЭлементОтбора.ПравоеЗначение);
					КоллекцияЭлементовОтбора.Удалить(ЭлементОтбора);
				Иначе
					Индекс = Индекс + 1;
				КонецЕсли;
	...
	Если Номенклатуры.Количество() > 1 Тогда
		ЕдиныйОтбор = КоллекцияЭлементовОтбора.Добавить(ТипОтбора);
		ЕдиныйОтбор.ВидСравнения   = ВидСравненияКомпоновкиДанных.ВСписке;
		ЕдиныйОтбор.ЛевоеЗначение  = ПолеНоменклатура;
		ЕдиныйОтбор.ПравоеЗначение = Номенклатуры;
	ИначеЕсли Номенклатуры.Количество() = 1 Тогда
		ИзначальныйОтбор = КоллекцияЭлементовОтбора.Добавить(ТипОтбора);
		ИзначальныйОтбор.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
		ИзначальныйОтбор.ЛевоеЗначение  = ПолеНоменклатура;
		ИзначальныйОтбор.ПравоеЗначение = Номенклатуры[0].Значение;
	КонецЕсли;
	Если Склады.Количество() > 1 Тогда
		ЕдиныйОтбор = КоллекцияЭлементовОтбора.Добавить(ТипОтбора);
		ЕдиныйОтбор.ВидСравнения   = ВидСравненияКомпоновкиДанных.ВСписке;
		ЕдиныйОтбор.ЛевоеЗначение  = ПолеСклад;
		ЕдиныйОтбор.ПравоеЗначение = Склады;
	ИначеЕсли Склады.Количество() = 1 Тогда
		ИзначальныйОтбор = КоллекцияЭлементовОтбора.Добавить(ТипОтбора);
		ИзначальныйОтбор.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
		ИзначальныйОтбор.ЛевоеЗначение  = ПолеСклад;
		ИзначальныйОтбор.ПравоеЗначение = Склады[0].Значение;
	КонецЕсли;
	Если Партнеры.Количество() > 1 Тогда
		ЕдиныйОтбор = КоллекцияЭлементовОтбора.Добавить(ТипОтбора);
		ЕдиныйОтбор.ВидСравнения   = ВидСравненияКомпоновкиДанных.ВСписке;
		ЕдиныйОтбор.ЛевоеЗначение  = ПолеПартнер;
		ЕдиныйОтбор.ПравоеЗначение = Партнеры;
	ИначеЕсли Партнеры.Количество() = 1 Тогда
		ИзначальныйОтбор = КоллекцияЭлементовОтбора.Добавить(ТипОтбора);
		ИзначальныйОтбор.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
		ИзначальныйОтбор.ЛевоеЗначение  = ПолеПартнер;
		ИзначальныйОтбор.ПравоеЗначение = Партнеры[0].Значение;
	КонецЕсли;
	Если Характеристики.Количество() > 1 Тогда
		ЕдиныйОтбор = КоллекцияЭлементовОтбора.Добавить(ТипОтбора);
		ЕдиныйОтбор.ВидСравнения   = ВидСравненияКомпоновкиДанных.ВСписке;
		ЕдиныйОтбор.ЛевоеЗначение  = ПолеХарактеристика;
		ЕдиныйОтбор.ПравоеЗначение = Характеристики;
	ИначеЕсли Характеристики.Количество() = 1 Тогда
		ИзначальныйОтбор = КоллекцияЭлементовОтбора.Добавить(ТипОтбора);
		ИзначальныйОтбор.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
		ИзначальныйОтбор.ЛевоеЗначение  = ПолеХарактеристика;
		ИзначальныйОтбор.ПравоеЗначение = Характеристики[0].Значение;
	КонецЕсли;
	
	Для Каждого Группа Из ГруппыОтбора Цикл
		СоединитьОтборыПоПолюВЕдиныйОтбор(Группа.Элементы, Истина);
	КонецЦикла;
...
Функция ТекстФильтрыЗагрузкиИсключаютВсеДанныеПрогноза(ВидФильтра)
	
	Если ВидФильтра = "Общий" Тогда
		ТекстОшибки = НСтр("ru = 'Общий фильтр на загрузку исключил все данные прогноза.
			|Скорректируйте фильтр и попробуйте еще раз получить прогноз.
			|Настройка фильтра происходит в разделе ""4. Фильтры"" помощника подключения.'");
	ИначеЕсли ВидФильтра = "ВидаПлана" Тогда
		ТекстОшибки = НСтр("ru = 'Фильтр на загрузку у вида плана исключил все данные прогноза.
			|Скорректируйте фильтр и попробуйте еще раз получить прогноз.
			|Настройка фильтра происходит в разделе ""Настройки заполнения"" вида плана.'");
	Иначе // Комбинация фильтров.
		ТекстОшибки = НСтр("ru = 'Фильтры на загрузку (общий и у вида плана) исключили все данные прогноза.
			|Скорректируйте фильтры и попробуйте еще раз получить прогноз.
			|Настройка фильтров происходит:
			|1) Общий в разделе ""4. Фильтры"" помощника подключения.
			|2) Вида плана в разделе ""Настройки заполнения"".'");
	КонецЕсли;
