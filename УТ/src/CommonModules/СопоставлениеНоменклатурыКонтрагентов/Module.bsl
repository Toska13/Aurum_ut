...
Процедура ЗаполнитьОбъектНоменклатурыКонтрагента(ОбъектНоменклатураКонтрагента, ДанныеНоменклатурыКонтрагента, Знач НоменклатураИБ = Неопределено,
	...
	Если ПустаяСтрока(ОбъектНоменклатураКонтрагента.Наименование) Тогда
	ДанныеОбъекта.Наименование = ДанныеОбъекта.НаименованиеПолное;
	Иначе
		ДанныеОбъекта.Наименование = ОбъектНоменклатураКонтрагента.Наименование;
	КонецЕсли;
...
Процедура УдалитьСоответствиеНоменклатуры(Знач НоменклатураКонтрагента) Экспорт
	...
	ДополнительныеПараметры.ДанныеРегистраНоменклатурыКонтрагентов = Ложь;
	Отказ = Ложь;
	СоздатьОбновитьНоменклатуруКонтрагента(ДанныеНоменклатурыКонтрагента,
...
Процедура СоздатьВременнуюТаблицуСоответствияНоменклатуры(Знач ТаблицаОтбора, Знач ИмяВременнойТаблицы, МенеджерВременныхТаблиц) Экспорт
	...
	|	НоменклатураКонтрагентовБЭД.НаименованиеНоменклатуры КАК Наименование,
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВключитьИспользованиеФОСловаряСопоставленияНоменклатурыБЭД", Истина);
	Методы.Вставить("ОбновлениеСловаряСопоставленияНоменклатурыБЭД", Истина);
...
Функция ТекстЗапросаКРегистру()
	...
	|	Сопоставление.НаименованиеНоменклатуры КАК Наименование,
...
Процедура ОбновитьДополнитьДанныеСопоставления(Знач Соответствие, Знач ДанныеДокумента, Знач ИспользоватьХарактеристики)
	...
	Для Каждого Элемент Из ДанныеДокумента Цикл
		Если ЗначениеЗаполнено(Элемент.Значение) // перезаполнение только для непустых значений
			И ДанныеБазы[Элемент.Ключ] <> Элемент.Значение Тогда
