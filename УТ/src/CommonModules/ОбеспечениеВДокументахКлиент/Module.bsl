...
//  * КонтекстИспользования - Строка - контекст использования.
//  * Заказ - Неопределено - ссылка на заказ, если контекст "ЗАКАЗ" или "ТАБЛИЦА".
//  * Заказы - Неопределено - массив заказов, передается, если контекст "СПИСОКЗАКАЗОВ".
//  * АдресТаблицыДвижений - Неопределено - адрес в хранилище, если конекст "ТАБЛИЦА".
//  * ТолькоПросмотр - Булево - если ложь, то не будут доступны команды изменения объекта.
//  * ДоступныеОстаткиПараметрыДокумента - Неопределено
//                                       - см. ОбеспечениеВДокументахСервер.ДоступныеОстаткиПараметрыВстраивания.
//  * ДопустимыеВариантыОбеспечения - Неопределено - массив вариантов обеспечения, влияет на доступность команд установки действий
//  * ЗаказыРегистраторыДвижений - Неопределено - массив используется в контексте "ТАБЛИЦА", если переданный параметр Заказ не является регистратором движений.
//  * ЭтапДинамическойСтруктурыЗаказа - Булево - признак вызова формы из этапа производства динамической структуры заказа в контексте "ТАБЛИЦА".
//
Функция ПараметрыФормыСостояниеОбеспеченияЗаказов(КонтекстИспользования) Экспорт
	...
	ПараметрыФормы.Вставить("Заказы", Неопределено);
	ПараметрыФормы.Вставить("АдресТаблицыДвижений", Неопределено);
	ПараметрыФормы.Вставить("ТолькоПросмотр", Ложь);
	ПараметрыФормы.Вставить("ДоступныеОстаткиПараметрыДокумента", Неопределено);
	ПараметрыФормы.Вставить("ДопустимыеВариантыОбеспечения", Неопределено);
	ПараметрыФормы.Вставить("ЗаказыРегистраторыДвижений", Неопределено);
	ПараметрыФормы.Вставить("ЭтапДинамическойСтруктурыЗаказа", Ложь);
	Возврат ПараметрыФормы;
	
КонецФункции

// Используется для ограничения команд установки действий в обработке "Состояние обеспечения".
// Возвращаемое значение:
//  Соответствие Из Булево - Истина, если вариант обеспечения допустим для устанвоки в документе.
Функция ВсеДопустимыеВариантыОбеспечения() Экспорт
	
	ВсеВарианты = Новый Соответствие();
	ВсеВарианты.Вставить(ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.Отгрузить"), Истина);
	ВсеВарианты.Вставить(ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.СоСклада"), Истина);
	ВсеВарианты.Вставить(ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.РезервироватьПоМереПоступления"), Истина);
	ВсеВарианты.Вставить(ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.КОбеспечению"), Истина);
	ВсеВарианты.Вставить(ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.НеТребуется"), Истина);
	Возврат ВсеВарианты;
...
Процедура ЗаполнитьДатыНаВторомШагеОбеспечениеПотребностей(СтрокаТаблицы) Экспорт
	
	СтрокаТаблицы.ДатаПоставки = '00010101';
	СтрокаТаблицы.ГраницаПериодаОбеспечения = '00010101';
	Если СтрокаТаблицы.КЗаказуТовары = 1
			И СтрокаТаблицы.ФормироватьЗаказыПоРасписанию Тогда
		СтрокаТаблицы.ДатаПоставки = СтрокаТаблицы.ДатаПоставкиПоГрафику;
		СтрокаТаблицы.ГраницаПериодаОбеспечения = СтрокаТаблицы.ГраницаПериодаОбеспеченияПоГрафику;
	ИначеЕсли СтрокаТаблицы.КЗаказуТовары = 2 Тогда
		СтрокаТаблицы.ДатаПоставки = СтрокаТаблицы.ДатаПоставкиПоГрафику;
		СтрокаТаблицы.ГраницаПериодаОбеспечения = СтрокаТаблицы.ГраницаПериодаОбеспеченияПоГрафику;
	ИначеЕсли СтрокаТаблицы.КЗаказуТовары = 0 Тогда
		СтрокаТаблицы.КЗаказуТовары = 0;
		СтрокаТаблицы.ДатаПоставки = СтрокаТаблицы.ДатаПоставкиПоТочкеЗаказа;
		// До наступления даты заказа обеспечивать потребности до даты ближайшей поставки по графику.
		СтрокаТаблицы.ГраницаПериодаОбеспечения = СтрокаТаблицы.ГраницаПериодаОбеспеченияПоТочкеЗаказа;
	КонецЕсли;
...
Процедура ВвестиКоличествоДляРазбиенияПослеВвода(Количество, ПараметрыОбработки) Экспорт
	...
	ИначеЕсли Количество > ДоступноеКоличество + Количество
		...
		Если ВариантОбеспеченияВыбранноеЗначение.ВариантОбеспечения
				= ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.Отгрузить") Тогда
			ТекстСообщения = НСтр("ru = 'Количество в новой строке не может быть больше доступного для отгрузки количества в текущей строке.'");
		Иначе
			ТекстСообщения = НСтр("ru = 'Количество в новой строке не может быть больше доступного для резервирования количества в текущей строке.'");
		КонецЕсли;
	КонецЕсли;
...
Функция ПараметрыФормыРезервы() Экспорт
	...
	Параметры.Вставить("КонтролируемаяПартия");
