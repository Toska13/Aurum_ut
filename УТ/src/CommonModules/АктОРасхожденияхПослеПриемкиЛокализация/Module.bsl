...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если Объект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ЗакупкаПоИмпорту
		...
		И ХозОперацииЗакупкаВСтранахЕАЭС.Найти(Объект.ХозяйственнаяОперация) = Неопределено Тогда
...
Процедура СоздатьПоРезультатамПроверкиИПодбораИСМП(ПараметрыОкончанияСканирования, Проводить) Экспорт
	...
	Иначе
		...
		ПерезаполнитьТоварыПоВидуПродукции(ДокументОбъект, ПараметрыОкончанияСканирования.ПроверяемыйДокумент);
	КонецЕсли;
	...
	СтруктураДействийНоваяСтрока.Вставить("ЗаполнитьСтавкуНДС", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(ДокументОбъект));
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСоглашенияСПоставщиками") И ЗначениеЗаполнено(ДокументОбъект.Соглашение) Тогда
		СтруктураДействийНоваяСтрока.Вставить("ЗаполнитьУсловияЗакупок", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияЦеныЗакупкиВСтрокеТЧ(ДокументОбъект));
	Иначе
		СтруктураДействийНоваяСтрока.Вставить("ЗаполнитьЦенуЗакупки", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияЦеныЗакупкиВСтрокеТЧ(ДокументОбъект));
	КонецЕсли;
...
Функция ТекстЗапросаЗаполнения(ДокументОснование)
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
	    ...
	    |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		|		ПО ТаблицаНоменклатура.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
		|			И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
	    |СГРУППИРОВАТЬ ПО
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ВозвратТоваровОтКлиента") Тогда
	    ...
	    |	Товары.СтатусУказанияСерий КАК СтатусУказанияСерий,
	    |	Товары.Ссылка КАК ДокументОснование,
	    |	Товары.Ссылка.ЗаявкаНаВозвратТоваровОтКлиента КАК ЗаказПоставщику,
	    |	Товары.КодСтроки КАК КодСтроки,
	    |	ИСТИНА КАК ЗаполненоПоОснованию,
	    |	Товары.КоличествоУпаковок КАК КоличествоУпаковокПоДокументу,
	    |	Товары.Количество КАК КоличествоПоДокументу,
	    ...
	    |	Товары.НомерГТД КАК НомерГТД,
	    |	Товары.Ссылка.НомерДокументаПокупателя КАК НомерПаспорта
	    |ИЗ
	    ...
	    |СГРУППИРОВАТЬ ПО
	    |	ТаблицаНоменклатура.Номенклатура";
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПоступлениеТоваровОтХранителя") Тогда
		Возврат "
		|ВЫБРАТЬ
	    |	ТаблицаНоменклатура.Номенклатура КАК Номенклатура
	    |ПОМЕСТИТЬ ТаблицаНоменклатура
	    ...
	    |	Товары.СтатусУказанияСерий КАК СтатусУказанияСерий,
	    |	Товары.Ссылка КАК ДокументОснование,
	    |	Товары.Распоряжение КАК ЗаказПоставщику,
	    |	Товары.КодСтроки КАК КодСтроки,
	    |	ИСТИНА КАК ЗаполненоПоОснованию,
	    |	Товары.КоличествоУпаковок КАК КоличествоУпаковокПоДокументу,
	    |	Товары.Количество КАК КоличествоПоДокументу,
	    ...
	    |	Серии.Характеристика КАК Характеристика,
	    |	Серии.Назначение КАК Назначение,
	    |	Серии.Ссылка.Склад КАК Склад,
	    |	Серии.Ссылка КАК ДокументОснование,
	    |	ИСТИНА КАК ЗаполненоПоОснованию
	    |ИЗ
	    |	Документ.ПоступлениеТоваровОтХранителя.Серии КАК Серии
	    |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		|		ПО Серии.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
		|			И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
	    |ГДЕ
	    ...
	    |СГРУППИРОВАТЬ ПО
	    |	ТаблицаНоменклатура.Номенклатура";
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПриемкаТоваровНаХранение") Тогда
		Возврат "
		|ВЫБРАТЬ
	    |	ТаблицаНоменклатура.Номенклатура КАК Номенклатура
	    |ПОМЕСТИТЬ ТаблицаНоменклатура
	    ...
	    |	ИСТИНА КАК ЗаполненоПоОснованию
	    |ИЗ
	    |	Документ.ПриемкаТоваровНаХранение.Серии КАК Серии
	    |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		|		ПО Серии.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
		|			И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
	    |ГДЕ
...
Процедура ПерезаполнитьТоварыПоВидуПродукции(ДокументОбъект, ДокументОснование)
	
	ТекстЗапроса = ТекстЗапросаЗаполнения(ДокументОснование);
	...
	ТаблицаНоменклатура  = Результат[3].Выбрать();
...
Процедура ЗаполнитьТаблицыШтрихкодов(ДокументОбъект, ПараметрыОкончанияСканирования, 
	...
	ПроверкаЗаверешенаСРасхождениями = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ПараметрыОкончанияСканирования, "ШтрихкодыУпаковокПланЭДО", Неопределено) <> Неопределено;
	...
	Если ПроверкаЗаверешенаСРасхождениями Тогда
		...
		Запрос.Текст = СтрШаблон("%1
		                         |%2",ТекстЗапроса, ТекстЗапросаРасхождения);
	Иначе
	...
	Результат = Запрос.ВыполнитьПакет();
	ВыборкаШтрихкодыРасхожденияКУдалению   = Результат[1].Выбрать();
	
	ПараметрыПоискаПродукция = Новый Структура("Номенклатура, Характеристика, ЗначениеШтрихкода");
	
	Если ПроверкаЗаверешенаСРасхождениями Тогда
		
		ВыборкаШтрихкодыФактКУдалению          = Результат[3].Выбрать();
		ВыборкаШтрихкодыРасхожденияКДобавлению = Результат[7].Выбрать();
	...
	Иначе
		...
		Пока ВыборкаШтрихкодыРасхожденияКУдалению.Следующий() Цикл
			...
			СтрокиФактКУдалению = Новый Массив;
			СтрокиКУдалению = ДокументОбъект.ШтрихкодыУпаковокРасхождения.НайтиСтроки(ПараметрыПоискаПродукция);
			
			Для Каждого СтрокаУдалить Из СтрокиКУдалению Цикл
				
				Если СтрокаУдалить.ТипРасхождения = Перечисления.ТипыРасхожденийИСМП.Недостача Тогда
					СтрокаФакт = ДокументОбъект.ШтрихкодыУпаковокФактЭДО.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаФакт, СтрокаУдалить);
				Иначе // Излишек
					СтрокиФактКУдалению = ДокументОбъект.ШтрихкодыУпаковокФактЭДО.НайтиСтроки(ПараметрыПоискаПродукция);
					Для Каждого СтрокаФакт Из СтрокиФактКУдалению Цикл
						 ДокументОбъект.ШтрихкодыУпаковокФактЭДО.Удалить(СтрокаФакт);
					КонецЦикла;
				КонецЕсли;
