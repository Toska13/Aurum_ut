...
Функция ПрочитатьКорневойЭлементXML(СтрокаXML) Экспорт
	...
	Возврат ОбщегоНазначенияБПОКлиентСервер.ПрочитатьКорневойЭлементXML(СтрокаXML);
...
Функция ПрочитатьПараметрыИзXML(СтрокаXML) Экспорт
	...
	Возврат ОбщегоНазначенияБПОКлиентСервер.ПрочитатьПараметрыИзXML(СтрокаXML);
...
// Функция возвращает Истина, если внедрена ГосИС
//
// Возвращаемое значение:
//  Булево.
Функция ИспользуетсяГосИС() Экспорт
	
	Возврат ОбщегоНазначенияБПО.ПодсистемаСуществует("ГосИС.ИСМП");
...
// Возвращает настройки прокси на клиенте.
//             
// Возвращаемое значение:
//  Структура
//
Функция НастройкиПроксиНаКлиенте() Экспорт
	...
	ПараметрыПрокси.Вставить("Использовать", Ложь);
	ПараметрыПрокси.Вставить("Сервер");
	ПараметрыПрокси.Вставить("Порт");
	ПараметрыПрокси.Вставить("Пользователь");
	ПараметрыПрокси.Вставить("Пароль");
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяБСП() Тогда
		...
		Если ТипЗнч(НастройкаПроксиНаКлиенте) = Тип("Соответствие") Тогда
			...
			Если ИспользоватьПрокси И НЕ ИспользоватьСистемныеНастройки Тогда    
				ПараметрыПрокси.Использовать = Истина;
				ПараметрыПрокси.Сервер       = НастройкаПроксиНаКлиенте.Получить("Сервер");
				ПараметрыПрокси.Порт         = НастройкаПроксиНаКлиенте.Получить("Порт");
				ПараметрыПрокси.Пользователь = НастройкаПроксиНаКлиенте.Получить("Пользователь");
				ПараметрыПрокси.Пароль       = НастройкаПроксиНаКлиенте.Получить("Пароль");
			КонецЕсли;
		КонецЕсли;    
...
// Возвращает Истина, если используется подсистемы "ТС ПИоТ" и эти подсистемы существует в конфигурации.
// Предназначена для реализации вызова необязательной подсистемы (условного вызова).
//
// Возвращаемое значение:
//  Булево.
//
Функция ИспользуетсяТСПИоТ() Экспорт
	
	Возврат ПодсистемаСуществует("ПоддержкаОборудования.ПодключаемоеОборудование.ТСПИоТ");
...
Функция ФорматироватьJSON(ТекстСообщенияJSON, УдалитьПробелыИПереносыСтрок = Ложь, КлючиРазворачиванияBASE64 = Неопределено) Экспорт
	...
	Если ПустаяСтрока(ТекстСообщенияJSON) Тогда
		Возврат "";
	КонецЕсли;
	...
	Если КлючиРазворачиванияBASE64 <> Неопределено
		...
		Для Каждого КлючИЗначение Из КлючиРазворачиванияBASE64 Цикл
			...
			Если ДанныеПоКлючу = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Данные[КлючИЗначение.Ключ] = ТекстJSONВОбъект( МенеджерОборудованияКлиентСервер.Base64ВШтрихкод(ДанныеПоКлючу), Истина);
		КонецЦикла;
	КонецЕсли;
...
//  Строка
//
Функция УбратьНедопустимыеСимволыXML(Знач Текст) Экспорт
	
	Если ИспользуетсяБСП() Тогда
		// Вызов БСП
		ОбщегоНазначенияКлиентСерверМодуль = ОбщийМодуль("ОбщегоНазначенияКлиентСервер");
		Возврат ОбщегоНазначенияКлиентСерверМодуль.УдалитьНедопустимыеСимволыXML(Текст);
		// Конец Вызов БСП
	Иначе
		Возврат ОбработкаСтрокиXML.УдалитьНедопустимыеСимволыXML(Текст);
	КонецЕсли;
