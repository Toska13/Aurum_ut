...
Процедура ПодготовитьСообщенияКПередаче(ДинамическийСписок, ПараметрыПередачи) Экспорт
	...
	ПараметрыОпределенияДоступности.Подсистема = ИнтеграцияЗЕРНОКлиентСервер.ИмяПодсистемы();
...
Функция ПараметрыФормыПодбораПартий() Экспорт
	...
	// Параметр отбора
	Результат.Вставить("РазделУчета", ПредопределенноеЗначение("Перечисление.РазделыУчетаЗЕРНО.ПустаяСсылка"));
...
Функция ИнициализироватьПараметрыОткрытияФормыПодбора() Экспорт
	...
	ПараметрыОткрытияФормы.Вставить("РазделУчета");
...
Процедура ОткрытьФормуПодбораОКПД2(Форма, ТекущиеДанные, Владелец, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма.Объект, "РазделУчета") Тогда
		ПараметрыФормы.РазделУчета = Форма.Объект.РазделУчета;
	КонецЕсли;
...
Функция ПараметрыИнтерактивногоВыбораОтбораЗаполнения(ПолноеИмяДокумента, ДокументОснование, КорректировочныйДокумент = Ложь)
	...
	ВозвращаемоеЗначение.ОбъектыДляВыбора = ИнтеграцияЗЕРНОВызовСервера.РазделыУчетаДанныхЗаполнения(
