...
// Запрос КМ из XML пакета.
// 
// Параметры:
//  ДанныеXML - Строка - XML.
// 
// Возвращаемое значение:
//  Структура - см. ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПараметрыЗапросКМ()
//
Функция ЗапросКМИзXMLПакета(ДанныеXML) Экспорт
	
	Возврат МенеджерОборудованияМаркировкаСлужебныйКлиентСервер.ЗапросКМИзXMLПакета(ДанныеXML);
...
// Сформировать данные для отчета выбытии МТ для ЛМ ЧЗ.
// 
// Параметры:
...
Функция СформироватьДанныеОтчетаВыбытииМТ(ОбщиеПараметры, ПараметрыФискализации) Экспорт
	...
	Для Каждого Позиция Из ОбщиеПараметры.ПозицииЧека Цикл
		Если Позиция.Свойство("ФискальнаяСтрока") 
			И Не ПустаяСтрока(Позиция.КонтрольнаяМарка) И Позиция.Количество = 1 Тогда   
			
			ДробноеКоличество = Позиция.ДробноеКоличество;
			Знаменатель = ?(ДробноеКоличество.Знаменатель = Неопределено, 0, ДробноеКоличество.Знаменатель);  
			Числитель   = ?(ДробноеКоличество.Числитель = Неопределено, 0, ДробноеКоличество.Числитель);
			// Если дробное количество - пропускаем.
			Если Знаменатель > 0 И Числитель > 0 Тогда 
				Продолжить;
			КонецЕсли;               
			
			// Если частичное выбытие мерного товара - пропускаем.
			МераКоличества = МенеджерОборудованияМаркировкаСлужебныйКлиентСервер.МераКоличестваПредметаРасчетаПоКодуЕдиницыИзмерения(Позиция.КодЕдиницыИзмерения); 
			Если МераКоличества = Перечисления.МераКоличестваПредметаРасчетаККТ.Килограмм Или     
				МераКоличества = Перечисления.МераКоличестваПредметаРасчетаККТ.Литр Тогда
				Продолжить;
			КонецЕсли;          
			
			РезультатПроверки = МенеджерОборудованияМаркировка.РезультатПроверкиКМ(Позиция.КонтрольнаяМарка);
			Если РезультатПроверки.Результат Тогда
				Если РезультатПроверки.ПланируемыйСтатус = Перечисления.ПланируемыйСтатусМаркируемогоТовара.ШтучныйТоварРеализован
					Или РезультатПроверки.ПланируемыйСтатус = Перечисления.ПланируемыйСтатусМаркируемогоТовара.МерныйТоварРеализован 
					Или РезультатПроверки.ПланируемыйСтатус = Перечисления.ПланируемыйСтатусМаркируемогоТовара.ШтучныйТоварВозвращен Тогда
					ПозицияЧека = Новый Структура(); 
					ПозицияЧека.Вставить("КонтрольнаяМарка"   , Позиция.КонтрольнаяМарка);    
					ПозицияЧека.Вставить("КодЕдиницыИзмерения", Позиция.КодЕдиницыИзмерения); 
					...
					ПозицияЧека.Вставить("ДробноеКоличество", ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПараметрыДробноеКоличество()); 
...
// Поиск результата проверки КМ по журналу операций проверки
//
// Параметры:
...
// Возвращаемое значение:
//  Структура
Функция РезультатПроверкиКМ(КодМаркировки)  Экспорт
	
	Возврат РегистрыСведений.ОперацииПроверкиКМ.РезультатПроверкиКМ(КодМаркировки);
...
Процедура ЗаполнитьОтраслевойРеквизитОбщественноеПитание(Позиция) Экспорт
	...
	Если ТипыТоваров.ОбщественноеПитание Тогда
		...
		ОтраслевойРеквизит.ДатаДокументаОснования = Дата("20220326"); // 1263 - 26.03.2022
		ОтраслевойРеквизит.НомерДокументаОснования = "477"; // 1264
		ЗначениеРеквизита = "mode=horeca"; // 1265    
		Если ПустаяСтрока(ОтраслевойРеквизит.ЗначениеРеквизита) Тогда // 1265
			ОтраслевойРеквизит.ЗначениеРеквизита = ЗначениеРеквизита;
		Иначе
			ОтраслевойРеквизит.ЗначениеРеквизита = ОтраслевойРеквизит.ЗначениеРеквизита + "&" + ЗначениеРеквизита;    
...
Процедура ЗаполнитьОтраслевойРеквизитИзделияИзНатуральногоМеха(Позиция) Экспорт
	...
	Если ТипыТоваров.ИзделияИзНатуральногоМеха Тогда
		...
		ЗначениеРеквизита = "crpt=mrk"; // 1265       
		Если ПустаяСтрока(ОтраслевойРеквизит.ЗначениеРеквизита) Тогда // 1265
			ОтраслевойРеквизит.ЗначениеРеквизита = ЗначениеРеквизита;
		Иначе
			ОтраслевойРеквизит.ЗначениеРеквизита = ОтраслевойРеквизит.ЗначениеРеквизита + "&" + ЗначениеРеквизита;    
...
Процедура ЗаполнитьОтраслевойРеквизитМолочнаяПродукцияСНечитаемымиКМ(Позиция) Экспорт
	...
	Если ТипыТоваров.МолочнаяПродукцияСНечитаемымиКМ Тогда   
		...
		ЗначениеРеквизита = "pd=" + ДатаПроизводства + "&km=tech"; // 1265 - ГГММДД – дата производства товара     
		Если ПустаяСтрока(ОтраслевойРеквизит.ЗначениеРеквизита) Тогда // 1265
			ОтраслевойРеквизит.ЗначениеРеквизита = ЗначениеРеквизита;
		Иначе
			ОтраслевойРеквизит.ЗначениеРеквизита = ОтраслевойРеквизит.ЗначениеРеквизита + "&" + ЗначениеРеквизита;    
