...
Процедура ПриВыполненииОтдельныхПроверокПродавцаИПокупателя(ПараметрыОбработки) Экспорт
	...
	Попытка
		...
		Для Каждого Строка Из ДанныеШтрихкодов Цикл
			...
			Если ШтрихкодированиеОбщегоНазначенияИСМП.ВладелецМожетОтличатьсяПоДатеПроизводства(Строка.ДанныеСервиса, ПараметрыОбработки.ПараметрыСканирования, Истина) Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ПриПолученииСтатусаДокументаИзОчереди(СтрокаОчереди) Экспорт
	...
	Если СтрокаОчереди.СтатусДокументаВСервисе = Перечисления.СтатусыДокументовИСМП.Проверен
