...
//
// Параметры:
//	Ставка - Число - Значение ставки НДС.
//	РасчетнаяСтавка       - Булево - значение реквизита РасчетнаяСтавка.
// Возвращаемое значение:
//	СправочникСсылка.СтавкиНДС - Соответствующая перечислению ставка НДС.
//
Функция НайтиСтавкуНДС(Ставка, РасчетнаяСтавка = Ложь) Экспорт
...
// Ищет первый ДОПП, который привязан к передаваемому документу
// 
// Параметры:
//  Ссылка - ДокументССылка
// 
// Возвращаемое значение:
//  Неопределено, Произвольный - Получить ДОПП по основанию
Функция ПолучитьДОПППоОснованию(Ссылка) Экспорт
	
	МассивДОПП = Документы.ДокументОПредстоящейПоставке.НайтиДОПППоОснованию(Ссылка).МассивДОПП;
	Если МассивДОПП.Количество()>0 Тогда
		Возврат МассивДОПП[0];
	Иначе
...
// отчет "Статистическая форма учета перемещения товаров".
// 
// Параметры:
//  Реквизиты                     - Структура - содержит реквизиты документа, на основании которого
//                                 открывается статистическая форма учета перемещения товаров.
//  поля структуры:
...
Функция ЗаполнитьСчетаФактурыОснования(ДанныеЗаполнения) Экспорт
	...
	ЭтоНачислениеНДСПриОтгрузке = ?(ДанныеЗаполнения.Свойство("НачислениеНДСПриОтгрузке"), ДанныеЗаполнения.НачислениеНДСПриОтгрузке, Ложь);
	ЭтоИсправление = ?(ДанныеЗаполнения.Свойство("Исправление"), ДанныеЗаполнения.Исправление, Ложь);
	...
	КонецЕсли;
	Документы.СчетФактураВыданный.ПоместитьТаблицуТоваровТорговыхДокументов(МассивСторнируемыхДокументовОснований,
	...
	|	И НЕ вт_ТаблицаТоваров.ИсходныйСчетФактура = ЗНАЧЕНИЕ(Документ.СчетФактураВыданный.ПустаяСсылка)
	...
	|ГДЕ
	|	НЕ вт_ТаблицаТоваров.ИсходныйСчетФактура = ЗНАЧЕНИЕ(Документ.СчетФактураВыданный.ПустаяСсылка)
	|	И НЕ &НачислениеНДСПриОтгрузке
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
