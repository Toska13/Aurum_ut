...
//   Допущение: в документе по одной номенклатуре нет одновременно строк с указанием серий и строк без серий.
//
// Параметры:
...
Функция ДанныеДокументаСоответствуютДаннымУпаковок(ПроверяемыйДокумент, АвтоматическиФормироватьОСУ = Ложь) Экспорт
	...
	ДанныеШапки = ШтрихкодированиеИС.ПолучитьСтруктуруРеквизитовДокумента(ПроверяемыйДокумент);
	...
	ВидыПродукцииПоддерживающиеОСУ = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииОбъемноСортовогоУчета();
...
Процедура ПроверитьСведенияОМаркировке_5_02(ДанныеШтрихкодовУпаковок, Отказ) Экспорт
	...
	Если СтрокиУпаковок.Количество() Тогда
		Для Каждого НеРаспределенныйКод Из СтрокиУпаковок Цикл
			Если Не ЗначениеЗаполнено(НеРаспределенныйКод.Штрихкод) Тогда
				Продолжить;
			ИначеЕсли КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(НеРаспределенныйКод.ВидПродукции)
				Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(НеРаспределенныйКод.ВидПродукции) Тогда
				Продолжить;
			КонецЕсли;
			Если ВыведенныеСообщения.Получить(НеРаспределенныйКод.Номенклатура) = Неопределено Тогда
				ВыведенныеСообщения.Вставить(НеРаспределенныйКод.Номенклатура, Новый Соответствие);
...
Процедура ЗаполнениеПоказателейМаркировкиВДереве_5_02(ДеревоДанных, ДанныеШтрихкодовУпаковок) Экспорт
	...
	Для Каждого СтрокаДанныхШтрихкодов Из ДанныеШтрихкодовУпаковок Цикл
		...
		Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(СтрокаДанныхШтрихкодов.ВидПродукции) Тогда
			ЕстьТабачнаяПродукция = Истина;
		ИначеЕсли КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииИСМПЕГАИС(СтрокаДанныхШтрихкодов.ВидПродукции) Тогда
...
Процедура ЗаполнитьСведенияОМаркировкеУКД(Приемник, Источник, ДанныеШтрихкодовУпаковокДо, ДанныеШтрихкодовУпаковокПосле) Экспорт
	...
	Для Каждого СтрокаУпаковки Из СтрокиУпаковокДо Цикл
		...
		Иначе //групповая и индивидуальная
...
Процедура ЗаполнитьСведенияОМаркировкеУКД2020(Приемник, Источник, ДанныеШтрихкодовУпаковокДо, ДанныеШтрихкодовУпаковокПосле) Экспорт
	...
	Для Каждого СтрокаУпаковки Из СтрокиУпаковокДо Цикл
		...
		Если СтрокаУпаковки.ВидУпаковки = Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет
			И КонструкторИСМПКлиентСервер.ЭтоСырьеГИСМТ(СтрокаУпаковки.ВидПродукции) Тогда
			НоваяСтрока.Код = СтрокаУпаковки.ЗначениеШтрихкода;
			СтрокаУпаковки.Обработан = Истина;
		ИначеЕсли СтрокаУпаковки.ВидУпаковки = Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет
				И Не ЗначениеЗаполнено(СтрокаУпаковки.Штрихкод) Тогда
	...
	Если КодыОСУДо.Количество() И Не КодыОСУПосле.Количество() И Приемник.Количество = 0 Тогда
		...
		Для Каждого СтрокаУпаковки Из КодыОСУДо Цикл
			...
			СтруктураДанных.Вставить("ЗначениеШтрихкода", СтрокаУпаковки.ЗначениеШтрихкода);
...
Процедура ДобавитьСведенияОУчастникахОбмена_5_02(ДанныеШтрихкодовУпаковок, Источник, СтруктураДанных, ДеревоДанных, ДатаСведений, Отказ) Экспорт
	...
	ПараметрыУказанияМОД = КонструкторПараметрыУказанияМОДПоВидуПродукции();
	
	Для Каждого ДанныеПоВидуПродукции Из ПараметрыУказанияДанныхПоМОДВЭлектронномДокументе.ПараметрыУказанияМОДПоВидамПродукции Цикл
		
		ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителя                               = ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителя Или ДанныеПоВидуПродукции.Значение.ТребуетсяУказаниеКодаФИАСПоМОДОтправителя;
		ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяИндивидуальныхПредпринимателей = ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяИндивидуальныхПредпринимателей Или ДанныеПоВидуПродукции.Значение.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяИндивидуальныхПредпринимателей;
		ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяЮридическихЛиц                 = ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяЮридическихЛиц Или ДанныеПоВидуПродукции.Значение.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяЮридическихЛиц;
		ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателя                                = ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателя Или ДанныеПоВидуПродукции.Значение.ТребуетсяУказаниеКодаФИАСПоМОДПолучателя;
		ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяИндивидуальныхПредпринимателей  = ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяИндивидуальныхПредпринимателей Или ДанныеПоВидуПродукции.Значение.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяИндивидуальныхПредпринимателей;
		ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяЮридическихЛиц                  = ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяЮридическихЛиц Или ДанныеПоВидуПродукции.Значение.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяЮридическихЛиц;
		
	КонецЦикла;
	
	Для Каждого ТипУчастника Из Участники Цикл
		
		ДанныеПродавца = (ТипУчастника.Ключ = "Продавец");
		Если ДанныеПродавца И Не ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителя Тогда
			Продолжить;
		КонецЕсли;
		
		Если Не ДанныеПродавца И Не ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателя Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Каждого Участник Из СписокУчастниковСтороны Цикл
			...
			Если ДанныеПродавца И Не ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяИндивидуальныхПредпринимателей
			...
			Если ДанныеПродавца И Не ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяЮридическихЛиц
			...
			Если Не ДанныеПродавца И Не ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяИндивидуальныхПредпринимателей
				И ЭтоИндивидуальныйПредприниматель Тогда
			...
			Если Не ДанныеПродавца И Не ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяЮридическихЛиц
				И Не ЭтоИндивидуальныйПредприниматель Тогда
			...
			Если СтруктураАдресов[ТипУчастника.Ключ].АвтоматическоеЗаполнение Тогда
				...
				Если ЗначениеЗаполнено(ВладелецКонтактнойИнформации) И ЗначениеЗаполнено(ВидКонтактнойИнформации) Тогда
					...
					Если ЗначениеЗаполнено(КонтактнаяИнформация) Тогда
						...
						ИначеЕсли Не ЗначениеЗаполнено(ИдентификаторГАР_ФИАС) И Не ЗначениеЗаполнено(СтруктураАдресов[ТипУчастника.Ключ].КодФИАС) Тогда
...
Процедура ДобавитьСведенияОПеревозочныхДокументах_5_02(ДанныеШтрихкодовУпаковок, Источник, СтруктураДанных, ДеревоДанных, ДатаСведений, Отказ) Экспорт
	...
	Если Не ПараметрыУказанияДанныхПоМОДВЭлектронномДокументе.ТребуетсяУказаниеПеревозочныхДокументов Тогда
		Возврат;
	КонецЕсли;
...
// Функция инициализирует пустой конструктор параметров указания МОД по виду продукции для дальнейшего заполнения
// 
// Возвращаемое значение:
//  Структура - Конструктор параметры указания МОДПо виду продукции:
...
// * ТребуетсяУказаниеКодаФИАСПоМОДОтправителяИндивидуальныхПредпринимателей - Булево - Истина, если требуется указание ФИАС/ГАР для ИП по отправителю
// * ТребуетсяУказаниеКодаФИАСПоМОДОтправителяЮридическихЛиц - Булево        - Истина, если требуется указание ФИАС/ГАР для юр.лиц по отправителю
// * ТребуетсяУказаниеКодаФИАСПоМОДПолучателяИндивидуальныхПредпринимателей  - Булево - Истина, если требуется указание ФИАС/ГАР для ИП по получателю
// * ТребуетсяУказаниеКодаФИАСПоМОДПолучателяЮридическихЛиц                  - Булево - Истина, если требуется указание ФИАС/ГАР для юр.лиц по получателю
// * ПроверятьМОДОтправителяТолькоПоПодконтрольнымОбъектамВЕТИС              - Булево
// * ПроверятьМОДПолучателяТолькоПоПодконтрольнымОбъектамВЕТИС               - Булево
Функция КонструкторПараметрыУказанияМОДПоВидуПродукции() Экспорт
	...
	СтруктураВозврата.Вставить("ТребуетсяУказаниеКодаФИАСПоМОДОтправителяИндивидуальныхПредпринимателей", Ложь);
	СтруктураВозврата.Вставить("ТребуетсяУказаниеКодаФИАСПоМОДОтправителяЮридическихЛиц",                 Ложь);
	СтруктураВозврата.Вставить("ТребуетсяУказаниеКодаФИАСПоМОДПолучателяИндивидуальныхПредпринимателей",  Ложь);
	СтруктураВозврата.Вставить("ТребуетсяУказаниеКодаФИАСПоМОДПолучателяЮридическихЛиц",                  Ложь);
	СтруктураВозврата.Вставить("ПроверятьМОДОтправителяТолькоПоПодконтрольнымОбъектамВЕТИС",              Истина);
	СтруктураВозврата.Вставить("ПроверятьМОДПолучателяТолькоПоПодконтрольнымОбъектамВЕТИС",               Истина);
...
// 
// Параметры:
//  ВидыПродукции - Массив из СправочникСсылка.ВидыПродукцииИС, СправочникСсылка.КлючиВидовПродукцииИС - Виды продукции
...
// * ПрослеживаемыеВидыПродукции              - Массив из СправочникСсылка.ВидыПродукцииИС, СправочникСсылка.КлючиВидовПродукцииИС - список прослеживаемых между МОД видов продукций
// * ПараметрыУказанияМОДПоВидамПродукции - Соответствие Из КлючИЗначение:
//		** Ключ - СправочникСсылка.ВидыПродукцииИС, СправочникСсылка.КлючиВидовПродукцииИС - вид продукции
//		** Значение - см. КонструкторПараметрыУказанияМОДПоВидуПродукции
...
Функция ПараметрыУказанияДанныхПоМОДВЭлектронномДокументеУПД_5_02(ВидыПродукции, НаДату = Неопределено) Экспорт
	...
	СтруктураВозврата.Вставить("ПараметрыУказанияМОДПоВидамПродукции",     Новый Соответствие);
	СтруктураВозврата.Вставить("ТребуетсяУказаниеПеревозочныхДокументов",  Ложь);
	
	Если Не ТипЗнч(ВидыПродукции) = Тип("Массив") Тогда
		МассивВидовПродукции = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ВидыПродукции);
	Иначе
		МассивВидовПродукции = ВидыПродукции;
	КонецЕсли;
	
	ТребуетсяУказаниеПеревозочныхДокументов = Ложь;
	
	Для Каждого ПроверяемыйВидПродукции Из МассивВидовПродукции Цикл
		
		ПеремещениеПрослеживается = КонструкторИСМПКлиентСервер.ДоступноПеремещениеМеждуМОД(ПроверяемыйВидПродукции);
		
		Если ПеремещениеПрослеживается И ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПеремещенийМеждуМОД(ПроверяемыйВидПродукции, НаДату) Тогда
		...
		ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяЮридическихЛиц                 = КонструкторИСМПКлиентСервер.ВУПДПередаватьКодФИАСОтправителяПоЮридическимЛицам(ПроверяемыйВидПродукции);
		ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяИндивидуальныхПредпринимателей = КонструкторИСМПКлиентСервер.ВУПДПередаватьКодФИАСОтправителяПоИндивидуальнымПредпринимателям(ПроверяемыйВидПродукции);
		ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяЮридическихЛиц                  = КонструкторИСМПКлиентСервер.ВУПДПередаватьКодФИАСПолучателяПоЮридическимЛицам(ПроверяемыйВидПродукции);
		ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяИндивидуальныхПредпринимателей  = КонструкторИСМПКлиентСервер.ВУПДПередаватьКодФИАСПолучателяПоИндивидуальнымПредпринимателям(ПроверяемыйВидПродукции);
		ПараметрыУказанияМОД.ПроверятьМОДОтправителяТолькоПоПодконтрольнымОбъектамВЕТИС              = КонструкторИСМПКлиентСервер.ВУПДПередаватьКодФИАСОтправителяТолькоПоПоднадзорнымОбъектамВЕТИС(ПроверяемыйВидПродукции);
		ПараметрыУказанияМОД.ПроверятьМОДПолучателяТолькоПоПодконтрольнымОбъектамВЕТИС               = КонструкторИСМПКлиентСервер.ВУПДПередаватьКодФИАСПолучателяТолькоПоПоднадзорнымОбъектамВЕТИС(ПроверяемыйВидПродукции);
		
		ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителя = ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяИндивидуальныхПредпринимателей
			Или ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяЮридическихЛиц;
			
		ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателя = ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяИндивидуальныхПредпринимателей
			Или ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяЮридическихЛиц;
		
		СтруктураВозврата.ПараметрыУказанияМОДПоВидамПродукции.Вставить(ПроверяемыйВидПродукции, ПараметрыУказанияМОД);
		
		ТребуетсяУказаниеПеревозочныхДокументов = ТребуетсяУказаниеПеревозочныхДокументов Или КонструкторИСМПКлиентСервер.ВУПДФормата_5_02_ПередаватьПеревозочныеДокументы(ПроверяемыйВидПродукции);
		
	КонецЦикла;
	
	СтруктураВозврата.ТребуетсяУказаниеПеревозочныхДокументов = ТребуетсяУказаниеПеревозочныхДокументов;
...
//  ВидПродукцииИлиКлюч - СправочникСсылка.ВидыПродукцииИС, СправочникСсылка.КлючиВидовПродукцииИС - Вид продукции
...
Функция КодМаркировкиДляПередачиУПД(ЗначениеШтрихкода, ВидПродукцииИлиКлюч, РезультатРазбора = Неопределено, ТипШтрихкодаSSCC = Неопределено) Экспорт
	
	Если ТипЗнч(ВидПродукцииИлиКлюч) = Тип("СправочникСсылка.КлючиВидовПродукцииИС") Тогда
		ВидыПродукцииКлюча = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииКлюча(ВидПродукцииИлиКлюч, "ГИСМТ");
		Если ВидыПродукцииКлюча.Количество() = 1 Тогда
			ВидПродукции = ВидыПродукцииКлюча[0];
		Иначе
			ВидПродукции = Неопределено;
		КонецЕсли;
	Иначе
		ВидПродукции = ВидПродукцииИлиКлюч;
	КонецЕсли;
...
// 	 * ВидПродукции       - СправочникСсылка.ВидыПродукцииИС - вид продукции номенклатуры для кодировки значения штрихкода
...
//   ВидыПродукцииИС   - СправочникСсылка.ВидыПродукцииИС, Массив Из СправочникСсылка.ВидыПродукцииИС, Неопределено -
...
Функция Содержимое(Знач ШтрихкодыУпаковок, Знач ВидыПродукцииИС = Неопределено, ДоКомплектующих = Ложь, ВключатьШтрихкодУпаковки = Ложь) Экспорт
	...
	ШтрихкодированиеИС.ВыделитьШтрихкодыСодержащиеВидыПродукции(ПроверенныеШтрихкоды, ВидыПродукцииИС);
	...
	Пока ВложенныеЗаписи.Количество() Цикл
		...
		ВложенныеЗаписи = ОбщегоНазначенияКлиентСервер.РазностьМассивов(
			ДанныеВложенныхШтрихкодов[1].Выгрузить().ВыгрузитьКолонку("Штрихкод"), ПроверенныеШтрихкоды);
...
// 	 * ЕдиницаИзмеренияКод - Строка - опционально, если передано - проверять строковый код единицы измерения по ОКЕИ
// 	 * Количество     - Число - количество
//  ПараметрыСканирования - См. ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования
// Возвращаемое значение:
...
Функция СодержимоеИКодыОСУ(Упаковки, ТаблицаТоваров, ПараметрыСканирования, ИмяКолонкиКоличество = "Количество") Экспорт
	...
	СодержимоеРезультат.Колонки.Добавить("ВидПродукции",       Новый ОписаниеТипов(ТипыВидовПродукции));
...
Процедура НормализоватьШтрихкодыУпаковок(Товары, ШтрихкодыУпаковок) Экспорт
	...
	ТекстЗапроса = ТекстЗапроса +
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ЕСТЬNULL(СвойстваМаркируемойПродукции.ВидПродукции.ВидПродукцииГИСМТ, ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)) КАК ВидПродукции
		|ИЗ
		|	СвойстваМаркируемойПродукции КАК СвойстваМаркируемойПродукции
		|ГДЕ
		|	ЕСТЬNULL(СвойстваМаркируемойПродукции.ВидПродукции.ВидПродукцииГИСМТ, ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)";
...
Процедура НормализоватьШтрихкодыУпаковокУКД(ШтрихкодыУпаковок) Экспорт
	...
	ТекстЗапроса = ТекстЗапроса +
		...
		|	СвойстваМаркируемойПродукции.ВидПродукции.ВидПродукцииГИСМТ КАК ВидПродукции
		|ИЗ
		|	СвойстваМаркируемойПродукции КАК СвойстваМаркируемойПродукции
		|ГДЕ
	...
	Пока ВыборкаПоВидуПродукции.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаПоВидуПродукции.ВидПродукции) Тогда
...
//
// Возвращаемое значение:
//  Массив из см. ДанныеШтрихкодаИзСтрокиДереваЭДО - массив штрихкодов упаковок
	...
	Если СведенияОМаркировке <> Неопределено И ЗначениеЗаполнено(СведенияОМаркировке.Значение) Тогда
		...
		Для Каждого Маркировка Из СведенияОМаркировке.Строки Цикл
			...
			Если ЗначениеЗаполнено(ЛогистическаяУпаковка) Тогда
				ДанныеШтрихкода = ДанныеШтрихкодаИзСтрокиДереваЭДО();
				ДанныеШтрихкода.ЗначениеШтрихкода = ЛогистическаяУпаковка;
				ДанныеШтрихкода.ИсточникДанных = "СведенияОМаркировке";
			...
			Если ЗначениеЗаполнено(КоличествоПоОСУ) И ЗначениеЗаполнено(ДанныеГТИН.Значение) Тогда
				...
				КодОСУ = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ОСУПоШтрихкодуEAN(
					ДанныеГТИН.Значение, КоличествоПоОСУ, Ложь);
				ДанныеШтрихкода = ДанныеШтрихкодаИзСтрокиДереваЭДО();
				ДанныеШтрихкода.ЗначениеШтрихкода = КодОСУ;
				ДанныеШтрихкода.ИсточникДанных = "ГТИН";
				ШтрихкодыУпаковок.Добавить(ДанныеШтрихкода);
			...
			Если КонтрольныеЗнаки <> Неопределено И ЗначениеЗаполнено(КонтрольныеЗнаки.Значение) Тогда
				...
				Для Каждого КонтрольныйЗнак Из КонтрольныеЗнаки.Строки Цикл
					...
					ДанныеШтрихкода = ДанныеШтрихкодаИзСтрокиДереваЭДО();
					ДанныеШтрихкода.ЗначениеШтрихкода = ЗначениеРеквизитаВДереве(
						КонтрольныйЗнак, ПутьКИЗ + ".НомерСтроки.КонтрольныйЗнак");
					ДанныеШтрихкода.ИсточникДанных = "КонтрольныйЗнак";
					ШтрихкодыУпаковок.Добавить(ДанныеШтрихкода);
			...
			Если ИдентификаторыУпаковок <> Неопределено И ЗначениеЗаполнено(ИдентификаторыУпаковок.Значение) Тогда
				...
				Для Каждого ИдентификаторУпаковки Из ИдентификаторыУпаковок.Строки Цикл
					...
					ДанныеШтрихкода.ИсточникДанных = "ИдентификаторУпаковки";
					ШтрихкодыУпаковок.Добавить(ДанныеШтрихкода);
...
// * ШтрихкодУпаковки - СправочникСсылка.ШтрихкодыУпаковокТоваров - Штрихкод упаковки
// * ЗначениеШтрихкода - Строка - Значение штрихкода
// * ИсточникДанных - Строка - Источник данных (для заполнения из ЭДО) "ИдентификаторУпаковки", "КонтрольныйЗнак", "ГТИН", "СведенияОМаркировке"
// * Номенклатура - ОпределяемыйТип.Номенклатура - Номенклатура
// * Характеристика - ОпределяемыйТип.ХарактеристикаНоменклатуры - Характеристика
Функция НоваяТаблицаШтрихкодыУпаковок(ДобавитьНоменклатуру = Ложь) Экспорт
	...
	ШтрихкодыУпаковок.Колонки.Добавить("ИсточникДанных",   Новый ОписаниеТипов("Строка",,,,Новый КвалификаторыСтроки(200)));
...
// 	ШтрихкодыУпаковок - ТаблицаЗначений - Таблица штрихкодов (См. НоваяТаблицаШтрихкодыУпаковок).
// 	СведенияОТоваре - СтрокаДереваЗначений - Строка сведений о товаре из документа ЭДО.
// 	СтрокаТовары - СтрокаТабличнойЧасти, СтрокаТаблицыЗначений - Строка таблицы товаров с колонками "Номенклатура", "Характеристика" для определения идентификаторов сыря
Процедура ДобавитьШтрихкодыТаблицуШтрихкодовУпаковок_5_02(ШтрихкодыУпаковок, СведенияОТоваре, СтрокаТовары = Неопределено) Экспорт
	
	ШтрихкодыУпаковокСтрокиТЧ = ШтрихкодыУпаковокИзСтрокиДереваЭДО_5_02(СведенияОТоваре);
	Для Каждого СтрокаДанных Из ШтрихкодыУпаковокСтрокиТЧ Цикл
		
		НоваяСтрока = ШтрихкодыУпаковок.Добавить();
		Если СтрокаТовары <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТовары);
		КонецЕсли;
		НоваяСтрока.ЗначениеШтрихкода = СтрокаДанных.ЗначениеШтрихкода;
		НоваяСтрока.ИсточникДанных    = СтрокаДанных.ИсточникДанных;
...
Функция ПроверитьПоддержкуФорматаЭДО(ФорматУПД) Экспорт
	...
	Если СтандартнаяОбработка Тогда
		...
		Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ФорматыЭДО");
...
Функция ДеревоЭлектронногоДокумента(ПутьКМакетуДанныхЭД) Экспорт
	...
	Если СтандартнаяОбработка Тогда
		...
		Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ДеревоЭлектронногоДокументаБЭД");
...
Функция ЗначениеРеквизитаВДереве(ДеревоДанных, ПолныйПуть, СообщатьОбОшибке = Ложь) Экспорт
	...
	Если СтандартнаяОбработка Тогда
		Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ЭлектронноеВзаимодействие");
...
Функция ИдентификаторУчетнойЗаписиЭДО(Организация) Экспорт
	...
	Если СтандартнаяОбработка Тогда
		...
		Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("УчетныеЗаписиЭДО");
...
Процедура СформироватьДанныеСообщенияОтправителя(ДанныеСообщения, ДеревоДанных, СтруктураЭД, ИдентификаторОтправителя, ИдентификаторПолучателя) Экспорт
	...
	Если СтандартнаяОбработка Тогда
		...
		Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ЭлектронныеДокументыЭДО");
		МодульФНС = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ФорматыЭДО");
...
Функция СформироватьДеревоОтветногоТитулаПокупателя(ОбъектУчета, ИмяФайла, ДвоичныеДанныеФайла) Экспорт
	...
	Если СтандартнаяОбработка Тогда
		...
		Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ФорматыЭДО");
		МодульИнтеграция = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияЭДО");
...
Функция ЗаполнитьПодписантаИЭлектроннуюПодпись(ОписаниеФайла, Организация, СертификатПодписи, ПодписьПродавца = Неопределено, ВыбраннаяМЧД = Неопределено) Экспорт
	...
	Если СтандартнаяОбработка Тогда
		...
		МодульБЭД = ОбщегоНазначения.ОбщийМодуль("ОбновлениеИнформационнойБазыБЭД");
		ВерсияБЭД = МодульБЭД.ВерсияБиблиотеки();
		
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБЭД, "1.9.16.1") >= 0 Тогда
			ИдентификаторОтправителя = ИдентификаторУчетнойЗаписиЭДО(Организация);
			ОписаниеДополненногоФайла = Модуль.ЗаполнитьДанныеПодписантаДокумента(ОписаниеФайла, Организация, СертификатПодписи, СодержаниеДокумента, ИдентификаторОтправителя, ВыбраннаяМЧД);
		Иначе
		...
		Если СтандартнаяОбработка Тогда
			...
			МодульРаботаСФайлами   = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("РаботаСФайламиБЭД");
			МодульОбщегоНазначения = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ОбщегоНазначенияБЭД");
...
Функция СтатусДокументооборота(Объект) Экспорт
	...
	Если СтандартнаяОбработка Тогда
		Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ОбменСКонтрагентами");
...
Функция ПредставлениеЭлектронногоДокументаПоФайлу(СообщениеОтправителя, ДвоичныеДанныеОсновногоФайла, 
	...
	ЭлектронноеВзаимодействиеИСМППереопределяемый.ПриОпределенииПредставленияЭлектронногоДокументаПоФайлу(СообщениеОтправителя, ДвоичныеДанныеОсновногоФайла, 
...
Функция ПараметрыЗаполненияТаблицыШтрихкодовАктОРасхождениях()
	...
	ПараметрыЗаполнения.Вставить("ЗаполнятьТипРасхождения", Ложь);
...
Функция ТаблицаМаркируемойПродукцииДокумента(Документ, ВидыМаркируемойПродукции) Экспорт
	...
	ТаблицаПродукции.Колонки.Добавить("ВидПродукцииГИСМТ",  Новый ОписаниеТипов("СправочникСсылка.ВидыПродукцииИС"));
	ТаблицаПродукции.Колонки.Добавить("КлючВидовПродукции", Новый ОписаниеТипов("СправочникСсылка.КлючиВидовПродукцииИС"));
...
Функция СформироватьКодОСУИзGTIN(СтрокаТовары, СтрокаУпаковки, Поле = "Количество")
	...
	Возврат ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ОСУПоШтрихкодуEAN(
		СтрокаУпаковки.ЗначениеШтрихкода, КоличествоПоТегу37, Ложь);
...
Функция МожноРаспределить(СтрокиТовара, СтрокиРаспределения, Что, Куда, Накоплено)
	...
	Если Куда.ВидУпаковки = Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет
		...
		ОсталосьРаспределить = КоличествоПотребительскихУпаковок;
		Для Каждого Строка Из СтрокиРаспределения Цикл 
			Если ОсталосьРаспределить = 0 Тогда
				Прервать;
			КонецЕсли;
			МожноРаспределить = Мин(Строка.КоличествоПотребительскихУпаковок - Накоплено[Строка], ОсталосьРаспределить);
			Если МожноРаспределить = 0 Тогда
				Продолжить;
			КонецЕсли;
			Накоплено[Строка] = Накоплено[Строка] + МожноРаспределить; 
			ОсталосьРаспределить = ОсталосьРаспределить - МожноРаспределить;
			Если Накоплено["ЕстьСерии"] = Истина Тогда
				ЭлементРаспределения = Новый Структура;
				ЭлементРаспределения.Вставить("Серия", СтрокиТовара[Что].Серия);
				ЭлементРаспределения.Вставить("Количество", МожноРаспределить);
				Строка.КоличестваПоСтрокеОбъемноСортовогоУчета.Добавить(ЭлементРаспределения);
			КонецЕсли;
		КонецЦикла;
		Возврат ОсталосьРаспределить = 0;
...
// Добавляет в таблицу коды ОСУ для строк, по которым коды маркировки не указывались. Преобразует и сворачивает 
//   указанные яавно коды ОСУ по GTIN (и сериям, если колонка серий присутствует в таблице товаров).
//   Считаем что таблица товаров соответствует таблице штрихкодов упаковок (с учетом невнесенных ОСУ).
//
// Параметры:
//  ТаблицаТоваров	 - ТаблицаЗначений - исходная таблица товаров: 
// 	 * Ссылка         - Произвольный - объект-владелец штрихкода
// 	 * Номенклатура   - ОпределяемыйТип.Номенклатура - номенклатура
// 	 * Характеристика - ОпределяемыйТип.ХарактеристикаНоменклатуры - характеристика (будет добавлена при отсутствии)
// 	 * Серия          - ОпределяемыйТип.СерияНоменклатуры - серия (опциональная колонка)
// 	 * Количество     - Число - количество
// 	 * ЕдиницаИзмеренияКод - Строка - опционально, код единицы измерения по ОКЕИ для проверки поддержки в системе прослеживаемости фармацевтического сырья
//  ПараметрыСканирования - См. ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования
...
Процедура ДополнитьРезультатКодамиОСУ(ТаблицаТоваров, ПараметрыСканирования, ЧастичноеСодержимоеРезультат, ИмяКолонкиКоличествоТаблицыТовары = "Количество") Экспорт
	...
	ТекстЗапроса = 
		...
		|			ГДЕ ЗначенияЭлементовКонструктора.ЭлементКонструктора = ЗНАЧЕНИЕ(Справочник.ЭлементыКонструктораВидовПродукцииИС.ДатыНачалаОформленияУПДОСУ)
		...
		|			ГДЕ ЗначенияПользователяЭлементовКонструктора.ЭлементКонструктора = ЗНАЧЕНИЕ(Справочник.ЭлементыКонструктораВидовПродукцииИС.ДатыНачалаОформленияУПДОСУ)
		|				И НЕ ЗначенияПользователяЭлементовКонструктора.ПометкаУдаления
		|			) КАК ДатыНачалаОформленияУПДОСУ
		|			ПО СвойстваМаркируемойПродукции.ВидПродукции.ВидПродукцииГИСМТ = ДатыНачалаОформленияУПДОСУ.ВидПродукции
		|		ПО ДанныеШтрихкодовУпаковок.Номенклатура = СвойстваМаркируемойПродукции.Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		|		ПО &ТоварыОписаниеНоменклатурыИС
		|ГДЕ
		|	СвойстваМаркируемойПродукции.ВидПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииПоддерживаютОСУ)
		|	И НЕ СвойстваМаркируемойПродукции.ВидПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииПоддерживаютОСУПриВнутреннемПеремещении)
		|	И ДатыНачалаОформленияУПДОСУ.Значение <> ДАТАВРЕМЯ(1, 1, 1)
		|	И ДатыНачалаОформленияУПДОСУ.Значение < &КонтролируемаяДата
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|		ПО &ТоварыОписаниеНоменклатурыИС
		|ГДЕ
		|	СвойстваМаркируемойПродукции.ВидПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииПоддерживаютОСУПриВнутреннемПеремещении)
		|";
	...
	Если ЭтоДокументВнутреннегоПеремещения Тогда
		ВидыПродукцииПоддерживаютОСУПриВнутреннемПеремещении = КонструкторИСМПКлиентСервер.ВидыПродукцииВнутреннегоПеремещенияМеждуМОД();
	Иначе
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Товары",                       Товары);
	Запрос.УстановитьПараметр("ВидыПродукцииПоддерживаютОСУ", КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииОбъемноСортовогоУчета());
	...
	ЕстьСерии = ТаблицаТоваров.Колонки.Найти("Серия")<>Неопределено;
	...
	СтрокиСырье        = Новый Соответствие();
	ДобавленныеСтроки  = Новый Массив;
	ПроверятьНаДату    = Неопределено;
	ДанныеДляЗапросаКоличестваПоGTIN = Новый Соответствие;
	...
	// Замена явно указанных кодов ОСУ на автоматические, (с количеством и суммированием по GTIN)
	Для Каждого СтрокаТЧ Из МаркируемаяПродукцияДляОСУ Цикл
		
		ВидПродукцииИСМП = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(СтрокаТЧ.ВидПродукцииИС, "ГИСМТ");
		
		Если Не ИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции(ВидПродукцииИСМП) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если КодыСтроки.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		КодыПоGTIN = Новый Соответствие;
		ЕстьКодыОСУ = Ложь;
		ОбщееКоличествоКодов = 0;
		ОбщееКоличество = СтрокаТЧ.Количество;
		ЭтоСырьеГИСМТ = КонструкторИСМПКлиентСервер.ЭтоСырьеГИСМТ(ВидПродукцииИСМП);
		Для Каждого КодСтроки Из КодыСтроки Цикл
			Если КодСтроки.ВидУпаковки = Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет Тогда
				GTIN = Сред(КодСтроки.ЗначениеШтрихкода, 3, 14);
				НакопленноеКоличество = КодыПоGTIN.Получить(GTIN);
				Если ЕстьСерии Тогда
					НакопленноеКоличествоПоСерии = ?(НакопленноеКоличество = Неопределено, 0, НакопленноеКоличество.Получить(КодСтроки.Серия));
					Если НакопленноеКоличество = Неопределено Тогда
						НакопленноеКоличество = Новый Соответствие;
					КонецЕсли;
					НакопленноеКоличество.Вставить(КодСтроки.Серия, НакопленноеКоличествоПоСерии + КодСтроки.КоличествоПотребительскихУпаковок);
					КодыПоGTIN.Вставить(GTIN, НакопленноеКоличество);
				Иначе
					КодыПоGTIN.Вставить(GTIN, ?(НакопленноеКоличество = Неопределено, 0, НакопленноеКоличество) + КодСтроки.КоличествоПотребительскихУпаковок);
				КонецЕсли;
				КодСтроки.ЗначениеШтрихкода = GTIN;
				ЕстьКодыОСУ = Истина;
			КонецЕсли;
			ОбщееКоличествоКодов = ОбщееКоличествоКодов + КодСтроки.КоличествоПотребительскихУпаковок;
			КодСтроки.ВидПродукции = СтрокаТЧ.ВидПродукцииИС;
			Если ЭтоСырьеГИСМТ Тогда
				ДобавитьСтрокуНоменклатуреСырьяДляПроверкиКодаЕдиницы(СтрокиСырье, СтрокаТЧ, ПараметрыСканирования);
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого КодСтроки Из КодыСтроки Цикл
			Если КодСтроки.ВидУпаковки = Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет Тогда
				...
				Если ЕстьСерии Тогда
					СерииПоGTIN = КодыПоGTIN.Получить(КодСтроки.ЗначениеШтрихкода);
					КодСтроки.КоличествоПотребительскихУпаковок = СерииПоGTIN.Получить(КодСтроки.Серия);
					СерииПоGTIN.Вставить(КодСтроки.Серия, 0);
				Иначе 
					КодСтроки.КоличествоПотребительскихУпаковок = КодыПоGTIN.Получить(КодСтроки.ЗначениеШтрихкода);
					КодыПоGTIN.Вставить(КодСтроки.ЗначениеШтрихкода, 0);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	...
	Для Каждого СтрокаТЧ Из МаркируемаяПродукцияДляОСУ Цикл
		...
		ЭтоСырьеГИСМТ = КонструкторИСМПКлиентСервер.ЭтоСырьеГИСМТ(СтрокаТЧ.ВидПродукцииИС);
		...
		Если ЭтоСырьеГИСМТ Тогда
			...
			Если Не СтрокаТЧ.АвтоматическийОСУИС Тогда
				...
				НоваяСтрока.ТекстОшибкиИС = СтрШаблон(
					НСтр("ru = 'Невозможно автоматическое формирование идентификационного кода фармацевтического сырья, укажите конкретное сырье в форме проверки или сопоставьте номенклатуру %1 с единственным сырьем ГИС МТ.'"),
			...
			ИначеЕсли Не ЗначениеЗаполнено(СтрокаТЧ.GTIN) Тогда
				ЕстьОшибки = Истина;
				НоваяСтрока.ТекстОшибкиИС = СтрШаблон(
					НСтр("ru = 'Требуется сопоставление номенклатуры %1 с фармацевтическим сырьем ГИС МТ.'"),
					ОбщегоНазначенияИС.ПредставлениеНоменклатуры(СтрокаТЧ.Номенклатура, СтрокаТЧ.Характеристика));
			Иначе
				ДобавитьСтрокуНоменклатуреСырьяДляПроверкиКодаЕдиницы(СтрокиСырье, НоваяСтрока, ПараметрыСканирования);
			КонецЕсли;
			
		Иначе
			Если СтрокаТЧ.ТребуетВзвешивания Тогда
				ДанныеДляЗапросаКоличестваПоGTIN.Вставить(СтрокаТЧ.GTIN,
	...
	ПроверитьКодыЕдиницИзмеренияСырья(СтрокиСырье, ТаблицаТоваров, ЕстьОшибки, ПараметрыСканирования);
	...
	РезультатЗапросаКоличестваПоGTIN  = ПроверкаИПодборПродукцииИСМПВызовСервера.КоличествоПотребительскихУпаковокПоGTIN(
		ДанныеДляЗапросаКоличестваПоGTIN, ПараметрыСканирования);
	
	Для Каждого СтрокаТЧ Из ДобавленныеСтроки Цикл
		Если СтрокаТЧ.ТребуетВзвешивания Тогда
			Если РезультатЗапросаКоличестваПоGTIN.ТребуетсяОбновлениеКлючаСессии Тогда
				ЕстьОшибки = Истина;
				...
				Продолжить;
			КонецЕсли;
			Если РезультатЗапросаКоличестваПоGTIN.КоличествоПотребительскихУпаковокПоGTIN = Неопределено Тогда
				ЕстьОшибки = Истина;
				СтрокаТЧ.ТекстОшибкиИС = СтрШаблон(
				...
				Продолжить;
			Иначе
				ОписаниеШтрихкода = РезультатЗапросаКоличестваПоGTIN.КоличествоПотребительскихУпаковокПоGTIN.Получить(СтрокаТЧ.ЗначениеШтрихкода);
...
Функция ЗаполнитьАдресУчастникаУПД_5_02(ДеревоДанных, АдресЗначение, АдресПредставление, Участник, ТаблицаКодовФИАСДляТегаТекстовойИнформации, УточненныйКодФИАС = "") Экспорт
	...
	ПутьКДаннымАдресаОбъект                  = Участник + ПутьККорнюАдреса + ".АвтоматическиЗаполняемый.ОбъектКонтактнойИнформации";
	ПутьКДаннымАдресаВидКонтактнойИнформации = Участник + ПутьККорнюАдреса + ".АвтоматическиЗаполняемый.ВидКонтактнойИнформации";
	...
	ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных, ПутьКДаннымАдресаОбъект, Неопределено);
	ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных, ПутьКДаннымАдресаВидКонтактнойИнформации, Неопределено);
...
// Функция возвращает признак наличия в таблице прослеживаемой при перемещении между МОД продукции:
//  условие - на указанную дату действует дата начала контроля прослеживаемости перемещения между МОД.
// 
// Параметры:
//  ДанныеШтрихкодовУпаковок - ТаблицаЗначений - Данные штрихкодов упаковок
//  НаДату                   - Дата            - Дата контроля
//  ИмяПоляВидПродукции      - Строка          - Имя поля, содержащий вид продукции в виде ключа или конкретного вида продукции
// 
// Возвращаемое значение:
//  Булево - Истина, если в таблице упаковок содержатся виды продукции, подлежащей прослеживанию при перемещении между МОД
Функция ШтрихкодыУпаковокСодержатПродукциюПодконтрольногоПеремещенияМеждуМОД(ДанныеШтрихкодовУпаковок, НаДату, ИмяПоляВидПродукции = "ВидПродукции") Экспорт
	
	Если Не ТипЗнч(ДанныеШтрихкодовУпаковок) = Тип("ТаблицаЗначений") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ДанныеШтрихкодовУпаковок.Колонки.Найти(ИмяПоляВидПродукции) = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ВидыПродукции = Новый Массив;
	
	Для Каждого СтрокаДанныхШтрихкодовУпаковок Из ДанныеШтрихкодовУпаковок Цикл
		
		ВидыПродукцииПоКлючу = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукции(СтрокаДанныхШтрихкодовУпаковок[ИмяПоляВидПродукции]);
		
		Если ТипЗнч(ВидыПродукцииПоКлючу) = Тип("Массив") Тогда
			Для Каждого ВидПродукцииПоКлючу Из ВидыПродукцииПоКлючу Цикл
				
				Если ВидыПродукции.Найти(ВидПродукцииПоКлючу) <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
				
				ВидыПродукции.Добавить(ВидПродукцииПоКлючу);
				
			КонецЦикла;
		Иначе
			ВидыПродукции.Добавить(ВидыПродукцииПоКлючу);
		КонецЕсли;
		
	КонецЦикла;
	
	Для Каждого ВидПродукции Из КонструкторИСМПКлиентСервер.ВидыПродукцииПрослеживаемогоПеремещенияМеждуМОД() Цикл
...
Процедура ЗаполнитьСтруктуруАдресовУчастниковОбменаПоПеремещениюМеждуМОД(ОбъектУчета, СтруктураДанных, СтруктураАдресов)
	...
	ЭтоИндивидуальныйПредприниматель = ОбщегоНазначенияИСВызовСервера.ПартнерЭтоИндивидуальныйПредприниматель(ДанныеШапки.Организация);
	...
	ПараметрыУказанияДанныхПоМОДВЭлектронномДокументе = ПараметрыУказанияДанныхПоМОДВЭлектронномДокументеУПД_5_02(ДанныеШапки.ТоварнаяГруппа, ДанныеШапки.ДатаДокумента);
	
	ПараметрыУказанияМОД = ПараметрыУказанияДанныхПоМОДВЭлектронномДокументе.ПараметрыУказанияМОДПоВидамПродукции.Получить(ДанныеШапки.ТоварнаяГруппа);
	...
	Если ТребуетсяУказаниеФИАСГрузоотправителя Тогда
	...
	Если ТребуетсяУказаниеФИАСГрузополучателя Тогда
...
Процедура ЗаполнитьДанныеМашиночитаемойДоверенности(Организация, СертификатПодписи, ВыбраннаяМЧД)
	...
	Если СтандартнаяОбработка Тогда
		...
		Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("МашиночитаемыеДоверенности");
		МодульКриптографии = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("КриптографияБЭД");
...
Функция ДанныеШтрихкодаИзСтрокиДереваЭДО()
	
	ВозвращаемоеЗначение = Новый Структура;
	ВозвращаемоеЗначение.Вставить("ЗначениеШтрихкода");
	ВозвращаемоеЗначение.Вставить("ИсточникДанных");
...
// Записать соответствие номенклатуры сырья.
// 
// Параметры:
//  ШтрихкодыУпаковок - см. НоваяТаблицаШтрихкодыУпаковок
Процедура ЗаписатьСоответствиеНоменклатурыСырьяПоЭДО(ШтрихкодыУпаковок) Экспорт
	...
	Если ВидыПродукцииСырье.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ВидыПродукцииСырье", ВидыПродукцииСырье);
	Запрос.УстановитьПараметр("Источник",           ШтрихкодыУпаковок);
	СписокЗапросов.Добавить(
		"ВЫБРАТЬ
		|	Источник.Номенклатура      КАК Номенклатура,
		|	Источник.Характеристика    КАК Характеристика,
		|	Источник.ИсточникДанных    КАК ИсточникДанных,
		|	Источник.ЗначениеШтрихкода КАК ЗначениеШтрихкода
		|ПОМЕСТИТЬ Т
		|ИЗ
	...
	СписокЗапросов.Добавить(
		...
		|	ЕСТЬNULL(КлассификаторСырьяГИСМТ.Ссылка, ЗНАЧЕНИЕ(Справочник.КлассификаторСырьяГИСМТ.ПустаяСсылка)) КАК Сырье,
		...
		|	ПО ОписаниеНоменклатурыИС.Номенклатура = Т.Номенклатура
		|	И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииСырье)
		|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлассификаторСырьяГИСМТ КАК КлассификаторСырьяГИСМТ
		|	ПО КлассификаторСырьяГИСМТ.Идентификатор = Т.ЗначениеШтрихкода
		|	И НЕ КлассификаторСырьяГИСМТ.ПометкаУдаления
		|ГДЕ
		|	Т.ЗначениеШтрихкода <> """"
		|	И Т.ИсточникДанных = ""ИдентификаторУпаковки""");
	СписокЗапросов.Добавить(
		"ВЫБРАТЬ
		|	Источник.Номенклатура      КАК Номенклатура,
		|	Источник.Характеристика    КАК Характеристика,
		|	Источник.Сырье             КАК Сырье
		|ИЗ
		|	Источник КАК Источник
		|ГДЕ
		|	Источник.Сырье <> ЗНАЧЕНИЕ(Справочник.КлассификаторСырьяГИСМТ.ПустаяСсылка)",
	...
	//@skip-warning
	ТаблицаСопоставления = РезультатЗапроса["ТаблицаСопоставления"].Выгрузить();
	//@skip-warning
	СоздатьСырье = РезультатЗапроса["СоздатьСырье"].Выгрузить();
	
	Если ТаблицаСопоставления.Количество() = 0 И СоздатьСырье.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеПрефиксов = Неопределено;
	
	МаксимальныйИндекс = СоздатьСырье.Количество() - 1;
	Для Счетчик = 0 По МаксимальныйИндекс Цикл
		ТекущийИндекс = МаксимальныйИндекс - Счетчик;
		СтрокаТаблицы = СоздатьСырье[ТекущийИндекс];
		Если ДанныеПрефиксов = Неопределено Тогда
			ДанныеПрефиксов = РазборКодаМаркировкиИССлужебныйКлиентСерверПовтИсп.ПрефиксыВидовПродукцииИдентификационногоКодаСырья();
		КонецЕсли;
		Если ЗначениеЗаполнено(ДанныеПрефиксов.РегулярноеВыражениеВсе)
			И Не СтрПодобнаПоРегулярномуВыражению(СтрокаТаблицы.ЗначениеШтрихкода, ДанныеПрефиксов.РегулярноеВыражениеВсе) Тогда
			СоздатьСырье.Удалить(ТекущийИндекс);
		КонецЕсли;
	КонецЦикла;
	
	Справочники.КлассификаторСырьяГИСМТ.СоздатьСырьеПоНоменклатуре(СоздатьСырье);
	
	Для Каждого СтрокаТаблицы Из СоздатьСырье Цикл
		НоваяСтрока = ТаблицаСопоставления.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
	КонецЦикла;
	
	РегистрыСведений.СоответствиеНоменклатурыСырьяГИСМТ.ЗаписатьСоответствие(Неопределено, ТаблицаСопоставления);
	
КонецПроцедуры

Процедура ДобавитьСтрокуНоменклатуреСырьяДляПроверкиКодаЕдиницы(СтрокиСырье, СтрокаМаркируемойПродукции, ПараметрыСканирования)
	
	Если ПараметрыСканирования = Неопределено
		Или Не (ПараметрыСканирования.ВидОперацииИСМП = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ВидОперацииОптоваяПродажа()
			Или ПараметрыСканирования.ВидОперацииИСМП = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ВидОперацииОптовыйВозврат()) Тогда
		Возврат;
	КонецЕсли;
	
	СтрокиПоНоменклатуре = СтрокиСырье[СтрокаМаркируемойПродукции.Номенклатура];
	Если СтрокиПоНоменклатуре = Неопределено Тогда
		СтрокиПоНоменклатуре = Новый Массив;
		СтрокиСырье[СтрокаМаркируемойПродукции.Номенклатура] = СтрокиПоНоменклатуре;
	КонецЕсли;
	
	СтрокиПоНоменклатуре.Добавить(СтрокаМаркируемойПродукции);
	
КонецПроцедуры

Процедура ПроверитьКодыЕдиницИзмеренияСырья(СтрокиСырье, ТаблицаТоваров, ЕстьОшибки, ПараметрыСканирования)
	
	Если СтрокиСырье.Количество() Тогда
		
		Если ТаблицаТоваров.Колонки.Найти("ЕдиницаИзмеренияКод") = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		Для Каждого КлючИЗначение Из СтрокиСырье Цикл
			
			Номенклатура        = КлючИЗначение.Ключ;
			МаркируемаяПродукция = КлючИЗначение.Значение;
			СтруктураПоиска = Новый Структура;
			СтруктураПоиска.Вставить("Номенклатура", Номенклатура);
			
			ПоискСтрок = ТаблицаТоваров.НайтиСтроки(СтруктураПоиска);
			
			Для Каждого СтрокаТаблицы Из ПоискСтрок Цикл
				
				КодОКЕИ = СокрЛП(СтрокаТаблицы.ЕдиницаИзмеренияКод);
				Для Каждого СтрокаМаркируемойПродукции Из МаркируемаяПродукция Цикл
					
					Если Не КонструкторИСМПКлиентСервер.СырьеПоддерживаетКодОКЕИ(
						СтрокаМаркируемойПродукции.ВидПродукции, КодОКЕИ, ПараметрыСканирования.ВидОперацииИСМП) Тогда
					
						ЕстьОшибки = Истина;
						СтрокаМаркируемойПродукции.ТекстОшибкиИС = СтрШаблон(
							НСтр("ru = 'Код единицы измерения по ОКЕИ ""%1"" для номенклатуры ""%2"" не поддерживается в системе прослеживаемости фармацевтического сырья ГИС МТ'"),
							КодОКЕИ,
							ОбщегоНазначенияИС.ПредставлениеНоменклатуры(СтрокаМаркируемойПродукции.Номенклатура, СтрокаМаркируемойПродукции.Характеристика));
					КонецЕсли;
