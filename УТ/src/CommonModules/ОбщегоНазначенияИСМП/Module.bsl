...
// 
// Возвращаемое значение:
//  ФиксированныйМассив из СправочникСсылка.ВидыПродукцииИС - Виды продукции обязательной маркировки
...
Функция ВестиУчетМаркируемойПродукции(ВидМаркируемойПродукции) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию(ИмяФункциональнойОпцииВестиУчет) Тогда
		...
		Возврат УчитываемыеВидыПродукции.Найти(ВидМаркируемойПродукции) <> Неопределено;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Функция ВывестиHTTPЗапросВЛог(HTTPЗапрос, ПараметрыОтправкиHTTPЗапросов, HTTPМетод, ПутьКФайлу = Неопределено, ТекстОшибки = "") Экспорт
	...
	ОбщегоНазначенияИС.ПроверитьРасположениеФайлаВоВременномКаталоге(ПутьКФайлу);
	Если ПутьКФайлу <> Неопределено Тогда
...
Функция СортированнаяТаблицаЛокальныхМодулейЧестныйЗнак(Организация, РабочееМесто = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|	НастройкиПодключенияЛокальныхМодулейИСМП.ФорматОбмена              КАК ФорматОбмена,
...
Функция ТребуетсяОбновлениеСпискаCDNПлощадок() Экспорт
	...
	Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииОбязательнойОнлайнПроверкиПередРозничнойПродажейНаДату().Количество() = 0 Тогда
		Возврат Ложь;
	КонецЕсли;
...
// Длительная операция получения товарных групп по организации. При необходимости загружает классификатор видов продукции ИС.
// 
// Параметры:
//  ПараметрыФоновогоЗадания - Структура - Параметры запуска фонового задания
//  АдресРезультата - Строка - адрес возврата результата
Процедура ТоварныеГруппыПоОрганизацииДлительнаяОперация(ПараметрыФоновогоЗадания, АдресРезультата) Экспорт
	...
	Если Не ОбщегоНазначенияИС.КлассификаторНастроекВидовПродукцийЗагружен() Тогда
		ОшибкаЗагрузкиКлассификатора = ОбщегоНазначенияИС.ЗагрузитьКлассификаторНастроекВидовПродукций();
	КонецЕсли;
	...
	РезультатЗапроса = ИнтерфейсИСМПОбщегоНазначения.ЗапроситьСтатусРегистрацииУчастникаИТоварныеГруппы(
		ПараметрыФоновогоЗадания.Организация,
		РаботаСКонтрагентамиИСВызовСервера.ИННКПППоОрганизацииКонтрагенту(ПараметрыФоновогоЗадания.Организация).ИНН);
	...
	ВозвращаемоеЗначение.ТоварныеГруппы                 = РезультатЗапроса.ТоварныеГруппы;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ТоварныеГруппыПоОрганизацииДлительнаяОперация", Истина);
	Методы.Вставить("УстановитьПараметрыСеанса");
	Методы.Вставить("ВыполнитьПолучениеКонфигурацииЛокальногоМодуляЧЗДлительнаяОперация", Истина);
	Методы.Вставить("ВыполнитьПроверкуНастройкиТСПИоТДлительнаяОперация", Истина);
	Методы.Вставить("ВыполнитьНастройкуТоварныхГруппЛокальногоМодуляЧЗДлительнаяОперация", Истина);
	Методы.Вставить("ВыполнитьИнициализациюКонфигурацииЛокальногоМодуляЧЗДлительнаяОперация", Истина);
	Методы.Вставить("ВыполнитьЗапросСтатусаЛокальногоМодуляЧЗДлительнаяОперация", Истина);
	Методы.Вставить("ВыполнитьСменуПароляАдминистрированияЛокальногоМодуляЧЗДлительнаяОперация", Истина);
	Методы.Вставить("ВыполнитьАктуализациюCDNПлощадокДлительнаяОперация", Истина);
...
Функция КонструкторПодключенияЛокальногоМодуляЧестныйЗнак() Экспорт
	...
	СтруктураЛокальногоМодуля.Вставить("ФорматОбмена",              Перечисления.ВерсииФорматаОбменаЛокальногоМодуляИСМП.ПустаяСсылка());
...
Функция МинимальныеЦеныТабачнойПродукции(ШтрихкодыУпаковок, ПараметрыСканирования) Экспорт
	...
	ВидПродукцииТабак           = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииКоторымДоступноПолеМРЦ();
			...
			Если МаксимальнаяРозничнаяЦена <> Неопределено И МаксимальнаяРозничнаяЦена > 0 Тогда
				...
				Если ДанныеРазбора.ВидУпаковки <> Перечисления.ВидыУпаковокИС.Потребительская Тогда
					...
					Если EANКодаМаркировки <> Неопределено Тогда
						...
						ДанныеПоШтрихкодамEAN = ДанныеПоШтрихкодам.ДанныеПоШтрихкодамEAN;
						
						СтрокаДанных = ДанныеПоШтрихкодамEAN.Найти(EANКодаМаркировки, "ШтрихкодEAN");
...
Процедура ЗаполнитьПоСтатистике(Поля, ДанныеЗаполнения, ДанныеПоследнихДокументов, ПоляОтбора = "ТоварнаяГруппа") Экспорт
...
Процедура УточнитьВидПродукцииПоТНВЭДОКПД(ВидПродукцииГИСМТ, ДанныеДляУточнения, ДанныеКеша) Экспорт
	
	Если Не ЗначениеЗаполнено(ВидПродукцииГИСМТ)
		Или ДанныеДляУточнения = Неопределено
		Или Не ИнтерфейсИСМПОбщегоНазначенияКлиентСервер.ЕстьДанныеДляУточненияВидаПродукции(ДанныеДляУточнения) Тогда
		Возврат;
	КонецЕсли;
	
	Если ДанныеКеша.КэшТНВЭДОКПД = Неопределено Тогда
		
		ДанныеКеша.УчитываемыеВидыПродукцииИСМП = ОбщегоНазначения.СкопироватьРекурсивно(
			ОбщегоНазначенияИСМПКлиентСерверПовтИсп.УчитываемыеВидыМаркируемойПродукции(), Ложь);
		
		ДанныеКеша.КэшТНВЭДОКПД = Новый ТаблицаЗначений();
		ДанныеКеша.КэшТНВЭДОКПД.Колонки.Добавить("ТНВЭД",        ОбщегоНазначения.ОписаниеТипаСтрока(50));
		ДанныеКеша.КэшТНВЭДОКПД.Колонки.Добавить("ОКПД2",        ОбщегоНазначения.ОписаниеТипаСтрока(50));
		ДанныеКеша.КэшТНВЭДОКПД.Колонки.Добавить("ВидПродукции", Новый ОписаниеТипов("СправочникСсылка.ВидыПродукцииИС"));
		
		ДанныеКеша.КэшТНВЭДОКПД.Индексы.Добавить("ТНВЭД,ОКПД2");
		
	КонецЕсли;
	
	ВсеВидыПродукцииТоварнойГруппы = ДанныеКеша.ВсеВидыПродукцииТоварнойГруппы[ВидПродукцииГИСМТ];
	Если ВсеВидыПродукцииТоварнойГруппы = Неопределено Тогда
		ТоварнаяГруппа = КонструкторИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппаПоВидуПродукции(
			ВидПродукцииГИСМТ);
		ВсеВидыПродукцииТоварнойГруппы = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(
			ТоварнаяГруппа);
		ДанныеКеша.ВсеВидыПродукцииТоварнойГруппы[ВидПродукцииГИСМТ] = ВсеВидыПродукцииТоварнойГруппы;
	КонецЕсли;
	
	Если ВсеВидыПродукцииТоварнойГруппы.Количество() < 2 Тогда
		Возврат;
	КонецЕсли;
	
	ВидПродукцииДоУточнения = Неопределено;
	
	СтруктураПоиска = Новый Структура;
	СтруктураПоиска.Вставить("ТНВЭД");
	СтруктураПоиска.Вставить("ОКПД2");
	ЗаполнитьЗначенияСвойств(СтруктураПоиска, ДанныеДляУточнения);
	НайденныйСтроки = ДанныеКеша.КэшТНВЭДОКПД.НайтиСтроки(СтруктураПоиска);
	Если НайденныйСтроки.Количество() Тогда
		Если ЗначениеЗаполнено(НайденныйСтроки[0].ВидПродукции) Тогда
			ВидПродукцииГИСМТ = НайденныйСтроки[0].ВидПродукции;
		КонецЕсли;
		Возврат; // Поиск закончен, если вид продукции уже определялся ранее
	КонецЕсли;
	
	НоваяСтрока = ДанныеКеша.КэшТНВЭДОКПД.Добавить();
	ЗаполнитьЗначенияСвойств(НоваяСтрока, СтруктураПоиска);
	УточненныеВидыПродукции = ИнтерфейсИСМПОбщегоНазначенияВызовСервера.ВидыПродукцииПоДаннымУточнения(ДанныеДляУточнения);
	
	Если УточненныеВидыПродукции.Количество() <> 1
		Или ВсеВидыПродукцииТоварнойГруппы.Найти(УточненныеВидыПродукции[0]) = Неопределено Тогда
		Возврат; // Поиск закончен, если нет однозначного выбора
	КонецЕсли;
	
	Если УточненныеВидыПродукции[0] <> ВидПродукцииГИСМТ Тогда
		ВидПродукцииДоУточнения = ВидПродукцииГИСМТ;
	КонецЕсли;
	
	ВидПродукцииГИСМТ        = УточненныеВидыПродукции[0];
	НоваяСтрока.ВидПродукции = ВидПродукцииГИСМТ;
	
	Если ДанныеКеша.УчитываемыеВидыПродукцииИСМП.Найти(НоваяСтрока.ВидПродукции) <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	// Включаем ведения учета по уточненному виду продукции
	...
	СтрокаДанных.ВидПродукции = НоваяСтрока.ВидПродукции;
	СтрокаДанных.ВидПродукцииВерхнегоУровня = Истина;
	ПараметрыЗаписи.Данные.Добавить(СтрокаДанных);
	РезультатЗаписи = АдминистрированиеИСМП.ЗаписатьДанныеМаркируемойПродукции(
		ПараметрыЗаписи);
	...
	Запрос.Текст =
		...
		|	ЗначенияПользователяЭлементовКонструктораВидовПродукцииИС.Значение,
		|	ЗначенияПользователяЭлементовКонструктораВидовПродукцииИС.Атрибуты,
		|	ЗначенияПользователяЭлементовКонструктораВидовПродукцииИС.ПометкаУдаления
		|ИЗ
		|	РегистрСведений.ЗначенияПользователяЭлементовКонструктораВидовПродукцииИС КАК
		|		ЗначенияПользователяЭлементовКонструктораВидовПродукцииИС
		|ГДЕ
		|	ЗначенияПользователяЭлементовКонструктораВидовПродукцииИС.ВидПродукции = &ВидПродукции
		|	И ЗначенияПользователяЭлементовКонструктораВидовПродукцииИС.ЭлементКонструктора В (&ЭлементыКонструктора)
		|ИТОГИ
		|ПО
		|	ЭлементКонструктора";
	...
	Запрос.УстановитьПараметр("ВидПродукции",         ВидПродукцииДоУточнения);
	
	РезультатЗапроса = Запрос.Выполнить();
	
	ВыборкаЭлементКонструктора = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	Пока ВыборкаЭлементКонструктора.Следующий() Цикл
		
		ВыборкаДетальныеЗаписи = ВыборкаЭлементКонструктора.Выбрать();
		
		НаборЗаписей = РегистрыСведений.ЗначенияПользователяЭлементовКонструктораВидовПродукцииИС.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.ЭлементКонструктора.Установить(ВыборкаЭлементКонструктора.ЭлементКонструктора);
		НаборЗаписей.Отбор.ВидПродукции.Установить(ВидПродукцииГИСМТ);
	...
	ОбновитьПовторноИспользуемыеЗначения();
	ДанныеКеша.УчитываемыеВидыПродукцииИСМП.Добавить(НоваяСтрока.ВидПродукции);
