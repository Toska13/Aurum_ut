...
//   ПараметрыВыполнения - см. ПодключаемыеКомандыКлиент.ПараметрыВыполненияКоманды
//
Процедура ПереходПраваСобственностиНаОснованииЗаказа(МассивСсылок, ПараметрыВыполнения) Экспорт
	
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.ЗамерВремени(
		"ОбщийМодуль.СозданиеНаОснованииУТКлиент.ПереходПраваСобственностиНаОснованииЗаказа",, Истина);
	
	ПараметрыВыполненияКоманды = ПараметрыВыполненияКоманды();
	ЗаполнитьЗначенияСвойств(ПараметрыВыполненияКоманды,
		ПараметрыВыполнения.ОписаниеКоманды.ДополнительныеПараметры);
	
	ОчиститьСообщения();
	
	ПараметрыОткрытия = СозданиеНаОснованииУТВызовСервера.ПереходПраваСобственностиПараметрыОткрытияФормы(
		МассивСсылок);
...
//   ПараметрыВыполнения - см. ПодключаемыеКомандыКлиент.ПараметрыВыполненияКоманды
//
Процедура ОтгрузкаТоваровКлиентуСоздатьНаОсновании(МассивСсылок, ПараметрыВыполнения) Экспорт
	
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.ЗамерВремени(
		"ОбщийМодуль.СозданиеНаОснованииУТКлиент.ОтгрузкаТоваровКлиентуСоздатьНаОсновании",, Истина);
	...
	ПараметрыОткрытия = СозданиеНаОснованииУТВызовСервера.ОтгрузкаТоваровКлиентуПараметрыОткрытияФормы(
		МассивСсылок);
	
	Если Не ПараметрыОткрытия = Неопределено Тогда
	
		ОткрытьФорму(
			"Документ.ОтгрузкаТоваровКлиенту.Форма.ФормаДокумента",
			ПараметрыОткрытия,
			ПараметрыВыполненияКоманды.Источник,
			ПараметрыВыполненияКоманды.Уникальность,
			ПараметрыВыполненияКоманды.Окно,
...
//   ПараметрыВыполнения - см. ПодключаемыеКомандыКлиент.ПараметрыВыполненияКоманды
//
Процедура СоздатьЗапросКоммерческогоПредложения(МассивСсылок, ПараметрыВыполнения) Экспорт
	...
	Если ТипЗнч(ПараметрКоманды) = Тип("ДокументСсылка.ЗаявкаНаОбеспечение")
		Или ТипЗнч(ПараметрКоманды) = Тип("ДокументСсылка.ЗаявкаНаЗакупку") Тогда
		
		ОткрытьФорму(ПараметрыВыполнения.ОписаниеКоманды.ДополнительныеПараметры.ИмяФормы,
			Новый Структура("Основание", МассивСсылок),
	...
	Иначе
		...
		СоздатьОбеспечивающийЗаказ(МассивСсылок, ПараметрыВыполнения);
...
//   ПараметрыВыполнения - см. ПодключаемыеКомандыКлиент.ПараметрыВыполненияКоманды
//
Процедура СоздатьСписаниеНедостачТоваровНаОснованииПоГруппеСкладов(МассивСсылок, ПараметрыВыполнения) Экспорт
	...
	ПараметрыСоздания = СозданиеНаОснованииУТВызовСервера.СоздатьСписаниеНедостачТоваровНаОснованииПоГруппеСкладовПараметрыСоздания(ПараметрКоманды);
	
	Если ПараметрыСоздания.НесколькоСкладов Тогда
		
		ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения(
			"СоздатьСписаниеНедостачТоваровНаОснованииПоГруппеСкладовПослеВыбораСклада",
	...
	Иначе
		...
		СоздатьСписаниеНедостачТоваровНаОснованииПоГруппеСкладовПослеВыбораСклада(
			ПараметрыСоздания.СписокСкладов[0], ПараметрыВыполнения);
...
//   ПараметрыВыполнения - см. ПодключаемыеКомандыКлиент.ПараметрыВыполненияКоманды
//
Процедура СоздатьПорчуТоваровНаОснованииПоГруппеСкладов(МассивСсылок, ПараметрыВыполнения) Экспорт
	...
	ПараметрыСоздания = СозданиеНаОснованииУТВызовСервера.СоздатьПорчуТоваровНаОснованииПоГруппеСкладовПараметрыСоздания(ПараметрКоманды);
	
	Если ПараметрыСоздания.НесколькоСкладов Тогда
		
		ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения(
			"СоздатьПорчуТоваровНаОснованииПоГруппеСкладовПослеВыбораСклада",
			СозданиеНаОснованииУТКлиент, ПараметрыВыполнения);
...
//   ПараметрыВыполнения - см. ПодключаемыеКомандыКлиент.ПараметрыВыполненияКоманды
//
Процедура СоздатьВозвратТоваровПоставщикуНаОснованииПоГруппеСкладов(МассивСсылок, ПараметрыВыполнения) Экспорт
	...
	ПараметрыСоздания = СозданиеНаОснованииУТВызовСервера.СоздатьВозвратТоваровПоставщикуНаОснованииПоГруппеСкладовПараметрыСоздания(ПараметрКоманды);
	
	Если ПараметрыСоздания.НесколькоСкладов Тогда
		
		ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения(
			"СоздатьВозвратТоваровПоставщикуНаОснованииПоГруппеСкладовПослеВыбораСклада",
			СозданиеНаОснованииУТКлиент, ПараметрыВыполнения);
...
//   ПараметрыВыполнения - см. ПодключаемыеКомандыКлиент.ПараметрыВыполненияКоманды
//
Процедура СоздатьОтгрузкуТоваровСХраненияНаОснованииПоГруппеСкладов(МассивСсылок, ПараметрыВыполнения) Экспорт
	...
	ПараметрыСоздания = СозданиеНаОснованииУТВызовСервера.СоздатьОтгрузкуТоваровСХраненияНаОснованииПоГруппеСкладовПараметрыСоздания(ПараметрКоманды);
	
	Если ПараметрыСоздания.НесколькоСкладов Тогда
		
		ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения(
			"СоздатьОтгрузкуТоваровСХраненияНаОснованииПоГруппеСкладовПослеВыбораСклада",
			СозданиеНаОснованииУТКлиент, ПараметрыВыполнения);
...
Процедура СоздатьЗаказНаПеремещениеНаОснованииПоГруппеСкладовПослеВыбораСклада(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ДополнительныеПараметры.МассивСсылок) = Тип("Массив")
				И ДополнительныеПараметры.МассивСсылок.Количество() = 0
...
Процедура СоздатьСписаниеНедостачТоваровНаОснованииПоГруппеСкладовПослеВыбораСклада(Результат, ДополнительныеПараметры) Экспорт
	...
	ОткрытьФорму("Документ.СписаниеНедостачТоваров.ФормаОбъекта",
		ПараметрыОткрытия,
		ДополнительныеПараметры.Форма,
		Новый УникальныйИдентификатор);
...
Процедура СоздатьПорчуТоваровНаОснованииПоГруппеСкладовПослеВыбораСклада(Результат, ДополнительныеПараметры) Экспорт
	...
	ОткрытьФорму("Документ.ПорчаТоваров.ФормаОбъекта",
		ПараметрыОткрытия,
		ДополнительныеПараметры.Форма,
		Новый УникальныйИдентификатор);
...
Процедура СоздатьВозвратТоваровПоставщикуНаОснованииПоГруппеСкладовПослеВыбораСклада(Результат, ДополнительныеПараметры) Экспорт
...
Процедура СоздатьОтгрузкуТоваровСХраненияНаОснованииПоГруппеСкладовПослеВыбораСклада(Результат, ДополнительныеПараметры) Экспорт
	...
	ОткрытьФорму("Документ.ОтгрузкаТоваровСХранения.ФормаОбъекта",
		ПараметрыОткрытия,
		ДополнительныеПараметры.Форма,
		Новый УникальныйИдентификатор);
...
// Функция-конструктор параметров выполнения команды
//
// Возвращаемое значение:
//  Структура:
//		* Источник 				- Произвольный - форма или элемент управления другой формы
...
Функция ПараметрыВыполненияКоманды()
	...
	ПараметрыВыполненияКоманды.Вставить("Источник", Неопределено);
	...
	Возврат ПараметрыВыполненияКоманды;
