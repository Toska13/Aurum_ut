...
Функция ПолучитьДанныеДляПечатнойФормыУКД(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	"ВЫБРАТЬ
	|	ТаблицаДанныхДокументов.Ссылка								КАК Ссылка,
	|	ДанныеПредыдущегоДокумента.Ссылка							КАК ИсходныйДокумент,
	|	ЕСТЬNULL(ДанныеПредыдущегоДокумента.Номер, НЕОПРЕДЕЛЕНО)	КАК НомерСчетаФактуры,
	|	ЕСТЬNULL(ДанныеПредыдущегоДокумента.Дата, НЕОПРЕДЕЛЕНО)		КАК ДатаСчетаФактуры,
	|	НЕОПРЕДЕЛЕНО												КАК НомерИсправленияСчетаФактуры,
	|	НЕОПРЕДЕЛЕНО												КАК ДатаИсправленияСчетаФактуры
	|ИЗ
	...
	|ПО
	|	ТаблицаДанныхДокументов.Ссылка = ПредыдущиеДокументы.Основной
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	Документ.ОтгрузкаТоваровКлиенту КАК ДанныеПредыдущегоДокумента
	|ПО
	|	ПредыдущиеДокументы.Предыдущий = ДанныеПредыдущегоДокумента.Ссылка
	|ИТОГИ ПО
	|	Ссылка
	|;
	|
	...
	|	ЕСТЬNULL(ТаблицаТоваровПредыдущие.Количество, 0)                       КАК КоличествоБазовыхЕдиницДо
	|ИЗ
	|	ОтгрузкаТоваровКлиентуТаблицаТоваров КАК ТаблицаТоваровПредыдущие
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	ОтгрузкаТоваровКлиентуТаблицаТоваров КАК ТаблицаТоваровПослеКорректировки
	|ПО
