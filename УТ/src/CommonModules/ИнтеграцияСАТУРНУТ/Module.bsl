...
Процедура ЗаполнитьТабличнуюЧастьДокумента(ТабличнаяЧасть, РезультатЗапроса, ДанныеЗаполнения, ДоступнаПустаяТаблица = Ложь, ДополнительныеПараметры = Неопределено)
	...
	ЗаполнятьСкладИзТабличнойЧасти                 = Ложь;
	ОрганизацияДляЗаполненияСкладаИзТабличнойЧасти = Неопределено;
	
	ЗаполнятьИдентификаторСтроки             = Ложь;
	ИдентификаторДляЗаполненияВСтроке        = Неопределено;
	ИмяПоляИдентификаторДляЗаполненияВСтроке = "";
	
	ДетализироватьДанныеДоНомераГруппыЗатрат       = Ложь;
	ТаблицаСДетализациейДоГруппЗатрат              = Неопределено;
	ТаблицаДляДетализацииСырьяДоНомераГруппыЗатрат = Неопределено;
	
	СоответствиеИдентификаторов = Неопределено;
	
	Если ДополнительныеПараметры <> Неопределено Тогда
		
		Если ДополнительныеПараметры.Свойство("ЗаполнитьСкладИзТабличнойЧасти") Тогда
			
			ЗаполнятьСкладИзТабличнойЧасти                 = Истина;
			ОрганизацияДляЗаполненияСкладаИзТабличнойЧасти = ДополнительныеПараметры.Организация;
			
		КонецЕсли;
		
		Если ДополнительныеПараметры.Свойство("ЗаполнитьИдентификаторСтроки") Тогда
			
			ЗаполнятьИдентификаторСтроки             = Истина;
			ИмяПоляИдентификаторДляЗаполненияВСтроке = "Идентификатор";
			
			Если ТипЗнч(ДополнительныеПараметры.ЗаполнитьИдентификаторСтроки) = Тип("Структура") Тогда
				
				Если ДополнительныеПараметры.ЗаполнитьИдентификаторСтроки.Свойство("ИдентификаторЗначение")
					И ЗначениеЗаполнено(ДополнительныеПараметры.ЗаполнитьИдентификаторСтроки.ИдентификаторЗначение) Тогда
					ИдентификаторДляЗаполненияВСтроке = ДополнительныеПараметры.ЗаполнитьИдентификаторСтроки.ИдентификаторЗначение;
				КонецЕсли;
				
				Если ДополнительныеПараметры.ЗаполнитьИдентификаторСтроки.Свойство("ИмяПоляИдентификатор")
					И ЗначениеЗаполнено(ДополнительныеПараметры.ЗаполнитьИдентификаторСтроки.ИмяПоляИдентификатор) Тогда
					ИмяПоляИдентификаторДляЗаполненияВСтроке = ДополнительныеПараметры.ЗаполнитьИдентификаторСтроки.ИмяПоляИдентификатор;
				КонецЕсли;
		...
		Если ДополнительныеПараметры.Свойство("ДетализацияДоНомераГруппыЗатрат") Тогда
			
			Если ДополнительныеПараметры.ДетализацияДоНомераГруппыЗатрат.Свойство("ТаблицаСДетализациейДоГруппЗатрат") Тогда
				
				ДетализироватьДанныеДоНомераГруппыЗатрат = Истина;
				ТаблицаСДетализациейДоГруппЗатрат        = ДополнительныеПараметры.ДетализацияДоНомераГруппыЗатрат.ТаблицаСДетализациейДоГруппЗатрат;
				
			ИначеЕсли ДополнительныеПараметры.ДетализацияДоНомераГруппыЗатрат.Свойство("ТаблицаДляЗаполненияСырьяСИдентификаторами") Тогда
				
				ДетализироватьДанныеДоНомераГруппыЗатрат       = Истина;
				ТаблицаДляДетализацииСырьяДоНомераГруппыЗатрат = ДополнительныеПараметры.ДетализацияДоНомераГруппыЗатрат.ТаблицаДляЗаполненияСырьяСИдентификаторами;
				
			КонецЕсли;
			
			Если ДополнительныеПараметры.Свойство("СоответствиеИдентификаторов") Тогда
				СоответствиеИдентификаторов = ДополнительныеПараметры.СоответствиеИдентификаторов;
			Иначе
				ДополнительныеПараметры.Вставить("СоответствиеИдентификаторов", Новый Соответствие());
				СоответствиеИдентификаторов = ДополнительныеПараметры.СоответствиеИдентификаторов;
			КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗаполнятьСкладИзТабличнойЧасти И НоваяСтрока.НомерСтроки = 1 Тогда
			
			ПараметрыЗаполнения = Новый Структура("Организация, Склад, Подразделение",
				ОрганизацияДляЗаполненияСкладаИзТабличнойЧасти,
				Выборка.Склад,
		...
		Если ДетализироватьДанныеДоНомераГруппыЗатрат И ТаблицаСДетализациейДоГруппЗатрат <> Неопределено Тогда
			
			СтруктураПоиска = Новый Структура("Номенклатура, Характеристика, Серия");
			ЗаполнитьЗначенияСвойств(СтруктураПоиска, НоваяСтрока);
			
			СтрокиСДетализациейЗатрат = ТаблицаСДетализациейДоГруппЗатрат.НайтиСтроки(СтруктураПоиска);
			
			Если СтрокиСДетализациейЗатрат.Количество() Тогда
				НоваяСтрока.Идентификатор = Новый УникальныйИдентификатор();
				СоответствиеИдентификаторов.Вставить(НоваяСтрока.Идентификатор, СтрокиСДетализациейЗатрат[0].НомерГруппыЗатрат);
			КонецЕсли;
			
		КонецЕсли;
		
		Если ДетализироватьДанныеДоНомераГруппыЗатрат И ТаблицаДляДетализацииСырьяДоНомераГруппыЗатрат <> Неопределено Тогда
			
			Если ЗначениеЗаполнено(НоваяСтрока.ИдентификаторСтрокиТоваров) Тогда
				Продолжить;
			КонецЕсли;
			...
			СтрокиСырья = ТаблицаДляДетализацииСырьяДоНомераГруппыЗатрат.НайтиСтроки(СтруктураОтбора);
			
			Если СтрокиСырья.Количество() = 1 Тогда
				НоваяСтрока[ИмяПоляИдентификаторДляЗаполненияВСтроке] = СтрокиСырья[0].НомерГруппыЗатрат;
			Иначе
				
				Для Каждого СтрокаСырья Из СтрокиСырья Цикл
					
					Если НоваяСтрока.Количество < СтрокаСырья.Количество Тогда
						Продолжить;
					КонецЕсли;
					
					НоваяСтрока[ИмяПоляИдентификаторДляЗаполненияВСтроке] = СтрокаСырья.НомерГруппыЗатрат;
		...
		Если ЗаполнятьИдентификаторСтроки Тогда
			...
			Если ЗначениеЗаполнено(НоваяСтрока[ИмяПоляИдентификаторДляЗаполненияВСтроке]) Тогда
				Продолжить;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(ИдентификаторДляЗаполненияВСтроке) Тогда
				ИдентификаторДляЗаполненияВСтроке = Строка(Новый УникальныйИдентификатор());
			КонецЕсли;
			
			НоваяСтрока[ИмяПоляИдентификаторДляЗаполненияВСтроке] = ИдентификаторДляЗаполненияВСтроке;
...
Процедура ЗаполнитьТабличнуюЧастьДокументаПроизводственнаяОперацияСАТУРН(ТабличнаяЧасть, РезультатЗапроса, ДанныеЗаполнения, ДоступнаПустаяТаблица = Ложь, ДополнительныеПараметры = Неопределено)
	...
	ИдентификаторДляЗаполненияВСтроке        = Неопределено;
	ИмяПоляИдентификаторДляЗаполненияВСтроке = "Идентификатор";
	НеЗаполнятьИдентификаторСтроки           = Ложь;
	...
	Если ДополнительныеПараметры <> Неопределено Тогда
		...
		Если ДополнительныеПараметры.Свойство("ИмяПоляИдентификатор") Тогда
			ИмяПоляИдентификаторДляЗаполненияВСтроке = ДополнительныеПараметры.ИмяПоляИдентификатор;
		КонецЕсли;
		...
		Если ДополнительныеПараметры.Свойство("ДетализацияДоНомераГруппыЗатрат") Тогда
			...
			Если ДополнительныеПараметры.ДетализацияДоНомераГруппыЗатрат.Свойство("ТаблицаДляЗаполненияСырьяСИдентификаторами") Тогда
		...
		Если ДополнительныеПараметры.Свойство("НеЗаполнятьИдентификатор") Тогда
			НеЗаполнятьИдентификаторСтроки = ДополнительныеПараметры.НеЗаполнятьИдентификатор;
		КонецЕсли;
		
		Если ДополнительныеПараметры.Свойство("ИдентификаторЗначение") Тогда
			ИдентификаторДляЗаполненияВСтроке = ДополнительныеПараметры.ИдентификаторЗначение;
		КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДетализироватьДанныеДоНомераГруппыЗатрат И ТаблицаСДетализациейДоГруппЗатрат <> Неопределено Тогда
			...
			Если СтрокиСДетализациейЗатрат.Количество() Тогда
				НоваяСтрока.Идентификатор = Новый УникальныйИдентификатор();
				СоответствиеИдентификаторов.Вставить(СтрокиСДетализациейЗатрат[0].НомерГруппыЗатрат, НоваяСтрока.Идентификатор);
			КонецЕсли;
		...
		Если ДетализироватьДанныеДоНомераГруппыЗатрат И ТаблицаДляДетализацииСырьяДоНомераГруппыЗатрат <> Неопределено Тогда
			...
			Если СтрокиСырья.Количество() Тогда
				
				ИдентификаторПродукции = СоответствиеИдентификаторов.Получить(СтрокиСырья[0].НомерГруппыЗатрат);
				НоваяСтрока[ИмяПоляИдентификаторДляЗаполненияВСтроке] = ИдентификаторПродукции;
		...
		Если НеЗаполнятьИдентификаторСтроки Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьОрганизациюМестоХраненияСАТУРНПоРеквизитамДокумента(ДокументОбъект, РеквизитыДокумента, ИспользоватьПроизводственныйОбъект = Ложь, ЗаполнитьГрузоотправителя = Ложь)
	...
	Если ЗаполнитьГрузоотправителя Тогда
		СтрокаСопоставленияГрузоотправителя = ИнтеграцияСАТУРН.ДобавитьВТаблицуОтбораСопоставлений(
			ТаблицаСопоставлений,
	...
	Если ЗаполнитьГрузоотправителя И ЗначениеЗаполнено(СтрокаСопоставленияГрузоотправителя.ОрганизацияСАТУРН) Тогда
		ДокументОбъект.ГрузоотправительОрганизацияСАТУРН = СтрокаСопоставленияГрузоотправителя.ОрганизацияСАТУРН;
	КонецЕсли;
...
Функция ТекстТоварыДокументаБезСерий(ИмяДокумента, ИмяТЧТовары = ".Товары")
	...
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииСАТУРН В (&ВидыПродукции)
	|ГДЕ
...
Функция ТекстТоварыДокументаССериями(ИмяДокумента, ИмяТЧТовары = ".Товары", ИмяТЧСерии = ".Серии")
	...
	|		ПО ТаблицаТовары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|ГДЕ
	|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииСАТУРН В (&ВидыПродукции)
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаТовары.Номенклатура,
	|	ТаблицаТовары.Характеристика,
...
Функция ТекстЗапросаЗаполненияПродукцииПроизводстваИзПрикладногоДокументаСДетализациейДоНомераГруппыЗатрат(ТекстТоварыДокумента)
	...
	|	ТоварыСерии.Характеристика       КАК Характеристика,
	|	ТоварыСерии.Серия                КАК Серия,
	//|	ТоварыСерии.НомерГруппыЗатрат    КАК НомерГруппыЗатрат,
	|	ТоварыСерии.Количество           КАК Количество
	|ПОМЕСТИТЬ ТоварыКОформлению
	|ИЗ
	|	ТоварыСерии КАК ТоварыСерии
	...
	|	ОформленныеТовары.Характеристика КАК Характеристика,
	|	ОформленныеТовары.Серия          КАК Серия,
	//|	ОформленныеТовары.Идентификатор  КАК Идентификатор,
	|	-ОформленныеТовары.Количество    КАК Количество
	|ИЗ
	|	Документ.ПроизводственнаяОперацияСАТУРН.Товары КАК ОформленныеТовары
	|ГДЕ
	...
	|	ТоварыКОформлению.Характеристика                  КАК Характеристика,
	|	ТоварыКОформлению.Серия                           КАК Серия,
	//|	ТоварыКОформлению.НомерГруппыЗатрат               КАК Идентификатор,
	|	СопоставленныеПозиции.ПАТ                         КАК ПАТ,
	|	СУММА(ТоварыКОформлению.Количество)               КАК Количество,
	|	СУММА(ТоварыКОформлению.Количество)               КАК КоличествоУпаковок,
	|	МАКСИМУМ(ТоварыКОформлению.Номенклатура.Артикул)            КАК АртикулПроизводителя,
	...
	|	СопоставленныеПозиции.ПАТ
	//|	СопоставленныеПозиции.ПАТ,
	//|	ТоварыКОформлению.НомерГруппыЗатрат
	|ИМЕЮЩИЕ
	|	СУММА(ТоварыКОформлению.Количество) > 0
	|";
...
Процедура ЗаполнитьТабличнуюЧастьТоварыПроизводственнойОперации(ДокументОбъект, ДанныеЗаполнения, ТекстТоварыДокумента, ДополнительныеПараметры = Неопределено)
	...
	Запрос.УстановитьПараметр("КонечныеСтатусыАктПрименения", Документы.АктПримененияСАТУРН.КонечныеСтатусы());
	...
	Запрос.УстановитьПараметр("ВидыПродукции",                КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииСАТУРН());
	
	Если ДополнительныеПараметры <> Неопределено
		И ДополнительныеПараметры.Свойство("ДетализацияДоНомераГруппыЗатрат") Тогда
		
		Запрос.Текст = ТекстЗапросаЗаполненияПродукцииПроизводстваИзПрикладногоДокументаСДетализациейДоНомераГруппыЗатрат(ТекстТоварыДокумента);
		РезультатЗапроса = Запрос.ВыполнитьПакет();
		
		РезультатЗапросаТовары = РезультатЗапроса[РезультатЗапроса.Количество() - 1];
		РезультатЗапросаНомераГруппЗатрат = РезультатЗапроса[РезультатЗапроса.Количество() - 2];
		
		ДополнительныеПараметры.ДетализацияДоНомераГруппыЗатрат.Вставить("ТаблицаСДетализациейДоГруппЗатрат", РезультатЗапросаНомераГруппЗатрат.Выгрузить());
	...
	ЗаполнитьТабличнуюЧастьДокументаПроизводственнаяОперацияСАТУРН(ДокументОбъект.Товары, РезультатЗапросаТовары, ДанныеЗаполнения,, ДополнительныеПараметры);
...
Функция ТекстЗапросаТаблицаЗаполненияСырьяСИдентификаторами()
...
Процедура ЗаполнитьТабличнуюЧастьСырьеПроизводственнойОперации(ДокументОбъект, ДанныеЗаполнения, ТекстТоварыДокумента, ДополнительныеПараметры = Неопределено)
	...
	ДетализацияДоНомераГруппыЗатрат = Ложь;
	Если Не ДополнительныеПараметры = Неопределено И ДополнительныеПараметры.Свойство("ДетализацияДоНомераГруппыЗатрат") Тогда
		ДетализацияДоНомераГруппыЗатрат = ДополнительныеПараметры.ДетализацияДоНомераГруппыЗатрат;
	КонецЕсли;
	
	Если ДетализацияДоНомераГруппыЗатрат Тогда
		Запрос.Текст = Запрос.Текст + ТекстЗапросаТаблицаЗаполненияСырьяСИдентификаторами();
	КонецЕсли;
	...
	Если ДетализацияДоНомераГруппыЗатрат Тогда
		...
		РезультатЗапроса = Запрос.ВыполнитьПакет();
		РезультатТовары  = РезультатЗапроса[РезультатЗапроса.Количество() - 2];
		
		ДополнительныеПараметры.ДетализацияДоНомераГруппыЗатрат.Вставить("ТаблицаДляЗаполненияСырьяСИдентификаторами", РезультатЗапроса[РезультатЗапроса.Количество() - 1].Выгрузить());
		ДополнительныеПараметры.ДетализацияДоНомераГруппыЗатрат.Удалить("ТаблицаСДетализациейДоГруппЗатрат");
		
	Иначе
		РезультатЗапроса = Запрос.Выполнить();
		РезультатТовары  = РезультатЗапроса;
	КонецЕсли;
	
	ЗаполнитьТабличнуюЧастьДокументаПроизводственнаяОперацияСАТУРН(ДокументОбъект.Сырье, РезультатТовары, ДанныеЗаполнения, Истина, ДополнительныеПараметры);
...
Процедура ЗаполнитьПроизводственнуюОперациюСАТУРННаОснованииСборкиТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ДополнительныеПараметрыЗаполненияСырья = Новый Структура();
	ДополнительныеПараметрыЗаполненияСырья.Вставить("ИмяПоляИдентификатор", "ИдентификаторСтрокиТоваров");
	
	Если РеквизитыДокумента.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СборкаТоваров Тогда
		
		ИдентификаторСтрокиТоваров = "";
		Если ДокументОбъект.Товары.Количество() Тогда
			ИдентификаторСтрокиТоваров = ДокументОбъект.Товары[0].Идентификатор;
		КонецЕсли;
		
		ДополнительныеПараметрыЗаполненияСырья.Вставить("ИдентификаторЗначение", ИдентификаторСтрокиТоваров);
		
	Иначе
		ДополнительныеПараметрыЗаполненияСырья.Вставить("НеЗаполнятьИдентификатор", Истина);
	КонецЕсли;
	
	ЗаполнитьТабличнуюЧастьСырьеПроизводственнойОперации(ДокументОбъект, ДанныеЗаполнения, ТекстТоварыДокумента, ДополнительныеПараметрыЗаполненияСырья);
...
Процедура ОбработкаЗаполненияДокументаАктИнвентаризацииСАТУРН(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ОприходованиеИзлишковОтгруженныхТоваров")Тогда
		ЗаполнитьАктИнвентаризацииСАТУРННаОснованииОприходованияИзлишковОтгруженныхТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.СписаниеОтгруженныхТоваров")Тогда
		ЗаполнитьАктИнвентаризацииСАТУРННаОснованииСписанияОтгруженныхТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
...
Процедура ЗаполнитьАктИнвентаризацииСАТУРННаОснованииОприходованияИзлишковОтгруженныхТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	""Оприходование излишков товаров"" КАК ПричинаИнвентаризации
	|ИЗ
	|	Документ.ОприходованиеИзлишковОтгруженныхТоваров КАК ДокументСсылка
	|ГДЕ
	|	ДокументСсылка.Ссылка = &ДокументОснование");
	
	ЗаполнитьШапкуАктаИнвентаризации(ДокументОбъект, ДанныеЗаполнения, Запрос);
	
	ТекстТоварыДокумента = ТекстТоварыДокументаБезСерий("ОприходованиеИзлишковОтгруженныхТоваров");
	ЗаполнитьТабличнуюЧастьАктаИнвентаризации(ДокументОбъект, ДанныеЗаполнения, ТекстТоварыДокумента, 1);
...
Процедура ЗаполнитьАктИнвентаризацииСАТУРННаОснованииСписанияОтгруженныхТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	""Списание недостач товаров"" КАК ПричинаИнвентаризации
	|ИЗ
	|	Документ.СписаниеОтгруженныхТоваров КАК ДокументСсылка
	|ГДЕ
	|	ДокументСсылка.Ссылка = &ДокументОснование");
	...
	|ПОМЕСТИТЬ ТоварыСерии
	|ИЗ
	|	Документ.СписаниеОтгруженныхТоваров.Товары КАК Товары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО Товары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|ГДЕ
	|	Товары.Ссылка = &ДокументОснование
...
Процедура ОбработкаЗаполненияДокументаНакладнаяСАТУРН(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту")Тогда
		ЗаполнитьНакладнуюСАТУРННаОснованииОтгрузкиТоваровКлиенту(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
...
Процедура ЗаполнитьТабличнуюЧастьНакладной(ДокументОбъект, ДанныеЗаполнения, ТекстТоварыДокумента)
	...
	Запрос.УстановитьПараметр("ПустаяСерия",                      Метаданные.ОпределяемыеТипы.СерияНоменклатуры.Тип.ПривестиЗначение());
	Запрос.УстановитьПараметр("ВидыПродукции",                    КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииСАТУРН());
	Запрос.УстановитьПараметр("ВидыПродукцииПартионнаяПродукция", КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииСАТУРН());
...
Функция ТекстЗапросаЗаполненияНакладнойИзПрикладногоДокумента(ТекстТоварыДокумента)
	...
	|			ПО ТабличнаяЧасть.Номенклатура.КодТНВЭД = КлассификаторТНВЭД.Ссылка
	|СГРУППИРОВАТЬ ПО
	|	ТабличнаяЧасть.Номенклатура,
	|	ТабличнаяЧасть.Характеристика,
	|	ТабличнаяЧасть.Серия,
	...
	|	ТоварыКОформлению.Серия                           КАК Серия,
	|	СопоставленныеПозиции.ПАТ                         КАК ПАТ,
	|	СопоставленныеПозиции.КодТНВЭД                    КАК КодТНВЭД,
	|	СУММА(ТоварыКОформлению.Количество)               КАК Количество,
	|	СУММА(ТоварыКОформлению.Количество)               КАК КоличествоУпаковок
	|ИЗ
	|	ТоварыКОформлению КАК ТоварыКОформлению
	...
	|	ТоварыКОформлению.Серия,
	|	СопоставленныеПозиции.ПАТ,
	|	СопоставленныеПозиции.КодТНВЭД
	|ИМЕЮЩИЕ СУММА(ТоварыКОформлению.Количество)>0
	|";
...
Процедура ЗаполнитьНакладнуюСАТУРННаОснованииОтгрузкиТоваровКлиенту(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ТекстТоварыДокумента = ИнтеграцияИСУТ.ТекстЗапросаОтгрузкаТоваровКлиенту() + "
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаТовары.Номенклатура      КАК Номенклатура,
	...
	|ПОМЕСТИТЬ ТоварыСерии
	|ИЗ
	|	ТаблицаТоварыОтгрузкаТоваровКлиенту КАК ТаблицаТовары
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаТовары.Номенклатура,
	|	ТаблицаТовары.Характеристика,
	|	ТаблицаТовары.Серия
...
Процедура ОбработкаЗаполненияДокументаИмпортПродукцииСАТУРН(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		ЗаполнитьИмпортПродукцииСАТУРННаОснованииЗаказаПоставщику(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьТабличнуюЧастьИмпортПродукцииСАТУРН(ДокументОбъект, ДанныеЗаполнения, ТекстТоварыДокумента, ДополнительныеПараметры = Неопределено)
	
	ДокументОбъект.Товары.Очистить();
	
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаЗаполненияИмпортПродукцииСАТУРНИзПрикладногоДокумента(ТекстТоварыДокумента);
	
	Запрос.УстановитьПараметр("ЭтаСсылка",         ДокументОбъект.Ссылка);
	Запрос.УстановитьПараметр("ДокументОснование", ДанныеЗаполнения);
	Запрос.УстановитьПараметр("Документ",          ДанныеЗаполнения);
	Запрос.УстановитьПараметр("КонечныеСтатусы",   Документы.ИмпортПродукцииСАТУРН.КонечныеСтатусы());
	Запрос.УстановитьПараметр("ПустаяСерия",       Метаданные.ОпределяемыеТипы.СерияНоменклатуры.Тип.ПривестиЗначение());
	Запрос.УстановитьПараметр("ВидыПродукции",     КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииСАТУРН());
...
Процедура ЗаполнитьШапкуИмпортаПродукции(ДокументОбъект, ДанныеЗаполнения, Запрос, ДополнительныеПараметры = Неопределено)
	...
	Если Не ЭтоПерезаполнение
		...
		ЗаполнитьЗначенияСвойств(ДокументОбъект, РеквизитыДокумента);
		ЗаполнитьОрганизациюМестоХраненияСАТУРНПоРеквизитамДокумента(ДокументОбъект, РеквизитыДокумента,, Истина);
		
	КонецЕсли;
	
КонецПроцедуры

Функция ТекстЗапросаЗаполненияИмпортПродукцииСАТУРНИзПрикладногоДокумента(ТекстТоварыДокумента)
	...
	|ПОМЕСТИТЬ ОформленныеДокументы
	|ИЗ
	|	Документ.ИмпортПродукцииСАТУРН КАК ТаблицаДокументы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыДокументовСАТУРН КАК СтатусыДокументовСАТУРН
	|		ПО СтатусыДокументовСАТУРН.Документ = ТаблицаДокументы.Ссылка
	|ГДЕ
	|	ТаблицаДокументы.ДокументОснование = &ДокументОснование
	...
	|ПОМЕСТИТЬ ОформленныеТоварыПоДокументамСАТУРН
	|ИЗ
	|	Документ.ИмпортПродукцииСАТУРН.Товары КАК ОформленныеТовары
	|ГДЕ
	|	ОформленныеТовары.Ссылка В (ВЫБРАТЬ Т.Ссылка ИЗ ОформленныеДокументы КАК Т)
	|
	|;
	...
	|			И СопоставленныеПозиции.Серия = ТоварыКОформлению.Серия
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлассификаторТНВЭД КАК КлассификаторТНВЭД
	|			ПО ТоварыКОформлению.Номенклатура.КодТНВЭД = КлассификаторТНВЭД.Ссылка
	|СГРУППИРОВАТЬ ПО
	|	ТоварыКОформлению.Подразделение,
	|	ТоварыКОформлению.Склад,
	|	ТоварыКОформлению.Номенклатура,
...
Процедура ЗаполнитьИмпортПродукцииСАТУРННаОснованииПриобретениеТоваровУслуг(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	ДокументСсылка.Склад         КАК Склад,
	|	ДокументСсылка.Контрагент    КАК Контрагент,
	|	СтраныСАТУРН.Ссылка          КАК СтранаЭкспортер,
	|	ВЫБОР
	|		КОГДА
	|			ДокументСсылка.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС),
	...
	|		ТОГДА
	|			ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСАТУРН.ИмпортПродукцииИзЕАЭС)
	|		КОГДА
	|			ДокументСсылка.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаПоИмпорту),
	...
	|		ТОГДА
	|			ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСАТУРН.ИмпортПродукцииПредварительноеУведомление)
	|		ИНАЧЕ
	|			ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСАТУРН.ИмпортПродукции)
	|		КОНЕЦ
	|	КАК Операция
	|ИЗ
	|	Документ.ПриобретениеТоваровУслуг КАК ДокументСсылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлассификаторСтранСАТУРН КАК СтраныСАТУРН
	|			ПО ДокументСсылка.Контрагент.СтранаРегистрации.КодАльфа2 = СтраныСАТУРН.КодАльфа2
	|ГДЕ
	|	ДокументСсылка.Ссылка = &ДокументОснование");
	
	ЗаполнитьШапкуИмпортаПродукции(ДокументОбъект, ДанныеЗаполнения, Запрос);
	
	ТекстТоварыДокумента = ТекстТоварыДокументаССериями("ПриобретениеТоваровУслуг");
	ЗаполнитьТабличнуюЧастьИмпортПродукцииСАТУРН(ДокументОбъект, ДанныеЗаполнения, ТекстТоварыДокумента);
	
КонецПроцедуры

Процедура ЗаполнитьИмпортПродукцииСАТУРННаОснованииЗаказаПоставщику(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
...
Функция ДанныеПрикладныхДокументовИзИмпортПродукцииСАТУРН(ДокументСсылка, ПолучитьШапку = Истина, ПолучитьТовары = Истина)
	...
	Если ПолучитьШапку Тогда
		...
		|	ВЫБОР
		|		КОГДА ИмпортПродукцииСАТУРН.Операция = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСАТУРН.ИмпортПродукцииИзЕАЭС)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС)
		|		КОГДА ИмпортПродукцииСАТУРН.Операция = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСАТУРН.ИмпортПродукцииПредварительноеУведомление)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаПоИмпорту)
		|	КОНЕЦ КАК ХозяйственнаяОперация,
		|	ИмпортПродукцииСАТУРН.ГрузоотправительОрганизацияСАТУРН.Контрагент КАК Контрагент,
		|	Контрагенты.Партнер                                                КАК Партнер
		|ИЗ
		|	Документ.ИмпортПродукцииСАТУРН КАК ИмпортПродукцииСАТУРН
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Контрагенты
		|			ПО ИмпортПродукцииСАТУРН.ГрузоотправительОрганизацияСАТУРН.Контрагент = Контрагенты.Ссылка
		|ГДЕ
...
Процедура ЗаполнитьВозвратТоваровОтКлиентаНаОснованииНакладнаяСАТУРН(ДокументОбъект, ДанныеЗаполнения) Экспорт
	...
	ЗаполнитьТабличныеЧастиПрикладногоДокументаНаОснованииНакладнаяСАТУРН(
		ДокументОбъект,
		СтруктураЗаполненияТабличныхЧастейПоНакладнаяСАТУРН(РезультатЗапроса.Товары, Истина, Истина), ДанныеЗаполнения);
...
Процедура ЗаполнитьЗаказПоставщикуНаОснованииИмпортПродукцииСАТУРН(ДокументОбъект, ДанныеЗаполнения) Экспорт
	...
	ЗаполнитьТабличныеЧастиПрикладногоДокументаНаОснованииИмпортПродукцииСАТУРН(
		ДокументОбъект,
		СтруктураЗаполненияТабличныхЧастейПоИмпортПродукцииСАТУРН(РезультатЗапроса.Товары), ДанныеЗаполнения);
	
	Параметры = ЗакупкиСервер.ДополнительныеПараметрыОтбораДоговоров();
	Параметры.ВалютаВзаиморасчетов = ДокументОбъект.Валюта;
	
	ДокументОбъект.Договор = ЗакупкиСервер.ПолучитьДоговорПоУмолчанию(ДокументОбъект, ДокументОбъект.ХозяйственнаяОперация, Параметры);
	
	ЗакупкиВызовСервера.ЗаполнитьБанковскиеСчетаПоДоговору(ДокументОбъект.Договор, ДокументОбъект.БанковскийСчет);
	...
	СтруктураПараметров.НаправлениеДеятельности = ДокументОбъект.НаправлениеДеятельности;
	
	ДокументОбъект.БанковскийСчет = ДенежныеСредстваСервер.ПолучитьБанковскийСчетОрганизацииПоУмолчанию(
		СтруктураПараметров, ДокументОбъект.БанковскийСчет, ДокументОбъект.ФормаОплаты);
	
	НоменклатураПартнеровСервер.ЗаполнитьНоменклатуруПартнераПоНоменклатуреПриИзмененииПартнера(ДокументОбъект.Товары, ДокументОбъект.Партнер);
