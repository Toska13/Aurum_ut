...
Функция ПереводыСБПДоступны() Экспорт
	...
	Возврат ПравоДоступа("Изменение", Метаданные.РегистрыСведений.ИдентификаторыОперацийСБПc2b)
		...
		И СистемаБыстрыхПлатежейСлужебный.ДобавлениеПлатежныхСсылокДоступно();
...
Функция НастройкиПодключенияОперации(ДокументОперации) Экспорт
	...
	ИмяФункции = "ПереводыСБПc2b.НастройкиПодключенияОперации";
	...
	Если ИспользоватьЧастичныеОплаты
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ПлатежнаяСсылкаСБП.Ссылка КАК ДокументОпераций
...
//
Функция НастройкиВозвратаОплаты(
		ОбъектОплаты,
		НастройкаПодключения) Экспорт
	
	// Проверка входных параметров
	ИмяФункции = "ПереводыСБПc2b.НастройкиВозвратаОплаты";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяФункции,
	...
	ИначеЕсли ТипЗнч(ОбъектОплаты) = Тип("Строка") Тогда
		...
		ПереводыСБПc2bСервис.ПроверитьИдентификаторОбъектаОплаты(ДанныеСтроки, РезультатОперации);
		
		Если ЗначениеЗаполнено(РезультатОперации.КодОшибки) Тогда
			ВызватьИсключение РезультатОперации.СообщениеОбОшибке;
		КонецЕсли;
		
		ИдентификаторУчастника = РегистрыСведений.ИдентификаторыОперацийСБПc2b.ИдентификаторУчастникаПоИдентификаторамОперации(
			СистемаБыстрыхПлатежейСлужебный.НовыйИдентификаторыОперацииСБП(
				ДанныеСтроки[1],
				ДанныеСтроки[2]));
		
		Если ИдентификаторУчастника = Неопределено Тогда
			ПараметрыПодключения = СистемаБыстрыхПлатежейСлужебный.ПараметрыНастройкиПодключения(НастройкаПодключения);
			ИдентификаторУчастника = ПараметрыПодключения.ИдентификаторУчастника;
		КонецЕсли;
	...
	Если ИдентификаторУчастника = Неопределено Тогда
		ДанныеНастройки = Справочники.НастройкиПодключенияКСистемеБыстрыхПлатежей.ДанныеНастройкиПодключения(
			НастройкаПодключения);
		ИдентификаторУчастника = ДанныеНастройки.ИдентификаторУчастника
	КонецЕсли;
...
Функция ДинамическаяСсылка(
	...
	// Проверка входных параметров
	ИмяФункции = "ПереводыСБПc2b.ДинамическаяСсылка";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяФункции,
		"ДокументОперации",
		ДокументОперации,
...
Функция КассоваяСсылка(НастройкаПодключения) Экспорт
	...
	// Проверка входных параметров
	ИмяФункции = "ПереводыСБПc2b.КассоваяСсылка";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяФункции,
		"НастройкаПодключения",
		НастройкаПодключения,
...
Функция АктивироватьКассовуюСсылку(
	...
	// Проверка входных параметров
	ИмяФункции = "ПереводыСБПc2b.АктивироватьКассовуюСсылку";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяФункции,
		"ДокументОперации",
		ДокументОперации,
	...
	Возврат ПереводыСБПc2bСервис.АктивироватьКассовуюСсылку(
		ДокументОперации,
		ДанныеСсылки,
		ПараметрыНастройкиПодключения,
		ДополнительныеПараметры);
...
Функция ДеактивироватьКассовуюСсылку(
	...
	// Проверка входных параметров
	ИмяФункции = "ПереводыСБПc2b.ДеактивироватьКассовуюСсылку";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяФункции,
		"НастройкаПодключения",
		НастройкаПодключения,
	...
	Возврат ПереводыСБПc2bСервис.ДеактивироватьКассовуюСсылку(
		ПараметрыНастройкиПодключения,
		ДанныеСсылки);
...
Функция КарточкаКассовойСсылки(
	...
	// Проверка входных параметров
	ИмяФункции = "ПереводыСБПc2b.КарточкаКассовойСсылки";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяФункции,
		"ДанныеСсылки",
		ДанныеСсылки,
	...
	Если ПараметрыПечати.ТипПечати = 1 Тогда
		Макет = СистемаБыстрыхПлатежейСлужебный.ПолучитьМакетСБП("CashLinkWide");
	ИначеЕсли ПараметрыПечати.ТипПечати = 2 Тогда
		Макет = СистемаБыстрыхПлатежейСлужебный.ПолучитьМакетСБП("CashLinkLogo");
	ИначеЕсли ПараметрыПечати.ТипПечати = 3 Тогда
		Макет = СистемаБыстрыхПлатежейСлужебный.ПолучитьМакетСБП("CashLinkNarrow");
	ИначеЕсли ПараметрыПечати.ТипПечати = 4 Тогда
		Макет = СистемаБыстрыхПлатежейСлужебный.ПолучитьМакетСБП("CashLinkNarrowHorizontal");
	ИначеЕсли ПараметрыПечати.ТипПечати = 5 Тогда
		Макет = СистемаБыстрыхПлатежейСлужебный.ПолучитьМакетСБП("CashLinkSquare");
	ИначеЕсли ПараметрыПечати.ТипПечати = 6 Тогда
		Макет = СистемаБыстрыхПлатежейСлужебный.ПолучитьМакетСБП("CashLinkCircle");
	Иначе
		Возврат ТабличныйДокумент;
	КонецЕсли;
	...
	ДанныеQRКода = НастройкиПлатежныхСистем.ИзображениеQRКода(
...
Функция СтатусОплаты(
	...
	// Проверка входных параметров
	ИмяФункции = "ПереводыСБПc2b.СтатусОплаты";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяФункции,
		"ДокументОперации",
		ДокументОперации,
	...
	РезультатОперации = ПереводыСБПc2bСервис.СтатусОплаты(
		ДокументОперации,
		ПараметрыНастройкиПодключения,
		ДлительныйВызов);
...
//        ** НомерСчета,  - Строка, Неопределено - маскированный номер счета, на который будут
//             переведены денежные средства;
//        ** НомерТелефона - Строка, Неопределено - Строка - маскированный номер телефона,
//             к которому привязан;
...
Функция ВозвратОплаты(
	...
	// Проверка входных параметров
	ИмяФункции = "ПереводыСБПc2b.ВозвратОплаты";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяФункции,
		"ДокументВозврата",
		ДокументВозврата,
	...
	РезультатОперации = ПереводыСБПc2bСервис.ВозвратОплаты(
		ДокументВозврата,
		ОбъектОплаты,
		УчастникСБП,
		ПараметрыНастройкиПодключения,
...
Функция ПодтвердитьВозврат(
		ДокументВозврата,
		НастройкаПодключения = Неопределено) Экспорт
	
	// Проверка входных параметров
	ИмяФункции = "ПереводыСБПc2b.ПодтвердитьВозврат";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяФункции,
		"ДокументВозврата",
		ДокументВозврата,
	...
	Возврат ПереводыСБПc2bСервис.ПодтвердитьВозврат(
		ДокументВозврата,
		ПараметрыНастройкиПодключения);
...
Функция СтатусВозврата(
	...
	// Проверка входных параметров
	ИмяФункции = "ПереводыСБПc2b.СтатусВозврата";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяФункции,
		"ДокументВозврата",
		ДокументВозврата,
	...
	РезультатОперации = ПереводыСБПc2bСервис.СтатусВозврата(
		ДокументВозврата,
		ПараметрыНастройкиПодключения,
		ДлительныйВызов);
...
Процедура УдалитьИдентификаторыОперации(ДокументОперации) Экспорт
	...
	// Проверка входных параметров
	ИмяФункции = "ПереводыСБПc2b.УдалитьИдентификаторыОперации";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяФункции,
		"ДокументОперации",
		ДокументОперации,
	...
	Попытка
		...
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ДанныеОперацийСБПc2b");
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru = 'Не удалось удалить данные операции (с2b) СБП.'");
...
Функция ОперацииПоДокументу(ДокументОперации) Экспорт
	...
	// Проверка входных параметров
	ИмяФункции = "ПереводыСБПc2b.ОперацииПоДокументу";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяФункции,
		"ДокументОперации",
		ДокументОперации,
...
Функция УстановитьОтложенноеПолучениеСтатуса(ДокументОперации, Значение) Экспорт
	...
	// Проверка входных параметров
	ИмяФункции = "ПереводыСБПc2b.УстановитьОтложенноеПолучениеСтатуса";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяФункции,
		"ДокументОперации",
		ДокументОперации,
...
Функция ИзображениеQRКодаВозврата(ДокументОперации, Размер) Экспорт
	...
	// Проверка входных параметров
	ИмяФункции = "ПереводыСБПc2b.ИзображениеQRКодаВозврата";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяФункции,
		"ДокументОперации",
		ДокументОперации,
	...
	Возврат НастройкиПлатежныхСистем.ИзображениеQRКода(
...
Функция ИдентификаторыОперацииОплаты(ДокументОперации) Экспорт
	...
	// Проверка входных параметров
	ИмяФункции = "ПереводыСБПc2b.ИдентификаторыОперацииОплаты";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяФункции,
		"ДокументОперации",
		ДокументОперации,
...
Функция ПлатежныеСсылкиПоДокументамОпераций(
	...
	// Проверка входных параметров
	ИмяФункции = "ПереводыСБПc2b.ПлатежныеСсылкиПоДокументамОпераций";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяФункции,
		"ДокументыОпераций",
		ДокументыОпераций,
...
Функция ИдентификаторыОперации(ПараметрыОперации) Экспорт
	...
	Возврат СистемаБыстрыхПлатежейСлужебный.ИдентификаторСБП()
		+ ":"
...
Функция ИдентификаторФункциональнойСсылки(ДокументОперации) Экспорт
	...
	// Проверка входных параметров
	ИмяФункции = "ПереводыСБПc2b.ИдентификаторФункциональнойСсылки";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяФункции,
		"ДокументОперации",
		ДокументОперации,
...
Функция СтатусыОпераций() Экспорт
	...
	Возврат НастройкиПлатежныхСистем.СтатусыОпераций("СБП").Получить("СБП");
