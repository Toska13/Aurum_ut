...
//  ПараметрыПереводаИзТакском - Неопределено 
//  						- см. СинхронизацияЭДО.ПараметрыПереводаИзТакском
//
// Возвращаемое значение:
//  Структура:
...
Функция ДанныеДляСинхронизации(Знач ОтпечаткиСертификатов, КонтекстДиагностики, Знач УчетныеЗаписи = Неопределено,
	Знач ВыбранныеСертификаты = Неопределено, ПараметрыПереводаИзТакском = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТекущийИдентификаторОрганизации <> Выборка.ИдентификаторОрганизации Тогда
			...
			Если ЗначениеЗаполнено(ПараметрыПереводаИзТакском) Тогда
				СпособОбменаДляПодмены = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыПереводаИзТакском,
					"СпособОбменаДляПодмены", Перечисления.СпособыОбменаЭД.ПустаяСсылка());
				Если ЗначениеЗаполнено(СпособОбменаДляПодмены) Тогда
					КлючСинхронизации.СпособОбмена = СпособОбменаДляПодмены;
				КонецЕсли;
			КонецЕсли;
...
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
//  ОбрабатыватьТолькоОбъектыAPIТакском - Булево
// 
// Возвращаемое значение:
//  См. СинхронизацияЭДОКлиентСервер.НовыеКлючиСинхронизации
Функция ДанныеДляСинхронизацииИзРасшифрованныхМаркеров(РасшифрованныеМаркеры, КонтекстДиагностики, ОбрабатыватьТолькоОбъектыAPIТакском = Ложь) Экспорт
	...
	Для Каждого РасшифрованныйМаркер Из РасшифрованныеМаркеры Цикл
		...
		Если ОбрабатыватьТолькоОбъектыAPIТакском Тогда
			КлючСинхронизации.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском;
		Иначе	
...
Процедура ПолучитьНовыеЭлектронныеДокументыЧерезОператораЭДО(
	...
	СкорректироватьДатыПолученияУчетныхЗаписейПереведенныхИзТакскомВХаб(ДатыПолучения, КлючиСинхронизации);
	...
	Для Каждого КлючИЗначение Из КлючиСинхронизации Цикл
		...
		Если УспешноеПолучениеТранспортныхКонтейнеров И ПараметрыПолучения.СдвигатьДату
			...
			ИзменитьСостояниеОбмена(КлючСинхронизации, "ДатаПолученияДокументов",
				ДатаПолученияЭД);
		КонецЕсли;
...
//  Свойство - Строка
//  Значение - Число
Процедура ИзменитьСостояниеОбмена(КлючСинхронизации, Свойство, Значение)
	...
	Если КлючСинхронизации.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском 
		И УчетныеЗаписиЭДО.УчетнаяЗаписьПереведенаИзТакском(ИдентификаторУчетнойЗаписи) Тогда
			
		Набор = РегистрыСведений.УчетныеЗаписиПереведенныеИзТакскомЭДО.СоздатьНаборЗаписей();
		Набор.Отбор.ИдентификаторЭДО.Установить(ИдентификаторУчетнойЗаписи);
		Набор.Прочитать();

		СтрокаНабора = Набор[0];
		
		Если Свойство = "ДатаПолученияДокументов" Тогда
			СтрокаНабора.ДатаПолучения = Значение;
		ИначеЕсли Свойство = "ДатаПолученияЭПД" Тогда 
			СтрокаНабора.ДатаПолученияЭПД = Значение;
		ИначеЕсли Свойство = "ПроверочнаяДатаПолученияДокументов" Тогда 
			СтрокаНабора.ПроверочнаяДатаПолучения = Значение;
		КонецЕсли;
		
		Набор.Записать();
		
	Иначе
		СинхронизацияЭДО.ИзменитьСостояниеОбмена(ИдентификаторУчетнойЗаписи, Свойство, Значение);
	КонецЕсли;
		
КонецПроцедуры 

// Параметры:
//  ДатыПолучения - см. СинхронизацияЭДО.ДатыЗапросаДанныхОператораЭДО
//  КлючиСинхронизации - см. СинхронизацияЭДОКлиентСервер.НовыеКлючиСинхронизации
Процедура СкорректироватьДатыПолученияУчетныхЗаписейПереведенныхИзТакскомВХаб(ДатыПолучения, КлючиСинхронизации)
	
	СписокУчетныхЗаписейПереведенныхИзТакском = УчетныеЗаписиЭДО.СписокУчетныхЗаписейПереведенныхИзТакском();
	...
	ИдентификаторыПереведенныхВТакскомУЗ = ОбщегоНазначенияБЭДКлиентСервер.ПересечениеМассивов(ИдентификаторыУчетныхЗаписей, 
		СписокУчетныхЗаписейПереведенныхИзТакском);
	
	СписокИдентификаторовКЗапросуДатПолучения = Новый Массив;
	Для Каждого КлючСинхронизации Из КлючиСинхронизации Цикл
		ИдентификаторЭДО = КлючСинхронизации.Ключ;
		СвойстваКлюча = КлючСинхронизации.Значение;
		Если СвойстваКлюча.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском 
			И ИдентификаторыПереведенныхВТакскомУЗ.Найти(ИдентификаторЭДО) <> Неопределено Тогда
			
			СписокИдентификаторовКЗапросуДатПолучения.Добавить(ИдентификаторЭДО);
		КонецЕсли;
		
	КонецЦикла;
	
	ДатыПолученияПереведенныхУЗ = СинхронизацияЭДО.ДатыПолученияУчетныхЗаписейПереведенныхИзТакскомВХаб(СписокИдентификаторовКЗапросуДатПолучения);
	ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(ДатыПолучения, ДатыПолученияПереведенныхУЗ, Истина);
...
//  КлючСинхронизации - см. СинхронизацияЭДОКлиентСервер.НовыйКлючСинхронизации
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
//  ДатаПолученияЭДИзНастройки - Число
//  ДатаУспешногоПолученияЭД - Число
//  ПроверочнаяДатаПолученияЭДИзНастройки - Число
//  НезагруженныеИдентификаторы - Массив из Строка
Процедура ПроверитьНеполученныеДокументы(КлючСинхронизации, КонтекстДиагностики,
	...
	Если Не ЗначениеЗаполнено(ДатаПолученияЭДИзНастройки) И ЗначениеЗаполнено(ДатаУспешногоПолученияЭД) Тогда
		Если ПроверочнаяДатаПолученияЭДИзНастройки <> ДатаУспешногоПолученияЭД Тогда
			ИзменитьСостояниеОбмена(КлючСинхронизации, "ПроверочнаяДатаПолученияДокументов", ДатаУспешногоПолученияЭД);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Функция ОтправитьТранспортныеКонтейнеры(ТранспортныеКонтейнеры, КлючиСинхронизации, КонтекстДиагностики,
	...
	ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер("ОтправитьТранспортныеКонтейнеры");
	...
	Для Каждого ТранспортныйКонтейнер Из ТранспортныеКонтейнеры Цикл
		...
		КонецЦикла;
	КонецЦикла;
	ОбщегоНазначенияБЭД.ЗакончитьЗамер("ОтправитьТранспортныеКонтейнеры", ОписаниеЗамера,
		ТранспортныеКонтейнеры.Количество());
	Возврат Результат;
...
Процедура ОтправитьЭлектронныеДокументыПоРасписанию() Экспорт
	...
	КлючЗамера = "ОтправитьЭлектронныеДокументыПоРасписанию";
	...
	Попытка
		...
		СписокУчетныхЗаписейПереводимыхИзТакском = УчетныеЗаписиЭДО.СписокУчетныхЗаписейПереведенныхИзТакском();
		Если ЗначениеЗаполнено(СписокУчетныхЗаписейПереводимыхИзТакском) 
			И Не УчетныеЗаписиЭДО.НаступилаДатаОтключенияAPIТакском() Тогда
			
			ДанныеДляСинхронизации = ДанныеДляСинхронизации(ОтпечаткиСертификатов, 
				КонтекстДиагностики, 
				СписокУчетныхЗаписейПереводимыхИзТакском,
				СертификатыПоОтпечаткам(ОтпечаткиСертификатов));
			
			ПодготовитьКонтейнеры(ДанныеДляСинхронизации.КлючиСинхронизации, КонтекстДиагностики);
			РезультатОтправкиТакском = ОтправитьКонтейнеры(ДанныеДляСинхронизации.КлючиСинхронизации, ОтпечаткиСертификатов,
				КонтекстДиагностики, Истина);
			Результат.КоличествоОтправленных = Результат.КоличествоОтправленных + РезультатОтправкиТакском.КоличествоОтправленных;
		КонецЕсли;
	...
	Исключение
		...
		ДиагностикаЭДО.ОбработатьОшибки(КонтекстДиагностики);
	КонецПопытки;
	ОбщегоНазначенияБЭД.ЗакончитьЗамер(КлючЗамера,ОписаниеЗамера, Результат.КоличествоОтправленных);
КонецПроцедуры
...
Процедура ПолучитьНовыеЭлектронныеДокументыПоРасписанию() Экспорт
	...
	КлючЗамера = "ПолучитьНовыеЭлектронныеДокументыПоРасписанию";
	ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер(КлючЗамера);
	...
	Попытка
		...
		УчетныеЗаписиЭДО.ПеревестиУчетныеЗаписиТакскомВХаб(ДанныеДляСинхронизации.КлючиСинхронизации);
		...
		Если ЗначениеЗаполнено(СписокУчетныхЗаписейПереводимыхИзТакском) 
			...
			ДанныеДляСинхронизации = ДанныеДляСинхронизации(ОтпечаткиСертификатов, 
				...
				СписокУчетныхЗаписейПереводимыхИзТакском,
				СертификатыПоОтпечаткам(ОтпечаткиСертификатов),
				ПараметрыПереводаИзТакском);
			
			РезультатЗагрузкиПоТакском = ПолучитьКонтейнеры(ДанныеДляСинхронизации.КлючиСинхронизации, ОтпечаткиСертификатов,
				КонтекстДиагностики);
				
			РезультатЗагрузкиОбработки = ЭлектронныеДокументыЭДО.ЗагрузитьОбработатьДанныеОбъектовКонтейнеров(
				РезультатЗагрузкиПоТакском.ДанныеКонтейнеров, КонтекстДиагностики, ОтпечаткиСертификатов);
					
			Если ЗначениеЗаполнено(РезультатЗагрузкиОбработки.ДокументыДляОбработкиИзвещений) Тогда
				
				ПараметрыВыполнения = ЭлектронныеДокументыЭДОКлиентСервер.НовыеПараметрыВыполненияДействийПоЭДО();
				ПараметрыВыполнения.ВыполнитьТолькоПоТакском = Истина;
				ПараметрыВыполнения.ОбъектыДействий.ЭлектронныеДокументы = ОбщегоНазначения.ВыгрузитьКолонку(
					РезультатЗагрузкиОбработки.ДокументыДляОбработкиИзвещений, "Ключ");
				ПараметрыВыполнения.ОбработатьТолькоИзвещения = Истина;
			...
			ЗагруженоПоТакском = РезультатЗагрузкиОбработки.ИтогДействийПоЭДО.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.Загрузить];
		
			Если ЗагруженоПоТакском = Неопределено Тогда
				ЗагруженоПоТакском = 0;
			КонецЕсли;
			
			Загружено = Загружено + ЗагруженоПоТакском;
			Результат.ПолученоКонтейнеров = Результат.ПолученоКонтейнеров + РезультатЗагрузкиПоТакском.ПолученоКонтейнеров;
		КонецЕсли;
	...
	Исключение
		...
		ДиагностикаЭДО.ОбработатьОшибки(КонтекстДиагностики);
	КонецПопытки;
	ОбщегоНазначенияБЭД.ЗакончитьЗамер(КлючЗамера, ОписаниеЗамера, Загружено);
КонецПроцедуры
...
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
//  РезультатыОтправкиПолучения - Неопределено, См. СинхронизацияЭДО.ОтправитьОбъекты
//  ВыполнитьТолькоПоТакском - Булево
// 
// Возвращаемое значение:
//  Структура - Отправить контейнеры:
...
Функция ОтправитьКонтейнеры(КлючиСинхронизации, ОтпечаткиСертификатов, КонтекстДиагностики,
	РезультатыОтправкиПолучения = Неопределено, ВыполнитьТолькоПоТакском = Ложь)
	...
	ПараметрыДействийПоЭДО.ВыполнитьТолькоПоТакском = ВыполнитьТолькоПоТакском;
	
	РезультатОтправки = ЭлектронныеДокументыЭДО.ОтправитьВсеДокументы(УчетныеЗаписи, ПараметрыДействийПоЭДО,
		КонтекстДиагностики);
...
Функция ПолучитьКонтейнеры(КлючиСинхронизации, ОтпечаткиСертификатов, КонтекстДиагностики,
	...
	ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер("ПолучениеИРаспаковкаТранспортныхКонтейнеров");
	...
	ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("ПолучениеТранспортныхКонтейнеров", ОписаниеЗамера, ПолученоКонтейнеров);
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ЭлектронноеАктированиеЕИС") Тогда
	...
	ОбщегоНазначенияБЭД.ЗакончитьЗамер("ПолучениеИРаспаковкаТранспортныхКонтейнеров", ОписаниеЗамера,
		ПолученоКонтейнеров);
	Возврат Результат;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОтправитьЭлектронныеДокументыПоРасписанию", Истина);
	Методы.Вставить("ПараметрыСинхронизации", Истина);
	Методы.Вставить("ПолучитьНовыеЭлектронныеДокументыПоРасписанию", Истина);
	Методы.Вставить("СформироватьИОтправитьТранспортныеКонтейнеры", Истина);
