...
Функция ИмяСобытияЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru = 'Электронное взаимодействие.Обмен с банками'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗаписатьВЖурналРегистрации(Комментарий, Уровень, Данные = Неопределено, ПостфиксСобытия = "",
	...
	Если ЗначениеЗаполнено(ПостфиксСобытия) Тогда
		ИмяСобытияЖурналаРегистрации = СтрШаблон(НСтр("ru = '%1.%2'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВключитьАвтоматическоеОбновлениеСпискаБанков");
	Методы.Вставить("ОбработатьРегистрПодписываемыеВидыЭД");
	Методы.Вставить("ПеренестиСтатусыЭлектронныхПодписей");
