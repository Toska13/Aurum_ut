...
//
Функция НовыеДанныеДокументаДляВыгрузки() Экспорт
	...
	ДанныеДокументаДляВыгрузки.Вставить("ФорматОсновногоТитула", "");
	ДанныеДокументаДляВыгрузки.Вставить("ИдентификаторКонтрагента", "");
...
Функция СвойстваОбъектовУчетаЭлектронныхДокументовДляВыгрузкиВФНС(ОбъектыУчета = Неопределено) Экспорт
	...
	КонверторТипов.Вставить(Перечисления.ТипыДокументовЭДО.СчетНаОплату, "СчетНаОплату");
	КонверторТипов.Вставить(Перечисления.ТипыДокументовЭДО.СчетОферта, "СчетОферта");
...
Процедура РаспределитьЭлектронныеДокументыПоПакетам(ДанныеДокумента, ПакетыДокументовДляВыгрузки)
	...
	Если ТрансграничныйЭДО.ЭтоИностранныйОператор(ДанныеДокумента.ИдентификаторКонтрагента) Тогда
		ПакетыДокументовДляВыгрузки.НеформализованныеPDF.Добавить(ДанныеДокумента);
		Возврат;
	КонецЕсли;
	
	Если ДанныеДокумента.ТипРегламента = Перечисления.ТипыРегламентовЭДО.Неформализованный
		И ЭлектронныеДокументыЭДО.ВозможнаВыгрузкаТолькоПоФормализованномуРегламенту(ДанныеДокумента.ТипДокумента) Тогда
		
		ОписаниеФайла = РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла();
		ОписаниеФайла.ДвоичныеДанные = РаботаСФайлами.ДвоичныеДанныеФайла(ФайлДокумента, Ложь);
		...
		Если ЗначениеЗаполнено(СодержаниеСообщения) Тогда
			...
			Если СодержаниеСообщения.Формат = ПоддерживаемыеФорматы.CML208
				Или СодержаниеСообщения.Формат = ПоддерживаемыеФорматы.CML402 Тогда
				ШаблонОшибки =
					НСтр("ru = 'В формате CML тип документа ""%1"" не предназначен для предоставления в ФНС.'");
	...
	Для Каждого ДанныеДокумента Из ПакетыДокументовДляВыгрузки.НеформализованныеPDF Цикл
		...
		Если РезультатПроверкиПодписей.Результат Тогда
			...
			Для Каждого ДанныеПодписи Из УстановленныеПодписи Цикл
				...
				ТрансграничныйЭДО.ДополнитьВыгрузкуДляФНС(ДанныеДокумента, ОписьВыгрузки, АдресКаталога, ЭлементОписи);
...
//   * ИмяФайлаПодписи - Строка - Наименование файла подписи информации отправителя
//
Функция НоваяОписьВыгрузкиДокументыPDF() Экспорт
...
Функция СформироватьФайлОписанияВыгрузкиДокументыPDF(Организация, ДанныеДляОписи, ИмяФайла)
	...
	Попытка
		...
		Для Каждого СтрокаОписи Из ДанныеДляОписи Цикл
			...
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(
				ЭлементДокумент, "ИмяФайлаПодписи", СтрокаОписи.ИмяФайлаПодписи, Ложь, Ошибки);
...
Функция КодВидаДокументаБРОПоСвойствамЭДО(ТипРегламента, ТипДокумента, ТипЭлементаРегламента)
	...
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.СчетНаОплату Тогда
		...
		Иначе
			КодВидДокумента = "16";
		КонецЕсли;
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.СчетОферта Тогда	
		Если ТипРегламента = Перечисления.ТипыРегламентовЭДО.Формализованный Тогда
			КодВидДокумента = "17";
		Иначе
			КодВидДокумента = "18";
