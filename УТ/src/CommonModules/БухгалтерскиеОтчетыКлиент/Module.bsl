...
Процедура ОтборОбработатьВыборЗначения(Форма, Элемент, СтандартнаяОбработка, Значение, СписокПараметров, ТипПоля, МножественныйВыбор = Ложь) Экспорт
	...
	ИначеЕсли БухгалтерскийУчетКлиентСерверПереопределяемый.ТипЗначенияКорректировкиПрошлыхПериодов(ТипЭлемента) Тогда
		
		СтандартнаяОбработка = Ложь;
		БухгалтерскийУчетКлиентПереопределяемый.ОткрытьФормуВыбораКорректировкиПрошлыхПериодовНУ(СписокПараметров, ПараметрыФормы, Элемент);
...
Процедура ДополнительныеПоляПередНачаломДобавленияЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ПараметрыВыбранногоПоля) = Тип("Структура") Тогда
		...
		УстановитьСостоянииНеактуальности(Форма);
	КонецЕсли;
...
// Проверяет, что не запущено фоновое задание актуализации и устанавливает состояние табличного документа на форме.
//
// Параметры:
//  Форма        - ФормаКлиентскогоПриложения - анализируемая форма.
//
Процедура УстановитьСостоянииНеактуальности(Форма)
	
	ИспользуетсяОписаниеЗаданияАктуализации = ЗакрытиеМесяцаКлиентСервер.ИспользуетсяОписаниеЗаданияАктуализации(Форма);
	
	Если ИспользуетсяОписаниеЗаданияАктуализации Тогда
		ФормаВСостоянииНеактуальности = Форма.ОписаниеЗаданияАктуализации = Неопределено;
	Иначе
		
		ИспользуетсяОписаниеЗаданияФормированияОтчета = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ОписаниеЗаданияФормированияОтчета");
		
		ФормаВСостоянииНеактуальности = ?(ИспользуетсяОписаниеЗаданияФормированияОтчета, 
