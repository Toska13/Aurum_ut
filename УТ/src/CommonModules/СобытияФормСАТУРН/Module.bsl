...
Процедура ЗаполнениеРеквизитовФормы(Форма) Экспорт
	...
	Если ПараметрыИнтеграции <> Неопределено И ЗначениеЗаполнено(ПараметрыИнтеграции.ИмяРеквизитаФормы) Тогда
		...
		Иначе
			ТекстНадписи = ИнтеграцияСАТУРНВызовСервера.ДанныеПоляИнтеграцииВФормеДокументаОснования(Форма[ИмяРеквизитаФормыОбъект].Ссылка).ТекстНадписи;
...
Процедура ОбновитьСтатусыОформления(Ссылка, ПараметрыИнтеграцииГосИС, РеквизитыФормыСтатусовОформления) Экспорт
	...
	Если ПараметрыИнтеграции <> Неопределено И ЗначениеЗаполнено(ПараметрыИнтеграции.ИмяРеквизитаФормы) Тогда
		ТекстНадписи = ИнтеграцияСАТУРНВызовСервера.ДанныеПоляИнтеграцииВФормеДокументаОснования(Ссылка).ТекстНадписи;
...
Процедура ЗаполнитьГиперссылкиРеквизитов(Форма, ПараметрыПолученияПредставлений, ИмяПоляСопоставленияМестаХранения = "ТорговыйОбъект", ОбязательноеСопоставлениеОрганизации = Ложь, ОбязательноеСопоставлениМестаХранения = Ложь) Экспорт
	
	Объект = Форма.Объект;
	
	ЕстьРеквизитМестаХранения = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "МестоХранения");
	ДанныеПредставлений = ИнтеграцияСАТУРН.ПредставленияСопоставлений(ПараметрыПолученияПредставлений);
	...
	Для Каждого КлючИЗначение Из ДанныеПредставлений Цикл
		...
		Форма["Надпись" + ПараметрПредставления.Префикс + "ОрганизацияСАТУРН"] = ДанныеПредставления.ОрганизацияКонтрагент.Представление;
		Форма["Надпись" + ПараметрПредставления.Префикс + "Грузоотправитель"] = ДанныеПредставления.Грузоотправитель.Представление;
