...
#Область СтандартныеПодсистемы_БазоваяФункциональность
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПолучитьHTMLПредпросмотрОбъектаВФоне", Истина);
...
Функция ОбработатьУниверсальноеСообщение(Сообщение) Экспорт
	...
	Попытка
		...
		Если Результат = Неопределено Тогда
			...
			Если ПроверитьТипБИД(Сообщение, "DMILBatchRequest") Тогда
...
//   МассивВнешнихОбъектовБезОшибок - Массив из ОбъектXDTO - список внешних объектов, которые были успешно обработаны.
//
// Возвращаемое значение:
//   ОбъектXDTO
//
Функция ОписаниеОшибкиXDTO(Предмет = "", Описание = "", МассивВнешнихОбъектовБезОшибок = Неопределено) Экспорт
	
	Ошибка = СоздатьОбъектБИД("DMILError");
	Ошибка.subject = Предмет;
	Ошибка.description = Описание;
	
	Если ТипЗнч(МассивВнешнихОбъектовБезОшибок) = Тип("Массив") Тогда
		СписокВнешнихОбъектовБезОшибок = Ошибка.externalObjectsWithoutErrors; // СписокXDTO
		Для Каждого ВнешнийОбъект Из МассивВнешнихОбъектовБезОшибок Цикл
			СписокВнешнихОбъектовБезОшибок.Добавить(ВнешнийОбъект);
		КонецЦикла;
	КонецЕсли;
...
Функция HTMLПредпросмотрОбъекта(Прокси, ID, Тип, ИгнорироватьМаксРазмер = Ложь,
	...
	Если ОтветНаЗапросыПакета.responses.Количество() = 2 Тогда
		...
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(Прокси, ДанныеДокумента, "DMError") Тогда
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.УстановитьЗначенияВсехПолейНДПС(
				Результат,
				Ложь);
			ИмяСобытия = НСтр("ru = 'Получение настроек доступности по состоянию объекта'");
			ТекстОшибки = Новый Массив;
			ТекстОшибки.Добавить(СтрШаблон(
				НСтр("ru = 'Не удалось получить настройки доступности по состоянию объекта ДО (ID = ""%1"", тип = ""%2"") по причине:'"),
				ОбъектДляПолученияНДПС.ID,
				ОбъектДляПолученияНДПС.Тип));
			ТекстОшибки.Добавить(ДанныеДокумента.description);
			ЗаписьЖурналаРегистрации(
...
//   СписокВыражений - СписокXDTO - список выражений на встроенном языке, которые требуется выполнить на стороне ИС.
//
// Возвращаемое значение:
...
// Проверяет поддержку функционала получения основного файла документа ЭДО из 1С:Документооборота.
//
// Возвращаемое значение:
//   Булево
//
Функция ДоступенФункционалПолученияОсновногоФайлаДокументаЭДО() Экспорт
	
	Возврат ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("3.0.18.19");
...
// Проверяет поддерживает ли веб-сервис Документооборота функционал создания связей.
//
// Возвращаемое значение:
//   Булево
//
Функция ДоступноСозданиеСвязей() Экспорт
	
	Возврат ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("3.0.19.30");
...
// Возвращает подходящие объекты для создания на основании задачи.
//
// Параметры:
//   ЗадачаXDTO - ОбъектXDTO - объект XDTO типа DMTaskAction.
//   ДеревоПриложений - КоллекцияСтрокДереваЗначений - дерево приложений задачи.
...
//
// Возвращаемое значение:
//   Массив из см. ИнтеграцияС1СДокументооборот3КлиентСервер.ОбъектДляСозданияНаОснованииЗадачи
//
Функция ПодходящиеОбъектыДляСозданияНаОснованииЗадачи(ЗадачаXDTO, ДеревоПриложений,
		ОсновнойПредметID = "", ОсновнойПредметТип = "") Экспорт
	
	ОсновнойПредмет = ОсновнойПредметЗадачи(ЗадачаXDTO);
	ОсновнойПредметID = ОсновнойПредмет.ID;
	ОсновнойПредметТип = ОсновнойПредмет.Тип;
	
	ПодходящиеОбъекты = Новый Массив;
	
	ИмяПодсистемыИнтерфейсДокументовЭДО = "ЭлектронноеВзаимодействие.ОбменСКонтрагентами.ИнтерфейсДокументовЭДО";
	ЕстьИнтерфейсДокументовЭДО = ОбщегоНазначения.ПодсистемаСуществует(ИмяПодсистемыИнтерфейсДокументовЭДО);
	Если ЕстьИнтерфейсДокументовЭДО И ДоступенФункционалПолученияОсновногоФайлаДокументаЭДО() Тогда
		Для Каждого СтрокаПриложение Из ДеревоПриложений Цикл
			Для Каждого СтрокаФайл Из СтрокаПриложение.Строки Цикл
				Если СтрокаФайл.ПриложениеТип = "DMIncomingElectronicDocument"
						Или СтрокаФайл.ПриложениеТип = "DMOutgoingElectronicDocument" Тогда
					ПодходящийОбъект = ИнтеграцияС1СДокументооборот3КлиентСервер.ОбъектДляСозданияНаОснованииЗадачи();
					ПодходящийОбъект.ДокументЭДО.Наименование = СтрокаФайл.Приложение;
					ПодходящийОбъект.ДокументЭДО.ID = СтрокаФайл.ПриложениеID;
					ПодходящийОбъект.ДокументЭДО.Тип = СтрокаФайл.ПриложениеТип;
					ПодходящийОбъект.ПредставлениеОбъектаИС = СтрШаблон(
						Нстр("ru = 'Отразить электронный документ ""%1"" в учете'"),
						СтрокаФайл.Приложение);
					ПодходящиеОбъекты.Добавить(ПодходящийОбъект);
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
	...
	Пока ВыборкаПравил.Следующий() Цикл
		...
		Если ВыборкаПравил.ИспользоватьУсловиеПрименимостиПриЗагрузке Тогда
			
			ПравилоПодходит = РезультатВыраженияНаВстроенномЯзыке(
				ВыборкаПравил.УсловиеПрименимостиПриЗагрузке,
				ОсновнойПредмет.mainApplication);
			...
			Пока КлючевойРеквизит.Следующий() Цикл
							...
							Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(
								...
								ДопРеквизитПодходит =
									(ДопРеквизитXDTO.propertySimpleValue = КлючевойРеквизит.ЗначениеРеквизита)
										Или (ДопРеквизитXDTO.propertySimpleValue = КлючевойРеквизит.ЗначениеРеквизитаID);
				...
				ИначеЕсли ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(
					...
					Если ТипЗнч(ОсновнойПредмет.mainApplication[КлючевойРеквизит.Имя]) = Тип("ОбъектXDTO")
						...
						КонецЕсли;
					Иначе
						Если ОсновнойПредмет.mainApplication[КлючевойРеквизит.Имя] <> КлючевойРеквизит.ЗначениеРеквизита
								И ОсновнойПредмет.mainApplication[КлючевойРеквизит.Имя] <> КлючевойРеквизит.ЗначениеРеквизитаID Тогда
							ПравилоПодходит = Ложь;
							Прервать;
						КонецЕсли;
					КонецЕсли;
		...
		Если ПравилоПодходит Тогда
			ПодходящийОбъект = ИнтеграцияС1СДокументооборот3КлиентСервер.ОбъектДляСозданияНаОснованииЗадачи();
			ПодходящийОбъект.ПравилоИнтеграции = ВыборкаПравил.Ссылка;
			МассивПредставлениеОбъектаИС = Новый Массив;
			МассивПредставлениеОбъектаИС.Добавить(ВыборкаПравил.ПредставлениеОбъектаИССКлючевымиПолями);
			Если Не ПустаяСтрока(ВыборкаПравил.Комментарий) Тогда
				МассивПредставлениеОбъектаИС.Добавить(ВыборкаПравил.Комментарий);
			КонецЕсли;
			ПодходящийОбъект.ПредставлениеОбъектаИС = СтрСоединить(МассивПредставлениеОбъектаИС, ", ");
			ПодходящиеОбъекты.Добавить(ПодходящийОбъект);
		КонецЕсли;
...
// Возвращает из предварительных данных правило загрузки в ДО, соответствующее объекту ИС.
//
// Параметры:
//   Прокси - WSПрокси - объект для подключения к web-сервисам Документооборота.
//   ПредварительныеДанные - см. ИнтеграцияС1СДокументооборот3.ПредварительныеДанныеДляВыгрузкиВДО
//   ОбъектИС - ЛюбаяСсылка - объект ИС, для которого необходимо получить правило.
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMDataLoadingRule.
//   Неопределено - если подходящее правило загрузки не было найдено.
//
Функция ПравилоЗагрузкиВДОИзПредварительныхДанных(Прокси, ПредварительныеДанные, ОбъектИС) Экспорт
	
	ПравилоЗагрузкиВДО = Неопределено;
	
	ПравилоЗагрузкиВДОСтрока = ПредварительныеДанные.СоответствиеПравилЗагрузкиВДООбъектамИС[ОбъектИС];
	Если ПравилоЗагрузкиВДОСтрока <> Неопределено Тогда
		ПравилоЗагрузкиВДО = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СтрокаВОбъектXDTO(
			Прокси,
			ПравилоЗагрузкиВДОСтрока);
	КонецЕсли;
	
	Возврат ПравилоЗагрузкиВДО;
...
//   ТребуетсяСписокОбязательныхСвязей - Булево - требуется получение списка обязательных связей,
//     которые пользователь должен выбрать интерактивно.
//   НовыеОбъектыДО - Массив из см. ИнтеграцияС1СДокументооборот3КлиентСервер.НовыйОбъектДО
//
// Возвращаемое значение:
//   Структура:
//     * ДоступенФункционалФайлов - Булево - поддерживает ли веб-сервис Документооборота работу с файлами.
//     * ДоступноСозданиеСвязей - Булево - поддерживает ли веб-сервис Документооборота функционал
//         создания связей.
//     * СоответствиеПравилЗагрузкиВДООбъектамИС - Соответствие из КлючИЗначение - соответствующие правила загрузки в ДО:
//       ** Ключ - ОпределяемыйТип.ИнтеграцияС1СДокументооборотВсеСсылкиПереопределяемый - объект ИС.
//       ** Значение - Строка - строковое представление объекта XDTO типа DMDataLoadingRule.
//                   - Неопределено - если подходящее правило загрузки не было найдено.
...
//       ** Ключ - ОпределяемыйТип.ИнтеграцияС1СДокументооборотВсеСсылкиПереопределяемый - объект ИС.
//       ** Значение - СписокXDTO - список выражений на встроенном языке.
...
//     * СоответствиеСоздаваемыхСвязейОбъектамИС - Соответствие из КлючИЗначение:
//       ** Ключ - ОпределяемыйТип.ИнтеграцияС1СДокументооборотВсеСсылкиПереопределяемый - объект ИС.
//       ** Значение - Массив из см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.СвязьКСозданию
//     * СоответствиеОбязательныхСвязейОбъектамИС - Соответствие из КлючИЗначение:
//       ** Ключ - ОпределяемыйТип.ИнтеграцияС1СДокументооборотВсеСсылкиПереопределяемый - объект ИС.
//       ** Значение - Массив из см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.НастройкаСвязиОбъектовДО
//
Функция ПредварительныеДанныеДляВыгрузкиВДО(Прокси, СписокОбъектовИС, ПодходящиеПравилаИнтеграции,
		ТребуетсяСписокВыражений = Истина, ТребуетсяСписокОбязательныхСвязей = Истина,
		НовыеОбъектыДО = Неопределено) Экспорт
	
	Данные = Новый Структура;
	Данные.Вставить("ДоступенФункционалФайлов", ДоступенФункционалФайлов());
	Данные.Вставить("ДоступноСозданиеСвязей", ДоступноСозданиеСвязей());
	Данные.Вставить("СоответствиеПравилЗагрузкиВДООбъектамИС", Новый Соответствие);
	...
	Данные.Вставить("СоответствиеСоздаваемыхСвязейОбъектамИС", Новый Соответствие);
	Данные.Вставить("СоответствиеОбязательныхСвязейОбъектамИС", Новый Соответствие);
	
	Данные.СоответствиеПравилЗагрузкиВДООбъектамИС = СоответствиеПравилЗагрузкиВДООбъектамИС(
		Прокси,
		СписокОбъектовИС,
	...
	Если СписокОбъектовИС.Количество() = 0
			Или (Не ТребуетсяСписокВыгружаемыхТЧ
				И Не ТребуетсяСписокВыражений
				И Не Данные.ДоступенФункционалФайлов
				И Не Данные.ДоступноСозданиеСвязей) Тогда
		Возврат Данные;
	КонецЕсли;
	...
	КонецЕсли;
	Если Данные.ДоступенФункционалФайлов Тогда
	...
	КонецЕсли;
	Если Данные.ДоступноСозданиеСвязей Тогда
		ЗапросСоздаваемыхСвязей = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
			Прокси,
			"DMGetAddedRelationsRequest");
		ПравилаДляСоздаваемыхСвязей = ЗапросСоздаваемыхСвязей.objects; // СписокXDTO
	КонецЕсли;
	
	ДобавленныеПравила = Новый Массив;
	Для Каждого ОбъектИС Из СписокОбъектовИС Цикл
		ПравилоЗагрузкиВДО = ПравилоЗагрузкиВДОИзПредварительныхДанных(Прокси, Данные, ОбъектИС);
		...
		КонецЕсли;
		Если Данные.ДоступноСозданиеСвязей Тогда
			ПравилаДляСоздаваемыхСвязей.Добавить(КопияОбъектаXDTO(ПравилоЗагрузкиВДО, Прокси));
		КонецЕсли;
	...
	МассивВсехСоздаваемыхСвязей = Новый Массив;
	МассивВсехНастроекСвязей = Новый Массив;
	...
	КонецЕсли;
	Если Данные.ДоступенФункционалФайлов И ПравилаДляПечатныхФорм.Количество() > 0 Тогда
		ЗапросыПакета.Добавить(ЗапросПечатныхФорм);
	КонецЕсли;
	Если Данные.ДоступноСозданиеСвязей Тогда
		Если ПравилаДляСоздаваемыхСвязей.Количество() > 0 Тогда
			ЗапросыПакета.Добавить(ЗапросСоздаваемыхСвязей);
		КонецЕсли;
		Если ТребуетсяСписокОбязательныхСвязей Тогда
			СоответствиеСсылкаИзОбъектамИС = Новый Соответствие;
			Для Каждого ЭлементСоответствияПодходящиеПравила Из ПодходящиеПравилаИнтеграции Цикл
				Если ЭлементСоответствияПодходящиеПравила.Значение.Количество() <> 1 Тогда
					Продолжить;
				КонецЕсли;
				ПравилоИнтеграции = ЭлементСоответствияПодходящиеПравила.Значение[0];
				РеквизитыПравила = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПравилоИнтеграции, "ТипОбъектаДО, ВидДокументаДОID");
				Если РеквизитыПравила.ТипОбъектаДО = "DMDocument" Тогда
					СсылкаИзТип = "DMDocumentType";
					СсылкаИзID = РеквизитыПравила.ВидДокументаДОID;
				Иначе
					СсылкаИзТип = РеквизитыПравила.ТипОбъектаДО;
					СсылкаИзID = "";
				КонецЕсли;
				Если СсылкаИзТип = "DMCorrespondent" Тогда
					Продолжить;
				КонецЕсли;
				СсылкаИз = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО(
					СсылкаИзID,
					СсылкаИзТип);
				ЗапросУжеСоздан = Ложь;
				Для Каждого ЭлементСоответствияСсылкаИз Из СоответствиеСсылкаИзОбъектамИС Цикл
					Если ОбщегоНазначения.ДанныеСовпадают(ЭлементСоответствияСсылкаИз.Значение, СсылкаИз) Тогда
						ЗапросУжеСоздан = Истина;
						Прервать;
					КонецЕсли;
				КонецЦикла;
				Если Не ЗапросУжеСоздан Тогда
					ЗапросыПакета.Добавить(НастройкиСвязейЗапрос(Прокси, СсылкаИз));
				КонецЕсли;
				СоответствиеСсылкаИзОбъектамИС.Вставить(ЭлементСоответствияПодходящиеПравила.Ключ, СсылкаИз);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗапросыПакета.Количество() > 0 Тогда
		...
		Для Каждого Ответ Из ОтветНаЗапросыПакета.responses Цикл
			...
			ИначеЕсли ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(
					Прокси, Ответ, "DMGetAddedRelationsResponse") Тогда
				МассивВсехСоздаваемыхСвязей = Ответ.objectsAddedRelations; // СписокXDTO
				
			ИначеЕсли ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(
					Прокси, Ответ, "DMGetRelationsSettingsResponse") Тогда
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
					МассивВсехНастроекСвязей,
					Ответ.relationsSettings);
	...
	Для Каждого ОбъектИС Из СписокОбъектовИС Цикл
		...
		СписокАвтоматическиДобавляемыхСвязей = Новый Массив;
		...
		Если ПравилоЗагрузкиВДО <> Неопределено Тогда
			...
			Если Данные.ДоступноСозданиеСвязей Тогда
				Для Каждого Элемент Из МассивВсехСоздаваемыхСвязей Цикл
					Если Элемент.object.ID = ПравилоЗагрузкиВДО.ID Тогда
						СписокАвтоматическиДобавляемыхСвязей = Элемент.addedRelations;
						Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		...
		КонецЕсли;
		Если Данные.ДоступноСозданиеСвязей Тогда
			// Заполним связи, которые будут созданы автоматически.
			СвязиКСозданию = Новый Массив;
			ЗаполнитьСписокСвязиКСозданию(
				Прокси,
				СвязиКСозданию,
				ОбъектИС,
				СписокАвтоматическиДобавляемыхСвязей,
				НовыеОбъектыДО);
			Данные.СоответствиеСоздаваемыхСвязейОбъектамИС.Вставить(ОбъектИС, СвязиКСозданию);
			Если ТребуетсяСписокОбязательныхСвязей Тогда
				// Заполним обязательные связи, которые требуется выбрать интерактивно.
				ОбязательныеСвязи = Новый Массив;
				СсылкаИз = СоответствиеСсылкаИзОбъектамИС[ОбъектИС];
				Если СсылкаИз <> Неопределено Тогда
					Для Каждого НастройкаСвязи Из МассивВсехНастроекСвязей Цикл
						Если Не НастройкаСвязи.required Или НастройкаСвязи.relationFrom.objectID.type <> СсылкаИз.Тип Тогда
							Продолжить;
						КонецЕсли;
						Если ЗначениеЗаполнено(НастройкаСвязи.relationFrom.objectID.ID)
								И НастройкаСвязи.relationFrom.objectID.ID <> СсылкаИз.ID Тогда
							Продолжить;
						КонецЕсли;
						СвязьБудетСозданаАвтоматически = Ложь;
						СсылкаНаТип = НастройкаСвязи.relationTo.objectID.type;
						Если СсылкаНаТип = "DMDocumentType" Тогда
							СсылкаНаТип = "DMDocument";
						КонецЕсли;
						Для Каждого СвязьКСозданию Из СвязиКСозданию Цикл
							Если СвязьКСозданию.ТипСвязиID = НастройкаСвязи.relationType.objectID.ID
									И СвязьКСозданию.СсылкаНаТип = СсылкаНаТип Тогда
								СвязьБудетСозданаАвтоматически = Истина;
								Прервать;
							КонецЕсли;
						КонецЦикла;
						Если Не СвязьБудетСозданаАвтоматически Тогда
							СсылкаНа = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО(
								НастройкаСвязи.relationTo.objectID.ID,
								НастройкаСвязи.relationTo.objectID.type,
								НастройкаСвязи.relationTo.name,
								НастройкаСвязи.relationTo.objectID.presentation,
								НастройкаСвязи.relationTo.objectID.navigationRef);
							ТипСвязи = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО(
								НастройкаСвязи.relationType.objectID.ID,
								НастройкаСвязи.relationType.objectID.type,
								НастройкаСвязи.relationType.name,
								НастройкаСвязи.relationType.objectID.presentation,
								НастройкаСвязи.relationType.objectID.navigationRef);
							ОбязательнаяСвязь = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.НастройкаСвязиОбъектовДО(
								СсылкаИз,
								СсылкаНа,
								ТипСвязи);
							ОбязательнаяСвязь.ТипСвязиКомментарий = НастройкаСвязи.relationType.comment;
							ОбязательныеСвязи.Добавить(ОбязательнаяСвязь);
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
			Данные.СоответствиеОбязательныхСвязейОбъектамИС.Вставить(ОбъектИС, ОбязательныеСвязи);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПриСозданииНаСервере(Форма, МестоРазмещенияКоманд = Неопределено, ПараметрыОповещения = Неопределено) Экспорт
	...
	// Скроем лишние команды БИД.
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.УбратьДублиКомандыИнтеграции(
		Форма,
		Метаданные.ОбщиеКоманды.ИнтеграцияС1СДокументооборотНачатьОбработку.Имя);
...
//   ВыполняетсяОбновлениеОбъекта - Булево - признак наличия в пакетном запросе запроса на обновление объекта.
...
//
Функция РезультатВыполненияКомандыПоОбъекту(РезультатДействияСтрока, ДанныеДляОбновления,
		ВыполняетсяОбновлениеОбъекта = Ложь, КонтрольОтправкиФайлов = Неопределено) Экспорт
	...
	Если ВыполняетсяОбновлениеОбъекта Тогда
		ОтветОбновитьОбъект = ОтветНаЗапросыПакета.responses[0];
		РезультатВыполнения = ОтветНаЗапросыПакета.responses[1];
		НомерЗапросаДанныхДляОбновления = 2;
	...
	Иначе
		...
		НомерЗапросаДанныхДляОбновления = 1;
	КонецЕсли;
	...
	Если ОтветНаЗапросыПакета.responses.Количество() = НомерЗапросаДанныхДляОбновления + 1 Тогда
		ДанныеДляОбновления = ОтветНаЗапросыПакета.responses[НомерЗапросаДанныхДляОбновления];
...
//
Функция СоздатьОбъектДОПоПравилу(Прокси, ОбъектИС, Правило, СвязиКСозданию = Неопределено,
		ПредварительныеДанныеВХранилище = Неопределено, НовыеОбъектыДО = Неопределено) Экспорт
	...
	Если СвязанныйОбъектДО <> Неопределено Тогда
		Если ЗначениеЗаполнено(СвязанныйОбъектДО.ID) И ЗначениеЗаполнено(СвязанныйОбъектДО.type) Тогда
			Возврат ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьОбъект(
				Прокси,
				СвязанныйОбъектДО.type,
				СвязанныйОбъектДО.ID);
		Иначе
			Возврат НСтр("ru = 'Выгрузка в 1С:Документооборот уже началась. Повторите попытку еще раз.'");
		КонецЕсли;
	КонецЕсли;
	
	РегистрыСведений.ОбъектыИнтегрированныеС1СДокументооборотом.ДобавитьСвязь("", "", ОбъектИС, Ложь);
	...
	Попытка
		...
		ПредварительныеДанные = Неопределено;
		Если ПредварительныеДанныеВХранилище <> Неопределено Тогда
			ПредварительныеДанные = ПолучитьИзВременногоХранилища(ПредварительныеДанныеВХранилище);
			УдалитьИзВременногоХранилища(ПредварительныеДанныеВХранилище);
		КонецЕсли;
		...
		Если СвязиКСозданию <> Неопределено Тогда
			Для Каждого СвязьКСозданию Из СвязиКСозданию Цикл
				ПредварительныеДанные.СоответствиеСоздаваемыхСвязейОбъектамИС[ОбъектИС].Добавить(СвязьКСозданию);
			КонецЦикла;
		КонецЕсли;
		
		Результат = СоздатьОбъектыДОПоПравилам(
			Прокси,
			СписокОбъектовИС,
		...
		Иначе
			РегистрыСведений.ОбъектыИнтегрированныеС1СДокументооборотом.УдалитьСвязь("", "", ОбъектИС);
...
// Создает новые объекты Документооборота для списка объектов ИС по соответствующим им правилам.
//
// Параметры:
//   Прокси - WSПрокси - объект для подключения к web-сервисам Документооборота.
...
//
Функция СоздатьОбъектыДОПоПравилам(Прокси, СписокОбъектовИС, ПредварительныеДанные) Экспорт
	...
	Для Каждого ОбъектИС Из СписокОбъектовИС Цикл
		...
		ПравилоЗагрузкиВДО = ПравилоЗагрузкиВДОИзПредварительныхДанных(Прокси, ПредварительныеДанные, ОбъектИС);
		Если ПравилоЗагрузкиВДО = Неопределено Тогда
		...
		Если ПредварительныеДанные.ДоступноСозданиеСвязей Тогда
			ПеренестиСвязиКСозданиюВСтруктуруВходящихДанных(
				Прокси,
				ПредварительныеДанные.СоответствиеСоздаваемыхСвязейОбъектамИС[ОбъектИС],
	...
	ШтрихкодУстановленПриСоздании =
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("3.0.19.30");
	...
	Для Каждого СтруктураОтвета Из Ответ.objects Цикл
		...
		Если СтруктураОтвета.success Тогда
			...
			Если Не ШтрихкодУстановленПриСоздании Тогда
...
Функция СоответствиеПравилЗагрузкиВДООбъектамИС(Прокси, СписокОбъектовИС, ПодходящиеПравилаИнтеграции) Экспорт
	...
	Для Каждого ОбъектИС Из СписокОбъектовИС Цикл
		...
		Если ПодходящиеПравилаИнтеграции[ОбъектИС] <> Неопределено
			...
			ПравилоЗагрузкиВДОXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(
				Прокси,
				СсылкиНаПравилаЗагрузкиВДО[ПравилоИнтеграции],
				"DMDataLoadingRule");
			ПравилоЗагрузкиВДО = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ОбъектXDTOВСтроку(
				Прокси,
				ПравилоЗагрузкиВДОXDTO);
		КонецЕсли;
		Результат.Вставить(ОбъектИС, ПравилоЗагрузкиВДО);
	КонецЦикла;
...
// Выполняет перерисовку закладки обзор в форме.
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения - форма с закладкой "Обзор".
//   ОбъектДОПредставлениеXDTO - ОбъектXDTO - объект XDTO типа DMGetObjectsHTMLPresentationsResponse.
//   ОбъектXDTO - ОбъектXDTO - объект XDTO типа DMDocument или DMMeeting или DMCorrespondent.
//
Процедура ЗаполнитьЗакладкуОбзорВФорме(Форма, ОбъектДОПредставлениеXDTO, ОбъектXDTO) Экспорт
	
	РолиФайлов = Новый Массив;
	Если ЭтоДокументДО3(ОбъектXDTO.objectID.type) Тогда
	...
	Форма.ИспользоватьРолиФайлов = (РолиФайлов.Количество() > 0);
	...
	ПредставлениеОбъекта = ИнтеграцияС1СДокументооборот3КлиентСервер.ПредставлениеТипаОбъектаXDTO(
		ОбъектXDTO.objectID.type);
	
	ДеревоФайлов = Форма.ДеревоФайлов; // см. Обработка.ИнтеграцияС1СДокументооборот3.Форма.Документооборот.ДеревоФайлов
	СтрокиДереваФайлов = ДеревоФайлов.ПолучитьЭлементы();
	СтрокиДереваФайлов.Очистить();
	
	// 1. Выведем строку самого объекта.
	СтрокаОбъекта = СтрокиДереваФайлов.Добавить();
	СтрокаОбъекта.ИндексКартинки = 10;
	СтрокаОбъекта.Наименование = ПредставлениеОбъекта;
	СтрокаОбъекта.ID = Форма.ОбъектДОID;
	СтрокаОбъекта.Тип = Форма.ОбъектДОТип;
	СтрокаОбъекта.НавигационнаяСсылка = Форма.ОбъектДОНавигационнаяСсылка;
	СтрокаОбъекта.ПредставлениеHTML = ОбъектДОПредставлениеXDTO.ObjectsHTMLPresentations[0].htmlView;
	
	// 2. Выведем документы ЭДО (при наличии).
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ОбъектXDTO, "EDIDocuments") Тогда
		Для Каждого ДокументЭДО Из ОбъектXDTO.EDIDocuments Цикл
			НоваяСтрока = СтрокиДереваФайлов.Добавить();
			НоваяСтрока.ИндексКартинки = 30;
			НоваяСтрока.Наименование = СтрШаблон(НСтр("ru = '%1 (Документ ЭДО)'"), ДокументЭДО.objectID.presentation);
			НоваяСтрока.ID = ДокументЭДО.objectID.ID;
			НоваяСтрока.Тип = ДокументЭДО.objectID.type;
			НоваяСтрока.НавигационнаяСсылка = ДокументЭДО.objectID.navigationRef;
			НоваяСтрока.НомерКартинкиПодписанЗашифрован = -1;
		КонецЦикла;
	КонецЕсли;
	
	// 3. Выведем прикрепленные файлы.
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ОбновитьДеревоФайлов(
		ДеревоФайлов,
		Форма.ДоступныеРоли,
		СписокФайлов,
		Форма.ИспользоватьРолиФайлов,
		РолиФайлов);
	
	Элементы = Форма.Элементы;
	
	Если КоличествоФайлов = 0 Тогда
		Элементы.НаименованиеФайла.Заголовок = СтрШаблон(НСтр("ru = '%1 и файлы'"), ПредставлениеОбъекта);
	Иначе
		Элементы.НаименованиеФайла.Заголовок =
			СтрШаблон(НСтр("ru = '%1 и файлы (%2)'"), ПредставлениеОбъекта, КоличествоФайлов);
	КонецЕсли;
	
	Если Форма.ИспользоватьЭлектронныеПодписиДО Тогда
		Если ЭтоДокументДО3(ОбъектXDTO.objectID.type) Тогда
			Форма.АдресДвоичныхДанныхОбъекта.АдресСлепкаДокумента = ПоместитьВоВременноеХранилище(
				ОбъектXDTO.keyPropertiesValue, Форма.УникальныйИдентификатор);
		КонецЕсли;
		ПодписиОбъекта = Новый Массив;
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ОбъектXDTO, "signatures") Тогда
			ПодписиОбъекта = ОбъектXDTO.signatures; // СписокXDTO
			СтрокаОбъекта.ПодписанЭП = (ПодписиОбъекта.Количество() > 0);
		КонецЕсли;
		ОбъектДокументооборота =
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО(
				...
				Форма.ОбъектДОТип,
				Форма.ОбъектДО);
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьСписокПодписей(
			ОбъектДокументооборота,
			ПодписиОбъекта,
			СписокФайлов,
			Форма.ДеревоПодписей,
			Форма.КоличествоЭП,
			Форма.УникальныйИдентификатор);
	КонецЕсли;
	Форма.ПодписанЭП = СтрокаОбъекта.ПодписанЭП;
	Если СтрокаОбъекта.ПодписанЭП Тогда
		СтрокаОбъекта.НомерКартинкиПодписанЗашифрован = 2;
	Иначе
		СтрокаОбъекта.НомерКартинкиПодписанЗашифрован = -1;
	КонецЕсли;
	
	// Уточним доступность команд добавления.
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьНДПСизОбъектаXDTO(Форма, ОбъектXDTO);
	Элементы.ФайлыДобавить.Доступность = (Форма.СостояниеРазрешаетДобавлениеФайла
		Или Форма.СостояниеРазрешаетДобавлениеСканКопии) И Не СтрокаОбъекта.ПодписанЭП;
	Элементы.ФайлыДобавитьКонтекст.Доступность = (Форма.СостояниеРазрешаетДобавлениеФайла
		Или Форма.СостояниеРазрешаетДобавлениеСканКопии) И Не СтрокаОбъекта.ПодписанЭП;
	Элементы.ФайлыДобавитьПечатнуюФорму.Доступность = (Форма.СостояниеРазрешаетДобавлениеФайла
		Или Форма.СостояниеРазрешаетДобавлениеСканКопии) И Не СтрокаОбъекта.ПодписанЭП;
	Элементы.ФайлыДобавитьПечатнуюФормуКонтекст.Доступность = (Форма.СостояниеРазрешаетДобавлениеФайла
		Или Форма.СостояниеРазрешаетДобавлениеСканКопии) И Не СтрокаОбъекта.ПодписанЭП;
	Элементы.ФайлыВставитьКартинкуИзБуфера.Доступность = (Форма.СостояниеРазрешаетДобавлениеФайла
		Или Форма.СостояниеРазрешаетДобавлениеСканКопии) И Не СтрокаОбъекта.ПодписанЭП;
	Элементы.ФайлыИмпортФайловКонтекст.Доступность = (Форма.СостояниеРазрешаетДобавлениеФайла
		Или Форма.СостояниеРазрешаетДобавлениеСканКопии) И Не СтрокаОбъекта.ПодписанЭП;
	
	// Заполним группу состояний.
	ОбъектДОИмеетГруппуСостояний = ЭтоДокументДО3(ОбъектXDTO.objectID.type) Или ОбъектXDTO.objectID.type = "DMMeeting";
	Если Форма.ДоступноПолучениеДанныхФормы И ОбъектДОИмеетГруппуСостояний Тогда
		Элементы.ГруппаСостояния.Видимость = Истина;
		
		СтруктураЭлемента = СтруктураПоляКартинки(ОбъектXDTO.form.interactionStatusInObjectOverview);
		ЗаполнитьЗначенияСвойств(Элементы.КартинкаВзаимодействие, СтруктураЭлемента);
		Форма.СтатусВзаимодействие = СтруктураЭлемента.ИндексКартинки;
		
		СтруктураЭлемента = СтруктураПоляКартинки(ОбъектXDTO.form.categoryStatusInObjectOverview);
		ЗаполнитьЗначенияСвойств(Элементы.КартинкаКатегории, СтруктураЭлемента);
		Форма.СтатусКатегории = СтруктураЭлемента.ИндексКартинки;
		
		СтруктураЭлемента = СтруктураПоляКартинки(ОбъектXDTO.form.forumStatusInObjectOverview);
		ЗаполнитьЗначенияСвойств(Элементы.КартинкаФорум, СтруктураЭлемента);
		Форма.СтатусФорум = СтруктураЭлемента.ИндексКартинки;
		
		СтруктураЭлемента = СтруктураПоляКартинки(ОбъектXDTO.form.monitoringStatusInObjectOverview);
		ЗаполнитьЗначенияСвойств(Элементы.КартинкаКонтроль, СтруктураЭлемента);
		Форма.СтатусКонтроля = СтруктураЭлемента.ИндексКартинки;
		
		Если ЭтоДокументДО3(ОбъектXDTO.objectID.type) Тогда
			СтруктураЭлемента = СтруктураПоляКартинки(ОбъектXDTO.form.digitalSignatureStatusInObjectOverviews);
			ЗаполнитьЗначенияСвойств(Элементы.КартинкаЭП, СтруктураЭлемента);
			Форма.СтатусЭП = СтруктураЭлемента.ИндексКартинки;
			// Установим картинку ЭП для строки самого объекта.
			Если Форма.СтатусЭП > 0 Тогда
				СтрокаОбъекта.НомерКартинкиПодписанЗашифрован = Форма.СтатусЭП * 2;
			КонецЕсли;
			
			СтруктураЭлемента = СтруктураПоляКартинки(ОбъектXDTO.form.EDIStatusInObjectOverview);
			ЗаполнитьЗначенияСвойств(Элементы.КартинкаЭДО, СтруктураЭлемента);
			Форма.СтатусЭДО = СтруктураЭлемента.ИндексКартинки;
			
			СтруктураЭлемента = СтруктураПоляКартинки(ОбъектXDTO.form.caseStatusInObjectOverview);
			ЗаполнитьЗначенияСвойств(Элементы.КартинкаСостояниеДела, СтруктураЭлемента);
			Форма.СостояниеДелаХранения = СтруктураЭлемента.ИндексКартинки;
		Иначе
			Элементы.КартинкаЭП.Видимость = Ложь;
			Элементы.КартинкаЭДО.Видимость = Ложь;
...
Функция ВыполнитьДействиеЗадачиЗапрос(Прокси, ДанныеПоЗадаче) Экспорт
	...
	Контекст = ДанныеПоЗадаче.ТребуемоеИнтерактивноеДействие.Контекст;
	...
	ИначеЕсли ДанныеПоЗадаче.ТребуемоеИнтерактивноеДействие.ЗадатьПроизвольныйВопрос Тогда
		...
		РезультатXDTO.context = Контекст;
	...
	ИначеЕсли ДанныеПоЗадаче.ТребуемоеИнтерактивноеДействие.ИзменитьПараметрыПроцесса Тогда
		...
		Если ТипЗнч(Контекст) = Тип("Строка") И Не ПустаяСтрока(Контекст) Тогда
	...
	ИначеЕсли ДанныеПоЗадаче.ТребуемоеИнтерактивноеДействие.ВыполнитьПодписание Тогда
		
		РезультатXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
			Прокси,
			"DMInteractiveActionContext");
		РезультатXDTO.interactiveAction = "PerformSigning";
		РезультатXDTO.context = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
			Прокси,
			"DMSigningContext");
		СписокДанныхИПодписей = РезультатXDTO.context.dataAndSignatureList; // СписокXDTO
		Для Каждого ПодписанныйОбъект Из Контекст.НаборДанных Цикл
			ПодписанныйОбъектXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
				Прокси,
				"DMDataAndSignature");
			ПодписанныйОбъектXDTO.object = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(
				Прокси,
				ПодписанныйОбъект.Представление.Значение.ID,
				ПодписанныйОбъект.Представление.Значение.Тип);
			ПодписанныйОбъектXDTO.object.presentation = ПодписанныйОбъект.Представление.Представление;
			ПодписанныйОбъектXDTO.signature = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
				Прокси,
				"DMSignature");
			ДанныеПодписи = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеПодписи(
				ПодписанныйОбъект.СвойстваПодписи.Подпись,
				ПодписанныйОбъект.СвойстваПодписи.Сертификат,
				ПодписанныйОбъект.СвойстваПодписи);
			ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьXDTOПодпись(
				Прокси,
				ПодписанныйОбъектXDTO.signature,
				ДанныеПодписи);
			СписокДанныхИПодписей.Добавить(ПодписанныйОбъектXDTO);
		КонецЦикла;
		Если ЗначениеЗаполнено(Контекст.ДатаИсполнения) Тогда
			РезультатXDTO.context.executionDate = Контекст.ДатаИсполнения;
		КонецЕсли;
		Если ЗначениеЗаполнено(Контекст.Операция) Тогда
			РезультатXDTO.context.operation = Контекст.Операция;
		КонецЕсли;
		Если ЗначениеЗаполнено(Контекст.ЗаголовокДанных) Тогда
			РезультатXDTO.context.title = Контекст.ЗаголовокДанных;
		КонецЕсли;
		РезультатыИнтерактивныхДействий.Добавить(РезультатXDTO);
...
//   ПодходящиеОбъекты - см. ИнтеграцияС1СДокументооборот3.ПодходящиеОбъектыДляСозданияНаОснованииЗадачи
//
// Возвращаемое значение:
//   Строка - ключ созданной страницы задач.
//
Функция НоваяСтраницаКомандСозданияНаОснованииЗадачи(Форма, ПодходящиеОбъекты) Экспорт
	...
	Для Каждого ПодходящийОбъект Из ПодходящиеОбъекты Цикл
		Если ЗначениеЗаполнено(ПодходящийОбъект.ПравилоИнтеграции) Тогда
			Команда = "Создать";
			ГУИД = СтрЗаменить(Строка(ПодходящийОбъект.ПравилоИнтеграции.УникальныйИдентификатор()), "-", "_");
		Иначе
			Команда = ПодходящийОбъект.ДокументЭДО.Тип;
			ГУИД = СтрЗаменить(ПодходящийОбъект.ДокументЭДО.ID, "-", "_");
		КонецЕсли;
		ИмяКоманды = СтрШаблон("%1_%2", Команда, ГУИД);
		...
		Если Форма.Элементы.Найти(ИмяЭлемента) = Неопределено Тогда // Страховка от дубликатов правил.
			...
			Кнопка.Заголовок = ПодходящийОбъект.ПредставлениеОбъектаИС;
...
//     * Расширение - Строка
//     * АдресВХранилищеФайлаПредпросмотра - Строка
...
Функция НовоеДеревоПриложений() Экспорт
	...
	НовоеДеревоПриложений.Колонки.Добавить("АдресВХранилищеФайлаПредпросмотра", Новый ОписаниеТипов("Строка"));
...
Процедура ЗаполнитьОбработкуВФорме(Форма, ДанныеОбОбработкеXDTO) Экспорт
	...
	ДоступнаПечать =
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("3.0.14.31");
	...
	ИначеЕсли Форма.СостояниеОбработки = "Executing" Или Форма.СостояниеОбработки = "Stopped" Тогда
		...
		Если Форма.СостояниеОбработки = "Executing" Тогда
			...
			Форма.Элементы.ДекорацияОписание.Заголовок = СтрШаблон(НСтр("ru = 'Обработка начата %1'"),
				ФорматированнаяДатаСобытияОбработки(ДатаНачалаОбработки));
		ИначеЕсли Форма.СостояниеОбработки = "Stopped" Тогда
			...
			Форма.Элементы.ДекорацияОписание.Заголовок = СтрШаблон(НСтр("ru = 'Обработка на паузе с %1'"),
				ФорматированнаяДатаСобытияОбработки(ДатаИзмененияСостоянияОбработки));
		КонецЕсли;
		Форма.Элементы.ДекорацияОписание.Подсказка = Форма.Элементы.ДекорацияОписание.Заголовок;
	...
	Если ЗначениеЗаполнено(ДатаОкончания) Тогда
		...
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(Символы.НПП));
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(ФорматированнаяДатаСобытияОбработки(ДатаОкончания)));
...
Процедура ЗаполнитьДеревоСвязейВФорме(Форма, СписокСвязейXDTO) Экспорт
	...
	Для Каждого Связь Из СписокСвязейXDTO.relations Цикл
		...
		Если ЗначениеЗаполнено(СтрокаСвязанныйОбъект.Ссылка) Тогда
			...
			Попытка
				СтрокаСвязанныйОбъект.СвязанныйОбъект = СтрШаблон(НСтр("ru = '%1, объект %2'"),
		...
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(Связь.relatedObject, "extension") Тогда
			СтрокаСвязанныйОбъект.РасширениеФайла = Связь.relatedObject.extension;
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаСвязанныйОбъект.РасширениеФайла) Тогда
			СтрокаСвязанныйОбъект.ИндексКартинки =
				ИнтеграцияС1СДокументооборотБазоваяФункциональность.ИндексПиктограммыФайла(
					СтрокаСвязанныйОбъект.РасширениеФайла);
		КонецЕсли;
		...
		Для Каждого Файл Из Связь.relatedObject.files Цикл
			...
			СтрокаФайл.Зашифрован = Файл.encrypted;
		...
		Если Связь.relatedObject.hasAnotherRelations Тогда
			...
			СтрокаСвязь.СвязанныйОбъект = НСтр("ru = 'Связи'");
...
// Получает настройки связей объектов 1С:Документооборота.
//
// Параметры:
//   Прокси - WSПрокси - объект для подключения к web-сервисам Документооборота.
//   СсылкаИз - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО.
//   СсылкаНа - Строка
//            - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMGetRelationsSettingsResponse.
//
Функция НастройкиСвязей(Прокси, СсылкаИз, СсылкаНа = Неопределено) Экспорт
	
	Запрос = НастройкиСвязейЗапрос(Прокси, СсылкаИз, СсылкаНа);
	Ответ = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ВыполнитьЗапрос(Прокси, Запрос);
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьВозвратВебСервиса(Прокси, Ответ);
...
// Возвращает запрос для получения настроек связей объектов 1С:Документооборота.
//
// Параметры:
//   Прокси - WSПрокси - объект для подключения к web-сервисам Документооборота.
//   СсылкаИз - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО.
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMGetRelationsSettingsRequest.
//
Функция НастройкиСвязейЗапрос(Прокси, СсылкаИз, СсылкаНа = Неопределено) Экспорт
	
	Запрос = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMGetRelationsSettingsRequest");
	
	Запрос.relationFrom = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMObjectID");
	Запрос.relationFrom = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(
		Прокси,
		СсылкаИз.ID,
		СсылкаИз.Тип);
	
	Если ЗначениеЗаполнено(СсылкаНа)
			И ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоСуществует(Запрос, "relationTo") Тогда
		Запрос.relationTo = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMRelatedObject");
		Запрос.relationTo.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(
			Прокси,
			СсылкаНа.ID,
			СсылкаНа.Тип);
		Запрос.relationTo.name = СсылкаНа.Представление;
	КонецЕсли;
...
// Переносит список связей в структуру входящих данных, необходимую для создания и изменения объекта ДО.
//
// Параметры:
//   Прокси - WSПрокси - объект для подключения к web-сервисам Документооборота.
//   СвязиКСозданию - Массив из см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.СвязьКСозданию
//   СтруктураВходящихДанных - ОбъектXDTO - объект XDTO типа DMIncomingDataRequestStructure.
//
Процедура ПеренестиСвязиКСозданиюВСтруктуруВходящихДанных(Прокси, СвязиКСозданию, СтруктураВходящихДанных) Экспорт
	
	СписокСвязейОбъектаИС = СтруктураВходящихДанных.relations; // СписокXDTO
	
	Для Каждого СвязьКСозданию Из СвязиКСозданию Цикл
		СвязьXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMRelation");
		
		СвязьXDTO.object = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
			Прокси, "DMRelatedObject");
		СвязьXDTO.object.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси);
		СвязьXDTO.object.name = "";
		
		СвязьXDTO.relatedObject = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
			Прокси, "DMRelatedObject");
		СвязьXDTO.relatedObject.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(
			Прокси,
			СвязьКСозданию.СсылкаНаID,
			СвязьКСозданию.СсылкаНаТип);
		СвязьXDTO.relatedObject.name = СвязьКСозданию.СсылкаНа;
		
		СвязьXDTO.relationType = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
			Прокси, "DMRelationType");
		СвязьXDTO.relationType.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(
			Прокси,
			СвязьКСозданию.ТипСвязиID,
			"DMRelationType");
		СвязьXDTO.relationType.name = "";
		
		СвязьXDTO.comment = СвязьКСозданию.Комментарий;
		
		СписокСвязейОбъектаИС.Добавить(СвязьXDTO);
	КонецЦикла;
	
КонецПроцедуры

// Возвращает список объектов, подходящих для автоматического добавления связей к указанному объекту ИС.
// Учитывает табличные части объекта ИС.
//
// Параметры:
//   ОбъектИС - ЛюбаяСсылка - ссылка на исходный объект ИС, к которому добавляются связи.
//   СписокАвтоматическиДобавляемыхСвязей - СписокXDTO - список настроек создания связей, заданный в правилах
//     загрузки данных в ДО.
//   ИмяСобытия - Строка - имя события для журнала регистрации.
//
// Возвращаемое значение:
//   ТаблицаЗначений:
//     * Ссылка - ЛюбаяСсылка - ссылка на связываемый объект ИС.
//     * СвязанныйОбъектДО - Неопределено - если связываемый объект ИС еще не не выгружался в ДО.
...
//         выгрузки связываемого объекта ИС в ДО.
//     * Комментарий - Строка - комментарий к создаваемой связи.
//
Функция ПодходящиеОбъектыДляДобавленияСвязей(ОбъектИС, Знач СписокАвтоматическиДобавляемыхСвязей, ИмяСобытия) Экспорт
	
	ПодходящиеОбъекты = Новый ТаблицаЗначений;
	ПодходящиеОбъекты.Колонки.Добавить("Ссылка", ОбщегоНазначения.ОписаниеТипаВсеСсылки());
	ПодходящиеОбъекты.Колонки.Добавить("СвязанныйОбъектДО");
	ПодходящиеОбъекты.Колонки.Добавить("ТипСвязи", Новый ОписаниеТипов("Структура"));
	ПодходящиеОбъекты.Колонки.Добавить("ПравилоИнтеграции",
		Новый ОписаниеТипов("СправочникСсылка.ПравилаИнтеграцииС1СДокументооборотом3"));
	ПодходящиеОбъекты.Колонки.Добавить("Комментарий", Новый ОписаниеТипов("Строка"));
	
	ТипыОбъектовСПравилами =
	...
	Для Каждого НастройкаXDTO Из СписокАвтоматическиДобавляемыхСвязей Цикл
		Если ЗначениеЗаполнено(НастройкаXDTO.tabularSectionName) Тогда
			ИменаРеквизитов.Добавить(НастройкаXDTO.tabularSectionName);
		ИначеЕсли СтрНайти(НастройкаXDTO.attributeName, "ДопРеквизит_") = 0 Тогда
			ИменаРеквизитов.Добавить(НастройкаXDTO.attributeName);
		КонецЕсли;
	КонецЦикла;
	РеквизитыОбъектаИС = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОбъектИС, ИменаРеквизитов);
	
	ЗначенияКВыгрузке = Новый Соответствие;
	Для Каждого НастройкаXDTO Из СписокАвтоматическиДобавляемыхСвязей Цикл
		Если ЗначениеЗаполнено(НастройкаXDTO.tabularSectionName) Тогда
			ТабличнаяЧастьОбъектаИС = РеквизитыОбъектаИС[НастройкаXDTO.tabularSectionName].Выгрузить();
			Для Каждого Строка Из ТабличнаяЧастьОбъектаИС Цикл
				ЗначениеРеквизитаИС = Строка[НастройкаXDTO.attributeName];
				ДополнитьПодходящиеОбъектыДляСозданияСвязей(
					ЗначениеРеквизитаИС,
					ПодходящиеОбъекты,
					НастройкаXDTO,
					ЗначенияКВыгрузке);
			КонецЦикла;
		ИначеЕсли СтрНайти(НастройкаXDTO.attributeName, "ДопРеквизит_") = 1 Тогда
			Свойство = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДопРеквизитПоИмени(
				НастройкаXDTO.attributeName);
			ЗначениеРеквизитаИС = УправлениеСвойствами.ЗначениеСвойства(ОбъектИС, Свойство);
			ДополнитьПодходящиеОбъектыДляСозданияСвязей(
				ЗначениеРеквизитаИС,
				ПодходящиеОбъекты,
				НастройкаXDTO,
				ЗначенияКВыгрузке);
		Иначе
			ЗначениеРеквизитаИС = РеквизитыОбъектаИС[НастройкаXDTO.attributeName];
			ДополнитьПодходящиеОбъектыДляСозданияСвязей(
				ЗначениеРеквизитаИС,
				ПодходящиеОбъекты,
				НастройкаXDTO,
	...
	Для Каждого ЭлементСоответствия Из ЗначенияКВыгрузке Цикл
		Если ПодходящиеОбъекты.Найти(ЭлементСоответствия.Ключ, "Ссылка") <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Не удалось выгрузить объект ""%1"" в 1С:Документооборот по причине:
				|%2'"),
			ЭлементСоответствия.Ключ,
			СтрСоединить(ЭлементСоответствия.Значение, Символы.ПС));
...
Функция СвязиОбъектаЗапрос(Прокси, РеквизитыОбъекта) Экспорт
	...
	Запрос.object.name = РеквизитыОбъекта.Представление;
...
//   ИмяЭлемента - Строка - имя элемента формы, содержащего поле PDF документа.
//   ИмяРеквизита - Строка - имя реквизита формы, содержащего данные PDF документа.
//
Процедура ВывестиPDFДокументНаФорму(Форма, ИмяРодителя, ИмяЭлемента = "ДокументPDFПоле",
		ИмяРеквизита = "ДокументPDFРеквизит") Экспорт
	...
	ПолеPDF = Элементы.Вставить(ИмяЭлемента, Тип("ПолеФормы"), Элементы.Найти(ИмяРодителя), ГруппаКоманд);
	ПолеPDF.Вид = ВидПоляФормы.ПолеPDFДокумента;
	ПолеPDF.ПутьКДанным = ИмяРеквизита;
...
// Обработчик обновления БИД на версию 3.0.4.2. Обрабатывает записи табличной части ПравилаЗаполненияРеквизитовИС,
// очищает значение реквизита РежимИзмененияДанныхПроведенногоДокумента для справочников.
//
Процедура ПерейтиНаВерсию_3_0_4_2() Экспорт
	...
	ЗапросПравила = Новый Запрос(
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПравилаИнтеграцииС1СДокументооборотом3.ПравилаЗаполненияРеквизитовИС КАК ПравилаИнтеграцииС1СДокументооборотом3ПравилаЗаполненияРеквизитовИС
		|		ПО (ПравилаИнтеграцииС1СДокументооборотом3.Ссылка = ПравилаИнтеграцииС1СДокументооборотом3ПравилаЗаполненияРеквизитовИС.Ссылка)
		|ГДЕ
		|	НЕ ПравилаИнтеграцииС1СДокументооборотом3.ПометкаУдаления
		|	И ПравилаИнтеграцииС1СДокументооборотом3.ТипОбъектаИС ПОДОБНО ""Справочник.%""
		|	И ПравилаИнтеграцииС1СДокументооборотом3ПравилаЗаполненияРеквизитовИС.РежимИзмененияДанныхПроведенногоДокумента <> &ПустойРежим");
	...
	Пока ВыборкаПравила.Следующий() Цикл
		...
		Для Каждого Строка Из ПравилоОбъект.ПравилаЗаполненияРеквизитовИС Цикл
			Строка.РежимИзмененияДанныхПроведенногоДокумента = ПустойРежим;
		КонецЦикла;
...
Процедура ПроверитьСоответствиеПравилИнтеграцииМетаданным() Экспорт
	...
	Пока ВыборкаПравила.Следующий() Цикл
		...
		Иначе
			...
			КоличествоСтрок = ПравилоОбъект.ПравилаЗаполненияРеквизитовИС.Количество();
			Для Номер = 1 По КоличествоСтрок Цикл
				
				ИндексСтроки = КоличествоСтрок - Номер;
				Правило = ПравилоОбъект.ПравилаЗаполненияРеквизитовИС[ИндексСтроки];
				...
				Если Правило.Вариант <> Перечисления.ВариантыПравилЗаполненияРеквизитов.РеквизитОбъекта
						...
						Или ЗначениеЗаполнено(Правило.Таблица)
						Или Правило.ИмяРеквизитаОбъектаИС = "Представление"
						Или ОбъектМетаданных.Реквизиты.Найти(Правило.ИмяРеквизитаОбъектаИС) <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
				...
				ПравилоОбъект.ПравилаЗаполненияРеквизитовИС.Удалить(ИндексСтроки);
...
Функция УзелТекущегоПользователя() Экспорт
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(НастройкиАвторизации.ИмяПользователя) Тогда
			ВызватьИсключение
				НСтр("ru = 'Не установлено значение константы ""Имя пользователя для обмена с 1С:Документооборотом""'");
		КонецЕсли;
...
Процедура ДобавитьОписаниеДопРеквизитов(СписокРеквизитовXDTO, СписокРеквизитов, Синонимы)
	...
	Для Каждого Реквизит Из СписокРеквизитов Цикл
		...
		РеквизитXDTO.synonym = СтрШаблон(НСтр("ru = 'Доп. реквизит: %1'"), Реквизит.Наименование);
...
Процедура ДобавитьОписаниеТиповРеквизита(СписокТиповXDTO, ТипРеквизита, Синонимы)
	...
	Если ТипЗнч(ТипРеквизита) <> Тип("ОписаниеТипов") Тогда
		СписокТиповXDTO.Добавить(НСтр("ru = 'Неопределено'"));
		Возврат;
	КонецЕсли;
	
	Для Каждого ОписаниеТипа Из ТипРеквизита.Типы() Цикл
...
// Возвращает объект DMILBatchResponse, содержащий результаты выполнения пакетного запроса.
//
// Параметры:
//   Сообщение - ОбъектXDTO - объект XDTO типа DMILBatchRequest.
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMILBatchResponse или DMILError.
//
Функция ОбработатьПакет(Сообщение)
	
	Попытка
		
		ПакетОтветов = СоздатьОбъектБИД("DMILBatchResponse");
...
Функция ПроверитьВыражениеНаВстроенномЯзыке(Сообщение)
	...
	Попытка
		...
		Если СокрЛП(Сообщение.expressionOnBuiltInLanguage) <> "" Тогда
			...
			Если (Сообщение.forTabularSection = Истина) И ТипЗнч(Параметры.Результат) <> Тип("ТаблицаЗначений") Тогда
				ВызватьИсключение СтрШаблон(
...
Функция СинхронизироватьПользователейИнтегрированнойСистемыИПользователейДО(Сообщение)
	...
	Исключение
		...
		КонецЕсли;
		Возврат ОписаниеОшибкиXDTO(
			СтрШаблон(НСтр("ru = 'Ошибка при синхронизации пользователей ДО с пользователями %1'"),
				ИнтеграцияС1СДокументооборотБазоваяФункциональность.СокращенноеНаименованиеКонфигурации()),
			ПолучитьОписаниеОшибки(ИнформацияОбОшибке()));
...
Функция СтруктураМетаданныхИнтегрированнойСистемы(Сообщение)
	...
	Попытка
		...
		ПолучатьПравилаИнтеграции = Ложь;
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(Сообщение, "columnSet")
			...
			Для Каждого РеквизитКПолучению Из Сообщение.columnSet Цикл
				...
				ИначеЕсли НРег(РеквизитКПолучению) = "integrationrules" Тогда
					ПолучатьПравилаИнтеграции = Истина;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		
		Ответ.dataBaseName = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СокращенноеНаименованиеКонфигурации();
		...
		Для Каждого ОбъектМетаданных Из ОбъектыПоддерживающиеИнтеграцию Цикл
			...
			ПравилаИнтеграции = ОбъектМетаданныхXDTO.integrationRules; // СписокXDTO
			...
			Если ПолучатьВсе Или ПолучатьПравилаИнтеграции Тогда
				ПодходящиеПравила =
					ИнтеграцияС1СДокументооборот3ВызовСервера.ПодходящиеПравилаИнтеграцииПоТипуОбъектаИС(ПолноеИмя);
				Для Каждого ДанныеПравила Из ПодходящиеПравила Цикл
					ПравилоИнтеграцииXDTO = СоздатьОбъектБИД("DMILIntegrationRule");
					ПравилоИнтеграцииXDTO.objectID = ПолучитьDMILObjectIDПоСсылке(ДанныеПравила.Ссылка);
					ПравилоИнтеграцииXDTO.objectTypeDM = ДанныеПравила.ТипОбъектаДО;
					ПравилоИнтеграцииXDTO.documentTypeID = ДанныеПравила.ИдентификаторВидаДокумента;
					ПравилоИнтеграцииXDTO.documentType = ДанныеПравила.ПредставлениеВидаДокумента;
					
					ПравилаИнтеграции.Добавить(ПравилоИнтеграцииXDTO);
				КонецЦикла;
			КонецЕсли;
		...
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(СтрокаXDTO, "application") Тогда
			...
			Иначе
				...
				Если ЗначениеЗаполнено(НоваяСтрока.Ссылка) Тогда
					...
					Попытка
						НоваяСтрока.ПредставлениеПриложения = СтрШаблон(НСтр("ru = '%1, объект %2'"),
			...
			КонецЕсли;
		ИначеЕсли НоваяСтрока.ТипСтроки = "task" Тогда
			ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьОбъектныйРеквизит(
				НоваяСтрока,
				ОбъектXDTO,
				"Приложение");
...
Процедура ДополнитьПодходящиеОбъектыДляСозданияСвязей(ЗначениеРеквизитаИС, ПодходящиеОбъекты, НастройкаСозданияСвязей,
		ЗначенияКВыгрузке)
	
	Если Не ЗначениеЗаполнено(ЗначениеРеквизитаИС) Тогда
		Возврат;
	КонецЕсли;
	
	СуществующиеСвязи = ПодходящиеОбъекты.НайтиСтроки(Новый Структура("Ссылка", ЗначениеРеквизитаИС));
	Для Каждого СуществуюаяСвязь Из СуществующиеСвязи Цикл
		Если СуществуюаяСвязь.ТипСвязи.ID = НастройкаСозданияСвязей.relationType.ObjectID.ID Тогда
			// Такая связь уже есть.
			Возврат;
		КонецЕсли;
	КонецЦикла;
	
	Если ЗначенияКВыгрузке[ЗначениеРеквизитаИС] = Неопределено Тогда
		ЗначенияКВыгрузке[ЗначениеРеквизитаИС] = Новый Массив;
	КонецЕсли;
	
	СвязанныйОбъектДО =
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ДанныеОбъектаДОПоВнешнемуОбъекту(
			ЗначениеРеквизитаИС);
	// Даже если значение реквизита ИС уже имеет связанный объект в ДО, правило интеграции все равно нужно подобрать,
	// т.к. ссылки на объект ДО недостаточно чтобы определить, какая именно настройка создания связи подходит.
	...
	ПодходящиеПравилаИнтеграции = ИнтеграцияС1СДокументооборот3ВызовСервера.ПодходящиеПравилаИнтеграцииОбъекта(
		ЗначениеРеквизитаИС,
		Истина);
	ВидДокументаДОИзНастройки = "";
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(НастройкаСозданияСвязей, "documentType") Тогда
		ВидДокументаДОИзНастройки = НастройкаСозданияСвязей.documentType.ObjectID.ID;
	КонецЕсли;
	ПредставлениеСвязиМассив = Новый Массив;
	ПредставлениеСвязиМассив.Добавить(НастройкаСозданияСвязей.relationType.name);
	Если ЗначениеЗаполнено(НастройкаСозданияСвязей.comment) Тогда
		ПредставлениеСвязиМассив.Добавить(НастройкаСозданияСвязей.comment);
	КонецЕсли;
	ПредставлениеСвязи = СтрСоединить(ПредставлениеСвязиМассив, ", ");
	Если ПодходящиеПравилаИнтеграции.Количество() = 1 Тогда
		ПравилоИнтеграции = ПодходящиеПравилаИнтеграции[0];
		Если ПравилоИнтеграции.ТипОбъектаДО <> НастройкаСозданияСвязей.objectType.xdtoClassName Тогда
			ЗначенияКВыгрузке[ЗначениеРеквизитаИС].Добавить(СтрШаблон(
				НСтр("ru = 'Создание связи ""%1"". Тип объекта ДО ""%2"", указанный в правиле интеграции, не совпадает с типом объекта ДО ""%3"", указанным в настройке автоматического создания связей.'"),
				ПредставлениеСвязи,
				ПравилоИнтеграции.ТипОбъектаДО,
				НастройкаСозданияСвязей.objectType.xdtoClassName));
			ПравилоИнтеграции = Неопределено;
		КонецЕсли;
		Если ПравилоИнтеграции.ВидДокументаДОID <> ВидДокументаДОИзНастройки Тогда
			ЗначенияКВыгрузке[ЗначениеРеквизитаИС].Добавить(СтрШаблон(
				НСтр("ru = 'Создание связи ""%1"". Вид документа ""%2"", указанный в правиле интеграции, не совпадает с видом документа ""%3"", указанным в настройке автоматического создания связей.'"),
				ПредставлениеСвязи,
				ПравилоИнтеграции.ВидДокументаДО,
				НастройкаСозданияСвязей.documentType.name));
			ПравилоИнтеграции = Неопределено;
		КонецЕсли;
	Иначе
		ЗначенияКВыгрузке[ЗначениеРеквизитаИС].Добавить(СтрШаблон(
			НСтр("ru = 'Создание связи ""%1"". Не удалось определить подходящее правило интеграции. Количество найденных правил: %2.'"),
			ПредставлениеСвязи,
			ПодходящиеПравилаИнтеграции.Количество()));
	КонецЕсли;
	Если ПравилоИнтеграции = Неопределено Тогда
		// Настройка создания связи не подходит.
		Возврат;
	КонецЕсли;
	
	ПодходящийОбъект = ПодходящиеОбъекты.Добавить();
	ПодходящийОбъект.Ссылка = ЗначениеРеквизитаИС;
	ПодходящийОбъект.СвязанныйОбъектДО = СвязанныйОбъектДО;
	ПодходящийОбъект.ПравилоИнтеграции = ПравилоИнтеграции;
	ПодходящийОбъект.ТипСвязи =
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО(
			НастройкаСозданияСвязей.relationType.ObjectID.ID, НастройкаСозданияСвязей.relationType.ObjectID.type);
	ПодходящийОбъект.Комментарий = НастройкаСозданияСвязей.comment;
...
Процедура ЗаполнитьДанныеИзСпискаВыражений(Прокси, ОбъектИС, Реквизиты, ТабличныеЧасти, СписокВыражений)
	...
	Если ТипЗнч(СписокВыражений) <> Тип("СписокXDTO") И ТипЗнч(СписокВыражений) <> Тип("Массив") Тогда
		ВызватьИсключение
			НСтр("ru = 'Не удалось получить список выражений, необходимых для выгрузки объекта в 1С:Документооборот.
	...
	Для Каждого Выражение Из СписокВыражений Цикл
		...
		Если Выражение.type = "Attribute" Тогда
			...
			РезультатВыражения = РезультатВыраженияНаВстроенномЯзыке(
				...
				ОбъектИС);
			Реквизиты.Добавить(
				ПреобразоватьРеквизитОбъектаИСВОбъектXDTO(
					Прокси, Выражение.id, РезультатВыражения, Выражение.expressionOnBuiltInLanguage));
...
Процедура ЗаполнитьИнтерактивноеДействие(Прокси, ИнтерактивноеДействие, ИнтерактивноеДействиеXDTO)
	...
	ИначеЕсли НРег(ИнтерактивноеДействиеXDTO.interactiveAction) = "performsigning" Тогда
		ИнтерактивноеДействие.ВыполнитьПодписание = Истина;
		ИнтерактивноеДействие.Контекст = Новый Структура;
		ИнтерактивноеДействие.Контекст.Вставить("ДатаИсполнения", ИнтерактивноеДействиеXDTO.context.executionDate);
		ИнтерактивноеДействие.Контекст.Вставить("Операция", ИнтерактивноеДействиеXDTO.context.operation);
		ИнтерактивноеДействие.Контекст.Вставить("ЗаголовокДанных", ИнтерактивноеДействиеXDTO.context.title);
		ИнтерактивноеДействие.Контекст.Вставить("ОбъектыПодписания", Новый Массив);
		Для Каждого ДанныеИПодпись Из ИнтерактивноеДействиеXDTO.context.dataAndSignatureList Цикл
			ОбъектДО = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО(
				ДанныеИПодпись.object.ID,
				ДанныеИПодпись.object.type,,
				ДанныеИПодпись.object.presentation);
			ИнтерактивноеДействие.Контекст.ОбъектыПодписания.Добавить(
				Новый Структура("ОбъектДО, Данные", ОбъектДО, ДанныеИПодпись.binaryData));
		КонецЦикла;
...
// Используется для автоматического добавления связей по ссылкам из объекта ИС, при его выгрузке в ДО.
//
// Параметры:
//   Прокси - WSПрокси - объект для подключения к web-сервисам Документооборота.
//   СвязиКСозданию - Массив из см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.СвязьКСозданию
//   ОбъектИС - ЛюбаяСсылка - ссылка на исходный объект ИС.
//   СписокАвтоматическиДобавляемыхСвязей - СписокXDTO - список настроек создания связей, заданный в правилах
//     загрузки данных в ДО.
//   НовыеОбъектыДО - Массив из см. ИнтеграцияС1СДокументооборот3КлиентСервер.НовыйОбъектДО
//
Процедура ЗаполнитьСписокСвязиКСозданию(Прокси, СвязиКСозданию, ОбъектИС, СписокАвтоматическиДобавляемыхСвязей,
		НовыеОбъектыДО)
	
	Если НовыеОбъектыДО = Неопределено Тогда
		НовыеОбъектыДО = Новый Массив;
	КонецЕсли;
	
	Если СписокАвтоматическиДобавляемыхСвязей = Неопределено
			Или СписокАвтоматическиДобавляемыхСвязей.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		ПодходящиеОбъектыДляДобавленияСвязей = ПодходящиеОбъектыДляДобавленияСвязей(
			ОбъектИС,
			СписокАвтоматическиДобавляемыхСвязей,
			ИмяСобытия);
		Для Каждого ПодходящийОбъект Из ПодходящиеОбъектыДляДобавленияСвязей Цикл
			
			СвязьКСозданию = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.СвязьКСозданию();
			
			Если ПодходящийОбъект.СвязанныйОбъектДО = Неопределено Тогда
				// Создадим новый объект в ДО.
				НовыйОбъектДОИлиСообщение = СоздатьОбъектДОПоПравилу(
					Прокси,
					ПодходящийОбъект.Ссылка,
					ПодходящийОбъект.ПравилоИнтеграции,,,
					НовыеОбъектыДО);
				Если ТипЗнч(НовыйОбъектДОИлиСообщение) = Тип("ОбъектXDTO") Тогда
					СвязьКСозданию.СсылкаНа = НовыйОбъектДОИлиСообщение.name;
					СвязьКСозданию.СсылкаНаID = НовыйОбъектДОИлиСообщение.objectID.ID;
					СвязьКСозданию.СсылкаНаТип = НовыйОбъектДОИлиСообщение.objectID.type;
					НовыйОбъектДО = ИнтеграцияС1СДокументооборот3КлиентСервер.НовыйОбъектДО(
						НовыйОбъектДОИлиСообщение.objectID.ID,
						НовыйОбъектДОИлиСообщение.objectID.type,
						ПодходящийОбъект.Ссылка);
					НовыеОбъектыДО.Добавить(НовыйОбъектДО);
				Иначе
					ТекстОшибки = СтрШаблон(
						НСтр("ru = 'Не удалось выгрузить объект ""%1"" в 1С:Документооборот:
							|%2'"),
						ПодходящийОбъект.Ссылка,
						НовыйОбъектДОИлиСообщение);
					ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаписатьОшибку(ТекстОшибки, ИмяСобытия);
					Продолжить;
				КонецЕсли;
			Иначе
				// Добавим связь к существующему объекту ДО.
				СвязьКСозданию.СсылкаНа = ПодходящийОбъект.СвязанныйОбъектДО.name;
				СвязьКСозданию.СсылкаНаID = ПодходящийОбъект.СвязанныйОбъектДО.ID;
				СвязьКСозданию.СсылкаНаТип = ПодходящийОбъект.СвязанныйОбъектДО.type;
			КонецЕсли;
			
			СвязьКСозданию.ТипСвязиID = ПодходящийОбъект.ТипСвязи.ID;
			
			СвязьКСозданию.Комментарий = ПодходящийОбъект.Комментарий;
			
			СвязиКСозданию.Добавить(СвязьКСозданию);
...
Функция ПреобразоватьРеквизитОбъектаИСВОбъектXDTO(Прокси, ИмяРеквизита, ЗначениеРеквизита, Выражение = "")
	...
	Если ТипРеквизита = Тип("Неопределено") Или ТипРеквизита = Тип("Null") Тогда
	...
	Иначе
		ТекстОшибки = Новый Массив;
		ТекстОшибки.Добавить(НСтр("ru = 'Ошибка формирования структуры загружаемых в 1С:Документооборот данных.'"));
		Если ЗначениеЗаполнено(Выражение) Тогда
			ТекстОшибки.Добавить(СтрШаблон(
				НСтр("ru = 'Невозможно выгрузить данные из результата выражения на встроенном языке.
					|Тип результата: ""%1"".
					|Текст выражения на встроенном языке:
					|%2'"),
				ТипРеквизита,
				Выражение));
		Иначе
			ТекстОшибки.Добавить(СтрШаблон(
				...
				ТипРеквизита));
		КонецЕсли;
		Результат.error = СтрСоединить(ТекстОшибки, Символы.ПС);
...
Функция ПреобразоватьТаблицуОбъектаИСВОбъектXDTO(Прокси, ИмяТаблицы, ТаблицаЗначений, Колонки, Выражение = "")
	...
	Для Каждого Строка Из ТаблицаЗначений Цикл
		...
		Для Каждого Колонка Из Колонки Цикл
			РеквизитыСтроки.Добавить(
				ПреобразоватьРеквизитОбъектаИСВОбъектXDTO(Прокси, Колонка.Имя, Строка[Колонка.Имя], Выражение));
		КонецЦикла;
...
// Конструктор структуры поля картинки.
//
// Параметры:
//   ПолеКартинкиXDTO - ОбъектXDTO - объект XDTO типа DMPictureField:
//     * visible - Булево
//     * hint - Строка
//     * imageIndex - Число
//
...
//     * ИндексКартинки - Число
//
Функция СтруктураПоляКартинки(ПолеКартинкиXDTO)
	
	СтруктураПоляКартинки = Новый Структура;
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ПолеКартинкиXDTO, "visible") Тогда
		СтруктураПоляКартинки.Вставить("Видимость", ПолеКартинкиXDTO.visible);
	Иначе
		СтруктураПоляКартинки.Вставить("Видимость", Истина);
	КонецЕсли;
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ПолеКартинкиXDTO, "hint") Тогда
		СтруктураПоляКартинки.Вставить("Подсказка", ПолеКартинкиXDTO.hint);
	Иначе
		СтруктураПоляКартинки.Вставить("Подсказка", "");
	КонецЕсли;
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ПолеКартинкиXDTO, "imageIndex") Тогда
		СтруктураПоляКартинки.Вставить("ИндексКартинки", ПолеКартинкиXDTO.imageIndex);
	Иначе
		СтруктураПоляКартинки.Вставить("ИндексКартинки", -1);
	КонецЕсли;
	
	Возврат СтруктураПоляКартинки;
...
// Возвращает строку даты события обработки форматированную по принципу:
//   - 10:35, если событие произошло сегодня
//   - 11.03.2026 10:35, если событие произошло не сегодня
//
// Параметры:
...
//   Строка
//
Функция ФорматированнаяДатаСобытияОбработки(ДатаСобытия)
	
	ТекущееВремя = ТекущаяДатаСеанса();
	СобытиеПроизошлоСегодня = (НачалоДня(ДатаСобытия) = НачалоДня(ТекущееВремя));
	
	Если СобытиеПроизошлоСегодня Тогда
		ФорматДаты = НСтр("ru = 'ДФ=''HH:mm'''");
	Иначе
		ФорматДаты = НСтр("ru = 'ДФ=''dd.MM.yyyy HH:mm'''");
	КонецЕсли;
	
	Возврат Формат(ДатаСобытия, ФорматДаты);
