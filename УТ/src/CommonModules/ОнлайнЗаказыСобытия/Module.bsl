...
Процедура ПриИзмененииИспользованияНастройкиОплаты(
	...
	Если СпособОплаты = Перечисления.СпособыОплатыОнлайнЗаказов.ЯндексПэй
		И Не ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПлатежныеСистемы.ЯндексПэй") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ОнлайнЗаказыСлужебный.ПриДобавленииОбработчиковОбновления(Обработчики);
...
Процедура ПриФормированииПараметровОплат(
	...
	Для Каждого НастройкаОплаты Из НастройкиОплат Цикл
		...
		ИначеЕсли НастройкаОплаты.Ключ = Перечисления.СпособыОплатыОнлайнЗаказов.ЯндексПэй
			И ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПлатежныеСистемы.ЯндексПэй") Тогда
			
			МодульПлатежныеСистемыСлужебный = ОбщегоНазначения.ОбщийМодуль("ПлатежныеСистемыСлужебный");
			ПараметрыОплат.ЯндексПэй = МодульПлатежныеСистемыСлужебный.ПараметрыОплаты(
				ДокументЗаказа,
				НастройкаОплаты.Значение);
			
			ПараметрыОплат.ЯндексПэй.Вставить("НастройкаПодключения", НастройкаОплаты.Значение);
...
Процедура ПриЗаполненииНастроекОплатыФизическихЛиц(
	...
	Для Каждого НастройкаОплаты Из НастройкиОплатПодключения Цикл
		...
		Если НастройкаОплаты.Ключ = Перечисления.СпособыОплатыОнлайнЗаказов.СБПc2b
			...
			Если НастройкиОплатСервиса.СБП = Неопределено Тогда
				НастройкиОплатСервиса.СБП = Новый Структура;
			КонецЕсли;
			
			НастройкиОплатСервиса.СБП.Вставить(
				ОнлайнЗаказыСлужебный.ИдентификаторВариантаСБПC2B(),
		...
		ИначеЕсли НастройкаОплаты.Ключ = Перечисления.СпособыОплатыОнлайнЗаказов.ЯндексПэй
			...
			Если ПараметрыОплатЗаказа.ЯндексПэй <> Неопределено Тогда
				НастройкиОплаты.НазначениеПлатежа = ПараметрыОплатЗаказа.ЯндексПэй.НазначениеПлатежа;
			КонецЕсли;
			
			НастройкиОплатСервиса.ЯндексПэй = НастройкиОплаты;
...
Процедура ПриЗаполненииНастроекОплатыЮридическихЛиц(
	...
	Для Каждого НастройкаОплаты Из НастройкиОплатПодключения Цикл
		Если НастройкаОплаты.Ключ = Перечисления.СпособыОплатыОнлайнЗаказов.СБПb2b
			...
			НастройкиОплатСервиса.СБП.Вставить(
				ОнлайнЗаказыСлужебный.ИдентификаторВариантаСБПB2B(),
				НастройкиОплатыСБПb2b);
...
//    * ОплатаЯндексПэй - Структура, Неопределено - Содержит данные операции оплаты, в том случае когда
//        значение Свойства СпособОплаты = Перечисления.СпособыОплатыОнлайнЗаказов.ЯндексПэй:
...
Процедура ПриЗаполненииПараметровОплаты(
	...
	Если ДанныеЗаказа.СпособОплаты = Перечисления.СпособыОплатыОнлайнЗаказов.ЯндексПэй
		И ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПлатежныеСистемы.ЯндексПэй") Тогда
		МодульЯндексПэйСлужебный = ОбщегоНазначения.ОбщийМодуль("ЯндексПэйСлужебный");
		МодульЯндексПэйСлужебный.ЗаполнитьПараметрыОплаты(
			ДанныеЗаказа,
			НастройкаОплаты,
			ПараметрыОплаты);
	КонецЕсли;
...
Процедура ПриЗаписиДанныхОплаты(
	...
	ИначеЕсли СпособОплаты = Перечисления.СпособыОплатыОнлайнЗаказов.ОзонПэй
		...
		УстановитьПривилегированныйРежим(Истина);
		МодульПлатежныеСистемыСлужебный.ЗаписатьДанныеОплаты(
			ДокументЗаказа,
			ПараметрыОзон.РезультатОбработки.ПараметрыОперации,
			ПараметрыОзон.НастройкаПодключения);
		УстановитьПривилегированныйРежим(Ложь);
		
	ИначеЕсли СпособОплаты = Перечисления.СпособыОплатыОнлайнЗаказов.ЯндексПэй
		И ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПлатежныеСистемы.ЯндексПэй") Тогда

		ПараметрыЯндекс = ПараметрыОплаты.Получить("ЯндексПэй");
		...
		МодульПлатежныеСистемыСлужебный.ЗаписатьДанныеОплаты(
			ДокументЗаказа,
			ПараметрыЯндекс.РезультатОбработки.ПараметрыОперации,
			ПараметрыЯндекс.НастройкаПодключения);
		УстановитьПривилегированныйРежим(Ложь);
...
Процедура ПриПолученииДанныхОплат(
	...
	ИначеЕсли СпособОплаты = Перечисления.СпособыОплатыОнлайнЗаказов.ЯндексПэй
		...
		УстановитьПривилегированныйРежим(Истина);
		ДанныеОпераций = МодульЯндексПэйСлужебный.ДанныеОпераций(ОплаченныеЗаказы);
		УстановитьПривилегированныйРежим(Ложь);
...
Процедура ПриПолученииТокена(РезультатОперации, ВидТокена, НастройкаПодключения, СпособОплаты) Экспорт
	...
	ИначеЕсли ВидТокена = ОнлайнЗаказыСлужебный.ИдентификаторВидаТокенаПриемОплат()
		И (СпособОплаты = Перечисления.СпособыОплатыОнлайнЗаказов.ОзонПэй
		Или СпособОплаты = Перечисления.СпособыОплатыОнлайнЗаказов.ЯндексПэй) Тогда
	...
	Для Каждого НастройкиПодключенияПоВидуТокена Из ИспользуемыеНастройкиПодключения Цикл
		...
		ИначеЕсли НастройкиПодключенияПоВидуТокена.Ключ = ОнлайнЗаказыСлужебный.ИдентификаторВидаТокенаПриемОплат() Тогда
			...
			Для Каждого НастройкиПоСпособуОплаты Из НастройкиОплат Цикл
				...
				ИначеЕсли НастройкиПоСпособуОплаты.Ключ = Перечисления.СпособыОплатыОнлайнЗаказов.ЯндексПэй
					И ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПлатежныеСистемы.БазоваяФункциональностьПлатежныхСистем") Тогда
...
Процедура ПриПроверкеПараметровОплат(
	...
	Если ПараметрыОплат.ЯндексПэй <> Неопределено Тогда
		
		// У Яндекс Пэй информация попадает не в назначение платежа, а в комментарий.
		// В описании АПИ нет ограничений.
		
		МодульНастройкиПлатежныхСистемСлужебный.ПроверитьНазначениеПлатежа(
			ПараметрыОплат.ЯндексПэй.НазначениеПлатежа,
			РезультатОперации);
