...
//	ЗаполнитьСтавкуНДС - Булево - признак заполнения Ставки НДС по настройке Комитента. Значение по умолчанию Ложь.
//
Процедура ЗаполнитьТоварыПоОстаткамКОформлениюОтчетовКомитентуОСписании(Объект, ЕстьНомерГТД = Ложь, ЗаполнитьСтавкуНДС = Ложь) Экспорт
...
Процедура ЗаполнитьПоТоварамКОформлениюОтчетовКомитентуЗаПериод(Объект,
																...
																КонецПериода,
																ЕстьНомерГТД = Ложь,
																ИспользоватьНоменклатуруПартнера = Ложь,
																ЗаполнитьСтавкуНДС = Ложь) Экспорт
	
	ДокументПродажи = Документы.РеализацияТоваровУслуг.ПустаяСсылка();
	
	ТипОбъекта = ТипЗнч(Объект);
	Если ЗаполнитьСтавкуНДС
		Или ((ТипОбъекта = Тип("Структура")
			Или ТипОбъекта = Тип("ДанныеФормыСтруктура"))
	...
	Если ЗаполнитьСтавкуНДС
		...
		СтруктураПараметров.Вставить("СтруктураДействий", СтруктураДействий);
	...
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		...
		Если ДокументыКПересчету.Количество() > 0 Тогда
			Если ПланыОбмена.ГлавныйУзел() = Неопределено Тогда
				НеоперативнаяОчередьЗаданий.ВыполнитьЗаданияНеоперативнойОчередиПоСпискуДокументов(ДокументыКПересчету,
...
Процедура ЗапросПоТоварамУслугамКОформлениюОтчетовКомитентуЗаПериод(СтруктураПараметров, МенеджерВременныхТаблиц) Экспорт
	...
	|			ИНАЧЕ -ТоварыКОформлению.СуммаВыручкиКОформлению
	|		КОНЕЦ)                                      КАК СуммаВыручки,
	|	РеализацияТоваровУслугТовары.ЗаказКлиента       КАК ДокументРеализации,
	|	ТоварыКОформлению.ДатаСчетаФактуры              КАК ДатаСчетаФактуры,
	|	ТоварыКОформлению.НомерСчетаФактуры             КАК НомерСчетаФактуры,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОформлению.ДокументРеализации) = ТИП(Документ.РеализацияТоваровУслуг)
	...
	|		ПО РеализацияТоваровУслугТовары.Ссылка = ТоварыКОформлению.ДокументРеализации
	|			И РеализацияТоваровУслугТовары.Номенклатура = ТоварыКОформлению.Номенклатура
	|			И РеализацияТоваровУслугТовары.Характеристика = ТоварыКОформлению.Характеристика
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК КлючиАналитикиУчетаНоменклатуры
	|		ПО ТоварыКОформлению.АналитикаУчетаНоменклатуры = КлючиАналитикиУчетаНоменклатуры.Ссылка
	|
...
Процедура ЗаполнитьТЧПоТоварамКОформлениюОтчетовКомитентуЗаПериод(ТабличнаяЧасть,
	...
	СтруктураДействий = Новый Структура;
	ПараметрыДействия = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(ПараметрыЗаполнения);
	
	Если ПараметрыЗаполнения.Свойство("Партнер")
		И ТипЗнч(ПараметрыЗаполнения.Партнер) = Тип("СправочникСсылка.Организации") Тогда
		
		ПараметрыДействия.Организация = ПараметрыЗаполнения.Партнер;
		
	КонецЕсли;
	
	СтруктураДействий.Вставить("ЗаполнитьСтавкуНДС", ПараметрыДействия);
	
	Если ПараметрыЗаполнения.Свойство("СтруктураДействий") Тогда
	 	ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(СтруктураДействий, ПараметрыЗаполнения.СтруктураДействий);
	КонецЕсли;
	
	ВыборкаПоНоменклатуре = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	ДанныеЕстьСуммаПродажиНДС = Новый Соответствие;
			...
			Если ВыборкаПоХарактеристике.КоличествоОстаток >= ВыборкаПоХарактеристике.Количество Тогда
				...
				Пока Выборка.Следующий() Цикл
					ДобавитьСтрокуПоТоварамКОформлениюОтчетовКомитентуЗаПериод(ТабличнаяЧасть,
																				...
																				СтруктураДействий,
																				КэшированныеЗначения);
					ДанныеЕстьСуммаПродажиНДС.Вставить(
						ТабличнаяЧасть[ТабличнаяЧасть.Количество()-1],
						Выборка.ЕстьСуммаПродажиНДС);
				КонецЦикла;
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(
		ТабличнаяЧасть, СтруктураДействий, КэшированныеЗначения);
		
	Для Каждого СтрокаТЧ Из ДанныеЕстьСуммаПродажиНДС Цикл
		Если СтрокаТЧ.Значение Тогда
			СтрокаТЧ.Ключ.СуммаПродажиНДС = СуммаПродажиНДС(
				СтрокаТЧ.Ключ.СуммаПродажи, СтрокаТЧ.Ключ.СтавкаНДС);
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьТоварыПоОстаткамПереданныхНаКомиссию(Объект, КонецПериода, ЕстьСуммаПродажи) Экспорт
	...
	ПараметрыСтавкиНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект);
...
Процедура ЗаполнитьТоварыПоОстаткамПереданныхНаКомиссиюПоНовойСхеме(Объект, ОтчетКомиссионера = Истина) Экспорт
	...
	ПараметрыЗаполнитьСтавкуНДС.Дата = ДатаДокумента;
	...
	Если ЦенаВключаетНДС = Объект.ЦенаВключаетНДС Тогда
		...
		ПараметрыЗаполненияЦеныПродажи = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияЦеныВСтрокеТЧ(Объект);
		...
		СтруктураДействийРасчетаЦен.Вставить("ЗаполнитьСтавкуНДС", ПараметрыЗаполнитьСтавкуНДС);
		
		ПараметрыЗаполнитьСтавкуНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДСПоВидуЦен(Объект);
		ПараметрыЗаполнитьСтавкуНДС.Дата = ДатаДокумента;
		СтруктураДействийРасчетаЦен.Вставить("ЗаполнитьСтавкуНДСПоВидуЦен", ПараметрыЗаполнитьСтавкуНДС);
...
// Подсказка при выборе версии комиссионной торговли.
// 
// Возвращаемое значение:
//  Строка - Подсказка при выборе версии комиссионной торговли
Функция ПодсказкаПриВыбореВерсииКомиссионнойТорговли() Экспорт
	
	ПодсказкаШаблон = НСтр(
		"ru = '%2 – используется только модель комиссии %2, документы по комиссии %1 доступны для просмотра/редактирования;
		|%1/%2 – используются обе модели. Ввод новых документов/справочников возможен как для модели %1, так и для %2.'");
	
	Подсказка = СтрШаблон(ПодсказкаШаблон, ПостфиксСхемыКомиссии20(), ПостфиксСхемыКомиссии25());
		
	Возврат Подсказка;
	
КонецФункции

// Комментарий об использовании устаревшей версии комиссионной торговли.
// 
// Возвращаемое значение:
//  Строка - Предупреждение об устаревшей функциональности.
//
Функция КомментарийИспользованиеУстаревшейКомиссии() Экспорт
	
	ПодсказкаШаблон = НСтр("ru = 'Внимание! Функциональность ""Комиссионных продаж версии %1"" устарела и будет удалена в версии %2. Пожалуйста, перейдите на ""Комиссионные продажи версии %3"".'");
	
	Постфикс20 = ПостфиксСхемыКомиссии20();
	Постфикс25 = ПостфиксСхемыКомиссии25();
	ВерсияУдаления = СтрЗаменить(Постфикс20, ".0", ".6.3");
	
	Подсказка = СтрШаблон(ПодсказкаШаблон, Постфикс20, ВерсияУдаления, Постфикс25);
