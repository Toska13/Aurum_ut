...
Функция ПолучитьОтпечаткиСертификатовНаСервере(ПоказыватьОшибку = Ложь) Экспорт
	...
	Если МенеджерКриптографии = Неопределено Тогда
		...
		Возврат РезультатПолучения;
	КонецЕсли;
	КлючЗамера = "ПолучитьХранилищеСертификатов.Сервер";
...
Функция ПроверитьСертификат(МенеджерКриптографии, Сертификат, КонтекстДиагностики, Подпись = Неопределено) Экспорт
	...
	КлючЗамера = "КриптографияБЭД.ПроверитьСертификат.Сервер";
	ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер(КлючЗамера);
	...
	РезультатПроверки = КриптографияБЭДКлиентСервер.ПроверитьКвалифицированныйСертификат(Сертификат, КонтекстПроверки);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПолучитьДистрибутивCryptoProCSP", Истина);
