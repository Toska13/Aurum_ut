...
Функция УдалитьДокументПриОтменеФискальнойОперации(Ссылка) Экспорт
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
			...
			Иначе
				...
				Если Не ДокументОбъект = Неопределено Тогда
					...
					Исключение
						...
						СтрокиТекстаОшибки.Добавить(СтрШаблон(НСтр("ru = 'по причине: %4'", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()))));
		...
		Если СтрокиТекстаОшибки.Количество() Тогда
			ИмяСобытияЖурнала = НСтр("ru = 'Розничные продажи'", ОбщегоНазначения.КодОсновногоЯзыка());
...
//
// Параметры:
//	ДанныеТовара - Структура - коллекция, содержащая сведения о товаре.
//
// Возвращаемое значение:
//	Массив Из Строка - штрихкоды номенклатуры.
//
Функция ПолучитьШтрихкодыНоменклатуры(ДанныеТовара) Экспорт
	...
	НаборУпаковок = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеТовара.Номенклатура, "НаборУпаковок");
	...
	Запрос.УстановитьПараметр("Владелец",		ДанныеТовара.Номенклатура);
	...
	ЗаполнитьЗначенияСвойств(ПараметрыОтбора, ДанныеТовара);
	...
	Если Выборка.Следующий() Тогда
		...
		Если Не Выборка.Следующий() Тогда
			ПараметрыОтбора.Упаковка = ?(ЕдиничнаяУпаковка = ДанныеТовара.Упаковка,
										Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка(),
										ЕдиничнаяУпаковка);
			
			МассивШтрихкодов = РегистрыСведений.ШтрихкодыНоменклатуры.ШтрихкодыНоменклатуры(ПараметрыОтбора);
		КонецЕсли;
	КонецЕсли;
	...
	Если МассивШтрихкодов.Количество() = 0 Тогда
		МассивШтрихкодов = РегистрыСведений.ШтрихкодыНоменклатуры.ШтрихкодыНоменклатуры(ПараметрыОтбора);
	Иначе
		МассивШтрихкодовУпаковкиСтруктуры = РегистрыСведений.ШтрихкодыНоменклатуры.ШтрихкодыНоменклатуры(ПараметрыОтбора);
...
// 
// Возвращаемое значение:
//  Массив Из СправочникСсылка.ПодключаемоеОборудование - оборудование ЭТ.
//
...
// Пересчитывает данные об неотмененных оплатах платежными картами, используется в форме РМК возврат и РМК возврат подарочных сертификатов.
//
// Параметры:
//  АдресТаблицыНеотмененныхОплатПлатежнымиКартами - Строка - Адрес, указывающий на значение во временном хранилище.
//  НомерСтрокиОтменяемойОплатыПлатежнойКартой - Число - Номер строки, отменяемой оплаты, используется для пересчета относительно конкретной строки.
//  СуммаОтмененнойОплаты - Число - Сумма отмененной оплаты, используется для пересчета относительно конкретной строки.
//
// Возвращаемое значение:
//  Структура - со свойствами:
//   *КомбинацииСуммНеотмененныхОплатПлатежнымиКартами - Массив из Число - 
//   *НаименьшаяСуммаНеотмененнойОплатыПлатежнойКартой - Число - 
//   *КоличествоНеотмененныхОплатПлатежнымиКартами - Число - 
//   *ДанныеПервойНеотмененнойОплатыПлатежнымиКартами - Неопределено, Структура Из КлючИЗначение - 
//
Функция ПересчитатьДанныеНеотмененныхОплатПлатежнымиКартами(Знач АдресТаблицыНеотмененныхОплатПлатежнымиКартами, 
																	 Знач НомерСтрокиОтменяемойОплатыПлатежнойКартой = Неопределено,
																	 Знач СуммаОтмененнойОплаты = 0) Экспорт
	...
	Если Не ТаблицаНеотмененныхОплатПлатежнымиКартами = Неопределено Тогда
		...
		Если Результат.КоличествоНеотмененныхОплатПлатежнымиКартами > 0 Тогда
			...
			Для Каждого СтрокаОплаты Из ТаблицаНеотмененныхОплатПлатежнымиКартами Цикл
				...
				КонецЕсли;
			КонецЦикла;
			ДополнитьМассивЧиселКомбинациямиСуммЕгоЭлементов(Результат.КомбинацииСуммНеотмененныхОплатПлатежнымиКартами, 0, 0, Результат.КоличествоНеотмененныхОплатПлатежнымиКартами);			
		КонецЕсли;
...
// Дополняет массив чисел различными комбинациями из сумм первых элементов этого же массива.
//
// Параметры:
//  МассивКомбинаций - Массив из Число -
//  НачальнаяПозиция - Число - Значения начинаются с 0 и не могут быть больше, чем КоличествоПервыхЭлементовДляОбработки-1
//  НакопленнаяСумма - Число -
//  КоличествоПервыхЭлементовДляОбработки - Число - Количество первых элементов из массива, переданного в первом параметре, для которых будут расчитываться комбинации сумм.
//
Процедура ДополнитьМассивЧиселКомбинациямиСуммЕгоЭлементов(МассивКомбинаций, Знач НачальнаяПозиция, НакопленнаяСумма, Знач КоличествоПервыхЭлементовДляОбработки)

	Если НакопленнаяСумма <> 0 И МассивКомбинаций.Найти(НакопленнаяСумма) = Неопределено Тогда
		МассивКомбинаций.Добавить(НакопленнаяСумма);		
	КонецЕсли;
	
	Для Позиция = НачальнаяПозиция По КоличествоПервыхЭлементовДляОбработки - 1 Цикл
		ДополнитьМассивЧиселКомбинациямиСуммЕгоЭлементов(МассивКомбинаций, Позиция + 1, НакопленнаяСумма + МассивКомбинаций[Позиция], КоличествоПервыхЭлементовДляОбработки);
	КонецЦикла;
