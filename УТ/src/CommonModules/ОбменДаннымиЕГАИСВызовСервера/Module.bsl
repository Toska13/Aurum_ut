...
Функция НастройкиОбменаЕГАИС(ОрганизацииЕГАИС = Неопределено, ДатыПоследнегоЗапуска = Неопределено) Экспорт
	...
	|	
	|	ВЫБОР
	|		КОГДА НЕ НастройкиОбменаЕГАИС.ИспользоватьЗащищенноеСоединение ЕСТЬ NULL ТОГДА
	|			НастройкиОбменаЕГАИС.ИспользоватьЗащищенноеСоединение
	|		КОГДА НЕ ОбщиеНастройкиОбменаЕГАИС.ИспользоватьЗащищенноеСоединение ЕСТЬ NULL ТОГДА
	|			ОбщиеНастройкиОбменаЕГАИС.ИспользоватьЗащищенноеСоединение
	|		ИНАЧЕ ЛОЖЬ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ЕстьНастройкиОбменаЕГАИС Тогда
			...
			НастройкаОбменаЕГАИС.ИспользоватьЗащищенноеСоединение = Выборка.ИспользоватьЗащищенноеСоединение;
	...
	Для Каждого КлючИЗначение Из ДокументыКЗагрузке Цикл
		...
		Для Каждого ВходящиеДанные Из КлючИЗначение.Значение Цикл
			...
			Если ДанныеДокумента.Операция = Перечисления.ВидыДокументовЕГАИС.Квитанция
				...
				Если ПараметрыИдентификатораЗапросаV4 <> Неопределено И ПараметрыИдентификатораЗапросаV4.КодФСРАР <> СоответствиеОрганизацииЕГАИСиКодаФСРАР[ОрганизацияЕГАИС] Тогда
...
Функция СообщениеКПередачеXML(ДокументСсылка, ДальнейшееДействие, ДополнительныеПараметры = Неопределено)
	...
	Для Каждого ЭлементДанных Из ВозвращаемоеЗначение Цикл
		ЭлементДанных.ДальнейшееДействие = ДальнейшееДействие;
	КонецЦикла;
...
//
// Параметры:
//  КоллекцияСообщений - Массив из Структура - содержит структуру сообщений, которые подготавливаются к передаче.
//  Немедленно - Булево - Признак немедленной передачи сообщения в УТМ, без очереди сообщений.
...
//   * ИдентификаторВладельца - Неопределено, УникальныйИдентификатор - Уникальный идентификатор формы для сообщений пользователю.
//
Функция ПодготовитьСообщенияКПередаче(КоллекцияСообщений, Немедленно = Ложь, НастройкиОбменаЕГАИС = Неопределено, ИдентификаторВладельца = Неопределено) Экспорт
	...
	Для Каждого Сообщение Из КоллекцияСообщений Цикл
		...
		Ключ = ?(Сообщение.Документ = Неопределено, "", Сообщение.Документ);
		СообщенияПоДокументу = СообщенияПоДокументам[Ключ];
		Если СообщенияПоДокументу = Неопределено Тогда
			СообщенияПоДокументу = Новый Массив;
			СообщенияПоДокументам.Вставить(Ключ, СообщенияПоДокументу);
		КонецЕсли;
		...
		Попытка
			...
			Если ДальнейшееДействие <> Неопределено
				...
				ОбщегоНазначенияИС.УстановитьУправляемуюБлокировку(
					Метаданные.РегистрыСведений.СтатусыДокументовЕГАИС.ПолноеИмя(), ПоляБлокировки);
				
				ТекущееСостояние = РегистрыСведений.СтатусыДокументовЕГАИС.ТекущееСостояние(Документ);
			...
			Для Каждого Сообщение Из Сообщения Цикл
				...
				Если Не ЗначениеЗаполнено(Сообщение.ТекстОшибки) Тогда
					...
					Реквизиты.Вставить("ДальнейшееДействие",   Сообщение.ДальнейшееДействие);
