...
// В текущем сеансе устанавливает пользователя, от имени которого будет выполняться проверка дат запрета изменения данных.
// Для использования требуются полные права или привилегированный режим.
//
// Параметры:
//  Пользователь - Неопределено, СправочникСсылка.Пользователи, СправочникСсылка.ВнешниеПользователи - Пользователь, от имени которого будет выполняться проверка дат запрета изменений данных.
Процедура УстановитьПользователяДляПроверкиДатЗапрета(Пользователь = Неопределено) Экспорт
	
	ПараметрыСеанса.ПользовательДляПроверкиДатЗапрета = Пользователь;
