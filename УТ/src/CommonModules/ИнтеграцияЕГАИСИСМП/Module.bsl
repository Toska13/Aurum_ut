...
Функция ДоступнаРаботаСКодамиМаркировкиПива(Отказ = Неопределено) Экспорт
	...
	КоличествоВидовПродукции = 0;
	Для Каждого ВидПродукции Из ВидыПродукции Цикл
		Если МодульИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции(ВидПродукции) Тогда
			КоличествоВидовПродукции = КоличествоВидовПродукции + 1;
		КонецЕсли;
	КонецЦикла;
	
	Результат = КоличествоВидовПродукции > 0;
