...
Процедура УстановитьПризнакИспользованияМаркируемойПродукции(Форма) Экспорт
	...
	РазрешенОборотНемаркированныхОстатков = Истина;
	ВДокументеУказаныМарки                = Ложь;
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПараметрыИнтеграцииГосИС") Тогда
		...
		Если Не (ПараметрыИнтеграции = Неопределено) Тогда
			...
			ЕстьМаркируемаяПродукция = Ложь;
			УчитываемыеВидыПродукции = Новый Массив;
			
			ПараметрыСканирования       = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(Форма);
			ТаблицаМаркируемойПродукции = ШтрихкодированиеИССлужебный.ШтрихкодыУпаковокИзДокумента(Форма.Объект.Ссылка, ПараметрыСканирования);
			ВДокументеУказаныМарки      = ТаблицаМаркируемойПродукции.Количество() > 0;
			
			Для Каждого ВидПродукции Из ОбщегоНазначенияИСМПКлиентСерверПовтИсп.УчитываемыеВидыМаркируемойПродукции() Цикл
				
				Если Не ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПриПродажеОптомПоштучно(ВидПродукции, ДатаДокумента) Тогда
					Продолжить;
				КонецЕсли;
				
				УчитываемыеВидыПродукции.Добавить(ВидПродукции);
				
			КонецЦикла;
			
			ПроверитьНаличиеКолонкиРазрешенОборотНемаркированныхОстатков = Истина;
			
			Для Каждого СтрокаТабличнойЧастиТоваров Из ТабличнаяЧастьТовары Цикл
				
				Если ПроверитьНаличиеКолонкиРазрешенОборотНемаркированныхОстатков Тогда
					Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТабличнойЧастиТоваров, "РазрешенОборотНемаркированныхОстатков") Тогда
						Прервать;
					КонецЕсли;
					ПроверитьНаличиеКолонкиРазрешенОборотНемаркированныхОстатков = Ложь;
				КонецЕсли;
				
				//@skip-check bsl-legacy-check-static-feature-access-for-unknown-left-part
				Если Не СтрокаТабличнойЧастиТоваров.РазрешенОборотНемаркированныхОстатков Тогда
			...
			ПроверкаИПодборПродукцииИСМПУТ.ЕстьМаркируемаяПродукцияВКоллекции(
				ТабличнаяЧастьТовары, УчитываемыеВидыПродукции, ЕстьМаркируемаяПродукция);
				
		КонецЕсли;
	КонецЕсли;
	//-- Локализация
	
	ПоДокументуВозможноИспользованиеПродукцииБезМарок = РазрешенОборотНемаркированныхОстатков И Не ВДокументеУказаныМарки;
	
	Форма.ЕстьМаркируемаяПродукция     = ЕстьМаркируемаяПродукция И Не ПоДокументуВозможноИспользованиеПродукцииБезМарок;
	Форма.ИспользоватьУпаковочныеЛисты = Форма.ИспользоватьУпаковочныеЛисты И (Не ЕстьМаркируемаяПродукция И Не ПоДокументуВозможноИспользованиеПродукцииБезМарок);
