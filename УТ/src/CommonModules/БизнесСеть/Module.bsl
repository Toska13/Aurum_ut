...
// Возвращает количество подключенных контрагентов.
//
// Возвращаемое значение:
//  Число - количество подключенных контрагентов.
//
Функция КоличествоПодключенныхКонтрагентов() Экспорт
...
// 
// Возвращаемое значение:
//  Массив Из См. ДанныеОрганизацииИзJSON
//
Функция ДанныеОрганизацийПоНатуральнымИдентификаторам(НатуральныеИдентификаторы, Отказ = Ложь) Экспорт
	...
	Порции = БизнесСетьСлужебный.РазбитьМассивНаПорции(НатуральныеИдентификаторы, 50);
...
// Обработчик регламентного задания ОбновлениеИдентификаторовКонтрагентовБизнесСети
Процедура АктуализироватьКэшДанныхКонтрагентов() Экспорт
	
	// Обработчик БСП
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(
		Метаданные.РегламентныеЗадания.ОбновлениеИдентификаторовКонтрагентовБизнесСети);
	...
	ЗаписьЖурналаРегистрации(
		...
		НСтр(
			"ru = 'Начало обновления идентификаторов контрагентов Бизнес-сети'", 
			ОбщегоНазначения.КодОсновногоЯзыка()));
	
	// Выполнение логики регламентного задания
	...
	ЗаписьЖурналаРегистрации(
		...
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр(
			"ru = 'Завершение обновления идентификаторов контрагентов Бизнес-сети'", 
			ОбщегоНазначения.КодОсновногоЯзыка()));
КонецПроцедуры
...
Функция ТипОшибкиСервисНеДоступен()
...
Функция ПодключенныеОрганизации() Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	Результат = ДоступныеПодключенныеОрганизации();
	УстановитьПривилегированныйРежим(Ложь);
...
Функция ВидыДокументовСервиса() Экспорт
	...
	СписокВидовЭД.Добавить(Перечисления.ТипыДокументовЭДО.СчетОферта,              НСтр("ru = 'Счет-оферта'"));
...
Функция ОтправитьДанныеДокументовЧерезБизнесСетьВФоне(Знач УникальныйИдентификатор, Знач ИдентификаторЗадания, Знач ДополнительныеПараметры) Экспорт
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне(
		"БизнесСеть.ОтправитьДанныеДокументовЧерезБизнесСеть", ДополнительныеПараметры, ПараметрыВыполнения);
...
//  Организация               - ОпределяемыйТип.ОрганизацияКонтрагентEDI
//  ВидЭД                     - ПеречислениеСсылка.ТипыДокументовЭДО
//  ДанныеQRКодовПоДокументам - См. НовыйДанныеQRКода
// 
// Возвращаемое значение:
//  ТабличныйДокумент
//  Неопределено
Функция ФайлПредставленияПрикладногоДокументаСQRКодом(
	СсылкаНаДокумент, 
	Организация, 
	ВидЭД, 
	ДанныеQRКодовПоДокументам) Экспорт
	
	Если ДанныеQRКодовПоДокументам = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	БизнесСетьПереопределяемый.СформироватьПредставлениеПрикладногоДокументаСQRКодом(
		СсылкаНаДокумент, ВидЭД, СоответствиеДанныхQRКодов, ПечатнаяФорма);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("АктуализироватьКэшДанныхКонтрагентов", Истина);
	Методы.Вставить("ОтправитьДанныеДокументовЧерезБизнесСеть", Истина);
	Методы.Вставить("ОбновитьИнформациюОНовыхДокументахВСервисе", Истина);
	Методы.Вставить("ОтправитьУведомлениеОбОтправке", Истина);
	Методы.Вставить("ОтправитьДокументИПолучитьQRКодЧерезБизнесСеть", Истина);
	Методы.Вставить("ПолучитьQRКодыПоДокументам", Истина);
	Методы.Вставить("ПолучитьДанныеДокументаПоQRКоду", Истина);
	Методы.Вставить("ПодключитьОрганизациюПоКодуАктивацииВФоне", Истина);
	Методы.Вставить("ПолучениеДокументовСервиса", Истина);
	Методы.Вставить("ПодключитьОрганизациюПоТикетуИТСВФоне", Истина);
	Методы.Вставить("УстановитьСтатусЗагружен", Истина);
	Методы.Вставить("ЗагрузитьДокументы", Истина);
...
// Читает сведения об организации из данных сервиса 1С:Бизнес-сеть
// 
// Параметры:
//  ДанныеСервиса - Структура - Данные сервиса 1С:Бизнес-сеть
// 
// Возвращаемое значение:
//  Структура - Данные организации из JSON:
...
Функция НовыеПараметрыПовторногоВыполнения()
	
	Результат = Новый Структура;
	
	Результат.Вставить("МинимальныйИнтервал"              , 1);
	Результат.Вставить("МаксимальныйИнтервал"             , 1);
	Результат.Вставить("Коэффициент"                      , 1);
	Результат.Вставить("МаксимальнаяДлительностьПовторов" , 60);
	
	Возврат Результат;
	
КонецФункции

Функция ПараметрыПовторногоВыполненияСлишкомМногоЗапросов()
	
	Результат = НовыеПараметрыПовторногоВыполнения();
	
	Генератор = Новый ГенераторСлучайныхЧисел;
	
	Результат.МинимальныйИнтервал              = 1;
	Результат.МаксимальныйИнтервал             = 30;
	Результат.Коэффициент                      = Генератор.СлучайноеЧисло(1400, 1600) / 1000;
	Результат.МаксимальнаяДлительностьПовторов = 3 * 60;
	
	Возврат Результат;
	
КонецФункции

// Проверяет, является ли код HTTP-ответа 429
// 
// Параметры:
//  Ответ - HTTPОтвет
// 
// Возвращаемое значение:
//  Булево - Истина, если это HTTP-ответ 429
Функция ЭтоОтветСервисаСлишкомМногоЗапросов(Знач Ответ)
	Возврат Ответ.КодСостояния = 429;
КонецФункции

// Выполняет повторные запросы к сервису с увеличением интервала при превышении количества запросов к сервису
// 
// Параметры:
...
Процедура ОбработатьИнструкцииСервисаСлишкомМногоЗапросов(Соединение, Запрос, Ответ, Метод, Знач ПараметрыКоманды)
	
	ПараметрыПовторногоВыполнения = ПараметрыПовторногоВыполненияСлишкомМногоЗапросов();
	ТекущееВремя = ТекущаяУниверсальнаяДатаВМиллисекундах();
	ВремяЗавершения = ТекущееВремя + (ПараметрыПовторногоВыполнения.МаксимальнаяДлительностьПовторов * 1000);
	
	ТекущийИнтервал = ПараметрыПовторногоВыполнения.МинимальныйИнтервал;
	
	Пока Истина Цикл
		
		ОбщегоНазначенияБЭД.Пауза(ТекущийИнтервал);
		
		Ответ = ВызватьHTTPМетод(Соединение, Метод, Запрос, ПараметрыКоманды);
		
		Если Не ЭтоОтветСервисаСлишкомМногоЗапросов(Ответ) Тогда
			Прервать; 
		КонецЕсли;
		
		ТекущийИнтервал = Мин(
			ТекущийИнтервал * ПараметрыПовторногоВыполнения.Коэффициент, 
			ПараметрыПовторногоВыполнения.МаксимальныйИнтервал);
		
		Если ТекущаяУниверсальнаяДатаВМиллисекундах() > ВремяЗавершения Тогда
			// Защита от бесконечных повторов
			ВызватьИсключение
				НСтр(
					"ru = 'Ошибка получения ответа от сервиса: превышено время ожидания доступности сервиса'",
					ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
...
// Выполняет действия при получении инструкций от сервиса
// 
// Параметры:
//  Соединение       - HTTPСоединение
//  Запрос           - HTTPЗапрос
...
//  Метод            - Строка
//  ПараметрыКоманды - См. ОписаниеПараметровКомандыСервиса
Процедура ОбработатьИнструкцииСервиса(Знач Соединение, Знач Запрос, Ответ, Знач Метод, Знач ПараметрыКоманды)
	
	МаксимальнаяДлительностьВыполнения = 5 * 60; // 5 минут
	ВремяЗавершения = ТекущаяУниверсальнаяДатаВМиллисекундах() + МаксимальнаяДлительностьВыполнения * 1000;
	
	Пока Истина Цикл
		
		Если ТекущаяУниверсальнаяДатаВМиллисекундах() > ВремяЗавершения Тогда
			// Защита от бесконечной пост-обработки
			ВызватьИсключение
				НСтр(
					"ru = 'Ошибка пост-обработки результата запроса: превышено время пост-обработки'",
					ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
		
		Если ЭтоОтветСервисаСлишкомМногоЗапросов(Ответ) Тогда // Too Many Requests
			ОбработатьИнструкцииСервисаСлишкомМногоЗапросов(Соединение, Запрос, Ответ, Метод, ПараметрыКоманды);
		Иначе
			Прервать; // Отсутствуют инструкции, которые требуется выполнить
		КонецЕсли;
...
//  Метод            - Строка
//  Запрос           - HTTPЗапрос
//  ПараметрыКоманды - Строка - Имя запроса для логирования
Функция ВызватьHTTPМетод(Соединение, Метод, Запрос, Знач ПараметрыКоманды)
...
Функция ВыполнитьHTTPМетодСервиса(Соединение, Запрос, ПараметрыСоединения, ПараметрыКоманды, Отказ)
	...
	Попытка
		...
		Ответ = ВызватьHTTPМетод(Соединение, ПараметрыКоманды.Метод, Запрос, ПараметрыКоманды);
		
		ОбработатьИнструкцииСервиса(Соединение, Запрос, Ответ, ПараметрыКоманды.Метод, ПараметрыКоманды);
...
Функция ОбновитьИнформациюОНовыхДокументахВСервисеАсинхронно(Знач ПараметрыОбновления) Экспорт
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне(
		"БизнесСеть.ОбновитьИнформациюОНовыхДокументахВСервисе", ПараметрыОбновления, ПараметрыВыполнения);
...
Функция ПараметрыКомандыСписокВходящихДокументов(ПараметрыМетода, Отказ = Неопределено) Экспорт
	...
	Иначе
		СписокОрганизаций = ДоступныеПодключенныеОрганизации().ВыгрузитьКолонку("Организация");
	...
	Для каждого СведенияОбОрганизации Из СведенияОбОрганизациях Цикл
		
		ТекстОшибки = "";
		
		Идентификаторы = ИдентификаторыУчастника(
			СведенияОбОрганизации.ИНН,
			СведенияОбОрганизации.КПП, 
			СведенияОбОрганизации.Организация, 
			Отказ, 
			ТекстОшибки);
...
//  ДополнительныеПараметры - Структура    - Дополнительные параметры, переданные в преобразование
...
Процедура ЗагрузитьДокументы(Знач ПараметрыЗапроса, Знач АдресРезультата) Экспорт
	...
	Для каждого СтрокаЗагрузки Из ПараметрыЗапроса Цикл
		...
		ПараметрыОтраженияВУчете.СпособОбмена = Перечисления.СпособыОбменаЭД.Через1СБизнесСеть;
...
Функция ДанныеКлючаДоступаИзJSON(ДанныеСервиса)
	...
	Результат.Вставить("СрокДействияТокенаДоступа", ТекущаяДатаСеанса() + 
		ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеСервиса, "access_token_expires", 0));
	Результат.Вставить("СрокДействияТокенаОбновления", ТекущаяДатаСеанса() + 
		ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеСервиса, "refresh_token_expires", 0));
...
Процедура ПриостановитьДоступКСервису(ИдентификаторОрганизации)
	...
	Если Организация = Неопределено Тогда
		Возврат; // Организация ранее не была подключена
	КонецЕсли;
...
// Метод-конструктор таблицы сведений об организации
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Новые сведения об организации:
// * Организация  - ОпределяемыйТип.Организация - Ссылка на организацию
// * Наименование - Строка                      - Наименование организации
// * ИНН          - Строка                      - ИНН организации
// * КПП          - Строка                      - КПП организации
Функция НовыеСведенияОбОрганизации()
...
// Получает таблицу сведений об организации по массиву ссылок на организации
// 
// Параметры:
//  Организации - Массив Из ОпределяемыйТип.Организация - Набор организаций, сведения по которым требуется получить
// 
// Возвращаемое значение:
//  См. НовыеСведенияОбОрганизации
Функция СведенияОбОрганизациях(Знач Организации)
	
	// Служебные переменные
	Контекст       = "БизнесСеть.СведенияОбОрганизациях";
	ТипОрганизация = Метаданные.ОпределяемыеТипы.Организация.Тип;
	
	// Проверка входящих параметров
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(Контекст, "Организации", Организации, Тип("Массив"));
	
	Для ИндексЭлемента = 0 По Организации.ВГраница() Цикл
		ИмяПараметра        = СтрШаблон("Организации[%1]", ИндексЭлемента);
		ПроверяемоеЗначение = Организации[ИндексЭлемента];
		
		ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(Контекст, ИмяПараметра, ПроверяемоеЗначение, ТипОрганизация);
	КонецЦикла;
	
	// Подготавливаем структуру результата
	Результат = НовыеСведенияОбОрганизации();
	...
	Запрос.Текст =
		...
		|	&ПолеКПП          КАК КПП
		|ИЗ
		|	&ТаблицаСправочника КАК Данные
		|ГДЕ
		|	Данные.Ссылка В(&Организации)";
	
	// Устанавливаем имя справочника
	ИмяСправочника = ОбщегоНазначенияБЭД.ИмяПрикладногоСправочника("Организации");
	ОбщегоНазначенияКлиентСервер.Проверить(
		Не ПустаяСтрока(ИмяСправочника),
		НСтр(
			"ru = 'Не задано имя справочника организаций'",
			ОбщегоНазначения.КодОсновногоЯзыка()),
		Контекст);
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		СтрШаблон(
			НСтр(
				"ru = 'Не найден справочник организаций (имя: %1)'",
				ОбщегоНазначения.КодОсновногоЯзыка()),
			ИмяСправочника),
		Контекст);
	
	ПолноеИмяМетаданных = Справочник.ПолноеИмя();
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТаблицаСправочника", ПолноеИмяМетаданных);
	
	// Устанавливаем поля выпорки
	ИмяРеквизитаНаименование = ЭлектронноеВзаимодействие.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении(
		"НаименованиеОрганизации");
	ИмяРеквизитаИНН          = ЭлектронноеВзаимодействие.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении(
		"ИННОрганизации");
	ИмяРеквизитаКПП          = ЭлектронноеВзаимодействие.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении(
		"КППОрганизации");
	
	ОбщегоНазначенияКлиентСервер.Проверить(
		Не ПустаяСтрока(ИмяРеквизитаНаименование),
		НСтр(
			"ru = 'Не задано имя реквизита для наименования организации'",
			ОбщегоНазначения.КодОсновногоЯзыка()),
		Контекст);
	ОбщегоНазначенияКлиентСервер.Проверить(
		Не ПустаяСтрока(ИмяРеквизитаИНН),
		НСтр(
			"ru = 'Не задано имя реквизита для ИНН организации'",
			ОбщегоНазначения.КодОсновногоЯзыка()),
		Контекст);
	ОбщегоНазначенияКлиентСервер.Проверить(
		Не ПустаяСтрока(ИмяРеквизитаКПП),
		НСтр(
			"ru = 'Не задано имя реквизита для КПП организации'",
			ОбщегоНазначения.КодОсновногоЯзыка()),
		Контекст);
	
	ПутьКДаннымНаименование = СтрШаблон("Данные.%1", ИмяРеквизитаНаименование);
	ПутьКДаннымИНН          = СтрШаблон("Данные.%1", ИмяРеквизитаИНН);
	ПутьКДаннымКПП          = СтрШаблон("Данные.%1", ИмяРеквизитаКПП);
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПолеНаименование", ПутьКДаннымНаименование);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПолеИНН", ПутьКДаннымИНН);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПолеКПП", ПутьКДаннымКПП);
...
Процедура ИзменитьПризнакПовторногоПодключения(Организация, ТребуетсяПовторноеПодключение)
	...
	Контекст = "БизнесСеть.ИзменитьПризнакПовторногоПодключения";
	
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		Контекст, "Организация", Организация, Метаданные.ОпределяемыеТипы.Организация.Тип);
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		Контекст, "ТребуетсяПовторноеПодключение", ТребуетсяПовторноеПодключение, Тип("Булево"));
