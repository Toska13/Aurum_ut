...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВыполнитьДействияПоИнтеграциямЭДО", Истина);
	Методы.Вставить("ЗаписатьРезультатПроверкиПодписейДокумента", Истина);
	Методы.Вставить("ПредставлениеДанныхСообщения", Истина);
	Методы.Вставить("ПредставлениеДанныхСообщенияПоСсылке", Истина);
	Методы.Вставить("ПродолжитьВыполнениеДействийПоИнтеграциямЭДОПослеОбработкиРезультатов", Истина);
...
Процедура ЗаблокироватьДокументДляИзменения(ЭлектронныйДокумент, ЭтоВходящийДокумент = Ложь) Экспорт
	...
	ЭлектронныеДокументыЭДОСлужебный.ЗаблокироватьДокументДляИзменения(ЭлектронныйДокумент, ЭтоВходящийДокумент);
	
КонецПроцедуры

// Параметры:
//  КлючДокумента - См. НовыйКлючДокументаДляБлокировки
//  ЭтоВходящийДокумент - Булево
Процедура ЗаблокироватьДокументДляИзмененияПоКлючу(КлючДокумента, ЭтоВходящийДокумент = Ложь) Экспорт
	
	ИмяТаблицыДокумента = ИмяТаблицыДокументаЭДО(ЭтоВходящийДокумент);
	
	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить(ИмяТаблицыДокумента);
	ЭлементБлокировки.УстановитьЗначение("ИдентификаторДокумента", КлючДокумента.ИдентификаторДокумента);
	ЭлементБлокировки.УстановитьЗначение("ИдентификаторДокументооборота", КлючДокумента.ИдентификаторДокументооборота);
	ЭлементБлокировки.УстановитьЗначение("ИдентификаторСвязи", КлючДокумента.ИдентификаторСвязи);
	Блокировка.Заблокировать();
	
КонецПроцедуры

// Параметры:
//  КлючиДокументов - Массив из См. НовыйКлючДокументаДляБлокировки
//  ЭтоВходящиеДокументы - Булево
Процедура ЗаблокироватьДокументыДляИзмененияПоКлючам(КлючиДокументов, ЭтоВходящиеДокументы = Ложь) Экспорт
	
	ИмяТаблицыДокументов = ИмяТаблицыДокументаЭДО(ЭтоВходящиеДокументы);
	
	Блокировка = Новый БлокировкаДанных;
	Для Каждого КлючДокумента Из КлючиДокументов Цикл
		ЭлементБлокировки = Блокировка.Добавить(ИмяТаблицыДокументов);
		ЭлементБлокировки.УстановитьЗначение("ИдентификаторДокумента", КлючДокумента.ИдентификаторДокумента);
		ЭлементБлокировки.УстановитьЗначение("ИдентификаторДокументооборота", КлючДокумента.ИдентификаторДокументооборота);
		ЭлементБлокировки.УстановитьЗначение("ИдентификаторСвязи", КлючДокумента.ИдентификаторСвязи);
	КонецЦикла;
...
Функция НовыйКлючДокументаДляБлокировки() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ЭтоВходящийДокумент", Ложь);
...
// Определяет является ли формат формализованным и однотитульным.
// Параметры:
//  Формат - Строка
// 
// Возвращаемое значение:
//  Булево
//
Функция ЭтоФормализованныйФорматБезОтветногоТитула(Формат) Экспорт
	
	Возврат ФорматыЭДО.ЭтоФормализованныйФорматБезОтветногоТитула(Формат);
...
// Возвращает доступные типы не имеющие подпись.
// 
// Возвращаемое значение:
// 	Массив из ПеречислениеСсылка.ТипыДокументовЭДО - типы электронного документа.
Функция ТипыДокументовБезПодписи() Экспорт
	Возврат Перечисления.ТипыДокументовЭДО.ТипыДокументовБезПодписи();
КонецФункции
...
// Определяет относится ли тип документа к формализованным однотитульным документам.
// 
// Параметры:
// 	ТипДокумента - ПеречислениеСсылка.ТипыДокументовЭДО - Тип электронного документа.
// Возвращаемое значение:
// 	Булево - Признак типа служебного документа.
//
Функция ЭтоТипБезОтветногоТитулаПриФормализованномОбмене(ТипДокумента) Экспорт
	Возврат Перечисления.ТипыДокументовЭДО.ЭтоТипБезОтветногоТитулаПриФормализованномОбмене(ТипДокумента);
КонецФункции

// Определяет возможна ли для типа документа выгрузка в ФНС только по формализованному регламенту.
// 
// Параметры:
// 	ТипДокумента - ПеречислениеСсылка.ТипыДокументовЭДО - Тип электронного документа.
// Возвращаемое значение:
// 	Булево - Выгрузка возможна.
//
Функция ВозможнаВыгрузкаТолькоПоФормализованномуРегламенту(ТипДокумента) Экспорт
	Возврат Перечисления.ТипыДокументовЭДО.ЭтоТипБезОтветногоТитулаПриФормализованномОбмене(ТипДокумента);
КонецФункции
...
Функция ТипДокументаПоддерживаетЗагрузкуИзФайлаВФормализованномВиде(ТипДокумента, ТипРегламента) Экспорт
...
// Возвращаемое значение:
//  Булево
Функция ЭтоИсправляющийТипДокумента(ТипДокумента) Экспорт
	
	Возврат Перечисления.ТипыДокументовЭДО.ЭтоИсправляющий(ТипДокумента);
...
// Возвращаемое значение:
//  Булево
Функция ЭтоКорректирующийТипДокумента(ТипДокумента) Экспорт
	
	Возврат Перечисления.ТипыДокументовЭДО.ЭтоКорректирующий(ТипДокумента);
...
Функция КорректируемыеТипыДокументов() Экспорт
	...
	Возврат Перечисления.ТипыДокументовЭДО.КорректируемыеТипыДокументов();
...
// Возвращает виды документов с подписью.
// 
// Возвращаемое значение:
// 	Массив из СправочникСсылка.ВидыДокументовЭДО - доступные виды документов.
Функция ВидыДокументовСПодписью() Экспорт
	
	ТипыДокументовБезПодписи = ТипыДокументовБезПодписи();
	...
	Запрос.Текст =
		...
		|	Справочник.ВидыДокументовЭДО КАК ВидыДокументовЭДО
		|ГДЕ
		|	Не ВидыДокументовЭДО.ТипДокумента В (&ТипыДокументовБезПодписи)";
	
	Запрос.УстановитьПараметр("ТипыДокументовБезПодписи", ТипыДокументовБезПодписи);
...
Функция НастройкиРегламента(ОписаниеВидаДокумента, Формат) Экспорт
	...
	Если ЭтоФормализованныйФорматБезОтветногоТитула(Формат) Тогда
...
Процедура ПриИзмененииСопроводительнойЗаписки(ОписанияСообщений, Отказ) Экспорт
	...
	ЭлектронныйДокументСообщений = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОписанияСообщений[0].Ссылка,
		"ЭлектронныйДокумент");
		
	НачатьТранзакцию();
	Попытка
		ЭлектронныеДокументыЭДОСлужебный.ЗаблокироватьДокументДляИзменения(ЭлектронныйДокументСообщений);
...
Функция СведенияСообщенияУчастникаЭДО(ЭлектронныйДокумент) Экспорт
	...
	Пока ВыборкаФайлов.Следующий() Цикл
		Если ВыборкаОписанияВида.ВидПодписи = Перечисления.ВидыЭлектронныхПодписей.Простая Тогда
			...
			КонецЦикла;
		Иначе
			ЭлектронныеПодписи = КриптографияБЭД.УстановленныеПодписи(ВыборкаФайлов.Ссылка);
...
Функция ДанныеФайловДляВыгрузкиФНС(ЭлектронныеДокументы) Экспорт
	...
	Для Каждого ТекущийДокумент Из ЭлектронныеДокументы Цикл
		...
		Для Каждого ТекущееСообщение Из СообщенияТекущегоДокумента Цикл
			...
			ДанныеФайла.УстановленныеПодписи = КриптографияБЭД.УстановленныеПодписи(ТекущееСообщение.ОсновнойФайл);
...
Функция ДанныеДокументаДляЗагрузкиПоДаннымФайлаXML(ОписаниеФайла, СодержаниеФайла = Неопределено,
	...
	Если Не ТипДокументаПоддерживаетЗагрузкуИзФайлаВФормализованномВиде(
		ТипДокумента, СодержаниеФайла.ТипРегламента) Тогда
		Возврат Результат;
	КонецЕсли;
...
//  ** Значение - См. ЭлектроннаяПодпись.ПодписиОбъекта
//
...
Функция ЗаписатьРезультатПроверкиПодписейДокумента(ПроверенныеПодписи) Экспорт
	...
	Для Каждого РезультатПроверкиФайла Из ПроверенныеПодписи Цикл
		...
		УстановленныеПодписи = КриптографияБЭД.УстановленныеПодписи(РезультатПроверкиФайла.Ключ);
	...
	Если ОписаниеСообщения.Данные.Содержание.Свойство("СвойстваДокументовПоФорматам", СвойстваДокументовПоФорматам) Тогда
		...
		Если СвойстваДокументовПоФорматам.Свойство("АктОРасхождениях", АктОРасхождениях) Тогда
			...
			Если АктОРасхождениях.Свойство("ВариантФормированияИтоговПриемки", ВариантФормированияИтоговПриемки)
				...
				Если Не ВариантФормированияИтоговПриемки = ВариантыФормированияИтоговПриемки.НеФормируются Тогда
...
// 	ПараметрыДействийПоЭДО - Неопределено, 
...
Функция ОтправитьВсеДокументы(ИдентификаторыОрганизаций, ПараметрыДействийПоЭДО = Неопределено, КонтекстДиагностики = Неопределено) Экспорт
	...
	Если ПараметрыДействийПоЭДО = Неопределено Тогда
...
// Возвращаемое значение:
//  Булево
Функция ЭтоОшибкаФормированияОтвета(Ошибка) Экспорт
	
	Возврат Ошибка.ВидОшибки.Идентификатор = ИдентификаторОшибкиФормированияОтвета();
...
Функция ТипыДокументовЭДОВыгрузкиДляФНС() Экспорт
	...
	ТипыДокументов.Добавить(Перечисления.ТипыДокументовЭДО.СчетОферта);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ТипыДокументов, ТрансграничныйЭДО.ТипыДокументовЭДОВыгрузкиДляФНС());
...
#Область Документы_ЭлектронныйДокументВходящий_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.ЗапускатьИВПодчиненномУзлеРИБСФильтрами = Истина;
	Обработчик.Процедура = "Документы.ЭлектронныйДокументВходящийЭДО.ОбработатьДанныеДляПереходаНаНовуюВерсию_АктОРасхождениях";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("dcafb44d-22d0-431a-aa23-999aa799a09b");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ЭлектронныйДокументВходящийЭДО.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию_АктОРасхождениях";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: обновление входящих электронных документов, акт о расхождениях.'");
...
#Область ПервыйЗапуск
	...
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыБЭД.ПервыйЗапуск";
...
Функция ТребуетсяОтветнаяПодписьПоУмолчанию(ТипДокумента)
	...
	Если Не (ТипДокумента = Перечисления.ТипыДокументовЭДО.СчетФактура
		...
		Или  ТипДокумента = Перечисления.ТипыДокументовЭДО.СчетНаОплату
		Или  ТипДокумента = Перечисления.ТипыДокументовЭДО.СчетОферта
...
Функция ТребуетсяИзвещениеПоУмолчанию(ТипДокумента)
	...
	Если Не (ТипДокумента = Перечисления.ТипыДокументовЭДО.СчетФактура
		Или ТипДокумента = Перечисления.ТипыДокументовЭДО.КорректировочныйСчетФактура
		Или ТипДокумента = Перечисления.ТипыДокументовЭДО.СчетНаОплату
		Или ТипДокумента = Перечисления.ТипыДокументовЭДО.СчетОферта) Тогда
...
Функция ПроверитьПодписиФайлов(ПрисоединенныеФайлы, КонтекстДиагностики)
	...
	ДоступнаПроверкаПодписиВОблакеПриИнтерактивномЗапускеПроверки = КриптографияБЭД.ДоступнаПроверкаПодписиВОблаке()
		И ОбщегоНазначенияБЭД.ЭтоРежимЦентрЭДО();
