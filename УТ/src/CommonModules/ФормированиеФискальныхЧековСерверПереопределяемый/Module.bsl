...
Функция ПараметрыПредпросмотраЧека(ПараметрыОперации) Экспорт
	...
	// СуммаПредоплатыВнеОдногоДня
	ПараметрыПредпросмотраЧека.Вставить(
		"СуммаПредоплатыВнеОдногоДня",
		 ПредоплатаПоДокументуПоставкиВнеОдногоДняЛокализация(ПараметрыОперации.ДокументСсылка));
...
// Определяет дату операции для чека коррекции.
// Дата определяется на основе корректируемого документа или его документа-основания.
// Для документов реализации учитывается дата перехода права собственности.
// 
// Параметры:
...
Функция ДатаСовершенияКорректируемогоРасчета(ДокументСсылка) Экспорт
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		ИмяРеквизитаДокументаОснования = "ДокументОснование";
		ДокументРеализации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, ИмяРеквизитаДокументаОснования);
	КонецЕсли;
	
	Если ТипЗнч(ДокументРеализации) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		
		ИменаРеквизитовДатыРеализации = "Дата,ДатаПереходаПраваСобственности";
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументРеализации, ИменаРеквизитовДатыРеализации);
		
		ДатаКоррекции = РеквизитыДокумента.Дата;
		Если ЗначениеЗаполнено(РеквизитыДокумента.ДатаПереходаПраваСобственности) Тогда
			ДатаКоррекции = РеквизитыДокумента.ДатаПереходаПраваСобственности;
		КонецЕсли;
		
	Иначе
		ИмяРеквизитаДатыРеализации = "Дата";
		ДатаКоррекции = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументРеализации, ИмяРеквизитаДатыРеализации);
	КонецЕсли;
	//-- Локализация
...
Функция ВидыЧекаКоррекции(ДокументСсылка, ИмяКомандыПробитияЧека) Экспорт
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		...
		Если ЗначениеЗаполнено(ДокументОснование) Тогда
			Если ОборудованиеЧекопечатающиеУстройстваВызовСервера.ДанныеФискальнойОперации(ДокументОснование) = Неопределено Тогда
				ВидыЧекаКоррекции.Очистить();
				ВидыЧекаКоррекции.Добавить(Перечисления.ВидыЧековКоррекции.НеприменениеККТ);
			Иначе
				Если ВидыЧекаКоррекции.Найти(Перечисления.ВидыЧековКоррекции.БлокировкаЧекаВОФД) = Неопределено Тогда
					ВидыЧекаКоррекции.Добавить(Перечисления.ВидыЧековКоррекции.БлокировкаЧекаВОФД);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ОбъектыРасчетовСПризнакамиСпособаРасчетов(ДокументСсылка, ВозможныеПризнакиСпособаРасчетаПоДокументу, ПризнакиСпособаРасчетаАвто, ПризнакСпособаРасчета, ОбъектыРасчетов, СуммаДокумента, СуммаПредоплатыВРамкахОдногоДня) Экспорт
	...
	ТипыОбъектовРасчетов.Добавить(Тип("СправочникСсылка.ДоговорыМеждуОрганизациями"));
			...
			Если ПризнакиСпособаРасчетаАвто.Количество() > 0 Тогда
				...
				Для Каждого ПризнакСпособаРасчетаАвто Из ПризнакиСпособаРасчетаАвто Цикл
					...
					Если ВзаиморасчетыПоОбъектуОплатаКредита(ПризнакСпособаРасчетаАвто.ОбъектРасчета, ПризнакСпособаРасчетаАвто.ДокументРасчета) Тогда
						Если (ТипЗнч(ПризнакСпособаРасчетаАвто.ОбъектРасчета) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
							Или ТипЗнч(ПризнакСпособаРасчетаАвто.ОбъектРасчета) = Тип("СправочникСсылка.ДоговорыМеждуОрганизациями"))
							И ЗначениеЗаполнено(ПризнакСпособаРасчетаАвто.ДокументРасчета) Тогда
...
Функция ДокументПоставкиПоДокументуОплатыВРамкахЕдиногоЧека(ДокументСсылка) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	ДокументОплаты.Ссылка.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	ДокументОплаты.ОбъектРасчетов.Объект КАК Ссылка,
	|	ДокументОплаты.Ссылка.Дата КАК Дата,
	|	ИСТИНА КАК ЭтоПриходДенежныхСредств,
	...
	|	И ДокументОплаты.Ссылка.Проведен
	|СГРУППИРОВАТЬ ПО
	|	ДокументОплаты.Ссылка.ХозяйственнаяОперация,
	...
	|	ДокументОплаты.ОбъектРасчетов.Объект КАК Ссылка,
	|	ДокументОплаты.Ссылка.Дата КАК Дата,
	|	ЛОЖЬ КАК ЭтоПриходДенежныхСредств,
	|	СУММА(ДокументОплаты.Сумма) КАК СуммаДокумента
	|ИЗ
	|	Документ.РасходныйКассовыйОрдер.РасшифровкаПлатежа КАК ДокументОплаты
	|ГДЕ
	...
	|	ДокументОплаты.Ссылка.Дата КАК Дата,
	|	ВЫБОР
	|		КОГДА ДокументОплаты.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОплатыКлиенту)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ВЫБОР
	|			КОГДА ДокументОплаты.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеОплатыОтКлиента)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ НЕОПРЕДЕЛЕНО
	|		КОНЕЦ
	|	КОНЕЦ КАК ЭтоПриходДенежныхСредств,
	|	СУММА(ДокументОплаты.Сумма) КАК СуммаДокумента
	|ИЗ
	|	Документ.ОперацияПоПлатежнойКарте.РасшифровкаПлатежа КАК ДокументОплаты
	|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		Если ТипДокументаПоставкиРазрешенДляХозяйственнойОперации(ТипЗнч(Выборка.Ссылка), Выборка.ХозяйственнаяОперация)
			И ДокументПоставкиСВозможностьюПробитияЕдиногоЧека(Выборка.Ссылка, Выборка.ЭтоПриходДенежныхСредств) Тогда
			ДокументОплатыПоДокументуПоставки.Добавить(Выборка.Ссылка);
		КонецЕсли;
	КонецЦикла;
	//-- Локализация
...
Функция ОбновитьПараметрыФискальногоЧека(ПараметрыОперацииЧека, ВерсияФФД) Экспорт
	...
	Если ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ККТ Тогда
		ФорматноЛогическийКонтрольКлиентСервер.ПровестиФорматноЛогическийКонтроль(
			...
			ПараметрыОперацииЧека.ОборудованиеККТ);
	КонецЕсли;
	ОбновитьПараметрыФискальногоЧекаЧасоваяЗона(ПараметрыОперацииФискализацииЧека, ПараметрыОперацииЧека);
	//-- Локализация
...
Процедура ВидыВозможныхПризнаковСпособаРасчетаПоДокументу(ПризнакиСпособаРасчета, ДокументСсылка, ОбъектыРасчетов, ТипРасчетаДенежнымиСредствами, СуммаДокумента) Экспорт
	...
	Иначе
		...
		Если ДокументОплатыСВозможностьюПробитияЧеков(ДокументСсылка) Тогда
			...
			Если НЕ ОбъектыРасчетовДокументаАвансовые(ОбъектыРасчетов) Тогда
				Если СуммаОплатыБольшеСуммыПоставок(ДокументСсылка, ОбъектыРасчетов)
					ИЛИ СуммаОплатыРавнаСуммеОбъектовРасчетов(СуммаДокумента, ОбъектыРасчетов) Тогда
	...
	Иначе
		...
		Если ДокументОплатыСВозможностьюПробитияЧеков(ДокументСсылка) Тогда
			...
			Иначе
				...
				Если ДатаОплатыБольшеДатыПоставки(ДокументСсылка, ОбъектыРасчетов) Тогда
...
//   *Заказ - ДокументСсылка,СправочникСсылка
//   *Сумма - Число
//
Функция ОбъектыРасчетовДокументаОплаты(ДокументыОплаты, ИмяКомандыПробитияЧека) Экспорт
	...
	//++ Локализация
	ОбъектыРасчетовДокументаОплаты = ОбъектыРасчетовПоДокументамОплатыРазвернуто(ДокументыОплаты, ИмяКомандыПробитияЧека);
	...
	Если ДанныеПоОбъектамРасчетовИзРасшифровкиПлатежа <> Неопределено Тогда
		...
		Для Каждого ДанныеПоОбъектуРасчетов Из ДанныеПоОбъектамРасчетовИзРасшифровкиПлатежа Цикл
			...
			Если ДанныеПоОбъектуРасчетов.Заказ = ОбъектРасчета
				Или ТипЗнч(ДанныеПоОбъектуРасчетов.Заказ) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
				Или ТипЗнч(ДанныеПоОбъектуРасчетов.Заказ) = Тип("СправочникСсылка.ДоговорыКредитовИДепозитов")
				Или ТипЗнч(ДанныеПоОбъектуРасчетов.Заказ) = Тип("СправочникСсылка.ДоговорыМеждуОрганизациями") Тогда
...
Процедура ТаблицаТоваровСРаспределениемКодовМаркировкиИСМП(Товары, ДанныеДляИСМП) Экспорт
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		Если НЕ ДанныеДляИСМП = Неопределено
			...
			И ВидыПродукцииИСМП.Найти(КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииПоПодсистеме(СтрокаТЧ.ВидПродукцииИС, "ИСМП")) <> Неопределено Тогда
...
//		которые могут участвовать в едином чеке.
// 	ИмяКомандыПробитияЧека - Строка - Имя команды пробития чека
Процедура ДокументыЕдиногоЧекаЗаполнитьТаблицу(ДокументСсылка, ТаблицаДокументовЕдиногоЧека, ИмяКомандыПробитияЧека) Экспорт
	
	ДокументыОплаты = ДокументОплаты(ДокументСсылка);
	ОбъектыРасчетовПоДокументамОплаты = ОбъектыРасчетовПоДокументамОплатыРазвернуто(ДокументыОплаты, ИмяКомандыПробитияЧека);
	ОбъектыРасчетовЕдиногоЧека = Новый Массив;
	...
	Для Каждого ДокументОплаты Из ДокументыОплаты Цикл
		...
		ДанныеДокументаОплаты = ДокументОплатыСВозможностьюПробитияЕдиногоЧека(ДокументОплаты);
		ОбъектыРасчетов = ОбъектыРасчетовПоДокументамОплаты.НайтиСтроки(Новый Структура("ДокументОплаты", ДокументОплаты));
		Если НЕ ДоступнаВозможностьПробитияЕдиногоЧека(ДанныеДокументаОплаты, ОбъектыРасчетов) Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаДокументЕдиногоЧека = ТаблицаДокументовЕдиногоЧека.Добавить();
		СтрокаДокументЕдиногоЧека.Документ = ДокументОплаты;
		СтрокаДокументЕдиногоЧека.Сумма = ДанныеДокументаОплаты.СуммаДокумента;
		
		// Документы поставки
		Для Каждого СтрокаОбъектыРасчетов Из ОбъектыРасчетов Цикл
			Если ОбъектыРасчетовЕдиногоЧека.Найти(СтрокаОбъектыРасчетов.Заказ) = Неопределено Тогда
				СтрокаДокументЕдиногоЧека = ТаблицаДокументовЕдиногоЧека.Добавить();
				...
				ОбъектыРасчетовЕдиногоЧека.Добавить(СтрокаОбъектыРасчетов.Заказ)
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
	// Документы взаимозачета
	Если ОбъектыРасчетовЕдиногоЧека.Количество() Тогда
		ДокументыВзаимозачета = ДокументыВзаимозачета(ОбъектыРасчетовЕдиногоЧека);
...
Функция РазрешенКассовыйЧекКоррекцииДляТипаРасчетов(ТипРасчетовДенежнымиСредствами) Экспорт
	...
	Если ТипРасчетовДенежнымиСредствами = Перечисления.ТипыРасчетаДенежнымиСредствами.ПриходДенежныхСредств 
		Или ТипРасчетовДенежнымиСредствами = Перечисления.ТипыРасчетаДенежнымиСредствами.РасходДенежныхСредств
		Или ТипРасчетовДенежнымиСредствами = Перечисления.ТипыРасчетаДенежнымиСредствами.ВозвратДенежныхСредств Тогда
		Результат = Истина;
	КонецЕсли;
...
Процедура ОбновитьВзаиморасчеты(ПараметрыОперации)
	...
	//++ Локализация
	Если НЕ ВзаиморасчетыРассчитаны(ПараметрыОперации) Тогда
		
		РаспределитьРасчетыСКлиентами(ПараметрыОперации);
	
	ИначеЕсли ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		НеоперативнаяОчередьЗаданий.ВыполнитьЗаданияНеоперативнойОчередиВТранзакцииПроведенияДокумента(
...
Функция ПредоплатаПоДокументуПоставкиВнеОдногоДняЛокализация(ДокументСсылка)
	
	СуммаПредоплаты = 0;
	
	//++ Локализация
	Если Не ДокументПоставкиСВозможностьюПробитияЧеков(ДокументСсылка) Тогда
		Возврат СуммаПредоплаты;
	КонецЕсли;
	...
	|ГДЕ
	|	ОбъектыРасчетов.Объект = &ДокументСсылка
	|	И РасчетыСКлиентамиПоСрокам.ХозяйственнаяОперация = &ХозяйственнаяОперация
	|	И РасчетыСКлиентамиПоСрокам.ВидДвижения = &ВидДвижения
	|	И РасчетыСКлиентамиПоСрокам.ПредоплатаРегл > 0";
	Запрос.УстановитьПараметр("ДокументСсылка", ДокументСсылка);
	Запрос.УстановитьПараметр("ВидДвижения", ВидДвиженияНакопления.Расход);
	Запрос.УстановитьПараметр("ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.ЗачетАвансаКлиента);
	
	ВыборкаЗапроса = Запрос.Выполнить().Выбрать();
	
	Если ВыборкаЗапроса.Следующий() Тогда
		СуммаПредоплаты = ВыборкаЗапроса.ПредоплатаРегл;
	КонецЕсли;
	//-- Локализация
...
Функция ПризнакиСпособаРасчетаАвто(ПараметрыОперации, ОбъектыРасчетовПоДокументу)
	...
	Для Каждого ДанныеПоОбъектуРасчета Из ОбъектыРасчетовПоДокументу Цикл
		Если НЕ ЗначениеЗаполнено(ДанныеПоОбъектуРасчета.ОбъектРасчетов) Тогда
...
Функция ОпределитьВидПредоплатыПоВзаиморасчетам(ОбъектРасчета, Оплата, ПризнакиСпособаРасчетаАвто)
	...
	Попытка
		...
		Если СуммаДетальнойОплаты > 0 И СуммаПоставки > 0 Тогда
	...
	Для Каждого СтрокаПризнакаСпособаРасчетаАвто Из ПризнакиСпособаРасчетаАвто Цикл
		...
		Если ( ОбъектРасчетовАвансовый(СтрокаПризнакаСпособаРасчетаАвто.ОбъектРасчета)
			...
			Если НЕ ДокументОплатыСВозможностьюПробитияЧеков(СтрокаПризнакаСпособаРасчетаАвто.ОбъектРасчета)
				И ДокументОплатыСВозможностьюПробитияЧеков(СтрокаПризнакаСпособаРасчетаАвто.ДокументРасчета) Тогда
				СуммаПредоплаты = СуммаПредоплаты
					+ СтрокаПризнакаСпособаРасчетаАвто.Поставка
					+ СтрокаПризнакаСпособаРасчетаАвто.ПредоплатаВДеньРеализации;
			Иначе
...
Процедура ОбновитьПараметрыФискальногоЧекаМестоРасчетов(ПараметрыОперацииФискализацииЧека, ПараметрыОперацииЧека)
	...
	Если ПараметрыОперацииЧека.Свойство("АдресИнтернетМагазина") Тогда
		ФормированиеПараметровФискальногоЧекаСерверПереопределяемый.ПараметрыФискальногоЧекаЗаполнитьИнтернетРасчеты(
			ПараметрыОперацииФискализацииЧека, ПараметрыОперацииЧека.АдресИнтернетМагазина);
	КонецЕсли;
...
Процедура ОбновитьПараметрыФискальногоЧекаПризнакСпособаРасчетаПозицийЧека(ПараметрыОперацииФискализацииЧека, ПараметрыОперацииЧека)
	...
	Если ПараметрыОперацииЧека.ЕдиныйЧек Тогда
		...
		СуммаПредоплаты = ПараметрыОперацииЧека.СуммаПредоплатыВнеОдногоДня;
		
		ФормированиеПараметровФискальногоЧекаСервер.ОбновитьПараметрыФискальногоЧекаПозицийЧекаЕдиныйЧек(
			ПараметрыОперацииФискализацииЧека, ПараметрыОперацииЧека.ДокументыЕдиногоЧека, СуммаПредоплаты);
...
Процедура ОбновитьПараметрыФискальногоЧекаТаблицаОплат(ПараметрыОперацииФискализацииЧека, ПараметрыОперацииЧека)
	...
	Если ПараметрыОперацииЧека.ЕдиныйЧек Тогда
		...
		ФормированиеПараметровФискальногоЧекаСервер.ОбновитьПараметрыФискальногоЧекаТаблицаОплатЕдиныйЧек(
			ПараметрыОперацииФискализацииЧека, ПараметрыОперацииЧека.ДокументыЕдиногоЧека, СуммаПредоплаты);
...
Процедура ОбновитьПараметрыФискальногоЧекаЧасоваяЗона(ПараметрыОперацииФискализацииЧека, ПараметрыОперацииЧека)
	
	Если ПараметрыОперацииЧека.Свойство("ЧасоваяЗона") И ЗначениеЗаполнено(ПараметрыОперацииЧека.ЧасоваяЗона) Тогда
		ПараметрыОперацииФискализацииЧека.ЧасоваяЗона = ПараметрыОперацииЧека.ЧасоваяЗона;
	Иначе
		ПараметрыОперацииФискализацииЧека.ЧасоваяЗона = Неопределено;
	КонецЕсли;
...
Функция ДокументОплаты(ДокументСсылка)
	...
	ДанныеДокументаОплаты = ДокументОплатыСВозможностьюПробитияЕдиногоЧека(ДокументСсылка);
	Если ДанныеДокументаОплаты.ЕстьВозможностьПробитияЕдиногоЧека Тогда
		ДокументОплаты.Добавить(ДокументСсылка);
	Иначе
...
// Проверяет возможность пробития единого чека для документа оплаты и связанных с ним объектов расчетов.
// 
// Параметры:
//  ДанныеДокументаОплаты - см. ДокументОплатыСВозможностьюПробитияЕдиногоЧека
//  ОбъектыРасчетов - Массив из ДокументСсылка
//  
// Возвращаемое значение:
//  Булево
Функция ДоступнаВозможностьПробитияЕдиногоЧека(ДанныеДокументаОплаты, ОбъектыРасчетов)
	
	Если Не ЗначениеЗаполнено(ДанныеДокументаОплаты.Ссылка) ИЛИ ОбъектыРасчетов = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если Не ДанныеДокументаОплаты.ЕстьВозможностьПробитияЕдиногоЧека Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если Не ПоВсемОбъектамРасчетовВозможностиПробитиеЕдиногоЧекаПоставки(ОбъектыРасчетов, ДанныеДокументаОплаты.ЭтоПриходДенежныхСредств) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ДатаОплатыОтличаетсяОтДатыПоставки(ДанныеДокументаОплаты.Ссылка, ОбъектыРасчетов) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ПоДокументуОплатыПробитЧек(ДанныеДокументаОплаты.Ссылка) Тогда
		Возврат Ложь;
	КонецЕсли;
...
// Возвращает структуру с описанием документа оплаты единого чека. Данные применимы, только если свойство 
// ЕстьВозможностьПробитияЕдиногоЧека установлено в значение Истина.
// 
// Параметры:
...
//   *ЕстьВозможностьПробитияЕдиногоЧека - Булево
//   *ЭтоПриходДенежныхСредств - Неопределено,Булево
...
Функция ДокументОплатыСВозможностьюПробитияЕдиногоЧека(ДокументСсылка)
	...
	Результат.Вставить("ЕстьВозможностьПробитияЕдиногоЧека", Не РезультатЗапроса.Пустой());
	Результат.Вставить("ЭтоПриходДенежныхСредств", Неопределено);
	Результат.Вставить("СуммаДокумента", 0);
	
	Если Результат.ЕстьВозможностьПробитияЕдиногоЧека Тогда
		Выборка = РезультатЗапроса.Выбрать();
		Если Выборка.Следующий() Тогда
			Результат.ЭтоПриходДенежныхСредств = Выборка.ЭтоПриходДенежныхСредств;
			Результат.СуммаДокумента = Выборка.СуммаДокумента;
		КонецЕсли;
	КонецЕсли;
...
// Проверяет, что переданный тип документа поставки разрешен для использования в едином чеке для указанной 
// хозайственной операции.
//
// Параметры:
//  ТипДокументаПоставки - Тип - Тип документа поставки для проврерки
//  ХозяйственнаяОперация - ПеречислениеСсылка.ХозяйственныеОперации - Хозяйственная операция, для которой выполняется проверка.
//
// Возвращаемое значение:
//  Булево -
//
Функция ТипДокументаПоставкиРазрешенДляХозяйственнойОперации(ТипДокументаПоставки, ХозяйственнаяОперация)	
	...
	Иначе
		РазрешенныеТипы.Добавить(Тип("ДокументСсылка.ВозвратТоваровОтКлиента"));
	КонецЕсли;

	Возврат РазрешенныеТипы.Найти(ТипДокументаПоставки) <> Неопределено;
КонецФункции

Функция ДокументПоставкиСВозможностьюПробитияЕдиногоЧека(ДокументСсылка, ЭтоПриходДенежныхСредств)
	
	Если ЭтоПриходДенежныхСредств = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылка);
	Запрос.УстановитьПараметр("ЭтоПриходДенежныхСредств", ЭтоПриходДенежныхСредств);
	...
	|	И &ЭтоПриходДенежныхСредств = ИСТИНА
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И &ЭтоПриходДенежныхСредств = ЛОЖЬ	
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		Или РеализацияТоваровУслуг.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности))
...
Функция ПоВсемОбъектамРасчетовВозможностиПробитиеЕдиногоЧекаПоставки(ОбъектыРасчетов, ЭтоПриходДенежныхСредств)
	
	ПоВсемОбъектамРасчетовВозможностиПробитиеЕдиногоЧекаПоставки = Истина;
	
	Для Каждого СтрокаОбъектыРасчетов Из ОбъектыРасчетов Цикл
		
		Если Не ДокументПоставкиСВозможностьюПробитияЕдиногоЧека(СтрокаОбъектыРасчетов.Заказ, ЭтоПриходДенежныхСредств) Тогда
...
Функция ОпределитьАвтоПризнакСпособаРасчетовПоОнлайнРасчетам(ПараметрыОперации, ДанныеПоОбъектуРасчетов)
	...
	ДатаДокументаСсылки = НачалоДня(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "Дата"));
	
	ДолгОрганизации = 0;
	Пока Выборка.Следующий() Цикл
		
		Если (ДокументОплатыСВозможностьюПробитияЧеков(Выборка.ДокументРегистратор)
				И ДокументПоставкиСВозможностьюПробитияЧеков(Выборка.ДокументРасчета)
				И Выборка.ДокументРасчета = ДокументСсылка
				И ДатаДокументаСсылки = Выборка.Период)
			Или (Выборка.ДокументРегистратор = ДокументСсылка
				Или (ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.КорректировкаРеализации")
					И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ДокументОснование") = Выборка.ДокументРегистратор)) Тогда
			...
			СтруктураПризнака.Вставить("ПризнакСпособаРасчета");
			СтруктураПризнака.Вставить("Заказ");
			СтруктураПризнака.Вставить("ПредоплатаВДеньРеализации", 0);
			...
			Если ДокументПоставкиСВозможностьюПробитияЧеков(ДокументСсылка) Тогда
				Если ТипЗнч(Выборка.ДокументРегистратор) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
					Если ЗначениеЗаполнено(Выборка.ДатаВозникновения) Тогда
						СтруктураПризнака.Поставка = Выборка.ДолгКлиента + Выборка.НашДолг;
					Иначе
						Продолжить;
					КонецЕсли;
				ИначеЕсли ДокументОплатыСВозможностьюПробитияЧеков(Выборка.ДокументРегистратор) Тогда
					СтруктураПризнака.ДокументРасчета = Выборка.ДокументРегистратор;
					СтруктураПризнака.ПредоплатаВДеньРеализации = МодульЧисла(Выборка.ДолгКлиента) + МодульЧисла(Выборка.НашДолг);
				Иначе
					СтруктураПризнака.Поставка = МодульЧисла(Выборка.ДолгКлиента) + МодульЧисла(Выборка.НашДолг);
				КонецЕсли;
			Иначе
			...
			Если СтруктураПризнака.Поставка > 0 
				ИЛИ СтруктураПризнака.Оплата > 0 
				ИЛИ (СтруктураПризнака.Поставка < 0 
					И ТипЗнч(Выборка.ДокументРегистратор) = Тип("ДокументСсылка.КорректировкаРеализации"))
				ИЛИ СтруктураПризнака.ПредоплатаВДеньРеализации > 0 Тогда
...
Функция РассчитатьСтруктуруОплатДокументаПоставки(ПризнакиСпособаРасчетаАвто,
	...
	Иначе
		...
		Для Каждого СтрокаПризнакиСпособаРасчетаАвто Из ПризнакиСпособаРасчетаАвто Цикл
			...
			ДокументОплаты = ДокументОплатыСВозможностьюПробитияЕдиногоЧека(СтрокаПризнакиСпособаРасчетаАвто.ДокументРасчета);
			Если ДокументОплаты.ЕстьВозможностьПробитияЕдиногоЧека
			...
			Иначе
				...
				СуммаПредоплаты = СуммаПредоплаты + ПризнакиСпособаРасчетаАвто.Итог("ПредоплатаВДеньРеализации");
...
Функция ДанныеПоОбъектамРасчетовПоДокументу(ДокументСсылка, ИмяКомандыПробитияЧека)
	...
	Иначе
		...
		Возврат МодульЛокализации.ДанныеПоОбъектамРасчетовПоДокументу(ДокументСсылка, ИмяКомандыПробитияЧека);
...
Функция ДанныеПоОбъектамРасчетовИзРасшифровкиПлатежа(ДокументСсылка)
	...
	ДокументОплаты = ДокументОплатыСВозможностьюПробитияЕдиногоЧека(ДокументСсылка);
	...
	Если ТипЗнч(РасшифровкаПлатежа) = Тип("ТаблицаЗначений") Тогда
		...
		Если ДокументОплаты.ЕстьВозможностьПробитияЕдиногоЧека Тогда
	...
	Для Каждого СтрокаРасшифровкиПлатежа Из РасшифровкаПлатежа Цикл
		Если ЗначениеЗаполнено(СтрокаРасшифровкиПлатежа.ОбъектРасчетов) Тогда
			...
			Если (ТипЗнч(Объект) = Тип("ДокументСсылка.ЗаказКлиента")
				...
				И ДокументОплаты.ЕстьВозможностьПробитияЕдиногоЧека
				И ДокументПоставкиСВозможностьюПробитияЕдиногоЧека(СтрокаРасшифровкиПлатежа.ОснованиеПлатежа, ДокументОплаты.ЭтоПриходДенежныхСредств) Тогда
...
// Возвращает объекты расчетов в привязке к документам оплаты.
// 
// Параметры:
//  ДокументыОплаты - Массив Из ДокументСсылка - Документы оплаты
//  ИмяКомандыПробитияЧека - Строка - Имя команды пробития чека
// Возвращаемое значение:
//  Неопределено,ТаблицаЗначений:
//   *Заказ - ДокументСсылка,СправочникСсылка
...
//   *Сумма - Число
//
Функция ОбъектыРасчетовПоДокументамОплатыРазвернуто(ДокументыОплаты, ИмяКомандыПробитияЧека)
	...
	Для Каждого ДокументОплаты Из ДокументыОплаты Цикл
		Если ОбъектыРасчетовДокументаОплаты = Неопределено Тогда
			...
			ОбъектыРасчетовДокументаОплаты.Колонки.Добавить("ДокументОплаты");
			ОбъектыРасчетовДокументаОплаты.ЗаполнитьЗначения(ДокументОплаты, "ДокументОплаты");
		Иначе
			...
			Для Каждого СтрокаДанныхПоОбъекту Из ДанныеПоОбъектамРасчетовПоДокументу Цикл
				...
				НоваяСтрокаОбъектовРасчетов.ДокументОплаты = ДокументОплаты;		
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;

	Если ОбъектыРасчетовДокументаОплаты <> Неопределено Тогда
		ОбъектыРасчетовДокументаОплаты.Свернуть("Заказ,ДокументОплаты", "Сумма");
	КонецЕсли;
