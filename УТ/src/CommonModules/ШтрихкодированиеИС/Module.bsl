...
//
// Параметры:
//  СтрокиТаблицы - Массив Из СтрокаТаблицыЗначений - Массив из строк таблицы значений типа
...
//   * МаркируемаяПродукция - Булево - Истина, если продукция подлежит маркированию.
...
Функция ДанныеШтрихкодовУпаковок(СтрокиТаблицы, ПараметрыСканирования, ТипыУпаковок, ПроверкаНаGS1 = Ложь) Экспорт
	...
	ДанныеШтрихкодовУпаковок.Колонки.Добавить("Количество",                        Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(18,8)));
	...
	ДанныеШтрихкодовУпаковок.Колонки.Добавить("ВидПродукции",             Новый ОписаниеТипов("СправочникСсылка.ВидыПродукцииИС"));
	...
	СписокЗапросов.Добавить(ШтрихкодированиеОбщегоНазначенияИС.ТекстЗапросаСвойстваМаркируемойПродукции(, ПараметрыСканирования));
...
Процедура ОбработатьШтрихкодыУпаковокПоДаннымБазы(ПараметрыСканирования, ДанныеПоШтрихкодам, ШтрихкодыМаркируемойПродукции, ТипыУпаковок, ЗаполнитьПоСсылкам) Экспорт
	...
	НайденныеУпаковки = ДанныеШтрихкодовУпаковок(ШтрихкодыМаркируемойПродукции, ПараметрыСканирования, ТипыУпаковок);
	...
	КолонкиЗаполнения = ШтрихкодированиеОбщегоНазначенияИС.ИменаКолонокДляЗаполнения().ШтрихкодыУпаковок;
	...
	Для Каждого НайденнаяУпаковка Из НайденныеУпаковки Цикл
		...
		Если ЗначениеЗаполнено(НайденнаяУпаковка.Номенклатура) Тогда
			...
			Если СтрокаДанныхКодаМаркировки.ВидПродукции <> НайденнаяУпаковка.ВидПродукции Тогда
				...
				ШтрихкодированиеОбщегоНазначенияИС.УточнитьДанныеРазбораПоВидуПродукции(
					СтрокаДанныхКодаМаркировки, НайденнаяУпаковка.ВидПродукции, ПараметрыСканирования, ОбщиеМодули, ЭтоСканированиеВФормуПроверкиИПодбора);
			...
			Если Не ЗаполнитьПоСсылкам Тогда
				...
				ОбновлятьКоличество = Истина;
				Если СтрокаДанныхКодаМаркировки.ДанныеРазбора <> Неопределено Тогда
					СоставКодаМаркировки = СтрокаДанныхКодаМаркировки.ДанныеРазбора.СоставКодаМаркировки;
					Если СоставКодаМаркировки.Свойство("МассаНеттоВКг")
						Или СоставКодаМаркировки.Свойство("ОбъемВЛитрах") Тогда
						ОбновлятьКоличество = Ложь;
					КонецЕсли;
				КонецЕсли;
				
				Если ОбновлятьКоличество Тогда
				...
				Иначе
					СтрокаДанныхКодаМаркировки.ТребуетсяОбновлениеШтрихкода = Истина;
				КонецЕсли;
		...
		ИначеЕсли Не СтрокаДанныхКодаМаркировки.ЭтоКодВводаОстатков
				И Не СтрокаДанныхКодаМаркировки.ПрисутствуетАлкогольнаяПродукция
				И Не ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМЕХ(СтрокаДанныхКодаМаркировки.ВидПродукции) Тогда
		...
		Если ПараметрыСканирования.ИСМП И ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаДанныхКодаМаркировки.ВидПродукции, Истина) Тогда
			
			//@skip-check wrong-string-literal-content
			МодульШтрихкодированиеИСМП = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияИСМП");
			МодульШтрихкодированиеИСМП.УстановитьПризнакОтказаОтПроверкиКодовМаркировкиПоСтроке(СтрокаДанныхКодаМаркировки, ПараметрыСканирования, ЭтоСканированиеВФормуПроверкиИПодбора);
		...
		Если СтрокаДанныхКодаМаркировки.СтрокаДерева <> Неопределено Тогда
			...
			Если КолонкиЗаполненияСтрокой = Неопределено Тогда
				...
				Для Каждого Колонка Из СтрокаДанныхКодаМаркировки.СтрокаДерева.Владелец().Колонки Цикл
					Если КолонкиЗаполнения[Колонка.Имя] <> Неопределено
						И ДанныеПоШтрихкодам.ДанныеКодовМаркировки.Колонки.Найти(Колонка.Имя) <> Неопределено Тогда
...
// Также возможно получение информации из сервиса Государственных Информационных Систем,
// если данное взаимодействие и получение информации предусмотрено.
//
// Параметры:
//  ДокументСсылка - ДокументСсылка - Ссылка на документ.
//  ВидПродукции - Неопределено, СправочникСсылка.ВидыПродукцииИС - Вид продукции, по которому необходимо осуществить
...
//  ОтключитьИспользованиеСервиса - Булево - Истина, если не требуется обращение в сервис.
// Возвращаемое значение:
//  Структура - Описание:
//   * ШтрихкодыПродукцииДляСопоставления - Массив Из Строка - Массив штрихкодов, которые необходимо сопоставить с номенклатурой,
//   * ВидыПродукции - Массив из СправочникСсылка.ВидыПродукцииИС - Массив видов продукции,
...
Функция ВложенныеШтрихкодыУпаковокПоДокументу(ДокументСсылка, ВидПродукции = Неопределено, Детализация = Неопределено, 
	ОтключитьИспользованиеСервиса = Ложь, ПараметрыСканирования = Неопределено) Экспорт

	Если ПараметрыСканирования = Неопределено Тогда
		ДанныеШапки = ПолучитьСтруктуруРеквизитовДокумента(ДокументСсылка);
		ВидыПродукцииПоТоварнойГруппе = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукции(ДанныеШапки.ТоварнаяГруппа);
		ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(ДанныеШапки,, ВидыПродукцииПоТоварнойГруппе);
	КонецЕсли;

	ВложенныеШтрихкоды = Неопределено;

	Если ПараметрыСканирования.ИСМП // Документы ИСМП и документы производства/импорта пива - продукция
			И ИнтеграцияИС.СодержитсяВПодсистеме("ГосИС", ДокументСсылка.Метаданные()) Тогда
		...
		ВложенныеШтрихкоды = Модуль.ВложенныеШтрихкодыУпаковокПоДокументу(ДокументСсылка, ПараметрыСканирования);
	ИначеЕсли ПараметрыСканирования.ЕГАИС // Документы ЕГАИС, включая производство пива - сырье
			И ИнтеграцияИС.СодержитсяВПодсистеме("ГосИС.ЕГАИС", ДокументСсылка.Метаданные()) Тогда
		Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеЕГАИС");
		ВложенныеШтрихкоды = Модуль.ВложенныеШтрихкодыУпаковокПоДокументу(ДокументСсылка);
	Иначе
		Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеИСМП");
		ВложенныеШтрихкоды = Модуль.ВложенныеШтрихкодыУпаковокПоДокументу(ДокументСсылка, ПараметрыСканирования);
	КонецЕсли;
...
//
// Параметры:
//  МассивУпаковок - Массив из СправочникСсылка.ШтрихкодыУпаковокТоваров - Коллекция элементов справочника Штрихкоды упаковок товаров.
...
Функция ЗначенияШтрихкодовИзДереваМаркируемойПродукции(ДеревоМаркируемойПродукции, ЗначенияШтрихкодов = Неопределено, НаличиеКолонок = Неопределено) Экспорт
	...
	Для Каждого СтрокаДерева Из ДеревоМаркируемойПродукции.Строки Цикл
		...
		ИначеЕсли СтрокаДерева.ТипУпаковки = Перечисления.ПрочиеЗоныПересчетаПродукцииИС.ГруппировкаОбъемноСортовогоУчетаПоGTIN Тогда
			СтрокаДерева.Штрихкод = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ОСУПоШтрихкодуEAN(
				СтрокаДерева.GTIN, СтрокаДерева.КоличествоПодчиненныхПотребительскихУпаковок);
			Результат.Добавить(СтрокаДерева);
		КонецЕсли;
...
Процедура ЗаполнитьДанныеRFID(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	Если ШтрихкодированиеОбщегоНазначенияИС.ДопустимаПродукцияИзНатуральногоМеха(ПараметрыСканирования)
		...
		Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
			...
			Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМЕХ(СтрокаДанных.ВидПродукции)
...
//   * ВидПродукции - Неопределено, СправочникСсылка.ВидыПродукцииИС - Вид продукции кода маркировки.
...
Функция ОбщиеКолонкиКоллекцийВложенныхШтрихкодов()
	...
	КоллекцияОбязательныхКолонок.Вставить("РазрешительныйРежимИдентификаторЭкземпляраПО", Новый ОписаниеТипов("Строка"));
	КоллекцияОбязательныхКолонок.Вставить("РазрешительныйРежимВерсияБазы",                Новый ОписаниеТипов("Строка"));
	...
	КоллекцияОбязательныхКолонок.Вставить("СтатусРД",                                     Новый ОписаниеТипов("ПеречислениеСсылка.ГруппыСтатусовРазрешительныхДокументовИСМП"));
...
//	                         Штрихкоды упаковок документа, с дополнительными данными частичного выбытия.
...
Функция ВложенныеШтрихкодыУпаковок(Упаковки, ПараметрыСканирования = Неопределено, ЗаполнитьСведенияПоШтрихкодамEAN = Ложь, СортироватьДанные = Ложь) Экспорт
	...
	ШтрихкодированиеОбщегоНазначенияИС.ДополнитьДанныеШтрихкодовСпецификойМаркируемойПродукции(ДанныеПоШтрихкодам, ПараметрыСканирования, ПараметрыДополнения, Истина);
...
Процедура ЗаполнитьДанныеВложенныхШтрихкодовДереваУпаковок(ПараметрыСканирования, ДанныеПоШтрихкодам) Экспорт
	...
	Если (ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоПроверкаКодовМаркировкиИСМП(ПараметрыСканирования)
		Или ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоОтчетПроизводственнойЛинии(ПараметрыСканирования))
...
Процедура ЗаполнитьСведенияПоШтрихкодамПоДаннымПулаGTIN(ДанныеПоШтрихкодам, ПараметрыСканирования, ДанныеКодовМаркировки = Неопределено) Экспорт
	...
	Если ПараметрыСканирования.ИСМП Тогда
		...
		ЭтоКонтекстРозничногоДокумента = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ЭтоВидОперацииРозничнаяПродажа(ПараметрыСканирования.ВидОперацииИСМП)
			Или ПараметрыСканирования.ВидОперацииИСМП = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ВидОперацииРозничныйВозврат();
		
		Если ЭтоКонтекстОптовогоДокумента Или ЭтоКонтекстРозничногоДокумента Тогда
			ТребуетсяПоискВПуле = Ложь;
		КонецЕсли;
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаДанных.ПолныйКодМаркировки) Или ТребуетсяПоискВПуле Тогда
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Если ДанныеПулаПоКоду <> Неопределено Тогда
			...
			Иначе
				СтрокаДанных.ЕстьВПулеКодовМаркировки = (ДанныеПулаПоКоду.ПолныйКодМаркировки = СтрокаДанных.ПолныйКодМаркировки);
			КонецЕсли;
			...
			Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(СтрокаДанных.ВидПродукции)
			...
			Если Не ДанныеПулаПоКоду.СохраненоПриСканировании Тогда
				...
				Если Не ЗначениеЗаполнено(СтрокаДанных.ВидУпаковки) Тогда
					...
					Если СтрокаДанных.ВидПродукции <> ДанныеПулаПоКоду.ВидПродукции Тогда
						...
						ШтрихкодированиеОбщегоНазначенияИС.УточнитьДанныеРазбораПоВидуПродукции(
							СтрокаДанных, ДанныеПулаПоКоду.ВидПродукции, ПараметрыСканирования, ОбщиеМодули, ЭтоСканированиеВФормуПроверкиИПодбора);
					...
					Если Не ВидыПродукцииПринадлежатТоварнойГруппе(
						СтрокаДанных.ВидПродукции,
						ДанныеПулаПоКоду.ВидПродукции) Тогда
		...
		НайденныеШтрихкодыEAN = ШтрихкодированиеОбщегоНазначенияИС.НайденныеШтрихкодыEAN(ЗначениеПоиска, ЗначениеУточнения, ИнформацияПоШтрихкодамEAN);
			...
			ИначеЕсли Не СтрокаДанных.НоменклатураСопоставленаПоУПД
				...
				Если ЗначениеЗаполнено(СтрокаДанных.ВидПродукции) Тогда
					...
					Если УточнитьПоДаннымРазбора Тогда
						...
						ШтрихкодированиеОбщегоНазначенияИС.УточнитьДанныеРазбораПоВидуПродукции(
							СтрокаДанных, СтрокаДанных.ВидПродукции, ПараметрыСканирования,, ЭтоСканированиеВФормуПроверкиИПодбора);
		...
		Если ДанныеПулаПоКоду <> Неопределено
			...
			Если Не ЗначениеЗаполнено(СтрокаДанных.Номенклатура)
				...
				Если СтрокаДанных.ВидПродукции <> ДанныеПулаПоКоду.ВидПродукции Тогда
					...
					ШтрихкодированиеОбщегоНазначенияИС.УточнитьДанныеРазбораПоВидуПродукции(
						СтрокаДанных, ДанныеПулаПоКоду.ВидПродукции, ПараметрыСканирования,, ЭтоСканированиеВФормуПроверкиИПодбора);
		...
		Если ПараметрыСканирования.ИСМП
			...
			И ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаДанных.ВидПродукции, Истина)
		Тогда
		...
		Если Не ЧтениеШтрихкода.Результат = Неопределено Тогда
			Если ЧтениеШтрихкода.ТипШтрихкода = Перечисления.ТипыШтрихкодов.GS1_128
				...
				Иначе
					...
					ТипВходныхДанных = 1;
				КонецЕсли;
			ИначеЕсли ТипВходныхДанных <> Неопределено
				И (ЧтениеШтрихкода.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC
					Или ЧтениеШтрихкода.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC18
					Или ЧтениеШтрихкода.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC20) Тогда
				ЗначениеШтрихкодаДляКомпоненты = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ШтрихкодВBase64(
...
// * РазрешительныйРежимИдентификаторЭкземпляраПО - ОпределяемыйТип.УникальныйИдентификаторИС, Неопределено - идентификатор экземпляра ПО Локальный модуль
// * РазрешительныйРежимВерсияБазы                - ОпределяемыйТип.УникальныйИдентификаторИС, Неопределено - версия баз черных списков Локального модуля
// * ВыбытиеБутылки                               - Булево, Неопределено                                    - признак выбытия бутылки ЕГАИС
// * МинимальнаяРозничнаяЦена                     - Число - минимальная розничная цена для НСП
Функция НовыйЭлементКоллекцииУпаковокДляРаспределенияПоТоварам() Экспорт
	...
	ВозвращаемоеЗначение.Вставить("РазрешительныйРежимИдентификаторЭкземпляраПО", Неопределено);
	ВозвращаемоеЗначение.Вставить("РазрешительныйРежимВерсияБазы",                Неопределено);
	ВозвращаемоеЗначение.Вставить("ВыбытиеБутылки",                               Неопределено);
	ВозвращаемоеЗначение.Вставить("МинимальнаяРозничнаяЦена",                     Неопределено);
...
Функция ДобавитьИзменитьКодМаркировкиВскрытойПотребительскойУпаковки(ПараметрыСканирования, Товары, ШтрихкодыУпаковок = Неопределено) Экспорт
	...
	Если Не ИспользуетсяЧастичноеВыбытиеПриПродаже(ПараметрыСканирования) Тогда
		Возврат ВозвращаемоеЗначение;
	КонецЕсли;
	...
	ШтрихкодированиеОбщегоНазначенияИС.НормализироватьДанныеДереваУпаковокПоПереданнымКолонкам(ТаблицаШтрихкоды, ДобавляемыеКолонки);
	...
	Для Каждого СтрокаТаблицы Из ТаблицаТовары Цикл
		...
		Если НайденныеСтроки.Количество() > 1
			И Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(НайденныеСтроки[0].ВидПродукции)
		...
		Для Каждого СтрокаВскрытойУпаковки Из НайденныеСтроки Цикл
			...
			Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(СтрокаВскрытойУпаковки.ВидПродукции) Тогда
...
// 
// Параметры:
//   ШтрихкодыДляПроверки - Массив Из СправочникСсылка.ШтрихкодыУпаковокТоваров - проверяемые элементы.
//   ВидыПродукции - Массив Из СправочникСсылка.КлючиВидовПродукцииИС, СправочникСсылка.ВидыПродукцииИС, Неопределено - Вид отбираемой продукции.
Процедура ВыделитьШтрихкодыСодержащиеВидыПродукции(ШтрихкодыДляПроверки, Знач ВидыПродукции = Неопределено) Экспорт
	...
	Если ТипЗнч(ВидыПродукции) = Тип("Массив") Тогда
		...
		Для Каждого ВидПродукции Из ВидыПродукции Цикл
			Если ТипЗнч(ВидПродукции) = ТипКлючиВидовПродукцииИС Тогда
				...
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ВидыПродукцииДляПроверки, ВидыПродукцииКлюча, Истина);
			ИначеЕсли ТипЗнч(ВидПродукции) = ТипВидыПродукцииИС Тогда
				Если ВидыПродукцииДляПроверки.Найти(ВидПродукции) = Неопределено Тогда
					ВидыПродукцииДляПроверки.Добавить(ВидПродукции);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		
	ИначеЕсли ТипЗнч(ВидыПродукции) = ТипКлючиВидовПродукцииИС Тогда
		
		ВидыПродукцииКлюча = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииКлюча(ВидыПродукции);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			ВидыПродукцииДляПроверки, ВидыПродукцииКлюча);
		
	ИначеЕсли ТипЗнч(ВидыПродукции) = ТипВидыПродукцииИС Тогда
	...
	|	ВЫБОР
	|		КОГДА ОписаниеНоменклатуры.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииДляПроверки)
	|			ТОГДА ИСТИНА
	|		КОГДА ОписаниеНоменклатуры.КлючВидовПродукции.ВидПродукцииЕГАИС В (&ВидыПродукцииДляПроверки)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ПодходящаяПродукция
	|ИЗ
	|	Справочник.ШтрихкодыУпаковокТоваров КАК ШтрихкодыУпаковок
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ШтрихкодыУпаковокТоваров.ВложенныеШтрихкоды КАК ТаблицаВложенныеШтрихкоды
	|		ПО ТаблицаВложенныеШтрихкоды.Ссылка = ШтрихкодыУпаковок.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатуры
	|		ПО ОписаниеНоменклатуры.Номенклатура = ШтрихкодыУпаковок.Номенклатура
	|ГДЕ
	|	ШтрихкодыУпаковок.Ссылка В (&ШтрихкодУпаковки)
	|	И ТаблицаВложенныеШтрихкоды.Ссылка ЕСТЬ NULL
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	ШтрихкодыДляПроверки = Результат;
...
Функция РаспределениеШтрихкодовУпаковокПоТоварам(ПараметрыРаспределения, Товары, МассивУпаковок) Экспорт
	...
	ДанныеШапки               = ПолучитьСтруктуруРеквизитовДокумента(ПараметрыРаспределения.ДокументСсылка);
	...
	Если ШтрихкодированиеОбщегоНазначенияИС.ПрисутствуетПродукцияИСМП(ПараметрыСканирования.ДопустимыеВидыПродукции)
		...
		МодульОбщегоНазначенияИСМПКлиентСервер = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ОбщегоНазначенияИСМПКлиентСервер");
		МодульОбщегоНазначенияИСМПКлиентСервер.УстановитьПараметрСканированияЗапрашиватьДанныеСервисаИСМП(ПараметрыСканирования, Ложь);
	КонецЕсли;
	...
	ТаблицаТовары.Колонки.Добавить("Сумма",                  ОбщегоНазначения.ОписаниеТипаЧисло(15, 3));
	...
	ТаблицаТовары.Колонки.Добавить("УпаковкаОтсутствует",    Новый ОписаниеТипов("Булево"));
	...
	Для Каждого СтрокаТовары Из Товары Цикл
		...
		НоваяСтрока.УпаковкаОтсутствует = Не ЗначениеЗаполнено(НоваяСтрока.Упаковка);
	...
	ШтрихкодыУпаковок = ВложенныеШтрихкодыУпаковок(
		МассивУпаковок,
		ПараметрыСканирования,,
		Истина);
	...
	Для Каждого СтрокаТаблицы Из ТаблицаШтрихкоды Цикл
		...
		Если ДанныеПовторяющегосяКода <> Неопределено Тогда
			...
			Для Каждого СтрокаДанных Из ДанныеПовторяющегосяКода Цикл
				Если Не ЗначениеЗаполнено(СтрокаДанных.ЧастичноеВыбытиеКоличество) Тогда
					Продолжить;
				КонецЕсли;
				...
				Иначе
					...
					НоваяСтрока.РазрешительныйРежимИдентификаторЭкземпляраПО = СтрокаДанных.РазрешительныйРежимИдентификаторЭкземпляраПО;
					НоваяСтрока.РазрешительныйРежимВерсияБазы                = СтрокаДанных.РазрешительныйРежимВерсияБазы;
				КонецЕсли;
			КонецЦикла;
	...
	СвойстваЗаполненияИСМП  = "Штрихкод,ВидПродукции,ВидУпаковки,ПолныйКодМаркировки,СоставКодаМаркировки,
			                  |ШтрихкодУпаковки,ТипШтрихкода,КоличествоПотребительскихУпаковок,
	...
	СвойстваЗаполненияМОТП  = "МинимальнаяРозничнаяЦена";
	...
	Для Каждого СтрокаШтрихкод Из ТаблицаШтрихкоды Цикл
		...
		ТребуетсяПоискТоваровПриРаспределенииСУчетомЦены = ТребуетсяПоискТоваровПриРаспределенииСУчетомЦены(
			СтрокаШтрихкод, ПараметрыСканирования);
		Если ТребуетсяПоискТоваровПриРаспределенииСУчетомЦены Тогда
			СтруктураПоиска.Вставить("Цена", СтрокаШтрихкод.МРЦ);
		КонецЕсли;
		
		НайденныеСтрокиТовары = ТаблицаТовары.НайтиСтроки(СтруктураПоиска);
		
		Если ТребуетсяПоискТоваровПриРаспределенииСУчетомЦены
			И НайденныеСтрокиТовары.Количество() = 0
			И СтрокаШтрихкод.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая
			И СтрокаШтрихкод.КоличествоПотребительскихУпаковок > 0
			И ЗначениеЗаполнено(СтрокаШтрихкод.МРЦ) Тогда
			
			МРЦПачки = СтрокаШтрихкод.МРЦ / СтрокаШтрихкод.КоличествоПотребительскихУпаковок;
			СтруктураПоиска.Цена = МРЦПачки;
			СтруктураПоиска.Вставить("УпаковкаОтсутствует", Истина);
			НайденныеСтрокиТовары = ТаблицаТовары.НайтиСтроки(СтруктураПоиска);
		...
		Для Каждого СтрокаТовары Из НайденныеСтрокиТовары Цикл
			...
			РезультатСписания = ШтрихкодированиеИСКлиентСервер.НовыйРезультатРаспределенияШтрихкодовПоТоварам();
			Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(СтрокаШтрихкод.ВидПродукции) Тогда
			...
			Иначе
				...
				Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(СтрокаШтрихкод.ВидПродукции) Тогда
					ЗаполнитьЗначенияСвойств(РезультатСписания, СтрокаШтрихкод, СвойстваЗаполненияМОТП);
				КонецЕсли;
			...
			РезультатСписания.Сумма         = СтрокаТовары.Сумма;

			ОбработкаИтоговойСтрокиРаспределения(РезультатСписания, СтрокаШтрихкод, ПараметрыНормализацииКМ, ПараметрыНормализацииМОТП, ПараметрыНормализацииИСМП);
			...
			Если ЭтоОшибкаТребуетВзвешивания Тогда
				...
				РезультатСписания.ЗапрещеноИгнорироватьОшибку = Истина;
				Прервать;
			КонецЕсли;
	...
	Если ОбщегоНазначенияИС.ПодсистемаСуществует("ИСМП") Тогда
		МодульШтрихкодированиеОбщегоНазначенияИСМПКлиентСервер = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияИСМПКлиентСервер");
		МодульШтрихкодированиеОбщегоНазначенияИСМПКлиентСервер.ПриУстановкеДополнительныхСвойствЭлеметовПроверкиСредствамиККТ(
	...
	Для Каждого СтрокаШтрихкод Из ТаблицаШтрихкоды Цикл
		...
		Если ТребуетсяПоискТоваровПриРаспределенииСУчетомЦены(СтрокаШтрихкод, ПараметрыСканирования) Тогда
			...
			СтрокиБезУчетаЦены = ТаблицаТовары.НайтиСтроки(СтруктураПоиска);
			
			Если СтрокиБезУчетаЦены.Количество() Тогда
				...
				СписатьКоличествоТоваров = СтрокаТовары.Количество;
				Если СписатьКоличествоТоваров < СтрокаШтрихкод.Количество Тогда
					СписатьКоличествоТоваров = СтрокаШтрихкод.Количество;
				КонецЕсли;
				СтрокаТовары              = СтрокиБезУчетаЦены[0];
				СтрокаТовары.Количество   = СтрокаТовары.Количество - СписатьКоличествоТоваров;
				...
				РезультатСписания.ТекстОшибки = СтрШаблон(
					НСтр("ru ='Не удалось распределить код маркировки с МРЦ %1 по товарам:
						      |%2 (%3)'"),
					Формат(СтрокаШтрихкод.МРЦ, "ЧН=0; ЧГ=0;"),
					СтрокаШтрихкод.Штрихкод,
					Представление);
				
				ДобавитьОбщуюОшибку = Ложь;
				
			КонецЕсли;
			
		КонецЕсли;
		
		Если ДобавитьОбщуюОшибку Тогда
...
//   * ВидыПродукции - Массив Из СправочникСсылка.ВидыПродукцииИС - Коллекция видов продукции ИС, которые присутствуют в дереве упаковок
...
Функция НовыйЭлементДополненияВложенныхШтрихкодовУпаковокДанныеГИСМТ() Экспорт
	...
	ВозвращаемоеЗначение.Вставить("МинимальнаяРозничнаяЦена");
...
Процедура ОбновитьКэшМаркируемойПродукции(Форма, ПараметрыСканирования = Неопределено) Экспорт
	...
	ЗначенияВложенныхШтрихкодовМарок = ЗначенияВложенныхШтрихкодовУпаковок(УпаковкиОбъекта, ПараметрыСканирования);
...
Процедура ОбработатьУточнениеДанныхДляФормыПроверкиИПодбора(РезультатВыбора, РезультатОбработки, ПараметрыСканирования, КэшированныеЗначения) Экспорт
	...
	Если ТипЗнч(РезультатВыбора) = Тип("Строка") Тогда
		...
		Если ЭтоАдресВременногоХранилища(РезультатОбработки.АдресДереваУпаковок) Тогда
			
			ДеревоУпаковок = ПолучитьИзВременногоХранилища(РезультатОбработки.АдресДереваУпаковок);
			СтрокаДерева = ДеревоУпаковок.Строки.Найти(ДанныеШтрихкода.НормализованныйШтрихкод, "НормализованныйШтрихкод", Ложь);
			
			Если СтрокаДерева <> Неопределено Тогда
				
				МодульШтрихкодированиеИСМПСлужебный = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеИСМПСлужебный");
				МодульШтрихкодированиеИСМПСлужебный.НормализоватьДанныеДереваУпаковок(СтрокаДерева, ПараметрыСканирования);
				...
				ДанныеШтрихкода.МаркируемаяПродукция = СтрокаДерева.МаркируемаяПродукция;
				ДанныеШтрихкода.ПроизвольнаяЕдиницаУчета = СтрокаДерева.ПроизвольнаяЕдиницаУчета;
				ДанныеШтрихкода.ТребуетВзвешивания = СтрокаДерева.ТребуетВзвешивания;
	...
	ИначеЕсли РезультатВыбора.Свойство("ШтрихкодУпаковки") Тогда
		...
		// Уточнена необходимость сохранения выбора
		Если РезультатВыбора.Свойство("ДополнительныеПараметры")
			И РезультатВыбора.ДополнительныеПараметры <> Неопределено
			И РезультатВыбора.ДополнительныеПараметры.Свойство("ЗапомнитьВыбор") Тогда
			Если РезультатОбработки.ДополнительныеПараметры = Неопределено Тогда
				РезультатОбработки.ДополнительныеПараметры = Новый Структура;
			КонецЕсли;
			РезультатОбработки.ДополнительныеПараметры.Вставить("ДанныеВыбора", ОбщегоНазначения.СкопироватьРекурсивно(РезультатВыбора.ДополнительныеПараметры.ДанныеВыбора));
			РезультатОбработки.ДополнительныеПараметры.Вставить("ЗапомнитьВыбор", РезультатВыбора.ДополнительныеПараметры.ЗапомнитьВыбор);
		КонецЕсли;
	Иначе
		...
		РеквизитыУточнения = Новый Структура;
		РеквизитыУточнения.Вставить("ТребуетсяВыборСерии", Ложь);
		РеквизитыУточнения.Вставить("ТребуетсяВыборВСД",   Ложь);
		РеквизитыУточнения.Вставить("ТребуетсяВыборАлкогольнойПродукции", Ложь);
		РеквизитыУточнения.Вставить("ТребуетсяВыборСправки2", Ложь);
		ЗаполнитьЗначенияСвойств(ДанныеШтрихкода, РеквизитыУточнения);
		...
		Если Не ЗначениеЗаполнено(ДанныеШтрихкода.ВидПродукции) Тогда
			ДанныеШтрихкода.ВидПродукции = ОбщегоНазначенияИС.ОписаниеНоменклатуры(
				ДанныеШтрихкода.Номенклатура)[ДанныеШтрихкода.Номенклатура].КлючВидовПродукции;
		КонецЕсли;
		...
		ИначеЕсли КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ДанныеШтрихкода.ВидПродукции)
...
Функция ОбработатьУточнениеДанных(Форма, РезультатВыбора, РезультатОбработки, КэшированныеЗначения) Экспорт
	...
	ИначеЕсли ДанныеШтрихкода.ВидУпаковки = Перечисления.ВидыУпаковокИС.Набор
			И ДанныеШтрихкода.ТипУпаковки = Перечисления.ТипыУпаковок.МультитоварнаяУпаковка
			И ЗначениеЗаполнено(ДанныеШтрихкода.АдресДереваУпаковок) Тогда
		Возврат ОбработатьУточнениеДанныхДляУпаковки(Форма, РезультатВыбора, РезультатОбработки, КэшированныеЗначения);
	Иначе
...
Процедура ЗаполнитьИзвестныеУпаковкиРасширенные(ПараметрыСканирования, ДанныеПоШтрихкодам, ШтрихкодыУпаковокДляОпределения, ШтрихкодИСтрокаДанныхКодаМаркировки) Экспорт
	...
	Если НайденныеУпаковки.Количество() > 0 Тогда
		...
		Для Каждого СтрокаДляИзмененияШтрихкода Из СтрокиДляИзмененияШтрихкода Цикл
			...
			ШтрихкодированиеОбщегоНазначенияИС.ЗаполнитьСтрокуДанныхКодовМаркировкиПоДаннымРазбора(СтрокаДанныхКодаМаркировки, ОбщиеМодули, ПараметрыСканирования);
		...
		ДанныеПоШтрихкодамДобавленныеСтроки = ШтрихкодированиеОбщегоНазначенияИС.ИнициализацияДанныхПоШтрихкодам(Ложь);
		ЗаполнитьЗначенияСвойств(ДанныеПоШтрихкодамДобавленныеСтроки, ДанныеПоШтрихкодам);
		ДанныеПоШтрихкодамДобавленныеСтроки.ВложенныеШтрихкоды = Неопределено;
		ДанныеПоШтрихкодамДобавленныеСтроки.ДанныеКодовМаркировки = ДобавленныеСтроки;
		ЗаполнитьКоэффициентыВложенностиУпаковокПоШтрихкодамEAN(ДанныеПоШтрихкодамДобавленныеСтроки, ПараметрыСканирования);
...
Функция ШтрихкодыУпаковок(ЗначенияШтрихкодов, ТипыУпаковок = Неопределено, ПроверкаНаGS1 = Ложь, УчитыватьОСУ = Истина) Экспорт
	...
	ШтрихкодыУпаковок.Колонки.Добавить("Количество",                        Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(18,8)));
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ИСМП") Тогда
		ГруппировкаОбъемноСортовогоУчетаПоGTIN = ПредопределенноеЗначение("Перечисление.ПрочиеЗоныПересчетаПродукцииИС.ГруппировкаОбъемноСортовогоУчетаПоGTIN");
	КонецЕсли;
	
	ЕстьСвойствоЭлементаТипУпаковки  = Неопределено;
	ЭтоФиксированнаяКоллекцияКолонок = Неопределено;
	...
	Для Каждого ЭлементЗначения Из ЗначенияШтрихкодов Цикл
		...
		Если ТипЗнч(ЭлементЗначения) = Тип("СтрокаТаблицыЗначений")
			...
			Если ЭлементЗначения.ВидУпаковки = Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет Тогда
				ЭтоОСУ = Истина;
			ИначеЕсли ГруппировкаОбъемноСортовогоУчетаПоGTIN <> Неопределено Тогда
				
				Если ЭтоФиксированнаяКоллекцияКолонок = Неопределено Тогда
					ЭтоФиксированнаяКоллекцияКолонок = (ТипЗнч(ЭлементЗначения) = Тип("СтрокаТаблицыЗначений")
						Или ТипЗнч(ЭлементЗначения) = Тип("СтрокаДереваЗначений"));
				КонецЕсли;
				Если ЭтоФиксированнаяКоллекцияКолонок Тогда
					Если ЕстьСвойствоЭлементаТипУпаковки = Неопределено Тогда
						ЕстьСвойствоЭлементаТипУпаковки = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементЗначения, "ТипУпаковки");
					КонецЕсли;
				Иначе
					ЕстьСвойствоЭлементаТипУпаковки = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементЗначения, "ТипУпаковки");
				КонецЕсли;
				Если ЕстьСвойствоЭлементаТипУпаковки
		...
		Если ПроверкаНаGS1 И СтрНачинаетсяС(Штрихкод, "(") Тогда
			...
			Если ПараметрыШтрихкода.Результат = Неопределено
				Или ПараметрыШтрихкода.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC
				Или ПараметрыШтрихкода.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC18
				Или ПараметрыШтрихкода.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC20 Тогда
				Продолжить;
			КонецЕсли;
	...
	|ГДЕ
	|	ТаблицаШтрихкодов.ЭтоОСУ
	|	И ТаблицаШтрихкодов.ХешСумма = ШтрихкодыУпаковокТоваров.ХешСумма
	|	И 1 = 1
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	ТаблицаШтрихкодов КАК ТаблицаШтрихкодов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ШтрихкодыУпаковокТоваров КАК ШтрихкодыУпаковокТоваров
	|		ПО ШтрихкодыУпаковокТоваров.ХешСуммаНормализации = """"
	|		И ШтрихкодыУпаковокТоваров.ХешСуммаЗначенияШтрихкода = """"
	|		И ТаблицаШтрихкодов.Штрихкод = ШтрихкодыУпаковокТоваров.ЗначениеШтрихкода
	|ГДЕ
	|	1 = 1
	|;
	|
...
Процедура СформироватьДанныеДокументаОснования(ПараметрыСканирования) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыСканирования.ДокументОснование)
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ИСМП") Тогда
			Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияИСМПВЕТИС");
		...
		Если Не ДанныеСформированы И ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ЕГАИС") Тогда
			Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияЕГАИСИСМП");
			Модуль.СформироватьДанныеДокументаОснования(
				ТаблицаДанныхДокументаОснования, ПараметрыСканирования.ДокументОснование, ДанныеСформированы);
		КонецЕсли;
...
//  ПараметрыСканирования - См. ПараметрыСканирования
//  ОтборПоВидуПродукции - Неопределено, СправочникСсылка.ВидыПродукцииИС - фильтр для добавления
...
Процедура ДополнитьТаблицуДанныеКодовМаркировкиПоСтрокамДереваУпаковок(ДанныеПоШтрихкодам, ДеревоУпаковок, Изменения = Неопределено, ПараметрыСканирования = Неопределено, ПараметрыДополнения = Неопределено) Экспорт
	...
	КолонкиДанныеСервисаБезПредставления = Неопределено;
	...
	Для Каждого СтрокаДерева Из ДеревоУпаковок.Строки Цикл
		...
		Если Не ПараметрыДополнения.ЭтоЗагрузкаИзТСД И НайденныеСтроки.Количество() Тогда
			...
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				...
				ВидПродукцииДоЗаполнения = НайденнаяСтрока.ВидПродукции;
				...
				Если ЗначениеЗаполнено(ПараметрыДополнения.ПараметрыСлиянияДанных.КолонкиДанныеСервиса)
					...
					Если КолонкиДанныеСервисаБезПредставления = Неопределено Тогда
						КолонкиМассивом = СтрРазделить(ПараметрыДополнения.ПараметрыСлиянияДанных.КолонкиДанныеСервиса, ",", Ложь);
						ИндексКолонки   = КолонкиМассивом.Найти("ПредставлениеНоменклатуры");
						Если ИндексКолонки = Неопределено Тогда
							КолонкиДанныеСервисаБезПредставления = ПараметрыДополнения.ПараметрыСлиянияДанных.КолонкиДанныеСервиса;
						Иначе
							КолонкиМассивом.Удалить(ИндексКолонки);
							КолонкиДанныеСервисаБезПредставления = СтрСоединить(КолонкиМассивом, ",");
						КонецЕсли;
					КонецЕсли;
					Если ЗначениеЗаполнено(НайденнаяСтрока.ПредставлениеНоменклатуры)
						И ЗначениеЗаполнено(НайденнаяСтрока.Номенклатура)
						И Не ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоПроверкаКодовМаркировкиИСМП(ПараметрыСканирования) Тогда
						КолонкиДанныеСервиса = КолонкиДанныеСервисаБезПредставления;
					Иначе
						КолонкиДанныеСервиса = ПараметрыДополнения.ПараметрыСлиянияДанных.КолонкиДанныеСервиса;
					КонецЕсли;
					ЗаполнитьЗначенияСвойств(НайденнаяСтрока, СтрокаДерева, КолонкиДанныеСервиса);
					ПоляСкопированы = Истина;
				КонецЕсли;
				
				Если ЗначениеЗаполнено(ПараметрыДополнения.ПараметрыСлиянияДанных.КолонкиДанныеСервиса)
					И ЗначениеЗаполнено(ПараметрыДополнения.ПараметрыСлиянияДанных.КолонкиИББезСервиса)
					И Не ЗначениеЗаполнено(СтрокаДерева.Статус)
					И Не ЗначениеЗаполнено(НайденнаяСтрока.Статус) Тогда
					ЗаполнитьЗначенияСвойств(НайденнаяСтрока, СтрокаДерева, ПараметрыДополнения.ПараметрыСлиянияДанных.КолонкиИББезСервиса);
				КонецЕсли;
				...
				// Если ключ вида продукции был заменен видом продукции, но вид соответствует ключу
				// значит ключ нужно оставить в данных строки
				Если ОбщегоНазначенияИСКлиентСерверПовтИсп.ВидПродукцииСоответствуетКлючу(
					НайденнаяСтрока.ВидПродукции, ВидПродукцииДоЗаполнения) Тогда
					НайденнаяСтрока.ВидПродукции = ВидПродукцииДоЗаполнения;
				КонецЕсли;
		...
		Иначе
			...
			Если ЗначениеЗаполнено(СтрокаДерева.ВидПродукции) Тогда
				...
				Если ТипЗнч(СтрокаДерева.ВидПродукции) = ТипКлючиВидовПродукцииИС Тогда
					ВидПродукцииКлюча = Неопределено;
					Если ПараметрыСканирования.ИСМП Тогда
						ВидПродукцииКлюча = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(СтрокаДерева.ВидПродукции, "ГИСМТ");
					КонецЕсли;
					Если ВидПродукцииКлюча = Неопределено И ПараметрыСканирования.ЕГАИС Тогда
						ВидПродукцииКлюча = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(СтрокаДерева.ВидПродукции, "ЕГАИС");
					КонецЕсли;
					НоваяСтрока.ВидыПродукцииКодаМаркировки.Добавить(ВидПродукцииКлюча);
				Иначе
			...
			ИначеЕсли ПараметрыДополнения.ЕстьКолонкаТипУпаковки
				...
				Если СтрокаДерева.ДанныеРазбора <> Неопределено
					И НоваяСтрока.ВидыПродукцииКодаМаркировки.Количество() = 0 Тогда
					Для Каждого ВидПродукции Из СтрокаДерева.ДанныеРазбора.ВидыПродукции Цикл
						НоваяСтрока.ВидыПродукцииКодаМаркировки.Добавить(ВидПродукции);
					КонецЦикла;
				КонецЕсли;
	...
	ДанныеПоШтрихкодам.ВидыПродукции = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ДанныеКодовМаркировки.ВыгрузитьКолонку("ВидПродукции"));
...
Функция СоздатьМаркированныйТоварПоДаннымШтрихкода(ДанныеШтрихкода, ПараметрыСканирования, РезультатОбработки = Неопределено) Экспорт
	...
	Если ДанныеНовойУпаковки.ТипУпаковки = Перечисления.ТипыУпаковок.МаркированныйТовар Тогда
		Если ОбщегоНазначенияИСПовтИсп.ЭтоМаркируемаяПродукцияЕГАИС(ДанныеШтрихкода.ВидПродукции)
...
Функция ВидыПродукцииПринадлежатТоварнойГруппе(ВидПродукции, ВидПродукцииВПулеКодовМаркировки) Экспорт
	
	ТоварнаяГруппа1 = КонструкторИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппаПоВидуПродукции(ВидПродукции);
	ТоварнаяГруппа2 = КонструкторИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппаПоВидуПродукции(ВидПродукцииВПулеКодовМаркировки);
	
	Возврат ТоварнаяГруппа1 = ТоварнаяГруппа2;
...
Процедура РасширенныеПроверкиШтрихкодов(ПараметрыСканирования, ДанныеПоШтрихкодам, ЕстьОшибки) Экспорт
	...
	Если ПараметрыСканирования.ПропускатьСтрокиСОшибкамиПриЗагрузкеИзТСД Или Не ЕстьОшибки Тогда
		...
		Если ПараметрыСканирования.ЕГАИС
			И ШтрихкодированиеОбщегоНазначенияИС.ПрисутствуетАлкогольнаяПродукция(ВидыПродукции) Тогда
...
Функция ТекстЗапросаСоставаУпаковокВИерархии(ПараметрыСканирования)
	...
	ТекстЗапросаСоставаУпаковок = ТекстЗапросаИсходныеДанные + ТекстЗапросаВложенныхШтрихкодов(ПараметрыСканирования);
...
Функция ТекстЗапросаВложенныхШтрихкодов(ПараметрыСканирования)
...
Функция ОбработатьУточнениеНоменклатуры(ФормаПараметрыСканирования, РезультатВыбора, РезультатОбработки, КэшированныеЗначения)
	...
	СвойстваМаркируемойПродукции = ОбщегоНазначенияИС.СвойстваМаркируемойПродукции(ДанныеШтрихкода.Номенклатура, ПараметрыСканирования);
	...
	РезультатПроверки = ШтрихкодированиеОбщегоНазначенияИС.ТребуетсяОткрытьФормуВводаКодаМаркировки(ДанныеШтрихкода, ПараметрыСканирования,, ПараметрыСканирования.ЭтоСканированиеВФормеПроверкиИПодбора);
...
Функция ОбработатьУточнениеДанныхДляМаркированногоТовара(Форма, РезультатВыбора, РезультатОбработки, КэшированныеЗначения)
	...
	ЭтоСтрокаДанныхЧастичногоВыбытия = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоСтрокаДанныхЧастичногоВыбытия(
		ДанныеВыбора, ПараметрыСканирования);
...
Процедура ДополнитьДеревоУпаковокИШтрихкодыУпаковокУточненнымиДанными(ДанныеШтрихкода, ИменаСвойств, ПерезаписатьШтрихкодУпаковки)
	...
	Если ПерезаписатьШтрихкодУпаковки Тогда
	...
	Если ДанныеШтрихкода.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая
		...
		НайденнаяСтрока = ДеревоУпаковок.Строки.Найти(ДанныеШтрихкода.НормализованныйШтрихкод, "НормализованныйШтрихкод");
		Если НайденнаяСтрока = Неопределено Тогда
			СтрокиДерева = ДеревоУпаковок.Строки;
		Иначе
			СтрокиДерева = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(НайденнаяСтрока);
		КонецЕсли;
		...
		Для Каждого СтрокаДерева Из СтрокиДерева Цикл
			...
			Если ПерезаписатьШтрихкодУпаковки
			...
			Для Каждого СтрокаПотребительскойУпаковки Из СтрокаДерева.Строки Цикл
				...
				Если ПерезаписатьШтрихкодУпаковки
					И ЗначениеЗаполнено(СтрокаПотребительскойУпаковки.ШтрихкодУпаковки) Тогда
...
Функция ПроверитьНеобходимостьУточнения(СтрокаДерева, ДанныеКодовМаркировки, ПараметрыСканирования, ДанныеДокументаОснования)
	...
	Если СтрокаДерева.Строки.Количество() = 0
		И (ЗначениеЗаполнено(СтрокаДерева.GTIN)
			Или СтрокаДерева.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская)
	Тогда
		...
		Если Не ТребуетсяУточнение Тогда
			Если ПараметрыСканирования.ИСМП Тогда
				МодульСпецифики = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеИСМПСлужебный");
		...
		Если Не ТребуетсяУточнение Тогда
			Если ПараметрыСканирования.ЕГАИС Тогда
				МодульСпецифики = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеЕГАИС");
				ТребуетсяУточнение = МодульСпецифики.ПроверитьНеобходимостьУточнения(СтрокаДерева, ПараметрыСканирования);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПоместитьДанныеСтрокДереваВСоответствие(СтрокиДерева, Соответствие)
	...
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		...
		Если Не ГрупповаяОбработкаШтрихкодовИС.ЭтоАгрегатТСД(СтрокаДерева.НормализованныйШтрихкод) Тогда
...
Процедура ЗаполнитьСтрокуДереваУпаковокПоВыборке(СтрокаДерева, Выборка, Уровень, ОбщиеМодули,
	...
	ИначеЕсли СтрокаДерева.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC
		Или СтрокаДерева.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC18
		Или СтрокаДерева.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC20
...
Функция ЗаполнитьСтрокуДереваПоДаннымРазбораШтрихкода(СтрокаДерева, ОбщиеМодули, НастройкиРазбора, ПараметрыСканирования = Неопределено)
	...
	Иначе
		ВидыПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииКлюча(СтрокаДерева.ВидПродукции);
	КонецЕсли;
	...
	Если ДанныеРазбора = Неопределено
		...
		Если ВыключенныеВидыПродукции.Количество() Тогда
			НастройкиРазбора.Общие = РазборКодаМаркировкиИССлужебныйПовтИсп.НастройкиРазбораКодаМаркировки(, Ложь);
		КонецЕсли;
		Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМОТП(СтрокаДерева.ВидПродукции)
			И СтрокаДерева.ВидПродукции = ВидыПродукции Тогда
			ВидыПродукцииДляРазбора = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииМОТП();
		КонецЕсли;
	...
	Если ДанныеРазбора <> Неопределено Тогда
		...
		Если ДанныеРазбора.ТипШтрихкода <> Перечисления.ТипыШтрихкодов.SSCC
			Или СтрокаДерева.ТипШтрихкода <> Перечисления.ТипыШтрихкодов.SSCC18
				И СтрокаДерева.ТипШтрихкода <> Перечисления.ТипыШтрихкодов.SSCC20 Тогда
	...
	Для Каждого СтрокаДерева1 Из ДеревоЗначений1.Строки Цикл
		...
		Если ПерваяИтерация Тогда
			...
			Если НайденнаяСтрокаДерева2 = Неопределено Тогда
				...
				Если ЕстьКолонкаСодержитСерыеКоды
					И СтрокаДерева1.СодержитСерыеКоды Тогда
					Продолжить;
				КонецЕсли;

				Если СтрокаДерева1.ВидУпаковки = Перечисления.ВидыУпаковокИС.Набор
					И Не СозданиеНаборов Тогда
					Продолжить;
				КонецЕсли;
				
				Если (ТипЗнч(ДеревоЗначений1) = Тип("ДеревоЗначений") Или ТипЗнч(ДеревоЗначений1) = Тип("Структура"))
					И СтрокаДерева1.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
					Продолжить;
				КонецЕсли;
...
//  из дерева и соответствия штрихкодов и строк дерева.
//
// Параметры:
//  ДеревоУпаковок - ДеревоЗначений, СтрокаДереваЗначений - строки для проверки на соответствие виду продукции.
//  СоответствиеСтрокДереваУпаковок - Соответствие из КлючИЗначение - соответствие Штрихкодов упаковок и строк дерева.
...
Функция ВложенныеШтрихкодыУпаковокПоДаннымМенеджераВТ(ВыборкаПерваяИтерация, МенеджерВременныхТаблиц, ПараметрыСканирования, ПараметрыДополнения) 
	...
	Пока ПродолжатьПоиск Цикл
		...
		Иначе
			...
			ТекстЗапроса = ТекстЗапросаВложенныхШтрихкодов(ПараметрыСканирования);
	...
	Для Каждого КлючЗначение Из СоответствиеСтрокДереваУпаковок Цикл
		...
		Если КлючЗначение.Значение.ТипУпаковки = Перечисления.ТипыУпаковок.МаркированныйТовар Тогда
			...
			Если ПараметрыДополнения.ДанныеЧастичногоВыбытия[НоваяСтрока.ШтрихкодУпаковки] <> Неопределено Тогда
				Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(НоваяСтрока.ВидПродукции) Тогда
	...
	Для Каждого КлючЗначение Из ИменаМодулейПараметровДополнения Цикл
		Если Не КлючЗначение.Значение Тогда
			Продолжить;
		КонецЕсли;
		Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль(КлючЗначение.Ключ);
...
//  ОтборПоВидуПродукции - Неопределено, СправочникСсылка.ВидыПродукцииИС - отбор по виду продукции
...
//
// Возвращаемое значение:
//  Массив из СправочникСсылка.ВидыПродукцииИС - массив вложенных видов продукции.
...
//
// Параметры:
//  ДеревоУпаковок - ДеревоЗначений, СтрокаДереваЗначений - строки, которые будут удалены из соответствия
//  СоответствиеСтрокДереваУпаковок - Соответствие из КлючИЗначение - соответствие Штрихкодов упаковок и строк дерева
...
Функция ШтрихкодыУпаковокПоДаннымТоваровИМаркируемойПродукции(Форма, ПараметрыСканирования)
	...
	Для Каждого СтрокаТЧ Из ТабличнаяЧастьШтрихкоды Цикл
		...
		Для Каждого ИмяКолонки Из МассивИменКолонокШтрихкодовУпаковки Цикл
			...
			Если ЕстьКолонкаЧастичноеВыбытиеВариантУчета Тогда
				...
				КонецЕсли;
				Если ЗначениеЗаполнено(ЭлементЧастичногоВыбытия.ЧастичноеВыбытиеВариантУчета)
					И (ЭлементЧастичногоВыбытия.ЧастичноеВыбытиеВариантУчета = Перечисления.ВариантыУчетаЧастичногоВыбытияИС.НастроеннаяНоменклатура
						И ЗначениеЗаполнено(ЭлементЧастичногоВыбытия.ЧастичноеВыбытиеНоменклатура)
						Или ЭлементЧастичногоВыбытия.ЧастичноеВыбытиеВариантУчета <> Перечисления.ВариантыУчетаЧастичногоВыбытияИС.НастроеннаяНоменклатура)Тогда
	...
	Запрос.УстановитьПараметр("ВидыПродукцииПоддерживающиеПеременноеКоличество", КонструкторИСМПКлиентСервер.ВидыПродукцииПоддерживающиеПеременноеКоличествоМернойПродукции());
	РезультатыЗапроса = ОбщегоНазначенияИС.ВыполнитьПакетЗапросов(Запрос, ТекстыЗапроса);
...
Процедура ЗаполнитьДанныеПоШтрихкодамНаОснованииСохраненногоВыбора(ДанныеПоШтрихкодам, ПараметрыСканирования, ЭлементыДляИзменения) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если ЗначениеЗаполнено(ДанныеСохраненногоВыбора.Агрегация) Тогда
			ЗаполнитьДанныеПоШтрихкодамНаОснованииРезультатаУточненияСоставаУпаковки(
				СтрокаДанных, ДанныеСохраненногоВыбора.Агрегация, ПараметрыСканирования, ЭлементыДляИзменения);
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьДанныеПоШтрихкодамНаОснованииРезультатаУточненияСоставаУпаковки(СтрокаДанных, ДанныеСохраненногоВыбора, ПараметрыСканирования, ЭлементыДляИзменения)
	
	Для Каждого ВариантВыбора Из ДанныеСохраненногоВыбора Цикл
		
		Если ШтрихкодированиеИСКлиентСервер.ТребуетсяСброситьСохраненныйВыбор(ВариантВыбора, СтрокаДанных) Тогда
			Продолжить;
		КонецЕсли;
...
Функция ЗначенияВложенныхШтрихкодовУпаковок(КоллекцияУпаковок, ПараметрыСканирования)
	...
	Запрос = Новый Запрос(ТекстЗапросаСоставаУпаковокВИерархии(ПараметрыСканирования));
...
Процедура ЗаполнитьПоДокументуИОснованию(ДанныеПоШтрихкодам, ПараметрыСканирования, Форма) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаДанных.ВидПродукции) Тогда
			МодульСпецификиИСМП = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеИСМПСлужебный");
	...
	Если НайденныеУпаковки.Количество() > 0 Тогда
		...
		Для Каждого НайденнаяУпаковка Из НайденныеУпаковки Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаДанныхКодаМаркировки.ВидУпаковки)
				...
				Если ТипЗнч(СтрокаДанныхКодаМаркировки.ВидПродукции) = Тип("СправочникСсылка.КлючиВидовПродукцииИС") Тогда
					ВидыПродукцииКлюча = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииКлюча(СтрокаДанныхКодаМаркировки.ВидПродукции);
					Если ВидыПродукцииКлюча.Количество() = 1 Тогда
						ВидПродукцииКлюча = ВидыПродукцииКлюча[0];
					Иначе
						ВидПродукцииКлюча = Неопределено;
					КонецЕсли;
				Иначе
					ВидПродукцииКлюча = СтрокаДанныхКодаМаркировки.ВидПродукции;
				КонецЕсли;
				
				ДоступныеВидыУпаковок = СтрокаДанныхКодаМаркировки.ДанныеРазбора.ВидыУпаковокПоВидамПродукции[ВидПродукцииКлюча];
				Если СтрокаДанныхКодаМаркировки.ТипУпаковки = Перечисления.ТипыУпаковок.МультитоварнаяУпаковка
					И ДоступныеВидыУпаковок <> Неопределено
...
Функция ВложенныеШтрихкодыПоМассивуУпаковокБезСпецифики(МассивУпаковок, ПараметрыСканирования)
	...
	НастройкиРазбора = Новый Структура;
	НастройкиРазбора.Вставить("Кеш",              РазборКодаМаркировкиИССлужебныйКлиентСервер.ИнициализироватьНастройкиИспользующиеРезультатыПредыдущихРазборов());
	НастройкиРазбора.Вставить("Общие",            РазборКодаМаркировкиИССлужебныйПовтИсп.НастройкиРазбораКодаМаркировки(, ТолькоУчитываемыеВидыПродукции));
	НастройкиРазбора.Вставить("Пользовательские", ПользовательскиеПараметрыРазбораКодаМаркировки);
...
Процедура ЗаполнитьТаблицуШтрихкодовЧастичногоВыбытияИзДереваУпаковокРекурсивно(ТаблицаПриемник, ДеревоУпаковок, ТребуетсяЧастичноеВыбытие) Экспорт
	...
	Для Каждого СтрокаДерева Из ДеревоУпаковок.Строки Цикл
		...
		ИначеЕсли Не ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаДерева.ВидПродукции, Истина)
			И Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(СтрокаДерева.ВидПродукции) Тогда
			Продолжить;
		ИначеЕсли СтрокаДерева.ВидУпаковки = Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет
...
Функция ИспользуетсяЧастичноеВыбытиеПриПродаже(ПараметрыСканирования)
	...
	Если ОбщегоНазначенияИС.ПодсистемаСуществует("ИСМП") Тогда
		...
		Для Каждого ВидПродукции Из ВидыПродукцииИСМП Цикл
			Если ОбщегоНазначенияИСКлиентСерверПовтИсп.ПоддерживаетсяЧастичноеВыбытие(
				ВидПродукции,
				ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ОперацияИС(ПараметрыСканирования, ВидПродукции)) Тогда
	...
	Если ОбщегоНазначенияИС.ПодсистемаСуществует("ЕГАИС") Тогда
		МодульОбщегоНазначенияЕГАИС = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ОбщегоНазначенияЕГАИС");
		ИспользуетсяМаркируемаяПродукцияЕГАИС = МодульОбщегоНазначенияЕГАИС.ИспользуетсяМаркируемаяПродукция();
		Для Каждого ВидПродукции Из КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЕГАИСМаркируемые() Цикл
			Если ИспользуетсяМаркируемаяПродукцияЕГАИС
				И ОбщегоНазначенияИСКлиентСерверПовтИсп.ПоддерживаетсяЧастичноеВыбытие(
					ВидПродукции,
					ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ОперацияИС(ПараметрыСканирования, ВидПродукции)) Тогда
				Возврат ШтрихкодированиеОбщегоНазначенияИС.ТребуетсяЧастичноеВыбытие(ПараметрыСканирования);
...
#Область РаспределениеШтрихкодовУпаковокПоТоварам
...
Процедура ОбработкаИтоговойСтрокиРаспределения(РезультатСписания, СтрокаШтрихкод, ПараметрыНормализацииКМ, ПараметрыНормализацииМОТП, ПараметрыНормализацииИСМП)
	...
	Если ЗначениеЗаполнено(РезультатСписания.ПолныйКодМаркировки)
		И КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(РезультатСписания.ВидПродукции, Истина) Тогда
		...
		Если ДанныеРазбора <> Неопределено Тогда
			...
			Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(РезультатСписания.ВидПродукции) Тогда
				НормализованныйКодМаркировки = РазборКодаМаркировкиИССлужебныйКлиентСервер.НормализоватьКодМаркировки(
					ДанныеРазбора, РезультатСписания.ВидПродукции, ПараметрыНормализацииМОТП);
			Иначе

				НормализованныйКодМаркировки = РазборКодаМаркировкиИССлужебныйКлиентСервер.НормализоватьКодМаркировки(
					ДанныеРазбора, РезультатСписания.ВидПродукции, ПараметрыНормализацииИСМП);

			КонецЕсли;

			РезультатСписания.КодИдентификации = НормализованныйКодМаркировки;

		КонецЕсли;

	КонецЕсли;
	
	Если МодульКонструкторИСМПКлиентСервер.КодМаркировкиВключаетНулевойМРЦ(РезультатСписания.ВидПродукции)
	...
	ИначеЕсли МодульКонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(РезультатСписания.ВидПродукции)
	...
	Для Каждого ЭлементОписания Из ИсходныеСтроки Цикл
		...
		Для Каждого СтрокаРаспределения Из ВозвращаемоеЗначениеРаспределения.РаспределенныеСтроки[ЭлементОписания.СтрокаТовары] Цикл
			...
			ИначеЕсли ЗначениеЗаполнено(ОписаниеНоменклатуры.КоличествоВПотребительскойУпаковке) Тогда
				Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ИспользоватьЧислительИЗнаменательПриЧастичномВыбытииНаККТ(СтрокаРаспределения.ВидПродукции) Тогда
...
// 
// Параметры:
//  СтрокиРаспределения - Массив из см. ШтрихкодированиеИСКлиентСервер.НовыйРезультатРаспределенияШтрихкодовПоТоварам
...
Процедура ДополнитьРаспределениеПоТоварамБезШтрихкодов(ВозвращаемоеЗначение, ПараметрыРаспределения, ПараметрыСканирования, Дата, ТаблицаТовары, Товары)
	...
	ПоддерживаетсяОбъемноСортовойУчет = ПараметрыСканирования.Свойство("ПоддерживаетсяОбъемноСортовойУчет")
	...
	Если ПоддерживаетсяОбъемноСортовойУчет Тогда
		...
		ТаблицаДанныхОСУ.Колонки.Добавить("ВидПродукции",                       Новый ОписаниеТипов("СправочникСсылка.ВидыПродукцииИС"));
		...
		МодульЭлектронноеВзаимодействиеИСМП = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ЭлектронноеВзаимодействиеИСМП");
		МодульЭлектронноеВзаимодействиеИСМП.ДополнитьРезультатКодамиОСУ(
	...
	Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
		...
		Если ПоддерживаетсяОбъемноСортовойУчет Тогда
			...
			Если СтрокиАвтоматическогоОСУ.Количество() Тогда
				...
				РезультатСписания.Штрихкод = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ОСУПоШтрихкодуEAN(
					СтрокаАвтоматическогоОСУ.ЗначениеШтрихкода, КоличествоПотребительскихУпаковок);
		...
		Если ВидПродукции = Неопределено Тогда
			ВидПродукции = ОбщегоНазначенияИС.ОписаниеНоменклатуры(
				СтрокаТовары.Номенклатура)[СтрокаТовары.Номенклатура].КлючВидовПродукции;
		...
		Если ЭтоПродукцияИСМП Тогда
			...
			МодульОбщегоНазначенияИСМППовтИсп = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ОбщегоНазначенияИСМППовтИсп");
			Если ПараметрыРаспределения.ЭтоПробитиеЧека Тогда
				ТребуетсяКонтрольМарок = МодульОбщегоНазначенияИСМППовтИсп.ДействуетУведомительныйРежимПоВидуПродукции(ВидПродукции, Дата)
					И Не МодульОбщегоНазначенияИСМППовтИсп.РазрешенОборотНемаркируемыхОстатковПродукции(ВидПродукции, Дата);
			Иначе
				ТребуетсяКонтрольМарок = МодульОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПоВидуОперацииИПродукции(ОперацияИС, ВидПродукции, Дата);
			КонецЕсли;
...
Функция ТребуетсяПоискТоваровПриРаспределенииСУчетомЦены(СтрокаШтрихкод, ПараметрыСканирования)
	Возврат КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(СтрокаШтрихкод.ВидПродукции)
		И ЗначениеЗаполнено(СтрокаШтрихкод.МРЦ)
		И (СтрокаШтрихкод.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая
			Или СтрокаШтрихкод.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская)
		И ПараметрыСканирования.ИспользуетсяРаспределениеКодовПоТоварамСУчетомМРЦ;
КонецФункции
