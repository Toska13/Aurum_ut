...
//  СправочникМетаданные - ОбъектМетаданных - метаданные справочника для проверки прав доступа.
//
Процедура ДобавитьКомандыСозданияБанковскихСчетов(Форма, СправочникМетаданные) Экспорт
	...
	Если Не ПравоДоступа("Добавление", СправочникМетаданные) Тогда
		Возврат;
	КонецЕсли;
...
// Добавляет доступные формы оплаты
//
// Параметры:
//  ФормыОплаты - Массив из ПеречислениеСсылка.ФормыОплаты - список форм оплаты. 
//
Процедура ДобавитьДоступныеФормыОплаты(ФормыОплаты) Экспорт

	//++ Локализация
	ФормыОплаты.Добавить(Перечисления.ФормыОплаты.СистемаБыстрыхПлатежей);
	//-- Локализация
...
//  Отказ            - Булево - Признак отказа от продолжения работы
//  ХозяйственнаяОперацияДокумента - ПеречислениеСсылка.ХозяйственныеОперации - хозяйственная операция документа
//  ОшибкиЗаполнения - Строка - Строка, накапливающая ошибки проверок.
//
Процедура ПроверитьКорреспонденциюБанковскихСчетов(ДокументОбъект, Отказ, ХозяйственнаяОперацияДокумента, ОшибкиЗаполнения = "") Экспорт
	...
	ЭтоОперацияМеждуОрганизациями    = Не ОперацииМеждуОрганизациями.Найти(ХозяйственнаяОперацияДокумента) = Неопределено;
	ЭтоОперацияОрганизацияКонтрагент = Не ОперацииОрганизацияКонтрагент.Найти(ХозяйственнаяОперацияДокумента) = Неопределено;
	ЭтоЗапрещеннаяОперация           = Не ЗапрещенныеОперации.Найти(ХозяйственнаяОперацияДокумента) = Неопределено;
...
// Очищает реквизиты платежей в бюджет после проверки даты применения Приказа 58н 
//
// Параметры:
...
Процедура ПриИзмененииПравилПлатежейВБюджет(Объект, ПредыдущийПериод, НовыйПериод) Экспорт
	...
	ДействовалиНовыеПравила = ПлатежиВБюджетКлиентСервер.ДействуетПриказ58н_202604(ПредыдущийПериод);
	ДействуютНовыеПравила = ПлатежиВБюджетКлиентСервер.ДействуетПриказ58н_202604(НовыйПериод);
...
//     * Подразделение - Строка, Неопределено - Филиал, имеющий регистрацию в налоговом органе
...
Функция СписокКППОрганизации(Организация) Экспорт
	...
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	Регистрации.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	Регистрации.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане
	|ПОМЕСТИТЬ КППКрупнейшегоНалогоплательщика
	|ИЗ
	|	РегистрСведений.КрупнейшийНалогоплательщикИсторияРегистрацийВНалоговомОргане.СрезПоследних(, СтруктурнаяЕдиница = &Организация) КАК Регистрации
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|
	|ВЫБРАТЬ
	|	КППКрупнейшегоНалогоплательщика.РегистрацияВНалоговомОргане.КПП,
	|	"""",
	|	КППКрупнейшегоНалогоплательщика.РегистрацияВНалоговомОргане
	|ИЗ
	|	КППКрупнейшегоНалогоплательщика КАК КППКрупнейшегоНалогоплательщика
	|	
	|ИНДЕКСИРОВАТЬ ПО
	|	КПП
	|;
...
//	Объект - ДокументОбъект - Документ оплаты.
//	Отказ - Булево - Признак отказа от продолжения работы
//	НепроверяемыеРеквизиты - Массив из Строка - Непроверяемые реквизиты документа
//	ФлагОбменСБанками - Булево - используется обмен с банком
//	ОшибкиЗаполнения - Строка - Накопленное описание ошибок.
//
Процедура ПроверитьЗаполнениеНалоговыхРеквизитов(Объект, Отказ, НепроверяемыеРеквизиты, ФлагОбменСБанками = Ложь, ОшибкиЗаполнения = "") Экспорт
	...
	Дата = ?(ЗначениеЗаполнено(Объект.Дата), Объект.Дата, ТекущаяДатаСеанса());
	...
	ПараметрыПроверкиЗаполнения.Вставить("ПрименениеПриказа107н",
		Дата >= Константы.ДатаНачалаПримененияПриказа107н.Получить());
	ПараметрыПроверкиЗаполнения.Вставить("ПрименениеПриказа126н",
		ДатаНачалаПримененияПриказа126н <> '00010101' 
			И Дата >= ДатаНачалаПримененияПриказа126н);
	ПараметрыПроверкиЗаполнения.Вставить("ПрименениеПриказа58н",
		ПлатежиВБюджетКлиентСервер.ДействуетПриказ58н_202604(Дата));
	...
	Если ПеречислениеВБюджет И МассивОпераций.Найти(ХозяйственнаяОперация) <> Неопределено Тогда
		...
		Если ПлатежиВБюджетКлиентСервер.СтатусыПлательщика(
			...
			ТекстОшибки = НСтр("ru = 'Неверное значение поля ""Статус плательщика""'");
		...
		ДоступныеСтатусыПлательщика = ПлатежиВБюджетКлиентСервер.ДоступныеСтатусыПлательщикаПоВидуПлатежа(ВидПеречисленияВБюджет, Объект.КодБК, Объект.Дата);
		
		Если ПараметрыПроверкиЗаполнения.ПрименениеПриказа58н
			И ДоступныеСтатусыПлательщика.НайтиПоЗначению(Объект.СтатусСоставителя) = Неопределено Тогда
		
			СтрокаДоступныхСтатусов = СтрСоединить(ДоступныеСтатусыПлательщика.ВыгрузитьЗначения(), ", ");
			ТекстОшибки =
				СтрШаблон(НСтр("ru = 'В поле ""Статус плательщика"" следует указать одно из доступных значений: %1'"), СтрокаДоступныхСтатусов);
			Если ФлагОбменСБанками Тогда
				ДобавитьОшибкуЗаполнения(ОшибкиЗаполнения, ТекстОшибки);
			Иначе
		...
		ИначеЕсли ВидПеречисленияВБюджет = Перечисления.ВидыПеречисленийВБюджет.ИнойПлатеж Тогда
			ПроверитьЗаполнениеРеквизитовИныхПлатежейВБюджет(Объект, ПараметрыПроверкиЗаполнения, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОперацияССамозанятымПриИзменении(Форма, ЕстьПривязкаЧека = Истина) Экспорт
	...
	ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(Форма, "Контрагент");
...
// Возвращает платежную ссылку b2b.
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка - Документ, для который выполняется поиск платежной ссылки.
// 
// Возвращаемое значение:
//  Строка, Неопределено - Платежная ссылка b2b
Функция ПлатежнаяСсылкаb2b(ДокументСсылка) Экспорт

	Результат = Неопределено;
	
	//++ Локализация
	Если ПереводыСБПb2b.ЧтениеПереводовСБПДоступно() Тогда
		
		ДанныеПлатежа = РегистрыСведений.ИдентификаторыОперацийСБПb2b.ДанныеПлатежнойСсылки(ДокументСсылка);
		
		Если ДанныеПлатежа <> Неопределено Тогда
			Результат = ДанныеПлатежа.ПлатежнаяСсылка;
		КонецЕсли;
...
// Возвращает двоичные данные платежные ссылки СБП b2b.
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка - Документ, для который выполняется поиск платежной ссылки.
//  Размер - Число - Размер картинки QR-кода.
// 
// Возвращаемое значение:
//  Неопределено, ДвоичныеДанные - Изображение QRКода СБПb2b
Функция ИзображениеQRКодаСБПb2b(ДокументСсылка, Размер = 200) Экспорт

	ДвоичныеДанные = Неопределено;
	
	//++ Локализация

	ПлатежнаяСсылка = ПлатежнаяСсылкаb2b(ДокументСсылка);
	
	Если ПлатежнаяСсылка <> Неопределено Тогда
		
		ДвоичныеДанные = СистемаБыстрыхПлатежей.ИзображениеQRКодаСБП(ПлатежнаяСсылка, Размер, 0);
...
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - Форма документа.
//  ПараметрыКоманды - см. НовыйПараметрыКомандыОбновленияСтатусаОплатыСБП.
//
Процедура ДобавитьКомандуОбновленияСтатусаОплатыСБП(Форма, ПараметрыКоманды) Экспорт

	//++ Локализация
	СистемаБыстрыхПлатежейУТ.ДобавитьКомандуОбновленияСтатусаОплатыСБП(Форма, ПараметрыКоманды);
	//-- Локализация
...
// Возвращает структуру параметров команды обновления статуса оплаты СБП.
// 
// Возвращаемое значение:
//  Структура:
// * Родитель - Неопределено - форма документа или группа на форме.
// * МестоРасположения - Неопределено - элемент, перед которым будет расположена команда.
Функция НовыйПараметрыКомандыОбновленияСтатусаОплатыСБП() Экспорт
...
// Проверяет получателя, указанного в документе, и получателя, указанного в платежной ссылке СБП.
// 
// Параметры:
//  ДокументОбъект - ДокументОбъект - Документ, содержащий входящую платежную ссылку СБП.
//  РежимЗаписи - РежимЗаписиДокумента - режим записи документа.
//  Отказ - Булево - признак отказа от записи.
//
Процедура ПроверитьПолучателяПлатежныхСсылокСБП(ДокументОбъект, РежимЗаписи, Отказ) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВходящиеПлатежныеСсылкиСБП.ДокументОперации КАК ДокументОперации,
		...
		|	ВходящиеПлатежныеСсылкиСБП.ИНН КАК ИНН,
		|	ВходящиеПлатежныеСсылкиСБП.НомерСчета КАК СчетПолучателя,
		|	ВходящиеПлатежныеСсылкиСБП.ПроверкаПройдена КАК ПроверкаПройдена
		|ИЗ
		|	РегистрСведений.ВходящиеПлатежныеСсылкиСБП КАК ВходящиеПлатежныеСсылкиСБП
		|ГДЕ
		|	ВходящиеПлатежныеСсылкиСБП.ДокументОперации = &ДокументОперации";
	
	Запрос.УстановитьПараметр("ДокументОперации", ДокументОбъект.Ссылка);
	...
	Попытка
		...
		ЭлементБлокировки = Блокировка.Добавить(Метаданные.РегистрыСведений.ВходящиеПлатежныеСсылкиСБП.ПолноеИмя());
		ЭлементБлокировки.УстановитьЗначение("ДокументОперации", ДокументОбъект.Ссылка);
		...
		Для Каждого Реквизиты Из МассивПараметров Цикл
			
			Если Реквизиты.ТипРасчетов <> Перечисления.ТипыРасчетовСПартнерами.РасчетыСПоставщиком Тогда
				Продолжить;
			КонецЕсли;
			
			Контрагент = ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(ДокументОбъект, Реквизиты.Контрагент);
			
			Если ЗначениеЗаполнено(Контрагент) Тогда
				ПараметрыПроверки.ИНН = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "ИНН");
			КонецЕсли;
			
			СчетКонтрагента = ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(ДокументОбъект, Реквизиты.БанковскийСчетКонтрагента);
			
			Если ЗначениеЗаполнено(СчетКонтрагента) Тогда
				ПараметрыПроверки.НомерСчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СчетКонтрагента, "НомерСчета");
			КонецЕсли;
		...
		Пока Выборка.Следующий() Цикл
			...
			СсылкаКорректная = ДенежныеСредстваКлиентСерверЛокализация.ПолучательПлатежнойСсылкиСоответствуетДокументу(
				ПараметрыПроверки, Выборка);
			
			Если (Не СсылкаКорректная
				И Выборка.ПроверкаПройдена)
				Или (СсылкаКорректная
				И Не Выборка.ПроверкаПройдена) Тогда
				
				Если Не СсылкаКорректная Тогда
					
					ТекстСообщения = СтрШаблон(
						НСтр("ru = 'Реквизиты получателя платежа, указанные в платежной ссылке %1, не соответствуют данным получателя в документе.
									|Получатель платежа СБП: ИНН - %2 , счет - %3'"),
						Выборка.ПлатежнаяСсылка, Выборка.ИНН, Выборка.СчетПолучателя);
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
					
				КонецЕсли;
				
				НаборЗаписей = РегистрыСведений.ВходящиеПлатежныеСсылкиСБП.СоздатьНаборЗаписей();
				НаборЗаписей.Отбор.ДокументОперации.Установить(Выборка.ДокументОперации);
				НаборЗаписей.Отбор.ПлатежнаяСсылка.Установить(Выборка.ПлатежнаяСсылка);
				НаборЗаписей.Прочитать();
				
				Для Каждого Запись Из НаборЗаписей Цикл
					Запись.ПроверкаПройдена = СсылкаКорректная;
				КонецЦикла;
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Не удалось выполнить проверку платежных ссылок СБП по причине: %1'"),
...
// Заполняет реквизиты плательщика в объекте документа на указанную дату.
//
// Параметры:
//   ДокументОбъект - ДокументОбъект.ЗаявкаНаРасходованиеДенежныхСредств, ДокументОбъект.СписаниеБезналичныхДенежныхСредств, ДанныеФормыСтруктура - 
//                    документ, в который заполняются реквизиты плательщика
//   Дата - Дата - дата, на которую определяются реквизиты плательщика
//
Процедура ЗаполнитьРеквизитыПлательщика(ДокументОбъект, Дата) Экспорт

	СтруктураРеквизитовПлательщика = РеквизитыПлательщика(ДокументОбъект, Дата);
	ЗаполнитьЗначенияСвойств(ДокументОбъект, СтруктураРеквизитовПлательщика);

КонецПроцедуры

// Обновляет реквизиты плательщика в форме объекта на указанную дату.
//
// Параметры:
//   ДокументОбъект - ДокументОбъект.ЗаявкаНаРасходованиеДенежныхСредств, ДокументОбъект.СписаниеБезналичныхДенежныхСредств, ДанныеФормыСтруктура - 
//                    документ, в котором обновляются реквизиты плательщика
//   Дата - Дата - Дата, на которую актуализируются реквизиты плательщика
//
Процедура ОбновитьРеквизитыПлательщика(ДокументОбъект, Дата) Экспорт

	РеквизитыПлательщика = РеквизитыПлательщика(ДокументОбъект, Дата);
	ПреобразоватьПустойРеквизитПлательщика(ДокументОбъект.ПеречислениеВБюджет, ДокументОбъект.ИННПлательщика);
	ПреобразоватьПустойРеквизитПлательщика(ДокументОбъект.ПеречислениеВБюджет, ДокументОбъект.КПППлательщика);
	ДокументОбъект.ТекстПлательщика = РеквизитыПлательщика.ТекстПлательщика;

КонецПроцедуры

// Возвращает реквизиты плательщика для указанного объекта.
//
// Параметры:
//   Объект - ДокументОбъект.ЗаявкаНаРасходованиеДенежныхСредств,
//            ДокументОбъект.СписаниеБезналичныхДенежныхСредств - документ оплаты.
//          - ДанныеФормыСтруктура - структура с данными документа.
//          - Структура - данные плательщика и платежа:
...
//            * ПеречислениеВБюджет - Булево - признак перечисления денежных средств в бюджет.
//            * ТипНалога - СправочникСсылка.ВидыНалоговВзносов - тип налога при перечислении в бюджет, используется при Истина в ПеречислениеВБюджет.
//            * КодБК - Строка - КБК при перечислении в бюджет, используется при Истина в ПеречислениеВБюджет.
//   Дата - Дата - дата, на которую формируются реквизиты
//
// Возвращаемое значение:
//   см. ДанныеПлательщика
//
Функция РеквизитыПлательщика(Объект, Дата) Экспорт
	
	Результат = ДанныеПлательщика(Объект.Организация, Объект.БанковскийСчет, Дата, Объект.ПеречислениеВБюджет);
	
	Если Объект.ПеречислениеВБюджет Тогда
		
		ИНН = Результат.ИННПлательщика;
		ИНН = СтрЗаменить(ИНН, "0", "");
		
		Если ПустаяСтрока(ИНН) Тогда
			Результат.ИННПлательщика = "0";
		КонецЕсли;
		
		КПП = Результат.КПППлательщика;
		КПП = СтрЗаменить(КПП, "0", "");
		
		Если (ПустаяСтрока(КПП) 
			Или Результат.ИННПлательщика = "0") Тогда
			Результат.КПППлательщика = "0";
		КонецЕсли;
		
		Если ПлатежиВБюджетКлиентСервер.ДействуетПриказ58н_202604(Дата)
			...
			И СтрДлина(Результат.ИННПлательщика) <> 5 Тогда
			Результат.КПППлательщика = "0";
		КонецЕсли;
		
	Иначе
		
		Если Лев(Результат.ИННПлательщика, 2) = "00" Тогда
			Результат.ИННПлательщика = "0";
		КонецЕсли;
		
		Если Лев(Результат.КПППлательщика, 2) = "00" Тогда
			Результат.КПППлательщика = "0";
		КонецЕсли;
...
//  НаДату - Дата - дата актуальности данных
//  ЭтоПеречислениеВБюджет - Булево - признак перечисления в бюджет
// 
// Возвращаемое значение:
//  Структура - данные плательщика:
// * ИННПлательщика - Строка - ИНН.
// * КПППлательщика - Строка - КПП.
// * ТекстПлательщика - Строка - наименование плательщика в печатных документах.
//
Функция ДанныеПлательщика(Организация, БанковскийСчет, НаДату, ЭтоПеречислениеВБюджет) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ИННПлательщика", "");
	Результат.Вставить("КПППлательщика", "");
	Результат.Вставить("ТекстПлательщика", "");
	
	Если ЗначениеЗаполнено(Организация) Тогда
		
		Отбор = ДенежныеСредстваСервер.ПараметрыДляПоискаДанныхСторонДокумента();
		Отбор.Дата = ?(НаДату = Дата('00010101'), ТекущаяДатаСеанса(), НаДату);
		Отбор.Организация = Организация;
		
		РеквизитыСторон = ДенежныеСредстваСервер.СведенияОПолучателеПлательщикеПоСтруктуре(Истина, Отбор);
		
		Результат.ИННПлательщика = РеквизитыСторон.ИННПлательщика;
		Результат.КПППлательщика = РеквизитыСторон.КПППлательщика;
		Результат.ТекстПлательщика = РеквизитыСторон.НаименованиеСокращенноеПлательщика;
		
		Если ЗначениеЗаполнено(БанковскийСчет) Тогда
			
			Если ЭтоПеречислениеВБюджет Тогда
				ТекстКорреспондента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(БанковскийСчет, "ТекстКорреспондентаБюджет");
			Иначе
...
// Устанавливает текст надписи с реквизитами стороны (плательщика или получателя) на форме в зависимости от выбранной хозяйственной операции.
//
// Параметры:
//   ЭтоПлательщик - Булево - признак того, что сторона является плательщиком.
//   Форма - ФормаКлиентскогоПриложения - форма документа.
//   РеквизитыСтороны - см. ОписаниеРеквизитовСтороныДляПроверки
//
Процедура УстановитьНадписьРеквизитыПлательщикаПолучателя(ЭтоПлательщик, Форма, РеквизитыСтороны) Экспорт

	Объект = Форма.Объект;
	
	Если ЗначениеЗаполнено(РеквизитыСтороны.Объект) Тогда
		ТекстНадписи = НадписьРеквизитыПлательщикаПолучателя(
			РеквизитыСтороны.ИНН, РеквизитыСтороны.КПП, РеквизитыСтороны.Наименование, РеквизитыСтороны.ЮрФизЛицо, ЭтоПлательщик, Объект.ПеречислениеВБюджет);
	Иначе
		ТекстНадписи = "";
	КонецЕсли;
	
	Если СтрДлина(ТекстНадписи) > 70 Тогда
		ТекстНадписи = Лев(ТекстНадписи, 70) + "...";
	КонецЕсли;
	
	Если ЭтоПлательщик Тогда
		Форма.НадписьРеквизитыПлательщика = ТекстНадписи;
	Иначе
		Форма.НадписьРеквизитыПолучателя = ТекстНадписи;
	КонецЕсли;

КонецПроцедуры

// Проверяет реквизиты плательщика или получателя и обновляет надпись на форме в зависимости от наличия ошибок заполнения
//
// Параметры:
//   ЭтоПлательщик - Булево - Истина для проверки плательщика, Ложь для получателя
//   Форма - ФормаКлиентскогоПриложения - Форма, содержащая проверяемые данные
//   РеквизитыСтороны - см. ОписаниеРеквизитовСтороныДляПроверки
//
Процедура ПроверитьРеквизитыПлательщикаПолучателя(ЭтоПлательщик, Форма, РеквизитыСтороны) Экспорт

	Объект = Форма.Объект;
	Ошибка = Ложь;
	
	Если Объект.ПеречислениеВБюджет
		И ЭтоПлательщик Тогда
		ОшибкиЗаполнения = ПлатежиВБюджет.ОшибкиЗаполненияРеквизитовПлательщика(Объект, РеквизитыСтороны, Ложь, Ошибка);
	ИначеЕсли Объект.ПеречислениеВБюджет
		И Не ЭтоПлательщик Тогда
		ОшибкиЗаполнения = ПлатежиВБюджет.ОшибкиЗаполненияРеквизитовПолучателя(Объект, РеквизитыСтороны, Ложь, Ошибка);
	Иначе
		ОшибкиЗаполнения = ОшибкиЗаполненияРеквизитовПлательщикаПолучателя(ЭтоПлательщик, Объект, РеквизитыСтороны, Ложь, Ошибка);
	КонецЕсли;
	
	Если ЭтоПлательщик Тогда
		ЭлементФормы = Форма.Элементы.НадписьРеквизитыПлательщика;
	Иначе
		ЭлементФормы = Форма.Элементы.НадписьРеквизитыПолучателя;
	КонецЕсли;
	
	Если ОшибкиЗаполнения.Количество() <> 0 Тогда
		ЭлементФормы.ЦветТекста = ЦветаСтиля.РезультатПроблемаЦвет;
		ЭлементФормы.Подсказка = СтрСоединить(ОшибкиЗаполнения, Символы.ПС);
	Иначе
		ЭлементФормы.ЦветТекста = ЦветаСтиля.ГиперссылкаЦвет;
		ЭлементФормы.Подсказка = "";
	КонецЕсли;

КонецПроцедуры

// Проверяет корректность заполнения реквизитов плательщика (ИНН, КПП, наименование плательщика)
// и формирует массив описаний найденных ошибок.
//
// Параметры:
//   ЭтоПлательщик - Булево - Истина для проверки плательщика, Ложь для получателя
//   ДокументОбъект - ДокументОбъект.ЗаявкаНаРасходованиеДенежныхСредств, ДокументОбъект.СписаниеБезналичныхДенежныхСредств, ДанныеФормыСтруктура - 
//                    документ, содержащий проверяемые реквизиты плательщика
//   РеквизитыСтороны - см. ОписаниеРеквизитовСтороныДляПроверки
//   ВыводитьСообщение - Булево - если Истина, выводит сообщения пользователю об ошибках
//   Отказ - Булево - признак отмены проведения документа (используется при выводе сообщений при проведении)
//
// Возвращаемое значение:
//   Массив из Строка - массив текстовых описаний найденных ошибок заполнения
Функция ОшибкиЗаполненияРеквизитовПлательщикаПолучателя(ЭтоПлательщик, ДокументОбъект, РеквизитыСтороны, ВыводитьСообщение, Отказ = Ложь) Экспорт

	ОшибкиЗаполнения = Новый Массив;
	
	Если Не ЗначениеЗаполнено(РеквизитыСтороны.Объект) Тогда
		Возврат ОшибкиЗаполнения;
	КонецЕсли;
	
	ИНН = СокрЛП(РеквизитыСтороны.ИНН);
	КПП = СокрЛП(РеквизитыСтороны.КПП);
	
	Если ПустаяСтрока(РеквизитыСтороны.Наименование) Тогда
		
		Если ЭтоПлательщик Тогда
			ОшибкиЗаполнения.Добавить(НСтр("ru = 'Наименование плательщика не заполнено'"));
		Иначе
			ОшибкиЗаполнения.Добавить(НСтр("ru = 'Наименование получателя не заполнено'"));
		КонецЕсли;
		
	КонецЕсли;
	
	Если ЭтоПлательщик
		Или (Не ЭтоПлательщик И Не ДокументОбъект.ПеречислениеВБюджет) Тогда
		ЭтоЮрЛицо = СтрДлина(ИНН) = 10;
		ЭтоФизЛицо = СтрДлина(ИНН) = 12;
		ЭтоИнЮрЛицо = СтрДлина(ИНН) = 5;
	Иначе
		ЭтоЮрЛицо = (РеквизитыСтороны.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо);
		ЭтоФизЛицо = (РеквизитыСтороны.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо
			Или РеквизитыСтороны.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ИндивидуальныйПредприниматель);
		ЭтоИнЮрЛицо = (РеквизитыСтороны.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент);
	КонецЕсли;
	
	НулевойИНН = (ИНН = "0");
	
	ПолеИНН = ?(ЭтоПлательщик, НСтр("ru = 'ИНН плательщика'"), НСтр("ru = 'ИНН получателя'"));
	ПолеКПП = ?(ЭтоПлательщик, НСтр("ru = 'КПП плательщика'"), НСтр("ru = 'КПП получателя'"));
	
	Если ЭтоИнЮрЛицо Тогда
		
		Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ИНН) Тогда
			
			Шаблон = НСтр("ru = '%1 должен состоять только из цифр'");
			ОшибкиЗаполнения.Добавить(СтрШаблон(Шаблон, ПолеИНН));
			
		КонецЕсли;
		
	ИначеЕсли Не НулевойИНН И ИНН <> "" И Не ЭтоИнЮрЛицо И Не ДокументОбъект.ПеречислениеВБюджет Тогда
		
		ТекстОшибки = "";
		
		Если Не РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(ИНН,
			?(Не ЭтоЮрЛицо И Не ЭтоФизЛицо И Не ЭтоИнЮрЛицо, РеквизитыСтороны.ЮрФизЛицо <> Перечисления.ЮрФизЛицо.ИндивидуальныйПредприниматель И РеквизитыСтороны.ЮрФизЛицо <> Перечисления.ЮрФизЛицо.ФизЛицо, ЭтоЮрЛицо),
			ТекстОшибки) Тогда
			ОшибкиЗаполнения.Добавить(СтрЗаменить(ТекстОшибки, НСтр("ru = 'ИНН'"), ПолеИНН));
		КонецЕсли;
	...
	Если Лев(ИНН, 2) = "00" Тогда
		...
		Шаблон = НСтр("ru = 'Первый и второй знаки %1 не могут одновременно принимать значение ""0""'");
		ОшибкиЗаполнения.Добавить(СтрШаблон(Шаблон, ПолеИНН));
	...
	Если Лев(КПП, 2) = "00" Тогда
		...
		Шаблон = НСтр("ru = 'Первый и второй знаки %1 не могут одновременно принимать значение ""0""'");
		ОшибкиЗаполнения.Добавить(СтрШаблон(Шаблон, ПолеКПП));
		
	КонецЕсли;
	
	Если ЭтоЮрЛицо И КПП <> "" И СтрДлина(КПП) <> 9
		И Не ДокументОбъект.ПеречислениеВБюджет Тогда
		
		Шаблон = НСтр("ru = '%1 должен содержать 9 символов'");
		ОшибкиЗаполнения.Добавить(СтрШаблон(Шаблон, ПолеКПП));
		
	КонецЕсли;
	
	Если ЭтоФизЛицо И КПП <> "" И КПП <> "0" Тогда
		
		Шаблон = НСтр("ru = '%1 для физлица не заполняется'");
		ОшибкиЗаполнения.Добавить(СтрШаблон(Шаблон, ПолеКПП));
	...
	Если ВыводитьСообщение Тогда
		...
		Если ЭтоПлательщик Тогда
			Поле = "НадписьРеквизитыПлательщика";
		Иначе
			Поле = "НадписьРеквизитыПолучателя";
		КонецЕсли;
...
// Заполняет реквизиты получателя (ИНН, КПП, Наименование) на форме в зависимости от выбранной хозяйственной операции.
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения - Форма документа для заполнения реквизитов
//   Дата - Дата - Дата актуальности данных получателя
//   Получатель - СправочникСсылка.Контрагенты, СправочникСсылка.Организации, СправочникСсылка.ФизическиеЛица, Неопределено - Контрагент-получатель
//   СчетПолучателя - СправочникСсылка.БанковскиеСчетаОрганизаций, СправочникСсылка.БанковскиеСчетаКонтрагентов, Неопределено - Счет получателя
//
Процедура ЗаполнитьРеквизитыПолучателя(Форма, Дата, Получатель, СчетПолучателя) Экспорт
	
	Результат = ДанныеПолучателя(Получатель, СчетПолучателя, Дата);
	Форма.ИННПолучателя = Результат.ИННПолучателя;
	Форма.КПППолучателя = Результат.КПППолучателя;
	Форма.ТекстПолучателя = Результат.ТекстПолучателя;
	
КонецПроцедуры

// Формирует структуру с данными получателя: ИНН, КПП и наименование.
//
// Параметры:
//   Получатель - СправочникСсылка.Контрагенты, СправочникСсылка.Организации, СправочникСсылка.ФизическиеЛица, Неопределено - ссылка на получателя
//   БанковскийСчет - СправочникСсылка.БанковскиеСчетаКонтрагентов, СправочникСсылка.БанковскиеСчетаОрганизаций, Неопределено - банковский счет получателя
//   НаДату - Дата - дата актуальности данных
//
// Возвращаемое значение:
//   Структура:
//     * ИННПолучателя - Строка - ИНН получателя
//     * КПППолучателя - Строка - КПП получателя
//     * ТекстПолучателя - Строка - наименование получателя
//
Функция ДанныеПолучателя(Получатель, БанковскийСчет, НаДату) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ИННПолучателя", "");
	Результат.Вставить("КПППолучателя", "");
	Результат.Вставить("ТекстПолучателя", "");
	
	Если ЗначениеЗаполнено(Получатель) Тогда
		
		Отбор = ДенежныеСредстваСервер.ПараметрыДляПоискаДанныхСторонДокумента();
		Отбор.Дата = ?(НаДату = Дата('00010101'), ТекущаяДатаСеанса(), НаДату);
		Отбор.Контрагент = Получатель;
		
		РеквизитыСторон = ДенежныеСредстваСервер.СведенияОПолучателеПлательщикеПоСтруктуре(Истина, Отбор);
		
		Результат.ИННПолучателя = РеквизитыСторон.ИННПолучателя;
		Результат.КПППолучателя = РеквизитыСторон.КПППолучателя;
		
		Если ТипЗнч(Получатель) = Тип("СправочникСсылка.Организации") Тогда
			Результат.ТекстПолучателя = РеквизитыСторон.НаименованиеСокращенноеПолучателя;
		Иначе
			Результат.ТекстПолучателя = РеквизитыСторон.НаименованиеПолноеПолучателя;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(БанковскийСчет) Тогда
			...
			Запрос.Текст = 
				"ВЫБРАТЬ РАЗРЕШЕННЫЕ
				|	БанковскиеСчетаКонтрагентов.ИННКорреспондента КАК ИНН,
				|	БанковскиеСчетаКонтрагентов.КППКорреспондента КАК КПП,
				|	БанковскиеСчетаКонтрагентов.ТекстКорреспондента КАК Наименование
				|ИЗ
				|	Справочник.БанковскиеСчетаКонтрагентов КАК БанковскиеСчетаКонтрагентов
				|ГДЕ
				|	БанковскиеСчетаКонтрагентов.Ссылка = &Ссылка
				|
				|ОБЪЕДИНИТЬ ВСЕ
				|
				|ВЫБРАТЬ
				|	"""",
				|	"""",
				|	БанковскиеСчетаОрганизаций.ТекстКорреспондента
				|ИЗ
			...
			Пока РезультатЗапроса.Следующий() Цикл
				...
				Если ЗначениеЗаполнено(РезультатЗапроса.ИНН) Тогда
					Результат.ИННПолучателя = РезультатЗапроса.ИНН;
				КонецЕсли;
				
				Если ЗначениеЗаполнено(РезультатЗапроса.КПП) Тогда
					Результат.КПППолучателя = РезультатЗапроса.КПП;
				КонецЕсли;
				
				Если ЗначениеЗаполнено(РезультатЗапроса.Наименование) Тогда
					Результат.ТекстПолучателя = РезультатЗапроса.Наименование;
				КонецЕсли;
...
// Формирует строковое представление реквизитов плательщика (ИНН, КПП и Наименование) с учетом локализации.
//
// Параметры:
//   ИНН - Строка - идентификационный номер налогоплательщика
//   КПП - Строка - код причины постановки на учет
//   ТекстКорреспондента - Строка - наименование или описание корреспондента
//   ЮрФизЛицо - ПеречислениеСсылка.ЮрФизЛицо - тип контрагента (юрлицо, физлицо, ИП, юрлицо (нерезидент))
//   ЭтоПлательщик - Булево - признак, что это плательщик, а не получатель
//   ЭтоПеречислениеВБюджет - Булево - признак перечисления средств в бюджет
//
// Возвращаемое значение:
//   Строка - отформатированная строка с реквизитами плательщика
//
Функция НадписьРеквизитыПлательщикаПолучателя(ИНН, КПП, ТекстКорреспондента, ЮрФизЛицо, ЭтоПлательщик = Ложь, ЭтоПеречислениеВБюджет = Ложь) Экспорт
	...
	Если Не ЗначениеЗаполнено(КПП) Тогда
		...
		Если ЭтоПлательщик Тогда
			...
			Если Не ЭтоПеречислениеВБюджет И СтрДлина(СокрЛП(ИНН)) = 12 Тогда
				СтрокаКПП = "";
			Иначе
...
// Возвращает структуру с набором реквизитов стороны для проверки.
//
// Возвращаемое значение:
//   Структура:
//     * ИНН - Строка - ИНН стороны
//     * КПП - Строка - КПП стороны
//     * Наименование - Строка - Наименование стороны
//     * Объект - Произвольный - Объект стороны
//     * ЮрФизЛицо - ПеречислениеСсылка.ЮрФизЛицо - Признак юр./физ. лица
//
Функция ОписаниеРеквизитовСтороныДляПроверки() Экспорт

	РеквизитыСтороны = Новый Структура;
	РеквизитыСтороны.Вставить("ИНН", "");
	РеквизитыСтороны.Вставить("КПП", "");
	РеквизитыСтороны.Вставить("Наименование", "");
	РеквизитыСтороны.Вставить("Объект", Неопределено);
	РеквизитыСтороны.Вставить("ЮрФизЛицо", Перечисления.ЮрФизЛицо.ПустаяСсылка());
	
	Возврат РеквизитыСтороны;
...
Процедура ИзменитьНомерПлатежногоПоручения(Объект) Экспорт
	...
	Если ПоследниеТриСимвола = "000"
		...
		ОсновнойПрефикс = ?(ПозицияДефис > 0, Лев(Объект.Номер, ПозицияДефис), "");
		...
		ПараметрыНомера.Вставить("Номер", НомерДокумента);
		ПараметрыНомера.Вставить("Префикс", Префикс);
		ПараметрыНомера.Вставить("Длина", ДлинаНомера);
		
		УвеличитьНомерПлатежногоДокумента(Объект, ПараметрыНомера);
...
Процедура ЗаполнитьНастройкиПолейФормы(Настройки) Экспорт
	...
	ГруппаНеХозОперации			   = Финансы.НовыйОтбор(Элемент.Условие,,, Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ГруппаНеХозОперации.ТипГруппы  = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаНе;
	ГруппаИлиХозОперации		   = Финансы.НовыйОтбор(ГруппаНеХозОперации, , , Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ГруппаИлиХозОперации.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
	Финансы.НовыйОтбор(ГруппаИлиХозОперации, "ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.ВыдачаЗаймов);
	Финансы.НовыйОтбор(ГруппаИлиХозОперации, "ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.ВыдачаЗаймаСотруднику);
...
Процедура ИнициализироватьБанковскиеСчетаСпискаКонтрагентов(Объект, ДанныеЗаполнения = Неопределено) Экспорт
	...
	Если (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаПоставщику
		...
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеЗаполнения, "ОперацияССамозанятым")
...
// Выполняет очистку реквизитов оплаты с самозанятым.
// 
// Параметры:
//  Объект - ДанныеФормыСтруктура - документ объект.
//
Процедура ОчиститьРеквизитыОплатыСамозанятому(Объект) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьВыплатыСамозанятым")
		...
		Объект.ОперацияССамозанятым = Ложь;
		Объект.СписокКонтрагентов = Ложь;
...
Процедура ПроверитьЗаполнениеДокументаСФлагомСамозанятого(Объект, Ошибка, ФлагОбменСБанками = Ложь, ОшибкиЗаполнения = "") Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			Иначе
				ТекстыСообщений.Добавить(НСтр("ru = 'В строке №%1 необходимо скорректировать объект, созданный при включенной настройке ""Выплаты самозанятым"" или создать новый:'"));
			КонецЕсли;
...
// Возвращает признак доступности платежной ссылки в документе по форме оплаты.
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка - документ, по которому определяется доступность платежной ссылки.
//  ФормаОплаты - ПеречислениеСсылка.ФормыОплаты - форма оплаты.
//  Входящая - Булево - признак направления платежной ссылки.
// 
// Возвращаемое значение:
//  Булево
Функция ДоступнаГиперссылкаСБП(ДокументСсылка, ФормаОплаты, Входящая) Экспорт

	Если Не Входящая Тогда
		КомандаОтображается = ВключенаГенерацияПлатежнойСсылкиСБППоДокументу(ДокументСсылка);
	Иначе
		КомандаОтображается = Истина;
	КонецЕсли;
	
	Возврат КомандаОтображается
		И (ФормаОплаты = Перечисления.ФормыОплаты.ПустаяСсылка()
			Или ФормаОплаты = Перечисления.ФормыОплаты.СистемаБыстрыхПлатежей)
		И ДокументСсылка <> Неопределено
		И ?(Входящая, СистемаБыстрыхПлатежейУТ.ДоступноДобавлениеВходящейСсылкиПоДокументу(ДокументСсылка), СистемаБыстрыхПлатежейУТ.ПереводыСБППоДокументуДоступны(ДокументСсылка));

КонецФункции

// Возвращает признак использования команды генерации платежной ссылки СБП по документу.
// 
// Параметры:
//  ДокументСсылка - ОпределяемыйТип.ДокументОперацииСБП - Документ, включенный в список документов, по которым доступно
//   генерации платежной ссылки по умолчанию.
// 
// Возвращаемое значение:
//  Булево - Включена генерация платежной ссылки СБППо документу.
Функция ВключенаГенерацияПлатежнойСсылкиСБППоДокументу(ДокументСсылка) Экспорт

	ХранилищеНастроекСБП = Константы.НастройкиОтображенияКомандПлатежныхСистем.Получить();
	НастройкиВидимости = ХранилищеНастроекСБП.Получить(); // ФиксированноеСоответствие
	
	Результат = Истина;
	
	Если НастройкиВидимости <> Неопределено Тогда
		
		НастройкиВидимостиСБП = НастройкиВидимости.Получить("СистемаБыстрыхПлатежей");
		
		Если НастройкиВидимостиСБП <> Неопределено Тогда
			Видимость = НастройкиВидимостиСБП.Получить(ДокументСсылка.Метаданные().ПолноеИмя())
		Иначе
			Видимость = НастройкиВидимости.Получить(ДокументСсылка.Метаданные().ПолноеИмя());
		КонецЕсли;
		
		Результат = ?(Видимость = Неопределено, Ложь, Видимость);
...
//  * ПрименениеПриказа107н - Булево - Признак применения приказа 107н.
//  * ПрименениеПриказа126н - Булево - Признак применения приказа 126н.
//  * ЕдиныйНалоговыйПлатеж - Булево - Признак единого налогового платежа.
//  * ПрименениеПриказа58н - Булево - Признак применения приказа 58н.
//  Отказ - Булево - Признак отказа от продолжения работы
//  ФлагОбменСБанками - Булево - используется обмен с банком
//  ОшибкиЗаполнения - Строка - Накопленное описание ошибок.
//
Процедура ПроверитьЗаполнениеРеквизитовНалоговыхПлатежей(Объект, ПараметрыПроверкиЗаполнения, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения)
	...
	ПроверитьЗаполнениеКБК(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверкиЗаполнения);
	ПроверитьЗаполнениеОКТМО(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверкиЗаполнения);
	...
	ПроверитьЗаполнениеПоказателяНомера(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверкиЗаполнения);
	ПроверитьЗаполнениеПоказателяДаты(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверкиЗаполнения);
	...
	Если ПлатежиВБюджет.ИспользуетсяЕдиныйНалоговыйПлатеж(Объект.Организация, Объект.Дата)
		И Объект.ТипНалога = Справочники.ВидыНалоговВзносов.ЕдиныйНалоговыйПлатеж Тогда
		КодБК = ВидыНалоговВзносовЛокализация.КБКПоВидуНалоговогоОбязательства(Объект.ТипНалога, , Объект.Дата);
		Если Не ПустаяСтрока(Объект.КодБК) И СокрЛП(Объект.КодБК) <> КодБК Тогда
		...
		Если ПараметрыПроверкиЗаполнения.ПрименениеПриказа58н
			И Объект.СтатусСоставителя <> "01" Тогда
			
			ТекстОшибки = НСтр("ru = 'В поле ""Статус плательщика"" следует указать значение ""01""'");
			Если ФлагОбменСБанками Тогда
				ДобавитьОшибкуЗаполнения(ОшибкиЗаполнения, ТекстОшибки);
			Иначе
				ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, , "НадписьРеквизитыПлатежаВБюджет", , Отказ);
		...
		ИначеЕсли ДействуетПриказ199н_202302
			И Объект.СтатусСоставителя <> "01"
			И Объект.СтатусСоставителя <> "27" Тогда
			
			ТекстОшибки =
				НСтр("ru = 'В поле ""Статус плательщика"" следует указать значение ""01"" (для кредитных организаций ""27"")'");
			Если ФлагОбменСБанками Тогда
				ДобавитьОшибкуЗаполнения(ОшибкиЗаполнения, ТекстОшибки);
			Иначе
				ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, , "НадписьРеквизитыПлатежаВБюджет", , Отказ);
	...
	ИначеЕсли Объект.ТипНалога = Справочники.ВидыНалоговВзносов.ОбеспечительныйПлатежСПОТ Тогда
		КодБК = ВидыНалоговВзносовЛокализация.КБКПоВидуНалоговогоОбязательства(Объект.ТипНалога, , Объект.Дата);
		Если Не ПустаяСтрока(Объект.КодБК) И СокрЛП(Объект.КодБК) <> КодБК Тогда
			ТекстОшибки = НСтр("ru = 'КБК не соответствует реквизитам обеспечительного платежа'");
			Если ФлагОбменСБанками Тогда
				ДобавитьОшибкуЗаполнения(ОшибкиЗаполнения, ТекстОшибки);
			Иначе
	...
	ИначеЕсли ПлатежиВБюджетКлиентСервер.ПлатежАдминистрируетсяНалоговымиОрганами(Объект.КодБК)
		И Не ПараметрыПроверкиЗаполнения.ПрименениеПриказа58н
		И ДействуетПриказ199н_202302
		И Объект.СтатусСоставителя <> "01"
		...
		ТекстОшибки =
			НСтр("ru = 'В поле ""Статус плательщика"" следует указать значение ""01"", ""13"" (для кредитных организаций ""27"") или ""02"" (для уведомления об исчисленных суммах налога)'");
		Если ФлагОбменСБанками Тогда
			ДобавитьОшибкуЗаполнения(ОшибкиЗаполнения, ТекстОшибки);
		Иначе
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, , "НадписьРеквизитыПлатежаВБюджет", , Отказ);
	...
	Если СтрНайти("АП,АР", ПоказательОснования) > 0
		И Не ПараметрыПроверкиЗаполнения.ПрименениеПриказа58н Тогда
	...
	ИначеЕсли СтрНайти("ТП,ЗД", ПоказательОснования) > 0
		И Не ПараметрыПроверкиЗаполнения.ПрименениеПриказа58н Тогда
...
Процедура ПроверитьЗаполнениеРеквизитовТаможенныхПлатежей(Объект, ПараметрыПроверкиЗаполнения, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения)
	...
	ПроверитьЗаполнениеОКТМО(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверкиЗаполнения);
	ПроверитьЗаполнениеУИН(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения);
	ПроверитьЗаполнениеКодТаможенногоОргана(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверкиЗаполнения);
	ПроверитьЗаполнениеПоказателяНомера(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверкиЗаполнения);
	ПроверитьЗаполнениеПоказателяДаты(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверкиЗаполнения);
	
	Если ПараметрыПроверкиЗаполнения.ПрименениеПриказа58н
		И Не ЗначениеЗаполнено(Объект.ПоказательОснования) Тогда
		ТекстОшибки = НСтр("ru = 'Поле ""Основание"" не заполнено'");
		Если ФлагОбменСБанками Тогда
			ДобавитьОшибкуЗаполнения(ОшибкиЗаполнения, ТекстОшибки);
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,, "НадписьРеквизитыПлатежаВБюджет",, Отказ);
		КонецЕсли;
	ИначеЕсли ПлатежиВБюджетКлиентСервер.ОснованияПлатежа(
...
//  ОшибкиЗаполнения - Строка - Накопленное описание ошибок.
//
Процедура ПроверитьЗаполнениеРеквизитовИныхПлатежейВБюджет(Объект, ПараметрыПроверкиЗаполнения, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения)
	...
	Если Не ПараметрыПроверкиЗаполнения.ПрименениеПриказа58н
		И Объект.СтатусСоставителя = "08" Тогда
		Для каждого ЭлементСтруктуры Из СтруктураПолей Цикл
			Если Объект[ЭлементСтруктуры.Ключ] <> "0" Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При статусе плательщика ""08"" следует указать ""0"" в поле ""%1""'"),
...
Процедура ПроверитьЗаполнениеКБК(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверки)
	
	КБК = Объект.КодБК;
	НомерСчетаПолучателя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.БанковскийСчетКонтрагента, "НомерСчета");
	
	Если Объект.ВидПеречисленияВБюджет = Перечисления.ВидыПеречисленийВБюджет.ИнойПлатеж
		И ПустаяСтрока(КБК)
		И Лев(НомерСчетаПолучателя, 5) <> "03100"
		И ПараметрыПроверки.ПрименениеПриказа58н Тогда
		
		ТекстОшибки = НСтр("ru = 'КБК должен состоять из 20 знаков или ""0""'");
	...
	ИначеЕсли Объект.ВидПеречисленияВБюджет = Перечисления.ВидыПеречисленийВБюджет.ИнойПлатеж 
		И СокрЛП(КБК) = "0"
		И Лев(НомерСчетаПолучателя, 5) <> "03100"
		И ПараметрыПроверки.ПрименениеПриказа58н Тогда
		Возврат;
	ИначеЕсли Объект.ВидПеречисленияВБюджет = Перечисления.ВидыПеречисленийВБюджет.ИнойПлатеж 
		И СокрЛП(КБК) = "0"
		И Не ПараметрыПроверки.ПрименениеПриказа58н Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеОКТМО(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверки)
	
	КодОКАТО = Объект.КодОКАТО;
	ТекстОшибки = "";
	
	Если ПустаяСтрока(КодОКАТО)
		И ПараметрыПроверки.ПрименениеПриказа58н Тогда
		
		ТекстОшибки =  НСтр("ru = 'ОКТМО должен состоять из 8 знаков или ""0""'");
...
Процедура ПроверитьЗаполнениеКодТаможенногоОргана(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверки)

	КодТаможенногоОргана = Объект.ПоказательПериода;
	
	ТекстОшибки = "";
	
	Если ПараметрыПроверки.ПрименениеПриказа58н 
		И ПустаяСтрока(КодТаможенногоОргана) Тогда
		
		ТекстОшибки = НСтр("ru = 'Код таможенного органа должен состоять из 8 знаков или ""0""'");
		
	ИначеЕсли ПустаяСтрока(КодТаможенногоОргана) Тогда
		
		ТекстОшибки = НСтр("ru = 'Поле ""Код таможенного органа"" не заполнено'");
		
	ИначеЕсли ПустаяСтрока(СокрЛП(СтрЗаменить(КодТаможенногоОргана, "0", "")))
		И СокрЛП(КодТаможенногоОргана) <> "0" Тогда
		
		ТекстОшибки = НСтр("ru = 'Все знаки Кода таможенного органа не могут одновременно принимать значение ""0""'");
		
	ИначеЕсли СтрДлина(СокрП(КодТаможенногоОргана)) <> 8
		И СокрП(КодТаможенногоОргана) <> "0" Тогда
		
		ТекстОшибки = НСтр("ru = 'Код таможенного органа должен состоять из 8 знаков'");
...
Процедура ПроверитьЗаполнениеПоказателяНомера(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверки)

	ВидПлатежа = Объект.ВидПеречисленияВБюджет;
	ПоказательНомера = Объект.ПоказательНомера;
	ПоказательОснования = Объект.ПоказательОснования;
	ТекстОшибки = "";
	
	Если ВидПлатежа = Перечисления.ВидыПеречисленийВБюджет.ИнойПлатеж
		И Не ПараметрыПроверки.ПрименениеПриказа58н Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ПоказательНомера) Тогда
		ТекстОшибки = НСтр("ru = 'Поле ""Номер документа"" не заполнено'");
	ИначеЕсли ПараметрыПроверки.ПрименениеПриказа58н
		И ПоказательНомера = "0"
		И (Объект.СтатусСоставителя = "05" Или Объект.СтатусСоставителя = "19" Или Объект.СтатусСоставителя = "28" Или Объект.СтатусСоставителя = "32")
		И СокрП(Объект.ИННПлательщика) = "0"
		И (СокрП(Объект.ИдентификаторПлатежа) = "0" Или Объект.ИдентификаторПлатежа = "") Тогда
		ТекстОшибки = НСтр("ru = 'Поле ""Номер документа"" должно быть заполнено конкретным значением и не ""0""'");
	ИначеЕсли ПараметрыПроверки.ПрименениеПриказа58н
		И ПоказательОснования <> "0" И ПоказательОснования <> "00"
		И ПоказательНомера = "0" Тогда
		ТекстОшибки = НСтр("ru = 'Поле ""Номер документа"" должно быть заполнено конкретным значением и не ""0""'");
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеПоказателяДаты(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверки)
	...
	Если Не ЗначениеЗаполнено(ПоказательДаты) Тогда
		ТекстОшибки = НСтр("ru = 'Поле ""Дата документа"" не заполнено'");
	ИначеЕсли ПараметрыПроверки.ПрименениеПриказа58н
		И ПоказательОснования = "00"
		И ПоказательДаты <> "00" Тогда
		ТекстОшибки = НСтр("ru = 'Поле ""Дата документа"" должно быть заполнено значением ""00""'");
	КонецЕсли;
...
Процедура УвеличитьНомерПлатежногоДокумента(Объект, ПараметрыНомера)

	НовыйНомер = ПараметрыНомера.Номер + 1;
	НомерДокумента = Формат(НовыйНомер, "ЧГ=0");
	
	Пока СтрДлина(НомерДокумента) < ПараметрыНомера.Длина Цикл
		НомерДокумента = "0" + НомерДокумента;
	КонецЦикла;
	
	НомерДокумента = ПараметрыНомера.Префикс + НомерДокумента;
	
	МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоСсылке(Объект.Ссылка);
	
	Если МенеджерОбъекта.ЭтоУникальныйНомерДокумента(Объект.Дата, НомерДокумента) Тогда
		Объект.Номер = НомерДокумента;
	Иначе
		
		ПараметрыНомера.Номер = НовыйНомер;
		УвеличитьНомерПлатежногоДокумента(Объект, ПараметрыНомера);
...
#Область РеквизитыПлательщика

Процедура ПреобразоватьПустойРеквизитПлательщика(ЭтоПеречислениеВБюджет, ЗначениеРеквизита)

	Если ЗначениеРеквизита = "" И ЭтоПеречислениеВБюджет Тогда
		ЗначениеРеквизита = "0";
	ИначеЕсли ЗначениеРеквизита = "0" И Не ЭтоПеречислениеВБюджет Тогда
		ЗначениеРеквизита = "";
	КонецЕсли;
