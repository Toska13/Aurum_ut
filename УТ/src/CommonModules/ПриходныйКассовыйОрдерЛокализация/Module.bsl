...
Функция СформироватьПечатнуюФормуКО1(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	ИндексПервойВыборки = ?(МассивТекстовЗапроса.Количество() = 1, 0, 1);
	Выборка           = МассивРезультатов[ИндексПервойВыборки].Выбрать();
	КорСчета          = МассивРезультатов[ИндексПервойВыборки+1].Выгрузить();
