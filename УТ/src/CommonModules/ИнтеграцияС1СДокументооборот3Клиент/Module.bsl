...
Процедура ВыбратьЗначениеПоТипуОбъекта(ОписаниеОповещения, ТипОбъектаВыбора, Отбор = Неопределено,
	...
	ПараметрыФормы.Вставить("Заголовок",
		ИнтеграцияС1СДокументооборот3КлиентСервер.ПредставлениеТипаОбъектаXDTO(ТипОбъектаВыбора,, Истина));
	...
	ИначеЕсли ТипОбъектаВыбора = "DMFile" Тогда
		ОткрытьФорму("Обработка.ИнтеграцияС1СДокументооборотБазоваяФункциональность.Форма.ВыборФайла",
			ПараметрыФормы,
			ФормаВладелец,,,,
			ОписаниеОповещения,
			РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Функция ВызванаИнтерактивнаяОбработка(РезультатДействия, ТипОперации, ПовторноеДействие) Экспорт
	...
	Контекст = РезультатДействия.ТребуемоеИнтерактивноеДействие.Контекст;
	...
	ПараметрыОповещения = ПараметрыОповещенияПослеВыполненияИнтерактивногоДействияНадЗадачей();
	ПараметрыОповещения.ТипОперации = ТипОперации;
	ПараметрыОповещения.ПовторноеДействие = ПовторноеДействие;
	ПараметрыОповещения.ПервоначальныйКонтекст = Контекст;
	...
	ИначеЕсли РезультатДействия.ТребуемоеИнтерактивноеДействие.ЗадатьПроизвольныйВопрос Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения(
			...
			ЭтотОбъект,
			ПараметрыОповещения);
		ПоказатьВопросПередВыполнениемЗадачи(Контекст, ОписаниеОповещения);
		ВызванаИнтерактивнаяОбработка = Истина;
	...
	ИначеЕсли РезультатДействия.ТребуемоеИнтерактивноеДействие.ИзменитьПараметрыПроцесса Тогда
		...
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ОткрытьОбъект(
			Контекст.ДействиеТип,
			Контекст.ДействиеID,,
			ПараметрыФормы,
			ОписаниеОповещения);
		ВызванаИнтерактивнаяОбработка = Истина;
	
	ИначеЕсли РезультатДействия.ТребуемоеИнтерактивноеДействие.ВыполнитьПодписание Тогда
		
		МодульЭлектроннаяПодписьКлиент =
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.МодульЭлектроннаяПодписьКлиент(Истина);
		Если МодульЭлектроннаяПодписьКлиент <> Неопределено Тогда
			ОписаниеОповещения = Новый ОписаниеОповещения(
				"ОбработатьДанныеВыполненияПослеПодписания",
				ЭтотОбъект,
				ПараметрыОповещения);
			НаборДанных = Новый Массив;
			Для Каждого ОбъектДляПодписи Из Контекст.ОбъектыПодписания Цикл
				ДанныеДляПодписи = Новый Структура;
				ДанныеДляПодписи.Вставить("Данные", ОбъектДляПодписи.Данные);
				ДанныеДляПодписи.Вставить("Представление", Новый Структура("Значение, Представление",
					ОбъектДляПодписи.ОбъектДО, ОбъектДляПодписи.ОбъектДО.Представление));
				НаборДанных.Добавить(ДанныеДляПодписи);
			КонецЦикла;
			ОписаниеДанных = Новый Структура;
			ОписаниеДанных.Вставить("Операция", Контекст.Операция);
			ОписаниеДанных.Вставить("ЗаголовокДанных", Контекст.ЗаголовокДанных);
			ОписаниеДанных.Вставить("НаборДанных", НаборДанных);
			ОписаниеДанных.Вставить("ПредставлениеНабора", НСтр("ru = 'Объекты (%1)'"));
			МодульЭлектроннаяПодписьКлиент.Подписать(ОписаниеДанных, Неопределено, ОписаниеОповещения);
		КонецЕсли;
		ВызванаИнтерактивнаяОбработка = Истина;
...
// Обработчик события, возникающего при нажатии (клике мышью или с помощью клавиатуры) на любом элементе
// инструкции к выражению на встроенном языке.
//
// Параметры:
//   Элемент - ПолеФормы - элемент формы вида "Поле HTML документа".
//   ДанныеСобытия - ФиксированнаяСтруктура - параметр заполняется фиксированной структурой со свойствами:
//     * Anchor - ВнешнийОбъект
//     * Element - ВнешнийОбъект
//     * Button - ВнешнийОбъект
//     * Event - ВнешнийОбъект
//     * Document - ВнешнийОбъект
//     * HRef - ВнешнийОбъект
//   СтандартнаяОбработка - Булево - признак выполнения стандартной обработки события.
//
Процедура ИнструкцияПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка) Экспорт
...
Процедура НачатьСозданиеСвязанногоОбъектаДО(ОбъектИС, ОписаниеОповещения = Неопределено,
	...
	Если ОбъектИС.Пустая() Тогда
		...
		ПоказатьПредупреждение(,ТекстОшибки,, НСтр("ru = 'Не удалось создать объект 1С:Документооборота'"));
		Возврат
	КонецЕсли;
	...
	Иначе
		...
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Для объекта %1 нет подходящих правил интеграции.'"), ОбъектИС);
		ПоказатьПредупреждение(,ТекстОшибки,, НСтр("ru = 'Не удалось создать объект 1С:Документооборота'"));
...
Процедура НачатьСозданиеСвязанногоОбъектаДОПослеВыбораПравила(Знач Результат, Параметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("СправочникСсылка.ПравилаИнтеграцииС1СДокументооборотом3") Тогда
		Параметры.Вставить("Правило", Результат);
		
	ИначеЕсли ТипЗнч(Результат) = Тип("Структура") Тогда
		Параметры.Вставить("Правило", Результат.Ссылка);
		
	КонецЕсли;
	
	ПредварительныеДанные = ИнтеграцияС1СДокументооборот3ВызовСервера.ПредварительныеДанныеДляВыгрузкиВДО(
		Параметры.ОбъектИС,
		Параметры.Правило);
	Параметры.Вставить("ПредварительныеДанныеВХранилище", ПредварительныеДанные.ПредварительныеДанныеВХранилище);
	ОбязательныеСвязи = ПредварительныеДанные.ОбязательныеСвязи;
	
	Параметры.Вставить("УникальныйИдентификаторФормы", Новый УникальныйИдентификатор);
	Если Параметры.ОписаниеОповещения <> Неопределено
		...
		Параметры.УникальныйИдентификаторФормы = Параметры.ОписаниеОповещения.Модуль.УникальныйИдентификатор;
	КонецЕсли;
	
	Для Каждого НовыйОбъектДО Из ПредварительныеДанные.НовыеОбъектыДО Цикл
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.Оповестить_ДобавлениеСвязи(
			НовыйОбъектДО.ID,
			НовыйОбъектДО.Тип,
			НовыйОбъектДО.СсылкаНаОбъект,
			Параметры.ЗадаватьВопросОПереносеФайлов,
			Параметры.УникальныйИдентификаторФормы);
	КонецЦикла;
	
	Если ОбязательныеСвязи.Количество() = 0 Тогда
		
		НачатьСозданиеСвязанногоОбъектаДОПослеВыбораПравилаИОбязательнойСвязи(Новый Массив, Параметры);
		
	Иначе
		
		ПараметрыФормы = Новый Структура("ОбязательныеСвязи", ОбязательныеСвязи);
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"НачатьСозданиеСвязанногоОбъектаДОПослеВыбораПравилаИОбязательнойСвязи",
			ЭтотОбъект,
			Параметры);
		ОткрытьФорму("Обработка.ИнтеграцияС1СДокументооборот3.Форма.СозданиеОбязательныхСвязей",
			ПараметрыФормы,,,,,
			ОписаниеОповещения,
			РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
// Продолжает создание нового объекта Документооборота после выбора обязательных связей.
//
// Параметры:
...
//
Процедура НачатьСозданиеСвязанногоОбъектаДОПослеВыбораПравилаИОбязательнойСвязи(СвязиКСозданию, Параметры) Экспорт
	
	Если ТипЗнч(СвязиКСозданию) <> Тип("Массив") Тогда
		Возврат;
	КонецЕсли;
	...
	ОбъектДОИлиСообщение = ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.СоздатьОбъектДОПоПравилу(
		...
		Параметры.Правило,
		СвязиКСозданию,
		Параметры.ПредварительныеДанныеВХранилище);
	...
	Иначе // Структура с описанием созданного объекта результата.
		...
		Если Параметры.ОписаниеОповещения <> Неопределено Тогда
			Если ТипЗнч(Параметры.ОписаниеОповещения.ДополнительныеПараметры) = Тип("Структура") Тогда
				Параметры.ОписаниеОповещения.ДополнительныеПараметры.Вставить("СозданНовыйОбъектДО", Истина);
			КонецЕсли;
...
Процедура ОткрытьОбъектДО(Знач СсылкаНаОбъектДО = Неопределено, Знач АдресБазыДО = "") Экспорт
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ОткрытьНавигационнуюСсылку(
		СтрШаблон("%1#%2", АдресБазыДО, СсылкаНаОбъектДО));
...
//   УникальныйИдентификаторФормы - УникальныйИдентификатор - идентификатор вызвавшей формы.
//
Процедура ОткрытьОбъектНаПросмотр(ОбъектИС, ОбъектДО, РасширениеФайла, УникальныйИдентификаторФормы) Экспорт
	...
	ИначеЕсли ОбъектДО.Тип = "DMIncomingElectronicDocument" Или ОбъектДО.Тип = "DMOutgoingElectronicDocument" Тогда
		Наименование = "";
		РасширениеФайла = "";
		АдресФайла = ИнтеграцияС1СДокументооборот3ВызовСервера.ПолучитьФайлДокументаЭДОИПоместитьВХранилище(
			ОбъектДО.ID,
			ОбъектДО.Тип,
			УникальныйИдентификаторФормы,
			РасширениеФайла,
			Наименование);
		Если ЭтоФормализованныйДокументЭДО(РасширениеФайла) И ЗначениеЗаполнено(ОбъектДО.НавигационнаяСсылка) Тогда
			ОткрытьОбъектДО(ОбъектДО.НавигационнаяСсылка);
		Иначе
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ОткрытьПолученныйФайл(
				АдресФайла,
				Наименование,
				РасширениеФайла,
				УникальныйИдентификаторФормы);
	...
	ИначеЕсли ЗначениеЗаполнено(ОбъектДО.НавигационнаяСсылка)
			И (ИнтеграцияС1СДокументооборот3КлиентПовтИсп.ЭтоДокументДО3(ОбъектДО.Тип)
				...
				Или ОбъектДО.Тип = "DMMeeting") Тогда
		ОткрытьОбъектДО(ОбъектДО.НавигационнаяСсылка);
...
Процедура ПоказатьПредупреждениеИзДО(ОписаниеОповещенияОЗавершении = Неопределено, ТекстПредупреждения, Таймаут = 0,
	...
	Для НомерСтроки = 1 По ТекстовыйДокумент.КоличествоСтрок() Цикл
		...
		Если Не ПустаяСтрока(НавигационнаяСсылка) Тогда
			Если СтрНайти(ТекущаяСтрока, НСтр("ru = 'Ссылка на объект в ИС:'")) = 1 Тогда
			...
			ИначеЕсли Не УстановленаСсылкаНаОбъектДО Тогда
				...
				Если СтрНайти(ТекущаяСтрока, НСтр("ru = 'Ссылка на объект в ДО:'")) = 1 Тогда
...
//     параметра.
//
Процедура ОбработкаКомандыНачатьОбработку(Знач МассивСсылок, ПараметрыВыполнения) Экспорт
...
// Открывает форму выбора роли для выделенных файлов.
//
// Параметры:
//   ДеревоФайлов - ДанныеФормыДерево - реквизит формы, содержащий дерево файлов.
//   ЭлементыДеревоФайлов - ТаблицаФормы - элемент формы, содержащий дерево файлов.
//   ПараметрыФормы - Структура - параметры формы выбора роли:
//     * ВладелецФайлов - Строка - идентификатор объекта ДО, являющегося владельцем файлов.
//     * ДоступныеРоли - СписокЗначений из Строка - доступные роли файлов.
//     * ИдентификаторФормыВладельца - УникальныйИдентификатор - идентификатор формы, из которой выполняется выбора.
//     * ИспользоватьРолиФайлов - Булево - признак использования ролей файлов.
//
Процедура ВыбратьРольФайла(ДеревоФайлов, ЭлементыДеревоФайлов, ПараметрыФормы) Экспорт
	
	Если Не ПараметрыФормы.ИспользоватьРолиФайлов Или ПараметрыФормы.ДоступныеРоли.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Для данного вида документов не настроены роли файлов.'"));
		Возврат;
	КонецЕсли;
	
	СписокФайлов = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыделенныеФайлы(
		ДеревоФайлов,
		ЭлементыДеревоФайлов.ВыделенныеСтроки);
	
	Если СписокФайлов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыФормы.Вставить("ВыбраннаяРоль", ТекущаяРольФайла(ДеревоФайлов, ЭлементыДеревоФайлов.ТекущаяСтрока));
...
Процедура ЗадатьВопросОПереносеФайловВДО(ПараметрыПереносаФайлов, ОповещениеВыполнитьПеренос = Неопределено,
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
		Кнопки.Добавить("ПоказатьФайлы", НСтр("ru='Показать файлы'"));
	КонецЕсли;
...
Процедура ЗадатьВопросОПереносеФайловВДОПродолжение(Результат, ПараметрыОповещения) Экспорт
	...
	Если Результат.Значение = КодВозвратаДиалога.Да Тогда
		...
		Иначе
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли Результат.Значение = "ПоказатьФайлы" Тогда
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ОткрытьФормуСпискаФайловБСП(
			ПараметрыОповещения.ПараметрыПереносаФайлов.ВладелецИС);
	КонецЕсли;
...
Процедура ЗаменитьРольФайлов(СписокФайлов, ВыбраннаяРольID, ВладелецФайлов,
	...
	Для Каждого Файл Из ДанныеФайлов Цикл
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Изменена роль файла ""%1""'"), Файл.Наименование);
...
// Возвращает данные перетаскиваемых файлов в дереве файлов.
//
// Параметры:
//   ДеревоФайлов - ДанныеФормыДерево - дерево файлов.
//   ЗначениеПеретаскивания - Файл - файл, перетаскиваемый с диска.
//                          - СсылкаНаФайл - файл, перетаскиваемый с диска.
//                          - Массив из Файл - список файлов, перетаскиваемых с диска.
//                          - Массив из СсылкаНаФайл - список файлов, перетаскиваемых с диска.
//                          - Массив из Число - список идентификаторов перетаскиваемых строк дерева файлов.
//
// Возвращаемое значение:
//   Массив из Файл
//   Массив из см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеФайла
//
Функция МассивФайловИзПараметровПеретаскивания(ДеревоФайлов, ЗначениеПеретаскивания) Экспорт
	...
	ИначеЕсли ТипЗнч(ЗначениеПеретаскивания) = Тип("СсылкаНаФайл") И ЗначениеПеретаскивания.Файл.ЭтоФайл() Тогда
		
		МассивФайлов.Добавить(ЗначениеПеретаскивания.Файл);
	...
	ИначеЕсли ТипЗнч(ЗначениеПеретаскивания) = Тип("Массив") Тогда
		...
		Для Каждого ПеретаскиваемыйФайл Из ЗначениеПеретаскивания Цикл
			...
			ИначеЕсли ТипЗнч(ПеретаскиваемыйФайл) = Тип("СсылкаНаФайл") И ПеретаскиваемыйФайл.Файл.ЭтоФайл() Тогда
				МассивФайлов.Добавить(ПеретаскиваемыйФайл.Файл);
	...
	ИначеЕсли ТипЗнч(ЗначениеПеретаскивания) = Тип("Число") Тогда
		
		ФайлДерева = ДеревоФайлов.НайтиПоИдентификатору(ЗначениеПеретаскивания);
		Если ФайлДерева <> Неопределено И ФайлДерева.Тип <> "DMFileRole" Тогда
			ДанныеФайла = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеФайла(
				ФайлДерева.Наименование,
				ФайлДерева.ID,
...
// Определяет, можно ли сохранить объект ДО на диск.
//
// Параметры:
...
//   Булево
//
Функция ОбъектМожноСохранитьНаДиск(ТипОбъектаДО) Экспорт
	
	Возврат (ТипОбъектаДО = "DMFile")
		Или (ТипОбъектаДО = "DMIncomingElectronicDocument")
		Или (ТипОбъектаДО = "DMOutgoingElectronicDocument");
...
// Ищет строку дерева файлов по идентификатору файла.
//
// Параметры:
//   ДеревоФайлов - ДанныеФормыДерево - дерево файлов.
//   ИспользоватьРолиФайлов - Булево - признак использования ролей файлов.
//   ИскомыйИдентификатор - Строка - идентификатор, по которому требуется найти строку.
//   ИскомыйТип - Строка - тип, по которому требуется найти строку.
//
// Возвращаемое значение:
//   ДанныеФормыЭлементДерева - строка дерева файлов.
//
Функция СтрокаДереваФайловПоID(ДеревоФайлов, ИспользоватьРолиФайлов, ИскомыйИдентификатор,
		ИскомыйТип = Неопределено) Экспорт
	
	Для Каждого Строка Из ДеревоФайлов.ПолучитьЭлементы() Цикл
		
		Если Строка.ID = ИскомыйИдентификатор И Строка.Тип = ИскомыйТип Тогда
			Возврат Строка;
			
		ИначеЕсли Строка.ID = ИскомыйИдентификатор И ИскомыйТип = Неопределено Тогда
			Возврат Строка;
		...
		Если ИспользоватьРолиФайлов Тогда
			Для Каждого СтрокаФайл Из Строка.ПолучитьЭлементы() Цикл
				Если СтрокаФайл.ID = ИскомыйИдентификатор И СтрокаФайл.Тип = ИскомыйТип Тогда
					Возврат СтрокаФайл;
					
				ИначеЕсли СтрокаФайл.ID = ИскомыйИдентификатор И ИскомыйТип = Неопределено Тогда
					Возврат СтрокаФайл;
...
// Открывает форму просмотра ЭП или файл.
//
// Параметры:
//   ДеревоПодписейТекущиеДанные - ДанныеФормыЭлементДерева - текущая строка дерева подписей.
//   ДеревоФайлов - ДанныеФормыДерево - дерево файлов.
//   ИспользоватьРолиФайлов - Булево - признак использования ролей файлов.
//   УникальныйИдентификаторФормы - УникальныйИдентификатор - идентификатор вызвавшей формы.
//
Процедура ОткрытьПодпись(ДеревоПодписейТекущиеДанные, ДеревоФайлов, ИспользоватьРолиФайлов,
		УникальныйИдентификаторФормы) Экспорт
	
	Если ДеревоПодписейТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ЭтоПодпись(ДеревоПодписейТекущиеДанные) Тогда
		
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ОткрытьПодпись(ДеревоПодписейТекущиеДанные);
		
	ИначеЕсли ДеревоПодписейТекущиеДанные.ОбъектТип = "DMFile" Тогда
		
		СтрокаДереваФайлов = СтрокаДереваФайловПоID(
			ДеревоФайлов,
			ИспользоватьРолиФайлов,
			ДеревоПодписейТекущиеДанные.ОбъектID,
			ДеревоПодписейТекущиеДанные.ОбъектТип);
		Если СтрокаДереваФайлов <> Неопределено Тогда
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ОткрытьФайл(
				СтрокаДереваФайлов.ID,
				СтрокаДереваФайлов.Наименование,
				СтрокаДереваФайлов.Расширение,
				УникальныйИдентификаторФормы);
		КонецЕсли;
...
// Удаляет выделенные электронные подписи.
//
// Параметры:
//   ДеревоПодписейВыделенныеСтроки - Массив из Число - массив индексов выделенных строк подписей.
...
//   УникальныйИдентификаторФормы - УникальныйИдентификатор - идентификатор вызвавшей формы.
//
Процедура УдалитьПодпись(ДеревоПодписейВыделенныеСтроки, ДеревоПодписей, ВладелецФайла,
	...
	ВыделенныеПодписи = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыделенныеПодписи(
		ДеревоПодписейВыделенныеСтроки,
		ДеревоПодписей);
...
Функция ДанныеПоЗадаче(Форма, ID, Тип, Заголовок, ПараметрыОперации = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(РезультатВариантаНомер) Тогда
		...
		Форма.Элементы.РезультатДействияТекстом.ТолькоПросмотр = Истина;
	КонецЕсли;
...
// Возвращает данные основного приложения задачи.
//
// Параметры:
//   СтрокаДереваПриложений - ДанныеФормыЭлементДерева - текущая строка дерева приложений задачи.
//   ДеревоПриложений - ДанныеФормыДерево - дерево приложений задачи.
//
// Возвращаемое значение:
//   ДанныеФормыЭлементДерева
//
Функция ОсновнойПредметВДеревеПриложений(СтрокаДереваПриложений, ДеревоПриложений) Экспорт
...
//   УникальныйИдентификаторФормы - УникальныйИдентификатор - идентификатор вызвавшей формы.
//
Процедура Подключаемый_СоздатьСвязанныйОбъект(Команда, ОсновнойПредметID, ОсновнойПредметТип,
		УникальныйИдентификаторФормы) Экспорт
	
	ИмяКоманды = Команда.Имя;
	ГУИД = Прав(ИмяКоманды, 36);
	ГУИД = СтрЗаменить(ГУИД, "_", "-");
	
	ЧастиКоманды = СтрРазделить(ИмяКоманды, "_");
	
	Если ЧастиКоманды[0] = "Создать" Тогда
		
		// Создадим новый объект ИС по правилу интеграции.
		УИДПравила = Новый УникальныйИдентификатор(ГУИД);
	...
	Иначе
		...
		// Отразим в учете документ ЭДО.
		ИмяПодсистемыИнтерфейсДокументовЭДО = "ЭлектронноеВзаимодействие.ОбменСКонтрагентами.ИнтерфейсДокументовЭДО";
		ЕстьИнтерфейсДокументовЭДО = ОбщегоНазначенияКлиент.ПодсистемаСуществует(ИмяПодсистемыИнтерфейсДокументовЭДО);
		Если ЕстьИнтерфейсДокументовЭДО Тогда
			...
			Если ЭтоФормализованныйДокументЭДО(Расширение) Тогда
				ВыборВыполнен = Истина;
				ДополнительныеПараметры = Новый Структура("СсылкаНаДокумент, УникальныйИдентификатор",
					Неопределено, УникальныйИдентификаторФормы);
				ИмяМодуляИнтерфейсЭДО = "ИнтерфейсДокументовЭДОКлиент";
				МодульИнтерфейсДокументовЭДОКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(ИмяМодуляИнтерфейсЭДО);
				МодульИнтерфейсДокументовЭДОКлиент.ЗагрузитьДанныеИзФайлаОбработатьРезультатПомещенияФайла(
					ВыборВыполнен,
					АдресФайла,
					"data.xml",
					ДополнительныеПараметры);
			Иначе
				ПоказатьПредупреждение(,НСтр("ru = 'Отразить в учете можно только формализованный документ ЭДО'"));
			КонецЕсли;
...
Асинх Процедура ПриложенияЗадачиФайлыВставитьКартинкуИзБуфера(Знач СтрокаДереваПриложений,
	...
	Иначе
		...
		Если НеобходимоДобавитьСканКопию = КодВозвратаДиалога.Да Тогда
			РазрешенаТолькоСканКопия = СостояниеРазрешаетДобавлениеСканКопии И Не СостояниеРазрешаетДобавлениеФайла;
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.СоздатьФайлИзБуфера(
				ПараметрыНачалаСозданияФайла(СтрокаДереваПриложений, УникальныйИдентификаторФормы, РазрешенаТолькоСканКопия));
		КонецЕсли;
...
Процедура ПриложенияЗадачиФайлыСохранитьКак(СтрокаДереваПриложений, Форма) Экспорт
	...
	Если Форма.Элементы.ДеревоПриложений.ВыделенныеСтроки.Количество() > 1 Тогда
		Для Каждого ВыбраннаяСтрока Из Форма.Элементы.ДеревоПриложений.ВыделенныеСтроки Цикл
			...
			Если ОбъектМожноСохранитьНаДиск(ДанныеСтроки.ПриложениеТип) Тогда
				ВыделенныеФайлы.Добавить(ДанныеФайлаДляСохранения(ДанныеСтроки, Форма));
			КонецЕсли;
		КонецЦикла;
	Иначе
		Если ОбъектМожноСохранитьНаДиск(СтрокаДереваПриложений.ПриложениеТип) Тогда
			ВыделенныеФайлы.Добавить(ДанныеФайлаДляСохранения(СтрокаДереваПриложений, Форма));
		КонецЕсли;
	КонецЕсли;
...
//
Процедура УстановитьДоступностьКомандПриложенийЗадачи(ТекущаяСтрока, Форма, ЭтоТонкийWindowsКлиент) Экспорт
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ЭтоДругоеПриложение = ЗначениеЗаполнено(ТекущиеДанные.ПриложениеТип)
			...
			И (ТекущиеДанные.ПриложениеТип <> "DMTaskAction");
	...
	Форма.Элементы.ДеревоПриложенийКонтекстноеМенюФайлыУдалить.Видимость =
		ЭтоЗаполняемоеПриложение
		Или (ЭтоФайлПриложения И ДоступноИнтерактивноеИзменениеФайлов);
	Форма.Элементы.ДеревоПриложенийКонтекстноеМенюСохранитьКак.Видимость =
		ОбъектМожноСохранитьНаДиск(ТекущиеДанные.ПриложениеТип);
	Форма.Элементы.ДеревоПриложенийКонтекстноеМенюКопироватьВБуферОбмена.Видимость =
		ЭтоФайл Или ЭтоДругоеПриложение;
	
	// Доступность.
	Форма.Элементы.ДеревоПриложенийКонтекстноеМенюОткрытьКарточку.Доступность = ЭтоФайл;
	Форма.Элементы.ДеревоПриложенийКонтекстноеМенюСохранитьКак.Доступность =
		ОбъектМожноСохранитьНаДиск(ТекущиеДанные.ПриложениеТип) И Не ТекущиеДанные.Зашифрован;
...
//
Процедура НачатьОбработку(Предмет, ОписаниеОповещения = Неопределено, ИспользоватьПриоритетныеПравила = Ложь) Экспорт
	
	Параметры = Новый Структура("Предмет");
	Параметры.Вставить("ОписаниеОповещения", ОписаниеОповещения);
	Параметры.Вставить("СозданНовыйОбъектДО", Ложь);
	
	Если ТипЗнч(Предмет) = Тип("Структура") Тогда
		
		Параметры.Предмет = Предмет;
		НачатьОбработкуПослеПоискаСвязанногоОбъектаДО(Неопределено, Параметры);
		
	ИначеЕсли ТипЗнч(Предмет) = Тип("Массив") Тогда
		
		Для Каждого ОбъектИС Из Предмет Цикл
			Параметры.Предмет = ОбъектИС;
...
Процедура НачатьОбработкуПослеПоискаСвязанногоОбъектаДО(Результат, Параметры) Экспорт
	...
	ТребуетсяОбновлениеОбъектаДО = Не Параметры.СозданНовыйОбъектДО;
	ТекстОшибки = "";
	Если ИнтеграцияС1СДокументооборот3ВызовСервера.НачатьОбработку(
			Параметры.Предмет, ТребуетсяОбновлениеОбъектаДО, ТекстОшибки) Тогда
	...
	Иначе
		...
		ПоказатьПредупреждениеИзДО(,ТекстОшибки,,
			СтрШаблон(НСтр("ru = 'Не удалось начать обработку ""%1""'"), Параметры.Предмет.name));
...
// Перечисление видов изменений участников в действиях.
//
// Возвращаемое значение:
//   Структура:
//     * ИзменениеИсполнителя - Строка
//     * ПеремещениеВверх - Строка
//     * ПеремещениеВниз - Строка
//     * ДобавлениеЭтапа - Строка
//
Функция ВидыИзмененийУчастниковДействия() Экспорт
	
	ВидыИзменений = Новый Структура;
	ВидыИзменений.Вставить("ИзменениеИсполнителя", "ИзменениеИсполнителя");
	ВидыИзменений.Вставить("ПеремещениеВверх", "ПеремещениеВверх");
	ВидыИзменений.Вставить("ПеремещениеВниз", "ПеремещениеВниз");
	ВидыИзменений.Вставить("ДобавлениеЭтапа", "ДобавлениеЭтапа");
	
	Возврат ВидыИзменений;
...
Процедура ДобавитьЭтапДействия(Форма, ФункцияЭтапа) Экспорт
	...
	ПараметрыОповещенияИзмененияУчастников = НовыеПараметрыОповещенияИзмененияУчастников();
	ПараметрыОповещенияИзмененияУчастников.ТекущиеДанные = ТекущиеДанные;
	ПараметрыОповещенияИзмененияУчастников.ВидИзменения = ВидыИзмененийУчастниковДействия().ДобавлениеЭтапа;
	
	Оповестить("ОбновитьПредставленияВДеревеУчастников", ПараметрыОповещенияИзмененияУчастников,
...
// Конструктор параметров оповщения об изменении участников действия.
//
// Возвращаемое значение:
//   Структура:
//     * ТекущиеДанные - Неопределено - изменяемой строки нет.
//                     - ДанныеФормыЭлементКоллекции - изменяемая строка участников.
//     * ВидИзменения - Неопределено - не идентифицирован вид изменения.
//                    - Строка - см. ВидыИзмененийУчастниковДействия
//
Функция НовыеПараметрыОповещенияИзмененияУчастников() Экспорт
	
	Параметры = Новый Структура;
	Параметры.Вставить("ТекущиеДанные", Неопределено);
	Параметры.Вставить("ВидИзменения", Неопределено);
...
Процедура ПереместитьУчастникаДействияВверх(Форма, МожноМенятьПорядокЭтапов = Истина, ПоПунктам = Ложь) Экспорт
	...
	ПараметрыОповещенияИзмененияУчастников = НовыеПараметрыОповещенияИзмененияУчастников();
	ПараметрыОповещенияИзмененияУчастников.ТекущиеДанные = ТекущиеДанные;
	ПараметрыОповещенияИзмененияУчастников.ВидИзменения = ВидыИзмененийУчастниковДействия().ПеремещениеВверх;
...
Процедура ПереместитьУчастникаДействияВниз(Форма, МожноМенятьПорядокЭтапов = Истина, ПоПунктам = Ложь) Экспорт
	...
	ПараметрыОповещенияИзмененияУчастников = НовыеПараметрыОповещенияИзмененияУчастников();
	ПараметрыОповещенияИзмененияУчастников.ТекущиеДанные = ТекущиеДанные;
	ПараметрыОповещенияИзмененияУчастников.ВидИзменения = ВидыИзмененийУчастниковДействия().ПеремещениеВниз;
	
	Оповестить("ОбновитьПредставленияВДеревеУчастников", ПараметрыОповещенияИзмененияУчастников, Форма.УникальныйИдентификатор);
	УчастникиДействияПриАктивизацииСтроки(Форма);
...
Процедура ПредставлениеУчастникаДействияНачалоВыбораЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	ПараметрыОповещенияИзмененияУчастников = НовыеПараметрыОповещенияИзмененияУчастников();
	ПараметрыОповещенияИзмененияУчастников.ТекущиеДанные = ТекущиеДанные;
	ПараметрыОповещенияИзмененияУчастников.ВидИзменения = ВидыИзмененийУчастниковДействия().ИзменениеИсполнителя;
	
	Оповестить("ОбновитьПредставленияВДеревеУчастников", ПараметрыОповещенияИзмененияУчастников,
		ПараметрыОповещения.Форма.УникальныйИдентификатор);
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		Иначе
			...
			Если ДанныеВыбора.Количество() = 1 И Не ДанныеВыбора[0].Значение.isNotAvailableAccordingCommunicationRules Тогда
				...
				Оповестить("ОбновитьПредставленияВДеревеУчастников",
					ПараметрыОповещенияИзмененияУчастников, Форма.УникальныйИдентификатор);
...
Процедура УчастникиДействияПередНачаломДобавления(Форма, Отказ, Копирование, ПараметрыДобавленияУчастников) Экспорт
	...
	Если СтрокаУчастника.Свойство("ВышестоящийУчастник") Тогда
		СтрокаУчастника.ВышестоящийУчастник = "00000000-0000-0000-0000-000000000000";
	КонецЕсли;
...
Функция ЭтапыДействияУникальны(Участники) Экспорт
	...
	Для Каждого ЭлементЭтап Из Участники.ПолучитьЭлементы() Цикл
		Если ЗначениеЗаполнено(ЭлементЭтап.НаименованиеЭтапа)
			...
			ПоказатьПредупреждение(,СтрШаблон(НСтр("ru = 'Повторяется этап ""%1""'"), ЭлементЭтап.НаименованиеЭтапа));
			Возврат Ложь;
		КонецЕсли;
...
// Возвращает текущие данные строки дерева связей для просмотра связанного объекта.
//
// Параметры:
//   ДеревоСвязей - ТаблицаФормы - дерево связей на форме.
//
// Возвращаемое значение:
//   ДанныеФормыЭлементДерева
//   Неопределено - если текущие данные не выделены.
//
Функция ДеревоСвязейТекущиеДанныеДляПросмотра(Знач ДеревоСвязей) Экспорт
	
	ТекущиеДанные = ДеревоСвязей.ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено И (ТекущиеДанные.ЭтоВеткаСвязи Или ТекущиеДанные.ЭтоРольФайла) Тогда
		ТекущиеДанные = ТекущиеДанные.ПолучитьРодителя();
	КонецЕсли;
...
// Начинает добавление связи к указаному объекту Документооборота.
//
// Параметры:
//   ИсходныйОбъектДО - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО
//
Процедура НачатьДобавлениеСвязи(ИсходныйОбъектДО) Экспорт
	
	ОткрытьФорму("Обработка.ИнтеграцияС1СДокументооборот3.Форма.СозданиеСвязи",
		ИсходныйОбъектДО,,,,,,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

// Начинает удаление связи из дерева связей.
//
// Параметры:
//   ДеревоСвязей - ТаблицаФормы - дерево связей на форме.
//   ИсходныйОбъектДО - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО
//
Асинх Процедура НачатьУдалениеСвязи(Знач ДеревоСвязей, Знач ИсходныйОбъектДО) Экспорт
	
	ТекущиеДанные = ДеревоСвязей.ТекущиеДанные;
	
	Если Не СвязьМожноУдалить(ТекущиеДанные) Тогда
		Возврат;
	КонецЕсли;
	...
	СвязанныйОбъект = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО(
		ТекущиеДанные.СвязанныйОбъектID,
		ТекущиеДанные.СвязанныйОбъектТип,
		ТекущиеДанные.СвязанныйОбъект,
		ТекущиеДанные.СвязаннаяСтрока);
	
	ТипСвязи = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО(
		ТекущиеДанные.ТипСвязиID,
		"DMRelationType");
	
	ИнтеграцияС1СДокументооборот3ВызовСервера.УдалитьСвязьОбъектовДО(
		ИсходныйОбъектДО,
		СвязанныйОбъект,
		ТипСвязи);
	
	Оповестить("УдалениеСвязиОбъектовДО", ИсходныйОбъектДО);
...
Процедура ОткрытьСвязанныйОбъектНаПросмотр(Знач СтрокаДереваСвязей, УникальныйИдентификаторФормы) Экспорт
	...
	Если ЗначениеЗаполнено(СтрокаДереваСвязей.СвязаннаяСтрока) Тогда
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ОткрытьНавигационнуюСсылку(
			СтрокаДереваСвязей.СвязаннаяСтрока);
		Возврат;
	КонецЕсли;
...
// Открывает связи объекта Документооборота.
//
// Параметры:
//   ОбъектДО - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО
//
Процедура ПоказатьСвязиОбъекта(ОбъектДО) Экспорт
	
	ОткрытьФорму("Обработка.ИнтеграцияС1СДокументооборот3.Форма.СвязиОбъекта", ОбъектДО,, ОбъектДО.ID);
	
КонецПроцедуры

// Разворачивает строку дерева связей.
//
// Параметры:
//   РазворачиваемаяСтрока - Число - индекс разворачиваемой строки.
//   ДеревоСвязей - ДанныеФормыДерево - реквизит формы, содержащий дерево связей.
//   ЭлементыДеревоСвязей - ТаблицаФормы - элемент формы, содержащий дерево связей.
//
Процедура РазвернутьУровеньДереваСвязей(РазворачиваемаяСтрока, ДеревоСвязей, ЭлементыДеревоСвязей) Экспорт
	
	Если РазворачиваемаяСтрока = 0 Тогда
		КоллекцияСтрокУровеньТиповСвязей = ДеревоСвязей.ПолучитьЭлементы();
	Иначе
		КоллекцияСтрокУровеньТиповСвязей = ДеревоСвязей.НайтиПоИдентификатору(РазворачиваемаяСтрока).ПолучитьЭлементы();
	КонецЕсли;
	...
	Для Каждого СтрокаТипСвязи Из КоллекцияСтрокУровеньТиповСвязей Цикл
		...
		ЭлементыДеревоСвязей.Развернуть(ИдентификаторСтроки, Ложь);
		...
		Для Каждого СтрокаСвязанныйОбъект Из КоллекцияСтрокУровеньСвязанныхОбъектов Цикл
			...
			ЭлементыДеревоСвязей.Развернуть(ИдентификаторСтроки, СтрокаСвязанныйОбъект.ЭтоРольФайла);
			...
			Для Каждого СтрокаФайл Из КоллекцияСтрокУровеньФайлов Цикл
				Если Не СтрокаФайл.ЭтоВеткаСвязи Тогда
					...
					ЭлементыДеревоСвязей.Развернуть(ИдентификаторСтроки, СтрокаФайл.ЭтоРольФайла);
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
...
// Возвращает признак возможности удаления связи.
//
// Параметры:
//   ДеревоСвязейТекущиеДанные - ДанныеФормыЭлементДерева - текущая строка дерева связей.
//
// Возвращаемое значение:
//   Булево
//
Функция СвязьМожноУдалить(ДеревоСвязейТекущиеДанные) Экспорт
	
	Если ДеревоСвязейТекущиеДанные = Неопределено
			Или ДеревоСвязейТекущиеДанные.ЭтоРодитель
			Или ДеревоСвязейТекущиеДанные.ЭтоРольФайла
			Или ДеревоСвязейТекущиеДанные.ЭтоВеткаСвязи Тогда
		Возврат Ложь;
	КонецЕсли;
	
	СтрокаРодитель = ДеревоСвязейТекущиеДанные.ПолучитьРодителя();
	
	Если ДеревоСвязейТекущиеДанные.СвязанныйОбъектТип = "DMFile" И Не СтрокаРодитель.ЭтоРодитель Тогда
		// Это файл, приложенный к связанному документу.
		Возврат Ложь;
	КонецЕсли;
...
Функция ДанныеФайлаДляСохранения(СтрокаДереваПриложений, Форма)
	
	Если СтрокаДереваПриложений.ПриложениеТип = "DMIncomingElectronicDocument"
			Или СтрокаДереваПриложений.ПриложениеТип = "DMOutgoingElectronicDocument" Тогда
		Расширение = "xml";
	Иначе
		Расширение = СтрокаДереваПриложений.Расширение;
	КонецЕсли;
...
//
Процедура НачатьОбработкуПроверитьПодключениеЗавершение(Результат, МассивСсылок) Экспорт
	
	Если Результат = Истина Тогда
		// авторизация успешна
		НачатьОбработку(МассивСсылок);
	КонецЕсли;
	
КонецПроцедуры

// Обработчик оповещения после выбора фактического исполнителя.
//
// Параметры:
//   Результат - Структура - выбранный исполнитель.
//   ПараметрыОповещения - см. ПараметрыОповещенияПослеВыполненияИнтерактивногоДействияНадЗадачей
//
Процедура ОбработатьДанныеВыполненияПослеВыбораИсполнителя(Результат, ПараметрыОповещения) Экспорт
	
	Если ТипЗнч(Результат) <> Тип("Структура") Тогда
		// Вернуться к задаче.
		ПриВозвращенииКЗадаче(ПараметрыОповещения.ПовторноеДействие.Модуль);
		Возврат;
	КонецЕсли;
...
// Обработчик оповещения после изменения парамтров процесса.
//
// Параметры:
...
// Обработчик оповещения после подписания объектов электронной подписью.
//
// Параметры:
//   Результат - Неопределено.
//   ПараметрыОповещения - см. ПараметрыОповещенияПослеВыполненияИнтерактивногоДействияНадЗадачей
//
Процедура ОбработатьДанныеВыполненияПослеПодписания(Результат, ПараметрыОповещения) Экспорт
	...
	ТребуемоеИнтерактивноеДействие =
		ПараметрыОповещения.ПовторноеДействие.ДополнительныеПараметры.ТребуемоеИнтерактивноеДействие;
	ТребуемоеИнтерактивноеДействие.ВыполнитьПодписание = Истина;
	ТребуемоеИнтерактивноеДействие.Контекст = ПараметрыОповещения.ПервоначальныйКонтекст;
	ТребуемоеИнтерактивноеДействие.Контекст.Вставить("НаборДанных", Результат.НаборДанных);
...
//   СтрокаДереваПриложений - ДанныеФормыЭлементДерева - текущая строка дерева приложений.
//   УникальныйИдентификаторФормы - УникальныйИдентификатор - идентификатор вызвавшей формы.
//   РазрешенаТолькоСканКопия - Булево - признак того, что добавляемый файл может быть только скан-копией.
//
// Возвращаемое значение:
//   см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПараметрыНачалаСозданияФайла
//
Функция ПараметрыНачалаСозданияФайла(СтрокаДереваПриложений, УникальныйИдентификаторФормы, РазрешенаТолькоСканКопия)
	...
	ПараметрыНачалаСоздания.ЯвляетсяСканКопией = РазрешенаТолькоСканКопия;
...
// Конструктор структуры параметров обработчика оповещения, который будет вызван после выполнения требуемого
// интерактивного действия над задачей.
//
// Возвращаемое значение:
//   Структура:
//     * ТипОперации - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ТипОперацииНадЗадачей
//     * ПовторноеДействие - ОписаниеОповещения - метод, который будет вызван по завершении интерактивной обработки.
//     * ПервоначальныйКонтекст - Структура, Строка - произвольные данные, переданные изначально.
//
Функция ПараметрыОповещенияПослеВыполненияИнтерактивногоДействияНадЗадачей()
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ТипОперации", Неопределено);
	ПараметрыОповещения.Вставить("ПовторноеДействие", Неопределено);
	ПараметрыОповещения.Вставить("ПервоначальныйКонтекст", Неопределено);
...
Процедура ПриВозвращенииКЗадаче(Модуль)
	
	Если ТипЗнч(Модуль) = Тип("ФормаКлиентскогоПриложения") Тогда
		Модуль.Элементы.РезультатДействияТекстом.ТолькоПросмотр = Ложь;
		Модуль.ПодключитьОбработчикОжидания("СохранитьРезультатДействия", 0.2, Истина);
	КонецЕсли;
...
Функция ЭтоФормализованныйДокументЭДО(Расширение)
	
	Возврат (НРег(Расширение) = "xml");
