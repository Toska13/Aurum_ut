...
Процедура ЗапуститьКэшированиеВФоне() Экспорт
	...
	ДлительныеОперации.ВыполнитьПроцедуру(ПараметрыВыполнения, 
		"ВизуализацияЭДОСлужебный.ПолучениеВизуализацииЭлектронныхДокументовЭДО"); 
...
Процедура ОбновитьКэшВизуализации(ДокументЭДО) Экспорт
	...
	Если Не ЗначениеЗаполнено(ОсновнойФайл) Тогда
		ТекстСообщения = НСтр("ru = 'Основной файл документа не доступен или удален.'");
		ЗаписатьОшибкуВЖурналРегистрации(ДокументЭДО, ТекстСообщения);
		Возврат;
	КонецЕсли;
	СвойстваОсновнойФайл = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОсновнойФайл, "Расширение, Размер");
	Если СвойстваОсновнойФайл.Размер = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Основной файл документа не доступен. Не удалось прочитать размер файла.'");
		ЗаписатьОшибкуВЖурналРегистрации(ДокументЭДО, ТекстСообщения);
		Возврат;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(СвойстваОсновнойФайл.Расширение) Тогда
		ТекстСообщения = НСтр("ru = 'Расширение основного файла не заполнено.'");
		ЗаписатьОшибкуВЖурналРегистрации(ДокументЭДО, ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ПолучениеВизуализацииЭлектронныхДокументовЭДО() Экспорт
	...
	ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер("ПолучениеВизуализацииЭлектронныхДокументовЭДО");
	СчетчикДокументов = 0;
	Запрос = Новый Запрос;
	...
	Пока Не Результат.Пустой() Цикл
		...
		Результат = Запрос.Выполнить();
	КонецЦикла;
	ОбщегоНазначенияБЭД.ЗакончитьЗамер("ПолучениеВизуализацииЭлектронныхДокументовЭДО", ОписаниеЗамера,
		СчетчикДокументов);
КонецПроцедуры
...
Процедура ОбновитьКэшВизуализацииЭлектронногоДокументаЭДО(ДокументЭДО) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		ОбновитьВизуализациюДокументаЭДОИзОчереди(ПараметрыВыполнения);
	КонецЕсли;
	ОбщегоНазначенияБЭД.ЗакончитьЗамер("ПолучениеВизуализацииЭлектронныхДокументовЭДО", ОписаниеЗамера, 1);
КонецПроцедуры
...
Функция ПроштампованныйФайл(СообщениеЭДО) Экспорт
	...
	КонецЕсли;
	ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер("ПолучениеВизуализацииPdfИзКэша.ФоновоеЗадание");
	...
	ОбщегоНазначенияБЭД.ЗакончитьЗамер("ПолучениеВизуализацииPdfИзКэша.ФоновоеЗадание", ОписаниеЗамера);
	Если КэшВизуализации.ТребуетсяОбновить Тогда
		ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер("ОбновлениеКэшаPdf.ФоновоеЗадание");
		ОбновитьКэшВизуализации(ЭлектронныйДокумент);
		КэшВизуализации = ПолучитьВизуализациюИзКэша(ЭлектронныйДокумент, НастройкиПредставления);
		ОбщегоНазначенияБЭД.ЗакончитьЗамер("ОбновлениеКэшаPdf.ФоновоеЗадание", ОписаниеЗамера);
	КонецЕсли;
...
Функция НовыеПараметрыВизуализацииДокумента() Экспорт
	...
	ПараметрыВизуализацииДокумента.Вставить("ВыводитьШтампыПодписей", Истина);
...
Функция ВизуализацияДокумента(ДанныеДокумента, ПараметрыВизуализации, ТребуетсяОбновитьКэш = Ложь) Экспорт
	...
	Если Не ПараметрыВизуализации.ВыводитьШтампыПодписей Тогда
		Возврат ВизуализацияДокументаБезШтампов(ДанныеДокумента, ПараметрыВизуализации, НастройкиПредставления);
	КонецЕсли;
...
// Готовит визуализацию документа без штампов и возвращает результат
//  Параметры:
//  ДанныеДокумента - См. НовыеСвойстваДокументаДляФормированияВизуализации
//  ПараметрыВизуализации - См. НовыеПараметрыВизуализацииДокумента
//  НастройкиПредставления - см. ВизуализацияЭДО.НовыеНастройкиПредставления
...
// Возвращаемое значение:
//  См. НовыйРезультатПредставленияДанныхСообщения
Функция ВизуализацияДокументаБезШтампов(ДанныеДокумента, ПараметрыВизуализации, НастройкиПредставления)
	Результат = НовыйРезультатПредставленияДанныхСообщения();
	ДанныеДокументаДляПроверкиРаспознавания = НовыеДанныеДокументаДляПроверкиРаспознавания();
	ЗаполнитьЗначенияСвойств(ДанныеДокументаДляПроверкиРаспознавания.ПроверкаЗаписанногоДокумента, ДанныеДокумента);
	ДанныеДокументаДляПроверкиРаспознавания.ПроверкаЗаписанногоДокумента.ТипЭлементаРегламента = 
	...
	Если Не ДокументРаспознан(ДанныеДокументаДляПроверкиРаспознавания) Тогда
		Результат = ПредставлениеДанныхСообщенияПоФайлу(ДанныеДокумента.Ссылка);
		Если Не РаботаСФайламиБЭДКлиентСервер.ЭтоРасширениеФайлаPDF(Результат.РасширениеФайла) 
			И ФайлПодходитДляОбработкиЧерезСервис1СШтамп(Результат.РасширениеФайла,
			Результат.РазмерФайла) Тогда
		
			РезультатШтампования = ВизуализацияДокументаЭДОСПомощьюСервиса1СШтамп(
				Результат.ПредставлениеДокумента,
				Результат.РасширениеФайла,
				БиблиотекаКартинок.Пустая);
		
			Если Не РезультатШтампования.ЕстьОшибки Тогда
				Результат.ПредставлениеДокумента = РезультатШтампования.ДвоичныеДанные;
			КонецЕсли;
		КонецЕсли;
		Возврат Результат;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		...
		ПредставлениеДанных = МодульОбменСГИСЭПД.ПриВизуализацииДокумента(ПараметрыПолученияПредставления,
			СвойстваСообщений);
		Если ПредставлениеДанных <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(Результат, ПредставлениеДанных);
			Возврат Результат;
		КонецЕсли;
	КонецЕсли;
	...
	КонецЦикла;
	ПараметрыПолученияПредставления.ВыводитьШтампыПодписей = Ложь;
	Результат = ПредставлениеДанныхСообщенияПоСсылке(ПараметрыПолученияПредставления);
...
Процедура ЗаписатьОшибкуВЖурналРегистрации(ДокументЭДО, ПричинаОшибки)
	ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось обновить кэш визуализации для документа %1 по причине:'")
		+ Символы.ПС + ПричинаОшибки,
		ДокументЭДО);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПолучениеВизуализацииЭлектронныхДокументовЭДО", Истина);
