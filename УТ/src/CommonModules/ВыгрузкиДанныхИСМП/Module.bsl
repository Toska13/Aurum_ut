...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗагрузитьСведенияОбОтклоненияхИзГИСМТ", Истина);
	...
	Пока Строка <> Неопределено Цикл
		...
		Для Каждого ЭлементКоллекции Из ДанныеСтроки Цикл
			Если ЭтоРазделитель Тогда
				ЕстьЗамена = СтрНайти(ЭлементКоллекции, Замена) > 0;
				Если ЕстьЗамена Тогда
					ЭлементКоллекции = СтрЗаменить(ЭлементКоллекции, Замена, "");
				КонецЕсли;
			...
			Иначе
				...
				Если ЕстьЗамена Тогда
					ЭлементКоллекции = СтрШаблон("%1%2", Замена, ЭлементКоллекции);
					ЕстьЗамена = Ложь;
				КонецЕсли;
				Если ИмяКолонки <> Неопределено Тогда
...
Процедура ПодготовитьДанныеКПередачеПоНастройкеРегламентногоЗадания(ПараметрыОбмена)
	...
	Если ПараметрыОбмена.Организации <> Неопределено Тогда
		...
		"И ОчередьСообщений.Организация В (&Организации)";
