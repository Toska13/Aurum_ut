...
// Параметры:
//  ОбработчикЗавершения - ОписаниеОповещения - Обработчик, в который будет передан массив готовых к документообороту объектов учета
// 
// Возвращаемое значение:
//  Структура:
// * Настройки - См. ПереопределяемыеНастройкиПодготовкиКДокументообороту
// * ОбработчикЗавершения - ОписаниеОповещения
Функция ПараметрыПодготовкиКДокументообороту(ОбработчикЗавершения) Экспорт
	ПараметрыПодготовкиКДокументообороту = Новый Структура;
	ПараметрыПодготовкиКДокументообороту.Вставить("Настройки", ПереопределяемыеНастройкиПодготовкиКДокументообороту());
	ПараметрыПодготовкиКДокументообороту.Вставить("ОбработчикЗавершения", ОбработчикЗавершения);
	Возврат ПараметрыПодготовкиКДокументообороту;
КонецФункции

// Возвращаемое значение:
//  Структура:
// * ВыводитьПодтверждениеПриМассовомФормировании - Булево - по-умолчанию Истина, выводить диалог подтверждения действия при массовом формировании документов
// * ПроверятьПроведенностьДокументов - Булево - по-умолчанию Истина, выполнять проверку проведения документов, можно переопределить с помощью
//                                               См. ЭлектронноеВзаимодействиеКлиентПереопределяемый.ВыполнитьПроверкуПроведенияДокументов
Функция ПереопределяемыеНастройкиПодготовкиКДокументообороту() Экспорт
	Настройки = Новый Структура;
	Настройки.Вставить("ВыводитьПодтверждениеПриМассовомФормировании", Истина);
	Настройки.Вставить("ПроверятьПроведенностьДокументов", Истина);
	ПереопределенныеПараметры = ОбщегоНазначенияКлиент.СкопироватьРекурсивно(Настройки);
	ОбменСКонтрагентамиКлиентПереопределяемый.ПараметрыЭтапаПодготовкиКДокументообороту(ПереопределенныеПараметры);
	ЗаполнитьЗначенияСвойств(Настройки, ПереопределенныеПараметры);
	Возврат Настройки;
КонецФункции

// Параметры:
//  НаборОбъектовУчета - Массив Из ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО
//  ПараметрыДействия - См. ПараметрыПодготовкиКДокументообороту
Процедура ПодготовитьКДокументообороту(Знач НаборОбъектовУчета, Знач ПараметрыДействия) Экспорт
	
	Если ПараметрыДействия.Настройки.ВыводитьПодтверждениеПриМассовомФормировании Тогда
		ПараметрыДействия.Настройки.ВыводитьПодтверждениеПриМассовомФормировании = Ложь;
		Если НаборОбъектовУчета.Количество() > 1 Тогда
			...
			КонтекстВопроса = Новый Структура;
			КонтекстВопроса.Вставить("НаборОбъектовУчета", НаборОбъектовУчета);
			КонтекстВопроса.Вставить("ПараметрыДействия", ПараметрыДействия);
			ПродолжитьПодготовкуПослеПодтверждения = Новый ОписаниеОповещения("ПродолжитьПодготовкуПослеПодтверждения", ЭтотОбъект, КонтекстВопроса);
			ПоказатьВопрос(ПродолжитьПодготовкуПослеПодтверждения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	Если ПараметрыДействия.Настройки.ПроверятьПроведенностьДокументов Тогда
		ПараметрыДействия.Настройки.ПроверятьПроведенностьДокументов = Ложь;
		Оповещение = Новый ОписаниеОповещения("ПодготовитьКДокументообороту", ЭтотОбъект, ПараметрыДействия);
		ОбщегоНазначенияБЭДКлиент.ВыполнитьПроверкуПроведенияДокументов(НаборОбъектовУчета, Оповещение);
		Возврат;
	КонецЕсли;
	КлючЗамера = "СформироватьПодписатьОтправитьЗавершить";
	ОписаниеЗамера = ОбщегоНазначенияБЭДКлиент.НачатьЗамерДлительнойОперации(КлючЗамера);
	КоличествоДанных = НаборОбъектовУчета.Количество();
	ОповещениеОЗавершенииЗамера = ОбщегоНазначенияБЭДКлиент.ОповещениеОЗавершенииЗамераДлительнойОперации(КлючЗамера, 
		ОписаниеЗамера, КоличествоДанных, ПараметрыДействия.ОбработчикЗавершения);
	СтандартнаяОбработка = Истина;
	ОбменСКонтрагентамиКлиентПереопределяемый.ПриПроверкеГотовностиКДокументообороту(НаборОбъектовУчета, 
		ОповещениеОЗавершенииЗамера, СтандартнаяОбработка);
	
	Если СтандартнаяОбработка = Истина Тогда
		ВыполнитьОбработкуОповещения(ОповещениеОЗавершенииЗамера, НаборОбъектовУчета);
	КонецЕсли;
...
//  КонтекстВопроса - Структура:
//  * НаборОбъектовУчета - См. ПодготовитьКДокументообороту.НаборОбъектовУчета
//  * ПараметрыДействия - См. ПодготовитьКДокументообороту.ПараметрыДействия
Процедура ПродолжитьПодготовкуПослеПодтверждения(Ответ, КонтекстВопроса) Экспорт
	Если Ответ = КодВозвратаДиалога.Да Тогда
		ПодготовитьКДокументообороту(КонтекстВопроса.НаборОбъектовУчета, КонтекстВопроса.ПараметрыДействия);
	Иначе
		ВыполнитьОбработкуОповещения(КонтекстВопроса.ПараметрыДействия.ОбработчикЗавершения, Новый Массив);
	КонецЕсли;
КонецПроцедуры
