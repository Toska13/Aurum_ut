...
Функция ДляУказанияСерийНуженСерверныйВызов(Форма, ПараметрыУказанияСерий, Текст, ТекущиеДанные = Неопределено, ОпределятьРаспоряжение = Истина, ВыдаватьСообщения = Истина) Экспорт
	...
	Иначе
		...
		Если НоменклатураКлиентСервер.ВЭтомСтатусеСерииУказываютсяВТЧТовары(ТекущиеДанные.СтатусУказанияСерий, ПараметрыУказанияСерий)
			...
			Если ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.ПоступлениеТоваровОтХранителя"
				Или ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.ПоступлениеТоваровОтКлиента"
				Или ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.ВыкупТоваровХранителем"
				Или ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.СписаниеОтгруженныхТоваров"
				Или ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.ОприходованиеИзлишковОтгруженныхТоваров"
				...
				Или (ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.РеализацияТоваровУслуг"
					И ПараметрыУказанияСерий.ОперацияДокумента = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПереходПраваСобственности")) Тогда
				ПараметрыПодбораСерий.Вставить("Склад", ИсточникДанныхФормы.Договор);
			ИначеЕсли ПараметрыУказанияСерий.ПоляСвязи.Найти("Склад") <> Неопределено
			...
			Если ПараметрыУказанияСерий.ИмяПоляКонтролируемаяПартия = Неопределено Тогда
				КонтролируемаяПартия = ПредопределенноеЗначение("Справочник.КонтролируемыеПартии.ПустаяСсылка");
			...
			Иначе
				КонтролируемаяПартия = Форма.Объект[ПараметрыУказанияСерий.ИмяПоляКонтролируемаяПартия];
			КонецЕсли;
			ПараметрыПодбораСерий.Вставить("КонтролируемаяПартия", КонтролируемаяПартия);
