...
Процедура УстановитьСостояниеНовостейДляПользователей(
	...
	НаименованиеМетода = "ОбработкаНовостей.УстановитьСостояниеНовостейДляПользователей"; // АПК:1297 Это идентификатор.
	...
	ОбработанныеНовости = Новый Соответствие;
	...
	Если ТипЗнч(СтруктураОтборовЭталон.СписокНовостей) = ТипСписокЗначений Тогда
		...
		Для Каждого ТекущийПользователь Из МассивПользователей Цикл
			...
			Для Каждого ТекущаяНовость Из СтруктураОтборовЭталон.СписокНовостей Цикл
				ОбработанныеНовости.Вставить(ТекущаяНовость.Значение);
		...
		Если ОбработатьРезультатЗапроса = Истина Тогда
			Если НЕ РезультатЗапроса.Пустой() Тогда
				...
				Пока Выборка.Следующий() Цикл
					ОбработанныеНовости.Вставить(Выборка.Новость);
	...
	ПредыдущиеЗначенияСостояний = ОбработкаНовостейКлиентСервер.НовыйОписательЗначенийСостоянияНовости(); // Все значения = Неопределено, мы не считывали значения состояний.
	НовыеЗначенияСостояний = ОбработкаНовостейКлиентСервер.НовыйОписательЗначенийСостоянияНовости();
	ЗаполнитьЗначенияСвойств(НовыеЗначенияСостояний, ЗначенияСостояний, СписокПолейДляЗаполнения);
	МассивНовостей = Новый Массив;
	Для Каждого ТекущаяНовостьКлючЗначение Из ОбработанныеНовости Цикл
		МассивНовостей.Добавить(ТекущаяНовостьКлючЗначение.Ключ);
	КонецЦикла;
	ОбработкаНовостейПереопределяемый.ПослеИзмененияСостоянияНовостей(
		МассивПользователей,
		МассивНовостей,
		ПредыдущиеЗначенияСостояний,
		НовыеЗначенияСостояний);
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		Формат(МассивНовостей.Количество(), "ЧЦ=15; ЧДЦ=; ЧРГ=' '; ЧГ=3,0"));
	ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Сервис и регламент'", КодОсновногоЯзыка), // ИмяСобытия.
		НСтр("ru='Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей'", КодОсновногоЯзыка) , // ИдентификаторШага.
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
...
Процедура ИзменитьПометкуНовостей(Знач МассивНовостей, Знач Пометка = 1) Экспорт
	...
	Иначе
		...
		ПредыдущиеЗначенияСостояний = ОбработкаНовостейКлиентСервер.НовыйОписательЗначенийСостоянияНовости(); // Все значения = Неопределено, мы не считывали значения состояний.
		НовыеЗначенияСостояний = ОбработкаНовостейКлиентСервер.НовыйОписательЗначенийСостоянияНовости(, Пометка);
		// Состояние оповещений не трогаем
		ОбработкаНовостейПереопределяемый.ПослеИзмененияСостоянияНовостей(
...
Процедура ИзменитьПризнакПрочтенностиНовостей(Знач МассивНовостей, Знач ПризнакПрочтенности = Истина) Экспорт
	...
	Иначе
		...
		ПредыдущиеЗначенияСостояний = ОбработкаНовостейКлиентСервер.НовыйОписательЗначенийСостоянияНовости(); // Все значения = Неопределено, мы не считывали значения состояний.
		НовыеЗначенияСостояний = ОбработкаНовостейКлиентСервер.НовыйОписательЗначенийСостоянияНовости(ПризнакПрочтенности);
		// Состояние оповещений не трогаем
		ОбработкаНовостейПереопределяемый.ПослеИзмененияСостоянияНовостей(
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Пользователи.ТекущийПользователь()),
			МассивНовостей,
...
Процедура ИзменитьПризнакУдаленияИзСпискаНовостей(Знач МассивНовостей, Знач ПризнакУдаленияИзСписка = Истина) Экспорт
	...
	Иначе
		...
		ПредыдущиеЗначенияСостояний = ОбработкаНовостейКлиентСервер.НовыйОписательЗначенийСостоянияНовости(); // Все значения = Неопределено, мы не считывали значения состояний.
		НовыеЗначенияСостояний = ОбработкаНовостейКлиентСервер.НовыйОписательЗначенийСостоянияНовости(,,,, ПризнакУдаленияИзСписка);
		// Состояние оповещений не трогаем
		ОбработкаНовостейПереопределяемый.ПослеИзмененияСостоянияНовостей(
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Пользователи.ТекущийПользователь()),
			МассивНовостей,
...
Процедура ИзменитьПризнакПрочитатьПозже(Знач МассивНовостей, Знач ПрочитатьПозже = Истина) Экспорт
	...
	Иначе
		...
		ПредыдущиеЗначенияСостояний = ОбработкаНовостейКлиентСервер.НовыйОписательЗначенийСостоянияНовости(); // Все значения = Неопределено, мы не считывали значения состояний.
		НовыеЗначенияСостояний = ОбработкаНовостейКлиентСервер.НовыйОписательЗначенийСостоянияНовости(,,,,, ПрочитатьПозже);
		// Состояние оповещений не трогаем
		ОбработкаНовостейПереопределяемый.ПослеИзмененияСостоянияНовостей(
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Пользователи.ТекущийПользователь()),
			МассивНовостей,
...
Процедура ИзменитьПризнакОповещенияНовостей(Знач МассивНовостей, Знач ОповещениеВключено = Ложь, Знач ДатаНачалаОповещения = '00010101') Экспорт
	...
	Иначе
		...
		ПредыдущиеЗначенияСостояний = ОбработкаНовостейКлиентСервер.НовыйОписательЗначенийСостоянияНовости(); // Все значения = Неопределено, мы не считывали значения состояний.
		НовыеЗначенияСостояний = ОбработкаНовостейКлиентСервер.НовыйОписательЗначенийСостоянияНовости(,, ОповещениеВключено, ДатаНачалаОповещения); // ДатаНачалаОповещения
		ОбработкаНовостейПереопределяемый.ПослеИзмененияСостоянияНовостей(
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Пользователи.ТекущийПользователь()),
			МассивНовостей,
			ПредыдущиеЗначенияСостояний,
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостей.ПриИзмененииЗначенийОтборовОбщих"; // АПК:1297 Это идентификатор.
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостей.ПриИзмененииЗначенийОтборовДляОбластиДанных"; // АПК:1297 Это идентификатор.
	КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостей.ПриИзмененииЗначенийОтборовПользовательских"; // АПК:1297 Это идентификатор.
	КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
...
Функция РазрешенаРаботаСНовостямиТекущемуПользователю() Экспорт
	...
	Результат = ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостямиТекущемуПользователю();

	// Разрешено "отключать" подсистему, если она была включена. Например, для работы "стартовых помощников".
	// Если же подсистема уже отключена, то запретить включение.

	// Если изначально РазрешенаРаботаСНовостямиТекущемуПользователю = Ложь, то не разрешать включать.
	// Например, если у пользователя нет ролей "ЧтениеНовостей", а ему включат "РазрешенаРаботаСНовостямиТекущемуПользователю",
	//  то будут ошибки доступа к объектам метаданных.

	Если Результат = Ложь Тогда
		Возврат Результат;
	КонецЕсли;

	ИнтеграцияПодсистемБИП.РазрешенаРаботаСНовостямиТекущемуПользователю(Результат);
	ОбработкаНовостейПереопределяемый.РазрешенаРаботаСНовостямиТекущемуПользователю(Результат);
...
//   И доступны нужные роли
//   И это не внешний пользователь.
// Дополнение: на итоговую работу с интернетом также влияет
//  глобальный переключатель ОбщегоНазначения.РазрешенДоступКИнтернетСервисам.
//
// Возвращаемое значение:
...
Процедура ЗаписатьСообщениеВЖурналРегистрации(
	...
	ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
			ИмяСобытия,
			ИдентификаторШага,
			УровеньРегистрации, // АПК:1297 Это идентификатор.
			ОбъектМетаданных,
			Данные,
			Комментарий,
