...
//
Процедура ЗагрузитьЦеныOzon(УчетнаяЗаписьМаркетплейса, ВидыЦен)
...
// 
// Параметры:
//   ВыбранныеЦены - ТаблицаЗначений - таблица видов цен, см. ВыбранныеЦены из УстановкаЦенСервер.ИнициализироватьСтруктуруФормы.
//
...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Форма.ИмяФормы = "Обработка.ЖурналДокументовПродажи.Форма.СписокДокументов" Тогда
		Если СтрНайти(КлючНазначенияФормы, "РаботаСПоставкамиМП") > 0
				Или СтрНайти(КлючНазначенияФормы, "РаботаСПродажамиМП") > 0 Тогда
			
			ГруппаРодитель = Форма.Элементы.Найти("СписокГруппаКомандыСписка");
			ЭлементПеред = Форма.Элементы.Найти("СписокГруппаЗагрузитьИзСкана");
			ИнтеграцияСМаркетплейсамиСервер.ДобавитьПоискЧерезСервис(Форма, ГруппаРодитель, ЭлементПеред);
		КонецЕсли;
	КонецЕсли;
