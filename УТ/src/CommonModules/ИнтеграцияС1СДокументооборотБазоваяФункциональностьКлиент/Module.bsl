...
Процедура ВыбратьЗначениеРеквизитаДО(Форма, Элемент, Типы, ПараметрыВыбораЗначения) Экспорт
	...
	Иначе
		...
		Если ТипЗнч(Типы) = Тип("Массив") Тогда
			...
			Для Каждого ТипXDTO Из Типы Цикл
				СписокТипов.Добавить(ТипXDTO, ПредставлениеТипаОбъектаXDTO(ТипXDTO, Настройки));
			КонецЦикла;
		Иначе
			СписокТипов = Типы;
		КонецЕсли;
...
//   ДополнительныйРеквизитИС - ПланВидовХарактеристикСсылка.ДополнительныеРеквизитыИСведения - передается, если
//     требуется выбрать значение дополнительного реквизита.
//   Оповещение - ОписаниеОповещения - вызывает процедуру формы, в которую передается выбранное значение.
//
Процедура ВыбратьЗначениеРеквизитаИС(Форма, Элемент, Тип, Представление, ЗначениеРеквизита, ДополнительныйРеквизитИС,
		Оповещение) Экспорт
	...
	ПараметрыВыбратьЗначение.Вставить("ЗначениеРеквизита", ЗначениеРеквизита);
		ПараметрыВыбратьЗначение.Вставить("ДополнительныйРеквизитИС", ДополнительныйРеквизитИС);
...
//
Функция ПредставлениеТипаОбъектаXDTO(ТипXDTO, Настройки = Неопределено, МножественноеЧисло = Ложь,
		ДляФормыВыбора = Ложь) Экспорт
	...
	Если Настройки.ИспользоватьИнтеграциюДО2 Тогда
		МодульИнтеграцияС1СДокументооборотКлиентСервер = ОбщегоНазначенияКлиент.ОбщийМодуль(
	...
	ИначеЕсли Настройки.ИспользоватьИнтеграциюДО3 Тогда
		МодульИнтеграцияС1СДокументооборот3КлиентСервер = ОбщегоНазначенияКлиент.ОбщийМодуль(
...
// Открывает форму создания нового объекта ИС, заполненную по переданным данным.
// Если пользователь интерактивно запишет объект ИС, связь с исходным объектом ДО будет установлена автоматически в
// обработчике оповещения о закрытии формы.
//
// Параметры:
...
Процедура СоздатьИнтегрированныйОбъектИзДанныхЗаполнения(ДанныеЗаполнения, Форма = Неопределено) Экспорт
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервераПереопределяемый.ПередОткрытиемФормыСозданияНовогоОбъектаИСНаОснованииОбъектаДО(
...
// Открывает форму списка файлов, хранящихся в базе ИС.
// Аналог метода БСП РаботаСФайламиКлиент.ОткрытьФормуСпискаФайлов, который принудительно показывает файлы,
// хранящихся в базе ИС.
//
// Параметры:
...
//   ПараметрыФормы - см. РаботаСФайламиКлиент.ПараметрыОткрытияФормыСпискаФайлов
//   ВладелецФормы - ФормаКлиентскогоПриложения, Неопределено - форма, из которой вызвана команда.
//   ОповещениеОЗакрытии - ОписаниеОповещения - содержит описание процедуры, которая будет вызвана при закрытии формы.
//
Процедура ОткрытьФормуСпискаФайловБСП(Знач ВладелецФайлов, Знач ПараметрыФормы = Неопределено,
	...
	Если ПараметрыФормы = Неопределено Тогда
		ВерсияБСП = СтандартныеПодсистемыВызовСервера.ВерсияБиблиотеки();
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБСП, "3.1.11.142") > 0 Тогда
			МодульРаботаСФайламиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("РаботаСФайламиКлиент");
			ПараметрыФормы = МодульРаботаСФайламиКлиент.ПараметрыОткрытияФормыСпискаФайлов(ВладелецФайлов);
		Иначе
			ПараметрыФормы = Новый Структура("ВладелецФайла", ВладелецФайлов);
		КонецЕсли;
	КонецЕсли;
	
	ИмяФормы = "Обработка.РаботаСФайлами.Форма.ПрисоединенныеФайлы";
	ОткрытьФорму(ИмяФормы, ПараметрыФормы, ВладелецФормы,,,, ОповещениеОЗакрытии);
...
// Проверяет возможность установки признака является скан-копией оригинала.
//
// Параметры:
//   ШаблонID - Строка - идентификатор шаблона файла.
//   Расширение - Строка - расширение файла.
//   СписокРасширенийСканКопийОригиналов - Строка - список возможных расширений скан-копий оригиналов.
//
// Возвращаемое значение:
//   Булево
//
Асинх Функция УстановкаПризнакаЯвляетсяОригиналомВозможна(Знач ШаблонID, Знач Расширение,
		Знач СписокРасширенийСканКопийОригиналов) Экспорт
	
	Если ЗначениеЗаполнено(ШаблонID) Тогда
...
//     * ОбъектID - Строка
//
Функция ВыделенныеПодписи(ВыделенныеСтроки, ДеревоПодписей, ОбъектID = Неопределено) Экспорт
	
	Результат = Новый Массив;
	
	Для Каждого Элемент Из ВыделенныеСтроки Цикл
		ДанныеСтроки = ДеревоПодписей.НайтиПоИдентификатору(Элемент);
		Если ЭтоПодпись(ДанныеСтроки) Тогда
			ДобавитьПодписьВМассив(ДанныеСтроки, Результат, ДанныеСтроки, ОбъектID);
		ИначеЕсли ТипЗнч(ДанныеСтроки) = Тип("ДанныеФормыЭлементДерева") Тогда
			Для Каждого Подпись Из ДанныеСтроки.ПолучитьЭлементы() Цикл
				ДобавитьПодписьВМассив(ДанныеСтроки, Результат, Подпись, ДанныеСтроки.ОбъектID)
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
Процедура ИнформироватьОПодписании(Представление, ДобавленныеПодписиОбъектов, ДокументID,
	...
	Для Каждого Элемент Из ДобавленныеПодписиОбъектов Цикл
		...
		ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
		ДействиеНадФайлом.ПодписаниеДокументаСФайлами = Истина;
		ДействиеНадФайлом.ИнформироватьФормуФайла = Истина;
		ДействиеНадФайлом.ИнформироватьФормуСпискаФайлов = Истина;
		ИнформироватьОбИзмененииФайла(
			ДействиеНадФайлом,
...
// Возвращает общий модуль электронной подписи для клиента, если существует соответствующая подсистема, и включена
// настройка интеграции, разрешающая интегрированной системе заниматься созданием электронных
// подписей для объектов 1С:Документооборота.
//
// Параметры:
//   ДляСоздания - Булево - признак того, что модуль запрашивается для создания подписей.
//
// Возвращаемое значение:
//   ОбщийМодуль, Неопределено - общий модуль электронной подписи или Неопределено при отсутствии
//     подсистемы или выключенной настройке.
//
Функция МодульЭлектроннаяПодписьКлиент(ДляСоздания = Ложь) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		Если Не ДляСоздания
				Или ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентПовтИсп.СоздаватьЭПДляОбъектов1СДокументооборотаСредствамиИС() Тогда
			МодульЭлектроннаяПодписьКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ЭлектроннаяПодписьКлиент");
		Иначе
			ПоказатьПредупреждение(, НСтр("ru = 'Возможность создания подписей выключена в настройках интеграции с 1С:Документооборотом.
				|
				|Нужно включить настройку, либо подписать объект на стороне 1С:Документооборота.'"));
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(, НСтр("ru = 'Отсутствует стандартная подсистема ""Электронная подпись"".'"));
	КонецЕсли;
	
	Возврат МодульЭлектроннаяПодписьКлиент;
...
// Формирует подпись для объекта 1С:Документооборота и всех его вложенных файлов.
//
// Параметры:
...
//
Процедура НачатьФормированиеПодписиОбъекта(ОбъектДО, Файлы, АдресДвоичныхДанныхОбъекта,
		ОписаниеОповещенияОЗакрытии) Экспорт
	
	МодульЭлектроннаяПодписьКлиент = МодульЭлектроннаяПодписьКлиент(Истина);
	...
	Для Каждого ОбъектДляПодписи Из МассивОбъектовДляПодписи Цикл
		...
		Если ОбъектДляПодписи.ОбъектСсылкаДляПодписи = ОбъектДО.ID Тогда
			ДанныеДляПодписи.Вставить("Представление",
				Новый Структура("Значение, Представление", ОбъектДО.Представление, ОбъектДО.Представление));
		ИначеЕсли ОбъектДляПодписи.ОбъектТип = "DMFile" Тогда
			Строки = Новый Массив;
			Для Каждого Файл Из Файлы Цикл
				Если Файл.ID = ОбъектДляПодписи.ОбъектСсылкаДляПодписи Тогда
					ПредставлениеФайла = СтрШаблон(НСтр("ru = 'Файл: %1'"), Файл.Наименование);
					Строки.Добавить(Файл);
				КонецЕсли
			КонецЦикла;
			...
			Иначе
				ДанныеДляПодписи.Вставить("Представление", ПредставлениеТипаОбъектаXDTO(ОбъектДляПодписи.ОбъектТип));
			КонецЕсли;
		Иначе
			ДанныеДляПодписи.Вставить("Представление", ПредставлениеТипаОбъектаXDTO(ОбъектДляПодписи.ОбъектТип));
		КонецЕсли;
	...
	ОписаниеДанных.Вставить("Представление", ОбъектДО.Наименование);
	ОписаниеДанных.Вставить("ПредставлениеНабора", НСтр("ru = 'Объекты (%1)'"));
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("Форма", ОписаниеОповещенияОЗакрытии.Модуль);
...
Процедура ОткрытьПодпись(ТаблицаПодписейТекущиеДанные) Экспорт
	...
	Если ТаблицаПодписейТекущиеДанные = Неопределено Или Не ЗначениеЗаполнено(ТаблицаПодписейТекущиеДанные.Объект) Тогда
		Возврат;
	КонецЕсли;
	
	МодульЭлектроннаяПодписьКлиент = МодульЭлектроннаяПодписьКлиент();
	Если МодульЭлектроннаяПодписьКлиент = Неопределено Тогда
		Возврат;
	КонецЕсли;
	МодульЭлектроннаяПодписьСлужебныйКлиентСервер = ОбщегоНазначенияКлиент.ОбщийМодуль("ЭлектроннаяПодписьСлужебныйКлиентСервер");
	
	СвойстваПодписи = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.СтруктураСтрокиДереваПодписей(
		ТаблицаПодписейТекущиеДанные);
	Если Не ЗначениеЗаполнено(СвойстваПодписи.ДатаПроверкиПодписи) Тогда
		СвойстваПодписи.ДатаПроверкиПодписи = ТекущаяДата(); // Использование оправдано: фиксируется дата-время на клиенте.
	КонецЕсли;
	Если Не ЗначениеЗаполнено(СвойстваПодписи.ТипПодписи) Тогда
		Попытка
			СвойстваИзДвоичныхДанных = МодульЭлектроннаяПодписьСлужебныйКлиентСервер.СвойстваПодписиИзДвоичныхДанных(
				СвойстваПодписи.АдресПодписи,
				ОбщегоНазначенияКлиент.ДатаСеанса() - ОбщегоНазначенияКлиент.ДатаУниверсальная());
			СвойстваПодписи.ТипПодписи = СвойстваИзДвоичныхДанных.ТипПодписи;
		Исключение
			СвойстваПодписи.ТипПодписи = Неопределено;
		КонецПопытки;
	КонецЕсли;
	
	МодульЭлектроннаяПодписьКлиент.ОткрытьПодпись(СвойстваПодписи);
...
//   ТаблицаПодписей - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ТаблицаПодписей
//   АдресДвоичныхДанныхОбъекта - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.АдресДвоичныхДанныхОбъекта
//   ОписаниеОповещения - ОписаниеОповещения - обработчик, вызываемый после проверки подписей.
//   ВыделенныеПодписи - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыделенныеПодписи
//
Процедура ПроверитьПодписи(ТаблицаПодписей, АдресДвоичныхДанныхОбъекта = Неопределено,
		ОписаниеОповещения = Неопределено, ВыделенныеПодписи = Неопределено) Экспорт
	...
	ОбновитьАдресДвоичныхДанныхОбъекта(ТаблицаПодписей, ВыделенныеПодписи, АдресДвоичныхДанныхОбъекта);
	...
	Для Каждого ДанныеСтроки Из ТаблицаПодписей Цикл
		...
		Если НастройкиЭП.ПроверятьЭлектронныеПодписиНаСервере Тогда
			...
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ПроверитьПодписи(
				...
				АдресДвоичныхДанныхОбъекта,
				Программа);
			ЗаполнитьЗначенияСвойств(ДанныеСтроки, ТаблицыПодписейПослеПроверки[0],, "ТипПодписи");
			СоответствиеТиповПодписиКриптографииПеречислениюXDTO =
				ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.СоответствиеТиповПодписиКриптографииПеречислениюXDTO();
			ДанныеСтроки.ТипПодписи = СоответствиеТиповПодписиКриптографииПеречислениюXDTO[
				ТаблицыПодписейПослеПроверки[0].ТипПодписи];
			
		Иначе
			
			Параметры = Новый Структура;
			Параметры.Вставить("ТаблицаПодписей", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДанныеСтроки));
			Параметры.Вставить("АдресДвоичныхДанныхОбъекта", АдресДвоичныхДанныхОбъекта);
...
Процедура СохранитьПодпись(ТаблицаПодписейТекущиеДанные) Экспорт
	...
	Если Не ЭтоПодпись(ТаблицаПодписейТекущиеДанные) Тогда
		Возврат;
	КонецЕсли;
...
//
// Параметры:
//   УдаленныеСтроки - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыделенныеПодписи
//   ТаблицаПодписей - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ТаблицаПодписей
...
Асинх Процедура УдалитьПодписи(Знач УдаленныеСтроки, Знач ТаблицаПодписей, Знач ВладелецФайла,
	...
	ФайлыДляОповещения = ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.УдалитьПодписи(
		УдаленныеСтроки,
		ТаблицаПодписей);
	...
	Для Каждого ЭлементСоответствия Из ФайлыДляОповещения Цикл
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Удалена подпись файла ""%1""'"), ЭлементСоответствия.Значение);
		ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
		ДействиеНадФайлом.УдалениеЭПФайла = Истина;
		ДействиеНадФайлом.ИнформироватьФормуФайла = Истина;
		ДействиеНадФайлом.ИнформироватьФормуСпискаФайлов = Истина;
		ИнформироватьОбИзмененииФайла(
			ДействиеНадФайлом,
...
// Завершает проверку подписей, устанавливая их статус и дату проверки.
//
// Параметры:
//   ДеревоПодписей - ДанныеФормыДерево - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.СтруктураСтрокиДереваПодписей
//   ТаблицаПодписей - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ТаблицаПодписей
//
Процедура УстановитьСтатусПодписей(ДеревоПодписей, ТаблицаПодписей) Экспорт
	...
	Для Каждого ВладелецПодписи Из ДеревоПодписей.ПолучитьЭлементы() Цикл
		...
		Для Каждого Подпись Из ВладелецПодписи.ПолучитьЭлементы() Цикл
			...
			СтрокаРезультатаПроверкиЭП = СтрокаРезультатаПроверкиЭП(Подпись, ТаблицаПодписей);
			...
			ПозицияРазделителя = СтрНайти(Подпись.КомуВыданСертификатИСтатус, " (", НаправлениеПоиска.СКонца);
			Если ПозицияРазделителя > 0 И Сред(Подпись.КомуВыданСертификатИСтатус, ПозицияРазделителя + 4, 1) = "." Тогда
				Подпись.КомуВыданСертификатИСтатус = Лев(Подпись.КомуВыданСертификатИСтатус, ПозицияРазделителя - 1);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(Подпись.ДатаПроверкиПодписи) Тогда
				Подпись.КомуВыданСертификатИСтатус = СтрШаблон("%1 (%2)",
					Подпись.КомуВыданСертификатИСтатус,
					Формат(Подпись.ДатаПроверкиПодписи, НСтр("ru = 'ДФ=''dd.MM.yyyy HH:mm'''")));
			КонецЕсли;
...
// Определяет, является ли строка дерева подписей строкой, содержащей данные о подписи.
//
// Параметры:
//   СтрокаДереваПодписей - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.СтруктураСтрокиДереваПодписей
//
// Возвращаемое значение:
//   Булево
//
Функция ЭтоПодпись(СтрокаДереваПодписей) Экспорт
	
	Если СтрокаДереваПодписей = Неопределено Тогда
		Возврат Ложь;
	Иначе
		Возврат ЭтоАдресВременногоХранилища(СтрокаДереваПодписей.АдресПодписи);
	КонецЕсли;
...
Асинх Процедура ПолучениеФайлаПослеВыбораИсточника(Знач Результат, Знач Параметры) Экспорт
	...
	Если Результат = "ВзятьИзДокументооборота" Тогда
		...
		Иначе
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Неизвестный тип файла: %1'"), Параметры.ТекущийФайл.Тип);
		КонецЕсли;
		ПроверитьШифрованиеФайла(Параметры.ТекущийФайл);
		Параметры.Вставить("АдресВременногоХранилищаФайла", Адрес);
...
Асинх Процедура СоздатьФайлыСДискаАсинх(Знач ВыбранныеФайлы, Знач Параметры) Экспорт
	...
	НастройкиДО = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентПовтИсп.ПолучитьНастройки();
	СписокРасширенийСканКопий = НастройкиДО.СписокРасширенийСканКопийОригиналов;
	ЭтоРасширениеСканКопии = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ЭтоРасширениеСканКопии(
		Параметры.Файл.Расширение,
		СписокРасширенийСканКопий);
	Параметры.ЯвляетсяСканКопией = Параметры.ЯвляетсяСканКопией Или ЭтоРасширениеСканКопии;
	
	Если Параметры.ЯвляетсяСканКопией И Не ПустаяСтрока(СписокРасширенийСканКопий) И Не ЭтоРасширениеСканКопии Тогда
		ТекстПредупреждения = СтрШаблон(
...
Процедура НачатьДобавлениеЭПИзФайлаЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	КонецЕсли;
	ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
	ДействиеНадФайлом.ДобавлениеЭПФайла = Истина;
	ДействиеНадФайлом.ИнформироватьФормуФайла = Истина;
	ДействиеНадФайлом.ИнформироватьФормуСпискаФайлов = Истина;
	ИнформироватьОбИзмененииФайла(
		ДействиеНадФайлом,
...
Процедура ПодписатьФайлЗавершение(ОписаниеДанных, СвойстваФайла) Экспорт
	...
	Если ТипЗнч(ОписаниеДанных) = Тип("Структура") И ОписаниеДанных.Свойство("СвойстваПодписи", СвойстваПодписи) Тогда
		...
		МассивПодписей = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
			ДанныеПодписи(
				СвойстваПодписи.Подпись,
				СвойстваПодписи.Сертификат,
				СвойстваПодписи));
	...
	Для Каждого ДанныеСтроки Из Параметры.ТаблицаПодписей Цикл
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ПодписьВыделена(
			...
			ПроверитьОднуПодпись(
				...
				Параметры.АдресДвоичныхДанныхОбъекта);
		КонецЕсли;
	КонецЦикла;
...
// Обработчик оповещения "ВыбратьЗначениеРеквизитаДОЗавершение".
//
// Параметры:
//   ЗначениеТипа - Тип
...
Процедура ВыбратьЗначениеРеквизитаИСЗавершениеВыбораТипа(ЗначениеТипа, ПараметрыВыбратьЗначение) Экспорт
	...
	Иначе
		...
		Если ИмяОбъекта = "Справочник.ЗначенияСвойствОбъектов" Тогда
			Отбор = Новый Структура("Владелец", ПараметрыВыбратьЗначение.ДополнительныйРеквизитИС);
...
Процедура СоздатьИнтегрированныйОбъектЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	ДобавитьСвязьВДО = Истина;
	ЗарегистрироватьДляОбмена = Истина;
	...
	Если Не Ждать РасширениеРаботыСФайламиПодключеноАсинх() Тогда
		...
		ИначеЕсли Параметры.ТекущийФайл.Тип = "DMFile" Тогда
			Адрес = ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ПолучитьФайлИПоместитьВХранилище(
				...
				Параметры.ТекущийФайл); // Использование оправдано: фиксируется дата-время на клиенте.
		ИначеЕсли Параметры.ТекущийФайл.Тип = "DMFileVersion" Тогда
			Адрес = ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ПолучитьВерсиюФайлаИПоместитьВХранилище(
				Параметры.ТекущийФайл.ID,
				ИспользоватьВизуализациюШтампаЭП,
...
Процедура ПроверитьШифрованиеФайла(ДанныеФайла)
	
	Если ДанныеФайла.Зашифрован Тогда
		ВызватьИсключение(
			НСтр("ru = 'Файл зашифрован. Расшифровка файла доступна только на стороне 1С:Документооборот.'"));
	КонецЕсли;
...
Процедура ОбновитьАдресДвоичныхДанныхОбъекта(ТаблицаПодписей, ВыделенныеПодписи, АдресДвоичныхДанныхОбъекта)
	
	Если АдресДвоичныхДанныхОбъекта = Неопределено Тогда
		АдресДвоичныхДанныхОбъекта =
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.АдресДвоичныхДанныхОбъекта(
				Новый УникальныйИдентификатор);
	КонецЕсли;
	
	ФайлыБезДвоичныхДанных = Новый Массив;
	Для Каждого ДанныеСтроки Из ТаблицаПодписей Цикл
		Если Не ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ПодписьВыделена(
				ВыделенныеПодписи, ДанныеСтроки) Тогда
			Продолжить;
		...
		Если Не ЭтоАдресВременногоХранилища(АдресДвоичныхДанныхФайла)
				И ФайлыБезДвоичныхДанных.Найти(ДанныеСтроки.ОбъектID) = Неопределено Тогда
			ФайлыБезДвоичныхДанных.Добавить(ДанныеСтроки.ОбъектID);
		КонецЕсли;
	КонецЦикла;
	
	Если ФайлыБезДвоичныхДанных.Количество() > 0 Тогда
		ДвоичныеДанныеФайлов = ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ДвоичныеДанныеФайлов(
			ФайлыБезДвоичныхДанных,
			Ложь);
		Для Каждого ДвоичныеДанныеФайла Из ДвоичныеДанныеФайлов Цикл
			АдресДвоичныхДанныхОбъекта.АдресаДвоичныхДанныхФайлов[ДвоичныеДанныеФайла.Ключ] =
				ПоместитьВоВременноеХранилище(
					ДвоичныеДанныеФайла.Значение,
					АдресДвоичныхДанныхОбъекта.УникальныйИдентификаторФормы);
		КонецЦикла;
	КонецЕсли;
