...
Функция ДанныеПробитогоФискальногоЧекаПоДокументу(ДокументСсылка, ТипРасчета = Неопределено) Экспорт
	...
	Если ДанныеПробитогоФискальногоЧекаПоДокументу = Неопределено Тогда
		Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЧекККМ") Тогда
			ДанныеПробитогоФискальногоЧекаПоДокументу = ПодключаемоеОборудованиеУТВызовСервера.ДанныеФискальнойОперацииСУчетомЧекаККМКоррекции(ДокументСсылка);
		Иначе
...
// Проверяет данные оптового документа перед выполнением фискальной операции на форме обработки ПредпросмотрЧека.
// Перед проверкой документ должен быть проведен и флаг модифицированности формы должен быть снят.
//
// Параметры:
//  Объект - ДокументОбъект - Объект формы документа
//  ПараметрыПроверки - Структура Из КлючИЗначение - Обязательный состав свойств для передачи данных из клиентского контекста:
//   *ИмяФормы - Строка
//   *ТипРасчета - ПеречислениеСсылка.ТипыРасчетаДенежнымиСредствами, Неопределено - Используется для документа ВзаимозачетЗадолженности, по умолчанию Неопределено.
//   *ИмяКомандыПробитияЧека - Строка - По умолчанию пустая строка.
//
// Возвращаемое значение:
//  Булево - 
//
Функция ПроверкаДанныхПередФискальнойОперацией(Объект, ПараметрыПроверки) Экспорт
	...
	ИмяФормы = ПараметрыПроверки.ИмяФормы;
	ТипРасчета = ПараметрыПроверки.ТипРасчета;
	ИмяКомандыПробитияЧека = ПараметрыПроверки.ИмяКомандыПробитияЧека;
	
	ТекстСообщения = "";
	Если ПробитФискальныйЧекПоДокументу(Объект.Ссылка, ТипРасчета) Тогда
		Результат = Ложь;
		ТекстОшибки = НСтр("ru = 'По документу уже был пробит чек'");
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
	ИначеЕсли Не ОбъектыРасчетовВДокументеОплатыВведеныКорректно(Объект.Ссылка, ТекстСообщения, ИмяКомандыПробитияЧека) Тогда
		Результат = Ложь;
		ТекстОшибки = СтрШаблон(НСтр("ru = '%1'"), ТекстСообщения);
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
	Иначе
		// Показываем все найденные ошибки при проверке данных
		Результат = ШтрихкодыУпаковокПоДокументамЗаполнены(Объект, ИмяФормы) И Результат;
		Результат = РозничныеПродажиЛокализация.ПроверкаДанныхМаркированногоТовараДокумента(Объект) И Результат;
	КонецЕсли;
...
// Выполняет проверку табличной части ШтрихкодыУпаковок документах продажи, указанных в расшифровке платежа оптовых документов оплаты.
//
// Параметры:
//  Объект - ДокументОбъект - Объект формы документа
//  ИмяФормы - Строка - Имя формы, из контекста которой сделан вызов текущего метода.
//
// Возвращаемое значение:
//  Булево - 
//
Функция ШтрихкодыУпаковокПоДокументамЗаполнены(Объект, ИмяФормы)
	
	Результат = Истина;
	
	//++ Локализация
	Если Не (ИмяФормы = "Документ.ПриходныйКассовыйОрдер.Форма.ФормаДокумента"
		Или ИмяФормы = "Документ.ОперацияПоПлатежнойКарте.Форма.ФормаДокумента"
		Или ИмяФормы = "Документ.ПоступлениеБезналичныхДенежныхСредств.Форма.ФормаДокумента") Тогда
	...
	ФормированиеФискальныхЧековСерверПереопределяемый
		.ДокументыЕдиногоЧекаЗаполнитьТаблицу(Объект.Ссылка, ДокументыЕдиногоЧека, "");
	...
	Для Каждого СтрокаДокументыЕдиногоЧека Из ДокументыЕдиногоЧека Цикл
		...
		Если ТипЗнч(СтрокаДокументыЕдиногоЧека.Документ) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			ДокументыРеализации.Добавить(СтрокаДокументыЕдиногоЧека.Документ);
		КонецЕсли;
		
	КонецЦикла;
	
	Если ДокументыРеализации.Количество() > 0
		И Не ФормированиеФискальныхЧековСерверПереопределяемый.ШтрихкодыУпаковокПоДокументамЗаполнены(ДокументыРеализации) Тогда
		Результат = Ложь;
