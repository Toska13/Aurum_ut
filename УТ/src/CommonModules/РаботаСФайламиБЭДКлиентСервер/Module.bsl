...
#Область ДопустимыеТипыФайлов

// Проверяет помещенные файлы и исключает недопустимые.
// 
// Параметры:
...
//
Функция ИсключитьНедопустимыеФайлы(ПомещенныеФайлы) Экспорт
	
	ПроверяемыеФайлы = Новый Массив(); // Массив из Строка
	Для Каждого ПомещенныйФайл Из ПомещенныеФайлы Цикл
		ПроверяемыеФайлы.Добавить(ПомещенныйФайл.ПолноеИмя);
	КонецЦикла;
	
	ФайлыНедопустимыхТипов = НайтиФайлыНедопустимыхТипов(ПроверяемыеФайлы);
	Если НЕ ЗначениеЗаполнено(ФайлыНедопустимыхТипов) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	КоличествоПомещаемыхФайлов = ПомещенныеФайлы.Количество();
	
	Для Счетчик = 1 По КоличествоПомещаемыхФайлов Цикл
		
		Индекс = КоличествоПомещаемыхФайлов - Счетчик;
		ПомещенныйФайл = ПомещенныеФайлы[Индекс];
		Если ФайлыНедопустимыхТипов.Найти(ПомещенныйФайл.ПолноеИмя) <> Неопределено Тогда
			ПомещенныеФайлы.Удалить(Индекс);
		КонецЕсли;
		
	КонецЦикла;
	
	Результат = Новый Структура("ТекстПредупреждения, ТекстЗаголовка, СписокФайлов", "", "", Новый Массив());
	Результат.ТекстЗаголовка = НСтр("ru = 'Обнаружен небезопасный файл'");
	КоличествоФайлов = ФайлыНедопустимыхТипов.Количество();
	Если КоличествоФайлов > 1 Тогда
		Результат.ТекстЗаголовка = СтрШаблон(
			НСтр("ru = 'Обнаружены небезопасные файлы (%1)'"),
			Формат(КоличествоФайлов, "ЧГ=;"));
	КонецЕсли;
	ТекстПояснения = НСтр("ru = 'Файлы некоторых типов нельзя отправлять по ЭДО.
		|При необходимости выберите другие файлы и повторите отправку.'");
	Ссылка = "https://edo.1c.ru/handbook/rabota-s-elektronnymi-dokumentami/kakie-dokumenty-mozhno-zagruzhat/";
	
	МассивФайлов = Новый Массив();
	РазмерСпискаНедопустимыхФайловВУведомлении = 5;
	РазмерСписка = Мин(РазмерСпискаНедопустимыхФайловВУведомлении, КоличествоФайлов);
	ОстатокСписка = Макс(0, КоличествоФайлов - РазмерСписка);
	
	Для Индекс = 1 По РазмерСписка Цикл
		МассивФайлов.Добавить(ФайлыНедопустимыхТипов[Индекс - 1]);
	КонецЦикла;
	
	Если ОстатокСписка > 0 Тогда
		ТекстОстатка = СтрШаблон(НСтр("ru = 'и еще %1'"), ОстатокСписка);
		МассивФайлов.Добавить(ТекстОстатка);
		Результат.СписокФайлов = ФайлыНедопустимыхТипов;
	КонецЕсли;
	
	СписокФайлов = СтрСоединить(МассивФайлов, Символы.ПС);
	НадписьСсылки = НСтр("ru = 'Узнать больше про действующие ограничения можно здесь'");
	ТекстПредупреждения = СтрШаблон("%1
		|
		|%2
		|<a href=""%3"">%4</a>.", СписокФайлов, ТекстПояснения, Ссылка, НадписьСсылки);
	
	Результат.ТекстПредупреждения = Новый ФорматированнаяСтрока(ТекстПредупреждения);
	Возврат Результат;
...
//  * ПустоеИмяФайла - Булево
//  * ПустоеРасширение - Булево
...
Функция ПроверитьИмяФайлаСУчетомКодировки(Знач ИмяФайла, Кодировка = "windows-1251") Экспорт
	...
	ИмяФайлаПослеКорректировки = 
		ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(ИмяФайлаПослеКорректировки, "_");
	ИмяФайлаПослеКорректировки = СтрЗаменить(ИмяФайлаПослеКорректировки, "и" + Символ(774), "й");
	ИмяФайлаПослеКорректировки = СтрЗаменить(ИмяФайлаПослеКорректировки, "е" + Символ(776), "е");
	...
	СтруктураИмениФайла = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ИмяФайлаПослеКорректировки);
	ПустоеИмяФайла = ПустаяСтрока(СтруктураИмениФайла.ИмяБезРасширения);
	ПустоеРасширение = ПустаяСтрока(СтруктураИмениФайла.Расширение);
	
	Результат = Новый Структура;
	Результат.Вставить("ИмеютсяНекорректныеСимволы", ИмеютсяНекорректныеСимволы);
	Результат.Вставить("ПустоеИмяФайла", ПустоеИмяФайла);
	Результат.Вставить("ПустоеРасширение", ПустоеРасширение);
...
Функция ВидОшибкиРаботаСФайлами() Экспорт
	...
	ОписаниеРешения = НСтр("ru = '<a href = ""%1"">Выполните</a> диагностику работы с файлами'");
...
// Проверяет недопустимые к отправке файлы по их типам.
// 
// Параметры:
//  ИменаФайлов - Массив из Строка
//
// Возвращаемое значение:
//  Массив из Строка
//
Функция НайтиФайлыНедопустимыхТипов(ИменаФайлов)
	
	Результат = Новый Массив();
	ДопустимыеТипы = ДопустимыеТипыОтправляемыхФайлов();
	
	Для Каждого ИмяФайла Из ИменаФайлов Цикл
		Файл = Новый Файл(ИмяФайла);
		Если НЕ ЗначениеЗаполнено(Файл.Расширение)
			ИЛИ НЕ ДопустимыеТипы[НРег(СтрЗаменить(Файл.Расширение, ".", ""))] = Ложь Тогда
...
// Допустимые типы отправляемых файлов.
// 
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//  * Ключ - Строка - Расширение файла
//  * Значение - Булево - Разрешено отправлять только внутри архива
//
Функция ДопустимыеТипыОтправляемыхФайлов()
	
	Результат = Новый Соответствие();
	РасширенияСтрокой = "
		|xml,pdf,doc,docx,xls,xlsx,odt,ods,rtf,txt,csv,jpg,jpeg,jfif,tif,
		|tiff,png,bmp,webp,json,gsfx,bmpn,sig,sgn,p7s,gzip,zip,rar,7z";
	Разделители = "
		|,";
	Расширения = СтрРазделить(РасширенияСтрокой, Разделители, Ложь);
	Для Каждого Расширение Из Расширения Цикл
		Результат.Вставить(Расширение, Ложь);
	КонецЦикла;
