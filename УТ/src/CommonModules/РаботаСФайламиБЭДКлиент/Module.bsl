...
//  Булево
//
Функция ФайлМожноОтправитьПоЭДО(ПолноеИмяФайла) Экспорт
	
	ОписаниеФайла = Новый Структура("ПолноеИмя", ПолноеИмяФайла);
	ОписанияФайлов = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОписаниеФайла);
	РезультатОперации = РаботаСФайламиБЭДКлиентСервер.ИсключитьНедопустимыеФайлы(ОписанияФайлов);
	ПоказатьПредупреждениеОНедопустимыхФайлах(РезультатОперации);
	Возврат ЗначениеЗаполнено(ОписанияФайлов);
	
КонецФункции

// Показывает информацию о недопустимых файлах.
// 
// Параметры:
//  ДанныеПредупреждения - см. РаботаСФайламиБЭДКлиентСервер.ИсключитьНедопустимыеФайлы
//  БлокироватьИнтерфейс - Булево
// 
Процедура ПоказатьПредупреждениеОНедопустимыхФайлах(ДанныеПредупреждения, БлокироватьИнтерфейс = Ложь) Экспорт
	
	Если НЕ ЗначениеЗаполнено(ДанныеПредупреждения) Тогда
		Возврат;
	КонецЕсли;
	
	Оповещение = Неопределено;
	Кнопки = Новый СписокЗначений();
	Кнопки.Добавить(КодВозвратаДиалога.ОК, НСтр("ru = 'ОК'"));
	
	Если ЗначениеЗаполнено(ДанныеПредупреждения.СписокФайлов) Тогда
		
		ЗаголовокКнопки = СтрШаблон(НСтр("ru = 'Список файлов (%1)'"), ДанныеПредупреждения.СписокФайлов.Количество());
		Кнопки.Добавить(КодВозвратаДиалога.Да, ЗаголовокКнопки);
		Оповещение = Новый ОписаниеОповещения("ПоказатьПредупреждениеОНедопустимыхФайлахЗавершение",
			ЭтотОбъект, Новый Структура("СпискФайлов", ДанныеПредупреждения.СписокФайлов));
		
	КонецЕсли;
	
	ПараметрыВопроса = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
	ПараметрыВопроса.Заголовок = ДанныеПредупреждения.ТекстЗаголовка;
	ПараметрыВопроса.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
	ПараметрыВопроса.Картинка = БиблиотекаКартинок.ДиалогВосклицание;
	ПараметрыВопроса.БлокироватьВесьИнтерфейс = БлокироватьИнтерфейс;
	ТекстПредупреждения = СтроковыеФункцииКлиент.ФорматированнаяСтрока(ДанныеПредупреждения.ТекстПредупреждения);
...
// Обработчик оповещения о недопустимых файлах.
// 
// Параметры:
...
//
Процедура ПоказатьПредупреждениеОНедопустимыхФайлахЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если НЕ ЗначениеЗаполнено(Результат) ИЛИ Результат.Значение <> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли;
	
	Текст = Новый ТекстовыйДокумент();
	Текст.ДобавитьСтроку(СтрСоединить(ДополнительныеПараметры.СпискФайлов, Символы.ПС));
	Текст.Показать(НСтр("ru = 'Перечень обнаруженных небезопасных файлов'"));
