...
#Область ИспользоватьРозничныеПродажиТабачнойПродукцииТолькоПоМРЦ

// Контроль интерактивного редактирования колонок табличной части товаров (при начале редактирования) 
// Параметры:
//	Элемент - ТаблицаФормы - табличная часть, которая редактируется
//	Отказ - Булево
//	Форма - ФормаКлиентскогоПриложения
Процедура КонтрольПродажиТабачнойПродукцииТолькоПоМРЦ(Элемент, Отказ, Форма) Экспорт
	
	Если НЕ РозничныеПродажиКлиентСерверЛокализация.РозничныеПродажиТабачнойПродукцииТолькоПоМРЦ(Форма) Тогда
		Возврат;
	КонецЕсли;
	
	Если Форма.ИмяФормы = "Обработка.ПодборТоваровВДокументПродажи.Форма.Форма" Тогда
		ИмяЭлементаТЧ = Форма.Элементы.Корзина.Имя;
	ИначеЕсли Форма.ИмяФормы = "Документ.ЗаявкаНаВозвратТоваровОтКлиента.Форма.ФормаДокумента" Тогда
		ИмяЭлементаТЧ = Форма.Элементы.ЗаменяющиеТовары.Имя;
	Иначе
		ИмяЭлементаТЧ = Форма.Элементы.Товары.Имя;
	КонецЕсли;
	
	Если Элемент.Имя <> ИмяЭлементаТЧ Тогда
		Возврат;
	КонецЕсли;
	...
	Если НЕ РозничныеПродажиКлиентСерверЛокализация.ЭтоВидПродукцииСМРЦ(ТекущиеДанные.ВидПродукцииИС) Тогда
		Возврат;
	КонецЕсли;
	
	// Перечень колонок, для ручного изменения цены
	КолонкиМРЦ = Новый Массив;
	КолонкиМРЦ.Добавить(Форма.Элементы[ИмяЭлементаТЧ+"ПроцентРучнойСкидки"].Имя);
	КолонкиМРЦ.Добавить(Форма.Элементы[ИмяЭлементаТЧ+"СуммаРучнойСкидки"].Имя);
	Если КолонкиМРЦ.Найти(Элемент.ТекущийЭлемент.Имя) = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
#Область ПотверждениеВозрастаПокупателя

// Вызов механизма подтверждения возраста покупателя через мессендре Макс. 
// Параметры:
//	Форма - ФормаКлиентскогоПриложения
//  Команда - КомандаФормы
//
Процедура ПодтверждениеВозрастаПокупателяЧерезМакс(Форма, Команда) Экспорт

	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ТипРезультатаПроверкиВозрастаВМакс", МенеджерОборудованияКлиент.ТипРезультатаПроверкиВозрастаВМакс());

	Оповещение = Новый ОписаниеОповещения("Подключаемый_ПодтверждениеВозрастаПокупателяЧерезМаксЗавершение", Форма, ДополнительныеПараметры);
	ПараметрыПроверки = МенеджерОборудованияКлиент.ПараметрыОткрытияФормыПроверкиВозрастаВМакс();
	ПараметрыПроверки.ОтображатьQRКодНаДисплееПокупателя = Истина;
	Если Форма.ИмяФормы = "Документ.ЧекККМ.Форма.ФормаДокументаРМК" Тогда
		ПараметрыПроверки.Вставить("Организация", Форма.Объект.Организация);
	ИначеЕсли Форма.ИмяФормы = "Обработка.ПредпросмотрЧека.Форма.Форма" Тогда
		ПараметрыПроверки.Вставить("Организация", Форма.Организация);
	КонецЕсли;
	МенеджерОборудованияКлиент.ОткрытьФормуПроверкиВозраста(Оповещение, ЭтотОбъект, ПараметрыПроверки);

КонецПроцедуры

// Обработчик нажатия навигационной ссылки для вызова механизма проверки возраста покупателя.
// 
// Параметры:
//	Форма - ФормаКлиентскогоПриложения
//	Элемент - ГруппаФормы, ДекорацияФормы, КнопкаФормы, ПолеФормы, ТаблицаФормы -
//  НавигационнаяСсылкаФорматированнойСтроки - Строка
//  СтандартнаяОбработка - Булево
//
Процедура СостояниеПроверкиВозрастаПокупателяОбработкаНавигационнойСсылки(Форма, Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка) Экспорт
	ОчиститьСообщения();
	Если НавигационнаяСсылкаФорматированнойСтроки = РозничныеПродажиКлиентСерверЛокализация.ИмяДействияПодтверждениеВозрастаПокупателяЧерезМакс() Тогда
		СтандартнаяОбработка = Ложь;
		ПодтверждениеВозрастаПокупателяЧерезМакс(Форма, Форма.Команды[РозничныеПродажиКлиентСерверЛокализация.ИмяКомандыПодтверждениеВозрастаПокупателяЧерезМакс()]);
	КонецЕсли;
КонецПроцедуры
...
//  Команда - КомандаФормы -
//
Процедура ОткрытьФормуНастройкиРасписанияРегламентногоЗадания(Форма, Команда) Экспорт
...
//-- Локализация
	ИначеЕсли Команда = Форма.Команды.НастроитьРасписаниеУдаленияНеиспользуемыхДанныхОПринадлежностиЗапасовТорговыхТочек 
		И Форма.ОписаниеНастраиваемыхРегламентныхЗаданий.Свойство("УдалениеНеиспользуемыхДанныхОПринадлежностиЗапасовТорговыхТочек") Тогда
		ЗаполнитьЗначенияСвойств(ПараметрыФормы, Форма.ОписаниеНастраиваемыхРегламентныхЗаданий.УдалениеНеиспользуемыхДанныхОПринадлежностиЗапасовТорговыхТочек);
	КонецЕсли;
