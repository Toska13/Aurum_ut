...
//  * ТребуетсяОднородностьОтвета - Булево
...
//  * ИспользуетсяКонтрольОтраженияВУчетеЭДО - Булево
...
//  * КонтрольОтраженияВУчете - См. ЛегкийИнтерфейсДокументовЭДОСлужебный.НовыеДанныеКонтроляОтраженияВУчете
...
// Конструктор дополнительного состояния.
//
// Возвращаемое значение: 
...
//  * ВыводитьЧатУниверсальныхСообщений - Булево
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ДанныеФайлаИнформацииОтправителяДокументаПоИнтеграциям", Истина);
	Методы.Вставить("ЗаполнитьДанныеКомпонентовПросмотраДокументаАсинхронно", Истина);
...
Процедура НастроитьПанельКомандКарточкиВходящегоДокумента(Форма, ДанныеКомпонента)
	...
	Форма.ТребуетсяОднородностьОтвета = ДанныеКомпонента.ТребуетсяОднородностьОтвета;
...
Процедура НастроитьПанельПакетаКарточкиДокумента(Форма, ДанныеКомпонента)
	...
	Если Форма.СоставПакета.Количество() Тогда
		Для Каждого ДанныеДокументаПакета Из Форма.СоставПакета Цикл
			...
			Если ЭтоВходящийЭДО(ДанныеДокументаПакета.ЭлектронныйДокумент) 
				И ДанныеКомпонента.ИспользуетсяКонтрольОтраженияВУчетеЭДО Тогда
				ШаблонСтроки = НСтр("ru = 'Учтен (%1)'");
				КоличествоОснований = ДокументСоставаПакета.ПолноеСостояние.КонтрольОтраженияВУчете.КоличествоОснований;
				ДанныеДокументаПакета.ВУчете = СтрШаблон(ШаблонСтроки, КоличествоОснований);
			КонецЕсли;
	...
	Иначе
		Для Каждого ДанныеДокументаПакета Из ДанныеКомпонента.ДанныеДокументовПакета Цикл
			...
			Если ЭтоВходящийЭДО(ДанныеДокументаПакета.ЭлектронныйДокумент) 
				И ДанныеКомпонента.ИспользуетсяКонтрольОтраженияВУчетеЭДО Тогда
				ШаблонСтроки = НСтр("ru = 'Учтен (%1)'");
				КоличествоОснований = ДанныеДокументаПакета.ПолноеСостояние.КонтрольОтраженияВУчете.КоличествоОснований;
				НовыйДокументСоставаПакета.ВУчете = СтрШаблон(ШаблонСтроки, КоличествоОснований);
			КонецЕсли;
...
Процедура НастроитьПанельСостоянияКарточкиДокумента(Форма, ДанныеКомпонента)
	...
	ПредставлениеДляЗакрытогоПакета = 
		ПредставлениеСостоянияЗакрытогоПакетаПоПредставлениюСостоянияЭДО(ПредставлениеПоСостояниюЭДО);
	ТекстСостояния = ?(ДанныеКомпонента.ТребуетсяОднородностьОтвета, 
		ПредставлениеДляЗакрытогоПакета, ПредставлениеПоСостояниюЭДО);
	
	ПредставлениеПоОтражениюВУчетеЗакрытогоПакета = 
		ПредставлениеСостоянияКонтроляОтраженияВУчетеЗакрытогоПакета(ПредставлениеПоОтражениюВУчете);
	ТекстОтраженияВУчете = ?(ДанныеКомпонента.ТребуетсяОднородностьОтвета,
		ПредставлениеПоОтражениюВУчетеЗакрытогоПакета, ПредставлениеПоОтражениюВУчете);
	...
	ИначеЕсли ЭтоВходящийЭДО
		...
		Форма.ТекстПанелиСостояния = 
			СтрШаблон("%1, %2", ТекстСостояния, НРег(ТекстОтраженияВУчете));
	Иначе
		Форма.ТекстПанелиСостояния = ТекстСостояния;
	КонецЕсли;
...
Функция ПредставлениеСостоянияПоДаннымСостоянияЭДО(СостояниеЭДО)
	...
	ПредставленияПоСостояниямЭДО.Вставить(
		Перечисления.СостоянияДокументовЭДО.ЗакрытСОтклонениемПриглашения, ВариантыПредставлений.ПриглашениеОтклонено);
...
Функция ПредставлениеСостоянияПоДаннымКонтроляОтраженияВУчетеИПредставлениюСостоянияЭДО(ДанныеПринятияКУчету,
	...
	ИначеЕсли ДанныеПринятияКУчету.ТребуетсяОтражениеВУчете
		И Не ДанныеПринятияКУчету.СозданУчетныйДокумент
		И Не Аннулирован 
		И Не ДанныеПринятияКУчету.ТребуетЗаполнениеОтветногоТитула Тогда
		Представление = ВариантыПредставлений.СоздатьУчетныйДокумент;
	ИначеЕсли ДанныеПринятияКУчету.ТребуетсяОтражениеВУчете
		И Не ДанныеПринятияКУчету.СозданУчетныйДокумент
		И Не Аннулирован 
		И ДанныеПринятияКУчету.ТребуетЗаполнениеОтветногоТитула Тогда
		Представление = ВариантыПредставлений.СоздатьУчетныйДокументЗаполнитьОтветныйТитул;
	ИначеЕсли ДанныеПринятияКУчету.ТребуетсяОтражениеВУчете
...
Функция ЦветПанелиСостоянияПоПредставлениям(ПредставлениеПоСостояниюЭДО, ПредставлениеПоОтражениюВУчете)
	...
	ЦветаПоСостояниямЭДО.Вставить(ВариантыПредставленийПоСостояниямЭДО.ЗавершенОшибка,
		ВариантыЦвета.Красный);
	ЦветаПоСостояниямЭДО.Вставить(ВариантыПредставленийПоСостояниямЭДО.ПриглашениеОтклонено,
		ВариантыЦвета.Красный);
...
// * СоздатьУчетныйДокументЗаполнитьОтветныйТитул - Строка
...
Функция ПредставленияСостоянияПоДаннымКонтроляОтраженияВУчете()
	...
	Представления.Вставить("СоздатьУчетныйДокументЗаполнитьОтветныйТитул", НСтр("ru = 'Принять к учету'"));
...
// Параметры:
//  ПредставлениеПоОтражениюВУчете - Строка
//
// Возвращаемое значение:
//  Строка
Функция ПредставлениеСостоянияКонтроляОтраженияВУчетеЗакрытогоПакета(ПредставлениеПоОтражениюВУчете)
	
	Представления = Новый Структура;
	Представления.Вставить("ПринятКУчету", НСтр("ru = 'Все документы в пакете приняты к учету'"));
	Представления.Вставить("НеПринятКУчету", НСтр("ru = 'Не все документы в пакете приняты к учету'"));
	Представления.Вставить("СоздатьУчетныйДокумент", НСтр("ru = 'Принять к учету документы в пакете'"));
	Представления.Вставить("ПровестиУчетныйДокумент", НСтр("ru = 'Провести учетные документы'"));
	
	СопоставленныеПредставления = Новый Соответствие();
	Для Каждого КлючИЗначение Из ПредставленияСостоянияПоДаннымКонтроляОтраженияВУчете() Цикл
		СопоставленныеПредставления.Вставить(КлючИЗначение.Значение, 
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Представления, КлючИЗначение.Ключ, КлючИЗначение.Значение));
	КонецЦикла;
	
	Возврат СопоставленныеПредставления[ПредставлениеПоОтражениюВУчете];
...
// * Создать - Строка
// * КонтрагентОтказалВПодписи - Строка
// * ОтказаноВПодписиКонтрагенту - Строка
// * ПриглашениеОтклонено - Строка
//
Функция ПредставленияСостоянияПоДаннымСостоянияЭДО()
	...
	Представления.Вставить("ПриглашениеОтклонено", НСтр("ru = 'Контрагент отклонил приглашение'"));
	Возврат Представления;

КонецФункции

// Параметры:
//  ПредставлениеПоСостояниюЭДО - Строка
//
// Возвращаемое значение:
//  Строка
Функция ПредставлениеСостоянияЗакрытогоПакетаПоПредставлениюСостоянияЭДО(ПредставлениеПоСостояниюЭДО)
	
	Представления = Новый Структура;
	Представления.Вставить("Аннулирован", НСтр("ru = 'Пакет аннулирован'"));
	Представления.Вставить("Завершен", НСтр("ru = 'Документооборот пакета завершен'"));
	Представления.Вставить("Отменен", НСтр("ru = 'Пакет закрыт принудительно'"));
	Представления.Вставить("Отклонен", НСтр("ru = 'Пакет отклонен'"));
	Представления.Вставить("ЗавершенОшибка", НСтр("ru = 'Завершен (не удалось отправить)'"));
	Представления.Вставить("ЖдатьОтветНаПриглашение", НСтр("ru = 'Ожидать ответ на приглашение от контрагента'"));
	Представления.Вставить("Отправлен", НСтр("ru = 'Отправлен контрагенту, ожидать ответ'"));
	Представления.Вставить("ОтправленДляИсправления", 
		НСтр("ru = 'Пакет отправлен контрагенту для исправления, ожидать ответ'"));
	Представления.Вставить("ОтправленДляАннулирования", 
		НСтр("ru = 'Пакет отправлен контрагенту для аннулирования, ожидать ответ'"));
	Представления.Вставить("Отправить", НСтр("ru = 'Отправить пакет'"));
	Представления.Вставить("ОтправитьПовторно", НСтр("ru = 'Оправить пакет повторно'"));
	Представления.Вставить("Подписать", НСтр("ru = 'Подписать пакет'"));
	Представления.Вставить("Утвердить", НСтр("ru = 'Утвердить пакет'"));
	Представления.Вставить("Отклонить", НСтр("ru = 'Отклонить пакет'"));
	Представления.Вставить("Исправить", НСтр("ru = 'Исправить пакет'"));
	Представления.Вставить("Аннулировать", НСтр("ru = 'Аннулировать пакет'"));
	Представления.Вставить("ПодтвердитьАннулирование", НСтр("ru = 'Подтвердить аннулирование пакета'"));
	Представления.Вставить("Создать", НСтр("ru = 'Сохранить документ'"));
	
	СопоставленныеПредставления = Новый Соответствие();
	Для Каждого КлючИЗначение Из ПредставленияСостоянияПоДаннымСостоянияЭДО() Цикл
		СопоставленныеПредставления.Вставить(КлючИЗначение.Значение, 
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Представления, КлючИЗначение.Ключ, КлючИЗначение.Значение));
	КонецЦикла;
	
	Возврат СопоставленныеПредставления[ПредставлениеПоСостояниюЭДО];
...
Процедура НастроитьПанельЗакладокВходящегоДокумента(Форма, ДанныеКомпонента)
	...
	Если ДанныеКомпонента.ВыводитьЧатУниверсальныхСообщений Тогда
...
Процедура НастроитьПанельКомандПоПравамКарточкиВходящегоДокумента(Форма, ДанныеКомпонента)
	...
	Если Не ДанныеКомпонента.ЕстьПравоОбработкиДокументов Тогда
		...
		Форма.Элементы.КнопкаПометитьНаУдаление.Видимость = Ложь;
	КонецЕсли;
...
Процедура НастроитьПанельКомандПоПравамКарточкиИсходящегоДокумента(Форма, ДанныеКомпонента)
	...
	Если Не ДанныеКомпонента.ЕстьПравоОбработкиДокументов Тогда
		...
		Форма.Элементы.ГруппаСохранитьДобавитьВПакет.Видимость = Ложь;
		Форма.Элементы.КнопкаЗаписатьДокументПодменюЕще.Видимость = Ложь;
		Форма.Элементы.КнопкаЗагрузитьФормализованныйДокументСДиска.Видимость = Ложь;
		Форма.Элементы.КнопкаПометитьНаУдаление.Видимость = Ложь;
		Форма.Элементы.ГруппаНастройкиОтраженияВУчете.Видимость = Ложь;
		Форма.Элементы.ОчиститьЗаписку.Видимость = Ложь;
	КонецЕсли;
...
Процедура НастроитьПанельСообщений(Форма, ДанныеКомпонента)
	Если Не ИнтерфейсДокументовЭДО.ВыводитьЧатУниверсальныхСообщений() Тогда
		Возврат;
	КонецЕсли;
