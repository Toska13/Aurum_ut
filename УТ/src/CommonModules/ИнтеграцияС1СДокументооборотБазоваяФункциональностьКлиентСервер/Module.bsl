...
//     * ПредставлениеВидаДокумента - Строка - представление вида документа ДО.
...
Функция ДанныеПравилаИнтеграции() Экспорт
	...
	Результат.Вставить("ПредставлениеВидаДокумента", "");
...
//     * Зашифрован - Булево - тип объекта XDTO.
//
...
//     * ДобавлениеЭПФайла - Булево
//     * УдалениеЭПФайла - Булево
//     * ПодписаниеДокументаСФайлами - Булево
...
Функция ДействиеНадФайлом() Экспорт
	...
	ДействиеНадФайлом.Вставить("ДобавлениеЭПФайла", Ложь);
	ДействиеНадФайлом.Вставить("УдалениеЭПФайла", Ложь);
	ДействиеНадФайлом.Вставить("ПодписаниеДокументаСФайлами", Ложь);
...
Функция ИмяТипаИзПолногоИмени(ПолноеИмя, ЭтоСсылка = Истина, ВызыватьИсключение = Истина) Экспорт
	...
	Если СоставИмени.Количество() < 2
		...
		Если ВызыватьИсключение Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Не удалось определить имя типа из строки ""%1""'"), ПолноеИмя);
		Иначе
			Возврат "";
		КонецЕсли;
	КонецЕсли;
...
// Конструктор структуры, описывающей настройку связи между объектами ДО.
//
// Параметры:
//   СсылкаИз - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО
//   СсылкаНа - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО
...
//     * СсылкаНа - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО
//     * ТипСвязи - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО
//     * ТипСвязиКомментарий - Строка
//
Функция НастройкаСвязиОбъектовДО(СсылкаИз, СсылкаНа, ТипСвязи) Экспорт
	
	НастройкаСвязиОбъектовДО = Новый Структура;
	НастройкаСвязиОбъектовДО.Вставить("СсылкаИз", СсылкаИз);
	НастройкаСвязиОбъектовДО.Вставить("СсылкаНа", СсылкаНа);
	НастройкаСвязиОбъектовДО.Вставить("ТипСвязи", ТипСвязи);
	НастройкаСвязиОбъектовДО.Вставить("ТипСвязиКомментарий", "");
	
	Возврат НастройкаСвязиОбъектовДО;
...
// Конструктор структуры параметров оповещения о получении HTML-предпросмотра объекта.
//
// Возвращаемое значение:
//   Структура:
//     * ID - Строка - идентификатор объекта ДО.
//     * Тип - Строка - тип объекта XDTO.
//     * Расширение - Строка - расширение файла.
//     * ПредставлениеHTML - Строка - HTML-представление.
//     * ПредпросмотрУрезан - Булево - признак урезанного предпросмотра.
//     * ДвоичныеДанные - ДвоичныеДанные, Неопределено - двоичные данные файла.
//
Функция ПараметрыОповещенияПолученHTMLПредпросмотрОбъекта() Экспорт
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ID", "");
	ПараметрыОповещения.Вставить("Тип", "");
	ПараметрыОповещения.Вставить("Расширение", "");
	ПараметрыОповещения.Вставить("ПредставлениеHTML", "");
	ПараметрыОповещения.Вставить("ПредпросмотрУрезан", Ложь);
	ПараметрыОповещения.Вставить("ДвоичныеДанные", Неопределено);
...
// Конструктор структуры, описывающей недостающую связь между объектами ДО.
//
// Возвращаемое значение:
//  Структура:
//     * СсылкаНа - Строка
...
//     * Комментарий - Строка
//
Функция СвязьКСозданию() Экспорт
	
	СвязьКСозданию = Новый Структура;
	СвязьКСозданию.Вставить("СсылкаНа", "");
	СвязьКСозданию.Вставить("СсылкаНаID", "");
	СвязьКСозданию.Вставить("СсылкаНаТип", "");
	СвязьКСозданию.Вставить("ТипСвязиID", "");
	СвязьКСозданию.Вставить("Комментарий", "");
	
	Возврат СвязьКСозданию;
...
//
Функция СообщениеВПредпросмотр(Знач Сообщение, ЗаголовокСообщенияВОбластиПредпросмотра) Экспорт
...
Функция СообщениеОбОтсутствииПредпросмотра(ЗаголовокСообщенияВОбластиПредпросмотра) Экспорт
	...
	Возврат СообщениеВПредпросмотр(
		НСтр("ru='Данные предпросмотра отсутствуют'"), ЗаголовокСообщенияВОбластиПредпросмотра);
	
КонецФункции

// Выводит сообщение о получении предпросмотра.
//
// Возвращаемое значение:
//   Строка
//
Функция СообщениеОЗагрузке() Экспорт
	
	Результат = Новый Массив;
	
	Результат.Добавить(
		"<html><head><meta content=""text/html"" http-equiv=""Content-Type""></head><style>
		|.center {
		|	display: flex;
		|	align-items: center;
		|	position: absolute;
		...
		|</style><body><div class=""center""><img src=""data:image/gif;base64,
		|R0lGODlhMAAwAOMAAPzeNPzqfPzmVPzulPziRPzypPziNPzujPzqbPzynPzuhPzm
		|XPzunPziTPziPP3xpSH/C05FVFNDQVBFMi4wAwEAAAAh+QQJCQAPACwAAAAAMAAw
		|AAAE/vDJSessjA0mAwpHYo1keQ3HESgK9ywA4DhCMJj4mAxg0B+3V2woWwRzpoJq
		|+Qu4FgZAVBozGJGjAo/JbUlgVWLMEShgTwpu0/mdhqPwhfnMUHNVT6pbDJCfH1pN
		|anlDcHoyR0kDIg8agj9BUFR8iBIYJUogLpl3QC4HCA0Oh1EOQUqJFQw/Lo4qAxtZ
		|qwRuppYqXhYJd5soDHMmq6O2gFwuFGqaEhl/DFcPu02pBYJLx3+At4/ADwMKTd8p
		|2Kpb1RMJj9bjFJzby2mP1+uN6WwPdiAH3PN16QfF6eTNaxROUIZ6+wb2s5OhIJNU
		|AyWkiOfNDsSI5bhsgAVLgwZG0REpZNhIUmDIkyjHaeDIEiTKlSw7eutR0OTAmYMY
		|OOxxcSA+Tz9tjmtnjBo+odhW4TOT8Ye+k0SdvnuUC2M9F9GWRqzzE1jThxGjLvl3
		|rmtIV0322TmwCakFLYzQvnrLa5m3XzkaKgOU0aTSvY7C4R2R4YDDTeqybOkFLsUi
		|uyl2puj1lIQWxI96uOAhmZUltxcmBpVYDxfbP2IFBfn6qCcJuelWlwYxICEODD9p
		|S2A9NtY62Ew21/Md9msu0ZBAo+aqmfRYZikt/Pq4LAP0PxEAACH5BAkJAA8ALAAA
		|AAAwADAAg/zeNPzqdPzulPzmVPziTPzuhPzypPziNPzynPzqbPzqfPzunPzmXPzu
		|jPziPP3xpQT+8MlJ6zRriSX1QpwljqRkCE2jFEXYFIoqCEZpi4gQK3sjSAKYatio
		|3WyGYW/nKihVsUboWDktny3gdbkwUh8n5xYaCo53tO9j8byqXGflT21tK8vCOOJ4
		|2q91djFzQYFDIRgkSTGHdikbHSiBj2ADDF4VCz0hGmiTFhlPUiYEAAcKOKGHKF03
		|oIsdAwCyDlMTba9rtTcnh7GlsgxVbrhqVaQHsr9zQHkqQg3FmL6yyKXBEghxusUG
		|Dr/Us3MLYm3b0QrV1ckBEoVF0Z8E6sjIA2CF5vAK4MkAtJmBLsGbsMDbL3oApDRD
		|M1CEAXkAIkoEoIDQlWUNKSSIiBBZAg3UMzaI9JORQoMECRikXHmqpMuXJUGGDEny
		|pcyZIvPkyTfQTLlxWzC6LLQBH0xFdrqc4VkM4JYagNwIhIfUTYeFzphSiVoOWyEF
		|U9WwKeSF6xKh3NxRQEA2plkVAhsd0lpFgB9OcixUTbEpCCsbGYSo0iTCqag/KxaF
		|zfWC8D1idXHV2eHEx91IWA+feDeCl4lATLRgdYyITwpt7eKsOExl75hBqmWIfXtR
		|y1cfixOxiQPbnaemtN/YDkp319ssD04/+e0Sg3IFcIhkgFkYg90OGaYXiwAAIfkE
		|CQkADwAsAAAAADAAMACD/N40/Op0/O6U/OZU/OJM/O6E/PKk/OI0/PKc/Ops/Op8
		|/O6c/OZc/O6M/OI8/fGlBP7wyUnrNGuJJfVCnCWOpGQITaMURdgUiioIRmmLiBAr
		|eyNIAphq2KjdbIZhb+cqKFWxRuhYOS2fLeB1uTBSHyfnFhoKjne072PxvKpcZ+VP
		|bW0ry8I44njar3V2MXNBgUMhGCRJMYd2KRsdKIGPYCgkCz0hGmiTFhlPUiZDcxUI
		|n4coXTeei5CYFm2sa1N8k3VoVW6xalWBXg9mSkINuxWajRMIcbPEir0dYm3LxGtn
		|cI2+039jw81X0tkLeU8ZZ9jZa4UZ4rfnr4Ea0D2j7ROAVxszMx4CfvQTGfgCfvNH
		|sCAVDfkS9iPYIIHDhw4VAFvBxOCDBAAyaswYINyWecv0FhDYuFEBmzEDiSEYSRLA
		|gSLVCipwSfNARgcc7H0yx4zlRpsDnm3JQk+BzaMbA0goVa7dAgckjx6YonMJyF0G
		|CCDdyIACUzs81SwYkBHpywqNDqUU0acD2Y1BefUA9adFWArqYq2U+u2SoVZCUonw
		|JC7TW6VsAZ1a4sSHHw0p1qUwzODuCUZbKv6imM7EWgpJCsUSrcLJZ7mkB8Vhd7Cq
		|HdWp747AkFpLIR+nRxjbUuYMp3O2hhBNcS/3F9p3JJgyPq2LhscZMhCLAAAh+QQJ
		|CQAPACwAAAAAMAAwAAAE/vDJSessjA0mNUucJY6kVAzHEShKeCiBOgxFaYvJEAf7
		|MUgDmGp4qN1shWFv51IoVbFD6Fg5LZ8t4HXJMFIfJ+cWGgqOd7Tvg/G8qlxn5U9t
		|bSvLwnjieNqvdXYxc0GBQyEYJEkxh3YpGx0ogY9gKCQMPSEaaJMWGU9SJkNzFQmf
		|hyhdN56LkJgWbaxrU3yTdWhVbrFqVYFeD2ZKQge7FZqNEwlxs8SKvR1ibcvEa2dw
		|jb7Tf2PDzVfS2Qx5Txln2NlrhRnit+evgRrQPaPtE4BXGzMzHgN+9BMZ+AJ+80ew
		|IBUN+RL2I4gwoT5gK5gY/LXuTbgt8/wV2pCuYLdxzAWqFbzkzN4nc8wKDVtTccVA
		|NSa5SChVrh2bQl5iymn3scfKmTjBmdAZAFujQy9FMFgwxxg7Ch9B/WmBslgAAgAI
		|zPn4jWQsTUJSdbokwAAAAAa0tpJqoc6pJU58+DmAoAHWs2bPqqVUJFGtQBIfLMCL
		|9mzhrE2rtk2hTMJgw4QJO8iIpJA8x2YzG857doHiTkSfzHkMmbMBAwsW8mETZ7Rm
		|02gDJC3hNJpjyJEFUGamM4vg12h1T8TA+M1ttA4EBNg9EgM/CQEQ6PoSAQAh+QQJ
		|CQAOACwAAAAAMAAwAIP83jT86nz85lT87pT84kz88qT84jT87oz86mz88pz87oT8
		|5lz87pz84jz98aUAAAAE/tDJSessjA0mNUucJY6kVAzHEShKeCiBOgxFaYvJEAf7
		|MUgDmGp4qN1shWFv51IoVbFD6Fg5LZ8t4HXJMFIdJ+cWGgqOd7Svg/G8qlxn5U9t
		|bSvLwnjieNqvdXYxc0GBQyEYJEkxh3YpGx0ogY9gKCQMPSEaaJMWGU9SJkNzFQmf
		|hyhdN56LkJgWbaxrU3yTdWhVbrFqVYFeDmZKQge7FZqNEwlxs8SKvR1ibcvEa2dw
		|jb7Tf2PDzVfS2Qx5Txln2NlrhRnit+evgRrQPaPtE4BXGzMzHgN+9BMZ+AJ+80ew
		|IBUN+RL2I4gwoT5gK5gY/LXuTbgt8/wV2pCuYLdxyQWqFbzkzN4nc8wKDVtTccVA
		|NSa5SChVrh2bQl5iymn3scfKmTjpGVuCrdGhlyKSGBkqw8JHUGsENAiAsthFqLag
		|FnO1ZgEAAAYIBEjViY04RrqqADokAOxXsA0IIJijIUVFrCiqUjpE4K1bvwEkKNkS
		|C9GRBG0N/P2q2EDgX3EM0enrd/HXx4WWZESyoIFbxZ/BYs4sQ++IkJ4ZVwaAOU4A
		|ENkGLADd2G1rSUipyAb91rFgO2kMXhJgAPRtQRkmklKBYKWH5LsiAAAh+QQJCQAQ
		|ACwAAAAAMAAwAIT83jT86nT85lT87pT84kT87oT88qT84jT86mz88pz86nz85lz8
		|7pz84kz87oz84jz98aUAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
		|AAAAAAAAAAAAAAAF/iAkjmQ5GgwzMKLKJKwpz7RoDI6jFEXsFArdYGCoGWWJQVCx
		|dAxEA6Bu6igejYZpc+kraHVBR+xaum2/Peh5y7CSITfvGhyLzpfEN4TxPet8d1pP
		|emZ9WnVSgQlXN4t7SoZBg1GRUzEoNFlBl4Y5Ky04kZ9wODQMTTEqeKMmKV9iNlOD
		|JQmvlzhtR66boKgmfbx7Y4yjhXhlfsF6ZZFuEHZaUg7LJaqdIwmBw9SazS1yfdvU
		|e3eAnc7jj3PT3Wfi6QyJXyl36Ol7lSnyx/e/kSrgmszqNwLSmRVDhrgY4IjgiBQI
		|I75zSLEimQIIMmrMOM2iioQgGSAAQLIkSQQW0p/t+6PApEkBE8cZ/ILDZckHMZcZ
		|qMfgwQEAP4MCUFAx3hwWAmwCbWCPW6WOAYBK/QmUKMGZbFpQtYmzX611YxZMHbug
		|KRlrfkgM2GrywcBxxrSgEyv0gFsbOU006iXIxIAHbQdpymVEX7C47xQEvQtnnllX
		|8jgpKyOWMZ8tXpw4UpFjJSxSVWgkWDAIrRYualb6gpNXcqdglZZ4yVsGK7A6gfid
		|te1ORI7ceQhdzuc7kBizR+JmfXaH1b0bhsxtCV6UM2rmslKkrIaCIagUtE2EAAAh
		|+QQJCQAQACwAAAAAMAAwAIT83jT86nT85lT87pT84kT87oT88qT84jT86mz88pz8
		|6nz85lz87pz84kz87oz84jz98aUAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
		|AAAAAAAAAAAAAAAAAAAAAAAF/iAkjmQ5GgwzMKLKJKwpz7RoDI6jFEXsFArdYGCo
		|GWWJQVCxdAxEA6Bu6igejYZpc+kraHVBR+xaum2/Peh5y7CSITfvGhyLzpfEN4Tx
		|Pet8d1pPV1YqgQp1UoEJhAuDWYGDUX1fMSg0BgsABIOGXzkrLTiUCqFwODQKAKuc
		|NkpCpiYpn5ZTgyUDDwAHu617OG1Hs0ExnsQmmqvKvil6N5aveLi81LsAD7d6JJB9
		|bhDJyuEL3trGnyMMD9XVD2PaNoExAeHhBwHvsopfgwLW6wTk8PEh9SQdPWX38Mm6
		|k8WfvwPuFNogJUbVQQACIkqEQJEBAoe8ACDYaALHmigI11IuQLASgQOSJVSsGCJT
		|I8ybON/IHMJzAKOcO3vOVKTIpkQ7fVLok5aT4oqOOLmdaXPH6DsGDCFE+xTwndRz
		|e5bqSENyKxsRCSgq6Kozxxw3ZgVt/Nrk5Yi0b0maW0KOkhgbVmXc+Lk328S6xaIE
		|M6L02KkmNrFOSbyDGNtZ+mo5NmHm72MtXpwQHiXW840qmEzRBfNHjVjIgMlkUeuZ
		|9g7Psm07UWPbMOO4ZyQdcsIWE59IvOc4CVxjr5Y6d2JJ7NyErNt9zLWhuI5IBK3s
		|EtuoIJyimbYQACH5BAkJAA4ALAAAAAAwADAAg/zeNPzqfPzmVPzulPziTPzypPzi
		|NPzujPzqbPzynPzuhPzmXPzunPziPP3xpQAAAAT+0MlJ60wnBCXZYAljjWQ5MYHQ
		|AGwgHUqQDUNh3uOwrCxgAC4HTEY8HGy4m+7XYwJfmWjxIEpaCgGer7cNDqUZIgNp
		|dRQWTdbP6VWAZTFYrexgCLbctLfIz8QGdHV3W2tcbX19VTcFAwkdg1yFXm9gVBIF
		|ildRVQUEhWoECIB1NIhGVR6jIyibj6ABISOYlagDMZkXtI8NsDgMDK2kcQq4Qnwy
		|VbFljJwDfKqXfcGBFAWUAWQOtnxxB9QUHqbeEgmmyN8T1uZIDG6IxdSs7y/i2ejh
		|lN7qlPDf7aa/1qGzAAzgvzfQBk4w8s4WpYQKtZn6QIOGBw+OIoK7aNGixo/FIAem
		|qlgxI8iRJCnG2SAmpLaVUn7BJAJR4bUDH272o7YvEZZrOwPJe2PDWT5733pW6jDT
		|T1ArRuc5wCBQJENKZKIiVKh0CgWqWCPi42MvH6enFRhlHDtD05RaxJASPNisCLyh
		|tVhSkVsHGExO09IarSvFDc61A4w0tWQmMd/GgBG1lNjU7iW0l64CpHdzA2MrXR9K
		|0GquZgm2E0ebizKHzqzVo0hX+nBP9mTbMzAvkk0MSh/aIWfF5CyDim5/mBp1+PWL
		|WgQAO3NvM09ISGQyakJ4YUNGZ3RmVm9mbU5Ib3BSTDZ4WEw2eWhSOWNrZHBQVEFJ
		|Z2JlT2oxUEVxWDBTVnFFUXJsUDA="" width=""48"" height=""48""/>
		|<p style=""padding-left: 10px"">");
	Результат.Добавить(НСтр("ru = 'Получение предпросмотра'"));
	Результат.Добавить("...</p></div></body></html>");
...
Функция СообщениеОРедактировании(ЗаголовокСообщенияВОбластиПредпросмотра) Экспорт
	...
	Возврат СообщениеВПредпросмотр(
		СтрШаблон("%1<br>
			...
			НСтр("ru='Данный файл занят вами на редактирование'"),
			НСтр("ru='Открыть'")),
...
Функция УдалитьТехИнформациюИзТекстаОшибки(Знач ТекстОшибки) Экспорт
	...
	Для НомерСтроки = 1 По КоличествоСтрок Цикл
		...
		Если СтрНайти(СтрокаОшибки, "{ОбщийМодуль.") = 1
				Или СтрНайти(СтрокаОшибки, "{Справочник.") = 1
				Или СтрНайти(СтрокаОшибки, "{Документ.") = 1
...
// Проверяет, является ли переданный параметр структурой параметров оповещения о получении HTML-предпросмотра объекта.
//
// Параметры:
...
//   Булево
//
Функция ЭтоПараметрыОповещенияПолученHTMLПредпросмотрОбъекта(Параметр) Экспорт
	
	Если ТипЗнч(Параметр) <> Тип("Структура") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Для Каждого Элемент Из ПараметрыОповещенияПолученHTMLПредпросмотрОбъекта() Цикл
		Если Не Параметр.Свойство(Элемент.Ключ) Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
// Конструктор структуры, предназначенной для передачи адреса слепка документа, и адресов двоичных данных файлов.
//
// Параметры:
//   УникальныйИдентификаторФормы - УникальныйИдентификатор - идентификатор вызвавшей формы.
...
//   Структура:
//     * УникальныйИдентификаторФормы - УникальныйИдентификатор - идентификатор вызвавшей формы.
//     * АдресСлепкаДокумента - Строка
//     * АдресаДвоичныхДанныхФайлов - Соответствие из КлючИЗначение:
//       ** Ключ - Строка - идентификатор файла.
//       ** Значение - Строка - адрес временного хранилища двоичных данных файла.
//
Функция АдресДвоичныхДанныхОбъекта(УникальныйИдентификаторФормы, АдресСлепкаДокумента = "") Экспорт
	
	АдресДвоичныхДанныхОбъекта = Новый Структура;
	АдресДвоичныхДанныхОбъекта.Вставить("УникальныйИдентификаторФормы", УникальныйИдентификаторФормы);
	АдресДвоичныхДанныхОбъекта.Вставить("АдресСлепкаДокумента", АдресСлепкаДокумента);
	АдресДвоичныхДанныхОбъекта.Вставить("АдресаДвоичныхДанныхФайлов", Новый Соответствие);
	
	Возврат АдресДвоичныхДанныхОбъекта;
...
Функция ДанныеПодписи(Подпись, ДвоичныеДанныеСертификата, СвойстваПодписи) Экспорт
	...
	Если ТипЗнч(СвойстваПодписи.ТипПодписи) = Тип("Строка") Тогда
		ДанныеПодписи.ТипПодписи = СвойстваПодписи.ТипПодписи;
	Иначе
		СоответствиеТиповПодписиКриптографииПеречислениюXDTO = СоответствиеТиповПодписиКриптографииПеречислениюXDTO();
		ДанныеПодписи.ТипПодписи = СоответствиеТиповПодписиКриптографииПеречислениюXDTO[СвойстваПодписи.ТипПодписи];
	КонецЕсли;
...
//               - Неопределено
//
Функция ПараметрыПроверкиЭП(ДанныеСтроки, СоответствиеИдОбъектаИДвоичныхДанных, АдресДвоичныхДанныхОбъекта) Экспорт
	...
	Если ПараметрыПроверкиЭП.ДвоичныеДанныеОбъекта = Неопределено Тогда
		...
		Если ДанныеСтроки.ОбъектТип = "DMFile" Тогда
			АдресДвоичныхДанныхФайла = АдресДвоичныхДанныхОбъекта.АдресаДвоичныхДанныхФайлов[ДанныеСтроки.ОбъектID];
			Если ЭтоАдресВременногоХранилища(АдресДвоичныхДанныхФайла) Тогда
				ПараметрыПроверкиЭП.ДвоичныеДанныеОбъекта = ПолучитьИзВременногоХранилища(АдресДвоичныхДанныхФайла); // ДвоичныеДанные
			Иначе
			...
			КонецЕсли;
		Иначе
			ПараметрыПроверкиЭП.ДвоичныеДанныеОбъекта = ПолучитьИзВременногоХранилища(АдресДвоичныхДанныхОбъекта.АдресСлепкаДокумента); // ДвоичныеДанные
		КонецЕсли;
...
// Возвращает соответствие значений перечисления ТипыПодписиКриптографии значениям типа XDTO DMSignatureType.
//
// Возвращаемое значение:
//   Соответствие из КлючИЗначение - параметры выбора:
//     * Ключ - ПеречислениеСсылка.ТипыПодписиКриптографии
//     * Значение - Строка
//
Функция СоответствиеТиповПодписиКриптографииПеречислениюXDTO() Экспорт
	
	СоответствиеТипов = Новый Соответствие;
	
	Попытка
		ТипПодписи = "Перечисление.ТипыПодписиКриптографии.ОбычнаяCMS";
		СоответствиеТипов.Вставить(ПредопределенноеЗначение(ТипПодписи), "CMS");
		
		ТипПодписи = "Перечисление.ТипыПодписиКриптографии.БазоваяCAdESBES";
		СоответствиеТипов.Вставить(ПредопределенноеЗначение(ТипПодписи), "CAdESBES");
		
		ТипПодписи = "Перечисление.ТипыПодписиКриптографии.СПолнымНаборомПроверочныхДанныхCAdESC";
		СоответствиеТипов.Вставить(ПредопределенноеЗначение(ТипПодписи), "CAdESC");
		
		ТипПодписи = "Перечисление.ТипыПодписиКриптографии.СМеткойДоверенногоВремениCAdEST";
		СоответствиеТипов.Вставить(ПредопределенноеЗначение(ТипПодписи), "CAdEST");
		
		ТипПодписи = "Перечисление.ТипыПодписиКриптографии.CAdESXLong";
		СоответствиеТипов.Вставить(ПредопределенноеЗначение(ТипПодписи), "CAdESXLong");
		
		ТипПодписи = "Перечисление.ТипыПодписиКриптографии.CAdESXType1";
		СоответствиеТипов.Вставить(ПредопределенноеЗначение(ТипПодписи), "CAdESXType1");
		
		ТипПодписи = "Перечисление.ТипыПодписиКриптографии.CAdESXType2";
		СоответствиеТипов.Вставить(ПредопределенноеЗначение(ТипПодписи), "CAdESXType2");
		
		ТипПодписи = "Перечисление.ТипыПодписиКриптографии.CAdESXLongType1";
		СоответствиеТипов.Вставить(ПредопределенноеЗначение(ТипПодписи), "CAdESXLongType1");
		
		ТипПодписи = "Перечисление.ТипыПодписиКриптографии.РасширеннаяДолгосрочнаяCAdESXLongType2";
		СоответствиеТипов.Вставить(ПредопределенноеЗначение(ТипПодписи), "CAdESXLongType2");
		
		ТипПодписи = "Перечисление.ТипыПодписиКриптографии.CAdESAv2";
		СоответствиеТипов.Вставить(ПредопределенноеЗначение(ТипПодписи), "CAdESAv2");
		
		ТипПодписи = "Перечисление.ТипыПодписиКриптографии.АрхивнаяCAdESAv3";
		СоответствиеТипов.Вставить(ПредопределенноеЗначение(ТипПодписи), "CAdESAv3");
		
		ТипПодписи = "Перечисление.ТипыПодписиКриптографии.НеОпределен";
		СоответствиеТипов.Вставить(ПредопределенноеЗначение(ТипПодписи), "NotDefined");
	Исключение
		ВызватьИсключение НСтр("ru = 'Отсутствует стандартная подсистема ""Электронная подпись"".'");
	КонецПопытки;
...
Функция СтатусЭП(Неверна, ОписаниеОшибки) Экспорт
	...
	Возврат ?(Неверна,
		СтрШаблон(НСтр("ru = 'Подпись недействительна: %1'"), ОписаниеОшибки),
		НСтр("ru = 'Подпись действительна'"));
...
Функция СтруктураПодписи() Экспорт
	...
	ДанныеПодписи.Вставить("ДатаПроверкиПодписи", Неопределено);
	ДанныеПодписи.Вставить("ПодписьВерна", Неопределено);
	ДанныеПодписи.Вставить("ПодписанныйОбъект", Неопределено);
	ДанныеПодписи.Вставить("ПорядковыйНомер", Неопределено);
	ДанныеПодписи.Вставить("ТребуетсяПроверка", Неопределено);
	...
	ДанныеПодписи.Вставить("ТипПодписи", Неопределено);
	ДанныеПодписи.Вставить("СрокДействияПоследнейМеткиВремени", Неопределено);
	
	ДанныеПодписи.Вставить("ОписаниеСертификата", Неопределено);
	
	ДанныеПодписи.Вставить("ОшибкаПриАвтоматическомПродлении", Неопределено);
	ДанныеПодписи.Вставить("ИдентификаторПодписи", Неопределено);
	ДанныеПодписи.Вставить("РезультатПроверкиПодписиПоМЧД", Неопределено);
...
Функция СтруктураСтрокиДереваПодписей(СтрокаДереваПодписей) Экспорт
	...
	СтруктураСтроки.Вставить("ТипПодписи", Неопределено);
	Для Каждого Элемент Из СоответствиеТиповПодписиКриптографииПеречислениюXDTO() Цикл
		Если Элемент.Значение = СтрокаДереваПодписей.ТипПодписи Тогда
			СтруктураСтроки.ТипПодписи = Элемент.Ключ;
			Прервать;
		КонецЕсли;
	КонецЦикла;
