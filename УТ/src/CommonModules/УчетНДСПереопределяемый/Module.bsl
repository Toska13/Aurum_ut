...
Процедура ДополнитьИнформациейОСчетахФактурахНаОтгрузку(ДополнительнаяИнформация, СтруктураПараметров, Запрос) Экспорт
	...
	УчетНДСРФ.ДополнитьИнформациейОСчетахФактурахНаОтгрузку(ДополнительнаяИнформация, СтруктураПараметров, Запрос);
...
//   * Фильтр        - ДокументСсылка, Массив, Неопределено - Документ или список документов,
...
Функция ОпределитьНаличиеСчетовФактурПолученных(ПараметрыПоискаСчетовФактур) Экспорт
	...
	|	И НЕ НДСПредъявленный.ВидЦенности В (ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ЭлектронныеУслуги), ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.НеотфактурованныйНДС))
	...
	|				ЕСТЬNULL(ИнойДокументПодтвержденияНДС.Ссылка,
	|					ЕСТЬNULL(ДокументБронирования.Ссылка, НЕОПРЕДЕЛЕНО))))) КАК СчетФактура,
	...
	|				ЕСТЬNULL(ИнойДокументПодтвержденияНДС.Проведен,
	|					 ЕСТЬNULL(ДокументБронирования.Проведен, ЛОЖЬ))))) КАК СчетФактураПроведен,
	...
	|					ЕСТЬNULL(ИнойДокументПодтвержденияНДС.ДатаВходящегоДокумента, 
	|						ЕСТЬNULL(ДокументБронирования.Дата, ДАТАВРЕМЯ(1, 1, 1)))))) КАК СчетФактураДата,
	...
	|				ЕСТЬNULL(ИнойДокументПодтвержденияНДС.НомерВходящегоДокумента,
	|					 ЕСТЬNULL(ДокументБронирования.НомерБилета, """"))))) КАК СчетФактураНомер,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОперацияСЭлектроннымБилетом КАК ДокументБронирования
	|		ПО ОснованияСФПолученные.Основание = ДокументБронирования.Ссылка
	|			И ДокументБронирования.СпособСписанияНапрямую
	...
	|					ИЛИ НЕ ДокументБронирования.Ссылка ЕСТЬ NULL И ДокументБронирования.Ссылка.Проведен
	|		ИНАЧЕ
	...
	|				И (ДокументБронирования.Ссылка ЕСТЬ NULL ИЛИ НЕ ДокументБронирования.Ссылка.Проведен)
	|	КОНЕЦ)
	|	И (ВЫБОР
	...
	|			КОГДА НЕ ДокументБронирования.Ссылка ЕСТЬ NULL
	|				ТОГДА ДокументБронирования.Ссылка.Дата <= &ДатаСФНеБолее
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ)
	|
	|ОБЪЕДИНИТЬ ВСЕ
...
// 
// Возвращаемое значение:
//  Булево - Применяется постановление 15
...
// 
// Возвращаемое значение:
//  Булево - Применяется постановление1448
...
// Возвращает номер постановления Правительства от 1 января 2026 года.
// 
// Возвращаемое значение:
//  Строка - номер постановление
Функция НомерПостановления2026() Экспорт
	
	Возврат "1096";
	
КонецФункции

// Возвращает дату постановления Правительства от 1 января 2026 года.
// 
// Возвращаемое значение:
//  Дата - дата постановление
Функция ДатаПостановления2026() Экспорт
	
	Возврат Формат(Дата(2024,08,16), "ДЛФ=DD");
	
КонецФункции

// Возвращает номер постановления Правительства от 23 января 2026 года.
// 
// Возвращаемое значение:
//  Строка - номер постановление
Функция НомерПостановления26() Экспорт
	
	Возврат "26";
	
КонецФункции

// Возвращает дату постановления Правительства от 23 января 2026 года.
// 
// Возвращаемое значение:
//  Дата - дата постановление
Функция ДатаПостановления26() Экспорт
	
	Возврат Формат(Дата(2026,01,23), "ДЛФ=DD");
...
// 
// Возвращаемое значение:
//  Булево - Заполняется раздел7 декларации НДС
