...
#Область ЗамерыПроизводительностиБСП
...
// Возвращаемое значение:
//  ОписаниеОповещения - ОписаниеОповещения
Функция ОповещениеОЗавершенииЗамераДлительнойОперации(КлючЗамера,
		ОписаниеЗамера, КоличествоДанных, ОбработчикЗавершения) Экспорт

	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ОповещениеОЗавершении", ОбработчикЗавершения);
	ДополнительныеПараметры.Вставить("КлючЗамера", КлючЗамера);
	ДополнительныеПараметры.Вставить("ОписаниеЗамера", ОписаниеЗамера);
	ДополнительныеПараметры.Вставить("КоличествоДанных", КоличествоДанных);
	Возврат Новый ОписаниеОповещения("ПослеЗамераДлительнойОперации", 
		ОбщегоНазначенияБЭДКлиент, ДополнительныеПараметры);
КонецФункции
...
// Возвращаемое значение:
//  ОписаниеОповещения - ОписаниеОповещения
Функция ОповещениеОЗавершенииЗамера(КлючЗамера, ОбработчикЗавершения) Экспорт

	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ОповещениеОЗавершении", ОбработчикЗавершения);
	ДополнительныеПараметры.Вставить("КлючЗамера", КлючЗамера);
	Возврат Новый ОписаниеОповещения("ПослеЗамера", 
		ОбщегоНазначенияБЭДКлиент, ДополнительныеПараметры);
КонецФункции
...
// Возвращаемое значение:
//  - Неопределено
//  - см. ОценкаПроизводительностиКлиент.ЗамерВремени
Функция НачатьЗамер(ИмяКлючевойОперации) Экспорт
	Если ИмяКлючевойОперации = ""
		Или Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		Возврат Неопределено;
	КонецЕсли;
	Возврат ОценкаПроизводительностиКлиент.ЗамерВремени(ИмяКлючевойОперации, Ложь, Ложь);
КонецФункции
...
// Возвращаемое значение:
//  - Неопределено
//  - см. ОценкаПроизводительностиКлиент.НачатьЗамерДлительнойОперации
Функция НачатьЗамерДлительнойОперации(ИмяКлючевойОперации) Экспорт
	Если ИмяКлючевойОперации = ""
		Или Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		Возврат Неопределено;
	КонецЕсли;
	Возврат ОценкаПроизводительностиКлиент.НачатьЗамерДлительнойОперации(ИмяКлючевойОперации, Ложь, Ложь);
КонецФункции
...
Процедура ПослеЗамераДлительнойОперации(Результат, ДополнительныеПараметры) Экспорт
	ОценкаПроизводительностиКлиент.ЗакончитьЗамерДлительнойОперации(ДополнительныеПараметры.ОписаниеЗамера,
		ДополнительныеПараметры.КоличествоДанных, ДополнительныеПараметры.КлючЗамера);
...
//               - Неопределено
//  Ошибка - Булево
Процедура ЗакончитьЗамер(КлючДействия, Ошибка = Ложь) Экспорт
	Если КлючДействия = Неопределено
		Или Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		Возврат;
	КонецЕсли;
	ОценкаПроизводительностиКлиент.ЗавершитьЗамерВремени(КлючДействия, Ошибка);
КонецПроцедуры
...
//  *КлючЗамера - Строка
//  *ОповещениеОЗавершении - ОписаниеОповещения
Процедура ПослеЗамера(Результат, ДополнительныеПараметры) Экспорт
	ЗакончитьЗамер(ДополнительныеПараметры.КлючЗамера);
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеОЗавершении, Результат);
КонецПроцедуры
