...
// Добавляет общие свойства по настройке формата поля курса валюты на форме платежного документа
//
// Параметры:
//	Настройки - ТаблицаЗначений - таблица с колонками:
//		* Поля - Массив из Строка - поля, для которых определяются настройки отображения
//		* Условие - ОтборКомпоновкиДанных - условия применения настройки
//		* Свойства - Структура - имена и значения свойств
Процедура ЗаполнитьУсловноеОформлениеТочностьКурса(Настройки) Экспорт
	...
	ГруппаИ = ФинансоваяОтчетностьСервер.НовыйОтбор(Элемент.Условие,,, Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ГруппаИ.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ;
	
	ФинансоваяОтчетностьСервер.НовыйОтбор(ГруппаИ, "Дополнительно.ВалютаРеглУчета", Новый ПолеКомпоновкиДанных("Дополнительно.ВалютаВзаиморасчетов"), , ВидСравненияКомпоновкиДанных.НеРавно);
	ФинансоваяОтчетностьСервер.НовыйОтбор(ГруппаИ, "Дополнительно.ВалютаРеглУчета", Новый ПолеКомпоновкиДанных("Валюта"), , ВидСравненияКомпоновкиДанных.НеРавно);
	
	Элемент.Свойства.Вставить("ФорматРедактирования", "ЧДЦ=8");
	...
	ФинансоваяОтчетностьСервер.НовыйОтбор(ГруппаИли, "Дополнительно.ВалютаРеглУчета", Новый ПолеКомпоновкиДанных("Дополнительно.ВалютаВзаиморасчетов"), , ВидСравненияКомпоновкиДанных.Равно);
	ФинансоваяОтчетностьСервер.НовыйОтбор(ГруппаИли, "Дополнительно.ВалютаРеглУчета", Новый ПолеКомпоновкиДанных("Валюта"), , ВидСравненияКомпоновкиДанных.Равно);
	
	Элемент.Свойства.Вставить("ФорматРедактирования", СтрШаблон("ЧДЦ=%1", БазоваяТочностьКурса));
	
КонецПроцедуры

// Настраивает формат редактирования точности курса в колонке расшифровки платежа списком
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - Форма. 
//
Процедура УстановитьОформлениеТочностьКурса(Форма) Экспорт
	
	БазоваяТочностьКурса = Метаданные.ОпределяемыеТипы.КурсВалюты.Тип.КвалификаторыЧисла.РазрядностьДробнойЧасти;
	
	ВалютаРеглУчета = Форма.ВалютаРеглУчета;
	ВалютаДокумента = Форма.Объект.Валюта;
	Если ВалютаДокумента = ВалютаРеглУчета Тогда
		Форма.Элементы.РасшифровкаПлатежаКурсЧислительВзаиморасчетов.ФорматРедактирования = СтрШаблон("ЧДЦ=%1", БазоваяТочностьКурса);
	Иначе
		ВсеВалютыВзаиморасчетовРавныВалютеРеглУчета = Истина;
		Для Каждого СтрокаРасшифровки Из Форма.Объект.РасшифровкаПлатежа Цикл
			Если ЗначениеЗаполнено(СтрокаРасшифровки.ВалютаВзаиморасчетов)
				И СтрокаРасшифровки.ВалютаВзаиморасчетов <> ВалютаРеглУчета Тогда
				ВсеВалютыВзаиморасчетовРавныВалютеРеглУчета = Ложь;
			КонецЕсли;
		КонецЦикла;
		Если ВсеВалютыВзаиморасчетовРавныВалютеРеглУчета Тогда
			Форма.Элементы.РасшифровкаПлатежаКурсЧислительВзаиморасчетов.ФорматРедактирования = СтрШаблон("ЧДЦ=%1", БазоваяТочностьКурса);
		Иначе
			Форма.Элементы.РасшифровкаПлатежаКурсЧислительВзаиморасчетов.ФорматРедактирования = "ЧДЦ=8";
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьПараметрыВыбораЗаявки(Объект, ЭлементЗаявка) Экспорт
	...
	СписокФормОплаты = Новый Массив;
	
	Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.СписаниеБезналичныхДенежныхСредств") Тогда
		
		Если ПолучитьФункциональнуюОпцию("ПоддержкаПлатежейРФ") Тогда
			СписокФормОплаты = ДенежныеСредстваКлиентСерверЛокализация.ФормыОплатыЗаявокНаБезналичнуюОплату();
		Иначе
			СписокФормОплаты.Добавить(Перечисления.ФормыОплаты.Безналичная);
		КонецЕсли;
		
		МассивПараметровВыбора.Добавить(Новый ПараметрВыбора("Отбор.ФормаОплаты", СписокФормОплаты));
		МассивПараметров.Добавить(
	...
	ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.РасходныйКассовыйОрдер") Тогда
		...
		СписокФормОплаты.Добавить(Перечисления.ФормыОплаты.Наличная);
	...
	ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ОперацияПоПлатежнойКарте") Тогда
		...
		СписокФормОплаты.Добавить(Перечисления.ФормыОплаты.ПлатежнаяКарта);
		МассивПараметровВыбора.Добавить(Новый ПараметрВыбора("Отбор.ФормаОплаты", СписокФормОплаты));
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаДенежныхСредствПодотчетнику Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьЗаявкиНаРасходованиеДенежныхСредств") Тогда
			ТипыЗаявок.Добавить("ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств");
		КонецЕсли;
		
		Если ПолучитьФункциональнуюОпцию("ИспользоватьЗаявкиНаКомандировку") Тогда
			ТипыЗаявок.Добавить("ДокументСсылка.ЗаявкаНаКомандировку");
		КонецЕсли;
		
		Если ТипыЗаявок.Количество() <> 0 Тогда
			ЭлементЗаявка.ОграничениеТипа = Новый ОписаниеТипов(СтрСоединить(ТипыЗаявок, ","));
		КонецЕсли;
...
// Удаляет формы оплаты, недоступные для выбора, из списка по умолчанию.
// 
// Параметры:
//  Элемент - ПолеФормы - элемент "Форма оплаты".
//
Процедура ИсключитьНедоступныеФормыОплаты(Элемент) Экспорт

	ДоступныеФормыОплаты = ДоступныеФормыОплатыДляВыбора();
	
	СписокНаУдаление = Новый Массив;
	
	Для Каждого ЭлементВыбора Из Элемент.СписокВыбора Цикл
		
		Если ДоступныеФормыОплаты.Найти(ЭлементВыбора.Значение) = Неопределено Тогда
			СписокНаУдаление.Добавить(ЭлементВыбора);
		КонецЕсли;
		
	КонецЦикла;
	
	Для Каждого ЭлементУдаления Из СписокНаУдаление Цикл
		Элемент.СписокВыбора.Удалить(ЭлементУдаления);
	КонецЦикла;

КонецПроцедуры

// Возвращает список форм оплаты, доступных для выбора.
// 
// Параметры:
//  ИспользуетсяЛюбаяФормаОплаты - Булево - используется любая форма оплаты
// 
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.ФормыОплаты - Доступные формы оплаты для выбора.
Функция ДоступныеФормыОплатыДляВыбора(ИспользуетсяЛюбаяФормаОплаты = Истина) Экспорт

	ФормыОплаты = Новый Массив;
	ФормыОплаты.Добавить(Перечисления.ФормыОплаты.Наличная);
	ФормыОплаты.Добавить(Перечисления.ФормыОплаты.Безналичная);
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОплатуПлатежнымиКартами") Тогда
		ФормыОплаты.Добавить(Перечисления.ФормыОплаты.ПлатежнаяКарта);
	КонецЕсли;
	
	ФормыОплаты.Добавить(Перечисления.ФормыОплаты.Взаимозачет);
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьБонусныеПрограммыЛояльности") Тогда
		ФормыОплаты.Добавить(Перечисления.ФормыОплаты.БонусныеБаллы);
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодарочныеСертификаты") Тогда
		ФормыОплаты.Добавить(Перечисления.ФормыОплаты.ПодарочныйСертификат);
	КонецЕсли;
	
	ДенежныеСредстваСерверЛокализация.ДобавитьДоступныеФормыОплаты(ФормыОплаты);
	
	Если ИспользуетсяЛюбаяФормаОплаты Тогда
		ФормыОплаты.Добавить(Перечисления.ФормыОплаты.ПустаяСсылка());
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыДокументаПоФормеОплаты(ФормаОплаты, ДанныеЗаполнения, ВалютаПлатежа = Неопределено) Экспорт
	...
	ИначеЕсли ФормаОплаты = Перечисления.ФормыОплаты.Безналичная
		Или ФормаОплаты = Перечисления.ФормыОплаты.СистемаБыстрыхПлатежей Тогда
		
		СтруктураПараметров = ПараметрыЗаполненияБанковскогоСчетаОрганизации();
		СтруктураПараметров.Организация    = ДанныеЗаполнения.Организация;
		СтруктураПараметров.Валюта         = ВалютаПлатежа;
		
		БанковскийСчет = ПолучитьБанковскийСчетОрганизацииПоУмолчанию(СтруктураПараметров,
			ДанныеЗаполнения.БанковскийСчет, ФормаОплаты);
...
Процедура ЗаполнитьДокументПоЗаявкамНаРасходованиеДенежныхСредств(ДанныеЗаполнения, ТабличнаяЧасть, ФормаОплаты, Документ = Неопределено) Экспорт
	...
	|	ВЫБОР
	|		КОГДА Заявка.ФормаОплатыЗаявки = ЗНАЧЕНИЕ(Перечисление.ФормыОплаты.СистемаБыстрыхПлатежей)
	...
	Если ЗначениеЗаполнено(БанковскийСчетКасса) Тогда
		Если ФормаОплаты = Перечисления.ФормыОплаты.Безналичная
			Или ФормаОплаты = Перечисления.ФормыОплаты.СистемаБыстрыхПлатежей Тогда
	...
	//++ Локализация
	Если ДанныеЗаполнения.ИННПлательщика = Неопределено Тогда
		
		РеквизитыПлательщика = ДенежныеСредстваСерверЛокализация.РеквизитыПлательщика(ДанныеЗаполнения, ТекущаяДатаСеанса());
		ДанныеЗаполнения.ИННПлательщика = РеквизитыПлательщика.ИННПлательщика;
		ДанныеЗаполнения.КПППлательщика = РеквизитыПлательщика.КПППлательщика;
		ДанныеЗаполнения.ТекстПлательщика = РеквизитыПлательщика.ТекстПлательщика;
	...
	Если ДанныеЗаполнения.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.КонвертацияВалюты Тогда
		СтруктураПараметров = ПараметрыЗаполненияБанковскогоСчетаОрганизации();
		СтруктураПараметров.Организация = ДанныеЗаполнения.Организация;
		СтруктураПараметров.Валюта = ДанныеЗаполнения.ВалютаКонвертации;
	...
	|	РасшифровкаДокумента.Партнер,
	|	РасшифровкаДокумента.Контрагент,
	|	РасшифровкаДокумента.Ссылка.Организация,
	|	ВЫБОР
	...
	|	РасшифровкаПлатежа.Организация,
...
//
Процедура ПроверитьЗаполнитьБанковскийСчетОрганизацииПоВладельцу(Знач Организация, БанковскийСчет,
	ФормаОплаты = Неопределено, НаправлениеДеятельности = Неопределено, ОплатаВВалюте = Неопределено) Экспорт
	...
	СтруктураПараметров = ПараметрыЗаполненияБанковскогоСчетаОрганизации(Организация);
	СтруктураПараметров.НаправлениеДеятельности = НаправлениеДеятельности;
	Если ОплатаВВалюте <> Неопределено Тогда
		СтруктураПараметров.ИсключитьСчетаВВалюте = ОплатаВВалюте;
		СтруктураПараметров.Валюта = ЗначениеНастроекПовтИсп.ВалютаРегламентированногоУчетаОрганизации(Организация);
	КонецЕсли;
	
	БанковскийСчет = ПолучитьБанковскийСчетОрганизацииПоУмолчанию(СтруктураПараметров, БанковскийСчет, ФормаОплаты);
...
Функция ПолучитьВалютуОплаты(ФормаОплаты = Неопределено, БанковскийСчет = Неопределено, Касса = Неопределено) Экспорт
	...
	Если (ФормаОплаты = Перечисления.ФормыОплаты.Безналичная 
		...
		Или Не ЗначениеЗаполнено(ФормаОплаты))
...
//
Функция ОплатитьСтрокиГрафика(СтрокиГрафика, ТипДокумента, Группировать = Истина) Экспорт
	...
	ДокументыКСозданию = ДокументыКСозданию(СтрокиГрафика, ТипДокумента, Группировать);
...
Функция ДокументыКСозданию(СтрокиГрафика, ТипДокумента, Группировать = Истина) Экспорт
	...
	|			КОНЕЦ
	|		КОГДА ГрафикПлатежей.ОбъектОплаты ССЫЛКА Документ.ПриобретениеТоваровУслуг
	|		ТОГДА ВЫРАЗИТЬ(ГрафикПлатежей.ОбъектОплаты КАК Документ.ПриобретениеТоваровУслуг).БанковскийСчетКонтрагента
	|		КОГДА ГрафикПлатежей.ОбъектОплаты ССЫЛКА Документ.ПриобретениеУслугПрочихАктивов
	|		ТОГДА ВЫРАЗИТЬ(ГрафикПлатежей.ОбъектОплаты КАК Документ.ПриобретениеУслугПрочихАктивов).БанковскийСчетКонтрагента
	|		ИНАЧЕ
	...
	|			ЛОЖЬ
	|		КОГДА ГрафикПлатежей.ФормаОплаты = ЗНАЧЕНИЕ(Перечисление.ФормыОплаты.СистемаБыстрыхПлатежей) ТОГДА
	|			ЛОЖЬ
	...
	Запрос.УстановитьПараметр("Группировать", Группировать);
	...
	Пока ВыборкаГруппировки.Следующий() Цикл
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ВыборкаГруппировки.БанковскийСчетКонтрагента) Тогда
				ДанныеЗаполнения.Вставить("БанковскийСчетКонтрагента", ВыборкаГруппировки.БанковскийСчетКонтрагента);
			КонецЕсли;
	...
	Пока ВыборкаСтроки.НайтиСледующий(Новый Структура("Группируется", Ложь)) Цикл
		...
		Иначе
			...
			Иначе
				...
				ЗаполнитьЗначенияСвойств(ОтборПлатежей, ВыборкаСтроки);
				ОтобранныеПлатежи = ТаблицаОплаты.НайтиСтроки(ОтборПлатежей);
				
				ДанныеЗаполнения.Вставить("Основание",
					ОбщегоНазначения.ТаблицаЗначенийВМассив(ТаблицаОплаты.Скопировать(ОтобранныеПлатежи)));
			КонецЕсли;
		КонецЕсли;
...
Процедура УдалитьПлатежи(СсылкиНаУдаление, АдресРезультата) Экспорт
	...
	Для каждого Ссылка Из ТабСсылок Цикл
		Если ТипЗнч(Ссылка[1]) = Тип("РегистрСведенийКлючЗаписи.ЗаданияКРаспределениюРасчетовСКлиентами")
			...
			Или ТипЗнч(Ссылка[1]) = Тип("РегистрСведенийКлючЗаписи.УдалитьВспомогательнаяИнформацияВзаиморасчетов")
			Или ТипЗнч(Ссылка[1]) = Тип("РегистрСведенийКлючЗаписи.ЗаданияКРаспределениюВзаиморасчетов")
			Или ТипЗнч(Ссылка[1]) = Тип("РегистрСведенийКлючЗаписи.ЗаданияНеоперативногоДопроведенияДокументов")
			//++ Локализация
...
Функция СформироватьДанныеЗаполненияПоВозвратуПлатежаОтЗаказКлиента(ЗаказКлиента, ДокументОбъектЗаполнения, ФормаОплаты = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|			ТОГДА ЗаказКлиента.Договор.БанковскийСчетКонтрагента
		|		ИНАЧЕ ЗаказКлиента.БанковскийСчетКонтрагента
		|	КОНЕЦ КАК БанковскийСчетКонтрагента,
		...
		|			ИЛИ &ФормаОплаты = ЗНАЧЕНИЕ(Перечисление.ФормыОплаты.СистемаБыстрыхПлатежей)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
...
Процедура ЗаполнитьРасшифровкуПлатежаПоЗаказуКлиента(ДанныеЗаполнения, ТабличнаяЧастьРасшифровкаПлатежа) Экспорт
	...
	ПараметрыДанных = ВзаиморасчетыЗаполнениеРасшифровкиПлатежа.ПараметрыЗаполненияРасшифровкиПлатежаПоЗаказу();
	...
	ВзаиморасчетыЗаполнениеРасшифровкиПлатежа.ЗаполнитьРасшифровкуПлатежаПоЗаказуКлиента(ПараметрыДанных,
...
// Возвращает текущий банковский счет организации из общих настроек.
//
// Возвращаемое значение:
//   СправочникСсылка.БанковскиеСчетаОрганизаций - текущий банковский счет организации или пустая ссылка
//
Функция ТекущийБанковскийСчетОрганизации() Экспорт

	Результат = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ТекущийБанковскийСчет", ""); // СправочникСсылка.БанковскиеСчетаОрганизаций
	
	Если Результат = Неопределено Тогда
		Возврат Справочники.БанковскиеСчетаОрганизаций.ПустаяСсылка();
	ИначеЕсли ЗначениеЗаполнено(Результат)
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Результат, "ЭтоГруппа") Тогда
		Возврат Справочники.БанковскиеСчетаОрганизаций.ПустаяСсылка();
	КонецЕсли;
...
// Получает банковский счет организации по следующим правилам:
// Если переданный в качестве параметра банковский счет уже заполнен - возвращает его.
// Если переданная в качестве параметра форма оплаты не предусматривает заполнение банковского счета - возвращает пустую
// ссылку на банковский счет.
// Если определенный по переданным параметрам банковский счет единственный в информационной базе - возвращает его.
// Если банковский счет в базе не один - возвращает пустую ссылку на банковский счет.
//
// Параметры:
//   ПараметрыСчета - см. ПараметрыЗаполненияБанковскогоСчетаОрганизации.
...
//
Функция ПолучитьБанковскийСчетОрганизацииПоУмолчанию(ПараметрыСчета = Неопределено, БанковскийСчет = Неопределено,
	ФормаОплаты = Неопределено) Экспорт
	
	Если ЗначениеЗаполнено(БанковскийСчет) Тогда
		Возврат БанковскийСчет;
	ИначеЕсли Не ЗначениеЗаполнено(ФормаОплаты)
		Или ФормаОплаты = Перечисления.ФормыОплаты.Безналичная
		Или ФормаОплаты = Перечисления.ФормыОплаты.СистемаБыстрыхПлатежей Тогда
		
		Если ПараметрыСчета = Неопределено Тогда
			ПараметрыСчета = ПараметрыЗаполненияБанковскогоСчетаОрганизации();
		КонецЕсли;
		
		Возврат ЗначениеНастроекПовтИсп.БанковскийСчетОрганизацииПоУмолчанию(ПараметрыСчета);
	Иначе
		Возврат Справочники.БанковскиеСчетаОрганизаций.ПустаяСсылка();
	КонецЕсли;
...
// Получает банковский счет контрагента по следующим правилам:
// Если переданный в качестве параметра банковский счет уже заполнен - возвращает его.
// Если переданная в качестве параметра форма оплаты не предусматривает заполнение банковского счета или в параметрах
// счета не указан Контрагент - возвращает пустую ссылку на банковский счет.
// Если определенный по переданным параметрам банковский счет единственный в информационной базе - возвращает его.
// Если банковский счет в базе не один - возвращает пустую ссылку на банковский счет.
//
// Параметры:
//  ПараметрыСчета - см. ПараметрыЗаполненияБанковскогоСчетаКонтрагента.
...
//
Функция ПолучитьБанковскийСчетКонтрагентаПоУмолчанию(ПараметрыСчета, БанковскийСчет = Неопределено,
	ФормаОплаты = Неопределено) Экспорт
	
	Если ЗначениеЗаполнено(БанковскийСчет) Тогда
		Возврат БанковскийСчет;
	ИначеЕсли ЗначениеЗаполнено(ПараметрыСчета.Контрагент)
		И (Не ЗначениеЗаполнено(ФормаОплаты)
		Или ФормаОплаты = Перечисления.ФормыОплаты.Безналичная
		Или ФормаОплаты = Перечисления.ФормыОплаты.СистемаБыстрыхПлатежей) Тогда
		
		Возврат ЗначениеНастроекПовтИсп.БанковскийСчетКонтрагентаПоУмолчанию(ПараметрыСчета);
	Иначе
...
Процедура ОчиститьНеиспользуемыеРеквизитыФормыОплаты(ДокументОбъект, ФормаОплаты, ОчищатьДляПустойФормыОплаты = Ложь) Экспорт
	...
	КонецЕсли;
	Если ФормаОплаты <> Перечисления.ФормыОплаты.Безналичная
		И ФормаОплаты <> Перечисления.ФормыОплаты.СистемаБыстрыхПлатежей Тогда
...
Процедура ПересчетСуммДокументаВВалюту(Объект, ТекущаяВалюта, НоваяВалюта) Экспорт
	...
	Если Не ЗначениеЗаполнено(ТекущаяВалюта)
		Или Не ЗначениеЗаполнено(НоваяВалюта) Тогда
		Возврат;
	КонецЕсли;
...
// Устарела. Возвращает параметры вызова функции ПолучитьБанковскийСчетОрганизацииПоУмолчанию
//	
// Возвращаемое значение:
...
// Возвращает параметры вызова функции ПолучитьБанковскийСчетКонтрагентаПоУмолчанию.
//
// Параметры:
//	Контрагент - СправочникСсылка.Контрагенты, СправочникСсылка.ФизическиеЛица - Владелец банковского счета.
//
// Возвращаемое значение:
//    Структура - Структура параметров для определения счета:
//		* Контрагент            - СправочникСсылка.Контрагенты, СправочникСсылка.ФизическиеЛица, Неопределено -
//								Контрагент, для которого необходимо получить счет.
//		* Валюта                - СправочникСсылка.Валюты, Неопределено - Валюта счета.
//		* КорреспондирующийСчет - СправочникСсылка.БанковскиеСчетаКонтрагентов, Неопределено - Корреспондирующий счет.
//		* ИсключитьСчетаВВалюте - Булево - если Истина, будут исключены счета в валюте, переданной в параметре Валюта.
//
Функция ПараметрыЗаполненияБанковскогоСчетаКонтрагента(Контрагент = Неопределено) Экспорт
	
	Структура = Новый Структура;
	Структура.Вставить("Контрагент", Контрагент);
	Структура.Вставить("Валюта", Неопределено);
	Структура.Вставить("КорреспондирующийСчет", Неопределено);
	Структура.Вставить("ИсключитьСчетаВВалюте", Ложь);
...
// Возвращаемое значение:
//    Структура - Структура параметров для определения счета:
//		* Организация             - СправочникСсылка.Организации, Неопределено - Организация, для которой необходимо
//									получить счет.
//		* Валюта                  - СправочникСсылка.Валюты, Неопределено - Валюта счета.
//		* НаправлениеДеятельности - СправочникСсылка.НаправленияДеятельности, Неопределено -
//									Желательное направление деятельности счета.
//		* КорреспондирующийСчет   - СправочникСсылка.БанковскиеСчетаОрганизаций, Неопределено - Корреспондирующий счет.
//		* ИсключитьСчетаВВалюте   - Булево - если Истина, будут исключены счета в валюте, переданной в параметре Валюта.
//
Функция ПараметрыЗаполненияБанковскогоСчетаОрганизации(Организация = Неопределено) Экспорт
	
	Структура = Новый Структура;
	Структура.Вставить("Организация", Организация);
	Структура.Вставить("Валюта", Неопределено);
	Структура.Вставить("НаправлениеДеятельности", Неопределено);
	Структура.Вставить("КорреспондирующийСчет", Неопределено);
	Структура.Вставить("ИсключитьСчетаВВалюте", Ложь);
...
Процедура ДобавитьЭтап_ПроверкаКурсовЗачетаВАвансовыхОтчетах(ТаблицаЭтапов, ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(
		ТаблицаЭтапов, ТекущийРодитель, Справочники.ОперацииЗакрытияМесяца.ПроверкаКурсовЗачетаВАвансовыхОтчетах);
		
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Проверка курсов зачета в авансовых отчетах'"; // @НСтр-1
...
#Область ПолучениеДанныхПоДокументам

// Возвращает значения реквизитов документа или неопределено.
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка - Документ, в котором используется СБП.
//  ТипРасчетов - ПеречислениеСсылка.ТипыРасчетовСПартнерами - Тип расчетов с партнером.
//  ВыбранныеРеквизиты - Соответствие из КлючИЗначение - реквизиты, которые необходимо получить по документу.
//  ПолучитьСуммыВзаиморасчетов - Булево - признак получения суммы взаиморасчетов.
// 
// Возвращаемое значение:
//  Структура - данные документа:
//  *ДокументОперации - ДокументСсылка, СправочникСсылка.ДоговорыКонтрагентов - документ или договор, по которому выполняется операция оплаты.
//  *ДокументОснование - ДокументСсылка, Неопределено - основание документа операции.
//                       Тип Неопределено может быть, если параметр ДокументСсылка введен без основания.
//                       Обязательно заполнен если у параметра ДокументСсылка тип ДокументСсылка.СчетНаОплатуКлиенту.
//  *Организация - СправочникСсылка.Организации - организация документа.
//  *ФормаОплаты - ПеречислениеСсылка.ФормыОплаты - форма оплаты документа.
//  *БанковскийСчетКонтрагента - Неопределено, СправочникСсылка.БанковскиеСчетаОрганизаций, СправочникСсылка.БанковскиеСчетаКонтрагентов - банковский счет получателя.
//  *БанковскийСчетОрганизации - СправочникСсылка.БанковскиеСчетаОрганизаций - банковский счет организации.
//  *НомерСчета - Строка - номер банковского счета организации.
//  *ВидКонтрагента - ПеречислениеСсылка.ЮрФизЛицо - вид получателя.
//  *Контрагент - СправочникСсылка.Контрагенты, СправочникСсылка.Организации - получатель.
//  *ИНН - Строка - ИНН получателя.
//  *Проведен - Булево - признак проведения документа в учете.
//  *ПорядокРасчетов - ПеречислениеСсылка.ПорядокРасчетов - порядок расчетов в документе.
//  *ИдентификаторПлатежа - Строка - идентификатор платежа документа.
//  *ДатаПлатежа - Дата, Неопределено - дата платежа по документу.
//  *СуммаДокумента - Число - сумма документа.
//  *ВалютаДокумента - СправочникСсылка.Валюты - валюта документа.
//  *НазначениеПлатежа - Строка - назначение платежа по документу.
//  *ЭтапыОплаты - Неопределено, Строка - таблица этапов оплаты, помещенное во временной хранилище.
//  *ЭтоЗаказ - Булево - признак документа-заказа.
//  *ЭтоПродажаЗакупка - Булево - признак документа-накладной.
//  *ИзменяетПланОплаты - Булево - уменьшает или увеличивает план оплаты.
//  *ЗаказКакСчет - Булево - заказ используется как счет.
//  *Склад - СправочникСсылка.Склады, Неопределено - склад по документу.
//  *ДополнительнаяИнформация - Строка - дополнительная информация.
//
Функция ДанныеДокумента(ДокументСсылка, ТипРасчетов, ВыбранныеРеквизиты, ПолучитьСуммыВзаиморасчетов = Ложь) Экспорт

	СтруктураРеквизитов = ПараметрыПоТипуРасчетов(ДокументСсылка, ТипРасчетов);
	
	Если СтруктураРеквизитов = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не определены параметры взаиморасчетов документа.'");
	КонецЕсли;
	
	ДокументМетаданные = ДокументСсылка.Метаданные();
	ПолноеИмя = ДокументМетаданные.ПолноеИмя();
	
	СхемаЗапроса = Новый СхемаЗапроса;
	ЗапросВыбора = СхемаЗапроса.ПакетЗапросов[0];
	ОператорВыбора = ЗапросВыбора.Операторы[0];
	ОператорВыбора.Источники.Добавить(ПолноеИмя, "Объект");
	ОператорВыбора.Отбор.Добавить("Объект.Ссылка = &Ссылка");
	
	Если ОбщегоНазначения.ЭтоДокумент(ДокументМетаданные) Тогда
		ДобавитьВыборкуПоляВСхемуЗапроса(ЗапросВыбора, "Объект.Проведен", "", "Проведен");
	Иначе
		ДобавитьВыборкуПоляВСхемуЗапроса(ЗапросВыбора, "", "", "Проведен", "Ложь");
	КонецЕсли;
	
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СчетНаОплатуКлиенту") Тогда
		ДобавитьВыборкуПоляВСхемуЗапроса(ЗапросВыбора, СтруктураРеквизитов.ДокументОснование, "", "ДокументОперации");
	Иначе
		ДобавитьВыборкуПоляВСхемуЗапроса(ЗапросВыбора, "Объект.Ссылка", "", "ДокументОперации");
	КонецЕсли;
	
	Для Каждого ТекущийРеквизит Из ВыбранныеРеквизиты Цикл
		
		ПозицияТочки = СтрНайти(ТекущийРеквизит.Ключ, ".");
		
		Если ПозицияТочки > 0 Тогда
			ИмяРеквизита = СокрЛП(Сред(ТекущийРеквизит.Ключ, 1, ПозицияТочки - 1));
			ПодчиненныйРеквизит = СокрЛП(Сред(ТекущийРеквизит.Ключ, ПозицияТочки + 1, СтрДлина(ТекущийРеквизит.Ключ)));
		Иначе
		...
		ЗначениеПоУмолчанию = 
			?(ТипЗнч(СтруктураРеквизитов[ИмяРеквизита]) = Тип("Булево"), Формат(СтруктураРеквизитов[ИмяРеквизита], "БЛ=Ложь; БИ=Истина"), "Неопределено");
		ДобавитьВыборкуПоляВСхемуЗапроса(ЗапросВыбора, СтруктураРеквизитов[ИмяРеквизита], ПодчиненныйРеквизит, ТекущийРеквизит.Значение, ЗначениеПоУмолчанию);
	...
	Если Выборка.Следующий() Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Выборка, "ЭтапыОплаты")
			И ЗначениеЗаполнено(Выборка.ЭтапыОплаты) Тогда
			Результат.ЭтапыОплаты = ПоместитьВоВременноеХранилище(Выборка.ЭтапыОплаты.Выгрузить(), Новый УникальныйИдентификатор);
		КонецЕсли;
	
	Иначе
		ВызватьИсключение НСтр("ru = 'Не определены данные документа.'");
	КонецЕсли;
	
	Если ПолучитьСуммыВзаиморасчетов Тогда
		
		СостояниеВзаиморасчетов = ВзаиморасчетыСервер.СостояниеВзаиморасчетов(ДокументСсылка, СтруктураРеквизитов);
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(Результат, СостояниеВзаиморасчетов, Истина);
...
// Возвращает вид контрагента документа операции.
// 
// Параметры:
//  Документ - ДокументСсылка - Документ, по которому необходимо получить вид контрагента.
//  ТипРасчетов - ПеречислениеСсылка.ТипыРасчетовСПартнерами - Тип расчетов с партнером.
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.ЮрФизЛицо - Вид контрагента документа операции
Функция ВидКонтрагентаДокументаОперации(Документ, ТипРасчетов) Экспорт

	ВидКонтрагента = Перечисления.ЮрФизЛицо.ПустаяСсылка();
	
	Если Документ.Пустая() Тогда
		Возврат ВидКонтрагента;
	КонецЕсли;
	
	СтруктураПараметров = ПараметрыПоТипуРасчетов(Документ, ТипРасчетов);
	
	Если СтруктураПараметров = Неопределено Тогда
		
		СтруктураПараметров = Новый Структура;
		
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Документ, "Контрагент") Тогда
	...
	Если СтруктураПараметров <> Неопределено Тогда
		...
		ЗапросВыбора = СхемаЗапроса.ПакетЗапросов[0];
		ОператорВыбора = ЗапросВыбора.Операторы[0];
		ДополнитьОписаниеВыборкиПоДокументу(ОператорВыбора, Документ);
		ДобавитьВыборкуПоляВСхемуЗапроса(ЗапросВыбора, СтруктураПараметров.Контрагент, "ЮрФизЛицо", "ВидКонтрагента");
		...
		Если Выборка.Следующий() Тогда
			ВидКонтрагента = ?(Выборка.ВидКонтрагента = Неопределено, ВидКонтрагента, Выборка.ВидКонтрагента);
		КонецЕсли;
...
// 
// Параметры:
//   ДокументОплаты - ДокументСсылка - документ оплаты (совпадает с ДокументОснование, кроме случая счета на оплату клиенту)
//   ДокументОснование - ДокументСсылка, СправочникСсылка.ДоговорыКонтрагентов - документ-основание или договор (для счета на оплату)
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Суммы документа:
//  * СуммаНДС - Число - сумма НДС.
//  * СуммаСНДС - Число - сумма с НДС.
//  * СтавкаНДС - Число - ставка НДС
//
Функция СуммыДокумента(ДокументОплаты, ДокументОснование) Экспорт

	ЭтоСчет = ТипЗнч(ДокументОплаты) = Тип("ДокументСсылка.СчетНаОплатуКлиенту");
	...
	Если ЭтоСчет Тогда
		...
		Запрос.Текст = 
			...
			|ГДЕ
			|	ЭтапыГрафикаОплаты.Ссылка = &ДокументОплаты";
		
		Запрос.УстановитьПараметр("ДокументОплаты", ДокументОплаты);
		ЭтапыГрафикаОплаты = Запрос.Выполнить().Выгрузить();
		
		ТаблицаСумм = СуммыСчетаНаОплату(ДокументОснование, ЭтапыГрафикаОплаты);
		
	Иначе
		
		ТекстыЗапросов = Новый Массив;
		ТекстыЗапросов.Добавить(ТекстЗапросаСуммПолнойОплаты(ДокументОснование));
		
		ИтоговаяВыборка =
			"ВЫБРАТЬ
			|	ТаблицаСумм.СтавкаНДС КАК СтавкаНДС,
			|	ВЫРАЗИТЬ(ТаблицаСумм.СуммаНДС КАК ЧИСЛО(31,2)) КАК СуммаНДС,
			|	ВЫРАЗИТЬ(ТаблицаСумм.СуммаСНДС КАК ЧИСЛО(31,2)) КАК СуммаСНДС
			|ИЗ
			|	&ТаблицаСумм КАК ТаблицаСумм";
		
		ИтоговаяВыборка = СтрЗаменить(ИтоговаяВыборка, "&ТаблицаСумм", "СуммыПолнойОплаты");
		
		ТекстыЗапросов.Добавить(ИтоговаяВыборка);
		
		Запрос = Новый Запрос;
		Запрос.Текст = СтрСоединить(ТекстыЗапросов, ";");
		Запрос.УстановитьПараметр("ДокументОперации", ДокументОснование);
...
// Возвращает параметры расчетов с клиентом.
// 
// Параметры:
//  Документ - ДокументСсылка - Документ
//  ТипРасчетов - ПеречислениеСсылка.ТипыРасчетовСПартнерами - тип расчета, по которому определяются параметры для выборки.
// 
// Возвращаемое значение:
//  см. ВзаиморасчетыСервер.ПараметрыМеханизма
//
Функция ПараметрыПоТипуРасчетов(Документ, ТипРасчетов) Экспорт

	МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоСсылке(Документ);
	
	ПараметрыМеханизма = МенеджерОбъекта.ПараметрыВзаиморасчеты(Документ);
	...
	Для Каждого ПараметрыДокумента Из МассивПараметров Цикл
		
		Если ПараметрыДокумента.ТипРасчетов <> ТипРасчетов Тогда
			Продолжить;
		КонецЕсли;
		
		СтруктураПараметров = ОбщегоНазначения.СкопироватьРекурсивно(ПараметрыДокумента);
...
// Возвращает назначение платежа по документу операции.
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка - документ операции.
//  ДанныеДокумента - Структура -Данные документа операции:
//  *Номер - Строка - номер документа.
//  *Дата - Дата - дата документа.
//  *КодВалюты - Строка - код валюты документа.
//  *Валюта - СправочникСсылка.Валюты - валюта документа.
//  *ПорядокРасчетов - ПеречислениеСсылка.ПорядокРасчетов - порядок расчетов документа операции.
//  *ЭтоЗаказ - Булево - признак того, что документ операции является заказом.
//  *Договор - СправочникСсылка.ДоговорыКонтрагентов - договор в документе операции.
//  ТаблицаСумм - ТаблицаЗначений - Таблица с суммами документа:
//  *СтавкаНДС - СправочникСсылка.СтавкиНДС - ставка НДС.
//  *СуммаНДС - Число - сумма НДС.
//  *СуммаСНДС - Число - сумма с НДС.
// 
// Возвращаемое значение:
//  Строка - Назначение платежа документа операции
Функция НазначениеПлатежаДокументаОперации(ДокументСсылка, ДанныеДокумента, ТаблицаСумм) Экспорт
	...
	Если ДанныеДокумента.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов
		Или (ДанныеДокумента.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоДоговорамНакладным
			И ДанныеДокумента.ЭтоЗаказ) Тогда
		
		ДанныеДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеДокумента.Договор, "Дата, Номер");
		ДатаОснования = Формат(ДанныеДоговора.Дата, "ДЛФ=D");
		НомерОснования = ДанныеДоговора.Номер;
		
		Если ЗначениеЗаполнено(ДатаОснования)
			И ЗначениеЗаполнено(НомерОснования) Тогда
			ПредставлениеОснования = СтрШаблон(НСтр("ru = 'Оплата по договору №%1 от %2'"), НомерОснования, ДатаОснования);
		Иначе
			ПредставлениеОснования = НСтр("ru = 'Оплата по договору'");
		КонецЕсли;
		
	Иначе
		
		ДатаОснования = Формат(ДанныеДокумента.Дата, "ДЛФ=D");
		НомерОснования = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеДокумента.Номер);
		
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПредставленияДокументов, ИмяДокумента) Тогда
			ПредставлениеОснования = СтрШаблон(НСтр("ru = 'Оплата по %1 №%2 от %3'"), ПредставленияДокументов[ИмяДокумента], НомерОснования, ДатаОснования);
		Иначе
			ПредставлениеОснования = СтрШаблон(НСтр("ru = 'Оплата по накладной №%1 от %2'"), НомерОснования, ДатаОснования);
		КонецЕсли;
		
	КонецЕсли;
	
	ШаблонНДС = НСтр("ru = 'НДС (%1) %2 %3'");
	
	ТаблицаНДС = ТаблицаСумм.Скопировать(, "СтавкаНДС, СуммаНДС");
	ТаблицаНДС.Свернуть("СтавкаНДС, СуммаНДС");
	
	Если ДанныеДокумента.КодВалюты = "643" Тогда
		Валюта = НСтр("ru = 'руб.'");
	Иначе
		Валюта = СокрЛП(ДанныеДокумента.Валюта);
	КонецЕсли;
	
	СтрокиСНДС = Новый Массив;
	...
	Для Каждого СтрокаТаблицы Из ТаблицаНДС Цикл
		...
		СтрокиСНДС.Добавить(СтрШаблон(ШаблонНДС, СтрокаТаблицы.СтавкаНДС, Формат(СтрокаТаблицы.СуммаНДС, "ЧДЦ=2; ЧРД=-; ЧН=0-00; ЧГ="), Валюта));
		
	КонецЦикла;
	
	ТекстНДС = СтрСоединить(СтрокиСНДС, ", ");
	...
	Шаблон = НСтр("ru = '%1 Сумма %2 %3 %4'");
	Результат = СтрШаблон(Шаблон, 
		ПредставлениеОснования,
		Формат(ТаблицаСумм.Итог("СуммаСНДС"), "ЧДЦ=2; ЧРД=-; ЧН=0-00; ЧГ="),
		Валюта,
		ТекстНДС);
	
	Результат = СтрЗаменить(Результат, Символ(160), Символ(32));
...
Функция НадписьВалютыИКурсПлатежа(Форма) Экспорт
	...
	БазоваяФорматнаяСтрока = СтрШаблон("ЧДЦ=%1", Метаданные.ОпределяемыеТипы.КурсВалюты.Тип.КвалификаторыЧисла.РазрядностьДробнойЧасти);
	ШаблонКурса = "(%1 %2 = %3 %4)";
	СтрокиНадписи = Новый Массив;
	Для Каждого СтрокаТаблицы Из КопияРасшифровки Цикл
		
		Если СтрокиНадписи.Количество() = 3 Тогда
		...
		Если СтрокаТаблицы.ВалютаВзаиморасчетов <> Объект.Валюта Тогда
			Если Объект.Валюта = Форма.ВалютаРеглУчета Тогда
				ФорматнаяСтрока = БазоваяФорматнаяСтрока;
				СтрокаКурса = СтрШаблон(ШаблонКурса,
					СтрокаТаблицы.КурсЗнаменательВзаиморасчетов,
					СтрокаТаблицы.ВалютаВзаиморасчетов,
					Строка(Формат(СтрокаТаблицы.КурсЧислительВзаиморасчетов, ФорматнаяСтрока)),
					Объект.Валюта);
			Иначе
				Если СтрокаТаблицы.ВалютаВзаиморасчетов = Форма.ВалютаРеглУчета Тогда
					ФорматнаяСтрока = БазоваяФорматнаяСтрока;
				Иначе
					ФорматнаяСтрока = "ЧДЦ=8";
				КонецЕсли;
				СтрокаКурса = СтрШаблон(ШаблонКурса,
					СтрокаТаблицы.КурсЗнаменательВзаиморасчетов,
					Объект.Валюта,
			...
			СтрокиНадписи.Добавить(
				СтрШаблон("%1 %2 %3",
					Формат(СтрокаТаблицы.СуммаВзаиморасчетов, "ЧДЦ=2"),
					СтрокаТаблицы.ВалютаВзаиморасчетов,
					СтрокаКурса));
		Иначе
			СтрокиНадписи.Добавить(
	...
	Возврат СтрШаблон("%1: %2", НСтр("ru = 'Расчеты'"), СтрСоединить(СтрокиНадписи, "; "));
	...
	Если ВалютаДокумента <> ВалютаВзаиморасчетов Тогда 
		...
		Если ВариантКурсаДоговора = Перечисления.ВариантыКурсаДоговора.УстановленныйВДоговоре Тогда
			...
			Иначе
				...
				КурсИКратность = РаботаСКурсамиВалютУТ.ПолучитьКроссКурсВалют(КурсДокумента, КурсВзаиморасчетов, 8);
			КонецЕсли;
		Иначе
			Если ВалютаДокумента = ВалютаРеглУчета Тогда
...
//  ИзмениласьВалютаПлатежа - Булево - признак изменения валюты в объекте.
//
Процедура ОбновитьВалютуВзаиморасчетовРасшифровкиПлатежа(ИмяДокумента, Объект, ВалютаРегламентированногоУчета, ИзмениласьВалютаПлатежа = Истина) Экспорт
	...
	Если ИмяКолонки <> "" Тогда
		...
		Для каждого СтрокаТаблицы Из Объект.РасшифровкаПлатежа Цикл
			...
			ИначеЕсли ИзмениласьВалютаПлатежа Тогда
...
//  ЗаполнитьИдентификаторПлатежа - Булево - признак необходимости заполнения поля "Идентификатор платежа"
//  										в табличной части "Расшифровка платежа" документа.
//
Процедура ЗаполнитьДанныеОбъектаРасчетовРасшифровкиПлатежа(
	ДокументОбъект, ИдентификаторСтроки = Неопределено, ЗаполнитьИдентификаторПлатежа = Ложь) Экспорт
	...
	ИменаРеквизитов.Добавить("Объект");
	
	Если ЗаполнитьИдентификаторПлатежа Тогда
		ИменаРеквизитов.Добавить("ИдентификаторПлатежа");
	КонецЕсли;
	...
	Если ИдентификаторСтроки = Неопределено Тогда
		...
		ДанныеОбъектов = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(СписокОбъектов, ИменаРеквизитов);
		...
		Для Каждого СтрокаТаблицы Из ДокументОбъект.РасшифровкаПлатежа Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.ОбъектРасчетов) Тогда
				...
				Если ЗаполнитьИдентификаторПлатежа Тогда
					СтрокаТаблицы.ИдентификаторПлатежа = ДанныеОбъектов[СтрокаТаблицы.ОбъектРасчетов]["ИдентификаторПлатежа"];
				КонецЕсли;
			...
			Иначе
				...
				Если ЗаполнитьИдентификаторПлатежа Тогда
					СтрокаТаблицы.ИдентификаторПлатежа = "";
				КонецЕсли;
	...
	Иначе
		...
		ДанныеОбъекта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаТаблицы.ОбъектРасчетов, ИменаРеквизитов);
		...
		Если ЗаполнитьИдентификаторПлатежа Тогда
			СтрокаТаблицы.ИдентификаторПлатежа = ДанныеОбъекта.ИдентификаторПлатежа;
		КонецЕсли;
...
Функция ЗапросОстатковЗаявокНаРасходованиеДенежныхСредств(ДанныеОтбора) Экспорт
	...
	|	И (Заявка.ФормаОплатыЗаявки В (&ФормаОплаты))
	...
	ФормыОплаты = ЗначениеПараметраФормыОплатыЗаявки(ДанныеОтбора.ФормаОплаты);
	Запрос.УстановитьПараметр("ФормаОплаты", ФормыОплаты);
...
Функция ПолучательПлатежа(ДанныеДокумента) Экспорт
	...
	ИначеЕсли ДанныеДокумента.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаДенежныхСредствВДругуюОрганизацию
		...
		Если ДанныеДокумента.ФормаОплаты.Найти(Перечисления.ФормыОплаты.Безналичная) <> Неопределено Тогда
			Получатель = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеДокумента.БанковскийСчетПолучатель, "Владелец");
		ИначеЕсли ДанныеДокумента.ФормаОплаты.Найти(Перечисления.ФормыОплаты.Наличная) <> Неопределено Тогда
...
//  
// Возвращаемое значение:
//  Массив из Число - индексы строк, в которых изменились объекты расчетов
Функция ПроверитьЗаполнитьОбъектРасчетов(ДанныеДокумента, ИдентификаторСтроки = Неопределено) Экспорт
		...
		Если ПорядокРасчетовДоговора = Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов
			...
			Если ЗначениеЗаполнено(ОбъектРасчетовДоговор) Тогда
				...
				Если ИдентификаторСтроки <> Неопределено Тогда
					...
					ИзмененныеСтроки.Добавить(ДанныеДокумента.РасшифровкаПлатежа.Индекс(СтрокаРасшифровки));
				Иначе
					ОбъектыРасчетов = ОбъектыРасчетовСервер.ВсеОбъектыРасчетовПоСсылке(ДанныеДокумента.Ссылка);
...
Процедура ЗаполнитьКурсВзаиморасчетовСтрокиРасшифровки(Объект, ВалютаРегламентированногоУчета, ДатаКурса, СтрокаРасшифровки) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "СписокКонтрагентов")
		...
		// Если договор в строке расшифровки заполнен, то берем его, если нет - из шапки
		Или ЗначениеЗаполнено(ПараметрыЗаполнения.Договор) Тогда
...
Процедура ЗаполнитьДанныеФормыДокументаПоДоговору(Форма, РеквизитыДоговора) Экспорт
	...
	Если РеквизитыДоговора = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(Форма.УчетнаяИнформацияДоговора) Тогда
			...
			// Если Статья движения денежных средств в договоре не заполнена, пустое значение не переносим в платежку.
			Если РеквизитыДоговора.Свойство("СтатьяДвиженияДенежныхСредств")
				И Не ЗначениеЗаполнено(РеквизитыДоговора.СтатьяДвиженияДенежныхСредств) Тогда
				РеквизитыДоговора.Удалить("СтатьяДвиженияДенежныхСредств");
			КонецЕсли;
...
// Создает список форм оплаты заявок по значению.
//
// Параметры:
//  ФормыОплаты  - Массив из ПеречислениеСсылка.ФормыОплаты, ПеречислениеСсылка.ФормыОплаты, Неопределено - формы оплаты.
//
// Возвращаемое значение:
//   Массив из ПеречислениеСсылка.ФормыОплаты
//
Функция ЗначениеПараметраФормыОплатыЗаявки(Знач ФормыОплаты) Экспорт

	Если ТипЗнч(ФормыОплаты) <> Тип("Массив") Тогда
		ФормыОплаты = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ФормыОплаты);
	КонецЕсли;
	
	Если ФормыОплаты.Найти(Перечисления.ФормыОплаты.ПустаяСсылка()) = Неопределено Тогда
		ФормыОплаты.Добавить(Перечисления.ФормыОплаты.ПустаяСсылка());
	КонецЕсли;
...
Процедура РассчитатьКурсЧислительВзаиморасчетовВСтрокеРасшифровки(СтрокаТЧ, ВалютаДокумента, ВалютаРеглУчета) Экспорт
	...
	ИначеЕсли ВалютаДокумента = ВалютаРеглУчета Тогда
		...
		Если СтрокаТЧ.СуммаВзаиморасчетов <> 0 Тогда
			СтрокаТЧ.КурсЧислительВзаиморасчетов = Окр(СтрокаТЧ.Сумма / СтрокаТЧ.СуммаВзаиморасчетов * СтрокаТЧ.КурсЗнаменательВзаиморасчетов, БазоваяТочностьКурса);
		Иначе
			СтрокаТЧ.КурсЧислительВзаиморасчетов = 1;
		КонецЕсли;
	Иначе
		ТочностьКурса = ?(СтрокаТЧ.ВалютаВзаиморасчетов = ВалютаРеглУчета, БазоваяТочностьКурса, 8);
		Если СтрокаТЧ.Сумма <> 0 Тогда
			СтрокаТЧ.КурсЧислительВзаиморасчетов = Окр(СтрокаТЧ.СуммаВзаиморасчетов / СтрокаТЧ.Сумма * СтрокаТЧ.КурсЗнаменательВзаиморасчетов, ТочностьКурса);
		Иначе
...
Процедура СформироватьКурсЗнаменательТекстом(СтрокаТЧ, ВалютаДокумента, ВалютаРеглУчета) Экспорт
	...
	Если СтрокаТЧ.КурсЧислительВзаиморасчетов <> 0 Тогда
		Если ВалютаДокумента = ВалютаРеглУчета Тогда
			...
			ФорматнаяСтрока = СтрШаблон("ЧДЦ=%1", БазоваяТочностьКурса);
			СтрокаТЧ.КурсЧислительВзаиморасчетовТекст = СтрШаблон(ШаблонКурса,
				...
				Строка(Формат(СтрокаТЧ.КурсЧислительВзаиморасчетов, ФорматнаяСтрока)),
				ВалютаДокумента);
		Иначе
			Если СтрокаТЧ.ВалютаВзаиморасчетов = ВалютаРеглУчета Тогда
				ФорматнаяСтрока = СтрШаблон("ЧДЦ=%1", БазоваяТочностьКурса);
			Иначе
				ФорматнаяСтрока = "ЧДЦ=8";
			КонецЕсли;
...
// Формирует наименование плательщика или получателя платежа в платежном поручении.
//
// Параметры:
//   Наименование - Строка - наименование плательщика или получателя.
//   ПрямыеРасчеты - Булево - признак расчетов без банка-корреспондента.
//   НомерСчета - Строка - номер счета плательщика или получателя
//   Банк - Строка - наименование банка-корреспондента счета плательщика или получателя.
//   Город - Строка - город банка-корреспондента.
//
// Возвращаемое значение:
//   Строка - сформированное наименование плательщика или получателя платежа
//
Функция НаименованиеПлательщикаПолучателяПлатежа(Наименование, ПрямыеРасчеты, НомерСчета, Банк, Город) Экспорт
	...
	Иначе
		...
		Если ПустаяСтрока(НомерСчета) Тогда
			Результат = Наименование;
		ИначеЕсли ПустаяСтрока(Банк) Тогда
			Результат = СтрШаблон(
				НСтр("ru = '%1 р/с %2'"), Наименование, СокрЛП(НомерСчета));
		ИначеЕсли ПустаяСтрока(Город) Тогда
			Результат = СтрШаблон(
				НСтр("ru = '%1 р/с %2 в %3'"), Наименование, СокрЛП(НомерСчета), СокрЛП(Банк));
		Иначе
			Результат = СтрШаблон(
				НСтр("ru = '%1 р/с %2 в %3 %4'"), Наименование, СокрЛП(НомерСчета), СокрЛП(Банк), СокрЛП(Город));
		КонецЕсли;
...
Функция ОпределитьТипСоздаваемогоДокумента(ДанныеОплаты)
	...
	Если ДанныеОплаты.ПоступлениеСписание = Перечисления.ТипыДвиженияДенежныхСредств.Списание Тогда
		Если ДанныеОплаты.ФормаОплаты = Перечисления.ФормыОплаты.Безналичная
			Или ДанныеОплаты.ФормаОплаты = Перечисления.ФормыОплаты.СистемаБыстрыхПлатежей Тогда
...
Процедура СформироватьНадписьГиперссылкиРеквизитыОбъектаРасчетов(Форма, Объект, ПроверятьРасхождения = Ложь) Экспорт
	...
	Если СуммаНеРаспределено = 0 Тогда
		...
		Если НЕ ЭтоОперацияВозвратаДС Тогда
			...
			Форма.ГиперссылкаРеквизитыОбъектаРасчетов = НСтр("ru = 'Объекты расчетов указаны для всей суммы платежа'");
			
		Иначе
			ЭлементФормы.Доступность = Истина;
			Форма.ГиперссылкаРеквизитыОбъектаРасчетов = НСтр("ru = 'Аналитика возврата'");
	...
	Иначе
		...
		ЭлементФормы.Доступность = Истина;
		Форма.ГиперссылкаРеквизитыОбъектаРасчетов =
			СтрШаблон(НСтр("ru = 'Без указания объекта расчетов: %1 %2'"), Формат(СуммаНеРаспределено, "ЧДЦ=2"), Объект.Валюта);
...
Функция ТекстыЗапросаПроверкаКурсовЗачетаВАвансовыхОтчетах()
	...
	|		Данные.СуммаРеглНачальныйОстаток
	|		* (1 - ВЫБОР
	...
	|				ТОГДА -Данные.СуммаКонечныйОстаток
	|			ИНАЧЕ 0
	|		КОНЕЦ * ВтКоэффициентыПересчетаВалют.КоэффициентПересчетаРегл
	|		//Оборот документа
	|		+ Данные.СуммаРеглОборот Между -0.9 и 0.9)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	Данные.СуммаНачальныйОстаток < 0
	|	И Данные.СуммаРеглНачальныйОстаток / Данные.СуммаНачальныйОстаток <> 1
	|	И Не ((Данные.СуммаНачальныйОстаток - Данные.СуммаКонечныйОстаток)
	|			* ВтКоэффициентыПересчетаВалют.КоэффициентПересчетаРегл
	|			+ Данные.СуммаРеглОборот Между -0.9 И 0.9)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	И Не (
	|		// С остатка списывается пропорционально (по курсу остатка)
	|		Данные.СуммаУпрНачальныйОстаток
	|		* (1 - ВЫБОР
	|			КОГДА Данные.СуммаКонечныйОстаток > 0
	|				ТОГДА Данные.СуммаКонечныйОстаток
	|			ИНАЧЕ 0
	...
	|				ТОГДА -Данные.СуммаКонечныйОстаток
	|			ИНАЧЕ 0
	|		КОНЕЦ * ВтКоэффициентыПересчетаВалют.КоэффициентПересчетаУпр
	|		//Оборот документа
	|		+ Данные.СуммаУпрОборот Между -0.9 и 0.9)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	И Данные.СуммаУпрНачальныйОстаток / Данные.СуммаНачальныйОстаток <> 1
	|	И Не ((Данные.СуммаНачальныйОстаток - Данные.СуммаКонечныйОстаток)
	|			* ВтКоэффициентыПересчетаВалют.КоэффициентПересчетаУпр
	|			+ Данные.СуммаУпрОборот Между -0.9 И 0.9)
	|";
...
Процедура ДобавитьВыборкуПоляВСхемуЗапроса(ЗапросВыбора, Поле, ВложенноеПоле, ПсевдонимПоля, ЗначениеПоля = "Неопределено")

	ОператорВыбора = ЗапросВыбора.Операторы[0];
	
	Если ДоступнаВыборкаПоля(Поле) Тогда
		
		НовоеПоле = Поле;
		
		Если ВложенноеПоле <> "" Тогда
			
			ДополнительноеПоле = Новый Массив;
			ДополнительноеПоле.Добавить(Поле);
			ДополнительноеПоле.Добавить(ВложенноеПоле);
			НовоеПоле = СтрСоединить(ДополнительноеПоле, ".");
			
		КонецЕсли;
		
		ОператорВыбора.ВыбираемыеПоля.Добавить(НовоеПоле);
		
	Иначе
		ОператорВыбора.ВыбираемыеПоля.Добавить(ЗначениеПоля);
	КонецЕсли;
	
	Колонка = ЗапросВыбора.Колонки.Получить(ЗапросВыбора.Колонки.Количество() - 1);
	Колонка.Псевдоним = ПсевдонимПоля;

КонецПроцедуры

Процедура ДополнитьОписаниеВыборкиПоДокументу(ОператорВыбора, Документ)

	ПолноеИмя = Документ.Метаданные().ПолноеИмя();
	
	ОператорВыбора.Источники.Добавить(ПолноеИмя, "Объект");
	ОператорВыбора.ВыбираемыеПоля.Добавить("Объект.Ссылка");
	ОператорВыбора.Отбор.Добавить("Объект.Ссылка = &Ссылка");

КонецПроцедуры

Функция ДоступнаВыборкаПоля(ПараметрДокумента)

	Если ПараметрДокумента <> "" Тогда
		
		СписокЧастей = СтрРазделить(ПараметрДокумента, ".");
		
		Если СписокЧастей[0] = "Объект" Тогда
			Возврат Истина;
		КонецЕсли;
...
Функция ТекстЗапросаСуммПолнойОплаты(Документ)

	Если ТипЗнч(Документ) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		ТекстЗапроса = 
			"ВЫБРАТЬ
			|	СУММА(ДанныеТабличнойЧасти.СуммаНДС) КАК СуммаНДС,
			|	СУММА(ДанныеТабличнойЧасти.СуммаСНДС) КАК СуммаСНДС,
			|	ДанныеТабличнойЧасти.СтавкаНДС КАК СтавкаНДС,
			|	ВЫБОР
			|		КОГДА ДанныеТабличнойЧасти.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
			|				И ДанныеТабличнойЧасти.Ссылка.ТребуетсяЗалогЗаТару
			|			ТОГДА ИСТИНА
			|		ИНАЧЕ ЛОЖЬ
			|	КОНЕЦ КАК ЭтоЗалогЗаТару
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	Документ.РеализацияТоваровУслуг.Товары КАК ДанныеТабличнойЧасти
			|ГДЕ
			|	(ДанныеТабличнойЧасти.Номенклатура.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
			|			ИЛИ ДанныеТабличнойЧасти.Ссылка.ТребуетсяЗалогЗаТару
			|			ИЛИ НЕ ДанныеТабличнойЧасти.Ссылка.ВернутьМногооборотнуюТару)
			|	И ДанныеТабличнойЧасти.Ссылка = &ДокументОперации
			|
			|СГРУППИРОВАТЬ ПО
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.АктВыполненныхРабот") Тогда
		ТекстЗапроса = 
			...
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	Документ.АктВыполненныхРабот.Услуги КАК ДанныеТабличнойЧасти
			|ГДЕ
			|	ДанныеТабличнойЧасти.Ссылка = &ДокументОперации
			|СГРУППИРОВАТЬ ПО
			|	ДанныеТабличнойЧасти.СтавкаНДС
			|";
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.РеализацияУслугПрочихАктивов") Тогда
		ТекстЗапроса = 
			...
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	Документ.РеализацияУслугПрочихАктивов.Доходы КАК ДанныеТабличнойЧасти
			|ГДЕ
			|	ДанныеТабличнойЧасти.Ссылка = &ДокументОперации
			|СГРУППИРОВАТЬ ПО
			|	ДанныеТабличнойЧасти.СтавкаНДС
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		ТекстЗапроса = 
			...
			|	ДанныеТабличнойЧасти.СтавкаНДС КАК СтавкаНДС,
			|	ВЫБОР
			|		КОГДА ДанныеТабличнойЧасти.Ссылка.ДокументОснование ССЫЛКА Документ.РеализацияТоваровУслуг
			|				И ДанныеТабличнойЧасти.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
			|				И ДанныеТабличнойЧасти.Ссылка.ДокументОснование.ТребуетсяЗалогЗаТару
			|			ТОГДА ИСТИНА
			|		ИНАЧЕ ЛОЖЬ
			|	КОНЕЦ КАК ЭтоЗалогЗаТару
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	Документ.КорректировкаРеализации.Расхождения КАК ДанныеТабличнойЧасти
			|ГДЕ
			|	ДанныеТабличнойЧасти.Ссылка = &ДокументОперации
			|	И НЕ ДанныеТабличнойЧасти.Ссылка.ВидКорректировки В (
			|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаРеализацииУвеличениеЗадолженностиСводно), 
			|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаРеализацииУменьшениеЗадолженностиСводно))
			|
			|СГРУППИРОВАТЬ ПО
			|	ДанныеТабличнойЧасти.СтавкаНДС,
			|	ВЫБОР
			...
			|
			|ИМЕЮЩИЕ
			|	СУММА(ДанныеТабличнойЧасти.СуммаСНДС) > 0
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			|	СУММА(КорректировкаЗадолженности.СуммаНДС),
			|	СУММА(КорректировкаЗадолженности.СуммаСНДС),
			...
			|ГДЕ
			|	КорректировкаЗадолженности.Ссылка = &ДокументОперации
			|	И КорректировкаЗадолженности.Ссылка.ВидКорректировки В (
			|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаРеализацииУвеличениеЗадолженностиСводно), 
			|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаРеализацииУменьшениеЗадолженностиСводно))
			|
			|СГРУППИРОВАТЬ ПО
			|	КорректировкаЗадолженности.СтавкаНДС
			|
			|ИМЕЮЩИЕ
			|	СУММА(КорректировкаЗадолженности.СуммаСНДС) <> 0
			|";
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
		ТекстЗапроса =
			...
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	Документ.ЗаявкаНаВозвратТоваровОтКлиента.ЗаменяющиеТовары КАК ДанныеТабличнойЧасти
			|ГДЕ
			|	ДанныеТабличнойЧасти.Ссылка = &ДокументОперации
			|	И НЕ ДанныеТабличнойЧасти.Отменено
			|	И (ДанныеТабличнойЧасти.Номенклатура.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
			|			ИЛИ ДанныеТабличнойЧасти.Ссылка.ТребуетсяЗалогЗаТару
			|			ИЛИ НЕ ДанныеТабличнойЧасти.Ссылка.ВернутьМногооборотнуюТару)
			|
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ВыкупВозвратнойТарыКлиентом") Тогда
		ТекстЗапроса = 
			"ВЫБРАТЬ
			|	ЛОЖЬ КАК ЭтоЗалогЗаТару,
			|	ДанныеТабличнойЧасти.СтавкаНДС КАК СтавкаНДС,
			...
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	Документ.ВыкупВозвратнойТарыКлиентом.Товары КАК ДанныеТабличнойЧасти
			|ГДЕ
			|	ДанныеТабличнойЧасти.Ссылка = &ДокументОперации
			|СГРУППИРОВАТЬ ПО
			|	ДанныеТабличнойЧасти.СтавкаНДС
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ВыкупТоваровХранителем") Тогда
		ТекстЗапроса = 
			"ВЫБРАТЬ
			|	ЛОЖЬ КАК ЭтоЗалогЗаТару,
			|	ДанныеТабличнойЧасти.СтавкаНДС КАК СтавкаНДС,
			...
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	Документ.ВыкупТоваровХранителем.Товары КАК ДанныеТабличнойЧасти
			|ГДЕ
			|	ДанныеТабличнойЧасти.Ссылка = &ДокументОперации
			|СГРУППИРОВАТЬ ПО
			|	ДанныеТабличнойЧасти.СтавкаНДС
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ОтчетКомиссионераОСписании") Тогда
		ТекстЗапроса = 
			"ВЫБРАТЬ
			|	ЛОЖЬ КАК ЭтоЗалогЗаТару,
			|	ДанныеТабличнойЧасти.СтавкаНДС КАК СтавкаНДС,
			...
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	Документ.ОтчетКомиссионераОСписании.Товары КАК ДанныеТабличнойЧасти
			|ГДЕ
			|	ДанныеТабличнойЧасти.Ссылка = &ДокументОперации
			|СГРУППИРОВАТЬ ПО
			|	ДанныеТабличнойЧасти.СтавкаНДС
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ОтчетКомитентуОЗакупках") Тогда
		ТекстЗапроса = 
			"ВЫБРАТЬ
			|	ЛОЖЬ КАК ЭтоЗалогЗаТару,
			|	ДанныеТабличнойЧасти.СтавкаНДС КАК СтавкаНДС,
			...
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	Документ.ОтчетКомитентуОЗакупках.Товары КАК ДанныеТабличнойЧасти
			|ГДЕ
			|	ДанныеТабличнойЧасти.Ссылка = &ДокументОперации
			|СГРУППИРОВАТЬ ПО
			|	ДанныеТабличнойЧасти.СтавкаНДС
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ОтчетКомитенту") Тогда
		ТекстЗапроса =  
			"ВЫБРАТЬ
			|	ДанныеТабличнойЧасти.СуммаНДСВознаграждения КАК СуммаНДС,
			|	ДанныеТабличнойЧасти.СтавкаНДСВознаграждения КАК СтавкаНДС,
			|	ДанныеТабличнойЧасти.СуммаВознаграждения КАК СуммаСНДС,
			|	ЛОЖЬ КАК ЭтоЗалогЗаТару
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	Документ.ОтчетКомитенту КАК ДанныеТабличнойЧасти
			|ГДЕ
			|	ДанныеТабличнойЧасти.Ссылка = &ДокументОперации";
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ОтчетКомиссионера") Тогда
		ТекстЗапроса =  
			"ВЫБРАТЬ
			|	ЛОЖЬ КАК ЭтоЗалогЗаТару,
			|	СУММА(ДанныеТабличнойЧасти.СуммаПродажиНДС) КАК СуммаНДС,
			|	СУММА(ДанныеТабличнойЧасти.СуммаПродажи) КАК СуммаСНДС,
			|	ДанныеТабличнойЧасти.СтавкаНДС КАК СтавкаНДС
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	Документ.ОтчетКомиссионера.Товары КАК ДанныеТабличнойЧасти
			|ГДЕ
			|	ДанныеТабличнойЧасти.Ссылка = &ДокументОперации
			|
			|СГРУППИРОВАТЬ ПО
	...
	Иначе
		ТекстЗапроса =  
			...
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	Документ.ЗаказКлиента.Товары КАК ДанныеТабличнойЧасти
			|ГДЕ
			|	НЕ ДанныеТабличнойЧасти.Отменено
			|	И (ДанныеТабличнойЧасти.Номенклатура.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
			|			ИЛИ ДанныеТабличнойЧасти.Ссылка.ТребуетсяЗалогЗаТару
			|			ИЛИ НЕ ДанныеТабличнойЧасти.Ссылка.ВернутьМногооборотнуюТару)
			|	И ДанныеТабличнойЧасти.Ссылка = &ДокументОперации
...
Функция ТекстЗапросаСуммЧастичнойОплаты()
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ЭтапыГрафикаОплаты.ЭтоЗалогЗаТару
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(СуммыПолнойОплаты.СуммаСНДС) КАК СуммаСНДС,
	|	СуммыПолнойОплаты.ЭтоЗалогЗаТару КАК ЭтоЗалогЗаТару
	|ПОМЕСТИТЬ ОбщаяСумма
	|ИЗ
	|	СуммыПолнойОплаты КАК СуммыПолнойОплаты
	|
	|СГРУППИРОВАТЬ ПО
	|	СуммыПолнойОплаты.ЭтоЗалогЗаТару
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СуммыДокумента.СтавкаНДС КАК СтавкаНДС,
	|	ВЫБОР
	|		КОГДА ОбщаяСумма.СуммаСНДС = 0
	|			ТОГДА 0
	|		ИНАЧЕ ЭтапыОплаты.СуммаПлатежа / ОбщаяСумма.СуммаСНДС * СуммыДокумента.СуммаСНДС
	|	КОНЕЦ КАК СуммаСНДС,
	|	ВЫБОР
	|		КОГДА ОбщаяСумма.СуммаСНДС = 0
	|			ТОГДА 0
	|		ИНАЧЕ ЭтапыОплаты.СуммаПлатежа / ОбщаяСумма.СуммаСНДС * СуммыДокумента.СуммаНДС
	|	КОНЕЦ КАК СуммаНДС
	|ПОМЕСТИТЬ СуммыЧастичнойОплаты
	|ИЗ
	|	СуммыПолнойОплаты КАК СуммыДокумента
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ЭтапыОплаты КАК ЭтапыОплаты
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОбщаяСумма КАК ОбщаяСумма
	|			ПО ЭтапыОплаты.ЭтоЗалогЗаТару = ОбщаяСумма.ЭтоЗалогЗаТару
	|		ПО СуммыДокумента.ЭтоЗалогЗаТару = ЭтапыОплаты.ЭтоЗалогЗаТару
	|";

КонецФункции

// Вычисляет суммы к оплате с учетом НДС по документу-основанию и этапам оплаты документа "Счет на оплату клиенту"
//
// Параметры:
//   ДокументОснование - СправочникСсылка.ДоговорыКонтрагентов, ДокументСсылка - документ, определяющий ставку НДС
//   ЭтапыОплаты - ТаблицаЗначений - таблица с этапами оплаты (суммы, даты, проценты):
//   * СуммаПлатежа - Число - сумма платежа по этапу
//   * ЭтоЗалогЗаТару - Булево - признак суммы залога за тару
//   * ДатаПлатежа - Дата - дата платежа по этапу
//   * ПроцентПлатежа - Число - процент платежа по этапу
//
// Возвращаемое значение:
//   ТаблицаЗначений - таблица с итоговыми суммами:
//     * СтавкаНДС - Число - ставка НДС
//     * СуммаНДС - Число - сумма НДС
//     * СуммаСНДС - Число - сумма с учетом НДС
//
Функция СуммыСчетаНаОплату(ДокументОснование, ЭтапыОплаты)
	
	ЗапросЭтаповОплаты = Новый Запрос;
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	ЗапросЭтаповОплаты.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	ЗапросЭтаповОплаты.Текст = 
		"ВЫБРАТЬ
		...
		|ПОМЕСТИТЬ ЭтапыГрафикаОплаты
		|ИЗ
		|	&ЭтапыОплаты КАК ЭтапыОплаты";
	ЗапросЭтаповОплаты.УстановитьПараметр("ЭтапыОплаты", ЭтапыОплаты);
	ЗапросЭтаповОплаты.Выполнить();
	...
	Если ТипЗнч(ДокументОснование) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		...
		ТекстЗапроса = 
			...
			|	СУММА(ЭтапыОплаты.СуммаПлатежа) КАК СуммаСНДС,
			|	СУММА(ЭтапыОплаты.СуммаПлатежа) * ДоговорыКонтрагентов.СтавкаНДС.Ставка / (100 + ДоговорыКонтрагентов.СтавкаНДС.Ставка) КАК СуммаНДС,
			|	ЭтапыОплаты.ЭтоЗалогЗаТару КАК ЭтоЗалогЗаТару
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	ЭтапыГрафикаОплаты КАК ЭтапыОплаты
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
			|		ПО ДоговорыКонтрагентов.Ссылка = &ДокументОперации
			|
			|СГРУППИРОВАТЬ ПО
			|	ДоговорыКонтрагентов.СтавкаНДС,
			|	ЭтапыОплаты.ЭтоЗалогЗаТару,
			|	ДоговорыКонтрагентов.СтавкаНДС.Ставка";
	...
	ТекстыЗапросов.Добавить(ТекстЗапросаСуммЧастичнойОплаты());
	...
	ИтоговаяВыборка =
		...
		|	ВЫРАЗИТЬ(ТаблицаСумм.СуммаСНДС КАК ЧИСЛО(31,2)) КАК СуммаСНДС
		|ИЗ
		|	СуммыЧастичнойОплаты КАК ТаблицаСумм";
...
// Процедура заполняет реквизит СуммаНДС табличной части РасшифровкаПлатежа.
//
// Параметры:
//	Объект - ДанныеФормыСтруктура - Текущий объект.
//	ИдСтроки - Число - Идентификатор строки для обновления ставки.
//
Процедура ПересчитатьСуммуНДС(Объект, ИдСтроки) Экспорт
