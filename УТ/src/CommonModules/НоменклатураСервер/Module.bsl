...
Функция ДопустимыеТипыНоменклатурыДляВыбора(ХозяйственнаяОперация, ИспользоватьНаборы = Ложь) Экспорт
	...
	Если Не ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоставкаПодПринципала 
		И Не ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтгрузкаТоваровКлиенту
		И Не ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности
		И Не ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеТоваровОтКлиента Тогда
...
Функция СписокХозяйственныхОперацийИсключающихУслугиИРаботы() Экспорт
	...
	СписокОпераций.Добавить(Перечисления.ХозяйственныеОперации.ПереходПраваСобственности);
	СписокОпераций.Добавить(Перечисления.ХозяйственныеОперации.ПоступлениеТоваровОтКлиента);
	СписокОпераций.Добавить(Перечисления.ХозяйственныеОперации.КонтрольКачестваТоваров);
...
//			* ЭтоПроверкаТекущейСтроки - Булево - Включает режим проверки строки, выведенной из таблицы в отдельные поля ввода
...
Функция ПараметрыПроверкиЗаполненияХарактеристик() Экспорт
	...
	ПараметрыПроверки.Вставить("ЭтоПроверкаТекущейСтроки",	Ложь);
...
Процедура ПроверитьЗаполнениеХарактеристик(Объект,МассивНепроверяемыхРеквизитов,Отказ,ПараметрыПроверки = Неопределено) Экспорт
	...
	Если ПараметрыПроверки.ЭтоПроверкаТекущейСтроки Тогда
	...
	Иначе
		...
		ПутьКДанным = ПараметрыПроверки.ПутьКДаннымТаблицыФормы;
	КонецЕсли;
	...
	ИначеЕсли ВыводитьНомераСтрок Тогда
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.НеЗаполненаХарактеристика Тогда
			...
			Поле = ПутьКПолюТаблицы(ПараметрыПроверки,
									Выборка.НомерСтроки,
...
//			* ИноеПредставлениеУпаковка - Строка - значение иного представления упаковки, отличного от синонима реквизита ТЧ.
//			* ВыводитьНомераСтрок - Булево - значение по умолчанию Истина.
...
Функция ПараметрыПроверкиЗаполненияУпаковок() Экспорт
	...
	ПараметрыПроверки.Вставить("ИноеПредставлениеУпаковка",	"");
	ПараметрыПроверки.Вставить("ВыводитьНомераСтрок",		Истина);
	ПараметрыПроверки.Вставить("ОтборПроверяемыхСтрок",		Новый Структура);
	ПараметрыПроверки.Вставить("ИменаПолейССуффиксом",		Новый Структура);
...
Процедура ПроверитьЗаполнениеУпаковок(Объект,МассивНепроверяемыхРеквизитов,Отказ,ПараметрыПроверки = Неопределено) Экспорт
	...
	ПредставлениеТЧ					= МетаданныеОбъекта.ТабличныеЧасти[ИмяТЧ].Синоним;
	ПредставлениеРеквизитаУпаковка	= ?(ПустаяСтрока(ПараметрыПроверки.ИноеПредставлениеУпаковка),
		МетаданныеОбъекта.ТабличныеЧасти[ИмяТЧ].Реквизиты[ИмяПоляУпаковка].Синоним,
		ПараметрыПроверки.ИноеПредставлениеУпаковка);
...
// Определяет поддерживаемые типы номенклатуры по параметрам выбора документа
// 
// Параметры:
//  Ссылка - ДокументСсылка - Определяет тип документа для которого необходимо получить подходящие типы номенклатуры
//  ИмяТабличнойЧасти - Строка - Имя табличной части, в которой хранится номенклатура
//  ИмяРеквизитаНоменклатуры - Строка - Имя реквизита номенклатуры
// 
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.ТипыНоменклатуры - Поддерживаемые типы номенклатуры документа
Функция ПоддерживаемыеТипыНоменклатурыДокумента(Ссылка, ИмяТабличнойЧасти = "Товары", ИмяРеквизитаНоменклатуры = "Номенклатура") Экспорт

	ПараметрыВыбора = Ссылка.Метаданные().ТабличныеЧасти[ИмяТабличнойЧасти].Реквизиты[ИмяРеквизитаНоменклатуры].ПараметрыВыбора;
	...
	Для Каждого ПараметрВыбора Из ПараметрыВыбора Цикл
		Если ПараметрВыбора.Имя = "Отбор.ТипНоменклатуры" Тогда
			Если ТипЗнч(ПараметрВыбора.Значение) = Тип("ФиксированныйМассив") Тогда
				ВозвращаемоеЗначение = ПараметрВыбора.Значение;
				Прервать;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Если ВозвращаемоеЗначение = Неопределено Тогда
		// Все значения:
		ВозвращаемоеЗначение = НоменклатураКлиентСервер.ОтборПоТоваруМногооборотнойТареУслугеРаботе(Истина);
	КонецЕсли;
...
Процедура ЗаполнитьСтатусыУказанияСерий(Объект,
	...
	Если ПараметрыУказанияСерий.ТоварВШапке Тогда
		...
		ТаблицаТоваров.Колонки.Добавить("Количество",ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля(ДопустимыйЗнак.Неотрицательный));
		...
		Иначе
			...
			ТаблицаСерий.Колонки.Добавить("Количество",ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля(ДопустимыйЗнак.Неотрицательный));
...
Функция ПроверитьСериюРассчитатьСтатусПриИзмененииРеквизитаВТЧ(ТекущаяСтрока, Склад, ПараметрыУказанияСерий) Экспорт
	...
	ТаблицаТоваров.Колонки.Добавить(ПараметрыУказанияСерий.ИмяПоляКоличество,ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля(ДопустимыйЗнак.Неотрицательный));
	ТаблицаТоваров.Колонки.Добавить("НомерСтроки",Новый ОписаниеТипов("Число",Новый КвалификаторыЧисла(5,0,ДопустимыйЗнак.Неотрицательный)));
	...
	ТаблицаСерий.Колонки.Добавить("Номенклатура",Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТаблицаСерий.Колонки.Добавить("Характеристика",Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
	ТаблицаСерий.Колонки.Добавить(ПараметрыУказанияСерий.ИмяПоляКоличество,ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля(ДопустимыйЗнак.Неотрицательный));
...
//  ПараметрыУказанияСерий - Структура, Массив Из Структура параметров указания серий, возвращаемая соответствующей процедурой модуля менеджера документа.
		...
		Для Каждого Параметры Из ПараметрыУказанияСерий Цикл
			...
			Если Параметры.ИменаПолейССуффиксом.Количество() Тогда
				...
				Если ИменаПолейССуффиксом.Свойство("Серия") Тогда
					...
					Иначе
						Если Параметры.ТоварВШапке Тогда
							МассивНепроверяемыхРеквизитов.Добавить(СтрШаблон("%1", ИмяПоляСерия));
						Иначе
	...
	Для Каждого Параметры Из ПараметрыУказанияСерий Цикл
		...
		Если Параметры.ТоварВШапке Тогда
			...
			Если НоменклатураКлиентСервер.СтатусыСерийСерияНеУказана().Найти(ДокументОбъект.СтатусУказанияСерий) <> Неопределено
				И ДокументОбъект.СтатусУказанияСерий <> 13
				И Параметры.ИмяТЧТовары <> Параметры.ИмяТЧСерии Тогда
			...
			ИначеЕсли ДокументОбъект.СтатусУказанияСерий = 1
				Или ДокументОбъект.СтатусУказанияСерий = 13 Тогда
				
				ИмяПоляСерия = ?(Параметры.ИменаПолейССуффиксом.Количество() > 0
									И ИменаПолейССуффиксом.Свойство("Серия"),
								ИменаПолейССуффиксом.Серия,
								"Серия");
				ИмяРеквизитаСерия = ?(ПустаяСтрока(ПараметрыПроверки.ИмяРеквизитаСерия),
										МетаданныеДокумента.Реквизиты[ИмяПоляСерия].Синоним,
										ПараметрыПроверки.ИмяРеквизитаСерия);
				
				ТекстСообщения = НСтр("ru = 'Поле ""%1"" не заполнено'");
				ТекстСообщения = СтрШаблон(ТекстСообщения, ИмяРеквизитаСерия);
		...
		ИначеЕсли Параметры.ИмяТЧТовары = Параметры.ИмяТЧСерии Тогда
			...
			Иначе
				...
				ИспользоватьУпаковки = Параметры.ИспользуютсяТоварныеМеста
										Или Параметры.ПоляСвязи.Найти("Упаковка") <> Неопределено;
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
											"&ИмяПоляУпаковка",
				...
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
											"&ИмяПоляКоличество",
											"ТаблицаСерий." + Параметры.ИмяПоляКоличество);
			...
			Запрос.УстановитьПараметр("СтатусыСерийСериюМожноУказать",
										НоменклатураКлиентСервер.СтатусыСерийСериюМожноУказать());
...
Функция ПараметрыФормыУказанияСерий(Объект,ПараметрыУказанияСерий,ТекущиеДанныеИдентификатор,Форма, Метаданные = Неопределено, ЗаголовокКолонкиКоличество = "") Экспорт
	...
	Если РегистрироватьСерии Тогда
		...
		ТаблицаСерий.Колонки.Добавить("Серия", Новый ОписаниеТипов("СправочникСсылка.СерииНоменклатуры"));
		ТаблицаСерий.Колонки.Добавить("Количество", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля(ДопустимыйЗнак.Неотрицательный));
		ТаблицаСерий.Колонки.Добавить("КоличествоУпаковок", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля(ДопустимыйЗнак.Неотрицательный));
	...
	Если ПараметрыУказанияСерий.ИмяПоляКонтролируемаяПартия = Неопределено Тогда
		КонтролируемаяПартия = Справочники.КонтролируемыеПартии.ПустаяСсылка();
	ИначеЕсли СтрНайти(ПараметрыУказанияСерий.ИмяПоляКонтролируемаяПартия, ".") > 0 Тогда
		КонтролируемаяПартия = ТекущиеДанные[СтрЗаменить(ПараметрыУказанияСерий.ИмяПоляКонтролируемаяПартия, ".", "")];
	Иначе
		КонтролируемаяПартия = Объект[ПараметрыУказанияСерий.ИмяПоляКонтролируемаяПартия];
	КонецЕсли;
	ПараметрыФормыУказанияСерий.Вставить("КонтролируемаяПартия", КонтролируемаяПартия);
...
//  Действия					 - Структура - описание действий, которые необходимо выполнить со строками после указания серий, см. ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ
...
// Удалим прежние строки серий
	...
	Если ЕстьУпаковкиВТЧТовары
		...
		Если ПараметрыУказанияСерий.ИмяТЧСерии = ПараметрыУказанияСерий.ИмяТЧТовары Тогда
			СтруктураДействий = ОбщегоНазначения.СкопироватьРекурсивно(Действия); // Структура
			СтруктураДействий.Вставить("ПересчитатьКоличествоУпаковок");
			СтруктураДействий.Удалить("ПересчитатьКоличествоЕдиниц");
		КонецЕсли;
		
		ОбрабатываемыеСтроки = Новый Массив();
		СерияВТЧТовары = ПараметрыУказанияСерий.ИмяТЧСерии = ПараметрыУказанияСерий.ИмяТЧТовары;
		...
		Если СерияВТЧТовары Тогда
			...
			Для каждого ОбрабатываемаяСтрока Из ОбрабатываемыеСтроки Цикл
				КоличествоУпаковокПослеУдаленияСерий = КоличествоУпаковокПослеУдаленияСерий 
					+ ОбрабатываемаяСтрока.КоличествоУпаковок; 
			КонецЦикла;
		КонецЕсли;
	...
	Иначе
		...
		Если Действия <> Неопределено Тогда
			...
			Для Каждого ОбрабатываемаяСтрока Из ОбрабатываемыеСтроки Цикл
				Если ЕстьУпаковкиВТЧТовары Тогда
					КоличествоУпаковокПослеУдаленияСерий = КоличествоУпаковокПослеУдаленияСерий + ОбрабатываемаяСтрока.КоличествоУпаковок; 
				Иначе
					КоличествоУпаковокДоУдаленияСерий = КоличествоУпаковокДоУдаленияСерий + СтрМас[ПараметрыУказанияСерий.ИмяПоляКоличество];
				КонецЕсли;
			КонецЦикла;
	...
	Если ПараметрыУказанияСерий.ТоварВШапке Тогда
		...
		Если КоличествоСерий <> КоличествоТоваров 
			...
			ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(Объект, Действия, КешированныеЗначения);
			...
			Для Каждого СтрокаТоваров Из НайденныеСтрокиТоваров Цикл
				...
				Если ЕстьУпаковки
					...
					Для Каждого СтрТабл Из ТаблицаТары Цикл
						...
						ОбрабатываемыеСтроки.Добавить(СтрокаТары);
...
// Заполняет информацию о складе.
//
// Параметры:
//	Объект - ДанныеФормыКоллекция, ДокументОбъект - документ, для которого нужно получить информацию о складе.
//	ПараметрыСклада - см. ШтрихкодированиеНоменклатурыКлиентСервер.ПараметрыОбработкиШтрихкодов.
//	ПараметрыУказанияСерий - см. НоменклатураКлиентСервер.ПараметрыУказанияСерий.
//
Процедура ЗаполнитьСкладПоПараметрамУказанияСерий(Объект, ПараметрыСклада, ПараметрыУказанияСерий) Экспорт
	...
	СкладыВТЧ		= СкладыУказываютсяВТабличнойЧасти(ПараметрыУказанияСерий);
	
	Если ИмяПоляСклад <> Неопределено Тогда
		Если Не СкладыВТЧ
			И ПараметрыУказанияСерий.ИмяПоляСкладПолучатель <> Неопределено
			И ПараметрыУказанияСерий.ИмяПоляСкладОтправитель <> Неопределено Тогда
			
			Склад = Новый Структура;
			Склад.Вставить("Отправитель", Объект[ПараметрыУказанияСерий.ИмяПоляСкладОтправитель]);
			Склад.Вставить("Получатель", Объект[ПараметрыУказанияСерий.ИмяПоляСкладПолучатель]);
			
		ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, ИмяПоляСклад) Тогда
			Склад = Объект[ИмяПоляСклад];
		КонецЕсли;
	КонецЕсли;
	
	ПараметрыСклада.Склад = Склад
	
КонецПроцедуры

// Возвращает признак учета серий по складам табличной части товаров.
//
// Параметры:
//	ПараметрыУказанияСерий - см. НоменклатураКлиентСервер.ПараметрыУказанияСерий.
//
// Возвращаемое значение:
//	Булево - признак учета серий по складам табличной части товаров.
//
Функция СкладыУказываютсяВТабличнойЧасти(ПараметрыУказанияСерий) Экспорт
	...
	Возврат ПараметрыУказанияСерий.ПоляСвязи.Найти(ПараметрыУказанияСерий.ИмяПоляСклад) <> Неопределено
		Или ПараметрыУказанияСерий.ИменаПолейДополнительные.Найти(ПараметрыУказанияСерий.ИмяПоляСклад) <> Неопределено;
	
КонецФункции

// Возвращает статус указания серий по умолчанию для переданных параметров "Номенклатура" и "Склад".
//
// Параметры:
//	Номенклатура			- СправочникСсылка.Номенклатура - номенклатура для которой рассчитывается статус указания
//																серий.
//	Склад					- СправочникСсылка.Склады, Структура, Неопределено - информация о складе документа.
//	ПараметрыУказанияСерий	- см. НоменклатураКлиентСервер.ПараметрыУказанияСерий.
//
// Возвращаемое значение:
//	Число - статуса указания серий по умолчанию.
//
Функция СтатусУказанияСерийПоУмолчанию(Номенклатура, Склад, ПараметрыУказанияСерий) Экспорт
	
	ТаблицаТоваров = ТаблицаТоваровДляРасчетаСтатусовУказанияСерий(ПараметрыУказанияСерий);
	...
	Если ПараметрыУказанияСерий.ЭтоЗаказ Тогда
		Если ТаблицаТоваров.Колонки.Найти("ВариантОбеспечения") <> Неопределено Тогда
			СтрокаТовара.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить;
		КонецЕсли;
	КонецЕсли;
	
	Если ПараметрыУказанияСерий.ЭтоОрдер
		И ПараметрыУказанияСерий.ПоляСвязи.Найти("Действие") <> Неопределено Тогда
	...
	Если ПараметрыУказанияСерий.ИмяПоляСклад <> Неопределено
		И СкладыУказываютсяВТабличнойЧасти(ПараметрыУказанияСерий) Тогда
		
		СтрокаТовара[ПараметрыУказанияСерий.ИмяПоляСклад] = Склад;
		
	КонецЕсли;
	
	ТаблицаСерий = ТаблицаСерийДляРасчетаСтатусовУказанияСерий(ПараметрыУказанияСерий);
	...
	Если Выборка.Следующий() Тогда
		СтатусУказанияСерий = Выборка.СтатусУказанияСерий;
	КонецЕсли;
...
Процедура ЗаполнитьСлужебныеРеквизитыПоНоменклатуреВСтруктуре(СтруктураДанных, СтруктураДействий) Экспорт
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.НормализоватьДействия(СтруктураДействий);
	
	СтруктураДопДанных = ПакетнаяОбработкаТабличнойЧастиСервер.ОписаниеДополнительнойИнформации(СтруктураДействий);
	...
	Запрос = Новый Запрос(ПакетнаяОбработкаТабличнойЧастиСервер.ПолучитьТекстЗапросаПоСлужебнымРеквизитамТЧ(СтруктураДействий, СтруктураДопДанных));
...
Процедура ДополнитьТекстЗапросаРеквизитыПоНоменклатуре(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействийЗаполнения.Количество() <> 0 Тогда
		...
		КэшированныеЗначения.Вставить("РеквизитыЗаполненияНоменклатуры", СтруктураДопДанных.РеквизитыЗаполнения);
...
//
// Параметры:
//  СтруктураДанных - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ.ТекущаяСтрока
//  СтруктураДействий - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ.СтруктураДействий
//  КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения
//
Процедура ЗаполнитьСлужебныеРеквизитыПоНоменклатуреВСтруктуреПакетно(СтруктураДанных, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	ДанныеЗаполнения = ПакетнаяОбработкаТабличнойЧастиСервер.ДанныеДляОбработкиСтроки(
		"РеквизитыПоНоменклатуре", КэшированныеЗначения);
	
	Если ДанныеЗаполнения <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(СтруктураДанных, ДанныеЗаполнения[0],
			КэшированныеЗначения.РеквизитыЗаполненияНоменклатуры);
	КонецЕсли;
...
// 
// Возвращаемое значение:
//  Структура - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ.СтруктураДействий
//
...
// Возвращает параметры условного оформления для поля номенклатуры другого качества.
//
// Возвращаемое значение:
//	Структура - коллекция, содержащая следующие параметры:
//		* ИмяПоляНоменклатура - Строка - имя поля номенклатуры другого качества.
//		* ПутьКПолюНоменклатурыИсходногоКачества - Строка - путь к полю номенклатуры исходного качества.
//		* ИспользуетсяПризнакПорчи - Булево - определяет наличие признака, хранящего информацию об использовании
//												товара другого качества.
//		* ПутьКПолюПорча - Строка - путь к полю, хранящего информацию об использовании товара другого качества.
//
Функция ПараметрыУсловногоОформленияНоменклатурыДругогоКачества() Экспорт
	
	ПараметрыУсловногоОформления= Новый Структура;
	ПараметрыУсловногоОформления.Вставить("ИмяПоляНоменклатура", "ТоварыНоменклатураОприходование");
	ПараметрыУсловногоОформления.Вставить("ПутьКПолюНоменклатурыИсходногоКачества", "Объект.Товары.Номенклатура");
	ПараметрыУсловногоОформления.Вставить("ИспользуетсяПризнакПорчи", Ложь);
	ПараметрыУсловногоОформления.Вставить("ПутьКПолюПорча", "");

	Возврат ПараметрыУсловногоОформления;
	
КонецФункции

// Устанавливает условное оформление для поля номенклатуры другого качества.
//
// Параметры:
//	Форма - ФормаКлиентскогоПриложения - форма, в которой выполняется установка условного оформления.
//	ПараметрыУсловногоОформления - см. ПараметрыУсловногоОформленияНоменклатурыДругогоКачества.
//
Процедура УстановитьУсловноеОформлениеНоменклатурыДругогоКачества(Форма, ПараметрыУсловногоОформления = Неопределено) Экспорт
	...
	Если ПараметрыУсловногоОформления = Неопределено Тогда
		ПараметрыУсловногоОформления = ПараметрыУсловногоОформленияНоменклатурыДругогоКачества();
	КонецЕсли;
	
	ЭлементУсловногоОформления = Форма.УсловноеОформление.Элементы.Добавить();
	
	ПолеЭлемента = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ПараметрыУсловногоОформления.ИмяПоляНоменклатура);
	
	ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ПараметрыУсловногоОформления.ПутьКПолюНоменклатурыИсходногоКачества);
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	
	Если ПараметрыУсловногоОформления.ИспользуетсяПризнакПорчи Тогда
		ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ПараметрыУсловногоОформления.ПутьКПолюПорча);
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<укажите товар исходного качества>'"));
...
Процедура УстановитьУсловноеОформлениеЕдиницИзмерения(Форма, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ДополнительныеПараметры.ПутьКПолюИспользоватьАдресноеХранение) Тогда
		...
		Если ЗначениеЗаполнено(ДополнительныеПараметры.ПутьКПолюТипИзмеряемойВеличины) Тогда
			...
			ЭлементОтбора = Контейнер.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ДополнительныеПараметры.ПутьКПолюТипИзмеряемойВеличины);
...
// 
// Параметры:
//	ИспользуетсяАдресноеХранение - Булево - Признак использования адресного хранения.
//	ИспользуетсяТипИзмеряемойВеличины - Булево - Признак использования типа измеряемой величины.
// 
// Возвращаемое значение:
//   Структура - свойства:
...
Процедура ПроверитьЗаполнениеКоличества(Объект, ПроверяемыеРеквизиты, Отказ, ПараметрыПроверки = Неопределено) Экспорт
	...
	Иначе
		...
		Если ПараметрыПроверки.ЭтоПроверкаТекущейСтроки Тогда 
			...
			ШаблонОшибкаКоличества = НСтр("ru = 'Не заполнено поле ""%ПредставлениеКолонки%""'");
			ШаблонОшибкаПересчета = НСтр("ru = 'Обнаружено нулевое количество при пересчете в единицу хранения'");	
		...
		Для Каждого СтрокаТаб Из Объект[ИмяТЧ] Цикл
			...
			Если СтрокаТаб.Количество = 0
				...
				Поле = ПутьКПолюТаблицы(ПараметрыПроверки, СтрокаТаб.НомерСтроки, "КоличествоУпаковок");
			...
			Если СтрокаТаб[ИменаПолейССуффиксом.Количество] = 0
				...
				Поле = ПутьКПолюТаблицы(ПараметрыПроверки, СтрокаТаб.НомерСтроки, ИменаПолейССуффиксом.КоличествоУпаковок);
...
Процедура ПроверитьОкруглениеКоличества(Объект, Отказ, ПараметрыПроверки = Неопределено) Экспорт
	...
	Результат = ПроверитьОкруглениеКоличестваВнутренний(Объект, ПараметрыПроверки, ДопустимыйПроцентОтклонения); // РезультатЗапроса
	Если Результат.Пустой() Тогда
		Возврат;
	КонецЕсли;
	
	Если ИмяТЧ = "Объект"
		Или ПараметрыПроверки.ЭтоПроверкаТекущейСтроки Тогда
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.КоличествоОтклонение > Выборка.КоличествоМаксимальнаяПогрешность Тогда
			...
			Иначе
				...
				Поле = ПутьКПолюТаблицы(ПараметрыПроверки, Выборка.НомерСтроки, "КоличествоУпаковок");
		...
		Если Выборка.КоличествоСуффиксОтклонение > Выборка.КоличествоСуффиксМаксимальнаяПогрешность Тогда
			...
			Иначе
				...
				Поле = ПутьКПолюТаблицы(ПараметрыПроверки, Выборка.НомерСтроки, ИменаПолейССуффиксом.КоличествоУпаковок);
...
Процедура ОкруглитьКоличествоШтучныхТоваров(Объект, РежимЗаписи = Неопределено, ПараметрыОкругления = Неопределено) Экспорт
	...
	Если ИмяТЧ = "Объект" Тогда
		...
		Таблица.Колонки.Добавить("Количество", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля(ДопустимыйЗнак.Неотрицательный));
...
Процедура ДополнитьТекстЗапросаСодержаниеУслуги(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		...
		ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "СодержаниеУслуги");
...
Процедура ДополнитьТекстЗапросаКодТНВЭДПоНоменклатуре(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		...
		ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "КодТНВЭДПоНоменклатуре");
...
Процедура ДополнитьТекстЗапросаПроверитьКорректностьЗаполнитьХарактеристикиИУпаковки(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
			"ЗаполнитьХарактеристикуНекачественногоТовара",
			СтруктураДействий,
			КэшированныеЗначения,
			ПараметрыДействия) Тогда
		
		СписокЗапросов = ТекстыЗапросовЗаполнитьХарактеристикуНекачественногоТовара();
		Для Каждого ЭлементСписка Из СписокЗапросов Цикл
			ОписаниеЗапроса.ТекстыЗапросов.Добавить(ЭлементСписка.Значение, ЭлементСписка.Представление);
		КонецЦикла;
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
			"ЗаполнитьУпаковкуНекачественногоТовара",
			СтруктураДействий,
			КэшированныеЗначения,
			ПараметрыДействия) Тогда
		
		СписокЗапросов = ТекстыЗапросовЗаполнитьУпаковкуНекачественногоТовара();
		Для Каждого ЭлементСписка Из СписокЗапросов Цикл
			ОписаниеЗапроса.ТекстыЗапросов.Добавить(ЭлементСписка.Значение, ЭлементСписка.Представление);
		КонецЦикла;
...
Процедура ДополнитьТекстЗапросаПересчитатьКоличествоУпаковокСуффикс(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		...
		ПараметрыЗапроса.Вставить("ИмяПоляКоличество", КоличествоССуффиксом.Количество);
		ПараметрыЗапроса.Вставить("ИмяПоляКоличествоУпаковок", КоличествоССуффиксом.КоличествоУпаковок);
		ПараметрыЗапроса.Вставить("ИмяТаблицы", "ПересчитатьКоличествоУпаковокСуффикс");
		
		СписокЗапросов = ТекстыЗапросовПересчитатьКоличествоУпаковок(ПараметрыЗапроса);
...
// Добавляет запросы в пакет запросов для проверки и расчета статусов указания серий.
//
// Параметры:
//  СтруктураДействий - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения
//  ОписаниеЗапроса - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОписаниеЗапроса
//  КэшированныеЗначения - Структура
//
Процедура ДополнитьТекстЗапросаПроверитьСериюРассчитатьСтатус(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		...
		ТаблицаТоваровПредварительная = КэшированныеЗначения.ОписаниеЗапроса.ПараметрыЗапроса.ИсточникДанных;
		...
		ТипХарактеристикиНоменклатуры = Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры");
		ТипСерииНоменклатуры = Новый ОписаниеТипов("СправочникСсылка.СерииНоменклатуры");
		ТипКоличество = ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля(ДопустимыйЗнак.Неотрицательный);
		ТипНомерСтроки = Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(36));
		...
		Если ПараметрыУказанияСерий.ИменаПолейССуффиксом.Количество() Тогда
			...
			Если ИменаПолейССуффиксом.Свойство("Номенклатура") Тогда
				...
				ДобавитьКолонкуВТаблицуТоваров(
					ТаблицаТоваров,
					ИменаПолейССуффиксом.Номенклатура,
					ТипНоменклатура);
			...
			Если ИменаПолейССуффиксом.Свойство("Серия") Тогда
				...
				ДобавитьКолонкуВТаблицуТоваров(
					ТаблицаТоваров,
					ИменаПолейССуффиксом.Серия,
					ТипСерииНоменклатуры);
				
			КонецЕсли;
			
		КонецЕсли;
		
		ТаблицаТоваров.Колонки.Добавить(ПараметрыУказанияСерий.ИмяПоляКоличество, ТипКоличество);
		ТаблицаТоваров.Колонки.Добавить("НомерСтроки", ТипНомерСтроки);
		
		ДополнитьТаблицуКолонкамиПоПолямПараметровУказанияСерий(ПараметрыУказанияСерий, ТаблицаТоваров, МетаданныеОбъекта);
		
		ТаблицаСерий = Новый ТаблицаЗначений;
		ТаблицаСерий.Колонки.Добавить("Номенклатура", ТипНоменклатура);
		ТаблицаСерий.Колонки.Добавить("Характеристика", ТипХарактеристикиНоменклатуры);
		ТаблицаСерий.Колонки.Добавить(ПараметрыУказанияСерий.ИмяПоляКоличество, ТипКоличество);
		
		ДополнитьТаблицуКолонкамиПоПолямПараметровУказанияСерий(ПараметрыУказанияСерий, ТаблицаСерий, МетаданныеОбъекта, Истина);
		
		Для Каждого СтрокаТЧ Из ТаблицаТоваровПредварительная Цикл
			ЗаполнитьЗначенияСвойств(ТаблицаТоваров.Добавить(), СтрокаТЧ);
		КонецЦикла;
		
		ТаблицаТоваров.ЗагрузитьКолонку(
			ТаблицаТоваровПредварительная.ВыгрузитьКолонку("ИдентификаторСтрокиВТ"),
			"НомерСтроки");
		
		ТаблицаСтатусыУказанияСерий = ВыборкаИзЗапросаРасчетаСтатусовУказанияСерий(
		...
		ОписаниеЗапроса.ПараметрыЗапроса.Вставить("ТаблицаСтатусыУказанияСерий", ТаблицаСтатусыУказанияСерий);
		
		СписокЗапросов = ТекстыЗапросовПроверитьСериюРассчитатьСтатус(
			ПараметрыУказанияСерий.ИменаПолейСтатусУказанияСерий);
...
// Добавляет запрос в пакет запросов для проверки корректности указания и заполнения серии номенклатуры.
//
// Параметры:
//	СтруктураДействий - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения.
//	ОписаниеЗапроса - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОписаниеЗапроса.
//	КэшированныеЗначения - Структура.
//
Процедура ДополнитьТекстЗапросаПроверитьСериюПоВладельцу(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	
	Перем ПараметрыДействия;
	
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
			"ПроверитьСериюПоВладельцу",
			СтруктураДействий,
			КэшированныеЗначения,
			ПараметрыДействия) Тогда
		
		СписокЗапросов = ТекстыЗапросовПроверитьСериюПоВладельцу();
...
// Возвращаемое значение:
//  Структура - содержит:
//  	* ТипПолитики - ФиксированныйМассив Из ПеречислениеСсылка.ТипыПолитикУказанияСерий
//  	* ДляСклада - Булево
...
//  	* УчетСерийПереданныхПереработчикуТоваров - Булево
//  	* УчетСерийВПереданныхНаКомиссиюТоваров - Булево
//  	* УчетСерийТоваровОтгруженныхКлиенту - Булево
//
Функция ОтборПолитикУчетаСерийПоПараметрам(Параметры) Экспорт
	...
	ИспользоватьПроизводствоНаСтороне     = Ложь;
	...
	Если ЗначениеЗаполнено(ВидНоменклатуры)
		И (ИспользоватьНеотфактурованныеПоставки
			...
			Или ИспользоватьПередачуТоваровНаХранение
			Или ИспользоватьКомиссиюПриПродажах
			Или ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5) Тогда
		...
		ИменаРеквизитов.Вставить(
			...
			"ПолитикаУчетаСерий.УчетСерийВПереданныхНаКомиссиюТоваров");
		ИменаРеквизитов.Вставить(
			"УчетСерийТоваровОтгруженныхКлиенту",
			"ПолитикаУчетаСерий.УчетСерийТоваровОтгруженныхКлиенту");
		...
		Отбор.Вставить(
			"УчетСерийВПереданныхНаКомиссиюТоваров",
			РеквизитыТекущейПолитикиУчетаСерий.УчетСерийВПереданныхНаКомиссиюТоваров);
		
		Отбор.Вставить(
			"УчетСерийТоваровОтгруженныхКлиенту",
			РеквизитыТекущейПолитикиУчетаСерий.УчетСерийТоваровОтгруженныхКлиенту);
...
Функция ОтборВладельцаПодчиненныхСправочников(ВидНоменклатуры, ИмяРеквизитаВладелец) Экспорт
	...
	ИначеЕсли ИмяРеквизитаВладелец = "ВладелецСерий" Тогда
		...
		Если ОсобенностиУчетаОтКоторыхЗависятРеквизитыСерий.Найти(ВидНоменклатуры.ОсобенностьУчета) <> Неопределено Тогда
			ОтборВладельцаПодчиненныхСправочников.Вставить("ОсобенностьУчета", 
				Новый ФиксированныйМассив(СовместимыеОсобенностиУчетаПоСерии(ВидНоменклатуры.ОсобенностьУчета)));
		Иначе
			ДанныеВыбора = Новый СписокЗначений;
...
Функция СовместимыеОсобенностиУчетаПоСерии(ОсобенностьУчета)
	
	ОсобенностиУчета = Новый Массив;
	Исключить = Новый Соответствие;
	ПерваяОсобенность = Истина;
	ОписанияИспользованияРеквизитовСерии = Справочники.ВидыНоменклатуры.ОписанияИспользованияРеквизитовСерии();
	...
	Для Каждого Описания из ОписанияИспользованияРеквизитовСерии Цикл
		...
		Если Не ПустаяСтрока(Описания.ОсобенностиУчета) Тогда
			...
			ИменаОсобенностей = СтрРазделить(Описания.ОсобенностиУчета, ",");
			ОсобенностиОписания = Новый Массив;
			
			Для Каждого ИмяОсобенности из ИменаОсобенностей Цикл
				ЗначениеПеречисления = ПредопределенноеЗначение("Перечисление.ОсобенностиУчетаНоменклатуры." + ИмяОсобенности);
				ОсобенностиОписания.Добавить(ЗначениеПеречисления);
			КонецЦикла;
			
			Если ОсобенностиОписания.Найти(ОсобенностьУчета) = Неопределено Тогда
				Для Каждого ЗначениеПеречисления Из ОсобенностиОписания Цикл
					Исключить.Вставить(ЗначениеПеречисления);
				КонецЦикла;
				Продолжить;
			КонецЕсли;
			
			Если ПерваяОсобенность Тогда
				ОсобенностиУчета = ОсобенностиОписания;
				ПерваяОсобенность = Ложь;
			Иначе 
				Пересечение = Новый Массив;
				Для Каждого ЗначениеПеречисления Из ОсобенностиУчета Цикл
					Если ОсобенностиОписания.Найти(ЗначениеПеречисления) <> Неопределено Тогда
						Пересечение.Добавить(ЗначениеПеречисления);
					КонецЕсли;
				КонецЦикла;
				ОсобенностиУчета = Пересечение;
			КонецЕсли;
	...
	Для Каждого КлючИЗначение Из Исключить Цикл
		ИсключениеОсобенности = ОсобенностиУчета.Найти(КлючИЗначение.Ключ);
		Если ИсключениеОсобенности <> Неопределено Тогда
			ОсобенностиУчета.Удалить(ИсключениеОсобенности);
		КонецЕсли;
	КонецЦикла;
...
Процедура ДополнитьТаблицуКолонкамиПоПолямПараметровУказанияСерий(ПараметрыУказанияСерий, Таблица, Знач Метаданные = Неопределено, ЭтоТаблицаСерий = Ложь) Экспорт
	...
	Если ПараметрыУказанияСерий.ПоляСвязи.Количество() > 0
		Или Не ЭтоТаблицаСерий И ПараметрыУказанияСерий.ИменаПолейДополнительные.Количество() > 0 Тогда
...
Процедура ЗаполнитьСерииПоFEFOВТЧСерии(Объект, ПараметрыУказанияСерий)
	...
	КонтролируемаяПартия = Неопределено;
	Если ПараметрыУказанияСерий.ИмяПоляКонтролируемаяПартия = Неопределено Тогда
		КонтролируемаяПартия = "ЗНАЧЕНИЕ(Справочник.КонтролируемыеПартии.ПустаяСсылка)";
	ИначеЕсли СтрНайти(ПараметрыУказанияСерий.ИмяПоляКонтролируемаяПартия, ".") > 0 Тогда
		КонтролируемаяПартия = "ДанныеДокумента." + СтрЗаменить(ПараметрыУказанияСерий.ИмяПоляКонтролируемаяПартия, ".", "");
	Иначе
		КонтролируемаяПартия = "&КонтролируемаяПартия";
		Запрос.УстановитьПараметр("КонтролируемаяПартия", Объект[ПараметрыУказанияСерий.ИмяПоляКонтролируемаяПартия]);
	КонецЕсли;
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&КонтролируемаяПартияПереопределяемый", КонтролируемаяПартия);
	...
	Если ПараметрыУказанияСерий.ИмяПоляКонтролируемаяПартия <> Неопределено
			И СтрНайти(ПараметрыУказанияСерий.ИмяПоляКонтролируемаяПартия, ".") = 0 Тогда
		Запрос.УстановитьПараметр("КонтролируемаяПартия", Объект[ПараметрыУказанияСерий.ИмяПоляКонтролируемаяПартия]);
	КонецЕсли;
	...
	Пока ВыборкаНомераСтрок.Следующий() Цикл
		...
		ИзменяемаяСтрока = СтрокаТоваров; // СтрокаТабличнойЧасти
			...
			Если ПараметрыУказанияСерий.ИмяПоляКонтролируемаяПартия = Неопределено Тогда
				КонтролируемаяПартия = "ЗНАЧЕНИЕ(Справочник.КонтролируемыеПартии.ПустаяСсылка)";
			ИначеЕсли СтрНайти(ПараметрыУказанияСерий.ИмяПоляКонтролируемаяПартия, ".") > 0 Тогда
				КонтролируемаяПартия = СтрокаТоваров[СтрЗаменить(ПараметрыУказанияСерий.ИмяПоляКонтролируемаяПартия, ".", "")];
			Иначе
				КонтролируемаяПартия = Объект[ПараметрыУказанияСерий.ИмяПоляКонтролируемаяПартия];
			КонецЕсли;
			
			ОстаткиСерийПоТовару = ОстаткиСерийПоТоваруОрдера(СтрокаТоваров, СоответствиеОстатки, КонтролируемаяПартия);
		...
		Для Каждого СтруктураОстаток Из ОстаткиСерийПоТовару Цикл
			...
			ОбрабатываемыеСтроки.Добавить(ИзменяемаяСтрока);
			...
			Если ОстатокТовара > 0 Тогда
				...
				ИсключаемыеСвойства = "Количество,КоличествоУпаковок,Серия";
				
				
				ЗаполнитьЗначенияСвойств(ИзменяемаяСтрока, СтрокаТоваров,,ИсключаемыеСвойства);
...
Функция ДанныеДляЗаполненияСерийПоFEFOВОрдере(Объект, ПараметрыУказанияСерий, Запрос)
	...
	|	&КонтролируемаяПартияПереопределяемый КАК КонтролируемаяПартия,
	...
	|	ДанныеДокумента.КонтролируемаяПартия
	|ПОМЕСТИТЬ ТаблицаТоваровДляЗапроса
	|ИЗ
	|	ТаблицаТоваров КАК ДанныеДокумента
	|ГДЕ
	...
	|	СерииТоваров.КонтролируемаяПартия КАК КонтролируемаяПартия,
	...
	|				И ТоварыНаСкладах.КонтролируемаяПартия = ТаблицаТоваров.КонтролируемаяПартия
	...
	|	СерииТоваров.КонтролируемаяПартия,
	|	СерииТоваров.Серия,
	...
	Пока ВыборкаОстатки.СледующийПоЗначениюПоля("Номенклатура") Цикл
		...
		Пока ВыборкаОстатки.СледующийПоЗначениюПоля("Характеристика") Цикл
			...
			Пока ВыборкаОстатки.СледующийПоЗначениюПоля("Назначение") Цикл
				СоответствиеХарактеристика.Вставить(ВыборкаОстатки.Назначение, Новый Соответствие);
				СоответствиеНазначение = СоответствиеХарактеристика[ВыборкаОстатки.Назначение];
				Пока ВыборкаОстатки.СледующийПоЗначениюПоля("КонтролируемаяПартия") Цикл
					СоответствиеНазначение.Вставить(ВыборкаОстатки.КонтролируемаяПартия, Новый Массив);
					МассивОстатки = СоответствиеНазначение[ВыборкаОстатки.КонтролируемаяПартия]; // Массив
...
Функция ОстаткиСерийПоТоваруОрдера(СтрокаТоваров, СоответствиеОстатки, КонтролируемаяПартия)
	...
	Если СоответствиеОстатки[СтрокаТоваров.Номенклатура] = Неопределено			
		...
		Или СоответствиеОстатки[СтрокаТоваров.Номенклатура][СтрокаТоваров.Характеристика][СтрокаТоваров.Назначение] = Неопределено
		Или СоответствиеОстатки[СтрокаТоваров.Номенклатура][СтрокаТоваров.Характеристика][СтрокаТоваров.Назначение][КонтролируемаяПартия] = Неопределено Тогда
		
		Остатки = Неопределено;
		
	Иначе
		
		Остатки = СоответствиеОстатки[СтрокаТоваров.Номенклатура][СтрокаТоваров.Характеристика][СтрокаТоваров.Назначение][КонтролируемаяПартия];
...
Функция ТекстЗапросаДляЗаполненияСерийПОFEFO(ПараметрыУказанияСерий)
	...
	|	ДанныеДокумента.Характеристика КАК Характеристика,
	|	ДанныеДокумента.Назначение КАК Назначение,
	|	&КонтролируемаяПартияПереопределяемый КАК КонтролируемаяПартияДляЗапроса,
	...
	|	ТаблицаТоваровДляЗапроса.КонтролируемаяПартияДляЗапроса КАК КонтролируемаяПартия,
	...
	|	ТаблицаТоваровДляЗапроса.Характеристика,
	|	ТаблицаТоваровДляЗапроса.Назначение,
	|	ТаблицаТоваровДляЗапроса.КонтролируемаяПартияДляЗапроса";
	...
	|		РегистрНакопления.ТоварыНаСкладах.Остатки(
	|				,
	|				(Номенклатура, Характеристика, Назначение, КонтролируемаяПартия, Склад) В
	|					(ВЫБРАТЬ
	|						ТаблицаТоваровДляЗапроса.Номенклатура,
	|						ТаблицаТоваровДляЗапроса.Характеристика,
	|						ТаблицаТоваровДляЗапроса.Назначение,
	...
	|				И ТоварыНаСкладах.КонтролируемаяПартия = ТаблицаТоваровДляЗапроса.КонтролируемаяПартия
	...
	|			И (ДанныеДокумента.КонтролируемаяПартияДляЗапроса = ТаблицаОстатковПоСериям.КонтролируемаяПартия)
...
Функция ЗаполнитьСерииПоДаннымТоваровУПартнеров(ТаблицаТовары, ДокументОбъект, ЗаполнятьПоОтрицательнымОстаткам) Экспорт
	...
	|ГДЕ
	|	ТаблицаДокумента.СтатусУказанияСерий <> 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|
	|ВЫБРАТЬ
	|	&Соглашение КАК ДокументПоступления,
	|	ТаблицаДокумента.Номенклатура КАК Номенклатура,
	|	ТаблицаДокумента.Характеристика КАК Характеристика,
	|	ТаблицаДокумента.Назначение КАК Назначение,
	|	ТаблицаДокумента.Склад КАК Склад
	...
	|
	|ВЫБРАТЬ
	|	&Договор КАК ДокументПоступления,
	|	ТаблицаДокумента.Номенклатура КАК Номенклатура,
	|	ТаблицаДокумента.Характеристика КАК Характеристика,
	|	ТаблицаДокумента.Назначение КАК Назначение,
	|	ТаблицаДокумента.Склад КАК Склад
	...
	|		&Коэффициент * ТоварыКПоступлениюОстатки.КОформлениюПоступленийПоНакладнымОстаток КАК КоличествоПоДаннымПартнера
	|	ИЗ
	|		РегистрНакопления.ТоварыКПоступлению.Остатки(
	|				,
	|				(ДокументПоступления, Номенклатура, Характеристика, Назначение, Склад) В
	|					(ВЫБРАТЬ
	|						ТаблицаДокумента.ДокументПоступления КАК ДокументПоступления,
	|						ТаблицаДокумента.Номенклатура КАК Номенклатура,
	|						ТаблицаДокумента.Характеристика КАК Характеристика,
	...
	|						ТаблицаДокумента.Склад КАК Склад
	|					ИЗ
	|						ВТТаблицаДокумента КАК ТаблицаДокумента)) КАК ТоварыКПоступлениюОстатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		-&Коэффициент * ТоварыКПоступлению.КОформлениюПоступленийПоНакладным
	|	ИЗ
	|		РегистрНакопления.ТоварыКПоступлению КАК ТоварыКПоступлению
	|	ГДЕ
	|		ТоварыКПоступлению.Регистратор = &Ссылка
	|		И ТоварыКПоступлению.Активность) КАК ТаблицаОстатков
	|
	|СГРУППИРОВАТЬ ПО
	...
	Если ЗаполнятьПоОтрицательнымОстаткам Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Распоряжение", "ТаблицаДокумента.ЗаказПоставщику");
		Коэффициент = -1;
	Иначе
		Распоряжение = ДокументОбъект.Распоряжение;
		Распоряжение = ?(Распоряжение <> Неопределено, Распоряжение, Справочники.ДоговорыКонтрагентов.ПустаяСсылка());
	КонецЕсли;
	...
	Для Каждого Строка Из ТаблицаТоварыУПартнеров Цикл
	...
	Ключ = "%1 Склад, Номенклатура, Характеристика, Назначение";
	
	Если РаспоряжениеЗаказ И ЗаполнятьПоОтрицательнымОстаткам Тогда
		ПолеКлюча = "ЗаказПоставщику,";
	Иначе
		ПолеКлюча = "";
	КонецЕсли;
	
	Ключ = СтрШаблон(Ключ, ПолеКлюча);
...
// Создает таблицу товаров для расчета статусов указания серия.
//
// Параметры:
//	ПараметрыУказанияСерий - см. НоменклатураКлиентСервер.ПараметрыУказанияСерий.
//
// Возвращаемое значение:
//	ТаблицаЗначений - таблица товаров, которая содержит следующие колонки:
//		* НомерСтроки - Число - номер строки таблицы.
//		* Номенклатура - СправочникСсылка.Номенклатура - товар, для которого рассчитывается статус указания серий.
...
//
Функция ТаблицаТоваровДляРасчетаСтатусовУказанияСерий(ПараметрыУказанияСерий)
	
	МетаданныеОбъекта	= ОбщегоНазначенияУТ.МетаданныеОбъектаПоПолномуИмени(ПараметрыУказанияСерий.ПолноеИмяОбъекта);
	ТипНомерСтроки		= Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(5, 0, ДопустимыйЗнак.Неотрицательный));
	ТипНоменклатура		= Новый ОписаниеТипов("СправочникСсылка.Номенклатура");
	...
	ТаблицаТоваров.Колонки.Добавить("НомерСтроки", ТипНомерСтроки);
	ТаблицаТоваров.Колонки.Добавить("Номенклатура", ТипНоменклатура);
	ТаблицаТоваров.Колонки.Добавить("Характеристика", ТипХарактеристика);
	ТаблицаТоваров.Колонки.Добавить("Серия", ТипСерия);
	ТаблицаТоваров.Колонки.Добавить(ПараметрыУказанияСерий.ИмяПоляКоличество, ТипКоличество);
	...
	Если ПараметрыУказанияСерий.ИменаПолейССуффиксом.Количество() Тогда
		...
		Если ИменаПолейССуффиксом.Свойство("Характеристика") Тогда
			...
			ДобавитьКолонкуВТаблицуТоваров(ТаблицаТоваров, ИмяРеквизита, ТипХарактеристика);
		КонецЕсли;
		
		Если ИменаПолейССуффиксом.Свойство("Серия") Тогда
			ИмяРеквизита = ИменаПолейССуффиксом.Серия;
			
			ДобавитьКолонкуВТаблицуТоваров(ТаблицаТоваров, ИмяРеквизита, ТипСерия);
		КонецЕсли;
	КонецЕсли;
...
// Создает таблицу серий для расчета статусов указания серия.
//
// Параметры:
//	ПараметрыУказанияСерий - см. НоменклатураКлиентСервер.ПараметрыУказанияСерий.
//
...
//		* Характеристика - СправочникСсылка.ХарактеристикиНоменклатуры - характеристика товара.
//
Функция ТаблицаСерийДляРасчетаСтатусовУказанияСерий(ПараметрыУказанияСерий)
	...
	ТаблицаСерий = Новый ТаблицаЗначений;
	ТаблицаСерий.Колонки.Добавить("Номенклатура", ТипНоменклатура);
	ТаблицаСерий.Колонки.Добавить("Характеристика", ТипХарактеристика);
	ТаблицаСерий.Колонки.Добавить(ПараметрыУказанияСерий.ИмяПоляКоличество, ТипКоличество);
	
	ДополнитьТаблицуКолонкамиПоПолямПараметровУказанияСерий(ПараметрыУказанияСерий,
															ТаблицаСерий,
															МетаданныеОбъекта,
															Истина);
...
//
// Параметры:
// 	ДанныеВыбора - СписокЗначений Из ЭлементСпискаЗначений - 
...
Процедура УпаковкиЕдиницыИзмеренияОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	Если Параметры.Свойство("ВыборРодителя") Тогда
		...
		|	УпаковкиНоменклатуры.Наименование			КАК ПредставлениеСовпадения,
		|	0											КАК Порядок
		|ПОМЕСТИТЬ УпаковкиНоменклатурыПоиск
		|ИЗ
		...
		|	И (НЕ &ОтборПоСтрокеПоиска
		|		ИЛИ УпаковкиНоменклатуры.Наименование ПОДОБНО &СтрокаПоиска СПЕЦСИМВОЛ ""~"")
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	УпаковкиНоменклатуры.ПометкаУдаления		КАК ПометкаУдаления,
		|	ПРЕДСТАВЛЕНИЕ(УпаковкиНоменклатуры.Ссылка)	КАК УпаковкаПредставление,
		|	УпаковкиНоменклатуры.НаименованиеЯзык1		КАК ПредставлениеСовпадения,
		|	1											КАК Порядок
		|ИЗ
		|	Справочник.УпаковкиЕдиницыИзмерения КАК УпаковкиНоменклатуры
		|ГДЕ
		|	&ИспользуютсяДопЯзыки
		|	И УпаковкиНоменклатуры.Ссылка <> &Ссылка
		|	И УпаковкиНоменклатуры.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Упаковка)
		|	И УпаковкиНоменклатуры.Владелец = &Владелец
		|	И УпаковкиНоменклатуры.НаименованиеЯзык1 ПОДОБНО &СтрокаПоиска СПЕЦСИМВОЛ ""~""
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	УпаковкиНоменклатуры.ПометкаУдаления		КАК ПометкаУдаления,
		|	ПРЕДСТАВЛЕНИЕ(УпаковкиНоменклатуры.Ссылка)	КАК УпаковкаПредставление,
		|	УпаковкиНоменклатуры.НаименованиеЯзык2		КАК ПредставлениеСовпадения,
		|	2											КАК Порядок
		|ИЗ
		|	Справочник.УпаковкиЕдиницыИзмерения КАК УпаковкиНоменклатуры
		|ГДЕ
		...
		|	И УпаковкиНоменклатуры.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Упаковка)
		|	И УпаковкиНоменклатуры.Владелец = &Владелец
		|	И УпаковкиНоменклатуры.НаименованиеЯзык2 ПОДОБНО &СтрокаПоиска СПЕЦСИМВОЛ ""~""
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Упаковка,
		|	Порядок
		...
		|	МИНИМУМ(УпаковкиНоменклатуры.Порядок)	КАК Порядок
		|ПОМЕСТИТЬ УпаковкиНоменклатурыПоПорядку
		|ИЗ
		|	УпаковкиНоменклатурыПоиск КАК УпаковкиНоменклатуры
		|
		|СГРУППИРОВАТЬ ПО
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	УпаковкиНоменклатурыПоПорядку.Упаковка			КАК Упаковка,
		|	УпаковкиНоменклатурыПоиск.ПометкаУдаления		КАК ПометкаУдаления,
		|	УпаковкиНоменклатурыПоиск.УпаковкаПредставление	КАК УпаковкаПредставление,
		|	УпаковкиНоменклатурыПоПорядку.Порядок			КАК Порядок,
		|	ЕСТЬNULL(УпаковкиНоменклатурыПоиск.ПредставлениеСовпадения, """") КАК ПредставлениеСовпадения
		|ИЗ
		|	УпаковкиНоменклатурыПоПорядку КАК УпаковкиНоменклатурыПоПорядку
		|		ЛЕВОЕ СОЕДИНЕНИЕ УпаковкиНоменклатурыПоиск КАК УпаковкиНоменклатурыПоиск
		|		ПО УпаковкиНоменклатурыПоПорядку.Упаковка = УпаковкиНоменклатурыПоиск.Упаковка
		|			И УпаковкиНоменклатурыПоПорядку.Порядок = УпаковкиНоменклатурыПоиск.Порядок
		|
		|УПОРЯДОЧИТЬ ПО
		|	Порядок,
		|	ПредставлениеСовпадения";
		
		ОтборПоСтрокеПоиска		= Параметры.СтрокаПоиска <> Неопределено
									И Не ПустаяСтрока(Параметры.СтрокаПоиска);
		СтрокаПоиска			= ?(ОтборПоСтрокеПоиска,
									ОбщегоНазначения.СформироватьСтрокуДляПоискаВЗапросе(Параметры.СтрокаПоиска) + "%",
									"");
		ИспользуютсяДопЯзыки	= ?(ОтборПоСтрокеПоиска,
									МультиязычностьПовтИсп.КонфигурацияИспользуетТолькоОдинЯзык(Ложь),
									Ложь);
		
		Запрос.УстановитьПараметр("Ссылка",					Параметры.Ссылка);
		Запрос.УстановитьПараметр("Владелец",				Параметры.Отбор.Владелец);
		Запрос.УстановитьПараметр("ОтборПоСтрокеПоиска",	ОтборПоСтрокеПоиска);
		Запрос.УстановитьПараметр("СтрокаПоиска",			СтрокаПоиска);
		Запрос.УстановитьПараметр("ИспользуютсяДопЯзыки",	ИспользуютсяДопЯзыки);
	...
	ИначеЕсли Параметры.Свойство("Номенклатура")
		И Не (Параметры.Свойство("ВсегдаВыбиратьЕдиницуИзмерения")
			И Не ЗначениеЗаполнено(Параметры.Номенклатура)) Тогда 
...
Процедура ЗаполнитьДанныеВыбораУпаковками(Номенклатура, ДанныеВыбора, Параметры, Знач ПоказыватьМерныеЕдиницыИзмерения, ДобавлятьЕдиницуХранения, ПоказыватьТоварныеМеста)
	...
	|	Справочник.Номенклатура КАК Номенклатура
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК СпрУпаковки
	|		ПО Номенклатура.ЕдиницаИзмерения = СпрУпаковки.Ссылка
	|ГДЕ
	|	Номенклатура.Ссылка = &Номенклатура
	|	И (&ДобавлятьЕдиницуХранения
	|		ИЛИ СпрУпаковки.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Вес)
	|		ИЛИ СпрУпаковки.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Объем)
	|		ИЛИ СпрУпаковки.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Длина)
	|		ИЛИ СпрУпаковки.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Площадь)
	|		ИЛИ СпрУпаковки.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Мощность)
	|		ИЛИ СпрУпаковки.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Энергия)
	|		ИЛИ СпрУпаковки.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.ЭлектрическийЗаряд)
	|		ИЛИ СпрУпаковки.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Время))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ПОМЕСТИТЬ УпаковкиЕдиницыИзмеренияСгруппированная
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	УпаковкиЕдиницыИзмерения.Упаковка КАК Упаковка,
	|	УпаковкиЕдиницыИзмерения.ПометкаУдаления КАК ПометкаУдаления,
	|	УпаковкиЕдиницыИзмерения.ЭтоЕдиницаХранения КАК ЭтоЕдиницаХранения,
	|	ДанныеСправочника.Наименование КАК ПредставлениеСовпадения,
	|	УпаковкиЕдиницыИзмерения.Порядок КАК Порядок,
	|	0 КАК ПорядокПоиска,
	|	УпаковкиЕдиницыИзмерения.Коэффициент КАК Коэффициент,
	|	УпаковкиЕдиницыИзмерения.ДопустимыйКоэффициент КАК ДопустимыйКоэффициент
	|ПОМЕСТИТЬ УпаковкиНоменклатурыПоиск
	|ИЗ
	|	УпаковкиЕдиницыИзмеренияСгруппированная КАК УпаковкиЕдиницыИзмерения
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК ДанныеСправочника
	|		ПО УпаковкиЕдиницыИзмерения.Упаковка = ДанныеСправочника.Ссылка
	|
	|ГДЕ
	|	НЕ &ОтборПоСтрокеПоиска
	...
	|	УпаковкиЕдиницыИзмерения.ПометкаУдаления КАК ПометкаУдаления,
	|	УпаковкиЕдиницыИзмерения.ЭтоЕдиницаХранения КАК ЭтоЕдиницаХранения,
	|	ДанныеСправочника.НаименованиеЯзык1 КАК ПредставлениеСовпадения,
	|	УпаковкиЕдиницыИзмерения.Порядок КАК Порядок,
	|	1 КАК ПорядокПоиска,
	|	УпаковкиЕдиницыИзмерения.Коэффициент КАК Коэффициент,
	|	УпаковкиЕдиницыИзмерения.ДопустимыйКоэффициент КАК ДопустимыйКоэффициент
	...
	|ГДЕ
	|	&ИспользуютсяДопЯзыки
	|	И ДанныеСправочника.НаименованиеЯзык1 ПОДОБНО &СтрокаПоиска СПЕЦСИМВОЛ ""~""
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	УпаковкиЕдиницыИзмерения.ПометкаУдаления КАК ПометкаУдаления,
	|	УпаковкиЕдиницыИзмерения.ЭтоЕдиницаХранения КАК ЭтоЕдиницаХранения,
	|	ДанныеСправочника.НаименованиеЯзык2 КАК ПредставлениеСовпадения,
	|	УпаковкиЕдиницыИзмерения.Порядок КАК Порядок,
	|	2 КАК ПорядокПоиска,
	|	УпаковкиЕдиницыИзмерения.Коэффициент КАК Коэффициент,
	|	УпаковкиЕдиницыИзмерения.ДопустимыйКоэффициент КАК ДопустимыйКоэффициент
	...
	|ГДЕ
	|	&ИспользуютсяДопЯзыки
	|	И ДанныеСправочника.НаименованиеЯзык2 ПОДОБНО &СтрокаПоиска СПЕЦСИМВОЛ ""~""
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Упаковка,
	|	ПорядокПоиска
	...
	|ВЫБРАТЬ
	|	УпаковкиЕдиницыИзмерения.Упаковка КАК Упаковка,
	|	МИНИМУМ(УпаковкиЕдиницыИзмерения.ПорядокПоиска) КАК ПорядокПоиска
	|ПОМЕСТИТЬ УпаковкиНоменклатурыПоПорядку
	|ИЗ
	|	УпаковкиНоменклатурыПоиск КАК УпаковкиЕдиницыИзмерения
	|
	|СГРУППИРОВАТЬ ПО
	|	УпаковкиЕдиницыИзмерения.Упаковка
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Упаковка,
	|	ПорядокПоиска
	...
	|	УпаковкиНоменклатурыПоПорядку.Упаковка КАК Упаковка,
	|	УпаковкиНоменклатурыПоиск.ПометкаУдаления КАК ПометкаУдаления,
	|	УпаковкиНоменклатурыПоиск.ЭтоЕдиницаХранения КАК ЭтоЕдиницаХранения,
	|	ПРЕДСТАВЛЕНИЕ(УпаковкиНоменклатурыПоПорядку.Упаковка) КАК УпаковкаПредставление,
	|	УпаковкиНоменклатурыПоиск.Порядок КАК Порядок,
	|	УпаковкиНоменклатурыПоПорядку.ПорядокПоиска КАК ПорядокПоиска,
	|	ЕСТЬNULL(УпаковкиНоменклатурыПоиск.ПредставлениеСовпадения, """") КАК ПредставлениеСовпадения,
	|	УпаковкиНоменклатурыПоиск.Коэффициент КАК Коэффициент,
	|	УпаковкиНоменклатурыПоиск.ДопустимыйКоэффициент КАК ДопустимыйКоэффициент
	|ИЗ
	|	УпаковкиНоменклатурыПоПорядку КАК УпаковкиНоменклатурыПоПорядку
	|		ЛЕВОЕ СОЕДИНЕНИЕ УпаковкиНоменклатурыПоиск КАК УпаковкиНоменклатурыПоиск
	|		ПО УпаковкиНоменклатурыПоПорядку.Упаковка = УпаковкиНоменклатурыПоиск.Упаковка
	|			И УпаковкиНоменклатурыПоПорядку.ПорядокПоиска = УпаковкиНоменклатурыПоиск.ПорядокПоиска
	|
	|УПОРЯДОЧИТЬ ПО
	|	Порядок,
	|	Коэффициент УБЫВ,
...
Процедура ХарактеристикиНоменклатурыОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	Для Каждого КлючИЗначениеОтбора Из Параметры.Отбор Цикл
		...
		Если ИменаНестандартныхОтборов.Свойство(ИмяОтбора, ИмяНестандартногоОтбора) Тогда
			...
			Если ИмяНестандартногоОтбора = "ТипНоменклатуры" 
				...
				ТекстОтбора = "(СпрНоменклатура.ТипНоменклатуры В (&ТипНоменклатуры)
				...
				УсловиеОтбора.Добавить(ТекстОтбора);
				...
				ОписаниеСоединения =
					...
					|И ВариантыКомплектацииНоменклатуры.Характеристика = СпрХарактеристики.Ссылка
	...
	Если НЕ ЗначениеЗаполнено(УсловиеОтбора) Тогда
		УсловиеОтбора.Добавить("ИСТИНА");
	КонецЕсли;
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеОтбора", СтрСоединить(УсловиеОтбора, "
	|	И "));
	Запрос.Текст = ТекстЗапроса;
	ДанныеВыбора = Новый СписокЗначений;
...
Функция ПроверитьОкруглениеКоличестваВнутренний(Объект, ПараметрыПроверки, ДопустимыйПроцентОтклонения)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТДанныеТаблицы.НомерСтроки КАК НомерСтроки,
	|	ВТДанныеТаблицы.Номенклатура КАК Номенклатура,
	|	ВТДанныеТаблицы.КоличествоУпаковок КАК КоличествоУпаковок,
	|	ВТДанныеТаблицы.КоличествоВДокументе КАК КоличествоВДокументе,
	|	ВТДанныеТаблицы.Упаковка КАК Упаковка,
	|	ВТДанныеТаблицы.КоличествоУпаковокСуффикс КАК КоличествоУпаковокСуффикс,
	|	ВТДанныеТаблицы.КоличествоВДокументеСуффикс КАК КоличествоВДокументеСуффикс
	|ПОМЕСТИТЬ ВТДляЗапроса
	|ИЗ
	|	ВТДанныеТаблицы КАК ВТДанныеТаблицы
	|ГДЕ
	|	ЕСТЬNULL(ВТДанныеТаблицы.Упаковка.ТипУпаковки, НЕОПРЕДЕЛЕНО) <> ЗНАЧЕНИЕ(Перечисление.ТипыУпаковокНоменклатуры.ТоварноеМесто)
	|	И ВТДанныеТаблицы.Номенклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины = &ШтучныйТип
	|	И &УсловиеОтбораСтрокПоДополнительнымПолям
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТДанныеТаблицы";
	...
	ИначеЕсли ИмяТЧ = "Объект" Тогда
		...
		Таблица.Колонки.Добавить("Упаковка", Новый ОписаниеТипов("СправочникСсылка.УпаковкиЕдиницыИзмерения"));
		Таблица.Колонки.Добавить("Количество", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля(ДопустимыйЗнак.Неотрицательный));
		Таблица.Колонки.Добавить("КоличествоУпаковок", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля(ДопустимыйЗнак.Неотрицательный));
	...
	Запрос.Текст = ТекстЗапроса;
	Запрос.Выполнить();
	Результат = МенеджерВременныхТаблиц.Таблицы[0].ПолучитьДанные();
	Если Результат.Пустой() Тогда
		Возврат Результат;
	КонецЕсли;
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ВТДляЗапроса.НомерСтроки КАК НомерСтроки,
	|	ВТДляЗапроса.Упаковка КАК Упаковка,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК УпаковкиЕдиницыИзмерения
	|		ПО ВТДляЗапроса.Упаковка = УпаковкиЕдиницыИзмерения.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТДляЗапроса.НомерСтроки КАК НомерСтроки,
	|	ВТКоэффициенты.ТипИзмеряемойВеличины В (&МерныеТипы) КАК МожноОкруглять,
	|	ВТДляЗапроса.Номенклатура.ЕдиницаИзмерения КАК БазоваяЕдиницаИзмерения,
	|	ВТДляЗапроса.Номенклатура КАК Номенклатура,
	|	ВТКоэффициенты.Упаковка КАК Упаковка,
	|	ВТКоэффициенты.КоэффициентУпаковки КАК КоэффициентУпаковки,
	|	ВЫРАЗИТЬ(ВТДляЗапроса.КоличествоУпаковок * ВТКоэффициенты.КоэффициентУпаковки КАК ЧИСЛО(15, 3)) КАК Количество,
	|	ВЫРАЗИТЬ(ВТДляЗапроса.КоличествоУпаковок * ВТКоэффициенты.КоэффициентУпаковки КАК ЧИСЛО(15, 0)) КАК КоличествоОкругленное,
	|	ВТДляЗапроса.КоличествоВДокументе КАК КоличествоВДокументе,
	|	ВЫРАЗИТЬ(ВТДляЗапроса.КоличествоУпаковокСуффикс * ВТКоэффициенты.КоэффициентУпаковки КАК ЧИСЛО(15, 3)) КАК КоличествоСуффикс,
	|	ВЫРАЗИТЬ(ВТДляЗапроса.КоличествоУпаковокСуффикс * ВТКоэффициенты.КоэффициентУпаковки КАК ЧИСЛО(15, 0)) КАК КоличествоСуффиксОкругленное,
	...
	|		ПО ВТДляЗапроса.Упаковка = ВТКоэффициенты.Упаковка
	|		И ВТДляЗапроса.НомерСтроки = ВТКоэффициенты.НомерСтроки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ПутьКПолюТаблицы(ПараметрыПроверки, НомерСтроки, ИмяКолонки)
	
	Если ПараметрыПроверки.ЭтоПроверкаТекущейСтроки Тогда
		
		ВозвращаемоеЗначение = ОбщегоНазначенияУТКлиентСервер.ПутьКЭлементуТекущейСтрокиТаблицыФормы(
			ПараметрыПроверки.ИмяТЧ, // Считаем, что элемент имеет то же имя, что и ТЧ
			ИмяКолонки);
		
	Иначе
		
		ВозвращаемоеЗначение = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
...
Функция ТекстыЗапросовПроверитьЗаполнитьУпаковкуПоВладельцу()
	...
	|ПОМЕСТИТЬ ПроверитьЗаполнитьУпаковкуПоВладельцу_Проверка
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВтИсточникДанных.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ
	|	ИЗ
	...
	Результат.Добавить(ТекстЗапроса, "ПроверитьЗаполнитьУпаковкуПоВладельцу_Проверка");
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ВтИсточникДанных.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|	ПоискУпаковок.ИдентификаторСтрокиВТ ЕСТЬ НЕ NULL КАК ПроверкаУспешна
	|ИЗ
	|	ВтИсточникДанных КАК ВтИсточникДанных
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПроверитьЗаполнитьУпаковкуПоВладельцу_Проверка КАК ПоискУпаковок
	|		ПО ВтИсточникДанных.ИдентификаторСтрокиВТ = ПоискУпаковок.ИдентификаторСтрокиВТ";
...
// Возвращает список запросов для поиска идентичной характеристики.
//
// Возвращаемое значение:
// 	СписокЗначений Из Строка.
//
Функция ТекстыЗапросовЗаполнитьХарактеристикуНекачественногоТовара()
	...
	"ВЫБРАТЬ
	|	ВтИсточникДанных.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|	ЕСТЬNULL(МАКСИМУМ(ТаблицаХарактеристик.Ссылка), ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)) КАК
	|		Характеристика,
	|	ЕСТЬNULL(МАКСИМУМ(ТаблицаНоменклатурыИсточника.ИспользованиеХарактеристик),
	|				ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.ПустаяСсылка)) КАК НоменклатураИспользованиеХарактеристик
	|ПОМЕСТИТЬ ХарактеристикиНекачественногоТовара
	|ИЗ
	|	ВтИсточникДанных КАК ВтИсточникДанных
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК ТаблицаНоменклатурыИсточника
	|		ПО ВтИсточникДанных.Номенклатура = ТаблицаНоменклатурыИсточника.Ссылка
	|		И
	|			ТаблицаНоменклатурыИсточника.ИспользованиеХарактеристик = ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.ИндивидуальныеДляНоменклатуры)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатуры КАК ТаблицаХарактеристикИсточника
	|		ПО ВтИсточникДанных.Характеристика = ТаблицаХарактеристикИсточника.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатуры КАК ТаблицаХарактеристик
	|		ПО ВтИсточникДанных.Номенклатура = ТаблицаХарактеристик.Владелец
	|		И ТаблицаХарактеристикИсточника.Ссылка <> ТаблицаХарактеристик.Ссылка
	|		И ТаблицаХарактеристикИсточника.Наименование = ТаблицаХарактеристик.Наименование
	|СГРУППИРОВАТЬ ПО
	|	ВтИсточникДанных.ИдентификаторСтрокиВТ";
	
	Результат.Добавить(ТекстЗапроса, "ХарактеристикиНекачественногоТовара");
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаРезультат.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|	ТаблицаРезультат.Характеристика КАК Характеристика,
	|	ТаблицаРезультат.НоменклатураИспользованиеХарактеристик КАК НоменклатураИспользованиеХарактеристик
	|ИЗ
	|	ХарактеристикиНекачественногоТовара КАК ТаблицаРезультат";
	
	Результат.Добавить(ТекстЗапроса, "ЗаполнитьХарактеристикуНекачественногоТовара");
	
	Возврат Результат;
	
КонецФункции

// Возвращает список запросов для поиска идентичной упаковки.
//
// Возвращаемое значение:
// 	СписокЗначений Из Строка.
//
Функция ТекстыЗапросовЗаполнитьУпаковкуНекачественногоТовара()
	...
	"ВЫБРАТЬ
	|	ВтИсточникДанных.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|	ТаблицаНоменклатуры.НаборУпаковок = ЗНАЧЕНИЕ(Справочник.НаборыУпаковок.ИндивидуальныйДляНоменклатуры) КАК
	|		ЭтоИндивидуальныйНабор,
	|	ВтИсточникДанных.Номенклатура КАК Номенклатура,
	|	ВтИсточникДанных.Упаковка КАК Упаковка,
	|	ВЫБОР
	|		КОГДА ТаблицаНоменклатуры.НаборУпаковок = ЗНАЧЕНИЕ(Справочник.НаборыУпаковок.ИндивидуальныйДляНоменклатуры)
	|			ТОГДА ВтИсточникДанных.Номенклатура
	|		ИНАЧЕ ЕСТЬNULL(ТаблицаНоменклатуры.НаборУпаковок, ЗНАЧЕНИЕ(Справочник.НаборыУпаковок.ПустаяСсылка))
	|	КОНЕЦ КАК ВладелецУпаковок
	|ПОМЕСТИТЬ ЗаполнитьУпаковкуНекачественногоТовара_ИсточникДанных
	|ИЗ
	|	ВтИсточникДанных КАК ВтИсточникДанных
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК ТаблицаНоменклатуры
	|		ПО ВтИсточникДанных.Номенклатура = ТаблицаНоменклатуры.Ссылка
	...
	Результат.Добавить(ТекстЗапроса, "ЗаполнитьУпаковкуНекачественногоТовара_ИсточникДанных");
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ИсточникДанных.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|	ИсточникДанных.ЭтоИндивидуальныйНабор КАК ЭтоИндивидуальныйНабор,
	|	ЕСТЬNULL(МАКСИМУМ(УпаковкаНекачественногоТовара.Ссылка),
	|		ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)) КАК Упаковка
	|ПОМЕСТИТЬ УпаковкиНекачественногоТовара
	|ИЗ
	|	ЗаполнитьУпаковкуНекачественногоТовара_ИсточникДанных КАК ИсточникДанных
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК УпаковкаКачественногоТовара
	|		ПО ИсточникДанных.Упаковка = УпаковкаКачественногоТовара.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК УпаковкаНекачественногоТовара
	|		ПО УпаковкаКачественногоТовара.ЕдиницаИзмерения = УпаковкаНекачественногоТовара.ЕдиницаИзмерения
	|		И УпаковкаКачественногоТовара.СкладскаяГруппа = УпаковкаНекачественногоТовара.СкладскаяГруппа
	|		И ИсточникДанных.ВладелецУпаковок = УпаковкаНекачественногоТовара.Владелец
	|		И ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, 1) = ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки2, 1)
	|СГРУППИРОВАТЬ ПО
	|	ИсточникДанных.ИдентификаторСтрокиВТ,
	|	ИсточникДанных.ЭтоИндивидуальныйНабор";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ТекстЗапросаКоэффициентУпаковки1",
		Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки(
			"УпаковкаКачественногоТовара"));
		
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ТекстЗапросаКоэффициентУпаковки2",
		Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки(
			"УпаковкаНекачественногоТовара"));
	
	Результат.Добавить(ТекстЗапроса, "УпаковкиНекачественногоТовара");
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаРезультат.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|	ТаблицаРезультат.ЭтоИндивидуальныйНабор КАК ЭтоИндивидуальныйНабор,
	|	ТаблицаРезультат.Упаковка КАК Упаковка
	|ИЗ
	|	УпаковкиНекачественногоТовара КАК ТаблицаРезультат";
	
	Результат.Добавить(ТекстЗапроса, "ЗаполнитьУпаковкуНекачественногоТовара");
...
Функция ТекстыЗапросовЗаполнитьУчетноеКоличествоНоменклатуры()
	...
	|	ВтИсточникДанных.Номенклатура КАК Номенклатура,
	|	ВтИсточникДанных.Упаковка КАК Упаковка,
	|	ЕСТЬNULL(РегистрСведенийАналитикаУчетаНоменклатуры.КлючАналитики,
	|		ЗНАЧЕНИЕ(Справочник.КлючиАналитикиУчетаНоменклатуры.ПустаяСсылка)) КАК КлючАналитики
	|ПОМЕСТИТЬ ТаблицаТоваровСАналитикойУчетаНоменклатуры
	|ИЗ
	|	ВтИсточникДанных КАК ВтИсточникДанных
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК РегистрСведенийАналитикаУчетаНоменклатуры
	|		ПО ВтИсточникДанных.Номенклатура = РегистрСведенийАналитикаУчетаНоменклатуры.Номенклатура
	|		И ВтИсточникДанных.Характеристика = РегистрСведенийАналитикаУчетаНоменклатуры.Характеристика
	|		И (РегистрСведенийАналитикаУчетаНоменклатуры.МестоХранения = &Склад)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	КлючАналитики";
	
	Результат.Добавить(ТекстЗапроса, "ТаблицаТоваровСАналитикойУчетаНоменклатуры");
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		ТаблицаТоваров.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|		ТаблицаТоваров.Номенклатура КАК Номенклатура,
	|		ТаблицаТоваров.Упаковка КАК Упаковка,
	|		ТоварыОрганизаций.КоличествоОстаток КАК КоличествоОстаток
	|	ИЗ
	|		ТаблицаТоваровСАналитикойУчетаНоменклатуры КАК ТаблицаТоваров
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыОрганизаций.Остатки(&МоментВремени, Организация = &Организация
	|			И АналитикаУчетаНоменклатуры В
	|				(ВЫБРАТЬ
	|					НаборКлючейАналитики.КлючАналитики КАК КлючАналитики
	|				ИЗ
	|					ТаблицаТоваровСАналитикойУчетаНоменклатуры КАК НаборКлючейАналитики)) КАК ТоварыОрганизаций
	|			ПО ТаблицаТоваров.КлючАналитики = ТоварыОрганизаций.АналитикаУчетаНоменклатуры
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	|		ТаблицаТоваров.ИдентификаторСтрокиВТ,
	|		ТаблицаТоваров.Номенклатура,
	|		ТаблицаТоваров.Упаковка,
	...
	|			ПО ТоварыОрганизаций.Регистратор = &Регистратор
	|			И ТаблицаТоваров.КлючАналитики = ТоварыОрганизаций.АналитикаУчетаНоменклатуры) КАК Подзапрос
	|СГРУППИРОВАТЬ ПО
...
// 	СписокЗначений Из Строка.
//
Функция ТекстыЗапросовПересчитатьКоличествоУпаковок(ПараметрыЗапроса = Неопределено)
	
	Если ПараметрыЗапроса = Неопределено Тогда
		
		ИмяПоляКоличество = "Количество";
		ИмяПоляКоличествоУпаковок = "КоличествоУпаковок";
		ИмяТаблицы = "ПересчитатьКоличествоУпаковок";
		
	Иначе
		
		ИмяПоляКоличество = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ПараметрыЗапроса,
			"ИмяПоляКоличество",
			"Количество");
		
		ИмяПоляКоличествоУпаковок = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ПараметрыЗапроса,
			"ИмяПоляКоличествоУпаковок",
			"КоличествоУпаковок");
		
		ИмяТаблицы = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ПараметрыЗапроса,
			"ИмяТаблицы",
			"ПересчитатьКоличествоУпаковок");
	...
	|	ВЫРАЗИТЬ(&ИмяПоляКоличество КАК ЧИСЛО) / ВЫБОР
...
Функция ТекстыЗапросовПересчитатьКоличествоУпаковокПоВесуОбъему(ИмяТаблицы, ИмяПоляКоличество, ИмяПоляКоличествоУпаковок, ВариантПересчета)
	...
	|	&ИмяПоляКоличествоУпаковок КАК ИмяПоляКоличествоУпаковок,
	|	ЕСТЬNULL(&ПолеКоличествоУпаковок, 0) * ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки, 1) КАК Количество,
	|	ЕСТЬNULL(&ПолеКоличествоУпаковок, 0) КАК КоличествоУпаковок,
	|	ЕСТЬNULL(&ТекстЗапросаКоэффициентЕдиницыИзмеренияВеса, 0) КАК КоэффициентВес,
	|	ЕСТЬNULL(&ТекстЗапросаКоэффициентЕдиницыИзмеренияОбъема, 0) КАК КоэффициентОбъем
	|ИЗ
	|	ВтИсточникДанных КАК ВтИсточникДанных";
	...
	Если ВариантПересчета = "Вес" Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			...
			"ВтИсточникДанных.Вес * ВЫРАЗИТЬ(&ТекстЗапросаКоэффициентЕдиницыИзмеренияВеса / ВтИсточникДанных.ВесУпаковки КАК ЧИСЛО(27, 11))");
		
	ИначеЕсли ВариантПересчета = "Объем" Тогда
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			"&ПолеКоличествоУпаковок",
			"ВтИсточникДанных.Объем * ВЫРАЗИТЬ(&ТекстЗапросаКоэффициентЕдиницыИзмеренияОбъема / ВтИсточникДанных.ОбъемУпаковки КАК ЧИСЛО(27, 11))");
...
// Возвращает список запросов для расчета статусов указания серий.
//
// Параметры:
// 	ИменаПолейСтатусУказанияСерий - Массив Из Строка - имена колонок статусов указания серий.
//
// Возвращаемое значение:
// 	СписокЗначений Из Строка.
//
Функция ТекстыЗапросовПроверитьСериюРассчитатьСтатус(ИменаПолейСтатусУказанияСерий)
	...
	|	ВтИсточникДанных.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|	ВЫБОР
	|		КОГДА (ТаблицаСерий.Ссылка ЕСТЬ NULL 
	|				ИЛИ ТаблицаНоменклатуры.Ссылка ЕСТЬ NULL)
	|		ТОГДА
	|			ЛОЖЬ
	|		ИНАЧЕ
	|		НЕ (ТаблицаСерий.ВидНоменклатуры.ВладелецСерий = ТаблицаНоменклатуры.ВидНоменклатуры.ВладелецСерий
	|			И ТаблицаСерий.ВидНоменклатуры.ВладелецСерий <> ЗНАЧЕНИЕ(Справочник.ВидыНоменклатуры.ПустаяСсылка)
	|			И ТаблицаНоменклатуры.ВидНоменклатуры.ВладелецСерий <> ЗНАЧЕНИЕ(Справочник.ВидыНоменклатуры.ПустаяСсылка)
	|				ИЛИ ТаблицаНоменклатуры.ВладелецСерий = ТаблицаСерий.ВидНоменклатуры
	|				ИЛИ ТаблицаНоменклатуры.ВидНоменклатуры = ТаблицаСерий.ВидНоменклатуры)
	|	КОНЕЦ КАК РазличаютсяВладельцыСерииИНоменклатуры
	|ИЗ
	|	ВтИсточникДанных КАК ВтИсточникДанных
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК ТаблицаНоменклатуры
	|			ПО ВтИсточникДанных.Номенклатура = ТаблицаНоменклатуры.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СерииНоменклатуры КАК ТаблицаСерий
	|			ПО ВтИсточникДанных.Серия = ТаблицаСерий.Ссылка";
	
	Результат.Добавить(ТекстЗапроса, "ПроверитьСерию");
	
	ПоляВыборкиСтатусов = Новый Массив;
	
	Если ИменаПолейСтатусУказанияСерий.Количество() = 0 Тогда
		ПоляВыборкиСтатусов.Добавить("ТаблицаСтатусыУказанияСерий.СтатусУказанияСерий");
	Иначе
		
		Для Каждого ИмяПоля Из ИменаПолейСтатусУказанияСерий Цикл
			ПоляВыборкиСтатусов.Добавить(СтрШаблон("ТаблицаСтатусыУказанияСерий.%1 КАК %1", ИмяПоля));
		КонецЦикла;
		
	КонецЕсли;
	
	ТекстПоляСтатусыУказанияСерий = СтрСоединить(ПоляВыборкиСтатусов, "," + Символы.ПС + Символы.Таб);
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаСтатусыУказанияСерий.НомерСтроки КАК НомерСтроки,
	|	&ТекстПоляСтатусыУказанияСерий
	|ПОМЕСТИТЬ ТаблицаСтатусыУказанияСерий
	|ИЗ
	|	&ТаблицаСтатусыУказанияСерий КАК ТаблицаСтатусыУказанияСерий";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ТекстПоляСтатусыУказанияСерий",
		ТекстПоляСтатусыУказанияСерий);
	
	Результат.Добавить(ТекстЗапроса, "ТаблицаСтатусыУказанияСерий");
	...
	|ИЗ
	|	ВтИсточникДанных КАК ВтИсточникДанных
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаСтатусыУказанияСерий КАК ТаблицаСтатусыУказанияСерий
	|		ПО ВтИсточникДанных.ИдентификаторСтрокиВТ = ТаблицаСтатусыУказанияСерий.НомерСтроки";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ТекстПоляСтатусыУказанияСерий",
		ТекстПоляСтатусыУказанияСерий);
	
	Результат.Добавить(ТекстЗапроса, "РассчитатьСтатус");
	
	Возврат Результат;
	
КонецФункции

// Возвращает список запросов для проверки серии по владельцу.
//
// Возвращаемое значение:
// 	СписокЗначений Из Строка.
//
Функция ТекстыЗапросовПроверитьСериюПоВладельцу()
	...
	|	ВтИсточникДанных.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|	ВЫБОР
	|		КОГДА ТаблицаНоменклатуры.ВидНоменклатуры.ИспользоватьСерии ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ТаблицаНоменклатуры.ВидНоменклатуры.ИспользоватьСерии
	...
	Результат.Добавить(ТекстЗапроса, "ПроверитьСериюПоВладельцу");
