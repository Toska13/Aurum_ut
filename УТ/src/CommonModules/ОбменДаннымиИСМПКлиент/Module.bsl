...
Процедура ПодготовитьСообщенияКПередаче(ДинамическийСписок, ПараметрыПередачи) Экспорт
	...
	ПараметрыОпределенияДоступности = ОбменДаннымиИСКлиентСервер.ПараметрыОпределенияДоступностиДействий();
	ПараметрыОпределенияДоступности.Подсистема = ОбщегоНазначенияИСМПКлиентСервер.ИмяПодсистемы();
	
	ОбменДаннымиИСКлиент.ОпределитьДоступностьДействий(
		Контекст, ИменаКолонокДальнейшиеДействия, ПараметрыОпределенияДоступности);
...
Процедура ОбработатьРезультатОбменаСлужебный(РезультатОбмена, ДополнительныеПараметрыДлительнойОперации)
	...
	Иначе
		...
		ИначеЕсли РезультатОбмена.МОДКПроверкеРегистрации <> Неопределено Тогда
			Контекст = Новый Структура;
			Контекст.Вставить("Форма",                                  Форма);
			Контекст.Вставить("Организация",                            Организация);
			Контекст.Вставить("ОповещениеПриЗавершении",                ОповещениеПриЗавершении);
			...
			ИмяПроцедурыОповещения = "ПродолжитьВыполнениеОбменаПослеУточненияПараметровРегистрацииМОД";
			
			//@skip-check notify-description-to-server-procedure
			СогласиеНаРегистрациюМОДЗавершение = Новый ОписаниеОповещения(ИмяПроцедурыОповещения,
				ОбщегоНазначенияКлиент.ОбщийМодуль("ИнтеграцияИСМПСлужебныйКлиент"),
				Контекст);
			
			МодульИнтеграцияИСМПКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ИнтеграцияИСМПКлиент");
			МодульИнтеграцияИСМПКлиент.ОткрытьФормуРегистрацииМестаОсуществленияДеятельностиГИСМТ(РезультатОбмена.МОДКПроверкеРегистрации, 
				ЭтотОбъект, СогласиеНаРегистрациюМОДЗавершение);
