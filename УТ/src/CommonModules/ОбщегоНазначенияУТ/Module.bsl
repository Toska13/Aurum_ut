...
Процедура ЗаписатьВЖурналСообщитьПользователю(ПараметрыЖурнала, УровеньЖурнала, ИмяСобытия, Знач Комментарий = "", ИнформацияОбОшибке = Неопределено) Экспорт
	...
	Если ТипЗнч(ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке") Тогда
		Если Комментарий = "" Тогда
			ТекстСообщенияПользователю = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
			Комментарий = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		Иначе
			ТекстСообщенияПользователю = Комментарий + Символы.ПС + ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
			Комментарий = Комментарий + Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		КонецЕсли;
	Иначе
		Если ТипЗнч(ИнформацияОбОшибке) = Тип("Строка")
			...
			КонецЕсли;
		КонецЕсли;
		ТекстСообщенияПользователю = Комментарий;
	КонецЕсли;
	...
	Если УровеньЖурнала = УровеньЖурналаРегистрации.Ошибка
		...
		ОбщегоНазначения.СообщитьПользователю(СокрЛП(ТекстСообщенияПользователю), ПараметрыЖурнала.Данные);
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(Объект, ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоРасчетныхСчетов")
		...
		Иначе
			...
			Объект.БанковскийСчет = ДенежныеСредстваСервер.ПолучитьБанковскийСчетОрганизацииПоУмолчанию( ,
...
// 	РеквизитыИсключения - Структура - Структура имен реквизитов (в т.ч. реквизиты табличных частей), изменение которых проверять не требуется.
// 	ТабличныеЧастиИсключения - Структура - Структура имен табличных частей, изменение которых проверять не требуется.
// 	ПроверитьПроведен - Булево - Истина, если требуется проверить изменение признака Проведен.
//
// Возвращаемое значение:
...
Функция ИзмененияДокумента(Объект, РеквизитыИсключения = Неопределено, ТабличныеЧастиИсключения = Неопределено, ПроверитьПроведен = Ложь) Экспорт
	...
	Если ПроверитьПроведен Тогда
		РеквизитыДокумента.Вставить("Проведен");
	КонецЕсли;
	...
	Для каждого ТабличнаяЧасть Из МетаданныеДокумента.ТабличныеЧасти Цикл
		...
		Для каждого Реквизит Из ТабличнаяЧасть.Реквизиты Цикл
			Если СтрНайти(Реквизит.Имя, "Удалить") = 1
				ИЛИ Реквизит.Тип = СтрокаНеограниченнойДлинны
				ИЛИ РеквизитыИсключения.Свойство(Реквизит.Имя) Тогда
				Продолжить;
			КонецЕсли;
	...
	Для каждого Реквизит Из РеквизитыДокумента Цикл
		...
		Если Выборка[Реквизит.Ключ] <> Объект[Реквизит.Ключ]
			ИЛИ ПроверитьПроведен
				И Реквизит.Ключ = "Проведен"
				И (Объект.ДополнительныеСвойства.ПроведениеДокументов.СвойстваДокумента.РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения
						И Выборка[Реквизит.Ключ] = Истина
					ИЛИ	Объект.ДополнительныеСвойства.ПроведениеДокументов.СвойстваДокумента.РежимЗаписи = РежимЗаписиДокумента.Проведение
						И Выборка[Реквизит.Ключ] = Ложь) Тогда
...
Функция ПроверитьЗаполнениеРеквизитовОбъекта(Объект, МассивПроверяемыхРеквизитов) Экспорт
	...
	Для Каждого ТабличнаяЧасть Из ТабличныеЧасти Цикл
		...
		МетаданныеТабличнойЧасти = МетаданныеОбъекта.ТабличныеЧасти[ИмяТабличнойЧасти];
		...
		Для Каждого СтрокаТабличнойЧасти Из ТабличнаяЧастьОбъекта Цикл
			...
			Для Каждого ИмяРеквизита Из МассивРеквизитов Цикл
				...
				Если Не ЗначениеЗаполнено(СтрокаТабличнойЧасти[ИмяРеквизита]) Тогда
					
					ТекстОшибки = СтрЗаменить(ШаблонОшибкиРеквизитаТЧ, "%ИмяРеквизита%", МетаданныеТабличнойЧасти.Реквизиты[ИмяРеквизита].Синоним);
					ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%", Формат(СтрокаТабличнойЧасти.НомерСтроки, "ЧГ=0"));
					ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ИмяТабличнойЧасти%", МетаданныеТабличнойЧасти.Синоним);
					
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						ТекстОшибки,
						Объект,
						ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(ИмяТабличнойЧасти, СтрокаТабличнойЧасти.НомерСтроки, ИмяРеквизита),
						,
						Отказ);
...
//	        - ДанныеФормыКоллекция - обрабатываемая таблица
//	Строки  - Массив Из СтрокаТаблицыЗначений - Массив удаляемых строк
//	        - Массив Из ДанныеФормыЭлементКоллекции - Массив удаляемых строк
//
Процедура УдалитьСтрокиТаблицы(Таблица, Строки) Экспорт
...
Процедура НайтиИУдалитьСтрокиТаблицыЗначений(Таблица, СтруктураПоиска, УдалятьПопавшиеВОтбор = Истина) Экспорт
	...
	Если НЕ УдалятьПопавшиеВОтбор Тогда
		...
		Иначе
			...
			УдалитьСтрокиТаблицы(Таблица, УдаляемыеСтроки);
	...
	Иначе
		...
		ИначеЕсли Строки.Количество() = Таблица.Количество() Тогда
			Таблица.Очистить();
		Иначе
			УдалитьСтрокиТаблицы(Таблица, Строки);
		КонецЕсли;
...
//                               "РегистрыНакопления.РасчетыСКлиентамиПоСрокам.Измерения.ОбъектРасчетов".
//
// Возвращаемое значение:
//  ОбъектМетаданных - 
//
...
// Возвращает описание типа количественного поля.
//
// Параметры:
//  ЗнакПоля - ДопустимыйЗнак - допустимый знак количественного поля (по умолчанию Любой)
//
// Возвращаемое значение:
//  ОписаниеТипов - Описание типа количественного поля
//
...
//
Процедура УстановитьПодпискуНаСобытияИзмененияЭлементовФормы(Форма, ИменаЭлементов) Экспорт
...
Функция ДоступныеХозяйственныеОперацииИДокументы(ТаблицаЗначений, ОтборХозяйственныеОперации, ОтборТипыДокументов, КлючНазначенияИспользования) Экспорт
	...
	ДоступностьДокументаПоФО = Новый Соответствие();
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			ДокументДоступенПоФО = ДоступностьДокументаПоФО.Получить(ОбъектМетаданных);
				
			Если ДокументДоступенПоФО = Неопределено Тогда
				ДокументДоступенПоФО = ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных);
				ДоступностьДокументаПоФО.Вставить(ОбъектМетаданных, ДокументДоступенПоФО);
			КонецЕсли;
			
			Если ДокументДоступенПоФО Тогда
...
//  ЕстьОсновнаяТаблица - Булево - Признак, что у динамического списка установлена основная таблица
// 
// Возвращаемое значение:
//  Неопределено, Структура -
//
Функция ДанныеПоШтрихКодуПечатнойФормы(Штрихкод, ХозяйственныеОперацииИДокументы, ЕстьОсновнаяТаблица = Истина) Экспорт 
	...
	Если Ссылки.Количество() > 0 Тогда
		...
		Если ЕстьОсновнаяТаблица Тогда
			ТекстЗапроса =
			"ВЫБРАТЬ
		...
		Если Не Результат.Пустой() Тогда
			...
			Иначе
				ДанныеПоШтрихКоду.КлючЗаписи = Новый КлючСтрокиДинамическогоСписка(СтруктураКлючевыхПолей);
			КонецЕсли;
		КонецЕсли;
...
//
// Параметры:
//  Отбор               - Соответствие из Структура - коллекция настроек отборов
...
Процедура ДобавитьЭлементОтбораВКоллекцию(Отбор, ЛевоеЗначение, ПравоеЗначение, ВидСравненияЗапроса = "") Экспорт
	...
	ОбщегоНазначенияУТКлиентСервер.ДобавитьЭлементОтбораВКоллекцию(Отбор, ЛевоеЗначение, ПравоеЗначение, ВидСравненияЗапроса);
...
// Формирует текст запроса получения данных из таблицы значений. При этом нетипизированные колонки будут исключены из обработки.
// 
// Параметры:
// 	Таблица - ТаблицаЗначений - исходная таблица значений
// 	ИмяВременнойТаблицы - Строка - имя формируемой временной таблицы
// 	ПоляИндекса - Строка, Массив - перечень полей, по которым будет построен индекс временной таблицы
// Возвращаемое значение:
// 	Строка - текст запроса получения данных таблицы значений
//
Функция ТекстЗапросаДанныеТаблицыЗначений(Таблица, ИмяВременнойТаблицы, ПоляИндекса = "") Экспорт
	...
	ШаблонПоля = "Таблица.%1 КАК %1";
	Для каждого Колонка Из Таблица.Колонки Цикл
		Если ЗначениеЗаполнено(Колонка.ТипЗначения) Тогда
			ПоляВыборки.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПоля, Колонка.Имя));
		КонецЕсли;
	КонецЦикла;
	...
	Если ТипЗнч(ПоляИндекса) = Тип("Массив") Тогда
		ПоляИндекса = СтрСоединить(ПоляИндекса, "," + Символы.ПС + Символы.Таб);
	КонецЕсли;
...
// Оформляет начало строки в соответствии с платформенным поиском
// 
// Параметры:
//  ИсходнаяСтрока - Строка - Которую необходимо оформить
//  СтрокаПоиска - Строка - Определяет количество символов, которые необходимо оформить. 
//  	Если начало исходной строки без учета регистра не содержит строку поиска, то оформление не выполняется.
// 
// Возвращаемое значение:
//  ФорматированнаяСтрока - Оформленная строка
Функция СтрВыделитьОформлениемНачалоСтроки(ИсходнаяСтрока, СтрокаПоиска) Экспорт
	
	ЧислоСимволов = СтрДлина(СтрокаПоиска);
	ОформляемаяЧасть = Лев(ИсходнаяСтрока, ЧислоСимволов);
	
	Если ВРег(ОформляемаяЧасть) <> ВРег(СтрокаПоиска) Тогда
		
		Возврат Новый ФорматированнаяСтрока(ИсходнаяСтрока);
		
	КонецЕсли;
	
	ОформляемаяЧасть = Новый ФорматированнаяСтрока(ОформляемаяЧасть, ШрифтыСтиля.ЖирныйШрифт, ЦветаСтиля.ЦветАкцента);
	НеоформляемаяЧасть = Сред(ИсходнаяСтрока, ЧислоСимволов + 1);
	
	ОформленнаяСтрока = Новый ФорматированнаяСтрока(ОформляемаяЧасть, НеоформляемаяЧасть);
	
	Возврат ОформленнаяСтрока;
...
//  Коэффициенты        - Массив из Число - коэффициенты распределения, могут быть положительны или отрицательными
//  Точность            - Число  - точность округления при распределении. Необязателен.
//
// Возвращаемое значение:
//  Массив из Число - массив размерностью равный массиву коэффициентов, содержит
//           суммы в соответствии с весом коэффициента (из массива коэффициентов).
//           В случае, если распределить невозможно,
...
//	Результат = ОбщегоНазначенияУТ.РаспределитьСуммуПропорциональноКоэффициентамРазныхЗнаков(1, Коэффициенты); // Результат = [-1, 2].
//
Функция РаспределитьСуммуПропорциональноКоэффициентамРазныхЗнаков(Знач РаспределяемаяСумма, Знач Коэффициенты, Знач Точность = 2) Экспорт
	...
	Для Индекс = 0 По Коэффициенты.Количество() - 1 Цикл
		...
		Если МаксимальныйКоэффициент < Коэффициент
			Или МаксимальныйКоэффициент = 0 Тогда
	...
	Если СуммаКоэффициентов = 0 Тогда
		// Недопустимо значение параметра Коэффициенты
		// Ожидалось: общая сумма коэффициентов не нулевая
		Возврат Неопределено;
	КонецЕсли;
...
// Устарела. Следует использовать ОбщегоНазначенияУТ.УдалитьСтрокиТаблицы
...
// Возвращает наименования основных печатных форм или электронных документов, которые были сформированы по документам
//
// Параметры:
//  Ссылки - ЛюбаяСсылка
...
//
// Возвращаемое значение:
//  Соответствие из КлючИЗначение - Соответствие документов и наименований для печати:
...
//
Функция НаименованияЭлектронныхДокументовИлиОсновныхПечатныхФорм(Знач Ссылки) Экспорт
	
	ТипПараметра = ТипЗнч(Ссылки);
	Если ТипПараметра <> Тип("Массив") И ТипПараметра <> Тип("ФиксированныйМассив") Тогда
		Ссылки = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Ссылки);
	КонецЕсли;
	
	ПредставленияДокументов = Новый Соответствие;
	Для Каждого Ссылка Из Ссылки Цикл
		ПредставленияДокументов.Вставить(Ссылка, Неопределено);
	КонецЦикла;
	
	Если ПолучитьФункциональнуюОпцию("ВыводитьНаименованияИсходящихДокументовВАктахСверкиИСчетахФактурах") Тогда
		ОбщегоНазначенияЛокализация.ПриПолученииНаименованийНапечатанныхОсновныхПечатныхФорм(Ссылки, ПредставленияДокументов);
		
		СсылкиДляПоиска = Новый Массив;
		Для Каждого Элемент Из ПредставленияДокументов Цикл
			Если Не ЗначениеЗаполнено(Элемент.Значение) Тогда
				СсылкиДляПоиска.Добавить(Элемент.Ключ);
			КонецЕсли;
		КонецЦикла;
		
		СформированныеПечатныеФормы = УправлениеПечатью.НаименованияСформированныхОсновныхПечатныхФорм(СсылкиДляПоиска);
		Для Каждого Ссылка Из СсылкиДляПоиска Цикл
			ПредставленияДокументов[Ссылка] = СформированныеПечатныеФормы[Ссылка];
		КонецЦикла;
	КонецЕсли;
...
#Область ПереопределяемыеФункцииОбщегоНазначения
...
Процедура ПриОпределенииПодчиненныхОбъектов(ПодчиненныеОбъекты) Экспорт
	...
	ПодчиненныйОбъект.ПодчиненныйОбъект = Метаданные.Справочники.КлючиАналитикиУчетаНоменклатуры;
	ПодчиненныйОбъект.ПоляСвязей = Справочники.КлючиАналитикиУчетаНоменклатуры.КлючевыеРеквизиты();
	ПодчиненныйОбъект.ПриПоискеЗаменыСсылок = "Справочники.КлючиАналитикиУчетаНоменклатуры";
	ПодчиненныйОбъект.ВыполнятьАвтоматическийПоискЗаменСсылок = Истина;
	
	ПодчиненныйОбъект = ПодчиненныеОбъекты.Добавить();
	ПодчиненныйОбъект.ПодчиненныйОбъект = Метаданные.Справочники.КлючиАналитикиУчетаНаборов;
	ПодчиненныйОбъект.ПоляСвязей = Справочники.КлючиАналитикиУчетаНаборов.КлючевыеРеквизиты();
	ПодчиненныйОбъект.ПриПоискеЗаменыСсылок = "Справочники.КлючиАналитикиУчетаНаборов";
	ПодчиненныйОбъект.ВыполнятьАвтоматическийПоискЗаменСсылок = Истина;
	
	ПодчиненныйОбъект = ПодчиненныеОбъекты.Добавить();
	ПодчиненныйОбъект.ПодчиненныйОбъект = Метаданные.Справочники.КлючиАналитикиУчетаПоПартнерам;
	ПодчиненныйОбъект.ПоляСвязей = Справочники.КлючиАналитикиУчетаПоПартнерам.КлючевыеРеквизиты();
	ПодчиненныйОбъект.ПриПоискеЗаменыСсылок = "Справочники.КлючиАналитикиУчетаПоПартнерам";
	ПодчиненныйОбъект.ВыполнятьАвтоматическийПоискЗаменСсылок = Истина;
	
	ПодчиненныйОбъект = ПодчиненныеОбъекты.Добавить();
	ПодчиненныйОбъект.ПодчиненныйОбъект = Метаданные.Справочники.Назначения;
	ПодчиненныйОбъект.ПоляСвязей = Справочники.Назначения.КлючевыеРеквизиты();
	ПодчиненныйОбъект.ПриПоискеЗаменыСсылок = "Справочники.Назначения";
	ПодчиненныйОбъект.ВыполнятьАвтоматическийПоискЗаменСсылок = Истина;
	
	ПодчиненныйОбъект = ПодчиненныеОбъекты.Добавить();
	ПодчиненныйОбъект.ПодчиненныйОбъект = Метаданные.Справочники.ОбъектыРасчетов;
	ПодчиненныйОбъект.ПоляСвязей = Справочники.ОбъектыРасчетов.КлючевыеРеквизиты();
	ПодчиненныйОбъект.ПриПоискеЗаменыСсылок = "Справочники.ОбъектыРасчетов";
	ПодчиненныйОбъект.ВыполнятьАвтоматическийПоискЗаменСсылок = Истина;
	
	ПодчиненныйОбъект = ПодчиненныеОбъекты.Добавить();
	ПодчиненныйОбъект.ПодчиненныйОбъект = Метаданные.Справочники.КлючиРеестраДокументов;
	ПодчиненныйОбъект.ПоляСвязей = Справочники.КлючиРеестраДокументов.КлючевыеРеквизиты();
	ПодчиненныйОбъект.ПриПоискеЗаменыСсылок = "Справочники.КлючиРеестраДокументов";
	ПодчиненныйОбъект.ВыполнятьАвтоматическийПоискЗаменСсылок = Истина;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов") Тогда
		ПодчиненныйОбъект = ПодчиненныеОбъекты.Добавить();
		ПодчиненныйОбъект.ПодчиненныйОбъект = Метаданные.Справочники.Контрагенты;
		ПодчиненныйОбъект.ПоляСвязей = "Партнер";
		ПодчиненныйОбъект.ВыполнятьАвтоматическийПоискЗаменСсылок = Истина;
	КонецЕсли;
...
// см. ПоискИЗаменаДублейНСИПереопределяемый.ПриПоискеДублейНСИ
Процедура ПриПоискеДублейНСИ(ПарыДублей, Правило) Экспорт
	
	ИдентификаторКонтрагенты = ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Справочник.Контрагенты");
	ОбъектМетаданныхПравила = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Правило, "ОбъектМетаданных");
	
	Если ОбъектМетаданныхПравила = ИдентификаторКонтрагенты
		...
		|	ТаблицаДублей.Ссылка1 КАК Ссылка1
		|ПОМЕСТИТЬ ВТ
		|ИЗ
		...
		|	Контрагенты1.Партнер КАК Ссылка1
		|ИЗ
		|	ВТ КАК ВТ
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Контрагенты
		|		ПО ВТ.Ссылка = Контрагенты.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Контрагенты1
		|		ПО ВТ.Ссылка1 = Контрагенты1.Ссылка";
		Запрос.УстановитьПараметр("ТаблицаДублей", ПарыДублей);
...
// см. ПоискИЗаменаДублейНСИПереопределяемый.ПроверитьВозможностьЗаменыДублей
Процедура ПроверитьВозможностьЗаменыДублей(ПарыЗамен, НевозможныеЗамены, Тип) Экспорт
	
	ТипыОбъектаАвторизации = Метаданные.ОпределяемыеТипы.ВнешнийПользователь.Тип;
	Если ТипыОбъектаАвторизации.СодержитТип(Тип) Тогда
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	ОбъектАвторизации В (&Отбор)";
		
		Отбор = Новый Массив;
		Для Каждого КлючИЗначение Из ПарыЗамен Цикл
			Отбор.Добавить(КлючИЗначение.Ключ);
		КонецЦикла;
		...
		Выборка = РезультатЗапроса.Выбрать();
		Пока Выборка.Следующий() Цикл
			ОписаниеОшибки = НевозможныеЗамены.Добавить();
			ОписаниеОшибки.ОбъектОшибки = Выборка.ВнешнийПользователь;
			ОписаниеОшибки.Ссылка = Выборка.ОбъектАвторизации;
			ТекстОшибки = СтрШаблон(
				НСтр("ru='Партнер %1 связан с внешним пользователем. Перед заменой партнера необходимо удалить внешнего пользователя.'"),
				Выборка.ОбъектАвторизации);
...
Процедура ДоступныеДляВыбораЗначенияПеречисления(ИмяПеречисления, ДанныеВыбора, Параметры = Неопределено, ИсключаемыеЗначения = Неопределено) Экспорт
	...
	СпособПоискаСтроки = СпособПоискаСтрокиПриВводеПоСтроке.ЛюбаяЧасть;
	Если Параметры.Свойство("СпособПоискаСтрокиПриВводеПоСтроке") Тогда
		СпособПоискаСтроки = Параметры.СпособПоискаСтрокиПриВводеПоСтроке;
	КонецЕсли;
	...
	ЕстьОтборПоСтроке = ЗначениеЗаполнено(Параметры.СтрокаПоиска);
	СтрокаПоиска = ?(ЕстьОтборПоСтроке, НРег(Параметры.СтрокаПоиска), "");
	
	Пока Выборка.Следующий() Цикл
			
		Если ЕстьОтборПоСтроке Тогда
			
			ПредставлениеДляПоиска = НРег(Выборка.Представление);
			
			Если СпособПоискаСтроки = СпособПоискаСтрокиПриВводеПоСтроке.Начало
				И СтрНачинаетсяС(ПредставлениеДляПоиска, СтрокаПоиска) Тогда
				
				Представление = СтрНайтиИВыделитьОформлением(
					Выборка.Представление,
					СтрокаПоиска);
					
			ИначеЕсли СпособПоискаСтроки = СпособПоискаСтрокиПриВводеПоСтроке.ЛюбаяЧасть
				И СтрНайти(ПредставлениеДляПоиска, СтрокаПоиска) > 0 Тогда
				
				Представление = Выборка.Представление;	
					
			Иначе // Не пройден отбор по строке
...
Функция ПолучитьУникальныйИдентификаторПлатежаСКонтрольнымРазрядом(Код,Сдвиг = Неопределено)
	...
	СтрокаБукв = "АБВГДЕЖЗИКЛМНОПРСТУФХЦЧШЩЭЮЯЪЫЬABCDEFGHIJKLMNOPQRSTUVWXYZ"; //@NON-NLS
	НовКод = "";
...
// Выполняет проверку является ли переданный тип помеченным к удалению в будущих релизах.
//
// Параметры:
// 	ТипОбъекта - Тип
//
...
//  Булево -
//
Функция ОбъектКУдалению(ТипОбъекта) Экспорт
	
	ОбъектМетаданных = Метаданные.НайтиПоТипу(ТипОбъекта);
	Если ОбъектМетаданных = Неопределено Тогда
		Возврат Ложь;
	Иначе 
		Возврат СтрНачинаетсяС(ОбъектМетаданных.Имя, "Удалить");    
	КонецЕсли;
КонецФункции
...
Функция СформироватьГиперссылкуКОформлению(ХозяйственныеОперацииИДокументы, ПараметрыФормирования, Заголовок = "") Экспорт
	...
	Для Каждого МенеджерРасчетаГиперссылок Из МенеджерыРасчетаГиперссылок Цикл
		...
		Если ТипЗнч(Гиперссылка) <> Тип("Массив") Тогда 
			Гиперссылка = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Гиперссылка);
		КонецЕсли;
		
		Для Каждого ТекущаяГиперссылка Из Гиперссылка Цикл 
			Если ЗначениеЗаполнено(ТекущаяГиперссылка) Тогда
				Если МассивСтрок.Количество()>1 Тогда
					МассивСтрок.Добавить("; ");
				КонецЕсли;
				МассивСтрок.Добавить(ТекущаяГиперссылка);
			КонецЕсли;
		КонецЦикла;
...
Функция ХозяйственныеОперацииДокументовОтраженныеВРеестре(ТипыДокументов) Экспорт
	...
	Для Каждого СтрМас Из ТипыДокументов Цикл
		Если ОбщегоНазначения.ЭтоСсылка(СтрМас) Тогда
			...
			Если ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
...
// Заменяет источник поля Ссылка и поля СторнируемыйДокумент в тексте запроса динамического списка, построенного на регистре сведений Реестр документов
...
Процедура ЗаменитьПолеСсылкаКонструкциейВыразитьПоТипамДокументов(ЭлементФормыСписок, ТипыДокументов = Неопределено, ПсевдонимРеестраДокументов = Неопределено) Экспорт
	...
	ВыбранныеТипыДокументов = Новый Массив;
	Если ТипыДокументов = Неопределено Тогда
		ПолныеИменаДокументов = Новый Массив;
		ОтборСписка = ДинамическийСписок.Отбор;// ОтборКомпоновкиДанных 
		Для Каждого ЭлементОтбора Из ОтборСписка.Элементы Цикл
			Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных") И ЭлементОтбора.Использование Тогда
				Если ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТипСсылки") Тогда
					Для Каждого Элемент Из ЭлементОтбора.ПравоеЗначение Цикл
						ВыбранныеТипыДокументов.Добавить(Новый Структура("ТипДокумента, ПолноеИмяДокумента", ТипЗнч(Элемент.Значение.ЗначениеПустойСсылки), Элемент.Значение.ПолноеИмя));
						ПолныеИменаДокументов.Добавить(Элемент.Значение.ПолноеИмя);
					КонецЦикла;
					Прервать;
				КонецЕсли;
			КонецЕсли;
	...
	РазыменовываниеДобавлено 		= ДополнительныеСвойства.Свойство("РазыменовываниеСсылки")
										И Не СтрНайти(ДинамическийСписок.ТекстЗапроса, ПсевдонимРеестраДокументов + ".Ссылка КАК Ссылка,");
	...
	Если ПолныеИменаДокументов.Количество() Тогда
		ТипыДокументовРазыменовывание 	= СтрСоединить(РазыменовываниеВыразить, Символы.ПС);
		Разыменовывание 				= "(ВЫБОР " + ТипыДокументовРазыменовывание + " КОНЕЦ)"; 
		Разыменовывание 				= СтрЗаменить(Разыменовывание,
											"&ПолеСсылка", 
	...
	// Разыменование поля СторнируемыйДокумент       
	ТипыДокументовИсправлений = Метаданные.ОпределяемыеТипы.ИсправляемыеДокументы.Тип.Типы();  
	Если Не ВыбранныеТипыДокументов.Количество() Тогда
		Если ТипЗнч(ТипыДокументов) = Тип("ДанныеФормыКоллекция") Тогда
			ВыбранныеТипыДокументов = ВыделенныеХозяйственныеОперацииИДокументы(ТипыДокументов.Выгрузить());
			Если ВыбранныеТипыДокументов.Колонки.Найти("ТипДокумента") = Неопределено Тогда
				ВыбранныеТипыДокументов.Колонки.Добавить("ТипДокумента");
				Для Каждого Строка Из ВыбранныеТипыДокументов Цикл
					ТипДокумента = Тип(СтрЗаменить(Строка.ПолноеИмяДокумента,"Документ","ДокументСсылка"));
					Строка.ТипДокумента = ТипДокумента;
				КонецЦикла;
			КонецЕсли;
			ВыбранныеТипыДокументов.Свернуть("ТипДокумента, ПолноеИмяДокумента");
		ИначеЕсли ТипЗнч(ТипыДокументов) = Тип("Строка")  Тогда
			ВыбранныеТипыДокументов = Новый Массив;      
			ТипДокумента = Тип(СтрЗаменить(ТипыДокументов,"Документ.","ДокументСсылка."));
			ВыбранныеТипыДокументов.Добавить(Новый Структура("ТипДокумента, ПолноеИмяДокумента", ТипДокумента, ТипыДокументов))
		Иначе
			ВыбранныеТипыДокументов = Новый Массив;
			Если Не ТипыДокументов = Неопределено Тогда
				Для Каждого Элемент Из ТипыДокументов Цикл
					ТипДокумента = Тип(СтрЗаменить(Элемент,"Документ.","ДокументСсылка."));
					ВыбранныеТипыДокументов.Добавить(Новый Структура("ТипДокумента, ПолноеИмяДокумента", ТипДокумента, Элемент))
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	ПолныеИменаДокументовИсправлений = Новый Массив;
	Для Каждого Строка Из ВыбранныеТипыДокументов Цикл
		Если ТипыДокументовИсправлений.Найти(Строка.ТипДокумента) <> Неопределено Тогда 
			ПолныеИменаДокументовИсправлений.Добавить(Строка.ПолноеИмяДокумента);
		КонецЕсли;
	КонецЦикла;
	
	Если Не ПолныеИменаДокументовИсправлений.Количество() Тогда    
		ПолноеИмяДокумента = Метаданные.НайтиПоТипу(ТипыДокументовИсправлений[0]).ПолноеИмя();
		ПолныеИменаДокументовИсправлений.Добавить(ПолноеИмяДокумента);
	КонецЕсли;        
	
	РазыменовываниеДобавлено 		= ДополнительныеСвойства.Свойство("РазыменовываниеИсправляемыхДокументов")
										И Не СтрНайти(ДинамическийСписок.ТекстЗапроса, "ИНАЧЕ " + ПсевдонимРеестраДокументов + ".СторнируемыйДокумент");
	БлокЗамены 						= ?(РазыменовываниеДобавлено, 
										ДополнительныеСвойства.РазыменовываниеИсправляемыхДокументов, ПсевдонимРеестраДокументов + ".СторнируемыйДокумент");

	РазыменовываниеВыразить 		= Новый Массив();
	
	Для каждого ТипДокументов Из ПолныеИменаДокументовИсправлений Цикл
		РазыменовываниеВыразить.Добавить(СтрЗаменить(ШаблонКонструкции, "ДокументСсылка.ЗаказКлиента", ТипДокументов) + Символы.ПС
			+ " ТОГДА ВЫРАЗИТЬ(&ПолеСсылка КАК " + ТипДокументов + ") "); // @query-part-1
	КонецЦикла;
	...
	Разыменовывание					 = СтрЗаменить(Разыменовывание,
										"&ПолеСсылка", 
										ПсевдонимРеестраДокументов + ".СторнируемыйДокумент");

    СвойстваСписка.ТекстЗапроса		= СтрЗаменить(СвойстваСписка.ТекстЗапроса,
										"ИНАЧЕ " + БлокЗамены,
										"ИНАЧЕ " + Разыменовывание);
	...
	ДополнительныеСвойства.Вставить("РазыменовываниеИсправляемыхДокументов", 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Разыменовывание, ПсевдонимРеестраДокументов));
КонецПроцедуры

// Проверяет доступность ссылочного объекта матаданных по установленным значениям функциональных опций.
// 
// Параметры:
//  ОбъектМетаданных - ОбъектМетаданных - ссылочный объект метаданных, например Метаданные.Справочники.Валюты
// 
// Возвращаемое значение:
...
Функция ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных)
	...
	ПустаяСсылкаОбъекта = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ПолноеИмя).ПустаяСсылка();
	
	СхемаКомпоновкиДанных = Новый СхемаКомпоновкиДанных;

	ИсточникДанных = СхемаКомпоновкиДанных.ИсточникиДанных.Добавить();
	ИсточникДанных.Имя = "ТестовыйЗапрос";
	ИсточникДанных.ТипИсточникаДанных = "Local";
	
	НаборДанныхОбъектСхемыКомпоновкиДанных = СхемаКомпоновкиДанных.НаборыДанных.Добавить(
		Тип("НаборДанныхЗапросСхемыКомпоновкиДанных"));
	НаборДанныхОбъектСхемыКомпоновкиДанных.Имя = "НаборДанных";
	НаборДанныхОбъектСхемыКомпоновкиДанных.ИсточникДанных = "ТестовыйЗапрос";
	
	НаборДанныхОбъектСхемыКомпоновкиДанных.Запрос = "ВЫБРАТЬ ИСТИНА КАК Поле";
	
	Поле = НаборДанныхОбъектСхемыКомпоновкиДанных.Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
	Поле.ПутьКДанным = "Поле";
	
	Параметр = СхемаКомпоновкиДанных.Параметры.Добавить();
	Параметр.Имя = ПолноеИмя;
	Параметр.ТипЗначения = Новый ОписаниеТипов(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТипЗнч(ПустаяСсылкаОбъекта)));
	
	ОсновнойВариант = СхемаКомпоновкиДанных.ВариантыНастроек[0].Настройки; // НастройкиКомпоновкиДанных.

	ПользовательскийПараметр = ОсновнойВариант.ПараметрыДанных.Элементы.Добавить();
	ПользовательскийПараметр.ИдентификаторПользовательскойНастройки = Новый УникальныйИдентификатор();
	ПользовательскийПараметр.Параметр = Новый ПараметрКомпоновкиДанных(ПолноеИмя);
	
	КомпоновщикНастроек = Новый КомпоновщикНастроекКомпоновкиДанных;
	КомпоновщикНастроек.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(СхемаКомпоновкиДанных));
	КомпоновщикНастроек.ЗагрузитьНастройки(ОсновнойВариант);
	
	Возврат КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Количество() <> 0;
