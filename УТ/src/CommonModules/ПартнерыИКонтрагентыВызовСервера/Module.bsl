...
Функция МассивУстановленныхПараметровОтбораПоТипуПартнера(Параметры) Экспорт
	...
	КонецЕсли;
	Если Параметры.Отбор.Свойство("Перевозчик") Тогда
		МассивУстановленныхОтборов.Добавить("Перевозчик");
	КонецЕсли;
...
Функция КонтрагентыДанныеВыбора(Параметры) Экспорт
	...
	УсловияПоиска = "Контрагенты.Наименование ПОДОБНО &СтрокаВвода СПЕЦСИМВОЛ ""~""";
	...
	Для НомерДопЯзыка = 1 По МультиязычностьСервер.КоличествоДополнительныхЯзыков() Цикл
		...
		Если МультиязычностьСервер.ИспользуетсяДополнительныйЯзык(НомерДопЯзыка) Тогда
			...
			|	ИЛИ " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Контрагенты.%1 ПОДОБНО &СтрокаВвода СПЕЦСИМВОЛ ""~""",
...
Функция ПартнерыДанныеВыбора(Параметры) Экспорт
	...
	Если МассивУстановленныхОтборов.Количество() > 0 ИЛИ ИспользоватьПартнеровКакКонтрагентов Тогда
		...
		Если ИспользоватьПартнеровКакКонтрагентов Тогда
			...
			ШаблонПоиска = "Контрагенты.Наименование ПОДОБНО &СтрокаВвода СПЕЦСИМВОЛ ""~""";
		...
		Иначе
			...
			ШаблонПоиска = "Партнеры.Наименование ПОДОБНО &СтрокаВвода СПЕЦСИМВОЛ ""~""";
		...
		Для НомерДопЯзыка = 1 По МультиязычностьСервер.КоличествоДополнительныхЯзыков() Цикл
			...
			Если МультиязычностьСервер.ИспользуетсяДополнительныйЯзык(НомерДопЯзыка) Тогда
				...
				ИменаРеквизитов = МультиязычностьСервер.ИменаРеквизитовСУчетомКодаЯзыка("Наименование", Константы[ИмяКонстантыЯзыка].Получить());
				УсловияПоиска = УсловияПоиска + "
				|	ИЛИ " + СтрЗаменить(ШаблонПоиска, "Наименование", ИменаРеквизитов["Наименование"]);
				
			КонецЕсли;
			
		КонецЦикла;
		
		УсловияПоиска = СтрШаблон("(%1)", УсловияПоиска);
