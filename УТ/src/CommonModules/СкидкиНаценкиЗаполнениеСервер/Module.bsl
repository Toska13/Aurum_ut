...
Процедура НазначитьРучнуюСкидку(Объект, ИмяТЧ, СуммаСкидкиКРаспределению, ВходныеПараметры) Экспорт
	...
	//++ Локализация
	ЕстьСтрокиГдеРучнаяСкидкаНеПрименилась = Ложь;
	РозничныеПродажиТабачнойПродукцииТолькоПоМРЦ = 
		РозничныеПродажиКлиентСерверЛокализация.РозничныеПродажиТабачнойПродукцииТолькоПоМРЦ(ВходныеПараметры);;
	//-- Локализация
	Для каждого СтрокаТЧ Из СтрокиТабличнойЧасти Цикл
		
		//++ Локализация
		Если РозничныеПродажиТабачнойПродукцииТолькоПоМРЦ
			И РозничныеПродажиКлиентСерверЛокализация.ЭтоВидПродукцииСМРЦ(СтрокаТЧ["ВидПродукцииИС"]) Тогда
			
			ЕстьСтрокиГдеРучнаяСкидкаНеПрименилась = Истина;
			Продолжить;
	...
	КонецЦикла;
	//++ Локализация
	Если ЕстьСтрокиГдеРучнаяСкидкаНеПрименилась Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Есть строки, для которых ручная скидка/наценка недопустима'"));
	КонецЕсли;
	//-- Локализация
...
Функция АдресДанныхДляРасчетаРучныхСкидокВоВременномХранилище(
	...
	Если ТолькоДляВыделенныхСтрок Тогда
		...
		Для Каждого ТекСтрока Из Форма.Элементы[?(ИмяТаблицыВыделенныхСтрок = Неопределено, ИмяТаблицы, ИмяТаблицыВыделенныхСтрок)].ВыделенныеСтроки Цикл
			...
			//++ Локализация
			Если РозничныеПродажиЛокализация.ВСтрокеВидПродукцииСМРЦ(Форма, СтрокаКоллекции) Тогда
				Продолжить;
			КонецЕсли;
			//-- Локализация
	...
	Иначе
		Если РеализацияСверхЗаказа Тогда
			...
			Для Каждого СтрокаТЧ Из НайденныеСтроки Цикл
				//++ Локализация
				Если РозничныеПродажиЛокализация.ВСтрокеВидПродукцииСМРЦ(Форма, СтрокаТЧ) Тогда
					Продолжить;
				КонецЕсли;
				//-- Локализация
				НоваяСтрока = Товары.Добавить();
...
Функция СтруктураСообщений(Объект, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ЕстьСкидкиНаценки = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "СкидкиНаценки");
	Если ЕстьСкидкиНаценки Тогда
	...
	Если ТипЗнч(Объект.Ссылка) <> Тип("ДокументСсылка.ЧекККМ") 
		И ТипЗнч(Объект.Ссылка) <> Тип("ДокументСсылка.ЧекККМВозврат") Тогда
	...
	НеВыводитьСообщенияПоСоглашению = (ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЧекККМ"))
	                                   Или (ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЧекККМВозврат"))
