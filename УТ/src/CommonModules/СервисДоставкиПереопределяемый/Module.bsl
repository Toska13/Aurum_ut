...
Процедура ЗаполнитьПараметрыЗаказаНаДоставку(Параметры) Экспорт
	...
	Иначе
		...
		Если Не РезультатПоТоварнымПозициям.Пустой() Тогда
			...
			Если ДанныеПоУмолчанию.ВыгружатьКодыМаркировки Тогда
				ВыборкаКодовМаркировки = Результаты[9].Выбрать();
				ИспользованныеКодыМаркировки = Новый Соответствие;
			...
			Пока ВыборкаПоПозиции.Следующий() Цикл
				...
				Если ДанныеПоУмолчанию.ВыгружатьКодыМаркировки Тогда
					...
					Если ИспользованныеКодыМаркировки.Получить(ВыборкаПоПозиции.Номенклатура) = Неопределено Тогда
						ИспользованныеКодыМаркировки.Вставить(ВыборкаПоПозиции.Номенклатура, Новый Массив);
					КонецЕсли;
					
					ИспользованныеКодыМаркировкиНоменклатуры = ИспользованныеКодыМаркировки.Получить(ВыборкаПоПозиции.Номенклатура);
					СтруктураПоиска.Номенклатура = ВыборкаПоПозиции.Номенклатура;
					
					Пока ВыборкаКодовМаркировки.НайтиСледующий(СтруктураПоиска) Цикл
						
						ЗначениеШтрихкода = ВыборкаКодовМаркировки.ЗначениеШтрихкода;
						Если ИспользованныеКодыМаркировкиНоменклатуры.Найти(ЗначениеШтрихкода) = Неопределено Тогда
							НоваяСтрокаПозиции.КодМаркировки = ЗначениеШтрихкода;
							ИспользованныеКодыМаркировкиНоменклатуры.Добавить(ЗначениеШтрихкода);
							Прервать;
						КонецЕсли;
						
					КонецЦикла;
					
					ВыборкаКодовМаркировки.Сбросить();
...
//  ИмяЭлемента - Строка - имя элемента форма, для которого определяется контактное лицо
//
Процедура ЗаполнитьПараметрыКонтактногоЛицаПоУмолчанию(Форма, Параметры, ИмяЭлемента = "") Экспорт
	...
	Если Параметры.КонтактноеЛицо.СпособОпределенияКонтактногоЛица = 3 
		...
		Если Форма.ДокументыОснования.Количество() = 1 Тогда
			...
			Если Менеджер <> Неопределено Тогда
				КонтактноеЛицоОрганизации = Менеджер.ФизическоеЛицо;
				Если ЗначениеЗаполнено(КонтактноеЛицоОрганизации) Тогда
					...
					Если ПараметрыКонтрагента.ЭтоОрганизация Тогда
						Параметры.КонтактноеЛицо.Ссылка = КонтактноеЛицоОрганизации;
	...
	ИначеЕсли Параметры.КонтактноеЛицо.СпособОпределенияКонтактногоЛица = 4 Тогда // Ответственный за склад
		...
		ИмяЭлемента = ?(ЗначениеЗаполнено(ИмяЭлемента), ИмяЭлемента, Форма.ТекущийЭлемент.Имя);
		КонтактноеЛицоОрганизации = Неопределено;
		...
		Если ЗначениеЗаполнено(Параметры.Адрес.Владелец) Тогда
			Если ТипЗнч(Параметры.Адрес.Владелец) = Тип("СправочникСсылка.Склады") Тогда
				КонтактноеЛицоОрганизации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.Адрес.Владелец, "ТекущийОтветственный");
			КонецЕсли;
		ИначеЕсли Форма.ДокументыОснования.Количество() = 1 Тогда
			
			ДокументОснование = Форма.ДокументыОснования[0].Значение;
			
			Если ИмяЭлемента = "ОтправительКонтрагент" Тогда
				
				Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Склад", ДокументОснование.Метаданные()) Тогда
					КонтактноеЛицоОрганизации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование.Склад, "ТекущийОтветственный");
				ИначеЕсли ОбщегоНазначения.ЕстьРеквизитОбъекта("СкладОтправитель", ДокументОснование.Метаданные()) Тогда
					КонтактноеЛицоОрганизации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование.СкладОтправитель, "ТекущийОтветственный");
				КонецЕсли;
			
			ИначеЕсли ИмяЭлемента = "ПолучательКонтрагент" Тогда
				
				Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Склад", ДокументОснование.Метаданные()) Тогда
					КонтактноеЛицоОрганизации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование.Склад, "ТекущийОтветственный");
				ИначеЕсли ОбщегоНазначения.ЕстьРеквизитОбъекта("СкладПолучатель", ДокументОснование.Метаданные()) Тогда
					КонтактноеЛицоОрганизации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование.СкладПолучатель, "ТекущийОтветственный");
				КонецЕсли;
	...
	ИначеЕсли Параметры.КонтактноеЛицо.СпособОпределенияКонтактногоЛица = 5 Тогда // Текущий пользователь
		...
		КонтактноеЛицоОрганизации  = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыСеанса.ТекущийПользователь, "ФизическоеЛицо");
...
Функция ТекстЗапросаВыборкаИзДанныхОснованийПоТоварам()
	Возврат 
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг.ШтрихкодыУпаковок КАК РеализацияТоваровУслугШтрихкодыУпаковок
		|		ПО ДанныеОснований.ДокументОснование = РеализацияТоваровУслугШтрихкодыУпаковок.Ссылка";
