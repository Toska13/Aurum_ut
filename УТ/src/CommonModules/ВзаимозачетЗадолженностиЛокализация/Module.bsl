...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктВзаимозачетаMicrosoftWord") Тогда 
		ПараметрыПечати = Новый Структура;
		ПараметрыПечати.Вставить("ИмяМакета", "ПФ_DOC_АктВзаимозачета_ru");
        ПараметрыПечати.Вставить("СинонимМакета",НСтр("ru = 'Акт взаимозачета задолженности'"));
		ПараметрыПечати.Вставить("Идентификатор","АктВзаимозачетаMicrosoftWord");
		ПараметрыПечати.Вставить("ШаблонИмениДокумента", 
					НСтр("ru = '[Организация]-[КонтрагентДебитор]-[КонтрагентКредитор] Акт взаимозачета задолженности №[Номер] от [Дата]'"));
		
		ПечатьДокументаWord(КоллекцияПечатныхФорм, МассивОбъектов, ПараметрыПечати);
	КонецЕсли;
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктПереуступкиДолгаMicrosoftWord") Тогда
		ПараметрыПечати = Новый Структура;
		ПараметрыПечати.Вставить("ИмяМакета", "ПФ_DOC_АктПереуступкиДолга_ru");
        ПараметрыПечати.Вставить("СинонимМакета",НСтр("ru = 'Акт переуступки долга'"));
		ПараметрыПечати.Вставить("Идентификатор","АктПереуступкиДолгаMicrosoftWord");
		ПараметрыПечати.Вставить("ШаблонИмениДокумента", 
					НСтр("ru = '[Организация]-[КонтрагентДебитор]-[КонтрагентКредитор] Акт переуступки долга №[Номер] от [Дата]'"));
		
		ПечатьДокументаWord(КоллекцияПечатныхФорм, МассивОбъектов, ПараметрыПечати);
	КонецЕсли;
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктПереуступкиДолгаМеждуОрганизациямиMicrosoftWord") Тогда
		ПараметрыПечати = Новый Структура;
		ПараметрыПечати.Вставить("ИмяМакета", "ПФ_DOC_АктПереуступкиДолгаМеждуОрганизациями_ru");
        ПараметрыПечати.Вставить("СинонимМакета",НСтр("ru = 'Акт переуступки долга между организациями'"));
		ПараметрыПечати.Вставить("Идентификатор","АктПереуступкиДолгаМеждуОрганизациямиMicrosoftWord");
		ПараметрыПечати.Вставить("ШаблонИмениДокумента", 
					НСтр("ru = '[Организация]-[ОрганизацияКредитор] Акт переуступки долга между организациями №[Номер] от [Дата]'"));
...
Процедура ПечатьДокументаWord(КоллекцияПечатныхФорм, МассивОбъектов, ПараметрыПечати)
	ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, ПараметрыПечати.Идентификатор);
	Если ПечатнаяФорма <> Неопределено Тогда
		
		МакетИДанныеОбъекта = УправлениеПечатью.МакетыИДанныеОбъектовДляПечати("Документ.ВзаимозачетЗадолженности", 
										ПараметрыПечати.ИмяМакета, МассивОбъектов);
		
		ОфисныеДокументы = Новый Соответствие;
		
		ЗначенияРеквизитовДокументов = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(МассивОбъектов, 
										"Организация,ОрганизацияКредитор,КонтрагентДебитор,КонтрагентКредитор,Номер,Дата,Ссылка");
		Для Каждого Ссылка Из МассивОбъектов Цикл
			...
			ИмяДокумента = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(ПараметрыПечати.ШаблонИмениДокумента, ЗначенияРеквизитовДокументов[Ссылка]);
			
			Если ПараметрыПечати.Идентификатор = "АктВзаимозачетаMicrosoftWord" Тогда
				АдресХранилищаОфисныйДокумент = УправлениеПечатьюУТСервер.ПечатьАктаВзаимозачетаЗадолженностиMicrosoftWord(Ссылка, МакетИДанныеОбъекта, ПараметрыПечати.ИмяМакета);
			ИначеЕсли ПараметрыПечати.Идентификатор = "АктПереуступкиДолгаMicrosoftWord" Тогда
				АдресХранилищаОфисныйДокумент = УправлениеПечатьюУТСервер.ПечатьАктаПереуступкиДолгаMicrosoftWord(Ссылка, МакетИДанныеОбъекта, ПараметрыПечати.ИмяМакета);
			Иначе
				АдресХранилищаОфисныйДокумент = УправлениеПечатьюУТСервер.ПечатьАктаПереуступкиДолгаМеждуОрганизациямиMicrosoftWord(Ссылка, МакетИДанныеОбъекта, ПараметрыПечати.ИмяМакета);
			КонецЕсли;
		...
		ПечатнаяФорма.СинонимМакета    = ПараметрыПечати.СинонимМакета;
