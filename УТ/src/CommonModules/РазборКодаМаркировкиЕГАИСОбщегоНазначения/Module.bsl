...
Процедура ДополнитьОписанияКодовМаркировки(ДанныеОписанийКодаМаркировки, ВидПродукции, ДанныеОбщегоМодуля) Экспорт
	...
	Если Не ОбщегоНазначенияИСПовтИсп.ЭтоМаркируемаяПродукцияЕГАИС(ВидПродукции) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДополнительныеПараметры(НастройкиРазбораКодаМаркировки, ВидПродукции, МодулиВыбраннойПодсистемы)
	...
	Если НастройкиРазбораКодаМаркировки.ТолькоСервер Тогда
		...
		ИмяОбщегоМодуля = "ШтрихкодированиеОбщегоНазначенияЕГАИС";
		
		ПроверкаШтрихкодаАкцизнойМарки.Вставить("ИмяОбщегоМодуляЕГАИС", ИмяОбщегоМодуля);
		ПроверкаШтрихкодаАкцизнойМарки.Вставить("ОбщийМодульЕГАИС",     ОбщегоНазначения.ОбщийМодуль(ИмяОбщегоМодуля));
