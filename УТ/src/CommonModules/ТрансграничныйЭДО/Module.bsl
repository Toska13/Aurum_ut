...
#Область ВыгрузкаДляФНС

// Дополнить выгрузку для ФНС.
// 
// Параметры:
//  ДанныеДокумента - см. ИнтеграцияБРОЭДО.НовыеДанныеДокументаДляВыгрузки
//  ОписьВыгрузки - см. ИнтеграцияБРОЭДО.НоваяОписьВыгрузкиДокументыPDF
//  АдресКаталога - Строка
//  ЭлементОписи - СтрокаТаблицыЗначений: см. ИнтеграцияБРОЭДО.НоваяОписьВыгрузкиДокументыPDF
//
Процедура ДополнитьВыгрузкуДляФНС(ДанныеДокумента, ОписьВыгрузки, АдресКаталога, ЭлементОписи) Экспорт
	
	Если НЕ ЭтоИностранныйОператор(ДанныеДокумента.ИдентификаторКонтрагента) Тогда
		Возврат;
	КонецЕсли;
	
	ЭлектронныеДокументы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДанныеДокумента.ЭлектронныйДокумент);
	...
	ПараметрыВизуализации.Формат = ДанныеДокумента.ФорматОсновногоТитула;
	
	ПечатныеФормы = ИнтерфейсДокументовЭДО.СформироватьПечатныеФормыЭлектронныхДокументов(
		ЭлектронныеДокументы, ПараметрыСоставаПечати, ПараметрыВизуализации);
		
	Если ЗначениеЗаполнено(ПечатныеФормы) Тогда
		
		Файл = Новый Файл(ЭлементОписи.ИмяФайлаДанных);
		ПолноеИмяФайла = СтрШаблон("%1%2.pdf", АдресКаталога, Файл.ИмяБезРасширения);
		ПечатныеФормы[0].ТабличныйДокумент.Записать(ПолноеИмяФайла, ТипФайлаТабличногоДокумента.PDF);
		УдалитьФайлы(АдресКаталога + ЭлементОписи.ИмяФайлаДанных);
		ЭлементОписи.ИмяФайлаДанных = ПолноеИмяФайла;
		УдалитьФайлы(АдресКаталога + ЭлементОписи.ИмяФайлаПодписи);
		ЭлементОписи.ИмяФайлаПодписи = "";
	...
	МассивЭД = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДанныеДокумента.ЭлектронныйДокумент);
	Файлы = ЭлектронныеДокументыЭДО.ПрисоединенныеФайлыЭлектронныхДокументов(МассивЭД);
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Подписи.ФайлКвитанцииДТС КАК ФайлКвитанцииДТС
		|ИЗ
		|	РегистрСведений.ЭлектронныеПодписиТЭДО КАК Подписи
		|ГДЕ
		|	Подписи.ПодписанныйОбъект В (&ПрисоединенныеФайлы)
		|	И Подписи.ПодписанныйОбъект.ВладелецФайла.ТипЭлементаРегламента В (&ТипыЭлементовРегламента)";
		
	Запрос.УстановитьПараметр("ПрисоединенныеФайлы", Файлы.ВыгрузитьКолонку("ПрисоединенныйФайл"));
	Запрос.УстановитьПараметр("ТипыЭлементовРегламента", ТипыЭлементовРегламента);
	Таблица = Запрос.Выполнить().Выгрузить();
	ПрисоединенныеФайлы = Таблица.ВыгрузитьКолонку("ФайлКвитанцииДТС");
	...
	Для Каждого ПрисоединенныйФайл Из ПрисоединенныеФайлы Цикл
		...
		ДвоичныеДанныеКвитанции = ДвоичныеДанныеФайлов[ПрисоединенныйФайл]; // ДвоичныеДанные
		ДанныеКвитанции = ДанныеКвитанцииДТС(ДвоичныеДанныеКвитанции);
		Если НЕ ЗначениеЗаполнено(ДанныеКвитанции.Представление) Тогда
			Продолжить;
		КонецЕсли;
		
		ОписаниеКвитанции = ОписьВыгрузки.Добавить();
		
		Представление = ДанныеКвитанции.Представление; // см. РаботаСФайламиБЭД.НовоеОписаниеФайла
		ОписаниеКвитанции.ИмяФайлаДанных = Представление.ИмяФайла;
		ОписаниеКвитанции.РазмерФайлаДанных = Представление.ДвоичныеДанные.Размер();
		Представление.ДвоичныеДанные.Записать(АдресКаталога + ОписаниеКвитанции.ИмяФайлаДанных);
		
		Подпись = ДанныеКвитанции.Подпись; // см. РаботаСФайламиБЭД.НовоеОписаниеФайла
		ОписаниеКвитанции.ИмяФайлаПодписи = Подпись.ИмяФайла;
		Подпись.ДвоичныеДанные.Записать(АдресКаталога + ОписаниеКвитанции.ИмяФайлаПодписи);
		
		ЗаполнитьЗначенияСвойств(ОписаниеКвитанции, ЭлементОписи,
			"ТипДокументаЭДО, ЭлектронныйДокумент, ДатаДокумента, НомерДокумента");
		
	КонецЦикла;
	
КонецПроцедуры

// см. ЭлектронныеДокументыЭДО.ТипыДокументовЭДОВыгрузкиДляФНС
Функция ТипыДокументовЭДОВыгрузкиДляФНС() Экспорт
	
	Результат = Новый Массив(); // см. ЭлектронныеДокументыЭДО.ТипыДокументовЭДОВыгрузкиДляФНС
	СтандартныеТипыДокументов = СтандартныеТипыДокументов();
	Результат.Добавить(СтандартныеТипыДокументов.ТоварнаяНакладнаяРБ);
	Возврат Результат;
...
// При определении использования универсальных сообщений.
// 
// Параметры:
//  ПараметрыДокумента - см. РегламентыЭДО.НовыеПараметрыОпределенияИспользованияУС
//  ИспользоватьУС - Булево
//
Процедура ПриОпределенииИспользованияУниверсальныхСообщений(ПараметрыДокумента, ИспользоватьУС) Экспорт
	
	Если ИспользоватьУС И (ЭтоИностранныйОператор(ПараметрыДокумента.ИдентификаторОрганизации)
		ИЛИ ЭтоИностранныйОператор(ПараметрыДокумента.ИдентификаторКонтрагента)) Тогда
		ИспользоватьУС = Ложь;
	КонецЕсли;
	
КонецПроцедуры

// Заполняет состояние ЭД по входящему отклонению.
// 
// Параметры:
...
// 
// Возвращаемое значение:
//  Булево - Заполнить состояние по входящему отклонению
Функция ЗаполнитьСостояниеПоВходящемуОтклонению(ПараметрыДокумента, СостоянияЭлементовРегламента, Состояние) Экспорт
	...
	|ИдентификаторСвязи";
	ДанныеЭД = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПараметрыДокумента.Ссылка, Реквизиты);
	
	Если НЕ ЭтоИностранныйОператор(ДанныеЭД.ИдентификаторКонтрагента) Тогда
		Возврат Ложь;
	КонецЕсли;

	ДанныеДляЗаполнения = ДанныеДляЗаполненияСостоянияЭД();
	ДанныеДляЗаполнения.ТипДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЭД.ВидДокумента, "ТипДокумента");
	ЗаполнитьЗначенияСвойств(ДанныеДляЗаполнения, ДанныеЭД);
	
	Для Каждого Обработчик Из Обработчики() Цикл
		Обработчик.ЗаполнитьСостояниеПоВходящемуОтклонению(ДанныеДляЗаполнения, СостоянияЭлементовРегламента, Состояние);
...
// Дополняет коллекцию корректируемых типов ЭД.
// 
// Параметры:
//  ТипыЭД - Соответствие из КлючИЗначение:
//  * Ключ - ПеречислениеСсылка.ТипыДокументовЭДО
//  * Значение - ПеречислениеСсылка.ТипыДокументовЭДО
//
Процедура ДополнитьКорректируемыеТипыЭД(ТипыЭД) Экспорт
	
	Для Каждого Обработчик Из Обработчики() Цикл
		Обработчик.ДополнитьКорректируемыеТипыЭД(ТипыЭД);
	КонецЦикла;
	
КонецПроцедуры

// Дополняет коллекцию исправляющих типов ЭД.
// 
// Параметры:
//  ТипыЭД - Соответствие из КлючИЗначение:
//  * Ключ - ПеречислениеСсылка.ТипыДокументовЭДО
//  * Значение - Булево
//
Процедура ДополнитьИсправляющиеТипыЭД(ТипыЭД) Экспорт
	
	Для Каждого Обработчик Из Обработчики() Цикл
		Обработчик.ДополнитьИсправляющиеТипыЭД(ТипыЭД);
	КонецЦикла;
...
//  Булево -
//
Функция ЭтоИностранныйОператор(ИдентификаторУчастника) Экспорт
	...
	Для Каждого Обработчик Из Обработчики() Цикл
		Если Обработчик.ЭтоОператорТЭДО(ИдентификаторУчастника) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
// Проверяет служебное имя дополнительного поля
// 
// Параметры:
//  ИмяПоля - Строка
//  Формат - Строка
...
Функция ЭтоСлужебноеИмяДополнительногоПоля(ИмяПоля, Формат) Экспорт
	...
	Для Каждого Обработчик Из Обработчики() Цикл
		Если Обработчик.ЭтоСлужебноеИмяДополнительногоПоля(ИмяПоля, Формат) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
// Дополняет настройку данными дополнительных полей шапки
// 
// Параметры:
//  ИдентификаторКонтрагента - Строка
//  ПараметрыЗаполнения - см. ФорматыЭДО.ПараметрыЗаполненияДополнительныхПолей
//
Процедура ДополнитьНастройкуЗаполненияДополнительныхПолей(ИдентификаторКонтрагента, ПараметрыЗаполнения) Экспорт
	
	Для Каждого Обработчик Из Обработчики() Цикл
		
		ДанныеДополнительныхПолей = Обработчик.ДанныеДополнительныхПолей(ИдентификаторКонтрагента, ПараметрыЗаполнения);
		Если НЕ ЗначениеЗаполнено(ДанныеДополнительныхПолей) Тогда
			Продолжить;
		КонецЕсли;
		
		РазделыДополнительныхПолей = ФорматыЭДО.РазделыДополнительныхПолейФорматаЭлектронногоДокумента(
			ПараметрыЗаполнения.НастройкиОбмена.ТипДокумента, ПараметрыЗаполнения.НастройкиОбмена.Формат);
			
		Если РазделыДополнительныхПолей.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;
		
		НастройкаЗаполнения = ПараметрыЗаполнения.НастройкаЗаполнения;
		Если НастройкаЗаполнения = Неопределено Тогда
			НастройкаЗаполнения = НастройкиОтправкиЭДО.НоваяНастройкаЗаполненияДополнительныхПолей();
			ПараметрыЗаполнения.НастройкаЗаполнения = НастройкаЗаполнения;
		КонецЕсли;
		...
		Для Каждого Раздел Из РазделыДополнительныхПолей Цикл
			...
			Если Раздел.Тип = ТипыРазделовДополнительныхПолей.Шапка Тогда
				...
				Для Каждого КлючИЗначение Из ДанныеДополнительныхПолей Цикл
					
					Если НастройкаЗаполнения.Найти(КлючИЗначение.Ключ, "Имя") <> Неопределено Тогда
						Продолжить;
					КонецЕсли;
					
					НоваяНастройка = НастройкаЗаполнения.Добавить();
					НоваяНастройка.Версия = НастройкиЭДОКлиентСервер.ВерсияНастроекДополнительныхПолейПоФормуле();
					НоваяНастройка.Заполнение = "ПоФормуле";
					НоваяНастройка.Значение = СтрШаблон("""%1""", КлючИЗначение.Значение);
					Идентификатор = "ТЭДО_" + РаботаСФайламиБЭД.НовыйUUID();
					НоваяНастройка.Идентификатор = Идентификатор;
					НоваяНастройка.Имя = КлючИЗначение.Ключ;
					НоваяНастройка.Правило = НСтр("ru = 'По Формуле'");
					НоваяНастройка.Представление = "";
					НоваяНастройка.Раздел = ТипыРазделовДополнительныхПолей.Шапка;
					НоваяНастройка.Описание = "";
...
// Заполняет параметры связи для иностранных операторов
// 
// Параметры:
//  ПараметрыСвязи - см. ИнтерфейсДокументовЭДО.НовыеПараметрыСвязиИдентификаторов
//
Процедура ЗаполнитьПараметрыСвязиИдентификаторов(ПараметрыСвязи) Экспорт
	
	Для Каждого Обработчик Из Обработчики() Цикл
		Обработчик.ЗаполнитьПараметрыСвязиИдентификаторов(ПараметрыСвязи);
	КонецЦикла;
...
Функция НаименованиеСтраныПоКоду(КодСтраны) Экспорт
	...
	Возврат ?(ЗначениеЗаполнено(ДанныеСтраны.НаименованиеПолное), 
		ДанныеСтраны.НаименованиеПолное, ДанныеСтраны.Наименование);
...
// Возвращает данные квитанции ДТС.
// 
// Параметры:
//  ДвоичныеДанные - ДвоичныеДанные
...
// Возвращаемое значение:
//  Структура:
//  * Представление - см. РаботаСФайламиБЭД.НовоеОписаниеФайла, Неопределено -
//  * Подпись - см. РаботаСФайламиБЭД.НовоеОписаниеФайла, Неопределено -
//
Функция ДанныеКвитанцииДТС(ДвоичныеДанные, Знач ИмяФайлаБезРасширения = "")
	...
	Квитанция = ПрочитатьКвитанциюДТС(ДвоичныеДанные);
	
	Если НЕ ЗначениеЗаполнено(Квитанция) Тогда
		Возврат Результат;
	КонецЕсли;
	
	ПредставлениеСтрокой = Квитанция["printForm"]; // Строка
	Если НЕ ЗначениеЗаполнено(ПредставлениеСтрокой) Тогда
		Возврат Результат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ИмяФайлаБезРасширения) Тогда
		ИмяФайлаБезРасширения = РаботаСФайламиБЭД.НовыйUUID();
	КонецЕсли;
	
	Представление = РаботаСФайламиБЭД.НовоеОписаниеФайла();
	Представление.ДвоичныеДанные = Base64Значение(ПредставлениеСтрокой);
	Представление.ИмяФайла = СтрШаблон("%1.pdf", ИмяФайлаБезРасширения);
	
	ПодписьСтрокой = Квитанция["printFormSignature"]; // Строка
	Если НЕ ЗначениеЗаполнено(ПодписьСтрокой) Тогда
		Возврат Результат;
	КонецЕсли;
	
	Подпись = РаботаСФайламиБЭД.НовоеОписаниеФайла();
	Подпись.ДвоичныеДанные = Base64Значение(ПодписьСтрокой);
	Подпись.ИмяФайла = СтрШаблон("%1.sig", ИмяФайлаБезРасширения);
...
// Описывает данные для заполнения состояния ЭД.
// 
// Возвращаемое значение:
//  Структура:
//  * ТипДокумента - ПеречислениеСсылка.ТипыДокументовЭДО
...
//
Функция ДанныеДляЗаполненияСостоянияЭД() Экспорт
	...
	Результат.Вставить("ИдентификаторСвязи", "");
	Возврат Результат;
