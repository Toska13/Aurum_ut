...
Процедура СкладыОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	УсловияПоиска = "Склады.Наименование ПОДОБНО &СтрокаПоиска СПЕЦСИМВОЛ ""~""";
	...
	Для НомерДопЯзыка = 1 По МультиязычностьСервер.КоличествоДополнительныхЯзыков() Цикл
		...
		Если МультиязычностьСервер.ИспользуетсяДополнительныйЯзык(НомерДопЯзыка) Тогда
			...
			|	ИЛИ " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Склады.%1 ПОДОБНО &СтрокаПоиска СПЕЦСИМВОЛ ""~""",
