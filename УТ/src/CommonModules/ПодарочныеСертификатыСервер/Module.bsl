...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ) Экспорт
	...
	ИначеЕсли ТипСсылкиОбъекта = Тип("ДокументСсылка.ВзаимозачетЗадолженности") Тогда
		...
		КоличествоПодарочныхСертификатов = ПроверитьЗаполнениеТабличнойЧастиРасшифровкаПлатежа(Объект, Отказ);
		Если КоличествоПодарочныхСертификатов Тогда
			Если (Объект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносАвансаКлиентаМеждуКонтрагентами) Тогда
				Если Не Объект.КонтрагентДебитор = Справочники.Контрагенты.РозничныйПокупатель Тогда

					ТекстОшибки = НСтр("ru = 'При продаже подарочного сертификата клиентом-получателем должен быть указан Розничный покупатель'");
		...
		Если Выборка.Следующий() Тогда
			...
			Если ТипСсылкиОбъектаФормы = Тип("ДокументСсылка.РасходныйКассовыйОрдер") 
				...
				Если Не ЗначениеЗаполнено(НайденнаяСтрока.СтавкаНДС) Тогда
					...
					СтруктураПараметровДействия = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Форма.Объект);
...
// Параметры:
//  Объект - ДокументОбъект - Текущий документ.
//  ИмяТабличнойЧасти - Строка - Имя метаданных табличной части документа.
//  Отказ - Булево - Признак отказа от проведения документа.
//
// Возвращаемое значение:
//  Число - Количество строк табличной части, в которых объектом расчета является подарочный сертификат
//
Функция ПроверитьЗаполнениеТабличнойЧастиРасшифровкаПлатежа(Объект, Отказ)
	
	КоличествоПодарочныхСертификатов = 0;
	...
	Если ТипСсылкиОбъекта = Тип("ДокументСсылка.ВзаимозачетЗадолженности") Тогда
		...
		ИначеЕсли (Объект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносАвансаКлиентаМеждуКонтрагентами) Тогда
			ИмяТабличнойЧасти = Объект.Ссылка.Метаданные().ТабличныеЧасти.ДебиторскаяЗадолженность.Имя;
			ПроверятьРавенствоСуммыИНоминала = Ложь;
		Иначе
			Возврат КоличествоПодарочныхСертификатов;
		КонецЕсли;
	ИначеЕсли ТипСсылкиОбъекта = Тип("ДокументСсылка.ВводОстатковВзаиморасчетов") Тогда
	...
	КоличествоПодарочныхСертификатов = ПодарочныеСертификатыПоОбъектамРасчетов.Количество();
	Если КоличествоПодарочныхСертификатов = 0 Тогда
		Возврат КоличествоПодарочныхСертификатов;
	КонецЕсли;
