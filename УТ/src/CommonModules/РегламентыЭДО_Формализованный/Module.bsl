...
Функция СостояниеВходящегоДокумента(ПараметрыДокумента, СостоянияЭлементовРегламента) Экспорт
	...
	Если ТрансграничныйЭДО.ЗаполнитьСостояниеПоВходящемуОтклонению(
...
Функция ДокументБезОтветногоТитула(ПараметрыДокумента) Экспорт
	...
	ТипПараметровДокумента = ТипЗнч(ПараметрыДокумента);
	ИмяПараметра = "ТипДокумента";
	
	Если ТипПараметровДокумента = Тип("Структура") Тогда
		ПараметрыДокумента.Свойство(ИмяПараметра, ТипДокумента);
	ИначеЕсли ТипПараметровДокумента = Тип("СтрокаТаблицыЗначений") Тогда
		ПараметрыДокументаСтруктурой = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(ПараметрыДокумента);
		ПараметрыДокументаСтруктурой.Свойство(ИмяПараметра, ТипДокумента);
	ИначеЕсли ТипПараметровДокумента = Тип("ВыборкаИзРезультатаЗапроса")
		И ПараметрыДокумента.Владелец().Колонки.Найти(ИмяПараметра) <> Неопределено Тогда
		ТипДокумента = ПараметрыДокумента[ИмяПараметра]; // ПеречислениеСсылка.ТипыДокументовЭДО		
	КонецЕсли;
	
	Возврат ЭлектронныеДокументыЭДО.ЭтоТипБезОтветногоТитулаПриФормализованномОбмене(ТипДокумента);
...
Функция ЗаполнитьСостояниеПоВходящейИнформацииОтправителя(СостоянияЭлементовРегламента, Состояние)
	...
	ИначеЕсли ЭлементРегламента.Состояние = Перечисления.СостоянияСообщенийЭДО.ПодготовкаКОтправке Тогда
		...
		ТипДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭлементРегламента.Ссылка,
			"ВидСообщения.ТипДокумента", Истина);
		Если ЭлектронныеДокументыЭДО.ЭтоТипБезОтветногоТитулаПриФормализованномОбмене(ТипДокумента) Тогда
...
Функция ЗаполнитьСостояниеПоИсходящейИнформацииПолучателя(ПараметрыДокумента, СостоянияЭлементовРегламента, Состояние)
	...
	Если ДокументБезОтветногоТитула(ПараметрыДокумента) Тогда
		Если ПараметрыДокумента.ОбменБезПодписи Тогда	
			Возврат Ложь;
		Иначе
			Если РегламентыЭДО.ЕстьЭлементРегламента(СостоянияЭлементовРегламента,
				Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияОтправителя, ЭлементРегламента)
...
Функция ЗаполнитьСостояниеПоВходящемуОтклонению(ПараметрыДокумента, СостоянияЭлементовРегламента, Состояние)
	...
	Если РегламентыЭДО.ОдинИзЭлементовРегламентаВходитВКоллекцию(СостоянияЭлементовРегламента, ТипыУОУ) 
...
Функция ЗаполнитьСостояниеПоВходящейИнформацииПолучателя(ПараметрыДокумента, СостоянияЭлементовРегламента, Состояние)
	...
	Если ДокументБезОтветногоТитула(ПараметрыДокумента) Тогда
		Если ПараметрыДокумента.ТребуетсяПодтверждение Тогда	
