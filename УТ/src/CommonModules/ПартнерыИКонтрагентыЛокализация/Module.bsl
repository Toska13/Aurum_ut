...
Процедура ДобавитьКомандыОтчетов_Контрагенты(КомандыОтчетов, Параметры) Экспорт
	...
	ВариантыОтчетовЛокализация.ДобавитьКомандуПроверкаКонтрагента(КомандыОтчетов);
	ВариантыОтчетовЛокализация.ДобавитьКомандуПоискКонтрагентов(КомандыОтчетов);
...
Процедура ДобавитьКомандыОтчетов_Партнеры(КомандыОтчетов, Параметры) Экспорт
	...
	Если ИспользоватьПартнеровКакКонтрагентов Тогда
		...
		ВидимостьВФормах = "ФормаЭлемента,ФормаСписка,ФормаСпискаБезПолнотекстовогоПоиска,ФормаЭлементаРеквизитыКонтрагента";
		
		ВариантыОтчетовЛокализация.ДобавитьКомандуПроверкаКонтрагента(КомандыОтчетов, ВидимостьВФормах);
		ВариантыОтчетовЛокализация.ДобавитьКомандуПоискКонтрагентов(КомандыОтчетов, ВидимостьВФормах);
...
#Область ПроцедурыИФункцииНастройкиУчетаНДСПриУСНКомитентов

// Возвращает таблицу актуальных значения настроек по списку контрагентов на дату.
// 
// Параметры:
// 	ИмяРегистра - Строка - имя регистра сведений.
// 	Контрагент - Массив из СправочникСсылка.Контрагенты - массив ссылок контрагентов.
...
// 		* Период - Дата
// 		* ОсновноеНалогообложение - ПеречислениеСсылка.ТипыНалогообложенияНДС
// 		* СтавкаНДС - СправочникСсылка.СтавкиНДС
// 		* УчетнаяПолитикаСуществует - Булево
//
Функция ТаблицаДействующихНалоговУчетныхПолитикПоКонтрагентам(ИмяРегистра, Контрагент, ДатаДействия = Неопределено, ВозвращатьПоУмолчанию = Истина) Экспорт
	
	ТаблицаРезультатЗапроса = Новый ТаблицаЗначений();
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРезультатЗапроса Цикл
		...
		СписокПараметров = ПартнерыИКонтрагентыЛокализацияКлиентСервер.ДоступныеРегистрыНалоговУчетныхПолитикКонтрагентов();
...
// 	Контрагент - СправочникСсылка.Контрагенты - ссылка на справочник Контрагенты.
// 	ИмяРегистра - Строка - наименование периодического регистра сведений.
// 	ДатаДействия - Дата - период получения данных.
//
Процедура ЗаполнитьОписаниеДействующихПараметровКонтрагентов(Форма, Контрагент, ИмяРегистра = "", Знач ДатаДействия = "") Экспорт
	...
	СоответствиеЭлементов = СоответствиеЭлементовФормыКонтрагентов();
	Если ЗначениеЗаполнено(ИмяРегистра) Тогда
		...
		ЭлементОписаниеУчетнойПолитики.Заголовок = РегистрыСведений[ИмяРегистра].ОписаниеДействующихПараметров(Контрагент,
				ДатаДействия);
		Возврат;
	КонецЕсли;

	МассивПараметров = ПартнерыИКонтрагентыЛокализацияКлиентСервер.ДоступныеРегистрыНалоговУчетныхПолитикКонтрагентов();
...
// Возвращает таблицу действующих значения настроек по списку контрагентов на дату.
// 
// Параметры:
// 	ИмяРегистра - Строка - имя регистра сведений.
// 	Контрагент - Массив из СправочникСсылка.Контрагенты - массив ссылок контрагентов.
...
// 	ВозвращатьПоУмолчанию - Булево - признак заполнения значений по умолчанию, 
// 		если передана ложь - вернет пустую таблицу.
// 	ВыдаватьИсключение - Булево - признак вызова исключения.
//
// Возвращаемое значение:
// 	Неопределено - если нет ни одной записи настроек учета неопределено.
// 	Структура Из КлючИЗначение :
// 		Ключ - Строка - имя реквизита из РС "Настройки учета НДС при УСН Комитентов"
// 		Значение - значение реквизита в РС "Настройки учета НДС при УСН Комитентов".
//
Функция ДействующиеПараметрыНалоговУчетныхПолитикКонтрагентов(ИмяРегистра, Контрагент, Знач ДатаДействия = Неопределено, ВозвращатьПоУмолчанию = Истина, ВыдаватьИсключение = Ложь) Экспорт
	...
	ТаблицаРезультатЗапроса = ТаблицаДействующихНалоговУчетныхПолитикПоКонтрагентам(ИмяРегистра, Контрагент, ДатаДействия, ВозвращатьПоУмолчанию);
	...
	ИначеЕсли ВыдаватьИсключение Тогда
		...
		ШаблонИсключение = НСтр("ru='Не заданы параметры ""%1"" для контрагента ""%2"" на дату %3.'");
		ВызватьИсключение СтрШаблон(ШаблонИсключение, НазваниеРегистра, Контрагент, Формат(ДатаДействия, "ДЛФ=DD;"));
	КонецЕсли;
	//-- Локализация
...
//
// Возвращаемое значение:
// 	Соответствие Из КлючИЗначение - Соответствие элементов формы, где:
// 		* Ключ - Строка - имя настройки.
// 		* Значение - Структура:
//      ** МассивЭлементов - Массив имен элементов формы.
//      ** ЭлементОписание - Строка - имя элемента формы для описания настройки.
//
Функция СоответствиеЭлементовФормыКонтрагентов()  Экспорт
	
	СоответствиеЭлементов = Новый Соответствие();

	//++ Локализация
	МассивЭлементов = Новый Массив();
	МассивЭлементов.Добавить("ЛокализацияГруппаНастройкиУчетаНДСКомитента");
	МассивЭлементов.Добавить("ОтступНастройкиУчетаНДСКомитента");
	МассивЭлементов.Добавить("ГруппаНастройкиУчетаНДСПриУСНКомитента");
	МассивЭлементов.Добавить("ОписаниеНастройкиУчетаНДСПриУСНКомитента");
	МассивЭлементов.Добавить("ОткрытьНастройкиУчетаНДСПриУСНКомитента");

	СтруктураЭлементов = Новый Структура();
	СтруктураЭлементов.Вставить("МассивЭлементов", МассивЭлементов);
	СтруктураЭлементов.Вставить("ЭлементОписание", "ОписаниеНастройкиУчетаНДСПриУСНКомитента");
	
	СоответствиеЭлементов.Вставить("НастройкиУчетаНДСПриУСНКомитентов", СтруктураЭлементов);
	//-- Локализация
...
// Заполнение формы ПрайсЛист - ФормаНастройкиЗаполненияСтавокНДС.
// 
// Параметры:
...
//
Процедура ПолучитьНалогообложениеКонтрагента(Контрагент, ДатаДействия, СтавкаНДС) Экспорт

	//++ Локализация
	
	ДействующиеНастройки = ДействующиеПараметрыНалоговУчетныхПолитикКонтрагентов(
			"НастройкиУчетаНДСПриУСНКомитентов",
			Контрагент,
			ДатаДействия,
			Ложь);
	
	Если ЗначениеЗаполнено(ДействующиеНастройки) И ЗначениеЗаполнено(ДействующиеНастройки.СтавкаНДС) Тогда
		СтавкаНДС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДействующиеНастройки.СтавкаНДС, "ПеречислениеСтавкаНДС");
	Иначе
...
Процедура ПриСозданииНаСервереФормаПомощникНового(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли Параметры.Свойство("ИНН") Тогда
		...
		ЗаполнитьИННПомощникНового(Форма, Параметры.ИНН);
		
	ИначеЕсли Параметры.Свойство("РаботаСКонтрагентамиСозданиеИзДосьеКонтрагента")
		И Параметры.РаботаСКонтрагентамиСозданиеИзДосьеКонтрагента
		И Параметры.Свойство("ТекстЗаполнения")
		И ЗначениеЗаполнено(Параметры.ТекстЗаполнения) Тогда
		
		ЗаполнитьИННПомощникНового(Форма, Параметры.ТекстЗаполнения);
...
Процедура ДополнитьДанныеКонтрагентаПриЗаписи(ОбъектКонтрагент, Форма) Экспорт
	...
	ОбъектКонтрагент.РегистрационныйНомер  = ?(ОбъектКонтрагент.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо
													...
													Или ОбъектКонтрагент.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ИндивидуальныйПредприниматель,
...
Процедура ЗаполнитьИтоговуюИнформацию(ИтоговаяИнформация, Форма) Экспорт
	...
	Если (Форма.ИспользуютсяТолькоПартнеры Или Форма.ДействиеСКонтрагентом = 1) И Форма.УказыватьЮридическиеРеквизиты Тогда
		...
		Если Форма.ЕстьДубльПоОснРеквизитам Тогда 
			...
			ИтоговаяИнформация = СтрЗаменить(ИтоговаяИнформация, "#InformationRepetitiveINN#", СообщениеДубльИНН(СтруктураИНН.ИНН_КПП, РезультатПоиска.НайденныеСовпадения[0]));
...
Процедура ЗаполнитьИННПомощникНового(Форма, ИНН)
	
	Если СтрДлина(ИНН) = 12 Тогда
		
		Форма.ИННПартнерФизЛицо = ИНН;
		Форма.ЭтоКомпания = 1;
