...
Функция СтатусыКодовМаркировки(МассивИсходныхСтрок, Организация = Неопределено, ЗапрашиватьСтатусыКодовЕАЭС = Ложь) Экспорт
	...
	Для Каждого ИсходнаяСтрока Из МассивИсходныхСтрок Цикл
		...
		Иначе
			СтрокиДляЗапроса.Добавить(ИсходнаяСтрока);
		КонецЕсли;
...
//	* ТребуетсяОбновлениеКлючаСессии - Булево - Необходимость обновления ключа сессии.
//	* РезультатОтправкиЗапроса - (См. ОбменДаннымиИСМПКлиентСервер.ОбработатьРезультатОтправкиHTTPЗапросаКакJSON).
//	* ТоварнаяГруппа - СправочникСсылка.ТоварныеГруппыГИСМТ - товарная группа по данным сервиса.
//	* ТекстОшибки - Строка - Текст сообщения об ошибке.
Функция ТоварнаяГруппаПоКодамИдентификации(МассивКодовМаркировки, Организация = Неопределено) Экспорт
	...
	ВозвращаемоеЗначение.Вставить("ТребуетсяОбновлениеКлючаСессии", КлючСессии = Неопределено);
	ВозвращаемоеЗначение.Вставить("РезультатОтправкиЗапроса",       Неопределено);
	ВозвращаемоеЗначение.Вставить("ТоварнаяГруппа",                 Неопределено);
	ВозвращаемоеЗначение.Вставить("ТекстОшибки",                    "");
					...
					Для Каждого ЭлементДанных Из ДанныеОбработки Цикл
						...
						Иначе
							...
							Если Значение <> Неопределено Тогда
								ВозвращаемоеЗначение.ТоварнаяГруппа = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ТоварнаяГруппаПоРеквизиту("Имя", Значение);
							КонецЕсли;
						КонецЕсли;
...
// Параметры:
//  Организация - ОпределяемыйТип.Организация - Организация
//  ВидМаркируемойПродукции - СправочникСсылка.ВидыПродукцииИС - Вид продукции
//  GTIN - Массив Из Строка, Строка - GTIN для которых необходимо получить количество
//  ПараметрыНавигации - Неопределено, 
//  	Структура - Структура со свойствами:
//      * ЗаписейНаСтранице - Число - Количество записей.
...
Функция ОстатокGTINПоДаннымВиртуальногоБаланса(Организация, ВидМаркируемойПродукции, GTIN, ПараметрыНавигации = Неопределено) Экспорт
	...
	Если ПараметрыНавигации <> Неопределено Тогда
		...
		ТелоЗапроса.pagination.Вставить("perPage", ПараметрыНавигации.ЗаписейНаСтранице);
		ТелоЗапроса.pagination.Вставить("page", ПараметрыНавигации.НомерСтраницы);
	КонецЕсли;
...
Функция ПараметрыЗапросаКодовМаркировкиПоФильтру() Экспорт
	...
	ВозвращаемоеЗначение.Фильтр.Вставить("ВидПродукции", Справочники.ВидыПродукцииИС.ПустаяСсылка());
...
Функция КодыМаркировкиПоФильтру(ПараметрыЗапроса) Экспорт
	...
	ТелоЗапроса.filter.productGroups.Добавить(ИнтерфейсИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппа(ПараметрыЗапроса.Фильтр.ВидПродукции));
			...
			Иначе
				...
				Для Каждого СтрокаДанных Из Результат Цикл
					...
					ВозвращаемоеЗначение.ДанныеКодовМаркировки.Вставить(
						...
						ИнтерфейсИСМПОбщегоНазначения.ПараметрыКодаМаркировкиМОТП(СтрокаДанных, , , Эталон));
...
// 	ВидПродукцииТоварнаяГруппа - СправочникСсылка.ВидыПродукцииИС, СправочникСсылка.ТоварныеГруппыГИСМТ - Вид продукции или товарная группа.
// 	НомерСтраницы - Число                              - Номер страницы.
// 	Организация   - ОпределяемыйТип.Организация        - Организация.
...
Функция НайтиПродукциюПоИННПроизводителя(ИНН, ВидПродукцииТоварнаяГруппа, НомерСтраницы = 0, Организация = Неопределено, ВариантAPI = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ВидПродукцииТоварнаяГруппа) Тогда
		ПараметрыЗапроса.Добавить(СтрШаблон("pg=%1", ИнтерфейсИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппа(ВидПродукцииТоварнаяГруппа)));
	Иначе
	...
	Если РезультатОтправкиЗапроса.ОтветПолучен Тогда
		...
		Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
			...
			Иначе
				...
				КонецЦикла;
				ОтветДанныеПоШтрихкодуEAN = ИнтерфейсИСМПОбщегоНазначения.ДанныеПродукцииПоШтрихкодуEAN(МассивGTIN);
...
// Получает остатки сырья постранично из сервиса ИСМП с учетом навигации и фильтрации
//
// Параметры:
//   ПараметрыНавигации - Структура:
//                         * КоличествоЗаписей - Число - количество записей на странице
//                         * Курсор - Строка - курсор для навигации между страницами
//                         * ЕстьСледующаяСтраница - Булево - признак наличия следующей страницы (заполняется в функции)
//   ВидПродукции - Строка - вид продукции для фильтрации (необязательно)
//   Организация - СправочникСсылка.Организации - организация для авторизации (необязательно)
//   ПараметрыПоиска - Структура
//   * Идентификаторы - Массив из Строка - фильтр 
//
// Возвращаемое значение:
//   Структура:
...
//     * Сырье - Массив из Структура - массив данных сырья
Функция ОстаткиСырьяПостранично(ПараметрыНавигации, ВидПродукции, Организация = Неопределено, ПараметрыПоиска = Неопределено) Экспорт
	...
	ВозвращаемоеЗначение.Вставить("РезультатОтправкиЗапроса",       Неопределено);
	ВозвращаемоеЗначение.Вставить("ТекстОшибки",                    "");
	ВозвращаемоеЗначение.Вставить("Сырье",                          Новый Соответствие);
	...
	Если ЗначениеЗаполнено(ПараметрыПоиска)
		И ПараметрыПоиска.Свойство("Идентификаторы") Тогда
		
		Фильтры = Новый Массив;
		
		Фильтр = Новый Структура;
		Фильтр.Вставить("attribute", "product.id");
		Фильтр.Вставить("operation", "in");
		Фильтр.Вставить("values",    Новый Массив);
		Фильтр.values = ПараметрыПоиска.Идентификаторы;
		
		Фильтры.Добавить(Фильтр);
		
		ТелоЗапроса.Вставить("filters", Фильтры);
		
	КонецЕсли;
	
	Сортировка = Новый Структура;
	Сортировка.Вставить("attribute", "createdAt");
	Сортировка.Вставить("order",     "ASC");
	ТелоЗапроса.Вставить("sorting", Сортировка);
	
	Пагинация = Новый Структура;
	Пагинация.Вставить("direction", "NEXT");
	Пагинация.Вставить("limit",     ПараметрыНавигации.КоличествоЗаписей);
	
	Если ЗначениеЗаполнено(ПараметрыНавигации.Курсор) Тогда
		Пагинация.Вставить("cursor", ПараметрыНавигации.Курсор);
	КонецЕсли;
	
	ТелоЗапроса.Вставить("pagination", Пагинация);
	...
	URLЗапроса = СтрШаблон(
			"api/v3/true-api/sim/warehouse/balance/actual%1",
			ПараметрыЗапроса(ПараметрыЗапроса));
	
	ЗаголовокHTTP = Новый Соответствие();
	ЗаголовокHTTP.Вставить("Content-Type",   "application/json; charset=utf-8");
	ЗаголовокHTTP.Вставить("Accept-Charset", "utf-8");
	ЗаголовокHTTP.Вставить("X-Request-Id",   Строка(Новый УникальныйИдентификатор()));
	...
	РезультатЗапроса = ОбменДаннымиИСМПКлиентСервер.ОтправитьДанныеВСервис(
		...
		КлючСессии,
		"POST",
		ОбщегоНазначенияИСМПКлиентСервер.ПараметрыОтправкиHTTPЗапросов("", Истина), ЗаголовокHTTP);
	
	РезультатОтправкиЗапроса = ОбменДаннымиИСМПКлиентСервер.ОбработатьРезультатОтправкиHTTPЗапросаКакJSON(РезультатЗапроса);
	
	ВозвращаемоеЗначение.РезультатОтправкиЗапроса = РезультатОтправкиЗапроса;
	ПараметрыНавигации.ЕстьСледующаяСтраница = Ложь;
	...
	Если РезультатОтправкиЗапроса.ОтветПолучен Тогда
		...
		Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
			...
			Иначе
				...
				Если ДанныеОбработки.Свойство("items") Тогда
					
					Для Каждого ЭлементДанных Из ДанныеОбработки.items Цикл
						
						Если ЭлементДанных.Свойство("product") Тогда
							ДанныеСырья = ДанныеСырья(ЭлементДанных.product);
							Если КонструкторИСМПКлиентСервер.ВидыПродукцииПоИдентификаторуТипаСырья(ДанныеСырья.Тип).Количество() Тогда
								ВозвращаемоеЗначение.Сырье.Вставить(ЭлементДанных.product.id, ДанныеСырья);
							КонецЕсли;
						КонецЕсли;
						
					КонецЦикла;
					
					Если ДанныеОбработки.items.Количество() > 0 Тогда
						ПоследнийЭлемент = ДанныеОбработки.items[ДанныеОбработки.items.Количество() - 1];
						ПараметрыНавигации.Курсор = Строка(ПоследнийЭлемент.id);
					КонецЕсли;
					
				КонецЕсли;
				
				Если ДанныеОбработки.Свойство("nextPage") Тогда
					ПараметрыНавигации.ЕстьСледующаяСтраница = ДанныеОбработки.nextPage;
				КонецЕсли;
...
// Параметры:
// 	Организация  - ОпределяемыйТип.Организация                      - Организация.
// 	ВидПродукции - СправочникСсылка.ВидыПродукцииИС, Неопределено - ВидПродукции.
...
// Получить результат обработки квитанции УПД по идентификатору документа
// 
// Параметры:
// 	Организация                        - ОпределяемыйТип.Организация - Организация.
// 	ИдентификаторЭлектронногоДокумента - Строка                      - идентификатор УПД
// Возвращаемое значение:
// 	Структура - Описание:
// 	* ТребуетсяОбновлениеКлючаСессии - Булево - Признак необходимости обновления ключа сессии.
// 	* РезультатОтправкиЗапроса - Структура - (См. ОбменДаннымиИСМПКлиентСервер.ОбработатьРезультатОтправкиHTTPЗапросаКакJSON)
// 	* Статус             - ПеречислениеСсылка.СтатусыДокументовИСМП - Статус документа ИС МП
// 	* СтатусОбработки    - ПеречислениеСсылка.СтатусыОбработкиСообщенийИСМП - Статус обработки сообщения ИС МП
// 	* СписокОпераций     - Массив из Структура - 
// 	* ТекстОшибки        - Строка - Текст ошибки.
// 	* ОшибкаСервиса      - Булево - Истина, если при проверке в ГИС МТ возникла ошибка сервиса.
	...
	Если РезультатОтправкиЗапроса.ОтветПолучен Тогда
		...
		Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
			...
			Иначе
				...
				СтатусПоДаннымГИСМТ = ДанныеОбработки.Получить("state");
				
				Если СтатусПоДаннымГИСМТ = "SUCCESS" Тогда
					Статус = Перечисления.СтатусыДокументовИСМП.Проверен;
				ИначеЕсли СтатусПоДаннымГИСМТ = "FAILED" Тогда
					Статус = Перечисления.СтатусыДокументовИСМП.ЕстьОшибки;
				ИначеЕсли СтатусПоДаннымГИСМТ = "IN_PROGRESS" Тогда
					Статус = Перечисления.СтатусыДокументовИСМП.Обрабатывается;
				Иначе
					ВызватьИсключение СтрШаблон(НСтр("ru = 'Неизвестный статус документа: %1'"), СтатусПоДаннымГИСМТ);
				КонецЕсли;
				
				Если Статус = Перечисления.СтатусыДокументовИСМП.ЕстьОшибки Тогда
					
					СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаОтклонена;
					ОперацииДокумента = ДанныеОбработки.Получить("operations");
					
					Для Каждого ОперацияОбработки Из ОперацииДокумента Цикл
						
						ДеталиОперации = ОперацияОбработки.Получить("details");
						
						Если ДеталиОперации = Неопределено Тогда
							Продолжить;
						КонецЕсли;
						
						ОшибкиОперации = ДеталиОперации.Получить("errors");
						
						Если ОшибкиОперации = Неопределено Тогда
							Продолжить;
						КонецЕсли;
						
						Для Каждого ОписаниеОшибки Из ОшибкиОперации Цикл
							
							КодОшибки           = ОписаниеОшибки.Получить("code");
							ТекстОписанияОшибки = ОписаниеОшибки.Получить("text");
							
							Если ТекстОписанияОшибки = Неопределено Тогда
								
								РазвернутоеОписаниеОшибки = ОписаниеОшибки.Получить("error");
								ТекстОписанияОшибки       = РазвернутоеОписаниеОшибки.Получить("details");
								
								Если ТекстОписанияОшибки = Неопределено Тогда
									
									ИдентификаторыОшибок = "";
									
									Для Каждого СтрокаИдентификаторовОшибок Из РазвернутоеОписаниеОшибки Цикл
									
										ИдентификаторыОшибок = ИдентификаторыОшибок + ?(ЗначениеЗаполнено(ИдентификаторыОшибок), ", ", "")
											+ СтрокаИдентификаторовОшибок.Ключ;
											
									КонецЦикла;
									
									ТекстОписанияОшибки = СтрШаблон(НСтр("ru = 'Код ошибки - %1, идентификатор ошибки - %2.
										|
										|Подробную расшифровку ошибки можно получить на сайте Честного знака в методических рекомендациях:
										|
										|https://честныйзнак.рф/business/doc/?id=Методические_рекомендации_по_оформлению_документов_ЭДО.html'"),
										КодОшибки,
										ИдентификаторыОшибок);
							...
							ВозвращаемоеЗначение.ТекстОшибки = ВозвращаемоеЗначение.ТекстОшибки + ?(ЗначениеЗаполнено(ВозвращаемоеЗначение.ТекстОшибки), Символы.ПС, "")
								+ ТекстОписанияОшибки;
...
// Выполняет постраничный поиск мест осуществления деятельности в системе ИСМП с использованием фильтрации.
//
// Параметры:
//   Организация - СправочникСсылка.Организации - организация, для которой выполняется поиск
//   ПараметрыНавигации - Структура:
//     * КоличествоЗаписей - Число - количество записей на странице
//     * Курсор - Строка - курсор для навигации по страницам
//     * ЕстьСледующаяСтраница - Булево - признак наличия следующей страницы (заполняется функцией)
//   ПараметрыЗапроса - Структура:
//     * ИННУчастника - Массив из Строка - массив ИНН участников для фильтрации
//     * ОтборПоМОД - Массив из Строка - массив идентификаторов МОД для фильтрации
//     * ОтборПоФИАС - Массив из Строка - массив идентификаторов ФИАС для фильтрации
//     * ТоварныеГруппы - Массив из СправочникСсылка.ТоварныеГруппыГИСМТ - массив товарных групп для поиска
//
// Возвращаемое значение:
//   Структура:
//     * ТребуетсяОбновлениеКлючаСессии - Булево - если Истина, требуется обновление ключа сессии
//     * РезультатОтправкиЗапроса - Структура, Неопределено - подробный результат отправки HTTP-запроса
//     * ТекстОшибки - Строка - текст ошибки при неудачном выполнении запроса
//     * МестаОсуществленияДеятельности - Массив из Структура - массив найденных мест осуществления деятельности
Функция МестаОсуществленияДеятельностиСырьеПостранично(Организация, ПараметрыНавигации, ПараметрыЗапроса) Экспорт
	...
	Если ПараметрыЗапроса.Свойство("ИННУчастника") И ПараметрыЗапроса.ИННУчастника.Количество() Тогда
			Фильтр = ФильтрЗапросаМестОсуществленияДеятельностиСырье();

			Фильтр.operation = "equals";
			Фильтр.values    = ПараметрыЗапроса.ИННУчастника;
			Фильтр.attribute = "participantInn";

			Фильтры.Добавить(Фильтр);

	КонецЕсли;

	Если ПараметрыЗапроса.Свойство("ОтборПоМОД") И ПараметрыЗапроса.ОтборПоМОД.Количество() Тогда
		Фильтр = ФильтрЗапросаМестОсуществленияДеятельностиСырье();

		Фильтр.operation = "equals";
		Фильтр.values    = ПараметрыЗапроса.ОтборПоМОД;
		Фильтр.attribute = "modId";

		Фильтры.Добавить(Фильтр);

	КонецЕсли;

	Если ПараметрыЗапроса.Свойство("ОтборПоФИАС") И ПараметрыЗапроса.ОтборПоФИАС.Количество() Тогда
		Фильтр = ФильтрЗапросаМестОсуществленияДеятельностиСырье();

		Фильтр.operation = "equals";
		Фильтр.values    = ПараметрыЗапроса.ОтборПоФИАС;
		Фильтр.attribute = "fiasId";
	...
	Для Каждого ТоварнаяГруппа Из ПараметрыЗапроса.ТоварныеГруппы Цикл
	...
	Для Каждого ТоварнаяГруппаИмя Из МассивИменТоварныхГрупп Цикл
		ПараметрыСтрокиЗапроса.Добавить(СтрШаблон("pg=%1", ТоварнаяГруппаИмя));
	КонецЦикла;
	
	URLЗапроса = СтрШаблон("api/v3/true-api/sim/mods/search%1", ПараметрыЗапроса(ПараметрыСтрокиЗапроса));
			...
			Иначе
				...
				Если ДанныеОбработки.Свойство("items") Тогда
					...
					Для Каждого ЭлементДанных Из ДанныеОбработки.items Цикл
						...
						СтрокаРезультат.Вставить("Адрес",                      ЭлементДанных.address);
						СтрокаРезультат.Вставить("ПредставлениеВидыПродукции", "");
						СтрокаРезультат.Вставить("ИНН",                        ЭлементДанных.participantInn);
						СтрокаРезультат.Вставить("КПП",                        "");
						СтрокаРезультат.Вставить("ТоварныеГруппы",             МассивИменТоварныхГрупп);
						СтрокаРезультат.Вставить("ИдентификаторМОД",           ЭлементДанных.id);
...
Функция ДанныеСырья(ЭлементДанных)
	...
	Если ЭлементДанных.Свойство("id") Тогда
		Результат.Идентификатор = ЭлементДанных.id;
	КонецЕсли;
	
	Если ЭлементДанных.Свойство("type") Тогда
		Результат.Тип = Формат(ЭлементДанных.type, "ЧГ=;");
	КонецЕсли;
	
	Если ЭлементДанных.Свойство("name") Тогда
		Результат.Наименование = ЭлементДанных.name;
	КонецЕсли;
...
// Создает структуру фильтра запроса мест осуществления деятельности сырья.
//
// Возвращаемое значение:
//   Структура:
//     * operation - Строка - операция фильтра
//     * values - Массив - значения фильтра
//     * attribute - Строка - атрибуты фильтра
Функция ФильтрЗапросаМестОсуществленияДеятельностиСырье()
	
	Фильтр = Новый Структура;
	Фильтр.Вставить("operation", "");
	Фильтр.Вставить("values",    Новый Массив);
	Фильтр.Вставить("attribute", "");
