...
// Формирует временную таблицу настроек учета НДС контрагентов в менеджере временных таблиц.
//
// Параметры:
//	МенеджерВТ - МенеджерВременныхТаблиц - менеджер временных таблица, в которую будет помещен результат получения настроек.
//	Период - Дата - период, на который выполняется получение данных настроек.
//
Процедура СформироватьВТНастройкиУчетаНДСКонтрагентов(МенеджерВТ, Знач Период) Экспорт
	
	ТекстЗапросаУничтоженияВТ = "";
	
	Если МенеджерВТ.Таблицы.Найти("ВТНастройкиУчетаНДСКонтрагентов") <> Неопределено Тогда
		ТекстЗапросаУничтоженияВТ =
	...
	ТекстЗапросаПоУмолчанию = ТекстЗапросаУничтоженияВТ +
	"ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)				КАК Комитент,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)	КАК ОсновноеНалогообложение,
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетНДС") Тогда
		ТекстЗапросаВТ = УчетНДСЛокализация.ТекстЗапросаВТНастройкиУчетаНДСКонтрагентов();
		
		Если ПустаяСтрока(ТекстЗапросаВТ) Тогда
			ТекстЗапроса = ТекстЗапросаПоУмолчанию;
		Иначе
			ТекстЗапроса = ТекстЗапросаУничтоженияВТ + ТекстЗапросаВТ;
...
// Формирует временную таблицу настроек учета НДС организаций в менеджере временных таблиц.
//
// Параметры:
//	МенеджерВТ - МенеджерВременныхТаблиц - менеджер временных таблица, в которую будет помещен результат получения настроек.
//	Период - Дата - период, на который выполняется получение данных настроек.
//	ОрганизацииБезНДС - Массив Из СправочникСсылка.Организации - организации не использующие учет НДС.
//	ПолучитьРазрешенные - Булево - признак получения разрешенных по правам доступа записей настроек.
//
Процедура СформироватьВТНастройкиУчетаНДСОрганизаций(МенеджерВТ, Знач Период, ОрганизацииБезНДС, ПолучитьРазрешенные = Ложь) Экспорт
	
	ТекстЗапросаУничтоженияВТ = "";
	
	Если МенеджерВТ.Таблицы.Найти("ВТНастройкиУчетаНДСОрганизаций") <> Неопределено Тогда
		ТекстЗапросаУничтоженияВТ =
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетНДС") Тогда
		ТекстЗапросаВТ = УчетНДСЛокализация.ТекстЗапросаВТНастройкиУчетаНДСОрганизаций();
		
		Если ПустаяСтрока(ТекстЗапросаВТ) Тогда
			ТекстЗапроса = ТекстЗапросаПоУмолчанию;
		Иначе
			ТекстЗапроса = ТекстЗапросаУничтоженияВТ + ТекстЗапросаВТ;
			ТекстЗапроса = ?(Не ПолучитьРазрешенные,
...
//   ИсходящийДокумент - Булево - для налогообложения Облагается НДС у покупателей, для Исходящий документов ставку НДС исправлять не нужно.
//   
// Возвращаемое значение:
//   СправочникСсылка.СтавкиНДС - Ставка НДС по номенклатуре и типу налогообложения
//
Функция СтавкаНДСПоНоменклатуреИНалогообложению(Номенклатура, НалогообложениеНДС, Организация, Дата, УказаннаяСтавкаНДС = Неопределено, ИсходящийДокумент = Ложь) Экспорт
	...
	Параметры = ПараметрыПолученияСтавкиНДС(Номенклатура, НалогообложениеНДС, Организация, Дата, УказаннаяСтавкаНДС, СтавкаНДС);
	Параметры.ИсходящийДокумент = ИсходящийДокумент;
	
	УчетНДСЛокализация.ЗаполнитьСтавкуНДСПоНоменклатуреИНалогообложению(Параметры, СтавкаНДС);
	
	Возврат СтавкаНДС;

КонецФункции

// Параметры получения ставки НДС.
// 
// Параметры:
...
//  Дата - Дата
//  УказаннаяСтавкаНДС - СправочникСсылка.СтавкиНДС, Неопределено - передается ставка НДС 5% или 7% 
...
//  * Организация - СправочникСсылка.Организации 
//  * Дата - Дата 
//  * УказаннаяСтавкаНДС - СправочникСсылка.СтавкиНДС, Неопределено - Указанная ставка НДС
//  * СтавкаНДС -  СправочникСсылка.СтавкиНДС - Ставка НДС
//	* ИсходящийДокумент - булево
Функция ПараметрыПолученияСтавкиНДС(Номенклатура, НалогообложениеНДС, Организация, Дата, УказаннаяСтавкаНДС, СтавкаНДС) Экспорт
	
	Параметры = Новый Структура();
	Параметры.Вставить("Номенклатура", Номенклатура);
	Параметры.Вставить("НалогообложениеНДС", НалогообложениеНДС);
	Параметры.Вставить("Организация", Организация);
	Параметры.Вставить("Дата", Дата);
	Параметры.Вставить("СтавкаНДС", СтавкаНДС);
	Параметры.Вставить("УказаннаяСтавкаНДС", УказаннаяСтавкаНДС);
	Параметры.Вставить("ИсходящийДокумент", Ложь);
...
//           Если дата пустая, то будут получена ставка НДС на текущую дату.
// 
// Возвращаемое значение:
...
Функция СтавкаНДСНоменклатуры(Номенклатура, Организация, Дата) Экспорт
	...
	СтавкиНДС = СтавкиНДСПоМассивуНоменклатуры(МассивНоменклатуры, Организация, Дата);
	Возврат СтавкиНДС[Номенклатура];
	
КонецФункции

// Возвращает соответствие ставок НДС для массива номенклатуры.
//
// Параметры:
//  МассивНоменклатуры - Массив из СправочникСсылка.Номенклатура - МассивНоменклатуры, для которых необходимо получить ставки НДС.
...
//           Если дата пустая, то будут получены ставки НДС на текущую дату.
//
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//   * Ключ - СправочникСсылка.Номенклатура
//   * Значение - СправочникСсылка.СтавкиНДС
//
Функция СтавкиНДСПоМассивуНоменклатуры(МассивНоменклатуры, Организация, Дата) Экспорт
	...
	ДатаЗапроса = ?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДатаСеанса());
	СтранаОрганизации = ЗначениеНастроекКлиентСерверПовтИсп.СтранаРегистрацииОрганизации(Организация);
	
	Запрос.УстановитьПараметр("Дата", ДатаЗапроса);
	Запрос.УстановитьПараметр("Номенклатуры", МассивНоменклатуры);
	Запрос.УстановитьПараметр("Страна", СтранаОрганизации);
	Запрос.УстановитьПараметр("ПустаяСтрана", Справочники.СтраныМира.ПустаяСсылка());
	
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	
	ВыборкаСоСтраной = РезультатЗапроса[0].Выбрать();
	Пока ВыборкаСоСтраной.Следующий() Цикл
		Результат.Вставить(ВыборкаСоСтраной.Номенклатура, ВыборкаСоСтраной.СтавкаНДС);
	КонецЦикла;
	
	ВыборкаБезСтраны = РезультатЗапроса[1].Выбрать();
	Пока ВыборкаБезСтраны.Следующий() Цикл
		Если Результат.Получить(ВыборкаБезСтраны.Номенклатура) = Неопределено Тогда
			Результат.Вставить(ВыборкаБезСтраны.Номенклатура, ВыборкаБезСтраны.СтавкаНДС);
		КонецЕсли;
	КонецЦикла;
	
	ОсновнаяСтавкаНДС = Справочники.СтавкиНДС.ПустаяСсылка();
	
	ВыборкаОсновныхСтавок = РезультатЗапроса[2].Выбрать();
	Если ВыборкаОсновныхСтавок.Следующий() Тогда
		ОсновнаяСтавкаНДС = ВыборкаОсновныхСтавок.СтавкаНДС;
	КонецЕсли;
	
	Для Каждого ЭлементНоменклатуры Из МассивНоменклатуры Цикл
		Если Результат.Получить(ЭлементНоменклатуры) = Неопределено Тогда
			Результат.Вставить(ЭлементНоменклатуры, ОсновнаяСтавкаНДС);
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьСтавкуНДСДляПлатежей(СтавкаНДС, Знач ТипНалогообложенияНДС, Организация, Дата) Экспорт
	...
	Если НЕ СтавкаНДСКорректна(СтавкаНДС, ТипНалогообложенияНДС, Организация, Дата) Тогда
		...
		НоваяСтавкаНДС = СтавкаНДСПоУмолчанию(Организация, Дата);
...
Процедура ПолучитьДанныеВыбораСтавкиНДС(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт 
	...
	|ГДЕ
	|	НЕ СтавкиНДСТипыНалогообложенияНДС.Ссылка.ПометкаУдаления
...
//   НалогообложениеНДС - ПеречислениеСсылка.ТипыНалогообложенияНДС - Налогообложение НДС операции.
//   Дата - Дата - Используется для получения ставки НДС по организации на определенную дату
//   КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения.
//   УказаннаяСтавкаНДС - СправочникСсылка.СтавкиНДС
//   
// Возвращаемое значение:
...
//
Функция СтавкаНДСПоНоменклатуреИНалогообложениюПакетно(Номенклатура, НалогообложениеНДС, Дата, КэшированныеЗначения, УказаннаяСтавкаНДС = Неопределено) Экспорт
	
	СтавкаНДС = Неопределено;
	ОрганизацияДляСтавкиНДС = Неопределено;
	...
	КэшированныеЗначения.Свойство("ОрганизацияДляСтавкиНДС", ОрганизацияДляСтавкиНДС);
	Если ОрганизацияДляСтавкиНДС = Неопределено Тогда
		ОрганизацияДляСтавкиНДС = Справочники.Организации.ПустаяСсылка();
	КонецЕсли;
	
	Параметры = ПараметрыПолученияСтавкиНДС(Номенклатура, НалогообложениеНДС, ОрганизацияДляСтавкиНДС, Дата, УказаннаяСтавкаНДС, СтавкаНДС);
	Параметры.ИсходящийДокумент = КэшированныеЗначения.ИсходящийДокумент;
	
	УчетНДСЛокализация.ЗаполнитьСтавкуНДСПоНоменклатуреИНалогообложениюПакетно(Параметры, КэшированныеЗначения, СтавкаНДС);
...
Функция СтавкаНДСПоТипуНалогообложенияПакетно(ТипНалогообложенияНДС, КэшированныеЗначения) Экспорт
	...
	Если ТипНалогообложенияНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС Тогда
		ОсновныеСтавкиПоДате = ПакетнаяОбработкаТабличнойЧастиСервер.ДанныеДляОбработкиСтроки(
			"ОсновныеСтавкиПоДате", КэшированныеЗначения);
			
		Если ОсновныеСтавкиПоДате <> Неопределено Тогда
	...
	Если ЗначениеЗаполнено(ТипНалогообложенияНДС)
		И ЗначениеЗаполнено(КэшированныеЗначения.ДанныеДляОбработки)
		И КэшированныеЗначения.ДанныеДляОбработки.Свойство("СтавкаНДСПоТипуНалогообложенияНаДату") Тогда
		
		СтавкаНДСПоТипуНалогообложенияНаДату = 
			ПакетнаяОбработкаТабличнойЧастиСервер.ДанныеДляОбработкиСтроки(
				"СтавкаНДСПоТипуНалогообложенияНаДату", КэшированныеЗначения);
		
		Если СтавкаНДСПоТипуНалогообложенияНаДату <> Неопределено Тогда
			СтавкаНДСПриоритет3 = СтавкаНДСПоТипуНалогообложенияНаДату.НайтиСтроки(Новый Структура("Приоритет", 3));
			
			Если СтавкаНДСПоТипуНалогообложенияНаДату.Количество() - СтавкаНДСПриоритет3.Количество() = 1 Тогда
...
Функция СтавкаНДСПоУмолчаниюПакетно(КэшированныеЗначения, Знач РасчетнаяСтавка = Ложь, Знач НалогообложениеНДС = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(СтавкаНДС) Тогда
		СтавкаНДС = СтавкаНДСПоТипуНалогообложенияПакетно(Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС,
...
// Возвращает ставку НДС по виду цены для текущей обрабатываемой строки табличной части в механизмах пакетной обработки строк
// табличных частей. 
//
// Параметры:
//   КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения.
//   
// Возвращаемое значение:
//   Неопределено - не найдены данные.
//   Структура - структура данных строки по виду цен.
//
Функция СтавкаНДСПоВидуЦенПакетно(КэшированныеЗначения) Экспорт
	
	СтруктураДанных = Неопределено;
	ДанныеДляОбработки = ПакетнаяОбработкаТабличнойЧастиСервер.ДанныеДляОбработкиСтроки("СтавкиНДСПоВидамЦенНоменклатуры", КэшированныеЗначения); 
	Если	Не ДанныеДляОбработки = Неопределено 
			И ДанныеДляОбработки.Количество() Тогда
		СтруктураДанных = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(ДанныеДляОбработки[0]);
	КонецЕсли;
...
// Возвращает ставку НДС по настройке Комитента для текущей обрабатываемой строки табличной части
// в механизмах пакетной обработки строк табличных частей. 
//
// Параметры:
//   КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения.
//   
// Возвращаемое значение:
//   Неопределено - не найдены данные.
//   Структура - структура данных строки по настройке комитента.
//
Функция СтавкаНДСПоПоНастройкеУчетаНДСПриУСНКомитентовПакетно(КэшированныеЗначения) Экспорт
	
	СтруктураДанных = Неопределено;
	ДанныеДляОбработки = ПакетнаяОбработкаТабличнойЧастиСервер.ДанныеДляОбработкиСтроки("СтавкиНДСПоНастройкеУчетаНДСПриУСНКомитентов", КэшированныеЗначения); 
	Если	Не ДанныеДляОбработки = Неопределено 
			И ДанныеДляОбработки.Количество() Тогда
		СтруктураДанных = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(ДанныеДляОбработки[0]);
	КонецЕсли;
...
// Устанавливает ставку НДС в указанной колонке текущей строки, если значение отличается от переданного.
//
// Параметры:
//   ТекущаяСтрока - СтрокаТаблицыЗначений - Строка, в которой устанавливается ставка НДС
//   СтавкаНДС - Строка - Устанавливаемое значение ставки НДС
//   ИмяКолонкиСтавкиНДС - Строка - Имя колонки для установки ставки НДС
//   КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения.
//
Процедура ЗаполнитьСтавкуНДСТекущейСтрокиПакетно(ТекущаяСтрока, СтавкаНДС, ИмяКолонкиСтавкиНДС, КэшированныеЗначения) Экспорт

	Если ТекущаяСтрока[ИмяКолонкиСтавкиНДС] <> СтавкаНДС Тогда
		
		ТекущаяСтрока[ИмяКолонкиСтавкиНДС] = СтавкаНДС;
...
Процедура ДополнитьТекстЗапросаСтавкаНДСПоСпискуНоменклатуры(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
			"ЗаполнитьСтавкуНДС, ЗаполнитьСтавкуНДССуффикс, СкорректироватьСтавкуНДС,
			|ЗаполнитьСтавкуНДСПоВидуЦен, ЗаполнитьСтавкуНДССуффиксПоВидуЦен, ЗаполнитьСтавкуНДСПоНастройкеУчетаПриУСНКомитента",
			СтруктураДействий,
			КэшированныеЗначения,
		...
		Если СтруктураДействий.Свойство("СкорректироватьСтавкуНДС") Тогда
			...
			Если ВернутьМногооборотнуюТару 
				И Не ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных
					("ТипНоменклатуры", КэшированныеЗначения) Тогда
						ТекстЗапроса =
							"ВЫБРАТЬ
						...
						ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ТипыНоменклатуры_Тара");
			КонецЕсли;
		КонецЕсли;
		
		ТипНалогообложенияНДС = СтруктураПараметровДействия.НалогообложениеНДС;
			
		Если Не ПродажиСервер.ХозяйственнаяОперацияСоСтавкойБезНДС(ХозяйственнаяОперация)
			Или ТипНалогообложенияНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС Тогда
			...
			НалогообложениеВТЧ = СтруктураПараметровДействия.Свойство("НалогообложениеНДСВТабличнойЧасти")
				И СтруктураПараметровДействия.НалогообложениеНДСВТабличнойЧасти;
			...
			Если ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("Номенклатура", КэшированныеЗначения) Тогда
				
				ТекстЗапроса 	= 
				"ВЫБРАТЬ
				|	Товары.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
				|	Товары.СтавкаНДС КАК СтавкаНДС,
				|	&ТипНалогообложенияНДС КАК НалогообложениеНДС,
				...
				Если НалогообложениеВТЧ Тогда
					ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТипНалогообложенияНДС", "Товары.НалогообложениеНДС"); //@Query-part-2
				Иначе
			...
			Иначе
				...
				НалогообложениеВТЧ = СтруктураПараметровДействия.НалогообложениеНДСВТабличнойЧасти;
				ТекстЗапроса =
				"ВЫБРАТЬ
				|	Товары.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
				|	Товары.СтавкаНДС КАК СтавкаНДС,
			...
			Если НЕ (ТипНалогообложенияНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС
				ИЛИ ТипНалогообложенияНДС = Перечисления.ТипыНалогообложенияНДС.РеверсивноеОбложениеНДС)
				ИЛИ НалогообложениеВТЧ Тогда
			...
			// Актуальность ставки НДС по дате и по типу налогообложения
			
			ТекстЗапроса = "
				|ВЫБРАТЬ РАЗЛИЧНЫЕ
				|	Товары.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
				|	ВЫБОР
				|		КОГДА СтавкиНДСТипыНалогообложенияНДС.Ссылка ЕСТЬ NULL
				|			ТОГДА ЛОЖЬ
				|		ИНАЧЕ ИСТИНА
				|	КОНЕЦ КАК Актуальна
				|ИЗ
			...
			ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "АктуальностьСтавкиНДС");
		...
		Иначе
			...
			ТекстЗапроса = "
				...
				|	Товары.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
				|	ВЫБОР
				|		КОГДА Товары.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
				|			ТОГДА ИСТИНА
				|		ИНАЧЕ ЛОЖЬ
				|	КОНЕЦ КАК Актуальна
				|ИЗ
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
			...
			Если СтруктураПараметровДействия <> Неопределено 
				И СтруктураПараметровДействия.Свойство("СтавкаНДС", СтавкаНДС) Тогда
					...
					|	ВтИсточникДанных КАК Товары
					|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтавкиНДС КАК СтавкиНДС
					|	ПО &СтавкаНДСПараметрыПересчета = СтавкиНДС.Ссылка
					|";
					
					ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "СтавкаПоСтавкеНДС");
					ОписаниеЗапроса.ПараметрыЗапроса.Вставить("СтавкаНДСПараметрыПересчета",
						СтавкаНДС);
...
Процедура ДополнитьТекстЗапросаСтавкаПоСтавкеНДССуффикс(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
			...
			Если СтруктураПараметровДействия <> Неопределено 
				И СтруктураПараметровДействия.Свойство("СтавкаНДС", СтавкаНДС) 
				И ЗначениеЗаполнено(СтавкаНДС) Тогда
...
// Добавляет запрос в пакет запросов для получения данных, необходимых для заполнения поля СтавкаНДС по данным
// из выбранного Вида цен и регистра сведений "Цены номенклатуры 2.5" при обработке строк табличной части.
//
// Параметры:
//  СтруктураДействий - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ.СтруктураДействий
//  ОписаниеЗапроса - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОписаниеЗапроса
//  КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения
//
Процедура ДополнитьТекстЗапросаСтавкаНДСПоВидуЦен(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	
	Перем СтруктураПараметровДействия;
	
	ТребуетсяВыполнитьДействия = ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		"ЗаполнитьСтавкуНДСПоВидуЦен,ЗаполнитьСтавкуНДССуффиксПоВидуЦен",
		СтруктураДействий,
		КэшированныеЗначения,
	...
	Если ТребуетсяВыполнитьДействия Тогда
		...
		Если СтруктураПараметровДействия.ИспользуетсяЦенообразование25 Тогда
			
			Если НЕ ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("ВидЦены", КэшированныеЗначения) Тогда
				
				СтруктураПараметровДействия.ЗаполнитьВидЦены = Истина;
			...
			Если НЕ ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("Серия", КэшированныеЗначения) Тогда
				ПакетнаяОбработкаТабличнойЧастиСервер.ДополнитьКолонкиИсточникаДанных(
					ОписаниеЗапроса,
					"Серия",
			...
			Если НЕ ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("Упаковка", КэшированныеЗначения) Тогда
				ПакетнаяОбработкаТабличнойЧастиСервер.ДополнитьКолонкиИсточникаДанных(
					ОписаниеЗапроса,
					"Упаковка",
			...
			НалогообложениеНДСЭкспорт = УчетНДСУПСлужебный.НалогообложенияНДСПродажиНаЭкспорт();
			ПродажаНаЭкспорт = ЗначениеЗаполнено(НалогообложениеНДСЭкспорт.Найти(СтруктураПараметровДействия.НалогообложениеНДС));
			ОписаниеЗапроса.ПараметрыЗапроса.Вставить("ПродажаНаЭкспорт", ПродажаНаЭкспорт);
			...
			|	ТаблицаТовары.Упаковка КАК Упаковка,
			|	ЕСТЬNULL(ВЫБОР
			|		КОГДА ТаблицаВидовНоменклатуры.НастройкиКлючаЦенПоХарактеристике = ЗНАЧЕНИЕ(Перечисление.ВариантОтбораДляКлючаЦен.НеИспользовать)
			...
			|	ЕСТЬNULL(ВЫБОР
			|		КОГДА ТаблицаВидовНоменклатуры.НастройкиКлючаЦенПоСерии = ЗНАЧЕНИЕ(Перечисление.ВариантОтбораДляКлючаЦен.НеИспользовать)
			...
			|	ВЫБОР
			|		КОГДА ТаблицаВидовНоменклатуры.НастройкиКлючаЦенПоУпаковке = ЗНАЧЕНИЕ(Перечисление.ВариантОтбораДляКлючаЦен.НеИспользовать)
			|				ИЛИ (ТаблицаВидовНоменклатуры.НастройкиКлючаЦенПоУпаковке = ЗНАЧЕНИЕ(Перечисление.ВариантОтбораДляКлючаЦен.Использовать)
			|					И ТаблицаУпаковкиЕИ.Владелец = ЗНАЧЕНИЕ(Справочник.НаборыУпаковок.БазовыеЕдиницыИзмерения))
			|			ТОГДА ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
			|		ИНАЧЕ ТаблицаТовары.Упаковка
			|	КОНЕЦ КАК УпаковкаЦО,
			|	ЕСТЬNULL(ВЫБОР
			|		КОГДА ТаблицаВидыЦен.ВариантУказанияСтавокНДС = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСтавокНДСВПрайсЛисте.ПустаяСсылка)
			|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСтавокНДСВПрайсЛисте.НеУказыватьСтавкиНДС)
			|		ИНАЧЕ ТаблицаВидыЦен.ВариантУказанияСтавокНДС
			|	КОНЕЦ, ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСтавокНДСВПрайсЛисте.НеУказыватьСтавкиНДС)) КАК ВариантУказанияСтавокНДС,
			|	ТаблицаВидыЦен.СтавкаНДС КАК ВидЦеныСтавкаНДС,
			|	ТаблицаВидыЦен.Страна КАК Страна
			...
			|ПО
			|	ТаблицаТовары.Характеристика = ТаблицаХарактеристик.Ссылка
			|ЛЕВОЕ СОЕДИНЕНИЕ
			|	Справочник.СерииНоменклатуры КАК ТаблицаСерий
			|ПО
			|	ТаблицаТовары.Серия = ТаблицаСерий.Ссылка
			|ЛЕВОЕ СОЕДИНЕНИЕ
			|	Справочник.ВидыНоменклатуры КАК ТаблицаВидовНоменклатуры
			|ПО
			|	ТаблицаНоменклатуры.ВидНоменклатуры = ТаблицаВидовНоменклатуры.Ссылка
			|ЛЕВОЕ СОЕДИНЕНИЕ
			|	Справочник.ВидыЦен КАК ТаблицаВидыЦен
			|ПО
			|	ТаблицаТовары.ВидЦены = ТаблицаВидыЦен.Ссылка
			|ЛЕВОЕ СОЕДИНЕНИЕ
			|	Справочник.УпаковкиЕдиницыИзмерения КАК ТаблицаУпаковкиЕИ
			|ПО
			|	ТаблицаТовары.Упаковка = ТаблицаУпаковкиЕИ.Ссылка
			|ИНДЕКСИРОВАТЬ ПО
			|	Номенклатура,
			|	ХарактеристикаЦО,
			|	СерияЦО,
			...
			ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ТаблицаОтбора");
			...
			ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ЦеныНоменклатурыСрезПоследних");
			...
			|	ТаблицаОтбора.ХарактеристикаЗаполнена КАК ХарактеристикаЗаполнена,
			|	ТаблицаОтбора.Серия КАК Серия,
			|	ТаблицаОтбора.ВариантУказанияСтавокНДС КАК ВариантУказанияСтавокНДС,
			|	ВЫБОР
			|		КОГДА ТаблицаОтбора.ВариантУказанияСтавокНДС = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСтавокНДСВПрайсЛисте.УказыватьОбщуюСтавкуНДС)
			|				ИЛИ ТаблицаОтбора.ВариантУказанияСтавокНДС = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСтавокНДСВПрайсЛисте.УказыватьСтавкиНДСПострочно)
			|			ТОГДА ИСТИНА
			|		ИНАЧЕ ЛОЖЬ
			|	КОНЕЦ КАК УказыватьСтавкуНДС,
			|	ТаблицаОтбора.ВидЦеныСтавкаНДС КАК ВидЦеныСтавкаНДС,
			|	ТаблицаОтбора.Страна КАК Страна,
			|	ЕСТЬNULL(ТаблицаЦен.СтавкаНДС, ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)) КАК СтавкаНДС,
			|	ЕСТЬNULL(ТаблицаСтавкиНДС.Ставка, 99999) КАК ЗначениеСтавки,
			|	ВЫБОР
			|		КОГДА ТаблицаСтавкиНДС.Ставка ЕСТЬ НЕ NULL
			|				И ((&ПродажаНаЭкспорт И ТаблицаСтавкиНДС.Ставка <> 0)
			|				ИЛИ (НЕ &ПродажаНаЭкспорт И ТаблицаСтавкиНДС.Ставка = 0))
			|			ТОГДА ИСТИНА
			|		ИНАЧЕ 
			|			ЛОЖЬ
			|	КОНЕЦ ОшибкаПродажиНаЭкспорт
			|ИЗ
			|	ТаблицаОтбора КАК ТаблицаОтбора
			|ЛЕВОЕ СОЕДИНЕНИЕ
			|	ЦеныНоменклатурыСрезПоследних КАК ТаблицаЦен
			|ПО
			|	ТаблицаОтбора.Номенклатура = ТаблицаЦен.Номенклатура
			|	И ТаблицаОтбора.ХарактеристикаЦО = ТаблицаЦен.ХарактеристикаЦО
			|	И ТаблицаОтбора.СерияЦО = ТаблицаЦен.СерияЦО
			|	И ТаблицаОтбора.УпаковкаЦО = ТаблицаЦен.УпаковкаЦО
			|	И ТаблицаОтбора.ВидЦены = ТаблицаЦен.ВидЦены
			|ЛЕВОЕ СОЕДИНЕНИЕ
			|	Справочник.СтавкиНДС КАК ТаблицаСтавкиНДС
			|ПО
			|	ТаблицаЦен.СтавкаНДС = ТаблицаСтавкиНДС.Ссылка
			|";
			
			ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "СтавкиНДСПоВидамЦенНоменклатуры");
...
//
// Параметры:
//  ОбрабатываемаяКоллекция - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ.КоллекцияСтрок
//  СтруктураДействий - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ.СтруктураДействий
//  КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения
//
Процедура ЗаполнитьКолонкуДатаПолученияСтавки(ОбрабатываемаяКоллекция, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		...
		Если СтруктураПараметровДействия.Свойство("ПоДатеИсправления")
			И СтруктураПараметровДействия.ПоДатеИсправления Тогда
			СтруктураПараметровДействия.Дата = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураПараметровДействия.ИсправляемыйДокумент, "Дата");
		КонецЕсли;
		
		Дата = ?(ЗначениеЗаполнено(СтруктураПараметровДействия.Дата), СтруктураПараметровДействия.Дата, ТекущаяДатаСеанса());
		
		ИспользоватьДатуПоступления = СтруктураПараметровДействия.Свойство("ПоДатеПоступления") 
			И СтруктураПараметровДействия.ПоДатеПоступления
			И ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("ДокументПоступления", КэшированныеЗначения);
		
		Если ИспользоватьДатуПоступления Тогда
			МассивПоступлений = Новый Массив();
			Для Каждого Стр Из ОбрабатываемаяКоллекция Цикл
				Если ЗначениеЗаполнено(Стр.ДокументПоступления) Тогда
					МассивПоступлений.Добавить(Стр.ДокументПоступления);
				КонецЕсли;
			КонецЦикла;
			ДатыПоступления = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(МассивПоступлений, "Дата"); 
		КонецЕсли;
		...
		ИспользоватьДатуРеализации = СтруктураПараметровДействия.Свойство("ПоДатеРеализации") 
			И СтруктураПараметровДействия.ПоДатеРеализации
			И ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("ДокументРеализации", КэшированныеЗначения);
		
		Если ИспользоватьДатуРеализации Тогда
			МассивРеализаций = Новый Массив();
			Для Каждого Стр Из ОбрабатываемаяКоллекция Цикл
				Если ЗначениеЗаполнено(Стр.ДокументРеализации) Тогда
					МассивРеализаций.Добавить(Стр.ДокументРеализации);
				КонецЕсли;
			КонецЦикла;
			ДатыРеализация = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(МассивРеализаций, "Дата"); 
		КонецЕсли;
		...
		Если Не ПродажиСервер.ХозяйственнаяОперацияСоСтавкойБезНДС(ХозяйственнаяОперация)
			Или СтруктураПараметровДействия.НалогообложениеНДС
				= Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС Тогда
					Для Каждого СтрокаКоллекции Из ОбрабатываемаяКоллекция Цикл
						Если ИспользоватьДатуОтгрузки И ЗначениеЗаполнено(СтрокаКоллекции.ДатаОтгрузки) Тогда
							СтрокаКоллекции.ДатаПолученияСтавки = СтрокаКоллекции.ДатаОтгрузки;
						ИначеЕсли ИспользоватьДатуРеализации И ЗначениеЗаполнено(СтрокаКоллекции.ДокументРеализации) Тогда
							СтрокаКоллекции.ДатаПолученияСтавки = ДатыРеализация[СтрокаКоллекции.ДокументРеализации].Дата;
						ИначеЕсли ИспользоватьДатуПоступления И ЗначениеЗаполнено(СтрокаКоллекции.ДокументПоступления) Тогда
							СтрокаКоллекции.ДатаПолученияСтавки = ДатыПоступления[СтрокаКоллекции.ДокументПоступления].Дата;
						Иначе
							СтрокаКоллекции.ДатаПолученияСтавки = Дата;
						КонецЕсли;
					КонецЦикла;
		КонецЕсли;
...
// Заполняет для формы "ФормаДокументаДоВводаОстатков" колонку ВидЦены в промежуточной таблице,
// сформированной на основе обрабатываемых строк табличной части в механизмах пакетной обработки строк табличных частей. 
//
// Параметры:
//  ОбрабатываемаяКоллекция - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ.КоллекцияСтрок
//  СтруктураДействий - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ.СтруктураДействий
//  КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения
//
Процедура ЗаполнитьКолонкуВидЦены(ОбрабатываемаяКоллекция, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		...
		Если СтруктураПараметровДействия.ЗаполнитьВидЦены Тогда
			
			ВидЦены = СтруктураПараметровДействия.ВидЦены;
			Если НЕ ЗначениеЗаполнено(ВидЦены) Тогда
				
				КэшированныеЗначения.Свойство("ИспользоватьСоглашенияСКлиентами", ИспользоватьСоглашенияСКлиентами);
				...
				РеквизитШапки = Неопределено;
				Если ЗначениеЗаполнено(СтруктураПараметровДействия.Партнер) Тогда
					РеквизитШапки = СтруктураПараметровДействия.Партнер;
				КонецЕсли;
			
				Если ЗначениеЗаполнено(СтруктураПараметровДействия.Соглашение) И ИспользоватьСоглашенияСКлиентами Тогда
					РеквизитШапки = СтруктураПараметровДействия.Соглашение;
				КонецЕсли;
			
				Если ЗначениеЗаполнено(РеквизитШапки) Тогда
				
					СтруктураДанных = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РеквизитШапки, "ИндивидуальныйВидЦены, ВидЦен");
					ВидЦены = СтруктураДанных.ВидЦен;
					Если ЗначениеЗаполнено(СтруктураДанных.ИндивидуальныйВидЦены) Тогда
						ВидЦены = СтруктураДанных.ИндивидуальныйВидЦены;
					КонецЕсли;
				КонецЕсли;
			...
			Если ЗначениеЗаполнено(ВидЦены) Тогда
				Для Каждого СтрокаКоллекции Из ОбрабатываемаяКоллекция Цикл
					СтрокаКоллекции.ВидЦены = ВидЦены;
				КонецЦикла;
			КонецЕсли;
...
// Процедура - заменяет ставку НДС в справочниках
// Параметры:
//  ПараметрыПолученияДанных - Структура - См. Обработки.ЗаменаСтавокНДС.ПараметрыПолученияДанных()
//
Процедура ЗаменитьСтавкуНДС(ПараметрыПолученияДанных) Экспорт
	...
	Запрос.УстановитьПараметр("Страна",        ПараметрыПолученияДанных.Страна);
	Запрос.УстановитьПараметр("ПустаяДата",    Дата(1,1,1));
	
	МассивТекстовЗапросов = Новый Массив;
	ИзменятьСтавкуВНоменклатуре = Ложь;
		
	Для Каждого Справочник Из ПараметрыПолученияДанных.СписокСправочников Цикл
		
		Если Справочник.Значение = "Номенклатура" Тогда
			
			ИзменятьСтавкуВНоменклатуре = Истина;
			
		Иначе
			
			СОтборомПоПериоду = Справочник.Значение <> "ВидыНоменклатуры";
			ТекстЗапросДляСправочников = ТекстЗапросаОбъектыДляЗамены(СОтборомПоПериоду);
			ТекстЗапросДляСправочников = СтрЗаменить(ТекстЗапросДляСправочников, "ДоговорыКонтрагентов", Справочник.Значение);
			МассивТекстовЗапросов.Добавить(ТекстЗапросДляСправочников); 
	...
	Если МассивТекстовЗапросов.Количество() Тогда
		...
		Пока Справочник.Следующий() Цикл
			...
			Если Не ЗначениеЗаполнено(Справочник.Ссылка) Тогда
				Продолжить;
			КонецЕсли;
			...
			Попытка
				...
				СправочникОбъект = Справочник.Ссылка.ПолучитьОбъект();
				ЭлементБлокировки = Справочник.Ссылка.Метаданные().ПолноеИмя();
				ЗаблокироватьИЗаписатьСправочник(СправочникОбъект, ПараметрыПолученияДанных.НоваяСтавка, ПараметрыПолученияДанных.ТекущаяСтавка, ЭлементБлокировки);
	...
	Если ИзменятьСтавкуВНоменклатуре Тогда
		
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	СтавкиНДСНоменклатурыСрезПоследних.Номенклатура КАК Ссылка
		|ИЗ
		|	РегистрСведений.СтавкиНДСНоменклатуры.СрезПоследних(&Период, Страна = &Страна) КАК СтавкиНДСНоменклатурыСрезПоследних
		|ГДЕ
		|	СтавкиНДСНоменклатурыСрезПоследних.СтавкаНДС = &ТекущаяСтавка
		|";
		
		ЗаписьРегистра = Запрос.Выполнить().Выбрать();
		
		Пока ЗаписьРегистра.Следующий() Цикл
			
			Если Не ЗначениеЗаполнено(ЗаписьРегистра.Ссылка) Тогда
				Продолжить;
			КонецЕсли;
			...
			Попытка
				...
				ЭлементБлокировки.УстановитьЗначение("Страна", ПараметрыПолученияДанных.Страна);
				ЭлементБлокировки.УстановитьЗначение("Номенклатура", ЗаписьРегистра.Ссылка);
				Блокировка.Заблокировать();
				
				НаборЗаписей = РегистрыСведений.СтавкиНДСНоменклатуры.СоздатьНаборЗаписей();
				НаборЗаписей.Отбор.Период.Установить(НачалоМесяца(ПараметрыПолученияДанных.Период));
				НаборЗаписей.Отбор.Номенклатура.Установить(ЗаписьРегистра.Ссылка);
				НаборЗаписей.Отбор.Страна.Установить(ПараметрыПолученияДанных.Страна);
				
				НоваяЗапись = НаборЗаписей.Добавить();
				НоваяЗапись.Период       = НачалоМесяца(ПараметрыПолученияДанных.Период);
				НоваяЗапись.Номенклатура = ЗаписьРегистра.Ссылка;
				НоваяЗапись.Страна       = ПараметрыПолученияДанных.Страна;
				НоваяЗапись.СтавкаНДС    = ПараметрыПолученияДанных.НоваяСтавка;
				
				НаборЗаписей.ДополнительныеСвойства.Вставить("ПропуститьАктуализациюСтавкиНДСНоменклатуры");
				НаборЗаписей.Записать();
				
				СправочникОбъект = ЗаписьРегистра.Ссылка.ПолучитьОбъект();
				ЗаблокироватьИЗаписатьСправочник(СправочникОбъект, ПараметрыПолученияДанных.НоваяСтавка, ПараметрыПолученияДанных.ТекущаяСтавка, "Справочник.Номенклатура");
			...
			Исключение
				...
				ОбновлениеИнформационнойБазыУТ.СообщитьОНеудачнойОбработке(ИнформацияОбОшибке(), ЗаписьРегистра.Ссылка);
...
Процедура ЗаполнитьНалогообложениеНДСЗакупки(НалогообложениеНДС, ПараметрыЗаполнения, КэшированныеЗначенияПараметров = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыЗаполнения.Договор) Тогда
		...
		ТипДоговора = ТипЗнч(ПараметрыЗаполнения.Договор);
		Если ТипДоговора = Тип("СправочникСсылка.ДоговорыМеждуОрганизациями") Тогда
...
//  ПутьКПолюНалогообложения - Строка - Путь к реквизиту формы, содержащему значение налогообложение.
//                                      если он отличается от "Объект.НалогообложениеНДС".
Процедура УстановитьУсловноеОформлениеСуммНДСПоНалогообложениюЗакупки(Форма,
	                                          ...
	                                          ПутьКПолюНалогообложения = "Объект.НалогообложениеНДС") Экспорт
	
	ЧастиПути = СтрРазделить(ПутьКПолюНалогообложения, ".");
	ЧастиПути.Удалить(ЧастиПути.ВГраница()); // Удаление имени реквизита.
	Родитель = ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(
		Форма,
		СтрСоединить(ЧастиПути, ".")); 
	
	НалогообложениеВТЧ = ТипЗнч(Родитель) = Тип("ДанныеФормыКоллекция");
	...
	СписокНалогообложений.ЗагрузитьЗначения(УчетНДСУПКлиентСервер.ТипыНалогообложенияСкрывающиеРеквизитыНДС(ИмяПоляВводаСтавкиНДС));
	
	ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ПутьКПолюНалогообложения);
	...
	УчетНДСЛокализация.ДополнитьУсловноеОформлениеСуммНДСПоНалогообложениюЗакупки(
		Форма, ИмяПоляВводаСтавкиНДС, ИмяПоляВводаСуммыНДС, ИмяПоляВводаСуммыСНДС, ПутьКПолюНалогообложения);
...
// Заполняет хозяйственную операцию в табличной части из соглашения или договора.
//
// Параметры:
//	ТекущаяСтрока - Структура - Строка табличной части, в которой выполняется заполнение налогообложения.
//	СтруктураДействий - Структура:
//		* ЗаполнитьНалогообложениеНДСЗакупки - см. УчетНДСУПКлиентСервер.ПараметрыПакетногоЗаполненияНалогообложенияНДСЗакупки
//	КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения.
//
Процедура ЗаполнитьПакетноНалогообложениеНДСЗакупки(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	ПараметрыДействия = СтруктураДействий.ЗаполнитьНалогообложениеНДСЗакупки;
	МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоСсылке(ПараметрыДействия.Документ);
	ПараметрыЗаполнения = МенеджерОбъекта.ПараметрыНалогообложенияНДС(ТекущаяСтрока, ПараметрыДействия);
	ЗаполнитьНалогообложениеНДСЗакупки(ТекущаяСтрока.НалогообложениеНДС, ПараметрыЗаполнения);
...
// Устанавливает параметры заполнения видов запасов при комиссионных продажах.
//
// Параметры:
...
Процедура УстановитьПараметрыЗаполненияВидовЗапасовПриКомиссионныхПродажах(ПараметрыЗаполнения) Экспорт
	...
	УчетНДСЛокализация.УстановитьПараметрыЗаполненияВидовЗапасовПриКомиссионныхПродажах(ПараметрыЗаполнения);
...
// Проверяет необходимость формирования счетов-фактур выданных по переданным параметрам регистрации.
//
// Параметры:
// 	ПараметрыРегистрации - См. УчетНДСУПКлиентСервер.ПараметрыРегистрацииСчетовФактурВыданных.
//
// Возвращаемое значение:
// 	Булево - Признак необходимости формирования счета-фактуры.
//
Функция СчетФактураВыданныйНеТребуется(ПараметрыРегистрации) Экспорт
	...
	//++ Локализация
	СчетФактураНеТребуется = УчетНДСРФ.СчетФактураВыданныйНеТребуется(ПараметрыРегистрации);
	//-- Локализация
...
Процедура ОтразитьВводОстатковСобственныхТоваров(Запрос, ТекстыЗапроса, Регистры, ТекстТовары) Экспорт
	...
	|	ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ВводОстатковСобственныхТоваров) КАК НастройкаХозяйственнойОперации
	|ИЗ
	|	#Товары КАК Товары
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
...
#Область НДСПредъявленный
	...
	|				И НЕ СпрВидыЗапасов.Ссылка ЕСТЬ NULL 
	|               И &ФормироватьВидыЗапасовПоГруппамФинансовогоУчета
	|			ТОГДА ЕСТЬNULL(СпрВидыЗапасов.ГруппаФинансовогоУчета.ВидЦенностиНДС, ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.Товары)) 
	|		КОГДА ЕСТЬNULL(СпрНоменклатура.ТипНоменклатуры, НЕОПРЕДЕЛЕНО) В (&ТипыНоменклатурыТовар)
	|				И НЕ Ценности.СписатьНаРасходы = ИСТИНА
...
// Дополняет тексты запроса проведения документа текстами отражения отгрузки товаров в учете НДС.
// Операция формирует движения по регистрам НДССостояниеРеализации0 (в случае реализации на экспорт), ДанныеОснованийСчетовФактур.
//
// В документе должна быть объявлена экспортная процедура СформироватьСуммыДокументаВВалютахУчета,
// которая инициализирует втСуммыДокументовВВалютахУчета (См. РегистрыСведений.СуммыДокументовВВалютахУчета.СформироватьПоДаннымДокумента).
//
...
Процедура ОтразитьОтгрузкуКлиенту(Запрос, ТекстыЗапроса, Регистры, ТекстЦенности) Экспорт
	...
	УчетНДСЛокализация.ОтразитьОтгрузкуКлиенту(Запрос, ТекстыЗапроса, Регистры, ТекстЦенности);
...
#Область НДСПредъявленный
	...
	ТекстыОбщегоЗапросаНДСПредъявленный = Новый Массив;
	...
	|	ИСТИНА КАК ПрименяютсяОбщиеСтавкиНДС
	|
	...
	УчетНДСЛокализация.ДополнитьТекстУСНСОбщимиСтавкамиНДС(ТекстЗапросаУчетнаяПолитика);
	
	ТекстыОбщегоЗапросаНДСПредъявленный.Добавить(ТекстЗапросаУчетнаяПолитика);
	...
	|			И Ценности.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.НалоговыйАгентПоНДС)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
	|		КОГДА УчетнаяПолитикаОрганизаций.ПрименяетсяУСН И НЕ УчетнаяПолитикаОрганизаций.ПрименяютсяОбщиеСтавкиНДС
	|			ТОГДА Ценности.ВидДеятельностиНДС
	|		КОГДА Ценности.ДокументПоступления ЕСТЬ NULL
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
	|		ИНАЧЕ Ценности.ВидДеятельностиНДС
	|	КОНЕЦ КАК ВидДеятельностиНДС,
	|	НЕОПРЕДЕЛЕНО КАК ИсправленныйСчетФактура,
	...
	|	ПО
	|		Ценности.Ссылка = УчетнаяПолитикаОрганизаций.Ссылка
	|		И Ценности.Период = УчетнаяПолитикаОрганизаций.Период
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		Справочник.Назначения КАК Назначения
	|	ПО
	...
	ТекстыОбщегоЗапросаНДСПредъявленный.Добавить(ТекстЗапросаНДСПредъявленный);
...
#КонецОбласти
	...
	ТекстыШаблоновОтражения = Новый Структура();
	Если ПроведениеДокументов.ТребуетсяТаблицаДляДвижений("НДСПредъявленный", Регистры) Тогда 
		ТекстыШаблоновОтражения.Вставить("НДСПредъявленный", СтрСоединить(ТекстыОбщегоЗапросаНДСПредъявленный));
	КонецЕсли;
	Если ПроведениеДокументов.ТребуетсяТаблицаДляДвижений("ДвиженияПоНДС", Регистры) Тогда 
		ТекстыШаблоновОтражения.Вставить("ДвиженияПоНДС", ТекстЗапросаДвиженияПоНДС);
	КонецЕсли;
...
Процедура ЗаписатьДанные(ТаблицыДляДвижений, Документ, МенеджерВременныхТаблиц, Отказ) Экспорт
	...
	УчетНДСЛокализация.ЗаписатьДанныеНастройкиУчетаНДСПриУСНКомитентов(ТаблицыДляДвижений, Документ, МенеджерВременныхТаблиц, Отказ);
...
Процедура ДобавитьЭтап_РаспределениеНДС(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	ПредшествующийЭтап = Справочники.ОперацииЗакрытияМесяца.РасчетПартийИСебестоимости;
	//++ Локализация


	//-- Локализация
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.РаспределениеНДС,,,,
		ПредшествующийЭтап);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Распределение НДС'"; // @НСтр-1
...
Процедура ДобавитьЭтап_ФормированиеДвиженийПоНДС(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Формирование движений по НДС'"; // @НСтр-1
...
// 	Операция - СправочникСсылка.ОперацииЗакрытияМесяца - Операция закрытия месяца, на выполнение которой формируется задание.
...
// Устанавливает параметры формы выбора документа основания для заполнения 
//  отчета "Статистическая форма учета перемещения товаров".
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - форма выбора документа-основания для заполнения статформы. 
//
//
Процедура УстановитьСвойстваФормыВыбораДляСтатформыВФТС(Форма) Экспорт
	...
	Если ИмяФормы = "Документ.РеализацияТоваровУслуг.Форма.ФормаВыбора" Тогда
	...
	Иначе
		...
		Элементы.ГруппаОтборыДляСтатформы.Видимость = Ложь;
		
		Если ИмяФормы = "Документ.РеализацияТоваровУслуг.Форма.ФормаВыбора" Тогда
			Элементы[ИмяСписка + "КлиентКонтрагент"].Видимость = Ложь;
		КонецЕсли;
		
		Элементы[ИмяСписка + "СтранаРеализации"].Видимость = Ложь;
		Элементы[ИмяСписка + "НалогообложениеНДС"].Видимость = Ложь;
...
Процедура ПриОпределенииНастроекПечати(Настройки) Экспорт
	...
	УчетНДСЛокализация.ПриОпределенииНастроекПечати(Настройки);
...
Функция ТекстЗапросаОбъектыДляЗамены(СОтборомПоПериоду)
	
	Если СОтборомПоПериоду Тогда
		...
		|ГДЕ
		|	Справочник.СтавкаНДС = &ТекущаяСтавка
...
Процедура ЗаблокироватьИЗаписатьСправочник(СправочникОбъект, СтавкаНДС, ТекущаяСтавкаНДС, ЭлементБлокировки)
	
	Если СправочникОбъект.СтавкаНДС <> ТекущаяСтавкаНДС Тогда
		Возврат;
	КонецЕсли;
	
	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить(ЭлементБлокировки);
