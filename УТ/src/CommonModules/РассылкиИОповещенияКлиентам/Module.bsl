...
Процедура АнализПлановыеОтгрузки(ВидОповещения)
	...
	Если Выборка.Принудительная Тогда
		...
		|ИЗ
		|	#РаспоряженияНаОтгрузкуОборотыРазвернутая КАК Отбор
		...
		ПараметрыФормированияТаблицы.ТекстУсловия =
			...
			|		ИЗ
			|			РегистрСведений.РаспоряженияНаОтгрузкуИВозвратКВыполнению КАК ОткрытыеЗаказы
			|		ГДЕ
			|			ТИПЗНАЧЕНИЯ(ОткрытыеЗаказы.Распоряжение) = ТИП(Документ.ЗаказКлиента))";
