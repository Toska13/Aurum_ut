...
//	Партнер - СправочникСсылка.Партнеры, Неопределено - партнер, для которого выберите условия цен поставщиков. Если "Неопределено", то выбирается в форме
//	ЦенаВключаетНДС - Булево, Неопределено - Отбор по виду цены, Неопределено - отбор отсутствует.
//
Процедура ВыбратьВидЦеныПоставщика(ОписаниеОповещения, Партнер = Неопределено, ЦенаВключаетНДС = Неопределено) Экспорт
	
	Отбор = Новый Структура;
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("Отбор", Отбор);
	ПараметрыОткрытия.Вставить("ВыбиратьПартнера", Ложь);
	
	Если Партнер = Неопределено Тогда
		ПараметрыОткрытия.ВыбиратьПартнера = Истина;
	Иначе
		Отбор.Вставить("Владелец", Партнер);
	КонецЕсли;
