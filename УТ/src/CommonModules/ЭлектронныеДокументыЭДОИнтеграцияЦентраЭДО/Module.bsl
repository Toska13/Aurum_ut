...
Функция СоздатьПакетМетодовОперацииПоДействиямЭДО(ПараметрыОперации, КонтекстДиагностики)
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.СформироватьОтвет) Тогда
		...
		Если ЕстьОшибкиФормированияОтветныхТитулов(КонтекстДиагностики) Тогда
			Возврат Результат;;
		КонецЕсли;
...
Функция ОшибкиФормированияДляИнтерактивногоИсправления(КонтекстДиагностики)
	...
	Пока Счетчик > 0 Цикл
		...
		Если ЭлектронныеДокументыЭДО.ЭтоОшибкаФормированияОтвета(Ошибка)
...
// Возвращаемое значение:
//  Булево
Функция ЕстьОшибкиФормированияОтветныхТитулов(КонтекстДиагностики)
	
	ОшибкиДиагностики = ОбработкаНеисправностейБЭДКлиентСервер.ПолучитьОшибки(КонтекстДиагностики);
	Если Не ЗначениеЗаполнено(ОшибкиДиагностики) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Для Каждого Ошибка Из ОшибкиДиагностики Цикл
		Если ЭлектронныеДокументыЭДО.ЭтоОшибкаФормированияОтвета(Ошибка) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Функция ДанныеСинхронизацииДокументовЭДОИзФорматаСервиса(НаборДанныхВФорматеСервиса) Экспорт
...
Функция ЗапуститьВыгрузкуВЦентрЭДО(ИдентификаторЭДО) Экспорт
	...
	Если АктивныеФоновыеЗадания.Количество() = 0 Тогда
		...
		ДлительныеОперации.ВыполнитьПроцедуру(ПараметрыВыполнения, Метаданные.РегламентныеЗадания.ВыгрузкаВЦентрЭДО.ИмяМетода);
	КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВыгрузитьНаборДокументовВЦентрЭДО", Истина);
	Методы.Вставить("ВыполнитьВыгрузкуВЦентрЭДО", Истина);
	Методы.Вставить("ЗаполнитьДанныеДляПодписанияПоВыбраннымСертификатам", Истина);
	Методы.Вставить("ИзменитьДанныеВходящегоДокументаВЦентреЭДО", Истина);
	Методы.Вставить("ПолнаяСинхронизацияДокументовЦентраЭДО", Истина);
