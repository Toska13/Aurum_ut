...
//
// Параметры:
// 	Значение - Массив из СтрокаТаблицыЗначений, Строка - Коды маркировки для которых необходимо построить дерево упаковок
...
Функция ДеревоУпаковок(Значение, Детализация = Неопределено, ПараметрыСканирования = Неопределено) Экспорт
	...
	ДеревоУпаковок.Колонки.Добавить("ВидПродукции",              Новый ОписаниеТипов(ТипыВидовПродукции));
	...
	ДеревоУпаковок.Колонки.Добавить("КоличествоГрупповыхУпаковок",       Новый ОписаниеТипов("Число"));
	...
	Для Каждого СтрокаКодаМаркировки Из ИсходныйМассивСтрок Цикл
		...
		Если СтрокаКодаМаркировки.ВидУпаковки = Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет
			Или СтрокаКодаМаркировки.ВидУпаковки = Перечисления.ВидыУпаковокИС.НеЯвляетсяУпаковкой Тогда
	...
	Если ДополнитьКолонкамиИСМП Или ДополнитьКолонкамиМОТП Тогда
	...
	ШтрихкодированиеОбщегоНазначенияИС.НормализироватьДанныеДереваУпаковокПоПереданнымКолонкам(ДеревоУпаковок, ДополнительныеКолонки);
	...
	Для Каждого ПакетКодовМаркировки Из ИсходныеПакетыКодовМаркировки Цикл
		...
		Для Каждого СтрокаКодаМаркировки Из ПакетКодовМаркировки Цикл
			...
			Если ДанныеСтатуса <> Неопределено
				И ЗначениеЗаполнено(ДанныеСтатуса.ВидУпаковки) Тогда
				ВидУпаковки  = ДанныеСтатуса.ВидУпаковки;
			КонецЕсли;
			
			Если ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская
				Или ВидУпаковки = Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет Тогда
				...
				Иначе
					...
					Если СтрокаКодаМаркировкиБезСкобок <> Неопределено
						...
						Иначе
							...
							ДанныеСтатуса.Статус = Перечисления.СтатусыКодовМаркировкиИСМП.Неопределен;
						КонецЕсли;
...
// 	РазбиратьВложения           - Булево                                  - флаг необходимости разбора вложенных кодов маркировки.
// Возвращаемое значение:
//	Структура - Описание:
//	* ТребуетсяОбновлениеКлючаСессии - Булево - Признак необходимости обновления ключа сессии.
//	* РезультатОтправкиЗапроса - (См. ОбменДаннымиИСМПКлиентСервер.ОбработатьРезультатОтправкиHTTPЗапросаКакJSON).
//	* СтатусыКодовМаркировки - Соответствие из КлючИЗначение - кодов маркировки и структур:
...
Функция ЗапроситьСтатусыКодовМаркировки(ДанныеДляЗапроса, Организация = Неопределено, ЗапрашиватьСтатусыКодовЕАЭС = Ложь, РазбиратьВложения = Истина) Экспорт
	...
	Иначе
		...
		Если МассивИсходныхСтрок.Количество() = 0 Тогда
			ВозвращаемоеЗначение.ТребуетсяОбновлениеКлючаСессии = Ложь;
		КонецЕсли;
	...
	Для Каждого СтрокаКодаМаркировки Из МассивИсходныхСтрок Цикл
		...
		Если СтрокаКодаМаркировки.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая Тогда
			...
			Если СтрокаКодаМаркировки.ТипШтрихкода <> Перечисления.ТипыШтрихкодов.SSCC
				И СтрокаКодаМаркировки.ТипШтрихкода <> Перечисления.ТипыШтрихкодов.SSCC18
				И СтрокаКодаМаркировки.ТипШтрихкода <> Перечисления.ТипыШтрихкодов.SSCC20 Тогда
				СтрокиДляЗапроса.Добавить(СтрокаКодаМаркировки);
			...
			Для Каждого ВидПродукции Из ВидыПродукции Цикл
				...
				СтрокиДляЗапроса.Добавить(НормализированнаяСтрока);
	...
	Если ВозвращаемоеЗначение.ТребуетсяОбновлениеКлючаСессии
		И СтрокиДляЗапроса.Количество() Тогда
		Возврат ВозвращаемоеЗначение;
	КонецЕсли;
	
	Если СтрокиДляЗапроса.Количество() Тогда
		...
		ПараметрыЗапросаСтатусов.ЗапрашиватьСтатусыКодовЕАЭС = ЗапрашиватьСтатусыКодовЕАЭС;
		ПараметрыЗапросаСтатусов.РазбиратьВложения           = РазбиратьВложения;
		
		РезультатЗапросаСтатусовКодовМаркировок = ЗапроситьСтатусыКодовМаркировкиПакетно(
			СтрокиДляЗапроса, ПараметрыЗапросаСтатусов);
	...
	Для Каждого КлючИЗначение Из ИсходныеСтрокиЗапросов Цикл
		...
		Для Каждого ЗапрошеннаяСтрока Из ЗапрашиваемыеСтроки Цикл
			...
			Если ДанныеСтатуса = Неопределено
				...
				И Не ТекущиеДанные.Статус = Перечисления.СтатусыКодовМаркировкиИСМП.Неопределен Тогда
				ДанныеСтатуса = ТекущиеДанные;
			КонецЕсли;
			
			Если ИсходнаяСтрокаЗапроса.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC
				И (ЗапрошеннаяСтрока.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC18
					Или ЗапрошеннаяСтрока.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC20) Тогда
				ИсходнаяСтрокаЗапроса.ТипШтрихкода = ЗапрошеннаяСтрока.ТипШтрихкода;
			КонецЕсли;
...
//  ПараметрыЗапросаСтатусов - см. ИнициализироватьПараметрыЗапросаСтатусов
// Возвращаемое значение:
//	Структура - Описание:
//	* ТребуетсяОбновлениеКлючаСессии - Булево - Признак необходимости обновления ключа сессии.
//	* РезультатОтправкиЗапроса - (См. ОбменДаннымиИСМПКлиентСервер.ОбработатьРезультатОтправкиHTTPЗапросаКакJSON).
...
Функция ЗапроситьСтатусыКодовМаркировкиПакетно(МассивИсходныхСтрок, ПараметрыЗапросаСтатусов) Экспорт
	...
	РазбиратьВложения           = ПараметрыЗапросаСтатусов.РазбиратьВложения;
	
	ПараметрыЗапросаКлючаСессии = ОбщегоНазначенияИСМПКлиентСервер.ПараметрыЗапросаКлючаСессии(Организация);
	КлючСессии = ИнтерфейсАвторизацииИСМПСлужебный.ПроверитьОбновитьКлючСессии(ПараметрыЗапросаКлючаСессии);
	
	Если ЗначениеЗаполнено(Организация)
		И ЗначениеЗаполнено(ПараметрыЗапросаКлючаСессии.ИНН) Тогда
		ИНН = ПараметрыЗапросаКлючаСессии.ИНН;
	Иначе
		ИНН = "";
	КонецЕсли;
	...
	ПараметрыЗапросаПоТоварнымГруппам = Новый Соответствие;
	ТоварныеГруппыПоВидамПродукции    = Новый Соответствие;
	ТипКлючиВидовПродукцииИС          = Тип("СправочникСсылка.КлючиВидовПродукцииИС");
	ТипВидПродукцииИС                 = Тип("СправочникСсылка.ВидыПродукцииИС");
	ТипТоварнаяГруппаГИСМТ            = Тип("СправочникСсылка.ТоварныеГруппыГИСМТ");
	КэшВидовПродукцииПоКлючам         = Новый Соответствие;
	...
	Для Каждого ЭлементМассива Из МассивИсходныхСтрок Цикл
		...
		Иначе
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
				СтрокаТаблицы, "ВидыПродукцииКодаМаркировки")
				И СтрокаТаблицы.ВидыПродукцииКодаМаркировки.Количество() Тогда
			...
			ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
				СтрокаТаблицы, "ДанныеРазбора")
				И СтрокаТаблицы.ДанныеРазбора.ВидыПродукции.Количество() Тогда
				Для Каждого ВидПродукции Из СтрокаТаблицы.ДанныеРазбора.ВидыПродукции Цикл
					ВидыПродукции.Добавить(ВидПродукции);
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
		...
		ИспользоватьТоварнуюГруппу = КонструкторИСМПКлиентСервер.ВЗапросеСтатусовИспользоватьТоварнуюГруппу(СтрокаТаблицы.ВидПродукции);
		Если ИспользоватьТоварнуюГруппу Тогда
			
			ТоварнаяГруппа = ТоварныеГруппыПоВидамПродукции[СтрокаТаблицы.ВидПродукции];
			
			Если ТоварнаяГруппа = Неопределено Тогда
				
				ТипЗначенияВидПродукции = ТипЗнч(СтрокаТаблицы.ВидПродукции);
				
				Если ТипЗначенияВидПродукции = ТипКлючиВидовПродукцииИС
					Или ТипЗначенияВидПродукции = ТипВидПродукцииИС Тогда
					ТоварнаяГруппаПоВидуПродукции = КонструкторИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппаПоВидуПродукции(
						СтрокаТаблицы.ВидПродукции);
					Если ТоварнаяГруппаПоВидуПродукции <> Неопределено Тогда
						ТоварнаяГруппа = ТоварнаяГруппаПоВидуПродукции;
					КонецЕсли;
				ИначеЕсли ТипЗначенияВидПродукции = ТипТоварнаяГруппаГИСМТ Тогда
					ТоварнаяГруппа = СтрокаТаблицы.ВидПродукции;
				КонецЕсли;
				
				ТоварныеГруппыПоВидамПродукции[СтрокаТаблицы.ВидПродукции] = ТоварнаяГруппа;
				
			КонецЕсли;
			
		КонецЕсли;
		
		ПараметрыЗапроса = ПараметрыЗапросаПоТоварнымГруппам[ТоварнаяГруппа];
		Если ПараметрыЗапроса = Неопределено Тогда
			
			ПараметрыЗапроса = Новый Структура;
			ПараметрыЗапроса.Вставить("ПараметрыURL",    Новый Массив);
			ПараметрыЗапроса.Вставить("ПакетыКодов",     Новый Массив);
			ПараметрыЗапроса.Вставить("ТекущийПакет",    Неопределено);
			ПараметрыЗапроса.Вставить("ДобавленныеКоды", Новый Соответствие);
			ПараметрыЗапроса.Вставить("КодовВПакете",    ИнтеграцияИСМППовтИсп.КоличествоКодовВПакетеДляЗапросаСтатусов());
			...
			Если ИспользоватьТоварнуюГруппу Тогда
				ПараметрыЗапроса.ПараметрыURL.Добавить(
					СтрШаблон(
						"pg=%1",
						ИнтерфейсИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппа(СтрокаТаблицы.ВидПродукции)));
			КонецЕсли;
			
			ПараметрыЗапросаПоТоварнымГруппам.Вставить(ТоварнаяГруппа, ПараметрыЗапроса);
		...
		Если ВидыПродукции.Количество() = 0 Тогда
			...
			ДобавитьКодМаркировкиДляПакентногоЗапросаСтатусов(ПараметрыЗапроса, НормализованныйКодМаркировки);
			...
			Для Каждого ВидПродукции Из ВидыПродукции Цикл
				...
				Иначе
					...
					Иначе
						...
						НормализованныйКодМаркировкиСНулями = ОбменДаннымиИСМП.КодМаркировкиДляПередачиИСМП(СтрокаТаблицы, ПараметрыНормализацииИСМПЛидирующиеНули);
						Если НормализованныйКодМаркировкиСНулями <> НормализованныйКодМаркировки Тогда
							МассивДобавляемыхКодов.Добавить(НормализованныйКодМаркировкиСНулями);
						КонецЕсли;
	...
	РезультатыОтправкиДляОбработки = Новый Массив;
	
	СтруктураДополнительныхПараметров = Новый Структура();
	СтруктураДополнительныхПараметров.Вставить("НастройкиРазбора",  НастройкиРазбораКодаМаркировки);
	СтруктураДополнительныхПараметров.Вставить("РазбиратьВложения", РазбиратьВложения);
	
	Для Каждого КлючИЗначениеПараметровЗапроса Из ПараметрыЗапросаПоТоварнымГруппам Цикл
		
		ТоварнаяГруппа   = КлючИЗначениеПараметровЗапроса.Ключ;
		ПараметрыЗапроса = КлючИЗначениеПараметровЗапроса.Значение;
		
		Для Каждого ТекущийПакет Из ПараметрыЗапроса.ПакетыКодов Цикл
			
			ОтсортированныеКодыМаркировки = Новый СписокЗначений;
			ОтсортированныеКодыМаркировки.ЗагрузитьЗначения(ТекущийПакет);
			...
			Если РезультатОтправкиЗапроса.ОтветПолучен Тогда
				...
				Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
					...
					РезультатыОтправкиДляОбработки.Добавить(РезультатОтправкиЗапроса);
			...
			ИначеЕсли ВозвращаемоеЗначение.РезультатОтправкиЗапроса.КодСостояния = 404 Тогда
				...
				Если ЗапрашиватьСтатусыКодовЕАЭС И ПараметрыЗапроса.ПараметрыЗапросаПоКодамЕАЭС.Количество() Тогда
					...
					Если РезультатЗапросаСтатусовКодовМаркировкиЕАЭС.ТребуетсяОбновлениеКлючаСессии Тогда
						ВозвращаемоеЗначение.ТребуетсяОбновлениеКлючаСессии = Истина;
						Возврат ВозвращаемоеЗначение;
					КонецЕсли;
					Если ЗначениеЗаполнено(РезультатЗапросаСтатусовКодовМаркировкиЕАЭС.ТекстОшибки) Тогда
	...
	СтрокиSSCC = Новый Соответствие;
	
	Для Каждого РезультатОтправкиЗапроса Из РезультатыОтправкиДляОбработки Цикл
		...
		Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
			...
			Иначе
				...
				Если ТипЗнч(ДанныеОбработки) = Тип("Массив") Тогда
					...
					Эталон.Вставить("Источник",       Неопределено);
					Эталон.Вставить("Результат",      Неопределено);
					Эталон.Вставить("КлючиСравнения", ИнтерфейсИСМПОбщегоНазначения.КлючиХешаПараметровКодаМаркировкиМОТП());
					...
					Для Каждого ЭлементДанных Из ДанныеОбработки Цикл
						...
						Иначе
							...
							Если СтрокаТаблицы <> Неопределено
								И ЗначениеЗаполнено(СтрокаТаблицы.ВидПродукции) Тогда
								
								Если ТипЗнч(СтрокаТаблицы.ВидПродукции) = ТипКлючиВидовПродукцииИС Тогда
									ВидПродукции = КэшВидовПродукцииПоКлючам[СтрокаТаблицы.ВидПродукции];
									Если ВидПродукции = Неопределено Тогда
										ВидПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(
											СтрокаТаблицы.ВидПродукции, "ГИСМТ");
										КэшВидовПродукцииПоКлючам[СтрокаТаблицы.ВидПродукции] = ВидПродукции;
									КонецЕсли;
								ИначеЕсли ТипЗнч(СтрокаТаблицы.ВидПродукции) = ТипВидПродукцииИС Тогда
									ВидПродукции = СтрокаТаблицы.ВидПродукции;
								КонецЕсли;
								
							ИначеЕсли ЗначениеЗаполнено(ТоварнаяГруппа) Тогда
								
								ВидПродукции = КэшВидовПродукцииПоКлючам[ТоварнаяГруппа];
								Если ВидПродукции = Неопределено Тогда
									...
									Если ВидыПродукции.Количество() Тогда
										...
										КэшВидовПродукцииПоКлючам[ТоварнаяГруппа] = ВидПродукции;
									КонецЕсли;
								КонецЕсли;
							...
							ПараметрыКодаМаркировки = ИнтерфейсИСМПОбщегоНазначения.ПараметрыКодаМаркировкиМОТП(
								ДанныеКодаМаркировки, ВидПродукции, СтруктураДополнительныхПараметров, Эталон);
						...
						Если СтрокаТаблицы <> Неопределено Тогда
							...
							Если КодОшибки <> "404" Тогда
								...
								ЕстьОшибки = ЗначениеЗаполнено(КодОшибки);
								Если ЗапрашиваемыйКодМаркировки <> Неопределено
									И СтрокаТаблицы.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC
									И (Не ЕстьОшибки Или КодОшибки = "404") Тогда
									ДанныеSSCC = Новый Структура;
									Если Не ЕстьОшибки Тогда
										ДлинаКода = СтрДлина(ЗапрашиваемыйКодМаркировки);
										Если ДлинаКода = 18 Тогда
											ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC18;
										Иначе
											ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC20;
										КонецЕсли;
									Иначе
										ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC20;
									КонецЕсли;
									ДанныеSSCC.Вставить("ТипШтрихкода", ТипШтрихкода);
									ДанныеSSCC.Вставить("КодНайден", Не ЕстьОшибки);
									ДанныеSSCC.Вставить("ЭтоШтрихкодВладельца", ИНН = ПараметрыКодаМаркировки.ИННВладельца);
									Если СтрокиSSCC[СтрокаТаблицы] = Неопределено Тогда
										СтрокиSSCC.Вставить(СтрокаТаблицы, Новый Массив);
									КонецЕсли;
									СтрокиSSCC[СтрокаТаблицы].Добавить(ДанныеSSCC);
								КонецЕсли;
		...
		ИначеЕсли РезультатОтправкиЗапроса.КодСостояния = 404 Тогда
			...
			Если ТипЗнч(РезультатОтправкиЗапроса.Объект) = Тип("Массив") Тогда
				
				Для Каждого ЭлементДанных Из РезультатОтправкиЗапроса.Объект Цикл
					...
					Если КодОшибки = "404" Тогда
						...
						Если РезультатПоиска <> Неопределено Тогда
							...
							Для Каждого КлючИЗначение Из РезультатПоиска Цикл
								...
								Если ЗапрашиваемыйКодМаркировки <> Неопределено
									И СтрокаТаблицы.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC Тогда
									ДанныеSSCC = Новый Структура;
									ДанныеSSCC.Вставить("ТипШтрихкода", Перечисления.ТипыШтрихкодов.SSCC20);
									ДанныеSSCC.Вставить("КодНайден", Ложь);
									ДанныеSSCC.Вставить("ЭтоШтрихкодВладельца", Ложь);
									Если СтрокиSSCC[СтрокаТаблицы] = Неопределено Тогда
										СтрокиSSCC.Вставить(СтрокаТаблицы, Новый Массив);
									КонецЕсли;
									СтрокиSSCC[СтрокаТаблицы].Добавить(ДанныеSSCC);
	...
	Для Каждого СтрокаSSCC Из СтрокиSSCC Цикл
		...
		Для Каждого ДанныеSSCC Из СтрокаSSCC.Значение Цикл
			Если ДанныеSSCC.КодНайден Тогда
				Если ДанныеSSCC.ЭтоШтрихкодВладельца Тогда
					ТипШтрихкода = ДанныеSSCC.ТипШтрихкода;
					Прервать;
				Иначе
					ТипШтрихкода = ДанныеSSCC.ТипШтрихкода;
				КонецЕсли;
			ИначеЕсли ТипШтрихкода = Неопределено Тогда
				ТипШтрихкода = ДанныеSSCC.ТипШтрихкода;
			КонецЕсли;
		КонецЦикла;
		Если ТипШтрихкода <> Неопределено Тогда
			СтрокаSSCC.Ключ.ТипШтрихкода = ТипШтрихкода;
		КонецЕсли;
	КонецЦикла;
...
// 
// Параметры:
// 	СоответствиеИсходныхСтрок - Соответствие из КлючИЗначение               - коды маркировки.
// 	СтатусыКодовМаркировкиКеш - Соответствие из КлючИЗначение, Неопределено - Кеш статусов кодов маркировки.
// 	Организация               - ОпределяемыйТип.Организация                 - Организация.
...
//
// Параметры:
// 	ДанныеДляЗапроса       - Массив из Структура, СтрокаТаблицыЗначений - Массив кодов маркировки.
// 	СтатусыКодовМаркировкиКеш - Соответствие, Неопределено                - Кеш статусов кодов маркировки.
// 	Организация               - ОпределяемыйТип.Организация               - Организация.
// Возвращаемое значение:
//	Структура - Описание:
...
Функция ПроверитьСеруюЗонуКодовМаркировкиПакетно(МассивИсходныхСтрок, СостояниеСеройЗоныКеш = Неопределено, Организация = Неопределено) Экспорт
	...
	Если ВидыПродукцииПоддерживающиеСеруюЗону.Количество() Тогда
		ВидПродукции = ВидыПродукцииПоддерживающиеСеруюЗону[0];
	КонецЕсли;
				...
				Если ТипЗнч(ДанныеОбработки) = Тип("Массив") Тогда
					...
					Для Каждого ЭлементМассива Из ДанныеОбработки Цикл
						...
						Если Значение <> Неопределено Тогда
							ВидУпаковки = ИнтерфейсИСМПОбщегоНазначения.ВидУпаковки(Значение, ВидПродукции);
						Иначе
						...
						СтрокиТаблицы = СоответствиеКодовМаркировки[КодМаркировки];
						Если СтрокиТаблицы = Неопределено Тогда
							КодМаркировки = ЭлементДанных["cisWithoutBrackets"];
							СтрокиТаблицы = СоответствиеКодовМаркировки[КодМаркировки];
						КонецЕсли;
...
// 
// Параметры:
// 	ПакетКодовМаркировки - Структура - Строка кода маркировки.
// 	Организация - ОпределяемыйТип.Организация - Организация
// Возвращаемое значение:
...
//		* ДанныеОбАгрегации - Неопределено -
//					        - Массив из Структура:
...
Функция ЗапроситьДанныеОбАгрегацииКодовМаркировкиПакетно(ПакетКодовМаркировки, Организация = Неопределено) Экспорт
	...
	Для Каждого ЭлементПакета Из ПакетКодовМаркировки Цикл
		...
		Если КонструкторИСМПКлиентСервер.ВЗапросеВложенныхКИИспользоватьТоварнуюГруппу(ЭлементПакета.ВидПродукции) Тогда
			ИмяТоварнойГруппы = ИнтерфейсИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппа(ЭлементПакета.ВидПродукции);
		Иначе
			ИмяТоварнойГруппы = "";
		КонецЕсли;
		
		ТелоЗапроса = ТоварныеГруппы[ИмяТоварнойГруппы];
		Если ТелоЗапроса = Неопределено Тогда
			ТелоЗапроса = Новый Массив;
			ТелоЗапроса.Добавить(ЭлементПакета.ИсходныйШтрихкод);
			ТоварныеГруппы.Вставить(ИмяТоварнойГруппы, ТелоЗапроса);
		Иначе
			ТелоЗапроса.Добавить(ЭлементПакета.ИсходныйШтрихкод);
		КонецЕсли;
	...
	Для Каждого КлючИЗначение Из ТоварныеГруппы Цикл
		...
		ОтсортированныеКодыМаркировки = Новый СписокЗначений;
		ОтсортированныеКодыМаркировки.ЗагрузитьЗначения(КлючИЗначение.Значение);
		ОтсортированныеКодыМаркировки.СортироватьПоЗначению();
		...
		Если РезультатОтправкиЗапроса.ОтветПолучен Тогда
			...
			Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
				...
				Для Каждого КлючИЗначениеДанныхОбработки Из ДанныеОбработки Цикл
					ДанныеОбАгрегации.Вставить(КлючИЗначениеДанныхОбработки.Ключ, КлючИЗначениеДанныхОбработки.Значение);
				КонецЦикла;
	...
	ВозвращаемоеЗначение.ДанныеОбАгрегации = ДанныеОбАгрегации;
...
//		** Статус - ПеречислениеСсылка.СтатусыКодовМаркировкиИСМП - Статус кода.
...
Функция КодСтраныЕАЭСПоСерийномуНомеру(СтрокаТаблицы) Экспорт
	...
	Если ШтрихкодированиеОбщегоНазначенияИС.ЭтоПолныйКодМаркировки(СтрокаТаблицы, ЗначениеЗаполнено(СтрокаТаблицы.ВидУпаковки))
		И (Не ЗначениеЗаполнено(СтрокаТаблицы.ВидПродукции)
			Или КонструкторИСМПКлиентСервер.ВидПродукцииПоддерживаетЗапросСтатусовЕАЭС(СтрокаТаблицы.ВидПродукции)) Тогда
	...
	Если СтрокаТаблицы.ДанныеРазбора = Неопределено
		...
		Для Каждого РезультатРазбора Из СтрокаТаблицы.ПримечаниеКРазборуШтрихкода.РезультатРазбора Цикл
			...
			Для Каждого ВидПродукции Из РезультатРазбора.ВидыПродукции Цикл
				Если КонструкторИСМПКлиентСервер.ВидПродукцииПоддерживаетЗапросСтатусовЕАЭС(ВидПродукции) Тогда
			...
			Если НайденыВидыПродукцииПоддерживающиеЗапросСтатусовЕАЭС Тогда
				...
				Иначе
					...
					Для Каждого ВидПродукции Из РезультатРазбора.ВидыПродукции Цикл
						Если КонструкторИСМПКлиентСервер.ВидПродукцииПоддерживаетЗапросСтатусовЕАЭС(ВидПродукции)
...
Функция ИнициализироватьПараметрыЗапросаСтатусов() Экспорт
	...
	Результат.Вставить("РазбиратьВложения",           Истина);
...
Функция ДобавлятьУпаковкуВДерево(СтрокаКодаМаркировки, Детализация, ДанныеВерхнегоУровня, ВидУпаковки)
	...
	Если ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
		...
		Если Детализация = Перечисления.ДетализацияСтруктурыХраненияИС.ПалетыСМонотоварнымиКоробами Тогда
			...
			Если ДанныеВерхнегоУровня.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая
				И (ДанныеВерхнегоУровня.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC
					Или ДанныеВерхнегоУровня.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC18
					Или ДанныеВерхнегоУровня.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC20) Тогда
	...
	Иначе
		...
		Если (Детализация = Перечисления.ДетализацияСтруктурыХраненияИС.ПотребительскиеУпаковки)
			...
			Или (Детализация = Перечисления.ДетализацияСтруктурыХраненияИС.ГрупповыеУпаковки
				И ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая)
			// В детализации Блоки с пачками - пропускаются все упаковки кроме блоков
			Или (Детализация = Перечисления.ДетализацияСтруктурыХраненияИС.ГрупповыеУпаковкиСПотребительскими
				И ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая)
			Или (Детализация = Перечисления.ДетализацияСтруктурыХраненияИС.ПалетыСМонотоварнымиКоробами
...
Функция ЗаполненВидУпаковкиПоДаннымСтатуса(СтрокаКодаМаркировки, ПараметрыКодаМаркировки)
	...
	// Производители могут создавать логистические упаковки вида (01)GTIN(21)СерийныйНомер,
	// которые пересекаются с шаблоном потребительских/групповых упаковок
	Если ПараметрыКодаМаркировки.ВидУпаковки <> Перечисления.ВидыУпаковокИС.Логистическая
		И Не РазборКодаМаркировкиИССлужебныйКлиентСервер.ВидУпаковкиСоответствуетРазбору(
...
Процедура ЗаполнитьСтрокуДереваПоДополнительнымКолонкам(СтрокаДерева, ДанныеСтатуса, ДополнительныеКолонки, ДополнительныеПоляВДанных, ДанныеСтатусаОтсутствуют = Ложь);
	...
	Если СписокПолей = Неопределено Тогда
		...
		ПоляИсключения.Вставить("ВидУпаковки", Истина);
		ПоляИсключения.Вставить("ВидПродукции", Истина);
		Если Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(СтрокаДерева.ВидПродукции) Тогда
			ПоляИсключения.Вставить("GTIN", Истина);
		КонецЕсли;
		...
		Для Каждого КлючИЗначение Из ДополнительныеКолонки Цикл
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеСтатуса, КлючИЗначение.Ключ)
				И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаДерева, КлючИЗначение.Ключ)
				И ПоляИсключения[КлючИЗначение.Ключ] = Неопределено Тогда
	...
	ПересчитатьТипШтрихкода = Ложь;
	
	Если ДополнительныеКолонки.Свойство("ВидПродукции")
		И ЗначениеЗаполнено(ДанныеСтатуса.ВидПродукции)
		И Не ИнтеграцияИСМППовтИсп.ВидыПродукцииПринадлежатТоварнойГруппе(СтрокаДерева.ВидПродукции, ДанныеСтатуса.ВидПродукции) Тогда
		СтрокаДерева.ВидПродукции = ДанныеСтатуса.ВидПродукции;
		ПересчитатьТипШтрихкода = Истина;
	КонецЕсли;
	
	Если Не ДанныеСтатусаОтсутствуют Тогда
		
		Если ДополнительныеКолонки.Свойство("ВидУпаковки")
			И ЗначениеЗаполнено(ДанныеСтатуса.ВидУпаковки)
			И ДанныеСтатуса.ВидУпаковки <> СтрокаДерева.ВидУпаковки Тогда
			СтрокаДерева.ВидУпаковки = ДанныеСтатуса.ВидУпаковки;
			ПересчитатьТипШтрихкода = Истина;
		КонецЕсли;
		
	КонецЕсли;
	
	Если ПересчитатьТипШтрихкода Тогда
		
		ПримечаниеКРезультатуРазбора = Неопределено;
		ДанныеРазбора = РазборКодаМаркировкиИССлужебный.РазобратьКодМаркировки(
			СтрокаДерева.Штрихкод, СтрокаДерева.ВидПродукции, ПримечаниеКРезультатуРазбора);
	...
	Если СтрНайти(СписокПолей, "МРЦ") > 0
		И Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(СтрокаДерева.ВидПродукции) Тогда
		СтрокаДерева.МРЦ = 0;
	КонецЕсли;
...
Процедура ЗаполнитьСтрокуДереваСпецификойПоСоставуКода(СтрокаДерева, СтрокаКодаМаркировки, ПараметрыЗаполнения, ЭтоФиктивнаяСтрока = Ложь);
	...
	// МРЦ
	Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(СтрокаДерева.ВидПродукции)
...
Процедура ЗаполнитьДеревоУпаковокРекурсивно(ПараметрыЗаполнения)
	...
	ИначеЕсли ТипЗнч(КодыМаркировкиУпаковок) <> Тип("Число")
		...
		Для Каждого ЗначениеВложенногоКода Из КодыМаркировкиУпаковок Цикл
			...
			Если ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда // Не имеет вложенных кодов
				ВложенныеКодыМаркировки = Новый Соответствие;
			ИначеЕсли ВидУпаковки = Перечисления.ВидыУпаковокИС.Набор
			...
			Если ДанныеСтатуса = Неопределено Тогда
				...
				ДанныеСтатусаОтсутствуют = Истина;
			Иначе
			...
			Если ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая
				Или ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая
				Или ВидУпаковки = Перечисления.ВидыУпаковокИС.Набор Тогда // Логистическая упаковка или блок
				
				Если ДобавлятьУпаковкуВДерево(СтрокаКодаМаркировки, Детализация, ДанныеВерхнегоУровня, ВидУпаковки) Тогда
...
Процедура ДобавитьКодМаркировкиДляПакентногоЗапросаСтатусов(ПараметрыЗапроса, НормализованныйКодМаркировки)
	
	Если ПараметрыЗапроса.ТекущийПакет = Неопределено
		Или ПараметрыЗапроса.ТекущийПакет.Количество() = ПараметрыЗапроса.КодовВПакете Тогда
		ПараметрыЗапроса.ТекущийПакет = Новый Массив;
		ПараметрыЗапроса.ПакетыКодов.Добавить(ПараметрыЗапроса.ТекущийПакет);
	КонецЕсли;
	
	ПараметрыЗапроса.ТекущийПакет.Добавить(НормализованныйКодМаркировки);
