...
Функция НастройкиРазбораКодаМаркировкиДляХэшаНормализации(ВидПродукции = Неопределено) Экспорт
	...
	Если ВидПродукции = Неопределено
		Или Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(ВидПродукции)
...
Функция ПрефиксыВидовПродукцииИдентификационногоКодаСырья() Экспорт
	
	ВозвращаемоеЗначение = Новый Структура();
	ВозвращаемоеЗначение.Вставить("ПрефиксыПоВидуПродукции", Новый Соответствие());
	ВозвращаемоеЗначение.Вставить("РегулярноеВыражениеВсе",  "");
	
	ЭлементКонструктора = ПредопределенноеЗначение(
		"Справочник.ЭлементыКонструктораВидовПродукцииИС.ПрефиксИдентификационногоКодаСырья");
	...
	Для Каждого КлючИЗначение Из ДанныеЭлемента.ВидыПродукции Цикл
		...
		ПоляВидаПродукции = КлючИЗначение.Значение;
		Если ПоляВидаПродукции = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Префиксы = ВозвращаемоеЗначение.ПрефиксыПоВидуПродукции[КлючИЗначение.Ключ];
		Если Префиксы = Неопределено Тогда
			Префиксы = Новый Массив;
			ВозвращаемоеЗначение.ПрефиксыПоВидуПродукции[КлючИЗначение.Ключ] = Префиксы;
		КонецЕсли;
		
		Для Каждого КлючИЗначениеПоВидуПродукции Из ПоляВидаПродукции Цикл
			Префиксы.Добавить(КлючИЗначениеПоВидуПродукции.Ключ);
			ВариантыПоиска.Добавить(
				СтрШаблон("^%1[0-9]{11}", КлючИЗначениеПоВидуПродукции.Ключ));
		КонецЦикла;
		
	КонецЦикла;
	
	ВозвращаемоеЗначение.РегулярноеВыражениеВсе = СтрСоединить(ВариантыПоиска, "|");
