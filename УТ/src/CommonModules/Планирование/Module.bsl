...
Процедура ОтразитьДвижения(ТаблицыДляДвижений, Движения, Отказ) Экспорт
	...
	Если ТаблицыДляДвижений.Свойство(ИмяТаблицы)
		...
		Запрос = Новый Запрос("
			|ВЫБРАТЬ
			|	Комплекты.ВариантКомплектации КАК ВариантКомплектации,
			|	Комплекты.Назначение КАК Назначение,
			|	Комплекты.Склад КАК Склад,
			|	Комплекты.ДатаВыпуска КАК ДатаВыпуска,
			|	ВЫБОР
			|		КОГДА Комплекты.Количество >= Комплекты.КЗаказу
			|			ТОГДА Комплекты.Количество
			|		ИНАЧЕ Комплекты.КЗаказу
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	Товары.Ссылка КАК ВариантКомплектации,
			...
			|	Комплекты.Склад КАК Склад,
			|	Комплекты.ДатаВыпуска КАК ДатаВыпуска,
			|	Комплекты.Количество * Товары.КоличествоУпаковок / Товары.Ссылка.Количество КАК КоличествоУпаковок,
			|	Комплекты.Количество * Товары.Количество / Товары.Ссылка.Количество КАК Количество,
			|	Товары.ДоляСтоимости КАК ДоляСтоимости
			|ИЗ
			|	Справочник.ВариантыКомплектацииНоменклатуры.Товары КАК Товары
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТКомплекты КАК Комплекты
			|		ПО Комплекты.ВариантКомплектации = Товары.Ссылка
			|
			|УПОРЯДОЧИТЬ ПО
			|	Товары.НомерСтроки");
		
		Запрос.УстановитьПараметр("Комплекты", ТаблицыДляДвижений[ИмяТаблицы]);
		
		ДанныеВариантовКомплектации = Запрос.Выполнить().Выгрузить();
		
		Отбор = Новый Структура();
		Отбор.Вставить("ВариантКомплектации");
		Отбор.Вставить("Назначение");
		Отбор.Вставить("Склад");
		Отбор.Вставить("ДатаВыпуска");
		
		Для каждого ЭлементКомплект Из ТаблицыДляДвижений[ИмяТаблицы] Цикл
			
			ЗаполнитьЗначенияСвойств(Отбор, ЭлементКомплект);
			
			Комплектующие = ДанныеВариантовКомплектации.НайтиСтроки(Отбор);
			
			Для каждого ЭлементКомплектующее Из Комплектующие Цикл
...
Процедура ОбработкаПроверкиЗаполненияПоСценариюВидуПлана(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт 
	...
	Если ЗначениеЗаполнено(Объект.ВидПлана) Тогда
		РеквизитыВидаПлана = "ЗаполнятьПодразделение, ЗаполнятьОрганизацию, ЗаполнятьПартнера, ЗаполнятьПартнераВТЧ,
			|ЗаполнятьСоглашение, ЗаполнятьСоглашениеВТЧ, ЗаполнятьСклад,ЗаполнятьСкладВТЧ, ЗаполнятьПланОплат,
			|ЗаполнятьМенеджера, ЗаполнятьФорматМагазина, ЗаполнятьПоДефициту";
		...
		Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПланЗакупок") Тогда
			Если НЕ НастройкиЗаполнения.ЗаполнятьПодразделение Тогда
				МассивНепроверяемыхРеквизитов.Добавить("Подразделение");
			КонецЕсли;
			Если НЕ НастройкиЗаполнения.ЗаполнятьОрганизацию Тогда
				МассивНепроверяемыхРеквизитов.Добавить("Организация");
			КонецЕсли;
		...
		ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.ПланПродаж") Тогда
			...
			Если НЕ НастройкиЗаполнения.ЗаполнятьСоглашениеВТЧ ИЛИ Объект.КроссТаблица Тогда
				МассивНепроверяемыхРеквизитов.Добавить("Товары.Соглашение");
			КонецЕсли;
			Если НастройкиЗаполнения.ЗаполнятьСкладВТЧ И НЕ Объект.КроссТаблица И Не Объект.ЗаполнятьПоДаннымСервиса Тогда
				ПараметрыПроверки = Новый Структура("ИмяТЧ, ПредставлениеТЧ", "Товары", НСтр("ru = 'Товары'"));
				ПроверитьЗаполнениеСклада(Объект, Отказ, ПараметрыПроверки);
			КонецЕсли;
			Если Объект.ПланироватьПоСумме И Объект.ЗаполнятьПланОплат <> НастройкиЗаполнения.ЗаполнятьПланОплат Тогда
...
//  СоздаватьЭлемент			 - Булево		 - Флаг определяет, создавать ли элемент формы
//  СтруктураДействий			 - Структура	 - Структура действий элемента, Ключ - имя события, значение - имя процедуры обработчика события.
Процедура ДобавитьПолеКроссТаблицыСумма(ПараметрыВывода, СоздаватьЭлемент = Истина, ПараметрыПоляСумма, СтруктураДействий = Неопределено) Экспорт 
	...
	СтруктураПоля = Новый Структура;
	СтруктураПоля.Вставить("ПрефиксРеквизитаКолонки", ПараметрыПоляСумма.Префикс);
	СтруктураПоля.Вставить("УдалятьРеквизитыТаблицы", Истина);
	СтруктураПоля.Вставить("СоздаватьЭлемент", СоздаватьЭлемент);
	СтруктураПоля.Вставить("ТипЭлемента", ОбщегоНазначенияУТ.ОписаниеТипаДенежногоПоля());
	СтруктураПоля.Вставить("Заголовок", ПараметрыПоляСумма.Заголовок);
	СтруктураПоля.Вставить("СоздаватьИтоговыеРеквизиты", Истина);
	СтруктураПоля.Вставить("ШиринаЭлемента", 6);
	СтруктураПоля.Вставить("СвойстваЭлемента", Новый Структура());
	СтруктураПоля.СвойстваЭлемента.Вставить("ВыделятьОтрицательные", Истина);
...
Процедура ЗаполнитьТаблицуИзКроссТаблицы(ЭтотОбъектТаблица, ОбъектТаблица, КроссТаблица, Параметры, СтруктураДействий = Неопределено, 
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(
		ЭтотОбъектТаблица, СтруктураДействий, КэшированныеЗначения);
	
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(
		ОбъектТаблица, СтруктураДействий, КэшированныеЗначения);
...
Функция ДополнительныеПараметрыДляРасчетаПоФормуле(Параметры) Экспорт
	...
	СоздаватьРеквизитСОтбором = ВидПлана.ЗаполнятьПартнера 
		...
		ИЛИ ВидПлана.ЗаполнятьОрганизацию
...
Процедура ИнициализироватьДопПараметры(ДополнительныеПараметрыТовары, ДополнительныеПараметрыТоварыПоПериодам, Параметры) Экспорт
	...
	ВидПланаРеквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Параметры.Форма.Объект.ВидПлана, 
		"ТипПлана, ЗаполнятьПартнера, ЗаполнятьСоглашение, ЗаполнятьСклад, ЗаполнятьПодразделение, ЗаполнятьОрганизацию,
		|ЗаполнятьМенеджера, ЗаполнятьФорматМагазина, ЗаполнятьНазначениеВТЧ");
...
Функция ЗаполнитьПоФормуле(Параметры) Экспорт 
	...
	Для каждого ИмяОперанда Из МассивИменНаборов Цикл
		...
		ИначеЕсли ИмяОперанда = "СвободныеОстатки" Тогда
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Планирование.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "11.5.26.68";
	Обработчик.РежимВыполнения = "Монопольно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("37d37b7b-990a-4cf5-81c7-79e32dbc7554");
	Обработчик.Комментарий = НСтр("ru = 'Синхронизирует значения новых констант использования обработок формирования документов по планам с их ведущими константами: использование документов и соответствующих планов.'");
	
	Читаемые = Новый Массив;
	Изменяемые = Новый Массив;
	МетаданныеКонстант = Метаданные.Константы;
	Для Каждого ОписаниеКонстанты Из ОписаниеКонстантФормированияДокументовПоПланам() Цикл
		Читаемые.Добавить(МетаданныеКонстант[ОписаниеКонстанты.ИспользованиеПланов].ПолноеИмя());
		Читаемые.Добавить(МетаданныеКонстант[ОписаниеКонстанты.ИспользованиеДокументов].ПолноеИмя());
		Изменяемые.Добавить(МетаданныеКонстант[ОписаниеКонстанты.ИспользованиеОбработки].ПолноеИмя());
	КонецЦикла; 
	
	Читаемые = ОбщегоНазначенияКлиентСервер.СвернутьМассив(Читаемые);
	Изменяемые = ОбщегоНазначенияКлиентСервер.СвернутьМассив(Изменяемые);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию() Экспорт
	...
	ВедущиеКонстанты = Новый Структура; // Для инициализиации пересчета всех зависимых констант 1 раз.
	Для Каждого ОписаниеКонстанты Из ОписаниеКонстантФормированияДокументовПоПланам() Цикл
		ВедущиеКонстанты.Вставить(ОписаниеКонстанты.ИспользованиеОбработки, ОписаниеКонстанты.ИспользованиеДокументов);
	КонецЦикла;
	
	Для Каждого КлючИЗначение Из ВедущиеКонстанты Цикл
		СинхронизироватьКонстантыФормированияПоПланам(КлючИЗначение.Значение);
	КонецЦикла;
...
Процедура СинхронизироватьЗначенияПодчиненныхКонстант(ИмяКонстанты, ЗначениеКонстанты, Отказ) Экспорт
	...
	СинхронизироватьКонстантыФормированияПоПланам(ИмяКонстанты);
...
Функция ПолучитьЗамещенныеПериоды(Параметры, ИмяДокумента , ИмяРеквизитаНачалоПериода = "НачалоПериода") Экспорт
	...
	|	И &ТекстЗапросаОтборОрганизация
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст,
		"&ТекстЗапросаОтборОрганизация",
		?(Параметры.Свойство("Организация"),"ПланЗамещающий.Организация = &Организация","Истина"));
...
Процедура СбалансированностьПлановОбработкаПроверкиЗаполнения(КомпоновщикНастроек, Отказ) Экспорт 
	...
	ПланыОстатков   = ПолучитьОтбор(КомпоновщикНастроек, "ТипПлана", 7);
...
Процедура СбалансированностьПлановПриКомпоновкеРезультата(СхемаКомпоновкиДанных, КомпоновщикНастроек, ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка) Экспорт
	...
	ПланыЗакупок               = ПолучитьОтбор(КомпоновщикНастроек, "ТипПлана", 1);
	ПланыОстатков              = ПолучитьОтбор(КомпоновщикНастроек, "ТипПлана", 7);
	ПланыПродаж                = ПолучитьОтбор(КомпоновщикНастроек, "ТипПлана", 2);
	ПланыПроизводства          = ПолучитьОтбор(КомпоновщикНастроек, "ТипПлана", 3);
	ПланыСборкиКомплекты       = ПолучитьОтбор(КомпоновщикНастроек, "ТипПлана", 4);
	ПланыРазборкиКомплекты     = ПолучитьОтбор(КомпоновщикНастроек, "ТипПлана", 5);
	ПланыСборкиКомплектующие   = ПолучитьОтбор(КомпоновщикНастроек, "ТипПлана", 8);
	ПланыРазборкиКомплектующие = ПолучитьОтбор(КомпоновщикНастроек, "ТипПлана", 9);
	ПланыМатериалов            = ПолучитьОтбор(КомпоновщикНастроек, "ТипПлана", 6);
	ПланыВнутреннихПотреблений = ПолучитьОтбор(КомпоновщикНастроек, "ТипПлана", 11);
	КорректировкиПотребностей  = ПолучитьОтбор(КомпоновщикНастроек, "ТипПлана", 10);
	Статус                     = ПолучитьОтбор(КомпоновщикНастроек, "Статус", "");
	СтатусыСценария            = Новый Массив;
...
Функция ПолучитьОтбор(ЭлементСтруктуры, ИмяПоля, ПравоеЗначение = 0)
	...
	Для каждого Элемент Из Отборы Цикл
		Если ТипЗнч(Элемент) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			Отбор = ПолучитьОтбор(Элемент.Элементы, Поле, ПравоеЗначение);
			Если Отбор <> Неопределено Тогда
				Прервать;
			КонецЕсли; 
		Иначе
			Если Элемент.ЛевоеЗначение = Поле 
				И (НЕ ЗначениеЗаполнено(ПравоеЗначение) 
					ИЛИ ЗначениеЗаполнено(ПравоеЗначение) 
					И Элемент.ПравоеЗначение = ПравоеЗначение) Тогда
				Отбор = Элемент;
				Прервать;
			КонецЕсли;
		КонецЕсли;
...
Функция ПроверитьСуществованиеУжеСозданныхПланов(ДанныеФормы, ИмяТипаДокумента) Экспорт
	...
	Если ИмяТипаДокумента = "ПланЗакупок" Тогда
		...
		|	И План.Организация = &Организация
		|	И План.Склад = &Склад
...
Функция РаспределитьКоличествоПропорциональноКоэффициентамИКратностиСтроки(Знач РаспределяемаяСумма, КоэффициентУпаковки, Коэффициенты, КратностьТовара)
	...
	Если Коэффициенты.Количество() = 0 Или Не ЗначениеЗаполнено(РаспределяемаяСумма) ИЛИ КратностьТовара = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ПеренестиИлиРаспределитьНаСервере(ПараметрыРаспределения, Знач ВыделенныеСтроки, ФормированиеЗаказов) Экспорт
	...
	СтрокиДляПакетнойОбработки = Новый Массив;
	...
	Для каждого ТекущаяСтрока Из ВыделенныеСтроки Цикл
		...
		ИначеЕсли ПараметрыРаспределения.Операция = "Распределение" Тогда
			...
			СтрокиДляПакетнойОбработки.Добавить(СтрокаТЧ);
			...
			Для НомерПериода = 2 По КоличествоПериодов Цикл
				...
				СтрокиДляПакетнойОбработки.Добавить(НоваяСтрока);
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		СтрокиДляПакетнойОбработки,
		СтруктураДействий,
		ФормированиеЗаказов,
		КэшированныеЗначения);
		
	Для Каждого Строка Из СтрокиДляПакетнойОбработки Цикл
			
		Если ПараметрыРаспределения.ТипПланирования = "ПланированиеЗакупок" Тогда
			Строка.ОбщийВес = Строка.Количество * Строка.Вес;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьПланыПоДефициту(Сценарий, ВидПлана, НачалоПериода, ОкончаниеПериода, ПланОбъект = Неопределено) Экспорт
	...
	Попытка
		...
		Реквизиты = "ТипПлана, КоличествоПериодов, ЗаполнятьПодразделение, ЗаполнятьОрганизацию, ЗаполнятьПартнера, ВидЦены,
...
Процедура СопоставитьЗагружаемыеДанные(АдресЗагружаемыхДанных, АдресТаблицыСопоставления, СписокНеоднозначностей, ПолноеИмяТабличнойЧасти, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ИспользуютсяСоглашения Тогда
		ТекстыЗапроса.Добавить(ТекстЗапросаВтДанныеСоглашений(ДополнительныеПараметры.ТипПлана));
	КонецЕсли;
...
Функция ТекстЗапросаВтДанныеСоглашений(ТипПлана)
	ТекстЗапроса = "
	|ВЫБРАТЬ
	...
	Если ТипПлана = "ПланПродаж" Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "СоглашенияСПоставщиками", "СоглашенияСКлиентами")
	КонецЕсли;
...
#Область КонстантыФормированияПоПланам

// Обновляет значения констант использования обработок формирования документов по планам. 
//
// Параметры:
//   ИмяИзмененнойКонстанты - Строка - Имя измененной константы, которая инициировала синхронизацию.
Процедура СинхронизироватьКонстантыФормированияПоПланам(ИмяИзмененнойКонстанты)
	
	ОписаниеКонстант = ОписаниеКонстантФормированияДокументовПоПланам();
	
	ЗависимыеКонстанты = Новый Массив;
	
	Для Каждого ОписаниеКонстанты Из ОписаниеКонстант Цикл
		
		Если ИмяИзмененнойКонстанты = ОписаниеКонстанты.ИспользованиеПланов Тогда
			ЗависимыеКонстанты.Добавить(ОписаниеКонстанты.ИспользованиеОбработки);
			Продолжить;
		КонецЕсли;
		
		Если ИмяИзмененнойКонстанты = ОписаниеКонстанты.ИспользованиеДокументов Тогда
			ЗависимыеКонстанты.Добавить(ОписаниеКонстанты.ИспользованиеОбработки);
			Продолжить;
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ЗависимыеКонстанты) Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого ЗависимаяКонстанта Из ЗависимыеКонстанты Цикл
		
		Отбор = Новый Структура("ИспользованиеОбработки", ЗависимаяКонстанта);
		ОписанияКонстанты = ОписаниеКонстант.НайтиСтроки(Отбор);
		
		ИспользоватьОбработку = Ложь;
		Для Каждого ОписаниеКонстанты Из ОписанияКонстанты Цикл
			
			ИспользоватьОбработку = Константы[ОписаниеКонстанты.ИспользованиеДокументов].Получить()
				И Константы[ОписаниеКонстанты.ИспользованиеПланов].Получить();
			
			Если ИспользоватьОбработку Тогда
				Прервать;
			КонецЕсли;
			
		КонецЦикла;
		
		Если ИспользоватьОбработку <> Константы[ЗависимаяКонстанта].Получить() Тогда
			Константы[ЗависимаяКонстанта].Установить(ИспользоватьОбработку);
		КонецЕсли;
...
// Функциональные опции, включающие обработки формирования документов по планам зависят от двух констант:
// 1. Использование планов, по которым будут вводиться документы.
// 2. Использование документов, которые будут вводиться по планам.
//
// Возвращаемое значение:
//  ТаблицаЗначений - Описание взаимосвязей:
//    * ИспользованиеОбработки - Строка - Имя константы, от которой зависит использование обработки.
//    * ИспользованиеПланов - Строка - Имя константы, включающей использование планов по которым формируются документы в обработке.
//    * ИспользованиеДокументов - Строка - Имя константы, включающей использование документов, которые формируются по планам.
Функция ОписаниеКонстантФормированияДокументовПоПланам()

	ОписаниеТипаСтрока150 = ОбщегоНазначения.ОписаниеТипаСтрока(150);

	ВозвращаемоеЗначение = Новый ТаблицаЗначений;
	ВозвращаемоеЗначение.Колонки.Добавить("ИспользованиеОбработки",  ОписаниеТипаСтрока150);
	ВозвращаемоеЗначение.Колонки.Добавить("ИспользованиеПланов",     ОписаниеТипаСтрока150);
	ВозвращаемоеЗначение.Колонки.Добавить("ИспользованиеДокументов", ОписаниеТипаСтрока150);
	
	МетаданныеКонстант = Метаданные.Константы;
	
	НовоеОписаниеКонстантыФормированияДокументовПоПланам(
		ВозвращаемоеЗначение,
		МетаданныеКонстант.ИспользоватьЗаявкиНаЗакупку,
		МетаданныеКонстант.ИспользоватьПланированиеЗакупок,
		МетаданныеКонстант.ИспользоватьФормированиеЗаявокНаЗакупкуПоПлану);
	
	НовоеОписаниеКонстантыФормированияДокументовПоПланам(
		ВозвращаемоеЗначение,
		МетаданныеКонстант.ИспользоватьЗаказыПоставщикам,
		МетаданныеКонстант.ИспользоватьПланированиеЗакупок,
		МетаданныеКонстант.ИспользоватьФормированиеЗаказовПоставщикуПоПлану);
	
	НовоеОписаниеКонстантыФормированияДокументовПоПланам(
		ВозвращаемоеЗначение,
		МетаданныеКонстант.ИспользоватьЗаказыНаСборку,
		МетаданныеКонстант.ИспользоватьПланированиеСборкиРазборки,
		МетаданныеКонстант.ИспользоватьФормированиеЗаказовНаСборкуПоПлану);
	
	Возврат ВозвращаемоеЗначение;

КонецФункции

Функция НовоеОписаниеКонстантыФормированияДокументовПоПланам(ОписаниеКонстант,
	ИспользованиеДокументов, ИспользованиеПланов, ИспользованиеОбработки)
	
	ОписаниеКонстанты                         = ОписаниеКонстант.Добавить();
	ОписаниеКонстанты.ИспользованиеДокументов = ИспользованиеДокументов.Имя;
	ОписаниеКонстанты.ИспользованиеПланов     = ИспользованиеПланов.Имя;
	ОписаниеКонстанты.ИспользованиеОбработки  = ИспользованиеОбработки.Имя;
