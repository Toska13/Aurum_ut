...
Функция ДатаПоСтроковомуПредставлению(ПредставлениеДаты) Экспорт
	...
	РегулярноеВыражение = "\d{1,2}[\s_]+(?:января|январь|янв|февраля|февраль|февр|марта|март|мар|апреля|апрель|апр|мая|май|"
		...
		+ "janvarja|janvarya|janv|yanvarya|yanv|fevralja|fevralya|fevr|marta|mart|mar|aprelja|aprelya|apr|maja|maya|may|ijunja|iyunya|ijun|iyun|"
	...
	Месяцы.Вставить("yanvarya", 1);
	Месяцы.Вставить("yanv", 1);
...
// Добавляет индекс в таблицу, если его еще нет.
// 
// Параметры:
//  Таблица - ТаблицаЗначений
//  ДобавляемыйИндекс - Строка
// 
Процедура ДобавитьИндексПриНеобходимости(Таблица, ДобавляемыйИндекс) Экспорт
	
	ЕстьИндекс = Ложь;
	
	Для Каждого СуществующийИндекс Из Таблица.Индексы Цикл
		
		Если Строка(СуществующийИндекс) = ДобавляемыйИндекс Тогда
	...
	Таблица.Индексы.Добавить(ДобавляемыйИндекс);
...
Функция ДанныеЮрФизЛица(ЮрФизЛицо) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ЮрФизЛицо) Тогда
		Возврат Сведения;
	КонецЕсли;
	ОписаниеЗамера = НачатьЗамер("Переопределяемый.ПолучитьДанныеЮрФизЛица");
	ЭлектронноеВзаимодействиеПереопределяемый.ПолучитьДанныеЮрФизЛица(ЮрФизЛицо, Сведения);
	ЗакончитьЗамер("Переопределяемый.ПолучитьДанныеЮрФизЛица", ОписаниеЗамера);
	Возврат Сведения;
...
// Возвращаемое значение:
//  - Неопределено
//  - см. ОценкаПроизводительности.НачатьЗамерДлительнойОперации
Функция НачатьЗамер(ИмяКлючевойОперации) Экспорт
	Если ИмяКлючевойОперации = ""
		Или Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		Возврат Неопределено;
	КонецЕсли;
	Возврат ОценкаПроизводительности.НачатьЗамерДлительнойОперации(ИмяКлючевойОперации);
КонецФункции

// Параметры:
//  ШагДействия - Строка
//  ОписаниеЗамера - см. ОценкаПроизводительности.НачатьЗамерДлительнойОперации
//  ОбработаноДанных - Число
//  Комментарий - Строка
Процедура ЗафиксироватьЭтап(ШагДействия, ОписаниеЗамера, ОбработаноДанных = 0, Комментарий = "") Экспорт
	Если ОписаниеЗамера = Неопределено Или ОписаниеЗамера.Количество() = 0
		Или Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		Возврат;
	КонецЕсли;
	ОценкаПроизводительности.ЗафиксироватьЗамерДлительнойОперации(ОписаниеЗамера, ОбработаноДанных, ШагДействия, 
		Комментарий);
КонецПроцедуры
...
//                   - Неопределено
//  Комментарий - Строка
Процедура ЗакончитьЗамер(КлючДействия, ОписаниеЗамера, ОбработаноДанных = 0, Комментарий = "") Экспорт
	Если КлючДействия = Неопределено
		Или Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		Возврат;
	КонецЕсли;
	Если ОбработаноДанных = Неопределено Тогда
		ОбработаноДанных = 0;
	КонецЕсли;
	
	ОценкаПроизводительности.ЗакончитьЗамерДлительнойОперации(ОписаниеЗамера, ОбработаноДанных, КлючДействия, 
		Комментарий);
КонецПроцедуры
...
// См. СерверныеОповещения.ОтправитьСерверноеОповещение
