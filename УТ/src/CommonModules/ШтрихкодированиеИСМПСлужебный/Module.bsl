...
Функция ПроверитьНеобходимостьУточнения(СтрокаДерева, ПараметрыСканирования) Экспорт
	...
	Если ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗаполнятьДанныеВЕТИС(ПараметрыСканирования) Тогда
		Если ТипЗнч(СтрокаДерева.ВидПродукции) = Тип("СправочникСсылка.КлючиВидовПродукцииИС")
			И Не ЗначениеЗаполнено(СтрокаДерева.ИдентификаторПроисхожденияВЕТИС) Тогда
			Возврат ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияВЕТИС(СтрокаДерева.ВидПродукции);
		ИначеЕсли Не ЗначениеЗаполнено(СтрокаДерева.ИдентификаторПроисхожденияВЕТИС) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
	
	Если ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗаполнятьСрокГодности(ПараметрыСканирования)
		И КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСоСрокамиГодности(СтрокаДерева.ВидПродукции)
...
//  по которой не завершен подбор маркируемой продукции, из текущего документа.
//
// Параметры:
...
Функция ЗаполнитьПоДокументуИОснованию(СтрокаДанных, ПараметрыСканирования, ДанныеДокументаОснования, ДанныеДокумента) Экспорт
	...
	ТребуетсяЗаполнитьИдентификаторВЕТИС = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗаполнятьДанныеВЕТИС(ПараметрыСканирования)
		И Не ЗначениеЗаполнено(СтрокаДанных.ИдентификаторПроисхожденияВЕТИС);
	ТребуетсяЗаполнитьСрокГодности = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗаполнятьСрокГодности(ПараметрыСканирования)
	...
	Для Каждого ВариантВыбора Из НайденныеСтроки Цикл
		...
		ИначеЕсли ПроверятьИзКолонкиКоличествоПодобрано И ВариантВыбора.Количество <= ВариантВыбора.КоличествоПодобрано Тогда
			Продолжить;
		ИначеЕсли ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗаполнятьДанныеВЕТИС(ПараметрыСканирования)
...
Процедура ОбновитьДеревоУпаковок(ДанныеПоШтрихкодам, Знач ОбновленныеКодыМаркировки, ПараметрыСканирования) Экспорт
	...
	Для Каждого СтрокаТаблицыКоды Из ОбновленныеКодыМаркировки Цикл
		Если СтрокаТаблицыКоды.СтрокаДерева <> Неопределено Тогда
			...
			Если ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗаполнятьСрокГодности(ПараметрыСканирования)
					Или ПараметрыСканирования.КонтролироватьОкончаниеСрокаГодности Тогда
	...
	Пока ОбновляемыеГруппы.Количество() Цикл
		...
		Для Каждого Группа Из ОбновляемыеГруппы Цикл
			...
			Для Каждого ВложенныйЭлемент Из Группа.Строки Цикл
				...
				ИначеЕсли ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗаполнятьСрокГодности(ПараметрыСканирования)
						И Не ЗначениеЗаполнено(ВложенныйЭлемент.ГоденДо) Тогда
				...
				ИначеЕсли Результат.ИдентификаторПроисхожденияВЕТИС <> ВложенныйЭлемент.ИдентификаторПроисхожденияВЕТИС
					Или (ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗаполнятьСрокГодности(ПараметрыСканирования)
						И Результат.ГоденДо <> ВложенныйЭлемент.ГоденДо) Тогда
					Результат.ОбщаяСпецифика = Ложь;
				КонецЕсли;
			КонецЦикла;
...
Процедура ДополнитьДанныеПоШтрихкодамСпецификой(ДанныеПоШтрихкодам, ПараметрыСканирования, ПараметрыДополнения = Неопределено, ЗаполнятьСтрокиДерева = Ложь) Экспорт
	...
	Для Каждого СтрокаДанныхКодовМаркировки Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМЕХ(СтрокаДанныхКодовМаркировки.ВидПродукции)
			И Не ЗначениеЗаполнено(СтрокаДанныхКодовМаркировки.GTIN) Тогда
			МассивСтрокМЕХ.Добавить(СтрокаДанныхКодовМаркировки);
		КонецЕсли;
	...
	Если МассивСтрокБезЗапросаСтатусовИСМП.Количество() Тогда
		ДополнитьДанныеПоШтрихкодамСпецификойБезЗапросаСтатусовИСМП(
			ДанныеПоШтрихкодам, ПараметрыСканирования, МассивСтрокБезЗапросаСтатусовИСМП);
	КонецЕсли;
	
	Если МассивСтрокМЕХ.Количество() Тогда
		РегистрыСведений.ДанныеRFIDИСМП.ЗаполнитьДанныеRFID(МассивСтрокМЕХ);
	КонецЕсли;
	...
	// Заполнение количества потребительских упаковок по коэффициентам упаковок и коэффициентам из таблицы Описание GTIN
	...
	ШтрихкодированиеМОТП.ДополнитьДанныеПоШтрихкодамСпецификойМинимальныхЦен(ДанныеПоШтрихкодам, ПараметрыСканирования);
...
Процедура ДополнитьДанныеПоШтрихкодамПараметрамиДополнения(ДанныеПоШтрихкодам, ПараметрыСканирования, ПараметрыДополнения) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если СтрокаДанных.ШтрихкодУпаковки = Неопределено
			Или Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(СтрокаДанных.ВидПродукции, Истина) Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого КлючИЗначение Из СтрокиДляЗаполнения Цикл
		...
		Если ДанныеЗаполненияПоЧастичномуВыбытию <> Неопределено
			И КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(СтрокаДанных.ВидПродукции, Истина) Тогда
		...
		Если ДанныеЗапросаГИСМТ <> Неопределено
			...
			Для Каждого КлючИЗначение Из ШаблонныйЭлементДляЗаполненияДанныхГИСМТ Цикл
				...
				Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
					СтрокаДанных, КлючИЗначение.Значение) Тогда
					Продолжить;
				КонецЕсли;
				
				Если СтрокаДанных.СтрокаДерева <> Неопределено Тогда
					Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаДанных.СтрокаДерева, КлючИЗначение.Значение) Тогда
...
Процедура ЗаполнитьПредставленияДанных(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	Если ПараметрыСканирования.ВидОперацииИСМП = Перечисления.ВидыОперацийИСМП.ПодтверждениеПоступленияКИЗ
		Или ПараметрыСканирования.ВидОперацииИСМП = Перечисления.ВидыОперацийИСМП.СписаниеЧерезСУЗМех Тогда
	...
	Если ПропускатьПоНаличиюВПуле
		И (ПроверятьСтруктуруКодовМаркировки
		   ...
		   И (Не ДанныеРазбора.Детализация.СодержитРазделительGS
		      ...
		      И КонструкторИСМПКлиентСервер.КодМаркировкиМожетВключатьЗаводскойСерийныйНомер(ДанныеШтрихкода.ВидПродукции)))
...
Процедура ЗаполнитьДанныеНеизвестныхУпаковок(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если ПрисутствуетПродукцияИСМП
			...
			И Не ЭтоЗаполненнаяУпаковкаСокращеннойДетализации Тогда
		...
		Если Не СтрокаДанных.Идентифицирован
			...
			И ЭтоЛогистическаяИлиГрупповаяУпаковка Тогда
...
Функция ВложенныеШтрихкодыУпаковок(ШтрихкодыУпаковок, ПараметрыСканирования, СортироватьДанные = Ложь) Экспорт
	...
	ШтрихкодированиеИССлужебный.ЗаполнитьПараметрыДополненияВложенныхШтрихкодов(
		ПараметрыДополнения, ШтрихкодыУпаковок, ПараметрыСканирования);
...
//  СортироватьДанные - Булево - Необходимость сортировки данных дерева.
// Возвращаемое значение:
//  См. ШтрихкодированиеИССлужебный.ИнициализацияРезультатаФормированияВложенныхШтрихкодов
Функция ВложенныеШтрихкоды(ДанныеШтрихкодовСписок, ПараметрыСканирования, ДляУпаковокТребоватьПодключениеМОТП = Истина, СортироватьДанные = Ложь) Экспорт
	...
	Иначе
		...
		ШтрихкодированиеОбщегоНазначенияИС.ЗаполнитьДанныеНеизвестнойМаркируемойПродукции(ДанныеПоШтрихкодам, ПараметрыСканирования);
...
Процедура СвернутьОднородныеОбъемноСортовыеКоды(ВложенныеШтрихкодыУпаковок, ШтрихкодыУПД) Экспорт
	...
	Для Каждого СтрокаGTIN Из ОбъемноСортовыеКоды Цикл
		...
		Для Индекс = 0 По ИсходныеСтроки.ВГраница() Цикл
			...
			Приемник.ПлановоеКоличествоПотребительскихУпаковок = Приемник.ПлановоеКоличествоПотребительскихУпаковок + ВУПД * Источник.ПлановоеКоличествоПотребительскихУпаковок;
...
Функция ВложенныеШтрихкодыПоДаннымИСМП(ДанныеШтрихкодовСписок, ПараметрыСканирования, НормализоватьВложенныеШтрихкоды = Истина,
	ДляУпаковокТребоватьПодключениеМОТП = Ложь, КешДанныхРазбора = Неопределено) Экспорт
	
	РезультатФормированияВложенныхШтрихкодов = ШтрихкодированиеИССлужебный.ИнициализацияРезультатаФормированияВложенныхШтрихкодов();
	РезультатФормированияВложенныхШтрихкодов.Вставить("СоответствиеСтрокДерева", Новый Соответствие);
	
	ДанныеПоШтрихкодам = ШтрихкодированиеОбщегоНазначенияИС.ИнициализацияДанныхПоШтрихкодам(Ложь);
	ШтрихкодированиеОбщегоНазначенияИС.ПодготовитьДанныеКодовМаркировкиДляОбработки(
		ДанныеПоШтрихкодам, ДанныеШтрихкодовСписок, ПараметрыСканирования, КешДанныхРазбора);
	...
	Если ПараметрыСканирования.ДопустимыПроверкиСеройЗоныМОТП
		...
		РезультатПроверкиСеройЗоны = ШтрихкодированиеМОТП.ПроверитьЛогистическиеИГрупповыеУпаковкиНаСодержаниеСерыхКодов(
...
Функция ПредставлениеОстаточныхШтрихкодов(СтрокиТаблицыДанных, ПараметрыСканирования) Экспорт
	...
	ТаблицаПредставлениеОстаточныхШтрихкодов.Колонки.Добавить("ТоварнаяГруппа",           Новый ОписаниеТипов("СправочникСсылка.ТоварныеГруппыГИСМТ"));
	...
	РегистрыСведений.КэшОписанияОстатковИСМП.ЗаполнитьТаблицуПредставленийGTINОстатки(
		ТаблицаПредставлениеОстаточныхШтрихкодов);
...
Процедура ЗаполнитьРасширенноеПредставлениеОшибки(ПредставлениеОшибки, ПараметрыДополнения, Форма) Экспорт
	...
	ЭтоПроверкаИПодборПродукцииИС = СтрНачинаетсяС(Форма.Параметры.ИмяФормыИсточник, "Обработка.ПроверкаИПодборПродукцииИС.Форма.ПроверкаИПодбор");
	...
	Если ПараметрыДополнения.ДанныеПроверкиНаККТ <> Неопределено Тогда
		...
		Если Не ПараметрыДополнения.ДанныеПроверкиНаККТ.ЭтоДокументОплаты
			...
			Если Пользователи.ЭтоПолноправныйПользователь()
				Или ПравоДоступа("Использование", Метаданные.Обработки.ПроверкаИПодборПродукцииИС) Тогда
				
				ИмяКомандыПроверкаПодбор = "ОткрытьФормуПроверкиИПодбора";
				
				Команда          = Форма.Команды.Добавить(ИмяКомандыПроверкаПодбор);
				...
				НовыйЭлемент.ИмяКоманды = ИмяКомандыПроверкаПодбор;
	...
	Если ЗначениеЗаполнено(ПараметрыДополнения.Статус) Тогда
		...
		Если ПараметрыДополнения.Статус = Перечисления.СтатусыКодовМаркировкиИСМП.Неопределен Тогда
		...
		Иначе
			...
			Если КодМаркировкиЗаблокированКонтролирующимиОрганами(ПараметрыДополнения.ОсобоеСостояние)
				Или КодМаркировкиЗаблокированКонтролирующимиОрганами(ПараметрыДополнения.Статус) Тогда
				Если Не ЗначениеЗаполнено(ПараметрыДополнения.СписокОрганизацийБлокировкиКМ) Тогда
					ТекстДополнительногоСообщенияБлокировки = НСтр("ru = 'Оборот товара был заблокирован органами государственной власти.'");
				Иначе
		...
		Если Не ВыведеноОтключение И Не ПараметрыДополнения.ЗапретитьОтключениеКонтроляСтатусов Тогда
			...
			Если ЭтоПроверкаИПодборПродукцииИС Тогда
	...
	Если ПараметрыДополнения.ЧастичноеВыбытиеНастроитьКарточкуНоменклатуры Тогда
		...
		Если ЗначениеЗаполнено(ПараметрыДополнения.ЧастичноеВыбытиеНоменклатура) Тогда
			...
			СоставСтроки.Добавить(" ");
			ПредставлениеСтрокиЧастичноеВыбытие = КонструкторИСМПОбщегоНазначенияКлиентСервер.ПредставлениеПродажаЧастичногоВыбытияПоВидуПродукции(
				Форма.ВидПродукции,
				"Винительный");
			СоставСтроки.Добавить(ПредставлениеСтрокиЧастичноеВыбытие);
...
Функция ЭтоОперацияВыводаИзОборота(Операция)
	...
	Операции.Добавить(Перечисления.ВидыОперацийИСМП.УдалитьВыводИзОборотаПересортицаПоКодам);
...
Функция ЭтоОперацияВозвратаВОборот(Операция)
	...
	Операции.Добавить(Перечисления.ВидыОперацийИСМП.УдалитьВозвратВОборотПриДистанционномСпособеПродажи);
	Операции.Добавить(Перечисления.ВидыОперацийИСМП.УдалитьВозвратВОборотПриПродажеЧерезВендинговыйАппарат);
	Операции.Добавить(Перечисления.ВидыОперацийИСМП.УдалитьВозвратВОборотТовараВыведенногоИзОборотаВЦеляхНеСвязанныхСРеализацией);
	Операции.Добавить(Перечисления.ВидыОперацийИСМП.УдалитьВозвратТовараВОборотПоСделкеСГосТайной);
	Операции.Добавить(Перечисления.ВидыОперацийИСМП.УдалитьВозвратВОборотТовараПриобретавшегосяНеДляПродажи);
	Операции.Добавить(Перечисления.ВидыОперацийИСМП.УдалитьВозвратВОборотТовараВыведенногоДляПроизводственныхЦелей);
...
Функция КодМаркировкиВыведенИзОборота(Статус) Экспорт
	...
	СтатусыКодМаркировкиВыведенИзОборота.Добавить(Перечисления.СтатусыКодовМаркировкиИСМП.Продан);
...
Функция КодМаркировкиЕщеНеВведенВОборот(Статус)
	...
	СтатусыКодМаркировкиНеВведенВОборот.Добавить(Перечисления.СтатусыКодовМаркировкиИСМП.НанесенНеОплачен);
	СтатусыКодМаркировкиНеВведенВОборот.Добавить(Перечисления.СтатусыКодовМаркировкиИСМП.НанесенОплачен);
...
Функция КодМаркировкиВОбороте(Статус) Экспорт
	...
	СтатусыКодМаркировкиВОбороте.Добавить(Перечисления.СтатусыКодовМаркировкиИСМП.ВведенВОборотВозвращен);
...
Функция ТребуетсяВыполнитьПеремаркировку(ПараметрыДополнения)
	...
	Если ПараметрыДополнения.СпособВводаВОборот = Перечисления.СпособыВводаВОборотСУЗ.Перемаркировка
		И (ПараметрыДополнения.Статус = Перечисления.СтатусыКодовМаркировкиИСМП.Нанесен
		Или ПараметрыДополнения.Статус = Перечисления.СтатусыКодовМаркировкиИСМП.НанесенНеОплачен
		Или ПараметрыДополнения.Статус = Перечисления.СтатусыКодовМаркировкиИСМП.НанесенОплачен
...
Процедура ОпределениеВидаПродукции(ДанныеПоШтрихкодам, ПараметрыСканирования, Принудительно = Ложь) Экспорт
	...
	Если ЭтоПроверкаКодовМаркировкиИСМП Тогда
		...
		Для Каждого СтрокаТаблицы Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.Штрихкод)
				Или Не ЗначениеЗаполнено(СтрокаТаблицы.GTIN)
				Или ЗначениеЗаполнено(СтрокаТаблицы.КодОКПД2) Тогда
				Продолжить;
			КонецЕсли;

			МассивСтрок = СтрокиПоGTIN[СтрокаТаблицы.GTIN];

			Если МассивСтрок = Неопределено Тогда
				МассивСтрок = Новый Массив;
				СтрокиПоGTIN[СтрокаТаблицы.GTIN] = МассивСтрок;
			КонецЕсли;
		...
		ЗаполнитьДанныеПродукцииПоДаннымСервиса(СтрокиПоGTIN, ДанныеПоШтрихкодам, ПараметрыСканирования);
	...
	РезультатЗапроса = ИнтерфейсМОТП.ЗапроситьСтатусыКодовМаркировки(
		...
		Ложь); // Разбирать вложения при определении товарной группы не требуется
		...
		Если ЗначениеЗаполнено(ВидПродукции) Тогда
			...
			Если СтрокаТаблицыДанных.ДанныеРазбора = Неопределено Тогда
				...
				ШтрихкодированиеОбщегоНазначенияИС.УточнитьДанныеРазбораПоВидуПродукции(
					СтрокаТаблицыДанных, СтрокаТаблицыДанных.ВидПродукции, ПараметрыСканирования, ОбщиеМодули, ЭтоСканированиеВФормуПроверкиИПодбора);
...
Процедура ДополнитьДанныеПоШтрихкодамДаннымиПулаКодовМаркировки(ДанныеПоШтрихкодам, ПараметрыСканирования, ВыполнитьПринудительно = Ложь) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаДанных.ПолныйКодМаркировки) И Не ТребуетсяПоискВПуле Тогда
			Продолжить;
		КонецЕсли;
	...
	Если КодыМаркировки.Количество() > 0 Тогда
		...
		КодыМаркировкиВПуле = ШтрихкодированиеИСМП.РезультатПоискаВПулеКодовМаркировки(
			КодыМаркировки, "Организация, ТоварнаяГруппа, ПолныйКодМаркировки, Номенклатура, Характеристика, GTIN, СпособВводаВОборот, ВидКИЗ, РазмерКИЗ, МаксимальнаяРозничнаяЦена");
		...
		Для Каждого СтрокаТаблицы Из КодыМаркировкиВПуле Цикл
			...
			Иначе
				СтрокаДанных.ЕстьВПулеКодовМаркировки = (СтрокаТаблицы.ПолныйКодМаркировки = СтрокаДанных.ПолныйКодМаркировки);
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.ВидПродукции)
				И КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(СтрокаТаблицы.ВидПродукции)
			...
			ИначеЕсли Не ЗначениеЗаполнено(СтрокаТаблицы.ВидПродукции)
				И КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(СтрокаТаблицы.ТоварнаяГруппа)
				И Не ЗначениеЗаполнено(СтрокаДанных.ПредставлениеНоменклатуры) Тогда
				СтрокаДанных.ПредставлениеНоменклатуры = РегистрыСведений.ПулКодовМаркировкиСУЗ.ПредставлениеКИЗ(СтрокаТаблицы);
			КонецЕсли;
			...
			Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(СтрокаДанных.ВидПродукции)
...
Функция ПроверитьНаОшибкиГрупповуюУпаковку(СтрокаДанных, СтрокаДерева, ПравилаПроверки) Экспорт
	...
	Если СтрокаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая Тогда
		...
		Если ПравилаПроверки.Свойство("КонтролироватьВложенныеТипыШтрихкодаГрупповыхУпаковок")
			И ДоступноНесколькоПотребительскихШаблоновПоТипуШтрихкодаДляВидаПродукции(СтрокаДанных.ВидПродукции) Тогда
...
Процедура СинхронизироватьРезультатПроверкиСредствамиККТ(ДеревоМаркированнойПродукции, СсылкаНаДокумент) Экспорт
...
Процедура ОчиститьДанныеРазрешительногоРежимаПриИзмененииОрганизации(ДокументОбъект) Экспорт
	...
	Если Не ДанныеШапкиЗаписываемогоОбъекта.Организация = ДанныеШапкиОбъектаВБазе.Организация Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПараметрыСканированияЗаписываемогоОбъекта, "ИмяКолонкиМРЦ")
			И ЗначениеЗаполнено(ПараметрыСканированияЗаписываемогоОбъекта.ИмяКолонкиМРЦ) Тогда
			ПустыеЗначенияКолонокКОчистке.Вставить(ПараметрыСканированияЗаписываемогоОбъекта.ИмяКолонкиМРЦ, "");
		КонецЕсли;
	...
	Если Не ПараметрыСканированияОбъектаВБазе.ИмяКолонкиРазрешительныйРежимИдентификаторЗапросаГИСМТ = ПараметрыСканированияЗаписываемогоОбъекта.ИмяКолонкиРазрешительныйРежимИдентификаторЗапросаГИСМТ
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПараметрыСканированияОбъектаВБазе, "ИмяКолонкиМРЦ")
			И ЗначениеЗаполнено(ПараметрыСканированияОбъектаВБазе.ИмяКолонкиМРЦ) Тогда
			ПустыеЗначенияКолонокКОчистке.Вставить(ПараметрыСканированияОбъектаВБазе.ИмяКолонкиМРЦ, "");
		КонецЕсли;
...
Процедура ЗаполнитьСтрокиТаблицыДанныхДаннымиЧастичногоВыбытия(ДанныеПоШтрихкодам, ДанныеЗаполнения, ПараметрыСканирования) Экспорт
	...
	Для Каждого СтрокаНастройки Из Настройки Цикл
		...
		Для Каждого СтрокаТаблицы Из ИсходнаяСтрока.СтрокиПоНоменклатуре Цикл
			...
			Если Не СтрокаТаблицы.ЭтоШтрихкодНоменклатуры
				И ОбщегоНазначенияИСМПКлиентСервер.ЗначениеПараметраСканированияЗапрашиватьДанныеСервисаИСМП(ПараметрыСканирования, СтрокаТаблицы.ОтключитьЗапросВСервисПоОперацииИПродукции) Тогда
...
Процедура НормализоватьДанныеВложенныхШтрихкодов(ВложенныеШтрихкоды, ПараметрыСканирования, ДанныеКодовМаркировки = Неопределено, ЗаполнятьСтрокиДерева = Ложь) Экспорт
	
	ДеревоУпаковок = ВложенныеШтрихкоды.ДеревоУпаковок;
	НормализоватьДанныеДереваУпаковок(ДеревоУпаковок, ПараметрыСканирования, ДанныеКодовМаркировки,, ЗаполнятьСтрокиДерева);
...
//  ДеревоУпаковок - ДеревоЗначений - Дерево упаковок, построенное в соответствие с хранением в справочнике ШтрихкодыУпаковок.
//  ПараметрыСканирования - (См. ШтрихкодированиеОбщегоНазначенияИСКлиент.ПараметрыСканирования).
Процедура СоздатьЭлементыШтрихкодовУпаковокПоДереву(ДеревоУпаковок, ПараметрыСканирования) Экспорт
...
Процедура ЗаполнитьУпаковкуВерхнегоУровняДерева(ДеревоУпаковок, УпаковкаВерхнегоУровня = Неопределено) Экспорт
...
Процедура РасширеннаяПроверкаНаОшибкиПоПравилам(ДанныеПоШтрихкодам, ПравилаПроверкиНаОшибки, ПараметрыСканирования, ЕстьОшибки) Экспорт
		...
		Если ПравилаПроверкиНаОшибки.Свойство("КонтрольСтатусов")
			...
			Для Каждого СтрокаДерева Из ПравилаПроверкиНаОшибки.КонтрольСтатусов.СтрокиСОшибками Цикл
				...
				Если ЗначениеЗаполнено(СтрокаДерева.Статус)
					...
					И СтрокаДерева.ОсобоеСостояние <> Перечисления.СтатусыКодовМаркировкиИСМП.Неопределен Тогда
					ТекстОшибки = СтрШаблон(
						НСтр("ru = 'Статус: %1 (%2)'"),
	...
	Если ПравилаПроверкиНаОшибки.Свойство("КонтрольРазрешительныхДокументовИСМП")
		...
		Для Каждого СтрокаДерева Из ПравилаПроверкиНаОшибки.КонтрольРазрешительныхДокументовИСМП Цикл
			...
			Если СтрокаДерева.ДанныеРазрешительнойДокументации <> Неопределено И ЗначениеЗаполнено(СтрокаДерева.ДанныеРазрешительнойДокументации.ОбщаяГруппаСтатуса) Тогда
...
Процедура ДополнитьДанныеПоШтрихкодамСпецификойЧастичногоВыбытия(ДанныеПоШтрихкодам, ПараметрыСканирования)
	...
	Если Не ПараметрыСканирования.ИспользуетсяСоответствиеШтрихкодовСтрокДерева
		...
		И ДанныеПоШтрихкодам.ВложенныеШтрихкоды.Свойство("МаркированныеТовары") Тогда
...
Процедура ПроверитьНаОшибкиСтрокиДерева(СтрокиДерева, ПравилаПроверкиНаОшибки, ПараметрыСканирования, ДанныеПоШтрихкодам, ЕстьОшибки)
	...
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		...
		Если ПравилаПроверкиНаОшибки.Свойство("КонтрольОкончанияСрокаГодности") Тогда
			...
			Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСоСрокамиГодности(СтрокаДерева.ВидПродукции)
		...
		Если ПравилаПроверкиНаОшибки.Свойство("КонтрольРазрешительныхДокументовИСМП")
			...
			И ШтрихкодированиеОбщегоНазначенияИСМП.РазрешительнаяДокументацияТребуетПроверкиПоДатеПроизводства(СтрокаДерева) Тогда
			
			ПропуститьВложенныеУпаковкиГрупповойЛогистическойУпаковки = СтрокаДерева.Родитель <> Неопределено
				И (СтрокаДерева.Родитель.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая
			...
			Если Не ПропуститьВложенныеУпаковкиГрупповойЛогистическойУпаковки Тогда
				...
				ИначеЕсли СтрокаДерева.ДанныеРазрешительнойДокументации = Неопределено Тогда
...
Процедура ПроверитьДанныеСтрокиПоСрокуГодности(СтрокаДанных, ПравилоПроверки, ПараметрыСканирования)
	...
	Если Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСОбязательнымУказаниемСроковГодности(СтрокаДанных.ВидПродукции) Тогда
		Возврат;
	КонецЕсли;
	
	Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСОбязательнымУказаниемСроковГодности(СтрокаДанных.ВидПродукции)
		И ИнтеграцияИСМПВЕТИСКлиентСервер.ЭтоВидПродукцииИСМПВЕТИС(СтрокаДанных.ВидПродукции) Тогда
...
Процедура ДополнитьДанныеПоШтрихкодамСпецификойБезЗапросаСтатусовИСМП(ДанныеПоШтрихкодам, ПараметрыСканирования, МассивСтрок = Неопределено) Экспорт
...
Функция СтрокиТаблицыДанныхДляПолученияСпецифики(ДанныеКодовМаркировки, СпецификаШтрихкодовГрупповыхИЛогистическихУпаковок, ПараметрыСканирования) Экспорт
...
Функция ВозможнаПродукцияИСМП(СтрокаДанных, ВключатьТабачнуюПродукцию = Истина)
	
	Если СтрокаДанных.ЭтоШтрихкодНоменклатуры Тогда
		Возврат Ложь;
	ИначеЕсли ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаДанных.ВидПродукции, ВключатьТабачнуюПродукцию) Тогда
		Возврат Истина;
	ИначеЕсли ЗначениеЗаполнено(СтрокаДанных.ВидПродукции) Тогда
		Возврат Ложь;
	ИначеЕсли ВключатьТабачнуюПродукцию И СтрокаДанных.ПрисутствуетТабачнаяПродукция Тогда // Флаг заполняется если неизвестно АТП это или классический табак
		Возврат Истина;
	КонецЕсли;
	
	Для Каждого ВидПродукции Из СтрокаДанных.ВидыПродукцииКодаМаркировки Цикл
		Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(ВидПродукции, ВключатьТабачнуюПродукцию) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПривестиДанныеМаркированныхТоваровВСоответствиеДеревуУпаковок(МаркированныеТовары) Экспорт
...
Процедура ДополнитьТаблицуДанныеКодовМаркировкиСпецификой(
	...
	Если СпецификаШтрихкодовПотребительскихУпаковок <> Неопределено Тогда
		...
		Для Каждого КлючЗначение Из СпецификаШтрихкодовПотребительскихУпаковок Цикл
			...
			ВидПродукцииДоЗаполнения = СтрокаТаблицыДанных.ВидПродукции;
			...
			Для Каждого КлючИЗначение Из СпецификаСтроки Цикл
				...
				ДобавитьСвойствоКЗаполнению = (ЗначениеЗаполнено(КлючИЗначение.Значение) И ЕстьСвойство);
				
				Если ДобавитьСвойствоКЗаполнению
					И ИмяСвойства = "ПредставлениеНоменклатуры"
					И ЗначениеЗаполнено(СтрокаТаблицыДанных[ИмяСвойства]) Тогда
					ДобавитьСвойствоКЗаполнению = Ложь;
				КонецЕсли;
				
				Если ДобавитьСвойствоКЗаполнению Тогда
			...
			Если ОбщегоНазначенияИСКлиентСерверПовтИсп.ВидПродукцииСоответствуетКлючу(
				СтрокаТаблицыДанных.ВидПродукции, ВидПродукцииДоЗаполнения) Тогда
				СтрокаТаблицыДанных.ВидПродукции = ВидПродукцииДоЗаполнения;
			КонецЕсли;
...
Процедура НормализоватьДанныеДереваУпаковок(ДеревоУпаковок, ПараметрыСканирования, ДанныеКодовМаркировки = Неопределено, ТолькоПересчетНоменклатурыИКоличества = Ложь, ЗаполнятьСтрокиДерева = Ложь) Экспорт
	...
	Если ПараметрыСканирования.ЭтоПроверкаКодовМаркировкиИСМП Тогда
		...
		ШтрихкодированиеОбщегоНазначенияИС.НормализироватьДанныеДереваУпаковокПоПереданнымКолонкам(ДеревоУпаковок, КоллекцияДополнительныхКолонок, ДобавленныеКолонки);
	КонецЕсли;
	
	ПараметрыДерева = ШтрихкодированиеОбщегоНазначенияМОТП.ПараметрыПостроенияДерева(ПараметрыСканирования);
	
	Кэш = Новый Структура;
	Кэш.Вставить("КэшНоменклатуры",     Новый Соответствие);
	Кэш.Вставить("ЗапросИУстановкаМРЦ", Новый Массив);
	Кэш.Вставить("ВычислениеМРЦ",       Новый Массив);
	
	Кэш.Вставить("ДобавленныеКолонки", ДобавленныеКолонки);
	Кэш.Вставить("ЗаполняемыеКолонки", Неопределено);
	
	Кэш.Вставить("ТолькоПересчетНоменклатурыИКоличества",      ТолькоПересчетНоменклатурыИКоличества);
	Кэш.Вставить("КолонкиДляПересчетаНоменклатурыИКоличества", Новый Соответствие);
	Кэш.Вставить("ЗаполнятьСтрокиДерева",                      ЗаполнятьСтрокиДерева);
	
	Кэш.КолонкиДляПересчетаНоменклатурыИКоличества.Вставить("Номенклатура", Истина);
	Кэш.КолонкиДляПересчетаНоменклатурыИКоличества.Вставить("Характеристика", Истина);
	Кэш.КолонкиДляПересчетаНоменклатурыИКоличества.Вставить("Серия", Истина);
	Кэш.КолонкиДляПересчетаНоменклатурыИКоличества.Вставить("Количество", Истина);
	Кэш.КолонкиДляПересчетаНоменклатурыИКоличества.Вставить("КоличествоПотребительскихУпаковок", Истина);
	Кэш.КолонкиДляПересчетаНоменклатурыИКоличества.Вставить("КоличествоГрупповыхУпаковок", Истина);
	Кэш.КолонкиДляПересчетаНоменклатурыИКоличества.Вставить("МаркируемаяПродукция", Истина);
	Кэш.КолонкиДляПересчетаНоменклатурыИКоличества.Вставить("ПроизвольнаяЕдиницаУчета", Истина);
	Кэш.КолонкиДляПересчетаНоменклатурыИКоличества.Вставить("ТребуетВзвешивания", Истина);
	Кэш.КолонкиДляПересчетаНоменклатурыИКоличества.Вставить("ТипУпаковки", Истина);
	
	ПараметрыЗаполнения = ПараметрыЗаполненияСтрокДерева();
	ПараметрыЗаполнения.ДанныеКодовМаркировки = ДанныеКодовМаркировки;
	ПараметрыЗаполнения.ПараметрыДерева       = ПараметрыДерева;
	ПараметрыЗаполнения.ПараметрыСканирования = ПараметрыСканирования;
	ПараметрыЗаполнения.ВариантПолученияМРЦ   = ПараметрыСканирования.ВариантПолученияМРЦ;
	ПараметрыЗаполнения.Кэш                   = Кэш;
	
	Если ТипЗнч(ДеревоУпаковок) = Тип("СтрокаДереваЗначений") Тогда
		ДеревоЗначенийДляОбхода = Новый Структура;
		ДеревоЗначенийДляОбхода.Вставить("Строки", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДеревоУпаковок));
	Иначе
		ДеревоЗначенийДляОбхода = ДеревоУпаковок;
	КонецЕсли;
	
	ЗаполнитьСтрокиДерева(ДеревоЗначенийДляОбхода, ПараметрыЗаполнения);
	
	ДопустимаТабачнаяПродукция = ШтрихкодированиеОбщегоНазначенияИС.ДопустимаТабачнаяПродукция(ПараметрыСканирования);
	Если ДопустимаТабачнаяПродукция Тогда
		ШтрихкодированиеМОТП.ЗапросИРасчетМРЦПоСтрокамДерева(
...
Процедура ПараметрыСлиянияДанных(ПараметрыЗаполнения, Приемник, Источник) Экспорт
	...
	Если ПараметрыЗаполнения.Кэш.ЗаполняемыеКолонки = Неопределено Тогда
		...
		ЗаполняемыеКолонки.Вставить("КолонкиИББезСервиса",   "");
		...
		ЗаполняемыеКолонки.КолонкиИББезСервиса  = СтрСоединить(КолонкиИББезСервиса, ",");
...
Процедура ЗаполнитьСтрокиДерева(СтрокаВерхнегоУровня, ПараметрыЗаполнения,
	...
	Для Каждого СтрокаДерева Из СтрокаВерхнегоУровня.Строки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаДерева.ВидПродукции)
			И Не ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаДерева.ВидПродукции, Истина) Тогда
			Продолжить;
		КонецЕсли;
		...
		ВложенныеПараметрыЗаполнения = ПараметрыЗаполненияСтрокДерева();
		ВложенныеПараметрыЗаполнения.ДанныеКодовМаркировки = ПараметрыЗаполнения.ДанныеКодовМаркировки;
		ВложенныеПараметрыЗаполнения.ПараметрыДерева       = ПараметрыЗаполнения.ПараметрыДерева;
		ВложенныеПараметрыЗаполнения.ПараметрыСканирования = ПараметрыЗаполнения.ПараметрыСканирования;
		ВложенныеПараметрыЗаполнения.Кэш                   = ПараметрыЗаполнения.Кэш;
		ВложенныеПараметрыЗаполнения.ВариантПолученияМРЦ   = ПараметрыЗаполнения.ВариантПолученияМРЦ;
		
		Если ПараметрыДерева.УчитыватьМРЦ Тогда
			Если ЭтоКодМаркировкиБлока Тогда
				Если ЗначениеЗаполнено(СтрокаДерева.МРЦ) Тогда
					ВложенныеПараметрыЗаполнения.ВариантПолученияМРЦ = "ВычислениеОтРодителя";
				ИначеЕсли ПараметрыЗаполнения.ВариантПолученияМРЦ = "ВычислениеИЗапрос" Тогда
					ПараметрыЗаполнения.Кэш.ЗапросИУстановкаМРЦ.Добавить(СтрокаДерева);
					ВложенныеПараметрыЗаполнения.ВариантПолученияМРЦ = "ВычислениеОтРодителя";
				КонецЕсли;
			ИначеЕсли ПараметрыЗаполнения.ВариантПолученияМРЦ = "ВычислениеОтРодителя" Тогда
				ПараметрыЗаполнения.Кэш.ВычислениеМРЦ.Добавить(СтрокаДерева);
			КонецЕсли;
		КонецЕсли;
		
		ЗаполнитьСтрокиДерева(
			СтрокаДерева, ВложенныеПараметрыЗаполнения, УпаковкаВерхнегоУровня, Ложь);
...
Процедура ЗаполнитьСтрокуПоДаннымРазбораКодаМаркировки(СтрокаДанных, СтрокаДанныхДополнительно, ПараметрыСканирования) Экспорт
	...
	ИначеЕсли КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(СтрокаДанных.ВидПродукции)
...
Процедура ЗаполнитьХарактеристикиСтрокиДереваПоПодчиненнымСтрокам(СтрокаДерева, СтрокаДанных)
	...
	Если ТипЗнч(СтрокаДерева.ТипУпаковки) = Тип("ПеречислениеСсылка.ПрочиеЗоныПересчетаПродукцииИС") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСтрокуДереваПоДаннымТаблицыКодыМаркировок(СтрокаДерева, СтрокаДанных, ПараметрыЗаполнения)
	...
	Если Не ПустаяСтрока(ЗаполняемыеКолонки.КолонкиИББезСервиса)
		И Не ЗначениеЗаполнено(СтрокаДерева.Статус) Тогда

		ЗаполнитьЗначенияСвойств(СтрокаДерева, СтрокаДанных, ЗаполняемыеКолонки.КолонкиИББезСервиса);
...
Процедура ОбработатьДеревоПолученноеИзИСМП(ДеревоУпаковок, ДанныеПоШтрихкодам, ПараметрыСканирования, ПараметрыОбработки) Экспорт
	...
	КонецЕсли;
	ДополнительныеКолонки.Вставить("ВидПродукции", Новый ОписаниеТипов("СправочникСсылка.ВидыПродукцииИС"));
	ДополнительныеКолонки.Вставить("ВидУпаковки",  Новый ОписаниеТипов("ПеречислениеСсылка.ВидыУпаковокИС"));
	ДополнительныеКолонки.Вставить("ТипШтрихкода", Новый ОписаниеТипов("ПеречислениеСсылка.ТипыШтрихкодов"));
	...
	Изменения.Вставить("ДобавленныеСтроки", Новый Массив);
	Изменения.Вставить("ИзмененныеСтроки",  Новый Массив);
	Изменения.Вставить("ЗаполненGTIN",      Новый Массив);
	Изменения.Вставить("СодержитСерыеКоды", Ложь);
	...
	ДополнитьСтрокиДанныхКодовМаркировкиПоДаннымПродукцииEAN(ДанныеПоШтрихкодам, ПараметрыСканирования);

КонецПроцедуры

Процедура ДополнитьСтрокиДанныхКодовМаркировкиПоДаннымПродукцииEAN(ДанныеПоШтрихкодам, ПараметрыСканирования)

	Если Не ПараметрыСканирования.ЭтоПроверкаКодовМаркировкиИСМП Тогда
		Возврат;
	КонецЕсли;

	РезультатЗапросаПоEAN = ДанныеПоШтрихкодам.РезультатЗапросаПоEAN;
	Если РезультатЗапросаПоEAN = Неопределено Тогда
		ДанныеПоШтрихкодам.РезультатЗапросаПоEAN = Новый Соответствие;
		РезультатЗапросаПоEAN = ДанныеПоШтрихкодам.РезультатЗапросаПоEAN;
	КонецЕсли;
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаДанных.GTIN) Тогда
			Продолжить;
		КонецЕсли;

		ДанныеПоGTIN = РезультатЗапросаПоEAN[СтрокаДанных.GTIN];

		Если ДанныеПоGTIN = Неопределено Тогда

			МассивСтрок = СтрокиПоGTIN[СтрокаДанных.GTIN];

			Если МассивСтрок = Неопределено Тогда
				МассивСтрок = Новый Массив;
				СтрокиПоGTIN[СтрокаДанных.GTIN] = МассивСтрок;
			КонецЕсли;
		...
		Иначе
			...
			ЗаполнитьСтрокуИнформациейОПродукцииПоДаннымСервиса(СтрокаДанных, ДанныеПоGTIN);
...
Процедура ЗаполнитьДанныеПродукцииПоДаннымСервиса(СтрокиПоGTIN, ДанныеПоШтрихкодам, ПараметрыСканирования)

	ДанныеДляЗапросаПоGTIN = Новый Массив;

	Для Каждого КлючИЗначение Из СтрокиПоGTIN Цикл
		GTIN = КлючИЗначение.Ключ;
		ДанныеДляЗапросаПоGTIN.Добавить(GTIN);
	КонецЦикла;

	Если ДанныеДляЗапросаПоGTIN.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;

	РезультатЗапросаПоEAN = ИнтерфейсИСМПОбщегоНазначения.ДанныеПродукцииПоШтрихкодуEAN(
		ДанныеДляЗапросаПоGTIN,
		ПараметрыСканирования.Организация);

	Если РезультатЗапросаПоEAN.ТребуетсяОбновлениеКлючаСессии Тогда
		ДанныеПоШтрихкодам.ТребуетсяОбновлениеКлючаСессии = РезультатЗапросаПоEAN.ТребуетсяОбновлениеКлючаСессии;
		ДанныеПоШтрихкодам.ТекстОшибки                    = РезультатЗапросаПоEAN.ТекстОшибки;
		ДанныеПоШтрихкодам.ЕстьОшибки                     = Истина;
		Возврат;
	ИначеЕсли ЗначениеЗаполнено(РезультатЗапросаПоEAN.ТекстОшибки) Тогда
		ДанныеПоШтрихкодам.ТекстОшибки = РезультатЗапросаПоEAN.ТекстОшибки;
		ДанныеПоШтрихкодам.ЕстьОшибки  = Истина;
		Возврат;
	Иначе

		Если ДанныеПоШтрихкодам.РезультатЗапросаПоEAN = Неопределено Тогда
			ДанныеПоШтрихкодам.РезультатЗапросаПоEAN = РезультатЗапросаПоEAN.ДанныеПродукцииПоШтрихкодуEAN;
		Иначе
			ДанныеПоШтрихкодам.РезультатЗапросаПоEAN = Новый Соответствие();
			Для Каждого КлючИЗначение Из РезультатЗапросаПоEAN.ДанныеПродукцииПоШтрихкодуEAN Цикл
				ДанныеПоШтрихкодам.РезультатЗапросаПоEAN[КлючИЗначение.Ключ] = КлючИЗначение.Значение;
			КонецЦикла;
		КонецЕсли;
		...
		Для Каждого КлючИЗначение Из СтрокиПоGTIN Цикл
			...
			ДанныеПоGTIN = РезультатЗапросаПоEAN.ДанныеПродукцииПоШтрихкодуEAN[GTIN];
			...
			Для Каждого СтрокаТаблицы Из МассивСтрок Цикл
				ЗаполнитьСтрокуИнформациейОПродукцииПоДаннымСервиса(СтрокаТаблицы, ДанныеПоGTIN);
			КонецЦикла;
...
Процедура ЗаполнитьСтрокуИнформациейОПродукцииПоДаннымСервиса(СтрокаТаблицы, ДанныеПоGTIN)

	Если ЗначениеЗаполнено(ДанныеПоGTIN.ОКПД2)
		И Не ЗначениеЗаполнено(СтрокаТаблицы.КодОКПД2) Тогда
		СтрокаТаблицы.КодОКПД2 = ДанныеПоGTIN.ОКПД2;
	КонецЕсли;
	Если ЗначениеЗаполнено(ДанныеПоGTIN.ГруппаОКПД2)
		И Не ЗначениеЗаполнено(СтрокаТаблицы.ГруппаОКПД2) Тогда
		СтрокаТаблицы.ГруппаОКПД2 = ДанныеПоGTIN.ГруппаОКПД2;
	КонецЕсли;

	СтрокаТаблицы.КодАлкогольнойПродукции       = ДанныеПоGTIN.КодАлкогольнойПродукции;
	СтрокаТаблицы.ПодлежитВетеринарномуКонтролю = ДанныеПоGTIN.ПодлежитВетеринарномуКонтролю;
	...
	Если ЗначениеЗаполнено(ДанныеПоGTIN.ОКПД2)
		И Не ЗначениеЗаполнено(СтрокаТаблицы.СтрокаДерева.КодОКПД2) Тогда
		СтрокаТаблицы.СтрокаДерева.КодОКПД2 = ДанныеПоGTIN.ОКПД2;
	КонецЕсли;
	Если ЗначениеЗаполнено(ДанныеПоGTIN.ГруппаОКПД2)
		И Не ЗначениеЗаполнено(СтрокаТаблицы.СтрокаДерева.ГруппаОКПД2) Тогда
		СтрокаТаблицы.СтрокаДерева.ГруппаОКПД2 = ДанныеПоGTIN.ГруппаОКПД2;
	КонецЕсли;

	СтрокаТаблицы.СтрокаДерева.КодАлкогольнойПродукции       = ДанныеПоGTIN.КодАлкогольнойПродукции;
	СтрокаТаблицы.СтрокаДерева.ПодлежитВетеринарномуКонтролю = ДанныеПоGTIN.ПодлежитВетеринарномуКонтролю;
...
Процедура ЗаполнитьПредставлениеНоменклатурыПоДаннымИСМП(ДанныеПоШтрихкодам, СтрокиДанныхДляСопоставления, СписокEAN) Экспорт
...
Процедура СопоставитьКодыМаркировкиНоменклатуреПоДаннымУПД(ДанныеКодовМаркировки, ПараметрыСканирования, ЭлектронныйДокумент = Неопределено, ТипЭД = "УПД") Экспорт
	...
	Если Не ПараметрыСканирования.ВыполняетсяЗагрузкаДанныхВФормуПроверкиИПодбора Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого КлючЗначение Из ДанныеНоменклатурыПоДаннымУПД Цикл
		...
		Если ТипЗнч(ВидПродукции) = Тип("СправочникСсылка.КлючиВидовПродукцииИС") Тогда
			ВидыПродукцииКлюча = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииКлюча(ВидПродукции, "ГИСМТ");
			...
			КонецЕсли;
		Иначе
			ВидПродукцииКлюча = ВидПродукции;
		КонецЕсли;
		
		ДанныеРазбора = РазборКодаМаркировкиИССлужебный.РазобратьКодМаркировки(ШтрихкодУпаковки, ВидПродукцииКлюча);
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Если СведенияПоEAN = Неопределено Тогда
			...
			Иначе
				...
				ОтметитьСопоставлениеПоУПД = Не УпрощеннаяИнтеграцияБЭД
					Или (УпрощеннаяИнтеграцияБЭД И ЗначениеЗаполнено(СведенияПоEAN.Номенклатура));
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаДанных, "НоменклатураСопоставленаПоУПД")
					И ОтметитьСопоставлениеПоУПД Тогда
...
Процедура ЗаполнитьДанныеКодовМаркировкиПоДаннымУПД(ДанныеКодовМаркировки, ПараметрыСканирования) Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаНаОбъект)
		...
		ЭтоДокументПриобретения = ПроверкаИПодборПродукцииИС.ЭтоДокументПриобретения(СсылкаНаОбъект);
		ЭтоАктОРасхождениях     = ПроверкаИПодборПродукцииИС.ЭтоАктОРасхождениях(СсылкаНаОбъект);
...
Процедура ЗаполнитьДобавленныеСтрокиПоНоменклатуреСопоставленнойПоУПД(ДанныеКодовМаркировки, ДобавленныеСтроки, ПараметрыСканирования) Экспорт
	...
	Для Каждого СтрокаДанных Из ДобавленныеСтроки Цикл
		...
		ОтметитьСопоставлениеПоУПД = Не УпрощеннаяИнтеграцияБЭД
					Или (УпрощеннаяИнтеграцияБЭД И ЗначениеЗаполнено(СтрокаЗаполнения.Номенклатура));
		...
		СтрокаДанных.НоменклатураСопоставленаПоУПД = ОтметитьСопоставлениеПоУПД;
...
//   и (См. НормализоватьСвойстваКоллекцииВложенныхШтрихкодов).
Функция ИнициализацияВложенныхШтрихкодов(ДеревоУпаковок = Неопределено, ИнициализироватьМаркированныеТовары = Истина, ПараметрыСканирования = Неопределено) Экспорт
...
Функция ИнициализацияРезультатаПолученияВложенныхШтрихкодовПоДаннымИСМП() Экспорт
...
Функция ДанныеМаркируемойНоменклатурыПоДаннымУПД(УчетныйДокумент = Неопределено, ЭлектронныйДокумент = Неопределено, ТипЭД = "УПД") Экспорт
	
	Если ЗначениеЗаполнено(УчетныйДокумент) Или ИнтеграцияИС.УпрощеннаяИнтеграцияБЭД() Тогда
...
Функция ДанныеМаркируемойНоменклатурыПоДаннымДереваУПД_2019(ДеревоДокумента)
	...
	ПутьКИндивидуальнымУпаковкам155После = "СведенияОТоварах.НомерСтроки.СведенияОМаркировкеПосле.КодыИндивидуальныхУпаковок";
	ПутьКИндивидуальнымУпаковкам820 = "СведенияОТоварах.НомерСтроки.СведенияОМаркировке.ИндивидуальныеУпаковки";
	ПутьКИндивидуальнымУпаковкам820После = "СведенияОТоварах.НомерСтроки.СведенияОМаркировкеПосле.ИндивидуальныеУпаковки";
	...
	ПутьККоличеству                 = "СведенияОТоварах.НомерСтроки.Количество";
	
	ПутьКИЗ = "СведенияОТоварах.НомерСтроки.СведенияОМаркировке.КонтрольныеИдентификационныеЗнаки";
	ПутьКИЗПосле = "СведенияОТоварах.НомерСтроки.СведенияОМаркировкеПосле.КонтрольныеИдентификационныеЗнаки";
	ПутьТранспортныеУпаковки = "СведенияОТоварах.НомерСтроки.СведенияОМаркировке.ТранспортныеУпаковки";
	ПутьТранспортныеУпаковкиПосле = "СведенияОТоварах.НомерСтроки.СведенияОМаркировкеПосле.ТранспортныеУпаковки";
	...
	Для Каждого СтрокаТовары Из СведенияОТоварах.Строки Цикл
		...
		Количество = СтрокаТовары.Строки.Найти(ПутьККоличеству, "ПолныйПуть", Истина);
		Если Количество = Неопределено Тогда
			Количество = 0;
		Иначе
			Количество = Количество.Значение;
		КонецЕсли;
		...
		ДанныеНоменклатуры.Количество                = Количество;
		...
		Если ИндивидуальныеУпаковки = Неопределено Тогда
			ИндивидуальныеУпаковки = СтрокаТовары.Строки.Найти(ПутьКИндивидуальнымУпаковкам820После, "ПолныйПуть", Истина);
		КонецЕсли;
		Если ИндивидуальныеУпаковки = Неопределено Тогда
			ИндивидуальныеУпаковки = СтрокаТовары.Строки.Найти(ПутьКИндивидуальнымУпаковкам155, "ПолныйПуть", Истина);
		КонецЕсли;
		Если ИндивидуальныеУпаковки = Неопределено Тогда
			ИндивидуальныеУпаковки = СтрокаТовары.Строки.Найти(ПутьКИндивидуальнымУпаковкам155После, "ПолныйПуть", Истина);
		КонецЕсли;
		
		КИЗ = СтрокаТовары.Строки.Найти(ПутьКИЗ, "ПолныйПуть", Истина);
		Если КИЗ = Неопределено Тогда
			КИЗ = СтрокаТовары.Строки.Найти(ПутьКИЗПосле, "ПолныйПуть", Истина);
		КонецЕсли;
		Если КИЗ <> Неопределено Тогда
		...
		Если ТранспортныеУпаковки = Неопределено Тогда
			ТранспортныеУпаковки = СтрокаТовары.Строки.Найти(ПутьТранспортныеУпаковкиПосле, "ПолныйПуть", Истина);
		КонецЕсли;
		Если ТранспортныеУпаковки <> Неопределено Тогда
...
Функция ДанныеМаркируемойНоменклатурыПоДаннымДереваУПД_5_02(ДеревоДокумента)
	...
	ПутьККоличеству     = "ТаблицаСчетаФактуры.СведенияОбОтгруженныхПозициях.НомерСтроки.Количество";
...
//   * МультитоварнаяУпаковка - Булево - Истина, если агрегат содержит разные позиции номенклатуры.
//   * ВидУпаковки - ПеречислениеСсылка.ВидыУпаковокИС, Неопределено - вид упаковки.
//   * ВидПродукции - СправочникСсылка.ВидыПродукцииИС, Неопределено - вид продукции, значение для заполнения.
Процедура ЗаполнитьВидПродукцииДанныхЭДО(ДанныеНоменклатурыПоДаннымЭДО)
	...
	ТоварыПоУПД.Колонки.Добавить("Количество",             ОбщегоНазначения.ОписаниеТипаЧисло(18,3));
...
Процедура ДополнитьСоответствиеКодовМаркировкиНоменклатуре_5_02(СоответствиеКодаМаркировкиНоменклатуре, ДанныеУпаковок, ДанныеНоменклатуры, ГТИН)
	...
	Для Каждого ДанныеУпаковки Из ДанныеУпаковок.Строки Цикл
		...
		Если Не КоличествоПоОСУ = Неопределено И ЗначениеЗаполнено(КоличествоПоОСУ) И Не ГТИН = Неопределено Тогда
			...
			КодОСУ = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ОСУПоШтрихкодуEAN(
				ГТИН, КоличествоПоОСУ, Ложь);
			ДанныеНоменклатуры.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая;
...
// Получает данные кодов маркировки и соответствующую кодам номенклатуру и характеристику информационной базы по данным
//   УПД учетного или электронного документа (или УКД, ТОРГ2). Тип документа определяется в параметре.
// 
// Параметры:
//  УчетныйДокумент - ЛюбаяСсылка - Документ, формируемый на основании УПД.
//  ЭлектронныйДокумент - ДокументСсылка - УПД (УКД, ТОРГ2), если работа напрямую с электронным документом.
//                      - Неопределено - будет получен из учетного.
//                      - ДокументСсылка.ЭлектронныйДокументИсходящийЭДО - возвращается та жк ссылка.
//                      - ДокументСсылка.ЭлектронныйДокументВходящийЭДО - возвращается та же ссылка.
...
Функция ДанныеМаркируемойНоменклатурыКодовМаркировкиПоДаннымЭДО(УчетныйДокумент = Неопределено, ЭлектронныйДокумент = Неопределено, ТипДокумента = "УПД") Экспорт
	
	Если Не ЗначениеЗаполнено(УчетныйДокумент) И Не ЗначениеЗаполнено(ЭлектронныйДокумент) Тогда
		Возврат Неопределено;
	КонецЕсли;
...
//   * Брак - Булево - используется при загрузке из торг2.
//   * Недостача - Булево - используется при загрузке из торг2.
Функция ТаблицаТоваровКодовМаркировкиПоДаннымДереваЭДО(ДеревоДокумента, ТипЭлектронногоДокумента) Экспорт
	...
	Иначе
		...
		ПутьККоличеству      = "СведенияОТоварах.НомерСтроки.Количество";
		ПутьКНаименованию    = "СведенияОТоварах.НомерСтроки.ТоварНаименование";
		ПутьККодуЕдиницы     = "СведенияОТоварах.НомерСтроки.ЕдиницаИзмеренияКод";
		ПутьКЕдинице         = "СведенияОТоварах.НомерСтроки.ЕдиницаИзмеренияНаименование";
		
		Если ТипЭлектронногоДокумента = "УПД" Тогда
			ПутьКУпаковкам = ИнициализироватьЗаполнитьПутьКУпаковкамУПД();
		ИначеЕсли ТипЭлектронногоДокумента = "УКДДо" Тогда
			ПутьКУпаковкам = ИнициализироватьЗаполнитьПутьКУпаковкамУКД(, Ложь);
		ИначеЕсли ТипЭлектронногоДокумента = "УКДПосле" Тогда
			ПутьКУпаковкам = ИнициализироватьЗаполнитьПутьКУпаковкамУКД(Ложь);
		Иначе
	...
	Для Каждого СтрокаТовары Из СведенияОТоварах.Строки Цикл
		...
		ДанныеНоменклатуры.Вставить("Количество",     0);
		ДанныеНоменклатуры.Вставить("НомерСтроки",    0);
		ДанныеНоменклатуры.Вставить("ТоварНаименование",              "");
		ДанныеНоменклатуры.Вставить("ЕдиницаИзмеренияКод",            "");
		ДанныеНоменклатуры.Вставить("ЕдиницаИзмеренияНаименование",   "");
		
		Если ТипЭлектронногоДокумента <> "ТОРГ2" Тогда
			
			ДанныеНоменклатуры.НомерСтроки = СтрокаТовары.Значение;
			
			Поля = Новый Соответствие;
			Поля.Вставить("Количество",                   ПутьККоличеству);
			Поля.Вставить("ТоварНаименование",            ПутьКНаименованию);
			Поля.Вставить("ЕдиницаИзмеренияКод",          ПутьККодуЕдиницы);
			Поля.Вставить("ЕдиницаИзмеренияНаименование", ПутьКЕдинице);
			Для Каждого Поле Из Поля Цикл
				СтрокаПоля = СтрокаТовары.Строки.Найти(Поле.Значение, "ПолныйПуть", Истина);
				Если СтрокаПоля <> Неопределено Тогда
					ДанныеНоменклатуры[Поле.Ключ] = СтрокаПоля.Значение;
				КонецЕсли;
			КонецЦикла;
...
Функция ТаблицаТоваровКодовМаркировкиПоДаннымДереваЭДО_5_02(ДеревоДокумента, ТипЭлектронногоДокумента) Экспорт
	...
	ПутьККоличеству   = "ТаблицаСчетаФактуры.СведенияОбОтгруженныхПозициях.НомерСтроки.Количество";
	ПутьКНаименованию = "ТаблицаСчетаФактуры.СведенияОбОтгруженныхПозициях.НомерСтроки.ТоварНаименование";
	ПутьККодуЕдиницы  = "ТаблицаСчетаФактуры.СведенияОбОтгруженныхПозициях.НомерСтроки.ЕдиницаИзмеренияКод";
	ПутьКЕдинице      = "ТаблицаСчетаФактуры.СведенияОбОтгруженныхПозициях.НомерСтроки.ЕдиницаИзмеренияНаименование";
...
Функция ИнициализироватьТаблицуТоваровУПДУКД(ТипЭлектронногоДокумента = "УПД") Экспорт
	...
	ТаблицаТовары.Колонки.Добавить("ВидПродукции",        Новый ОписаниеТипов("СправочникСсылка.КлючиВидовПродукцииИС"));
	ТаблицаТовары.Колонки.Добавить("Количество",          ОбщегоНазначения.ОписаниеТипаЧисло(18, 3));
	ТаблицаТовары.Колонки.Добавить("НомерСтроки",         ОбщегоНазначения.ОписаниеТипаЧисло(10, 0));
	ТаблицаТовары.Колонки.Добавить("ТоварНаименование",   ОбщегоНазначения.ОписаниеТипаСтрока(255));
	ТаблицаТовары.Колонки.Добавить("ЕдиницаИзмеренияКод", ОбщегоНазначения.ОписаниеТипаСтрока(50));
	ТаблицаТовары.Колонки.Добавить("ЕдиницаИзмеренияНаименование", ОбщегоНазначения.ОписаниеТипаСтрока(50));
...
Функция ИнициализироватьТаблицуТоваровТОРГ2() Экспорт
...
Функция ИнициализироватьЗаполнитьПутьКУпаковкамУКД(ДоКорректировки = Истина, ПослеКорректировки = Истина)
	
	ПутьКУпаковкам = Новый Соответствие;
	
	Если ПослеКорректировки Тогда
...
Функция ИнициализацияДанныхНоменклатурыДляУПД()
	...
	ДанныеНоменклатуры.Вставить("Количество",                Неопределено);
...
Процедура ДополнитьДанныеПоШтрихкодамУпаковок(ДанныеКодовМаркировки, ВложенныеШтрихкоды, ПараметрыСканирования) Экспорт
	...
	Если ПараметрыСканирования.ВыполняетсяЗагрузкаДанныхВФормуПроверкиИПодбора
		...
		И ПроверкаИПодборПродукцииИС.ЭтоДокументПриобретения(ПараметрыСканирования.СсылкаНаОбъект)
	...
	ШтрихкодыУпаковок = ШтрихкодированиеИС.ДанныеШтрихкодовУпаковок(ДанныеКодовМаркировки, ПараметрыСканирования, ТипыУпаковок, Истина);
...
Функция ДоступноНесколькоПотребительскихШаблоновПоТипуШтрихкодаДляВидаПродукции(ВидПродукции)
	
	Возврат ИнтеграцияИСМППовтИсп.ДоступноНесколькоПотребительскихШаблоновПоТипуШтрихкода().Найти(
		КонструкторИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппаПоВидуПродукции(ВидПродукции)) <> Неопределено;
	
КонецФункции

Функция ПараметрыЗаполненияСтрокДерева()
	
	ПараметрыЗаполнения = Новый Структура;
	ПараметрыЗаполнения.Вставить("ДанныеКодовМаркировки");
	ПараметрыЗаполнения.Вставить("ПараметрыДерева");
	ПараметрыЗаполнения.Вставить("ПараметрыСканирования");
	ПараметрыЗаполнения.Вставить("ВариантПолученияМРЦ");
	ПараметрыЗаполнения.Вставить("Кэш");
