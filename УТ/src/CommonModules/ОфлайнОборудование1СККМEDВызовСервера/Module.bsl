...
//
Функция КомпонентыОбмена(НаправлениеОбмена, ВерсияФорматаОбмена, РасширениеФормата, ВерсияФорматаОбменаПриЗагрузке = "", РасширениеФорматаПриЗагрузке = "") 
	...
	Если ОбщегоНазначенияБПО.ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными") Тогда
		...
		ТекВерсияФормата     = ?(НаправлениеОбмена = "Отправка", ВерсияФорматаОбмена, ВерсияФорматаОбменаПриЗагрузке);
...
Функция ВыгрузитьДанныеВXML(СтруктураПараметров)
	...
	КомпонентыОбмена = КомпонентыОбмена("Отправка", Параметры.ВерсияФорматаОбмена, СтруктураПараметров.РасширениеФормата);
...
Процедура ЗаписатьВФайлПризнакОбработан(КомпонентыОбмена)
	...
	ТипОбъекта = ФабрикаXDTO.Тип("http://v8.1c.ru/edi/edi_stnd/EnterpriseData/" + КомпонентыОбмена.ВерсияФорматаОбмена, СтрокаТипОбъекта);
...
Процедура ЗагрузкаСообщения(СтруктураПараметров, АдресРезультата) Экспорт
	...
	ЭкземплярОборудования = СтруктураПараметров.ЭкземплярОборудования;
	...
	РезультатЗагрузки = ЗагрузитьДанныеИзXML(ЧтениеXML, ЭкземплярОборудования);
...
//
Функция ЗагрузитьДанныеИзXML(ЧтениеXML, ЭкземплярОборудования)
	...
	КомпонентыОбмена.ПараметрыКонвертации.Вставить("ЭкземплярОборудования", ЭкземплярОборудования);
...
Процедура ЗаполнитьСписокОбъектовКВыгрузке(КомпонентыОбмена, СтруктураПараметров) 
	...
	Если ПрайсЛист.Свойство("Товары") Тогда
		ИменаРеквизитовТовар = Новый Структура;

		Для каждого Товар Из ПрайсЛист.Товары Цикл

			Если Не ИменаРеквизитовТовар.Свойство("Ссылка") Тогда
				МетаданныеОбъекта = Товар.СсылкаНоменклатура.Метаданные();

				Для Каждого Реквизит Из МетаданныеОбъекта.СтандартныеРеквизиты Цикл
					ИменаРеквизитовТовар.Вставить(Реквизит.Имя);
				КонецЦикла;
			...
			ТоварСтруктура = ОбщегоНазначенияБПО.ЗначенияРеквизитовОбъекта(Товар.СсылкаНоменклатура, ИменаРеквизитовТовар);
			...
			Если Товар.ИмеетХарактеристики Тогда
				Для каждого Характеристика Из Товар.Характеристики Цикл
					...
					Если Характеристика.ИмеетУпаковки Тогда
						...
						ИменаРеквизитовУпаковкаХарактеристики = Новый Структура;
						Для каждого Упаковка Из Характеристика.Упаковки Цикл

							Если Не ИменаРеквизитовУпаковкаХарактеристики.Свойство("Ссылка") Тогда
								МетаданныеОбъекта = Упаковка.СсылкаУпаковки.Метаданные();

								Для Каждого Реквизит Из МетаданныеОбъекта.СтандартныеРеквизиты Цикл
									ИменаРеквизитовУпаковкаХарактеристики.Вставить(Реквизит.Имя);
								КонецЦикла;
							...
							УпаковкаСтруктура = ОбщегоНазначенияБПО.ЗначенияРеквизитовОбъекта(Упаковка.СсылкаУпаковки, ИменаРеквизитовУпаковкаХарактеристики);
...
Функция ВыгрузитьДанныеНаСервере(ДанныеВыгрузки) Экспорт
	...
	Возврат МодульДлительныеОперации.ВыполнитьВФоне(
		"ОфлайнОборудование1СККМEDВызовСервера.РезультатВыгрузкиВXML", 
		СтруктураПараметров, 
		ПараметрыВыполнения);
	// Конец Вызов БСП
...
// Возвращаемое значение:
//  Неопределено.
Функция ЗагрузитьДанныеНаСервере(ПараметрыОперации) Экспорт
	...
	Возврат МодульДлительныеОперации.ВыполнитьВФоне(
		"ОфлайнОборудование1СККМEDВызовСервера.ЗагрузкаСообщения", 
		ПараметрыОперации, 
		ПараметрыВыполнения);
	// Конец Вызов БСП
...
Процедура ПакетыОбработаны(Отказ, ПакетыОбработаны, Пакеты, ВыходныеПараметры, ПараметрыОборудования) Экспорт
	
	Результат = Истина;
	
	ВерсияФорматаОбмена = ПараметрыОборудования.ВерсияФорматаОбмена;
	URIПространстваИмен = "http://v8.1c.ru/edi/edi_stnd/EnterpriseData/" + ВерсияФорматаОбмена;
	СтрокаТипОбъекта = "Обработан";
...
Функция ПолучитьТекстXMLДанныеЗагружены(Параметры) Экспорт
	...
	Если ОбщегоНазначенияБПО.ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными") Тогда
		...
		ТекВерсияФормата     = Параметры.ВерсияФорматаОбмена;
		...
		ТипОбъекта = ФабрикаXDTO.Тип(КомпонентыОбмена.XMLСхема, СтрокаТипОбъекта);
