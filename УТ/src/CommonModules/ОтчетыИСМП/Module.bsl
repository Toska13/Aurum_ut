	...
	ИначеЕсли ИмяОтчета = Метаданные.Отчеты.СведенияОбОтклоненияхИСМП.Имя Тогда
		...
		Если КлючПредопределенногоВарианта = "РасшифровкаПоКодамМаркировки" Тогда
			...
			Если НастройкиОтчета.ДополнительныеСвойства.Свойство("ФормаПараметрыОтбор") 
				...
				НастройкиОтчета.ДополнительныеСвойства.ФормаПараметрыОтбор.Свойство("ТоварнаяГруппа", ТоварнаяГруппа);
			КонецЕсли;
			
			Если ТоварнаяГруппа = Неопределено
				И СписокОтчетов.Количество() Тогда
				ТоварнаяГруппа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СписокОтчетов.Получить(0).Значение, "ТоварнаяГруппа");
			КонецЕсли;
			
			Если Не ИнтеграцияИСМПВЕТИСКлиентСервер.ЭтоВидПродукцииИСМПВЕТИС(
				КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ТоварнаяГруппа)) Тогда
