...
Процедура ДополнитьТаблицуТоваров(Форма, ПараметрыИнтеграции, ПутьКРеквизиту, ДобавляемыеРеквизиты) Экспорт
	...
	Колонка = Новый РеквизитФормы(
		"ВидПродукцииИС",
		Новый ОписаниеТипов("СправочникСсылка.ВидыПродукцииИС"),
		ПутьКРеквизиту, 
...
Процедура ДобавитьКоманднуюПанельСверкиКодовМаркировки(Форма, ПараметрыИнтеграции) Экспорт
	...
	ИмяКоманднойПанели = ИмяКоманднойПанели(ПараметрыИнтеграции.ИмяКоманднойПанели);
...
Процедура ДобавитьКнопкуСверкиКодовМаркировки(Форма, ПараметрыИнтеграции) Экспорт
	...
	Кнопка = Форма.Элементы.Вставить(
		...
		Тип("КнопкаФормы"),
		Форма.Элементы[ИмяКоманднойПанели(ПараметрыИнтеграции.ИмяКоманднойПанели)],
		ПоследующийЭлемент);
...
//   * ИмяКоманднойПанели                         - Строка - при размещении кнопки сверки КМ в сущствующем подменю.
//   * ИмяПоследующегоЭлементаФормыПодменю        - Строка - элемент, перед которым должен быть вставлена кнопка сверки.
//
...
//    * Признан - ПеречислениеСсылка.ВариантыДействийПоРасхождениямКодовМаркировкиИСМП - Обязательная колонка, если 4й параметр Истина. Вариант действия для строки расхождения.
...
// Возвращаемое значение:
//  См. НоваяТаблицаМаркируемойПродукции
//
...
//  * Номенклатура - ОпределяемыйТип.Номенклатура - номенклатура.
//
Функция СформироватьТаблицуПродукцииПринятойБезРасхождений(ДокументОснование, НоменклатураПоДаннымЭДО) Экспорт
	
	Результат = Новый ТаблицаЗначений;
	Результат.Колонки.Добавить("Номенклатура", Метаданные.ОпределяемыеТипы.Номенклатура.Тип);
	
	СверкаКодовМаркировкиИСМППереопределяемый.ПриОпеределенииТаблицыПродукцииПринятойБезРасхождений(ДокументОснование, НоменклатураПоДаннымЭДО, Результат);
...
Функция ИнициализироватьТаблицуТоваровЗначенийШтрихкодов() Экспорт
	...
	ТаблицаТовары.Колонки.Добавить("ВидПродукции",      Новый ОписаниеТипов("СправочникСсылка.ВидыПродукцииИС"));
...
//  * Признан           - ПеречислениеСсылка.ВариантыДействийПоРасхождениямКодовМаркировкиИСМП - варинаты действий по расхождениям.
//
Функция ИнициализацияТаблицыШтрихкодыУпаковок(ИспользоватьРасхождения = Ложь) Экспорт
	...
	Если ИспользоватьРасхождения Тогда
		...
		ТЗ.Колонки.Добавить("Признан",        Новый ОписаниеТипов("ПеречислениеСсылка.ВариантыДействийПоРасхождениямКодовМаркировкиИСМП"));
	КонецЕсли;
...
Функция НоваяТаблицаМаркируемойПродукции()
	...
	ТаблицаМаркируемойПродукции.Колонки.Добавить("ВидПродукции",   Новый ОписаниеТипов("СправочникСсылка.КлючиВидовПродукцииИС"));
...
Функция ТаблицыТОРГ2ПланФактПоДеревуУпаковокИСМП(ДеревоМаркированнойПродукции, НедопустимыеШтрихкоды,
	...
	Для Каждого СтрокаДерева Из ДеревоМаркированнойПродукции.Строки Цикл
		...
		НормализованныйШтрихкод = СтрокаДерева.НормализованныйШтрихкод;
		...
		Иначе
			...
			Иначе
				...
				Иначе
					...
					Для Каждого СтрокаТаблицы Из ВложенныйРезультат.ШтрихкодыУпаковокФактЭДО Цикл
						СтрокаТаблицы.ЗначениеШтрихкода = ЗначениеШтрихкода;
						СтрокаТаблицы.НормализованныйШтрихкод = НормализованныйШтрихкод;
						СтрокаТаблицы.ВидУпаковки = СтрокаДерева.ВидУпаковки;
					КонецЦикла;
...
Функция ИнициализацияТаблицыПриемки() Экспорт
	...
	ТЗ.Колонки.Добавить("НормализованныйШтрихкод", ОбщегоНазначения.ОписаниеТипаСтрока(200));
...
Функция КолонкиГруппировкиТаблицыТОРГ2()
	Возврат "Номенклатура, Характеристика, ЗначениеШтрихкода, НормализованныйШтрихкод, ВидУпаковки, Брак";
КонецФункции
...
#Область ДанныеПечатнойФормы

// Сформировать печатную форму расхождения по кодам маркировки.
// 
// Параметры:
//  ДанныеПечати - Структура - Данные печати
//  Макет - ТабличныйДокумент - Макет
// 
// Возвращаемое значение:
//  ТабличныйДокумент - Сформировать печатную форму расхождения по кодам маркировки
Функция СформироватьПечатнуюФормуРасхожденияПоКодамМаркировки(ДанныеПечати, Макет) Экспорт
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.АвтоМасштаб         = Ложь;
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_РасхожденияПоКодамМаркировкиИСМП";
	
	ОбластьМакетаШапка            = Макет.ПолучитьОбласть("Шапка");
	ОбластьМакетаЗаголовокТаблицы = Макет.ПолучитьОбласть("Заголовок");
	ОбластьМакетаСтрока           = Макет.ПолучитьОбласть("Строка");
	
	ОбластьМакетаЗаголовокТаблицы.Параметры.ПредставлениеЭД = Строка(ДанныеПечати.ПроверяемыйДокумент);
	ТабличныйДокумент.Вывести(ОбластьМакетаЗаголовокТаблицы);
	
	ТабличныйДокумент.Вывести(ОбластьМакетаШапка);
	
	НомерСтроки = 0;
	Для Каждого ЭлементКоллекции Из ДанныеПечати.РасхожденияПоКодам Цикл
		
		НомерСтроки = НомерСтроки +1;
		ОбластьМакетаСтрока.Параметры.Номер = НомерСтроки;
		ОбластьМакетаСтрока.Параметры.ПредставлениеНоменклатуры = ЭлементКоллекции.Ключ;
		Если ЭлементКоллекции.Значение.ИзлишкиКМ.Количество() Тогда
			ОбластьМакетаСтрока.Параметры.ИзлишкиКМ = СтрСоединить(ЭлементКоллекции.Значение.ИзлишкиКМ, ", ")
		Иначе
			ОбластьМакетаСтрока.Параметры.ИзлишкиКМ = "-";
		КонецЕсли;
		
		Если ЭлементКоллекции.Значение.НедостачиКМ.Количество() Тогда
			ОбластьМакетаСтрока.Параметры.НедостачиКМ = СтрСоединить(ЭлементКоллекции.Значение.НедостачиКМ, ", ")
		Иначе
			ОбластьМакетаСтрока.Параметры.НедостачиКМ = "-";
		КонецЕсли;
	...
	ПолнаяДатаДокумента = Формат(ТекущаяДатаСеанса(), "ДЛФ=DD");
...
// 
// Параметры:
//  ПараметрыОкончанияПроверки - Структура - Параметры окончания проверки
// 
// Возвращаемое значение:
//  Неопределено,
//  Соответствие из КлючИЗначение:
//    Ключ - Строка - Представление номенклатуры
//    Значение - Структура - ИзлишкиКМ, НедостачиКМ.
Функция РасхожденияПриПриемке(ПараметрыОкончанияПроверки) Экспорт
	
	ШтрихкодыУпаковокПланЭДО = Неопределено;
	Если Не ПараметрыОкончанияПроверки.Свойство("ШтрихкодыУпаковокПланЭДО", ШтрихкодыУпаковокПланЭДО)
		Или ШтрихкодыУпаковокПланЭДО = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ШтрихкодыУпаковокПланЭДО", ПараметрыОкончанияПроверки.ШтрихкодыУпаковокПланЭДО);
	Запрос.УстановитьПараметр("ШтрихкодыУпаковокФактЭДО", ПараметрыОкончанияПроверки.ШтрихкодыУпаковокФактЭДО);
	Запрос.УстановитьПараметр("ТаблицаПодобраннойПровереннойПродукции", ПараметрыОкончанияПроверки.ТаблицаПодобраннойПровереннойПродукции);
	Запрос.Текст = "ВЫБРАТЬ
				|	ШтрихкодыУпаковокПланЭДО.Номенклатура КАК Номенклатура,
				|	ШтрихкодыУпаковокПланЭДО.Характеристика КАК Характеристика,
				|	ШтрихкодыУпаковокПланЭДО.НормализованныйШтрихкод КАК ЗначениеШтрихкода,
				|	ШтрихкодыУпаковокПланЭДО.КоличествоНедостача КАК КоличествоНедостача,
				...
				|	ШтрихкодыУпаковокФактЭДО.Номенклатура КАК Номенклатура,
				|	ШтрихкодыУпаковокФактЭДО.Характеристика КАК Характеристика,
				|	ШтрихкодыУпаковокФактЭДО.НормализованныйШтрихкод КАК ЗначениеШтрихкода,
				|	ШтрихкодыУпаковокФактЭДО.КоличествоИзлишек КАК КоличествоИзлишек,
				|	ШтрихкодыУпаковокФактЭДО.КоличествоБрак КАК КоличествоБрак,
				|	ШтрихкодыУпаковокФактЭДО.КоличествоПринято КАК КоличествоПринято,
				|	ШтрихкодыУпаковокФактЭДО.Брак КАК Брак
				...
				|////////////////////////////////////////////////////////////////////////////////2
				|ВЫБРАТЬ
				|	ТаблицаПодобраннойПровереннойПродукции.Номенклатура КАК Номенклатура
				|ПОМЕСТИТЬ ПроверяемаяНоменклатура
				|ИЗ
				|	&ТаблицаПодобраннойПровереннойПродукции КАК ТаблицаПодобраннойПровереннойПродукции
				|
				|ИНДЕКСИРОВАТЬ ПО
				|	Номенклатура
				|;
				...
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПроверяемаяНоменклатура КАК ПроверяемаяНоменклатура
				|		ПО ТаблицаРасхождений.Номенклатура = ПроверяемаяНоменклатура.Номенклатура
				|УПОРЯДОЧИТЬ ПО
				|	Номенклатура,
				|	Характеристика";
	...
	РасхожденияПоКодам = Новый Соответствие;
	
	Пока ВыборкаРасхождения.Следующий() Цикл
		
		ПредставлениеНоменклатуры = ОбщегоНазначенияИС.ПредставлениеНоменклатуры(
			ВыборкаРасхождения.Номенклатура,
			ВыборкаРасхождения.Характеристика);
		
		РезультатПроверки = РасхожденияПоКодам[ПредставлениеНоменклатуры];
		Если РезультатПроверки = Неопределено Тогда
			РезультатПроверки = Новый Структура;
			РезультатПроверки.Вставить("ИзлишкиКМ",   Новый Массив);
			РезультатПроверки.Вставить("НедостачиКМ", Новый Массив);
			РасхожденияПоКодам[ПредставлениеНоменклатуры] = РезультатПроверки;
		КонецЕсли;
		
		Если ВыборкаРасхождения.Излишек Тогда
			РезультатПроверки.ИзлишкиКМ.Добавить(ВыборкаРасхождения.ЗначениеШтрихкода);
		Иначе
			РезультатПроверки.НедостачиКМ.Добавить(ВыборкаРасхождения.ЗначениеШтрихкода);
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат РасхожденияПоКодам;
...
Функция ИмяКоманднойПанели(ИмяКоманднойПанели)
	Если ЗначениеЗаполнено(ИмяКоманднойПанели) Тогда
		Возврат ИмяКоманднойПанели;
	КонецЕсли;
	Возврат "КоманднаяПанельСверкаКодовМаркировкиПродукцииГосИС";
КонецФункции
