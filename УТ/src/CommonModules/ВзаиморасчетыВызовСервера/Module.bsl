...
Процедура ФормаПередЗаписьюНаКлиентеСервер(Форма) Экспорт
	...
	Для Каждого СтруктураПараметров Из ДополненныеПараметры.МассивПараметров Цикл
		...
		Если СтруктураПараметров.Свойство("ИсточникСуммТабличнаяЧасть") И СтруктураПараметров.ИсточникСуммТабличнаяЧасть Тогда
			...
			СтруктураПараметров.Вставить("СуммыДокумента", ВзаиморасчетыСлужебный.СуммыДокумента(Форма.Объект,СтруктураПараметров));
		КонецЕсли;
	...
	ЭтапыОплатыСервер.ПроверитьЗаполнитьЭтапыГрафикаОплаты(Форма, ДополненныеПараметры);
	ВзаиморасчетыРаботаСФормами.ОбновитьТекстГиперссылкиЭтапыОплаты(Форма);
КонецПроцедуры
...
// 
// Параметры:
//  ДанныеФормыСтруктура - ДанныеФормыСтруктура - Данные формы в которую нужно вернуть адрес ТЗ.
// 
// Возвращаемое значение:
//  Строка - Адрес во временном хранилище.
Функция ПоместитьСуммыПоЗаказамВоВременноеХранилище(ЗНАЧ ДанныеФормыСтруктура, ЗНАЧ СтруктураПараметров) Экспорт
	Если СтруктураПараметров.ИсточникСуммТабличнаяЧасть Тогда
		Возврат ПоместитьВоВременноеХранилище(ВзаиморасчетыСервер.ТаблицаСуммПоЗаказам(ДанныеФормыСтруктура, СтруктураПараметров), Новый УникальныйИдентификатор());
	Иначе
		Возврат "";
	КонецЕсли;
КонецФункции
