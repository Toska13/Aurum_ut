...
//   * Текст - Строка, Массив из Строка - Вводимый в поле выбора номер для поиска. Массив номеров УИП из клиент-банк.
...
Процедура РассчитатьРеквизитыФормыДетальныхЗаписей(ТаблицаФормы, ЕстьРасхождения, ТекущаяСтрока = Неопределено) Экспорт
	...
	Для каждого СтрокаТабличнойЧасти Из МассивСтрок Цикл
		...
		Если НЕ ЕстьРасхождения Тогда
			СтрокаТабличнойЧасти.ТипРасхождений = 0;	// Сопоставленные
		ИначеЕсли СтрокаТабличнойЧасти.ЕстьДанныеОрганизации И НЕ СтрокаТабличнойЧасти.ЕстьДанныеКонтрагента
			ИЛИ НЕ СтрокаТабличнойЧасти.ЕстьДанныеОрганизации И СтрокаТабличнойЧасти.ЕстьДанныеКонтрагента
			ИЛИ СтрокаТабличнойЧасти.НомерДокумента <> СтрокаТабличнойЧасти.НомерДокументаКонтрагент
...
Функция ПредставлениеФормыОплаты(ФормаОплаты)
	...
	ИначеЕсли ФормаОплаты = ПредопределенноеЗначение("Перечисление.ФормыОплаты.СистемаБыстрыхПлатежей") Тогда
		Представление = НСтр("ru='К оплате СБП'");
	КонецЕсли;
...
Функция МассивДатОтгрузок(Форма, СтруктураПараметров) Экспорт
	
	Если Не СтруктураПараметров.ЭтоЗаказ Тогда
		Возврат Новый Массив;
	КонецЕсли;
	
	ДатыОтгрузки = ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Форма, СтруктураПараметров.ДатаОтгрузки);
	
	Если ТипЗнч(ДатыОтгрузки) = Тип("Массив") Тогда
		
		ТабличнаяЧасть = ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Форма, СтруктураПараметров.ПутьКДаннымТЧ);
		...
		Возврат ДатыОтгрузки;
		
	ИначеЕсли ЗначениеЗаполнено(ДатыОтгрузки) Тогда
		
		Возврат ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДатыОтгрузки);
