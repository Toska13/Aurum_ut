...
Функция ИспользуетсяДоставка() Экспорт
	...
	Возврат ДоставкаТоваровВызовСервераПовтИсп.ИспользуетсяДоставка();
...
Функция ИспользуетсяДоставка20() Экспорт
	...
	Возврат ДоставкаТоваровВызовСервераПовтИсп.ИспользуетсяДоставка20();
...
Функция ИспользуетсяДоставка25() Экспорт
	...
	Возврат ДоставкаТоваровВызовСервераПовтИсп.ИспользуетсяДоставка25();
	
КонецФункции

// Определяет по хозяйственной операции необходимость доставки в документе
// 
// Параметры:
//  Операция - ПеречислениеСсылка.ХозяйственныеОперации - 
// 
// Возвращаемое значение:
//  Булево - Хозяйственная операция участвует в доставке
//
Функция ХозяйственнаяОперацияУчаствуетВДоставке(Операция) Экспорт
	
	ЭтоОперацияДокументаБезДоставки =
		(Операция = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПустаяСсылка")
			ИЛИ Операция = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионера")
			ИЛИ Операция = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионераБезПереходаПраваСобственности")
			ИЛИ Операция = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.РеализацияКлиентуРеглУчет")
			ИЛИ Операция = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПереходПраваСобственности")
			ИЛИ Операция = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоставкаПодПринципала"));
	
	Возврат НЕ ЭтоОперацияДокументаБезДоставки;
...
//  ИспользоватьУправлениеДоставкой - Булево - признак того, что используется Управление доставкой.
// 
// Возвращаемое значение:
//  Структура - Получить пустую структуру реквизитов доставки:
// * СпособДоставки - ПеречислениеСсылка.СпособыДоставки - 
// * СпособДоставкиОбъект - ПеречислениеСсылка.СпособыДоставки - 
...
// * Курьер - СправочникСсылка.ФизическиеЛица - 
//
Функция ПолучитьПустуюСтруктуруРеквизитовДоставки(ДокОбъект = Неопределено, ИспользоватьУправлениеДоставкой = Истина) Экспорт
...
//
// Параметры:
//	Список - СписокЗначений из Произвольный - список в котором выполняется поиск
//	ИмяПоляСтруктуры - Строка - имя искомого свойства структуры
//	ИскомоеЗначение - Произвольный - значение, которое требуется найти
//
// Возвращаемое значение:
// 	- Неопределено -
//	- Структура - структура, значения свойств которой совпадает с искомым, если таких несколько возвращается первая найденная.
//
Функция НайтиВСпискеСтруктур(Список, ИмяПоляСтруктуры, ИскомоеЗначение) Экспорт
	
	Перем НайденноеЗначение; // Произвольный
...
Процедура ОчиститьСпискиВыбораАдресовПолучателяОтправителя(ЭлементыФормы) Экспорт
	...
	Если ЭлементыФормы.Найти("АдресДоставкиСамовывоз") <> Неопределено Тогда
		ЭлементыФормы.АдресДоставкиСамовывоз.СписокВыбора.Очистить();
	КонецЕсли;
...
//
// Возвращаемое значение:
//	Массив из см. ОписаниеРаспоряженияНаДоставку
//
Функция ОписанияРаспоряженийНаДоставку() Экспорт
	
	Описания = Новый Массив; // Массив из см. ОписаниеРаспоряженияНаДоставку
...
Функция ПредставлениеПолучателяОтправителя(ПолучательОтправитель, Перевозчик, ВидДоставки) Экспорт
	...
	Если ЗначениеЗаполнено(Перевозчик) Тогда
		...
		Текст = СтрЗаменить(Текст, "%ПолучательОтправитель%", Строка(ПолучательОтправитель));
		Возврат СтрЗаменить(Текст, "%Перевозчик%", Строка(Перевозчик));
...
Функция ДопИнфоИзмененоПользователем(ЭлементыФормы, ДокОбъект) Экспорт
	...
	ИначеЕсли ЭлементыФормы.Найти("АдресДоставки") <> Неопределено Тогда
		...
		ДопИнфоИзменено = Не ДопИнфоЕстьВСпискеВыбораЭлемента(ЭлементыФормы.АдресДоставки, ДопИнфо);
...
//
// Параметры:
//  Форма - см. Обработка.РабочееМестоМенеджераПоДоставке.Форма.Форма
//  	  - см. Обработка.РабочееМестоМенеджераПоДоставке.Форма.ФормаРаспоряженияПоПункту
//  	  - см. Документ.ЗаданиеНаПеревозку.Форма.ФормаДокумента  
...
Процедура ОтобразитьНедогрузПерегруз(Форма, ДанныеВесОбъем) Экспорт
	...
	Если ДанныеВесОбъем = Неопределено
	 ИЛИ ДанныеВесОбъем.ГрузоподъемностьВЕдПользователя = 0 Тогда
		...
		Форма.НадписьНедогрузВес = Новый ФорматированнаяСтрока("");
		Элементы.ДекорацияВес.Картинка = Новый Картинка;
		
	ИначеЕсли НедогрузВес >= 0 Тогда
		
		Текст = СтрЗаменить(Шаблон, "%Количество%", Строка(НедогрузВес));
		Текст = СтрЗаменить(Текст, "%ЕдиницаИзмерения%", Строка(Форма.ЕдиницаИзмеренияВеса));
		
		Форма.НадписьНедогрузВес = Новый ФорматированнаяСтрока(Текст, ЖирныйШрифт, ЦветЗеленый);
	...
	Иначе
		...
		Текст = СтрЗаменить(Шаблон, "%Количество%", Строка(-НедогрузВес));
		Текст = СтрЗаменить(Текст, "%ЕдиницаИзмерения%", Строка(Форма.ЕдиницаИзмеренияВеса));
		Форма.НадписьНедогрузВес = Новый ФорматированнаяСтрока(Текст, ЖирныйШрифт, ЦветКрасный);
	...
	Если ДанныеВесОбъем = Неопределено
	 ИЛИ ДанныеВесОбъем.ВместимостьВЕдПользователя = 0 Тогда
		...
		Форма.НадписьНедогрузОбъем = Новый ФорматированнаяСтрока("");
		Элементы.ДекорацияОбъем.Картинка = Новый Картинка;
		
	ИначеЕсли НедогрузОбъем >= 0 Тогда
		
		Текст = СтрЗаменить(Шаблон, "%Количество%", Строка(НедогрузОбъем));
		Текст = СтрЗаменить(Текст, "%ЕдиницаИзмерения%", Строка(Форма.ЕдиницаИзмеренияОбъема));
		Форма.НадписьНедогрузОбъем = Новый ФорматированнаяСтрока(Текст, ЖирныйШрифт, ЦветЗеленый);
	...
	Иначе
		...
		Текст = СтрЗаменить(Шаблон, "%Количество%", Строка(-НедогрузОбъем)); 
		Текст = СтрЗаменить(Текст, "%ЕдиницаИзмерения%", Строка(Форма.ЕдиницаИзмеренияОбъема));
		Форма.НадписьНедогрузОбъем = Новый ФорматированнаяСтрока(Текст, ЖирныйШрифт, ЦветКрасный);
...
// Возвращает пустую структуру параметров распоряжения.
// 
// Возвращаемое значение:
//  Структура - Параметры распоряжения:
// * СпособДоставки - ПеречислениеСсылка.СпособыДоставки - 
// * АдресДоставки - Строка - 
// * АдресДоставкиЗначение - Строка - 
// * АдресДоставкиЗначенияПолей - Строка - 
...
// * ОсобыеУсловияПеревозкиОписание - Строка - 
// * Курьер - СправочникСсылка.ФизическиеЛица - 
// * Ссылка - Неопределено, ДокументСсылка, СправочникСсылка - Ссылка на распоряжение
// * Дата - Дата - 
...
//
Функция ПараметрыРаспоряжения() Экспорт
	
	Параметры = ПолучитьПустуюСтруктуруРеквизитовДоставки();
	...
	Параметры.Вставить("Контрагент", ПредопределенноеЗначение("Справочник.Контрагенты.ПустаяСсылка"));
	Параметры.Вставить("Грузополучатель", ПредопределенноеЗначение("Справочник.Контрагенты.ПустаяСсылка"));
	Параметры.Вставить("Соглашение", ПредопределенноеЗначение("Справочник.СоглашенияСКлиентами.ПустаяСсылка"));
	Параметры.Вставить("Договор", ПредопределенноеЗначение("Справочник.ДоговорыКонтрагентов.ПустаяСсылка"));
	Параметры.Вставить("РеализацияПоЗаказам", Ложь);
...
// Новые параметры настройки доставки.
// 
// Возвращаемое значение:
//  Структура - Новые параметры настройки доставки:
// * ВидПланированияДоставки - ПеречислениеСсылка.ВидыПланированияДоставки -
// * РазрешитьИзменениеВидаПланированияДоставки - Булево - 
// * СпособУказанияДоставки - ПеречислениеСсылка.СпособыУказанияДоставки -
// * РазрешитьИзменениеСпособаУказанияДоставки - Булево - 
// * СпособДоставки - ПеречислениеСсылка.СпособыДоставки25 - 
// * ИспользуемыеСпособыДоставки - Массив из ПеречислениеСсылка.СпособыДоставки25 - не должно использоваться вместе с ИсключаемыеСпособыДоставки
// * ИсключаемыеСпособыДоставки - Массив из ПеречислениеСсылка.СпособыДоставки25 - не должно использоваться вместе с ИспользуемыеСпособыДоставки
// * Партнер - СправочникСсылка.Партнеры - 
...
// * ВремяДоставкиС - Дата, ОписаниеТипов - 
// * ВремяДоставкиПо - Дата, ОписаниеТипов - 
// * ДополнительнаяИнформацияПоДоставке - Строка - 
// * ОсобыеУсловияПеревозки - Булево - 
// * ОсобыеУсловияПеревозкиОписание - Строка - 
// * Курьер - СправочникСсылка.ФизическиеЛица -
// * ИспользуетсяЕдинаяДатаОтгрузки - Булево - если Истина, то скрывается колонка дат отгрузок в построчной доставке.
// * Товары - Массив из см. НовыеДанныеСтрокиДоставки
// * Склады - Массив из см. НовыеДанныеСтрокиСкладовДоставки
// 
Функция НовыеПараметрыНастройкиДоставки() Экспорт
	...
	ИсключаемыеСпособыДоставки = Новый Массив; // Массив из ПеречислениеСсылка.СпособыДоставки25
	
	Параметры = Новый Структура;
	
	Параметры.Вставить("ВидПланированияДоставки", ПредопределенноеЗначение("Перечисление.ВидыПланированияДоставки.ТекущимДокументом"));
	Параметры.Вставить("РазрешитьИзменениеВидаПланированияДоставки", Истина);
	Параметры.Вставить("СпособУказанияДоставки", ПредопределенноеЗначение("Перечисление.СпособыУказанияДоставки.НаВесьДокумент"));
	Параметры.Вставить("РазрешитьИзменениеСпособаУказанияДоставки", Истина);
	Параметры.Вставить("СпособДоставки", ПредопределенноеЗначение("Перечисление.СпособыДоставки25.ПустаяСсылка"));
	Параметры.Вставить("ИспользуемыеСпособыДоставки", ИспользуемыеСпособыДоставки);
	Параметры.Вставить("ИсключаемыеСпособыДоставки", ИсключаемыеСпособыДоставки);
	Параметры.Вставить("Партнер", ПредопределенноеЗначение("Справочник.Партнеры.ПустаяСсылка"));
	Параметры.Вставить("ТочкаДоставки", ПредопределенноеЗначение("Справочник.ТочкиДоставки.ПустаяСсылка"));
	Параметры.Вставить("ЗонаДоставки", ПредопределенноеЗначение("Справочник.ЗоныДоставки.ПустаяСсылка"));
	Параметры.Вставить("ДоставкаДоПеревозчика", Ложь);
	Параметры.Вставить("Перевозчик", ПредопределенноеЗначение("Справочник.Партнеры.ПустаяСсылка"));
	Параметры.Вставить("ТочкаДоставкиПеревозчика", ПредопределенноеЗначение("Справочник.ТочкиДоставки.ПустаяСсылка"));
	Параметры.Вставить("ЗонаДоставкиПеревозчика", ПредопределенноеЗначение("Справочник.ЗоныДоставки.ПустаяСсылка"));
	Параметры.Вставить("ВремяДоставкиС", ТипВремя);
	Параметры.Вставить("ВремяДоставкиПо", ТипВремя);
	Параметры.Вставить("ДополнительнаяИнформацияПоДоставке", ТипСтрока0);
	Параметры.Вставить("ОсобыеУсловияПеревозки", Ложь);
	Параметры.Вставить("ОсобыеУсловияПеревозкиОписание", ТипСтрока0);
	Параметры.Вставить("Курьер", ПредопределенноеЗначение("Справочник.ФизическиеЛица.ПустаяСсылка"));
	Параметры.Вставить("ИспользуетсяЕдинаяДатаОтгрузки", Истина);
	
	Товары = Новый Массив; // Массив из см. НовыеДанныеСтрокиДоставки
	Параметры.Вставить("Товары", Товары);
	
	Склады = Новый Массив; // Массив из см. НовыеДанныеСтрокиСкладовДоставки
...
// Новые данные строки доставки.
// 
// Возвращаемое значение:
//  Структура - Новые данные строки доставки:
// * НомерСтрокиДокумента - Число - неотрицательно, длина 9
// * КодСтроки - Число - неотрицательно, длина 10
// * Номенклатура - СправочникСсылка.Номенклатура - 
// * Характеристика - СправочникСсылка.ХарактеристикиНоменклатуры - 
// * ХарактеристикиИспользуются - Булево - 
// * Серия - СправочникСсылка.СерииНоменклатуры -
// * СтатусУказанияСерий - Число - неотрицательно, длина 2, точность 0ы 
// * КоличествоУпаковок - Число - неотрицательно, длина 15, точность 3
...
// * ТочкаДоставкиПеревозчика - СправочникСсылка.ТочкиДоставки - 
// * ЗонаДоставкиПеревозчика - СправочникСсылка.ЗоныДоставки -
// * ВремяДоставкиС - Дата - время доставки
// * ВремяДоставкиПо - Дата - время доставки 
// * ДополнительнаяИнформацияПоДоставке - Строка - 
// * ОсобыеУсловияПеревозки - Булево - 
// * ОсобыеУсловияПеревозкиОписание - Строка -
// * ДатаОтгрузки - Дата - 
// 
Функция НовыеДанныеСтрокиДоставки() Экспорт
	
	ТипСтрока0 = "";
	
	Параметры = Новый Структура;
	
	Параметры.Вставить("НомерСтрокиДокумента", 0);
	Параметры.Вставить("КодСтроки", 0);
	...
	Параметры.Вставить("Серия", ПредопределенноеЗначение("Справочник.СерииНоменклатуры.ПустаяСсылка"));
	Параметры.Вставить("СтатусУказанияСерий", 0);
	Параметры.Вставить("КоличествоУпаковок", 0);
	Параметры.Вставить("Упаковка", ПредопределенноеЗначение("Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка"));
	Параметры.Вставить("СпособДоставки", ПредопределенноеЗначение("Перечисление.СпособыДоставки25.ПустаяСсылка"));
	Параметры.Вставить("ТочкаДоставки", ПредопределенноеЗначение("Справочник.ТочкиДоставки.ПустаяСсылка"));
	Параметры.Вставить("ЗонаДоставки", ПредопределенноеЗначение("Справочник.ЗоныДоставки.ПустаяСсылка"));
	Параметры.Вставить("ДоставкаДоПеревозчика", Ложь);
	...
	Параметры.Вставить("ТочкаДоставкиПеревозчика", ПредопределенноеЗначение("Справочник.ТочкиДоставки.ПустаяСсылка"));
	Параметры.Вставить("ЗонаДоставкиПеревозчика", ПредопределенноеЗначение("Справочник.ЗоныДоставки.ПустаяСсылка"));
	Параметры.Вставить("ВремяДоставкиС", Дата(1, 1 , 1));
	Параметры.Вставить("ВремяДоставкиПо", Дата(1, 1 , 1));
	Параметры.Вставить("ДополнительнаяИнформацияПоДоставке", ТипСтрока0);
	Параметры.Вставить("ОсобыеУсловияПеревозки", Ложь);
	Параметры.Вставить("ОсобыеУсловияПеревозкиОписание", ТипСтрока0);
	Параметры.Вставить("ДатаОтгрузки", Дата(1, 1 , 1));
	
	Возврат Параметры;
	
КонецФункции

// Новые данные строки складов доставки.
// 
// Возвращаемое значение:
//  Структура - Новые параметры настройки складов доставки:
// * Склад - СправочникСсылка.Склады - 
// * ТочкаСклада - СправочникСсылка.ТочкиДоставки - 
// * ЗонаСклада - СправочникСсылка.ЗоныДоставки -
// 
Функция НовыеДанныеСтрокиСкладовДоставки() Экспорт
	
	Параметры = Новый Структура;
	
	Параметры.Вставить("Склад", ПредопределенноеЗначение("Справочник.Склады.ПустаяСсылка"));
	Параметры.Вставить("ТочкаСклада", ПредопределенноеЗначение("Справочник.ТочкиДоставки.ПустаяСсылка"));
	Параметры.Вставить("ЗонаСклада", ПредопределенноеЗначение("Справочник.ЗоныДоставки.ПустаяСсылка"));
	
	Возврат Параметры;
	
КонецФункции

// Описывает базовые значения по умолчанию для параметров доставки
//
// Возвращаемое значение:
//  См. НовыеПараметрыНастройкиДоставки
//
Функция НастройкиПараметровДоставкиПоУмолчанию() Экспорт
	
	Параметры = НовыеПараметрыНастройкиДоставки();
	Параметры.ВидПланированияДоставки = ПредопределенноеЗначение("Перечисление.ВидыПланированияДоставки.ТекущимДокументом");
	Параметры.СпособУказанияДоставки = ПредопределенноеЗначение("Перечисление.СпособыУказанияДоставки.НаВесьДокумент");
	Параметры.СпособДоставки = ПредопределенноеЗначение("Перечисление.СпособыДоставки25.НашимиСилами");
	Параметры.ДоставкаДоПеревозчика = Ложь;
	
	Возврат Параметры;
	
КонецФункции

// Используется в списках точек доставки для скрытия или показа архивных точек
// 
// Параметры:
//  Форма - см. Справочник.ТочкиДоставки.Форма.ФормаСписка
//        - см. Справочник.ТочкиДоставки.Форма.ФормаВыбора
//        - см. Справочник.ТочкиДоставки.Форма.ТочкиДоставкиВладельца
//
Процедура ИзменитьОтборАктуальныхТочек(Форма) Экспорт
	
	Список = Форма.Список;
	
	ИмяПараметра = "СкрыватьНеиспользуемые";
	...
	ТребуетсяСкрыть = НЕ ЗначениеПараметра.Использование;
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Список, ИмяПараметра, Истина, ТребуетсяСкрыть);
	
	ИмяКнопкиКоманды = "ФормаОтображатьНеиспользуемые";
	
	Если ТребуетсяСкрыть Тогда
		
		Картинка = БиблиотекаКартинок.ФлагНеУстановлен;
		
	Иначе
		
		Картинка = БиблиотекаКартинок.ФлагУстановлен;
		
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, ИмяКнопкиКоманды, "Картинка", Картинка);
	
КонецПроцедуры

// Параметры:
//  РезультатПроверки - см. ДоставкаТоваров.ПроверитьВозможностьПереходаНаДоставку25
// 
// Возвращаемое значение:
//  Строка -
//
Функция ТекстПроверкиВозможностиПереходаНаДоставку25(РезультатПроверки) Экспорт
	
	Если РезультатПроверки.ЕстьОстаткиКОтгрузке Тогда
		
		ТекстПредупреждения = НСтр("ru = 'Необходимо отгрузить все товары по распоряжениям с запланированной доставкой со складов с настройкой ""Собирать товары в порядке доставки"".'");
		
	ИначеЕсли РезультатПроверки.ЕстьСкладыСборкаПослеДоставки Тогда
		
		ТекстПредупреждения = НСтр("ru = 'Используются ордерные склады с настройкой ""Собирать товары в порядке доставки"", сборка по данным рейса не поддерживается при порядке оформления ордеров ""Сначала заказы и накладные, затем ордера"".'");
...
//  Строка -
//
Функция ТекстПроверкиАктивныхСеансов() Экспорт
	
	ТекстСообщения = НСтр("ru = 'Для перехода на доставку 2.5 необходимо закрыть все сеансы, кроме текущего.'");
...
//  Строка -
//
Функция ТекстУсловногоОформленияНеИспользуетсяВесОбъем() Экспорт
	
	ТекстНеИспользуется = "---";
	
	Возврат ТекстНеИспользуется;
...
//
// Параметры:
//	ПолеВводаФормы - ПолеФормы, РасширениеПоляФормыДляПоляВвода -
//	               - Массив из ПолеФормы, РасширениеПоляФормыДляПоляВвода - поле ввода времени доставки
...
Процедура ЗаполнитьСписокВыбораПоляВремени(ПолеВводаФормы, Интервал = 3600) Экспорт
	...
	Если ТипЗнч(ПолеВводаФормы) = Тип("ПолеФормы") Тогда
		ЭлементыФормы = Новый Массив; // Массив из ПолеФормы
	...
	СписокВыбора = Новый СписокЗначений; // СписокЗначений из Дата
...
// Возвращаемое значение:
//  Структура - Описание распоряжения на доставку:
// * Имя - Строка -
// * ЭтоЗаказ - Булево - 
// * ИмяТЧ - Строка - 
// * ИмяПоляСклад - Строка - 
// * ИмяПоляПолучательОтправитель - Строка - 
// * ИмяПоляНазначение - Строка - 
// * ИмяПоляСерия - Строка - 
//
