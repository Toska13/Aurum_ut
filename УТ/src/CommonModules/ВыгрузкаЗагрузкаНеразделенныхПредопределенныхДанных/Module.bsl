...
Процедура ПередЗагрузкойДанных(Контейнер) Экспорт
	...
	Если Не ЗначениеЗаполнено(ИменаДополнительноЗагружаемых) Тогда
		Возврат;
	КонецЕсли;
	
	ДополнительноЗагружаемыеТипы = Новый Массив();
	
	Для Каждого ИмяДополнительноЗагружаемогоОбъекта Из ИменаДополнительноЗагружаемых Цикл
		ДополнительноЗагружаемыеТипы.Добавить(Метаданные.НайтиПоПолномуИмени(ИмяДополнительноЗагружаемогоОбъекта));
	КонецЦикла;
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ПараметрыЗагрузки.ЗагружаемыеТипы, ДополнительноЗагружаемыеТипы, Истина);
