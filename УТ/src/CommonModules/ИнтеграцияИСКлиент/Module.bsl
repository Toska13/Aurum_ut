...
Процедура ОбновитьПолеИнтеграцииВФормеДокументаОснования(Форма, Подсистема) Экспорт
	...
	Если ПараметрыИнтеграции <> Неопределено И ЗначениеЗаполнено(ПараметрыИнтеграции.ИмяРеквизитаФормы) Тогда
		...
		ТекстНадписи = Подсистема.МодульВызовСервера.ДанныеПоляИнтеграцииВФормеДокументаОснования(Форма[ИмяРеквизитаОбъект].Ссылка).ТекстНадписи;
	...
	ИнтеграцияИСКлиентПереопределяемый.ПриОбновлениияПоляИнтеграцииВФормеДокументаОснования(Форма, Подсистема);
...
// * ВидПродукции                    - Неопределено, СправочникСсылка.ВидыПродукцииИС - Вид продукции номенклатуры.
...
// Вызывается при изменении вида продукции ГИСМТ в настройке интеграции с ФГИС
// Получает и устанавливает ключ по видам продукции, указанным в форме.
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения.
//
Процедура ВидПродукцииГИСМТПриИзменении(Форма) Экспорт
	
	ПриИзмененииВидаПродукцииИнтеграцииСФГИС(
		Форма,
		КонструкторИСОбщегоНазначенияКлиентСервер.СуффиксПодсистемыМаркировкиДляКлючаВидовПродукции());
	
КонецПроцедуры

// Вызывается при изменении флага ФГИС в настройке интеграции с ФГИС
// Устанавливает в форме вид продукции, связанный с флагом.
// Получает и устанавливает ключ по видам продукции, указанным в форме.
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения.
//
Процедура ФлагФГИСПриИзменении(Форма) Экспорт
	
	Если ЗначениеЗаполнено(Форма.ДанныеПодсистемФГИС.ИмяПодсистемы) Тогда
		
		Если Форма.ФлагФГИС Тогда
			Форма.ВидПродукцииФГИС = Форма.ДанныеПодсистемФГИС.ВидПродукции;
		Иначе
			Форма.ВидПродукцииФГИС = Неопределено;
		КонецЕсли;
		
		ВидПродукцииФГИСПриИзменении(Форма);
		
	КонецЕсли;
	
КонецПроцедуры

// Вызывается при изменении вида продукции ФГИС в настройке интеграции с ФГИС
// Устанавливает в форме вид продукции, связанный с видом продукции ФГИС.
// Получает и устанавливает ключ по видам продукции, указанным в форме.
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения.
//
Процедура ВидПродукцииФГИСПриИзменении(Форма) Экспорт
	
	Если ЗначениеЗаполнено(Форма.ДанныеПодсистемФГИС.ИмяПодсистемы) Тогда
		
		Форма["ВидПродукции"+Форма.ДанныеПодсистемФГИС.ИмяПодсистемы] = Форма.ВидПродукцииФГИС;
		
		ПриИзмененииВидаПродукцииИнтеграцииСФГИС(Форма, Форма.ДанныеПодсистемФГИС.ИмяПодсистемы);
...
Процедура ПриИзмененииВидаПродукцииИнтеграцииСФГИС(Форма, СуффиксВидаПродукции)
	...
	Для Каждого Суффикс Из КонструкторИСОбщегоНазначенияКлиентСервер.СуффиксыВидовПродукции() Цикл
		...
		Если ЗначениеЗаполнено(Форма[ВидПродукцииИмяЭлемента])
			...
			Если Суффикс <> СуффиксПодсистемыМаркировки
				И ЗначениеЗаполнено(Форма.ДанныеПодсистемФГИС.ИмяПодсистемы) Тогда
				Форма.ВидПродукцииФГИС = Неопределено;
				Форма.ФлагФГИС = Ложь;
			КонецЕсли;
		ИначеЕсли ИнтеграцияИСКлиентСервер.ИспользуетсяПодсистемаИнтеграцииСФГИС(Форма, Суффикс)
			...
			Если ЗначениеЗаполнено(ЗависимыеВидыПродукции[Суффикс].Значение) Тогда
				...
				Если ЗначениеЗаполнено(Форма.ДанныеПодсистемФГИС.ИмяПодсистемы) Тогда
					Форма.ВидПродукцииФГИС = ЗависимыеВидыПродукции[Суффикс].Значение;
				КонецЕсли;
			ИначеЕсли ЗависимыеВидыПродукции[Суффикс].Обязательный Тогда
				Если ЗначениеЗаполнено(Форма.ДанныеПодсистемФГИС.ВидПродукции) Тогда
					Форма[ВидПродукцииИмяЭлемента] = Форма.ДанныеПодсистемФГИС.ВидПродукции;
					Форма.ВидПродукцииФГИС = Форма[ВидПродукцииИмяЭлемента];
				Иначе
					...
					Если ВидыПродукцииПоИмениПодсистемы.Количество() = 1 Тогда
						Форма[ВидПродукцииИмяЭлемента] = ВидыПродукцииПоИмениПодсистемы[0];
						Форма.ВидПродукцииФГИС = Форма[ВидПродукцииИмяЭлемента];
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
	...
	УстановитьКлючВидовПродукции(Форма);
	ИнтеграцияИСКлиентСервер.НастроитьЭлементыФормыИнтеграцииСФГИСПоВидамПродукцииКлюча(Форма);
	
КонецПроцедуры

Процедура УстановитьКлючВидовПродукции(Форма)
	
	ПоляКлюча = Новый Структура(СтрСоединить(КонструкторИСОбщегоНазначенияКлиентСервер.ИменаПолейКлючаВидовПродукции(), ","));
	ЗаполнитьЗначенияСвойств(ПоляКлюча, Форма);
	Форма.КлючВидовПродукции = ИнтеграцияИСВызовСервера.ПолучитьКлючВидовПродукции(ПоляКлюча);
