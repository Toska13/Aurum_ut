...
Функция ВызватьHTTPМетод(HTTPМетод, URL, ПараметрыЗапроса, СоответствиеПолейОтвета, Заголовки = Неопределено, ПараметрыПреобразования = Неопределено)
	...
	Если Ответ.КодСостояния = 200 Тогда
		...
		ContentType = ОбщегоНазначенияБТСКлиентСервер.ЗаголовокHTTP(Ответ, "Content-Type");
		...
		ИначеЕсли ContentType = "application/octet-stream" Тогда
			...
			Результат.Вставить("Имя", СтрЗаменить(ОбщегоНазначенияБТСКлиентСервер.ЗаголовокHTTP(Ответ, "Content-Disposition"), "attachment; filename=", ""));
		КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьСертификатВКонтейнерИХранилище", Истина);
	Методы.Вставить("СоздатьКонтейнерИЗапросНаСертификат", Истина);
