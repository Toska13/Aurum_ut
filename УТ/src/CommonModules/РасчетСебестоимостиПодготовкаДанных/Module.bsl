...
Процедура ПодготовкаИсходныхДанныхКРасчету(ПараметрыРасчета) Экспорт
	...
	// При использовании среднескользящей оценки могут быть изменены любые регистры, по которым могут быть движения у исправляемых документов.
	РасчетСебестоимостиПрикладныеАлгоритмы.ОбновитьРасчетныеКэшиРегистров(
		ПараметрыРасчета,
		НЕ РасчетСебестоимостиПовтИсп.ИспользуетсяСреднескользящаяОценка());
...
Процедура ПерепровестиДокументыПоРегистрам(ПараметрыРасчета)
	...
	|	И НЕ ДД.РасчетПартий
	|	И НЕ ДД.РасчетСебестоимости
	|	И НЕ ДД.АналитикаУчетаПоПартнерам.Организация В (&ОрганизацииСреднескользящая)
	|";
	...
	ТипыРегистраторовСНестандартнойЛогикойФормированияДвижений = Новый Массив();
	РасчетСебестоимостиПодготовкаДанныхЛокализация.ДополнитьТипыРегистраторовСНестандартнойЛогикойФормированияДвижений(
		ТипыРегистраторовСНестандартнойЛогикойФормированияДвижений);
	
	Запрос.УстановитьПараметр("ТипыДокументовРегламентные", ОбщегоНазначенияКлиентСервер.РазностьМассивов(
		РасчетСебестоимостиПрикладныеАлгоритмы.ТипыДокументовРегламентные(),
		ТипыРегистраторовСНестандартнойЛогикойФормированияДвижений));
	...
	ОписаниеРегистра = ПараметрыРасчета.Движения[Метаданные.РегистрыНакопления.ПрочиеДоходы.Имя];
	ОписаниеРегистра.НадоОбновитьРасчетныйКэш = Истина;
...
Процедура ИсправитьПроблемыВДвиженияхИДокументах(ПараметрыРасчета)
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	// Таблица аналитик с отрицательными остатками. По таким аналитикам нет смысла проверять нулевую стоимость в движениях
	// документов при использовании метода оценки "Среднескользящая", т.к. такие остатки возникают при отсутствии приходов и
	// соответственно по ним нельзя рассчитать среднескользящую стоимость.
	// Выборка остатков выполняется на дату актуальности, т.к. при определении среднескользящей стоимости остатки берутся
	// так же на дату актуальности. См. запрос формирования таблицы ВТСебестоимостьОстаток в процедуре 
	// ДобавитьТекстЗапросаРасчетСреднескользящаяСтоимость() #Область ДобавитьТекстЗапросаСреднескользящаяСтоимость.
	|ВЫБРАТЬ
	...
	// Запрос оставлен для целей отладки мезанизма исправления движений.
	|		И ЛОЖЬ
	|		И НЕ РазделУчета В (
	...
	// При использовании среднескользящей допустимые типы записей не проверяем, т.к. этапы распределения партий не выполняются.
	|	ГДЕ
	...
	|			ИЛИ ЕСТЬNULL(Реализация.Дата, &НачалоПериода) < &НачалоПериода)
	|		И НЕ Т.Сторно
	|		И НЕ Аналитика.Организация В(&ОрганизацииСреднескользящая)
	|
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	// При среднескользящей вид деятельности НДС не заполняется.
	|		И НЕ Т.Организация В (&ОрганизацииСреднескользящая)
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	|		И НЕ Т.РасчетПартий
	|		И НЕ Т.РасчетСебестоимости
	|		И Т.МестоХранения = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|		И НЕ Т.РасчетПартий
	|		И НЕ Т.РасчетСебестоимости
	|		И Т.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПриобретенияПрошлогоПериода)
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	// Проверка наличия не рассчитанной стоимости при среднескользящей.
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		""СебестоимостьТоваров"",
	|		Т.Регистратор,
	|		Т.Организация,
	...
	|		85 КАК КодОшибки
	|	ИЗ
	// Выборка выполняется из таблицы движений регистра для использования подходящих индексов при соединении с табоицей ОтрицательныеОстатки.
	|		РегистрНакопления.СебестоимостьТоваров КАК Т
	|		ЛЕВОЕ СОЕДИНЕНИЕ ОтрицательныеОстатки КАК ОтрицательныеОстатки
	|			ПО ОтрицательныеОстатки.АналитикаУчетаНоменклатуры = Т.АналитикаУчетаНоменклатуры
	|			И ОтрицательныеОстатки.РазделУчета = Т.РазделУчета
	|			И ОтрицательныеОстатки.ВидЗапасов = Т.ВидЗапасов
	|			И ОтрицательныеОстатки.Организация = Т.Организация
	|	ГДЕ
	|		Т.Период МЕЖДУ &НачалоПериода И &КонецПериода
	// Запрос оставлен для целей отладки мезанизма исправления движений.
	|		И ЛОЖЬ
	...
	|		И ОтрицательныеОстатки.АналитикаУчетаНоменклатуры ЕСТЬ NULL
	// В отчете давальцу между организациями нет плановой стоимости выпуска, поэтому для этого документа не может быть 
	// рассчитана среднескользящая стоимость.
	|		И Т.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтчетДавальцуМеждуОрганизациями)
	// Для типа запасов "Продукция давальца" стоимость может быть пустая.
	|		И ЕСТЬNULL(Т.ВидЗапасов.ТипЗапасов, НЕОПРЕДЕЛЕНО) <> ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияДавальца)
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|			ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаСкладах),
	|			ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты))
	|		И ТИПЗНАЧЕНИЯ(Т.Регистратор) В (ТИП(Документ.ТаможеннаяДекларацияИмпорт),
	...
	|		88
	|	ИЗ
	|		РегистрНакопления.ДвиженияПоПрочимАктивамПассивам КАК Т
	|	ГДЕ
	|		Т.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|		И Т.Организация В (&МассивОрганизаций)
	|		И Т.Активность
	...
	|		ВТДвиженияСебестоимостьТоваров КАК Т
	|	ГДЕ
	|		ТИПЗНАЧЕНИЯ(Т.Регистратор) = ТИП(Документ.ТаможеннаяДекларацияИмпорт)
	|		И Т.ИдентификаторСтроки = """"
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|		И (Т.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПриобретенияПрошлогоПериода)
	|			ИЛИ Т.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.КорректировкаПриобретенияПрошлогоПериода))
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	// В сторно движениях по регистру должен быть заполнен период отклонения.
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		""ПрочиеРасходы"",
	|		Т.Регистратор,
	|		Т.Организация,
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// Проверка соответствия кор аналитики расходного движения и аналитики приходного движения.
	// Если в движении заполнен кор раздел учета, то должно быть приходное движение с аналитикой, 
	// соответствующей кор реквизитам расходного движения.
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		""СебестоимостьТоваров"",
	|		Т.Регистратор,
	|		Т.Организация,
	...
	|			И ДвиженияПриход.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|			И ДвиженияПриход.АналитикаУчетаНоменклатуры = Т.КорАналитикаУчетаНоменклатуры
	|			И ДвиженияПриход.РазделУчета = Т.КорРазделУчета
	|			И ДвиженияПриход.ВидДеятельностиНДС = Т.КорВидДеятельностиНДС
	|	ГДЕ
	|		Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|		И Т.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности)
	...
	|		И ЕСТЬNULL(Т.АналитикаУчетаНоменклатуры.Назначение.ВидДеятельностиНДС, ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка))
	|			<> Т.ВидДеятельностиНДС
	|		И Т.Регистратор ССЫЛКА Документ.ВозвратТоваровПоставщику
	|		И НЕ Т.РазделУчета В (&ЗабалансовыеРазделыУчета)
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// Если в документе указана одна и та же номенклатура, характеристика и назначение, но с разным подразделением, 
	// статьей и аналитикой в нескольких строках, то во всех таких строках должна быть аналитика учета партий с разным 
	// кодом строки, что бы одинаковые строки формировали разные партии.
	// Запрос заполнения аналитики учета партий выполняется в функции ТекстЗапросаКодыСтрокДляАналитикиУчетаПартий()
	// документа ПоступлениеУслугВПодразделение. 
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		""СебестоимостьТоваров"",
	|		Т.Регистратор,
	|		Т.Организация,
	...
	|			И Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|			И Т.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Партия)
	|			И ТИПЗНАЧЕНИЯ(Т.Регистратор) = ТИП(Документ.ПоступлениеУслугВПодразделение)
	|			И Т.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки)
	|		СГРУППИРОВАТЬ ПО
	|			Т.Регистратор,
	|			Т.Организация,
	...
	|		И Т.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности)
	|		И Т.ОперацияУчетаСебестоимости = ЗНАЧЕНИЕ(Перечисление.ОперацииУчетаСебестоимости.Перемещение)		
	|		И Т.ЗаказКлиента = НЕОПРЕДЕЛЕНО
	|	) КАК Т
	|
	|ГДЕ
	|	Т.КодОшибки <> 0
	...
	Запрос.Текст = ТекстЗапросаВТДвиженияСебестоимостьТоваров()
		+ РасчетСебестоимостиЗаполнениеПартий.ТекстИсправленныеДокументы()
	...
	РасшифровкаКодовОшибок.Вставить(78, НСтр("ru='При выпуске продукции по плановой стоимости нет движений по разделу учета ""Выпуск продукции"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(79, НСтр("ru='При выпуске продукции по фактической стоимости есть движения по разделу учета ""Выпуск продукции"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(80, НСтр("ru='При выпуске продукции по фиксированной стоимости есть движения по разделу учета ""Выпуск продукции"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(81, НСтр("ru='При выпуске продукции в цеховую кладовую не правильно заполнено место хранения в разделе учета ""Выпуск продукции"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(82, НСтр("ru='Не корректно заполнено место хранения в регистре ""Отклонения в стоимости товаро"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(83, НСтр("ru='Не корректно заполнен период отклонений для корректировки приобретения прошлого периода в регистре ""Отклонения в стоимости товаров"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(84, НСтр("ru='Не заполнена партия в движениях себестоимости товаров по разделу учета ""Неотфактурованные поставки"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(85, НСтр("ru='Не рассчиталась стоимость по методу оценки ""Среднескользящая"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(87, НСтр("ru='Заполнены различные коды строк в аналитике учета партии приходного документа при одинаковых аналитиках в строках документа'", ОбщегоНазначения.КодОсновногоЯзыка()));	
	РасшифровкаКодовОшибок.Вставить(88, НСтр("ru='Не заполнены реквизиты расчета партий и расчета себестоимости в движениях по регистру ""Движения Прочие активы-пассивы""'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(89, НСтр("ru='Не правильный тип записи в сторно движениях перемещения товаров'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(91, НСтр("ru='Не заполнен реквизит ""Документ движения"" в движениях отчета давальцу между организациями.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(92, НСтр("ru='Не заполнен идентификатор строки в движениях документа ""Таможенная декларация на импорт"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(93, НСтр("ru='Не заполнен реквизит ""Период отклонения"" в сторно движениях по регистру ""Себестоимость товаров"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(94, НСтр("ru='Не заполнен реквизит ""Период отклонения"" в сторно движениях по регистру ""Прочие расходы"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(95, НСтр("ru='Не заполнен реквизит ""Период отклонения"" в сторно движениях по регистру ""Партии прочих расходов"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(96, НСтр("ru='Не корректно заполнен кор. вид деятельности НДС в расходном движении ""Реализация (товары в пути)"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(97, НСтр("ru='Не корректно заполнен вид деятельности НДС в движениях документа ""Возврат товаров поставщику"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(98, НСтр("ru='Заполнены различные коды строк в аналитике учета партии приходного документа при одинаковых аналитиках в строках документа'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(99, НСтр("ru='Не заполнен реквизит ""Заказ клиента"" в движениях реализации товаров и услуг.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	// При использовании среднескользящей стоимости проверки движений не выполняются, т.к. механизм проведения документа
	// по одному регистру не учитывает что одновременно могут быть сформированы движения по корреспондирующим регистрам. 
	|	И НЕ Т.Организация В (&ОрганизацииСреднескользящая)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	РасшифровкаКодовОшибок.Вставить(5, НСтр("ru='В движениях по регистру ""Прочие расходы"" излишне заполнена сумма постоянной разницы.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	|			И НЕ &ИспользоватьУчетПрочихДоходовРасходовРегл)
	|	  )
	// Исключаем выборку движений по отклонениям в стоимости, сформированные при проведении документа.
	// Проверка сумм в таких движениях не требуется.
	|	И Т.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтклонениеВСтоимостиТоваровДоходы)
	|
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	// Исключаем выборку движений по отклонениям в стоимости, сформированные при проведении документа.
	// Проверка сумм в таких движениях не требуется.
	|	И Т.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтклонениеВСтоимостиТоваровРасходы)
	// Движения с этими хоз операциями, формируются при выполнении операции "Закрытие расходов от выбытия ОС",
	// поэтому проверка таких движений не требуется.
	|	И НЕ Т.ХозяйственнаяОперация В (
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакрытиеРасходовОтРеализацииОС),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакрытиеРасходовОтРеализацииОСПослеПереходаПрав),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакрытиеРасходовОтРеализацииОССОтложеннымПереходомПрав),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакрытиеРасходовОтСписанияОС))
	// При использовании среднескользящей стоимости проверки движений не выполняются, т.к. механизм проведения документа
	// по одному регистру не учитывает что одновременно могут быть сформированы движения по корреспондирующим регистрам. 
	|	И НЕ Т.Организация В (&ОрганизацииСреднескользящая)
	|
	...
	|	И НЕ Т.РасчетПартий
	|	И НЕ Т.РасчетСебестоимости
	| 	И Т.РежимЗакрытияМесяца = ЗНАЧЕНИЕ(Перечисление.РежимыЗакрытияМесяца.ПустаяСсылка)
	|	И НЕ ТИПЗНАЧЕНИЯ(Т.Регистратор) В (&ТипыРегламентныхДокументов)
	|	И ТИПЗНАЧЕНИЯ(Т.Регистратор) <> ТИП(Документ.КорректировкаПриобретения)
	|	И НЕ ТИПЗНАЧЕНИЯ(Т.Регистратор) В (&ТипыДокументовОСиНМА)
	|	И ТИПЗНАЧЕНИЯ(Т.Регистратор) <> ТИП(Документ.НачислениеРеверсивногоНДС)
	|	И ТИПЗНАЧЕНИЯ(Т.Регистратор) <> ТИП(Документ.РасчетПроцентныхРасходовДисконтирования)
	//++ Локализация
	...
	// Движения документа "Прочее оприходование товаров" с хоз операцией "Оприходование (при выбытии ОС)" формируются
	// при выполнении операции "Закрытие расходов от выбытия ОС", поэтому проверка таких движений не требуется.
	|	И НЕ Т.ХозяйственнаяОперация В (
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОприходованиеПриВыбытииОС),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаРасходовОтВыбытияОС),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаСтоимостиТМЦОприходованныхПриВыбытииОС))
	// Движения с этими хоз операциями, формируются при выполнении операции "Закрытие расходов от выбытия ОС",
	// поэтому проверка таких движений не требуется.
	|	И НЕ Т.ХозяйственнаяОперация В (
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакрытиеРасходовОтРеализацииОС),
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// В конфигурации "Управление торговлей" в регистре "Прочие расходы" сумма постоянной разницы не заполняется,
	// т.к. механизмы распределения расходов этой конфигурации не умеют работать с постойнной разницей.
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	""ПрочиеРасходы"" 		 КАК ИмяРегистра,
	|	Т.Регистратор 			 КАК Ссылка,
	|	Т.Организация 			 КАК Организация,
	...
	// При использовании среднескользящей стоимости проверки движений не выполняются, т.к. механизм проведения документа
	// по одному регистру не учитывает что одновременно могут быть сформированы движения по корреспондирующим регистрам. 
	|	И НЕ Декларация.Организация В (&ОрганизацииСреднескользящая)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|ВЫБРАТЬ ПЕРВЫЕ 0
	|	""СебестоимостьТоваров"" КАК ИмяРегистра,
	...
	РасшифровкаКодовОшибок.Вставить(3, НСтр("ru='Лишние движения по регистру ""Движения Прочие активы и пассивы"" у документа ""Корректировка приобретения"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	|ИЗ
	|	РегистрНакопления.ДвиженияПоПрочимАктивамПассивам КАК Т
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КорректировкаПриобретения КАК ДокументКорректировки
	|		ПО ДокументКорректировки.Ссылка = Т.Регистратор
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КорректировкаПриобретения.Расхождения КАК КорректировкаРасхождения
	|		ПО КорректировкаРасхождения.Ссылка = Т.Регистратор
	|ГДЕ
	|	Т.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И Т.Организация В (&МассивОрганизаций)
	|	И Т.Активность
	|	И НЕ Т.РасчетПартий
	|	И НЕ Т.РасчетСебестоимости
	|	И Т.Статья <> ЕСТЬNULL(ВЫРАЗИТЬ(ДокументКорректировки.СтатьяРасходов КАК ПланВидовХарактеристик.СтатьиАктивовПассивов), НЕОПРЕДЕЛЕНО)
	|	И Т.Статья <> ЕСТЬNULL(ВЫРАЗИТЬ(ДокументКорректировки.СтатьяДоходов КАК ПланВидовХарактеристик.СтатьиАктивовПассивов), НЕОПРЕДЕЛЕНО)
	|	И Т.Статья <> ЕСТЬNULL(ВЫРАЗИТЬ(КорректировкаРасхождения.СтатьяРасходов КАК ПланВидовХарактеристик.СтатьиАктивовПассивов), НЕОПРЕДЕЛЕНО)
	|";
	...
	// При использовании среднескользящей стоимости проверки движений не выполняются, т.к. механизм проведения документа
	// по одному регистру не учитывает что одновременно могут быть сформированы движения по корреспондирующим регистрам. 
	|	И НЕ СебестоимостьТоваров.Организация В (&ОрганизацииСреднескользящая)
	|";
	...
	РасшифровкаКодовОшибок = Новый Соответствие;
	// При возврате товара/тары на другой склад или по другому назначению должно вместо 3 движений (сторно продажи, перекидка на другой склад/назначение
	//делаться одно движения сразу по новым данным, как это реализовано в регл. учете.
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru = 'Неправильное движение возврата на другой склад в документе ""Возврат товаров от клиента"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(2, НСтр("ru = 'Неправильное движение возврата на другой склад в документе ""Возврат товаров между организациями"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//++ Локализация
	...
	|ГДЕ
	|	СебестоимостьТоваров.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И СебестоимостьТоваров.Регистратор ССЫЛКА Документ.ВозвратТоваровОтКлиента
	...
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СторноПереданнойТарыВозвратНаДругойСклад))
	|	И СебестоимостьТоваров.Количество < 0 И СебестоимостьТоваров.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	// При использовании среднескользящей стоимости проверки движений не выполняются, т.к. механизм проведения документа
	// по одному регистру не учитывает что одновременно могут быть сформированы движения по корреспондирующим регистрам. 
	|	И НЕ СебестоимостьТоваров.Организация В (&ОрганизацииСреднескользящая)
	|
	...
	|ГДЕ
	|	СебестоимостьТоваров.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И СебестоимостьТоваров.Регистратор ССЫЛКА Документ.ВозвратТоваровМеждуОрганизациями
	|	И СебестоимостьТоваров.ХозяйственнаяОперация В (
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СторноРеализацииВозвратНаДругойСклад),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СторноПереданнойТарыВозвратНаДругойСклад))
	|	И СебестоимостьТоваров.Количество < 0 И СебестоимостьТоваров.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	...
	ПараметрыРасчета,
		Запрос,
		РасшифровкаКодовОшибок,
		НСтр("ru = 'исправление движений документов при возврате товара/тары на другой склад'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасшифровкаКодовОшибок.Вставить(3, НСтр("ru='Сформированы движения по регистру ""Партии прочих расходов"" для организации с методом оценки стоимости ""ФИФО (взвешенная)"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасшифровкаКодовОшибок.Вставить(1,
		НСтр("ru = 'Отличаются суммы в движениях по регистру ""Себестоимость товаров"" от регистра ""Закупки""'",
			ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(2,
		НСтр("ru = 'Отличаются суммы в движениях по регистру ""Закупки"" от регистра ""Себестоимость товаров""'",
			ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(3,
		НСтр("ru = 'Отличаются суммы в движения по регистру ""Отклонения в стоимости товаров"" от регистра ""Закупки""'",
			ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	|		И СебестоимостьТоваров.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		И НЕ СебестоимостьТоваров.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость)
	...
	|		ОтклоненияВСтоимостиТоваров.Номенклатура КАК Номенклатура,
	|		ОтклоненияВСтоимостиТоваров.Характеристика КАК Характеристика,
	|		ОтклоненияВСтоимостиТоваров.ОтклоненияВСтоимостиСНДС КАК ДопРасходы,
	|		ОтклоненияВСтоимостиТоваров.ОтклоненияВСтоимостиБезНДС КАК ДопРасходыБезНДС,
	|		ОтклоненияВСтоимостиТоваров.ОтклоненияВСтоимостиРегл
	|			+ ОтклоненияВСтоимостиТоваров.НДСРегл КАК ДопРасходыРегл
	|	ИЗ
	...
	|		И ОтклоненияВСтоимостиТоваров.Регистратор ССЫЛКА Документ.ТаможеннаяДекларацияИмпорт
	|		И ОтклоненияВСтоимостиТоваров.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	""ОтклоненияВСтоимостиТоваров"" КАК ИмяРегистра,
	|	Т.Регистратор КАК Ссылка,
	|	Т.Организация КАК Организация,
	|	3 КАК КодОшибки
	|ИЗ
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументыПоОтдельнымРегистрамНакопления(
		...
		Запрос,
		РасшифровкаКодовОшибок,
		НСтр("ru = 'исправление движений документа ""Таможенная декларация на импорт"" по регистрам ""Себестоимость товаров"" и ""Закупки"".'",
			ОбщегоНазначения.КодОсновногоЯзыка()));
	
	#КонецОбласти
	
	#Область СебестоимостьТоваровСуммыДвиженийАкцизов
	
	РасшифровкаКодовОшибок = Новый Соответствие;
	РасшифровкаКодовОшибок.Вставить(1,
		НСтр("ru = 'Отличаются суммы в движениях по регистру ""Себестоимость товаров"" от регистра ""Движения по акцизам""'",
			ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(2,
		НСтр("ru = 'Отличаются суммы в движения по регистру ""Отклонения в стоимости товаров"" от регистра ""Движения по акцизам""'",
			ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	|ПОМЕСТИТЬ ВтСуммыДвиженийАкцизов
	|ИЗ
	|	(ВЫБРАТЬ
	|		СебестоимостьТоваров.Организация КАК Организация,
	|		СебестоимостьТоваров.Регистратор КАК Регистратор,
	...
	|		И СебестоимостьТоваров.Активность
	|		И СебестоимостьТоваров.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		И СебестоимостьТоваров.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость)
	|		И НЕ СебестоимостьТоваров.РасчетПартий
	|		И НЕ СебестоимостьТоваров.РасчетСебестоимости
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	...
	|		ОтклоненияВСтоимостиТоваров.Регистратор КАК Регистратор,
	|		ОтклоненияВСтоимостиТоваров.Номенклатура КАК Номенклатура,
	|		ОтклоненияВСтоимостиТоваров.ИдентификаторФинЗаписи,
	|		ОтклоненияВСтоимостиТоваров.НастройкаХозяйственнойОперации,
	|		ОтклоненияВСтоимостиТоваров.ОтклоненияВСтоимостиРегл КАК ДопРасходыРегл,
	|		ОтклоненияВСтоимостиТоваров.ОтклоненияВСтоимостиУпр КАК ДопРасходыУпр
	|	ИЗ
	|		РегистрНакопления.ОтклоненияВСтоимостиТоваров КАК ОтклоненияВСтоимостиТоваров
	|	ГДЕ
	|		ОтклоненияВСтоимостиТоваров.Период МЕЖДУ &НачалоПериода И &КонецПериода
	...
	|
	|	ВЫБРАТЬ
	|		ДвиженияПоАкцизам.Организация,
	|		ДвиженияПоАкцизам.Регистратор,
	|		ДвиженияПоАкцизам.Номенклатура,
	|		ДвиженияПоАкцизам.ИдентификаторФинЗаписи,
	|		ДвиженияПоАкцизам.НастройкаХозяйственнойОперации,
	|		-ДвиженияПоАкцизам.СуммаАкцизаРегл КАК ДопРасходыРегл,
	|		-ДвиженияПоАкцизам.СуммаАкцизаРегл КАК ДопРасходыУпр
	|	ИЗ
	|		РегистрНакопления.ДвиженияПоАкцизам КАК ДвиженияПоАкцизам
	|	ГДЕ
	|		ДвиженияПоАкцизам.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|		И ДвиженияПоАкцизам.Организация В (&МассивОрганизаций)
	|		И ДвиженияПоАкцизам.Активность
	|		И ДвиженияПоАкцизам.ВидОперацииСПодакцизнымТоваром В (
	|			ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСПодакцизнымТоваром.ВвозИзЕАЭС),
	|			ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСПодакцизнымТоваром.Импорт))
	|) КАК Т
	|СГРУППИРОВАТЬ ПО
	|	Т.Регистратор,
	|	Т.Номенклатура,
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументыПоОтдельнымРегистрамНакопления(
		...
		Запрос,
		РасшифровкаКодовОшибок,
		НСтр("ru = 'исправление движений на сумму акциза по регистру ""Себестоимость товаров"".'",
			ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	|	МАКСИМУМ(РаспределениеПрочихЗатрат.НДД) КАК НДД,
	...
	|	РеквизитыСЗадвоеннымиДокументами.НДД КАК НДД
	|ИЗ
	...
	Попытка
		...
		Пока ДокументыДляПроведения.Следующий() Цикл
			...
			ДокументОбъект.НДД = ДокументыДляПроведения.НДД;
	...
	// При использовании среднескользящей стоимости проверки движений не выполняются, т.к. механизм проведения документа
	// по одному регистру не учитывает что одновременно могут быть сформированы движения по корреспондирующим регистрам. 
	|		И НЕ Аналитики.Организация В (&ОрганизацииСреднескользящая)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	РасшифровкаКодовОшибок = Новый Соответствие;
	РасшифровкаКодовОшибок.Вставить(1,
		НСтр("ru = 'Сформированы некорректные движения по регистру ""Отклонения в стоимости товаров""'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(2,
		НСтр("ru = 'Движения по регистру ""Отклонения в стоимости товаров"" не требуются для используемой учетной политики'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	|	Т.Регистратор КАК Регистратор
	|ПОМЕСТИТЬ ВТРегистраторыСОтклонениями
	|ИЗ
	|	РегистрНакопления.ОтклоненияВСтоимостиТоваров КАК Т
	|ГДЕ
	|	Т.Период МЕЖДУ &НачалоПериода И &КонецПериода
	...
	|	Т.Организация КАК Организация,
	|	Т.Регистратор КАК Регистратор,
	|	(УчетнаяПолитика.МетодОценкиСтоимостиТоваров = ЗНАЧЕНИЕ(Перечисление.МетодыОценкиСтоимостиТоваров.Среднескользящая)) КАК Среднескользящая,
	|	УчетнаяПолитика.ДетализироватьОтклоненияПоНаправлениямДеятельности,
	|	УчетнаяПолитика.ДетализироватьОтклоненияПоПодразделениям,
	|	УчетнаяПолитика.ДетализироватьОтклоненияПоСкладам
	|ПОМЕСТИТЬ ВТРегистраторыСОтклонениямиИУчетнымиПолитиками
	|ИЗ
	|	ВТРегистраторыСОтклонениями КАК Т
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаФинансовогоУчета.СрезПоследних(&НачалоПериода, Организация В(&МассивОрганизаций)) КАК УчетнаяПолитика
	|		ПО Т.ГоловнаяОрганизация = УчетнаяПолитика.Организация
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Регистратор
	|;
	|/////////////////////////////////////////////////////////////////////////////
	|
	// Условия заполнения полей должны соответствовать тексту запроса в РасчетСебестоимостиПроведениеДокументов.ДобавитьТекстЗапросаОтклоненияВСтоимостиТоваров
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Т.Период 										КАК Период,
	|	Т.Регистратор 									КАК Регистратор,
	|	Т.Организация 									КАК Организация,
	...
	|	Т.ВидЗапасов 									КАК ВидЗапасов,
	|	ВЫБОР
	|		КОГДА НЕ Регистраторы.ДетализироватьОтклоненияПоНаправлениямДеятельности
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
	|			ИНАЧЕ ЕСТЬNULL(Т.АналитикаУчетаНоменклатуры.Назначение.НаправлениеДеятельности,
	|				ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка))
	|	КОНЕЦ											КАК НаправлениеДеятельности,
	|	ВЫБОР
	|		КОГДА НЕ Регистраторы.ДетализироватьОтклоненияПоПодразделениям
	...
	|	ВЫБОР
	|		КОГДА НЕ Регистраторы.ДетализироватьОтклоненияПоСкладам
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|			ИНАЧЕ Т.АналитикаУчетаНоменклатуры.МестоХранения
	|	КОНЕЦ											КАК МестоХранения
	|ПОМЕСТИТЬ ВТДвиженияСебестоимостиДляОтклонений
	|ИЗ
	|	РегистрНакопления.СебестоимостьТоваров КАК Т
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТРегистраторыСОтклонениямиИУчетнымиПолитиками КАК Регистраторы
	|		ПО Т.Регистратор = Регистраторы.Регистратор
	|		И Регистраторы.Среднескользящая
	|ГДЕ
	|	Т.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И Т.Организация В(&МассивОрганизаций)
	|
	...
	|/////////////////////////////////////////////////////////////////////////////
	|
	// Движения регистратора по себестоимости не соответствуют его движениям по отклонениям
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	""ОтклоненияВСтоимостиТоваров"" КАК ИмяРегистра,
	|	Т.Регистратор КАК Ссылка,
	|	Т.Организация КАК Организация,
	...
	|		ПО Т.Регистратор = Регистраторы.Регистратор
	|		И Регистраторы.Среднескользящая
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТДвиженияСебестоимостиДляОтклонений КАК Себестоимость
	|		ПО Т.Номенклатура = Себестоимость.Номенклатура
	|		И Т.Период = Себестоимость.Период
	|		И Т.Регистратор = Себестоимость.Регистратор
	|		И Т.Организация = Себестоимость.Организация
	|		И Т.Характеристика = Себестоимость.Характеристика
	|		И Т.ВидЗапасов = Себестоимость.ВидЗапасов
	|		И Т.НаправлениеДеятельности = Себестоимость.НаправлениеДеятельности
	|		И Т.Подразделение = Себестоимость.Подразделение
	|		И Т.МестоХранения = Себестоимость.МестоХранения
	|ГДЕ
	|	Т.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И Т.Организация В(&МассивОрганизаций)
	|	И НЕ Т.РасчетПартий
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Есть движения по отклонениям при методе оценки стоимости, отличным от среднескользящей
	|ВЫБРАТЬ
	|	""ОтклоненияВСтоимостиТоваров"" КАК ИмяРегистра,
	|	Т.Регистратор КАК Ссылка,
	|	Т.Организация КАК Организация,
	|	2 КАК КодОшибки
	|ИЗ
	|	ВТРегистраторыСОтклонениямиИУчетнымиПолитиками КАК Т
	|ГДЕ
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументыПоОтдельнымРегистрамНакопления(
		...
		Запрос,
		РасшифровкаКодовОшибок,
		НСтр("ru = 'исправление движений документов по регистру ""Отклонения в стоимости товаров"".'",
			ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ТекстЗапросаСтатьиСРаздельнымУчетомНДСВРегистреПартииПрочихРасходов() Экспорт
	Возврат
		...
		// см. РегистрыНакопления.ПартииПрочихРасходов.ТекстЗапросаТаблицаВтПартииПрочихРасходов()
		// см. УчетНДСУП.ОтразитьПриобретениеУПоставщика()
		// см. ПартионныйУчетСервер.ТекстыЗапросовСторнирования()
		|		И (
		//++ Локализация
...
Функция ТекстЗапросаВТДвиженияСебестоимостьТоваров() Экспорт
	Возврат
		...
		// Полноценный учет готовой продукции по плановой стоимости поддерживается при списании отклонений на себестоимость продаж,
		// кроме метода оценки "Среднескользящая". При среднескользящей стоимости всегда ведеься учет продукции по плановой стоимости.
		|	(ВЫБОР
		|		КОГДА ЕСТЬNULL(УчетныеПолитикиФинУчета.УчетГотовойПродукцииПоПлановойСтоимости, ЛОЖЬ)
		|		 И ЕСТЬNULL(УчетныеПолитикиФинУчета.СписыватьОтклоненияВСтоимостиПродукцииНаСебестоимостьПродаж, ЛОЖЬ)
		|		 ИЛИ (ЕСТЬNULL(УчетныеПолитикиФинУчета.МетодОценкиСтоимостиТоваров, НЕОПРЕДЕЛЕНО)
		|				= ЗНАЧЕНИЕ(Перечисление.МетодыОценкиСтоимостиТоваров.Среднескользящая))
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ КОНЕЦ)		 КАК УчетГотовойПродукцииПоПлановойСтоимости,
...
Функция ТекстЗапросаДокументыСДублямиДвиженийПоВыручкеИСебестоимостиПродаж() Экспорт
	Возврат
		...
		|	И Т.Количество = 0
		|	И Т.Стоимость <> 0
		|	И НЕ Т.РасчетСебестоимости И НЕ Т.Сторно
		|	И НЕ Т.Регистратор ССЫЛКА Документ.КорректировкаРегистров
...
Функция ТекстЗапросаДокументыСПропавшимиДвижениямиПоВыручкеИСебестоимостиПродаж()
	Возврат
		...
		|			ИЛИ Себестоимость.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ВозвратНаДругойСклад))
		...
		// Для возврата товаров от клиента прошлых периодов себестоимость возврата определяется по движениям документа реализации.
		...
		|	И НЕ ЕСТЬNULL(Т.ВидЗапасов.Устаревший, ЛОЖЬ)
		|	И Т.Активность
		|	И НЕ Т.РасчетПартий
		|	И НЕ Т.РасчетСебестоимости
		|	И Себестоимость.Регистратор ЕСТЬ NULL
		...
		|		И Выручка.НастройкаХозяйственнойОперации = Т.НастройкаХозяйственнойОперации
		...
		// Для хоз. операции "Оприходование по возврату" расчет себестоимости не выполняется (себестоимость определяется данными документа),
		// поэтому по таким операциям не нужно проверять соответствие движений регистров.
		// По хоз операции "Реализация комиссионного товара" нет связи регистров себестоимости товаров и себестоимости продаж.
		// Для хоз операции "Реализация (только регл. учет)" нет движений по регистру "Выручка и себестоимость продаж".
		|	И НЕ Т.ХозяйственнаяОперация В (
		|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОприходованиеПоВозврату),
		...
		|	И ЕСТЬNULL(Т.ВидЗапасов.ТипЗапасов, НЕОПРЕДЕЛЕНО) <> ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
		|	И Т.НастройкаХозяйственнойОперации <> ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ОприходованиеПоВозврату)
		|	И ТИПЗНАЧЕНИЯ(Т.Регистратор) В (
		|		ТИП(Документ.РеализацияТоваровУслуг),
		|		ТИП(Документ.ОтчетОРозничныхВозвратах))
		|	И Т.Активность
		|	И НЕ Т.РасчетПартий
		|	И НЕ Т.РасчетСебестоимости
		|	И Выручка.Регистратор ЕСТЬ NULL
...
//		хозяйственной операцией "Выпуск продукции", и типом записи "Партия" или "ПередачаПродукцииИзПроизводства".
...
Функция ТекстЗапросаВключениеУчетаСебестоимости() Экспорт
	Возврат "
		...
		|	И ДД.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыТаможенныхДеклараций.ВыпущеноСТаможни)
		|;
		// Выбираем документы ввода остатков, которые не отражаются в себестоимости
		|ВЫБРАТЬ
		|	Т.Ссылка КАК Ссылка
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТВводОстатковСВыключеннымУчетомСебестоимости КАК ВводОстатков
		...
		// При использовании среднескользящей стоимости проверки движений не выполняются, т.к. механизм проведения документа
		// по одному регистру не учитывает что одновременно могут быть сформированы движения по корреспондирующим регистрам. 
		|	И НЕ ДокументыСДвижениями.Организация В (&ОрганизацииСреднескользящая)
		|;
...
Функция ТекстЗапросаВключениеУчетаСебестоимостиПоНазначениям() Экспорт
	Возврат "
		...
		// При использовании среднескользящей стоимости проверки движений не выполняются, т.к. механизм проведения документа
		// по одному регистру не учитывает что одновременно могут быть сформированы движения по корреспондирующим регистрам. 
		|	И НЕ ДокументыСНазначениями.Организация В (&ОрганизацииСреднескользящая)
		|;
