...
//
Функция ПодготовитьЗапросПроверкиПодключенияКУТМ(АдресУТМ, ПортУТМ, Таймаут, ОбменНаСервере, ИдентификаторВладельца = Неопределено,
	ИспользоватьЗащищенноеСоединение = Неопределено) Экспорт
	...
	Если ИспользоватьЗащищенноеСоединение <> Неопределено Тогда
		НастройкаОбменаЕГАИС.ИспользоватьЗащищенноеСоединение = ИспользоватьЗащищенноеСоединение;
	КонецЕсли;
...
//  * АлкогольнаяПродукцияЕГАИС - СправочникСсылка.КлассификаторАлкогольнойПродукцииЕГАИС - Алкогольная продукция.
...
//   * МаркируемаяПродукцияЕГАИС - Булево - .
//   * УпакованнаяПродукцияЕГАИС - Булево - . 
Функция РеквизитыАлкогольнойПродукцииДляСозданияНоменклатуры(АлкогольнаяПродукция) Экспорт
	...
	|	КлассификаторАлкогольнойПродукцииЕГАИС.ТипПродукции = ЗНАЧЕНИЕ(Перечисление.ТипыПродукцииЕГАИС.Упакованная) КАК УпакованнаяПродукция
	|ИЗ
	...
	Реквизиты.Вставить("УпакованнаяПродукцияЕГАИС",           ВыборкаРеквизиты.УпакованнаяПродукция);
...
// Возвращаемое значение:
//  Структура:
//  * ТекстНадписи - ФорматированнаяСтрока, Строка - описание доступных действий ЕГАИС
//  * ДанныеОписания - Массив из см. ИнтеграцияЕГАИС.ДанныеДокументаЕГАИС
Функция ДанныеПоляИнтеграцииВФормеДокументаОснования(Ссылка) Экспорт
	...
	Для Каждого КлючИЗначение Из ОформлениеДокументовПоПриоритетам Цикл
		...
		Для Каждого ДанныеДокументаЕГАИС Из Данные Цикл
			...
			Если Не ЗначениеЗаполнено(ДанныеДокументаЕГАИС.ТекстНадписи) Тогда
				Продолжить;
			КонецЕсли;
			...
			ДанныеДокументаЕГАИС.Удалить("МетаданныеДокумента");
			ВозвращаемоеЗначение.ДанныеОписания.Добавить(ДанныеДокументаЕГАИС);
			ФорматированныеСтроки.Добавить(ДанныеДокументаЕГАИС.ТекстНадписи);
			
		КонецЦикла;
		
	КонецЦикла;
	
	ВозвращаемоеЗначение.ТекстНадписи = Новый ФорматированнаяСтрока(ФорматированныеСтроки);
	Возврат ВозвращаемоеЗначение;
...
Процедура ЗаписатьСвязьДокументаПоступленияИТТНЕГАИС(ДокументСсылка, ДокументПоступления) Экспорт
	...
	Исключение
		...
		ОбщегоНазначенияЕГАИСВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
// 
// Параметры:
//  СсылкиДокументов - Массив из ОпределяемыйТип.ДокументыЕГАИС
// 
// Возвращаемое значение:
//  Булево - Возможна отправка подтверждения
Функция ВозможнаОтправкаПодтверждения(СсылкиДокументов) Экспорт
	
	Отказ = Ложь;
	
	СтатусыПроверкиИПодбора = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(СсылкиДокументов, "СтатусПроверкиИПодбора");
	
	КоличествоПроверенныхДокументов = 0;
	
	Для Каждого КлючЗначение Из СтатусыПроверкиИПодбора Цикл
		Если КлючЗначение.Значение = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиИПодбораИС.Завершено") Тогда
...
Функция РеквизитыИсточникаДанныхПараметровСканирования(ИсточникДанных) Экспорт
	
	Если ТипЗнч(ИсточникДанных) = Тип("ДокументСсылка.ЧекЕГАИС")
		Или ТипЗнч(ИсточникДанных) = Тип("ДокументСсылка.ЧекЕГАИСВозврат")
		Или ТипЗнч(ИсточникДанных) = Тип("ДокументСсылка.ВозвратИзРегистра2ЕГАИС")
		Или ТипЗнч(ИсточникДанных) = Тип("ДокументСсылка.ЗапросАкцизныхМарокЕГАИС") Тогда
		ОписаниеПолейОбъекта = "ОрганизацияЕГАИС, Ссылка, ДокументОснование";
	ИначеЕсли ТипЗнч(ИсточникДанных) = Тип("ДокументСсылка.ТТНВходящаяЕГАИС") Тогда
		ОписаниеПолейОбъекта = Новый Структура("Грузополучатель, Ссылка, ДокументОснование");
	ИначеЕсли ТипЗнч(ИсточникДанных) = Тип("ДокументСсылка.АктПостановкиНаБалансЕГАИС")
		Или ТипЗнч(ИсточникДанных) = Тип("ДокументСсылка.АктСписанияЕГАИС") Тогда
		ОписаниеПолейОбъекта = "ОрганизацияЕГАИС, ВидДокумента, Ссылка, ДокументОснование";
	ИначеЕсли ТипЗнч(ИсточникДанных) = Тип("ДокументСсылка.ТТНИсходящаяЕГАИС") Тогда
		ОписаниеПолейОбъекта = "Ссылка, ДокументОснование, Грузоотправитель";
	Иначе
		Возврат ИсточникДанных;
	КонецЕсли;
	
	Возврат ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ИсточникДанных, ОписаниеПолейОбъекта);
