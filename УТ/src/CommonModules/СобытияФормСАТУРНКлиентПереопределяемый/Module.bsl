...
Процедура ПриИзмененииНоменклатуры(Форма, ТекущаяСтрока, КэшированныеЗначения, ПараметрыУказанияСерий = Неопределено,
	...
	Если ИнтеграцияИСУТКлиентСервер.ТребуетсяЗаполнениеКодаТНВЭДСАТУРН(Форма, ТекущаяСтрока) Тогда
...
Процедура ОткрытьФормуПодбораНоменклатуры(Форма, ОповещениеПриЗавершении = Неопределено) Экспорт
	...
	ВидыПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииСАТУРН();
...
Процедура ОткрытьФормуПодбораТНВЭД(Форма, ТекущиеДанные, Владелец, ДополнительныеПараметры = Неопределено) Экспорт
	...
	//++ НЕ ГОСИС
	ОбработкаВыбораТНВЭД = Новый ОписаниеОповещения(
		"ЗаполнитьКодТНВЭДПоСтрокеПослеВыбораИзСервиса", ИнтеграцияИСУТКлиент, ТекущиеДанные);
	ВыбранноеЗначениеТНВЭД = ИнтеграцияИСВызовСервераУТ.НайтиЭлементКодТНВЭДПоКоду(ТекущиеДанные.КодТНВЭД);
	ПараметрыОткрытияФормы = Новый Структура("ТекущаяСтрока, РежимВыбора", ВыбранноеЗначениеТНВЭД, Истина);
	
	ОткрытьФорму("Справочник.КлассификаторТНВЭД.Форма.ФормаВыбора", ПараметрыОткрытияФормы, Форма,,,, ОбработкаВыбораТНВЭД);
