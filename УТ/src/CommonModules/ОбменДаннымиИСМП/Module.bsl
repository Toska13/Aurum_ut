...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПодготовитьКПередачеДлительнаяОперация", Истина);
	Методы.Вставить("ВыполнитьОбменДлительнаяОперация", Истина);
	Методы.Вставить("ПродолжитьВыполнениеОбменаДлительнаяОперация", Истина);
...
Функция ПодготовитьКПередачеУниверсально(ВходящиеДанные, ПараметрыОбмена) Экспорт
	...
	Для Каждого ПараметрыОбработкиДокумента Из ВходящиеДанные Цикл
		...
		Если ЗначениеЗаполнено(ПараметрыОбработкиДокумента.Ссылка) Тогда
			ОбновлениеИнформационнойБазы.ПроверитьОбъектОбработан(ПараметрыОбработкиДокумента.Ссылка);
		КонецЕсли;
		...
		Если ПараметрыОбработкиДокумента.ДальнейшееДействие = Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.СогласиеОПредоставлениеИнформации Тогда
			...
			МодульИнтеграцияИСМПСлужебный = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияИСМПСлужебный");
	...
	ТребуетсяПроверкаНаличияМОД      = ПараметрыОбмена.МОДКПроверкеРегистрации <> Неопределено;

	ДоступныеСертификаты          = Неопределено;
	АдресКОпределениюВидаУпаковки = "";
	СписокМОДКПроверкеРегистрации = Неопределено;
	...
	Если ТребуетсяПроверкаНаличияМОД И ОбщегоНазначенияИСМП.ЭтоРасширеннаяВерсияГосИС() Тогда
		СообщенияКПроверкеНаличияМОД = СообщенияКПроверкеНаличияМОДВСервисе(ПараметрыОбмена);
		
		ПроверитьКлючиСессии(СообщенияКПроверкеНаличияМОД, СертификатыДляПодписанияНаСервере, ПараметрыОбмена);
		ДоступныеСертификаты(СообщенияКПроверкеНаличияМОД, ДоступныеСертификаты);
		
		МодульПроверкиМОД           = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияИСМПСлужебный");
		РезультатыПроверкиМОДВГИСМТ = МодульПроверкиМОД.МОДКРегистрацииВГИСМТПоСпискуДокументов(
			ПараметрыОбмена.МОДКПроверкеРегистрации);
		
		Если РезультатыПроверкиМОДВГИСМТ.ТребуетсяОбновлениеКлючаСессии
			Или РезультатыПроверкиМОДВГИСМТ.МОДКРегистрации.Количество() = 0 Тогда
			ТребуетсяПроверкаНаличияМОД             = Ложь;
			ПараметрыОбмена.МОДКПроверкеРегистрации = Неопределено;
		ИначеЕсли РезультатыПроверкиМОДВГИСМТ.МОДКРегистрации.Количество() > 0 Тогда
			СписокМОДКПроверкеРегистрации = РезультатыПроверкиМОДВГИСМТ.МОДКРегистрации;
		КонецЕсли;
		
	КонецЕсли;
	
	Если ТребуетсяОжидание И Не ТребуетсяПодписание И Не ТребуетсяОпределениеВидаУпаковки 
		И Не ТребуетсяПроверкаНаличияМОД Тогда
	...
	Если (ТребуетсяОжидание Или ТребуетсяПодписание Или ТребуетсяОпределениеВидаУпаковки Или ТребуетсяПроверкаНаличияМОД)
		...
		ВозвращаемоеЗначение.МОДКПроверкеРегистрации        = СписокМОДКПроверкеРегистрации;
...
Функция СообщениеКПередачеJSON(ДокументСсылка, ДальнейшееДействие, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли ДальнейшееДействие = Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ЗапроситеМестаОсуществленияДеятельности Тогда
		...
		СообщениеJSON.ТоварнаяГруппа = ПараметрыЗапросаМОД.ТоварнаяГруппа;
...
// Подготавливает сообщения к передаче в ГИС МТ.
//
// Параметры:
...
Процедура ПодготовитьСообщенияКПередаче(СообщенияXML, ПараметрыОбмена, ИдентификаторЗаявки = Неопределено) Экспорт
	...
	Для Каждого Сообщение Из КоллекцияСообщенийXML Цикл
		...
		Если Документ = Неопределено Тогда
			Документ = "";
		КонецЕсли;
		СообщенияПоДокументу = СообщенияПоДокументам[Документ];
		Если СообщенияПоДокументу = Неопределено Тогда
			СообщенияПоДокументу = Новый Массив;
			СообщенияПоДокументам.Вставить(Документ, СообщенияПоДокументу);
		КонецЕсли;
	...
	Исключения.Вставить(Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ЗапроситеGTINНаОстатки, Истина);
	
	СоответствиеДействий = Новый Соответствие;
	СоответствиеДействий.Вставить(
		Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ВведитеВОборот,
		Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ПередайтеДанные);
	СоответствиеДействий.Вставить(
		Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ПередайтеДанные,
		Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ВведитеВОборот);
	СоответствиеДействий.Вставить(
		Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ПередайтеДанныеСИсправлениями,
		Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ВведитеВОборотСИсправлениями);
	СоответствиеДействий.Вставить(
		Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ВведитеВОборотСИсправлениями,
		Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ПередайтеДанныеСИсправлениями);
	...
	Для Каждого КлючИЗначение Из СообщенияПоДокументам Цикл
		...
		Попытка
			...
			Если ДальнейшееДействие <> Неопределено
				И Исключения[ДальнейшееДействие] <> Истина
				...
				ДействиеСиноним = СоответствиеДействий[Сообщение.ДальнейшееДействие];
				...
				Если  Сообщение.ДальнейшееДействие <> ТекущееСостояние.ДальнейшееДействие1
					...
					Если ДействиеСиноним = Неопределено
						Или ДействиеСиноним <> ТекущееСостояние.ДальнейшееДействие1
						И ДействиеСиноним <> ТекущееСостояние.ДальнейшееДействие2
						И ДействиеСиноним <> ТекущееСостояние.ДальнейшееДействие3 Тогда
			...
			Для Каждого Сообщение Из СообщенияПоДокументу Цикл
				...
				Если Не ЗначениеЗаполнено(Сообщение.ТекстОшибки) Тогда
					...
					РеквизитыОчереди.Вставить("ТоварнаяГруппа",               Сообщение.ТоварнаяГруппа);
					...
					Результат = ПодготовитьСообщениеКПередаче(
						Сообщение,
						РеквизитыОчереди);
					Результаты.Вставить(Сообщение, Результат);
		...
		Для Каждого Сообщение Из СообщенияПоДокументу Цикл
			...
			Результат = Результаты[Сообщение];
			Если Не ЗначениеЗаполнено(Сообщение.ТекстОшибки)
			...
			Иначе
				...
				Иначе
					// Текст ошибки сгенерирован в исключении
				КонецЕсли;
			...
			Если ПодготовленоКПередаче Тогда
				...
				ЭлементОчереди.ТоварнаяГруппа            = Сообщение.ТоварнаяГруппа;
				...
				ЭлементОчереди.ИдентификаторЗаявки          = Результат.РеквизитыОчереди.ИдентификаторЗаявки;
...
// Подготавливает сообщение к передаче в сервис ГИС МТ.
//
// Параметры:
//  Сообщение - см. СтруктураСообщенияXML
//  РеквизитыОчереди - Неопределено, Структура - Параметры очереди сообщений ГИС МТ
//
// Возвращаемое значение:
...
Функция ПодготовитьСообщениеКПередаче(Сообщение, РеквизитыОчереди)
	...
	ВозвращаемоеЗначение.Вставить("РеквизитыОчереди",   РеквизитыОчереди);
...
Процедура ПодготовитьСообщенияПоАбстрактнойОперацииПодключениеКОборудованиюРозлива(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	ВидыПродукции = ОбщегоНазначенияИСКлиентСервер.ПересечениеМассивов(
	...
	Если ОбщегоНазначенияИСМП.ЭтоРасширеннаяВерсияГосИС() Тогда
		...
		МодульИнтерфейсМОТП = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтерфейсМОТП");
...
Процедура ДобавитьСообщениеВОчередьНаПередачуДанных(Сообщение, Реквизиты)
	...
	// Назначение сообщений: ИСМП, МОТП или СУЗ
	НоваяЗапись.ТоварнаяГруппа            = Реквизиты.ТоварнаяГруппа;
...
Функция ПараметрыОбмена(Организации = Неопределено, УникальныйИдентификатор = Неопределено, ЭтоРегламентноеЗадание = Ложь) Экспорт
	...
	ПараметрыОбмена.Вставить("МОДКПроверкеРегистрации",  Неопределено);
	...
	Если ОбщегоНазначенияИСМП.ЭтоРасширеннаяВерсияГосИС() Тогда
		МодульИнтеграцияИСМП = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияИСМП");
...
Функция ИнициализиоватьОчередьСообщений()
	...
	// Назначение сообщений: ИСМП, МОТП или СУЗ
	ОчередьСообщений.Колонки.Добавить("ТоварнаяГруппа");
...
Функция СтруктураСообщенияJSON() Экспорт
	...
	СообщениеJSON.Вставить("ВерсияГосИС", ОбщегоНазначенияИС.ВерсияБиблиотеки());
	СообщениеJSON.Вставить("ТоварнаяГруппа");
...
Функция КатегорииОпераций()
	...
	ПередачаДанных.Вставить(
		...
		НСтр("ru = 'Передача сведений о вводе в оборот продукции, полученной из стран ЕАЭС'"));
	ПередачаДанных.Вставить(
		Перечисления.ВидыОперацийИСМП.ВводВОборотСырьяПолучениеТовараВнеЕАЭС,
		НСтр("ru = 'Передача сведений о вводе в оборот маркированной продукции, полученной из стран вне ЕАЭС'"));
	ПередачаДанных.Вставить(
	...
	ПередачаДанных.Вставить(
		Перечисления.ВидыОперацийИСМП.УдалитьВыводИзОборотаВПроцессеРеализацииПоДоговоруРассрочки,
	...
	Если ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ЭтоРасширеннаяВерсияГосИС() Тогда
		Менеджер = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени("Справочник.ПричиныВыводаИзОборотаИСМП");
		ПричиныВыводаИзОборота = Менеджер.ОписаниеПричин();
		ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(ПередачаДанных, ПричиныВыводаИзОборота);
	КонецЕсли;
	
	ПередачаДанных.Вставить(
		Перечисления.ВидыОперацийИСМП.ВозвратВОборот,
		НСтр("ru = 'Передача сведений о возврате в оборот'"));
	...
	ПередачаДанных.Вставить(
		...
		НСтр("ru = 'Передача сведений о списании эмитированных кодов маркировки через ГИС МТ'"));
	ПередачаДанных.Вставить(
		Перечисления.ВидыОперацийИСМП.СписаниеЧерезГИСМТОбщийДокумент,
		НСтр("ru = 'Передача сведений о списании эмитированных кодов маркировки через ГИС МТ (Общий документ)'"));
	ПередачаДанных.Вставить(
		Перечисления.ВидыОперацийИСМП.СписаниеЧерезСУЗ,
		НСтр("ru = 'Передача сведений о списании эмитированных кодов маркировки через СУЗ'"));
	ПередачаДанных.Вставить(
		Перечисления.ВидыОперацийИСМП.СписаниеЧерезСУЗМех,
		НСтр("ru = 'Передача сведений о списании эмитированных кодов маркировкич через СУЗ (Мех)'"));
	...
	ПередачаДанных.Вставить(
		Перечисления.ВидыОперацийИСМП.УдалитьСписаниеВведенныхВОборотКодовМаркировки,
	...
	ПередачаДанных.Вставить(
		Перечисления.ВидыОперацийИСМП.ОтчетИСМПОстаткиВиртуальногоСклада,
		НСтр("ru = 'Запрос остатков виртуального склада'"));
	ПередачаДанных.Вставить(
	...
	ПолучениеДанных.Вставить(
		Перечисления.ВидыОперацийИСМП.ОтчетИСМПОстаткиВиртуальногоСклада,
		НСтр("ru = 'Загрузка остатков виртуального склада'"));
...
Функция СтатусыЦветТекстаТребуетВнимания() Экспорт
	...
	Если ОбщегоНазначенияИСМП.ЭтоРасширеннаяВерсияГосИС() Тогда
		...
		МодульИнтеграциИСМП = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияИСМП");
...
// * МОДКПроверкеРегистрации - Неопределено, Соответствие из КлючИЗначение - 
Функция РезультатВыполненияОбменаДанными() Экспорт
	...
	ВозвращаемоеЗначение.Вставить("ИзвлекатьДанныеЛогаЗапросов",
...
Функция СообщенияКПроверкеНаличияМОДВСервисе(ПараметрыОбмена)
...
Функция ОбработатьОчередьСообщений(ПараметрыОбмена) Экспорт
	...
	ОбновлениеИнформационнойБазы.ПроверитьОбъектОбработан(Метаданные.РегистрыСведений.ОчередьСообщенийИСМП.ПолноеИмя());
	...
	Если ОбщегоНазначенияИСМП.ЭтоРасширеннаяВерсияГосИС() Тогда
		МодульРасширеннойОбработки = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияИСМПСлужебный");
	КонецЕсли;
...
Процедура ПостроитьОчередьСообщений(ПараметрыОбмена, СообщениеИлиДокумент = Неопределено) Экспорт
	...
	|	
	|	Очередь.ТоварнаяГруппа            КАК ТоварнаяГруппа,
	...
	Пока Выборка.Следующий() Цикл
		...
		ЭлементОчереди.ТоварнаяГруппа            = Выборка.ТоварнаяГруппа;
...
Процедура ОбработатьЭлементОчередиПолученияДанных(ЭлементОчереди, ПараметрыОбмена, МодульРасширеннойОбработки)
	...
	Если ЭлементОчереди.Назначение = Перечисления.НазначениеСообщенийИСМП.ИСМП Тогда
		...
		ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ПеремещениеМеждуМОД Тогда
			ЗагрузитьРезультатОбработкиПеремещенияМеждуМОДИСМП(ЭлементОчереди, ПараметрыОбмена);
		Иначе
...
Функция СообщениеМожноПередавать(ЭлементОчереди, ПараметрыОбмена, НайденныеЗависимости = Неопределено, ДатаСледующейИтерации = Неопределено, МодульРасширеннойОбработки = Неопределено)
	...
	Если ПовторноеФормирование И (ПараметрыОбмена.КОпределениюВидаУпаковки <> Неопределено 
		Или ПараметрыОбмена.МОДКПроверкеРегистрации <> Неопределено) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ПродолжитьВыполнениеОбменаУниверсально(ПодписанныеСообщенияПоОрганизациям, ПараметрыОбмена) Экспорт
	...
	Пока ВыполнитьИтерацию Цикл
		...
		Если ТребуетсяПроверкаНаличияМОД И ОбщегоНазначенияИСМП.ЭтоРасширеннаяВерсияГосИС() Тогда
			...
			ПроверитьКлючиСессии(СообщенияКПроверкеНаличияМОД, Неопределено, ПараметрыОбмена);
			ДоступныеСертификаты(СообщенияКПроверкеНаличияМОД, ДоступныеСертификаты);
...
Процедура ПередатьСообщениеИСМП(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей, НаборЗаписейОснование)
	...
	ДанныеОбработки = ИнтерфейсИСМПОбщегоНазначения.СоздатьДокументПоСообщению(
		...
		РеквизитыИсходящегоСообщения.ТоварнаяГруппа,
...
Процедура ЗагрузитьРезультатОбработкиПеремещенияМеждуМОДИСМП(ЭлементОчереди, ПараметрыОбмена)
	
	ОписаниеЗамера = ОценкаПроизводительности.НачатьЗамерДлительнойОперации(
		"ОбщийМодуль.ИнтеграцияИСМПСлужебный.ЗагрузитьРезультатОбработкиДокумента");
		
	МодульИнтерфейсИСМП = ОбщегоНазначения.ОбщийМодуль("ИнтерфейсИСМП");
	РезультатОбработкиЗаявки = МодульИнтерфейсИСМП.СтатусОбработкиУниверсальногоДокументаПоИдентификатору(ЭлементОчереди.Организация, ЭлементОчереди.ИдентификаторЗаявки);
...
Функция ПредставлениеСтатуса(Статус, ВходящиеДальнейшееДействие, ДопустимыеДействия, ДополнительноеСостояние = Неопределено) Экспорт
	...
	Для Каждого ДальнейшееДействие Из ДальнейшиеДействия Цикл
		...
		ИначеЕсли ДальнейшееДействие = Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ВведитеВОборот Тогда
			ТекстГиперссылки = "ПередатьДанные";
		...
		ИначеЕсли ДальнейшееДействие = Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ВведитеВОборотСИсправлениями Тогда
			ТекстГиперссылки = "ПередатьДанныеСИсправлениями";
