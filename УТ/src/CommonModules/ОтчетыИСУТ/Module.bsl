...
Функция ОформленныеДокументыВыводуИзОборотаИСМП()
	...
	Отгрузки = "";
	Если ДобавитьОснование(МассивТекстовЗапросов, "ОтгрузкаТоваровКлиенту", "Товары", Истина) Тогда
		ДобавитьОснование(МассивТекстовЗапросов, "ОтгрузкаТоваровКлиенту", "Серии");
		Отгрузки = СтрСоединить(МассивТекстовЗапросов, " ОБЪЕДИНИТЬ ВСЕ ");
		...
		Отгрузки = СтрЗаменить(Отгрузки, ".Проведен", ".Проведен "+ДопОтбор);
	КонецЕсли;
	МассивТекстовЗапросов.Очистить();
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(Отгрузки) Тогда
		МассивТекстовЗапросов.Добавить(Отгрузки);
	КонецЕсли;
...
Функция ОформленныеДокументыПоОтгрузкеПродукцииВЕТИС()
	...
	ДобавитьОснование(МассивТекстовЗапросов, "РеализацияТоваровУслуг", "Серии");
	ДобавитьОснование(МассивТекстовЗапросов, "ОтгрузкаТоваровКлиенту", "Товары", Истина);
	ДобавитьОснование(МассивТекстовЗапросов, "ОтгрузкаТоваровКлиенту", "Серии");
...
Функция ОформленныеДокументыПоДвижениюЗЕРНО()
	...
	ДобавитьОснованиеСВидомДвижения(МассивТекстовЗапросов, "ОтгрузкаТоваровКлиенту", "Товары", Истина, "-");
	ДобавитьОснованиеСВидомДвижения(МассивТекстовЗапросов, "ОтгрузкаТоваровКлиенту", "Серии",, "-");
	...
	ДобавитьОснованиеСВидомДвижения(МассивТекстовЗапросов, "ОприходованиеИзлишковОтгруженныхТоваров", "Товары");
	ДобавитьОснованиеСВидомДвижения(МассивТекстовЗапросов, "СписаниеОтгруженныхТоваров", "Товары",, "-");
...
Процедура ДоработатьЗапросПродукцииЗЕРНО(Текст)
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО Товары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
	|;
	|
	|";
...
Процедура ДоработатьЗапросПродукцииСАТУРН(Текст, ПостфиксВременнойТаблицыИз = "", ПостфиксВременнойТаблицыПоместить = "")
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО Товары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииСАТУРН <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
	|;
	|
...
Процедура УстановитьПараметрыОтчетаСАТУРН(ОтчетОбъект)
	...
	ТипыДокументаОснованияИсходящейНакладной.Добавить(Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту"));
