...
//
// Параметры:
//  Владелец - Произвольный - форма или элемент формы который будет использоваться в качестве
//               владельца открываемой формы.
...
//    Может принимать значения "СистемаБыстрыхПлатежей", "ИнтернетЭквайринг", "ОзонПэй", "ЯндексПэй".
//  ПараметрыПодключения - Неопределено - использовать параметры по умолчанию.
//                       - см. НовыйПараметрыПодключения.
//  
Процедура ПодключитьПлатежнуюСистему(ПлатежнаяСистема, ПараметрыПодключения) Экспорт
	
	// Проверка входных параметров
	ИмяМетода = "НастройкиПлатежныхСистемКлиент.ПодключитьПлатежнуюСистему";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		ИмяМетода,
		"ПлатежнаяСистема",
		ПлатежнаяСистема,
	...
	ОжидаемыеТипы.Добавить(Тип("Неопределено"));
	ОжидаемыеТипы.Добавить(Тип("Структура"));
	ОжидаемыеТипы.Добавить(Тип("ФиксированнаяСтруктура"));
...
//   * СообщениеОбОшибке - Строка - информация о причине запрета выбора указанного значения. Если заполнена, то
...
//     закрытии формы;
//   * РежимОткрытияОкна - РежимОткрытияОкнаФормы - указывает режим
//     открытия окна формы клиентского приложения;
