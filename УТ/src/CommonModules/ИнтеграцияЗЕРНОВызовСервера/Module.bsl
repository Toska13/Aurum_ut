...
//
// Возвращаемое значение:
//   Массив Из ПеречислениеСсылка.РазделыУчетаЗЕРНО - различные группы продукции в документе ЗЕРНО, заполненном 
//     по основанию без фильтра.
Функция РазделыУчетаДанныхЗаполнения(ПолноеИмяДокумента, Ссылка, КорректировочныйДокумент = Ложь) Экспорт
	
	ДоступныеРазделыУчета = Новый Массив;
	
	Если Не ИнтеграцияЗЕРНОКлиентСерверПовтИсп.ВестиУчетПродукции() Тогда
		Возврат ДоступныеРазделыУчета;
	КонецЕсли;
	...
	Возврат ИнтеграцияЗЕРНО.РазделыУчетаПоТаблицеНоменклатуры(Товары);
...
Функция СообщенияВОчередиТребующиеПодписания(ОрганизацииПодразделения = Неопределено, Сообщение = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|	ОчередьСообщенийЗЕРНО.РазделУчета                  КАК РазделУчета,
...
Функция ДанныеВыбораОКПД2(Текст, РазделУчета) Экспорт
	...
	Возврат ИнтеграцияЗЕРНО.ДанныеВыбораОКПД2(Текст, РазделУчета);
...
Функция ОрганизацияСертификата(Сертификат) Экспорт
	
	Если Не ЗначениеЗаполнено(Сертификат) Тогда
		Возврат ОбщегоНазначенияИС.ПустоеЗначениеОпределяемогоТипа("Организация");
	ИначеЕсли Не Метаданные.РегистрыСведений.ОчередьСообщенийЗЕРНО.Реквизиты.Сертификат.Тип.СодержитТип(ТипЗнч(Сертификат)) Тогда
		ВызватьИсключение НСтр("ru = 'Не верный тип значения сертификата для получения организации'");
	КонецЕсли;
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Сертификат, "Организация");
	
КонецФункции

Функция ЗначенияРеквизитовДляОпределенияДоступностиДействий(СсылкиНаДокументы, ПараметрыОпределения) Экспорт
	
	ИменаРеквизитов = ИнтеграцияЗЕРНОКлиентСервер.ИменаРеквизитовОпределенияДоступностиДействий();
	Реквизиты = Новый Массив;
	Для Каждого КлючИЗначение Из ИменаРеквизитов Цикл
		Реквизиты.Добавить(КлючИЗначение.Ключ);
	КонецЦикла;
	
	Возврат ОбщегоНазначения.ЗначенияРеквизитовОбъектов(СсылкиНаДокументы, Реквизиты);
