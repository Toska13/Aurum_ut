...
Процедура ПодключитьОрганизациюЛокально(Организация, ИдентификаторОрганизации, ИНН, КПП, КлючДоступа)
	
	НачатьТранзакцию();
	
	Попытка
		
		ЗаписатьИдентификаторОрганизации(Организация, ИдентификаторОрганизации, ИНН, КПП);
...
Процедура ЗаписатьИдентификаторОрганизации(Организация, Идентификатор, ИНН, КПП)
	...
	Попытка
		...
		// Заполним новую запись безопасно, для совместимости с разными версиями БЭД.
		ДанныеДляЗаполнения = Новый Структура;
		ДанныеДляЗаполнения.Вставить("Организация",   Организация);
		ДанныеДляЗаполнения.Вставить("Идентификатор", Идентификатор);
		ДанныеДляЗаполнения.Вставить("ИНН",           ИНН);
		ДанныеДляЗаполнения.Вставить("КПП",           КПП);
		
		ЗаполнитьЗначенияСвойств(НоваяЗапись, ДанныеДляЗаполнения);
...
Процедура ПодключитьОрганизациюПоКодуАктивации(Организация, КодАктивации, Отказ)
	...
	ИдентификаторОрганизации = ДанныеОрганизации[0].Идентификатор;
	ИНН                      = ДанныеОрганизации[0].ИНН;
	КПП                      = ДанныеОрганизации[0].КПП;
	
	ПодключитьОрганизациюЛокально(Организация, ИдентификаторОрганизации,ИНН, КПП, КлючДоступа);
...
Функция ТокенДоступаОрганизации(ПараметрыАвторизации, ЕстьОшибки, ТекстОшибки)
	...
	Попытка
		Токен = БизнесСеть_ТокенДоступаОрганизации(ПараметрыАвторизации.Организация);
...
#Область СовместимостьБЭД_191644
...
//
Функция БизнесСеть_ТокенДоступаОрганизации(Организация)
...
Функция ПолучитьОбновитьКлючДоступа(ИдентификаторОрганизации, ПринудительноОбновлятьКлючДоступа, Отказ)
	...
	Если ЗначениеЗаполнено(КлючДоступа) Тогда
		...
		Если БизнесСеть.ТребуетсяПовторноеПодключениеОрганизации(Неопределено, ИдентификаторОрганизации) Тогда
...
Процедура ПодключитьОрганизациюПоТикетуИТС(ИдентификаторОрганизации, Отказ)
	...
	КлючДоступа = КлючДоступаПоКодуАктивации(КодАктивации, БизнесСеть.НатуральныйИдентификаторОрганизации(Организация), Отказ);
...
Функция ИдентификаторБазыКлючаДоступаАктуален(КлючДоступа, ИдентификаторОрганизации)
	...
	Если БизнесСеть.ИдентификаторИнформационнойБазы() <> КлючДоступа.ИдентификаторИнформационнойБазы Тогда
