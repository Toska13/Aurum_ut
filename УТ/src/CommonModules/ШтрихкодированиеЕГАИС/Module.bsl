...
Процедура ДобавитьКолонкиТаблицыДанныеКодовМаркировки(ДанныеПоШтрихкодам, ПараметрыСканирования = Неопределено, ДанныеКодовМаркировки = Неопределено, ТребуетсяЧастичноеВыбытие = Неопределено) Экспорт
	...
	ОбязательныеКолонки.Вставить("Статус",                  Новый ОписаниеТипов(ТипыЗначенийКолонкиСтатус));
	...
	ОбязательныеКолонки.Вставить("ТребуетсяВыборАлкогольнойПродукции", Новый ОписаниеТипов("Булево"));
	ОбязательныеКолонки.Вставить("ТребуетсяВыборСправки2",             Новый ОписаниеТипов("Булево"));
	...
	Если ТребуетсяЧастичноеВыбытие Тогда
		...
		ОбязательныеКолонки.Вставить("ЕмкостьПотребительскойУпаковки",      ОбщегоНазначения.ОписаниеТипаЧисло(10, 3));
		ОбязательныеКолонки.Вставить("ЧастичноеВыбытиеВыведено",            ОбщегоНазначения.ОписаниеТипаЧисло(10, 3));
...
Процедура ЗаполнитьСтрокиДерева(СтрокаВерхнегоУровня, ПараметрыЗаполнения,
	...
	Для Каждого СтрокаДерева Из СтрокаВерхнегоУровня.Строки Цикл
		...
		Если Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(СтрокаДерева.ВидПродукции) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура НормализоватьДанныеДереваУпаковок(ДеревоУпаковок, ПараметрыСканирования, ДанныеКодовМаркировки = Неопределено, ТолькоПересчетНоменклатурыИКоличества = Ложь, ЗаполнятьСтрокиДерева = Ложь) Экспорт
	...
	ПараметрыЗаполнения.Кэш.КолонкиДляПересчетаНоменклатурыИКоличества.Вставить("МаркируемаяПродукция", Истина);
	...
	Пока НуженПоискВложенныхШтрихкодов Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если СоответствиеСтрокДереваУпаковок[Выборка.Упаковка] = Неопределено Тогда
				...
				СтрокаРодитель.ВидПродукции = ВидПродукции;
			...
			Если ЗначениеЗаполнено(Выборка.УпаковкаУровень1) Тогда
				...
				Если СоответствиеСтрокДереваУпаковок[Выборка.УпаковкаУровень1] = Неопределено Тогда
					...
					СтрокаУпаковкиУровень1.ВидПродукции = ВидПродукции;
				...
				Если ЗначениеЗаполнено(Выборка.УпаковкаУровень2) Тогда
					...
					Если СоответствиеСтрокДереваУпаковок[Выборка.УпаковкаУровень2] = Неопределено Тогда
						...
						СтрокаУпаковкиУровень2.ВидПродукции = ВидПродукции;
					...
					Если ЗначениеЗаполнено(Выборка.УпаковкаУровень3) Тогда
						...
						Если СоответствиеСтрокДереваУпаковок[Выборка.УпаковкаУровень3] = Неопределено Тогда
							...
							СтрокаУпаковкиУровень3.ВидПродукции = ВидПродукции;
...
Функция ТекстЗапросаВложенныхШтрихкодовПоДокументу(ПараметрыФормированияТекстаЗапроса) Экспорт
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "%ТекстЗапросаСвойстваМаркируемойПродукции%", 
		ШтрихкодированиеОбщегоНазначенияИС.ТекстЗапросаСвойстваМаркируемойПродукции(
			, Новый Структура("ЕГАИС, ИСМП", Истина, Ложь)));
...
//Проверяет необходимость уточнения кода маркировки в составе упаковки, специфика ЕГАИС
//  * Заполнение Справки2
//  * Заполнение алкогольной продукции
//
// Параметры:
...
Функция ПроверитьНеобходимостьУточнения(СтрокаДерева, ПараметрыСканирования) Экспорт
	...
	Если ПараметрыСканирования.ЗапрашиватьАлкогольнуюПродукцию
		И Не ЗначениеЗаполнено(СтрокаДерева.АлкогольнаяПродукция) Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если ПараметрыСканирования.ЗапрашиватьСправку2
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если СтрокаДанных.Идентифицирован
			Или (ЗначениеЗаполнено(СтрокаДанных.ВидПродукции)
				И Не ОбщегоНазначенияИСПовтИсп.ЭтоМаркируемаяПродукцияЕГАИС(СтрокаДанных.ВидПродукции))
...
Процедура СоздатьШтрихкодУпаковкиПоДаннымШтрихкода(ДанныеШтрихкода, ПараметрыСканирования) Экспорт
	...
	Если ПараметрыСканирования.СоздаватьШтрихкодУпаковки
		...
		И Не ДанныеШтрихкода.ЭтоШтрихкодНоменклатуры
	...
	Если ДанныеПоШтрихкодам.ВложенныеШтрихкоды = Неопределено Тогда
		...
		Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
			...
			ИначеЕсли Не ПараметрыСканирования.ТолькоЕГАИС
				И КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииИСМПЕГАИС(СтрокаДанных.ВидПродукции) Тогда
				Продолжить;
			ИначеЕсли Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(СтрокаДанных.ВидПродукции) Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ЗаполнитьТаблицуДанныеКодовМаркировкиОписаниемНоменклатуры(ДанныеКодовМаркировки, ПараметрыСканирования) Экспорт
	...
	Для Каждого Строка Из ДанныеКодовМаркировки Цикл
		Если ЗначениеЗаполнено(Строка.Номенклатура)
			И ОбщегоНазначенияИСПовтИсп.ЭтоМаркируемаяПродукцияЕГАИС(Строка.ВидПродукции) Тогда
	...
	Для Каждого СтрокаТаблицы Из ДанныеКодовМаркировки Цикл
		...
		Если Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(СтрокаТаблицы.ВидПродукции) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьДанныеНеизвестныхШтрихкодовСНомеромИСерией(СтрокиДанных, ПараметрыСканирования)
	...
	Если ВидыПродукцииЕГАИС.Количество() =1 Тогда
		ВидПродукции = ВидыПродукцииЕГАИС[0];
	Иначе
...
Функция ШтрихкодыУпаковокДляФормированияСпецифики(ДанныеКодовМаркировки)
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(СтрокаДанных.ВидПродукции)
...
Процедура ДополнитьДанныеПоШтрихкодамПараметрамиДополнения(ДанныеПоШтрихкодам, ПараметрыСканирования, ПараметрыДополнения) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(СтрокаДанных.ВидПродукции) Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого КлючИЗначение Из СтрокиДляЗаполнения Цикл
		...
		Если ДанныеЗаполненияПоЧастичномуВыбытию <> Неопределено
			И КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(СтрокаДанных.ВидПродукции) Тогда
