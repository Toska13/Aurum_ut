...
//       * КорректировочноеУведомление - Булево - Признак какое нужно формировать уведомление. Если КорректировочноеУведомление = Истина, нужно формировать корректировочное уведомление.
//
Функция ПараметрыРегистрацииУведомленийОВвозеПрослеживаемыхТоваров(Объект) Экспорт
	
	ПараметрыРегистрации = Новый Структура;
	
	ПараметрыРегистрации.Вставить("Ссылка",                        Объект.ДокументПоступления);
	...
	ПараметрыРегистрации.Вставить("КорректировочноеУведомление",   Истина);
	
	//++ Локализация
	
	ЗакупкаВСтранахЕАЭС = ЗакупкиСервер.ХозяйственныеОперацииПоОсновной(Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС);
	Если ТипЗнч(Объект.ДокументПоступления) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
		ХозяйственнаяОперацияОснования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументПоступления, "ХозяйственнаяОперация");
		Если ЗакупкаВСтранахЕАЭС.Найти(ХозяйственнаяОперацияОснования) <> Неопределено Тогда
...
//++ Локализация
	...
	|	ЕСТЬNULL(Аналитика.КлючАналитики, ТаблицаВидыЗапасов.АналитикаУчетаНоменклатуры) КАК АналитикаУчетаНоменклатуры,
	...
	|			И ТаблицаВидыЗапасов.АналитикаУчетаНоменклатуры.СтатьяКалькуляции = Аналитика.СтатьяКалькуляции
	|			И Аналитика.МестоХранения = ПриобретениеТоваровУслуг.Склад
	...
	|ГДЕ
	|	ПриобретениеТоваровУслуг.ХозяйственнаяОперация В (&ХозОперацииЗакупкаВСтранахЕАЭС)
