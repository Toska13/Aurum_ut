...
//		* ИспользоватьКоличествоСерии - Булево - признак партионного учета серий номенклатуры.
...
//		* ПоставляетсяВМногооборотнойТаре - Булево - Истина - признак того, что номенклатура поставляется в таре.
...
//		* ИспользоватьУпаковки - Булево - Истина - признак использования упаковок.
//		* Серия - СправочникСсылка.СерииНоменклатуры - серия товара.
//		* СерииИспользуются - Булево - Истина - признак использования серий.
//		* СтатусыУказанияСерий - Соответствие Из КлючИЗначение - коллекция содержащая сведения о статусах указания серий на складах:
//			** Ключ - СправочникСсылка.Склады - склад учета номенклатуры.
//			** Значение - Число - статус указания серии на складе.
//		* ШтрихкодНоменклатуры - Булево - признак, что информация о штрихкоде получена из регистра сведений ШтрихкодыНоменклатуры.
//		* НеуникальныйШтрихкод - Булево - признак уникальности штрихкода.
//		* ПолученПоДаннымИБ - Булево - признак, что данные штрихкода номенклатуры получены из ИБ.
//
Функция СтруктураКэшируемойИнформацииПоШтрихкоду() Экспорт
	
	ИнформацияПоШтрихкоду = Новый Структура;
	
	ИнформацияПоШтрихкоду.Вставить("Номенклатура",						ПредопределенноеЗначение("Справочник.Номенклатура.ПустаяСсылка"));
	ИнформацияПоШтрихкоду.Вставить("ТипНоменклатуры",					ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.ПустаяСсылка"));
	ИнформацияПоШтрихкоду.Вставить("ИспользоватьКоличествоСерии",		Истина);
	ИнформацияПоШтрихкоду.Вставить("Характеристика",					ПредопределенноеЗначение("Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка"));
	ИнформацияПоШтрихкоду.Вставить("Упаковка",							ПредопределенноеЗначение("Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка"));
	ИнформацияПоШтрихкоду.Вставить("ПредставлениеНоменклатуры",			"");
	ИнформацияПоШтрихкоду.Вставить("ПодакцизныйТовар",					Ложь);
	ИнформацияПоШтрихкоду.Вставить("МаркируемаяПродукция",				Ложь);
	ИнформацияПоШтрихкоду.Вставить("ХарактеристикиИспользуются",		Ложь);
	ИнформацияПоШтрихкоду.Вставить("ПоставляетсяВМногооборотнойТаре",	Ложь);
	ИнформацияПоШтрихкоду.Вставить("Коэффициент",						1);
	ИнформацияПоШтрихкоду.Вставить("ЕдиницаИзмерения",					ПредопределенноеЗначение("Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка"));
	ИнформацияПоШтрихкоду.Вставить("ЕдиничнаяУпаковка",					ПредопределенноеЗначение("Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка"));
	ИнформацияПоШтрихкоду.Вставить("НесколькоЕдиничныхУпаковок",		Ложь);
	...
	ИнформацияПоШтрихкоду.Вставить("ИспользоватьУпаковки",				Ложь);
	ИнформацияПоШтрихкоду.Вставить("Серия",								ПредопределенноеЗначение("Справочник.СерииНоменклатуры.ПустаяСсылка"));
	ИнформацияПоШтрихкоду.Вставить("СерииИспользуются",					Ложь);
	ИнформацияПоШтрихкоду.Вставить("СтатусыУказанияСерий",				Новый Соответствие);
	ИнформацияПоШтрихкоду.Вставить("ШтрихкодНоменклатуры",				Истина);
	ИнформацияПоШтрихкоду.Вставить("НеуникальныйШтрихкод",				Ложь);
	ИнформацияПоШтрихкоду.Вставить("ПолученПоДаннымИБ",					Истина);
	
	ПакетнаяОбработкаТабличнойЧастиКлиентСерверЛокализация.ДополнитьСтруктуруКэшируемойИнформацииПоШтрихкоду(
		ИнформацияПоШтрихкоду);
...
// Параметры заполнения соглашения поставщика по умолчанию.
// 
// Возвращаемое значение:
//  Структура - Параметры заполнения соглашения поставщика по умолчанию:
// * ТолькоДействующее - Булево - Если сброшен, то выбирается без отбора по статусу 
// * ХозяйственнаяОперация - Неопределено, Массив Из ПеречислениеСсылка.ХозяйственныеОперации, ПеречислениеСсылка.ХозяйственныеОперации - Отбор по соответствующему реквизиту.
// * ЦенаВключаетНДС - Неопределено, Булево - Отбор по соответствующему реквизиту.
// * ИспользуютсяДоговорыКонтрагентов - Неопределено, Булево - Отбор по соответствующему реквизиту.
Функция ПараметрыЗаполненияСоглашенийПоставщика() Экспорт
	
	ВозвращаемоеЗначение = Новый Структура;
	
	ВозвращаемоеЗначение.Вставить("ТолькоДействующее",                Истина);
	ВозвращаемоеЗначение.Вставить("ХозяйственнаяОперация",            Неопределено);
	ВозвращаемоеЗначение.Вставить("ЦенаВключаетНДС",                  Неопределено);
	ВозвращаемоеЗначение.Вставить("ИспользуютсяДоговорыКонтрагентов", Неопределено);
	
	Возврат ВозвращаемоеЗначение;
	
КонецФункции

// Параметры заполнения договора по умолчанию.
// 
// Возвращаемое значение:
//  Структура - Параметры заполнения договора по умолчанию:
// * Организация - СправочникСсылка.Организации - Отбор договора по организации
// * ХозяйственнаяОперация -Неопределено, Массив Из ПеречислениеСсылка.ХозяйственныеОперации, ПеречислениеСсылка.ХозяйственныеОперации - Отбор по хозяйственной операции
// * ВалютаВзаиморасчетов - Неопределено, СправочникСсылка.Валюты - Отбор по валюте взаиморасчетов
// * НаправлениеДеятельности - Неопределено, СправочникСсылка.НаправленияДеятельности - Отбор по направлению деятельности
// * КомиссионныеПродажи25 - Неопределено, Булево - Отбор по признаку комиссионной продажи.
// * ПорядокРасчетов - Неопределено, ПеречислениеСсылка.ПорядокРасчетов - Отбор по порядоку расчетов.
// * ПорядокОформленияСписанияНедостачТоваровПринятыхНаХранение - Неопределено, ПеречислениеСсылка.ПорядокОформленияСписанияТоваровПринятыхНаХранение - Отбор по соответстующему реквизиту.
// * ДоговорССамозанятым - Булево - Значение отбора по соответствующему реквизиту.
// * Налогообложение     - ПеречислениеСсылка.ТипыНалогообложенияНДС  - Определяет использование агентского НДС
// * ТипДоговора         - Неопределено, Массив из ПеречислениеСсылка.ТипыДоговоров - отбор по типу договора
Функция ПараметрыЗаполненияДоговораПоУмолчанию() Экспорт
	
	ВозвращаемоеЗначение = Новый Структура;
	ВозвращаемоеЗначение.Вставить("Организация",             ПредопределенноеЗначение("Справочник.Организации.ПустаяСсылка"));
	ВозвращаемоеЗначение.Вставить("ХозяйственнаяОперация",   Неопределено);
	ВозвращаемоеЗначение.Вставить("ВалютаВзаиморасчетов",    Неопределено);
	ВозвращаемоеЗначение.Вставить("НаправлениеДеятельности", Неопределено);
	ВозвращаемоеЗначение.Вставить("КомиссионныеПродажи25",   Неопределено);
	ВозвращаемоеЗначение.Вставить("ПорядокРасчетов",         Неопределено);
	ВозвращаемоеЗначение.Вставить("ТипДоговора",             Неопределено);
	ВозвращаемоеЗначение.Вставить("ПорядокОформленияСписанияНедостачТоваровПринятыхНаХранение",
		Неопределено);
	
	ВозвращаемоеЗначение.Вставить("Налогообложение",
		ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка"));
	
	ВозвращаемоеЗначение.Вставить("ДоговорССамозанятым", Ложь);
	
	Возврат ВозвращаемоеЗначение;
	
КонецФункции

// Параметры заполнения для действия "ЗаполнитьОперациюПоСоглашениюДоговору" 
// 
// Возвращаемое значение:
// Структура:
//  * ЗначениеПоУмолчанию - ПеречислениеСсылка.ХозяйственныеОперации - Значение операции, устанавливаемое, если не удалось заполнить из соглашения и договора.
Функция ПараметрыЗаполненияОперацииПоСоглашениюДоговору() Экспорт
	
	ВозвращаемоеЗначение = Новый Структура;
	ВозвращаемоеЗначение.Вставить("ЗначениеПоУмолчанию", ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПустаяСсылка"));
	
	Возврат ВозвращаемоеЗначение;
	
КонецФункции

// Возвращает структуру, содержащую поля значений, используемых для заполнения видов цен в строках табличной части документа.
//
// Параметры:
//	ВидЦены - Строка, СправочникСсылка.ВидыЦен - имя поля или значение вида цен.
//
// Возвращаемое значение:
//  Структура - Параметры заполнения вида цен в строке ТЧ:
// * ВидЦены - Строка, СправочникСсылка.ВидыЦен - имя поля или значение вида цен.
// * ВидЦеныПоУмолчанию - Неопределено, СправочникСсылка.ВидыЦен - значение, используемое для заполнения по умолчанию.
// * ПараметрыОтбора - Структура - параметры отбора вида цен:
// ** ЦенаВключаетНДС - Неопределено, Булево - если Неопределено, то отбор не задан.
// ** Статус - ПеречислениеСсылка.СтатусыДействияВидовЦен - если Неопределено, то отбор не задан.
// ** ИспользоватьПриПродаже - Неопределено, Булево - если Неопределено, то отбор не задан.
// ** ИспользоватьПриПередачеПродукцииДавальцу - Неопределено, Булево - если Неопределено, то отбор не задан.
//
Функция ПараметрыЗаполненияВидаЦенВСтрокеТЧ(ВидЦены) Экспорт
	
	ПараметрыОтбора = Новый Структура;
	ПараметрыОтбора.Вставить("ЦенаВключаетНДС");
	ПараметрыОтбора.Вставить("Статус", ПредопределенноеЗначение("Перечисление.СтатусыДействияВидовЦен.Действует"));
	ПараметрыОтбора.Вставить("ИспользоватьПриПродаже");
	ПараметрыОтбора.Вставить("ИспользоватьПриПередачеПродукцииДавальцу");
	
	ПараметрыЗаполненияВидаЦен = Новый Структура;
	ПараметрыЗаполненияВидаЦен.Вставить("ВидЦены", ВидЦены);
	ПараметрыЗаполненияВидаЦен.Вставить("ВидЦеныПоУмолчанию");
	ПараметрыЗаполненияВидаЦен.Вставить("ПараметрыОтбора", ПараметрыОтбора);
	
	Возврат ПараметрыЗаполненияВидаЦен;
...
//
// Параметры:
//	Объект - ДанныеФормыСтруктура, Структура - данные документа:
//		* Дата - Дата
...
//	Структура - структура, используемая для пересчета сумм НДС в табличной части документа:
//		* НалогообложениеНДС - ПеречислениеСсылка.ТипыНалогообложенияНДС - Налогообложение документа.
//		* Организация - Неопределено, СправочникСсылка.Организации - Организация документа
//		* Дата - Дата, Неопределено - Дата документа
...
//		* Суффикс - Строка
//		* ИнициализацияВходящегоДокумента - Булево
//		* ЗаполнениеНаОснованииКопирование - Булево
//		* НалогообложениеНДСВТабличнойЧасти - Булево
//		* УказаннаяСтавкаНДС - Неопределено
//
Функция ПараметрыЗаполненияСтавкиНДС(Объект = Неопределено, ПоДатеОтгрузки = Ложь, ВернутьМногооборотнуюТару = Ложь) Экспорт
	
	Параметры = Новый Структура;
	Параметры.Вставить("Дата", Неопределено);
	...
	Параметры.Вставить("УказаннаяСтавкаНДС", Неопределено);
...
// Возвращает структуру, содержащую параметры для заполнения ставки НДС 
// по Виду цены и регистра сведений "Цены номенклатуры 2.5" при обработке строк табличной части.
//
// Параметры:
...
//
// Возвращаемое значение:
//	Структура - структура, используемая для заполнения ставки НДС по виду цены в табличной части документа:
//		* Дата - Дата - Дата документа
//		* Организация - СправочникСсылка.Организации - Организация документа
//		* Партнер - СправочникСсылка.Партнеры - Партнер документа
//		* Соглашение - СправочникСсылка.СоглашенияСКлиентами - Соглашение документа
//		* ВидЦены - СправочникСсылка.ВидыЦен - вид цены выбранный для заполнния по строкам документа.
//		* ЗаполнитьВидЦены - Булево - Признак того, что необходимо заполнить вид цены для товара
//		* ВернутьМногооборотнуюТару - Булево - Признак необходимости возврата многооборотной тары.
//		* Суффикс - Строка - Префикс реквизита заполнения ставки НДС.
//		* ХозяйственнаяОперация - ПеречислениеСсылка.ХозяйственныеОперации - Хозяйственная операция документа.
//		* НалогообложениеНДС - ПеречислениеСсылка.ТипыНалогообложенияНДС - Налогообложение документа.
//		* ПоДатеОтгрузки - Булево - Признак того, что ставки необходимо получать на дату отгрузки
//
Функция ПараметрыЗаполненияСтавкиНДСПоВидуЦен(Объект = Неопределено, ПоДатеОтгрузки = Ложь) Экспорт
	
	Параметры = Новый Структура;
	Параметры.Вставить("Дата");
	Параметры.Вставить("Организация");
	Параметры.Вставить("СтранаРегистрацииОрганизации");
	Параметры.Вставить("Партнер");
	Параметры.Вставить("Соглашение");
	Параметры.Вставить("ВидЦены");
	Параметры.Вставить("ЗаполнитьВидЦены", Ложь);
	Параметры.Вставить("ВернутьМногооборотнуюТару", Ложь);
	Параметры.Вставить("Суффикс", "");
	Параметры.Вставить("ХозяйственнаяОперация", ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПустаяСсылка"));
	Параметры.Вставить("НалогообложениеНДС", ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС"));
	Параметры.Вставить("ПоДатеОтгрузки", ПоДатеОтгрузки);
	...
	Если Объект <> Неопределено Тогда
		...
		Параметры.Дата = Объект.Дата;
		Если ПоДатеОтгрузки Тогда
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "НеОтгружатьЧастями")
				И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "ДатаОтгрузки")
				И Объект.НеОтгружатьЧастями И ЗначениеЗаполнено(Объект.ДатаОтгрузки) Тогда
					
				Параметры.Дата = Объект.ДатаОтгрузки;
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "Организация") Тогда
			Параметры.Организация = Объект.Организация;
			Параметры.СтранаРегистрацииОрганизации = ЗначениеНастроекКлиентСерверПовтИсп.СтранаРегистрацииОрганизации(Объект.Организация);
		КонецЕсли;
		
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "НалогообложениеНДС") Тогда
			Параметры.НалогообложениеНДС = Объект.НалогообложениеНДС;
		КонецЕсли;
		
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "ВернутьМногооборотнуюТару") Тогда
			Параметры.ВернутьМногооборотнуюТару = Объект.ВернутьМногооборотнуюТару;
		КонецЕсли;
		
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "ХозяйственнаяОперация") Тогда
			Параметры.ХозяйственнаяОперация = Объект.ХозяйственнаяОперация;
		КонецЕсли;
...
// Возвращает структуру, содержащую валюту документа, валюту регламентированного учета организации
// и валюту управленческого учета.
//
// Параметры:
// 	ВалютаДокумента - СправочникСсылка.Валюты - валюта документа.
// 	ВалютаРегл - СправочникСсылка.Валюты - валюта регламентированного учета организации.
...
//
Функция ПараметрыЗаполненияСуммыНДСРеглУпр(ВалютаДокумента, ВалютаРегл, ВалютаУпр) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ВалютаДокумента", ВалютаДокумента);
	Результат.Вставить("ВалютаРегл", ВалютаРегл);
...
//		* ЦенаВключетНДС - Неопределено, Булево - Отбор по признаку "ЦенаВключаетНДС" при поиске вида цен. Если не заполнен, то отбор не используется. 
//		* ИспользоватьСоглашениеИзТаблицы - Булево - Указывает на необходимость использования соглашения не из шапки, а из табличной части.
Функция ПолучитьСтруктуруЗаполненияЦеныЗакупкиВСтрокеТЧ(Объект, ИмяПоляВидЦены = Неопределено) Экспорт
	...
	СтруктураЗаполненияЦены.Вставить("ИспользоватьСоглашениеИзТаблицы", Ложь);
	СтруктураЗаполненияЦены.Вставить("ЦенаВключаетНДС", Неопределено);
...
//		* Соглашение - СправочникСсылка.СоглашенияСКлиентами - соглашение по документу.
//		* Партнер - СправочникСсылка.Партнеры - партнер по документу.
//		* Договор - СправочникСсылка.ДоговорыКонтрагентов - договор по документу.
//		* Ссылка - ДокументСсылка - ссылка документа.
//		* ЭтоПродажаКонечномуПокупателю - Булево - Определяет вид цены, используемый из договора комиссии 2.5.
//
Функция ПолучитьСтруктуруЗаполненияУсловийПродажВСтрокеТЧ(Объект) Экспорт
	...
	ВозвращаемоеЗначение = Новый Структура;
	ВозвращаемоеЗначение.Вставить("Ссылка");
	ВозвращаемоеЗначение.Вставить("Дата");
	ВозвращаемоеЗначение.Вставить("Валюта");
	ВозвращаемоеЗначение.Вставить("Партнер");
	ВозвращаемоеЗначение.Вставить("Соглашение");
	ВозвращаемоеЗначение.Вставить("Договор");
	
	ЭтоПродажаКонечномуПокупателю = ТипыДокументовПродажиКонечномуПокупателю.Найти(ТипЗнч(Объект.Ссылка)) <> Неопределено;
	
	ВозвращаемоеЗначение.Вставить("ЭтоПродажаКонечномуПокупателю", ЭтоПродажаКонечномуПокупателю);
	
	ЗаполнитьЗначенияСвойств(ВозвращаемоеЗначение, Объект);
...
// Параметры:
//  Объект				 - ДанныеФормыСтруктура - данные документа.
//  СкидкиНеИспользуются - Булево				- Истина, если в документе не используются скидки. 
...
//  * ИмяКоличества - Строка - имя поля количества в табличной части документа.
//
Функция ПолучитьСтруктуруПересчетаЦеныСкидкиВПродажахВТЧ(Объект, СкидкиНеИспользуются = Ложь, ИмяКоличества = "КоличествоУпаковок") Экспорт
	
	СтруктураЗаполненияЦены = Новый Структура;
	
	Если Не СкидкиНеИспользуются Тогда
...
// 	ИмяПоляСклад - Строка
//  ИмяПоляСписатьНаРасходы - Строка
// 	
// Возвращаемое значение:
// 	Структура - Описание:
// * ИмяПоляСклад - Строка
...
// * ИмяПоляСписатьНаРасходы - Строка
//
Функция ПолучитьСтруктуруЗаполненияСкладаВСтрокеТЧ(Объект, СкладГруппа, ИмяПоляСкладОбъекта = "Склад", ИмяПоляСклад = "Склад", ИмяПоляСписатьНаРасходы = "") Экспорт
	...
	СтруктураПараметров.Вставить("ИмяПоляСписатьНаРасходы", ИмяПоляСписатьНаРасходы);
...
// Возвращает структуру, содержащую параметры для заполнения признака подакцизный товар
// документа.
//
// Параметры:
//	Объект - ДанныеФормыСтруктура, ДокументОбъект - данные документа.
...
//		* Дата - Дата - Дата документа
//
Функция ПараметрыЗаполненияПризнакаПодакцизныйТовар(Объект) Экспорт
...
// Возвращает структуру, содержащую параметры для пересчета суммы акциза в строках табличной части документа.
//
// Параметры:
//	Объект - ДанныеФормыСтруктура, ДокументОбъект - данные документа.
//	СтавкаЗаЕдиницуХранения - Булево - Признак того, что ставку необходимо получать за единицу хранения номенклатуры.
//
// Возвращаемое значение:
//	Структура - структура, используемая для пересчета суммы акциза в строках табличной части документа:
//		* Дата - Дата - дата документа.
//		* Организация - СправочникСсылка.Организации - организация по документу, осуществляющая хозяйственную операцию.
//		* СтавкаЗаЕдиницуХранения - Булево - признак получения ставки акциза за единицу хранения номенклатуры.
//		* ИмяПоляКоличество - Строка - имя поля количество в табличной части документа.
//
Функция ПараметрыПересчетаСуммыАкциза(Объект = Неопределено, СтавкаЗаЕдиницуХранения = Ложь) Экспорт
	
	Параметры = Новый Структура;
	Параметры.Вставить("Дата");
	Параметры.Вставить("Организация");
	Параметры.Вставить("СтавкаЗаЕдиницуХранения", СтавкаЗаЕдиницуХранения);
	...
	Если Не ЗначениеЗаполнено(Параметры.Дата) Тогда
		Параметры.Дата = ОбщегоНазначенияУТВызовСервера.ДатаСеанса();
	КонецЕсли;
...
Процедура ЗаполнитьРасхожденияПослеОтгрузки(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения = Неопределено) Экспорт
	...
	Если ТекущаяСтрока.КоличествоУпаковокРасхождения = 0 Тогда
		ТекущаяСтрока.ДействиеСТоваром = ПредопределенноеЗначение("Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.ПустаяСсылка");
	ИначеЕсли ТекущаяСтрока.КоличествоУпаковокРасхождения < 0
		И ТекущаяСтрока.ДействиеСТоваром = ПредопределенноеЗначение("Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.ОприходоватьВПуть") Тогда
		
		ТекущаяСтрока.ДействиеСТоваром = ПредопределенноеЗначение("Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.ПустаяСсылка");
		
	ИначеЕсли ТекущаяСтрока.КоличествоУпаковокРасхождения > 0
		И (ТекущаяСтрока.ДействиеСТоваром = ПредопределенноеЗначение("Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.Списать")
		ИЛИ ТекущаяСтрока.ДействиеСТоваром = ПредопределенноеЗначение("Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.СписатьИзПути")
		ИЛИ ТекущаяСтрока.ДействиеСТоваром = ПредопределенноеЗначение("Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.Вернуть")
		ИЛИ ТекущаяСтрока.ДействиеСТоваром = ПредопределенноеЗначение("Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.Ожидать")) Тогда
...
// Формирует названия идентификаторов суммы НДС с суффиксом
// 
// Параметры:
// 	Суффикс - Строка - суффикс
// 
...
// * СуммаНДС - Строка - "СуммаНДС" + суффикс
//
Функция СуммаНДСССуффиксом(Суффикс) Экспорт
	
	СтруктураВозврата = Новый Структура("Сумма, СуммаНДС");
	
	Если Не ЗначениеЗаполнено(Суффикс) Тогда
		СтруктураВозврата.Сумма = "СуммаБезНДСВал";
		СтруктураВозврата.СуммаНДС = "СуммаНДС";
	ИначеЕсли Суффикс = "Уменьшение" Тогда
		СтруктураВозврата.Сумма = "СуммаБезНДСВалУменьшение";
		СтруктураВозврата.СуммаНДС = "СуммаНДСУменьшение";
	ИначеЕсли Суффикс = "Увеличение" Тогда
		СтруктураВозврата.Сумма = "СуммаБезНДСВалУвеличение";
		СтруктураВозврата.СуммаНДС = "СуммаНДСУвеличение";
	Иначе
		ВызватьИсключение НСтр("ru = 'Неизвестный суффикс для суммы НДС:'") + " " + Суффикс;
	КонецЕсли;
...
//	КэшированныеЗначения - см. ПолучитьСтруктуруКэшируемыеЗначения.
//
Процедура УстановитьСуммуНДСРеглУпрВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения = Неопределено) Экспорт
	
	Перем ПараметрыДействия;
	
	Если СтруктураДействий.Свойство("УстановитьСуммуНДСРеглУпр", ПараметрыДействия) Тогда
		
		Если ПараметрыДействия.ВалютаДокумента = ПараметрыДействия.ВалютаРегл
			И ТекущаяСтрока.СуммаНДСРегл > 0 Тогда
			ТекущаяСтрока.СуммаНДСРегл = ТекущаяСтрока.СуммаНДС;
		Иначе
			ТекущаяСтрока.СуммаНДСРегл = 0;
		КонецЕсли;
		
		Если ПараметрыДействия.ВалютаДокумента = ПараметрыДействия.ВалютаУпр
			И ТекущаяСтрока.СуммаНДСУпр > 0 Тогда
			ТекущаяСтрока.СуммаНДСУпр = ТекущаяСтрока.СуммаНДС;
		Иначе
...
// Пересчитывает сумму НДС в текущей строке табличной части документа.
//
// Параметры:
//	ТекущаяСтрока - Структура - структура со свойствами строки документа.
//	СтруктураДействий - Структура - структура с действиями, которые нужно произвести.
//	КэшированныеЗначения - см. ПолучитьСтруктуруКэшируемыеЗначения.
//
Процедура ПересчитатьСуммуНДСОтгрузкиВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения = Неопределено) Экспорт
	
	Перем ПараметрыДействия;
	Перем СтавкаНДС;
	
	Если СтруктураДействий.Свойство("ПересчитатьСуммуНДСОтгрузки", ПараметрыДействия) Тогда
		
		НалогообложениеНДС = Неопределено;
		ПараметрыДействия.Свойство("НалогообложениеНДС", НалогообложениеНДС);
		
		Суффикс = Новый Массив;
		
		Если ПараметрыДействия.Свойство("Суффикс", Суффикс)
			И Не ЗначениеЗаполнено(Суффикс) Тогда
			Суффикс.Добавить("");
		КонецЕсли;
		
		Если Не (ПараметрыДействия <> Неопределено
				И ПараметрыДействия.Свойство("СтавкаНДС", СтавкаНДС)) Тогда
		...
		Для Каждого ТекСуффикс Из Суффикс Цикл 
			
			СуммаНДСССуффиксом = СуммаНДСССуффиксом(ТекСуффикс);
		
			ТекущаяСтрока[СуммаНДСССуффиксом.СуммаНДС] = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(
				ТекущаяСтрока[СуммаНДСССуффиксом.Сумма],
				ТекПроцентНДС,
				ПараметрыДействия.ЦенаВключаетНДС,
				НалогообложениеНДС);
			ТекущаяСтрока["Сумма"+ТекСуффикс] = ТекущаяСтрока[СуммаНДСССуффиксом.Сумма];
...
// Выполняет заполнение поля 'Помещение' в строке значением по умолчанию (например, значением из шапки).
//
// Параметры:
//  ТекущаяСтрока - ДанныеФормыСтруктура, Структура - строка, для которой требуется заполнить поле 'Помещение'.
//  СтруктураДействий - Структура - структура с действиями, которые нужно произвести.
//  КэшированныеЗначения - см. ПолучитьСтруктуруКэшируемыеЗначения
//
Процедура ЗаполнитьПомещениеВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения = Неопределено) Экспорт
	
	Если СтруктураДействий.Свойство("ЗаполнитьПомещениеВСтрокеТЧ") Тогда
		ТекущаяСтрока.Помещение = СтруктураДействий.ЗаполнитьПомещениеВСтрокеТЧ;
	КонецЕсли;
...
// Заполняет представление условий закупки в строке таблицы
//
// Параметры:
//	ТекущаяСтрока - Структура - структура со свойствами строки документа.
//	СтруктураДействий - Структура - структура с действиями, которые нужно произвести.
//	КэшированныеЗначения - см. ПолучитьСтруктуруКэшируемыеЗначения.
//
Процедура ЗаполнитьПредставлениеУсловийЗакупки(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	Если Не СтруктураДействий.Свойство("ЗаполнитьПредставлениеУсловийЗакупки") Тогда
		Возврат;
	КонецЕсли;
	
	ТекущаяСтрока.ПредставлениеУсловийЗакупки = ПредставлениеУсловийЗакупки(
		Строка(ТекущаяСтрока.Партнер), // Использование кэша представлений
		ТекущаяСтрока.Сумма,
		ТекущаяСтрока.СуммаСНДС);
	
КонецПроцедуры

// Формирует строковое представление условий закупки
// Параметры:
//  ПредставлениеПартнера - Строка - Поставщик, предоставляющий условия
//  Сумма - Число - Сумма без НДС
//  СуммаСНДС - Число - Сумма с НДС
// Возвращаемое значение:
//  Строка - Представление условий закупки
Функция ПредставлениеУсловийЗакупки(ПредставлениеПартнера, Сумма, СуммаСНДС) Экспорт

	УсловияЗакупокЗаполнены = Ложь;
	Если Не ЗначениеЗаполнено(ПредставлениеПартнера) Тогда
		Партнер = НСтр("ru = '<Не выбран>'");
	Иначе
		УсловияЗакупокЗаполнены = Истина;
		Партнер = ПредставлениеПартнера;
	КонецЕсли;
	...
	ТекстПредставления = "";
	Если УсловияЗакупокЗаполнены Тогда
		
		Если Сумма = СуммаСНДС Тогда
			ТекстПредставления = СтрШаблон(
				"%1 %2",
				Партнер,
				Формат(СуммаСНДС, "ЧДЦ=2"));
		Иначе
			ТекстПредставления = СтрШаблон(
				НСтр("ru = '%1 %2 (%3 с НДС)'"),
				Партнер,
...
// Заполняет признак "Списать на расходы" и при необходимости сбрасывает подразделение.
//
// Параметры:
//	ТекущаяСтрока - Структура - структура со свойствами строки документа.
//	СтруктураДействий - Структура - структура с действиями, которые нужно произвести.
//	КэшированныеЗначения - см. ПолучитьСтруктуруКэшируемыеЗначения.
//
Процедура ЗаполнитьПризнакСписатьНаРасходыИПодразделение(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения = Неопределено) Экспорт
	
	Перем ПараметрыДействия;
	
	Если НЕ СтруктураДействий.Свойство("ЗаполнитьПризнакСписатьНаРасходыИПодразделение", ПараметрыДействия) Тогда
		Возврат;
	КонецЕсли;
	
	ЭтоФактуровкаПоТоварам = Ложь;
	Если ТипЗнч(ПараметрыДействия) = Тип("Структура")
		И ПараметрыДействия.Свойство("ЭтоФактуровкаПоТоварам") Тогда
		ЭтоФактуровкаПоТоварам = ПараметрыДействия.ЭтоФактуровкаПоТоварам;
	КонецЕсли;
	
	ЭтоФактуровкаПоУслугамИРаботам = Ложь;
	Если ТипЗнч(ПараметрыДействия) = Тип("Структура")
		И ПараметрыДействия.Свойство("ЭтоФактуровкаПоУслугамИРаботам") Тогда
		ЭтоФактуровкаПоУслугамИРаботам = ПараметрыДействия.ЭтоФактуровкаПоУслугамИРаботам;
	КонецЕсли;
	...
	Если ТекущаяСтрока.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Товар")
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.Назначение) Или ЭтоФактуровкаПоТоварам Тогда
			ТекущаяСтрока.СписатьНаРасходы = Ложь; 
			ТекущаяСтрока.Подразделение = Неопределено;
		ИначеЕсли Не ТекущаяСтрока.СписатьНаРасходы Тогда
			ТекущаяСтрока.Подразделение = Неопределено;
		КонецЕсли;
		
	ИначеЕсли ТекущаяСтрока.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Работа") Тогда
		Если ЗначениеЗаполнено(ТекущаяСтрока.Назначение) Или ЭтоФактуровкаПоУслугамИРаботам Тогда
			ТекущаяСтрока.СписатьНаРасходы = Ложь; 
		КонецЕсли;
		Если ТекущаяСтрока.ТипНазначения = ПредопределенноеЗначение("Перечисление.ТипыНазначений.ПоставкаПодПринципала")
			Или ЭтоФактуровкаПоУслугамИРаботам Тогда
			ТекущаяСтрока.Подразделение = Неопределено;
		КонецЕсли;
		
	ИначеЕсли ТекущаяСтрока.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Услуга") Тогда
		Если ЭтоФактуровкаПоУслугамИРаботам Тогда
			ТекущаяСтрока.СписатьНаРасходы = Ложь;
			ТекущаяСтрока.Подразделение = Неопределено;
		Иначе
...
Функция ПоляСтрокиПоДействиям(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения, ДополнительныеПараметрыЗаполнения = Неопределено) Экспорт
	...
	Если СтруктураДействий.Свойство(ИмяДействия, СтруктураПараметровДействия) Тогда
		
		ДополнитьОписаниеПолей(ИмяДействия,
			Новый Структура(СтруктураПараметровДействия[ИмяДействия].ЧитаемыеПоля),
			Новый Структура(СтруктураПараметровДействия[ИмяДействия].ИзменяемыеПоля),
			КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ЗаполнитьПомещениеВСтрокеТЧ", СтруктураПараметровДействия) Тогда
		...
		ДополнитьОписаниеПолей("ЗаполнитьПомещениеВСтрокеТЧ",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ЗаполнитьСоглашениеПоставщикаПоУмолчанию", СтруктураПараметровДействия) Тогда
		
		ЧитаемыеПоля = Новый Структура();
		ЧитаемыеПоля.Вставить("Партнер");
		ЧитаемыеПоля.Вставить("Соглашение");
		
		ИзменяемыеПоля = Новый Структура();
		ИзменяемыеПоля.Вставить("Соглашение");
		
		ДополнитьОписаниеПолей("ЗаполнитьСоглашениеПоставщикаПоУмолчанию",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
		
	КонецЕсли;
		
	Если СтруктураДействий.Свойство("ЗаполнитьРеквизитыПоСоглашениюПоставщика", СтруктураПараметровДействия) Тогда
		
		ЧитаемыеПоля = Новый Структура();
		ЧитаемыеПоля.Вставить("Соглашение");
		
		ИзменяемыеПоля = Новый Структура;
		ИзменяемыеПоля.Вставить("ИспользуютсяДоговорыКонтрагентов");
		
		ДополнитьОписаниеПолей("ЗаполнитьРеквизитыПоСоглашениюПоставщика",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
		
	КонецЕсли;
	
	Если СтруктураДействий.Свойство("ЗаполнитьКоличествоСоглашенийПоставщика", СтруктураПараметровДействия) Тогда
		
		ЧитаемыеПоля = Новый Структура();
		ЧитаемыеПоля.Вставить("Партнер");
		
		ИзменяемыеПоля = Новый Структура();
		ИзменяемыеПоля.Вставить("КоличествоСоглашений");
		
		ДополнитьОписаниеПолей("ЗаполнитьКоличествоСоглашенийПоставщика",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
		
	КонецЕсли;
	
	Если СтруктураДействий.Свойство("ЗаполнитьКонтрагентаПартнераПоУмолчанию", СтруктураПараметровДействия) Тогда
		
		ЧитаемыеПоля = Новый Структура();
		ЧитаемыеПоля.Вставить("Партнер");
		ЧитаемыеПоля.Вставить("Контрагент");
		...
		ИзменяемыеПоля = Новый Структура();
		ИзменяемыеПоля.Вставить("Контрагент");
		
		ДополнитьОписаниеПолей("ЗаполнитьКонтрагентаПартнераПоУмолчанию",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
		
	КонецЕсли;
	
	Если СтруктураДействий.Свойство("ЗаполнитьДоговорПоУмолчанию", СтруктураПараметровДействия) Тогда
		
		ЧитаемыеПоля = Новый Структура();
		ЧитаемыеПоля.Вставить("Организация");
		ЧитаемыеПоля.Вставить("Партнер");
		ЧитаемыеПоля.Вставить("Контрагент");
		ЧитаемыеПоля.Вставить("Договор");
		...
		ДополнитьОписаниеПолей("ЗаполнитьДоговорПоУмолчанию",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ЗаполнитьОперациюПоСоглашениюДоговору") Тогда
		...
		ЧитаемыеПоля.Вставить("Соглашение");
		ЧитаемыеПоля.Вставить("Договор");
		ЧитаемыеПоля.Вставить("ХозяйственнаяОперация");
		
		ИзменяемыеПоля = Новый Структура;
		ИзменяемыеПоля.Вставить("ХозяйственнаяОперация");
		
		ДополнитьОписаниеПолей("ЗаполнитьОперациюПоСоглашениюДоговору",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
		
	КонецЕсли;
	
	Если СтруктураДействий.Свойство("ЗаполнитьНалогообложениеНДСЗакупки", СтруктураПараметровДействия) Тогда
		...
		ИзменяемыеПоля.Вставить("НалогообложениеНДС");
		
		ДополнитьОписаниеПолей("ЗаполнитьНалогообложениеНДСЗакупки",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ЗаполнитьСтавкуНДСПоВидуЦен", СтруктураПараметровДействия)
			Или СтруктураДействий.Свойство("ЗаполнитьСтавкуНДССуффиксПоВидуЦен", СтруктураПараметровДействия) Тогда
		
		ЧитаемыеПоля = Новый Структура();
		ЧитаемыеПоля.Вставить("Номенклатура");
		ЧитаемыеПоля.Вставить("Характеристика", ПредопределенноеЗначение("Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка"));
		ЧитаемыеПоля.Вставить("Серия", ПредопределенноеЗначение("Справочник.СерииНоменклатуры.ПустаяСсылка"));
		ЧитаемыеПоля.Вставить("Упаковка", ПредопределенноеЗначение("Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка"));
		ЧитаемыеПоля.Вставить("ВидЦены", ПредопределенноеЗначение("Справочник.ВидыЦен.ПустаяСсылка"));
		ЧитаемыеПоля.Вставить("ТипНоменклатуры", ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.ПустаяСсылка"));
		...
		ДополнитьОписаниеПолей("ЗаполнитьСтавкуНДСПоВидуЦен",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ЗаполнитьСтавкуНДСПоНастройкеУчетаПриУСНКомитента", СтруктураПараметровДействия) Тогда
		...
		ДополнитьОписаниеПолей("ЗаполнитьСтавкуНДСПоНастройкеУчетаПриУСНКомитента",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ПересчитатьСуммуСНДС") Тогда
		...
		ИзменяемыеПоля = Новый Структура();
		ИзменяемыеПоля.Вставить("СуммаСНДС", 0);
	...
	Если СтруктураДействий.Свойство("ПроверитьЗаполнитьСклад", СтруктураПараметровДействия) Тогда
		...
		Если ТипЗнч(СтруктураПараметровДействия) = Тип("Структура")
			И СтруктураПараметровДействия.Свойство("ИмяПоляСписатьНаРасходы")
			И ЗначениеЗаполнено(СтруктураПараметровДействия.ИмяПоляСписатьНаРасходы) Тогда
			ЧитаемыеПоля.Вставить(СтруктураПараметровДействия.ИмяПоляСписатьНаРасходы);
		КонецЕсли;
	...
	Если СтруктураДействий.Свойство("ЗаполнитьПризнакСписатьНаРасходыИПодразделение") Тогда
		
		ЧитаемыеПоля = Новый Структура();
		ЧитаемыеПоля.Вставить("ТипНоменклатуры");
		ЧитаемыеПоля.Вставить("Назначение");
		ЧитаемыеПоля.Вставить("ТипНазначения");
		
		ИзменяемыеПоля = Новый Структура();
		ИзменяемыеПоля.Вставить("СписатьНаРасходы");
		ИзменяемыеПоля.Вставить("Подразделение");
		
		ДополнитьОписаниеПолей("ЗаполнитьПризнакСписатьНаРасходыИПодразделение",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("УстановитьСуммуНДСРеглУпр") Тогда
		...
		ДополнитьОписаниеПолей("УстановитьСуммуНДСРеглУпр",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
		
	КонецЕсли;
		
	Если СтруктураДействий.Свойство("ПересчитатьСуммуНДСОтгрузки", СтруктураПараметровДействия) Тогда
		...
		Для Каждого ТекСуффикс Из Суффикс Цикл 
			СуммаНДСССуффиксом = СуммаНДСССуффиксом(ТекСуффикс);
			ЧитаемыеПоля.Вставить(СуммаНДСССуффиксом.Сумма);
			ИзменяемыеПоля.Вставить(СуммаНДСССуффиксом.СуммаНДС);
		КонецЦикла;
		
		ЧитаемыеПоля.Вставить("СтавкаНДС");
		
		ДополнитьОписаниеПолей("ПересчитатьСуммуНДСОтгрузки",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ЗаполнитьПризнакПодакцизныйТовар", СтруктураПараметровДействия)
		...
		ИзменяемыеПоля = Новый Структура();
		ИзменяемыеПоля.Вставить("ПодакцизныйТовар");
	...
	Если СтруктураДействий.Свойство("ЗаполнитьРеквизитыРасчетаАкциза", СтруктураПараметровДействия)
		И ЗначениеЗаполнено(СтруктураПараметровДействия) Тогда
		...
		ИзменяемыеПоля = Новый Структура();
		ИзменяемыеПоля.Вставить("ПодакцизныйТовар");
		ИзменяемыеПоля.Вставить("КатегорияПодакцизныхТоваров");
		ИзменяемыеПоля.Вставить("ТвердаяСтавкаАкциза");
		ИзменяемыеПоля.Вставить("ЕдиницаИзмеренияАкциза");
		ИзменяемыеПоля.Вставить("КоэффициентПересчета");
		ИзменяемыеПоля.Вставить("НалоговаяБазаАкцизы");
		ИзменяемыеПоля.Вставить("СуммаАкциза");
		ИзменяемыеПоля.Вставить("МаркируемаяПродукция");
		
		ДополнитьОписаниеПолей("ЗаполнитьРеквизитыРасчетаАкциза",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ПроверитьСериюПоВладельцу") Тогда
		...
		ИзменяемыеПоля = Новый Структура();
		ИзменяемыеПоля.Вставить("Серия");
		ИзменяемыеПоля.Вставить("СерииИспользуются");
		
		ДополнитьОписаниеПолей("ПроверитьСериюПоВладельцу",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
		
	КонецЕсли;
	
	Если СтруктураДействий.Свойство("ПроверитьРегистрациюШтрихкода") Тогда
		...
		ИзменяемыеПоля = Новый Структура();
		ИзменяемыеПоля.Вставить("Зарегистрирован");
		
		ДополнитьОписаниеПолей("ПроверитьРегистрациюШтрихкода",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ПересчитатьРасхожденияПослеОтгрузки") Тогда
		...
		ИзменяемыеПоля.Вставить("ДействиеСТоваром");
	...
	Если СтруктураДействий.Свойство("ПриИзмененииТипаНоменклатуры", СтруктураПараметровДействия) Тогда
		
		ЧитаемыеПоля = Новый Структура();
		ЧитаемыеПоля.Вставить("ТипНоменклатуры");
		ЧитаемыеПоля.Вставить("Отменено");
		Если ТипЗнч(СтруктураПараметровДействия) = Тип("Структура")
			И СтруктураПараметровДействия.Свойство("ЕстьСписатьНаРасходы") Тогда
			ЧитаемыеПоля.Вставить("СписатьНаРасходы");
		КонецЕсли;
	...
	Если СтруктураДействий.Свойство("ЗаполнитьСлужебныеРеквизитыПоНазначению", СтруктураПараметровДействия) Тогда
		...
		Если Не (ТипЗнч(СтруктураПараметровДействия) = Тип("Структура")
			...
			И СтруктураПараметровДействия.ТолькоТипНазначения = Истина) Тогда
				...
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущаяСтрока, "Партнер") Тогда
	...
	Если СтруктураДействий.Свойство("ЗаполнитьПодразделениеВСтрокеТЧ", СтруктураПараметровДействия) Тогда
	...
	Если СтруктураДействий.Свойство("ПересчитатьСуммуАкциза", СтруктураПараметровДействия) Тогда
		...
		ЧитаемыеПоля.Вставить("Номенклатура");
		Если ЗначениеЗаполнено(СтруктураПараметровДействия) Тогда
			ЧитаемыеПоля.Вставить(СтруктураПараметровДействия.ИмяПоляКоличество);
		КонецЕсли;
		
		ИзменяемыеПоля = Новый Структура();
		ИзменяемыеПоля.Вставить("СуммаАкциза");
		
		ДополнитьОписаниеПолей("ПересчитатьСуммуАкциза",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
		
	КонецЕсли;
		
	Если СтруктураДействий.Свойство("ЗаполнитьПредставлениеУсловийЗакупки", СтруктураПараметровДействия) Тогда
		...
		ИзменяемыеПоля = Новый Структура();
		ИзменяемыеПоля.Вставить("ПредставлениеУсловийЗакупки");
		
		ДополнитьОписаниеПолей("ЗаполнитьПредставлениеУсловийЗакупки",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
...
Функция ИменаПолейПоСтатусуУказанияСерий(ИмяПоляСтатус)
	...
	Если ИмяПоляСтатус = "СтатусУказанияСерий"
		Или ИмяПоляСтатус = "СтатусУказанияСерийНедостачи" Тогда
