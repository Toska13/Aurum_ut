...
// Обработать получение данных с электронных весов.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
//  ОповещениеОЗавершении - ОписаниеОповещения - оповещение обработки штрихкода
//  ДанныеОборудования - Структура, Неопределено - данные подключаемого оборудования
//  КодМаркировкиДляУточнения - Структура - служебный реквизит формы данных уточнения
Процедура ОбработатьПолучениеДанныхСЭлектронныхВесов(Форма, ОповещениеОЗавершении, ДанныеОборудования, КодМаркировкиДляУточнения) Экспорт
	
	Если ДанныеОборудования = Неопределено
		Или Не ДанныеОборудования.Результат Тогда
		Подключаемый_ОткрытьФормуУточненияДанных(ЭтотОбъект, ОповещениеОЗавершении);
		Возврат;
	КонецЕсли;
	
	КодМаркировкиДляУточнения.РезультатОбработкиШтрихкода.ДанныеШтрихкода.Количество = ДанныеОборудования.Вес;
	...
	ВыполнитьОбработкуОповещения(ОповещениеОЗавершении, ДанныеШтрихкода);
...
Процедура УточнитьДанныеУПользователя(ФормаВладелец, ПараметрыОткрытияФормы, ОповещениеПоЗавершениюУточненияДанных) Экспорт
	...
	ИначеЕсли ПараметрыОткрытияФормы.Операция = "ПодключениеКегаКОборудованиюРозлива" Тогда
		...
		ПараметрыОткрытия.Склад               = ПараметрыСканирования.Склад;
...
Процедура УточненияДанныхЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ЭтоПродукцияИСМП               = ПараметрыСканирования.ИСМП
		И КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(ВидПродукции, Истина);
	
	ТребуетсяПроверкаРазрешительногоРежимаНаЛокальномМодулеЧЗ = Ложь;
	ПроведенаПроверкаНаЛокальномМодулеЧЗ                      = Ложь;
	РазрешенаПроверкаНаККТ                                    = ЭтоПродукцияИСМП;
	...
	Если ЭтоПродукцияИСМП  Тогда
		...
		КонецЕсли;
		Если ТребуетсяПроверкаРазрешительногоРежимаНаТСПИоТ
	...
	Если ЭтоПродукцияИСМП
		И ПараметрыСканирования.Свойство("ККТФФД12ИСМП")
		И ПараметрыСканирования.ТребуетсяПроверкаСредствамиККТ
		И Не РезультатОбработкиШтрихкода.ПроверкаСредствамиККТЗавершена
		И РазрешенаПроверкаНаККТ Тогда
...
Функция НачатьПолучениеВесаАвтоматически(Форма) Экспорт
	...
	Если Форма.КодМаркировкиДляУточнения <> Неопределено
		И КонструкторИСМПКлиентСервер.ВидПродукцииПоддерживаетПеременноеКоличествоМернойПродукции(Форма.КодМаркировкиДляУточнения.РезультатОбработкиШтрихкода.ДанныеШтрихкода.ВидПродукции)
...
Процедура ОткрытьФормуНевозможностиДобавленияОтсканированного(Форма, ПараметрыОткрытияФормы, ОповещениеОЗакрытии = Неопределено) Экспорт
	...
	ОткрытьФорму("Обработка.ПроверкаИПодборПродукцииИС.Форма.ИнформацияОНевозможностиДобавленияОтсканированного", 
...
Функция ИнициализацияСтруктурыДанныхСохраненногоВыбора() Экспорт
	...
	// Специфика для ЕГАИС
	ДанныеВыбора.Вставить("АлкогольнаяПродукция",            Неопределено);
	ДанныеВыбора.Вставить("Справка2",                        Неопределено);
...
Функция ПроверитьФорматШтрихкода(Штрихкод, ФорматBase64) Экспорт
	...
	Если РезультатРаспаковки <> Штрихкод Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			ВидыПродукции,
			КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииИСМП(Истина), Истина);
...
Функция ПреобразоватьВПараметрыОткрытияФормыУточнения(КодМаркировкиДляУточнения)
	...
	Если ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоСтрокаДанныхЧастичногоВыбытия(
		ДанныеШтрихкода, ДанныеДляУточнения.ПараметрыСканирования) Тогда
...
// Сценарий: Неуникальные штрихкоды + варианты уточнения данных по неуникальному штрихкоду
// из формы документа (или формы проверки и подбора)
Процедура ДополнитьПараметрыУточненияИзФормыИсточникаПоСпискуВыбора(ФормаВладелец, ПараметрыОткрытияФормы, СписокВыбора)
	
	// * Только если список неуникальных штрихкодов заполнен
	Если Не ПараметрыОткрытияФормы.РежимПодбораИзДокумента Тогда
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПараметрыОткрытияФормы.ЧастичноеВыбытиеВариантУчета) Тогда
		Возврат;
	КонецЕсли;
	
	ФормаИсточник = ФормаВладелец;
	Если ФормаИсточник.ИмяФормы = "Обработка.ПроверкаИПодборПродукцииИС.Форма.ФормаВводаКодаМаркировки" Тогда
	...
	РаботаСПустойНоменклатурой = ПараметрыСканирования.РазрешенаОбработкаКодовСПустойНоменклатурой;
	
	// Обработка кодов маркировки со сроками годности и идентифиакторами ВетИС
	ЕстьВЕТИС = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗаполнятьДанныеВЕТИС(ПараметрыСканирования);
	ЕстьСрокГодности = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗаполнятьСрокГодности(ПараметрыСканирования);

	// Обработка кодов маркировки ЕГАИС
	ЕстьАлкогольнаяПродукция = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗапрашиватьАлкогольнуюПродукцию(ПараметрыСканирования);
	ЕстьСправка2             = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗапрашиватьСправку2(ПараметрыСканирования);
	
	Смещение = 0;
	ВыборОбновлен = Ложь;
	
	// Форма проверки и подбора
	Если ФормаИсточник.ИмяФормы = "Обработка.ПроверкаИПодборПродукцииИС.Форма.ПроверкаИПодбор" Тогда
		...
		Для Каждого ЭлементСписка Из СписокВыбора Цикл
			...
			ДополнитьПоляПоиска(КолонкиТовары, ПоляПоиска, "Номенклатура", ЭлементСписка.Номенклатура);
			ДополнитьПоляПоиска(КолонкиТовары, ПоляПоиска, "Характеристика", ЭлементСписка.Характеристика);
		
			Если ФормаИсточник.Элементы.ПодобраннаяМаркируемаяПродукцияГруппаСерия.Видимость // Есть серии
				И Не ЗначениеЗаполнено(ЭлементСписка.Серия) Тогда
				ПоляДокумента.Вставить("Серия");
			Иначе
				ДополнитьПоляПоиска(КолонкиТовары, ПоляПоиска, "Серия", ЭлементСписка.Серия);
			КонецЕсли;
			...
			Если ЕстьАлкогольнаяПродукция Тогда
				ПоляДокумента.Вставить("АлкогольнаяПродукция");
			КонецЕсли;
			
			Если ЕстьСправка2 Тогда
				ПоляДокумента.Вставить("Справка2");
			КонецЕсли;
			...
			Если НеЗавершенПодбор.Количество() И ПоляДокумента.Количество() Тогда
				ОбновитьСтроку = Истина;
				Для Каждого СтрокаПодбор Из НеЗавершенПодбор Цикл
					ЭтоСтрокаУточнения = (Не ЕстьВЕТИС Или ЗначениеЗаполнено(СтрокаПодбор.ИдентификаторПроисхожденияВЕТИС))
						И (Не ЕстьСрокГодности Или ЗначениеЗаполнено(СтрокаПодбор.ГоденДо))
						И (Не ЕстьАлкогольнаяПродукция Или ЗначениеЗаполнено(СтрокаПодбор.АлкогольнаяПродукция))
						И (Не ЕстьСправка2 Или ЗначениеЗаполнено(СтрокаПодбор.Справка2));
					ДобавитьСтрокуВПодбор = (ЭтоСтрокаУточнения Или СтрокаПодбор.Серия <> ЭлементСписка.Серия)
						И СтрокаПодбор.КоличествоПодобрано < СтрокаПодбор.Количество;
					Если ДобавитьСтрокуВПодбор Тогда
						Если ОбновитьСтроку Тогда
							НовыйЭлементВыбора = ПараметрыОткрытияФормы.ДанныеДокумента[Смещение];
						Иначе
							НовыйЭлементВыбора = ОбщегоНазначенияКлиент.СкопироватьРекурсивно(ЭлементСписка);
						КонецЕсли;
						ДанныеУточнения = ОбщегоНазначенияКлиент.СкопироватьРекурсивно(ПоляДокумента);
						Если Не ВыборОбновлен Тогда
							ВыборОбновлен = Истина;
							ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ПараметрыОткрытияФормы.ДанныеДокумента[0], ДанныеУточнения, Ложь);
						КонецЕсли;	
						ЗаполнитьЗначенияСвойств(ДанныеУточнения, СтрокаПодбор);
						ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(НовыйЭлементВыбора, ДанныеУточнения, Истина);
...
Процедура ДополнитьПараметрыУточненияИзФормыИсточника(ФормаВладелец, ПараметрыОткрытияФормы, УточнитьУпаковку = Ложь) Экспорт
	...
	Если ФормаИсточник.ИмяФормы = "Обработка.ПроверкаИПодборПродукцииИС.Форма.ПроверкаИПодбор" Тогда
		...
		Если ЕстьАлкогольнаяПродукция Тогда
			Если УточнитьУпаковку Или Не ЗначениеЗаполнено(ПараметрыОткрытияФормы.АлкогольнаяПродукция) Тогда
				ПоляДокумента.Вставить("АлкогольнаяПродукция");
			Иначе
				ДополнитьПоляПоиска(КолонкиТовары, ПоляПоиска, "АлкогольнаяПродукция", ПараметрыОткрытияФормы.АлкогольнаяПродукция);
			КонецЕсли;
		КонецЕсли;
		Если ЕстьСправка2 Тогда
			Если УточнитьУпаковку Или Не ЗначениеЗаполнено(ПараметрыОткрытияФормы.Справка2) Тогда
				ПоляДокумента.Вставить("Справка2");
			Иначе
				ДополнитьПоляПоиска(КолонкиТовары, ПоляПоиска, "Справка2", ПараметрыОткрытияФормы.Справка2);
			КонецЕсли;
		КонецЕсли;
...
Процедура ИнформироватьВРасширеннойВерсииОНевозможностиДобавления(Форма, РезультатОбработкиШтрихкода, ПараметрыСканирования, ОбщаяОшибка = Ложь) Экспорт
	...
	ОткрытьФорму("Обработка.ПроверкаИПодборПродукцииИС.Форма.ИнформацияОНевозможностиДобавленияОтсканированного", ПараметрыОткрытия, Форма);
