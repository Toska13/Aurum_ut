...
// Рассчитывает признак предмета расчета фискальной операции.
// 
// Параметры:
//  ТипНоменклатуры - ПеречислениеСсылка.ТипыНоменклатуры - 
//  ПодакцизныйТовар - Булево - 
//  УказанаКМ - Булево - Признак указания контрольной марки в строке чека
//  СтрокаПозицииЧека - Структура - :
// * ОсобенностьУчета - ПеречислениеСсылка.ОсобенностиУчетаНоменклатуры - .
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.ПризнакиПредметаРасчета, Неопределено - Признак предмета расчета фискальной операции
Функция ПризнакПредметаРасчетаФискальнойОперации(ТипНоменклатуры, ПодакцизныйТовар, УказанаКМ = Истина,
	СтрокаПозицииЧека = Неопределено) Экспорт
	...
	Если ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Товар")
		...
		Если ПодакцизныйТовар Тогда
			...
			//++ Локализация
			ИначеЕсли КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(СтрокаПозицииЧека.ВидПродукцииИС) Тогда
				
				Возврат ПредопределенноеЗначение("Перечисление.ПризнакиПредметаРасчета.ПодакцизныйТовар");
				
			ИначеЕсли КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(СтрокаПозицииЧека.ВидПродукцииИС, Истина) Тогда
		...
		Иначе
			...
			//++ Локализация
			ИначеЕсли КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(СтрокаПозицииЧека.ВидПродукцииИС, Истина)
				И Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(СтрокаПозицииЧека.ВидПродукцииИС) Тогда
				Возврат ПредопределенноеЗначение("Перечисление.ПризнакиПредметаРасчета.ТоварМаркируемыйСИИмеющийКМ");
			//-- Локализация
...
Процедура ОбновитьДанныеИнформационнойПанели(Форма, Данные) Экспорт
	...
	Если СуммовыеПараметры.Сумма <> 0 Тогда
		...
		Если СуммовыеПараметры.Свойство("СуммаРучнойСкидки") Тогда
			Если  СуммовыеПараметры.Свойство("ПроцентРучнойСкидки") Тогда
				ФорматПроцента = ФорматнаяСтрока(СуммовыеПараметры.ПроцентРучнойСкидки, 2);
			Иначе
				ФорматПроцента = Формат;
			КонецЕсли;
			Если СуммовыеПараметры.СуммаРучнойСкидки > 0 Тогда
				...
				РучнаяСкидка = Новый ФорматированнаяСтрока(
					РучнаяСкидка,
					Новый ФорматированнаяСтрока(" ( " + Формат(СуммовыеПараметры.ПроцентРучнойСкидки, ФорматПроцента) + "%" + " )",,,,"РучнаяСкидка"));
			КонецЕсли;
			Если СуммовыеПараметры.СуммаРучнойСкидки < 0 Тогда
				РучнаяСкидка = Новый ФорматированнаяСтрока(
					РучнаяСкидка,
		...
		Если СуммовыеПараметры.Свойство("СуммаАвтоматическойСкидки") Тогда
			Если  СуммовыеПараметры.Свойство("ПроцентАвтоматическойСкидки") Тогда
				ФорматПроцента = ФорматнаяСтрока(СуммовыеПараметры.ПроцентАвтоматическойСкидки, 2);
			Иначе
				ФорматПроцента = Формат;
			КонецЕсли;
			Если СуммовыеПараметры.СуммаАвтоматическойСкидки > 0 Тогда
				...
				АвтоматическаяСкидка = Новый ФорматированнаяСтрока(
					АвтоматическаяСкидка,
					Новый ФорматированнаяСтрока(" ( " + Формат(СуммовыеПараметры.ПроцентАвтоматическойСкидки,  ФорматПроцента) + "%" + " )",,,,"АвтоматическаяСкидка"));
			КонецЕсли;
			Если СуммовыеПараметры.СуммаАвтоматическойСкидки < 0 Тогда
				АвтоматическаяСкидка = Новый ФорматированнаяСтрока(
					АвтоматическаяСкидка,
...
Процедура УстановитьАктивностьЭлементов(Форма) Экспорт
	...
	//++ Локализация
	Если Форма.ИмяФормы = "Документ.ЧекККМ.Форма.ФормаОплатыЭСФСС" Тогда
		
		ПолеФормыНомерЭлектронногоСертификатаФСС = Форма.Элементы.НомерЭлектронногоСертификатаФСС; // ПолеФормы
		Если Форма.ТекущийЭлемент = ПолеФормыНомерЭлектронногоСертификатаФСС
			И Форма.АктивноеПоле <> ПолеФормыНомерЭлектронногоСертификатаФСС.Имя Тогда
			
			РозничныеПродажиКлиентСервер.УстановитьАктивныйЭлемент(Форма, ПолеФормыНомерЭлектронногоСертификатаФСС);
...
Процедура УстановитьАктивныйЭлемент(Форма, Элемент) Экспорт
	...
	//++ Локализация
	Если Форма.ИмяФормы = "Документ.ЧекККМ.Форма.ФормаОплатыЭСФСС" Тогда
		МассивЭлементов.Добавить(Форма.Элементы.НомерЭлектронногоСертификатаФСС);
	КонецЕсли;
	//-- Локализация
...
Процедура СохранитьВКэшДанныеДинамическихРеквизитовТаблицФормы(Форма) Экспорт
	...
	РозничныеПродажиКлиентСерверЛокализация.СохранитьВКэшДанныеДинамическихРеквизитовТаблицФормы(Форма);
...
Процедура ВосстановитьИзКэшаДанныеДинамическихРеквизитовТаблицФормы(Форма) Экспорт
	...
	РозничныеПродажиКлиентСерверЛокализация.ВосстановитьИзКэшаДанныеДинамическихРеквизитовТаблицФормы(Форма);
...
Функция БезОсобенностейУчета(СтрокаПозицииЧека)
	...
	Если ЗначениеЗаполнено(СтрокаПозицииЧека)
		И НЕ ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаПозицииЧека, "ВидПродукцииИС") Тогда
	...
	//++ Локализация
	Если ЗначениеЗаполнено(СтрокаПозицииЧека)
		И ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаПозицииЧека, "ВидПродукцииИС")
		И Не (КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(СтрокаПозицииЧека.ВидПродукцииИС, Истина)
			Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияЕГАИС(СтрокаПозицииЧека.ВидПродукцииИС)) Тогда
...
// Возвращает массив имен форм документов, которые поддерживают функционал рабочего места кассира.
//
// Возвращаемое значение:
//  Массив из Строка -
Функция ИменаФормРМК() Экспорт

	ИменаФормРМК = Новый Массив;
	ИменаФормРМК.Добавить("Документ.ЧекККМ.Форма.ФормаДокументаРМК");
	ИменаФормРМК.Добавить("Документ.ЧекККМВозврат.Форма.ФормаДокументаРМК");
	ИменаФормРМК.Добавить("Документ.РеализацияПодарочныхСертификатов.Форма.ФормаДокументаРМК");
	ИменаФормРМК.Добавить("Документ.ВозвратПодарочныхСертификатов.Форма.ФормаДокументаРМК");
	
	Возврат ИменаФормРМК;
КонецФункции
