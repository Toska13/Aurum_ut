...
// Возвращает истина если переданный QR код это цифровой ID
//
// Параметры:
//  QRКод - Строка
//
// Возвращаемое значение:
//  Булево
Функция ЭтоЦифровойIDМакс(Знач QRКод) Экспорт
	
	Если ЭтоСтрокаBase64(QRКод) Тогда
		QRКод = ПолучитьСтрокуИзДвоичныхДанных(Base64Значение(QRКод));
	КонецЕсли;
	
	НастройкиСоединенияпроверкиВозраста = МенеджерОборудованияКлиентСервер.НастройкиСоединенияпроверкиВозраста();
	МаскаЦифровогоID = НастройкиСоединенияпроверкиВозраста.МаскаЦифровогоID;
	Возврат СтрНачинаетсяС(QRКод, МаскаЦифровогоID);
...
Функция НастройкиСоединенияпроверкиВозраста() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("Хост", "ext-api.max.ru");
	Результат.Вставить("Порт", 443);
	Результат.Вставить("Таймаут", 5);
	Результат.Вставить("АдресРесурса", "/esia/age-verification?session_id=%1");
	Результат.Вставить("МаскаЦифровогоID", "https://www.gosuslugi.ru/m");
	Результат.Вставить("СсылкаПолученияЦифровогоID", "https://trk.mail.ru/c/f01sg8?mt_tmpl=v1");

	Возврат Результат;
	
КонецФункции

Функция НастройкиСоединенияПроверкиСтатусаMAX() Экспорт
	...
	Результат.Вставить("Таймаут", 5);
	Результат.Вставить("АдресРесурса", "/esia/age-verification?session_id=%1");
	Результат.Вставить("АдресРесурсаРасширенный", "v2/business/pos/age-verification?session_id=%1&verification_details=true");
	Результат.Вставить("МаскаЦифровогоID", "https://www.gosuslugi.ru/m1/");
	Результат.Вставить("СсылкаПолученияЦифровогоID", "https://trk.mail.ru/c/f01sg8?mt_tmpl=v1");
...
Функция ЭтоСтрокаBase64(Строка) Экспорт
	...
	Если ДлинаСтроки < 16 Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ДлинаСтроки % 4 <> 0 Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция РезультатВызоваМетодаHTTP() Экспорт
	...
	Результат.Вставить("ВызовНаСервере", Неопределено);
