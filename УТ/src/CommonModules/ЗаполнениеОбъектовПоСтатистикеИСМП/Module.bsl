...
Функция ДанныеЗаполненияЗаказаНаЭмиссиюКодовМаркировкиСУЗ(Организация) Экспорт
	...
	ДанныеЗаполнения.Вставить("ТоварнаяГруппа", Справочники.ТоварныеГруппыГИСМТ.ПустаяСсылка());
	...
	Объект.Вставить("Ссылка", Документы.ЗаказНаЭмиссиюКодовМаркировкиСУЗ.ПустаяСсылка());
	ДоступныеТоварныеГруппы = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступныеТоварныеГруппы(Объект);
	
	Если ДоступныеТоварныеГруппы.Количество() = 0 Тогда
	...
	|	ЗаказНаЭмиссиюКодовМаркировкиСУЗ.ТоварнаяГруппа                      КАК ТоварнаяГруппа,
	...
	ДанныеЗаполнения.ТоварнаяГруппа = ТоварнаяГруппа(ДанныеПоследнихДокументов, ДоступныеТоварныеГруппы);
	
	ВидыПродукцииПоТоварнойГруппе = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ДанныеЗаполнения.ТоварнаяГруппа);
	
	ЗаполнитьПоСтатистике(
		"Организация, ПроизводственныйОбъект",
		ДанныеЗаполнения, ДанныеПоследнихДокументов);
	
	ПоляЗаполнения = "СпособВводаВОборот, ПроизводственныйОбъектАдрес, ПроизводственныйОбъектАдресСтрокой";
	
	Если МаркировкаТоваровКлиентСервер.ДоступноПолеИдентификаторПроизводственнойЛинии(ДанныеЗаполнения.ТоварнаяГруппа) Тогда
		ПоляЗаполнения = ПоляЗаполнения + ", ИдентификаторПроизводственнойЛинии";
	КонецЕсли;
	
	Если МаркировкаТоваровКлиентСервер.ДоступноПолеПроизводственныйОбъект(ДанныеЗаполнения.ТоварнаяГруппа) Тогда
		ПоляЗаполнения = ПоляЗаполнения + ", ПроизводственныйОбъектИдентификатор";
	КонецЕсли;
	
	Если ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеСервисПровайдер(
		ВидыПродукцииПоТоварнойГруппе, ДанныеЗаполнения.СпособВводаВОборот) Тогда
		ПоляЗаполнения = ПоляЗаполнения + ", СервисПровайдер";
	КонецЕсли;
	
	Если ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеТипОплатыКодовМаркировки(ВидыПродукцииПоТоварнойГруппе) Тогда
		ПоляЗаполнения = ПоляЗаполнения + ", ТипОплатыКодовМаркировки";
	КонецЕсли;
	
	Если МаркировкаТоваровКлиентСервер.ДоступноПолеGLNОрганизации(ДанныеЗаполнения.ТоварнаяГруппа) Тогда
		ПоляЗаполнения = ПоляЗаполнения + ", GLNОрганизации, НомерДоговораСОператором, ДатаДоговораСОператором, СпособПолучения";
	КонецЕсли;
			
	ЗаполнитьПоСтатистике(ПоляЗаполнения,
			ДанныеЗаполнения, ДанныеПоследнихДокументов,
			"ТоварнаяГруппа, Организация");
		
	// Значения по-умолчанию
	Если Не ЗначениеЗаполнено(ДанныеЗаполнения.СпособВводаВОборот) Тогда
		ДоступныеСпособыВводаВОборот = ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступныеСпособыВводаВОборот(ВидыПродукцииПоТоварнойГруппе);
		Если ДоступныеСпособыВводаВОборот.Количество() > 0 Тогда
			ДанныеЗаполнения.СпособВводаВОборот = ДоступныеСпособыВводаВОборот[0];
		КонецЕсли;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ДанныеЗаполнения.ТипОплатыКодовМаркировки)
		И СтрНайти(ПоляЗаполнения, "ТипОплатыКодовМаркировки") > 0 Тогда
...
Функция ДанныеЗаполненияМаркировкиТоваровИСМП(Организация, Операция, ТоварнаяГруппа) Экспорт
	...
	ДанныеЗаполнения.Вставить("ОперацияНанесения");
	...
	Объект = Новый Структура;
	Объект.Вставить("Ссылка", Документы.МаркировкаТоваровИСМП.ПустаяСсылка());
	Если ЗначениеЗаполнено(Операция) Тогда
		Объект.Вставить("Операция", Операция);
	КонецЕсли;
	ДоступныеТоварныеГруппы = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступныеТоварныеГруппы(Объект);
	...
	|	МаркировкаТоваровИСМП.ОперацияНанесения      КАК ОперацияНанесения,
	|	МаркировкаТоваровИСМП.ТоварнаяГруппа         КАК ТоварнаяГруппа,
	...
	|	И (МаркировкаТоваровИСМП.Операция = &Операция Или &БезУчетаОперации)
	|	И (МаркировкаТоваровИСМП.ТоварнаяГруппа = &ТоварнаяГруппа Или &БезУчетаТоварнойГруппы)
	...
	Запрос.УстановитьПараметр("БезУчетаОперации",       Не ЗначениеЗаполнено(Операция));
	Запрос.УстановитьПараметр("ТоварнаяГруппа",         ТоварнаяГруппа);
	Запрос.УстановитьПараметр("БезУчетаТоварнойГруппы", Не ЗначениеЗаполнено(ТоварнаяГруппа));
	...
	Если МаркировкаТоваровКлиентСервер.ДоступноПолеПринятоеРешениеКодТаможенногоОргана(ДанныеЗаполнения.ТоварнаяГруппа) Тогда
	...
	ДоступныОперацииНанесения = МаркировкаТоваровКлиентСервер.ДоступенОтчетОНанесении(ДанныеЗаполнения.ТоварнаяГруппа);
	
	// Значения по-умолчанию
	Если Не ЗначениеЗаполнено(ДанныеЗаполнения.Операция) Тогда
		Если МаркировкаТоваровКлиентСервер.ДоступнаОперацияВводВОборотПроизводствоРФ(ДанныеЗаполнения.ТоварнаяГруппа) Тогда
			ДанныеЗаполнения.Операция = Перечисления.ВидыОперацийИСМП.ВводВОборотПроизводствоРФ;
		ИначеЕсли ДоступныОперацииНанесения Тогда
			ДанныеЗаполнения.Операция = Перечисления.ВидыОперацийИСМП.ОтчетОВерификацииНанесенныхКМ;
		КонецЕсли;
	КонецЕсли;
	
	Если ДоступныОперацииНанесения
		И ИнтеграцияИСМПКлиентСервер.ОперацииНанесенияКодовМаркировки().Найти(ДанныеЗаполнения.Операция) = Неопределено Тогда
		ЗаполнитьПоСтатистике(
			"ОперацияНанесения",
			ДанныеЗаполнения, ДанныеПоследнихДокументов,
			"ТоварнаяГруппа, Организация, Операция");
	КонецЕсли;
...
Функция ДанныеЗаполненияСписанияКодовМаркировкиИСМП(Организация) Экспорт
	...
	Объект = Новый Структура;
	Объект.Вставить("Ссылка", Документы.СписаниеКодовМаркировкиИСМП.ПустаяСсылка());
	ДоступныеТоварныеГруппы = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступныеТоварныеГруппы(Объект);
	...
	|	СписаниеКодовМаркировкиИСМП.ТоварнаяГруппа КАК ТоварнаяГруппа,
	|	СписаниеКодовМаркировкиИСМП.Операция КАК Операция,
	...
	|ГДЕ
	|	СписаниеКодовМаркировкиИСМП.Проведен
	|	И (СписаниеКодовМаркировкиИСМП.Организация = &Организация
	...
	ЗаполнитьПоСтатистике(
		"Организация, Операция, ПричинаСписания",
		ДанныеЗаполнения, ДанныеПоследнихДокументов,
		"ТоварнаяГруппа");
	
	Если МаркировкаТоваровКлиентСервер.ДоступноПолеПроизводственныйОбъект(ДанныеЗаполнения.ТоварнаяГруппа,
		ДанныеЗаполнения.ПричинаСписания) Тогда
	...
	Если МаркировкаТоваровКлиентСервер.ПроверятьЗаполнениеПоляGLNОрганизации(ДанныеЗаполнения.ТоварнаяГруппа, ДанныеЗаполнения.Операция) Тогда
	...
	Если ДанныеЗаполнения.Операция = Перечисления.ВидыОперацийИСМП.УдалитьСписаниеКодовВыбывшихДоОбязательностиМаркировки
		Или ДанныеЗаполнения.Операция = Перечисления.ВидыОперацийИСМП.СписаниеЭмитированныхКодовМаркировки
		Или ДанныеЗаполнения.Операция = Перечисления.ВидыОперацийИСМП.УдалитьСписаниеЭмитированныхКодовМаркировкиПриПоступлении Тогда
		ДанныеЗаполнения.Операция = Перечисления.ВидыОперацийИСМП.ПустаяСсылка();
	КонецЕсли;
...
Функция ДанныеЗаполненияВыводаИзОборотаИСМП(Организация, ТоварнаяГруппа) Экспорт
	
	ДанныеЗаполнения = Новый Структура;
	ДанныеЗаполнения.Вставить("Организация");
	ДанныеЗаполнения.Вставить("Причина");
	...
	ДанныеЗаполнения.Вставить("ТоварнаяГруппа", ТоварнаяГруппа);
	...
	|	ВыводИзОборотаИСМП.ТоварнаяГруппа                  КАК ТоварнаяГруппа,
	|	ВыводИзОборотаИСМП.Причина                         КАК Причина,
	...
	|	И (ВыводИзОборотаИСМП.ТоварнаяГруппа = &ТоварнаяГруппа Или &БезУчетаТоварнойГруппы)
	...
	Если ЗначениеЗаполнено(ТоварнаяГруппа) Тогда
		Если КонструкторИСМПКлиентСервер.ЭтоСырьеГИСМТ(ТоварнаяГруппа) Тогда
			Поля = "Причина, Контрагент, СтранаНазначения";
			ЗаполнитьПоСтатистике(Поля, ДанныеЗаполнения, ДанныеПоследнихДокументов);
			ДанныеЗаполнения.ОбъемноСортовойУчет = Истина;
		Иначе
			Поля = "Причина, Контрагент, СтранаНазначения, ОбъемноСортовойУчет";
			ЗаполнитьПоСтатистике(Поля, ДанныеЗаполнения, ДанныеПоследнихДокументов);
		КонецЕсли;
	Иначе
		Объект = Новый Структура;
		Объект.Вставить("Ссылка", Документы.ВыводИзОборотаИСМП.ПустаяСсылка());
		ДоступныеТоварныеГруппы = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступныеТоварныеГруппы(Объект);
		Если ДоступныеТоварныеГруппы.Количество() > 0 Тогда
			ДанныеЗаполнения.ТоварнаяГруппа = ТоварнаяГруппа(ДанныеПоследнихДокументов, ДоступныеТоварныеГруппы);
			ЗаполнитьПоСтатистике(
				"Причина, Контрагент, СтранаНазначения",
				ДанныеЗаполнения, ДанныеПоследнихДокументов);
			Если КонструкторИСМПКлиентСервер.ЭтоСырьеГИСМТ(ДанныеЗаполнения.ТоварнаяГруппа) Тогда
				ДанныеЗаполнения.Вставить("ОбъемноСортовойУчет", Истина);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ДанныеЗаполненияОтгрузкиТоваровИСМП(Организация, Контрагент = Неопределено) Экспорт
	...
	Объект = Новый Структура;
	Объект.Вставить("Ссылка", Документы.ОтгрузкаТоваровИСМП.ПустаяСсылка());
	ДоступныеТоварныеГруппы = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступныеТоварныеГруппы(Объект);
	...
	|	ОтгрузкаТоваровИСМП.ТоварнаяГруппа                  КАК ТоварнаяГруппа,
	...
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|	И ОтгрузкаТоваровИСМП.ТоварнаяГруппа В (&ДоступныеТоварныеГруппы)
	|УПОРЯДОЧИТЬ ПО
	...
	Запрос.УстановитьПараметр("ДоступныеТоварныеГруппы", ДоступныеТоварныеГруппы);
	...
	ЗаполнитьПоСтатистике(
		"СтранаНазначения",
		ДанныеЗаполнения, ДанныеПоследнихДокументов, "ТоварнаяГруппа,Операция");
...
Функция ДанныеЗаполненияВозвратаВОборотИСМП(Организация) Экспорт
	...
	ДанныеЗаполнения.Вставить("Организация");
	ДанныеЗаполнения.Вставить("ТоварнаяГруппа", Справочники.ТоварныеГруппыГИСМТ.ПустаяСсылка());
	ДанныеЗаполнения.Вставить("ВидВозврата");
	
	Объект = Новый Структура;
	Объект.Вставить("Ссылка", Документы.ВозвратВОборотИСМП.ПустаяСсылка());
	ДоступныеТоварныеГруппы = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступныеТоварныеГруппы(Объект);
	...
	|	ВозвратВОборотИСМП.ТоварнаяГруппа КАК ТоварнаяГруппа,
	|	ВозвратВОборотИСМП.ВидВозврата    КАК ВидВозврата
	|ИЗ
...
Функция ДанныеЗаполненияПеремаркировкиТоваровИСМП(Организация) Экспорт
	...
	Объект = Новый Структура;
	Объект.Вставить("Ссылка", Документы.ПеремаркировкаТоваровИСМП.ПустаяСсылка());
	ДоступныеТоварныеГруппы = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступныеТоварныеГруппы(Объект);
	...
	|	ПеремаркировкаТоваровИСМП.ТоварнаяГруппа КАК ТоварнаяГруппа,
	...
	ВидыПродукции = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ДанныеЗаполнения.ТоварнаяГруппа);
	
	Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияПоддерживающаяИндивидуализациюКИЗ(ВидыПродукции) Тогда
...
Функция ДанныеЗаполненияУточненияСведенийОКодахМаркировкиИСМП(Организация) Экспорт
	...
	Объект = Новый Структура;
	Объект.Вставить("Ссылка", Документы.УточнениеСведенийОКодахМаркировкиИСМП.ПустаяСсылка());
	ДоступныеТоварныеГруппы = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступныеТоварныеГруппы(Объект);
	...
	|	УточнениеСведенийОКодахМаркировкиИСМП.ТоварнаяГруппа КАК ТоварнаяГруппа,
	...
	ЗаполнитьПоСтатистике(
		"Организация, ТоварнаяГруппа, Операция", 
		ДанныеЗаполнения, ДанныеПоследнихДокументов);
...
Функция ДанныеЗаполненияОтчетИСМП(Организация, Операция) Экспорт
...
Функция ДанныеЗаполненияПеремещениеМеждуМОД_ИСМП(Организация) Экспорт
	...
	Объект = Новый Структура;
	Объект.Вставить("Ссылка", Документы.ПеремещениеМеждуМОДИСМП.ПустаяСсылка());
	ДоступныеТоварныеГруппы = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступныеТоварныеГруппы(Объект);
	...
	|	ПеремещениеМеждуМОД_ИСМП.ТоварнаяГруппа КАК ТоварнаяГруппа,
	...
	ЗаполнитьПоСтатистике(
		"Организация, ТоварнаяГруппа, Отправитель, Получатель",
		ДанныеЗаполнения, ДанныеПоследнихДокументов, "");
...
Процедура ЗаполнитьДанныеПоТоварамЗаказаНаЭмиссиюКодовМаркировкиСУЗ(ОбъектЗаполнения, МассивСтрок = Неопределено, Знач ДополнительныеПоляПоиска = "") Экспорт
	...
	Если ДополнительныеПоляПоиска.Найти("Шаблон") = Неопределено Тогда
		ОбрабатыватьГрупповыеУпаковки = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетГрупповыеУпаковки(ВидыПродукцииПоТоварнойГруппе);
		Если ОбрабатыватьГрупповыеУпаковки Тогда
			ШаблоныГрупповойУпаковки = ИнтеграцияИСМПКлиентСервер.ШаблоныКодовПоВидуПродукции(Объект.ТоварнаяГруппа, 4);
		КонецЕсли;
		
		ОбрабатыватьНаборы = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетНаборы(ВидыПродукцииПоТоварнойГруппе);
		Если ОбрабатыватьНаборы Тогда
			ШаблоныНабора = ИнтеграцияИСМПКлиентСервер.ШаблоныКодовПоВидуПродукции(Объект.ТоварнаяГруппа, 5);
		КонецЕсли;
	КонецЕсли;
				...
				Если СтруктураДанных <> Неопределено
					...
					Если КоличествоБезУпаковок > 0 Тогда
						...
						Если ЗаполнятьРеквизитыФормы Тогда
							...
							ЗаказНаЭмиссиюСУЗКлиентСервер.УстановитьПредставлениеШаблонаВСтроке(СтрокаСоставУпаковки);
						КонецЕсли;
					...
					Иначе
						...
						Если ЗаполнятьРеквизитыФормы Тогда
							...
							ЗаказНаЭмиссиюСУЗКлиентСервер.УстановитьПредставлениеШаблонаВСтроке(СтрокаТовары);
						КонецЕсли;
	...
	Если Объект.СпособВводаВОборот = Перечисления.СпособыВводаВОборотСУЗ.МаркировкаОстатков Тогда
		ПараметрыОписанияОстатков = ИнтеграцияИСМПКлиентСервер.ИспользованиеПараметровОписанияОстатковПоВидуПродукции(ВидыПродукцииПоТоварнойГруппе);
		ЗаполнятьТоварныйЗнак = ПараметрыОписанияОстатков.ИспользоватьТоварныйЗнак;
	Иначе
		ЗаполнятьТоварныйЗнак = ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеТоварныйЗнак(ВидыПродукцииПоТоварнойГруппе);
	КонецЕсли;
	...
	ШаблонПоУмолчанию = ИнтеграцияИСМПКлиентСервер.ШаблонКодаМаркировкиПоВидуПродукции(
		Объект.ТоварнаяГруппа, 0);
	
	ВсеДоступныеШаблоны = ИнтеграцияИСМПКлиентСервер.ШаблоныКодовПоВидуПродукции(
		Объект.ТоварнаяГруппа, 3);
	...
	Для Каждого СтрокаТовары Из СтрокиТовары Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТовары.Шаблон)
			...
			СтрокаТовары.Шаблон = Неопределено;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(СтрокаТовары.Шаблон) Тогда
			СтрокаТовары.Шаблон = ШаблонПоУмолчанию;
		КонецЕсли;
...
Функция ТекстЗапросаЗаполненияТоварыЗаказаНаЭмиссиюСУЗ()
	...
	|		И ДокументыПоНоменклатуре.Ссылка.Проведен
	|		И ДокументыПоНоменклатуре.Ссылка.Организация = &Организация
	|		И ДокументыПоНоменклатуре.Ссылка.ТоварнаяГруппа = &ТоварнаяГруппа
...
Функция ТоварнаяГруппа(ДанныеПоследнихДокументов, ДоступныеТоварныеГруппы)
	
	ТоварнаяГруппа = Неопределено;
	
	ДанныеАнализа = ДанныеПоследнихДокументов.Скопировать(,"ТоварнаяГруппа, Количество");
	ДанныеАнализа.Свернуть("ТоварнаяГруппа", "Количество");
	ДанныеАнализа.Сортировать("Количество Убыв");
	Если ДанныеАнализа.Количество() > 0 Тогда
		ТоварнаяГруппа = ДанныеАнализа[0].ТоварнаяГруппа;
	...
	Если ТоварнаяГруппа = Неопределено
		И ДоступныеТоварныеГруппы.Количество() Тогда
...
Процедура ЗаполнитьПоСтатистике(Поля, ДанныеЗаполнения, ДанныеПоследнихДокументов, ПоляОтбора = "ТоварнаяГруппа")
