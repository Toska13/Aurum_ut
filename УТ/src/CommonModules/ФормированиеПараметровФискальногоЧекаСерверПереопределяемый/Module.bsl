...
// 	Кассир - СправочникСсылка.Пользователи, Неопределено - Пользователь, фискализирующий документ
...
//  Неопределено, ТаблицаЗначений - Таблицу с товарными позициями с количественными и суммовыми показателями:
...
Процедура ОбновитьПараметрыФискальногоЧекаПозицийЧекаККМ(ПараметрыФискальногоЧека, ДокументСсылка) Экспорт
	...
	ПредметыРасчетов = ПредметыРасчетовПоДокументу(
		ДокументСсылка,
		ЭтоЧекКоррекцииИсправленииОшибки(ПараметрыФискальногоЧека));
...
// 
// Параметры:
//  ПараметрыФискальногоЧека - см. ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПараметрыОперацииФискализацииЧека
...
Процедура ОбновитьПараметрыФискальногоЧекаСертификатыФСС(ПараметрыФискальногоЧека, ОплатыПлатежнымиКартами) Экспорт
	...
	Если ОплатыЭСФСС.Количество() > 0 Тогда
		...
		ПараметрыФискальногоЧекаДетализацияОплаты = ПараметрыФискальногоЧека.ДетализацияОплаты; // Массив
		ПараметрыФискальногоЧекаДетализацияОплаты.Добавить(ДетализацияОплаты);
...
// Параметры:
// 	ПараметрыФискальногоЧека - Структура - Параметры фискального чека
// 	ДокументыЕдиногоЧека - Массив Из Структура - Документы, входящие в единый чек
// 	СуммаПредоплаты - Число - Сумма предоплаты
//
Процедура ОбновитьПараметрыФискальногоЧекаПозицийЧекаЕдиныйЧек(ПараметрыФискальногоЧека, ДокументыЕдиногоЧека, СуммаПредоплаты = 0) Экспорт
	//++ Локализация
	СуммаОплаты = 0;
	СуммаПоставки = 0;
	...
	Если СуммаПредоплаты > 0
		И СуммаОплаты < СуммаПоставки
		И  (СуммаПоставки - СуммаОплаты) >= СуммаПредоплаты Тогда
		СуммаОплаты = СуммаОплаты + СуммаПредоплаты;
	КонецЕсли;
	...
	ЧекКоррекцииИсправленииОшибки = ЭтоЧекКоррекцииИсправленииОшибки(ПараметрыФискальногоЧека);
	
	Для Каждого ДокументПоставки Из ДокументыПоставки Цикл
		
		ПредметыРасчетов = ПредметыРасчетовПоДокументу(
			ДокументПоставки,
			ЧекКоррекцииИсправленииОшибки);
		ПараметрыФискальныхСтрокЧека = ПараметрыФискальныхСтрокЧека(ПредметыРасчетов, ПараметрыФискальногоЧека);
...
// 	СуммаПредоплаты - Число - Сумма предоплаты
//
Процедура ОбновитьПараметрыФискальногоЧекаТаблицаОплатЕдиныйЧек(ПараметрыФискальногоЧека, ДокументыЕдиногоЧека, СуммаПредоплаты = 0) Экспорт
	//++ Локализация
	ТаблицаОплат = Новый Массив();
	...
	Если СуммаПредоплаты > 0
		...
		СтрокаОплаты = Новый Структура();
		СтрокаОплаты.Вставить("ТипОплаты", Перечисления.ТипыОплатыККТ.Предоплата);//ТипОплаты);
		СтрокаОплаты.Вставить("Сумма", СуммаПредоплаты);//СуммаОплаты);
		ТаблицаОплат.Добавить(СтрокаОплаты);
...
Процедура ОбновитьПараметрыФискальногоЧекаПозицийЧекаПоДокументуПоставки(ПараметрыФискальногоЧека, ОбъектыРасчетовСПризнакамиСпособаРасчетов) Экспорт
	...
	Для Каждого ОбъектРасчетовСПризнакомСпособаРасчетов Из ОбъектыРасчетовСПризнакамиСпособаРасчетов Цикл
		...
		ПредметыРасчетов = ПредметыРасчетовПоДокументу(ОбъектРасчетов, ЧекКоррекцииИсправленииОшибки);
...
// Авансы по подарочным сертификатам детализируются в чеке до объекта расчета.
// 
// Параметры:
// 	ПараметрыФискальногоЧека - Структура - Параметры фискального чека
...
Процедура ОбновитьПараметрыФискальногоЧекаПозицийЧекаПоДокументуОплаты(ПараметрыФискальногоЧека, ОбъектыРасчетовСПризнакамиСпособаРасчетов, ИмяКомандыПробитияЧека) Экспорт
	...
	ЭтоОплатаПодарочногоСертификата = Ложь;
	
	БезналичнаяОплатаПоОбъектуРасчетов = Истина;
	Если Не ОбъектыРасчетовСПризнакамиСпособаРасчетов = Неопределено
		И ОбъектыРасчетовСПризнакамиСпособаРасчетов.Количество() = 1 
		И ОбъектыРасчетовСПризнакамиСпособаРасчетов[0].ОбъектРасчетов = ПараметрыФискальногоЧека.ДокументОснование Тогда
		БезналичнаяОплатаПоОбъектуРасчетов = Ложь;
	КонецЕсли;
	
	СуммыАвансовПоСтавкамНДСПоВсемОбъектамРасчетов = Новый ТаблицаЗначений();
	СуммыАвансовПоСтавкамНДСПоВсемОбъектамРасчетов.Колонки.Добавить("ОбъектРасчетов");
	...
	Для Каждого ОбъектРасчетовСПризнакомСпособаРасчетов Из ОбъектыРасчетовСПризнакамиСпособаРасчетов Цикл
		...
		Если ПризнакСпособаРасчета = Перечисления.ПризнакиСпособаРасчета.Аванс Тогда
			...
			Для Каждого СуммаАвансовПоСтавкамНДС Из СуммыАвансовПоСтавкамНДС Цикл
				...
				СуммаАвансаПоСтавкеНДСПоВсемОбъектамРасчетов.ОбъектРасчетов = ОбъектРасчетовСПризнакомСпособаРасчетов.ОбъектРасчетов;
				Если Не ЭтоОплатаПодарочногоСертификата И 
					ТипЗнч(ОбъектРасчетовСПризнакомСпособаРасчетов.ОбъектРасчетов) = Тип("СправочникСсылка.ПодарочныеСертификаты") Тогда
					ЭтоОплатаПодарочногоСертификата = Истина;
				КонецЕсли;
	...
	Если СуммыАвансовПоСтавкамНДСПоВсемОбъектамРасчетов.Количество() > 0 Тогда
		Если ЭтоОплатаПодарочногоСертификата Тогда
			СуммыАвансовПоСтавкамНДСПоВсемОбъектамРасчетов.Свернуть("ОбъектРасчетов,СтавкаНДС", "СуммаНДС,Сумма");
		Иначе
		СуммыАвансовПоСтавкамНДСПоВсемОбъектамРасчетов.Свернуть("СтавкаНДС", "СуммаНДС, Сумма");
		КонецЕсли;
		
		ДанныеДляЗаполнения = Новый Структура;
		ДанныеДляЗаполнения.Вставить("СуммыАвансовПоСтавкамНДС", СуммыАвансовПоСтавкамНДСПоВсемОбъектамРасчетов);
		ДанныеДляЗаполнения.Вставить("БезналичнаяОплатаПоОбъектуРасчетов", БезналичнаяОплатаПоОбъектуРасчетов);
		ДанныеДляЗаполнения.Вставить("ЭтоОплатаПодарочногоСертификата", ЭтоОплатаПодарочногоСертификата);
		
		ПривестиДанныеКТребуемомуФорматуПозицийЧекаПоДокументамОплатыАванс(ОбщегоНазначения.СкопироватьРекурсивно(ПараметрыФискальногоЧека),
																			ПозицииЧека,
																			ДанныеДляЗаполнения);
	КонецЕсли;
...
Функция ЭтоЧекКоррекцииИсправленииОшибки(ПараметрыФискальногоЧека)
	
	Если ТипЗнч(ПараметрыФискальногоЧека) = Тип("Структура")
		И ПараметрыФискальногоЧека.Свойство("НеприменениеККТ")
		И ПараметрыФискальногоЧека.Свойство("СформироватьЧекКоррекцииСторно")
		И ПараметрыФискальногоЧека.НеприменениеККТ = Ложь
		И ПараметрыФискальногоЧека.СформироватьЧекКоррекцииСторно = Истина Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Функция ПредметыРасчетовПоДокументу(ДокументСсылка, ЧекКоррекцииИсправленииОшибки = Ложь)
	...
	Если МодульЛокализации = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не настроен модуль локализации по документу.'"); 
	Иначе
		Возврат МодульЛокализации.ПредметыРасчетовПоДокументу(ДокументСсылка, ЧекКоррекцииИсправленииОшибки);
	КонецЕсли;
...
Функция ПараметрыФискальныхСтрокЧека(ПредметыРасчетов, ПараметрыФискальногоЧека)
	...
	ЭтоЧекКоррекцииИсправленияОшибки = ПараметрыФискальногоЧека.СформироватьЧекКоррекции = Истина 
		И ПараметрыФискальногоЧека.Свойство("НеприменениеККТ")
		И ЭтоЧекКоррекцииИсправленияОшибки(
			ПараметрыФискальногоЧека.ДокументОснование,
			ПараметрыФискальногоЧека.НеприменениеККТ);
	...
	Для Каждого СтрокаДляДобавленияВЧек Из ПредметыРасчетов Цикл
		...
		Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаДляДобавленияВЧек, "Штрихкод")
			...
			ИспользоватьЧислительИЗнаменатель = Ложь;
			Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаДляДобавленияВЧек, "РезультатРаспределения")
				И ЗначениеЗаполнено(СтрокаДляДобавленияВЧек.РезультатРаспределения) Тогда
				ИспользоватьЧислительИЗнаменатель = КонструкторИСМПОбщегоНазначенияКлиентСервер.ИспользоватьЧислительИЗнаменательПриЧастичномВыбытииНаККТ(
					СтрокаДляДобавленияВЧек.РезультатРаспределения.ВидПродукции);
			КонецЕсли;
			...
			Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаДляДобавленияВЧек, "РезультатРаспределения")
				...
				Если ВерсияФФД = "1.2" И ЗначениеЗаполнено(ПолныйКодМаркировки) Тогда
					Если (СтрокаДляДобавленияВЧек.РезультатРаспределения.ЧастичноеВыбытие
							И ИспользоватьЧислительИЗнаменатель)
						Или ЭтоЧекКоррекцииИсправленияОшибки Тогда
				...
				Если Не ЭтоЧекКоррекцииИсправленияОшибки
					И ЗначениеЗаполнено(ИдентификаторУстройства)
					...
					Если СтрокаДляДобавленияВЧек.РезультатРаспределения.ЧастичноеВыбытие
						И ИспользоватьЧислительИЗнаменатель Тогда
			...
			Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаДляДобавленияВЧек, "РезультатРаспределения")
				...
				И КонструкторИСМПКлиентСервер.ВидПродукцииИспользуетВетеринарныйСертификат(СтрокаДляДобавленияВЧек.РезультатРаспределения.ВидПродукции) Тогда
			...
			ИначеЕсли ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаДляДобавленияВЧек, "РезультатРаспределения")
				...
				И Не ИспользоватьЧислительИЗнаменатель Тогда
				
				МераКоличества = КонструкторИСМПОбщегоНазначенияКлиентСервер.МераКоличестваПредметаРасчетаПриЧастичномВыбытии(
					СтрокаДляДобавленияВЧек.РезультатРаспределения.ВидПродукции);
				
				Если НЕ ЗначениеЗаполнено(Упаковка) Тогда
					Упаковка = ИнтеграцияИСМПУТ.СокращеноеПредставлениеМерыКоличестваПредметаРасчета(МераКоличества);
				КонецЕсли;
				
				Если ВерсияФФД = "1.2" Тогда
					КодМеры = ОборудованиеЧекопечатающиеУстройства.КодМерыКоличестваПредметаРасчетаККТ(МераКоличества);
					СтрокаПозицииЧека.КодЕдиницыИзмерения = ОборудованиеЧекопечатающиеУстройства.КодЕдиницыИзмеренияПоККТКоду(КодМеры);
				Иначе
		...
		Если Не ЭтоЧекКоррекцииИсправленияОшибки Тогда
		...
		СтрокаПозицииЧека.ПризнакПредметаРасчета = РозничныеПродажиКлиентСервер.ПризнакПредметаРасчетаФискальнойОперации(
			...
			ЗначениеЗаполнено(СтрокаПозицииЧека.КонтрольнаяМарка)
				Или ЭтоЧекКоррекцииИсправленияОшибки, // УказанаКМ
		...
		Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаДляДобавленияВЧек, "ИспользованСертификатФСС") Тогда
			СтрокаПозицииЧека.ИспользованСертификатФСС = СтрокаДляДобавленияВЧек.ИспользованСертификатФСС;
		КонецЕсли;
...
Процедура ЗаполнитьСтрокуПоДаннымРазбораШтрихкода(СтрокаПозицииЧека, СтрокаДляДобавленияВЧек)
	...
	Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаДляДобавленияВЧек, "ВидПродукцииИС")
		И ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаДляДобавленияВЧек, "Штрихкод")
		И ЗначениеЗаполнено(СтрокаДляДобавленияВЧек.ВидПродукцииИС)
		И ЗначениеЗаполнено(СтрокаДляДобавленияВЧек.Штрихкод) Тогда
...
Процедура ПривестиДанныеКТребуемомуФорматуПозицийЧекаПоДокументамОплатыАванс(Знач ПараметрыФискальногоЧека, ПозицииЧека, ДанныеДляЗаполнения)
	...
	ОбновитьПараметрыФискальногоЧекаПозицийЧекаПоДокументамОплатыАвансДляПриведенияКФормату(ПараметрыФискальногоЧека, ПозицииЧекаДляПриведенияКФормату, ДанныеДляЗаполнения);
	ПараметрыФискальногоЧекаЗаполнитьПозицииЧека(ПараметрыФискальногоЧека, ПозицииЧекаДляПриведенияКФормату);
	
	ТаблицаОплат = Новый Массив();
	ДобавитьВТаблицуОплат(ТаблицаОплат, Перечисления.ТипыОплатыККТ.Наличные, ДанныеДляЗаполнения.СуммыАвансовПоСтавкамНДС.Итог("Сумма"));
...
Процедура ОбновитьПараметрыФискальногоЧекаПозицийЧекаПоДокументамОплатыАвансДляПриведенияКФормату(ПараметрыФискальногоЧека, ПозицииЧека, ДанныеДляЗаполнения)
	...
	Для Каждого СуммаАванса Из ДанныеДляЗаполнения.СуммыАвансовПоСтавкамНДС Цикл
		...
		НаименованиеОплаты = "";
		Если ДанныеДляЗаполнения.ЭтоОплатаПодарочногоСертификата Тогда
			НаименованиеОплаты =  Строка(СуммаАванса.ОбъектРасчетов);
		Иначе
			СтрокиНаименованияОплаты = Новый Массив;
			ЭтоПриходДенежныхСредств = Истина;
		Если ТипРасчетаДенежнымиСредствами = Перечисления.ТипыРасчетаДенежнымиСредствами.ПриходДенежныхСредств
			...
			Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПоступлениеБезналичныхДенежныхСредств")
					И ДанныеДляЗаполнения.БезналичнаяОплатаПоОбъектуРасчетов Тогда
					СтрокиНаименованияОплаты.Добавить(НСтр("ru = 'Оплата на р/с'"));
				ИначеЕсли ДокументБезналичнойОплаты(ДокументСсылка) И ДанныеДляЗаполнения.БезналичнаяОплатаПоОбъектуРасчетов Тогда
					СтрокиНаименованияОплаты.Добавить(НСтр("ru = 'Оплата картой'"));
			Иначе
					СтрокиНаименованияОплаты.Добавить(НСтр("ru = 'Оплата'"));
			КонецЕсли;
		Иначе
				ЭтоПриходДенежныхСредств = Ложь;
				СтрокиНаименованияОплаты.Добавить(НСтр("ru = 'Выдано'"));
		КонецЕсли;
		Если ЗначениеЗаполнено(КлиентАвансовогоПлатежаНаименование) Тогда
				Если ЭтоПриходДенежныхСредств Тогда
					СтрокиНаименованияОплаты.Добавить(НСтр("ru = 'от:'"));
		Иначе
					СтрокиНаименованияОплаты[СтрокиНаименованияОплаты.ВГраница()] = СтрокиНаименованияОплаты[СтрокиНаименованияОплаты.ВГраница()] + ":";
				КонецЕсли;
				СтрокиНаименованияОплаты.Добавить(КлиентАвансовогоПлатежаНаименование);
			КонецЕсли;
			НаименованиеОплаты = СтрСоединить(СтрокиНаименованияОплаты, Символ(32));
		КонецЕсли;
...
// Функция определяет, является ли переданный документ чеком коррекции исправления ошибки
// Возвращает Истина, если документ является одним из зарегистрированных типов чеков коррекции.
//
// Параметры:
//	ДокументСсылка - ДокументСсылка - Ссылка на документ, который необходимо проверить.
//	НеприменениеККТ - Булево - Признак чека коррекции вида "Неприменение ККТ", если Истина
//
// Возвращаемое значение:
//	Булево - Истина, если документ является чеком коррекции, иначе Ложь.
// 
Функция ЭтоЧекКоррекцииИсправленияОшибки(ДокументСсылка, НеприменениеККТ)
	
	Если НеприменениеККТ = Истина Тогда
		Возврат Ложь;
	КонецЕсли;
	
	// Массив типов документов, которые считаются чеками коррекции.
	// Вынесение в отдельную переменную упрощает расширение и чтение кода.
	МассивТиповКоррекций = Новый Массив;
	МассивТиповКоррекций.Добавить(Тип("ДокументСсылка.КорректировкаРеализации"));
	МассивТиповКоррекций.Добавить(Тип("ДокументСсылка.ЧекККМКоррекции"));
	
	// Проверка вхождения типа документа в заранее подготовленный список.
	// Так как ТипЗнч() вызывается только один раз, код работает эффективнее.
	СписокТиповКоррекций = Новый ОписаниеТипов(МассивТиповКоррекций);
	
	Возврат СписокТиповКоррекций.СодержитТип(ТипЗнч(ДокументСсылка));
