...
// Подготавливает на печать инструкцию покупателя MAX
//
// Возвращаемое значение:
//  ТабличныйДокумент - инструкция покупателя MAX.
//
Функция МакетИнструкцииПокупателяMAX() Экспорт
	
	ИнструкцияПокупателейMAX = Новый ТабличныйДокумент;
	Макет = ПолучитьОбщийМакет("ИнструкцияПокупателейMAXБПО");
	ОбластьКартинкаИнструкции = Макет.ПолучитьОбласть("ОбластьКартинкаИнструкции");
	ОбластьКартинкаИнструкции.Параметры.КартинкаИнструкции =
		БиблиотекаКартинок.QRКодДляПроверкиВозрастаВМаксДисплейПокупателяБПО;
	ИнструкцияПокупателейMAX.Вывести(ОбластьКартинкаИнструкции);
	ИнструкцияПокупателейMAX.АвтоМасштаб = Истина;
	
	МенеджерОборудованияВызовСервераПереопределяемый.ДополнитьМакетИнструкцииПокупателяMAX(ИнструкцияПокупателейMAX);
	
	Если ТипЗнч(ИнструкцияПокупателейMAX) <> Тип("ТабличныйДокумент") Тогда
		ИнструкцияПокупателейMAX = Новый ТабличныйДокумент;
	КонецЕсли;
	
	Возврат ИнструкцияПокупателейMAX;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбработатьДрайверыПриОбновленииИнформационнойБазы");
	Методы.Вставить("ОбновитьУстановленноеПодключаемоеОборудованиеССайта");
