...
Процедура ОбработкаЗаполнения(Объект, ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипДанныхЗаполнения = Тип("ДокументСсылка.ДокументОПредстоящейПоставке") Тогда
		ЗаполнитьДокументНаОснованииДОПП(Объект, ДанныеЗаполнения);
	КонецЕсли;
...
Функция ПредметыРасчетовПоДокументу(ДокументСсылка, ЧекКоррекцииИсправленииОшибки = Ложь) Экспорт
	...
	|	
	|	ЕСТЬNULL(СпрНоменклатура.Контрагент, ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)) КАК Агент,
	|	
...
//       * КорректировочноеУведомление - Булево - Признак какое нужно формировать уведомление. Если КорректировочноеУведомление = Ложь, нужно формировать Первичное уведомление.
//
Функция ПараметрыРегистрацииУведомленийОВвозеПрослеживаемыхТоваров(Объект) Экспорт
	...
	ПараметрыРегистрации.Вставить("КорректировочноеУведомление", Ложь);
...
//
Процедура ДополнитьТекстыЗапросовПроведенияУчетомНДС(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект) Экспорт
	
	//++ Локализация

	ТекстЗапросаТаблицаНастройкиУчетаНДСПриУСНКомитентов(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект);
...
Процедура ТекстЗапросаТаблицаНастройкиУчетаНДСПриУСНКомитентов(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект = Неопределено)
	
	ИмяРегистра = "НастройкиУчетаНДСПриУСНКомитентов";
	
	Если НЕ ПроведениеДокументов.ТребуетсяТаблицаДляДвижений(ИмяРегистра, Регистры) Тогда
		Возврат;
	КонецЕсли;
	 
	Если ДокументОбъект = Неопределено Или Не ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПриемНаКомиссию Тогда
		Возврат;
	КонецЕсли;
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ЕСТЬNULL(СтавкиНДС.СоответствующаяБазоваяСтавка, ТаблицаТовары.СтавкаНДС) СтавкаНДС
	|ПОМЕСТИТЬ ТаблицаЗаменныеСтавкиНДС
	|ИЗ
	|	ДанныеДокументаТаблицаТовары КАК ТаблицаТовары
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	Справочник.СтавкиНДС КАК СтавкиНДС
	|ПО
	|	СтавкиНДС.Ссылка = ТаблицаТовары.СтавкаНДС
	|	И СтавкиНДС.РасчетнаяСтавка
	|;
	|////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаСтавки.СтавкаНДС КАК СтавкаНДС,
	|	СтавкиНДС.ПеречислениеСтавкаНДС КАК ПеречислениеСтавкаНДС,
	|	СтавкиНДС.Ставка КАК СтавкаЗначение
	|ПОМЕСТИТЬ
	|	ТаблицаСтавкиНДС
	|ИЗ
	|	ТаблицаЗаменныеСтавкиНДС КАК ТаблицаСтавки
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	Справочник.СтавкиНДС КАК СтавкиНДС
	|ПО
	|	СтавкиНДС.Ссылка = ТаблицаСтавки.СтавкаНДС
	|;
	|////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(ТаблицаСтавки.СтавкаЗначение) КАК СтавкаЗначение
	|ПОМЕСТИТЬ МаксимальнаяСтавка
	|ИЗ
	|	ТаблицаСтавкиНДС КАК ТаблицаСтавки
	|;
	|////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Таблица.Период КАК Период,
	|	Таблица.Комитент КАК Комитент,
	|	ВЫБОР
	|		КОГДА Таблица.ПеречислениеСтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5) ИЛИ Таблица.ПеречислениеСтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЛьготноеНалогообложениеНДСНаУСН)
	|		ИНАЧЕ Таблица.ТоварыНалогообложение
	|	КОНЕЦ КАК ОсновноеНалогообложение,
	|	ВЫБОР
	|		КОГДА Таблица.ТоварыНалогообложение = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
	|		КОГДА Таблица.ПеречислениеСтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5) ИЛИ Таблица.ПеречислениеСтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7)
	|			ТОГДА Таблица.ТоварыСтавкаНДС
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	|	КОНЕЦ КАК СтавкаНДС
	|ИЗ
	|	(ВЫБРАТЬ
	|		НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ) КАК Период,
	|		&Контрагент КАК Комитент,
	|		&НалогообложениеНДС КАК ТоварыНалогообложение,
	|		МАКСИМУМ(ЕСТЬNULL(НастройкиУчетаНДС.ОсновноеНалогообложение,
	|			ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка))) КАК НастройкиНалогообложение,
	|		МАКСИМУМ(ЕСТЬNULL(НастройкиУчетаНДС.СтавкаНДС, ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка))) КАК НастройкиСтавкаНДС,
	|		МАКСИМУМ(ТаблицаСтавки.СтавкаНДС) КАК ТоварыСтавкаНДС,
	|		МАКСИМУМ(ТаблицаСтавки.ПеречислениеСтавкаНДС) КАК ПеречислениеСтавкаНДС
	|	ИЗ
	|		ТаблицаСтавкиНДС КАК ТаблицаСтавки
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		МаксимальнаяСтавка КАК МаксимальнаяСтавка
	|	ПО
	|		МаксимальнаяСтавка.СтавкаЗначение = ТаблицаСтавки.СтавкаЗначение
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.НастройкиУчетаНДСПриУСНКомитентов.СрезПоследних(НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ),
	|			Комитент = &Контрагент) КАК НастройкиУчетаНДС
	|	ПО ИСТИНА
	|	) КАК Таблица
	|ГДЕ
	|	ВЫБОР
	|		КОГДА Таблица.ТоварыНалогообложение = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|			ТОГДА Таблица.НастройкиНалогообложение <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|					ИЛИ Таблица.НастройкиСтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
	|		КОГДА Таблица.ПеречислениеСтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5) ИЛИ Таблица.ПеречислениеСтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7)
	|			ТОГДА Таблица.НастройкиНалогообложение <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЛьготноеНалогообложениеНДСНаУСН)
	|					или Таблица.ТоварыСтавкаНДС <> Таблица.НастройкиСтавкаНДС
	|		ИНАЧЕ Таблица.ТоварыНалогообложение <> Таблица.НастройкиНалогообложение
	|				ИЛИ Таблица.НастройкиСтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	|	КОНЕЦ
	|";
...
Процедура ЗаполнитьДокументНаОснованииДОПП(Объект, ДОПП)
	
	ДанныеЗаполнения = Документы.ДокументОПредстоящейПоставке.ДанныеЗаполненияНаОсновании(ДОПП);
	
	Шапка = ДанныеЗаполнения.Шапка;
	Если Шапка.ТипДоговора = Перечисления.ТипыДоговоров.ВвозИзЕАЭС 
		Или Не ЗначениеЗаполнено(Шапка.Договор) Тогда
		Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС;
	Иначе
		Возврат;
	КонецЕсли;
	
	ЗаполнитьЗначенияСвойств(Объект, ДанныеЗаполнения.Шапка);
