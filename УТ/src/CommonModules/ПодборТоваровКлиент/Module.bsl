...
Процедура ПриАктивизацииСтрокиИерархииНоменклатуры(Форма) Экспорт
	...
	Если Форма.ВариантНавигации <>
		ПредопределенноеЗначение("Перечисление.ВариантыНавигацииВФормахНоменклатуры.ПоИерархии") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОбработчикАктивизацииСтрокиИерархииНоменклатуры(Форма) Экспорт
	...
	Если ЗначениеЗаполнено(Форма.СтрокаПоискаНоменклатура) Тогда
		...
		ВыполнениеРасширенногоПоискаВозможно(
			Форма,
			Новый ОписаниеОповещения("ВыполнитьПоискНоменклатурыЗавершение", Форма));
...
// 	ДополнительныеПараметры - Структура Из Произвольный
//
...
Процедура ИзменитьВариантНавигации(Форма) Экспорт
	...
	СписокВыбораВариантовНавигации.Добавить(
		ПредопределенноеЗначение("Перечисление.ВариантыНавигацииВФормахНоменклатуры.ПоВидамИСвойствам"),
		НСтр("ru = 'Навигация по видам и свойствам'"));
	
	СписокВыбораВариантовНавигации.Добавить(
		ПредопределенноеЗначение("Перечисление.ВариантыНавигацииВФормахНоменклатуры.ПоИерархии"),
...
Процедура ЦеныНоменклатуры(Форма) Экспорт
	...
	ИначеЕсли ПодборТоваровКлиентСервер.ЭтоФормаПодбораВДокументыПродажи(Форма.КодФормы) Тогда
...
Процедура ПолучитьИнформациюОТовареПриПродаже(Форма) Экспорт
	...
	ПараметрыПолученияЦеныИОстатков.Номенклатура = Форма.ТекущаяСтрокаНоменклатуры.Номенклатура;
	ПараметрыПолученияЦеныИОстатков.Соглашение = Соглашение;
	ПараметрыПолученияЦеныИОстатков.Валюта = Валюта;
	ПараметрыПолученияЦеныИОстатков.Склады = Форма.Склады;
	ПараметрыПолученияЦеныИОстатков.ВидыЦен = Форма.ВидыЦен;
	ПараметрыПолученияЦеныИОстатков.КонтролируемаяПартия = Форма.КонтролируемаяПартия;
	ПараметрыПолученияЦеныИОстатков.ПолучитьОстатки = Форма.ОтображатьОстатки;
	ПараметрыПолученияЦеныИОстатков.ПолучитьЦену = Не Форма.РежимПодбораБезСуммовыхПараметров;
	
	Если Форма.НавигацияПоХарактеристикам Тогда
		
		Если ЗначениеЗаполнено(Форма.ТекущаяСтрокаХарактеристик.Характеристика) Тогда
			ПараметрыПолученияЦеныИОстатков.Характеристика = Форма.ТекущаяСтрокаХарактеристик.Характеристика;
		Иначе
			Возврат;
		КонецЕсли;
		
	Иначе
		ПараметрыПолученияЦеныИОстатков.Характеристика = Неопределено;
	КонецЕсли;
...
Процедура ПолучитьИнформациюОТовареПриЗакупке(Форма) Экспорт
	...
	ПараметрыПолученияЦеныИОстатков = ПодборТоваровКлиентСервер.НовыйПараметрыПолученияЦеныИОстатковТовара();
	ПараметрыПолученияЦеныИОстатков.Номенклатура = Номенклатура;
	ПараметрыПолученияЦеныИОстатков.Характеристика = Характеристика;
	ПараметрыПолученияЦеныИОстатков.Соглашение = Форма.Соглашение;
	ПараметрыПолученияЦеныИОстатков.Валюта = Форма.Валюта;
	ПараметрыПолученияЦеныИОстатков.Склады = Форма.Склады;
	ПараметрыПолученияЦеныИОстатков.ПолучитьОстатки = Форма.ОтображатьОстатки;
	ПараметрыПолученияЦеныИОстатков.ПолучитьЦену = Не Форма.РежимПодбораБезСуммовыхПараметров;
	
	ИнформацияОТоваре = ПодборТоваровВызовСервера.ЦенаЗакупкиИОстаткиТовара(ПараметрыПолученияЦеныИОстатков);
...
Процедура НастроитьПоиск(Форма) Экспорт
	...
	ОткрытьФорму("Справочник.Номенклатура.Форма.НастройкаВариантаПоиска", 
		Новый Структура("ВариантПоискаТоваров", Форма.ВариантПоискаТоваров),,,,,
		Новый ОписаниеОповещения("НастроитьПоискЗавершение",
			ЭтотОбъект,
			Новый Структура("Форма", Форма)), РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
// Возвращает результат проверки наличия в выделенных строках списка Номенклатура ссылок на элементы справочника.
//
// Параметры:
//	Форма - ФормаКлиентскогоПриложения - форма, содержащая список Номенклатура.
//
// Возвращаемое значение:
//	Булево - Истина, если в выделенных строках есть ссылки на элементы справочника Номенклатура.
//
Функция ЕстьВыделенныеСтрокиВСпискеНоменклатуры(Форма) Экспорт
	
	ИмяСписка			= ПодборТоваровКлиентСервер.ИмяСпискаНоменклатурыПоВариантуПоиска(Форма);
	СписокНоменклатуры	= Форма.Элементы[ИмяСписка];
	ВыделенныеСтроки	= ОбщегоНазначенияУТКлиент.ПроверитьПолучитьВыделенныеВСпискеСсылки(СписокНоменклатуры);
	
	ЕстьВыделенныеСтроки = ВыделенныеСтроки.Количество() > 0;
	
	Возврат ЕстьВыделенныеСтроки;
...
Процедура ПолучитьДанныеПеретаскивания(Форма, Элемент, ПараметрыПеретаскивания) Экспорт
	...
	Для каждого КлючСтроки Из ПараметрыПеретаскивания.Значение Цикл
		...
		Если ПодборТоваровКлиентСервер.ЭтоФормаПодбораВДокументыПродажи(Форма.КодФормы)
			И Не ЗначениеЗаполнено(ПараметрыТовара.ВидЦены) Тогда
				
			Если ЗначениеЗаполнено(Форма.ВидЦены) Тогда
				ВидЦеныДляПодстановки = Форма.ВидЦены;
			ИначеЕсли Форма.ВидыЦен.Количество() = 1 Тогда
				ВидЦеныДляПодстановки = Форма.ВидыЦен.Получить(0).Значение;
			Иначе
				ВидЦеныДляПодстановки = ПредопределенноеЗначение("Справочник.ВидыЦен.ПустаяСсылка");
			КонецЕсли;
			
			ПараметрыТовара.ВидЦены = ВидЦеныДляПодстановки;
		...
		ПараметрыТовара.ХарактеристикиИспользуются = ДанныеСтроки.ХарактеристикиИспользуются;
...
//
// Параметры:
// 	ЗначениеВыбора - КодВозвратаДиалога
// 	ДополнительныеПараметры - Структура Из Произвольный
//
Процедура УстановитьФиксированныйОтборЗавершение(ЗначениеВыбора, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеВыбора = Неопределено Тогда
		...
		ТекущиеДанные.Отбор =
			?(ТипЗначенияОтбора = Тип("Строка") Или ТипЗначенияОтбора = Тип("СписокЗначений"),
	...
	Если ТипЗначенияОтбора = Тип("Строка")
	...
	Иначе
		...
		Если ЗначениеВыбора <> Неопределено Тогда
			...
			Если ЗначениеВыбора.ИнтервалОт <> ТекущиеДанные.ИнтервалОт 
				...
				ТекущиеДанные.ПредставлениеОтбора = ПредставлениеИнтервалаОтбора(
					ЗначениеВыбора.ИнтервалОт,
...
Функция ВариантДействийПриИзмененииОтбораДереваОтборов(Форма)
	...
	Если ТекущиеДанные.ФиксированноеЗначение Тогда
		...
		Если ТекущиеДанные.Отбор Тогда
			...
			Если Не (ТипЗначенияОтбора = Тип("Строка")
					И ЗначениеЗаполнено(ТекущиеДанные.ЗначениеОтбора)
				Или ТипЗначенияОтбора <> Тип("Строка")
					И (ЗначениеЗаполнено(ТекущиеДанные.ИнтервалОт)
...
Функция СкладДоступенДляВыбора(Форма, Склад)
	...
	Возврат Форма.Склады.НайтиПоЗначению(Склад) <> Неопределено;
...
//
Функция ОписаниеКоличестваВСтрокеОстатка(
	Количество,
	НаименованиеУпаковкиЕдиницыИзмерения,
	ХарактеристикиИспользуются,
