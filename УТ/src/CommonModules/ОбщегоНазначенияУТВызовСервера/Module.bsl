	...
	Для Каждого СоставДокументов Из СоответствиеТипов Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Попытка
				...
				ШаблонТекущейОшибки = ШаблонОшибкиЗаблокировать;
				...
				ШаблонТекущейОшибки = ШаблонОшибкиЗаписать;
				
				Объект = Выборка.Ссылка.ПолучитьОбъект(); // ДокументОбъект
				Если Объект.УстановитьСтатус(НовыйСтатус, ДополнительныеПараметры) Тогда
					
					Объект.Записать(?(Выборка.ЗаписьПроведением,
			...
			Исключение
				...
				ТекстОшибки = СтрЗаменить(ШаблонТекущейОшибки,
					"%Документ%",
					Выборка.Представление);
				
				ТекстОшибкиКраткий = СтрЗаменить(ТекстОшибки,
					"%ОписаниеОшибки%",
					ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
				
				ТекстОшибкиПодробный = СтрЗаменить(ТекстОшибки,
					"%ОписаниеОшибки%",
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибкиКраткий, Выборка.Ссылка);
...
// Выполняет попытку отмены проведения документов и возвращает коллекцию, содержащую сведения о документах, для которых
// не выполнена отмена проведения.
//
// Параметры:
//	МассивСсылок - Массив Из ДокументСсылка - массив ссылок документов.
//
// Возвращаемое значение:
//	Массив Из Структура - коллекция, содержащая сведения о документах, для которых не выполнена отмена проведения:
//		* Ссылка - ДокументСсылка - документ, для которого не удалось выполнить отмену проведения.
//		* ОписаниеОшибки - Строка - текст описания ошибки из-за которой не удалось выполнить отмену проведения.
//									Отсутствующее описание ошибки означает, что документ раннее не был проведен.
//
Функция ОтменаПроведенияДокументов(МассивСсылок) Экспорт
	...
	Для Каждого Ссылка Из НепроведенныеДокументы Цикл
		НераспроведенныеДокументы.Добавить(Новый Структура("Ссылка, ОписаниеОшибки", Ссылка, ""));
	КонецЦикла;
	...
	Для Каждого Ссылка Из РаспроводимыеДокументы Цикл
		...
		Попытка
			ИмяДокумента = Ссылка.Метаданные().ПолноеИмя();
			
			БлокировкаДанных = Новый БлокировкаДанных;
			
			ЭлементБлокировкиДанных = БлокировкаДанных.Добавить(ИмяДокумента);
		...
		Исключение
			...
			НераспроведенныеДокументы.Добавить(Новый Структура("Ссылка, ОписаниеОшибки", Ссылка, ОписаниеОшибки));
		КонецПопытки;
	КонецЦикла;
	
	Возврат НераспроведенныеДокументы;
...
Функция БанковскийСчетОрганизацииПоУмолчанию() Экспорт
	...
	Возврат ДенежныеСредстваСервер.ПолучитьБанковскийСчетОрганизацииПоУмолчанию();
...
// Записать в журнал сообщить пользователю о результатах проведения или отмены проведения.
//
// Параметры:
//	ДанныеОНепроведенныхДокументах - см. ОбщегоНазначения.ПровестиДокументы.
//	Проведение - Булево - признак режима записи документа. Если значение Истина - тогда, осуществляется запись в журнал
//							сведения о результатах проведения документов.
//
// Возвращаемое значение:
//	Массив из ДокументСсылка - коллекция ссылок на документы, для которых не удалось выполнить проведение или отмену
//								проведения. В коллекцию не включаются ссылки на документы, для которых отсутствуют
//								описание ошибки.
//
Функция ЗаписатьВЖурналСообщитьПользователюОРезультатахПроведенияОтменыПроведения(ДанныеОНепроведенныхДокументах,
																					Проведение = Истина) Экспорт
...
Процедура ПараметрыРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Параметры.Вставить("ОткрытьУведомлениеОбУстаревшейФункциональности", Ложь);
	...
	Если НЕ ОбщегоНазначения.РазделениеВключено() Тогда
		Параметры.ОткрытьУведомлениеОбУстаревшейФункциональности =
			Пользователи.ЭтоПолноправныйПользователь() 
			И Константы.УведомитьОбУстаревшейФункциональности.Получить() 
			И Обработки.ГотовностьКПереходу26.ПроверитьИспользованиеУстаревшейФункциональности();
	КонецЕсли;
...
Процедура ПроверитьПараметрыРеквизитовОбъекта(ПараметрыРеквизитовОбъекта) Экспорт
	...
	Для каждого ПараметрыРеквизита Из ПараметрыРеквизитовОбъектаТаблица Цикл
		Если ПараметрыРеквизита.Количество > 1 И ПараметрыРеквизита.ИмяРеквизита <> "" Тогда
			ТекстСообщения = НСтр("ru = 'Отладка: для реквизита ""%1"" свойство ""%2"" определяется несколько раз.'");
