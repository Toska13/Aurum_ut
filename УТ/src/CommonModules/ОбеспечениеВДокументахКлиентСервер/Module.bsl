...
//   * ЗаполняемыеПоля - Строка - заполняемые поля при возврате из формы подбора складов и серий в форму документа.
//
Функция ПараметрыФормыЗапросаКоличестваИСерий(Все = Истина) Экспорт
	...
	ПараметрыФормы.Вставить("КонтролируемаяПартия",            Неопределено);
	...
	ПараметрыФормы.Вставить("Обособленно",                        Ложь);
...
//  ПараметрОбособленно - Булево - значение признака "Обособленно".
//
// Возвращаемое значение:
//  Структура - структура с полями:
//   * ВариантОбеспечения - ПеречислениеСсылка.ВариантыОбеспечения - значение варианта обеспечения.
//   * Обособленно - Булево - значение признака "Обособленно".
//
Функция ВариантИФлагПоУмолчаниюДляПодбора(
	ТипНоменклатуры,
	ДопустимыеОбособленныеДействия,
	ДопустимыеНеобособленныеДействия,
	ОграничиватьВариантыОбеспечения,
	Знач ВариантОбеспеченияИФлагОбособленно,
	ПараметрОбособленно) Экспорт
	...
	Результат = Новый Структура("Обособленно,ВариантОбеспечения",
		Ложь, ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.ПустаяСсылка"));
	
	ДопустимыеВариантыОбеспечения = ДопустимыеВариантыОбеспечения(
		ТипНоменклатуры,
		ДопустимыеОбособленныеДействия,
		ДопустимыеНеобособленныеДействия,
		ОграничиватьВариантыОбеспечения,
	...
	Если Обособленно = Неопределено Тогда
	...
	Если Обособленно И ДопустимыеВариантыОбеспечения.Обособленные.Найти(ВариантОбеспечения) <> Неопределено 
			Или Не Обособленно И ДопустимыеВариантыОбеспечения.Необособленные.Найти(ВариантОбеспечения) <> Неопределено Тогда
			
			Результат.Обособленно = Обособленно;
			Результат.ВариантОбеспечения = ВариантОбеспеченияИФлагОбособленно[ИмяСвойства].ВариантОбеспечения;
		
	ИначеЕсли Обособленно И ДопустимыеВариантыОбеспечения.Необособленные.Найти(ВариантОбеспечения) <> Неопределено
				Или Не Обособленно И ДопустимыеВариантыОбеспечения.Обособленные.Найти(ВариантОбеспечения) <> Неопределено Тогда
			
			Результат.Обособленно = Не Обособленно;
			Результат.ВариантОбеспечения = ВариантОбеспечения;
			
	Иначе
		
		Если Обособленно И ДопустимыеВариантыОбеспечения.Обособленные.Количество() > 0
				Или Не Обособленно И ДопустимыеВариантыОбеспечения.Необособленные.Количество() = 0 Тогда
...
Функция ДопустимыеВариантыОбеспечения(ТипНоменклатуры, ДопустимыеОбособленныеДействия, ДопустимыеНеобособленныеДействия, ОграничиватьВариантыОбеспечения, ВариантОбеспеченияИФлагОбособленно) Экспорт
	...
	Если ОграничиватьВариантыОбеспечения Тогда
		...
		Если ВариантОбеспеченияИФлагОбособленно[ИмяСвойства].Обособленно = Неопределено Тогда
		...
		ИначеЕсли ВариантОбеспеченияИФлагОбособленно[ИмяСвойства].Обособленно Тогда
...
// Возвращает структуру, содержащую поля, участвующие в пакетной обработке строк табличных частей.
// 
// Возвращаемое значение:
//  Структура - Структура полей проверить необходимость обновления колонки доступно:
// * Номенклатура - СправочникСсылка.Номенклатура, Неопределено - номенклатура,
// * Характеристика - СправочникСсылка.ХарактеристикиНоменклатуры, Неопределено - характеристика,
// * ВариантОбеспечения - ПеречислениеСсылка.ВариантыОбеспечения, Неопределено - вариант обеспечения.
//
Функция СтруктураПолейПроверитьНеобходимостьОбновленияКолонкиДоступно() Экспорт
	
	СтруктураПолей = Новый Структура();
	СтруктураПолей.Вставить("Номенклатура", Неопределено);
	СтруктураПолей.Вставить("Характеристика", Неопределено);
	СтруктураПолей.Вставить("ВариантОбеспечения", Неопределено);
	Возврат СтруктураПолей;
	
КонецФункции

// Структура параметров расчета цикла обеспечения.
// Возвращаемое значение:
//  Структура - структура с полями:
//   * СрокИсполненияЗаказа - см. Справочник.СпособыОбеспеченияПотребностей.СрокИсполненияЗаказа
//   * ФормироватьЗаказыПоТочкеЗаказа - см. Справочник.СпособыОбеспеченияПотребностей.ФормироватьЗаказыПоТочкеЗаказа
//   * ФормироватьЗаказыПоРасписанию - см. Справочник.СпособыОбеспеченияПотребностей.ФормироватьЗаказыПоРасписанию
//   * ГрафикПоставок - см. Справочник.СпособыОбеспеченияПотребностей.ГрафикПоставок
//   * ПерваяДатаВГрафикеПоставок - см. Справочник.СпособыОбеспеченияПотребностей.ПерваяДатаВГрафикеПоставок
//   * ПериодичностьГрафикаПоставок - см. Справочник.СпособыОбеспеченияПотребностей.ПериодичностьГрафикаПоставок
//   * КоличествоПериодовВИнтервалеМеждуПоставками - см. Справочник.СпособыОбеспеченияПотребностей.КоличествоПериодовВИнтервалеМеждуПоставками
//   * ВариантОбеспечиваемогоПериода - см. Справочник.СпособыОбеспеченияПотребностей.ВариантОбеспечиваемогоПериода
//   * ОбеспечиваемыйПериод - см. Справочник.СпособыОбеспеченияПотребностей.ОбеспечиваемыйПериод
Функция ПараметрыРасчетаЦиклаОбеспечения() Экспорт
	
	Результат = Новый Структура(
		"СрокИсполненияЗаказа,
		|ФормироватьЗаказыПоТочкеЗаказа,
		|ФормироватьЗаказыПоРасписанию,
		|ГрафикПоставок,
		|ПерваяДатаВГрафикеПоставок,
		|ПериодичностьГрафикаПоставок,
		|КоличествоПериодовВИнтервалеМеждуПоставками,
...
Функция ПакетнаяОбработкаСтруктураПолейТекущейСтроки(ПараметрыДействия) Экспорт
	
	СтруктураТаблицы = Неопределено;
	
	ПараметрыВстраивания = ПараметрыДействия.ПараметрыВстраивания;
	ШаблонСериализации = ПараметрыВстраивания.ШаблонСериализацииДанныхФормы;
	ЧастиИмени = СтрРазделить(ПараметрыВстраивания.ИмяТаблицы, ".");
	ИмяТаблицы = ЧастиИмени[ЧастиИмени.Количество() - 1];
	
	Если ПараметрыВстраивания.ИмяТаблицы = Неопределено Тогда
		СтруктураТаблицы = ШаблонСериализации.Объект;
	ИначеЕсли Не ШаблонСериализации.Свойство(ИмяТаблицы, СтруктураТаблицы) Тогда
		
		Для Каждого КлючИЗначение Из ШаблонСериализации Цикл
			
			Если ТипЗнч(КлючИЗначение.Значение) = Тип("Структура") Тогда
				
				Если КлючИЗначение.Значение.Свойство(ИмяТаблицы, СтруктураТаблицы) Тогда
					Прервать;
				КонецЕсли;
