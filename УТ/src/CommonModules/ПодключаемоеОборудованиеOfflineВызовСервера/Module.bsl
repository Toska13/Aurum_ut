...
// Фиксация ошибки в журнале регистрации из клиентского контекста. Предназнаен для вызова из клиентского контекста.
// Параметры:
//	ИмяСобытия - Строка - Имя события журнала регистрации, 
//						  имя должно однозначно указывать на контекст вызова текущего метода,
//						  Например, название модуля и метода, разделенных точкой.
//	ТекстОшибки - Строка
Процедура ЗаписатьОшибкуВЖурналРегистрации(Знач ИмяСобытия, Знач ТекстОшибки) Экспорт
	Если ПустаяСтрока(ИмяСобытия) Тогда
		Возврат;
	КонецЕсли;
	ФиксируемоеИмя = "ПодключаемоеОборудованиеOfflineВызовСервера.ЗаписатьОшибкуВЖурналРегистрации."+ИмяСобытия;
	ЗаписьЖурналаРегистрации(ФиксируемоеИмя, УровеньЖурналаРегистрации.Ошибка, , , ТекстОшибки);
КонецПроцедуры
