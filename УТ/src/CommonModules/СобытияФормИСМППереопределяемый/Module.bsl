...
Процедура ЗаполнениеРеквизитовФормы(Форма) Экспорт
	...
	ПараметрыИнтеграции = Форма.ПараметрыИнтеграцииГосИС.Получить("ФормаПроверкиИПодбора");
	...
	Если Не ЗначениеЗаполнено(ПараметрыИнтеграции)
		Или ОбщегоНазначенияИСМПКлиентСерверПовтИсп.УчитываемыеВидыМаркируемойПродукции().Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ПараметрыИнтеграции Из Сценарии Цикл
		...
		Если ПараметрыИнтеграции.РазмещатьЭлементыИнтерфейса
			...
			ПараметрыЗаполненияРеквизитов.Вставить(ОбработчикЗаполнитьПризнакМаркируемаяПродукция, Новый Структура("Номенклатура", "МаркируемаяПродукция"));
			НоменклатураСервер.ЗаполнитьСлужебныеРеквизитыПоНоменклатуреВКоллекции(Форма[Объект][Товары], ПараметрыЗаполненияРеквизитов);
			
			Если ПараметрыИнтеграции.ДоступныОбъемноСортовыеКоды
				И ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ДопустимаПродукцияИСМП(ПараметрыСканирования, Истина) Тогда
				ПроверкаИПодборПродукцииИСМП.ЗаполнитьПризнакАвтоматическийОСУИСВТаблице(Форма[Объект][Товары],, ПараметрыСканирования, ПараметрыСканирования.ДатаДокумента);
			КонецЕсли;
		...
		Если ПараметрыИнтеграции.Свойство("ИспользоватьКолонкуРасхожденияПоКодамМаркировки")
			...
			Если ДоступноСогласованиеРасхождений Тогда
				ТребуетсяОбработкаКодовМаркировки = ИнтеграцияИСМПУТ.ПриОпределенииНеобходимостиОбработкиКодовМаркировкиВДокументе(
					Форма[Объект],
...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли Форма.ИмяФормы = "Документ.ЧекККМКоррекции.Форма.ФормаДокумента" Тогда
	
		ИнтеграцияИСУТ.МодифицироватьИнициализироватьФормуЧекаКоррекции(Форма, Истина);
	...
	ИначеЕсли Форма.ИмяФормы = "Документ.ЗаявкаНаВозвратТоваровОтКлиента.Форма.ФормаДокумента" Тогда
		...
		ИнтеграцияИСУТ.МодифицироватьИнициализироватьФормуЗаявкаНаВозвратТоваровОтКлиента(Форма);
		
	ИначеЕсли Форма.ИмяФормы = "Обработка.ПодборТоваровВДокументПродажи.Форма.Форма" Тогда
		
		ИнтеграцияИСУТ.МодифицироватьИнициализироватьФормуПодборТоваровВДокументПродажи(Форма)
...
Процедура ПриЧтенииНаСервере(Форма, ТекущийОбъект) Экспорт
	...
	ИначеЕсли ИмяФормы = "Документ.ЧекККМКоррекции.Форма.ФормаДокумента" Тогда

		ИнтеграцияИСУТ.МодифицироватьИнициализироватьФормуЧекаКоррекции(Форма);
...
Процедура ПриИзмененииЭлемента(Форма, Элемент, ДополнительныеПараметры) Экспорт
	...
	Если ИнтеграцияИСУТКлиентСервер.ЕстьПострочныйСтатусПроверкиВФорме(Форма) Тогда
		...
		Если Элемент = "Товары" Или Элемент = "ОтгружаемыеТовары" Тогда
			
			ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(Форма);
			
			Если ШтрихкодированиеОбщегоНазначенияИС.ДопустимаПродукцияИСМП(ПараметрыСканирования, Истина) Тогда
				ПроверкаИПодборПродукцииИСМП.ЗаполнитьПризнакАвтоматическийОСУИСВТаблице(Форма.Объект[Элемент],, ПараметрыСканирования, ПараметрыСканирования.ДатаДокумента);
			КонецЕсли;
