...
Процедура ЗаполнитьЗначенияНДСПродажиПоДаннымРеализации(Объект) Экспорт
	...
	|		СтрокиСРеализацией
	|		ЛЕВОЕ СОЕДИНЕНИЕ
	|		Документ.ОтгрузкаТоваровКлиенту.Товары КАК ОтгрузкаТоваровКлиенту
	|			ПО СтрокиСРеализацией.СчетФактураВыставленныйДокументОснование = ОтгрузкаТоваровКлиенту.Ссылка И
	|			СтрокиСРеализацией.Номенклатура = ОтгрузкаТоваровКлиенту.Номенклатура  И
	|			СтрокиСРеализацией.Характеристика = ОтгрузкаТоваровКлиенту.Характеристика  И
	|			СтрокиСРеализацией.Упаковка = ОтгрузкаТоваровКлиенту.Упаковка 
	|				ЛЕВОЕ СОЕДИНЕНИЕ
	|					Документ.РеализацияТоваровУслуг.ВидыЗапасов КАК РеализацияТоваровИУслугВидыЗапасов
	|					ПО ОтгрузкаТоваровКлиенту.Ссылка.ЗаказКлиента = РеализацияТоваровИУслугВидыЗапасов.Ссылка.ЗаказКлиента 
	|						И ОтгрузкаТоваровКлиенту.Номенклатура = РеализацияТоваровИУслугВидыЗапасов.АналитикаУчетаНоменклатуры.Номенклатура
	|						И ОтгрузкаТоваровКлиенту.Характеристика = РеализацияТоваровИУслугВидыЗапасов.АналитикаУчетаНоменклатуры.Характеристика
	|						ЛЕВОЕ СОЕДИНЕНИЕ
	|							РегистрСведений.СуммыДокументовВВалютахУчета КАК СуммыДокументовВВалютахУчета
	|							ПО РеализацияТоваровИУслугВидыЗапасов.Ссылка = СуммыДокументовВВалютахУчета.Регистратор И
	|							РеализацияТоваровИУслугВидыЗапасов.ИдентификаторСтроки  =  СуммыДокументовВВалютахУчета.ИдентификаторСтроки	
	...
	|				ЛЕВОЕ СОЕДИНЕНИЕ
	|					Документ.РеализацияТоваровУслуг.ВидыЗапасов КАК РеализацияТоваровИУслугВидыЗапасов
	|					ПО ОтгрузкаТоваровКлиенту.Ссылка = РеализацияТоваровИУслугВидыЗапасов.Ссылка.ЗаказКлиента 
	|						И ОтгрузкаТоваровКлиенту.Номенклатура = РеализацияТоваровИУслугВидыЗапасов.АналитикаУчетаНоменклатуры.Номенклатура
	|						И ОтгрузкаТоваровКлиенту.Характеристика = РеализацияТоваровИУслугВидыЗапасов.АналитикаУчетаНоменклатуры.Характеристика
	|						ЛЕВОЕ СОЕДИНЕНИЕ
	|							РегистрСведений.СуммыДокументовВВалютахУчета КАК СуммыДокументовВВалютахУчета
