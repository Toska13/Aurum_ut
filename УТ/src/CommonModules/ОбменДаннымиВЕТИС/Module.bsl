		...
		Если ДанныеСтроки = Неопределено Тогда
			...
			Если НайденныеСтроки.Количество() > 0 Тогда
				Для Каждого СтрокаТЧ Из НайденныеСтроки Цикл
					...
					Если ЕстьКолонкаДатаПроизводства Тогда
					...
					Иначе
						ПроверятьДатуПроизводства = Ложь;
					КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбменДаннымиРегламентноеЗадание", Истина);
...
Функция ЗапросНастроекРегламентыхЗаданий(ОрганизацииВЕТИС) Экспорт
	...
	|	ЕСТЬNULL(ТаблицаВремяВыполнения.ВремяВыполнения, -1),
