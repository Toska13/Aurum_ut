...
// 
// Параметры:
//  РезультатРаспределения - см. ШтрихкодированиеИСКлиентСервер.НовыйРезультатРаспределенияШтрихкодовПоТоварам.
// 
...
Функция НовыйЭлементПроверкиСредствамиККТПоДаннымРаспределения(РезультатРаспределения) Экспорт
	...
	ЗаполнитьЗначенияСвойств(
		...
		"ВидПродукции,ВидУпаковки,КодМаркировки,ПолныйКодМаркировки,СоставКодаМаркировки,ШтрихкодУпаковки,Сумма,ЗапрещеноИгнорироватьОшибку,
		|Количество,ТекстОшибки,КодЕдиницыИзмерения,КоличествоПотребительскихУпаковок,МинимальнаяРозничнаяЦена,
...
Процедура ОбработатьСобытиеПотоковойПечати(Форма, ДанныеШтрихкода, ПараметрыСканирования, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "Организация") Тогда
		...
		СтруктураПечати.ВидПродукции   = ДанныеВыбора.ВидПродукции;
	...
	Если ДанныеВыбора.ТребуетВзвешивания Тогда
		...
		Если ЗначениеЗаполнено(СтруктураПечати.Количество) Тогда
			Если КонструкторИСМПКлиентСервер.КодМаркировкиМожетВключатьВес(СтруктураПечати.ВидПродукции) Тогда
				ДополнитьПолныйКодМаркировки = "3103";
			ИначеЕсли КонструкторИСМПКлиентСервер.КодМаркировкиМожетВключатьОбъем(СтруктураПечати.ВидПродукции) Тогда
...
Процедура ОбработкаНавигационнойСсылкиТекстаОшибки(Форма, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ОтключитьКонтрольВладельцев" Тогда
		...
		Если ВладелецФормы <> Неопределено Тогда
			...
			Если ШтрихкодированиеИСКлиентСервер.ЭтоКонтекстОбъекта(ВладелецФормы, "Обработка.ПроверкаИПодборПродукцииИС") Тогда
...
Процедура ОповещениеОЗакрытииФормыОшибки(РезультатЗакрытияФормы, ДополнительныеПараметры) Экспорт
	...
	Если ПараметрыПроверки <> Неопределено Тогда
		...
		ИначеЕсли ПараметрыПроверки.ОткрытьФормуПроверки Тогда
			...
			ПроверкаИПодборПродукцииИСКлиент.ОткрытьФормуПроверкиИПодбора(
...
Функция ДанныеПроверкиПередОбработкойШтрихкода(ВидМаркируемойПродукции, Штрихкод, ПараметрыСканирования, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(ВидМаркируемойПродукции) = Тип("СправочникСсылка.КлючиВидовПродукцииИС") Тогда
		ВидПродукции = Неопределено;
		Если ПараметрыСканирования.ИСМП Тогда
			ВидПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(ВидМаркируемойПродукции, "ГИСМТ");
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ВидПродукции) И ПараметрыСканирования.ЕГАИС Тогда
			ВидПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(ВидМаркируемойПродукции, "ЕГАИС");
		КонецЕсли;
	Иначе
	...
	ДанныеРазбора = РазборКодаМаркировкиИССлужебныйКлиент.РазобратьКодМаркировки(
		Штрихкод, ВидПродукции, ПримечаниеКРезультатуРазбора);
	
	Если ДанныеРазбора = Неопределено
		И ПримечаниеКРезультатуРазбора.ИдентификаторОшибки <> "КодМаркировкиСоответствуетНесколькимШаблонам"
		Или (ДанныеРазбора <> Неопределено
			И ДанныеРазбора.ВидыУпаковокПоВидамПродукции[ВидПродукции].Количество() = 0) Тогда
		ВозвращаемоеЗначение.ТекстОшибки = НСтр("ru = 'Недопустимый формат штрихкода'");
		ВозвращаемоеЗначение.ЕстьОшибки  = Истина;
		Возврат ВозвращаемоеЗначение;
	КонецЕсли;
