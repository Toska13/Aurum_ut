...
Функция СоздатьБизнесЗаказНаЭмиссиюКодовМаркировки_V2(ТелоЗапросаJSON, ТоварнаяГруппа, ПараметрыСУЗ, Подпись = Неопределено) Экспорт
	...
	Если ПараметрыСУЗ.ФорматОбмена = Перечисления.ВерсииФорматаОбменаСУЗ.V3
		И Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ТоварнаяГруппа) Тогда
		URLЗапроса = СтрШаблон("api/v3/order?omsId=%1", ПараметрыСУЗ.Идентификатор);
	Иначе
		URLЗапроса = СтрШаблон(
			URLЗапроса("api/v2/extension/orders?omsId=%1", ТоварнаяГруппа),
...
Функция ПередатьОтчетОСписанииКодовМаркировки_V2(ТелоЗапросаJSON, ТоварнаяГруппа, Операция, ПараметрыСУЗ, Подпись = Неопределено) Экспорт
	...
	Иначе
		URLЗапроса = СтрШаблон(
			URLЗапроса("api/v2/extension/dropout?omsId=%1", ТоварнаяГруппа),
			ПараметрыСУЗ.Идентификатор);
	КонецЕсли;
...
Функция ПередатьОтчетОбИспользованииКодовМаркировки_V2(ТелоЗапросаJSON, ЭлементОчереди, ПараметрыСУЗ, Подпись = Неопределено) Экспорт
	...
	Иначе
		URLЗапроса = СтрШаблон(
			URLЗапроса_V2("api/v2/extension/utilisation?omsId=%1", ЭлементОчереди.ТоварнаяГруппа),
			ПараметрыСУЗ.Идентификатор);
	КонецЕсли;
...
Функция ПередатьОтчетОбАгрегацииКодовМаркировки_V2(ТелоЗапросаJSON, ЭлементОчереди, ПараметрыСУЗ, Подпись = Неопределено) Экспорт
	...
	Иначе
		URLЗапроса = СтрШаблон(
			URLЗапроса_V2("api/v2/extension/aggregation?omsId=%1", ЭлементОчереди.ТоварнаяГруппа),
			ПараметрыСУЗ.Идентификатор);
	КонецЕсли;
...
Функция ПодтвердитьПоступлениеКИЗ_V3(ТелоЗапросаJSON, ТоварнаяГруппа, ПараметрыСУЗ, Подпись = Неопределено) Экспорт
	...
	URLЗапроса = СтрШаблон(
		URLЗапроса("api/v2/extension/acceptance?omsId=%1", ТоварнаяГруппа),
		ПараметрыСУЗ.Идентификатор);
...
Функция ЗапроситьМестаОсуществленияДеятельности(ТоварнаяГруппа, ПараметрыСУЗ, ТелоЗапросаJSON) Экспорт
	...
	Если ПараметрыСУЗ.ФорматОбмена = Перечисления.ВерсииФорматаОбменаСУЗ.V3
		...
		URLЗапроса = СтрШаблон("api/v3/mod?omsId=%1&productGroup=%2",
			ПараметрыСУЗ.Идентификатор,
			КонструкторИСМПОбщегоНазначенияКлиентСервер.ИмяТоварнойГруппы(ТоварнаяГруппа));
	Иначе
		URLЗапроса = СтрШаблон(
			URLЗапроса("api/v2/extension/mod?omsId=%1", ТоварнаяГруппа),
			ПараметрыСУЗ.Идентификатор);
	КонецЕсли;
...
Функция ЗакрытьПодзаказПоGTIN_V2(ПараметрыЗакрытия, ТоварнаяГруппа, ПараметрыСУЗ, ТелоЗапросаJSON, Подпись) Экспорт
	...
	Иначе
		...
		Если ПараметрыЗакрытия.Свойство("GTIN") Тогда
			URLЗапроса = СтрШаблон(
				URLЗапроса_V2("api/v2/extension/buffer/close?orderId=%1&gtin=%2&omsId=%3&lastBlockId=%4", ТоварнаяГруппа),
		...
		Иначе
			URLЗапроса = СтрШаблон(
				URLЗапроса_V2("api/v2/extension/buffer/close?orderId=%1&omsId=%2", ТоварнаяГруппа),
				ПараметрыЗакрытия.ИдентификаторБизнесЗаказа,
				ПараметрыСУЗ.Идентификатор);
		КонецЕсли;
...
Функция ЗакрытьЗаказНаЭмиссию_V3(ПараметрыЗакрытия, ТоварнаяГруппа, ПараметрыСУЗ, ТелоЗапросаJSON) Экспорт
	...
	URLЗапроса = СтрШаблон(
		URLЗапроса("api/v2/extension/orders/close?omsId=%1", ТоварнаяГруппа),
		ПараметрыСУЗ.Идентификатор);
...
Функция ЗапроситьКодыМаркировкиИзБизнесЗаказа_V2(ПараметрыЗапроса, КоличествоКодовМаркировки, ИдентификаторБлокаКодов, ЭлементОчереди, ПараметрыСУЗ) Экспорт
	...
	ТоварнаяГруппа = ЭлементОчереди.ТоварнаяГруппа;
	...
	Если ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ПовторноеПолучениеКодовМаркировкиИзСУЗ Тогда
		...
		Иначе
			URLЗапроса = СтрШаблон(
				URLЗапроса("api/v2/extension/codes/retry%1", ТоварнаяГруппа),
				ИнтерфейсИСМП.ПараметрыЗапроса(ПараметрыURL));
		КонецЕсли;
	Иначе
		
		Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ТоварнаяГруппа) Тогда
			ЗаголовокHTTP.Вставить("Content-Type", "application/json");
			...
			КонецЕсли;
			URLЗапроса = СтрШаблон(
				URLЗапроса("api/v2/extension/codes%1", ТоварнаяГруппа),
				ИнтерфейсИСМП.ПараметрыЗапроса(ПараметрыURL));
		Иначе
			ПараметрыURL.Добавить(СтрШаблон("gtin=%1",     ПараметрыЗапроса.GTIN));
...
Функция ЗапроситьСписокИдентификаторовПакетовБизнесЗаказа_V2(ПараметрыЗапроса, ТоварнаяГруппа, ПараметрыСУЗ) Экспорт
	...
	Иначе
		URLЗапроса = СтрШаблон(
			URLЗапроса_V2("api/v2/extension/codes/blocks?%1", ТоварнаяГруппа),
...
Функция СтатусПулаКодовМаркировкиИзБизнесЗаказа_V2(ПараметрыЗапроса, ТоварнаяГруппа, ПараметрыСУЗ) Экспорт
	...
	Иначе
		URLЗапроса = СтрШаблон(
			URLЗапроса_V2("api/v2/extension/buffer/status?omsId=%1&orderId=%2&gtin=%3", ТоварнаяГруппа),
			ПараметрыСУЗ.Идентификатор,
			ПараметрыЗапроса.ИдентификаторБизнесЗаказа,
			ПараметрыЗапроса.GTIN);
	КонецЕсли;
		...
		Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
			...
			Иначе
				...
				Если Статус = Перечисления.СтатусыПулаКодовМаркировкиСУЗ.Отклонен Тогда
					...
					Если ДанныеОбработки.Свойство("rejectionReason") Тогда
						ВозвращаемоеЗначение.ТекстОшибки = ДанныеОбработки.rejectionReason;
					Иначе
						ВозвращаемоеЗначение.ТекстОшибки = НСтр("ru = 'Заказ на эмиссию отклонен. Проверьте баланс счета в СУЗ.'");
					КонецЕсли;
...
Функция ПроверитьДоступностьСУЗ_V2(ТоварнаяГруппа, ПараметрыСУЗ) Экспорт
	...
	Иначе
		Если ТоварнаяГруппа = Неопределено Тогда
			Для Каждого ДоступнаяТоварнаяГруппа Из КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступныеТоварныеГруппы() Цикл
				Если ЗаказНаЭмиссиюСУЗКлиентСервер.ПоддерживаетсяФорматОбменаСУЗ(ПараметрыСУЗ.ФорматОбмена, ДоступнаяТоварнаяГруппа) Тогда
					ТоварнаяГруппа = ДоступнаяТоварнаяГруппа;
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		Если ТоварнаяГруппа = Неопределено Тогда
			ВозвращаемоеЗначение.ТекстОшибки = НСтр(
				"ru = 'Не найдено ни одной товарной группы по которой ведется учет маркируемой продукции'");
		КонецЕсли;
		Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ТоварнаяГруппа) Тогда
			ЗаголовокHTTP.Вставить("Content-Type", "application/json");
			URLЗапроса = СтрШаблон(
				URLЗапроса_V2("api/v3/extension/ping?omsId=%1", ТоварнаяГруппа),
				ПараметрыСУЗ.Идентификатор);
		Иначе
			URLЗапроса = СтрШаблон(
				URLЗапроса_V2("api/v2/extension/ping?omsId=%1", ТоварнаяГруппа),
				ПараметрыСУЗ.Идентификатор);
		КонецЕсли;
	КонецЕсли;
...
Функция СтатусЗаказаПоИдентификатору(ТоварнаяГруппа, ПараметрыСУЗ, ИдентификаторБизнесЗаказа) Экспорт

	ВозвращаемоеЗначение = СтатусыБизнесЗаказов_V2(ТоварнаяГруппа, ПараметрыСУЗ, ИдентификаторБизнесЗаказа);
	ВозвращаемоеЗначение.Вставить("Операция", Перечисления.ВидыОперацийИСМП.ПолучениеСтатусаПулаКодовМаркировкиИзСУЗ);
	ВозвращаемоеЗначение.Вставить("Статус",   Перечисления.СтатусыОбработкиЗаказовНаЭмиссиюКодовМаркировкиИСМП.ПустаяСсылка());
...
Функция СтатусыБизнесЗаказов_V2(ТоварнаяГруппа, ПараметрыСУЗ, ИдентификаторЗаказа = Неопределено) Экспорт
	...
	Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ТоварнаяГруппа)
	...
	Иначе
		URLЗапроса = СтрШаблон(
			URLЗапроса("api/v2/extension/orders%1", ТоварнаяГруппа),
			ИнтерфейсИСМП.ПараметрыЗапроса(ПараметрыURL));
	КонецЕсли;
...
Функция РезультатОбработкиОтчетаПоДаннымКвитанции(ИдентификаторОтчета, ТоварнаяГруппа, ИдентификаторСобытия, ПараметрыСУЗ) Экспорт
	...
	Иначе
		URLЗапроса = СтрШаблон(
			URLЗапроса_V2("api/v2/extension/receipts/document?omsId=%1&resultDocId=%2&docId=%3", ТоварнаяГруппа),
			ПараметрыСУЗ.Идентификатор,
			ИдентификаторОтчета,
			ИдентификаторСобытия);
	КонецЕсли;
				...
				Если СодержимоеДокумента <> Неопределено
					...
					Для Каждого СтрокаКодаМаркировки Из СодержимоеДокумента["cisList"] Цикл
						...
						Если ЗначениеЗаполнено(ТекстОшибки) И КодОшибки <> 0 Тогда
							...
							ДанныеРазбора = РазборКодаМаркировкиИССлужебный.РазобратьКодМаркировки(КодМаркировки, ТоварнаяГруппа);
							Если ДанныеРазбора <> Неопределено Тогда
								...
								Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(ТоварнаяГруппа) Тогда
									...
									НормализованныйКодМаркировкиБезМРЦ = РазборКодаМаркировкиИССлужебныйКлиентСервер.НормализоватьКодМаркировки(
										ДанныеРазбора, ТоварнаяГруппа, ПараметрыНормализации);
...
Функция СтатусКвитанцииДокумента_V2(ИдентификаторОтчета, ТоварнаяГруппа, ПараметрыСУЗ) Экспорт
	...
	Иначе
		URLЗапроса = СтрШаблон(
			URLЗапроса_V2("api/v2/extension/receipts/receipt?omsId=%1&resultDocId=%2", ТоварнаяГруппа),
			ПараметрыСУЗ.Идентификатор,
			ИдентификаторОтчета);
	КонецЕсли;
...
Функция СтатусОбработкиОтчета_V2(ИдентификаторОтчета, ТоварнаяГруппа, ПараметрыСУЗ, ВидОперации) Экспорт
	...
	Иначе
		Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ТоварнаяГруппа) Тогда
			URLЗапроса = СтрШаблон(
				URLЗапроса("api/v2/extension/reports?omsId=%1&reportId=%2", ТоварнаяГруппа),
				ПараметрыСУЗ.Идентификатор,
				ИдентификаторОтчета);
		Иначе
			URLЗапроса = СтрШаблон(
				URLЗапроса("api/v2/extension/report/info?omsId=%1&reportId=%2", ТоварнаяГруппа),
				ПараметрыСУЗ.Идентификатор,
				ИдентификаторОтчета);
		КонецЕсли;
	КонецЕсли;
...
Функция ЗапроситьСписокПродукции_V2(ПараметрыЗапроса, ТоварнаяГруппа, ПараметрыСУЗ) Экспорт
	...
	URLЗапроса = СтрШаблон(
		URLЗапроса_V2("api/v2/extension/product/info?omsId=%1", ТоварнаяГруппа),
		ПараметрыСУЗ.Идентификатор);
...
Функция ЗапроситьТокен_V2(ПараметрыЗапроса, ТоварнаяГруппа, ПараметрыСУЗ) Экспорт
	...
	Иначе
		URLЗапроса = СтрШаблон(
			URLЗапроса_V2("api/v2/extension/token?omsId=%1&username=%2&password=%3", ТоварнаяГруппа),
			ПараметрыСУЗ.Идентификатор,
			ПараметрыЗапроса.ИмяПользователя,
			ПараметрыЗапроса.Пароль);
	КонецЕсли;
...
Функция СписокСервисПровайдеров_V2(ТоварнаяГруппа, ПараметрыСУЗ) Экспорт
	...
	Иначе
		URLЗапроса = СтрШаблон(
			URLЗапроса_V2("api/v2/extension/providers?omsId=%1", ТоварнаяГруппа),
			ПараметрыСУЗ.Идентификатор);
	КонецЕсли;
			...
			Иначе
				...
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеОбработки, "providers") Тогда
					Для Каждого ЭлементДанных Из ДанныеОбработки.providers Цикл
						...
						Данные.Вставить("ТоварныеГруппы", Новый Массив);
						
						Для Каждого ИмяТоварнойГруппы Из ЭлементДанных.productGroups Цикл
							Если ИмяТоварнойГруппы = "pharma" Тогда
								Продолжить;
							КонецЕсли;
							Данные.ТоварныеГруппы.Добавить(
								ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ТоварнаяГруппаПоРеквизиту("Имя", ИмяТоварнойГруппы));
						КонецЦикла;
...
Процедура ТелоЗапросаЗаказНаЭмиссиюКодовМаркировки(ТелоЗапроса, ТоварнаяГруппа)
	...
	КонвертироватьТелоЗапросаФормат_V3(ТелоЗапроса, ТоварнаяГруппа, Поля);
	
КонецПроцедуры

Процедура ТелоЗапросаОтчетОВерификацииНанесенныхКМ(ТелоЗапроса, ТоварнаяГруппа)
...
Процедура ТелоЗапросаПоФорматуОбмена(ТелоЗапроса, ФорматОбмена, Операция, ТоварнаяГруппа) Экспорт
	...
	Если ФорматОбмена = Перечисления.ВерсииФорматаОбменаСУЗ.V3
		...
		Если Операция = Перечисления.ВидыОперацийИСМП.ЗаказНаЭмиссиюКодовМаркировки Тогда
			ТелоЗапросаЗаказНаЭмиссиюКодовМаркировки(ТелоЗапроса, ТоварнаяГруппа);
		ИначеЕсли Операция = Перечисления.ВидыОперацийИСМП.ОтчетОВерификацииНанесенныхКМ Тогда
			ТелоЗапросаОтчетОВерификацииНанесенныхКМ(ТелоЗапроса, ТоварнаяГруппа);
		ИначеЕсли Операция = Перечисления.ВидыОперацийИСМП.Агрегация Тогда
			ТелоЗапросаАгрегация(ТелоЗапроса, ТоварнаяГруппа);
		ИначеЕсли Операция = Перечисления.ВидыОперацийИСМП.СписаниеЭмитированныхКодовМаркировки 
			Или Операция = Перечисления.ВидыОперацийИСМП.СписаниеЧерезСУЗ Тогда
			ТелоЗапросаСписаниеЭмитированныхКодовМаркировки(ТелоЗапроса, ТоварнаяГруппа);
		КонецЕсли;
...
Функция ТоварнаяГруппаСУЗ(ТоварнаяГруппа) Экспорт
	
	Результат = КонструкторИСМПОбщегоНазначенияКлиентСервер.ИмяТоварнойГруппы(ТоварнаяГруппа);
	
	Если Результат = Неопределено Тогда
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Не удалось определить имя товарной группы: %1'"),
			ТоварнаяГруппа);
	КонецЕсли;
...
Функция URLЗапроса(URLЗапроса, ТоварнаяГруппа)
	...
	Возврат URLЗапроса_V2(НовыйURLЗапроса, ТоварнаяГруппа);

КонецФункции

Функция URLЗапроса_V2(URLЗапроса, ТоварнаяГруппа)
	
	Возврат СтрЗаменить(URLЗапроса, "extension", ТоварнаяГруппаСУЗ(ТоварнаяГруппа));
	
КонецФункции

#Область Формат_V3

Процедура ТелоЗапросаАгрегация(ТелоЗапроса, ТоварнаяГруппа)
...
Процедура ТелоЗапросаСписаниеЭмитированныхКодовМаркировки(ТелоЗапроса, ТоварнаяГруппа)
...
Процедура КонвертироватьТелоЗапросаФормат_V3(ТелоЗапроса, ТоварнаяГруппа, Поля)
	...
	ТелоЗапроса.Вставить(Поля.ПолеВидаПродукции, ТоварнаяГруппаСУЗ(ТоварнаяГруппа));
