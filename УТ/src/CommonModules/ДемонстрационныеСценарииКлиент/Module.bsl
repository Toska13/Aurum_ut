...
Процедура ПриНажатииВПолеHTMLДокумента(Форма, Элемент, ДанныеСобытия, СтандартнаяОбработка) Экспорт
	...
	Если ДанныеСобытия <> Неопределено
		...
		ПрефиксВнутренняяСсылкаДокумента = "e1c:";
		...
		ИначеЕсли СтрНачинаетсяС(ДанныеСобытия.Href, ПрефиксВнутренняяСсылкаДокумента) Тогда
			
			ПозицияЯкоря = СтрНайти(ДанныеСобытия.Href, "#");
			
			Если ПозицияЯкоря > 0 Тогда
				ТекстПредупреждения = НСтр("ru = 'Переход по ссылке внутри документа не поддерживается'");
