...
Функция ПодготовитьДанныеКПередачеПоНастройкеРегламентногоЗадания(ПараметрыОбмена) Экспорт
	...
	Если Выборка.ВидНастройкиОбмена = Перечисления.ВидыНастроекОбменаЗЕРНО.ЗагрузкаМестФормированияПартий Тогда
		...
		ПараметрыОбработкиСообщений.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.Зерно;
	...
	ИначеЕсли Выборка.ВидНастройкиОбмена = Перечисления.ВидыНастроекОбменаЗЕРНО.ЗагрузкаСДИЗ Тогда
		...
		ПараметрыОбработкиСообщений.РазделУчета = ПараметрыНастройкиОбмена.РазделУчета;
		ПараметрыОбработкиСообщений.Операци     = Перечисления.ВидыОперацийЗЕРНО.ЗапросСДИЗ;
		ПараметрыОбработкиСообщений.Ссылка      = Справочники.СДИЗЗЕРНО.ПустаяСсылка();

		Если ПараметрыНастройкиОбмена.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.Зерно Тогда
...
Процедура ПостроитьОчередьСообщений(ПараметрыОбмена, СообщениеИлиДокумент = Неопределено) Экспорт
	...
	Если ПараметрыОбмена.НастройкаРегламентногоЗадания = Неопределено
		...
		|	Очередь.РазделУчета        КАК РазделУчета,
		|
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбработатьПротоколОбменаДляПереходаНаНовуюВерсиюКонструктораНастроек(ПротоколОбмена);
		...
		ЭлементОчереди.РазделУчета        = Выборка.РазделУчета;
...
Функция ЗагрузитьИзменения(ПараметрыСинхронизации) Экспорт
	...
	Для Каждого СтрокаДатаНачалаЗагрузки Из ДатыНачалаЗагрузки Цикл
		...
		ДанныеОперации.Свойство("РазделУчета", ПараметрыОбработкиСообщений.РазделУчета);
...
//   * ДальнейшееДействие           - Неопределено, ПеречислениеСсылка.ДействиеССообщениемЗЕРНО -
Функция СтруктураСообщенияXML() Экспорт
	...
	СообщениеXML.Вставить("ВидПродукции", Неопределено); // Устарело
	СообщениеXML.Вставить("РазделУчета", Неопределено);
	...
	СообщениеXML.Вставить("ДальнейшееДействие", Неопределено);
...
Функция СтруктураСообщенияXMLПоДокументу(ДанныеДокумента, Операция, НомерВерсии) Экспорт
	...
	СообщениеXML.РазделУчета      = ДанныеДокумента.РазделУчета;
...
Функция ПараметрыФормированияСообщенияXML() Экспорт
	...
	ВозвращаемоеЗначение.Вставить("РазделУчета");
...
Функция ИмяПоляНомерСДИЗ(РазделУчета) Экспорт
	
	Если РазделУчета = Перечисления.РазделыУчетаЗЕРНО.Зерно Тогда
		Возврат "sdizNumber";
	Иначе
...
Функция ИмяПоляНомерПартии(РазделУчета) Экспорт
...
Функция ИмяПоляНомерПартииСоздать(РазделУчета) Экспорт
...
Функция ИмяПоляПричинаОтказаОтГашения(РазделУчета) Экспорт
...
Функция ИмяПоляКорректируемыйСДИЗ(РазделУчета) Экспорт
...
Функция ИмяПоляКорректирующийСДИЗ(РазделУчета) Экспорт
...
Функция ИмяПоляИдентификаторОтказаПогашения(РазделУчета) Экспорт
...
Процедура ДобавитьКЗагрузке(ПараметрыОбмена, ИмяТаблицы, Идентификатор, СсылкаНаОбъект, Организация, Подразделение, РазделУчета = Неопределено, ВидКлассификатора = Неопределено) Экспорт
	...
	Если ДанныеСсылки = Неопределено Тогда
		...
		ДанныеСсылки.Вставить("РазделУчета",       РазделУчета);
...
Функция ОбработатьОчередьСообщений(ПараметрыОбмена)
	...
	ОбновлениеИнформационнойБазы.ПроверитьОбъектОбработан(Метаданные.РегистрыСведений.ОчередьСообщенийЗЕРНО.ПолноеИмя());
	...
	Для Каждого КлючИЗначение Из ПараметрыОбмена.КОтложеннойЗагрузке Цикл
		...
		Для Каждого КлючИЗначениеИмяТаблицы Из КлючИЗначение.Значение Цикл
			...
			ИначеЕсли МенеджерОбъекта = Справочники.РеестрПартийЗЕРНО Тогда
				...
				ТаблицаИзмерений.Колонки.Добавить("РазделУчета",   Новый ОписаниеТипов("ПеречислениеСсылка.РазделыУчетаЗЕРНО"));
				...
				ТаблицаИзмерений.Индексы.Добавить("РазделУчета");
				ТаблицаИзмерений.Индексы.Добавить("РазделУчета, Подразделение");
				...
				Для Каждого КлючИЗначениеПоСсылке Из ДанныеЗагрузки Цикл
					...
					СтруктураПоиска = Новый Структура();
					СтруктураПоиска.Вставить("РазделУчета",  ПараметрыСсылки.РазделУчета);
				...
				Для Каждого СтрокаТаблицы Из ТаблицаИзмерений Цикл
					...
					ДополнительныеПараметрыЗагрузки.ПараметрыОбработкиДокумента.РазделУчета      = СтрокаТаблицы.РазделУчета;
			...
			ИначеЕсли МенеджерОбъекта = Справочники.АктыОтбораПробЗЕРНО Тогда
				...
				Для Каждого КлючИЗначениеПоСсылке Из ДанныеЗагрузки Цикл
					...
					ДополнительныеПараметрыЗагрузки.ПараметрыОбработкиДокумента.РазделУчета      = ПараметрыСсылки.РазделУчета;
					ДополнительныеПараметрыЗагрузки.ПараметрыОбработкиДокумента.ПараметрыЗапроса = ПараметрыЗапроса;
					ДополнительныеПараметрыЗагрузки.ПараметрыОбработкиДокумента.Ссылка           = МенеджерОбъекта.ПустаяСсылка();
...
Функция СообщениеКПередачеXML(СсылкаНаОбъект, ДальнейшееДействие, ДополнительныеПараметры)
	...
	Для Каждого ЭлементКоллекции Из СообщенияXML Цикл
...
Функция ИнициализироватьТаблицуПротоколОбмена() Экспорт
	...
	ПротоколОбмена.Колонки.Добавить("РазделУчета",            Новый ОписаниеТипов("ПеречислениеСсылка.РазделыУчетаЗЕРНО"));
...
Процедура ОбработатьПротоколОбменаДляПереходаНаНовуюВерсиюКонструктораНастроек(ПротоколОбмена)
	
	Если ПротоколОбмена <> Неопределено
		И ПротоколОбмена.Колонки.Найти("РазделУчета") = Неопределено Тогда 
		ПротоколОбмена.Колонки.Добавить("РазделУчета", Новый ОписаниеТипов("ПеречислениеСсылка.РазделыУчетаЗЕРНО"));
		Для Каждого СтрокаПротокола Из ПротоколОбмена Цикл
			Если ЗначениеЗаполнено(СтрокаПротокола.ВидПродукции) Тогда
				СтрокаПротокола.РазделУчета = ОбновлениеИнформационнойБазыЗЕРНО.РазделУчетаПоВидуПродукции(СтрокаПротокола.ВидПродукции);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ДобавитьВПротоколОбмена(ЭлементОчереди, ДанныеОбработки, ПараметрыОбмена, ЭтоВременнаяОшибка = Ложь) Экспорт
	...
	Если ЗаписьПротокола = Неопределено Тогда
		...
		ЗаписьПротокола.РазделУчета            = ЭлементОчереди.РазделУчета;
...
Процедура ПодготовитьСообщенияКПередаче(СообщенияXML, ПараметрыОбмена, ТребуетсяПодписать = Ложь)
	...
	Для Каждого Сообщение Из КоллекцияСообщенийXML Цикл
		...
		СообщенияПоДокументу = СообщенияПоДокументам[Сообщение.СсылкаНаОбъект];
		Если СообщенияПоДокументу = Неопределено Тогда
			СообщенияПоДокументу = Новый Массив;
			СообщенияПоДокументам.Вставить(Сообщение.СсылкаНаОбъект, СообщенияПоДокументу);
		КонецЕсли;
	...
	Для Каждого КлючИЗначение Из СообщенияПоДокументам Цикл
		...
		Для Каждого СообщениеПоДокументу Из СообщенияПоДокументу Цикл
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СообщениеПоДокументу, "ПодписываемоеТекущееДействие") 
				И СообщениеПоДокументу.ПодписываемоеТекущееДействие = Перечисления.ДействиеССообщениемЗЕРНО.ПодтверждениеПолучения Тогда
				Прервать;
			КонецЕсли;
		...
		Попытка
			...
			Если ДальнейшееДействие <> Неопределено
				...
				И ОбщегоНазначения.ЭтоДокумент(Документ.Метаданные()) Тогда
				
				ПоляБлокировки = Новый Структура;
				ПоляБлокировки.Вставить("ОбъектСинхронизации", Документ);
				
				ОбщегоНазначенияИС.УстановитьУправляемуюБлокировку(
					Метаданные.РегистрыСведений.СтатусыОбъектовСинхронизацииЗЕРНО.ПолноеИмя(), ПоляБлокировки);
				
				ТекущееСостояние = РегистрыСведений.СтатусыОбъектовСинхронизацииЗЕРНО.ТекущееСостояние(Документ);
			...
			Для Каждого Сообщение Из Сообщения Цикл
				...
				Если Не ЗначениеЗаполнено(Сообщение.ТекстОшибки) Тогда
					...
					РеквизитыОчереди.Вставить("РазделУчета", РазделУчетаПоСообщению(Сообщение));
		...
		Для Каждого Сообщение Из КоллекцияСообщенийXML Цикл
			...
			Если ЭтоПовторноеПодписание
				...
				Если Не ПодготовленоКПередаче Тогда
					...
					СтрокаРезультата.РазделУчета = РазделУчетаПоСообщению(Сообщение);
			...
			Если ПодготовленоКПередаче Тогда
				...
				ЭлементОчереди.РазделУчета         = РазделУчетаПоСообщению(Сообщение);
...
Процедура ДобавитьСообщениеВОчередьНаПередачуДанных(Сообщение, Реквизиты)
	...
	НоваяЗапись.РазделУчета        = Реквизиты.РазделУчета;
...
Функция СтруктураИзменения(ЭлементОчереди = Неопределено) Экспорт
	...
	СтруктураИзменения.Вставить("РазделУчета");
...
Функция ИмяЗапросаЗЕРНОИзНастроекОбмена(Параметры)
	...
	ИначеЕсли Параметры.Операция = Перечисления.ВидыОперацийЗЕРНО.ЗапросПартий Тогда
		Если Параметры.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна Тогда
			ИмяЗапросаЗЕРНО = Перечисления.ИмяЗапросаЗЕРНО.ПартииППЗ;
		ИначеЕсли Параметры.ЭтоЭлеватор Тогда
	...
	ИначеЕсли Параметры.Операция = Перечисления.ВидыОперацийЗЕРНО.ЗапросСДИЗ Тогда
		...
		СтруктураПоиска = Новый Структура;
		СтруктураПоиска.Вставить("РазделУчета",                    Параметры.РазделУчета);
...
Функция РазделУчетаПоСообщению(Сообщение)
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Сообщение, "РазделУчета") Тогда
		Возврат Сообщение.РазделУчета;
	ИначеЕсли ЗначениеЗаполнено(Сообщение.ВидПродукции) Тогда
		Возврат ОбновлениеИнформационнойБазыЗЕРНО.РазделУчетаПоВидуПродукции(Сообщение.ВидПродукции);
	Иначе
		Возврат Перечисления.РазделыУчетаЗЕРНО.ПустаяСсылка();
	КонецЕсли;
...
Функция СформироватьНовыйКонвертСообщения(ЭлементОчереди, ПараметрыОбмена, ИдентификаторЗаявки = Неопределено)
	...
	КоллекцияПараметров = ИнтеграцияЗЕРНОКлиентСерверПовтИсп.ПараметрыЦиклаСообщения(ЭлементОчереди.ТекущееДействие);
	
	Для Каждого ПараметрыСообщения Из КоллекцияПараметров Цикл
...
Функция СоответствиеОперацийИмяЗапроса()
	...
	СтруктураЗапроса.Вставить("РазделУчета", Перечисления.РазделыУчетаЗЕРНО.Зерно);
	...
	СтруктураЗапроса = СтруктураЗапросаЗерно(Справочники.РеестрПартийЗЕРНО.ПустаяСсылка(),
		Перечисления.ВидыОперацийЗЕРНО.ЗапросПартий);
	СтруктураЗапроса.Вставить("РазделУчета", Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна);
	...
	//ППЗ
	РазделУчета = Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна;
...
Функция ТаблицаПоискаИмяЗапросаСДИЗ()
	...
	Таблица.Колонки.Добавить("РазделУчета");
	...
	РазделУчета = Перечисления.РазделыУчетаЗЕРНО.Зерно;
	ЭтоЭлеватор = Ложь;
	ДобавитьНастройкуИмяЗапросаЗерно(Таблица, Перечисления.ИмяЗапросаЗЕРНО.СДИЗЗерно, РазделУчета,,ЭтоЭлеватор);
	ДобавитьНастройкуИмяЗапросаЗерно(Таблица, Перечисления.ИмяЗапросаЗЕРНО.СДИЗЗерноПогашение, РазделУчета, Истина,,ЭтоЭлеватор);
	ДобавитьНастройкуИмяЗапросаЗерно(Таблица, Перечисления.ИмяЗапросаЗЕРНО.СДИЗЗерноОтказОтПогашения, РазделУчета, Истина, Истина,ЭтоЭлеватор);
	
	ЭтоЭлеватор = Истина;
	ДобавитьНастройкуИмяЗапросаЗерно(Таблица, Перечисления.ИмяЗапросаЗЕРНО.СДИЗЗерноЭлеватор, РазделУчета,,ЭтоЭлеватор);
	ДобавитьНастройкуИмяЗапросаЗерно(Таблица, Перечисления.ИмяЗапросаЗЕРНО.СДИЗЗерноЭлеваторПогашение, РазделУчета, Истина,,ЭтоЭлеватор);
	ДобавитьНастройкуИмяЗапросаЗерно(Таблица, Перечисления.ИмяЗапросаЗЕРНО.СДИЗЗерноЭлеваторОтказОтПогашения, РазделУчета, Истина, ЭтоЭлеватор);
	
	РазделУчета = Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна;
	ДобавитьНастройкуИмяЗапросаЗерно(Таблица, Перечисления.ИмяЗапросаЗЕРНО.СДИЗППЗ, РазделУчета);
	ДобавитьНастройкуИмяЗапросаЗерно(Таблица, Перечисления.ИмяЗапросаЗЕРНО.СДИЗППЗПогашение, РазделУчета, Истина);
	ДобавитьНастройкуИмяЗапросаЗерно(Таблица, Перечисления.ИмяЗапросаЗЕРНО.СДИЗППЗОтказОтПогашения, РазделУчета, Истина, Истина);
	
	Возврат Таблица;
	
КонецФункции

Процедура ДобавитьНастройкуИмяЗапросаЗерно(Таблица, ИмяЗапросаЗЕРНО, РазделУчета, Погашения = Ложь, ОтказыОтПогашений = Ложь, ЭтоЭлеватор = Ложь)
