...
//  ВключатьМногооборотнуюТару	- Булево - признак включения в отбор многооборотнрй тары.
// 
// Возвращаемое значение:
//  Массив - состоит из элементов типа ПеречислениеСсылка.ТипыНоменклатуры.
//
Функция ОтборПоТоваруМногооборотнойТаре(ВключатьНабор = Истина, ВключатьМногооборотнуюТару = Истина) Экспорт
	...
	Если ВключатьМногооборотнуюТару Тогда
...
//		* ИмяПоляКонтролируемаяПартия - Неопределено,Строка - имя реквизита связанного с контролируемой партией,
//		                                Неопределено, если такого реквизита нет. Если реквизит в табличной части, то перед его именем ставится ".".
...
//		* ПараметрыПолученияДанныхИзРегистров - Структура - содержит дополнительные параметры запроса, используемые
//								в функции ТекстЗапросаФормированияТаблицыДанныеРегистров.
//
...
Функция ГоденДоПоДатеПроизводства(ДатаПроизводства, СрокГодности, ЕдиницаИзмеренияСрокаГодности) Экспорт
	...
	Если ЕдиницаИзмеренияСрокаГодности = ПредопределенноеЗначение("Перечисление.ЕдиницыИзмеренияВремени.Год") Тогда
		ГоденДо = ДобавитьМесяц(ДатаПроизводства, СрокГодности * 12);
...
Процедура ПересчитатьСтатусУказанияСерийПриОбработке(ПараметрыУказанияСерий, ТекущийСтатус, СерииУказаныПолностью, КоличествоСерий, ВариантОбеспечения = Неопределено, ТекущиеДанные = Неопределено) Экспорт
	...
	ПрименятьСтатусМожноУказать = (ПараметрыУказанияСерий.ПодготовкаОрдера
									...
									Или ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.СчетФактураВыданный"
