...
Процедура ИсправитьПроблемыВДвиженияхИДокументах(ПараметрыРасчета) Экспорт
	...
	РасшифровкаКодовОшибок = Новый Соответствие;
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru = 'Лишние движения по регистру ""Партии прочих расходов"" при УСН.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	Т.Ссылка КАК Организация
	|ПОМЕСТИТЬ ВтОрганизацииНаУСН
	|ИЗ
	|	Справочник.Организации КАК Т
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТУчетныеПолитикиОрганизаций КАК ОрганизацииНаУСН
	|		ПО Т.Ссылка = ОрганизацииНаУСН.Организация
	|		И ОрганизацииНаУСН.СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.Упрощенная)
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТНастройкиУчетаУСН КАК НастройкиУчетаУСН
	|		ПО Т.Ссылка = НастройкиУчетаУСН.Организация
	|		И НЕ НастройкиУчетаУСН.УчетнаяПолитикаСуществует
	|ГДЕ
	...
	|ИЗ
	|	РегистрНакопления.ПартииПрочихРасходов КАК Т
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтОрганизацииНаУСН КАК ОрганизацииНаУСН
	|		ПО Т.Организация = ОрганизацииНаУСН.Организация
	|ГДЕ
	|	Т.Период МЕЖДУ &НачалоПериода И &КонецПериода
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументыПоОтдельнымРегистрамНакопления(
		...
		Запрос,
		РасшифровкаКодовОшибок,
		НСтр("ru = 'ошибки в движениях по регистру ""Партии прочих расходов""'", ОбщегоНазначения.КодОсновногоЯзыка()));
