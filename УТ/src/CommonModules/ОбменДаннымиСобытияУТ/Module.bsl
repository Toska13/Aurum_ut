...
// Документ "Сторно" событие "Перед обработкой ПРО"
// 
// Параметры:
//  Объект - ДокументОбъект.Сторно - Регистрируемый объект
//  Отказ - Булево - Флаг, указывающий на то, необходимо ли отказаться от выгрузки объекта
//
Процедура ДокументСторноПередОбработкойПРО(Объект, Отказ) Экспорт
	
	Отказ = Истина;
	
	НеподдерживаемыеВидыДокументов = Новый Массив;
	
	НеподдерживаемыеВидыДокументов.Добавить(Тип("ДокументСсылка.ПередачаТоваровМеждуОрганизациями"));
	НеподдерживаемыеВидыДокументов.Добавить(Тип("ДокументСсылка.ВозвратТоваровМеждуОрганизациями"));
	НеподдерживаемыеВидыДокументов.Добавить(Тип("ДокументСсылка.ОтчетПоКомиссииМеждуОрганизациями"));
	НеподдерживаемыеВидыДокументов.Добавить(Тип("ДокументСсылка.ОтчетПоКомиссииМеждуОрганизациямиОСписании"));
	НеподдерживаемыеВидыДокументов.Добавить(Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту"));
	
	СоставПланаОбмена = Метаданные.ПланыОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат.Состав;
	
	Если ОбщегоНазначения.СсылкаСуществует(Объект.СторнируемыйДокумент) 
		И НеподдерживаемыеВидыДокументов.Найти(ТипЗнч(Объект.СторнируемыйДокумент)) = Неопределено
		И СоставПланаОбмена.Найти(Объект.СторнируемыйДокумент.Метаданные()) <> Неопределено Тогда
