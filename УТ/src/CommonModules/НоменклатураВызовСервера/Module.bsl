...
Процедура НоменклатураОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	НоменклатураЛокализация.ОбработкаПолученияДанныхВыбора(Параметры, Запрос, ТекстЗапроса);
	...
	Если Параметры.Свойство("ВыборТовараДругогоКачества")
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьОбобщенныйУчетНекачественныхТоваров") Тогда
			...
			Если РеквизитыНоменклатуры.ИспользоватьСерии = Истина
	...
	Если Параметры.Свойство("ВариантыРасчетаЦеныНабора", ВариантыРасчетаЦеныНабора) Тогда
		
		Запрос.УстановитьПараметр("ВариантыРасчетаЦеныНабора", ВариантыРасчетаЦеныНабора);
		
		УсловиеОтбора = УсловиеОтбора +
			Символы.ПС +
			"	И ВЫБОР
			|		КОГДА СпрНоменклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Набор)
			|			ТОГДА СпрНоменклатура.Ссылка НЕ В
			|			(ВЫБРАТЬ
			...
			|				И Наборы.Основной
			|				И Наборы.ВариантРасчетаЦеныНабора НЕ В (&ВариантыРасчетаЦеныНабора))
			|		ИНАЧЕ ИСТИНА
			|	КОНЕЦ";
...
Процедура ВидыНоменклатурыОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
    ...
    Если Параметры.Свойство("Номенклатура")
		...
		УсловияПоиска = "НовыйВидНоменклатуры.Наименование ПОДОБНО &СтрокаПоиска СПЕЦСИМВОЛ ""~""";
		...
		Для НомерДопЯзыка = 1 По МультиязычностьСервер.КоличествоДополнительныхЯзыков() Цикл
			...
			Если МультиязычностьСервер.ИспользуетсяДополнительныйЯзык(НомерДопЯзыка) Тогда
				...
				|	ИЛИ " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("НовыйВидНоменклатуры.%1 ПОДОБНО &СтрокаПоиска СПЕЦСИМВОЛ ""~""",
	...
	Иначе
		...
		Для Каждого Отбор Из Параметры.Отбор Цикл
			...
			Если ТипЗнч(Отбор.Значение) = Тип("ФиксированныйМассив")
				Или ТипЗнч(Отбор.Значение) = Тип("Массив") Тогда
				
				СтрокаОтбора = "НовыйВидНоменклатуры.%1 В (&%2)"
				
			Иначе
				
				СтрокаОтбора = "НовыйВидНоменклатуры.%1 = &%2"
				
			КонецЕсли;
			
			УсловияОтбора = УсловияОтбора + "
			|	И " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаОтбора, Отбор.Ключ, Отбор.Ключ);
