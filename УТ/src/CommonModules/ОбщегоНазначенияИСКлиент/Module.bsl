...
Процедура ПослеПериодическогоПолученияДанныхКлиентаНаСервере(Результаты) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ГосИС.ЗЕРНО") Тогда
		МодульИнтеграцияЗЕРНОКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ИнтеграцияЗЕРНОКлиент");
		МодульИнтеграцияЗЕРНОКлиент.ПослеПериодическогоПолученияДанныхКлиентаНаСервере(Результаты);
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ГосИС.ЕГАИС") Тогда
		МодульОбщегоНазначенияЕГАИСКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ОбщегоНазначенияЕГАИСКлиент");
		МодульОбщегоНазначенияЕГАИСКлиент.ПослеПериодическогоПолученияДанныхКлиентаНаСервере(Результаты);
	КонецЕсли;
...
Процедура ОбработкаВнешнегоСобытия(Источник, Событие, Данные) Экспорт
	...
	Если ПараметрыПриложения["ПодключаемоеОборудованиеСобытиеОбработаноДанныеИС"] = Неопределено Тогда
		Возврат;
	ИначеЕсли ПараметрыПриложения["ПодключаемоеОборудованиеСобытиеОбработаноДанныеИС"][Данные] <> Неопределено Тогда
...
Процедура ВыделитьТекущуюОбласть(ТабличныйДокумент, ДанныеВыделеннойОбласти) Экспорт
	
	Если ДанныеВыделеннойОбласти <> Неопределено
		И ТабличныйДокумент.ТекущаяОбласть.Верх = ДанныеВыделеннойОбласти.Верх
		И ТабличныйДокумент.ТекущаяОбласть.Низ = ДанныеВыделеннойОбласти.Низ
		И ТабличныйДокумент.ТекущаяОбласть.Лево = ДанныеВыделеннойОбласти.Лево
		И ТабличныйДокумент.ТекущаяОбласть.Право = ДанныеВыделеннойОбласти.Право Тогда
		Возврат;
	КонецЕсли;
	
	СнятьВыделениеВыделеннойОбласти(ТабличныйДокумент, ДанныеВыделеннойОбласти);
	УстановитьВыделениеВыделеннойОбласти(ТабличныйДокумент, ДанныеВыделеннойОбласти);
...
Процедура СнятьВыделениеВыделеннойОбласти(ТабличныйДокумент, ДанныеВыделеннойОбласти)
	
	Если ДанныеВыделеннойОбласти = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	КрайВерх = Макс(ТабличныйДокумент.ФиксацияСверху - 1, 0);
	КрайНиз = ТабличныйДокумент.ВысотаТаблицы;
	КрайЛево = 1;
	КрайПраво = ТабличныйДокумент.ШиринаТаблицы;
	
	Область = ТабличныйДокумент.Область(ДанныеВыделеннойОбласти.Верх, КрайЛево, ДанныеВыделеннойОбласти.Верх, КрайПраво);
	Область.ГраницаСверху = ДанныеВыделеннойОбласти.ГраницаСверху;
	
	Область = ТабличныйДокумент.Область(ДанныеВыделеннойОбласти.Низ, КрайЛево, ДанныеВыделеннойОбласти.Низ, КрайПраво);
	Область.ГраницаСнизу = ДанныеВыделеннойОбласти.ГраницаСнизу;
	
	Область = ТабличныйДокумент.Область(КрайВерх, ДанныеВыделеннойОбласти.Лево, КрайНиз, ДанныеВыделеннойОбласти.Лево);
	Область.ГраницаСлева = ДанныеВыделеннойОбласти.ГраницаСлева;
	
	Область = ТабличныйДокумент.Область(КрайВерх, ДанныеВыделеннойОбласти.Право, КрайНиз, ДанныеВыделеннойОбласти.Право);
	Область.ГраницаСправа = ДанныеВыделеннойОбласти.ГраницаСправа;
	
КонецПроцедуры

Процедура УстановитьВыделениеВыделеннойОбласти(ТабличныйДокумент, ДанныеВыделеннойОбласти)
	
	ТекущаяОбласть = ТабличныйДокумент.ТекущаяОбласть;
	
	Если ТекущаяОбласть = Неопределено
		Или ТекущаяОбласть.ТипОбласти <> ТипОбластиЯчеекТабличногоДокумента.Прямоугольник
		Или ТекущаяОбласть.Верх = 0
		Или ТекущаяОбласть.Низ = 0 Тогда
	...
	Если ТекущаяОбласть.Верх > КрайНиз Тогда
		Верх = 0;
	Иначе
		Верх = Макс(ТекущаяОбласть.Верх, КрайВерх);
	КонецЕсли;
	
	Если ТекущаяОбласть.Низ < КрайВерх Тогда
		Низ = 0;
	Иначе
		Низ = Мин(ТекущаяОбласть.Низ, КрайНиз);
	КонецЕсли;
	
	Если ТекущаяОбласть.Лево > КрайПраво Тогда
		Лево = 0;
	Иначе
		Лево = Макс(ТекущаяОбласть.Лево, КрайЛево);
	КонецЕсли;
	
	Если ТекущаяОбласть.Право < КрайЛево Тогда
		Право = 0;
	Иначе
		Право = Мин(ТекущаяОбласть.Право, КрайПраво);
	КонецЕсли;
	
	Если Верх = 0 Или Низ = 0 Или Лево = 0 Или Право = 0 Тогда
		ДанныеВыделеннойОбласти = Неопределено;
		Возврат;
	КонецЕсли;
	
	ОбластьВыделяемая = ТабличныйДокумент.Область(Верх, Лево, Низ, Право);
	
	ДанныеВыделеннойОбласти = Новый Структура;
	ДанныеВыделеннойОбласти.Вставить("Верх",  Верх);
	ДанныеВыделеннойОбласти.Вставить("Низ",   Низ);
	ДанныеВыделеннойОбласти.Вставить("Лево",  Лево);
	ДанныеВыделеннойОбласти.Вставить("Право", Право);
	ДанныеВыделеннойОбласти.Вставить("ГраницаСверху", ОбластьВыделяемая.ГраницаСверху);
	ДанныеВыделеннойОбласти.Вставить("ГраницаСнизу",  ОбластьВыделяемая.ГраницаСнизу);
	ДанныеВыделеннойОбласти.Вставить("ГраницаСлева",  ОбластьВыделяемая.ГраницаСлева);
	ДанныеВыделеннойОбласти.Вставить("ГраницаСправа", ОбластьВыделяемая.ГраницаСправа);
	
	НоваяГраница = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 3);
	
	Область = ТабличныйДокумент.Область(Верх, КрайЛево, Верх, КрайПраво);
	Область.ГраницаСверху = НоваяГраница;
	
	Область = ТабличныйДокумент.Область(Низ, КрайЛево, Низ, КрайПраво);
	Область.ГраницаСнизу = НоваяГраница;
	
	Область = ТабличныйДокумент.Область(КрайВерх, Лево, КрайНиз, Лево);
	Область.ГраницаСлева = НоваяГраница;
	
	Область = ТабличныйДокумент.Область(КрайВерх, Право, КрайНиз, Право);
	Область.ГраницаСправа = НоваяГраница;
