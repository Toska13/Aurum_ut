...
Процедура НадписьЭтапыОплатыНажатие(Форма, Элемент, СтандартнаяОбработка, Оповещение = Неопределено) Экспорт
	...
	ПараметрыОткрытия.Вставить("Контрагент",                              ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Форма, СтруктураПараметров.Контрагент));
	ПараметрыОткрытия.Вставить("БанковскийСчетКонтрагента",               ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Форма, СтруктураПараметров.БанковскийСчетКонтрагента));
	ПараметрыОткрытия.Вставить("ТСП",                                     ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Форма, СтруктураПараметров.ТСП));
	...
	ПараметрыОткрытия.Вставить("Календарь",                               ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Форма, СтруктураПараметров.Календарь));
	ПараметрыОткрытия.Вставить("СрокПереходаПраваСобственности",          ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Форма, СтруктураПараметров.СрокПереходаПраваСобственности));
	...
	ПараметрыОткрытия.Вставить("ЭтоПереходПраваСобственности2_5",         СтруктураПараметров.ЭтоПереходПраваСобственности2_5);
...
Процедура НадписьЭтапыОплатыНажатиеЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗакрытия.Свойство("ИдентификаторПлатежа") Тогда
		МассивПолей.Добавить("ИдентификаторПлатежа");
	КонецЕсли;
...
Процедура ПодборВРасшифровкуПлатежа(Форма, ИмяЭлемента, Оповещение = Неопределено, ДополнительныеОтборы = Неопределено) Экспорт 
	...
	ОплатаПоЗаявкам      = ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Форма, СтруктураПараметров.ОплатаПоЗаявкам, , Ложь);
	...
	ПараметрыОткрытия.Вставить("УчитыватьФилиалы",                      СтруктураПараметров.УчитыватьФилиалы);
	...
	ПараметрыОткрытия.Вставить("ПодборВОплатуПоЗаявкам",                ОплатаПоЗаявкам);
...
Процедура ОбъектРасчетовНачалоВыбора(Форма, Элемент, СтандартнаяОбработка, ВыборОснованияПлатежа = ЛОЖЬ, ТекущееЗначение = Неопределено, ДополнительныеОтборы = Неопределено) Экспорт
	...
	НастройкиВыбора.Вставить("УчитыватьФилиалы", СтруктураПараметров.УчитыватьФилиалы);
...
Процедура ВалютыИКурсДокументаНажатие(Форма, Элемент, СтандартнаяОбработка, Оповещение = Неопределено) Экспорт
	...
	// Для платежек валюта взаиморасчетов в табличной части Расшифровка платежа
	ТабличнаяЧасть = ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Форма,
		СтруктураПараметров.ПутьКДаннымТЧРасшифровкаПлатежа);
	Если ТабличнаяЧасть <> Неопределено И СтруктураПараметров.ЭтоПлатежИлиПрочийДокумент И СтруктураПараметров.ВалютаДокументаТолькоПросмотр
		И СтруктураПараметров.ВалютаВзаиморасчетовТолькоПросмотр И ТабличнаяЧасть.Количество() > 0 Тогда
		ИдентификаторСтроки = ТабличнаяЧасть[0].ПолучитьИдентификатор();
		ПараметрыОткрытия.Вставить("ВалютаВзаиморасчетов", ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Форма, СтруктураПараметров.ВалютаВзаиморасчетов, ИдентификаторСтроки));
		ПараметрыОткрытия.Вставить("ВариантКурсаДоговора", ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Форма, "Объект.РасшифровкаПлатежа.ВариантКурсаДоговора", ИдентификаторСтроки));
		ПараметрыОткрытия.Вставить("Договор", ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Форма, "Объект.РасшифровкаПлатежа.Договор", ИдентификаторСтроки));
	Иначе
	...
	ПараметрыОткрытия.Вставить("СуммаДокументаТолькоПросмотр",       СтруктураПараметров.СуммаДокументаТолькоПросмотр);
...
Процедура ВалютыИКурсДокументаНажатиеЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗакрытия.НеобходимПересчетСуммДокумента Тогда
		МассивПолей.Добавить("СуммаДокумента");
	КонецЕсли;
	...
	// Для платежек необходимо обработать табличную часть Расшифровка платежа
	ТабличнаяЧасть = ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Форма, СтруктураПараметров.ПутьКДаннымТЧРасшифровкаПлатежа);
	Если ТабличнаяЧасть <> Неопределено
		И СтруктураПараметров.ЭтоПлатежИлиПрочийДокумент
		И СтруктураПараметров.ВалютаДокументаТолькоПросмотр
		И СтруктураПараметров.ВалютаВзаиморасчетовТолькоПросмотр Тогда
		
		Если СтарыеЗначенияИзмененныхРеквизитов.Свойство("КурсЧислительВзаиморасчетов") Тогда
			Для Каждого СтрокаРасшифровки Из ТабличнаяЧасть Цикл
				СтрокаРасшифровки.КурсЧислительВзаиморасчетов = РезультатЗакрытия.КурсЧислитель;
			КонецЦикла;
		КонецЕсли;
		
		Если СтарыеЗначенияИзмененныхРеквизитов.Свойство("КурсЗнаменательВзаиморасчетов") Тогда
			Для Каждого СтрокаРасшифровки Из ТабличнаяЧасть Цикл
				СтрокаРасшифровки.КурсЗнаменательВзаиморасчетов = РезультатЗакрытия.КурсЗнаменатель;
			КонецЦикла;
		КонецЕсли;
		
		Если (СтарыеЗначенияИзмененныхРеквизитов.Свойство("КурсЧислительВзаиморасчетов")
			Или СтарыеЗначенияИзмененныхРеквизитов.Свойство("КурсЗнаменательВзаиморасчетов"))
			И СтарыеЗначенияИзмененныхРеквизитов.Свойство("СуммаДокумента") Тогда
			Для Каждого СтрокаРасшифровки Из ТабличнаяЧасть Цикл
				ДенежныеСредстваКлиентСервер.РассчитатьСуммуВСтрокеРасшифровки(СтрокаРасшифровки, РезультатЗакрытия.Валюта, Форма.ВалютаРеглУчета);
			КонецЦикла;
		КонецЕсли;
		
		Если СтарыеЗначенияИзмененныхРеквизитов.Свойство("СуммаДокумента") Тогда
			ФинансыКлиентСервер.ПересчитатьСуммыВСтрокеРасшифровкиПлатежа(Форма.Объект, РезультатЗакрытия.СуммаДокумента, Ложь);
		КонецЕсли;
		
		Если СтарыеЗначенияИзмененныхРеквизитов.Свойство("СуммаДокумента")
			И Не СтарыеЗначенияИзмененныхРеквизитов.Свойство("СуммаВзаиморасчетов") Тогда
			
			Для Каждого СтрокаТЧ Из ТабличнаяЧасть Цикл
				ДенежныеСредстваКлиентСервер.РассчитатьСуммуВзаиморасчетовВСтрокеРасшифровки(
					СтрокаТЧ, РезультатЗакрытия.Валюта, Форма.ВалютаРеглУчета);
			КонецЦикла;
		КонецЕсли;
		
		Если СтарыеЗначенияИзмененныхРеквизитов.Свойство("СуммаВзаиморасчетов")
			Или ТабличнаяЧасть.Итог("СуммаВзаиморасчетов") <> РезультатЗакрытия.СуммаВзаиморасчетов Тогда
			
			Если ТабличнаяЧасть.Количество() = 1 Тогда
				ТабличнаяЧасть[0].СуммаВзаиморасчетов = РезультатЗакрытия.СуммаВзаиморасчетов;
			Иначе
				Для Каждого СтрокаТЧ Из ТабличнаяЧасть Цикл
					ДенежныеСредстваКлиентСервер.РассчитатьСуммуВзаиморасчетовВСтрокеРасшифровки(
						СтрокаТЧ, РезультатЗакрытия.Валюта, Форма.ВалютаРеглУчета);
				...
				Если ТабличнаяЧасть.Итог("СуммаВзаиморасчетов") <> РезультатЗакрытия.СуммаВзаиморасчетов Тогда
					
					СуммаРазницы = ТабличнаяЧасть.Итог("СуммаВзаиморасчетов") - РезультатЗакрытия.СуммаВзаиморасчетов;
					НомерСтроки = ТабличнаяЧасть.Количество() - 1;
					
					Пока НомерСтроки >= 0 Цикл
						СтрокаРасшифровки = ТабличнаяЧасть[НомерСтроки];
						...
						ИначеЕсли СтрокаРасшифровки.СуммаВзаиморасчетов > СуммаРазницы Тогда
							...
							Если СтрокаРасшифровки.ВалютаВзаиморасчетов = РезультатЗакрытия.Валюта Тогда
	...
	РезультатыОбработки.Вставить("СуммаДокумента", РезультатЗакрытия.СуммаДокумента);
...
#Область НесколькоДатОтгрузки

// При редактировании табличной части заказа определяет, что изменились даты отгрузки 
// и необходим вызов ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов.
// 
// Параметры:
// 	Форма - ФормаКлиентскогоПриложения - Форма документа/справочника.
// 
// Возвращаемое значение:
//  Булево - Обнаружено изменение дат отгрузок
Функция ЕстьИзменениеДатОтгрузок(Форма) Экспорт
	
	ЕстьИзменениеДатОтгрузок = Ложь;
	
	ДополненныеПараметрыМеханизма = ОбщегоНазначенияУТКлиентСервер.ПолучитьДанныеМеханизмаИзКэшаФормы(Форма, "Взаиморасчеты");
	Для Каждого СтруктураПараметров Из ДополненныеПараметрыМеханизма.МассивПараметров Цикл
		ДатыОтгрузки = ВзаиморасчетыКлиентСервер.МассивДатОтгрузок(Форма, СтруктураПараметров);
		Если Не ОбщегоНазначенияКлиентСервер.СпискиЗначенийИдентичны(СтруктураПараметров.ДатыОтгрузки, ДатыОтгрузки, Истина) Тогда
			ЕстьИзменениеДатОтгрузок = Истина;
		КонецЕсли;
	КонецЦикла;
	
	Возврат ЕстьИзменениеДатОтгрузок;
	
КонецФункции

// Кэширует в параметрах механизма текущие даты отгрузки заказа
// 
// Параметры:
// 	Форма - ФормаКлиентскогоПриложения - Форма документа/справочника.
Процедура СохранитьДатыОтгрузокВПараметрах(Форма) Экспорт
	ДополненныеПараметрыМеханизма = ОбщегоНазначенияУТКлиентСервер.ПолучитьДанныеМеханизмаИзКэшаФормы(Форма, "Взаиморасчеты");
	Для Каждого СтруктураПараметров Из ДополненныеПараметрыМеханизма.МассивПараметров Цикл
		СтруктураПараметров.ДатыОтгрузки = ВзаиморасчетыКлиентСервер.МассивДатОтгрузок(Форма, СтруктураПараметров);
	КонецЦикла;
КонецПроцедуры

// Сбрасывает флаг, по которому вызывается вопрос о перезаполнении графика при изменении дат отгрузок
// 
// Параметры:
// 	Форма - ФормаКлиентскогоПриложения - Форма документа/справочника.
Процедура ВопросОПерезаполненииГрафикаПриИзмененииДатОтгрузокЗадан(Форма) Экспорт
	ДополненныеПараметрыМеханизма = ОбщегоНазначенияУТКлиентСервер.ПолучитьДанныеМеханизмаИзКэшаФормы(Форма, "Взаиморасчеты");
	Для Каждого СтруктураПараметров Из ДополненныеПараметрыМеханизма.МассивПараметров Цикл
