...
Процедура ПредложенияИсточниковПоЗапросуКоммерческихПредложений(Знач СсылкаНаОбъект, Знач АнализируемыеИсточники, НайденныеПредложения) Экспорт
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(СоглашенияСПоставщиками.СрокОтгрузки + СоглашенияСПоставщиками.СрокДоставки, 0) КАК СрокПоставки,
		...
		|		ИНАЧЕ &ЦеныНоменклатурыПоставщиков
		|	КОНЕЦ,
		|	ЕСТЬNULL(СоглашенияСПоставщиками.СрокОтгрузки + СоглашенияСПоставщиками.СрокДоставки, 0),
		...
		|	ГотоваяВыборка.ДокументОснование КАК ДокументОснование,
		|	ГотоваяВыборка.Регистратор КАК Регистратор,
		|	ГотоваяВыборка.НоменклатураПартнера КАК НоменклатураПоставщика,
...
// Возвращает флаг использования функционала запросов коммерческих предложений.
// 
// Возвращаемое значение:
//  Булево
Функция ИспользоватьЗапросыКоммерческихПредложенийПоставщиков() Экспорт
	
	ВозвращаемоеЗначение = Ложь;
	
	//++ Локализация
	ВозвращаемоеЗначение = ОбщегоНазначения.ПодсистемаСуществует(
...
// Заполняет условия закупки в таблице по коммерческим предложениям:
// 
// Параметры:
//  Товары - ТаблицаЗначений, ДанныеФормыКоллекция - В которой необходимо заполнить цены и их источник (партнер, контрагент, ставка НДС).
//  ПараметрыЗаполнения - См. КоммерческиеПредложенияДокументыУТ.ПараметрыЗаполненияУсловийЗакупкиПоКоммерческимПредложениям
// 
// Возвращаемое значение:
//  Структура:
// * Отказ - Булево - Если взведен, то цены невозможно заполнить, причина ошибки указана в "ТекстОшибки".
// * ТекстОшибки - Строка - Заполняется, если взведен флаг "Отказ". Информация о причине невозможности выполнить заполнение цен.
// * НомераСтрокСНезаполненнымиУсловиямиЗакупки - Массив Из Число - номера строк, для которых отсутствуют условия закупки.
// * ИзмененныеСтроки - Массив Из СтрокаТаблицыЗначений, ДанныеФормыЭлементКоллекции - Измененные (и добавленные) строки табличной части.
Функция ЗаполнитьУсловияЗакупкиПоКоммерческимПредложениям(Товары, ПараметрыЗаполнения) Экспорт
	
	ВозвращаемоеЗначение = Новый Структура;
	ВозвращаемоеЗначение.Вставить("Отказ", Истина);
	ВозвращаемоеЗначение.Вставить("ТекстОшибки", НСтр("ru = 'Функционал отсутствует в системе.'"));
	ВозвращаемоеЗначение.Вставить("НомераСтрокСНезаполненнымиУсловиямиЗакупки", Новый Массив);
	ВозвращаемоеЗначение.Вставить("ИзмененныеСтроки", Новый Массив);
	
	//++ Локализация
	
	ВозвращаемоеЗначение.Отказ = Ложь;
	ВозвращаемоеЗначение.ТекстОшибки = "";
	
	ПараметрыПолучения = ПараметрыПолученияВыбранныхУсловийЗакупки();
	ПараметрыПолучения.Документ = ПараметрыЗаполнения.Документ;
	ПараметрыПолучения.ДатаКурса = ПараметрыЗаполнения.ДатаКурса;
	ПараметрыПолучения.Валюта = ПараметрыЗаполнения.Валюта;
	ПараметрыПолучения.ЦенаВключаетНДС = ПараметрыЗаполнения.ЦенаВключаетНДС;
	
	ВыбранныеУсловияЗакупки = ВыбранныеУсловияЗакупки(ПараметрыПолучения);
	
	Если ВыбранныеУсловияЗакупки = Неопределено Тогда
		
		ВозвращаемоеЗначение.Отказ = Истина;
		ВозвращаемоеЗначение.ТекстОшибки = НСтр("ru = 'Отсутствуют созданные на основании запросы коммерческих предложений с выбранными предложениями'");
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ТоварыПриемника.НомерСтроки КАК НомерСтроки,
	|	ТоварыПриемника.НоменклатураСтрокой КАК НоменклатураСтрокой,
	|	ТоварыПриемника.Номенклатура КАК Номенклатура,
	|	ТоварыПриемника.Характеристика КАК Характеристика,
	|	ТоварыПриемника.Упаковка КАК Упаковка,
	|	ТоварыПриемника.Партнер КАК Партнер,
	|	ТоварыПриемника.Контрагент КАК Контрагент
	|ПОМЕСТИТЬ ТоварыПриемника
	|ИЗ
	|	&ТоварыПриемника КАК ТоварыПриемника
	|
	...
	|	ТоварыПриемника КАК ТоварыПриемника
	|ГДЕ
	|	ТоварыПриемника.НомерСтроки В(&ОтборСтрок)
	|
	|ИНДЕКСИРОВАТЬ ПО НАБОРАМ
	|(
	|	(Номенклатура,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТоварыПредложений.ИндексСтроки + 1 КАК НомерСтроки,
	|	ТоварыПредложений.НоменклатураТекстом КАК НоменклатураСтрокой,
	|	ТоварыПредложений.Номенклатура КАК Номенклатура,
	|	ТоварыПредложений.Характеристика КАК Характеристика,
	|	ТоварыПредложений.Упаковка КАК Упаковка,
	|	ТоварыПредложений.Цена КАК Цена,
	|	ТоварыПредложений.Партнер КАК Партнер,
	|	ТоварыПредложений.Контрагент КАК Контрагент
	|ПОМЕСТИТЬ ТоварыПредложений
	|ИЗ
	|	&ТоварыПредложений КАК ТоварыПредложений
	|
	|ИНДЕКСИРОВАТЬ ПО НАБОРАМ
	|(
	|	(Номенклатура,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТоварыПриемника.НомерСтроки КАК НомерСтрокиПриемника,
	|	ЕСТЬNULL(ТоварыПредложений.НомерСтроки, 0) КАК НомерСтрокиПредложений,
	|	&Цена КАК Цена,
	|	ТоварыПриемника.Партнер = ТоварыПредложений.Партнер КАК ПартнерСовпадает,
	|	ТоварыПриемника.Контрагент = ТоварыПредложений.Партнер КАК КонтрагентСовпадает
	|ПОМЕСТИТЬ ДоступныеЦены
	|ИЗ
	|	ТоварыПриемника КАК ТоварыПриемника
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОтборСтрок КАК ОтборСтрок
	|		ПО ТоварыПриемника.Номенклатура = ОтборСтрок.Номенклатура
	|			И ТоварыПриемника.Характеристика = ОтборСтрок.Характеристика
	|			И (ОтборСтрок.Номенклатура <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТоварыПредложений КАК ТоварыПредложений
	|		ПО ТоварыПриемника.Номенклатура = ТоварыПредложений.Номенклатура
	|			И ТоварыПриемника.Характеристика = ТоварыПредложений.Характеристика
	|			И (ТоварыПриемника.Номенклатура <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
	|			И (ТоварыПредложений.Номенклатура <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ТоварыПриемника.НомерСтроки,
	|	ЕСТЬNULL(ТоварыПредложений.НомерСтроки, 0),
	|	&Цена,
	|	ТоварыПриемника.Партнер = ТоварыПредложений.Партнер,
	|	ТоварыПриемника.Контрагент = ТоварыПредложений.Партнер
	|ИЗ
	|	ТоварыПриемника КАК ТоварыПриемника
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОтборСтрок КАК ОтборСтрок
	|		ПО ТоварыПриемника.НоменклатураСтрокой = ОтборСтрок.НоменклатураСтрокой
	|			И (ОтборСтрок.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТоварыПредложений КАК ТоварыПредложений
	|		ПО ТоварыПриемника.НоменклатураСтрокой = ТоварыПредложений.НоменклатураСтрокой
	|			И (ТоварыПриемника.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
	|			И (ТоварыПредложений.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДоступныеЦены.НомерСтрокиПриемника КАК НомерСтрокиПриемника
	|ИЗ
	...
	|ВЫБРАТЬ
	|	ДоступныеЦены.НомерСтрокиПриемника КАК НомерСтрокиПриемника,
	|	ДоступныеЦены.НомерСтрокиПредложений КАК НомерСтрокиПредложений,
	|	ДоступныеЦены.Цена КАК Цена,
	|	ДоступныеЦены.ПартнерСовпадает КАК ПартнерСовпадает
	|ИЗ
	|	ДоступныеЦены КАК ДоступныеЦены
	|
	|УПОРЯДОЧИТЬ ПО
	|	ПартнерСовпадает УБЫВ,
	|	КонтрагентСовпадает УБЫВ,
	|	НомерСтрокиПриемника,
	|	НомерСтрокиПредложений";
	
	ТекстЗапроса = СтрЗаменить(
		ТекстЗапроса,
		"&Цена", 
		СтрШаблон("ЕСТЬNULL(ТоварыПредложений.Цена, 0) * ЕСТЬNULL(%1, 1) / ЕСТЬNULL(%2, 1)",
			Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки("ТоварыПриемника.Упаковка", "ТоварыПриемника.Номенклатура"),
			Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки("ТоварыПредложений.Упаковка", "ТоварыПредложений.Номенклатура")));
			
	// Кроме цены могут заполняться прочие колонки условий закупки из предложений. Для безопасного заполнения формируем список их имен:
	ИменаНаследуемыхКолонок = Новый Массив; 
	
	// Приведение колонок входной таблицы к ожидаемому виду с учетом параметров:
	ИменаКолонокИсточника = Новый Массив;
	ИменаКолонокИсточника.Добавить("НомерСтроки");
	ИменаКолонокИсточника.Добавить("Номенклатура");
	ИменаКолонокИсточника.Добавить("Характеристика");
	ИменаКолонокИсточника.Добавить("Упаковка");
	
	Если ПараметрыЗаполнения.ЕстьПартнер Тогда
		ИменаКолонокИсточника.Добавить("Партнер");
		ИменаНаследуемыхКолонок.Добавить("Партнер");
	КонецЕсли;
	
	Если ПараметрыЗаполнения.ЕстьКонтрагент Тогда
		ИменаКолонокИсточника.Добавить("Контрагент");
		ИменаНаследуемыхКолонок.Добавить("Контрагент");
	КонецЕсли;
	
	Если ПараметрыЗаполнения.ЕстьНоменклатураСтрокой Тогда
		ИменаКолонокИсточника.Добавить("НоменклатураСтрокой");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Товары)
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Товары[0], "СтавкаНДС") Тогда
		ИменаНаследуемыхКолонок.Добавить("СтавкаНДС");
	КонецЕсли;
	
	ИменаНаследуемыхКолонок = СтрСоединить(ИменаНаследуемыхКолонок, ", ");
	
	ИменаКолонокИсточника = СтрСоединить(ИменаКолонокИсточника, ", ");
	Если ТипЗнч(Товары) = Тип("ТаблицаЗначений") Тогда
		КлючиПоиска = Товары.Скопировать(, ИменаКолонокИсточника);
	Иначе
		КлючиПоиска = Товары.Выгрузить(, ИменаКолонокИсточника);
	КонецЕсли;
	
	Если Не ПараметрыЗаполнения.ЕстьПартнер Тогда
		КлючиПоиска.Колонки.Добавить("Партнер", Новый ОписаниеТипов("СправочникСсылка.Партнеры"));
	КонецЕсли;
	
	Если Не ПараметрыЗаполнения.ЕстьКонтрагент Тогда
		КлючиПоиска.Колонки.Добавить("Контрагент", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"));
	КонецЕсли;
	
	Если Не ПараметрыЗаполнения.ЕстьНоменклатураСтрокой Тогда
		КлючиПоиска.Колонки.Добавить("НоменклатураСтрокой", ОбщегоНазначения.ОписаниеТипаСтрока(300));
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапроса;
	Запрос.УстановитьПараметр("ТоварыПриемника", КлючиПоиска);
	Запрос.УстановитьПараметр("ТоварыПредложений", ВыбранныеУсловияЗакупки);
	Запрос.УстановитьПараметр("ОтборСтрок", ПараметрыЗаполнения.НомераСтрок);
	
	РезультатПакета = Запрос.ВыполнитьПакет();
	ВыборкаСтрокиПриемника = РезультатПакета[РезультатПакета.Количество() - 2].Выбрать();
	Выборка = РезультатПакета[РезультатПакета.Количество() - 1].Выбрать();
	
	// Формирование соответствия по исходному номеру строки приемника.
	// Хранит остатки к распределению.
	// А также исходную строку таблицы приемника, т.к. ее номер может быть изменен (при разбивке по ценам строки вставляются перед исходной).
	ОстаткиКРаспределению = Новый Соответствие;
	Пока ВыборкаСтрокиПриемника.Следующий() Цикл
		
		СтрокаПриемника = Товары.Получить(ВыборкаСтрокиПриемника.НомерСтрокиПриемника - 1);
		ИзменятьДанные = ПараметрыЗаполнения.НомераСтрок.Найти(ВыборкаСтрокиПриемника.НомерСтрокиПриемника) <> Неопределено;
		
		ОписаниеИсходнойСтроки = Новый Структура;
		ОписаниеИсходнойСтроки.Вставить("СтрокаПриемника", СтрокаПриемника);
		ОписаниеИсходнойСтроки.Вставить("КРаспределению", СтрокаПриемника.Количество);
		ОписаниеИсходнойСтроки.Вставить("ИзменятьДанные", ИзменятьДанные);
		
		ОстаткиКРаспределению.Вставить(СтрокаПриемника.НомерСтроки, ОписаниеИсходнойСтроки);
		
	КонецЦикла;
	
	Пока Выборка.Следующий() Цикл
		
		ОписаниеИсходнойСтроки = ОстаткиКРаспределению.Получить(Выборка.НомерСтрокиПриемника);
		
		Если ОписаниеИсходнойСтроки.КРаспределению = 0 // По указанной строке выполнено распределение.
			Или Выборка.НомерСтрокиПредложений = 0 Тогда // Не подобрана строка предложения.
			Продолжить;
		КонецЕсли;
		
		СтрокаПредложения = ВыбранныеУсловияЗакупки.Получить(Выборка.НомерСтрокиПредложений - 1);
		КоличествоПоЦене = Мин(ОписаниеИсходнойСтроки.КРаспределению, СтрокаПредложения.Количество);
		
		Если КоличествоПоЦене = 0 Тогда
			// По данной строке предложения уже раннее было списано количество. Переход к следующей.
			Продолжить;
		КонецЕсли;
		
		// Списываем "Остатки" из предложения и к распределению:
		ОписаниеИсходнойСтроки.КРаспределению = ОписаниеИсходнойСтроки.КРаспределению - КоличествоПоЦене;
		СтрокаПредложения.Количество = СтрокаПредложения.Количество - КоличествоПоЦене;
		
		Если Не ОписаниеИсходнойСтроки.ИзменятьДанные Тогда
			// Распределение выполнялось только для списания остатка, чтобы при любом количестве выделенных строк цены были заполнены одинаково.
			Продолжить;
		КонецЕсли;
			
		Если ОписаниеИсходнойСтроки.КРаспределению = 0 Тогда
			// Цена "закрывает" все оставшееся количество в строке приемника. Изменяем исходную строку.
			СтрокаПриемника = ОписаниеИсходнойСтроки.СтрокаПриемника;
		Иначе // "Закрывается" не все количество. Вставляем строку перед исходной.
			СтрокаПриемника = Товары.Вставить(ОписаниеИсходнойСтроки.СтрокаПриемника.НомерСтроки - 1);
			ЗаполнитьЗначенияСвойств(СтрокаПриемника, ОписаниеИсходнойСтроки.СтрокаПриемника);
		КонецЕсли;
		
		СтрокаПриемника.Количество = КоличествоПоЦене;
		СтрокаПриемника.Цена = Выборка.Цена;
		Если Не ПустаяСтрока(ИменаНаследуемыхКолонок) Тогда
			ЗаполнитьЗначенияСвойств(СтрокаПриемника, СтрокаПредложения, ИменаНаследуемыхКолонок);
		КонецЕсли;
		
		ВозвращаемоеЗначение.ИзмененныеСтроки.Добавить(СтрокаПриемника);
		
	КонецЦикла;
	
	СтруктураОчистки = Новый Структура("Цена, Партнер, Контрагент");
	
	НомераСтрокСНезаполненнымиУсловиямиЗакупки = Новый ТаблицаЗначений; // Для сортировки возвращаемых номеров строк
	НомераСтрокСНезаполненнымиУсловиямиЗакупки.Колонки.Добавить("НомерСтроки", ОбщегоНазначения.ОписаниеТипаЧисло(15, 0));
	
	Для Каждого КлючИЗначение Из ОстаткиКРаспределению Цикл
		
		ОписаниеИсходнойСтроки = КлючИЗначение.Значение;
		СтрокаПриемника = ОписаниеИсходнойСтроки.СтрокаПриемника;
		
		Если Не ОписаниеИсходнойСтроки.ИзменятьДанные
			Или ОписаниеИсходнойСтроки.КРаспределению = 0 Тогда
			Продолжить;
		КонецЕсли;
		 
		СтрокаПриемника.Количество = ОписаниеИсходнойСтроки.КРаспределению;
		ЗаполнитьЗначенияСвойств(СтрокаПриемника, СтруктураОчистки);
		
		ВозвращаемоеЗначение.ИзмененныеСтроки.Добавить(ОписаниеИсходнойСтроки.СтрокаПриемника);
		
		СтрокаНомерСтрокиСНезаполненнымиУсловиямиЗакупки = НомераСтрокСНезаполненнымиУсловиямиЗакупки.Добавить();
		СтрокаНомерСтрокиСНезаполненнымиУсловиямиЗакупки.НомерСтроки = СтрокаПриемника.НомерСтроки;
		
	КонецЦикла;
	
	НомераСтрокСНезаполненнымиУсловиямиЗакупки.Сортировать("НомерСтроки");
	ВозвращаемоеЗначение.НомераСтрокСНезаполненнымиУсловиямиЗакупки = НомераСтрокСНезаполненнымиУсловиямиЗакупки.ВыгрузитьКолонку("НомерСтроки");
	
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(ВозвращаемоеЗначение.ИзмененныеСтроки,
...
Процедура КоммерческоеПредложениеКлиентуОбработкаЗаполнения(Объект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗапросКоммерческогоПредложенияОтКлиента") Тогда
		...
		ПараметрыЗаполненияСтавкиНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(ДанныеЗаполнения);
...
Функция ДобавитьКомандуСоздатьНаОсновании(ИмяОбъекта, КомандыСозданияНаОсновании) Экспорт
	...
	Если ИмяОбъекта = Метаданные.Документы.ЗапросКоммерческихПредложенийПоставщиков.Имя Тогда
		...
		Если ПравоДоступа("Добавление", Метаданные.Документы.ЗапросКоммерческихПредложенийПоставщиков) Тогда
			...
			КомандаСоздатьНаОсновании.Обработчик = "СозданиеНаОснованииУТКлиент.СоздатьЗапросКоммерческогоПредложения";
...
Процедура ЗапросКоммерческихПредложенийПоставщиковОбработкаЗаполнения(Объект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаявкаНаЗакупку") Тогда
		
		 ЗаполнитьЗапросКоммерческихПредложенийПоставщиковНаОснованииЗаявкиНаЗакупку(Объект, ДанныеЗаполнения);
		
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаявкаНаОбеспечение") Тогда
		
		ЗаполнитьЗапросКоммерческихПредложенийПоставщиковНаОснованииЗаявкиНаОбеспечение(Объект, ДанныеЗаполнения);
...
Процедура ЗаполнитьЗапросКоммерческихПредложенийПоставщиковНаОснованииЗаявкиНаЗакупку(Объект, ЗаявкаНаЗакупку)
	
	ТекстыЗапросов = Новый СписокЗначений;
	ТекстыЗапросов.Добавить(
	"ВЫБРАТЬ
	|	ЗаявкаНаЗакупку.Организация КАК Организация,
	|	ЗаявкаНаЗакупку.Валюта КАК Валюта,
	|	ЗаявкаНаЗакупку.Менеджер КАК Менеджер,
	|	ЗаявкаНаЗакупку.ЦенаВключаетНДС КАК ЦенаВключаетНДС,
	|	ЗаявкаНаЗакупку.Ссылка КАК ДокументОснование
	|ИЗ
	...
	|	ЗаявкаНаЗакупкуТовары.КоличествоУпаковок КАК Количество,
	|	ЗаявкаНаЗакупкуТовары.Склад КАК Склад,
	|	ПРЕДСТАВЛЕНИЕ(ЗаявкаНаЗакупкуТовары.Номенклатура) КАК НоменклатураПредставление,
	|	ПРЕДСТАВЛЕНИЕ(ЗаявкаНаЗакупкуТовары.Характеристика) КАК ХарактеристикаПредставление
	|ПОМЕСТИТЬ Товары
	|ИЗ
	...
	|	Товары.ХарактеристикаПредставление КАК ХарактеристикаПредставление
	|ИЗ
	|	Товары КАК Товары",
	"Товары");
	...
	РезультатЗапросаШапка = РезультатПакета.Шапка; // РезультатЗапроса
	ВыборкаШапка = РезультатЗапросаШапка.Выбрать();
	ВыборкаШапка.Следующий();
	
	ЗаполнитьЗначенияСвойств(Объект, ВыборкаШапка);
	
	РезультатЗапросаСклады = РезультатПакета.Склады; // РезультатЗапроса
	ВыборкаСклады = РезультатЗапросаСклады.Выбрать();
	ВыборкаСклады.Следующий();
	
	ЗаполнитьЗначенияСвойств(Объект, ВыборкаСклады);
	
	РезультатЗапросаТовары = РезультатПакета.Товары; // РезультатЗапроса
	...
	Пока ВыборкаТовары.Следующий() Цикл
		...
		СтрокаТоваров.ИсточникДобавленияТовара = 0;
		
		Если ЗначениеЗаполнено(ВыборкаТовары.Характеристика) Тогда
			
			СтрокаТоваров.НаименованиеСтрокой = СтрШаблон("%1 (%2)",
				ВыборкаТовары.НоменклатураПредставление,
				ВыборкаТовары.ХарактеристикаПредставление);
			
		Иначе
			
			СтрокаТоваров.НаименованиеСтрокой = ВыборкаТовары.НоменклатураПредставление;
...
Процедура ЗаполнитьЗапросКоммерческихПредложенийПоставщиковНаОснованииЗаявкиНаОбеспечение(Объект, ЗаявкаНаОбеспечение)
	...
	ЗаполнитьЗначенияСвойств(Объект, ДанныеЗаполнения.Шапка, "Организация, Валюта, Склад");
	Объект.ДокументОснование = ЗаявкаНаОбеспечение;
	
	ПредставленияНоменклатуры = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(
		ДанныеЗаполнения.Товары.ВыгрузитьКолонку("Номенклатура"),
		"Представление");
		
	ПредставленияХарактеристик = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(
		ДанныеЗаполнения.Товары.ВыгрузитьКолонку("Характеристика"),
		"Представление");
	...
	Для Каждого СтрокаИсточник Из ДанныеЗаполнения.Товары Цикл
		...
		Если ЗначениеЗаполнено(СтрокаИсточник.Номенклатура) Тогда
			СтрокаТоваров.ИсточникДобавленияТовара = 0;
		Иначе
			СтрокаТоваров.ИсточникДобавленияТовара = 2;
			СтрокаТоваров.НоменклатураТекстом = СтрокаИсточник.НоменклатураСтрокой;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СтрокаИсточник.Номенклатура)
			И ЗначениеЗаполнено(СтрокаИсточник.Характеристика) Тогда
			
			СтрокаТоваров.НаименованиеСтрокой = СтрШаблон("%1 (%2)",
				ПредставленияНоменклатуры.Получить(СтрокаИсточник.Номенклатура),
				ПредставленияХарактеристик.Получить(СтрокаИсточник.Характеристика));
			
		Иначе
			
			СтрокаТоваров.НаименованиеСтрокой = СтрокаИсточник.НоменклатураСтрокой;
...
Процедура ЗапросКоммерческихПредложенийПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка)
	...
	Если ПравоДоступа("Добавление", Метаданные.Справочники.НоменклатураПрисоединенныеФайлы) 
		И ПравоДоступа("Изменение", Метаданные.Справочники.НоменклатураПрисоединенныеФайлы) Тогда
			
		ОбщегоНазначенияКлиентСервер.УстановитьРеквизитФормыПоПути(Форма,
			 "ПозволятьДобавлениеФайлаИзСистемыДляВнутреннейНоменклатуры", Истина);
...
Функция ПараметрыПолученияВыбранныхУсловийЗакупки()
	
	ВозвращаемоеЗначение = Новый Структура;
	
	ВозвращаемоеЗначение.Вставить("Документ",  Неопределено);
	ВозвращаемоеЗначение.Вставить("ДатаКурса", ТекущаяДатаСеанса());
	ВозвращаемоеЗначение.Вставить("Валюта",    Справочники.Валюты.ПустаяСсылка());
...
Функция ВыбранныеУсловияЗакупки(ПараметрыПолучения)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Шапка.МоментВремени КАК МоментВремени,
	|	ТаблицаВыбранныеИсточники.НомерСтроки КАК ИсходныйНомерСтроки,
	|	ТаблицаТовары.Номенклатура КАК Номенклатура,
	|	ТаблицаТовары.НоменклатураТекстом КАК НоменклатураТекстом,
	|	ТаблицаТовары.Характеристика КАК Характеристика,
	|	ТаблицаВыбранныеИсточники.Количество КАК КоличествоУпаковок,
	|	ТаблицаТовары.ЕдиницаИзмерения КАК Упаковка,
	|	ТаблицаВыбранныеИсточники.ЦенаВВалютеЗапроса КАК Цена,
	|	ТаблицаВыбранныеИсточники.СуммаВВалютеЗапроса КАК Сумма,
	|	ТаблицаВыбранныеИсточники.СуммаВВалютеЗапроса КАК СуммаСНДС,
	|	Шапка.Валюта КАК Валюта,
	|	ТаблицаВыбранныеИсточники.ЦенаВключаетНДС КАК ЦенаВключаетНДС,
	|	ТаблицаВыбранныеИсточники.ПоставщикСсылка.Партнер КАК Партнер,
	|	ТаблицаВыбранныеИсточники.ПоставщикСсылка КАК Контрагент,
	|	ТаблицаВыбранныеИсточники.СтавкаНДСПоставщика КАК СтавкаНДС
	|ИЗ
	|	Документ.ЗапросКоммерческихПредложенийПоставщиков.Товары КАК ТаблицаТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗапросКоммерческихПредложенийПоставщиков КАК Шапка
	|		ПО ТаблицаТовары.Ссылка = Шапка.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗапросКоммерческихПредложенийПоставщиков.ВыбранныеИсточники КАК ТаблицаВыбранныеИсточники
	|		ПО ТаблицаТовары.Ссылка = ТаблицаВыбранныеИсточники.Ссылка
	|			И ТаблицаТовары.ИдентификаторСтрокиЗапроса = ТаблицаВыбранныеИсточники.ИдентификаторСтрокиЗапроса
	|ГДЕ
	|	Шапка.ДокументОснование = &ДокументОснование
	|	И Шапка.Проведен
	|
	|УПОРЯДОЧИТЬ ПО
	|	Валюта,
	...
	Запрос.УстановитьПараметр("ДокументОснование", ПараметрыПолучения.Документ);
	ТаблицыКПересчету = ОбеспечениеСервер.РазбитьТаблицуПоЗначениюКлюча(
		Запрос.Выполнить().Выгрузить(),
		"Валюта, ЦенаВключаетНДС");
		
	ВозвращаемоеЗначение = Неопределено;
	
	ОписаниеТипаДенежногоПоля = ОбщегоНазначенияУТ.ОписаниеТипаДенежногоПоля();
	
	Для Каждого ОписаниеТаблицы Из ТаблицыКПересчету Цикл
		
		ТоварыКПересчету = ОписаниеТаблицы.Таблица; // ТаблицаЗначений
		ТоварыКПересчету.Колонки.Добавить("СуммаНДС", ОписаниеТипаДенежногоПоля);
		
		Если ВозвращаемоеЗначение = Неопределено Тогда
			
			ВозвращаемоеЗначение = ТоварыКПересчету.СкопироватьКолонки();
			
		КонецЕсли;
		
		// Расчет вспомогательных сумм:
		ЦенаВключаетНДС = ?(ПараметрыПолучения.ЦенаВключаетНДС = Неопределено,
			ОписаниеТаблицы.Ключ.ЦенаВключаетНДС, // В параметрах не указано, берем из документа.
			ПараметрыПолучения.ЦенаВключаетНДС);
			
		Для Каждого СтрокаТоваров Из ТоварыКПересчету Цикл
			
			ПроцентСтавкиНДС = УчетНДСУПКлиентСервер.ЗначениеСтавкиНДС(СтрокаТоваров.СтавкаНДС);
			Если ПроцентСтавкиНДС = Неопределено Тогда
				ПроцентСтавкиНДС = 0;
			КонецЕсли;
			
			СтрокаТоваров.СуммаНДС = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(СтрокаТоваров.Сумма,
				ПроцентСтавкиНДС,
				ОписаниеТаблицы.Ключ.ЦенаВключаетНДС);
			
			Если ЦенаВключаетНДС Тогда
				СтрокаТоваров.СуммаСНДС = СтрокаТоваров.Сумма + СтрокаТоваров.СуммаНДС;
			Иначе
				СтрокаТоваров.Сумма = СтрокаТоваров.СуммаСНДС - СтрокаТоваров.СуммаНДС;
			КонецЕсли;
			
			// Пересчет цены, если флаг в параметрах указан и он отличен от флага в документе:
			Если ЦенаВключаетНДС <> ОписаниеТаблицы.Ключ.ЦенаВключаетНДС Тогда
				
				Если ЦенаВключаетНДС Тогда
					СтрокаТоваров.Цена = Окр(СтрокаТоваров.СуммаСНДС / СтрокаТоваров.КоличествоУпаковок, 2);
				Иначе
					СтрокаТоваров.Цена = Окр(СтрокаТоваров.Сумма / СтрокаТоваров.КоличествоУпаковок, 2);
				КонецЕсли;
		...
		// Пересчет цен в валюту документа-основания:
		Если ОписаниеТаблицы.Ключ.Валюта <> ПараметрыПолучения.Валюта Тогда
		
			ПараметрыПересчета = Ценообразование.ПараметрыПересчетаСуммТаблицыВВалюту(
					ОписаниеТаблицы.Ключ.ЦенаВключаетНДС,
					ОписаниеТаблицы.Ключ.Валюта,
					ПараметрыПолучения.Валюта,
					ПараметрыПолучения.ДатаКурса);
			
			Ценообразование.ПересчитатьСуммыТаблицыВВалюту(ТоварыКПересчету, ПараметрыПересчета);
			
		КонецЕсли;
		
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТоварыКПересчету, ВозвращаемоеЗначение);
		
	КонецЦикла;
	
	Если ВозвращаемоеЗначение <> Неопределено Тогда
		
		// Сортировка таблицы для единого результата:
		ИменаКолонокСортировки = Новый Массив;
		ИменаКолонокСортировки.Добавить("МоментВремени");
		ИменаКолонокСортировки.Добавить("ИсходныйНомерСтроки");
		
		ВозвращаемоеЗначение.Сортировать(СтрСоединить(ИменаКолонокСортировки, ", "));
		Для Каждого ИмяКолонки Из ИменаКолонокСортировки Цикл
			ВозвращаемоеЗначение.Колонки.Удалить(ИмяКолонки);
		КонецЦикла;
		
		ВозвращаемоеЗначение.Колонки.Добавить("Количество", ОбщегоНазначения.ОписаниеТипаЧисло(15, 3));
		
		Действия = Новый Структура;
		Действия.Вставить("ПересчитатьКоличествоЕдиниц");
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ВозвращаемоеЗначение, Действия);
		
		ОбщегоНазначенияУТ.ПронумероватьТаблицуЗначений(ВозвращаемоеЗначение, "ИндексСтроки");
