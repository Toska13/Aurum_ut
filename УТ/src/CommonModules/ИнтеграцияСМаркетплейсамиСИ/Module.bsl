...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	ИсключениеПроверки.ПолноеИмяОбъекта = "ОбщийМодуль.ИнтеграцияСМаркетплейсамиСИ";
	...
	Настройки.ИсключенияКогдаУказанныйМетодНеВызывается.Вставить("ОбщийМодуль.ИнтеграцияСМаркетплейсамиСИ", Методы);
				...
				Иначе
					...
					Исключение
						...
						ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
							ТекстОшибки);
						Результат.Ошибки.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Ошибка при записи ""%1"". Возможно, объект уже редактируется.'"), Карточка.Представление));
						Продолжить;
...
Функция ЗаполнитьОписаниеКарточекТоваров(Знач ПараметрыМетода, Знач АдресРезультата = "") Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не РезультатПереноса Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Невозможен перенос описания в карточку товара %1'"), Выборка.ПредставлениеОбъектаМаркетплейса);
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.Действие = Запрос.Параметры.КодОшибка Тогда
				...
				ТекстОшибки = СтрШаблон(НСтр("ru = 'В карточке %1 должен быть идентификатор %2'"), Выборка.НоменклатураКонтрагента, Выборка.ИдентификаторНоменклатурыСервиса);
...
Функция СобытиеЖурналаРегистрации(ВидМаркетплейса = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ВидМаркетплейса) Тогда
		Возврат СтрШаблон(НСтр("ru = 'Интеграция с %1'", ОбщегоНазначения.КодОсновногоЯзыка()), 
			ИнтеграцияСМаркетплейсамиСИКлиентСервер.МаркетплейсПоВиду(ВидМаркетплейса));
	Иначе
...
Функция ЗапросВыгрузитьОграниченияОстатковТоваров(ПараметрыМетода)
	...
	Синонимы = Новый Соответствие;
	Синонимы.Вставить(НСтр("ru = 'Номенклатура'"), НСтр("ru = 'Номенклатура'"));
	Синонимы.Вставить(НСтр("ru = 'ТоварныеКатегории'"), НСтр("ru = 'ТоварнаяКатегория'"));
	Синонимы.Вставить(НСтр("ru = 'ВидыНоменклатуры'"), НСтр("ru = 'ВидНоменклатуры'"));
...
Функция ЗапросВыгрузитьЦены(ПараметрыМетода)
	...
	Если НастройкиВидаЦенНеЗаполнены(НастройкиУчетнойЗаписи, ТипыЦенМаркетплейса) Тогда
		ТекстСообщения = НСтр("ru = 'Не установлены виды цен для выгрузки.'", ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(
