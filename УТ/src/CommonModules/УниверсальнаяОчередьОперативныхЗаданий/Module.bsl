...
//
// Параметры:
//  ТаблицаОчередей - ТаблицаЗначений, Неопределено -
...
//
Процедура Запустить(Знач ТаблицаОчередей = Неопределено) Экспорт
	
	ОтложенныеЗадания.Запустить(?(ЗначениеЗаполнено(ТаблицаОчередей), ТаблицаОчередей, "УниверсальнаяОчередьОперативныхЗаданий"));
