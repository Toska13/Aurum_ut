...
Функция ДанныеПрикладныхДокументовИзВходящейТранспортнойОперацииВЕТИС(ДокументСсылка)
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ПредприятияОтправителя.ТорговыйОбъект, НЕОПРЕДЕЛЕНО) = НЕОПРЕДЕЛЕНО
	|			ТОГДА ВходящаяТранспортнаяОперацияВЕТИС.ГрузоотправительХозяйствующийСубъект.Контрагент.Партнер
	|		ИНАЧЕ ПредприятияОтправителя.ТорговыйОбъект
	|	КОНЕЦ КАК Партнер,
...
Процедура ЗаполнитьТабличнуюЧастьИсходящейТранспортнойОперации(ДокументОбъект, ДанныеЗаполнения, ИмяДокумента, Описание)
	...
	ВидыПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииВЕТИС();
...
Функция ДанныеПрикладныхДокументовИзИсходящейТранспортнойОперацииВЕТИС(ДокументСсылка)
	...
	|	ПредприятияПолучателя.ТорговыйОбъект                                               КАК ГрузополучательПредприятие,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ПредприятияПолучателя.ТорговыйОбъект, НЕОПРЕДЕЛЕНО) = НЕОПРЕДЕЛЕНО
	|			ТОГДА ИсходящаяТранспортнаяОперацияВЕТИС.ГрузополучательХозяйствующийСубъект.Контрагент.Партнер
	|		ИНАЧЕ ПредприятияПолучателя.ТорговыйОбъект
	|	КОНЕЦ                                                                              КАК Партнер,
	|	ПредприятияОтправителя.ПроизводственныйОбъект                                      КАК ГрузоотправительПодразделение,
	...
	|	ИсходящаяТранспортнаяОперацияВЕТИС.ГрузополучательХозяйствующийСубъект.Контрагент.Партнер КАК КонтрагентПартнер
	|ИЗ
...
Процедура ОбработкаЗаполненияДокументаИсходящаяТранспортнаяОперацияВЕТИС(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту")Тогда
		ЗаполнитьИсходящуюТранспортнуюОперациюВЕТИСНаОснованииОтгрузкиТоваровКлиенту(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
...
Процедура ЗаполнитьИсходящуюТранспортнуюОперациюВЕТИСНаОснованииОтгрузкиТоваровКлиенту(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	ДокументСсылка.Склад.ЭтоГруппа       КАК ЭтоГруппаСкладов
	|ИЗ
	...
	ЗаполнитьШапкуИсходящейТранспортнойОперации(ДокументОбъект, ДанныеЗаполнения, Запрос);
	ЗаполнитьТабличнуюЧастьИсходящейТранспортнойОперации(
		ДокументОбъект, ДанныеЗаполнения, "ОтгрузкаТоваровКлиенту", ИнтеграцияИСУТ.ТекстЗапросаОтгрузкаТоваровКлиенту());
...
Процедура ЗаполнитьВозвратТоваровОтКлиентаНаОснованииВходящейТранспортнойОперацииВЕТИС(ДокументОбъект, ДанныеЗаполнения) Экспорт
	...
	СтруктураЗаполненияТЧ = СтруктураЗаполненияТабличныхЧастейПоВходящейТранспортнойОперации(
																		ПакетРезультатовЗапроса, Истина, Истина);
	ЗаполнитьТабличныеЧастиПрикладногоДокументаНаОснованииВходящейТранспортнойОперации(ДокументОбъект,СтруктураЗаполненияТЧ);
...
Процедура ЗаполнитьВозвратТоваровПоставщикуНаОснованииИсходящейТранспортнойОперацииВЕТИС(ДокументОбъект, ДанныеЗаполнения) Экспорт
	...
	СтруктураЗаполненияТЧ = СтруктураЗаполненияТабличныхЧастейПоИсходящейТранспортнойОперации(
																	ПакетРезультатовЗапроса,Истина, Истина);
	ЗаполнитьТабличныеЧастиПрикладногоДокументаНаОснованииИсходящейТранспортнойОперации(ДокументОбъект,СтруктураЗаполненияТЧ);
	ДокументОбъект.ЗаполнитьУсловияЗакупокПоУмолчанию();
...
Функция ЗаполнитьСгенерироватьСерии(ТабличнаяЧастьТовары,
	...
	СтрокиКОбработкеТовары = Новый Массив;
	СтрокиКОбработкеУточнения = Новый Массив;
	...
	Для Каждого СтрТабл из ТаблицаУстановкиСерий Цикл
		...
		Иначе
			...
			Если СтрТабл.ЕстьУточнения Тогда
				СтрокиКОбработкеУточнения.Добавить(СтрокаТЧ);
			Иначе
				СтрокиКОбработкеТовары.Добавить(СтрокаТЧ);
			КонецЕсли;
		КонецЕсли;
		
	КонецЦикла;
	
	Если СтрокиКОбработкеТовары.Количество() Тогда
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			СтрокиКОбработкеТовары,
			СтруктураДействий,
	...
	Если СтрокиКОбработкеУточнения.Количество() Тогда
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			СтрокиКОбработкеУточнения,
			СтруктураДействий,
