...
Процедура РассчитатьВсе(ПараметрыЗапуска, ПараметрыРасчета = Неопределено, ПараметрыОтладки = Неопределено) Экспорт
	...
	РассчитываемыйПериод = ТекущаяДатаСеанса();
	
	Для Каждого СтрокаСхемыРасчета Из СхемаРасчета Цикл
		
		РассчитываемыйПериод = НачалоМесяца(СтрокаСхемыРасчета.Дата);
		...
		// Этап отражения фактуровки поставки товаров при использовании метода оценки стоимости "Среднескользящая".
		// Формирует движения по регистрам:
		// - СебестоимостьТоваров
		// - ОтклонениеВСтоимостиТоваров
		РасчетСебестоимостиЗаполнениеПартий.ФактуровкаПоставокПриСреднескользящей(ПараметрыРасчета);
		...
		РасчетСебестоимостиКорректировкаСтоимости.СформироватьДвиженияПоОтклонениямВСтоимостиТоваров(ПараметрыРасчета);
	...
	Если АвтоматическоеТестирование Тогда
		РасчетСебестоимостиПрикладныеАлгоритмы.ВернутьСостояниеИтоговРегистров(
			СостояниеИтоговРегистров,
			КонецМесяца(КонецМесяца(РассчитываемыйПериод) + 1)); // на конец следующего месяца
	Иначе
...
Функция РассчитатьВсеВПопыткеИсключении(ПараметрыЗапуска, ПараметрыРасчета = Неопределено, ПараметрыОтладки = Неопределено) Экспорт
	...
	Попытка
		...
		ИначеЕсли ПараметрыЗапуска.Свойство("РасчетПрошлыхПериодов") 
		 И ПараметрыЗапуска.РасчетПрошлыхПериодов
		 И ПараметрыЗапуска.Свойство("ВыполняетсяЗакрытиеМесяца")
		 И ПараметрыЗапуска.ВыполняетсяЗакрытиеМесяца Тогда
		 	РассчитатьСебестоимостьПрошлыхПериодов(ПараметрыЗапуска.Дата, ПараметрыРасчета, ПараметрыОтладки);
		Иначе
...
// Определяет необходимость пересчета себестоимости за прошлые периоды.
// 
// Параметры:
//  МассивОрганизаций - СправочникСсылка.Организации, Массив из СправочникСсылка.Организации - организации для проверки
// 
// Возвращаемое значение:
//  Дата - дата самого раннего периода, требующего пересчета
Функция ТребуетсяПересчетСебестоимостиЗаПрошлыеПериоды(МассивОрганизаций) Экспорт
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаФинансовогоУчета.СрезПоследних() КАК УчетнаяПолитика
	|		ПО (УчетнаяПолитика.Организация = Т.ГоловнаяОрганизация)
	|ГДЕ
	|	УчетнаяПолитика.РаспределятьОтклоненияВПрошлыхПериодах
	|	И УчетнаяПолитика.МетодОценкиСтоимостиТоваров <> ЗНАЧЕНИЕ(Перечисление.МетодыОценкиСтоимостиТоваров.Среднескользящая)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация
	|;
	...
	|ВЫБРАТЬ
	|	Т.Организация КАК Организация,
	|	МИНИМУМ(НАЧАЛОПЕРИОДА(Т.Месяц, МЕСЯЦ)) КАК Период
	|ПОМЕСТИТЬ ВТПериоды
	|ИЗ
	|	РегистрСведений.ЗаданияКРасчетуСебестоимости КАК Т
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОтборОрганизации КАК Отбор
	...
	Если Выборка.Следующий() И ЗначениеЗаполнено(Выборка.Период) Тогда
		Запрос.УстановитьПараметр("НачалоПериода", НачалоМесяца(Выборка.Период)); // для предварительного отбора обрабатываемых данных
	Иначе
		Возврат ПериодПересчета;
	КонецЕсли;
	...
	|ИЗ
	// Запрос аналогичен РасчетСебестоимостиРешениеСЛУ.ВтОтклоненияПрочиеРасходыКРаспределению()
	|(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НАЧАЛОПЕРИОДА(Т.ПериодОтклонения, МЕСЯЦ) КАК ПериодОтклонения
	|ИЗ
	|	РегистрНакопления.ПрочиеРасходы КАК Т
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТПериоды КАК Отбор
	|		ПО Т.Организация = Отбор.Организация
	...
	|				ИНАЧЕ -(Т.СуммаРегл - Т.ПостояннаяРазница - Т.ВременнаяРазница)
	|			КОНЕЦ) <> 0
	|		ИЛИ СУММА(ВЫБОР
	|				КОГДА Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|					ТОГДА Т.СуммаНДД
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Запрос аналогичен РасчетСебестоимостиРешениеСЛУ.ВтОтклоненияКРаспределению()
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НАЧАЛОПЕРИОДА(Т.ПериодОтклонения, МЕСЯЦ) КАК ПериодОтклонения
	|ИЗ
	...
	|				ИНАЧЕ -Т.СтоимостьНДД
	|			КОНЕЦ) <> 0)
	|) КАК Т";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() И ЗначениеЗаполнено(Выборка.ПериодОтклонения) Тогда
		ПериодПересчета = НачалоМесяца(Выборка.ПериодОтклонения);
	КонецЕсли;
...
// Выполняет пересчет себестоимости за прошлые периоды.
// 
// Параметры:
//  Период - Дата - Период расчета
//  МассивОрганизаций - Массив из СправочникСсылка.Организации - Массив организаций для пересчета
// 
Процедура РассчитатьПрошлыеПериоды(Период, МассивОрганизаций) Экспорт
	
	ПараметрыЗапуска = Новый Структура;
	ПараметрыЗапуска.Вставить("Дата", 					    НачалоМесяца(Период));
	ПараметрыЗапуска.Вставить("МассивОрганизаций",  	    МассивОрганизаций);
	ПараметрыЗапуска.Вставить("ТолькоОднаОрганизация",		Ложь);
	ПараметрыЗапуска.Вставить("МестоВызоваРасчета", 	    "РасчетСебестоимости.РассчитатьПрошлыеПериоды");
	ПараметрыЗапуска.Вставить("ВыполняетсяЗакрытиеМесяца",  Ложь);
	ПараметрыЗапуска.Вставить("РасчетПрошлыхПериодов",  	Истина);
	ПараметрыЗапуска.Вставить("ИдентификаторРасчета", 		Новый УникальныйИдентификатор);
	
	РассчитатьВсеВПопыткеИсключении(ПараметрыЗапуска);
...
//		Подробнее см. пояснения в коде РасчетСебестоимостиПрикладныеАлгоритмы.ИнициализироватьПараметрыРасчетаПартий к параметру ПараметрыОтладки.
//
Процедура РассчитатьСебестоимостьПрошлыхПериодов(Период = Неопределено, ПараметрыРасчета = Неопределено, ПараметрыОтладки = Неопределено) Экспорт
...
Функция ЭтапыСПодготовкойДанныхДляСледующихЭтапов() Экспорт
	...
	РасчетСебестоимостиЗаполнениеПартий.ДополнитьЭтапыСПодготовкойДанныхДляСледующихЭтапов(СписокЭтапов);
	...
	РасчетСебестоимостиКорректировкаСтоимости.ДополнитьЭтапыСПодготовкойДанныхДляСледующихЭтапов(СписокЭтапов);
...
Функция ТекстЗапросаДляРасчетаЭтапа(ИмяЭтапа, ПараметрыРасчета = Неопределено) Экспорт
	...
	РасчетСебестоимостиКорректировкаСтоимости.ТекстЗапросаДляРасчетаЭтапа(ИмяЭтапа, ПараметрыРасчета, ТекстЗапроса);
