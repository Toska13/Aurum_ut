...
Процедура ПроверитьЗаполнениеАкцизныхМарок(Объект, Отказ, НаДату = Неопределено, РеквизитыОбъекта = Неопределено, СообщатьОбОшибках = Истина, ФормированиеЭлектронногоДокумента = Ложь) Экспорт
	...
	ВидыПродукцииГИСМТ = Новый Массив;
	Если НаДату = Неопределено Тогда
		...
		Для Каждого ВидПродукции Из ОбщегоНазначенияИСМП.ВидыПродукцииОбязательнойМаркировки() Цикл
			...
			ВидыПродукцииГИСМТ.Добавить(ВидПродукции);
	...
	Если Не ВидыПродукцииГИСМТ.Количество() Тогда
		Возврат;
	КонецЕсли;
	...
	КонецЕсли;
	ЗапросМаркируемыхТоваров.УстановитьПараметр("ВидыПродукцииГИСМТ", ВидыПродукцииГИСМТ);
	...
	|	И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииГИСМТ)
	|";
	...
	ИначеЕсли Не ПараметрыУказанияСерий.Свойство("ИмяТЧТовары")
		И ПараметрыУказанияСерий.Свойство(ИмяТаблицыТовары) Тогда
		ПараметрыУказанияСерий = ПараметрыУказанияСерий[ИмяТаблицыТовары];
	КонецЕсли;
...
Функция ЕстьПроверкаИПодборВФорме(Форма) Экспорт
	...
	Если ИмяФормы = "Документ.РасходныйОрдерНаТовары.Форма.ФормаДокумента"
		...
		Или ИмяФормы = "Документ.ЧекККМКоррекции.Форма.ФормаДокумента"
...
// Возвращает признак действия поэкземплярного учета по продукции.
// 
// Параметры:
//  ВидПродукции - СправочникСсылка.ВидыПродукцииИС - вид продукции ИС
//  Дата         - Дата - дата контроля.
// 
// Возвращаемое значение:
//  Булево - Истина, если действует поэкземплярный учет продукции на дату.
Функция ДействуетПоэкземплярныйУчетПродукцииНаДату(ВидПродукции, Дата) Экспорт
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ИСМП")
		И ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(ВидПродукции, Истина) Тогда
		
		МодульОбщегоНазначенияИСМППовтИсп = ОбщегоНазначения.ОбщийМодуль("ОбщегоНазначенияИСМППовтИсп");
		Возврат МодульОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПриПродажеОптомПоштучно(ВидПродукции, Дата);
...
// Обработка элементов справочника Номенклатура при переходе на конструктор настроек видов продукции
// 
Процедура ОбработатьНоменклатуруПриПереходеНаКонструкторНастроек() Экспорт
	...
	СписокЗапросов.Добавить(
		...
		|		ПО ОписаниеНоменклатурыИС.Номенклатура = СправочникНоменклатура.Ссылка
		|ГДЕ
		|	(СправочникНоменклатура.ОсобенностьУчета В (&ОсобенностиУчета) ИЛИ СправочникНоменклатура.АлкогольнаяПродукция)
		|	И ЕСТЬNULL(ОписаниеНоменклатурыИС.КлючВидовПродукции, &ПустойКлючВидовПродукции) = &ПустойКлючВидовПродукции",
		"Номенклатура");
	
	СписокЗапросов.Добавить(
		"ВЫБРАТЬ
		|	ДанныеСправочника.Ссылка           КАК Ссылка,
		|	ДанныеСправочника.ОсобенностьУчета КАК ОсобенностьУчета,
		|	1 В
		|		(ВЫБРАТЬ ПЕРВЫЕ 1
		|			1
		|		ИЗ
		...
		|		ГДЕ
		|			Номенклатура.ВидНоменклатуры = ДанныеСправочника.Ссылка
		|			И ЕСТЬNULL(Номенклатура.ВидАлкогольнойПродукции.Маркируемый, ЛОЖЬ)) КАК ЕстьПодакцизный,
		|	1 В
		|		(ВЫБРАТЬ ПЕРВЫЕ 1
		|			1
		|		ИЗ
		...
		|		ГДЕ
		|			Номенклатура.ВидНоменклатуры = ДанныеСправочника.Ссылка
		|			И НЕ ЕСТЬNULL(Номенклатура.ВидАлкогольнойПродукции.Маркируемый, ЛОЖЬ)) КАК ЕстьНеподакцизный
		|ИЗ
		|	Справочник.ВидыНоменклатуры КАК ДанныеСправочника
		|ГДЕ
		|	ДанныеСправочника.ОсобенностьУчета В (&ОсобенностиУчета)",
		"ВидыНоменклатуры");
	
	Запрос.УстановитьПараметр("МерныеТипы",               Справочники.УпаковкиЕдиницыИзмерения.МерныеТипыЕдиницИзмерений());
	Запрос.УстановитьПараметр("ОсобенностиУчета",         ОсобенностиУчетаИС());
	...
	//@skip-warning
	ТаблицаНоменклатуры = РезультатЗапроса["Номенклатура"].Выгрузить();
	//@skip-warning
	ТаблицаВидовНоменклатуры = РезультатЗапроса["ВидыНоменклатуры"].Выгрузить();
	
	Если ТаблицаНоменклатуры.Количество() = 0
		И ТаблицаВидовНоменклатуры.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Номенклатура = ТаблицаНоменклатуры.ВыгрузитьКолонку("Ссылка");
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Номенклатура, ТаблицаВидовНоменклатуры.ВыгрузитьКолонку("Ссылка"));
	...
	Для Каждого СтрокаТаблицы Из ТаблицаНоменклатуры Цикл
		...
		Если СтрокаТаблицы.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьАлкогольнаяПродукцияДо9Процентов Тогда
			Продолжить; // Нет соответствия в классификаторе видов продукции
		ИначеЕсли ЗначениеЗаполнено(СтрокаТаблицы.ОсобенностьУчета) Тогда
			СтрокаТаблицы.ВидПродукции = ИнтеграцияИСУТКлиентСервер.ОсобенностьУчетаПоВидуПродукции(
				СтрокаТаблицы.ОсобенностьУчета);
		ИначеЕсли СтрокаТаблицы.АлкогольнаяПродукция Тогда
			СтрокаТаблицы.ВидПродукции = Перечисления.ВидыПродукцииИС.Алкогольная;
			СтрокаТаблицы.ЭтоПодакцизнаяПродукцияЕГАИС = Истина;
		КонецЕсли;
		
		ТекущееОписание = ОписанияНоменклатуры[СтрокаТаблицы.Ссылка];
		ТекущееОписание.ВариантИспользованияЕдиницыХранения = Перечисления.ВариантыИспользованияЕдиницыХраненияИС.ЗаданУпаковками;
		
		ОбновлениеИнформационнойБазыГосИС.ЗаполнитьОписаниеНоменклатурыПриПереходеНаКонструкторНастроек(
			ТекущееОписание, СтрокаТаблицы, ДанныеКеша, Истина);
		
		Если Не ЗначениеЗаполнено(ТекущееОписание.КлючВидовПродукции) Тогда
		...
		ПроверитьРазделитьПорциюЗаписиОписанияНоменклатурыПриПереходеНаКонструкторНастроекВидовПродукции(НаборЗаписей);
		НоваяСтрока = НаборЗаписей.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущееОписание);
	...
	Для Каждого СтрокаТаблицы Из ТаблицаВидовНоменклатуры Цикл
		...
		ТекущееОписание = ОписанияНоменклатуры[СтрокаТаблицы.Ссылка];
		Если ЗначениеЗаполнено(ТекущееОписание.КлючВидовПродукции)
			Или СтрокаТаблицы.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьАлкогольнаяПродукцияДо9Процентов Тогда
			Продолжить;
		КонецЕсли;
		ТекущееОписание.ВариантИспользованияЕдиницыХранения = Перечисления.ВариантыИспользованияЕдиницыХраненияИС.ЗаданУпаковками;
		ВидПродукции = ИнтеграцияИСУТКлиентСервер.ОсобенностьУчетаПоВидуПродукции(СтрокаТаблицы.ОсобенностьУчета);
		
		ЭтоПодакцизнаяПродукцияЕГАИС = Неопределено;
		
		Если ВидПродукции = Перечисления.ВидыПродукцииИС.Алкогольная Тогда
			Если СтрокаТаблицы.ЕстьПодакцизный И СтрокаТаблицы.ЕстьНеподакцизный
				Или Не СтрокаТаблицы.ЕстьПодакцизный И Не СтрокаТаблицы.ЕстьНеподакцизный Тогда
				Продолжить; // по виду номенклатуры есть подакцизный и не подакцизный вид алкогольной продукции
			Иначе
				ЭтоПодакцизнаяПродукцияЕГАИС = СтрокаТаблицы.ЕстьПодакцизный;
			КонецЕсли;
		КонецЕсли;
		
		ТекущееОписание.КлючВидовПродукции = Справочники.КлючиВидовПродукцииИС.КлючПоЗначениюПеречисления(
			ВидПродукции,
			ДанныеКеша,
			ЭтоПодакцизнаяПродукцияЕГАИС,,
			Истина);
		
		Если Не ЗначениеЗаполнено(ТекущееОписание.КлючВидовПродукции) Тогда
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Не удалось определить ключ видов продукции для вида номенклатуры ""%1"" при переходе на конструктор настроек видов продукции'"),
				СтрокаТаблицы.Ссылка);
		КонецЕсли;
	...
	ЗаписатьДанныеОписанияНоменклатурыПриПереходеНаКонструкторНастроекВидовПродукции(НаборЗаписей);
	
	ОбновитьПовторноИспользуемыеЗначения();
	
КонецПроцедуры

// Особенности учета ИС.
// 
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.ОсобенностиУчетаНоменклатуры - Особенности учета БГосИС
Функция ОсобенностиУчетаИС() Экспорт
	
	Массив = Новый Массив;
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьТабачнаяПродукция);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьОбувнаяПродукция);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьЛегкаяПромышленность);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьЛегкаяПромышленность2025);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьМолочнаяПродукцияПодконтрольнаяВЕТИС);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьМолочнаяПродукцияБезВЕТИС);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьШины);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьФотоаппараты);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьВелосипеды);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьКреслаКоляски);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьДухи);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьАльтернативныйТабак);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьУпакованнаяВода);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьАнтисептики);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьБАДы);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьНикотиносодержащаяПродукция);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьПиво);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьБезалкогольноеПиво);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьСоковаяПродукция);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьМорепродуктыПодконтрольныеВЕТИС);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьКормаДляЖивотныхБезВЕТИС);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьКормаДляЖивотныхПодконтрольныеВЕТИС);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьКормаДляЖивотныхВлажныеБезВЕТИС);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьКормаДляЖивотныхВлажныеПодконтрольныеВЕТИС);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьМясоПодконтрольноеВЕТИС);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьВетеринарныеПрепараты);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьИгрыИИгрушкиДляДетей);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьРадиоэлектроннаяПродукция);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьТитановаяМеталлопродукция);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьКонсервированнаяПродукцияБезВЕТИС);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьКонсервированнаяПродукцияПодконтрольнаяВЕТИС);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьРастительныеМасла);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьОптоволокноИОптоволоконнаяПродукция);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьПарфюмерныеИКосметическиеСредстваИБытоваяХимия);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьПечатнаяПродукция);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьОтопительныеПриборы);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьСтроительныеМатериалы);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьПиротехническиеИзделияИСредстваПожарнойБезопасности);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьАлкогольнаяПродукцияДо9Процентов);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьТелефоныИНоутбуки);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьКабельнаяПродукция);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьБакалея);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьМоторныеМасла);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьАвтозапчастиИКомплектующиеТранспортныхСредств);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьБезалкогольныеНапитки);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьПивоВПотребительскихУпаковках);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьТехническиеСредстваРеабилитации);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьМедицинскиеИзделия);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьМедицинскиеИзделия20);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьПолимерныеТрубы);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьСладостиИКондитерскиеИзделия);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьПечатныеПлаты);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьДезинфицирующиеСредства);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьБАДы2025);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьСпортивноеПитание);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьРастворимыеЗавариваемыеНапитки);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьТоварыЛичнойГигиены);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьБритвыИЛезвия);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьНапиткиССодержаниемМолокаКакаоПрочие);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьПродукцияИзНатуральногоМеха);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьАлкогольнаяПродукция);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьПодконтрольнаяПродукцияВЕТИС);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьЗерноВЕТИС);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьПродуктыПереработкиЗернаВЕТИС);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьАлкогольнаяПродукция);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьЗерно);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьЗерноВЕТИС);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьПродуктыПереработкиЗерна);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьПодконтрольнаяПродукцияСАТУРН);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьТоварыИзНатуральногоМеха);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УдалитьЛегкаяПромышленность2026);
...
// Переопределить текст запроса динамического списка номенклатуры для отбора по виду продукции.
// 
// Параметры:
//  ИсходныйТекстЗапроса - Строка - исходный текс запроса динамического списка
//  ВидыПродукцииИС - Массив из СправочникСсылка.ВидыПродукцииИС
Процедура ПереопределитьТекстЗапросаДинамическогоСпискаНоменклатурыДляОтбораПоВидуПродукции(ИсходныйТекстЗапроса, ВидыПродукцииИС) Экспорт
	
	УстановитьФильтрГИСМТ  = Ложь;
	УстановитьФильтрЗЕРНО  = Ложь;
	УстановитьФильтрВЕТИС  = Ложь;
	УстановитьФильтрСАТУРН = Ложь;
	УстановитьФильтрЕГАИС  = Ложь;
	...
	Для Каждого ВидПродукции Из ВидыПродукцииИС Цикл
		...
		Если ТипЗнч(ВидПродукции) = Тип("СправочникСсылка.ТоварныеГруппыГИСМТ") Тогда
			ВидыПродукцииПоТоварнойГруппе = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ВидПродукции);
			Для Каждого ВидПродукцииПоТоварнойГруппе Из ВидыПродукцииПоТоварнойГруппе Цикл
				УстановитьФильтрВЕТИС = УстановитьФильтрВЕТИС Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияВЕТИС(ВидПродукцииПоТоварнойГруппе);
				УстановитьФильтрЗЕРНО = УстановитьФильтрЗЕРНО Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияЗЕРНО(ВидПродукцииПоТоварнойГруппе);
				УстановитьФильтрСАТУРН = УстановитьФильтрСАТУРН Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияСАТУРН(ВидПродукцииПоТоварнойГруппе);
				УстановитьФильтрЕГАИС = УстановитьФильтрЕГАИС Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(ВидПродукцииПоТоварнойГруппе);
				УстановитьФильтрГИСМТ = УстановитьФильтрГИСМТ Или КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(ВидПродукцииПоТоварнойГруппе, Истина);
				ВидыПродукции.Добавить(ВидПродукцииПоТоварнойГруппе);
			КонецЦикла;
		Иначе
			УстановитьФильтрВЕТИС = УстановитьФильтрВЕТИС Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияВЕТИС(ВидПродукции);
			УстановитьФильтрЗЕРНО = УстановитьФильтрЗЕРНО Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияЗЕРНО(ВидПродукции);
			УстановитьФильтрСАТУРН = УстановитьФильтрСАТУРН Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияСАТУРН(ВидПродукции);
			УстановитьФильтрЕГАИС = УстановитьФильтрЕГАИС Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(ВидПродукции);
			УстановитьФильтрГИСМТ = УстановитьФильтрГИСМТ Или КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(ВидПродукции, Истина);
			ВидыПродукции.Добавить(ВидПродукции);
		КонецЕсли;
		
	КонецЦикла;
	
	Если Не УстановитьФильтрВЕТИС
		И Не УстановитьФильтрЗЕРНО
		И Не УстановитьФильтрСАТУРН
		И Не УстановитьФильтрЕГАИС
		И Не УстановитьФильтрГИСМТ Тогда
		Возврат;
	КонецЕсли;
	
	Подзапросы  = Новый Массив;
	Если УстановитьФильтрВЕТИС Тогда
		Подзапросы.Добавить(
			"		ВЫБРАТЬ
			|			ОписаниеНоменклатурыИС.Номенклатура КАК Ссылка,
			|			ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка) КАК Свойство,
			|			ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииВЕТИС КАК Значение
			|		ИЗ
			|			РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
			|		ГДЕ
			|			ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииВЕТИС <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
			|");
	КонецЕсли;
	Если УстановитьФильтрЗЕРНО Тогда
		Подзапросы.Добавить(
			"		ВЫБРАТЬ
			|			ОписаниеНоменклатурыИС.Номенклатура КАК Ссылка,
			|			ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка) КАК Свойство,
			|			ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО КАК Значение
			|		ИЗ
			|			РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
			|		ГДЕ
			|			ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
			|");
	КонецЕсли;
	Если УстановитьФильтрСАТУРН Тогда
		Подзапросы.Добавить(
			"		ВЫБРАТЬ
			|			ОписаниеНоменклатурыИС.Номенклатура КАК Ссылка,
			|			ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка) КАК Свойство,
			|			ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииСАТУРН КАК Значение
			|		ИЗ
			|			РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
			|		ГДЕ
			|			ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииСАТУРН <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
			|");
	КонецЕсли;
	Если УстановитьФильтрЕГАИС Тогда
		Подзапросы.Добавить(
			"		ВЫБРАТЬ
			|			ОписаниеНоменклатурыИС.Номенклатура КАК Ссылка,
			|			ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка) КАК Свойство,
			|			ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЕГАИС КАК Значение
			|		ИЗ
			|			РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
			|		ГДЕ
			|			ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЕГАИС <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
			|");
	КонецЕсли;
	Если УстановитьФильтрГИСМТ Тогда
		Подзапросы.Добавить(
			"		ВЫБРАТЬ
			|			ОписаниеНоменклатурыИС.Номенклатура КАК Ссылка,
			|			ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка) КАК Свойство,
			|			ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ КАК Значение
			|		ИЗ
			|			РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
			|		ГДЕ
			|			ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
			|");
	КонецЕсли;
	
	Подзапрос = СтрСоединить(Подзапросы, Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС);
	
	ТекстЗапроса = "
	|{ХАРАКТЕРИСТИКИ
	|	ТИП(Справочник.Номенклатура)
	|	ВИДЫХАРАКТЕРИСТИК (ВЫБРАТЬ
	|			ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка) КАК Ссылка,
	|			""ВидПродукцииИС"" КАК Наименование,
	|			ТИП(Справочник.ВидыПродукцииИС) КАК ТипЗначения)
	|	ПОЛЕКЛЮЧА Ссылка
	|	ПОЛЕИМЕНИ Наименование
	|	ПОЛЕТИПАЗНАЧЕНИЯ ТипЗначения
	|	ЗНАЧЕНИЯХАРАКТЕРИСТИК (" + Подзапрос + ")
	|	ПОЛЕОБЪЕКТА Ссылка
	|	ПОЛЕВИДА Свойство
	|	ПОЛЕЗНАЧЕНИЯ Значение}";
	
	ИсходныйТекстЗапроса = ИсходныйТекстЗапроса + Символы.ПС + ТекстЗапроса;
	
КонецПроцедуры

// Возвращает параметры отбора номенклатуры по виду продукции.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - форма клиентского приложения
// 
// Возвращаемое значение:
//  Структура - параметры отбора номенклатуры по виду продукции:
//   * ВидыПродукцииИС - Неопределено
//                     - Массив из СправочникСсылка.ВидыПродукцииИС
//   * ВидСравненияОтбора - ВидСравненияКомпоновкиДанных
Функция ПараметрыОтбораНоменклатурыПоВидуПродукции(Форма) Экспорт
	
	ВозвращаемоеЗначение = Новый Структура();
	ВозвращаемоеЗначение.Вставить("ВидыПродукцииИС",    Неопределено);
	ВозвращаемоеЗначение.Вставить("ВидСравненияОтбора", ВидСравненияКомпоновкиДанных.ВСписке);
	
	ОтборПоМаркируемойПродукции = Неопределено;
	ВидыПродукцииИС             = Неопределено;
	Если Форма.Параметры.Свойство("МаркируемаяПродукция", ОтборПоМаркируемойПродукции)
		И ТипЗнч(ОтборПоМаркируемойПродукции) = Тип("Булево") Тогда
		ВозвращаемоеЗначение.ВидыПродукцииИС = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.УчитываемыеВидыМаркируемойПродукции();
		Если Не ОтборПоМаркируемойПродукции Тогда
			ВозвращаемоеЗначение.ВидСравненияОтбора = ВидСравненияКомпоновкиДанных.НеВСписке;
		КонецЕсли;
		Возврат ВозвращаемоеЗначение;
	ИначеЕсли Форма.Параметры.Свойство("Отбор")
		И Форма.Параметры.Отбор.Свойство("ВидыПродукцииИС", ВидыПродукцииИС) Тогда
		ВозвращаемоеЗначение.ВидыПродукцииИС = ВидыПродукцииИС;
		Возврат ВозвращаемоеЗначение;
	КонецЕсли;
	
КонецФункции

// Установит отбор номенклатуры по виду продукции ИС.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
//  ВидыПродукции - Массив из СправочникСсылка.ВидыПродукцииИС -
//  ВидСравненияОтбора - ВидСравненияКомпоновкиДанных
Процедура УстановитьОтборНоменклатурыПоВидуПродукцииИС(Форма, ВидыПродукции, ВидСравненияОтбора) Экспорт
	...
	ПодборТоваровСервер.ДобавитьЭлементОтбора(
		...
		"Ссылка.ВидПродукцииИС", ВидыПродукции, ВидСравненияОтбора, "ОтборВидПродукцииИС");
	
КонецПроцедуры

// Заменить дубли ключей видов продукции ИС.
Процедура ЗаменитьДублиКлючейВидовПродукцииИС() Экспорт
	
	ДублиКлючей = Справочники.КлючиВидовПродукцииИС.ДублиКлючейВидовПродукции();
	
	Если ДублиКлючей.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ОбщегоНазначенияУТ.ЗаменитьСсылки(ДублиКлючей);
	СписокСсылок = Новый Массив;
	Для Каждого КлючИЗначение Из ДублиКлючей Цикл
	...
	Для Каждого Значение Из СписокСсылок Цикл
		
		СтруктураПоискаПоСсылкам = Новый Структура("Ссылка", Значение);
		ПоискПоСсылкам = РезультатыПоиска.НайтиСтроки(СтруктураПоискаПоСсылкам);
		
		Если ПоискПоСсылкам.Количество() = 0 Тогда
			СправочникОбъект = Значение.ПолучитьОбъект();
			...
			Исключение
				...
				НСтр("ru = 'При пометке на удаление дубля ключа видов продукции ИС %1 возникла ошибка:
				           |Текст ошибки: %2'"),
				СправочникОбъект,
				...
				ОбщегоНазначенияИСВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
					ТекстОшибки,
					НСтр("ru='Обмен данными'", ОбщегоНазначения.КодОсновногоЯзыка()),
					Метаданные.Справочники.КлючиВидовПродукцииИС,
					Значение);
			КонецПопытки;
		КонецЕсли;
...
Процедура ЗаполнитьДанныеПоШтрихкодамEAN(ДанныеПоШтрихкодамEAN, ПараметрыСканирования) Экспорт
	...
	РегистрыСведений.ШтрихкодыНоменклатуры.ПолучитьДанныеПоШтрихкодам(КэшированныеЗначения, ШтрихкодыEAN, Истина);
	...
	Для Каждого КлючЗначение Из КэшированныеЗначения.Штрихкоды Цикл
		...
		Если КлючЗначение.Значение <> Неопределено Тогда
			Если ТипЗнч(КлючЗначение.Значение) = Тип("Массив") Тогда
				Для Каждого ДанныеПоШтрихкоду Из КлючЗначение.Значение Цикл
					Если ЗначениеЗаполнено(ДанныеПоШтрихкоду.Номенклатура) Тогда
						СписокНоменклатуры.Добавить(ДанныеПоШтрихкоду.Номенклатура);
					КонецЕсли;
				КонецЦикла;
			Иначе
				ДанныеПоШтрихкоду = КлючЗначение.Значение;
	...
	Если ПараметрыСканирования <> Неопределено И ПараметрыСканирования.ЕГАИС Тогда
		УсловиеЕГАИС = "(ЕГАИС.Значение
	|	И ЕСТЬNULL(ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЕГАИС,
	|		ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
	|	И ОписаниеНоменклатурыИС.Номенклатура.ВидАлкогольнойПродукции.Маркируемый)";
	Иначе 
		УсловиеЕГАИС = "ЛОЖЬ";
	КонецЕсли;
	
	Если ПараметрыСканирования <> Неопределено И ПараметрыСканирования.ИСМП Тогда
		УсловиеИСМП = "(ИСМП.Значение
	|	И ЕСТЬNULL(ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ,
	|		ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка))";
	Иначе 
		УсловиеИСМП = "ЛОЖЬ";
	КонецЕсли;
	...
	|	ОписаниеНоменклатурыИС.КлючВидовПродукции КАК ВидПродукции,
	|	&УсловиеЕГАИС ИЛИ &УсловиеИСМП КАК МаркируемаяПродукция
	|ИЗ
	|	Справочник.Номенклатура КАК спрНоменклатура
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО ОписаниеНоменклатурыИС.Номенклатура = спрНоменклатура.Ссылка,
	|	Константа.ВестиСведенияДляДекларацийПоАлкогольнойПродукции КАК ЕГАИС,
	|	Константа.ВестиУчетМаркируемойПродукцииИСМП КАК ИСМП
	|ГДЕ
	|	спрНоменклатура.Ссылка В(&СписокНоменклатуры)";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеЕГАИС", УсловиеЕГАИС);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеИСМП", УсловиеИСМП);
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодамEAN Цикл
		...
		Для Каждого ШтрихкодПоиска Из ШтрихкодыПоиска Цикл
			РезультатПолученияДанных = КэшированныеЗначения.Штрихкоды.Получить(ШтрихкодПоиска);
			
			Если РезультатПолученияДанных = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			Если ТипЗнч(РезультатПолученияДанных) = Тип("Массив") Тогда
				Для Каждого ДанныеПоШтрихкоду Из РезультатПолученияДанных Цикл
					ЗаполнитьДанныеПоШтрихкоду(ДанныеПоШтрихкоду, СведенияОНоменклатуре, СтрокаДанных,
						ДанныеПоШтрихкодамРезультат);
				КонецЦикла;
			Иначе
				ДанныеПоШтрихкоду = РезультатПолученияДанных;
		...
		РезультатПолученияДанных = КэшированныеЗначения.Штрихкоды.Получить(ВРег(СтрокаДанных.ШтрихкодEAN));
...
Процедура ПриСозданииНаСервереФормыШтрихкодаНоменклатуры(Форма, Параметры) Экспорт
	...
	НовыйРеквизит = Новый РеквизитФормы(
		...
		НСтр("ru = 'Количество потребительских упаковок ГИС МТ'"));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	НовыйРеквизит = Новый РеквизитФормы(
		"ВидУпаковкиИС",
		Новый ОписаниеТипов("ПеречислениеСсылка.ВидыУпаковокИС"),,
		НСтр("ru = 'Вид упаковки ГИС МТ'"));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	ТипыВидаПродукции = Новый Массив;
	ТипыВидаПродукции.Добавить(Тип("СправочникСсылка.ВидыПродукцииИС"));
	ТипыВидаПродукции.Добавить(Тип("СправочникСсылка.КлючиВидовПродукцииИС"));
	НовыйРеквизит = Новый РеквизитФормы(
		"ВидПродукцииИС",
		Новый ОписаниеТипов(ТипыВидаПродукции),,
		НСтр("ru = 'Вид продукции ИС'"));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	
	Форма.ИзменитьРеквизиты(ДобавляемыеРеквизиты);
	
	НовыйЭлемент = Форма.Элементы.Добавить("ВидУпаковкиИС", Тип("ПолеФормы"));
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "ВидУпаковкиИС";
	...
	НовыйЭлемент = Форма.Элементы.Добавить("КоличествоПотребительскихУпаковокИС", Тип("ПолеФормы"));
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "КоличествоПотребительскихУпаковокИС";
	НовыйЭлемент.УстановитьДействие("ПриИзменении", "Подключаемый_ПриИзмененииЭлемента");
	
КонецПроцедуры

Процедура ПриСозданииНаСервереФормыСпискаШтрихкодаНоменклатуры(Форма, Параметры) Экспорт
	
	ОтборНоменклатура = Неопределено;
	Если Не ПолучитьФункциональнуюОпцию("ВестиУчетМаркируемойПродукцииИСМП")
		Или Не Параметры.Свойство("Номенклатура", ОтборНоменклатура)
		Или Не ЗначениеЗаполнено(ОтборНоменклатура) Тогда
		Возврат;
	КонецЕсли;
	
	Элементы = Форма.Элементы;
	
	Форма.Список.ТекстЗапроса = СтрЗаменить(
		Форма.Список.ТекстЗапроса,
		"ШтрихкодыНоменклатуры.Упаковка КАК Упаковка",
		"ШтрихкодыНоменклатуры.Упаковка КАК Упаковка,
		|ЗНАЧЕНИЕ(Перечисление.ВидыУпаковокИС.ПустаяСсылка) КАК ВидУпаковкиИС,
		|ВЫРАЗИТЬ(0 КАК ЧИСЛО(5, 0)) КАК КоличествоПотребительскихУпаковокИС");
	
	НовыйЭлемент = Форма.Элементы.Добавить("ВидУпаковкиИС", Тип("ПолеФормы"), Форма.Элементы.Список);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Список.ВидУпаковкиИС";
	НовыйЭлемент.Заголовок = НСтр("ru = 'Вид упаковки ГИС МТ'");
	
	НовыйЭлемент = Форма.Элементы.Добавить("КоличествоПотребительскихУпаковокИС", Тип("ПолеФормы"), Форма.Элементы.Список);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Список.КоличествоПотребительскихУпаковокИС";
	НовыйЭлемент.Заголовок = НСтр("ru = 'Количество потребительских упаковок ГИС МТ'");
	
	ДоступныеВидыПродукции = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.УчитываемыеВидыМаркируемойПродукции();
	
	ВидыУпаковок = Новый Массив;
	ВидыУпаковок.Добавить(Перечисления.ВидыУпаковокИС.Потребительская);
	ВидыУпаковок.Добавить(Перечисления.ВидыУпаковокИС.Групповая);
	...
	Для Каждого ВидПродукции Из ДоступныеВидыПродукции Цикл
		...
		Для Каждого ВидУпаковки Из ВидыУпаковок Цикл
			...
			ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных("ВидУпаковкиИС");
			
			ПредставлениеВидаУпаковки = КонструкторИСМПОбщегоНазначенияКлиентСервер.ПредставлениеВидаУпаковкиПоВидуПродукции(
				ВидПродукции, ВидУпаковки);
			
			Если Не ЗначениеЗаполнено(ПредставлениеВидаУпаковки) Тогда
				ПредставлениеВидаУпаковки = Строка(ВидУпаковки);
			КонецЕсли;
			
			ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Список.ВидУпаковкиИС");
			ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
			ОтборЭлемента.ПравоеЗначение = ВидУпаковки;
			
			ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Список.ВидПродукцииГИСМТ");
			ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.ВСписке;
			ОтборЭлемента.ПравоеЗначение = ВидПродукции;
		
			Элемент.Оформление.УстановитьЗначениеПараметра("Текст", ПредставлениеВидаУпаковки);
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных("КоличествоПотребительскихУпаковокИС");
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Список.КоличествоПотребительскихУпаковокИС");
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
	...
	ВидПродукцииИС = ОбщегоНазначенияИС.ОписаниеНоменклатуры(
		ОтборНоменклатура)[ОтборНоменклатура].КлючВидовПродукции;
	
	ВидимостьВидаУпаковки = ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(ВидПродукцииИС, Истина);
	Элементы.ВидУпаковкиИС.Видимость                       = ВидимостьВидаУпаковки;
	Элементы.КоличествоПотребительскихУпаковокИС.Видимость = ВидимостьВидаУпаковки;
...
Процедура ПриПолученииДанныхСпискаШтрихкодовНоменклатурыНаСервере(ИмяЭлемента, Настройки, Строки) Экспорт
	...
	ЕстьПолеВидУпаковки = Неопределено;
	ЕстьПолеКоличество = Неопределено;
	
	Для Каждого СтрокаДанных Из Строки Цикл
		Если ЕстьПолеВидУпаковки = Неопределено Тогда
			ЕстьПолеВидУпаковки = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
				СтрокаДанных.Значение.Данные,
				"ВидУпаковкиИС");
		КонецЕсли;
		Если ЕстьПолеКоличество = Неопределено Тогда
			ЕстьПолеКоличество = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
				СтрокаДанных.Значение.Данные,
				"КоличествоПотребительскихУпаковокИС");
		КонецЕсли;
		Если Не ЕстьПолеВидУпаковки И Не ЕстьПолеКоличество Тогда
			Прервать;
		КонецЕсли;
		ДанныеСтроки = СтрокаДанных.Значение.Данные;
		GTIN = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.GTINПоШтрихкодуEAN(ДанныеСтроки.Штрихкод);
	...
	ОписанияGTIN = РегистрыСведений.ОписаниеGTINИС.ПолучитьОписание(МассивGTIN);
	...
	Для Каждого СтрокаДанных Из Строки Цикл
		...
		Если ОписаниеGTIN = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Если ЕстьПолеВидУпаковки Тогда
			ДанныеСтроки.ВидУпаковкиИС = ОписаниеGTIN.ВидУпаковки;
		КонецЕсли;
		Если ЕстьПолеКоличество Тогда
			ДанныеСтроки.КоличествоПотребительскихУпаковокИС = ОписаниеGTIN.КоличествоПотребительскихУпаковок;
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗаполнитьПараметрыФормыШтрихкодаНоменклатуры(Форма) Экспорт
	...
	Форма.ВидПродукцииИС = ОбщегоНазначенияИС.ОписаниеНоменклатуры(Запись.Номенклатура)[Запись.Номенклатура].КлючВидовПродукции;
	
	GTIN = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.GTINПоШтрихкодуEAN(Запись.Штрихкод);
	
	ОписаниеGTIN = РегистрыСведений.ОписаниеGTINИС.ПолучитьОписание(GTIN)[GTIN];
	Если ОписаниеGTIN <> Неопределено Тогда
		Форма.ВидУпаковкиИС                       = ОписаниеGTIN.ВидУпаковки;
		Форма.КоличествоПотребительскихУпаковокИС = ОписаниеGTIN.КоличествоПотребительскихУпаковок;
	КонецЕсли;
	
КонецПроцедуры

Процедура НастроитьЭлементыФормыШтрихкодаНоменклатуры(Форма) Экспорт
...
Процедура ПриЗаписиНаСервереШтрихкодаНоменклатуры(Форма, Отказ, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	ДанныеОписания = РегистрыСведений.ОписаниеGTINИС.ИнициализироватьДанныеОписания();
	ДанныеОписания.GTIN = GTIN;
	ДанныеОписания.ВидУпаковки = Форма.ВидУпаковкиИС;
	ДанныеОписания.КоличествоПотребительскихУпаковок = Форма.КоличествоПотребительскихУпаковокИС;
	
	НачатьТранзакцию();
	
	Попытка
		РегистрыСведений.ОписаниеGTINИС.УстановитьОписание(ДанныеОписания);
		ЗафиксироватьТранзакцию();
	Исключение
		
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Ошибка записи описания GTIN'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		
		ОбщегоНазначения.СообщитьПользователю(
			СтрШаблон(
				НСтр("ru = 'Ошибка записи описания GTIN по причине: %1'"),
...
// Текст запроса реализация товаров услуг.
// 
// Возвращаемое значение:
//  Строка - Текст запроса реализация товаров услуг
Функция ТекстЗапросаРеализацияТоваровУслуг() Экспорт
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|	ПО ОписаниеНоменклатурыИС.Номенклатура = ТаблицаТовары.Номенклатура
	|	И (ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукции)
	|	ИЛИ ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЕГАИС В (&ВидыПродукции)
	|	ИЛИ ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииВЕТИС В (&ВидыПродукции)
	|	ИЛИ ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииСАТУРН В (&ВидыПродукции)
	|	ИЛИ ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО В (&ВидыПродукции))
	|ГДЕ
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|	ПО ОписаниеНоменклатурыИС.Номенклатура = ТаблицаСерии.Номенклатура
	|	И (ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукции)
	|	ИЛИ ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЕГАИС В (&ВидыПродукции)
	|	ИЛИ ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииВЕТИС В (&ВидыПродукции)
	|	ИЛИ ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииСАТУРН В (&ВидыПродукции)
...
// Текст запроса отгрузка товаров клиенту.
// 
// Возвращаемое значение:
//  Строка - Текст запроса отгрузка товаров клиенту
Функция ТекстЗапросаОтгрузкаТоваровКлиенту() Экспорт
	
	ТекстЗапроса = ТекстЗапросаРеализацияТоваровУслуг();
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РеализацияТоваровУслуг", "ОтгрузкаТоваровКлиенту");
	Возврат ТекстЗапроса;
	
КонецФункции

// Текст запроса внутреннее потребление.
// 
// Возвращаемое значение:
//  Строка - Текст запроса внутреннее потребление
...
// Текст запроса передача товаров хранителю.
// 
// Возвращаемое значение:
//  Строка - Текст запроса передача товаров хранителю
...
// Текст запроса перемещение товаров.
// 
// Возвращаемое значение:
//  Строка - Текст запроса перемещение товаров
...
// Текст запроса сборка товаров.
// 
// Возвращаемое значение:
//  Строка - Текст запроса сборка товаров
...
// Текст запроса возврат товаров поставщику.
// 
// Возвращаемое значение:
//  Строка - Текст запроса возврат товаров поставщику
...
// Текст запроса отчет о розничных продажах.
// 
// Возвращаемое значение:
//  Строка - Текст запроса отчет о розничных продажах
Функция ТекстЗапросаОтчетОРозничныхПродажах() Экспорт
	...
	|	ИЛИ ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииСАТУРН В (&ВидыПродукции)
	|	ИЛИ ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО В (&ВидыПродукции))
	|	И (ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииПартионнаяПродукция)
	|	ИЛИ ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЕГАИС В (&ВидыПродукцииПартионнаяПродукция)
	|	ИЛИ ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииВЕТИС В (&ВидыПродукцииПартионнаяПродукция)
	|	ИЛИ ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииСАТУРН В (&ВидыПродукцииПартионнаяПродукция)
	|	ИЛИ ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО В (&ВидыПродукцииПартионнаяПродукция))
	|
	|ГДЕ
	|	ТаблицаТовары.Ссылка В (&Документ)
	|СГРУППИРОВАТЬ ПО
...
// Текст запроса корректировка реализации.
// 
// Параметры:
//  УчетОтрицательныхРасхождений - Булево - Учет отрицательных расхождений
// 
// Возвращаемое значение:
//  Строка - Текст запроса корректировка реализации
...
//  ШтрихкодыУпаковокИсключить - ТаблицаЗначений - Уже использованные штрихкоды упаковок в документах по распоряжению
//                             - Неопределено - если отсутствуют
Процедура ЗаполнитьШтрихКодыУпаковок(РаспоряженияДляОрдеров, Товары, ШтрихкодыУпаковок, ШтрихкодыУпаковокИсключить = Неопределено) Экспорт
...
//++ Локализация
	...
	Если ТаблицаШтрихкодыУпаковок.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	Если ШтрихкодыУпаковокИсключить = Неопределено Тогда
		ШтрихкодыУпаковокИсключить = Новый Массив;
	КонецЕсли;
	
	Дополнение = Новый Структура("ИзОрдера", Истина);
	...
	КонецЦикла;
	Для Каждого Строка Из ШтрихкодыУпаковокИсключить Цикл
		ВсеШтрихкоды.Вставить(Строка.ШтрихкодУпаковки, Истина);
	КонецЦикла;
	...
	Для Каждого Строка Из Товары Цикл
		...
		Для каждого Марка Из НайденныеОтгруженныеМарки Цикл
			...
			Если ВсеШтрихкоды.Получить(Марка.Ссылка) = Истина Тогда
				Продолжить;
			КонецЕсли;
			
			ВсеШтрихкоды.Вставить(Марка.Ссылка, Истина);
			СтрокаТабМарки = ШтрихкодыУпаковок.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТабМарки, Строка);
			ЗаполнитьЗначенияСвойств(СтрокаТабМарки, Дополнение);
			СтрокаТабМарки.ШтрихкодУпаковки = Марка.Ссылка;
...
Функция ШтрихкодыПоРаспоряжению(Распоряжения)
	...
	|///////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Штрихкоды.ШтрихкодУпаковки КАК Ссылка,
	...
	СоставШтрихкодов = ЭлектронноеВзаимодействиеИСМП.ЧастичноеСодержимое(ШтрихкодыУпаковок, Ложь);
	Возврат СоставШтрихкодов;
	
КонецФункции

Процедура ПеренестиШтрихкодыУпаковок(Форма, ШтрихкодыУпаковок, ИмяТЧ) Экспорт
	...
	Если Не ИнтеграцияИСУТКлиентСервер.ЕстьПострочныйСтатусПроверкиВФорме(Форма) Тогда
		Возврат;
	КонецЕсли;
...
// Заполнить штрихкоды упаковок по акту о расхождениях.
// 
// Параметры:
//  ДокументОбъект - ДокументОбъект.ОтгрузкаТоваровСХранения, ДокументОбъект.ПередачаТоваровХранителю - Заполняемый документ
//  АктОРасхождениях - ДокументСсылка.АктОРасхожденияхПослеОтгрузки - Акт о расхождениях
Процедура ЗаполнитьШтрихкодыУпаковокПоАктуОРасхождениях(ДокументОбъект, АктОРасхождениях) Экспорт
	...
	|ГДЕ
	|	АктОРасхожденияхПослеОтгрузкиШтрихкодыУпаковок.Ссылка = &АктОРасхождениях";
...
Функция ДанныеПеревозки(Основание, ТранспортнаяНакладная) Экспорт
	...
	Иначе
		...
		|
		|ВЫБРАТЬ
		|	ОтгрузкаТоваровКлиенту.ЗаказКлиента
		|ИЗ
		|	Документ.ОтгрузкаТоваровКлиенту КАК ОтгрузкаТоваровКлиенту
		|ГДЕ
		|	ОтгрузкаТоваровКлиенту.Ссылка = &Распоряжение
		|	И ОтгрузкаТоваровКлиенту.ЗаказКлиента <> ЗНАЧЕНИЕ(Документ.ЗаказКлиента.ПустаяСсылка)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|ГДЕ
		|	ОтгрузкаТоваровКлиентуТовары.Ссылка = &Распоряжение
		|	И ОтгрузкаТоваровКлиентуТовары.ЗаказКлиента <> ЗНАЧЕНИЕ(Документ.ЗаказКлиента.ПустаяСсылка)
		|	И ОтгрузкаТоваровКлиентуТовары.Ссылка.ЗаказКлиента = ЗНАЧЕНИЕ(Документ.ЗаказКлиента.ПустаяСсылка)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
...
Процедура ЗаполнитьПоПараметрамСоздания(Объект, ПараметрыСоздания) Экспорт
	...
	Если (ПараметрыСоздания.Свойство("ВидАлкогольнойПродукцииЕГАИС")
		...
		Если Не ЗначениеЗаполнено(Объект.ВидНоменклатуры) Тогда
			...
			|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
			|	ПО ВидыНоменклатуры.Ссылка = ОписаниеНоменклатурыИС.Номенклатура
			|	И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЕГАИС В (&ВидыПродукцииЕГАИС)
			|
			|УПОРЯДОЧИТЬ ПО
			|	ВидАлкогольнойПродукцииСовпадает УБЫВ";
			Запрос.УстановитьПараметр("ВидАлкогольнойПродукции", Объект.ВидАлкогольнойПродукции);
			Если ПараметрыСоздания.МаркируемаяПродукцияЕГАИС = Ложь Тогда
	...
	Если Не ЗначениеЗаполнено(Объект.ВидНоменклатуры)
		...
		|	Справочник.ВидыНоменклатуры КАК ВидыНоменклатуры
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		|	ПО ОписаниеНоменклатурыИС.Номенклатура = ВидыНоменклатуры.Ссылка
		|ГДЕ
		|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииВЕТИС <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)";
...
Процедура ДополнитьПараметрыУказанияСерийФормыУточнениеСоставаУпаковкиИС(Параметры, Объект) Экспорт
	...
	НовыйРеквизит = Новый РеквизитФормы("ТорговыйОбъект", Новый ОписаниеТипов("СправочникСсылка.Склады"));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	...
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	НовыйРеквизит = Новый РеквизитФормы("СтатусУказанияСерийОтгруженныхТоваров", Новый ОписаниеТипов("Число"), "ДанныеДляУточнения");
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
...
#Область ДокументЗаказКлиента

// Параметры:
//	Форма - ФормаКлиентскогоПриложения
Процедура МодифицироватьИнициализироватьФормуЗаказКлиента(Форма) Экспорт
	
	Если ПроверитьМодификацияФормыВыполнена(Форма) Тогда
		ИнициализироватьФормуЗаказКлиента(Форма);
		Возврат;
	КонецЕсли;
	...
	Если ИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции() Тогда
		...
		ПараметрыИнтеграцииГосИС = Новый Соответствие(Форма.ПараметрыИнтеграцииГосИС);
		ПроверкаИПодборПродукцииИС.ДополнитьТаблицуТоваров(
			Форма, ПараметрыИнтеграцииГосИС, "Объект.Товары", ДобавляемыеРеквизиты);
...
#Область ДокументЗаявкаНаВозвратТоваровОтКлиента

// Параметры:
//	Форма - ФормаКлиентскогоПриложения
Процедура МодифицироватьИнициализироватьФормуЗаявкаНаВозвратТоваровОтКлиента(Форма) Экспорт
	
	Если ПроверитьМодификацияФормыВыполнена(Форма) Тогда
		ИнициализироватьФормуЗаявкаНаВозвратТоваровОтКлиента(Форма);
		Возврат;
	КонецЕсли;
...
Процедура ИнициализироватьФормуЗаявкаНаВозвратТоваровОтКлиента(Форма) Экспорт
	
	СтруктураДействий = Новый Структура;
	ПакетнаяОбработкаТабличнойЧастиКлиентСерверЛокализация.ДополнитьСтруктуруДействийПриИзмененииЭлемента(
		Форма, "НоменклатураПризнаки", СтруктураДействий);
	НоменклатураСервер.ЗаполнитьСлужебныеРеквизитыПоНоменклатуреВКоллекции(Форма.Объект.ЗаменяющиеТовары, СтруктураДействий);
	
КонецПроцедуры

#КонецОбласти

#Область ПодборТоваровВДокументПродажи

// Параметры:
//	Форма - ФормаКлиентскогоПриложения
Процедура МодифицироватьИнициализироватьФормуПодборТоваровВДокументПродажи(Форма) Экспорт
	
	Если ПроверитьМодификацияФормыВыполнена(Форма) Тогда
		ИнициализироватьФормуПодборТоваровВДокументПродажи(Форма);
		Возврат;
	КонецЕсли;
	...
	Если ИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции() Тогда
		...
		ПараметрыИнтеграцииГосИС = Новый Соответствие(Форма.ПараметрыИнтеграцииГосИС);
		ПроверкаИПодборПродукцииИС.ДополнитьТаблицуТоваров(Форма, ПараметрыИнтеграцииГосИС, "Объект.Корзина", ДобавляемыеРеквизиты);
...
Процедура ИнициализироватьФормуПодборТоваровВДокументПродажи(Форма) Экспорт
	
	СтруктураДействий = Новый Структура;
	ПакетнаяОбработкаТабличнойЧастиКлиентСерверЛокализация.ДополнитьСтруктуруДействийПриИзмененииЭлемента(Форма, "НоменклатураПризнаки", СтруктураДействий);
	НоменклатураСервер.ЗаполнитьСлужебныеРеквизитыПоНоменклатуреВКоллекции(Форма.Объект.Корзина, СтруктураДействий);
...
#Область Реквизиты
	...
	Если ЕстьЕГАИС Или ЕстьИСМП Тогда
		...
		ПараметрыИнтеграцииГосИС.Вставить(
			"ПроверкаИПодбор",
			ПроверкаИПодборПродукцииИС.ПараметрыИнтеграцииФормыПроверкиИПодбора(Форма));
	...
	Если Форма.ИмяФормы = "Документ.ЧекККМ.Форма.ФормаДокументаРМК" Тогда
		...
		Реквизит = Новый РеквизитФормы("ПродаетсяПоРазрешению",
...
// Добавляет на форму документа ЧекККМКоррекции реквизиты ГосИС, инициализирует их и добавляет условное оформление элементов формы.
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения - ФормаДокументаРМК
//   НастроитьУсловноеОформление - Булево - 
//
Процедура МодифицироватьИнициализироватьФормуЧекаКоррекции(Форма, НастроитьУсловноеОформление = Ложь) Экспорт
		
	ЕстьЕГАИС = ПолучитьФункциональнуюОпцию("ВестиСведенияДляДекларацийПоАлкогольнойПродукции");
	ЕстьИСМП  = ИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции();
	
	Если Не ПроверитьМодификацияФормыВыполнена(Форма) Тогда
	...
	#Область Реквизиты
		...
		Реквизит = Новый РеквизитФормы("СохраненВыборПоМаркируемойПродукции", Новый ОписаниеТипов("Булево"));
		ДобавляемыеРеквизиты.Добавить(Реквизит);
		Реквизит = Новый РеквизитФормы("УсловноеОформлениеДляМаркируемойПродукцииНастроено", Новый ОписаниеТипов("Булево"));
		ДобавляемыеРеквизиты.Добавить(Реквизит);
	...
	#Область Инициализация
		...
		ИнициализироватьФормуЧекаКоррекции(Форма, ЕстьЕГАИС, ЕстьИСМП);
		
	#КонецОбласти

	КонецЕсли;
	
	Если (ЕстьЕГАИС Или ЕстьИСМП) 
		И НастроитьУсловноеОформление = Истина 
		И Не Форма.УсловноеОформлениеДляМаркируемойПродукцииНастроено Тогда
	...
	#Область Интерфейс
		...
		ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ЭлементыФормы.ТоварыНоменклатура.Имя);
		ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ЭлементыФормы.ТоварыХарактеристика.Имя);
		ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ЭлементыФормы.ТоварыСерия.Имя);
		ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ЭлементыФормы.ТоварыГруппаКолонокСерия.Имя);
		ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ЭлементыФормы.ТоварыКомитент.Имя);
		
		ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ВидКоррекции");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = Перечисления.ВидыЧековКоррекции.ИсправлениеОшибки;

		ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Товары.МаркируемаяПродукция");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	...
	#КонецОбласти
		Форма.УсловноеОформлениеДляМаркируемойПродукцииНастроено = Истина;
...
Процедура ИнициализироватьФормуЧекаКоррекции(Форма, ЕстьЕГАИС, ЕстьИСМП)
...
#Область РеализацияТоваровУслугПереходПраваСобственности

// Обновить интерфейс реализация товаров услуг по операции.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - Форма
Процедура ОбновитьИнтерфейсРеализацияТоваровУслугПоОперации(Форма) Экспорт
	
	Видимость = Форма.Объект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ПереходПраваСобственности;
	
	ИмяПоля = "СоответствиеТребованиямГИСМТГосИС";
	Если Форма.Элементы.Найти(ИмяПоля) <> Неопределено Тогда
		Форма.Элементы[ИмяПоля].Видимость = Видимость И ЗначениеЗаполнено(Форма.Элементы[ИмяПоля].Заголовок);
	КонецЕсли;
	
	ИмяПоля = "ТекстПерейтиКОбработкеПередачиТоваровМеждуОрганизациями";
	Если Форма.Элементы.Найти(ИмяПоля) <> Неопределено Тогда
		Видимость = Видимость И ЗначениеЗаполнено(Форма[ИмяПоля]);
		Форма.Элементы[ИмяПоля].Видимость = Видимость;
	КонецЕсли;
...
Процедура ПроверкаЗаполненияПоВидамУпаковок(Форма, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если ЭтоНоменклатура
		И КонструкторИСМПОбщегоНазначенияКлиентСервер.ПоддерживаетсяПодключениеКОборудованиюРозлива(Форма.ВидПродукцииГИСМТ)
	...
	Если Форма.ИспользуетсяЧастичноеВыбытиеИС Тогда
		...
		ИначеЕсли ЭтоНоменклатура Тогда
			...
			ОсобенностьУчетаИлиКлючВидовПродукции = НоменклатураЛокализация.ОсобенностьУчетаИлиКлючВидовПродукции(Форма);
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru = 'Поле ""%1"" не заполнено.'"),
					ИнтеграцияИСКлиентСервер.ПредставлениеЧастичногоВыбытияПоВидуПродукции(ОсобенностьУчетаИлиКлючВидовПродукции)),,
				"УпаковкаЧастичногоВыбытияИС",,
				Отказ);
		Иначе
...
Процедура ЗаполнитьЗначениеПолейПоВидамУпаковок(Форма) Экспорт
	...
	Форма.ЭтоМернаяЕдиницаИС             = ДанныеНастройкиВидаУпаковки.МернаяЕдиница;
	
	Если Не ЗначениеЗаполнено(Форма.ПотребительскаяУпаковкаИС)
		И Не ДанныеНастройкиВидаУпаковки.МернаяЕдиница
		И Не Форма.ИмяФормы = "Справочник.НаборыУпаковок.Форма.ФормаЭлемента" Тогда
...
Функция НовыеДанныеНастройкиВидаУпаковки()
	...
	ДанныеНастройкиВидаУпаковки.Вставить("КлючВидовПродукции",           Неопределено);
...
Функция ДанныеНастройкиВидаУпаковки(Форма)
	...
	Если ЗначениеЗаполнено(ИсточникДанных) Тогда
		СвойИсточникДляКлюча = ИсточникДанных <> Объект.Ссылка И ЗначениеЗаполнено(Объект.Ссылка);
		Если СвойИсточникДляКлюча Тогда
			Данные = Новый Массив;
			Данные.Добавить(ИсточникДанных);
			Данные.Добавить(Объект.Ссылка);
		Иначе
			Данные = ИсточникДанных;
		КонецЕсли;
		
		ДанныеОписания = ОбщегоНазначенияИС.ОписаниеНоменклатуры(Данные);
		
		ДанныеОписанияУпаковки = ДанныеОписания[ИсточникДанных];
		ДанныеНастройкиВидаУпаковки.ПотребительскаяУпаковка      = ДанныеОписанияУпаковки.ПотребительскаяУпаковка;
		ДанныеНастройкиВидаУпаковки.УпаковкаЧастичногоВыбытия    = ДанныеОписанияУпаковки.УпаковкаЧастичногоВыбытия;
		ДанныеНастройкиВидаУпаковки.ЧастичноеВыбытиеИспользуется = ЗначениеЗаполнено(ДанныеОписанияУпаковки.ВариантЧастичногоВыбытия);
		Если СвойИсточникДляКлюча Тогда
			ДанныеОписанияКлюча = ДанныеОписания[Объект.Ссылка];
			ДанныеНастройкиВидаУпаковки.КлючВидовПродукции = ДанныеОписанияКлюча.КлючВидовПродукции;
		Иначе
			ДанныеНастройкиВидаУпаковки.КлючВидовПродукции = ДанныеОписанияУпаковки.КлючВидовПродукции;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаписатьДанныеПоВидамУпаковок(Форма, ТекущийОбъект, ПараметрыЗаписи, Отказ) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "КлючВидовПродукции") Тогда
		КлючВидовПродукции = Форма.КлючВидовПродукции;
	Иначе
		КлючВидовПродукции = Справочники.КлючиВидовПродукцииИС.ПустаяСсылка();
	КонецЕсли;
	
	ЗаписатьТолькоДанныеНоменклатуры = ТипЗнч(ТекущийОбъект) = Тип("СправочникОбъект.Номенклатура")
		И ТипЗнч(ИсточникОписания) <> Тип("СправочникСсылка.Номенклатура");
	
	Если ЗаписатьТолькоДанныеНоменклатуры Тогда
		Если ДанныеНастройки.КлючВидовПродукции = КлючВидовПродукции Тогда
			Возврат;
		КонецЕсли;
		ИсточникОписания = ТекущийОбъект.Ссылка;
	КонецЕсли;
	...
	Попытка
		...
		ДанныеОписания.КлючВидовПродукции                  = КлючВидовПродукции;
		
		Если Не ЗаписатьТолькоДанныеНоменклатуры Тогда
			...
			Если Форма.ЭтоМернаяЕдиницаИС Тогда
				Если ЗначениеЗаполнено(Форма.ПотребительскаяУпаковкаИС) Тогда
					...
					ДанныеОписания.ВариантИспользованияЕдиницыХранения = Перечисления.ВариантыИспользованияЕдиницыХраненияИС.МернаяПродукцияСФиксированнымЗначением;
				Иначе
			...
			Иначе
				...
				ДанныеОписания.УпаковкаЧастичногоВыбытия = Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка();
			КонецЕсли;
		КонецЕсли;
...
Процедура ИспользованиеЭлементов(ВидимостьЭлементов, Объект, РежимВидимостиПоказатьТолькоВажные, РежимПроверкиИспользованияРеквизитов) Экспорт
	...
	Если Не РежимПроверкиИспользованияРеквизитов Тогда
		...
		КлючВидовПродукции = НоменклатураЛокализация.ОсобенностьУчетаИлиКлючВидовПродукции(Объект);
		Если ТипЗнч(КлючВидовПродукции) = Тип("ПеречислениеСсылка.ОсобенностиУчетаНоменклатуры") Тогда
			КлючВидовПродукции = Справочники.КлючиВидовПродукцииИС.ПустаяСсылка();
		КонецЕсли;
		
		ДоступноЧастичноеВыбытие = ОбщегоНазначенияИСКлиентСерверПовтИсп.ПоддерживаетсяЧастичноеВыбытие(КлючВидовПродукции);
		ИспользутсяУпаковки      = (ПолучитьФункциональнуюОпцию("ИспользоватьУпаковкиНоменклатуры") И Объект.ИспользоватьУпаковки);
		
		ЭтоМаркируемаяАлкогольнаяПродукция = КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(КлючВидовПродукции);
		ЭтоПродукцияИСМП = КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(КлючВидовПродукции);
...
Процедура ДобавитьКомандыНастройкиИнтеграцииСФГИСВФормуСпискаНоменклатуры(Форма) Экспорт
	
	Если Не ОбщегоНазначенияИС.ВедетсяУчетПродукцииИС()
		Или Не ПравоДоступа("Просмотр",       Метаданные.ОбщиеФормы.НастройкаИнтеграцииНоменклатурыИС)
		Или Не ПравоДоступа("Редактирование", Метаданные.Справочники.Номенклатура) Тогда
		Возврат;
	КонецЕсли;
	
	ЭлементыФормы = Форма.Элементы;
	
	ИмяКомандыНастройкаИнтеграцииНоменклатурыИС = "НастройкаИнтеграцииНоменклатурыИС";
	КомандаФормы = Форма.Команды.Добавить(ИмяКомандыНастройкаИнтеграцииНоменклатурыИС);
	КомандаФормы.Действие  = "Подключаемый_ВыполнитьПереопределяемуюКоманду";
	КомандаФормы.Заголовок = НСтр("ru = 'Настроить интеграцию с ФГИС'");
		
	Кнопка = ЭлементыФормы.Вставить(
		"КоманднаяПанельСписокРасширенныйПоискНоменклатураФорма" + ИмяКомандыНастройкаИнтеграцииНоменклатурыИС,
		Тип("КнопкаФормы"),
		ЭлементыФормы.КоманднаяПанельСписокРасширенныйПоискНоменклатура,
		ЭлементыФормы.ГруппаКомандыФормыСписокРасширенныйПоискНоменклатура);
	Кнопка.ИмяКоманды            = ИмяКомандыНастройкаИнтеграцииНоменклатурыИС;
	Кнопка.ТолькоВоВсехДействиях = Ложь;
	
	Кнопка = ЭлементыФормы.Вставить(
		"СписокРасширенныйПоискНоменклатураКонтекстноеМеню" + ИмяКомандыНастройкаИнтеграцииНоменклатурыИС,
		Тип("КнопкаФормы"),
		ЭлементыФормы.СписокРасширенныйПоискНоменклатура.КонтекстноеМеню,
		ЭлементыФормы.СписокРасширенныйПоискНоменклатураКонтекстноеМенюНоменклатураСАналогичнымиСвойствами);
	Кнопка.ИмяКоманды = ИмяКомандыНастройкаИнтеграцииНоменклатурыИС;
	
КонецПроцедуры

Процедура ПроверитьРазделитьПорциюЗаписиОписанияНоменклатурыПриПереходеНаКонструкторНастроекВидовПродукции(НаборЗаписей)
	
	КоличествоВПорции = 10000;
	
	Если НаборЗаписей.Количество() >= КоличествоВПорции Тогда
		ЗаписатьДанныеОписанияНоменклатурыПриПереходеНаКонструкторНастроекВидовПродукции(НаборЗаписей);
		НаборЗаписей = РегистрыСведений.ОписаниеНоменклатурыИС.СоздатьНаборЗаписей();
	КонецЕсли;
...
Процедура ЗаписатьДанныеОписанияНоменклатурыПриПереходеНаКонструкторНастроекВидовПродукции(НаборЗаписей)
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ШаблонСообщения = НСтр("ru = 'Не удалось обработать номенклатуру при переходе на конструктор настроек видов продукции по причине:
							         |%1'");
		ТекстСообщения = СтрШаблон(ШаблонСообщения,
			ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ТекстСообщенияПодробно = СтрШаблон(ШаблонСообщения,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(
			ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Ошибка,
			Метаданные.РегистрыСведений.ОписаниеНоменклатурыИС,,
...
// Выполняет заполнение результирующей таблицы данных штрихкодов EAN.
//
// Параметры:
//	ДанныеПоШтрихкоду - Структура -
//	СведенияОНоменклатуре - ТаблицаЗначений - 
//	ДанныеEAN - СтрокаТаблицыЗначений - 
//	ТаблицаДанныхШтрихкодовEAN - ТаблицаЗначений -
//
Процедура ЗаполнитьДанныеПоШтрихкоду(ДанныеПоШтрихкоду, СведенияОНоменклатуре, ДанныеEAN, ТаблицаДанныхШтрихкодовEAN)
	
	Если Не ЗначениеЗаполнено(ДанныеПоШтрихкоду.Номенклатура) Тогда
		Возврат;
	КонецЕсли;
	
	НоваяСтрока = ТаблицаДанныхШтрихкодовEAN.Добавить();
	...
	НоваяСтрока.Количество	= ?(ДанныеEAN.Весовой, ДанныеEAN.Количество, ДанныеПоШтрихкоду.Коэффициент);
	НоваяСтрока.ШтрихкодEAN	= ДанныеEAN.ШтрихкодEAN;
