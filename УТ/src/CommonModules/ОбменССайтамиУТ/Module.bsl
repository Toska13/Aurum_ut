...
Процедура ЗаполнитьТабличнуюЧастьДокумента(Документ, ТаблицаТоваровСайт, ПрикладныеПараметры) Экспорт
	...
	СтруктураПересчетаЦены  = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруПересчетаЦеныСкидкиВПродажахВТЧ(Документ, 
	...
	СтруктураДействий.Вставить(
		...
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияСкладаВСтрокеТЧ(Документ, Ложь));
	...
	ТаблицаТоварыДокумента = Документ.Товары.Выгрузить();
	ТаблицаТоварыДокумента.Колонки.Добавить("ТипНоменклатуры", Новый ОписаниеТипов("ПеречислениеСсылка.ТипыНоменклатуры"));
	...
	ВсяНоменклатура = ТаблицаТоваровСайт.ВыгрузитьКолонку("Номенклатура");
	Наборы = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ВсяНоменклатура, "ТипНоменклатуры");
	...
	Для Каждого ТекСтрока Из ТаблицаТоваровСайт Цикл
		...
		ЭтоНабор = Наборы.Получить(ТекСтрока.Номенклатура) = Перечисления.ТипыНоменклатуры.Набор;
		...
		Если ЭтоНабор Тогда
			...
			СоставНаборов.Вставить(ТекСтрока, Новый Массив());
			
			Для Каждого ПодобранныйТовар Из ПодобранныеТовары Цикл
				НоваяСтрока = ТаблицаТоварыДокумента.Добавить();
				НоваяСтрока.НомерСтроки = НомерСтроки;
				НоваяСтрока.СтавкаНДС = СтавкаНДСПоЗначениюПриЗагрузке(СоответствиеСтавокНДС, ТекСтрока.СтавкаНДС);
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ПодобранныйТовар);
				МассивСтрокНабора = СоставНаборов.Получить(ТекСтрока);
	...
	СтрокиВсехНаборов = Новый Массив();
	
	Для Каждого Набор Из СоставНаборов Цикл
		Для Каждого Комплектующие Из Набор.Значение Цикл
			СтрокиВсехНаборов.Добавить(Комплектующие);
		КонецЦикла;
	КонецЦикла;
	
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		СтрокиВсехНаборов,
		СтруктураДействийНабор,
		ТаблицаТоварыДокумента,
		КэшированныеЗначения);
	
	Для Каждого Набор Из СоставНаборов Цикл
		СуммаНабора = 0; 
		НоменклатураНабора = Набор.Ключ;
		НоменклатураСумма = НоменклатураНабора.Сумма;
		
		Для Каждого Комплектующие Из Набор.Значение Цикл
			СуммаНабора = СуммаНабора + Комплектующие.Сумма; 
		КонецЦикла;
		
		КоэффициентПересчета = НоменклатураСумма / ?(СуммаНабора = 0, 1, СуммаНабора);
		Если КоэффициентПересчета = 1 Тогда
			Продолжить
		КонецЕсли;
		
		ПогрешностьСтоимости = НоменклатураСумма;
		Для Каждого Комплектующие Из Набор.Значение Цикл
			
			СтрокаДляИзменения = ТаблицаТоварыДокумента.Найти(Комплектующие.НомерСтроки, "НомерСтроки");
			Если КоэффициентПересчета <> 1 Тогда
				СтрокаДляИзменения.Сумма = Комплектующие.Сумма * КоэффициентПересчета;
			КонецЕсли;
			
			ПогрешностьСтоимости = ПогрешностьСтоимости - Комплектующие.Сумма;
			
		КонецЦикла;
		
		Если ПогрешностьСтоимости <> 0 Тогда
			СтрокаДляИзменения.Сумма = СтрокаДляИзменения.Сумма + ПогрешностьСтоимости;
		КонецЕсли;
		
	КонецЦикла;
	
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ТаблицаТоварыДокумента, СтруктураДействий);
	
	Документ.Товары.Загрузить(ТаблицаТоварыДокумента);
...
//
Функция СтавкаНДСПоЗначениюПриЗагрузке(СоответствиеСтавокНДС, СтавкаНалогаСтрока) Экспорт
	
	Если ТипЗнч(СтавкаНалогаСтрока) = Тип("Число") Тогда
		СтавкаНалогаСтрока = Строка(СтавкаНалогаСтрока);
	КонецЕсли;
	
	СтавкаНДС = СоответствиеСтавокНДС.Получить(СтавкаНалогаСтрока);
...
// Метод выполняет проверку значения режима замещения 
...
Процедура АдаптироватьЗначениеЗамещенияРегистра(Замещение) Экспорт
	...
	Если ЭтаВерсияНижеБСП_3_1_11
		ИЛИ Замещение = Неопределено
		ИЛИ ТипЗнч(Замещение) = Тип("Булево") Тогда
	...
	ЭтоРасширенныйРежимЗамещения = МодульОбщегоНазначения.ЭтоЗамещениеНабораЗаписей(Замещение)
		ИЛИ МодульОбщегоНазначения.ЭтоОбновлениеНабораЗаписей(Замещение)
		ИЛИ МодульОбщегоНазначения.ЭтоСлияниеНабораЗаписей(Замещение)
		ИЛИ МодульОбщегоНазначения.ЭтоУдалениеНабораЗаписей(Замещение);
