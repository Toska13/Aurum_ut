...
Функция ДанныеВходящегоОбъекта(ТранспортныйКонтейнер,
	...
	ДанныеКонтейнера.Вставить("ДанныеТЭДО", Неопределено);
	...
	ИначеЕсли ЕстьСвойство(ЭД, "ИдФайл") Тогда
		...
		ИначеЕсли ТипЭлементаВерсииЭД = "ИнформацияПокупателяУПД" Тогда
			Если ДокументЭД.ВерсФорм = "5.01" Тогда
				ДокументооборотИД = ПолучитьИдДокументооборотаПоИдФайла(
					ДокументЭД.ИнфПок.ИдИнфПрод.ИдФайлИнфПр, ДокументооборотИД);
			Иначе
				ДокументооборотИД = ПолучитьИдДокументооборотаПоИдФайла(
					ДокументЭД.Документ.ИдИнфПрод.ИдФайлИнфПр, ДокументооборотИД);
			КонецЕсли;
...
Процедура ЗаполнитьИзДанныхКонтракта(Контракт, Объект, ЭтоКонтрактДляЗаказчика = Ложь) Экспорт
	...
	Объект.ТипСопровождения = ?(Объект.ТребуетсяБанковскоеСопровождение, 
		Перечисления.ТипыСопровожденияГосударственныхКонтрактов.Банковское,
		Перечисления.ТипыСопровожденияГосударственныхКонтрактов.Казначейское);
	...
	ЕстьОбособленноеПодразделениеПоставщика = Ложь;
	Если ЭтоКонтрактДляЗаказчика Тогда
		...
		Если ЗначениеЗаполнено(ДанныеКонтрагента.КППОбособленногоПодразделенияПоставщика) Тогда
			ЕстьОбособленноеПодразделениеПоставщика = Истина;
			Если НЕ ЗначениеЗаполнено(Объект.ГоловнаяОрганизацияПоставщика) Тогда
				Объект.ГоловнаяОрганизацияПоставщика = ЭлектронноеАктированиеЕИС.
					НайтиСоздатьКонтрагентаПоСведениямОЗаказчике(ДанныеКонтрагента);
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(Объект.Контрагент) Тогда
				ДанныеКонтрагента.СокращенноеНаименование =
					ДанныеКонтрагента.ПолноеНаименованиеОбособленногоПодразделенияПоставщика;
				ДанныеКонтрагента.ПолноеНаименование =
					ДанныеКонтрагента.ПолноеНаименованиеОбособленногоПодразделенияПоставщика;
				ДанныеКонтрагента.КПП = 
					ДанныеКонтрагента.КППОбособленногоПодразделенияПоставщика;
	...
	Если НЕ ЭтоКонтрактДляЗаказчика ИЛИ НЕ ЕстьОбособленноеПодразделениеПоставщика Тогда
		Если НЕ ЗначениеЗаполнено(Объект.Контрагент) Тогда
			ДанныеКонтрагента = СведенияОКонтрагентеИзСведенийОКонтракте(Контракт, ЭтоКонтрактДляЗаказчика);
			Объект.Контрагент = ЭлектронноеАктированиеЕИС.
				НайтиСоздатьКонтрагентаПоСведениямОЗаказчике(ДанныеКонтрагента);
	...
	Если Контракт.СведенияОПоставщиках.Количество() > 0 Тогда
		...
		Объект.ПолноеНаименованиеОбособленногоПодразделенияПоставщика
			= ДанныеПоставщика.ПолноеНаименованиеОбособленногоПодразделенияПоставщика;
		Объект.КППОбособленногоПодразделенияПоставщика
			= ДанныеПоставщика.КППОбособленногоПодразделенияПоставщика;
...
Функция ПолучитьШапкуПакетаЕИСЗаказчика(ОбъектЭД,
	...
	ИначеЕсли ТипЭлементаВерсииЭД = ТипЭлементаВерсииИнформацияПоставщикаУПД()
		...
		ИдентификаторыУчастников = ИзвлечьИдентификаторыУчастниковИзИдФайл(Результат.ИдФайл);
		Результат.Отправитель = ИдентификаторыУчастников.Отправитель;
		Результат.Получатель = ИдентификаторыУчастников.Получатель;
		Если ТипЭлементаВерсииЭД = ТипЭлементаВерсииИнформацияПоставщикаУПД()
			...
			Если НЕ ЭтоУКД Тогда
				Если ОбъектЭД.ВерсФорм = "5.03" Тогда
					Результат.Дата = СтроковыеФункцииКлиентСервер.СтрокаВДату(
						ОбъектЭД.Документ.СвСчФакт.ДатаДок);
				Иначе
					// Ранее использовался формат УПД 5.01.
					Результат.Дата = СтроковыеФункцииКлиентСервер.СтрокаВДату(
...
Процедура ПолучитьДокументыЗаказчика(
	...
	КодОсновногоЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
	ИмяСобытия = НСтр("ru = 'Получение из ЕИС документов заказчика'", КодОсновногоЯзыка);
	ТекстОшибки = "";
		...
		Для Каждого ОписаниеДокумента Из МассивОписанийДокументов Цикл
			...
			Если НЕ РезультатПолученияЭД.Выполнено Тогда
				...
				ЗаписьЖурналаРегистрации(ИмяСобытия,
					УровеньЖурналаРегистрации.Ошибка, , ,
					ТекстОшибкиЖурнал);
				Если СтрНайти(РезультатПолученияЭД.ОписаниеОшибки, "Документ с идентификатором") > 0
					И СтрНайти(РезультатПолученияЭД.ОписаниеОшибки, "не найден") > 0 Тогда
...
Функция ВидОшибкиСбойПриПолученииДокументов(Идентификатор, ЗаголовокПроблемы, ТекстОшибки)
	...
	ВидОшибки.ОписаниеПроблемы = НСтр("ru = 'Произошла ошибка:'") + Символы.ПС + ТекстОшибки;
...
Процедура ЗаполнитьНоменклатуруКонтракта(Контракт, Объект)
	...
	Для Каждого СтрокаЭД Из Контракт.ОбъектыЗакупки Цикл
		...
		СтавкиНДС.Вставить("5", 0.05);
		СтавкиНДС.Вставить("7", 0.07);
		...
		Если Ставка = Неопределено Тогда
			Попытка
				Ставка = Число(СтрокаЭД.СтавкаНДС) / 100;
			Исключение
				Ставка = Неопределено;
			КонецПопытки;
		КонецЕсли;
		...
		Наименование = ВРег(СтрЗаменить(Лев(СтрокаЭД.Наименование, 255), " ", "")
...
Функция ТекстЗапросаДляПоискаСопоставленнойНоменклатурыКонтракта()
	...
	|		ПО (НоменклатураКонтрагентовБЭД.ВладелецНоменклатуры = &Владелец)
...
// Функция извлекает идентификаторы отправителя и получателя из строки
// Параметры:
//  СтрокаИмени - Строка - в формате ON_NSCHFDOPPR_2ZK-CUS-01484000040_2ZK-SUP-00020000040_...
// Возвращаемое значение:
//  Структура - с полями Отправитель и Получатель
//   * Отправитель - Строка                          - идентификатор отправителя.
//   * Получатель  - Строка                          - идентификатор получателя.
Функция ИзвлечьИдентификаторыУчастниковИзИдФайл(СтрокаИмени) Экспорт
	
	Результат = Новый Структура("Отправитель, Получатель",
		"", "");
	МассивЭлементов = СтрРазделить(СтрокаИмени, "_");
	...
	Результат.Отправитель = ?(СтрНайти(Врег(МассивЭлементов[2]), "SUP") = 0, МассивЭлементов[3], МассивЭлементов[2]);
	Результат.Получатель = ?(СтрНайти(Врег(МассивЭлементов[3]), "CUS") = 0, МассивЭлементов[2], МассивЭлементов[3]);
