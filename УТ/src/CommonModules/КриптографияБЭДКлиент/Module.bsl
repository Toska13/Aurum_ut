...
Процедура ПолучитьОтпечаткиСертификатов(Оповещение) Экспорт
	КлючЗамера = ОбщегоНазначенияБЭДКлиент.НачатьЗамер("ПолучитьХранилищеСертификатов.Клиент");
	ОповещениеОЗавершенииЗамера = ОбщегоНазначенияБЭДКлиент.ОповещениеОЗавершенииЗамера(КлючЗамера, Оповещение);
	
	КриптографияБЭДСлужебныйКлиент.ПолучитьОтпечаткиСертификатов(ОповещениеОЗавершенииЗамера, Истина);
...
Процедура Подписать(ОписаниеДанных, КонтекстДиагностики, Форма = Неопределено, Оповещение = Неопределено,
	...
	Если ЭтоТипПодписиCAdESBES(ТипПодписи) Тогда
		КлючЗамера = "КриптографияБЭД.Подписать.Клиент.Базовая";
	Иначе
		КлючЗамера = "КриптографияБЭД.Подписать.Клиент.Усовершенствованная";
	КонецЕсли;
	
	КоличествоДанных = 1;
	Если ОписаниеДанных.Свойство("НаборДанных") И ТипЗнч(ОписаниеДанных.НаборДанных) = Тип("Массив") Тогда
		КоличествоДанных = ОписаниеДанных.НаборДанных.Количество();
	КонецЕсли;
	...
	ОповещениеОЗавершенииЗамера = ОбщегоНазначенияБЭДКлиент.ОповещениеОЗавершенииЗамераДлительнойОперации(КлючЗамера, 
		ОписаниеЗамера, КоличествоДанных, Оповещение);
	...
	Контекст.Вставить("Оповещение", ОповещениеОЗавершенииЗамера);
...
Процедура ПроверитьСертификат(ОбработчикРезультатаПроверки, Сертификат, КонтекстДиагностики, Подпись = Неопределено,
	...
	КлючЗамера = ОбщегоНазначенияБЭДКлиент.НачатьЗамер("КриптографияБЭД.ПроверитьСертификат.Клиент");
	ОповещениеОЗавершенииЗамера =  ОбщегоНазначенияБЭДКлиент.ОповещениеОЗавершенииЗамера(КлючЗамера,
		ОбработчикРезультатаПроверки);
	КриптографияБЭДСлужебныйКлиент.ПроверитьСертификат(ОповещениеОЗавершенииЗамера, Сертификат, КонтекстДиагностики,
...
Процедура Расшифровать(ОписаниеДанных, КонтекстДиагностики, Форма = Неопределено, Оповещение = Неопределено,
	...
	КлючЗамера = ОбщегоНазначенияБЭДКлиент.НачатьЗамер("КриптографияБЭД.Расшифровать.Клиент");
	ОповещениеОЗавершенииЗамера = ОбщегоНазначенияБЭДКлиент.ОповещениеОЗавершенииЗамера(КлючЗамера,
		СлужебноеОповещение);
#Если Не МобильныйКлиент Тогда
	ЭлектроннаяПодписьКлиент.Расшифровать(ОписаниеДанных, Форма, ОповещениеОЗавершенииЗамера);
#Иначе
	МобильныйЭДОКлиент.Расшифровать(ОписаниеДанных, Форма, ОповещениеОЗавершенииЗамера);
#КонецЕсли
...
Процедура ПроверитьПодпись(ОповещениеОЗавершении, ИсходныеДанные, Подпись, КонтекстДиагностики, МенеджерКриптографии = Неопределено) Экспорт
	...
	КлючЗамера = "КриптографияБЭД.ПроверитьПодпись.Клиент";
	ОписаниеЗамера = ОбщегоНазначенияБЭДКлиент.НачатьЗамерДлительнойОперации(КлючЗамера);
	
	ОповещениеОЗавершенииЗамера = ОбщегоНазначенияБЭДКлиент.ОповещениеОЗавершенииЗамераДлительнойОперации(КлючЗамера,
		ОписаниеЗамера, 1, ОповещениеОЗавершении);
	...
	Контекст.ОповещениеОЗавершении = ОповещениеОЗавершенииЗамера;
...
Процедура НачатьРасшифровкуНабораДанных(Оповещение, НаборДанныхДляРасшифровки, ПараметрыРасшифровки = Неопределено) Экспорт
	...
	КлючЗамера = "КриптографияБЭД.НачатьРасшифровкуНабораДанных.Клиент";
	ОписаниеЗамера = ОбщегоНазначенияБЭДКлиент.НачатьЗамерДлительнойОперации(КлючЗамера);
	КоличествоДанных = НаборДанныхДляРасшифровки.Количество();
	ОповещениеОЗавершенииЗамера = ОбщегоНазначенияБЭДКлиент.ОповещениеОЗавершенииЗамераДлительнойОперации(КлючЗамера,
		ОписаниеЗамера, КоличествоДанных, Оповещение);
	КонтекстРасшифровки = КриптографияБЭДСлужебныйКлиент.НовыйКонтекстРасшифровкиНабораДанных();
	КонтекстРасшифровки.ОповещениеОЗавершении = ОповещениеОЗавершенииЗамера;
...
// см. КриптографияБЭДСлужебныйКлиент.ОткрытьФормуРасширенногоПредставленияОшибки
Процедура ОткрытьФормуРасширенногоПредставленияОшибки(КонтекстДиагностики, 
	ДополнительныеПараметры = Неопределено) Экспорт
	
	КриптографияБЭДСлужебныйКлиент.ОткрытьФормуРасширенногоПредставленияОшибки(КонтекстДиагностики, 
		ДополнительныеПараметры);

КонецПроцедуры

// Конец ЭлектронноеВзаимодействие.ОбменСКонтрагентами.МашиночитаемыеДоверенности
