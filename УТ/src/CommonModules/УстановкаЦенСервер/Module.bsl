	...
	Для Каждого ВидЦены Из Форма.ВыбранныеЦены Цикл
		...
		Если ВидЦены.Выбрана Или ВидЦены.Влияет Тогда
			...
			Иначе
				...
				Если ИспользуетсяЦенообразование25 Тогда
					ДеревоЗначений.Колонки.Добавить("ВариантУказанияСтавокНДС"			+ ИмяКолонки, Новый ОписаниеТипов("ПеречислениеСсылка.ВариантыУказанияСтавокНДСВПрайсЛисте"), ВидЦены.Наименование);
					ДеревоЗначений.Колонки.Добавить("Страна"							+ ИмяКолонки, Новый ОписаниеТипов("СправочникСсылка.СтраныМира"), ВидЦены.Наименование);
					ДеревоЗначений.Колонки.Добавить("СтавкаНДС"							+ ИмяКолонки, Новый ОписаниеТипов("СправочникСсылка.СтавкиНДС"), НСтр("ru = 'Ставка НДС'"));
					ДеревоЗначений.Колонки.Добавить("ИсходнаяСтавкаНДС"					+ ИмяКолонки, Новый ОписаниеТипов("СправочникСсылка.СтавкиНДС"), ВидЦены.Наименование);
					ДеревоЗначений.Колонки.Добавить("ИзмененаВручнуюСтавкаНДС"			+ ИмяКолонки, Новый ОписаниеТипов("Булево"), ВидЦены.Наименование);
					ДеревоЗначений.Колонки.Добавить("ИзмененаАвтоматическиСтавкаНДС"	+ ИмяКолонки, Новый ОписаниеТипов("Булево"), ВидЦены.Наименование);
					ДеревоЗначений.Колонки.Добавить("КартинкаИзмененаВручнуюСтавкаНДС"	+ ИмяКолонки, Новый ОписаниеТипов("Число"),  ВидЦены.Наименование);
				КонецЕсли;
	...
	Если ТипЗнч(Форма.ДеревоЦен) = Тип("ДанныеФормыДерево") Или ТипЗнч(Форма.ДеревоЦен) = Тип("ДанныеФормыКоллекция") Тогда
		...
		Для Каждого ВидЦены Из Форма.ВыбранныеЦены Цикл
			...
			Если ВидЦены.Выбрана
				...
				Если ПодборТоваровКлиентСервер.ЭтоФормаПрайсЛиста(Форма.КодФормы)
					Или ПодборТоваровКлиентСервер.ЭтоФормаПрайсЛистаПоставщиков(Форма.КодФормы) Тогда
				...
				Если Не ЭтоРегистрацияЦенПоставщика
					...
					УказыватьОбщуюСтавкуНДС = ВидЦены.ВариантУказанияСтавокНДС = Перечисления.ВариантыУказанияСтавокНДСВПрайсЛисте.УказыватьОбщуюСтавкуНДС;
					УказыватьСтавкиНДСПострочно = ВидЦены.ВариантУказанияСтавокНДС = Перечисления.ВариантыУказанияСтавокНДСВПрайсЛисте.УказыватьСтавкиНДСПострочно;
					Если УказыватьОбщуюСтавкуНДС ИЛИ УказыватьСтавкиНДСПострочно Тогда
					
						НоваяГруппаСтавкиНДС = ДобавитьГруппуФормы(Форма, "ГруппаСтавкаНДС" + ИмяКолонки, НСтр("ru='Ставка НДС'"), Ложь, Неопределено, ГруппировкаКолонок.ВЯчейке, НоваяГруппа);
						
						Если УказыватьСтавкиНДСПострочно Тогда
							СтруктураПараметров = Новый Структура();
							СтруктураПараметров.Вставить("ШиринаПоля",             2);
							СтруктураПараметров.Вставить("Родитель",               НоваяГруппаСтавкиНДС);
							СтруктураПараметров.Вставить("ПутьКДанным",            "ДеревоЦен.ИзмененаВручнуюСтавкаНДС" + ИмяКолонки);
							СтруктураПараметров.Вставить("ТолькоПросмотрПоля",     Истина);
							СтруктураПараметров.Вставить("ОтображатьВШапке",       Ложь);
							СтруктураПараметров.Вставить("Вид",                    ВидПоляФормы.ПолеКартинки);
							СтруктураПараметров.Вставить("КартинкаШапки",          БиблиотекаКартинок.УстановитьЦены);
							СтруктураПараметров.Вставить("КартинкаЗначений",       БиблиотекаКартинок.УстановитьЦены);
							СтруктураПараметров.Вставить("Подсказка",              НСтр("ru = 'Ставка НДС изменена вручную'"));
							СтруктураПараметров.Вставить("Видимость",              ВидЦены.Редактировать);
							
							НовоеПоле = ДобавитьПолеФормы(Форма, "СтавкаНДСКартинка" + ИмяКолонки, СтруктураПараметров);
							НовоеПоле.Заголовок = "";
							НовоеПоле.ПоложениеЗаголовка  = ПоложениеЗаголовкаЭлементаФормы.Нет;
						КонецЕсли;
						...
						НовыйМассив = Новый Массив();
						НовыйМассив.Добавить(Новый СвязьПараметраВыбора("Страна", "Элементы.ДеревоЦен.ТекущиеДанные.Страна" + ИмяКолонки, РежимИзмененияСвязанногоЗначения.НеИзменять));
						
						Если ЭтоПрайсЛист(Форма) Тогда
							
							НовыйМассив.Добавить(Новый СвязьПараметраВыбора("НачалоПериода", "Дата", РежимИзмененияСвязанногоЗначения.НеИзменять));
							Если ЗначениеЗаполнено(Форма.ДатаЗавершенияПериодаДействияЦен) Тогда
								НовыйМассив.Добавить(Новый СвязьПараметраВыбора("КонецПериода", "ДатаЗавершенияПериодаДействияЦен", РежимИзмененияСвязанногоЗначения.НеИзменять));
							КонецЕсли;
							
						ИначеЕсли ЭтоУстановкаЦенНоменклатуры(Форма) Тогда
							
							НовыйМассив.Добавить(Новый СвязьПараметраВыбора("НачалоПериода", "Объект.Дата", РежимИзмененияСвязанногоЗначения.НеИзменять));
						...
						СтруктураПараметров = Новый Структура();
						СтруктураПараметров.Вставить("Заголовок",              НСтр("ru='Ставка НДС'"));
						СтруктураПараметров.Вставить("ОбработчикПриИзменении", "Подключаемый_ДеревоЦенСтавкаНДСПриИзменении");
						СтруктураПараметров.Вставить("ШиринаПоля",             8);
						СтруктураПараметров.Вставить("ЦветФона",               ЦветФона);
						СтруктураПараметров.Вставить("ЦветФонаЗаголовка",      ЦветЗаголовка);
						СтруктураПараметров.Вставить("Родитель",               НоваяГруппаСтавкиНДС);
						СтруктураПараметров.Вставить("СвязиПараметровВыбора",  НовыеСвязи);
						СтруктураПараметров.Вставить("ПутьКДанным",            "ДеревоЦен.СтавкаНДС" + ИмяКолонки);
						СтруктураПараметров.Вставить("ГоризонтальноеПоложение", ГоризонтальноеПоложениеЭлемента.Право);
						СтруктураПараметров.Вставить("ТолькоПросмотрПоля",     НЕ ВидЦены.Редактировать ИЛИ НЕ УказыватьСтавкиНДСПострочно);
					
						НовоеПоле = ДобавитьПолеФормы(Форма, "СтавкаНДС" + ИмяКолонки, СтруктураПараметров);
					КонецЕсли;
				КонецЕсли;
				...
				Если НЕ ЭтоРегистрацияЦенПоставщика и ИспользуетсяЦенообразование25 Тогда
					...
					ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(НовыйЭлементУсловногоОформления.Отбор, "ДеревоЦен.СтавкаНДС" + ИмяКолонки,
							ВидСравненияКомпоновкиДанных.НеРавно, Новый ПолеКомпоновкиДанных("ДеревоЦен.ИсходнаяСтавкаНДС" + ИмяКолонки));
					
					НовоеПоле = НовыйЭлементУсловногоОформления.Поля.Элементы.Добавить();
					НовоеПоле.Использование = Истина;
					НовоеПоле.Поле = Новый ПолеКомпоновкиДанных("ДеревоЦен" + "СтавкаНДС" + ИмяКолонки);
					
					НовыйЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста", WebЦвета.ТемноЗеленый);
					
					// Условное оформление незаполненной ставки НДС
					Если ЭтоУстановкаЦенНоменклатуры(Форма) Тогда
						...
						ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(НовыйЭлементУсловногоОформления.Отбор, "ДеревоЦен.ИндексКартинки",
							ВидСравненияКомпоновкиДанных.Равно, 0);
						ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(НовыйЭлементУсловногоОформления.Отбор, "ДеревоЦен.ХарактеристикиИспользуются",
							ВидСравненияКомпоновкиДанных.Равно, Ложь);
						ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(НовыйЭлементУсловногоОформления.Отбор, "ДеревоЦен.СерииИспользуются",
							ВидСравненияКомпоновкиДанных.Равно, Ложь);
						ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(НовыйЭлементУсловногоОформления.Отбор, "ДеревоЦен.УпаковкиИспользуются",
							ВидСравненияКомпоновкиДанных.Равно, Ложь);
						...
						НовыйЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ОтметкаНезаполненного", Истина);
						
						// Используются - Характеристики, Серии, Упаковки
						НовыйЭлементУсловногоОформления = Форма.УсловноеОформление.Элементы.Добавить();
						НовыйЭлементУсловногоОформления.ИдентификаторПользовательскойНастройки = ИмяКолонки;
						НовыйЭлементУсловногоОформления.Представление = "СозданоПрограммно";
						НовыйЭлементУсловногоОформления.Использование = Истина;
						...
						ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(НовыйЭлементУсловногоОформления.Отбор, "ДеревоЦен.СтавкаНДС" + ИмяКолонки,
							ВидСравненияКомпоновкиДанных.НеЗаполнено);
						ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(НовыйЭлементУсловногоОформления.Отбор, "ДеревоЦен.ИндексКартинки",
							ВидСравненияКомпоновкиДанных.Равно, 2);
						
						ГруппаОтбора = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(НовыйЭлементУсловногоОформления.Отбор, "ГруппаСозданоПрограммно",
							ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
						ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора, "ДеревоЦен.ХарактеристикиИспользуются",
							ВидСравненияКомпоновкиДанных.Равно, Истина);
						ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора, "ДеревоЦен.СерииИспользуются",
							ВидСравненияКомпоновкиДанных.Равно, Истина);
						ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора, "ДеревоЦен.УпаковкиИспользуются",
							ВидСравненияКомпоновкиДанных.Равно, Истина);
							
						ГруппаОтбораНе = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(НовыйЭлементУсловногоОформления.Отбор, "ГруппаНеСозданоПрограммно",
							ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаНе);
						ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораНе, "ДеревоЦен.ХарактеристикиИспользуются",
							ВидСравненияКомпоновкиДанных.Равно, Истина);
						ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораНе, "ДеревоЦен.СерииИспользуются",
							ВидСравненияКомпоновкиДанных.Равно, Истина);
						ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораНе, "ДеревоЦен.УпаковкиИспользуются",
							ВидСравненияКомпоновкиДанных.Равно, Истина);
						ГруппаОтбораИли = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(ГруппаОтбораНе, "ГруппаИлиСозданоПрограммно",
							ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
						ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораИли, "ДеревоЦен.ХарактеристикаЦО",
							ВидСравненияКомпоновкиДанных.НеЗаполнено);
						ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораИли, "ДеревоЦен.СерияЦО",
							ВидСравненияКомпоновкиДанных.НеЗаполнено);
...
Процедура ЗагрузитьНастройкиОтбораПоУмолчанию(Форма) Экспорт
	...
	Если ПодборТоваровКлиентСервер.ЭтоФормаПрайсЛистаПоставщиков(Форма.КодФормы) Тогда
...
Функция ЗагрузитьСправочникВидовЦен(Форма, НастройкиКомпоновкиДанных, МассивВидовЦен = Неопределено, ТаблицаНастроекВидовЦен = Неопределено) Экспорт
	...
	|ПОМЕСТИТЬ ДатыУстановкиЦен
	|ИЗ
	|	&РегистрСведенийЦеныНоменклатурыСрезПоследних КАК ЦеныНоменклатуры
	|СГРУППИРОВАТЬ ПО
	...
	|	ВидыЦен.ВариантУказанияСтавокНДС КАК ВариантУказанияСтавокНДС,
	|	ВидыЦен.Страна КАК Страна,
	|	ВидыЦен.СтавкаНДС КАК СтавкаНДС,
	|	ВидыЦен.СхемаКомпоновкиДанных КАК СхемаКомпоновкиДанных,
	|	ВЫБОР
	|		КОГДА ТаблицаРазрешенных.Ссылка ЕСТЬ NULL
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|		ЕСТЬNULL(НижняяГраницаДиапазонаЦен, 0) КАК НижняяГраницаДиапазонаЦен,
	...
	|	Справочник.ВидыЦен КАК ВидыЦен
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	ТаблицаРазрешенных КАК ТаблицаРазрешенных
	|ПО
	|	ВидыЦен.Ссылка = ТаблицаРазрешенных.Ссылка
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	 ДатыУстановкиЦен КАК ДатыУстановкиЦен
	|ПО
	|	ДатыУстановкиЦен.ВидЦены = ВидыЦен.Ссылка
	...
	Если ТаблицаНастроекВидовЦен <> Неопределено Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДанныеТаблицыНастроек1,", 
			"	ВЫБОР КОГДА ТаблицаНастроекВидовЦен.ВидЦены ЕСТЬ НЕ NULL
	...
	ЕстьРеквизитВидыЦен = ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(Форма.Объект, "ВидыЦен");
	Для Каждого СтрокаВидаЦен Из ТаблицаВидовЦен Цикл
...
Функция ДобавитьТоварыПоставщика(Форма, Знач ТаблицаТовары, КэшДанных) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ТаблицаНоменклатура.Номенклатура.ИспользованиеХарактеристик В 
	...
	Если Не ИспользоватьТаблицуЗначенийВместоДереваЗначений Тогда
		...
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.НоменклатураКонтрагентов.ПустаяСсылка)
	...
	Пока ВыборкаНоменклатура.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаНоменклатура.Номенклатура) Тогда
			...
			Если НайденнаяСтрока <> Неопределено
				Или ТаблицаНоменклатуры.НайтиСтроки(ПараметрыОтбора).Количество() Тогда
			...
			Иначе
				...
				НоваяСтрокаТЗ = НоваяСтрокаТаблицыСоответствия(КэшДанных, ЭлементНоменклатура, Истина);
			...
			Пока ВыборкаХарактеристики.Следующий() Цикл
				...
				Если ЗначениеЗаполнено(ВыборкаХарактеристики.Характеристика) Или НоменклатураНеЗаполнена Тогда
					...
					Иначе
						...
						НоваяСтрокаТЗ = НоваяСтрокаТаблицыСоответствия(КэшДанных, ЭлементХарактеристика, Истина);
...
Процедура ДобавитьТоварыПоОтбору(Форма,
	...
	ТаблицаНедоступныхТоваров = ТоварыНедоступныеДляУстановкиЦен(ТаблицаТовары);
	Если ТаблицаНедоступныхТоваров.Количество() Тогда
		
		ТекстНедоступныеТовары = "";
		
		Для Каждого СтрокаНедоступныйТовар Из ТаблицаНедоступныхТоваров Цикл
			
			ТекстНедоступныеТовары = ТекстНедоступныеТовары
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("- %1 - %2",
					НоменклатураКлиентСервер.ПредставлениеНоменклатуры(
						СтрокаНедоступныйТовар.Номенклатура,
						СтрокаНедоступныйТовар.Характеристика),
					СтрокаНедоступныйТовар.Причина)
				+ Символы.ПС;
			
		КонецЦикла;
		
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Установка цен недоступна для товаров:
				|%1'"),
			СокрЛП(ТекстНедоступныеТовары));
...
Процедура РассчитатьЦены(Форма, ПараметрыРасчета, КэшДанных = Неопределено) Экспорт
	...
	ЗаполнитьСтавкуНДСВТаблицеНоменклатуры(Форма, ТаблицаНоменклатуры, МассивВидовЦен);
...
Процедура ВычислитьЦеныПоДаннымИБ(Форма,
	...
	Иначе
		...
		ВидыЦенПоДаннымИБ = Новый Массив(); // Массив из СправочникСсылка.ВидыЦен, СправочникСсылка.ВидыЦенПоставщиков
		Для Каждого СтрокаВидЦены Из МассивСтрокВидовЦен Цикл
...
Процедура ЗагрузитьЗначенияБазовыхЦен(Форма,
	...
	Если СтруктураПараметров.МассивСтрокВидовЦен = Неопределено Тогда
		...
		СтруктураПараметров.МассивСтрокВидовЦен = Новый Массив(); // Массив из СправочникСсылка.ВидыЦен, СправочникСсылка.ВидыЦенПоставщиков -
		Для Каждого ВидЦены Из Форма.ВыбранныеЦены Цикл
			Если ВидЦены.Влияет Тогда
...
Функция ТаблицаТовары(Форма, КэшДанных = Неопределено, Параметры = Неопределено, ТаблицаНоменклатуры = Неопределено) Экспорт
	...
	Для Каждого СтрокаТЗ Из ТаблицаДанных Цикл
		...
		Если ИерархическийСписок Тогда
			Если ДеревоЦенЭтоДанныеФормыДерево И ТипЗнч(СтрокаДереваЦен) = Тип("ДанныеФормыЭлементДерева") Тогда
				ЕстьПодСтроки = (СтрокаДереваЦен.ПолучитьЭлементы().Количество() > 0);
			ИначеЕсли ТипЗнч(СтрокаДереваЦен) = Тип("СтрокаДереваЗначений") Тогда
				ЕстьПодСтроки = (СтрокаДереваЦен.Строки.Количество() > 0);
			Иначе
				ЕстьПодСтроки = Ложь;
			КонецЕсли;
		...
		Если ТолькоИзмененные Тогда
			Если НЕ ЕстьИзмененияЦенВСтроке(СтрокаДереваЦен, ВыбранныеЦены, ПоИзменениямЗаДень) Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗагрузитьСтарыеЦеныНоменклатурыПредприятия(Форма, ТаблицаНоменклатуры, КэшДанных, ТаблицаНоменклатурыЗапрещеннойКРедактированию = Неопределено) Экспорт
	...
	Если ПодборТоваровКлиентСервер.ЭтоФормаПрайсЛиста(Форма.КодФормы) Тогда
...
Процедура ЗагрузитьСтарыеЦеныНоменклатурыПредприятия2_5(Форма, ТаблицаНоменклатуры, КэшДанных, ТаблицаНоменклатурыЗапрещеннойКРедактированию = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Выборка.ДействующаяВалюта) Тогда
				ВалютаДляЗаполнения = Выборка.ДействующаяВалюта;
			ИначеЕсли Не ЗначениеЗаполнено(Выборка.ВидЦены) Тогда
				ВалютаДляЗаполнения = Справочники.Валюты.ПустаяСсылка();
			Иначе
				
				ВидЦеныНоменклатуры = Выборка.ВидЦены; // СправочникСсылка.ВидыЦен
				ВалютаДляЗаполнения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					ВидЦеныНоменклатуры,
					"ВалютаЦены");
				
			КонецЕсли;
			
			НайденнаяСтрока["Валюта" + ИмяКолонки] = ВалютаДляЗаполнения;
...
Процедура ЗагрузитьСтарыеЦеныНоменклатурыПрайсЛист2_5(Форма, ТаблицаНоменклатуры, КэшДанных, ТаблицаНоменклатурыЗапрещеннойКРедактированию = Неопределено) Экспорт
	...
	|	ВидыЦен.Страна КАК Страна,
	|	ВЫБОР
	|		КОГДА ВидыЦен.ВариантУказанияСтавокНДС = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСтавокНДСВПрайсЛисте.УказыватьОбщуюСтавкуНДС)
	|			ТОГДА ВидыЦен.СтавкаНДС
	|		КОГДА ВидыЦен.ВариантУказанияСтавокНДС = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСтавокНДСВПрайсЛисте.УказыватьСтавкиНДСПострочно)
	|			ТОГДА ДействующиеЦены.СтавкаНДС
	...
	Пока Выборка.Следующий() Цикл
		...
		НайденнаяСтрока["ВариантУказанияСтавокНДС" + ИмяКолонки] = Выборка.ВариантУказанияСтавокНДС;
		НайденнаяСтрока["Страна" + ИмяКолонки] = Выборка.Страна;
		НайденнаяСтрока["СтавкаНДС" + ИмяКолонки] = Выборка.СтавкаНДС;
		НайденнаяСтрока["ИсходнаяСтавкаНДС" + ИмяКолонки] = Выборка.СтавкаНДС;
	...
	Для Каждого ВидЦены Из Параметры.Форма.ВыбранныеЦены Цикл
		...
		Если ВидЦены.Выбрана Или Параметры.СохранятьБазовые И ВидЦены.Влияет Тогда
			...
			Если Не ЭтоРегистрацияЦенПоставщика Тогда
				...
				Если	Параметры.Форма.ИспользуетсяЦенообразование25
						И ЕстьКолонка(СтрокаЦен, "СтавкаНДС" + ИмяКолонки) Тогда
					НоваяСтрока.СтавкаНДС  = СтрокаЦен["СтавкаНДС" + ИмяКолонки];
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция КоэффициентыУпаковокНоменклатурыДереваТоваров(Форма, ТаблицаНоменклатуры = Неопределено, КэшДанных = Неопределено) Экспорт
	...
	Если ТаблицаНоменклатуры = Неопределено Тогда
		...
		ДобавитьУпаковкиИзДереваРекурсивно(
			Форма.ДеревоЦен,
			МассивВидовЦен,
	...
	Иначе
		...
		Для Каждого ПозицияНоменклатуры Из ТаблицаНоменклатуры Цикл
			...
			ДобавитьУпаковкиНоменклатурыСтрокиТаблицыЦенВТаблицу(
				СтрокаТаблицыЦен,
				МассивВидовЦен,
				ТаблицаУпаковокНоменклатуры);
...
Процедура ЗагрузитьТабличнуюЧастьТовары(Форма, КэшДанных) Экспорт
	...
	Если Не ЭтоРегистрацияЦенПоставщика Тогда
		...
		Если Форма.ИспользуетсяЦенообразование25 Тогда
			...
			|	ВЫБОР
			|		КОГДА ВидыЦен.ВариантУказанияСтавокНДС = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСтавокНДСВПрайсЛисте.УказыватьОбщуюСтавкуНДС)
			|				И ВременнаяТаблицаТовары.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
			|			ТОГДА ВидыЦен.СтавкаНДС
			|		КОГДА ВидыЦен.ВариантУказанияСтавокНДС = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСтавокНДСВПрайсЛисте.УказыватьСтавкиНДСПострочно)
			|				И ВременнаяТаблицаТовары.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
			|			ТОГДА ЦеныНоменклатуры.СтавкаНДС
			|		ИНАЧЕ ВременнаяТаблицаТовары.СтавкаНДС
			|	КОНЕЦ КАК СтавкаНДС
			|ИЗ
			|	ВременнаяТаблицаТовары КАК ВременнаяТаблицаТовары
			|	
...
Функция НайтиСтрокуДереваЦен(Форма, ВходящиеДанныеПоиска, КэшДанных, ОбновитьТаблицуСоответвия = Истина) Экспорт
	...
	Если ЭтоРегистрацияЦенПоставщика(Форма, КэшДанных) Тогда
		...
		Если Форма.ИспользоватьНоменклатуруПартнеров
			И ЗначениеЗаполнено(ДанныеПоиска.НоменклатураПартнера) Тогда
...
Процедура ДеревоЦенУпаковкаВалютаПриИзменении(Форма, ИдентификаторСтроки, ИмяТекущейКолонки, СтрокаПроверки = "Упаковка") Экспорт
	...
	Если СтрокаПроверки = "Упаковка" 
		...
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(Данные, СтруктураДействий, Неопределено);
	...
	Для Каждого ТекЭлементНоменклатура Из ЭлементыНоменклатура Цикл
		...
		Если ТекЭлементНоменклатура.ХарактеристикиИспользуются Тогда
			...
			Если Не ДобавленыХарактеристики Тогда
				ТекстОшибки = НСтр("ru='Для номенклатуры ""%1"" в строке %2 списка ""Таблица цен"" включено использование характеристик, но характеристики не указаны. Добавьте характеристики и укажите их цены или удалите данную номенклатуру из списка.'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, СтрокаТаблицы.Номенклатура, Формат(СтрокаТаблицы.НомерСтроки, "ЧГ="));
...
Процедура ДобавитьТоварыПоОтборамСКД(Форма, ЭтоАвтообновлениеЦен = Ложь) Экспорт
	...
	ПоместитьПустуюТаблицуВМенеджерВременныхТаблиц(МенеджерВременныхТаблиц);
...
Процедура ПолучитьДанныеПоНоменклатуре(НастройкиОтбора, ТаблицаНоменклатуры) Экспорт
	...
	СтруктураНастроек.Вставить("ДатаПолученияОстатков", НастройкиОтбора.Дата);
	...
	Если НастройкиОтбора.ЗначенияОтбора.Свойство("ПоМассивуНоменклатуры", ЗначениеОтбора) Тогда
		...
		Если ТипЗнч(ЗначениеОтбора) = Тип("Массив") Тогда
			...
			ПравоеЗначение.ЗагрузитьЗначения(ЗначениеОтбора);
		Иначе
	...
	ЕстьКолонкаХарактеристика = СтруктураРезультата.ТаблицаТоваров.Колонки.Найти("Характеристика") <> Неопределено;
	ЕстьКолонкаХарактеристикаЦО = СтруктураРезультата.ТаблицаТоваров.Колонки.Найти("ХарактеристикаЦО") <> Неопределено;
	Для Каждого СтрокаТЧ Из СтруктураРезультата.ТаблицаТоваров Цикл
		...
		Если НастройкиОтбора.ИспользуетсяЦенообразование25 Тогда
			Если НастройкиОтбора.ИспользоватьХарактеристикиНоменклатуры Тогда
				ХарактеристикаЦО = Справочники.ХарактеристикиНоменклатурыДляЦенообразования.ПустаяСсылка();
				Если ЕстьКолонкаХарактеристикаЦО Тогда
					ХарактеристикаЦО = СтрокаТЧ.ХарактеристикаЦО;
				ИначеЕсли ЕстьКолонкаХарактеристика
					И ЗначениеЗаполнено(СтрокаТЧ.Характеристика) Тогда
					ХарактеристикаЦО = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТЧ.Характеристика, "ХарактеристикаНоменклатурыДляЦенообразования");
				КонецЕсли;
				НоваяСтрока.ХарактеристикаЦО = ХарактеристикаЦО;
			КонецЕсли;
...
// Устанавливает для строк дерева Ставку НДС 
//
// Параметры:
//  Форма - см. ПостроитьДеревоЦен.Форма
//  ПараметрыРасчета - Структура - Структура параметров расчета
//  КэшДанных - см. ИнициализироватьСтруктуруКэшаДанных
//
Процедура ИзменитьСтавкиНДС(Форма, ПараметрыРасчета, КэшДанных = Неопределено) Экспорт

	Параметры = Новый Структура();
	
	Если ЗначениеЗаполнено(ПараметрыРасчета.СтавкаНДС) Тогда
		СоответствиеВидовЦен = УчетНДСЛокализация.ПолучитьСтавкуНДСПоВидуЦены(ПараметрыРасчета);
		Если Не ЗначениеЗаполнено(СоответствиеВидовЦен) Тогда
			Возврат;
		КонецЕсли;
		Параметры.Вставить("Операция",	"ИзменитьСтавкиНДС");
		Параметры.Вставить("ВидыЦен",	СоответствиеВидовЦен);
	Иначе
		
		ТаблицаЦенНоменклатуры = ПолучитьСтавкуПоНоменклатуре(Форма, ПараметрыРасчета);
		Если Не ЗначениеЗаполнено(ТаблицаЦенНоменклатуры) Тогда
			Возврат;
		КонецЕсли;
	
		Параметры.Вставить("Операция",					"ИзменитьСтавкиНДСИзНоменклатуры");
		Параметры.Вставить("ВидыЦен",					ПараметрыРасчета.ВыбранныеВидыЦен);
		Параметры.Вставить("ТаблицаЦенНоменклатуры",	ТаблицаЦенНоменклатуры);
	КонецЕсли;
...
Функция ИнициализироватьТаблицуВыбранныеЦены()
	...
	ТаблицаВыбранныеЦены.Колонки.Добавить("ВариантУказанияСтавокНДС",						Новый ОписаниеТипов("ПеречислениеСсылка.ВариантыУказанияСтавокНДСВПрайсЛисте"));
	ТаблицаВыбранныеЦены.Колонки.Добавить("Страна",											Новый ОписаниеТипов("СправочникСсылка.СтраныМира"));
	ТаблицаВыбранныеЦены.Колонки.Добавить("СтавкаНДС",										Новый ОписаниеТипов("СправочникСсылка.СтавкиНДС"));
...
Процедура ИнициализироватьВыбранныеЦеныНоменклатуры(Форма, МассивВидоВЦен = Неопределено,ТаблицаВидовЦен = Неопределено)
	...
	Для Каждого СтрокаТЧ Из Таблица Цикл
		...
		Если СтрокаТЧ.Статус = Перечисления.СтатусыДействияВидовЦен.Действует Или
			...
			НоваяСтрока.ВариантУказанияСтавокНДС						= СтрокаТЧ.ВариантУказанияСтавокНДС;
			НоваяСтрока.Страна											= СтрокаТЧ.Страна;
	...
	Если ТаблицаНоменклатуры.Количество() > 0 Тогда
		...
		Если СтрокаВидЦены.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБ
			...
			Если ПроверкаСКДПройдена2_5 Тогда
				...
				МенеджерВременныхТаблиц = Неопределено;
				Параметр = ПараметрыКомпоновки.Найти("ИспользоватьДанныеДокумента");
				Если Параметр <> Неопределено Тогда
					ПоместитьДанныеТаблицыВМенеджерВременныхТаблиц(МенеджерВременныхТаблиц, Параметр, ТаблицаНоменклатуры, СтрокаВидЦены);
				КонецЕсли;
...
// 	ВидыЦен - Массив Из СправочникСсылка.ВидыЦен - виды цен, для которых необходимо применить округление
//
Процедура ПрименитьОкруглениеКРучнымЦенам(Форма, ТаблицаНоменклатуры, ВидыЦен, КэшДанных)
	...
	Для Каждого СтрокаТаблицыНоменклатуры Из ТаблицаНоменклатуры Цикл
		...
		Для Каждого СтрокаВидЦены Из ВидыЦен Цикл
			...
			Если СтрокаТаблицыЦен["СтараяЦена" + ИмяКолонки] <> СтрокаТаблицыЦен[ИмяКолонки] Тогда
				СтрокаТаблицыЦен["ИзмененаВручную" + СтрокаВидЦены.ИмяКолонки] = Истина;
			КонецЕсли;
...
Процедура ЗагрузитьТоварыВДеревоЦен(Форма, ВыборкаНоменклатура, КэшДанных)
	...
	Пока ВыборкаНоменклатура.Следующий() Цикл
		...
		Иначе
			...
			Если Форма.ИспользуетсяЦенообразование25 Тогда
				ЭлементНоменклатура["СтавкаНДС" + ИмяКолонки] = ВыборкаНоменклатура.СтавкаНДС;
				ЭлементНоменклатура["ИсходнаяСтавкаНДС" + ИмяКолонки] = ВыборкаНоменклатура.СтавкаНДС;
			КонецЕсли;
...
Процедура КонтрольКоличестваСтрок(Параметры)
	...
	Если Параметры.КоличествоСтрок + Параметры.КоличествоВидовЦен > 99999 Тогда
...
Функция ДобавитьПолеФормы(Форма, Имя, ДополнительныеПараметры)
	...
	СтруктураПараметров.Вставить("РедактированиеТекста", Неопределено);
	...
	Если СтруктураПараметров.РедактированиеТекста <> Неопределено Тогда
		НовоеПоле.РедактированиеТекста = СтруктураПараметров.РедактированиеТекста;
	КонецЕсли;
...
Процедура ТаблицаСоответствия(Форма, КэшДанных, Переформировать = Ложь)
	...
	Если ТипЗнч(Форма.ДеревоЦен) = Тип("ДанныеФормыКоллекция") Тогда
		...
		Для Каждого ЭлементНоменклатура Из ЭлементыНоменклатура Цикл
			...
			НоваяСтрокаТЗ = НоваяСтрокаТаблицыСоответствия(КэшДанных, ЭлементНоменклатура);
...
Функция НоваяСтрокаТаблицыСоответствия(КэшДанных, СтрокаИсточник, ЗаполнятьИдентификатор = Ложь)
	
	СтрокаСоответствия = КэшДанных.ТаблицаСоответствияНоменклатурыСтрокамДереваТоваров.Добавить();
	
	ЗаполнитьЗначенияСвойств(СтрокаСоответствия, СтрокаИсточник);
	
	СтрокаСоответствия.СтрокаДереваЦен = СтрокаИсточник;
	Если ЗаполнятьИдентификатор Тогда
		СтрокаСоответствия.ИдентификаторСтроки = СтрокаИсточник.ПолучитьИдентификатор();
	КонецЕсли;
...
Функция ЕстьИзмененияСтавкиНДСВСтроке(СтрокаДереваЦен, ВыбранныеЦены, ПоИзменениямЗаДень = Ложь)
	
	СтавкаНДСИзменена = Ложь;
	Для Каждого ВидЦены Из ВыбранныеЦены Цикл
		
		Если	(СтрокаДереваЦен["ИзмененаВручнуюСтавкаНДС" + ВидЦены.ИмяКолонки]
				Или СтрокаДереваЦен["ИзмененаАвтоматическиСтавкаНДС" + ВидЦены.ИмяКолонки]) Тогда
...
Функция ЭтоРегистрацияЦенПоставщика(Форма, КэшДанных = Неопределено)
	...
	Если КэшДанных = Неопределено Тогда
		ЭтоРегистрацияЦенПоставщика = (ЭтоРегистрацияЦенНоменклатурыПоставщика(Форма)
			Или ПодборТоваровКлиентСервер.ЭтоФормаПрайсЛистаПоставщиков(Форма.КодФормы)
...
Функция ЭтоПрайсЛист(Форма)
	...
	Возврат (Форма.КодФормы = "Обработка_ПрайсЛист_Форма");
	...
	Если Параметры.ДобавлятьВсеСтроки Или Не СтрокиОбработаны Тогда
		...
		ИначеЕсли Параметры.Операция = "ИзменитьЦеныНаПроцент" Тогда
			Для Каждого ВидЦен Из Параметры.ВидыЦен Цикл
				...
				ЭлементДереваЦенИсточник["ИзмененаАвтоматически" + ИмяКолонки] = Ложь;
			КонецЦикла;
		ИначеЕсли Параметры.Операция = "ИзменитьСтавкиНДС" Тогда
			
			Для Каждого ВидЦен Из Параметры.ВидыЦен Цикл
				ИмяКолонки = ИмяКолонкиПоВидуЦены(ВидЦен.Ключ, КэшДанных);
				
				ЭлементДереваЦенИсточник["СтавкаНДС" + ИмяКолонки] = ВидЦен.Значение;
				ЭлементДереваЦенИсточник["ИзмененаВручнуюСтавкаНДС" + ИмяКолонки] =
					(НЕ ЭлементДереваЦенИсточник["ИсходнаяСтавкаНДС" + ИмяКолонки] = ЭлементДереваЦенИсточник["СтавкаНДС" + ИмяКолонки]);
				ЭлементДереваЦенИсточник["ИзмененаАвтоматическиСтавкаНДС" + ИмяКолонки] = Ложь;
			КонецЦикла;
			
		ИначеЕсли Параметры.Операция = "ИзменитьСтавкиНДСИзНоменклатуры" Тогда
			
			Для Каждого ВидЦен Из Параметры.ВидыЦен Цикл
				ИмяКолонки = ИмяКолонкиПоВидуЦены(ВидЦен, КэшДанных);
				
				ПараметрыПоиска = Новый Структура("Номенклатура, Страна, ВидЦены",
						ЭлементДереваЦенИсточник.Номенклатура, ЭлементДереваЦенИсточник["Страна" + ИмяКолонки], ВидЦен);
				МассивНайденных = Параметры.ТаблицаЦенНоменклатуры.НайтиСтроки(ПараметрыПоиска);
				Если ЗначениеЗаполнено(МассивНайденных) Тогда
					ЗначениеСтавкиНДС = МассивНайденных[0].Ставка;
				Иначе
					Продолжить;
				КонецЕсли;
				
				ЭлементДереваЦенИсточник["СтавкаНДС" + ИмяКолонки] = ЗначениеСтавкиНДС;
				ЭлементДереваЦенИсточник["ИзмененаВручнуюСтавкаНДС" + ИмяКолонки] =
					(НЕ ЭлементДереваЦенИсточник["ИсходнаяСтавкаНДС" + ИмяКолонки] = ЭлементДереваЦенИсточник["СтавкаНДС" + ИмяКолонки]);
				ЭлементДереваЦенИсточник["ИзмененаАвтоматическиСтавкаНДС" + ИмяКолонки] = Ложь;
			КонецЦикла;
	...
	Для Каждого ЭлементДереваЦенТекущий Из ЭлементыДереваЦенТекущие Цикл
		...
		Если Не УровнемНижеДобавилиСтроки Тогда
			Если Параметры.ТолькоИзмененные Тогда
				...
				Если ИспользуетсяЦенообразование25 И НЕ ЦеныИзменены Тогда
					ЦеныИзменены = ЕстьИзмененияСтавкиНДСВСтроке(ЭлементДереваЦенТекущий, Параметры.ВидыЦен);
				КонецЕсли;
...
Функция ДобавитьТоварыПредприятия(Форма, Знач ТаблицаТовары, КэшДанных)
	...
	Если ИерархическоеДерево Тогда
		...
		Если ЗначениеЗаполнено(КлючевоеПоле) Тогда
			...
			Для Каждого СтрокаДерева Из ДеревоИерархия.Строки Цикл
				УдалитьДублиСтрокИзИерархии(СтрокаДерева, СтрокаДерева.Строки.Количество(), КлючевоеПоле);
			КонецЦикла;
			
			ПоискПустойСтроки = ДеревоИерархия.Строки.Найти(Справочники.Номенклатура.ПустаяСсылка(), КлючевоеПоле);
			Если ПоискПустойСтроки <> Неопределено Тогда
				ДеревоИерархия.Строки.Удалить(ПоискПустойСтроки);
			КонецЕсли;
...
Процедура УдалитьДублиСтрокИзИерархии(СтрокаДерева, КоличествоСтрок, ИмяПоля)
	
	Для Индекс = 1 По СтрокаДерева.Строки.Количество() Цикл
		
		ДочерняяСтрока = СтрокаДерева.Строки.Получить(КоличествоСтрок - Индекс);
		
		Если ДочерняяСтрока[ИмяПоля] = СтрокаДерева[ИмяПоля] Тогда
			СтрокаДерева.Строки.Удалить(ДочерняяСтрока);
		ИначеЕсли Не ЗначениеЗаполнено(ДочерняяСтрока[ИмяПоля]) Тогда
			
		Иначе
			УдалитьДублиСтрокИзИерархии(ДочерняяСтрока, ДочерняяСтрока.Строки.Количество(), ИмяПоля);
		КонецЕсли;
...
Функция ДобавитьТоварыПредприятия2_5(Форма, Знач ТаблицаТовары, КэшДанных)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаВидыНоменклатуры КАК ВидыНоменклатуры
	...
	|	И ВидыЦен.ВариантУказанияСтавокНДС = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСтавокНДСВПрайсЛисте.УказыватьСтавкиНДСПострочно)
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаТоваров.Номенклатура КАК Номенклатура,
	|	ТаблицаТоваров.ХарактеристикаЦО КАК ХарактеристикаЦО,
	|	ТаблицаТоваров.СерияЦО КАК СерияЦО,
	|	ТаблицаТоваров.УпаковкаЦО КАК УпаковкаЦО,
	|	ВидыЦен.Ссылка КАК ВидЦены,
	|	ВЫБОР
	|		КОГДА ВидыЦен.ВариантУказанияСтавокНДС = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСтавокНДСВПрайсЛисте.УказыватьОбщуюСтавкуНДС)
	|			ТОГДА ВидыЦен.СтавкаНДС
	...
	|	КОНЕЦ КАК СтавкаНДС
	|ИЗ
	|	ТаблицаНоменклатура КАК ТаблицаТоваров
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	Справочник.ВидыЦен КАК ВидыЦен
	|ПО
	|	ИСТИНА
	...
	|ПО
	|	ТаблицаТоваров.Номенклатура = ЦеныНоменклатуры.Номенклатура
	|	И ТаблицаТоваров.ХарактеристикаЦО = ЦеныНоменклатуры.ХарактеристикаЦО
	|	И ТаблицаТоваров.СерияЦО = ЦеныНоменклатуры.СерияЦО
	|	И ТаблицаТоваров.УпаковкаЦО = ЦеныНоменклатуры.УпаковкаЦО
	|	И ВидыЦен.Ссылка = ЦеныНоменклатуры.ВидЦены
	|ГДЕ
	|	ВидыЦен.Ссылка В (&ВидыЦен)
	|	И ВидыЦен.Ссылка ЕСТЬ НЕ NULL
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаТоваров.Номенклатура,
	|	ТаблицаТоваров.ХарактеристикаЦО,
	|	ТаблицаТоваров.СерияЦО,
	|	ТаблицаТоваров.УпаковкаЦО,
	|	ВидыЦен.Ссылка,
	|	ВЫБОР
	|		КОГДА ВидыЦен.ВариантУказанияСтавокНДС = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСтавокНДСВПрайсЛисте.УказыватьОбщуюСтавкуНДС)
	|			ТОГДА ВидыЦен.СтавкаНДС
	...
	ВыборкаНоменклатура = РезультатЗапроса[3].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	Упаковки = РезультатЗапроса[4].Выгрузить();
	Упаковки.Индексы.Добавить("Номенклатура, ХарактеристикаЦО, СерияЦО, УпаковкаЦО");
	
	Страны = РезультатЗапроса[5].Выгрузить();
	СтавкиНДС = РезультатЗапроса[6].Выгрузить();
	СтавкиНДС.Индексы.Добавить("Номенклатура, ХарактеристикаЦО, СерияЦО, УпаковкаЦО");
	...
	Параметры.Вставить("Страны", Страны);
	Параметры.Вставить("СтавкиНДС", СтавкиНДС);
...
Процедура ДобавитьТоварПредприятия(Форма, КэшДанных, ВыборкаСтрок, ЭлементДереваЦен, Параметры, Знач Уровень)
	...
	Пока ВыборкаСтрок.Следующий() Цикл
		...
		Иначе
			...
			НоваяСтрокаТЗ = НоваяСтрокаТаблицыСоответствия(КэшДанных, ТекущийЭлемент, Параметры.ДеревоЦенЭтоДанныеФормыДерево);
			
			ЗаполнитьСтрану(Параметры.Страны, КэшДанных, ТекущийЭлемент);
...
Процедура ДобавитьТоварПредприятияФрагмент(Форма, КэшДанных, ТекущийЭлемент, ВыборкаСтрок, Параметры)
	...
	НайденныеСтроки = Параметры.СтавкиНДС.НайтиСтроки(ПараметрыОтбора);
	Для Каждого ВыборкаСтавкиНДС Из НайденныеСтроки Цикл
		
		ТекущийЭлемент["СтавкаНДС" + ИмяКолонкиПоВидуЦены(ВыборкаСтавкиНДС.ВидЦены, КэшДанных)] = ВыборкаСтавкиНДС.СтавкаНДС;
...
Процедура СкопироватьСтрокиДереваИерархии(Форма, Приемник, Источник, КэшДанных)
	...
	Для Каждого СтрокаИсточника Из СтрокиИсточника Цикл
		...
		СтрокаПриемника = НайтиСтрокуДереваЦен(Форма, СтрокаИсточника, КэшДанных, Ложь);
		...
		Если СтрокаПриемника = Неопределено Тогда
			...
			НоваяСтрокаТаблицыСоответствия(КэшДанных, СтрокаПриемника, ДеревоЦенЭтоДанныеФормыДерево);
		КонецЕсли;
		
		СкопироватьСтрокиДереваИерархии(Форма, СтрокаПриемника, СтрокаИсточника, КэшДанных);
	КонецЦикла; 
...
Процедура ЗаполнитьСтрану(ВыбранныеСтраны, КэшДанных, ТекущийЭлемент)
	
	Для Каждого СтрокаТаблицы Из ВыбранныеСтраны Цикл
		ТекущийЭлемент["Страна" + ИмяКолонкиПоВидуЦены(СтрокаТаблицы.ВидЦены, КэшДанных)] = СтрокаТаблицы.Страна;
	КонецЦикла;
...
Функция ШаблонОшибкиВычисленияЦеныПоДаннымИБ()
	
	Возврат НСтр("ru = 'Ошибка расчета цены с помощью запроса к данным ИБ по виду цен ""%1""'");
	
КонецФункции

// Удаляет из исходной таблицы товары, недоступные для установки цен, и возвращает их в отдельной таблице
// с указанием причины недоступности.
//
// Параметры:
// 	ТаблицаТовары - ТаблицаЗначений - исходная таблица товаров.
//
// Возвращаемое значение:
// 	ТаблицаЗначений - таблица недоступных товаров:
// 		* Номенклатура - СправочникСсылка.Номенклатура - номенклатура
// 		* Характеристика - СправочникСсылка.ХарактеристикиНоменклатуры - характеристика номенклатуры
// 		* Причина - Строка - причина недоступности установки цен на товар.
//
Функция ТоварыНедоступныеДляУстановкиЦен(ТаблицаТовары)
	
	Если ТаблицаТовары.Колонки.Найти("Номенклатура") = Неопределено
		Или ТаблицаТовары.Колонки.Найти("Характеристика") = Неопределено Тогда
		Возврат Новый ТаблицаЗначений;
	КонецЕсли;
	
	Результат = ТаблицаТовары.Скопировать(Новый Массив, "Номенклатура, Характеристика");
	Результат.Колонки.Добавить("Причина", Новый ОписаниеТипов("Строка",,,, Новый КвалификаторыСтроки(200)));
	
	// наборы, цена на которые складывается из цен комплектующих
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТаблицаНоменклатуры", ТаблицаТовары.Скопировать(, "Номенклатура, Характеристика"));
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТаблицаНоменклатуры.Номенклатура КАК Номенклатура,
	|	ТаблицаНоменклатуры.Характеристика КАК Характеристика
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВариантыКомплектацииНоменклатуры КАК СправочникВариантыКомплектации
	|		ПО ТаблицаНоменклатуры.Номенклатура = СправочникВариантыКомплектации.Владелец
	|		И ТаблицаНоменклатуры.Характеристика = СправочникВариантыКомплектации.Характеристика
	|ГДЕ
	|	СправочникНоменклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Набор)
	|	И НЕ СправочникВариантыКомплектации.ПометкаУдаления
	|	И СправочникВариантыКомплектации.Основной
	|	И СправочникВариантыКомплектации.ВариантРасчетаЦеныНабора =
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрока.Причина = НСтр("ru = 'набор, цена складывается из цен комплектующих'");
		
		СтруктураПоиска = Новый Структура("Номенклатура, Характеристика",
			Выборка.Номенклатура,
			Выборка.Характеристика);
		
		ПоискСтрок = ТаблицаТовары.НайтиСтроки(СтруктураПоиска);
		Для Каждого СтрокаКУдалению Из ПоискСтрок Цикл
...
Процедура ДобавитьУпаковкиИзДереваРекурсивно(ДеревоЦен, МассивВидовЦен, ТаблицаУпаковокНоменклатуры)
	
	ЭлементыДерева = ?(ТипЗнч(ДеревоЦен) = Тип("ДанныеФормыДерево")
		Или ТипЗнч(ДеревоЦен) = Тип("ДанныеФормыЭлементДерева"),
		ДеревоЦен.ПолучитьЭлементы(),
		ДеревоЦен.Строки);
	
	Для Каждого ЭлементДерева Из ЭлементыДерева Цикл
		
		ДобавитьУпаковкиНоменклатурыСтрокиТаблицыЦенВТаблицу(ЭлементДерева, МассивВидовЦен, ТаблицаУпаковокНоменклатуры);
		ДобавитьУпаковкиИзДереваРекурсивно(ЭлементДерева, МассивВидовЦен, ТаблицаУпаковокНоменклатуры);
...
// Получение ставки НДС для Номенклатуры.
//
// Параметры:
//  Форма - см. ПостроитьДеревоЦен.Форма
//  ПараметрыРасчета - Структура - Структура параметров расчета
//  
// Возвращаемое значение:
//  ТаблицаЗначений - Таблица номенклатуры и ставки НДС:
//   * Номенклатура - СправочникСсылка.Номенклатура - Номенклатура
//   * Страна - СправочникСсылка.СтраныМира - Страна
//   * Ставка - СправочникСсылка.СтавкиНДС - Ставка НДС
//   * ВидЦены - СправочникСсылка.ВидыЦен - Вид цены
Функция ПолучитьСтавкуПоНоменклатуре(Форма, ПараметрыРасчета)
	...
	|	СтавкиНДСНоменклатуры.СтавкаНДС КАК Ставка,
	|	Таблица.ВидЦены КАК ВидЦены
	|ИЗ
	|	РегистрСведений.СтавкиНДСНоменклатуры.СрезПоследних(&ДатаДействия, 
	|		Номенклатура В (&МассивНоменклатуры)
	|		И Страна В (ВЫБРАТЬ РАЗЛИЧНЫЕ Страна ИЗ ТаблицаВидыЦен)
	|	) КАК СтавкиНДСНоменклатуры 
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	ТаблицаВидыЦен КАК Таблица
	|ПО
	|	СтавкиНДСНоменклатуры.Страна = Таблица.Страна
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	Справочник.СтавкиНДС КАК Ставки
	|ПО
	|	СтавкиНДСНоменклатуры.СтавкаНДС = Ставки.Ссылка
	|ГДЕ
	...
	Если ПараметрыРасчета.ТолькоВыделенныеСтроки Тогда
		...
		Для Каждого ВыделеннаяСтрока Из Форма.Элементы.ДеревоЦен.ВыделенныеСтроки Цикл
			СтрокаТаблицыЦен = Форма.ДеревоЦен.НайтиПоИдентификатору(ВыделеннаяСтрока);
			МассивНоменклатуры.Добавить(СтрокаТаблицыЦен.Номенклатура);
		КонецЦикла;
	...
	Иначе	
		...
		ДеревоЦенЭтоДанныеФормыДерево = (ТипЗнч(Форма.ДеревоЦен) = Тип("ДанныеФормыДерево"));
		Если ДеревоЦенЭтоДанныеФормыДерево Тогда
			НоменклатуруДанныеФормыДеревоВМассив(Форма.ДеревоЦен.ПолучитьЭлементы(), МассивНоменклатуры);
		Иначе
			НоменклатуруДереваВМассив(Форма.ДеревоЦен.Строки, МассивНоменклатуры);
		КонецЕсли;
...
Процедура НоменклатуруДанныеФормыДеревоВМассив(СтрокиИерархии, МассивНоменклатуры)
	
	Для Каждого СтрокаИерархии Из СтрокиИерархии Цикл
		Если ЗначениеЗаполнено(СтрокаИерархии.Номенклатура)
			И МассивНоменклатуры.Найти(СтрокаИерархии.Номенклатура) = Неопределено Тогда
			МассивНоменклатуры.Добавить(СтрокаИерархии.Номенклатура);
		КонецЕсли;
		НоменклатуруДанныеФормыДеревоВМассив(СтрокаИерархии.ПолучитьЭлементы(), МассивНоменклатуры);
	КонецЦикла;
	
КонецПроцедуры

Процедура НоменклатуруДереваВМассив(СтрокиИерархии, МассивНоменклатуры)
	...
	Для Каждого СтрокаИерархии Из СтрокиИерархии Цикл
		Если ЗначениеЗаполнено(СтрокаИерархии.Номенклатура)
			...
			МассивНоменклатуры.Добавить(СтрокаИерархии.Номенклатура);
		КонецЕсли;
		НоменклатуруДереваВМассив(СтрокаИерархии.Строки, МассивНоменклатуры);
	КонецЦикла;
	
КонецПроцедуры

// Добавляет в таблицу номенклатуры колонки для ставок НДС по видам цен, с вариантом "Указывать ставки НДС построчно".
//
// Параметры:
//   Форма - см. ПостроитьДеревоЦен.Форма
//   ТаблицаНоменклатуры - ТаблицаЗначений - в которую добавляются колонки ставок НДС.
//   МассивВидовЦен - Массив из СправочникСсылка.ВидыЦен - виды цен для обработки.
//   Если не задан, заполняется выбранными и доступными для редактирования из таблицы формы "Выбранные цены".
Процедура ЗаполнитьСтавкуНДСВТаблицеНоменклатуры(Форма, ТаблицаНоменклатуры, МассивВидовЦен = Неопределено) Экспорт
		...
		Для Каждого СтрокаВидЦены Из Форма.ВыбранныеЦены Цикл
			Если СтрокаВидЦены.Выбрана 
				...
				И (СтрокаВидЦены.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБ
					...
					Или СтрокаВидЦены.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБПоПоставщикам 
					Или СтрокаВидЦены.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБПоСебестоимости) Тогда
				МассивВидовЦен.Добавить(СтрокаВидЦены);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого ВидЦены Из МассивВидовЦен Цикл
		...
		Если ЗначениеЗаполнено(ВидЦены["ИмяКолонки"])
			И ВидЦены.ВариантУказанияСтавокНДС = Перечисления.ВариантыУказанияСтавокНДСВПрайсЛисте.УказыватьСтавкиНДСПострочно Тогда
			
			ИмяКолонки = "СтавкаНДС" + ВидЦены["ИмяКолонки"];
			ТаблицаНоменклатуры.Колонки.Добавить(ИмяКолонки, Новый ОписаниеТипов("СправочникСсылка.СтавкиНДС"));
		
			ДеревоЦенЭтоДанныеФормыДерево = (ТипЗнч(Форма.ДеревоЦен) = Тип("ДанныеФормыДерево"));
			Если ДеревоЦенЭтоДанныеФормыДерево Тогда
				ЗаполнитьСтавкуНДСДанныеФормыДеревоВТаблицуНоменклатуры(Форма.ДеревоЦен.ПолучитьЭлементы(), ТаблицаНоменклатуры, ИмяКолонки);
			КонецЕсли;
...
// Процедура рекурсивно заполняет ставку НДС в таблице номенклатуры данными из иерархической структуры (дерева).
//
// Параметры:
//   СтрокиИерархии - ДанныеФормыДерево - Строки дерева, содержащие ставки НДС и реквизиты номенклатуры
//   ТаблицаНоменклатуры - ТаблицаФормы - Таблица формы номенклатуры, в которую записывается ставка НДС
//   ИмяКолонки - Строка - Имя колонки в источнике и приемнике, содержащей ставку НДС
Процедура ЗаполнитьСтавкуНДСДанныеФормыДеревоВТаблицуНоменклатуры(СтрокиИерархии, ТаблицаНоменклатуры, ИмяКолонки)
	
	Для Каждого СтрокаИерархии Из СтрокиИерархии Цикл
		
		СтавкаНДС = СтрокаИерархии[ИмяКолонки];
		Если ЗначениеЗаполнено(СтавкаНДС) И ЗначениеЗаполнено(СтрокаИерархии.Номенклатура) Тогда
			
			ПараметрыПоиска = Новый Структура;
			ПараметрыПоиска.Вставить("Номенклатура", СтрокаИерархии.Номенклатура);
			ПараметрыПоиска.Вставить("ХарактеристикаЦО", СтрокаИерархии.ХарактеристикаЦО);
			ПараметрыПоиска.Вставить("СерияЦО", СтрокаИерархии.СерияЦО);
			ПараметрыПоиска.Вставить("УпаковкаЦО", СтрокаИерархии.УпаковкаЦО);
			
			МассивНайденных = ТаблицаНоменклатуры.НайтиСтроки(ПараметрыПоиска);
			Если ЗначениеЗаполнено(МассивНайденных) Тогда
				МассивНайденных[0][ИмяКолонки] = СтавкаНДС;
			КонецЕсли;
			
		КонецЕсли;
		
		ЗаполнитьСтавкуНДСДанныеФормыДеревоВТаблицуНоменклатуры(СтрокаИерархии.ПолучитьЭлементы(), ТаблицаНоменклатуры, ИмяКолонки);
		
	КонецЦикла;
	
КонецПроцедуры

// Помещает данные из Таблицы номенклатуры во временную таблицу менеджера временных таблиц
// и настраивает параметры использования.
//
// Параметры:
//   МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - менеджер временных таблиц для размещения данных
//   Параметр - Структура - структура с параметрами использования (ключи "Значение" и "Использование")
//   ТаблицаНоменклатуры - ТаблицаЗначений - таблица значений с данными из документа "Установки цен номенклатуры"
//   СтрокаВидЦены - СтрокаТаблицыЗначений - обрабатываемая строка вида цены
Процедура ПоместитьДанныеТаблицыВМенеджерВременныхТаблиц(МенеджерВременныхТаблиц, Параметр, ТаблицаНоменклатуры, СтрокаВидЦены)
	
	Если Не ЗначениеЗаполнено(СтрокаВидЦены["ИмяКолонки"])
		Или СтрокаВидЦены.ВариантУказанияСтавокНДС <> Перечисления.ВариантыУказанияСтавокНДСВПрайсЛисте.УказыватьСтавкиНДСПострочно Тогда
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТаблицаСтавкиНДС.Номенклатура КАК Номенклатура,
	|	ТаблицаСтавкиНДС.ХарактеристикаЦО КАК ХарактеристикаЦО,
	|	ТаблицаСтавкиНДС.СерияЦОДляСвязи КАК СерияЦО,
	|	ТаблицаСтавкиНДС.УпаковкаЦОДляСвязи КАК УпаковкаЦО,
	|	ТаблицаСтавкиНДС.СтавкаНДС КАК СтавкаНДС
	|ПОМЕСТИТЬ ТаблицаДанныхУстановкиЦенНоменклатуры
	|ИЗ
	|	&ТаблицаСтавкиНДСНоменклатуры КАК ТаблицаСтавкиНДС
	|";
	
	ИмяКолонки = "СтавкаНДС" + СтрокаВидЦены["ИмяКолонки"];
	ТаблицаСтавкиНДСНоменклатуры = ТаблицаНоменклатуры.Скопировать(, "Номенклатура, ХарактеристикаЦО, СерияЦОДляСвязи, УпаковкаЦОДляСвязи, " + ИмяКолонки);
	ТаблицаСтавкиНДСНоменклатуры.Колонки[ИмяКолонки].Имя = "СтавкаНДС";
	
	Запрос.УстановитьПараметр("ТаблицаСтавкиНДСНоменклатуры", ТаблицаСтавкиНДСНоменклатуры);
	Запрос.Выполнить();
	
КонецПроцедуры

// Помещает пустые данные во временную таблицу "ТаблицаДанныхУстановкиЦенНоменклатуры" менеджера временных таблиц.
//
// Параметры:
//   МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - менеджер временных таблиц для размещения данных
Процедура ПоместитьПустуюТаблицуВМенеджерВременныхТаблиц(МенеджерВременныхТаблиц)
