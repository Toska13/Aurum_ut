...
Процедура ПриОткрытииСменыНаККТ(ИдентификаторУстройстваККТ, ПараметрыРегистрации) Экспорт
	...
	//++ НЕ ГОСИС
	РозничныеПродажиКлиентЛокализация.ЗапуститьПриНеобходимостиОбновлениеСпискаCDNПлощадок(ИдентификаторУстройстваККТ, ПараметрыРегистрации);
...
Процедура ПриФискализацииЧекаСМаркированнымиТоварами(ИдентификаторУстройстваККТ, ОтчетВыбытииМТ) Экспорт
	...
	//++ НЕ ГОСИС
	ОперацииПоРаботеСКодамиМаркировки = ИнтерфейсИСМПОбщегоНазначенияКлиентСервер.ОперацииПриРаботеСКодамиМаркировки();
	
	ОперацииВыбытияМарок = Новый Массив;
	ОперацииВыбытияМарок.Добавить(ПредопределенноеЗначение("Перечисление.ТипыРасчетаДенежнымиСредствами.ПриходДенежныхСредств"));
	ОперацииВыбытияМарок.Добавить(ПредопределенноеЗначение("Перечисление.ТипыРасчетаДенежнымиСредствами.ВозвратРасходаДенежныхСредств"));
	
	Операция = ?(ОперацииВыбытияМарок.Найти(ОтчетВыбытииМТ.ТипРасчета) = Неопределено, ОперацииПоРаботеСКодамиМаркировки.Возврат, ОперацииПоРаботеСКодамиМаркировки.Продажа);
	
	ПараметрыПробитияЧека          = ИнтеграцияТСПИоТКлиент.ПараметрыПробитияЧека();
	ПараметрыПробитияЧека.ККТ      = ИдентификаторУстройстваККТ;
	ПараметрыПробитияЧека.Операция = Операция;
	
	Для Каждого СтрокаОтчетаОВыбытии ИЗ ОтчетВыбытииМТ.ПозицииЧека Цикл
		
		Если Не ЗначениеЗаполнено(СтрокаОтчетаОВыбытии.НормализованныйКодМаркировки) Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаЗаписи = ИнтеграцияТСПИоТКлиент.ДанныеКодовИдентификации();
		
		СтрокаЗаписи.КодИдентификации             = СтрокаОтчетаОВыбытии.НормализованныйКодМаркировки;
		СтрокаЗаписи.ДробноеКоличествоЧислитель   = СтрокаОтчетаОВыбытии.ДробноеКоличество.Числитель;
		СтрокаЗаписи.ДробноеКоличествоЗнаменатель = СтрокаОтчетаОВыбытии.ДробноеКоличество.Знаменатель;
		СтрокаЗаписи.Количество                   = СтрокаОтчетаОВыбытии.Количество;
		
		ПараметрыПробитияЧека.ДанныеКИ.Добавить(СтрокаЗаписи);
		
	КонецЦикла;
	
	ИнтеграцияТСПИоТКлиент.ПослеФискализацииЧекаСМаркированнымиТоварами(ПараметрыПробитияЧека);
	//-- НЕ ГОСИС
