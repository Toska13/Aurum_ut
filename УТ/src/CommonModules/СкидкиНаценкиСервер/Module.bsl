...
Функция Рассчитать(Объект, ВходныеПараметры) Экспорт
	...
	ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЧекККМВозврат") Тогда
		...
		РезультатРасчета = РассчитатьПоЧекуККМВозврат(Объект, ВходныеПараметры);
...
// Выполняет расчет скидок по чеку ККМВозврат.
//
// Параметры:
//  Объект - ДокументОбъект, ДанныеФормыСтруктура - Объект, в котором требуется рассчитать скидки (наценки).
//  ВходныеПараметры - Структура - Структура со свойствами:
...
//   * ПараметрыРасчета - Структура - Структура параметров расчета.
//
Функция РассчитатьПоЧекуККМВозврат(Объект, ВходныеПараметры)
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	Товары.НомерСтроки        КАК КлючСвязи,
	|	&ПоляНабора,
	|	Товары.Номенклатура       КАК Номенклатура,
	|	Товары.Характеристика     КАК Характеристика,
	|	Товары.Упаковка           КАК Упаковка,
	...
	ПримененныеСкидкиНаценки = Новый Структура;
	ПримененныеСкидкиНаценки.Вставить("ДеревоСкидок" ,Новый ДеревоЗначений);
	ПримененныеСкидкиНаценки.Вставить("ТаблицаСкидкиНаценки" ,Новый ТаблицаЗначений);
	ПримененныеСкидкиНаценки.Вставить("ПараметрыРасчета" ,ПараметрыРасчета);
...
//++ Локализация
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаДерева, "СпособПримененияСкидки")
		Или Не СтрокаДерева.СпособПримененияСкидки = Перечисления.СпособыПримененияСкидокНаценок.НачислитьБонусныеБаллы Тогда
		РозничныеПродажиЛокализация.ПрименитьЗначениеСкидкиКТовару(НоваяСтрока, Товар, ПараметрыРасчета);
	КонецЕсли;
//-- Локализация
...
Функция ДанныеФильтраПоНоменклатуре(ИсточникДанных, СтрокаЗамены) Экспорт
	...
	Иначе
		...
		Поля = "ФильтрПоНоменклатуре.Номенклатура,ФильтрПоНоменклатуре.Характеристика,ФильтрПоНоменклатуре.Склад";
...
Процедура ПодготовитьТекстЗапросаДляПроверкиУсловияИРасчетаКратности(ТекстЗапроса, УсловиеПредоставления)
	...
	Если УсловиеПредоставления.КритерийОграниченияПримененияЗаОбъемПродаж = Перечисления.КритерииОграниченияПримененияСкидкиНаценкиЗаОбъемПродаж.Сумма Тогда
		...
		Если УсловиеПредоставления.ТипСравнения = Перечисления.ТипыСравненияЗначенийСкидокНаценок.НеМенее Тогда
			Если УсловиеПредоставления.ЗначениеУсловияОграничения <> 0 Тогда
				ТекстКратности = "ВЫБОР КОГДА ЕСТЬNULL(Товары.Сумма, 0) > 0 ТОГДА ВЫРАЗИТЬ((ЕСТЬNULL(Товары.Сумма, 0) / &ЗначениеУсловияОграниченияКэшируется - 0.5) КАК Число(15,0)) ИНАЧЕ 0 КОНЕЦ";
			Иначе
				ТекстКратности = "
				|ВЫБОР
	...
	ИначеЕсли УсловиеПредоставления.КритерийОграниченияПримененияЗаОбъемПродаж = Перечисления.КритерииОграниченияПримененияСкидкиНаценкиЗаОбъемПродаж.Количество
		...
		Если УсловиеПредоставления.ТипСравнения = Перечисления.ТипыСравненияЗначенийСкидокНаценок.НеМенее Тогда
			Если УсловиеПредоставления.ЗначениеУсловияОграничения <> 0 Тогда
				ТекстКратности = "ВЫБОР КОГДА ЕСТЬNULL(Товары.Количество, 0) > 0 ТОГДА ВЫРАЗИТЬ((ЕСТЬNULL(Товары.Количество, 0) / &ЗначениеУсловияОграниченияКэшируется - 0.5) КАК Число(15,0)) ИНАЧЕ 0 КОНЕЦ";
			Иначе
				ТекстКратности = "
				|ВЫБОР
...
Функция ДанныеПоТоварамИНаборам(ТекстЗапроса, ПараметрыЗапроса, ИспользоватьНаборы) Экспорт
	...
	//++ Локализация
	ИспользоватьМРЦ = 
		ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПараметрыЗапроса, "Товары")
		И ТипЗнч(ПараметрыЗапроса.Товары) = Тип("ТаблицаЗначений")
		И ПараметрыЗапроса.Товары.Колонки.Найти("ВидПродукцииИС") <> Неопределено;
	//-- Локализация
	...
	//++ Локализация
	|	"+?(ИспользоватьМРЦ, "Товары.ВидПродукцииИС КАК ВидПродукцииИС,","")+"
	//-- Локализация
	|";
	...
	//++ Локализация
	|	"+?(ИспользоватьМРЦ, "Таблица.ВидПродукцииИС КАК ВидПродукцииИС,","")+"
	//-- Локализация
	|";
	...
	//++ Локализация
	|	"+?(ИспользоватьМРЦ, "Неопределено КАК ВидПродукцииИС,","")+"
	//-- Локализация
	...
	|	Таблица.Характеристика,
	//++ Локализация
	|	"+?(ИспользоватьМРЦ, "Таблица.ВидПродукцииИС,","")+"
	//-- Локализация
	|	Таблица.Упаковка,
	|	Таблица.Серия,
	|	Таблица.Склад,
...
Процедура ПриСозданииЧтенииНаСервере(Форма, ТекущийОбъект = Неопределено) Экспорт
	...
	Если ТипЗнч(Форма.Объект.Ссылка) = Тип("СправочникСсылка.УсловияПредоставленияСкидокНаценок") Тогда
		...
		//++ Локализация
		Форма.Элементы.ФормаОплаты.СписокВыбора.Добавить(Перечисления.ФормыОплаты.СистемаБыстрыхПлатежей);
		//-- Локализация
...
Процедура ДобавитьКомандыСозданияНовыхУсловийПредоставленияСкидокНаценок(Форма, ГруппаСоздать) Экспорт
	...
	Для Каждого ЭлементСписка Из КомандыФормы Цикл
		...
		КомандаФормы.Действие = "Подключаемый_ВыполнитьКомандуСозданияУсловийПредоставленияСкидокНаценок";
