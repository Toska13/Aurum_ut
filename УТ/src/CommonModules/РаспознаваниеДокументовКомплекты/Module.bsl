...
Функция КандидатыКомплекта(ДанныеОбязательногоДокумента)
	...
	КандидатыКомплекта = Новый Массив;
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеОбязательногоДокумента, "ВидДокумента") Тогда
		ВидДокументаОтбор = ДанныеОбязательногоДокумента.ВидДокумента;
	Иначе
	...
	|	МАКСИМУМ(ЕСТЬNULL(РаспознанныйДокументРеквизитыДокумента.Значение, """")) КАК ВидДокумента
	|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(Выборка.ВидДокумента) Тогда
			ВидДокумента = ?(ДанныеОбязательногоДокумента.Направление 
				= Перечисления.НаправленияРаспознанногоДокумента.Исходящий,
				"РТИУ",
				"ПТИУ");
		Иначе
		...
		Если ЗначениеЗаполнено(ВидДокументаОтбор) И Не ВидДокумента = ВидДокументаОтбор Тогда
			Продолжить;
		КонецЕсли;
		
		КандидатыКомплекта.Добавить(Выборка.Ссылка);
...
Функция НайтиКомплектныеДокументы(ДанныеОбязательногоДокумента) Экспорт
	...
	Если КомплектныеДокументы.Количество() > 1 Тогда
		РаспознаваниеДокументовСлужебный.СкорректироватьКомплектДокументовПоПравуДоступаДобавление(КомплектныеДокументы,
			ДанныеОбязательногоДокумента.Ссылка);
	КонецЕсли;
...
Функция АктыВыполненныхРаботСУслугами(ТипыДокументов, ПараметрыСоздания)
	
	АктыСУслугами = Новый Массив;
	...
	Для Каждого ТипДокумента Из ТипыДокументов Цикл
		Если ПараметрыСоздания.СозданныеДокументы.Получить(ТипДокумента) = Неопределено Тогда
			Если ТипДокумента = Тип("ДокументСсылка.АктВыполненныхРабот") Тогда
				Акт = ПараметрыСоздания.РаспознанныеДокументыПоТипам.Получить(
					Перечисления.ТипыДокументовРаспознаваниеДокументов.АктОбОказанииУслуг);
				Акты.Добавить(Акт);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	Если ЗначениеЗаполнено(Акты) Тогда
		...
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Акты", Акты);
		...
		|	Документ.РаспознанныйДокумент.РеквизитыТабличныхЧастей КАК РаспознанныйДокументРеквизитыТабличныхЧастей
		|ГДЕ
		|	РаспознанныйДокументРеквизитыТабличныхЧастей.Ссылка В (&Акты)
		|	И НЕ РаспознанныйДокументРеквизитыТабличныхЧастей.СтрокаУдалена
		...
		|	И ВЫРАЗИТЬ(РаспознанныйДокументРеквизитыТабличныхЧастей.Значение КАК Справочник.Номенклатура).ТипНоменклатуры В
		|	(&ТипыНоменклатуры)";
		
		РезультатЗапроса = Запрос.Выполнить();
		Выборка = РезультатЗапроса.Выбрать();
		Пока Выборка.Следующий() Цикл
			АктыСУслугами.Добавить(Выборка.Ссылка);
		КонецЦикла;
	
	КонецЕсли;
	
	Возврат АктыСУслугами;
	
КонецФункции // АктыВыполненныхРаботСУслугами()

// Преобразует двоичные данные картинки в текст HTML
//
// Параметры:
//  ДвоичныеДанные - ДвоичныеДанные
//  Позиция - Строка - положение изображения
// 
...
Процедура ОбновитьИЗаполнитьСвязанныеИСозданныеДокументы(ПараметрыСоздания, КоличествоСвязанных = 0) Экспорт
	...
	АктыСУслугами = АктыВыполненныхРаботСУслугами(ТипыДокументов, ПараметрыСоздания);
	...
	Для Каждого ТипДокумента Из ТипыДокументов Цикл
		Если ПараметрыСоздания.СозданныеДокументы.Получить(ТипДокумента) = Неопределено Тогда
			...
			Если ТипДокумента = Тип("ДокументСсылка.АктВыполненныхРабот") Тогда
				Акт = ПараметрыСоздания.РаспознанныеДокументыПоТипам.Получить(
					Перечисления.ТипыДокументовРаспознаваниеДокументов.АктОбОказанииУслуг);
				Если АктыСУслугами.Найти(Акт) = Неопределено Тогда
					Продолжить;	
				КонецЕсли;
			КонецЕсли;
...
Процедура ДополнитьДанныеДокументовПоСтроке(СтрокаТаблицы) Экспорт
	...
	СтрокаТаблицы.Наименование = СтрШаблон(НСтр("ru = '%1 № %2'"), СтрокаТаблицы.ТипДокумента,
		СтрокаТаблицы.НомерДокумента);
