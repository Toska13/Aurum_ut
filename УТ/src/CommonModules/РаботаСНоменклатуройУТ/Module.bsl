...
Процедура СоздатьНоменклатуру(ДанныеЗаполнения, НоменклатураСсылка) Экспорт
	...
	Для каждого Элемент Из ДанныеЗаполнения.Классификаторы Цикл
		Если СтрНайти(ВРег(Элемент.Наименование), "TNVED") <> 0 Тогда
			НоменклатураОбъект.КодТНВЭД = НайтиСоздатьЭлементКлассификатораТНВЭД(Элемент.Идентификатор);
		ИначеЕсли СтрНайти(ВРег(Элемент.Наименование), "OKPD2") <> 0 Тогда
			НоменклатураОбъект.КодОКПД2 = НайтиСоздатьЭлементКлассификатораОКПД2(Элемент.Идентификатор);
		КонецЕсли;
	КонецЦикла; 
	...
	Если ДанныеНоменклатурыСервиса.Классификаторы.Количество() > 0 Тогда
		Для каждого Элемент Из ДанныеНоменклатурыСервиса.Классификаторы Цикл
			...
			Если СтрНайти(ВРег(Элемент.Наименование), "TNVED") <> 0 и Строка(КодТНВЭД) <> Элемент.Идентификатор Тогда
				КодТНВЭДСервис = НайтиСоздатьЭлементКлассификатораТНВЭД(Элемент.Идентификатор);
			...
			ИначеЕсли СтрНайти(ВРег(Элемент.Наименование), "OKPD2") <> 0 и Строка(КодОКПД2) <> Элемент.Идентификатор Тогда
				КодОКПД2Сервис = НайтиСоздатьЭлементКлассификатораОКПД2(Элемент.Идентификатор);
...
Процедура ПоискНоменклатурыПоШтрихкодуПриСозданииНаСервере(Форма) Экспорт
	...
	ПоискНоменклатурыПоШтрихкодуИнициализироватьФорму(Форма);
	ПоискНоменклатурыПоШтрихкодуУстановитьУсловноеОформление(Форма);
	ПоискНоменклатурыПоШтрихкодуЗаполнитьШтрихкодыНоменклатуры(Форма);
...
Процедура ПоискНоменклатурыПоШтрихкодуОбработкаПроверкиЗаполнения(Форма, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	РегистрыСведений.ШтрихкодыНоменклатуры.ПроверитьЗаполнениеСерии(ШтрихкодыНоменклатуры, Отказ);
	РегистрыСведений.ШтрихкодыНоменклатуры.ПроверкаНеуникальныхШтрихкодовНоменклатуры(ШтрихкодыНоменклатуры, Отказ);
...
Процедура ЗарегистрироватьШтрихкоды(ШтрихкодыНоменклатуры) Экспорт
	...
	РегистрыСведений.ШтрихкодыНоменклатуры.РегистрацияШтрихкодовНоменклатуры(ШтрихкодыНоменклатуры);
	
КонецПроцедуры

// См. РаботаСНоменклатуройПереопределяемый.ПоискНоменклатурыПоШтрихкодуДанныеПоШтрихкодам.
Процедура ПоискНоменклатурыПоШтрихкодуДанныеПоШтрихкодам(ДанныеПоШтрихкодам) Экспорт
	
	ДанныеПоШтрихкодам.Вставить("Зарегистрирован",				Ложь);
	ДанныеПоШтрихкодам.Вставить("ЗарегистрированОбработкой",	Ложь);
	ДанныеПоШтрихкодам.Вставить("Серия",						Неопределено);
	ДанныеПоШтрихкодам.Вставить("Упаковка",						Неопределено);
...
Процедура ПолучитьЗначенияРеквизитовНоменклатурыИзФормы(Форма, ЗначенияРеквизитов) Экспорт
	...
	Объект = Форма.Объект; // СправочникОбъект.Номенклатура -
	
	Для каждого ЭлементКоллекции Из ЗначенияРеквизитов Цикл
		ЗначенияРеквизитов[ЭлементКоллекции.Ключ] = Объект[ЭлементКоллекции.Ключ];
	КонецЦикла;
	
	ПараметрыОтбора = РегистрыСведений.ШтрихкодыНоменклатуры.ПараметрыОтбораШтрихкодовНоменклатурыПоУмолчанию();
	ПараметрыОтбора.Номенклатура	= Объект.Ссылка;
	ПараметрыОтбора.Упаковка		= Объект.ЕдиницаИзмерения;
	
	ШтрихКоды = РегистрыСведений.ШтрихкодыНоменклатуры.ШтрихкодыНоменклатуры(ПараметрыОтбора);
	...
	ИначеЕсли ДанныеИнтерактивногоЗаполнения <> Неопределено Тогда
		...
		Для каждого ДанныеЗаполнения Из ДанныеИнтерактивногоЗаполнения Цикл
			...
			Если ЕстьКлассификаторы Тогда
				Для каждого Элемент Из ДанныеЗаполнения.Классификаторы Цикл
					Если СтрНайти(ВРег(Элемент.Наименование), "TNVED") <> 0 Тогда
						Форма.Объект.КодТНВЭД = НайтиСоздатьЭлементКлассификатораТНВЭД(Элемент.Идентификатор);
					ИначеЕсли СтрНайти(ВРег(Элемент.Наименование), "OKPD2") <> 0 Тогда
						Форма.Объект.КодОКПД2 = НайтиСоздатьЭлементКлассификатораОКПД2(Элемент.Идентификатор);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли; 
...
// Если их нет, то создает элементы справочника в соответствии с данными сервиса 1С:Облачные классификаторы.
//
// Параметры:
//  КодКлассификатора - Строка - Строка с кодом классификатора ТН ВЭД.
//
// Возвращаемое значение:
//  СправочникСсылка.КлассификаторТНВЭД - ссылка на элемент классификатора.
Функция НайтиСоздатьЭлементКлассификатораТНВЭД(КодКлассификатора) Экспорт
	
	КодКонфигуратора = СтрЗаменить(КодКлассификатора, " ", "");
	Результат = Справочники.КлассификаторТНВЭД.НайтиПоКоду(КодКонфигуратора);
	Отказ = Ложь;
	...
	Если Результат.Пустая() Тогда
		...
		ОписаниеПараметровЗапроса.Коды       = КодКлассификатора;
		...
		Если ДанныеСервиса.Количество() Тогда
...
//
// Параметры:
//  КодКлассификатора - Строка - Строка с кодом классификатора ОКПД2.
//
// Возвращаемое значение:
//  СправочникСсылка.КлассификаторОКПД2 - ссылка на элемент классификатора.
Функция НайтиСоздатьЭлементКлассификатораОКПД2(КодКлассификатора) Экспорт
	
	Результат = Справочники.КлассификаторОКПД2.НайтиПоКоду(КодКлассификатора);
	Отказ = Ложь;
	...
	Если Результат.Пустая() Тогда
		...
		ОписаниеПараметровЗапроса.НаборПолей = РаботаСНоменклатуройСлужебный.НаборПолейСтандартный();
		ОписаниеПараметровЗапроса.Код        = КодКлассификатора;
...
Процедура ФормализованныеРеквизитыНоменклатурыДляВыгрузки(ФормализованныеРеквизиты) Экспорт 
	...
	Если Не ФормализованныеРеквизиты.Свойство("Сертификаты") Тогда
		ФормализованныеРеквизиты.Вставить("Сертификаты", НСтр("ru = 'Сертификаты номенклатуры'"));
	КонецЕсли;
	Если Не ФормализованныеРеквизиты.Свойство("СоставНаборов") Тогда
		ФормализованныеРеквизиты.Вставить("СоставНаборов", НСтр("ru = 'Состав наборов'"));
	КонецЕсли;
...
Процедура ИнициализацияЗапросаВыборкиДанныхДляВыгрузки(Запрос) Экспорт
	...
	+ УчетПодакцизныхТоваров.ТекстЗапросаВтПодакцизныеТовары("ОтборНоменклатуры")
	...
	|			ТОГДА &НДС20
	|		КОГДА СправочникНоменклатура.СтавкаНДС.ПеречислениеСтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС22)
	|			ТОГДА &НДС22
	|		КОГДА СправочникНоменклатура.СтавкаНДС.ПеречислениеСтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС22_122)
	|			ТОГДА &НДС22
	...
	|	ЛОЖЬ КАК ПродукцияМаркируемаяДляГИСМ,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатуры КАК ХарактеристикиНоменклатуры
	|		ПО ОтборНоменклатуры.Характеристика = ХарактеристикиНоменклатуры.Ссылка
	...
	|		ПО ОтборНоменклатуры.Номенклатура = ВтПодакцизныеТовары.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеВетИС КАК ДанныеВетИС
	|		ПО ДанныеВетИС.Номенклатура = ОтборНоменклатуры.Номенклатура
	|			И (ДанныеВетИС.Характеристика = ОтборНоменклатуры.Характеристика)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура,
	|	Характеристика
	...
	|	ХарактеристикиНоменклатурыДополнительныеРеквизиты.ТекстоваяСтрока,
	|	ИСТИНА
	|ИЗ
	|	Товары КАК Товары
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Сертификаты.Номенклатура КАК Номенклатура,
	|	Сертификаты.Характеристика КАК Характеристика,
	|	Сертификаты.Сертификат КАК Сертификат,
	|	Сертификаты.Номер КАК Номер,
	|	Сертификаты.ДатаНачалаСрокаДействия КАК ДатаНачалаСрокаДействия,
	|	Сертификаты.ДатаОкончанияСрокаДействия КАК ДатаОкончанияСрокаДействия,
	|	Сертификаты.ТипСертификата КАК ТипСертификата,
	|	Сертификаты.ОрганВыдавшийДокумент КАК ОрганВыдавшийДокумент
	...
	|			ПО ОтборНоменклатуры.ВидНоменклатуры = Сертификаты.ВидНоменклатуры
	|				И Сертификаты.Номенклатура = &ПустоеЗначениеНоменклатуры
	|				И Сертификаты.Характеристика = &ПустоеЗначениеХарактеристики
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОбластиДействияСертификатовНоменклатуры КАК Сертификаты
	|			ПО ОтборНоменклатуры.ВидНоменклатуры = Сертификаты.ВидНоменклатуры
	|				И ОтборНоменклатуры.Номенклатура = Сертификаты.Номенклатура
	|				И Сертификаты.Характеристика = &ПустоеЗначениеХарактеристики
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	|			ПО ОтборНоменклатуры.ВидНоменклатуры = Сертификаты.ВидНоменклатуры
	|				И ОтборНоменклатуры.Номенклатура = Сертификаты.Номенклатура
	|				И (ОтборНоменклатуры.Характеристика ЕСТЬ NULL)) КАК Сертификаты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|		ПО СправочникНоменклатура.Ссылка = Сертификаты.Номенклатура
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СертификатыНоменклатуры КАК СправочникСертификатыНоменклатуры
	|		ПО СправочникСертификатыНоменклатуры.Ссылка = Сертификаты.Сертификат
	|ГДЕ
	|	СправочникНоменклатура.ВестиУчетСертификатовНоменклатуры
	|	И НЕ СправочникСертификатыНоменклатуры.ПометкаУдаления
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура
	|;
	...
	|		СУММА(ВариантыКомплектацииНоменклатурыТовары.КоличествоУпаковок) КАК Количество
	|	ИЗ
	|		ОтборНоменклатуры КАК ОтборНоменклатуры
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|			ПО ОтборНоменклатуры.Номенклатура = СправочникНоменклатура.Ссылка
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВариантыКомплектацииНоменклатуры КАК ВариантыКомплектацииНоменклатуры
	|			ПО ОтборНоменклатуры.Номенклатура = ВариантыКомплектацииНоменклатуры.Владелец
	|				И ОтборНоменклатуры.Характеристика = ВариантыКомплектацииНоменклатуры.Характеристика
	...
	|		ВариантыКомплектацииНоменклатурыТовары.Упаковка) КАК ВложенныйЗапрос
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ШтрихкодыНоменклатуры КАК Штрихкоды
	|	ПО ВложенныйЗапрос.НоменклатураНабора = Штрихкоды.Номенклатура
	|		И ВложенныйЗапрос.ХарактеристикаНабора = Штрихкоды.Характеристика
	|		И ВложенныйЗапрос.УпаковкаНабора = Штрихкоды.Упаковка
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ВложенныйЗапрос.УпаковкаНабора
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура";
	
	Запрос.УстановитьПараметр("ПустоеЗначениеНоменклатуры", Метаданные.ОпределяемыеТипы.НоменклатураРаботаСНоменклатурой.Тип.ПривестиЗначение());
...
Процедура ПолучитьТоварныеГруппыНоменклатуры(ТоварныеГруппыНоменклатуры) Экспорт 
	...
	Пока Выборка.Следующий() Цикл
		ТоварныеГруппыНоменклатуры.Вставить(
			Выборка.Ссылка,
			КонструкторИСМПОбщегоНазначенияКлиентСервер.ИмяТоварнойГруппы(Выборка.ВидПродукцииГИСМТ));
	КонецЦикла;
КонецПроцедуры
...
// См. РаботаСНоменклатуройПереопределяемый.ИнициализацияЗапросаОтбораНоменклатурыСХарактеристиками.
Процедура ИнициализацияЗапросаОтбораНоменклатурыСХарактеристиками(ТекстЗапроса) Экспорт 
	...
	|	Справочник.ХарактеристикиНоменклатуры КАК ХарактеристикиНоменклатуры
	|ГДЕ
	|	НЕ ХарактеристикиНоменклатуры.Владелец.ИспользованиеХарактеристик = ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.НеИспользовать)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка,
	|	Характеристика
	...
	|ВЫБРАТЬ
	|	СправочникНоменклатура.Ссылка КАК Номенклатура,
	|	СправочникНоменклатура.Характеристика КАК Характеристика
	|ИЗ
	|	Вт_ИсходныеДанные КАК СправочникНоменклатура
	|{ГДЕ
	|	СправочникНоменклатура.Ссылка.* КАК Номенклатура,
	|	СправочникНоменклатура.Ссылка.ВидНоменклатуры.* КАК ВидНоменклатуры,
	|	СправочникНоменклатура.Ссылка.Родитель.* КАК Родитель,
	|	СправочникНоменклатура.Ссылка.Код,
...
Функция ПолучитьШтрихкодыПоНоменклатуре(Номенклатура, Упаковка)
...
Процедура ПоискНоменклатурыПоШтрихкодуИнициализироватьФорму(Форма)
	
	Элементы = Форма.Элементы;
	
	СвязьПараметраВыбораСНоменклатурой		= Новый СвязьПараметраВыбора(
														"Номенклатура",
														"Элементы.ШтрихкодыНоменклатуры.ТекущиеДанные.Номенклатура",
														РежимИзмененияСвязанногоЗначения.НеИзменять);
	СвязьПараметраВыбораСХарактеристикой	= Новый СвязьПараметраВыбора("Характеристика",
														"Элементы.ШтрихкодыНоменклатуры.ТекущиеДанные.Характеристика",
														РежимИзмененияСвязанногоЗначения.НеИзменять);
	
	// Связи параметров выбора с номенклатурой
	СвязиПараметровВыбора = Новый Массив;
	СвязиПараметровВыбора.Добавить(СвязьПараметраВыбораСНоменклатурой);
	
	// Связь серии с номенклатурой.
	СвязиПараметровВыбораСерии = Новый Массив;
	СвязиПараметровВыбораСерии.Добавить(СвязьПараметраВыбораСНоменклатурой);
	СвязиПараметровВыбораСерии.Добавить(СвязьПараметраВыбораСХарактеристикой);
	
	// Параметры выбора упаковки.
	ПараметрыВыбораУпаковки = Новый Массив;
	ПараметрыВыбораУпаковки.Добавить(Новый ПараметрВыбора("ПоказыватьТоварныеМеста", Истина));
	ПараметрыВыбораУпаковки.Добавить(Новый ПараметрВыбора("ПоказыватьМерныеЕдиницыИзмерения", Ложь));
	...
	//++ Локализация
	МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы(
	...
	МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы(
												"СерииИспользуются",
												ТипБулево, "ШтрихкодыНоменклатуры"));
	...
	НовыйЭлемент				= Элементы.Вставить(
									...
									Элементы.ШтрихкодыНоменклатурыНоменклатураСервиса);
	...
	НовыйЭлемент				= Элементы.Вставить(
									"ГруппаУпаковкаЕдиницаИзмерения",
									Тип("ГруппаФормы"),
									Элементы.ШтрихкодыНоменклатуры,
									Элементы.ШтрихкодыНоменклатурыКоличество);
	...
	НовыйЭлемент							= Элементы.Вставить(
												"Упаковка",
												Тип("ПолеФормы"),
												Элементы.ГруппаУпаковкаЕдиницаИзмерения);
	НовыйЭлемент.Вид						= ВидПоляФормы.ПолеВвода;
	...
	НовыйЭлемент.ПараметрыВыбора			= Новый ФиксированныйМассив(ПараметрыВыбораУпаковки);
	НовыйЭлемент.Видимость					= Истина;
	...
	НовыйЭлемент.ФормаВыбора				= "Справочник.УпаковкиЕдиницыИзмерения.Форма.ФормаВыбораИзДокументов";
	НовыйЭлемент.Ширина						= 10;
	
	НовыйЭлемент.УстановитьДействие("ПриИзменении", "Подключаемый_ПоискНоменклатурыПоШтрихкодуПриИзменении");
	
	Элемент = Элементы.Найти("ШтрихкодыНоменклатурыЕдиницаИзмерения");
	Элемент.ОтображатьВШапке	= Ложь;
	Элемент.Видимость			= Ложь;
	
	Элементы.Переместить(Элемент, Элементы.ГруппаУпаковкаЕдиницаИзмерения);
	...
	НовыйЭлемент							= Элементы.Вставить(
												...
												Элементы.ГруппаУпаковкаЕдиницаИзмерения);
	НовыйЭлемент.Вид						= ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.Заголовок					= НСтр("ru = 'Серия'");
	НовыйЭлемент.ПутьКДанным				= "ШтрихкодыНоменклатуры.Серия";
	НовыйЭлемент.СвязиПараметровВыбора		= Новый ФиксированныйМассив(СвязиПараметровВыбораСерии);
	НовыйЭлемент.Видимость					= ПолучитьФункциональнуюОпцию("ИспользоватьСерииНоменклатурыСклад");
	НовыйЭлемент.КнопкаВыбора				= Истина;
	НовыйЭлемент.КнопкаВыпадающегоСписка	= Ложь;
	НовыйЭлемент.ОтображениеКнопкиВыбора	= ОтображениеКнопкиВыбора.ОтображатьВПолеВвода;
...
Процедура ПоискНоменклатурыПоШтрихкодуУстановитьУсловноеОформление(Форма)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСерииНоменклатурыСклад") Тогда
		...
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных("Серия");
	...
	ПараметрыУсловногоОформления.ПутьКПолюУпаковка			= "ШтрихкодыНоменклатуры.Упаковка";
	
	НоменклатураСервер.УстановитьУсловноеОформлениеЕдиницИзмерения(Форма, ПараметрыУсловногоОформления);
	
КонецПроцедуры

Процедура ПоискНоменклатурыПоШтрихкодуЗаполнитьШтрихкодыНоменклатуры(Форма)
	
	ШтрихкодыНоменклатуры			= Форма.ШтрихкодыНоменклатуры;
	ТаблицаШтрихкодыНоменклатуры	= ШтрихкодыНоменклатуры.Выгрузить(); // ТаблицаЗначений -
	Штрихкоды						= ТаблицаШтрихкодыНоменклатуры.ВыгрузитьКолонку("Штрихкод");
	...
	ТаблицаШтрихкодыНоменклатуры.Очистить();
	ТаблицаШтрихкодыНоменклатуры.Индексы.Добавить("Штрихкод");
	...
	Для Каждого СтрокаТЧ Из ШтрихкодыНоменклатуры Цикл
		ДанныеШтрихкода = КэшированныеЗначения.Штрихкоды[СтрокаТЧ.Штрихкод];
		НайденнаяСтрока = ТаблицаШтрихкодыНоменклатуры.Найти(СтрокаТЧ.Штрихкод, "Штрихкод");
		
		Если НайденнаяСтрока = Неопределено Тогда
			НоваяСтрока = ТаблицаШтрихкодыНоменклатуры.Добавить();
	...
	ШтрихкодыНоменклатуры.Загрузить(ТаблицаШтрихкодыНоменклатуры);
