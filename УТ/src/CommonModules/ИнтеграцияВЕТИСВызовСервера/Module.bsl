...
// Заполняет форматированную строку (связи с документами ВЕТИС) для формы прикладного документа со статусом оформления ИСМП
//
// Параметры:
...
//  Структура:
//  * ТекстНадписи - ФорматированнаяСтрока, Строка - описание доступных действий ВЕТИС
//  * ДанныеОписания - Массив из см. ИнтеграцияВЕТИС.ОписаниеОформленныхДокументовВЕТИС
...
// 
// Параметры:
// 	ДокументСсылка - ДокументСсылка - Документ, которому необходимо установить соответствующий статус.
// Возвращаемое значение:
// 	Структура - Описание:
...
Процедура ЗаполнитьСсылкуИПредставлениеСопоставленияТорговогоОбъекта(СоответствуетОрганизации, ИмяХозяйствующегоСубъекта, СсылкаИПредставление, СопоставленныеОбъекты)
	...
	Если СопоставленоОбъектов = 0
		И ((СоответствуетОрганизации И Не ИнтеграцияИС.УпрощеннаяИнтеграцияБЭД())
			Или ИнтеграцияВЕТИС.ИспользуетсяСопоставлениеТорговыхОбъектовКонтрагентов()) Тогда
			...
			Если РезультатВыполненияЗапроса <> Неопределено
				...
				Если ЭлементыДанныхСпискаКЗагрузке.Количество() > 0 Тогда
					...
					Исключение
						...
						ИнтеграцияВЕТИС.ЗаписатьОшибкуВЖурналРегистрации(ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
//    * ПродукцияВЕТИС        - СправочникСсылка.ПродукцияВЕТИС  - продукция ВетИС
Функция ПараметрыСозданияНоменклатуры(ПродукцияВЕТИС, ЕдиницаИзмеренияВЕТИС, ТребуетсяСопоставление = Ложь) Экспорт
	...
	ПараметрыНоменклатуры.Вставить("ПродукцияВЕТИС");
	...
	|	ПродукцияВЕТИС.Ссылка       КАК ПродукцияВЕТИС,
	...
	Если РеквизитыПродукцииВЕТИС.Следующий() Тогда
		...
		ПараметрыНоменклатуры.ПродукцияВЕТИС    = РеквизитыПродукцииВЕТИС.ПродукцияВЕТИС;
	КонецЕсли;
...
Функция ДанныеАдресаПоАдресуXML(АдресXML, АдресПредставление) Экспорт
	...
	ДанныеАдреса.НомерСтроения            = СтрСоединить(Корпуса, ", ");
...
Функция ОтправитьДляОбработки(ТелоЗапроса, Порт, АдресРесурса, SOAPAction, НастройкаОбмена)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			...
			ТекстСообщения + Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
// 
// Возвращаемое значение:
//  Массив Из См. ИнтеграцияВЕТИСКлиентСервер.СтруктураСообщенияXML
//
Функция СообщениеКПередачеXML(ДокументСсылка, ПараметрыПередачи, ДополнительныеПараметры = Неопределено)
	...
	Для Каждого ЭлементДанных Из ВозвращаемоеЗначение Цикл
		ЭлементДанных.ДальнейшееДействие = ПараметрыПередачи.ДальнейшееДействие;
	КонецЦикла;
