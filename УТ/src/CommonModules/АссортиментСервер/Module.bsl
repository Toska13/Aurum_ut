...
Функция ТекстыЗапросовЗаполненияЦенПакетнаяОбработка()
	...
	|	ЕСТЬNULL(ТаблицаКомплектующих.Ссылка.Владелец, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)) КАК ВладелецКомплекта
	|ПОМЕСТИТЬ ПакетнаяОбработка_Ассортимент_ВладельцыКомплектов
	|ИЗ
	...
	|		ПО ВтИсточникДанных.Номенклатура = ТаблицаКомплектующих.Номенклатура
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура,
	|	ВладелецКомплекта";
	
	Результат.Добавить(ТекстЗапроса, "ПакетнаяОбработка_Ассортимент_ВладельцыКомплектов");
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаВладельцев.Номенклатура КАК Номенклатура,
	|	МАКСИМУМ(ЕСТЬNULL(ТаблицаСреза.ВидЦены, ЗНАЧЕНИЕ(Справочник.ВидыЦен.ПустаяСсылка))) КАК ВидЦены
	|ПОМЕСТИТЬ ПакетнаяОбработка_Ассортимент_Ассортимент
	|ИЗ
	|	ПакетнаяОбработка_Ассортимент_ВладельцыКомплектов КАК ТаблицаВладельцев
	...
	|
	|			ВЫБРАТЬ
	|				ТаблицаВладельцев.ВладелецКомплекта КАК ВладелецКомплекта
	|			ИЗ
	|				ПакетнаяОбработка_Ассортимент_ВладельцыКомплектов КАК ТаблицаВладельцев)) КАК ТаблицаСреза
	|		ПО ТаблицаСреза.Номенклатура В (ТаблицаВладельцев.Номенклатура, ТаблицаВладельцев.ВладелецКомплекта)
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаВладельцев.Номенклатура
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура";
	
	Результат.Добавить(ТекстЗапроса, "ПакетнаяОбработка_Ассортимент_Ассортимент");
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаСреза.Номенклатура КАК Номенклатура,
	|	ТаблицаСреза.Характеристика КАК Характеристика,
	|	ТаблицаСреза.ВидЦены КАК ВидЦены,
	|	ТаблицаСреза.Валюта КАК Валюта,
	|	ТаблицаСреза.Упаковка КАК Упаковка,
	|	ТаблицаСреза.Цена КАК Цена
	|ПОМЕСТИТЬ ПакетнаяОбработка_Ассортимент_ЦеныНоменклатуры
	|ИЗ
	|	РегистрСведений.ЦеныНоменклатуры.СрезПоследних(КОНЕЦПЕРИОДА(&Дата, ДЕНЬ), (Номенклатура, Характеристика) В
	|		(ВЫБРАТЬ
	|			ВтИсточникДанных.Номенклатура КАК Номенклатура,
	|			ВтИсточникДанных.Характеристика КАК Характеристика
	|		ИЗ
	|			ВтИсточникДанных КАК ВтИсточникДанных)) КАК ТаблицаСреза
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура,
	|	Характеристика,
	...
	Результат.Добавить(ТекстЗапроса, "ПакетнаяОбработка_Ассортимент_ЦеныНоменклатуры");
	...
	|ИЗ
	|	ВтИсточникДанных КАК ВтИсточникДанных
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПакетнаяОбработка_Ассортимент_Ассортимент КАК Ассортимент
	|		ПО ВтИсточникДанных.Номенклатура = Ассортимент.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПакетнаяОбработка_Ассортимент_ЦеныНоменклатуры КАК ЦеныНоменклатурыСрезПоследних
