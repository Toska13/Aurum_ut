...
Функция ЗагрузитьВходящиеДокументыУниверсально(ПараметрыЗагрузки, ПараметрыОбмена) Экспорт
	...
	Объект.Вставить("Ссылка", Документы.ПриемкаТоваровИСМП.ПустаяСсылка());
	...
	Для Каждого ОрганизацияДляОбмена Из Организации Цикл
		...
		Для Каждого ТоварнаяГруппа Из ДоступныеТоварныеГруппы Цикл
			...
			АбстрактноеСообщениеJSON.ТоварнаяГруппа            = ТоварнаяГруппа;
...
Функция ОпределитьСтанциюУправленияЗаказами(СообщенияJSON, СообщениеJSON, Шапка, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли РезультатПолученияНастройки.НастройкаОбмена.ФорматОбмена = Перечисления.ВерсииФорматаОбменаСУЗ.V1 Тогда
		ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(
			СообщениеJSON,
			СтрШаблон(НСтр("ru = 'Формат обмена с СУЗ версии v1 не поддерживается для товарной группы: %1'") , Шапка.ТоварнаяГруппа));
		СообщенияJSON.Добавить(СообщениеJSON);
		ВозвращаемоеЗначение.Успешно = Ложь;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеШтрихкодовУпаковок(Объект, Отказ) Экспорт
	...
	ВидыПродукцииПоТоварнойГруппе = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукции(Объект.ТоварнаяГруппа);
	ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(Объект, Неопределено, ВидыПродукцииПоТоварнойГруппе);
	...
	ЭтоВидПродукцииСоСрокамиГодности = КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСоСрокамиГодности(Объект.ТоварнаяГруппа);
	ЭтоПродукцияПодконтрольнаяВЕТИС = ИнтеграцияИСМПВЕТИСКлиентСервер.ЭтоВидПродукцииИСМПВЕТИС(Объект.ТоварнаяГруппа);
	
	ЭтоПродукцияСоСрокамиГодностиИлиПодконтрольнаяВЕТИС =
		(ТипЗнч(Объект) = Тип("ДокументОбъект.МаркировкаТоваровИСМП") Или ТипЗнч(Объект) = Тип("ДокументОбъект.УточнениеСведенийОКодахМаркировкиИСМП"))
		И (ЭтоВидПродукцииСоСрокамиГодности Или ЭтоПродукцияПодконтрольнаяВЕТИС);
	...
	Если ЭтоПродукцияСоСрокамиГодностиИлиПодконтрольнаяВЕТИС Тогда
	...
	Для Каждого Расхождение Из РезультатПроверки.СтрокиСРасхождением Цикл
		...
		ИначеЕсли ЭтоПродукцияСоСрокамиГодностиИлиПодконтрольнаяВЕТИС Тогда
...
Функция ЗаполнитьGTINМаркировкиОстатков(ТаблицаНоменклатуры, Организация, ТоварнаяГруппа) Экспорт
	...
	ПрефиксыНаименования = ЗаказНаЭмиссиюСУЗКлиентСервер.ПрефиксыНаименованияУпрощенногоОписанияОстатков(ТоварнаяГруппа);
	...
	Для Каждого СтрокаТовары Из ТаблицаНоменклатуры Цикл
		...
		МассивСтрокПоиска.Добавить(СтрокаДляПоискаGTINМаркировкиОстатков(ТоварнаяГруппа, СтрокаТовары));
		Для Каждого ПрефиксНаименования Из ПрефиксыНаименования Цикл
			МассивСтрокПоиска.Добавить(
				СтрШаблон("%1. %2",
					ПрефиксНаименования,
					МассивСтрокПоиска[0]));
		КонецЦикла;
	...
	ПараметрыЗаполнения.Вставить("ТоварнаяГруппа",      ТоварнаяГруппа);
	...
	Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМОТП(ТоварнаяГруппа) Тогда
	...
	Пока СоответствиеСтрокПоиска.Количество() И ПовторятьИтерацию Цикл
		...
		РезультатЗапроса = ИнтерфейсИСМП.НайтиПродукциюПоИННПроизводителя(Неопределено, ТоварнаяГруппа, НомерСтраницы, Организация);
...
Функция ОбязательныеПоляОписанияОстатковПоСтрокеДокумента(СтрокаТовары, ТоварнаяГруппа) Экспорт
	
	СтрокаОписания = Новый Структура;
	
	КоличествоЗнаковТНВЭД = МаркировкаТоваровКлиентСервер.КоличествоЗнаковТНВЭД(ТоварнаяГруппа);
	ЗначениеТНВЭД = Лев(СтрокаТовары.КодТНВЭД, КоличествоЗнаковТНВЭД);
	
	Если КоличествоЗнаковТНВЭД = 2 Тогда
		СтрокаОписания.Вставить("tnved_code_2", ЗначениеТНВЭД);
	ИначеЕсли КоличествоЗнаковТНВЭД = 4 Тогда
	...
	Если ПараметрыОписанияОстатков.ИспользоватьТоварныйЗнак Тогда
		...
		Если Не ЗначениеЗаполнено(ТоварныйЗнак)
			И ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМОТП(ТоварнаяГруппа) Тогда
...
Функция ПодготовитьОчередьЗагрузкиКодовМаркировкиПоЗаказуНаЭмиссию(ПараметрыЗапроса, ЭлементОчереди, ЗагружатьДо, ПараметрыОбмена, НастройкаОбменаСУЗ) Экспорт
	...
	СообщениеJSON.Описание = ОбменДаннымиИСМП.ОписаниеОперацииПолученияДанных(Операция);
	...
	СообщениеJSON.ТоварнаяГруппа            = ЭлементОчереди.ТоварнаяГруппа;
...
Функция ПодготовитьКПередачеСообщениеНаЗакрытиеПодзаказаНаЭмиссиюКодовМаркировки(ЭлементОчередиОснование, Знач ЗагружатьДо, ПараметрыЗапроса, ПараметрыОбмена, НастройкаОбменаСУЗ) Экспорт
	...
	СообщениеJSON.ТоварнаяГруппа            = ЭлементОчередиОснование.ТоварнаяГруппа;
	СообщениеJSON.Операция                  = Операция;
	СообщениеJSON.Назначение                = Перечисления.НазначениеСообщенийИСМП.СтанцияУправленияЗаказами;
	СообщениеJSON.ФорматОбмена              = НастройкаОбменаСУЗ.ФорматОбмена;
	СообщениеJSON.СтанцияУправленияЗаказами = НастройкаОбменаСУЗ.СтанцияУправленияЗаказами;
...
Функция ИндексКартинкиПоВидуУпаковкиИВидуПродукции(ВидУпаковки) Экспорт
	...
	ИначеЕсли ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая
		...
		СмещениеВидаУпаковки = 3;
	Иначе
		СмещениеВидаУпаковки = 2;
	КонецЕсли;
	
	Возврат СмещениеВидаУпаковки;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗагрузитьВходящиеДокументыДлительнаяОперация", Истина);
	
КонецПроцедуры

#Область Сырье

Функция СырьеПоОстаткамНаСкладе(Организация = Неопределено, ПараметрыПоиска = Неопределено) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ТребуетсяОбновлениеКлючаСессии", Ложь);
	Результат.Вставить("ТекстОшибки",                    "");
	Результат.Вставить("Сырье",                          Новый Соответствие);
	
	ВидыПродукцииСырьеГИСМТ = КонструкторИСМПКлиентСервер.ВидыПродукцииСырьеГИСМТ();
	
	КешТоварныхГрупп = Новый Соответствие;
	
	Для Каждого ВидПродукции Из ВидыПродукцииСырьеГИСМТ Цикл
		
		ТоварнаяГруппа = ИнтерфейсИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппа(ВидПродукции);
		Если КешТоварныхГрупп[ТоварнаяГруппа] <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		КешТоварныхГрупп.Вставить(ТоварнаяГруппа, Истина);
		
		ПараметрыНавигации = ИнициализироватьПараметрыНавигации();
		ПараметрыНавигации.ЕстьСледующаяСтраница = Истина;
		ПараметрыНавигации.КоличествоЗаписей     = 50;
		
		Пока ПараметрыНавигации.ЕстьСледующаяСтраница Цикл
			
			РезультатЗапроса = ИнтерфейсИСМП.ОстаткиСырьяПостранично(ПараметрыНавигации, ВидПродукции, Организация, ПараметрыПоиска);
			
			Если ЗначениеЗаполнено(РезультатЗапроса.ТекстОшибки)
				Или РезультатЗапроса.ТребуетсяОбновлениеКлючаСессии
				Или РезультатЗапроса.Сырье.Количество() = 0 Тогда
				ЗаполнитьЗначенияСвойств(Результат, РезультатЗапроса, "ТребуетсяОбновлениеКлючаСессии, ТекстОшибки");
				Прервать;
			КонецЕсли;
			
			Для Каждого КлючИЗначение Из РезультатЗапроса.Сырье Цикл
				Результат.Сырье.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение);
			КонецЦикла;
...
// Возвращает список мест осуществления деятельности сырья постранично через интерфейс ИСМП.
//
// Параметры:
//   Организация - СправочникСсылка.Организации
//   ПараметрыОтбора - Структура - параметры фильтрации, может содержать КоличествоЗаписейВЗапросе
//
// Возвращаемое значение:
//   Структура:
...
Функция СписокМестОсуществленияДеятельностиСырье(Организация, ПараметрыОтбора) Экспорт
	...
	ОтборПоИНН = Новый Массив;
	
	Если ПараметрыОтбора.Свойство("ИННУчастника") Тогда
		ОтборПоИНН = ПараметрыОтбора.ИННУчастника;
	КонецЕсли;
	
	Если ОтборПоИНН.Количество() = 0 Тогда
		РеквизитыОрганизации = ОбщегоНазначенияИС.ИННКППGLNОрганизации(Организация,
			ОбщегоНазначенияИС.ПустоеЗначениеОпределяемогоТипа("ПроизводственныйОбъектИС"));
		
		ОтборПоИНН.Добавить(РеквизитыОрганизации.ИНН);
		
	КонецЕсли;
	
	Для Каждого ИННУчастника Из ОтборПоИНН Цикл
		ПараметрыНавигации = ИнициализироватьПараметрыНавигации();
		ПараметрыНавигации.ЕстьСледующаяСтраница = Истина;

		Если ПараметрыОтбора.Свойство("КоличествоЗаписейВЗапросе") Тогда
			ПараметрыНавигации.КоличествоЗаписей = ПараметрыОтбора.КоличествоЗаписейВЗапросе;
		Иначе
			ПараметрыНавигации.КоличествоЗаписей = 50;
		КонецЕсли;
		
		МассивИННУчастника = Новый Массив;
		МассивИННУчастника.Добавить(ИННУчастника);
		
		ПараметрыОтбора.Вставить("ИННУчастника", МассивИННУчастника);

		Пока ПараметрыНавигации.ЕстьСледующаяСтраница Цикл
			РезультатЗапроса = ИнтерфейсИСМП.МестаОсуществленияДеятельностиСырьеПостранично(Организация,
				ПараметрыНавигации, ПараметрыОтбора);

			Если ЗначениеЗаполнено(РезультатЗапроса.ТекстОшибки) Или РезультатЗапроса.ТребуетсяОбновлениеКлючаСессии
				Или РезультатЗапроса.МестаОсуществленияДеятельности.Количество() = 0 Тогда
				ЗаполнитьЗначенияСвойств(Результат, РезультатЗапроса, "ТребуетсяОбновлениеКлючаСессии, ТекстОшибки");
				Прервать;
			КонецЕсли;

			Для Каждого МОД Из РезультатЗапроса.МестаОсуществленияДеятельности Цикл
				Результат.МестаОсуществленияДеятельности.Добавить(МОД);
			КонецЦикла;
...
Функция МаксимальноеКоличествоОдновременныхВыгрузок() Экспорт
	Возврат 10;
КонецФункции
...
Функция ОбработатьЭлементОчередиПолученияДанных(ЭлементОчереди, ПараметрыОбмена) Экспорт
	...
	Если ЭлементОчереди.Назначение = Перечисления.НазначениеСообщенийИСМП.ИСМП Тогда
		...
		ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ОтчетИСМПОстаткиВиртуальногоСклада Тогда
			ЗагрузитьСообщенияПоЗапросуОстатковВиртуальногоСкладаИСМП(ЭлементОчереди, ПараметрыОбмена);
		Иначе
			СообщениеОбработано = Ложь;
		КонецЕсли;
	...
	Для Каждого СтрокаМаркированногоТовара Из МаркированныеТовары Цикл
		Если ЗначениеЗаполнено(СтрокаМаркированногоТовара.РодительскаяУпаковка) Тогда
			...
			ВидыПродукцииПоТоварнойГруппе = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(
				ОбщийЭлементОчереди.ТоварнаяГруппа);
			...
			НормализованныйКодУпаковки = РазборКодаМаркировкиИССлужебныйКлиентСервер.НормализоватьКодМаркировки(
				ДанныеРазбора, ВидыПродукцииПоТоварнойГруппе, ПараметрыНормализацииПрочее);
			...
			ИначеЕсли ПараметрыОбработки.ОбрабатыватьКодыУпаковок Тогда
				...
				РезультатРазбора = ШтрихкодированиеОбщегоНазначенияИС.НоваяСтруктураОбработкиШтрихкодаУпрощенныйРазбор(
					СтрокаМаркированногоТовара.Штрихкод, ВидыПродукцииПоТоварнойГруппе, НастройкиРазбораКодаМаркировки);
				НормализованныйКодМаркировки = РазборКодаМаркировкиИССлужебныйКлиентСервер.НормализоватьКодМаркировки(
					РезультатРазбора.ДанныеРазбора, ВидыПродукцииПоТоварнойГруппе, ПараметрыНормализацииПрочее);
...
Процедура ЗаполнитьСообщенияJSONФормированиеАТК(СтрокаДереваВерхнегоУровня, СообщенияJSON, ОбщийЭлементОчереди, ЗагружатьДо, Настройки)
	...
	ВидыПродукцииПоТоварнойГруппе = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ОбщийЭлементОчереди.ТоварнаяГруппа);
	...
	Для Каждого СтрокаДерева Из СтрокаДереваВерхнегоУровня.Строки Цикл
		Если ЗначениеЗаполнено(СтрокаДерева.Штрихкод) Тогда
			...
			РезультатРазбора = ШтрихкодированиеОбщегоНазначенияИС.НоваяСтруктураОбработкиШтрихкодаУпрощенныйРазбор(
				СтрокаДерева.Штрихкод, ВидыПродукцииПоТоварнойГруппе, Настройки.НастройкиРазбораКодаМаркировки);
			...
			КодДляПередачиИСМП = РазборКодаМаркировкиИССлужебныйКлиентСервер.НормализоватьКодМаркировки(
				РезультатРазбора, ВидыПродукцииПоТоварнойГруппе, Настройки.ПараметрыНормализацииПрочее);
...
Функция ЗаполнитьСообщениеJSONАбстрактнаяОперация(Операция, ОбщийЭлементОчереди)
	...
	АбстрактноеСообщениеJSON.ТоварнаяГруппа            = ОбщийЭлементОчереди.ТоварнаяГруппа;
	АбстрактноеСообщениеJSON.Операция                  = Операция;
	АбстрактноеСообщениеJSON.Назначение                = Перечисления.НазначениеСообщенийИСМП.ИСМП;
	АбстрактноеСообщениеJSON.СтанцияУправленияЗаказами = Неопределено;
...
//  ЗагружатьДо - Строка - Идентификатор сообщения.
//  ПараметрыОбмена - См. ОбменДаннымиИСМП.ПараметрыОбмена.
//  УпаковкиДокумента - Соответствие из КлючИЗначение - соответствие упаковок обрабатываемого документа:
		...
		Если ЗначениеЗаполнено(СтрокаМаркированногоТовара.РодительскаяУпаковка) Тогда
			...
			Если УпаковкиДокумента[КодУпаковки] = Неопределено Тогда
				...
				РезультатРазбора = ШтрихкодированиеОбщегоНазначенияИС.НоваяСтруктураОбработкиШтрихкода(
					СтрокаМаркированногоТовара.РодительскаяУпаковка, ВидыПродукцииПоТоварнойГруппе, ПользовательскиеПараметрыРазбораКодаМаркировки);
				...
				НормализованныйКодУпаковки = РазборКодаМаркировкиИССлужебныйКлиентСервер.НормализоватьКодМаркировки(
					РезультатРазбора, ВидыПродукцииПоТоварнойГруппе, ПараметрыНормализацииПрочее);
...
// Обрабатывает иерархию родительских упаковок и заполняет соответствие упаковок к разагрегации.
// Параметры:
...
Функция ОбработатьУпаковкиВерхнегоУровня(Знач РодительскаяУпаковка, УпаковкиДокумента, 
	...
	Пока ЗначениеЗаполнено(РодительскаяУпаковка) Цикл
		РезультатРазбора = ШтрихкодированиеОбщегоНазначенияИС.НоваяСтруктураОбработкиШтрихкода(
			РодительскаяУпаковка, ВидыПродукцииПоТоварнойГруппе, ПользовательскиеПараметрыРазбораКодаМаркировки);
...
Функция ПодготовитьКПередачеСообщенияДляАгрегацииКодовМаркировкиИСМППоДеревуУпаковок(
	...
	ПараметрыАгрегации.Вставить("КешТоварныеГруппыПоВидуПродукции",      Новый Соответствие);
...
Функция ПодготовитьКПередачеСообщениеАгрегацииКодовМаркировкиИСМП(СтрокаДереваВерхнегоУровня, ОбщийЭлементОчереди, Знач ЗагружатьДо, СообщенияJSON,
	...
	КодУпаковки = РазборКодаМаркировкиИССлужебныйКлиентСервер.НормализоватьКодМаркировки(
		СтрокаДереваВерхнегоУровня, ВидыПродукцииПоТоварнойГруппе, ПараметрыНормализацииПрочее);
	...
	Если Не СтрокаДереваВерхнегоУровня.ВидУпаковки = Перечисления.ВидыУпаковокИС.Набор
		Или КонструкторИСМПКлиентСервер.АгрегироватьНаборыКакЛогистическиеУпаковки(ВидыПродукцииПоТоварнойГруппе) Тогда
		...
		Если СтрокаДереваВерхнегоУровня.ВидУпаковки = Перечисления.ВидыУпаковокИС.Набор Тогда
			ЭлементАгрегации.Вставить("unitSerialNumberType", "PRODUCT_SET");
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого СтрокаДерева Из СтрокаДереваВерхнегоУровня.Строки Цикл
		ЭлементАгрегации.sntins.Добавить(
			РазборКодаМаркировкиИССлужебныйКлиентСервер.НормализоватьКодМаркировки(
				СтрокаДерева, ВидыПродукцииПоТоварнойГруппе, ПараметрыНормализацииПрочее));
	КонецЦикла;
	...
	Если СтрокаДереваВерхнегоУровня.ВидУпаковки = Перечисления.ВидыУпаковокИС.Набор
		И Не КонструкторИСМПКлиентСервер.АгрегироватьНаборыКакЛогистическиеУпаковки(ВидыПродукцииПоТоварнойГруппе) Тогда
...
Функция ПодготовитьКПередачеСообщенияДляАгрегацииКодовМаркировкиИСМП(
	...
	ИначеЕсли ОбщийЭлементОчереди.РеквизитыИсходящегоСообщения.ПараметрыЗапроса.ОрганизацияИНН = СтрокаДереваВерхнегоУровня.ИННВладельца
		Или ПустаяСтрока(СтрокаДереваВерхнегоУровня.ИННВладельца) Тогда
		
		Если ЗначениеЗаполнено(СтрокаДереваВерхнегоУровня.ВидПродукции) Тогда
			
			ТоварнаяГруппа = ПараметрыАгрегации.КешТоварныеГруппыПоВидуПродукции[СтрокаДереваВерхнегоУровня.ВидПродукции];
			Если ТоварнаяГруппа = Неопределено Тогда
				ТоварнаяГруппа =  КонструкторИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппаПоВидуПродукции(СтрокаДереваВерхнегоУровня.ВидПродукции);
				ПараметрыАгрегации.КешТоварныеГруппыПоВидуПродукции.Вставить(СтрокаДереваВерхнегоУровня.ВидПродукции, ТоварнаяГруппа);
			КонецЕсли;
			Если ТоварнаяГруппа <> ОбщийЭлементОчереди.ТоварнаяГруппа Тогда
		...
		Если Не СтрокаДереваОбработана Тогда
			...
			Если РезультатЗапросаПоСтроке = Неопределено Тогда
				...
				СтрокаКодаМаркировки = ШтрихкодированиеОбщегоНазначенияИС.НоваяСтруктураОбработкиШтрихкода(КодУпаковки, ВидыПродукцииПоТоварнойГруппе);
			...
			Для Каждого КлючИЗначение Из ВложенныеШтрихкодыУпаковокИзРезультатаЗапросаДанных Цикл
				...
				Если ТребуетсяУдаление Тогда
					...
					КодМаркировкиДляПередачи = РазборКодаМаркировкиИССлужебныйКлиентСервер.НормализоватьКодМаркировки(
						СтрокаДанных, ВидыПродукцииПоТоварнойГруппе, ПараметрыНормализацииПрочее);
	...
	Если Операция = Перечисления.ВидыОперацийИСМП.АгрегацияИзменение Тогда
		...
		Если ТелоЗапросаДобавление <> Неопределено И ТелоЗапросаДобавление.uit_uitu_list.Количество() > 0 Тогда
			...
			СообщениеJSON.ТоварнаяГруппа = ОбщийЭлементОчереди.ТоварнаяГруппа;
			СообщениеJSON.Операция       = Операция;
		...
		Если ТелоЗапросаУдаление <> Неопределено И ТелоЗапросаУдаление.uit_uitu_list.Количество() > 0 Тогда
			...
			СообщениеJSONУдаление.ТоварнаяГруппа = ОбщийЭлементОчереди.ТоварнаяГруппа;
...
Функция ПодготовитьКПередачеСообщенияДляАгрегацииКодовМаркировкиМОТП(ПараметрыОбработки, СтрокаДереваВерхнегоУровня) Экспорт
	...
	Если СтрокаДереваВерхнегоУровня.Статус = Перечисления.СтатусыКодовМаркировкиИСМП.Неопределен
...
Функция ВыполнитьКонтрольСоставаАгрегатаПоДаннымИСМП(СтрокаДереваВерхнегоУровня, ОбщийЭлементОчереди, ПараметрыОбмена, ПараметрыНормализации)
	...
	Если СтрокаДереваВерхнегоУровня.Статус <> Перечисления.СтатусыКодовМаркировкиИСМП.Неопределен Тогда
		...
		Если ОбщийЭлементОчереди.РеквизитыИсходящегоСообщения.ПараметрыЗапроса.ОрганизацияИНН <> СтрокаДереваВерхнегоУровня.ИННВладельца
			И ЗначениеЗаполнено(СтрокаДереваВерхнегоУровня.ИННВладельца) Тогда
...
Функция ВыявитьРасхожденияВСоставахДеревьев(ДеревоУпаковок, ДеревоУпаковокПоДаннымСервиса, ПолнаяПроверкаДеревьев = Истина, ПараметрыСканирования = Неопределено)
	...
	Если ПолнаяПроверкаДеревьев Тогда
		РезультатПроверки = ВыявитьРасхожденияВСтрокахДеревьев(ДеревоУпаковок, ДеревоУпаковокПоДаннымСервиса, КэшПроверенныхСтрок,
			КэшНенайденныхСтрок, Истина,, ПараметрыСканирования);
	...
	РезультатПроверки = ВыявитьРасхожденияВСтрокахДеревьев(ДеревоУпаковокПоДаннымСервиса, ДеревоУпаковок, КэшПроверенныхСтрок,
		КэшНенайденныхСтрок, Ложь,, ПараметрыСканирования);
...
Функция ВыявитьРасхожденияВСтрокахДеревьев(ДеревоЗначений1, ДеревоЗначений2, КэшПроверенныхСтрок,
	КэшНенайденныхСтрок, ПерваяИтерация, ДеревоПроверкиИсходное = Неопределено, ПараметрыСканирования = Неопределено)
	...
	Для Каждого СтрокаДерева1 Из ДеревоЗначений1.Строки Цикл
		...
		Если СоответствующаяСтрокаДерева2 <> Неопределено Тогда
			ПараметрыПроверкиВложенныхСтрок = ВыявитьРасхожденияВСтрокахДеревьев(СтрокаДерева1, СоответствующаяСтрокаДерева2,
				КэшПроверенныхСтрок, КэшНенайденныхСтрок, ПерваяИтерация, ДеревоПроверкиИсходное, ПараметрыСканирования);
	...
	Если Не ПерваяИтерация Тогда
		Если ТипЗнч(ДеревоЗначений1) = Тип("СтрокаДереваЗначений") Тогда
			...
			Иначе
				...
				ДополнитьДанныеJSONДляОперацийЕАЭС(ДеревоЗначений1, СтрокаJSON, КешПоНоменклатуре, ПараметрыСканирования);
			КонецЕсли;
...
Процедура ДополнитьДанныеJSONДляОперацийЕАЭС(СтрокаДанных, СтрокаJSON, КешПоНоменклатуре, ПараметрыСканирования)
	...
	Если ТипЗнч(СтрокаДанных) <> Тип("СтрокаДереваЗначений")
		...
		Если СтрокаJSON.accepted Тогда
		...
		Если МаркировкаТоваровКлиентСервер.ДоступноПолеСуммаАкциза(СтрокаДанных.ВидПродукции, ПараметрыСканирования.ВидОперацииИСМП) Тогда
		...
		Если МаркировкаТоваровКлиентСервер.ДоступноПолеСодержаниеЭтиловогоСпирта(
			СтрокаДанных.ВидПродукции, ПараметрыСканирования.ВидОперацииИСМП) Тогда
			СтрокаJSON.Вставить("alco_content", СтрокаДанных.ДоляСпирта);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СтрокаДанных.НомерСерии) Тогда
...
Процедура ОбработкаЗагрузкиПереданногоДокумента(ЭлементОчереди, ПараметрыОбмена, РезультатОбработкиЗаявки, ПараметрыОбновленияСтатуса) Экспорт
		...
		Если РезультатОбработкиЗаявки.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаВыполнена Тогда
			...
			Для Каждого СтрокаТовары Из ДокументОбъект.Товары Цикл
				...
				СтруктураПоиска = ОбязательныеПоляОписанияОстатковПоСтрокеДокумента(
					СтрокаТовары, ДокументОбъект.ТоварнаяГруппа);
					...
					Если КешОписанияОстатков.Получить(СтрокаТовары.GTIN) = Неопределено Тогда
						...
						Иначе
							ПараметрыОписанияОстатков = ИнтеграцияИСМПКлиентСервер.ИспользованиеПараметровОписанияОстатковПоВидуПродукции(
								...
								ИнтеграцияИСМПКлиентСервер.КодТНВЭДДляПередачиВИСМП(СтрокаТовары.КодТНВЭД, ДокументОбъект.ТоварнаяГруппа));
							Если ПараметрыОписанияОстатков.ИспользоватьНаименование Тогда
								Если ЗначениеЗаполнено(СтрокаТовары.Наименование) Тогда
						...
						СтруктураОписания.Вставить("ТоварнаяГруппа", ДокументОбъект.ТоварнаяГруппа);
	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ОтчетОбИспользованииРасчетСтатуса
		...
		Если РезультатОбработкиЗаявки.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаВыполнена Тогда
			...
			УдалитьПолныеКодовМаркировкиИзПула = (ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ОтчетОбИспользованииРасчетСтатуса)
				Или Не МаркировкаТоваровКлиентСервер.ДоступенОтчетОНанесении(ЭлементОчереди.ТоварнаяГруппа)
					И Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ЭлементОчереди.ТоварнаяГруппа)
			...
			Если УдалитьПолныеКодовМаркировкиИзПула Тогда
				КодыМаркировки = ШтрихкодированиеИСМП.НоваяТаблицаПоискаКодаМаркировкиВПуле(ЭлементОчереди.ТоварнаяГруппа);
	...
	ИначеЕсли (ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.СписаниеПроверкаСтатусаОбработкиДокумента
			Или ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.КвитанцияДокумента
				...
				И СписаниеКодовМаркировкиКлиентСервер.ЭтоСписаниеКодовМаркировки(
					ЭлементОчереди.РеквизитыИсходящегоСообщения.ПараметрыЗапроса.ОперацияДокумента))
		И РезультатОбработкиЗаявки.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаВыполнена Тогда
	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ОтчетИСМПСведенияОбОтклонениях
		...
		Или ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ОтчетИСМПОстаткиВиртуальногоСклада
...
Процедура ЗагрузитьРезультатОбработкиЗапросаЗаказНаЭмиссиюСУЗАбстрактнаяОперация(ЭлементОчереди, ПараметрыОбмена)
	...
	Иначе
		...
		ДанныеСтатусаПоПулу = Документы.ЗаказНаЭмиссиюКодовМаркировкиСУЗ.ДанныеРасчетаСтатусаЗаказаПоПулу(
			ЭлементОчереди.Документ, ЭлементОчереди.ТоварнаяГруппа);
...
Процедура ЗагрузитьРезультатОбработкиЗаказаНаЭмиссиюКодовМаркировкиСУЗ(ЭлементОчереди, ПараметрыОбмена, НастройкаОбменаСУЗ)
	...
	Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ЭлементОчереди.ТоварнаяГруппа) Тогда
		...
		Попытка
			...
			Иначе
				...
				РезультатЗапросаСтатуса = ИнтерфейсСУЗ.СтатусЗаказаПоИдентификатору(
					ЭлементОчереди.ТоварнаяГруппа, НастройкаОбменаСУЗ, ЭлементОчереди.ИдентификаторЗаявки);
...
Процедура ЗагрузитьСодержаниеРезультатаОбработкиДокумента(ЭлементОчереди, ПараметрыОбмена)
	...
	ИмяПоляСортировки = КонструкторИСМПКлиентСервер.ИмяПоляСортировки(
		ЭлементОчереди.ТоварнаяГруппа,
		Перечисления.ВидыОперацийИСМП.ЗаказНаЭмиссиюКодовМаркировкиЗапросGTINНаОстатки);
	Если ЗначениеЗаполнено(ИмяПоляСортировки) Тогда
		ПараметрыОбработки.ИмяПоляСортировки = ИмяПоляСортировки;
	КонецЕсли;
...
Процедура ЗаполнитьПоОписаниюОстатков(ПараметрыЗаполнения, ДанныеОписанияОстатков, СохранятьОписание = Ложь)
	...
	ТоварнаяГруппа      = ПараметрыЗаполнения.ТоварнаяГруппа;
		...
		Для Каждого КлючИЗначение Из ДанныеОписанияОстатков Цикл
			...
			Если НормализованноеСтроковоеЗначение(НаименованиеПолное) = НормализованноеСтроковоеЗначение(ИскомоеНаименование) Тогда
				...
				Для Каждого СтрокаТовары Из СтрокиТоваров Цикл
					...
					Если ЗначениеЗаполнено(ОписаниеПозиции.ТНВЭД) Тогда
						ТНВЭД = ОписаниеПозиции.ТНВЭД;
					ИначеЕсли ЗначениеЗаполнено(ОписаниеПозиции.ГруппаТНВЭД) Тогда
						ТНВЭД = ОписаниеПозиции.ГруппаТНВЭД;
					Иначе
						Продолжить;
					КонецЕсли;
					
					Если Не СтрНачинаетсяС(СтрокаТовары.КодТНВЭД, ТНВЭД) Тогда
						Продолжить;
					КонецЕсли;
					...
					Если СохранятьОписание И КешОписанияОстатков.Получить(GTIN) = Неопределено Тогда
						...
						СтруктураОписания.Вставить("КодТНВЭД",       ТНВЭД);
						СтруктураОписания.Вставить("Организация",    Организация);
						СтруктураОписания.Вставить("ТоварнаяГруппа", ТоварнаяГруппа);
						СтруктураОписания.Вставить("ДанныеОписания", СтрокаТовары);
...
//
Функция СтрокаДляПоискаGTINМаркировкиОстатков(ТоварнаяГруппа, СтрокаТовары)
	
	ЧастиНазвания = Новый Массив;
	
	ПараметрыОписанияОстатков = ИнтеграцияИСМПКлиентСервер.ИспользованиеПараметровОписанияОстатковПоВидуПродукции(
		ТоварнаяГруппа, СтрокаТовары.КодТНВЭД);
	...
	Если ПараметрыОписанияОстатков.ИспользоватьТоварныйЗнак Тогда
		Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(ТоварнаяГруппа) Тогда
			ОписаниеТоварногоЗнака = ОписаниеТоварногоЗнака(СтрокаТовары.ТоварныйЗнак, "Отсутствует");
		Иначе
			ОписаниеТоварногоЗнака = ОписаниеТоварногоЗнака(СтрокаТовары.ТоварныйЗнак);
		КонецЕсли;
		ЧастиНазвания.Добавить(ОписаниеТоварногоЗнака);
	КонецЕсли;
	
	Если ПараметрыОписанияОстатков.ИспользоватьВозрастнуюКатегорию
		И ЗначениеЗаполнено(СтрокаТовары.ВозрастнаяКатегория) Тогда
		ЧастиНазвания.Добавить(СокрЛП(СтрокаТовары.ВозрастнаяКатегория));
	КонецЕсли;
	
	Если ПараметрыОписанияОстатков.ИспользоватьЦелевойПол
		И ЗначениеЗаполнено(СтрокаТовары.ЦелевойПол) Тогда
	...
	Если ПараметрыОписанияОстатков.ИспользоватьМодель
		И ЗначениеЗаполнено(СтрокаТовары.Модель) Тогда
...
Функция СообщениеМожноПередавать(ЭлементОчереди, ПараметрыОбмена, НайденныеЗависимости, ДатаСледующейИтерации) Экспорт
	...
	Если ЭлементОчереди.Назначение = Перечисления.НазначениеСообщенийИСМП.СтанцияУправленияЗаказами
		...
		Если ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ЗаказНаЭмиссиюКодовМаркировки
			И Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ЭлементОчереди.ТоварнаяГруппа) Тогда
	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ОтчетИСМПСписокКИНаБалансе Тогда
		...
		ПараметрыОтбора = Новый Структура;
		ПараметрыОтбора.Вставить("ТекущийСтатус",  Перечисления.СтатусыОбработкиОтчетаИСМП.Обрабатывается);
		ПараметрыОтбора.Вставить("Операция", ЭлементОчереди.Операция);
		
		СообщенияВОбработке = ПараметрыОбмена.ОчередьСообщений.НайтиСтроки(ПараметрыОтбора);
		
		Если СообщенияВОбработке.Количество() >= МаксимальноеКоличествоОдновременныхВыгрузок() Тогда
			ДатаСледующейИтерации = ТекущаяДатаСеанса() + ОбменДаннымиИСМП.ВремяОжидания(1);
			Возврат Ложь;
		КонецЕсли;
...
Процедура ЗагрузитьРезультатОбработкиОтчетаСУЗ(ЭлементОчереди, ПараметрыОбмена, НастройкаОбменаСУЗ)
	...
	РезультатОбработкиЗаявки = ИнтерфейсСУЗ.СтатусОбработкиОтчета_V2(
		ЭлементОчереди.ИдентификаторЗаявки, ЭлементОчереди.ТоварнаяГруппа, НастройкаОбменаСУЗ, ЭлементОчереди.Операция);
	...
	Попытка
			...
			Если РезультатОбработкиЗаявки.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаВыполнена Тогда
				...
				Если ПараметрыЗапроса <> Неопределено Тогда
					...
					Если ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ОтчетОВерификацииНанесенныхКМ
						Или СписаниеКодовМаркировкиКлиентСервер.ЭтоСписаниеКодовМаркировки(ЭлементОчереди.Операция) Тогда
		...
		Если СписаниеКодовМаркировкиКлиентСервер.ЭтоСписаниеКодовМаркировки(ЭлементОчереди.Операция)
...
Функция ЗапроситьКодыМаркировкиИзБизнесЗаказа(ПараметрыЗапроса, ДанныеПула, ЭлементОчереди, ПараметрыОбмена, НастройкаОбменаСУЗ)
	...
	Если РезультатЗапросаКодовМаркировки.ДанныеБлока <> Неопределено Тогда
		...
		ПараметрыСтрокЗаказа.Вставить("ТоварнаяГруппа",    ЭлементОчереди.ТоварнаяГруппа);
		...
		Для Каждого ЗначениеШтрихкода Из РезультатЗапросаКодовМаркировки.ДанныеБлока.КодыМаркировки Цикл
			...
			ЗаписьНабора.ТоварнаяГруппа            = СтрокаЗаказаНаЭмиссию.ТоварнаяГруппа;

			СпособВводаВОборот = Неопределено;
			Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(СтрокаЗаказаНаЭмиссию.ТоварнаяГруппа) Тогда
			...
			Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(СтрокаЗаказаНаЭмиссию.ТоварнаяГруппа) Тогда
				ЗаписьНабора.МаксимальнаяРозничнаяЦена = СтрокаЗаказаНаЭмиссию.МаксимальнаяРозничнаяЦена;
			ИначеЕсли КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСОбязательнымУказаниемСроковГодности(СтрокаЗаказаНаЭмиссию.ТоварнаяГруппа) Тогда
...
Функция ДанныеПоТоварамЗаказаНаЭмиссию(ЭлементОчереди, ДанныеПула, ПараметрыЗапроса)
	...
	Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ЭлементОчереди.ТоварнаяГруппа) Тогда
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТабличнаяЧасть.ТоварнаяГруппа                         КАК ТоварнаяГруппа,
		|	ТабличнаяЧасть.СпособВводаВОборот                     КАК СпособВводаВОборот,
		|	ТабличнаяЧасть.ВидКИЗ                                 КАК ВидКИЗ,
		|	ТабличнаяЧасть.РазмерКИЗ                              КАК РазмерКИЗ,
		|	ТабличнаяЧасть.Номенклатура                           КАК Номенклатура,
	...
	Иначе
		...
		|			И ТабличнаяЧасть.СпособВводаВОборот <> ЗНАЧЕНИЕ(Перечисление.СпособыВводаВОборотСУЗ.ПустаяСсылка)
		...
		|				ИЛИ Шапка.ТоварнаяГруппа В (&ТоварныеГруппыМОТП)
		...
		ТоварныеГруппыМОТП = Новый Массив;
		ВидыПродукцииМОТП = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииМОТП();
		Для Каждого ВидПродукцииМОТП Из ВидыПродукцииМОТП Цикл
			ТоварныеГруппыМОТП.Добавить(КонструкторИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппаПоВидуПродукции(ВидПродукцииМОТП));
		КонецЦикла;
		Запрос.УстановитьПараметр("ТоварныеГруппыМОТП", ТоварныеГруппыМОТП);
...
Процедура ВыбратьСтрокуДляРаспределенияКодовМаркировки(ПараметрыСтрокЗаказа, ПараметрыКода)
	...
	ЭтоМех = КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ПараметрыСтрокЗаказа.ТоварнаяГруппа);
...
// * Шаблон - СправочникСсылка.ШаблоныКодовМаркировкиСУЗ -
...
Функция НоваяТаблицаПараметровЗаказаКодовМаркировки() Экспорт
	...
	ПараметрыЗаказа.Колонки.Добавить("Шаблон",             Новый ОписаниеТипов("СправочникСсылка.ШаблоныКодовМаркировкиСУЗ"));
...
Функция ЗапроситьСтатусПулаКодовМаркировкиПоСтрокеЗаказаИПолучитьКоды(ЭлементОчереди, ПараметрыОбмена, НастройкаОбменаСУЗ)
	...
	ПараметрыЗапроса = ЭлементОчереди.РеквизитыИсходящегоСообщения.ПараметрыЗапроса;
	ТребуетсяAPIМех   = КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ЭлементОчереди.ТоварнаяГруппа);
	...
	ИначеЕсли НастройкаОбменаСУЗ.ФорматОбмена = Перечисления.ВерсииФорматаОбменаСУЗ.V2
		...
		Если ТребуетсяAPIМех Тогда
			Результат = ИнтерфейсСУЗ.СтатусЗаказаПоИдентификатору(
				ЭлементОчереди.ТоварнаяГруппа, НастройкаОбменаСУЗ, ПараметрыЗапроса.ИдентификаторБизнесЗаказа)
		Иначе
			Результат = ИнтерфейсСУЗ.СтатусПулаКодовМаркировкиИзБизнесЗаказа_V2(
				ПараметрыЗапроса, ЭлементОчереди.ТоварнаяГруппа,
				НастройкаОбменаСУЗ);
		КонецЕсли;
	Иначе
		ВызватьИсключение СтрШаблон(
...
Функция ДанныеПовторногоПолученияКодов(ЭлементОчереди, ПараметрыОбмена, НастройкаОбменаСУЗ)
	...
	РезультатЗапроса = ИнтерфейсСУЗ.ЗапроситьСписокИдентификаторовПакетовБизнесЗаказа_V2(
		ПараметрыЗапросаКодов, ЭлементОчереди.ТоварнаяГруппа, НастройкаОбменаСУЗ);
...
Функция ЗагрузитьДокументы(ЭлементОчереди, ПараметрыОбмена, ПараметрыЗагрузки)
	...
	Для Каждого ИдентификаторДокумента Из ДокументыДляЗагрузки Цикл
		...
		ЭлементОчередиДляЗагрузкиДокумента.Вставить("ТоварнаяГруппа", ЭлементОчереди.ТоварнаяГруппа);
		
		ПараметрыОтбора = Новый Структура;
		ПараметрыОтбора.Вставить("Идентификатор",     ИдентификаторДокумента.Идентификатор);
		
		ИмяПоляСортировки = КонструкторИСМПКлиентСервер.ИмяПоляСортировки(ЭлементОчереди.ТоварнаяГруппа, Перечисления.ВидыОперацийИСМП.Приемка);
		Если ЗначениеЗаполнено(ИмяПоляСортировки) Тогда
			ПараметрыОбработки = ИнтерфейсИСМП.НовыеПараметрыОбработкиСоставаДокумента();
			ПараметрыОбработки.ИмяПоляСортировки = ИмяПоляСортировки;
		КонецЕсли;
		...
		Если СоставДокумента.Количество() > 0 Тогда
			...
			ПроверитьТоварнуюГруппуДокументаПоДаннымСервиса(
				СоставДокумента, 
				ЭлементОчереди.Организация,
				ЭлементОчереди.ТоварнаяГруппа,
			...
			Для Каждого ДанныеДокумента Из СтруктураОбработкиДокумента.ДанныеДокумента Цикл
				...
				ДокументОбъект.ТоварнаяГруппа = ЭлементОчереди.ТоварнаяГруппа;
				...
				ДоступноПолеGTIN = МаркировкаТоваровКлиентСервер.ДоступноПолеGTIN(ДокументОбъект.ТоварнаяГруппа,
					ДокументОбъект.Операция);
				...
				Для Каждого СтрокаКодаМаркировки Из СоставДокумента Цикл
					...
					ИначеЕсли ЭтоОбъемноСортовойУчет Тогда
						Если ИнтеграцияИСМПВЕТИСКлиентСервер.ЭтоВидПродукцииИСМПВЕТИС(ДокументОбъект.ТоварнаяГруппа) Тогда
				...
				Иначе
					...
					РезультатЗапросаВложенныхШтрихкодов = Неопределено;
					ДополнитьТабличныеЧастиПоДаннымИСМП(ДокументОбъект, ДанныеШтрихкодов, РезультатЗапросаВложенныхШтрихкодов);

					Если РезультатЗапросаВложенныхШтрихкодов <> Неопределено Тогда
						СтрокаРезультата = ОбменДаннымиИСМП.СтруктураИзменения();
						...
						СтрокаРезультата.ТекстОшибки           = РезультатЗапросаВложенныхШтрихкодов.ТекстОшибки;
						СтрокаРезультата.ПодготовленоКПередаче = Ложь;
						СтрокаРезультата.Передано              = Ложь;
						СтрокаРезультата.Принято               = Истина;
						СтрокаРезультата.НовыйСтатус           = Неопределено;
			...
			ДополнительныеПараметры.Вставить("ВидПродукции",    ЭлементОчереди.ТоварнаяГруппа);
...
Функция ВходящиеДокументы(ЭлементОчереди, ПараметрыЗагрузки)
	...
	Пока ПараметрыНавигации = Неопределено 
		...
		РезультатЗапроса = ИнтерфейсИСМП.НайтиДокументыПоОтбору(
			ЭлементОчереди.Организация, ЭлементОчереди.ТоварнаяГруппа,
...
Процедура ПроверитьТоварнуюГруппуДокументаПоДаннымСервиса(СоставДокумента, Организация, ТоварнаяГруппа, РезультатЗапроса)
	...
	Если РезультатПолученияДанных.ТоварнаяГруппа <> ТоварнаяГруппа Тогда
		РезультатЗапроса = РезультатПолученияДанных;
		РезультатЗапроса.ТекстОшибки = СтрШаблон(
			...
			ТоварнаяГруппа,
			РезультатЗапроса.ТоварнаяГруппа);
...
Процедура ДополнитьТабличныеЧастиПоДаннымИСМП(ДокументПриемка, ДанныеШтрихкодов, РезультатЗапроса)
	...
	ВидыПродукцииПоТоварнойГруппе = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ДокументПриемка.ТоварнаяГруппа);
	ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(ДокументПриемка, Неопределено, ВидыПродукцииПоТоварнойГруппе);
	...
	ЭтоПриемкаПродукцииИзМеха        = КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ВидыПродукцииПоТоварнойГруппе);
	...
	ИначеЕсли ЭтоПриемкаИзЕАЭССПризнаниемКМ Или ЭтоПриемкаТрансграничнаяТорговля Тогда
		...
		Для Каждого СтрокаДанных Из ДанныеШтрихкодов Цикл
			...
			ДанныеРазбора = ГрупповаяОбработкаШтрихкодовИС.ВидУпаковкиИПредставлениеШтрихкодаУпрощенныйРазбор(
				СтрокаДанных.Штрихкод, ВидыПродукцииПоТоварнойГруппе, КешДанныхРазбора, НастройкиРазбораКодаМаркировки);
...
Процедура ОбработкаДокументаОбъемноСортовогоУчета(ДокументПриемка, ДанныеШтрихкодов)
	...
	ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(
		ДокументПриемка, Неопределено, КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукции(ДокументПриемка.ТоварнаяГруппа));
	...
	Для Каждого СтрокаДанных Из ДанныеШтрихкодов Цикл
		...
		Если ЗначениеЗаполнено(СтрокаДанных.ВСД) Тогда
...
//Заполняет соответствие штрихкодов упаковок количеством вложенных единиц продукции
//
// Параметры:
...
Процедура ПередатьЗаказНаЭмиссиюКодовМаркировкиСУЗ(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей, НастройкаОбменаСУЗ, НаборЗаписейОснование)
	...
	ИначеЕсли НастройкаОбменаСУЗ.ФорматОбмена = Перечисления.ВерсииФорматаОбменаСУЗ.V2
		...
		ДанныеОбработки = ИнтерфейсСУЗ.СоздатьБизнесЗаказНаЭмиссиюКодовМаркировки_V2(
			РеквизитыИсходящегоСообщения.ТекстСообщенияJSON, ЭлементОчереди.ТоварнаяГруппа,
	...
	Если ДанныеОбработки.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаПринята
		...
		Иначе
			...
			Для Каждого ПараметрыЗапроса Из ЭлементОчереди.РеквизитыИсходящегоСообщения.ПараметрыЗапроса Цикл
				...
				Если Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ЭлементОчереди.ТоварнаяГруппа) Тогда
					НовыйЭлементОчереди = ПодготовитьОчередьЗагрузкиКодовМаркировкиПоЗаказуНаЭмиссию(
						ПараметрыЗапроса,
						ЭлементОчереди, ЗагружатьДо, ПараметрыОбмена, НастройкаОбменаСУЗ);
				КонецЕсли;
...
Процедура ПередатьОтчетСУЗ(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей, НастройкаОбменаСУЗ, НаборЗаписейОснование)
	...
	ИначеЕсли НастройкаОбменаСУЗ.ФорматОбмена = Перечисления.ВерсииФорматаОбменаСУЗ.V2
		...
		ИначеЕсли СписаниеКодовМаркировкиКлиентСервер.ЭтоСписаниеКодовМаркировки(ЭлементОчереди.Операция) Тогда
			
			ДанныеОбработки = ИнтерфейсСУЗ.ПередатьОтчетОСписанииКодовМаркировки_V2(
				РеквизитыИсходящегоСообщения.ТекстСообщенияJSON, ЭлементОчереди.ТоварнаяГруппа, ЭлементОчереди.Операция,
				НастройкаОбменаСУЗ, Подпись);
		...
		ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ЗакрытиеЗаказаНаЭмиссиюСУЗ Тогда
			...
			ДанныеОбработки = ИнтерфейсСУЗ.ЗакрытьЗаказНаЭмиссию_V3(
				...
				ЭлементОчереди.ТоварнаяГруппа, НастройкаОбменаСУЗ, РеквизитыИсходящегоСообщения.ТекстСообщенияJSON);
	...
	Если ЗначениеЗаполнено(ТекстОшибки) И ОбработкаСтрокиXML.НайтиНедопустимыеСимволыXML(ДанныеОбработки.ТекстОшибки) Тогда
...
Процедура ЗагрузитьРезультатОбработкиОтчетОбИспользованииКМРасчетСтатусаКодовМаркировки(ЭлементОчереди, ПараметрыОбмена)
	...
	ПараметрыЗапроса = ЭлементОчереди.РеквизитыИсходящегоСообщения.ПараметрыЗапроса;
	ВидыПродукцииПоТоварнойГруппе = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ЭлементОчереди.ТоварнаяГруппа);
	...
	Для Каждого ЗначениеШтрихкода Из ПараметрыЗапроса.КодыМаркировкиДляКонтроляСтатусов Цикл
		СтруктураЗначения = ШтрихкодированиеОбщегоНазначенияИС.НоваяСтруктураОбработкиШтрихкода(
			ЗначениеШтрихкода, ВидыПродукцииПоТоварнойГруппе);
	...
	Если Не ЗначениеЗаполнено(РезультатПолученияДанных.ТекстОшибки)
		...
		Для Каждого КлючИЗначение Из РезультатПолученияДанных.СтатусыКодовМаркировки Цикл
			...
			ИначеЕсли ЭтоПовторныйЗапросКвитанции
				И СтатусКодов = Перечисления.СтатусыКодовМаркировкиИСМП.Неопределен Тогда
				СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.СтатусНеопределен;
			Иначе
...
Процедура ЗагрузитьРезультатПроверкаСтатусаОбработкиДокумента(ЭлементОчереди, ПараметрыОбмена)
	...
	РезультатОбработкиЗаявки = ИнтерфейсИСМПОбщегоНазначения.СтатусОбработкиДокументаПоИдентификатору(
		ЭлементОчереди.ИдентификаторЗаявки,
		ЭлементОчереди.Организация, ЭлементОчереди.ТоварнаяГруппа, ЭлементОчереди.Операция);
...
Процедура ПодготовитьСообщенияПоАбстрактнойОперацииВводВОборотИмпортСФТС(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	ВидыПродукцииПоТоварнойГруппе = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукции(ЭлементОчереди.ТоварнаяГруппа);
	
	ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(
		ДанныеШапки, Неопределено, ВидыПродукцииПоТоварнойГруппе);
	Настройки = Новый Структура(
	...
	Если ДанныеШапки.ОтчетПроизводственнойЛинии Тогда
		...
		РезультатОбработкиДанныхОтчета = РазборКодаМаркировкиИСОтчетПроизводственнойЛинии.НормализоватьДанныеОтчетаПроизводственнойЛинии(
			ДанныеОтчета, ВидыПродукцииПоТоварнойГруппе, ПараметрыРазбора);
	...
	Если Не ЕстьОшибки Тогда
		...
		ВидыПродукцииПоТоварнойГруппе = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ДанныеШапки.ТоварнаяГруппа);
		...
		Если МаркировкаТоваровКлиентСервер.ВОтчетеОНанесенииПередаватьКПП(ВидыПродукцииПоТоварнойГруппе) Тогда
		...
		Если МаркировкаТоваровКлиентСервер.ДляТоварнойГруппыИспользуетсяДатаПроизводства(
			ВидыПродукцииПоТоварнойГруппе, ДанныеШапки.Операция)
			И ЗначениеЗаполнено(ДанныеШапки.ДатаПроизводства) Тогда
		...
		Если ИнтеграцияИСМПВЕТИСКлиентСервер.ЭтоВидПродукцииИСМПВЕТИС(ЭлементОчереди.ТоварнаяГруппа) Тогда
			Настройки.УказыватьИдентификаторПроисхожденияВЕТИС = Истина;
			Настройки.ЗаполнятьВложенныеКоды = Истина;
		КонецЕсли;
		
		ИменаРеквизитовКодМаркировки = КонструкторИСМПКлиентСервер.ИмяРеквизитаКодМаркировки(ДанныеШапки.ТоварнаяГруппа, ДанныеШапки.Операция);
		Если ЗначениеЗаполнено(ИменаРеквизитовКодМаркировки) Тогда
			Настройки.Вставить("ИмяРеквизитаКодМаркировкиДополнительное", ИменаРеквизитовКодМаркировки);
		КонецЕсли;
...
Процедура ПодготовитьСообщенияПоАбстрактнойАгрегации(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Иначе
		...
		РезультатОбработкиДанныхОтчета = РазборКодаМаркировкиИСОтчетПроизводственнойЛинии.НормализоватьДанныеОтчетаПроизводственнойЛинии(
			ДанныеОтчета,
			КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукции(ЭлементОчереди.ТоварнаяГруппа),
			ПараметрыРазбора);
		...
		// При подготовке дерева упаковок в сценарии работы с ТСД не следует выполнять обращения к ГИС МТ.
		// Все необходимые обращения выполнит механизм штрихкодирования.
		Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМОТП(ЭлементОчереди.ТоварнаяГруппа) Тогда
			ШтрихкодированиеИСМПСлужебный.НормализоватьДанныеВложенныхШтрихкодов(
				ДанныеПоШтрихкодам.ВложенныеШтрихкоды, ПараметрыСканирования);
	...
	Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМОТП(ЭлементОчереди.ТоварнаяГруппа) Тогда
		...
		ПараметрыПроверки.ПараметрыПроверкиИерархииАгрегатов       = ПроверкаИПодборПродукцииИСМПКлиентСервер.ДопустимыеВложенияАгрегатовПоВидамПродукции(ЭлементОчереди.ТоварнаяГруппа);
		...
		Если КонструкторИСМПКлиентСервер.КонтролироватьСоставГрупповойУпаковки(ЭлементОчереди.ТоварнаяГруппа) Тогда
	...
	Иначе
		...
		Если КонструкторИСМПКлиентСервер.ВидПродукцииПоддерживаетАгрегациюКодовМаркировкиЧерезСУЗ(ЭлементОчереди.ТоварнаяГруппа) Тогда
...
Процедура ПодготовитьСообщенияПоАбстрактнойОперацииОтгрузкаИСМП(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Если РезультатОбработкиЗаявки.Статус <> Перечисления.СтатусыДокументовИСМП.ОжидаетсяПодтверждениеПоступления 
		...
		ПараметрыОбновленияСтатуса.Вставить("ТоварнаяГруппа",             ЭлементОчереди.ТоварнаяГруппа);
...
Процедура ПодготовитьСообщенияПоАбстрактнойОперацииПриемкаИСМП(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	ДоступнаДатаПроизводства = МаркировкаТоваровКлиентСервер.ДляТоварнойГруппыИспользуетсяДатаПроизводства(ВидыПродукцииПоТоварнойГруппе, Шапка.Операция);
	ДоступенСрокГодности = КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСоСрокамиГодности(ВидыПродукцииПоТоварнойГруппе,, Шапка.Операция);
	ДоступноПолеСуммаАкциза = МаркировкаТоваровКлиентСервер.ДоступноПолеСуммаАкциза(ВидыПродукцииПоТоварнойГруппе, Шапка.Операция);
	ТребуетсяУказаниеДолиСпирта = МаркировкаТоваровКлиентСервер.ДоступноПолеСодержаниеЭтиловогоСпирта(ВидыПродукцииПоТоварнойГруппе, Шапка.Операция);
	...
	Если ОперацияПодтверждениеПриемки Тогда
		...
		Для Каждого СтрокаТаблицы Из Товары Цикл
			...
			Если ДоступноПолеСуммаАкциза Тогда
		...
		Иначе
			...
			РезультатПолученияДереваИСМП = ИнтерфейсМОТП.ДеревоУпаковок(
...
Функция РазобратьКодМаркировкиДляБизнесЗаказа(КодМаркировки, СтрокаЗаказаНаЭмиссию, КэшРазбораКодаМаркировки)
	...
	Если КэшРазбораКодаМаркировки = Неопределено Тогда
		...
		ВидыПродукции = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(СтрокаЗаказаНаЭмиссию.ТоварнаяГруппа);
		Настройки = РазборКодаМаркировкиИССлужебный.НастройкиРазбораКодаМаркировки(ВидыПродукции);
		...
		ПримечаниеКРезультатуРазбора = Неопределено;
		ДанныеРазбора = РазборКодаМаркировкиИССлужебный.РазобратьКодМаркировки(
			КодМаркировки, ВидыПродукции, ПримечаниеКРезультатуРазбора, Настройки, ПользовательскиеПараметрыРазбораКодаМаркировки);
		...
		Если Не ЗначениеЗаполнено(ДанныеРазбора.ВидУпаковки) Тогда
			...
			Если Не РазборКодаМаркировкиИССлужебныйКлиентСервер.ВидУпаковкиСоответствуетРазбору(ВидыПродукции, ВидУпаковкиШаблона, ДанныеРазбора) Тогда
		...
		Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСОбязательнымУказаниемСроковГодности(СтрокаЗаказаНаЭмиссию.ТоварнаяГруппа) Тогда
...
Функция ПредставлениеНастройкиВариантаУчетаПоВидуПродукции(ВидПродукции, ВариантУчета) Экспорт
	...
	Если ВариантУчета = Перечисления.ВариантыУчетаЧастичногоВыбытияИС.НастроеннаяНоменклатура Тогда
		Результат =  НСтр("ru = 'Применяется разупаковка с передачей в торговый зал'");
	Иначе
		ПредставлениеЧастичногоВыбытия = КонструкторИСМПОбщегоНазначенияКлиентСервер.ПредставлениеПродажаЧастичногоВыбытияПоВидуПродукции(ВидПродукции);
		Результат = СтрШаблон(
			НСтр("ru = 'Поддерживается %1'"),
...
Функция ДеревоУпаковокПоДаннымОтчетаПроизводственнойЛинии(ДанныеШапки, ИсточникДанных, КешДанныхРазбора, ПараметрыЗаполнения = Неопределено,
	...
	Иначе
		...
		КонецЦикла;
		ДанныеПоШтрихкодамEANРезультат = ШтрихкодированиеОбщегоНазначенияИС.ДанныеПоШтрихкодамEAN(ДанныеEAN, ПараметрыСканирования, ПараметрыЗаполнения.GTINПотребительскихУпаковок);
	...
	ВидПродукцииСтрокиДерева = Неопределено;
	
	Если ЗначениеЗаполнено(ДанныеШапки.Номенклатура) Тогда
		ОписаниеНоменклатуры = ОбщегоНазначенияИС.ОписаниеНоменклатуры(ДанныеШапки.Номенклатура)[ДанныеШапки.Номенклатура];
		Если ЗначениеЗаполнено(ОписаниеНоменклатуры.КлючВидовПродукции) Тогда
			ВидПродукцииСтрокиДерева = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(
				ОписаниеНоменклатуры.КлючВидовПродукции, "ГИСМТ");
		КонецЕсли;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ВидПродукцииСтрокиДерева) Тогда
		ВидыПродукцииПоТоварнойГруппе = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(
			ДанныеШапки.ТоварнаяГруппа);
		...
		Для Каждого ВидПродукции Из ВидыПродукцииПоТоварнойГруппе Цикл
			Если УчитываемыеВидыПродукции.Найти(ВидПродукции) <> Неопределено Тогда
				ВидПродукцииСтрокиДерева = ВидПродукции;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого СтрокаДанныхОтчета Из ИсточникДанных Цикл
		...
		СтрокаДерева.ВидПродукции   = ВидПродукцииСтрокиДерева;
...
Процедура ПодготовитьСообщенияПоАбстрактнойОперацииОтчетОбИспользовании(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Для Каждого КлючИЗначение Из ПараметрыЗапроса.КодыМаркировкиДляКонтроляСтатусов Цикл
		Для Каждого ЗначениеШтрихкода Из КлючИЗначение.Значение Цикл
			СтруктураЗначения = ШтрихкодированиеОбщегоНазначенияИС.НоваяСтруктураОбработкиШтрихкода(
				ЗначениеШтрихкода, ВидПродукцииПоТоварнойГруппе);
			МассивСтрок.Добавить(СтруктураЗначения);
...
Процедура ЗагрузитьРезультатПолученияКвитанцииДокументаСУЗ(ЭлементОчереди, ПараметрыОбмена, НастройкаОбменаСУЗ)
	...
	РезультатОбработкиЗаявки = ИнтерфейсСУЗ.СтатусКвитанцииДокумента_V2(
		ЭлементОчереди.ИдентификаторЗаявки, ЭлементОчереди.ТоварнаяГруппа, НастройкаОбменаСУЗ);
	...
	Если Не ТребуетсяПовторОперации
		...
		РезультатОбработкиОтчета = ИнтерфейсСУЗ.РезультатОбработкиОтчетаПоДаннымКвитанции(
			ЭлементОчереди.ИдентификаторЗаявки, ЭлементОчереди.ТоварнаяГруппа, РезультатОбработкиЗаявки.ИдентификаторСобытия, НастройкаОбменаСУЗ);
		...
		Если РезультатОбработкиЗаявки.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаВыполнена
			...
			Если РезультатОбработкиЗаявки.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаВыполнена
				Или (ЭлементОчереди.РеквизитыИсходящегоСообщения.ПараметрыЗапроса.Свойство("ОперацияДокумента")
					И СписаниеКодовМаркировкиКлиентСервер.ЭтоСписаниеКодовМаркировки(
						ЭлементОчереди.РеквизитыИсходящегоСообщения.ПараметрыЗапроса.ОперацияДокумента)) Тогда
...
Процедура ЗаполнитьКодыМаркировкиДляПоискаВПулеПоШтрихкодамУпаковок(ЭлементОчереди, КодыМаркировки, ТоварнаяГруппаВидПродукции)
	
	РезультатПоиска = ШтрихкодированиеИС.ВложенныеШтрихкодыУпаковокПоДокументу(
		ЭлементОчереди.Документ,
		КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ЭлементОчереди.ТоварнаяГруппа),,
		Истина);
	...
	Для Каждого СтрокаТЧМаркированныеТовары Из РезультатПоиска.ВложенныеШтрихкоды.МаркированныеТовары Цикл
		...
		Если СтрокаТЧМаркированныеТовары.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
			...
			ШтрихкодированиеИСМП.ДобавитьКодМаркировкиВТаблицуДляПоискаВПуле(
				СтрокаТЧМаркированныеТовары, КодыМаркировки, ТоварнаяГруппаВидПродукции);
		...
		Если СтрокаРодитель <> Неопределено Тогда
			Если СоответствиеГрупповыхУпаковок[СтрокаРодитель.Штрихкод] = Неопределено Тогда
				...
				Если СтрокаРодитель.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая
					...
					ШтрихкодированиеИСМП.ДобавитьКодМаркировкиВТаблицуДляПоискаВПуле(
						СтрокаРодитель, КодыМаркировки, ТоварнаяГруппаВидПродукции);
...
Процедура ЗаполнитьСведенияИмпортСФТСПоДаннымАТКОтчетаПроизводственнойЛинии(ОбщийЭлементОчереди, ГруппыКодов, ДанныеШапки, ДанныеАТК, Настройки)
	...
	Для Каждого СтрокаАТК Из ДанныеАТК Цикл
		...
		РезультатРазбора = ШтрихкодированиеОбщегоНазначенияИС.НоваяСтруктураОбработкиШтрихкодаУпрощенныйРазбор(
			СтрокаАТК.ШтрихкодУпаковки, ВидыПродукцииПоТоварнойГруппе, НастройкиРазбораКодаМаркировки);
		...
		Если Настройки.ЗаполнятьВложенныеКоды Тогда
			...
			Для Каждого КодМаркировки Из ВложенныеКоды Цикл
				...
				РезультатРазбора = ШтрихкодированиеОбщегоНазначенияИС.НоваяСтруктураОбработкиШтрихкодаУпрощенныйРазбор(
					КодМаркировки, ВидыПродукцииПоТоварнойГруппе, НастройкиРазбораКодаМаркировки);
...
Процедура УдалитьПолныеКодовМаркировкиИзПулаПослеУспешнойОбработкиСообщения(ЭлементОчереди, КодыМаркировки, ТоварнаяГруппаВидПродукции)
	
	Если КодыМаркировки.Количество() = 0 Тогда
		ЗаполнитьКодыМаркировкиДляПоискаВПулеПоШтрихкодамУпаковок(ЭлементОчереди, КодыМаркировки, ТоварнаяГруппаВидПродукции);
	КонецЕсли;
...
//    КешСтатусовКодовМаркировки - Соответствие - соответствие упаковок обрабатываемого документа:
//      Ключ - Строка - нормализованный код упаковки.
	...
	Для Каждого СтрокаТЧМаркированныеТовары Из МаркированныеТовары Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТЧМаркированныеТовары.РодительскаяУпаковка) Тогда
			...
			Если СтрокаРодитель <> Неопределено
				...
				ВидыПродукцииПоТоварнойГруппе = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ЭлементОчереди.ТоварнаяГруппа);
				РезультатРазбора = ШтрихкодированиеОбщегоНазначенияИС.НоваяСтруктураОбработкиШтрихкода(
					СтрокаРодитель.НормализованныйШтрихкод, ВидыПродукцииПоТоварнойГруппе);
...
Процедура ПодготовитьСообщенияПоЗапросуВыгрузкиИСМП(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Если РезультатПолученияДанных.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаПринята
		...
		Иначе
			...
			ЭлементОчереди.ТекущийСтатус                = Перечисления.СтатусыОбработкиОтчетаИСМП.Обрабатывается;
...
Процедура ЗагрузитьРезультатПолученияВыгрузки(ЭлементОчереди, ПараметрыОбмена)
	...
	РезультатОбработкиЗаявки = ВыгрузкиДанныхИСМП.СтатусЗаданияНаВыгрузку(
		ЭлементОчереди.Организация, ЭлементОчереди.ИдентификаторЗаявки, ЭлементОчереди.ТоварнаяГруппа);
		...
		Если СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаВыполнена
			...
			Если СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.Ошибка
				...
				Если МассивОшибок.Количество() = 0 Тогда
					МассивОшибок.Добавить(СтрШаблон(
						Нстр("ru = 'Ошибка получения выгрузки ""%1"" по товарной группе ""%2"" из сервиса, описание ошибки отсутствует.'"),
						ЭлементОчереди.ИдентификаторЗаявки,
						ЭлементОчереди.ТоварнаяГруппа));
				Иначе
					...
					КонецЕсли;
					Если МассивОшибок.Количество() Тогда
						МассивОшибок.Вставить(0, СтрШаблон("Ошибка получения выгрузки ""%1"" по товарной группе ""%2""  из сервиса:",
							ЭлементОчереди.Операция,
							ЭлементОчереди.ТоварнаяГруппа));
					КонецЕсли;
				КонецЕсли;
...
Процедура ЗагрузитьСообщенияПоЗапросуОстатковВиртуальногоСкладаИСМП(ЭлементОчереди, ПараметрыОбмена)
	
	ОписаниеЗамера = ОценкаПроизводительности.НачатьЗамерДлительнойОперации(
		"ОбщийМодуль.ИнтеграцияИСМПСлужебный.ЗагрузитьОстаткиВиртуальногоСкладаИСМП");
	...
	СтатусОшибкаПередачи = Перечисления.СтатусыОбработкиОтчетаИСМП.ОшибкаПередачи;
	
	ДанныеВыгрузки = Новый ТаблицаЗначений;
	ДанныеВыгрузки.Колонки.Добавить("GTIN", ОбщегоНазначения.ОписаниеТипаСтрока(14));
	ДанныеВыгрузки.Колонки.Добавить("Количество", ОбщегоНазначения.ОписаниеТипаЧисло(10));
	
	ПараметрыНавигации = Новый Структура;
	ПараметрыНавигации.Вставить("НомерСтраницы", 0);
	ПараметрыНавигации.Вставить("ЗаписейНаСтранице", 1000);
	
	ЕстьДанныеНаСтранице = Истина;
	
	Пока ЕстьДанныеНаСтранице Цикл
		
		РезультатПолученияДанных = ИнтерфейсИСМП.ОстатокGTINПоДаннымВиртуальногоБаланса(
			ЭлементОчереди.Организация, 
			ЭлементОчереди.ТоварнаяГруппа, 
			Неопределено,
			ПараметрыНавигации);
		...
		Если ОбменДаннымиИСМП.ТребуетсяПовторОперации(ЭлементОчереди, РезультатПолученияДанных) Тогда
			// Если с момента последнего изменения статуса прошло более 5 минут,
			// то отложим обработку элемента очереди до следующей попытки обмена.
			ОтложитьОбработкуЭлементаОчереди(ЭлементОчереди, ПараметрыОбмена);
			Возврат;
		...
		Если ЗначениеЗаполнено(РезультатПолученияДанных.ТекстОшибки) Тогда
			ОбменДаннымиИСМП.ЗавершитьПодготовкуСообщенийАбстрактнойОперации(ЭлементОчереди, ПараметрыОбмена, СтатусОшибкаПередачи, РезультатПолученияДанных.ТекстОшибки);
			Возврат;
		КонецЕсли;
		
		ЕстьДанныеНаСтранице = Ложь;
		Если РезультатПолученияДанных.ОстатокGTIN <> Неопределено Тогда
			
			Для Каждого СтрокаОтвет Из РезультатПолученияДанных.ОстатокGTIN Цикл
				
				ЕстьДанныеНаСтранице = Истина;
				Если СтрокаОтвет.Значение <= 0 Тогда
					Продолжить;
				КонецЕсли;
				СтрокаТаблицы = ДанныеВыгрузки.Добавить();
				СтрокаТаблицы.GTIN = СтрокаОтвет.Ключ;
				СтрокаТаблицы.Количество = СтрокаОтвет.Значение;
				
			КонецЦикла;
			
		КонецЕсли;
		
		РезультатПолученияДанных.Вставить("СтатусОбработки", Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаВыполнена);
		РезультатПолученияДанных.Вставить("Операция",        ЭлементОчереди.Операция);
		ПредставлениеПротоколаОбмена = ОбменДаннымиИСМП.ДобавитьВПротоколОбмена(
			ЭлементОчереди,
	...
	Попытка
		...
		ЭтоЗагрузкаОстатков = Ложь;
		Если ПараметрыЗапроса.Свойство("ЭтоЗагрузкаОстатков", ЭтоЗагрузкаОстатков)
			И ЭтоЗагрузкаОстатков
			И ДанныеВыгрузки.Количество() = 0 Тогда
			// Не записывать пустой файл выгрузки, обновить дату синхронизации
		...
		Иначе
			...
			Если ОбменДаннымиИСМП.ТребуетсяОбновлениеКлючаСессии(ПараметрыОбновленияСтатуса, ЭлементОчереди, ПараметрыОбмена) Тогда
				ОтменитьТранзакцию();
				ОценкаПроизводительности.ЗафиксироватьЗамерДлительнойОперации(ОписаниеЗамера, 1, ТипИСМП);
				Возврат;
			КонецЕсли;
		...
		СтрокаРезультата.Сообщение                    = ЭлементОчереди.Сообщение;
		СтрокаРезультата.ТекстСообщения               = РезультатПолученияДанных.РезультатОтправкиЗапроса.ТекстВходящегоСообщенияJSON;
		СтрокаРезультата.ПредставлениеПротоколаОбмена = ПредставлениеПротоколаОбмена;
	...
	Исключение
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'При загрузке остатков виртуального склада по документу %1 возникла ошибка:
			           |Текст ошибки: %2
			           |Организация: %3
			           |Текст JSON см. в журнале регистрации.'"),
			ЭлементОчереди.Документ,
		...
		ТекстОшибкиЖурналРегистрации = СтрШаблон(
			...
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
			ЭлементОчереди.Организация,
			РезультатПолученияДанных.РезультатОтправкиЗапроса.ТекстВходящегоСообщенияJSON);
...
Функция ИнициализироватьПараметрыНавигации()
	
	ПараметрыНавигации = Новый Структура;
	ПараметрыНавигации.Вставить("ЕстьСледующаяСтраница", Неопределено);
	ПараметрыНавигации.Вставить("КоличествоЗаписей",     Неопределено);
	ПараметрыНавигации.Вставить("Курсор",                Неопределено);
	
	Возврат ПараметрыНавигации;
	
КонецФункции

// Проверяет, зарегистрировано ли место осуществления деятельности организации в ГИС МТ для указанной товарной группы.
//
// Параметры:
//   Организация    - ЛюбаяСсылка - организация
//   ИНН            - Строка - ИНН организации
//   КПП            - Строка - КПП организации
//   КодФИАС        - Строка - код ФИАС места осуществления деятельности
//   ТоварнаяГруппа - ЛюбаяСсылка - товарная группа
//
// Возвращаемое значение:
//   Структура:
//     * ТребуетсяОбновлениеКлючаСессии - Булево - признак необходимости обновления ключа сессии
//     * ТекстОшибки                    - Строка - описание ошибки
//     * МОДЗарегистрирован             - Булево - признак регистрации места осуществления деятельности в ГИС МТ
Функция МестоОсуществленияДеятельностиЗарегистрированоВГИСМТ(Организация, ИНН, КПП, КодФИАС, ТоварнаяГруппа) Экспорт
	
	ПараметрыОтбора = Новый Структура;
	ПараметрыОтбора.Вставить("ИННУчастника", ИНН);
	ПараметрыОтбора.Вставить("КППУчастника", КПП);
	ПараметрыОтбора.Вставить("КодФИАС",      КодФИАС);
	
	РезультатПроверкиМОД = ИнтерфейсИСМПОбщегоНазначения.СписокМестОсуществленияДеятельности(Организация, ПараметрыОтбора);
	
	Результат = Новый Структура;
	Результат.Вставить("ТребуетсяОбновлениеКлючаСессии", РезультатПроверкиМОД.ТребуетсяОбновлениеКлючаСессии);
	Результат.Вставить("ТекстОшибки",                    РезультатПроверкиМОД.ТекстОшибки);
	Результат.Вставить("МОДЗарегистрирован",             Ложь);
	
	Если Не Результат.ТребуетсяОбновлениеКлючаСессии Тогда
		МОДЗарегистрированВГИСМТ = Ложь;
		ИмяТоварнойГруппы        = ИнтерфейсИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппа(ТоварнаяГруппа);

		Если РезультатПроверкиМОД.МестаОсуществленияДеятельности.Количество() > 0 Тогда
			ТоварныеГруппыМОД = РезультатПроверкиМОД.МестаОсуществленияДеятельности[0].ТоварныеГруппы;

			Если ТоварныеГруппыМОД <> Неопределено И ТоварныеГруппыМОД.Найти(ИмяТоварнойГруппы) <> Неопределено Тогда
				МОДЗарегистрированВГИСМТ = Истина;
			КонецЕсли;

		КонецЕсли;
		
		Результат.МОДЗарегистрирован = МОДЗарегистрированВГИСМТ;
...
Функция МОДКРегистрацииВГИСМТПоСпискуДокументов(СписокДокументов) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ТребуетсяОбновлениеКлючаСессии", Ложь);
	Результат.Вставить("ТекстОшибки",                    "");
	Результат.Вставить("МОДКРегистрации",                Новый Массив);
	
	ТребуетсяРегистрация = Новый Массив;
	РезультатыПроверки   = Новый Соответствие;
	МОДыВСервисеГИСМТ    = Новый Соответствие;
	
	Для Каждого ДокументПроверки Из СписокДокументов Цикл
		ТоварнаяГруппа    = ДокументПроверки.ТоварныеГруппы[0];
		ИмяТоварнойГруппы = ИнтерфейсИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппа(ТоварнаяГруппа);
		
		МассивКлючейПроверки = Новый Массив;
		МассивКлючейПроверки.Добавить(ДокументПроверки.ИНН);
		МассивКлючейПроверки.Добавить(ДокументПроверки.КПП);
		МассивКлючейПроверки.Добавить(ДокументПроверки.КодФИАС);
		МассивКлючейПроверки.Добавить(ИмяТоварнойГруппы);

		КлючПроверки              = СтрСоединить(МассивКлючейПроверки, ",");
		РезультатыПроверкиПоКлючу = РезультатыПроверки.Получить(КлючПроверки);

		Если РезультатыПроверкиПоКлючу <> Неопределено Тогда
			СписокМОДПоПараметрам = РезультатыПроверкиПоКлючу;
		Иначе
			ПараметрыОтбора = Новый Структура;
			ПараметрыОтбора.Вставить("ИННУчастника",   ДокументПроверки.ИНН);
			ПараметрыОтбора.Вставить("КППУчастника",   ДокументПроверки.КПП);
			ПараметрыОтбора.Вставить("КодФИАС",        ДокументПроверки.КодФИАС);
			ПараметрыОтбора.Вставить("ТоварнаяГруппа", ТоварнаяГруппа);
			
			РезультатПроверкиМОД = ИнтерфейсИСМПОбщегоНазначения.СписокМестОсуществленияДеятельности(
				ДокументПроверки.Организация, ПараметрыОтбора);

			Если РезультатПроверкиМОД.ТребуетсяОбновлениеКлючаСессии Тогда
				Результат.ТребуетсяОбновлениеКлючаСессии = Истина;
			КонецЕсли;
			
			Если РезультатПроверкиМОД.ТребуетсяОбновлениеКлючаСессии 
				Или ЗначениеЗаполнено(РезультатПроверкиМОД.ТекстОшибки) Тогда
					Прервать;
			КонецЕсли;
			
			СписокМОДПоПараметрам = РезультатПроверкиМОД.МестаОсуществленияДеятельности;
			
			РезультатыПроверки.Вставить(КлючПроверки, СписокМОДПоПараметрам);
			
			Если СписокМОДПоПараметрам.Количество() Тогда
				МОДыВСервисеГИСМТ.Вставить(ДокументПроверки.Организация, СписокМОДПоПараметрам[0]);
			КонецЕсли;
			
		КонецЕсли;
		
		Если Не СписокМОДПоПараметрам.Количество() Тогда
			ТребуетсяРегистрация.Добавить(ДокументПроверки);
		КонецЕсли;

	КонецЦикла;
	
	Если МОДыВСервисеГИСМТ.Количество() Тогда
		Отказ = Не РегистрыСведений.МестаОсуществленияДеятельностиИСМП.ОбновитьСписокМОДПриПроверкеДокументов(МОДыВСервисеГИСМТ);
		
		Если Отказ Тогда
			ВызватьИсключение НСтр("ru = 'Не удалось записать места осуществления деятельности.
									|Подробности см. в журнале регистрации.'");
		КонецЕсли;
		
	КонецЕсли;
	
	Результат.МОДКРегистрации = ТребуетсяРегистрация;
