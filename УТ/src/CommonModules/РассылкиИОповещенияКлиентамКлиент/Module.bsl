...
Процедура ПереместитьВложениеВУдаленные(Форма) Экспорт
	...
	ВложенияТаблицаФормы = Форма.Элементы.Вложения; // ТаблицаФормы
	ВложенияДанные       = Форма.Вложения; // ДанныеФормыКоллекция
	УдаленныеВложения = Форма.УдаленныеВложения; // СписокЗначений - 
	ТекущиеДанные = ВложенияТаблицаФормы.ТекущиеДанные; // ДанныеФормыЭлементКоллекции 
	Если (ТекущиеДанные <> Неопределено)  Тогда
		Если ТекущиеДанные.Расположение = 0 Тогда
			УдаленныеВложения.Добавить(ТекущиеДанные.Ссылка);
		КонецЕсли;
		Индекс = ВложенияДанные.Индекс(ТекущиеДанные);
