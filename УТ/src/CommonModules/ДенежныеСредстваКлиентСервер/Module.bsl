...
// 
// Параметры:
//  Счет - ДанныеФормыЭлементКоллекции, Строка - счет для проверки.
	...
	Если ТипЗнч(ДанныеВыписки) = Тип("Структура") Тогда
		...
		Если ТипЗнч(СписокСчетовВыписки) = Тип("Массив") Тогда
			...
			Если ТипЗнч(Счет) = Тип("Строка") Тогда
				Результат = СписокСчетовВыписки.Найти(Счет) <> Неопределено;
			Иначе
...
Процедура ЗаполнитьОсновныеРеквизитыБанка(Форма) Экспорт
	...
	Если Не Форма.Объект.СчетЦифровогоРубля
			И Форма.ИспользуетсяБанкДляРасчетов Тогда
...
Процедура ПересчитатьПодчиненныеСтрокиДерева(СтрокаДерева, ДнейПланирования) Экспорт
	...
	ИначеЕсли СтрокаДерева.ВидСтроки = 1 Тогда // Оборот в валюте ДС, либо в валюте платежа
		...
		СтрокаДерева.День0 = СтрокаДерева.Просрочено;
		СтрокаДерева.НеснижаемыйОстаток = 0;
	...
	Для Каждого СтрокаДереваДочерняя Из Строки Цикл
		ПересчитатьПодчиненныеСтрокиДерева(СтрокаДереваДочерняя, ДнейПланирования);
	КонецЦикла;
...
// Функция - Рассчитать сумму комиссии.
// "Поступление" - комиссия уже вычтене из суммы поступления.
// "Списание" - комиссия в сумме документа.
// "ЭквайринговаяОперация" - комиссия считается сверху.
//
// Параметры:
//  Сумма	 - Число - Сумма документа
//  Ставка	 - Число - Ставка комиссии (%)
//  Операция - Строка - "Поступление", "Возврат", "ЭкваринговаяОперация".
// 
// Возвращаемое значение:
//  Число - Сумма комиссии.
//
Функция РассчитатьСуммуКомиссии(Сумма, Ставка, Операция = "ЭквайринговаяОперация") Экспорт
	
	Если Операция = "Поступление" Тогда
		Если Ставка <> 100 Тогда
			Возврат Сумма * Ставка / (100 - Ставка);
		Иначе
			Возврат Сумма;
		КонецЕсли;
	ИначеЕсли Операция = "ЭквайринговаяОперация" Тогда
		Возврат Сумма / 100 * Ставка;
	ИначеЕсли Операция = "Возврат" Тогда
		Возврат Сумма * Ставка / (100 + Ставка);
	Иначе
		ВызватьИсключение(НСтр("ru = 'Неизвестный вид операции эквайринга.'"));
	КонецЕсли;
	
КонецФункции

// Функция - Рассчитать ставку комиссии.
// "Поступление" - комиссия уже вычтене из суммы поступления.
// "Списание" - комиссия в сумме документа.
// "ЭквайринговаяОперация" - комиссия считается сверху.
//
// Параметры:
//  Сумма	 - Число - Сумма документа
//  Комиссия	 - Число - Сумма комиссии
//  Операция - Строка - "Поступление", "Возврат", "ЭкваринговаяОперация".
// 
// Возвращаемое значение:
//  Число - Ставка комиссии.
//
Функция РассчитатьСтавкуКомиссии(Сумма, Комиссия, Операция = "ЭквайринговаяОперация") Экспорт
...
// Заполняет данные банковского счета в структуру по данным формы-источника.
//
// Параметры:
//  ДанныеСчета - см. НовыеДанныеБанковскогоСчета.
//  Источник - ФормаКлиентскогоПриложения - форма-источник для заполнения.
//  Суффикс - Строка - дополнение к наименованию реквизита на форме.
//
Процедура ЗаполнитьДанныеБанковскогоСчета(ДанныеСчета, Источник, Суффикс) Экспорт

	Если ДанныеСчета = Неопределено Тогда
		ДанныеСчета = НовыеДанныеБанковскогоСчета();
	КонецЕсли;
	...
	Если Суффикс <> "" Тогда
		...
		Для Каждого ЭлементСчета Из ДанныеСчета Цикл
			Результат.Вставить(ЭлементСчета.Ключ, ЭлементСчета.Ключ + Суффикс);
		КонецЦикла;
	
		Для Каждого ЭлементРезультата Из Результат Цикл
			ДанныеСчета[ЭлементРезультата.Ключ] = Источник[ЭлементРезультата.Значение];
		КонецЦикла;
		
	Иначе
		ЗаполнитьЗначенияСвойств(ДанныеСчета, Источник);
	КонецЕсли;

КонецПроцедуры

// Возвращает пустую структуру данных банковского счета по умолчанию.
// 
// Возвращаемое значение:
//  Структура - данные счета:
//  * НомерСчета - Строка - номер счета.
//  * ЭтоIBAN - Булево - признак счета IBAN.
//  * СтранаБанка - СправочникСсылка.СтраныМира - страна банка.
//
Функция НовыеДанныеБанковскогоСчета() Экспорт

	Реквизиты = Новый Структура;
	Реквизиты.Вставить("НомерСчета", "");
	Реквизиты.Вставить("ЭтоIBAN", Ложь);
	Реквизиты.Вставить("СтранаБанка", ПредопределенноеЗначение("Справочник.СтраныМира.ПустаяСсылка"));
	
	Возврат Реквизиты;

КонецФункции

// Очищает банковские реквизиты на форме.
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - форма, на которой выполняется очистка.
//  БанковскиеРеквизиты - Строка, Структура - список реквизитов банка.
//  ЭтоБанкДляРасчетов - Булево - признак реквизитов банка для расчетов.
//  ЭтоГоловнойКонтрагент - Булево - признак реквизитов банка головного контрагента.
//
Процедура ОчиститьБанковскиеРеквизитыНаФорме(Форма, Знач БанковскиеРеквизиты) Экспорт

	Если ТипЗнч(БанковскиеРеквизиты) = Тип("Строка") Тогда
		БанковскиеРеквизиты = Новый Структура(БанковскиеРеквизиты);
	КонецЕсли;
	
	Для каждого КлючИЗначение Из БанковскиеРеквизиты Цикл
		Форма[КлючИЗначение.Ключ] = "";
	КонецЦикла;

КонецПроцедуры

// Возвращает строку банковских реквизитов.
// 
// Параметры:
//  ПолныйСписок - Булево - признак, формирующий строку со всеми реквизитами банка.
//  ЭтоБанкДляРасчетов - Булево - признак формирования списка реквизитов банка для расчетов.
// 
// Возвращаемое значение:
//  Строка - Реквизиты банка
//
Функция РеквизитыБанка(ПолныйСписок = Ложь, ЭтоБанкДляРасчетов = Ложь) Экспорт
	
	Если ЭтоБанкДляРасчетов Тогда
		ИменаРеквизитов = "БИКБанкаДляРасчетов, СВИФТБанкаДляРасчетов, НаименованиеБанкаДляРасчетов,
		| НаименованиеБанкаДляРасчетовМеждународное, КоррСчетБанкаДляРасчетов,
	...
	Иначе
		ИменаРеквизитов = "БИКБанка, СВИФТБанка, НаименованиеБанка, НаименованиеБанкаМеждународное, КоррСчетБанка,
	...
	Если ПолныйСписок Тогда
		...
		Если ЭтоБанкДляРасчетов Тогда
			ИменаРеквизитов = СтрШаблон("%1, СтранаБанкаДляРасчетов", ИменаРеквизитов);
		Иначе
			ИменаРеквизитов = СтрШаблон("%1, СтранаБанка", ИменаРеквизитов);
		КонецЕсли;
