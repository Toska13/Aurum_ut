...
//
// Параметры:
//  ДанныеОчередей - Строка, Массив из Строка, ТаблицаЗначений, Неопределено -
...
//
Процедура Запустить(Знач ДанныеОчередей = Неопределено) Экспорт
	
	ИнформационнаяБазаФайловая = ОбщегоНазначения.ИнформационнаяБазаФайловая();
	РежимОтладки               = ОбщегоНазначения.РежимОтладки();
	
	ТипОчереди = ТипЗнч(ДанныеОчередей);
	...
	Если ТипОчереди = Тип("ТаблицаЗначений") Тогда
		...
		ТаблицаОчередей = ДанныеОчередей;
	...
	ИначеЕсли Не ИнформационнаяБазаФайловая И Не РежимОтладки Тогда
		...
		Если ТипОчереди = Тип("Строка") Тогда
			...
			ТаблицаОчередей = ОчередиЗаданий().Скопировать(Новый Структура("Очередь", ДанныеОчередей), ПоляЗапуска());
			
		ИначеЕсли ТипОчереди = Тип("Массив") Тогда
			
			Если ДанныеОчередей.Количество() = 1 Тогда
				
				ТаблицаОчередей = ОчередиЗаданий().Скопировать(Новый Структура("Очередь", ДанныеОчередей[0]), ПоляЗапуска());
			...
			Иначе
				...
				Для каждого Очередь Из ДанныеОчередей Цикл
...
// 
// Параметры:
//  ДанныеОчередей - Строка, Массив из Строка, Структура, Неопределено - очереди, для которых нужно инициализировать таблицу
...
//
Функция ИнициализироватьТаблицуЗаданий(ДанныеОчередей = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеОчередей) Тогда
		...
		Если ТипОчереди = Тип("Строка") Тогда
			...
			СтрокаТаблицы.Очередь = ДанныеОчередей;
			СтрокаТаблицы.Задания = РегистрыСведений[ДанныеОчередей].СоздатьНаборЗаписей().ВыгрузитьКолонки();
		...
		Иначе
			...
			Для каждого ЭлементОчередь Из ДанныеОчередей Цикл
...
Функция ОчередиЗаданий() Экспорт
	...
	Параметры = ПараметрыОчередиЗаданий();
	Параметры.Очередь                     = МетаРегистр.ЗаданияКОповещениюПользователей.Имя;
	Параметры.ИмяМетода                   = "ОповещенияПользователейОСобытиях.ВыполнитьЗаданияКОповещениюПользователей";
	Параметры.АналитикаРасчета            = "ВидОповещения";
	Параметры.ВыполнитьВТранзакцииВФайловойБазе = Ложь;
	ДобавитьОчередь(ТаблицаОчередей, Параметры);
	
	Параметры = ПараметрыОчередиЗаданий();
	Параметры.Очередь                     = МетаРегистр.ЗаданияКРаспределениюВзаиморасчетов.Имя;
	Параметры.ИмяМетода                   = "РегистрыСведений.ЗаданияКРаспределениюВзаиморасчетов.ВыполнитьОтложенноеРаспределениеВзаиморасчетов";
	Параметры.АналитикаРасчета            = "АналитикаУчетаПоПартнерам, ОбъектРасчетов, Валюта";
	Параметры.ПорцияРасчета               = НеоперативнаяОчередьЗаданийПовтИсп.РазмерПорцииРаспределениеВзаиморасчетовПоСрокам();
	Параметры.РежимРаспределенияЗаданий   = "МАКСИМУМ_ЗАДАНИЙ_В_ПОТОК";
...
//
// Параметры:
//  ТаблицаОчередей        - см. ВыполнитьСтарт.ТаблицаОчередей
//  ЗаписатьОшибку - Булево - Если ИСТИНА, тогда ошибка запуска управляющего потока записывается в журнал регистрации.
//
Процедура Стартер(ТаблицаОчередей, ЗаписатьОшибку = Ложь) Экспорт
	
	РежимОтладки = ОбщегоНазначения.РежимОтладки();
	
	ЕстьЗадания = ТаблицаОчередей.Колонки.Найти("Задания") <> Неопределено;
...
Процедура УправляющийПоток(Очередь, Задания = Неопределено, ОбработатьТолькоТекущиеЗадания = Ложь) Экспорт
	...
	Иначе
		...
		МассивОчередей.Добавить(Очередь);
		
		ТаблицаНастроек  = НастройкиОчередей(МассивОчередей);
...
//  ТекстОшибки              - см. ВыполнитьЗадания.ТекстОшибки
//
Процедура ЗаданияКРаспределениюВзаиморасчетов(
			ПараметрыОчереди,
			ПараметрыДанных,
			ЗафиксироватьОшибку = Ложь,
			ОбработатьВТекущемСеансе = Ложь,
...
//  ТекстОшибки              - см. ВыполнитьЗадания.ТекстОшибки
//
Процедура ЗаданияКОповещениюПользователей(
			ПараметрыОчереди,
			ПараметрыДанных,
			ЗафиксироватьОшибку = Ложь,
			ОбработатьВТекущемСеансе = Ложь,
...
Процедура УдалитьЗадания(ПараметрыОчереди, Данные, Ошибки, ТекстИсключения = "")
	...
	Очередь                     = ПараметрыОчереди.Очередь;
	ВыполнитьЗаданияВТранзакции = ПараметрыОчереди.ВыполнитьЗаданияВТранзакции;
	ПолучатьЗаписиКУдалениюПоИдентификаторам = Не ПараметрыОчереди.ВыборкаЗаданийПоУмолчанию;
	
	НаборЗаписей = РегистрыСведений[Очередь].СоздатьНаборЗаписей();
	ПорцияДанныхКУдалению = НаборЗаписей.Выгрузить();
	
	Если ПолучатьЗаписиКУдалениюПоИдентификаторам Тогда
		ПорцияДанныхКУдалению.Свернуть("ИдентификаторЗаписи");
	КонецЕсли;
	
	МассивОбработанныхСтрок = Новый Массив;
	МассивСтрокПорцииДанных = Новый Массив;
	...
	Если ВыполнитьЗаданияВТранзакции Тогда 
		...
		Попытка
			...
			Для каждого СтрокаДанных Из Данные Цикл
				...
				ЗаполнитьЗначенияСвойств(ПорцияДанныхКУдалению.Добавить(), СтрокаДанных);
				
				МассивСтрокПорцииДанных.Добавить(СтрокаДанных);
				
				Если ПорцияДанныхКУдалению.Количество() = РазмерПорции Тогда
					
					УдалитьЗаписиИзРегистраОчередиЗаданий(Очередь, ПорцияДанныхКУдалению, НаборЗаписей, ПолучатьЗаписиКУдалениюПоИдентификаторам);
					ПорцияДанныхКУдалению.Очистить();
					
					ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивОбработанныхСтрок, МассивСтрокПорцииДанных);
					МассивСтрокПорцииДанных.Очистить();
					
				КонецЕсли;
				
			КонецЦикла;
			
			Если ПорцияДанныхКУдалению.Количество() > 0 Тогда
			...
			Для каждого СтрокаКУдалению Из МассивОбработанныхСтрок Цикл
				Данные.Удалить(СтрокаКУдалению);
			КонецЦикла;
	...
	Иначе
		...
		ВозниклаОшибкаПриУдалении = Ложь;
		...
		Для каждого СтрокаДанных Из Данные Цикл
			...
			Если ПорцияДанныхКУдалению.Количество() = РазмерПорции Тогда
				...
				Исключение 
					...
					ВозниклаОшибкаПриУдалении = Истина;
		...
		Если Не ВозниклаОшибкаПриУдалении И ПорцияДанныхКУдалению.Количество() > 0 Тогда
...
Процедура УдалитьЗаписиИзРегистраОчередиЗаданий(Очередь, ДанныеКУдалению, Знач НаборЗаписей = Неопределено, ПолучатьДанныеКУдалениюПоИдентификаторам = Ложь)
	
	Если НаборЗаписей = Неопределено Тогда
		НаборЗаписей = РегистрыСведений[Очередь].СоздатьНаборЗаписей();
	КонецЕсли;
	
	Если ПолучатьДанныеКУдалениюПоИдентификаторам Тогда
		
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("ИдентификаторыЗаписей", ДанныеКУдалению.ВыгрузитьКолонку("ИдентификаторЗаписи"));
		Запрос.Текст = "
		|ВЫБРАТЬ *
		|ИЗ
		|	&ТаблицаОчереди КАК Очередь
		|ГДЕ
		|	Очередь.ИдентификаторЗаписи В (&ИдентификаторыЗаписей)";
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"&ТаблицаОчереди" , "РегистрСведений." + Очередь);
		
		УстановитьПривилегированныйРежим(Истина);
		ЗаписиЗаданийКУдалению = Запрос.Выполнить().Выгрузить();
		УстановитьПривилегированныйРежим(Ложь);
			
	Иначе
		
		ЗаписиЗаданийКУдалению = ДанныеКУдалению.Скопировать();
		
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Истина);
	НаборЗаписей.ОбменДанными.Загрузка = Истина;
	НаборЗаписей.Загрузить(ЗаписиЗаданийКУдалению);
...
// 
// Параметры:
//  МассивОчередей - Массив из Строка, Неопределено - массив имен регистров сведений очередей заданий.
// 
// Возвращаемое значение:
...
//
Функция НастройкиОчередей(МассивОчередей = Неопределено)
	...
	|ГДЕ
	|	Настройки.Очередь В (&МассивОчередей)";
	
	Если ЗначениеЗаполнено(МассивОчередей) Тогда
		Запрос.УстановитьПараметр("МассивОчередей", МассивОчередей);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Настройки.Очередь В (&МассивОчередей)", "ИСТИНА");
	КонецЕсли;
