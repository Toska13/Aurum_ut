...
// Возвращает список видов заданий, по которым не следует выполнять пакетную обработку гранул расчета в одной транзации.
// Вид задания следует исключить из пакетной обработки гранул расчёта, если связанная с ним процедура-обработчик 
// не поддерживает одновременную обработку нескольких объектов (гранул расчета) или обработка одного объекта может быть длительной по времени.
// 
// Возвращаемое значение:
//  Массив Из СправочникСсылка.ВидыНеоперативныхЗаданий - Виды заданий
Функция ВидыЗаданийНедоступныеКПакетнойОбработкеГранулРасчета() Экспорт
	
	СписокВидовЗаданий = Новый Массив;
	
	СписокВидовЗаданий.Добавить(Справочники.ВидыНеоперативныхЗаданий.АктуализацияДвиженийПоФинансовымРегистрам);
	СписокВидовЗаданий.Добавить(Справочники.ВидыНеоперативныхЗаданий.ОтражениеДвиженийПоАкцизам);
	СписокВидовЗаданий.Добавить(Справочники.ВидыНеоперативныхЗаданий.АктуализацияОперацийИСостоянияЭлектронногоБилета);
	СписокВидовЗаданий.Добавить(Справочники.ВидыНеоперативныхЗаданий.РаспределениеПродажПоНаправлениямДеятельности);
	
	Возврат СписокВидовЗаданий;
