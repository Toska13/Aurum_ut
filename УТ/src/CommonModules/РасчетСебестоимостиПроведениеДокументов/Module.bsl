...
Функция ОписаниеОперацииСебестоимостиВнешнееПоступление(ПараметрыОтражения) Экспорт
	...
	// Необязательные поля.
	ДобавитьОписаниеПоляОперации(МетаданныеРегистра, ОписаниеОперации.Поля, "ПериодОтклонения", Новый ОписаниеТипов("Дата"), Истина);
...
Функция ОписаниеОперацииСебестоимостиРеализация(ПараметрыОтражения) Экспорт
	...
	ДобавитьОписаниеПоляОперации(МетаданныеРегистра, ОписаниеОперации.Поля, "СпособОпределенияСебестоимости",, Истина);
	ДобавитьОписаниеПоляОперации(МетаданныеРегистра, ОписаниеОперации.Поля, "ДокументДвижения",, Истина);
	ДобавитьОписаниеПоляОперации(МетаданныеРегистра, ОписаниеОперации.Поля, "ПериодПродажи",, Истина);
...
Процедура ОтразитьОперацииВУчетеСебестоимости(ПараметрыОтражения)
	...
	Для Каждого ТекущаяОперацияДокумента Из ВидыОперацийДокумента Цикл
		...
		// Сформируем текст запроса формирования временной таблицы отражения операции документа.
...
Процедура ОтразитьОперациюСебестоимостиВнешнееПоступление(ПараметрыОтражения)
	...
	|	(ВЫБОР
	|		КОГДА ДанныеДокумента.ЕстьПолеЗапроса_ПериодОтклонения
	|		 И ДанныеДокумента.ПериодОтклонения < НАЧАЛОПЕРИОДА(ДанныеДокумента.Период, МЕСЯЦ)
	|			ТОГДА ДанныеДокумента.ПериодОтклонения
	|		 ИНАЧЕ ДАТАВРЕМЯ(1,1,1) КОНЕЦ) КАК ПериодОтклонения,
	|
	// Поля учета номенклатуры
...
Процедура ОтразитьОперациюСебестоимостиПеремещение(ПараметрыОтражения)
	...
	|		КОГДА ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносОстатковПодНовыйВидДеятельности)
	|			И НАЧАЛОПЕРИОДА(ДанныеДокумента.Период, Месяц) <> ДанныеДокумента.Период
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ПеремещениеНаОстаток)
...
Процедура ОтразитьОперациюСебестоимостиРеализация(ПараметрыОтражения)
	...
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.ЕстьПолеЗапроса_ПериодПродажи
	|			ТОГДА ДанныеДокумента.ПериодПродажи
	|		ИНАЧЕ ДанныеДокумента.ПериодДокументаИсточника
...
Процедура ОтразитьОперациюСебестоимостиСписаниеНаРасходыАктивы(ПараметрыОтражения)
	...
	|	ВЫБОР КОГДА НЕ ДанныеДокумента.ХозяйственнаяОперация В(
	|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтчетДавальцу2_5),
	|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПроизводствоПродукцииИзДавальческогоСырья2_5),
	|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РемонтПродукцииИзДавальческогоСырья2_5),
	|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыполнениеРаботСИспользованиемДавальческогоСырья2_5),
	...
	|			И ДанныеДокумента.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПроизводствоПродукцииИзДавальческогоСырья2_5)
	|			И ДанныеДокумента.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РемонтПродукцииИзДавальческогоСырья2_5)
	|			И ДанныеДокумента.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыполнениеРаботСИспользованиемДавальческогоСырья2_5)
	|			И ДанныеДокумента.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыполнениеРемонтныхРаботСИспользованиемДавальческогоСырья2_5)
...
Функция ТекстЗапросаКодыСтрокДляАналитикиУчетаПартий(ИмяДокумента, ИменаТабличныхЧастей, ИмяРеквизитаАналитики = "АналитикаУчетаНоменклатуры") Экспорт
	...
	// Для документа ПоступлениеУслугВПодразделение используется отдельная функция ТекстЗапросаВтТаблицаАналитикУчетаПартий()
	// для получения кодов строк, которая размещена в модуле менеджера документа ПоступлениеУслугВПодразделение.
...
Процедура ДобавитьТекстЗапросаОтклоненияВСтоимостиТоваров(ПараметрыОтражения)
	...
	|	И НЕ (ЕСТЬNULL(Т.ВидЗапасовТипЗапасов, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.СобственныйТоварПоНеотфактурованнойПоставке)
	|		И НАЧАЛОПЕРИОДА(Т.ПериодДокументаИсточника, МЕСЯЦ) >= НАЧАЛОПЕРИОДА(Т.Период, МЕСЯЦ))
	|	И (&ТекстУсловия)";
