...
Функция НовыйСтруктураШага() Экспорт
	...
	СтруктураШага.Вставить("ВысотаДляТекста",  0);
...
Процедура РазместитьКартинки(Форма, СтруктураНавигацииПомощника, Сдвиг = 0, ПостфиксЭлемента = "", ОтмечатьТекущий = Ложь)
	...
	Если ЭлементНавигацииКартинка.Родитель.ГоризонтальноеПоложениеПодчиненных = ГоризонтальноеПоложениеЭлемента.Авто
	Или ЭлементНавигацииКартинка.Родитель.ГоризонтальноеПоложениеПодчиненных = ГоризонтальноеПоложениеЭлемента.Лево Тогда 
	...
	Для НомерШага = 1 По СтруктураНавигацииПомощника.Количество() Цикл
		...
		// Линия
		ЦветЛинии   = ?(ПредыдущийШагВыполнен И (СтруктураШага.Выполнен Или НомерШагаСоСдвигом = НомерШагаФормы), "Выполненный", "НеВыполненный");
		...
		ПредыдущийШагВыполнен = СтруктураШага.Выполнен;
...
Процедура РазместитьТексты(Форма, СтруктураНавигацииПомощника, Сдвиг = 0, ПостфиксЭлемента = "", ОтмечатьТекущий = Истина)
	...
	Для НомерШага = 1 По КоличествоШагов Цикл
		...
		Если СтруктураШага.КоличествоВерсий > 1 Тогда
			...
			ВысотаДляТекста = 0;
			Если СтруктураШага.Свойство("ВысотаДляТекста", ВысотаДляТекста) И ВысотаДляТекста > 0 Тогда
				ЭлементФормы.Высота = ВысотаДляТекста;
			Иначе
