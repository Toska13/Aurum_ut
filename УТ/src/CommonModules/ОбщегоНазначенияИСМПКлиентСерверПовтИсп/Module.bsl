...
// Проверяет, используется ли расширенная версия ГосИС
//
// Возвращаемое значение:
//  Булево - Истина, если используется расширенная версия
...
//
// Возвращаемое значение:
//   ФиксированныйМассив Из СправочникСсылка.ВидыПродукцииИС - виды маркируемой продукции
//
Функция УчитываемыеВидыМаркируемойПродукции(ПродукцияИСМП = Истина, ПродукцияМОТП = Истина) Экспорт
	...
	Для Каждого ВидПродукции Из ВидыМаркируемойПродукции Цикл
		Если (ПродукцияИСМП И Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(ВидПродукции))
			Или (ПродукцияМОТП И КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(ВидПродукции)) Тогда
...
Функция ТоварнаяГруппа(ЗначениеПоиска, ДанныеДляУточнения = Неопределено) Экспорт
	...
	Возврат ИнтерфейсИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппа(ЗначениеПоиска, ДанныеДляУточнения);
...
//
//Возвращаемое значение:
//   ФиксированныйМассив Из СправочникСсылка.ВидыПродукцииИС - виды маркируемой продукции, по которым установлен 
...
Функция ДатаНачалаКонтроляПриПродажеОптомПоштучно(ВидПродукции) Экспорт
	...
	ДатаОптКонтрольПоштучно = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДатаНачалаОформленияУПДПоштучно(
		ВидПродукции);
	
	Возврат ДатаОптКонтрольПоштучно;
	
КонецФункции

// Возвращает дату начала контроля разрешительной документации по маркируемой продукции
// 
// Параметры:
//  ВидПродукции - СправочникСсылка.ВидыПродукцииИС - вид маркируемой продукции
// 
// Возвращаемое значение:
//  Дата - Дата начала контроля
Функция ДатаКонтроляРазрешительнойДокументации(ВидПродукции) Экспорт
	
	ДатаКонтрольРД = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДатаНачалаКонтроляРазрешительныхДокументов(
		ВидПродукции);
	
	Возврат ДатаКонтрольРД;
	
КонецФункции

Функция ДанныеТоварнойГруппы(ТоварнаяГруппа) Экспорт
	
	Возврат ОбщегоНазначенияИСМПВызовСервера.ДанныеТоварнойГруппы(ТоварнаяГруппа);
	
КонецФункции

Функция ТоварнаяГруппаПоРеквизиту(ИмяРеквизита, ЗначениеРеквизита) Экспорт
	
	Возврат ОбщегоНазначенияИСМПВызовСервера.ТоварнаяГруппаПоРеквизиту(ИмяРеквизита, ЗначениеРеквизита);
...
// 
// Параметры:
//  ВидПродукции - СправочникСсылка.ВидыПродукцииИС, СправочникСсылка.КлючиВидовПродукцииИС - Вид продукции для проверки
...
Функция ПродукцияПодлежитОбязательнойОнлайнПроверкеПередРозничнойПродажей(ВидПродукции, ВидОперации = Неопределено, ТребуетсяПроверкаСредствамиККТ = Истина) Экспорт
	...
	МассивПродукцийСПроверкой = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииОбязательнойОнлайнПроверкиПередРозничнойПродажейНаДату();
	
	Если ТипЗнч(ВидПродукции) = Тип("СправочникСсылка.КлючиВидовПродукцииИС") Тогда
		ПроверяемыйВидПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(ВидПродукции, "ГИСМТ");
	Иначе
		ПроверяемыйВидПродукции = ВидПродукции;
	КонецЕсли;
	
	ПродукцияПодлежитПроверке = МассивПродукцийСПроверкой.Найти(ПроверяемыйВидПродукции) <> Неопределено;
...
Функция ПродукцияПодлежитОбязательнойОффлайнПроверкеПередРозничнойПродажей(ВидПродукции, ВидОперации = Неопределено, ТребуетсяПроверкаСредствамиККТ = Истина) Экспорт
	...
	МассивПродукцийСПроверкой = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииОбязательнойОфлайнПроверкиПередРозничнойПродажейНаДату();
...
// Учитывается только сырье.
// 
// Возвращаемое значение:
//  Булево - Учитывается только сырье
Функция УчитываетсяТолькоСырье() Экспорт
	...
	Если ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ЭтоРасширеннаяВерсияГосИС() Тогда
		...
		#КонецЕсли
		ВидыПродукцииСырье = МодульКонструкторИСМПКлиентСервер.ВидыПродукцииСырьеГИСМТ();
		УчитываемыеВидыПродукции = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.УчитываемыеВидыМаркируемойПродукции();
		Возврат (УчитываемыеВидыПродукции.Количество() = ВидыПродукцииСырье.Количество());
	Иначе
		Возврат Ложь;
	КонецЕсли;
