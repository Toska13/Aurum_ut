...
Процедура ДокументыОснованиеПредставлениеНажатие(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли Форма.ДокументыОснования.Количество() > 1 Тогда
		...
		ИначеЕсли РасхожденияКлиентСервер.ТипОснованияОтгрузкаТоваровКлиенту(Форма.Объект.ТипОснованияАктаОРасхождении) Тогда
			ЗаголовокФормыПросмотра = НСтр("ru='Отгрузки товаров клиенту (%КоличествоДокументов%)'");
...
// 	* СтрокиДляПересчетаСерий               - Массив из Число - список номеров строк для обновления статуса указания серии.
//
Функция УстановитьВариантДействияВыделеннымСтрокам(
	Форма,
	...
	СтруктураВозврата.Вставить("СтрокиДляПересчетаСерий", Новый Массив);
	КоличествоИзмененныхСтрокСоответствие = Новый Соответствие;
	
	КоличествоИзмененныхСтрокСоответствие.Вставить(ДействиеСДокументомНедостачи, 0);
	КоличествоИзмененныхСтрокСоответствие.Вставить(ДействиеСДокументомИзлишки, 0);
	Если ДействиеСТоваромНедостачи <> Неопределено Тогда
		КоличествоИзмененныхСтрокСоответствие.Вставить(ДействиеСТоваромНедостачи, 0);
	КонецЕсли;
	Если ДействиеСТоваромИзлишки <> Неопределено Тогда
		КоличествоИзмененныхСтрокСоответствие.Вставить(ДействиеСТоваромИзлишки, 0);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ДействиеСДокументомНедостачи)
		И Не ЗначениеЗаполнено(ДействиеСДокументомИзлишки) Тогда
...
Процедура ТоварыДействиеНачалоВыбора(ТекущиеДанные, Объект, Форма, ТипАкта, ПоказыватьПояснение = Ложь) Экспорт
	...
	Если Не ОбщегоНазначенияУТКлиентСервер.АвторизованВнешнийПользователь() Тогда
		...
		ЕстьТара = (ТекущиеДанные.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.МногооборотнаяТара"));
		...
		ПараметрыФормы.Вставить("ЕстьТара",                      ЕстьТара);
		
		Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.АктОРасхожденияхПослеОтгрузки") Тогда
			ПараметрыФормы.Вставить("ВыбранноеДействиеСДокументом", ТекущиеДанные.Действие);
			ПараметрыФормы.Вставить("ВыбранноеДействиеСТоваром", ТекущиеДанные.ДействиеСТоваром);
			ПараметрыФормы.Вставить("Договор", Объект.Договор);
		ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.АктОРасхожденияхПослеПеремещения") Тогда
			ПараметрыФормы.Вставить("ВыбранноеДействиеСДокументом", ТекущиеДанные.Действие);
		...
		Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.АктОРасхожденияхПослеОтгрузки")
			...
			ИмяФормы = "Обработка.РаботаСАктамиРасхождений.Форма.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки";
		Иначе
...
Процедура ИзменитьДействиеВыделенныхСтрок(Объект, Форма, ТипАкта, ПоказыватьПояснение = Ложь) Экспорт
	...
	Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.АктОРасхожденияхПослеОтгрузки") Тогда
		ИзменитьДействиеВыделенныхСтрокВАктеПослеОтгрузки(Объект, Форма, ТипАкта, ПоказыватьПояснение);
	Иначе
		ИзменитьДействиеВыделенныхСтрокВАктеПослеПриемкиПеремещения(Объект, Форма, ТипАкта, ПоказыватьПояснение);
	КонецЕсли;
	
КонецПроцедуры

Процедура ИзменитьДействиеВыделенныхСтрокВАктеПослеОтгрузки(Объект, Форма, ТипАкта, ПоказыватьПояснение = Ложь)
	...
	МассивДействийСДокументомПоНедостачам = Новый Массив;
	МассивДействийСДокументомПоИзлишкам = Новый Массив;
	
	МассивДействийСТоваромПоНедостачам = Новый Массив;
	МассивДействийСТоваромПоИзлишкам = Новый Массив;
	...
	Для Каждого ВыделеннаяСтрока Из Форма.Элементы.Товары.ВыделенныеСтроки Цикл
		...
		Если ДанныеСтроки.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.МногооборотнаяТара") Тогда
			ЕстьТара = Истина;
		КонецЕсли;
		...
		Если ЭтоИзлишки Тогда
			
			Если МассивДействийСДокументомПоИзлишкам.Найти(ДанныеСтроки.Действие) = Неопределено Тогда
				МассивДействийСДокументомПоИзлишкам.Добавить(ДанныеСтроки.Действие);
			КонецЕсли;
			
			Если МассивДействийСТоваромПоИзлишкам.Найти(ДанныеСтроки.ДействиеСТоваром) = Неопределено Тогда
				МассивДействийСТоваромПоИзлишкам.Добавить(ДанныеСтроки.ДействиеСТоваром);
			КонецЕсли;
			
		КонецЕсли;
		
		Если ЭтоНедостачи Тогда
			
			Если МассивДействийСДокументомПоНедостачам.Найти(ДанныеСтроки.Действие) = Неопределено Тогда
				МассивДействийСДокументомПоНедостачам.Добавить(ДанныеСтроки.Действие);
			КонецЕсли;
			
			Если МассивДействийСТоваромПоНедостачам.Найти(ДанныеСтроки.ДействиеСТоваром) = Неопределено Тогда
				МассивДействийСТоваромПоНедостачам.Добавить(ДанныеСтроки.ДействиеСТоваром);
			КонецЕсли;
	...
	Если МассивДействийСДокументомПоИзлишкам.Количество() = 0
		И МассивДействийСДокументомПоНедостачам.Количество() = 0 Тогда
	...
	Если МассивДействийСДокументомПоИзлишкам.Количество() = 1 Тогда
		ПараметрыФормы.Вставить("ДействиеСДокументомИзлишки", МассивДействийСДокументомПоИзлишкам[0]);
	Иначе
		ПараметрыФормы.Вставить("ДействиеСДокументомИзлишки", Неопределено);
	КонецЕсли;
	
	Если МассивДействийСДокументомПоНедостачам.Количество() = 1 Тогда
		ПараметрыФормы.Вставить("ДействиеСДокументомНедостачи", МассивДействийСДокументомПоНедостачам[0]);
	Иначе
		ПараметрыФормы.Вставить("ДействиеСДокументомНедостачи", Неопределено);
	КонецЕсли;
	
	Если МассивДействийСТоваромПоИзлишкам.Количество() = 1 Тогда
		ПараметрыФормы.Вставить("ДействиеСТоваромИзлишки", МассивДействийСТоваромПоИзлишкам[0]);
	Иначе
		ПараметрыФормы.Вставить("ДействиеСТоваромИзлишки", Неопределено);
	КонецЕсли;
	
	Если МассивДействийСТоваромПоНедостачам.Количество() = 1 Тогда
		ПараметрыФормы.Вставить("ДействиеСТоваромНедостачи", МассивДействийСТоваромПоНедостачам[0]);
	Иначе
		ПараметрыФормы.Вставить("ДействиеСТоваромНедостачи", Неопределено);
	КонецЕсли;
	
	ПараметрыФормы.Вставить("ЕстьИзлишки",   МассивДействийСДокументомПоИзлишкам.Количество() > 0);
	ПараметрыФормы.Вставить("ЕстьНедостачи", МассивДействийСДокументомПоНедостачам.Количество() > 0);
	ПараметрыФормы.Вставить("ЕстьТара",      ЕстьТара);
	
	ОткрытьФорму(
		"Обработка.РаботаСАктамиРасхождений.Форма.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки",
		ПараметрыФормы,
		,
		,
		,
...
Процедура ИзменитьДействиеВыделенныхСтрокВАктеПослеПриемкиПеремещения(Объект, Форма, ТипАкта, ПоказыватьПояснение = Ложь)
	...
	Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.АктОРасхожденияхПослеПеремещения") Тогда
		ИмяФормы = "Обработка.РаботаСАктамиРасхождений.Форма.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки";
		ИмяПараметраДействиеИзлишки = "ДействиеСДокументомИзлишки";
		ИмяПараметраДействиеНедостачи = "ДействиеСДокументомНедостачи";
	Иначе
		ИмяФормы = "Перечисление.ВариантыДействийПоРасхождениямВАктеПослеПриемки.Форма.ФормаВыбора";
		ПараметрыФормы.Вставить("ПоВинеСтороннейКомпании", ПоВинеСтороннейКомпании);
		ИмяПараметраДействиеИзлишки = "ДействиеИзлишки";
		ИмяПараметраДействиеНедостачи = "ДействиеНедостачи";
	КонецЕсли;
	
	Если МассивДействийПоИзлишкам.Количество() = 1 Тогда
		ПараметрыФормы.Вставить(ИмяПараметраДействиеИзлишки, МассивДействийПоИзлишкам[0]);
	Иначе
		ПараметрыФормы.Вставить(ИмяПараметраДействиеИзлишки, Неопределено);
	КонецЕсли;
	
	Если МассивДействийПоНедостачам.Количество() = 1 Тогда
		ПараметрыФормы.Вставить(ИмяПараметраДействиеНедостачи, МассивДействийПоНедостачам[0]);
	Иначе
		ПараметрыФормы.Вставить(ИмяПараметраДействиеНедостачи, Неопределено);
	КонецЕсли;
...
Процедура ТоварыУпаковкаПриИзменении(Форма, КэшированныеЗначения) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(Форма.Объект.Соглашение) Тогда
		Если Форма.Объект.ТипОснованияАктаОРасхождении = ПредопределенноеЗначение("Перечисление.ТипыОснованияАктаОРасхождении.ПриобретениеТоваровУслуг") Тогда
			СтруктураДействий.Вставить("ЗаполнитьУсловияЗакупок", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияЦеныЗакупкиВСтрокеТЧ(Форма.Объект));
		ИначеЕсли РасхожденияКлиентСервер.ТипОснованияРеализацияТоваровУслуг(Форма.Объект.ТипОснованияАктаОРасхождении)
				Или РасхожденияКлиентСервер.ТипОснованияОтгрузкаТоваровКлиенту(Форма.Объект.ТипОснованияАктаОРасхождении)
				Или РасхожденияКлиентСервер.ТипОснованияПередачаТоваровХранителю(Форма.Объект.ТипОснованияАктаОРасхождении)
			Тогда
			
			СтруктураДействий.Вставить("ЗаполнитьУсловияПродаж", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияУсловийПродажВСтрокеТЧ(Форма.Объект));
...
Процедура ПоказатьПредупреждениеДляЗаполненныхНаОснованииСтрок() Экспорт
	...
	ТекстСообщения = НСтр("ru = 'Для строк, заполненных по основанию, возможно редактирование колонок ""Фактическое количество"", ""Номер паспорта"".
		|После отражения расхождений по количеству становятся доступны для редактирования колонки ""Сумма (факт)"", ""НДС (факт)"" и ""Как отработать расхождения"".'");
...
Процедура ОбработкаКомандыОтчетаОформлениеДокументов(Форма, ОписаниеКоманды, АктОРасхождениях, ОснованиеАкта, ОформляемыйДокумент) Экспорт
	...
	ИначеЕсли ОписаниеКоманды = "Оформить" Тогда
		...
		ИначеЕсли ТипЗнч(ОформляемыйДокумент) = Тип("Структура")
			...
			И ТипЗнч(ОформляемыйДокумент.ОформляемыйДокумент) = Тип("ДокументСсылка.ПоступлениеТоваровОтКлиента") Тогда
			
			ПараметрыФормы.Вставить("АктОРасхождениях", АктОРасхождениях);
			ПараметрыФормы.Вставить("ОснованиеАкта", ОснованиеАкта);
			ПараметрыФормы.Вставить("УменьшитьОтгрузкуПоРаспоряжению", ОформляемыйДокумент.УменьшитьОтгрузкуПоРаспоряжению);
			
			ОткрытьФорму(
				"Документ.ПоступлениеТоваровОтКлиента.Форма.ФормаДокумента",
		...
		ИначеЕсли ТипЗнч(ОформляемыйДокумент) = Тип("Структура")
			И ОформляемыйДокумент.Свойство("ОформляемыйДокумент")
			И ТипЗнч(ОформляемыйДокумент.ОформляемыйДокумент) = Тип("ДокументСсылка.ОтгрузкаТоваровСХранения") Тогда
			
			Если ОформляемыйДокумент.Исправление Тогда
				ПараметрыФормы.Вставить("АктОРасхождениях", АктОРасхождениях);
				ПараметрыФормы.Вставить("ОснованиеАкта",    ОснованиеАкта);
				ПараметрыФормы.Вставить("Исправление",      Истина);
				ОткрытьФорму("Документ.ОтгрузкаТоваровСХранения.Форма.ФормаДокумента",
					Новый Структура("Основание", ПараметрыФормы), Форма);
			ИначеЕсли ОформляемыйДокумент.ЕстьИсправление
				И Не ЗначениеЗаполнено(ОформляемыйДокумент.Склад) Тогда
				ПараметрыФормы.Вставить("АктОРасхождениях",     АктОРасхождениях);
				ПараметрыФормы.Вставить("ОснованиеАкта",        ОснованиеАкта);
				ПараметрыФормы.Вставить("ИсправлениеОснования", ОформляемыйДокумент.ДокументИсправление);
				ОткрытьФорму("Документ.ОтгрузкаТоваровСХранения.Форма.ФормаДокумента",
					Новый Структура("Основание", ПараметрыФормы), Форма);
			ИначеЕсли ЗначениеЗаполнено(ОформляемыйДокумент.Склад) Тогда
				ПараметрыФормы.Вставить("АктОРасхождениях", АктОРасхождениях);
				ПараметрыФормы.Вставить("ОснованиеАкта",    ОснованиеАкта);
				ПараметрыФормы.Вставить("Склад",            ОформляемыйДокумент.Склад);
				Если ОформляемыйДокумент.ЕстьИсправление Тогда
					ПараметрыФормы.Вставить("ИсправлениеОснования", ОформляемыйДокумент.ДокументИсправление);
				КонецЕсли;
				ОткрытьФорму("Документ.ОтгрузкаТоваровСХранения.Форма.ФормаДокумента",
					Новый Структура("Основание", ПараметрыФормы), Форма);
		...
		ИначеЕсли ТипЗнч(ОформляемыйДокумент) = Тип("Структура")
			И ОформляемыйДокумент.Свойство("ОформляемыйДокумент")
			И ТипЗнч(ОформляемыйДокумент.ОформляемыйДокумент) = Тип("ДокументСсылка.ПоступлениеТоваровОтХранителя") Тогда
			
			ПараметрыФормы.Вставить("АктОРасхождениях", АктОРасхождениях);
			ПараметрыФормы.Вставить("ОснованиеАкта",    ОснованиеАкта);
			Если ОформляемыйДокумент.Исправление Тогда
				ПараметрыФормы.Вставить("Исправление",  Истина);
			ИначеЕсли ОформляемыйДокумент.ЕстьИсправление Тогда
				ПараметрыФормы.Вставить("ИсправлениеОснования", ОформляемыйДокумент.ДокументИсправление);
			КонецЕсли;
			Параметры = Новый Структура("Основание", ПараметрыФормы);
		...
		ИначеЕсли ТипЗнч(ОформляемыйДокумент) = Тип("Структура")
			И ОформляемыйДокумент.Свойство("ОформляемыйДокумент")
			И ТипЗнч(ОформляемыйДокумент.ОформляемыйДокумент) = Тип("ДокументСсылка.ПриемкаТоваровНаХранение") Тогда
		...
		ИначеЕсли ТипЗнч(ОформляемыйДокумент) = Тип("Структура")
			И ОформляемыйДокумент.Свойство("ОформляемыйДокумент")
			И ТипЗнч(ОформляемыйДокумент.ОформляемыйДокумент) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
		...
		ИначеЕсли ТипЗнч(ОформляемыйДокумент) = Тип("Структура")
			...
			ЭтоВозвратВПуть = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОформляемыйДокумент, "ЭтоВозвратВПуть", Ложь);
			...
			ПараметрыФормы.Вставить("ЭтоВозвратВПуть",  ЭтоВозвратВПуть);
		...
		ИначеЕсли ТипЗнч(ОформляемыйДокумент) = Тип("Структура")
			...
			И ТипЗнч(ОформляемыйДокумент.ИсправляемыйДокумент) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда
			
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("АктОРасхождениях", АктОРасхождениях);
			ПараметрыФормы.Вставить("ОснованиеАкта",    ОснованиеАкта);
			ПараметрыФормы.Вставить("Исправление",      ОформляемыйДокумент.Исправление);
			ПараметрыФормы.Вставить("Допоставка",       ОформляемыйДокумент.Допоставка);
			
			ПараметрыОткрытияФормы = Новый Структура("Основание", ПараметрыФормы);
			
			ОткрытьФорму("Документ.ОтгрузкаТоваровКлиенту.Форма.ФормаДокумента", ПараметрыОткрытияФормы);
		...
		ИначеЕсли ТипЗнч(ОформляемыйДокумент) = Тип("ДокументСсылка.ОприходованиеИзлишковОтгруженныхТоваров") Тогда
			
			ПараметрыФормы.Вставить("АктОРасхождениях", АктОРасхождениях);
			ПараметрыФормы.Вставить("ОснованиеАкта", ОснованиеАкта);
			ОткрытьФорму(
				"Документ.ОприходованиеИзлишковОтгруженныхТоваров.Форма.ФормаДокумента",
				Новый Структура("Основание", ПараметрыФормы),
				Форма);
			
		ИначеЕсли ТипЗнч(ОформляемыйДокумент) = Тип("Структура")
			И ОформляемыйДокумент.Свойство("ОформляемыйДокумент")
			И ТипЗнч(ОформляемыйДокумент.ОформляемыйДокумент) = Тип("ДокументСсылка.СписаниеОтгруженныхТоваров") Тогда
			
			ПараметрыФормы.Вставить("АктОРасхождениях", АктОРасхождениях);
			ПараметрыФормы.Вставить("ОснованиеАкта", ОснованиеАкта);
			ПараметрыФормы.Вставить("УменьшитьОтгрузкуПоРаспоряжению",
				ОформляемыйДокумент.УменьшитьОтгрузкуПоРаспоряжению);
			
			ОткрытьФорму(
				"Документ.СписаниеОтгруженныхТоваров.Форма.ФормаДокумента",
				Новый Структура("Основание", ПараметрыФормы),
				Форма);
