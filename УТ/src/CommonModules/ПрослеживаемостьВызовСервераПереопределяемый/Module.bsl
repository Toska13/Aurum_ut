...
//
Функция РаспоряжениеКОформлениюУведомленияОВвозеПрослеживаемыхТоваровПоОснованию(ПервичныйДокумент, КорректировочноеУведомление = Ложь) Экспорт
	
	Результат = Неопределено;
	Ссылка = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПервичныйДокумент, "Ссылка");
	ТаблицаРаспоряжений = Документы.УведомлениеОВвозеПрослеживаемыхТоваров.РаспоряженияКОформлениюУведомленийОВвозеПоОснованию(Ссылка, КорректировочноеУведомление);
	Если ТаблицаРаспоряжений.Количество() Тогда
		...
		КонецЦикла;  
		Результат.Вставить("ИсправляемыйДокумент", Документы.УведомлениеОВвозеПрослеживаемыхТоваров.ПустаяСсылка());
		Результат.Вставить("КорректировочноеУведомление", Ложь);
		Результат.Вставить("НомерКорректировки", 0);
		Если КорректировочноеУведомление Тогда
			ТаблицаУведомленийОВвозе = Документы.УведомлениеОВвозеПрослеживаемыхТоваров.УведомленияОВвозеПрослеживаемыхТоваровПоОснованию(Ссылка, Истина, Результат.КодТНВЭД); 
			Если ТаблицаУведомленийОВвозе.Количество() Тогда
				ТаблицаУведомленийОВвозе.Сортировать("НомерКорректировки Убыв");
				Результат.ИсправляемыйДокумент = ТаблицаУведомленийОВвозе[0].Ссылка;
				Результат.НомерКорректировки = ТаблицаУведомленийОВвозе[0].НомерКорректировки + 1; 
				Результат.КорректировочноеУведомление = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
