...
//			* АналитикиУчетаПоПартнерам - Массив - Содержит массив ключей аналитик по партнерам.
//			* Организации - Массив - Содержит массив организаций.
//		ПроверятьНачалоПереходаНаНовуюАрхитектуруВзаиморасчетов - Булево - Признак того, что переход на новую архитектуру взаиморасчетов уже начат.
//
Процедура РаспределитьВсеРасчетыСКлиентами(ОкончаниеПериодаРасчета, АналитикиРасчета = Неопределено, ПроверятьНачалоПереходаНаНовуюАрхитектуруВзаиморасчетов = Истина) Экспорт
	
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		Возврат;
	КонецЕсли;
	
	Если ПроверятьНачалоПереходаНаНовуюАрхитектуруВзаиморасчетов
		И Константы.НачатПереходНаНовуюАрхитектуруВзаиморасчетов.Получить() Тогда
		ВызватьИсключение НСтр("ru = 'Начат переход на новую архитектуру взаиморасчетов. 
						|Распределение расчетов с клиентами не может быть выполнено до окончания перехода.'");
	КонецЕсли;
	...
	Если НЕ Константы.НачатПереходНаНовуюАрхитектуруВзаиморасчетов.Получить() Тогда
...
//		ПроверятьНачалоПереходаНаНовуюАрхитектуруВзаиморасчетов - Булево - Признак того, что переход на новую архитектуру взаиморасчетов уже начат.
//
Процедура РаспределитьВсеРасчетыСПоставщиками(ОкончаниеПериодаРасчета, АналитикиРасчета = Неопределено, ПроверятьНачалоПереходаНаНовуюАрхитектуруВзаиморасчетов = Истина) Экспорт
	...
	Если ПроверятьНачалоПереходаНаНовуюАрхитектуруВзаиморасчетов 
		И Константы.НачатПереходНаНовуюАрхитектуруВзаиморасчетов.Получить() Тогда
		ВызватьИсключение НСтр("ru = 'Начат переход на новую архитектуру взаиморасчетов. 
						|Распределение расчетов с поставщиками не может быть выполнено до окончания перехода.'");
	КонецЕсли;
...
Процедура ЗаписатьРасчетныеПартии(МенеджерРегистра, РасчетныеПартии, Регистраторы)
	...
	Если Не РасчетныеПартии.Количество() = 0 Тогда
		...
		Если Константы.НачатПереходНаНовуюАрхитектуруВзаиморасчетов.Получить() Тогда
			Движения.ДополнительныеСвойства.Вставить("ПропуститьПроверкуЗапретаИзменения", Истина);
		КонецЕсли;
