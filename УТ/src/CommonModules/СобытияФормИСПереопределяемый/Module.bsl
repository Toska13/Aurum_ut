...
Процедура ПриОпределенииПараметровИнтеграцииФормыПрикладногоОбъекта(Форма, МодулиИнтеграции) Экспорт
	...
	Если ИмяФормы = "Документ.ЧекККМВозврат.Форма.ФормаДокументаРМК"
		...
		Или ИмяФормы = "Обработка.ПодборТоваровВДокументПродажи.Форма.Форма"
		Или ИмяФормы = "Документ.РасходныйОрдерНаТовары.Форма.ФормаДокумента"
		Или ИмяФормы = "Документ.ОтборРазмещениеТоваров.Форма.ФормаДокумента"
		Или ИмяФормы = "Документ.ПриходныйОрдерНаТовары.Форма.ФормаДокумента"
		...
		Или ИмяФормы = "Документ.ОтгрузкаТоваровКлиенту.Форма.ФормаДокумента"
		Или ((ИмяФормы = "Документ.ПоступлениеТоваровОтХранителя.Форма.ФормаДокумента"
			Или ИмяФормы = "Документ.ПриемкаТоваровНаХранение.Форма.ФормаДокумента"
			Или ИмяФормы = "Документ.ПоступлениеТоваровОтКлиента.Форма.ФормаДокумента")
				И Не ЗначениеЗаполнено(Форма.Объект.ИсправляемыйДокумент)) Тогда
...
Процедура ПриИзмененииЭлемента(Форма, Элемент, ДополнительныеПараметры) Экспорт
	...
	//++ НЕ ГОСИС
	Если Элемент = "ХозяйственнаяОперация" Тогда
		ИнтеграцияИСУТ.ОбновитьИнтерфейсРеализацияТоваровУслугПоОперации(Форма);
	КонецЕсли;
	...
	Если Форма.ИмяФормы = "Документ.ЧекККМ.Форма.ФормаДокументаРМК"
		...
		Если Элемент = "Товары" Тогда
			...
			Если Форма.ИмяФормы = "Документ.ЧекККМ.Форма.ФормаДокументаРМК" Тогда
				РозничныеПродажиЛокализация.ОбновитьСтатусыИРазрешеннияТоваров(Форма.Объект);
			КонецЕсли;
		...
		ИначеЕсли Элемент = "Серии" Тогда
			...
			ДокументОбъект = Форма.Объект; // ДокументОбъект
			
			ИнтеграцияЕГАИСУТ.ЗаполнитьАлкогольнуюПродукцию(ДокументОбъект.Товары, ДокументОбъект.Серии);
			ПроверкаИПодборПродукцииИС.ПрименитьКешШтрихкодовУпаковок(Форма, ПроверкаИПодборПродукцииИСМПУТ.НастройкиИсточникаКешаЧека(), Истина);
...
Процедура ПослеЗаписиНаСервереФормыПрикладногоОбъекта(Форма) Экспорт
	...
	ИначеЕсли Форма.ИмяФормы = "Документ.ЗаказКлиента.Форма.ФормаДокумента" Тогда
		ИнтеграцияИСУТ.МодифицироватьИнициализироватьФормуЗаказКлиента(Форма);
	КонецЕсли;
	//-- НЕ ГОСИС
	Возврат;
...
Процедура ПриСозданииНаСервереВФормеПрикладногоОбъекта(Форма, Отказ, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	Если Форма.ИмяФормы = "Справочник.ШаблоныЭтикетокИЦенников.Форма.ПомощникНового" Тогда
		...
		СписокВыбора = Форма.Элементы.Назначение.СписокВыбора; // СписокЗначений
		
		СписокВыбора.Добавить(Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляШтрихкодовУпаковок);
		Если Форма.ПараметрыИнтеграцииГосИС.Получить("ЕГАИС")<>Неопределено Тогда
			СписокВыбора.Добавить(Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляАкцизныхМарок);
		КонецЕсли;
		Если ПолучитьФункциональнуюОпцию("ВестиУчетМаркируемойПродукцииИСМП") Тогда
			СписокВыбора.Добавить(Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаКодМаркировкиИСМП);
		КонецЕсли;
	...
	ИначеЕсли Форма.ИмяФормы = "Документ.ЗаявкаНаВозвратТоваровОтКлиента.Форма.ФормаДокумента" Тогда
		РаботаСТабличнымиЧастями.ИнициализироватьКэшСтрок(Форма.Элементы.ЗаменяющиеТовары);
	ИначеЕсли Форма.ИмяФормы = "Обработка.ПодборТоваровВДокументПродажи.Форма.Форма" Тогда
		РаботаСТабличнымиЧастями.ИнициализироватьКэшСтрок(Форма.Элементы.Корзина);
...
//  ПараметрыСканирования  - Структура, Неопределено - параметры сканирования формы
Процедура ПриИзмененииНоменклатуры(Форма, ТекущаяСтрока, КэшированныеЗначения = Неопределено, ПараметрыУказанияСерий = Неопределено, ПараметрыСканирования = Неопределено) Экспорт
	...
	Если ПараметрыСканирования <> Неопределено Тогда
		ЕГАИС = ПараметрыСканирования.ЕГАИС;
		ИСМП = ПараметрыСканирования.ИСМП;
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПараметрыСканирования") Тогда
		ЕГАИС = Форма.ПараметрыСканирования.ЕГАИС;
		ИСМП = Форма.ПараметрыСканирования.ИСМП;
	КонецЕсли;
	...
	Если ИнтеграцияИСУТ.ЕстьРеквизит(ТекущаяСтрока, "Серия", ПроверятьОбъект) Тогда
		...
		Если ПараметрыУказанияСерий <> Неопределено Тогда
			...
			Если Форма.ИмяФормы = "ОбщаяФорма.УточнениеСоставаУпаковкиИС" Тогда
				Склад = Форма.Склад;
			ИначеЕсли СтрНачинаетсяС(Форма.ИмяФормы, "Обработка.ПроверкаИПодбор")
				Или Форма.ИмяФормы = "ОбщаяФорма.ФормаУточненияДанныхИС" Тогда
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущаяСтрока, "ВидПродукции") Тогда
		ДанныеТекущейСтроки.Вставить("ВидПродукции");
	...
	Если ПроверятьОбъект Тогда
		...
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ДанныеТекущейСтроки, СтруктураДействий, КэшированныеЗначения);
