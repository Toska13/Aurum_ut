...
Процедура ЗаполнитьПоДокументу(Форма, ТаблицаДокументы, ОтображатьСИерархией = Ложь) Экспорт
	...
	|	&ТоварнаяГруппаЗаказНаЭмиссию                              КАК ТоварнаяГруппа,
	...
	|	ОчередьСообщенийИСМП.ТоварнаяГруппа КАК ТоварнаяГруппа,
	...
	|	ВЫБОР
	|		КОГДА ОчередьСообщенийИСМП.ТоварнаяГруппа В (&ТоварныеГруппыМЕХ)
	...
	|	ЕСТЬNULL(Сообщения.ТоварнаяГруппа, НЕОПРЕДЕЛЕНО)                     КАК ТоварнаяГруппа,
	...
	|	МАКСИМУМ(ТоварнаяГруппа)
	|ПО
	|	Документ
	|");
	
	Если ОбщегоНазначенияИСМП.ЭтоРасширеннаяВерсияГосИС() Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТоварнаяГруппаЗаказНаЭмиссию", "ЗаказНаЭмиссиюКодовМаркировкиСУЗ.ТоварнаяГруппа");
	...
	Иначе
		Запрос.УстановитьПараметр("ТоварнаяГруппаЗаказНаЭмиссию", Справочники.ТоварныеГруппыГИСМТ.ПустаяСсылка());
	КонецЕсли;
	...
	ТоварныеГруппыМЕХ = Новый Массив;
	ВидыПродукцииМех  = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииМЕХ();
	
	Для каждого ВидПродукцииМЕХ Из ВидыПродукцииМех Цикл
		ТоварныеГруппыМЕХ.Добавить(КонструкторИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппаПоВидуПродукции(ВидПродукцииМЕХ));
	КонецЦикла;
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Пока ВыборкаПоФайлам.Следующий() Цикл
			...
			Если ТипЗнч(ВыборкаПоФайлам.Ссылка) = Тип("Строка") Тогда
				...
				РеквизитыИсходящегоСообщения.Вставить("ТоварнаяГруппа", ВыборкаПоФайлам.ТоварнаяГруппа);
...
Процедура ДополнитьПредставлениеОперации(СтрокаВторогоУровня, ПараметрыОбновления)
	...
	Если Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ПараметрыОбновления.ТоварнаяГруппа)
	...
	ИначеЕсли ТипЗнч(ПараметрыОбновления.Ссылка) <> Тип("Структура") Тогда
		ТекстСообщения = ПротоколОбменаИС.ТекстСообщенияXMLИзПротокола(ПараметрыОбновления.Ссылка);
		ЗаменитьИменаУстаревшихПеречисленийВТекстеПротоколаОбмнена(ТекстСообщения);
		ПротоколОбмена = ОбщегоНазначения.ЗначениеИзСтрокиXML(ТекстСообщения);
	КонецЕсли;
...
Процедура ЗаменитьИменаУстаревшихПеречисленийВТекстеПротоколаОбмнена(ТекстСообщения) Экспорт
	
	ЗначенияЗамены = Новый Соответствие;
	Для Каждого Значение Из Перечисления.ВидыОперацийИСМП Цикл
		ИмяПеречисления = XMLСтрока(Значение);
		ЗначенияЗамены.Вставить(СтрЗаменить(ИмяПеречисления, "Удалить", ""), ИмяПеречисления);
	КонецЦикла;
	
	ШаблонЗамены = "<Value>%1</Value>";
	Для Каждого КлючИЗначение Из ЗначенияЗамены Цикл
		ЧтоЗаменить   = СтрШаблон(ШаблонЗамены, КлючИЗначение.Ключ);
		НаЧтоЗаменить = СтрШаблон(ШаблонЗамены, КлючИЗначение.Значение);
		ТекстСообщения = СтрЗаменить(ТекстСообщения, ЧтоЗаменить, НаЧтоЗаменить);
	КонецЦикла;
