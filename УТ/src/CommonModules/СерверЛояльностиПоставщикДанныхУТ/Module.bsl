...
//
// Параметры:
//  Результат - см. СерверЛояльностиПоставщикДанныхПереопределяемый.ЗаполнитьТаблицуУправляемыхСкидок.Результат
//  ПараметрыРасчета - см. СерверЛояльностиПоставщикДанныхПереопределяемый.ЗаполнитьТаблицуУправляемыхСкидок.ПараметрыРасчета
//
Процедура ЗаполнитьТаблицуУправляемыхСкидок(Результат, Знач ПараметрыРасчета) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	НастройкаОтборов = ЗаполнитьТаблицуУправляемыхСкидокПараметрыРасчета(ПараметрыРасчета);
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрСведений.ДействиеСкидокНаценок.СрезПоследних(&СкидкиНаДату, СкидкаНаценка.Управляемая) КАК ДействиеСкидокНаценокСрезПоследних
		|ГДЕ
		|	ДействиеСкидокНаценокСрезПоследних.Статус = &СтатусДействует
		|";
	
	Запрос.Параметры.Вставить("Магазин", НастройкаОтборов.Магазин);
...
//  ДисконтнаяКарта - ОпределяемыйТип.КартаЛояльностиСерверЛояльности - 
//                    карта лояльности, по которой будут заполнены данные о бонусах
//  СтруктураОтвета - см. СерверЛояльностиПоставщикДанныхПереопределяемый.ЗаполнитьДанныеОБонусах.СтруктураОтвета
//
...
Процедура НайтиКартуКлиента(Данные, Результат = Неопределено) Экспорт
	...
	Если Данные.Свойство("ГУИДВидаКарты") И ЗначениеЗаполнено(Данные.ГУИДВидаКарты) Тогда
...
// Параметры:
//  ДисконтнаяКарта - ОпределяемыйТип.КартаЛояльностиСерверЛояльности
//  СтруктураОтвета - см. СерверЛояльностиПоставщикДанныхПереопределяемый.ЗаполнитьСтруктуруОтветаДаннымиКарты.СтруктураОтвета
...
//  СтруктураОтвета - см. СерверЛояльностиПоставщикДанныхПереопределяемый.ЗаполнитьСтруктуруОтветаДаннымиКлиента.СтруктураОтвета
//
...
Процедура РассчитатьАвтоматическиеСкидки(ПараметрыРасчета, Результат) Экспорт
	...
	ЭтоВозврат = ПараметрыРасчета.Свойство("ЭтоВозврат") И ПараметрыРасчета.ЭтоВозврат = Истина;
	...
	Если ЭтоВозврат Тогда
		...
		ВходныеПараметры.Вставить("ТолькоПредварительныйРасчет", Истина);
	КонецЕсли;
...
Процедура ДесериализоватьПараметрыРасчета(ВходящиеДанные, Результат) Экспорт
	...
	Попытка
		...
		Если ПроверитьКорректностьВходящихДанных(ВходящиеДанныеВJSON, ПараметрыРасчета) Тогда
			...
			ЭтоВозврат = Ложь;
			ВходящиеДанныеВJSON.Свойство("Return", ЭтоВозврат);
			ПараметрыРасчета.Вставить("ЭтоВозврат", ЭтоВозврат);
...
//
// Параметры:
//  ИменаРеквизитов - Структура - определение в см. СерверЛояльностиПоставщикДанных.ЗаполнитьПустуюСтруктуруИменаРеквизитовМетаданных.ИменаРеквизитов:
...
Процедура ЗаполнитьИменаРеквизитовПоставщикаДанных(ИменаРеквизитов) Экспорт
	...
	ИменаРеквизитов.ИмяРеквизитаДисконтнаяКарта = "КартаЛояльности";
	ИменаРеквизитов.ИмяРеквизитаТаблицаОплата = "Товары";
	ИменаРеквизитов.ИмяРеквизитаСуммаБонусов = "СуммаБонусныхБалловКСписаниюВВалюте";
...
//  СтруктураОтвета - см. СерверЛояльностиПоставщикДанныхПереопределяемый.НайтиКартыЛояльности.СтруктураОтвета
//
...
//  СтруктураОтвета - см. СерверЛояльностиПоставщикДанныхПереопределяемый.ЗаполнитьСтруктуруОтветаНомерамиКарты.СтруктураОтвета
//
...
//  ПараметрыРасчета - см. СерверЛояльностиПоставщикДанныхПереопределяемый.ЗаполнитьТаблицуВидовКарт.ПараметрыРасчета
//
Процедура ЗаполнитьТаблицуВидовКарт(Результат, ПараметрыРасчета = Неопределено) Экспорт
...
//  ДисконтнаяКарта - ОпределяемыйТип.КартаЛояльностиСерверЛояльности - 
//                    карта лояльности, по которой будут заполнены данные о бонусах
//  СтруктураОтвета - см. СерверЛояльностиПоставщикДанныхПереопределяемый.ЗаполнитьДанныеОбОборотахПродаж.СтруктураОтвета
//
...
// Контроль полученных данных карты клиента
// Параметры:
//	ПараметрыМетода - см. СерверЛояльностиПоставщикДанных.ПараметрыДанныхКартыКлиента
//	СтруктураОтвета - см. СерверЛояльностиПоставщикДанныхПереопределяемый.ПроверитьДанныеКартыКлиента.СтруктураОтвета
Процедура ПроверитьДанныеКартыКлиента(ПараметрыМетода, СтруктураОтвета) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	ИспользоватьКартыЛояльности = Константы.ИспользоватьКартыЛояльности.Получить();
	УстановитьПривилегированныйРежим(Ложь);

	Если НЕ ИспользоватьКартыЛояльности Тогда
		СтруктураОтвета.Error = Истина;
		СтруктураОтвета.ErrorMessage = НСтр("ru = 'Карты лояльности не поддерживаются.'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
...
//                                см. СерверЛояльностиПоставщикДанныхПереопределяемый.ПолучитьОстатки.СтруктураОтвета
//
...
//   *Свободно - Число -
//
Функция ТаблицаОстатковПоПараметрам(ПараметрыПолученияОстатков)
	...
	ТекстЗапроса = 
		...
		|//////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЕСТЬNULL(ПРЕДСТАВЛЕНИЕССЫЛКИ(Расчет.Склад), """") КАК МестоХранения,
		|	ЕСТЬNULL(ПРЕДСТАВЛЕНИЕССЫЛКИ(Расчет.Характеристика), """") КАК Характеристика,
		|	Расчет.КоличествоБезКонтроляРезервовДляДействияОтгрузить
		|		/ ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки, 1) КАК Остаток,
		|	Расчет.КоличествоДляДействияОтгрузить
		|		/ ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки, 1) КАК Свободно,
		|	(Расчет.КоличествоБезКонтроляРезервовДляДействияОтгрузить - Расчет.КоличествоДляДействияОтгрузить)
		|		/ ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки, 1) КАК ВРезерве
		|ИЗ
	...
	ТекстЗапросаКоэффициентУпаковки = Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки(
		"ВЫРАЗИТЬ(&Упаковка КАК Справочник.УпаковкиЕдиницыИзмерения)", // @query-part-1
		"Расчет.Номенклатура"); // @query-part-1
...
Функция СериализоватьРезультатРасчета(Объект)
	...
	Для каждого СтрокаТовар Из Объект.Товары Цикл
		...
		ЭтоВозврат = ТипЗнч(Объект) = Тип("ДокументОбъект.ЧекККМВозврат");
		Если ЭтоВозврат Тогда
			СтруктураСтроки.Вставить("AmountOfAutomaticDiscount", 0);
			СтруктураСтроки.Вставить("AmountOfManualDiscount",  0);
			СтруктураСтроки.Вставить("PercentOfAutomaticDiscount",  0);
			СтруктураСтроки.Вставить("PercentOfManualDiscount", 0);
		Иначе
...
// Параметры:
// КартаКлиента - см. ПолучитьКартуПоУникальномуИдентификатору
// СтруктураОтвета - см. НайтиКартыЛояльности.СтруктураОтвета
...
// Параметры:
//	ЗапросКарт - Запрос
//	СтруктураОтвета - см. НайтиКартуЛояльностиПоСсылке.СтруктураОтвета 
...
Процедура ДополнитьСтруктуруСкидокСообщениями(СериализованнаяСтруктураСкидок, ЧекККМОбъект)
	...
	Для каждого СтрокаСообщения Из СтруктураСообщений.Сообщения Цикл
		...
		СтруктураСтроки.Вставить("MessageText", 
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаСообщения.СкидкаНаценка, "ТекстСообщения"));
...
Процедура ДополнитьСтруктуруСкидокТаблицейБонусов(СериализованнаяСтруктураСкидок, РезультатРасчета, ЧекККМОбъект)
	...
	Для каждого ЭлементБонус Из СкидкиНаценки Цикл
		...
		СтруктураСтроки.Вставить("LineUID",       ЭлементБонус.ИдентификаторСтроки);
...
// Параметры:
//	ВходящиеПараметры - см. ЗаполнитьТаблицуУправляемыхСкидок.ПараметрыРасчета
// Возвращаемое значение:
//	Структура:
//	*ИспользоватьОтборы - Булево - При Ложь остальные свойства рассмотрению не подлежат
//	*Магазин - СправочникСсылка.Склады - Склад для которого предназначены скидки
//	*Клиент - Неопределено - не используется в текущей версии
//	*МаксимальныйПроцентНаценки - Число - 
//	*МаксимальныйПроцентСкидки - Число - 
//	*КартаЛояльности - СправочникСсылка.КартыЛояльности - 
Функция ЗаполнитьТаблицуУправляемыхСкидокПараметрыРасчета(Знач ВходящиеПараметры = Неопределено)
	
	Результат = Новый Структура;
	Результат.Вставить("ИспользоватьОтборы", Ложь);
	Результат.Вставить("Магазин", ПредопределенноеЗначение("Справочник.Склады.ПустаяСсылка"));
	Результат.Вставить("Клиент", Неопределено);
	Результат.Вставить("МаксимальныйПроцентНаценки", 0);
	Результат.Вставить("МаксимальныйПроцентСкидки", 0);
	//Результат.Вставить("ИдентификаторКартыКлиента", "");
	//Результат.Вставить("УникальныйИдентификаторКарты", "");
	...
	Если ТипЗнч(ВходящиеПараметры) = Тип("Структура") Тогда
		СвойствоИмя = "ИспользоватьОтборы";
		Если ВходящиеПараметры.Свойство(СвойствоИмя) И ВходящиеПараметры[СвойствоИмя] = Истина Тогда
			Результат[СвойствоИмя] = Истина;
		Иначе
			Результат[СвойствоИмя] = Ложь;
		...
		Если ВходящиеПараметры.Свойство(СвойствоИмя, ЗначениеСвойства) Тогда
			Попытка
				СсылкаИД = Новый УникальныйИдентификатор(ЗначениеСвойства);
				МагазинСсылка =  Справочники.Склады.ПолучитьСсылку(СсылкаИД);
				Результат[СвойствоИмя] = МагазинСсылка;
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ШаблонОписанияОшибки = НСтр(
					"ru='Ошибка получения элемента %1 по идентификатору(%2):
				...
				ЗаписьЖурналаРегистрации(
					"СерверЛояльностиПоставщикДанныхУТ.ЗаполнитьТаблицуУправляемыхСкидокПараметрыРасчета",
					УровеньЖурналаРегистрации.Ошибка,,,
					Описание);
			КонецПопытки;
		КонецЕсли;
		
		СвойствоИмя = "КартаЛояльности";
		ЗначениеСвойства = Неопределено;
		ИскатьКартуПоНомеру = Ложь;
		Если ВходящиеПараметры.Свойство("УникальныйИдентификаторКарты", ЗначениеСвойства)
			И ЗначениеЗаполнено(ЗначениеСвойства) Тогда
			Попытка
				СсылкаИД = Новый УникальныйИдентификатор(ЗначениеСвойства);
				КартаЛояльностиСсылка =  Справочники.КартыЛояльности.ПолучитьСсылку(СсылкаИД);
				Результат[СвойствоИмя] = КартаЛояльностиСсылка;
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ШаблонОписанияОшибки = НСтр(
					"ru='Ошибка получения элемента %1 по идентификатору(%2):
				...
				ЗаписьЖурналаРегистрации(
					...
					Описание);
			КонецПопытки;
		ИначеЕсли ВходящиеПараметры.Свойство("ИдентификаторКартыКлиента", ЗначениеСвойства) Тогда
			ИскатьКартуПоНомеру = Истина;
		КонецЕсли;
		Если ИскатьКартуПоНомеру Тогда
			Запрос = Новый Запрос;
			Запрос.Параметры.Вставить("НомерКарты", ЗначениеСвойства);
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	МАКСИМУМ(КартыЛояльности.Ссылка) КАК Ссылка,
			|	КартыЛояльности.Штрихкод КАК Штрихкод,
			|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ КартыЛояльности.Ссылка) КАК ВсегоПоНомеру
			|ИЗ
			|	Справочник.КартыЛояльности КАК КартыЛояльности
			|ГДЕ
			|	КартыЛояльности.Штрихкод = &НомерКарты
			|
			|СГРУППИРОВАТЬ ПО
			|	КартыЛояльности.Штрихкод";
			Выборка = Запрос.Выполнить().Выбрать();
			Если Выборка.Следующий() Тогда
				Результат[СвойствоИмя] = Выборка.Ссылка;
			КонецЕсли;
		КонецЕсли;
