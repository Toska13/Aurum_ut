...
// Возвращаемое значение:
//  Структура - Структура со свойствами:
//   * ТекстНадписи - Строка - Представление документа.
...
Функция ДанныеДокументаЕГАИС(МетаданныеДокумента, ДокументыПоОснованию, СтатусыОформления) Экспорт
	...
	ВозвращаемоеЗначение = Новый Структура;
	ВозвращаемоеЗначение.Вставить("ТекстНадписи",        Новый ФорматированнаяСтрока(ФорматированныеСтроки));
	...
	ВозвращаемоеЗначение.Вставить("ПолноеИмяМетаданных", МетаданныеДокумента.ПолноеИмя());
	ВозвращаемоеЗначение.Вставить("ИмяКоманды",          ИмяКоманды);
	...
	Для Каждого КлючИЗначение Из СтатусыОформления Цикл
		...
		Если КлючИЗначение.Ключ = Метаданные.Документы.АктПостановкиНаБалансЕГАИС.Имя Тогда
			...
			Если Метаданные.ОпределяемыеТипы.ОснованиеАктаПостановкиНаБалансЕГАИС.Тип.СодержитТип(ТипДокумента) Тогда
				...
				ИначеЕсли Строка(Данные.ТекстНадписи) = ПредставлениеДокумента(МетаданныеПостановкиНаБаланс).КомандаСоздать Тогда
		...
		Если КлючИЗначение.Ключ = Метаданные.Документы.ПередачаВРегистр2ЕГАИС.Имя Тогда
			...
			Если Метаданные.ОпределяемыеТипы.ОснованиеПередачиВРегистр2ЕГАИС.Тип.СодержитТип(ТипДокумента) Тогда
				...
				ИначеЕсли Строка(Данные.ТекстНадписи) = ПредставлениеДокумента(МетаданныеПередачаВРегистр2).КомандаСоздать Тогда
...
Процедура ПроверитьЗаполнениеШтрихкодовУпаковок(Объект, Отказ) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ВидыПродукции, КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииИСМПЕГАИС());
	
	ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(Объект, Неопределено, ВидыПродукции);
	
	СписокУпаковок = Объект.ШтрихкодыУпаковок.ВыгрузитьКолонку("ШтрихкодУпаковки");
	
	Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияИСМПСлужебный");
...
// Проверяет производителей продукции в документе согласно данным ЕГАИС.
//
// Параметры:
//   ДокументОбъект - ДокументОбъект - Объект документа, содержащего информацию о продукции
//   Отказ - Булево - Флаг отказа от дальнейшей обработки при обнаружении несоответствий
Процедура ПроверитьПроизводителейПродукции(ДокументОбъект, Отказ) Экспорт

	Запрос = Новый Запрос;

	Запрос.Текст =
	"ВЫБРАТЬ
	|	УведомлениеОПланируемомИмпортеЕГАИСТовары.НомерСтроки КАК НомерСтроки,
	|	УведомлениеОПланируемомИмпортеЕГАИСТовары.АлкогольнаяПродукция КАК АлкогольнаяПродукция
	|ПОМЕСТИТЬ Товары
	|ИЗ
	|	&Товары КАК УведомлениеОПланируемомИмпортеЕГАИСТовары
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	КлассификаторАлкогольнойПродукцииЕГАИС.Представление КАК АлкогольнаяПродукцияПредставление,
	|	КлассификаторАлкогольнойПродукцииЕГАИС.Производитель.Представление КАК ПроизводительПредставление,
	|	КлассификаторАлкогольнойПродукцииЕГАИС.Производитель.ТипОрганизации КАК ПроизводительТипОрганизации
	|ИЗ
	|	Товары КАК Товары
	...
	|ГДЕ
	|	КлассификаторАлкогольнойПродукцииЕГАИС.Производитель.ТипОрганизации = &ИндивидуальныйПредпринимательРФ
	|	ИЛИ КлассификаторАлкогольнойПродукцииЕГАИС.Производитель.ТипОрганизации = &ЮридическоеЛицоРФ";

	Запрос.УстановитьПараметр("Товары", ДокументОбъект.Товары.Выгрузить(, "НомерСтроки, АлкогольнаяПродукция"));
	Запрос.УстановитьПараметр("ИндивидуальныйПредпринимательРФ", Перечисления.ТипыОрганизацийЕГАИС.ИндивидуальныйПредпринимательРФ);
	Запрос.УстановитьПараметр("ЮридическоеЛицоРФ", Перечисления.ТипыОрганизацийЕГАИС.ЮридическоеЛицоРФ);
	...
	ШаблонОшибки = НСтр("ru = 'В строке %1 алкогольная продукция ""%2"", производимая в РФ не может быть импортирована.'");
	
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
		ТекстОшибки = СтрШаблон(ШаблонОшибки,
				Выборка.НомерСтроки, Выборка.АлкогольнаяПродукцияПредставление);
		
		ПутьКТабличнойЧасти = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", Выборка.НомерСтроки, "АлкогольнаяПродукция");
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, ДокументОбъект, ПутьКТабличнойЧасти,, Отказ);
	КонецЦикла;
...
Функция НайтиОбъектПоИдентификаторуЗапроса(ИдентификаторЗапроса, ИскатьДокументОснование = Истина) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		ВозвращаемоеЗначение = СтруктураРезультатПоискаОбъекта();
		
		ВозвращаемоеЗначение.Ссылка				= Выборка.Ссылка;
		ВозвращаемоеЗначение.ДокументОснование	= ДокументОснование;
		ВозвращаемоеЗначение.Операция			= Выборка.Операция;
		ВозвращаемоеЗначение.ИсходящееСообщение	= Выборка.ИсходящееСообщение;
...
Функция ЗагрузитьОрганизацию(ДанныеОрганизацииXDTO, ЗагрузкаИзКлассификатора = Ложь, Кэш = Неопределено) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеОрганизацииXDTO, "VersionWB")
		И ВРег(ДанныеОрганизацииXDTO.VersionWB) = ВРег("WayBill_v3") Тогда
...
Функция ЗагрузитьОтветНаЗапросОстатковВРегистре1(ДанныеДокумента, ОрганизацияЕГАИС, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Иначе
		Операция = Перечисления.ВидыДокументовЕГАИС.ЗапросОстатковВРегистре1;
		ДальнейшееДействие = Перечисления.ДальнейшиеДействияПоВзаимодействиюЕГАИС.ОжидайтеПолучениеОстатков;
		Статус = Перечисления.СтатусыОбработкиОстатковЕГАИС.ПереданВУТМ;
		
		РезультатПоиска = НайтиОбъектОжидающийОтвета("ОстаткиЕГАИС", Статус, Операция, ДальнейшееДействие,
			ОрганизацияЕГАИС, ДанныеДокумента.Объект.RestsDate, ДанныеДокумента.ФорматОбмена);
	КонецЕсли;
...
Функция ЗагрузитьОтветНаЗапросОтчетаОстаткиВРегистре1(ДанныеДокумента, ОрганизацияЕГАИС, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеДокумента.ИдентификаторЗапроса) Тогда
		РезультатПоиска = НайтиОбъектПоИдентификаторуЗапроса(
			ДанныеДокумента.ИдентификаторЗапроса, Ложь);
	Иначе
		Операция = Перечисления.ВидыДокументовЕГАИС.ЗапросОтчетаОстаткиВРегистре1;
		ДальнейшееДействие = Перечисления.ДальнейшиеДействияПоВзаимодействиюЕГАИС.ОжидайтеПолучениеОтчета;
		Статус = Перечисления.СтатусыОбработкиОтчетаЕГАИС.ПереданВУТМ;
		
		РезультатПоиска = НайтиОбъектОжидающийОтвета("ОтчетЕГАИС", Статус, Операция, ДальнейшееДействие, 
			ОрганизацияЕГАИС, ДанныеДокумента.Объект.RestsDate, ДанныеДокумента.ФорматОбмена);
	КонецЕсли;
	...
	Если РезультатДобавленияЗаписи.НовоеСообщение Тогда
		...
		Если ОтразитьФиксацию И ЗначениеЗаполнено(ДанныеОснованияКвитанции.Ссылка) Тогда
			...
			Если ЗаписыватьДокумент Тогда
				...
				ИначеЕсли ПерезаписатьИдентификаторЕГАИС(ОснованиеКвитанцииМетаданные.Имя, ДанныеОснованияКвитанции.Операция) Тогда
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("СверткаРегистраСоответствиеНоменклатурыЕГАИС", Истина);
...
Функция ИспользуетсяЧастичноеВыбытие(МаркируемойПродукции = Истина) Экспорт
	
	Если МаркируемойПродукции Тогда
		ВидыПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЕГАИСМаркируемые();
	Иначе
		ВидыПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЕГАИС();
	КонецЕсли;
	
	Возврат ОбщегоНазначенияИС.ИспользуетсяЧастичноеВыбытие(ВидыПродукции);
...
Функция СостояниеОбмена(ОрганизацииЕГАИС) Экспорт
	...
	|	РегистрСведений.НастройкиОбменаЕГАИС КАК НастройкиОбменаЕГАИС
	|ГДЕ
	|	НастройкиОбменаЕГАИС.ИдентификаторФСРАР <> &ПустойИдентификаторФСРАР
	...
	Запрос.УстановитьПараметр("ПустойИдентификаторФСРАР", ОбщегоНазначенияИС.ПустоеЗначениеОпределяемогоТипа("СтрокаЕГАИС"));
...
//  ОперацияКвитанции - ПеречислениеСсылка.ВидыДокументовЕГАИС
// Возвращаемое значение:
//  Булево - Необходимость перезаписи идентификатора ЕГАИС.
//
Функция ПерезаписатьИдентификаторЕГАИС(ИмяДокумента, ОперацияКвитанции = Неопределено)
	...
	Если ОперацияКвитанции = Неопределено 
			Или ОперацияКвитанции <> Перечисления.ВидыДокументовЕГАИС.КвитанцияЗапросаНаОтменуПроведенияТТНПодтверждение Тогда
		ДокументыМеняющиеИдентификаторЕГАИС.Добавить("ТТНИсходящаяЕГАИС");
	КонецЕсли;
...
//  МетаданныеДокумента - Произвольный - Использовать списки к оформлению
...
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
//  ПрефиксыСписковКОфорлениюСтрокой - Строка - Префиксы элементов формы
//  ИменаЭлеменовСтрокой - Строка - Имена элементов формы для обработки
Процедура УстановитьВидимостьОрганизацияЕГАИСВСпискахДокументов(Форма, ПрефиксыСписковКОфорлениюСтрокой,
	ИменаЭлеменовСтрокой = "") Экспорт
	
	УпрощеннаяИнтеграцияБЭД = ИнтеграцияИС.УпрощеннаяИнтеграцияБЭД();
	Если УпрощеннаяИнтеграцияБЭД Тогда
		
		Если Форма.Элементы.Найти("ОрганизацияЕГАИС") <> Неопределено Тогда
			Форма.Элементы["ОрганизацияЕГАИС"].Видимость = Ложь;
		КонецЕсли;
		
		ПрефиксыСписков = СтрРазделить(ПрефиксыСписковКОфорлениюСтрокой, ",");
		Для Каждого Префикс Из ПрефиксыСписков Цикл
			
			ИмяЭлемента = СтрШаблон("%1%2%3", "Страницы", Префикс, "ОтборОрганизацияЕГАИС");
			Если Форма.Элементы.Найти(ИмяЭлемента) <> Неопределено Тогда
				Форма.Элементы[ИмяЭлемента].Видимость = Ложь;
			КонецЕсли;
			
			ИмяЭлемента = СтрШаблон("%1%2%3", "Список", Префикс, "ОрганизацияЕГАИС");
			Если Форма.Элементы.Найти(ИмяЭлемента) <> Неопределено Тогда
				Форма.Элементы[ИмяЭлемента].Видимость = Ложь;
			КонецЕсли;
		...
		Если ЗначениеЗаполнено(ИменаЭлеменовСтрокой) Тогда
			ИменаЭлементов = СтрРазделить(ИменаЭлеменовСтрокой, ",");
...
Функция ЭтоНеиспользуемыйСтатусДокумента(СтатусОбработки)
	...
	Если ОбщегоНазначенияЕГАИС.ЭтоРасширеннаяВерсияГосИС() Тогда
		МодульИнтеграцияЕГАИСПереопределяемый = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияЕГАИСПереопределяемый");
...
Функция ЗагрузитьАлкогольнуюПродукцию(СписокАлкогольнойПродукцииXDTO)
	...
	ПустойИдентификаторУпаковки = ОбщегоНазначенияИС.ПустоеЗначениеОпределяемогоТипа("СтрокаЕГАИС");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не Выборка.ЕстьИзменения Тогда
			...
			Для Каждого СтрокаТаблицы Из МассивСтрок Цикл
				...
				ДанныеСопоставления.Вставить("Номенклатура", СтрокаТаблицы.Номенклатура);
				ДанныеСопоставления.Вставить("Характеристика", СтрокаТаблицы.Характеристика);
				ДанныеАлкогольнойПродукции.ТаблицаСопоставления[СтрокаТаблицы.ИдентификаторУпаковки].Добавить(ДанныеСопоставления);
			КонецЦикла;
		...
		СопоставленнаяНоменклатура = Новый Массив;
		...
		Иначе
			...
			Для Каждого СтрокаТаблицы Из МассивСтрок Цикл
				...
				Если СопоставленнаяНоменклатура.Найти(СтрокаТаблицы.Номенклатура) = Неопределено Тогда
					СопоставленнаяНоменклатура.Добавить(СтрокаТаблицы.Номенклатура);
				КонецЕсли;
		...
		ИнтеграцияЕГАИСПереопределяемый.ПриЗагрузкеАлкогольнойПродукции(АлкогольнаяПродукцияОбъект.Ссылка, Истина, СопоставленнаяНоменклатура);
		
		Если СопоставленнаяНоменклатура.Количество()
			И ДанныеАлкогольнойПродукции.ТаблицаСопоставления.Количество() = 0 Тогда
			
			ДанныеСопоставления = Новый Структура;
			ДанныеСопоставления.Вставить("Номенклатура", СопоставленнаяНоменклатура[0]);
			ДанныеСопоставления.Вставить("Характеристика", ОбщегоНазначенияИС.ПустоеЗначениеОпределяемогоТипа("ХарактеристикаНоменклатуры"));
			
			ДанныеАлкогольнойПродукции.ТаблицаСопоставления.Вставить(ПустойИдентификаторУпаковки, Новый Массив);
			ДанныеАлкогольнойПродукции.ТаблицаСопоставления[ПустойИдентификаторУпаковки].Добавить(ДанныеСопоставления);
...
Функция ТекстЗапросаПроверкиНеобходимостиЗагрузкиКлассификатора(Классификатор)
	...
	Если Классификатор = "ClassifierOfTypesOfAlcoholicProducts" Тогда
...
// Возвращает данные последнего исходящего запроса по документу.
//
// Параметры:
//  ИмяДокумента - Строка
//  Статус - ПеречислениеСсылка - Статусы обрбаотки ЕГАИС
...
//   Структура - данные последнего исходящего запроса. Неопределено - если запрос не найден.
//
Функция НайтиОбъектОжидающийОтвета(ИмяДокумента, Статус, ВидДокумента, ДальнейшееДействие, ОрганизацияЕГАИС, ДатаДокумента, ФорматОбмена)
	
	ДатаДокумента = МестноеВремя(ДатаДокумента, ЧасовойПоясСеанса());
	...
	ТекстЗапроса =
		...
		|		ПО СтатусыДокументовЕГАИС.Документ = ТаблицаДокумента.Ссылка
		|ГДЕ
		|	ТаблицаДокумента.ОрганизацияЕГАИС = &ОрганизацияЕГАИС
		|	И ТаблицаДокумента.ВидДокумента = &ВидДокумента
		|	И ТаблицаДокумента.Дата < &Дата
		|	И ТаблицаДокумента.Проведен
		|	И СтатусыДокументовЕГАИС.Статус = &Статус
		|	И СтатусыДокументовЕГАИС.ДальнейшееДействие1 = &ДальнейшееДействие
		|
		|УПОРЯДОЧИТЬ ПО
	...
	Запрос.УстановитьПараметр("ОрганизацияЕГАИС", ОрганизацияЕГАИС);
	Запрос.УстановитьПараметр("ДальнейшееДействие", ДальнейшееДействие);
	...
	ВозвращаемоеЗначение.Ссылка			= Выборка.Ссылка;
	ВозвращаемоеЗначение.Операция		= ВидДокумента;
	ВозвращаемоеЗначение.ФорматОбмена	= ФорматОбмена;
	
	Возврат ВозвращаемоеЗначение;
	
КонецФункции

Функция СтруктураРезультатПоискаОбъекта()
	
	Результат = Новый Структура;
	Результат.Вставить("Ссылка",             Неопределено);
	Результат.Вставить("ДокументОснование",  Неопределено);
	Результат.Вставить("Операция",           Неопределено);
	Результат.Вставить("ИсходящееСообщение", Неопределено);
	Результат.Вставить("ФорматОбмена",       Перечисления.ФорматыОбменаЕГАИС.ПустаяСсылка());
