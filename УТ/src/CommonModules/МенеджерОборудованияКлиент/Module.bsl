...
Функция ПараметрыПодключенияУстройства(ДанныеУстройства) Экспорт
	...
	// ++ Локализация
	Подключение.Вставить("ТСПИоТ"                  , ДанныеУстройства.ТСПИоТ);   
	Подключение.Вставить("ТСПИоТПараметрыXML"      , ДанныеУстройства.ТСПИоТПараметрыXML);     
	Подключение.Вставить("ТСПИоТВнутренний"        , ДанныеУстройства.ТСПИоТВнутренний);
	// -- Локализация     
...
Функция ПодключаемоеОборудование() Экспорт
	...
	Если ПодключаемоеОборудование = Неопределено Тогда
		...
		ПодключаемоеОборудование.Вставить("ФискальныеУстройства", Новый Массив()); 
  		// ++ Локализация
		ПодключаемоеОборудование.Вставить("ПодключенныеЭкземплярыПИоТ", Новый Соответствие());
		ПодключаемоеОборудование.Вставить("АутентификацияПИоТ", Новый Массив());   
		// -- Локализация
		...
		ПодключаемоеОборудование.Вставить("ПроверкаВозрастаМаксНаСервере", Ложь);
...
Процедура НачатьВыполнениеКоманды_Завершение(РезультатВыполнения, ПараметрыВыполнениеКоманды) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если ПараметрыВыполнениеКоманды.Команда = "CheckFiscalization" Тогда                                                                                   
			Если РезультатВыполнения.Свойство("ОтчетВыбытииМТ") И РезультатВыполнения.ОтчетВыбытииМТ.Количество()>0 Тогда                  
				МенеджерОборудованияКлиентПереопределяемый.ПриФискализацииЧекаСМаркированнымиТоварами(ПараметрыВыполнениеКоманды.ИдентификаторУстройства, РезультатВыполнения.ОтчетВыбытииМТ);
			КонецЕсли;
			// ++ Локализация
			Если ОбщегоНазначенияБПОКлиент.ИспользуетсяДисплеиПокупателя() И ВыводQRКодаЧекаНаДисплейПокупателяПослеФискализации() Тогда
...
// Параметры для открытия формы проверки возраста в мессенджере Макс
//
// Возвращаемое значение:
//  Структура:
//   * РежимКСО - Булево - используется режим КСО
//   * ТокенОрганизации - Строка - токен организации для проверки возраста в мессенджере Макс
//   * ОтображатьQRКодНаДисплееПокупателя - Булево - отображать QR код ссылки на дисплее покупателя
Функция ПараметрыОткрытияФормыПроверкиВозрастаВМакс() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("РежимКСО", Ложь);
	Результат.Вставить("ТокенОрганизации", "");
	Результат.Вставить("ОтображатьQRКодНаДисплееПокупателя", Истина);
	Возврат Результат;
	
КонецФункции

// Открывает форму для проверки возраста в мессенжере Макс, и возвращает результат в переданном оповещении
// 
// Параметры:
//  ОповещениеЗавершения - ОписаниеОповещения - оповещение при закрытии формы, возвращает РезультатПроверкиВозрастаВМакс
//  Владелец - ФормаКлиентскогоПриложения
//  ПараметрыПроверки - см. ПараметрыОткрытияФормыПроверкиВозрастаВМакс;
//
Процедура ОткрытьФормуПроверкиВозраста(ОповещениеЗавершения, Владелец, ПараметрыПроверки = Неопределено) Экспорт
	
	Если ПараметрыПроверки = Неопределено Тогда
		ПараметрыПроверки = ПараметрыОткрытияФормыПроверкиВозрастаВМакс();
	КонецЕсли;
	
	ОткрытьФорму("ОбщаяФорма.ПроверкаВозрастаВМаксБПО", 
		ПараметрыПроверки, 
		Владелец,
		,
		,
...
// Возвращает структуру результат проверки возраста в мессенджере Макс
//
// Возвращаемое значение:
//  Структура:
//   * Результат - Строка - одно из значения структуры ТипРезультатаПроверкиВозрастаВМакс()
//   * КодОтвета - Число
//   * ВремяЗапроса - Дата
//   * ОписаниеОшибки - Строка
//
Функция РезультатПроверкиВозрастаВМакс() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("Результат", ТипРезультатаПроверкиВозрастаВМакс().Ошибка);
	Результат.Вставить("ОшибкаРасширенная", "");
	Результат.Вставить("КодОтвета", 0);
	Результат.Вставить("КодОшибкиРасшифровка", "");
	Результат.Вставить("ВремяЗапроса", Дата(1,1,1));
...
//   * ОписаниеОшибки - Строка
//
Функция РезультатПроверкиСтатусаВМакс() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("Результат", ТипРезультатаПроверкиСтатусаВМакс().Ошибка);
	Результат.Вставить("КодОтвета", 0);
	Результат.Вставить("КодОшибкиРасшифровка", "");
	Результат.Вставить("ВремяЗапроса", Дата(1,1,1));
	Результат.Вставить("КодОшибки", "");
	Результат.Вставить("ОписаниеОшибки", "");
	Результат.Вставить("ПолныйРезультат", "");
	Результат.Вставить("Телефон", "");
	Статусы = Новый Структура;
	Статусы.Вставить("Взрослый", Ложь);
	Статусы.Вставить("Студент", Ложь);
	Статусы.Вставить("СтудентУчебноеЗаведение", "");
	Статусы.Вставить("Многодетный", Ложь);
	Статусы.Вставить("МногодетныйРегионВыдачиДокумента", "");
	Статусы.Вставить("Пенсионер", Ложь);
	Статусы.Вставить("Инвалид", Ложь);
	Статусы.Вставить("ИнвалидГруппа", "");
...
// Возвращает стуруктуру, возможные типы результата проверки возраста
// 
// Возвращаемое значение:
//  Структура:
//   * Успех - Строка
//   * Отмена - Строка
//   * Ошибка - Строка
//   * ПозватьСотрудника - Строка
//
Функция ТипРезультатаПроверкиВозрастаВМакс() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ВозрастПодтвержден", "ВозрастПодтвержден");
	Результат.Вставить("Отмена", "Отмена");
	Результат.Вставить("Ошибка", "Ошибка");
	Результат.Вставить("ПозватьСотрудника", "ПозватьСотрудника");
	Возврат Результат;
...
//   * ПозватьСотрудника - Строка
//
Функция ТипРезультатаПроверкиСтатусаВМакс() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("СтатусПроверен", "СтатусПроверен");
	Результат.Вставить("СтатусПодтвержден", "СтатусПодтвержден");
	Результат.Вставить("Отмена", "Отмена");
	Результат.Вставить("Ошибка", "Ошибка");
	Результат.Вставить("ПозватьСотрудника", "ПозватьСотрудника");
	Возврат Результат;
...
// Выполняет провреку возраста по переданному Цифровому ID
//
// Параметры:
//  ЦифровойID - Строка - цифровой ID сосканированный из приложения Макс
//  ТокенОрганизации - Строка - токен организации для доступа к удаоленной проверке
//
// Возвращаемое значение:
//  Структура - см. РезультатПроверкиВозрастаВМакс
// 
Асинх Функция ПроверитьВозрастАсинх(ЦифровойID, ТокенОрганизации) Экспорт
	
	Результат = РезультатПроверкиВозрастаВМакс();
	РезультатСтатус = Ждать ПроверитьСтатусАсинх(ЦифровойID, ТокенОрганизации);
	
	Если РезультатСтатус.Результат = ТипРезультатаПроверкиСтатусаВМакс().СтатусПроверен
		Или РезультатСтатус.Результат = ТипРезультатаПроверкиСтатусаВМакс().СтатусПодтвержден Тогда
		Результат.Результат = ТипРезультатаПроверкиВозрастаВМакс().ВозрастПодтвержден;
	Иначе
		Результат.Результат = РезультатСтатус.Результат;
	КонецЕсли;
	Результат.КодОтвета = РезультатСтатус.КодОтвета;
	Результат.ВремяЗапроса = РезультатСтатус.ВремяЗапроса;
	Результат.ОписаниеОшибки = РезультатСтатус.ОписаниеОшибки;
	Результат.КодОшибкиРасшифровка = РезультатСтатус.КодОшибкиРасшифровка;
	
	Возврат Результат;
	
КонецФункции

// Выполняет проверку статуса по переданному Цифровому ID
//
// Параметры:
//  ЦифровойID - Строка - цифровой ID сканированный из приложения Макс
//  ТокенОрганизации - Строка - токен организации для доступа к удаоленной проверке
//
// Возвращаемое значение:
//  Структура - см. РезультатПроверкиВозрастаВМакс
// 
Асинх Функция ПроверитьСтатусАсинх(ЦифровойID, ТокенОрганизации) Экспорт
	
	Результат = РезультатПроверкиСтатусаВМакс();
	
	НастройкиСоединения = МенеджерОборудованияКлиентСервер.НастройкиСоединенияПроверкиСтатусаMAX();
	ПараметрыСоединения = НовыеПараметрыСоединения();
	ЗаполнитьЗначенияСвойств(ПараметрыСоединения, НастройкиСоединения);
	ПараметрыСоединения.АдресРесурса = СтрШаблон(НастройкиСоединения.АдресРесурсаРасширенный, ЦифровойID);
	ПараметрыСоединения.ТокенДоступа = ТокенОрганизации;
	
	РезультатВызова = Ждать ВызватьHTTPМетодАсинх("GET", ПараметрыСоединения);
	Результат.КодОтвета = РезультатВызова.КодОтвета;
	
	КодыОтвета = ".200.400.401.404.429.500.";
	Если СтрНайти(КодыОтвета, "." + Строка(РезультатВызова.КодОтвета) + ".") > 0 Тогда
		Попытка 
			Значение = ПрочитатьЗначениеJSON(РезультатВызова.ТелоОтвета);
			
			СтатусЗапроса = ОбщегоНазначенияБПОКлиентСервер.СвойствоСтруктурыБезопасно(Значение, "status");
			СтатусПроверки = ОбщегоНазначенияБПОКлиентСервер.СвойствоСтруктурыБезопасно(Значение, "data.verification_status");
			Если СтатусЗапроса = "success" И СтатусПроверки = "completed" Тогда
				
				ЗаполнитьРезультатыПроверкиВМессенджереМакс(Результат, Значение.data);
				ЗапомнитьДанныеДляЛогирования(Значение);
			Иначе
				ЗаполнитьОписаниеОшибкиПриПроверкеВМессенджереМакс(Результат, Значение);
			КонецЕсли;
			ЗаполнитьМетаданныеПриПроверкеВМессенджереМакс(Результат, Значение);
			
			Результат.ПолныйРезультат = РезультатВызова.ТелоОтвета;
		Исключение
			ОбщегоНазначенияБПОКлиент.ЗаписатьОшибкуВЖурналРегистрации(
				НСтр("ru = 'БПО.Проверка возраста'"), 
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			Результат.ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Ошибка разбора JSON: %1'"), РезультатВызова.ТелоОтвета);
		КонецПопытки;
		
		Если РезультатВызова.КодОтвета = 404 Тогда
			Результат.ОписаниеОшибки = НСтр("ru = 'Ошибка подтверждения статуса. Обновите QR код в MAX и попробуйте еще раз.'");
			Результат.КодОшибкиРасшифровка = "ОшибкаЦифровогоID";
		ИначеЕсли РезультатВызова.КодОтвета = 401 Тогда
			Результат.ОписаниеОшибки = НСтр("ru = 'Ошибка токена доступа партнера. Проверьте что указан корректный токен организации.'");
			Результат.КодОшибкиРасшифровка = "ОшибкаАвторизации";
		ИначеЕсли РезультатВызова.КодОтвета <> 200 Тогда 
			Результат.КодОшибкиРасшифровка = "ОшибкаПрочая";
		КонецЕсли;
		
	Иначе
		Результат.ОписаниеОшибки = РезультатВызова.ОписаниеОшибки;
		Результат.КодОшибкиРасшифровка = "ОшибкаПрочая";
	КонецЕсли;
...
//
// Параметры:
//   ДанныеОтвета - Структура - данные запроса на архивацию логов:
...
// Возвращает подключенный экземпляр ПИоТ из локального кеша клиентского приложения
// 
Функция ПодключенныйЭкземплярПИоТ(ИдентификаторУстройства) Экспорт
	
	ПодключаемоеОборудование = ПодключаемоеОборудование();
	Результат = ПодключаемоеОборудование.ПодключенныеЭкземплярыПИоТ.Получить(ИдентификаторУстройства);
	
	Возврат Результат;
	
КонецФункции

// Добавляет подключенный экземпляр ПИоТ в локальный кеша клиентского приложения
// 
// Параметры:
//   ИдентификаторУстройства - Строка -
//   ОбъектВнешнейКомпоненты - 
// 
Процедура ДобавитьПодключенныйЭкземплярПИоТ(ИдентификаторУстройства, ОбъектВнешнейКомпоненты) Экспорт
	
	ПодключаемоеОборудование = МенеджерОборудованияКлиент.ПодключаемоеОборудование();
	ПодключаемоеОборудование.ПодключенныеЭкземплярыПИоТ.Вставить(ИдентификаторУстройства, ОбъектВнешнейКомпоненты);
...
Функция НовыеПараметрыСоединения() Экспорт
	...
	Результат = Новый Структура;
	Результат.Вставить("ВызовНаСервере", Ложь);
	Результат.Вставить("Хост", "");
	Результат.Вставить("Порт", 80);
	Результат.Вставить("Таймаут", 2);
...
Асинх Функция ВызватьHTTPМетодАсинх(Метод, ПараметрыСоединения) Экспорт
	...
	ПодключаемоеОборудование = ПодключаемоеОборудование();
	ВызовНаСервере = ПодключаемоеОборудование.ПроверкаВозрастаМаксНаСервере;
	Если ВызовНаСервере Тогда
		Результат = МенеджерОборудованияВызовСервера.ВызватьHTTPМетодСервер(Метод, ПараметрыСоединения);
		Если Результат.Таймаут Тогда
			Результат = Ждать ВызватьHTTPМетодКлиентАсинх(Метод, ПараметрыСоединения);
		КонецЕсли;
	Иначе
		Результат = Ждать ВызватьHTTPМетодКлиентАсинх(Метод, ПараметрыСоединения);
		Если Результат.Таймаут Тогда
		...
		КонецЕсли;
	КонецЕсли;
	ПодключаемоеОборудование.ПроверкаВозрастаМаксНаСервере = Результат.ВызовНаСервере;
...
// Функция возвращает признак аутентификация в ГИС МТ модуля ТС ПИоТ
// 
// Параметры:
//  ИдентификаторУстройства - УникальныйИдентификатор -Идентификатор устройства
// 
// Возвращаемое значение:
//  Булево
Функция ПроверкаАутентификацияПИоТПройдена(ИдентификаторУстройства) Экспорт
	
	Результат = Ложь;
	
	ПодключаемоеОборудование = ПодключаемоеОборудование();
	АутентификацияПИоТ = ПодключаемоеОборудование.АутентификацияПИоТ;      
	
	Для Каждого ФискальноеУстройство Из АутентификацияПИоТ Цикл
		Если ФискальноеУстройство.ИдентификаторУстройства = ИдентификаторУстройства Тогда
				Результат = ФискальноеУстройство.Свойство("АутентификацияПИоТ") И ФискальноеУстройство.АутентификацияПИоТ;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
// Установить признак аутентификация в ГИС МТ модуля ТС ПИоТ
// 
// Параметры:
//  ИдентификаторУстройства - УникальныйИдентификатор -Идентификатор устройства
// 
...
//  Булево  
//
Процедура УстановитьАутентификацияПИоТПройдена(ИдентификаторУстройства, Признак = Истина) Экспорт
	
	ПодключаемоеОборудование = ПодключаемоеОборудование();
	АутентификацияПИоТ = ПодключаемоеОборудование.АутентификацияПИоТ;
	
	Для Каждого ФискальноеУстройство Из АутентификацияПИоТ Цикл
		Если ФискальноеУстройство.ИдентификаторУстройства = ИдентификаторУстройства И ФискальноеУстройство.Свойство("АутентификацияПИоТ") Тогда           
			ФискальноеУстройство.АутентификацияПИоТ = Признак;
			Возврат;
		КонецЕсли;
	КонецЦикла;   
	...
	Если Признак Тогда
		ФискальноеУстройство = Новый Структура();
		ФискальноеУстройство.Вставить("ИдентификаторУстройства", ИдентификаторУстройства);
		ФискальноеУстройство.Вставить("АутентификацияПИоТ", Признак);
		АутентификацияПИоТ.Добавить(ФискальноеУстройство);
	КонецЕсли;
...
#Область ПроверкаСтатусаВМессенджереМакс

Процедура ЗаполнитьРезультатыПроверкиВМессенджереМакс(Результат, Данные)
	
	Детали = Данные.verification_details;
	Если Детали.Свойство("phone") Тогда
		Результат.Телефон = Строка(Детали.phone);
	КонецЕсли;
	
	Статусы = Результат.Статусы;
	Статусы.Взрослый = 
		ОбщегоНазначенияБПОКлиентСервер.СвойствоСтруктурыБезопасно(Детали, "adult.status", Ложь);
		
	Если Детали.Свойство("student") Тогда
		Статусы.Студент = Детали.student.status;
		Статусы.СтудентУчебноеЗаведение = 
			Строка(Детали.student.studenttickets_organization_name);
	КонецЕсли;
	Если Детали.Свойство("certificateLargeFamily") Тогда
		Статусы.Многодетный = Детали.certificateLargeFamily.status;
		Статусы.МногодетныйРегионВыдачиДокумента = 
			Строка(Детали.certificateLargeFamily.district);
	КонецЕсли;
	Статусы.Пенсионер = 
		ОбщегоНазначенияБПОКлиентСервер.СвойствоСтруктурыБезопасно(Детали, "pensioner.status", Ложь);
	Если Детали.Свойство("disabledPerson") Тогда
		Статусы.Инвалид = Детали.disabledPerson.status;
		Статусы.ИнвалидГруппа = 
			Строка(Детали.disabledPerson.codeGroupDisability);
	КонецЕсли;
		
	Результат.Результат = ТипРезультатаПроверкиСтатусаВМакс().СтатусПроверен;
	
КонецПроцедуры

Процедура ЗапомнитьДанныеДляЛогирования(Значение)
	
	ДанныеПоследнейПроверкиСтатусаВМакс = Новый Структура;
	Попытка
		ДанныеПоследнейПроверкиСтатусаВМакс.Вставить("session_id", 
			ОбщегоНазначенияБПОКлиентСервер.СвойствоСтруктурыБезопасно(Значение, "data.session_id"));
		ДанныеПоследнейПроверкиСтатусаВМакс.Вставить("request_id", 
			ОбщегоНазначенияБПОКлиентСервер.СвойствоСтруктурыБезопасно(Значение, "metadata.request_id"));
		ДанныеПоследнейПроверкиСтатусаВМакс.Вставить("sign", 
			ОбщегоНазначенияБПОКлиентСервер.СвойствоСтруктурыБезопасно(Значение, "data.verification_details.raw_data.sign"));
		ДанныеПоследнейПроверкиСтатусаВМакс.Вставить("data", 
			ОбщегоНазначенияБПОКлиентСервер.СвойствоСтруктурыБезопасно(Значение, "data.verification_details.raw_data.data"));
		ДанныеДляЛогирования = ЗаписатьЗначениеJSON(ДанныеПоследнейПроверкиСтатусаВМакс);
		
		ПодключаемоеОборудование().ДанныеПоследнейПроверкиСтатусаВМакс = ДанныеДляЛогирования;
	Исключение
		ОбщегоНазначенияБПОКлиент.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'БПО. Проверка статуса'"), 
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	
КонецПроцедуры

Процедура ЗаполнитьОписаниеОшибкиПриПроверкеВМессенджереМакс(Результат, Значение)
	
	Если Не Значение.Свойство("error") Тогда
		Возврат;
	КонецЕсли;
	
	Ошибка = Новый Структура("code, details, message","","",""); 
	ЗаполнитьЗначенияСвойств(Ошибка, Значение.error);
	
	Результат.Результат = ТипРезультатаПроверкиСтатусаВМакс().Ошибка;
	Результат.КодОшибки = Ошибка.code;
	Результат.ОписаниеОшибки = 
		СтрШаблон("%1. %2", Ошибка.message, Ошибка.details);
	
КонецПроцедуры

Процедура ЗаполнитьМетаданныеПриПроверкеВМессенджереМакс(Результат, Значение)
	
	Если Не Значение.Свойство("metadata") Тогда
		Возврат;
	КонецЕсли;
	
	Данные = Значение.metadata;
#Если Не ВебКлиент Тогда
	Попытка
		Результат.ВремяЗапроса = ПрочитатьДатуJSON(Данные.timestamp, ФорматДатыJSON.ISO);
	Исключение
		ОбщегоНазначенияБПОКлиент.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'БПО.Проверка возраста'"), 
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
#Иначе
	Результат.ВремяЗапроса = Неопределено;
#КонецЕсли
...
Асинх Функция ВызватьHTTPМетодКлиентАсинх(Метод, ПараметрыСоединения)
	
	Результат = МенеджерОборудованияКлиентСервер.РезультатВызоваМетодаHTTP();
	Результат.ВызовНаСервере = Ложь;
	Прокси = Неопределено;
	
	Если ПараметрыСоединения.Порт = 443 Тогда
		ЗащищенноеСоединение = ОбщегоНазначенияБПОКлиентСервер.НовоеЗащищенноеСоединение();
		Соединение = Новый HTTPСоединение(
			ПараметрыСоединения.Хост, ПараметрыСоединения.Порт, , , Прокси, ПараметрыСоединения.Таймаут, ЗащищенноеСоединение);
	Иначе
		Соединение = Новый HTTPСоединение(
			ПараметрыСоединения.Хост, ПараметрыСоединения.Порт, , , Прокси, ПараметрыСоединения.Таймаут);
	КонецЕсли;
		
	Если ЗначениеЗаполнено(ПараметрыСоединения.ТокенДоступа) Тогда
		ПараметрыСоединения.Заголовки.Вставить("Authorization", ПараметрыСоединения.ТокенДоступа);
	КонецЕсли;
	HTTPЗапрос = Новый HTTPЗапрос(ПараметрыСоединения.АдресРесурса, ПараметрыСоединения.Заголовки);
	
	Попытка
		HTTPОтвет = Ждать Соединение.ВызватьHTTPМетодАсинх(Метод, HTTPЗапрос);
	...
	Исключение
		...
		Если ИнформацияОбОшибке.ЯвляетсяОшибкойКатегории(КатегорияОшибки.ОшибкаСети) Тогда
			Результат.Таймаут = Истина;
		Иначе
			ОбщегоНазначенияБПОКлиент.ЗаписатьОшибкуВЖурналРегистрации(
				НСтр("ru = 'Ошибка доступа к сервису проверки возраста Макс'"), 
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецЕсли;
		Результат.ОписаниеОшибки = ОбработкаОшибок.ПредставлениеОшибкиДляПользователя(ИнформацияОбОшибке);
	КонецПопытки;
