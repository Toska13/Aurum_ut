...
Функция ЗапросНастроекОтправки(ИмяВременнойТаблицы, Отбор = Неопределено) Экспорт
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(Отбор.ИдентификаторПолучателя) Тогда
		ПоляУсловия.Добавить(СтрШаблон("НастройкиОтправкиЭлектронныхДокументовПоВидам.ИдентификаторПолучателя В(%1)",
			Отбор.ИдентификаторПолучателя));
	КонецЕсли;
...
Функция ОбменПоТипуДокументаМожноЗавершитьБезИзвещения(ТипДокумента) Экспорт 
	...
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.СчетОферта);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьФорматПоУмолчаниюВНастройках", Истина);
