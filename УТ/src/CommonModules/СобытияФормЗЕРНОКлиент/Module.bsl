...
//  РазделУчета - Неопределено, ПеречислениеСсылка.РазделыУчетаЗЕРНО - Доступный раздел учета
//  СтандартнаяОбработка - Булево - Стандартная обработка
//  ОписаниеОповещения - Неопределено - Описание оповещения
//  Реквизиты - Неопределено - Реквизиты
Процедура ПриНачалеВыбораНоменклатуры(Элемент, РазделУчета, СтандартнаяОбработка, ОписаниеОповещения = Неопределено, Знач Реквизиты = Неопределено) Экспорт
	
	СобытияФормИСКлиентПереопределяемый.ПриНачалеВыбораНоменклатуры(
		Элемент, ИнтеграцияЗЕРНОКлиентСервер.ВидыПродукции(РазделУчета), СтандартнаяОбработка, ОписаниеОповещения, Реквизиты);
...
Процедура ОКПД2АвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка, РазделУчета = Неопределено) Экспорт
	
	Если ЗначениеЗаполнено(Текст) Тогда
		СтандартнаяОбработка = Ложь;
		ДанныеВыбора = Новый СписокЗначений;
		Для Каждого ЭлементМассива Из ИнтеграцияЗЕРНОВызовСервера.ДанныеВыбораОКПД2(Текст, РазделУчета) Цикл
			ДанныеВыбора.Добавить(ЭлементМассива, ЭлементМассива.Представление);
		КонецЦикла;
	КонецЕсли;
	
КонецПроцедуры

Процедура ОКПД2ОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка, РазделУчета = Неопределено) Экспорт
...
Процедура КодТНВЭДАвтоПодбор(Форма, Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка, ОКПД2) Экспорт
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "КэшВидовСельхозКультур") Тогда
			ДанныеМассив = ИнтеграцияЗЕРНОВызовСервера.ДанныеВыбораКодТНВЭД(Текст, ОКПД2, Форма["КэшВидовСельхозКультур"]);
		Иначе
			ДанныеМассив = ИнтеграцияЗЕРНОВызовСервера.ДанныеВыбораКодТНВЭД(Текст, ОКПД2);
		КонецЕсли;
		ДанныеВыбора = Новый СписокЗначений;
		Для Каждого ЭлементМассива Из ДанныеМассив Цикл
			ДанныеВыбора.Добавить(ЭлементМассива.КодТНВЭД, ЭлементМассива.Представление);
		КонецЦикла;
	КонецЕсли;
...
Процедура ПолеАдресаПриИзменении(Форма, Элемент, ПутьКДаннымАдреса = "") Экспорт
	...
	ПоляФормы.ПутьКЭлементу[ПоляФормы.ИмяЭлемента] =
		ОбщегоНазначенияИСВызовСервера.ЗначенияПолейКонтактнойИнформации(Текст, Форма.ВидКонтактнойИнформации);
...
Процедура ПолеАдресаАвтоПодбор(Форма, ВидДанных,
	...
	СписокАвтоподбора = ИнтеграцияИСВызовСервера.СписокАвтоподбора(ВидДанных, ТекстДляАвтоПодбора,,,Истина);
	ДанныеВыбора = Новый СписокЗначений;
	Для Каждого КлючИЗначение Из СписокАвтоподбора Цикл
		ДанныеВыбора.Добавить(КлючИЗначение.Ключ, КлючИЗначение.Значение);
	КонецЦикла;
