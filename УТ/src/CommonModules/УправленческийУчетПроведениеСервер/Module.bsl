...
Функция ОбновитьДвиженияПоОборотнымРегистрам(МассивДокументов) Экспорт
	...
	Если МассивДокументов.Количество() > 0 Тогда
		СформироватьДвиженияДенежныеСредстваКонтрагентПоКлиентам(МассивДокументов, ИзмененныеДокументы);
	КонецЕсли;
	Если МассивДокументов.Количество() > 0 Тогда
		СформироватьДвиженияДенежныеСредстваКонтрагентПоПоставщикам(МассивДокументов, ИзмененныеДокументы);
	КонецЕсли;
	Если МассивДокументов.Количество() > 0 Тогда
...
Процедура СформироватьДвиженияДенежныеСредстваКонтрагентПоКлиентам(МассивДокументов, ИзмененныеДокументы = Неопределено, ДляОтладки = Неопределено) Экспорт
	...
	Попытка
		...
		ДанныеДвижений = ДанныеДвиженийДенежныеСредстваКонтрагент(МассивДокументов, Истина, МенеджерВременныхТаблиц,
			ДляОтладки);
		
		ВыборкаПоДокументам = ДанныеДвижений.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаПоДокументам.Следующий() Цикл
			ЗаписатьДвиженияДенежныеСредстваКонтрагент(ВыборкаПоДокументам, ИзмененныеДокументы, ДляОтладки);
		КонецЦикла;
...
Процедура СформироватьДвиженияДенежныеСредстваКонтрагентПоПоставщикам(МассивДокументов, ИзмененныеДокументы = Неопределено, ДляОтладки = Неопределено) Экспорт
	...
	Попытка
		МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
		ДанныеДвижений = ДанныеДвиженийДенежныеСредстваКонтрагент(МассивДокументов, Ложь, МенеджерВременныхТаблиц,
			ДляОтладки);
...
Процедура СформироватьДвиженияКонтрагентКонтрагент(МассивДокументовРасчетов, ИзмененныеДокументы = Неопределено, ДляОтладки = Неопределено) Экспорт
	...
	Попытка
		...
		ДанныеДвижений = ДанныеДвиженийКонтрагентКонтрагент(МассивДокументовРасчетов, ДляОтладки);
		
		Если Не ДанныеДвижений = Неопределено Тогда
			ВыборкаПоДокументам = ДанныеДвижений.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			Пока ВыборкаПоДокументам.Следующий() Цикл
				ЗаписатьДвиженияКонтрагентКонтрагент(ВыборкаПоДокументам, ИзмененныеДокументы);
				СформироватьДвиженияПоПрочимАктивамПассивам(ВыборкаПоДокументам);
			КонецЦикла;
		КонецЕсли;
...
// Данные движений "Движения Денежные средства - Контрагент".
// 
// Параметры:
//  МассивДокументов - Массив из ДокументСсылка - Массив документов
//  ЭтоРасчетыСКлиентами - Булево - Признак расчета с клиентами
//  МенеджерВременныхТаблиц - Неопределено, МенеджерВременныхТаблиц - Менеджер временных таблиц
//  ДляОтладки - Неопределено, Структура - Структура, в которой возвращается информация для отладки.
// 
// Возвращаемое значение:
//  РезультатЗапроса - Данные движений "Движения Денежные средства - Контрагент"
//
Функция ДанныеДвиженийДенежныеСредстваКонтрагент(МассивДокументов, ЭтоРасчетыСКлиентами = Истина,
	МенеджерВременныхТаблиц = Неопределено, ДляОтладки = Неопределено) Экспорт
...
// Данные движений "Движения Контрагент - Контрагент"
// 
// Параметры:
//  МассивДокументовРасчетов - Массив Из ДокументСсылка - Массив документов, для которых необходимо сформировать движения.
//  ДляОтладки - Структура, Неопределено - Структура, в которой возвращается информация для отладки.
// 
// Возвращаемое значение:
//  Неопределено, РезультатЗапроса - Данные движений "Движения Контрагент - Контрагент"
//
Функция ДанныеДвиженийКонтрагентКонтрагент(МассивДокументовРасчетов, ДляОтладки = Неопределено) Экспорт
	...
	Если ТаблицаДвижений.Количество() > 0 Тогда
		...
		ДанныеДвижений = ЗапросДвижений.Выполнить();
	КонецЕсли;
...
Функция ДвиженияДенежныеСредстваКонтрагент(МенеджерВременныхТаблиц, ДляОтладки = Неопределено)
	...
	|	МАКСИМУМ(ИзмененияДвижений.Сторно) КАК Сторно
	|
	...
	|	ИЛИ МАКСИМУМ(ИзмененияДвижений.Сторно)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Регистратор
	|;
...
// Запись движений регистра "Движения Денежные средства - Контрагент"
// 
// Параметры:
//  ВыборкаДанныеДвижений - ВыборкаИзРезультатаЗапроса - Выборка по регистраторам
//  ИзмененныеДокументы - ТаблицаЗначений, Неопределено - Измененные документы
//  ДляОтладки - Неопределено, Структура - Структура, в которой возвращается информация для отладки
//
Процедура ЗаписатьДвиженияДенежныеСредстваКонтрагент(ВыборкаДанныеДвижений, ИзмененныеДокументы,
	ДляОтладки = Неопределено) Экспорт

	НаборЗаписей = РегистрыНакопления.ДвиженияДенежныеСредстваКонтрагент.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Регистратор.Установить(ВыборкаДанныеДвижений.Регистратор);
	НаборЗаписей.ДополнительныеСвойства.Вставить("НеСохранятьОтложенные");
	НаборЗаписей.ДополнительныеСвойства.Вставить("АктуализацияДвиженийДенежныеСредстваКонтрагент");
	Выборка = ВыборкаДанныеДвижений.Выбрать();
	...
	Если ИзмененныеДокументы <> Неопределено Тогда
		...
		НоваяСтрока.Ссылка = ВыборкаДанныеДвижений.Регистратор;
	КонецЕсли;
	
КонецПроцедуры

// Записать движения регистра "Движения Контрагент - Контрагент"
// 
// Параметры:
//  ВыборкаПоДокументам - ВыборкаИзРезультатаЗапроса - Выборка по регистраторам
//  ИзмененныеДокументы - ТаблицаЗначений, Неопределено - Измененные документы
//
Процедура ЗаписатьДвиженияКонтрагентКонтрагент(ВыборкаПоДокументам, ИзмененныеДокументы) Экспорт
	...
	НаборЗаписей.ДополнительныеСвойства.Вставить("АктуализацияДвиженийКонтрагентКонтрагент");
...
Процедура РаспределитьОплатуПоРасчетнымДокументам(НаборЗаписей)
	...
	Для Каждого Оплата Из Оплаты Цикл
		...
		Для Каждого Запись Из РасчетныеДокументы Цикл
			//спишем долг
			Если Запись.СуммаПостоплатыРегл <> 0 Тогда
				СуммаВВалютеПлатежа = ВзаиморасчетыСервер.СписатьСуммуПропорционально(
					СуммаОплатыВВалютеПлатежа,
					Запись.СуммаПостоплатыРегл,
					ВсегоВВалютеРегл);
				ВсегоВВалютеРегл = ВсегоВВалютеРегл - Запись.СуммаПостоплатыРегл;
				Запись.СуммаПостоплатыВВалютеПлатежа = СуммаВВалютеПлатежа;
			КонецЕсли;
			//спишем предоплату
			Если Запись.СуммаПредоплатыРегл <> 0 Тогда
				СуммаВВалютеПлатежа = ВзаиморасчетыСервер.СписатьСуммуПропорционально(
					СуммаОплатыВВалютеПлатежа,
					Запись.СуммаПредоплатыРегл,
					ВсегоВВалютеРегл);
				ВсегоВВалютеРегл = ВсегоВВалютеРегл - Запись.СуммаПредоплатыРегл;
...
//Платежные документы: АО, СписаниеБДС, РКО
		...
		"//При оплате за филиал под приход в документах АвансовыйОтчет, СписаниеБДС и РКО подставим Расход в головной организации
		|ВЫБРАТЬ
		|	РасчетыСПартнерами.Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	РасчетыСПартнерами.Регистратор КАК Регистратор,
...
Процедура ПровестиВзаимозачет(ДебиторскаяЗадолженность, КредиторскаяЗадолженность, ТаблицаДвижений) Экспорт
	...
	ВзаимозачетЗадолженности = Перечисления.ХозяйственныеОперации.ВзаимозачетЗадолженности;
	Для Каждого Дебет Из ДебиторскаяЗадолженность Цикл
		...
		Для Каждого Кредит Из КредиторскаяЗадолженность Цикл
			...
			Если Дебет.ВалютаВзаиморасчетов <> Кредит.КорВалютаВзаиморасчетов
				ИЛИ (Дебет.СуммаВВалютеВзаиморасчетов <> Кредит.КорСуммаВВалютеВзаиморасчетов
					И Дебет.ХозяйственнаяОперация = ВзаимозачетЗадолженности) Тогда
...
Функция ВыполнитьЗамены(ТекстЗапроса, ИмяТаблицы, ИмяВременнойТаблицы, Сторно)
	...
	Если Сторно Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РасчетыСПоставщиками.Период", "Т.ДатаОтраженияВУчете");
...
Функция ТекстЗапросаДвиженияКонтрагентКонтрагент()
	...
	|
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ИзмененныеДвижения.ТипСуммы) КАК ТипСуммыИзменен,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ИзмененныеДвижения.КорТипСуммы) КАК КорТипСуммыИзменен,
	|
	...
	|
	|		ЗаписиРегистра.ТипСуммы КАК ТипСуммы,
	|		ЗаписиРегистра.КорТипСуммы КАК КорТипСуммы,
	|
	...
	|		НовыеДвижения.Регистратор КАК Регистратор,
	|
	|		НовыеДвижения.ТипСуммы КАК ТипСуммы,
	|		НовыеДвижения.КорТипСуммы КАК КорТипСуммы,
	|
	|		-НовыеДвижения.Сумма КАК Сумма,
	|		-НовыеДвижения.СуммаБезНДС КАК СуммаБезНДС,
	|		-НовыеДвижения.СуммаРегл КАК СуммаРегл,
	...
	|
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ИзмененныеДвижения.ТипСуммы) > 1
	|	ИЛИ КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ИзмененныеДвижения.КорТипСуммы) > 1
	|
	...
	|	НовыеДвижения.Подразделение,
	|
	|	НовыеДвижения.Организация.ГоловнаяОрганизация КАК ОрганизацияГоловнаяОрганизация,
	|
	|	НовыеДвижения.НаправлениеДеятельности,
	|	НовыеДвижения.Партнер,
	|	НовыеДвижения.Контрагент КАК Контрагент,
	...
	|	НовыеДвижения.Договор,
	|
	|	КонтрагентыОрганизации.ГоловнаяОрганизация КАК КонтрагентГоловнаяОрганизация,
	|	ОбъектыРасчетов.Подразделение КАК ОбъектРасчетовПодразделение,
	|	ОбъектыРасчетов.УникальныйИдентификатор КАК ОбъектРасчетовУникальныйИдентификатор,
	|
	|	НовыеДвижения.КорНаправлениеДеятельности,
	|	НовыеДвижения.КорПартнер,
	|	НовыеДвижения.КорКонтрагент КАК КорКонтрагент,
	...
	|	НовыеДвижения.КорДоговор,
	|
	|	КорКонтрагентыОрганизации.ГоловнаяОрганизация КАК КорКонтрагентГоловнаяОрганизация,
	|	КорОбъектыРасчетов.Подразделение КАК КорОбъектРасчетовПодразделение,
	|	КорОбъектыРасчетов.УникальныйИдентификатор КАК КорОбъектРасчетовУникальныйИдентификатор,
	|
	|	НовыеДвижения.ТипСуммы,
	|	НовыеДвижения.КорТипСуммы,
	|
	...
	|	ПО НовыеДвижения.ОбъектРасчетов = ОбъектыРасчетов.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ОбъектыРасчетов КАК КорОбъектыРасчетов
	|	ПО НовыеДвижения.КорОбъектРасчетов = КорОбъектыРасчетов.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Организации КАК КонтрагентыОрганизации
	|	ПО НовыеДвижения.Контрагент = КонтрагентыОрганизации.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Организации КАК КорКонтрагентыОрганизации
	|	ПО НовыеДвижения.КорКонтрагент = КорКонтрагентыОрганизации.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НастройкиХозяйственныхОпераций КАК НастройкиХозяйственныхОпераций
	|	ПО НовыеДвижения.ХозяйственнаяОперация = НастройкиХозяйственныхОпераций.ХозяйственнаяОперация
	|
	|УПОРЯДОЧИТЬ ПО
	|	Период,
	|	Регистратор,
...
Процедура СформироватьДвиженияПоПрочимАктивамПассивам(ВыборкаПоДокументам) Экспорт
	
	МетаРегистрНакопления = Метаданные.РегистрыНакопления.ДвиженияПоПрочимАктивамПассивам;
	ВыборкаПоДокументам.Сбросить();
	
	Пока ВыборкаПоДокументам.Следующий() Цикл
		МетаДокумент = ВыборкаПоДокументам.Регистратор.Метаданные();
		Если НЕ МетаДокумент.Движения.Содержит(МетаРегистрНакопления) Тогда
			Продолжить;
		КонецЕсли;
		...
		СоответствиеОперацийИдентификаторам = Новый Соответствие();
		КоличествоЗаписей = НаборЗаписей.Количество();
		
		Для НомерЗаписи = 1 По КоличествоЗаписей Цикл
			ОбратныйИндекс = КоличествоЗаписей - НомерЗаписи;
			ТекущаяЗапись = НаборЗаписей[ОбратныйИндекс];
			Если ТекущаяЗапись.Статья = ПланыВидовХарактеристик.СтатьиАктивовПассивов.РасчетыСФилиалами Тогда
				СоответствиеОперацийИдентификаторам.Вставить(ТекущаяЗапись.ИдентификаторФинЗаписи, ТекущаяЗапись.НастройкаХозяйственнойОперации);
		...
		Выборка = ВыборкаПоДокументам.Выбрать();
		Пока Выборка.Следующий() Цикл
			Если Выборка.Организация <> Выборка.КорКонтрагент
			   И Выборка.ОрганизацияГоловнаяОрганизация = Выборка.КорКонтрагентГоловнаяОрганизация Тогда
				НоваяЗапись = НаборЗаписей.Добавить();
				НоваяЗапись.Период = Выборка.Период;
				НоваяЗапись.Организация = Выборка.Организация;
				НоваяЗапись.Подразделение = Выборка.КорОбъектРасчетовПодразделение;
				НоваяЗапись.НаправлениеДеятельности = Выборка.КорНаправлениеДеятельности;
				НоваяЗапись.Статья = ПланыВидовХарактеристик.СтатьиАктивовПассивов.РасчетыСФилиалами;
				НоваяЗапись.Аналитика = Выборка.КорКонтрагент;
				НоваяЗапись.ДебетКредит = Перечисления.ВидыДвиженийПрочихАктивовПассивов.Дебет;
				НоваяЗапись.СуммаРегл = Выборка.СуммаРегл;
				НоваяЗапись.СуммаУпр = Выборка.Сумма;
				НоваяЗапись.СуммаСНДС = Выборка.Сумма;
				НоваяЗапись.СуммаБезНДС = Выборка.СуммаБезНДС;
				НоваяЗапись.ИдентификаторФинЗаписи = Выборка.ОбъектРасчетовУникальныйИдентификатор;
				Если СоответствиеОперацийИдентификаторам[НоваяЗапись.ИдентификаторФинЗаписи] <> Неопределено Тогда
					НоваяЗапись.НастройкаХозяйственнойОперации = СоответствиеОперацийИдентификаторам[НоваяЗапись.ИдентификаторФинЗаписи];
				ИначеЕсли Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаПоставщику
					ИЛИ Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаДенежныхСредствВДругуюОрганизацию
					ИЛИ Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеТаможне
					ИЛИ Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОплатыОтКлиента
					ИЛИ Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзДругойОрганизации Тогда
					НоваяЗапись.НастройкаХозяйственнойОперации = Справочники.НастройкиХозяйственныхОпераций.ПереносПлатежаМеждуФилиалами;
				ИначеЕсли Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОплатыКлиенту
					ИЛИ Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратДенежныхСредствОтПоставщика Тогда
					НоваяЗапись.НастройкаХозяйственнойОперации = Справочники.НастройкиХозяйственныхОпераций.ПереносАванса;
				Иначе
					НоваяЗапись.НастройкаХозяйственнойОперации = Выборка.НастройкаХозяйственнойОперации;
				КонецЕсли;
			КонецЕсли;
			
			Если Выборка.Организация <> Выборка.Контрагент
			   И Выборка.ОрганизацияГоловнаяОрганизация = Выборка.КонтрагентГоловнаяОрганизация Тогда
				НоваяЗапись = НаборЗаписей.Добавить();
				НоваяЗапись.Период = Выборка.Период;
				НоваяЗапись.Организация = Выборка.Организация;
				НоваяЗапись.Подразделение = Выборка.ОбъектРасчетовПодразделение;
				НоваяЗапись.НаправлениеДеятельности = Выборка.НаправлениеДеятельности;
				НоваяЗапись.Статья = ПланыВидовХарактеристик.СтатьиАктивовПассивов.РасчетыСФилиалами;
				НоваяЗапись.Аналитика = Выборка.Контрагент;
				НоваяЗапись.ДебетКредит = Перечисления.ВидыДвиженийПрочихАктивовПассивов.Кредит;
				НоваяЗапись.СуммаРегл = Выборка.СуммаРегл;
				НоваяЗапись.СуммаУпр = Выборка.Сумма;
				НоваяЗапись.СуммаСНДС = Выборка.Сумма;
				НоваяЗапись.СуммаБезНДС = Выборка.СуммаБезНДС;
				НоваяЗапись.ИдентификаторФинЗаписи = Выборка.КорОбъектРасчетовУникальныйИдентификатор;
				Если СоответствиеОперацийИдентификаторам[НоваяЗапись.ИдентификаторФинЗаписи] <> Неопределено Тогда
					НоваяЗапись.НастройкаХозяйственнойОперации = СоответствиеОперацийИдентификаторам[НоваяЗапись.ИдентификаторФинЗаписи];
				ИначеЕсли Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаПоставщику
					ИЛИ Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаДенежныхСредствВДругуюОрганизацию
			...
			Если Выборка.ОрганизацияГоловнаяОрганизация <> Выборка.КонтрагентГоловнаяОрганизация
			   И Выборка.ОрганизацияГоловнаяОрганизация <> Выборка.КорКонтрагентГоловнаяОрганизация Тогда
				Если Выборка.Организация = Выборка.КорКонтрагентГоловнаяОрганизация
				 ИЛИ Выборка.ОрганизацияГоловнаяОрганизация = Выборка.КорКонтрагент Тогда
					НоваяЗапись = НаборЗаписей.Добавить();
					НоваяЗапись.Период = Выборка.Период;
					НоваяЗапись.Организация = Выборка.Организация;
					НоваяЗапись.Подразделение = Выборка.КорОбъектРасчетовПодразделение;
				...
				Если Выборка.Организация = Выборка.КонтрагентГоловнаяОрганизация
				 ИЛИ Выборка.ОрганизацияГоловнаяОрганизация = Выборка.Контрагент Тогда
					НоваяЗапись = НаборЗаписей.Добавить();
					НоваяЗапись.Период = Выборка.Период;
					НоваяЗапись.Организация = Выборка.Организация;
					НоваяЗапись.Подразделение = Выборка.ОбъектРасчетовПодразделение;
