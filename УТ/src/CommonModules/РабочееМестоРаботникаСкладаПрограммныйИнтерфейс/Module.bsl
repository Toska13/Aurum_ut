...
Функция ИнформацияОСерииИзШтрихкода(ШтрихкодСерии, ИспользоватьНомерСерии, ИспользоватьСрокГодностиСерии) Экспорт
	...
	ИнформацияИзШтрихкодаСерии = ШтрихкодированиеНоменклатурыСервер.ИнформацияОСерииИзШтрихкода(
									ШтрихкодСерии,
...
Процедура ЗаполнитьСтатусыУказанияСерийТоварыОтборРазмещение(Задание, ПараметрыЗадания)
	...
	Если ТипЗадания = "Отбор" Тогда
		...
		|	ЕСТЬNULL(ВЫРАЗИТЬ(ТоварыОтбор.Номенклатура.Описание КАК СТРОКА(1000)), """") КАК ОписаниеНоменклатуры,
		...
		|	ЕСТЬNULL(ВЫРАЗИТЬ(ТоварыОтбор.Номенклатура.Описание КАК СТРОКА(1000)), """"),
	...
	ИначеЕсли ТипЗадания = "Размещение" Тогда
		...
		|	ЕСТЬNULL(ВЫРАЗИТЬ(ТоварыРазмещение.Номенклатура.Описание КАК СТРОКА(1000)), """") КАК ОписаниеНоменклатуры,
		...
		|	ЕСТЬNULL(ВЫРАЗИТЬ(ТоварыРазмещение.Номенклатура.Описание КАК СТРОКА(1000)), """"),
	...
	ПараметрыОтбораШтрихкодов = РегистрыСведений.ШтрихкодыНоменклатуры.ПараметрыОтбораШтрихкодовНоменклатурыПоУмолчанию();
	...
	Для Каждого ТаблицаТоваров Из ТоварыЗадания Цикл
		...
		Для Каждого СтрокаТоваров Из Таблица Цикл
			...
			ЗаполнитьЗначенияСвойств(ПараметрыОтбораШтрихкодов, СтрокаТоваров);
			
			ШтрихкодыНоменклатуры = РегистрыСведений.ШтрихкодыНоменклатуры.ШтрихкодыНоменклатуры(
										ПараметрыОтбораШтрихкодов);
			...
			Если ЗначениеЗаполнено(СтрокаТоваров.Упаковка)
				...
				ПараметрыОтбораШтрихкодов.Упаковка = Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка();
			...
			Если ШтрихкодыНоменклатуры.Количество() > 0 Тогда
				СтрокаТоваров.ШтрихкодНоменклатуры = СтрСоединить(ШтрихкодыНоменклатуры, ", ");
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
