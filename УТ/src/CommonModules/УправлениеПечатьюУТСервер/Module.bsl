...
// Формирует печатную форму документа Акт выполненных работ с использованием макета
...
//
Функция ПечатьАктВыполненныхРаботMicrosoftWord(ДокументСсылка, МакетИДанныеОбъекта, ИмяМакета) Экспорт
	...
	Попытка
		...
		Область = УправлениеПечатью.ОбластьМакета(Макет,Секции[ИмяМакета].ЗаголовокДокумента);
		УправлениеПечатью.ПрисоединитьОбластьИЗаполнитьПараметры(ПечатнаяФорма, Область, ДанныеОбъекта, Ложь);
		
		Если ЗначениеЗаполнено(ДанныеОбъекта.ДополнительнаяИнформацияШапки) Тогда
			Область = УправлениеПечатью.ОбластьМакета(Макет,Секции[ИмяМакета].ДополнительнаяИнформацияШапки);
			УправлениеПечатью.ПрисоединитьОбластьИЗаполнитьПараметры(ПечатнаяФорма, Область, ДанныеОбъекта, Ложь);
		КонецЕсли;
		
		Область = УправлениеПечатью.ОбластьМакета(Макет,Секции[ИмяМакета].ЗаголовокШапки);
		...
		ОбластьШапки = УправлениеПечатью.ОбластьМакета(Макет,Секции[ИмяМакета]["ШапкаТаблицы" + СуффиксОбласти]);
		УправлениеПечатью.ПрисоединитьОбласть(ПечатнаяФорма, ОбластьШапки, Ложь);
		...
		Для Каждого СтрокаТЧ Из ДанныеОбъекта.Услуги Цикл
			...
			Область = УправлениеПечатью.ОбластьМакета(Макет, ОбластьСтроки);
			УправлениеПечатью.ПрисоединитьИЗаполнитьКоллекцию(ПечатнаяФорма, Область, Масс, Ложь);
			
		КонецЦикла;
		
		Область = УправлениеПечатью.ОбластьМакета(Макет,Секции[ИмяМакета].Итого);
		УправлениеПечатью.ПрисоединитьОбластьИЗаполнитьПараметры(ПечатнаяФорма, Область, ДанныеОбъекта, Ложь);
		
		Если ЕстьНДС Тогда
			Область = УправлениеПечатью.ОбластьМакета(Макет,Секции[ИмяМакета].ИтогоНДС);
			УправлениеПечатью.ПрисоединитьОбластьИЗаполнитьПараметры(ПечатнаяФорма, Область, ДанныеОбъекта, Ложь);
		КонецЕсли;
		
		Область = УправлениеПечатью.ОбластьМакета(Макет,Секции[ИмяМакета].СуммаПрописью);
		УправлениеПечатью.ПрисоединитьОбластьИЗаполнитьПараметры(ПечатнаяФорма, Область, ДанныеОбъекта, Ложь);
		
		Область = УправлениеПечатью.ОбластьМакета(Макет,Секции[ИмяМакета].Подписи);
		УправлениеПечатью.ПрисоединитьОбластьИЗаполнитьПараметры(ПечатнаяФорма, Область, ДанныеОбъекта, Ложь);
		
		Если ЗначениеЗаполнено(ДанныеОбъекта.ДополнительнаяИнформация) Тогда
			Область = УправлениеПечатью.ОбластьМакета(Макет,Секции[ИмяМакета].ДополнительнаяИнформация);
			УправлениеПечатью.ПрисоединитьОбластьИЗаполнитьПараметры(ПечатнаяФорма, Область, ДанныеОбъекта, Ложь);
		КонецЕсли;
...
// Формирует печатную форму документа Акт взаимозачета задолженности с использованием макета
// в формате офисного документа Office Open XML.
//
// Параметры:
//  ДокументСсылка      - ЛюбаяСсылка - объект, по которому требуется сформировать печатную форму.
...
//  Строка - адрес хранилища, куда помещается сформированный файл.
//
Функция ПечатьАктаВзаимозачетаЗадолженностиMicrosoftWord(ДокументСсылка, МакетИДанныеОбъекта, ИмяМакета) Экспорт
	...
	Попытка
		...
		Если Не ДанныеОбъекта.Взаимозачет Тогда
			...
			Если ДанныеОбъекта.ВзаимозачетМеждуОрганизациями Тогда
				Текст = НСтр("ru = 'Для взаимозачета с участием двух организаций требуется печатать акт переуступки долга.'");
			Иначе
		...
		Иначе
			...
			Область = УправлениеПечатью.ОбластьМакета(Макет, Секции[ИмяМакета].ДебиторскаяЗадолженность);
			УправлениеПечатью.ПрисоединитьОбластьИЗаполнитьПараметры(ПечатнаяФорма, Область, ДанныеОбъекта, Истина);
			...
			УправлениеПечатью.ПрисоединитьИЗаполнитьКоллекцию(ПечатнаяФорма,Область, ДанныеОбъекта.ДебиторскаяЗадолженность, Ложь);
			
			// Выводим данные кредиторской задолженности.
			Область = УправлениеПечатью.ОбластьМакета(Макет, Секции[ИмяМакета].КредиторскаяЗадолженность);
			УправлениеПечатью.ПрисоединитьОбластьИЗаполнитьПараметры(ПечатнаяФорма, Область, ДанныеОбъекта, Истина);
			...
			Область = УправлениеПечатью.ОбластьМакета(Макет, Секции[ИмяМакета].СтрокаТаблицы);
			УправлениеПечатью.ПрисоединитьИЗаполнитьКоллекцию(ПечатнаяФорма,Область, ДанныеОбъекта.КредиторскаяЗадолженность, Ложь);
...
// Формирует печатную форму документа Акт переуступки долга с использованием макета
// в формате офисного документа Office Open XML.
//
// Параметры:
//  ДокументСсылка      - ЛюбаяСсылка - объект, по которому требуется сформировать печатную форму.
...
//  Строка - адрес хранилища, куда помещается сформированный файл.
//
Функция ПечатьАктаПереуступкиДолгаMicrosoftWord(ДокументСсылка, МакетИДанныеОбъекта, ИмяМакета) Экспорт
	...
	Попытка
		...
		Иначе
			...
			// Выводим подвал документа.
			Область = УправлениеПечатью.ОбластьМакета(Макет, Секции[ИмяМакета].Итог);
			УправлениеПечатью.ПрисоединитьОбластьИЗаполнитьПараметры(ПечатнаяФорма, Область, ДанныеОбъекта, Истина);
...
// Формирует печатную форму документа Акт переуступки долга между организациями с использованием макета
// в формате офисного документа Office Open XML.
//
// Параметры:
//  ДокументСсылка      - ЛюбаяСсылка - объект, по которому требуется сформировать печатную форму.
...
//  Строка - адрес хранилища, куда помещается сформированный файл.
//
Функция ПечатьАктаПереуступкиДолгаМеждуОрганизациямиMicrosoftWord(ДокументСсылка, МакетИДанныеОбъекта, ИмяМакета) Экспорт
	...
	Попытка
		...
		Если ДанныеОбъекта.Взаимозачет ИЛИ НЕ ДанныеОбъекта.ВзаимозачетМеждуОрганизациями Тогда
...
Процедура ПриОпределенииНастроекПечати(Настройки) Экспорт
	...
	ТМЦВЭксплуатацииСервер.ПриОпределенииНастроекПечати(Настройки);
	...
	СписокОбъектов.Добавить(Документы.ЗаявкаНаКонтрольКачестваТоваров);
	СписокОбъектов.Добавить(Документы.АктКонтроляКачестваТоваров);
	...
	СписокОбъектов.Добавить(Документы.ЗаказНаДоставку);
	...
	СписокОбъектов.Добавить(Документы.Рейс);
	...
	УчетНДСУП.ПриОпределенииНастроекПечати(Настройки);
	//-- НЕ ГОСИС
