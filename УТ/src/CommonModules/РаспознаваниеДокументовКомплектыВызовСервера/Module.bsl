		...
		Для Каждого ЭтотТипДокумента Из ТипыДокументов Цикл
			...
			Для Каждого ТипРаспознанного Из ТипыРаспознанных Цикл
				...
				Если ДанныеСозданного.ПрикрепитьСкан
					И РаспознаваниеДокументовКомплекты.НужноПрикрепитьДополнительныйСкан(
						ПараметрыСоздания.ТипКомплекта, ТипРаспознанного, ЭтотТипДокумента) Тогда
					ОткудаПрикреплятьСканы.Добавить(ДопРаспознанныйОбъект);
				КонецЕсли;
			...
			Иначе
				...
				Если СоздаваемыйДокумент = Неопределено Тогда
					...
					Если РаспознаваниеДокументовТипы.ЭтоПоступлениеТоваровУслуг(ЭтотТипДокумента) 
						...
						Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
							СоздаваемыйДокументОбъект, ИмяРеквизитаСклад) Тогда
							СоздаваемыйДокументОбъект[ИмяРеквизитаСклад]
								= РаспознаваниеДокументовСлужебныйКлиентСервер.ЗначениеРеквизитаДокумента(РаспознанныйОбъект, "Склад");
				...
				Иначе
					...
					Если РаспознаваниеДокументовТипы.ЭтоПоступлениеТоваровУслуг(ЭтотТипДокумента) 
						...
						ТипДокументаОснования = ТипЗнч(СоздаваемыйДокумент);
						Если РаспознаваниеДокументовТипы.ЭтоЗаказКлиента(ТипДокументаОснования) Тогда
							МассивСсылок = Новый Массив;
							МассивСсылок.Добавить(СоздаваемыйДокумент);
							ПараметрыОткрытия = СозданиеНаОснованииУТВызовСервера.РеализацияТоваровУслугПараметрыОткрытияФормы(МассивСсылок);
							ПараметрыЗаполнения = ПараметрыОткрытия.Основание;
							РеквизитыШапки = Неопределено;
							Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПараметрыЗаполнения, "РеквизитыШапки") Тогда
								РеквизитыШапки = ПараметрыЗаполнения.РеквизитыШапки;
							КонецЕсли;
							Если РеквизитыШапки <> Неопределено
							И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(РеквизитыШапки, "Подразделение") Тогда
								ПараметрыЗаполнения.РеквизитыШапки.Подразделение =
									РаспознаваниеДокументовСлужебныйКлиентСервер.ЗначениеРеквизитаДокумента(РаспознанныйОбъект, "Подразделение");
							КонецЕсли;
						Иначе
						...
						Если РаспознаваниеДокументовТипы.ЭтоЗаказПоставщику(ТипДокументаОснования) И ТипЗнч(СоздаваемыйДокументОбъект)
								= Тип("ДокументОбъект.ПриобретениеТоваровУслуг") Тогда
							ТоварыУслуги = ДанныеРаспознанного.ПараметрыЗаполнения.ТоварыУслуги;
							Для Каждого Строка Из ТоварыУслуги Цикл
								Если ЗначениеЗаполнено(Строка.НомерГТД) Тогда
									ПараметрыПоиска = Новый Структура("Номенклатура, НомерСтроки", Строка.Номенклатура, Строка.НомерСтроки);
									ИскомыеСтроки = СоздаваемыйДокументОбъект.Товары.НайтиСтроки(ПараметрыПоиска);
									Если ИскомыеСтроки.Количество() > 0 Тогда
										ИскомыеСтроки[0].НомерГТД = Строка.НомерГТД;
									КонецЕсли;
								КонецЕсли;
							КонецЦикла;
						КонецЕсли;
						...
						КонецЕсли;
						РаспознаваниеДокументовПереопределяемый.ПриПроведенииДокументаНаОснованииРаспознанного(
							СоздаваемыйДокументОбъект);
					...
					ИначеЕсли РаспознаваниеДокументовТипы.ЭтоСчетФактураПолученный(ЭтотТипДокумента)
						...
						ПараметрыСозданияСФ.Вставить("ДокументОснование", СоздаваемыйДокумент);
					...
					ИначеЕсли РаспознаваниеДокументовТипы.ЭтоАктВыполненныхРабот(ЭтотТипДокумента) Тогда
...
Функция ДанныеЯчейкиШапкиНаСервере(ПараметрыПолучения) Экспорт
	...
	Если ПараметрыПолучения.ИмяРеквизита = "БанковскийСчет" Тогда
		...
		Если ТипЗнч(ПараметрыПолучения.ИсточникПоляБанковскийСчетВладелец) = Тип("СправочникСсылка.Организации") Тогда
			СтрокаЗамены = "ВЫРАЗИТЬ(СписокВыбораРеквизитовРаспознаваниеДокументов.Значение КАК Справочник.БанковскиеСчетаОрганизаций).Владелец = &Владелец";
		Иначе
			СтрокаЗамены = "ВЫРАЗИТЬ(СписокВыбораРеквизитовРаспознаваниеДокументов.Значение КАК Справочник.БанковскиеСчетаКонтрагентов).Владелец = &Владелец";
		КонецЕсли;
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДополнительныйОтбор", СтрокаЗамены);
	...
	ИначеЕсли ПараметрыПолучения.ИмяРеквизита = "Договор" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДополнительныйОтбор",
			"ВЫРАЗИТЬ(СписокВыбораРеквизитовРаспознаваниеДокументов.Значение КАК Справочник.ДоговорыКонтрагентов).Контрагент = &Контрагент
			|	И ВЫРАЗИТЬ(СписокВыбораРеквизитовРаспознаваниеДокументов.Значение КАК Справочник.ДоговорыКонтрагентов).ТипДоговора В(&ВидыДоговора)
	...
	Если Выборка.Следующий() Тогда
		...
		КартинкаВнутриHTML = РаспознаваниеДокументовКомплекты.КартинкаКакТекстHTML(ДвоичныеДанные);
