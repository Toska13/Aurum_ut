...
// Ключ назначения использования для формы обработки "ОбеспечениеПотребностей", открываемой для формирования заказов. 
// 
// Возвращаемое значение:
//  Строка - Ключ назначения использования формирования заявок по потребностям
Функция КлючНазначенияИспользованияФормированияЗаказовПоПотребностям() Экспорт
	
	Возврат ""; // Для использования значения по умолчанию
	
КонецФункции

// Ключ назначения использования для формы обработки "ОбеспечениеПотребностей", открываемой для формирования заявок. 
// 
// Возвращаемое значение:
//  Строка - Ключ назначения использования формирования заявок по потребностям
Функция КлючНазначенияИспользованияФормированияЗаявокПоПотребностям() Экспорт
	
	Возврат "ФормированиеЗаявокПоПотребностям";
