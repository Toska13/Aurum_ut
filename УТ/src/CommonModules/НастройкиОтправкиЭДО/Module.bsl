...
// Описывает настройку заполнения дополнительных полей.
// 
// Параметры:
//  СтруктураДанных - Неопределено, Массив из Структура -
// 
...
//  * Правило - Строка, Неопределено -
//  * Заполнение - Строка, Неопределено -
//  * Значение - Строка, Неопределено -
//  * Версия - Строка, Неопределено -
//  * Раздел - Строка, Неопределено -
//
Функция НоваяНастройкаЗаполненияДополнительныхПолей(СтруктураДанных = Неопределено) Экспорт
	Возврат НастройкиОтправкиЭДОСлужебный.НоваяНастройкаЗаполненияДополнительныхПолей(СтруктураДанных);
КонецФункции
...
Процедура СоздатьНастройкиОтправки(Знач Организация, Знач Контрагент, Знач ДоговорКонтрагента,
	...
	ЭтоПерваяНастройкаПоКонтрагенту = Не НастройкиЭДО.ЕстьНастройкиОтправки(Организация, Контрагент);
	
	Если Не ЗначениеЗаполнено(ДоговорКонтрагента) 
		Или ЭтоПерваяНастройкаПоКонтрагенту Тогда 
	...
	ЭтоОбменЧерезОператора = СинхронизацияЭДОКлиентСервер.ЭтоОбменЧерезОператора(Данные.СпособОбменаЭД);
	...
	Если ЗначениеЗаполнено(ТекущаяТаблица) Тогда
		АктуальнаяСтрокаНастройки = ТекущаяТаблица[0];
		
		Если АктуальнаяСтрокаНастройки.ИдентификаторПолучателя <> ИдентификаторКонтрагента Тогда
			ПараметрыПриглашения = ПриглашенияЭДОКлиентСервер.КлючПриглашения();
			ПараметрыПриглашения.ИдентификаторОрганизации = АктуальнаяСтрокаНастройки.ИдентификаторОтправителя;
			ПараметрыПриглашения.ИдентификаторКонтрагента = АктуальнаяСтрокаНастройки.ИдентификаторПолучателя;
			ЗаполнятьПредставлениеКонтрагента = Ложь;
			ДанныеПриглашенияТекущейНастройки = ПриглашенияЭДО.ДанныеПриглашенияПоПараметрам(
				ПараметрыПриглашения,
				ЗаполнятьПредставлениеКонтрагента);
			// Подменяем идентификаторы т.к. ТекущиеНастройкиОтправкиЭлектронныхДокументовПоВидам возвращает записанные 
			// настройки в случае наличия по контрагенту, запланировали пересмотр работы с настройками
			Если ДанныеПриглашенияТекущейНастройки.Статус = Перечисления.СтатусыПриглашений.Отклонено Тогда
				ТекущаяТаблица.ЗаполнитьЗначения(ИдентификаторОрганизации, "ИдентификаторОтправителя");
				ТекущаяТаблица.ЗаполнитьЗначения(ИдентификаторКонтрагента, "ИдентификаторПолучателя");
			КонецЕсли;
		КонецЕсли;
...
// Определяет требуется ли синхронизация с сервисом настроек по дате последнего обновления
//
// Возвращаемое значение:
//  Булево
Функция ТребуетсяОбновлениеФорматовПоДатеСинхронизации() Экспорт
	
	ТекущаяДата = ТекущаяДатаСеанса();
	
	СинхронизацияФорматовЭД = Перечисления.ТипыСинхронизацииСервисаНастроекЭДО.СинхронизацияФорматовЭД;
	ДатаПоследнейПроверки = СинхронизацияЭДО.ДатаПоследнейПроверкиДанныхСервисаНастроек(СинхронизацияФорматовЭД);
	
	Возврат НачалоДня(ТекущаяДата) > ДатаПоследнейПроверки;
...
// 
// Параметры:
//  ПоддерживаемыеФорматы - См. ФорматыЭДО_ФНС.ПоддерживаемыеФорматы
//  ФорматыЭлектронныхДокументов - Неопределено
//                               - См. ЭлектронныеДокументыЭДО.ФорматыЭлектронныхДокументов
// 
// Возвращаемое значение:
//  Булево
Функция ТребуетсяОбновлениеФорматовИСвязейВидовФорматовДляУПД_5_03(ПоддерживаемыеФорматы,
	ФорматыЭлектронныхДокументов = Неопределено) Экспорт
	
	Если ФорматыЭлектронныхДокументов = Неопределено Тогда
		ФорматыЭлектронныхДокументов = ЭлектронныеДокументыЭДО.ФорматыЭлектронныхДокументов();
	КонецЕсли;
	
	Отбор = Новый Структура("ИдентификаторФормата", ПоддерживаемыеФорматы.УПД_5_03.ИнформацияПродавца);
	ФорматыДокументов = ФорматыЭДО.ФорматыЭлектронныхДокументов();
	НайденныеФорматы = ФорматыДокументов.НайтиСтроки(Отбор);
	ФорматУПД_5_03_ЗагруженВИнформационнуюБазу = ЗначениеЗаполнено(НайденныеФорматы);
	
	Отбор = Новый Структура;
	Отбор.Вставить("ИдентификаторФормата", ПоддерживаемыеФорматы.УПД2019.ИнформацияПродавца);
	Отбор.Вставить("Отключен", Ложь);
	НайденныеФорматы = ФорматыЭлектронныхДокументов.НайтиСтроки(Отбор);
	ФорматУПД_5_01_ПродолжаетДействовать = ЗначениеЗаполнено(НайденныеФорматы);
	
	Возврат Не ФорматУПД_5_03_ЗагруженВИнформационнуюБазу Или ФорматУПД_5_01_ПродолжаетДействовать;
...
// Возвращаемое значение:
//  Булево
Функция ТребуетсяОбновленияФорматаВНастройкахОтправкиДляУПД_5_03(ПоддерживаемыеФорматы, ТекущийФормат,
	ВерсияФорматаУстановленаВручную, ФорматыЭлектронныхДокументов = Неопределено) Экспорт
	...
	Отбор = Новый Структура;
	Отбор.Вставить("ИдентификаторФормата", ТекущийФормат);
	Отбор.Вставить("Отключен", Истина);
	ОтключенныеФорматы = ФорматыЭлектронныхДокументов.НайтиСтроки(Отбор);
	
	ЭтоФормат551 = ТекущийФормат = ПоддерживаемыеФорматы.ПередачаТоваров.ИнформацияПродавца;
	ЭтоФормат552 = ТекущийФормат = ПоддерживаемыеФорматы.ПередачаРабот.ИнформацияПродавца;
	
	Возврат ОтключенныеФорматы.Количество() <> 0
		Или (Не ВерсияФорматаУстановленаВручную И (ЭтоФормат551 Или ЭтоФормат552));
...
//  ФорматыЭлектронныхДокументов - Неопределено
//                               - См. ЭлектронныеДокументыЭДО.ФорматыЭлектронныхДокументов
Процедура ОбновитьВерсиюФорматаВНастройкахОтправки(ТекущаяНастройка,
	ФорматыЭлектронныхДокументов = Неопределено) Экспорт
	...
	Отбор = Новый Структура;
	Отбор.Вставить("ВидДокумента", ТекущаяНастройка.ВидДокумента);
	Отбор.Вставить("Отключен", Ложь);
	...
	НовыеФорматыПоВидуДокумента = ФорматыЭлектронныхДокументов.НайтиСтроки(Отбор);
	Если НовыеФорматыПоВидуДокумента.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	НовыйФормат = НовыеФорматыПоВидуДокумента[0];
	
	НастройкаОтправки = НастройкиЭДОКлиентСервер.НоваяНастройкаОтправки();
	ЗаполнитьЗначенияСвойств(НастройкаОтправки, ТекущаяНастройка);
	
	НастройкаОтправки.Формат = НовыйФормат.ИдентификаторФормата;
	НастройкаОтправки.ВерсияФорматаУстановленаВручную = Ложь;
	
	УстановитьПривилегированныйРежим(Истина);
	ИзменитьНастройку(НастройкаОтправки);
	УстановитьПривилегированныйРежим(Ложь);
	
КонецПроцедуры

// Проверяет корректность установленного формата для вида документа
//
// Параметры:
//  ВидДокумента - СправочникСсылка.ВидыДокументовЭДО
//  ТекущийФормат - Строка
...
// Возвращаемое значение:
//  Булево
Функция УстановленныйФорматКорректенДляТекущегоВидаДокумента(ВидДокумента, ТекущийФормат,
	ФорматыЭлектронныхДокументов = Неопределено) Экспорт
	...
	ОтборФормализованныхДокументов = Новый Структура();
	ОтборФормализованныхДокументов.Вставить("ВидДокумента", ВидДокумента);
	ОтборФормализованныхДокументов.Вставить("Действует", Истина);
	СписокФормализованныхДокументов = ФорматыЭлектронныхДокументов.НайтиСтроки(ОтборФормализованныхДокументов);
	Если Не СписокФормализованныхДокументов.Количество() Тогда
		Возврат Истина;
	КонецЕсли;
	
	ОтборФорматов = Новый Структура();
	ОтборФорматов.Вставить("ВидДокумента", ВидДокумента);
	ОтборФорматов.Вставить("ИдентификаторФормата", ТекущийФормат);
	СписокПоддерживаемыхФорматыЭД = ФорматыЭлектронныхДокументов.НайтиСтроки(ОтборФорматов);
	
	Возврат ЗначениеЗаполнено(СписокПоддерживаемыхФорматыЭД);
...
// Возвращаемое значение:
//  Число
Функция КоличествоКонтрагентовСЗаданнымиНастройками(ВидДокумента, Формат) Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ НастройкиОтправкиЭлектронныхДокументовПоВидам.Получатель) КАК КоличествоКонтрагентов
		|ИЗ
		|	РегистрСведений.НастройкиОтправкиЭлектронныхДокументовПоВидам КАК НастройкиОтправкиЭлектронныхДокументовПоВидам
		|ГДЕ
		|	НастройкиОтправкиЭлектронныхДокументовПоВидам.ВидДокумента = &ВидДокумента
		|	И НастройкиОтправкиЭлектронныхДокументовПоВидам.ВерсияФормата = &ВерсияФормата";
	
	Запрос.УстановитьПараметр("ВерсияФормата", Формат);
	Запрос.УстановитьПараметр("ВидДокумента", ВидДокумента);
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		Результат = ВыборкаДетальныеЗаписи.КоличествоКонтрагентов;
	КонецЕсли;
...
//  НовыйФормат - Строка
// 
Процедура ИзменитьФорматВНастройкахОтправкиПоВидуДокументаДляВсехКонтрагентов(ВидДокумента, СтарыйФормат,
	НовыйФормат) Экспорт
	
	СообщениеПрогресс = НСтр("ru='Изменение формата в настройках ЭДО с контрагентами.
		|Обработано %1 из %2 настроек'");
	...
	Запрос.Текст =
		...
		|		ПО НастройкиОтправкиЭлектронныхДокументовПоВидам.Отправитель = НастройкиЗаполненияДополнительныхПолей.Отправитель
		|		И НастройкиОтправкиЭлектронныхДокументовПоВидам.Получатель = НастройкиЗаполненияДополнительныхПолей.Получатель
		|		И НастройкиОтправкиЭлектронныхДокументовПоВидам.Договор = НастройкиЗаполненияДополнительныхПолей.Договор
		|		И НастройкиОтправкиЭлектронныхДокументовПоВидам.ВидДокумента = НастройкиЗаполненияДополнительныхПолей.ВидЭлектронногоДокумента
		|		И НастройкиОтправкиЭлектронныхДокументовПоВидам.ВерсияФормата = НастройкиЗаполненияДополнительныхПолей.Формат
		|ГДЕ
		|	НастройкиОтправкиЭлектронныхДокументовПоВидам.ВидДокумента = &ВидДокумента
		|	И НастройкиОтправкиЭлектронныхДокументовПоВидам.ВерсияФормата = &ВерсияФормата";
	
	Запрос.УстановитьПараметр("ВерсияФормата", СтарыйФормат);
	Запрос.УстановитьПараметр("ВидДокумента", ВидДокумента);
	...
	ЭтоФормализованныйФорматБезОтветногоТитула =
		ЭлектронныеДокументыЭДО.ЭтоФормализованныйФорматБезОтветногоТитула(НовыйФормат);
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Для Каждого Запись Из НаборЗаписей Цикл
				...
				Запись.ВерсияФорматаУстановленаВручную = Истина;
				Если ЭтоФормализованныйФорматБезОтветногоТитула Тогда
					Запись.ТребуетсяОтветнаяПодпись = Ложь;
				КонецЕсли;
				
				Если СчетчикНастроек % 10 = 0 Тогда
					ДлительныеОперации.СообщитьПрогресс(, СтрШаблон(СообщениеПрогресс, СчетчикНастроек,
						КоличествоНастроек));
				КонецЕсли;
				
				СчетчикНастроек = СчетчикНастроек + 1;
		...
		Исключение
			...
			ВидОперации = НСтр("ru = 'Изменение формата в настройках отправки ЭДО'");
		...
		Если ПереходНаНовыйФорматВыполнен И Выборка.НастройкиЗаполненияДополнительныхПолей <> Неопределено Тогда
			ДанныеДляПереносаНастроек = НовыеДанныеДляПереносаНастроекЗаполненияДополнительныхПолей();
			ЗаполнитьЗначенияСвойств(ДанныеДляПереносаНастроек, Выборка,
				"Отправитель, Получатель, Договор, ВидДокумента");
			ДанныеДляПереносаНастроек.СтарыйФормат = СтарыйФормат;
			ДанныеДляПереносаНастроек.НовыйФормат = НовыйФормат;
			ДанныеДляПереносаНастроек.Настройка = Выборка.НастройкиЗаполненияДополнительныхПолей;
			ПеренестиНастройкиЗаполненияДополнительныхПолей(ДанныеДляПереносаНастроек);
		КонецЕсли;
...
#Область НастройкиЗаполненияДополнительныхПолей

// Параметры:
//  ДанныеДляПереносаНастроек - См. НовыеДанныеДляПереносаНастроекЗаполненияДополнительныхПолей
Процедура ПеренестиНастройкиЗаполненияДополнительныхПолей(ДанныеДляПереносаНастроек) Экспорт
	
	НовыеДанныеНастройки = РегистрыСведений.НастройкиЗаполненияДополнительныхПолей.НовыеДанныеНастройкиЗаполненияПолей();
	ЗаполнитьЗначенияСвойств(НовыеДанныеНастройки, ДанныеДляПереносаНастроек, "Отправитель, Получатель, Договор");
	НовыеДанныеНастройки.ВидЭлектронногоДокумента = ДанныеДляПереносаНастроек.ВидДокумента;
	НовыеДанныеНастройки.Формат = ДанныеДляПереносаНастроек.НовыйФормат;
	
	РазделыДопПолейСтарогоФормата = ЭлектронныеДокументыЭДО.РазделыДополнительныхПолейФорматаЭлектронногоДокумента(
		ДанныеДляПереносаНастроек.ВидДокумента, ДанныеДляПереносаНастроек.СтарыйФормат);
		
	РазделыДопПолейНовогоФормата = ЭлектронныеДокументыЭДО.РазделыДополнительныхПолейФорматаЭлектронногоДокумента(
		ДанныеДляПереносаНастроек.ВидДокумента, ДанныеДляПереносаНастроек.НовыйФормат);
	
	ИменаРазделовДопПолейСтарогоФормата = РазделыДопПолейСтарогоФормата.ВыгрузитьКолонку("Имя");
	ИменаРазделовДопПолейНовогоФормата = РазделыДопПолейНовогоФормата.ВыгрузитьКолонку("Имя");
	
	Если ОбщегоНазначения.КоллекцииИдентичны(ИменаРазделовДопПолейСтарогоФормата,
		ИменаРазделовДопПолейНовогоФормата) Тогда
		НовыеДанныеНастройки.Настройка = ДанныеДляПереносаНастроек.Настройка.Получить();
		РегистрыСведений.НастройкиЗаполненияДополнительныхПолей.ЗаписатьНастройкуЗаполненияПолей(
			НовыеДанныеНастройки);
		Возврат;
	КонецЕсли;
	
	НастройкаJSON = ДанныеДляПереносаНастроек.Настройка.Получить();
	НаборПолейСтарогоФормата = ОбщегоНазначения.JSONВЗначение(НастройкаJSON,, Ложь);
	НаборПолейНовогоФормата = Новый Массив;
	
	Для Каждого ДанныеПоля Из НаборПолейСтарогоФормата Цикл
		Если ИменаРазделовДопПолейНовогоФормата.Найти(ДанныеПоля.Раздел) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		НаборПолейНовогоФормата.Добавить(ДанныеПоля);
	КонецЦикла;
	
	НовыеДанныеНастройки.Настройка = ОбщегоНазначения.ЗначениеВJSON(НаборПолейНовогоФормата);
	РегистрыСведений.НастройкиЗаполненияДополнительныхПолей.ЗаписатьНастройкуЗаполненияПолей(НовыеДанныеНастройки);
...
//  * Настройка - Неопределено
//              - ХранилищеЗначения
Функция НовыеДанныеДляПереносаНастроекЗаполненияДополнительныхПолей() Экспорт
	
	НовыеДанныеДляПереноса = Новый Структура;
	НовыеДанныеДляПереноса.Вставить("Отправитель", Неопределено);
	НовыеДанныеДляПереноса.Вставить("Получатель", Неопределено);
	НовыеДанныеДляПереноса.Вставить("Договор", Неопределено);
	НовыеДанныеДляПереноса.Вставить("ВидДокумента", Справочники.ВидыДокументовЭДО.ПустаяСсылка());
	НовыеДанныеДляПереноса.Вставить("СтарыйФормат", "");
	НовыеДанныеДляПереноса.Вставить("НовыйФормат", "");
	НовыеДанныеДляПереноса.Вставить("Настройка", Неопределено);
	Возврат НовыеДанныеДляПереноса;
	
КонецФункции

#КонецОбласти // НастройкиЗаполненияДополнительныхПолей
...
Функция ТекущиеНастройкиОтправкиЭлектронныхДокументовПоВидам(Знач Организация, Знач Контрагент, Знач ДоговорКонтрагента, Знач ТаблицаНастроек) Экспорт
	...
	ИзменитьВерсиюФорматаУПДВТекущихНастройкахОтправкиДокументовПоВидам(Организация, Контрагент, ДоговорКонтрагента);
...
//                     - ОпределяемыйТип.ДоговорСКонтрагентомЭДО
Процедура ИзменитьВерсиюФорматаУПДВТекущихНастройкахОтправкиДокументовПоВидам(Организация, Контрагент,
	ДоговорКонтрагента)
	...
	Попытка
		...
		Для Каждого Запись Из НаборЗаписей Цикл
			...
			ЭтоФорматУПД2019 = Запись.ВерсияФормата = ПоддерживаемыеФорматы.УПД2019.ИнформацияПродавца;
			ЭтоФорматУПД_5_02 = Запись.ВерсияФормата = ПоддерживаемыеФорматы.УПД_5_02.ИнформацияПродавца;
			ЭтоФормат551 = Запись.ВерсияФормата = ПоддерживаемыеФорматы.ПередачаТоваров.ИнформацияПродавца;
			ЭтоФормат552 = Запись.ВерсияФормата = ПоддерживаемыеФорматы.ПередачаРабот.ИнформацияПродавца;
			
			Если ЭтоФорматУПД2019 Или ЭтоФорматУПД_5_02
				Или (Не Запись.ВерсияФорматаУстановленаВручную И (ЭтоФормат551 Или ЭтоФормат552)) Тогда
				ТребуетсяЗапись = Истина;
				Запись.ВерсияФормата = ПоддерживаемыеФорматы.УПД_5_03.ИнформацияПродавца;
			КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Изменение формата УПД в настройках отправки ЭДО'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ИзменитьФорматВНастройкахОтправкиПоВидуДокументаДляВсехКонтрагентов", Истина);
