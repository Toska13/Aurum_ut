...
Процедура ЗаполнитьСведенияОМаркировкеУКД2020(Приемник, Источник, ДанныеШтрихкодовУпаковокДо, ДанныеШтрихкодовУпаковокПосле, СтандартнаяОбработка) Экспорт
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Источник, "КоличествоБазовыхЕдиницДо") Тогда
		Возврат;
	КонецЕсли;
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Источник, "Серия") Тогда
		ИсточникСтруктурой.Вставить("Серия", Источник.Серия);
	КонецЕсли;
	ИсточникСтруктурой.Вставить("КоличествоДо", Источник.КоличествоБазовыхЕдиницДо);
	ИсточникСтруктурой.Вставить("Количество", Источник.КоличествоБазовыхЕдиниц);
	ЭлектронноеВзаимодействиеИСМП.ЗаполнитьСведенияОМаркировкеУКД2020(Приемник, ИсточникСтруктурой, ДанныеШтрихкодовУпаковокДо, ДанныеШтрихкодовУпаковокПосле);
...
Процедура ЗаполнитьСведенияОбАдресахУчастниковОбмена_5_02(СтруктураДанных, Источник, СтруктураАдресов, Отказ, ВыводитьСообщение = Истина) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		Или ТипЗнч(Источник) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту")
		...
		Если ВыборкаШапки.Следующий() Тогда
			...
			ВидыПродукции = ОбщегоНазначения.ВыгрузитьКолонку(СтруктураДанных.Маркировка, "ВидПродукции", Истина);
			ПараметрыУказанияДанных = ЭлектронноеВзаимодействиеИСМП.ПараметрыУказанияДанныхПоМОДВЭлектронномДокументеУПД_5_02(ВидыПродукции, ВыборкаШапки.Дата);
			
			Если Не ПараметрыУказанияДанных.ПрисутствуетПрослеживаемаяПоМОДПродукция Тогда
				Возврат;
			КонецЕсли;
...
Процедура ДобавитьСведенияОПеревозочныхДокументах_5_02(СтруктураДанных, Источник, ДанныеПеревозочныхДокументов) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		...
		Если ВыборкаШапки.Следующий() Тогда
			...
			Если Не ЗаполнениеПроизведено
				И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ВыборкаШапки, "Исправление") 
				И ВыборкаШапки.Исправление
				И ВыборкаШапки.ДокументОснование = Источник Тогда
				ДобавитьСведенияОПеревозочныхДокументах_5_02(СтруктураДанных, ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Источник, "ИсправляемыйДокумент"), ДанныеПеревозочныхДокументов);
			КонецЕсли;
...
//  СтандартнаяОбработка - Булево - признак стандартной обработки.
Процедура ПриОпределенииПредставленияЭлектронногоДокументаПоФайлу(СообщениеОтправителя, ДвоичныеДанныеОсновногоФайла, ДвоичныеДанныеОтветногоФайла, ПараметрыВизуализации, ТабличныйДокумент, СтандартнаяОбработка) Экспорт
