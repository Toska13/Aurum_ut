...
#Область Продажи
	...
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										...
										"Организация");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										Метаданные.Документы.ПоступлениеТоваровОтКлиента.ПолноеИмя(),
										"Дата",
										"Продажи",
										"Организация");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										Метаданные.Документы.СписаниеОтгруженныхТоваров.ПолноеИмя(),
										"Дата",
										"Продажи",
										"Организация");
	...
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										...
										"Организация");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										Метаданные.Документы.ОприходованиеИзлишковОтгруженныхТоваров.ПолноеИмя(),
										"Дата",
										"Продажи",
										"Организация");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
...
#Область СверкиИКорректировкиЗадолженности
	...
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										...
										"Организация");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										Метаданные.Документы.КорректировкаГрафикаВзаиморасчетов.ПолноеИмя(),
										"Дата",
										"СверкиИКорректировкиЗадолженности",
										"Организация");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										...
										"ОрганизацияПолучатель");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										Метаданные.Документы.КорректировкаРасчетовСПодотчетнымиЛицами.ПолноеИмя(),
										"Дата",
										"СверкиИКорректировкиЗадолженности",
										"Организация");
#КонецОбласти
...
#Область СкладскиеОперации
	...
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										...
										"Организация");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										Метаданные.Документы.Рейс.ПолноеИмя(),
										"ДатаВыезда",
										"СкладскиеОперации",
										"СкладОтправления");
#КонецОбласти
...
#Область ФинансовыйКонтур
	...
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										...
										"Организация");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										Метаданные.Документы.ОперацияСЭлектроннымБилетом.ПолноеИмя(),
										"Дата",
										"ФинансовыйКонтур",
										"Организация");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										Метаданные.РегистрыНакопления.ДвиженияПоАкцизам.ПолноеИмя(),
										"Период",
										"ФинансовыйКонтур",
										"Организация");
...
Процедура ПередПроверкойЗапретаИзменения(Объект, ПроверкаЗапретаИзменения, УзелПроверкиЗапретаЗагрузки, ВерсияОбъекта) Экспорт
	...
	Если ТипРегистратора = Тип("ДокументСсылка.ОперацияСЭлектроннымБилетом") Тогда
		ПроверкаЗапретаИзменения = Ложь;
	КонецЕсли;
	...
	Если ТипОбъекта = Тип("ДокументОбъект.Рейс") Тогда
		
		ПроверятьСтаруюВерсию = Ложь;
		ПроверятьНовуюВерсию = (Объект.ДатаВыезда <> Дата(1, 1, 1));
		
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			
			ДатаСтаройВерсии = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "ДатаВыезда");
			Если ДатаСтаройВерсии <> Дата(1, 1, 1) Тогда
				
				ПроверятьСтаруюВерсию = Истина;
				
			КонецЕсли;
			
		КонецЕсли;
		
		Если НЕ ПроверятьСтаруюВерсию
		   И НЕ ПроверятьНовуюВерсию Тогда
	...
	Если РасчетСебестоимостиПрикладныеАлгоритмы.ЭтоСлужебныйДокументКонвертацииОстатков(Объект)
	 Или РасчетСебестоимостиПрикладныеАлгоритмы.ЭтоСлужебныйДокументКонвертацииОстатков(Регистратор) Тогда
		ПроверкаЗапретаИзменения = Ложь;
	КонецЕсли;
