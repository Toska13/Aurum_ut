...
Процедура СверитьДокументСТорговойПлощадкой(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	ПараметрыДиалога.Фильтр             = НСтр("ru = 'XML файл'") + " (*.xml)|*.xml|" 
											+ НСтр("ru = 'Книга Excel 97'") + " (*.xls)|*.xls|"
											+ НСтр("ru = 'Книга Excel 2007'") + " (*.xlsx)|*.xlsx|"
											+ СтрШаблон(НСтр("ru = 'Электронная таблица %1'"), "OpenDocument") + " (*.ods)|*.ods|"
											+ НСтр("ru = 'Все файлы'") + " (*.*)|*.*";
...
Процедура ОбработкаИзмененияПолей(Форма, Элемент = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Форма.ИмяФормы = "Справочник.Номенклатура.Форма.ФормаЭлемента" Тогда
		...
		Если ИмяЭлемента = "ВидНоменклатуры"
			...
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ВходящиеПараметры, "НоменклатураКонтрагента")
					И ЗначениеЗаполнено(ВходящиеПараметры.НоменклатураКонтрагента) Тогда
...
Функция ПодготовитьПараметрыФормыСопоставленияИзВыгрузкиЗагрузкиДанных(Знач ТаблицаСопоставленияДанных,
	...
	Если Параметры.ИмяОбъектаМетаданных = "Обработка.УправлениеПродажамиНаOzon" Тогда
		ПараметрыФормы.ПолеОбработчикаСобытияПриИзменении =
			ПолеОбработчикаСобытияПриИзмененииПриСопоставленииДанных(Параметры.ИмяОбъектаМетаданных);
...
//  Строка
//
Функция ПолеОбработчикаСобытияПриИзмененииПриСопоставленииДанных(ИмяОбъектаМетаданных) Экспорт
	
	ПолеОбработчикаСобытияПриИзменении = "";
	
	Если ИмяОбъектаМетаданных = "Обработка.УправлениеПродажамиНаOzon" Тогда
		ПолеОбработчикаСобытияПриИзменении = "ТаблицаСопоставленияДанных_" + "Номенклатура";
	КонецЕсли;
	
	Возврат ПолеОбработчикаСобытияПриИзменении
...
Функция ДополнительныеПараметрыФормыЭлементаНоменклатуры(Параметры, НаборНоменклатурыКонтрагентов)
	...
	ДополнительныеПараметры.Вставить("НоменклатураКонтрагента", НаборНоменклатурыКонтрагентов[0]);
