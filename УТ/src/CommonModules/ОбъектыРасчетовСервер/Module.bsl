...
Функция РеквизитыОбъектаРасчетовПоСтруктуре(Объект, Ссылка, СтруктураПараметров, РежимЗаписи) Экспорт
	...
	НаправлениеДеятельности = ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Объект, СтруктураПараметров.НаправлениеДеятельности,, Справочники.НаправленияДеятельности.ПустаяСсылка());
	Если ЗначениеЗаполнено(НаправлениеДеятельности) Тогда
		ЗначенияРеквизитовНД = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НаправлениеДеятельности, "УчетДоходов,УчетРасчетовСПоставщиками");
		Если СтруктураПараметров.ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСКлиентом И НЕ ЗначенияРеквизитовНД.УчетДоходов
			ИЛИ СтруктураПараметров.ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСПоставщиком И НЕ ЗначенияРеквизитовНД.УчетРасчетовСПоставщиками Тогда
				НаправлениеДеятельности = Справочники.НаправленияДеятельности.ПустаяСсылка();
		КонецЕсли;
	КонецЕсли;
	ПараметрыОбъектаРасчетов.Вставить("НаправлениеДеятельности", НаправлениеДеятельности);
	...
	ПараметрыОбъектаРасчетов.Вставить("Служебный", СтруктураПараметров.ОбъектРасчетовСлужебный);
...
Функция СсылкаЯвляетсяОбъектомРасчетов(Объект, СтруктураПараметров) Экспорт
	...
	МожетЯвлятьсяОбъектомРасчетов = СтруктураПараметров.МожетЯвлятьсяОбъектомРасчетов;
	...
	Возврат МожетЯвлятьсяОбъектомРасчетов
		И (ЭтоСправочник
...
// Получает объект расчетов по основанию платежа
// 
// Параметры:
// 	ОснованиеПлатежа - СправочникСсылка, ДокументСсылка - 
// 
// Возвращаемое значение:
// 	- СправочникСсылка.ОбъектыРасчетов - Найденный объект расчетов.
//
Функция ОбъектРасчетовПоОснованиюПлатежа(Знач ОснованиеПлатежа) Экспорт
	
	СсылкаДляПоиска = ОснованиеПлатежа;
	
	Если ТипЗнч(ОснованиеПлатежа) = Тип("ДокументСсылка.СчетНаОплатуКлиенту")
	//++ Локализация
		Или ТипЗнч(ОснованиеПлатежа) = Тип("ДокументСсылка.СчетФактураВыданный")
	//-- Локализация
	Тогда
		ДокументОснование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОснованиеПлатежа, "ДокументОснование");
		Если ЗначениеЗаполнено(ДокументОснование) Тогда
			СсылкаДляПоиска = ДокументОснование;
		КонецЕсли;
	КонецЕсли;
	
	ОбъектРасчетовПоСсылке = ПолучитьОбъектРасчетовПоСсылке(СсылкаДляПоиска);
	
	Если ЗначениеЗаполнено(ОбъектРасчетовПоСсылке) Тогда
		Возврат ОбъектРасчетовПоСсылке;
	КонецЕсли;
	
	ОбъектРасчетовИзСсылки = ОбъектРасчетовИзСсылки(СсылкаДляПоиска);
	
	Если ТипЗнч(ОбъектРасчетовИзСсылки) = Тип("Массив") Тогда
		Если ОбъектРасчетовИзСсылки.Количество() > 0 Тогда
			ОбъектРасчетов = ОбъектРасчетовИзСсылки[0];
		Иначе
			ОбъектРасчетов = Неопределено;
		КонецЕсли;
	Иначе
		ОбъектРасчетов = ОбъектРасчетовИзСсылки;
	КонецЕсли;
...
Процедура ОчиститьОбъектыРасчетов(Объект, МассивСтруктур)
	...
	Для Каждого СтруктураПараметров Из МассивСтруктур Цикл
		...
		Если ЗначениеЗаполнено(СтруктураПараметров.ПутьКДаннымТЧРасшифровкаПлатежа)
			...
			ОбъектыРасчетов = РасшифровкаПлатежа.ВыгрузитьКолонку("ОбъектРасчетов");
			...
			Для Каждого СтрокаТЧ Из РасшифровкаПлатежа Цикл
				Если ОбъектыОбъектовРасчетов[СтрокаТЧ.ОбъектРасчетов] = Ссылка Тогда
...
Процедура ЗаполнитьОчиститьОбъектРасчетов(Объект, МассивСтруктур, РежимЗаписи = Неопределено)
	...
	Для Каждого СтруктураПараметров Из МассивСтруктур Цикл
		...
		Если РеквизитОбъектРасчетов = Неопределено
		...
		ОбъектРасчетовНеНужноЗаполнять = Ложь;
		//Определение.
		ДополнительныеКритерииПоиска.ВернутьПервый = Истина;
		Если СсылкаЯвляетсяОбъектомРасчетов(Объект, СтруктураПараметров) Тогда
		...
		ИначеЕсли ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов
			И (ЭтоЗаказ ИЛИ ЭтоПродажаЗакупка ИЛИ ЭтоПлатежИлиПрочийДокумент И ЗначениеЗаполнено(СтруктураПараметров.ОбъектРасчетов)) И ИзменяетРасчеты
			ИЛИ ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоДоговорамНакладным И ЭтоЗаказ И ИзменяетРасчеты Тогда
		...
		ИначеЕсли ЭтоПродажаЗакупка И НакладнаяПоЗаказам И НЕ ЗначениеЗаполнено(ИмяРеквизитаТЧЗаказ) И ЗначениеЗаполнено(ЗаказОснование) И ИзменяетРасчеты
			ИЛИ ЭтоПлатежИлиПрочийДокумент И ЗначениеЗаполнено(ЗаказОснование) И ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоЗаказам И ИзменяетРасчеты Тогда
		...
		ИначеЕсли ЭтоПродажаЗакупка И НакладнаяПоЗаказам И ЗначениеЗаполнено(ИмяРеквизитаТЧЗаказ) 
			И ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоЗаказам И ИзменяетРасчеты Тогда
		...
		ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.КорректировкаРеализации")
			ИЛИ ТипЗнч(Ссылка) = Тип("ДокументСсылка.КорректировкаПриобретения") Тогда
			ОбъектРасчетов = ПолучитьОбъектРасчетовПоСсылке(
					ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Объект, "Объект.ДокументОснование"),
					Организация,
		...
		Иначе
			// Очищаем реквизит
			ОбъектРасчетов = Справочники.ОбъектыРасчетов.ПустаяСсылка();
			ОбъектРасчетовНеНужноЗаполнять = Истина;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ОбъектРасчетов) И Не ОбъектРасчетовНеНужноЗаполнять 
