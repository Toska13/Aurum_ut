...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Форма.ИмяФормы = "Справочник.Номенклатура.Форма.ФормаЭлемента" Тогда
		...
		Если ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени("Подсистема.Продажи.Подсистема.РаботаСТорговымиПлощадками") <> Неопределено Тогда
			МодульИнтеграцияСМаркетплейсамиСервер = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияСМаркетплейсамиСервер");
			МодульИнтеграцияСМаркетплейсамиСервер.ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка, ДополнительныеПараметры);
		КонецЕсли;
	КонецЕсли;
	...
	Если Форма.ИмяФормы = "Справочник.Номенклатура.Форма.ФормаСписка" Тогда
		ИнтеграцияИСУТ.ДобавитьКомандыНастройкиИнтеграцииСФГИСВФормуСпискаНоменклатуры(Форма);
	КонецЕсли;
	...
	Если Форма.ИмяФормы = "Справочник.Номенклатура.Форма.ФормаВыбора" Тогда
		...
		Если ПараметрыОтбора <> Неопределено Тогда
			...
			ИнтеграцияИСУТ.ПереопределитьТекстЗапросаДинамическогоСпискаНоменклатурыДляОтбораПоВидуПродукции(
				Форма.СписокНоменклатура.ТекстЗапроса, ПараметрыОтбора.ВидыПродукцииИС);
			ИнтеграцияИСУТ.УстановитьОтборНоменклатурыПоВидуПродукцииИС(Форма, ПараметрыОтбора.ВидыПродукцииИС, ПараметрыОтбора.ВидСравненияОтбора);
		КонецЕсли;
	КонецЕсли;
	...
	Если Форма.ИмяФормы = "Справочник.СертификатыНоменклатуры.Форма.ФормаСпискаКонтекст"
			И ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени("Подсистема.Продажи.Подсистема.РаботаСТорговымиПлощадками") <> Неопределено Тогда
		МодульИнтеграцияСМаркетплейсамиСервер = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияСМаркетплейсамиСервер");
		МодульИнтеграцияСМаркетплейсамиСервер.ПриСозданииФормыСпискаКонтекстСертификатовНоменклатуры(Форма);
	КонецЕсли;
	
	Если Форма.Параметры.Свойство("ДополнительныеСвойстваДинамическихСписков")
			И ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени("Подсистема.Продажи.Подсистема.РаботаСТорговымиПлощадками") <> Неопределено Тогда
		МодульИнтеграцияСМаркетплейсамиСерверЛокализация = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияСМаркетплейсамиСерверЛокализация");
		МодульИнтеграцияСМаркетплейсамиСерверЛокализация.ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка, ДополнительныеПараметры);
	КонецЕсли;
	...
	Если Форма.ИмяФормы = "РегистрСведений.ШтрихкодыНоменклатуры.Форма.ФормаЗаписи" Тогда
		Если Не ЗначениеЗаполнено(Форма.Запись.ИсходныйКлючЗаписи.Штрихкод) Тогда
			ИнтеграцияИСУТ.ПриСозданииНаСервереФормыШтрихкодаНоменклатуры(Форма, ДополнительныеПараметры);
			ИнтеграцияИСУТ.ЗаполнитьПараметрыФормыШтрихкодаНоменклатуры(Форма);
			ИнтеграцияИСУТ.НастроитьЭлементыФормыШтрихкодаНоменклатуры(Форма);
		КонецЕсли;
	КонецЕсли;
	
	Если Форма.ИмяФормы = "РегистрСведений.ШтрихкодыНоменклатуры.Форма.ФормаСписка" Тогда
		ИнтеграцияИСУТ.ПриСозданииНаСервереФормыСпискаШтрихкодаНоменклатуры(Форма, ДополнительныеПараметры);
	КонецЕсли;
	//-- Локализация
...
Процедура ПриЗаписиНаСервере(Форма, Отказ, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	Если Форма.ИмяФормы = "РегистрСведений.ШтрихкодыНоменклатуры.Форма.ФормаЗаписи" Тогда
		ИнтеграцияИСУТ.ПриЗаписиНаСервереШтрихкодаНоменклатуры(Форма, Отказ, ТекущийОбъект, ПараметрыЗаписи);
		ИнтеграцияИСУТ.ЗаполнитьПараметрыФормыШтрихкодаНоменклатуры(Форма);
		ИнтеграцияИСУТ.НастроитьЭлементыФормыШтрихкодаНоменклатуры(Форма);
	КонецЕсли;
	//-- Локализация
