...
//
// Параметры:
//  Графики       - Массив из СправочникСсылка.Календари - массив элементов для которых составляются расписания.
//  ДатаНачала    - Дата   - дата начала периода, за который нужно составить расписания.
//  ДатаОкончания - Дата   - дата окончания периода.
//  ПереходящиеСмены - Булево - если Истина, то интервалы расписания могут пересекать границу суток.
//    Например может быть возвращен интервал "03 апр. 20:00-04:00" если график настроен соответствующим образом.
//    Если значение параметра Ложь, то аналогичное расписание будет представлено двумя интервалами:
//    "03 апр. 20:00-00:00" и "04 апр. 00:00-04:00".
//
// Возвращаемое значение:
//   ТаблицаЗначений:
...
//
Функция РасписанияРаботыНаПериод(Графики, ДатаНачала, ДатаОкончания, ПереходящиеСмены = Ложь) Экспорт
	
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	
	// Создаем временную таблицу расписаний.
	СоздатьВТРасписанияРаботыНаПериод(МенеджерВременныхТаблиц, Графики, ДатаНачала, ДатаОкончания, ПереходящиеСмены);
	...
	Если ПереходящиеСмены Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеУпорядочивания", "ГрафикРаботы, ДатаГрафика, НомерСтроки");
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеУпорядочивания", "ГрафикРаботы, ДатаГрафика, ВремяНачала");
	КонецЕсли;
...
//    "03 апр. 20:00-00:00" и "04 апр. 00:00-04:00".
//
Процедура СоздатьВТРасписанияРаботыНаПериод(МенеджерВременныхТаблиц, Графики, ДатаНачала, ДатаОкончания, ПереходящиеСмены = Ложь) Экспорт
	...
	|			И (ДанныеПроизводственногоКалендаря.Дата МЕЖДУ &ДатаНачалаПредшествующая И &ДатаОкончания)
	...
	|			И (КалендарныеГрафики.ДатаГрафика МЕЖДУ &ДатаНачалаПредшествующая И &ДатаОкончания)
	|			И (КалендарныеГрафики.ДеньВключенВГрафик)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РасписаниеРаботы.Ссылка      КАК Ссылка,
	|	РасписаниеРаботы.НомерДня    КАК НомерДня,
	|	РасписаниеРаботы.НомерСтроки КАК НомерСтроки,
	|	РасписаниеРаботы.ВремяНачала КАК ВремяНачала,
	|	ВЫБОР
	|		КОГДА РасписаниеРаботы.ВремяОкончания > РасписаниеРаботы.ВремяНачала
	|			ТОГДА РасписаниеРаботы.ВремяОкончания
	|		ИНАЧЕ НАЧАЛОПЕРИОДА(РасписаниеРаботы.ВремяНачала, ДЕНЬ)
	|	КОНЕЦ                        КАК ВремяОкончания,
	|	ЛОЖЬ                         КАК ПереходящаяЧасть
	|ПОМЕСТИТЬ ВТРасписаниеРаботы
	|ИЗ
	|	Справочник.Календари.РасписаниеРаботы КАК РасписаниеРаботы
	|ГДЕ
	|	НЕ &ПереходящиеСмены
	|	И РасписаниеРаботы.Ссылка В(&Календари)
	|	И (РасписаниеРаботы.ВремяОкончания < РасписаниеРаботы.ВремяНачала
	|			И РасписаниеРаботы.ВремяОкончания <> НАЧАЛОПЕРИОДА(РасписаниеРаботы.ВремяНачала, ДЕНЬ)
	|		ИЛИ НЕ ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	|				Справочник.Календари.РасписаниеРаботы КАК Т
	|			ГДЕ
	|				Т.Ссылка = РасписаниеРаботы.Ссылка
	|				И Т.НомерДня = РасписаниеРаботы.НомерДня
	|				И Т.НомерСтроки < РасписаниеРаботы.НомерСтроки
	|				И (Т.ВремяНачала >= РасписаниеРаботы.ВремяНачала
	|					ИЛИ Т.ВремяОкончания > РасписаниеРаботы.ВремяНачала
	|					ИЛИ Т.ВремяОкончания <= Т.ВремяНачала)))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА РасписаниеРаботы.ВремяОкончания > РасписаниеРаботы.ВремяНачала
	|				ИЛИ РасписаниеРаботы.ВремяОкончания = НАЧАЛОПЕРИОДА(РасписаниеРаботы.ВремяНачала, ДЕНЬ)
	|			ТОГДА РасписаниеРаботы.ВремяНачала
	|		ИНАЧЕ НАЧАЛОПЕРИОДА(РасписаниеРаботы.ВремяНачала, ДЕНЬ)
	|	КОНЕЦ                           КАК ВремяНачала,
	|	РасписаниеРаботы.ВремяОкончания КАК ВремяОкончания,
	|	ИСТИНА                          КАК ПереходящаяЧасть
	|ИЗ
	|	Справочник.Календари.РасписаниеРаботы КАК РасписаниеРаботы
	|ГДЕ
	|	НЕ &ПереходящиеСмены
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДниВключенныеВГрафик.ГрафикРаботы КАК ГрафикРаботы,
	|	ДниВключенныеВГрафик.ДатаГрафика  КАК Период,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(РасписанияРаботыПредпраздничногоДня.ПереходящаяЧасть, ЕСТЬNULL(РасписанияРаботы.ПереходящаяЧасть, ЛОЖЬ)) = ЛОЖЬ
	|			ТОГДА ДниВключенныеВГрафик.ДатаГрафика
	|		ИНАЧЕ ДОБАВИТЬКДАТЕ(ДниВключенныеВГрафик.ДатаГрафика, ДЕНЬ, 1)
	|	КОНЕЦ                             КАК ДатаГрафика,
	|	ДниВключенныеВГрафик.НомерДня     КАК НомерДня,
	|	ЕСТЬNULL(РасписанияРаботыПредпраздничногоДня.НомерСтроки, РасписанияРаботы.НомерСтроки) КАК НомерСтроки,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТРасписаниеРаботы КАК РасписанияРаботы
	|		ПО (РасписанияРаботы.Ссылка = ДниВключенныеВГрафик.ГрафикРаботы)
	|			И (РасписанияРаботы.НомерДня = ДниВключенныеВГрафик.НомерДня)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТРасписаниеРаботы КАК РасписанияРаботыПредпраздничногоДня
	...
	Если ПереходящиеСмены Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ДатаНачалаПредшествующая", "&ДатаНачала");
	Иначе
		Запрос.УстановитьПараметр("ДатаНачалаПредшествующая", НачалоДня(ДатаНачала - 86400));
	КонецЕсли;
