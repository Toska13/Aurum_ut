...
Функция ТекстыЗапросовСторнирования(МетаданныеДокумента) Экспорт
	...
	Если ДвиженияДокумента.Содержит(МетаданныеРегистра) Тогда
		...
		|	НАЧАЛОПЕРИОДА(ДанныеРегистра.Период, МЕСЯЦ) КАК ПериодПервичныхДвижений,
		|	ВЫБОР 
		|		КОГДА ДанныеРегистра.ПериодОтклонения <> ДАТАВРЕМЯ(1,1,1)
		|			ТОГДА ДанныеРегистра.ПериодОтклонения
		|		КОГДА ДанныеРегистра.Период >= НАЧАЛОПЕРИОДА(ДокументыСторно.Дата, МЕСЯЦ)
		...
		Исключения.Добавить("ПериодОтклонения");
	...
	Если ДвиженияДокумента.Содержит(МетаданныеРегистра) Тогда
		...
		// см. РасчетСебестоимостиПодготовкаДанных.ТекстЗапросаСтатьиСРаздельнымУчетомНДСВРегистреПартииПрочихРасходов()
...
Функция ТекстСторнирующегоЗапроса(МетаданныеРегистра, МетаданныеДокумента) Экспорт
	...
	|	ДокументыСторно.Ссылка  КАК Регистратор,
	|	ДокументыСторно.Дата КАК Период,
	|	ДанныеРегистра.Период КАК ПериодКорректировки,
	|	&ПоляВыборки,
	|	ИСТИНА КАК Сторно
	|ИЗ
	...
	ПереопределяемыеПоля.Добавить("ПериодКорректировки");
	ПереопределяемыеПоля.Добавить("ПериодОтклонения");
	ПереопределяемыеПоля.Добавить("ДокументДвижения");
	...
	ЕстьПериодОтклонения = МетаданныеРегистра.Реквизиты.Найти("ПериодОтклонения") <> Неопределено;
	ЕстьДокументДвижения = МетаданныеРегистра.Реквизиты.Найти("ДокументДвижения") <> Неопределено;
		...
		Если МетаданныеРегистра.Измерения.Найти(ИмяПоля) = Неопределено Тогда
			...
			ИначеЕсли ИмяПоля = "РасчетСебестоимости" И ЕстьРасчетСебестоимости Тогда
				ШаблонПоля = 
					...
					|		ИНАЧЕ ДанныеРегистра.РасчетСебестоимости
					|	КОНЕЦ КАК РасчетСебестоимости";
			ИначеЕсли ЕстьПериодОтклонения И ИмяПоля = "ПериодОтклонения" Тогда
				ШаблонПоля = 
					"	ВЫБОР
					|		КОГДА ДанныеРегистра.ПериодОтклонения <> ДАТАВРЕМЯ(1,1,1)
					|			ТОГДА ДанныеРегистра.ПериодОтклонения
					...
					|			ТОГДА ДАТАВРЕМЯ(1,1,1)
					|		ИНАЧЕ ДанныеРегистра.Период КОНЕЦ КАК ПериодОтклонения";
			ИначеЕсли ЕстьДокументДвижения И ИмяПоля = "ДокументДвижения" Тогда
				ШаблонПоля = 
					"	ВЫБОР
					|		КОГДА ДанныеРегистра.ДокументДвижения = ЗНАЧЕНИЕ(Документ.РаспределениеПрочихЗатрат.ПустаяСсылка)
					|			ТОГДА НЕОПРЕДЕЛЕНО
					|		ИНАЧЕ ДанныеРегистра.ДокументДвижения КОНЕЦ КАК ДокументДвижения";
			Иначе
				Продолжить;
			КонецЕсли;
		...
		Иначе
			...
			ИначеЕсли ИмяПоля = "ВидДеятельностиНДС" Тогда
				ШаблонПоля = 
					...
					|		ИНАЧЕ ДанныеРегистра.ВидДеятельностиНДС
					|	КОНЕЦ КАК ВидДеятельностиНДС";
			ИначеЕсли ИмяПоля = "ПериодОтклонения" Тогда
				ШаблонПоля = 
					"	ВЫБОР
					|		КОГДА ДанныеРегистра.ПериодОтклонения <> ДАТАВРЕМЯ(1,1,1)
					|			ТОГДА ДанныеРегистра.ПериодОтклонения
...
Функция ТекстЗапросаПрочиеРасходы(Запрос)
	...
	|ИЗ
	|	ВтДатыТаблицаИзмененийПрочиеРасходы КАК ДатыТаблицаИзменений
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаФинансовогоУчета КАК УчетнаяПолитика
	|		ПО ДатыТаблицаИзменений.Организация = УчетнаяПолитика.Организация
	|		И ДатыТаблицаИзменений.НачалоПериода >= УчетнаяПолитика.Период
	|СГРУППИРОВАТЬ ПО
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаФинансовогоУчета КАК УчетнаяПолитика
	|		ПО ВтПериодыУчетнойПолитики.Период = УчетнаяПолитика.Период
	|		И ВтПериодыУчетнойПолитики.Организация = УчетнаяПолитика.Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|";
	
	ТекстУничтожитьВт = "ВтДатыТаблицаИзмененийПрочиеРасходы, ВтПериодыУчетнойПолитики
	|";
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтУчетнаяПолитикаПоОрганизациям КАК УчетнаяПолитика
	|		ПО УчетнаяПолитика.Организация = ТаблицаИзменений.Организация
	|ГДЕ
	...
	// Если заполнен период отклонения, то необходимо сформировать задания к расчету себестоимости для распределения
	// расходов по базе прошлого периода. 
	|	ИЛИ ЕСТЬNULL(УчетнаяПолитика.РаспределятьОтклоненияВПрошлыхПериодах, ЛОЖЬ)
	|		И ТаблицаИзменений.ПериодОтклонения <> ДАТАВРЕМЯ(1,1,1)
	|		И ТаблицаИзменений.ПериодОтклонения < НАЧАЛОПЕРИОДА(ТаблицаИзменений.Период, МЕСЯЦ)
	|	)
