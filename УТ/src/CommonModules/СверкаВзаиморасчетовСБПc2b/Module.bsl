...
Функция ЗапросОтчетПоСверкеОборотов(НастройкаПодключения, НачалоПериода, КонецПериода) Экспорт
	...
	РезультатЗапроса = ПереводыСБПc2bСервис.ЗапросОтчетаПоСверкеОборотов(
...
Функция СостояниеОтчетаПоСверкеОборотов(НастройкаПодключения, ПараметрыЗапроса, ДлительностьОперации = 0) Экспорт
	...
	Если ДлительностьОперации <> 0 Тогда
		...
		Пока СистемаБыстрыхПлатежейСлужебный.ВозможенВызовОперации(НастройкиВызова) Цикл
			...
			РезультатЗапроса = ПереводыСБПc2bСервис.СостояниеОтчетаПоСверкеОборотов(
				ПараметрыНастройкиПодключения,
				ПараметрыЗапроса.Идентификатор,
				ПараметрыЗапроса.ДатаЗапросаСтатуса);
			
			Если РезультатЗапроса.СтатусОперации <> НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииВыполняется() Тогда
				Прервать;
			КонецЕсли;
...
Функция ЗапросОтчетаПоСверкеОпераций(НастройкаПодключения, НачалоПериода, КонецПериода) Экспорт
	...
	РезультатЗапроса = ПереводыСБПc2bСервис.ЗапросОтчетаПоСверкеОпераций(
		ПараметрыНастройкиПодключения,
		НачалоПериода,
		КонецПериода,
		ДатаЗапросаСтатуса);
...
Функция СостояниеОтчетаПоСверкеОпераций(
	...
	Если ДлительностьОперации <> 0 Тогда
		...
		Пока СистемаБыстрыхПлатежейСлужебный.ВозможенВызовОперации(НастройкиВызова) Цикл
			...
			РезультатЗапроса = ПереводыСБПc2bСервис.СостояниеОтчетаПоСверкеОпераций(
				ПараметрыНастройкиПодключения,
				ПараметрыЗапроса.Идентификатор,
				ПараметрыЗапроса.ДатаЗапросаСтатуса);
...
Функция ИдентификаторТипаОперацииОплата() Экспорт
	...
	Возврат ПереводыСБПc2bСервис.ИдентификаторТипаОперацииОплата();
...
Функция ИдентификаторТипаОперацииВозврат() Экспорт
	...
	Возврат ПереводыСБПc2bСервис.ИдентификаторТипаОперацииВозврат();
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("СверкаВзаиморасчетовПоОборотамИнтерактивныйЗапрос", Истина);
	Методы.Вставить("ПроверкаГотовностиСверкиВзаиморасчетовСБПc2b", Истина);
...
Процедура СлужебнаяПроверкаГотовностиСверкиВзаиморасчетовСБПc2b(СверкаВзаиморасчетов) Экспорт
	...
	Если ЗначениеЗаполнено(РезультатОперации.КодОшибки)
		Или РезультатОперации.СтатусОперации = НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииОшибка() Тогда
...
Функция СлужебнаяСверкаВзаиморасчетовПоОборотам(НастройкаПодключения, Знач НачалоПериода, Знач КонецПериода)
	...
	Если РезультатЗапроса.СтатусОперации = НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииОшибка() Тогда
	...
	ИначеЕсли РезультатЗапроса.СтатусОперации = НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииВыполнена() Тогда
...
Функция НовыйДокументСверкаВзаиморасчетовПоОборотам(ПараметрыЗаполнения)
	...
	СверкаОбъект.Заполнить(Неопределено);
...
//
// Параметры:
//  СверкаВзаиморасчетов - ДокументОбъект.СверкаВзаиморасчетовСБПc2b - объект для обновления;
...
Процедура ЗаполнитьСверкуВзаиморасчетов(ПараметрыЗаполнения)
	...
	ДокументыОплат = ПереводыСБПc2bСлужебный.ДокументыНастройкиПодключенияЗаПериод(
		...
		НастройкиПлатежныхСистемСлужебный.ДатаВUTC(ПараметрыЗаполнения.НачалоПериода),
		НастройкиПлатежныхСистемСлужебный.ДатаВUTC(ПараметрыЗаполнения.КонецПериода));
...
Функция СверкаВзаиморасчетовПоОперациям(СверкаВзаиморасчетов) Экспорт
	...
	Если РезультатЗапросаОтчета.СтатусОперации <> НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииВыполнена() Тогда
	...
	ОперацииСБП = ПереводыСБПc2bСлужебный.ОперацииНастройкиПодключенияЗаПериод(
		РеквизитыСверкиВзаиморасчетов.НастройкаПодключения,
		НастройкиПлатежныхСистемСлужебный.ДатаВUTC(РеквизитыСверкиВзаиморасчетов.НачалоПериода),
		НастройкиПлатежныхСистемСлужебный.ДатаВUTC(РеквизитыСверкиВзаиморасчетов.КонецПериода));
	...
	|				ИЛИ ВТ_ОперацииВИнформационнойБазе.ТипОперации ЕСТЬ NULL
