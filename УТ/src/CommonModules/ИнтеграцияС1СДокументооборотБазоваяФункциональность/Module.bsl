...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВыполнитьЗапросВФоне", Истина);
	Методы.Вставить("ВыполнитьОбменДанными", Истина);
	Методы.Вставить("ВыполнитьПакетныйЗапросВФоне", Истина);
	Методы.Вставить("ЗаписатьОбъектВФоне", Истина);
	Методы.Вставить("ПодключитьсяИПолучитьВерсиюСервисаВФоне", Истина);
	Методы.Вставить("ПолучитьОбъектВФоне", Истина);
...
#Область СтандартныеПодсистемы_РегламентныеЗадания
...
Процедура ПриОпределенииПсевдонимовОбработчиков(СоответствиеИменПсевдонимам) Экспорт
	...
	СоответствиеИменПсевдонимам.Вставить(Метаданные.РегламентныеЗадания.ОбменДаннымиС1СДокументооборотом.ИмяМетода);
...
Процедура ПриПолученииСпискаШаблонов(ШаблоныЗаданий) Экспорт
	...
	ШаблоныЗаданий.Добавить(Метаданные.РегламентныеЗадания.ОбменДаннымиС1СДокументооборотом.Имя);
...
Процедура ВыполнитьОбменДанными() Экспорт
	...
	ИмяСобытия = НСтр("ru = 'Отправка данных'");
	...
	ОтправитьДанные(Прокси, Настройки, ИмяСобытия);
...
// Создает новый объект ИС из объекта XDTO.
//
// Параметры:
//   МетаданныеОбъектаИС - ОбъектМетаданных - метаданные объекта ИС.
...
//   СсылкаПоИдентификатору - ЛюбаяСсылка - ссылка нового, созданная по идентификатору объекта XDTO.
//   Владелец - ЛюбаяСсылка - владелец элемента справочника, который нужно создать в ИС.
//            - Неопределено - если владелец отсутствует.
//
// Возвращаемое значение:
//   ЛюбаяСсылка - ссылка на созданный объект.
//
Функция НовыйОбъектИСИзОбъектаXDTO(МетаданныеОбъектаИС, ОбъектXDTO, ЗаполняемыйОбъектИС = Неопределено,
		СсылкаПоИдентификатору = Неопределено, Владелец = Неопределено) Экспорт
	...
	ИначеЕсли СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(ОбъектXDTO.objectID.id) Тогда
		СсылкаПоИдентификатору = Менеджер.ПолучитьСсылку(Новый УникальныйИдентификатор(ОбъектXDTO.objectID.id));
		НовыйОбъект.УстановитьСсылкуНового(СсылкаПоИдентификатору);
	КонецЕсли;
	...
	ПриСозданииПоСсылке(НовыйОбъект, ОбъектXDTO, ЗаполняемыйОбъектИС);
	
	НовыйОбъект.Записать();
	
	Если СвойствоУстановлено(ОбъектXDTO, "objectID")
			И ОбъектXDTO.objectID.ID <> ""
			И ОбъектXDTO.objectID.type <> ""
			И Не ОбъектДОПоддерживаетОбмен(ОбъектXDTO.objectID.type) Тогда
...
// Проверяет поддерживает ли объект ДО механизм обмена данными.
//
// Параметры:
...
//   Булево
//
Функция ОбъектДОПоддерживаетОбмен(ТипОбъектаДО) Экспорт
	...
	Настройки = ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ИспользоватьИнтеграцию();
	Если Настройки.ИспользоватьИнтеграциюДО2 Тогда
		ОбъектыСПоддержкойОбмена.Добавить("DMInternalDocument");
		ОбъектыСПоддержкойОбмена.Добавить("DMIncomingDocument");
		ОбъектыСПоддержкойОбмена.Добавить("DMOutgoingDocument");
	ИначеЕсли Настройки.ИспользоватьИнтеграциюДО3 Тогда
		ОбъектыСПоддержкойОбмена.Добавить("DMDocument");
		ОбъектыСПоддержкойОбмена.Добавить("DMMeeting");
	КонецЕсли;
	
	Возврат ОбъектыСПоддержкойОбмена.Найти(ТипОбъектаДО) <> Неопределено;
...
// Убирает лишние символы из выражения на встроенном языке.
//
// Параметры:
//   ВычисляемоеВыражение - Строка - исходное выражение.
//
// Возвращаемое значение:
//   Строка
//
Функция ВыражениеБезЛишнихСимволов(ВычисляемоеВыражение) Экспорт
	
	ВыражениеБезЛишнихСимволов = НРег(ВычисляемоеВыражение);
	ВыражениеБезЛишнихСимволов = СтрЗаменить(ВыражениеБезЛишнихСимволов, Символы.ВК, "");
	ВыражениеБезЛишнихСимволов = СтрЗаменить(ВыражениеБезЛишнихСимволов, Символы.ВТаб, "");
	ВыражениеБезЛишнихСимволов = СтрЗаменить(ВыражениеБезЛишнихСимволов, Символы.НПП, "");
	ВыражениеБезЛишнихСимволов = СтрЗаменить(ВыражениеБезЛишнихСимволов, Символы.ПС, "");
	ВыражениеБезЛишнихСимволов = СтрЗаменить(ВыражениеБезЛишнихСимволов, Символы.ПФ, "");
	ВыражениеБезЛишнихСимволов = СтрЗаменить(ВыражениеБезЛишнихСимволов, Символы.Таб, "");
	ВыражениеБезЛишнихСимволов = СтрЗаменить(ВыражениеБезЛишнихСимволов, " ", "");
	
	Возврат ВыражениеБезЛишнихСимволов;
...
Функция ЗависимостиКонстант() Экспорт
	...
	Если ЕстьПодсистемаДО3() Тогда
		...
		ДобавитьСтрокуТаблицыЗависимостиКонстант(
			...
			"ИспользоватьИнтеграциюС1СДокументооборот3",
			Ложь,
			"СоздаватьЭПДляОбъектов1СДокументооборотаСредствамиИС",
			Ложь);
...
Функция ПоддерживаетсяАутентификацияОС() Экспорт
	...
	Возврат (ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, "8.3.7.1") >= 0);
...
//     * ИспользоватьПеренаправлениеЗадач - Булево
...
//     * ПоказыватьPDFСредствами1С - Булево
...
Функция ПолучитьНастройки() Экспорт
	...
	Если СвойствоУстановлено(НастройкиXDTO, "useTasksForwarding") Тогда
		Настройки.Вставить("ИспользоватьПеренаправлениеЗадач", НастройкиXDTO.useTasksForwarding);
	Иначе
		Настройки.Вставить("ИспользоватьПеренаправлениеЗадач", Истина);
	КонецЕсли;
	...
	Если СвойствоУстановлено(НастройкиXDTO, "showPdfBy1C") Тогда
		Настройки.Вставить("ПоказыватьPDFСредствами1С", НастройкиXDTO.showPdfBy1C);
	Иначе
...
Функция ПолучитьПрокси(ВызыватьИсключение = Истина, НастройкиАвторизации = Неопределено,
	...
	ИспользоватьИмяВебСервиса = Истина;
	ОпределениеВебСервисаДО = ОпределениеВебСервисаДО(
		АдресВебСервисаДокументооборота,
		ДанныеСервиса,
		ИспользоватьИмяВебСервиса,
		НастройкиАвторизации,
		МестоположениеПрокси,
		ТекстСообщенияОбОшибке);
	Если ОпределениеВебСервисаДО = Неопределено Тогда
		ИспользоватьИмяВебСервиса = Ложь;
		ОпределениеВебСервисаДО = ОпределениеВебСервисаДО(
			АдресВебСервисаДокументооборота,
			ДанныеСервиса,
			ИспользоватьИмяВебСервиса,
...
Функция ПредставлениеТипаОбъектаXDTO(ТипXDTO, МножественноеЧисло = Ложь, ДляФормыВыбора = Ложь) Экспорт
	...
	Если Настройки.ИспользоватьИнтеграциюДО2 Тогда
		МодульИнтеграцияС1СДокументооборотКлиентСервер = ОбщегоНазначения.ОбщийМодуль(
			"ИнтеграцияС1СДокументооборотКлиентСервер");
		Возврат МодульИнтеграцияС1СДокументооборотКлиентСервер.ПредставлениеТипаОбъектаXDTO(
			ТипXDTO,
			МножественноеЧисло,
			ДляФормыВыбора);
		
	ИначеЕсли Настройки.ИспользоватьИнтеграциюДО3 Тогда
		МодульИнтеграцияС1СДокументооборот3КлиентСервер = ОбщегоНазначения.ОбщийМодуль(
			"ИнтеграцияС1СДокументооборот3КлиентСервер");
		Возврат МодульИнтеграцияС1СДокументооборот3КлиентСервер.ПредставлениеТипаОбъектаXDTO(
			ТипXDTO,
			МножественноеЧисло,
			ДляФормыВыбора);
...
Процедура ПриСозданииНаСервере(Форма, МестоРазмещенияКоманд = Неопределено, ПараметрыОповещения = Неопределено,
	...
	Если РеквизитФормыПоИмени(РеквизитыФормы, "ИмяРеквизитаОбъектаИнтегрируемогоС1СДокументооборотом") = Неопределено Тогда
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(
			...
			НСтр("ru = 'Имя реквизита, содержащего ссылку на интегрируемый с 1С:Документооборотом объект'")));
	КонецЕсли;
	...
	Если МестоРазмещенияКоманд = Неопределено И Форма.Элементы.Найти("Список") <> Неопределено
		...
		Для Каждого ЭлементФормы Из Форма.Элементы.Список.КоманднаяПанель.ПодчиненныеЭлементы Цикл
			Если ТипЗнч(ЭлементФормы) = Тип("ГруппаФормы") Тогда
				Для Каждого ЭлементГруппы Из ЭлементФормы.ПодчиненныеЭлементы Цикл
					Если ТипЗнч(ЭлементГруппы) = Тип("ГруппаФормы")
						...
						МестоРазмещенияКоманд = ЭлементФормы;
						Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			...
			КонецЕсли;
		КонецЦикла;
		Если МестоРазмещенияКоманд = Неопределено
				И Форма.Элементы.Список.ПоложениеКоманднойПанели <> ПоложениеКоманднойПанелиЭлементаФормы.Нет
				И Форма.Элементы.Список.КоманднаяПанель.ПодчиненныеЭлементы.Количество() > 0 Тогда
			МестоРазмещенияКоманд = Форма.Элементы.Список.КоманднаяПанель;
		КонецЕсли;
	КонецЕсли;
	Если МестоРазмещенияКоманд = Неопределено И Форма.ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиФормы.Нет Тогда
...
Процедура ПроверитьАлгоритмНаЗапрещенныеВыражения(Знач АлгоритмДляПроверки) Экспорт
	...
	АлгоритмДляПроверки = ВыражениеБезЛишнихСимволов(АлгоритмДляПроверки);
	...
	Исключения.Добавить(".ВыполнитьПакет()");
...
Функция РеквизитыОбъектаИС(ТипОбъектаИС, ДляВыражения = Ложь) Экспорт
	...
	Для Каждого МетаданныеРеквизита Из МетаданныеОбъекта.СтандартныеРеквизиты Цикл
		...
		Если МетаданныеРеквизита.Имя = "Код"
			...
			Если МетаданныеРеквизита.Имя = "ПометкаУдаления" Тогда
				Реквизит.Представление = НСтр("ru = 'Пометка удаления'");
			Иначе
	...
	Для Каждого МетаданныеРеквизита Из МетаданныеОбъекта.Реквизиты Цикл
		...
		Если СтрНачинаетсяС(МетаданныеРеквизита.Имя, "Удалить")
				Или СтрНайти(МетаданныеРеквизита.Синоним, НСтр("ru = '(Удалить)'")) > 0 Тогда
			Продолжить;
		КонецЕсли;
	...
	Если Метаданные.ФункциональныеОпции.Найти("ИспользоватьДополнительныеРеквизитыИСведения") <> Неопределено
		...
		Если УправлениеСвойствами.ИспользоватьДопРеквизиты(ВладелецСвойств) Тогда
			...
			Для Каждого ДопРеквизит Из НаборДопРеквизитов Цикл
				...
				Если Не ЗначениеЗаполнено(ДопРеквизит) Или Не ЗначениеЗаполнено(СвойстваДопРеквизита) Тогда
					Продолжить;
				КонецЕсли;
...
Функция СоздатьExternalObject(Прокси, ВнешнийОбъектИС = Неопределено, ID = "", Тип = "", Наименование = "") Экспорт
	...
	Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ВнешнийОбъектИС)) Тогда
		ВнешнийОбъектИСМетаданные = ВнешнийОбъектИС.Метаданные();
		НавигационнаяСсылка = "";
		Представление = Строка(ВнешнийОбъектИС);
		// Значения по-умолчанию.
		ExternalObject.name = Строка(ВнешнийОбъектИС);
		ExternalObject.type = ПолучитьИмяТипаСсылки(ВнешнийОбъектИС);
		Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ВнешнийОбъектИС)) Тогда
			Если Не Перечисления.ТипВсеСсылки().СодержитТип(ТипЗнч(ВнешнийОбъектИС)) Тогда
				НавигационнаяСсылка = ПолучитьНавигационнуюСсылку(ВнешнийОбъектИС);
			КонецЕсли;
			...
			Иначе
				Представление = ВнешнийОбъектИСМетаданные.Синоним;
			КонецЕсли;
		КонецЕсли;
		Если СвойствоСуществует(ExternalObject, "navigationRef") Тогда
			ExternalObject.navigationRef = НавигационнаяСсылка;
		КонецЕсли;
		Если СвойствоСуществует(ExternalObject, "presentation") Тогда
			ExternalObject.presentation = Представление;
		КонецЕсли;
		Если СвойствоСуществует(ExternalObject, "barcodeData")
				И (ОбщегоНазначения.ЭтоДокумент(ВнешнийОбъектИСМетаданные)
					Или ОбщегоНазначения.ЭтоСправочник(ВнешнийОбъектИСМетаданные)) Тогда
			ExternalObject.barcodeData = "";
...
Функция СоздатьExternalObjectID(Прокси, ВнешнийОбъектИС = Неопределено, ID = "", Тип = "") Экспорт
	...
	ExternalObjectID = СоздатьОбъект(Прокси, "ExternalObjectID");
	ExternalObjectID.ID = ID;
	ExternalObjectID.type = Тип;
	...
	Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ВнешнийОбъектИС)) Тогда
		...
		Представление = Строка(ВнешнийОбъектИС);
		// Значения по-умолчанию.
		ExternalObjectID.type = ПолучитьИмяТипаСсылки(ВнешнийОбъектИС);
		Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ВнешнийОбъектИС)) Тогда
			Если Не Перечисления.ТипВсеСсылки().СодержитТип(ТипЗнч(ВнешнийОбъектИС)) Тогда
				НавигационнаяСсылка = ПолучитьНавигационнуюСсылку(ВнешнийОбъектИС);
			КонецЕсли;
			Если Не ВнешнийОбъектИС.Пустая() Тогда
				// Уточним идентификатор внешнего объекта.
				ExternalObjectID.ID = ПолучитьIDПоСсылке(ВнешнийОбъектИС);
			Иначе
				Представление = ВнешнийОбъектИСМетаданные.Синоним;
			КонецЕсли;
		КонецЕсли;
		Если СвойствоСуществует(ExternalObjectID, "navigationRef") Тогда
			ExternalObjectID.navigationRef = НавигационнаяСсылка;
		КонецЕсли;
		Если СвойствоСуществует(ExternalObjectID, "presentation") Тогда
			ExternalObjectID.presentation = Представление;
		КонецЕсли;
		Если СвойствоСуществует(ExternalObjectID, "barcodeData")
				И (ОбщегоНазначения.ЭтоДокумент(ВнешнийОбъектИСМетаданные)
					Или ОбщегоНазначения.ЭтоСправочник(ВнешнийОбъектИСМетаданные)) Тогда
			ExternalObjectID.barcodeData = "";
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьПереопределяемый.ПриОпределенииШтрихкодаДокумента(
				ВнешнийОбъектИС,
				ExternalObjectID.barcodeData);
		КонецЕсли;
...
Функция СокращенноеНаименованиеКонфигурации() Экспорт
	...
	СокращенноеНаименование = НСтр("ru = 'ИС'");
...
// Команды БИД могут появиться на форме объекта ИС в разделе "Создание на основании" разными способами:
//   1. Автоматически: платформа отобразит общую команду, если в командном интерфейсе
//      конкретной формы не выключена видимость команды.
//   2. Программно: через механизм БСП СозданиеНаОснованииПереопределяемый.ПередДобавлениемКомандСозданияНаОсновании,
//      если конкретный объект ИС был подключен к механизму БСП в методе
//      СозданиеНаОснованииПереопределяемый.ПриОпределенииОбъектовСКомандамиСозданияНаОсновании.
// Таким образом, мы заранее не знаем каким из способов была выведена команда.
// Если отработали оба способа - команда будет дублироваться на форме.
// Данная процедура позволяет скрыть лишнюю команду.
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения - форма, из которой вызвана процедура.
//   ИмяКоманды - Строка - имя команды, дубли которой требуется скрыть.
//
Процедура УбратьДублиКомандыИнтеграции(Форма, ИмяКоманды) Экспорт
	
	СписокКоманд = Новый ТаблицаЗначений;
	СписокКоманд.Колонки.Добавить("Команда");
	СписокКоманд.Колонки.Добавить("Приоритет");
	
	Для Каждого ЭлементФормы Из Форма.Элементы Цикл
		Если ТипЗнч(ЭлементФормы) = Тип("КнопкаФормы")
				И ЭлементФормы.Видимость
				И СтрЗаканчиваетсяНа(ЭлементФормы.Имя, ИмяКоманды)
				И СтрНайти(ЭлементФормы.Имя, "КонтекстноеМеню") = 0 Тогда
			НоваяСтрока = СписокКоманд.Добавить();
			НоваяСтрока.Команда = ЭлементФормы;
			НоваяСтрока.Приоритет = ?(СтрНачинаетсяС(ЭлементФормы.Имя, "ФормаОбщаяКоманда"), 1, 0);
		КонецЕсли;
	КонецЦикла;
	
	СписокКоманд.Сортировать("Приоритет");
	
	Если СписокКоманд.Количество() > 1 Тогда
		Для Индекс = 1 По СписокКоманд.Количество() - 1 Цикл
			СписокКоманд[Индекс].Команда.Видимость = Ложь;
		КонецЦикла;
	КонецЕсли;
...
Функция ИмяПечатнойФормы(Источник, ПредставлениеКоманды, СинонимМакета, ИмяМакета) Экспорт
	...
	Если ОбщегоНазначения.ЭтоДокумент(ИсточникМетаданные) И ИсточникМетаданные.ДлинаНомера > 0 Тогда
		...
		ИмяФайла = СтрШаблон(НСтр("ru = '%1 №%2 от %3'"),
			...
			Формат(ДатаНомер.Дата, "ДФ=dd.MM.yyyy")); // Проектное решение. Локальный формат даты (ДЛФ) не
				// используется, т.к. интеграция сопоставляет файлы печатных
				// форм по наименованию, которое должно быть фиксированным.
	Иначе
...
//
Функция КомандыПечатиФормы(Знач ИмяФормы) Экспорт
	...
	МодульУправлениеПечатью = ОбщегоНазначения.ОбщийМодуль("УправлениеПечатью");
	КоллекцияКомандПечати = МодульУправлениеПечатью.СоздатьКоллекциюКомандПечати();
	
	ОбъектМетаданныхФорма = Метаданные.НайтиПоПолномуИмени(ИмяФормы);
	ОбъектМетаданных = Неопределено;
	
	Если ОбъектМетаданныхФорма = Неопределено Тогда
		ЧастиИмени = СтрРазделить(ИмяФормы, ".", Ложь);
		Если ЧастиИмени.Количество() > 1 Тогда
			ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(СтрШаблон("%1.%2", ЧастиИмени[0], ЧастиИмени[1]));
			Если ОбъектМетаданных <> Неопределено Тогда
				ОбъектМетаданныхФорма = ОбъектМетаданных.ОсновнаяФормаСписка;
				Если ОбъектМетаданныхФорма <> Неопределено Тогда
					ИмяФормы = ОбъектМетаданныхФорма.ПолноеИмя();
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли Не Метаданные.ОбщиеФормы.Содержит(ОбъектМетаданныхФорма) Тогда
		ОбъектМетаданных = ОбъектМетаданныхФорма.Родитель();
	КонецЕсли;
	
	Если ОбъектМетаданныхФорма <> Неопределено
			И ОбъектМетаданных <> Неопределено
			И МодульУправлениеПечатью.ИсточникиКомандПечати().Найти(ОбъектМетаданных) <> Неопределено Тогда
		КоллекцияКомандПечати = МодульУправлениеПечатью.КомандыПечатиФормы(ИмяФормы);
		КоллекцияКомандПечати.Колонки.Добавить("ДополнительныеПараметрыВСтроку");
		ПередДобавлениемКомандПечати(ИмяФормы, КоллекцияКомандПечати, Истина);
		Для Каждого КомандаПечати Из КоллекцияКомандПечати Цикл
			КомандаПечати.ДополнительныеПараметрыВСтроку = ПараметрыПечатиВСтроку(
				КомандаПечати.ДополнительныеПараметры);
		КонецЦикла;
		СвернутьКомандыПечатиФормы(КоллекцияКомандПечати);
	КонецЕсли;
...
Функция ПрисоединенныеФайлыВИС(ВладелецФайлов) Экспорт
	...
	Если Метаданные.Справочники[ИмяСправочникаХранилищаФайлов].Иерархический Тогда
		Запрос.Текст = Запрос.Текст + " И НЕ Файлы.ЭтоГруппа";
	КонецЕсли;
...
Функция КлючСопоставленияПользователяТокенаДоступа() Экспорт
	...
	ИначеЕсли Ключ = Перечисления.КлючиСопоставленияПользователей1СДокументооборот.АдресЭлектроннойПочты Тогда
		КлючСопоставленияПользователя = ТекущийПользовательИБ.АдресЭлектроннойПочты;
	КонецЕсли;
...
//
Функция ДанныеПодписиИзXDTO(ПодписьXDTO) Экспорт
	...
	Если СвойствоУстановлено(ПодписьXDTO, "signatureType") Тогда
		ДанныеПодписи.ТипПодписи = ПодписьXDTO.signatureType;
	КонецЕсли;
...
Процедура ЗаполнитьXDTOПодпись(Прокси, ПодписьXDTO, ДанныеПодписи) Экспорт
	...
	Если СвойствоСуществует(ПодписьXDTO, "signatureType") И ЗначениеЗаполнено(ДанныеПодписи.ТипПодписи) Тогда
		ПодписьXDTO.signatureType = ДанныеПодписи.ТипПодписи;
	КонецЕсли;
	
КонецПроцедуры

// Заполняет список подписей объекта 1С:Документооборота и его подчиненных файлов.
//
// Параметры:
//   ОбъектДокументооборота - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО
//   ПодписиОбъекта - СписокXDTO - список подписей документа Документооборота.
//   ФайлыОбъекта - СписокXDTO - список файлов документа Документооборота.
...
//   КоличествоЭП - Число - количество ЭК, которое требуется вывести в заголовке страницы.
//   УникальныйИдентификаторФормы - УникальныйИдентификатор - идентификатор вызвавшей формы.
//
Процедура ЗаполнитьСписокПодписей(ОбъектДокументооборота, ПодписиОбъекта, ФайлыОбъекта, ДеревоПодписей, КоличествоЭП,
	...
	Для Каждого ПодписьXDTO Из ПодписиОбъекта Цикл
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДополнитьПодписиОбъектов(
		ПодписиОбъектов,
		ОбъектДокументооборота.Наименование,
		ОбъектДокументооборота.ID,
		ОбъектДокументооборота.Тип,
		Ложь,
		МассивПодписей);
	...
	ДеревоПодписей.ПолучитьЭлементы().Очистить();
	ЭлементыДерева = ДеревоПодписей.ПолучитьЭлементы(); // ДанныеФормыКоллекцияЭлементовДерева
	...
	Для Каждого ЭлементСоответствия Из ПодписиОбъектов Цикл
		...
		Если ДанныеВладельцаПодписи.Тип <> "DMFile" И ДанныеВладельцаПодписи.МассивПодписей.Количество() <> 0 Тогда
			...
			НоваяСтрокаВладельцаПодписи.КомуВыданСертификат = КомуВыданСертификат(ДанныеВладельцаПодписи);
			...
			КонецЕсли;
			НоваяСтрокаВладельцаПодписи.Объект = ОбъектДокументооборота.Наименование;
			НоваяСтрокаВладельцаПодписи.ОбъектИмя = ОбъектДокументооборота.Наименование;
			НоваяСтрокаВладельцаПодписи.ОбъектID = ОбъектДокументооборота.ID;
			НоваяСтрокаВладельцаПодписи.ОбъектТип = ОбъектДокументооборота.Тип;
	...
	Для Каждого ЭлементСоответствия Из ПодписиОбъектов Цикл
		...
		Если ДанныеВладельцаПодписи.Тип = "DMFile" И ДанныеВладельцаПодписи.МассивПодписей.Количество() <> 0 Тогда
			...
			НоваяСтрокаВладельцаПодписи.Объект = ДанныеВладельцаПодписи.Наименование;
			НоваяСтрокаВладельцаПодписи.ОбъектИмя = ДанныеВладельцаПодписи.Наименование;
			НоваяСтрокаВладельцаПодписи.ОбъектID = ДанныеВладельцаПодписи.ID;
			НоваяСтрокаВладельцаПодписи.ОбъектТип = ДанныеВладельцаПодписи.Тип;
...
Процедура ЗаполнитьСтрокуДереваПодписей(НоваяСтрока, НомерСтроки, Подпись, ДанныеВладельцаПодписи,
	...
	НоваяСтрока.ПодписьВерна = Ложь;
	НоваяСтрока.Статус = НСтр("ru = 'Не проверена'");
	...
	НоваяСтрока.Объект = ДанныеВладельцаПодписи.Наименование;
...
//
Процедура ПодписатьДокумент(ДобавленныеПодписиОбъектов, ДеревоПодписей) Экспорт
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ЗанестиИнформациюОПодписяхОбъектов(
		ДобавленныеПодписиОбъектов);
...
//   ТаблицаПодписей - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ТаблицаПодписей
//   ВыделенныеПодписи - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыделенныеПодписи
//   АдресДвоичныхДанныхОбъекта - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.АдресДвоичныхДанныхОбъекта
//
Процедура ПроверитьПодписи(МенеджерКриптографии, ТаблицаПодписей, ВыделенныеПодписи = Неопределено,
		АдресДвоичныхДанныхОбъекта = Неопределено) Экспорт
...
// Проверяет, может ли интегрированная система заниматься созданием электронных
// подписей для объектов 1С:Документооборота.
//
// Возвращаемое значение:
//   Булево
//
Функция СоздаватьЭПДляОбъектов1СДокументооборотаСредствамиИС() Экспорт
	
	Результат = Истина;
	СтандартнаяОбработка = Истина;
	
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьПереопределяемый.ПриПроверкеВозможностиСоздаватьЭПДляОбъектов1СДокументооборотаСредствамиИС(
		Результат,
		СтандартнаяОбработка);
	...
	Если СтандартнаяОбработка Тогда
		Настройки = ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ИспользоватьИнтеграцию();
		Если Настройки.ИспользоватьИнтеграциюДО3 Тогда
			Результат = Константы.СоздаватьЭПДляОбъектов1СДокументооборотаСредствамиИС.Получить();
		КонецЕсли;
	КонецЕсли;
...
// Делает копию таблицы КонтрольОтправкиФайлов с заданным отбором по источнику.
//
// Параметры:
//   ИсходнаяТаблица - см. ИнтеграцияС1СДокументооборотБазоваяФункциональность.КонтрольОтправкиФайлов
//   МассивИсточников - Массив из ЛюбаяСсылка - будут возвращены строки, реквизит "Источник" которых
//     входит в данный массив.
//
// Возвращаемое значение:
//   см. ИнтеграцияС1СДокументооборотБазоваяФункциональность.КонтрольОтправкиФайлов
//
Функция КопияТаблицыКонтрольОтправкиФайлов(ИсходнаяТаблица, МассивИсточников)
	...
	Для НомерСтроки = 1 По КоличествоСтрок Цикл
		...
		Строка = КопияТаблицы[Индекс];
		Если (МассивИсточников.Найти(Строка.Источник) = Неопределено) Тогда
			КопияТаблицы.Удалить(Индекс);
		КонецЕсли;
	КонецЦикла;
...
//
Процедура ОтправитьДанные(Прокси, Настройки, ИмяСобытия)
	...
	Попытка
		...
		ЗапросКБазе = Новый Запрос(
			...
			|ГДЕ
			|	ОчередьСообщенийВ1СДокументооборот.КоличествоПопытокОтправки < &ПредельноеКоличествоПопытокОтправкиСообщенияОбмена
			|
			|УПОРЯДОЧИТЬ ПО
			|	МоментВремени");
	...
	Исключение
		ЗаписатьОшибку(ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ИмяСобытия);
	КонецПопытки;
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ПроверитьРезультатОтправки(Прокси, Запрос, Ответ, Настройки, Выборка) Тогда
		...
		Исключение
			...
			ЗаписатьОшибку(ТекстСообщенияОбОшибке, ИмяСобытия, Строка(ИдентификаторСообщения));
...
Процедура ОчиститьУстаревшиеДанные()
	...
	Запрос = Новый Запрос(
		...
		|	И ОчередьСообщенийВ1СДокументооборот.КоличествоПопытокОтправки >= &ПредельноеКоличествоПопытокОтправкиСообщенияОбмена
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
//
Процедура ПодготовитьДанныеДляОтправки(Прокси, Настройки, ИмяСобытия,
	...
	Для Каждого ИнтегрированныйОбъект Из ИнтегрированныеОбъекты Цикл
		...
		Попытка
			...
			ПорцияОбъектов.Добавить(ОбъектXDTO);
...
Функция ПроверитьРезультатОтправки(Прокси, Запрос, Ответ, Настройки, РеквизитыСообщения)
	...
	ПорцияОбъектовЗапросаСОшибкой = Новый Массив;
	НеОтправленныеОбъектыИС = Новый Массив;
	ПорцияОбъектовЗапросаБезОшибок = Новый Массив;
	ОтправленныеОбъектыИС = Новый Массив;
	...
	Для Каждого ОбъектXDTO Из Ответ.objects Цикл
		...
		Если ОбъектXDTO.success Тогда
			ПорцияОбъектовЗапросаБезОшибок.Добавить(ОбъектЗапросаXDTO);
			ОтправленныеОбъектыИС.Добавить(ОбъектИС);
			Продолжить;
		Иначе
			ПорцияОбъектовЗапросаСОшибкой.Добавить(ОбъектЗапросаXDTO);
			НеОтправленныеОбъектыИС.Добавить(ОбъектИС);
		КонецЕсли;
	...
	Если ПорцияОбъектовЗапросаБезОшибок.Количество() = 0 Тогда
		ВызватьИсключение ТекстСообщенияОбОшибке;
	КонецЕсли;
	...
	Попытка
		...
		ЭлементБлокировки = БлокировкаДанных.Добавить("РегистрСведений.ОчередьСообщенийВ1СДокументооборот");
		ЭлементБлокировки.УстановитьЗначение("МоментВремени", РеквизитыСообщения.МоментВремени);
		ЭлементБлокировки.УстановитьЗначение("Идентификатор", РеквизитыСообщения.Идентификатор);
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		
		БлокировкаДанных.Заблокировать();
		
		РеквизитыСообщенияСОшибкой = РегистрыСведений.ОчередьСообщенийВ1СДокументооборот.РеквизитыСообщения();
		РеквизитыСообщенияСОшибкой.МоментВремени = РеквизитыСообщения.МоментВремени;
		РеквизитыСообщенияСОшибкой.Идентификатор = РеквизитыСообщения.Идентификатор;
		РеквизитыСообщенияСОшибкой.ТекстСообщенияОбОшибке = ТекстСообщенияОбОшибке;
		
		МенеджерЗаписиОчередь = РегистрыСведений.ОчередьСообщенийВ1СДокументооборот.СоздатьМенеджерЗаписи();
		МенеджерЗаписиОчередь.МоментВремени = РеквизитыСообщения.МоментВремени;
		МенеджерЗаписиОчередь.Идентификатор = РеквизитыСообщения.Идентификатор;
		МенеджерЗаписиОчередь.Прочитать();
		Если МенеджерЗаписиОчередь.Выбран() Тогда
			РеквизитыСообщенияСОшибкой.КоличествоПопытокОтправки = МенеджерЗаписиОчередь.КоличествоПопытокОтправки + 1;
			КонтрольОтправкиФайлов = МенеджерЗаписиОчередь.КонтрольОтправкиФайлов.Получить();
			КонтрольОтправкиФайловБезОшибок = КопияТаблицыКонтрольОтправкиФайлов(
				КонтрольОтправкиФайлов,
				ОтправленныеОбъектыИС);
			КонтрольОтправкиФайловСОшибкой = КопияТаблицыКонтрольОтправкиФайлов(
				КонтрольОтправкиФайлов,
				НеОтправленныеОбъектыИС);
			МенеджерЗаписиОчередь.Удалить();
		Иначе
			КонтрольОтправкиФайловБезОшибок = КонтрольОтправкиФайлов();
			КонтрольОтправкиФайловСОшибкой = КонтрольОтправкиФайлов();
		КонецЕсли;
		
		СформироватьСообщение(
			Прокси,
			ПорцияОбъектовЗапросаСОшибкой,
			КонтрольОтправкиФайловСОшибкой,
			РеквизитыСообщенияСОшибкой);
		
		РеквизитыСообщенияБезОшибок = РегистрыСведений.ОчередьСообщенийВ1СДокументооборот.РеквизитыСообщения();
		
		СформироватьСообщение(
			Прокси,
			ПорцияОбъектовЗапросаБезОшибок,
			КонтрольОтправкиФайловБезОшибок,
			РеквизитыСообщенияБезОшибок);
		
		РегистрыСведений.ИсторияОтправкиСообщенийВ1СДокументооборот.ПеренестиСообщениеВИсторию(
			РеквизитыСообщенияБезОшибок.МоментВремени,
			РеквизитыСообщенияБезОшибок.Идентификатор);
...
Процедура ПриСозданииПоСсылке(ОбъектИС, ОбъектXDTO, ЗаполняемыйОбъектИС = Неопределено)
	...
	Если ТипЗнч(ОбъектИС) = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ТипВалюты(Истина)
		...
		Если СвойствоУстановлено(ОбъектXDTO, "fullName") И ЗначениеЗаполнено(ОбъектXDTO.fullName) Тогда
	...
	ИначеЕсли ТипЗнч(ОбъектИС) = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ТипСтраныМира(Истина)
		...
		КонецЕсли;
		Если СвойствоУстановлено(ОбъектXDTO, "longDescription") И ЗначениеЗаполнено(ОбъектXDTO.longDescription) Тогда
...
Процедура СформироватьСообщение(Прокси, ПорцияОбъектов, КонтрольОтправкиФайлов, РеквизитыСообщения = Неопределено)
...
Процедура ДобавитьКомандуСозданияНаОсновании(ТаблицаКоманд, Команда, Обработчик, ВидВРег, ФункциональныеОпции, Порядок,
	...
	КомандаСозданияНаОсновании.МножественныйВыбор = МножественныйВыбор;
...
Функция КомуВыданСертификат(ДанныеВладельцаПодписи)
	
	Возврат СтрШаблон("%1 ""%2""",
		ПредставлениеТипаОбъектаXDTO(ДанныеВладельцаПодписи.Тип),
	...
	Попытка
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБСП, "3.1.9.64") >= 0
			...
			Если КОбъектуМожноПрисоединятьФайлы
				...
				ВыводитсяКомандаФайлыБСП = Истина;
			КонецЕсли;
			
		КонецЕсли;
		
		Если Не ВыводитсяКомандаФайлыБСП Тогда
			// Команды "Файлы" из БСП на форме нет. Проверим необходимость вывода команды "Файлы" из БИД.
...
Функция ОпределениеВебСервисаДО(АдресВебСервисаДокументооборота, ДанныеСервиса, ИспользоватьИмяВебСервиса,
		НастройкиАвторизации, МестоположениеПрокси, ТекстСообщенияОбОшибке)
	
	ОпределениеВебСервисаДО = ПроверкаПодключенияПоЛогинуПаролю(
		АдресВебСервисаДокументооборота,
		ДанныеСервиса,
		ИспользоватьИмяВебСервиса,
	...
	Если ОпределениеВебСервисаДО = Неопределено И Не ЗначениеЗаполнено(НастройкиАвторизации.ИмяПользователя) Тогда
		
		ОпределениеВебСервисаДО = ПроверкаПодключенияЧерезJWT(
			АдресВебСервисаДокументооборота,
			ДанныеСервиса,
			ИспользоватьИмяВебСервиса,
			МестоположениеПрокси,
	...
	Если ОпределениеВебСервисаДО = Неопределено И Не ЗначениеЗаполнено(НастройкиАвторизации.ИмяПользователя) Тогда
		...
		ОпределениеВебСервисаДО = ПроверкаПодключенияЧерезОС(
			АдресВебСервисаДокументооборота,
			ДанныеСервиса,
			ИспользоватьИмяВебСервиса,
			НастройкиАвторизации.ИспользуетсяАутентификацияОС,
	...
	Возврат ОпределениеВебСервисаДО;
...
Функция ПолучитьИОбновитьКэшWSОпределения(ДанныеСервиса, МестоположениеWSDL, ИмяПользователя, Пароль,
	...
	ТекстСообщенияОбОшибке = "";
	ОпределениеВебСервисаДО = Неопределено;
	...
	Попытка
		...
		Если РезультатСкачивания.Статус Тогда
			...
			ОпределениеВебСервисаДО = Новый WSОпределения(ИмяФайлаWSDL,,,, ДанныеСервиса.Таймаут, ЗащищенноеСоединение);
			...
			КонецЕсли;
		Иначе
			СообщениеИзРезультатаСкачивания = РезультатСкачивания.СообщениеОбОшибке;
			НачалоТелаОтветаОтСервера = СтрНайти(РезультатСкачивания.СообщениеОбОшибке, "{""#exception""");
			Если НачалоТелаОтветаОтСервера > 0 Тогда
				Попытка
					СтрокаJSON = СтрЗаменить(Сред(РезультатСкачивания.СообщениеОбОшибке, НачалоТелаОтветаОтСервера), "#", "x");
					ЧтениеJSON = Новый ЧтениеJSON;
					ЧтениеJSON.УстановитьСтроку(СтрокаJSON);
					СтруктураСообщения = ПрочитатьJSON(ЧтениеJSON);
					ЧтениеJSON.Закрыть();
					Описание = Новый Массив;
					Описание.Добавить(СтруктураСообщения.exception.descr);
					Если СтруктураСообщения.exception.Свойство("inner") Тогда
						Описание.Добавить(СтруктураСообщения.exception.inner.descr);
					КонецЕсли;
					СообщениеИзРезультатаСкачивания = СтрСоединить(Описание, Символы.ПС);
				Исключение
					СообщениеИзРезультатаСкачивания = РезультатСкачивания.СообщениеОбОшибке;
				КонецПопытки;
			КонецЕсли;
			ТекстСообщенияОбОшибке = СокрЛП(СтрШаблон(
				...
				СообщениеИзРезультатаСкачивания));
		КонецЕсли;
...
Функция ПолучитьПроксиИзПараметровСеанса(ВызыватьИсключение = Истина)
	...
	Попытка
		...
		ЗащищенноеСоединение = ОбщегоНазначенияКлиентСервер.НовоеЗащищенноеСоединение();
		ОпределениеВебСервисаДО = Новый WSОпределения(ИмяФайлаWSDL,,,, ТаймаутСервиса(), ЗащищенноеСоединение);
...
Процедура ПроверитьОднуПодпись(ДанныеСтроки, МенеджерКриптографии, СоответствиеИдОбъектаИДвоичныхДанных,
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		...
		Исключение
			Неверна = Истина;
			ОписаниеОшибки = НСтр("ru = 'не указан менеджер криптографии'");
		КонецПопытки;
	Иначе
		Неверна = Истина;
		ОписаниеОшибки = НСтр("ru = 'отсутствует стандартная подсистема ""Электронная подпись""'");
	КонецЕсли;
...
Функция ПроверкаПодключенияПоЛогинуПаролю(АдресВебСервисаДокументооборота, ДанныеСервиса, ИспользоватьИмяВебСервиса,
		ИмяПользователя, Пароль, ТекстСообщенияОбОшибке)
	...
	Если ЗначениеЗаполнено(ИмяПользователя) Тогда
		Попытка
			...
			ОпределениеВебСервисаДО = ПолучитьИОбновитьКэшWSОпределения(
				ДанныеСервиса,
				МестоположениеWSDL,
				ИмяПользователя,
				Пароль);
...
Функция ПроверкаПодключенияЧерезJWT(АдресВебСервисаДокументооборота, ДанныеСервиса, ИспользоватьИмяВебСервиса,
		МестоположениеПрокси, ИспользуетсяАутентификацияJWT, ТекстСообщенияОбОшибке)
...
Функция ПроверкаПодключенияЧерезОС(АдресВебСервисаДокументооборота, ДанныеСервиса, ИспользоватьИмяВебСервиса,
		ИспользуетсяАутентификацияОС, ТекстСообщенияОбОшибке)
