...
Процедура ПриОпределенииЗапросаТоварыДокументаОснования(МетаданныеОснования, МетаданныеДокументаИСМП,
	...
	ИначеЕсли МетаданныеОснования = Метаданные.Документы.МаркировкаТоваровИСМП Тогда
		...
		|	И ТаблицаТовары.Ссылка.Операция <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотСырьяПроизводствоРФ)
		|	И ТаблицаТовары.Ссылка.Операция <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотСырьяПолучениеТовараИзЕАЭС)
		|	И ТаблицаТовары.Ссылка.Операция <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотСырьяПолучениеТовараВнеЕАЭС)
		|	И ТаблицаТовары.Ссылка.Операция <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотСырьяОстатки)
...
Функция ОпределитьСтатусыОформленияДокументов(МассивДокументов, МетаданныеДокументаИСМП, МенеджерВТ) Экспорт
	...
	Для Каждого ВидПродукции Из КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииМОТП() Цикл
		ТоварныеГруппыМОТП.Добавить(КонструкторИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппаПоВидуПродукции(ВидПродукции));
	КонецЦикла;
	...
	ШаблонЗапросаВТОформленныеДокументы = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	И (ТаблицаДокументы.ТоварнаяГруппа В (&ТоварныеГруппыМОТП)
		...
		|		ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотСырьяОстатки),
		|		ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотСырьяПолучениеТовараВнеЕАЭС),
		|		ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотСырьяПолучениеТовараИзЕАЭС),
		|		ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотСырьяПроизводствоРФ),
	...
	|ГДЕ
	|	ОформленныеТовары.Шаблон.ТипКодаМаркировки <> ЗНАЧЕНИЕ(Перечисление.ТипыКодовМаркировкиИСМП.ГрупповаяУпаковка)
	|
