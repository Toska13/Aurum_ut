...
Процедура РаспределениеПартийНДС(ПараметрыРасчета, СтрокаСхемыРасчета, НомерЭтапа) Экспорт
	...
	Если ПараметрыРасчета.РежимЗакрытияМесяца = Перечисления.РежимыЗакрытияМесяца.ПредварительноеЗакрытие Тогда
		...
		Если НЕ РасчетСебестоимостиПрикладныеАлгоритмы.ВременнаяТаблицаСуществует(ПараметрыРасчета, "ВТСписаниеПостатейныхЗатрат") Тогда
			...
			Запрос.Текст = РасчетСебестоимостиНДС.ТекстОписаниеДанныхДляРаспределениеПартийНДСПоПартиямПрочихРасходов();
	...
	Если НомерЭтапа = 1 Тогда
		...
		// Этап 11
		// Формирует временную таблицу ВТДетализацияПартийТоваровДляНДСиУСН
		РасчетСебестоимостиНДС.ПодготовкаДанныхДляУчетаНДСиУСН(ПараметрыРасчета);
		
		//-- Локализация
		
		// Этап 11.1.2
		// Формирует временную таблицу ВТДетализацияПартийТоваровДляНДСиУСН_ДопРасходы
		РасчетСебестоимостиНДС.ПодготовкаДанныхДляУчетаНДСиУСН_ДопРасходы(ПараметрыРасчета);
		...
		// Формирует движения по регистрам:
		// - ДетализацияСебестоимостиПартииТоваровПостатейныеЗатраты
		РасчетСебестоимостиНДС.РаспределениеПартийНДСПоДопРасходам(ПараметрыРасчета);
	...
	ИначеЕсли НомерЭтапа = 2 Тогда
		...
		РасчетСебестоимостиНДС.РаспределениеПартийНДСПоПартиямПрочихРасходов(ПараметрыРасчета);
		
		// Этап 12.0.1
		// Формирует движения по регистрам:
		// - ДетализацияПартийТоваровДляНДСиУСН
		РасчетСебестоимостиНДС.ПодготовкаДанныхДляУчетаНДСиУСН_Постатейные(ПараметрыРасчета);
		
		// Этап 11.1.3
		// Формирует движения по регистру:
		// - ДетализацияПартийТоваровДляНДСиУСН
		РасчетСебестоимостиНДС.ПодготовкаДанныхДляДетализацииПартийТоваровДляНДСиУСН(ПараметрыРасчета);
...
Процедура ВключитьИсключитьНДСВСтоимость(ПараметрыРасчета, ТолькоПостатейные = Ложь) Экспорт
	...
	Если НЕ ТолькоПостатейные Тогда
		...
		РасчетСебестоимостиНДС.ВключитьИсключитьНДСВСтоимостьСреднескользящая(ПараметрыРасчета, Истина);
	КонецЕсли;
	РасчетСебестоимостиНДС.ВключитьИсключитьНДСВСтоимость24(ПараметрыРасчета, ТолькоПостатейные);
	
	Если ТолькоПостатейные Тогда
		РасчетСебестоимостиНДС.ПодготовкаДанныхДляВключенияИсключенияНДСДопРасходовВСтоимостьСреднескользящая(ПараметрыРасчета);
	КонецЕсли;
...
Процедура ВключитьИсключитьНДСВСтоимостьПродаж(ПараметрыРасчета) Экспорт
	...
	РасчетСебестоимостиНДС.ВключитьИсключитьНДСВСтоимостьСреднескользящая(ПараметрыРасчета, Ложь);
...
Процедура ДополнитьПараметрыРасчета(ПараметрыРасчета) Экспорт
	...
	ПараметрыРасчета.УчетныеПолитики.Вставить("УчетПартийНДСДопРасходы",   Новый Структура);
	ПараметрыРасчета.УчетныеПолитики.УчетПартийНДСДопРасходы.Вставить(
		"ИспользуютВТекущемПериоде",
		Новый Массив);
	...
	ПараметрыРасчета.УчетныеПолитики.Вставить("ДетализироватьСебестоимостьПродукцииДоИсходныхЗатрат", Новый Структура);
	ПараметрыРасчета.УчетныеПолитики.ДетализироватьСебестоимостьПродукцииДоИсходныхЗатрат.Вставить(
		"ИспользуютВТекущемПериоде",
		Новый Массив);
	ПараметрыРасчета.УчетныеПолитики.ДетализироватьСебестоимостьПродукцииДоИсходныхЗатрат.Вставить(
		"ИспользовалиВПрошломПериоде",
	...
	|	УчетнаяПолитикаФинансовогоУчетаСрезПоследних.ДетализироватьСебестоимостьПродукцииДоИсходныхЗатрат
	|ПОМЕСТИТЬ ВТФинУчет
	|ИЗ
	...
	|	ЕСТЬNULL(ВТФинУчет.ДетализироватьСебестоимостьПродукцииДоИсходныхЗатрат,
	|		ИСТИНА) КАК ДетализироватьСебестоимостьПродукцииДоИсходныхЗатрат,
	//++ Локализация
	...
	|		ЗНАЧЕНИЕ(Перечисление.МетодыОценкиСтоимостиТоваров.СредняяЗаМесяц)) КАК МетодОценкиСтоимостиТоваров,
	|	ЕСТЬNULL(ВТФинУчетПрошлыйПериод.ДетализироватьСебестоимостьПродукцииДоИсходныхЗатрат,
	|		ИСТИНА) КАК ДетализироватьСебестоимостьПродукцииДоИсходныхЗатрат,
	//++ Локализация
	...
	Для Каждого ПараметрыУчетнойПолитики Из УчетныеПолитики Цикл
		...
		Если ПараметрыРасчета.ПартионныйУчетВерсии22
		 И (ПараметрыУчетнойПолитики.РаздельныйУчетТоваровПоНалогообложениюНДС
			...
			ОрганизацииСУчетнойПолитикой = ПараметрыРасчета.УчетныеПолитики.УчетПартийНДСДопРасходы.ИспользуютВТекущемПериоде; // Массив
			ОрганизацииСУчетнойПолитикой.Добавить(ПараметрыУчетнойПолитики.Организация);
			
			// Настройка детализации себестоимости продукции до исходных затрат применятся только при методах оценки
			// "Средняя за месяц" и "Среднескользящая".
			Если (ПараметрыУчетнойПолитики.ДетализироватьСебестоимостьПродукцииДоИсходныхЗатрат
			 		И НЕ ПараметрыУчетнойПолитики.МетодОценкиСтоимостиТоваров = Перечисления.МетодыОценкиСтоимостиТоваров.ФИФОСкользящаяОценка)
			 И ПараметрыУчетнойПолитики.СистемаНалогообложения = Перечисления.СистемыНалогообложения.Общая Тогда
				ОрганизацииСУчетнойПолитикой = ПараметрыРасчета.УчетныеПолитики.ДетализироватьСебестоимостьПродукцииДоИсходныхЗатрат.ИспользуютВТекущемПериоде; // Массив
				ОрганизацииСУчетнойПолитикой.Добавить(ПараметрыУчетнойПолитики.Организация);
			КонецЕсли;
		...
		Если ПараметрыРасчета.ПартионныйУчетВерсии22
		 И (ПараметрУчетнойПолитикиПрошлогоПериода.РаздельныйУчетТоваровПоНалогообложениюНДС
			...
			Если (ПараметрУчетнойПолитикиПрошлогоПериода.ДетализироватьСебестоимостьПродукцииДоИсходныхЗатрат
					ИЛИ ПараметрУчетнойПолитикиПрошлогоПериода.МетодОценкиСтоимостиТоваров = Перечисления.МетодыОценкиСтоимостиТоваров.ФИФОСкользящаяОценка)
				И ПараметрУчетнойПолитикиПрошлогоПериода.СистемаНалогообложения = Перечисления.СистемыНалогообложения.Общая Тогда
				
				ОрганизацииСУчетнойПолитикой = ПараметрыРасчета.УчетныеПолитики.ДетализироватьСебестоимостьПродукцииДоИсходныхЗатрат.ИспользовалиВПрошломПериоде; // Массив
				ОрганизацииСУчетнойПолитикой.Добавить(ПараметрыУчетнойПолитики.Организация);
			КонецЕсли;
...
Процедура ДополнитьСвойстваЗапроса(Запрос, ПараметрыРасчета) Экспорт
	...
	Запрос.УстановитьПараметр("ОрганизацииСУчетомПартийНДСДопРасходы",				 ПараметрыРасчета.УчетныеПолитики.УчетПартийНДСДопРасходы.ИспользуютВТекущемПериоде);
	
	Запрос.УстановитьПараметр("ОрганизацииСДетализациейСебестоимостиПродукцииДоИсходныхЗатрат",
		ПараметрыРасчета.УчетныеПолитики.ДетализироватьСебестоимостьПродукцииДоИсходныхЗатрат.ИспользуютВТекущемПериоде);
	Запрос.УстановитьПараметр("ОрганизацииСДетализациейСебестоимостиПродукцииДоИсходныхЗатратИспользовалиВПрошломПериоде",
		ПараметрыРасчета.УчетныеПолитики.ДетализироватьСебестоимостьПродукцииДоИсходныхЗатрат.ИспользовалиВПрошломПериоде);
...
Процедура ДополнитьПараметрыЗапросаДляПроведения(Реквизиты, Период, Организация, СтруктураПараметров) Экспорт
	...
	Если ЗначениеЗаполнено(ТекущаяОрганизация) Тогда
		...
		СтруктураПараметров.Вставить("РаздельныйУчетТоваровУСН", ПараметрыУчетнойПолитикиУСН <> Неопределено
		    И ПараметрыУчетнойПолитикиУСН.УчетнаяПолитикаСуществует
