...
// * ВозможныОтрицательныеБазисы - Булево -
//
Функция ИнициализироватьПараметрыРаспределенияМетодомУменьшаемогоОстатка(
	...
	ПараметрыРаспределения.Вставить("ВозможныОтрицательныеБазисы", 	 	 Ложь);
...
Процедура ИнициализироватьЧисловыеПоляРаспределенияМетодомУменьшаемогоОстатка(ПараметрыРаспределения,
	...
	БазисИсточникаОригинальный	= "СУММА(%1." + ПолеБазисаИсточника + ")";
	...
	БазисИсточникаОригинальныйИменованный = БазисИсточникаОригинальный + " КАК " + ПолеБазисаИсточника;
	...
	Для Каждого ИмяПоля Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(РесурсыИсточника, ",", Истина, Истина) Цикл
		...
		ДополнитьСтроку(ОтборНенулевыеСуммыИсточника, "СУММА(%1." + ИмяПоля + ") <> 0",,,, " ИЛИ ");
		
		ДополнитьСтроку(ЦеныРесурсовИменованные,  "%1.Цена" + ИмяПоля + " КАК Цена" + ИмяПоля);
		ДополнитьСтроку(ЦеныСуммРесурсовИменованные,  "ВЫБОР КОГДА " + БазисИсточникаПоложительный + " = 0 ТОГДА 0
			|	ИНАЧЕ СУММА(%1." + ИмяПоля + ") / " + БазисИсточникаПоложительный + " КОНЕЦ КАК Цена" + ИмяПоля);
	...
	ПравилоРаспределения.Шаблоны.Вставить("БазисИсточникаОригинальныйИменованный",	БазисИсточникаОригинальныйИменованный);
	...
	ПравилоРаспределения.Шаблоны.Вставить("ОтборНенулевыеСуммыИсточника",			ОтборНенулевыеСуммыИсточника);
	...
	Если ПараметрыРаспределения.ПравилаРаспределения.Количество() = 0 Тогда
		...
		ПараметрыРаспределения.Шаблоны.Вставить("БазисИсточникаОригинальныйИменованный",		"");
		...
		ПараметрыРаспределения.Шаблоны.Вставить("ОтборНенулевыеСуммыИсточника",			  		"");
	...
	ПараметрыРаспределения.Шаблоны.Вставить("БазисИсточникаОригинальныйИменованный",
		СоединитьСтроки(ПараметрыРаспределения.Шаблоны.БазисИсточникаОригинальныйИменованный, БазисИсточникаОригинальныйИменованный,,,, ПараметрыРаспределения.Шаблоны.РазделительПолей));
	ПараметрыРаспределения.Шаблоны.Вставить("БазисБазыПоложительныйИменованный",
	...
	ПараметрыРаспределения.Шаблоны.Вставить("ОтборНенулевыеСуммыИсточника",
		СоединитьСтроки(ПараметрыРаспределения.Шаблоны.ОтборНенулевыеСуммыИсточника, ОтборНенулевыеСуммыИсточника,,,, " ИЛИ "));
	ПараметрыРаспределения.Шаблоны.Вставить("ЦеныРесурсовИменованные",
...
Процедура РаспределитьМетодомУменьшаемогоОстатка(ПараметрыРасчета, ПараметрыРаспределения) Экспорт
	...
	ЧисловыеПоля = СоединитьСтроки(
		?(ПараметрыРаспределения.ВозможныОтрицательныеБазисы,
			ПараметрыРаспределения.Шаблоны.БазисИсточникаОригинальныйИменованный,
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, ",&ОтборНенулевыеСуммыИсточника",
		"ИМЕЮЩИЕ" + Символы.ПС + ПараметрыРаспределения.Шаблоны.ОтборНенулеваяСуммаБазисовИсточника
		+ ?(ПараметрыРаспределения.Шаблоны.ОтборНенулевыеСуммыИсточника = "", "",  "
			|	И (" + ПараметрыРаспределения.Шаблоны.ОтборНенулевыеСуммыИсточника + ")"));
	...
	Для Каждого ПравилоРаспределения Из ПараметрыРаспределения.ПравилаРаспределения Цикл // по базисам распределения
		...
		ОписаниеДанных.Вставить("ИмяВТИсходной", "ВТИсходнаяТИ");
		ОписаниеДанных.Вставить("ИмяВТРезультата", "ВТБазисыИсточникаНарастающимИтогом");
		ОписаниеДанных.Вставить("ПолеКлючаЗаписи", "КлючТИ");
		ОписаниеДанных.Вставить("ПолеБазиса", ПравилоРаспределения.ПолеБазисаИсточника);
		...
		ОписаниеДанных = Новый Структура;
		ОписаниеДанных.Вставить("ИмяВТИсходной", "ВТИсходнаяТБ");
		ОписаниеДанных.Вставить("ИмяВТРезультата", "ВТБазисыБазыНарастающимИтогом");
		ОписаниеДанных.Вставить("ПолеКлючаЗаписи", "КлючТБ");
		ОписаниеДанных.Вставить("ПолеБазиса", ПравилоРаспределения.ПолеБазисаБазы);
...
Процедура РассчитатьПолеТаблицыНарастающимИтогом(ПараметрыРасчета, Запрос, ОписаниеДанных, ВозможныОтрицательныеБазисы, УпорядочитьПоКлючу = Ложь)
	
	РасчетСебестоимостиПротоколРасчета.НачалоФормированиеВременнойТаблицы(ПараметрыРасчета, ОписаниеДанных.ИмяВТРезультата);
	...
	Если УпорядочитьПоКлючу Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПолеБазиса УБЫВ", ОписаниеДанных.ПолеКлючаЗаписи);
	КонецЕсли;
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПолеКлючаЗаписи", ОписаниеДанных.ПолеКлючаЗаписи);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПолеБазиса", 	    ОписаниеДанных.ПолеБазиса);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ИмяВТИсходной",   ОписаниеДанных.ИмяВТИсходной);
	
	Если ВозможныОтрицательныеБазисы Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "УБЫВ", "");
	КонецЕсли;
	
	ТаблицаИсходная = Запрос.Выполнить().Выгрузить();
	ТаблицаИсходная.Колонки.Добавить("Базис_Итог", ТаблицаИсходная.Колонки[ОписаниеДанных.ПолеБазиса].ТипЗначения);
	...
	Для Каждого ТекущаяСтрока Из ТаблицаИсходная Цикл
		...
		Базис_Итог = Базис_Итог + ТекущаяСтрока[ОписаниеДанных.ПолеБазиса];
		ТекущаяСтрока.Базис_Итог = Базис_Итог;
		
	КонецЦикла;
	
	ТаблицаИсходная.Колонки.Удалить(ОписаниеДанных.ПолеБазиса);
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПолеКлючаЗаписи", ОписаниеДанных.ПолеКлючаЗаписи);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ИмяВТРезультата", ОписаниеДанных.ИмяВТРезультата);
