...
Процедура ДополнитьТекстЗапросаЗаполнитьЦенуЗакупки(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		...
		Если ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("Упаковка", КэшированныеЗначения) Тогда
			ИменаКолонок.ИмяКолонкиУпаковка = "Упаковка";
		КонецЕсли;
		
		Если ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("Партнер", КэшированныеЗначения) Тогда
			ИменаКолонок.ИмяКолонкиПартнер = "Партнер";
		КонецЕсли;
		
		Если ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("ВидЦеныПоставщика", КэшированныеЗначения) Тогда
			
			ИменаКолонок.ИмяКолонкиВидЦены = "ВидЦеныПоставщика";
			
		ИначеЕсли ПараметрыДействия.Свойство("ВидЦеныПоставщика")
			И ЗначениеЗаполнено(ЗначениеЗаполнено(ПараметрыДействия.ВидЦеныПоставщика)) Тогда
			
			ОписаниеЗапроса.ПараметрыЗапроса.Вставить("ВидЦеныПоставщика",
				ПараметрыДействия.ВидЦеныПоставщика);
			
		ИначеЕсли ПараметрыДействия.ИспользоватьСоглашениеИзТаблицы Тогда 
			
			ИменаКолонок.ИмяКолонкиСоглашение = "Соглашение";
			
		ИначеЕсли ЗначениеЗаполнено(ПараметрыДействия.Соглашение) Тогда
			
			ОписаниеЗапроса.ПараметрыЗапроса.Вставить("ВидЦеныПоставщика",
				ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					ПараметрыДействия.Соглашение,
					"ВидЦеныПоставщика"));
		...
		Если ЗначениеЗаполнено(ПараметрыДействия.Организация) Тогда
			СтранаРегистрации = ЗначениеНастроекКлиентСерверПовтИсп.СтранаРегистрацииОрганизации(ПараметрыДействия.Организация);
		Иначе
			СтранаРегистрации = ЗначениеНастроекКлиентСерверПовтИсп.ОсновнаяСтрана();
		КонецЕсли;
		
		Если Не ПараметрыДействия.Свойство("ВернутьМногооборотнуюТару") Тогда
			ПараметрыДействия.Вставить("ВернутьМногооборотнуюТару", Ложь);
		КонецЕсли;
		
		ТекстыЗапросов = ТекстыЗапросовЗаполненияЦенПакетнаяОбработка(ПараметрыДействия, ИменаКолонок);
		Для Каждого Запрос Из ТекстыЗапросов Цикл
			ОписаниеЗапроса.ТекстыЗапросов.Добавить(Запрос.Значение, Запрос.Представление);
		КонецЦикла;
		
		СтруктураПараметровОтбора = НовыйПараметрыОтбораПолучитьЦенуПоОтбору();
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(СтруктураПараметровОтбора, ПараметрыДействия, Истина);
		
		Для Каждого Параметр Из СтруктураПараметровОтбора Цикл
		...
		ОписаниеЗапроса.ПараметрыЗапроса.Вставить("СтранаРегистрации", СтранаРегистрации);
...
Процедура ДополнитьТекстЗапросаЗаполнитьУсловияЗакупок(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		...
		Если ПараметрыДействия.Свойство("ВидЦеныПоставщика")
			И ЗначениеЗаполнено(ПараметрыДействия.ВидЦеныПоставщика) Тогда
		...
		ТекстыЗапросов = ТекстыЗапросовЗаполненияУсловийЗакупокПакетнаяОбработка(ПараметрыДействия, ИменаКолонок);
		Для Каждого Запрос Из ТекстыЗапросов Цикл
			ОписаниеЗапроса.ТекстыЗапросов.Добавить(Запрос.Значение, Запрос.Представление);
		КонецЦикла;
...
// 	ИменаКолонок - см. ЦеныПредприятияЗаполнениеСервер.ИменаКолонокВременнойТаблицыТоваров.
//
// Возвращаемое значение:
// 	СписокЗначений Из Строка - тексты запросов получения цен для вставки.
//
Функция ТекстыЗапросовЗаполненияЦенПакетнаяОбработка(ПараметрыДействия, ИменаКолонок) Экспорт
	...
	Если ЗначениеЗаполнено(ИменаКолонок.ИмяКолонкиУпаковка) Тогда
		ТекстПоляУпаковка = СтрШаблон("ВтИсточникДанных.%1", ИменаКолонок.ИмяКолонкиУпаковка);
	Иначе
		ТекстПоляУпаковка = "ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)";
	КонецЕсли;
	
	Если ПараметрыДействия.ЦенаВключаетНДС <> Неопределено Тогда
		ТекстВидЦеныСОтбором =
		"ВЫБОР
		|		КОГДА ВЫРАЗИТЬ(&ВидЦеныПоставщика КАК Справочник.ВидыЦенПоставщиков).ЦенаВключаетНДС = &ЦенаВключаетНДС
		|			ТОГДА &ВидЦеныПоставщика
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ВидыЦенПоставщиков.ПустаяСсылка)
		|	КОНЕЦ"; //@Query-part
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ВидЦеныПоставщика", ТекстВидЦеныСОтбором);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИменаКолонок.ИмяКолонкиПартнер) Тогда
		
		ТекстПоляПартнер = СтрШаблон(
			"ВтИсточникДанных.%1",
			ИменаКолонок.ИмяКолонкиПартнер);
	...
	Если ЗначениеЗаполнено(ИменаКолонок.ИмяКолонкиВидЦены) Тогда
		
		ТекстПоляВидЦены = СтрШаблон(
			"ВтИсточникДанных.%1",
			ИменаКолонок.ИмяКолонкиВидЦены);
	...
	Если ЗначениеЗаполнено(ИменаКолонок.ИмяКолонкиСоглашение) Тогда
		ТекстПоляВидЦены = СтрШаблон(
			"ВЫБОР КОГДА ВтИсточникДанных.%1 = НЕОПРЕДЕЛЕНО ТОГДА
			|		ЗНАЧЕНИЕ(Справочник.ВидыЦенПоставщиков.ПустаяСсылка)
			|	ИНАЧЕ
			|		ЕСТЬNULL(ВЫРАЗИТЬ(ВтИсточникДанных.%1 КАК Справочник.СоглашенияСПоставщиками).ВидЦеныПоставщика,
			|				 ЗНАЧЕНИЕ(Справочник.ВидыЦенПоставщиков.ПустаяСсылка))
			|	КОНЕЦ",
			ИменаКолонок.ИмяКолонкиСоглашение);
	...
	Результат.Добавить(ТекстЗапроса, "ЗаполнитьЦенуЗакупкиПодготовительная");
	...
	|ПОМЕСТИТЬ ЦеныЗакупки_КурсВалюты
	|ИЗ
	...
	Результат.Добавить(ТекстЗапроса, "ЦеныЗакупки_КурсВалюты");
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаКурсов.КурсЧислитель КАК КурсЧислитель,
	|	ТаблицаКурсов.КурсЗнаменатель КАК КурсЗнаменатель
	|ПОМЕСТИТЬ ЦеныЗакупки_КурсВалютыПересчет
	|ИЗ
	|	РегистрСведений.ОтносительныеКурсыВалют.СрезПоследних(&Дата, Валюта = &Валюта
	|	И БазоваяВалюта = &БазоваяВалюта) КАК ТаблицаКурсов";
	
	Результат.Добавить(ТекстЗапроса, "ЦеныЗакупки_КурсВалютыПересчет");
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаСтавокНДС.Номенклатура КАК Номенклатура,
	|	ТаблицаСтавокНДС.СтавкаНДС КАК СтавкаНДС
	|ПОМЕСТИТЬ ЦеныЗакупки_СтавкиНДС
	|ИЗ
	|	РегистрСведений.СтавкиНДСНоменклатуры.СрезПоследних(&Дата, Номенклатура В
	|		(ВЫБРАТЬ
	|			ВтИсточникДанных.Номенклатура
	|		ИЗ
	|			ВтИсточникДанных КАК ВтИсточникДанных)
	|	И Страна В (&СтранаРегистрации, ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка))) КАК ТаблицаСтавокНДС
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура";
	
	Результат.Добавить(ТекстЗапроса, "ЦеныЗакупки_СтавкиНДС");
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаСтавокНДС.СтавкаНДС КАК СтавкаНДС
	|ПОМЕСТИТЬ ЦеныЗакупки_ОсновныеСтавкиНДС
	|ИЗ
	|	РегистрСведений.ОсновныеСтавкиНДС.СрезПоследних(&Дата, Страна = &СтранаРегистрации) КАК ТаблицаСтавокНДС";
	
	Результат.Добавить(ТекстЗапроса, "ЦеныЗакупки_ОсновныеСтавкиНДС");
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаРегистра.Номенклатура КАК Номенклатура,
	|	ТаблицаРегистра.Характеристика КАК Характеристика,
	|	ТаблицаРегистра.ВидЦеныПоставщика КАК ВидЦеныПоставщика,
	|	ТаблицаРегистра.Валюта КАК Валюта,
	|	ТаблицаРегистра.Упаковка КАК Упаковка,
	|	ТаблицаРегистра.Цена КАК Цена
	|ПОМЕСТИТЬ ЦеныНоменклатурыПоставщиковСрезПоследних
	|ИЗ
	|	РегистрСведений.ЦеныНоменклатурыПоставщиков.СрезПоследних(КОНЕЦПЕРИОДА(&Дата, ДЕНЬ), (Номенклатура, Характеристика,
	|		Партнер, ВидЦеныПоставщика) В
	|		(ВЫБРАТЬ
	|			ТаблицаТоваров.Номенклатура КАК Номенклатура,
	|			ТаблицаТоваров.Характеристика КАК Характеристика,
	|			ТаблицаТоваров.Партнер КАК Партнер,
	|			ТаблицаТоваров.ВидЦеныПоставщика КАК ВидЦеныПоставщика
	|		ИЗ
	|			ЗаполнитьЦенуЗакупкиПодготовительная КАК ТаблицаТоваров)) КАК ТаблицаРегистра
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура,
	|	Характеристика,
	...
	Результат.Добавить(ТекстЗапроса, "ЦеныНоменклатурыПоставщиковСрезПоследних");
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаИсточникДанных.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|	ВЫБОР
	...
	|	КОНЕЦ КАК СтавкаНДС,
	|	ВЫБОР
	|		КОГДА ТаблицаИсточникДанных.Упаковка <> ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	...
	|	КОНЕЦ КАК Цена
	|ИЗ
	|	ЗаполнитьЦенуЗакупкиПодготовительная КАК ТаблицаИсточникДанных
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК ТаблицаНоменклатуры
	|		ПО ТаблицаИсточникДанных.Номенклатура = ТаблицаНоменклатуры.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЦеныНоменклатурыПоставщиковСрезПоследних КАК ЦеныНоменклатурыПоставщиковСрезПоследних
	|		ПО ТаблицаИсточникДанных.Номенклатура = ЦеныНоменклатурыПоставщиковСрезПоследних.Номенклатура
	|		И ТаблицаИсточникДанных.Характеристика = ЦеныНоменклатурыПоставщиковСрезПоследних.Характеристика
	|		И ТаблицаИсточникДанных.ВидЦеныПоставщика = ЦеныНоменклатурыПоставщиковСрезПоследних.ВидЦеныПоставщика
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЦеныЗакупки_КурсВалюты КАК КурсыВалютыЦены
	|		ПО (ЦеныНоменклатурыПоставщиковСрезПоследних.Валюта = КурсыВалютыЦены.Валюта)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЦеныЗакупки_КурсВалютыПересчет КАК КурсыВалюты
	|		ПО (ИСТИНА)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЦеныЗакупки_СтавкиНДС КАК СтавкиНДСНоменклатуры
	|		ПО ТаблицаИсточникДанных.Номенклатура = СтавкиНДСНоменклатуры.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЦеныЗакупки_ОсновныеСтавкиНДС КАК ОсновныеСтавкиНДС
	|		ПО (ИСТИНА)";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ТекстЗапросаКоэффициентУпаковки1",
		Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки(
			"ТаблицаИсточникДанных.Упаковка",
			"ТаблицаИсточникДанных.Номенклатура"));
...
// 	СписокЗначений Из Строка - тексты запросов получения цен для вставки.
//
Функция ТекстыЗапросовЗаполненияУсловийЗакупокПакетнаяОбработка(ПараметрыДействия, ИменаКолонок) Экспорт
	...
	|	&Партнер КАК Партнер,
	|	&ВидЦеныПоставщика КАК ВидЦеныПоставщика
	|ПОМЕСТИТЬ ЗаполнитьУсловияЗакупокПодготовительная
	|ИЗ
	|	ВтИсточникДанных КАК ВтИсточникДанных
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК ТаблицаУпаковок
	|		ПО &ТекстПоляУпаковка = ТаблицаУпаковок.Ссылка
	...
	Если ПараметрыДействия.ЦенаВключаетНДС <> Неопределено Тогда
		ТекстВидЦеныСОтбором =
		"ВЫБОР КОГДА ВЫРАЗИТЬ(&ВидЦеныПоставщика КАК Справочник.ВидыЦенПоставщиков).ЦенаВключаетНДС = &ЦенаВключаетНДС ТОГДА
		| 		&ВидЦеныПоставщика
		|	ИНАЧЕ
		|		ЗНАЧЕНИЕ(Справочник.ВидыЦенПоставщиков.ПустаяСсылка)
		|	КОНЕЦ"; //@Query-part
	...
	Если ЗначениеЗаполнено(ИменаКолонок.ИмяКолонкиВидЦены) Тогда
		...
		ТекстПоляВидЦены = СтрШаблон(
			"ВЫБОР
			|		КОГДА &ВидЦеныПоставщика = ЗНАЧЕНИЕ(Справочник.ВидыЦенПоставщиков.ПустаяСсылка)
			|			ТОГДА ВтИсточникДанных.%1
			|		ИНАЧЕ &ВидЦеныПоставщика
			|	КОНЕЦ",
			ИменаКолонок.ИмяКолонкиВидЦены);
	...
	Результат.Добавить(ТекстЗапроса, "ЗаполнитьУсловияЗакупокПодготовительная");
	...
	|	ТаблицаКурсов.КурсЧислитель КАК КурсЧислитель,
	|	ТаблицаКурсов.КурсЗнаменатель КАК КурсЗнаменатель
	|ПОМЕСТИТЬ УсловияЗакупок_КурсВалюты
	|ИЗ
	|	РегистрСведений.ОтносительныеКурсыВалют.СрезПоследних(&Дата, БазоваяВалюта = &БазоваяВалюта) КАК ТаблицаКурсов
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	Результат.Добавить(ТекстЗапроса, "УсловияЗакупок_КурсВалюты");
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаКурсов.КурсЧислитель КАК КурсЧислитель,
	|	ТаблицаКурсов.КурсЗнаменатель КАК КурсЗнаменатель
	|ПОМЕСТИТЬ УсловияЗакупок_КурсВалютыПересчет
	|ИЗ
	|	РегистрСведений.ОтносительныеКурсыВалют.СрезПоследних(&Дата, Валюта = &Валюта
	|	И БазоваяВалюта = &БазоваяВалюта) КАК ТаблицаКурсов";
	
	Результат.Добавить(ТекстЗапроса, "УсловияЗакупок_КурсВалютыПересчет");
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаСтавокНДС.Номенклатура КАК Номенклатура,
	|	ТаблицаСтавокНДС.СтавкаНДС КАК СтавкаНДС
	|ПОМЕСТИТЬ УсловияЗакупок_СтавкиНДС
	|ИЗ
	|	РегистрСведений.СтавкиНДСНоменклатуры.СрезПоследних(&Дата, Номенклатура В
	|		(ВЫБРАТЬ
	|			ВтИсточникДанных.Номенклатура
	...
	Результат.Добавить(ТекстЗапроса, "УсловияЗакупок_СтавкиНДС");
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаСтавокНДС.СтавкаНДС КАК СтавкаНДС
	|ПОМЕСТИТЬ УсловияЗакупок_ОсновныеСтавкиНДС
	|ИЗ
	|	РегистрСведений.ОсновныеСтавкиНДС.СрезПоследних(&Дата, Страна = &СтранаРегистрации) КАК ТаблицаСтавокНДС";
	
	Результат.Добавить(ТекстЗапроса, "УсловияЗакупок_ОсновныеСтавкиНДС");
	...
	|			ТаблицаТоваров.ВидЦеныПоставщика КАК ВидЦеныПоставщика
	|		ИЗ
	|			ЗаполнитьУсловияЗакупокПодготовительная КАК ТаблицаТоваров)) КАК ТаблицаРегистра
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура,
	|	Характеристика";
	...
	"ВЫБРАТЬ
	|	ТаблицаИсточникДанных.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|	ТаблицаИсточникДанных.ВидЦеныПоставщика ВидЦеныПоставщика,
	|	ВЫБОР
	|		КОГДА &ВернутьМногооборотнуюТару = ИСТИНА
	|		И ТаблицаНоменклатуры.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
	...
	|	КОНЕЦ КАК Цена
	|ИЗ
	|	ЗаполнитьУсловияЗакупокПодготовительная КАК ТаблицаИсточникДанных
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК ТаблицаНоменклатуры
	|		ПО ТаблицаИсточникДанных.Номенклатура = ТаблицаНоменклатуры.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЦеныНоменклатурыПоставщиковСрезПоследних КАК ЦеныНоменклатурыПоставщиковСрезПоследних
	|		ПО ТаблицаИсточникДанных.Номенклатура = ЦеныНоменклатурыПоставщиковСрезПоследних.Номенклатура
	|		И ТаблицаИсточникДанных.Характеристика = ЦеныНоменклатурыПоставщиковСрезПоследних.Характеристика
	|		ЛЕВОЕ СОЕДИНЕНИЕ УсловияЗакупок_КурсВалюты КАК КурсыВалютыЦены
	|		ПО (ЦеныНоменклатурыПоставщиковСрезПоследних.Валюта = КурсыВалютыЦены.Валюта)
	|		ЛЕВОЕ СОЕДИНЕНИЕ УсловияЗакупок_КурсВалютыПересчет КАК КурсыВалюты
	|		ПО (ИСТИНА)
	|		ЛЕВОЕ СОЕДИНЕНИЕ УсловияЗакупок_СтавкиНДС КАК СтавкиНДСНоменклатуры
	|		ПО ТаблицаИсточникДанных.Номенклатура = СтавкиНДСНоменклатуры.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ УсловияЗакупок_ОсновныеСтавкиНДС КАК ОсновныеСтавкиНДС
	|		ПО (ИСТИНА)";
