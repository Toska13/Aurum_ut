...
Процедура ПроверкаВозрастаМаксПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если ПустаяСтрока(Форма.ТокенОрганизации) Тогда 
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма.Параметры, "Организация") 
			И ЗначениеЗаполнено(Форма.Параметры.Организация) Тогда
			УстановитьПривилегированныйРежим(Истина);
			Запрос = Новый Запрос;
			Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
			               |	НастройкиАвторизацииМакс.ТокенАвторизации КАК ТокенОрганизации
			               |ИЗ
			               |	Справочник.НастройкиАвторизацииМакс КАК НастройкиАвторизацииМакс
			               |ГДЕ
			               |	НастройкиАвторизацииМакс.Организация = &Организация
			               |	И НЕ НастройкиАвторизацииМакс.ПометкаУдаления
			               |
			               |УПОРЯДОЧИТЬ ПО
			               |	НастройкиАвторизацииМакс.Ссылка";
			Запрос.УстановитьПараметр("Организация", Форма.Параметры.Организация);
			ВыборкаНастроек = Запрос.Выполнить().Выбрать();
			УстановитьПривилегированныйРежим(Ложь);
			Если ВыборкаНастроек.Следующий() Тогда
				Форма.ТокенОрганизации = ВыборкаНастроек.ТокенОрганизации;
			Иначе
				Отказ = Истина;
				ШаблонОписанияОшибки = НСтр("ru = 'Для организации %1 не настроено подключение к сервису Макс'");
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОписанияОшибки, Форма.Параметры.Организация);
...
// Переопределение формирования инструкции покупателя MAX
//
// Параметры:
//  ИнструкцияПокупателейMAX - ТабличныйДокумент - исходный макет инструкциb покупателя MAX
//
Процедура ДополнитьМакетИнструкцииПокупателяMAX(ИнструкцияПокупателейMAX) Экспорт
