...
#Область ИндивидуальныеПараметрыИОтборы
	...
	ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансПолученныеАвансы")
		ИЛИ ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансЗадолженностьКлиентов") Тогда
	...
	ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансВыданныеАвансы")
		ИЛИ ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансЗадолженностьПередПоставщиками") Тогда
	...
	ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансЗадолженностьСобственныхОрганизаций") Тогда
	...
	ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансОбязательстваПередСобственнымиОрганизациями") Тогда
		Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
	...
	ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансДенежныеСредстваБезналичные")
		ИЛИ ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансДенежныеСредстваНаличные")
		ИЛИ ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансДенежныеСредстваВПути") Тогда
		...
		Если ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансДенежныеСредстваБезналичные") Тогда
			ТипДС.ПравоеЗначение = Перечисления.ТипыДенежныхСредств.Безналичные;
		ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансДенежныеСредстваНаличные") Тогда
			ТипДС.ПравоеЗначение = Перечисления.ТипыДенежныхСредств.Наличные;
		ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансДенежныеСредстваВПути") Тогда
			ТипДС.ПравоеЗначение = Перечисления.ТипыДенежныхСредств.ДенежныеСредстваВПути;
		КонецЕсли;
		Отбор.Вставить("ТипДенежныхСредств", ТипДС);
		
	ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансДенежныеСредстваУПодотчетныхЛиц") Тогда
	...
	ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансЗадолженностьПоКредитам")
		ИЛИ ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансДепозитыВБанках")
		...
		Если ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансЗадолженностьПоКредитам") Тогда
			ХарактерДоговора.ПравоеЗначение = Перечисления.ХарактерыДоговоровФинансовыхИнструментов.КредитИлиЗайм;
		ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансДепозитыВБанках") Тогда
	...
	ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансТоварыНаОптовыхСкладах")
		ИЛИ ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансТоварыВРознице")
		ИЛИ ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансТоварыПереданныеНаКомиссию")
		ИЛИ ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансТоварыПереданныеВПереработку") Тогда
		...
		ОтборТипЗапасов.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВидЗапасов.ТипЗапасов");
		Если ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансТоварыНаОптовыхСкладах") Тогда
			ОтборТипЗапасов.ПравоеЗначение = Перечисления.ТипыЗапасов.Товар;
			
		ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансТоварыПереданныеНаКомиссию") Тогда
			ОтборТипЗапасов.ПравоеЗначение = Перечисления.ТипыЗапасов.КомиссионныйТовар;
			
		ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансТоварыПереданныеВПереработку") Тогда
	...
	ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансТоварыКОформлениюОтчетовКомитенту") Тогда
	...
	ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансНезавершенноеПроизводство") Тогда
		
	ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансРасходыБудущихПериодов")
		ИЛИ ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансДоходыТекущегоПериода")
		ИЛИ ПоляРасшифровки.Показатель = СтатьяАктиваПассива("УпрБалансРасходыТекущегоПериода") Тогда
...
// Вызывается для отчетов "Применение серии номенклатуры" и "Структура серии номенклатуры"
// 
// Параметры:
//  Расшифровка - ДанныеРасшифровкиКомпоновкиДанных
//  АдресРасшифровки - Строка
//  
// Возвращаемое значение:
//  Структура - структура полей для расшифровки:
// * Номенклатура - СправочникСсылка.Номенклатура
// * Характеристика - СправочникСсылка.ХарактеристикиНоменклатуры
// * Серия - СправочникСсылка.СерииНоменклатуры
//
Функция ПараметрыРасшифровкиОтчетаПоСериям(Расшифровка, АдресРасшифровки) Экспорт
	...
	ИменаПолейРасшифровки.Добавить("Номенклатура");
	ИменаПолейРасшифровки.Добавить("Характеристика");
	ИменаПолейРасшифровки.Добавить("Серия");
	
	ВыбранноеПоле = ДанныеРасшифровки.Элементы[Расшифровка];
	Идентификатор = Расшифровка;
	Граница = ДанныеРасшифровки.Элементы.Количество();
	
	Пока Идентификатор <> 0 Цикл
	
		ВыбранноеПоле = ДанныеРасшифровки.Элементы[Идентификатор];
	...
	Пока Истина Цикл
		...
		Если ТипЗнч(ВыбранноеПоле) = Тип("ЭлементРасшифровкиКомпоновкиДанныхПоля") Тогда
		
			Поля = ВыбранноеПоле.ПолучитьПоля();
			
			Для Каждого Элемент Из Поля Цикл
			
				Если ИменаПолейРасшифровки.Найти(Элемент.Поле) <> Неопределено Тогда
					ПараметрыРасшифровки.Вставить(Элемент.Поле, Элемент.Значение);
				КонецЕсли;
		...
		Если Идентификатор = Граница Тогда
			Прервать;
		КонецЕсли;
...
// Вызывается для отчета "СверкаРасчетовСКонтрагентами"
// 
// Параметры:
//  Адрес - Строка
//  Расшифровка - ЗначениеПоляРасшифровкиКомпоновкиДанных 
// 
// Возвращаемое значение:
//  Структура - Получить параметры расшифровки отчета:
// * ОткрытьОбъект - Булево 
...
Функция ПолучитьПараметрыРасшифровкиОтчета(Адрес, Расшифровка) Экспорт
	...
	ДанныеРасшифровки = ДанныеОбъекта.ДанныеРасшифровки;//ДанныеРасшифровкиКомпоновкиДанных
