...
Процедура МинимальныеЦеныАлкогольнойПродукции(Товары, ПараметрыПроверки)
	...
	|	втТоварыПолная.ВидОперации КАК ВидОперации,
	|	втТоварыПолная.Крепость КАК Крепость,
	|	ЕСТЬNULL(втПодобранныеЦены.МинимальнаяЦенаБезУчетаОбъема, 0) КАК МинимальнаяЦенаБезУчетаОбъема,
	|	ВЫБОР
	|		КОГДА втПодобранныеЦены.МинимальнаяЦенаБезУчетаОбъема ЕСТЬ NULL
	|			ТОГДА 0
	|		ИНАЧЕ ВЫБОР
