...
Процедура СоздатьШаблонПоЗаказуНаДоставку(Форма, Значение) Экспорт
	...
	Если Элементы.Найти("ТипЗаказа") <> Неопределено Тогда
...
Функция ОбработатьВыборДокументовОснованийАвтоматически(Форма, ВыбранныеДокументы) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДобавляемыеДокументы) Тогда
		Возврат Результат;
	КонецЕсли;
	
	ПараметрыПроверки = ПараметрыПроверкиОснований();
	ПараметрыПроверки.ПроверитьСпособДоставки =
		СервисДоставкиКлиентПереопределяемый.ЭтоДокументРеализации(ДобавляемыеДокументы[0]);
	ПараметрыПроверки.ПолучитьКоличествоЗаказов = Истина;
	
	ДанныеПроверки = 
		СервисДоставкиВызовСервера.ДанныеПроверкиОснованияЗаказаНаДоставку(ДобавляемыеДокументы, ПараметрыПроверки);
	...
	Если ПараметрыПроверки.ПроверитьСпособДоставки И Не ДанныеПроверки.СпособДоставкиПрименим Тогда
		ТекстПредупреждения = СтрШаблон(
			НСтр("ru = 'Ввод Заказа на доставку возможен только для способа доставки  ""Перевозчик (с нашего склада)"".
				 |По документам %1 способ доставки отличается от указанного.'"), ДанныеПроверки.ТекстПояснения);
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат Ложь;
	КонецЕсли;
	
	Результаты = ДанныеПроверки.КоличествоЗаказовПоОснованиям;
...
// Новый параметры для открытия формы ВыборВремениПередачиГруза.
// 
// Возвращаемое значение:
//  Структура - Новые параметры открытия формы:
//   * ВариантОтображения - ЧастиДаты - вариант отображения формы (Дата, Время, ДатаВремя)
//   * ТолькоПросмотр - Булево - 
//   * ТипГрузоперевозки - Число - 
//   * ВариантВыбораВремени - Число - вариант 1 - отгрузка, 2 - доставка
//   * Дата - Дата - 
//   * ВремяРаботыС - Дата - 
//   * ВремяРаботыПо - Дата - 
//   * ВремяОбедС - Дата - 
//   * ВремяОбедПо - Дата - 
//   * ФиксированноеВремя - Булево
Функция НовыйПараметрыОткрытьФормуВыборВремениПередачиГруза() Экспорт
	
	Результат = Новый Структура();
	
	Результат.Вставить("ВариантОтображения", ЧастиДаты.ДатаВремя);
	...
	Результат.Вставить("ФиксированноеВремя", Ложь);
...
// Фильтрует переданный список значений по вхождению строки поиска в представление значений элементов списка
// и выделяет первое найденное вхождение цветом.
// 
// Параметры:
//  СписокИсточник - СписокЗначений Из Произвольный- любой список значений, строковые представления которого будут форматироваться по строке поиска
//  СтрокаПоиска - Строка - подстрока поиска в представлении 
// 
// Возвращаемое значение:
//  СписокЗначений Из Произвольный - отфильтрованный список значений, для которого представление элемента - форматированная строка. 
Функция ФорматированныйСписокПоСтрокеПоиска(СписокИсточник, Знач СтрокаПоиска) Экспорт

	Результат = Новый СписокЗначений;
	
	СтрокаПоиска = НРег(СтрокаПоиска);
	Для Каждого ЭлементИсточника Из СписокИсточник Цикл
		ПозицияВхожденияСтрокиПоиска = СтрНайти(НРег(ЭлементИсточника.Представление), СтрокаПоиска);
		Если ПозицияВхожденияСтрокиПоиска > 0 Тогда
			ЭлементПредставление = ЭлементИсточника.Представление;
			ФорматированноеПредставление = Новый Массив;
			
			ЧастьСтрокиБезОформления = Лев(ЭлементПредставление, ПозицияВхожденияСтрокиПоиска - 1);
			ЧастьСтрокиСОформлением = Сред(ЭлементПредставление, ПозицияВхожденияСтрокиПоиска);
			Если Не ПустаяСтрока(ЧастьСтрокиБезОформления) Тогда
				ФорматированноеПредставление.Добавить(ЧастьСтрокиБезОформления);
			КонецЕсли;
			ФорматированноеПредставление.Добавить(СтрНайтиИВыделитьОформлением(ЧастьСтрокиСОформлением, СтрокаПоиска));
			
			Результат.Добавить(ЭлементИсточника.Значение, Новый ФорматированнаяСтрока(ФорматированноеПредставление));
		КонецЕсли;
	КонецЦикла;
...
// Конструктор параметров проверки документов-оснований для Заказов на доставку
// 
// Возвращаемое значение:
//  Структура - Параметры проверки оснований:
// * ПроверитьСпособДоставки - Булево - 
// * ПолучитьКоличествоЗаказов - Булево - 
Функция ПараметрыПроверкиОснований() Экспорт
	Результат = Новый Структура;
	Результат.Вставить("ПроверитьСпособДоставки", Ложь);
	Результат.Вставить("ПолучитьКоличествоЗаказов", Ложь);
	Возврат Результат;
КонецФункции
