...
//		* Элемент                      - ПолеФормы, Неопределено - поле выбора соглашения.
//		* Партнер                      - СправочникСсылка.Партнеры, Неопределено - ссылка на партнера, для которого
//																					нужно выбрать соглашение.
//		* Организация                  - СправочникСсылка.Организации, Неопределено - ссылка на организацию, для которой
//																						выбирается соглашение.
//		* Документ                     - СправочникСсылка.СоглашенияСКлиентами, Неопределено - ссылка на ранее выбранное
//																								соглашение для начального
//																								позиционирования в списке.
...
//		* ХозяйственнаяОперация        - ПеречислениеСсылка.ХозяйственныеОперации, Неопределено - хозяйственная операция объекта,
//																									по которой будет осуществляться
//																									отбор соглашения.
//		* ДанныеФормыСтруктура         - ДанныеФормыСтруктура, Неопределено - данные формы инициатора, выбора соглашения.
//		* ИспользуютсяДоговорыКонтрагентов - Булево, Неопределено - признак отбора, позволяющий включать в список выбора
//																	соглашения, для которых обязательно указание договора.
//		* КомиссионныеПродажи25        - Булево, Неопределено - признак отбора, позволяющий включать в список выбора
//																соглашения, которые используют настройку комиссионных
//																продаж версии 2.5.
//		* ОтгрузкаБезПереходаПраваСобственности25 - Булево, Неопределено - признак отбора, позволяющий включать в список
//																			выбора соглашения, по которым возможна
//																			реализация товаров без без перехода права
//																			собственности версии 2.5.
//																			Признак указания использования договоров
//																			контрагентов в соглашении должен принимать
//																			значение 'Истина'.
//		* ТолькоОперацииПередачи        - Булево, Неопределено - признак отбора, позволяющий включать в список выбора
//																соглашения, которые используют настройку комиссионных
//																продаж версии 2.5.
//		* ОперацииПередачиОтгрузки      - Булево, Неопределено - признак отбора, позволяющий включать в список выбора
//																соглашения, которые используют настройку комиссионных продаж версии 2.5.
//																и реализация товаров без без перехода права собственности версии 2.5.
//
Функция ПараметрыНачалаВыбораСоглашенияСКлиентом() Экспорт
	
	СтруктураПараметров = Новый Структура;
	
	СтруктураПараметров.Вставить("Элемент", Неопределено);
	...
	СтруктураПараметров.Вставить("ОтгрузкаБезПереходаПраваСобственности25", Неопределено);
	СтруктураПараметров.Вставить("ТолькоОперацииПередачи", Неопределено);
	СтруктураПараметров.Вставить("ОперацииПередачиОтгрузки", Неопределено);
...
Процедура НачалоВыбораСоглашенияСКлиентом(СтруктураПараметров, СтандартнаяОбработка) Экспорт
	...
	Если Не ЗначениеЗаполнено(Партнер) Тогда
		...
		ПараметрыФормы = Новый Структура("Отбор", ОтборКлиент);
		ОписаниеОповещенияЗакрытияФормы = Новый ОписаниеОповещения("НачалоВыбораСоглашенияСКлиентомЗавершение",
																	ЭтотОбъект,
																	СтруктураПараметров);
...
// Выполняет подготовку параметров для открытия формы выбора справочника Соглашения с клиентами, после указания пользователем
// клиента и открывает форму выбора.
//
// Параметры:
...
Процедура НачалоВыбораСоглашенияСКлиентомЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДанныеФормыСтруктура <> Неопределено Тогда
		...
		Если ДанныеФормыСтруктура.Свойство("Контрагент") Тогда
			ПартнерыИКонтрагентыВызовСервера.ЗаполнитьКонтрагентаПартнераПоУмолчанию(
				Партнер,
				ДанныеФормыСтруктура.Контрагент,
				Истина);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОповеститьОбОтменеВыделенныхСтрок(ПричинаОтмены) Экспорт
	...
	ШаблонТекстаОповещения = НСтр("ru='Выделенные строки отменены%ТекстПричинаОтмены%.'");
	
	Если ЗначениеЗаполнено(ПричинаОтмены) Тогда
		ТекстПричинаОтмены =
			СтрЗаменить(НСтр("ru = ' по причине ""%ПричинаОтмены%""'"), "%ПричинаОтмены%", ПричинаОтмены);
	Иначе
		ТекстПричинаОтмены = "";
	КонецЕсли;
	
	ТекстОповещения = СтрЗаменить(ШаблонТекстаОповещения, "%ТекстПричинаОтмены%", ТекстПричинаОтмены);
	...
	Если КоличествоОтмененныхСтрок = 0 Тогда
		...
		Иначе
			...
			Если ПроверятьОстатки Тогда
				ШаблонТекстаОповещения = НСтр("ru='Все непоставленные строки отменены%ТекстПричинаОтмены%.'");
			Иначе
				ШаблонТекстаОповещения = НСтр("ru='Все строки отменены%ТекстПричинаОтмены%.'");
			КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из МассивСтрок Цикл
		...
		Если СтрокаТаблицы <> Неопределено Тогда
			...
			Если СтруктураДействийСИзмененнымиСтроками <> Неопределено Тогда
				ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(СтрокаТаблицы, СтруктураДействийСИзмененнымиСтроками, Неопределено);
			КонецЕсли;
...
Процедура ОбработкаНавигационнойСсылкиПодчиненныхДокументов(Форма, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка) Экспорт
	...
	МассивСтрок = СтрРазделить(НавигационнаяСсылкаФорматированнойСтроки, Разделитель);
	...
	Если МассивСтрок.Количество() > 2 Тогда
		...
		ИмяХозяйственнойОперации = МассивСтрок[2];
		ХозяйственнаяОперация = ПредопределенноеЗначение(ИмяХозяйственнойОперации);
	...
	ИначеЕсли Действие = "Оформить" Тогда
		...
		ОбщегоНазначенияУТКлиент.ОткрытьФормуПослеПроверкиПроведенностиДокумента(ПолноеИмя + ".ФормаОбъекта",
			?(ЗначениеЗаполнено(ХозяйственнаяОперация),
				Новый Структура("ХозяйственнаяОперация", ХозяйственнаяОперация),
				Новый Структура()),
			Форма,
			"Независимый",
			"Основание");
...
Процедура СформироватьКомплектДокументовВызов(Результат, ДополнительныеПараметры) Экспорт
	...
	ОформитьОтгрузку = ЭтоОтгрузкаТоваров(ДополнительныеПараметры);
	ДополнительныеПараметры.Вставить("СоответствиеРаспоряжений", ИнициализироватьРаспоряжения(МассивСсылок, ОформитьОтгрузку));
...
Процедура СформироватьКомплектДокументов(Форма, СоответствиеРаспоряжений, ПараметрыФормирования = Неопределено, НакопленныйСписокОшибок = Неопределено) Экспорт
	...
	ОформитьОтгрузку = ЭтоОтгрузкаТоваров(ПараметрыФормирования);
	Если ОформитьОтгрузку = Ложь Тогда
	...
	Иначе
		ПараметрыСозданныхДокументов = ПродажиВызовСервера.ОформитьОтгрузкуНаСервере(СоответствиеРаспоряжений, ПараметрыФормирования);
	КонецЕсли;
...
// 	ОформитьОтгрузку - Булево - Признак отгрузки товаров
// 	
// Возвращаемое значение:
// 	Соответствие - где:
...
// 
Функция ИнициализироватьРаспоряжения(СписокСсылок, ОформитьОтгрузку) Экспорт
	...
	ИначеЕсли ТипЗнч(СписокСсылок) = Тип("ДанныеФормыКоллекция") Тогда
		...
		Для Каждого Строка Из СписокСсылок Цикл
			...
			Если ОформитьОтгрузку = Истина Тогда
				Структура.СостояниеНакладной = Строка.СостояниеОтгрузки;
				Структура.СостояниеОрдера = Строка.СостояниеОрдераОтгрузки;
				
				Если НЕ ТипЗнч(Ключ) = Тип("Структура") Тогда
					НайденноеРаспоряжение = СоответствиеРаспоряжений.Получить(Ключ);
					Если НЕ НайденноеРаспоряжение = Неопределено Тогда
						Если Структура.СостояниеНакладной > НайденноеРаспоряжение.СостояниеНакладной Тогда
							Структура.СостояниеНакладной = НайденноеРаспоряжение.СостояниеНакладной;
						КонецЕсли;
						
						Если Структура.СостояниеОрдера > НайденноеРаспоряжение.СостояниеОрдера Тогда
							Структура.СостояниеОрдера = НайденноеРаспоряжение.СостояниеОрдера;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Функция ПараметрыФормированияДокументов(ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.Свойство("ОформитьОтгрузку") Тогда
		ПараметрыФормирования.ОформитьОтгрузку = ДополнительныеПараметры.ОформитьОтгрузку;
	КонецЕсли;
	
	Если ДополнительныеПараметры.Свойство("ИмяКомандыОтгрузки") Тогда
		ПараметрыФормирования.ИмяКомандыОтгрузки = ДополнительныеПараметры.ИмяКомандыОтгрузки;
	КонецЕсли;
...
Функция ЭтоОтгрузкаТоваров(ПараметрыФормирования)
	
	ОформитьОтгрузку = Ложь;
	Если ПараметрыФормирования.Свойство("ОформитьОтгрузку") Тогда
		ОформитьОтгрузку = ПараметрыФормирования.ОформитьОтгрузку;
	КонецЕсли;
	
	Возврат ОформитьОтгрузку;
