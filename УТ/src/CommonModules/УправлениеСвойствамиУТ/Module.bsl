...
// Выполняет заполнение колонок, содержащих описание дополнительных реквизитов справочника.
//
// Параметры:
//	Форма - ФормаКлиентскогоПриложения - форма, содержащая заполняемую таблицу.
//	РеквизитТаблица - Строка - путь к реквизиту раблицы формы.
//	КолонкаВладельцаСвойств - Строка - имя колонки, содержащей сведения о владельце дополнительных реквизитов.
//
Процедура ЗаполнитьКолонкиДополнительныхРеквизитов(Форма, РеквизитТаблица, КолонкаВладельцаСвойств) Экспорт
	...
	ТаблицаФормы = ТаблицаПоИмени(Форма, РеквизитТаблица);
	
	Если ТаблицаФормы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ТаблицаВладельцевСвойств = ТаблицаФормы.Выгрузить(); // ТаблицаЗначений - 
	ТаблицаВладельцевСвойств.Индексы.Добавить(КолонкаВладельцаСвойств);
	
	ПриведенноеИмяТаблицы = СтрЗаменить(РеквизитТаблица, ".", "_");
	
	ТаблицаСвойств = Форма[ПриведенноеИмяТаблицы + "Свойства_ОписаниеДополнительныхРеквизитов"].Выгрузить(); // ТаблицаЗначений - 
	ТаблицаСвойств.Индексы.Добавить("Свойство");
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТаблицаДопРеквизитов.Ссылка				КАК Серия,
	|	ТаблицаДопРеквизитов.Свойство			КАК Свойство,
	|	ТаблицаДопРеквизитов.Значение			КАК Значение,
	|	ТаблицаДопРеквизитов.ТекстоваяСтрока	КАК ТекстоваяСтрока
	|ИЗ
	|	Справочник.СерииНоменклатуры.ДополнительныеРеквизиты КАК ТаблицаДопРеквизитов
	|ГДЕ
	|	ТаблицаДопРеквизитов.Ссылка В(&Серии)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Серия
	|
	...
	Запрос.УстановитьПараметр("Серии", ТаблицаВладельцевСвойств.ВыгрузитьКолонку(КолонкаВладельцаСвойств));
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	ТекущаяСерия	= Неопределено;
	ОтборСтрок		= Новый Структура(КолонкаВладельцаСвойств);
	СтрокиСерий		= Неопределено;
	
	Пока Выборка.Следующий() Цикл
		СтрокиСвойств = ТаблицаСвойств.Найти(Выборка.Свойство, "Свойство");
		...
		Если Выборка.Серия <> ТекущаяСерия Тогда
			ТекущаяСерия = Выборка.Серия;
			ОтборСтрок[КолонкаВладельцаСвойств] = Выборка.Серия;
			СтрокиСерий = ТаблицаВладельцевСвойств.НайтиСтроки(ОтборСтрок);
		КонецЕсли;
		
		РеквизитЗначение = ?(СтрокиСвойств.МногострочноеПолеВвода > 0, Выборка.ТекстоваяСтрока, Выборка.Значение);
		
		Для Каждого СтрокаСерии Из СтрокиСерий Цикл
			СтрокаСерии[СтрокиСвойств.ИмяРеквизитаЗначение] = РеквизитЗначение;
		КонецЦикла;
	КонецЦикла;
	
	ТаблицаФормы.Загрузить(ТаблицаВладельцевСвойств);
...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	ДоставкаТоваров.ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти);
	...
	Если НЕ ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПодсистемы") Тогда
		...
		МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
			"ru = 'Структура предприятия'", КодыЯзыков); // @НСтр-2
		...
		Набор.Используется = Истина;
		МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
			"ru = 'Физические лица'", КодыЯзыков); // @НСтр-2
		...
		Набор.Используется = Истина;
		МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
			"ru = 'Виды документов физических лиц'", КодыЯзыков); // @НСтр-2
		...
		Набор.Используется = Истина;
		МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
			"ru = 'Зачисления ДС на лицевые счета'", КодыЯзыков); // @НСтр-2
	...
	Набор.Используется = Истина;
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Сверки начальных остатков по складам'", КодыЯзыков); // @НСтр-2
	...
	Набор.Используется = Истина;
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Договоры подключения к платежным системам'", КодыЯзыков); // @НСтр-2
	
	Набор = Элементы.Добавить();
	Набор.ИмяПредопределенногоНабора = "Документ_ОперацияСЭлектроннымБилетом";
	Набор.Ссылка = Новый УникальныйИдентификатор("b9b299f9-f870-44c7-bbea-b00f8b7b1ae0");
	Набор.Используется = Истина;
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Операции с электронными билетами'", КодыЯзыков); // @НСтр-2
	
	//++ Локализация
	
	Набор = Элементы.Добавить();
	Набор.ИмяПредопределенногоНабора = "Документ_БронированиеSmartway";
	Набор.Ссылка = Новый УникальныйИдентификатор("10f6823d-bddf-47c7-a866-9d03bda5c570");
	Набор.Используется = Истина;
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Бронирования Smartway'", КодыЯзыков); // @НСтр-2
	//-- Локализация
	...
	Набор.Используется = Истина;
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Заявки на командировку'", КодыЯзыков); // @НСтр-2
	...
	Набор.Используется = Истина;
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Ценовые группы'", КодыЯзыков); // @НСтр-2
	...
	Набор.Используется = Истина;
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Корректировка назначения товаров'", КодыЯзыков); // @НСтр-2
	
	Набор = Элементы.Добавить();
	Набор.ИмяПредопределенногоНабора = "Документ_АктКонтроляКачестваТоваров";
	Набор.Ссылка = Новый УникальныйИдентификатор("3a094440-02bd-4bde-919b-c4e36fe634ec");
	Набор.Используется = Истина;
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Акт контроля качества товаров'", КодыЯзыков); // @НСтр-2
	
	Набор = Элементы.Добавить();
	Набор.ИмяПредопределенногоНабора = "Документ_ЗаявкаНаКонтрольКачестваТоваров";
	Набор.Ссылка = Новый УникальныйИдентификатор("a7b1e6db-350c-4739-a9de-1cb0240ee2c0");
	Набор.Используется = Истина;
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Заявка на контроль качества товаров'", КодыЯзыков); // @НСтр-2
	...
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Номенклатура'", КодыЯзыков); // @НСтр-2
	...
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(ДочернийНабор, "Наименование",
		"ru = 'Конкуренты'", КодыЯзыков); // @НСтр-2
	...
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(ДочернийНабор, "Наименование",
		"ru = 'Поставщики'", КодыЯзыков); // @НСтр-2
	...
	Набор.Используется = Истина;
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Серии номенклатуры'", КодыЯзыков); // @НСтр-2
	...
	Набор.Используется = Истина;
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Характеристики номенклатуры'", КодыЯзыков); // @НСтр-2
	...
	Набор.Используется = Истина;
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Инциденты'", КодыЯзыков); // @НСтр-2
	...
	Наименование = "ru = 'Заявки на обеспечение'"; // @НСтр
	ДобавитьЭлементНабораСвойств(Элементы, "46ead614-6e00-11f0-bd11-e89c25df8ebe", 
	"Документ_ЗаявкаНаОбеспечение", Наименование, КодыЯзыков);
	
	Наименование = "ru = 'Заявки на закупку'"; // @НСтр
	ДобавитьЭлементНабораСвойств(Элементы, "46ead615-6e00-11f0-bd11-e89c25df8ebe", 
	"Документ_ЗаявкаНаЗакупку", Наименование, КодыЯзыков);
	...
	Наименование = "ru = 'Корректировки графика взаиморасчетов'"; // @НСтр
	ДобавитьЭлементНабораСвойств(Элементы, "19200cc1-040f-4ff1-a667-ae81feaac1f0", 
	"Документ_КорректировкаГрафикаВзаиморасчетов", Наименование, КодыЯзыков);
	...
	Наименование = "ru = 'Корректировки расчетов с подотчетными лицами'"; // @НСтр
	ДобавитьЭлементНабораСвойств(Элементы, "c40cbb36-a7a5-4213-a8ed-bb788eaf289a", 
	"Документ_КорректировкаРасчетовСПодотчетнымиЛицами", Наименование, КодыЯзыков);
	...
	Наименование = "ru = 'Оприходования излишков отгруженных товаров'"; // @НСтр
	ДобавитьЭлементНабораСвойств(Элементы, "46f35046-5db1-4da9-83f6-b4cd9f83905b", 
	"Документ_ОприходованиеИзлишковОтгруженныхТоваров", Наименование, КодыЯзыков);
	...
	Наименование = "ru = 'Отгрузка товаров клиентам'"; // @НСтр
	ДобавитьЭлементНабораСвойств(Элементы, "6b0018c6-9353-4e4b-9dc4-189db10560ae", 
	"Документ_ОтгрузкаТоваровКлиенту", Наименование, КодыЯзыков);
	...
	Наименование = "ru = 'Списания отгруженных товаров'"; // @НСтр
	ДобавитьЭлементНабораСвойств(Элементы, "02214fff-b37e-4819-a630-691a9197cd68", 
	"Документ_СписаниеОтгруженныхТоваров", Наименование, КодыЯзыков);
	...
	Наименование = "ru = 'Поступления товаров от клиентов'"; // @НСтр
	ДобавитьЭлементНабораСвойств(Элементы, "124b1db9-e1e0-4cdc-ad87-7e23dee17814", 
	"Документ_ПоступлениеТоваровОтКлиента", Наименование, КодыЯзыков);
	...
	Наименование = "ru = '(не используется) Дополнительные отчеты и обработки'"; // @НСтр
	...
	Наименование = "ru = 'Подтверждения ставки НДС 0%'"; // @НСтр
	ДобавитьЭлементНабораСвойств(Элементы, "75066758-e8a1-4ec0-b3af-bf1e21ec7f24", 
	"Документ_ПодтверждениеНулевойСтавкиНДС", Наименование, КодыЯзыков);
	
	Наименование = "ru = 'Документ о предстоящей поставке (ДОПП)'"; // @НСтр
	ДобавитьЭлементНабораСвойств(Элементы, "9c539d3a-9b24-4bb5-81b6-29b2d3d7e79c", 
	"Документ_ДокументОПредстоящейПоставке", Наименование, КодыЯзыков);
	...
	Набор.Используется = Истина;
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Объекты расчетов'", КодыЯзыков); // @НСтр-2

	Набор = Элементы.Добавить();
	Набор.ИмяПредопределенногоНабора = "Справочник_ВидыОповещенийПользователей";
	Набор.Ссылка = Новый УникальныйИдентификатор("fc305419-3db3-4971-b4e5-c90826790356");
	Набор.Используется  = ПолучитьФункциональнуюОпцию("ИспользоватьОповещенияПользователейОСобытиях");
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Виды оповещений пользователей'", КодыЯзыков); // @НСтр-2
