...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ДанныеОснованияПлатежаВСервисе", Истина);
	Методы.Вставить("ИнтеграцияПоНастройкеВыполнена", Истина);
	Методы.Вставить("НастройкиОнлайнОплатыБезДоговора", Истина);
	Методы.Вставить("ОбновитьКонстантуПредопределенныеШаблоныСообщений");
	Методы.Вставить("ОтменаПлатежнойСсылки", Истина);
	Методы.Вставить("ПлатежнаяСсылка", Истина);
	Методы.Вставить("СтатусПодключенияМагазина", Истина);
...
Функция НастройкаИнтеграцииВыполнена(Организация) Экспорт
	...
	Если ЗначениеЗаполнено(НастройкиОнлайнОплаты.ИдентификаторМагазина)
		...
		Результат = Истина;
	Иначе
		РезультатПроверки = ИнтеграцияПоНастройкеВыполнена(НастройкиОнлайнОплаты.Ссылка);
		Результат = РезультатПроверки.НастройкаВыполнена;
	КонецЕсли;
...
Функция ИнтеграцияПоНастройкеВыполнена(НастройкаОнлайнОплаты) Экспорт
	...
	ЗначениеРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		НастройкаОнлайнОплаты,
		"СДоговором, ИдентификаторМагазина");
	
	Если Не ЗначениеРеквизитов.СДоговором Тогда
		Результат.НастройкаВыполнена = Истина;
		Возврат Результат;
	КонецЕсли;
	...
	ИначеЕсли РезультатПроверкиСтатуса.СтатусПодключения = СтатусыПодключенияМагазина().Запрещено Тогда
		ЗаполнитьЗначенияСвойств(Результат, РезультатПроверкиСтатуса);
...
//    * ИдентификаторОснованияПлатежа - Строка, Неопределено - идентификатор основания платежа, по которому выполняется запрос.
//
// Возвращаемое значение:
...
Функция СтатусПодключенияМагазина(Знач ВходящиеПараметры) Экспорт
	...
	Пока Истина Цикл
		...
		Если РезультатКоманды.КодСостояния = -1 Тогда
			Результат.СообщениеОбОшибке = ОбщегоНазначения.ЗапрещенДоступКИнтернетСервисамТекстСообщения();
		ИначеЕсли РезультатКоманды.КодСостояния = -2 Тогда
			Результат.СообщениеОбОшибке = ИнтернетПоддержкаПользователей.ЗапрещенДоступКИнтернетПоддержкеТекст();
		КонецЕсли;
...
Процедура ОбновитьПараметрыПредопределенныхШаблонов(ШаблоныДляОбновления)
	...
	Попытка
		...
		ШаблоныСообщенийДляОбновления = Новый ТаблицаЗначений;
		ШаблоныСообщенийДляОбновления.Колонки.Добавить("ШаблоныДляОбновления");
		ШаблоныСообщенийДляОбновления.ЗагрузитьКолонку(ШаблоныДляОбновления, "ШаблоныДляОбновления");
		
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("Справочник.ШаблоныСообщений");
		ЭлементБлокировки.ИсточникДанных = ШаблоныСообщенийДляОбновления;
		ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Ссылка", "ШаблоныДляОбновления");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		Блокировка.Заблокировать();
...
Функция ДанныеОснованияПлатежа(Знач ОснованиеПлатежа)
	...
	ТаблицаНоменклатуры.Колонки.Добавить("НаименованиеВЧеке");
...
Функция ДанныеОснованияПлатежаТоварыЧек(Знач ДанныеОснованияПлатежа)
	...
	Для каждого СтрокаНоменклатуры Из ДанныеОснованияПлатежа.Номенклатура Цикл
		...
		Если Не ПустаяСтрока(СтрокаНоменклатуры.НаименованиеВЧеке) Тогда
			СтрокаДанных.description = СтрокаНоменклатуры.НаименованиеВЧеке;
		КонецЕсли;
...
Процедура ПодготовитьПараметрыВызоваКомандыСервиса(
	...
	ИначеЕсли ИмяКоманды = "ОбновитьДанныеЗаказа" Тогда
		...
		Заголовки.Вставить(
			"X-Program-Nick",
			КодироватьСтроку(ИнтернетПоддержкаПользователей.ИмяПрограммы(), СпособКодированияСтроки.КодировкаURL));
		Заголовки.Вставить(
			"X-Program-Version",
			КодироватьСтроку(ИнтернетПоддержкаПользователей.ВерсияКонфигурации(), СпособКодированияСтроки.КодировкаURL));
...
Процедура ВыполнитьКомандуСервиса(Знач ИмяКоманды, ПараметрыКоманды, ИспользоватьНовыйПротокол, Результат, Отказ)
	...
	Если ЗначениеЗаполнено(HTTPОтвет.СообщениеОбОшибке) Тогда
		...
		Если HTTPОтвет.КодСостояния = -1 Тогда
			СообщениеОбОшибке = ОбщегоНазначения.ЗапрещенДоступКИнтернетСервисамТекстСообщения();
		ИначеЕсли HTTPОтвет.КодСостояния = -2 Тогда
			СообщениеОбОшибке = ИнтернетПоддержкаПользователей.ЗапрещенДоступКИнтернетПоддержкеТекст();
		Иначе
			СообщениеОбОшибке = ?(
