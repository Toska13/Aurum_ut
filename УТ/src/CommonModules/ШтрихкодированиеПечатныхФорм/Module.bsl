...
// Получает идентификатор ссылки из числового кода, считанного из штрихкода табличного документа.
//
// Параметры:
//  Штрихкод - Строка - считанный штрихкод табличного документа.
//  ДополнятьСтроку - Булево - признак необходимости дополнения строки штрихкода нулями.
// 
// Возвращаемое значение:
//  - Неопределено - введенный штрихкод не является числовым кодом ссылки.
//  - Строка - идентификатор ссылки из числового кода.
//
Функция ИдентификаторИзШтрихкодаТабличногоДокумента(Штрихкод, ДополнятьСтроку = Истина) Экспорт
	...
	ДлинаИдентификатораБезРазделителей = 32;
	ДлинаИдентификатораСРазделителем = 36;
	
	ШтрихкодВШестнадцатеричномВиде = ПреобразоватьДесятичноеЧислоВШестнадцатеричнуюСистемуСчисления(Число(Штрихкод));
	
	Если ДополнятьСтроку Тогда
		ШтрихкодВШестнадцатеричномВиде = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(
			ШтрихкодВШестнадцатеричномВиде, ДлинаИдентификатораБезРазделителей);
		
	ИначеЕсли СтрДлина(ШтрихкодВШестнадцатеричномВиде) <> ДлинаИдентификатораБезРазделителей Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Если СтрДлина(Идентификатор) <> ДлинаИдентификатораСРазделителем Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПолучитьСсылкуПоШтрихкодуТабличногоДокумента(Штрихкод, Менеджеры = Неопределено) Экспорт
	...
	Идентификатор = ИдентификаторИзШтрихкодаТабличногоДокумента(Штрихкод);
