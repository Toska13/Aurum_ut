...
Процедура СоздатьОбновитьДоговорСоглашение(МобильныйДоговор, Сообщение) Экспорт
	...
	Иначе
		...
		Если ВСоглашенииИспользуютсяДоговоры Тогда
			...
			СтруктураПараметров.Организация = ДоговорОбъект.Организация;
			СтруктураПараметров.Валюта = ДоговорОбъект.ВалютаВзаиморасчетов;
			ДоговорОбъект.БанковскийСчет =
...
Процедура СоздатьОбновитьЗаданиеТорговомуПредставителю(МобильноеЗадание, УзелОбмена, Сообщение, НастройкиЗадания) Экспорт
	...
	ПараметрыДействия = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(ЗаданиеОбъект);
	...
	Для Каждого СтрокаТовара Из МобильноеЗадание.Товары Цикл
		...
		НоменклатураХарактеристикаУпаковка		= СтруктураНоменклатураХарактеристикаУпаковка(
			...
			СтрокаТовара["Упаковка"]); //@NON-NLS-1 @NON-NLS-2
		...
		ОбрабатываемыеСтроки.Добавить(СтрокаДокумента);
...
Процедура ЗаписатьОрганизациюСтроки(ЗаписьXML, Знач СтрокаДанных)
	...
	СтруктураПараметров = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации();
	СтруктураПараметров.Организация = СтрокаДанных.ОрганизацияСсылка;
	СтруктураПараметров.Валюта = МобильноеПриложениеЗаказыКлиентовПовтИсп.ВалютаЗаказов();
...
Функция ДанныеДляВыгрузкиОстатков(Узел)
	...
	ОбеспечениеВДокументахСервер.РасчетОстатковДляУстановкиДействий(Набор.Запрос, ИменаТаблиц);
...
Процедура ВыгрузитьОбъектыСсылочногоТипа(ЗаписьXML, ВыгруженныеОбъекты, СтрокаПрайсЛиста, УзелОбмена)
	...
	Если ЗначениеЗаполнено(СтрокаПрайсЛиста.НоменклатураРодитель)
		...
		СтруктураРодителяНоменклатуры["СтрокаПоиска"] = СокрЛП(СтруктураРодителяНоменклатуры.Code)
			+СокрЛП(СтруктураРодителяНоменклатуры.Description); //@NON-NLS-1
	...
	Если ЗначениеЗаполнено(СтрокаПрайсЛиста.НоменклатураЕдиницаИзмерения)
		...
		СтруктураЕдиницыИзмерения["МеждународноеСокращение"] =
			СтрокаПрайсЛиста.НоменклатураЕдиницаИзмеренияМеждународноеСокращение;  //@NON-NLS-1
		СтруктураЕдиницыИзмерения["НаименованиеПолное"] =
			СтрокаПрайсЛиста.НоменклатураЕдиницаИзмеренияНаименованиеПолное;  //@NON-NLS-1
	...
	Если ЗначениеЗаполнено(СтрокаПрайсЛиста.УпаковкаСсылка)
		...
		Если ЗначениеЗаполнено(СтрокаПрайсЛиста.УпаковкиНоменклатурыЕдиницаИзмерения) Тогда
			...
			СокрЛП(СтрокаПрайсЛиста.УпаковкиНоменклатурыЕдиницаИзмерения.УникальныйИдентификатор());  //@NON-NLS-1
			Если ВыгруженныеОбъекты.Получить(СтрокаПрайсЛиста.УпаковкиНоменклатурыЕдиницаИзмерения) = Неопределено Тогда
				...
				СтруктураЕдиницыИзмерения["МеждународноеСокращение"] = 
				СтрокаПрайсЛиста.УпаковкиНоменклатурыЕдиницаИзмеренияМеждународноеСокращение; //@NON-NLS-1
				СтруктураЕдиницыИзмерения["НаименованиеПолное"] =
				СтрокаПрайсЛиста.УпаковкиНоменклатурыЕдиницаИзмеренияНаименованиеПолное; //@NON-NLS-1
		...
		Если ЗначениеЗаполнено(СтрокаПрайсЛиста.УпаковкиНоменклатурыВесЕдиницаИзмерения) 
			...
			СтруктураУпаковки["ВесЕдиницаИзмерения"] =
				СокрЛП(СтрокаПрайсЛиста.УпаковкиНоменклатурыВесЕдиницаИзмерения.УникальныйИдентификатор());  //@NON-NLS-1
			Если ВыгруженныеОбъекты.Получить(СтрокаПрайсЛиста.УпаковкиНоменклатурыВесЕдиницаИзмерения) = Неопределено Тогда
				...
				СтруктураЕдиницыИзмерения["МеждународноеСокращение"] =
				СтрокаПрайсЛиста.УпаковкиНоменклатурыВесЕдиницаИзмеренияМеждународноеСокращение; //@NON-NLS-1
				СтруктураЕдиницыИзмерения["НаименованиеПолное"] =
				СтрокаПрайсЛиста.УпаковкиНоменклатурыВесЕдиницаИзмеренияНаименованиеПолное; //@NON-NLS-1
		...
		СтруктураУпаковки["Штрихкод"] = ШтрихкодНоменклатуры(СтрокаПрайсЛиста.НоменклатураСсылка,
			СтрокаПрайсЛиста.ХарактеристикаСсылка, СтрокаПрайсЛиста.УпаковкаСсылка); //@NON-NLS-1
	...
	Если ЗначениеЗаполнено(СтрокаПрайсЛиста.Номенклатура) И (СтрокаПрайсЛиста.ТребуетсяОтправкаТовара)
		...
		СтруктураНоменклатуры["ВидНоменклатуры"]		=
			СокрЛП(СтрокаПрайсЛиста.НоменклатураВидНоменклатуры.УникальныйИдентификатор()); //@NON-NLS-1
		СтруктураНоменклатуры["ЕдиницаИзмерения"]		=
			СокрЛП(СтрокаПрайсЛиста.НоменклатураЕдиницаИзмерения.УникальныйИдентификатор()); //@NON-NLS-1
		СтруктураНоменклатуры["НаименованиеПолное"]	= ПолноеНаименованиеНоменклатурыМП; //@NON-NLS-1
		
		СтруктураНоменклатуры["СтавкаНДСЧислом"]		=
			ЦенообразованиеКлиентСервер.ПолучитьСтавкуНДСЧислом(СтрокаПрайсЛиста.СтавкаНДССсылка)/100; //@NON-NLS-1
		...
		СтруктураНоменклатуры["СтрокаПоиска"]			= СокрЛП(СтруктураНоменклатуры.Code)
			+ СокрЛП(СтруктураНоменклатуры.Description) + СокрЛП(СтруктураНоменклатуры["Артикул"]); //@NON-NLS-1 @NON-NLS-2
		...
		СтруктураНоменклатуры["ЕдиницаЗаказа"] = ?(ЗначениеЗаполнено(СтрокаПрайсЛиста.Упаковка), СтрокаПрайсЛиста.Упаковка,
			НоменклатураЕдиницаИзмеренияКакУпаковка); //@NON-NLS-1
			
		СтруктураНоменклатуры["Штрихкод"] = ШтрихкодНоменклатуры(СтрокаПрайсЛиста.НоменклатураСсылка,
			СтрокаПрайсЛиста.ХарактеристикаСсылка, СтруктураНоменклатуры["ЕдиницаЗаказа"]); //@NON-NLS-1 @NON-NLS-2
		...
		Если ЗначениеЗаполнено(СтрокаПрайсЛиста.ВесУпаковкаСсылка)
			...
			СтруктураНоменклатуры["ВесЕдиницаИзмерения"] =
				ИдентификаторУпаковкиМП(ЗаписьXML, ВыгруженныеОбъекты, СтруктураЗаписиУпаковки); //@NON-NLS-1 
		...
		Если ЗначениеЗаполнено(СтрокаПрайсЛиста.ФайлКартинкиСсылка) Тогда
			СтруктураНоменклатуры["ФайлКартинки"] =
				ИдентификаторФайлаКартинки(ЗаписьXML, ВыгруженныеОбъекты, СтрокаПрайсЛиста, УзелОбмена); //@NON-NLS-1 
		Иначе
	...
	Пока УпаковкиНоменклатуры.Следующий() Цикл
		...
		Если ВыгруженныеОбъекты.Получить(СоответствиеУпаковки) = Неопределено Тогда
			...
			Если ЗначениеЗаполнено(УпаковкиНоменклатуры.ЕдиницаИзмерения) Тогда
				СтруктураУпаковки["ЕдиницаИзмерения"] =
					СокрЛП(УпаковкиНоменклатуры.ЕдиницаИзмерения.УникальныйИдентификатор()); //@NON-NLS-1
			...
			Если ЗначениеЗаполнено(УпаковкиНоменклатуры.ВесЕдиницаИзмерения) Тогда
				СтруктураУпаковки["ВесЕдиницаИзмерения"] =
					СокрЛП(УпаковкиНоменклатуры.ВесЕдиницаИзмерения.УникальныйИдентификатор()); //@NON-NLS-1
				Если ВыгруженныеОбъекты.Получить(УпаковкиНоменклатуры.ВесЕдиницаИзмерения) = Неопределено Тогда
					...
					СтруктураЕдиницыИзмерения["МеждународноеСокращение"] =
						УпаковкиНоменклатуры.ВесЕдиницаИзмеренияМеждународноеСокращение; //@NON-NLS-1
			...
			СтруктураУпаковки["Штрихкод"] = ШтрихкодНоменклатуры(СтруктураЗаписиУпаковки.Номенклатура,
				СтруктураЗаписиУпаковки.ХарактеристикаСсылка, СтруктураЗаписиУпаковки.УпаковкаСсылка); //@NON-NLS-1
	...
	Если ВыгруженныеОбъекты.Получить(СоответствиеУпаковки) = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(РезультатЗапросаПоУпаковке.ЕдиницаИзмерения) Тогда
			...
			Если ВыгруженныеОбъекты.Получить(РезультатЗапросаПоУпаковке.ЕдиницаИзмерения) = Неопределено Тогда
				...
				СтруктураЕдиницыИзмерения["МеждународноеСокращение"] = 
					РезультатЗапросаПоУпаковке.ЕдиницаИзмеренияМеждународноеСокращение; //@NON-NLS-1
		...
		Если ЗначениеЗаполнено(РезультатЗапросаПоУпаковке.ВесЕдиницаИзмерения) Тогда
			...
			Если ВыгруженныеОбъекты.Получить(РезультатЗапросаПоУпаковке.ВесЕдиницаИзмерения) = Неопределено Тогда
				...
				СтруктураЕдиницыИзмерения["МеждународноеСокращение"] =
					РезультатЗапросаПоУпаковке.ВесЕдиницаИзмеренияМеждународноеСокращение; //@NON-NLS-1
...
Функция ИдентификаторФайлаКартинки(ЗаписьXML, ВыгруженныеОбъекты, СтрокаПрайса, УзелОбмена)
	...
	Если ВыгруженныеОбъекты.Получить(ИдентификаторФайлаКартинки) = Неопределено 
		...
		РеквизитыФайлаКартинкиXDTO["ВладелецФайла"] =
			СокрЛП(СтрокаПрайса.ФайлКартинкиВладелецФайла.УникальныйИдентификатор()); //@NON-NLS-1
...
Процедура ИнициализироватьДокумент(ДокументОбъект)
	...
	ПараметрыСчета = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации(ДокументОбъект.Организация);
	ДокументОбъект.БанковскийСчет = ДенежныеСредстваСервер.ПолучитьБанковскийСчетОрганизацииПоУмолчанию(
		ПараметрыСчета, ДокументОбъект.БанковскийСчет);
	
	ПараметрыСчета = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаКонтрагента(ДокументОбъект.Контрагент);
	ДокументОбъект.БанковскийСчетКонтрагента = ДенежныеСредстваСервер.ПолучитьБанковскийСчетКонтрагентаПоУмолчанию(
		ПараметрыСчета, ДокументОбъект.БанковскийСчетКонтрагента);
...
Функция ИнформацияОКонтрагенте(Знач СтрокаТаблицы)
	...
	СтруктураКонтрагента["ИННКПП"] = СокрЛП(СтрокаТаблицы.КонтрагентИНН)
		+ ?(ЗначениеЗаполнено(СтрокаТаблицы.КонтрагентКПП),"/" + СокрЛП(СтрокаТаблицы.КонтрагентКПП), ""); //@NON-NLS-1
	...
	СтруктураКонтрагента["ПредставлениеКонтрагента"] = СтрокаТаблицы.КонтрагентНаименованиеПолное
		+ " (" +СтруктураКонтрагента["ИННКПП"] + ")";//@NON-NLS-1 @NON-NLS-3
...
Функция СоглашениеСКлиентомПоОрганизацииИВидуЦен(СтрокаТаблицы)
	...
	СтрокаСоглашения["Дата"] = ?(ЕстьДоговор И ЗначениеЗаполнено(СтрокаТаблицы.ДоговорДата), СтрокаТаблицы.ДоговорДата,
		СтрокаТаблицы.СоглашениеДата); //@NON-NLS-1
	...
	СтрокаСоглашения["Номер"] = ?(ЕстьДоговор И ЗначениеЗаполнено(СтрокаТаблицы.ДоговорНомер), СтрокаТаблицы.ДоговорНомер,
		СтрокаТаблицы.СоглашениеНомер); //@NON-NLS-1
	СтрокаСоглашения["Организация"] = СокрЛП(СтрокаТаблицы.Организация.УникальныйИдентификатор()); //@NON-NLS-1
	СтрокаСоглашения["Пояснение"] =
		СтрШаблон(НСтр("ru = '%1 (%2, %3, %4)'"),
		ПредставлениеДоговора, СокрЛП(СтрокаТаблицы.КонтрагентСсылка),
			СокрЛП(СтрокаТаблицы.Организация), СокрЛП(СтрокаТаблицы.ВидЦены)); //@NON-NLS-1
	ИспользуетсяНесколькоСкладов = ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоСкладов");
	Если ИспользуетсяНесколькоСкладов Тогда
	...
	УникальныйКлюч = СтрокаСоглашения["Организация"] + "/"
		...
		+ Формат(СтрокаСоглашения["Комиссия"], "БЛ=0; БИ=1"); //@NON-NLS-1 @NON-NLS-3 @NON-NLS-5 @NON-NLS-7 @NON-NLS-9 @NON-NLS-11
...
Процедура СоздатьОбновитьПриходныйКассовыйОрдер(СтруктураОплаты, Сообщение)
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ПриходныйКассовыйОрдерДоговор) Тогда
			ПараметрыЗаполненияРасшифровки = ВзаиморасчетыЗаполнениеРасшифровкиПлатежа.ПараметрыЗаполненияРасшифровкиПлатежаПоЗаказу();
			...
			ВзаиморасчетыЗаполнениеРасшифровкиПлатежа.ЗаполнитьРасшифровкуПлатежаПоЗаказуКлиента(
