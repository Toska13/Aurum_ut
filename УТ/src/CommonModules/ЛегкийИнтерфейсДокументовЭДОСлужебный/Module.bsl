...
Функция НовыеДанныеКомпонентаПанельКомандВходящегоДокумента() Экспорт
	...
	ДанныеКомпонента.Вставить("ТребуетсяОднородностьОтвета", Ложь);
	Возврат ДанныеКомпонента;
...
Процедура ЗаполнитьДанныеКомпонентаПанельПакетаПоПараметрам(ДанныеКомпонента, ДанныеДокументовПакета) Экспорт
	...
	ДанныеКомпонента.ИспользуетсяКонтрольОтраженияВУчетеЭДО = ОтражениеВУчетеЭДО.ТребуетсяКонтрольОтраженияВУчете();
...
Функция НовыеДанныеКомпонентаПанельПакета() Экспорт
	...
	ДанныеКомпонента.Вставить("ИспользуетсяКонтрольОтраженияВУчетеЭДО", Ложь);
	Возврат ДанныеКомпонента;
...
Процедура ЗаполнитьДанныеКомпонентаПанельСостояния(ДанныеКомпонента, СвойстваДокумента) Экспорт
	...
	Если СвойстваДокумента.ЭтоВходящийЭДО Тогда
		ЗаполнитьДанныеПоПакетуВходящегоДокументаВДанныхКомпонента(ДанныеКомпонента, СвойстваДокумента);
	КонецЕсли;
	...
	Запрос = Новый Запрос;
	Запрос.Текст = СтрСоединить(ТекстыЗапроса, ОбщегоНазначения.РазделительПакетаЗапросов());
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ЭлектронныйДокумент", "&ЭлектронныеДокументы");
	
	УстановитьПривилегированныйРежим(Истина);

	ЭлектронныеДокументы = Новый Массив;
	Если ДанныеКомпонента.ТребуетсяОднородностьОтвета Тогда
		ИдентификаторПакета = ПакетыДокументовЭДОПовтИспНаВызов.ИдентификаторПакетаДокумента(СвойстваДокумента.Ссылка);
		ЭлектронныеДокументы = ПакетыДокументовЭДОПовтИспНаВызов.ДокументыПакета(Строка(ИдентификаторПакета));
		ВидИРегламентПоДокументу = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ЭлектронныеДокументы, 
			"ТипРегламента, ВидДокумента");
	Иначе
		ЭлектронныеДокументы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СвойстваДокумента.Ссылка);
		ВидИРегламентПоДокументу = Новый Соответствие();
		ВидИРегламентПоДокументу.Вставить(СвойстваДокумента.Ссылка, 
			Новый Структура("ТипРегламента, ВидДокумента", 
				СвойстваДокумента.ТипРегламента, СвойстваДокумента.ВидДокумента));
	КонецЕсли;
	...
	СостоянияДокументовСКомментарием = РезультатыЗапросов[0].Выгрузить();
	Если Не РезультатыЗапросов[0].Пустой() Тогда
		СводноеСостояние = 
			ПакетыДокументовЭДО.СостояниеПакета(СостоянияДокументовСКомментарием.ВыгрузитьКолонку("Состояние"));
		СтрокаСостояния = СостоянияДокументовСКомментарием.Найти(СводноеСостояние, "Состояние");
		ЗаполнитьДанныеКомпонентаПанельСостоянияПоСостояниюЭДО(ДанныеКомпонента, СводноеСостояние, 
			СтрокаСостояния.Комментарий);
	КонецЕсли;
	
	ВыборкаДанныхПоОбъектамУчета = РезультатыЗапросов[1].Выбрать();
	ДокументыОтраженныеВУчете = Новый Соответствие();
	Пока ВыборкаДанныхПоОбъектамУчета.Следующий() Цикл
		ЗаполнитьДанныеКомпонентаПанельСостоянияДляПринятогоКУчетуДокумента(ДанныеКомпонента);
		ДокументыОтраженныеВУчете.Вставить(ВыборкаДанныхПоОбъектамУчета.ЭлектронныйДокумент, Истина);
	КонецЦикла;

	ВыборкаДанныхПоОтражениюВУчете = РезультатыЗапросов[2].Выбрать();
	Пока ВыборкаДанныхПоОтражениюВУчете.Следующий() Цикл
		ДанныеКонтроля = НовыеДанныеКонтроляОтраженияВУчете();
	...
	ВидыДокументовПоТипам = ЭлектронныеДокументыЭДО.ТипыДокументовПоВиду();
	Для Каждого ЭлектронныйДокумент Из ЭлектронныеДокументы Цикл
		РегламентИВидДокумента = ВидИРегламентПоДокументу.Получить(ЭлектронныйДокумент);
		Если Не ДокументыОтраженныеВУчете.Получить(ЭлектронныйДокумент) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Если Не СвойстваДокумента.ЭтоВходящийЭДО Тогда
			Продолжить;
		КонецЕсли;
		СтрокаСостояния = СостоянияДокументовСКомментарием.Найти(ЭлектронныйДокумент, "ЭлектронныйДокумент");
		СостояниеДокумента = ?(СтрокаСостояния = Неопределено, 
			Перечисления.СостоянияДокументовЭДО.ПустаяСсылка(), СтрокаСостояния.Состояние);
		
		ДанныеКомпонента.КонтрольОтраженияВУчете.ТребуетЗаполнениеОтветногоТитула =
			РегламентыЭДО.ТребуетсяЗаполненияОтветногоТитула(СостояниеДокумента, 
				РегламентИВидДокумента.ТипРегламента, ВидыДокументовПоТипам[РегламентИВидДокумента.ВидДокумента]);
	КонецЦикла;
...
// * ТребуетЗаполнениеОтветногоТитула - Булево
// * КоличествоОснований - Число
Функция НовыеДанныеКонтроляОтраженияВУчете() Экспорт
	...
	ДанныеПринятияКУчету.Вставить("ТребуетЗаполнениеОтветногоТитула", Ложь);
	ДанныеПринятияКУчету.Вставить("КоличествоОснований", 0);
...
Процедура ЗаполнитьДанныеКомпонентаПанельЗакладокВходящегоДокумента(ДанныеКомпонента, ОписанияПодписей,
	...
	ДанныеКомпонента.ВыводитьЧатУниверсальныхСообщений = ИнтерфейсДокументовЭДО.ВыводитьЧатУниверсальныхСообщений();
...
Функция НовыеДанныеКомпонентаПанельЗакладокВходящегоДокумента() Экспорт
	...
	ДанныеКомпонента.Вставить("ВыводитьЧатУниверсальныхСообщений", Ложь);
	Возврат ДанныеКомпонента;
...
Процедура ЗаполнитьДанныеКомпонентаПанельСостоянияДляПринятогоКУчетуДокумента(ДанныеКомпонента)
	...
	ДанныеКомпонента.КонтрольОтраженияВУчете.КоличествоОснований = 
		ДанныеКомпонента.КонтрольОтраженияВУчете.КоличествоОснований + 1;
...
Процедура ЗаполнитьДанныеПоПакетуВходящегоДокументаВДанныхКомпонента(ДанныеКомпонента, СвойстваДокумента)
	...
	Если ЗначениеЗаполнено(ИдентификаторПакета) Тогда
		ДанныеКомпонента.ТребуетсяОднородностьОтвета = 
			ПакетыДокументовЭДОПовтИспНаВызов.ЭтоЗакрытыйПакет(Строка(ИдентификаторПакета))
			ИЛИ СинхронизацияЭДО.ТребуетсяОднородностьОтвета(СвойстваДокумента.ИдентификаторКонтрагента);
	КонецЕсли;
...
Процедура ЗаполнитьДанныеПоДоступнымДействиямПоЭДОВходящегоДокументаВДанныхКомпонента(ДанныеКомпонента,
	...
	Если ДанныеКомпонента.ТребуетсяОднородностьОтвета Тогда
		ИдентификаторПакета = ПакетыДокументовЭДОПовтИспНаВызов.ИдентификаторПакетаДокумента(СвойстваДокумента.Ссылка);
		ВсеДокументыПакета = ПакетыДокументовЭДОПовтИспНаВызов.ДокументыПакета(Строка(ИдентификаторПакета));
		СписокРеквизитов = ОбщегоНазначенияКлиентСервер.КлючиСтруктурыВСтроку(СвойстваДокументаДляОпределенияДействий);
		СвойстваДокументовПакета = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ВсеДокументыПакета, СписокРеквизитов);
	Иначе
		ЗаполнитьЗначенияСвойств(СвойстваДокументаДляОпределенияДействий, СвойстваДокумента);
		СвойстваДокументовПакета = Новый Соответствие();
		СвойстваДокументовПакета.Вставить(СвойстваДокумента.Ссылка, СвойстваДокументаДляОпределенияДействий);
	КонецЕсли;
	
	Для Каждого КлючИЗначение Из СвойстваДокументовПакета Цикл
		
		СостояниеЭДОДокумента = ЭлектронныеДокументыЭДОПовтИспНаВызов.СостояниеДокумента(КлючИЗначение.Ключ);
		ДоступныеДействияДокумента = ЭлектронныеДокументыЭДО.ДействияПоСостояниюДокумента(СостояниеЭДОДокумента, 
			КлючИЗначение.Значение);
		ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(ДанныеКомпонента.ДоступныеДействияПоЭДОДляДокумента, 
			ДоступныеДействияДокумента, Истина);
...
Процедура ДобавитьОписаниеУстановленнойПодписи(ОписанияПодписей, УстановленнаяПодпись)
	...
	Если ЗначениеЗаполнено(УстановленнаяПодпись.РезультатПроверкиПоМЧД) Тогда
		ОписаниеПодписи.ПодписьПоДоверенностиВерна = 
			Не КриптографияБЭД.ПриПроверкеПодписиВозниклиОшибки(УстановленнаяПодпись);
	КонецЕсли;
