...
Функция ЭтоСервисИнтеграции(Знач ОбъектМетаданных) Экспорт
	...
	Возврат ОбщегоНазначенияБТСПовтИсп.МодельКонфигурации().ВсеСервисыИнтеграции.Получить(ОбъектМетаданных) = Истина;
...
// Преобразовывает дату из исходного часового пояса в часовой пояс сеанса
// Параметры:
//  Дата - Дата - дата в исходном часовом поясе.
//  ИсходныйЧасовойПояс - Строка - часовой пояс даты.
Функция ДатаВЧасовомПоясеСеанса(Дата, ИсходныйЧасовойПояс) Экспорт
	Возврат МестноеВремя(УниверсальноеВремя(Дата, ИсходныйЧасовойПояс), ЧасовойПоясСеанса());
КонецФункции
...
// Устарел. Следует использовать ОбщегоНазначенияБТСКлиентСервер.ЗаголовокHTTP
...
// Преобразует объект XDTO в структуру с учетом вложенности свойств.
// Списки XDTO преобразуются в массив структур в составе структуры.
//
// Параметры:
...
//
Функция СтруктураИзОбъектаXDTO(ОбъектXDTO) Экспорт
	
	Результат = Новый Структура;
	Для Каждого Свойство Из ОбъектXDTO.Свойства() Цикл
		Результат.Вставить(Свойство.Имя, ЗначениеСвойстваXDTO(ОбъектXDTO[Свойство.Имя]));
	КонецЦикла;
...
Функция ЗначениеСвойстваXDTO(СвойствоXDTO)
	
	Если ТипЗнч(СвойствоXDTO) = Тип("ОбъектXDTO") Тогда
		Результат = Новый Структура;
		Для Каждого Свойство Из СвойствоXDTO.Свойства() Цикл
			Результат.Вставить(Свойство.Имя, ЗначениеСвойстваXDTO(СвойствоXDTO[Свойство.Имя]));
		КонецЦикла; 
	ИначеЕсли ТипЗнч(СвойствоXDTO) = Тип("СписокXDTO") Тогда
		Результат = Новый Массив;
		Для Каждого Элемент Из СвойствоXDTO Цикл
			Результат.Добавить(ЗначениеСвойстваXDTO(Элемент));
		КонецЦикла; 
	Иначе
		Результат = СвойствоXDTO;
	КонецЕсли;
