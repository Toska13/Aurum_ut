...
Процедура РаспределениеДопРасходовМеждуПартиямиИТоварами(ПараметрыРасчета) Экспорт
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ИнициализироватьТрансляциюПартий(
		...
		"Регистратор", // ПоляИндексирования
...
Функция ТекстПриемникиПоТоварамПрошлыхПериодовОтклоненийВСтоимости()
	...
	ТекстЗапроса = "
		...
		|	ПО Источник.Организация = ВТУчетныеПолитикиФинУчета.Организация
		...
		// кроме метода оценки ФИФО, т.к. товары доступны для выбытия только после ввода таможенной декларации.
		// При ФИФО поддерживается ввод документа "Сторно" по таможенной декларации прошлого периода и ввод новой
		// таможенной декларации, т.к. по партии можно распределить стоимость на выбытия в прошлых периодах.
		...
		|			 И ВТУчетныеПолитикиФинУчета.МетодОценкиСтоимостиТоваров <> ЗНАЧЕНИЕ(Перечисление.МетодыОценкиСтоимостиТоваров.ФИФОСкользящаяОценка)
		|				ТОГДА NULL
		...
		|		)
		// Отклонения прошлых периодов распределяются в общем модуле РасчетСебестоимостиРешениеСЛУ 
		// в этапе ПодготовкаДанныхДляРешенияСЛУ_РасчетСебестоимостиПрошлыхПериодов.
		|	И НЕ (Источник.Организация В (&ОрганизацииСРаспределениемОтклоненийВПрошлыхПериодах)
		|		И Источник.ПериодОтклонения < &НачалоПериода) 
		|
		|СГРУППИРОВАТЬ ПО
		|	Источник.Регистратор,
		|	Источник.Период,
...
Функция ТекстДвижениеРасходОтклоненийВСтоимостиСебестоимостьТоваров()
	Возврат 
		...
		|		И ДД.АналитикаУчетаПартий = ДД.КорАналитикаУчетаПартий
...
Функция ТекстДвижениеПриходОтклоненийВСтоимостиПрочиеРасходы() Экспорт
	...
	Возврат "
		...
		|		ДД.АналитикаУчетаНоменклатуры,
		|		ДД.ИдентификаторФинЗаписи,
		|		ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ВключениеНДСВСтоимость),
		|
		|		0 КАК Сумма,
		|		0 КАК СуммаБезНДС,
		|		0 КАК СуммаРегл,
...
Функция ТекстОписаниеДанныхДляРаспределенияДополнительныхРасходов() Экспорт
	...
	ТекстЗапроса = 
		...
		|	Себестоимость.Период КАК ПериодКорректировки,
		...
		|	Себестоимость.ВидДеятельностиНДС КАК ВидДеятельностиНДСПартии,
...
Функция ТекстЗапросаДляРаспределенияДополнительныхРасходов(ПараметрыРасчета)
	...
	ТекстыПодготовкиДанных.Добавить(ТекстАналитикиБезСтоимости());
	ТекстыПодготовкиДанных.Добавить(ТекстРассчитаннаяСтоимостьПартийТоваров());
	ТекстыПодготовкиДанных.Добавить(ТекстПервичныеПриемникиСоСтоимостью());
	...
	ТекстыПодготовкиДанных.Добавить(ПоказателиБазыРаспределенияПриемниковПрошлыхПериодов());	
	ТекстыПодготовкиДанных.Добавить(РасчетСебестоимостиПрикладныеАлгоритмы.ПодставитьШаблоныВесаИОбъемаВТекстЗапроса(
		ТекстАналитикиУчетаНоменклатурыСОбъемомИВесами(), "СН"));
	
	ТекстыВыборкиДанных = Новый Массив;
	ТекстыВыборкиДанных.Добавить(ТекстОписаниеДанныхДляРаспределенияДополнительныхРасходов());
	ТекстыВыборкиДанных.Добавить(ТекстДолиДополнительныхРасходов());
...
Функция ТекстАналитикиУчетаНоменклатурыСОбъемомИВесами()
	Возврат "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Показатели.КорАналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры
	|ПОМЕСТИТЬ ИспользуемыеАУН
	|ИЗ
	...
	|ИЗ
	// Комментарий 4761. Условия выборки и соединения должны совпадать с запросом ТекстДолиДополнительныхРасходов - ТекстДолиДополнительныхРасходов_3.
	|	Регистраторы КАК ДД
	...
	|
	|ВЫБРАТЬ
	|	ПриемникиПоТоварам.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры
	|ИЗ
	// Комментарий 4774. Условия выборки и соединения должны совпадать с запросом ТекстДолиДополнительныхРасходов - ТекстДолиДополнительныхРасходов_1.
	// Одно исключение в текущем запросе не нужны левые соединения.
	// Выбираем только те аналитики учета номенклатуры, которые должны быть обработаны.
	|	Регистраторы КАК Регистраторы
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПриемникиПоТоварам КАК ПриемникиПоТоварам
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ИспользуемыеАУН.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
	|	СН.Ссылка КАК Номенклатура,
	|	СН.ТипНоменклатуры КАК ТипНоменклатуры,
	|	&Вес КАК Вес,
	|	&Объем КАК Объем
	|ПОМЕСТИТЬ АналитикиУчетаНоменклатурыСОбъемомИВесами
	|ИЗ
	|	ИспользуемыеАУН КАК ИспользуемыеАУН
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СН
	|		ПО (СН.Ссылка = ИспользуемыеАУН.АналитикаУчетаНоменклатуры.Номенклатура)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	АналитикаУчетаНоменклатуры
	|;
...
Функция ТекстАналитикиЗаказов()
	...
	ТекстЗапроса = 
		...
		|		Регистраторы.АналитикаРасходов,
		|		ВЫБОР
		|			КОГДА Заказ.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПеремещениеТоваровМеждуФилиалами)
		|				ТОГДА Заказ.ОрганизацияПолучатель
		|			ИНАЧЕ Регистраторы.Организация
...
Функция ТекстПриобретенияПоТаможеннымДекларациям()
	...
	ТекстЗапроса =
		...
		|ПОМЕСТИТЬ ВТДвиженияСебестоимостьТоваровПоДекларациям
		|ИЗ
		|	ВТОтборРегистраторыДеклараций КАК Отбор
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СебестоимостьТоваров КАК СебестоимостьТоваров
		|		ПО СебестоимостьТоваров.Регистратор = Отбор.Регистратор
		|			И СебестоимостьТоваров.Период <= &КонецПериода
		...
		|	ТоварыДекларации.ДокументПоступления КАК ПриобретениеТоваровУслуг,
		|	ТоварыДекларации.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
		|	СУММА(ТоварыДекларации.Количество) КАК Количество
		|
		|ПОМЕСТИТЬ ДекларацииБезНДСНулеваяТаможеннаяПошлина
		|ИЗ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДвиженияСебестоимостьТоваровПоДекларациям КАК Себестоимость
		|		ПО Себестоимость.Регистратор = Декларации.Ссылка
		|			И Себестоимость.Период = Декларации.Дата
		|			И Себестоимость.АналитикаУчетаНоменклатуры = ТоварыДекларации.АналитикаУчетаНоменклатуры
		|ГДЕ
		|	Себестоимость.Регистратор ЕСТЬ NULL
		|СГРУППИРОВАТЬ ПО
		|	Декларации.Ссылка,
		|	ТоварыДекларации.ДокументПоступления,
		|	ТоварыДекларации.АналитикаУчетаНоменклатуры
		|
		|ИНДЕКСИРОВАТЬ ПО
		...
		|	0 КАК КоличествоБезСтоимостей
		|
		|ПОМЕСТИТЬ ВТДвиженияСебестоимостиПоДекларациям
		|ИЗ
		|	ВТДвиженияСебестоимостьТоваровПоДекларациям КАК Т
		|ГДЕ
		...
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДД.Декларация КАК Регистратор,
		|	Т.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
		|	Т.Организация КАК Организация,
		|	Т.ВидЗапасов КАК ВидЗапасов,
		|	ДД.ПриобретениеТоваровУслуг КАК ДокументИсточник,
		...
		|	Т.Период,
		|	0 КАК ДопРасходы,
		|	ДД.Количество КАК КоличествоБезСтоимостей
		|ИЗ
		|	ДекларацииБезНДСНулеваяТаможеннаяПошлина КАК ДД
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СебестоимостьТоваров КАК Т
		|		ПО (Т.Регистратор = ДД.ПриобретениеТоваровУслуг)
		|			И (Т.АналитикаУчетаНоменклатуры = ДД.АналитикаУчетаНоменклатуры)
		...
		|	Т.Организация КАК Организация,
		|	СУММА(Т.ДопРасходы) КАК ДопРасходы,
		|	СУММА(Т.КоличествоБезСтоимостей) КАК КоличествоБезСтоимостей,
		...
		|	ВЫРАЗИТЬ(ВЫБОР
		|		КОГДА ДвиженияСвернутые.ДопРасходы = 0 И ДвиженияСвернутые.КоличествоБезСтоимостей = 0
		|			ТОГДА ДвиженияСвернутые.Количество
		|		КОГДА ДвиженияСвернутые.ДопРасходы = 0
		|			ТОГДА ДвиженияСвернутые.Количество * Т.КоличествоБезСтоимостей / ДвиженияСвернутые.КоличествоБезСтоимостей
		...
		|			И Себестоимость.ПериодМесяц = &НачалоПериода
		|	ГДЕ
		|		ДД.АналитикаРасходов = ЗНАЧЕНИЕ(Документ.ТаможеннаяДекларацияИмпорт.ПустаяСсылка)
		|
		|	) КАК ТоварыПоДекларациям
		|
		|ИНДЕКСИРОВАТЬ ПО НАБОРАМ
		|(
		|	(РегистраторСебестоимости, АналитикаУчетаНоменклатуры),
		|	(Декларация)
		|)
		|";
...
Функция ШаблонТекстаПервичныеПриемникиПоЗаказам()
	...
	Шаблон = 
		...
		// При среднескользящей распределение на выбытия в прошлых периодах выполняется по данным 
		// регистра "Отклонения в стоимости товаров". 
		// Запросы функции ПоказателиБазыРаспределенияПриемниковПрошлыхПериодов выполняться не должны.
		|	(ВЫБОР
		|		КОГДА ИсточникБазы.Организация В (&ОрганизацииСреднескользящая)
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА КОНЕЦ)                    КАК ЭтоПартияПрошлогоПериода,
		...
		|	МАКСИМУМ(ВЫБОР КОГДА ИсточникБазы.ИдентификаторСтроки <> """"
		|		ТОГДА ИсточникБазы.ИдентификаторСтроки
		|		ИНАЧЕ ИсточникБазы.ИдентификаторФинЗаписи КОНЕЦ) КАК ИдентификаторФинЗаписи,	
		...
		|	СУММА(ИсточникБазы.СтоимостьРегл) КАК СтоимостьНУ,
		|	
		|	МИНИМУМ(НАЧАЛОПЕРИОДА(ИсточникБазы.Период, МЕСЯЦ)) КАК ПериодПоступления
		...
		|		КОГДА ИсточникБазы.Организация В (&ОрганизацииСреднескользящая)
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА КОНЕЦ), // ЭтоПартияПрошлогоПериода
		...
		|		КОГДА РеестрДокумента.ДатаДокументаИБ ЕСТЬ NULL ИЛИ РеестрДокумента.ДатаДокументаИБ < &ДатаПереходаНаПартионныйУчетВерсии22
		|		ТОГДА &ДатаПереходаНаПартионныйУчетВерсии22
		|		ИНАЧЕ РеестрДокумента.ДатаДокументаИБ
		|	КОНЕЦ, МЕСЯЦ)) КАК ПериодПоступления
		|ИЗ
		|	Регистраторы КАК ДД
		|
		...
		|			И АналитикиЗаказов.Организация = ИсточникБазы.Организация
		|
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК АналитикаОтбора
		|		ПО ИсточникБазы.АналитикаУчетаНоменклатуры = АналитикаОтбора.КлючАналитики
		|
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокумента
		|		ПО РеестрДокумента.Ссылка = ВЫБОР
		...
		|				ИНАЧЕ АналитикиЗаказов.АналитикаРасходов КОНЕЦ
		|			И НЕ РеестрДокумента.ДополнительнаяЗапись
		|ГДЕ
...
Функция ШаблонТекстаПервичныеПриемникиПоДекларациям()
	...
	Шаблон = 
		"ВЫБРАТЬ
		|	""ТекстРаспределенияДополнительныхРасходов_1"" КАК ЗапросИсточник,
		|	ДД.Ссылка КАК Регистратор,
		// При среднескользящей распределение на выбытия в прошлых периодах выполняется по данным 
		// регистра "Отклонения в стоимости товаров". 
		// Запросы функции ПоказателиБазыРаспределенияПриемниковПрошлыхПериодов выполняться не должны.
		...
		|		ТОГДА ИсточникБазы.ИдентификаторСтроки
		|		ИНАЧЕ ИсточникБазы.ИдентификаторФинЗаписи КОНЕЦ) КАК ИдентификаторФинЗаписи,
		|	СУММА(Приобретения.Количество) КАК Количество,
		|	СУММА(ИсточникБазы.Стоимость 
		|		* ВЫБОР
		...
		|		КОНЕЦ
		|	) КАК Стоимость,
		|	СУММА(ИсточникБазы.СтоимостьБезНДС 
		|		* ВЫБОР
		|			КОГДА ИсточникБазы.Количество = 0
		|				ТОГДА 1
		|			ИНАЧЕ Приобретения.Количество / ИсточникБазы.Количество
		|		КОНЕЦ
		|	) КАК СтоимостьБезНДС,
		|	СУММА(ИсточникБазы.СтоимостьРегл 
		|		* ВЫБОР
		|			КОГДА ИсточникБазы.Количество = 0
		|				ТОГДА 1
		|			ИНАЧЕ Приобретения.Количество / ИсточникБазы.Количество
		|		КОНЕЦ
		|	) КАК СтоимостьРегл,
		|	СУММА(ИсточникБазы.СтоимостьУпр 
		|		* ВЫБОР
		|			КОГДА ИсточникБазы.Количество = 0
		|				ТОГДА 1
		|			ИНАЧЕ Приобретения.Количество / ИсточникБазы.Количество
		|		КОНЕЦ
		|	) КАК СтоимостьУпр,
		|	СУММА(ИсточникБазы.СтоимостьНДД 
		|		* ВЫБОР
		|			КОГДА ИсточникБазы.Количество = 0
		|				ТОГДА 1
		|			ИНАЧЕ Приобретения.Количество / ИсточникБазы.Количество
		...
		|
		|ВЫБРАТЬ
		|	""ТекстРаспределенияДополнительныхРасходов_2"" КАК ЗапросИсточник,
		|	ДД.Ссылка КАК Регистратор,
		|	ЛОЖЬ                                       КАК ЭтоПартияПрошлогоПериода,
		|	ИсточникБазы.РазделУчета                   КАК РазделУчета,
		|	ИсточникБазы.АналитикаУчетаНоменклатуры    КАК АналитикаУчетаНоменклатуры,
		...
		|				КОГДА ИсточникБазы.Партия <> НЕОПРЕДЕЛЕНО ТОГДА ИсточникБазы.Партия
		|				КОГДА ИсточникБазы.ДокументИсточник <> НЕОПРЕДЕЛЕНО ТОГДА ИсточникБазы.ДокументИсточник
		|				ИНАЧЕ Приобретения.Декларация КОНЕЦ
		|			И НЕ РеестрДокумента.ДополнительнаяЗапись
		|ГДЕ
		|	НЕ ИсточникБазы.Количество = 0
		|	И (&УсловиеСоединения)
...
Функция ШаблонТекстаПервичныеПриемникиПоДокументам()
	...
	Шаблон = 
		...
		|		КОГДА ИсточникБазы.Организация В (&ОрганизацииСреднескользящая)
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИсточникБазы.ЭтоПартияПрошлогоПериода КОНЕЦ) КАК ЭтоПартияПрошлогоПериода,
		|	(ВЫБОР
		...
		|	ИсточникБазы.АналитикаФинансовогоУчета     КАК АналитикаФинансовогоУчета,
		|	ИсточникБазы.ВидДеятельностиНДС            КАК ВидДеятельностиНДС,
		|	ИсточникБазы.ИдентификаторФинЗаписи        КАК ИдентификаторФинЗаписи,
		...
		|	ИсточникБазы.ПериодПоступления             КАК ПериодПоступления
		|ПОМЕСТИТЬ ТаблицаДанных
		|ИЗ
		|	Регистраторы КАК ДД
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ГруппыОбъектовДополнительныхРасходов КАК Группа
		...
		|		КОГДА ИсточникБазы.Организация В (&ОрганизацииСреднескользящая)
		|			ТОГДА ЛОЖЬ
		// При полном варианте распределения расходов на выбытия в прошлом периоде дата партии не учитывается.
		// Если документ - источник базы отражен в текущем периоде, то расходы относятся к текущему периоду.
		|		КОГДА ИсточникБазы.Организация В (&ОрганизацииСРаспределениемОтклоненийВПрошлыхПериодах)
		|			ТОГДА ЛОЖЬ
		...
		|	СУММА(ИсточникБазы.СтоимостьРегл)          КАК СтоимостьНУ,
		|	
		// Доп расходы с пустой аналитикой всегда относятся к текущему периоду.
		...
		|			КОГДА ИсточникБазы.Партия <> НЕОПРЕДЕЛЕНО ТОГДА ИсточникБазы.Партия
		|			КОГДА ИсточникБазы.ДокументИсточник <> НЕОПРЕДЕЛЕНО ТОГДА ИсточникБазы.ДокументИсточник
		|			КОГДА ИсточникБазы.АналитикаРасходов <> НЕОПРЕДЕЛЕНО ТОГДА ИсточникБазы.АналитикаРасходов
		|			ИНАЧЕ NULL КОНЕЦ
		...
		|				КОГДА ИсточникБазы.ДокументИсточник <> НЕОПРЕДЕЛЕНО ТОГДА ИсточникБазы.ДокументИсточник
		|				КОГДА ИсточникБазы.АналитикаРасходов <> НЕОПРЕДЕЛЕНО ТОГДА ИсточникБазы.АналитикаРасходов
		|				ИНАЧЕ ИсточникБазы.Регистратор КОНЕЦ
		|			И НЕ РеестрДокумента.ДополнительнаяЗапись
		|ГДЕ
		...
		// При среднескользящей выборка движений из раздела учета "Неотфактурованные поставки" выполняется отдельным запросом,
		// т.к. движения по фактровке поставки формируются для документа "Регистратор расчета себестоимости".
		|	И НЕ (ИсточникБазы.Организация В (&ОрганизацииСреднескользящая)
		|		И ИсточникБазы.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки))
		|	И (&УсловиеСоединения)
		|
		|СГРУППИРОВАТЬ ПО
		...
		|		КОГДА НЕ РеестрПартии.ДатаДокументаИБ ЕСТЬ NULL И РеестрПартии.ДатаДокументаИБ < &НачалоПериода
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ КОНЕЦ, // ЭтоПартияПрошлогоПериода
		|	(ВЫБОР
		|		КОГДА ИсточникБазы.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки)
		|		 И ИсточникБазы.КорРазделУчета <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПустаяСсылка)
		|			ТОГДА ИсточникБазы.КорРазделУчета
		|		ИНАЧЕ ИсточникБазы.РазделУчета КОНЕЦ),
		|	(ВЫБОР
		|		КОГДА ИсточникБазы.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки)
		|		 И ИсточникБазы.КорРазделУчета <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПустаяСсылка)
		|			ТОГДА ИсточникБазы.КорАналитикаУчетаНоменклатуры
		|		ИНАЧЕ ИсточникБазы.АналитикаУчетаНоменклатуры КОНЕЦ),
		|	(ВЫБОР
		|		КОГДА ИсточникБазы.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки)
		|		 И ИсточникБазы.КорРазделУчета <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПустаяСсылка)
		|			ТОГДА ИсточникБазы.КорВидЗапасов
		|		ИНАЧЕ ИсточникБазы.ВидЗапасов КОНЕЦ),
		|	ИсточникБазы.Организация,
		|	(ВЫБОР
		|		КОГДА ИсточникБазы.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки)
		|		 И ИсточникБазы.КорРазделУчета <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПустаяСсылка)
		|			ТОГДА ИсточникБазы.КорПартия
		|		ИНАЧЕ ИсточникБазы.Партия КОНЕЦ),
		|	(ВЫБОР
		|		КОГДА ИсточникБазы.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки)
		|		 И ИсточникБазы.КорРазделУчета <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПустаяСсылка)
		|			ТОГДА ИсточникБазы.КорАналитикаУчетаПартий
		|		ИНАЧЕ ИсточникБазы.АналитикаУчетаПартий КОНЕЦ),
		|	ИсточникБазы.АналитикаФинансовогоУчета,
		|	ИсточникБазы.ВидДеятельностиНДС
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		// База распределения по фактуровкам поставок при среднескользящей (текущий месяц).
		|ВЫБРАТЬ
		|	""ШаблонТекстаПервичныеПриемникиПоДокументам_4"" КАК ЗапросИсточник,
		|	ДД.Ссылка КАК Регистратор,
		|	ЛОЖЬ КАК ЭтоПартияПрошлогоПериода,
		|	(ВЫБОР
		|		КОГДА ИсточникБазы.КорРазделУчета <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПустаяСсылка)
		|			ТОГДА ИсточникБазы.КорРазделУчета
		|		ИНАЧЕ ИсточникБазы.РазделУчета КОНЕЦ)  КАК РазделУчета,
		|	(ВЫБОР
		|		КОГДА ИсточникБазы.КорРазделУчета <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПустаяСсылка)
		...
		|	СУММА(ИсточникБазы.СтоимостьУпр)           КАК СтоимостьУпр,
		|	СУММА(ИсточникБазы.СтоимостьНДД)           КАК СтоимостьНДД,
		|	СУММА(ИсточникБазы.СтоимостьРегл - ИсточникБазы.ПостояннаяРазница - ИсточникБазы.ВременнаяРазница) КАК СтоимостьНУ,
		|	
		// Доп расходы с пустой аналитикой всегда относятся к текущему периоду.
		|	&НачалоПериода КАК ПериодПоступления
		|ИЗ
		...
		|
		// Условие соединения с реестром документов для партий должно соответствовать условию в запросе формирования
		// временной таблицы ВТДвиженияСебестоимостиПоДокументам - ДополнительныеТаблицыДляПриемниковПоДокументам_3
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрПартии
		|	ПО РеестрПартии.Ссылка = ИсточникБазы.Партия
		|		И НЕ РеестрПартии.ДополнительнаяЗапись
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокумента
		|	ПО РеестрДокумента.Ссылка = ИсточникБазы.Партия
		|		И НЕ РеестрДокумента.ДополнительнаяЗапись
		|ГДЕ
		|	ДД.АналитикаРасходов = &ПустоеЗначение
		|	И ТИПЗНАЧЕНИЯ(ИсточникБазы.КорПартия) = ТИПЗНАЧЕНИЯ(ДД.АналитикаРасходов)
		|	И ИсточникБазы.Количество <> 0
		// Уловия выборки данных должны соответствовать запросу ДополнительныеТаблицыДляПриемниковПоДокументам_3.
		|	И ИсточникБазы.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки)
		|	И ИсточникБазы.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Дополнение)
		|	И ТИПЗНАЧЕНИЯ(ИсточникБазы.Регистратор) = ТИП(Документ.РегистраторРасчетаСебестоимости)
		|	И ИсточникБазы.Организация В (&ОрганизацииСреднескользящая)
		// Дополнительные расходы не должны распределяться на услуги.
		// Услуги могут быть в разделе учета "Неотфактурованные поставки".
		|	И АналитикаОтбора.Номенклатура.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
		// Дополнительные расходы не должны распределяться на работы в разделе "Неотфактурованные поставки",
...
Функция ШаблонТекстаПервичныеПриемникиПоСправочникам()
	...
	Шаблон = 
		...
		|	И ИсточникБазы.РазделУчета <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НезавершенноеПроизводство)
		// Исключаем распределение на забалансовые товары, кроме полуфабриката двальца, по нему может быть балансовая стоимость.
		|	И (НЕ ИсточникБазы.РазделУчета В (&ЗабалансовыеРазделыУчета)
		|		ИЛИ ЕСТЬNULL(ИсточникБазы.ВидЗапасов.ТипЗапасов, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПолуфабрикатДавальца))
		...
		|		ИЛИ ИсточникБазы.Регистратор ССЫЛКА Документ.ПоступлениеТоваровОтХранителя
		|		ИЛИ ИсточникБазы.Регистратор ССЫЛКА Документ.ПоступлениеТоваровНаСклад
		// В базу распределения включаем передачу готовой продукции на склад, т.к. выпуск продукции в подразделение или
		// цеховую кладовую не включается в базу распределения по условию на раздел учета ПроизводственныеЗатраты. 
		|		ИЛИ ИсточникБазы.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Перемещение)
		|			И ИсточникБазы.ХозяйственнаяОперация В (
		...
		|				ИЛИ ЕСТЬNULL(ВидЗапасов.ТипЗапасов, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПолуфабрикатДавальца)) 
...
Функция ШаблонТекстаПервичныеПриемникиПоПереработке()
	...
	Шаблон = 
		...
		|	ИсточникБазы.ВидДеятельностиНДС            КАК ВидДеятельностиНДС,
		|	"""" КАК ИдентификаторФинЗаписи,
		|	СУММА(ИсточникБазы.КоличествоОстаток)      КАК Количество,
		|	СУММА(ИсточникБазы.СтоимостьОстаток)       КАК Стоимость,
		|	СУММА(ИсточникБазы.СтоимостьБезНДСОстаток) КАК СтоимостьБезНДС,
		|	СУММА(ИсточникБазы.СтоимостьРеглОстаток)   КАК СтоимостьРегл,
		|	СУММА(ИсточникБазы.СтоимостьУпрОстаток)    КАК СтоимостьУпр,
		|	СУММА(ИсточникБазы.СтоимостьНДДОстаток)    КАК СтоимостьНДД,
		|	СУММА(ИсточникБазы.СтоимостьРеглОстаток)   КАК СтоимостьНУ,
		|	
		|	МИНИМУМ(НАЧАЛОПЕРИОДА(ВЫБОР
		|		КОГДА РеестрДокумента.ДатаДокументаИБ ЕСТЬ NULL ИЛИ РеестрДокумента.ДатаДокументаИБ < &ДатаПереходаНаПартионныйУчетВерсии22
		|		ТОГДА &ДатаПереходаНаПартионныйУчетВерсии22
...
Функция ДополнительныеТаблицыДляПриемниковПоДокументам()
	...
	|	(ВЫБОР
	|		КОГДА ИсточникБазы.Организация В (&ОрганизацииСреднескользящая)
	|			ТОГДА ИсточникБазы.ВидДеятельностиНДСДокумента
	|		ИНАЧЕ ИсточникБазы.ВидДеятельностиНДС КОНЕЦ) КАК ВидДеятельностиНДС,
	|	МАКСИМУМ(ВЫБОР КОГДА ИсточникБазы.ИдентификаторСтроки <> """"
	|		ТОГДА ИсточникБазы.ИдентификаторСтроки
	|		ИНАЧЕ ИсточникБазы.ИдентификаторФинЗаписи КОНЕЦ) КАК ИдентификаторФинЗаписи,
	|	СУММА(ИсточникБазы.Количество) КАК Количество,
	|	СУММА(ИсточникБазы.Стоимость + ИсточникБазы.ДопРасходы) КАК Стоимость,
	|	СУММА(ИсточникБазы.СтоимостьБезНДС + ИсточникБазы.ДопРасходыБезНДС) КАК СтоимостьБезНДС,
	|	СУММА(ИсточникБазы.СтоимостьРегл + ИсточникБазы.ДопРасходыРегл) КАК СтоимостьРегл,
	|	СУММА(ИсточникБазы.СтоимостьУпр + ИсточникБазы.ДопРасходыУпр) КАК СтоимостьУпр,
	|	СУММА(ИсточникБазы.СтоимостьНДД) КАК СтоимостьНДД,
	|	СУММА(ИсточникБазы.СтоимостьРегл + ИсточникБазы.ДопРасходыРегл) КАК СтоимостьНУ,
	|	МИНИМУМ(НАЧАЛОПЕРИОДА(ИсточникБазы.Период, МЕСЯЦ)) КАК ПериодПоступления
	|
	...
	|			ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ПеремещениеОбособленно)))
	// Выборка из раздела учета "Неотфактурованные поставки" используется для получения базы распределения
	// по документам "Приобретение товаров и услуг (фактуровка поставки)", кроме организаций на среднескользящей.
	// База распределения по фактуровкам поставок при среднескользящей формируется в запросе ВТДвиженияСебестоимостиПоДокументам_4.
	|	И НЕ (ИсточникБазы.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки)
	|		И ИсточникБазы.Организация В (&ОрганизацииСреднескользящая))
	|	И НЕ ИсточникБазы.РазделУчета В (
	|		ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НезавершенноеПроизводство))
	|	И (НЕ ИсточникБазы.РазделУчета В (&ЗабалансовыеРазделыУчета)
	...
	|		КОГДА ИсточникБазы.Организация В (&ОрганизацииСреднескользящая)
	|			ТОГДА ИсточникБазы.ВидДеятельностиНДСДокумента
	|		ИНАЧЕ ИсточникБазы.ВидДеятельностиНДС КОНЕЦ) // ВидДеятельностиНДС
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|			ИНАЧЕ NULL КОНЕЦ
	|		И НЕ РеестрПартии.ДополнительнаяЗапись
	// Условие соединения с реестром документов для партий должно соответствовать условию в функции ТекстПриемникиПоТоварамПрошлыхПериодовДополнительныхРасходов
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокумента
	|		ПО РеестрДокумента.Ссылка = ВЫБОР
	|				КОГДА ИсточникБазы.Партия <> НЕОПРЕДЕЛЕНО ТОГДА ИсточникБазы.Партия
	|				КОГДА ИсточникБазы.ДокументИсточник <> НЕОПРЕДЕЛЕНО ТОГДА ИсточникБазы.ДокументИсточник
	|				КОГДА ИсточникБазы.АналитикаРасходов <> НЕОПРЕДЕЛЕНО ТОГДА ИсточникБазы.АналитикаРасходов
	|				ИНАЧЕ Отбор.Ссылка КОНЕЦ
	|			И НЕ РеестрДокумента.ДополнительнаяЗапись
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтСписанияНаРасходы КАК СписанияНаРасходы
	|		ПО СписанияНаРасходы.Период = ИсточникБазы.Период
	...
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ, // ЭтоПартияПрошлогоПериода
	|	ВЫБОР
	|		КОГДА ИсточникБазы.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки)
	|			И ИсточникБазы.КорРазделУчета <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПустаяСсылка)
	|		ТОГДА ИСТИНА ИНАЧЕ ЛОЖЬ
	...
	// База распределения по фактуровкам поставок при среднескользящей (текущий месяц).
	|ВЫБРАТЬ
	|	""ВТДвиженияСебестоимостиПоДокументам_3"" КАК ЗапросИсточник,
	|	ИсточникБазы.Регистратор,
	|	Отбор.ИсходныйДокумент КАК ИсходныйДокумент,
	...
	|	ИсточникБазы.КорАналитикаУчетаПартий,
	|	ИсточникБазы.АналитикаФинансовогоУчета,
	|	ИсточникБазы.ВидДеятельностиНДСДокумента КАК ВидДеятельностиНДС,
	|	МАКСИМУМ(ВЫБОР КОГДА ИсточникБазы.ИдентификаторСтроки <> """"
	|		ТОГДА ИсточникБазы.ИдентификаторСтроки
	|		ИНАЧЕ ИсточникБазы.ИдентификаторФинЗаписи КОНЕЦ) КАК ИдентификаторФинЗаписи,
	|	СУММА(ИсточникБазы.Количество) КАК Количество,
	...
	|	СУММА(ИсточникБазы.СтоимостьУпр + ИсточникБазы.ДопРасходыУпр) КАК СтоимостьУпр,
	|	СУММА(ИсточникБазы.СтоимостьНДД) КАК СтоимостьНДД,
	|	СУММА(ИсточникБазы.СтоимостьРегл + ИсточникБазы.ДопРасходыРегл - ИсточникБазы.ПостояннаяРазница - ИсточникБазы.ВременнаяРазница) КАК СтоимостьНУ,
	|	
	|	МИНИМУМ(НАЧАЛОПЕРИОДА(ВЫБОР
	|		КОГДА РеестрДокумента.ДатаДокументаИБ ЕСТЬ NULL ИЛИ РеестрДокумента.ДатаДокументаИБ < &ДатаПереходаНаПартионныйУчетВерсии22
	|		ТОГДА &ДатаПереходаНаПартионныйУчетВерсии22
	...
	|	ВТКэшРасчетныеОборотыСебестоимостьТоваров КАК ИсточникБазы
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСвязиДокументовАналитикиРасходов КАК Отбор
	|		ПО ИсточникБазы.КорПартия = Отбор.Ссылка
	// Условие соединения с реестром документов для партий должно соответствовать условию в функции ШаблонТекстаПервичныеПриемникиПоДокументам_4
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрПартии
	|		ПО РеестрПартии.Ссылка = ИсточникБазы.Партия
	|		И НЕ РеестрПартии.ДополнительнаяЗапись
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокумента
	...
	|	ИсточникБазы.СлужебноеВидДвиженияПриход
	|	И ИсточникБазы.Количество <> 0
	// Уловия выборки данных должны соответствовать запросу ШаблонТекстаПервичныеПриемникиПоДокументам_4.
	|	И ИсточникБазы.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки)
	|	И ИсточникБазы.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Дополнение)
	|	И ТИПЗНАЧЕНИЯ(ИсточникБазы.Регистратор) = ТИП(Документ.РегистраторРасчетаСебестоимости)
	|	И ИсточникБазы.Организация В (&ОрганизацииСреднескользящая)
	...
	|	ИсточникБазы.КорАналитикаУчетаПартий,
	|	ИсточникБазы.АналитикаФинансовогоУчета,
	|	ИсточникБазы.ВидДеятельностиНДСДокумента
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// База распределения по фактуровкам поставок при среднескользящей (прошлый месяц).
	|ВЫБРАТЬ
	|	""ВТДвиженияСебестоимостиПоДокументам_4"" КАК ЗапросИсточник,
	|	ИсточникБазы.Регистратор,
	|	Отбор.ИсходныйДокумент КАК ИсходныйДокумент,
	|	ИСТИНА КАК ЭтоПартияПрошлогоПериода,
	|	ВЫБОР КОГДА ИсточникБазы.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки)
	...
	|			И НЕ РеестрДокумента.ДополнительнаяЗапись
	|ГДЕ
	|	ИсточникБазы.Организация В (&ОрганизацииСреднескользящая)
	|	И ИсточникБазы.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	И ИсточникБазы.Период < &НачалоПериода
	|	И ИсточникБазы.Количество <> 0
	|	И ИсточникБазы.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Дополнение)
...
Функция ДополнительныеТаблицыДляПриемниковПоПрочимРасходам()
	...
	|	ИсточникБазы.АналитикаФинансовогоУчета     КАК АналитикаФинансовогоУчета,
	|	ИсточникБазы.ВидДеятельностиНДС            КАК ВидДеятельностиНДС,
	|	МАКСИМУМ(ИсточникБазы.ИдентификаторФинЗаписи) КАК ИдентификаторФинЗаписи,
	|	СУММА(ИсточникБазы.Количество)             КАК Количество,
	|	СУММА(ИсточникБазы.Стоимость)              КАК Стоимость,
	|	СУММА(ИсточникБазы.СтоимостьБезНДС)        КАК СтоимостьБезНДС,
	|	СУММА(ИсточникБазы.СтоимостьРегл)          КАК СтоимостьРегл,
	...
	|	ИсточникБазы.АналитикаФинансовогоУчета     КАК АналитикаФинансовогоУчета,
	|	ИсточникБазы.ВидДеятельностиНДС            КАК ВидДеятельностиНДС,
	|	(ВЫБОР КОГДА ИсточникБазы.ИдентификаторСтроки <> """"
	|		ТОГДА ИсточникБазы.ИдентификаторСтроки
	|		ИНАЧЕ ИсточникБазы.ИдентификаторФинЗаписи КОНЕЦ) КАК ИдентификаторФинЗаписи,
	|	ИсточникБазы.Количество                    КАК Количество,
	|	ИсточникБазы.Стоимость                     КАК Стоимость,
...
Функция ТекстПервичныеПриемникиДополнительныхРасходов(ПараметрыРасчета)
	...
	ТекстПервичныеПриемники = 
		...
		|	
		|	МИНИМУМ(Т.ПериодПоступления) КАК ПериодПоступления
...
Функция ТекстАналитикиБезСтоимости()
	...
	|	Источник.ВидДеятельностиНДС
	|
	|ПОМЕСТИТЬ АналитикиБезСтоимости
	|ИЗ
	|	ПервичныеПриемники КАК Источник
	|ГДЕ
	|	&РасшифровкаРаспределения
	|	И НЕ Источник.ЭтоПартияПрошлогоПериода
	|	И (Источник.Стоимость = 0
	|		ИЛИ Источник.СтоимостьБезНДС = 0
	|		ИЛИ Источник.СтоимостьРегл = 0
	|		ИЛИ Источник.СтоимостьУпр = 0)
	|";
...
Функция ТекстРассчитаннаяСтоимостьПартийТоваров()
	...
	|
	|ПОМЕСТИТЬ РассчитаннаяСтоимостьПартийТоваров
	|ИЗ
	|	РегистрНакопления.СебестоимостьТоваров КАК Себестоимость
	|ГДЕ
	|	&РасшифровкаРаспределения
	|	И Себестоимость.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И Себестоимость.Активность
	|	И Себестоимость.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Партия)
	|	И (Организация, АналитикаУчетаНоменклатуры, ВидЗапасов, РазделУчета, 
	|			Партия, АналитикаУчетаПартий, АналитикаФинансовогоУчета, ВидДеятельностиНДС) В
	|		(ВЫБРАТЬ
	|			Аналитики.Организация,
	|			Аналитики.АналитикаУчетаНоменклатуры,
	|			Аналитики.ВидЗапасов,
	|			Аналитики.РазделУчета,
	|			Аналитики.Партия,
	|			Аналитики.АналитикаУчетаПартий,
	|			Аналитики.АналитикаФинансовогоУчета,
	|			Аналитики.ВидДеятельностиНДС
	|		ИЗ
	|			АналитикиБезСтоимости КАК Аналитики
	|		)
	|СГРУППИРОВАТЬ ПО
	|	Себестоимость.АналитикаУчетаНоменклатуры,
	|	Себестоимость.РазделУчета,
...
Функция ТекстПервичныеПриемникиСоСтоимостью() // вт ПервичныеПриемникиСоСтоимостью
	...
	|	Источник.АналитикаФинансовогоУчета,
	|	Источник.ВидДеятельностиНДС,
	|	Источник.ИдентификаторФинЗаписи,
	|	Источник.ПериодПоступления,
	|	Источник.Количество КАК Количество,
	|	(ВЫБОР 
	|		КОГДА Источник.Стоимость <> 0 ТОГДА Источник.Стоимость
	|		КОГДА ЕСТЬNULL(СтоимостьПартий.Стоимость, 0) <> 0
	|			ТОГДА Источник.Количество * (СтоимостьПартий.Стоимость / СтоимостьПартий.Количество)
	|		ИНАЧЕ Источник.Количество * ЕСТЬNULL(Цены.Стоимость, 0) КОНЕЦ) КАК Стоимость,
	|	(ВЫБОР 
	|		КОГДА Источник.СтоимостьБезНДС <> 0 ТОГДА Источник.СтоимостьБезНДС
	|		КОГДА ЕСТЬNULL(СтоимостьПартий.СтоимостьБезНДС, 0) <> 0
	|			ТОГДА Источник.Количество * (СтоимостьПартий.СтоимостьБезНДС / СтоимостьПартий.Количество)
	|		ИНАЧЕ Источник.Количество * ЕСТЬNULL(Цены.СтоимостьБезНДС, 0) КОНЕЦ) КАК СтоимостьБезНДС,
	|	(ВЫБОР 
	|		КОГДА Источник.СтоимостьРегл <> 0 ТОГДА Источник.СтоимостьРегл
	|		КОГДА ЕСТЬNULL(СтоимостьПартий.СтоимостьРегл, 0) <> 0
	|			ТОГДА Источник.Количество * (СтоимостьПартий.СтоимостьРегл / СтоимостьПартий.Количество)
	|		ИНАЧЕ Источник.Количество * ЕСТЬNULL(Цены.СтоимостьРегл, 0) КОНЕЦ) КАК СтоимостьРегл,
	|	(ВЫБОР 
	|		КОГДА Источник.СтоимостьУпр <> 0 ТОГДА Источник.СтоимостьУпр
	|		КОГДА ЕСТЬNULL(СтоимостьПартий.СтоимостьУпр, 0) <> 0
	|			ТОГДА Источник.Количество * (СтоимостьПартий.СтоимостьУпр / СтоимостьПартий.Количество)
	|		ИНАЧЕ Источник.Количество * ЕСТЬNULL(Цены.СтоимостьУпр, 0) КОНЕЦ) КАК СтоимостьУпр,
	|	(ВЫБОР 
	|		КОГДА Источник.СтоимостьНДД <> 0 ТОГДА Источник.СтоимостьНДД
	|		КОГДА ЕСТЬNULL(СтоимостьПартий.СтоимостьНДД, 0) <> 0
	|			ТОГДА Источник.Количество * (СтоимостьПартий.СтоимостьНДД / СтоимостьПартий.Количество)
	|		ИНАЧЕ Источник.Количество * ЕСТЬNULL(Цены.СтоимостьНДД, 0) КОНЕЦ) КАК СтоимостьНДД,
	|	(ВЫБОР 
	|		КОГДА Источник.СтоимостьНУ <> 0 ТОГДА Источник.СтоимостьНУ
	|		КОГДА ЕСТЬNULL(СтоимостьПартий.СтоимостьРегл, 0) <> 0
	|			ТОГДА Источник.Количество * (СтоимостьПартий.СтоимостьРегл / СтоимостьПартий.Количество)
	|		ИНАЧЕ Источник.Количество * ЕСТЬNULL(Цены.СтоимостьРегл, 0) КОНЕЦ) КАК СтоимостьНУ
	|
	|ПОМЕСТИТЬ ПервичныеПриемникиСоСтоимостью
	|ИЗ
	|	ПервичныеПриемники КАК Источник
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТСтоимостьПартийТоваров КАК Цены
	|	ПО Цены.Организация = Источник.Организация
	|		И Цены.Период = НАЧАЛОПЕРИОДА(&НачалоПериода, МЕСЯЦ)
	|		И Цены.АналитикаУчетаНоменклатуры = Источник.АналитикаУчетаНоменклатуры
	|		И Цены.ВидЗапасов = Источник.ВидЗапасов
	|		И Цены.РазделУчета = Источник.РазделУчета
	|		И Цены.Партия = Источник.Партия
	|		И Цены.АналитикаУчетаПартий = Источник.АналитикаУчетаПартий
	|		И Цены.АналитикаФинансовогоУчета = Источник.АналитикаФинансовогоУчета
	|		И Цены.ВидДеятельностиНДС = Источник.ВидДеятельностиНДС
	|
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РассчитаннаяСтоимостьПартийТоваров КАК СтоимостьПартий
	|	ПО СтоимостьПартий.Организация = Источник.Организация
	|		И СтоимостьПартий.АналитикаУчетаНоменклатуры = Источник.АналитикаУчетаНоменклатуры
	|		И СтоимостьПартий.ВидЗапасов = Источник.ВидЗапасов
	|		И СтоимостьПартий.РазделУчета = Источник.РазделУчета
	|		И СтоимостьПартий.Партия = Источник.Партия
	|		И СтоимостьПартий.АналитикаУчетаПартий = Источник.АналитикаУчетаПартий
	|		И СтоимостьПартий.АналитикаФинансовогоУчета = Источник.АналитикаФинансовогоУчета
	|		И СтоимостьПартий.ВидДеятельностиНДС = Источник.ВидДеятельностиНДС
	|		И СтоимостьПартий.Количество <> 0
	|";
...
Функция ТекстКоэффициентыПриведенияБазыРаспределенияДополнительныхРасходов()
	...
	ТекстЗапроса = 
		...
		|ИЗ
		|	ПервичныеПриемникиСоСтоимостью КАК ДД
		|ГДЕ
		|	НЕ &РасшифровкаРаспределения
		|
		|СГРУППИРОВАТЬ ПО
		|	ДД.Регистратор
		|";
...
Функция ТекстПервичныеПриемникиПриведенная()
	...
	|	Источник.ПериодПоступления,
	|	Источник.Количество КАК Количество,
	|	ВЫРАЗИТЬ(Источник.Стоимость / ЕСТЬNULL(КоэффициентыПриведения.Коэффициент, 1) КАК ЧИСЛО (23,10)) КАК Стоимость,
	|	ВЫРАЗИТЬ(Источник.СтоимостьБезНДС / ЕСТЬNULL(КоэффициентыПриведения.КоэффициентБезНДС, 1) КАК ЧИСЛО (23,10)) КАК СтоимостьБезНДС,
	|	ВЫРАЗИТЬ(Источник.СтоимостьРегл / ЕСТЬNULL(КоэффициентыПриведения.КоэффициентРегл, 1) КАК ЧИСЛО (23,10)) КАК СтоимостьРегл,
	|	ВЫРАЗИТЬ(Источник.СтоимостьУпр / ЕСТЬNULL(КоэффициентыПриведения.КоэффициентУпр, 1) КАК ЧИСЛО (23,10)) КАК СтоимостьУпр,
	|	ВЫРАЗИТЬ(Источник.СтоимостьНДД / ЕСТЬNULL(КоэффициентыПриведения.КоэффициентНДД, 1) КАК ЧИСЛО (23,10)) КАК СтоимостьНДД,
	|	ВЫРАЗИТЬ(Источник.СтоимостьНУ / ЕСТЬNULL(КоэффициентыПриведения.КоэффициентНУ, 1) КАК ЧИСЛО (23,10)) КАК СтоимостьНУ
	|
	|ПОМЕСТИТЬ ПервичныеПриемникиПриведенная
	|ИЗ
	|	ПервичныеПриемникиСоСтоимостью КАК Источник
...
Функция КорректировкаПриемниковПоРегистраторам()
	...
	ТекстЗапроса =
		...
		|		Источник.ПериодПоступления КАК ПериодПоступления
		|	ИЗ
		...
		|		Данные.ИдентификаторФинЗаписи,
		|		Данные.Количество,
		// Коэффициенты по количеству нужны для уменьшения базы, если в поступлении на склад количество меньше,
		// чем в документе приобретения товаров и услуг.
		|		ВЫБОР
		|			КОГДА Источник.Количество = 0
		|				ТОГДА 0
		|			ИНАЧЕ Источник.Стоимость * (Данные.Количество / Источник.Количество)
		|		КОНЕЦ КАК Стоимость,
		|		ВЫБОР
		|			КОГДА Источник.Количество = 0
		|				ТОГДА 0
...
Функция ПоказателиБазыРаспределенияПриемниковПрошлыхПериодов()
	...
	ТекстЗапросаВТОстаткиСебестоимостиПредварительная = "
		...
		|		&ГраницаКонецПредыдущегоПериода, 
		|		Организация В (&МассивОрганизаций)
		|		И НЕ Организация В (&ОрганизацииСРаспределениемОтклоненийВПрошлыхПериодах)
		|		И НЕ РазделУчета В (&ЗабалансовыеРазделыУчета)
	...
	ТекстЗапросаВТФИФОРеализацииБезПереходаПраваСобственности = "
		...
		|	ПО РеестрСторно.СторнируемыйДокумент = Т.Регистратор
		...
		|				ИСТИНА
		|			ИЗ
		|				ВТОтборФИФОПриемникиПрошлыхПериодовПоПартии КАК ОтборПартия
		|			ГДЕ
		|				Т.Партия = ОтборПартия.Партия
		...
		|	И Т.Количество > 0
		|	И Т.Активность
		|	И НЕ Т.Организация В (&ОрганизацииСРаспределениемОтклоненийВПрошлыхПериодах)
	...
	ТекстЗапросаВТФИФООстаткиСебестоимости = "
		...
		|ОБЪЕДИНИТЬ ВСЕ
		|
		// При изменении метода оценки стоимости остатки на начала периода будут с пустой партией.
		|ВЫБРАТЬ
		|	Т.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
		|	Т.Номенклатура КАК Номенклатура,
		|	Т.Характеристика КАК Характеристика,
		...
		|	ВТОстаткиСебестоимостиПредварительная КАК Т
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТОтборФИФО КАК ОтборПартии
		|		ПО Т.Организация = ОтборПартии.Организация
		|		И Т.Номенклатура = ОтборПартии.Номенклатура
		|		И Т.Характеристика = ОтборПартии.Характеристика
		|ГДЕ
		// Параметр РаспределениеДопРасходовПоВыбывшимТоварам использовать не нужно
	...
	ТекстЗапросаВТФИФОРеализацииПрошлыхПериодов = "
		...
		|	И НЕ Т.АналитикаУчетаПоПартнерам.Организация В (&ОрганизацииСРаспределениемОтклоненийВПрошлыхПериодах)
		|	И Т.Активность
		// Условия ниже должны соответствовать условиям отбора выбытий в ВТФИФОСписанияИПрочиеВыбытия
	...
	ТекстЗапросаВТФИФОСписанияИПрочиеВыбытия = "
		...
		|		КОГДА НЕ Реестр.Подразделение ЕСТЬ NULL
		|			ТОГДА Реестр.Подразделение
		...
		|	ВЫБОР
		|		КОГДА ЛОЖЬ ТОГДА Т.КорНаправлениеДеятельности
		...
		|		И ОтборНоменклатура.Характеристика = ОтборПартии.Характеристика
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК Реестр
		|		ПО Реестр.Ссылка = Т.Регистратор
		|		И НЕ Реестр.ДополнительнаяЗапись
		|ГДЕ
		|	&РаспределениеДопРасходовПоВыбывшимТоварам
		|	И Т.Период < &НачалоПериода
	...
	ТекстЗапросаВТФИФОДетально = "
		...
		|	Т.НастройкаХозяйственнойОперации,
		|	Т.ИдентификаторФинЗаписи,
		|	ДАТАВРЕМЯ(1,1,1) КАК ПериодКорректировки
		|
		...
		|	Т.НастройкаХозяйственнойОперации,
		|	Т.ИдентификаторФинЗаписи,
		|	НАЧАЛОПЕРИОДА(Т.Период, МЕСЯЦ) КАК ПериодКорректировки
		|ИЗ
	...
	ТекстЗапросаПоСредней =
		...
		|	И НЕ Списания.Организация В (&ОрганизацииСРаспределениемОтклоненийВПрошлыхПериодах)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Партия
		|;
		...
		|		ПО Отбор.АналитикаУчетаНоменклатуры = ДД.АналитикаУчетаНоменклатуры
		|		И Отбор.Организация = ДД.Организация
		|		И Отбор.ВидЗапасов = ДД.ВидЗапасов
		|		И Отбор.МетодОценкиСтоимостиТоваров = ЗНАЧЕНИЕ(Перечисление.МетодыОценкиСтоимостиТоваров.СредняяЗаМесяц)
		|ГДЕ
		|	&РаспределениеДопРасходовПоВыбывшимТоварам
		...
		|	И НЕ ДД.Организация В (&ОрганизацииСРаспределениемОтклоненийВПрошлыхПериодах)
		|;
		|////////////////////////////////////////////////////////////////////////////////
		...
		|		ИЛИ ДД.КорРазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыВПути)
		|		ИЛИ ДД.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПереданныеПереработчику))
		...
		|	ДД.АналитикаУчетаПартий КАК АналитикаУчетаПартийИсходная,
		|	ДД.КорПартия КАК Партия,
		|	ДД.КорАналитикаУчетаПартий КАК АналитикаУчетаПартий,
		|	ДД.КорВидЗапасов КАК ВидЗапасов,
		...
		|		И ДД.АналитикаУчетаПартий = Отбор.АналитикаУчетаПартий
		...
		|	И ДД.Организация В (&ОрганизацииСредняяЗаМесяц)
		|	И ДД.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|	И ДД.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.СобственныеТоварыВПути)
		|	И НЕ ДД.Организация В (&ОрганизацииСРаспределениемОтклоненийВПрошлыхПериодах)
		|
		...
		|		ПО ДД.Партия = Отбор.Партия
		|		И ДД.АналитикаУчетаПартий = Отбор.АналитикаУчетаПартий
		|		И ДД.Организация = Отбор.Организация
		|		И ДД.АналитикаУчетаНоменклатуры.Номенклатура = Отбор.Номенклатура
		|		И ДД.АналитикаУчетаНоменклатуры.Характеристика = Отбор.Характеристика
		|		И (ДД.АналитикаУчетаНоменклатуры.Серия = Отбор.Серия
		|			ИЛИ ДД.АналитикаУчетаНоменклатуры.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка))
		...
		|	И ДД.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.СлужебноеИзменениеМетодаОценкиСтоимости)
		|	И НЕ ДД.Организация В (&ОрганизацииСРаспределениемОтклоненийВПрошлыхПериодах)
		...
		|	Перемещения.АналитикаУчетаПартий КАК АналитикаУчетаПартийПеремещения,
		|	(ВЫБОР
		...
		|		И (Перемещения.ПартияИсходная = ДД.ПартияИсходная
		|			ИЛИ Перемещения.ПартияИсходная = НЕОПРЕДЕЛЕНО)
		...
		|ГДЕ
		|	НЕ (Перемещения.ПартияИсходная = НЕОПРЕДЕЛЕНО И Перемещения.Организация <> ДД.Организация)
		|;
		|////////////////////////////////////////////////////////////////////////////////
		|
		|ВЫБРАТЬ
		...
		|	ДД.АналитикаУчетаПартийПеремещения,
		...
		|		ТОГДА Движения.КорВидДеятельностиНДС
		|		ИНАЧЕ Движения.ВидДеятельностиНДС
		|	КОНЕЦ КАК КорВидДеятельностиНДС,
		|	ВЫБОР КОГДА Движения.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.СлужебноеИзменениеМетодаОценкиСтоимости)
		...
		|		И (Движения.АналитикаУчетаПартий = ДД.АналитикаУчетаПартийПеремещения
		...
		|		И (Остатки.АналитикаУчетаПартий = ДД.АналитикаУчетаПартийПеремещения
		|			ИЛИ (ДД.ПартияПеремещения = НЕОПРЕДЕЛЕНО
		...
		|	ДД.АналитикаРасходов,
		|	ВЫБОР
		|		КОГДА ЛОЖЬ ТОГДА ДД.КорНаправлениеДеятельности
		|		ИНАЧЕ ЕСТЬNULL(ДД.КорАналитикаУчетаНоменклатуры.Назначение.НаправлениеДеятельности, ДД.КорНаправлениеДеятельности)
		|	КОНЕЦ КАК КорНаправлениеДеятельности,
		|	ДД.СтатьяАктивовПассивов,
		|	ДД.АналитикаАктивовПассивов,
		|	ДД.НастройкаСчетовУчета,
		...
		// Исключаем реализацию с отложенным переходом права собственности.
		|	И НЕ (ДД.КорРазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыВПути)
		|		И ДД.КорАналитикаУчетаНоменклатуры.ТипМестаХранения = ЗНАЧЕНИЕ(Перечисление.ТипыМестХранения.ДоговорКонтрагента))
		...
		|	ЕСТЬNULL(НАЧАЛОПЕРИОДА(Выручка.Период, МЕСЯЦ), ДАТАВРЕМЯ(1,1,1)) КАК ПериодКорректировки,
		|	ЛОЖЬ КАК СписаниеНаРасходыМалоценныхТМЦ,
		|
		...
		|	ЕСТЬNULL(НАЧАЛОПЕРИОДА(Выручка.Период, МЕСЯЦ), ДАТАВРЕМЯ(1,1,1))
		|ИМЕЮЩИЕ
		...
		|	(ВЫБОР КОГДА Источник.Организация В (&ОрганизацииСреднескользящая)
		|		ТОГДА ВозможныеАналитики.КорВидДеятельностиНДС
		|		ИНАЧЕ ВозможныеАналитики.ВидДеятельностиНДС КОНЕЦ = ВыбытиеДетально.ВидДеятельностиНДС
	...
	ТекстЗапросаПоказателиБазыРаспределенияПриемниковПрошлыхПериодов = "
		...
		|	Источник.Период,
		|	Источник.ТипЗаписи,
		// При методе оценки ФИФО период поступления заполняется началом текущего периода, т.к. при ФИФО база 
		// распределения включает направления выбытия в прошлых периодах.
		// Поле ПериодПоступления используется при формировании движений по регистру "Отклонения в стоимости товаров"
		// в процедуре СформироватьДвиженияОтклоненияВСтоимостиТоваровДополнительныеРасходыЗапросом().
		|	&НачалоПериода КАК ПериодПоступления,
		...
		// При распределении на выбытия товаров в прошлых периодах доп расходы отражаем по виду деятельности НДС выбытия
		// для корректного отражения включения/исключения НДС в стоимости.
		// Включение/исключение НДС в стоимости будет отражено движением по товарам на складах, а не движением по направлению
		// выбытия товаров для уменьшения количества движений по регистрам.
		|	(ВЫБОР
		|		КОГДА ВыбытиеДетально.КорВидДеятельностиНДС <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяССылка)
		|		 И ВыбытиеДетально.КорВидДеятельностиНДС <> НЕОПРЕДЕЛЕНО
		|		 И НЕ Источник.Организация В (&ОрганизацииСреднескользящая)
		|			ТОГДА ВыбытиеДетально.КорВидДеятельностиНДС
		|		ИНАЧЕ ВыбытиеДетально.ВидДеятельностиНДС КОНЕЦ) КАК КорВидДеятельностиНДС,
		|	ВыбытиеДетально.ВидДеятельностиНДС КАК ВидДеятельностиНДСПартии,
		...
		|	КОНЕЦ КАК ИдентификаторФинЗаписи,
		|	ВЫБОР
		|		КОГДА Источник.ДопРасходыРеглНДСКРаспределению <> 0
		|			И Источник.НастройкаХозяйственнойОперации = ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ВключениеНДСВСтоимость)
		|		ТОГДА ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.РаспределениеРасходовНаСебестоимость)
		|		ИНАЧЕ ВыбытиеДетально.НастройкаХозяйственнойОперации
		...
		|	ВыбытиеДетально.ПериодКорректировки КАК ПериодКорректировки,
		|	Источник.МетодОценкиСтоимостиТоваров КАК МетодОценкиСтоимостиТоваров,
		|	
		...
		|				ИЛИ ВыбылоСводно.Партия <> НЕОПРЕДЕЛЕНО)
		|
		...
		|			И ВыбылоСводно.Организация				= ВыбытиеДетально.Организация
		|ГДЕ
		...
		|	Источник.Период,
		|	Источник.ТипЗаписи,
		// При методе оценки "Средняя за месяц" период поступления заполняется началом текущего периода, т.к. при ФИФО база 
		// распределения включает направления выбытия в прошлых периодах.
		// Поле ПериодПоступления используется при формировании движений по регистру "Отклонения в стоимости товаров"
		// в процедуре СформироватьДвиженияОтклоненияВСтоимостиТоваровДополнительныеРасходыЗапросом().
		|	&НачалоПериода КАК ПериодПоступления,
		...
		// Вид деятельности должен быть тот же, за исключением случаев изменения вида деятельности НДС.
		|	И (ВЫБОР КОГДА Источник.Организация В (&ОрганизацииСреднескользящая)
		|		ТОГДА ВозможныеАналитики.КорВидДеятельностиНДС
		|		ИНАЧЕ ВозможныеАналитики.ВидДеятельностиНДС КОНЕЦ = ВыбытиеДетально.ВидДеятельностиНДС
		|		ИЛИ НЕ ИзменениеВидаДеятельностиНДС.АналитикаУчетаНоменклатуры ЕСТЬ NULL)
		|
		...
		|	Источник.ТипЗаписи,
		|	Источник.ПериодПоступления,
		|	Источник.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
		|	Источник.РазделУчета КАК РазделУчета,
		|	Источник.ВидЗапасов КАК ВидЗапасов,
		...
		|
		|	Источник.АналитикаУчетаНоменклатуры КАК КорАналитикаУчетаНоменклатуры,
		|	Источник.РазделУчета КАК КорРазделУчета,
		|	Источник.ВидЗапасов КАК КорВидЗапасов,
		|	Источник.Организация КАК КорОрганизация,
		|	НЕОПРЕДЕЛЕНО КАК КорПартия,
		|	НЕОПРЕДЕЛЕНО КАК КорАналитикаУчетаПартий,
		|	НЕОПРЕДЕЛЕНО КАК КорАналитикаФинансовогоУчета,
		|	ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка) КАК КорВидДеятельностиНДС,
		|	ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка) КАК КорВидДеятельностиНДСПартии,
		|	ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка) КАК ВидДеятельностиНДСВыбытия,
		|
		|	НЕОПРЕДЕЛЕНО КАК Подразделение,
		|	НЕОПРЕДЕЛЕНО КАК СтатьяРасходовСписанияРегл,
		|	НЕОПРЕДЕЛЕНО КАК СтатьяРасходовСписанияУпр,
		...
		|	НЕОПРЕДЕЛЕНО КАК КорНаправлениеДеятельности,
		|	НЕОПРЕДЕЛЕНО КАК ХозяйственнаяОперация,
		|	Источник.ИдентификаторФинЗаписи КАК ИдентификаторФинЗаписи,
		|	НЕОПРЕДЕЛЕНО КАК НастройкаХозяйственнойОперации,
		|
		|	НЕОПРЕДЕЛЕНО КАК АналитикаУчетаПоПартнерам,
		|	НЕОПРЕДЕЛЕНО КАК ЗаказКлиента,
		...
		|	Источник.МетодОценкиСтоимостиТоваров КАК МетодОценкиСтоимостиТоваров,
		|
		|	0 КАК КоличествоДетально,
		|	0 КАК КоличествоСводно,
		|	Источник.СтоимостьКРаспределению КАК СтоимостьСНДС,
		|	Источник.СтоимостьБезНДСКРаспределению КАК СтоимостьБезНДС,
		|	Источник.СтоимостьРеглКРаспределению КАК СтоимостьРегл,
		|	Источник.СтоимостьУпрКРаспределению КАК СтоимостьУпр,
		|	Источник.КоличествоКРаспределению КАК Количество,
		|	Источник.ДопРасходыСНДСКРаспределению КАК ДопРасходыСНДС,
		|	Источник.ДопРасходыБезНДСКРаспределению КАК ДопРасходыБезНДС,
		|	Источник.ДопРасходыРеглНДСКРаспределению КАК ДопРасходыРегл,
		|	Источник.ДопРасходыУпрНДСКРаспределению КАК ДопРасходыУпр,
		|	Источник.НДСРеглКРаспределению КАК НДСРегл,
		|	Источник.НДСУпрКРаспределению КАК НДСУпр,
		|	Источник.ПостояннаяРазницаКРаспределению КАК ПостояннаяРазница,
		|	Источник.ВременнаяРазницаКРаспределению КАК ВременнаяРазница,
		|	Источник.СтоимостьНДДКРаспределению КАК СтоимостьНДД
		|ИЗ
		|	ПриемникиПоТоварамПрошлыхПериодов КАК Источник
		|ГДЕ
		|	Источник.МетодОценкиСтоимостиТоваров = ЗНАЧЕНИЕ(Перечисление.МетодыОценкиСтоимостиТоваров.Среднескользящая)
		|		
		|ИНДЕКСИРОВАТЬ ПО
		|	Регистратор
		|";
...
Функция ТекстДолиДополнительныхРасходов()
	...
	ТекстЗапроса =
		...
		// Приемник СебестоимостьТоваров
		|	ПриемникиПоТоварам.ПериодПоступления КАК ПериодПоступления,
		|	ДАТАВРЕМЯ(1,1,1) КАК ПериодКорректировки,
		...
		// При использовании среднескольхящей партионные измерения должны быть пустые.
		|	(ВЫБОР
		|		КОГДА ПриемникиПоТоварам.Организация В (&ОрганизацииСреднескользящая)
		|			ТОГДА НЕОПРЕДЕЛЕНО
		|		ИНАЧЕ ПриемникиПоТоварам.Партия КОНЕЦ) КАК КорПартия,
		|	(ВЫБОР
		|		КОГДА ПриемникиПоТоварам.Организация В (&ОрганизацииСреднескользящая)
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.КлючиАналитикиУчетаПартий.ПустаяСсылка)
		|		ИНАЧЕ ПриемникиПоТоварам.АналитикаУчетаПартий КОНЕЦ) КАК КорАналитикаУчетаПартий,
		|	(ВЫБОР
		|		КОГДА ПриемникиПоТоварам.Организация В (&ОрганизацииСреднескользящая)
		|			ТОГДА НЕОПРЕДЕЛЕНО
		|		ИНАЧЕ ПриемникиПоТоварам.АналитикаФинансовогоУчета КОНЕЦ) КАК КорАналитикаФинансовогоУчета,
		|	ПриемникиПоТоварам.ВидДеятельностиНДС КАК КорВидДеятельностиНДС,
		|	НЕОПРЕДЕЛЕНО КАК ВидДеятельностиНДСПартии,
		...
		|	ПриемникиПоТоварам.ИдентификаторФинЗаписи КАК ИдентификаторФинЗаписи,
		// Приемник Прочие расходы/Прочие активы
		...
		|			ТОГДА ЕСТЬNULL(АУН_ОбъемВес.Объем, 0) * ПриемникиПоТоварам.Количество
		|		КОГДА Регистраторы.БазаРаспределенияПоПартиям = ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.ВесТоваров)
		|			ТОГДА ЕСТЬNULL(АУН_ОбъемВес.Вес, 0) * ПриемникиПоТоварам.Количество
		|		ИНАЧЕ
		|			0
		...
		|ИЗ
		// Комментарий 4774. Условия выборки и соединения должны совпадать с запросом ТекстАналитикиУчетаНоменклатурыСОбъемомИВесами
		|	Регистраторы КАК Регистраторы
		|		ЛЕВОЕ СОЕДИНЕНИЕ ПриемникиПоТоварам КАК ПриемникиПоТоварам
		|		ПО Регистраторы.Ссылка = ПриемникиПоТоварам.Регистратор
		|			
		|		ЛЕВОЕ СОЕДИНЕНИЕ АналитикиУчетаНоменклатурыСОбъемомИВесами КАК АУН_ОбъемВес
		|		ПО АУН_ОбъемВес.АналитикаУчетаНоменклатуры = ПриемникиПоТоварам.АналитикаУчетаНоменклатуры
		|ГДЕ
		|	НЕ ПриемникиПоТоварам.ЭтоПартияПрошлогоПериода
		|	ИЛИ ПриемникиПоТоварам.Организация В (&ОрганизацииСРаспределениемОтклоненийВПрошлыхПериодах)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		// Приемник СебестоимостьТоваров
		|	Показатели.ПериодПоступления КАК ПериодПоступления,
		|	Показатели.ПериодКорректировки КАК ПериодКорректировки,
		...
		|	Показатели.ВидДеятельностиНДСПартии КАК ВидДеятельностиНДСПартии,
		...
		|		КОГДА Регистраторы.БазаРаспределенияПоПартиям = ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.ОбъемТоваров)
		|			ТОГДА ЕСТЬNULL(АУН_ОбъемВес.Объем, 0) * Показатели.Количество
		|		КОГДА Регистраторы.БазаРаспределенияПоПартиям = ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.ВесТоваров)
		|			ТОГДА ЕСТЬNULL(АУН_ОбъемВес.Вес, 0) * Показатели.Количество
		|		ИНАЧЕ
		|			0
		|	КОНЕЦ КАК ДоляСтоимости,
		|	ВЫБОР
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ АналитикиУчетаНоменклатурыСОбъемомИВесами КАК АУН_ОбъемВес
		|		ПО АУН_ОбъемВес.АналитикаУчетаНоменклатуры = Показатели.КорАналитикаУчетаНоменклатуры
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	НАЧАЛОПЕРИОДА(ИсточникБазы.Период, ДЕНЬ) КАК ПериодПоступления,
		|	ДАТАВРЕМЯ(1,1,1) КАК ПериодКорректировки,
		|	НЕОПРЕДЕЛЕНО КАК КорРазделУчета,
		|	НЕОПРЕДЕЛЕНО КАК КорАналитикаУчетаНоменклатуры,
		|	НЕОПРЕДЕЛЕНО КАК КорВидЗапасов,
		...
		|			ТОГДА ЕСТЬNULL(АУН_ОбъемВес.Объем, 0) * ЕСТЬNULL(Приобретение.Количество, Передача.Количество)
		|		КОГДА ДД.БазаРаспределенияПоПартиям = ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.ВесТоваров)
		|			ТОГДА ЕСТЬNULL(АУН_ОбъемВес.Вес, 0) * ЕСТЬNULL(Приобретение.Количество, Передача.Количество)
		|		ИНАЧЕ
		|			0
		|	КОНЕЦ КАК ДоляСтоимости,
		|	ВЫБОР
		...
		|	КОНЕЦ КАК ДоляСтоимостиУпр
		|ИЗ
		// Комментарий 4761. Условия выборки и соединения должны совпадать с запросом ТекстАналитикиУчетаНоменклатурыСОбъемомИВесами.
		|	Регистраторы КАК ДД
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходы КАК ИсточникБазы
		|			ПО ДД.АналитикаРасходов = ИсточникБазы.Регистратор
		|			И ИсточникБазы.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ АналитикиУчетаНоменклатурыСОбъемомИВесами КАК АУН_ОбъемВес
		|			ПО АУН_ОбъемВес.АналитикаУчетаНоменклатуры = ИсточникБазы.АналитикаУчетаНоменклатуры
		|ГДЕ
		...
		|			И НЕ АУН_ОбъемВес.Номенклатура ЕСТЬ NULL
		|			И АУН_ОбъемВес.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
		...
		|
		|ВЫБРАТЬ
		|	""ТекстДолиДополнительныхРасходов_5"" КАК ЗапросИсточник,
		|	ДД.Ссылка КАК Регистратор,
		// Приемник Прочие расходы
		|	&НачалоПериода КАК ПериодПоступления,
		|	ДАТАВРЕМЯ(1,1,1) КАК ПериодКорректировки,
		|	НЕОПРЕДЕЛЕНО КАК КорРазделУчета,
		|	НЕОПРЕДЕЛЕНО КАК КорАналитикаУчетаНоменклатуры,
		...
		|	ИсточникБазы.ИдентификаторФинЗаписи КАК ИдентификаторФинЗаписи,
		// Приемник Прочие расходы/Прочие активы
		|	ИсточникБазы.СтатьяРасходовСписания                                 КАК КорСтатьяРасходов,
		|	ИсточникБазы.АналитикаРасходов                                      КАК КорАналитикаРасходов,
		|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ПустаяСсылка) КАК КорСтатьяАктивовПассивов,
		|	НЕОПРЕДЕЛЕНО                                                        КАК КорАналитикаАктивовПассивов,
		|	НЕОПРЕДЕЛЕНО                                                        КАК КорНастройкаСчетовУчета,
		|	ИсточникБазы.Подразделение                                          КАК КорПодразделение,
		|	ИсточникБазы.КорНаправлениеДеятельности                             КАК КорНаправлениеДеятельности,
		//Приемник Выручка и себестоимость
		|	ЗНАЧЕНИЕ(Справочник.КлючиАналитикиУчетаПоПартнерам.ПустаяСсылка) КАК АналитикаУчетаПоПартнерам,
		|	НЕОПРЕДЕЛЕНО КАК ЗаказКлиента,
		|	НЕОПРЕДЕЛЕНО КАК Менеджер,
		...
		|		КОГДА ДД.БазаРаспределенияПоПартиям = ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.ОбъемТоваров)
		|			ТОГДА ЕСТЬNULL(АУН_ОбъемВес.Объем, 0) * ИсточникБазы.Количество
		|		КОГДА ДД.БазаРаспределенияПоПартиям = ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.ВесТоваров)
		|			ТОГДА ЕСТЬNULL(АУН_ОбъемВес.Вес, 0) * ИсточникБазы.Количество
		|		ИНАЧЕ
		|			0
		|	КОНЕЦ КАК ДоляСтоимости,
		|	ВЫБОР
		...
		|			И ИсточникБазы.Период <= &КонецПериода
		|			И ИсточникБазы.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки)
		|			И НЕ ИсточникБазы.РасчетПартий
		|			И НЕ ИсточникБазы.РасчетСебестоимости
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СН
		...
		|ГДЕ
		|	ДД.СтатьяРасходов = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.НДСНалоговогоАгента)
		|	И ИсточникБазы.СтатьяРасходовСписания <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)
		|";
...
Функция ТекстЗапросаБазаРаспределенияРасходовНаПродажи()
	...
	МассивТекстовПакетаЗапросов = Новый Массив;
	
	#Область ВтОтборПродажПоПартнеру
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	АналитикаРасходов,
	|	Организация";
	МассивТекстовПакетаЗапросов.Добавить(ТекстЗапроса);
	#КонецОбласти
	
	#Область ВтОтборПродажПоЗаказу
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПрочиеРасходы.Организация КАК Организация,
	|	ПрочиеРасходы.АналитикаРасходов КАК АналитикаРасходов,
	|	ПрочиеРасходы.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ЕСТЬNULL(РеестрДокументов.ДатаДокументаИБ, &НачалоПериода) < &НачалоПериода КАК ПрошлыйПериод,
	|	РеестрДокументов.Ссылка ЕСТЬ NULL КАК ПустаяСсылка,
	|	НЕ ПрочиеРасходы.АналитикаРасходов ССЫЛКА Документ.ОтгрузкаТоваровКлиенту КАК АналитикаДокументПродажи
	|ПОМЕСТИТЬ ВтОтборПродажПоЗаказу
	|ИЗ
	|	ВТКэшРасчетныеОстаткиПрочиеРасходы КАК ПрочиеРасходы
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументов
	|	ПО РеестрДокументов.Ссылка = ПрочиеРасходы.АналитикаРасходов
	...
	|	ИЛИ ПрочиеРасходы.АналитикаРасходов ССЫЛКА Документ.ОтгрузкаТоваровКлиенту
	|	)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	ПрочиеРасходы.АналитикаРасходов КАК АналитикаРасходов,
	|	ПрочиеРасходы.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	Заказ.Дата < &НачалоПериода КАК ПрошлыйПериод,
	|	ЛОЖЬ КАК ПустаяСсылка,
	|	ЛОЖЬ КАК АналитикаДокументПродажи
	|ИЗ
	|	ВТКэшРасчетныеОстаткиПрочиеРасходы КАК ПрочиеРасходы
	...
	#Область ВтОтборПродажПоРеализациямПрошлыхПериодов
	ТекстЗапроса =
	"ВЫБРАТЬ
	...
	|	Отбор.АналитикаДокументПродажи КАК АналитикаДокументПродажи
	...
	#Область ВтНезаполненныеАналитикиРасходов
	ТекстЗапроса =
	"ВЫБРАТЬ 
	|	Отбор.Организация КАК Организация,
	|	Отбор.АналитикаРасходов КАК АналитикаРасходов,
	...
	#Область ВтБазаРаспределенияПродажИсхДанные
	
	МассивТекстовЗапроса = Новый Массив;
	
	#Область ОтборПоПартнеру
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	""ОтборПоПартнеру"" КАК ЗапросИсточник,
	|	Отбор.АналитикаРасходов КАК АналитикаРасходов,
	|	Отбор.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	Отбор.Организация КАК Организация,
	...
	|	ВыручкаИСебестоимостьПродаж.СтавкаНДС,
	|	ВыручкаИСебестоимостьПродаж.СуммаВыручки
	|		- ВыручкаИСебестоимостьПродаж.СуммаАкцизаУпр КАК ВыручкаУпр,
	|	ВыручкаИСебестоимостьПродаж.СуммаВыручкиРегл
	|		- ВыручкаИСебестоимостьПродаж.СуммаАкцизаРегл КАК ВыручкаРегл,
	...
	|		)
	// Движения по распределению расходов по базе прошлого периода не включаются в базу распределения расходов на продажу.
	|	И ТИПЗНАЧЕНИЯ(ВыручкаИСебестоимостьПродаж.Регистратор) <> ТИП(Документ.РегистраторРасчетаСебестоимости)
	|";
	МассивТекстовЗапроса.Добавить(ТекстЗапроса);
	#КонецОбласти
	
	#Область ОтборПоЗаказу
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	""ОтборПоЗаказу"" КАК ЗапросИсточник,
	|	Отбор.АналитикаРасходов КАК АналитикаРасходов,
	|	Отбор.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	Отбор.Организация КАК Организация,
	|	ВыручкаИСебестоимостьПродаж.Период КАК Период,
	...
	|ИЗ
	|	ВтОтборПродажПоЗаказу КАК Отбор
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТКэшРасчетныеОборотыВыручкаИСебестоимостьПродаж КАК ВыручкаИСебестоимостьПродаж
	...
	#Область ОтборПоРегистраторам
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	""ОтборПоРегистраторам"" КАК ЗапросИсточник,
	|	Отбор.АналитикаРасходов КАК АналитикаРасходов,
	|	Отбор.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	Отбор.Организация КАК Организация,
	|	ВыручкаИСебестоимостьПродаж.Период КАК Период,
	...
	|	ВтОтборПродажПоЗаказу КАК Отбор
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТКэшРасчетныеОборотыВыручкаИСебестоимостьПродаж КАК ВыручкаИСебестоимостьПродаж
	|			ПО ВыручкаИСебестоимостьПродаж.Регистратор = Отбор.АналитикаРасходов
	|			И ВыручкаИСебестоимостьПродаж.ЗаказКлиента <> ВыручкаИСебестоимостьПродаж.Регистратор
	|			И НЕ ВыручкаИСебестоимостьПродаж.РасчетПартий И Отбор.АналитикаДокументПродажи
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаПоПартнерам КАК КлючиАналитикиУчетаПоПартнерам
	|			ПО ВыручкаИСебестоимостьПродаж.АналитикаУчетаПоПартнерам = КлючиАналитикиУчетаПоПартнерам.Ссылка
	|			И КлючиАналитикиУчетаПоПартнерам.Организация = Отбор.Организация
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК КлючиАналитикиУчетаНоменклатуры
	...
	|		ИЛИ ВыручкаИСебестоимостьПродаж.СуммаВыручкиРегл <> 0
	|		ИЛИ ВыручкаИСебестоимостьПродаж.Стоимость + ВыручкаИСебестоимостьПродаж.ДопРасходы + ВыручкаИСебестоимостьПродаж.Трудозатраты
	|		+ ВыручкаИСебестоимостьПродаж.ПостатейныеПостоянныеСНДС + ВыручкаИСебестоимостьПродаж.ПостатейныеПеременныеСНДС <> 0
	|		ИЛИ ВыручкаИСебестоимостьПродаж.СтоимостьРегл + ВыручкаИСебестоимостьПродаж.ДопРасходыРегл + ВыручкаИСебестоимостьПродаж.ТрудозатратыРегл
	|		+ ВыручкаИСебестоимостьПродаж.ПостатейныеПостоянныеРегл + ВыручкаИСебестоимостьПродаж.ПостатейныеПеременныеРегл <> 0)
	// Движения по распределению расходов по базе прошлого периода не включаются в базу распределения расходов на продажу.
	|	И ТИПЗНАЧЕНИЯ(ВыручкаИСебестоимостьПродаж.Регистратор) <> ТИП(Документ.РегистраторРасчетаСебестоимости)
	|";
	МассивТекстовЗапроса.Добавить(ТекстЗапроса);
	...
	#Область ОтборПоЗаказамПрошлыхПериодов
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	""ОтборПоЗаказамПрошлыхПериодов"" КАК ЗапросИсточник,
	|	Отбор.АналитикаРасходов КАК АналитикаРасходов,
	|	Отбор.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	Отбор.Организация КАК Организация,
	...
	|	ВыручкаИСебестоимостьПродаж.ИсточникГФУНоменклатуры
	|ИЗ
	|	ВтОтборПродажПоРеализациямПрошлыхПериодов КАК Отбор
	...
	#Область ОтборПоРегистраторамПрошлыхПериодов
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	""ОтборПоРегистраторамПрошлыхПериодов"" КАК ЗапросИсточник,
	|	Отбор.АналитикаРасходов КАК АналитикаРасходов,
	|	Отбор.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	Отбор.Организация КАК Организация,
	|	&КонецПериода КАК Период,
	...
	#Область ОтборПоСделке
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	""ОтборПоСделке"" КАК ЗапросИсточник,
	|	Отбор.АналитикаРасходов КАК АналитикаРасходов,
	|	Отбор.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	Отбор.Организация КАК Организация,
	|	ВыручкаИСебестоимостьПродаж.Период КАК Период,
	...
	#Область ОтборПоНезаполненнымАналитикам
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	""ОтборПоНезаполненнымАналитикам"" КАК ЗапросИсточник,
	|	Отбор.АналитикаРасходов КАК АналитикаРасходов,
	|	Отбор.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	Отбор.Организация КАК Организация,
	|	ВыручкаИСебестоимостьПродаж.Период КАК Период,
	...
	#Область ВтБазаРаспределенияПродажДанные
	ТекстЗапроса =
	"ВЫБРАТЬ
	...
	#Область ВтБазаРаспределенияПродажСуммы
	ТекстЗапроса =
	"ВЫБРАТЬ
	...
	#Область ВтБазаРаспределенияНаПродажи
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ВтБазаРаспределенияПродажИсхДанные.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ВтБазаРаспределенияПродажИсхДанные.АналитикаРасходов КАК АналитикаРасходов,
	...
	|	ВтБазаРаспределенияПродажИсхДанные.СтавкаНДС,
	|	ВЫБОР
	...
	МассивТекстовПакетаЗапросов.Добавить("");
	
	ТекстЗапроса = СтрСоединить(МассивТекстовПакетаЗапросов, ОбщегоНазначения.РазделительПакетаЗапросов());
...
Функция ТекстЗапросаБазаРаспределенияРасходовНаЗаказы()
	...
	|	ДД.Склад КАК Склад,
	|	ДД.Организация КАК Организация,
	|	ДД.СерияУказывается КАК СерияУказывается
	|ПОМЕСТИТЬ ВтОтборПоЗаказуКлиентаСвернуто
	|ИЗ
	|	ВтОтборПоЗаказуКлиентаДетально КАК ДД
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	АналитикаРасходов,
	|	Номенклатура,
	...
	|	1 КАК КодИсточника,
	|	ДД.Регистратор КАК Регистратор,
	|	ДД.Распоряжение.Организация КАК Организация,
	...
	|	КОНЕЦ КАК Серия,
	|	ДД.Период КАК Период,
	|	ДД.Показатель = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Отгружено) КАК Факт,
	|	ВЫБОР
	|		КОГДА ДД.Показатель = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Отгружено)
	|			ТОГДА ДД.Количество
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК КоличествоФакт,
	|	ВЫБОР
	|		КОГДА ДД.Показатель = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Заказано)
	|			ТОГДА ДД.Количество
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК КоличествоПлан,
	|	ВЫБОР
	|		КОГДА ДД.Показатель = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Отгружено)
	|			ТОГДА &Вес * ДД.Количество
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ВесФакт,
	|	ВЫБОР
	|		КОГДА ДД.Показатель = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Заказано)
	...
	|ИЗ
	|	РегистрНакопления.РаспоряженияНаОтгрузкуИВозврат КАК ДД
	...
	|	И ДД.КодСтроки <> 0
	|	И ДД.Показатель В (ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Заказано),
	...
	|	И НЕ ТИПЗНАЧЕНИЯ(ДД.Регистратор) = ТИП(Документ.ОтгрузкаТоваровКлиенту)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Себестоимость продаж прошлого периода и корректировки реализации.
	|ВЫБРАТЬ
	|	2 КАК КодИсточника,
	|	ДД.Регистратор КАК Регистратор,
	...
	|	ИСТИНА КАК Факт,
	|	СУММА(ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДД.Регистратор) = ТИП(Документ.КорректировкаРеализации)
	|		 ИЛИ ВтОтборПоЗаказуКлиентаСвернуто.АналитикаРасходов ЕСТЬ NULL
	|		 ИЛИ ДД.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|			ТОГДА ДД.Количество
	|		ИНАЧЕ 0 КОНЕЦ) КАК КоличествоФакт,
	|	СУММА(ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДД.Регистратор) = ТИП(Документ.КорректировкаРеализации)
	|		 ИЛИ ВтОтборПоЗаказуКлиентаСвернуто.АналитикаРасходов ЕСТЬ NULL
	|			ТОГДА ДД.Количество
	|		ИНАЧЕ 0 КОНЕЦ) КАК КоличествоПлан,
	|	СУММА(ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДД.Регистратор) = ТИП(Документ.КорректировкаРеализации)
	|		 ИЛИ ВтОтборПоЗаказуКлиентаСвернуто.АналитикаРасходов ЕСТЬ NULL
	|		 ИЛИ ДД.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|			ТОГДА &Вес * ДД.Количество
	|		ИНАЧЕ 0 КОНЕЦ) КАК ВесФакт,
	|	СУММА(ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДД.Регистратор) = ТИП(Документ.КорректировкаРеализации)
	|		 ИЛИ ВтОтборПоЗаказуКлиентаСвернуто.АналитикаРасходов ЕСТЬ NULL
	|			ТОГДА &Вес * ДД.Количество
	|		ИНАЧЕ 0 КОНЕЦ) КАК ВесПлан,
	|	СУММА(ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДД.Регистратор) = ТИП(Документ.КорректировкаРеализации)
	|		 ИЛИ ВтОтборПоЗаказуКлиентаСвернуто.АналитикаРасходов ЕСТЬ NULL
	|		 ИЛИ ДД.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|			ТОГДА &Объем * ДД.Количество
	|		ИНАЧЕ 0 КОНЕЦ) КАК ОбъемФакт,
	|	СУММА(ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДД.Регистратор) = ТИП(Документ.КорректировкаРеализации)
	|		 ИЛИ ВтОтборПоЗаказуКлиентаСвернуто.АналитикаРасходов ЕСТЬ NULL
	|			ТОГДА &Объем * ДД.Количество
	|		ИНАЧЕ 0 КОНЕЦ) КАК ОбъемПлан,
	|	СУММА(ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДД.Регистратор) = ТИП(Документ.КорректировкаРеализации)
	|		 ИЛИ ВтОтборПоЗаказуКлиентаСвернуто.АналитикаРасходов ЕСТЬ NULL
	|		 ИЛИ ДД.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|			ТОГДА ДД.СуммаВыручки
	|		ИНАЧЕ 0 КОНЕЦ) КАК СуммаФакт,
	|	СУММА(ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДД.Регистратор) = ТИП(Документ.КорректировкаРеализации)
	|		 ИЛИ ВтОтборПоЗаказуКлиентаСвернуто.АналитикаРасходов ЕСТЬ NULL
	|			ТОГДА ДД.СуммаВыручки
	|		ИНАЧЕ 0 КОНЕЦ) КАК СуммаПлан,
	...
	|		
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтОтборПоЗаказуКлиентаСвернуто КАК ВтОтборПоЗаказуКлиентаСвернуто
	|	ПО ВтОтборПоЗаказуКлиентаСвернуто.АналитикаРасходов = ДД.ЗаказКлиента
	|		И ВтОтборПоЗаказуКлиентаСвернуто.Организация = ДД.АналитикаУчетаПоПартнерам.Организация
	|		И ВтОтборПоЗаказуКлиентаСвернуто.Номенклатура = КлючиАналитикиУчетаНоменклатуры.Номенклатура
	|		И ВтОтборПоЗаказуКлиентаСвернуто.Характеристика = КлючиАналитикиУчетаНоменклатуры.Характеристика
	|		И ВтОтборПоЗаказуКлиентаСвернуто.Склад = КлючиАналитикиУчетаНоменклатуры.МестоХранения
	|		И (ВтОтборПоЗаказуКлиентаСвернуто.Серия = КлючиАналитикиУчетаНоменклатуры.Серия
	|			ИЛИ НЕ ВтОтборПоЗаказуКлиентаСвернуто.СерияУказывается)
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СпрНоменклатура
	|		ПО ДД.АналитикаУчетаНоменклатуры.Номенклатура = СпрНоменклатура.Ссылка
	|ГДЕ
	|	ДД.Активность
	|	И ДД.Период < &НачалоПериода
	|
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Себестоимость продаж текущего периода и корректировки реализации.
	|ВЫБРАТЬ
	|	3 КАК КодИсточника,
	|	ДД.Регистратор КАК Регистратор,
	|	ДД.АналитикаУчетаПоПартнерам.Организация КАК Организация,
	...
	|		ПО ДД.АналитикаУчетаНоменклатуры.Номенклатура = СпрНоменклатура.Ссылка
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(ДД.Регистратор) <> ТИП(Документ.РегистраторРасчетаСебестоимости)
	|
	|СГРУППИРОВАТЬ ПО
	|	ДД.Регистратор,
	|	ДД.АналитикаУчетаПоПартнерам.Организация,
	...
	|			ТОГДА ВЫРАЗИТЬ(СУММА(ДД.КоличествоПродано) КАК ЧИСЛО(28,10)) / МАКСИМУМ(ПланФакт.КоличествоПлан)
	|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(ДД.КоличествоФакт) КАК ЧИСЛО(28,10)) / МАКСИМУМ(ПланФакт.КоличествоПлан)
	|	КОНЕЦ КАК КоличествоДоля,
	|	ВЫБОР
	|		КОГДА МАКСИМУМ(ПланФакт.ВесПлан) = 0 ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(ДД.ВесФакт) КАК ЧИСЛО(28,10)) / МАКСИМУМ(ПланФакт.ВесПлан)
	|	КОНЕЦ КАК ВесДоля,
	|	ВЫБОР КОГДА МАКСИМУМ(ПланФакт.ОбъемПлан) = 0 ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(ДД.ОбъемФакт) КАК ЧИСЛО(28,10)) / МАКСИМУМ(ПланФакт.ОбъемПлан)
	|	КОНЕЦ КАК ОбъемДоля,
	|	ВЫБОР КОГДА МАКСИМУМ(ПланФакт.СуммаПлан) = 0 ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(ДД.СуммаФакт) КАК ЧИСЛО(28,10)) / МАКСИМУМ(ПланФакт.СуммаПлан)
	|	КОНЕЦ КАК СуммаДоля,
	|	СУММА(ДД.СебестоимостьСНДС)	КАК СебестоимостьСНДС,
	|	СУММА(ДД.СебестоимостьБезНДС) КАК СебестоимостьБезНДС,
	|	СУММА(ДД.СебестоимостьРегл)	КАК СебестоимостьРегл,
	...
	|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(ДД.СебестоимостьСНДС) КАК ЧИСЛО(28,10)) / МАКСИМУМ(ПланФакт.СебестоимостьСНДСПлан)
	|	КОНЕЦ КАК СебестоимостьСНДСДоля,
	|	ВЫБОР
	|		КОГДА МАКСИМУМ(ПланФакт.СебестоимостьБезНДСПлан) = 0 ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(ДД.СебестоимостьБезНДС) КАК ЧИСЛО(28,10)) / МАКСИМУМ(ПланФакт.СебестоимостьБезНДСПлан)
	|	КОНЕЦ КАК СебестоимостьБезНДСДоля,
	|	ВЫБОР
	|		КОГДА МАКСИМУМ(ПланФакт.СебестоимостьРеглПлан) = 0 ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(ДД.СебестоимостьРегл) КАК ЧИСЛО(28,10)) / МАКСИМУМ(ПланФакт.СебестоимостьРеглПлан)
	|	КОНЕЦ КАК СебестоимостьРеглДоля,
	|	ВЫБОР
	|		КОГДА МАКСИМУМ(ПланФакт.СебестоимостьУпрПлан) = 0 ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(ДД.СебестоимостьУпр) КАК ЧИСЛО(28,10)) / МАКСИМУМ(ПланФакт.СебестоимостьУпрПлан)
	...
	|ГДЕ
	|	ДД.Факт
	// При использовании полного варианта распределения на выбытия в прошлых периодах расходы на продажу
	// распределяются только при наличии базы распределения в текущем месяце.
	// На прошлые периоды распределение выполняется в этапе ПодготовкаДанныхДляРешенияСЛУ_РасчетСебестоимостиПрошлыхПериодов.
	|	И НЕ (ПланФакт.Организация В (&ОрганизацииСРаспределениемОтклоненийВПрошлыхПериодах)
	|		И ПланФакт.Период < &НачалоПериода)
	|
	|СГРУППИРОВАТЬ ПО
	|	ДД.Регистратор,
	|	ДД.ЗаказКлиента,
...
Функция ТекстЗапросаВыборкаРаспределениеРасходовНаЗаказКлиента() 
	...
	|	База.Сторно,
	|	База.СтавкаНДС,
	|	ВЫРАЗИТЬ(Расходы.Сумма * (Расходы.КоэффПоВыручкеУпр * База.СуммаДоля + Расходы.КоэффПоКоличествуУпр * База.КоличествоДоля
	|		+ Расходы.КоэффПоВесуУпр * База.ВесДоля + Расходы.КоэффПоОбъемуУпр * База.ОбъемДоля
	|		+ Расходы.КоэффПоСебестоимостиУпр * База.СебестоимостьСНДСДоля) КАК ЧИСЛО(31,2)) КАК Сумма,
	|	ВЫРАЗИТЬ(Расходы.СуммаБезНДС * (Расходы.КоэффПоВыручкеУпр * База.СуммаДоля + Расходы.КоэффПоКоличествуУпр * База.КоличествоДоля
	|		+ Расходы.КоэффПоВесуУпр * База.ВесДоля + Расходы.КоэффПоОбъемуУпр * База.ОбъемДоля
	|		+ Расходы.КоэффПоСебестоимостиУпр * База.СебестоимостьБезНДСДоля) КАК ЧИСЛО(31,2)) КАК СуммаБезНДС,
	|	ВЫРАЗИТЬ(Расходы.СуммаУпр * (Расходы.КоэффПоВыручкеУпр * База.СуммаДоля + Расходы.КоэффПоКоличествуУпр * База.КоличествоДоля
	|		+ Расходы.КоэффПоВесуУпр * База.ВесДоля + Расходы.КоэффПоОбъемуУпр * База.ОбъемДоля
	|		+ Расходы.КоэффПоСебестоимостиУпр * База.СебестоимостьУпрДоля) КАК ЧИСЛО(31,2)) КАК СуммаУпр,
	|	ВЫРАЗИТЬ(Расходы.СуммаРегл * (Расходы.КоэффПоВыручкеРегл * База.СуммаДоля + Расходы.КоэффПоКоличествуРегл * База.КоличествоДоля
	|		+ Расходы.КоэффПоВесуРегл * База.ВесДоля + Расходы.КоэффПоОбъемуРегл * База.ОбъемДоля
	|		+ Расходы.КоэффПоСебестоимостиРегл * База.СебестоимостьРеглДоля) КАК ЧИСЛО(31,2)) КАК СуммаРегл,
	|	ВЫРАЗИТЬ(Расходы.ПостояннаяРазница * (Расходы.КоэффПоВыручкеРегл * База.СуммаДоля + Расходы.КоэффПоКоличествуРегл * База.КоличествоДоля
	|		+ Расходы.КоэффПоВесуРегл * База.ВесДоля + Расходы.КоэффПоОбъемуРегл * База.ОбъемДоля
	|		+ Расходы.КоэффПоСебестоимостиРегл * База.СебестоимостьРеглДоля) КАК ЧИСЛО(31,2)) КАК ПостояннаяРазница,
	|	ВЫРАЗИТЬ(Расходы.ВременнаяРазница * (Расходы.КоэффПоВыручкеРегл * База.СуммаДоля + Расходы.КоэффПоКоличествуРегл * База.КоличествоДоля
	|		+ Расходы.КоэффПоВесуРегл * База.ВесДоля + Расходы.КоэффПоОбъемуРегл * База.ОбъемДоля
	|		+ Расходы.КоэффПоСебестоимостиРегл * База.СебестоимостьРеглДоля) КАК ЧИСЛО(31,2)) КАК ВременнаяРазница
...
Функция ПолучитьОписаниеГруппОбъектовПоНастройкамОтбора(ТекстПолученияОбъектов, ПараметрыРасчета,
	ИмяВременнойТаблицыГруппыОбъектов, НедоступныеПоляОтбора = Неопределено, ВключенаОптимизация = Ложь) Экспорт
	...
	ОписаниеГрупп.Колонки.Добавить("УсловиеРавенства", Новый ОписаниеТипов("Булево"));
	...
	Пока Выборка.Следующий() Цикл
		...
		ОписаниеГруппы.УсловиеРавенства = ОписаниеГруппы.ТекстУсловия = "ИсточникБазы.Ссылка = &ТекущееПодразделение"
			И ОписаниеГруппы.ПоказательБазыРаспределения = "НаправлениеРаспределения"
			И ОписаниеГруппы.КоличествоПараметров = 4;

		Если Не (ВключенаОптимизация И ОписаниеГруппы.УсловиеРавенства) Тогда
...
Процедура ДополнитьТекстамиПоОписаниюГрупп(ТекстыЗапросов, ОписаниеГрупп, ИсточникШаблона, ВключенаОптимизация = Ложь) Экспорт
