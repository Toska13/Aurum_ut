...
Функция КорректируютсяЗаписиВЖурналеСФ() Экспорт
	...
	МассивВозврата.Добавить(Тип("ДокументСсылка.ОперацияСЭлектроннымБилетом"));
	
	Возврат МассивВозврата;
	
КонецФункции

// Обновляет реквизиты счетов фактур выданных.
//
// Параметры:
//  Задания - ТаблицаЗначений - колонки соответствуют измерениям, ресурсам и реквизитам регистра сведений "УниверсальнаяОчередьОперативныхЗаданий", есть и служебные колонки:
//             * Объект                 - ДокументСсылка.СчетФактураВыданный - ссылка на документ, который нужно обновить
//             * Задание                - ПеречислениеСсылка.ЗаданияУниверсальнойОчереди -
//             * ИдентификаторЗаписи    - УникальныйИдентификатор -
//             * ДополнительныеДанные   - ХранилищеЗначения -
//             * ДатаЗаписи             - Дата                    -
...
//
Процедура ОбновлениеСчетовФактурВыданных(Задания, ИдентификаторыНеОбработанныхЗаписей, 
	ДополнительныеСвойства = Неопределено) Экспорт
		
	Запрос = Новый Запрос;
	Запрос.Текст = 	ТекстЗапроса_ОбновлениеСчетовФактурВыданных();
	Запрос.УстановитьПараметр("СчетФактуры", Задания.ВыгрузитьКолонку("Объект"));	
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.КоличествоРазныхДокументовОснований = 1 Тогда
			...
			Попытка
				...
				ЭлементБлокировки.УстановитьЗначение("Ссылка", Выборка.СчетФактура);
				...
				ЗаполнитьЗначенияСвойств(ДокументОбъект, Выборка, "Партнер, Договор, Склад, НаправлениеДеятельности");
			...
			Исключение
				...
				ДополнитьОшибкиОбработки(ИдентификаторыНеОбработанныхЗаписей, Задания, 
					Выборка.СчетФактура, ТекстОшибки);
...
// Обновляет реквизиты элементов формы.
//
// Параметры:
//	Элементы - ЭлементыФормы
//	ДатаИзменения - Дата - дата на которую получаем базовый лимит.
Процедура ОписаниеЛимитовПримененияСтавокНДС(Элементы, ДатаИзменения) Экспорт
	
	СтрокаШаблон = "Для организаций и ИП на УСН с доходом до %1 млн руб.";
	...
	СтрокаШаблон = "Для организаций и ИП на УСН с доходом от %1 до %2 млн руб.";
	...
	СтрокаШаблон = "Для организаций на УСН с доходом более %1 млн руб. и выбравших применение общих ставок НДС.
				|Для организаций, утративших право на применение УСН, после превышения оборота в %2 млн руб.";
	Элементы.ПрочиеСтавкиНДС.Подсказка = СтрШаблон(СтрокаШаблон, ЛимитСтавкаБезНДС, ЛимитСтавка7);
	
	Если Элементы.ПрочиеСтавкиНДС.СписокВыбора.Количество() Тогда
		СтрокаШаблон = НСтр("ru = 'Применяются общие ставки НДС (%1), применяются все вычеты'") + Символы.ПС;
		ОсновнаяСтавкаНДС = УчетНДСУП.СтавкаНДСПоТипуНалогообложения(Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС, , ДатаИзменения);
		Ставка = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОсновнаяСтавкаНДС, "Ставка");
		Ставка = Строка(Ставка) + "%, 10%";
		Элементы.ПрочиеСтавкиНДС.СписокВыбора[0].Представление = СтрШаблон(СтрокаШаблон, 
			Ставка);
	КонецЕсли;
...
// Устанавливает список стран, отличных от России с признаком УчастникЕАЭС = Истина.
// 
// Параметры:
//  Список - СписокЗначений Из СправочникСсылка.СтраныМира - список для заполнения.
//
Процедура ЗаполнитьСписокСтранУчастниковЕАЭС(Список) Экспорт
...
// Устанавливает список типов налогообложений НДС документов отгрузки, используемых для подачи
// отчета "Статистическая форма учета перемещения товаров".
// 
// Параметры:
//  Список - СписокЗначений Из ПеречислениеСсылка.ТипыНалогообложенияНДС - список для заполнения.
//
Процедура ЗаполнитьСписокНалогообложенийНДСДляСтатформыВФТС(Список) Экспорт
	
	Список.Очистить();
	Список.Добавить(Перечисления.ТипыНалогообложенияНДС.ПродажаНаЭкспорт);
	Список.Добавить(Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДСВСтранеЕАЭС);
...
//
// Возвращаемое значение:
//	ТаблицаЗначений - список организаций не применяющих ЛьготноеНалогообложениеНДСПриУСН.
//
...
// Получение основной ставки НДС для Организации на текущий период.
// 
// Параметры:
//  Организация - СправочникСсылка.Организации - Организация. по которой нужно получить данных по Ставке НДС
//  Период - Дата.
//  СтавкаНДСПриУСН - ПеречислениеССылка.СтавкиНДС - установленная в Параметрах ставка НДС
// 
// Возвращаемое значение:
//  Число - числовое значение применяемой ставки НДС.
Функция СтавкаНДСПриУСН(Организация, Период, СтавкаНДСПриУСН = "") Экспорт
	
	Если НЕ ЗначениеЗаполнено(СтавкаНДСПриУСН) Тогда 
		ОсновнаяСтавкаНДС = УчетНДСУП.СтавкаНДСПоУмолчанию(Организация, Период);
		...
		КонецЕсли;
	Иначе
		ОсновнаяСтавкаНДС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтавкаНДСПриУСН, "Ставка");
	КонецЕсли;
	
	Возврат ОсновнаяСтавкаНДС;
...
Функция ТекстВыбораСтавкиНДСПоУмолчанию(ПараметрыЗапроса, ИсточникДаты = Неопределено, РасчетнаяСтавка = Ложь) Экспорт
	...
	Если РасчетнаяСтавка Тогда
		...
		СтавкаНДСДО2026 	= УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС20_120);
		СтавкаНДСПосле2026 	= УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС22_122);
	Иначе
		СтавкаНДСДО2019		= УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС18);
		СтавкаНДСДО2026 	= УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС20);
		СтавкаНДСПосле2026 	= УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС22_122);
	КонецЕсли;
	
	ПараметрыЗапроса.Вставить("СтавкаНДСДО2026", СтавкаНДСДО2026);
	ПараметрыЗапроса.Вставить("СтавкаНДСДО2019", СтавкаНДСДО2019);
	ПараметрыЗапроса.Вставить("СтавкаНДСПосле2026", СтавкаНДСПосле2026);
	...
	Иначе
		Возврат СтрШаблон(
		"ВЫБОР
		|	КОГДА %1 >= ДАТАВРЕМЯ(2026, 1, 1) ИЛИ (%1 = ДАТАВРЕМЯ(1, 1, 1) И ДАТАВРЕМЯ(%2) >= ДАТАВРЕМЯ(2026, 1, 1))
		|		ТОГДА &СтавкаНДСПосле2026
		|	КОГДА %1 >= ДАТАВРЕМЯ(2019, 1, 1) ИЛИ (%1 = ДАТАВРЕМЯ(1, 1, 1) И ДАТАВРЕМЯ(%2) >= ДАТАВРЕМЯ(2019, 1, 1))
		|		ТОГДА &СтавкаНДСДО2026
...
// Замена ставки НДС 20% на 22% для России с 01.01.2026
Процедура УстановкаСтавкиНДС22() Экспорт
	
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ЗаменаСтавкиНДС);
	
	УстановитьПривилегированныйРежим(Истина);
	
	ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru = 'Начат процесс замены Ставки НДС 20% на 22% с 01.01.2026г.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыПолученияДанных.ТекущаяСтавка = УчетНДСРФВызовСервера.СтавкаНДСПоЗначениюПеречисления(
								Перечисления.СтавкиНДС.НДС20, Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС);
	ПараметрыПолученияДанных.НоваяСтавка   = УчетНДСРФВызовСервера.СтавкаНДСПоЗначениюПеречисления(
								Перечисления.СтавкиНДС.НДС22, Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС);
	ПараметрыПолученияДанных.Период        = Дата(2026,1,1);
	ПараметрыПолученияДанных.Страна        = Справочники.СтраныМира.Россия;
	ПараметрыПолученияДанных.СписокСправочников = СписокСправочников;
	УчетНДСУП.ЗаменитьСтавкуНДС(ПараметрыПолученияДанных);
		
	ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), 
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru = 'Закончено выполнение регламентной операции Замена ставки НДС 20% на 22% с 01.01.2026г.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
//
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсиюОсновныеСтавкиНДС(Параметры) Экспорт
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсиюОсновныеСтавкиНДС(Параметры) Экспорт
	
	МетаданныеОбъекта = Метаданные.РегистрыСведений.ОсновныеСтавкиНДС;
	...
	Попытка
		...
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ОсновныеСтавкиНДС");
		ЭлементБлокировки.УстановитьЗначение("Страна", Справочники.СтраныМира.Россия);
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		Блокировка.Заблокировать();
		...
		НаборЗаписей.Отбор.Период.Установить(Дата(2026,1,1));
		
		НоваяЗапись = НаборЗаписей.Добавить();
		НоваяЗапись.СтавкаНДС = УчетНДСРФВызовСервера.СтавкаНДСПоЗначениюПеречисления(
			Перечисления.СтавкиНДС.НДС22, Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС);
		НоваяЗапись.Страна = Справочники.СтраныМира.Россия;
		НоваяЗапись.Период = Дата(2026,1,1);
	...
	Исключение
		...
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ОбработкаОшибок.ПредставлениеОшибкиДляПользователя(ИнформацияОбОшибке()));
...
Функция СчетаФактурыПолученныеПоДокументуЗакупки(ПараметрыРегистрации)
	...
	Если СчетФактураТребуется Тогда
		...
		Если СчетФактураТребуется Тогда
			...
			ЕстьПравоПросмотраДокументаПоОснованию = ПравоДоступа("Просмотр", Метаданные.НайтиПоПолномуИмени(ПолноеИмяДокумента));
			...
			Если Не ЕстьПравоПросмотраДокументаПоОснованию Тогда
				НавигационнаяСсылка = "";
			КонецЕсли;
		КонецЕсли;
...
Функция ТекстЗапросаРасхожденияТорговыхДокументовИСчетовФактурПолученных(ИмяВременнойТаблицы = Неопределено)
	...
	|				И НЕ СуммыДокументовВВалютахУчета.Регистратор ССЫЛКА Документ.ВозвратТоваровПоставщику 
	|					ТОГДА 1
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК СуммаУвеличение,
	|	СУММА((СуммыДокументовВВалютахУчета.СуммаБезНДС + СуммыДокументовВВалютахУчета.СуммаНДС) * ВЫБОР
	|			КОГДА СуммыДокументовВВалютахУчета.Регистратор ССЫЛКА Документ.ВозвратТоваровПоставщику
	|				ТОГДА 1
	|			КОГДА СуммыДокументовВВалютахУчета.СуммаБезНДС + СуммыДокументовВВалютахУчета.СуммаНДС > 0
	|				ТОГДА 0
	|			ИНАЧЕ -1
...
Функция СчетаФактурыВыданныеНаОсновании(ПараметрыРегистрации) Экспорт
	...
	Если ПараметрыРегистрации.ПереходПраваСобственности Тогда
		
		ПараметрыДанных = Новый Структура;
		ПараметрыДанных.Вставить("МассивОрганизаций", ОбщегоНазначенияУТКлиентСервер.Массив(ПараметрыРегистрации.Организация));
		ПараметрыДанных.Вставить("КонецПериода", КонецМесяца(ПараметрыРегистрации.Дата));
		ПараметрыДанных.Вставить("БезОграниченияПериода", Истина);
		Если ЗначениеЗаполнено(ПараметрыРегистрации.Ссылка) Тогда 
			Если ТипЗнч(ПараметрыРегистрации.Ссылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда 
				РаспоряженияНаОтгрузку = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыРегистрации.Ссылка, "Товары").Выгрузить().ВыгрузитьКолонку("ЗаказКлиента");
				ОбщегоНазначенияУТ.УдалитьПовторяющиесяЭлементыМассива(РаспоряженияНаОтгрузку);
				ПараметрыДанных.Вставить("Распоряжения", РаспоряженияНаОтгрузку);
			ИначеЕсли ТипЗнч(ПараметрыРегистрации.Ссылка) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда 
				РаспоряженияНаОтгрузку = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыРегистрации.Ссылка, "ДокументОснование.Товары", Истина).Выгрузить().ВыгрузитьКолонку("ЗаказКлиента");
				ОбщегоНазначенияУТ.УдалитьПовторяющиесяЭлементыМассива(РаспоряженияНаОтгрузку);
				ПараметрыДанных.Вставить("Распоряжения", РаспоряженияНаОтгрузку);
			КонецЕсли;
		КонецЕсли;
		
		ТекстСчетФактура = НСтр("ru = 'Выставить ИСФ/КСФ по отгрузкам'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		ЕстьСчетаФактурыКОформлению = Документы.СчетФактураВыданный.ЕстьСчетаФактурыПоОтгрузкамКОформлению(ПараметрыДанных) > 0;
		Если ЕстьСчетаФактурыКОформлению Тогда 
			НавигационнаяСсылка = "ВвестиКорректировочныйСчетФактуруВыданныйПоОтгрузке";
		Иначе
			НавигационнаяСсылка = "";
			ЦветТекста = ЦветаСтиля.НедоступныеДанныеЦвет;
		КонецЕсли;
...
Процедура АктуализироватьСчетаФактурыВыданныеПриПроведении(ПараметрыРегистрации) Экспорт
	...
	СчетаФактурыВыданные = Новый Массив;
	...
	Пока Выборка.Следующий() Цикл
		...
		СчетаФактурыВыданные.Добавить(Выборка.Ссылка);
	КонецЦикла;
	
	ЗарегистрироватьЗаданияАктуализацииСчетаФактуры(СчетаФактурыВыданные); 
...
Функция ЗаполнитьСчетФактураВыданный(СчетФактура, ПараметрыРегистрации) Экспорт
	...
	ДанныеЗаполнения.Вставить("НачислениеНДСПриОтгрузке", ПараметрыРегистрации.НачислениеНДСПриОтгрузке);
...
// см. УчетНДСУП.СчетФактураВыданныйНеТребуется
Функция СчетФактураВыданныйНеТребуется(ПараметрыРегистрации) Экспорт
	...
	ИначеЕсли ПараметрыРегистрации.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДСВСтранеЕАЭС  Тогда
		СчетФактураНеТребуется = Истина;
	ИначеЕсли ПараметрыРегистрации.ПереходПраваСобственности Тогда 
		СчетФактураНеТребуется = Истина;
	КонецЕсли;
...
Функция ТекстЗапросаСчетовФактурВыданныхПокупателямДляОтчетаКомитенту() Экспорт
	...
	|
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(ИмяТаблицыОтбор.ДокументРеализации) <> ТИП(Документ.ЗаказКлиента)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|		Документ.СчетФактураВыданный.ДокументыОснования КАК СчетФактураВыданныйПоРеализации
	|	ПО
	|		СчетФактураВыданныйПоРеализации.ДокументОснование.ЗаказКлиента = ИмяТаблицыОтбор.ДокументРеализации
	|		И СчетФактураВыданныйПоРеализации.Ссылка.Организация = ИмяТаблицыОтбор.Организация
	|		И СчетФактураВыданныйПоРеализации.Ссылка.Проведен
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(ИмяТаблицыОтбор.ДокументРеализации) = ТИП(Документ.ЗаказКлиента)
...
Процедура ДополнитьТекстЗапросаСчетФактурыПеревыставленныеВОтчетеКомитентуОЗакупках(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		...
		ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "СчетФактурыПеревыставленныеВОтчетеКомитентуОЗакупках");
...
// Процедура - Установить видимость реквизита формы Страна реализации
//
// Параметры:
//  Форма				 - ФормаКлиентскогоПриложения - Форма с реквизитом Страна реализации.
//
Процедура УстановитьВидимостьСтранаРеализации(Форма) Экспорт
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		ЗначениеЗаполнено(Форма.Объект.НалогообложениеНДС)
			И Форма.Объект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаНаЭкспорт);
...
Процедура ДобавитьЭтап_ОформлениеСчетовФактурИТаможенныхДокументов(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ОформлениеСчетовФактурИТаможенныхДокументов,
		Ложь, Истина, Ложь,
		Справочники.ОперацииЗакрытияМесяца.ОформлениеОтчетовКомитентамОСписании);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Оформление счетов-фактур и таможенных документов'"; // @НСтр-1
...
Процедура Использование_ОформлениеСчетовФактурИТаможенныхДокументов(ПараметрыОбработчика) Экспорт
	...
	Если ОрганизацииКОбработке.Количество() = 0 Или ПараметрыОбработчика.АвтоматическоеТестирование Тогда
	...
	РезультатыПоОрганизациям = Обработки.ПомощникПоУчетуНДС.ОперацииПоНДСКВыполнению(ПараметрыУчетаНДС);
	
	Для Каждого Результат Из РезультатыПоОрганизациям Цикл
		...
		Если Результат.ЕстьОбязательныеОперацииКВыполнению Тогда     
			...
			ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'По организации ""%1"" требуется оформление документов по учету НДС'", ОбщегоНазначения.КодОсновногоЯзыка()), 
			...
			Если ЭтоБазоваяВерсия Тогда
				...
				ДействиеВыполнить.ПараметрыФормы.Вставить("Организация", Результат.Организация);
		...
		Если Результат.ЕстьСчетФактураПолученныйАвансОшибкиСумм Тогда 
			...
			СтруктураБыстрогоОтбора.Вставить("Организация", Результат.Организация);
...
Процедура ПолучитьПартии(ПараметрыРасчета) Экспорт
	...
	|	Партии.Организация КАК Организация,
	|	Партии.АналитикаУчетаДокументаПоступления.Контрагент КАК Контрагент,
	...
	|	Партии.АналитикаУчетаДокументаПоступления.СтавкаНДС КАК СтавкаНДС,
	|	ВЫБОР
	...
	|		Партии.АналитикаУчетаПартий КАК АналитикаУчетаДокументаПоступления,
	...
	|			КОГДА ТИПЗНАЧЕНИЯ(Партии.Регистратор) = ТИП(Документ.ВозвратТоваровОтКлиента)
	|				И ТИПЗНАЧЕНИЯ(Партии.ДокументИсточник) В (&ТипыДокументовЭкспорта)
	...
	|		Партии.ИдентификаторФинЗаписи КАК ИдентификаторФинЗаписи,
	|		ВЫБОР
	|			КОГДА ТИПЗНАЧЕНИЯ(Партии.Регистратор) = ТИП(Документ.РаспределениеПрочихЗатрат)
	|				И Партии.СтатьяРасходов = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.НДСНалоговогоАгента)
	|				И Партии.КорСтатьяРасходов <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)
	|				ТОГДА ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ВключениеНДСВСтоимость)
	|			ИНАЧЕ
	...
	|				И Партии.ДокументПоступленияРасходов = НЕОПРЕДЕЛЕНО
	...
	|			КОГДА ТИПЗНАЧЕНИЯ(Партии.Регистратор) = ТИП(Документ.КорректировкаРеализации)
	|				ТОГДА ВЫРАЗИТЬ(Партии.Регистратор КАК Документ.КорректировкаРеализации).ДокументОснование
	|			КОГДА ТИПЗНАЧЕНИЯ(Партии.Регистратор) = ТИП(Документ.ОтгрузкаТоваровКлиенту)
	|					И ВЫРАЗИТЬ(Партии.Регистратор КАК Документ.ОтгрузкаТоваровКлиенту).Исправление
	|				ТОГДА ВЫРАЗИТЬ(Партии.Регистратор КАК Документ.ОтгрузкаТоваровКлиенту).ИсправляемыйДокумент
	|			КОГДА ТИПЗНАЧЕНИЯ(Партии.Регистратор) = ТИП(Документ.ВозвратТоваровОтКлиента)
	|				И ТИПЗНАЧЕНИЯ(Партии.ДокументИсточник) В (&ТипыДокументовЭкспорта)
	|				ТОГДА Партии.ДокументИсточник
	|			КОГДА ТИПЗНАЧЕНИЯ(Партии.Регистратор) В (&ТипыДокументовЭкспорта)
	...
	|		И Партии.НДС <> 0
	|		И Партии.КорВидДеятельностиНДС <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)
	|		И Партии.ХозяйственнаяОперация НЕ В 
	|			(ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности),
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности))
	|		И НЕ Партии.Сторно
	|
...
Процедура ПолучитьИсправленныеСчетаФактурыДокументы(ПараметрыРасчета) Экспорт
	...
	|	Задания.СчетФактура,
	|	НЕОПРЕДЕЛЕНО,
	|	СторноПриобретения.Ссылка,
	|	0,
	|	ДАТАВРЕМЯ(1,1,1),
	|	СторноПриобретения.Дата,
	|	СторноПриобретения.Дата,
	|	ВЫБОР 
	|		КОГДА НАЧАЛОПЕРИОДА(СторноПриобретения.Дата, КВАРТАЛ) > НАЧАЛОПЕРИОДА(ПриобретениеТоваровУслуг.Дата, КВАРТАЛ)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ 
	|	КОНЕЦ,
	|	ВЫБОР 
	|		КОГДА НАЧАЛОПЕРИОДА(СторноПриобретения.Дата, КВАРТАЛ) > НАЧАЛОПЕРИОДА(ПриобретениеТоваровУслуг.Дата, КВАРТАЛ)
	|			ТОГДА КОНЕЦПЕРИОДА(ПриобретениеТоваровУслуг.Дата, КВАРТАЛ)
	...
	|		ПО Задания.СчетФактура = ПриобретениеТоваровУслуг.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.Сторно КАК СторноПриобретения
	|		ПО ПриобретениеТоваровУслуг.Ссылка = СторноПриобретения.СторнируемыйДокумент
	|			И (СторноПриобретения.Проведен)
	|			И (СторноПриобретения.Дата <= &КонецПериода)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	СчетФактура
...
Процедура ПолучитьРазрезыВычетаНДС(ПараметрыРасчета) Экспорт
	...
	|ПОМЕСТИТЬ ИсходныеРеализацииЭкспорт
	|ИЗ
	...
	|		И НДССостояниеРеализации0.Состояние В (
	|				ЗНАЧЕНИЕ(Перечисление.НДССостоянияРеализация0.ПодтвержденаРеализация0),
	|				ЗНАЧЕНИЕ(Перечисление.НДССостоянияРеализация0.ЧастичноПодтвержденаРеализация0),
	|				ЗНАЧЕНИЕ(Перечисление.НДССостоянияРеализация0.НеПодтвержденаРеализация0))
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	РеализацияЭкспорт
	...
	|ПОМЕСТИТЬ АктуальныеРеализацииЭкспорт
	|ИЗ
	...
	|					Т.РеализацияЭкспорт
	|				ИЗ
	|					ИсходныеРеализацииЭкспорт КАК Т)) КАК ИсправлениеДокументов
	|ГДЕ
	...
	|		И ТИПЗНАЧЕНИЯ(ИсправлениеДокументов.ИсправляемыйДокумент) = ТИПЗНАЧЕНИЯ(ИсправлениеДокументов.ПоследнийДокументЦепочки)
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВЫБОР
	|		КОГДА НЕ АктуальныеРеализацииЭкспорт.ИсправляемыйДокумент ЕСТЬ NULL
	|			ТОГДА АктуальныеРеализацииЭкспорт.АктуальныйДокумент
	|		ИНАЧЕ ИсходныеРеализацииЭкспорт.РеализацияЭкспорт
	|	КОНЕЦ КАК АктуальныйДокументРеализации,
	|	ВЫБОР
	|		КОГДА НЕ АктуальныеРеализацииЭкспорт.ИсправляемыйДокумент ЕСТЬ NULL
	|			ТОГДА АктуальныеРеализацииЭкспорт.ИсправляемыйДокумент
	|		ИНАЧЕ ИсходныеРеализацииЭкспорт.РеализацияЭкспорт
	|	КОНЕЦ КАК ДокументРеализации,
	|	ИсходныеРеализацииЭкспорт.Организация КАК Организация,
	|	ИсходныеРеализацииЭкспорт.Состояние КАК Состояние,
	|	ИсходныеРеализацииЭкспорт.ДатаПодтверждения КАК ДатаПодтверждения
	|ПОМЕСТИТЬ НДССостояниеРеализации0СИсправлениями
	|ИЗ
	|	ИсходныеРеализацииЭкспорт КАК ИсходныеРеализацииЭкспорт
	|	ЛЕВОЕ СОЕДИНЕНИЕ АктуальныеРеализацииЭкспорт КАК АктуальныеРеализацииЭкспорт
	|	ПО ИсходныеРеализацииЭкспорт.РеализацияЭкспорт = АктуальныеРеализацииЭкспорт.ИсправляемыйДокумент
	|		И НЕ АктуальныеРеализацииЭкспорт.СторнированДокумент
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(НДССостояниеРеализации0.Состояние, НЕОПРЕДЕЛЕНО)
	|						В (ЗНАЧЕНИЕ(Перечисление.НДССостоянияРеализация0.ПодтвержденаРеализация0),
	|						   ЗНАЧЕНИЕ(Перечисление.НДССостоянияРеализация0.ЧастичноПодтвержденаРеализация0))
	...
	|			ТОГДА НДССостояниеРеализации0.АктуальныйДокументРеализации
	|		
	|		КОГДА ЕСТЬNULL(ПодтверждениеОплатыНДСВБюджет.ДатаПодтвержденияОплаты, ДАТАВРЕМЯ(1,1,1))
	|					>= ЕСТЬNULL(НДССостояниеРеализации0.ДатаПодтверждения, ДАТАВРЕМЯ(1,1,1))
	|				И ЕСТЬNULL(ПодтверждениеОплатыНДСВБюджет.ДатаПодтвержденияОплаты, ДАТАВРЕМЯ(1,1,1))
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		НДССостояниеРеализации0СИсправлениями КАК НДССостояниеРеализации0
	|	ПО
	|		РазрезыВычетаНДС.Организация = НДССостояниеРеализации0.Организация
	|		И РазрезыВычетаНДС.РеализацияЭкспорт = НДССостояниеРеализации0.ДокументРеализации
	|
	...
	|				ИЛИ (НЕ ИсправленныеСчетаФактуры.СчетФактура ЕСТЬ NULL И РазрезыВычетаНДС.ИсправленныйСчетФактура = НЕОПРЕДЕЛЕНО И ТИПЗНАЧЕНИЯ(ИсправленныеСчетаФактуры.Ссылка) = ТИП(Документ.Сторно))
	|	КОНЕЦ
	|	И ВЫБОР
...
Процедура ПолучитьБлокировкиНДС(ПараметрыРасчета) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЧастичноПодтвержденныеРеализации0.Организация КАК Организация,
	|	ЧастичноПодтвержденныеРеализации0.СчетФактура КАК СчетФактура,
	|	ВЫРАЗИТЬ(ЧастичноПодтвержденныеРеализации0.НДСПриобретения *
	|		ВЫРАЗИТЬ((ЧастичноПодтвержденныеРеализации0.СуммаРеализации - ЧастичноПодтвержденныеРеализации0.СуммаПодтверждения)
	|		/ ЧастичноПодтвержденныеРеализации0.СуммаРеализации КАК ЧИСЛО(21, 8)) КАК ЧИСЛО(15, 2)) КАК ЗаблокированныйНДС,
	|	ЧастичноПодтвержденныеРеализации0.ДокументОтгрузки КАК РеализацияЭкспорт
	|ПОМЕСТИТЬ ЧастичноПодтвержденныеРеализации0
	|ИЗ
	|	НалоговаяБазаРеализации0 КАК ЧастичноПодтвержденныеРеализации0
	|ГДЕ
	|	ЧастичноПодтвержденныеРеализации0.Состояние = ЗНАЧЕНИЕ(Перечисление.НДССостоянияРеализация0.ЧастичноПодтвержденаРеализация0)
	|	И ЧастичноПодтвержденныеРеализации0.СуммаРеализации > ЧастичноПодтвержденныеРеализации0.СуммаПодтверждения
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ГДЕ
	|		ОплаченныеЭлектронныеУслуги.СуммаПриобретения <> ОплаченныеЭлектронныеУслуги.СуммаОплаты
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ЧастичноПодтвержденныеРеализации0.Организация,
	|		ЧастичноПодтвержденныеРеализации0.СчетФактура,
	|		СУММА(ЧастичноПодтвержденныеРеализации0.ЗаблокированныйНДС)
	|	ИЗ
	|		ЧастичноПодтвержденныеРеализации0 КАК ЧастичноПодтвержденныеРеализации0
	|	СГРУППИРОВАТЬ ПО
	|		ЧастичноПодтвержденныеРеализации0.Организация,
	|		ЧастичноПодтвержденныеРеализации0.СчетФактура) КАК ВсеБлокировки
	|СГРУППИРОВАТЬ ПО
	...
	|		ЧастичноПодтвержденныеРеализации0.Организация КАК Организация,
	|		ЧастичноПодтвержденныеРеализации0.СчетФактура КАК СчетФактура,
	|		СУММА(ЧастичноПодтвержденныеРеализации0.ЗаблокированныйНДС) КАК НДС,
	|		ЧастичноПодтвержденныеРеализации0.РеализацияЭкспорт КАК РеализацияЭкспорт
	|ПОМЕСТИТЬ БлокировкаВычетаНДСРеализацияЭкспорт
	|ИЗ
	|		ЧастичноПодтвержденныеРеализации0 КАК ЧастичноПодтвержденныеРеализации0
	|	СГРУППИРОВАТЬ ПО
	|		ЧастичноПодтвержденныеРеализации0.Организация,
	|		ЧастичноПодтвержденныеРеализации0.СчетФактура,
	|		ЧастичноПодтвержденныеРеализации0.РеализацияЭкспорт";
...
// См. УчетНДСУПСлужебный.РассчитатьЗаписиНДСПредъявленныйВосстановлениеПодВидДеятельности
Процедура РассчитатьЗаписиНДСПредъявленныйВосстановлениеПодВидДеятельности(ПараметрыРасчета) Экспорт
	...
	|	МАКСИМУМ(ПартииНДСКРаспределению.Регистратор) КАК Регистратор
	|ПОМЕСТИТЬ ИсправленныеСФКРаспределению
	|ИЗ
	...
	|	ПО
	|		ПартииНДСКРаспределению.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|		И НДСПредъявленныйОстаткиНаКонецПериода.ИсправленныйСчетФактура = ПартииНДСКРаспределению.Регистратор
	|		И ПартииНДСКРаспределению.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОпределяетсяРаспределением)
	|
	|СГРУППИРОВАТЬ ПО
	|	НДСПредъявленныйОстаткиНаКонецПериода.ИсправленныйСчетФактура
	|;
	|
	|ВЫБРАТЬ
	|	1 КАК Приоритет,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СчетаФактурыДокументы.ДатаЗаписиКнигиПокупок, ДАТАВРЕМЯ(1, 1, 1)) >= &НачалоПериода
	|			ТОГДА СчетаФактурыДокументы.ДатаЗаписиКнигиПокупок
	|		ИНАЧЕ &КонецПериода
	|	КОНЕЦ КАК Период,
	|	ЕСТЬNULL(СчетаФактурыДокументы.Ссылка, НДСПредъявленный.СчетФактура) КАК Регистратор,
	...
	|	НДСПредъявленный.ОтнестиКПредыдущемуКварталу КАК ОтнестиКПредыдущемуКварталу,
	|	НДСПредъявленный.ИсправляемыйПериод КАК ИсправляемыйПериод,
	|	НДСПредъявленный.ДатаИсправленияРаспределенияНДС КАК ДатаИсправленияРаспределенияНДС,
	...
	|		ПО НДСПредъявленный.Организация = СчетаФактурыДокументы.Организация
	...
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ИсправленныеСФКРаспределению КАК ИсправленныеСФКРаспределению
	|	ПО
	|		НДСПредъявленный.ИсправленныйСчетФактура = ИсправленныеСФКРаспределению.ИсправленныйСчетФактура
	|
	|ГДЕ
	...
	|		И ИсправленныеСФКРаспределению.Регистратор ЕСТЬ NULL
	...
	|	И НЕ НДСПредъявленный.СчетФактура = НЕОПРЕДЕЛЕНО
	|
	|СГРУППИРОВАТЬ ПО
	|	ЕСТЬNULL(СчетаФактурыДокументы.Ссылка, НДСПредъявленный.СчетФактура),
	...
	|	НДСПредъявленный.ОтнестиКПредыдущемуКварталу,
	|	НДСПредъявленный.ИсправляемыйПериод,
	|	НДСПредъявленный.ДатаИсправленияРаспределенияНДС,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СчетаФактурыДокументы.ДатаЗаписиКнигиПокупок, ДАТАВРЕМЯ(1, 1, 1)) >= &НачалоПериода
	|			ТОГДА СчетаФактурыДокументы.ДатаЗаписиКнигиПокупок
	|		ИНАЧЕ &КонецПериода
...
// См. УчетНДСУПСлужебный.ПолучитьНалоговуюБазуРеализации0
Процедура ПолучитьНалоговуюБазуРеализации0(ПараметрыРасчета) Экспорт
	...
	|ИЗ
	|	НДСПредъявленныйОстаткиНаКонецПериода КАК НДСПредъявленныйОстаткиНаКонецПериода
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.НДССостояниеРеализации0.СрезПоследних(&КонецПериода) КАК НДССостояниеРеализации0
	|		ПО НДСПредъявленныйОстаткиНаКонецПериода.Организация = НДССостояниеРеализации0.Организация
	|		И НДСПредъявленныйОстаткиНаКонецПериода.РеализацияЭкспорт = НДССостояниеРеализации0.ДокументРеализации
	...
	|ИЗ
	|	РегистрСведений.СуммыДокументовВВалютахУчета.СрезПоследних(, Регистратор В
	|		(ВЫБРАТЬ
	|			Т.ДокументОтгрузки
	|		ИЗ
	...
	|	НДССостояниеРеализации0.Состояние КАК Состояние,
	|	СуммыДокументовВВалютахУчета.БазаНДСРегл КАК СуммаРеализации
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЕСТЬNULL(НДСПредъявленный.Организация, НДСПредъявленныйИзменениеВидаДеятельности.Организация) КАК Организация,
	|	ЕСТЬNULL(НДСПредъявленный.СчетФактура, НДСПредъявленныйИзменениеВидаДеятельности.СчетФактура) КАК СчетФактура,
	|	НДССостояниеРеализации0.ДокументОтгрузки КАК ДокументОтгрузки,
	|	НДССостояниеРеализации0.Состояние КАК Состояние,
	|	СУММА(ЕСТЬNULL(НДСПредъявленный.НДС, НДСПредъявленныйИзменениеВидаДеятельности.НДС)) КАК НДСПриобретения
	|ПОМЕСТИТЬ НДСПредъявленныйРеализацияЭкспорт
	|ИЗ
	|	НДССостояниеРеализации0 КАК НДССостояниеРеализации0
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.НДСПредъявленный КАК НДСПредъявленный
	|		ПО НДССостояниеРеализации0.ДокументОтгрузки = НДСПредъявленный.РеализацияЭкспорт
	|		И НДССостояниеРеализации0.СчетФактура = НДСПредъявленный.СчетФактура
	|		И НДСПредъявленный.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		И НДСПредъявленный.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортСырьевыхТоваровУслуг)
	...
	|			ИЛИ НДСПредъявленный.НастройкаХозяйственнойОперации = ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ВводОстатковНДСПоПриобретеннымЦенностям))
	|		ЛЕВОЕ СОЕДИНЕНИЕ НДСПредъявленныйИзменениеВидаДеятельности КАК НДСПредъявленныйИзменениеВидаДеятельности
	|		ПО НДССостояниеРеализации0.ДокументОтгрузки = НДСПредъявленныйИзменениеВидаДеятельности.РеализацияЭкспорт
	|		И НДССостояниеРеализации0.СчетФактура = НДСПредъявленныйИзменениеВидаДеятельности.СчетФактура
	|		И НДСПредъявленныйИзменениеВидаДеятельности.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		И НДСПредъявленныйИзменениеВидаДеятельности.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортСырьевыхТоваровУслуг)
	|		И НДСПредъявленныйИзменениеВидаДеятельности.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияНДСПредъявленный.Реализация)
	|СГРУППИРОВАТЬ ПО
	|	ЕСТЬNULL(НДСПредъявленный.Организация, НДСПредъявленныйИзменениеВидаДеятельности.Организация),
	|	ЕСТЬNULL(НДСПредъявленный.СчетФактура, НДСПредъявленныйИзменениеВидаДеятельности.СчетФактура),
	|	НДССостояниеРеализации0.ДокументОтгрузки,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НДСПредъявленныйРеализацияЭкспорт.Организация КАК Организация,
	|	НДСПредъявленныйРеализацияЭкспорт.СчетФактура КАК СчетФактура,
	|	НДСПредъявленныйРеализацияЭкспорт.Состояние КАК Состояние,
	|	МАКСИМУМ(НДСПредъявленныйРеализацияЭкспорт.НДСПриобретения) КАК НДСПриобретения,
	|	МАКСИМУМ(ДокументыРеализации0.СуммаРеализации) КАК СуммаРеализации,
	|	ДокументыРеализации0.ДокументОтгрузки КАК ДокументОтгрузки,
	|	ЕСТЬNULL(КодыОперацийНалоговаяБаза.КодОперации, """") КАК КодОперации,
	|	СУММА(ЕСТЬNULL(КодыОперацийНалоговаяБаза.ПродажиСНДС0, ПодтверждениеНулевойСтавкиНДС.ПродажиСНДС0)) КАК СуммаПодтверждения
	|ПОМЕСТИТЬ НалоговаяБазаРеализации0ВРазрезеКодовОпераций
	|ИЗ
	|	ДокументыРеализации0 КАК ДокументыРеализации0
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ НДСПредъявленныйРеализацияЭкспорт КАК НДСПредъявленныйРеализацияЭкспорт
	|		ПО ДокументыРеализации0.ДокументОтгрузки = НДСПредъявленныйРеализацияЭкспорт.ДокументОтгрузки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПодтверждениеНулевойСтавкиНДС.Состав КАК ПодтверждениеНулевойСтавкиНДС
	|		ПО ДокументыРеализации0.ДокументОтгрузки = ПодтверждениеНулевойСтавкиНДС.ДокументОтгрузки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ НДССостояниеРеализации0 КАК НДССостояниеРеализации0
	|		ПО ПодтверждениеНулевойСтавкиНДС.ДокументОтгрузки = НДССостояниеРеализации0.ДокументОтгрузки
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПодтверждениеНулевойСтавкиНДС.КодыОперацийНалоговаяБаза КАК КодыОперацийНалоговаяБаза
	|		ПО КодыОперацийНалоговаяБаза.Ссылка = ПодтверждениеНулевойСтавкиНДС.Ссылка
	|		И КодыОперацийНалоговаяБаза.КлючСтроки = ПодтверждениеНулевойСтавкиНДС.КлючСтроки
	|ГДЕ
	|	ПодтверждениеНулевойСтавкиНДС.Ссылка.Проведен
	|	И ПодтверждениеНулевойСтавкиНДС.Ссылка.Дата <= &КонецПериода
	|	И НЕ ЕСТЬNULL(КодыОперацийНалоговаяБаза.НесырьевыеТовары, ЛОЖЬ)
	|СГРУППИРОВАТЬ ПО
	|	ЕСТЬNULL(КодыОперацийНалоговаяБаза.КодОперации, """"),
	|	ДокументыРеализации0.ДокументОтгрузки,
	|	НДСПредъявленныйРеализацияЭкспорт.Состояние,
	|	НДСПредъявленныйРеализацияЭкспорт.СчетФактура,
	|	НДСПредъявленныйРеализацияЭкспорт.Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НалоговаяБазаРеализации0ВРазрезеКодовОпераций.Организация КАК Организация,
	|	НалоговаяБазаРеализации0ВРазрезеКодовОпераций.СчетФактура КАК СчетФактура,
	|	НалоговаяБазаРеализации0ВРазрезеКодовОпераций.Состояние КАК Состояние,
	|	МАКСИМУМ(НалоговаяБазаРеализации0ВРазрезеКодовОпераций.НДСПриобретения) КАК НДСПриобретения,
	|	НалоговаяБазаРеализации0ВРазрезеКодовОпераций.ДокументОтгрузки КАК ДокументОтгрузки,
	|	МАКСИМУМ(НалоговаяБазаРеализации0ВРазрезеКодовОпераций.СуммаРеализации) КАК СуммаРеализации,
	|	СУММА(НалоговаяБазаРеализации0ВРазрезеКодовОпераций.СуммаПодтверждения) КАК СуммаПодтверждения
	|ПОМЕСТИТЬ НалоговаяБазаРеализации0
	|ИЗ
	|	НалоговаяБазаРеализации0ВРазрезеКодовОпераций КАК НалоговаяБазаРеализации0ВРазрезеКодовОпераций
	|СГРУППИРОВАТЬ ПО
	|	НалоговаяБазаРеализации0ВРазрезеКодовОпераций.Организация,
	|	НалоговаяБазаРеализации0ВРазрезеКодовОпераций.СчетФактура,
	|	НалоговаяБазаРеализации0ВРазрезеКодовОпераций.Состояние,
	|	НалоговаяБазаРеализации0ВРазрезеКодовОпераций.ДокументОтгрузки";
...
Процедура ПолучитьЗачетыАвансовПолученныхСчетамиФактурами(МенеджерВременныхТаблиц, ПараметрыРасчета) Экспорт
	...
	УстановитьПараметрыЗапросаЗачетыАвансовПолученныхСчетамиФактурами(Запрос, ПараметрыРасчета);
	
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов.Организация КАК Организация,
	|	ЗачетыАвансовСчетамиФактурами.Регистратор КАК ДокументРегистратор,
	|	ЗачетыАвансовСчетамиФактурами.Контрагент КАК Контрагент,
	|	ЗачетыАвансовСчетамиФактурами.Договор КАК Договор,
	|	МАКСИМУМ(ЗачетыАвансовСчетамиФактурами.Период) КАК Дата,
	|	ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов КАК ОбъектРасчетов,
	|	ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов.НалогообложениеНДС КАК НалогообложениеНДС,
	|	ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов.Валюта КАК Валюта,
	|	ЗачетыАвансовСчетамиФактурами.РасчетныйДокумент КАК ДокументОплаты,
	|	СУММА(ЗачетыАвансовСчетамиФактурами.СуммаРегл) КАК СуммаРегл,
	|	СУММА(ЗачетыАвансовСчетамиФактурами.Сумма) КАК Сумма
	|ПОМЕСТИТЬ ЗачетыАвансовПолученныхСчетамиФактурамиПредварительная
	|ИЗ
	|	РегистрНакопления.ЗачетыАвансовСчетамиФактурами КАК ЗачетыАвансовСчетамиФактурами
	|ГДЕ
	|	ЗачетыАвансовСчетамиФактурами.Период МЕЖДУ &ДатаНачала И &ДатаОкончания
	|	И (НЕ &ВключитьОтборПоОрганизациям
	|			ИЛИ ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов.Организация В (&Организации))
	|	И НЕ ЗачетыАвансовСчетамиФактурами.КорректировкаНаУменьшение
	|	И ЗачетыАвансовСчетамиФактурами.Активность = ИСТИНА
	|СГРУППИРОВАТЬ ПО
	|	ЗачетыАвансовСчетамиФактурами.Контрагент,
	|	ЗачетыАвансовСчетамиФактурами.Договор,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗачетыАвансовСчетамиФактурами.Организация КАК Организация,
	|	ЗачетыАвансовСчетамиФактурами.ДокументРегистратор КАК ДокументРегистратор,
	|	ЗачетыАвансовСчетамиФактурами.Контрагент КАК Контрагент,
	|	ЗачетыАвансовСчетамиФактурами.Договор КАК Договор,
	|	ЗачетыАвансовСчетамиФактурами.Дата КАК Дата,
	|	ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов КАК ОбъектРасчетов,
	|	ЗачетыАвансовСчетамиФактурами.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ЗачетыАвансовСчетамиФактурами.НалогообложениеНДС КАК НалогообложениеНДС,
	|	ЗачетыАвансовСчетамиФактурами.Валюта КАК Валюта,
	|	ЗачетыАвансовСчетамиФактурами.ДокументОплаты КАК ДокументОплаты,
	|	ЗачетыАвансовСчетамиФактурами.СуммаРегл КАК СуммаРегл,
	|	ЗачетыАвансовСчетамиФактурами.Сумма КАК Сумма
	|ПОМЕСТИТЬ ЗачетыАвансовПолученныхСчетамиФактурами
	|ИЗ
	|	ЗачетыАвансовПолученныхСчетамиФактурамиПредварительная КАК ЗачетыАвансовСчетамиФактурами
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументов
	|		ПО ЗачетыАвансовСчетамиФактурами.ДокументРегистратор = РеестрДокументов.Ссылка
	|			И (НЕ &ВключитьОтборПоКонтрагенту
	|				ИЛИ ЗачетыАвансовСчетамиФактурами.Контрагент = РеестрДокументов.Контрагент)
	|			И (НЕ &ВключитьОтборПоКонтрагенту
	|				ИЛИ ЗачетыАвансовСчетамиФактурами.Договор = РеестрДокументов.Договор)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ЗачетыАвансовПолученныхСчетамиФактурамиПредварительная";
...
Процедура ПолучитьЗачетыАвансовПолученныхСчетамиФактурамиКорректировкиНаУменьшение(МенеджерВременныхТаблиц, ПараметрыРасчета) Экспорт
	...
	|	ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ВЫБОР
	|		КОГДА ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов.НалогообложениеНДС ЕСТЬ NULL
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
	|		ИНАЧЕ ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов.НалогообложениеНДС
	|	КОНЕЦ КАК НалогообложениеНДС,
	|	ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов.Валюта КАК Валюта,
	|	ЗачетыАвансовСчетамиФактурами.РасчетныйДокумент КАК ДокументОплаты,
	|	СУММА(-ЗачетыАвансовСчетамиФактурами.СуммаРегл) КАК СуммаРегл,
	|	СУММА(-ЗачетыАвансовСчетамиФактурами.Сумма) КАК Сумма
	|ПОМЕСТИТЬ ЗачетыАвансовПолученныхСчетамиФактурамиКорректировкиНаУменьшениеПредварительная
	|ИЗ
	|	РегистрНакопления.ЗачетыАвансовСчетамиФактурами КАК ЗачетыАвансовСчетамиФактурами
	|ГДЕ
	|	ЗачетыАвансовСчетамиФактурами.Период МЕЖДУ &ДатаНачала И &ДатаОкончания
	|	И (НЕ &ВключитьОтборПоОрганизациям
	|			ИЛИ ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов.Организация В (&Организации)
	|			ИЛИ ЗачетыАвансовСчетамиФактурами.Договор.Организация В (&Организации))
	|	И ЗачетыАвансовСчетамиФактурами.КорректировкаНаУменьшение
	|	И ЗачетыАвансовСчетамиФактурами.Активность = ИСТИНА
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗачетыАвансовСчетамиФактурами.Контрагент,
	...
	|	ЗачетыАвансовСчетамиФактурами.РасчетныйДокумент,
	|	ЗачетыАвансовСчетамиФактурами.Регистратор,
	|	ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов.Организация,
	|	ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов.НаправлениеДеятельности,
	|	ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов.Валюта,
	|	ВЫБОР
	|		КОГДА ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов.НалогообложениеНДС ЕСТЬ NULL
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
	|		ИНАЧЕ ЗачетыАвансовСчетамиФактурами.ОбъектРасчетов.НалогообложениеНДС
	...
	|	ЗачетыАвансовСчетамиФактурами.СуммаРегл КАК СуммаРегл,
	|	ЗачетыАвансовСчетамиФактурами.Сумма КАК Сумма
	|ПОМЕСТИТЬ ЗачетыАвансовПолученныхСчетамиФактурамиКорректировкиНаУменьшение
	|ИЗ
	|	ЗачетыАвансовПолученныхСчетамиФактурамиКорректировкиНаУменьшениеПредварительная КАК ЗачетыАвансовСчетамиФактурами
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументов
	|		ПО ЗачетыАвансовСчетамиФактурами.ДокументРегистратор = РеестрДокументов.Ссылка
	|			И (НЕ &ВключитьОтборПоКонтрагенту
	|				ИЛИ ЗачетыАвансовСчетамиФактурами.Контрагент = РеестрДокументов.Контрагент)
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ЗачетыАвансовПолученныхСчетамиФактурамиКорректировкиНаУменьшениеПредварительная";
...
Процедура ОписаниеПроверок_ФормированиеДвиженийПоНДС(ТаблицаПроверок) Экспорт
	...
	ОписаниеПроверки = ЗакрытиеМесяцаСервер.ДобавитьОписаниеНовойПроверки(ТаблицаПроверок,
		...
		Справочники.ОперацииЗакрытияМесяца.ФормированиеДвиженийПоНДС,
...
//  ПредшествующийЭтап - Справочники.ОперацииЗакрытияМесяца - этап, после которого будет следовать новый.
//
Процедура ДобавитьЭтап_ФормированиеСводныхСправокНДС(ТаблицаЭтапов, ТекущийРодитель) Экспорт
	
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ФормированиеСводныхСправокНДС,
		Ложь, Истина, Ложь,
		Справочники.ОперацииЗакрытияМесяца.РаспределениеДоходовПоНаправлениямДеятельности);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Формирование сводных справок НДС'"; // @НСтр-1
...
Процедура Выполнить_ФормированиеСводныхСправокНДС(ПараметрыОбработчика) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("УправлениеТорговлей") Тогда
		ПараметрыОбработчика.ДанныеЭтапа.Код = Справочники.ОперацииЗакрытияМесяца.ФормированиеСводныхСправокНДС;
	КонецЕсли;
...
// Возвращает коллекцию типов документов, используемых для учета денежных операций в разрезе НДС.
//
// Возвращаемое значение:
//   Массив из Тип - Содержит типы денежных документов: РасходныйКассовыйОрдер, ПриходныйКассовыйОрдер, ОперацияПоПлатежнойКарте, СписаниеБезналичныхДенежныхСредств, ОперацияПоЯндексКассе, РеализацияПодарочныхСертификатов, ВозвратПодарочныхСертификатов, ПоступлениеБезналичныхДенежныхСредств, ПервичныйДокумент
//
функция ТипыДенежныхДокументов() Экспорт
	
	ТипыДенежныхДокументов = Новый Массив;
	ТипыДенежныхДокументов.Добавить(Тип("ДокументСсылка.РасходныйКассовыйОрдер"));
	ТипыДенежныхДокументов.Добавить(Тип("ДокументСсылка.ПриходныйКассовыйОрдер"));
	ТипыДенежныхДокументов.Добавить(Тип("ДокументСсылка.ОперацияПоПлатежнойКарте"));
	ТипыДенежныхДокументов.Добавить(Тип("ДокументСсылка.СписаниеБезналичныхДенежныхСредств"));
	ТипыДенежныхДокументов.Добавить(Тип("ДокументСсылка.ОперацияПоЯндексКассе"));
	ТипыДенежныхДокументов.Добавить(Тип("ДокументСсылка.РеализацияПодарочныхСертификатов"));
	ТипыДенежныхДокументов.Добавить(Тип("ДокументСсылка.ВозвратПодарочныхСертификатов"));
	ТипыДенежныхДокументов.Добавить(Тип("ДокументСсылка.ПоступлениеБезналичныхДенежныхСредств"));
	ТипыДенежныхДокументов.Добавить(Тип("ДокументСсылка.ПервичныйДокумент"));
	
	Возврат ТипыДенежныхДокументов;
...
Процедура СформироватьНДСЗаписиКнигиПокупок(ПараметрыРасчета)
	...
	РассчитатьНДСЗаписиКнигиПокупокСторноПредварительно(ПараметрыРасчета);
	РассчитатьНДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно(ПараметрыРасчета);
	РассчитатьИтоговыеЗаписиКнигиПокупокПоНДСПредъявленныйИСторно(ПараметрыРасчета);
...
Процедура ПолучитьДокументыОплатыГрафа7(ПараметрыРасчета)
	...
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(Задания.СчетФактура) В (ТИП(Документ.ПервичныйДокумент), ТИП(Документ.ОперацияСЭлектроннымБилетом), ТИП(Документ.ТаможеннаяДекларацияИмпорт))
	|		ИЛИ &НачалоПериода < &ДатаНачалаОбязательногоПримененияЕНП
	|	
	|ОБЪЕДИНИТЬ ВСЕ
	|	
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура РассчитатьНДСЗаписиКнигиПокупокСторноПредварительно(ПараметрыРасчета)
	...
	|	ПО
	|		НДСЗаписиКнигиПокупок.ИсправленныйСчетФактура <> НЕОПРЕДЕЛЕНО
	|		И НДСЗаписиКнигиПокупок.ИсправленныйСчетФактура = ПрошлыеСторноИсправительногоСчетаФактуры.СторнируемыйДокумент
	...
	|	И НЕ (ТИПЗНАЧЕНИЯ(НДСЗаписиКнигиПокупок.Регистратор) = ТИП(Документ.РаспределениеНДС)
	|			И ТИПЗНАЧЕНИЯ(ДокументыСторноРегистратор.Ссылка) <> ТИП(Документ.РаспределениеНДС))
	...
	|	ВЫБОР 
	|		КОГДА НАЧАЛОПЕРИОДА(НДСЗаписиКнигиПокупокОтборСторно.Период, КВАРТАЛ) > НАЧАЛОПЕРИОДА(НДСЗаписиКнигиПокупок.Период, КВАРТАЛ)
	|			И НЕ НДСЗаписиКнигиПокупок.СторнирующаяЗаписьДопЛиста
	|			И ТИПЗНАЧЕНИЯ(НДСЗаписиКнигиПокупок.Регистратор) = ТИП(Документ.РаспределениеНДС)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ НДСЗаписиКнигиПокупок.СторнирующаяЗаписьДопЛиста
	...
	|	NULL                                          КАК НомерСчетаФактуры,
	|	NULL                                          КАК ДатаСчетаФактуры,
	|	NULL                                          КАК НомерИсправленияСчетаФактуры,
	|	NULL                                          КАК ДатаИсправленияСчетаФактуры
	|ПОМЕСТИТЬ НДСЗаписиКнигиПокупокСторноПредварительно
	|ИЗ
	|	РегистрНакопления.НДСЗаписиКнигиПокупок КАК НДСЗаписиКнигиПокупок
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция РассчитатьНДСЗаписиКнигиПокупокСторно(ПараметрыРасчета)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	НДСЗаписиКнигиПокупокСторноПредварительно.Приоритет              КАК Приоритет,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.Период                 КАК Период,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.Регистратор            КАК Регистратор,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.Организация            КАК Организация,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.Поставщик              КАК Поставщик,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.СчетФактура            КАК СчетФактура,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.ВидЦенности            КАК ВидЦенности,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.СтавкаНДС              КАК СтавкаНДС,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.ДатаОплаты             КАК ДатаОплаты,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.ДокументОплаты         КАК ДокументОплаты,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.Событие                КАК Событие,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.ДатаСобытия            КАК ДатаСобытия,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.ЗаписьДополнительногоЛиста   КАК ЗаписьДополнительногоЛиста,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.КорректируемыйПериод         КАК КорректируемыйПериод,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.СторнирующаяЗаписьДопЛиста   КАК СторнирующаяЗаписьДопЛиста,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.ДоговорКонтрагента      КАК ДоговорКонтрагента,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.ИсправленныйСчетФактура КАК ИсправленныйСчетФактура,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.СуммаБезНДС             КАК СуммаБезНДС,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.НДС                     КАК НДС,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.НДСУпр                  КАК НДСУпр,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.ХозяйственнаяОперация   КАК ХозяйственнаяОперация,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.НомерДокументаОплаты    КАК НомерДокументаОплаты,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.ДатаДокументаОплаты     КАК ДатаДокументаОплаты,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.КодВидаОперации         КАК КодВидаОперации,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.РегламентнаяОперация    КАК РегламентнаяОперация,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.Сторно                  КАК Сторно,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.ДокументОтгрузки        КАК ДокументОтгрузки,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.ВидДеятельностиНДС      КАК ВидДеятельностиНДС,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.КодОперации0            КАК КодОперации0,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.НаправлениеДеятельности       КАК НаправлениеДеятельности,
	|	НДСЗаписиКнигиПокупокСторноПредварительно.ПродавцомВыступаетОрганизация КАК ПродавцомВыступаетОрганизация,
	|	NULL КАК НомерСчетаФактуры,
	|	NULL КАК ДатаСчетаФактуры,
	|	NULL КАК НомерИсправленияСчетаФактуры,
	|	NULL КАК ДатаИсправленияСчетаФактуры
	...
	|ИЗ
	|	НДСЗаписиКнигиПокупокСторноПредварительно КАК НДСЗаписиКнигиПокупокСторноПредварительно
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно КАК НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно
	|	ПО
	|		НДСЗаписиКнигиПокупокСторноПредварительно.Регистратор = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.Регистратор
	|		И НДСЗаписиКнигиПокупокСторноПредварительно.Организация = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.Организация
	|		И НДСЗаписиКнигиПокупокСторноПредварительно.Поставщик = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.Поставщик
	|		И НДСЗаписиКнигиПокупокСторноПредварительно.СчетФактура = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.СчетФактура
	|		И НДСЗаписиКнигиПокупокСторноПредварительно.ВидЦенности = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.ВидЦенности
	|		И НДСЗаписиКнигиПокупокСторноПредварительно.СтавкаНДС = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.СтавкаНДС
	|		И НДСЗаписиКнигиПокупокСторноПредварительно.ИсправленныйСчетФактура = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.ИсправленныйСчетФактура
	|		И НДСЗаписиКнигиПокупокСторноПредварительно.Событие = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.Событие
	|		И НДСЗаписиКнигиПокупокСторноПредварительно.ВидДеятельностиНДС = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.ВидДеятельностиНДС
	|		И НДСЗаписиКнигиПокупокСторноПредварительно.НаправлениеДеятельности = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.НаправлениеДеятельности
	|		И ТИПЗНАЧЕНИЯ(НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.Регистратор) = ТИП(Документ.РаспределениеНДС)
	|ГДЕ
	|	ЕСТЬNULL(НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.СуммаБезНДС, 1) <> 0
	|	ИЛИ ЕСТЬNULL(НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.НДС, 1) <> 0
	|	ИЛИ ЕСТЬNULL(НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.НДСУпр, 1) <> 0
	|";
	Запрос.МенеджерВременныхТаблиц = ПараметрыРасчета.МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("НачалоПериода", ПараметрыРасчета.НачалоПериода);
	Запрос.УстановитьПараметр("КонецПериода", ПараметрыРасчета.КонецПериода);
...
Процедура РассчитатьНДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно(ПараметрыРасчета)
	...
	|	НДСПредъявленный.ВидДеятельностиНДС                             КАК ВидДеятельностиНДС,
	|	НДСПредъявленный.КодОперации                                    КАК КодОперации0,
	...
	|	NULL                                                            КАК НомерИсправленияСчетаФактуры,
	|	NULL                                                            КАК ДатаИсправленияСчетаФактуры
	|ПОМЕСТИТЬ НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно
	|ИЗ
	...
	|		КОГДА НДСПредъявленный.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияНДСПредъявленный.ПодтвержденаСтавка0) ТОГДА
	|			ВЫБОР КОГДА НДСПредъявленный.ВидДеятельностиНДС <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.Космос)
	|					И ЕСТЬNULL(НДССостояниеРеализации0.ДатаПодтверждения, ДАТАВРЕМЯ(1, 1, 1)) < НАЧАЛОПЕРИОДА(НДСПредъявленный.Период, КВАРТАЛ)
	|					И ЕСТЬNULL(СчетаФактурыДокументы.ДатаЗаписиКнигиПокупок, ДАТАВРЕМЯ(1, 1, 1)) > КОНЕЦПЕРИОДА(ЕСТЬNULL(НДССостояниеРеализации0.ДатаПодтверждения, ДАТАВРЕМЯ(1, 1, 1)), КВАРТАЛ)
	...
	|			КОНЕЦ
	|		КОГДА НДСПредъявленный.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияНДСПредъявленный.НеПодтвержденаСтавка0) ТОГДА
	|			ВЫБОР КОГДА НДСПредъявленный.ВидДеятельностиНДС <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.Космос)
	|					И ЕСТЬNULL(СчетаФактурыДокументы.ДатаЗаписиКнигиПокупок, ДАТАВРЕМЯ(1, 1, 1)) > КОНЕЦПЕРИОДА(ЕСТЬNULL(ДанныеПервичныхДокументов.ДатаРегистратора, ДАТАВРЕМЯ(1, 1, 1)), КВАРТАЛ)
	|				ТОГДА ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПокупки.ПредъявленНДСКВычетуВПоследующихПериодахНеПодтвержденаСтавка0)
	|			ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПокупки.НеПодтвержденаСтавка0)
	|			КОНЕЦ
	|	КОНЕЦ                                                           КАК Событие,
	|	КОНЕЦПЕРИОДА(НДСПредъявленный.Период, КВАРТАЛ)                  КАК ДатаСобытия,
	|	ВЫБОР 
	...
	|		КОГДА ЛОЖЬ
	|			ТОГДА """"
	|		ИНАЧЕ НДСПредъявленный.КодОперации
	...
	|		РегистрСведений.НДССостояниеРеализации0.СрезПоследних(&КонецПериода) КАК НДССостояниеРеализации0
	|	ПО
	|		НДСПредъявленный.Организация = НДССостояниеРеализации0.Организация
	|		И НДСПредъявленный.РеализацияЭкспорт = НДССостояниеРеализации0.ДокументРеализации
	|		И НДССостояниеРеализации0.Состояние В (
	|				ЗНАЧЕНИЕ(Перечисление.НДССостоянияРеализация0.ПодтвержденаРеализация0),
	|				ЗНАЧЕНИЕ(Перечисление.НДССостоянияРеализация0.ЧастичноПодтвержденаРеализация0),
	|				ЗНАЧЕНИЕ(Перечисление.НДССостоянияРеализация0.НеПодтвержденаРеализация0))
...
Процедура РассчитатьИтоговыеЗаписиКнигиПокупокПоНДСПредъявленныйИСторно(ПараметрыРасчета)
	...
	|	СУММА(ВложенныйЗапрос.НДСУпр)                               КАК НДСУпр,
	...
	|ПОМЕСТИТЬ НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно
	|ИЗ
	|	(ВЫБРАТЬ
	|		НДСЗаписиКнигиПокупокСторноПредварительно.Регистратор                КАК Регистратор,
	|		НДСЗаписиКнигиПокупокСторноПредварительно.Организация                КАК Организация,
	...
	|
	|	ВЫБРАТЬ
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.Регистратор                КАК Регистратор,
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.Организация                КАК Организация,
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.Поставщик                  КАК Поставщик,
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.СчетФактура                КАК СчетФактура,
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ВидЦенности                КАК ВидЦенности,
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.СтавкаНДС                  КАК СтавкаНДС,
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ИсправленныйСчетФактура    КАК ИсправленныйСчетФактура,
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.Событие                    КАК Событие,
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.СуммаБезНДС                КАК СуммаБезНДС,
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.НДС                        КАК НДС,
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.НДСУпр                     КАК НДСУпр,
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ВидДеятельностиНДС         КАК ВидДеятельностиНДС,
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.НаправлениеДеятельности    КАК НаправлениеДеятельности
	|	ИЗ
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно КАК НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ВложенныйЗапрос.ИсправленныйСчетФактура,
...
Функция РассчитатьНДСЗаписиКнигиПокупокПоНДСПредъявленный(ПараметрыРасчета)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.Приоритет                       КАК Приоритет,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.Период                          КАК Период,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.Регистратор                     КАК Регистратор,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.Организация                     КАК Организация,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.Поставщик                       КАК Поставщик,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.СчетФактура                     КАК СчетФактура,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ВидЦенности                     КАК ВидЦенности,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.СтавкаНДС                       КАК СтавкаНДС,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ДатаОплаты                      КАК ДатаОплаты,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ДокументОплаты                  КАК ДокументОплаты,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.Событие                         КАК Событие,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ДатаСобытия                     КАК ДатаСобытия,
	|	ВЫБОР
	|		КОГДА ИсправлениеРаспределениеНДС.Ссылка ЕСТЬ NULL 
	|			ТОГДА НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ЗаписьДополнительногоЛиста
	|		ИНАЧЕ
	|			ИСТИНА
	|	КОНЕЦ                                                                                 КАК ЗаписьДополнительногоЛиста,
	|	ВЫБОР
	|		КОГДА ИсправлениеРаспределениеНДС.Ссылка ЕСТЬ NULL 
	|			ТОГДА НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.КорректируемыйПериод
	|		ИНАЧЕ
	|			КОНЕЦПЕРИОДА(ИсправлениеРаспределениеНДС.НачалоПериода, КВАРТАЛ)
	|	КОНЕЦ                                                                                 КАК КорректируемыйПериод,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.СторнирующаяЗаписьДопЛиста      КАК СторнирующаяЗаписьДопЛиста,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ДоговорКонтрагента              КАК ДоговорКонтрагента,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ИсправленныйСчетФактура         КАК ИсправленныйСчетФактура,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.СуммаБезНДС                     КАК СуммаБезНДС,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.НДС                             КАК НДС,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.НДСУпр                          КАК НДСУпр,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ХозяйственнаяОперация           КАК ХозяйственнаяОперация,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.НомерДокументаОплаты            КАК НомерДокументаОплаты,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ДатаДокументаОплаты             КАК ДатаДокументаОплаты,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.КодВидаОперации                 КАК КодВидаОперации,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.РегламентнаяОперация            КАК РегламентнаяОперация,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.Сторно                          КАК Сторно,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ДокументОтгрузки                КАК ДокументОтгрузки,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ВидДеятельностиНДС              КАК ВидДеятельностиНДС,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.КодОперации0                    КАК КодОперации0,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.НаправлениеДеятельности         КАК НаправлениеДеятельности,
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ПродавцомВыступаетОрганизация   КАК ПродавцомВыступаетОрганизация,
	|	NULL КАК НомерСчетаФактуры,
	|	NULL КАК ДатаСчетаФактуры,
	|	NULL КАК НомерИсправленияСчетаФактуры,
	|	NULL КАК ДатаИсправленияСчетаФактуры
	...
	|ИЗ
	|	НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно КАК НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно КАК НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно
	|	ПО
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.Регистратор = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.Регистратор
	|		И НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.Организация = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.Организация
	|		И НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.Поставщик = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.Поставщик
	|		И НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.СчетФактура = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.СчетФактура
	|		И НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ВидЦенности = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.ВидЦенности
	|		И НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.СтавкаНДС = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.СтавкаНДС
	|		И НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ИсправленныйСчетФактура = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.ИсправленныйСчетФактура
	|		И НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.Событие = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.Событие
	|		И НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.ВидДеятельностиНДС = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.ВидДеятельностиНДС
	|		И НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.НаправлениеДеятельности = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.НаправлениеДеятельности
	|		И НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.СчетФактура = НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.СчетФактура
	|		И ТИПЗНАЧЕНИЯ(НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.Регистратор) = ТИП(Документ.РаспределениеНДС)
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		Документ.РаспределениеНДС КАК ИсправлениеРаспределениеНДС
	|	ПО
	|		НДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно.Регистратор = ИсправлениеРаспределениеНДС.Ссылка
	|		И ИсправлениеРаспределениеНДС.Исправление
	|		И НАЧАЛОПЕРИОДА(ИсправлениеРаспределениеНДС.НачалоПериода, КВАРТАЛ) <> НАЧАЛОПЕРИОДА(ИсправлениеРаспределениеНДС.Дата, КВАРТАЛ)
	|
	|ГДЕ
	|	ЕСТЬNULL(НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.СуммаБезНДС, 1) <> 0
	|	ИЛИ ЕСТЬNULL(НДСЗаписиКнигиПокупокПоНДСПредъявленныйСводно.НДС, 1) <> 0
...
Функция РассчитатьНДСЗаписиКнигиПокупокСторноПоИсправленнымСчетамФактурамВыданнымНаАванс(ПараметрыРасчета)
	...
	|	НДСЗаписиКнигиПокупок.СторнирующаяЗаписьДопЛиста КАК СторнирующаяЗаписьДопЛиста,
	|	НДСЗаписиКнигиПокупок.ДоговорКонтрагента      КАК ДоговорКонтрагента,
	|	НДСЗаписиКнигиПокупок.ИсправленныйСчетФактура КАК ИсправленныйСчетФактура,
	|	СУММА(НДСЗаписиКнигиПокупок.СуммаБезНДС)      КАК СуммаБезНДС,
	|	СУММА(НДСЗаписиКнигиПокупок.НДС)              КАК НДС,
	...
	|			КОГДА НАЧАЛОПЕРИОДА(НДСЗаписиКнигиПокупок.Период, Квартал) < 
	|					НАЧАЛОПЕРИОДА(ИсправленныеСчетаФактуры.Период, Квартал)
	|				ТОГДА НАЧАЛОПЕРИОДА(НДСЗаписиКнигиПокупок.Период, День)
	|		КОНЕЦ КАК КорректируемыйПериод,
	|		ЛОЖЬ                                          КАК СторнирующаяЗаписьДопЛиста,
	|		НДСЗаписиКнигиПокупок.ДоговорКонтрагента      КАК ДоговорКонтрагента,
	|		НДСЗаписиКнигиПокупок.ИсправленныйСчетФактура КАК ИсправленныйСчетФактура,
	...
	|	НДСЗаписиКнигиПокупок.СторнирующаяЗаписьДопЛиста,
...
Функция РассчитатьНДСЗаписиКнигиПокупокПоАвансуПолученному(ПараметрыРасчета)
	...
	|	НДСАвансыПолученныеЗачеты.Исправление КАК Исправление,
	|	НДСАвансыПолученныеЗачеты.ДатаЗачетаАванса КАК ДатаЗачетаАванса,
	...
	|	
	|ВЫБРАТЬ
	|	НДСАвансыПолученныеСторно.Период,
	|	НДСАвансыПолученныеСторно.Регистратор,
	|		НДСАвансыПолученныеСторно.Организация,
	|	НДСАвансыПолученныеСторно.ДокументЗачетаАванса,
	|	НДСАвансыПолученныеСторно.Контрагент,
	|	НДСАвансыПолученныеСторно.ДокументОплаты,
	|	НДСАвансыПолученныеСторно.СтавкаНДС,
	|	ЛОЖЬ,
	|	НДСАвансыПолученныеСторно.ДатаЗачетаАванса,
	|	НДСАвансыПолученныеСторно.ИсправленныйСчетФактура,
	|	НДСАвансыПолученныеСторно.СуммаБезНДС,
	|	НДСАвансыПолученныеСторно.НДС,
	|	НДСАвансыПолученныеСторно.НДСУпр,
	|	НДСАвансыПолученныеСторно.Событие,
	|	НДСАвансыПолученныеСторно.НаправлениеДеятельности,
	|	НДСАвансыПолученныеСторно.Сторно,
	|	ИСТИНА КАК СторноЗачетов
	|ИЗ
	|	НДСАвансыПолученныеСторноЗачетов КАК НДСАвансыПолученныеСторно
	|
	...
	|	ДокументыОснованияСФ.ДокументОснование КАК ДокументЗачетаАванса,
	|	СчетФактураВыданный.Ссылка           КАК СчетФактура
	|	ПОМЕСТИТЬ ВТ_ДанныеПоСчетФактурамВыданным
	|ИЗ
	|	Документ.СчетФактураВыданный КАК СчетФактураВыданный
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.ДокументыОснования КАК ДокументыОснованияСФ
	|		ПО СчетФактураВыданный.Ссылка = ДокументыОснованияСФ.Ссылка
	|ГДЕ
	...
	|			ИСТИНА
	|		ИЗ
	|			ВТ_НДСАвансыПолученныеЗачеты КАК НДСАвансыПолученныеЗачеты
	|		ГДЕ
	|			НДСАвансыПолученныеЗачеты.ДокументЗачетаАванса = ДокументыОснованияСФ.ДокументОснование
	|		)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|	
	|ВЫБРАТЬ // Двухходовки
	|	СчетФактураВыданный.Организация КАК Организация,
	|	СчетФактураВыданный.Ссылка      КАК ДокументЗачетаАванса,
	|	СчетФактураВыданный.Ссылка      КАК СчетФактура
	|ИЗ
	|	Документ.СчетФактураВыданный КАК СчетФактураВыданный
	|ГДЕ
	...
	|			ВТ_НДСАвансыПолученныеЗачеты КАК НДСАвансыПолученныеЗачеты
	|		ГДЕ
	|			НДСАвансыПолученныеЗачеты.ДокументЗачетаАванса = СчетФактураВыданный.Ссылка
	|		)
	|	
	|ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	|	СчетФактураВыданный.Организация  КАК Организация,
	|	КорректировкаРеализации.Ссылка   КАК ДокументЗачетаАванса,
	|	СчетФактураВыданный.Ссылка       КАК СчетФактура
	|ИЗ
	|	Документ.СчетФактураВыданный КАК СчетФактураВыданный
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.ДокументыОснования КАК ДокументыОснованияСФ
	...
	|		ПО
	|			КорректировкаРеализации.ДокументОснование = ДокументыОснованияСФ.ДокументОснование
	|ГДЕ
	|	НЕ СчетФактураВыданный.Исправление
	|	И НЕ СчетФактураВыданный.Корректировочный
	|	И НЕ СчетФактураВыданный.Перевыставленный
	...
	|			ВТ_НДСАвансыПолученныеЗачеты КАК НДСАвансыПолученныеЗачеты
	|		ГДЕ
	|			НДСАвансыПолученныеЗачеты.ДокументЗачетаАванса = КорректировкаРеализации.Ссылка
	|		)
	|	
	|ИНДЕКСИРОВАТЬ ПО НАБОРАМ (
	|	(Организация,
	...
	|	ВЫБОР
	|		КОГДА (НДСАвансыПолученныеЗачеты.Исправление ИЛИ НДСАвансыПолученныеЗачеты.СторноЗачетов)
	...
	|			ТОГДА ""26""
	|		КОГДА &ВерсияКодовВидовОпераций >= 6
	|			И ЕСТЬNULL(ИсправленныеСчетаФактурыВыданныеАванс.Ссылка.КодВидаОперации, СчетаФактурыВыданныеАванс.Ссылка.КодВидаОперации) = ""52""
	|			ТОГДА ""54""
	|		ИНАЧЕ ""22""
	|	КОНЕЦ                                                   КАК КодВидаОперации,
	|	ИСТИНА                                                  КАК РегламентнаяОперация,
	...
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВТ_ДанныеПоСчетФактурамВыданным КАК ДанныеПоСчетФактурамВыданным
	|	ПО
	|		НДСАвансыПолученныеЗачеты.Организация = ДанныеПоСчетФактурамВыданным.Организация
	|		И НДСАвансыПолученныеЗачеты.ДокументЗачетаАванса = ДанныеПоСчетФактурамВыданным.ДокументЗачетаАванса
	|		
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|			РегистрСведений.ЖурналУчетаСчетовФактур КАК ДанныеСчетаФактуры
	|		ПО
	|			ДанныеПоСчетФактурамВыданным.Организация = ДанныеСчетаФактуры.Организация
	|			И ДанныеПоСчетФактурамВыданным.СчетФактура = ДанныеСчетаФактуры.СчетФактура
	|	
	|ГДЕ
...
Функция РассчитатьНДСЗаписиКнигиПокупокПоАвансуВыданному(ПараметрыРасчета)
	...
	|	ВЫБОР
	|		КОГДА (НДСАвансыВыданныеЗачеты.Исправление ИЛИ НДСАвансыВыданныеЗачеты.СторноЗачетов)
	...
	|		НДСАвансыВыданныеЗачеты.Регистратор КАК Регистратор,
	|		НДСАвансыВыданныеЗачеты.Организация КАК Организация,
	|		НДСАвансыВыданныеЗачеты.ДокументЗачетаАванса КАК ДокументЗачетаАванса,
	...
	|		НДСАвансыВыданныеЗачеты.ДатаЗачетаАванса КАК ДатаЗачетаАванса,
	|		НДСАвансыВыданныеЗачеты.ИсправленныйСчетФактура КАК ИсправленныйСчетФактура,
	|		НДСАвансыВыданныеЗачеты.СуммаБезНДС КАК СуммаБезНДС,
	|		НДСАвансыВыданныеЗачеты.НДС КАК НДС,
	|		НДСАвансыВыданныеЗачеты.НДСУпр КАК НДСУпр,
	|		НДСАвансыВыданныеЗачеты.Событие КАК Событие,
	|		НДСАвансыВыданныеЗачеты.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|		НДСАвансыВыданныеЗачеты.Сторно КАК Сторно,
	|		ЛОЖЬ КАК СторноЗачетов
	|	ИЗ
	...
	|	
	|	ВЫБРАТЬ
	|		НДСАвансыВыданныеСторноЗачетов.Период,
	|		НДСАвансыВыданныеСторноЗачетов.Регистратор,
	|		НДСАвансыВыданныеСторноЗачетов.Организация,
	|		НДСАвансыВыданныеСторноЗачетов.ДокументЗачетаАванса,
	|		НДСАвансыВыданныеСторноЗачетов.Контрагент,
	|		НДСАвансыВыданныеСторноЗачетов.ДокументОплаты,
	|		НДСАвансыВыданныеСторноЗачетов.СтавкаНДС,
	|		ЛОЖЬ,
	|		НДСАвансыВыданныеСторноЗачетов.ДатаЗачетаАванса,
	|		НДСАвансыВыданныеСторноЗачетов.ИсправленныйСчетФактура,
	|		НДСАвансыВыданныеСторноЗачетов.СуммаБезНДС,
	|		НДСАвансыВыданныеСторноЗачетов.НДС,
	|		НДСАвансыВыданныеСторноЗачетов.НДСУпр,
	|		НДСАвансыВыданныеСторноЗачетов.Событие,
	|		НДСАвансыВыданныеСторноЗачетов.НаправлениеДеятельности,
	|		НДСАвансыВыданныеСторноЗачетов.Сторно,
	|		ИСТИНА КАК СторноЗачетов
	|	ИЗ
	|		НДСАвансыВыданныеСторноЗачетов КАК НДСАвансыВыданныеСторноЗачетов) КАК НДСАвансыВыданныеЗачеты
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
...
Процедура СформироватьНДСЗаписиКнигиПродаж(ПараметрыРасчета)
	...
	РассчитатьНДСЗаписиКнигиПродажСторноПредварительно(ПараметрыРасчета);
	РассчитатьНДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно(ПараметрыРасчета);
	РассчитатьИтоговыеЗаписиКнигиПродажПоНДСПредъявленныйИСторно(ПараметрыРасчета);
...
Процедура РассчитатьНДСЗаписиКнигиПродажСторноПредварительно(ПараметрыРасчета)
	...
	|		РегистрСведений.РеестрДокументов КАК ПрошлыеСторноИсправительногоСчетаФактуры
	|	ПО
	|		НДСЗаписиКнигиПродаж.ИсправленныйСчетФактура <>  НЕОПРЕДЕЛЕНО
	|		И НДСЗаписиКнигиПродаж.ИсправленныйСчетФактура = ПрошлыеСторноИсправительногоСчетаФактуры.СторнируемыйДокумент
	|		И ПрошлыеСторноИсправительногоСчетаФактуры.СторноИсправление
	|		И НЕ ПрошлыеСторноИсправительногоСчетаФактуры.ДополнительнаяЗапись
	|		И ПрошлыеСторноИсправительногоСчетаФактуры.Проведен
	|		И ПрошлыеСторноИсправительногоСчетаФактуры.ДатаДокументаИБ < &НачалоПериода
	...
	|	И НЕ (ТИПЗНАЧЕНИЯ(НДСЗаписиКнигиПродаж.Регистратор) = ТИП(Документ.РаспределениеНДС)
	|			И ТИПЗНАЧЕНИЯ(ДокументыСторноРегистратор.Ссылка) <> ТИП(Документ.РаспределениеНДС))
	|	И НЕ (ДокументыСторноРегистратор.СторнируемыйДокумент ЕСТЬ NULL
	|		И ДокументыСторноСчетФактура.СторнируемыйДокумент ЕСТЬ NULL
	|		И ДокументыСторноИсправленныйСчетФактура.СторнируемыйДокумент ЕСТЬ NULL)
	...
	|			ТОГДА НДСЗаписиКнигиПродажОтборСторно.Период
	...
	|		КОГДА НАЧАЛОПЕРИОДА(НДСЗаписиКнигиПродажОтборСторно.Период, КВАРТАЛ) > НАЧАЛОПЕРИОДА(НДСЗаписиКнигиПродаж.Период, КВАРТАЛ)
	|			И НЕ НДСЗаписиКнигиПродаж.ЗаписьДополнительногоЛиста
	|			ТОГДА НДСЗаписиКнигиПродаж.Исправление
	|		ИНАЧЕ ИСТИНА
	...
	|ПОМЕСТИТЬ НДСЗаписиКнигиПродажСторноПредварительно
	|ИЗ
	|	РегистрНакопления.НДСЗаписиКнигиПродаж КАК НДСЗаписиКнигиПродаж
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция РассчитатьНДСЗаписиКнигиПродажСторно(ПараметрыРасчета)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	НДСЗаписиКнигиПродажСторноПредварительно.Приоритет                     КАК Приоритет,
	|	НДСЗаписиКнигиПродажСторноПредварительно.Период                        КАК Период,
	|	НДСЗаписиКнигиПродажСторноПредварительно.Регистратор                   КАК Регистратор,
	|	НДСЗаписиКнигиПродажСторноПредварительно.Организация                   КАК Организация,
	|	НДСЗаписиКнигиПродажСторноПредварительно.Покупатель                    КАК Покупатель,
	|	НДСЗаписиКнигиПродажСторноПредварительно.СчетФактура                   КАК СчетФактура,
	|	НДСЗаписиКнигиПродажСторноПредварительно.ВидЦенности                   КАК ВидЦенности,
	|	НДСЗаписиКнигиПродажСторноПредварительно.СтавкаНДС                     КАК СтавкаНДС,
	|	НДСЗаписиКнигиПродажСторноПредварительно.ДатаОплаты                    КАК ДатаОплаты,
	|	НДСЗаписиКнигиПродажСторноПредварительно.ДокументОплаты                КАК ДокументОплаты,
	|	НДСЗаписиКнигиПродажСторноПредварительно.Событие                       КАК Событие,
	|	НДСЗаписиКнигиПродажСторноПредварительно.ДатаСобытия                   КАК ДатаСобытия,
	|	НДСЗаписиКнигиПродажСторноПредварительно.ЗаписьДополнительногоЛиста    КАК ЗаписьДополнительногоЛиста,
	|	НДСЗаписиКнигиПродажСторноПредварительно.КорректируемыйПериод          КАК КорректируемыйПериод,
	|	НДСЗаписиКнигиПродажСторноПредварительно.СторнирующаяЗаписьДопЛиста    КАК СторнирующаяЗаписьДопЛиста,
	|	НДСЗаписиКнигиПродажСторноПредварительно.ДоговорКонтрагента            КАК ДоговорКонтрагента,
	|	НДСЗаписиКнигиПродажСторноПредварительно.ИсправленныйСчетФактура       КАК ИсправленныйСчетФактура,
	|	НДСЗаписиКнигиПродажСторноПредварительно.Исправление                   КАК Исправление,
	|	НДСЗаписиКнигиПродажСторноПредварительно.ДатаСчетаФактурыКомиссионера  КАК ДатаСчетаФактурыКомиссионера,
	|	НДСЗаписиКнигиПродажСторноПредварительно.СуммаБезНДС                   КАК СуммаБезНДС,
	|	НДСЗаписиКнигиПродажСторноПредварительно.НДС                           КАК НДС,
	|	НДСЗаписиКнигиПродажСторноПредварительно.НДСУпр                        КАК НДСУпр,
	|	НДСЗаписиКнигиПродажСторноПредварительно.ХозяйственнаяОперация         КАК ХозяйственнаяОперация,
	|	НДСЗаписиКнигиПродажСторноПредварительно.НомерДокументаОплаты          КАК НомерДокументаОплаты,
	|	НДСЗаписиКнигиПродажСторноПредварительно.ДатаДокументаОплаты           КАК ДатаДокументаОплаты,
	|	НДСЗаписиКнигиПродажСторноПредварительно.КодВидаОперации               КАК КодВидаОперации,
	|	НДСЗаписиКнигиПродажСторноПредварительно.РегламентнаяОперация          КАК РегламентнаяОперация,
	|	НДСЗаписиКнигиПродажСторноПредварительно.Сторно                        КАК Сторно,
	|	НДСЗаписиКнигиПродажСторноПредварительно.ВидДеятельностиНДС            КАК ВидДеятельностиНДС,
	|	НДСЗаписиКнигиПродажСторноПредварительно.НаправлениеДеятельности       КАК НаправлениеДеятельности
	|ПОМЕСТИТЬ НДСЗаписиКнигиПродажСторно
	|ИЗ
	|	НДСЗаписиКнигиПродажСторноПредварительно КАК НДСЗаписиКнигиПродажСторноПредварительно
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно КАК НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно
	|	ПО
	|		НДСЗаписиКнигиПродажСторноПредварительно.Период = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.Период
	|		И НДСЗаписиКнигиПродажСторноПредварительно.Регистратор = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.Регистратор
	|		И НДСЗаписиКнигиПродажСторноПредварительно.Организация = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.Организация
	|		И НДСЗаписиКнигиПродажСторноПредварительно.Покупатель = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.Покупатель
	|		И НДСЗаписиКнигиПродажСторноПредварительно.СчетФактура = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.СчетФактура
	|		И НДСЗаписиКнигиПродажСторноПредварительно.ВидЦенности = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.ВидЦенности
	|		И НДСЗаписиКнигиПродажСторноПредварительно.СтавкаНДС = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.СтавкаНДС
	|		И НДСЗаписиКнигиПродажСторноПредварительно.ИсправленныйСчетФактура = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.ИсправленныйСчетФактура
	|		И НДСЗаписиКнигиПродажСторноПредварительно.Событие = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.Событие
	|		И НДСЗаписиКнигиПродажСторноПредварительно.ВидДеятельностиНДС = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.ВидДеятельностиНДС
	|		И НДСЗаписиКнигиПродажСторноПредварительно.НаправлениеДеятельности = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.НаправлениеДеятельности
	|		И ТИПЗНАЧЕНИЯ(НДСЗаписиКнигиПродажСторноПредварительно.Регистратор) = ТИП(Документ.РаспределениеНДС)
	|ГДЕ
	|	ЕСТЬNULL(НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.СуммаБезНДС, 1) <> 0
	|	ИЛИ ЕСТЬNULL(НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.НДС, 1) <> 0
	|	ИЛИ ЕСТЬNULL(НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.НДСУпр, 1) <> 0
	|";
...
Процедура РассчитатьНДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно(ПараметрыРасчета)
	...
	|	НДСПредъявленный.ВидДеятельностиНДС                          КАК ВидДеятельностиНДС,
	|	НДСПредъявленный.НаправлениеДеятельности                     КАК НаправлениеДеятельности
	|ПОМЕСТИТЬ НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно
	|ИЗ
...
Функция РассчитатьНДСЗаписиКнигиПродажПоНДСПредъявленный(ПараметрыРасчета)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.Приоритет                       КАК Приоритет,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.Период                          КАК Период,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.Регистратор                     КАК Регистратор,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.Организация                     КАК Организация,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.Покупатель                      КАК Покупатель,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.СчетФактура                     КАК СчетФактура,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.ВидЦенности                     КАК ВидЦенности,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.СтавкаНДС                       КАК СтавкаНДС,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.ДатаОплаты                      КАК ДатаОплаты,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.ДокументОплаты                  КАК ДокументОплаты,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.Событие                         КАК Событие,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.ДатаСобытия                     КАК ДатаСобытия,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.ЗаписьДополнительногоЛиста      КАК ЗаписьДополнительногоЛиста,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.КорректируемыйПериод            КАК КорректируемыйПериод,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.ДоговорКонтрагента              КАК ДоговорКонтрагента,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.СторнирующаяЗаписьДопЛиста      КАК СторнирующаяЗаписьДопЛиста,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.ИсправленныйСчетФактура         КАК ИсправленныйСчетФактура,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.Исправление                     КАК Исправление,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.ДатаСчетаФактурыКомиссионера    КАК ДатаСчетаФактурыКомиссионера,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.СуммаБезНДС                     КАК СуммаБезНДС,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.НДС                             КАК НДС,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.НДСУпр                          КАК НДСУпр,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.ХозяйственнаяОперация           КАК ХозяйственнаяОперация,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.НомерДокументаОплаты            КАК НомерДокументаОплаты,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.ДатаДокументаОплаты             КАК ДатаДокументаОплаты,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.КодВидаОперации                 КАК КодВидаОперации,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.РегламентнаяОперация            КАК РегламентнаяОперация,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.Сторно                          КАК Сторно,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.ВидДеятельностиНДС              КАК ВидДеятельностиНДС,
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.НаправлениеДеятельности         КАК НаправлениеДеятельности
	|ПОМЕСТИТЬ НДСЗаписиКнигиПродажПоНДСПредъявленный
	|ИЗ
	|	НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно КАК НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно КАК НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно
	|	ПО
	|		НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.Период = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.Период
	|		И НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.Регистратор = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.Регистратор
	|		И НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.Организация = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.Организация
	|		И НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.Покупатель = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.Покупатель
	|		И НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.СчетФактура = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.СчетФактура
	|		И НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.ВидЦенности = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.ВидЦенности
	|		И НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.СтавкаНДС = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.СтавкаНДС
	|		И НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.ИсправленныйСчетФактура = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.ИсправленныйСчетФактура
	|		И НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.ВидДеятельностиНДС = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.ВидДеятельностиНДС
	|		И НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.НаправлениеДеятельности = НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.НаправлениеДеятельности
	|		И ТИПЗНАЧЕНИЯ(НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.Регистратор) = ТИП(Документ.РаспределениеНДС)
	|ГДЕ
	|	ЕСТЬNULL(НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.СуммаБезНДС, 1) <> 0
	|	ИЛИ ЕСТЬNULL(НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.НДС, 1) <> 0
	|	ИЛИ ЕСТЬNULL(НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно.НДСУпр, 1) <> 0
...
Процедура РассчитатьИтоговыеЗаписиКнигиПродажПоНДСПредъявленныйИСторно(ПараметрыРасчета)
	...
	|	ВложенныйЗапрос.ВидДеятельностиНДС                          КАК ВидДеятельностиНДС,
	|	ВложенныйЗапрос.НаправлениеДеятельности                     КАК НаправлениеДеятельности
	|ПОМЕСТИТЬ НДСЗаписиКнигиПродажПоНДСПредъявленныйСводно
	|ИЗ
	|	(ВЫБРАТЬ
	|		НДСЗаписиКнигиПродажСторноПредварительно.Период                     КАК Период,
	|		НДСЗаписиКнигиПродажСторноПредварительно.Регистратор                КАК Регистратор,
	...
	|		НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно.НаправлениеДеятельности    КАК НаправлениеДеятельности
	|	ИЗ
	|		НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно КАК НДСЗаписиКнигиПродажПоНДСПредъявленныйПредварительно) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
...
Функция РассчитатьНДСЗаписиКнигиПродажПоАвансуВыданному(ПараметрыРасчета)
	...
	|	НДСАвансыВыданныеЗачеты.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	НДСАвансыВыданныеЗачеты.Сторно КАК Сторно,
	|	ЛОЖЬ КАК СторноРасчетов
	|ПОМЕСТИТЬ ВТ_НДСАвансыВыданныеЗачеты
	|ИЗ
	|	НДСАвансыВыданныеЗачеты КАК НДСАвансыВыданныеЗачеты
	|	
	|ОБЪЕДИНИТЬ ВСЕ
	|	
	|ВЫБРАТЬ
	|	НДСАвансыВыданныеСторно.Период,
	|	НДСАвансыВыданныеСторно.Регистратор,
	|	НДСАвансыВыданныеСторно.Организация,
	|	НДСАвансыВыданныеСторно.ДокументЗачетаАванса,
	|	НДСАвансыВыданныеСторно.Контрагент,
	|	НДСАвансыВыданныеСторно.ДокументОплаты,
	|	НДСАвансыВыданныеСторно.СтавкаНДС,
	|	ЛОЖЬ,
	|	НДСАвансыВыданныеСторно.ДатаЗачетаАванса,
	|	НДСАвансыВыданныеСторно.ИсправленныйСчетФактура,
	|	НДСАвансыВыданныеСторно.СуммаБезНДС,
	|	НДСАвансыВыданныеСторно.НДС,
	|	НДСАвансыВыданныеСторно.НДСУпр,
	|	НДСАвансыВыданныеСторно.Событие,
	|	НДСАвансыВыданныеСторно.НаправлениеДеятельности,
	|	НДСАвансыВыданныеСторно.Сторно,
	|	ИСТИНА
	|ИЗ
	|	НДСАвансыВыданныеСторноЗачетов КАК НДСАвансыВыданныеСторно
	|	
	|ИНДЕКСИРОВАТЬ ПО
	| Организация,
	| ДокументОплаты
	...
	|	КОНЕЦ                                                                         КАК ВидЦенности,
	|	ВЫРАЗИТЬ(НДСАвансыВыданныеЗачеты.СтавкаНДС КАК Справочник.СтавкиНДС).ПеречислениеСтавкаНДС КАК СтавкаНДС,
	|	РеквизитыДокументаОплаты.Дата                                                 КАК ДатаОплаты,
	|	НДСАвансыВыданныеЗачеты.ДокументОплаты                                        КАК ДокументОплаты,
	|	ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПродажи.ВосстановлениеНДС)                  КАК Событие,
	|	НДСАвансыВыданныеЗачеты.Период                                                КАК ДатаСобытия,
	|	ВЫБОР
	|		КОГДА (НДСАвансыВыданныеЗачеты.Исправление ИЛИ НДСАвансыВыданныеЗачеты.СторноРасчетов)
	|			И НАЧАЛОПЕРИОДА(НДСАвансыВыданныеЗачеты.Период, Квартал) > НАЧАЛОПЕРИОДА(НДСАвансыВыданныеЗачеты.ДатаЗачетаАванса, Квартал)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЗаписьДополнительногоЛиста,
	...
	|ИЗ
	|	ВТ_НДСАвансыВыданныеЗачеты КАК НДСАвансыВыданныеЗачеты
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.ДанныеПервичныхДокументов КАК РеквизитыДокументаОплаты
	|	ПО
...
Функция РассчитатьНДСЗаписиКнигиПродажСохранение(ПараметрыРасчета, ИменаТаблицНДСЗаписиКнигиПродаж)
	...
	|	(НДСЗаписиКнигиПродаж.Период < &НачалоПериода
	|			И НЕ НДСЗаписиКнигиПродаж.Сторно
	|			И НЕ НДСЗаписиКнигиПродаж.Исправление)
...
// Процедура заполняет список выбора видов транспорта на таможне.
// 
// Параметры:
//  СписокЗначений - СписокЗначений Из Строка - список значений  для заполнения видов транспорта на таможне.
//
Процедура ЗаполнитьСписокВыбораВидовТранспорта(СписокЗначений) Экспорт
	
	// Список взят из макета СпискиВыбора2015Кв4 отчета РегламентированныйОтчетРеестрНДСПриложение5 БРО.
	// Здесь дублирован специально, чтобы не было лишнего серверного вызова.
	...
	СписокЗначений.Добавить("32",
		НСтр("ru = '32 - Состав транспортных средств (тягач с прицепом (-ами) и полуприцепом (-ами) )'"));
	...
	СписокЗначений.Добавить("90",
		НСтр("ru = '90 - Транспортное средство, перемещающееся в качестве товара своим ходом'"));
	СписокЗначений.Добавить("99", НСтр("ru = '99 - Прочие'"));
	
КонецПроцедуры

// Инициализурует пустой российский адрес в качестве соответствия.
// 
// Параметры:
//  СсылкаНаОбъект - СправочникСсылка.Контрагенты, СправочникСсылка.Организации - владельцы контактной информации.
...
//
// Возвращаемое значение:
//  Соответствие Из КлючИЗначение - российский адрес в виде соответствия:
//   * Ключ - Строка - наименование реквизита в структуре российского адреса.
//   * Значение - Строка - значение реквизита в структуре российского адреса.
//
...
// Иницализирует пустой российский адрес в качестве соответствия.
//
// Возвращаемое значение:
//  Соответствие Из КлючИЗначение - российский адрес в виде соответствия:
//   * Ключ - Строка - наименование реквизита в структуре российского адреса.
//   * Значение - Строка - значение реквизита в структуре российского адреса.
//
Функция ПустойРоссийскийАдрес() Экспорт
...
// Заполняет информацию о населенном пункте в российского адресе, представленным в виде соответствия.
// Параметры:
//  РоссийскийАдрес - Соответствие Из КлючИЗначение - российский адрес в виде соответствия:
//   * Ключ - Строка - наименование реквизита в структуре российского адреса.
//   * Значение - Строка - значение реквизита в структуре российского адреса.
//
...
// Дополнить информацией о счетах фактурах на отгрузку.
// 
// Параметры:
//  ДополнительнаяИнформация - ТаблицаЗначений - см Отчеты.КнигаПокупок.НовыйДополнительнаяИнформацияПоСчетуФактуре.
//  СтруктураПараметров - Структура:
//  * Организация -СправочникСсылка.Организации.
// 	* НачалоПериодаДата - Дата.
// 	* КонецПериода - Дата.
// 	* ФормироватьДополнительныеЛисты - Булево.
// 	* ДополнительныеЛистыЗаТекущийПериод - Булево.
// 	* ГруппироватьПоКонтрагентам - Булево.
// 	* КонтрагентДляОтбора -СправочникСсылка.Контрагенты.
...
// 	* ЗаполнениеДокумента - Булево.
// 	* ЗаполнениеДекларации - Булево.
// 	* ФормироватьТабличныйДокумент - Булево.
// 	* ЭтоКнигаПокупок - Булево.
//  Запрос - Запрос.
...
Функция ТекстЗапросаДанныеОСчетахФактурахНаОтгрузку()
	...
	|	И (НДСЗаписиКнигиПокупок.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|			ИЛИ НДСЗаписиКнигиПокупок.КорректируемыйПериод МЕЖДУ &НачалоПериода И &КонецПериода)
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаИнформацияПоТаможеннымДекларациямПродажи()
	...
	|	1 КАК КоличествоЭлементов
	|ИЗ
	|	Документ.ОтгрузкаТоваровКлиенту.ВидыЗапасов КАК ТаблицаТоваров
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СписокСчетовФактур КАК СписокСчетовФактур
	|		ПО ТаблицаТоваров.Ссылка = СписокСчетовФактур.СчетФактура
	|ГДЕ
	|	ТаблицаТоваров.НомерГТД <> ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
...
Функция ТекстыЗапросовЗаполнитьСчетФактуруПолученнуюВОтчетеКомитентуОЗакупках()
	...
	"ВЫБРАТЬ
	|	ВтИсточникДанных.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|	ЕСТЬNULL(СчетаФактуры.Ссылка, ЗНАЧЕНИЕ(Документ.СчетФактураПолученный.ПустаяСсылка)) КАК Ссылка
	|ИЗ
	|	ВтИсточникДанных КАК ВтИсточникДанных
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаДокументовПриобретения КАК ТаблицаДокументовПриобретения
	|		ПО ВтИсточникДанных.ИдентификаторСтрокиВТ = ТаблицаДокументовПриобретения.ИдентификаторСтрокиВТ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураПолученный.ДокументыОснования КАК СчетаФактуры
	|		ПО ТаблицаДокументовПриобретения.ДокументПриобретения = СчетаФактуры.ДокументОснование
	|		И (СчетаФактуры.Ссылка.Проведен)
	|		И ТаблицаДокументовПриобретения.Организация В (СчетаФактуры.Ссылка.Организация, НЕОПРЕДЕЛЕНО)";
...
#Область ЗачетыАвансовПолученныхСчетамиФактурами

Процедура УстановитьПараметрыЗапросаЗачетыАвансовПолученныхСчетамиФактурами(Запрос, ПараметрыРасчета)
	...
	Запрос.УстановитьПараметр("ВключитьОтборПоКонтрагенту",        (ПараметрыРасчета.Свойство("РасчетныйДокумент") И ЗначениеЗаполнено(ПараметрыРасчета.РасчетныйДокумент)));
	Запрос.УстановитьПараметр("РасчетныйДокумент",                ?(ПараметрыРасчета.Свойство("РасчетныйДокумент") И ЗначениеЗаполнено(ПараметрыРасчета.РасчетныйДокумент), ПараметрыРасчета.РасчетныйДокумент, Неопределено));
	
КонецПроцедуры

#КонецОбласти

#Область ОбновлениеСчетовФактурВыданных

Процедура ЗарегистрироватьЗаданияАктуализацииСчетаФактуры(СчетаФактурыВыданные)
	
	ВидЗадания = Перечисления.ЗаданияУниверсальнойОчереди.ОбновлениеСчетовФактурВыданных;
	
	Для Каждого Документ Из СчетаФактурыВыданные Цикл
		УниверсальнаяОчередьОперативныхЗаданий.ДобавитьЗадание(Документ, ВидЗадания);
	КонецЦикла;
		
КонецПроцедуры

Функция ТекстЗапроса_ОбновлениеСчетовФактурВыданных()
	...
	|ГДЕ
	|	СчетФактураВыданныйДокументыОснования.Ссылка В (&СчетФактуры)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(РеестрДокументов.Ссылка) = ТИП(Документ.ЗаписьКнигиПродаж)
	...
	|ПОМЕСТИТЬ ВТ_ДанныеДокументовОснований
	|ИЗ
	|	РегистрСведений.РеестрДокументов КАК РеестрДокументов
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаписьКнигиПродаж КАК ЗаписьКнигиПродаж
	|		ПО (РеестрДокументов.Ссылка = ЗаписьКнигиПродаж.Ссылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ДокументыОснования КАК ВТ_ДокументыОснования
	|		ПО (РеестрДокументов.Ссылка = ВТ_ДокументыОснования.ДокументОснование)
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ДанныеДокументовОснований.СчетФактура КАК СчетФактура,
	|	КОЛИЧЕСТВО(*) КАК КоличествоРазныхДокументовОснований
	|ПОМЕСТИТЬ ВТ_РазныеДокументыОснования
	|ИЗ
	|	ВТ_ДанныеДокументовОснований КАК ВТ_ДанныеДокументовОснований
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ДанныеДокументовОснований.СчетФактура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТ_ДокументыОснования.СчетФактура КАК СчетФактура,
	|	ВТ_РазныеДокументыОснования.КоличествоРазныхДокументовОснований КАК КоличествоРазныхДокументовОснований
	|ПОМЕСТИТЬ ВТ_ДокументыКОбновлению
	|ИЗ
	|	ВТ_ДокументыОснования КАК ВТ_ДокументыОснования
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ДанныеДокументовОснований КАК ВТ_ДанныеДокументовОснований
	|		ПО ВТ_ДокументыОснования.СчетФактура = ВТ_ДанныеДокументовОснований.СчетФактура
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_РазныеДокументыОснования КАК ВТ_РазныеДокументыОснования
	|		ПО ВТ_ДокументыОснования.СчетФактура = ВТ_РазныеДокументыОснования.СчетФактура
	|ГДЕ
	|	ВТ_ДокументыОснования.СчетФактураПроведен
	|	И (ВТ_ДокументыОснования.Склад <> ВТ_ДанныеДокументовОснований.Склад
	|	ИЛИ ВТ_ДокументыОснования.Партнер <> ВТ_ДанныеДокументовОснований.Партнер
	|	ИЛИ ВТ_ДокументыОснования.Договор <> ВТ_ДанныеДокументовОснований.Договор
	|	ИЛИ ВТ_ДокументыОснования.НаправлениеДеятельности <> ВТ_ДанныеДокументовОснований.НаправлениеДеятельности)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ДокументыКОбновлению.СчетФактура КАК СчетФактура,
	|	ВТ_ДокументыКОбновлению.КоличествоРазныхДокументовОснований КАК КоличествоРазныхДокументовОснований,
	|	ЕСТЬNULL(ВТ_ДанныеДокументовОснований.Склад, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяССылка)) КАК Склад,
	|	ЕСТЬNULL(ВТ_ДанныеДокументовОснований.Партнер, ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяССылка)) КАК Партнер,
	|	ЕСТЬNULL(ВТ_ДанныеДокументовОснований.НаправлениеДеятельности,
	|		ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяССылка)) КАК НаправлениеДеятельности,
	|	ЕСТЬNULL(ВТ_ДанныеДокументовОснований.Договор, ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяССылка)) КАК Договор
	|ИЗ
	|	ВТ_ДокументыКОбновлению КАК ВТ_ДокументыКОбновлению
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ДанныеДокументовОснований КАК ВТ_ДанныеДокументовОснований
	|		ПО ВТ_ДокументыКОбновлению.СчетФактура = ВТ_ДанныеДокументовОснований.СчетФактура
	|		И (ВТ_ДокументыКОбновлению.КоличествоРазныхДокументовОснований = 1)";
	
	Возврат Текст;
	
КонецФункции

Процедура ДополнитьОшибкиОбработки(ИдентификаторыНеОбработанныхЗаписей, Задания, Знач СчетФактура, Знач ТекстОшибкиОбработки)

	Поля = Новый Структура("Объект", НСтр("ru='Документ'"));
	Отбор = Новый Структура("Объект");
		
	Отбор.Объект = СчетФактура;
	НайденныеСтроки = Задания.НайтиСтроки(Отбор); 
	
	Для Каждого Строка Из НайденныеСтроки Цикл
		ТекстОшибки = ОтложенныеЗадания.ТекстОшибкиВыполнения(Поля, Строка, ТекстОшибкиОбработки);
		ИдентификаторыНеОбработанныхЗаписей.Вставить(Строка.ИдентификаторЗаписи, ТекстОшибки);
	КонецЦикла;
