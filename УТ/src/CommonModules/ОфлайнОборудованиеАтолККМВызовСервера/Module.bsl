	...
	Пока ПродолжатьЧтениеФайла Цикл
		...
		Иначе
			...
			Если ТранзакцияЗаполнена(СтрЗаменить(Строка, ";", Символы.ПС), Транзакция, ФорматОбмена, ТекстОшибки) Тогда
				...
				ИначеЕсли Транзакция.Тип = 4 ИЛИ Транзакция.Тип = 14 Тогда
					
					//НалоговыеСтавки
					ДобавитьНалогиВЧек(ТекущаяСмена, Смены, Транзакция, ФорматОбмена);
				...
				Если Чек.Закрыт Тогда
					...
					Если Чек.ТипОперацииККМ = "Продажа" ИЛИ Чек.ТипОперацииККМ = "Возврат" Тогда
						...
						Для Каждого Товар Из Чек.Товары Цикл
							...
							Если Товар.Свойство("СтавкаНДС") Тогда
								ТоварЧекаККМ.СтавкаНДС  = СтавкаНДСПоКоду(Товар.СтавкаНДС);
							Иначе
								ТоварЧекаККМ.СтавкаНДС  = СтавкаНДСПоКоду(Неопределено);
							КонецЕсли;
...
Процедура ДобавитьТранзакциюВЧек(Смена, Смены, Транзакция, ФорматОбмена, Оплата = Ложь)
	...
	Иначе
		...
		Иначе
			...
			НовыйТовар.Вставить("СтавкаНДС",			Неопределено);
...
Функция НомераНалоговНаККМПоУмолчанию() Экспорт
	...
	Налоги.Вставить("НомерНалога22", 11);
...
Функция СпискиНалоговИКомбинацийНалогов(НалогиНаККМ)
	...
	// 22%
	СтавкаНалога22 = Новый Структура;
	СтавкаНалога22.Вставить("Код", 11);
	СтавкаНалога22.Вставить("Текст", НСтр("ru = 'НДС 22%'"));
	СтавкаНалога22.Вставить("Значение", 22);
	СтавкаНалога22.Вставить("НомерНалогаККМ", НалогиНаККМ.НомерНалога22);
	НалогНДС.Ставки.Добавить(СтавкаНалога22);
...
Процедура ДобавитьНалогиВЧек(Смена, Смены, Транзакция, ФорматОбмена)
	...
	Если НайденныеТовары.Количество() > 0 Тогда
		Товар = НайденныеТовары.Получить(НайденныеТовары.Количество()-1);
		Если Товар.Свойство("СтавкаНДС") Тогда
			Товар.СтавкаНДС = Транзакция.Поле11;
		КонецЕсли;
	Иначе
		Возврат;
	КонецЕсли;
...
Функция СтавкаНДСПоКоду(КодСтавкиНДС)
	
	Если КодСтавкиНДС = 1 Тогда
		СтавкаНДС = "20";
	ИначеЕсли КодСтавкиНДС = 2 Тогда
		СтавкаНДС = "10";
	ИначеЕсли КодСтавкиНДС = 3 Тогда
		СтавкаНДС = "none";
	ИначеЕсли КодСтавкиНДС = 4 Тогда
		СтавкаНДС = "0";
	ИначеЕсли КодСтавкиНДС = 5 Тогда
		СтавкаНДС = "10/110";
	ИначеЕсли КодСтавкиНДС = 6 Тогда
		СтавкаНДС = "20/120";
	ИначеЕсли КодСтавкиНДС = 7 Тогда
		СтавкаНДС = "5";
	ИначеЕсли КодСтавкиНДС = 8 Тогда
		СтавкаНДС = "7";
	ИначеЕсли КодСтавкиНДС = 9 Тогда
		СтавкаНДС = "5/105";
	ИначеЕсли КодСтавкиНДС = 10 Тогда
		СтавкаНДС = "7/107";
	ИначеЕсли КодСтавкиНДС = 11 Тогда
		СтавкаНДС = "22";
	ИначеЕсли КодСтавкиНДС = 12 Тогда
		СтавкаНДС = "22/122";
	Иначе
		СтавкаНДС = "22";
	КонецЕсли; 
...
Процедура ДополнитьФайл(Файл, ВыходныеПараметры, Параметры, Знач ДанныеДляЗаполнения = Неопределено)
	...
	Если Уровень = "Номенклатура" Тогда
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Источник.ЭтоМаркированнаяПродукция) И Источник.ЭтоМаркированнаяПродукция Тогда
				Если ФорматОбмена = 8 Тогда
					...
					ИначеЕсли Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.ТехническиеСредстваРеабилитации") Тогда
						ДанныеДляЗаполнения.Вставить("КодТипаНоменклатуры", 24);
					ИначеЕсли Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.БезалкогольноеПиво") Тогда
						ДанныеДляЗаполнения.Вставить("КодТипаНоменклатуры", 25);
					ИначеЕсли Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.Морепродукты") Тогда
						ДанныеДляЗаполнения.Вставить("КодТипаНоменклатуры", 26);
					ИначеЕсли Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.Велосипеды") Тогда
						ДанныеДляЗаполнения.Вставить("КодТипаНоменклатуры", 27);
					ИначеЕсли Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.ВетеринарныеПрепараты") Тогда
						ДанныеДляЗаполнения.Вставить("КодТипаНоменклатуры", 28);
					ИначеЕсли Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.КормаДляЖивотных")
						ИЛИ Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.КормаДляЖивотныхВлажные") Тогда
						ДанныеДляЗаполнения.Вставить("КодТипаНоменклатуры", 29);
					ИначеЕсли Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.РастительныеМасла") Тогда
						ДанныеДляЗаполнения.Вставить("КодТипаНоменклатуры", 30);
					ИначеЕсли Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.АлкогольнаяПродукцияДо9Процентов") Тогда
						ДанныеДляЗаполнения.Вставить("КодТипаНоменклатуры", 31);
					ИначеЕсли Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.КонсервированнаяПродукция") Тогда
						ДанныеДляЗаполнения.Вставить("КодТипаНоменклатуры", 32);
					ИначеЕсли Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.Бакалея") Тогда
						ДанныеДляЗаполнения.Вставить("КодТипаНоменклатуры", 33);
					ИначеЕсли Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.МоторныеМасла") Тогда
						ДанныеДляЗаполнения.Вставить("КодТипаНоменклатуры", 34);
					ИначеЕсли Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.ИгрыИИгрушкиДляДетей") Тогда
						ДанныеДляЗаполнения.Вставить("КодТипаНоменклатуры", 36);
					Иначе
		...
		МераКоличестваПредметаРасчета = ОборудованиеЧекопечатающиеУстройства.МераКоличестваПредметаРасчетаПоКодуЕдиницыИзмерения(Источник.КодЕдиницыИзмерения);
