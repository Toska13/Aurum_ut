...
Процедура ДоступныеВерсииУниверсальногоФормата(ВерсииФормата) Экспорт
	...
	ВерсииФормата.Вставить("1.21", МенеджерОбменаЧерезУниверсальныйФормат);
...
//-- Локализация
	...
	Если Параметры.ПроверятьНаИспользованиеОтгрузкиБезПереходаПраваСобственности Тогда
		
		ИспользоватьСтатусыРеализаций = Ложь;
		...
		Если Не ПолучитьФункциональнуюОпцию("ИспользоватьОтгрузкуБезПереходаПраваСобственности2_0") Тогда
			...
			|	ИСТИНА
			|ИЗ
			|	Документ.РеализацияТоваровУслуг КАК ТаблицаРеализаций
			|ГДЕ
			|	ТаблицаРеализаций.ХозяйственнаяОперация =
			...
			Если Не РезультатЗапроса.Пустой() Тогда
				...
				ИспользоватьСтатусыРеализаций = Истина;
				
			КонецЕсли;
			
		КонецЕсли;
		
		Если Не ПолучитьФункциональнуюОпцию("ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5")
		...
		Если Не ПолучитьФункциональнуюОпцию("ИспользоватьСтатусыРеализацийТоваровУслуг")
			И ИспользоватьСтатусыРеализаций Тогда
...
Процедура ПриВыгрузкеДанных(СтандартнаяОбработка,
	...
	РегистрыСведений.УточненияПринадлежностиЗапасовТорговыхТочек.ЗаполнениеУточненияПринадлежностиЗапасовТорговыхТочек(Получатель);
