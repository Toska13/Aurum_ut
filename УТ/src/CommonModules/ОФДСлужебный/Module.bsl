...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("КассыНастройкиПодключения", Истина);
	Методы.Вставить("ПолучитьУчастниковОФД", Истина);
...
Функция ПереопределитьСообщениеПользователю(
	...
	ИначеЕсли КодОшибки = КодОшибкиНеверныйФорматЗапроса()
		И КодОшибкиСервиса = "PERIOD_EXCEED" Тогда
		Возврат НСтр("ru = 'Превышен максимально допустимый период запроса.'");
	ИначеЕсли КодОшибки = КодОшибкиНеверныйФорматЗапроса()
...
Функция ЗаголовокАвторизацииОФД(
	...
	ТипАвторизации = "";
	ДанныеАутентификации.Свойство("type", ТипАвторизации);
	...
	ИначеЕсли ПустаяСтрока(ТипАвторизации) Тогда
		Результат.Ошибка = Истина;
		Результат.ИнформацияОбОшибке = НСтр("ru = 'Не заполнены данные авторизации.'");
	Иначе
...
Функция НовыйОписаниеРеквизита(
	...
	ОписаниеПоля.Вставить(
		...
		Тип.СодержитТип(
			Тип("СправочникСсылка.НастройкиПодключенияКОФД")));
...
Функция ЗначениеНастроекАутентификации(НастройкаПодключения)
	...
	Данные = ПолучитьНастройкиАутентификации(НастройкаПодключения);
	...
	Иначе
		...
		РезультатПолучения = ПолучитьУчастниковОФД();
		
		Если Не ПустаяСтрока(РезультатПолучения.КодОшибки) Тогда
			ВызватьИсключение НСтр("ru = 'Не удалось определить тип авторизации участника ОФД.'");
		КонецЕсли;
		
		ДанныеУчастника = Неопределено;
		РезультатПолучения.ДанныеУчастников.Свойство(ИдентификаторУчастника, ДанныеУчастника);
		
		Если Не ЗначениеЗаполнено(ДанныеУчастника) Тогда
			ВызватьИсключение НСтр("ru = 'Не удалось определить тип авторизации участника ОФД.'")
		КонецЕсли;
		
		ТипАутентификации = ДанныеУчастника.ТипАутентификации;
	...
	Результат.Вставить("ТипАутентификации", ТипАутентификации);
...
Функция ПолучитьУчастниковОФД() Экспорт
	...
	РезультатВызова = ОФДСервис.ОперацияНастройкиУчастниковОФД();
...
Функция ПроверитьПараметрыПодключения(
	...
	РезультатПроверки = ОФДСервис.ОперацияПроверитьПараметрыПодключения(
		ПараметрыНастройкиПодключения,
...
Процедура ПроверитьПараметрыПодключенияОбъект(ПараметрыПроцедуры, АдресРезультата) Экспорт
	...
	ПараметрыАутентификации = ПолучитьНастройкиАутентификации(
...
Функция КассыНастройкиПодключения(ПараметрыПроцедуры) Экспорт
	...
	Возврат ОФДСервис.ОперацияСписокКасс(
...
//
// Возвращаемое значение:
//  Строка - результат обработки данных аутентификации.
//
