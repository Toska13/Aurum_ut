...
// 			** Статья - Массив из Строка - Имена элементов формы для выбора статьи (Например, ТоварыСтатьяРасходов, СтатьяРасходовБезРазбиения)
// 			** АналитикаРасходов - Массив из Строка - Имена элементов формы для выбора аналитики статьи расходов.
// 			** АналитикаДоходов - Массив из Строка - Имена элементов формы для выбора аналитики статьи доходов.
// 			** АналитикаАктивовПассивов - Массив из Строка - Имена элементов формы для выбора аналитики статьи активов/пассивов.
// 		
// 		* ТипСтатьи - Строка - Имя служебного реквизита формы, в котором хранится числовое значение типа статьи.
...
//			- СписокЗначений из Произвольный - Значение реквизита, при котором выбор статьи доступен.
//		* УсловияОбязательногоЗаполненияАналитикиВСтроках - Структура
...
// 	ПараметрыВыбора - Массив из см. ДоходыИРасходыСервер.ПараметрыВыбораСтатьиИАналитики -
...
// 	ПроверяемыеРеквизиты - Массив из Строка  - Массив проверяемых реквизитов объекта.
// 	ПараметрыВыбора - Массив из см. ДоходыИРасходыСервер.ПараметрыВыбораСтатьиИАналитики -
// 					- см. ДоходыИРасходыСервер.ПараметрыВыбораСтатьиИАналитики
//
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты, Знач ПараметрыВыбора) Экспорт
	...
	Для Каждого ПараметрыВыбора Из МассивПараметровВыбора Цикл
		...
		Если ЭтоТаблица
			...
			Для Каждого Строка Из Объект[ИмяТаблицы] Цикл // СтрокаТабличнойЧасти
...
Процедура ПриСозданииНаСервере(Форма, Знач ПараметрыВыбора) Экспорт
	...
	Для каждого Параметры Из ДанныеВыборСтатейИАналитик.МассивПараметровВыбора Цикл
		...
		Если ЗначениеЗаполнено(Параметры.ЭлементыФормы.АналитикаРасходов) Тогда
			...
			СписокУсловий = Параметры.УсловияОбязательногоЗаполненияАналитикиВСтроках;
...
Процедура ДополнитьТекстЗапросаСтатьяАналитикаРасходовПоУмолчанию(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		...
		Если Не ЭтоФайловаяБаза Тогда
			...
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтИсточникДанных КАК Товары
			...
			|				И ПрочиеРасходы.Организация = &Организация_АналитикаРасходов
			|				И ПрочиеРасходы.Подразделение = &Подразделение_АналитикаРасходов
			|				И ПрочиеРасходы.ХозяйственнаяОперация = &ХозяйственнаяОперация_АналитикаРасходов
			|ГДЕ
			|	ИСТИНА В
			|			(ВЫБРАТЬ ПЕРВЫЕ 1
			|				ИСТИНА
			...
			|						Отбор.КлючАналитики,
			|						&Организация_АналитикаРасходов,
			|						&Подразделение_АналитикаРасходов,
			|						&ХозяйственнаяОперация_АналитикаРасходов,
			...
			ТекстЗапроса = "ВЫБРАТЬ
			|	Товары.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
			|	ЕСТЬNULL(ПрочиеРасходы.СтатьяРасходов, ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)) КАК СтатьяРасходов,
			|	ЕСТЬNULL(ПрочиеРасходы.АналитикаРасходов, Неопределено) КАК АналитикаРасходов,
			|	ПрочиеРасходы.Период КАК Период
			|ИЗ 
			|	ВтИсточникДанных КАК Товары
			|	ЛЕВОЕ СОЕДИНЕНИЕ ВтСтатьяАналитикаРасходовПоУмолчаниюПромежуточная КАК СтатьиРасходов
			|		ПО Товары.ИдентификаторСтрокиВТ = СтатьиРасходов.ИдентификаторСтрокиВТ
		...
		Иначе
			...
			//Получение данных о времени последней выбранной аналитике номенклатуры
			ТекстЗапроса = "ВЫБРАТЬ
			|	Товары.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
			|	Товары.Номенклатура КАК Номенклатура,
			|	Товары.Характеристика КАК Характеристика,
			|	МАКСИМУМ(ПрочиеРасходы.Период) КАК Период
			|ПОМЕСТИТЬ ВтДатаПоследнейАналитикиНоменклатуры
			|ИЗ
			|	ВтИсточникДанных КАК Товары
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК ТаблицаАналитик
			|		ПО (ТаблицаАналитик.Номенклатура = Товары.Номенклатура)
			|			И (ТаблицаАналитик.Характеристика = Товары.Характеристика)
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходы КАК ПрочиеРасходы
			|		ПО (ТаблицаАналитик.КлючАналитики = ПрочиеРасходы.АналитикаУчетаНоменклатуры)
			|			И (ПрочиеРасходы.Организация = &Организация_АналитикаРасходов)
			|			И (ПрочиеРасходы.Подразделение = &Подразделение_АналитикаРасходов)
			|			И (ПрочиеРасходы.ХозяйственнаяОперация = &ХозяйственнаяОперация_АналитикаРасходов)
			|
			...
			Если Не ИспользуетсяХарактеристика Тогда
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Товары.Характеристика КАК Характеристика,", "ИСТИНА КАК Характеристика,");
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Товары.Характеристика,", "ИСТИНА,");
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТаблицаАналитик.Характеристика = Товары.Характеристика", "ИСТИНА");
			КонецЕсли;

			ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ВтДатаПоследнейАналитикиНоменклатуры");

			//Получение данных о последней выбранной аналитике номенклатуры
			ТекстЗапроса = "ВЫБРАТЬ
			|	Товары.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
			|	Товары.Номенклатура КАК Номенклатура,
			|	Товары.Характеристика КАК Характеристика,
			|	Товары.Период КАК Период,
			|	МАКСИМУМ(ПрочиеРасходы.АналитикаУчетаНоменклатуры) КАК АналитикаУчетаНоменклатуры
			|ПОМЕСТИТЬ ВтПоследняяАналитика
			|ИЗ
			|	ВтДатаПоследнейАналитикиНоменклатуры КАК Товары
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК ТаблицаАналитик
			|		ПО (ТаблицаАналитик.Номенклатура = Товары.Номенклатура)
			|			И (ТаблицаАналитик.Характеристика = Товары.Характеристика)
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходы КАК ПрочиеРасходы
			...
			|			И (ПрочиеРасходы.Подразделение = &Подразделение_АналитикаРасходов)
			|			И (ПрочиеРасходы.ХозяйственнаяОперация = &ХозяйственнаяОперация_АналитикаРасходов)
			|			И (ПрочиеРасходы.Период = Товары.Период)
			|
			|СГРУППИРОВАТЬ ПО
			|	Товары.ИдентификаторСтрокиВТ,
			|	Товары.Номенклатура,
			...
			ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ВтПоследняяАналитика");
			
			//Данные о последней статье расходов и аналитике расходов по выбранной номенклатуре и характеристике
			...
			|ИЗ
			|	ВтИсточникДанных КАК Товары
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВтПоследняяАналитика КАК ПоследняяАналитика
			|		ПО Товары.ИдентификаторСтрокиВТ = ПоследняяАналитика.ИдентификаторСтрокиВТ
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходы КАК ПрочиеРасходы
			|		ПО (ПрочиеРасходы.Период = ПоследняяАналитика.Период)
			|			И (ПрочиеРасходы.АналитикаУчетаНоменклатуры = ПоследняяАналитика.АналитикаУчетаНоменклатуры)
			|			И (ПрочиеРасходы.Организация = &Организация_АналитикаРасходов)
			|			И (ПрочиеРасходы.Подразделение = &Подразделение_АналитикаРасходов)
			|			И (ПрочиеРасходы.ХозяйственнаяОперация = &ХозяйственнаяОперация_АналитикаРасходов)";
		...
		ОписаниеЗапроса.ПараметрыЗапроса.Вставить("Организация_АналитикаРасходов", ПараметрыДействия.Организация);
		ОписаниеЗапроса.ПараметрыЗапроса.Вставить("Подразделение_АналитикаРасходов", ПараметрыДействия.Подразделение);
		ОписаниеЗапроса.ПараметрыЗапроса.Вставить("ХозяйственнаяОперация_АналитикаРасходов", ПараметрыДействия.ХозяйственнаяОперация);
...
Процедура ДополнитьТекстЗапросаЗначенияРеквизитовСтатьи(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
			"ОчиститьСтатьюАналитикуРасходов,ИнициализироватьСтатьюИАналитикуПослеДобавленияСтроки,ЗаполнитьСтатьюАналитикуРасходовНоменклатуры",
			СтруктураДействий,
			КэшированныеЗначения,
			ПараметрыДействия) Тогда
				
		Если ПараметрыДействия.Свойство("ПараметрыВыбораСтатейИАналитик") Тогда
			ПолеСтатья = ПараметрыДействия.ПараметрыВыбораСтатейИАналитик.Статья;
		Иначе
			ПолеСтатья = ПараметрыДействия.МассивПараметровВыбораСтатейИАналитик[0].Статья;
		КонецЕсли;
		
		ТекстЗапроса = 
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	ВтИсточникДанных.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
			|	ПланВидовХарактеристикСтатьиРасходов.ТипЗначения КАК ТипЗначения,
			|	ПланВидовХарактеристикСтатьиРасходов.КонтролироватьЗаполнениеАналитики КАК КонтролироватьЗаполнениеАналитики,
			|	ПланВидовХарактеристикСтатьиРасходов.АналитикаРасходовЗаказРеализация КАК АналитикаРасходовЗаказРеализация,
			|	НЕОПРЕДЕЛЕНО КАК БезАналитики
			|ИЗ
			|	ВтИсточникДанных КАК ВтИсточникДанных
			|		ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.СтатьиРасходов КАК ПланВидовХарактеристикСтатьиРасходов
			|		ПО (&ЗначениеСтатейИАналитик = ПланВидовХарактеристикСтатьиРасходов.Ссылка)
			|		И ТИПЗНАЧЕНИЯ(&ЗначениеСтатейИАналитик) = ТИП(ПланВидовХарактеристик.СтатьиРасходов)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			|	ВтИсточникДанных.ИдентификаторСтрокиВТ,
			|	ПланВидовХарактеристикСтатьиДоходов.ТипЗначения,
			|	ПланВидовХарактеристикСтатьиДоходов.КонтролироватьЗаполнениеАналитики,
			|	НЕОПРЕДЕЛЕНО,
			|	НЕОПРЕДЕЛЕНО
			|ИЗ
			|	ВтИсточникДанных КАК ВтИсточникДанных
			|		ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.СтатьиДоходов КАК ПланВидовХарактеристикСтатьиДоходов
			|		ПО (&ЗначениеСтатейИАналитик = ПланВидовХарактеристикСтатьиДоходов.Ссылка)
			|		И ТИПЗНАЧЕНИЯ(&ЗначениеСтатейИАналитик) = ТИП(ПланВидовХарактеристик.СтатьиДоходов)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			|	ВтИсточникДанных.ИдентификаторСтрокиВТ,
			|	ПланВидовХарактеристикСтатьиАктивовПассивов.ТипЗначения,
			|	НЕОПРЕДЕЛЕНО,
			|	НЕОПРЕДЕЛЕНО,
			|	ПланВидовХарактеристикСтатьиАктивовПассивов.БезАналитики
			|ИЗ
			|	ВтИсточникДанных КАК ВтИсточникДанных
			|		ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.СтатьиАктивовПассивов КАК ПланВидовХарактеристикСтатьиАктивовПассивов
			|		ПО (&ЗначениеСтатейИАналитик = ПланВидовХарактеристикСтатьиАктивовПассивов.Ссылка)
			|		И ТИПЗНАЧЕНИЯ(&ЗначениеСтатейИАналитик) = ТИП(ПланВидовХарактеристик.СтатьиАктивовПассивов)";
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЗначениеСтатейИАналитик", "ВтИсточникДанных." + ПолеСтатья); //@Query-part-2
...
Процедура ДополнитьТекстЗапросаПризнакПринимаетсяКНУ(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		...
		ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
						...
						|ИЗ ВтИсточникДанных КАК ВтИсточникДанных
						|	ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.СтатьиРасходов КАК СтатьиРасходов
						|		ПО СтатьиРасходов.Ссылка = ИсточникДанных.СтатьяРасходов
						|";
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ИсточникДанных.СтатьяРасходов", "ВтИсточникДанных." + ПолеСтатьяРасходов);
...
// Удаляет типы с префиксом "удалить" из массива типов
// 
// Параметры:
//  ТипыОбъекта - Массив из Тип - Типы объекта
// 
// Возвращаемое значение:
//  Массив из Тип - Актуальные типы 
Функция УдалитьИзМассиваТипыКУдалению(ТипыОбъекта) Экспорт
	Если ТипыОбъекта.Количество() > 1 Тогда
		ТипыКУдалению = Новый Массив;
		Для Каждого Тип Из ТипыОбъекта Цикл
			Если ОбщегоНазначенияУТ.ОбъектКУдалению(Тип) Тогда 
				ТипыКУдалению.Добавить(Тип);
			КонецЕсли;
		КонецЦикла;
		Возврат ОбщегоНазначенияКлиентСервер.РазностьМассивов(ТипыОбъекта, ТипыКУдалению);
	Иначе
		Возврат ТипыОбъекта;
	КонецЕсли;
КонецФункции 
...
Функция ТекстыЗапросовСторнирования(МетаданныеДокумента) Экспорт
	...
	Если ДвиженияДокумента.Содержит(МетаданныеРегистра) Тогда
		ТекстыЗапросов.Вставить(МетаданныеРегистра.ПолноеИмя(),
			ПартионныйУчетСервер.ТекстСторнирующегоЗапроса(
				МетаданныеРегистра,
				МетаданныеДокумента));
	КонецЕсли;
...
#Область ПрочаяВыручка
	...
	|	ПрочаяВыручка.ПериодКорректировки КАК ПериодКорректировки,
	...
	|	ПрочаяВыручка.ВыручкаПостояннаяРазница КАК ВыручкаПостояннаяРазница,
	|	ВЫБОР
...
Процедура ДобавитьЭтап_РаспределениеРасходовБудущихПериодов(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.РаспределениеРасходовБудущихПериодов,
		Истина, Истина, Истина);
		
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Распределение расходов будущих периодов'"; // @НСтр-1
...
// 
// Возвращаемое значение:
//  Структура - Данные выбор статей и аналитик:
// * МассивПараметровВыбора - ФиксированныйМассив из см. ДоходыИРасходыСервер.ПараметрыВыбораСтатьиИАналитики - 
// * СоответствиеПараметровВыбораЭлементамФормы - Структура - 
//
...
// 	Форма - ФормаКлиентскогоПриложения - Форма объекта:
// 		* Объект - ДокументОбъект - Объект формы
// 	МассивПараметровВыбора - Массив из см. ДоходыИРасходыСервер.ПараметрыВыбораСтатьиИАналитики -
//
Процедура ПроверитьПараметрыВыбораСтатейИАналитик(Форма, МассивПараметровВыбора)
	...
	Для каждого ПараметрыВыбора Из МассивПараметровВыбора Цикл
		...
		МассивПроверяемыхРеквизитов = Новый Массив(); // Массив из Строка
...
Процедура ЗаполнитьРеквизитыСтатьиВСтрокеТаблицыПакетно(Строка, Знач ПараметрыВыбора, КэшированныеДанные)
	...
	Если ЗначениеЗаполнено(Статья) Тогда
		Если Статья = ПараметрыВыбора.ЗначениеПоУмолчанию Тогда
			РеквизитыСтатьи = ДоходыИРасходыВызовСервераПовтИсп.ЗначенияРеквизитовСтатьи(Статья);
		Иначе
...
//	Характеристика - СправочникСсылка.ХарактеристикиНоменклатуры, Неопределено - номенклатура, для которой выполняется заполнение статьи и аналитики расходов.
//
// Возвращаемое значение:
...
Функция ТекстЗапросаТаблицаСпособовРаспределения()
	...
	|	ТаблицаРасходы.АналитикаРасходов КАК АналитикаРасходов,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаРасходы.АналитикаРасходов) = ТИП(Документ.ЗаказПоставщику)
...
Функция ТекстЗапросаТаблицаДокументов()
	...
	|	ТаблицаСпособовРаспределения.ЗаказПоставщику КАК ЗаказПоставщику,
...
// Возвращает шаблон текста запроса для получения данных о поступлениях товаров.
//
// Параметры:
//   ТаблицаОтбора - Строка - таблица для отбора данных
//   УсловияСоединения1 - Строка - условия для соединения таблиц первого запроса объединения.
//   ПервыйЗапрос - Булево - признак первого запроса.
//
// Возвращаемое значение:
//   Строка - шаблон запроса для использования в дальнейшем для получения данных о поступлениях товаров.
//
Функция ШаблонТекстаЗапросаПоступленияТоваров(ТаблицаОтбора, УсловияСоединения1, ПервыйЗапрос)
	ТекстЗапроса = "
	|ВЫБРАТЬ
	|	Т.ИндексБазы КАК ИндексБазы,
	...
	|ИЗ
	|	&ТаблицаОтбора КАК Т
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СебестоимостьТоваров КАК УчетСебестоимости
	|		ПО (&УсловияСоединения1)
	|ГДЕ
	...
	|	И УчетСебестоимости.РазделУчета В (ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаСкладах), ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.СобственныеТоварыВПути))
	|	И НЕ ТИПЗНАЧЕНИЯ(УчетСебестоимости.Регистратор) = ТИП(Документ.ПриобретениеТоваровУслуг)
	// В обоих случаях использования данного шаблона запроса, ЗаказПоставщику должен быть тем же что и в ТаблицаОтбора.
	// Исходя из этого добавляем условие что если данные подходят по остальным условиям, в ТаблицаОтбора ЗаказПоставщику пустая ссылка.
	|	И Т.ЗаказПоставщику = ЗНАЧЕНИЕ(Документ.ЗаказПоставщику.ПустаяСсылка)
	|
	|СГРУППИРОВАТЬ ПО
	|	Т.ИндексБазы,
	...
	|	СУММА(УчетСебестоимости.Количество),
	|	СУММА(УчетСебестоимости.СтоимостьРегл)
	|ИЗ
	|	&ТаблицаОтбора КАК Т
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СебестоимостьТоваров КАК УчетСебестоимости
	|		ПО (&УсловияСоединения1)
	...
	|			И (Т.ЗаказПоставщику = ПриобретениеТоваровУслугТовары.ЗаказПоставщику
	|				ИЛИ Т.ЗаказПоставщику = ЗНАЧЕНИЕ(Документ.ЗаказПоставщику.ПустаяСсылка))
	|ГДЕ
	|	УчетСебестоимости.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	И УчетСебестоимости.Активность
	|	И УчетСебестоимости.Организация = &Организация
	...
	|	УчетСебестоимости.Период,
	|	УчетСебестоимости.Регистратор,
	|	ПриобретениеТоваровУслугВидыЗапасов.ЗаказПоставщику,
	|	СУММА(УчетСебестоимости.Количество),
	|	СУММА(УчетСебестоимости.СтоимостьРегл)
	|ИЗ
	|	&ТаблицаОтбора КАК Т
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СебестоимостьТоваров КАК УчетСебестоимости
	|		ПО (&УсловияСоединения1)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПриобретениеТоваровУслуг.ВидыЗапасов КАК ПриобретениеТоваровУслугВидыЗапасов
	|		ПО (УчетСебестоимости.Регистратор = ПриобретениеТоваровУслугВидыЗапасов.Ссылка)
	|			И (УчетСебестоимости.ИдентификаторСтроки = ПриобретениеТоваровУслугВидыЗапасов.ИдентификаторСтроки)
	|			И (Т.ЗаказПоставщику = ПриобретениеТоваровУслугВидыЗапасов.ЗаказПоставщику
	|				ИЛИ Т.ЗаказПоставщику = ЗНАЧЕНИЕ(Документ.ЗаказПоставщику.ПустаяСсылка))
	|ГДЕ
	|	УчетСебестоимости.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	И УчетСебестоимости.Активность
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТаблицаОтбора", ТаблицаОтбора); //@Query-part
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловияСоединения1", УсловияСоединения1); //@Query-part
	
	Если Не ПервыйЗапрос Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПОМЕСТИТЬ ВтПоступленияТоваров", ""); //@Query-part
	КонецЕсли;
	Возврат ТекстЗапроса;
...
Функция ТекстЗапросаТаблицаПоступленияТоваров()
	...
	// Первый набор запросов ДокументПоступления(Регистратор) и ЗаказПоставщику должны быть теми же что и в ТаблицаОтбора.
	ТаблицаОтбора = "ТаблицаДокументов"; //@Query-part
	УсловияСоединения1 = "УчетСебестоимости.Регистратор = Т.Ссылка"; //@Query-part
	ТекстыЗапросов.Добавить(ШаблонТекстаЗапросаПоступленияТоваров(ТаблицаОтбора, УсловияСоединения1, Истина));
	
	// Второй набор запросов период определен, только ЗаказПоставщику должен быть тем же что и в ТаблицаОтбора.
	ТаблицаОтбора = "ТаблицаСпособовРаспределения"; //@Query-part
	УсловияСоединения1 = "
		|	УчетСебестоимости.Период МЕЖДУ &ДатаНачала И &ДатаОкончания
		|	И (Т.АналитикаРасходов = УчетСебестоимости.АналитикаУчетаНоменклатуры.СкладскаяТерритория
		|		ИЛИ Т.АналитикаРасходов = УчетСебестоимости.АналитикаУчетаНоменклатуры.Номенклатура
		|		ИЛИ Т.АналитикаРасходов = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		|		ИЛИ Т.АналитикаРасходов = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))"; //@Query-part
	ТекстыЗапросов.Добавить(ШаблонТекстаЗапросаПоступленияТоваров(ТаблицаОтбора, УсловияСоединения1, Ложь));
...
Функция СвойстваЗапросаПолученияДанныхВыбораСтатейАктивовПассивов(Параметры)
	...
	ПараметрыЗапроса.Вставить("ДвиженияПоПрочимАктивамПассивам", Метаданные.РегистрыНакопления.ДвиженияПоПрочимАктивамПассивам.Имя);
	
	Если Параметры.Свойство("ОбъектыФинансовогоУчета") Тогда
		ВидыСтатей = Неопределено;
		Если Параметры.Отбор.Свойство("Вид", ВидыСтатей) И ЗначениеЗаполнено(ВидыСтатей) Тогда
			ОтборПоНазначениюСтатей = "
			|	СтатьиАктивовПассивов.Вид В (&ВидыСтатей)";
			ПараметрыЗапроса.Вставить("ВидыСтатей", ВидыСтатей);
		Иначе
			ОтборПоНазначениюСтатей = "
			|	СтатьиАктивовПассивов.Вид <> ЗНАЧЕНИЕ(Перечисление.ВидыСтатейАктивовПассивов.ПустаяСсылка)";
		КонецЕсли;
		...
		Если Параметры.Отбор.Свойство("ТипДанныхУчета", ТипыДанныхУчета) И ЗначениеЗаполнено(ТипыДанныхУчета) Тогда
			ОтборПоНазначениюСтатей = "
			|	И СтатьиАктивовПассивов.ТипДанныхУчета В (&ТипыДанныхУчета)";
			ПараметрыЗапроса.Вставить("ТипыДанныхУчета", ТипыДанныхУчета);
		КонецЕсли;
		...
		Если Параметры.Отбор.Свойство("СчетаУчетаВНастройкахПроводок", СчетаУчетаВНастройкахПроводок) Тогда
			ОтборПоНазначениюСтатей = ОтборПоНазначениюСтатей + "
			|	И " + ?(СчетаУчетаВНастройкахПроводок, "", "НЕ ") + "СтатьиАктивовПассивов.СчетаУчетаВНастройкахПроводок";
		КонецЕсли;
		...
		Если Параметры.Отбор.Свойство("НесобственныеПодконтрольныеЦенности", НесобственныеПодконтрольныеЦенности) Тогда
			ОтборПоНазначениюСтатей = ОтборПоНазначениюСтатей + "
			|	И " + ?(НесобственныеПодконтрольныеЦенности, "", "НЕ ") + "СтатьиАктивовПассивов.НесобственныеПодконтрольныеЦенности";
		КонецЕсли;
		
		ЕстьИсточникДанных = Ложь;
		Если Параметры.Свойство("ЕстьИсточникДанных", ЕстьИсточникДанных) Тогда
			ОтборПоНазначениюСтатей = ОтборПоНазначениюСтатей + "
			|	И " + ?(ЕстьИсточникДанных, "НЕ ", "") + "СтатьиАктивовПассивов.ИсточникДанных = """"";
		КонецЕсли;
		
		НастройкиФинансовогоУчета = Неопределено;
		Если Параметры.Свойство("НастройкиФинансовогоУчета", НастройкиФинансовогоУчета)
			И ТипЗнч(НастройкиФинансовогоУчета) = Тип("Структура") Тогда
			Если НЕ НастройкиФинансовогоУчета.УчетВыручкиСНДС
			 ИЛИ НЕ НастройкиФинансовогоУчета.УчетВыручкиБезНДС
			 ИЛИ НЕ НастройкиФинансовогоУчета.ФормированиеКорреспонденций Тогда
				СтрокиИсключаемыхЗначений = Новый Массив();
				РазделительСтрок = "," + Символы.ПС + Символы.Таб;
				
				Если НЕ НастройкиФинансовогоУчета.ФормированиеКорреспонденций Тогда
				...
				Если НЕ НастройкиФинансовогоУчета.УчетВыручкиБезНДС Тогда
				...
				Если НЕ НастройкиФинансовогоУчета.УчетВыручкиСНДС Тогда
				...
				ОтборПоНазначениюСтатей = ОтборПоНазначениюСтатей + "
				|	И НЕ СтатьиАктивовПассивов.Ссылка В (
				|	" + СтрСоединить(СтрокиИсключаемыхЗначений, РазделительСтрок) + ")";
			КонецЕсли;
			
			Если НЕ НастройкиФинансовогоУчета.УчетНесобственныхЦенностей Тогда
				ОтборПоНазначениюСтатей = ОтборПоНазначениюСтатей + "
				|	И НЕ СтатьиАктивовПассивов.НесобственныеПодконтрольныеЦенности";
			КонецЕсли;
		КонецЕсли;
	Иначе
		ОтборПоНазначениюСтатей = "
		|	(СтатьиАктивовПассивов.АктивПассив <> ЗНАЧЕНИЕ(Перечисление.ВидыСтатейУправленческогоБаланса.ПустаяСсылка)
		|		ИЛИ СтатьиАктивовПассивов.ИсточникДанных = &ДвиженияПоПрочимАктивамПассивам)";
		
		АктивПассив = Неопределено;
		Если Параметры.Отбор.Свойство("АктивПассив", АктивПассив) И ЗначениеЗаполнено(АктивПассив) Тогда
			ОтборПоНазначениюСтатей = ОтборПоНазначениюСтатей + "
			|	И СтатьиАктивовПассивов.АктивПассив В (&АктивПассив)";
			ПараметрыЗапроса.Вставить("АктивПассив", АктивПассив);
		КонецЕсли;
	КонецЕсли;
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборПоНазначениюСтатей", ОтборПоНазначениюСтатей);
	
	ОграничениеГруппИЭлементов = Новый Массив; // Массив из Булево
