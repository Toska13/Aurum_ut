...
Функция ПолучитьУсловияЗакупок(Знач Соглашение,
	...
	|	СоглашениеСПоставщиком.СрокОтгрузки                 КАК СрокОтгрузки,
...
Функция ПолучитьУсловияЗакупокПоУмолчанию(Знач Партнер, Знач ПараметрыОтбора = Неопределено) Экспорт
	...
	ВсеПараметрыОтбора.Вставить("ЦенаВключаетНДС",                         Неопределено);
	
	Если ПараметрыОтбора <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ВсеПараметрыОтбора, ПараметрыОтбора);
	КонецЕсли;
	
	// Получение соглашения по умолчанию:
	ПараметрыДействия = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСоглашенийПоставщика();
	ПараметрыДействия.ТолькоДействующее = ВсеПараметрыОтбора.ТолькоДействующее;
	ПараметрыДействия.ХозяйственнаяОперация = ВсеПараметрыОтбора.ХозяйственныеОперации;
	ПараметрыДействия.ЦенаВключаетНДС = ВсеПараметрыОтбора.ЦенаВключаетНДС;
	ПараметрыДействия.ИспользуютсяДоговорыКонтрагентов = ВсеПараметрыОтбора.ИспользуютсяДоговорыКонтрагентов;
	
	ДанныеСтроки = Новый Структура;
	ДанныеСтроки.Вставить("Партнер",    Партнер);
	ДанныеСтроки.Вставить("Соглашение", ВсеПараметрыОтбора.ВыбранноеСоглашение);
	
	Действия = Новый Структура;
	Действия.Вставить("ЗаполнитьСоглашениеПоставщикаПоУмолчанию", ПараметрыДействия);
	...
	// Получение условий закупок по выбранному соглашению по умолчанию:
	ВозвращаемоеЗначение = Неопределено;
	Если ЗначениеЗаполнено(ДанныеСтроки.Соглашение) Тогда
		ВозвращаемоеЗначение = ПолучитьУсловияЗакупок(
			ДанныеСтроки.Соглашение,
			ВсеПараметрыОтбора.УчитыватьГруппыСкладов,
			ВсеПараметрыОтбора.ИсключитьГруппыСкладовДоступныеВЗаказах);
	КонецЕсли;
...
// Возвращает дополнительные хозяйственные операции по основной.
// 
// Параметры:
...
Функция ХозяйственныеОперацииПоОсновной(ОсновнаяХозяйственнаяОперация) Экспорт
	...
	ДополнительныеХозяйственныеОперацииПоОсновным = ДополнительныеХозяйственныеОперацииПоОсновным(); 

	ПараметрыПоиска = Новый Структура("ОсновнаяОперация", ОсновнаяХозяйственнаяОперация);
	НайденныеСтроки = ДополнительныеХозяйственныеОперацииПоОсновным.НайтиСтроки(ПараметрыПоиска);
	Если НайденныеСтроки.Количество() = 0 Тогда
		ВозвращаемоеЗначение = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОсновнаяХозяйственнаяОперация);
	Иначе
		ВозвращаемоеЗначение = ОбщегоНазначения.ВыгрузитьКолонку(НайденныеСтроки, "ДополнительнаяОперация");
	КонецЕсли;
...
//
// Параметры:
//  КлючомЯвляютсяОперацииПереходаПраваСобственности	 - Булево - инвертирует ключи и значения возвращаемого соответствия.
// 
// Возвращаемое значение:
...
//
Функция ПарыОперацийРаздельнойЗакупки(КлючомЯвляютсяОперацииПереходаПраваСобственности = Ложь) Экспорт
	...
	Для Счетчик = 0 По 5 Цикл
		Если КлючомЯвляютсяОперацииПереходаПраваСобственности Тогда
...
// Получает вариант оформления документов поступления по услугам из договора.
//
// Параметры:
//  Договор	 - СправочникСсылка.ДоговорыКонтрагентов - договор с контрагентом.
// 
// Возвращаемое значение:
//	ПеречислениеСсылка.ВариантыОформленияДокументовПоУслугам - вариант оформления документов по услугам по указанному договору.
//
Функция ПолучитьВариантОформленияДокументовПоступленияПоУслугам(Договор) Экспорт
	
	ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПустаяСсылка();
	Если ЗначениеЗаполнено(Договор) Тогда
		УстановитьПривилегированныйРежим(Истина);
	...
	Возврат ВариантОформленияДокументовПоступленияПоУслугам;
...
//  ВариантыОформленияДокументовПоУслугам - ПеречислениеСсылка.ВариантыОформленияДокументовПоУслугам - вариант оформления документов по услугам по указанному договору.
// 
// Возвращаемое значение:
//	Булево - Это неотфактурованная поставка.
//
Функция ЭтоНеотфактурованнаяПоставкаУслугРабот(ХозяйственнаяОперация, ВариантОформленияДокументовПоУслугам) Экспорт
	...
	НеотфактурованныеПоставкиУслугРабот = ОперацииФактуровка.Найти(ХозяйственнаяОперация) <> Неопределено
		И ВариантОформленияДокументовПоУслугам = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется");
	
	Возврат НеотфактурованныеПоставкиУслугРабот;
	
КонецФункции

// См. ПродажиСервер.ДокументОснование
//
Функция ДокументОснование(Объект, ПоляПоиска) Экспорт
	
	Возврат ПродажиСервер.ДокументОснование(Объект, ПоляПоиска);
	
КонецФункции

// Процедура устанавливает условное оформление в списках для хозяйственной операции заказов поставщику.
// 
// Представление операций "Закупка у поставщика (фактуровка поставки)" и "Ввоз из ЕАЭС (фактуровка поставки)"
// меняем на "Закупка у поставщика (неотфактурованная поставка)" и "Ввоз из ЕАЭС (неотфактурованная поставка)", соответственно.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - Форма для условного оформления:
//  	* Список - ДинамическийСписок - Список заказов поставщику.
//  	* Элементы - ВсеЭлементыФормы - Элементы формы для условного оформления:
//  		** ХозяйственнаяОперация - ПолеФормы - Поле для отображения хозяйственной операции.
//
Процедура УстановитьУсловноеОформлениеОперацииЗаказаПоставщику(Форма) Экспорт
	
	СписокУсловноеОформление = Форма.Список.КомпоновщикНастроек.Настройки.УсловноеОформление;
	
	//
	Элемент = СписокУсловноеОформление.Элементы.Добавить();
	Элемент.Представление = НСтр("ru = 'Закупка у поставщика (неотфактурованная поставка)'");
	...
	ОтборЭлемента.ПравоеЗначение = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки;
	
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", Элемент.Представление);
	
	//
	Элемент = СписокУсловноеОформление.Элементы.Добавить();
	Элемент.Представление = НСтр("ru = 'Ввоз из ЕАЭС (неотфактурованная поставка)'");
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ХозяйственнаяОперация");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки;
...
Процедура ПроверитьКорректностьЗаполненияДокументаЗакупки(ДокументЗакупки, Отказ) Экспорт
	...
	ИначеЕсли ИмяТаблицы = "Документ.ЗаявкаНаЗакупку" Тогда
		
		ВсеСтрокиОтменены = ОбщегоНазначенияУТ.ВсеСтрокиОтменены(ДокументЗакупки, "Товары", "Отменено");
		НужноСогласовать = Не ОтклонениеОтУсловийЗакупок
			И Не ДокументЗакупки.Согласован // Ранее заявку уже согласовали, изменены незначительные реквизиты.
			И Перечисления.СтатусыЗаявокНаЗакупку.Согласована(ДокументЗакупки.Статус) // Проверка установки статуса, требующего проверки.
	...
	ИначеЕсли ИмяТаблицы = "Документ.ЗаявкаНаЗакупку" Тогда
		...
		Если НужноСогласовать Тогда
			МассивПроверок.Добавить("ВременнаяТаблицаКурсыВалют");
			МассивПроверок.Добавить("ВременнаяТаблицаСкладыКорректныеПоСоглашениям");
	...
	ИначеЕсли ИмяТаблицы = "Документ.ПриобретениеТоваровУслуг" Тогда
		...
		ЭтоЗакупкаРаботПоДоговоруПереработки = Ложь;
		Если ЗначениеЗаполнено(ДокументЗакупки.Договор) Тогда
			ТипДоговора = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументЗакупки.Договор, "ТипДоговора");
			ЭтоЗакупкаРаботПоДоговоруПереработки = (ТипДоговора = Перечисления.ТипыДоговоров.СПереработчиком2_5
													Или ТипДоговора = Перечисления.ТипыДоговоров.СПереработчиком2_5_ЕАЭС);
		КонецЕсли;
		
		Если ЭтоЗакупкаРаботПоДоговоруПереработки Тогда
			МассивПроверок.Добавить("НаличиеТоваровПоДоговоруПереработки");
		КонецЕсли;
	...
	Для Каждого ТекЭлемент Из МассивПроверок Цикл
		...
		ИначеЕсли ТекЭлемент = "ВременнаяТаблицаСкладыКорректныеПоСоглашениям" Тогда
			
			СформироватьЗапросВременнаяТаблицаСкладыКорректныеПоСоглашениям(ТекстЗапроса, ПараметрыЗапроса, ДокументЗакупки);
		...
		ИначеЕсли ТекЭлемент = "СоответствиеТоваровУсловиямЗакупок" Тогда
			...
			СформироватьЗапросТоварыДокументаЗакупки(ТекстЗапроса, ИмяТаблицы);
		...
		ИначеЕсли ТекЭлемент = "НаличиеТоваровПоДоговоруПереработки" Тогда 
			
			СформироватьЗапросНаличиеТоваровУслугВДокументеЗакупки(ТекстЗапроса);
	...
	Для ТекИндекс = 0 По МассивРезультатов.Количество()-1 Цикл
		...
		ИначеЕсли МассивПроверок[ТекИндекс] = "НаличиеТоваровПоДоговоруПереработки" Тогда
		
			СообщитьОбОшибкахНаличиеТоваровУслугРаботВДокументеЗакупки(Выборка, ДокументЗакупки, Отказ);
	...
	Если НевозможноПровестиБезСогласования Тогда
		...
		Если (ИмяТаблицы = "Документ.ЗаказПоставщику" И ИспользоватьСтатусы)
			Или ИмяТаблицы = "Документ.ЗаявкаНаЗакупку" Тогда 
...
Процедура ПроверитьРаздельноеОформлениеТоваров(Организация, ХозяйственнаяОперация, Договор, ТаблицаТовары, Отказ) Экспорт
	...
	Если ВариантОформленияЗакупок = Перечисления.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваровИУслуг Тогда
		Возврат;
	КонецЕсли;
	...
	Если Не Результат.Пустой() Тогда
		ШаблонСообщения = НСтр("ru='По организации ""%1"" в договоре ""%2"" не установлено раздельное оформление товаров, 
			|Проведение документа с хозяйственной операцией ""%3"" для товаров не допускается'");
...
Процедура ПроверитьНеобходимостьСогласованияУсловийЗакупки(Знач ДокументЗакупки,
	...
	Для ТекИндекс = 0 По МассивРезультатов.Количество()-1 Цикл
		...
		ИначеЕсли ТекЭлемент = "КорректностьЭтаповГрафикаОплаты" Тогда
			...
			ГрафикПоУмолчанию.Колонки.Добавить("Формула");
			...
			Если Выборка.Следующий() 
				...
				Иначе
					...
					ПараметрыЗаполнения.ТабличнаяЧасть = ТаблицаТовары;
	...
	Если ТаблицаПроверяемыеТовары.Количество() > 0 Тогда
		...
		ИначеЕсли ТипЗнч(ДокументЗакупки) = Тип("ДокументОбъект.ВозвратТоваровПоставщику") Тогда
			ЗапросТекст = "
				...
				|					И ВЫРАЗИТЬ(ТаблицаТовары.ДокументПоступления КАК Документ.ПриобретениеТоваровУслуг).ХозяйственнаяОперация В (&ОперацииТоварыВПутиИНеотфактуровка)
				|					ТОГДА
				|						ВЫБОР
				|							КОГДА ((ДанныеВидаНоменклатуры.ПолитикаУчетаСерий.УчетСерийВНеотфактурованныхПоставкахТоваров
				|										И НЕ ВЫРАЗИТЬ(ТаблицаТовары.ДокументПоступления КАК Документ.ПриобретениеТоваровУслуг).ХозяйственнаяОперация В (&ОперацииВПути))
				|									ИЛИ (ДанныеВидаНоменклатуры.ПолитикаУчетаСерий.УчетТоваровВПутиОтПоставщикаПоСериям)
				|										И ВЫРАЗИТЬ(ТаблицаТовары.ДокументПоступления КАК Документ.ПриобретениеТоваровУслуг).ХозяйственнаяОперация В (&ОперацииВПути))
		...
		|	И ТаблицаПроверяемыеТовары.Ссылка <> &СторнируемыйДокумент
		|
		|СГРУППИРОВАТЬ ПО
		...
		Если ИсправлениеДокументов.ЭтоИсправительныйДокумент(ДокументЗакупки) Тогда
			СторнируемыйДокумент = ДокументЗакупки.СторнируемыйДокумент;
		КонецЕсли;
		...
		Запрос.УстановитьПараметр("ОперацииВПути", ОперацииВПути);
...
Функция ПолучитьДоговорПоУмолчанию(
	...
	ПараметрыЗаполненияДоговора.ХозяйственнаяОперация = ХозяйственныеОперации;
	ПараметрыЗаполненияДоговора.ВалютаВзаиморасчетов = ДополнительныеПараметрыОтбораДоговоров.ВалютаВзаиморасчетов;
	ПараметрыЗаполненияДоговора.НаправлениеДеятельности = ДополнительныеПараметрыОтбораДоговоров.НаправлениеДеятельности;
	ПараметрыЗаполненияДоговора.КомиссионныеПродажи25 = ДополнительныеПараметрыОтбораДоговоров.КомиссионныеПродажи25;
	ПараметрыЗаполненияДоговора.ПорядокРасчетов = ДополнительныеПараметрыОтбораДоговоров.ПорядокРасчетов;
	ПараметрыЗаполненияДоговора.ПорядокОформленияСписанияНедостачТоваровПринятыхНаХранение = ДополнительныеПараметрыОтбораДоговоров.ПорядокОформленияСписанияНедостачТоваровПринятыхНаХранение;
	ПараметрыЗаполненияДоговора.Налогообложение = ДополнительныеПараметрыОтбораДоговоров.Налогообложение;
	ПараметрыЗаполненияДоговора.ДоговорССамозанятым = ДополнительныеПараметрыОтбораДоговоров.ДоговорССамозанятым;
	
	ДанныеСтроки = Новый Структура;
	ДанныеСтроки.Вставить("Партнер");
...
Функция ЗаполнитьТоварыПоФактическойПриемке(Товары, ПараметрыЗаполнения, ЕстьПринятыеТовары, ДополнительныеПараметрыПоиска = Неопределено) Экспорт
	...
	Для Каждого СтрокаКДозаполнению Из ИсходныеСтрокиКДозаполнениюСОстатками Цикл
		...
		Для Каждого СтрокаОстатков Из СтрокиТаблицыОстатков Цикл
			...
			ДобавляемоеКоличествоУпаковок = ДобавляемоеКоличество / СтрокаКДозаполнению.КоэффициентУпаковки;
...
Функция ДопустимыеОтклоненияМерныхТоваров(МассивРаспоряжений, РаспоряжениеЗаказ = Истина) Экспорт
	...
	Если ТипЗнч(МассивРаспоряжений[0]) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
		...
		|	СУММА(ТоварыКПоступлению.КОформлениюПоВозвратамКоличествоОборот *
		|		(&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100)) КАК ДопустимоеОтклонение
		|ИЗ
		|	#ЗаявкиНаВозвратОборотыРазвернутая КАК ТоварыКПоступлению
		|СГРУППИРОВАТЬ ПО
		...
		ПараметрыФормированияТаблицы.НачалоПериода = "&НачПериод";
		ПараметрыФормированияТаблицы.КонецПериода = "&КонПериод";
		...
		ПараметрыФормированияТаблицы.Показатели = "КОформлениюПоВозвратам";
...
// Заполняет список выбора вариантов приемки услуг
//
// Параметры:
//	ОформлениеОрдера - ПолеФормы - поле формы с настройкой оформления ордера.
//	ПриемкаУслуг - ПолеФормы - поле формы с настройкой приемки услуг.
//	ВариантОформленияДокументовПоступленияПоУслугам - ПеречислениеСсылка.ВариантыОформленияДокументовПоУслугам - вариант оформления раздельной закупки.
//
Процедура ЗаполнитьСписокВыбораВариантовПриемкиУслугПоДоговорам(ОформлениеОрдера, ПриемкаУслуг, ВариантОформленияДокументовПоступленияПоУслугам) Экспорт
	...
	ВыбранаНеотфактурованнаяПоставка = ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется;
	...
	Если ПриемкаУслуг <> Неопределено Тогда
		СписокВыбораПриемка = ПриемкаУслуг.СписокВыбора;
		СписокВыбораПриемка.Очистить();
		СписокВыбораПриемка.Добавить("Разделена", НСтр("ru = 'Разделена по документам'"));
		Если ИспользоватьДоговорыСПоставщиками Тогда
			СписокВыбораПриемка.Добавить("НеРазделена", НСтр("ru = 'Сгруппирована по договорам'"));
		КонецЕсли;
		ПриемкаУслуг.Доступность = СписокВыбораПриемка.Количество() > 1;
	КонецЕсли;
...
// Получает вариант приемки услуг по договору.
//
// Параметры:
//  Договор - СправочникСсылка.ДоговорыКонтрагентов - договор, определяющий вариант приемки услуг.
// 
// Возвращаемое значение:
//   - ПеречислениеСсылка.ВариантыПриемкиТоваров - найденный вариант приемки.
//
Функция ПолучитьВариантПриемкиУслуг(Договор) Экспорт
	
	ВариантПриемкиУслуг = Перечисления.ВариантыПриемкиТоваров.ПустаяСсылка();
	
	Если ЗначениеЗаполнено(Договор) Тогда
		УстановитьПривилегированныйРежим(Истина);
		
		РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Договор,
			"ВариантПриемкиТоваров, ВариантПриемкиУслуг");
		
		// Если реквизит ВариантПриемкиУслуг еще не заполнен, например, 
		// когда обновление справочника еще не завершено, возвращается 
		// значение рекзвита ВариантПриемкиТоваров.
		Если ЗначениеЗаполнено(РеквизитыДоговора.ВариантПриемкиУслуг) Тогда
			ВариантПриемкиУслуг = РеквизитыДоговора.ВариантПриемкиУслуг;
		Иначе
			ВариантПриемкиУслуг = РеквизитыДоговора.ВариантПриемкиТоваров;
		КонецЕсли;
		
		УстановитьПривилегированныйРежим(Ложь);
	КонецЕсли;
	
	Возврат ВариантПриемкиУслуг;
...
Процедура ЗаполнитьНомераГТДПоУчетнымДанным(ТаблицаТоваров, ВариантПолученияДанных, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеДанные.Вставить("ДействияСоСтрокой", ДействияСоСтрокой);
	ДополнительныеДанные.Вставить("ЗаполнитьТипНомераГТД", ЗаполнитьТипНомераГТД);
	ДополнительныеДанные.Вставить("ЗаполнятьКоличествоУпаковок", ЗаполнятьКоличествоУпаковок);
	ДополнительныеДанные.Вставить("ЗаполнятьСтрануПроисхождения", ЗаполнятьСтрануПроисхождения);
	ДополнительныеДанные.Вставить("ИсключитьСкладскиеСерииИзПоиска", (ВариантПолученияДанных = "ПоОстаткамВПути"));
	
	ЕстьЦены = НомераГТДКЗаполнению.Колонки.Найти("Цена") <> Неопределено
		И НомераГТДКЗаполнению.Итог("Цена") > 0;
	
	Если ЕстьЦены Тогда
		// Поиск строк с совпадающей ценой.
		ПоляПоискаСЦеной = ПоляПоиска + ", Цена";
		ДополнительныеДанные.Вставить("ПоляПоиска", ПоляПоискаСЦеной);
		ЗаполнитьНомераГТДПоУчетнымДаннымВнутренний(ТоварыКЗаполнению,
			НомераГТДКЗаполнению, ДополнительныеДанные);
	КонецЕсли;
	
	// Дораспределение по оставшимся строкам, без учета цены.
	ДополнительныеДанные.Вставить("ПоляПоиска", ПоляПоиска);
	ЗаполнитьНомераГТДПоУчетнымДаннымВнутренний(ТоварыКЗаполнению,
		НомераГТДКЗаполнению, ДополнительныеДанные);
	
	// Из-за неверного округления при распределении номеров ГТД требуется дополнительный контроль сумм.
	
	ПоляПоискаДо = "Номенклатура, Характеристика, Серия, Назначение";
	СтруктураПоискаДо = Новый Структура(ПоляПоискаДо);
	...
	ТоварыСуммаКЗаполнению.Свернуть(ПоляПоискаДо, ПоляСуммирования); 
	
	ТоварыКЗаполнениюСуммБезГТД = ТоварыКЗаполнению.Скопировать();
	ТоварыКЗаполнениюСуммБезГТД.Колонки.Добавить("ТребуетсяПроверка", Новый ОписаниеТипов("Булево"));
	Для Каждого Строка Из ТоварыКЗаполнениюСуммБезГТД Цикл
		Строка.ТребуетсяПроверка = ЗначениеЗаполнено(Строка.НомерГТД);
	КонецЦикла;
	
	ТоварыКЗаполнениюСуммБезГТД.Свернуть(ПоляПоискаДо, ПоляСуммирования + ", ТребуетсяПроверка");
	
	Для Каждого СтрокаТоваровДо Из ТоварыСуммаКЗаполнению Цикл
		
		ЗаполнитьЗначенияСвойств(СтруктураПоискаДо, СтрокаТоваровДо);
		
		// Контроль корректности сумм.
		
		РаспределенныеТоварыСуммы = ТоварыКЗаполнениюСуммБезГТД.НайтиСтроки(СтруктураПоискаДо);
		Если РаспределенныеТоварыСуммы.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		СуммыПоРаспределенномуТовару = РаспределенныеТоварыСуммы[0];
		
		Если Не СуммыПоРаспределенномуТовару.ТребуетсяПроверка Тогда
			Продолжить;
		КонецЕсли;
		
		// Если до распределения сумма была неизвестна, значит нет контрольного значения. Провести контроль невозможно.
		// Это сценарий ввода возврата вручную (не по документу-основанию), когда пользователь не заполнил цену.
		Если ПересчетСуммы
			И СтрокаТоваровДо.Сумма = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		ЕстьРазностьСумм = Ложь;
		Если ПересчетСуммы
			И СуммыПоРаспределенномуТовару.Сумма <> СтрокаТоваровДо.Сумма Тогда
			ЕстьРазностьСумм = Истина;
		КонецЕсли;
		Если ПересчетСуммыСНДС
			И СуммыПоРаспределенномуТовару.СуммаСНДС <> СтрокаТоваровДо.СуммаСНДС Тогда
			ЕстьРазностьСумм = Истина;
		КонецЕсли;
		Если ПересчетСуммыНДС
			И СуммыПоРаспределенномуТовару.СуммаНДС <> СтрокаТоваровДо.СуммаНДС Тогда
			ЕстьРазностьСумм = Истина;
		КонецЕсли;
		
		Если Не ЕстьРазностьСумм Тогда
			Продолжить;
		КонецЕсли;
		
		// Контроль не пройден, требуется исправить суммы.
		// Предполагается, что величина отклонения составляет единицы копеек.
		// Расхождение отражается в последней строке.
		
		СтрокиТоваров = ТоварыКЗаполнению.НайтиСтроки(СтруктураПоискаДо);
		ИндексПоследнейСтроки = СтрокиТоваров.Количество() - 1;
		СтрокаКВнесениюКорректировок = СтрокиТоваров[ИндексПоследнейСтроки];
		
		Если ПересчетСуммы Тогда
			Корректировка = СуммыПоРаспределенномуТовару.Сумма - СтрокаТоваровДо.Сумма;
			СтрокаКВнесениюКорректировок.Сумма = СтрокаКВнесениюКорректировок.Сумма - Корректировка;
		КонецЕсли;
		Если ПересчетСуммыНДС Тогда
			Корректировка = СуммыПоРаспределенномуТовару.СуммаНДС - СтрокаТоваровДо.СуммаНДС;
			СтрокаКВнесениюКорректировок.СуммаНДС = СтрокаКВнесениюКорректировок.СуммаНДС - Корректировка;
		КонецЕсли;
		Если ПересчетСуммыСНДС Тогда
			Корректировка = СуммыПоРаспределенномуТовару.СуммаСНДС - СтрокаТоваровДо.СуммаСНДС;
			СтрокаКВнесениюКорректировок.СуммаСНДС = СтрокаКВнесениюКорректировок.СуммаСНДС - Корректировка;
		КонецЕсли;
...
Процедура ОбработатьУказаниеНомераГТД(Форма,
	...
	Если ДанныеПодбора.ОстаточныйНомерГТД <> Неопределено Тогда
		...
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПересчитатьКоличествоУпаковокПриПодбореНомераГТД(
			ТекущаяСтрока,
			НоваяСтрока,
			ПараметрыПодбора.ИмяПоляКоличествоУпаковок);
		
		Если ТребуетсяОбработкаСтрок Тогда
			ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(НоваяСтрока, ДействияОбработкиСтроки, Неопределено);
		КонецЕсли;
	...
	Если ТребуетсяОбработкаСтрок Тогда
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ТекущаяСтрока, ДействияОбработкиСтроки, Неопределено);
	КонецЕсли;
...
Процедура ДополнитьТекстЗапросаСтранаПроисхожденияПоНомеруГТД(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		...
		Если Параметры = Неопределено Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&НомерГТДЗаполнениеСтраны", "ИсточникДанных.НомерГТД");
		Иначе
			ОписаниеЗапроса.ПараметрыЗапроса.Вставить("НомерГТДЗаполнениеСтраны", Параметры.НомерГТД);
		КонецЕсли;
		
		ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "СтранаПроисхожденияПоНомеруГТД");
...
Процедура ДополнитьТекстЗапросаТипНомераГТД(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		...
		ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ТипНомераГТД");
...
// Возвращаемое значение:
//	Структура - коллекция, которая содержит следующие свойства:
//		* ИзвлечьНоменклатуруИзОбъектаВКолонке - Строка - имя колонки, в которой содержится информация о номенклатуре,
//															если не задано явно.
//		* ИзвлечьХарактеристикуИзОбъектаВКолонке - Строка - имя колонки, в которой содержится информация о характеристике номенклатуры,
//															если не задано явно.
//		* ИзвлечьСериюИзОбъектаВКолонке - Строка - имя колонки, в которой содержится информация о серии номенклатуры,
//															если не задано явно.
...
Функция НайтиИлиСоздатьСоставныеПрослеживаемыеНомераГТД(Товары,
	...
	|////////////////////////////////////////////////////////////////////////////////
	// 5.
	// Сгурппированная таблица номеров ГТД, по ключевым полям.
	|ВЫБРАТЬ
	...
	|ПОМЕСТИТЬ ВТСоставнойНомерСгруппированная
	|ИЗ
	...
	|		ИЗ
	|			ВТСоставнойНомерСгруппированная КАК СоставнойНомер
	|		ГДЕ
	|			СоставнойНомер.ЭтоОсновнойКомплект
	|		
	|		ОБЪЕДИНИТЬ ВСЕ
	...
	КомплектующиеДляСозданияСоставногоНомераГТД = РезультатЗапроса[6].Выгрузить();
	ПроверкаКоличестваСписываемыхПрослеживаемыхМатериалов = РезультатЗапроса[5].Выгрузить();
	ТребуетсяЗадействоватьВторойСоставнойГТДВыборка = РезультатЗапроса[7].Выбрать();
	СписываемыеГТД = РезультатЗапроса[10].Выбрать();
...
//  ПоляОбъектные - Массив из Строка, Неопределено - Имена колонок (не числовых) в таблице ТоварыКОприходованию, которые нужно сохранить
...
Функция РазобратьСоставныеПрослеживаемыеНомераГТД(ТоварыКСписанию, ТоварыКОприходованию, ПоляОбъектные = Неопределено, ПоляЧисловые = Неопределено, Отказ = Ложь) Экспорт
	...
	|	МАКСИМУМ(ЕСТЬNULL(ВтКомплектующие.НомерГТД.Номенклатура, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))) КАК Номенклатура,
	|	ВЫРАЗИТЬ(СУММА(Комплектующие.КоличествоПоРНПТ) КАК ЧИСЛО(23, 11)) КАК КоличествоИсходное,
	|	ВЫРАЗИТЬ(СУММА(Комплектующие.КоличествоПоРНПТ * ВтКомплектующие.Количество) КАК ЧИСЛО(23, 11)) КАК Количество,
	...
	|
	|ГДЕ
	|	ВЫРАЗИТЬ(Товары.Количество - ВтСписокТоваров.Количество КАК ЧИСЛО(15, 3)) <> 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|
	|ГДЕ
	|	ВЫРАЗИТЬ(Товары.КоличествоПоРНПТ - ВтСписокТоваров.Количество КАК ЧИСЛО(23, 11)) <> 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ЕСТЬNULL(СправочникНоменклатура.НаименованиеПолное, """") КАК НаименованиеНоменклатуры,
	...
	|		ПО СписокКомплектующих.Номенклатура = СправочникНоменклатура.Ссылка";
	...
	Результат = РезультатЗапроса[РезультатЗапроса.ВГраница() - 1].Выгрузить(); // ТаблицаЗначений -
	Результат.Индексы.Добавить("НомерГТД, ЕдиницаИзмеренияТНВЭД");
	...
	Пока КомплектующиеРНПТ.Следующий() Цикл
		...
		Если ИсходноеКоличество <> 0 Тогда
			ШаблонСообщения = НСтр("ru = 'При разборке составных прослеживаемых комплектов зафиксирована ошибка: невозможно отнести номер РНПТ ""%1"" на прослеживаемое изделие (комплектующее), поскольку нет изделия (комплектующего) с подходящей единицей измерения ТНВЭД (""%2"").
			...
			ТекстСообщения = СтрШаблон(ШаблонСообщения, СокрЛП(КомплектующиеРНПТ.НомерГТД), СокрЛП(КомплектующиеРНПТ.ЕдиницаИзмеренияТНВЭД));
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Неопределено, "", "", Отказ);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(КомплектующиеРНПТ.Номенклатура)
			И КомплектующиеРНПТ.ТребуетсяЗаполнитьНоменклатуруВГТД Тогда
			
			АктуализацияНоменклатурыВРНПТ(КомплектующиеРНПТ.НомерГТД,
											КомплектующиеРНПТ.Номенклатура,
											КомплектующиеРНПТ.НаименованиеНоменклатуры);
...
// Получает таблицу товаров с прослеживаемыми номерами ГТД, переданной таблицы товаров, в том числе выполняется
// извлечение комплектующих из комплектов, прослеживаемых номеров ГТД.
//
// Параметры:
//	Товары - ТаблицаЗначений - таблица товаров:
//		* АналитикаУчетаНоменклатуры - СправочникСсылка.КлючиАналитикиУчетаНоменклатуры - аналитика учета номенклатуры.
//		* НомерГТД - СправочникСсылка.НомераГТД - номер ГТД товара.
//		* Количество - Число - количество товара в единице хранения.
//		* КоличествоПоРНПТ - Число - количество товара в единице измерения ТН ВЭД, для прослеживаемых товаров,
//									или единице хранения товара для комплектов, прослеживаемых номеров ГТД.
//
// Возвращаемое значение:
//	ТаблицаЗначений - таблица, содержащая аналитики учета прослеживаемых товаров с учетными данными прослеживаемости:
//		* АналитикаУчетаНоменклатуры - СправочникСсылка.КлючиАналитикиУчетаНоменклатуры - аналитика учета прослеживаемой номенклатуры.
//		* НомерГТД - СправочникСсылка.НомераГТД - прослеживаемый номер ГТД.
//		* КоличествоПоРНПТ - Число -  количество товара в единице измерения ТН ВЭД.
//
Функция ПолучитьПрослеживаемыеНомераГТД(Товары) Экспорт
	...
	|	Комплектующие.НомерРНПТ				КАК НомерГТД,
	// Номенклатура разбираемого комплекта.
	|	МАКСИМУМ(ЕСТЬNULL(СправочникНомераГТД.Номенклатура, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))) КАК Номенклатура,
	|	ВЫРАЗИТЬ(СУММА(Комплектующие.КоличествоПоРНПТ * Товары.Количество) КАК ЧИСЛО(23, 11)) КАК КоличествоПоРНПТ
	|ПОМЕСТИТЬ ВтСписокКомплектующих
	|ИЗ
	|	ВтТовары КАК Товары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НомераГТД.ПрослеживаемыеКомплектующие КАК Комплектующие
	|		ПО Товары.НомерГТД = Комплектующие.Ссылка
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НомераГТД КАК СправочникНомераГТД
	|		ПО Товары.НомерГТД = СправочникНомераГТД.Ссылка
	|		
	...
	|
	|ГДЕ
	|	ЕСТЬNULL(СправочникНоменклатура.ПрослеживаемыйТовар, ЛОЖЬ)
	|
	|СГРУППИРОВАТЬ ПО
	|	Товары.АналитикаУчетаНоменклатуры,
	|	Комплектующие.НомерРНПТ
	...
	|		ПО СписокКомплектующих.НомерГТД = СправочникНомераГТД.Ссылка
	|ГДЕ
	|	СписокКомплектующих.Номенклатура <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|	И ЕСТЬNULL(СправочникНомераГТД.Номенклатура, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|;
	|
	|//////////////////////////////////////////////////////////////////////////////// 3
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	ЕСТЬNULL(СправочникНоменклатура.ПрослеживаемыйТовар, ЛОЖЬ)
	|	И СправочникНомераГТД.ТипНомераГТД = ЗНАЧЕНИЕ(Перечисление.ТипыНомеровГТД.НомерРНПТ)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	Товары.КоличествоПоРНПТ				КАК КоличествоПоРНПТ
	|ИЗ
	|	ВтСписокКомплектующих КАК Товары";
	
	Запрос.УстановитьПараметр("Товары", Товары);
	
	РезультатЗапроса	= Запрос.ВыполнитьПакет();
	КомплектующиеРНПТ	= РезультатЗапроса[РезультатЗапроса.ВГраница() - 1].Выбрать();
	Результат			= РезультатЗапроса[РезультатЗапроса.ВГраница()].Выгрузить(); // ТаблицаЗначений -
...
// 	ИмяРеквизитаНаименование - Строка - Имя реквизита с наименованием входящего документа.
//
Процедура ЗаполнитьСписокВыбораНаименованиеВходящегоДокумента(Форма, Контрагент, ИмяРеквизитаНаименование = "НаименованиеВходящегоДокумента") Экспорт
	Элемент = Форма.Элементы.НаименованиеВходящегоДокумента;
	Документ = Форма.Объект; // ДокументОбъектИмяДокумента - 
	ПолноеИмя = Документ.Ссылка.Метаданные().ПолноеИмя();
	...
	|		И &ИмяПоляНаименование <> &ПустаяСтрока
	|	УПОРЯДОЧИТЬ ПО
	|		Таблица.Дата УБЫВ) КАК ВложеннаяТаблица";
	
	ИмяПоляНаименование = "Таблица" + "." + ИмяРеквизитаНаименование;
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИмяПоляНаименование", ИмяПоляНаименование);
...
Процедура ЗаполнитьПоступленияИЦены(ДокументВозврата, ИмяТаблицы) Экспорт
	...
	СтруктураПересчетаСуммы = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(ДокументВозврата);
	...
	СтруктураДействий.Вставить("ПересчитатьСумму");
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ТабличнаяЧастьВозврата, СтруктураДействий, Неопределено);
...
Процедура СформироватьЗапросВременнаяТаблицаШапкаДокументаЗакупки(ТекстЗапроса,
	...
	ТаблицаДокумента.Колонки.Добавить("ПорядокРасчетов",                      Новый ОписаниеТипов("ПеречислениеСсылка.ПорядокРасчетов, Неопределено"));
	...
	ТаблицаДокумента.Колонки.Добавить("ФормаОплаты",                          Новый ОписаниеТипов("ПеречислениеСсылка.ФормыОплаты, Неопределено"));
...
// Формирует запросы по элементам справочника "Склады" и помещает во временную таблицу
// К корректным складам относятся:
// 		- Склад, равный складу соглашения из таблицы документа
// 		- Склады, входящие в иерархию склада соглашения из таблицы документа.
//
Процедура СформироватьЗапросВременнаяТаблицаСкладыКорректныеПоСоглашениям(ТекстЗапроса,  ПараметрыЗапроса, ДокументЗакупки)
	...
	Запрос.Текст = 
		...
		|	Склады.Ссылка КАК Склад,
		|	Склады.ЭтоГруппа КАК СкладГруппа
		|ПОМЕСТИТЬ СкладыСоглашений
		|ИЗ
		|	Справочник.СоглашенияСПоставщиками КАК СоглашенияСПоставщиками
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК Склады
		|		ПО СоглашенияСПоставщиками.Склад = Склады.Ссылка
		|ГДЕ
		|	СоглашенияСПоставщиками.Ссылка В(&Соглашения)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СкладыСоглашений.Склад КАК Склад
		|ИЗ
		|	СкладыСоглашений КАК СкладыСоглашений
		|ГДЕ
		|	СкладыСоглашений.СкладГруппа";
	
	Запрос.УстановитьПараметр("Соглашения", ДокументЗакупки.Товары.ВыгрузитьКолонку("Соглашение"));
	
	РезультатПакета = Запрос.ВыполнитьПакетСПромежуточнымиДанными();
	Выборка = РезультатПакета[0].Выбрать();
	
	СкладыГруппы = РезультатПакета[1].Выгрузить().ВыгрузитьКолонку("Склад");
	ИерархияСкладов = Новый Соответствие;
	ЗапросИерархии = Новый Запрос;
	ТекстыИерархии = Новый Массив;
	Для ИндексЭлемента = 0 По СкладыГруппы.ВГраница() Цикл
		
		СкладГруппа = СкладыГруппы.Получить(ИндексЭлемента);
		ИмяПараметра = "Склад" + Формат(ИндексЭлемента, "ЧГ=0");
		
		ТекстЗапросаИерархии = СтрЗаменить(
		"ВЫБРАТЬ
		|	&Склад КАК Родитель,
		|	Склады.Ссылка КАК Элемент
		|ИЗ
		|	Справочник.Склады КАК Склады
		|ГДЕ
		...
		ТекстыИерархии.Добавить(ТекстЗапросаИерархии);
		ЗапросИерархии.УстановитьПараметр(ИмяПараметра, СкладГруппа);
		
	КонецЦикла;
	
	Если ЗначениеЗаполнено(ТекстыИерархии) Тогда
		ЗапросИерархии.Текст = СтрСоединить(ТекстыИерархии, ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении());
		ВыборкаИерархии = ЗапросИерархии.Выполнить().Выбрать();
		Пока ВыборкаИерархии.Следующий() Цикл
			
			ЭлементыГруппы = ИерархияСкладов.Получить(ВыборкаИерархии.Родитель);
			Если ЭлементыГруппы = Неопределено Тогда
				ЭлементыГруппы = Новый Массив;
				ИерархияСкладов.Вставить(ВыборкаИерархии.Родитель, ЭлементыГруппы);
			КонецЕсли;
			
			ЭлементыГруппы.Добавить(ВыборкаИерархии.Элемент);
			
		КонецЦикла;
	КонецЕсли;
	
	СкладыСоглашений = Новый ТаблицаЗначений;
	СкладыСоглашений.Колонки.Добавить("Соглашение", Новый ОписаниеТипов("СправочникСсылка.СоглашенияСПоставщиками"));
	СкладыСоглашений.Колонки.Добавить("Склад", Новый ОписаниеТипов("СправочникСсылка.Склады"));
	
	Пока Выборка.Следующий() Цикл
		
		ИерархияСклада = ИерархияСкладов.Получить(Выборка.Склад); 
		Если Не ЗначениеЗаполнено(ИерархияСклада) Тогда
			ИерархияСклада = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Выборка.Склад);
		КонецЕсли;
		
		Для Каждого Склад Из ИерархияСклада Цикл
			
			СтрокаТаблицы = СкладыСоглашений.Добавить();
			СтрокаТаблицы.Соглашение = Выборка.Соглашение;
	...
	ПараметрыЗапроса.Вставить("СкладыСоглашений", СкладыСоглашений);
	ТекстЗапроса = ТекстЗапроса + 
	"ВЫБРАТЬ
	|	СкладыСоглашений.Соглашение КАК Соглашение,
	|	СкладыСоглашений.Склад КАК Склад
	|ПОМЕСТИТЬ СкладыКорректныеПоСоглашениям
	|ИЗ
	|	&СкладыСоглашений КАК СкладыСоглашений"
...
// Формирует текст запроса для проверки в документе закупки наличия строк с типом номенклатуры, отличного от типа "Работа".
//
// Параметры:
// ТекстЗапроса - Строка - текстовая строка, к которой необходимо добавить текст запроса.
//
Процедура СформироватьЗапросНаличиеТоваровУслугВДокументеЗакупки(ТекстЗапроса)
...
// Параметры:
// 	ТекстЗапроса - Строка - текстовая строка, к которой необходимо добавить текст запроса.
// 	ИмяТаблицы - Строка - имя таблицы документа, по которой выполняются проверки.
//
Процедура СформироватьЗапросШапкаДокументаЗакупки(ТекстЗапроса, ИмяТаблицы)
	
	СформироватьЗапросПроверкиУсловийЗакупки(ТекстЗапроса, ИмяТаблицы, Истина);
...
// ТекстЗапроса - Строка - текстовая строка, к которой необходимо добавить текст запроса.
// ИмяТаблицы - Строка - имя таблицы документа, по которой выполняются проверки.
Процедура СформироватьЗапросТоварыДокументаЗакупки(ТекстЗапроса, ИмяТаблицы)

	СформироватьЗапросПроверкиУсловийЗакупки(ТекстЗапроса, ИмяТаблицы, Ложь);
	
КонецПроцедуры

// Формирует текст запроса для проверки условий закупки
//
// Параметры:
//  ТекстЗапроса - Строка - текстовая строка, к которой необходимо добавить текст запроса.
//  ИмяТаблицы - Строка - имя таблицы документа, по которой выполняются проверки.
//  ЭтоПроверкаШапки - Булево - Определяет тип проверок: шапка или табличная часть.
Процедура СформироватьЗапросПроверкиУсловийЗакупки(ТекстЗапроса, ИмяТаблицы, ЭтоПроверкаШапки)
	
	ВыбираемыеПоля = Новый Массив;
	
	Если Не ЭтоПроверкаШапки Тогда
		ВыбираемыеПоля.Добавить("ВременнаяТаблицаТовары.НомерСтроки КАК НомерСтроки"); // @query-part
	КонецЕсли;
	
	// Инициализация описания размещения реквизитов:
	РазмещениеШапка   = "Шапка";
	РазмещениеТаблица = "Таблица";
	
	РазмещениеРеквизитов = Новый Структура;
	РазмещениеРеквизитов.Вставить("Партнер",         РазмещениеШапка);
	РазмещениеРеквизитов.Вставить("Контрагент",      РазмещениеШапка);
	РазмещениеРеквизитов.Вставить("Организация",     РазмещениеШапка);
	РазмещениеРеквизитов.Вставить("ФормаОплаты",     РазмещениеШапка);
	РазмещениеРеквизитов.Вставить("Валюта",          РазмещениеШапка);
	РазмещениеРеквизитов.Вставить("ЦенаВключаетНДС", РазмещениеШапка);
	РазмещениеРеквизитов.Вставить("Соглашение",      РазмещениеШапка);
	
	РазмещениеРеквизитов.Вставить("ВидЦеныПоставщика", РазмещениеТаблица);
	
	ПроверятьОтмененныеСтроки = Истина;
	
	// Нестандартное размещение реквизитов:
	Если ИмяТаблицы = "Документ.ЗаявкаНаЗакупку" Тогда
		РазмещениеРеквизитов.Партнер    = РазмещениеТаблица;
		РазмещениеРеквизитов.Контрагент = РазмещениеТаблица;
		РазмещениеРеквизитов.Соглашение = РазмещениеТаблица;
		
		ПроверятьОтмененныеСтроки = Ложь;
	КонецЕсли;
	
	ДобавлятьПроверкиСоглашения = ДобавлятьПроверкуПоСоглашению(РазмещениеРеквизитов, "Соглашение", ЭтоПроверкаШапки);
	
	#Область ПроверкиОдноименныхРеквизитовНаСоответствие
	Для Каждого ОписаниеПроверки Из ОписаниеПроверокСоответствияЗначенийСоглашению() Цикл
		
		ИмяРеквизита = ОписаниеПроверки.ИмяРеквизита;
		ПустоеЗначение = ОписаниеПроверки.ПустоеЗначениеВЗапросе;
		
		Если ОписаниеПроверки.ПереопределенРасчетУсловий Тогда
			Продолжить;
		КонецЕсли;
		
		Если Не ДобавлятьПроверкуПоСоглашению(РазмещениеРеквизитов, ИмяРеквизита, ЭтоПроверкаШапки) Тогда
			Продолжить;
		КонецЕсли;
		
		ТекстУсловия = "СоглашениеСПоставщиком.ИмяРеквизита ЕСТЬ НЕ NULL // Соглашение указано
		|		И ДокументЗакупки.ИмяРеквизита <> НЕОПРЕДЕЛЕНО // Реквизит существует в документе
		|		И &ПроверкаНаПустоеЗначениеВСоглашении // Значение в соглашении заполнено
		|		И &ПроверкаНаПустоеЗначениеВДокументе // Значение в документе заполнено
		|		И СоглашениеСПоставщиком.ИмяРеквизита <> ДокументЗакупки.ИмяРеквизита КАК ИмяПоляЕстьОшибки,
		|	ПРЕДСТАВЛЕНИЕ(СоглашениеСПоставщиком.ИмяРеквизита) КАК ПсевдонимРеквизитаСоглашения,
		|	ПРЕДСТАВЛЕНИЕ(ДокументЗакупки.ИмяРеквизита) КАК ИмяРеквизита"; // @query-part

		ЗаменяемаяСтрока = "&ПроверкаНаПустоеЗначениеВСоглашении";
		Если ПустаяСтрока(ПустоеЗначение) Тогда
			ТекстУсловия = СтрЗаменить(ТекстУсловия, ЗаменяемаяСтрока, "ИСТИНА");
		Иначе
			ТекстУсловия = СтрЗаменить(
				ТекстУсловия, 
				ЗаменяемаяСтрока,
				"СоглашениеСПоставщиком.ИмяРеквизита <> " + ПустоеЗначение);
		КонецЕсли;
		
		ЗаменяемаяСтрока = "&ПроверкаНаПустоеЗначениеВДокументе";
		Если Не ОписаниеПроверки.ТолькоПриЗаполненномЗначенииВДокументе Или ПустаяСтрока(ПустоеЗначение) Тогда
			ТекстУсловия = СтрЗаменить(ТекстУсловия, ЗаменяемаяСтрока, "ИСТИНА");
		Иначе
			ТекстУсловия = СтрЗаменить(
				ТекстУсловия, 
				ЗаменяемаяСтрока,
				"ДокументЗакупки.ИмяРеквизита <> " + ПустоеЗначение);
		КонецЕсли;
		
		Если РазмещениеРеквизитов[ИмяРеквизита] = РазмещениеТаблица Тогда
			ТекстУсловия = СтрЗаменить(ТекстУсловия, "ДокументЗакупки", "ВременнаяТаблицаТовары");
		КонецЕсли;
		
		ТекстУсловия = СтрЗаменить(ТекстУсловия,
			"ПсевдонимРеквизитаСоглашения",
			ОписаниеПроверки.ПсевдонимРеквизитаСоглашения);
		
		ТекстУсловия = СтрЗаменить(ТекстУсловия, "ИмяПоляЕстьОшибки", ОписаниеПроверки.ИмяПоляЕстьОшибки);
		ТекстУсловия = СтрЗаменить(ТекстУсловия, "ИмяРеквизита", ИмяРеквизита);
		
		ВыбираемыеПоля.Добавить(ТекстУсловия);
		
	КонецЦикла;
	#КонецОбласти
	
	ПоддерживаетсяРаботаСМногооборотнойТарой = ИмяТаблицы <> "Документ.ЗаявкаНаЗакупку";
	
	#Область ПроверкиСоглашенияПоПараметрам	
	Если ДобавлятьПроверкиСоглашения И ПоддерживаетсяРаботаСМногооборотнойТарой Тогда
		
		ШаблонУсловия = "СоглашениеСПоставщиком.%1 ЕСТЬ НЕ NULL
		|	И СоглашениеСПоставщиком.%1 <> &%2 КАК %3"; // @query-part
		
		ТекстУсловия = СтрШаблон(
			ШаблонУсловия,
			"ВозвращатьМногооборотнуюТару",
			"ВозвратМногооборотнойТары",
			"ЕстьОшибкиЗаполненияВозвращатьМногооборотнуюТару");
		ВыбираемыеПоля.Добавить(ТекстУсловия);
		...
		ТекстУсловия = СтрШаблон(
			...
			"ТребуетсяЗалогЗаТару",
			"ТребуетсяЗалогЗаТару",
			"ЕстьОшибкиЗаполненияТребуетсяЗалогЗаТару");
		ВыбираемыеПоля.Добавить(ТекстУсловия);
	
	КонецЕсли;
	#КонецОбласти
	
	Если ДобавлятьПроверкиСоглашения Тогда
		ВыбираемыеПоля.Добавить("СоглашениеСПоставщиком.Статус ЕСТЬ НЕ NULL И
			|	СоглашениеСПоставщиком.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыСоглашенийСПоставщиками.НеСогласовано) КАК ЕстьОшибкиСтатусСоглашения"); // @query-part
	КонецЕсли;
	
	#Область ХозяйственнаяОперация
	Если ДобавлятьПроверкиСоглашения
		И ИмяТаблицы <> "Документ.ЗаявкаНаЗакупку" // Нет хозяйственной операции
		Тогда
		...
		|	ПРЕДСТАВЛЕНИЕ(ДокументЗакупки.ХозяйственнаяОперация) КАК ХозяйственнаяОперация,
		|	ПРЕДСТАВЛЕНИЕ(СоглашениеСПоставщиком.ХозяйственнаяОперация) КАК ХозяйственнаяОперацияСоглашения"); // @query-part
		
	КонецЕсли;
	#КонецОбласти
	
	#Область Склад
	ПроверятьСкладПоСоглашениюВТабличнойЧасти = Ложь;
	Если ЭтоПроверкаШапки И ДобавлятьПроверкиСоглашения Тогда
		// Проверка склада в шапке по соглашению в шапке:
		ВыбираемыеПоля.Добавить(
		"СоглашениеСПоставщиком.Склад ЕСТЬ НЕ NULL
		|		И СоглашениеСПоставщиком.Склад <> ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		|		И СкладыКорректныеПоСоглашению.Склад ЕСТЬ NULL КАК ЕстьОшибкиЗаполненияСклад,
		|	ЕСТЬNULL(СоглашениеСПоставщиком.Склад.ЭтоГруппа, ЛОЖЬ) КАК СкладСоглашенияЕстьГруппа,
		|	ПРЕДСТАВЛЕНИЕ(ДокументЗакупки.Склад) КАК Склад,
		|	ПРЕДСТАВЛЕНИЕ(СоглашениеСПоставщиком.Склад) КАК СкладСоглашения"); // @query-part
	ИначеЕсли ИмяТаблицы = "Документ.ЗаявкаНаЗакупку" Тогда
		// Проверка склада в ТЧ по соглашению в ТЧ:
		ВыбираемыеПоля.Добавить(
		"ВременнаяТаблицаТовары.Склад <> ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		|		И СоглашениеСПоставщиком.Склад ЕСТЬ НЕ NULL
		|		И СоглашениеСПоставщиком.Склад <> ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		|		И СкладыКорректныеПоСоглашениям.Склад ЕСТЬ NULL КАК ЕстьОшибкиЗаполненияСклад,
		|	ЕСТЬNULL(СоглашениеСПоставщиком.Склад.ЭтоГруппа, ЛОЖЬ) КАК СкладСоглашенияЕстьГруппа,
		|	ЕСТЬNULL(ДокументЗакупки.Склад.ЭтоГруппа, ЛОЖЬ) КАК СкладВШапкеГруппа,
		|	ПРЕДСТАВЛЕНИЕ(ВременнаяТаблицаТовары.Склад) КАК Склад,
		|	ПРЕДСТАВЛЕНИЕ(СоглашениеСПоставщиком.Склад) КАК СкладСоглашения"); // @query-part
		
		ПроверятьСкладПоСоглашениюВТабличнойЧасти = Истина;
	КонецЕсли;
	#КонецОбласти
	...
	Если ЭтоПроверкаШапки И ДобавлятьПроверкиСоглашения Тогда
		...
		ВыбираемыеПоля.Добавить( 
		"ДокументЗакупки.ПорядокРасчетов <> НЕОПРЕДЕЛЕНО // Реквизит существует в документе
		|	И ЕСТЬNULL(ДокументЗакупки.ПорядокРасчетов <> ВЫБОР 
		|		КОГДА СоглашениеСПоставщиком.ИспользуютсяДоговорыКонтрагентов ТОГДА
		|			ДокументЗакупки.Договор.ПорядокРасчетов
		|		ИНАЧЕ
		|			СоглашениеСПоставщиком.ПорядокРасчетов
		|		КОНЕЦ, ЛОЖЬ) КАК ЕстьОшибкиЗаполненияПорядокРасчетов,
		|	ПРЕДСТАВЛЕНИЕ(ВЫБОР
		|		КОГДА СоглашениеСПоставщиком.ИспользуютсяДоговорыКонтрагентов
		|			ТОГДА ДокументЗакупки.Договор.ПорядокРасчетов
		|		ИНАЧЕ СоглашениеСПоставщиком.ПорядокРасчетов
		|	КОНЕЦ) КАК ПорядокРасчетовСоглашения,
		|	СоглашениеСПоставщиком.ИспользуютсяДоговорыКонтрагентов КАК ИспользуютсяДоговорыКонтрагентов"); // @query-part
		#КонецОбласти
	КонецЕсли;
	
	Если ИмяТаблицы = "Документ.ПриобретениеТоваровУслуг" Тогда
		#Область ВалютаВзаиморасчетов
		// Проверка также использует валюту соглашения, но поле добавляется в обязательной проверке валюты.
		ВыбираемыеПоля.Добавить(
		"СоглашениеСПоставщиком.Валюта ЕСТЬ НЕ NULL И
		...
		|			КАК ЕстьОшибкиЗаполненияВалютаВзаиморасчетов,
		|	ПРЕДСТАВЛЕНИЕ(ДокументЗакупки.ВалютаВзаиморасчетов) КАК ВалютаВзаиморасчетов,
		|	ПРЕДСТАВЛЕНИЕ(СоглашениеСПоставщиком.Валюта) КАК ВалютаВзаиморасчетовСоглашения"); // @query-part
		#КонецОбласти
	КонецЕсли;
	
	#Область МногооборотнаяТара
	Если ИмяТаблицы = "Документ.ПриобретениеТоваровУслуг" Тогда 
		ВыбираемыеПоля.Добавить("ИСТИНА КАК ЕстьОшибкиДатаВозвратаМногооборотнойТары,
		|	СоглашениеСПоставщиком.РассчитыватьДатуВозвратаТарыПоКалендарю КАК РассчитыватьДатуВозвратаТарыПоКалендарю,
		|	СоглашениеСПоставщиком.КалендарьВозвратаТары КАК КалендарьВозвратаТары"); // @query-part
	ИначеЕсли ПоддерживаетсяРаботаСМногооборотнойТарой Тогда
		ВыбираемыеПоля.Добавить(
		"ВЫБОР
		|		КОГДА ДокументЗакупки.СрокВозвратаМногооборотнойТары <> СоглашениеСПоставщиком.СрокВозвратаМногооборотнойТары
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ЕстьОшибкиЗаполненияСрокВозвратаМногооборотнойТары"); // @query-part
	КонецЕсли;
	ВыбираемыеПоля.Добавить("&ВозвратМногооборотнойТары КАК ВозвратМногооборотнойТары,
		|	СоглашениеСПоставщиком.СрокВозвратаМногооборотнойТары КАК СрокВозвратаМногооборотнойТары"); // @query-part
	#КонецОбласти
	
	Если Не ЭтоПроверкаШапки Тогда
		#Область МаксимальноДопустимаяЦена
		
		ТекстУсловия = 
		"ПРЕДСТАВЛЕНИЕ(ВременнаяТаблицаТовары.Номенклатура) КАК Номенклатура,
		|	ПРЕДСТАВЛЕНИЕ(ВременнаяТаблицаТовары.Характеристика) КАК Характеристика,
		|	ПРЕДСТАВЛЕНИЕ(ДокументЗакупки.Валюта) КАК ВалютаЦены,
		|	&МаксимальноДопустимаяЦена КАК МаксимальноДопустимаяЦена,
		|	ЕСТЬNULL(СоглашениеСПоставщиком.КонтролироватьЦеныЗакупки, ЛОЖЬ)
		|		И ЦеныНоменклатурыПоставщиковСрезПоследних.Цена ЕСТЬ НЕ NULL
		|		И ВременнаяТаблицаТовары.Цена > &МаксимальноДопустимаяЦена КАК ЕстьОшибкиМаксимальноДопустимаяЦена"; // @query-part
		
		ТекстМаксимальноДопустимаяЦена = "
		|	// МаксимальноДопустимая цена
		|	ВЫБОР
		|		КОГДА
		...
		|			* ВЫБОР
		|				КОГДА ДокументЗакупки.Валюта <> ЦеныНоменклатурыПоставщиковСрезПоследних.Валюта
		|					ТОГДА ВЫБОР
		...
		ТекстМаксимальноДопустимаяЦена = СтрЗаменить(ТекстМаксимальноДопустимаяЦена,
			"&ТекстЗапросаКоэффициентУпаковки1",
			Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки(
		...
		ТекстУсловия = СтрЗаменить(ТекстУсловия, "&МаксимальноДопустимаяЦена", ТекстМаксимальноДопустимаяЦена);
		ВыбираемыеПоля.Добавить(ТекстУсловия);
		#КонецОбласти
	КонецЕсли;
	...
	Если ЭтоПроверкаШапки Тогда
		...
		|		ПО ДокументЗакупки.Соглашение = СоглашениеСПоставщиком.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ СкладыКорректныеПоСоглашению КАК СкладыКорректныеПоСоглашению
		|		ПО ДокументЗакупки.Склад = СкладыКорректныеПоСоглашению.Склад"; // @query-part
	Иначе
		ТекстЗапросаПроверки = 
		"ВЫБРАТЬ
		|	&ВыбираемыеПоля
		|ИЗ
		|	ВременнаяТаблицаТовары КАК ВременнаяТаблицаТовары
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаДокументЗакупки КАК ДокументЗакупки
		|		ПО (ИСТИНА)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СоглашенияСПоставщиками КАК СоглашениеСПоставщиком
		|		ПО (ВременнаяТаблицаТовары.Соглашение = СоглашениеСПоставщиком.Ссылка)
		|// ЕстьОшибкиМаксимальноДопустимаяЦена
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.ЦеныНоменклатурыПоставщиков.СрезПоследних(КОНЕЦПЕРИОДА(&Дата, ДЕНЬ),
		|				(Партнер, ВидЦеныПоставщика, Номенклатура, Характеристика) В (&ОтборЦенНоменклатурыВидЦеныПоставщика)
		|) КАК ЦеныНоменклатурыПоставщиковСрезПоследних
		|ПО
		|	ВременнаяТаблицаТовары.Партнер = ЦеныНоменклатурыПоставщиковСрезПоследних.Партнер
		|	И ВременнаяТаблицаТовары.ВидЦеныПоставщика = ЦеныНоменклатурыПоставщиковСрезПоследних.ВидЦеныПоставщика
		|	И ВременнаяТаблицаТовары.Номенклатура = ЦеныНоменклатурыПоставщиковСрезПоследних.Номенклатура
		|	И ВременнаяТаблицаТовары.Характеристика = ЦеныНоменклатурыПоставщиковСрезПоследних.Характеристика 
		|	И СоглашениеСПоставщиком.КонтролироватьЦеныЗакупки
		|ЛЕВОЕ СОЕДИНЕНИЕ
		...
		|) КАК КурсыВалюты
		|	ПО ИСТИНА"; // @query-part
		
		Если ПроверятьСкладПоСоглашениюВТабличнойЧасти Тогда
			ТекстЗапросаПроверки = ТекстЗапросаПроверки + "
			|	ЛЕВОЕ СОЕДИНЕНИЕ СкладыКорректныеПоСоглашениям КАК СкладыКорректныеПоСоглашениям
			|	ПО ВременнаяТаблицаТовары.Соглашение = СкладыКорректныеПоСоглашениям.Соглашение 
			|		И ВременнаяТаблицаТовары.Склад = СкладыКорректныеПоСоглашениям.Склад"; // @query-part
		КонецЕсли;
		
		ТребуетсяСвязьСДокументомЗакупки = РазмещениеРеквизитов.Партнер = РазмещениеШапка;
		
		ОтборЦенНоменклатурыВидЦеныПоставщика =
		"ВЫБРАТЬ
		|	ВременнаяТаблицаТовары.Партнер КАК Партнер,
		|	ВременнаяТаблицаТовары.ВидЦеныПоставщика КАК ВидЦеныПоставщика,
		|	ВременнаяТаблицаТовары.Номенклатура КАК Номенклатура,
		|	ВременнаяТаблицаТовары.Характеристика КАК Характеристика
		|ИЗ
		|	ВременнаяТаблицаТовары КАК ВременнаяТаблицаТовары"
			+ ?(ТребуетсяСвязьСДокументомЗакупки, "
			|	ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаДокументЗакупки КАК ДокументЗакупки ПО (ИСТИНА)", "");
		
		ТекстЗапросаПроверки = СтрЗаменить(ТекстЗапросаПроверки,
		"&ОтборЦенНоменклатурыВидЦеныПоставщика",
		ОтборЦенНоменклатурыВидЦеныПоставщика);
		
		Если РазмещениеРеквизитов.Соглашение = РазмещениеШапка Тогда
			ТекстЗапросаПроверки = СтрЗаменить(
				ТекстЗапросаПроверки,
				"ВременнаяТаблицаТовары.Соглашение",
				"ДокументЗакупки.Соглашение");
		КонецЕсли;
		
		Если РазмещениеРеквизитов.Партнер = РазмещениеШапка Тогда
			ТекстЗапросаПроверки = СтрЗаменить(
				ТекстЗапросаПроверки,
				"ВременнаяТаблицаТовары.Партнер",
				"ДокументЗакупки.Партнер");
		КонецЕсли;
		
		Если Не ПроверятьОтмененныеСтроки Тогда
			ТекстЗапросаПроверки = ТекстЗапросаПроверки + "
			|ГДЕ
			|	Не ВременнаяТаблицаТовары.Отменено"; // @query-part
		КонецЕсли;
	...
	ТекстЗапросаПроверки = СтрЗаменить(ТекстЗапросаПроверки,
		"&ВыбираемыеПоля",
		СтрСоединить(ВыбираемыеПоля, ",
		|	")) + ОбщегоНазначения.РазделительПакетаЗапросов();
		
	ТекстЗапроса = ТекстЗапроса + ТекстЗапросаПроверки;
	
КонецПроцедуры

Функция ДобавлятьПроверкуПоСоглашению(РазмещениеРеквизитов, ИмяРеквизита, ЭтоПроверкаШапки)
	
	Если РазмещениеРеквизитов.Соглашение = "Таблица"
		И ЭтоПроверкаШапки Тогда
		// Нельзя проверить шапку по соглашению, если соглашение в табличной части.
		Возврат Ложь;
		
	КонецЕсли;
	
	Возврат 
		// Проверка соглашения выполняется всегда, если оно в табличной части:
		РазмещениеРеквизитов.Соглашение = "Таблица"
		// Размещение реквизита совпадает с вариантом проверки:
		Или РазмещениеРеквизитов[ИмяРеквизита] = "Шапка" = ЭтоПроверкаШапки;
	
КонецФункции

// Перечень полей объектов, описание их проверки на соответствие значениям реквизитов в соглашении, 
// а также шаблоны текстов выводимых сообщений.
// 
// Возвращаемое значение:
//  ТаблицаЗначений:
//   * ИмяРеквизита - Строка - Имя реквизита документа и соглашения.
//   * ПсевдонимРеквизитаСоглашения - Строка - Псевдоним для поля запроса, содержащего представление реквизита соглашения.
//   * ИмяПоляЕстьОшибки - Строка - Псевдоним для поля запроса, содержащего флаг (булево) наличия найденных в проверке ошибок.
//   * ПредставлениеРеквизита - Строка - Представление имени реквизита для вывода в отчет.
//   * ПустоеЗначениеВЗапросе - Строка - Если не пустая, то проверка реквизита выполняется только при 
//                                     отличии значения в соглашении от указанного (проверка, что значение в соглашении заполнено).
//   * ТолькоПриЗаполненномЗначенииВДокументе - Булево - Если Истина и реквизит не заполнен в документе, проверка не выполняется,
//                                                      чтобы не дублировать сообщение о незаполненности.
//   * ПереопределенРасчетУсловий - Булево - Проверка выполняется не по стандартному условию сравнения, но сообщения выводятся аналогичным алгоритмом.
//
//   * ШаблонОшибкиВШапке - Строка - Шаблон для текста сообщения, выводимого в случае, если 
//                                     реквизит находится в шапке документа.
//   * ШаблонОшибкиВТаблице - Строка - Шаблон для текста сообщения, выводимого в случае, если
//                                     реквизит находится в табличной части документа.
//   * ШаблонОшибкиВШапкеДляОтчета - Строка - Шаблон для текста сообщения, выводимого в отчет в случае, если
//                                     реквизит находится в шапке документа.
//   * ШаблонОшибкиВТаблицеДляОтчета - Строка - Шаблон для текста сообщения, выводимого в отчет в случае, если
//                                     реквизит находится в табличной части документа.
//   * ШаблонОшибкиВыбораСоглашенияВТаблице - Строка - Шаблон для текста сообщения, выводимого в ошибку или отчет в случае, если
//                                     реквизит в документе "первичен" и ошибка выводится на некорректно выбранное соглашение в табличной части.
//   * ШаблонОшибкиОтличияОтТиповогоСоглашения - Строка - Шаблон для текста сообщения, выводимого в случае, если 
//                                     реквизит находится в шапке соглашения.
//   * ШаблонОшибкиОтличияОтТиповогоСоглашенияДляОтчета - Строка - Шаблон для текста сообщения, выводимого в отчет в случае, если 
//                                     реквизит находится в шапке соглашения.
Функция ОписаниеПроверокСоответствияЗначенийСоглашению()
	
	ОписаниеТипаСтрока150 = ОбщегоНазначения.ОписаниеТипаСтрока(150);
	ОписаниеТипаНеограниченнаяСтрока = ОбщегоНазначения.ОписаниеТипаСтрока(0);
	ОписаниеТипаБулево = Новый ОписаниеТипов("Булево");
	
	ВозвращаемоеЗначение = Новый ТаблицаЗначений;
	ВозвращаемоеЗначение.Колонки.Добавить("ИмяРеквизита",                                     ОписаниеТипаСтрока150);
	ВозвращаемоеЗначение.Колонки.Добавить("ПсевдонимРеквизитаСоглашения",                     ОписаниеТипаСтрока150);
	ВозвращаемоеЗначение.Колонки.Добавить("ИмяПоляЕстьОшибки",                                ОписаниеТипаСтрока150);
	ВозвращаемоеЗначение.Колонки.Добавить("ПредставлениеРеквизита",                           ОписаниеТипаСтрока150);
	ВозвращаемоеЗначение.Колонки.Добавить("ПустоеЗначениеВЗапросе",                           ОписаниеТипаСтрока150);
	ВозвращаемоеЗначение.Колонки.Добавить("ТолькоПриЗаполненномЗначенииВДокументе",           ОписаниеТипаБулево); 
	ВозвращаемоеЗначение.Колонки.Добавить("ПереопределенРасчетУсловий",                       ОписаниеТипаБулево);
	
	// Тексты сообщений идентичны, но формируются явно для каждого случая для корректной работы перевода строк на другие языки.
	ВозвращаемоеЗначение.Колонки.Добавить("ШаблонОшибкиВШапке",                               ОписаниеТипаНеограниченнаяСтрока);
	ВозвращаемоеЗначение.Колонки.Добавить("ШаблонОшибкиВТаблице",                             ОписаниеТипаНеограниченнаяСтрока);
	ВозвращаемоеЗначение.Колонки.Добавить("ШаблонОшибкиВШапкеДляОтчета",                      ОписаниеТипаНеограниченнаяСтрока);
	ВозвращаемоеЗначение.Колонки.Добавить("ШаблонОшибкиВТаблицеДляОтчета",                    ОписаниеТипаНеограниченнаяСтрока);
	ВозвращаемоеЗначение.Колонки.Добавить("ШаблонОшибкиВыбораСоглашенияВТаблице",             ОписаниеТипаНеограниченнаяСтрока);
	ВозвращаемоеЗначение.Колонки.Добавить("ШаблонОшибкиОтличияОтТиповогоСоглашения",          ОписаниеТипаНеограниченнаяСтрока);
	ВозвращаемоеЗначение.Колонки.Добавить("ШаблонОшибкиОтличияОтТиповогоСоглашенияДляОтчета", ОписаниеТипаНеограниченнаяСтрока);
	
	ОписаниеПроверки = НовоеОписаниеПроверкиСоответствияЗначенияСоглашению(ВозвращаемоеЗначение, 
		"ХозяйственнаяОперация",
		"ХозяйственнаяОперацияСоглашения",
		"ЕстьОшибкиЗаполненияХозяйственнаяОперация",
		НСтр("ru = 'Операция'"));
	ОписаниеПроверки.ПереопределенРасчетУсловий = Истина;
	ОписаниеПроверки.ШаблонОшибкиВШапке = НСтр("ru = 'Операция не должна отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблице = НСтр("ru = 'Операция не должна отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВШапкеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от операции в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблицеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от операции в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВыбораСоглашенияВТаблице = НСтр("ru = 'Операция в соглашении (%ЗначениеРеквизитаСоглашения%) не должна отличаться от операции в документе (%ЗначениеРеквизитаДокумента%) в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашения = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от операции в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашенияДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от операции в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	
	ОписаниеПроверки = НовоеОписаниеПроверкиСоответствияЗначенияСоглашению(ВозвращаемоеЗначение, 
		"ВалютаВзаиморасчетов",
		"ВалютаВзаиморасчетовСоглашения",
		"ЕстьОшибкиЗаполненияВалютаВзаиморасчетов",
		НСтр("ru = 'Валюта взаиморасчетов'"));
	ОписаниеПроверки.ПереопределенРасчетУсловий = Истина;
	ОписаниеПроверки.ШаблонОшибкиВШапке = НСтр("ru = 'Валюта взаиморасчетов"" не должна отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблице = НСтр("ru = 'Валюта взаиморасчетов"" не должна отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВШапкеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от валюты взаиморасчетов в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблицеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от валюты взаиморасчетов в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВыбораСоглашенияВТаблице = НСтр("ru = 'Валюта взаиморасчетов"" в соглашении (%ЗначениеРеквизитаСоглашения%) не должна отличаться от валюты взаиморасчетов в документе (%ЗначениеРеквизитаДокумента%) в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашения = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от валюты взаиморасчетов в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашенияДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от валюты взаиморасчетов в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	
	ОписаниеПроверки = НовоеОписаниеПроверкиСоответствияЗначенияСоглашению(ВозвращаемоеЗначение, 
		"Партнер",
		"ПартнерСоглашения",
		"ЕстьОшибкиЗаполненияПартнер",
		НСтр("ru = 'Партнер'"),
		"ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)");
	ОписаниеПроверки.ТолькоПриЗаполненномЗначенииВДокументе = Истина;
	ОписаниеПроверки.ШаблонОшибкиВШапке = НСтр("ru = 'Партнер не должен отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблице = НСтр("ru = 'Партнер не должен отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВШапкеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от партнера в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблицеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от партнера в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВыбораСоглашенияВТаблице = НСтр("ru = 'Партнер в соглашении (%ЗначениеРеквизитаСоглашения%) не должен отличаться от партнера в документе (%ЗначениеРеквизитаДокумента%) в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашения = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от партнера в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашенияДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от партнера в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	
	ОписаниеПроверки = НовоеОписаниеПроверкиСоответствияЗначенияСоглашению(ВозвращаемоеЗначение, 
		"Контрагент",
		"КонтрагентСоглашения",
		"ЕстьОшибкиЗаполненияКонтрагент",
		НСтр("ru = 'Контрагент'"),
		"ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)");
	ОписаниеПроверки.ТолькоПриЗаполненномЗначенииВДокументе = Истина;
	ОписаниеПроверки.ШаблонОшибкиВШапке = НСтр("ru = 'Контрагент не должен отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблице = НСтр("ru = 'Контрагент не должен отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВШапкеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от контрагента в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблицеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от контрагента в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВыбораСоглашенияВТаблице = НСтр("ru = 'Контрагент в соглашении (%ЗначениеРеквизитаСоглашения%) не должен отличаться от контрагента в документе (%ЗначениеРеквизитаДокумента%) в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашения = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от контрагента в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашенияДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от контрагента в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	
	ОписаниеПроверки = НовоеОписаниеПроверкиСоответствияЗначенияСоглашению(ВозвращаемоеЗначение, 
		"Организация",
		"ОрганизацияСоглашения",
		"ЕстьОшибкиЗаполненияОрганизация",
		НСтр("ru = 'Организация'"),
		"ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)");
	ОписаниеПроверки.ТолькоПриЗаполненномЗначенииВДокументе = Истина;
	ОписаниеПроверки.ШаблонОшибкиВШапке = НСтр("ru = 'Организация не должна отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблице = НСтр("ru = 'Организация не должна отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВШапкеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от организации в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблицеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от организации в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВыбораСоглашенияВТаблице = НСтр("ru = 'Организация в соглашении (%ЗначениеРеквизитаСоглашения%) не должна отличаться от организации в документе (%ЗначениеРеквизитаДокумента%) в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашения = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от организации в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашенияДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от организации в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	
	ОписаниеПроверки = НовоеОписаниеПроверкиСоответствияЗначенияСоглашению(ВозвращаемоеЗначение, 
		"Валюта",
		"ВалютаСоглашения",
		"ЕстьОшибкиЗаполненияВалюта",
		НСтр("ru = 'Валюта'"));
	ОписаниеПроверки.ШаблонОшибкиВШапке = НСтр("ru = 'Валюта не должна отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблице = НСтр("ru = 'Валюта не должна отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВШапкеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от валюты в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблицеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от валюты в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВыбораСоглашенияВТаблице = НСтр("ru = 'Валюта в соглашении (%ЗначениеРеквизитаСоглашения%) не должна отличаться от валюты в документе (%ЗначениеРеквизитаДокумента%) в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашения = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от валюты в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашенияДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от валюты в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	
	ОписаниеПроверки = НовоеОписаниеПроверкиСоответствияЗначенияСоглашению(ВозвращаемоеЗначение, 
		"ФормаОплаты",
		"ФормаОплатыСоглашения",
		"ЕстьОшибкиЗаполненияФормаОплаты",
		НСтр("ru = 'Форма оплаты'"),
		"ЗНАЧЕНИЕ(Перечисление.ФормыОплаты.ПустаяСсылка)");
	ОписаниеПроверки.ШаблонОшибкиВШапке = НСтр("ru = 'Форма оплаты не должна отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблице = НСтр("ru = 'Форма оплаты не должна отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВШапкеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от формы оплаты в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблицеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от формы оплаты в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВыбораСоглашенияВТаблице = НСтр("ru = 'Форма оплаты в соглашении (%ЗначениеРеквизитаСоглашения%) не должна отличаться от формы оплаты в документе (%ЗначениеРеквизитаДокумента%) в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашения = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от формы оплаты в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашенияДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от формы оплаты в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	
	ОписаниеПроверки = НовоеОписаниеПроверкиСоответствияЗначенияСоглашению(ВозвращаемоеЗначение, 
		"ЦенаВключаетНДС",
		"ЦенаВключаетНДССоглашения",
		"ЕстьОшибкиЗаполненияЦенаВключаетНДС",
		НСтр("ru = 'Признак ""Цена включает НДС""'"));
	ОписаниеПроверки.ШаблонОшибкиВШапке = НСтр("ru = 'Признак ""Цена включает НДС"" не должен отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблице = НСтр("ru = 'Признак ""Цена включает НДС"" не должен отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВШапкеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от значения в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблицеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от значения в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВыбораСоглашенияВТаблице = НСтр("ru = 'Признак ""Цена включает НДС"" в соглашении (%ЗначениеРеквизитаСоглашения%) не должен отличаться от значения в документе (%ЗначениеРеквизитаДокумента%) в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашения = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от значения в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашенияДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от значения в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	
	ОписаниеПроверки = НовоеОписаниеПроверкиСоответствияЗначенияСоглашению(ВозвращаемоеЗначение, 
		"ВидЦеныПоставщика",
		"ВидЦеныПоставщикаСоглашения",
		"ЕстьОшибкиЗаполненияВидЦеныПоставщика",
		НСтр("ru = 'Вид цен поставщика'"),
		"ЗНАЧЕНИЕ(Справочник.ВидыЦенПоставщиков.ПустаяСсылка)");
	ОписаниеПроверки.ШаблонОшибкиВШапке = НСтр("ru = 'Вид цен поставщика не должен отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблице = НСтр("ru = 'Вид цен поставщика не должен отличаться от значения в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВШапкеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от вида цен поставщика в соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиВТаблицеДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от вида цен поставщика в соглашении ""%ЗначениеРеквизитаСоглашения%"" в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиВыбораСоглашенияВТаблице = НСтр("ru = 'Вид цен поставщика в соглашении (%ЗначениеРеквизитаСоглашения%) не должен отличаться от вида цен поставщика в документе (%ЗначениеРеквизитаДокумента%) в строке %НомерСтроки% списка %ИмяТабличнойЧасти%'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашения = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от вида цен поставщика в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашенияДляОтчета = НСтр("ru = '""%ЗначениеРеквизитаДокумента%"" отличается от вида цен поставщика в типовом соглашении ""%ЗначениеРеквизитаСоглашения%""'");
	
	Возврат ВозвращаемоеЗначение;
	
КонецФункции

Функция НовоеОписаниеПроверкиСоответствияЗначенияСоглашению(ОписаниеПроверок, ИмяРеквизита, ПсевдонимРеквизитаСоглашения,
	ИмяПоляЕстьОшибки,
	ПредставлениеРеквизита, ПустоеЗначениеВЗапросе = "")
	
	ОписаниеПроверки = ОписаниеПроверок.Добавить();
	
	ОписаниеПроверки.ИмяРеквизита = ИмяРеквизита;
	ОписаниеПроверки.ПсевдонимРеквизитаСоглашения = ПсевдонимРеквизитаСоглашения;
	ОписаниеПроверки.ИмяПоляЕстьОшибки = ИмяПоляЕстьОшибки;
	ОписаниеПроверки.ПредставлениеРеквизита = ПредставлениеРеквизита;
	ОписаниеПроверки.ПустоеЗначениеВЗапросе = ПустоеЗначениеВЗапросе;
...
// 	ДокументЗакупки  - ДокументОбъект - объект, для которого необходимо вывести сообщения об ошибках
// 	Отказ            - Булево - Флаг отказа от проведения документа
// 	Отклонения       - Массив - Массив отклонений условий продаж по шапке документа.
//
Процедура СообщитьОбОшибкахШапкаДокументаЗакупки(Выборка,
	                                             ДокументЗакупки,
	                                             Отказ,
	                                             Отклонения = Неопределено)
	
	СообщитьОбОшибкахУсловийЗакупки(Выборка, ДокументЗакупки, Отказ, Отклонения);
...
Процедура СообщитьОбОшибкахТоварыДокументаЗакупки(Выборка,
	...
	СообщитьОбОшибкахУсловийЗакупки(Выборка, ДокументЗакупки, Отказ, ДеревоОтклонений);
	
КонецПроцедуры

// Выводит сообщения об ошибках в документе закупки
//
// Параметры:
// 	Выборка          - ВыборкаИзРезультатаЗапроса.
// 	ДокументЗакупки  - ДокументОбъект - объект, для которого необходимо вывести сообщения об ошибках.
// 	Отказ            - Булево - Флаг отказа от проведения документа.
// 	Отклонения       - Массив, ДеревоЗначений, Неопределено - Отклонения от условий закупки.
//
Процедура СообщитьОбОшибкахУсловийЗакупки(Выборка, ДокументЗакупки, Отказ, Отклонения)
	
	ВыводитьВОтчет = Отклонения <> Неопределено;

	НаличиеПроверок = Новый Структура; //Описывает наличие колонок с проверками в выборке запроса.
	
	// Описание неуниверсальных проверок:
	НаличиеПроверок.Вставить("ЕстьОшибкиСтатусСоглашения");
	НаличиеПроверок.Вставить("ЕстьОшибкиЗаполненияСклад");
	НаличиеПроверок.Вставить("ЕстьОшибкиЗаполненияВозвращатьМногооборотнуюТару");
	НаличиеПроверок.Вставить("ЕстьОшибкиЗаполненияТребуетсяЗалогЗаТару");
	НаличиеПроверок.Вставить("ЕстьОшибкиДатаВозвратаМногооборотнойТары");
	НаличиеПроверок.Вставить("ЕстьОшибкиЗаполненияСрокВозвратаМногооборотнойТары");
	НаличиеПроверок.Вставить("ЕстьОшибкиЗаполненияПорядокРасчетов");
	НаличиеПроверок.Вставить("ЕстьОшибкиМаксимальноДопустимаяЦена");
	
	// Универсальные проверки:
	ОписаниеПроверок = ОписаниеПроверокСоответствияЗначенийСоглашению();
	Для Каждого ОписаниеПроверки Из ОписаниеПроверок Цикл
		НаличиеПроверок.Вставить(ОписаниеПроверки.ИмяПоляЕстьОшибки);
	КонецЦикла;
	
	КолонкиВыборки = Выборка.Владелец().Колонки;
	
	ЗаполнитьЗначенияСвойств(НаличиеПроверок, КолонкиВыборки);
	ЭтоПроверкаШапки = КолонкиВыборки.Найти("НомерСтроки") = Неопределено;
	
	Для Каждого КлючИЗначение Из НаличиеПроверок Цикл
		ИмяПроверки = КлючИЗначение.Ключ;
		НаличиеПроверок[ИмяПроверки] = КлючИЗначение.Значение <> Неопределено;
	КонецЦикла;
	
	ТипДокументаЗакупки = ТипЗнч(ДокументЗакупки);
	ЭтоСоглашение = ТипДокументаЗакупки = Тип("СправочникОбъект.СоглашенияСПоставщиками")
		Или ТипДокументаЗакупки = Тип("СправочникСсылка.СоглашенияСПоставщиками"); 
	ЭтоЗаявкаНаЗакупку = ТипДокументаЗакупки = Тип("ДокументОбъект.ЗаявкаНаЗакупку")
		Или ТипДокументаЗакупки = Тип("ДокументСсылка.ЗаявкаНаЗакупку"); // Для отчета отклонений
	
	ПараметрыВыводаОшибок = Новый Структура;
	
	ПараметрыВыводаОшибок.Вставить("ДокументОбъект",   ДокументЗакупки);
	ПараметрыВыводаОшибок.Вставить("Отказ",            Ложь);
	
	ПараметрыВыводаОшибок.Вставить("ВыводитьВОтчет",   ВыводитьВОтчет);
	ПараметрыВыводаОшибок.Вставить("Отклонения",       Отклонения);
	
	ПараметрыВыводаОшибок.Вставить("ЭтоПроверкаШапки",  ЭтоПроверкаШапки);
	ПараметрыВыводаОшибок.Вставить("ИмяТабличнойЧасти", "Товары");
	ПараметрыВыводаОшибок.Вставить("НомерСтроки",       0);
	
	Пока Выборка.Следующий() Цикл
		
		Если Не ЭтоПроверкаШапки Тогда
			ПараметрыВыводаОшибок.НомерСтроки = Выборка.НомерСтроки;
		КонецЕсли;
		
		АдресОшибки = "";
	    Если Не ЭтоПроверкаШапки И Не ВыводитьВОтчет Тогда
		...
		Для Каждого ОписаниеПроверки Из ОписаниеПроверок Цикл
			...
			ИмяПроверки = ОписаниеПроверки.ИмяПоляЕстьОшибки;
			Если Не НаличиеПроверок[ИмяПроверки] Или Не Выборка[ИмяПроверки] Тогда
				Продолжить;
			КонецЕсли;
			
			Если ЭтоСоглашение Тогда
				
				// Вывод сообщений для типовых соглашений:
				ТекстОшибки = ?(ВыводитьВОтчет,
					ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашенияДляОтчета,
					ОписаниеПроверки.ШаблонОшибкиОтличияОтТиповогоСоглашения);
			
			ИначеЕсли ЭтоЗаявкаНаЗакупку Тогда
				
				// Вывод сообщений о неправильной выборе соглашения в табличной части:
				ТекстОшибки = ОписаниеПроверки.ШаблонОшибкиВыбораСоглашенияВТаблице;
			
			ИначеЕсли ЭтоПроверкаШапки Тогда
				
				// Вывод сообщений о реквизитах шапки: 
				ТекстОшибки = ?(ВыводитьВОтчет,
					ОписаниеПроверки.ШаблонОшибкиВШапкеДляОтчета,
					ОписаниеПроверки.ШаблонОшибкиВШапке);
					
			Иначе // Вывод сообщений о реквизитах в табличной части:
				
				ТекстОшибки = ?(ВыводитьВОтчет,
					ОписаниеПроверки.ШаблонОшибкиВТаблицеДляОтчета,
					ОписаниеПроверки.ШаблонОшибкиВТаблице);
			
			КонецЕсли;
			
			// Подстановка параметров в полученный шаблон:
			ТекстОшибки = СтрЗаменить(ТекстОшибки,
				"%ЗначениеРеквизитаДокумента%",
				Выборка[ОписаниеПроверки.ИмяРеквизита]);
				
			ТекстОшибки = СтрЗаменить(ТекстОшибки,
				"%ЗначениеРеквизитаСоглашения%",
				Выборка[ОписаниеПроверки.ПсевдонимРеквизитаСоглашения]);
				
			ТекстОшибки = СтрЗаменить(ТекстОшибки,
				"%ИмяТабличнойЧасти%",
				ПараметрыВыводаОшибок.ИмяТабличнойЧасти);
				
			ТекстОшибки = СтрЗаменить(ТекстОшибки,
				"%НомерСтроки%",
				ПараметрыВыводаОшибок.НомерСтроки);
				
			СообщитьОбОшибкеВУсловииЗакупки(ПараметрыВыводаОшибок,
				ТекстОшибки,
				ОписаниеПроверки.ИмяРеквизита,
				ОписаниеПроверки.ПредставлениеРеквизита);
				
		КонецЦикла;
		
		Если НаличиеПроверок.ЕстьОшибкиСтатусСоглашения И Выборка.ЕстьОшибкиСтатусСоглашения Тогда
			
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Соглашение находится в статусе ""Не согласовано""%1.%2'"),
						?(ВыводитьВОтчет, "", АдресОшибки),
						?(ВыводитьВОтчет Или Не ЭтоПроверкаШапки, "", " " + НСтр("ru = 'Проведение документа запрещено.'")));
		...
		Если НаличиеПроверок.ЕстьОшибкиЗаполненияСклад И Выборка.ЕстьОшибкиЗаполненияСклад Тогда
			
			Если ЭтоЗаявкаНаЗакупку Тогда
				
				Если Выборка.СкладСоглашенияЕстьГруппа Тогда
					ТекстОшибки = НСтр("ru = 'Группа складов соглашения ""%СкладСоглашения%"" не содержит склад-получатель ""%Склад%""%АдресОшибки%'");
				Иначе
					ТекстОшибки = НСтр("ru = 'Склад соглашения ""%СкладСоглашения%"" не должен отличаться от значения в документе ""%Склад%""%АдресОшибки%'");
				КонецЕсли;
			...
			ИначеЕсли НЕ ВыводитьВОтчет Тогда
				Если Выборка.СкладСоглашенияЕстьГруппа Тогда
					ТекстОшибки = НСтр("ru = 'Склад должен входить в группу складов ""%СкладСоглашения%""'");
				Иначе
					ТекстОшибки = НСтр("ru = 'Склад не должен отличаться от значения в соглашении ""%СкладСоглашения%""'");
				КонецЕсли;
			Иначе 
				Если Выборка.СкладСоглашенияЕстьГруппа Тогда
					ТекстОшибки = НСтр("ru = '""%Склад%"" должен входить в группу складов соглашения ""%СкладСоглашения%""'");
				Иначе
					ТекстОшибки = НСтр("ru = '""%Склад%"" отличается от склада в соглашении ""%СкладСоглашения%""'");
				КонецЕсли;
			КонецЕсли;
			
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%СкладСоглашения%", Выборка.СкладСоглашения);
		...
		#Область МногооборотнаяТара
		Если НаличиеПроверок.ЕстьОшибкиЗаполненияВозвращатьМногооборотнуюТару И Выборка.ЕстьОшибкиЗаполненияВозвращатьМногооборотнуюТару Тогда
			...
			Если НЕ ВыводитьВОтчет Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					ТекстОшибки,
					ДокументЗакупки,
					?(ЭтоСоглашение,"ВозвращатьМногооборотнуюТару","ВернутьМногооборотнуюТару"),
					,
					Отказ);
			Иначе 
				Отчеты.ОтклоненияОтУсловийЗакупок.ДобавитьОтклонениеШапкиВМассивОтклонений(НСтр("ru = 'Возврат тары'"), ТекстОшибки, Отклонения);
			КонецЕсли;
		КонецЕсли;
		
		Если НаличиеПроверок.ЕстьОшибкиЗаполненияСрокВозвратаМногооборотнойТары И Выборка.ЕстьОшибкиЗаполненияСрокВозвратаМногооборотнойТары И Выборка.ВозвратМногооборотнойТары Тогда
			...
			Иначе 
				...
				Отчеты.ОтклоненияОтУсловийЗакупок.ДобавитьОтклонениеШапкиВМассивОтклонений(НСтр("ru = 'Срок возврата тары'"), ТекстОшибки, Отклонения);
			КонецЕсли;

		КонецЕсли;
		
		Если НаличиеПроверок.ЕстьОшибкиЗаполненияТребуетсяЗалогЗаТару И Выборка.ЕстьОшибкиЗаполненияТребуетсяЗалогЗаТару Тогда
			...
			Если НЕ ВыводитьВОтчет Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					...
					Отказ);
			Иначе 
				Отчеты.ОтклоненияОтУсловийЗакупок.ДобавитьОтклонениеШапкиВМассивОтклонений(НСтр("ru = 'Требование залога за тару'"), ТекстОшибки, Отклонения);
			КонецЕсли;
		КонецЕсли;
		
		Если НаличиеПроверок.ЕстьОшибкиДатаВозвратаМногооборотнойТары И Выборка.ЕстьОшибкиДатаВозвратаМногооборотнойТары И Выборка.ВозвратМногооборотнойТары Тогда
			...
			Если ДатаВозвратаМногооборотнойТары <> ДокументЗакупки.ДатаВозвратаМногооборотнойТары Тогда
				...
				Если НЕ ВыводитьВОтчет Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
							...
							Отказ);
				Иначе 
					Отчеты.ОтклоненияОтУсловийЗакупок.ДобавитьОтклонениеШапкиВМассивОтклонений(НСтр("ru = 'Дата возврата тары'"), ТекстОшибки, Отклонения);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		#КонецОбласти
		
		Если НаличиеПроверок.ЕстьОшибкиЗаполненияПорядокРасчетов И Выборка.ЕстьОшибкиЗаполненияПорядокРасчетов Тогда
			Если Выборка.ИспользуютсяДоговорыКонтрагентов Тогда
				...
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ПорядокРасчетов%", Выборка.ПорядокРасчетовСоглашения);
				Если НЕ ВыводитьВОтчет Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						ТекстОшибки,
						ДокументЗакупки,
						...
						Отказ);
				Иначе 
					Отчеты.ОтклоненияОтУсловийЗакупок.ДобавитьОтклонениеШапкиВМассивОтклонений(НСтр("ru = 'Порядок расчетов'"), ТекстОшибки, Отклонения);
				КонецЕсли;
			Иначе
				Если ЭтоСоглашение Тогда
		...
		Если НаличиеПроверок.ЕстьОшибкиМаксимальноДопустимаяЦена И Выборка.ЕстьОшибкиМаксимальноДопустимаяЦена Тогда
			...
			Если ВыводитьВОтчет Тогда
				Отчеты.ОтклоненияОтУсловийЗакупок.ДобавитьОтклонениеСтрокиВДеревоОтклонений(Выборка.НомерСтроки, "Цена", ТекстОшибки, Отклонения);
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					ТекстОшибки,
					ДокументЗакупки,
	...
	Если ПараметрыВыводаОшибок.Отказ Тогда
		Отказ = Истина;
	КонецЕсли;
	
КонецПроцедуры

// Выводит сообщение на экран / в коллекцию отклонений для отчета
// Параметры:
//  ПараметрыВыводаОшибок - Структура:
//    * ДокументОбъект - ДокументОбъект - К которому относится ошибка
//    * Отказ - Булево - Возвращаемый параметр. Устанавливает отказ для обработки проверки заполнения
//
//    * ВыводитьВОтчет - Булево - Определяет вариант вывода: Истина - в коллекцию отклонений, Ложь - на экран.
//    * Отклонения - Массив, ДеревоЗначений - Коллекция для отчета.
//
//    * ЭтоПроверкаШапки - Булево - Определяет позиционирование сообщения: Истина - в шапку, ложь - в табличную часть.
//    * ИмяТабличнойЧасти - Строка - Имя табличной части документа.
//    * НомерСтроки - Число - для позиционирования сообщения.
//  ТекстОшибки - Строка - выводимое сообщение.
//  ИмяРеквизита - Строка - Имя реквизита для позиционирования сообщения об ошибке.
//  ПредставлениеРеквизита - Строка - Для вывода в отчет.
Процедура СообщитьОбОшибкеВУсловииЗакупки(ПараметрыВыводаОшибок, ТекстОшибки, ИмяРеквизита, ПредставлениеРеквизита)
	
	Если ПараметрыВыводаОшибок.ВыводитьВОтчет
		И ПараметрыВыводаОшибок.ЭтоПроверкаШапки Тогда
		
		Отчеты.ОтклоненияОтУсловийЗакупок.ДобавитьОтклонениеШапкиВМассивОтклонений(
			ПредставлениеРеквизита,
			ТекстОшибки,
			ПараметрыВыводаОшибок.Отклонения);
		
	ИначеЕсли ПараметрыВыводаОшибок.ВыводитьВОтчет
		И Не ПараметрыВыводаОшибок.ЭтоПроверкаШапки Тогда
		
		Отчеты.ОтклоненияОтУсловийЗакупок.ДобавитьОтклонениеСтрокиВДеревоОтклонений(
			ПараметрыВыводаОшибок.НомерСтроки,
			ПредставлениеРеквизита,
			ТекстОшибки,
			ПараметрыВыводаОшибок.Отклонения);
		
	Иначе // Вывод сообщения пользователю:
			
		Если ПараметрыВыводаОшибок.ЭтоПроверкаШапки Тогда
			
			ПутьКРеквизиту = ИмяРеквизита;
			
		Иначе
			
			ИмяРеквизитаТаблицы = ИмяРеквизита;
			Если ТипЗнч(ПараметрыВыводаОшибок.ДокументОбъект) = Тип("ДокументОбъект.ЗаявкаНаЗакупку") Тогда
				// Реквизиты выведены в "подвал", значения в таблице могут быть скрыты. Выводим на реквизит, отображаемый по умолчанию:
				ИмяРеквизитаТаблицы = "ПредставлениеУсловийЗакупки";
			КонецЕсли;
		...
		ОбщегоНазначения.СообщитьПользователю(
			ТекстОшибки,
			ПараметрыВыводаОшибок.ДокументОбъект,
			ПутьКРеквизиту,
			,
			ПараметрыВыводаОшибок.Отказ);
...
// Выводит сообщения об ошибках наличия товаров и услуг в документе закупки
//
// Параметры:
// Выборка         - ВыборкаИзРезультатаЗапроса
// ДокументЗакупки - ДокументОбъект - объект, для которого необходимо вывести сообщения об ошибках
// Отказ           - Булево - Флаг отказа от проведения документа.
//
Процедура СообщитьОбОшибкахНаличиеТоваровУслугРаботВДокументеЗакупки(Выборка, ДокументЗакупки, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = НСтр("ru = 'Нельзя указывать номенклатуру ""%Номенклатура%"" с типом ""%ТипНоменклатуры%"" (строка %НомерСтроки% списка ""Товары""), для документа с типом договора ""%ТипДоговора%""'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Номенклатура%",          Выборка.Номенклатура);
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ТипНоменклатуры%",       Выборка.ТипНоменклатуры);
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%",           Выборка.НомерСтроки);
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ТипДоговора%",           ТипДоговора);
...
Процедура СообщитьОбОшибкахКорректностьЭтаповГрафикаОплаты(Знач Выборка, Знач ДокументЗакупки, Отказ, ДеревоОтклонений = Неопределено, МассивОтклонений = Неопределено)
	...
	Если Выборка.Следующий() И НЕ Выборка.НакладнаяПоЗаказам Тогда
		...
		Иначе
			МенеджерДокумента = ОбщегоНазначения.МенеджерОбъектаПоСсылке(ДокументЗакупки.Ссылка);
			ПараметрыВзаиморасчеты = МенеджерДокумента.ПараметрыВзаиморасчеты(ДокументЗакупки);
			Если ТипЗнч(ПараметрыВзаиморасчеты) = Тип("Массив") Тогда
				СтруктураПараметров = ПараметрыВзаиморасчеты[0];
			Иначе
				СтруктураПараметров = ПараметрыВзаиморасчеты;
			КонецЕсли;
			ТаблицаТовары = ВзаиморасчетыСервер.ТаблицаСуммПоЗаказам(ДокументЗакупки, СтруктураПараметров);
			ПараметрыЗаполнения.ТабличнаяЧасть = ТаблицаТовары;
...
Функция ТекстЗапросаФормированияТаблицыНомеровГТД(ВариантПолученияДанных)
	...
	Если ВариантПолученияДанных = "ПоРаспоряжениям" Тогда
		...
		|	СУММА(НаборДанных.Количество)  КАК Количество,
		|	ВЫБОР
		|		КОГДА СУММА(НаборДанных.Количество) = 0
		|			ТОГДА СУММА(НаборДанных.Сумма)
		|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(НаборДанных.Сумма) / СУММА(НаборДанных.Количество) КАК ЧИСЛО(31,2))
		|	КОНЕЦ КАК Цена
		...
		|		(ТоварыРеализации.СуммаСНДС - ВЫБОР КОГДА ДанныеДокумента.ЦенаВключаетНДС ТОГДА 0 ИНАЧЕ ТоварыРеализации.СуммаНДС КОНЕЦ) КАК Сумма
		|	ИЗ
		|		ВТТаблицаОтбора КАК ТаблицаОтбора
		...
		|			ПО ТоварыРеализации.Ссылка = ДанныеДокумента.Ссылка
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|	ИЗ
		|		ВТТаблицаОтбора КАК ТаблицаОтбора
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтгрузкаТоваровКлиенту.ВидыЗапасов КАК ТоварыРеализации
		|			ПО ТаблицаОтбора.ДокументРеализации = ТоварыРеализации.Ссылка
		|				И ТаблицаОтбора.Номенклатура = ТоварыРеализации.АналитикаУчетаНоменклатуры.Номенклатура
		|				И ТаблицаОтбора.Характеристика = ТоварыРеализации.АналитикаУчетаНоменклатуры.Характеристика
		|				И ТаблицаОтбора.Назначение = ТоварыРеализации.АналитикаУчетаНоменклатуры.Назначение
		...
		|				И ТаблицаОтбора.Серия = ТоварыРеализации.АналитикаУчетаНоменклатуры.Серия
		|			
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВыкупВозвратнойТарыКлиентом КАК ДанныеДокумента
		|			ПО ТоварыРеализации.Ссылка = ДанныеДокумента.Ссылка
		|	
		|	) КАК НаборДанных
		|
		...
		|	ЕСТЬNULL(РеализованныеТовары.Цена, 0)  КАК Цена
		|ИЗ
		...
		|	ЕСТЬNULL(РеализованныеТовары.Цена, 0)
		|
		|ИМЕЮЩИЕ
	...
	ИначеЕсли ВариантПолученияДанных = "ПоОстаткамУХранителя" Тогда
		...
		|	ГДЕ
		|		ТоварыОрганизаций.АналитикаУчетаНоменклатуры В
		|				(ВЫБРАТЬ
		|					ВТТаблицаОтбора.КлючАналитики КАК КлючАналитики
		|				ИЗ
		|					ВТТаблицаОтбора КАК ВТТаблицаОтбора)
		...
		|		И ТоварыОрганизаций.НомерГТД <> ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)) КАК НаборДанных
		|ГДЕ
		|	НаборДанных.Количество > 0
		|
		|СГРУППИРОВАТЬ ПО
		|	НаборДанных.АналитикаУчетаНоменклатуры,
		|	НаборДанных.НомерГТД
		...
		|	ВТТаблицаПереданные.Количество                                КАК Количество
		|ИЗ
		|	ВТТаблицаПереданные КАК ВТТаблицаПереданные";
		
	ИначеЕсли ВариантПолученияДанных = "ПоОстаткамВПути" Тогда
		...
		|	ТаблицаОтбора.Назначение                           КАК Назначение,
		|	ВЫБОР
		|		КОГДА ТаблицаОтбора.СтатусУказанияСерийПереданныхТоваров <> 0
		|			ТОГДА ТаблицаОтбора.Серия
		|		ИНАЧЕ
		|			ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ                                              КАК Серия,
		|	ТаблицаОтбора.СтатусУказанияСерийПереданныхТоваров КАК СтатусУказанияСерийПереданныхТоваров
		|ПОМЕСТИТЬ ВТВнешняяТаблица
		|ИЗ
		|	&ТаблицаТовары КАК ТаблицаОтбора
		|;
		...
		|	ВТТаблицаПереданные.НомерГТД.СтранаПроисхождения                КАК СтранаПроисхождения,
		|	ВТТаблицаПереданные.Количество                                  КАК Количество,
		|	ЕСТЬNULL(ТаблицаОтбора.СтатусУказанияСерийПереданныхТоваров, 0) КАК СтатусУказанияСерийПереданныхТоваров
		|ИЗ
		|	ВТТаблицаПереданные КАК ВТТаблицаПереданные
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТВнешняяТаблица КАК ТаблицаОтбора
		|		ПО ВТТаблицаПереданные.АналитикаУчетаНоменклатуры.Номенклатура = ТаблицаОтбора.Номенклатура
		|			И ВТТаблицаПереданные.АналитикаУчетаНоменклатуры.Характеристика = ТаблицаОтбора.Характеристика
		|			И ВТТаблицаПереданные.АналитикаУчетаНоменклатуры.Назначение = ТаблицаОтбора.Назначение
		|			И ВТТаблицаПереданные.АналитикаУчетаНоменклатуры.Серия = ТаблицаОтбора.Серия";
...
Процедура ЗаполнитьНомераГТДПоУчетнымДаннымВнутренний(ТоварыКЗаполнению, НомераГТДКЗаполнению,
	ДополнительныеДанные)
	
	ДействияСоСтрокой = ДополнительныеДанные.ДействияСоСтрокой;
	ЗаполнитьТипНомераГТД = ДополнительныеДанные.ЗаполнитьТипНомераГТД;
	ЗаполнятьКоличествоУпаковок = ДополнительныеДанные.ЗаполнятьКоличествоУпаковок;
	ЗаполнятьСтрануПроисхождения = ДополнительныеДанные.ЗаполнятьСтрануПроисхождения;
	ПоляПоиска = ДополнительныеДанные.ПоляПоиска;
	ИсключитьСкладскиеСерииИзПоиска = ДополнительныеДанные.ИсключитьСкладскиеСерииИзПоиска;
	
	СтруктураПоиска = Новый Структура(ПоляПоиска);
	СтруктураПоискаБезСерий = Новый Структура(ПоляПоиска);
	СтруктураПоискаБезСерий.Удалить("Серия");
	
	Для Каждого СтрокаНомераГТДКЗаполнению Из НомераГТДКЗаполнению Цикл
		
		Если СтрокаНомераГТДКЗаполнению.Количество <= 0
			Или СтрокаНомераГТДКЗаполнению.НомерГТД = Справочники.НомераГТД.ПустаяСсылка() Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ИсключитьСкладскиеСерииИзПоиска
			И СтрокаНомераГТДКЗаполнению.СтатусУказанияСерийПереданныхТоваров = 0 Тогда
			
			ЗаполнитьЗначенияСвойств(СтруктураПоискаБезСерий, СтруктураПоиска);
			СтрокиТоваров = ТоварыКЗаполнению.НайтиСтроки(СтруктураПоискаБезСерий);
		...
		Для Каждого СтрокаТоваров Из СтрокиТоваров Цикл
			...
			Если СтрокаНомераГТДКЗаполнению.Количество >= СтрокаТоваров.Количество Тогда
				СтрокаНомераГТДКЗаполнению.Количество = СтрокаНомераГТДКЗаполнению.Количество - СтрокаТоваров.Количество;
			Иначе
				
				НоваяСтрТовары = ТоварыКЗаполнению.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрТовары, СтрокаТоваров);
				
				НовоеКоличество = СтрокаТоваров.Количество - СтрокаНомераГТДКЗаполнению.Количество;
				...
				СтрокаТоваров.Количество = СтрокаНомераГТДКЗаполнению.Количество;
				
				Если ДействияСоСтрокой <> Неопределено Тогда
					ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(НоваяСтрТовары, ДействияСоСтрокой, Неопределено);
				КонецЕсли;
				
				СтрокаНомераГТДКЗаполнению.Количество = 0;
			...
			Если ДействияСоСтрокой <> Неопределено Тогда
				ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтрокаТоваров, ДействияСоСтрокой, Неопределено);
			КонецЕсли;
			
			Если Не СтрокаНомераГТДКЗаполнению.Количество Тогда
				Прервать;
			КонецЕсли;
...
Процедура АктуализацияНоменклатурыВРНПТ(НомерГТД, Номенклатура, НаименованиеНоменклатуры = "")
	...
	Попытка
		...
		ЭлементБлокировки.УстановитьЗначение("Ссылка", НомерГТД);
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		Блокировка.Заблокировать();
		
		ОбъектГТД = НомерГТД.ПолучитьОбъект(); // СправочникОбъект.НомераГТД -
		ОбъектГТД.Номенклатура = Номенклатура;
		ОбъектГТД.НаименованиеТовара = ?(ПустаяСтрока(НаименованиеНоменклатуры),
										ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Номенклатура, "НаименованиеПолное"),
	...
	Исключение
		...
		// Отказ заполнения номенклатуры в шапке ГТД не является блокирующей проблемой.
		// Флаг не взводим.
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Разборка составного номера ГТД (РНПТ)'",
...
// Добавляет запрос в пакет запросов для получения:
//    - Соглашения поставщика по умолчанию.
//    - Количества соглашений поставщика.
//
// Параметры:
//  СтруктураДействий - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ.СтруктураДействий
//  ОписаниеЗапроса - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОписаниеЗапроса
//  КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения
//
Процедура ДополнитьТекстЗапросаЗаполнитьПараметрыСоглашенийПоставщика(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	
	Перем Параметры, ПараметрыЗаполненияСоглашения, ПараметрыЗаполненияКоличества;
	
	ЗаполнитьСоглашениеПоставщикаПоУмолчанию = ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		"ЗаполнитьСоглашениеПоставщикаПоУмолчанию",
		СтруктураДействий,
		КэшированныеЗначения,
		ПараметрыЗаполненияСоглашения);
		
	ЗаполнитьКоличествоСоглашенийПоставщика = ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		"ЗаполнитьКоличествоСоглашенийПоставщика",
		СтруктураДействий,
		КэшированныеЗначения,
		ПараметрыЗаполненияКоличества);
		
	Если Не ЗаполнитьСоглашениеПоставщикаПоУмолчанию
		И Не ЗаполнитьКоличествоСоглашенийПоставщика Тогда
		// Ни одно из действий не выполняется.
		Возврат;
	ИначеЕсли ЗаполнитьСоглашениеПоставщикаПоУмолчанию
		И ЗаполнитьКоличествоСоглашенийПоставщика
		И Не ОбщегоНазначения.КоллекцииИдентичны(ПараметрыЗаполненияСоглашения, ПараметрыЗаполненияКоличества) Тогда
		
		// Выполняются оба действия, но с различными параметрами.
		НаименованиеДействияЗаполнитьСоглашениеПоставщикаПоУмолчанию = "ЗаполнитьСоглашениеПоставщикаПоУмолчанию";
		НаименованиеДействияЗаполнитьКоличествоСоглашенийПоставщика = "ЗаполнитьКоличествоСоглашенийПоставщика";
		ШаблонТекстаОшибки = НСтр("ru = 'Недопустимый вызов одновременной обработки действий таблицы ""%1"" и ""%2"" с различными параметрами'");
		
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонТекстаОшибки,
			НаименованиеДействияЗаполнитьСоглашениеПоставщикаПоУмолчанию,
			НаименованиеДействияЗаполнитьКоличествоСоглашенийПоставщика);
		
		ВызватьИсключение ТекстОшибки;
		
	// Использование параметров из выполняемого действия
	ИначеЕсли ЗаполнитьКоличествоСоглашенийПоставщика Тогда
		Параметры = ПараметрыЗаполненияКоличества;
		ОписаниеЗапроса.ПривилегированныйРежим = Истина; // Обратная совместимость для заполнения видимости соглашений.
	Иначе
		Параметры = ПараметрыЗаполненияСоглашения;
	КонецЕсли;
	
	ПараметрыЗапроса = ОписаниеЗапроса.ПараметрыЗапроса;
	
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ИсточникДанных.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|	ЕСТЬNULL( // 1. Выбор текущего указанного соглашения:
	|		МАКСИМУМ(ВЫБОР КОГДА ИсточникДанных.Соглашение = СоглашенияСПоставщиками.Ссылка
	|					И &ОтборИсключенияХозяйственныхОпераций ТОГДА 
	|					СоглашенияСПоставщиками.Ссылка
	|				ИНАЧЕ 
	|					NULL
	|				КОНЕЦ),
	|		// 2. Проверка, что подходит единственное подходящее соглашение:
	|		ВЫБОР КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ 
	|				ВЫБОР КОГДА &ОтборИсключенияХозяйственныхОпераций ТОГДА
	|					СоглашенияСПоставщиками.Ссылка
	|				ИНАЧЕ
	|					NULL
	|				КОНЕЦ) = 1 ТОГДА 
	|			// 2. Выбор этого соглашения:
	|			МАКСИМУМ(
	|				ВЫБОР КОГДА &ОтборИсключенияХозяйственныхОпераций ТОГДА
	|					СоглашенияСПоставщиками.Ссылка
	|				ИНАЧЕ
	...
	|				КОНЕЦ)
	|
	|		ИНАЧЕ // 3. Нет возможности автоматически выбрать значение: 
	...
	|		ПО ИсточникДанных.Партнер = СоглашенияСПоставщиками.Партнер
	|		И &ИспользоватьСоглашенияСПоставщиками
	|		И НЕ СоглашенияСПоставщиками.ПометкаУдаления
	|		И &ОтборСтатус
	|		И &ОтборХозяйственныеОперации
	|		И &ОтборЦенаВключаетНДС
	|		И &ОтборИспользуютсяДоговорыКонтрагентов
	|
	|СГРУППИРОВАТЬ ПО
	|	ИсточникДанных.ИдентификаторСтрокиВТ";
	
	Если Не ЗаполнитьСоглашениеПоставщикаПоУмолчанию Тогда
		// При заполнении только количества колонка "Соглашение" не используется и может не существовать.
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			"ИсточникДанных.Соглашение",
			"ЗНАЧЕНИЕ(Справочник.СоглашенияСПоставщиками.ПустаяСсылка)");
	КонецЕсли;
	...
	ЗаменяемыйТекст = "&ОтборСтатус";
	Если Параметры.ТолькоДействующее Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			ЗаменяемыйТекст,
			"СоглашенияСПоставщиками.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыСоглашенийСПоставщиками.Действует)");
	Иначе	
		ТекстЗапроса = СтрЗаменить(ЗаменяемыйТекст,
			ЗаменяемыйТекст,
			"ИСТИНА");
	КонецЕсли;
	
	ЗаменяемыйТекст = "&ОтборХозяйственныеОперации";
	ЗаменяемыйТекстИсключения = "&ОтборИсключенияХозяйственныхОпераций"; // Для обратной совместимости исключаются хоз. операции из подбора значения по умолчанию.
	Если Параметры.ХозяйственнаяОперация = Неопределено Тогда
		
		ХозяйственныеОперации = Новый Массив;
		ХозяйственныеОперации.Добавить(Перечисления.ХозяйственныеОперации.ОказаниеАгентскихУслуг);
		...
		ТекстЗапроса = СтрЗаменить(
			ТекстЗапроса,
			ЗаменяемыйТекстИсключения, 
			"СоглашенияСПоставщиками.ХозяйственнаяОперация НЕ В (&ХозяйственныеОперации)");
	...
	Иначе
		...
		ТекстЗапроса = СтрЗаменить(
			ТекстЗапроса,
			ЗаменяемыйТекст, 
			"СоглашенияСПоставщиками.ХозяйственнаяОперация В (&ХозяйственныеОперации)");
		...
		ХозяйственныеОперации = Параметры.ХозяйственнаяОперация;
		
	КонецЕсли;
	ПараметрыЗапроса.Вставить("ХозяйственныеОперации", ХозяйственныеОперации);
	
	ЗаменяемыйТекст = "&ОтборЦенаВключаетНДС";
	...
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			ЗаменяемыйТекст,
			"СоглашенияСПоставщиками.ЦенаВключаетНДС = &ЦенаВключаетНДС");
			ПараметрыЗапроса.Вставить("ЦенаВключаетНДС", Параметры.ЦенаВключаетНДС);
	КонецЕсли;
	
	ЗаменяемыйТекст = "&ОтборИспользуютсяДоговорыКонтрагентов";
	Если Параметры.ИспользуютсяДоговорыКонтрагентов = Неопределено Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			ЗаменяемыйТекст,
			"ИСТИНА");
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			ЗаменяемыйТекст,
			"СоглашенияСПоставщиками.ИспользуютсяДоговорыКонтрагентов = &ИспользуютсяДоговорыКонтрагентов");
		ПараметрыЗапроса.Вставить("ИспользуютсяДоговорыКонтрагентов", Параметры.ИспользуютсяДоговорыКонтрагентов);
	КонецЕсли;
	
	ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ПараметрыСоглашенийПоставщика");
	
КонецПроцедуры

// Добавляет запрос в пакет запросов для получения служебных реквизитов соглашения при обработке строк таблицы.
// Параметры:
//  СтруктураДействий - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ.СтруктураДействий
//  ОписаниеЗапроса - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОписаниеЗапроса
//  КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения
//
Процедура ДополнитьТекстЗапросаЗаполнитьРеквизитыПоСоглашениюПоставщика(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	
	Перем Параметры;
	
	Если НЕ ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		"ЗаполнитьРеквизитыПоСоглашениюПоставщика",
		СтруктураДействий,
		КэшированныеЗначения,
		Параметры) Тогда
	...
	|	ВтИсточникДанных КАК ИсточникДанных
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СоглашенияСПоставщиками КАК СоглашенияСПоставщиками
	|		ПО ИсточникДанных.Соглашение = СоглашенияСПоставщиками.Ссылка";
		
	ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ЗаполнитьРеквизитыПоСоглашениюПоставщика");
	
КонецПроцедуры

// Добавляет запрос в пакет запросов для получения договора по умолчанию 
// обработке строк табличной части.
//
// Параметры:
//  СтруктураДействий - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ.СтруктураДействий
...
//  КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения
//
Процедура ДополнитьТекстЗапросаЗаполнитьДоговорПоУмолчанию(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	ЕстьОтборПоХозяйственнымОперациям = ЗначениеЗаполнено(Параметры.ХозяйственнаяОперация);
	ПараметрыЗапроса.Вставить("ЕстьОтборПоХозяйственнымОперациям", ЕстьОтборПоХозяйственнымОперациям);
	
	ДополнительныеХозяйственныеОперацииПоОсновным = ДополнительныеХозяйственныеОперацииПоОсновным();
	Если ЕстьОтборПоХозяйственнымОперациям Тогда
		
		Если ТипЗнч(Параметры.ХозяйственнаяОперация) = Тип("Массив") Тогда
			ХозяйственныеОперации = Параметры.ХозяйственнаяОперация;
		Иначе
			ХозяйственныеОперации = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Параметры.ХозяйственнаяОперация);
		КонецЕсли;
		
		// Дополним дополнительные хозяйственные операции соответствием:
		// ПустаяСсылка --> Массив из отбора.
		// Это позволит "умножить" количество записей входных параметров до всех возможных комбинаций
		Для Каждого ДополнительнаяОперация Из ХозяйственныеОперации Цикл
			НовоеОписаниеДополнительнойХозяйственнойОперации(ДополнительныеХозяйственныеОперацииПоОсновным,
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ДополнительныеХозяйственныеОперацииПоОсновным.ОсновнаяОперация,
	|	ДополнительныеХозяйственныеОперацииПоОсновным.ДополнительнаяОперация
	|ПОМЕСТИТЬ ДополнительныеХозяйственныеОперацииПоОсновным
	|ИЗ
	|	&ДополнительныеХозяйственныеОперацииПоОсновным КАК ДополнительныеХозяйственныеОперацииПоОсновным";
	ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ДополнительныеХозяйственныеОперацииПоОсновным");
	ПараметрыЗапроса.Вставить("ДополнительныеХозяйственныеОперацииПоОсновным", ДополнительныеХозяйственныеОперацииПоОсновным);
		
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ВходныеПараметры.ИдентификаторСтрокиВТ             КАК ИдентификаторСтрокиВТ,
	|	ВходныеПараметры.Организация                       КАК Организация,
	|	ВходныеПараметры.Партнер                           КАК Партнер,
	|	ВходныеПараметры.Контрагент                        КАК Контрагент,
	|	ВходныеПараметры.НаправлениеДеятельностиНазначения КАК НаправлениеДеятельностиНазначения,
	|	ВходныеПараметры.Договор                           КАК Договор,
	|	ВходныеПараметры.ПодбиратьДоговор                  КАК ПодбиратьДоговор,
	|	ВходныеПараметры.ЕстьОтборПоХозяйственнымОперациям КАК ЕстьОтборПоХозяйственнымОперациям,
	|	ЕСТЬNULL(ДополнительныеХозяйственныеОперацииПоОсновным.ДополнительнаяОперация,
	|		ВходныеПараметры.ХозяйственнаяОперация)         КАК ХозяйственнаяОперация
	|ПОМЕСТИТЬ ПараметрыПоискаДоговораПоУмолчанию
	|ИЗ
	|	(ВЫБРАТЬ
	|		ИсточникДанных.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|		&Организация                         КАК Организация,
	|		ЕСТЬNULL(ИерархияПартнеров.Родитель, 
	|		ИсточникДанных.Партнер)              КАК Партнер,
	|		ИсточникДанных.Контрагент            КАК Контрагент,
	|		&НаправлениеДеятельностиНазначения   КАК НаправлениеДеятельностиНазначения,
	|		ИсточникДанных.Договор               КАК Договор,
	|		&ПодбиратьДоговор                    КАК ПодбиратьДоговор,
	|		&ЕстьОтборПоХозяйственнымОперациям   КАК ЕстьОтборПоХозяйственнымОперациям,
	|		&ХозяйственнаяОперация               КАК ХозяйственнаяОперация
	|	ИЗ
	|		ВтИсточникДанных КАК ИсточникДанных
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИерархияПартнеров КАК ИерархияПартнеров
	|			ПО ИсточникДанных.Партнер = ИерархияПартнеров.Родитель) КАК ВходныеПараметры
	|	ЛЕВОЕ СОЕДИНЕНИЕ ДополнительныеХозяйственныеОперацииПоОсновным КАК ДополнительныеХозяйственныеОперацииПоОсновным
	|		ПО ВходныеПараметры.ХозяйственнаяОперация = ДополнительныеХозяйственныеОперацииПоОсновным.ОсновнаяОперация";
	
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("Соглашение", КэшированныеЗначения) Тогда 
		
		// Нельзя подбирать договор, если в соглашении указано, что договоры не используются.
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			"&ПодбиратьДоговор",
			"ЕСТЬNULL(
			|	ВЫРАЗИТЬ(ИсточникДанных.Соглашение КАК Справочник.СоглашенияСПоставщиками).ИспользуютсяДоговорыКонтрагентов,
			|	ИСТИНА)"); 
		
		// В приоритете используем хозяйственную операцию из соглашения для согласованности данных.
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			"&ХозяйственнаяОперация",
			"ЕСТЬNULL(
			|	ВЫРАЗИТЬ(ИсточникДанных.Соглашение КАК Справочник.СоглашенияСПоставщиками).ХозяйственнаяОперация,
			|	&ХозяйственнаяОперация)");
		
		// Если указано соглашение, то отбор по хозяйственной операции обязательно должен быть установлен.
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			"&ЕстьОтборПоХозяйственнымОперациям",
			"ВЫРАЗИТЬ(ИсточникДанных.Соглашение КАК Справочник.СоглашенияСПоставщиками).ХозяйственнаяОперация ЕСТЬ НЕ NULL
			|	ИЛИ &ЕстьОтборПоХозяйственнымОперациям");
			
	Иначе
		
		ПараметрыЗапроса.Вставить("ПодбиратьДоговор", Истина);
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("Назначение", КэшированныеЗначения)
		И Не ЗначениеЗаполнено(Параметры.НаправлениеДеятельности) Тогда
		// Используем направление деятельности из назначения для отбора договоров с порядком расчетов "ПоДоговорамКонтрагентов":
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, 
			"&НаправлениеДеятельностиНазначения",
			"ЕСТЬNULL(
			|	ВЫРАЗИТЬ(ИсточникДанных.Назначение КАК Справочник.Назначения).НаправлениеДеятельности,
			|	&НаправлениеДеятельностиНазначения)");
	КонецЕсли;
	...
	ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ПараметрыПоискаДоговораПоУмолчанию");
	
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПараметрыПоиска.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|	ЕСТЬNULL( // 1. Выбор текущего указанного договора:
	|		МАКСИМУМ(ВЫБОР КОГДА ПараметрыПоиска.Договор = ДоговорыКонтрагентов.Ссылка ТОГДА 
	|					ДоговорыКонтрагентов.Ссылка
	|				ИНАЧЕ 
	|					NULL
	|				КОНЕЦ),
	|			
	|		ВЫБОР КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ДоговорыКонтрагентов.Ссылка) = 1 ТОГДА 
	|			// 2. Единственный подходящий договор:
	|			МАКСИМУМ(ДоговорыКонтрагентов.Ссылка)
	|		ИНАЧЕ // 3. Нет возможности автоматически выбрать значение: 
	|			ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|		КОНЕЦ) КАК Договор
	|ИЗ
	|	ПараметрыПоискаДоговораПоУмолчанию КАК ПараметрыПоиска
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
	|		ПО ПараметрыПоиска.ПодбиратьДоговор
	|			И ПараметрыПоиска.Организация = ДоговорыКонтрагентов.Организация
	|			И ПараметрыПоиска.Партнер    = ДоговорыКонтрагентов.Партнер
	|			И ПараметрыПоиска.Контрагент = ДоговорыКонтрагентов.Контрагент
	|			И НЕ ДоговорыКонтрагентов.ПометкаУдаления
	|			И ДоговорыКонтрагентов.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыДоговоровКонтрагентов.Действует)
	|			И (НЕ ПараметрыПоиска.ЕстьОтборПоХозяйственнымОперациям
	|				ИЛИ ПараметрыПоиска.ХозяйственнаяОперация = ДоговорыКонтрагентов.ХозяйственнаяОперация
	|				ИЛИ &КомиссионныеПродажи25 <> НЕОПРЕДЕЛЕНО
	...
	|			И (ПараметрыПоиска.НаправлениеДеятельностиНазначения = НЕОПРЕДЕЛЕНО // Нет отбора
	|				ИЛИ ДоговорыКонтрагентов.НаправлениеДеятельности В (ПараметрыПоиска.НаправлениеДеятельностиНазначения, ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка))
	|				ИЛИ ДоговорыКонтрагентов.ПорядокРасчетов <> ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоДоговорамКонтрагентов)) // Отбор только для ""ПоДоговорамКонтрагентов""
	|			И &ОтборВалютаВзаиморасчетов
	|			И &ОтборПорядокРасчетов
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ПараметрыПоиска.ИдентификаторСтрокиВТ";
	
	ИменаПолейУсловногоОтбора = Новый Массив;
	ИменаПолейУсловногоОтбора.Добавить("ВалютаВзаиморасчетов");
	ИменаПолейУсловногоОтбора.Добавить("ПорядокРасчетов");
	ИменаПолейУсловногоОтбора.Добавить("НаправлениеДеятельности");
	ИменаПолейУсловногоОтбора.Добавить("ПорядокОформленияСписанияНедостачТоваровПринятыхНаХранение");
	ИменаПолейУсловногоОтбора.Добавить("ТипДоговора");
	
	Для Каждого ИмяПоля Из ИменаПолейУсловногоОтбора Цикл
		
		ЗначениеОтбора = Параметры[ИмяПоля];
		
		Если ИмяПоля = "НаправлениеДеятельности" Тогда
			
			ОтборИспользуется = ЗначениеОтбора <> Неопределено
				И ПолучитьФункциональнуюОпцию("ИспользоватьУчетЗатратПоНаправлениямДеятельности");
			
		ИначеЕсли ИмяПоля = "ПорядокОформленияСписанияНедостачТоваровПринятыхНаХранение" Тогда
			
			ОтборИспользуется = ЗначениеОтбора <> Неопределено;
			
		Иначе
			
			ОтборИспользуется = ЗначениеЗаполнено(ЗначениеОтбора);
			
		КонецЕсли;
			
		СтрокаЗамены = "ИСТИНА";
		Если ОтборИспользуется Тогда
				
			СтрокаЗамены = СтрШаблон("ДоговорыКонтрагентов.%1 В (&%1)", ИмяПоля);
				
		КонецЕсли;
				
		ПодстрокаПоиска = СтрШаблон("&Отбор%1", ИмяПоля);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ПодстрокаПоиска, СтрокаЗамены);
		ПараметрыЗапроса.Вставить(ИмяПоля, ЗначениеОтбора);
		
	КонецЦикла;
	
	ПараметрыЗапроса.Вставить("УчетАгентскогоНДС",     Параметры.Налогообложение = Перечисления.ТипыНалогообложенияНДС.НалоговыйАгентПоНДС);
	ПараметрыЗапроса.Вставить("КомиссионныеПродажи25", Параметры.КомиссионныеПродажи25);
	ПараметрыЗапроса.Вставить("ДоговорССамозанятым",   Параметры.ДоговорССамозанятым);
	
	ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ЗаполнитьДоговорПоУмолчанию");
	
КонецПроцедуры

// Добавляет запрос в пакет запросов для получения реквизитов договора при обработке строк табличной части.
//
// Параметры:
//  СтруктураДействий - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ.СтруктураДействий
//  ОписаниеЗапроса - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОписаниеЗапроса
//  КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения
//
Процедура ДополнитьТекстЗапросаЗаполнитьОперациюПоСоглашениюДоговору(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	
	Перем Параметры;
	
	Если НЕ ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		"ЗаполнитьОперациюПоСоглашениюДоговору",
		СтруктураДействий,
		КэшированныеЗначения,
		Параметры) Тогда
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	Источник.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|	ВЫБОР КОГДА Соглашения.Ссылка ЕСТЬ НЕ NULL Тогда
	|		Соглашения.ХозяйственнаяОперация
	|	КОГДА Договоры.Ссылка ЕСТЬ НЕ NULL Тогда
	|		Договоры.ХозяйственнаяОперация
	|	ИНАЧЕ
	|		&ЗначениеОперацииПоУмолчанию
	|	КОНЕЦ КАК ХозяйственнаяОперация
	|ИЗ
	|	ВтИсточникДанных КАК Источник
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК Договоры
	|		ПО Источник.Договор = Договоры.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СоглашенияСПоставщиками КАК Соглашения
	|		ПО Источник.Соглашение = Соглашения.Ссылка";
	
	ПараметрыЗапроса = ОписаниеЗапроса.ПараметрыЗапроса;
	ПараметрыЗапроса.Вставить("ЗначениеОперацииПоУмолчанию", Параметры.ЗначениеПоУмолчанию);
	ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ЗаполнитьОперациюПоСоглашениюДоговору");
	
КонецПроцедуры

// Добавляет запрос в пакет запросов для получения представлений ссылок, участвующих в формировании представления условий закупки
// Параметры:
//  СтруктураДействий - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ.СтруктураДействий
//  ОписаниеЗапроса - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОписаниеЗапроса
//  КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения
//
Процедура ДополнитьТекстЗапросаЗаполнитьПредставлениеУсловийЗакупки(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	
	Перем Параметры;
	
	Если НЕ ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		"ЗаполнитьПредставлениеУсловийЗакупки",
		СтруктураДействий,
		КэшированныеЗначения,
		Параметры) Тогда
	...
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ИсточникДанных.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|	ПРЕДСТАВЛЕНИЕ(ИсточникДанных.Партнер) КАК Партнер
	|ИЗ
	|	ВтИсточникДанных КАК ИсточникДанных";
		
	ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ЗаполнитьПредставлениеУсловийЗакупки");
...
Функция ДополнительныеХозяйственныеОперацииПоОсновным()
	
	ОписаниеТипаОпераций = Новый ОписаниеТипов("ПеречислениеСсылка.ХозяйственныеОперации");
	
	ВозвращаемоеЗначение = Новый ТаблицаЗначений();
	ВозвращаемоеЗначение.Колонки.Добавить("ОсновнаяОперация", ОписаниеТипаОпераций);
	ВозвращаемоеЗначение.Колонки.Добавить("ДополнительнаяОперация", ОписаниеТипаОпераций);

	ИспользоватьТоварыВПутиОтПоставщиков  = ПолучитьФункциональнуюОпцию("ИспользоватьТоварыВПутиОтПоставщиков");
	ИспользоватьНеотфактурованныеПоставки = ПолучитьФункциональнуюОпцию("ИспользоватьНеотфактурованныеПоставки");
	ИспользоватьВвозТоваровИзТаможенногоСоюза = ПолучитьФункциональнуюОпцию("ИспользоватьВвозТоваровИзТаможенногоСоюза");
	
	ЗакупкаУПоставщика = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика;
	НовоеОписаниеДополнительнойХозяйственнойОперации(ВозвращаемоеЗначение,
		ЗакупкаУПоставщика,
		ЗакупкаУПоставщика);
	
	ЗакупкаВСтранахЕАЭС = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС;
	НовоеОписаниеДополнительнойХозяйственнойОперации(ВозвращаемоеЗначение,
		ЗакупкаВСтранахЕАЭС,
		ЗакупкаВСтранахЕАЭС);
	
	ЗакупкаПоИмпорту = Перечисления.ХозяйственныеОперации.ЗакупкаПоИмпорту;	
	НовоеОписаниеДополнительнойХозяйственнойОперации(ВозвращаемоеЗначение,
		ЗакупкаПоИмпорту,
		ЗакупкаПоИмпорту);
	
	ПроизводствоУПереработчика2_5 = Перечисления.ХозяйственныеОперации.ПроизводствоУПереработчика2_5;
	НовоеОписаниеДополнительнойХозяйственнойОперации(ВозвращаемоеЗначение,
		ПроизводствоУПереработчика2_5,
		ПроизводствоУПереработчика2_5);
	...
	Если ИспользоватьТоварыВПутиОтПоставщиков Тогда
		...
		НовоеОписаниеДополнительнойХозяйственнойОперации(ВозвращаемоеЗначение,
			ЗакупкаУПоставщика,
			Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаТоварыВПути);
		...
		НовоеОписаниеДополнительнойХозяйственнойОперации(ВозвращаемоеЗначение,
			ЗакупкаВСтранахЕАЭС,
			Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСТоварыВПути);
		...
		НовоеОписаниеДополнительнойХозяйственнойОперации(ВозвращаемоеЗначение,
			ЗакупкаПоИмпорту,
			Перечисления.ХозяйственныеОперации.ЗакупкаПоИмпортуПоступлениеИзТоваровВПути);
...
Функция НовоеОписаниеДополнительнойХозяйственнойОперации(Связи, ОсновнаяОперация, ДополнительнаяОперация)
	
	ОписаниеСвязи = Связи.Добавить();
	
	ОписаниеСвязи.ОсновнаяОперация = ОсновнаяОперация;
	ОписаниеСвязи.ДополнительнаяОперация = ДополнительнаяОперация;
