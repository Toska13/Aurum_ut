...
Функция ПолучитьДанныеДляПечатнойФормыТОРГ12(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	Возврат ПолучитьДанныеДляПечатнойФормыШапкиТЧ(МенеджерВременныхТаблиц, ПараметрыПечати);
...
Функция СформироватьПечатнуюФормуРеестрНомеровГТД(МассивОбъектов, ОбъектыПечати)
	...
	|ВЫБРАТЬ
	|	ДанныеДокумента.Ссылка КАК Ссылка,
	|	ПРЕДСТАВЛЕНИЕ(ДанныеДокумента.Ссылка) КАК ПредставлениеСсылки,
	|	ПРЕДСТАВЛЕНИЕ(ДанныеДокумента.ХозяйственнаяОперация) КАК ПредставлениеХозяйственнойОперации
	|ИЗ
	|	Документ.РеализацияТоваровУслуг КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.Ссылка В(&МассивОбъектов)
	...
	ВыборкаПоДокументамПереходПраваСобственности = МассивРезультатов[4].Выбрать();
	...
	Пока ВыборкаПоДокументамПереходПраваСобственности.Следующий() Цикл
		
		ТекстСообщения = НСтр("ru = 'В документе %1 хозяйственная операция %2. Печать реестра номеров ГТД не требуется.'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстСообщения,
			ВыборкаПоДокументамПереходПраваСобственности.ПредставлениеСсылки,
			ВыборкаПоДокументамПереходПраваСобственности.ПредставлениеХозяйственнойОперации);
		
		ОбщегоНазначения.СообщитьПользователю(
			ТекстСообщения,
			ВыборкаПоДокументамПереходПраваСобственности.Ссылка);
...
Функция ПолучитьДанныеДляПечатнойФормыМ15(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	Пока ВыборкаОшибки.Следующий() Цикл
		...
		ТекстСообщения = НСтр("ru = 'В документе %1 хозяйственная операция %2. Печать М-15 не требуется.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстСообщения,
	...
	Если ЗначениеЗаполнено(Форма.Объект.Контрагент)
		...
		ДокументЯвляетсяРеализациейКлиенту =
			(Форма.Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту
				...
				ИЛИ Форма.Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности);
...
Функция ПредметыРасчетовПоДокументу(ДокументСсылка, ЧекКоррекцииИсправленииОшибки = Ложь) Экспорт
	...
	РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "ЦенаВключаетНДС, ТребуетсяЗалогЗаТару, ВернутьМногооборотнуюТару, Организация, Дата");
...
// Проверить, исправить наличие скидок оплат бонусными баллами в документе
// 
// Параметры:
//  ДокументОбъект - ДокументОбъект.РеализацияТоваровУслуг - Документ объект
Процедура ПроверитьИсправитьНаличиеСкидокОплатБонуснымиБалламиПоТабачнойПродукции(ДокументОбъект) Экспорт
	...
	ТипКонтрагента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект.Контрагент, "ЮрФизЛицо");
	Если ТипКонтрагента <> Перечисления.ЮрФизЛицо.ФизЛицо Тогда
		Возврат;
	КонецЕсли;
	
	ИмяТаблицыТоваров = "Товары";
	ОчиститьСкидкиОплатуБонуснымиБаллами = Истина;
	СообщитьОбОшибках = Истина;
	РозничныеПродажиЛокализация.ПроверитьИсправитьНаличиеСкидокОплатБонуснымиБаллами(
		ДокументОбъект,
		ИмяТаблицыТоваров,
		ОчиститьСкидкиОплатуБонуснымиБаллами,
		СообщитьОбОшибках);
	//-- Локализация
...
Процедура СформироватьОтгруженныеМарки(МассивЗаказовКлиентов, ОтгруженныеМарки) Экспорт
	...
	ОбработанныеОтгрузки = Новый Соответствие;
	Для Каждого Основание Из МассивЗаказовКлиентов Цикл
		Если ТипЗнч(Основание) <> Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда
			Продолжить;
		КонецЕсли;
		Если ОбработанныеОтгрузки.Получить(Основание)<>Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ОбработанныеОтгрузки.Вставить(Основание, Истина);
		
		АктуальноеИсправление = ИсправлениеДокументов.ПоследнийИсправительныйДокумент(Основание);
		Если АктуальноеИсправление = Неопределено Тогда
			ШтрихкодыУпаковокОтгрузки = ШтрихкодированиеИСМП.ВложенныеШтрихкодыУпаковокПоДокументу(
				Основание, Неопределено);
		Иначе
			ШтрихкодыУпаковокОтгрузки = ШтрихкодированиеИСМП.ВложенныеШтрихкодыУпаковокПоДокументу(
				АктуальноеИсправление.Ссылка, Неопределено);
		КонецЕсли;
		
		Если ШтрихкодыУпаковокОтгрузки.ЕстьОшибки Тогда
			Продолжить;
		ИначеЕсли ШтрихкодыУпаковокОтгрузки.ДанныеКодовМаркировки = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Для Каждого МаркированныйТовар Из ШтрихкодыУпаковокОтгрузки.ДанныеКодовМаркировки Цикл
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрока, МаркированныйТовар);
			НоваяСтрока.ШтрихкодУпаковки = СтрокаДерева.ШтрихкодУпаковки;
			НоваяСтрока.ЗаказКлиента = Основание;
...
Функция ТекстЗапросаТаблицаПрослеживаемыеТоварыОтгруженныеВЕАЭС(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	|														ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара),
	|														ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))
	|	И &ХозяйственнаяОперация НЕ В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаНаКомиссию),
...
Функция ПолучитьДанныеДляПечатнойФормыШапкиТЧ(МенеджерВременныхТаблиц, ПараметрыПечати)
