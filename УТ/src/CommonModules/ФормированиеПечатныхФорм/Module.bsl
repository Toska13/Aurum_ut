...
Функция ОписаниеОрганизации(СписокСведений, Список = "", СПрефиксом = Истина) Экспорт
	...
	ДополнительныеРеквизиты = Новый Структура("Банк,БИК,КоррСчет,СвидетельствоДатаВыдачи,БИКРКЦ,СчетЦифровогоРубля");
	...
	Для Каждого ИмяРеквизита Из МассивРеквизитов Цикл
		...
		Если СПрефиксом И ИмяРеквизита = "НомерСчета" Тогда
			...
			Если ЗначениеЗаполнено(ДополнительныеРеквизиты.СчетЦифровогоРубля)
				И ДополнительныеРеквизиты.СчетЦифровогоРубля Тогда
				
				ДополнениеОписания = СтрШаблон(НСтр("ru = 'счет цифрового рубля %1'"), ЗначениеРеквизита);
...
Функция СведенияОЮрФизЛице(ЮрФизЛицо, ДатаПериода, ДляФизЛицаТолькоИнициалы = Истина, Знач БанковскийСчет = Неопределено, КрупнейшийНалогоплательщик = Ложь ) Экспорт
	...
	ЗаполнитьСведенияОЮрФизЛице(Сведения, ЮрФизЛицо, ДатаПериода, ДляФизЛицаТолькоИнициалы, БанковскийСчет, КрупнейшийНалогоплательщик);
...
//
Процедура ЗаполнитьСведенияОЮрФизЛице(Сведения, ЮрФизЛицо, ДатаПериода, ДляФизЛицаТолькоИнициалы = Истина, Знач БанковскийСчет = Неопределено, КрупнейшийНалогоплательщик = Ложь ) Экспорт
	Если ЗначениеЗаполнено(ЮрФизЛицо)
		...
		Если ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Организации") Тогда
			...
			Если Не ЗначениеЗаполнено(БанковскийСчет) Тогда
				СтруктураПараметров = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации(ЮрФизЛицо);
		...
		УправлениеПечатьюЛокализация.ДополнитьСведенияОЮрФизЛице(Сведения,Реквизиты, ЮрФизЛицо, ДатаПериода, КрупнейшийНалогоплательщик);
...
Функция СформироватьТекстНДС(НалогообложениеНДС, ЦенаВключаетНДС)Экспорт
	...
	Если ПустаяСтрока(НалогообложениеНДС) Тогда
		Возврат ?(ЦенаВключаетНДС, Нстр("ru='Цена включает НДС.'"), Нстр("ru='Цена не включает НДС.'"));
	Иначе
		Возврат НалогообложениеНДС
			+ ?(ЦенаВключаетНДС, Нстр("ru='. Цена включает НДС.'"), Нстр("ru='. Цена не включает НДС.'"));
	КонецЕсли;
...
Процедура ПриЗаполненииУправляющихМодулей(УправляющиеМодули) Экспорт
	...
	УправляющиеМодули.Вставить(Метаданные.Документы.СверкаВзаиморасчетов2_4.ПолноеИмя());
	...
	УправляющиеМодули.Вставить(Метаданные.Документы.СписаниеОтгруженныхТоваров.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.СчетНаОплатуКлиенту.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ОприходованиеИзлишковОтгруженныхТоваров.ПолноеИмя());
	
	УправлениеПечатьюЛокализация.ПриЗаполненииУправляющихМодулей(УправляющиеМодули);
...
Процедура ДополнитьДеревоРодительскихДокументов(ТекущийДокумент, ДеревоРодитель)
		...
		Если РеквизитУдовлетворяетУсловиям(Реквизит) Тогда
			...
			Для Каждого ТекущийТип Из Реквизит.Тип.Типы() Цикл
				...
				Если МетаданныеРеквизита <> Неопределено
					...
					РеквизитыКПолучению.Добавить(Реквизит.Имя);
	...
	Если РеквизитыКПолучению.Количество() > 0 Тогда
		ДанныеКПолучению.Вставить("РеквизитыШапки", РеквизитыКПолучению);
	КонецЕсли;
	...
	Для Каждого ТЧ Из МетаданныеДокумента.ТабличныеЧасти Цикл
		...
		Если РеквизитыКПолучению.Количество() > 0 Тогда
			ДанныеКПолучению.Вставить(ТЧ.Имя, РеквизитыКПолучению);
		КонецЕсли;
	...
	Для Каждого КлючИЗначение Из ДанныеКПолучению Цикл
		
		Если КлючИЗначение.Ключ = "РеквизитыШапки" Тогда
			
			ТекстПакетаЗапроса = "
			|ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	&ИмяРеквизита 
			|ИЗ
			|	&ПолноеИмяОбъектаМетаданных КАК ИмяТаблицы
			|ГДЕ
			|	ИмяТаблицы.Ссылка = &ТекущийДокумент";
			
			СтрокаРеквизиты = "";
			
			СтрокаРеквизиты = СтрСоединить(КлючИЗначение.Значение, "," + Символы.ПС + Символы.Таб);
			
			ТекстПакетаЗапроса = СтрЗаменить(ТекстПакетаЗапроса, "&ПолноеИмяОбъектаМетаданных", ПолноеИмяОбъектаМетаданных);
			ТекстПакетаЗапроса = СтрЗаменить(ТекстПакетаЗапроса, "&ИмяРеквизита",               СтрокаРеквизиты);
		...
		Иначе
			...
			ТекстПакетаЗапроса = СтрЗаменить(ТекстПакетаЗапроса, "&ИмяТабличнойЧасти",  ПолноеИмяОбъектаМетаданных + "." + КлючИЗначение.Ключ);
			ТекстПакетаЗапроса = СтрЗаменить(ТекстПакетаЗапроса, "&ИмяРеквизита",       СтрокаРеквизиты);
		...
		ТекстЗапроса = ТекстЗапроса + ТекстПакетаЗапроса;
	...
	Для Каждого КлючИЗначение Из ДанныеКПолучению Цикл
		...
		ВыборкаДанных = РезультатЗапроса[ИндексЗапроса].Выбрать();
		
		Пока ВыборкаДанных.Следующий() Цикл
			
			Для Каждого ИмяРеквизита Из КлючИЗначение.Значение Цикл
				
				ЗначениеРеквизита = ВыборкаДанных[ИмяРеквизита];
