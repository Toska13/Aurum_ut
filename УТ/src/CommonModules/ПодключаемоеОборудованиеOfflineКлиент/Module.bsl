...
Процедура ВыгрузитьТоварыЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если СостояниеВыполнения.Обработано = СостояниеВыполнения.ТребуетсяВыполнить Тогда
		...
		Если ЗначениеЗаполнено(СостояниеВыполнения.ОписаниеОшибок) Тогда
			ПодключаемоеОборудованиеOfflineВызовСервера.ЗаписатьОшибкувЖурналРегистрации(
				"ПодключаемоеОборудованиеOfflineКлиент.ВыгрузитьТоварыЗавершение",
				ТекстСообщения);
		КонецЕсли;
