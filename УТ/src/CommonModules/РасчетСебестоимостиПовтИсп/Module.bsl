...
Функция ПравилаЗаполненияПоляТипЗаписи(ИмяДокументаДляОтбора = "") Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаДляОтбора) ИЛИ НРег(ИмяДокументаДляОтбора) = НРег(ТекущийДокумент) Тогда
		...
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.ВозвратНаДругойСклад;
		НовоеПравило.ДокументИсточникВПриходе = Истина;
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаДляОтбора) ИЛИ НРег(ИмяДокументаДляОтбора) = НРег(ТекущийДокумент) Тогда
		...
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость;
		НовоеПравило.ДокументИсточникВПриходе = Истина;
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаДляОтбора) ИЛИ НРег(ИмяДокументаДляОтбора) = НРег(ТекущийДокумент) Тогда
		...
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.КорректировкаВидаДеятельностиНДС;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.Перемещение;
		НовоеПравило.ТипЗаписиРасход = Перечисления.ТипыЗаписейПартий.Потребление;
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаДляОтбора) ИЛИ НРег(ИмяДокументаДляОтбора) = НРег(ТекущийДокумент) Тогда
		...
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереносОстатковПодНовыйВидДеятельности;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.Перемещение;
		НовоеПравило.ТипЗаписиРасход = Перечисления.ТипыЗаписейПартий.Потребление;
		...
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереносОстатковПодНовыйВидДеятельности;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.ПеремещениеНаОстаток;
		НовоеПравило.ТипЗаписиРасход = Перечисления.ТипыЗаписейПартий.Потребление;
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаДляОтбора) ИЛИ НРег(ИмяДокументаДляОтбора) = НРег(ТекущийДокумент) Тогда
		...
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.ВозвратПрошлыхПериодов;
	...
	#Область ОприходованиеИзлишковОтгруженныхТоваров
	
	ТекущийДокумент = Метаданные.Документы.ОприходованиеИзлишковОтгруженныхТоваров.Имя;
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаДляОтбора) ИЛИ НРег(ИмяДокументаДляОтбора) = НРег(ТекущийДокумент) Тогда
		...
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОприходованиеИзлишковОтгруженныхТоваров;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.Партия;
	...
	#Область ОтгрузкаТоваровКлиенту
	
	ТекущийДокумент = Метаданные.Документы.ОтгрузкаТоваровКлиенту.Имя;
	
	Если НЕ ЗначениеЗаполнено(ИмяДокументаДляОтбора) ИЛИ НРег(ИмяДокументаДляОтбора) = НРег(ТекущийДокумент) Тогда
		
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтгрузкаТоваровКлиенту;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.Перемещение;
		НовоеПравило.ТипЗаписиРасход = Перечисления.ТипыЗаписейПартий.Потребление;
	...
	ТекущийДокумент = Метаданные.Документы.СписаниеОтгруженныхТоваров.Имя;
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаДляОтбора) ИЛИ НРег(ИмяДокументаДляОтбора) = НРег(ТекущийДокумент) Тогда
		...
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеОтгруженныхТоваров;
		НовоеПравило.ТипЗаписиРасход = Перечисления.ТипыЗаписейПартий.Потребление;
	...
	#Область ПоступлениеТоваровОтКлиента
	
	ТекущийДокумент = Метаданные.Документы.ПоступлениеТоваровОтКлиента.Имя;
	
	Если НЕ ЗначениеЗаполнено(ИмяДокументаДляОтбора) ИЛИ НРег(ИмяДокументаДляОтбора) = НРег(ТекущийДокумент) Тогда
		
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеТоваровОтКлиента;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.Перемещение;
		НовоеПравило.ТипЗаписиРасход = Перечисления.ТипыЗаписейПартий.Потребление;
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаДляОтбора) ИЛИ НРег(ИмяДокументаДляОтбора) = НРег(ТекущийДокумент) Тогда
		...
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.СторноВозвратНаДругойСклад;
		НовоеПравило.ДокументИсточникВПриходе = Истина;
...
Функция ТекстВтСоответствиеВариантовРаспределенияНазначениямПравил() Экспорт
	...
	|		КОГДА Варианты.Ссылка = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода) 
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.НазначениеПравилРаспределенияРасходов.БезРаспределенияРасходов)
...
// Возвращает признак наличия хотя бы одной настройки распределения по направлениям деятельности.
//
// Возвращаемое значение:
// 	Булево -
Функция ЕстьНастройкиРаспределенияПоНаправлениямДеятельности() Экспорт
	...
	|ИЗ
	|	РегистрСведений.НастройкаРаспределенияПоНаправлениямДеятельности КАК Т
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НаправленияДеятельности КАК НаправленияДеятельности
	|		ПО Т.СпособРаспределения = НаправленияДеятельности.Ссылка
	...
	|		ПО Т.СпособРаспределения = СпособыРаспределения.Ссылка
	|		И СпособыРаспределения.Ссылка.ПравилоРаспределения = ЗНАЧЕНИЕ(Перечисление.ПравилаРаспределенияПоНаправлениямДеятельности.ПропорциональноКоэффициентам)
	|		И СпособыРаспределения.НаправлениеДеятельности.УчетДоходов
	|ГДЕ
	|	Т.Используется
	|   И (НЕ НаправленияДеятельности.Ссылка ЕСТЬ NULL
	|	  	ИЛИ НЕ СпособыРаспределения.Ссылка ЕСТЬ NULL)
	|";
...
// Возвращает признак распределения отклонений в прошлых периодах.
//
// Возвращаемое значение:
//  Булево - используется распределение отклонений в прошлых периодах.
//
Функция ИспользуетсяРаспределениеОтклоненийВПрошлыхПериодах() Экспорт
	...
	|		ПО Т.Ссылка = УчетныеПолитикиОрганизаций.Организация
	|ГДЕ
	|	УчетныеПолитикиОрганизаций.РаспределятьОтклоненияВПрошлыхПериодах";
	
	Возврат НЕ Запрос.Выполнить().Пустой();
	
КонецФункции

// Возвращает регистры, движения по которым формируются одновременно с движениями по себестоимости при среднескользящей.
//
// Возвращаемое значение:
//	Массив из ОбъектМетаданных - метаданные регистров
Функция СвязанныеРегистрыПриПроведенииСреднескользящая() Экспорт
	
	МассивРегистров = Новый Массив;
	МассивРегистров.Добавить(Метаданные.РегистрыНакопления.СебестоимостьТоваров);
	МассивРегистров.Добавить(Метаданные.РегистрыНакопления.ВыручкаИСебестоимостьПродаж);
	МассивРегистров.Добавить(Метаданные.РегистрыНакопления.ПрочиеРасходы);
	МассивРегистров.Добавить(Метаданные.РегистрыНакопления.ПрочиеДоходы);
	МассивРегистров.Добавить(Метаданные.РегистрыНакопления.ПрочиеАктивыПассивы);
	МассивРегистров.Добавить(Метаданные.РегистрыНакопления.ДвиженияПоПрочимАктивамПассивам);
	МассивРегистров.Добавить(Метаданные.РегистрыНакопления.ДвиженияНоменклатураДоходыРасходы);
	МассивРегистров.Добавить(Метаданные.РегистрыНакопления.ДвиженияНоменклатураНоменклатура);
	МассивРегистров.Добавить(Метаданные.РегистрыНакопления.ОтклоненияВСтоимостиТоваров);
...
//
// Параметры:
//	Операция - СправочникСсылка.ОперацииЗакрытияМесяца - операция, для которой получаются технологические параметры;
