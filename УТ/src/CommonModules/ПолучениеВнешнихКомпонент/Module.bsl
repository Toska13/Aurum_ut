...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ИнтерактивноеОбновлениеВнешнихКомпонентИзСервиса", Истина);
	Методы.Вставить("ИнтерактивноеОбновлениеВнешнихКомпонентИзФайла", Истина);
	Методы.Вставить("ОбновитьНастройкиПолучениеВнешнихКомпонент");
	Методы.Вставить("УстановитьНастройкиПолученияВнешнихКомпонент");
...
Процедура УстановитьНастройкиПолученияВнешнихКомпонент(Параметры) Экспорт
	...
	Если ЗаданияОбновления.Количество() > 0 Тогда
		...
		Попытка
			ВключитьАвтоматическоеОбновлениеВнешнихКомпонентИзСервиса();
...
Процедура ПриИзмененииДанныхАутентификации(Логин, Пароль) Экспорт
	...
	Если ЗначениеЗаполнено(Логин) Тогда
		Если АвтоматическаяЗагрузкаОбновленийОтключена() Тогда
			ВключитьАвтоматическоеОбновлениеВнешнихКомпонентИзСервиса();
...
Процедура ДобавитьРегламентноеЗаданиеОбновления(Использование = Истина)
	...
	Если ЗаданияОбновления.Количество() = 0 Тогда
		...
		Попытка
			...
			Если Использование
					И АвтоматическаяЗагрузкаОбновленийОтключена() Тогда
				ВключитьАвтоматическоеОбновлениеВнешнихКомпонентИзСервиса();
			КонецЕсли;
			ЗафиксироватьТранзакцию();
		Исключение
...
// Включает автоматическое обновление внешних компонент из сервиса.
//
Процедура ВключитьАвтоматическоеОбновлениеВнешнихКомпонентИзСервиса() Экспорт
...
//
// Параметры:
//  РезультатОперации  - Структура - см. функцию ОписаниеРезультатаЗагрузки().
//
Процедура ЗагрузитьФайлыВнешнихКомпонент(РезультатОперации)
	...
	Для Каждого ОписаниеВнешнейКомпоненты Из РезультатОперации.ДанныеВнешнихКомпонент Цикл
		...
		ПоместитьФайлВнешнейКомпонентыВХранилище(
			ОписаниеВнешнейКомпоненты,
			РезультатОтправки.Содержимое);
...
// Помещает файл внешней компоненты во временное хранилище.
//
// Параметры:
//  ОписаниеВнешнейКомпоненты - СтрокаТаблицыЗначений - строка таблицы ДанныеВнешнихКомпонент
//                              см. ОписаниеРезультатаЗагрузки.
//  ДанныеВнешнихКомпонент - ТаблицаЗначений - см ОписаниеРезультатаЗагрузки.
//
Процедура ПоместитьФайлВнешнейКомпонентыВХранилище(
	ОписаниеВнешнейКомпоненты,
	ДвоичныеДанные)
	...
	Исключение
		ОписаниеВнешнейКомпоненты.КодОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка получения файла внешней компоненты %1. '"),
			ОписаниеВнешнейКомпоненты.Идентификатор);
		Возврат;
	КонецПопытки;
	
	ФайлКомпоненты = Неопределено;
	Если ЧтениеАрхива.Элементы.Количество() = 2
		И ЧтениеАрхива.Элементы.Найти("EXTERNAL-COMPONENTS.JSON") <> Неопределено Тогда
		
		Для Каждого ЭлементАрхива Из ЧтениеАрхива.Элементы Цикл
		
			Если ВРег(ЭлементАрхива.ИсходноеПолноеИмя) = "EXTERNAL-COMPONENTS.JSON" Тогда
				Продолжить;
			Иначе
				ФайлКомпоненты = ЭлементАрхива;
			КонецЕсли;
			
		КонецЦикла;
		
		Если ФайлКомпоненты = Неопределено Тогда
			КодОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось найти файл внешней компоненты %1.'"),
				ОписаниеВнешнейКомпоненты.Идентификатор);
			ОписаниеВнешнейКомпоненты.КодОшибки = КодОшибки;
			ЗаписатьИнформациюВЖурналРегистрации(КодОшибки);
			Возврат;
		КонецЕсли;
		
		КаталогВнешнихКомпонент = ФайловаяСистема.СоздатьВременныйКаталог(
			Строка(Новый УникальныйИдентификатор));
		ЧтениеАрхива.Извлечь(ФайлКомпоненты, КаталогВнешнихКомпонент);
		ДанныеФайла = Новый ДвоичныеДанные(КаталогВнешнихКомпонент + ФайлКомпоненты.Имя);
		...
		ОписаниеВнешнейКомпоненты.КонтрольнаяСумма = ИнтернетПоддержкаПользователей.КонтрольнаяСуммаФайла(
			ДанныеФайла);
		
		// Очищаем временные файлы и освобождаем память.
		ФайловаяСистема.УдалитьВременныйКаталог(КаталогВнешнихКомпонент);
		
	Иначе
		ОписаниеВнешнейКомпоненты.АдресФайла = ПоместитьВоВременноеХранилище(ДвоичныеДанные);
	КонецЕсли;
...
Процедура ОбновитьВнешниеКомпонентыИзФайла(ИмяФайла)
	...
	Если Не ЗначениеЗаполнено(РезультатОперации.КодОшибки) Тогда
		...
		МодульВнешниеКомпонентыСервер.ОбновитьВнешниеКомпоненты(РезультатОперации.ДанныеВнешнихКомпонент, АдресРезультата);
		...
		Если Не РезультатОбновления.Результат Тогда
			...
			Для Каждого ОшибкаВРезультате Из РезультатОбновления.Ошибки Цикл
				ТекстОшибок.Добавить(ОшибкаВРезультате.Значение);
			КонецЦикла;
			ТекстОшибок = СтрСоединить(ТекстОшибок, Символы.ПС);
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибки при обновлении внешних компонентах:
					|%1'"),
				ТекстОшибок);
			ЗаписатьИнформациюВЖурналРегистрации(
				ТекстОшибки,
...
//                                Используется для проверки целостности файлов;
//     **АдресФайла             - Строка, Неопределено - содержит адрес файла внешней
//                                компоненты во временном хранилище;
//     **КодОшибки              - Строка - содержит код ошибки загрузки внешней компоненты;
//
...
Функция ПереопределитьСообщениеПользователю(КодОшибки)
	...
	Если КодОшибки = КодОшибкиНетДоступаКПрограмме() Тогда
...
// Возвращает признак использования автоматического обновления внешних компонент.
//
// Возвращаемое значение:
//  Булево - Ложь, когда отключено.
//
Функция АвтоматическаяЗагрузкаОбновленийОтключена()
	Возврат Константы.ВариантОбновленияВнешнихКомпонент.Получить() = РежимОбновленияОтключено();
КонецФункции
