...
Процедура ПровестиДокументы(Список, ТипСписка, ПараметрыОбновленияСписка = Неопределено) Экспорт
	...
	НепроведенныеДокументы =
		ОбщегоНазначенияУТВызовСервера.ЗаписатьВЖурналСообщитьПользователюОРезультатахПроведенияОтменыПроведения(
			ДанныеОНепроведенныхДокументах);
...
Процедура ОтменаПроведения(Список, ТипСписка, ПараметрыОбновленияСписка = Неопределено) Экспорт
	...
	НераспроведенныеДокументы =
		ОбщегоНазначенияУТВызовСервера.ЗаписатьВЖурналСообщитьПользователюОРезультатахПроведенияОтменыПроведения(
			ДанныеОНераспроведенныхДокументах, Ложь);
	...
	Если НераспроведенныеДокументы.Количество() > 0 Тогда
		...
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("Список", Список);
		ДополнительныеПараметры.Вставить("ПроведенныеДокументы", РаспроведенныеДокументы);
		ДополнительныеПараметры.Вставить("ТипСписка", ТипСписка);
		ДополнительныеПараметры.Вставить("ПараметрыОбновленияСписка", ПараметрыОбновленияСписка);
...
// Выполняет оповещение пользователя об осуществленном проведении или отмене проведения документов, а так же обновляет
// информацию об измененных данных динамического списка.
//
// Параметры:
...
Процедура ПроверитьПроведенностьДокументовЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса <> Неопределено Тогда
		...
		Иначе
			...
			Иначе
				РаботаСЖурналамиДокументовКлиент.УстановитьПризнакОтображенияНеактуальныхДанныхВСписке(
					ПараметрыОбновленияСписка.Форма,
					ПараметрыОбновленияСписка.ИмяСписка);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
//Процедура показывает сообщение о превышении количества в документе при вызове формы "Проверка заполнения документов"
//
// Параметры:
//  ВозвращаемыеПараметры - Структура - структура возврашаемых параметров из общей формы "ПроверкаЗаполненияДокументов" 
//  ТекстОснования - Строка - ограничения по Заказу
//
Процедура ВывестиСообщениеПревышенияПоКоличеству(ВозвращаемыеПараметры, ТекстОснования = "") Экспорт

	Если ТипЗнч(ВозвращаемыеПараметры) = Тип("Структура")
			И ВозвращаемыеПараметры.Свойство("ПревышениеЗапрещено")
			И ВозвращаемыеПараметры.ПревышениеЗапрещено = Истина Тогда
		
		ШаблонТекста = НСтр("ru = 'Превышение количества не предусмотрено для документа %1'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекста, ТекстОснования);
...
Процедура ПриНачалеРаботыСистемы(Параметры) Экспорт
	...
	Если ПараметрыРаботыКлиента.ОткрытьУведомлениеОбУстаревшейФункциональности Тогда
		ОткрытьФорму("Обработка.ГотовностьКПереходу26.Форма.ФормаУведомления26",,,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
	...
	ОповещенияПользователейОСобытияхКлиент.ПриНачалеРаботыСистемы(Параметры);
	
	#Если МобильныйКлиент Тогда
		Если ОбменМобильныеВызовПК.АвтономныйРежимДоступен() Тогда
			ОбменМобильныеАвтономныйКлиент.СоздатьУзелОбмена();
		КонецЕсли;
...
Процедура ПриГлобальномПоиске(СтрокаПоиска, ПланПоиска) Экспорт
	...
	ПланПоиска.Добавить("НайтиДокументВРеестре", "ОбщегоНазначенияУТ", Истина, , 5);
...
Функция ПроверитьНаличиеВыделенныхВСпискеСтрок(Список) Экспорт
	...
	Для Итератор = 0 По Список.ВыделенныеСтроки.ВГраница() Цикл
		Если ТипЗнч(Список.ВыделенныеСтроки[Итератор]) <> Тип("СтрокаГруппировкиДинамическогоСписка") Тогда
			ДанныеСтроки = Список.ДанныеСтроки(Список.ВыделенныеСтроки[Итератор]);
...
//
// Возвращаемое значение:
//	Массив Из ЛюбаяСсылка, ДанныеФормыСтруктура, ДанныеФормыЭлементКоллекции, ДанныеФормыЭлементДерева - массив ссылок выделенных в списке строк.
//
	...
	Для Итератор = 0 По Список.ВыделенныеСтроки.ВГраница() Цикл
		Если ТипЗнч(Список.ВыделенныеСтроки[Итератор]) <> Тип("СтрокаГруппировкиДинамическогоСписка") Тогда
			...
			Если ТипЗнч(Список.ВыделенныеСтроки[Итератор]) = Тип("КлючСтрокиДинамическогоСписка") Тогда
				МассивСсылок.Добавить(ДанныеСтроки);
			Иначе
...
Функция ЗаписатьОбъект(Форма, ПараметрыЗаписи)
	...
	Иначе
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПринудительноЗакрытьФорму") Тогда
...
Процедура ПриОпределенииВозможностиОбновления() Экспорт
	...
	ПроверитьВозможностьОбновленияСВерсии2_5_26(ПараметрыКлиента.ВерсияДанных);
...
Функция МинимальноДопустимаяВерсия() Экспорт
	...
	МинимальноДопустимаяВерсия = "11.5.22.49"; // для УТ11
...
// Выполняет проверку возможности обновления c версии ERP, КА 2.5.26 и УТ 11.5.26
// Переход со сборок < 103 должен выполняться строго через сборку 103
// 
Процедура ПроверитьВозможностьОбновленияСВерсии2_5_26(ВерсияДанных)
	
	ВерсияПроверки = "11.5.26";
	
	Если ОбщегоНазначенияКлиентСервер.ВерсияКонфигурацииБезНомераСборки(ВерсияДанных) <> ВерсияПроверки Тогда
		Возврат;
	КонецЕсли;
	
	Сборка103 = "11.5.26.103";

	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияДанных, Сборка103) < 0 Тогда
		Сообщение =
			НСтр("ru = 'Обновление на текущую версию допустимо только с версии %1.
			|(Недопустимая попытка обновления с версии %2)
			|Необходимо восстановить информационную базу из резервной копии
			|и повторить обновление согласно файлу 1cv8upd.htm'");
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение,
...
Процедура ОткрытьФормуПослеПроверкиПроведенностиДокументаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ДополнительныеПараметры.ИмяПараметраСсылка) Тогда
		...
		ХозяйственнаяОперация = ДополнительныеПараметры.ПараметрыФормы.ХозяйственнаяОперация;
		Если ЗначениеЗаполнено(ХозяйственнаяОперация) Тогда
			
			Основание = Новый Структура;
			Основание.Вставить("ДокументОснование", ДокументСсылка);
		...
		ДополнительныеПараметры.ПараметрыФормы.Вставить(ДополнительныеПараметры.ИмяПараметраСсылка, Основание);
...
Функция ДополнительныеПараметрыОбработкиОткрытияФормыПослеПроверкиПроведенностиДокумента()
	...
	ДополнительныеПараметры.Вставить("ПараметрыФормы", Новый Структура());
	ДополнительныеПараметры.ПараметрыФормы.Вставить("ХозяйственнаяОперация",
