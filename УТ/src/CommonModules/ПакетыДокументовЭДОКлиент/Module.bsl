...
Процедура ДобавитьДокументВПакетИзПрисоединенногоФайлаЗавершениеВыбора(ВыбранноеЗначение,
	...
	Если НЕ РаботаСФайламиБЭДКлиент.ФайлМожноОтправитьПоЭДО(ДанныеФайла.ИмяФайла) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьДокументВПакетИзФайлаНаДискеЗавершениеВыбораФайла(ПомещенныеФайлы, ПараметрыДобавленияВПакет) Экспорт
	...
	РезультатОперации = РаботаСФайламиБЭДКлиентСервер.ИсключитьНедопустимыеФайлы(ПомещенныеФайлы);
	РаботаСФайламиБЭДКлиент.ПоказатьПредупреждениеОНедопустимыхФайлах(РезультатОперации);
	Если НЕ ЗначениеЗаполнено(ПомещенныеФайлы) Тогда
		Возврат;
	КонецЕсли;
