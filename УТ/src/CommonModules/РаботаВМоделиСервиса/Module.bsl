...
Процедура СкопироватьДанныеОбласти(Знач ОбластьИсточник, Знач ОбластьПриемник) Экспорт
	...
	Попытка
		ВыгрузкаЗагрузкаОбластейДанных.ЗагрузитьОбластьИзАрхива(ИмяФайлаВыгрузки);
	Исключение
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииКопированиеОбластиДанных(), 
			УровеньЖурналаРегистрации.Ошибка, , , ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Попытка
...
Процедура ОбслуживаниеОбластейДанных() Экспорт
	...
	РегистрыСведений.ПотокиВыгрузкиЗагрузкиДанных.ПроверитьРаботуЗапущенныхПотоков();
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Менеджер.Статус = Перечисления.СтатусыОбластейДанных.Новая Тогда
			ИмяМетода = ИмяМетодаПодготовитьОбластьДанныхКИспользованию();
		Иначе
			РазблокироватьДанныеДляРедактирования(Ключ);
			Продолжить;
		КонецЕсли;
	...
	РасширенияВМоделиСервиса.ЗапланироватьОбновленияРасширенийВОбластях();
...
// Возвращает код абонента текущей области данных.
//
// Возвращаемое значение:
//   Число - код абонента
Функция КодАбонента() Экспорт
	...
	МенеджерЗаписи.ОбластьДанныхВспомогательныеДанные = ЗначениеРазделителяСеанса();
	УстановитьОтключениеБезопасногоРежима(Истина);
	УстановитьПривилегированныйРежим(Истина);
	...
	Если МенеджерЗаписи.Выбран() Тогда
		Результат = МенеджерЗаписи.КодАбонента;
	Иначе
		Результат = 0;
	КонецЕсли;
...
Функция ИспользуемыеПользователямиОбластиДанных() Экспорт
	...
	|ИЗ
	|	РегистрСведений.ОбластиДанных КАК ОбластиДанных
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ВидыПриложенийОбластейДанных КАК ВидыПриложенийОбластейДанных
	|		ПО ОбластиДанных.ОбластьДанныхВспомогательныеДанные = ВидыПриложенийОбластейДанных.ОбластьДанныхВспомогательныеДанные
	|ГДЕ
	|	ОбластиДанных.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОбластейДанных.Используется)
	|УПОРЯДОЧИТЬ ПО
	|	ОбластьДанных";
...
// Возвращает признак того, что информационная база находится в модели сервиса
//
// Возвращаемое значение:
//   Булево - Истина, если информационная база находится в модели сервиса
//
...
Процедура ПриОпределенииПсевдонимовОбработчиков(СоответствиеИменПсевдонимам) Экспорт
	...
	СоответствиеИменПсевдонимам.Вставить(ИмяМетодаПодготовитьОбластьДанныхКИспользованию());
...
Процедура ПриДобавленииПараметровРаботыКлиента(Параметры) Экспорт
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Если ЗначениеЗаполнено(КодАбонента) Тогда
			ДопИнфо.Добавить(СтрШаблон(НСтр("ru = 'абонент %1'"), Формат(КодАбонента, "ЧН=0; ЧГ=0")));
		КонецЕсли;
		
		КодПриложения = ЗначениеРазделителяСеанса();
		Если ЗначениеЗаполнено(КодПриложения) Тогда
			ДопИнфо.Добавить(СтрШаблон(НСтр("ru = 'приложение %1'"), Формат(КодПриложения, "ЧН=0; ЧГ=0")));
		КонецЕсли;
		
		Если ДопИнфо.Количество() > 0 Тогда
			ПредставлениеОбластиДанных = СтрШаблон("%1 [%2]", Выборка.Представление, СтрСоединить(ДопИнфо, ", "));
		Иначе
			ПредставлениеОбластиДанных = Выборка.Представление;
		КонецЕсли;
		
		Параметры.Вставить("ПредставлениеОбластиДанных", ПредставлениеОбластиДанных);
...
Процедура ПриОпределенииИсключенийНеразделенныхДанных(Исключения) Экспорт
	...
	Исключения.Добавить(Метаданные.РегистрыСведений.ПотокиВыгрузкиЗагрузкиДанных);
	Исключения.Добавить(Метаданные.РегистрыСведений.ЗапланированныеЗаданияОчереди);
...
Функция ИмяМетодаПодготовитьОбластьДанныхКИспользованию() Экспорт
	
	Возврат "РаботаВМоделиСервиса.ПодготовитьОбластьДанныхКИспользованию";
...
//   ИДФайлаВыгрузки - УникальныйИдентификатор - ид файла начальных данных.
//   Вариант - Строка - вариант начальных данных.
//   СообщениеОФатальнойОшибке - Строка - возвращаемый параметр.
//
// Возвращаемое значение:
//  Строка - один из вариантов "Успех", "ФатальнаяОшибка".
//
...
Функция ЗагрузитьОбластьИзПоставляемыхДанных(Знач ОбластьДанных, Знач ИДФайлаВыгрузки, Знач Вариант, СообщениеОФатальнойОшибке)
	...
	Попытка
		...
		ПараметрыЗагрузки.СвернутьЭлементыСправочникаПользователи = 
			(Не Константы.РежимИспользованияИнформационнойБазы.Получить() = Перечисления.РежимыИспользованияИнформационнойБазы.Демонстрационный);
		
		ВыгрузкаЗагрузкаОбластейДанных.ЗагрузитьОбластьИзАрхива(ИмяФайлаВыгрузки, ПараметрыЗагрузки);
...
Процедура ОчиститьДанныеИнформационнойБазы(УдалятьРасширения, УдалятьТекущегоПользователя = Истина)
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ВызватьИсключение НСтр(
			"ru = 'В информационной базе включено разделение, допустимо использование метода РаботаВМоделиСервиса.ОчиститьДанныеОбласти'");	
	КонецЕсли;
...
Процедура УдалитьДанныеПриложения(ОбластьДанных = Неопределено)
	...
	Если Не ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение НСтр(
			"ru = 'Удаление данных приложения может быть выполнено только при включенном использовании разделителей'");
	КонецЕсли;
	...
	Попытка
		...
		ВыключитьИспользованиеЗаданийОчереди();
...
Процедура ВыключитьИспользованиеЗаданийОчереди()
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ОчередьЗаданий.Ссылка КАК Задание
	|ИЗ
	|	Справочник.ОчередьЗаданий КАК ОчередьЗаданий
	|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		ОчередьЗаданий.ИзменитьЗадание(Выборка.Задание, Новый Структура("Использование", Ложь));
...
Процедура ОтключитьИОчиститьИсториюДанных(ОбъектМетаданных)
	...
	Если Расширение <> Неопределено
		И Не РаботаВМоделиСервисаПовтИсп.РасширениеПоддерживаетИсториюДанных(Расширение.Имя) Тогда
		Возврат;
	КонецЕсли;
...
Процедура АвторизоватьТекущегоПользователя()
	...
	Если РасширенияВМоделиСервиса.ЕстьУстановленныеРасширенияИзменяющиеСтруктуруДанных() Тогда
		...
		Задание = РасширенияКонфигурации.ВыполнитьФоновоеЗаданиеСРасширениямиБазыДанных(
			"РаботаВМоделиСервисаБТС.АвторизоватьТекущегоПользователя",
			Неопределено,
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбслуживаниеОбластейДанных", Истина);
	Методы.Вставить("УдалениеНеактуальныхДанных", Истина);
	Методы.Вставить("КонтрольРазделителейПриОбновлении");
