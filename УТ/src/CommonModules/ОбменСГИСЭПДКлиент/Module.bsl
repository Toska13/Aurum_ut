...
Процедура ПоказатьХранимыеДанныеЭПД(ГруппаДанных, Отбор, ФормаВладелец, ОписаниеОповещения, Отказ, ИдентификаторСтрокиРодителя = Неопределено) Экспорт
	...
	ИмяОбъекта = ПолучитьИмяОбъектаФормы(ФормаВладелец);
...
Функция ИдентификаторОтправителя(Объект)
	...
	ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЭлектронныйЗаказЗаявка") Тогда	
		Если ТипЗнч(Объект.СсылкаТитулГрузоотправителяГрузоотправитель) = ТипОрганизация
			Или ТипЗнч(Объект.СсылкаТитулГрузоотправителяСоставитель) = ТипОрганизация Тогда
			Результат = Объект.ИдентификаторГрузоотправителя;
		Иначе
			Результат = Объект.ИдентификаторПеревозчика;	
		КонецЕсли;
...
Процедура ЗаполнитьИдентификациюФайлаОбмена(Форма, ИнформацияПоПрефиксамТитула, ИдентификаторОтправителя, Переформировать = Ложь, ОбработкаЗавершения = Неопределено) Экспорт
	...
	Иначе
		...
		Если Форма <> Неопределено И Форма.Доступность Тогда
			ДополнительныеПараметры.Вставить("ТребуетсяРазблокироватьФорму", Истина);
...
Процедура ЗаполнитьИдентификациюФайлаОбмена_ПослеПолученияУИД(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ТребуетсяРазблокироватьФорму", ТребуетсяРазблокироватьФорму)
		И ТребуетсяРазблокироватьФорму Тогда
		...
		Если Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул1") Тогда
			...
			Если ТитулГрузоотправителяЛицоОсуществляющееПогрузкуЯвляетсяГрузоотправителем = "1"
				...
				Если СтрДлина(РеквизитыКонтрагента.ИНН) = 10 Тогда
					...
					ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
						Форма, "ТитулГрузоотправителяРеквизитыГрузоотправителяФЛ_ИНН", Неопределено, , КонтейнерОтображаемойВерсии);
					
				Иначе

					ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
						Форма, "ТитулГрузоотправителяРеквизитыГрузоотправителяФЛ_ИНН", РеквизитыКонтрагента.ИНН, , КонтейнерОтображаемойВерсии);
			...
			Если ОбменСГИСЭПДКлиентСервер.ПолучитьЗначениеРеквизитаИзСтруктурыФормы(
				...
				Если СтрДлина(РеквизитыКонтрагента.ИНН) = 10 Тогда
					...
					ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
						Форма, "ТитулГрузоотправителяИнойГрузоотправительФЛ_ИНН", Неопределено, , КонтейнерОтображаемойВерсии);
					
				Иначе
					
					ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
						Форма, "ТитулГрузоотправителяИнойГрузоотправительФЛ_ИНН", РеквизитыКонтрагента.ИНН, , КонтейнерОтображаемойВерсии);
