...
//   ЗарегистрироватьДляОбмена - Булево - Истина, если требуется зарегистрировать объект ИС для обмена с ДО.
//
Процедура ДобавитьСвязь(ID, Тип, ИнтегрированныйОбъект, НаличиеПрисоединенныхФайлов = Неопределено,
		ДобавитьСвязьВДО = Истина, ЗарегистрироватьДляОбмена = Ложь) Экспорт
	...
	Попытка
		...
		Если ЗарегистрироватьДляОбмена Тогда
			УзелДокументооборота = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.УзелДокументооборота();
			ПланыОбмена.ЗарегистрироватьИзменения(УзелДокументооборота, ИнтегрированныйОбъект);
		КонецЕсли;
...
//
Функция СоздатьОбъектДОПоПравилу(ОбъектИС, Правило, СвязиКСозданию = Неопределено,
		ПредварительныеДанныеВХранилище = Неопределено) Экспорт
	...
	ИначеЕсли Настройки.ИспользоватьИнтеграциюДО3 Тогда
		...
		ОбъектДОИлиСообщение = МодульИнтеграцияС1СДокументооборот3.СоздатьОбъектДОПоПравилу(
			...
			Правило,
			СвязиКСозданию,
			ПредварительныеДанныеВХранилище);
	Иначе
...
Процедура ПриИзмененииСостоянияСогласования(Знач Идентификатор, Знач Тип, Знач Состояние, Знач ВызовИзФормыОбъекта,
		Знач ПредметСогласования = Неопределено, Знач Установил = Неопределено, Знач ДатаУстановки = Неопределено) Экспорт
...
Функция ДвоичныеДанныеФайла(ID, ИспользоватьВизуализациюШтампаЭП = Истина) Экспорт
	...
	Файлы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ID);
	ДвоичныеДанныеФайлов = ДвоичныеДанныеФайлов(Файлы, ИспользоватьВизуализациюШтампаЭП);
	
	Возврат ДвоичныеДанныеФайлов[ID];
...
//
// Параметры:
//   Файлы - Массив из Строка - массив идентификаторов файлов, данные которых нужно получить.
//   ИспользоватьВизуализациюШтампаЭП - Булево - требуется получить двоичные данные файла с визуализацией штампа ЭП.
//
// Возвращаемое значение:
//   Соответствие из КлючИЗначение:
//     * Ключ - Строка - идентификатор файла.
//     * Значение - ДвоичныеДанные - двоичные данные файла из Документооборота.
//
Функция ДвоичныеДанныеФайлов(Файлы, ИспользоватьВизуализациюШтампаЭП = Истина) Экспорт
	...
	Для Каждого ФайлXDTO Из ДанныеФайлов(Файлы, ИспользоватьВизуализациюШтампаЭП) Цикл
		ДвоичныеДанныеФайлов.Вставить(ФайлXDTO.objectID.ID, ФайлXDTO.binaryData);
	КонецЦикла;
...
Функция ОписаниеФайла(ID, Форма = Неопределено, АдресВременногоХранилищаФайла = "") Экспорт
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(СведенияОФайле, "template") Тогда
		...
		ОписаниеФайла.Вставить("ШаблонID", СведенияОФайле.template.objectID.ID);
	Иначе
		ОписаниеФайла.Вставить("Шаблон", "");
		ОписаниеФайла.Вставить("ШаблонID", "");
	КонецЕсли;
	...
	Для Каждого ПодписьXDTO Из СведенияОФайле.signatures Цикл
		Подписи.Добавить(ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДанныеПодписиИзXDTO(ПодписьXDTO));
	КонецЦикла;
...
//   ИспользоватьВизуализациюШтампаЭП - Булево - требуется получить двоичные данные файла с визуализацией штампа ЭП.
//   УникальныйИдентификаторФормы - УникальныйИдентификатор - идентификатор вызвавшей формы.
//   ДанныеТекущегоФайла - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеФайла
//
// Возвращаемое значение:
...
Функция ПолучитьВерсиюФайлаИПоместитьВХранилище(ID, ИспользоватьВизуализациюШтампаЭП, УникальныйИдентификаторФормы,
		ДанныеТекущегоФайла = Неопределено) Экспорт
	...
	ДанныеВерсии = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьОбъект(
		...
		ID,
		ПолучаемыеПоля);
	Если ДанныеТекущегоФайла <> Неопределено Тогда
		ДанныеТекущегоФайла.Размер = ДанныеВерсии.size;
		ДанныеТекущегоФайла.ДатаМодификацииУниверсальная = ДанныеВерсии.modificationDateUniversal;
		ДанныеТекущегоФайла.Зашифрован = ДанныеВерсии.encrypted;
	КонецЕсли;
...
// Создает новый ДокументPDF, заполняет его данными из файла предпросмотра.
//
// Параметры:
//   АдресВХранилищеФайлаПредпросмотра - Строка - адрес файла предпросмотра во временном хранилище на сервере.
//
// Возвращаемое значение:
//   ДокументPDF
//   Неопределено - если во временном хранилище не оказалось данных файла.
//
Функция ПолучитьДокументPDF(АдресВХранилищеФайлаПредпросмотра) Экспорт
	
	ДанныеФайла = ПолучитьИзВременногоХранилища(АдресВХранилищеФайлаПредпросмотра);
...
Функция ПолучитьФайлИПоместитьВХранилище(ID, ИспользоватьВизуализациюШтампаЭП, УникальныйИдентификаторФормы,
		ДатаЗаема = Неопределено, ДанныеТекущегоФайла = Неопределено) Экспорт
	...
	Если ДанныеТекущегоФайла <> Неопределено Тогда
		ДанныеТекущегоФайла.Размер = ДанныеФайла.size;
		ДанныеТекущегоФайла.ДатаМодификацииУниверсальная = ДанныеФайла.modificationDateUniversal;
		ДанныеТекущегоФайла.Зашифрован = ДанныеФайла.encrypted;
	КонецЕсли;
...
Процедура ПолучитьОбъектыДляПодписи(ОбъектНаименование, ОбъектID, ОбъектТип, МассивОбъектовДляПодписи, Файлы,
		АдресДвоичныхДанныхОбъекта) Экспорт
	
	Прокси = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ПолучитьПрокси();
	
	СписокОбъектов = Новый Массив;
	Если СтрНайти(ОбъектТип, "Document") > 0 Тогда
		СписокОбъектов.Добавить(Новый Структура("ID, Тип", ОбъектID, ОбъектТип));
	КонецЕсли;
	...
	ДанныеОбъектовXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьОбъекты(
		Прокси,
		СписокОбъектов,
		Колонки);
	
	Для Каждого ОбъектXDTO Из ДанныеОбъектовXDTO Цикл
		ОбъектДляПодписи = ОбъектДляПодписи();
		ОбъектДляПодписи.ОбъектСсылкаДляПодписи = ОбъектXDTO.objectID.ID;
		ОбъектДляПодписи.ОбъектТип = ОбъектXDTO.objectID.type;
		ОбъектДляПодписи.ОбъектНаименование = ОбъектXDTO.name;
		Если ОбъектДляПодписи.ОбъектТип = "DMFile" Тогда
			ОбъектДляПодписи.Зашифрован = ОбъектXDTO.encrypted;
			ОбъектДляПодписи.ДвоичныеДанные = ОбъектXDTO.binaryData;
			АдресДвоичныхДанныхОбъекта.АдресаДвоичныхДанныхФайлов[ОбъектXDTO.objectID.ID] =
				ПоместитьВоВременноеХранилище(
					ОбъектXDTO.binaryData,
					АдресДвоичныхДанныхОбъекта.УникальныйИдентификаторФормы);
		Иначе
			ОбъектДляПодписи.Зашифрован = Ложь;
			ОбъектДляПодписи.ДвоичныеДанные = ОбъектXDTO.keyPropertiesValue;
			АдресДвоичныхДанныхОбъекта.АдресСлепкаДокумента = ПоместитьВоВременноеХранилище(
				ОбъектXDTO.keyPropertiesValue,
				АдресДвоичныхДанныхОбъекта.УникальныйИдентификаторФормы);
		КонецЕсли;
...
//
Процедура ПроверитьПодписи(ТаблицаПодписей, ВыделенныеПодписи = Неопределено, АдресДвоичныхДанныхОбъекта = Неопределено,
...
// подписей для объектов 1С:Документооборота. Предназначена для определения значения настройки на клиенте.
//
// Возвращаемое значение:
//   Булево
//
Функция СоздаватьЭПДляОбъектов1СДокументооборотаСредствамиИС() Экспорт
	...
	Возврат ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздаватьЭПДляОбъектов1СДокументооборотаСредствамиИС();
...
//
// Возвращаемое значение:
//   Соответствие из КлючИЗначение - неявно возвращаемое значение. Возвращает файлы, которые
//         нужно оповестить о том, что их подписи были удалены:
//     * Ключ - Строка - идентификатор файла.
...
Функция УдалитьПодписи(УдаляемыеПодписи, ТаблицаПодписей) Экспорт
	...
	ПодписиОбъектов = Новый Соответствие;
	ФайлыДляОповещения = Новый Соответствие;
	...
	Для Каждого ДанныеСтроки Из ТаблицаПодписей Цикл
		...
		Для Каждого СтрокаУдаленнойПодписи Из УдаляемыеПодписи Цикл
			Если СтрокаУдаленнойПодписи.ОбъектID = ДанныеСтроки.ОбъектID
				...
				Если ДанныеСтроки.ОбъектТип = "DMFile"
						И ФайлыДляОповещения[ДанныеСтроки.ОбъектID] = Неопределено Тогда
					ФайлыДляОповещения[ДанныеСтроки.ОбъектID] = ДанныеСтроки.ОбъектИмя;
				КонецЕсли;
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
//
Функция ДанныеФайлов(Файлы, ИспользоватьВизуализациюШтампаЭП = Истина)
...
Процедура ЗаполнитьСтрокуСпискаФайлов(СтрокаСпискаФайлов, ФайлXDTO, ТекущийПользовательИСотрудники)
	...
	Если СтрокаСпискаФайлов.Свойство("ДатаМодификации") И ЗначениеЗаполнено(ФайлXDTO.modificationDateUniversal) Тогда
