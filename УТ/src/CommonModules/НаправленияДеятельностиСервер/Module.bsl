...
#Область НастройкаДоступности

// Установить возможность редактирования НД.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
//  ПараметрыДоступности - см. ПараметрыВозможностьРедактированияНД
//
Процедура УстановитьВозможностьРедактированияНД(Форма, ПараметрыДоступности) Экспорт
	
	Если Не ЗначениеЗаполнено(ПараметрыДоступности.ИмяЭлементаНД) Тогда
		Возврат;
	КонецЕсли;
	
	Если ПараметрыДоступности.ЭтоЗаказ
		И ПараметрыДоступности.ЗаказКакСчет Тогда
		
		ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, ПараметрыДоступности.ИмяЭлементаНД, "ТолькоПросмотр", Истина);
		
	Иначе
		
		НДДоговораЗаполнено = Ложь;
		Если ЗначениеЗаполнено(ПараметрыДоступности.Договор)
			И (ТипЗнч(ПараметрыДоступности.Договор) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
				Или ТипЗнч(ПараметрыДоступности.Договор) = Тип("СправочникСсылка.ДоговорыМеждуОрганизациями")) Тогда
			УстановитьПривилегированныйРежим(Истина);
			НДДоговораЗаполнено = ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыДоступности.Договор, "НаправлениеДеятельности"));
			УстановитьПривилегированныйРежим(Ложь);
		КонецЕсли;
		
		Если ПараметрыДоступности.ЭтоЗаказ Тогда
			
			ТолькоПросмотр =
				Не ПараметрыДоступности.ПорядокРасчетов = ПредопределенноеЗначение("Перечисление.ПорядокРасчетов.ПоЗаказам")
				И Не ПараметрыДоступности.ПорядокРасчетов = ПредопределенноеЗначение("Перечисление.ПорядокРасчетов.ПоЗаказамНакладным")
				И Не ПараметрыДоступности.ПорядокРасчетов = ПредопределенноеЗначение("Перечисление.ПорядокРасчетов.ПоНакладным")
				И Не (ПараметрыДоступности.ПорядокРасчетов = ПредопределенноеЗначение("Перечисление.ПорядокРасчетов.ПоДоговорамКонтрагентов")
					И Не НДДоговораЗаполнено)
				И Не (ПараметрыДоступности.ПорядокРасчетов = ПредопределенноеЗначение("Перечисление.ПорядокРасчетов.ПоДоговорамНакладным")
					И Не НДДоговораЗаполнено);
			
		ИначеЕсли ПараметрыДоступности.НакладнаяПоЗаказу Тогда
			
			ТолькоПросмотр =
				Не ПараметрыДоступности.ПорядокРасчетов = ПредопределенноеЗначение("Перечисление.ПорядокРасчетов.ПоНакладным")
				И Не ПараметрыДоступности.ПорядокРасчетов = ПредопределенноеЗначение("Перечисление.ПорядокРасчетов.ПоЗаказамНакладным")
				И Не ПараметрыДоступности.ПорядокРасчетов = ПредопределенноеЗначение("Перечисление.ПорядокРасчетов.ПоДоговорамНакладным");
			
		ИначеЕсли ПараметрыДоступности.ЭтоСправочник Тогда
			ТолькоПросмотр = Ложь;
		Иначе
			
			ТолькоПросмотр =
				ПараметрыДоступности.ПорядокРасчетов = ПредопределенноеЗначение("Перечисление.ПорядокРасчетов.ПоДоговорамКонтрагентов")
					И НДДоговораЗаполнено;
			
		КонецЕсли;
		
		ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, ПараметрыДоступности.ИмяЭлементаНД, "ТолькоПросмотр", ТолькоПросмотр);
		
	КонецЕсли;
	
КонецПроцедуры

// Параметры возможность редактирования НД.
// 
// Возвращаемое значение:
//  Структура - Параметры возможность редактирования НД:
// * ЭтоСправочник - Булево
// * ЭтоЗаказ - Булево
// * ЗаказКакСчет - Булево
// * НакладнаяПоЗаказу - Булево
// * Договор - СправочникСсылка.ДоговорыКонтрагентов, СправочникСсылка.ДоговорыМеждуОрганизациями, Неопределено - договор, указанный в документе.
// * ПорядокРасчетов - ПеречислениеСсылка.ПорядокРасчетов
// * ИмяЭлементаНД - Строка
//
Функция ПараметрыВозможностьРедактированияНД() Экспорт
	
	ПараметрыДоступности = Новый Структура;
	
	ПараметрыДоступности.Вставить("ЭтоСправочник",      Ложь);
	ПараметрыДоступности.Вставить("ЭтоЗаказ",           Ложь);
	ПараметрыДоступности.Вставить("ЗаказКакСчет",       Ложь);
	ПараметрыДоступности.Вставить("НакладнаяПоЗаказу",  Ложь);
	
	ПараметрыДоступности.Вставить("Договор",            Неопределено);
	ПараметрыДоступности.Вставить("ПорядокРасчетов",    Перечисления.ПорядокРасчетов.ПустаяСсылка());
	
	ПараметрыДоступности.Вставить("ИмяЭлементаНД",      "НаправлениеДеятельности");
...
Функция ХозяйственнаяОперацияОбразуетДоход(ХозяйственнаяОперация) Экспорт
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию
		...
		ИЛИ ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПроизводствоПродукцииИзДавальческогоСырья2_5
		ИЛИ ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РемонтПродукцииИзДавальческогоСырья2_5
		ИЛИ ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыполнениеРаботСИспользованиемДавальческогоСырья2_5
		ИЛИ ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыполнениеРемонтныхРаботСИспользованиемДавальческогоСырья2_5
