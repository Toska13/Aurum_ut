...
//
// Параметры:
//  Объект		- ДокументОбъект.КорректировкаПриобретения, ДанныеФормыСтруктура	- документ, для которого необходимо получить параметры.
//
// Возвращаемое значение:
...
Функция ПараметрыРегистрацииУведомленийОВвозеПрослеживаемыхТоваров(Объект) Экспорт
	...
	ПараметрыРегистрации.Вставить("Ссылка",	Объект.ДокументОснование);
	...
	Если ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
		ХозяйственнаяОперацияОснования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументОснование, "ХозяйственнаяОперация");
...
//++ Локализация
	...
	|	ЕСТЬNULL(ВидыЗапасовСкладские.Ссылка, ТаблицаТовары.ВидЗапасов) КАК ВидЗапасов,
	|	ВЫБОР
	...
	|					ТОГДА ТаблицаТовары.СуммаВзаиморасчетовСНДС
