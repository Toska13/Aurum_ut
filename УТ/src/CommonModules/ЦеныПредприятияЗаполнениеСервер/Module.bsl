...
Функция ЗаполнитьЦены(ТабличнаяЧасть,
	...
	Если Параметры.РассчитыватьНаборы Тогда
		...
		Если НЕ ЕстьКоличествоУпаковок Тогда
			Таблица.Колонки.Добавить("КоличествоУпаковок", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
	...
	Если ЗначениеЗаполнено(Параметры.ВидЦены) Тогда
		...
		|							Т.Номенклатура
		|						ИЗ
		|							втТаблицаТовары КАК Т)) КАК СтавкиНДСНоменклатуры
		|		ПО (втТаблицаТовары.Номенклатура = СтавкиНДСНоменклатуры.Номенклатура)
	...
	ИначеЕсли ЗначениеЗаполнено(Параметры.Соглашение) Тогда
		...
		Если ТипЗнч(Параметры.Соглашение) = Тип("СправочникСсылка.СоглашенияСКлиентами")
			И ПродажиСервер.ЭтоСоглашениеПоНовойКомиссии(Параметры.Соглашение) Тогда
				// Для "СоглашенийПоНовойКомиссии" заполнение цен должно выполняться по договору
			Возврат ЦеныЗаполнены;
		КонецЕсли;
...
Процедура ДополнитьТекстЗапросаПроверитьЗаполнитьЦенуВыпуска(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		...
		Если Не ПараметрыДействия.Свойство("ВидЦены") Тогда
			ПараметрыДействия.Вставить("ВидЦены", Справочники.ВидыЦен.ПустаяСсылка());
		КонецЕсли;
...
// * ИмяКолонкиСоглашение - Строка - Если колонки в таблице нет, то используется параметр "Соглашение"
// * ИмяКолонкиПартнер - Строка - Если колонки в таблице нет, то используется параметр "Партнер"
Функция ИменаКолонокВременнойТаблицыТоваров() Экспорт
	...
	ИменаКолонок.Вставить("ИмяКолонкиСоглашение",	"");
	ИменаКолонок.Вставить("ИмяКолонкиПартнер",		"");
...
Функция ТекстЗапросаЦен2_5(ИмяВременнойТаблицыТоваров, ПолучатьСтавкуНДС = Ложь)
	...
	|	ТаблицаОтбора.Номенклатура КАК Номенклатура,
	|	ТаблицаОтбора.Упаковка КАК УпаковкаТовара,
	|	ЕСТЬNULL(Цены.Упаковка, ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)) КАК УпаковкаЦены
