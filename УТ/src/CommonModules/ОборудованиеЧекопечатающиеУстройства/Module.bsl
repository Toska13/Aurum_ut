		...
		Если ОбщиеПараметры.ПозицииЧека <> Неопределено Тогда
			...
			Для ИндексМассива = 0 По ОбщиеПараметры.ПозицииЧека.Количество() - 1 Цикл
				...
				Если ТекущаяПозиция.Свойство("ФискальнаяСтрока") Тогда // Фискальная строка
					...
					Если ТекущаяПозиция.ИспользованСертификатФСС Тогда       
						ЗапросПроверкиКода = ТекущаяПозиция.ЗапросПроверкиКода; 
						Если Не ПустаяСтрока(ЗапросПроверкиКода.ИдентификаторЗапроса) И Не ПустаяСтрока(ЗапросПроверкиКода.ВременнаяМетка) Тогда
							ЗаполнитьОтраслевойРеквизитСертификатФСС(ТекущаяПозиция);
						КонецЕсли;   
					КонецЕсли;   
...
Процедура ОбновитьПоставляемыеДрайвера(ДрайвераОборудования) Экспорт
	...
	Драйвер.ИдентификаторОбъекта = "EvoKKMExtension47"; 
	...
	Драйвер = ДрайвераОборудования.Добавить();
	Драйвер.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ККТ;
	Драйвер.ИмяДрайвера  = "ДрайверККТТорговыйБаланс54ФЗ";
	Драйвер.Наименование = НСтр("ru = 'Торговый Баланс:ККТ с передачей данных в ОФД'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	Драйвер.ИдентификаторОбъекта = "TBMDrvFR1C4";       
...
Функция ПодготовитьДанныеОперации(ПараметрыПодключения, Команда, ПараметрыОперации) Экспорт
	...
	Если Команда = "CheckFiscalization" Тогда
		...
		Если ОбщегоНазначенияБПО.ИспользуетсяМаркировка() Тогда   
			МодульМенеджерОборудованияМаркировка = ОбщегоНазначенияБПО.ОбщийМодуль("МенеджерОборудованияМаркировка");
			ПараметрыФискализации.ОтчетВыбытииМТ = МодульМенеджерОборудованияМаркировка.СформироватьДанныеОтчетаВыбытииМТ(ПараметрыОперации, ПараметрыФискализации);
		КонецЕсли;
	...
	ИначеЕсли Команда = "PiotStart" Или Команда = "PiotCheckKM" Тогда  
		...
		Если ПараметрыОперации.Свойство("Кассир") И ПараметрыОперации.Свойство("КассирИНН") Тогда
			ТекущийКассирФИО = ПараметрыОперации.Кассир; 
			ТекущийКассирИНН = ПараметрыОперации.КассирИНН;   
		Иначе
			ТекущийКассир = МенеджерОборудованияКлиентСервер.ТекущийКассирДляФискальныхОпераций();   
			ТекущийКассирФИО = ТекущийКассир.Кассир; 
			ТекущийКассирИНН = ТекущийКассир.КассирИНН;   
		КонецЕсли;
		ТекущийКассир = МенеджерОборудованияКлиентСервер.ТекущийКассирДляФискальныхОпераций();   
		ПараметрыАутентификации.Вставить("CashierName", ТекущийКассирФИО); 
		ПараметрыАутентификации.Вставить("CashierINN",  ТекущийКассирИНН);
...
Процедура ЗаполнитьОтраслевойРеквизитСертификатФСС(ТекущаяПозиция);
	
	// В Правила маркировки ТСР внесли комплектные изменения, один которых подразумевает передачу сведении в ГИС МТ (пп.О пункта 98):
	// о) сведения отраслевого реквизита предмета расчета фискального документа о реализации товаров, 
	// подтверждающие приобретение товаров с использованием электронного сертификата за счет средств федерального бюджета и бюджета 
	// Фонда пенсионного и социального страхования Российской Федерации.
	ЗначениеРеквизита = "calc=CERT";
	ОтраслевойРеквизит = ТекущаяПозиция.ОтраслевойРеквизит;
