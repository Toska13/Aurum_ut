...
Процедура ОткрытьТекущиеДелаЭДО(ПараметрыЗапуска = Неопределено, ОповещениеОЗакрытии = Неопределено,
	...
	КонецЕсли;
	ФормаТекущиеДелаПоЭДО = ФормаТекущиеДелаПоЭДО();
	КлючЗамера = СтрШаблон("%1.ОткрытиеФормы", ФормаТекущиеДелаПоЭДО);
...
// Возвращаемое значение:
//  Строка
Функция ИмяСобытияИзменениеДанныхСопоставленияНоменклатурыДокумента() Экспорт
	Возврат "ОбновитьСопоставлениеНоменклатурыДокумента";
КонецФункции
...
Процедура ЗагрузитьФормализованныйДокументПоДаннымФайлаXMLПослеДлительнойОперации(Результат,
	...
	Если РезультатЗагрузки.Предупреждения.Количество() > 0 И Не ЗначениеЗаполнено(РезультатЗагрузки.Ошибка) Тогда
...
// Параметры:
//  Комментарий - Строка
Процедура ПоказатьКомментарийКСостояниюДокумента(Комментарий) Экспорт
	
	Параметры = ОбщегоНазначенияБЭДКлиент.ПараметрыВводаСтроки();
	Параметры.ЗаголовокФормы = НСтр("ru = 'Комментарий'");
	Параметры.Строка = Комментарий;
	Параметры.Многострочность = Истина;
	ОбщегоНазначенияБЭДКлиент.ПоказатьВводСтрокиБЭД(Новый ОписаниеОповещения(), Параметры);
...
Процедура ОткрытьНовыйПроизвольныйЭлектронныйДокументНаОснованииПослеВыбораФайла(ОписаниеФайла,
	...
	Если НЕ РаботаСФайламиБЭДКлиент.ФайлМожноОтправитьПоЭДО(ОписаниеФайла.Имя) Тогда
		Возврат;
	КонецЕсли;
...
Процедура НачатьСозданиеЭлектронногоДокументаПоФайлу(ОповещениеОЗавершении, ПараметрыФормирования, ПараметрыФайла) Экспорт
	...
	Если НЕ РаботаСФайламиБЭДКлиент.ФайлМожноОтправитьПоЭДО(ПараметрыФайла.ИмяФайла) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПослеВыполненияДействийПоЭДО(Результат, ПараметрыОповещения) Экспорт
	...
	ОповеститьОбИзмененииДанныхСопоставленияНоменклатурыПослеВыполненияДействийПоЭДО(Результат, ЭлектронныеДокументы);
...
Процедура ОтправитьПолучитьДокументыЭДО(Форма, ОповещениеОЗавершении = Неопределено, УчетныеЗаписиОбмена = Неопределено) Экспорт
	...
	Контекст = НовыйКонтекстОтправкиПолученияДокументовЭДО();
	Контекст.ПараметрыОтправкиПолучения = ПараметрыОтправкиПолучения;
	Контекст.УчетныеЗаписиОбмена = УчетныеЗаписиОбмена;
	Если ОповещениеОЗавершении <> Неопределено Тогда
		Контекст.ОповещениеОЗавершении = ОповещениеОЗавершении;
	КонецЕсли;
	
	Если ПараметрыОтправкиПолучения.ПоказыватьСписокДокументовТакскомТребующихЗавершения Тогда
		
		ОписаниеОповещенияОЗавершенииВопроса = Новый ОписаниеОповещения(
			"ПродолжитьПослеПоказаВопросаЗавершенияДокументовТакском",
			ЭтотОбъект,
			Контекст);
		
		ПараметрыВопроса = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
		ПараметрыВопроса.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Истина;
		ПараметрыВопроса.Заголовок = НСтр("ru = 'Некоторые документы требуют завершения'");
		
		СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(
			ОписаниеОповещенияОЗавершенииВопроса,
			НСтр("ru = 'Некоторые документы требуют завершения документооборота в связи переключением оператора Такском на платформу 1С:Хаб. Показать список?'"),
			РежимДиалогаВопрос.ДаНет,
			ПараметрыВопроса);
		
	Иначе

		ОтправитьПолучитьДокументыЭДОСУчетомПринятияУсловийСервиса(Контекст);
	КонецЕсли;
...
//              * Значение - КодВозвратаДиалога
//              * БольшеНеЗадаватьЭтотВопрос - Булево
//  Контекст - см. НовыйКонтекстОтправкиПолученияДокументовЭДО 
Процедура ПродолжитьПослеПоказаВопросаЗавершенияДокументовТакском(РезультатВопроса, Контекст) Экспорт
	...
	Если РезультатВопроса.БольшеНеЗадаватьЭтотВопрос = Истина Тогда
		СохранитьНастройкуНеПоказыватьВопросОСпискеДокументовТакском();
	КонецЕсли;
	...
	ОписаниеОповещенияОЗавершенииПросмотраСписка = Новый ОписаниеОповещения(
			"ПродолжитьПослеПросмотраСпискаДокументовТакскомКЗавершению",
			ЭтотОбъект,
			Контекст);
			
	ОткрытьФорму("Обработка.ИнтерфейсДокументовЭДО.Форма.СписокДокументовТакскомКЗавершению",,,,,,
		ОписаниеОповещенияОЗавершенииПросмотраСписка, 
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
//  Контекст - см. НовыйКонтекстОтправкиПолученияДокументовЭДО
Процедура ПродолжитьПослеПросмотраСпискаДокументовТакскомКЗавершению(Результат, Контекст) Экспорт
	ОтправитьПолучитьДокументыЭДОСУчетомПринятияУсловийСервиса(Контекст);
КонецПроцедуры
...
//  * ОшибкиФормирования - Массив из Структура
//  * КонтекстДиагностики - см. ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики
//  КонтекстОтправкиПолучения - см. НовыйКонтекстОтправкиПолученияДокументовЭДО
Процедура ЗавершитьОтправкуПолучениеДокументовЭДО(Результат, КонтекстОтправкиПолучения) Экспорт
	
	КонтекстОтправкиПолучения.Результат = Результат;
	
	Если КонтекстОтправкиПолучения.ПараметрыОтправкиПолучения.ВключатьАвтоматическоеПолучениеЭДО Тогда
		
		Оповещение = Новый ОписаниеОповещения(
			"ЗавершитьОтправкуПолучениеПослеВключенияАвтоматическогоПолученияЭДО",
			ЭтотОбъект, КонтекстОтправкиПолучения);
		
		АвтоматическоеПолучениеЭДОКлиент.ВключитьПослеОтправкиПолученияДокументовЭДО(
			Оповещение, Результат.ПаролиСертификатов);
		
	Иначе
		
		ПослеОтправкиПолученияДокументовЭДО(Результат, КонтекстОтправкиПолучения.ОповещениеОЗавершении);
...
// Параметры:
//  Результат - см. ЗавершитьОтправкуПолучениеДокументовЭДО.Результат
//  ОповещениеОЗавершении - см. ОтправитьПолучитьДокументыЭДО.ОповещениеОЗавершении
Процедура ПослеОтправкиПолученияДокументовЭДО(Результат, ОповещениеОЗавершении) Экспорт
...
//  СтруктураОтвета - См. ИнтерфейсДокументовЭДОКлиентСервер.ДанныеРучногоФормированияТитула
//
Процедура ПодписатьДокументИзФормыРучногоФормированияОтветногоТитула(Форма, СтруктураОтвета) Экспорт
...
Процедура ОткрытьЭлектронныйДокументОбъектаУчета(ОбъектУчета, ПараметрыОткрытия = Неопределено) Экспорт
	...
	КлючЗамера = "ОткрытьЭлектронныйДокументОбъектаУчета";
	ОписаниеЗамера = ОбщегоНазначенияБЭДКлиент.НачатьЗамер(КлючЗамера);
	Если ПараметрыОткрытия = Неопределено Тогда
		ПараметрыОткрытия = НовыеПараметрыОткрытияЭлектронногоДокумента();
	КонецЕсли;
...
Процедура ОткрытьФормуЭлектронногоДокумента(ПараметрыФормы) Экспорт
	...
	КонецЕсли;
	КлючЗамера = СтрШаблон("%1.ОткрытиеФормы", ИмяФормы);
	ОписаниеЗамера = ОбщегоНазначенияБЭДКлиент.НачатьЗамер(КлючЗамера);
...
//  * КонтекстДиагностики - см. ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики
//  ЭлектронныеДокументы - Массив из ДокументСсылка.ЭлектронныйДокументВходящийЭДО,ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
Процедура ОповеститьОбИзмененииДанныхСопоставленияНоменклатурыПослеВыполненияДействийПоЭДО(Результат,
	ЭлектронныеДокументы)

	ДействиеОтклонения = ПредопределенноеЗначение("Перечисление.ДействияПоЭДО.Отклонить");
	ДействиеПринудительногоЗакрытия = ПредопределенноеЗначение("Перечисление.ДействияПоЭДО.ЗакрытьПринудительно");
	ДействиеВозвращенияВРаботу = ПредопределенноеЗначение("Перечисление.ДействияПоЭДО.ВернутьВРаботу");

	Если Результат.Итог.ОбработаноПоДействиям[ДействиеОтклонения] = Истина
		Или Результат.Итог.ОбработаноПоДействиям[ДействиеПринудительногоЗакрытия] = Истина
		Или Результат.Итог.ОбработаноПоДействиям[ДействиеВозвращенияВРаботу] = Истина Тогда
		Оповестить(ИмяСобытияИзменениеДанныхСопоставленияНоменклатурыДокумента(), ЭлектронныеДокументы);
	КонецЕсли;
...
Процедура ОбработатьРезультатПодготовкиПараметровПросмотраОткрытьНовыйДокумент(ПараметрыПросмотра, ОбъектУчета)
	...
	ТипПрочее = ПредопределенноеЗначение("Перечисление.ТипыДокументовЭДО.Прочее");
	ОписаниеОбъектаУчета = ПредварительныйПросмотр.РезультатПоискаНастроек.ОписаниеОбъектаУчета;
	Если ОписаниеОбъектаУчета.ТипДокумента = ТипПрочее
		И Не ПредварительныйПросмотр.РезультатПодготовкиВложения.Успех Тогда
		
		ОткрытьНовыйПроизвольныйЭлектронныйДокументНаОсновании(ОбъектУчета);
...
// Возвращаемое значение:
//  Структура - Новый контекст отправки получения документов ЭДО:
//  * ОповещениеОЗавершении - см. ОтправитьПолучитьДокументыЭДО.ОповещениеОЗавершении
//  * УчетныеЗаписиОбмена - см. ОтправитьПолучитьДокументыЭДО.УчетныеЗаписиОбмена
//  * ПараметрыОтправкиПолучения - см. ИнтерфейсДокументовЭДОВызовСервера.ПараметрыОтправкиПолученияДокументовЭДО
//  * Результат - см. ЗавершитьОтправкуПолучениеДокументовЭДО.Результат
Функция НовыйКонтекстОтправкиПолученияДокументовЭДО()
	
	Контекст = Новый Структура;
	Контекст.Вставить("ОповещениеОЗавершении", Новый ОписаниеОповещения);
	Контекст.Вставить("УчетныеЗаписиОбмена", Неопределено);
	Контекст.Вставить("ПараметрыОтправкиПолучения", Новый Структура);
...
//  КонтекстОтправкиПолучения - см. НовыйКонтекстОтправкиПолученияДокументовЭДО
Процедура ОтправитьПолучитьДокументыЭДОПослеПринятияУсловийСервиса(УсловияПриняты, КонтекстОтправкиПолучения) Экспорт
	
	Если УсловияПриняты <> Истина Тогда
		ВыполнитьОбработкуОповещения(КонтекстОтправкиПолучения.ОповещениеОЗавершении);
		Возврат;
	КонецЕсли;
	
	ПодготовитьОтборУчетныхЗаписейДляОтправкиПолученияДокументовЭДО(КонтекстОтправкиПолучения);
	
КонецПроцедуры

// Параметры:
//  КонтекстОтправкиПолучения - см. НовыйКонтекстОтправкиПолученияДокументовЭДО
Процедура НачатьОтправкуПолучениеДокументовЭДО(КонтекстОтправкиПолучения)
	
	ПараметрыВыполненияДействий = КонтекстОтправкиПолучения.ПараметрыОтправкиПолучения.ПараметрыВыполненияДействий;
	
	Оповещение = Новый ОписаниеОповещения("ЗавершитьОтправкуПолучениеДокументовЭДО",
		ИнтерфейсДокументовЭДОКлиент, КонтекстОтправкиПолучения);
...
// Параметры:
//  КонтекстОтправкиПолучения - см. НовыйКонтекстОтправкиПолученияДокументовЭДО
Асинх Процедура ПодготовитьОтборУчетныхЗаписейДляОтправкиПолученияДокументовЭДО(КонтекстОтправкиПолучения)
	
	УчетныеЗаписиОбмена = КонтекстОтправкиПолучения.УчетныеЗаписиОбмена;
	ПараметрыОтправкиПолучения = КонтекстОтправкиПолучения.ПараметрыОтправкиПолучения;
	...
	Если ПараметрыОтправкиПолучения.ИспользуетсяОтправкаПолучениеСОтборомПоОрганизации = Ложь 
		...
		НачатьОтправкуПолучениеДокументовЭДО(КонтекстОтправкиПолучения);
		Возврат;
...
//  КонтекстОтправкиПолучения - см. НовыйКонтекстОтправкиПолученияДокументовЭДО
Процедура ОтправитьПолучитьДокументыЭДОПослеОтбораУчетныхЗаписей(Результат, КонтекстОтправкиПолучения) Экспорт
	Если Результат = Неопределено Тогда
		ВыполнитьОбработкуОповещения(КонтекстОтправкиПолучения.ОповещениеОЗавершении, 
					ИнтерфейсДокументовЭДОКлиентСервер.НовыйИтогВыполненияДействийПоЭДО());
		Возврат;
	КонецЕсли;
	КонтекстОтправкиПолучения.ПараметрыОтправкиПолучения.ПараметрыВыполненияДействий.ОбъектыДействий.ИдентификаторыОрганизаций =
		Результат.УчетныеЗаписиОбмена;
	НачатьОтправкуПолучениеДокументовЭДО(КонтекстОтправкиПолучения);
КонецПроцедуры
...
// Параметры:
//  РезультатВключения - см. АвтоматическоеПолучениеЭДОКлиент.ВключитьПослеОтправкиПолученияДокументовЭДОЗавершение
//  КонтекстОтправкиПолучения - см. НовыйКонтекстОтправкиПолученияДокументовЭДО
Процедура ЗавершитьОтправкуПолучениеПослеВключенияАвтоматическогоПолученияЭДО(РезультатВключения, КонтекстОтправкиПолучения)  Экспорт
	
	Если ЗначениеЗаполнено(РезультатВключения) Тогда
		ОбработкаНеисправностейБЭДКлиентСервер.ДополнитьКонтекстДиагностики(
			КонтекстОтправкиПолучения.Результат.КонтекстДиагностики,
			РезультатВключения.КонтекстДиагностики);
	КонецЕсли;
	
	ПослеОтправкиПолученияДокументовЭДО(
		КонтекстОтправкиПолучения.Результат, КонтекстОтправкиПолучения.ОповещениеОЗавершении);
...
// Параметры:
//  Контекст - См. НовыйКонтекстОтправкиПолученияДокументовЭДО
Процедура ОтправитьПолучитьДокументыЭДОСУчетомПринятияУсловийСервиса(Контекст)
	
	Если ЗначениеЗаполнено(Контекст.ПараметрыОтправкиПолучения.УчетныеЗаписиЭДОБезПринятыхУсловийСервиса) Тогда
		...
		ПараметрыФормы = Новый Структура("УчетныеЗаписиЭДО",
			Контекст.ПараметрыОтправкиПолучения.УчетныеЗаписиЭДОБезПринятыхУсловийСервиса);
	...
	ПодготовитьОтборУчетныхЗаписейДляОтправкиПолученияДокументовЭДО(Контекст);

КонецПроцедуры

Процедура СохранитьНастройкуНеПоказыватьВопросОСпискеДокументовТакском()
	
	ОбщегоНазначенияКлиент.ХранилищеОбщихНастроекСохранить(
		"ПереводИзТакскомКлючНастроек",
		"НеПоказыватьВопросСпискаДокументовТакскомКЗавершению",
		Истина);
