...
Функция ДекодироватьТокенJWT(Токен) Экспорт
	...
	Если РезультатРазбора.Данные["exp"] = Неопределено Тогда
		ВозвращаемоеЗначение.ДействуетДо = Неопределено;
	Иначе
...
Функция ОбработатьОчередьСообщений(ПараметрыОбмена)
	...
	ОбновлениеИнформационнойБазы.ПроверитьОбъектОбработан(Метаданные.РегистрыСведений.ОчередьСообщенийСАТУРН.ПолноеИмя());
...
Функция СтруктураСообщенияJSON() Экспорт
	...
	СообщениеJSON.Вставить("ДальнейшееДействие",          Неопределено);
		...
		Попытка
			...
			Если ДальнейшееДействие <> Неопределено
				...
				ОбщегоНазначенияИС.УстановитьУправляемуюБлокировку(
					Метаданные.РегистрыСведений.СтатусыДокументовСАТУРН.ПолноеИмя(), ПоляБлокировки);
				
				ТекущееСостояние = РегистрыСведений.СтатусыДокументовСАТУРН.ТекущееСостояние(Документ);
