...
//
// Параметры:
//   ВидыПродукции - СправочникСсылка.ВидыПродукцииИС, Массив из СправочникСсылка.ВидыПродукцииИС - Служебный параметр. Фильтр по виду продукции.
...
Функция ОбщиеНастройкиРазбораКодаМаркировки(ТолькоСервер = Ложь, ВидыПродукции = Неопределено, ТолькоУчитываемыеВидыПродукции = Истина) Экспорт
	...
	Иначе
		УчитываемыеВидыМаркируемойПродукции = Новый Массив;
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(УчитываемыеВидыМаркируемойПродукции, КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииИСМП(Истина));
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(УчитываемыеВидыМаркируемойПродукции, КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЕГАИСМаркируемые());
	КонецЕсли;
	
	Если УчитываемыеВидыМаркируемойПродукции.Количество() = 0 Тогда
		Возврат ИнициализацияНастроекРазбораКодаМаркировки(ТолькоСервер, ТолькоУчитываемыеВидыПродукции);
	КонецЕсли;
	...
	Если ДоступныеВидыПродукции.Количество() = 0 Тогда
		НастройкиРазбораКодаМаркировки = ИнициализацияНастроекРазбораКодаМаркировки(ТолькоСервер, ТолькоУчитываемыеВидыПродукции);
		Если Не ТолькоСервер Тогда
	...
	Хеш = РазборКодаМаркировкиИССлужебныйКлиентСервер.ХешВидовПродукции(ДоступныеВидыПродукции);
	
	КэшНастроек = ПараметрыСеанса.НастройкиРазбораКодаМаркировкиИС;
	Если КэшНастроек.ПоВсемВидамПродукции_Хеш = Хеш Тогда
		Если ТолькоСервер Тогда
			АдресКэшаПоВидамПродукции = КэшНастроек.ПоВсемВидамПродукции_АдресДляСервера;
		Иначе
			АдресКэшаПоВидамПродукции = КэшНастроек.ПоВсемВидамПродукции_АдресДляКлиента;
		КонецЕсли;
	ИначеЕсли КэшНастроек.ПоУчитываемымВидамПродукции_Хеш = Хеш Тогда
		Если ТолькоСервер Тогда
			АдресКэшаПоВидамПродукции = КэшНастроек.ПоУчитываемымВидамПродукции_АдресДляСервера;
		Иначе
			АдресКэшаПоВидамПродукции = КэшНастроек.ПоУчитываемымВидамПродукции_АдресДляКлиента;
		КонецЕсли;
	Иначе
		АдресКэшаПоВидамПродукции = Неопределено;
	КонецЕсли;
	Если АдресКэшаПоВидамПродукции <> Неопределено
		И ЭтоАдресВременногоХранилища(АдресКэшаПоВидамПродукции) Тогда
		ДанныеКэшаПоВидамПродукции = ПолучитьИзВременногоХранилища(АдресКэшаПоВидамПродукции);
		Если ДанныеКэшаПоВидамПродукции <> Неопределено Тогда
			Если ТолькоСервер Тогда
				Для Каждого ИмяОбщегоМодуля Из ДанныеКэшаПоВидамПродукции.ИменаОбщихМодулей Цикл
					ОбщийМодуль = ДанныеКэшаПоВидамПродукции.ОбщиеМодули[ИмяОбщегоМодуля];
					Если ТипЗнч(ОбщийМодуль) <> Тип("ОбщийМодуль") Тогда
						ДанныеКэшаПоВидамПродукции.ОбщиеМодули.Вставить(ИмяОбщегоМодуля,
							ОбщегоНазначения.ОбщийМодуль(ИмяОбщегоМодуля));
					КонецЕсли;
				КонецЦикла;
				Если ДанныеКэшаПоВидамПродукции.ДополнительныеПараметры.Свойство("ЕГАИС")
					И ДанныеКэшаПоВидамПродукции.ДополнительныеПараметры.ЕГАИС.Свойство("ПроверкаШтрихкодаАкцизнойМарки") Тогда
					ДанныеОбщегоМодуляЕГАИС = ДанныеКэшаПоВидамПродукции.ДополнительныеПараметры.ЕГАИС.ПроверкаШтрихкодаАкцизнойМарки;
					Если ТипЗнч(ДанныеОбщегоМодуляЕГАИС.ОбщийМодульЕГАИС) <> Тип("ОбщийМодуль") Тогда
						ДанныеОбщегоМодуляЕГАИС.ОбщийМодульЕГАИС = ОбщегоНазначения.ОбщийМодуль(ДанныеОбщегоМодуляЕГАИС.ИмяОбщегоМодуляЕГАИС);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			Возврат ДанныеКэшаПоВидамПродукции;
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого ВидПродукции Из НастройкиРазбораКодаМаркировки.ДоступныеВидыПродукции Цикл
		...
		ЭтоПродукцияЕГАИС = КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(ВидПродукции);
		
		Если ЭтоПродукцияЕГАИС Тогда
		...
		Если ЭтоПродукцияЕГАИС Тогда
			МодулиВыбраннойПодсистемы.Настройка.ОбщийМодуль.ДополнитьОписанияКодовМаркировки(
				ДанныеОписанийКодаМаркировки,
				ВидПродукции,
				МодулиВыбраннойПодсистемы.Разбор);
		КонецЕсли;
	...
	Если МодулиПодсистемыИСМП <> Неопределено Тогда
		МодулиПодсистемыИСМП.Настройка.ОбщийМодуль.ДополнитьОписанияКодовМаркировки(
			ДанныеОписанийКодаМаркировки,
			НастройкиРазбораКодаМаркировки,
			МодулиПодсистемыИСМП);
	...
	Если МодулиПодсистемыИСМП <> Неопределено Тогда
		МодулиПодсистемыИСМП.Настройка.ОбщийМодуль.ДополнитьВспомогательнымиНастройкиРазбораКодаМаркировки(
			НастройкиРазбораКодаМаркировки,
			МодулиПодсистемыИСМП);
	КонецЕсли;
	...
	Для Каждого ШаблонКМ Из НастройкиРазбораКодаМаркировки.ШаблоныКодовМаркировки Цикл
		...
		Если ТолькоСервер Тогда
			ДанныеШаблона.СтрокаТаблицыШаблоныКодовМаркировки = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(ШаблонКМ);
		Иначе
			ДанныеШаблона.СтрокаТаблицыШаблоныКодовМаркировки = ШаблонКМ;
		КонецЕсли;
	КонецЦикла;
	
	НастройкиРазбораКодаМаркировки.Хеш = Хеш;
	
	Если ВидыПродукции = Неопределено Тогда
		
		ТекущийКэшНастроек = ИнициализироватьКэшНастроекРазбораКодаМаркировки();
		
		Если КэшНастроек <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(ТекущийКэшНастроек, КэшНастроек);
		КонецЕсли;
		
		Если ТолькоУчитываемыеВидыПродукции Тогда
			ТекущийКэшНастроек.ПоУчитываемымВидамПродукции_Хеш = Хеш;
			Если ТолькоСервер Тогда
				АдресИмя = "ПоУчитываемымВидамПродукции_АдресДляСервера";
			Иначе
				АдресИмя = "ПоУчитываемымВидамПродукции_АдресДляКлиента";
			КонецЕсли;
		Иначе
			ТекущийКэшНастроек.ПоВсемВидамПродукции_Хеш = Хеш;
			Если ТолькоСервер Тогда
				АдресИмя = "ПоВсемВидамПродукции_АдресДляСервера";
			Иначе
				АдресИмя = "ПоВсемВидамПродукции_АдресДляКлиента";
			КонецЕсли;
		КонецЕсли;
		
		Если ЭтоАдресВременногоХранилища(ТекущийКэшНастроек[АдресИмя]) Тогда
			Адрес = ПоместитьВоВременноеХранилище(НастройкиРазбораКодаМаркировки, ТекущийКэшНастроек[АдресИмя]);
		Иначе
			Адрес = ПоместитьВоВременноеХранилище(НастройкиРазбораКодаМаркировки, Новый УникальныйИдентификатор);
		КонецЕсли;
		
		ТекущийКэшНастроек[АдресИмя] = Адрес;
		
		ПараметрыСеанса.НастройкиРазбораКодаМаркировкиИС = Новый ФиксированнаяСтруктура(ТекущийКэшНастроек);
...
//     * Хеш - Строка.
//
Функция ИнициализацияНастроекРазбораКодаМаркировки(ТолькоСервер, ТолькоУчитываемыеВидыПродукции)
	...
	ШаблоныКодовМаркировкиПоВидамПродукции.Колонки.Добавить("ВидПродукции", Новый ОписаниеТипов("СправочникСсылка.ВидыПродукцииИС"));
	
	Настройки = Новый Структура;
	Настройки.Вставить("ДоступныеВидыПродукции",                  Новый Массив);
	Настройки.Вставить("Алфавит",                                 РазборКодаМаркировкиИССлужебныйКлиентСервер.ДопустимыеСимволыВКодеМаркировки());
	...
	Настройки.Вставить("ТолькоСервер",                            ТолькоСервер);
	Настройки.Вставить("ТолькоУчитываемыеВидыПродукции",          ТолькоУчитываемыеВидыПродукции);
	Настройки.Вставить("ИдентификаторыПримененияСРазделителемGS", РазборКодаМаркировкиИССлужебныйКлиентСервер.ИдентификаторыПримененияСРазделителемGS());
...
Функция ИнициализироватьКэшНастроекРазбораКодаМаркировки() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ПоВсемВидамПродукции_Хеш", "");
	Результат.Вставить("ПоВсемВидамПродукции_АдресДляСервера", "");
	Результат.Вставить("ПоВсемВидамПродукции_АдресДляКлиента", "");
	Результат.Вставить("ПоУчитываемымВидамПродукции_Хеш", "");
	Результат.Вставить("ПоУчитываемымВидамПродукции_АдресДляСервера", "");
	Результат.Вставить("ПоУчитываемымВидамПродукции_АдресДляКлиента", "");
...
Функция ОписаниеЭлементовШаблонаКодаМаркировкиВСтруктуру(НастройкиРазбораКодаМаркировки, ОписаниеЭлементовШаблонаКодаМаркировки) Экспорт
