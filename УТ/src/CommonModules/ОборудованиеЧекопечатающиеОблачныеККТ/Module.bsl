...
Процедура ЧекОблачногоЗаданияОбработан(ДокументОперации, Касса, ПараметрыОперации, ОбъектОбработан) Экспорт
	...
	Перем ИдентификаторЧекаВОчереди;
	СтандартнаяОбработка = Истина;
	
	ОборудованиеЧекопечатающиеОблачныеККТПереопределяемый.ПриПолученииИдентификатораЧека(ДокументОперации, ИдентификаторЧекаВОчереди, СтандартнаяОбработка);
	...
	Иначе
		...
		ПараметрыОперацииЧека.ДокументОснование = ДокументОперации;
		РаспределеннаяФискализацияПереопределяемый.ЗаполнитьДанныеФискализации(ПараметрыОперацииЧека, ДокументОперации);
	КонецЕсли;
	...
	Если РезультатПробития Тогда
		...
		ПараметрыФискализацииXML = ОборудованиеЧекопечатающиеУстройства.ПараметрыФискализацииЧека();
		ВерсияФФДККТ11 = "1.2"; // Признак ФФД 1.1
		
		ПараметрыФискализацииXML.РегистрационныйНомерККТ = "";
		ПараметрыФискализацииXML.ЗаводскойНомерФН  = "";
		ПараметрыФискализацииXML.ШиринаСтроки      = 0;
		ПараметрыФискализацииXML.РевизияИнтерфейса = 4004;
		ПараметрыФискализацииXML.ДокументОснование = ПараметрыОперацииЧека.ДокументОснование;
		ПараметрыФискализацииXML.ИдентификаторФискальнойЗаписи = ПараметрыОперацииЧека.ИдентификаторФискальнойЗаписи;
		ПараметрыФискализацииXML.Организация       = ПараметрыОперацииЧека.Организация;
		ПараметрыФискализацииXML.ТорговыйОбъект    = ПараметрыОперацииЧека.ТорговыйОбъект;
		ПараметрыФискализацииXML.ДополнительныйРеквизит = ПараметрыОперацииЧека.ДополнительныйРеквизит;  
		ПараметрыФискализацииXML.СубъектПерсональныхДанных = ПараметрыОперацииЧека.СубъектПерсональныхДанных;
		ПараметрыФискализацииXML.ЕстьПерсональныеДанные    = ПараметрыОперацииЧека.ЕстьПерсональныеДанные;
		ПараметрыФискализацииXML.Кассир       = ПараметрыОперацииЧека.Кассир;
		ПараметрыФискализацииXML.КассирИНН    = ПараметрыОперацииЧека.КассирИНН;
		ПараметрыФискализацииXML.Электронно   = ПараметрыОперацииЧека.Электронно;
		ПараметрыФискализацииXML.Отправляет1СSMS   = ПараметрыОперацииЧека.Отправляет1СSMS;
		ПараметрыФискализацииXML.ВерсияФФДККТ11    = ПараметрыОперацииЧека;
		ПараметрыФискализацииXML.ЕдиныйЧек         = ПараметрыОперацииЧека.ЕдиныйЧек;
		ПараметрыФискализацииXML.ТипДокумента = Перечисления.ТипыФискальныхДокументовККТ.КассовыйЧек;
		
		ОборудованиеЧекопечатающиеУстройства.СформироватьXMLПакетДляФискализацияЧека(ПараметрыОперацииЧека, ПараметрыФискализацииXML);
		...
		РезультатВыполнения.Вставить("ДанныеXML", ПараметрыФискализацииXML.ДанныеЧекаXML);
...
Функция СтатусПробитияЧека(ДокументОперации) Экспорт
	...
	Возврат МодульОблачныхКасс.СтатусПробитияЧека(ДокументОперации, 30);
	
КонецФункции

// Создает новое оборудование облачной ККТ
// 
// Параметры:
//  Организация - ОпределяемыйТип.ОрганизацияБПО - организация подключаемого оборудования.
//  Наименование - Строка - наименование облачной ККТ.
//
// Возвращаемое значение:
//  СправочникСсылка.ПодключаемоеОборудование.
//
Функция НовоеОборудованиеОблачнойККТ(Организация, Наименование) Экспорт
	...
	Попытка
		...
		НовоеОборудование.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ОблачнаяККТ;
		НовоеОборудование.ТипПодключения = Перечисления.ТипыПодключенияОборудования.ОбщийДоступ;
		НовоеОборудование.ДрайверОборудования = ПредопределенноеЗначение("Справочник.ДрайверыОборудования.ОбработчикОблачныхККТ");
		НовоеОборудование.УстройствоИспользуется = Истина;
		НовоеОборудование.СпособФорматноЛогическогоКонтроля = Перечисления.СпособыФорматноЛогическогоКонтроля.РазделятьСтроки;
		НовоеОборудование.ДопустимоеРасхождениеФорматноЛогическогоКонтроля = 0.01;
...
// Выполняет преобразование данных чека к формату облачной ККТ.
// 
// Параметры:
//  ДанныеЧека - см.ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПараметрыОперацииФискализацииЧека - структура данных чека.
//
// Возвращаемое значение:
//  Структура - структура данных в формате облачного сервиса.
//
Функция ДанныеВФорматеОблачныхККТ(ПараметрыДокумента) Экспорт
	...
	Если Не ПустаяСтрока(ПараметрыДокумента.ОтправительEmail) Тогда
		ПараметрыШапкиЧека.Вставить("senderEmail", ПараметрыДокумента.ОтправительEmail);
	КонецЕсли;
	
	Если НЕ ПустаяСтрока(ПараметрыДокумента.АдресРасчетов) Тогда   
		ПараметрыШапкиЧека.Вставить("saleAddress", ПараметрыДокумента.АдресРасчетов);
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ПараметрыДокумента.ПокупательEmail) Тогда  
		ПараметрыШапкиЧека.Вставить("customerEmail", СокрЛП(ПараметрыДокумента.ПокупательEmail));
	КонецЕсли;
	
	Если ПараметрыДокумента.Интернет Тогда
		ПараметрыШапкиЧека.Вставить("internetPayment", ПараметрыДокумента.Интернет);
	КонецЕсли;
	
	Если НЕ ПустаяСтрока(ПараметрыДокумента.ЧасоваяЗона) И НЕ ПараметрыДокумента.ЧасоваяЗона = 0 Тогда
		ПараметрыШапкиЧека.Вставить("timeZone", ПараметрыДокумента.ЧасоваяЗона);
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ПараметрыДокумента.ПокупательНомер) Тогда  
		...
		ПараметрыШапкиЧека.Вставить("customerPhone", ПокупательНомер);
	КонецЕсли;
	
	Если НЕ ПустаяСтрока(ПараметрыДокумента.НомерАвтомата) Тогда
		ПараметрыШапкиЧека.Вставить("automatNumber", ПараметрыДокумента.НомерАвтомата);
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ПараметрыДокумента.ПризнакАгента) Тогда
		...
		Если ЗначениеЗаполнено(ПараметрыАгента) Тогда
	...
	Если ПараметрыДокумента.ПозицииЧека <> Неопределено Тогда
		Для Каждого ПозицияЧека Из ПараметрыДокумента.ПозицииЧека Цикл
			Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
				...
				КонецЕсли;
				ПараметрыТовараЧека.name = Лев(ПозицияЧека.Наименование, 128); // Наименование товара.
				...
				Если ПозицияЧека.Свойство("ПризнакАгентаПоПредметуРасчета") И НЕ ПустаяСтрока(ПозицияЧека.ПризнакАгентаПоПредметуРасчета) Тогда
					ПризнакАгентаПоПредметуРасчета = ОборудованиеЧекопечатающиеУстройстваРФКлиентСервер.КодПризнакаАгента(ПозицияЧека.ПризнакАгентаПоПредметуРасчета);
					ПараметрыТовараЧека.Вставить("calculationAgent", ПризнакАгентаПоПредметуРасчета);
	...
	ИндексОплатыНаличными = Неопределено;
	...
	Если ТаблицаОплат <> Неопределено Тогда
		...
		Для ИндексОплаты = 0 По ТаблицаОплат.Количество() - 1 Цикл
			...
			Иначе
				...
				ИндексОплатыНаличными = ИндексОплаты;
			КонецЕсли;
		КонецЦикла;
		СуммаОплатЧека = СуммаЭлектронно + СуммаПредоплата + СуммаПостоплата + СуммаПредоставление + СуммаНаличные;
	...
	Если СуммаОплатЧека > СуммаЧека Тогда
		
		РазницаСумм = (СуммаОплатЧека - СуммаЧека);
		
		Если СуммаНаличные > РазницаСумм Тогда
			СуммаНаличные = СуммаНаличные - РазницаСумм;
			Если ИндексОплатыНаличными <> Неопределено Тогда
				ТаблицаОплат[ИндексОплатыНаличными].Сумма = СуммаНаличные;
			КонецЕсли;
		КонецЕсли;
		
		ПараметрыПробития.payments.cash = СуммаНаличные;
		
	КонецЕсли;
	
	Возврат ПараметрыПробития;
...
Функция ПробитьЧек(ДокументОперации, ИдентификаторОборудования, ПараметрыДокумента) Экспорт
	...
	Если РезультатФискализацииЧека.Свойство("РезультатПроверкиФЛК") Тогда
		
		РезультатВыполнения = МенеджерОборудованияКлиентСервер.ПараметрыВыполненияОперацииНаОборудовании(РезультатФискализацииЧека, РезультатФискализацииЧека.ТекстОшибки, 
			ИдентификаторОборудования);
		
		#Если НЕ МобильноеПриложениеСервер Тогда
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка форматно логического контроля при пробитии чека облачной кассы.'",
			ОбщегоНазначенияБПО.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			ДокументОперации,,
			РезультатФискализацииЧека.ТекстОшибки);
		#КонецЕсли
		
		ОбщегоНазначенияБПО.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Ошибка форматно логического контроля при пробитии чека облачной кассы. %1. Подробнее в журнале регистрации'"), 
			РезультатФискализацииЧека.ТекстОшибки));
...
Функция ФискализацияЧека(ДокументОперации, ИдентификаторОборудования, ПараметрыДокумента) Экспорт
	...
	СтандартнаяОбработка = Истина;
	ОборудованиеЧекопечатающиеОблачныеККТПереопределяемый.ДополнитьДанныеДляОблачнойККТ(ПараметрыДокумента, СтандартнаяОбработка);
	
	ОписаниеОшибки = "";
	СтруктураПроверкиФЛК = Новый Структура;
	Если ОбщегоНазначенияБПО.ИспользуетсяФорматноЛогическийКонтроль() Тогда
		...
		Если НЕ МодульФорматноЛогическийКонтроль.ВыполненаПроверкаОбязательностиИПравильностиЗаполненияТэгов(ПараметрыДокумента, 
			ИдентификаторОборудования, ОписаниеОшибки) Тогда
			СтруктураПроверкиФЛК.Вставить("РезультатПроверкиФЛК", Ложь);
			СтруктураПроверкиФЛК.Вставить("ТекстОшибки", ОписаниеОшибки);
			Возврат СтруктураПроверкиФЛК;
		КонецЕсли;
		МодульФорматноЛогическийКонтроль.ПривестиДанныеКТребуемомуФормату(ПараметрыДокумента);
	КонецЕсли;
	
	ПараметрыПробития = ДанныеВФорматеОблачныхККТ(ПараметрыДокумента);
...
#Область РазрешенныеМетодыДляВызоваКакПроизвольныйКод
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПробитьЧек", Истина);
	Методы.Вставить("ПолучитьСтатусПробитияЧека", Истина);
...
Функция РезультатВыполненияОперации(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Если Результат.ДанныеОперации.СтатусОперации = "Выполнена" Тогда
			...
			ПараметрыФискализацииXML.ШиринаСтроки      = 0;
			ПараметрыФискализацииXML.РевизияИнтерфейса = 4004;
			ПараметрыФискализацииXML.ДокументОснование = ДополнительныеПараметры.ПараметрыОперации.ДокументОснование;
			ПараметрыФискализацииXML.ИдентификаторФискальнойЗаписи = ДополнительныеПараметры.ПараметрыОперации.ИдентификаторФискальнойЗаписи;
			ПараметрыФискализацииXML.Организация       = ДополнительныеПараметры.ПараметрыОперации.Организация;
			ПараметрыФискализацииXML.ТорговыйОбъект    = ДополнительныеПараметры.ПараметрыОперации.ТорговыйОбъект;
			ПараметрыФискализацииXML.ДополнительныйРеквизит = ДополнительныеПараметры.ПараметрыОперации.ДополнительныйРеквизит;  
			ПараметрыФискализацииXML.СубъектПерсональныхДанных = ДополнительныеПараметры.ПараметрыОперации.СубъектПерсональныхДанных;
			ПараметрыФискализацииXML.ЕстьПерсональныеДанные    = ДополнительныеПараметры.ПараметрыОперации.ЕстьПерсональныеДанные;
			ПараметрыФискализацииXML.Кассир       = ДополнительныеПараметры.ПараметрыОперации.Кассир;
			ПараметрыФискализацииXML.КассирИНН    = ДополнительныеПараметры.ПараметрыОперации.КассирИНН;
			ПараметрыФискализацииXML.Электронно   = ДополнительныеПараметры.ПараметрыОперации.Электронно;
			ПараметрыФискализацииXML.Отправляет1СSMS   = ДополнительныеПараметры.ПараметрыОперации.Отправляет1СSMS;
			ПараметрыФискализацииXML.ВерсияФФДККТ11    = ВерсияФФДККТ11;
			ПараметрыФискализацииXML.ЕдиныйЧек         = ДополнительныеПараметры.ПараметрыОперации.ЕдиныйЧек;
			ПараметрыФискализацииXML.ТипДокумента = Перечисления.ТипыФискальныхДокументовККТ.КассовыйЧек;
			
			ОборудованиеЧекопечатающиеУстройства.СформироватьXMLПакетДляФискализацияЧека(ДополнительныеПараметры.ПараметрыОперации, ПараметрыФискализацииXML);
...
Функция ЗаполнитьДанныеАгента(ДанныеАгента) 
	...
	Если Не ПустаяСтрока(ДанныеАгента.ПлатежныйАгент.Операция) Тогда // Операция платежного агента
		ПараметрыАгента.Вставить("agentOperation", ДанныеАгента.ПлатежныйАгент.Операция);
	КонецЕсли;
	Если Не ПустаяСтрока(ДанныеАгента.ПлатежныйАгент.Телефон) Тогда // Телефон платежного агента            
		...
		ПараметрыАгента.Вставить("agentPhone", Телефон);
	КонецЕсли;
	Если Не ПустаяСтрока(ДанныеАгента.ОператорПоПриемуПлатежей.Телефон) Тогда // Телефон оператора по приему платежей         
		...
		ПараметрыАгента.Вставить("paymentProcessorPhone", Телефон);
	КонецЕсли;
	Если Не ПустаяСтрока(ДанныеАгента.ОператорПеревода.Телефон) Тогда // Телефон оператора перевода           
		...
		ПараметрыАгента.Вставить("acquirerOperatorPhone", Телефон);
	КонецЕсли;
	Если Не ПустаяСтрока(ДанныеАгента.ОператорПеревода.Наименование) Тогда // Наименование оператора перевода
		ПараметрыАгента.Вставить("acquirerOperatorName", ДанныеАгента.ОператорПеревода.Наименование);
	КонецЕсли;
	Если Не ПустаяСтрока(ДанныеАгента.ОператорПеревода.Адрес) Тогда // Адрес оператора перевода
		ПараметрыАгента.Вставить("acquirerOperatorAddress", ДанныеАгента.ОператорПеревода.Адрес);
	КонецЕсли;
	Если Не ПустаяСтрока(ДанныеАгента.ОператорПеревода.ИНН) Тогда // ИНН оператора перевода
		ПараметрыАгента.Вставить("acquirerOperatorINN", ДанныеАгента.ОператорПеревода.ИНН);
	КонецЕсли;
	
	Если ПараметрыАгента.Количество() = 0 Тогда
...
Функция ЗаполнитьДанныеПоставщика(ДанныеПоставщика) 
	...
	Если Не ПустаяСтрока(ДанныеПоставщика.Телефон) Тогда // Телефон поставщика      
		...
		ПараметрыПоставщика.Вставить("phone", Телефон);
	КонецЕсли;
	Если Не ПустаяСтрока(ДанныеПоставщика.Наименование) Тогда // Наименование поставщика
		ПараметрыПоставщика.Вставить("name", ДанныеПоставщика.Наименование);
	КонецЕсли;
	Если Не ПустаяСтрока(ДанныеПоставщика.ИНН) Тогда // ИНН поставщика
		ПараметрыПоставщика.Вставить("INN", ДанныеПоставщика.ИНН);
	КонецЕсли;
...
Процедура ЗаписатьСведенияОПокупателе(ПараметрыШапкиЧека, ПараметрыДокумента, СведенияОПокупателе);
	...
	Если Не ПустаяСтрока(Покупатель) Тогда
		ПараметрыПокупателя.Вставить("Info", Покупатель);
	КонецЕсли;
	Если Не ПустаяСтрока(ПокупательИНН) Тогда
		ПараметрыПокупателя.Вставить("INN", ПокупательИНН);
	КонецЕсли;
	Если Не ПустаяСтрока(ПараметрыДокумента.СведенияОПокупателе.ПокупательАдрес) Тогда
		ПараметрыПокупателя.Вставить("Address", ПараметрыДокумента.СведенияОПокупателе.ПокупательАдрес);
	КонецЕсли;
	...
	Если СведенияОПокупателе <> Неопределено И ТипПерсональныхДанных <> Неопределено И ТипПерсональныхДанных = Перечисления.ТипыПерсональныхДанныхККТ.ПаспортныеДанные Тогда
		...
		Если Не ПустаяСтрока(СведенияОПокупателе.Гражданство) Тогда
			ПараметрыПокупателя.Вставить("Citizenship", СведенияОПокупателе.Гражданство);
		КонецЕсли;
		...
		Если Не ПустаяСтрока(СведенияОПокупателе.ВидДокумента) Тогда
			...
			ПараметрыПокупателя.Вставить("DocumentTypeCode", ВидДокумента);
		КонецЕсли;
		
		Если Не ПустаяСтрока(СведенияОПокупателе.ДанныеДокумента) Тогда
			ПараметрыПокупателя.Вставить("DocumentData", СведенияОПокупателе.ДанныеДокумента);
		КонецЕсли;
		...
		Если Не ПустаяСтрока(СведенияОПокупателе.ДатаРождения) Тогда
			...
			ПараметрыПокупателя.Вставить("DateOfBirth", ДатаРождения);
		КонецЕсли;
	...
	Если (НЕ ПустаяСтрока(РеквизитКодаТовара) И НЕ ПустаяСтрока(ДанныеКодаТовара.ШтрихкодBase64)) Тогда // Запись тега 1163
		Если ПустаяСтрока(Позиция.КонтрольнаяМарка) И ТипыИдентификаторов.Найти(ДанныеКодаТовара.ТипИдентификатораТовара)<> Неопределено Тогда
			...
			Иначе
				...
				НаименованиеРеквизита = СтрЗаменить(ДанныеКодаТовара.НаименованиеРеквизита,".","_");
				ПараметрыКодаТовара.Вставить(НаименованиеРеквизита, ШтрихкодBase64);
			КонецЕсли;
			ПараметрыТовараЧека.Вставить("goodCodeData", ПараметрыКодаТовара);
		КонецЕсли;
	КонецЕсли; 
	
	Если ЗначениеЗаполнено(ПараметрыКодаТовара) ИЛИ НЕ ПустаяСтрока(Позиция.КонтрольнаяМарка) Тогда
