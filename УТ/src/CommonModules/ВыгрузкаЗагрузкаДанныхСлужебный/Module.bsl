...
Функция ЗагрузитьДанныеТекущейОбластиИзАрхива(Знач ДанныеФайла, Знач ПараметрыЗагрузки) Экспорт
	...
	Если Не (ВозможноПродолжениеПроцедурыЗагрузки Или ЭтоДифференциальнаяРезервнаяКопия(ПараметрыЗагрузки)) Тогда
		...
		РаботаВМоделиСервиса.ОчиститьДанныеОбласти(Не ПропуститьВосстановлениеРасширений, Ложь);
		...
		Если Не ПропуститьВосстановлениеРасширений Тогда
			...
			Если ЗначениеЗаполнено(ДанныеРасширений) Тогда
				...
				Если РазделениеВключено Тогда
					...
					ВосстановитьРасширения1СПлюсИзМС(ДанныеРасширений);
				Иначе
...
Процедура ВосстановитьРасширения1СПлюсИзМС(Знач ДанныеРасширений) Экспорт
	Модуль1СПлюс = ПолучитьМодуль1СПлюс();
	Если Не ДанныеРасширений.Свойство("Расширения1СПлюс") Или Модуль1СПлюс = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого Расширение Из ДанныеРасширений.Расширения1СПлюс Цикл
		Попытка
			ИмяФайлаРасширения = РаботаВМоделиСервиса.ПолучитьФайлИзХранилищаМенеджераСервиса(
									Новый УникальныйИдентификатор(Расширение.PathToData));
		Исключение
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось получить файл расширения
				|%1
				|%2'"),
				,
				ТехнологияСервиса.ПодробныйТекстОшибки(ИнформацияОбОшибке()));
			ИмяСобытияЖР = НСтр("ru = 'Расширения 1С:Плюс.Ошибка хранилища МС'", ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрации(ИмяСобытияЖР, УровеньЖурналаРегистрации.Ошибка, , , ТекстОшибки);
			ВызватьИсключение ТекстОшибки;
		КонецПопытки;
		
		Расширение.PathToData = ИмяФайлаРасширения;
	КонецЦикла;
	
	Модуль1СПлюс.ВосстановитьРасширенияИзМС(ДанныеРасширений.Расширения1СПлюс);
КонецПроцедуры
...
Функция ЗапуститьПотокиВыгрузкиЗагрузкиДанных(ПараметрыПотоков, Контейнер) Экспорт
	...
	Для НомерПотока = 1 По ПараметрыПотоков.КоличествоПотоков Цикл
		...
		Задание = ФоновыеЗадания.Выполнить(
			"ВыгрузкаЗагрузкаДанныхСлужебный.ВыгрузкаЗагрузкаДанныхИнформационнойБазыВПотоке",
			ПараметрыЗадания,
		...
		РегистрыСведений.ПотокиВыгрузкиЗагрузкиДанных.ДобавитьЗапись(Задание, Контейнер, ПараметрыПотоков.ВидПотоков);
...
Процедура ОжидатьВыгрузкуЗагрузкуДанныхВПотоках(Задания, ПараметрыПроцесса) Экспорт
	...
	ИдентификаторЗадания = ?(УправляющееЗадание = Неопределено,
		ОбщегоНазначенияКлиентСервер.ПустойУникальныйИдентификатор(),
		УправляющееЗадание.УникальныйИдентификатор);
	...
	Пока Истина Цикл
		...
		Пока НомерЗадания > 0 Цикл
			...
			Если ЗначениеЗаполнено(ТекстОшибки) Тогда
				...
				РегистрыСведений.ПотокиВыгрузкиЗагрузкиДанных.УдалитьЗапись(Задание.УникальныйИдентификатор);
		...
		Если ЗначениеЗаполнено(Ошибки) Тогда
			...
			Для Каждого Задание Из Задания Цикл
				...
				РегистрыСведений.ПотокиВыгрузкиЗагрузкиДанных.УдалитьЗапись(Задание);
				
			КонецЦикла;
			
			ОбработатьСообщенияПотоков(ТаблицаСообщений, Контейнер);
			
			РегистрыСведений.ПотокиВыгрузкиЗагрузкиДанных.УдалитьЗапись(ИдентификаторЗадания);
...
Функция НовыеПараметрыПотоковВыгрузкиЗагрузкиДанных() Экспорт
	...
	ПараметрыПотока.Вставить("ВидПотоков", Перечисления.ВидыПотоковВыгрузкиЗагрузкиДанных.Дополнительный);
...
Функция ПолучитьЗаданиеПараллельнойВыгрузкиЗагрузкиДанных() Экспорт
	...
	Если Задание = Неопределено И РаботаВМоделиСервиса.РазделениеВключено() Тогда
...
// Вид потока параллельной выгрузки загрузки данных.
// 
// Параметры:
//  Контейнер - ОбработкаОбъект.ВыгрузкаЗагрузкаДанныхМенеджерКонтейнера
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.ВидыПотоковВыгрузкиЗагрузкиДанных
Функция ВидПотокаПараллельнойВыгрузкиЗагрузкиДанных(Контейнер) Экспорт
	
	Если РаботаВМоделиСервиса.РазделениеВключено() И Контейнер.КоличествоПотоков() = 1 Тогда
		Возврат Перечисления.ВидыПотоковВыгрузкиЗагрузкиДанных.Основной;
	Иначе
		Возврат Перечисления.ВидыПотоковВыгрузкиЗагрузкиДанных.Дополнительный;
	КонецЕсли;
...
//  ПараметрыВыгрузкиЗагрузки - Структура
//  ЭтоЗагрузка - Булево
Процедура ПроверитьПараметрыПараллельнойВыгрузкиЗагрузкиДанных(ПараметрыВыгрузкиЗагрузки, ЭтоЗагрузка) Экспорт
	...
	Если Не (ПараметрыВыгрузкиЗагрузки.Свойство("ВидЗадания")
		...
		ВидЗадания = ?(ЭтоЗагрузка,
			Перечисления.ВидыЗаданийВыгрузкиЗагрузкиДанных.ПрочаяЗагрузкаДанных,
			Перечисления.ВидыЗаданийВыгрузкиЗагрузкиДанных.ПрочаяВыгрузкаДанных);
			
		ПараметрыВыгрузкиЗагрузки.Вставить("ВидЗадания", ВидЗадания);
...
Функция ЗагрузитьДанныеТекущейОбластиИзАрхиваВФоне(Знач ДанныеФайла, Знач ПараметрыЗагрузки)
	...
	Задание = РасширенияКонфигурации.ВыполнитьФоновоеЗаданиеСРасширениямиБазыДанных(
		"ВыгрузкаЗагрузкаДанныхСлужебный.НачатьЗагрузкуДанныхТекущейОбластиИзАрхива",
		ПараметрыЗадания,
...
Процедура ПослеЗагрузкиДанныхИнформационнойБазы(ПараметрыЗагрузки, Контейнер, Обработчики, ПотокЗаменыСсылок)
	...
	Если РазделениеВключено Тогда
		КаталогРасширений.ИзменитьЗначениеИспользованияВПриложении(КаталогРасширений.ДоступноВИнформационнойБазе());
...
// Запустить дополнительные потоки выгрузки загрузки данных.
// 
// Параметры:
//  Задания - Массив из ФоновоеЗадание - Запущенные задания
//  ПараметрыПроцесса - Структура - Для загрузки данных см. обработку ВыгрузкаЗагрузкаДанныхМенеджерЗагрузкиДанныхИнформационнойБазы.НовыеПараметрыПроцессаЗагрузкиДанныхИнформационнойБазы
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВыгрузкаЗагрузкаДанныхИнформационнойБазыВПотоке", Истина);
	Методы.Вставить("НачатьЗагрузкуДанныхТекущейОбластиИзАрхива", Истина);
