...
//
// Параметры:
//  СтруктураПараметровДействия - см. ШтрихкодированиеНоменклатурыКлиентСервер.ПараметрыОбработкиШтрихкодов.
//  КэшированныеЗначения        - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения.
//  ФормаВладелец               - ФормаКлиентскогоПриложения - форма-владелец, инициировавшая событие.
//
	...
	Если СтруктураПараметровДействия.НеизвестныеШтрихкоды.Количество() > 0 Тогда
		...
		Если КэшированныеЗначения <> Неопределено
			И Не ((КэшированныеЗначения.ПравоРегистрацииШтрихкодовНоменклатурыДоступно = Истина
					Или (СтруктураПараметровДействия.ПереноситьВДокументУточненныеТовары
						И СтруктураПараметровДействия.ДействияСНеизвестнымиШтрихкодами <> "РегистрацияОбязательна"))
				И СтруктураПараметровДействия.ИзменятьКоличество) Тогда
			
			НеуникальныеШтрихкоды = СтруктураПараметровДействия.НеуникальныеШтрихкоды;
			
			Если СтруктураПараметровДействия.УчитыватьУпаковочныеЛисты Тогда
				ШаблонНеизвестныйШтрихкод = НСтр("ru = 'Номенклатура или упаковочный лист со штрихкодом %1 не найдены'");
			Иначе
				ШаблонНеизвестныйШтрихкод = НСтр("ru = 'Номенклатура со штрихкодом %1 не найдена'");
			КонецЕсли;
			
			ШаблонНеуникальныйШтрихкод = НСтр("ru = 'Не удалось определить номенклатуру, так как штрихкоду %1 соответствует несколько товаров'");
			
			Для Каждого ТекНеизвестныйШтрихкод Из СтруктураПараметровДействия.НеизвестныеШтрихкоды Цикл
				Штрихкод = СокрЛП(ТекНеизвестныйШтрихкод.Штрихкод);
				
				Если НеуникальныеШтрихкоды.Получить(ТекНеизвестныйШтрихкод.Штрихкод) <> Неопределено Тогда
					СтрокаСообщения = СтрШаблон(ШаблонНеуникальныйШтрихкод, Штрихкод);
				Иначе
					СтрокаСообщения = СтрШаблон(ШаблонНеизвестныйШтрихкод, Штрихкод);
				КонецЕсли;
		...
		Иначе
			ИмяФормы = ?(СтруктураПараметровДействия.ПереноситьВДокументУточненныеТовары,
						"ОбщаяФорма.УточнениеШтрихкодаНоменклатуры",
						"ОбщаяФорма.ПоискНоменклатурыПоШтрихкоду");
			
			ОценкаПроизводительностиКлиент.ЗамерВремени(ИмяФормы);
			
			ОткрытьФорму(ИмяФормы, СтруктураПараметровДействия, ФормаВладелец, Новый УникальныйИдентификатор);
		КонецЕсли;
...
//  Количество - Число - количество товара по штрихкоду.
...
Процедура ПоказатьВводШтрихкода(ОповещениеУспешногоВвода, Количество = Неопределено, Заголовок = "") Экспорт 
	...
	ОписаниеОповещения		= Новый ОписаниеОповещения("ПоказатьВводШтрихкодаЗавершение",
														ЭтотОбъект,
														ДополнительныеПараметры);
	
	ПоказатьВводЗначения(ОписаниеОповещения, "", Заголовок);
...
//
// Параметры:
//  Штрихкоды - Массив Из Структура, Структура - Данные по штрихкодам.
...
//
// Параметры:
//	СтруктураДействий - см. ПараметрыОбработкиШтрихкодов.
...
Процедура ОпределитьУсловияТиповНоменклатурОбработкиШтрихкодов(СтруктураДействий, ТипыНоменклатуры) Экспорт
	...
	Если ТипыНоменклатурыМассив.Найти(ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Товар")) <> Неопределено Тогда
		ЕстьРаботы = ТипыНоменклатурыМассив.Найти(ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Работа")) <> Неопределено;
		ЕстьУслуги = ТипыНоменклатурыМассив.Найти(ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Услуга")) <> Неопределено;
		ЕстьТара   = ТипыНоменклатурыМассив.Найти(ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.МногооборотнаяТара")) <> Неопределено;
		...
		СтруктураДействий.ИспользоватьТаруВКачествеТовара = ЕстьТара
															И СтруктураДействий.ТолькоТовары;
	Иначе
		Если ТипыНоменклатурыМассив.Найти(ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Услуга")) <> Неопределено Тогда
			СтруктураДействий.ТолькоУслуги = Истина;
		ИначеЕсли ТипыНоменклатурыМассив.Найти(ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Работа")) <> Неопределено Тогда
			СтруктураДействий.ТолькоРаботы = Истина;
		ИначеЕсли ТипыНоменклатурыМассив.Найти(ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.МногооборотнаяТара")) <> Неопределено Тогда
...
Процедура ПоказатьВводШтрихкодаЗавершение(Штрихкод, ДополнительныеПараметры) Экспорт
	...
	Если Штрихкод <> Неопределено Тогда
		Если Не ПустаяСтрока(Штрихкод) Тогда
			...
			ВыполнитьОбработкуОповещения(ОповещениеУспешногоВвода, СтруктураДанныхШтрихкода(Штрихкод, Количество));
		КонецЕсли;
	КонецЕсли;
