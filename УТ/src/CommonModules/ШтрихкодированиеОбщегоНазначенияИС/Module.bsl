...
//  ВидПродукции - СправочникСсылка.ВидыПродукцииИС - Товарная группа, по которой будут инициализированы параметры сканирования.
...
Функция ПараметрыСканирования(Контекст = Неопределено, ФормаВыбора = Неопределено, ВидПродукции = Неопределено, ПараметрыРежимаИсправленияОшибок = Неопределено) Экспорт
	...
	МассивВидПродукции = Новый Массив;
	Если ТипЗнч(ВидПродукции) = Тип("Массив") Тогда
		МассивВидПродукции = ВидПродукции;
	ИначеЕсли ЗначениеЗаполнено(ВидПродукции) Тогда
		МассивВидПродукции.Добавить(ВидПродукции);
	КонецЕсли;
	
	ЗаполнитьПараметрыСканированияДляАлкогольнойПродукции(ПараметрыСканирования, Контекст, МассивВидПродукции, ФормаВыбора);
	ЗаполнитьПараметрыСканированияДляТабачнойПродукции(ПараметрыСканирования, Контекст, МассивВидПродукции, ФормаВыбора, ПараметрыРежимаИсправленияОшибок);
	ЗаполнитьПараметрыСканированияДляПродукцииИСМП(ПараметрыСканирования, Контекст, ФормаВыбора, МассивВидПродукции, ПараметрыРежимаИсправленияОшибок);
	
	Если ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоФормаПроверкиИПодбора(Контекст) Тогда
		Если Контекст.ФормаПроверкиИПодбора.Сценарий = 0 Тогда
			ШтрихкодированиеИСКлиентСерверПереопределяемый.ПриЗаполненииПараметровСканирования(ПараметрыСканирования, Контекст.Контекст, ВидПродукции);
		Иначе
			ШтрихкодированиеИСКлиентСерверПереопределяемый.ПриЗаполненииПараметровСканирования(ПараметрыСканирования, Контекст.Контекст, ВидПродукции, Контекст.ФормаПроверкиИПодбора.Сценарий);
		КонецЕсли;
		ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗаполнитьОрганизациюПараметровСканированияПриНеобходимостиПоДаннымФормыПроверкиИПодбора(ПараметрыСканирования, Контекст.ФормаПроверкиИПодбора);
	ИначеЕсли ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоФормаСУказаниемСценария(Контекст) Тогда
		ШтрихкодированиеИСКлиентСерверПереопределяемый.ПриЗаполненииПараметровСканирования(ПараметрыСканирования, Контекст.Контекст, ВидПродукции, Контекст.Сценарий);
	Иначе
...
//   * МаркируемаяПродукция (зависит от контекста),
//   * ВидПродукции (ключ видов продукции).
...
//
Функция ТекстЗапросаСвойстваМаркируемойПродукции(ТаблицаИсточник = "ДанныеШтрихкодовУпаковок", ПараметрыСканирования = Неопределено) Экспорт
	
	Ограничения = Новый Структура("ЕГАИС, ИСМП", Истина, Истина);
	Если ПараметрыСканирования <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(Ограничения, ПараметрыСканирования);
	КонецЕсли;
	
	Если Ограничения.ЕГАИС И Ограничения.ИСМП Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ДанныеШтрихкодовУпаковок.Номенклатура КАК Номенклатура,
		|	МАКСИМУМ
		|		(Не КлючиВидовПродукцииИС.Ссылка ЕСТЬ NULL
		|			И (КлючиВидовПродукцииИС.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
		|				Или (КлючиВидовПродукцииИС.ВидПродукцииЕГАИС <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
		|					И ЕСТЬNULL(ЗначенияЭлементовПользователя.ПометкаУдаления, ЛОЖЬ) = ЛОЖЬ
		|					И Не (ЗначенияЭлементовПользователя.ЭлементКонструктора ЕСТЬ NULL
		|						И ЗначенияЭлементов.ЭлементКонструктора ЕСТЬ NULL)))
		|			И (ЗначенияЭлементовСырье.ЭлементКонструктора ЕСТЬ NULL ИЛИ ЕСТЬNULL(ЗначенияЭлементовПользователяСырье.ПометкаУдаления, ЛОЖЬ) = ИСТИНА)
		|			И ЗначенияЭлементовПользователяСырье.ЭлементКонструктора ЕСТЬ NULL) КАК МаркируемаяПродукция,
		|	МАКСИМУМ(ЕСТЬNULL(КлючиВидовПродукцииИС.Ссылка, ЗНАЧЕНИЕ(Справочник.КлючиВидовПродукцииИС.ПустаяСсылка))) КАК ВидПродукции
		...
		|		ПО ОписаниеНоменклатурыИС.Номенклатура = ДанныеШтрихкодовУпаковок.Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиВидовПродукцииИС КАК КлючиВидовПродукцииИС
		|		ПО КлючиВидовПродукцииИС.Ссылка = ОписаниеНоменклатурыИС.КлючВидовПродукции
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗначенияПользователяЭлементовКонструктораВидовПродукцииИС КАК ЗначенияЭлементовПользователя
		|		ПО (ЗначенияЭлементовПользователя.ЭлементКонструктора = ЗНАЧЕНИЕ(Справочник.ЭлементыКонструктораВидовПродукцииИС.МаркируемыеВидыПродукцииЕГАИС))
		|			И (ЗначенияЭлементовПользователя.ВидПродукции = КлючиВидовПродукцииИС.ВидПродукцииЕГАИС)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗначенияЭлементовКонструктораВидовПродукцииИС КАК ЗначенияЭлементов
		|		ПО (ЗначенияЭлементов.ЭлементКонструктора = ЗНАЧЕНИЕ(Справочник.ЭлементыКонструктораВидовПродукцииИС.МаркируемыеВидыПродукцииЕГАИС))
		|			И (ЗначенияЭлементов.ВидПродукции = КлючиВидовПродукцииИС.ВидПродукцииЕГАИС)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗначенияЭлементовКонструктораВидовПродукцииИС КАК ЗначенияЭлементовСырье
		|		ПО (ЗначенияЭлементовСырье.ЭлементКонструктора = ЗНАЧЕНИЕ(Справочник.ЭлементыКонструктораВидовПродукцииИС.Сырье))
		|			И (ЗначенияЭлементовСырье.ВидПродукции = КлючиВидовПродукцииИС.ВидПродукцииГИСМТ)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗначенияПользователяЭлементовКонструктораВидовПродукцииИС КАК ЗначенияЭлементовПользователяСырье
		|		ПО (ЗначенияЭлементовПользователяСырье.ЭлементКонструктора = ЗНАЧЕНИЕ(Справочник.ЭлементыКонструктораВидовПродукцииИС.Сырье))
		|			И (ЗначенияЭлементовПользователяСырье.ВидПродукции = КлючиВидовПродукцииИС.ВидПродукцииГИСМТ)
		|СГРУППИРОВАТЬ ПО
		|	ДанныеШтрихкодовУпаковок.Номенклатура
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура";
	ИначеЕсли Ограничения.ЕГАИС Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ДанныеШтрихкодовУпаковок.Номенклатура     КАК Номенклатура,
		|	МАКСИМУМ
		|		(Не КлючиВидовПродукцииИС.Ссылка ЕСТЬ NULL
		|			И КлючиВидовПродукцииИС.ВидПродукцииЕГАИС <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
		|			И ЕСТЬNULL(ЗначенияЭлементовПользователя.ПометкаУдаления, ЛОЖЬ) = ЛОЖЬ
		|			И Не (ЗначенияЭлементовПользователя.ЭлементКонструктора ЕСТЬ NULL
		|				И ЗначенияЭлементов.ЭлементКонструктора ЕСТЬ NULL)) КАК МаркируемаяПродукция,
		|	МАКСИМУМ(ЕСТЬNULL(КлючиВидовПродукцииИС.Ссылка, ЗНАЧЕНИЕ(Справочник.КлючиВидовПродукцииИС.ПустаяСсылка))) КАК ВидПродукции
		|ПОМЕСТИТЬ СвойстваМаркируемойПродукции
		|ИЗ
		|	ТаблицаИсточник КАК ДанныеШтрихкодовУпаковок
		...
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура";
	ИначеЕсли Ограничения.ИСМП Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ДанныеШтрихкодовУпаковок.Номенклатура КАК Номенклатура,
		|	МАКСИМУМ (Не КлючиВидовПродукцииИС.Ссылка ЕСТЬ NULL
		|		И КлючиВидовПродукцииИС.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
		|		И (ЗначенияЭлементовСырье.ЭлементКонструктора ЕСТЬ NULL ИЛИ ЕСТЬNULL(ЗначенияЭлементовПользователяСырье.ПометкаУдаления, ЛОЖЬ) = ИСТИНА)
		|		И ЗначенияЭлементовПользователяСырье.ЭлементКонструктора ЕСТЬ NULL) КАК МаркируемаяПродукция,
		|	МАКСИМУМ(ЕСТЬNULL(КлючиВидовПродукцииИС.Ссылка, ЗНАЧЕНИЕ(Справочник.КлючиВидовПродукцииИС.ПустаяСсылка))) КАК ВидПродукции
	...
	Иначе
		...
		|	ЗНАЧЕНИЕ(Справочник.КлючиВидовПродукцииИС.ПустаяСсылка) КАК ВидПродукции
		|ПОМЕСТИТЬ СвойстваМаркируемойПродукции
		|ИЗ
		|	ТаблицаИсточник КАК ДанныеШтрихкодовУпаковок
		|СГРУППИРОВАТЬ ПО
...
Процедура ЗаполнитьВидыПродукцииДанныхКодовМаркировки(ДанныеПоШтрихкодам, ПараметрыСканирования, ДанныеКодовМаркировки = Неопределено) Экспорт
	...
	ТаблицаНоменклатуры.Колонки.Добавить("ВидПродукции",   Новый ОписаниеТипов("СправочникСсылка.КлючиВидовПродукцииИС"));
	
	ТипВидыПродукцииИС = Тип("СправочникСсылка.ВидыПродукцииИС");
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаДанных.Номенклатура)
			И ((Не ЗначениеЗаполнено(СтрокаДанных.ВидПродукции)
					Или ТипЗнч(СтрокаДанных.ВидПродукции) = ТипВидыПродукцииИС
				...
				Или СтрокаДанных.МаркируемаяПродукция = Неопределено) Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(СтрокаДанных.ВидПродукции)
			И СтрокаДанных.МаркируемаяПродукция = Истина
			И СтрокаДанных.СтрокаДерева <> Неопределено
			И СтрокаДанных.СтрокаДерева.ВидПродукции <> СтрокаДанных.ВидПродукции Тогда
			
			СтрокаДанных.СтрокаДерева.ВидПродукции = СтрокаДанных.ВидПродукции;
			
			Если ТипЗнч(СтрокаДанных.ВидПродукции) = Тип("СправочникСсылка.КлючиВидовПродукцииИС") Тогда
				ВидПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(
			...
			Если СтрокаДанных.ВидыПродукцииКодаМаркировки.Найти(ВидПродукции) = Неопределено Тогда
				СтрокаДанных.ВидыПродукцииКодаМаркировки.Добавить(ВидПродукции);
			КонецЕсли;
	...
	|"
	+
	ТекстЗапросаСвойстваМаркируемойПродукции(, ПараметрыСканирования)
	+
	"
	|;
	|/////////////////////////////////////////////
				...
				Если РезультатыРазборы.Количество() = 1 Тогда
					...
					Если НастройкиРазбора = Неопределено Тогда
						...
						ПользовательскиеПараметрыРазбораКодаМаркировки.ВалидироватьШтрихкодЛогистическойУпаковкиGS1128СОшибками = 
							Не ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоМаркировкаТоваровИСМП(ПараметрыСканирования);
					...
					Если ДанныеРазбора <> Неопределено Тогда
						...
						ЗаполнитьСтрокуДанныхКодовМаркировкиПоДаннымРазбора(СтрокаДанных, ОбщиеМодули, ПараметрыСканирования);
...
Процедура УточнитьДанныеРазбораПоВидуПродукции(СтрокаДанных, ВидПродукции, ПараметрыСканирования, ОбщиеМодули = Неопределено, ЭтоСканированиеВФормуПроверкиИПодбора = Ложь) Экспорт
	...
	Если ПримечаниеКРазборуШтрихкода <> Неопределено 
		...
		ТипКлючВидовПродукции = Тип("СправочникСсылка.КлючиВидовПродукцииИС");
		Если ТипЗнч(ВидПродукции) = ТипКлючВидовПродукции Тогда
			ВидыПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииКлюча(ВидПродукции);
		Иначе
			ВидыПродукции = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ВидПродукции);
		КонецЕсли;
		
		ВариантДанныхРазбора  = Неопределено;
		ВариантыДанныхРазбора = Неопределено;
		
		Для Каждого ДанныеРазбора Из ПримечаниеКРазборуШтрихкода.РезультатРазбора Цикл
			
			Для Каждого ВидПродукцииДляПоиска Из ВидыПродукции Цикл
				
				Если ДанныеРазбора.ВидыПродукции.Найти(ВидПродукцииДляПоиска) <> Неопределено Тогда
					
					Если ВариантДанныхРазбора = Неопределено
						И ВариантыДанныхРазбора = Неопределено Тогда
						ВариантДанныхРазбора = ДанныеРазбора;
						Продолжить;
					ИначеЕсли ВариантыДанныхРазбора = Неопределено Тогда
						
						ВариантыДанныхРазбора = Новый ТаблицаЗначений;
						ВариантыДанныхРазбора.Колонки.Добавить("ДанныеРазбора");
						ВариантыДанныхРазбора.Колонки.Добавить("Приоритет");
						
						НовыйВариант = ВариантыДанныхРазбора.Добавить();
						НовыйВариант.ДанныеРазбора = ВариантДанныхРазбора;
						НовыйВариант.Приоритет = ПриоритетВариантаРазбора(ВариантДанныхРазбора);
					...
					НовыйВариант = ВариантыДанныхРазбора.Добавить();
					НовыйВариант.ДанныеРазбора = ДанныеРазбора;
					НовыйВариант.Приоритет = ПриоритетВариантаРазбора(ДанныеРазбора);
		...
		Если ВариантыДанныхРазбора <> Неопределено
			И ВариантыДанныхРазбора.Количество() Тогда
			ВариантыДанныхРазбора.Сортировать("Приоритет Убыв");
			ВариантДанныхРазбора = ВариантыДанныхРазбора[0].ДанныеРазбора;
		КонецЕсли;
		
		Если ВариантДанныхРазбора <> Неопределено Тогда
		
			СтрокаДанных.ДанныеРазбора = ВариантДанныхРазбора;
			...
			Если ОбщегоНазначенияИС.ПодсистемаСуществует("ИСМП") Тогда
				МодульОбщегоНазначенияИСМП = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияИСМП");
	...
	Если ДопустимаТабачнаяПродукция(ПараметрыСканирования)
		...
		//@skip-check wrong-string-literal-content
		МодульШтрихкодированиеИСМП = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияИСМП");
		МодульШтрихкодированиеИСМП.УстановитьПризнакОтказаОтПроверкиКодовМаркировкиПоСтроке(СтрокаДанных, ПараметрыСканирования, ЭтоСканированиеВФормуПроверкиИПодбора);
...
Процедура ЗаполнитьСведенияПоШтрихкодамEANСУчетомВерсии(ДанныеПоШтрихкодам, ПараметрыСканирования, ДанныеКодовМаркировки = Неопределено) Экспорт
	...
	Если ПараметрыСканирования.РасширеннаяВерсияГосИС Тогда
		ЗаполнитьСведенияПоШтрихкодамПоДаннымПулаGTIN(ДанныеПоШтрихкодам, ПараметрыСканирования);
		Возврат;
	КонецЕсли;
	...
	Если ДопустимаПродукцияИСМП Тогда
		//@skip-check wrong-string-literal-content
		МодульШтрихкодированиеИСМПСлужебный = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияИСМП");
	...
	ЭтоСканированиеВФормуПроверкиИПодбора = ЭтоСканированиеВФормеПроверкиИПодбора(ПараметрыСканирования);
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(СтрокаДанных.GTIN)
				И (КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(СтрокаДанных.ВидыПродукцииКодаМаркировки)
					Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(СтрокаДанных.ВидыПродукцииКодаМаркировки)) Тогда
				Продолжить;
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(СтрокаДанных.ШтрихкодНоменклатуры)
				И УникальныеШтрихкодыEAN[СтрокаДанных.ШтрихкодНоменклатуры] = Неопределено Тогда
				ШтрихкодыEAN.Добавить(СтрокаДанных.ШтрихкодНоменклатуры);
				УникальныеШтрихкодыEAN.Вставить(СтрокаДанных.ШтрихкодНоменклатуры, Истина);
			КонецЕсли;
	...
	ДанныеПоШтрихкодамEANРезультат = ДанныеПоШтрихкодамEAN(
		ШтрихкодыEAN, ПараметрыСканирования, GTINПотребительскихУпаковок, Истина);
	...
	ИначеЕсли ЗначениеЗаполнено(ДанныеПоШтрихкодамEANРезультат.ТекстОшибки) Тогда
		ДанныеПоШтрихкодам.ТекстОшибки = ДанныеПоШтрихкодамEANРезультат.ТекстОшибки;
	...
	Если ОбщегоНазначенияИС.ЭтоРасширеннаяВерсияГосИС("ИСМП") Тогда
		МодульИнтеграцияИСМПКлиентСервер = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияИСМПКлиентСервер");
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		ЗначениеУточнения = СтрокаДанных.ШтрихкодНоменклатуры;
		...
		Иначе
			...
			Если ЭтоШтрихкодНоменклатуры 
				И ПараметрыСканирования.ДанныеВыбораПоМаркируемойПродукции <> Неопределено Тогда
				ДанныеВыбора = ПараметрыСканирования.ДанныеВыбораПоМаркируемойПродукции;
				Если ДанныеВыбора.Свойство("ЭтоШтрихкодНоменклатуры")
					И ДанныеВыбора.ЭтоШтрихкодНоменклатуры
					И GTIN = ДанныеВыбора.GTIN Тогда
					СтрокаДанных.ТребуетсяСопоставление = Ложь;
					ЗаполнитьЗначенияСвойств(СтрокаДанных, ДанныеВыбора);
				КонецЕсли;
			КонецЕсли;
		...
		НайденныеШтрихкодыEAN = НайденныеШтрихкодыEAN(ЗначениеПоиска, ЗначениеУточнения, ИнформацияПоШтрихкодамEAN);
		...
		Для Каждого СтрокаИнформации Из НайденныеШтрихкодыEAN Цикл
			...
			Если ТребуетсяСопоставление Тогда
				...
				Если Не (РезультатПоискаУникален = Неопределено И ВариантовВыбора = 0) Тогда
					...
					ИначеЕсли Не СтрокаДанных.ЭтоШтрихкодНоменклатуры
							...
							И Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(СтрокаИнформации.ВидПродукции, Истина) Тогда
						Продолжить;
					КонецЕсли;
				КонецЕсли;
			...
			Иначе
				Если СтрокаИнформации.Номенклатура <> СтрокаДанных.Номенклатура
					Или СтрокаИнформации.Характеристика <> СтрокаДанных.Характеристика Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(СтрокаДанных.ВидПродукции) Тогда
				...
				Если УточнитьПоДаннымРазбора Тогда
					...
					Если ТипЗнч(СтрокаДанных.ВидПродукции) = Тип("СправочникСсылка.КлючиВидовПродукцииИС") Тогда
						ВидПродукции = ВидыПродукцииГИСМТПоКлючам[СтрокаДанных.ВидПродукции];
						Если ВидПродукции = Неопределено Тогда
							ВидПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(
								СтрокаДанных.ВидПродукции, "ГИСМТ");
							ВидыПродукцииГИСМТПоКлючам[СтрокаДанных.ВидПродукции] = ВидПродукции;
						КонецЕсли;
					Иначе
						ВидПродукции = СтрокаДанных.ВидПродукции;
					КонецЕсли;
					...
					УточнитьДанныеРазбораПоВидуПродукции(
						СтрокаДанных, ВидПродукции, ПараметрыСканирования,, ЭтоСканированиеВФормуПроверкиИПодбора);
			...
			Если СтрокаДанных.ТребуетВзвешивания
				...
				Если СтрокаДанных.СоставКодаМаркировки.Свойство("МассаНеттоВКг")
				...
				Если СтрокаДанных.СоставКодаМаркировки.Свойство("ОбъемВЛитрах")
					И СтрокаДанных.СоставКодаМаркировки.ОбъемВЛитрах <> Неопределено Тогда
					СтрокаДанных.Количество = СтрокаДанных.СоставКодаМаркировки.ОбъемВЛитрах;
				КонецЕсли;
			...
			Если СтрокаДанных.СтрокаДерева <> Неопределено Тогда
				ЗаполнитьЗначенияСвойств(СтрокаДанных.СтрокаДерева, СтрокаДанных);
			КонецЕсли;
			
			Если ТребуетсяСопоставление Тогда
				ДанныеУточнения = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭлементУточненияНеуникальногоШтрихкода();
				ЗаполнитьЗначенияСвойств(ДанныеУточнения, СтрокаИнформации);
				ДанныеУточнения.ОбработатьБезМаркировки = СтрокаДанных.ОбработатьБезМаркировки;
		...
		Если ВидПродукцииУникален = Ложь Тогда
			...
			СтрокаДанных.ОбработатьБезМаркировки = Ложь;
...
// Возвращаемое значение:
//  Структура - Описание:
//   * ВидыПродукции - Массив Из СправочникСсылка.ВидыПродукцииИС - Массив видов продукции ИС.
...
Функция ИнициализироватьРезультатОбработкиШтрихкода(ПараметрыСканирования = Неопределено, ДанныеШтрихкода = Неопределено) Экспорт
	...
	Если ПрисутствуетАлкогольнаяПродукция(ВидыПродукции)Тогда
		...
		//@skip-check wrong-string-literal-content
		МодульШтрихкодированиеЕГАИС = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияЕГАИС");
...
Функция ИнициализацияДанныхПоШтрихкодам(ИндексироватьТаблицы = Истина) Экспорт
	...
	ДанныеПоШтрихкодам.Вставить("ПрисутствуетАлкогольнаяПродукция", Ложь);
	...
	ДанныеПоШтрихкодам.Вставить("СписокЛокальныхМодулейДляОбработки", Неопределено);
	ДанныеПоШтрихкодам.Вставить("ДанныеНастройкиТСПИоТ",              Неопределено);
	ДанныеПоШтрихкодам.Вставить("РезультатЗапросаПоEAN",              Неопределено);
...
// * ВидыПродукцииКодаМаркировки - Массив из СправочникСсылка.ВидыПродукцииИС - список видов продукции кода маркировки.
// * ВидПродукции - СправочникСсылка.ВидыПродукцииИС - Вид продукции номенклатуры.
...
Функция ИнициализацияТаблицыДанныхКодовМаркировки(ИндексироватьТаблицы = Истина) Экспорт
	...
	ДанныеКодовМаркировки.Колонки.Добавить("ПолныйКодМаркировки",       Новый ОписаниеТипов("Строка"));
	ДанныеКодовМаркировки.Колонки.Добавить("Штрихкод",                  Новый ОписаниеТипов("Строка"));
	ДанныеКодовМаркировки.Колонки.Добавить("ШтрихкодНоменклатуры",      Новый ОписаниеТипов("Строка"));
	...
	ДанныеКодовМаркировки.Колонки.Добавить("Количество",                        Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(18,8)));
	...
	ДанныеКодовМаркировки.Колонки.Добавить("КоличествоГрупповыхУпаковок",       Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,3)));
	...
	ДанныеКодовМаркировки.Колонки.Добавить("ВидПродукции",                  Новый ОписаниеТипов(ТипыВидовПродукции));
	...
	ДанныеКодовМаркировки.Колонки.Добавить("ПрисутствуетАлкогольнаяПродукция", Новый ОписаниеТипов("Булево"));
	...
	ДанныеКодовМаркировки.Колонки.Добавить("РазрешительныйРежимИдентификаторЭкземпляраПО", Новый ОписаниеТипов("Строка"));
	ДанныеКодовМаркировки.Колонки.Добавить("РазрешительныйРежимВерсияБазы",                Новый ОписаниеТипов("Строка"));
	...
	ДанныеКодовМаркировки.Колонки.Добавить("СписокОрганизацийБлокировкиКМ",                Неопределено);
...
//   * ВидыПродукции - Массив Из СправочникСсылка.ВидыПродукцииИС - Коллекция элементов перечисления видов продукции 
...
Функция ИнициализироватьТаблицуУточненияКоэффициентовУпаковок() Экспорт
	...
	УточнениеКоэффициентовУпаковок.Колонки.Добавить("ВидПродукции",                      Новый ОписаниеТипов(ТипыВидовПродукции));
...
Функция НоваяСтруктураОбработкиШтрихкода(КодМаркировки = Неопределено, ВидПродукции = Неопределено, ПользовательскиеПараметрыРазбораКодаМаркировки = Неопределено, Настройки = Неопределено, ДанныеРазбора = Неопределено) Экспорт
	
	СтруктураЗначения = ИнициализироватьНовуюСтруктуруОбработкиШтрихкода();
	
	Если ЗначениеЗаполнено(ВидПродукции) Тогда
		Если ТипЗнч(ВидПродукции) = Тип("Массив")
			И ВидПродукции.Количество() = 1 Тогда
			СтруктураЗначения.ВидПродукции = ВидПродукции[0];
		ИначеЕсли ТипЗнч(ВидПродукции) = Тип("СправочникСсылка.ВидыПродукцииИС") Тогда
	...
	Если ЗначениеЗаполнено(КодМаркировки) Тогда
		...
		Если ДанныеРазбора.ВидыПродукции.Количество() = 1 Тогда
			СтруктураЗначения.ВидПродукции = ДанныеРазбора.ВидыПродукции[0];
		КонецЕсли;
...
Функция ИменаКолонокДляЗаполнения() Экспорт
	...
	КолонкиДанныеСервиса.Вставить("ВидПродукции",              Истина);
	КолонкиДанныеСервиса.Вставить("ВидУпаковки",               Истина);
	
	КолонкиИББезСервиса = Новый Соответствие;
	КолонкиИББезСервиса.Вставить("ПредставлениеНоменклатуры", Истина);
	КолонкиИББезСервиса.Вставить("ВидПродукции",              Истина);
	КолонкиИББезСервиса.Вставить("ВидУпаковки",               Истина);
	
	КолонкиУточнениеПоДаннымСервиса = Новый Соответствие;
	КолонкиУточнениеПоДаннымСервиса.Вставить("ПредставлениеНоменклатуры", Истина);
	КолонкиУточнениеПоДаннымСервиса.Вставить("ВидПродукции",              Истина);
	КолонкиУточнениеПоДаннымСервиса.Вставить("ВидУпаковки",               Истина);
	
	КолонкиШтрихкодыУпаковок = ОбщегоНазначения.СкопироватьРекурсивно(КолонкиНоменклатура);
	КолонкиШтрихкодыУпаковок.Вставить("ШтрихкодУпаковки",      Истина);
	...
	ВозвращаемоеЗначение.Вставить("ШтрихкодыУпаковок", КолонкиШтрихкодыУпаковок);
	ВозвращаемоеЗначение.Вставить("КолонкиУточнениеПоДаннымСервиса", КолонкиУточнениеПоДаннымСервиса);
	ВозвращаемоеЗначение.Вставить("КолонкиИББезСервиса",             КолонкиИББезСервиса);
...
//
// Параметры:
//  ВидыПродукции - Массив Из СправочникСсылка.ВидыПродукцииИС, СправочникСсылка.КлючиВидовПродукцииИС - Коллекция видов продукции ИС
...
Функция ВидыПродукцииПоддерживаются(ВидыПродукции, ПараметрыСканирования) Экспорт
	...
	Для Каждого ВидПродукции Из ВидыПродукции Цикл
		...
		Если ТипЗнч(ВидПродукции) = ТипКлючиВидовПродукцииИС Тогда
			ВидПродукцииГИСМТ = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(ВидПродукции, "ГИСМТ");
			Если ЗначениеЗаполнено(ВидПродукцииГИСМТ) Тогда
				Если ПараметрыСканирования.ДопустимыеВидыПродукции.Найти(ВидПродукцииГИСМТ) = Неопределено Тогда
					Возврат Ложь;
				КонецЕсли;
			Иначе
				ВидыПродукцииКлюча = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииКлюча(ВидПродукции);
				Для Каждого ВидПродукцииКлюча Из ВидыПродукцииКлюча Цикл
					Если ПараметрыСканирования.ДопустимыеВидыПродукции.Найти(ВидПродукцииКлюча) <> Неопределено Тогда
						Возврат Истина;
					КонецЕсли;
				КонецЦикла;
				Возврат Ложь;
		...
		ИначеЕсли ТипЗнч(ВидПродукции) = ТипВидыПродукцииИС Тогда
...
//
// Параметры:
//  ВидыПродукции - Массив Из СправочникСсылка.ВидыПродукцииИС - Коллекция видов продукции ИС.
// Возвращаемое значение:
//  Булево - Истина, если найдена алкогольная продукция.
Функция ПрисутствуетАлкогольнаяПродукция(ВидыПродукции) Экспорт
	
	Для Каждого ВидПродукции Из ВидыПродукции Цикл
		Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияЕГАИС(ВидПродукции) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
//
Функция ПрисутствуетПродукцияИСМП(ВидыПродукции, ВключаяТабачнуюПродукцию = Ложь) Экспорт
...
Функция ПрисутствуетПродукцияИзНатуральногоМеха(ВидыПродукции) Экспорт
	...
	Для Каждого ВидПродукции Из КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииМЕХ() Цикл
...
Функция РезультатОбработкиШтрихкодаПриОшибках(ДанныеШтрихкодов, ДанныеШтрихкода, ПараметрыСканирования, ФормаУникальныйИдентификатор) Экспорт
	...
	Иначе
		...
		ЕстьПродукцияПодконтрольнаяЕГАИС = Ложь;
		Для Каждого ВидПродукции Из КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииИСМП() Цикл
			Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииИСМПЕГАИС(ВидПродукции)
				И ДанныеШтрихкода.ВидыПродукцииКодаМаркировки.Найти(ВидПродукции) <> Неопределено Тогда
				ЕстьПродукцияПодконтрольнаяЕГАИС = Истина;
			КонецЕсли;
		КонецЦикла;
		
		Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияЕГАИС(ДанныеШтрихкода.ВидыПродукцииКодаМаркировки)
			Или (Не ДопустимаПродукцияИСМП(ПараметрыСканирования, Истина)
				И ЕстьПродукцияПодконтрольнаяЕГАИС) Тогда
	...
	РезультатДобавления.ОшибкаДопустимостиВидовПродукции = ДанныеШтрихкодов.ОшибкаДопустимостиВидовПродукции;
...
Функция ОбщиеМодулиПодсистем() Экспорт
	...
	//@skip-check wrong-string-literal-content
	ОбщиеМодули.Вставить("ШтрихкодированиеМОТП", ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияМОТП"));
	//@skip-check wrong-string-literal-content
	ОбщиеМодули.Вставить("ШтрихкодированиеИСМП", ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияИСМП"));
...
Процедура НормализоватьИРазобратьНаСоставляющиеКодыМаркировки(ДанныеПоШтрихкодам, ПараметрыСканирования, КешДанныхРазбора = Неопределено, ДанныеКодовМаркировки = Неопределено) Экспорт
	...
	Если ДопустимыСтрокиДанныхБезВидаПродукции Тогда
		НастройкиРазбораКодаМаркировки = РазборКодаМаркировкиИССлужебныйПовтИсп.НастройкиРазбораКодаМаркировки(, Ложь);
		ВидыПродукции                  = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииИСМП(Истина);
	Иначе
		НастройкиРазбораКодаМаркировки = РазборКодаМаркировкиИССлужебныйПовтИсп.НастройкиРазбораКодаМаркировки();
		ВидыПродукции                  = ПараметрыСканирования.ДопустимыеВидыПродукции;
	КонецЕсли;
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Если ДанныеРазбораИПримечание = Неопределено Тогда
			...
			Если ТипЗнч(СтрокаДанных.ДополнительныеПараметры) = Тип("Структура")
				И СтрокаДанных.ДополнительныеПараметры.Свойство("ЗагруженИзЭлектронногоДокумента") Тогда
				
				Если ДанныеРазбора <> Неопределено
					И ДанныеРазбора.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC
					И СтрокаДанных.ДополнительныеПараметры.ЗагруженИзЭлектронногоДокумента Тогда
					Если СтрДлина(СтрокаДанных.Штрихкод) = 18 Тогда
						ДанныеРазбора.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC18;
					Иначе
						ДанныеРазбора.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC20;
					КонецЕсли;
				КонецЕсли;
				
				Если СтрокаДанных.ДополнительныеПараметры.Количество() = 1 Тогда
					СтрокаДанных.ДополнительныеПараметры = Неопределено;
				КонецЕсли;
		...
		Если ДанныеРазбора <> Неопределено Тогда
			...
			// Восстанавливать структуру кода маркировки
			Если Не ПроверятьСтруктуруКодовМаркировки
				И ПодсистемаИСМПСуществует Тогда
				
				МодульШтрихкодированиеОбщегоНазначенияИСМП.ПроверитьПолныйКодМаркировкиПоДаннымРазбора(
...
Процедура ИндексироватьКолонкиТаблицыДанныхКодовМаркировки(ДанныеКодовМаркировки) Экспорт
	...
	ДанныеКодовМаркировки.Индексы.Добавить("ШтрихкодBase64");
...
// 	СоздаватьНоменклатуру - Булево - создавать номенклатуру
// Возвращаемое значение:
//  Структура:
//  * ТекстОшибки - Строка
//  * ТребуетсяОбновлениеКлючаСессии - Булево
//  * ДанныеПоШтрихкодамEAN - ТаблицаЗначений:
//   ** Номенклатура - ОпределяемыйТип.Номенклатура - Номенклатура.
//   ** Характеристика - ОпределяемыйТип.ХарактеристикаНоменклатуры - Характеристика.
//   ** Серия - ОпределяемыйТип.СерияНоменклатуры - Серия.
//   ** Упаковка - ОпределяемыйТип.Упаковка - Упаковка.
...
//   ** ВидПродукции - СправочникСсылка.КлючиВидовПродукцииИС - Вид продукции ИС.
...
Функция ДанныеПоШтрихкодамEAN(ШтрихкодыEAN, ПараметрыСканирования = Неопределено, GTINПотребительскихУпаковок = Неопределено, СоздаватьНоменклатуру = Ложь) Экспорт
	
	ДанныеПоШтрихкодамEAN = Новый ТаблицаЗначений;
	
	ВозвращаемоеЗначение = Новый Структура;
	ВозвращаемоеЗначение.Вставить("ДанныеПоШтрихкодамEAN",          ДанныеПоШтрихкодамEAN);
	...
	ДанныеПоШтрихкодамEAN.Колонки.Добавить("ВидПродукции",                Новый ОписаниеТипов("СправочникСсылка.КлючиВидовПродукцииИС"));
	ДанныеПоШтрихкодамEAN.Колонки.Добавить("МаркируемаяПродукция",        Новый ОписаниеТипов("Булево"));
	ДанныеПоШтрихкодамEAN.Колонки.Добавить("Количество",                  Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(18, 8)));
	...
	ДанныеПоШтрихкодамEAN.Колонки.Добавить("ОсновнойШтрихкодEAN",         Новый ОписаниеТипов("Строка"));
	ДанныеПоШтрихкодамEAN.Колонки.Добавить("ДанныеШтрихкода");
	ДанныеПоШтрихкодамEAN.Индексы.Добавить("ШтрихкодEAN");
	ДанныеПоШтрихкодамEAN.Индексы.Добавить("ОсновнойШтрихкодEAN");
	
	Если ШтрихкодыEAN.Количество() = 0 Тогда
		Возврат ВозвращаемоеЗначение;
	КонецЕсли;
	
	ВходящиеШтрихкодыEAN = Новый Массив;
	...
	Для Каждого ШтрихкодEAN Из ШтрихкодыEAN Цикл
		...
		НоваяСтрока.ОсновнойШтрихкодEAN = ШтрихкодEAN;
		
		Если ВходящиеШтрихкодыEAN.Найти(ШтрихкодEAN) = Неопределено Тогда
			ВходящиеШтрихкодыEAN.Добавить(ШтрихкодEAN);
		КонецЕсли;
		
		//GTIN
		GTIN = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.GTINПоШтрихкодуEAN(ШтрихкодEAN);
		Если GTIN <> "" И GTIN <> ШтрихкодEAN Тогда
			НоваяСтрока = ДанныеПоШтрихкодамEAN.Добавить();
	...
	ШтрихкодированиеИСПереопределяемый.ПриЗаполненииИнформацииПоШтрихкодамEAN(ДанныеПоШтрихкодамEAN, ПараметрыСканирования);
	ВозвращаемоеЗначение.ДанныеПоШтрихкодамEAN = ДанныеПоШтрихкодамEAN;
	
	Если СоздаватьНоменклатуру
		И ВходящиеШтрихкодыEAN.Количество()
		И ОбщегоНазначенияИС.ЭтоРасширеннаяВерсияГосИС()
		И ПараметрыСканирования <> Неопределено
		И ОбщегоНазначенияИС.ПодсистемаСуществует("ИСМП")
		И ДопустимаПродукцияИСМП(ПараметрыСканирования, Истина) Тогда
		
		МодульИнтеграцияИС = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияИС");
		Если МодульИнтеграцияИС.СоздаватьНоменклатуруПоGTIN() Тогда
			
			МодульШтрихкодированиеИСМП = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеИСМП");
			РезультатДополнения = МодульШтрихкодированиеИСМП.ДополнитьДанныеПоШтрихкодамEANПриАвтоматическомСозданииНоменклатуры(
				ВходящиеШтрихкодыEAN, ДанныеПоШтрихкодамEAN, ПараметрыСканирования);
		
			Если РезультатДополнения.ТребуетсяОбновлениеКлючаСессии Тогда
				ВозвращаемоеЗначение.ТребуетсяОбновлениеКлючаСессии = Истина;
			ИначеЕсли ЗначениеЗаполнено(РезультатДополнения.ТекстОшибки) Тогда
				ВозвращаемоеЗначение.ТекстОшибки = РезультатДополнения.ТекстОшибки;
			ИначеЕсли ЗначениеЗаполнено(РезультатДополнения.ТребуетсяПовторОперации) Тогда
				ВозвращаемоеЗначение.ТребуетсяПовторОперации = РезультатДополнения.ТребуетсяПовторОперации;
			КонецЕсли;
	...
	ДанныеПоШтрихкодамEAN.Колонки.Добавить("ЕмкостьПотребительскойУпаковки",            Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(10, 3)));
	...
	Если ПараметрыСканирования <> Неопределено
		И ПараметрыСканирования.ДополнительныеПараметры <> Неопределено Тогда
		ПараметрыСканирования.ДополнительныеПараметры.Свойство("ДанныеУточнения", ДанныеУточнения);
	КонецЕсли;
	...
	Пока ИндексСтроки >= 0 Цикл
		...
		ИначеЕсли ДанныеУточнения <> Неопределено
			И (ДанныеСтроки.Номенклатура <> ДанныеУточнения.Номенклатура
			   Или ДанныеСтроки.Характеристика <> ДанныеУточнения.Характеристика) Тогда
			ДанныеПоШтрихкодамEAN.Удалить(ДанныеСтроки);
		КонецЕсли;
	...
	Для Каждого ЭлементДанных Из ДанныеПоШтрихкодамEAN Цикл
		...
		Если Не ДопустимаПродукцияИСМП
			И КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииИСМПЕГАИС(ЭлементДанных.ВидПродукции) Тогда
	...
	ОбновлениеИнформационнойБазыГосИС.ПроверитьОбработатьТаблицуНоменклатурыПриПереходеНаКонструкторНастроек(ВозвращаемоеЗначение.ДанныеПоШтрихкодамEAN);
...
Функция ПараметрыДополненияТаблицыДанныеКодовМаркировки(ДанныеПоШтрихкодам, ДеревоУпаковок, ИменаКолонокДляКопированияСвойств = "", ЭтоЗагрузкаИзТСД = Ложь) Экспорт
	...
	Если ТипЗнч(ИменаКолонокДляКопированияСвойств) = Тип("Массив") Тогда
		...
		ПараметрыСлиянияДанных.Вставить("КолонкиИББезСервиса",   "");
		
		КолонкиНоменклатуры            = Новый Массив;
		КолонкиДанныеСервиса           = Новый Массив;
		КолонкиИББезСервиса            = Новый Массив;
		ДобавитьКолонкиПоДаннымСервиса = Ложь;
		...
		Для Каждого ИмяКолонки Из ИменаКолонокДляКопированияСвойств Цикл
			...
			ИначеЕсли ИменаКолонокДляЗаполнения.ДанныеСервиса[ИмяКолонки] <> Неопределено Тогда
				Если ИменаКолонокДляЗаполнения.КолонкиУточнениеПоДаннымСервиса[ИмяКолонки] <> Неопределено Тогда
					ДобавитьКолонкиПоДаннымСервиса = Истина;
				Иначе
			...
			Если ИменаКолонокДляЗаполнения.КолонкиИББезСервиса[ИмяКолонки] <> Неопределено Тогда
				КолонкиИББезСервиса.Добавить(ИмяКолонки);
			КонецЕсли;
			
		КонецЦикла;
		
		// Если получены данные сервиса, то представление номенклатуры нужно заполять из сервиса.
		// Иначе представление номенклатуры заполняется по данным ИБ.
		Если ДобавитьКолонкиПоДаннымСервиса
			И КолонкиДанныеСервиса.Количество() Тогда
			Для Каждого КлючИЗначение Из ИменаКолонокДляЗаполнения.КолонкиУточнениеПоДаннымСервиса Цикл
				КолонкиДанныеСервиса.Добавить(КлючИЗначение.Ключ);
			КонецЦикла;
		ИначеЕсли ДобавитьКолонкиПоДаннымСервиса Тогда
			Для Каждого КлючИЗначение Из ИменаКолонокДляЗаполнения.КолонкиУточнениеПоДаннымСервиса Цикл
				КолонкиНоменклатуры.Добавить(КлючИЗначение.Ключ);
			КонецЦикла;
		КонецЕсли;
		...
		ПараметрыСлиянияДанных.КолонкиИББезСервиса  = СтрСоединить(КолонкиИББезСервиса, ",");
...
Функция РезультатОбработкиШтрихкодаМаркируемойПродукции(Форма, ДанныеШтрихкода, ПараметрыСканирования) Экспорт
	...
	ИначеЕсли ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ЕГАИС")
		И (ОбщегоНазначенияИСПовтИсп.ЭтоМаркируемаяПродукцияЕГАИС(ДанныеШтрихкода.ВидПродукции)
			Или (ПараметрыСканирования.Свойство("ТолькоЕГАИС") И ПараметрыСканирования.ТолькоЕГАИС)) Тогда
		
		МодульШтрихкодированиеОбщегоНазначенияЕГАИС = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияЕГАИС");
	...
	ИначеЕсли ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ИСМП")
		...
		//@skip-check wrong-string-literal-content
		МодульШтрихкодированиеОбщегоНазначенияМОТП  = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияМОТП");
...
Процедура ЗаполнитьДанныеПоШтрихкодам(ДанныеПоШтрихкодам, ПараметрыСканирования, Форма = Неопределено) Экспорт
	...
	ЭтоПроверкаКодовМаркировкиИСМП = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоПроверкаКодовМаркировкиИСМП(ПараметрыСканирования);
	ЭтоОтчетПроизводственнойЛинии = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоОтчетПроизводственнойЛинии(ПараметрыСканирования);
...
Процедура ДополнитьДанныеШтрихкодовСпецификойМаркируемойПродукции(ДанныеПоШтрихкодам, ПараметрыСканирования, ПараметрыДополнения = Неопределено, ЗаполнятьСтрокиДерева = Ложь) Экспорт
	...
	Если ДопустимаАлкогольнаяПродукция(ПараметрыСканирования)
		И (ПрисутствуетАлкогольнаяПродукция(ВидыПродукции)
		Или ДанныеПоШтрихкодам.ПрисутствуетАлкогольнаяПродукция) Тогда
		
		ДополнитьДанныеШтрихкодовРасширеннойСпецификойЕГАИС(ПараметрыСканирования, ДанныеПоШтрихкодам, ПараметрыДополнения);
	...
	Если ДопустимаТабачнаяПродукция(ПараметрыСканирования)
		...
		ДополнитьДанныеШтрихкодовРасширеннойСпецификойМОТП(ПараметрыСканирования, ДанныеПоШтрихкодам, ПараметрыДополнения, ЗаполнятьСтрокиДерева);
	...
	Если ДопустимаПродукцияИСМП(ПараметрыСканирования)
		...
		ДополнитьДанныеШтрихкодовРасширеннойСпецификойИСМП(ПараметрыСканирования, ДанныеПоШтрихкодам, ПараметрыДополнения);
...
Функция ВыполнитьПроверкиПередПолучениемДанныхПоШтрихкодам(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	КешНаличияСвойств = Новый Структура;
	КешНаличияСвойств.Вставить("ЕстьСвойствоСтрокиДанныхДанныеRFID", Неопределено);
	
	ЭтоСканированиеВФормуПроверкиИПодбора = ЭтоСканированиеВФормеПроверкиИПодбора(ПараметрыСканирования);
	
	ОбработанныеКодыМаркировки = Неопределено;
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		//@skip-check query-in-loop
		РезультатПроверки = ПроверкаШтрихкодаНаУжеДобавленный(СтрокаДанных, ПараметрыСканирования, ДанныеПоШтрихкодам, ОбработанныеКодыМаркировки);
		Если ЗначениеЗаполнено(РезультатПроверки) Тогда
			СтрокаДанных.ОбработанСОшибками = Истина;
			РезультатПроверки.ДанныеШтрихкода = СтрокаТаблицыДанныеКодовМаркировкиВСтруктуру(СтрокаДанных, Шаблон, ГрупповаяЗагрузка);
			РезультатПроверок.Вставить(СтрокаДанных.Штрихкод, РезультатПроверки);
		...
		РезультатПроверки = ТребуетсяОткрытьФормуВводаКодаМаркировки(СтрокаДанных, ПараметрыСканирования, КешНаличияСвойств, ЭтоСканированиеВФормуПроверкиИПодбора);
		Если ЗначениеЗаполнено(РезультатПроверки) Тогда
			СтрокаДанных.ОбработанСОшибками = Истина;
			РезультатПроверок.Вставить(СтрокаДанных.Штрихкод, РезультатПроверки);
			Продолжить;
		...
		РезультатПроверки = ПроверкаОшибкаНеЗагруженКлассификаторПоВидуПродукции(СтрокаДанных, ПараметрыСканирования);
		Если ЗначениеЗаполнено(РезультатПроверки) Тогда
			СтрокаДанных.ОбработанСОшибками = Истина;
			РезультатПроверок.Вставить(СтрокаДанных.Штрихкод, РезультатПроверки);
			Продолжить;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ЕГАИС") Тогда
		...
		//@skip-check wrong-string-literal-content
		Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ОбщегоНазначенияЕГАИС");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ИСМП") Тогда
		...
		//@skip-check wrong-string-literal-content
		Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ОбщегоНазначенияИСМП");
		Если Модуль.ИспользуетсяМаркируемаяПродукция() Тогда
			...
			Если ДопустимаТабачнаяПродукция(ПараметрыСканирования)
				...
				//@skip-check wrong-string-literal-content
				Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияМОТП");
...
Функция ТребуетсяОткрытьФормуВводаКодаМаркировки(СтрокаДанных, ПараметрыСканирования, КешНаличияСвойств = Неопределено, ЭтоСканированиеВФормуПроверкиИПодбора = Ложь) Экспорт
	...
	ЭтоЗапросКодаМаркировкиПослеСчитыванияEAN = СтрокаДанных.ЭтоШтрихкодНоменклатуры
		...
		И Не СтрокаДанных.ТребуетсяУточнениеЧастичногоВыбытия;
	
	Если КешНаличияСвойств = Неопределено Тогда
		ЕстьСвойствоСтрокиДанныхДанныеRFID = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаДанных, "ДанныеRFID");
	Иначе
		ЕстьСвойствоСтрокиДанныхДанныеRFID = КешНаличияСвойств.ЕстьСвойствоСтрокиДанныхДанныеRFID;
		Если ЕстьСвойствоСтрокиДанныхДанныеRFID = Неопределено Тогда
			ЕстьСвойствоСтрокиДанныхДанныеRFID = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаДанных, "ДанныеRFID");
			КешНаличияСвойств.ЕстьСвойствоСтрокиДанныхДанныеRFID = ЕстьСвойствоСтрокиДанныхДанныеRFID;
		КонецЕсли;
	КонецЕсли;
	
	ЭтоЗапросКодаМаркировкиПослеСчитыванияRFID = Не ЗначениеЗаполнено(СтрокаДанных.Штрихкод)
		И ЕстьСвойствоСтрокиДанныхДанныеRFID
		И СтрокаДанных.ДанныеRFID <> Неопределено;
	
	СохраненОтказОтВводаКодаМаркировки = СтрокаДанных.ЭтоШтрихкодНоменклатуры
		И ПараметрыСканирования.ДанныеВыбораПоМаркируемойПродукции <> Неопределено
		И ПараметрыСканирования.ДанныеВыбораПоМаркируемойПродукции.GTIN = СтрокаДанных.GTIN
		И ПараметрыСканирования.ДанныеВыбораПоМаркируемойПродукции.ОбработатьБезМаркировки;
	
	Если ПараметрыСканирования.ИСМП И ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаДанных.ВидПродукции, Истина) Тогда
		
		ТребуетсяВводМарок = СтрокаДанных.ВидУпаковки <> Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет
			И (ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПоВидуОперацииИПродукции(
			ПараметрыСканирования.ВидОперацииИСМП, СтрокаДанных.ВидПродукции) Или ЭтоСканированиеВФормуПроверкиИПодбора);
		Если Не СтрокаДанных.ОбработатьБезМаркировки И СохраненОтказОтВводаКодаМаркировки Тогда
			СтрокаДанных.ОбработатьБезМаркировки = СохраненОтказОтВводаКодаМаркировки;
		КонецЕсли;
	...
	Если ТребуетсяВводМарок
		И Не СохраненОтказОтВводаКодаМаркировки
		И (ЭтоЗапросКодаМаркировкиПослеСчитыванияEAN
...
Функция НормализироватьДанныеДереваУпаковокПоПереданнымКолонкам(ВходящаяКоллекция, КоллекцияДополнительныхКолонок, ДобавленныеКолонки = Неопределено) Экспорт
	
	Если ДобавленныеКолонки = Неопределено Тогда
		ДобавленныеКолонки = Новый Соответствие;
	КонецЕсли;
	
	Если ТипЗнч(ВходящаяКоллекция) = Тип("СтрокаДереваЗначений") Тогда
		Коллекция = ВходящаяКоллекция.Владелец();
	ИначеЕсли ТипЗнч(ВходящаяКоллекция) = Тип("СтрокаТаблицыЗначений") Тогда
		Коллекция = ВходящаяКоллекция.Владелец();
	Иначе
		Коллекция = ВходящаяКоллекция;
	КонецЕсли;
	...
	Для Каждого КлючЗначение Из КоллекцияДополнительныхКолонок Цикл
		...
		Колонка = Коллекция.Колонки.Найти(ИмяКолонки);
		...
		ИначеЕсли ОписаниеТипов <> Неопределено Тогда
			...
			НужноИзменитьТипКолонки = Ложь;
			
			СуществующиеТипы = Колонка.ТипЗначения.Типы();
			ОбъединенныеТипы = ОписаниеТипов.Типы();
			// Создать описание типов, включающее и тип1 и тип2
			Для Каждого Тип Из СуществующиеТипы Цикл
				Если ОбъединенныеТипы.Найти(Тип) = Неопределено Тогда
					НужноИзменитьТипКолонки = Истина;
					ОбъединенныеТипы.Добавить(Тип);
				КонецЕсли;
			КонецЦикла;
			
			Если НужноИзменитьТипКолонки Тогда
				
				Если Коллекция.Количество() = 0 Тогда
					Коллекция.Колонки.Удалить(Колонка);
					Коллекция.Колонки.Добавить(ИмяКолонки, Новый ОписаниеТипов(ОбъединенныеТипы));
				Иначе
					
					ВызватьИсключение НСтр("ru = 'Изменение типа заполненной коллекции'");
...
Процедура ОпределитьШтрихкодыВводаОстатков(ДанныеПоШтрихкодам, ПараметрыСканирования)
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если ОбщегоНазначенияИСПовтИсп.ЭтоМаркируемаяПродукцияЕГАИС(СтрокаДанных.ВидПродукции)
			Или ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМЕХ(СтрокаДанных.ВидПродукции)
		...
		Если Не ЕстьОшибки И ЗначениеЗаполнено(СтрокаДанных.GTIN) Тогда
			...
			Если ЗначениеЗаполнено(СтрокаДанных.СтрокаДерева) Тогда
				СтрокаДанных.СтрокаДерева.МаркируемаяПродукция = СтрокаДанных.МаркируемаяПродукция;
				СтрокаДанных.СтрокаДерева.ЭтоКодВводаОстатков  = СтрокаДанных.ЭтоКодВводаОстатков;
			КонецЕсли;
...
Процедура ЗаполнитьДанныеКодовМаркировкиВходящимиДанными(ДанныеПоШтрихкодам, СписокДанныхШтрихкода, ПараметрыСканирования)
	...
	Для Каждого ДанныеШтрихкода Из СписокДанныхШтрихкода Цикл
		...
		Если ТипЗнч(ДанныеШтрихкода) = Тип("Структура") Тогда
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ДанныеШтрихкода,,"Штрихкод");
			
			Если ЗаполнятьДанныеRFID = Неопределено Тогда
				ЗаполнятьДанныеRFID = ДанныеШтрихкода.Свойство("ДанныеRFID")
			...
			Если ДанныеШтрихкода.Свойство("ЗагруженИзЭлектронногоДокумента") Тогда
				НоваяСтрока.ДополнительныеПараметры = Новый Структура("ЗагруженИзЭлектронногоДокумента", ДанныеШтрихкода.ЗагруженИзЭлектронногоДокумента);
			КонецЕсли;
			
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(НоваяСтрока.ШтрихкодBase64) Тогда
			НоваяСтрока.ШтрихкодBase64 = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ШтрихкодВBase64(ДанныеШтрихкода.Штрихкод);
		КонецЕсли;
		
		Если ДанныеRFID <> Неопределено И Не ЗаполнятьДанныеRFID Тогда
		...
		Если ЗначениеЗаполнено(НоваяСтрока.ШтрихкодНоменклатуры) Тогда
			НоваяСтрока.EAN = НоваяСтрока.ШтрихкодНоменклатуры;
			НоваяСтрока.GTIN = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.GTINПоШтрихкодуEAN(НоваяСтрока.EAN);
		КонецЕсли;
...
Процедура ПроверитьНаСоответствиеНоменклатурыВидуПродукции(ДанныеПоШтрихкодам, ПараметрыСканирования, ЕстьОшибки)
	...
	Если ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоПроверкаКодовМаркировкиИСМП(ПараметрыСканирования)
		Или ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоОтчетПроизводственнойЛинии(ПараметрыСканирования) Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если ТипЗнч(СтрокаДанных.ВидПродукции) = ТипКлючиВидовПродукцииИС Тогда
			ВидПродукции = Неопределено;
			Если ПараметрыСканирования.ИСМП Тогда
				ВидПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(СтрокаДанных.ВидПродукции, "ГИСМТ");
			КонецЕсли;
			Если Не ЗначениеЗаполнено(ВидПродукции) И ПараметрыСканирования.ЕГАИС Тогда
				ВидПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(СтрокаДанных.ВидПродукции, "ЕГАИС");
			КонецЕсли;
		Иначе
			ВидПродукции = СтрокаДанных.ВидПродукции;
		КонецЕсли;
		...
		ИначеЕсли Не СтрокаДанных.ЭтоШтрихкодНоменклатуры И ЗначениеЗаполнено(ВидПродукции)
			И СтрокаДанных.ВидыПродукцииКодаМаркировки.Найти(ВидПродукции) = Неопределено Тогда
			
			НайденПереходныйВидПродукции = Ложь;
			Для Каждого ПроверяемыйВидПродукции Из СтрокаДанных.ВидыПродукцииКодаМаркировки Цикл
				Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ПереходныйВидПродукции(ВидПродукции, ПроверяемыйВидПродукции) Тогда
					НайденПереходныйВидПродукции = НайденПереходныйВидПродукции Или ЗначениеЗаполнено(СтрокаДанных.Номенклатура);
				КонецЕсли;
			КонецЦикла;
			
			Если Не НайденПереходныйВидПродукции Тогда
...
Процедура ПроверитьНаДопустимостьОбработкиВидовПродукции(ДанныеПоШтрихкодам, ПараметрыСканирования, ЕстьОшибки)
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		НеОпределенВидПродукции = Ложь;
		Если ЗначениеЗаполнено(ВидПродукции) Тогда
			
			Для Каждого ПроверяемыйВидПродукции Из КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииМОТП() Цикл
				
				ДопустимаПродукция = ПараметрыСканирования.ДопустимыеВидыПродукции.Найти(ПроверяемыйВидПродукции) <> Неопределено;
				Если ДопустимаПродукция
					И КонструкторИСМПОбщегоНазначенияКлиентСервер.ПереходныйВидПродукции(ВидПродукции, ПроверяемыйВидПродукции) Тогда
					ВыдаватьОшибку = ВыдаватьОшибку Или ЗначениеЗаполнено(СтрокаДанных.Номенклатура);
				Иначе
					ВыдаватьОшибку = Истина;
				КонецЕсли;
			...
			Если ПараметрыСканирования.ДопустимыеВидыПродукции.Найти(ВидПродукции) = Неопределено
				И ВыдаватьОшибку
				И ВидыПродукции.Найти(ВидПродукции) = Неопределено Тогда
		...
		ИначеЕсли ВыдаватьОшибку
			И СтрокаДанных.ПримечаниеКРазборуШтрихкода <> Неопределено
			И Не ЗначениеЗаполнено(СтрокаДанных.ПримечаниеКРазборуШтрихкода.ИдентификаторОшибки)
			И Не ЗначениеЗаполнено(ВидПродукции) Тогда
			
			// Код разобран, соответствует некоторым видам продукции, но определить вид продукции не удалось.
			НеОпределенВидПродукции = Истина;
		...
		Если ВидыПродукции.Количество() > 0 Тогда
			...
			Если УчитываемыеВидыМаркируемойПродукции = Неопределено И ПараметрыСканирования.РасширеннаяВерсияГосИС Тогда
				//@skip-check wrong-string-literal-content
				Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеИС");
		...
		ИначеЕсли НеОпределенВидПродукции Тогда
			
			Отказ = Истина;
			
			СтрокаДанных.ТекстОшибки = НСтр("ru = 'Не удалось определить вид продукции'");
...
Функция ПроверкаОшибкиПриРазбореКодаМаркировки(СтрокаДанных, ПараметрыСканирования)
	...
	Если ОбработкаСтрокиXML.НайтиНедопустимыеСимволыXML(СтрокаДанных.Штрихкод) = 0 Тогда
...
Функция КонтрольНаДопустимыеВидыПродукции(СтрокаДанных, ПараметрыСканирования)
	...
	Если Не ЗначениеЗаполнено(ВидПродукции) Тогда
		...
		Если СтрокаДанных.ПримечаниеКРазборуШтрихкода <> Неопределено
			...
			РезультатОбработки = РезультатОбработкиОшибкаКонтроляДопустимыхВидовПродукции(СтрокаДанных, ПараметрыСканирования, ВидыПродукции);
		...
		ИначеЕсли ЗначениеЗаполнено(СтрокаДанных.Номенклатура)
			...
			И ПараметрыСканирования.ДопустимыеВидыПродукции[0] <> ВидПродукции Тогда
	...
	ДопустимВидПродукции = ПараметрыСканирования.ДопустимыеВидыПродукции.Найти(ВидПродукции) <> Неопределено;
	
	Если Не ДопустимВидПродукции Тогда
		
		РезультатОбработки = РезультатОбработкиОшибкаКонтроляДопустимыхВидовПродукции(СтрокаДанных, ПараметрыСканирования, ВидПродукции);
...
//  ПараметрыСканирования - см. ШтрихкодированиеОбщегоНазначенияИСКлиент.ПараметрыСканирования
//  ДанныеПоШтрихкодам - см. ИнициализацияДанныхПоШтрихкодам
//  ОбработанныеКодыМаркировки - Неопределено, Соответствие - кеш обработанных строк
// Возвращаемое значение:
//   См. ИнициализироватьРезультатОбработкиШтрихкода.
Функция ПроверкаШтрихкодаНаУжеДобавленный(СтрокаДанных, ПараметрыСканирования, ДанныеПоШтрихкодам, ОбработанныеКодыМаркировки = Неопределено)
	
	Штрихкод = СтрокаДанных.Штрихкод;
	
	Если ОбработанныеКодыМаркировки = Неопределено Тогда
...
Функция ПроверкаОшибкаНеЗагруженКлассификаторПоВидуПродукции(СтрокаДанных, ПараметрыСканирования)
	...
	Если ТипЗнч(СтрокаДанных.ВидПродукции) = Тип("СправочникСсылка.КлючиВидовПродукцииИС") Тогда
		...
		Если ПараметрыСканирования.ИСМП Тогда
			...
			ВидПродукции     = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(СтрокаДанных.ВидПродукции, "ГИСМТ");
			Если ЗначениеЗаполнено(ВидПродукции) И Не ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(ВидПродукции, Истина) Тогда
				...
				СтрокаДанных.ТекстОшибки       = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ТекстОшибкиНеЗагруженКлассификаторПоВидуПродукции(ВидПродукции);
...
// Параметры:
//  СтрокаДанных - СтрокаТаблицыЗначений Из (См. ИнициализироватьТаблицуМаркированныеТовары).
//  ПоддерживаемыеВидыПродукции - Массив Из СправочникСсылка.ВидыПродукцииИС - Список допустимых видов продукции.
//  ВидПродукцииШтрихкода - СправочникСсылка.ВидыПродукцииИС, Массив из СправочникСсылка.ВидыПродукцииИС - вид продукции штрихкода.
// Возвращаемое значение:
//  Строка - Текст ошибки.
Функция ТекстОшибкиНеПоддерживаетсяВидПродукции(СтрокаДанных, ПоддерживаемыеВидыПродукции, ВидПродукцииШтрихкода)
	...
	Если ЗначениеЗаполнено(ВидПродукцииШтрихкода) Тогда
		Если ТипЗнч(ВидПродукцииШтрихкода) = Тип("Массив") Тогда
			СортированныеПредставления = Новый СписокЗначений();
			Для Каждого ВидПродукции Из ВидПродукцииШтрихкода Цикл
				СортированныеПредставления.Добавить(Строка(ВидПродукции));
			КонецЦикла;
			СортированныеПредставления.СортироватьПоЗначению();
			ПредставлениеВидаПродукции = СтрСоединить(СортированныеПредставления.ВыгрузитьЗначения(), ", ");
		Иначе
			ПредставлениеВидаПродукции = Строка(ВидПродукцииШтрихкода);
		КонецЕсли;
	КонецЕсли;
	
	Позиция = ОбработкаСтрокиXML.НайтиНедопустимыеСимволыXML(СтрокаДанных.Штрихкод);
	...
	Иначе
		СортированныеПредставления = Новый СписокЗначений();
		Для Каждого ВидПродукции Из ПоддерживаемыеВидыПродукции Цикл
			СортированныеПредставления.Добавить(Строка(ВидПродукции));
		КонецЦикла;
		СортированныеПредставления.СортироватьПоЗначению();
		ПоддерживаемыеВидыПродукцииСтрокой = СтрСоединить(СортированныеПредставления.ВыгрузитьЗначения(), ", ");
	КонецЕсли;
...
//  ВидПродукции - СправочникСсылка.ВидыПродукцииИС - Вид продукции, по которому будут инициализированы параметры сканирования.
//  ФормаВыбора - ФормаКлиентскогоПриложения - форма выбора.
Процедура ЗаполнитьПараметрыСканированияДляАлкогольнойПродукции(ПараметрыСканирования, Контекст, ВидПродукции, ФормаВыбора)
	...
	Если Не ЗначениеЗаполнено(ВидПродукции)
		Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(ВидПродукции) Тогда
		
		//@skip-check wrong-string-literal-content
		Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияЕГАИСКлиентСервер");
...
Процедура ЗаполнитьПараметрыСканированияДляТабачнойПродукции(ПараметрыСканирования, Контекст, ВидПродукции, ФормаВыбора, ПараметрыРежимаИсправленияОшибок = Неопределено)
	...
	Если Не ПараметрыСканирования.ПодсистемаИСМПСуществует Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ВидПродукции)
		Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(ВидПродукции) Тогда
		
		//@skip-check wrong-string-literal-content
		МодульШтрихкодированиеМОТПКлиентСервер = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияМОТПКлиентСервер");
...
Процедура ЗаполнитьПараметрыСканированияДляПродукцииИСМП(ПараметрыСканирования, Контекст, ФормаВыбора, ВидПродукции, ПараметрыРежимаИсправленияОшибок = Неопределено)
	...
	Если Не ЗначениеЗаполнено(ВидПродукции)
		Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(ВидПродукции) Тогда
		
		//@skip-check wrong-string-literal-content
		Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияИСМПКлиентСервер");
...
//
// Параметры:
//  Контекст - ФормаКлиентскогоПриложения, ДокументСсылка, СправочникСсылка, СправочникСсылка.ВидыПродукцииИС -
...
Функция ДопустимаАлкогольнаяПродукция(Контекст) Экспорт
	...
	Если ТипЗнч(Контекст) = Тип("СправочникСсылка.ВидыПродукцииИС") Тогда
		Возврат ОбщегоНазначенияИСПовтИсп.ЭтоМаркируемаяПродукцияЕГАИС(Контекст);
	КонецЕсли;
	
	Для Каждого ВидПродукции Из КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЕГАИС() Цикл
		Если ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ДопустимВидПродукции(
			Контекст, ВидПродукции) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
//
//Параметры:
//   Контекст - Структура, СправочникСсылка.ВидыПродукцииИС -
...
Функция ДопустимаПродукцияИзНатуральногоМеха(Контекст) Экспорт
	...
	Если ТипЗнч(Контекст) = Тип("СправочникСсылка.ВидыПродукцииИС") Тогда
		Возврат ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМЕХ(Контекст);
	КонецЕсли;
...
Функция ОбработатьШтрихкоды(ДанныеШтрихкодаСписок, ПараметрыСканирования, КэшированныеЗначения,
	...
	Иначе
		...
		ДополнитьТаблицуДанныеКодовМаркировкиПоСтрокамДереваУпаковок(ПараметрыСканирования, ДанныеПоШтрихкодам);
		ИндексироватьКолонкиТаблицыДанныхКодовМаркировки(ДанныеПоШтрихкодам.ДанныеКодовМаркировки);
	...
	Если ЕстьОшибкиИЧастичнаяОбработкаКодовМаркировкиНедоступна(ПараметрыСканирования, РезультатПроверок, ДанныеПоШтрихкодам) Тогда
		...
		Если ПараметрыСканирования.ИСМП Тогда
			...
			МодульЛогированиеЗапросовИСМП = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ЛогированиеЗапросовИСМПВызовСервера");
	...
	Если ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоПроверкаКодовМаркировкиИСМП(ПараметрыСканирования) Тогда
...
Процедура ЗаполнитьВидыПродукцииВДанныхПоШтрихкодам(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	Для Каждого ЭлементДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		ИначеЕсли ЭлементДанных.ПрисутствуетАлкогольнаяПродукция Тогда
			ДанныеПоШтрихкодам.ПрисутствуетАлкогольнаяПродукция = Истина;
...
//  ПараметрыСканирования - (См. ШтрихкодированиеОбщегоНазначенияИСКлиент.ПараметрыСканирования).
Процедура ЗаполнитьСтрокуДанныхКодовМаркировкиПоДаннымРазбора(СтрокаДанных, ОбщиеМодули, ПараметрыСканирования) Экспорт
	...
	ИначеЕсли СтрокаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Набор 
		И ПараметрыСканирования.Свойство("ЭтоМаркировкаТоваровИСМП")
		И ПараметрыСканирования.ЭтоМаркировкаТоваровИСМП Тогда
	...
	// признаки МОТП, ЕГАИС - будет определено позднее.
	Если СтрокаДанных.ВидыПродукцииКодаМаркировки.Количество() > 0 Тогда
		ПрисутствуетТабачнаяПродукция = Истина;
		ПрисутствуетАлкогольнаяПродукция = Истина;
		Для Каждого ВидПродукции Из СтрокаДанных.ВидыПродукцииКодаМаркировки Цикл
			ПрисутствуетТабачнаяПродукция = ПрисутствуетТабачнаяПродукция
				И ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМОТП(ВидПродукции);
			ПрисутствуетАлкогольнаяПродукция = ПрисутствуетАлкогольнаяПродукция
				И КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(ВидПродукции);
		КонецЦикла;
		СтрокаДанных.ПрисутствуетТабачнаяПродукция = ПрисутствуетТабачнаяПродукция;
		СтрокаДанных.ПрисутствуетАлкогольнаяПродукция = ПрисутствуетАлкогольнаяПродукция;
	КонецЕсли;
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если ТребуетсяСопоставлениеНоменклатуры Тогда
			...
			Если СоответствиеШтрихкодовКСопоставлению[ШтрихкодКСопоставлению] = Неопределено Тогда
				...
				ПараметрыСопоставления = ИнициализироватьПараметрыСопоставления();
				ЗаполнитьЗначенияСвойств(ПараметрыСопоставления, СтрокаДанных);
				ПараметрыСопоставления.Штрихкод   = ШтрихкодКСопоставлению;
				ПараметрыСопоставления.Количество = 1;
				
				ШтрихкодыКСопоставлению.Добавить(ПараметрыСопоставления);
...
Функция ЭтоАгрегатТСД(ЗначениеШтрихкода) Экспорт
	...
	Если ОбщегоНазначенияИС.ЭтоРасширеннаяВерсияГосИС() Тогда
		ОбщийМодуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ГрупповаяОбработкаШтрихкодовИС");
...
Процедура ЗаполнитьПараметрыСканированияРасширенные(ПараметрыСканирования, Контекст)
	...
	Если ОбщегоНазначенияИС.ЭтоРасширеннаяВерсияГосИС("ИСМП") Тогда
		...
		//@skip-check wrong-string-literal-content
		МодульШтрихкодированиеИСКлиентСервер = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеИСКлиентСервер");
...
Процедура НормализоватьДанныеВложенныхШтрихкодов(ДанныеПоШтрихкодам, ПараметрыСканирования, ДанныеКодовМаркировки = Неопределено) Экспорт
	...
	Если ДопустимаПродукцияИСМП(ПараметрыСканирования)
		Или ДопустимаТабачнаяПродукция(ПараметрыСканирования) Тогда
...
Функция ЭтоСканированиеВФормеПроверкиИПодбора(ПараметрыСканирования) Экспорт
	Возврат (ПараметрыСканирования.ЭтоСканированиеВФормеПроверкиИПодбора
		И Не ПараметрыСканирования.РежимБРМК
		И ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ВидОперацииИСМП(ПараметрыСканирования) <> Перечисления.ВидыОперацийИСМП.ВскрытиеПотребительскойУпаковки);
КонецФункции
...
Процедура ДополнитьТаблицуДанныеКодовМаркировкиПоСтрокамДереваУпаковок(ПараметрыСканирования, ДанныеПоШтрихкодам)
...
Процедура ЗаполнитьСведенияПоШтрихкодамПоДаннымПулаGTIN(ДанныеПоШтрихкодам, ПараметрыСканирования)

	//@skip-check wrong-string-literal-content
	Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеИС");
	Модуль.ЗаполнитьСведенияПоШтрихкодамПоДаннымПулаGTIN(ДанныеПоШтрихкодам, ПараметрыСканирования);

КонецПроцедуры

Функция НайденныеШтрихкодыEAN(Знач ЗначениеПоиска, Знач ЗначениеУточнения, ИнформацияПоШтрихкодамEAN) Экспорт
	
	// При наличии штрихкода уточнения, приоритеты:
	// 1) Пересечение штрихкода уточнения и штрихкода номенклатуры (совпадение номенклатуры и характеристики в двух наборах),
	// 2) Данные штрихкода номенклатуры,
	// 3) Данные штрихкода уточнения.
	НайденныеШтрихкодыEAN = ИнформацияПоШтрихкодамEAN.НайтиСтроки(Новый Структура("ШтрихкодEAN", ЗначениеПоиска));
	
	Если ЗначениеЗаполнено(ЗначениеУточнения) Тогда
		НайденныеШтрихкоды = ИнформацияПоШтрихкодамEAN.НайтиСтроки(Новый Структура("ШтрихкодEAN", ЗначениеУточнения));
	Иначе
		Возврат НайденныеШтрихкодыEAN;
	КонецЕсли;
	
	Если НайденныеШтрихкодыEAN.Количество() = 0 Тогда
	...
	Для Каждого НайденныйШтрихкод Из НайденныеШтрихкоды Цикл
		...
		// Неуточняющие данные штрихкодов
		Если НайденныйШтрихкод.ТребуетсяОбработкаШтрихкода Тогда
			Продолжить;
		КонецЕсли;
		
		ДанныеНоменклатуры = ДанныеУточнения[НайденныйШтрихкод.Номенклатура];
		Если ДанныеНоменклатуры = Неопределено Тогда
			ДанныеНоменклатуры = Новый Соответствие;
		КонецЕсли;
		ДанныеНоменклатуры.Вставить(НайденныйШтрихкод.Характеристика, Истина);
		ДанныеУточнения.Вставить(НайденныйШтрихкод.Номенклатура, ДанныеНоменклатуры);
		
	КонецЦикла;
	
	ПересечениеДанных = Новый Массив;
	Для Каждого НайденныйШтрихкод Из НайденныеШтрихкодыEAN Цикл
		Если НайденныйШтрихкод.ТребуетсяОбработкаШтрихкода Тогда
			Возврат НайденныеШтрихкодыEAN;
		КонецЕсли;
		ДанныеНоменклатуры = ДанныеУточнения[НайденныйШтрихкод.Номенклатура];
		Если ДанныеНоменклатуры <> Неопределено
			И ДанныеНоменклатуры[НайденныйШтрихкод.Характеристика] <> Неопределено Тогда
				ПересечениеДанных.Добавить(НайденныйШтрихкод);
		КонецЕсли;
	КонецЦикла;
	
	Если ПересечениеДанных.Количество() > 0 Тогда
		Возврат ПересечениеДанных;
	КонецЕсли;
...
Процедура ДополнитьДанныеШтрихкодовРасширеннойСпецификойЕГАИС(ПараметрыСканирования, ДанныеПоШтрихкодам, ПараметрыДополнения = Неопределено)
	...
	Если ПараметрыСканирования.РасширеннаяВерсияГосИС Тогда
		...
		МодульШтрихкодированиеЕГАИС.ДополнитьДанныеПоШтрихкодамСпецификой(ДанныеПоШтрихкодам, ПараметрыСканирования, ПараметрыДополнения);
	
	КонецЕсли;
	
КонецПроцедуры

Процедура ДополнитьДанныеШтрихкодовРасширеннойСпецификойМОТП(ПараметрыСканирования, ДанныеПоШтрихкодам, ПараметрыДополнения = Неопределено, ЗаполнятьСтрокиДерева = Ложь)
	...
	Если ПараметрыСканирования.РасширеннаяВерсияГосИС Тогда
		...
		//@skip-check wrong-string-literal-content
		МодульШтрихкодированиеМОТП = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеМОТП");
		МодульШтрихкодированиеМОТП.ДополнитьДанныеПоШтрихкодамСпецификой(ДанныеПоШтрихкодам, ПараметрыСканирования, ПараметрыДополнения, ЗаполнятьСтрокиДерева);
...
Процедура ДополнитьДанныеШтрихкодовРасширеннойСпецификойИСМП(ПараметрыСканирования, ДанныеПоШтрихкодам, ПараметрыДополнения = Неопределено, ЗаполнятьСтрокиДерева = Ложь)
	...
	Если ПараметрыСканирования.РасширеннаяВерсияГосИС Тогда
		...
		//@skip-check wrong-string-literal-content
		МодульШтрихкодированиеИСМПСлужебный = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеИСМПСлужебный");
		МодульШтрихкодированиеИСМПСлужебный.ДополнитьДанныеПоШтрихкодамСпецификой(ДанныеПоШтрихкодам, ПараметрыСканирования, ПараметрыДополнения, ЗаполнятьСтрокиДерева);
...
Функция СписокНоменклатурыДляУточненияСодержитМаркируемуюПродукцию(СписокНоменклатуры)
	
	ВозвращаемоеЗначение = Ложь;
	
	Для Каждого СтрокаДанных Из СписокНоменклатуры Цикл
		Если ЗначениеЗаполнено(СтрокаДанных.ВидПродукции) Тогда
...
//
// Параметры:
//  ДеревоУпаковок - ДеревоЗначений, СтрокаДереваЗначений - строки для проверки на соответствие виду продукции.
//  ВидПродукции - СправочникСсылка.ВидыПродукцииИС - Вид продукции для отбора строк дерева.
	...
	Для Каждого СтрокаДерева из ДеревоУпаковок.Строки Цикл
		...
		Если СтрокаДерева.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
			Если СтрокаДерева.ВидПродукции <> ВидПродукции Тогда
				Если Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ПереходныйВидПродукции(СтрокаДерева.ВидПродукции, ВидПродукции) Тогда
					ПараметрыПроверки.УдалитьТекущуюУпаковку = Истина;
					Если ПерваяИтерация Тогда
						МассивУдаляемыхСтрок.Добавить(СтрокаДерева);
					Иначе
...
Функция ТребуетсяЧастичноеВыбытие(ПараметрыСканирования, ДанныеПоШтрихкодам = Неопределено) Экспорт
	...
	Если ВозвращаемоеЗначение = Неопределено Тогда
		Если ПараметрыСканирования <> Неопределено
			...
			ВозвращаемоеЗначение = ОбщегоНазначенияИС.ИспользуетсяЧастичноеВыбытие();
...
Функция ОбработатьДанныеПоШтрихкодам(ДанныеПоШтрихкодам, ПараметрыСканирования, ЕстьОшибки, ФормаУникальныйИдентификатор, РезультатыПроверок) Экспорт
	...
	// Если требуется и уточнение планового количества и сопоставление номенклатуры,
	// то такие действия должны выполняться одновременно.
	ТребуетсяУточнениеКоэффициентовУпаковок = Ложь;
	...
	Для Каждого СтрокаТаблицы Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если СтрокаТаблицы.Входящий Тогда
			...
			ИначеЕсли СтрокаТаблицы.СтрокаДерева.Родитель = Неопределено
				Или ГрупповаяОбработкаШтрихкодовИС.ЭтоАгрегатТСД(СтрокаТаблицы.СтрокаДерева.Родитель.Штрихкод) Тогда
				ВходящийВерхнегоУровня = Истина;
			КонецЕсли;
		КонецЕсли;
		...
		Если СтрокаТаблицы.ТребуетсяУказаниеПлановогоКоличестваПотребительскихУпаковокПоGTIN
			Или СтрокаТаблицы.ТребуетсяСопоставление Тогда
			
			Если СтрокаТаблицы.ТребуетсяУказаниеПлановогоКоличестваПотребительскихУпаковокПоGTIN Тогда
				ТребуетсяУточнениеКоэффициентовУпаковок = Истина;
			КонецЕсли;
			
			ЕстьGTINВСоставеКода = СтрокаТаблицы.СоставКодаМаркировки <> Неопределено
				И СтрокаТаблицы.СоставКодаМаркировки.Свойство("GTIN")
				И СтрокаТаблицы.СоставКодаМаркировки.GTIN <> Неопределено;
			
			ЭтоGTIN02 = СтрокаТаблицы.СоставКодаМаркировки <> Неопределено
				И СтрокаТаблицы.СоставКодаМаркировки.Свойство("КоличествоВложенныхЕдиниц")
				И СтрокаТаблицы.СоставКодаМаркировки.КоличествоВложенныхЕдиниц <> Неопределено
				И ЕстьGTINВСоставеКода;
			ЭтоGTIN02 = ЭтоGTIN02 Или СтрокаТаблицы.ВидУпаковки = Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет;
			...
			Если НайденныеСтроки.Количество() = 0 Тогда
				...
				Если ЭтоGTIN02 Тогда
					
					ОписаниеGTIN.ВидУпаковки                       = Перечисления.ВидыУпаковокИС.ПустаяСсылка();
					ОписаниеGTIN.КоличествоПотребительскихУпаковок = 0;
			...
			Иначе
				...
				Если Не ЗначениеЗаполнено(ОписаниеGTIN.ВидУпаковки)
					И Не ЭтоGTIN02
					И ЗначениеЗаполнено(СтрокаТаблицы.ВидУпаковки) Тогда
					
					ОписаниеGTIN.ВидУпаковки                       = СтрокаТаблицы.ВидУпаковки;
					ОписаниеGTIN.КоличествоПотребительскихУпаковок = СтрокаТаблицы.КоличествоПотребительскихУпаковок;
	...
	Если Не ТребуетсяУточнениеКоэффициентовУпаковок
		И УточнениеКоэффициентовУпаковок.Количество() Тогда
		УточнениеКоэффициентовУпаковок = ИнициализироватьТаблицуУточненияКоэффициентовУпаковок();
	КонецЕсли;
	...
	ЭтоМаркировкаТоваровИСМП = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоМаркировкаТоваровИСМП(ПараметрыСканирования);
	
	Если ТребуетсяСопоставлениеНоменклатуры
		И Не ТребуетсяУточнениеКоэффициентовУпаковок
		И ДанныеПоШтрихкодам.ТребуетсяОбновлениеКлючаСессии
		И ОбщегоНазначенияИС.ЭтоРасширеннаяВерсияГосИС() Тогда
		ТребуетсяОбновлениеКлючаСессииПередСопоставлением = Истина;
	Иначе
		ТребуетсяОбновлениеКлючаСессииПередСопоставлением = Ложь;
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если Не ВходящийВерхнегоУровня Тогда
			// Сохранение полного кода при сканировании иерархиии упаковок в форме проверки и подбора
			// для сохранения кода вложенной упаковки (например, для передачи в СУЗ)
			Если ПараметрыСканирования.РасширеннаяВерсияГосИС
				И ДанныеПоШтрихкодам.ЭтоСканированиеВФормеПроверкиИПодбора
				И ПодсистемаИСМПСуществует
				И ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаТаблицы.ВидПродукции) Тогда
				МодульШтрихкодированиеИСМПСлужебный = ОбщегоНазначенияИС.ОбщийМодуль(
					"ШтрихкодированиеИСМПСлужебный", КэшОбщихМодулей);
				МодульШтрихкодированиеИСМПСлужебный.СохранениеКодаМаркировкиВПул(
					Неопределено, СтрокаТаблицы, ПараметрыСканирования);
			КонецЕсли;
			Продолжить;
		КонецЕсли;
		...
		ИначеЕсли ТребуетсяСопоставлениеНоменклатуры
			И Не ТребуетсяУточнениеКоэффициентовУпаковок
			И Не ТребуетсяОбновлениеКлючаСессииПередСопоставлением
			И Не ОбщаяОшибка Тогда
		...
		ИначеЕсли (ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМОТП(ВидПродукции)
			...
			// Например, для АТП и НСП шаблон потребительской упаковки идентичен шаблону групповой упаковки
			Или ((КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетГрупповыеУпаковки(ВидПродукции)
				Или КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетНаборы(ВидПродукции))
		...
		ИначеЕсли (ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская
				Или ВидУпаковки = Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет)
			И Не ОбщегоНазначенияИСПовтИсп.ЭтоМаркируемаяПродукцияЕГАИС(ВидПродукции)
			И Не ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура)
			И СтрокаТаблицы.СписокНоменклатуры.Количество() > 1
			И СписокНоменклатурыДляУточненияСодержитМаркируемуюПродукцию(СтрокаТаблицы.СписокНоменклатуры) Тогда
		...
		ИначеЕсли Не СтрокаТаблицы.Идентифицирован
			И Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(СтрокаТаблицы.ВидПродукции) Тогда
		...
		ИначеЕсли ДанныеПоШтрихкодам.ЭтоСканированиеВФормеПроверкиИПодбора Тогда
			...
			Если ДопустимаАлкогольнаяПродукция(ПараметрыСканирования)
				И (ОбщегоНазначенияИСПовтИсп.ЭтоМаркируемаяПродукцияЕГАИС(ВидПродукции)
		...
		ДополнитьРезультатОбработкиДополнительнымиПризнакамиРазрешительногоРежима(РезультатОбработки, ДанныеПоШтрихкодам, ДанныеШтрихкода, ПараметрыСканирования);
...
Процедура ЗаполнитьТребуетсяУказаниеПлановогоКоличестваПотребительскихУпаковокПоGTIN(ДанныеПоШтрихкодам, ПараметрыСканирования)
	...
	Если Не ДанныеПоШтрихкодам.ЗаполнитьФлагТребуетсяУказаниеПлановогоКоличестваПотребительскихУпаковокПоGTIN Тогда
		Возврат;
	ИначеЕсли ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоПроверкаКодовМаркировкиИСМП(ПараметрыСканирования) Тогда
		Возврат;
	ИначеЕсли ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоОтчетПроизводственнойЛинии(ПараметрыСканирования) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьНаДублиКодовМаркировки(ДанныеПоШтрихкодам, ПараметрыСканирования, ЕстьОшибки)
	...
	ЕстьСвойствоНайденВоВложенныхУпаковках = ПараметрыСканирования.РасширеннаяВерсияГосИС;
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		
		Если Не СтрокаДанных.Входящий
			Или (ЕстьСвойствоНайденВоВложенныхУпаковках И СтрокаДанных.НайденВоВложенныхУпаковках)
			Или (СтрокаДанных.ВидУпаковки <> Перечисления.ВидыУпаковокИС.Потребительская) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьПроверяемыеGTIN(ТаблицаНоменклатуры, ПроверяемыеGTIN, СоответствиеGTIN, ВключатьGTINОстатков = Истина, ИспользоватьХарактеристики = Истина) Экспорт
	...
	ОбновлениеИнформационнойБазыГосИС.ПроверитьОбработатьТаблицуНоменклатурыПриПереходеНаКонструкторНастроек(ТаблицаНоменклатуры);
...
Процедура ОбработатьШтрихкодыПоВскрытымПотребительскимУпаковкам(ДанныеПоШтрихкодам, ДанныеШтрихкодаСписок, ПараметрыСканирования, КешДанныхРазбора = Неопределено, ИсходныеДанныеКодовМаркировки = Неопределено)
	...
	ИначеЕсли КодыМаркировки.Количество() = 1 Тогда
		...
		ИсходныйШтрихкод                        = ДанныеШтрихкодаСписок[0].Штрихкод;
		...
		Если ДанныеПоШтрихкодам.ЗаменыШтрихкодов = Неопределено Тогда
			ДанныеПоШтрихкодам.ЗаменыШтрихкодов = Новый Соответствие;
		КонецЕсли;
		
		ДанныеПоШтрихкодам.ЗаменыШтрихкодов.Вставить(ИсходныйШтрихкод, ДанныеШтрихкодаСписок[0].Штрихкод);
...
Процедура ДополнитьРезультатОбработкиДополнительнымиПризнакамиРазрешительногоРежима(РезультатОбработкиШтрихкода, ДанныеПоШтрихкодам, ДанныеШтрихкода, ПараметрыСканирования)
	
	Если ДопустимаПродукцияИСМП(ПараметрыСканирования, Истина)
		И (ПрисутствуетТабачнаяПродукция(РезультатОбработкиШтрихкода.ВидыПродукции)
			Или ПрисутствуетПродукцияИСМП(РезультатОбработкиШтрихкода.ВидыПродукции)) Тогда
		
		МодульШтрихкодированиеИСМПСлужебный = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияИСМП");
		МодульШтрихкодированиеИСМПСлужебный.ДополнитьРезультатОбработкиДополнительнымиПризнакамиРазрешительногоРежима(РезультатОбработкиШтрихкода, ДанныеПоШтрихкодам, ДанныеШтрихкода);
...
Функция  ИнициализироватьПараметрыСопоставления()
	
	ПараметрыСопоставления = Новый Структура;
	ПараметрыСопоставления.Вставить("Штрихкод", "");
	ПараметрыСопоставления.Вставить("Количество", 1);
	ПараметрыСопоставления.Вставить("ПредставлениеНоменклатуры", "");
	ПараметрыСопоставления.Вставить("ВидПродукции");
	
	Возврат ПараметрыСопоставления;
	
КонецФункции

Функция ПриоритетВариантаРазбора(ДанныеРазбора)
	
	Если ДанныеРазбора.Свойство("СоставКодаМаркировки")
		И ДанныеРазбора.СоставКодаМаркировки.Свойство("ВключаетКриптоХвост") Тогда
		
		ВключаетКриптоХвост = ДанныеРазбора.СоставКодаМаркировки.ВключаетКриптоХвост;
		Если Не ТипЗнч(ВключаетКриптоХвост) = Тип("Булево") Тогда
			ВключаетКриптоХвост = Ложь;
		КонецЕсли;
