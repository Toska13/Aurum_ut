...
//  Массив из СправочникСсылка.ВидыПродукцииИС - используемые виды маркируемой продукции ИСМП
...
// Заполняет товарную группу документа по первой позиции табличной части Товары
//
// Параметры:
//   ДокументОбъект - ДокументОбъект - документ ГосИС с реквизитом "ТоварнаяГруппа" в шапке
//
Процедура ЗаполнитьТоварнуюГруппуПоТабличнойЧасти(ДокументОбъект) Экспорт
...
Процедура УдалениеНеиспользованныхКодовМаркировкиРегламентноеЗадание() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.УдалениеНеиспользованныхКодовМаркировкиИСМП);
...
#Область ОформлениеПоляВСД

// Настраивает условное оформление поля идентификатора происхождения ВЕТИС в табличной части формы.
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения - форма, для которой настраивается условное оформление
//   Префикс - Строка - префикс пути к данным табличной части, по умолчанию "Объект."
//   ИмяТаблицы - Строка - имя табличной части, по умолчанию "Товары"
Процедура УстановитьУсловноеОформлениеПоляВСД(Форма, Префикс = "Объект.", ИмяТаблицы = "Товары") Экспорт
	...
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы[ИмяТаблицы + "ИдентификаторПроисхожденияВЕТИС"].Имя);
	
	ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных(Префикс + ИмяТаблицы + ".ВидПродукцииВЕТИС");
	...
	ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных(Префикс + ИмяТаблицы + ".ИдентификаторПроисхожденияВЕТИС");
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.НеЗаполнено;
...
Процедура НастроитьВидимостьКолонкиКоличестваКодовМаркировки(Форма) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма.Объект, "ТоварнаяГруппа") Тогда
		ВидПродукцииИС = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(Форма.Объект.ТоварнаяГруппа);
	Иначе
		ВидПродукцииИС = Форма.Объект.ВидПродукции;
	КонецЕсли;
	
	Форма.Элементы.ТоварыКоличествоПотребительскихУпаковок.Видимость = ОбщегоНазначенияИС.ИспользуетсяПроизвольнаяЕдиницаУчета(ВидПродукцииИС);
...
// Длительная операция расчета проблем
// 
// Параметры:
//  ПараметрыФоновогоЗадания - Структура - Параметры запуска фонового задания
//  АдресРезультата - Строка - адрес возврата результата
Процедура ВыполнитьРасчетПроблем(ПараметрыФоновогоЗадания, АдресРезультата) Экспорт
	
	ВозвращаемоеЗначение = Новый Структура;
	ВозвращаемоеЗначение.Вставить("ТекстОшибки", "");
	ВозвращаемоеЗначение.Вставить("ЕстьИзменения", Ложь);
	
	Проблема = "НастройкаНоменклатурыПоТНВЭДиОКПД2";
	ТекущаяДата = НачалоДня(ТекущаяУниверсальнаяДата());
	ТребуетсяРасчет = Истина;
	
	ДанныеДоОбновления = Новый Структура;
	ДанныеДоОбновления.Вставить(Проблема, Ложь);
	
	Запрос = Новый Запрос;
	Запрос.Текст = СтрШаблон(
	"ВЫБРАТЬ
	|	Проблемы.Дата КАК Дата,
	|	Проблемы.ЕстьПроблема КАК ЕстьПроблема
	|ИЗ
	|	РегистрСведений.ПроблемыИСМП КАК Проблемы
	|ГДЕ
	|	Проблемы.Проблема = ""%1""",
	Проблема);
	
	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ПроблемыИСМП");
	...
	Попытка
		...
		Если Выборка.Следующий() Тогда
			...
			Иначе
				ДанныеДоОбновления[Проблема] = Выборка.ЕстьПроблема;
			КонецЕсли;
		КонецЕсли;
		
		Если ТребуетсяРасчет Тогда
			Запись = РегистрыСведений.ПроблемыИСМП.СоздатьМенеджерЗаписи();
			Запись.Проблема = Проблема;
			Запись.Дата = ТекущаяДата;
			Запись.ЕстьПроблема = ДанныеДоОбновления[Проблема];
			Запись.Записать();
		КонецЕсли;
	...
	Если ТребуетсяРасчет Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = ИнтеграцияИС.ТекстЗапросаТНВЭДиОКПД2Номенклатуры();
		РезультатЗапроса = Запрос.Выполнить();
		ЕстьПроблема = Не РезультатЗапроса.Пустой();
		Если ЕстьПроблема <> ДанныеДоОбновления[Проблема] Тогда
			Запись = РегистрыСведений.ПроблемыИСМП.СоздатьМенеджерЗаписи();
			Запись.Проблема = Проблема;
			Запись.Дата = ТекущаяДата;
			Запись.ЕстьПроблема = ЕстьПроблема;
			Запись.Записать();
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОчисткаПулаКодовМаркировкиРегламентноеЗадание", Истина);
	Методы.Вставить("УдалениеНеиспользованныхКодовМаркировкиРегламентноеЗадание", Истина);
	Методы.Вставить("ВыполнитьОбменРегламентноеЗадание", Истина);
	Методы.Вставить("ВыполнитьРасчетПроблем", Истина);
...
Функция ДоступнаРаботаСКодамиМаркировкиПивнойСлабоалкогольнойПродукции(Отказ = Неопределено) Экспорт
	...
	ВидыПродукцииВнутреннегоПеремещенияМеждуМОД = КонструкторИСМПКлиентСервер.ВидыПродукцииВнутреннегоПеремещенияМеждуМОД();
...
//Получает товарные группы, имеющиеся в таблице номенклатуры
//
//Параметры:
...
//Возвращаемое значение:
//   Массив Из СправочникСсылка.ТоварныеГруппыГИСМТ - товарные группы по переданной номенклатуре
//
Функция ТоварныеГруппыПоТаблицеНоменклатуры(Товары, ПараметрыЗаполненияПоКодамОстатков = Неопределено) Экспорт
	...
	Иначе
		...
		|	ЕСТЬNULL(КэшОписанияОстатковИСМП.ТоварнаяГруппа, Неопределено) КАК ТоварнаяГруппа
		...
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДанныеПоВидуПродукции.ВидПродукции КАК ВидПродукции,
		|	НЕОПРЕДЕЛЕНО                       КАК ТоварнаяГруппа
		|ИЗ
		|	СвойстваМаркируемойПродукции КАК ДанныеПоВидуПродукции
		|ГДЕ 
		...
		|ВЫБРАТЬ
		|	НЕОПРЕДЕЛЕНО,
		|	ДанныеОстаточныхGTIN.ТоварнаяГруппа
		|ИЗ
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРезультат Цикл
		Если ЗначениеЗаполнено(СтрокаТаблицы.ВидПродукции) Тогда
			ТоварныеГруппы = КонструкторИСОбщегоНазначенияКлиентСервер.ЗначенияЭлементаКонструктораПоВидуПродукции(
				Справочники.ЭлементыКонструктораВидовПродукцииИС.ТоварныеГруппыГИСМТ, СтрокаТаблицы.ВидПродукции);
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Результат, ТоварныеГруппы, Истина);
		ИначеЕсли ЗначениеЗаполнено(СтрокаТаблицы.ТоварнаяГруппа)
			И Результат.Найти(СтрокаТаблицы.ТоварнаяГруппа) = Неопределено Тогда
			Результат.Добавить(СтрокаТаблицы.ТоварнаяГруппа);
		КонецЕсли;
	КонецЦикла;
...
// Проверка заполнения колонки "Идентификатор происхождения ВетИС".
// 
// Параметры:
//  ДокументОбъект - ДокументОбъект - Проверямый объект документа.
//  Отказ          - Булево         - Признак ошибки заполнения документа.
//  Номенклатура   - ОпределяемыйТип.Номенклатура, Неопределено - Номенклатура.
//  ВыводитьСообщениеОбОшибке - Булево - .
Процедура ПроверкаЗаполненияИдентификатораПроисхожденияВЕТИС(ДокументОбъект, Отказ, Номенклатура = Неопределено, ВыводитьСообщениеОбОшибке = Истина) Экспорт
	
	ИспользуетсяПодсистемаВетИС = ИнтеграцияИСМПВЕТИС.ИспользуетсяПодсистемаВетИС();
	ЭтоСырьеГИСМТ = КонструкторИСМПКлиентСервер.ЭтоСырьеГИСМТ(ДокументОбъект.ТоварнаяГруппа);
	...
	Если Номенклатура = Неопределено Тогда
		Если ИспользуетсяПодсистемаВетИС Тогда
			ШаблонСообщения = НСтр("ru = 'Не заполнена колонка ""%1"" в строке %2 таблицы ""Товары""'");
		Иначе
			ШаблонСообщения = НСтр("ru = 'Требуется указание ""%1"" в строке %2 таблицы ""Товары"", включите ""Интеграцию с ВетИС'");
		КонецЕсли;
	Иначе
	...
	Запрос.Текст = 
		...
		|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииВЕТИС КАК ВидПродукцииВЕТИС
		|ИЗ
		|	РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		|ГДЕ
		|	ОписаниеНоменклатурыИС.Номенклатура В (&Номенклатура)
		|	И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииВЕТИС <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)";
	
	Если ИспользуетсяПодсистемаВетИС Или ЭтоСырьеГИСМТ Тогда
		ИмяРеквизита = "ИдентификаторПроисхожденияВЕТИС";
	Иначе
		ИмяРеквизита = "ИдентификаторПроисхожденияВЕТИССтрокой";
	КонецЕсли;
	...
	Если Номенклатура = Неопределено Тогда
		...
		Для Каждого СтрокаТовары Из ДокументОбъект.Товары Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТовары[ИмяРеквизита])
				Или Не ЗначениеЗаполнено(СтрокаТовары.Номенклатура) Тогда
				Продолжить;
			КонецЕсли;
			...
			Если НайденныеСтроки.Количество() Тогда
				ТекстСообщения = СтрШаблон(ШаблонСообщения, ЗаголовокКолонкиВЕТИС, СтрокаТовары.НомерСтроки);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,
					ДокументОбъект,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
						"Товары", СтрокаТовары.НомерСтроки, ИмяРеквизита),,
					Отказ);
			КонецЕсли;
	...
	Иначе
		...
		Если Не Результат.Пустой() Тогда
			...
			Если Не ЗначениеЗаполнено(ДокументОбъект[ИмяРеквизита]) Тогда
				ТекстСообщения = СтрШаблон(ШаблонСообщения, ЗаголовокКолонкиВЕТИС);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,
					ДокументОбъект,
					ИмяРеквизита,,
					Отказ);
...
// Проверка заполнения кодов единиц измерения номенклатуры для контура Сырья
// 
// Параметры:
//  ДокументОбъект  - ДокументОбъект - Проверямый объект документа.
//  ОперацияПричина - ПеречислениеСсылка.ВидыОперацийИСМП - операция.
//  Отказ           - Булево         - Признак ошибки заполнения документа.
Процедура ПроверкаЗаполненияДанныхФармацевтическогоСырья(ДокументОбъект, ОперацияПричина, Отказ) Экспорт
	...
	|	Источник.Упаковка     КАК Упаковка,
	...
	|	Товары.Сырье.ВидПродукции КАК ВидПродукции,
	...
	ИзменитьТекстЗапросаКодЕдиницыИзмерения(Запрос.Текст);
	
	Запрос.УстановитьПараметр("Источник",           ДокументОбъект.Товары.Выгрузить());
	Запрос.УстановитьПараметр("ВидыПродукцииСырье", КонструкторИСМПКлиентСервер.ВидыПродукцииСырьеГИСМТ());
	
	Товары = Запрос.Выполнить().Выгрузить();
	
	ШаблонСообщенияКодЕдиницы = НСтр("ru = 'В строке %1 таблицы ""Товары"" код единицы измерения по ОКЕИ ""%2"" для номенклатуры ""%3"" не поддерживается в системе прослеживаемости фармацевтического сырья'");
	ШаблонСообщенияТипСырья   = НСтр("ru = 'В строке %1 таблицы ""Товары"" не заполнено поле ""Вид продукции"" для фармацевтического сырья ""%2""'");
	
	Для Каждого СтрокаТовары Из Товары Цикл
		
		КодОКЕИ = СокрЛП(СтрокаТовары.ЕдиницаИзмеренияКод);
		Если Не КонструкторИСМПКлиентСервер.СырьеПоддерживаетКодОКЕИ(
			СтрокаТовары.ВидПродукцииГИСМТ, КодОКЕИ, ОперацияПричина) Тогда
		
			ТекстСообщения = СтрШаблон(
				ШаблонСообщенияКодЕдиницы,
		...
		Если Не ЗначениеЗаполнено(СтрокаТовары.ВидПродукции) Тогда
			ТекстСообщения = СтрШаблон(
				ШаблонСообщенияТипСырья,
				СтрокаТовары.НомерСтроки,
				СтрокаТовары.Сырье);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,
				ДокументОбъект,
				ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
					"Товары", СтрокаТовары.НомерСтроки, "Сырье"),,
				Отказ);
		КонецЕсли;
...
Процедура ИзменитьТекстЗапросаКодЕдиницыИзмерения(ТекстЗапроса, ПутьКРеквизитуНоменклатура = "Товары.Номенклатура", ПутьКРеквизитуЕдиницыИзмерения = "Товары.Упаковка") Экспорт
	
	Если КонструкторИСМПКлиентСервер.ВидыПродукцииСырьеГИСМТ().Количество() = 0 Тогда
	...
	ИнтеграцияИСМППереопределяемый.ПриОпределенииТекстаЗапросаЗначенияРеквизитаЕдиницыИзмерения(
		ТекстЗамены,
		ИмяРеквизита,
		ПутьКРеквизитуНоменклатура,
		ПутьКРеквизитуЕдиницыИзмерения);
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстЗапросаКодЕдиницыИзмерения", ТекстЗамены);
	
КонецПроцедуры

Процедура ЗаполнитьСлужебныеРеквизитыСырье(ВходящиеДанные) Экспорт
	
	ТаблицаДанных = Новый ТаблицаЗначений;
	ТаблицаДанных.Колонки.Добавить("ИндексСтроки",   ОбщегоНазначения.ОписаниеТипаЧисло(10));
	ТаблицаДанных.Колонки.Добавить("Сырье",          Новый ОписаниеТипов("СправочникСсылка.КлассификаторСырьяГИСМТ"));
	...
	Если ТипВходящихДанных = Тип("ДанныеФормыКоллекция")
		...
		Для Каждого СтрокаДанных Из ВходящиеДанные Цикл
			Если Не ЗначениеЗаполнено(СтрокаДанных.Сырье) Тогда
	...
	ИначеЕсли ТипВходящихДанных = Тип("ДанныеФормыЭлементКоллекции")
		...
		Если Не ЗначениеЗаполнено(ВходящиеДанные.Сырье) Тогда
			Возврат;
		КонецЕсли;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(КлассификаторСырьяГИСМТ.ВидПродукции, НЕОПРЕДЕЛЕНО) КАК ВидПродукцииСырья,
	|	ЕСТЬNULL(КлассификаторСырьяГИСМТ.Идентификатор, """") КАК Идентификатор,
	|	Товары.ИндексСтроки КАК ИндексСтроки
	|ИЗ
	|	Товары КАК Товары
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлассификаторСырьяГИСМТ КАК КлассификаторСырьяГИСМТ
	|			ПО КлассификаторСырьяГИСМТ.Ссылка = Товары.Сырье
	|";
	
	ВыборкаПоСтроке = Запрос.Выполнить().Выбрать();
	Пока ВыборкаПоСтроке.Следующий() Цикл
		...
		ЗаполнитьЗначенияСвойств(Данные, ВыборкаПоСтроке);
		
	КонецЦикла;
	
КонецПроцедуры

Функция ЗаполнитьТипСырьяПоОстаткамНаСервере(ДокументОбъект, КОпределениюТипаСырья = Неопределено) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ТекстОшибки",                         "");
	Результат.Вставить("ТребуетсяОбновлениеКлючаСессии",      Ложь);
	
	Если КОпределениюТипаСырья = Неопределено Тогда
		
		РезультатОбработки = ИдентификаторыСырьяКОпределениюТипа(ДокументОбъект);
		Если РезультатОбработки.ИдентификаторыСырья.Количество() = 0 Тогда
			Результат.ТекстОшибки = Нстр("ru = 'Заполнение ""Вида продукции"" в подобранном сырье не требуется.'");
			Возврат Результат;
		КонецЕсли;
		
		КОпределениюТипаСырья = РезультатОбработки.ИдентификаторыСырья;
		
	КонецЕсли;
	
	ПараметрыПоиска = Новый Структура;
	ПараметрыПоиска.Вставить("Идентификаторы", КОпределениюТипаСырья);
	
	РезультатЗапроса = ИнтеграцияИСМПСлужебный.СырьеПоОстаткамНаСкладе(ДокументОбъект.Организация, ПараметрыПоиска);
	...
	Справочники.КлассификаторСырьяГИСМТ.СоздатьОбновитьСырьеПоДаннымСервиса(РезультатЗапроса.Сырье);
	ЗаполнитьСлужебныеРеквизитыСырье(ДокументОбъект.Товары);
...
Функция ОписаниеОформленныхДокументов(МетаданныеДокумента, ДокументОснование, ДокументыПоОснованию, СтатусыОформления) Экспорт
	...
	Описание.Вставить("ПолноеИмяМетаданных", МетаданныеДокумента.ПолноеИмя());
	Описание.Вставить("ИмяКоманды",          ИмяКоманды);
...
Функция НастройкиОбменаСУЗ(Организации = Неопределено, ДатыПоследнегоЗапуска = Неопределено) Экспорт
	...
	|	
	|	ЗНАЧЕНИЕ(Перечисление.ВерсииФорматаОбменаСУЗ.V3) КАК ФорматОбмена,
	|	
	|	ВЫБОР
...
// Описание данных карточки номенклатуры.
// 
// Возвращаемое значение:
//  Структура - Описание данных карточки номенклатуры:
// * КлючВидовПродукцииИС - Неопределено, СправочникСсылка.КлючиВидовПродукцииИС -
// * ВидУпаковкиИС - Неопределено, ПеречислениеСсылка.ВидыУпаковокИС -
// * Наименование - Неопределено, Строка - наименование номенклатуры по данным сервиса 
// * ТоварСПеременнымВесом - Неопределено, Булево - признак переменного веса
// * ВесВГраммах - Неопределено, Число - вес потребительской упаковки в граммах
// * ОбъемВМиллитрах - Неопределено, Число - объем потребительской упаковки в миллилитрах
// * КодАлкогольнойПродукции - Неопределено, Строка - код алкогольной продукции
// * ПодлежитВетеринарномуКонтролю - Неопределено, Строка - код алкогольной продукции
// * ДополнительныеШтрихкоды - Массив из Структура - дополнительные штрихкоды номенклатуры:
//        *** ШтрихкодEAN - Строка - дополнительный штрихкод номенклатуры (упаковки)
//        *** Коэффициент - Число - коэффициент по GTIN (упаковки)
Функция ОписаниеДанныхКарточкиНоменклатуры() Экспорт
	
	ВозвращаемоеЗначение = Новый Структура();
	ВозвращаемоеЗначение.Вставить("КлючВидовПродукцииИС",    Неопределено);
	ВозвращаемоеЗначение.Вставить("ВидУпаковкиИС",           Неопределено);
	ВозвращаемоеЗначение.Вставить("Наименование",            Неопределено);
	ВозвращаемоеЗначение.Вставить("ТоварСПеременнымВесом",   Неопределено);
	ВозвращаемоеЗначение.Вставить("ВесВГраммах",             Неопределено);
	ВозвращаемоеЗначение.Вставить("ОбъемВМиллитрах",         Неопределено);
	ВозвращаемоеЗначение.Вставить("ОКПД2",                   Неопределено);
	ВозвращаемоеЗначение.Вставить("КодАлкогольнойПродукции", Неопределено);
	ВозвращаемоеЗначение.Вставить("ДополнительныеШтрихкоды", Новый Массив);
	ВозвращаемоеЗначение.Вставить("ПодлежитВетеринарномуКонтролю", Неопределено);
	
	Возврат ВозвращаемоеЗначение;
	
КонецФункции

// Описание результата создания номенклатуры.
// 
// Возвращаемое значение:
//  Структура - Описание результата создания номенклатуры:
// * ТекстОшибки - Строка - 
// * ТребуетсяОбновлениеКлючаСессии - Булево -
// * ТребуетсяПовторОперации - Булево - 
// * Результат - Соответствие из КлючИЗначение:
//   ** Ключ - Строка - Штрихкод EAN
//   ** Значение - Структура:
//     *** Номенклатура - ОпределяемыйТип.Номенклатура - ссылка на созданную номенклатуру
//     *** Характеристика - ОпределяемыйТип.ХарактеристикаНоменклатуры - ссылка на созданную характеристику
//     *** Отказ - Булево -
...
Функция ОписаниеРезультатаСозданияНоменклатуры() Экспорт
	...
	ВозвращаемоеЗначение.Вставить("Результат",                      Новый Соответствие());
	ВозвращаемоеЗначение.Вставить("Ошибки",                         Новый Соответствие());
...
// 	 * ВключатьGTINНаОстатки - Булево - Определяет необходимость включения в состав остаточных GTIN.
// Возвращаемое значение:
//  ТаблицаЗначений - ПроверкаИПодборПродукцииИС.ПустаяТаблицаОписанияGTIN.
Функция ДанныеДляУточненияКоэффициентовУпаковок(ВходящиеДанные, ИспользоватьХарактеристики = Истина, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ДанныеДляУточнения = ПроверкаИПодборПродукцииИС.ПустаяТаблицаОписанияGTIN();
	...
	Для Каждого GTIN Из ПроверяемыеGTIN Цикл
		...
		СтрокаДанных.ВидПродукции                       = ДанныеОписанияНоменклатуры.КлючВидовПродукции;
...
Функция ПолучитьДанныеСопоставленногоКлассификатораОКПД2(КодОКПД2, Наименование) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("НаименованиеПолное");
	Результат.Вставить("ЭлементСправочника");
	ИнтеграцияИСМППереопределяемый.ПриОпределенииСопоставленногоКлассификатораОКПД2(КодОКПД2, Результат, Наименование);
	Возврат Результат;
...
Процедура ЗаполнитьДоступныеВидыДокументовСертификации(Элемент, ВидПродукции) Экспорт
	...
	Если КонструкторИСМПКлиентСервер.ВидПродукцииИспользуетСертификатСоответствия(ВидПродукции) Тогда
		Элемент.СписокВыбора.Добавить(Перечисления.ВидыДокументовОбязательнойСертификацииИС.СертификатСоответствия);
	КонецЕсли;
	
	Если КонструкторИСМПКлиентСервер.ВидПродукцииИспользуетСвидетельствоОГосударственнойРегистрации(ВидПродукции) Тогда
	...
	Если КонструкторИСМПКлиентСервер.ВидПродукцииИспользуетДекларациюСоответствия(ВидПродукции) Тогда
	...
	Если КонструкторИСМПКлиентСервер.ВидПродукцииИспользуетВетеринарныйСертификат(ВидПродукции) Тогда
		Элемент.СписокВыбора.Добавить(Перечисления.ВидыДокументовОбязательнойСертификацииИС.ВетеринарныйСертификат);
	КонецЕсли;
...
Функция СостояниеОбмена(Организации) Экспорт
	...
	|	И НЕ РезультатыОбработкиДокументовИСМП.ЗавершеноПринудительно
	|ИМЕЮЩИЕ
	...
	|	РегистрСведений.ПроблемыИСМП КАК Проблемы
	|ГДЕ
	|	Проблемы.Проблема = ""НастройкаНоменклатурыПоТНВЭДиОКПД2""
	|	И Проблемы.ЕстьПроблема",
	"НастройкаНоменклатурыПоТНВЭДиОКПД2");
	
	// Настройте удаление кодов маркировки с истекшим сроком использования
	
	ТекстыЗапроса.Добавить(
	"ВЫБРАТЬ
	|	1 КАК УдалениеНеиспользованныхКодовМаркировкиИСМП
	|ПОМЕСТИТЬ ВТ_УдалениеНеиспользованныхКодовМаркировкиИСМП
	|ГДЕ
	|	&УдалениеНеиспользованныхКМ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_УдалениеНеиспользованныхКодовМаркировкиИСМП.УдалениеНеиспользованныхКодовМаркировкиИСМП
	|ИЗ
	|	ВТ_УдалениеНеиспользованныхКодовМаркировкиИСМП КАК ВТ_УдалениеНеиспользованныхКодовМаркировкиИСМП",
	"НастроитьУдалениеНеиспользованныхКодовМаркировки");
	...
	УдалениеНеиспользованныхКМ        = ОбменВключен("УдалениеНеиспользованныхКодовМаркировкиИСМП");
	...
	Запрос.УстановитьПараметр("УдалениеНеиспользованныхКМ",        УдалениеНеиспользованныхКМ);
...
// Это входящий электронный документ.
// 
// Параметры:
//  Документ - ДокументСсылка
// 
// Возвращаемое значение:
//  Булево - Это входящий электронный документ
Функция ЭтоВходящийЭлектронныйДокумент(Документ) Экспорт
	
	ЭтоВходящийЭД = Ложь;
	
	ИнтеграцияИСМППереопределяемый.ПриОпределенииВходящегоЭлектронногоДокумента(Документ, ЭтоВходящийЭД);
	
	Возврат ЭтоВходящийЭД;
	
КонецФункции

// Это исходящий электронный документ.
// 
// Параметры:
//  Документ - ДокументСсылка
// 
// Возвращаемое значение:
//  Булево - Это исходящий электронный документ
Функция ЭтоИсходящийЭлектронныйДокумент(Документ) Экспорт
	
	ЭтоИсходящийЭД = Ложь;
	
	ИнтеграцияИСМППереопределяемый.ПриОпределенииИсходящегоЭлектронногоДокумента(Документ, ЭтоИсходящийЭД);
	
	Возврат ЭтоИсходящийЭД;
	
КонецФункции

Процедура ОбновитьЗаписьПроблем(ЕстьПроблема, Проблема) Экспорт
	
	Запись = РегистрыСведений.ПроблемыИСМП.СоздатьМенеджерЗаписи();
	Запись.Проблема = Проблема;
	Запись.Прочитать();
	Если Запись.Выбран()
		И Запись.ЕстьПроблема = ЕстьПроблема Тогда
		Запись = Неопределено;
		Возврат;
	КонецЕсли;
	Запись.Дата = НачалоДня(ТекущаяУниверсальнаяДата());
	Запись.ЕстьПроблема = ЕстьПроблема;
	Запись.Записать();
	
КонецПроцедуры

Процедура УстановитьУсловноеОформлениеПоляСырье(Форма, ИмяЭлемента = "ТоварыСырье", ИмяТЧ = "Товары") Экспорт
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ИмяТЧ + "Сырье");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ИмяТЧ + "ВидПродукцииСырья");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
...
Функция ИдентификаторыСырьяКОпределениюТипа(ДокументОбъект)
	
	Результат = Новый Структура;
	Результат.Вставить("ИдентификаторыСырья", Новый Массив);
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	КлассификаторСырьяГИСМТ.Идентификатор КАК Идентификатор
	|ИЗ
	|	Справочник.КлассификаторСырьяГИСМТ КАК КлассификаторСырьяГИСМТ
	|ГДЕ
	|	КлассификаторСырьяГИСМТ.Ссылка В (&Сырье)
	|	И КлассификаторСырьяГИСМТ.ВидПродукции = ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
	|";
	
	Сырье = Новый Массив;
	Для Каждого СтрокаТовары Из ДокументОбъект.Товары Цикл
		Если ЗначениеЗаполнено(СтрокаТовары.Сырье) Тогда
			Сырье.Добавить(СтрокаТовары.Сырье);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СтрокаТаблицы Из РезультатЗапроса Цикл
		Результат.ИдентификаторыСырья.Добавить(СтрокаТаблицы.Идентификатор);
	КонецЦикла;
