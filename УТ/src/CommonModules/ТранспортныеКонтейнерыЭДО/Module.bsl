...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("РаспаковатьКонтейнеры", Истина);
...
Функция ИзменитьСтатусКонтейнера(ТранспортныйКонтейнер, Статус, КонтекстДиагностики = Неопределено, 
	...
	НачатьТранзакцию();
	Попытка
		ЗаблокироватьКонтейнер(ТранспортныйКонтейнер);
		
		КонтейнерОбъект = ТранспортныйКонтейнер.ПолучитьОбъект();
...
// Параметры:
//  Отбор - См. НовыйОтборКонтейнеровОбъектов
//  ИмяВременнойТаблицы - Строка
// 
// Возвращаемое значение:
//  Структура - См. ОбщегоНазначенияБЭД.НовоеОписаниеЗапроса
Функция ЗапросКонтейнеровОбъектовДляОтправки(Отбор, ИмяВременнойТаблицы) Экспорт
	...
	Если Отбор.СпособОбмена <> "" Тогда
		ПоляУсловия.Добавить(СтрШаблон("ТранспортныйКонтейнер.СпособОбменаЭД В(%1)", Отбор.СпособОбмена));
	КонецЕсли;
...
// Возвращаемое значение:
//  Структура - Новый отбор контейнеров объектов:
// * Объект - Строка
// * СпособОбмена - Строка
...
//  ВнешнийУИД - Строка
//             - Неопределено
Процедура ЗаблокироватьКонтейнер(Контейнер, Знач ВнешнийУИД = Неопределено) Экспорт
	
	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить("Документ.ТранспортныйКонтейнерЭДО");
	ЭлементБлокировки.УстановитьЗначение("Ссылка", Контейнер);
	Если ВнешнийУИД = Неопределено Тогда
		ВнешнийУИД = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контейнер, "ВнешнийУИД");
	КонецЕсли;
	ЭлементБлокировки.УстановитьЗначение("ВнешнийУИД", ВнешнийУИД);
...
//  ЗначениеРеквизитаВнешнийУИД - Строка
//                              - Неопределено
//  ТранспортныйКонтейнерЗаписан - Булево - Возвращаемый параметр, Истина в случае если была запись.
// 
// Возвращаемое значение:
//  ДокументСсылка.ТранспортныйКонтейнерЭДО - сформированный контейнер
Функция ЗаполнитьТранспортныйКонтейнер(ПараметрыКонтейнера, Знач ТранспортныйКонтейнер = Неопределено,
	ЗначениеРеквизитаВнешнийУИД = Неопределено, ТранспортныйКонтейнерЗаписан = Ложь) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	НачатьТранзакцию();
	Попытка
		Если ТранспортныйКонтейнер = Неопределено Тогда
			ЗаблокироватьКонтейнерПоВнешнемуУИД(ПараметрыКонтейнера.ВнешнийУИД);
			...
			КонецЕсли;
		Иначе
			ЗаблокироватьКонтейнер(ТранспортныйКонтейнер, ЗначениеРеквизитаВнешнийУИД);
		...
		Если Записывать Тогда
			...
			ТранспортныйКонтейнерЗаписан = Истина;
		КонецЕсли;
		ЗафиксироватьТранзакцию();
	Исключение
		ОтменитьТранзакцию();
...
Процедура ПриЗагрузкеОбъекта(Объект, ДанныеОбъекта, КонтекстДиагностики) Экспорт
	...
	Иначе
		...
		ДанныеПриглашения = ПриглашенияЭДО.ДанныеПриглашения(КлючПриглашения, ЗаполнятьПредставлениеКонтрагента);
	...
	ЗаблокироватьКонтейнер(ДанныеОбъекта.ТранспортныйКонтейнер);
	
	КонтейнерОбъект = ДанныеОбъекта.ТранспортныйКонтейнер.ПолучитьОбъект();
...
Процедура ЗаполнитьПараметрыКонтейнера(ПараметрыКонтейнера, ДвоичныеДанные, РезультатРаспаковки, КонтекстДиагностики,
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		ДанныеУчетнойЗаписи = УчетныеЗаписиЭДО.ДанныеУчетнойЗаписи(ПараметрыКонтейнера.Получатель, КэшЦиклов);
...
Функция СформироватьДанныеОбъектов(ТранспортныеКонтейнеры, КонтекстДиагностики = Неопределено)
	...
	ТипыДокументовТранспортнойИнформации = ТранспортныеКонтейнерыЭДОСлужебный.ТипыДокументовТранспортнойИнформации();
	...
	Для каждого ТранспортныйКонтейнер Из ТранспортныеКонтейнеры Цикл
		...
		Попытка
			...
			ЗаполнитьКоличествоКонтейнеровВПакете(
				ЗначенияРеквизитовКонтейнера.ИдентификаторПакета,
				КоличествоКонтейнеровВПакетахВсего);
			...
			Если ТребуетсяПроверкаЗаполненияКонтейнера
				...
				ТранспортныйКонтейнерЗаписан = Ложь;
				ЗаполнитьТранспортныйКонтейнер(
					ПараметрыКонтейнера,
					ТранспортныйКонтейнер,
					ЗначенияРеквизитовКонтейнера.ВнешнийУИД,
					ТранспортныйКонтейнерЗаписан);
				Если ТранспортныйКонтейнерЗаписан Тогда
			...
			Для Каждого Документ Из МетаданныеКонтейнера.Документы Цикл
				...
				ПараметрыДокумента = ТранспортныеКонтейнерыЭДОСлужебный.ПараметрыДокумента(
					...
					ДанныеОбъекта.ТипРегламента,
					Документ.КодТранзакции,
					Ложь,
					ТипыДокументовТранспортнойИнформации);
				...
				ДанныеОбъекта.ОписаниеДокумента.ДатаДокумента = ДанныеВходящегоОбъекта.Карточка.ДатаДокумента;
			...
			ЗаполнитьКоличествоКонтейнеровВПакете(
				ЗначенияРеквизитовКонтейнера.ИдентификаторПакета,
				КоличествоКонтейнеровВПакетахКРаспаковке);
		Исключение
			ТекстСообщения = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
...
//
Процедура ЗаполнитьКоличествоКонтейнеровВПакете(ИдентификаторПакета, КоличествоКонтейнеровВПакетах) 
...
// Параметры:
//  ВнешнийУИД - Строка
Процедура ЗаблокироватьКонтейнерПоВнешнемуУИД(ВнешнийУИД)
