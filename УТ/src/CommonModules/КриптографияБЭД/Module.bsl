...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПрограммыЭлектроннойПодписиИШифрованияНачальноеЗаполнение");
...
Функция Расшифровать(Данные, Пароль, КонтекстДиагностики, Программа = Неопределено) Экспорт
	...
	КлючЗамера = "КриптографияБЭД.Расшифровать.Сервер";
	...
	Если МенеджерКриптографии = Неопределено Тогда
		...
		ОбщегоНазначенияБЭД.ЗакончитьЗамер(КлючЗамера, ОписаниеЗамера, 0, КраткоеПредставлениеОшибки);
...
Функция Подписать(ОписаниеДанных, КонтекстДиагностики, ПаролиСертификатов = Неопределено, МенеджерКриптографии = Неопределено) Экспорт
	КлючЗамера = "КриптографияБЭД.Подписать.Сервер.Базовая";
	ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер(КлючЗамера);
	...
	КонецЦикла;
	ОбщегоНазначенияБЭД.ЗакончитьЗамер(КлючЗамера, ОписаниеЗамера,
		ОписаниеДанных.НаборДанных.Количество());
...
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
//  ИспользоватьДополнительныеПараметрыПроверки - Булево - определяет необходимость заполнения словаря дополнительных
//   параметров проверки подписи через сервис DSS.
// Возвращаемое значение:
// 	см. КриптографияБЭДКлиентСервер.НовыйРезультатПроверкиПодписи
Функция ПроверитьПодпись(МенеджерКриптографии, ИсходныеДанные, Подпись, КонтекстДиагностики, 
	ИспользоватьДополнительныеПараметрыПроверки = Истина) Экспорт
	
	Если ДоступнаПроверкаПодписиВОблаке() Тогда
		КлючЗамера = "КриптографияБЭД.ПроверитьПодпись.DSS";
		ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер(КлючЗамера);
		РезультатПроверки = ПроверитьПодписьЧерезСервисDSS(ИсходныеДанные, Подпись, КонтекстДиагностики,, 
			ИспользоватьДополнительныеПараметрыПроверки);
	Иначе
		КлючЗамера = "КриптографияБЭД.ПроверитьПодпись.Сервер";
		ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер(КлючЗамера);
...
//
// Возвращаемое значение:
//  см. ЭлектроннаяПодпись.ПодписиОбъекта
...
// Возвращаемое значение:
// 	Булево
Функция ПриПроверкеПодписиВозниклиОшибки(Подпись) Экспорт
	...
	ПроверкаМЧДВыполненаУспешно = Ложь;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСКонтрагентами") Тогда
		...
		Если Подпись.РезультатПроверкиПоМЧД <> Неопределено
			...
			ПроверкаМЧД = Подпись.РезультатПроверкиПоМЧД.ПротоколПроверки.ПроверкаМЧД;
			
			ПроверкаПолномочийВыполненаУспешно = 
				МодульМашиночитаемыеДоверенности.ПроверкаПолномочийВыполненаУспешно(ПроверкаМЧД);
					
			ПроверкаМЧДВыполненаУспешно = 
				МодульМашиночитаемыеДоверенности.ПроверкаМЧДВыполненаУспешно(ПроверкаМЧД);
	...
	Если Результат И РежимСтрогойПроверкиДоверенностейОтключен Тогда
		Если ПроверкаМЧДВыполненаУспешно 
			И (СертификатОтозванИлиИстек Или Не ПроверкаПолномочийВыполненаУспешно) Тогда
			Результат = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция ПараметрыДляЗаполненияШтампа(Подпись)
	...
	Иначе
		ЗначенияПараметров.Вставить("ОрганизацияПодписант", СвойстваПодписи.Владелец);
	КонецЕсли;
...
//  ИспользоватьДополнительныеПараметрыПроверки - Булево - определяет необходимость заполнения словаря дополнительных
//   параметров проверки подписи (VerifyParams).
// Возвращаемое значение:
// 	см. КриптографияБЭДКлиентСервер.НовыйРезультатПроверкиПодписи
Функция ПроверитьПодписьЧерезСервисDSS(ИсходныеДанные, Подпись, КонтекстДиагностики, 
		НастройкиСервисаОблачнойПодписи = Неопределено, ИспользоватьДополнительныеПараметрыПроверки = Истина)
	...
	Если ЗначениеЗаполнено(НастройкиСервисаОблачнойПодписи) Тогда
		...
		СвойстваПроверкиПодписи = Неопределено;
		Если ИспользоватьДополнительныеПараметрыПроверки Тогда
