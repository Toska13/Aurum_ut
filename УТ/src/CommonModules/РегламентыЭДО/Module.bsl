...
// Проверяет статус хранения информации отправителя
// 
// Параметры:
...
//  Булево
//
Функция ИнформацияОтправителяНаХранении(ЭлементыРегламента) Экспорт
	
	СтрокаТаблицы = ЭлементыРегламента.Найти(
		Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияОтправителя, "ТипЭлементаРегламента");
	
	Если СтрокаТаблицы <> Неопределено И СтрокаТаблицы.Состояние = Перечисления.СостоянияСообщенийЭДО.Хранение Тогда
		Возврат Истина;
	КонецЕсли;
...
// Возвращает признак однотитульного формализованного документа.
// 
// Параметры:
//  ПараметрыДокумента - См. РегламентыЭДО.НовыеПараметрыДокументаДляОпределенияСостояния
//  				   - СтрокаТаблицыЗначений из см. СинхронизацияЭДО.НовыеДанныеОбъектов
//  				   - ВыборкаИзРезультатаЗапроса
//
// Возвращаемое значение:
//  Булево
//
Функция ДокументБезОтветногоТитула(ПараметрыДокумента) Экспорт
	
	Если ПараметрыДокумента.ТипРегламента <> Перечисления.ТипыРегламентовЭДО.Формализованный Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат РегламентыЭДО_Формализованный.ДокументБезОтветногоТитула(ПараметрыДокумента);
	
КонецФункции

// Возвращает признак того, что входящий электронный документ может потребовать заполнения ответного титула
//
// Параметры:
...
// Возвращаемое значение:
//  Булево
Функция ТребуетсяЗаполненияОтветногоТитула(СостояниеДокумента, ТипРегламента, ТипДокумента) Экспорт
	
	ТипыДокументовИсключений = Новый Массив(); // массив из ПеречислениеСсылка.ТипыДокументовЭДО
	ТипыДокументовИсключений.Добавить(Перечисления.ТипыДокументовЭДО.СведенияОРеализацииКомиссионером);
	ТипыДокументовИсключений.Добавить(Перечисления.ТипыДокументовЭДО.СведенияОЗакупкеКомиссионером);
	ТипыДокументовИсключений.Добавить(Перечисления.ТипыДокументовЭДО.КорректировкаСведенийОЗакупкеКомиссионером);
	ТипыДокументовИсключений.Добавить(Перечисления.ТипыДокументовЭДО.КорректировкаСведенийОРеализацииКомиссионером);
	
	НуженОтветныйТитул = ЕстьИнформацияПолучателя(ТипРегламента) 
		И ТипыДокументовИсключений.Найти(ТипДокумента) = Неопределено;
	
	ОтветныйТитулЕщеНеСформирован = Не ЭтоСостояниеЗавершенногоЭДО(СостояниеДокумента)
		И Не СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ТребуетсяПодготовкаКОтправке
		И Не СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ОжидаетсяПодтверждениеОператора
		И Не СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ТребуетсяПовторнаяОтправка;
	
	НастройкаЗапретФормирования = ПолучитьФункциональнуюОпцию("ЗапретитьФормированиеОтветногоТитулаБезДокументовУчета");
	ТипыДокументовПопадающихПодЗапрет = Новый Массив(); // массив из ПеречислениеСсылка.ТипыДокументовЭДО
	ТипыДокументовПопадающихПодЗапрет.Добавить(Перечисления.ТипыДокументовЭДО.УПД);
	ТипыДокументовПопадающихПодЗапрет.Добавить(Перечисления.ТипыДокументовЭДО.УКД);
	
	ЗапретФормированияВручную = НастройкаЗапретФормирования 
		И Не ТипыДокументовПопадающихПодЗапрет.Найти(ТипДокумента) = Неопределено;
	
	Возврат НуженОтветныйТитул И ОтветныйТитулЕщеНеСформирован И Не ЗапретФормированияВручную;
