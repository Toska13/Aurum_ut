...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОтключитьКассу", Истина);
	Методы.Вставить("ПодключитьКассу", Истина);
...
Функция ОборотыКассовойСмены(Касса, НомерСмены) Экспорт
	...
	НастройкиАутентификации = ОФДСлужебный.ПолучитьНастройкиАутентификации(
		НастройкиКассы.НастройкаПодключения);
...
Функция get_kkts(ДанныеАутентификацииОФД)
	...
	ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("X-Datetime-Format");
	ЗаписьДанныхСообщения.ЗаписатьЗначение("Absolute");
...
Функция ПрочитатьДанные_get_receipts_short(ДанныеСервиса)
	...
	Попытка
		...
		Результат.Вставить("ДанныеЧеков", Новый Массив);
		
		Если Не ЗначениеЗаполнено(ДанныеСервиса.receipts) Тогда
			Возврат Результат;
		КонецЕсли;
