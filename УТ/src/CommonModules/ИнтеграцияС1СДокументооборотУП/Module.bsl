...
// Возвращает группу доступа партнеров по умолчанию.
//
// Возвращаемое значение:
//   СправочникСсылка.ГруппыДоступаПартнеров - группа доступа по умолчанию.
//   Неопределено - если группы доступа партнеров не используются.
//
Функция ГруппаДоступаПартнеровПоУмолчанию() Экспорт
	...
	Если Выборка.Следующий() Тогда
		Если Выборка.Количество() = 1 Или Выборка.ЭтоЭлементПоУмолчаниюДляОбмена Тогда
			ГруппаДоступаПоУмолчанию = Выборка.Ссылка;
		Иначе
	...
	Если НеобходимоСоздатьЭлемент Тогда
		...
		Попытка
			...
			ГруппаДоступаПоУмолчанию = ГруппаДоступаОбъект.Ссылка;
		Исключение
			ГруппаДоступаПоУмолчанию = Справочники.ГруппыДоступаПартнеров.ПустаяСсылка();
...
Функция СостояниеСогласованияУП(ТипРеквизита, ИмяРеквизита, ТипОбъекта, СостояниеСогласованияДО) Экспорт
	...
	ИначеЕсли ТипРеквизита = Тип("ПеречислениеСсылка.СтатусыДокументовРетроБонусов") Тогда
		...
		ИначеЕсли ТипОбъекта = Тип("ДокументОбъект.СписаниеРетроБонусовПоставщика")
				И Не ПолучитьФункциональнуюОпцию("ИспользоватьСогласованиеСписанийРетроБонусовПоставщиков") Тогда	
			Результат = Перечисления.СтатусыДокументовРетроБонусов.Согласован;
...
//
Функция ЮрФизЛицоИзОбъектаXDTO(ОбъектXDTO) Экспорт
	...
	Если Настройки.ИспользоватьИнтеграциюДО2 Тогда
		...
		Если ОбъектXDTO.legalPrivatePerson.objectID.ID = "ИндивидуальныйПредприниматель" Тогда //@NON-NLS-1
		...
		ИначеЕсли ОбъектXDTO.legalPrivatePerson.objectID.ID = "ФизЛицо" Тогда //@NON-NLS-1
...
Процедура ЗаполнитьСсылочныйОбъектXDTOИзПотребителя(Прокси, ОбъектXDTO, СсылкаНаПотребитель, Настройки)
	...
	ИначеЕсли ТипЗнч(СсылкаНаПотребитель) = Тип("ПеречислениеСсылка.ЮрФизЛицо") Тогда
		...
		Если Настройки.ИспользоватьИнтеграциюДО2 Тогда
			...
			ИначеЕсли СсылкаНаПотребитель = Перечисления.ЮрФизЛицо.ФизЛицо Тогда
				ОбъектXDTO.objectID.ID = "ФизЛицо"; //@NON-NLS-1
			ИначеЕсли СсылкаНаПотребитель = Перечисления.ЮрФизЛицо.ИндивидуальныйПредприниматель Тогда
				ОбъектXDTO.objectID.ID = "ИндивидуальныйПредприниматель"; //@NON-NLS-1
			ИначеЕсли СсылкаНаПотребитель = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент Тогда
				ОбъектXDTO.objectID.ID = "ЮрЛицоНеРезидент"; //@NON-NLS-1
			КонецЕсли;
		ИначеЕсли Настройки.ИспользоватьИнтеграциюДО3 Тогда
			ЗаполнитьПеречислениеXDTOИзПотребителя(ОбъектXDTO, СсылкаНаПотребитель, Настройки);
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(СсылкаНаПотребитель) = Тип("СправочникСсылка.СтавкиНДС") Тогда
		...
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(Прокси, ОбъектXDTO, "DMVATRateUniversal") Тогда
			Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаПотребитель,
				"НачалоПериода, КонецПериода, Ставка, РасчетнаяСтавка, Страна");
			Если ЗначениеЗаполнено(Реквизиты.НачалоПериода) Тогда
				ОбъектXDTO.beginOfPeriod = Реквизиты.НачалоПериода;
			КонецЕсли;
			Если ЗначениеЗаполнено(Реквизиты.КонецПериода) Тогда
				ОбъектXDTO.endOfPeriod = Реквизиты.КонецПериода;
			КонецЕсли;
			Если ЗначениеЗаполнено(Реквизиты.Ставка) Тогда
				ОбъектXDTO.rate = Реквизиты.Ставка;
			КонецЕсли;
			Если ЗначениеЗаполнено(Реквизиты.РасчетнаяСтавка) Тогда
				ОбъектXDTO.calculatedRate = Реквизиты.РасчетнаяСтавка;
			КонецЕсли;
			Если ЗначениеЗаполнено(Реквизиты.Страна) Тогда
				ОбъектXDTO.country = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
					Прокси,
					"DMCountry",
					Реквизиты.Страна);
			КонецЕсли;
			ЗначениеПеречисленияVATRate = "";
			ЗаполнитьПеречислениеXDTOИзПотребителя(ЗначениеПеречисленияVATRate, СсылкаНаПотребитель, Настройки);
			Если ЗначениеЗаполнено(ЗначениеПеречисленияVATRate) Тогда
				ОбъектXDTO.predefinedName = ЗначениеПеречисленияVATRate;
			КонецЕсли;
		Иначе
			//++ Локализация
			ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьРеквизитыИзПотребителя(
				Прокси,
				ОбъектXDTO,
				ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаНаПотребитель, "ПеречислениеСтавкаНДС"));
			//-- Локализация
		КонецЕсли;
