...
Процедура ИзменитьКачествоЗавершение(Номенклатура, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		...
		Если СтруктураДействий.Свойство("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", ПараметрыДействия)
				Или СтруктураДействий.Свойство("ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества", ПараметрыДействия)
				Или СтруктураДействий.Свойство("ЗаполнитьКонтролируемуюПартиюОтправителя", ПараметрыДействия) Тогда
			Для Каждого ИмяДействия Из ПараметрыДействия Цикл
				Если ИмяДействия.Ключ = "Ключ" Тогда
					Продолжить;
				КонецЕсли;
				Поля = СтрРазделить(ПараметрыДействия[ИмяДействия.Ключ].ИзменяемыеПоля, ",");
				Для Каждого Поле Из Поля Цикл
					СтрокаОприходования.Вставить(Поле);
				КонецЦикла;
				Поля = СтрРазделить(ПараметрыДействия[ИмяДействия.Ключ].ЧитаемыеПоля, ",");
				Для Каждого Поле Из Поля Цикл
					СтрокаОприходования.Вставить(Поле);
				КонецЦикла;
			КонецЦикла;
		...
		Если СтруктураДействий.Свойство("ЗаполнитьДубликатыЗависимыхРеквизитов", ПараметрыДействия) Тогда
			Для Каждого ИмяДействия Из ПараметрыДействия Цикл
				СтрокаОприходования.Вставить(ИмяДействия.Ключ);
				ЗависимыеРеквизиты = Новый Структура(ИмяДействия.Значение);
				Для Каждого Реквизит Из ЗависимыеРеквизиты Цикл
					СтрокаОприходования.Вставить(Реквизит.Ключ + ИмяДействия.Ключ);
				КонецЦикла;
			КонецЦикла;
		КонецЕсли;
		...
		Если ТекущаяСтрока.Свойство("НоменклатураОприходование") Тогда
			...
			ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(СтрокаОприходования, СтруктураДействий, КэшированныеЗначения);
			...
			Если СтруктураДействий.Свойство("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", ПараметрыДействия)
				...
				Для Каждого ИмяДействия Из ПараметрыДействия Цикл
					Если ИмяДействия.Ключ = "Ключ" Тогда
						Продолжить;
					КонецЕсли;
					ЗаполнитьЗначенияСвойств(ТекущаяСтрока, СтрокаОприходования, ПараметрыДействия[ИмяДействия.Ключ].ИзменяемыеПоля);
				КонецЦикла;
			КонецЕсли;
