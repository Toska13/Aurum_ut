...
// Формирует структуру доступных прав для пользователя
//
// Параметры:
...
Функция ПраваДоступаПоПользователям(Знач ПользовательВЕТИС = Неопределено, ЭтоРегламентноеЗадание = Ложь) Экспорт
	...
	ПраваДоступаПоПользователям = Запрос.Выполнить().Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкам); // ДеревоЗначений
...
// 
// Параметры:
//  ХозяйствующиеСубъекты - Массив из СправочникСсылка.ХозяйствующиеСубъектыВЕТИС - Структура, СправочникСсылка.ХозяйствующиеСубъектыВЕТИС - Организации, предприятия ВетИС для обмена.
//  ЭтоРегламентноеЗадание - Булево - признак регламентного задания (по умолчанию Ложь)
// 
// Возвращаемое значение:
//  Структура:
//  * ОбменНаСервере - Соответствие из КлючИЗначение:
//  ** Ключ - СправочникСсылка.ХозяйствующиеСубъектыВЕТИС
//  ** Значение - См. ИнтеграцияВЕТИСКлиентСервер.НоваяНастройкаПодключенияВЕТИС.
//  * Предприятия - Соответствие из КлючИЗначение
//  * БезНастроек - Массив из СправочникСсылка.ХозяйствующиеСубъектыВЕТИС
//  * Недоступные - Массив из СправочникСсылка.ХозяйствующиеСубъектыВЕТИС
...
//
// Параметры:
//  КоллекцияСообщений - Массив Из см. ИнтеграцияВЕТИСКлиентСервер.СтруктураСообщенияXML - содержит структуру сообщений, которые подготавливаются к передаче.
...
//
Функция ПодготовитьСообщенияКПередаче(КоллекцияСообщений, Немедленно, ПараметрыОбмена, ЗагружатьПосле = Неопределено) Экспорт
	...
	Для Каждого Сообщение Из КоллекцияСообщений Цикл
	...
	Исключения.Вставить(Перечисления.ДальнейшиеДействияПоВзаимодействиюВЕТИС.ВнеситеНомераТранспортныхСредств, Истина);
	...
	Для Каждого КлючИЗначение Из СообщенияПоДокументам Цикл
		...
		Попытка
			...
			Если ДальнейшееДействие <> Неопределено
				...
				Метаданные.РегистрыСведений.СтатусыДокументовВЕТИС.ПолноеИмя(), ПоляБлокировки);
				
				ТекущееСостояние = РегистрыСведений.СтатусыДокументовВЕТИС.ТекущееСостояние(Документ);
			...
			Для Каждого Сообщение Из Сообщения Цикл
				...
				Если Не ЗначениеЗаполнено(Сообщение.ТекстОшибки) Тогда
					...
					РеквизитыОчереди.Вставить("ДальнейшееДействие",           Сообщение.ДальнейшееДействие);
		...
		Для Каждого Сообщение Из Сообщения Цикл
			...
			Если Не ЗначениеЗаполнено(Сообщение.ТекстОшибки)
				...
				РеквизитыИсходящегоСообщения = Результат.Реквизиты;
				РеквизитыОчереди = Результат.РеквизитыОчереди;
...
Функция ЗагрузитьНаименованиеПродукции(ДанныеПродукции, СправочникОбъект = Неопределено, ПараметрыОбмена = Неопределено, БезЗаписи = Ложь, ТребуетсяПоиск = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(СправочникОбъект.GTIN)
		И ИнтеграцияИС.УпрощеннаяИнтеграцияБЭД()
		И Не ПродукцияСопоставленаСНоменклатурой(СправочникОбъект.Ссылка) Тогда
		
		ШтрихкодыEAN = Новый Массив;
		ШтрихкодыEAN.Добавить(РазборКодаМаркировкиИССлужебныйКлиентСерверПовтИсп.ШтрихкодEANИзGTIN(СправочникОбъект.GTIN));
		
		ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(СправочникОбъект);
		...
		Если ДанныеПоШтрихкодамEANРезультат.ТребуетсяОбновлениеКлючаСессии
			Или ДанныеПоШтрихкодамEANРезультат.ТребуетсяПовторОперации
			Или ЗначениеЗаполнено(ДанныеПоШтрихкодамEANРезультат.ТекстОшибки)
			Или ДанныеПоШтрихкодамEANРезультат.ДанныеПоШтрихкодамEAN.Количество() = 0 Тогда
			
			ЗаписатьПродукциюКСопоставлению(СправочникОбъект.Ссылка);
			
		Иначе
			
			Для Каждого СтрокаТаблицы Из ДанныеПоШтрихкодамEANРезультат.ДанныеПоШтрихкодамEAN Цикл
				ЗаписатьСопоставлениеСНоменклатурой(
...
Процедура ЗагрузитьТаблицыДвижений(ДополнительныеСвойства, Движения) Экспорт
	...
	СписокТаблиц = Новый Структура("ЗаписиСкладскогоЖурналаВРезервеВЕТИС");
	...
	Для Каждого Таблица Из ТаблицыДляДвижений Цикл
		...
		Если Не СписокТаблиц.Свойство(ИмяРегистра) Тогда
			Продолжить;
		КонецЕсли;
...
Функция ТекстЗапросаДвижениеСерийТоваров(МетаданныеДокумента) Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ ПЕРВЫЕ 0 ИСТИНА КАК Поле1";
...
// Продукция сопоставлена с номенклатурой.
// 
// Параметры:
//  ПродукцияВЕТИС - СправочникСсылка.ПродукцияВЕТИС
// 
// Возвращаемое значение:
//  Булево - Продукция сопоставлена с номенклатурой
Функция ПродукцияСопоставленаСНоменклатурой(ПродукцияВЕТИС) Экспорт
	...
	Запрос.Текст =
		...
		|	РегистрСведений.СоответствиеНоменклатурыВЕТИС КАК СоответствиеНоменклатурыВЕТИС
		|ГДЕ
		|	СоответствиеНоменклатурыВЕТИС.Продукция = &ПродукцияВЕТИС
		|	И СоответствиеНоменклатурыВЕТИС.Номенклатура <> &ПустаяНоменклатура";
...
// Записать сопоставление с номенклатурой.
// 
// Параметры:
//  ПродукцияВЕТИС - СправочникСсылка.ПродукцияВЕТИС
...
Процедура ЗаписатьСопоставлениеСНоменклатурой(ПродукцияВЕТИС, Номенклатура, Характеристика = Неопределено, Серия = Неопределено) Экспорт
	
	МенеджерЗаписи = РегистрыСведений.СоответствиеНоменклатурыВЕТИС.СоздатьМенеджерЗаписи();
	МенеджерЗаписи.Номенклатура   = Номенклатура;
	МенеджерЗаписи.Характеристика = Характеристика;
	МенеджерЗаписи.Серия          = Серия;
	МенеджерЗаписи.Продукция      = ПродукцияВЕТИС;
	МенеджерЗаписи.Записать();
	
	МенеджерЗаписи = РегистрыСведений.ПродукцияКСопоставлениюВЕТИС.СоздатьМенеджерЗаписи();
	МенеджерЗаписи.Продукция = ПродукцияВЕТИС;
	МенеджерЗаписи.Удалить();
	
КонецПроцедуры

// Записать продукцию к сопоставлению.
// 
// Параметры:
//  ПродукцияВЕТИС - СправочникСсылка.ПродукцияВЕТИС
Процедура ЗаписатьПродукциюКСопоставлению(ПродукцияВЕТИС) Экспорт
...
// Удалить продукцию к сопоставлению.
// 
// Параметры:
//  ПродукцияВЕТИС - СправочникСсылка.ПродукцияВЕТИС
Процедура УдалитьПродукциюКСопоставлению(ПродукцияВЕТИС) Экспорт
...
Процедура ЗаполнитьОписаниеКлассификаторов(Классификаторы, СохранятьФайлВКэш) Экспорт
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ДанныеКонтрагентаПоИННФоновоеЗадание", Истина);
	Методы.Вставить("УстановитьПараметрыСеанса");
	Методы.Вставить("СверткаРегистраСоответствиеНоменклатурыВЕТИС", Истина);
...
Функция ПодготовитьСообщениеКПередаче(ТекстСообщенияXML, Реквизиты, РеквизитыОчереди)
	...
	ВозвращаемоеЗначение.Вставить("Реквизиты",              Реквизиты);
...
Процедура ПриСозданииНаСервереФормыСпискаДокументовВЕТИС(Форма, ИменаСписков, ИменаСписковКОформлению) Экспорт
	...
	ИнтеграцияИС.УстановитьНавигационнуюСсылкуФормыСпискаДокументов(Форма, ИмяДокумента);
	...
	Если Метаданные.Документы.Найти(ИмяДокумента) = Неопределено
		...
		ИнтеграцияИС.УстановитьВидимостьОтветственный(Форма, ИспользоватьСпискиКОформлению);
		УстановитьВидимостьОрганизацииВЕТИС(Форма, ИспользоватьСпискиКОформлению);
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		ИспользоватьСпискиКОформлению = Истина;
...
// Установить видимость организации ВЕТИС. Скрывать элементы формы для режима УпрощеннаяИнтеграцияБЭД
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
//  ИспользоватьСпискиКОформлению - Булево - Использовать списки к оформлению
//  ИмяЭлементаОтборХозяйствующийСубъект - Строка - Имя элемента отбор хозяйствующий субъект
Процедура УстановитьВидимостьОрганизацииВЕТИС(Форма, ИспользоватьСпискиКОформлению,
	ИмяЭлементаОтборХозяйствующийСубъект = "СтраницыОформленоОтборОрганизацияВЕТИС") Экспорт
	
	УпрощеннаяИнтеграцияБЭД = ИнтеграцияИС.УпрощеннаяИнтеграцияБЭД();
	Если УпрощеннаяИнтеграцияБЭД Тогда
		
		Если Форма.Элементы.Найти(ИмяЭлементаОтборХозяйствующийСубъект) <> Неопределено Тогда
			Форма.Элементы[ИмяЭлементаОтборХозяйствующийСубъект].Видимость = Ложь;
		КонецЕсли;
		
		Если Форма.Элементы.Найти("ХозяйствующийСубъект") <> Неопределено Тогда
			Форма.Элементы["ХозяйствующийСубъект"].Видимость = Ложь;
		КонецЕсли;
		Если Форма.Элементы.Найти("Предприятие") <> Неопределено Тогда
			Форма.Элементы["Предприятие"].Видимость = Ложь;
		КонецЕсли;
		
		Если ИспользоватьСпискиКОформлению
			И Форма.Элементы.Найти("СтраницыКОформлениюОтборОрганизацияВЕТИС") <> Неопределено Тогда
			Форма.Элементы["СтраницыКОформлениюОтборОрганизацияВЕТИС"].Видимость = Ложь;
			
			Если Форма.Элементы.Найти("СписокКОформлениюХозяйствующийСубъект") <> Неопределено Тогда
				Форма.Элементы["СписокКОформлениюХозяйствующийСубъект"].Видимость = Ложь;
			КонецЕсли;
			
			Если Форма.Элементы.Найти("СписокКОформлениюПредприятие") <> Неопределено Тогда
				Форма.Элементы["СписокКОформлениюПредприятие"].Видимость = Ложь;
			КонецЕсли;
...
Функция ОписаниеОформленныхДокументовВЕТИС(МетаданныеДокументаВЕТИС, ДокументОснование, ДокументыПоОснованию, СтатусыОформления) Экспорт
	...
	Описание.Вставить("ПолноеИмяМетаданных", МетаданныеДокументаВЕТИС.ПолноеИмя());
...
Функция ДанныеЗаполненияДокументаМаркировкаТоваровИСМП(ЗаполнениеВыполнено, ДанныеЗаполнения, ДокументОбъект) Экспорт
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		...
		Если ДанныеЗаполнения.Свойство("ИмяФильтра")
			...
			ТоварнаяГруппа = ДанныеЗаполнения.ЗначениеФильтра;
		КонецЕсли;
	...
	Если ТоварнаяГруппа = Неопределено Тогда
		Если ЗначениеЗаполнено(ДокументОбъект.ТоварнаяГруппа) Тогда
			ТоварнаяГруппа = ДокументОбъект.ТоварнаяГруппа;
		Иначе
	...
	Если ВидПродукции = Неопределено Тогда
		Если ТоварнаяГруппа = Неопределено Тогда
			ВидПродукции = Новый Массив;
		Иначе
			ВидПродукции = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ТоварнаяГруппа);
		КонецЕсли;
	КонецЕсли;
	
	ЗаполнениеВыполнено = Истина;
	Результат = Новый Структура("Шапка, Товары, ШтрихкодыУпаковок");
	Результат.Вставить("ТоварнаяГруппа", ТоварнаяГруппа);
	
	Если ТипОснования = ПроизводственнаяОперация Тогда
		Запрос = Новый Запрос(
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТоварыДокументаОснования.Ссылка КАК Ссылка,
		|	ИСТИНА                          КАК ЕстьСырье
		|ПОМЕСТИТЬ ДанныеТабличнойЧастиТовары
		|ИЗ
		|	Документ.ПроизводственнаяОперацияВЕТИС.Товары КАК ТоварыДокументаОснования
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		|		ПО ТоварыДокументаОснования.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
		|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииСырье)
		|ГДЕ
		|	ТоварыДокументаОснования.Ссылка = &ДокументОснование
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ПустаяСсылка)
		|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ОтчетОВерификацииНанесенныхКМ)
		|	КОНЕЦ КАК ОперацияНанесения,
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ДанныеТабличнойЧастиТовары.ЕстьСырье, ЛОЖЬ)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотСырьяПроизводствоРФ)
		|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотПроизводствоРФ)
		|	КОНЕЦ     КАК Операция,
		|	ВЫБОР
	...
	Иначе
		...
		|ПОМЕСТИТЬ ДанныеТабличнойЧастиТовары
		|ИЗ
		|	Документ.ВходящаяТранспортнаяОперацияВЕТИС.Товары КАК ТоварыДокументаОснования
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		|		ПО ТоварыДокументаОснования.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
		|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииСырье)
		|ГДЕ
	...
	Если Не ДокументОбъект.ДополнительныеСвойства.Свойство("НеЗаполнятьТабличнуюЧасть") Тогда 
		...
		|ГДЕ
		|	СвойстваМаркируемойПродукции.ВидПродукции.ВидПродукцииГИСМТ В (&ВидПродукции)
		|;
		|/////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицаТовары.Номенклатура      КАК Номенклатура,
...
Функция ПроблемыКлассификаторовВПанелиОбмена(РасширенноеОписаниеТребуетсяЗагрузка = Ложь) Экспорт
	...
	РазделениеВключено             = ОбщегоНазначения.РазделениеВключено();
	ИдентификаторыКлассификаторов  = ИдентификаторыКлассификаторов();
	ОбщиеКлассификаторыБГосИС      = ОбщегоНазначенияИС.ИдентификаторыКлассификатораВидовПродукцииИС();
	...
	Если РазделениеВключено Тогда
		...
		Для каждого КлючИЗначение Из ОбщиеКлассификаторыБГосИС Цикл
			ТекстЗапроса = ОбщегоНазначенияИС.ТекстЗапросаПроверкиНеобходимостиЗагрузкиКлассификаторовИС(КлючИЗначение.Ключ);
