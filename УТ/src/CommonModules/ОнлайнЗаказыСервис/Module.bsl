...
Функция page_settings(ПараметрыОперации, ПараметрыНастройки)
	...
	Если Не ПустаяСтрока(ПараметрыОперации.ТокенЯндексПэй) Тогда
		ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("yandexPayAuthToken");
		ЗаписьДанныхСообщения.ЗаписатьЗначение(ПараметрыОперации.ТокенЯндексПэй);
	КонецЕсли;
...
Функция ОпределитьАктуальностьХеша(ТелоJSON)
	...
	Попытка
		...
		ЧтениеОтвета.УстановитьСтроку(ТелоJSON);
		Результат = ПрочитатьJSON(ЧтениеОтвета);
		Если СокрЛП(Результат.detail) = "Контрольная сумма заказа не совпадает" Тогда // АПК:1297 Ответ стороннего сервиса
			Возврат Ложь;
		Иначе
			Возврат Истина;
		КонецЕсли;
...
Функция ПрочитатьДанные_order_infos(ТелоJSON, НастройкаПодключения)
	...
	//        "qrc": "string",
	//        "paymentId": "string",
	//        "operationDate": "2026-04-27T07:06:13.924Z",
	//        "paymentModel": "C2B",
	//        "sbpOrderId": "string",
	//        "merchantId": "string",
	...
	//        "merchantId": "string",
	//        "paymentPurpose": "string",
	//        "operationDate": "2026-04-27T07:06:13.925Z"
	//      },
	//      "yandexPayPayment": {
	//        "saleToken": "string",
	//        "operationDate": "2026-04-27T07:06:13.925Z"
	...
	//      },
	//      "fiscalization": {
	//        "status": "REQUEST_EXPECTED",
	//        "providerResponseStatus": 0,
	//        "providerResponse": {},
	//        "attemptsCount": 0,
	...
	Попытка
		...
		Для Каждого ДанныеЗаказаВСервисе Из ДанныеСервиса.orders Цикл
			...
			Если ДанныеЗаказаВСервисе.Свойство("yandexPayPayment")
				И ЗначениеЗаполнено(ДанныеЗаказаВСервисе.yandexPayPayment) Тогда
				
				Эталон = Новый Структура;
				Эталон.Вставить("saleToken", "");
				Эталон.Вставить("operationDate", "");
				ЗаполнитьЗначенияСвойств(Эталон, ДанныеЗаказаВСервисе.yandexPayPayment);

				// АПИ яндекса не возвращает идентификатор транзакции операции оплаты,
				// поэтому sale_token устанавливается в качестве идентификатора оплаты и операции.
				...
				Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПлатежныеСистемы.ЯндексПэй") Тогда
					...
					ЗакодированныйИдентификатор = МодульЯндексПэйСлужебный.КодироватьИдентификаторОплаты(
						Эталон.saleToken,
						МодульЯндексПэйСлужебный.ТипСсылкиДинамическая());
				Иначе
					ЗакодированныйИдентификатор = Эталон.saleToken;
				КонецЕсли;
				
				ОплатаЯндексПэй = Новый Структура;
				ОплатаЯндексПэй.Вставить("ИдентификаторОплаты", ЗакодированныйИдентификатор);
				ОплатаЯндексПэй.Вставить("ИдентификаторОперации", ЗакодированныйИдентификатор);
				ОплатаЯндексПэй.Вставить("ДатаОперации", ДатаВUTC(Эталон.operationDate));
			...
			ДанныеЗаказа.Вставить("ОплатаЯндексПэй",         ОплатаЯндексПэй);
...
Функция СпособОплаты(ДанныеЗаказаВСервисе)
	...
	ИначеЕсли ИдентификаторПоДаннымСервиса = "SBP" Тогда
		Если ДанныеЗаказаВСервисе.sbpPayment.paymentModel = ОнлайнЗаказыСлужебный.ИдентификаторВариантаСБПC2B() Тогда
	...
	ИначеЕсли ИдентификаторПоДаннымСервиса = "YANDEX_PAY" Тогда
