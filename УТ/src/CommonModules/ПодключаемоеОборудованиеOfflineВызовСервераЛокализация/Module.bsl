...
Процедура ЗаполнитьСтруктуруПрайсЛистаИзДанныхКВыгрузке(СтруктураПрайсЛиста, ТоварыКВыгрузке) Экспорт
	...
	|	ЕСТЬNULL(ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ, НЕОПРЕДЕЛЕНО) КАК ВидПродукцииГИСМТ
	|ИЗ
	|	Товары КАК Товары
	...
	СоответствиеВидовМаркированнойПродукцииБПО = СоответствиеВидовМаркированнойПродукцииБПО();
	...
	Пока ВыборкаПоНоменклатуре.Следующий() Цикл
		...
		ВидПродукцииГИСМТ = ВыборкаПоНоменклатуре.ВидПродукцииГИСМТ;
		
		Если ЗначениеЗаполнено(ВидПродукцииГИСМТ) Тогда
			
			ТипМаркировкиККТ = КонструкторИСОбщегоНазначенияКлиентСервер.ТипМаркировкиККТПоВидуПродукции(ВидПродукцииГИСМТ);
			Если ЗначениеЗаполнено(ТипМаркировкиККТ) Тогда
				ЗаписьТовар.ТипМаркированнойПродукции = ТипМаркировкиККТ;
				ЗаписьТовар.ЭтоМаркированнаяПродукция = Истина;
			КонецЕсли;
			
			ВидыПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ЗначенияЭлементаКонструктораПоВидуПродукции(
			...
			Для Каждого ВидПродукции Из ВидыПродукции Цикл
				...
				Если ВидПродукции = Перечисления.ВидыПродукцииИС.Пиво
					...
					Если ВыборкаПоНоменклатуре.РазливноеПиво = ИСТИНА Тогда
						ВидМаркированнойПродукцииБПО = СоответствиеВидовМаркированнойПродукцииБПО[Перечисления.ВидыПродукцииИС.Пиво];
					Иначе
						ВидМаркированнойПродукцииБПО = СоответствиеВидовМаркированнойПродукцииБПО[Перечисления.ВидыПродукцииИС.ПивоВПотребительскихУпаковках];
						ЗаписьТовар.ЭтоАлкоголь      = Ложь;
					КонецЕсли;
				Иначе
					ВидМаркированнойПродукцииБПО = СоответствиеВидовМаркированнойПродукцииБПО[ВидПродукции];
				КонецЕсли;
				
				Если ЗначениеЗаполнено(ВидМаркированнойПродукцииБПО) Тогда
					ЗаписьТовар.ВидМаркированнойПродукции = ВидМаркированнойПродукцииБПО;
					ЗаписьТовар.ЭтоМаркированнаяПродукция = Истина;
				КонецЕсли;
...
Функция ПолучитьДанныеДляЗаполненияДокумента(ПодключаемоеОборудование, ТаблицаТоваров, ТаблицаШтрихкодовАкцизов, ЗагрузкаАкцизныхМарок)
	...
	// Возможна ситуация, когда в таблице товаров несколько одинаковых строк.
	Запрос = Новый Запрос;
	...
	|ВЫБРАТЬ
	|	Товары.Штрихкод КАК Штрихкод,
	|	МАКСИМУМ(ЕСТЬNULL(КодыТоваровПодключаемогоОборудованияOffline.Номенклатура.ТипНоменклатуры, ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.ПустаяСсылка))) КАК ТипНоменклатуры,
	|	МАКСИМУМ(ЕСТЬNULL(КодыТоваровПодключаемогоОборудованияOffline.Номенклатура, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))) КАК Номенклатура,
	|	МАКСИМУМ(ЕСТЬNULL(КодыТоваровПодключаемогоОборудованияOffline.Характеристика, ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка))) КАК Характеристика,
	|	МАКСИМУМ(ЕСТЬNULL(КодыТоваровПодключаемогоОборудованияOffline.Упаковка, ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка))) КАК Упаковка
	|ПОМЕСТИТЬ ДанныеВесовыхТоваровПоШтрихкоду
	|ИЗ
	|	Товары КАК Товары
	...
	|
	|ВЫБРАТЬ
	|	КодыТоваровПодключаемогоОборудованияOffline.ТипНоменклатуры КАК ТипНоменклатуры,
	...
	|ИЗ
	|	Товары КАК Товары
	|	ЛЕВОЕ СОЕДИНЕНИЕ ДанныеВесовыхТоваровПоШтрихкоду КАК КодыТоваровПодключаемогоОборудованияOffline
	|	ПО Товары.Штрихкод = КодыТоваровПодключаемогоОборудованияOffline.Штрихкод
	|
	|ГДЕ
	|	Товары.Весовой
	|;
	...
	Если ВыборкаПоСменам.НайтиСледующий(Новый Структура("НомерСмены", НомерКассовойСменыДляПоиска)) Тогда
		...
		Если ВыборкаПоИдентификаторам.НайтиСледующий(Новый Структура("НомерСмены,УникальныйИдентификатор", НомерКассовойСменыДляПоиска, УникальныйИдентификаторДляПоиска)) Тогда
			...
			Если ТаблицаНаборов.Количество() Тогда
				ТаблицаНаборов.Свернуть("Номенклатура,Характеристика,ПроцентРучнойСкидки,СтавкаНДС,Цена","Количество");
...
Функция ЗаполнитьОтчетОРозничныхВозвратах(ПараметрыПодключаемогоОборудования, ПараметрыКассовойСмены, ВыборкаПоСменам, ТаблицаАкцизныеМарки, ТаблицаОплат, ЗагрузкаАкцизныхМарок)
	...
	Если ВыборкаПоСменам.НайтиСледующий(Новый Структура("НомерСмены", НомерКассовойСменыДляПоиска)) Тогда
		...
		Если ВыборкаПоИдентификаторам.НайтиСледующий(Новый Структура("НомерСмены,УникальныйИдентификатор", НомерКассовойСменыДляПоиска, УникальныйИдентификаторДляПоиска)) Тогда
			...
			// Дозаполняем Товары наборами
			Если ТаблицаНаборов.Количество() Тогда
				ТаблицаНаборов.Свернуть("Номенклатура,Характеристика,ДокументРеализации,ПроцентРучнойСкидки,СтавкаНДС,Цена","Количество");
...
// Соответствие видов маркированной продукции БПО.
// 
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//  * Ключ - ПеречислениеСсылка.ВидыПродукцииИС
//  * Значение - ПеречислениеСсылка.ВидыМаркированнойПродукцииБПО
Функция СоответствиеВидовМаркированнойПродукцииБПО()
	
	ВозвращаемоеЗначение = Новый Соответствие();
	
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ПродукцияИзНатуральногоМеха,
		Перечисления.ВидыМаркированнойПродукцииБПО.ИзделияИзМеха);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.Табак,
		Перечисления.ВидыМаркированнойПродукцииБПО.Табак);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.Обувь,
		Перечисления.ВидыМаркированнойПродукцииБПО.Обувь);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность,
		Перечисления.ВидыМаркированнойПродукцииБПО.ЛегкаяПромышленность);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность2025,
		Перечисления.ВидыМаркированнойПродукцииБПО.ЛегкаяПромышленность);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.МолочнаяПродукцияБезВЕТИС,
		Перечисления.ВидыМаркированнойПродукцииБПО.МолочнаяПродукция);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.МолочнаяПродукцияПодконтрольнаяВЕТИС,
		Перечисления.ВидыМаркированнойПродукцииБПО.МолочнаяПродукция);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.Шины,
		Перечисления.ВидыМаркированнойПродукцииБПО.Шины);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.Фотоаппараты,
		Перечисления.ВидыМаркированнойПродукцииБПО.Фотоаппараты);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.Духи,
		Перечисления.ВидыМаркированнойПродукцииБПО.Духи);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.Велосипеды,
		Перечисления.ВидыМаркированнойПродукцииБПО.Велосипеды);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.КреслаКоляски,
		Перечисления.ВидыМаркированнойПродукцииБПО.КреслаКоляски);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.АльтернативныйТабак,
		Перечисления.ВидыМаркированнойПродукцииБПО.АльтернативныйТабак);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.УпакованнаяВода,
		Перечисления.ВидыМаркированнойПродукцииБПО.УпакованнаяВода);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.Антисептики,
		Перечисления.ВидыМаркированнойПродукцииБПО.Антисептики);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.БАДы,
		Перечисления.ВидыМаркированнойПродукцииБПО.БАДы);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.БАДы2025,
		Перечисления.ВидыМаркированнойПродукцииБПО.БАДы);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.СпортивноеПитание,
		Перечисления.ВидыМаркированнойПродукцииБПО.БАДы);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.НикотиносодержащаяПродукция,
		Перечисления.ВидыМаркированнойПродукцииБПО.НикотиносодержащаяПродукция);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.Пиво,
		Перечисления.ВидыМаркированнойПродукцииБПО.РазливноеПиво);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ПивоВПотребительскихУпаковках,
		Перечисления.ВидыМаркированнойПродукцииБПО.Пиво);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.СоковаяПродукция,
		Перечисления.ВидыМаркированнойПродукцииБПО.СоковаяПродукция);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.НапиткиССодержаниемМолокаКакаоПрочие,
		Перечисления.ВидыМаркированнойПродукцииБПО.СоковаяПродукция);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.БезалкогольноеПиво,
		Перечисления.ВидыМаркированнойПродукцииБПО.БезалкогольноеПиво);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.МорепродуктыПодконтрольныеВЕТИС,
		Перечисления.ВидыМаркированнойПродукцииБПО.Морепродукты);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ВетеринарныеПрепараты,
		Перечисления.ВидыМаркированнойПродукцииБПО.ВетеринарныеПрепараты);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ИгрыИИгрушкиДляДетей,
		Перечисления.ВидыМаркированнойПродукцииБПО.ИгрыИИгрушкиДляДетей);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.РадиоэлектроннаяПродукция,
		Перечисления.ВидыМаркированнойПродукцииБПО.РадиоэлектроннаяПродукция);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ТитановаяМеталлопродукция,
		Перечисления.ВидыМаркированнойПродукцииБПО.ТитановаяМеталлопродукция);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.РастительныеМасла,
		Перечисления.ВидыМаркированнойПродукцииБПО.РастительныеМасла);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ОптоволокноИОптоволоконнаяПродукция,
		Перечисления.ВидыМаркированнойПродукцииБПО.ОптоволокноИОптоволоконнаяПродукция);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ПарфюмерныеИКосметическиеСредстваИБытоваяХимия,
		Перечисления.ВидыМаркированнойПродукцииБПО.ПарфюмерныеИКосметическиеСредстваИБытоваяХимия);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ТоварыЛичнойГигиены,
		Перечисления.ВидыМаркированнойПродукцииБПО.ПарфюмерныеИКосметическиеСредстваИБытоваяХимия);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.БритвыИЛезвия,
		Перечисления.ВидыМаркированнойПродукцииБПО.ПарфюмерныеИКосметическиеСредстваИБытоваяХимия);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.КормаДляЖивотныхБезВЕТИС,
		Перечисления.ВидыМаркированнойПродукцииБПО.КормаДляЖивотных);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.КормаДляЖивотныхПодконтрольныеВЕТИС,
		Перечисления.ВидыМаркированнойПродукцииБПО.КормаДляЖивотных);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.МясоПодконтрольноеВЕТИС,
		Перечисления.ВидыМаркированнойПродукцииБПО.Мясо);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ПечатнаяПродукция,
		Перечисления.ВидыМаркированнойПродукцииБПО.ПечатнаяПродукция);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.СтроительныеМатериалы,
		Перечисления.ВидыМаркированнойПродукцииБПО.СтроительныеМатериалы);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ОтопительныеПриборы,
		Перечисления.ВидыМаркированнойПродукцииБПО.ОтопительныеПриборы);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.Бакалея,
		Перечисления.ВидыМаркированнойПродукцииБПО.Бакалея);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.РастворимыеЗавариваемыеНапитки,
		Перечисления.ВидыМаркированнойПродукцииБПО.Бакалея);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ТелефоныИНоутбуки,
		Перечисления.ВидыМаркированнойПродукцииБПО.ТелефоныИНоутбуки);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ПиротехническиеИзделияИСредстваПожарнойБезопасности,
		Перечисления.ВидыМаркированнойПродукцииБПО.ПиротехническиеИзделияИСредстваПожарнойБезопасности);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.КабельнаяПродукция,
		Перечисления.ВидыМаркированнойПродукцииБПО.КабельнаяПродукция);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.МоторныеМасла,
		Перечисления.ВидыМаркированнойПродукцииБПО.МоторныеМасла);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.БезалкогольныеНапитки,
		Перечисления.ВидыМаркированнойПродукцииБПО.БезалкогольныеНапитки);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ТехническиеСредстваРеабилитации,
		Перечисления.ВидыМаркированнойПродукцииБПО.ТехническиеСредстваРеабилитации);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.МедицинскиеИзделия,
		Перечисления.ВидыМаркированнойПродукцииБПО.МедицинскиеИзделия);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.КонсервированнаяПродукцияБезВЕТИС,
		Перечисления.ВидыМаркированнойПродукцииБПО.КонсервированнаяПродукция);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.КонсервированнаяПродукцияПодконтрольнаяВЕТИС,
		Перечисления.ВидыМаркированнойПродукцииБПО.КонсервированнаяПродукция);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ПолимерныеТрубы,
		Перечисления.ВидыМаркированнойПродукцииБПО.ПолимерныеТрубы);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.СладостиИКондитерскиеИзделия,
		Перечисления.ВидыМаркированнойПродукцииБПО.СладостиИКондитерскиеИзделия);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ПечатныеПлаты,
		Перечисления.ВидыМаркированнойПродукцииБПО.ПечатныеПлаты);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.ДезинфицирующиеСредства,
		Перечисления.ВидыМаркированнойПродукцииБПО.ДезинфицирующиеСредства);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.АлкогольнаяПродукцияДо9Процентов,
		Перечисления.ВидыМаркированнойПродукцииБПО.АлкогольнаяПродукцияДо9Процентов);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.КормаДляЖивотныхВлажныеПодконтрольныеВЕТИС,
		Перечисления.ВидыМаркированнойПродукцииБПО.КормаДляЖивотныхВлажные);
	ВозвращаемоеЗначение.Вставить(
		Перечисления.ВидыПродукцииИС.КормаДляЖивотныхВлажныеБезВЕТИС,
		Перечисления.ВидыМаркированнойПродукцииБПО.КормаДляЖивотныхВлажные);
...
Функция ТекстЗапросаДанныеТоваровДляВыгрузки(Знач Параметры, Знач ИспользоватьСерииНоменклатуры)
	...
	|	ЕСТЬNULL(КодыТоваров.Номенклатура.Артикул,"""")            КАК Артикул,
	|	ЕСТЬNULL(КодыТоваров.Номенклатура.КодВидаНоменклатурнойКлассификации,"""") КАК КодВидаНоменклатурнойКлассификации,
	...
	|	ЕСТЬNULL(ВЫРАЗИТЬ(КодыТоваров.Номенклатура.Описание КАК Строка(1000)),"""") КАК ОписаниеТовара,
	|	КодыТоваров.Номенклатура.ВидНоменклатуры КАК ВидНоменклатуры,
	|	КодыТоваров.Номенклатура.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	КодыТоваров.Номенклатура.ТипНоменклатуры КАК ТипНоменклатуры,
	|	КодыТоваров.Номенклатура.ОсобенностьУчета КАК ОсобенностьУчета,
	|	&ТекстЗапросаТипИзмеряемойВеличиныЕдиницыИзмерения КАК ТипИзмеряемойВеличины,
	|
	|	ЕСТЬNULL(КодыТоваров.Номенклатура.ВидАлкогольнойПродукции.Маркируемый, ЛОЖЬ) КАК ВидАлкогольнойПродукцииМаркируемый,
	|	КодыТоваров.Номенклатура.ВидАлкогольнойПродукции.Код КАК ВидАлкогольнойПродукцииКод,
	|	КодыТоваров.Номенклатура.АлкогольнаяПродукция КАК АлкогольнаяПродукция,
	|	КодыТоваров.Номенклатура.ОбъемДАЛ КАК ОбъемДАЛ,
	|	КодыТоваров.Номенклатура.Крепость КАК Крепость,
	|	КодыТоваров.Номенклатура.ПроизводительИмпортерКонтрагент.ИНН КАК ПроизводительИмпортерКонтрагентИНН,
	|	КодыТоваров.Номенклатура.ПроизводительИмпортерКонтрагент.КПП КАК ПроизводительИмпортерКонтрагентКПП,
	|
	...
	|	ЕСТЬNULL(КодыТоваров.Номенклатура.ЕдиницаИзмерения.Код, """") КАК ЕдиницаИзмеренияКод,
	|	ЕСТЬNULL(КодыТоваров.Номенклатура.ЕдиницаИзмерения.Наименование, """") КАК ЕдиницаИзмеренияНаименование,
	|
	...
	|	ВЫБОР КОГДА КодыТоваровПодключаемогоОборудованияOfflineИзменения.Узел = &УзелИнформационнойБазы
	|			ТОГДА 1
	|		ИНАЧЕ 0
	...
	|		ПО КодыТоваровПодключаемогоОборудованияOfflineИзменения.Код = КодыТоваров.Код
	...
	|ГДЕ
	|	КодыТоваров.ПравилоОбмена = &ПравилоОбмена
	|	И КодыТоваров.Номенклатура <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|	И ВЫБОР
	...
	ТекстЗапроса = 	УчетПодакцизныхТоваров.ТекстЗапросаВтПодакцизныеТовары("ТаблицаНоменклатуры","ТекущаяДата","СтранаРегистрации");
	Тексты.Добавить(ТекстЗапроса);
	...
	|	""""                                            КАК КодГруппы,
	|	КодыТоваровПодключаемогоОборудованияOffline.Номенклатура КАК Номенклатура,
	|	КодыТоваровПодключаемогоОборудованияOffline.Артикул КАК Артикул,
	|	КодыТоваровПодключаемогоОборудованияOffline.КодВидаНоменклатурнойКлассификации КАК КодВидаНоменклатурнойКлассификации,
	|	КодыТоваровПодключаемогоОборудованияOffline.НоменклатураНаименование КАК НоменклатураНаименование,
	|	КодыТоваровПодключаемогоОборудованияOffline.НоменклатураНаименованиеПолное КАК НоменклатураНаименованиеПолное,
	|	КодыТоваровПодключаемогоОборудованияOffline.ОписаниеТовара КАК ОписаниеТовара,
	|	КодыТоваровПодключаемогоОборудованияOffline.Характеристика КАК Характеристика,
	|	КодыТоваровПодключаемогоОборудованияOffline.ХарактеристикаНаименование КАК ХарактеристикаНаименование,
	|	КодыТоваровПодключаемогоОборудованияOffline.ХарактеристикаНаименованиеПолное КАК ХарактеристикаНаименованиеПолное,
	|	КодыТоваровПодключаемогоОборудованияOffline.ЕдиницаИзмеренияКод КАК КодЕдиницыИзмерения,
	|	КодыТоваровПодключаемогоОборудованияOffline.ЕдиницаИзмеренияНаименование КАК НаименованиеЕдиницыИзмерения,
	|	КодыТоваровПодключаемогоОборудованияOffline.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	КодыТоваровПодключаемогоОборудованияOffline.Упаковка КАК Упаковка,
	|	КодыТоваровПодключаемогоОборудованияOffline.УпаковкаНаименование КАК УпаковкаНаименование,
	|	ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, 1) КАК КоэффициентУпаковки,
	|	КодыТоваровПодключаемогоОборудованияOffline.Штрихкод КАК Штрихкод,
	...
	|	ВЫБОР
	|		КОГДА КодыТоваровПодключаемогоОборудованияOffline.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Вес)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Весовой,
	|	КодыТоваровПодключаемогоОборудованияOffline.ТипНоменклатуры КАК ТипНоменклатуры,
	...
	|	&ИспользованиеСерий,
	|	
	|	КодыТоваровПодключаемогоОборудованияOffline.ИндексКартинкиЕстьИзменения КАК ИндексКартинкиЕстьИзменения,
	|	
	|	&СтавкаНДС КАК СтавкаНДС,
	|	
	|	КодыТоваровПодключаемогоОборудованияOffline.АлкогольнаяПродукция КАК Алкоголь,
	|	
	|	ВЫБОР
	|		КОГДА (КодыТоваровПодключаемогоОборудованияOffline.АлкогольнаяПродукция
	|				И КодыТоваровПодключаемогоОборудованияOffline.ВидАлкогольнойПродукцииМаркируемый)
	|				ИЛИ ЕСТЬNULL(ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ, ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|	КодыТоваровПодключаемогоОборудованияOffline.ОсобенностьУчета КАК ОсобенностьУчета,
	|	
	|	ВЫБОР
	|		КОГДА КодыТоваровПодключаемогоОборудованияOffline.АлкогольнаяПродукция
	|			ТОГДА КодыТоваровПодключаемогоОборудованияOffline.ВидАлкогольнойПродукцииКод
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК КодВидаАлкогольнойПродукции,
	|	
	|	ВЫБОР
	|		КОГДА КодыТоваровПодключаемогоОборудованияOffline.АлкогольнаяПродукция
	|			ТОГДА КодыТоваровПодключаемогоОборудованияOffline.ОбъемДАЛ * 10
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ЕмкостьТары,
	|	
	|	ВЫБОР
	|		КОГДА КодыТоваровПодключаемогоОборудованияOffline.АлкогольнаяПродукция
	|			ТОГДА КодыТоваровПодключаемогоОборудованияOffline.Крепость
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Крепость,
	|	
	|	ВЫБОР
	|		КОГДА КодыТоваровПодключаемогоОборудованияOffline.АлкогольнаяПродукция
	|			ТОГДА КодыТоваровПодключаемогоОборудованияOffline.ПроизводительИмпортерКонтрагентИНН
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ИННПроизводителя,
	|	
	|	ВЫБОР
	|		КОГДА КодыТоваровПодключаемогоОборудованияOffline.АлкогольнаяПродукция
	|			ТОГДА КодыТоваровПодключаемогоОборудованияOffline.ПроизводительИмпортерКонтрагентКПП
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК КПППроизводителя
	|
	|ИЗ
	|	ТаблицаНоменклатуры КАК КодыТоваровПодключаемогоОборудованияOffline
	...
	|		ПО КодыТоваровПодключаемогоОборудованияOffline.Номенклатура = ВтПодакцизныеТовары.Номенклатура
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Если ИспользоватьСерииНоменклатуры Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&ИспользованиеСерий,","ВЫБОР КОГДА 1 В (ВЫБРАТЬ ПЕРВЫЕ 1 1 ИЗ Справочник.ВидыНоменклатуры.ПолитикиУчетаСерий КАК Т ГДЕ Т.Склад = &ПравилоОбменаСклад И Т.ПолитикаУчетаСерий.УказыватьПриОтгрузкеВРозницу И Т.Ссылка = КодыТоваровПодключаемогоОборудованияOffline.ВидНоменклатуры) ТОГДА ИСТИНА ИНАЧЕ ЛОЖЬ КОНЕЦ КАК УказыватьСерииПриОтгрузкеВРозницу,");
	Иначе
...
Функция ТекстЗапросаДанныеТоваровДляВыгрузки25(Знач Параметры, Знач ИспользоватьСерииНоменклатуры)
	...
	|	ЕСТЬNULL(КодыТоваров.Номенклатура.ЕдиницаИзмерения.Код, """") КАК ЕдиницаИзмеренияКод,
	|	ЕСТЬNULL(КодыТоваров.Номенклатура.ЕдиницаИзмерения.Наименование, """") КАК ЕдиницаИзмеренияНаименование,
	|	КодыТоваров.Характеристика.ХарактеристикаНоменклатурыДляЦенообразования КАК ХарактеристикаНоменклатурыДляЦенообразования,
	|
	|	КодыТоваров.Упаковка КАК Упаковка,
	|	ЕСТЬNULL(КодыТоваров.Упаковка.Наименование, """") КАК УпаковкаНаименование,
	|
	...
	|	КодыТоваровПодключаемогоОборудованияOffline.Номенклатура КАК Номенклатура,
	|	КодыТоваровПодключаемогоОборудованияOffline.Артикул КАК Артикул,
	|	КодыТоваровПодключаемогоОборудованияOffline.КодВидаНоменклатурнойКлассификации КАК
	|		КодВидаНоменклатурнойКлассификации,
	|	КодыТоваровПодключаемогоОборудованияOffline.НоменклатураНаименование КАК НоменклатураНаименование,
	|	КодыТоваровПодключаемогоОборудованияOffline.НоменклатураНаименованиеПолное КАК
	|		НоменклатураНаименованиеПолное,
	|	КодыТоваровПодключаемогоОборудованияOffline.ОписаниеТовара КАК
	|		ОписаниеТовара,
	|	КодыТоваровПодключаемогоОборудованияOffline.Характеристика КАК Характеристика,
	|	КодыТоваровПодключаемогоОборудованияOffline.ХарактеристикаНаименование КАК ХарактеристикаНаименование,
	|	КодыТоваровПодключаемогоОборудованияOffline.ХарактеристикаНаименованиеПолное КАК
	|		ХарактеристикаНаименованиеПолное,
	|	КодыТоваровПодключаемогоОборудованияOffline.ЕдиницаИзмеренияКод КАК КодЕдиницыИзмерения,
	|	КодыТоваровПодключаемогоОборудованияOffline.ЕдиницаИзмеренияНаименование КАК
	...
	|		ПО КодыТоваровПодключаемогоОборудованияOffline.ВидНоменклатуры = ВидыНоменклатуры.Ссылка
	...
	|			ИНАЧЕ КодыТоваровПодключаемогоОборудованияOffline.ХарактеристикаНоменклатурыДляЦенообразования
