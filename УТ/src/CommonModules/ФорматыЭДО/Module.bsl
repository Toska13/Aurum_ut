...
// Идентифицирует служебное имя дополнительного поля.
// 
// Параметры:
//  ИмяПоля - Строка
//  Формат - Строка
...
Функция ЭтоСлужебноеИмяДополнительногоПоля(ИмяПоля, Формат) Экспорт
	...
	Возврат Результат ИЛИ ТрансграничныйЭДО.ЭтоСлужебноеИмяДополнительногоПоля(ИмяПоля, Формат);
...
// См. ЭлектронныеДокументыЭДО.ЭтоФормализованныйФорматБезОтветногоТитула
Функция ЭтоФормализованныйФорматБезОтветногоТитула(Формат) Экспорт
	
	ПоддерживаемыеФорматы = ПоддерживаемыеФорматы();
	Возврат Формат = ПоддерживаемыеФорматы.ФНС.СчетНаОплату_5_01;
...
Функция ЗаполнитьДанныеПодписантаДокумента(ОписаниеФайла,
	...
	Если Не ЗначениеЗаполнено(ДолжностьПоСертификату) Тогда
		СвойстваСубъектаСертификата = КриптографияБЭД.СвойстваСубъектаСертификатаПоСсылке(СертификатПодписи);
		СвойстваИздателяСертификата = КриптографияБЭД.СвойстваИздателяСертификатаПоСсылке(СертификатПодписи);
		ВидСертификата = 
		...
		ДолжностьПоСертификату = ?(ВидСертификата = ВидыСертификатов.СертификатИП, "ИП", "");
	КонецЕсли;
	...
	Если Формат = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПродавца
		...
		Иначе
			...
			ИначеЕсли ЗначениеЗаполнено(ДанныеЮрФизЛица.СвидетельствоСерияНомер) Тогда
...
Функция СведенияОПодписанте(ОписаниеФайла, ОписаниеОшибки = "") Экспорт
	...
	ИначеЕсли ЕстьСвойствоXDTO(ДанныеФайлаЭД, "ИдФайл")
		...
		КонецЕсли;
	ИначеЕсли ЕстьСвойствоXDTO(ДанныеФайлаЭД, "ИдФайл")
		И СтрНайти(ДанныеФайлаЭД.ИдФайл, "ON_CHETOP") > 0 Тогда
			
		ВерсияФорматаСчета = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "ВерсФорм", "");
		Если ВерсияФорматаСчета = "5.01" Тогда
			ЭлементыИдентификатора = СтрРазделить(ДанныеФайлаЭД.ИдФайл, "_");
			Результат.ИдентификаторОрганизации = ЭлементыИдентификатора[3];
		КонецЕсли;
	КонецЕсли;
...
Процедура СброситьСведенияОПодписанте(ОписаниеФайла, ОписаниеОшибки = "") Экспорт
	...
	Если ПространствоИмен = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПродавца
		Или ПространствоИмен = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПродавца
		Или ПространствоИмен = ПоддерживаемыеФорматы.ФНС.СчетНаОплату_5_01 Тогда
		ВерсияСхемы = "4.01";
		ЛокальноеИмя = "Файл";
...
// Инициализирует параметры заполнения дополнительных полей
// 
// Параметры:
//  ОписаниеДанныхЭД - см. ДеревоЭлектронногоДокументаБЭД.ДеревоЭлектронногоДокумента -
//  НастройкиОбмена - см. НовыеНастройкиФормированияДокументаПоОбъектуУчета - 
//  НастройкаЗаполнения - см. НастройкаЗаполненияДополнительныхПолей - 
//  СсылкаНаОбъект - ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО, Неопределено -
...
//  * ОписаниеДанныхЭД - см. ДеревоЭлектронногоДокументаБЭД.ДеревоЭлектронногоДокумента -
//  * НастройкиОбмена - см. НовыеНастройкиФормированияДокументаПоОбъектуУчета -
//  * НастройкаЗаполнения - см. НастройкиЭДО.НастройкаЗаполненияДополнительныхПолей -
//  * СсылкаНаОбъект - ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО, Неопределено -
//  * ОбъектУчета - ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО, Неопределено -
//
Функция ПараметрыЗаполненияДополнительныхПолей(ОписаниеДанныхЭД,
	НастройкиОбмена, НастройкаЗаполнения, СсылкаНаОбъект, ОбъектУчета) Экспорт
	
	ПараметрыЗаполнения = Новый Структура();
	ПараметрыЗаполнения.Вставить("ОписаниеДанныхЭД", ОписаниеДанныхЭД);
	ПараметрыЗаполнения.Вставить("НастройкиОбмена", НастройкиОбмена);
	ПараметрыЗаполнения.Вставить("НастройкаЗаполнения", НастройкаЗаполнения);
	ПараметрыЗаполнения.Вставить("СсылкаНаОбъект", СсылкаНаОбъект);
...
//  Настройки - см. НовыеНастройкиФормированияДокументаПоОбъектуУчета
//  Данные - Структура:
//   * ДанныеДокумента - ДеревоЗначений
...
// 
// Возвращаемое значение:
//  см. ДополнитьДокументДаннымиДополнительныхПолей
//
Функция СформироватьЗначенияДополнительныхПолейДокумента(ОбъектУчета, Настройки, Данные, ОтключитьРассчетДополнительныхПолей) Экспорт
	...
	Иначе
		...
		СсылкаНаОбъект = ?(ЗначениеЗаполнено(СчетФактура), СчетФактура, ОбъектУчета);
		ПараметрыЗаполнения = ПараметрыЗаполненияДополнительныхПолей(
			ДанныеДокумента, Настройки, НастройкаЗаполнения, СсылкаНаОбъект, ОбъектУчета);
			НовыеДанныеДляФормированияОсновногоТитула();
		ТрансграничныйЭДО.ДополнитьНастройкуЗаполненияДополнительныхПолей(
			Данные.Участники.ИдентификаторПолучателя, ПараметрыЗаполнения);
		ЗначенияДополнительныхПолей = ДополнитьДокументДаннымиДополнительныхПолей(
			ПараметрыЗаполнения, Данные.ЗначенияДополнительныхПолей);
...
Функция ЗагружаемыйФайлСоответствуетЗагруженномуФайлу(ДанныеДокумента, ЗагружаемыйФайл, ЗагруженныйФайл) Экспорт
	...
	Если ПространствоИменЗагружаемыхДанных = ПространстваИменФорматов.УПД_5_02.ИнформацияПродавца
		Или ПространствоИменЗагружаемыхДанных = ПространстваИменФорматов.УПД_5_03.ИнформацияПродавца
		Или ПространствоИменЗагружаемыхДанных = ПространстваИменФорматов.СчетНаОплату_5_01 Тогда
...
Функция ИдентификаторыДатаНомерДокументаПоДаннымФайла(ОписаниеФайла, ОписаниеОшибки = "") Экспорт
	...
	Если РаспознаниеТекущего.Свойство("ИсходныйФормат") Тогда
		...
		Если РаспознаниеТекущего.ИсходныйФормат = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПродавца
			Или РаспознаниеТекущего.ИсходныйФормат = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПродавца
			Или РаспознаниеТекущего.ИсходныйФормат = ПоддерживаемыеФорматы.ФНС.СчетНаОплату101
			Или РаспознаниеТекущего.ИсходныйФормат = ПоддерживаемыеФорматы.ФНС.СчетНаОплату_5_01 Тогда
...
Функция РасшифровкаВариантовЗаполненияФорматовЭДО() Экспорт
	...
	НоваяСтрока = ВариантыЗаполнения.Добавить();
	НоваяСтрока.Формат = "ON_CHETOP_1_998_01_05_01_01";
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.СчетОферта;
	НоваяСтрока.ВариантЗаполнения = "СчетНаОплату_5_01";
	///////////////////////////////////////////////////////////////////////////////////
...
// Дополняет документ данными дополнительных полей.
// 
// Параметры:
//  ПараметрыЗаполнения - см. ФорматыЭДО.ПараметрыЗаполненияДополнительныхПолей
//  КэшДополнительныхДанных - Структура
// 
// Возвращаемое значение:
//  Структура:
// * Данные - Неопределено, Структура -
// * Ошибки - Неопределено, Структура -
// * Успех - Булево 
//
Функция ДополнитьДокументДаннымиДополнительныхПолей(ПараметрыЗаполнения, КэшДополнительныхДанных)
	...
	Если ПараметрыЗаполнения.НастройкаЗаполнения = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
...
// 
// Параметры:
//  ТаблицаТоваров - Неопределено - Таблица значений, содержит результат вызова см. НоваяТаблицаТоваров
//  НаборДанных - ОбъектXDTO - Источник данных
//  ДопПараметры - Структура -  с полями:
//   * ДеревоДопДанных - ДеревоЗначений - см. ДеревоДопДанных - Дерево содержащее дополнительные данные;
...
Процедура СформироватьТЗПоНаборуДанныхXDTO(ТаблицаТоваров, НаборДанных, ДопПараметры = Неопределено) 
	...
	Для Каждого ТекЭлемент Из НаборДанных Цикл
		...
		Если ДопСведТов <> Неопределено Тогда
			...
			КонецЕсли;
			ГТИН = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДопСведТов, "ГТИН",, "Строка");
			Если ЗначениеЗаполнено(ГТИН) Тогда
				СвойстваНоменклатуры.Штрихкод = ГТИН;
			КонецЕсли;
		...
		СвДТ = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ТекЭлемент, "СвДТ");
		Если СвДТ <> Неопределено Тогда
			СвойстваНоменклатуры.СтранаПроисхожденияКод = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(СвДТ,
				"КодПроисх", , "Строка");
		КонецЕсли;
		...
		НовЗапись = ТаблицаТоваров.Добавить();
		...
		Если ДополнительныеРеквизиты <> Неопределено Тогда
			Для Каждого ЭлементДанных Из ДополнительныеРеквизиты Цикл
				Если ТаблицаТоваров.Колонки.Найти("Доп_" + ЭлементДанных.Наименование) = Неопределено Тогда
					ТаблицаТоваров.Колонки.Добавить("Доп_" + ЭлементДанных.Наименование, ОбщегоНазначения.ОписаниеТипаСтрока(300));
	...
	ДополнитьИнформациюОТовареДаннымиДляСопоставления(ТаблицаТоваров, ЭтоСчетФактураНаАванс, СодержитИзменяемыеДанные);
...
// Заполняет данные дополнительных полей шапки.
// 
// Параметры:
//  ДополнительныеДанные - Неопределено, Структура - Дополнительные данные
//  ПараметрыЗаполнения - см. ФорматыЭДО.ПараметрыЗаполненияДополнительныхПолей
// 
// Возвращаемое значение:
//  Булево
