...
Процедура ЗаполнитьРеквизитыПоВидуНоменклатуры(Номенклатура,
	...
	СтрогоБерутсяИзВида.Добавить("АлкогольнаяПродукция");
	...
	ЗаполняютсяТолькоПриСозданииНоменклатуры.Добавить("КонтролироватьКачествоПоступившегоТовара");
	ЗаполняютсяТолькоПриСозданииНоменклатуры.Добавить("СрокКонтроляКачества");
...
//
// Параметры:
//  Номенклатура - СправочникОбъект.Номенклатура - элемент, для которого нужно отработать логику
//                                                                       связи реквизитов.
//               - ДанныеФормыСтруктура - номенклатура (из формы элемента)
//               - Структура - номенклатура с расширенными полями (из формы элемента)
Процедура ОтработатьЛогикуСвязиРеквизитов(Номенклатура) Экспорт
	
	ОсобенностьУчетаИлиКлючВидовПродукции = НоменклатураЛокализация.ОсобенностьУчетаИлиКлючВидовПродукции(Номенклатура);
	
	Если ТипЗнч(ОсобенностьУчетаИлиКлючВидовПродукции) = Тип("ПеречислениеСсылка.ОсобенностиУчетаНоменклатуры") Тогда
		РеквизитыПризнаковОсобенностейУчета = Справочники.Номенклатура.РеквизитыПризнаковОсобенностейУчета(Метаданные.Справочники.Номенклатура);
		Для Каждого ИмяРеквизита Из РеквизитыПризнаковОсобенностейУчета Цикл
			Номенклатура[ИмяРеквизита] = ОсобенностьУчетаИлиКлючВидовПродукции =
				ПредопределенноеЗначение("Перечисление.ОсобенностиУчетаНоменклатуры." + ИмяРеквизита);
		КонецЦикла;
	КонецЕсли;
	
	Номенклатура.АлкогольнаяПродукция = НоменклатураЛокализация.ЭтоПродукцияЕГАИС(ОсобенностьУчетаИлиКлючВидовПродукции);
		...
		Для каждого КлючЗначение Из ИспользованиеРеквизитов Цикл
			Если Не КлючЗначение.Значение Тогда
				Если МетаданныеОбъекта.Реквизиты.Найти(КлючЗначение.Ключ) <> Неопределено Тогда
					Номенклатура[КлючЗначение.Ключ] = МетаданныеОбъекта.Реквизиты.Найти(КлючЗначение.Ключ).Тип.ПривестиЗначение(Неопределено);
...
Функция РеквизитыПрослеживаемости(КодТНВЭД) Экспорт
	...
	Если ЗначениеЗаполнено(КодТНВЭД) Тогда
		...
		ЗаполнитьЗначенияСвойств(РеквизитыПрослеживаемости, ОбщегоНазначения.ЗначенияРеквизитовОбъекта(КодТНВЭД, ИменаРеквизитов));
...
Процедура ОпределитьПараметрыЗагрузкиДанныхИзФайла(Параметры) Экспорт
	...
	Параметры.ТипДанныхКолонки.Вставить("ПродаетсяПоРазрешению",  ТипБулево);
	...
	Параметры.ТипДанныхКолонки.Вставить("КонтролироватьКачествоПоступившегоТовара",  ТипБулево);
	...
	НеотрицательноеЧисло5_0  = Новый КвалификаторыЧисла(5, 0,  ДопустимыйЗнак.Неотрицательный);
	...
	Параметры.ТипДанныхКолонки.Вставить("СрокКонтроляКачества", Новый ОписаниеТипов("Число", НеотрицательноеЧисло5_0));
...
Функция ТекстЗначенияНеЗаполнен(ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "СхемаОбеспечения" Тогда
		Возврат НСтр("ru = '<не указана>'");
	ИначеЕсли ИмяРаздела = "СрокКонтроляКачества" Тогда
		Возврат НСтр("ru = '0'");
...
Функция ПолучитьИмяОбластиДопСтрокиПоИмениРаздела(ИмяРаздела)
	...
	Если ИмяРаздела = "ДополнительныеРеквизиты"
		...
		Или ИмяРаздела = "КонтролироватьКачествоПоступившегоТовара"
		Или ИмяРаздела = "СрокКонтроляКачества"
...
Функция ГлаваВидна(ИмяГлавы, СтруктураВидимости, КоличествоДопРеквизитов = 0)
	...
	Если ИмяГлавы = "ОбеспечениеИПроизводство" Тогда
		Возврат РазделВиден("ОбособленнаяЗакупкаПродажа", СтруктураВидимости)
					Или РазделВиден("СпособОбеспеченияПотребностей", СтруктураВидимости)
					Или РазделВиден("СхемаОбеспечения", СтруктураВидимости)
					Или РазделВиден("ГиперссылкаПерейтиВариантыКомплектации", СтруктураВидимости)
					Или РазделВиден("ГиперссылкаПерейтиНастройкаПоддержанияЗапасов", СтруктураВидимости)
					Или РазделВиден("ГиперссылкаПерейтиНастройкаСпособовОбеспеченияПотребностей", СтруктураВидимости)
					Или РазделВиден("ГиперссылкаПерейтиНастройкаПередачиМатериаловВПроизводство", СтруктураВидимости)
					Или РазделВиден("ГиперссылкаПерейтиПроизводство", СтруктураВидимости);
	КонецЕсли;
	
	Если ИмяГлавы = "КонтрольКачества" Тогда
		Возврат РазделВиден("КонтролироватьКачествоПоступившегоТовара", СтруктураВидимости)
					Или РазделВиден("СрокКонтроляКачества", СтруктураВидимости);
	КонецЕсли;
...
// Возвращает признак видимости реквизита.
//
// Параметры:
//	Реквизит - Строка, СтрокаТаблицыЗначений - описание реквизита.
//	ИмяРеквизита - Строка - имя реквизита.
//	РежимВидимостиПоказатьТолькоВажные - Булево - признак режима отображения важных реквизитов.
//	СтрокаНастройки - СтрокаТаблицыЗначений - описание настроек реквизита.
//
// Возвращаемое значение:
//	Булево - признак видимости реквизита.
//
...
Функция РеквизитыОтключенныеПоФО(ЭтоРеквизитыХарактеристик = Ложь) Экспорт
	...
	Если Не ИспользоватьКонтрольКачестваТоваров Тогда
		РеквизитыОтключенныеПоФО.Добавить("КонтролироватьКачествоПоступившегоТовара");
		РеквизитыОтключенныеПоФО.Добавить("СрокКонтроляКачества");
	КонецЕсли;
...
//
Функция ОбязательноОтображаемыеПриСозданииРеквизиты(ТипНоменклатуры, ОсобенностьУчетаИлиКлючВидовПродукции)
	...
	Если ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга
		И ОсобенностьУчетаИлиКлючВидовПродукции <> Перечисления.ОсобенностиУчетаНоменклатуры.ОрганизациейПродавцом Тогда
...
Функция НеотображаемыеВТаблицеНастроекРеквизиты() Экспорт
	...
	НеотображаемыеВТаблицеНастроекРеквизиты.Добавить("СрокКонтроляКачества");
	...
	НеотображаемыеВТаблицеНастроекРеквизиты.Добавить("УдалитьПодакцизныйТовар");
...
//	                                        СправочникСсылка.КлючиВидовПродукцииИС - особенность учета номенклатуры.
...
Функция ЗависимостьРеквизитовОтТипаНоменклатуры(ТипНоменклатуры,
	...
	НоваяСтрока.Использование = (ЭтоУслуга
									Или ЭтоРабота)
								И (ОсобенностьУчетаИлиКлючВидовПродукции = Перечисления.ОсобенностиУчетаНоменклатуры.ОрганизациейПоАгентскойСхеме
									Или ОсобенностьУчетаИлиКлючВидовПродукции = Перечисления.ОсобенностиУчетаНоменклатуры.Партнером)
	...
	НоваяСтрока.Использование = (ЭтоУслуга
									Или ЭтоРабота)
								И ОсобенностьУчетаИлиКлючВидовПродукции = Перечисления.ОсобенностиУчетаНоменклатуры.Партнером
								И (Не ИспользоватьХарактеристики
									Или ЭтоРеквизитыХарактеристик);
	...
	НоваяСтрока = ТаблицаРеквизитов.Добавить();
	НоваяСтрока.Реквизит      = Реквизиты.КонтролироватьКачествоПоступившегоТовара;
	НоваяСтрока.Использование = ЭтоТовар
								Или ЭтоМногооборотнаяТара;
	
	НоваяСтрока = ТаблицаРеквизитов.Добавить();
	НоваяСтрока.Реквизит      = Реквизиты.СрокКонтроляКачества;
	НоваяСтрока.Использование = ЭтоТовар
								Или ЭтоМногооборотнаяТара;
...
Функция ВсеРеквизиты(ТипНоменклатуры,
	...
	ОбязательноОтображаемыеПриСозданииРеквизиты   = ОбязательноОтображаемыеПриСозданииРеквизиты(ТипНоменклатуры,
																								ОсобенностьУчетаИлиКлючВидовПродукции);
	РекомендуемыеДляКонтроляУникальностиРеквизиты = РекомендуемыеДляКонтроляУникальностиРеквизиты(ТипНоменклатуры, ОсобенностьУчетаИлиКлючВидовПродукции);
	...
	РеквизитыСПереопределеннойОбязательностьюЗаполнения = РеквизитыСПереопределеннойОбязательностьюЗаполнения(ТипНоменклатуры, ОсобенностьУчетаИлиКлючВидовПродукции, Неопределено);
	...
	ДополнитьВсеРеквизитыРеквизитамиФормы(ВсеРеквизиты);
...
Процедура НастроитьФормуПоТаблицеНастроек(Форма) Экспорт
	...
	ЕстьОписаниеДопРеквизитов = ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеРеквизитыИСведения")
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "Свойства_ОписаниеДополнительныхРеквизитов");
	ИспользоватьНеуникальныеШтрихкодыНоменклатуры = ПолучитьФункциональнуюОпцию("ИспользоватьНеуникальныеШтрихкодыНоменклатуры");
	...
	Для Каждого СтрокаТаблицыРеквизитов Из Форма.ТаблицаНастроекРеквизитов Цикл
		...
		Если СтрокаТаблицыРеквизитов.ЭтоДопРеквизит Тогда
			...
			Если Не ЕстьОписаниеДопРеквизитов Тогда
				Продолжить;
			КонецЕсли;
		...
		Если СтрокаТаблицыРеквизитов.Уникален
			Или (ИмяЭлементаФормы = "Штрихкод"
				И Не ИспользоватьНеуникальныеШтрихкодыНоменклатуры) Тогда
...
Функция РеквизитыСПереопределеннойОбязательностьюЗаполнения(ТипНоменклатуры, ОсобенностьУчетаИлиКлючВидовПродукции, Объект) Экспорт
	...
	РеквизитыСПереопределеннойОбязательностьюЗаполнения.Вставить("Принципал", ОсобенностьУчетаИлиКлючВидовПродукции = Перечисления.ОсобенностиУчетаНоменклатуры.ОрганизациейПоАгентскойСхеме
																		Или ОсобенностьУчетаИлиКлючВидовПродукции = Перечисления.ОсобенностиУчетаНоменклатуры.Партнером);
																		
	РеквизитыСПереопределеннойОбязательностьюЗаполнения.Вставить("Контрагент", ОсобенностьУчетаИлиКлючВидовПродукции = Перечисления.ОсобенностиУчетаНоменклатуры.Партнером);
	...
	НоменклатураЛокализация.РеквизитыСПереопределеннойОбязательностьюЗаполнения(
		РеквизитыСПереопределеннойОбязательностьюЗаполнения, ТипНоменклатуры, ОсобенностьУчетаИлиКлючВидовПродукции, Объект);
...
Функция РекомендуемыеДляКонтроляУникальностиРеквизиты(ТипНоменклатуры, ОсобенностьУчетаИлиКлючВидовПродукции)
	
	РекомендуемыеДляКонтроляУникальностиРеквизиты = Новый Массив;
	
	НоменклатураЛокализация.РекомендуемыеДляКонтроляУникальностиРеквизиты(
		РекомендуемыеДляКонтроляУникальностиРеквизиты, ТипНоменклатуры, ОсобенностьУчетаИлиКлючВидовПродукции); 
...
Процедура ПроверитьУникальностьЭлементаПоРеквизитам(Форма) Экспорт
	...
	РеквизитыФормы		= ?(ЭтоНоменклатура, РеквизитыФормыТаблицыНастроек(), Неопределено);
	ОтборСтрок			= Новый Структура("Уникален", Истина);
	УникальныеРеквизиты	= Форма.ТаблицаНастроекРеквизитов.НайтиСтроки(ОтборСтрок);
	
	Если РеквизитыФормы <> Неопределено Тогда
		РеквизитыКонтроляУникальности = Новый Массив;
		
		Для Каждого РеквизитКонтроля Из УникальныеРеквизиты Цикл
			Если РеквизитыФормы.Получить(РеквизитКонтроля.ИмяРеквизита) = Неопределено Тогда
				РеквизитыКонтроляУникальности.Добавить(РеквизитКонтроля);
			КонецЕсли;
		КонецЦикла;
	Иначе
		РеквизитыКонтроляУникальности = УникальныеРеквизиты;
	КонецЕсли;
	...
	Для Каждого СтрокаУсловия Из РеквизитыКонтроляУникальности Цикл
		Если Не СтрокаУсловия.ЭтоДопРеквизит Тогда
			...
			Если ПустаяСтрока(СтрокаУсловия.ПутьКДанным) Тогда
				ПутьКДанным = СтрокаУсловия.ИмяРеквизита;
			Иначе
				ПутьКДанным = СтрокаУсловия.ПутьКДанным;
			КонецЕсли;
			
			Запрос.УстановитьПараметр(СтрокаУсловия.ИмяРеквизита, Форма.Объект[ПутьКДанным]);
		Иначе
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
								",&ТекстЗапросаСоединенияСТЧДопРеквизитов",
								СтрСоединить(ТекстыСоединения));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
								"&ТекстЗапросаУсловияПоРеквизитам",
								СтрСоединить(ТекстыУсловий, Символы.ПС + "И" + Символы.НПП));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
								"ИмяПроверяемогоСправочника",
								?(ЭтоНоменклатура, "Номенклатура", "ХарактеристикиНоменклатуры"));
...
Функция ВидыНоменклатурыИдентичныеПоНастройкамКонтроляУникальности(Объект)
	...
	|		И РеквизитыДляКонтроля.ИмяРеквизита В (&ИсключаемыеРеквизиты))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	ИсключаемыеРеквизиты.Добавить("ВидНоменклатуры");
	
	РеквизитыФормы = РеквизитыФормыТаблицыНастроек();
	
	Для Каждого КлючИЗначение Из РеквизитыФормы Цикл
		ИсключаемыеРеквизиты.Добавить(КлючИЗначение.Ключ);
	КонецЦикла;
	
	Запрос.УстановитьПараметр("ВидНоменклатуры",		Объект.ВидНоменклатуры);
	Запрос.УстановитьПараметр("ТипНоменклатуры",		Объект.ТипНоменклатуры);
	Запрос.УстановитьПараметр("ИсключаемыеРеквизиты",	ИсключаемыеРеквизиты);
...
// Возвращает коллекция реквизитов формы справочника, используемых в настройках контроля.
//
// Возвращаемое значение:
//	Соответствие Из КлючИЗначение - коллекция реквизитов формы справочника, где:
//		* Ключ - Строка - имя реквизита.
//		* Значение - Произвольный - значение реквизита по умолчанию.
//
Функция РеквизитыФормыТаблицыНастроек() Экспорт
	
	РеквизитыФормы = Новый Соответствие;
	РеквизитыФормы.Вставить("Штрихкод", "");
...
Функция ТаблицаНастроекРеквизитов(ВидНоменклатуры,
	...
	|ПОМЕСТИТЬ РеквизитыДляКонтроляТаблица
	|ИЗ
	...
	|		РеквизитыДляКонтроля.Использование КАК Использование,
	|		ЛОЖЬ КАК ЕстьРеквизитВМетаданных
	|	ИЗ
	|		РеквизитыДляКонтроляТаблица КАК РеквизитыДляКонтроля
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ВТДляЗапроса.ЕстьРеквизитВМетаданных
	|			ТОГДА ЕСТЬNULL(ВсеРеквизитыНоменклатурыТаблица.Представление, """")
	|		ИНАЧЕ ЕСТЬNULL(РеквизитыДляКонтроляТаблица.Представление, """")
	|	КОНЕЦ КАК Представление,
	...
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВсеРеквизитыНоменклатурыТаблица КАК ВсеРеквизитыНоменклатурыТаблица
	|		ПО ВТДляЗапроса.ИмяРеквизита = ВсеРеквизитыНоменклатурыТаблица.Имя
	|			И ВТДляЗапроса.ЕстьРеквизитВМетаданных = ИСТИНА
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ РеквизитыДляКонтроляТаблица КАК РеквизитыДляКонтроляТаблица
	|		ПО ВТДляЗапроса.ИмяРеквизита = РеквизитыДляКонтроляТаблица.ИмяРеквизита
	|			И ВТДляЗапроса.ЕстьРеквизитВМетаданных = ЛОЖЬ
	|ГДЕ
	...
	|
	|ВЫБРАТЬ
	|	ВТВсеДополнительныеРеквизиты.Свойство.Представление	КАК Представление,
	|	ИСТИНА												КАК ЭтоДопРеквизит,
	|	ВТДляЗапроса.ИмяРеквизита							КАК ИмяРеквизита,
	|	ВТВсеДополнительныеРеквизиты.Свойство				КАК Свойство,
	|	ВТВсеДополнительныеРеквизиты.ЗаполнятьОбязательно	КАК ЗаполнятьОбязательно,
	|	ЛОЖЬ												КАК Уникален,
	|	ВТВсеДополнительныеРеквизиты.ЗаполнятьОбязательно	КАК ОтображатьПриСоздании,
	|	ИСТИНА												КАК Использование,
	|	ВТВсеДополнительныеРеквизиты.Тип					КАК Тип,
	|	ВТВсеДополнительныеРеквизиты.Набор					КАК Набор,
	|	ИСТИНА												КАК ЭтоОбщийРеквизит,
	|	ВТВсеДополнительныеРеквизиты.НаборСвойств			КАК НаборСвойств
	|ИЗ
	|	ВТВсеДополнительныеРеквизиты КАК ВТВсеДополнительныеРеквизиты
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДляЗапроса КАК ВТДляЗапроса
	|		ПО ВТДляЗапроса.Свойство = ВТВсеДополнительныеРеквизиты.Свойство
	|
	|ГДЕ
	|	ВТДляЗапроса.ЭтоДопРеквизит ЕСТЬ NULL";
...
Функция РеквизитыПризнаковОсобенностейУчета(МетаданныеОбъекта) Экспорт
	...
	ОсобенностиУчета.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.СодержитДрагоценныеМатериалы);
	
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		ОсобенностиУчета,
		Перечисления.ОсобенностиУчетаНоменклатуры.ОсобенностиУчетаУслуг());
...
Функция ИспользованиеЭлементов(ПереданныйОбъект, РежимВидимостиПоказатьТолькоВажные, РежимПроверкиИспользованияРеквизитов) Экспорт
	...
	ОсобенностьУчетаИлиКлючВидовПродукции = НоменклатураЛокализация.ОсобенностьУчетаИлиКлючВидовПродукции(ПереданныйОбъект);
	...
	Для Каждого СтрТабл Из НастройкиРеквизитовПоВидуНоменклатуры Цикл
		...
		Иначе
			...
			Иначе
				Реквизит = ?(РеквизитыФормы.Получить(ИмяРеквизита) = Неопределено, ИмяРеквизита, СтрТабл);
				
				ИспользованиеЭлементов.Вставить(ИмяРеквизита, СтрТабл.Использование
															И (РежимПроверкиИспользованияРеквизитов
																Или ВидимостьРеквизита(Реквизит,
																				ИмяРеквизита,
																				РежимВидимостиПоказатьТолькоВажные,
																				СтрТабл)));
			КонецЕсли;
	...
	Если Не РежимПроверкиИспользованияРеквизитов Тогда
		ВидимостьЭлемента = Объект.КонтролироватьКачествоПоступившегоТовара
			И ИспользованиеЭлементов.КонтролироватьКачествоПоступившегоТовара;
		ИспользованиеЭлементов.Вставить("СрокКонтроляКачества", ВидимостьЭлемента);
		ВидимостьЭлемента = ВидимостьЭлемента И ИспользоватьКонтрольКачестваТоваров;
		ИспользованиеЭлементов.Вставить("ОтступСрокКонтроляКачестваКартинка", ВидимостьЭлемента);
		ИспользованиеЭлементов.Вставить("НадписьСрокКонтроляДней", ВидимостьЭлемента);
	КонецЕсли;
	...
	#Область ГруппаАкциза
	
	Если Не РежимПроверкиИспользованияРеквизитов Тогда
		
		Если ЭтоТовар Тогда
			ТаблицаКатегорий = УчетПодакцизныхТоваров.ПодакцизныеКатегорииТовара(Объект.Ссылка);
			ЕстьКатегория = ЗначениеЗаполнено(ТаблицаКатегорий)
				И ЗначениеЗаполнено(ТаблицаКатегорий[0].КатегорияПодакцизногоТовара);
			Если ЕстьКатегория Тогда
				ЕстьИстория = Истина;
			Иначе
				ТаблицаИсторииКатегорий = УчетПодакцизныхТоваров.ПолучитьИсториюКатегорийНоменклатуры(Объект.Ссылка);
			ЕстьИстория = 
				ЗначениеЗаполнено(ТаблицаИсторииКатегорий)
				И (ЗначениеЗаполнено(ТаблицаИсторииКатегорий[0].КатегорияПодакцизногоТовара)
					Или ТаблицаИсторииКатегорий.Количество() > 1);
			КонецЕсли;
		Иначе
			ЕстьКатегория = Ложь;
			ЕстьИстория = Ложь;
		...
		ИспользованиеЭлементов.Вставить("ГруппаКатегорий", ЭтоТовар И ИспользоватьПодакцизныеТовары);
		ИспользованиеЭлементов.Вставить("ГиперссылкаПерейтиТекущаяПодакцизнаяКатегорияНоменклатуры", ЭтоТовар
			И ИспользоватьПодакцизныеТовары);
		ИспользованиеЭлементов.Вставить("ГиперссылкаПерейтиПодакцизныеКатегорииНоменклатуры", ЭтоТовар
			И ИспользоватьПодакцизныеТовары
			И ЕстьИстория);
		ИспользованиеЭлементов.Вставить("ГруппаОписаниеПодакцизнойКатегории", ЭтоТовар
			И ИспользоватьПодакцизныеТовары
			И ЕстьКатегория);
...
//		* ГиперссылкаПерейтиПодакцизныеКатегорииНоменклатуры - Строка
//		* ГиперссылкаПерейтиТекущаяПодакцизнаяКатегорияНоменклатуры - Строка
//		* ОписаниеПодакцизнойКатегории - Строка
//
Функция ЗаголовкиЭлементов(Объект, ВидимостьЭлементов)
	...
	Если РазделВиден("ГиперссылкаПерейтиСоставНабора", ВидимостьЭлементов) Тогда
		...
		|	И НЕ ВариантыКомплектацииНоменклатуры.Ссылка.ПометкаУдаления";
		ТекстыЗапроса.Добавить(ТекстЗапроса, "ГиперссылкаПерейтиСоставНабора");
	КонецЕсли;
	
	Если РазделВиден("ГиперссылкаПерейтиВариантыКомплектации", ВидимостьЭлементов) Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ВариантыКомплектацииНоменклатуры.Ссылка КАК Ссылка
		|ИЗ
		...
		ТекстыЗапроса.Добавить(ТекстЗапроса, "ГиперссылкаПерейтиВариантыКомплектации");
	КонецЕсли;
	...
	Если РазделВиден("ГиперссылкаПерейтиСоставНабора", ВидимостьЭлементов) Тогда
		...
		Если Количество <> 0 Тогда
			ИмяГиперссылки = ?(ИспользуютсяХарактеристики, "НастроитьНабор", "СоставНабора");
			ЗаголовокГиперссылки = Новый ФорматированнаяСтрока(НСтр("ru = 'Состав набора'") + " (" + Количество + ")",,,,ИмяГиперссылки);
		Иначе
		...
		ВидимостьЭлементов.Вставить("ГиперссылкаПерейтиСоставНабораВнимание", Количество = 0);
	ИначеЕсли РазделВиден("ГиперссылкаПерейтиВариантыКомплектации", ВидимостьЭлементов) Тогда
		Количество = Таблицы.ГиперссылкаПерейтиВариантыКомплектации.Количество();
	...
	Если РазделВиден("ГиперссылкаПерейтиПодакцизныеКатегорииНоменклатуры", ВидимостьЭлементов) Тогда
		
		ТекстЗаголовка = "";
		Количество = 0;
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			ТаблицаАкцизов = УчетПодакцизныхТоваров.ПолучитьИсториюКатегорийНоменклатуры(Объект.Ссылка);
			Количество = ТаблицаАкцизов.Количество();
		КонецЕсли;
		Если Количество > 0 Тогда
			ТекстЗаголовка = НСтр("ru='История категорий (%1)'");
			ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%1", Количество);
		КонецЕсли;
		
		ЗаголовокГиперссылки = Новый ФорматированнаяСтрока(ТекстЗаголовка, , , , "КатегорииПодакцизныхТоваров");
		
		ЗаголовкиЭлементов.Вставить("ГиперссылкаПерейтиПодакцизныеКатегорииНоменклатуры", ЗаголовокГиперссылки);
		
	КонецЕсли;

	Если РазделВиден("ГиперссылкаПерейтиТекущаяПодакцизнаяКатегорияНоменклатуры", ВидимостьЭлементов) Тогда
		
		Количество = 0;
		ТекстЗаголовкаОписания = "";
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			ТаблицаКатегорий = УчетПодакцизныхТоваров.ПодакцизныеКатегорииТовара(Объект.Ссылка);
			Количество = ТаблицаКатегорий.Количество();
		КонецЕсли;
		
		Если Количество = 0 Тогда
			ТекстЗаголовка = НСтр("ru = '<Неподакцизный товар>'");
			ЗаголовокГиперссылки = Новый ФорматированнаяСтрока(ТекстЗаголовка, , , , "НоваяКатегорияНоменклатуры");
		ИначеЕсли Не ЗначениеЗаполнено(ТаблицаКатегорий[0].КатегорияПодакцизногоТовара) Тогда
			Если ПолучитьФункциональнуюОпцию("БазоваяВерсия") Тогда
				ТекстЗаголовка = НСтр("ru = '<Подакцизный товар>'");
			Иначе
				ТекстЗаголовка = НСтр("ru = '<Подакцизный товар. Указать категорию>'");
			КонецЕсли;
			ЗаголовокГиперссылки = Новый ФорматированнаяСтрока(ТекстЗаголовка, , , , "НоваяКатегорияНоменклатуры");
		ИначеЕсли Количество = 1 Тогда
			ТекстЗаголовка = Строка(ТаблицаКатегорий[0].КатегорияПодакцизногоТовара);
			ЗаголовокГиперссылки = Новый ФорматированнаяСтрока(ТекстЗаголовка, , , , "ТекущаяПодакцизнаяКатегорияНоменклатуры");
			Если ТаблицаКатегорий[0].ТипСтавки = Перечисления.ТипыСтавокПодакцизнойКатегории.Твердая Тогда
				ТекстЗаголовкаОписания = Строка(ТаблицаКатегорий[0].Ставка) + " " + Строка(ТаблицаКатегорий[0].Валюта)
					+ " " + НСтр("ru = 'за'") + " " + Строка(ТаблицаКатегорий[0].ЕдиницаИзмерения);
			ИначеЕсли ТаблицаКатегорий[0].ТипСтавки = Перечисления.ТипыСтавокПодакцизнойКатегории.Адвалорная Тогда
				ТекстЗаголовкаОписания = НСтр("ru = 'Адвалорная ставка'");
			КонецЕсли;
		Иначе
		...
		ЗаголовкиЭлементов.Вставить("ГиперссылкаПерейтиТекущаяПодакцизнаяКатегорияНоменклатуры", ЗаголовокГиперссылки);
		ЗаголовкиЭлементов.Вставить("ОписаниеПодакцизнойКатегории", ТекстЗаголовкаОписания);
...
Процедура ДополнитьВсеРеквизитыРеквизитамиФормы(ВсеРеквизиты)
	
	// Описание реквизита формы Штрихкод
	ЗначениеСвойств = ЗначениеСвойствРеквизита();
	ЗначениеСвойств.Имя = "Штрихкод";
	ЗначениеСвойств.Использование = Истина;
	ЗначениеСвойств.Представление = НСтр("ru = 'Штрихкод'");
	ЗначениеСвойств.ОбязательныйДляЗаполнения = Ложь;
	ЗначениеСвойств.Тип = Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(200, ДопустимаяДлина.Переменная));
	ЗначениеСвойств.ОбязательныйДляОтображенияПриСоздании = Истина;
	ЗначениеСвойств.РекомендуемыйДляКонтроляУникальности =
		Не ПолучитьФункциональнуюОпцию("ИспользоватьНеуникальныеШтрихкодыНоменклатуры");
	
	ВсеРеквизиты.Вставить(ЗначениеСвойств.Имя, Новый ФиксированнаяСтруктура(ЗначениеСвойств));
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("1ca93838-ddb9-4d7f-a184-a7471d68255c");
	...
	СписокОписаний.Добавить(НСтр("ru = 'Обновление справочника ""Номенклатура"":'"));
	СписокОписаний.Добавить(НСтр("ru = '- замена код ОКВЭД2 с типом справочник Классификатор ОКВЭД2 на строка';"));
	СписокОписаний.Добавить(НСтр("ru = '- очистка особенности учета для номенклатуры, подконтрольной государственным информационным системам';"));
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Справочник.Номенклатура КАК Номенклатура
	|ГДЕ
	|	НЕ Номенклатура.УдалитьКодОКВЭД2 = ЗНАЧЕНИЕ(Справочник.КлассификаторОКВЭД2.ПустаяСсылка)
	|	И НЕ Номенклатура.ЭтоГруппа
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний = Новый Массив;
	СписокОписаний.Добавить(НСтр("ru = 'Не удалось обработать справочник ""Номенклатура"" по обработчику:'"));
	СписокОписаний.Добавить(НСтр("ru = '- замена код ОКВЭД2 с типом справочник Классификатор ОКВЭД2 на строка';"));
	СписокОписаний.Добавить(НСтр("ru = '- очистка особенности учета для номенклатуры, подконтрольной государственным информационным системам';"));
	...
	Запрос.Текст = 
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК ДанныеСправочника
	...
	Пока Справочник.Следующий() Цикл
		...
		Попытка
			...
			Если СправочникОбъект <> Неопределено Тогда
				...
				#Область КодОКВЭД2
				Если ЗначениеЗаполнено(Справочник.КодОКВЭД2)
					И Не ЗначениеЗаполнено(СправочникОбъект.КодОКВЭД2) Тогда
					СправочникОбъект.КодОКВЭД2 = Справочник.КодОКВЭД2;
					ОбъектИзменен = Истина;
				КонецЕсли;
				#КонецОбласти
				
				#Область ОсобенностьУчетаИС
