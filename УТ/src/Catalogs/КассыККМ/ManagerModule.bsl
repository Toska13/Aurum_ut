...
// Определяет, создан ли хоть один элемент (фискальный регистратор) справочника Касса ККМ или нет
// 
// Возвращаемое значение:
//  Булево - создан один или несколько фискальных регистраторов
Функция КассаККМСозданаВСистеме() Экспорт  
	
	КассаККМСозданаВСистеме = Ложь;
	...
	Если Выборка.Следующий() Тогда
		КассаККМСозданаВСистеме = Истина;
	КонецЕсли;	
	
	Возврат КассаККМСозданаВСистеме;        
...
//
// Параметры:
//  Организация - СправочникСсылка.Организации - Отбор по владельцу (организация, которой принадлежит касса ККМ).
//
// Возвращаемое значение:
//	СправочникСсылка.КассыККМ - Найденная касса ККМ.
//
Функция КассаККМПоУмолчанию(Организация = Неопределено) Экспорт
	...
	ТекстЗаменыУсловиеОтбораПоОрганизации = "ИСТИНА";
	Если ЗначениеЗаполнено(Организация) Тогда
		ТекстЗаменыУсловиеОтбораПоОрганизации = "КассыККМ.Владелец = &Организация";
		Запрос.Параметры.Вставить("Организация", Организация);
	КонецЕсли;
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеОтбораПоОрганизации", ТекстЗаменыУсловиеОтбораПоОрганизации);
