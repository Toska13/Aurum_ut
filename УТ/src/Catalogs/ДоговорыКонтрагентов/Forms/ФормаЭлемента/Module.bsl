...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.ВариантОформленияДокументовПоступленияПоУслугам) Тогда
			Объект.ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугНеИспользуется;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Объект.ВариантОпределенияГрафикаИсполненияВДоговоре) Тогда
			Объект.ВариантОпределенияГрафикаИсполненияВДоговоре = Перечисления.ВариантыОпределенияГрафикаИсполненияВДоговоре.ЗаказыНакладные;
		КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("УправлениеТорговлей") Тогда
		ЭлементДляУдаления = Элементы.ВариантОформленияДокументовПоступленияПоУслугам.СписокВыбора.НайтиПоЗначению(
			Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется);
			
		Если Не ЭлементДляУдаления = Неопределено Тогда
			Элементы.ВариантОформленияДокументовПоступленияПоУслугам.СписокВыбора.Удалить(ЭлементДляУдаления);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьПодсказкуРаздельнаяЗакупкаРаботУслуг();
...
Процедура ПартнерПриИзмененииНаСервере()
	...
	ИсходныйДоговорССамозанятым = Объект.ДоговорССамозанятым;
	...
	Если ИсходныйДоговорССамозанятым <> Объект.ДоговорССамозанятым Тогда
...
Процедура ФиксированнаяСуммаДоговораПриИзменении(Элемент)
	...
	ВариантРегулярныеПлатежи = ПредопределенноеЗначение("Перечисление.ВариантыОпределенияГрафикаИсполненияВДоговоре.РегулярныеПлатежи");
	ВариантЗаказыНакладные = ПредопределенноеЗначение("Перечисление.ВариантыОпределенияГрафикаИсполненияВДоговоре.ЗаказыНакладные");
	
	Если Не ФиксированнаяСуммаДоговора
		И Не Объект.ВариантОпределенияГрафикаИсполненияВДоговоре = ВариантРегулярныеПлатежи Тогда
		Объект.ЗаданГрафикИсполнения = Ложь;
		Объект.ВариантОпределенияГрафикаИсполненияВДоговоре = ВариантЗаказыНакладные;
		Объект.Сумма = 0;
...
Процедура НадписьГрафикИсполненияНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Или Модифицированность Тогда
		...
		Возврат;
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Дата)
		И Не ЗначениеЗаполнено(Объект.ГрафикИсполненияДоговора) Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиент.СообщитьПользователю(
...
&НаКлиенте
Процедура ИспользоватьОформлениеДокументовРаздельнойЗакупкиТоваровПриИзменении(Элемент)
	
	Если ИспользоватьОформлениеДокументовРаздельнойЗакупкиТоваров Тогда
	...
	ИспользоватьОформлениеДокументовРаздельнойЗакупкиТоваровПриИзмененииСервер();
...
Процедура ВариантОформленияЗакупокПриИзменении(Элемент)
	...
	ВариантОформленияЗакупокПриИзмененииСервер();
...
&НаКлиенте
Процедура ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслугПриИзменении(Элемент)
	
	Если ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслуг Тогда
		Объект.ВариантОформленияДокументовПоступленияПоУслугам = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется");
	Иначе
		Объект.ВариантОформленияДокументовПоступленияПоУслугам = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугНеИспользуется");
	КонецЕсли;
	
	ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслугПриИзмененииСервер();
	
	Если Объект.ЕжемесячноеПоступлениеУслуг И Не Объект.ЕжемесячноеПоступлениеУслуг = Объект.ВариантОформленияДокументовПоступленияПоУслугам = 
		ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется") Тогда
		Объект.ЕжемесячноеПоступлениеУслуг = Ложь;
	КонецЕсли;
...
&НаКлиенте
Процедура ВариантОформленияДокументовПоступленияПоУслугамПриИзменении(Элемент)
	
	УстановитьПодсказкуРаздельнаяЗакупкаРаботУслуг();
	ВариантОформленияДокументовПоступленияПоУслугамПриИзмененииСервер();
	
	Если Объект.ЕжемесячноеПоступлениеУслуг И Не Объект.ВариантОформленияДокументовПоступленияПоУслугам = 
		ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется") Тогда
		Объект.ЕжемесячноеПоступлениеУслуг = Ложь;
	КонецЕсли;
...
&НаКлиенте
Процедура ОформлениеПоступленияУслугПриИзменении(Элемент)
	
	Модифицированность = Истина;
	ПриИзмененииНастроекПриемкиУслугСервер();	
	
КонецПроцедуры

&НаКлиенте
Процедура ПриемкаУслугПриИзменении(Элемент)
...
Процедура ВестиРасчетыЧерезКонечныхПокупателейПриИзменении(Элемент)
	...
	ВестиРасчетыЧерезКонечныхПокупателейПриИзмененииНаСервере();
...
&НаКлиенте
Процедура ВариантОпределенияГрафикаИсполненияВДоговореПриИзменении(Элемент)
	
	ВариантДоговор = ПредопределенноеЗначение("Перечисление.ВариантыОпределенияГрафикаИсполненияВДоговоре.Договор");
	ВариантРегулярныеПлатежи = ПредопределенноеЗначение("Перечисление.ВариантыОпределенияГрафикаИсполненияВДоговоре.РегулярныеПлатежи");
	
	Объект.ЗаданГрафикИсполнения = (Объект.ВариантОпределенияГрафикаИсполненияВДоговоре = ВариантДоговор
		Или Объект.ВариантОпределенияГрафикаИсполненияВДоговоре = ВариантРегулярныеПлатежи);
	
	УправлениеЭлементамиГрафикИсполнения();
	УстановитьВидимостьРегулярныеПлатежи();
...
&НаСервере
Процедура УстановитьДоступностьВариантОформленияЗакупок()

	ИспользоватьОформлениеДокументовРаздельнойЗакупкиТоваров = Не Объект.ВариантОформленияЗакупок = Перечисления.ВариантыОформленияЗакупок.НеРазделять;
	
	Элементы.ВариантОформленияЗакупок.Доступность = ИспользоватьОформлениеДокументовРаздельнойЗакупкиТоваров;
	
	ЗаполнитьСписокВыбораВариантаОформленияЗакупок();
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьСписокВыбораВариантаОформленияЗакупок()
	
	НеотфактурованныеПоставкиТоваровИУслуг = Перечисления.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваровИУслуг;
	
	НайденныйЭлемент = Элементы.ВариантОформленияЗакупок.СписокВыбора.НайтиПоЗначению(НеотфактурованныеПоставкиТоваровИУслуг);
	
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаПоИмпортуТоварыВПути Тогда
		Если НайденныйЭлемент <> Неопределено Тогда
			Элементы.ВариантОформленияЗакупок.СписокВыбора.Удалить(НайденныйЭлемент);
		КонецЕсли;
	ИначеЕсли ИспользоватьНеотфактурованныеПоставки Тогда
		Если НайденныйЭлемент = Неопределено Тогда
			Элементы.ВариантОформленияЗакупок.СписокВыбора.Добавить(НеотфактурованныеПоставкиТоваровИУслуг,
				НСтр("ru = 'По схеме ""Неотфактурованные поставки товаров""'"));
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура УстановитьДоступностьВариантОформленияДокументовПоступленияПоУслугам(УстанавливатьПриемкуУслуг = Истина)

	ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслуг = Объект.ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется
		Или Объект.ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется;
	
	Элементы.ВариантОформленияДокументовПоступленияПоУслугам.Доступность = ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслуг;
	
	Если Не (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС
		Или Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки
		...
		Или Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки) Тогда
		Объект.ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугНеИспользуется;
		Элементы.ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслуг.Доступность = Ложь;
		Элементы.ВариантОформленияДокументовПоступленияПоУслугам.Доступность = Ложь; 
	Иначе
		Элементы.ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслуг.Доступность = Истина;
		Элементы.ВариантОформленияДокументовПоступленияПоУслугам.Доступность = ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслуг;
	КонецЕсли;
	
	УстановитьДоступностьОформлениеПоступленияУслуг();
	УстановитьПараметрыПриемкиУслуг(УстанавливатьПриемкуУслуг);

КонецПроцедуры

&НаСервере
Процедура УстановитьДоступностьОформлениеПоступленияУслуг()

    Элементы.ОформлениеПоступленияУслуг.Доступность = Элементы.ВариантОформленияДокументовПоступленияПоУслугам.Доступность;
...
Процедура ТипДоговораПриИзмененииСервер()
	...
	Если ИспользоватьОформлениеДокументовРаздельнойЗакупкиТоваров
	...
	ДоставкаТоваров.ПриЧтенииСозданииРаспоряженийНаСервере(Элементы, Объект);
	ЗаполнениеОбъектовПоСтатистике.ЗаполнитьПодчиненныеРеквизитыОбъекта(Объект, "ТипДоговора");
	
	ЗаполнитьПараметрыВыбораПартнера();
	ЗаполнитьСписокВыбораВариантаОпределенияГрафикаИсполненияВДоговоре(
		Элементы.ВариантОпределенияГрафикаИсполненияВДоговоре.СписокВыбора, Объект.ТипДоговора);
	...
	УстановитьВидимостьКонтроляКачестваТоваров();
	УстановитьДоступностьВариантОформленияЗакупок();
	УстановитьДоступностьВариантОформленияДокументовПоступленияПоУслугам();
...
Функция ПроверитьФлагиПартнераКлиентПоставщик(Партнер, ХозяйственнаяОперация, ЭтоНовый)
	...
	Возврат Справочники.ДоговорыКонтрагентов.ПроверитьФлагиПартнераКлиентПоставщик(Партнер, ХозяйственнаяОперация,
		ЭтоНовый, Истина);
...
Процедура ПорядокРасчетовПриИзмененииСервер()
	...
	УстановитьВидимостьЦентрализованногоДоговора();
	УправлениеЭлементамиВариантКурсаДоговора();
	ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтаФорма, "ПорядокРасчетов");
...
Процедура ОбработатьБанковскиеСчета()
	...
	ЗаполнениеОбъектовПоСтатистике.ЗаполнитьПодчиненныеРеквизитыОбъекта(Объект, "Организация", , Истина);
	...
	ЗаполнениеОбъектовПоСтатистике.ЗаполнитьПодчиненныеРеквизитыОбъекта(Объект, "Контрагент", , Истина);
...
&НаСервере
Процедура ВариантОформленияДокументовПоступленияПоУслугамПриИзмененииСервер()
	
	УстановитьДоступностьЕжемесячноеПоступлениеУслуг();
	
	ПриИзмененииНастроекПриемкиУслугСервер();
	
	УстановитьДоступностьОформлениеПоступленияУслуг();
	
	ПриОкончанииИзмененияРеквизитаЛокализацииНаСервере("ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслуг", Неопределено);
	
КонецПроцедуры

&НаСервере
Процедура УстановитьДоступностьЕжемесячноеПоступлениеУслуг()

	Элементы.ЕжемесячноеПоступлениеУслуг.Доступность = Объект.ВариантОформленияДокументовПоступленияПоУслугам =
		Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется;

КонецПроцедуры

&НаКлиенте
Процедура УстановитьПодсказкуРаздельнаяЗакупкаРаботУслуг()

	Если Объект.ВариантОформленияДокументовПоступленияПоУслугам = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется") Тогда
		Элементы.ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслуг.Подсказка = НСтр("ru = 'Поступление услуг без первичных документов регистрируется документом ""Поступление услуг в подразделение"". 
			|При поступлении первичных документов поступление услуг сторнируется и регистрируется документ 
			|""Приобретение товаров и услуг"" с хозяйственной операцией ""Закупка у поставщика"".'");
	ИначеЕсли Объект.ВариантОформленияДокументовПоступленияПоУслугам = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется") Тогда
		Элементы.ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслуг.Подсказка = НСтр("ru = 'Поступление работ и услуг без первичных документов регистрируется документом ""Поступление услуг в подразделение"",  
			|по которому в дальнейшем производится ""фактуровка"" с использованием документа ""Приобретение товаров и услуг"" 
			|с хозяйственной операцией ""Закупка у поставщика (фактуровка поставки)"". Отраженные расходы корректируются на 
			|разницу между приобретением и поступлением.'");
	Иначе
		Элементы.ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслуг.Подсказка = НСтр("ru = 'Для раздельной закупки работ, услуг доступны следующие варианты: ""Поступление услуг фактурируется"" и 
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Если Не ЗначениеЗаполнено(Объект.ВариантПриемкиУслуг) Тогда
		Объект.ВариантПриемкиУслуг = Константы.ВариантПриемкиТоваров.Получить();
	КонецЕсли;	
	...
	ЗакупкиСервер.ЗаполнитьНастройкиВариантовПриемки(Объект.ВариантПриемкиУслуг, ОформлениеПоступленияУслуг, ПриемкаУслуг);
	ЗакупкиСервер.ЗаполнитьСписокВыбораВариантовПриемкиУслугПоДоговорам(Элементы.ОформлениеПоступленияУслуг,
																	Элементы.ПриемкаУслуг,
																	Объект.ВариантОформленияДокументовПоступленияПоУслугам);	
	
	Если Элементы.ОформлениеПоступленияУслуг.СписокВыбора.НайтиПоЗначению(ОформлениеПоступленияУслуг) = Неопределено Тогда
		Если Элементы.ОформлениеПоступленияУслуг.СписокВыбора.Количество() > 0 Тогда
			ОформлениеПоступленияУслуг = Элементы.ОформлениеПоступленияУслуг.СписокВыбора[0].Значение;
		КонецЕсли;
	КонецЕсли;
	...
	Элементы.ОформлениеПоступленияУслуг.Доступность = ПравоДоступа("Редактирование", Метаданные.Справочники.ДоговорыКонтрагентов);
	Элементы.ПриемкаУслуг.Доступность = ПравоДоступа("Редактирование", Метаданные.Справочники.ДоговорыКонтрагентов);	
	...
	УстановитьДоступностьВариантОформленияЗакупок();
	УстановитьДоступностьВариантОформленияДокументовПоступленияПоУслугам(Ложь);
	
	ИнициализироватьСписокВыбораВариантаОформленияЗакупокРаботУслуг();
	ИнициализироватьСписокВыбораВариантаОформленияЗакупокТоваров();
...
Процедура ЗаполнитьБанковскийСчетОрганизацииПоУмолчанию(Организация, ОплатаВВалюте, БанковскийСчет)
	...
	БанковскийСчет = Справочники.БанковскиеСчетаОрганизаций.ПолучитьБанковскийСчетПоУмолчанию(ПараметрыСчета);
...
Процедура ЗаполнитьБанковскийСчетКонтрагентаПоУмолчанию(Контрагент, ОплатаВВалюте, БанковскийСчетКонтрагента, Организация)
	...
	БанковскийСчетКонтрагента = Справочники.БанковскиеСчетаКонтрагентов.ПолучитьБанковскийСчетПоУмолчанию(
		Контрагент, ВалютаРегл, , ОплатаВВалюте);
...
Процедура УправлениеЭлементамиГрафикИсполнения()
	...
	ЗависимыеЭлементы.Добавить(Элементы.ВариантОпределенияГрафикаИсполненияВДоговоре);
	...
	ВариантДоговор = Перечисления.ВариантыОпределенияГрафикаИсполненияВДоговоре.Договор;
	
	Элементы.НадписьГрафикИсполнения.Видимость = Элементы.НадписьГрафикИсполнения.Видимость
		И (Объект.ВариантОпределенияГрафикаИсполненияВДоговоре = ВариантДоговор);
	
	Элементы.ФиксированнаяСуммаДоговора.Заголовок = ?(Объект.ВариантОпределенияГрафикаИсполненияВДоговоре = ВариантДоговор,
		НСтр("ru='Сумма договора фиксирована'"), НСтр("ru='Сумма договора (справочно)'"));
...
&НаСервере
Процедура ВестиРасчетыЧерезКонечныхПокупателейПриИзмененииНаСервере()
...
Процедура УстановитьВидимостьВариантовРаздельнойЗакупки()
	...
	ВидимостьПараметровЗакупок = ДоговорДоступенДляЗакупокДвухходовки И ДоступноОформлениеРаздельнойЗакупки;
														
	Элементы.ГруппаВариантОформленияЗакупокТоваров.Видимость = ВидимостьПараметровЗакупок;
	Элементы.ГруппаВариантОформленияЗакупокУслуг.Видимость 	 = ВидимостьПараметровЗакупок И ИспользоватьНеотфактурованныеПоставки;
	Элементы.ЕжемесячноеПоступлениеУслуг.Видимость 		     = ВидимостьПараметровЗакупок И ИспользоватьНеотфактурованныеПоставки;
	
	Если Не ИспользоватьВариантыОформленияРаздельнойЗакупки Тогда
		ЗаполнитьСписокВыбораВариантаОформленияЗакупокПоТоварамВПути();
	КонецЕсли;
	
	ВидимостьВариантовПриемки = ИспользоватьОформлениеЗакупок() 
		И (ИспользоватьОрдернуюСхемуПриПоступлении Или ДоступноОформлениеРаздельнойЗакупки);
	
	Элементы.ГруппаВариантыПриемки.Видимость = ВидимостьВариантовПриемки;
	Элементы.ГруппаВариантОформленияРаздельнойЗакупки.ОтображатьЗаголовок = ВидимостьПараметровЗакупок;
	Элементы.ГруппаВариантыПриемки.ОтображатьЗаголовок = Не ВидимостьПараметровЗакупок И ВидимостьВариантовПриемки;
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьСписокВыбораВариантаОформленияЗакупокПоТоварамВПути()
	
	Если ИспользоватьТоварыВПутиОтПоставщиков Тогда
		НайденныйЭлемент = Элементы.ВариантОформленияЗакупок.СписокВыбора.НайтиПоЗначению(
			Перечисления.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваровИУслуг);
	ИначеЕсли ИспользоватьНеотфактурованныеПоставки Тогда 
		НайденныйЭлемент = Элементы.ВариантОформленияЗакупок.СписокВыбора.НайтиПоЗначению(
			Перечисления.ВариантыОформленияЗакупок.ТоварыВПути);
	КонецЕсли;
...
Процедура ИнициализироватьВариантОформленияЗакупок()
	...
	Если Объект.ВариантОформленияЗакупок = Перечисления.ВариантыОформленияЗакупок.НеРазделять Тогда
		ИспользоватьОформлениеДокументовРаздельнойЗакупкиТоваров = Ложь;
	Иначе
		ИспользоватьОформлениеДокументовРаздельнойЗакупкиТоваров = Истина;
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ИспользоватьОформлениеДокументовРаздельнойЗакупкиТоваровПриИзмененииСервер()
	
	ИнициализироватьСписокВыбораВариантаОформленияЗакупокТоваров();
	УстановитьДоступностьВариантОформленияЗакупок();
	УстановитьДоступностьПриемки();
	
	ПриОкончанииИзмененияРеквизитаЛокализацииНаСервере("ИспользоватьОформлениеДокументовРаздельнойЗакупкиТоваров", Неопределено);
	
КонецПроцедуры

&НаСервере
Процедура ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслугПриИзмененииСервер()
...
&НаСервере
Процедура ВариантОформленияЗакупокПриИзмененииСервер()
	...
	ПриОкончанииИзмененияРеквизитаЛокализацииНаСервере("ВариантОформленияЗакупок", Неопределено);
	
КонецПроцедуры

&НаСервере
Процедура ИнициализироватьСписокВыбораВариантаОформленияЗакупокТоваров()

	Если ИспользоватьОформлениеДокументовРаздельнойЗакупкиТоваров Тогда
		Если Не Элементы.ВариантОформленияЗакупок.СписокВыбора.Количество() = 0 Тогда
			ЭлементСписка = Элементы.ВариантОформленияЗакупок.СписокВыбора.Получить(0);
			
			Если ЭлементСписка.Значение = Перечисления.ВариантыОформленияЗакупок.НеРазделять Тогда
				Элементы.ВариантОформленияЗакупок.СписокВыбора.Удалить(ЭлементСписка);
			КонецЕсли;
		КонецЕсли;
	Иначе
		Если Не Элементы.ВариантОформленияЗакупок.СписокВыбора.Количество() = 0 Тогда
			...
			Если Не ЭлементСписка.Значение = Перечисления.ВариантыОформленияЗакупок.НеРазделять Тогда
				Элементы.ВариантОформленияЗакупок.СписокВыбора.Вставить(0, 
					Перечисления.ВариантыОформленияЗакупок.НеРазделять, " ");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура УстановитьПараметрыПриемкиУслуг(УстанавливатьПриемкуУслуг = Истина)
	
	ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслуг = ЗначениеЗаполнено(Объект.ВариантОформленияДокументовПоступленияПоУслугам) 
		И Не Объект.ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугНеИспользуется;
	
	Если ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслуг И ОформлениеПоступленияУслуг <> "ПоДоговору" Тогда
		Элементы.ПриемкаУслуг.Доступность = Истина; 
		
	Иначе
		Элементы.ПриемкаУслуг.Доступность = Ложь;
		Если Элементы.ПриемкаУслуг.Видимость И НЕ ОформлениеПоступленияУслуг = "ПоДоговору" Тогда
			ПриемкаУслуг = "Разделена"; 
		ИначеЕсли Элементы.ПриемкаУслуг.Видимость И ОформлениеПоступленияУслуг = "ПоДоговору" Тогда
			ПриемкаУслуг = "НеРазделена";
			Если Не Объект.Ссылка.Пустая() И УстанавливатьПриемкуУслуг Тогда
				ПриемкаУслуг = "НеРазделена";
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура ПриИзмененииНастроекПриемкиУслугСервер(УстанавливатьПриемкуУслуг = Истина)

	ЗакупкиСервер.ЗаполнитьСписокВыбораВариантовПриемкиУслугПоДоговорам(Элементы.ОформлениеПоступленияУслуг,
																	Элементы.ПриемкаУслуг,
																	Объект.ВариантОформленияДокументовПоступленияПоУслугам);
	
	Если Элементы.ОформлениеПоступленияУслуг.СписокВыбора.Количество() = 1 Тогда
		ОформлениеПоступленияУслуг = Элементы.ОформлениеПоступленияУслуг.СписокВыбора[0].Значение;
	КонецЕсли;
	
	Объект.ВариантПриемкиУслуг = ЗакупкиСервер.ПолучитьВариантовПриемкиПоНастройкам(ОформлениеПоступленияУслуг, ПриемкаУслуг);
	
	Если Объект.ВариантОформленияЗакупок = Перечисления.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваров
		И (Объект.ВариантПриемкиУслуг = Перечисления.ВариантыПриемкиТоваров.ПоДоговорамПослеНакладных
			Или Объект.ВариантПриемкиУслуг = Перечисления.ВариантыПриемкиТоваров.РазделенаТолькоПоНакладным) Тогда
		
		Если Объект.ВариантПриемкиУслуг = Перечисления.ВариантыПриемкиТоваров.ПоДоговорамПослеНакладных Тогда
			Объект.ВариантПриемкиУслуг = Перечисления.ВариантыПриемкиТоваров.ПоДоговорамПослеЗаказовИлиНакладных;
		ИначеЕсли Объект.ВариантПриемкиУслуг = Перечисления.ВариантыПриемкиТоваров.РазделенаТолькоПоНакладным Тогда
			Объект.ВариантПриемкиУслуг = Перечисления.ВариантыПриемкиТоваров.РазделенаПоЗаказамИНакладным;
		КонецЕсли;
...
&НаСервере
Процедура ИнициализироватьСписокВыбораВариантаОформленияЗакупокРаботУслуг()

	Если ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслуг Тогда
		Если Не Элементы.ВариантОформленияДокументовПоступленияПоУслугам.СписокВыбора.Количество() = 0 Тогда
			ЭлементСписка = Элементы.ВариантОформленияДокументовПоступленияПоУслугам.СписокВыбора.Получить(0);
			
			Если ЭлементСписка.Значение = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугНеИспользуется Тогда
				Элементы.ВариантОформленияДокументовПоступленияПоУслугам.СписокВыбора.Удалить(ЭлементСписка);
			КонецЕсли;
		КонецЕсли;
	Иначе
		Если Не Элементы.ВариантОформленияДокументовПоступленияПоУслугам.СписокВыбора.Количество() = 0 Тогда
			...
			Если Не ЭлементСписка.Значение = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугНеИспользуется Тогда
				Элементы.ВариантОформленияДокументовПоступленияПоУслугам.СписокВыбора.Вставить(0, 
					Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугНеИспользуется, " ");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура УстановитьВидимостьКонтроляКачестваТоваров()
	...
	Если Объект.ТипДоговора = Перечисления.ТипыДоговоров.СПоставщиком
		...
		Или Объект.ТипДоговора = Перечисления.ТипыДоговоров.СПереработчиком2_5_ЕАЭС
		Или Объект.ТипДоговора = Перечисления.ТипыДоговоров.СПоклажедателем
		Или Объект.ТипДоговора = Перечисления.ТипыДоговоров.СКомитентом
		Или Объект.ТипДоговора = Перечисления.ТипыДоговоров.ВвозИзЕАЭС
		Или Объект.ТипДоговора = Перечисления.ТипыДоговоров.СКомитентомНаЗакупку
		Или Объект.ТипДоговора = Перечисления.ТипыДоговоров.Импорт
		Или Объект.ТипДоговора = Перечисления.ТипыДоговоров.СКомитентом
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"КачествоТоваровГарантированоПоставщиком",
		"Видимость",
		ВидимостьЭлемента);
...
&НаСервере
Процедура УстановитьЗаголовокПоляУИП()
	
	Элементы.ИдентификаторПлатежа.Заголовок =
		?(Справочники.ДоговорыКонтрагентов.ЭтоДоговорСКлиентом(Объект.ТипДоговора),
		НСтр("ru = 'УИП (для входящих платежей)'"),
		НСтр("ru = 'УИП (для исходящих платежей)'"));
...
Процедура УстановитьСвойстваКомментарияИспользованияУстаревшейКомиссии()
	...
	Элементы.ГруппаКомментарийИспользованиеУстаревшейКомиссии.Видимость = Не Объект.КомиссионныеПродажи25
...
Функция ТекущиеДанныеСпискаФайлов()
	...
	Если Не СтандартныеПодсистемыКлиент.ЭтоЭлементДинамическогоСписка(Элементы.Список) 
		Или Элементы.Список.ТекущиеДанные.ВладелецФайла <> Объект.Ссылка Тогда
...
Функция ВыделенныеСтрокиСпискаФайлов()
	...
	Для Каждого ВыделеннаяСтрока Из Элементы.Список.ВыделенныеСтроки Цикл
		...
		Если ТипЗнч(ВыделеннаяСтрока) <> Тип("СтрокаГруппировкиДинамическогоСписка") Тогда
			...
			Если ДанныеСтроки.ВладелецФайла = Объект.Ссылка Тогда
...
#Область РегулярныеПлатежи

&НаСервере
Процедура УстановитьВидимостьРегулярныеПлатежи()
	
	ВариантРегулярныеПлатежи = Перечисления.ВариантыОпределенияГрафикаИсполненияВДоговоре.РегулярныеПлатежи;

	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаРегулярныеПлатежи", "Видимость",
		(Объект.ВариантОпределенияГрафикаИсполненияВДоговоре = ВариантРегулярныеПлатежи));
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьСписокВыбораВариантаОпределенияГрафикаИсполненияВДоговоре(СписокВыбора, ТипДоговора)
	
	СписокВыбора.Очистить();
	СписокВыбора.Добавить(
		ПредопределенноеЗначение("Перечисление.ВариантыОпределенияГрафикаИсполненияВДоговоре.ЗаказыНакладные"));
	СписокВыбора.Добавить(
		ПредопределенноеЗначение("Перечисление.ВариантыОпределенияГрафикаИсполненияВДоговоре.Договор"));
	
	Если ВзаиморасчетыКлиентСервер.ТипыДоговоровСПоставщиком().НайтиПоЗначению(ТипДоговора) <> Неопределено Тогда
		СписокВыбора.Добавить(
			ПредопределенноеЗначение("Перечисление.ВариантыОпределенияГрафикаИсполненияВДоговоре.РегулярныеПлатежи"));
	КонецЕсли;
