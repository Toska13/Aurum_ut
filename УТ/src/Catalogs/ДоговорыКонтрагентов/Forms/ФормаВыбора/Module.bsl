...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПоказыватьДоговорыПоступлениеУслугСторнируется = Истина;
	Если Параметры.Свойство("ПоказыватьДоговорыПоступлениеУслугСторнируется") Тогда
		ПоказыватьДоговорыПоступлениеУслугСторнируется = Параметры.ПоказыватьДоговорыПоступлениеУслугСторнируется;
	КонецЕсли;
	...
	Если Параметры.Свойство("Соглашение", Соглашение)
		И ЗначениеЗаполнено(Соглашение)
		И ТипЗнч(Соглашение) = Тип("СправочникСсылка.СоглашенияСПоставщиками")
		И Не Параметры.Отбор.Свойство("ХозяйственнаяОперация") Тогда
	...
	УстановитьНедоступныеОтборыИзПараметров(Параметры.Отбор, Параметры.Свойство("ПоказыватьДоговорыРаздельнойЗакупки"),
		ПоказыватьРеализациюЧерезКомиссионера, ПоказыватьДоговорыПоступлениеУслугСторнируется);
	...
	Если ПоказыватьРеализациюЧерезКомиссионера Тогда
		...
		Если Параметры.Свойство("Контрагент") Тогда
			ПараметрыОтбора.Вставить("Контрагент", Параметры.Контрагент);
		КонецЕсли;
		Если Параметры.Свойство("Партнер") Тогда
			ПараметрыОтбора.Вставить("Партнер", Параметры.Партнер);
		КонецЕсли;
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, ЭтоГруппа, Параметр)
	...
	Если Не Копирование И ТипЗнч(ПараметрыОтбора) = Тип("Структура") Тогда
		
		Отказ = Истина;
		
		ЗначенияЗаполнения = Новый Структура;
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ЗначенияЗаполнения, ПараметрыОтбора);
		...
		Для Каждого ЭлементОтбора Из Список.Отбор.Элементы Цикл
			Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных")
				...
				Или ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПорядокРасчетов")) Тогда
		...
		ОткрытьФорму("Справочник.ДоговорыКонтрагентов.ФормаОбъекта", ПараметрыФормы, Элементы.Список, ЭтотОбъект);
...
//
&НаСервере
Процедура УстановитьНедоступныеОтборыИзПараметров(СтруктураОтбора, ПоказыватьДоговорыРаздельнойЗакупки, 
		ПоказыватьРеализациюЧерезКомиссионера, ПоказыватьДоговорыПоступлениеУслугСторнируется = Истина)
	...
	Для Каждого ЭлементОтбора Из СтруктураОтбора Цикл
		...
		Если ЭлементОтбора.Ключ = "НазначениеОбеспечения" Тогда
			...
			НаправленияДеятельности = Справочники.Назначения.НаправленияДеятельностиНазначений(
				ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ЗначениеОтбора));

			НаправлениеДеятельности = НаправленияДеятельности.Получить(ЗначениеОтбора);
			
			Если НаправлениеДеятельности = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			ДоступныеНаправленияДеятельности = Новый Массив;
			ДоступныеНаправленияДеятельности.Добавить(НаправлениеДеятельности);
			ДоступныеНаправленияДеятельности.Добавить(Справочники.НаправленияДеятельности.ПустаяСсылка());
				
			ГруппаИли = КомпоновкаДанныхКлиентСервер.ДобавитьГруппуОтбора(
				Список.Отбор.Элементы,
				ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
				
				КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(ГруппаИли,
					"НаправлениеДеятельности",
					ДоступныеНаправленияДеятельности,
					ВидСравненияКомпоновкиДанных.ВСписке);
		...
		Если ЭлементОтбора.Ключ = "ХозяйственнаяОперация" Тогда
			...
			Если ЗначениеОтбора = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки Или 
				ЗначениеОтбора = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки Тогда

				Если ПоказыватьДоговорыРаздельнойЗакупки Тогда
	                ГруппаОтбора = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(Список.Отбор, 
	                    "РазделениеЗакупок", ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
					
                    ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора, "ВариантОформленияЗакупок", 
						ВидСравненияКомпоновкиДанных.Равно, Перечисления.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваровИУслуг);
					...
					Если ПоказыватьДоговорыПоступлениеУслугСторнируется Тогда
					...
					ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора,
					    "ВариантОформленияДокументовПоступленияПоУслугам", ВидСравненияКомпоновкиДанных.ВСписке, СписокВариантовПоУслугам);
					
					Если ЗначениеОтбора = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки Тогда
						ЗначениеОтбора = ЗакупкиСервер.ХозяйственныеОперацииПоОсновной(Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика);
					Иначе
						ЗначениеОтбора = ЗакупкиСервер.ХозяйственныеОперацииПоОсновной(Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС);
					КонецЕсли;
                КонецЕсли;

			ИначеЕсли ЗначениеОтбора = Перечисления.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности
				Или ЗначениеОтбора = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности
				Или ЗначениеОтбора = Перечисления.ХозяйственныеОперации.ОтгрузкаТоваровКлиенту Тогда
				
				ЗначениеОтбора = Перечисления.ХозяйственныеОперации.РеализацияКлиенту
				
			ИначеЕсли ПоказыватьДоговорыРаздельнойЗакупки
