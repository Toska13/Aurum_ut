...
Функция ДобавитьОписаниеКолонок(МакетФормы, ИсточникДанных, ДобавитьСторно, ПутьКПолюНазначение, Префикс = "") Экспорт
	...
	Если ИсточникДанных = "ОбеспечениеЗаказов" Тогда
		...
		ПредставлениеВНаличии = СтрШаблон(НСтр("ru = 'Доступно для отгрузки на складе ""%1""'"), ПараметрПредставления);
		ПредставлениеАрхивныеОстатки = СтрШаблон(НСтр("ru = 'Заказано на складе ""%1""'"), ПараметрПредставления);
		ПредставлениеВНаличииДляРезервирования = СтрШаблон(НСтр("ru = 'В наличии на складе ""%1""'"), ПараметрПредставления);
		
		Колонки = Новый СписокЗначений();
		Колонки.Добавить("ВНаличии", ПредставлениеВНаличии, Ложь);
		Колонки.Добавить("ВНаличииДляРезервирования", ПредставлениеВНаличииДляРезервирования, Ложь);
		Колонки.Добавить("Потребность", ПредставлениеПотребность, Ложь);
		Колонки.Добавить("Архивное", ПредставлениеАрхивныеОстатки, Ложь);
		
		ВозвращаемоеЗначение = ДобавитьОписаниеОбеспечениеЗаказов(МакетФормы, ДобавитьСторно, Префикс, Колонки, Истина);
	...
	ИначеЕсли ИсточникДанных = "ОбеспечениеЗаказовРаботами" Тогда
		...
		ПредставлениеВНаличии = СтрШаблон(НСтр("ru = 'Доступно для отгрузки в подразделении ""%1""'"), ПараметрПредставления);
		ПредставлениеАрхивныеОстатки = СтрШаблон(НСтр("ru = 'Заказано в подразделении ""%1""'"), ПараметрПредставления);
		ПредставлениеВНаличииДляРезервирования = СтрШаблон(НСтр("ru = 'В наличии в подразделении ""%1""'"), ПараметрПредставления);
	...
	ИначеЕсли ИсточникДанных = "ОбеспечениеЗаказовВсеСклады" Тогда
		...
		Колонки = Новый СписокЗначений();
		Колонки.Добавить("ВНаличии", НСтр("ru = 'Доступно для отгрузки на всех складах'"), Ложь);
		Колонки.Добавить("ВНаличииДляРезервирования", НСтр("ru = 'В наличии на всех складах'"), Ложь);
		...
		Префикс = ?(ПустаяСтрока(Префикс), "ОбеспечениеЗаказовВсеСклады", Префикс);
		ВозвращаемоеЗначение = ДобавитьОписаниеОбеспечениеЗаказов(МакетФормы, ДобавитьСторно, Префикс, Колонки, Ложь);
		
		ВозвращаемоеЗначение.Отказ = Не ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоСкладов");
...
Процедура ФормаДокументаПриСозданииНаСервере(ФормаОбъекта, ПутиКПолюНазначение = Неопределено) Экспорт
	...
	Для Каждого ШаблонНазначения Из МакетФормыВыбораНазначений.ШаблоныНазначений Цикл
		...
		ТаблицаФормы = ТаблицаФормы(ФормаОбъекта.Элементы, ШаблонНазначения.ПутьКПолюНазначение);
		...
		Для Каждого ПолеОтбора Из ШаблонНазначения Цикл
			...
			Если ЗначениеЗаполнено(ПолеОтбора.Значение)
				...
				ПутиКДанным = ПутиКДаннымУсловияИспользования("", ПолеОтбора.Значение,
					ШаблонНазначения.ПутьКПолюНазначение, ТаблицаФормы, Неопределено);
				Для Каждого Путь Из ПутиКДанным Цикл
					СвязьНайдена = Ложь;
					Для Каждого ТекущаяСвязь Из СвязиШаблона[КлючПутиКПолюНазначение] Цикл
						Если ТекущаяСвязь.Имя = Путь.Ключ Тогда
							СвязьНайдена = Истина;
							Прервать;
						КонецЕсли;
					КонецЦикла;
					Если Не СвязьНайдена Тогда
						Связь = Новый СвязьПараметраВыбора(Путь.Ключ, Путь.Значение,
	...
	Для Каждого ПутьКПолюНазначение Из МассивПутейКПолюНазначение Цикл
		...
		Для Каждого ОписаниеКолонок Из МакетФормыВыбораНазначений.ОписанияКолонок Цикл
			...
			ИмяЭлементаФормы = ОписаниеКолонок.ИмяЭлементаФормы;
			ИмяТаблицыФормы = ОписаниеКолонок.ИмяТаблицыФормы;
		...
		Если ИмяЭлементаФормы <> Неопределено Тогда
			ПолеНазначение = ФормаОбъекта.Элементы[ИмяЭлементаФормы];
		Иначе
			ПолеНазначение = ПолеНазначение(ФормаОбъекта.Элементы, ПутьКПолюНазначение);
		КонецЕсли;
		Если ИмяТаблицыФормы <> Неопределено Тогда
			ТаблицаФормы = ФормаОбъекта.Элементы[ИмяТаблицыФормы];
		Иначе
			ТаблицаФормы = ТаблицаФормы(ФормаОбъекта.Элементы, ПутьКПолюНазначение);
		КонецЕсли;
...
// Для получения представления еще не созданного назначения на языке пользователя.
// Параметры:
//  ШаблонНазначения - см. ШаблонНового
//  РеквизитыДокумента - Структура - структура с полями:
//   * Номер - Строка - номер документа
//   * Дата - Дата - дата документа
//   * Получатель - СправочникСсылка.Партнеры - получатель
//  Возвращаемое значение:
//   Строка - Представление.
Функция ПредставлениеНазначенияПоШаблону(ШаблонНазначения, РеквизитыДокумента) Экспорт
	
	Представление = ПредставлениеНазначенияДляЗаписиВИнформационнуюБазу(
		ШаблонНазначения,
		РеквизитыДокумента);
	...
	Если Не ЭтоОсновнойЯзык Тогда
		...
		ПредставлениеТипа = НазначенияКлиентСервер.ПредставлениеТипаНазначения(ШаблонНазначения, "");
		Если ПредставлениеТипа <> Неопределено Тогда
			...
			Представление = ОбрезанноеПредставление + ПредставлениеТипа + ")";
...
Процедура ПриПоискеЗаменыСсылок(ПарыЗамен, НеобработанныеЗначенияОригиналов) Экспорт
	...
	Для Каждого НеобработанныйДубль Из НеобработанныеЗначенияОригиналов Цикл
		...
		ПарыЗамен.Вставить(НеобработанныйДубль.ЗаменяемоеЗначение, СоздатьПоКлючу(ПараметрыКлюча));
	КонецЦикла;
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если Не ЭтоОсновнойЯзык Тогда
		...
		ПредставлениеТипаНаОсновномЯзыке = НазначенияКлиентСервер.ПредставлениеТипаНазначения(Данные, КодОсновногоЯзыка);
		ПредставлениеТипаНаОсновномЯзыке = "(" + ПредставлениеТипаНаОсновномЯзыке + ")";
		
		ПредставлениеТипа = НазначенияКлиентСервер.ПредставлениеТипаНазначения(Данные, "");
		ПредставлениеТипа = "(" + ПредставлениеТипа + ")";
		Если ПредставлениеТипа <> Неопределено Тогда
			Представление = СтрЗаменить(Представление, ПредставлениеТипаНаОсновномЯзыке, ПредставлениеТипа);
		КонецЕсли;
...
Функция ТекстЗапросаНеобеспеченныхНазначенийОбеспечено(ПараметрыФормированияЗапроса) Экспорт
	...
	ТекстЗапроса =
		...
		|		+ Набор.РезервироватьНаСкладеИзЗаблокированныхОстаток
		|		+ Набор.РезервироватьПоМереПоступленияОстаток
		|		+ Набор.РезервироватьПоМереПоступленияИзЗаблокированныхОстаток
...
Процедура ПриЗаписиДокумента(Назначение, ШаблонНазначения, ЭтотОбъект, Получатель, ВидДеятельностиНДС) Экспорт
	...
	Если РеквизитыНазначения.Наименование <> Неопределено Тогда
		...
		Если ОбновитьПометкуУдаления
			...
			Или ОбновитьНаименование Тогда
	...
	Иначе // Это новое назначение
		...
		Если НазначениеОбъект.ТипНазначения = Перечисления.ТипыНазначений.ПоставкаПодПринципала Тогда
			НазначениеОбъект.УчитыватьСтавкиНДС = Истина;
		КонецЕсли;
...
Функция ПредставлениеНазначенияДляЗаписиВИнформационнуюБазу(КлючевыеПоля, РеквизитыДокумента)
...
Процедура УстановитьБлокировкуПоШаблону(Шаблон)
...
// * ИмяЭлементаФормы - Строка - имя элемента формы связанного с назначением
// * ИмяТаблицыФормы - Строка - имя элемента таблицы формы в которой расположено поле назначения
Функция МакетОписанияКолонок()
	...
	ОписаниеКолонок.Вставить("ИмяЭлементаФормы", Неопределено);
	ОписаниеКолонок.Вставить("ИмяТаблицыФормы", Неопределено);
...
Функция ДобавитьОписаниеОбеспечениеЗаказов(МакетФормы, ДобавитьСторно, Префикс, Колонки, ОтбиратьПоСкладу)
	...
	ТекстЗапроса =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ВЫБОР
		|		КОГДА ВЫРАЗИТЬ(&ПрефиксНоменклатура КАК Справочник.Номенклатура).ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
		|			ТОГДА &ПрефиксПодразделение
		|		ИНАЧЕ &ПрефиксСклад
		|	КОНЕЦ КАК Ссылка
		|ПОМЕСТИТЬ ПараметрСкладПереопределяемый";
	ТекстыЗапроса.Добавить(ТекстЗапроса);
	
	#Область ВременнаяТаблицаЗапасыИПотребностиПартий
	
	Тексты = Новый Массив();
	
	Если ОтбиратьПоСкладу Тогда
		ОтбиратьПоСкладуПараметр =
			"ВЫБРАТЬ
			|	Фильтр.Ссылка КАК Склад
			|ИЗ
			|	ПараметрСкладПереопределяемый КАК Фильтр";
		ОтбиратьПоСкладуУсловие =
			"ВЫБРАТЬ ПЕРВЫЕ 1
			|	ИСТИНА КАК ЕстьЗаписи
			|ИЗ
			|	ПараметрСкладПереопределяемый КАК Фильтр
			|ГДЕ
			|	Фильтр.Ссылка = Таблица.Склад";
	Иначе
		ОтбиратьПоСкладуПараметр = "Склад";
		ОтбиратьПоСкладуУсловие = "ИСТИНА";
	КонецЕсли;
	...
	Если ИспользоватьКонтрольКачестваТоваров Тогда
		...
		ТекстНабора =
			...
			|	Остатки.Заказ КАК Партия,
			|	Остатки.ЗаблокированоВНаличииОстаток
			|		- Остатки.РезервироватьНаСкладеИзЗаблокированныхОстаток
			|		- Остатки.РезервироватьПоМереПоступленияИзЗаблокированныхОстаток КАК ВНаличии,
			|	0 КАК РезервПоМереПодЗаказом,
			|	Остатки.ЗаблокированоВНаличииОстаток
			|		+ Остатки.ЗаблокированоПоступитОстаток КАК Запас,
			|	Остатки.РезервироватьНаСкладеИзЗаблокированныхОстаток
			|		+ Остатки.РезервироватьПоМереПоступленияИзЗаблокированныхОстаток КАК ПланируемаяОтгрузка
			|ИЗ
			|	РегистрНакопления.ЗапасыИПотребности.Остатки(,
			|		Назначение <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
			|			И НЕ Назначение В(&ПрефиксИсключаемыеНазначения)
			|			И Номенклатура = &ПрефиксНоменклатура
			|			И Характеристика = &ПрефиксХарактеристика
			|			И Склад В(&ОтбиратьПоСкладуПараметр)) КАК Остатки";
		...
		Если ДобавитьСторно Тогда
			...
			ТекстНабора =
				...
				|	Таблица.Заказ КАК Партия,
				|	ВЫБОР
				|		КОГДА Таблица.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
				|			ТОГДА - Таблица.ЗаблокированоВНаличии - Таблица.РезервироватьНаСкладеИзЗаблокированных
				|		ИНАЧЕ Таблица.ЗаблокированоВНаличии + Таблица.РезервироватьНаСкладеИзЗаблокированных
				|	КОНЕЦ КАК ВНаличии,
				|	0 КАК РезервПоМереПодЗаказом,
				|	ВЫБОР
				|		КОГДА Таблица.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
				|			ТОГДА - Таблица.ЗаблокированоВНаличии - Таблица.ЗаблокированоПоступит
				|		ИНАЧЕ Таблица.ЗаблокированоВНаличии + Таблица.ЗаблокированоПоступит
				|	КОНЕЦ КАК Запас,
				|	ВЫБОР
				|		КОГДА Таблица.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
				|			ТОГДА - Таблица.РезервироватьНаСкладеИзЗаблокированных - Таблица.РезервироватьПоМереПоступленияИзЗаблокированных
				|		ИНАЧЕ Таблица.РезервироватьНаСкладеИзЗаблокированных + Таблица.РезервироватьПоМереПоступленияИзЗаблокированных
				...
				|		И НЕ Таблица.Назначение В(&ПрефиксИсключаемыеНазначения)
				|		И Таблица.Регистратор = &ПрефиксРегистратор
				|		И Таблица.Номенклатура = &ПрефиксНоменклатура
				|		И Таблица.Характеристика = &ПрефиксХарактеристика
				|		И ИСТИНА В (&ОтбиратьПоСкладуУсловие)
				|	
				|	ОБЪЕДИНИТЬ ВСЕ
				|
				|	ВЫБРАТЬ
				|		Таблица.Назначение КАК Назначение,
				|		Таблица.ЗаказНаПоступление КАК Партия,
				|		0 ВНаличии,
				|		Таблица.Количество КАК РезервПоМереПодЗаказом,
				|		0 КАК Запас,
				...
				|			И НЕ Таблица.Назначение В(&ПрефиксИсключаемыеНазначения)
				|			И Таблица.ЗаказНаОтгрузку = &ПрефиксРегистратор
				|			И Таблица.Номенклатура = &ПрефиксНоменклатура
				|			И Таблица.Характеристика = &ПрефиксХарактеристика
				|			И ИСТИНА В(&ОтбиратьПоСкладуУсловие)
				...
				|			И Таблица.ХарактеристикаПотребности = ЗНАЧЕНИЕ(Перечисление.ХарактеристикиПотребности.РезервироватьПоМереПоступленияИзЗаблокированных)";
			Тексты.Добавить(ТекстНабора);
		...
		ТекстЗапроса =
			...
			|	Набор.Партия КАК Партия,
			|	СУММА(
			|		ВЫБОР
			|			КОГДА Набор.ВНаличии > 0
			|				ТОГДА Набор.ВНаличии
			|			ИНАЧЕ 0
			|		КОНЕЦ
			|		+ Набор.РезервПоМереПодЗаказом) КАК ВНаличии,
			|	СУММА(
			|		ВЫБОР
			|			КОГДА Набор.Запас > Набор.ПланируемаяОтгрузка
			|				ТОГДА Набор.Запас
			|			ИНАЧЕ Набор.ПланируемаяОтгрузка
			|		КОНЕЦ) КАК Распределено,
			|	СУММА(
			|		ВЫБОР
			|			КОГДА Набор.Запас > Набор.ПланируемаяОтгрузка
			|				ТОГДА 0
			|			ИНАЧЕ Набор.ПланируемаяОтгрузка - Набор.Запас
			|		КОНЕЦ) КАК НеобеспеченнаяПотребность,
			|	СУММА(Набор.ПланируемаяОтгрузка) КАК ПланируемаяОтгрузка
			|ПОМЕСТИТЬ ВременнаяТаблицаЗапасыИПотребностиПартийПереопределяемый
			|ИЗ
			|	&Набор КАК Набор
			|СГРУППИРОВАТЬ ПО
			|	Набор.Назначение,
	...
	#Область ВременнаяТаблицаЗапасыИПотребности
	...
	ТекстНабора =
		...
		|		- Остатки.РезервироватьПоМереПоступленияОстаток ВНаличии,
		|	0 КАК ВНаличииИзПартий,
		|	0 ВНаличииИзПартииСтрокиДокумента,
		|	0 КАК РезервПоМереПодЗаказом,
		|	Остатки.РезервироватьНаСкладеОстаток
		|		+ Остатки.РезервироватьПоМереПоступленияОстаток
		|		+ Остатки.ОтложитьРезервированиеОстаток
		|		+ Остатки.КОбеспечениюОстаток
		|		- Остатки.ВНаличииОстаток
		|		- Остатки.ЗаказаноОстаток КАК НеобеспеченнаяПотребность,
		|	0 КАК НеобеспеченнаяПотребностьИзПартииСтрокиДокумента,
		|	Остатки.РезервироватьНаСкладеОстаток
		|		+ Остатки.РезервироватьПоМереПоступленияОстаток
		|		+ Остатки.ОтложитьРезервированиеОстаток
		|		+ Остатки.КОбеспечениюОстаток КАК ПланируемаяОтгрузка
		|ИЗ
		|	РегистрНакопления.ЗапасыИПотребности.Остатки(,
		|		Назначение <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|			И НЕ Назначение В(&ПрефиксИсключаемыеНазначения)
	...
	Если ДобавитьСторно Тогда
		...
		ТекстНабора =
			...
			|		0 ВНаличииИзПартииСтрокиДокумента,
			|		Таблица.Количество КАК РезервПоМереПодЗаказом,
			|		0 КАК НеобеспеченнаяПотребность,
			|		0 КАК НеобеспеченнаяПотребностьИзПартииСтрокиДокумента,
			|		0 КАК ПланируемаяОтгрузка
			|	ИЗ
			|		РегистрСведений.РаспределениеЗапасов КАК Таблица
			...
			|			И Таблица.ХарактеристикаПотребности = ЗНАЧЕНИЕ(Перечисление.ХарактеристикиПотребности.РезервироватьПоМереПоступления)";
		Тексты.Добавить(ТекстНабора);
	...
	Если ИспользоватьКонтрольКачестваТоваров Тогда
		...
		ТекстНабора =
			...
			|	0 КАК ВНаличии,
			|	Партии.ВНаличии КАК ВНаличииИзПартий,
			|	ВЫБОР
			|		КОГДА &ПрефиксКонтролируемаяПартия = Партии.Партия
			|			ТОГДА Партии.ВНаличии
			|		ИНАЧЕ 0
			|	КОНЕЦ КАК ВНаличииИзПартииСтрокиДокумента,
			|	0 КАК РезервПоМереПодЗаказом,
			|	Партии.Распределено КАК НеобеспеченнаяПотребность,
			|	ВЫБОР
			|		КОГДА &ПрефиксКонтролируемаяПартия = Партии.Партия
			|			ТОГДА Партии.НеобеспеченнаяПотребность
			|		ИНАЧЕ 0
			|	КОНЕЦ КАК НеобеспеченнаяПотребностьИзПартииСтрокиДокумента,
			|	Партии.ПланируемаяОтгрузка КАК ПланируемаяОтгрузка
			|ИЗ
			|	ВременнаяТаблицаЗапасыИПотребностиПартийПереопределяемый КАК Партии";
		Тексты.Добавить(ТекстНабора);
		
	КонецЕсли;
	
	
	// Если назначение выбирают в документе, который перехватывает обеспечение от распоряжения на себя,
	// то данный блок добавляется.
	ТекстНабора =
		"ВЫБРАТЬ
		|	Запасы.Назначение КАК Назначение,
		|	0 КАК ВНаличии,
		|	0 КАК ВНаличииИзПартий,
		|	0 КАК ВНаличииИзПартииСтрокиДокумента,
		|	0 КАК РезервПоМереПодЗаказом,
		|	Запасы.ЗаказаноОстаток КАК НеобеспеченнаяПотребность,
		|	0 КАК НеобеспеченнаяПотребностьИзПартииСтрокиДокумента,
		|	0 КАК ПланируемаяОтгрузка
		|ИЗ
		|	РегистрНакопления.ЗапасыИПотребности.Остатки(,
		...
		|			И Номенклатура = &ПрефиксНоменклатура
		|			И Характеристика = &ПрефиксХарактеристика
		|			И Склад В(&ОтбиратьПоСкладуПараметр)) КАК Запасы";
	Тексты.Добавить(ТекстНабора);
	...
	ТекстЗапроса =
		...
		|	СУММА(
		|		ВЫБОР
		|			КОГДА Набор.ВНаличии + Набор.ВНаличииИзПартий > 0
		|				ТОГДА Набор.ВНаличии + Набор.ВНаличииИзПартий
		|			ИНАЧЕ 0
		|		КОНЕЦ
		|		+ Набор.РезервПоМереПодЗаказом) КАК ВНаличииДляРезервирования,
		|	СУММА(Набор.ВНаличииИзПартииСтрокиДокумента) КАК ВНаличииИзПартииСтрокиДокумента,
		|	СУММА(Набор.НеобеспеченнаяПотребность) КАК НеобеспеченнаяПотребность,
		|	СУММА(Набор.НеобеспеченнаяПотребностьИзПартииСтрокиДокумента) КАК НеобеспеченнаяПотребностьИзПартииСтрокиДокумента,
		|	СУММА(Набор.ПланируемаяОтгрузка) КАК ПланируемаяОтгрузка
		|ПОМЕСТИТЬ ВременнаяТаблицаЗапасыИПотребностиПереопределяемый
		|ИЗ
		|	&Набор КАК Набор
		|СГРУППИРОВАТЬ ПО
		|	Набор.Назначение
	...
	ВременныеТаблицы = СтрСоединить(ТекстыЗапроса, ОбщегоНазначения.РазделительПакетаЗапросов());
	
	ВременныеТаблицы = СтрЗаменить(ВременныеТаблицы, "&ОтбиратьПоСкладуПараметр", ОтбиратьПоСкладуПараметр);
	ВременныеТаблицы = СтрЗаменить(ВременныеТаблицы, "&ОтбиратьПоСкладуУсловие", ОтбиратьПоСкладуУсловие);
	...
	ТекстЗапроса =
		...
		|
			// В наличии для отгрузки.
		|	ВЫБОР
				// В документе не поддерживается отгрузка товаров на контроле (партия не определена),
				// либо в текущей строке документа отгружают проверенный товар (партия пустая).
		|		КОГДА
		|			&ПрефиксКонтролируемаяПартия В(
		|				НЕОПРЕДЕЛЕНО,
		|				ЗНАЧЕНИЕ(Справочник.КонтролируемыеПартии.ПустаяСсылка))
					// Свободный остаток для отгрузки товаров прошедших контроль.
		|			ТОГДА
		|				Набор.ВНаличии
					// В текущей строке документа отгружают товар конкретной партии на контроле (партия заполнена),
		|		ИНАЧЕ Набор.ВНаличииИзПартииСтрокиДокумента
		|	КОНЕЦ КАК ПрефиксВНаличии,
		|
			// В наличии для резервирования.
		|	ВЫБОР
				// В документе не поддерживается отгрузка товаров на контроле (партия не определена),
				// либо в текущей строке документа резервируют товар без указания партии на контроле (партия пустая).
		|		КОГДА
		|			&ПрефиксКонтролируемаяПартия В(
		|				НЕОПРЕДЕЛЕНО,
		|				ЗНАЧЕНИЕ(Справочник.КонтролируемыеПартии.ПустаяСсылка))
					// Свободный остаток для отгрузки товаров прошедших контроль.
		|			ТОГДА
		|				Набор.ВНаличииДляРезервирования
					// В текущей строке документа резервируют товар конкретной партии на контроле (партия заполнена),
		|		ИНАЧЕ Набор.ВНаличииИзПартииСтрокиДокумента
		|	КОНЕЦ КАК ПрефиксВНаличииДляРезервирования,
		|
			// Необеспеченная потребность/излишек в проверенных товарах.
		|	Набор.НеобеспеченнаяПотребность
		|	+ ВЫБОР
				// В документе не поддерживается обеспечение товарами с постановкой на контроль (партия не определена),
				// либо текущая строка документа обеспечивает без постановки на контроль
				// (партия не пуста, но флаг постановки на контроль отключен).
		|		КОГДА
		|			&ПрефиксКонтролируемаяПартия В(
		|					НЕОПРЕДЕЛЕНО,
		|					ЗНАЧЕНИЕ(Справочник.КонтролируемыеПартии.ПустаяСсылка))
		|				ИЛИ &ПрефиксПроконтролироватьКачество В (НЕОПРЕДЕЛЕНО, ЛОЖЬ)
		|			ТОГДА 0
					// Текущая строка документа обеспечивает товаром с постановкой на контроль конкретной партии.
		|		ИНАЧЕ Набор.НеобеспеченнаяПотребностьИзПартииСтрокиДокумента
		|	КОНЕЦ КАК ПрефиксПотребность,
		|	
		|	Набор.ПланируемаяОтгрузка КАК ПрефиксАрхивное
		|	,&ПоляВыбораПоследующие
		|ИЗ
		|	ВременнаяТаблицаЗапасыИПотребностиПереопределяемый КАК Набор
		|ГДЕ
		|	&ПрефиксВалюта = НЕОПРЕДЕЛЕНО
		|		ИЛИ ВЫРАЗИТЬ(Набор.Назначение КАК Справочник.Назначения).ТипНазначения <> ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.ПоставкаПодПринципала)
		|		ИЛИ ВЫРАЗИТЬ(Набор.Назначение КАК Справочник.Назначения).Договор.ВалютаВзаиморасчетов = &ПрефиксВалюта";
	...
	Подстановки.Вставить("ПрефиксИсключаемыеНазначения", "ИсключаемыеНазначения");
	Подстановки.Вставить("ПрефиксКонтролируемаяПартия", "КонтролируемаяПартия");
	Подстановки.Вставить("ПрефиксПроконтролироватьКачество", "ПроконтролироватьКачество");
	Подстановки.Вставить("ПрефиксВалюта", "Валюта");
	Подстановки.Вставить("ПараметрСкладПереопределяемый", "ПараметрСклад");
	Подстановки.Вставить("ВременнаяТаблицаЗапасыИПотребностиПартийПереопределяемый", "ВременнаяТаблицаЗапасыИПотребностиПартий");
	Подстановки.Вставить("ВременнаяТаблицаЗапасыИПотребностиПереопределяемый", "ВременнаяТаблицаЗапасыИПотребности");
	...
	ПутиКДанным.Вставить("ИсключаемыеНазначения");
	ПутиКДанным.Вставить("КонтролируемаяПартия");
	ПутиКДанным.Вставить("ПроконтролироватьКачество");
...
Функция ПолеНазначение(Элементы, ПутьКПолюНазначение)
	
	Текст = СтрЗаменить(ПутьКПолюНазначение, "Объект.", "");
	Пути = СтрРазделить(Текст, ".");
	ИмяЭлемента = СтрСоединить(Пути);
	Элемент = Элементы[ИмяЭлемента];
	Возврат Элемент;
	
КонецФункции

Функция ТаблицаФормы(Элементы, ПутьКПолюНазначение)
	
	Текст = СтрЗаменить(ПутьКПолюНазначение, "Объект.", "");
	Пути = СтрРазделить(Текст, ".");
	Элемент = Неопределено;
	Если Пути.Количество() = 2 Тогда
		Элемент = Элементы[Пути[0]];
	ИначеЕсли Пути.Количество() = 3 Тогда // РедактированиеЭтапов
		Элемент = Элементы[Пути[0] + Пути[1]];
	КонецЕсли;
	Возврат Элемент;
...
Процедура ИсточникПараметровПолучаемыхИзИБ(Префикс, Выражение, ПутьКПолюНазначение, Результат)
	...
	Если Выражение = "" Тогда
		ПодстрокиВыражений = Новый Массив();
	Иначе
...
Функция КлючевыеРеквизиты() Экспорт
	...
	ПоляСвязей = Новый Массив;
	Для Каждого Реквизит Из Метаданные.Справочники.Назначения.Реквизиты Цикл
		ПоляСвязей.Добавить(Реквизит.Имя);
	КонецЦикла;
...
// Создает новое назначение по ключевым полям или возвращает существующее.
//
// Параметры:
//  ПараметрыКлюча - Структура из КлючИЗначение:
...
//
Функция СоздатьПоКлючу(ПараметрыКлюча) Экспорт
	...
	|	И Назначения.Договор = &Договор
	|	И Назначения.ТипНазначения = &ТипНазначения
	|	И Назначения.ВидДеятельностиНДС = &ВидДеятельностиНДС
	|	И Назначения.ДвиженияПоСкладскимРегистрам = &ДвиженияПоСкладскимРегистрам
	|	И Назначения.КонтролироватьТолькоНаличие = &КонтролироватьТолькоНаличие
	|	И Назначения.УчитыватьСтавкиНДС = &УчитыватьСтавкиНДС";
	
	Запрос.УстановитьПараметр("Заказ", ПараметрыКлюча.Заказ);
	Запрос.УстановитьПараметр("НаправлениеДеятельности", ПараметрыКлюча.НаправлениеДеятельности);
	Запрос.УстановитьПараметр("Партнер", ПараметрыКлюча.Партнер);
	Запрос.УстановитьПараметр("Договор", ПараметрыКлюча.Договор);
	Запрос.УстановитьПараметр("ТипНазначения", ПараметрыКлюча.ТипНазначения);
	Запрос.УстановитьПараметр("ВидДеятельностиНДС", ПараметрыКлюча.ВидДеятельностиНДС);
	Запрос.УстановитьПараметр("ДвиженияПоСкладскимРегистрам", ПараметрыКлюча.ДвиженияПоСкладскимРегистрам);
	Запрос.УстановитьПараметр("КонтролироватьТолькоНаличие", ПараметрыКлюча.КонтролироватьТолькоНаличие);
	Запрос.УстановитьПараметр("УчитыватьСтавкиНДС", ПараметрыКлюча.УчитыватьСтавкиНДС);
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		ШаблонНазначения = ШаблонНового();
		ЗаполнитьЗначенияСвойств(ШаблонНазначения, ПараметрыКлюча);
		
		РеквизитыДокумента = Неопределено;
		Если ЗначениеЗаполнено(ПараметрыКлюча.Заказ)Тогда
			ИменаРеквизитов = Новый Массив;
			ИменаРеквизитов.Добавить("Дата");
			ИменаРеквизитов.Добавить("Номер");
			ИменаРеквизитов.Добавить("ПометкаУдаления");
			ИменаРеквизитов.Добавить(ИмяРеквизитаПолучательНазначения(ПараметрыКлюча.Заказ));
			
			РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
				ПараметрыКлюча.Заказ, ИменаРеквизитов);
			ПометкаУдаления = РеквизитыДокумента.ПометкаУдаления;
		Иначе
			Справочник = Неопределено;
			Если ЗначениеЗаполнено(ПараметрыКлюча.Договор) Тогда
				Справочник = ПараметрыКлюча.Договор;
			ИначеЕсли ЗначениеЗаполнено(ПараметрыКлюча.Партнер) Тогда
				Справочник = ПараметрыКлюча.Партнер;
			ИначеЕсли ЗначениеЗаполнено(ПараметрыКлюча.НаправлениеДеятельности) Тогда
				Справочник = ПараметрыКлюча.НаправлениеДеятельности;
			КонецЕсли;
			
			Если Справочник <> Неопределено Тогда
				ПометкаУдаления = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Справочник, "ПометкаУдаления");
			Иначе
		...
		Ключ = Справочники.Назначения.СоздатьЭлемент();
		ЗаполнитьЗначенияСвойств(Ключ, ПараметрыКлюча);
		Ключ.Наименование = ПредставлениеНазначения;
		Ключ.ПометкаУдаления = ПометкаУдаления;
		Ключ.Записать();
	Иначе
...
Функция ИмяРеквизитаПолучательНазначения(Ссылка)
	
	МаппингТипов = Новый Соответствие;
	
	МаппингТипов.Вставить(Тип("ДокументСсылка.ЗаказНаСборку"), "Склад");
	МаппингТипов.Вставить(Тип("ДокументСсылка.ЗаказНаПеремещение"), "СкладПолучатель");
	МаппингТипов.Вставить(Тип("ДокументСсылка.ЗаказНаВнутреннееПотребление"), "Подразделение");
	МаппингТипов.Вставить(Тип("ДокументСсылка.ЗаказКлиента"), "Партнер");
	МаппингТипов.Вставить(Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента"), "Партнер");
	
	
	Возврат МаппингТипов[(ТипЗнч(Ссылка))];
