...
Функция СведенияМЧД(МЧД) Экспорт
	...
	Реквизиты = "ДатаВыдачи, ДатаОкончания, ДатаОбновленияСтатуса, СтатусВРеестреФНС, ДоверительЮЛ_КПП,
				...
				|ЭлектроннаяПодпись, XMLфайлМЧД, ИмяФайлаЗаявленияНаОтзыв, ФайлЗаявленияНаОтзыв, 
				|ЭлектроннаяПодписьЗаявленияНаОтзыв, ПометкаУдаления";
	...
	СведенияМЧД.ДанныеПодписи = РеквизитыМЧД.ЭлектроннаяПодпись.Получить();
	СведенияМЧД.ДанныеФайла = РеквизитыМЧД.XMLфайлМЧД.Получить(); 
	СведенияМЧД.ДанныеФайлаЗаявленияНаОтзыв = РеквизитыМЧД.ФайлЗаявленияНаОтзыв.Получить();
	СведенияМЧД.ДанныеПодписиЗаявленияНаОтзыв = РеквизитыМЧД.ЭлектроннаяПодписьЗаявленияНаОтзыв.Получить();
...
//
Функция ЗаполнитьМЧД(НомерДоверенности, ИННДоверителя, ИННПредставителя, Сведения) Экспорт
	...
	ДанныеДоверенности.ИННПредставителя = ИННПредставителя;
	...
	РезультатЗагрузки = 
		ЗагрузитьЭлементИзФайлаОбмена(ДанныеДляЗагрузки,
			ТребуетсяПроверкаМЧДНаКлиенте, ДополнительныеСведения, ОбновлятьСуществующуюМЧД);
...
Функция ПолучитьДвоичныеДанныеЗаявленияНаОтмену(Ссылка) Экспорт
	...
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "ФайлЗаявленияНаОтзыв").Получить();
...
Функция ЗагрузитьМЧДИзФайла(ДанныеФайла, МЧД = Неопределено, ДополнительныеСведения = Неопределено) Экспорт
	...
	РезультатЗагрузки = ЗагрузитьЭлементИзФайлаОбмена(ДанныеДляЗагрузки, ТребуетсяПроверкаМЧДНаКлиенте, 
		ДополнительныеСведения, ОбновлятьСуществующуюМЧД, МЧД, РезультатПроверкиНаСервере);
	Результат.ТребуетсяПроверкаМЧДНаКлиенте = ТребуетсяПроверкаМЧДНаКлиенте;
...
//                                             и необходимо выполнить проверку на клиенте.
//
Функция ЗагрузитьЭлементИзФайлаОбмена(ДанныеДляЗагрузки, ТребуетсяПроверкаМЧДНаКлиенте, ДополнительныеСведения, 
	ОбновлятьСуществующий = Ложь, МЧД = Неопределено, РезультатПроверкиНаСервере = Неопределено) Экспорт
...
Функция ПолучитьДоверенностиПоОтбору(Доверитель, Представитель, ДействительныеНаДату) Экспорт
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.МашиночитаемыеДоверенностиОрганизаций.Представители КАК ТаблицаПредставителей
	|		ПО МашиночитаемыеДоверенностиОрганизаций.Ссылка = ТаблицаПредставителей.Ссылка
	...
	Если Не ПустаяСтрока(ПредставительИНН) Тогда
		...
		ТекстыУсловий.Добавить("(ТаблицаПредставителей.ФЛ_ИНН = &ПредставительИНН
		|			ИЛИ ТаблицаПредставителей.ЮЛ_ИНН = &ПредставительИНН)");
	КонецЕсли;
	
	Если Не ПустаяСтрока(ПредставительКПП) Тогда
		Запрос.УстановитьПараметр("ПредставительКПП", ПредставительКПП);
		ТекстыУсловий.Добавить("ТаблицаПредставителей.ЮЛ_КПП = &ПредставительКПП");
	КонецЕсли;
...
Процедура УстановитьПометкуОтозвана(Доверенность, Значение, ДатаОтзыва) Экспорт
	...
	ДоверенностьОбъект = Доверенность.ПолучитьОбъект();
	ДоверенностьОбъект.Отозвана = Значение;
	ДоверенностьОбъект.ДатаОтзыва = ?(Значение, ДатаОтзыва, '00010101');
	ДоверенностьОбъект.Записать();
...
Функция ЗаписатьЭлементСправочника(ДанныеДоверенности, ОбновлятьСуществующий, ДанныеДляЗагрузки,
	...
	Если ЕстьПодписи И ЭтоНайденнаяНеРеестроваяМЧД И Не ЗагружаемРеестровуюМЧД Тогда
		ХранилищеДанныхДоверенности = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НашлиЭлемент, "XMLфайлМЧД");
...
Функция ДанныеДоверенностиВСтрокуXMLВерсия002(ДанныеДоверенности)
	...
	Если ДанныеДоверенности.ДоверительЮЛ_ТипОрганизации = "ИО" Тогда
		...
		Если ЗначениеЗаполнено(ДанныеДоверенности.ДоверительФЛ_Гражданство) Тогда
			ДоверительФЛ_Гражданство = ИнтеграцияБСПБЭД.КодСтраны(ДанныеДоверенности.ДоверительФЛ_Гражданство);
		КонецЕсли;
	...
	ИначеЕсли ДанныеДоверенности.ДоверительЮЛ_ТипОрганизации = "ЮЛ" Тогда
		...
		Если ЗначениеЗаполнено(ДанныеДоверенности.ЛицоБезДовФЛ_Гражданство) Тогда
			ЛицоБезДовФЛ_Гражданство = ИнтеграцияБСПБЭД.КодСтраны(ДанныеДоверенности.ЛицоБезДовФЛ_Гражданство);
		КонецЕсли;
		...
		Если Не ПустаяСтрока(Представитель.ПредставительЮЛ_ИНН) Тогда
			...
			Если Представитель.ТипПредставителя = "ИП" Тогда
				...
				Если ЗначениеЗаполнено(Представитель.ПредставительФЛ_Гражданство) Тогда
					ПредставительФЛ_Гражданство = ИнтеграцияБСПБЭД.КодСтраны(Представитель.ПредставительФЛ_Гражданство);
				КонецЕсли;
...
Функция ДанныеДляПроверкиВРеестреФНС(Доверенность) Экспорт
	...
	Запрос.Текст =
		...
		|	МашиночитаемыеДоверенностиОрганизаций.НомерДоверенности КАК НомерДоверенности,
		|	МашиночитаемыеДоверенностиОрганизаций.СтатусВРеестреФНС КАК СтатусВРеестреФНС,
		|	МашиночитаемыеДоверенностиОрганизаций.ИдентификаторТранзакции КАК ИдентификаторТранзакции,
		|	МашиночитаемыеДоверенностиОрганизаций.ДатаОтзыва КАК ДатаОтзыва
		|ИЗ
		|	Справочник.МашиночитаемыеДоверенностиОрганизаций КАК МашиночитаемыеДоверенностиОрганизаций
		|ГДЕ
	...
	Если Выборка.Следующий() Тогда
		...
		ДанныеДоверенности.ДатаОтзыва = Выборка.ДатаОтзыва;
	КонецЕсли;
...
Функция ДанныеДоверенностиДляКонтейнера(Доверенность) Экспорт
	...
	Реквизиты = "XMLфайлМЧД, ЭлектроннаяПодпись, ДатаВыдачи, ДатаОкончания, СтатусВРеестреФНС,
		|НомерДоверенности, ДоверительЮЛ_ИНН, ДоверительФЛ_ИНН, Представители, СведенияОбИнформационнойСистеме";
	...
	Пока ВыборкаПредставители.Следующий() Цикл
		ДанныеДоверенности.ПредставительИНН = 
			?(ЗначениеЗаполнено(ВыборкаПредставители.ЮЛ_ИНН), ВыборкаПредставители.ЮЛ_ИНН, ВыборкаПредставители.ФЛ_ИНН);
		Прервать;
	КонецЦикла;
