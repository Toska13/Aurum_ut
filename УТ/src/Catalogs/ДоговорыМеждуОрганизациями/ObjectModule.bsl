...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ПараметрыКорреспонденции =
		ДенежныеСредстваСерверЛокализация.ПараметрыКорреспонденцииСоСчетомЦифровогорубля(БанковскийСчет,
	...
	Если Не ПараметрыКорреспонденции.ДопустимаКорреспонденцияСоСчетомЦифровогоРубля Тогда
		...
		ИмяЭлементаПозиционирования =
			?(ПараметрыКорреспонденции.КоррСчетЦифровогоРубля, "БанковскийСчет", "БанковскийСчетПолучателя");
...
Процедура ИнициализироватьСправочник(ДанныеЗаполнения = Неопределено, ЗаполнятьВсеРеквизиты = Истина) Экспорт
	...
	Если ЗаполнятьВсеРеквизиты Тогда
		...
		Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") И ДанныеЗаполнения.Свойство("ОрганизацияПолучатель") Тогда
		...
		Если ТипЗнч(ДанныеЗаполнения) <> Тип("Структура") Или Не ДанныеЗаполнения.Свойство("БанковскийСчетПолучателя") Тогда
			ПараметрыСчета = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации();
			ПараметрыСчета.Организация = ОрганизацияПолучатель;
			ПараметрыСчета.ИсключитьСчетаВВалюте = ОплатаВВалюте;
			ПараметрыСчета.Валюта = ЗначениеНастроекПовтИсп.ВалютаРегламентированногоУчетаОрганизации(Организация);
			БанковскийСчетПолучателя = ДенежныеСредстваСервер.ПолучитьБанковскийСчетОрганизацииПоУмолчанию(
				ПараметрыСчета, БанковскийСчетПолучателя);
		КонецЕсли;
