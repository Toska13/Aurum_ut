...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ТолькоОперацииПередачи")
		И ЗначениеЗаполнено(Параметры.ТолькоОперацииПередачи)
		И Параметры.ТолькоОперацииПередачи Тогда
		
		Если ИспользоватьПередачуНаОтветственноеХранениеСПравомПродажи
			ИЛИ ИспользоватьКомиссиюПриПродажах
			ИЛИ ИспользоватьОтгрузкуБезПереходаПраваСобственности Тогда
			
			ГруппаОтбора = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
							Список.Отбор.Элементы,
							"ТолькоОперацииПередачи",
		...
		ОперацииПередачиОтгрузки = Неопределено;
		Если ИспользоватьОтгрузкуБезПереходаПраваСобственности
			И Параметры.Свойство("ОперацииПередачиОтгрузки", ОперацииПередачиОтгрузки)
			И ОперацииПередачиОтгрузки = Истина Тогда
			
			ГруппаОтбораПередачиОтгрузки = ГруппаОтбора.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
			ГруппаОтбораПередачиОтгрузки.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ;
		
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораПередачиОтгрузки,
	...
	Если Не Параметры.ТолькоОперацииПередачи
		И ЗначениеЗаполнено(Параметры.ХозяйственнаяОперация) Тогда
	...
	Если Параметры.Свойство("КомиссионныеПродажи25")
		И ЗначениеЗаполнено(Параметры.КомиссионныеПродажи25)
		И ИспользоватьКомиссиюПриПродажах Тогда
		
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			Список,
			"КомиссионныеПродажи25",
			Параметры.КомиссионныеПродажи25,
			ВидСравненияКомпоновкиДанных.Равно,
			,
			Истина);
	...
	Если ИспользоватьОтгрузкуБезПереходаПраваСобственности
		И Параметры.Свойство("ОтгрузкаБезПереходаПраваСобственности25", ОтгрузкаБезПереходаПраваСобственности25)
		И ЗначениеЗаполнено(ОтгрузкаБезПереходаПраваСобственности25) Тогда
	...
	Если Параметры.Свойство("ИспользуютсяДоговорыКонтрагентов")
		И Параметры.ИспользуютсяДоговорыКонтрагентов <> Неопределено Тогда
		
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список,
			"ИспользуютсяДоговорыКонтрагентов",
			Параметры.ИспользуютсяДоговорыКонтрагентов,
			ВидСравненияКомпоновкиДанных.Равно,
			,
			Истина);
	...
	Если ТолькоТиповые
		Или (Параметры.ТолькоТиповые
			И Не ТолькоИндивидуальные) Тогда
