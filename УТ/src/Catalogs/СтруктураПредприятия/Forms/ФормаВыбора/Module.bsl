...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	Статус = Настройки.Получить("СтатусДляОтбора");
	Если ЗначениеЗаполнено(Статус) Тогда
		ПереключательСтатусов = Строка(Статус);
	КонецЕсли;
...
Процедура ПриСохраненииДанныхВНастройкахНаСервере(Настройки)
	Настройки.Вставить("СтатусДляОтбора", Статус);
КонецПроцедуры
#КонецОбласти
...
&НаКлиенте
Процедура ПереключательСтатусовПриИзменении(Элемент)
	УстановитьОтборПоСтатусу();
КонецПроцедуры
...
&НаКлиенте
Процедура СписокВыборЗавершение(Ответ, ВыбранноеЗначение) Экспорт
...
Процедура УстановитьОтборПоСтатусу()
	Если ПустаяСтрока(ПереключательСтатусов) Или ПереключательСтатусов = "Все" Тогда
		Статус = Перечисления.СтатусыОрганизацийПодразделений.ПустаяСсылка();
	ИначеЕсли ПереключательСтатусов = "НеДействует" Тогда
		Статус = Перечисления.СтатусыОрганизацийПодразделений.НеДействует;
		Элементы.Список.Отображение = ОтображениеТаблицы.Список;
	Иначе
