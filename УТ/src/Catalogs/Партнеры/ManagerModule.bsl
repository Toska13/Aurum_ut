...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("95824992-4f28-4d86-ab9e-99e9e5c15da4");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Если ИспользуютсяДопЯзыки Тогда
		...
		ЧастиЗапроса.Добавить(
			...
			|ГДЕ
			|	НЕ Партнеры.Наименование = """"
			|	И Партнеры.НаименованиеЯзык1 = """"
			|	
			|ОБЪЕДИНИТЬ 
			|
			|ВЫБРАТЬ
			...
			|	Справочник.Партнеры КАК Партнеры
			|ГДЕ
			|	НЕ ВЫРАЗИТЬ(Партнеры.Комментарий КАК СТРОКА(150)) = """"
			|	И ВЫРАЗИТЬ(Партнеры.КомментарийЯзык1 КАК СТРОКА(150)) = """"
			|	
			|ОБЪЕДИНИТЬ 
			|
			|ВЫБРАТЬ
			...
			|ГДЕ
			|	НЕ ВЫРАЗИТЬ(Партнеры.Комментарий КАК СТРОКА(150)) = """"
			|	И ВЫРАЗИТЬ(Партнеры.КомментарийЯзык2 КАК СТРОКА(150)) = """"
			|");
