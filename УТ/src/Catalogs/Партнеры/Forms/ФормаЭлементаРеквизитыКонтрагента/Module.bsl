...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Перечисления.ЮрФизЛицо.ВидКонтрагентаСПАРКРиски(Контрагент), ПараметрыОтображения);
	...
	Если СтрНайти(ИмяСобытия, "Запись_") > 0 Тогда
		Если ЗначениеЗаполнено(Параметр) И ТипЗнч(Параметр) = Тип("Структура") Тогда
			...
			Если ЗначениеЗаполнено(Контрагент)
				И (Контрагент = КонтрагентПартнера) Тогда
...
Процедура ГруппаСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница = Элементы.ГруппаКонтактнаяИнформация Тогда
		
		ИмяКоманды = Команды.ОткрытьСписокТочекДоставки.Имя;
		
		Если Элементы.Найти(ИмяКоманды) = Неопределено Тогда
			
			ДоставкаТоваровПриСменеСтраницы(ИмяКоманды);
...
&НаКлиенте
Процедура ОткрытьСписокТочекДоставки(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка)
	 ИЛИ Модифицированность Тогда
		...
		ТекстВопроса = Нстр("ru = 'Данные еще не записаны.
		|Переход к точкам доставки возможен только после записи элемента.
		|Записать элемент?'");
		
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ОткрытьФормуТочекДоставкиЗавершение", ЭтотОбъект);
		
		ПоказатьВопрос(ОповещениеОЗавершении, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		
	Иначе
		
		ОткрытьСписокПодчиненныхТочекДоставки();
...
Процедура ПриСозданииИЧтенииНаСервере()
	...
	ДоставкаТоваров.ЗаполнитьСписокВыбораПланированияДоставки(Элементы.ВидПланированияДоставки.СписокВыбора, Объект);
	ИспользуетсяДоставка25 = ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25();
	Если ИспользуетсяДоставка25 Тогда
		
		ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ШаблонЭтикетки", "Видимость", Ложь);
...
Процедура ЗаполнитьОписаниеДействующихПараметров(ИмяРегистра = "")
	...
	ПартнерыИКонтрагентыЛокализация.ЗаполнитьОписаниеДействующихПараметровКонтрагентов(ЭтотОбъект, КонтрагентПартнера, ИмяРегистра);
	
КонецПроцедуры

&НаСервере
Процедура ДоставкаТоваровПриСменеСтраницы(ИмяКоманды)
	
	ДоставкаТоваров.ДобавитьКомандуПереходаКСпискуТочекДоставки(
		ЭтотОбъект,
		Элементы.ГруппаКонтактнаяИнформация.Имя,
		ИмяКоманды);
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьСписокПодчиненныхТочекДоставки()
	...
	ОткрытьФорму("Справочник.ТочкиДоставки.Форма.ТочкиДоставкиВладельца", ПараметрыОткрытия);
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьФормуТочекДоставкиЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось записать элемент'");
