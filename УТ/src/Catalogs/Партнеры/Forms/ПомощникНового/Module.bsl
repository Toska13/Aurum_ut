...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Не Параметры.Свойство("РаботаСКонтрагентамиСозданиеИзДосьеКонтрагента")
	...
	Если ИспользуютсяДополнительныеЯзыки
		Или Не Метаданные.Языки.Количество() = 1 Тогда
		
		ПараметрыМультиязычныхРеквизитов = Новый Структура;
		
		Если ИспользуютсяТолькоПартнеры Тогда
			
			ПараметрыМультиязычныхРеквизитов.Вставить("СокрЮридическоеНаименованиеТолькоПартнеры", "Наименование");
			
			Элементы["СокрЮридическоеНаименованиеТолькоПартнеры"].КнопкаОткрытия = Истина;
			Элементы["СокрЮридическоеНаименованиеТолькоПартнеры"].УстановитьДействие("Открытие", "Подключаемый_Открытие");
			
		Иначе
			
			ПараметрыМультиязычныхРеквизитов.Вставить("ПубличноеНаименование", "Наименование");
			ПараметрыМультиязычныхРеквизитов.Вставить("СокрЮрНаименованиеКонтрагента", "Наименование");
			
			Элементы["ПубличноеНаименование"].КнопкаОткрытия = Истина;
			Элементы["ПубличноеНаименование"].УстановитьДействие("Открытие", "Подключаемый_Открытие");
			
			Элементы["СокрЮрНаименованиеКонтрагента"].КнопкаОткрытия = Истина;
			Элементы["СокрЮрНаименованиеКонтрагента"].УстановитьДействие("Открытие", "Подключаемый_Открытие");
			
		КонецЕсли;
		
		ПараметрыМультиязычныхРеквизитов.Вставить("РабочееНаименованиеЧастноеЛицо", "Наименование");
		Элементы["РабочееНаименованиеЧастноеЛицо"].КнопкаОткрытия = Истина;
		Элементы["РабочееНаименованиеЧастноеЛицо"].УстановитьДействие("Открытие", "Подключаемый_Открытие");
...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьВидимостьЗоныДоставки();
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ПослеВводаСтрокНаРазныхЯзыках"
		И ЭтотОбъект.УникальныйИдентификатор = Параметр.УникальныйИдентификатор Тогда
		
		Если ИспользуютсяТолькоПартнеры Тогда
			
			ПолноеЮридическоеНаименование = ПартнерОбъектДопРеквизиты.Наименование;
			
		Иначе
			
			Если ИзменениеНаименованияКонтрагентаПартнера = "Партнер" Тогда
				
				ПолноеНаименование = ПартнерОбъектДопРеквизиты.Наименование;
				ИзменениеНаименованияКонтрагентаПартнера = "";
				
				Если ЭтоКомпания = 1 Тогда
					
					КонтрагентДляМеждународныхРеквизитов.Наименование = ПартнерОбъектДопРеквизиты.Наименование;
					КонтрагентДляМеждународныхРеквизитов.НаименованиеЯзык1 = ПартнерОбъектДопРеквизиты.НаименованиеЯзык1;
					КонтрагентДляМеждународныхРеквизитов.НаименованиеЯзык2 = ПартнерОбъектДопРеквизиты.НаименованиеЯзык2;
					
				КонецЕсли;
				
			ИначеЕсли ИзменениеНаименованияКонтрагентаПартнера = "Контрагент" Тогда
				
				ПолноеЮридическоеНаименование = КонтрагентДляМеждународныхРеквизитов.Наименование;
				ИзменениеНаименованияКонтрагентаПартнера = "";
...
Процедура Подключаемый_Открытие(Элемент, СтандартнаяОбработка)
	...
	Если Элемент.Имя = "СокрЮрНаименованиеКонтрагента" Тогда
		
		МультиязычностьКлиент.ПриОткрытии(ЭтотОбъект, КонтрагентДляМеждународныхРеквизитов, Элемент, СтандартнаяОбработка);
		ИзменениеНаименованияКонтрагентаПартнера = "Контрагент";
		
	Иначе
		
		МультиязычностьКлиент.ПриОткрытии(ЭтотОбъект, ПартнерОбъектДопРеквизиты, Элемент, СтандартнаяОбработка);
		ИзменениеНаименованияКонтрагентаПартнера = "Партнер";
...
&НаКлиенте
Процедура УстановитьФактическийАдресРавнымЮридическому(Команда)
	
	Если Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаДополнительныеСведения Тогда
		
		ФактическийАдрес              = ЮридическийАдрес;
		ФактическийАдресЗначенияПолей = ЮридическийАдресЗначенияПолей;
		
	Иначе
		
		ФактическийАдресГоловногоКонтрагента              = ЮридическийАдресГоловногоКонтрагента;
		ФактическийАдресГоловногоКонтрагентаЗначенияПолей = ЮридическийАдресГоловногоКонтрагентаЗначенияПолей;
...
Функция СводныйЗапрос(СтруктураПоиска)
	...
	|		КОГДА РеквизитыКонтрагента.Реквизит = &СтрокаНалоговыйНомер
...
Процедура РедактированиеКонтактнойИнформацииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если СтрСравнить(ДополнительныеПараметры.ИмяРеквизитаПредставление, "ФактическийАдрес") = 0 Тогда
...
Процедура ПриИзмененииIBAN(ЭтоГоловнойКонтрагент=Ложь)
	...
	ДанныеСчета = ДенежныеСредстваКлиентСервер.НовыеДанныеБанковскогоСчета();
	ДенежныеСредстваКлиентСервер.ЗаполнитьДанныеБанковскогоСчета(ДанныеСчета, ЭтотОбъект, Постфикс);

	Элементы["ГруппаОшибкаВНомереСчета" + Постфикс].Видимость = Ложь;

	ЭтотОбъект["ИностранныйБанк" + Постфикс] = ЭтоИностранныйБанк(ДанныеСчета);
	...
	ДенежныеСредстваКлиентЛокализация.ПроверитьНомерСчета(ЭтотОбъект, Постфикс, ДанныеСчета);
...
Процедура ПриИзмененииИспользуетсяБанкДляРасчетов(ЭтоГоловнойКонтрагент=Ложь)
	...
	Иначе
		...
		ОчиститьБанковскиеРеквизиты(ЭтотОбъект, Истина, ЭтоГоловнойКонтрагент);
...
Процедура ЗаполнитьРеквизитыБанкаПоБанку(ТипБанка, Банк = "", ЭтоГоловнойКонтрагент=Ложь)
	...
	Если ТипБанка = "Банк" Тогда
		...
		Если ЭтоГоловнойКонтрагент Тогда
			...
			СтранаБанкаГоловнойКонтрагент = ДенежныеСредстваСервер.СтранаБанкаПоКоду(РеквизитыБанка.КодСтраны);
		Иначе
	...
	ИначеЕсли ТипБанка = "БанкДляРасчетов" Тогда
		...
		Если ЭтоГоловнойКонтрагент Тогда
			СтранаБанкаДляРасчетовГоловнойКонтрагент = ДенежныеСредстваСервер.СтранаБанкаПоКоду(РеквизитыБанка.КодСтраны);
		Иначе
...
Процедура ПриИзмененииБанка(ЭтоГоловнойКонтрагент=Ложь)
	...
	ПартнерыИКонтрагентыЛокализацияКлиент.ПриИзмененииБанкаЛокализация(ЭтотОбъект, Постфикс);
...
Функция ЗаполнитьРеквизитыБанкаПоБИК(ТипБанка, Знач БИК = "", ЭтоГоловнойКонтрагент=Ложь)
	...
	Если ТипБанка = "Банк" Тогда
		ОчиститьБанковскиеРеквизиты(ЭтотОбъект, Ложь, ЭтоГоловнойКонтрагент);
	Иначе
		ОчиститьБанковскиеРеквизиты(ЭтотОбъект, Истина, ЭтоГоловнойКонтрагент);
	КонецЕсли;
...
&НаКлиенте
Функция ЭтоИностранныйБанк(ДанныеСчета)

	Возврат ДенежныеСредстваКлиентЛокализация.ЭтоИностранныйБанк(МеждународныеРеквизитыБанковскихСчетов, ДанныеСчета);

КонецФункции

&НаКлиентеНаСервереБезКонтекста
Процедура ОчиститьБанковскиеРеквизиты(Форма, ЭтоБанкДляРасчетов = Ложь, ЭтоГоловнойКонтрагент = Ложь)

	Если Не ЭтоБанкДляРасчетов Тогда
		
		СтруктураРеквизитов = РеквизитыБанка(ЭтоГоловнойКонтрагент);
		
		Если ЭтоГоловнойКонтрагент Тогда
			СтруктураРеквизитов.Вставить("СтранаБанкаГоловнойКонтрагент");
		Иначе
			СтруктураРеквизитов.Вставить("СтранаБанка");
		КонецЕсли;
	...
	Иначе
		...
		Если ЭтоГоловнойКонтрагент Тогда
			СтруктураРеквизитов.Вставить("СтранаБанкаДляРасчетовГоловнойКонтрагент");
		Иначе
			СтруктураРеквизитов.Вставить("СтранаБанкаДляРасчетов");
		КонецЕсли;
		
	КонецЕсли;
	
	ДенежныеСредстваКлиентСервер.ОчиститьБанковскиеРеквизитыНаФорме(Форма, СтруктураРеквизитов);
...
Процедура ЗаписатьПартнераИСвязанныеДанные(СписокСозданных)
	...
	Если ДействиеПоРезультатамНайдено <> 0 Тогда
		...
		Если ЗначениеЗаполнено(ЗонаДоставки)
		   И ЗонаДоставкиДоступна(ЭтотОбъект) Тогда
			
			Если ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка20() Тогда
				
				ОбъектПартнер.ЗонаДоставки = ЗонаДоставки;
				
			КонецЕсли;
			
			ОбъектПартнер.ДополнительныеСвойства.Вставить("ЗонаДоставки", ЗонаДоставки);
...
&НаКлиенте
Процедура УстановитьВидимостьЗоныДоставки()
	
	ВидимостьЗоныДоставки = ЗонаДоставкиДоступна(ЭтотОбъект);
	
	ИменаЭлементов = Новый Массив;
	ИменаЭлементов.Добавить(Элементы.ЗонаДоставки.Имя);
	ИменаЭлементов.Добавить(Элементы.ЗонаДоставкиЧастноеЛицо.Имя);
	
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(
		Элементы,
		ИменаЭлементов,
		"Видимость",
		ВидимостьЗоныДоставки);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция ЗонаДоставкиДоступна(Форма)
	
	ДоступностьЗоныДоставки = НЕ ПустаяСтрока(Форма.ФактическийАдрес);
	
	Возврат ДоступностьЗоныДоставки;
