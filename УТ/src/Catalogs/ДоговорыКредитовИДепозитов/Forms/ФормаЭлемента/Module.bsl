...
Процедура ХарактерДоговораПриИзмененииСервер()
	...
	ОчиститьЗаполняемыеПоСтатистикеРеквизиты("ХарактерДоговора");
...
Процедура ТипДоговораСрочностиПриИзмененииСервер(ИмяРеквизита)
	...
	ОчиститьЗаполняемыеПоСтатистикеРеквизиты(ИмяРеквизита);
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	УправлениеТорговлей = Не ПолучитьФункциональнуюОпцию("УправлениеПредприятием")
...
Процедура ЗаполнитьБанковскийСчетПоУмолчанию(ВладелецСчета, ОплатаВВалюте, БанковскийСчетВладельца, ВалютаВзаиморасчетов, ВалютаРегламентированногоУчета)
	...
	БанковскийСчетВладельца = Неопределено;
	
	Запрос = Новый Запрос("
	|ВЫБРАТЬ ПЕРВЫЕ 2
	|	БанковскиеСчета.Ссылка КАК БанковскийСчет,
	|	БанковскиеСчета.Ссылка = &ОсновнойСчет КАК Основной,
	...
	Иначе
		ОсновнойСчет = Справочники.Организации.ПолучитьОсновнойБанковскийСчет(ВладелецСчета);
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ОсновнойСчет", ОсновнойСчет);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() И (Выборка.Количество() = 1 Или Выборка.Основной) Тогда
...
&НаСервере
Процедура ОчиститьЗаполняемыеПоСтатистикеРеквизиты(КлючевойРеквизит)
	...
	Для каждого ОписаниеРеквизита Из ОписаниеРеквизитов Цикл
		Если ОписаниеРеквизита.Значение.Свойство(КлючевойРеквизит) Тогда
