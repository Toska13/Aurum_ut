...
Процедура УстановитьВерсиюФорматаОбменаККМED(Параметры = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|	Справочник.ОфлайнОборудование КАК ОфлайнОборудование
		|ГДЕ
		|	ОфлайнОборудование.ОбработчикОфлайнОборудования = &ОбработчикОфлайнОборудования
		|";
	
	Запрос.УстановитьПараметр("ОбработчикОфлайнОборудования", Перечисления.ОбработчикиОфлайнОборудования.Обработчик1СККМED); 
	...
	Пока Выборка.Следующий() Цикл
		НачатьТранзакцию();
		Попытка
			СтруктураПараметров = ПараметрыУстройства(Выборка.Ссылка);
			
			Если Не СтруктураПараметров.Свойство("ВерсияФорматаОбмена") Тогда
				СтруктураПараметров.Вставить("ВерсияФорматаОбмена", "1.8.Cashbox");
			КонецЕсли;
			
			СохранитьПараметрыУстройства(Выборка.Ссылка, СтруктураПараметров);
			
			ЗафиксироватьТранзакцию();
		Исключение
			ОтменитьТранзакцию();
			ШаблонТекстаОшибки = НСтр("ru = 'Не удалось обработать обновление Офлайн-оборудования по причине: %1'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьВерсиюФорматаОбменаККМED", Истина);
