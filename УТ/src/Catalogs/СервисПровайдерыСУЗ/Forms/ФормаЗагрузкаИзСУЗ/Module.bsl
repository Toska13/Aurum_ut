...
Процедура ВыполнитьОбновлениеДанныхСписка()
	...
	Если ПараметрыЗаполнения.ТоварныеГруппы.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не найдено ни одного вида маркируемой продукции. Обновление невозможно.'"));
		Возврат;
	КонецЕсли;
	
	Для Каждого Организация Из ПараметрыЗаполнения.Организации Цикл
		
		РезультатЗагрузки = ЗагрузитьДанныеИзСУЗ(Организация, ПараметрыЗаполнения.ТоварныеГруппы);
		
		Если РезультатЗагрузки.ТребуетсяОбновлениеКлючаСессии Тогда
			ЗапроситьКлючСессииНачало(Организация, ПараметрыЗаполнения.ТоварныеГруппы);
...
Функция ПолучитьПараметрыЗаполнения()
	...
	ТоварныеГруппы = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступныеТоварныеГруппы();
	
	Для Индекс = 0 По ТоварныеГруппы.ВГраница() Цикл
		Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ТоварныеГруппы[Индекс]) Тогда
			ТоварныеГруппы.Удалить(Индекс);
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Результат.Вставить("Организации",    РезультатЗапроса.ВыгрузитьКолонку("Организация"));
	Результат.Вставить("ТоварныеГруппы", ТоварныеГруппы);
...
&НаКлиенте
Процедура ЗапроситьКлючСессииНачало(Организация, ТоварныеГруппы)
	...
	ДополнительныеПараметры.Вставить("ТоварныеГруппы", ТоварныеГруппы);
...
Процедура ЗапроситьКлючСессииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ОшибкаАвторизации Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			...
			ТекстОшибки));
	Иначе
		Результат = ЗагрузитьДанныеИзСУЗ(ДополнительныеПараметры.Организация, ДополнительныеПараметры.ТоварныеГруппы);
...
&НаСервере
Функция ЗагрузитьДанныеИзСУЗ(Организация, ТоварныеГруппы)
	...
	Для Каждого ТоварнаяГруппа Из ТоварныеГруппы Цикл
		...
		Если Не ЗаказНаЭмиссиюСУЗКлиентСервер.ПоддерживаетсяФорматОбменаСУЗ(НастройкаОбменаСУЗ.ФорматОбмена, ТоварнаяГруппа) Тогда
			Продолжить;
		КонецЕсли;
		
		Результат = ИнтерфейсСУЗ.СписокСервисПровайдеров_V2(ТоварнаяГруппа, НастройкаОбменаСУЗ);
		...
		Для Каждого Строка Из Результат.Список Цикл
			Если Строка.ТоварныеГруппы.Количество() > 0 Тогда
				СтрокаСписка = ЗагруженныеДанные[Строка.Идентификатор];
				...
				КонецЕсли;
				Для Каждого ТоварнаяГруппаСтроки Из Строка.ТоварныеГруппы Цикл
					Если СтрокаСписка.ТоварныеГруппы.НайтиПоЗначению(ТоварнаяГруппаСтроки) = Неопределено Тогда
						СтрокаСписка.ТоварныеГруппы.Добавить(ТоварнаяГруппаСтроки);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;
...
Процедура ВыполнитьЗагрузкуДанных()
	...
	Для Каждого Строка Из ВыбранныеСтроки Цикл
		...
		Для Каждого ТоварныеГруппыЭлемент Из Строка.ТоварныеГруппы Цикл
			СтрокаТоварныеГруппы = Объект.ТоварныеГруппы.Добавить();
			СтрокаТоварныеГруппы.ТоварнаяГруппа = ТоварныеГруппыЭлемент.Значение;
		КонецЦикла;
