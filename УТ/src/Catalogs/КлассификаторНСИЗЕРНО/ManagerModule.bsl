...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Обработчик.Версия          = ОбновлениеИнформационнойБазыГосИС.ВерсияОбработчиковОбновленияПереходаНаКонструкторНастроек(ОбъектМетаданных);
		Обработчик.Процедура       = "Справочники.КлассификаторНСИЗЕРНО.ОбработатьДанныеДляПереходаНаКонструкторНастроекВМонопольномРежиме";
	...
	Иначе
		...
		ОбновлениеИнформационнойБазыГосИС.ДобавитьОбработчикОбновленияПереходаНаКонструкторНастроек(
			Обработчики,
			ОбъектМетаданных,
			Новый УникальныйИдентификатор("b71ede39-c170-4225-942c-24e1f417ffe2"));
...
// Обработка данных перехода на конструктор настроек видов продукции в монопольном режиме для неразделенных классификаторов
// 
Процедура ОбработатьДанныеДляПереходаНаКонструкторНастроекВМонопольномРежиме() Экспорт
	...
	Запрос.Текст =
		...
		|ИЗ
		|	Справочник.КлассификаторНСИЗЕРНО КАК Таблица
		|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбъектДанных.РазделУчета = ОбновлениеИнформационнойБазыЗЕРНО.РазделУчетаПоВидуПродукции(ОбъектДанных.УдалитьВидПродукции);
...
Функция ЗагрузитьКлассификаторНСИ(ЭлементДанных, Организация, Подразделение, ПараметрыОбмена, ВидКлассификатора, СуществующийОбъект = Неопределено, ТребуетсяПоиск = Истина) Экспорт
	...
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовЗЕРНО.ОКПД2 Тогда
		...
		Если ЭлементДанных.isGrain <> Неопределено
			...
			СуществующийОбъект.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.Зерно;
		ИначеЕсли ЭлементДанных.isProduct <> Неопределено
			И ЭлементДанных.isProduct Тогда
			СуществующийОбъект.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна;
		Иначе
			СуществующийОбъект.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.ПустаяСсылка();
		КонецЕсли;
...
Процедура ПриЗагрузкеКлассификатора(Идентификатор, Версия, Адрес, Обработан, ДополнительныеПараметры, ФайлыКлассификатора) Экспорт
	...
	Попытка
		...
		Для Каждого ИмяФайла Из ФайлыКлассификатора Цикл
			...
			ДанныеОбработки.Вставить("ПрочитаноВерсиейAPI");
