...
// Возвращаемое значение:
//   Структура:
//   * ХозяйствующийСубъектВЕТИС - СправочникСсылка.ХозяйствующиеСубъектыВЕТИС - ссылка на организацию.
//   * ПредприятиеВЕТИС - СправочникСсылка.ПредприятияВЕТИС - ссылка на предприятие
Функция ХозяйствующийСубъектИПредприятиеПоУмолчанию() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ХозяйствующийСубъектВЕТИС", ПустаяСсылка());
	Результат.Вставить("ПредприятиеВЕТИС", Справочники.ПредприятияВЕТИС.ПустаяСсылка());
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 2
	|	ХозяйствующийСубъектВЕТИС.Ссылка                              КАК ХозяйствующийСубъект,
	|	КОЛИЧЕСТВО((ЕСТЬNULL(ПредприятияВЕТИС.Ссылка, НЕОПРЕДЕЛЕНО))) КАК КоличествоПредприятий,
	|	МАКСИМУМ(ЕСТЬNULL(ПредприятияВЕТИС.Ссылка, НЕОПРЕДЕЛЕНО))     КАК ПредприятиеВЕТИС
	|ИЗ
	|	Справочник.ХозяйствующиеСубъектыВЕТИС КАК ХозяйствующийСубъектВЕТИС
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХозяйствующиеСубъектыВЕТИС.Предприятия КАК ПредприятияВЕТИС
	|		ПО ХозяйствующийСубъектВЕТИС.Ссылка = ПредприятияВЕТИС.Ссылка
	|ГДЕ
	|	НЕ ХозяйствующийСубъектВЕТИС.ПометкаУдаления
	|	И ХозяйствующийСубъектВЕТИС.СоответствуетОрганизации
	|СГРУППИРОВАТЬ ПО
	|	ХозяйствующийСубъектВЕТИС.Ссылка
	|";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() И Выборка.Количество() = 1 Тогда
		
		Результат.ХозяйствующийСубъектВЕТИС = Выборка.ХозяйствующийСубъект;
		Если Выборка.КоличествоПредприятий = 1 Тогда
			Результат.ПредприятиеВЕТИС = Выборка.ПредприятиеВЕТИС;
		КонецЕсли;
...
Функция ХозяйствующийСубъектИПредприятиеПоОрганизацииИСкладу(Организация, Склад)
	...
	Если Выборка.Количество() = 1 Тогда
		Выборка.Следующий();
		ЗаполнитьЗначенияСвойств(Результат, Выборка);
	ИначеЕсли Не ИнтеграцияВЕТИС.ИспользуетсяСопоставлениеТорговыхОбъектовКонтрагентов()
		И Выборка.Следующий() Тогда
		ЗаполнитьЗначенияСвойств(Результат, Выборка,, "Предприятие");
	КонецЕсли;
