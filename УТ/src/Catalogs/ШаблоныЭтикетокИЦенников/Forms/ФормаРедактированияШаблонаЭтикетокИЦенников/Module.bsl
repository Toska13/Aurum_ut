...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Элементы.Высота.ТолькоПросмотр = Ложь;
		Элементы.Ширина.ТолькоПросмотр = Ложь;
		Элементы.РазмерЯчейки.ТолькоПросмотр = Ложь;
...
Процедура НазначениеПриИзмененииСервер(Знач ОчищатьНазначение = Истина)
	...
	ИначеЕсли Объект.Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляДоставки Тогда
		...
		МассивТипов.Добавить(Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту"));
