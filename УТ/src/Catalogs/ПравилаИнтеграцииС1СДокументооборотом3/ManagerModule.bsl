...
//   ТаблицаЗначений:
//     * Ссылка - СправочникСсылка.ПравилаИнтеграцииС1СДокументооборотом3 - правило.
...
//     * ВидДокументаДО - Строка - представление вида документа ДО.
//     * ВидДокументаДОID - Строка - идентификатор вида документа ДО.
//
Функция ВсеПравилаИнтеграции() Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	ПравилаИнтеграцииС1СДокументооборотом3.ПредставлениеОбъектаДО КАК ПредставлениеОбъектаДО,
		|	ПравилаИнтеграцииС1СДокументооборотом3.ПредставлениеОбъектаИС КАК ПредставлениеОбъектаИС,
		...
		|	ПравилаИнтеграцииС1СДокументооборотом3.ВидДокументаДО КАК ВидДокументаДО,
		|	ПравилаИнтеграцииС1СДокументооборотом3.ВидДокументаДОID КАК ВидДокументаДОID
		|ИЗ
		|	Справочник.ПравилаИнтеграцииС1СДокументооборотом3 КАК ПравилаИнтеграцииС1СДокументооборотом3
		|ГДЕ
		|	ПравилаИнтеграцииС1СДокументооборотом3.ПометкаУдаления = ЛОЖЬ");
...
// Формирует инструкцию, помогающую написать выражение на встроенном языке.
//
// Параметры:
//   Параметры - см. Справочники.ПравилаИнтеграцииС1СДокументооборотом3.ПараметрыВыраженияНаВстроенномЯзыке
//   РеквизитыОбъектаДО - см. Справочники.ПравилаИнтеграцииС1СДокументооборотом3.РеквизитыОбъектаДО
//
// Возвращаемое значение:
//   Строка - инструкция в формате HTML.
//
Функция ИнструкцияВыражения(Параметры, РеквизитыОбъектаДО = Неопределено) Экспорт
	...
	НаименованиеИС = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СокращенноеНаименованиеКонфигурации();
	
	Если Параметры.ЭтоУсловиеПрименимостиПриВыгрузке Или Параметры.ЭтоУсловиеПрименимостиПриЗагрузке Тогда
		ИнструкцияМассив.Добавить(СтрШаблон(
			НСтр("ru = 'Выражение на встроенном языке 1С:Предприятия определяет применимость правила при создании объекта %1 на основании объекта %2.
				|Результат вычисления должен присваиваться свойству <b>Результат</b> переменной <b>Параметры</b>.
				|Значение <b>Истина</b> говорит о применимости правила, <b>Ложь</b> - о неприменимости.
				|Место выполнения выражения: на стороне %3.'"),
			?(Параметры.ЭтоУсловиеПрименимостиПриВыгрузке, НСтр("ru = '1С:Документооборота'"), НаименованиеИС),
			?(Параметры.ЭтоУсловиеПрименимостиПриВыгрузке, НаименованиеИС, НСтр("ru = '1С:Документооборота'")),
			НаименованиеИС));
	Иначе
		ИнструкцияМассив.Добавить(СтрШаблон(
	...
	Если (Параметры.ЭтоУсловиеПрименимостиПриВыгрузке И Не ЗначениеЗаполнено(Параметры.ТипОбъектаИС))
			Или (Параметры.ЭтоУсловиеПрименимостиПриЗагрузке И Не ЗначениеЗаполнено(Параметры.ТипОбъектаДО)) Тогда
		Возврат СтрСоединить(ИнструкцияМассив);
	КонецЕсли;
	
	Если Параметры.ЭтоПравилоЗагрузкиТаблицы Тогда
		ИнструкцияМассив.Добавить(
			СтрШаблон(
		...
		МетаданныеОбъекта = Метаданные.НайтиПоПолномуИмени(Параметры.ТипОбъектаИС);
	...
	Если Параметры.ДоступнаТекущаяСтрока Тогда
		ИнструкцияМассив.Добавить("<p>");
		ИнструкцияМассив.Добавить(СтрШаблон(
	...
	Если Параметры.ЭтоПравилоЗагрузкиРеквизита
			Или Параметры.ЭтоПравилоЗагрузкиТаблицы
			Или Параметры.ЭтоУсловиеПрименимостиПриЗагрузке Тогда
		ИнструкцияМассив.Добавить(НСтр("ru = '1С:Документооборота'"));
	Иначе
		ИнструкцияМассив.Добавить(НаименованиеИС);
	КонецЕсли;
	...
	ВыводРеквизитовОбъектаИС = Ложь;
	Если Параметры.ЭтоПравилоЗагрузкиРеквизита Или Параметры.ЭтоПравилоЗагрузкиТаблицы Тогда
		Если Параметры.МестоВыполненияВыражения = Перечисления.МестаВыполненияВыраженийНаВстроенномЯзыке.НаСторонеИС Тогда
			РеквизитыИсточника = РеквизитыОбъектаДО(Параметры.ТипОбъектаДО, Параметры.ВидДокументаДОID, Ложь);
			РеквизитыИсточника.Сортировать("ДопРеквизит, ЭтоТаблица, Имя");
		ИначеЕсли Параметры.МестоВыполненияВыражения = Перечисления.МестаВыполненияВыраженийНаВстроенномЯзыке.НаСторонеДО Тогда
			...
			Запрос.objectType.xdtoClassName = Параметры.ТипОбъектаДО;
			Запрос.objectType.presentation = Параметры.ТипОбъектаДО;
			...
			РеквизитыИсточника = СтруктураТаблицыРеквизитовДО();
			
			Для Каждого МетаданныеРеквизит Из МетаданныеОбъекта.attributes Цикл
				НоваяСтрока = РеквизитыИсточника.Добавить();
			...
			Для Каждого МетаданныеТаблица Из МетаданныеОбъекта.tabularSections Цикл
				...
				КонецЦикла;
			КонецЦикла;
			РеквизитыИсточника.Сортировать("ДопРеквизит, ТаблицаДляСортировки, ЭтоТаблица УБЫВ, Имя");
		Иначе
			ВызватьИсключение НСтр("ru = 'Не указано место выполнения выражения'");
		КонецЕсли;
	ИначеЕсли Параметры.ЭтоУсловиеПрименимостиПриВыгрузке Тогда
		РеквизитыИсточника = ИнтеграцияС1СДокументооборотБазоваяФункциональность.РеквизитыОбъектаИС(
			Параметры.ТипОбъектаИС,
			Истина);
		РеквизитыИсточника.Сортировать("ЭтоДополнительныйРеквизитИС, ЭтоТаблица, Имя");
		ВыводРеквизитовОбъектаИС = Истина;
	ИначеЕсли Параметры.ЭтоУсловиеПрименимостиПриЗагрузке Тогда
		Если РеквизитыОбъектаДО = Неопределено Тогда
			РеквизитыИсточника = РеквизитыОбъектаДО(Параметры.ТипОбъектаДО, Параметры.ВидДокументаДОID, Ложь);
		Иначе
			РеквизитыИсточника = РеквизитыОбъектаДО;
		КонецЕсли;
		РеквизитыИсточника.Сортировать("ДопРеквизит, ЭтоТаблица, Имя");
	Иначе
		ВызватьИсключение НСтр("ru = 'Не указан тип выражения'");
	КонецЕсли;
	
	ВывестиТаблицуРеквизитов(ИнструкцияМассив, Параметры, РеквизитыИсточника, ВыводРеквизитовОбъектаИС);
	
	Если (Параметры.ЭтоПравилоЗагрузкиРеквизита Или Параметры.ЭтоПравилоЗагрузкиТаблицы)
			И Параметры.МестоВыполненияВыражения = Перечисления.МестаВыполненияВыраженийНаВстроенномЯзыке.НаСторонеИС Тогда
		
		ИнструкцияМассив.Добавить("<p>");
		ИнструкцияМассив.Добавить(НСтр("ru ='К объекту'"));
		ИнструкцияМассив.Добавить(" ");
		
		Если ЗначениеЗаполнено(НаименованиеИС) Тогда
			ИнструкцияМассив.Добавить(НаименованиеИС);
		Иначе
		...
		РеквизитыПриемника = ИнтеграцияС1СДокументооборотБазоваяФункциональность.РеквизитыОбъектаИС(
			Параметры.ТипОбъектаИС,
			Истина);
		РеквизитыПриемника.Колонки.Добавить("ДопРеквизит", Новый ОписаниеТипов("Булево"));
		РеквизитыПриемника.Сортировать("ЭтоДополнительныйРеквизитИС, ЭтоТаблица, Имя");
		
		ВывестиТаблицуРеквизитов(ИнструкцияМассив, Параметры, РеквизитыПриемника, Истина);
...
//     * ТаблицаИсточник - Строка - имя таблицы, из которой будет выполняться загрузка. Используется для заполнения
//          реквизитов таблицы, в случае если для самой таблицы настроен вариант заполнения "Из таблицы объекта ...".
...
//     заполнения объекта ИС.
//
Процедура ОбновитьПредставление(ПравилоИнтеграции) Экспорт
	
	// Соберем представление объекта ИС.
	ПравилоИнтеграции.ПредставлениеОбъектаИС = "";
	ПравилоИнтеграции.ПредставлениеОбъектаИССКлючевымиПолями = "";
	
	МассивПолноеПредставлениеОбъектаИС = Новый Массив;
	
	ОбъектПотребителя = Метаданные.НайтиПоПолномуИмени(ПравилоИнтеграции.ТипОбъектаИС);
	Если ОбъектПотребителя <> Неопределено Тогда
		ПравилоИнтеграции.ПредставлениеОбъектаИС = ?(ОбъектПотребителя.Синоним = "",
			ОбъектПотребителя.Имя,
			ОбъектПотребителя.Синоним);
		
		МассивПолноеПредставлениеОбъектаИС.Добавить(ПравилоИнтеграции.ПредставлениеОбъектаИС);
		
		// Дополним представление ключевыми реквизитами.
		Если Не ПравилоИнтеграции.ИспользоватьУсловиеПрименимостиПриВыгрузке Тогда
			Для Каждого КлючевойРеквизит Из ПравилоИнтеграции.КлючевыеРеквизитыИС Цикл
	...
	ПравилоИнтеграции.ПредставлениеОбъектаИССКлючевымиПолями = СтрСоединить(МассивПолноеПредставлениеОбъектаИС, ", ");
	
	Если СокрЛП(ПравилоИнтеграции.Комментарий) <> "" Тогда
		// Комментарий не должен входить в ПредставлениеОбъектаИС, но должен входить в наименование.
		МассивПолноеПредставлениеОбъектаИС.Добавить(СокрЛП(ПравилоИнтеграции.Комментарий));
	КонецЕсли;
	
	// Соберем представление объекта ДО.
	МассивПредставлениеОбъектаДО = Новый Массив;
	МассивПредставлениеОбъектаДО.Добавить(ПравилоИнтеграции.ПредставлениеОбъектаДО);
	
	Если Не ПравилоИнтеграции.ИспользоватьУсловиеПрименимостиПриЗагрузке Тогда
		Для Каждого КлючевойРеквизит Из ПравилоИнтеграции.КлючевыеРеквизитыДО Цикл
	...
	ПравилоИнтеграции.ПредставлениеОбъектаДОСКлючевымиПолями = СтрСоединить(МассивПредставлениеОбъектаДО, ", ");
	
	// Соберем наименование.
	ПравилоИнтеграции.Наименование = СтрШаблон("%1 - %2",
		СтрСоединить(МассивПолноеПредставлениеОбъектаИС, ", "),
		ПравилоИнтеграции.ПредставлениеОбъектаДОСКлючевымиПолями);
...
//     * Ссылка - СправочникСсылка.ПравилаИнтеграцииС1СДокументооборотом3
//     * ПредставлениеОбъектаДОСКлючевымиПолями - Строка
//     * ПредставлениеОбъектаИССКлючевымиПолями - Строка
//     * Комментарий - Строка
//     * ТипОбъектаДО - Строка
//     * ТипОбъектаИС - Строка
//     * ПредставлениеВидаДокумента - Строка
//     * ИдентификаторВидаДокумента - Строка
//     * ИспользоватьУсловиеПрименимостиПриВыгрузке - Булево
//     * УсловиеПрименимостиПриВыгрузке - Строка
//     * ИспользоватьУсловиеПрименимостиПриЗагрузке - Булево
//     * УсловиеПрименимостиПриЗагрузке - Строка
//     * КлючевыеРеквизитыДО - ВыборкаИзРезультатаЗапроса
//
Функция ОбщийНаборПравилИнтеграции(ТипОбъектаДО, ВидДокументаДОID = "") Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	ПравилаИнтеграцииС1СДокументооборотом3.Комментарий КАК Комментарий,
		|	ПравилаИнтеграцииС1СДокументооборотом3.ТипОбъектаДО КАК ТипОбъектаДО,
		|	ПравилаИнтеграцииС1СДокументооборотом3.ТипОбъектаИС КАК ТипОбъектаИС,
		|	ПравилаИнтеграцииС1СДокументооборотом3.ВидДокументаДО КАК ПредставлениеВидаДокумента,
		|	ПравилаИнтеграцииС1СДокументооборотом3.ВидДокументаДОID КАК ИдентификаторВидаДокумента,
		|	ПравилаИнтеграцииС1СДокументооборотом3.ИспользоватьУсловиеПрименимостиПриВыгрузке КАК ИспользоватьУсловиеПрименимостиПриВыгрузке,
		|	ПравилаИнтеграцииС1СДокументооборотом3.УсловиеПрименимостиПриВыгрузке КАК УсловиеПрименимостиПриВыгрузке,
		|	ПравилаИнтеграцииС1СДокументооборотом3.ИспользоватьУсловиеПрименимостиПриЗагрузке КАК ИспользоватьУсловиеПрименимостиПриЗагрузке,
		|	ПравилаИнтеграцииС1СДокументооборотом3.УсловиеПрименимостиПриЗагрузке КАК УсловиеПрименимостиПриЗагрузке,
...
//   ДопЗапросы - Массив из ОбъектXDTO - список дополнительных запросов, которые в целях оптимизации требуется
//     выполнить в пакете с прочими запросами к 1С:Документообороту.
//   ОтветыНаДопЗапросы - Массив из ОбъектXDTO - неявно возвращаемое значение, список ответов на дополнительные запросы.
//
// Возвращаемое значение:
//   см. Справочники.ПравилаИнтеграцииС1СДокументооборотом3.СтруктураТаблицыРеквизитовДО
//
Функция РеквизитыОбъектаДО(ТипОбъектаДО, ВидДокументаДОID, ПолучитьВсе, НастройкиВида = Неопределено,
		ДопЗапросы = Неопределено, ОтветыНаДопЗапросы = Неопределено) Экспорт
	...
	Если ДопЗапросы <> Неопределено Тогда
		ЗапросыПакета = ОбщегоНазначения.СкопироватьРекурсивно(ДопЗапросы);
		НачальныйНомерЗапроса = ЗапросыПакета.Количество();
	КонецЕсли;
	...
	Если ДопЗапросы <> Неопределено Тогда
		ОтветыНаДопЗапросы = Новый Массив;
		Для Индекс = 0 По ДопЗапросы.ВГраница() Цикл
			ОтветНаДопЗапрос = ОтветНаЗапросыПакета.responses[Индекс];
			ОтветыНаДопЗапросы.Добавить(ОтветНаДопЗапрос);
		КонецЦикла;
	КонецЕсли;
	...
	Если ИнтеграцияС1СДокументооборот3.ЭтоДокументДО3(ТипОбъектаДО) Тогда
		...
		ДобавитьРеквизит(Реквизиты, "deletionMark", НСтр("ru = 'Пометка удаления'"), "Булево");
		...
		Если ПолучитьВсе Или НастройкиТипа.statusEnabled = Истина Тогда
			Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(НастройкиВида, "actionTypes") Тогда
				...
				КонецЦикла;
			Иначе
				ДобавитьРеквизит(Реквизиты, "statusApproval", НСтр("ru = 'Состояние согласования'"), "DMDocumentStatus",,, Ложь);
				ДобавитьРеквизит(Реквизиты, "statusConfirmation", НСтр("ru = 'Состояние утверждения'"), "DMDocumentStatus",,, Ложь);
				ДобавитьРеквизит(Реквизиты, "statusPerformance", НСтр("ru = 'Состояние исполнения'"), "DMDocumentStatus",,, Ложь);
				ДобавитьРеквизит(Реквизиты, "statusRegistration", НСтр("ru = 'Состояние регистрации'"), "DMDocumentStatus",,, Ложь);
				ДобавитьРеквизит(Реквизиты, "statusSigning", НСтр("ru = 'Состояние подписания'"), "DMDocumentStatus",,, Ложь);
				Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоСуществует(
						НастройкиТипа, "statusAcquaintance") Тогда
					ДобавитьРеквизит(Реквизиты, "statusAcquaintance", НСтр("ru = 'Состояние ознакомления'"), "DMDocumentStatus",,, Ложь);
				ИначеЕсли ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоСуществует(
						НастройкиТипа, "statusExamination") Тогда
					ДобавитьРеквизит(Реквизиты, "statusExamination", НСтр("ru = 'Состояние ознакомления'"), "DMDocumentStatus",,, Ложь);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если ПолучитьВсе Или НастройкиВида.isAPaymentRequest = Истина Тогда
			ДобавитьРеквизит(Реквизиты, "recipient", НСтр("ru = 'Получатель'"), ТипСторона, Истина,, Ложь);
		КонецЕсли;
		...
		Если ПолучитьВсе Или НастройкиВида.productRowsEnabled = Истина Тогда
			...
			Если ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("3.0.16.33") Тогда
				ДобавитьРеквизитТаблицы(Реквизиты,
					"productRows.rows", "VATRateUniversal", НСтр("ru = 'Ставка НДС'"), "DMVATRateUniversal");
			Иначе
				ДобавитьРеквизитТаблицы(Реквизиты,
	...
	Для Каждого ДопСвойство Из ДопРеквизиты Цикл
		...
		ДобавитьДополнительныйРеквизит(
			Реквизиты,
			ДопСвойство.name,
			Типы,
...
// Конструктор структуры, описывающей параметры выражения на встроенном языке.
//
// Возвращаемое значение:
//  Структура:
//     * ЭтоПравилоЗагрузкиРеквизита - Булево - это выражение для заполнения реквизита объекта ИС.
//     * ЭтоПравилоЗагрузкиТаблицы - Булево - это выражение для заполнения таблицы объекта ИС.
//     * ЭтоУсловиеПрименимостиПриВыгрузке - Булево - это выражение, определяющее применимость правила
//         при выгрузке объектов в ДО.
//     * ЭтоУсловиеПрименимостиПриЗагрузке - Булево - это выражение, определяющее применимость правила
//         при загрузке объектов из ДО.
//     * ТипОбъектаДО - Строка - имя типа объекта ДО.
//     * ВидДокументаДОID - Строка - идентификатор вида документа ДО.
//     * ТипОбъектаИС - Строка - имя типа объекта ИС.
//     * ИмяРеквизитаОбъектаИС - Строка - имя заполняемого реквизита объекта ИС.
//     * ДоступнаТекущаяСтрока - Булево - Истина, если в контексте выражения заполняется строка табличной части.
//     * ТаблицаИсточника - Строка - наименование табличной части, являющейся источником заполнения
//         табличной части объекта ИС.
//     * МестоВыполненияВыражения - ПеречислениеСсылка.МестаВыполненияВыраженийНаВстроенномЯзыке - указывает,
//         на стороне какой системы (ДО или ИС) будет выполняться выражение.
//
Функция ПараметрыВыраженияНаВстроенномЯзыке() Экспорт
	
	ПараметрыВыражения = Новый Структура;
	
	ПараметрыВыражения.Вставить("ЭтоПравилоЗагрузкиРеквизита", Ложь);
	ПараметрыВыражения.Вставить("ЭтоПравилоЗагрузкиТаблицы", Ложь);
	ПараметрыВыражения.Вставить("ЭтоУсловиеПрименимостиПриВыгрузке", Ложь);
	ПараметрыВыражения.Вставить("ЭтоУсловиеПрименимостиПриЗагрузке", Ложь);
	ПараметрыВыражения.Вставить("ТипОбъектаДО", "");
	ПараметрыВыражения.Вставить("ВидДокументаДОID", "");
	ПараметрыВыражения.Вставить("ТипОбъектаИС", "");
	ПараметрыВыражения.Вставить("ИмяРеквизитаОбъектаИС", "");
	ПараметрыВыражения.Вставить("ДоступнаТекущаяСтрока", Ложь);
	ПараметрыВыражения.Вставить("ТаблицаИсточника", "");
	ПараметрыВыражения.Вставить("МестоВыполненияВыражения",
		Перечисления.МестаВыполненияВыраженийНаВстроенномЯзыке.НаСторонеИС);
	
	Возврат ПараметрыВыражения;
...
Процедура ПроверитьВыражениеПравилаЗагрузки(ВычисляемоеВыражение, МестоВыполненияВыражения, ТипОбъектаДО,
	...
	ИначеЕсли МестоВыполненияВыражения = Перечисления.МестаВыполненияВыраженийНаВстроенномЯзыке.НаСторонеДО Тогда
		...
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоСуществует(Запрос, "forTabularSection") Тогда
			Запрос.forTabularSection = ЭтоТаблица;
		КонецЕсли;
...
//   ТипОбъектаИС - Строка - тип объекта интегрированной системы.
//
Процедура ПроверитьВыражениеУсловияПрименимостиПриВыгрузке(ВычисляемоеВыражение, ТипОбъектаИС) Экспорт
	...
	Результат = ИнтеграцияС1СДокументооборот3.РезультатВыраженияНаВстроенномЯзыке(ВычисляемоеВыражение, ОбъектИС);
	
	ТипРезультата = ТипЗнч(Результат);
	Если ТипРезультата <> Тип("Булево") Тогда
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Результат выражения условия применимости правила имеет тип ""%1"", а должен иметь тип ""Булево"".'"),
			ТипРезультата);
	КонецЕсли;
...
//   КонтекстВыражения - см. Справочники.ПравилаИнтеграцииС1СДокументооборотом3.КонтекстВыраженияНаВстроенномЯзыке
//
Процедура ПроверитьВыражениеУсловияПрименимостиПриЗагрузке(ВычисляемоеВыражение, ТипОбъектаДО,
		КонтекстВыражения) Экспорт
	...
	Результат = ИнтеграцияС1СДокументооборот3.РезультатВыраженияНаВстроенномЯзыке(ВычисляемоеВыражение, ОбъектXDTO);
...
Процедура ПроверитьПравило(ПравилоИнтеграции) Экспорт
	...
	Если ПравилоИнтеграции.ИспользоватьУсловиеПрименимостиПриВыгрузке Тогда
		Если СокрЛП(ПравилоИнтеграции.УсловиеПрименимостиПриВыгрузке) = "" Тогда
			СписокОшибок.Добавить(
				НСтр("ru = 'Не заполнено произвольное выражение, определяющее применимость правила при выгрузке объектов в 1С:Документооборот'"));
		Иначе
			Попытка
			...
			Исключение
				...
				СписокОшибок.Добавить(
					СтрШаблон(
						НСтр("ru = 'Ошибка в условии применимости правила для выгрузки объектов в 1С:Документооборот:
	...
	Если ПравилоИнтеграции.ИспользоватьУсловиеПрименимостиПриЗагрузке Тогда
		Если СокрЛП(ПравилоИнтеграции.УсловиеПрименимостиПриЗагрузке) = "" Тогда
			СписокОшибок.Добавить(
				НСтр("ru = 'Не заполнено произвольное выражение, определяющее применимость правила при загрузке объектов из 1С:Документооборота'"));
		Иначе
			КонтекстВыражения = КонтекстВыраженияНаВстроенномЯзыке();
			КонтекстВыражения.ВидДокументаДОID = ПравилоИнтеграции.ВидДокументаДОID;
			Попытка
			...
			Исключение
				...
				СписокОшибок.Добавить(
					СтрШаблон(
						НСтр("ru = 'Ошибка в условии применимости правила для загрузки объектов из 1С:Документооборота:
							|Не удалось выполнить выражение на встроенном языке по причине:
							|%1'"),
						ТекстИсключения));
			КонецПопытки;
	...
	ПроверитьНаличиеДублей(ПравилоИнтеграции, СписокОшибок);
...
//     * ИмяРеквизитаВДО - Строка
//     * ЭтоОсновнойРеквизит - Булево
//
Функция СтруктураТаблицыРеквизитовДО() Экспорт
	...
	Реквизиты.Колонки.Добавить("ЭтоОсновнойРеквизит", Новый ОписаниеТипов("Булево"));
...
// Добавляет HTML-код для вывода реквизита объекта в инструкцию.
//
// Параметры:
//   ИнструкцияМассив - Массив из Строка - массив для накопления HTML-кода инструкции.
//   СтруктураРеквизита - Структура - структура, содержащая информацию о реквизите:
//     * Тип - СписокЗначений из Строка
...
// Добавляет в инструкцию пример выражения на встроенном языке.
//
// Параметры:
//   ИнструкцияМассив - Массив из Строка - массив для накопления HTML-кода инструкции.
//   ЗаголовокРаздела - Строка - краткое описание примера.
//   ПримерВыражения - Строка - текст выражения на встроенном языке.
//
...
// Добавляет в инструкцию список реквизитов объекта.
//
// Параметры:
//   ИнструкцияМассив - Массив из Строка - массив для накопления HTML-кода инструкции.
//   Параметры - см. Справочники.ПравилаИнтеграцииС1СДокументооборотом3.ПараметрыВыраженияНаВстроенномЯзыке
//   СоставРеквизитов - см. Справочники.ПравилаИнтеграцииС1СДокументооборотом3.РеквизитыОбъектаДО
//                    - см. ИнтеграцияС1СДокументооборотБазоваяФункциональность.РеквизитыОбъектаИС
//   ВыводРеквизитовОбъектаИС - Булево - Истина, если выводятся реквизиты объекта ИС, Ложь, если
//     выводятся реквизиты объекта ДО.
//
Процедура ВывестиТаблицуРеквизитов(ИнструкцияМассив, Параметры, СоставРеквизитов, ВыводРеквизитовОбъектаИС = Ложь)
	...
	Для Каждого СтруктураРеквизита Из СоставРеквизитов Цикл
		...
		Если ЗначениеЗаполнено(СтруктураРеквизита.Таблица)
				Или (Параметры.ЭтоУсловиеПрименимостиПриЗагрузке И Не СтруктураРеквизита.ЭтоОсновнойРеквизит) Тогда
			Продолжить;
		КонецЕсли;
	...
	Если ВыведенЗаголовокДопРеквизитов Тогда
		Если Параметры.ЭтоУсловиеПрименимостиПриВыгрузке Тогда
		...
		ИначеЕсли Параметры.ЭтоУсловиеПрименимостиПриЗагрузке Тогда
			...
			ПримерВыражения =
				...
				|	КонецЕсли;
				|КонецЦикла;";
		ИначеЕсли ВыводРеквизитовОбъектаИС Тогда
		...
		Иначе
			ЗаголовокРаздела = НСтр("ru = 'Пример обращения к дополнительным реквизитам источника'");
			Если Параметры.МестоВыполненияВыражения = Перечисления.МестаВыполненияВыраженийНаВстроенномЯзыке.НаСторонеДО Тогда
				Если Параметры.ЭтоПравилоЗагрузкиТаблицы Тогда
					ПримерВыражения =
						"ЗначениеДопРеквизита = Неопределено;
						|Для Каждого Строка Из Параметры.Источник.ДополнительныеРеквизиты Цикл
...
Процедура ДобавитьДополнительныйРеквизит(Реквизиты, Представление, Тип, ДопРеквизитID, ДопРеквизитТип)
	...
	НоваяСтрока.Представление = СтрШаблон(НСтр("ru = 'Доп. реквизит: %1'"), Представление);
	НоваяСтрока.ЭтоОсновнойРеквизит = Истина;
...
Процедура ДобавитьРеквизит(Реквизиты, Имя, Представление, Тип, ОбязательноеЗаполнение = Ложь, ИмяРеквизитаВДО = "",
	...
	НоваяСтрока.ЭтоОсновнойРеквизит = ЭтоОсновнойРеквизит;
...
Процедура ДобавитьРеквизитТаблицы(Реквизиты, ИмяТаблицы, Имя, Представление, Тип, ОбязательноеЗаполнение = Ложь)
	...
	НоваяСтрока.Имя = Имя;
	НоваяСтрока.Представление = Представление;
	НоваяСтрока.ЭтоОсновнойРеквизит = Ложь;
...
// Добавляет открывающий или закрывающий тег сворачиваемой группы в инструкцию выражения на встроенном языке.
//
// Параметры:
//   ИнструкцияМассив - Массив из Строка - массив для накопления HTML-кода инструкции.
//   ОткрывающийТег - Булево - признак добавления открывающего тега.
//
...
Процедура ПроверитьНаличиеДублей(ПравилоИнтеграции, СписокОшибок)
	
	Если ПравилоИнтеграции.ПометкаУдаления Тогда
		Возврат;
	КонецЕсли;
	...
	Если ПравилоИнтеграции.ИспользоватьУсловиеПрименимостиПриВыгрузке Тогда
		...
		Запрос = Новый Запрос(
			...
			|	Справочник.ПравилаИнтеграцииС1СДокументооборотом3 КАК ПравилаИнтеграцииС1СДокументооборотом3
			|ГДЕ
			|	ПравилаИнтеграцииС1СДокументооборотом3.Ссылка <> &Ссылка
			|	И ПравилаИнтеграцииС1СДокументооборотом3.ТипОбъектаИС = &ТипОбъектаИС
			|	И НЕ ПравилаИнтеграцииС1СДокументооборотом3.ПометкаУдаления
			|	И ПравилаИнтеграцииС1СДокументооборотом3.ИспользоватьУсловиеПрименимостиПриВыгрузке");
		Запрос.УстановитьПараметр("Ссылка", ПравилоИнтеграции.Ссылка);
		Запрос.УстановитьПараметр("ТипОбъектаИС", ПравилоИнтеграции.ТипОбъектаИС);
		
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			ТекущееВыражение = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ВыражениеБезЛишнихСимволов(
				ПравилоИнтеграции.УсловиеПрименимостиПриВыгрузке);
			СуществующееВыражение = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ВыражениеБезЛишнихСимволов(
				Выборка.УсловиеПрименимостиПриВыгрузке);
			Если ТекущееВыражение = СуществующееВыражение Тогда
				СписокОшибок.Добавить(СтрШаблон(
					НСтр("ru = 'Правило интеграции объекта ""%1"" уже существует.
						|Для создания еще одного правила требуется изменить произвольное выражение на закладке ""Условия применимости для выгрузки объектов в 1С:ДО"".'"),
						ПравилоИнтеграции.ПредставлениеОбъектаИССКлючевымиПолями));
				Прервать;
			КонецЕсли;
		КонецЦикла;
	...
	Иначе
		...
		КлючевыеРеквизитыИСКолонки = "Имя, ЗначениеРеквизита";
		ТекущиеКлючевыеРеквизитыИС = ПравилоИнтеграции.КлючевыеРеквизитыИС.Выгрузить(, КлючевыеРеквизитыИСКолонки);
		
		Запрос = Новый Запрос(
			"ВЫБРАТЬ
			|	ПравилаИнтеграцииС1СДокументооборотом3.Ссылка КАК Ссылка,
			|	ПравилаИнтеграцииС1СДокументооборотом3.КлючевыеРеквизитыИС КАК КлючевыеРеквизитыИС
			|ИЗ
			|	Справочник.ПравилаИнтеграцииС1СДокументооборотом3 КАК ПравилаИнтеграцииС1СДокументооборотом3
			|ГДЕ
			|	ПравилаИнтеграцииС1СДокументооборотом3.Ссылка <> &Ссылка
		...
		Запрос.Текст = СтрЗаменить(
			Запрос.Текст,
			"ПравилаИнтеграцииС1СДокументооборотом3.КлючевыеРеквизитыИС",
			СтрШаблон("ПравилаИнтеграцииС1СДокументооборотом3.КлючевыеРеквизитыИС.(%1)", КлючевыеРеквизитыИСКолонки));
		
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			Если ОбщегоНазначения.КоллекцииИдентичны(ТекущиеКлючевыеРеквизитыИС, Выборка.КлючевыеРеквизитыИС.Выгрузить()) Тогда
				СписокОшибок.Добавить(СтрШаблон(
					НСтр("ru = 'Правило интеграции объекта ""%1"" уже существует.
						|Для создания еще одного правила требуется изменить ключевые реквизиты на закладке ""Условия применимости для выгрузки объектов в 1С:ДО"".'"),
					ПравилоИнтеграции.ПредставлениеОбъектаИССКлючевымиПолями));
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
Процедура ПроверитьПравилаЗаполненияРеквизитовИС(ПравилоИнтеграции, ОбъектИС, ОбъектXDTO, РеквизитыОбъектаИС,
	...
	Для Каждого Правило Из ПравилоИнтеграции.ПравилаЗаполненияРеквизитовИС Цикл
		...
		Если Правило.ЭтоТаблица Тогда
			ПредставлениеТипаРеквизитаИС = НСтр("ru = 'таблицы'");
			
		ИначеЕсли Правило.ЭтоДополнительныйРеквизитИС Тогда
			ПредставлениеТипаРеквизитаИС = НСтр("ru = 'доп. реквизита'");
			
		ИначеЕсли Не ЗначениеЗаполнено(Правило.Таблица) Тогда
			ПредставлениеТипаРеквизитаИС = НСтр("ru = 'реквизита'");
			
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Правило.Таблица) Тогда
			ПредставлениеРеквизитаИС = СтрШаблон(НСтр("ru = 'реквизита ""%1"" таблицы ""%2""'"),
