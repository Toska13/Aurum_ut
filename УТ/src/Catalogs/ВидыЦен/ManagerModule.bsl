...
Функция ПолучитьБлокируемыеРеквизитыОбъекта() Экспорт
	...
	Результат.Добавить("СтавкиНДС; ВариантУказанияСтавокНДС, Страна, СтавкаНДС");
...
//  * ВариантУказанияСтавокНДС - ПеречислениеСсылка.ВариантыУказанияСтавокНДСВПрайсЛисте - вариант указания ставки НДС в прайс-листе
...
Функция ИнициализироватьПараметрыДляСозданияВидаЦен() Экспорт
	...
	Параметры.Вставить("ВариантУказанияСтавокНДС", Перечисления.ВариантыУказанияСтавокНДСВПрайсЛисте.НеУказыватьСтавкиНДС);
...
// Параметры обработчика обновления
// 
// Параметры:
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Справочники.ВидыЦен.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "11.5.26.33";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Справочники.ВидыЦен.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик["Идентификатор"] = Новый УникальныйИдентификатор("ec2835ce-ddd2-44a6-946d-4fb6b8830d04");
	
	СписокОписаний = Новый Массив;
	СписокОписаний.Добавить(НСтр("ru = 'Обновление справочника ""Виды цен"":'"));
	
	ОписаниеРеквизитов = СтрШаблон(НСтр("ru = '- при заполнении реквизитов ""Вариант указания ставок НДС; "", ""Ставка НДС; "", ""Страна"".'"));
	СписокОписаний.Добавить(ОписаниеРеквизитов);
...
// Выборка данных для обработчика обновления
// 
// Параметры:
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	И ВидыЦен.ВариантУказанияСтавокНДС = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСтавокНДСВПрайсЛисте.ПустаяСсылка)
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний = Новый Массив;
	СписокОписаний.Добавить(НСтр("ru = 'Не удалось обработать справочник ""Виды цен"" по обработчику:'"));
	
	ОписаниеРеквизитов = СтрШаблон(НСтр("ru = '- при заполнении реквизитов %1';"), "ВариантУказанияСтавокНДС; СтавкаНДС; Страна.");
	СписокОписаний.Добавить(ОписаниеРеквизитов);
	
	ОсновнаяСтрана = УчетНДСЛокализация.ПолучитьОсновнуюСтрану();
	...
	Для Каждого СсылкаНаОбъект Из НаборСсылок Цикл
		...
		Попытка
			...
			СправочникОбъект = ОбновлениеИнформационнойБазыУТ.ПроверитьПолучитьОбъект(
				СсылкаНаОбъект.Ссылка, СсылкаНаОбъект.ВерсияДанных, Параметры.Очередь); // СправочникОбъект
			...
			Если СправочникОбъект <> Неопределено Тогда
				...
				СправочникОбъект.ВариантУказанияСтавокНДС = Перечисления.ВариантыУказанияСтавокНДСВПрайсЛисте.НеУказыватьСтавкиНДС;
				СправочникОбъект.СтавкаНДС = Справочники.СтавкиНДС.БезНДС;
				
				Если ЗначениеЗаполнено(ОсновнаяСтрана) Тогда
					СправочникОбъект.Страна = ОсновнаяСтрана;
				КонецЕсли;
		...
		Исключение
			...
			ОбновлениеИнформационнойБазыУТ.СообщитьОНеудачнойОбработке(ИнформацияОбОшибке(), СсылкаНаОбъект.Ссылка);
			
			Если ПричинаИсключения = "ПлохиеДанные" Тогда
				
				ОписаниеПроблемы = ОбновлениеИнформационнойБазыУТ.ПроблемаСДанными(
					СсылкаНаОбъект.Ссылка, Рекомендация, ИнформацияОбОшибке());
