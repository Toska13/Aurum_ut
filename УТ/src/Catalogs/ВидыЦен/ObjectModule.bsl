...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Статус = Перечисления.СтатусыДействияВидовЦен.Действует
		И Не ПраваПользователяПовтИсп.УтверждениеВидовЦен()
		И Перечисления.СпособыЗаданияЦен.ЭтоНебезопасныйСпособЗаданияЦены(СпособЗаданияЦены) Тогда
		ТекстОшибки = НСтр("ru = 'Нет прав на установку статуса ""Действует"" для данного способа задания цены.
		|Сохраните вид цены с другим статусом и обратитесь к пользователю с ролью ""Утверждение видов цен"" или администратору.'");
	...
	Если Не ЭтоГруппа Тогда
		...
		Если ИспользуетсяЦенообразование25
				И (ИспользоватьПриОптовойПродаже Или ИспользоватьПриПередачеМеждуОрганизациями) Тогда
					
			ПроверяемыеРеквизиты.Добавить("ВариантУказанияСтавокНДС");
			Если ВариантУказанияСтавокНДС = Перечисления.ВариантыУказанияСтавокНДСВПрайсЛисте.УказыватьОбщуюСтавкуНДС Тогда
				ПроверяемыеРеквизиты.Добавить("СтавкаНДС");
			КонецЕсли;
			
			Если ПолучитьФункциональнуюОпцию("ИспользоватьМногострановойУчет") Тогда
				Если Не ВариантУказанияСтавокНДС = Перечисления.ВариантыУказанияСтавокНДСВПрайсЛисте.НеУказыватьСтавкиНДС Тогда
...
Процедура ЗаполнитьЗначенияПоУмолчанию()
	...
	ВариантУказанияСтавокНДС = Перечисления.ВариантыУказанияСтавокНДСВПрайсЛисте.НеУказыватьСтавкиНДС;
	Страна = ЗначениеНастроекКлиентСерверПовтИсп.ОсновнаяСтрана();
	
	Если ПраваПользователяПовтИсп.УтверждениеВидовЦен()
		Или Не ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоВидовЦен") Тогда
		// "Действует" устанавливается только для пользователя с достаточными правами
		// или при условии отсутствия других видов цен
		Статус = Перечисления.СтатусыДействияВидовЦен.Действует;
	Иначе
		Статус = Перечисления.СтатусыДействияВидовЦен.НеДействует;
	КонецЕсли;
