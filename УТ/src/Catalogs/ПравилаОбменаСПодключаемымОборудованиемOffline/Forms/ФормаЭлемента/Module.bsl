...
Процедура ОбновитьТаблицуТоваров(КодыТоваров, ПравилоОбмена, ВидЦены, ИндивидуальныйВидЦены, АдресНаНастройки)
	...
	Коэффициент1 = Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки(
						"ИсходныеДанныеПоследнийЗапрос.Упаковка",
						"ИсходныеДанныеПоследнийЗапрос.Номенклатура");
	Коэффициент2 = Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки(
						"ИсходныеДанныеПоследнийЗапрос.Номенклатура.ЕдиницаИзмерения",
						"ИсходныеДанныеПоследнийЗапрос.Номенклатура");
	ТекстПодстановки = СтрШаблон("ЕСТЬNULL(%1, 1)/ЕСТЬNULL(%2, 1)", Коэффициент1, Коэффициент2);
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ШаблонПодстановкиВычисляемогоКоэффициента", ТекстПодстановки);
