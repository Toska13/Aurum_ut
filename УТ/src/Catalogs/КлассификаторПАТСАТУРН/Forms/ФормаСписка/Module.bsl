...
Процедура ОбработатьПереданныеПараметры(Отказ)
	...
	// на текущий момент создание тукосмесей и комбипаков через АПИ не поддерживается
	Элементы.Создать.Видимость = Не РежимВыбора И ПараметрыПоиска.Количество() > 0 И ПравоДобавления И Ложь;
...
Процедура ОбработатьПараметрыПоискаВКлассификаторе(Результат, ДополнительныеПараметры) Экспорт
	...
	// на текущий момент создание тукосмесей и комбипаков через АПИ не поддерживается
	Элементы.Создать.Видимость = ПараметрыПоиска.Количество() > 0 И Ложь;
