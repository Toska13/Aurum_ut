...
Процедура ПриЗаписи(Отказ)
	...
	Если Закрыта
		И ЗначениеЗаполнено(ВидСделки)
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидСделки, "ТипСделки") = Перечисления.ТипыСделокСКлиентами.СделкиСРучнымПереходомПоЭтапам Тогда
...
Процедура ЗакрытьЭтапыСделки()
	
	Запрос = Новый Запрос;
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	ВидыСделокСКлиентамиЭтапыСделкиПоПродаже.ЭтапПроцессаПродажи КАК ЭтапПроцесса,
	|	СтатистикаСделокСКлиентами.ДатаНачала                        КАК ДатаНачала,
	|	СтатистикаСделокСКлиентами.Сделка                            КАК Сделка,
	|	СтатистикаСделокСКлиентами.ДатаОкончания                     КАК ДатаОкончания,
	|	СтатистикаСделокСКлиентами.Продолжительность                 КАК Продолжительность,
	|	СтатистикаСделокСКлиентами.Потенциал                         КАК Потенциал,
	...
	|		ПО ВидыСделокСКлиентамиЭтапыСделкиПоПродаже.ЭтапПроцессаПродажи = СтатистикаСделокСКлиентами.ЭтапПроцесса
	|			И (СтатистикаСделокСКлиентами.Сделка = &Сделка)
	|ГДЕ
	...
	ДатаОкончанияПредыдущегоЭтапа = Дата(1,1,1);
	ТекущаяДатаСеанса             = ТекущаяДатаСеанса();
	ЕстьИзменения                 = Ложь;
	
	Для Каждого СтрокаСтатистики Из ДанныеСтатистики Цикл
		
		Если ЗначениеЗаполнено(СтрокаСтатистики.ДатаНачала) 
			И Не ЗначениеЗаполнено(СтрокаСтатистики.ДатаОкончания) Тогда
			
			СтрокаСтатистики.ДатаОкончания = ТекущаяДатаСеанса;
			СтрокаСтатистики.ДатаОкончания = ТекущаяДатаСеанса;
			
			Если ЗначениеЗаполнено(ДатаОкончанияПредыдущегоЭтапа) Тогда
				СтрокаСтатистики.Продолжительность = Цел((СтрокаСтатистики.ДатаОкончания - ДатаОкончанияПредыдущегоЭтапа) / 86400);
			КонецЕсли;
			
			СтрокаСтатистики.Результат      = Статус;
			
			ЕстьИзменения = Истина;
			
		ИначеЕсли Не ЗначениеЗаполнено(СтрокаСтатистики.ДатаНачала)
			И Не ЗначениеЗаполнено(СтрокаСтатистики.ДатаОкончания)
			И Статус = Перечисления.СтатусыСделок.Выиграна Тогда
			
			СтрокаСтатистики.ДатаОкончания = ТекущаяДатаСеанса;
			СтрокаСтатистики.ДатаНачала    = ТекущаяДатаСеанса;
			СтрокаСтатистики.Потенциал     = ПотенциальнаяСуммаПродажи;
			СтрокаСтатистики.Результат     = Перечисления.СтатусыСделок.Выиграна;
			СтрокаСтатистики.Сделка        = Ссылка;
			
			Если ДатаОкончанияПредыдущегоЭтапа <> ТекущаяДатаСеанса
				И ЗначениеЗаполнено(ДатаОкончанияПредыдущегоЭтапа) Тогда
				СтрокаСтатистики.Продолжительность = Цел((СтрокаСтатистики.ДатаОкончания - ДатаОкончанияПредыдущегоЭтапа) / 86400);
			КонецЕсли;
		...
		ДатаОкончанияПредыдущегоЭтапа = СтрокаСтатистики.ДатаОкончания;
		
	КонецЦикла;
	
	Если ЕстьИзменения Тогда
		
		НаборЗаписей = РегистрыСведений.СтатистикаСделокСКлиентами.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Сделка.Установить(Ссылка);
		
		Для Каждого СтрокаСтатистики Из ДанныеСтатистики Цикл
			
			Если ЗначениеЗаполнено(СтрокаСтатистики.Сделка) Тогда
				
				ЗаписьНабора = НаборЗаписей.Добавить();
				ЗаполнитьЗначенияСвойств(ЗаписьНабора, СтрокаСтатистики);
