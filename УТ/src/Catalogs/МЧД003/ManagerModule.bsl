...
Функция ЗапросДляПодписания(ИмяВременнойТаблицы, Доверенности) Экспорт
	...
	ОписаниеЗапроса.СлужебныеПараметры.Вставить("ТаблицаДанныхМЧД", ТаблицаДанныхМЧДДляПодписания(Доверенности));
...
//  Доверенность - СправочникСсылка.МЧД003
//  Значение - Булево
Процедура УстановитьПометкуВерна(Доверенность, Значение) Экспорт
	
	ДоверенностьОбъект = Доверенность.ПолучитьОбъект();
	ДоверенностьОбъект.Верна = Значение;
	Если Значение Тогда
		ДоверенностьОбъект.ТекстОшибкиПроверкиМЧД = СтрШаблон(НСтр("ru='Проверена вручную: %1, %2'"), 
			Пользователи.ТекущийПользователь(), ТекущаяДатаСеанса());
	КонецЕсли;
...
//  Значение - Булево
//  ДатаОтзыва - Дата
Процедура УстановитьПометкуОтозвана(Доверенность, Значение, ДатаОтзыва) Экспорт
	
	ДоверенностьОбъект = Доверенность.ПолучитьОбъект();
	ДоверенностьОбъект.ДатаПрекращения = ?(Значение, ДатаОтзыва, '00010101');
	ДоверенностьОбъект.Записать();
...
Функция ДанныеДляЗаполненияМЧД(ОбъектМЧД) Экспорт
	...
	СвПередовXDTO = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ОбъектМЧД, "Документ.Передов.СвПередов", Неопределено);
	Если СвПередовXDTO <> Неопределено Тогда
		
		ДанныеДляЗаполнения.НомерРодительскойДоверенности = СвПередовXDTO.НомДоверN;
		
		СвПередПолнXDTO = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ОбъектМЧД, "Документ.Передов.СвПередПолн", , , Истина);
		ДоверительXDTO = СвПередПолнXDTO[0];
		Участник = Неопределено;
		ТипУчастника = ДоверительXDTO.ТипПерПолн;
		ТипыДоверителей = ТипыПредставителей();
		
		Если ТипУчастника = ТипыДоверителей.ЮридическоеЛицо Тогда
			РосОргПерПолнXDTO = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДоверительXDTO, "ПередПолн.РосОргПерПолн");
			Участник = ПолучитьУчастника_ЮридическоеЛицо(РосОргПерПолнXDTO);
		...
		ИначеЕсли ТипУчастника = ТипыДоверителей.ФизическоеЛицо Тогда	
			ФЛПерПолнXDTO = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДоверительXDTO, "ПередПолн.ФЛПерПолн");
			Участник = ПолучитьУчастника_ФизическоеЛицо(ФЛПерПолнXDTO);
		ИначеЕсли ТипУчастника = ТипыДоверителей.ФилиалЮридическогоЛица Тогда	
			ФилПерПолнXDTO = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДоверительXDTO, "ПередПолн.ФилПерПолн");
			Участник = ПолучитьУчастника_ИностраннаяОрганизация(ФилПерПолнXDTO); 
		ИначеЕсли ТипУчастника = ТипыДоверителей.ФилиалИностраннойОрганизации Тогда	
			ИнПерПолнXDTO = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДоверительXDTO, "ПередПолн.ИнПерПолн");
			Участник = ПолучитьУчастника_ИностраннаяОрганизация(ИнПерПолнXDTO);
		КонецЕсли;	
	...
	Иначе
		...
		СвПервДоверXDTO = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ОбъектМЧД, "Документ.Передов.СвПервДовер",
			Неопределено);
		Если СвПервДоверXDTO <> Неопределено Тогда

			ДанныеДляЗаполнения.НомерРодительскойДоверенности = СвПервДоверXDTO.НомДоверПерв;

			СвДоверПервXDTO = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ОбъектМЧД,
				"Документ.Передов.СвПервДовер.СвДоверПерв", , , Истина);
...
Функция СформироватьОбъектXDTOДоверенности(ДанныеМЧД) Экспорт
	...
	ПространствоИмен = ПространствоИменЕдинойМЧД();
	...
	СброситьВзаимоисключающиеИНеподдерживаемыеСвойстваXDTOМЧД(Объект);
...
Функция СформироватьОбъектXDTOПередоверия(ДанныеМЧД) Экспорт
	...
	ЗаполнитьОбъектМЧДРекурсивно(Объект, ДанныеМЧД, "Доверенность", ОшибкиЗаполнения);
	СброситьВзаимоисключающиеИНеподдерживаемыеСвойстваXDTOМЧДПередоверия(Объект);
...
// Проверяет заполнение объекта XDTO по данным формы ДобавлениеПредставителя.
// 
// Параметры:
//  Представитель - Структура
// 
// Возвращаемое значение: см. НовыйНаборОшибокЗаполненияОбъектаМЧД
//  
Функция ПроверитьЗаполнениеОбъектXDTOПредставителя(Представитель) Экспорт
	
	ОшибкиЗаполнения = НовыйНаборОшибокЗаполненияОбъектаМЧД();
	
	ПространствоИмен = ПространствоИменЕдинойМЧД();
	Тип = ФабрикаXDTO.Тип(ПространствоИмен, "СвУпПредТип"); // АПК:1036 не проверять строку на орфографию.
	Объект = ФабрикаXDTO.Создать(Тип); // см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.Доверенность.Документ.Довер.СвУпПред
	
	ЗаполнитьОбъектМЧДРекурсивно(Объект, Представитель, "СвУпПред", ОшибкиЗаполнения); // АПК:1036 не проверять строку на орфографию.
	
	СброситьНеподдерживаемыеСвойства(Объект);
	СброситьВзаимоисключающиеСвойстваПредставителя(Объект);
	
	ПроверитьОбъектМЧДРекурсивно(Объект, "СвУпПред", ОшибкиЗаполнения); // АПК:1036 не проверять строку на орфографию.
	ПроверитьУсловныеПоляПредставителяМЧД(Объект, ОшибкиЗаполнения, "СвУпПред"); // АПК:1036 не проверять строку на орфографию.
...
Функция СведенияМЧД(МЧД) Экспорт
	...
	Запрос.Текст =
		...
		|	МЧД003.Подписана КАК Подписана,
		...
		|	МЧД003.ПротоколПроверки КАК ПротоколПроверкиМЧД,
		|	МЧД003.ИмяФайлаЗаявленияНаОтзыв КАК ИмяФайлаЗаявленияНаОтзыв,
		|	МЧД003.ФайлЗаявленияНаОтзыв КАК ФайлЗаявленияНаОтзыв,
		...
		|ПОМЕСТИТЬ ПодписиЗаявлениеНаОтзыв
		|ИЗ
		|	Справочник.МЧД003.ПодписиЗаявленияНаОтзыв КАК МЧД003ПодписиЗаявленияНаОтзыв
		|ГДЕ
		|	МЧД003ПодписиЗаявленияНаОтзыв.Ссылка = &МЧД
		|
		|УПОРЯДОЧИТЬ ПО
		|	МЧД003ПодписиЗаявленияНаОтзыв.НомерСтроки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	Реквизиты = "НомерДоверенности, ДатаПолученияСведений, ДатаВыдачи, ДатаОкончания, 
				|ДатаОтзыва, Верна, Подписана, ДатаОтправкиЗаявленияНаОтзыв, СтатусВРеестреФНС, ПротоколПроверкиМЧД,
				|Отозвана, НомерРодительскойДоверенности, ИННДоверителяРодительскойДоверенности, 
				|ИмяФайлаЗаявленияНаОтзыв, ПометкаУдаления";
	...
	Если ЗначениеЗаполнено(Результат.ИННДоверителяРодительскойДоверенности) Тогда
		Результат.ИННПредставителейРодительскойДоверенности.Добавить(Результат.ИННДоверителя);
	КонецЕсли;
	
	Результат.ДанныеФайла = Выборка.ФайлМЧД.Получить();
	Результат.ДанныеФайлаЗаявленияНаОтзыв = Выборка.ФайлЗаявленияНаОтзыв.Получить();
	...
	ДанныеПодписи = Запрос.МенеджерВременныхТаблиц.Таблицы.Получить("Подписи").ПолучитьДанные();
	ВыборкаДанныеПодпись = ДанныеПодписи.Выбрать();
	Если ВыборкаДанныеПодпись.Следующий() Тогда 
		Результат.ДанныеПодписи = ВыборкаДанныеПодпись.Подпись.Получить();
	КонецЕсли;
	
	ДанныеПодписьЗаявлениеНаОтзыв = Запрос.МенеджерВременныхТаблиц.Таблицы.Получить("ПодписиЗаявлениеНаОтзыв").ПолучитьДанные();
	ВыборкаДанныеПодписьЗаявлениеНаОтзыв = ДанныеПодписьЗаявлениеНаОтзыв.Выбрать();
	Если ВыборкаДанныеПодписьЗаявлениеНаОтзыв.Следующий() Тогда 
		Результат.ДанныеПодписиЗаявленияНаОтзыв = ВыборкаДанныеПодписьЗаявлениеНаОтзыв.Подпись.Получить();
	КонецЕсли;
...
//  * ВариантПроверкиСертификатовМЧДЭДО - ПеречислениеСсылка.ВариантыПроверкиСертификатовМЧДЭДО
...
Функция ЗагрузитьМЧДИзФайла(ДанныеФайла, МЧД = Неопределено, ДополнительныеСведения = Неопределено) Экспорт
	...
	Результат.Вставить("ВариантПроверкиСертификатовМЧДЭДО", МашиночитаемыеДоверенности.ВариантПроверкиСертификатаДоверителя());
	...
	ДанныеДляЗагрузки.ДанныеЗаявленияНаОтмену = ДанныеАрхива.ДанныеЗаявленияНаОтмену;
	...
	РезультатПроверкиНаСервере = МашиночитаемыеДоверенности.НовыйОбщийРезультатПроверкиПодписиДоверенности(Истина);
		
	ЭтоРучноеСоздание = Ложь;
	ОбновлятьСуществующуюМЧД = Истина;
	
	РезультатЗагрузки = ЗагрузитьЭлементИзФайлаОбмена(ДанныеДляЗагрузки, ТребуетсяПроверкаМЧДНаКлиенте, 
		ЭтоРучноеСоздание, ДополнительныеСведения, ОбновлятьСуществующуюМЧД, МЧД, РезультатПроверкиНаСервере);
...
//                                             и необходимо выполнить проверку на клиенте.
//
Функция ЗагрузитьЭлементИзФайлаОбмена(ДанныеДляЗагрузки, ТребуетсяПроверкаМЧДНаКлиенте, ЭтоРучноеСоздание, 
	ДополнительныеСведения, ОбновлятьСуществующий = Ложь, МЧД = Неопределено, РезультатПроверкиНаСервере = Неопределено)
...
Функция ЗаписатьЭлементСправочника(ДанныеДоверенности, ОбновлятьСуществующий, ДанныеДляЗагрузки,
	...
	ЕстьПодписи = МашиночитаемыеДоверенности.ПодписиДоверенностиЗаполнены(НашлиЭлемент);
	
	Если ЕстьПодписи И ЭтоНайденнаяНеРеестроваяМЧД И Не ЗагружаемРеестровуюМЧД Тогда
		ХранилищеДанныхДоверенности = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НашлиЭлемент, "ФайлМЧД");
	...
	ИзменилсяНомерРодительскойДоверенности = ЗначениеЗаполнено(ДанныеДоверенности.НомерРодительскойДоверенности)
		И ОбъектСправочника.НомерРодительскойДоверенности <> ДанныеДоверенности.НомерРодительскойДоверенности;
	Если ИзменилсяНомерРодительскойДоверенности Тогда 
		ОбъектСправочника.НомерРодительскойДоверенности = ДанныеДоверенности.НомерРодительскойДоверенности		
	КонецЕсли;		
	
	ИзменилсяИННДоверителяРодительскойДоверенности = ЗначениеЗаполнено(ДанныеДоверенности.ИННДоверителяРодительскойДоверенности)
		И ОбъектСправочника.ИННДоверителяРодительскойДоверенности <> ДанныеДоверенности.ИННДоверителяРодительскойДоверенности;
	Если ИзменилсяИННДоверителяРодительскойДоверенности Тогда 
		ОбъектСправочника.ИННДоверителяРодительскойДоверенности = ДанныеДоверенности.ИННДоверителяРодительскойДоверенности;		
	КонецЕсли;
	
	ИзменилсяСтатусВРеестре = ЗагружаемРеестровуюМЧД
		И ДанныеДоверенности.СтатусВРеестреФНС <> ОбъектСправочника.СтатусВРеестреФНС;
	
	Если ИзменилсяСтатусВРеестре 
	...
	Если Не ЗагружаемРеестровуюМЧД
		И ЗначениеЗаполнено(ДанныеДляЗагрузки.ДанныеПодписиЗаявленияНаОтмену) 
		И ЗначениеЗаполнено(ДанныеДоверенности.ДатаОтзыва) Тогда 
		ОбъектСправочника.ДатаПрекращения = ДанныеДоверенности.ДатаОтзыва;
		ОбъектСправочника.СтатусВРеестреФНС = Перечисления.СтатусыМашиночитаемойДоверенностиВРеестреФНС.Отозвано;
	КонецЕсли;
	...
	ПроверенаВручную = МашиночитаемыеДоверенности.ПроверенаВручную(ОбъектСправочника);
...
//
Функция ПолучитьДвоичныеДанныеЗаявленияНаОтмену(Ссылка) Экспорт
	
	ФайлМЧД = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "ФайлЗаявленияНаОтзыв");
	Возврат ФайлМЧД.Получить();
...
// Возвращаемое значение:
//  ДвоичныеДанные
Функция СформироватьЗаявлениеНаОтмену(Ссылка) Экспорт
...
// * Код          - Строка
// * Наименование - Строка
// * ОграниченияПолномочия - Массив из Структура: см.НовыйДанныеОграниченияПолномочия()
Функция НовыеДанныеМашиночитаемогоПолномочия()
	...
	Результат.Вставить("ОграниченияПолномочия", Новый Массив);

	Возврат Результат;
	
КонецФункции

Функция НовыйДанныеОграниченияПолномочия()
	
	Результат = Новый Структура();
	
	Результат.Вставить("ИдОгр", 0);
	Результат.Вставить("КодЗначОгр", "");
	Результат.Вставить("КодОгр", "");
	Результат.Вставить("НаимЗначОгр", "");
	Результат.Вставить("НаимОгр", "");
	Результат.Вставить("ТексЗначОгр","");
...
Процедура ЗаполнитьДанныеПолномочий(ДанныеДляЗаполнения, ОбъектМЧД)
	...
	Иначе
		...
		Для Каждого Полномочие Из МашиночитаемыеПолномочия Цикл
			...
			СведенияОбОграниченияхПолномочия = Полномочие.ОгрСвПолн;
			Если ТипЗнч(СведенияОбОграниченияхПолномочия) <> Тип("СписокXDTO") Тогда
				СведенияОбОграниченияхПолномочия = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СведенияОбОграниченияхПолномочия);
			КонецЕсли;
			
			ОграниченияПолномочия = Новый Массив; // массив из Структура
			Для Каждого Ограничение Из СведенияОбОграниченияхПолномочия Цикл
				
				ДанныеОграничения = НовыйДанныеОграниченияПолномочия();
				ЗаполнитьЗначенияСвойств(ДанныеОграничения, Ограничение);
				ОграниченияПолномочия.Добавить(ДанныеОграничения);
				
			КонецЦикла;
			ДанныеПолномочия.ОграниченияПолномочия = ОграниченияПолномочия;
...
// 
// Возвращаемое значение:
//  См. МашиночитаемыеДоверенностиКлиентСервер.ДанныеДляПроверкиВРеестреФНС
Функция ДанныеДляПроверкиВРеестреФНС(Доверенность) Экспорт
	...
	Запрос.Текст =
		...
		|	МЧД003.НомерДоверенности КАК НомерДоверенности,
		|	МЧД003.СтатусВРеестреФНС КАК СтатусВРеестреФНС,
		|	МЧД003.ИдентификаторТранзакции КАК ИдентификаторТранзакции,
	...
	Если Выборка.Следующий() Тогда
		ДанныеДоверенности.НомерДоверенности = Выборка.НомерДоверенности;
		ДанныеДоверенности.СтатусВРеестреФНС = Выборка.СтатусВРеестреФНС;
		ДанныеДоверенности.ИдентификаторТранзакции = Выборка.ИдентификаторТранзакции;
		ДанныеДоверенности.ДатаОтзыва = Выборка.ДатаПрекращения;
	КонецЕсли;
...
// Формирует цепочку передоверия по имеющимся доверенностям в базе
//
// Параметры:
//  НомерРодительскойДоверенности - Строка
//  НомерДоверенности - Строка
...
// Возвращаемое значение:
//  Строка
Функция ПространствоИменЕдинойМЧД()
...
Функция ПолучитьДоверенностиПоОтбору(Доверитель, Представитель, ДействительныеНаДату) Экспорт
	...
	Если Не ПустаяСтрока(ДоверительИНН) Тогда
		...
		ТекстыУсловий.Добавить("ТаблицаДоверители.ТипДоверителя В (&ТипыДоверителей)");
	...
	Если Не ПустаяСтрока(ПредставительИНН) Тогда
		...
		ТекстыУсловий.Добавить("ТаблицаПредставители.ТипПредставителя В (&ТипыПредставителей)");
	КонецЕсли;
...
//
Функция ПодписиДоверенности(МЧД)
...
Функция ДанныеДоверенностиДляКонтейнера(Доверенность) Экспорт
	...
	Реквизиты = "ФайлМЧД, ДатаВыдачи, СрокДействия, НомерДоверенности, Доверители, Представители,  СтатусВРеестреФНС"; // АПК:1297 - не локализуется
	...
	ВыборкаПредставители = СвойстваДоверенности.Представители.Выбрать();
	Пока ВыборкаПредставители.Следующий() Цикл
		ДанныеДоверенности.ПредставительИНН = ВыборкаПредставители.ИНН;
		Прервать;
	КонецЦикла;
...
Функция ТабличныйДокументМЧД(ДоверенностьXDTO) Экспорт
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Справочник.МЧД003.КарточкаМЧД", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВывестиПолномочия(ТабличныйДокумент, Макет, Полномочия)
	...
	Если Полномочия.ТипПолн = ТипыПолномочийXDTO.Текстовые Тогда
		...
		ИсходныйТекст = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Полномочия, "ТекстПолн", "");
		ПроизвольныйТекст = Макет.ПолучитьОбласть("ПроизвольныйТекст");
		МашиночитаемыеДоверенности.ВывестиТекстовыеПолномочия(ИсходныйТекст, ТабличныйДокумент, ПроизвольныйТекст, 
			"Текст");
	...
	ИначеЕсли Полномочия.ТипПолн = ТипыПолномочийXDTO.Машиночитаемые Тогда	
		...
		Если МашПолнXDTO <> Неопределено Тогда
			...
			Для Каждого ПолномочиеXDTO Из МашПолнXDTO Цикл
				...
				ЕстьОграничения = ОгрСвПолнXDTO <> Неопределено;
				
				ПроизвольныеСведения = Макет.ПолучитьОбласть("ТекстОграничений");
				
				Если ЕстьОграничения Тогда
					
					Сведения = Макет.ПолучитьОбласть("ПроизвольныйТекст");
					Если ОгрСвПолнXDTO.Количество() > 0 Тогда
						ТекстЕстьОграничения = НСтр("ru='с ограничениями:'");
						Сведения.Параметры.Текст =
							СтрШаблон("%1%2, %3", ПрефиксСтроки, ПолномочиеXDTO.НаимПолн, ТекстЕстьОграничения);
					Иначе
						ТекстЕстьОграничения = НСтр("ru='ограничения не указаны.'");
						Сведения.Параметры.Текст =
							СтрШаблон("%1%2, %3", ПрефиксСтроки, ПолномочиеXDTO.НаимПолн, ТекстЕстьОграничения);
					КонецЕсли;
					ТабличныйДокумент.Вывести(Сведения);
					
					Для Каждого ОграничениеXDTO Из ОгрСвПолнXDTO Цикл	
						
						НаименованиеОграничения = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ОграничениеXDTO, "НаимОгр", "");
						
						ЗначениеОграничения = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ОграничениеXDTO, "НаимЗначОгр", "");
						ТекстЗначенияОграничения = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ОграничениеXDTO, "ТексЗначОгр", ""); 
						
						Если ЗначениеЗаполнено(ТекстЗначенияОграничения) Тогда 
							ЗначениеОграничения = ТекстЗначенияОграничения; 
						КонецЕсли;
						
						ПроизвольныеСведения.Параметры.Маркер = Символ(8226);
						ПроизвольныеСведения.Параметры.НаименованиеОграничения = НаименованиеОграничения;
						ПроизвольныеСведения.Параметры.ЗначениеОграничения = ЗначениеОграничения;
...
Процедура ЗаполнитьОбъектМЧДРекурсивно(Объект, ДанныеОбъекта, ПутьДанныхОбъекта, ОшибкиЗаполнения)
	...
	Для Каждого СвойствоОбъекта Из Объект.Свойства() Цикл
		...
		ИначеЕсли Не ЭтоСписок Тогда
			Если СвойствоОбъекта.Имя = "АдрМЖ" И Не ЗначениеЗаполнено(ЗначениеСвойства[0]) Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ЗаполнитьСвойствоОбъектаМЧДПростогоТипа(Объект, СвойствоОбъекта, Значение, Путь, ОшибкиЗаполнения)
	...
	Если СвойствоОбъекта.Имя = "Фамилия"
		Или СвойствоОбъекта.Имя = "Имя" 
		Или СвойствоОбъекта.Имя = "Отчество" Тогда
...
#Область СбросВзаимоисключающихИНеподдерживаемыхСвойств

// Сбросить взаимоисключающие и неподдерживаемые свойства XDTOМЧД.
// 
// Параметры:
//  Доверенность - см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.Доверенность
//
Процедура СброситьВзаимоисключающиеИНеподдерживаемыеСвойстваXDTOМЧДПередоверия(Доверенность)
	
	СброситьНеподдерживаемыеСвойстваПередоверия(Доверенность);
	
	СброситьВзаимоисключающиеСвойстваПередоверияПоПервоначальнойДоверенности(Доверенность);
...
Процедура СброситьВзаимоисключающиеСвойстваДоверителяРоссийскойОрганизацииПередоверия(Доверитель)
	...
	КлассификаторБулево = КлассификаторДаНет();
	
	РосОргДовер = Доверитель.ПередПолн.РосОргПерПолн;
	ЕстьУК = (РосОргДовер.ЕИОУК = КлассификаторБулево.Да);
	ЕстьИП = (РосОргДовер.ЕИОИП = КлассификаторБулево.Да);
	ЕстьФЛ = (РосОргДовер.ЕИОФЛ = КлассификаторБулево.Да);
...
Процедура СброситьВзаимоисключающиеСвойстваПередоверияПоПервоначальнойДоверенности(Доверенность)
...
//  Доверенность - см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.Доверенность
//
Процедура СброситьВзаимоисключающиеИНеподдерживаемыеСвойстваXDTOМЧД(Доверенность)
...
Процедура ПроверитьУсловныеПоляОбъектаМЧД(Доверенность, ОшибкиЗаполнения)
	...
	ПроверитьУсловныеПоляДоверителейМЧД(Доверенность, ОшибкиЗаполнения);
	ПроверитьУсловныеПоляПредставителейМЧД(Доверенность, ОшибкиЗаполнения);
	ПроверитьУсловныеПоляПолномочийМЧД(Доверенность, ОшибкиЗаполнения);
	
КонецПроцедуры

// Проверяет условные поля доверителей в МЧД.
// 
// Параметры:
//  Доверенность см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.Доверенность
//  ОшибкиЗаполнения см. НовыйНаборОшибокЗаполненияОбъектаМЧД
//
Процедура ПроверитьУсловныеПоляДоверителейМЧД(Знач Доверенность, ОшибкиЗаполнения)
...
// Проверяет условные поля представителей в МЧД.
// 
// Параметры:
//  Доверенность см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.Доверенность
//  ОшибкиЗаполнения см. НовыйНаборОшибокЗаполненияОбъектаМЧД
// 
Процедура ПроверитьУсловныеПоляПредставителейМЧД(Знач Доверенность, ОшибкиЗаполнения)
	...
	ПроверитьСовпадениеПредставителяИДоверителяМЧД(Доверенность, ОшибкиЗаполнения);
	
КонецПроцедуры

// Проверяет совпадение полей ИНН и СНИЛС представителя и доверителя в МЧД.
// 
// Параметры:
//  Доверенность - см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.Доверенность
//  ОшибкиЗаполнения - см. НовыйНаборОшибокЗаполненияОбъектаМЧД
// 
Процедура ПроверитьСовпадениеПредставителяИДоверителяМЧД(Знач Доверенность, ОшибкиЗаполнения)

	Доверители = Доверенность.Документ.Довер.СвДоверит; // см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.СвДоверитТип
	Представители = Доверенность.Документ.Довер.СвУпПред; // см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.СвУпПредТип
		
	ИННФизЛицаДоверителяДействующегоБезДоверенности = "";
	ИННФизЛицаПредставителя = "";
	
	Для Каждого Доверитель Из Доверители Цикл 
		
		ТипДоверителя = Доверитель.ТипДоверит; // Строка 
		
		Если ТипДоверителя = ТипыДоверителей().ЮридическоеЛицо Тогда
			
			РосОргДовер = Доверитель.Доверит.РосОргДовер; // см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.РосОргДоверТип
			Если РосОргДовер.ЕИОФЛ = КлассификаторДаНет().Да Тогда
				ИННФизЛицаДоверителяДействующегоБезДоверенности = РосОргДовер.ЛицоБезДов[0].СвФЛ.ИННФЛ; // Строка
			ИначеЕсли РосОргДовер.ЕИОИП = КлассификаторДаНет().Да Тогда
				ИННФизЛицаДоверителяДействующегоБезДоверенности = РосОргДовер.ЛицоБезДов[0].СВИП.ИННФЛ; // Строка
			ИначеЕсли РосОргДовер.ЕИОУК = КлассификаторДаНет().Да Тогда 
				ИННФизЛицаДоверителяДействующегоБезДоверенности = РосОргДовер.ЛицоБезДов[0].СВЮЛ.СвФЛ[0].ИННФЛ; // Строка		
			КонецЕсли;		
				
		ИначеЕсли ТипДоверителя = ТипыДоверителей().ИндивидуальныйПредприниматель Тогда
			ИННФизЛицаДоверителяДействующегоБезДоверенности = Доверитель.Доверит.ИПДовер.ИННФЛ; // Строка
		КонецЕсли;	
		
		Если ПустаяСтрока(ИННФизЛицаДоверителяДействующегоБезДоверенности) Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Каждого Представитель Из Представители Цикл
			...
			ТипПредставителя = Представитель.ТипПред; // Строка
			
			Если ТипПредставителя = ТипыПредставителей().ФизическоеЛицо Тогда
				ИННФизЛицаПредставителя = Представитель.Пред.СведФизЛ.ИННФЛ; // Строка
				Путь = СтрШаблон("Доверенность[0].Документ[0].Довер[0].СвУпПред[%1].Пред[0].СведФизЛ[0].ИННФЛ",
					ИндексПредставителя);
			ИначеЕсли ТипПредставителя = ТипыПредставителей().ИндивидуальныйПредприниматель Тогда 			
				ИННФизЛицаПредставителя = Представитель.Пред.СведИП.ИННФЛ; // Строка
				Путь = СтрШаблон("Доверенность[0].Документ[0].Довер[0].СвУпПред[%1].Пред[0].СведИП[0].ИННФЛ",
					ИндексПредставителя);
			Иначе
				ИндексПредставителя = ИндексПредставителя + 1;
				Продолжить;	
			...
			ДобавитьОшибкуСовпаденияИННПредставителяИДоверителяМЧД(ОшибкиЗаполнения, Путь, ИННФизЛицаПредставителя, 
				ИННФизЛицаДоверителяДействующегоБезДоверенности, КоличествоПредставителей, НомерСтроки);	
...
// Добавить ошибку совпадения ИННПредставителя и доверителя МЧД.
// 
// Параметры:
//  ОшибкиЗаполнения - см. НовыйНаборОшибокЗаполненияОбъектаМЧД
//  Путь - Строка
...
//  ИННДоверителя - Строка
//  КоличествоПредставителей - Число
//  НомерСтроки - Число
Процедура ДобавитьОшибкуСовпаденияИННПредставителяИДоверителяМЧД(ОшибкиЗаполнения, Знач Путь, Знач ИННПредставителя, 
	Знач ИННДоверителя, Знач КоличествоПредставителей, Знач НомерСтроки)
	
	Если Не ЗначениеЗаполнено(ИННПредставителя)
		Или ИННПредставителя <> ИННДоверителя Тогда
		Возврат;	
	КонецЕсли;		
			
	ТекстПояснения = НСтр("ru = 'пункт 3 статьи 182 ГК РФ ""Представитель не может совершать сделки от имени
		|представляемого (доверителя) в отношении себя лично, а также в отношении другого лица, представителем которого он одновременно является"".'");	
	
	Если КоличествоПредставителей = 1 Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Недопустимый ИНН представителя - физического лица. Основание: %1'"),
			ТекстПояснения);		
	Иначе
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Недопустимый ИНН представителя - физического лица в строке %1. Основание: %2'"),
			НомерСтроки, ТекстПояснения);	
	КонецЕсли;
...
// Проверяет условные поля объекта МЧД.
// 
// Параметры:
//  Доверенность - см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.Доверенность
//  ОшибкиЗаполнения - см. НовыйНаборОшибокЗаполненияОбъектаМЧД
...
Процедура ПроверитьУсловныеПоляОбъектаМЧДПередоверия(Доверенность, ОшибкиЗаполнения)
	...
	ПроверитьУсловныеПоляДоверителейМЧДПередоверия(Доверенность, ОшибкиЗаполнения);
	ПроверитьУсловныеПоляПредставителейМЧДПередоверия(Доверенность, ОшибкиЗаполнения);
	ПроверитьУсловныеПоляПолномочийМЧДПередоверия(Доверенность, ОшибкиЗаполнения);
	
КонецПроцедуры

// Проверяет условные поля доверителей МЧД передоверия.
// 
// Параметры:
//  Доверенность - см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.Доверенность
//  ОшибкиЗаполнения - см. НовыйНаборОшибокЗаполненияОбъектаМЧД
//
Процедура ПроверитьУсловныеПоляДоверителейМЧДПередоверия(Знач Доверенность, ОшибкиЗаполнения)
...
// Проверяет условные поля представителей МЧД передоверия.
// 
// Параметры:
//  Доверенность - см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.Доверенность
//  ОшибкиЗаполнения - см. НовыйНаборОшибокЗаполненияОбъектаМЧД
//
Процедура ПроверитьУсловныеПоляПредставителейМЧДПередоверия(Знач Доверенность, ОшибкиЗаполнения)
	...
	ПроверитьСовпадениеПредставителяИДоверителяМЧДПередоверия(Доверенность, ОшибкиЗаполнения)
	
КонецПроцедуры

// Проверяет совпадение полей ИНН и СНИЛС представителя и доверителя в МЧД передоверия.
// 
// Параметры:
//  Доверенность - см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.Доверенность
//  ОшибкиЗаполнения - см. НовыйНаборОшибокЗаполненияОбъектаМЧД
// 
Процедура ПроверитьСовпадениеПредставителяИДоверителяМЧДПередоверия(Знач Доверенность, ОшибкиЗаполнения)

	Доверители = Доверенность.Документ.Передов.СвПередПолн; // см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.СвПередПолн
	Представители = Доверенность.Документ.Передов.СвПолучПолн; // см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.СвУпПредТип
	...
	Для Каждого Доверитель Из Доверители Цикл 
		...
		ТипДоверителя = Доверитель.ТипПерПолн; // Строка 
		
		Если ТипДоверителя = ТипыДоверителей().ЮридическоеЛицо Тогда
			
			РосОргДовер = Доверитель.ПередПолн.РосОргПерПолн; // см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.РосОргДоверТип
			Если РосОргДовер.ЕИОФЛ = КлассификаторДаНет().Да Тогда
				ИННФизЛицаДоверителяДействующегоБезДоверенности = РосОргДовер.ЛицоБезДов[0].СвФЛ.ИННФЛ; // Строка
			ИначеЕсли РосОргДовер.ЕИОИП = КлассификаторДаНет().Да Тогда
				ИННФизЛицаДоверителяДействующегоБезДоверенности = РосОргДовер.ЛицоБезДов[0].СВИП.ИННФЛ; // Строка
		...
		ИначеЕсли ТипДоверителя = ТипыДоверителей().ИндивидуальныйПредприниматель Тогда
			ИННФизЛицаДоверителяДействующегоБезДоверенности = Доверитель.ПередПолн.ИППерПолн.ИННФЛ; // Строка
		КонецЕсли;	
		...
		Для Каждого Представитель Из Представители Цикл
			...
			Если ТипПредставителя = ТипыПредставителей().ФизическоеЛицо Тогда
				ИННФизЛицаПредставителя = Представитель.Пред.СведФизЛ.ИННФЛ; // Строка
				Путь = СтрШаблон("Доверенность[0].Документ[0].Передов[0].СвПолучПолн[%1].Пред[0].СведФизЛ[0].ИННФЛ",
					ИндексПредставителя);
			ИначеЕсли ТипПредставителя = ТипыПредставителей().ИндивидуальныйПредприниматель Тогда 			
				ИННФизЛицаПредставителя = Представитель.Пред.СведИП.ИННФЛ; // Строка
				Путь = СтрШаблон("Доверенность[0].Документ[0].Передов[0].СвПолучПолн[%1].Пред[0].СведИП[0].ИННФЛ",
					ИндексПредставителя);
			Иначе
				ИндексПредставителя = ИндексПредставителя + 1;
				Продолжить;	
...
Процедура ПроверитьУсловныеПоляДоверителяМЧДПередоверия(Доверитель, ОшибкиЗаполнения, ПутьДоверителя)
	...
	Если Доверитель.ТипПерПолн = ТипыДоверителей.ЮридическоеЛицо Тогда
		...
		Если Доверитель.ПередПолн.РосОргПерПолн <> Неопределено Тогда
			...
			СведенияЕИО.ЕИОУК = Доверитель.ПередПолн.РосОргПерПолн.ЕИОУК = КлассификаторБулево.Да;
			СведенияЕИО.ЕИОИП = Доверитель.ПередПолн.РосОргПерПолн.ЕИОИП = КлассификаторБулево.Да;
			СведенияЕИО.ЕИОФЛ = Доверитель.ПередПолн.РосОргПерПолн.ЕИОФЛ = КлассификаторБулево.Да;
...
Процедура ПроверитьУсловныеПоляДоверителяМЧД(Доверитель, ОшибкиЗаполнения, ПутьДоверителя)
	...
	ИначеЕсли Доверитель.ТипДоверит = ТипыДоверителей.ЮридическоеЛицо Тогда
		...
		Если Доверитель.Доверит.РосОргДовер <> Неопределено Тогда
			...
			СведенияЕИО = НовыеСведенияОЕдиноличномИсполнительномОргане();
			СведенияЕИО.ЕИОУК = Доверитель.Доверит.РосОргДовер.ЕИОУК = КлассификаторБулево.Да;
			СведенияЕИО.ЕИОИП = Доверитель.Доверит.РосОргДовер.ЕИОИП = КлассификаторБулево.Да;
			СведенияЕИО.ЕИОФЛ = Доверитель.Доверит.РосОргДовер.ЕИОФЛ = КлассификаторБулево.Да;
...
// Проверяет условные поля представителя МЧД.
// 
// Параметры:
//  Представитель - См. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.СвУпПредТип
//  ОшибкиЗаполнения - см. НовыйНаборОшибокЗаполненияОбъектаМЧД
...
Процедура ПроверитьУсловныеПоляПредставителяМЧД(Представитель, ОшибкиЗаполнения, ПутьПредставителя)
	...
	ТребуетсяИсключитьДУЛПредставителя = МашиночитаемыеДоверенности.ПроверитьУсловиеИсключенияСведенийДУЛПредставителяИзМЧД();
	...
	Если Представитель.ТипПред = ТипыПредставителей.ЮридическоеЛицо Тогда
		...
		ДобавитьОбязательныеПоляПроверкиПредставительЮридическоеЛицо(НаборОбязательныхПолей, Представитель, 
			ПутьПредставителя);
	
	ИначеЕсли Представитель.ТипПред = ТипыПредставителей.ФизическоеЛицо Тогда
		
		ДобавитьОбязательныеПоляПроверкиПредставительФизическоеЛицо(НаборОбязательныхПолей, Представитель, 
			ПутьПредставителя);
		
		Если ТребуетсяИсключитьДУЛПредставителя Тогда
			
			ДобавитьУсловныеПоляДУЛФЛПредставителяМЧДПриУсловииЗаполненностиОдного(НаборОбязательныхПолей, 
				Представитель, ПутьПредставителя);
		Иначе
			
			ДобавитьУсловныеПоляДУЛФЛПредставителяМЧД(НаборОбязательныхПолей, Представитель, ПутьПредставителя);
			
		КонецЕсли;
		
	ИначеЕсли Представитель.ТипПред = ТипыПредставителей.ИндивидуальныйПредприниматель Тогда
		
		ДобавитьОбязательныеПоляПроверкиПредставительИндивидуальныйПредприниматель(НаборОбязательныхПолей, 
			Представитель, ПутьПредставителя);
				
		Если ТребуетсяИсключитьДУЛПредставителя Тогда 
			
			ДобавитьУсловныеПоляДУЛИППредставителяМЧДПриУсловииЗаполненностиОдного(НаборОбязательныхПолей, 
				Представитель, ПутьПредставителя);
						
		Иначе
			
			ДобавитьУсловныеПоляДУЛИППредставителяМЧД(НаборОбязательныхПолей, Представитель, ПутьПредставителя);
...
// Добавить все условные поля, требующие проверки заполнения, документа удостоверяющего личность физического лица 
// (индивидуального предпринимателя), при условии заполненности хотя бы одного из его свойств. 
// Т.е. либо заполняем все поля, либо не заполняем ни одного.
// 
// Параметры:
//  НаборОбязательныхПолей - Массив из Структура:
// * ОбъектXDTO - Неопределено, 
//  			- ОбъектXDTO
// * ЛокальныйПуть - Строка 
// * ПолныйПутьПоля - Строка 
// * ТекстОшибки - Строка 
//  Представитель - ОбъектXDTO
//  ПутьПредставителя - Строка
Процедура ДобавитьУсловныеПоляДУЛИППредставителяМЧДПриУсловииЗаполненностиОдного(НаборОбязательныхПолей, 
	Знач Представитель, Знач ПутьПредставителя)
	
	НаборПолейДляПроверкиЧастичногоЗаполненияДУЛ = Новый Массив; // Массив из Структура см. НовыеДанныеУсловногоПоля()
	НаборОбязательныхПолейДляПроверкиДУЛ = Новый  Массив; // Массив из Структура см. НовыеДанныеУсловногоПоля()
	
	ДобавитьУсловныеПоляДУЛИППредставителяМЧД(НаборПолейДляПроверкиЧастичногоЗаполненияДУЛ, Представитель, 
		ПутьПредставителя);
	
	ТребуетсяОбязательноеЗаполнениеВсехПолейДУЛ = Ложь;
	Для Каждого Поле Из НаборПолейДляПроверкиЧастичногоЗаполненияДУЛ Цикл
		
		Если ДанныеОбъектаXDTOПоПути(Поле.ОбъектXDTO, Поле.ЛокальныйПуть) <> Неопределено Тогда
			ТребуетсяОбязательноеЗаполнениеВсехПолейДУЛ = Истина;
		Иначе
			НаборОбязательныхПолейДляПроверкиДУЛ.Добавить(Поле);
		КонецЕсли;
		
	КонецЦикла;
	
	Если ТребуетсяОбязательноеЗаполнениеВсехПолейДУЛ Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(НаборОбязательныхПолей, НаборОбязательныхПолейДляПроверкиДУЛ); 
	КонецЕсли;
	
КонецПроцедуры

// Добавить все условные поля, требующие проверки заполнения, документа удостоверяющего личность физического лица, 
// при условии заполненности хотя бы одного из его свойств. Т.е. либо заполняем все поля, либо не заполняем ни одного.
// Т.е. либо заполняем все поля, либо не заполняем ни одного.
//  
// Параметры:
//  НаборОбязательныхПолей - Массив из Структура:
...
//  Представитель - ОбъектXDTO
//  ПутьПредставителя - Строка
Процедура ДобавитьУсловныеПоляДУЛФЛПредставителяМЧДПриУсловииЗаполненностиОдного(НаборОбязательныхПолей, 
	Знач Представитель, Знач ПутьПредставителя)
		
	НаборПолейДляПроверкиЧастичногоЗаполненияДУЛ = Новый Массив; // Массив из Структура см. НовыеДанныеУсловногоПоля()
	НаборОбязательныхПолейДляПроверкиДУЛ = Новый  Массив; // Массив из Структура см. НовыеДанныеУсловногоПоля()
	
	ДобавитьУсловныеПоляДУЛФЛПредставителяМЧД(НаборПолейДляПроверкиЧастичногоЗаполненияДУЛ, Представитель, 
		ПутьПредставителя);
...
// Добавить обязательные поля, требующие проверки заполнения, для представителя - индивидуального предпринимателя.
// 
// Параметры:
//  НаборОбязательныхПолей - Массив из Структура:
// * ОбъектXDTO - Неопределено, 
...
//  Представитель - ОбъектXDTO
//  ПутьПредставителя - Строка
Процедура ДобавитьОбязательныеПоляПроверкиПредставительИндивидуальныйПредприниматель(НаборОбязательныхПолей, 
	Знач Представитель, Знач ПутьПредставителя)
...
// Добавить обязательные поля, требующие проверки заполнения, для представителя - физического лица.
// 
// Параметры:
//  НаборОбязательныхПолей - Массив из Структура:
// * ОбъектXDTO - Неопределено, 
...
//  Представитель - ОбъектXDTO
//  ПутьПредставителя - Строка
Процедура ДобавитьОбязательныеПоляПроверкиПредставительФизическоеЛицо(НаборОбязательныхПолей, Знач Представитель, 
	Знач ПутьПредставителя)
	...
	ДанныеПоля.ТекстОшибки = НСтр("ru = 'Не указаны сведения об ИНН физического лица.'");
	НаборОбязательныхПолей.Добавить(ДанныеПоля);
	...
	ДанныеПоля.ТекстОшибки = НСтр("ru = 'Не указаны сведения о СНИЛС физического лица.'");
	НаборОбязательныхПолей.Добавить(ДанныеПоля);
	...
	ДанныеПоля.ТекстОшибки = НСтр("ru = 'Не указаны сведения о дате рождения физического лица.'");
	НаборОбязательныхПолей.Добавить(ДанныеПоля);

КонецПроцедуры

// Добавить обязательные поля, требующие проверки заполнения, для представителя с типом юридическое лицо.
// 
// Параметры:
//  НаборОбязательныхПолей - Массив из Структура:
// * ОбъектXDTO - Неопределено, 
...
//  Представитель - ОбъектXDTO
//  ПутьПредставителя - Строка
Процедура ДобавитьОбязательныеПоляПроверкиПредставительЮридическоеЛицо(НаборОбязательныхПолей, Знач Представитель, 
	Знач ПутьПредставителя)
	...
	ДанныеПоля.ТекстОшибки = НСтр("ru = 'Не указаны сведения об ИНН представителя.'");
	НаборОбязательныхПолей.Добавить(ДанныеПоля);
	...
	ДанныеПоля.ТекстОшибки = НСтр("ru = 'Не указаны сведения об ОГРН представителя.'");
	НаборОбязательныхПолей.Добавить(ДанныеПоля);
	
КонецПроцедуры

// Добавляет условные поля, требующие проверки заполнения документа, удостоверяющего личность представителя - 
// физического лица.
// 
// Параметры:
//  НаборОбязательныхПолей - Массив из Структура:
...
//  Представитель - ОбъектXDTO
//  ПутьПредставителя - Строка
Процедура ДобавитьУсловныеПоляДУЛФЛПредставителяМЧД(НаборОбязательныхПолей, Знач Представитель, Знач ПутьПредставителя)
	
	ДанныеПоля = НовыеДанныеУсловногоПоля();
	ДанныеПоля.ОбъектXDTO = Представитель;
	ДанныеПоля.ЛокальныйПуть = "Пред.СведФизЛ.СведФЛ.УдЛичнФЛ.КодВидДок";
	ДанныеПоля.ПолныйПутьПоля =
		СтрШаблон("%1.Пред[0].СведФизЛ[0].СведФЛ[0].УдЛичнФЛ[0].КодВидДок", ПутьПредставителя);
	ДанныеПоля.ТекстОшибки = НСтр("ru = 'Не указан вид документа, удостоверяющего личность физического лица.'");
	НаборОбязательныхПолей.Добавить(ДанныеПоля);
	
	ДанныеПоля = НовыеДанныеУсловногоПоля();
	ДанныеПоля.ОбъектXDTO = Представитель;
	ДанныеПоля.ЛокальныйПуть = "Пред.СведФизЛ.СведФЛ.УдЛичнФЛ.СерНомДок";
	ДанныеПоля.ПолныйПутьПоля =
		СтрШаблон("%1.Пред[0].СведФизЛ[0].СведФЛ[0].УдЛичнФЛ[0].СерНомДок", ПутьПредставителя);
	ДанныеПоля.ТекстОшибки = НСтр("ru = 'Не указаны серия и номер документа, удостоверяющего личность физического лица.'");
	НаборОбязательныхПолей.Добавить(ДанныеПоля); 
	
	ДанныеПоля = НовыеДанныеУсловногоПоля();
	ДанныеПоля.ОбъектXDTO = Представитель;
	ДанныеПоля.ЛокальныйПуть = "Пред.СведФизЛ.СведФЛ.УдЛичнФЛ.ДатаДок";
	ДанныеПоля.ПолныйПутьПоля =
		СтрШаблон("%1.Пред[0].СведФизЛ[0].СведФЛ[0].УдЛичнФЛ[0].ДатаДок", ПутьПредставителя);
	ДанныеПоля.ТекстОшибки = НСтр("ru = 'Не указана дата выдачи документа, удостоверяющего личность физического лица.'");
	НаборОбязательныхПолей.Добавить(ДанныеПоля);
	
	ЗначениеКодаВидДокумента = ДанныеОбъектаXDTOПоПути(Представитель, "Пред.СведФизЛ.СведФЛ.УдЛичнФЛ.КодВидДок");
	
	Если ЗначениеКодаВидДокумента <> "10" Тогда
		...
		ДанныеПоля.ТекстОшибки = НСтр("ru = 'Не указаны сведения о наименовании органа, выдавшего документ, удостоверяющий личность физического лица.'");
		НаборОбязательныхПолей.Добавить(ДанныеПоля);
		...
		ДанныеПоля.ТекстОшибки = НСтр("ru = 'Не указаны сведения о коде подразделения органа, выдавшего документ, удостоверяющий личность физического лица.'");
		НаборОбязательныхПолей.Добавить(ДанныеПоля);
		
	КонецЕсли;
	
	Если ЗначениеКодаВидДокумента = "22" Тогда
		...
		ДанныеПоля.ТекстОшибки = НСтр("ru = 'Не указана дата истечения срока действия документа, удостоверяющего личность физического лица.'");
		НаборОбязательныхПолей.Добавить(ДанныеПоля);
...
//  Представитель - ОбъектXDTO
//  ПутьПредставителя - Строка
Процедура ДобавитьУсловныеПоляДУЛИППредставителяМЧД(НаборОбязательныхПолей, Знач Представитель, Знач ПутьПредставителя)
	
	ДанныеПоля = НовыеДанныеУсловногоПоля();
	ДанныеПоля.ОбъектXDTO = Представитель;
	ДанныеПоля.ЛокальныйПуть = "Пред.СведИП.СведФЛ.УдЛичнФЛ.КодВидДок";
	ДанныеПоля.ПолныйПутьПоля =
		СтрШаблон("%1.Пред[0].СведИП[0].СведФЛ[0].УдЛичнФЛ[0].КодВидДок", ПутьПредставителя);
	ДанныеПоля.ТекстОшибки = НСтр("ru = 'Не указан вид документа, удостоверяющего личность физического лица.'");
	НаборОбязательныхПолей.Добавить(ДанныеПоля);
	
	ДанныеПоля = НовыеДанныеУсловногоПоля();
	ДанныеПоля.ОбъектXDTO = Представитель;
	ДанныеПоля.ЛокальныйПуть = "Пред.СведИП.СведФЛ.УдЛичнФЛ.СерНомДок";
	ДанныеПоля.ПолныйПутьПоля =
		СтрШаблон("%1.Пред[0].СведИП[0].СведФЛ[0].УдЛичнФЛ[0].СерНомДок", ПутьПредставителя);
	ДанныеПоля.ТекстОшибки = НСтр("ru = 'Не указаны серия и номер документа, удостоверяющего личность физического лица.'");
	НаборОбязательныхПолей.Добавить(ДанныеПоля);
	
	ДанныеПоля = НовыеДанныеУсловногоПоля();
	ДанныеПоля.ОбъектXDTO = Представитель;
	ДанныеПоля.ЛокальныйПуть = "Пред.СведИП.СведФЛ.УдЛичнФЛ.ДатаДок";
	ДанныеПоля.ПолныйПутьПоля =
		СтрШаблон("%1.Пред[0].СведИП[0].СведФЛ[0].УдЛичнФЛ[0].ДатаДок", ПутьПредставителя);
	ДанныеПоля.ТекстОшибки = НСтр("ru = 'Не указана дата выдачи документа, удостоверяющего личность физического лица.'");
	НаборОбязательныхПолей.Добавить(ДанныеПоля);
	
	ЗначениеКодаВидДокумента = ДанныеОбъектаXDTOПоПути(Представитель, "Пред.СведИП.СведФЛ.УдЛичнФЛ.КодВидДок");
...
Процедура ПроверитьУсловныеПоляПолномочийМЧД(Доверенность, ОшибкиЗаполнения)
	...
	Если ДанныеОбъектаXDTOПоПути(Доверенность, "Документ.Довер.СвПолн.ТипПолн") = ТипыПолномочий().Текстовые Тогда
		...
		ТекстПолномочий = ДанныеОбъектаXDTOПоПути(Доверенность, ДанныеПоля.ЛокальныйПуть); // Строка
		Если ТекстПолномочий <> Неопределено Тогда
			ТекстПолномочийБезПереносовСтрок = СтрЗаменить(ТекстПолномочий, Символы.ПС, "");
			Если СтрДлина(ТекстПолномочийБезПереносовСтрок) > 10000 Тогда
				ДанныеПоля.ТекстОшибки = НСтр("ru = 'Текст полномочий превышает ограничение в 10000 символов.'");
				ДобавитьОшибкуЗаполненияОбъектаМЧД(ОшибкиЗаполнения,
					ДанныеПоля.ПолныйПутьПоля, ДанныеПоля.ТекстОшибки);
			КонецЕсли;
		КонецЕсли;
	...
	Иначе
		...
		ПроверитьМаксимальноеКоличествоОграниченийВПолномочиях(СписокМашиночитаемыхПолномочий, ОшибкиЗаполнения);
...
//                             Доверенность[0].<Свойство>[0].<Субсвойство>[Индекс].<Субсубсвойство>...
...
// Проверяет превышение максимально допустимого количества ограничений в полномочиях.
// 
// Параметры:
//  СписокМашиночитаемыхПолномочий - СписокXDTO
//  ОшибкиЗаполнения см. НовыйНаборОшибокЗаполненияОбъектаМЧД
Процедура ПроверитьМаксимальноеКоличествоОграниченийВПолномочиях(Знач СписокМашиночитаемыхПолномочий, ОшибкиЗаполнения)
	
	ТекущийНомерПолномочия = 1;
	МаксимальноеКоличествоОграничений = МашиночитаемыеДоверенности.МаксимальноеКоличествоОграниченийВПолномочии();
	
	Для Каждого ПолномочиеXDTO Из СписокМашиночитаемыхПолномочий Цикл
		...
		КоличествоОграниченийПолномочия = ОгрСвПолнXDTO.Количество();
		
		Если КоличествоОграниченийПолномочия > МаксимальноеКоличествоОграничений Тогда
			
			ТекстОшибки = 
				СтрШаблон(НСтр("ru = 'В полномочии %1 количество выбранных ограничений (%2) превышает максимально допустимое количество %3.'"),
				ТекущийНомерПолномочия,
				КоличествоОграниченийПолномочия,
				МаксимальноеКоличествоОграничений);
			Путь = "Доверенность[0].Документ[0].Довер[0].СвПолн[0].МашПолн";
			ДобавитьОшибкуЗаполненияОбъектаМЧД(ОшибкиЗаполнения, Путь, ТекстОшибки);
			
		КонецЕсли;
		
		ТекущийНомерПолномочия = ТекущийНомерПолномочия + 1;
...
// 
// Возвращаемое значение:
//  см. НоваяТаблицаДанныхМЧД 
Функция ТаблицаДанныхМЧДДляПодписания(Доверенности)
	
	ТаблицаДанныхМЧД = НоваяТаблицаДанныхМЧД();
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	МЧД003.Ссылка КАК Ссылка,
	|	МЧД003.СтатусВРеестреФНС КАК СтатусВРеестреФНС,
	|	ЕСТЬNULL(ЖурналМашиночитаемыхДоверенностей.ВсеДоверители, """") КАК ДоверителиИзЖурнала,
	|	МЧД003.Доверители.(
	|		Ссылка,
	|		НомерСтроки,
	|		ТипДоверителя,
	...
	|		КПП,
	|		ОГРН,
	|		СНИЛС) КАК ДоверителиИзДоверенности
	|ИЗ
	|	Справочник.МЧД003 КАК МЧД003
	...
	Пока Выборка.Следующий() Цикл
		...
		СведенияОСтатусеМЧД = НовыеСведенияОСтатусеМЧД();
		ЗаполнитьЗначенияСвойств(СведенияОСтатусеМЧД, Выборка);
		
		Если ЗначениеЗаполнено(Выборка.ДоверителиИзЖурнала) Тогда
			
			ДобавитьДоверенностиПоДоверителямЖурнала(ТаблицаДанныхМЧД,
				СведенияОСтатусеМЧД,
				РегистрыСведений.ЖурналМашиночитаемыхДоверенностей.СведенияОДоверителях(Выборка.ДоверителиИзЖурнала));
				
		Иначе
			ДобавитьДоверенностиПоДоверителямТабличнойЧасти(ТаблицаДанныхМЧД, 
				СведенияОСтатусеМЧД, 
				Выборка.ДоверителиИзДоверенности);
		КонецЕсли;
...
// Параметры:
//  ТаблицаДанныхМЧД - см. НоваяТаблицаДанныхМЧД
//  СведенияОСтатусеМЧД - см. НовыеСведенияОСтатусеМЧД
//  ДоверителиИзЖурнала - см. РегистрСведенийМенеджер.ЖурналМашиночитаемыхДоверенностей.СведенияОДоверителях
Процедура ДобавитьДоверенностиПоДоверителямЖурнала(ТаблицаДанныхМЧД, СведенияОСтатусеМЧД, ДоверителиИзЖурнала)
	
	Для Каждого ДанныеДоверителя Из ДоверителиИзЖурнала Цикл
		
		Если ДанныеДоверителя.ДанныеЮридическогоЛица <> Неопределено Тогда
			ДанныеЮридическогоЛица = ДанныеДоверителя.ДанныеЮридическогоЛица;
			ЛицаБезДоверенности = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеЮридическогоЛица, 
				"ЛицаБезДоверенности", 
				Новый Массив);
			
			ОписаниеДоверителя = НовоеОписаниеДоверителя();
			ЗаполнитьЗначенияСвойств(ОписаниеДоверителя, ДанныеЮридическогоЛица);
			ДобавитьЛицаБезДоверенности(ТаблицаДанныхМЧД, СведенияОСтатусеМЧД, ЛицаБезДоверенности, ОписаниеДоверителя);
		КонецЕсли;
		
		Если ДанныеДоверителя.ДанныеИностраннойОрганизации <> Неопределено Тогда
			ДанныеИностраннойОрганизации = ДанныеДоверителя.ДанныеИностраннойОрганизации;
			Руководители = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеИностраннойОрганизации, 
				"Руководители", 
				Новый Массив);
			
			ОписаниеДоверителя = НовоеОписаниеДоверителя();
			ЗаполнитьЗначенияСвойств(ОписаниеДоверителя, ДанныеИностраннойОрганизации);
			ДобавитьЛицаБезДоверенности(ТаблицаДанныхМЧД, СведенияОСтатусеМЧД, Руководители, ОписаниеДоверителя);
		КонецЕсли;
		
		Если ДанныеДоверителя.ДанныеИндивидуальногоПредпринимателя <> Неопределено Тогда
			ДанныеИП = ДанныеДоверителя.ДанныеИндивидуальногоПредпринимателя;
			ОписаниеДоверителя = НовоеОписаниеДоверителя();
			ЗаполнитьЗначенияСвойств(ОписаниеДоверителя, ДанныеИП);
			ДобавитьСтрокуМЧД(ТаблицаДанныхМЧД, СведенияОСтатусеМЧД, ОписаниеДоверителя);
		КонецЕсли;
		
		Если ДанныеДоверителя.ДанныеФизическогоЛица <> Неопределено Тогда
			ДанныеФизическогоЛица = ДанныеДоверителя.ДанныеФизическогоЛица;
			ОписаниеДоверителя = НовоеОписаниеДоверителя();
			ЗаполнитьЗначенияСвойств(ОписаниеДоверителя, ДанныеФизическогоЛица);
			ДобавитьСтрокуМЧД(ТаблицаДанныхМЧД, СведенияОСтатусеМЧД, ОписаниеДоверителя);
		КонецЕсли;
...
//  ОписаниеДоверителя - см. НовоеОписаниеДоверителя
Процедура ДобавитьЛицаБезДоверенности(ТаблицаДанныхМЧД, СведенияОСтатусеМЧД, ЛицаБезДоверенности, ОписаниеДоверителя)
	
	Для Каждого ЛицоБезДоверенности Из ЛицаБезДоверенности Цикл
		
		ЛицоБезДоверенностиИНН = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ЛицоБезДоверенности, "ИНН", "");
		ЭтоЮридическоеЛицоБезДоверенности = СтрДлина(СокрЛП(ЛицоБезДоверенностиИНН)) = 10;
		Если ЭтоЮридическоеЛицоБезДоверенности Тогда
			
			ОписаниеУК = НовоеОписаниеДоверителя();
			ЗаполнитьЗначенияСвойств(ОписаниеУК, ЛицоБезДоверенности);
			
			ЛицаБезДоверенностиУК = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ЛицоБезДоверенности, 
				"ЛицаБезДоверенности", 
				Новый Массив);
				
			Если ЗначениеЗаполнено(ЛицаБезДоверенностиУК) Тогда
				ДобавитьЛицаБезДоверенности(ТаблицаДанныхМЧД, 
					СведенияОСтатусеМЧД, 
					ЛицаБезДоверенностиУК, 
					ОписаниеУК);
			Иначе
				ДобавитьСтрокуМЧД(ТаблицаДанныхМЧД, СведенияОСтатусеМЧД, ОписаниеУК);
			КонецЕсли;
			
		Иначе
			ОписаниеДоверителяССНИЛС = ОбщегоНазначения.СкопироватьРекурсивно(ОписаниеДоверителя);
			ОписаниеДоверителяССНИЛС.СНИЛС = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ЛицоБезДоверенности, 
				"СНИЛС", 
				"");
			ДобавитьСтрокуМЧД(ТаблицаДанныхМЧД, СведенияОСтатусеМЧД, ОписаниеДоверителяССНИЛС);
		КонецЕсли;
...
//  ТаблицаДанныхМЧД - см. НоваяТаблицаДанныхМЧД
//  СведенияОСтатусеМЧД - см. НовыеСведенияОСтатусеМЧД
//  ДоверителиИзДоверенности - РезультатЗапроса
Процедура ДобавитьДоверенностиПоДоверителямТабличнойЧасти(ТаблицаДанныхМЧД, СведенияОСтатусеМЧД, ДоверителиИзДоверенности)
	ВыборкаДоверителей = ДоверителиИзДоверенности.Выбрать();
	Пока ВыборкаДоверителей.Следующий() Цикл
		ОписаниеДоверителя = НовоеОписаниеДоверителя();
		ЗаполнитьЗначенияСвойств(ОписаниеДоверителя, ВыборкаДоверителей);
		ДобавитьСтрокуМЧД(ТаблицаДанныхМЧД, СведенияОСтатусеМЧД, ОписаниеДоверителя);
	КонецЦикла;
КонецПроцедуры
...
//  СведенияОСтатусеМЧД - см. НовыеСведенияОСтатусеМЧД
//  ОписаниеДоверителя - см. НовоеОписаниеДоверителя
Процедура ДобавитьСтрокуМЧД(ТаблицаДанныхМЧД, СведенияОСтатусеМЧД, ОписаниеДоверителя)
	
	СтрокаМЧД = ТаблицаДанныхМЧД.Добавить();
	ЗаполнитьЗначенияСвойств(СтрокаМЧД, СведенияОСтатусеМЧД);
	СтрокаМЧД.ОрганизацияИНН = ОписаниеДоверителя.ИНН;
	СтрокаМЧД.ОрганизацияКПП = ОписаниеДоверителя.КПП;
	СтрокаМЧД.ОрганизацияОГРН = ОписаниеДоверителя.ОГРН;
	СтрокаМЧД.СНИЛСДоверителя = ОписаниеДоверителя.СНИЛС;
	
	ОтборОрганизации = Новый Структура("ИНН, КПП, ОГРН");
	ЗаполнитьЗначенияСвойств(ОтборОрганизации, ОписаниеДоверителя);
	ЭлектронноеВзаимодействиеПереопределяемый.НайтиСсылкуНаОбъект("Организации",
		СтрокаМЧД.Организация,
		, 
		ОтборОрганизации);
КонецПроцедуры

// Возвращаемое значение:
//  Структура - Новое описание доверителя:
// * ИНН - Строка
// * КПП - Строка
// * ОГРН - Строка
// * СНИЛС - Строка
Функция НовоеОписаниеДоверителя()
	Результат = Новый Структура;
	Результат.Вставить("ИНН", "");
	Результат.Вставить("КПП", "");
	Результат.Вставить("ОГРН", "");
...
// Возвращаемое значение:
//  Структура - Новые основные данные доверенности:
// * Ссылка - СправочникСсылка.МЧД003
// * СтатусВРеестреФНС - ПеречислениеСсылка.СтатусыМашиночитаемойДоверенностиВРеестреФНС
Функция НовыеСведенияОСтатусеМЧД()
	Результат = Новый Структура;
