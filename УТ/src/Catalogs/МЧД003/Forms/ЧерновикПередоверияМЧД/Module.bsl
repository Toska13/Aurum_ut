	...
	Если Доверенность.Количество() = 0 Тогда
		...
		Если ЗначениеЗаполнено(ИсходнаяМЧД) Тогда
			...
			Если Параметры.Ключ.Пустая() Тогда
				...
				УстановитьИспользуемыеПолномочия(ДеревоПолномочийСОграничениями, Истина);
			КонецЕсли;
...
Процедура ДоверенностьДокументПередовСвПолучПолнПредСведФизЛСведФЛУдЛичнФЛКодВидДокПриИзменении(Элемент)
	...
	МашиночитаемыеДоверенностиКлиент.УстановитьОформлениеНастройкиОтображенияИЗначенияПолейДУЛФЛПоУмолчанию(
		Элемент.Родитель,
		ПоляУдостоверяющиеЛичностьФизическогоЛица,
		Элементы.ДоверенностьДокументПередовСвПолучПолнПредСведФизЛСведФЛУдЛичнФЛСрокДействия);
...
Процедура ДоверенностьДокументПередовСвПолучПолнПредСведИПСведФЛУдЛичнФЛКодВидДокПриИзменении(Элемент)
	...
	МашиночитаемыеДоверенностиКлиент.УстановитьОформлениеНастройкиОтображенияИЗначенияПолейДУЛФЛПоУмолчанию(
		Элемент.Родитель,
		ПоляУдостоверяющиеЛичностьФизическогоЛица,
		Элементы.ДоверенностьДокументПередовСвПолучПолнПредСведИПСведФЛУдЛичнФЛСрокДействия);
...
&НаКлиенте
Процедура ДеревоПолномочийСОграничениямиВключеноПриИзменении(Элемент)
	
	ПорядковыйНомер = Элементы.ДеревоПолномочийСОграничениями.ТекущиеДанные.ПорядковыйНомер;
	Включено = Элементы.ДеревоПолномочийСОграничениями.ТекущиеДанные.Включено;
	УстановитьИспользуемыеПолномочия(ДеревоПолномочийСОграничениями, Включено, ПорядковыйНомер);
	УстановитьЗаголовокВкладкиПолномочияДляСоставныхПолномочий(ДеревоПолномочийСОграничениями, 
		ЗаголовокВкладкиПолномочия);
...
Функция ТипыДоверителей()
	...
	Возврат Справочники.МЧД003.ТипыДоверителей();
...
Процедура ЗаполнитьСписокВидовДокументовФизическогоЛица()
	...
	ЭлементыВыбораВидаДокумента.Добавить(Элементы.ДоверенностьДокументПередовСвПолучПолнПредСведФизЛСведФЛУдЛичнФЛКодВидДок);
	ЭлементыВыбораВидаДокумента.Добавить(Элементы.ДоверенностьДокументПередовСвПолучПолнПредСведИПСведФЛУдЛичнФЛКодВидДок);
...
Процедура УстановитьДоступностьЭлементов()
	...
	Элементы.ПолномочияПередоверия.Доступность = ЗначениеЗаполнено(ИсходнаяМЧД);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьИспользуемыеПолномочия(ДеревоПолномочий, Включено, ПорядковыйНомер = "")
	
	Для Каждого ЭлементДерева Из ДеревоПолномочий.ПолучитьЭлементы() Цикл
		Если ЗначениеЗаполнено(ПорядковыйНомер) И ЭлементДерева.ПорядковыйНомер = ПорядковыйНомер Тогда 
			ЭлементДерева.Включено = Включено;
		ИначеЕсли Не ЗначениеЗаполнено(ПорядковыйНомер) Тогда
			ЭлементДерева.Включено = Включено;
		КонецЕсли;
		УстановитьИспользуемыеПолномочия(ЭлементДерева, Включено, ПорядковыйНомер);
	КонецЦикла;
	
КонецПроцедуры

&НаСервере
Процедура УстановитьОтмеченныеПолномочияПоНастройкам();
	Для Каждого НастройкаПроверки Из НастройкиПроверкиРасширенные Цикл
		УстановитьИспользуемыеПолномочия(
			ДеревоПолномочийСОграничениями, НастройкаПроверки.Включено, НастройкаПроверки.ПорядковыйНомер); 
	КонецЦикла;
КонецПроцедуры
...
Процедура УстановитьЗаголовокСтраницыПолномочий()
	...
	Если ТипПолномочий = ТипыПолномочий().Текстовые Тогда
		...
		ТекстПолномочий = ЗначениеВДеревеДоверенности("Доверенность[0].Документ[0].Передов[0].СвПолн[0].ТекстПолн");
		
		Если Не ЗначениеЗаполнено(ТекстПолномочий) Тогда
			ЗаголовокВкладкиПолномочия = НСтр("ru = 'Не указаны'");
		ИначеЕсли ТекстПолномочий = МашиночитаемыеДоверенностиКлиентСервер.ПредставлениеНеограниченныхПолномочий() Тогда
			ЗаголовокВкладкиПолномочия = НСтр("ru = 'По умолчанию'");
		Иначе
			ЗаголовокВкладкиПолномочия = НСтр("ru = 'Указаны текстом'");
		КонецЕсли;
...
Процедура УдалитьТекущегоПредставителя()
	...
	ИначеЕсли КоличествоЗаписей = 1 Тогда
		...
		СброситьОформлениеЭлементовФормы(Элементы.ГруппаПредставительПередоверия, Истина);
	КонецЕсли;
...
Процедура ОбновитьДанныеПолномочийСОграничениямиПоДеревуПолномочий()
	...
	ВключенныеПолномочия = Новый Соответствие;
	ПолномочияВДеревеВизуализации = ДеревоПолномочийСОграничениями.ПолучитьЭлементы();
	Для Каждого ПолномочиеВДеревеВизуализации Из ПолномочияВДеревеВизуализации Цикл
		ВключенныеПолномочия.Вставить(
			ПолномочиеВДеревеВизуализации.ПорядковыйНомер, ПолномочиеВДеревеВизуализации.Включено);	
	КонецЦикла;
	...
	Для Каждого Полномочие Из ПолномочияСОграничениями Цикл
		...
		ДанныеПолномочия = Доверенность[0].Документ[0].Передов[0].СвПолн[0].МашПолн.Добавить();
		...
		ДанныеПолномочия._Использовать = ВключенныеПолномочия[Полномочие.ПорядковыйНомер];
...
Процедура ПрочитатьЧерновикДоверенности(ТекущийОбъект)
	...
	ПостроитьДеревоВизуализации();
	УстановитьОтмеченныеПолномочияПоНастройкам();
...
Процедура ЗаполнитьДеревоПолномочийСОграничениямиПоДаннымМЧД()
	...
	ПолномочияПоДаннымМЧД = ЗначениеВДеревеДоверенности("Доверенность[0].Документ[0].Передов[0].СвПолн[0].МашПолн");
	...
	Для Каждого ПолномочиеПоДаннымМЧД Из ПолномочияПоДаннымМЧД Цикл
		...
		ПорядковыйНомер = Число(ПолномочиеПоДаннымМЧД._НомерСтроки);
		...
		Если ПолномочиеПоДаннымМЧД.ОгрСвПолн.Количество() > 0 Тогда
			...
			Для Каждого ОграничениеПоДаннымМЧД Из ПолномочиеПоДаннымМЧД.ОгрСвПолн Цикл
				...
				ПолномочиеВДереве.ПорядковыйНомер = ПорядковыйНомер;
				ПолномочиеВДереве.КонтекстПолномочия = КонтекстПолномочийПоКодам[ПолномочиеПоДаннымМЧД.КодПолн];
				ПолномочиеВДереве.Включено = ПолномочиеПоДаннымМЧД._Использовать;
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьЗаголовокВкладкиПолномочияДляСоставныхПолномочий(ДеревоПолномочийСОграничениями, ЗаголовокВкладкиПолномочия)

	КоличествоВсегоПолномочий = ДеревоПолномочийСОграничениями.ПолучитьЭлементы().Количество();
	КоличествоВыбранныхПолномочий = 0;
	
	Для Каждого Полномочие Из ДеревоПолномочийСОграничениями.ПолучитьЭлементы() Цикл
		Если Полномочие.Включено Тогда
			КоличествоВыбранныхПолномочий = КоличествоВыбранныхПолномочий + 1;
		КонецЕсли;
	КонецЦикла;
	
	ЗаголовокВкладкиПолномочия = СтрШаблон(НСтр("ru = '%1 из %2'"),
			Строка(КоличествоВыбранныхПолномочий),
			Строка(КоличествоВсегоПолномочий));
