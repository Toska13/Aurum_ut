...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЭтоНовый() И СпособУстановкиКурса = Перечисления.СпособыУстановкиКурсаВалюты.НаценкаНаКурсДругойВалюты Тогда
		БазовыеВалюты = РаботаСКурсамиВалютУТ.БазовыеВалюты();
		Если БазовыеВалюты.Найти(Ссылка) <> Неопределено Тогда
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Курс валюты не может быть связан с курсом другой валюты, так как она является базовой или валютой регламентированного учета.'"));
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
