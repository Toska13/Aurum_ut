...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДоступа("Изменение", Метаданные.Справочники.СертификатыНоменклатуры) Тогда
		Элементы.ГруппаКоманднаяПанельУстановитьСтатус.Доступность = Ложь;
	КонецЕсли;
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПриСозданииНаСервере(ЭтотОбъект,
		Элементы.КоманднаяПанельСертификатыНоменклатуры);
...
Процедура ОткрытьИзображение(Команда)
	...
	Если ТекущаяСтрока.ЭтоГруппа Тогда
		ТекстСообщения = НСтр("ru='Для группы ""%1"" не требуется ведение учета изображений сертификатов'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, Строка(ТекущаяСтрока.Ссылка));
...
Процедура Подключаемый_ВыполнитьКомандуИнтеграции(Команда)
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыполнитьПодключаемуюКомандуИнтеграции(Команда, ЭтотОбъект, Элементы.СертификатыНоменклатуры);
