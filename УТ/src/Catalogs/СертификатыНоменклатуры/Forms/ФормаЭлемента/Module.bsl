...
Процедура ОбновитьСтатусРосаккредитацииФоновоеЗадание()
	Если ИТСПодключен Тогда
		...
		ПодключитьОбработчикОповещенияСтатусРосаккредитации(); 
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбновитьСтатусРосаккредитацииНаСервере()
	...
	Если ПравоДоступа("Изменение", Метаданные.Справочники.СертификатыНоменклатуры) Тогда
		Модифицированность = Истина;
	...
	РезультатЗаданияПолученияДаныыхРосаккредитации = Неопределено;
	...
	Если РезультатРасчета.Статус = "Выполняется" Тогда
		РезультатЗаданияПолученияДаныыхРосаккредитации = РезультатРасчета;
...
Процедура ОбновитьСтатусРосаккредитацииРезультатНаСервере(ВыполнениеПроверено = Ложь)
	...
	Если РезультатЗаданияПолученияДаныыхРосаккредитации <> Неопределено Тогда
		
		Ответ = ПолучитьИзВременногоХранилища(РезультатЗаданияПолученияДаныыхРосаккредитации.АдресРезультата);
		
		Если Ответ = Неопределено Тогда
			Возврат;
		Иначе
			ЕстьОшибки = Не ПустаяСтрока(Ответ.ТекстОшибки);
			
			Если ПравоДоступа("Изменение", Метаданные.Справочники.СертификатыНоменклатуры) Тогда
				Объект.СтатусРосаккредитации				= ?(ЕстьОшибки,
					Перечисления.СтатусыСертификатовНоменклатурыРосаккредитации.ПустаяСсылка(),
					НоменклатураЛокализация.СтатусРосаккредитацииИзСтроки(Ответ.Статус));
				Объект.СсылкаНаСертификатРосаккредитации	= ?(ЕстьОшибки, "", Ответ.Гиперссылка);
...
Функция ЗаполнитьПоДаннымРосаккредитацииНаСервере()
	...
	ШаблонКонкретныйТоварОтличаетсяСерия = НСтр("ru='Подобрана серия %1, которая отличается от уже выбранной %2. Переключить область действия на ""список товаров"" и заполнить?'");
			...
			Если ЗаполненКонкретныйТовар
				...
				ИначеЕсли Строка.Номенклатура = Номенклатура
					...
					Если ЗначениеЗаполнено(Строка.Характеристика)
						...
						ШаблонКонкретныйТоварОтличаетсяХарактеристика = НСтр("ru='Подобрана характеристика %1, которая отличается от уже выбранной. Переключить область действия на ""список товаров"" и заполнить?'");
						
						ТекстСообщения = СтрШаблон(ШаблонКонкретныйТоварОтличаетсяХарактеристика, Строка.Характеристика);
						
					ИначеЕсли Не ЗначениеЗаполнено(Строка.Характеристика)
						И ЗначениеЗаполнено(Характеристика) Тогда
						
						ШаблонКонкретныйТоварОтличаетсяХарактеристика = НСтр("ru='Подобрана характеристика, которая отличается от уже выбранной %1. Переключить область действия на ""список товаров"" и заполнить?'");
						
						ТекстСообщения = СтрШаблон(ШаблонКонкретныйТоварОтличаетсяХарактеристика, Характеристика);
						
					Иначе
						ТекстСообщения = СтрШаблон(ШаблонКонкретныйТоварОтличаетсяХарактеристика, Строка.Характеристика, Характеристика);
					КонецЕсли;
					
				ИначеЕсли Строка.Номенклатура = Номенклатура
					И Строка.Характеристика = Характеристика
					И Строка.Серия <> Серия Тогда
					
					Если ЗначениеЗаполнено(Строка.Серия)
						И Не ЗначениеЗаполнено(Серия) Тогда
						
						ШаблонКонкретныйТоварОтличаетсяСерия = НСтр("ru='Подобрана серия %1, которая отличается от уже выбранной. Переключить область действия на ""список товаров"" и заполнить?'");
						
						ТекстСообщения = СтрШаблон(ШаблонКонкретныйТоварОтличаетсяСерия, Строка.Серия);
						
					ИначеЕсли Не ЗначениеЗаполнено(Строка.Серия)
						И ЗначениеЗаполнено(Серия) Тогда
						
						ШаблонКонкретныйТоварОтличаетсяСерия = НСтр("ru='Подобрана серия, которая отличается от уже выбранной %1. Переключить область действия на ""список товаров"" и заполнить?'");
						
						ТекстСообщения = СтрШаблон(ШаблонКонкретныйТоварОтличаетсяСерия, Серия);
						
					Иначе
						ТекстСообщения = СтрШаблон(ШаблонКонкретныйТоварОтличаетсяСерия, Строка.Серия, Серия);
					КонецЕсли;
...
Процедура ОбновитьТекстыРосаккредитации()
	...
	ШаблонСтатусНеДействителен = НСтр("ru='<span style=""color: РезультатПроблемаЦвет; font: ВажнаяНадписьШрифт"">%1</span> (актуально на: %2)'");
...
&НаКлиенте
Процедура ПодключитьОбработчикОповещенияСтатусРосаккредитации()
	
	Если РезультатЗаданияПолученияДаныыхРосаккредитации <> Неопределено Тогда
		ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
		ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбновитьСтатусРосаккредитацииОповещение", ЭтотОбъект);
	
		ДлительныеОперацииКлиент.ОжидатьЗавершение(
			РезультатЗаданияПолученияДаныыхРосаккредитации, ОповещениеОЗавершении, ПараметрыОжидания);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьСтатусРосаккредитацииОповещение(Результат, ДополнительныеПараметры) Экспорт
