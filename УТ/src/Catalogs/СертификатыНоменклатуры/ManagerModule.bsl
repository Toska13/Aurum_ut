...
Функция УстановитьСтатусСертификатовНоменклатуры(ПараметрыЗаполнения) Экспорт
	...
	Для Каждого ОбрабатываемаяСтрока Из ПараметрыЗаполнения.ДанныеКОбработке Цикл
		...
		Если ОбработатьОбъект
			И СертификатОбъект.ЭтоГруппа Тогда
			
			ОбработатьОбъект = Ложь;
			
			ТекстСообщения = НСтр("ru = 'Установка статуса для группы ""%1"" не требуется'");
			ТекстСообщения = СтрШаблон(ТекстСообщения, Строка(ОбрабатываемыйСертификат));
...
// Возвращает массив сертификатов номенклатуры по документу.
//
// Параметры:
//	МассивОбъектов - Массив из ДокументСсылка - документы.
//
// Возвращаемое значение:
//	Массив из СправочникСсылка.СертификатыНоменклатуры - сертификаты номенклатуры по документу.
//
Функция СертификатыНоменклатурыДокумента(МассивОбъектов) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	СертификатыНоменклатуры = Новый Массив;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТоварыССертификатами.СертификатНоменклатуры КАК СертификатНоменклатуры,
	...
	|		И НЕ ТоварыССертификатами.СертификатНоменклатуры ЕСТЬ NULL
	|	
	|СГРУППИРОВАТЬ ПО
	|	ТоварыССертификатами.СертификатНоменклатуры,
	|	ТоварыССертификатами.ТипСертификата,
	|	ТоварыССертификатами.ПриоритетСертификата,
	|	ТоварыССертификатами.СтатусРосаккредитации,
	|	ТоварыССертификатами.ДатаОбновленияСтатусаРосаккредитации";
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		СертификатыНоменклатуры.Добавить(ВыборкаПоДокументам.СертификатНоменклатуры);
	КонецЦикла;
...
Функция СформироватьПечатнуюФормуРеестрСертификатовНоменклатуры(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	|	ПРЕДСТАВЛЕНИЕ(ВсеДокументы.Ссылка) КАК ПредставлениеСсылки,
	|	ПРЕДСТАВЛЕНИЕ(ВсеДокументы.ХозяйственнаяОперация) КАК ПредставлениеХозяйственнойОперации
	|ИЗ
	|	ВсеДокументы КАК ВсеДокументы
	|ГДЕ
	|	ВсеДокументы.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|;
	|
	|//////////////////////////////////////////////////////////////////////////////// 12
	|ВЫБРАТЬ
	...
	|ВЫБРАТЬ
	|	ДанныеДокументов.Ссылка           КАК Ссылка,
	|	ДанныеОтгрузки.Номер         КАК Номер,
	|	ДанныеОтгрузки.Дата          КАК Дата,
	|	НЕОПРЕДЕЛЕНО                      КАК НомерИсправляемогоДокумента,
	|	НЕОПРЕДЕЛЕНО                      КАК ДатаИсправляемогоДокумента,
	|	ДанныеОтгрузки.Представление КАК Представление
	|ИЗ
	|	ДокументыПоКоторымВедетсяУчетСертификатов КАК ДанныеДокументов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтгрузкаТоваровКлиенту КАК ДанныеОтгрузки
	|		ПО ДанныеДокументов.Ссылка = ДанныеОтгрузки.Ссылка
	|ГДЕ
	|	ДанныеДокументов.Ссылка ССЫЛКА Документ.ОтгрузкаТоваровКлиенту
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	ВыборкаОшибкиХозяйственнаяОперация = РезультатЗапроса.Выбрать();

	РезультатЗапроса = МассивРезультатов[КоличествоРезультатов - 4]; // РезультатЗапроса
	...
	СообщитьОРезультатахПроверкиХозяйственнойОперацииДокумента(ВыборкаОшибкиХозяйственнаяОперация);
...
Функция СформироватьПечатнуюФормуИзображенияСертификатов(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	|	Справочник.СертификатыНоменклатуры КАК СертификатыНоменклатуры
	|ГДЕ
	|	СертификатыНоменклатуры.Ссылка В (&МассивОбъектов)
	...
	|	СертификатыНоменклатуры КАК СертификатыНоменклатуры
	|ГДЕ
	|	СертификатыНоменклатуры.ЭтоГруппа = ИСТИНА
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка
	|АВТОУПОРЯДОЧИВАНИЕ";
	...
	ВыборкаПоГруппам = РезультатЗапроса.Выбрать();
	...
	Пока ВыборкаПоГруппам.Следующий() Цикл
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для группы ""%1"" не требуется ведение учета изображений сертификатов.'"),
			Строка(ВыборкаПоГруппам.Ссылка));
		ОбщегоНазначения.СообщитьПользователю(
			Текст,
			ВыборкаПоГруппам.Ссылка);
	КонецЦикла;
...
Функция СформироватьПечатнуюФормуИзображенияСертификатовИзДокументовБезДублей(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	РезультатЗапроса = МассивРезультатов[КоличествоРезультатов - 4]; // РезультатЗапроса
	ВыборкаОшибкиХозяйственнаяОперация = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
...
Функция ТекстЗапросаСертификатовНоменклатурыДокумента()
	...
	|	И ТаблицаТоваров.Ссылка.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ДанныеДокумента.Ссылка               КАК Ссылка,
	|	ДанныеДокумента.Дата                 КАК Дата,
	|	ДанныеДокумента.ОтгрузкаПоЗаказам    КАК ЕстьРаспоряжения,
	|	ТаблицаТоваров.ЗаказКлиента          КАК Распоряжение,
	|	МАКСИМУМ(ТаблицаТоваров.НомерСтроки) КАК НомерСтроки,
	|	ТаблицаТоваров.Номенклатура          КАК Номенклатура,
	|	ТаблицаТоваров.Характеристика        КАК Характеристика,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтгрузкаТоваровКлиенту КАК ДанныеДокумента
	|		ПО ТаблицаТоваров.Ссылка = ДанныеДокумента.Ссылка
	|ГДЕ
	|	ТаблицаТоваров.Ссылка В(&МассивОбъектов)
	|
	...
	|	ДанныеДокумента.Ссылка,
	|	ДанныеДокумента.Дата,
	|	ДанныеДокумента.ОтгрузкаПоЗаказам,
	|	ТаблицаТоваров.ЗаказКлиента,
	|	ТаблицаТоваров.Номенклатура,
	|	ТаблицаТоваров.Характеристика,
	|	ТаблицаТоваров.Серия,
...
Процедура СообщитьОРезультатахПроверкиХозяйственнойОперацииДокумента(Выборка)
	
	Пока Выборка.Следующий() Цикл
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В документе %1 хозяйственная операция %2. Печать сертификатов номенклатуры не требуется.'"),
			Выборка.ПредставлениеСсылки,
			Выборка.ПредставлениеХозяйственнойОперации);
		ОбщегоНазначения.СообщитьПользователю(
			Текст,
			Выборка.Ссылка);
	КонецЦикла;
