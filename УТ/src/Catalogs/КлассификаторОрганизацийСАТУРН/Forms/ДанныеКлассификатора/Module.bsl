...
Процедура Загрузить(Команда)
	...
	ЗагрузитьОрганизацию_ДлительнаяОперация();
...
&НаКлиенте
Процедура ЗагрузитьОрганизацию_ДлительнаяОперация()
	
	ИдентификаторыМестХранения   = Новый Массив;
	ИдентификаторыМестПрименения = Новый Массив;
	
	Для Каждого СтрокаТаблицы Из МестаПримененияКЗагрузке Цикл
		ИдентификаторыМестПрименения.Добавить(СтрокаТаблицы.Идентификатор);
	КонецЦикла;

	Для Каждого СтрокаТаблицы Из МестаХраненияКЗагрузке Цикл
		ИдентификаторыМестХранения.Добавить(СтрокаТаблицы.Идентификатор);
	КонецЦикла;
	
	ПараметрыОбработкиДокументов = ИнтеграцияСАТУРНСлужебныйКлиентСервер.ПараметрыОбработкиДокументов();
	ПараметрыОбработкиДокументов.Ссылка             = ПредопределенноеЗначение("Справочник.КлассификаторОрганизацийСАТУРН.ПустаяСсылка");
	...
	Если ИдентификаторыМестХранения.Количество() Тогда
		ПараметрыОбработкиДокументов.ДополнительныеПараметры.Вставить("ИдентификаторыМестХранения",   ИдентификаторыМестХранения);
	КонецЕсли;
	
	Если ИдентификаторыМестПрименения.Количество() Тогда
		ПараметрыОбработкиДокументов.ДополнительныеПараметры.Вставить("ИдентификаторыМестПрименения", ИдентификаторыМестПрименения);
	КонецЕсли;
...
Процедура Подключаемый_ПриЗавершенииОперации(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого ЭлементДанных Из Результат Цикл
		...
		Для Каждого ОбъектЗагрузки Из ЭлементДанных.Объект Цикл
			...
			Если Не ТипЗнч(ОбъектЗагрузки) = Тип("СправочникСсылка.КлассификаторОрганизацийСАТУРН") Тогда
				Продолжить;
			КонецЕсли;
			
			ОрганизацияСАТУРН = ОбъектЗагрузки;
			Прервать;
	...
	Если Не ЗначениеЗаполнено(ОрганизацияСАТУРН) Тогда
		Возврат;
	КонецЕсли;
	...
	ОбщегоНазначенияКлиент.ОповеститьОбИзмененииОбъекта(ОрганизацияСАТУРН);
