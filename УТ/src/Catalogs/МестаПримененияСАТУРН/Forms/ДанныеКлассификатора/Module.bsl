...
Процедура Загрузить(Команда)
	...
	ЗагрузитьМестоПрименения_ДлительнаяОперация();
...
&НаКлиенте
Процедура ЗагрузитьМестоПрименения_ДлительнаяОперация()
...
Процедура Подключаемый_ПриЗавершенииОперации(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого ЭлементДанных Из Результат Цикл
		...
		Для Каждого ОбъектЗагрузки Из ЭлементДанных.Объект Цикл
			...
			Если Не ТипЗнч(ОбъектЗагрузки) = Тип("СправочникСсылка.МестаПримененияСАТУРН") Тогда
				Продолжить;
			КонецЕсли;
			
			МестоПрименения = ОбъектЗагрузки;
			Прервать;
	...
	Если Не ЗначениеЗаполнено(МестоПрименения) Тогда
		Возврат;
	КонецЕсли;
