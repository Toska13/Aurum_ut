...
Процедура Аннулировать(Команда)
	...
	ДанныеОснования = Новый Структура("Основание, Корректировочный, Аннулирование", Объект.Ссылка, Истина, Истина);
	ОткрытьФорму("Документ.ОформлениеСДИЗЗЕРНО.ФормаОбъекта",
		Новый Структура("Основание", ДанныеОснования), ЭтотОбъект);
...
Процедура ОткрытьЧерезБраузер(Команда)
	...
	Если ЗначениеЗаполнено(Объект.ИдентификаторФГИС)Тогда 
		ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку(
			ИнтеграцияЗЕРНОКлиентСервер.ПутьКСерверуСИнформациейПоСДИЗ(Объект.ИдентификаторФГИС, Объект.РазделУчета));
	Иначе 
