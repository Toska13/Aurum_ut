...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ОбновлениеИнформационнойБазыГосИС.ДобавитьОбработчикОбновленияПереходаНаКонструкторНастроек(
		...
		Новый УникальныйИдентификатор("6ffe98de-e023-47bd-aedb-cb2869415401"));
	...
	ИначеЕсли Операция = Перечисления.ВидыОперацийЗЕРНО.ЗапросСДИЗ Тогда
		...
		Попытка
			...
			Для Каждого СтрокаДанных Из ДанныеДляОбработки Цикл
				...
				ИмяПоляКорректируемыйСДИЗ = ИнтеграцияЗЕРНОСлужебный.ИмяПоляКорректируемыйСДИЗ(ЭлементОчереди.РазделУчета);
				...
				ИмяПоляКорректирующийСДИЗ = ИнтеграцияЗЕРНОСлужебный.ИмяПоляКорректирующийСДИЗ(ЭлементОчереди.РазделУчета);
	...
	ИначеЕсли Операция = Перечисления.ВидыОперацийЗЕРНО.АннулированиеСДИЗ Тогда
		...
		Если ВРег(ПолученныеДанные.resultOperation) = "DONE" Тогда
			...
			Попытка
				...
				СДИЗ = СДИЗ(НомерСДИЗ, ЭлементОчереди.Организация, ЭлементОчереди.Подразделение, ЭлементОчереди.РазделУчета, ПараметрыОбмена);
...
Функция СообщенияЗагрузкиСДИЗ(ПараметрыОбработкиДокумента, ПараметрыОбмена)
	...
	ПараметрыФормирования.РазделУчета      = ПараметрыОбработкиДокумента.РазделУчета;
	...
	Для Каждого СтрокаТаблицы Из ПараметрыСообщений Цикл
		...
		Если ЗначениеЗаполнено(НовыеПараметрыФормирования.НомерСДИЗ) Тогда
			...
			Если ПараметрыФормирования.ЗапрашиватьПогашенияСДИЗ Тогда
				...
				ПараметрыОбработкиСообщений = ИнтеграцияЗЕРНОСлужебныйКлиентСервер.ПараметрыОбработкиСообщений();
				ПараметрыОбработкиСообщений.Ссылка        = ПустаяСсылка();
				ПараметрыОбработкиСообщений.РазделУчета   = НовыеПараметрыФормирования.РазделУчета;
...
Функция СообщениеЗагрузкиСДИЗ(ПараметрыФормирования, ПараметрыОбмена)
	...
	РазделУчета      = ПараметрыФормирования.РазделУчета;
	...
	СообщениеXML.РазделУчета      = РазделУчета;
...
Функция СообщениеЗагрузкиПогашенийСДИЗ(ПараметрыОбработкиДокумента, ПараметрыОбмена)
	...
	РазделУчета      = ПараметрыОбработкиДокумента.РазделУчета;
...
Функция СообщениеАннулированияСДИЗ(ПараметрыОбработкиДокумента, ПараметрыОбмена)
	...
	РеквизитыСДИЗ = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		...
		"РазделУчета, УполномоченноеЛицо, Идентификатор, НомерПартии");
	
	Если РеквизитыСДИЗ.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна Тогда
		ПространствоИмен   = ИнтеграцияЗЕРНОСлужебный.ПространствоИменПоИмениПакетаXDTO("zerno_api_gpb_sdiz", ПараметрыОбмена.ПараметрыОптимизации);
	...
	СообщениеXML = ИнтеграцияЗЕРНОСлужебный.СтруктураСообщенияXML();
	СообщениеXML.Операция         = Операция;
	СообщениеXML.РазделУчета      = РеквизитыСДИЗ.РазделУчета;
	СообщениеXML.ТипСообщения     = Перечисления.ТипыЗапросовИС.Исходящий;
	СообщениеXML.Версия           = 1;
	СообщениеXML.ПараметрыЗапроса = Новый Структура();
	...
	ИмяПоляНомерСДИЗ = ИнтеграцияЗЕРНОСлужебный.ИмяПоляНомерСДИЗ(РеквизитыСДИЗ.РазделУчета);
...
Функция СДИЗ(Идентификатор, Организация, Подразделение, РазделУчета, ПараметрыОбмена) Экспорт
		...
		Попытка
			...
			Если Не ЗначениеЗаполнено(СправочникСсылка) Тогда
				...
				ИнтеграцияЗЕРНОСлужебный.ДобавитьКЗагрузке(
					ПараметрыОбмена, ИмяТаблицы, Идентификатор, СправочникСсылка, Организация, Подразделение, РазделУчета);
			КонецЕсли;
...
Функция ЗагрузитьСДИЗ(ЭлементДанных, Организация, Подразделение, РазделУчета, ПараметрыОбмена, ПрочитаноВерсиейAPI) Экспорт
	...
	СуществующийОбъект.РазделУчета       = РазделУчета;
	...
	ИмяПоляНомерПартии = ИнтеграцияЗЕРНОСлужебный.ИмяПоляНомерПартии(РазделУчета);
	ИмяПоляПартия      = Справочники.РеестрПартийЗЕРНО.ИмяПоляПартия(РазделУчета);
	...
	Если ДанныеПартии <> Неопределено Тогда
		...
		ПараметрыЗагрузкиПартии.РазделУчета     = РазделУчета;
	...
	ЗаполнитьСДИЗПоВидуОперации(
		СуществующийОбъект, ЭлементДанных, Организация, Подразделение, РазделУчета, ПараметрыОбмена, ПрочитаноВерсиейAPI);
...
Процедура ЗаполнитьСДИЗПоВидуОперации(СуществующийОбъект, ЭлементДанных, Организация, Подразделение, РазделУчета, ПараметрыОбмена, ПрочитаноВерсиейAPI)
	...
	Иначе
		...
		Если ЭлементДанных.OperationInRF <> Неопределено Тогда
			...
			ЗаполнитьСведенияПриПриемке(СуществующийОбъект, ДанныеСДИЗ.Acceptance, Организация, Подразделение, РазделУчета, ПараметрыОбмена);
		...
		ЗаполнитьСведенияПриПеревозке(
			СуществующийОбъект, ДанныеСДИЗ.Shipping, Организация, Подразделение, ПараметрыОбмена, РазделУчета, ПрочитаноВерсиейAPI);
...
Процедура ЗаполнитьСведенияПриПриемке(СуществующийОбъект, Сведения, Организация, Подразделение, РазделУчета, ПараметрыОбмена)
	...
	Если РазделУчета = Перечисления.РазделыУчетаЗЕРНО.Зерно
...
Процедура ЗаполнитьСведенияПриПеревозке(СуществующийОбъект, Сведения, Организация, Подразделение, ПараметрыОбмена, РазделУчета, ПрочитаноВерсиейAPI)
