...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбработкаНовостей.РазрешенаРаботаСНовостямиТекущемуПользователю() <> Истина Тогда
		Отказ = Истина;
...
#КонецОбласти
	...
	// Запомнить начальные состояния новостей.
	Для Каждого ТекущаяНовость Из ТаблицаНовостей Цикл
		ТекущаяНовость.ПрочтенаПриОткрытии             = ТекущаяНовость.Прочтена;
		ТекущаяНовость.ОповещениеВключеноПриОткрытии   = ТекущаяНовость.ОповещениеВключено;
		ТекущаяНовость.ДатаНачалаОповещенияПриОткрытии = ТекущаяНовость.ДатаНачалаОповещения;
	КонецЦикла;
...
Функция ЗаписатьИзменениеСостоянияНовостиСервер(Знач лкТаблицаНовостей, Знач лкТекущийПользователь)
	...
	// Здесь для каждой новости может быть свое состояние.
	// Поэтому надо вначале "свернуть" таблицу, чтобы обрабатывать новости пачками.
	ТаблицаНовостейТаблицей = лкТаблицаНовостей.Выгрузить(); // лкТаблицаНовостей - это ДанныеФормыКоллекция.
	СгруппированныеСостоянияНовостей = ТаблицаНовостейТаблицей.Скопировать();
	СгруппированныеСостоянияНовостей.Свернуть("ОповещениеВключено,Прочтена,ДатаНачалаОповещения,ОповещениеВключеноПриОткрытии,ПрочтенаПриОткрытии,ДатаНачалаОповещенияПриОткрытии");
	Для Каждого ТекущееСостояние Из СгруппированныеСостоянияНовостей Цикл
		ПараметрыОтбора = Новый Структура("ОповещениеВключено,Прочтена,ДатаНачалаОповещения,ОповещениеВключеноПриОткрытии,ПрочтенаПриОткрытии,ДатаНачалаОповещенияПриОткрытии");
		ЗаполнитьЗначенияСвойств(ПараметрыОтбора, ТекущееСостояние);
		НовостиПоСостоянию = ТаблицаНовостейТаблицей.НайтиСтроки(ПараметрыОтбора);
		МассивНовостей = Новый Массив;
		Для Каждого ТекущаяНовостьПоСостоянию Из НовостиПоСостоянию Цикл
			МассивНовостей.Добавить(ТекущаяНовостьПоСостоянию.Новость);
		КонецЦикла;

		ПредыдущиеЗначенияСостояний = ОбработкаНовостейКлиентСервер.НовыйОписательЗначенийСостоянияНовости(
			ТекущееСостояние.ПрочтенаПриОткрытии,             // Прочтена
			Неопределено,                                     // Пометка
			ТекущееСостояние.ОповещениеВключеноПриОткрытии,   // ОповещениеВключено
			ТекущееСостояние.ДатаНачалаОповещенияПриОткрытии, // ДатаНачалаОповещения
			Неопределено,                                     // УдаленаИзСпискаНовостей
			Неопределено);                                    // ПрочитатьПозже
		НовыеЗначенияСостояний = ОбработкаНовостейКлиентСервер.НовыйОписательЗначенийСостоянияНовости(
			ТекущееСостояние.Прочтена,             // Прочтена
			Неопределено,                          // Пометка
			ТекущееСостояние.ОповещениеВключено,   // ОповещениеВключено
			ТекущееСостояние.ДатаНачалаОповещения, // ДатаНачалаОповещения
			Неопределено,                          // УдаленаИзСпискаНовостей
			Неопределено);                         // ПрочитатьПозже
		ОбработкаНовостейПереопределяемый.ПослеИзмененияСостоянияНовостей(
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(лкТекущийПользователь),
			МассивНовостей,
