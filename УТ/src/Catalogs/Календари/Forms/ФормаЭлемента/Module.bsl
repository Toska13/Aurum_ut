...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.СпособЗаполнения = Перечисления.СпособыЗаполненияГрафикаРаботы.ПоЦикламПроизвольнойДлины Тогда
		...
		// Проверка пересечения интервалов между днями
		НомераДнейВГрафике = Новый Массив;
		Для каждого Строка Из Объект.ШаблонЗаполнения.НайтиСтроки(Новый Структура("ДеньВключенВГрафик", Истина)) Цикл
			НомераДнейВГрафике.Добавить(Строка.НомерСтроки);
		КонецЦикла;
		
		Расписание = Новый ТаблицаЗначений;
		Расписание.Колонки.Добавить("НомерСтроки");
		Расписание.Колонки.Добавить("НомерДня");
		Расписание.Колонки.Добавить("ВремяНачала");
		Расписание.Колонки.Добавить("ВремяОкончания");
		
		Для каждого Строка Из Объект.РасписаниеРаботы Цикл
			Если НомераДнейВГрафике.Найти(Строка.НомерДня) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
		...
		Расписание.Сортировать("НомерДня, НомерСтроки");
		
		Если ДлинаЦикла <> 0 И Расписание.Количество() > 0 Тогда
			НомерДня = Расписание[0].НомерДня;
			Для Индекс = 0 По Расписание.Количество()-1 Цикл
				Строка = Расписание[Индекс];
				Если Строка.НомерДня = НомерДня Тогда
					НоваяСтрока = Расписание.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка,, "НомерДня");
					НоваяСтрока.НомерДня = НомерДня + ДлинаЦикла;
				Иначе
					Прервать;
				КонецЕсли;
			КонецЦикла;
		...
		Для Индекс = 0 По Расписание.Количество()-1 Цикл
			...
			Если Строка.НомерДня > НомерДня Тогда
				Если ГраницаРасписания > Строка.НомерДня
					Или ГраницаРасписания = Строка.НомерДня
						И ОкончаниеДня > Строка.ВремяНачала Тогда
					ОбщегоНазначения.СообщитьПользователю(
						НСтр("ru = 'Обнаружены пересекающиеся интервалы'"), , "Объект.ШаблонЗаполнения", , Отказ);
					Прервать;
				КонецЕсли;
				
				НомерДня = Строка.НомерДня;
				ГраницаРасписания = Строка.НомерДня;
				ОкончаниеДня = '00010101';
			КонецЕсли;
			
			Если Строка.ВремяНачала < ОкончаниеДня Тогда
				ГраницаРасписания = ГраницаРасписания + 1;
			КонецЕсли;
			
			Если Строка.ВремяНачала = Строка.ВремяОкончания
				Или Не ЗначениеЗаполнено(Строка.ВремяОкончания)
				Или Строка.ВремяНачала > Строка.ВремяОкончания Тогда
				ГраницаРасписания = ГраницаРасписания + 1;
			КонецЕсли;
...
Процедура СпособЗаполненияПриИзменении(Элемент)
	...
	Если Объект.СпособЗаполнения = ПредопределенноеЗначение("Перечисление.СпособыЗаполненияГрафикаРаботы.ПоНеделям") Тогда
		НомерДня = -1;
		ОкончаниеДня = Неопределено;
		УдалитьОставшиесяДни = Ложь;
		СтрокиКУдалению = Новый Массив;
		
		Для Индекс = 0 По Объект.РасписаниеРаботы.Количество()-1 Цикл
			Строка = Объект.РасписаниеРаботы[Индекс];
			...
			Если УдалитьОставшиесяДни
				Или Строка.ВремяНачала > Строка.ВремяОкончания 
					И ЗначениеЗаполнено(Строка.ВремяОкончания)
				Или Строка.ВремяНачала = Строка.ВремяОкончания
				Или ОкончаниеДня <> Неопределено
					И (ОкончаниеДня > Строка.ВремяНачала 
						Или Не ЗначениеЗаполнено(ОкончаниеДня)) Тогда
				СтрокиКУдалению.Добавить(Строка);
				УдалитьОставшиесяДни = Истина;
			КонецЕсли;
...
Функция ПредставлениеРасписанияДня(Форма, НомерДня)
	...
	Для Каждого СтрокаРасписания Из Форма.Объект.РасписаниеРаботы Цикл
		...
		ИначеЕсли СтрокаРасписания.ВремяНачала < СтрокаРасписания.ВремяОкончания Тогда
			СекундИнтервала = СтрокаРасписания.ВремяОкончания - СтрокаРасписания.ВремяНачала;
		ИначеЕсли Не ЗначениеЗаполнено(СтрокаРасписания.ВремяОкончания) Тогда
			СекундИнтервала = КонецДня(СтрокаРасписания.ВремяОкончания) - СтрокаРасписания.ВремяНачала + 1;
		Иначе
			ОсновнаяЧасть = КонецДня(СтрокаРасписания.ВремяНачала) - СтрокаРасписания.ВремяНачала + 1;
			ПереходящаяЧасть = СтрокаРасписания.ВремяОкончания - НачалоДня(СтрокаРасписания.ВремяОкончания);
...
Процедура НачатьЗаполнениеРасписанияДня(НомерДня, ИдентификаторСтрокиШаблона = Неопределено)
	...
	ПереходящиеСмены = Объект.СпособЗаполнения = ПредопределенноеЗначение(
		"Перечисление.СпособыЗаполненияГрафикаРаботы.ПоЦикламПроизвольнойДлины");
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("РасписаниеРаботы", РасписаниеРаботы(КонтекстВыбора.НомерДня));
	ПараметрыФормы.Вставить("ПереходящиеСмены", ПереходящиеСмены);
