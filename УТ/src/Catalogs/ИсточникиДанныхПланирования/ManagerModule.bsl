...
Функция ВыбратьДанные(ПараметрыИсточникаДанных, ПользовательскиеНастройки, ДополнительныеПараметры, КомпоновщикНастроекКомпоновкиДанных = Неопределено) Экспорт
	...
	ПодстановкаОсновногоСпособаОбеспеченияИОстатков(СхемаКомпоновкиДанных);
...
Процедура ПодстановкаОсновногоСпособаОбеспеченияИОстатков(СхемаКомпоновкиДанных)
	...
	Для Каждого НаборДанных Из СхемаКомпоновкиДанных.НаборыДанных Цикл
		...
		Если СтрНайти(НаборДанных.Запрос, ИменаТаблиц.Результат) > 0 Тогда
			
			ОбеспечениеВДокументахСервер.РасчетОстатковДляУстановкиДействий(НаборДанных.Запрос, ИменаТаблиц);
