...
Процедура ПриЗаписи(Отказ)
	...
	ИначеЕсли ИнтеграцияСМаркетплейсамиСИ.ЭтоМаркетплейсСИ(ВидМаркетплейса) Тогда
		...
		Префиксы = ИнтеграцияСМаркетплейсамиСИКлиентСервер.ПрефиксыРегламентныхЗаданий(ВидМаркетплейса);
		
		Если ИзменениеПометкиУдаления = 1 Тогда
			
			УзлыОбмена = ПланыОбмена.МаркетплейсыСИ.НайтиУзлы(Ссылка, Истина);
			Для Каждого УзелОбмена Из УзлыОбмена Цикл
				УзлыОбменаОбъект = УзелОбмена.ПолучитьОбъект();
				УзлыОбменаОбъект.ПометкаУдаления = Истина;
				УзлыОбменаОбъект.Записать();
			КонецЦикла;
		...
		Если ИзменениеПометкиУдаления = -1 Или ИзмененоПредставление Тогда
			Для Каждого Префикс Из Префиксы Цикл
				МенеджерОбъекта.УстановитьНаименованиеРегламентногоЗадания(Префикс.Значение + Идентификатор,
				 ИнтеграцияСМаркетплейсамиСИКлиентСервер.НаименованиеРегламентногоЗадания(ВидМаркетплейса, Префикс.Значение),
				 Наименование);
			КонецЦикла;
		КонецЕсли;
