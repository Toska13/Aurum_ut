...
Процедура ЗаполнитьУчетныеЗаписи()
	...
	Если ВидыТорговыхПлощадок.Количество() > 0 Тогда
		...
		Пока ВыборкаДанных.Следующий() Цикл
			Если ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоOzon(ВыборкаДанных.ВидМаркетплейса)
					Или ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоЯндексМаркет(ВыборкаДанных.ВидМаркетплейса) Тогда
				ПостфиксЛого = "2";
			Иначе
				ПостфиксЛого = "";
			КонецЕсли;
			...
			Элементы.УчетнаяЗапись.СписокВыбора.Добавить(ВыборкаДанных.Ссылка, 
				...
				,
				ИнтеграцияСМаркетплейсамиКлиентСервер.ЛоготипТорговойПлощадки(ВыборкаДанных.ВидМаркетплейса, ПостфиксЛого));
		КонецЦикла;
	КонецЕсли;
