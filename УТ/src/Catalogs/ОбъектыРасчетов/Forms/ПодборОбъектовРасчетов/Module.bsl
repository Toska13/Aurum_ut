...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПодборВОплатуПоЗаявкам = Параметры.ПодборВОплатуПоЗаявкам;
...
Процедура ПоместитьПлатежиВХранилище()
	...
	Если Платежи.Итог("Сумма") < СуммаДокумента И СуммаДокумента > 0 Тогда
		...
		КонецЕсли;
		ДополнительныеКритерии = ОбъектыРасчетовСервер.ДополнительныеКритерииПоиска();
		ДополнительныеКритерии.Контрагент = Контрагент;
		ДополнительныеКритерии.Партнер = СтрокаТаблицы.Партнер;
		СтрокаТаблицы.ОбъектРасчетов = ОбъектыРасчетовСервер.ПолучитьОбъектРасчетовПоСсылке(ОбъектСсылка, Организация, ТипРасчетов, ДополнительныеКритерии);
...
Процедура ЗаполнитьТаблицуПоРасчетамСПартнерами()
	...
	Реквизиты.Вставить("ПодборВОплатуПоЗаявкам", ПодборВОплатуПоЗаявкам);
...
Функция ПолучитьПолнуюСуммуСтроки(ДанныеСтроки)
	...
	Если ДанныеСтроки.ВалютаВзаиморасчетов <> ВалютаДокумента Тогда
		...
		КонецЦикла;
	КонецЕсли;
	Если ДанныеСтроки.ДолгПартнера <> 0 И ПодборДебиторскойЗадолженности
		И ТипРасчетов = ПредопределенноеЗначение("Перечисление.ТипыРасчетовСПартнерами.РасчетыСКлиентом") Тогда
		Возврат Мин(ДанныеСтроки.ДолгПартнера, ДанныеСтроки.КОплате - ДанныеСтроки.Оплачивается) / Коэффициент;
	ИначеЕсли ДанныеСтроки.ДолгПартнера <> 0 И ПодборДебиторскойЗадолженности
		И ТипРасчетов = ПредопределенноеЗначение("Перечисление.ТипыРасчетовСПартнерами.РасчетыСПоставщиком") Тогда
		Возврат ДанныеСтроки.ДолгПартнера / Коэффициент;
	ИначеЕсли ДанныеСтроки.НашДолг <> 0 И НЕ ПодборДебиторскойЗадолженности
		И ТипРасчетов = ПредопределенноеЗначение("Перечисление.ТипыРасчетовСПартнерами.РасчетыСПоставщиком") Тогда
		Возврат Мин(ДанныеСтроки.НашДолг, ДанныеСтроки.КОплате - ДанныеСтроки.Оплачивается) / Коэффициент;
	ИначеЕсли ДанныеСтроки.НашДолг <> 0 И НЕ ПодборДебиторскойЗадолженности
		И ТипРасчетов = ПредопределенноеЗначение("Перечисление.ТипыРасчетовСПартнерами.РасчетыСКлиентом") Тогда
		Возврат ДанныеСтроки.НашДолг / Коэффициент;
...
&НаКлиенте
Процедура ПодбиратьНаСуммуРассчитать()
	Если Не ПодбиратьНаСумму Тогда
