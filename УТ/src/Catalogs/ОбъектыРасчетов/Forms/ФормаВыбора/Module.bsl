...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ГруппаЭлементовСкрыватьСлужебные = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		СписокОбъектовРасчетов.Отбор.Элементы,
		НСтр("ru = 'Скрывать служебные'"),
		ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(
		ГруппаЭлементовСкрыватьСлужебные,
		"ТолькоОстатки",
...
Процедура ПриОткрытии(Отказ)
	...
	Если ХозяйственнаяОперацияВводаОстатков = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВводОстатковПремийКлиентам")
	 ИЛИ ХозяйственнаяОперацияВводаОстатков = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВводОстатковПремийПоставщиков") Тогда
	...
	Если СтрокаТаблицы <> Неопределено Тогда
		...
		Иначе
			Если ВернутьСтруктуру Тогда
				РезультатВыбора = ?(Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.ГруппаОбъектРасчетов,
					РезультатВыбораОбъектаРасчетов(СтрокаТаблицы),
					РезультатВыбораОснованияПлатежа(СтрокаТаблицы));
			Иначе
...
Процедура ОбъектыРасчетовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, "ГруппировкаСтроки") Тогда
		Если ВернутьСтруктуру Тогда
			РезультатВыбора = РезультатВыбораОбъектаРасчетов(СтрокаТаблицы);
		Иначе
			РезультатВыбора = СтрокаТаблицы.ОбъектРасчетов;
		КонецЕсли;
		ОповеститьОВыборе(РезультатВыбора);
...
Процедура ОснованияПлатежаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	СтрокаТаблицы = Элементы.ОснованияПлатежа.ТекущиеДанные;
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, "ГруппировкаСтроки") Тогда
		РезультатВыбора = РезультатВыбораОснованияПлатежа(СтрокаТаблицы);
		ОповеститьОВыборе(РезультатВыбора);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Функция РезультатВыбораОбъектаРасчетов(СтрокаТаблицы)
	...
	РезультатВыбора.Вставить("ОснованиеПлатежа", СтрокаТаблицы.ОснованиеПлатежа);
	...
	РезультатВыбора.Вставить("ЦентрализованныйДоговор", СтрокаТаблицы.ЦентрализованныйДоговор);
	РезультатВыбора.Вставить("КомиссияЧерезКонечныхПокупателей", СтрокаТаблицы.КомиссияЧерезКонечныхПокупателей);
	РезультатВыбора.Вставить("Объект", СтрокаТаблицы.Объект);
	РезультатВыбора.Вставить("ИдентификаторПлатежа", СтрокаТаблицы.ИдентификаторПлатежа);
...
&НаКлиенте
Функция РезультатВыбораОснованияПлатежа(СтрокаТаблицы)
...
// Текст запроса выбора основания платежа из динамического списка.
// 
// Параметры:
...
//  ТипОснования - Число - Возможные значения: 0 Документ, 1 Договор с контрагентом, 2 Договор между организациями.
//  	Отвечает за тиблицу источник записей.
//
...
Функция ТекстЗапросаОснованийПлатежа(ЭтоРасчетыСКлиентом, СтруктураОтбора, ТипОснования)
	...
	Если ТипОснования = 0 Тогда
		...
		Если ЗначениеЗаполнено(СтруктураОтбора.Контрагент) Тогда
			УсловияЗапроса.Добавить("(РеестрДокументов.Контрагент.Ключ В (&Контрагент) ИЛИ &ПоВсемКонтрагентам)");
		КонецЕсли;
	...
	ИначеЕсли ТипОснования = 2 Тогда
		ТекстЗапроса = "
		|ВЫБРАТЬ
		|	Договоры.Ссылка КАК ОснованиеПлатежа,
		...
		|ГДЕ
		|	(Договоры.ОрганизацияПолучатель = &Организация)
		|
		|	И &ДинамическиеУсловия
		|";
		...
		Если ЗначениеЗаполнено(СтруктураОтбора.Контрагент) Тогда
			УсловияЗапроса.Добавить(?(ЭтоРасчетыСКлиентом,
				"(Договоры.ОрганизацияПолучатель В (&Контрагент) ИЛИ &ПоВсемКонтрагентам)",
				"(Договоры.Организация В (&Контрагент) ИЛИ &ПоВсемКонтрагентам)"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриОткрытииНаСервере()
	...
	Если ЗначениеЗаполнено(НачальноеОснованиеПлатежа)
		Или ЗначениеЗаполнено(НачальныйОбъектРасчетов) Тогда
...
Процедура ОбновитьТекстЗапросаОбъектовРасчетов()
	...
	Если НЕ ТолькоСОстатком Или БезОтбораПоТипуРасчетов Тогда
		...
		|	ОбъектыРасчетов.Служебный КАК Служебный,
	...
	Если ВводОстатков Тогда
		...
		ИначеЕсли ХозяйственнаяОперацияВводаОстатков = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВводОстатковПремийПоставщиков") Тогда
			
			МассивУсловий.Добавить("(ВЫРАЗИТЬ(ОбъектыРасчетов.Объект КАК Документ.ПервичныйДокумент).ТипПервичногоДокумента = ЗНАЧЕНИЕ(Перечисление.ТипыПервичныхДокументов.АктПремииПоставщика))");
...
Процедура ОбъектРасчетовСозданиеОбработкаВыбораТипа(Результат, ДополнительныеПараметры)Экспорт 
	Если Результат <> Неопределено Тогда
		...
		ИначеЕсли ХозяйственнаяОперацияВводаОстатков = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВводОстатковЗадолженностиПоставщикам") Тогда
			...
			ЗначенияЗаполнения.Вставить("ЭтоПретензияПоставщику", Истина);
		ИначеЕсли ХозяйственнаяОперацияВводаОстатков = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВводОстатковПремийПоставщиков") Тогда
			ЗначенияЗаполнения.Вставить("ХозяйственнаяОперация", ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ОтражениеПремииПоставщикаРетроБонус"));
			ЗначенияЗаполнения.Вставить("ЭтоПретензияПоставщику", Ложь);
		КонецЕсли;
