...
Процедура ЗаполнитьУчитываемыеВидыПродукции(Параметры) Экспорт
	...
	Запрос.Текст =
		...
		|ВЫБРАТЬ
		|	СоответствиеКлючейОрганизацийИОрганизаций.Организация КАК Организация,
		|	МАКСИМУМ(ЕСТЬNULL(ВладельцыПартий.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаЗЕРНО.Зерно), ЛОЖЬ)) КАК УчетЗерна,
		|	МАКСИМУМ(ЕСТЬNULL(ВладельцыПартий.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна),
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсиюAPI_1_0_9");
	Методы.Вставить("ОбработатьДанныеДляПереходаНаНовуюВерсиюAPI_1_0_9");
