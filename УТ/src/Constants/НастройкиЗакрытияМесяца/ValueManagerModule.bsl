...
Функция ОписаниеГруппПараметровОперации(Операция) Экспорт
	...
	Если Операция = Справочники.ОперацииЗакрытияМесяца.ПустаяСсылка() Тогда
		ОписаниеГруппПараметров = ЗакрытиеМесяцаСервер.ОписаниеГруппПараметровОперацииЗакрытияМесяца();
	ИначеЕсли Операция = Справочники.ОперацииЗакрытияМесяца.РасчетПартийИСебестоимости Тогда
...
Функция ОписаниеПараметровОперации(Операция) Экспорт
	...
	ИначеЕсли Операция = Справочники.ОперацииЗакрытияМесяца.ФормированиеДвиженийПоНДС Тогда
...
Функция ПрочитатьВсеПараметры() Экспорт
	...
	ПрочитатьПараметрыОперации(Справочники.ОперацииЗакрытияМесяца.ПустаяСсылка(), ВсеПараметры);
	
	Выборка = Справочники.ОперацииЗакрытияМесяца.Выбрать();
	Пока Выборка.Следующий() Цикл
		ПрочитатьПараметрыОперации(Выборка.Ссылка, ВсеПараметры);
	КонецЦикла;
...
//	Ключ - элемент Справочники.ОперацииЗакрытияМесяца
