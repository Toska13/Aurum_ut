...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПараметрыСканирования.ИСМП Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("ПечатьЭтикетокИСМППереопределяемый");
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЭтоМаркировкаОстатков
		...
		И Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ВидПродукции) Тогда
	...
	Если Не УказатьСрокГодности
		Или Не ЭтоВидПродукцииСОбязательнымУказаниемСроковГодности(
			ВидПродукции, ПараметрыСканирования) Тогда
		МассивНепроверяемыхРеквизитов.Добавить("ГоденДо");
	КонецЕсли;
	
	Если Не ЗапрашиватьАлкогольнуюПродукцию Тогда
		МассивНепроверяемыхРеквизитов.Добавить("АлкогольнаяПродукция");
	КонецЕсли;
	
	Если Не ЗапрашиватьСправку2 Тогда
		МассивНепроверяемыхРеквизитов.Добавить("Справка2");
	КонецЕсли;
	...
	Если ЭтоЧастичноеВыбытие(ЭтотОбъект) Тогда
		РазрешеноПревышениеЕмкостиУпаковкиПриЧастичномВыбытии = КонструкторИСОбщегоНазначенияКлиентСервер.ЗначенияЭлементаКонструктораПоВидуПродукции(
			ПредопределенноеЗначение("Справочник.ЭлементыКонструктораВидовПродукцииИС.РазрешеноПревышениеЕмкостиУпаковкиПриЧастичномВыбытии"),
			ВидПродукции);
		...
		ИначеЕсли ЧастичноеВыбытиеКоличество = ЕмкостьПотребительскойУпаковки
			И Не РазрешеноПревышениеЕмкостиУпаковкиПриЧастичномВыбытии Тогда
			ЕстьОшибкаКоличества = Истина;
		КонецЕсли;
		...
		Если ЕстьОшибкаКоличества Тогда
			...
			ПредставлениеУпаковки = КонструкторИСМПОбщегоНазначенияКлиентСервер.ПредставлениеВидаУпаковкиПоВидуПродукции(
				ВидПродукции, Перечисления.ВидыУпаковокИС.Потребительская, "Родительный");
			
			ВариантПродажи = КонструкторИСМПОбщегоНазначенияКлиентСервер.ПредставлениеПродажаЧастичногоВыбытияПоВидуПродукции(
				ВидПродукции,
				"Родительный");
			...
			Иначе
				ТекстСообщения = СтрШаблон(
					НСтр("ru = 'При %1 количество должно быть меньше емкости %2.'"),
					ВариантПродажи,
...
Процедура НоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ВидПродукции) Тогда
		...
		Если ТипЗнч(ВидПродукции) = Тип("СправочникСсылка.КлючиВидовПродукцииИС") Тогда
			...
			ИначеЕсли ПараметрыСканирования.ЕГАИС Тогда
				ВидПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(ВидПродукции, "ЕГАИС");
				ВидыПродукции.Добавить(ВидПродукции);
			Иначе
...
Процедура КодМаркировкиПриИзменении(Элемент)
	...
	ПриИзмененииРеквизитаСвязанногоСОстаткомВскрытойУпаковки(ЭтотОбъект);
	НастроитьЭлементыФормыЧастичногоВыбытия(ЭтотОбъект);
...
Процедура АлкогольнаяПродукцияПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(Номенклатура) И ЗначениеЗаполнено(АлкогольнаяПродукция) Тогда
		АлкогольнаяПродукцияПриИзмененииСервер();
	КонецЕсли;
...
&НаСервере
Процедура АлкогольнаяПродукцияПриИзмененииСервер()
	
	ТаблицаСопоставления = Новый ТаблицаЗначений;
	ТаблицаСопоставления.Колонки.Добавить("АлкогольнаяПродукция",
		Новый ОписаниеТипов(
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
				ТипЗнч(АлкогольнаяПродукция))));
	ДанныеСопоставления = ТаблицаСопоставления.Добавить();
	ДанныеСопоставления.АлкогольнаяПродукция = АлкогольнаяПродукция;
	Модуль = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияЕГАИС");
	Модуль.ЗаполнитьСопоставленнуюПродукцию(ТаблицаСопоставления);
	Если ЗначениеЗаполнено(ДанныеСопоставления.Номенклатура) Тогда
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, ДанныеСопоставления);
		НоменклатураПриИзмененииСервер();
	КонецЕсли;
...
Процедура ОбновитьОтображениеПоляКоличество()
	...
	Если ТребуетВзвешивания И (РежимПечатиЭтикеток Или Не ЭтоEANИлиGTIN) Тогда
		...
		Элементы.ПолучитьВес.Видимость = КонструкторИСМПКлиентСервер.ВидПродукцииПоддерживаетПеременноеКоличествоМернойПродукции(ВидПродукции);
	Иначе
...
Процедура ВывестиПоясняющийТекст()
	...
	Если Не РежимПечатиЭтикеток И Не ПустаяСтрока(КодМаркировки) Тогда
		...
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(КодМаркировки, ШрифтыСтиля.ШрифтГиперссылкиГосИС, ЦветаСтиля.ЦветГиперссылкиГосИС,, "СкопироватьКодМаркировкиВБуферОбмена"));
	...
	Если ТипЗнч(Параметры.Номенклатура) = Тип("Массив") Тогда
		...
		МассивСтрок.Добавить(" ");
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(ШтрихкодEAN, ШрифтыСтиля.ШрифтГиперссылкиГосИС, ЦветаСтиля.ЦветГиперссылкиГосИС,, "СкопироватьШтрихкодБуферОбмена"));
		МассивСтрок.Добавить(" ");
...
Процедура ЗаполнитьДанныеФормыПриСозданииНаСервере()
	...
	ЗапрашиватьАлкогольнуюПродукцию = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗапрашиватьАлкогольнуюПродукцию(ПараметрыСканирования);
	ЗапрашиватьСправку2 = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗапрашиватьСправку2(ПараметрыСканирования);
	ЗаполнитьИдентификаторыЕГАИС();
...
Процедура ОбработатьРежимВыбораИзДокумента()
	...
	Если Параметры.РежимПодбораИзДокумента Тогда
		Если Параметры.ДанныеДокумента.Количество() Тогда
			...
			ТаблицаДанныеДокумента.Свернуть(КолонкиСвертки(),"Количество");
...
Процедура ЗаполнитьИдентификаторыПроисхожденияВЕТИС()
	...
	УказатьИдентификаторВЕТИС = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗаполнятьДанныеВЕТИС(ПараметрыСканирования)
		И ЗначениеЗаполнено(КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииПоПодсистеме(ВидПродукции, "ВЕТИС"));
...
&НаСервере
Процедура ЗаполнитьИдентификаторыЕГАИС()
	
	Если ЗапрашиватьАлкогольнуюПродукцию И Не ЗначениеЗаполнено(АлкогольнаяПродукция) Тогда
		
		Модуль = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияЕГАИСКлиентСервер");
		ПараметрыЗаполненияТабличнойЧасти = Модуль.ПараметрыЗаполненияТабличнойЧасти();
		ПараметрыЗаполненияТабличнойЧасти.ЗаполнитьАлкогольнуюПродукцию = Истина;
		
		РеквизитыСопоставления = Новый Структура;
		РеквизитыСопоставления.Вставить("Номенклатура", Номенклатура);
		РеквизитыСопоставления.Вставить("Характеристика", Характеристика);
		РеквизитыСопоставления.Вставить("Серия", Серия);
		РеквизитыСопоставления.Вставить("АлкогольнаяПродукция", АлкогольнаяПродукция);
		РеквизитыСопоставления.Вставить("СопоставлениеАлкогольнаяПродукция", "");
		РеквизитыСопоставления.Вставить("НоменклатураДляВыбора", Новый Массив);
		
		Модуль = ОбщегоНазначения.ОбщийМодуль("СобытияФормЕГАИС");
		Модуль.ЗаполнитьАлкогольнуюПродукциюСтрокиТабличнойЧасти(РеквизитыСопоставления, ПараметрыЗаполненияТабличнойЧасти);
		Элементы.АлкогольнаяПродукция.СписокВыбора.ЗагрузитьЗначения(РеквизитыСопоставления.НоменклатураДляВыбора);
		Если РеквизитыСопоставления.НоменклатураДляВыбора.Количество() = 1 Тогда
			АлкогольнаяПродукция = РеквизитыСопоставления.НоменклатураДляВыбора[0];
		КонецЕсли;
		
	КонецЕсли;
	
	Если ЗапрашиватьСправку2 И Не ЗначениеЗаполнено(Справка2) Тогда
		
		СписокВыбора = РегистрыНакопления["ОстаткиАлкогольнойПродукцииЕГАИС"].Справки2ВНаличии(ПараметрыСканирования.ОрганизацияЕГАИС, АлкогольнаяПродукция);
		Элементы.Справка2.СписокВыбора.ЗагрузитьЗначения(СписокВыбора);
		Если СписокВыбора.Количество() = 1 Тогда
			Справка2 = СписокВыбора[0];
		КонецЕсли;
...
Процедура ЗаполнитьДанныеДляПечатиЭтикеток()
	...
	Если Не РежимПечатиЭтикеток
		И ЗначениеЗаполнено(Параметры.ПолныйКодМаркировки)
	...
	ИначеЕсли ЗначениеЗаполнено(Номенклатура) Тогда
		ВидПродукцииНоменклатуры = ОбщегоНазначенияИС.ОписаниеНоменклатуры(Номенклатура)[Номенклатура].КлючВидовПродукции;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВидПродукцииНоменклатуры) Тогда
		...
		ИначеЕсли КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(ВидПродукцииНоменклатуры, Истина) Тогда
...
Процедура ОбновитьВидимостьДоступностьЭлементов()
	...
	Если УказатьСрокГодности Тогда
		...
		Элементы.ГруппаСрокГодности.ТолькоПросмотр = Ложь;
		Если Не ЭтоВидПродукцииСОбязательнымУказаниемСроковГодности(
			ВидПродукции, ПараметрыСканирования) Тогда
...
Процедура ОбновитьОтображениеПоляНоменклатура()
	...
	Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ВидПродукции) Тогда
	...
	ИначеЕсли ПараметрыСканирования.РазрешенаОбработкаКодовСПустойНоменклатурой
		...
		ДанныеДляПолученияПредставленияGTINОстатков.Колонки.Добавить("ВидПродукции",             Новый ОписаниеТипов("СправочникСсылка.ВидыПродукцииИС"));
...
Процедура УстановитьВидимостьДоступностьЭлементовПриСозданииНаСервере()
	...
	Элементы.ЗаводскойСерийныйНомер.Видимость = РежимПечатиЭтикеток
		И МаркировкаТоваровКлиентСервер.ДоступноПолеЗаводскойСерийныйНомерВнутриКМ(ВидПродукции);
	
	Элементы.КодМаркировки.Видимость = (КоличествоВскрытыхПотребительскихУпаковок > 1);
	
	Элементы.АлкогольнаяПродукция.Видимость = ЗапрашиватьАлкогольнуюПродукцию Или ЗначениеЗаполнено(АлкогольнаяПродукция);
	Элементы.АлкогольнаяПродукция.ТолькоПросмотр = Не ЗапрашиватьАлкогольнуюПродукцию;
	Элементы.Справка2.Видимость             = ЗапрашиватьСправку2;
	Элементы.АлкогольнаяПродукция.АвтоОтметкаНезаполненного = ЗапрашиватьАлкогольнуюПродукцию;
	Элементы.Справка2.АвтоОтметкаНезаполненного             = ЗапрашиватьСправку2;
...
Процедура ОбработатьШтрихкод(ДанныеШтрихкода, ШтрихкодНайден, ТекстОшибки = "")
	...
	ДанныеПоШтрихкодамEANРезультат = ШтрихкодированиеОбщегоНазначенияИС.ДанныеПоШтрихкодамEAN(Штрихкоды);
	...
	Иначе
		Для Каждого СтрокаТЧ Из ДанныеПоШтрихкодамEAN Цикл
			Подходит = КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(СтрокаТЧ.ВидПродукции, Истина)
				Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияЕГАИС(СтрокаТЧ.ВидПродукции);
			Если ПараметрыСканирования.ДопустимыеВидыПродукции.Количество() Тогда
				Если ТипЗнч(СтрокаТЧ.ВидПродукции) = Тип("СправочникСсылка.КлючиВидовПродукцииИС") Тогда
					ВидыПродукцииКлюча = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииКлюча(СтрокаТЧ.ВидПродукции);
					Подходит = Подходит И ОбщегоНазначенияИСКлиентСервер.ПересечениеМассивов(
						ПараметрыСканирования.ДопустимыеВидыПродукции,
						ВидыПродукцииКлюча).Количество();
				Иначе
...
Функция ДопустимыйФорматШтрихкода(ДанныеШтрихкода)
	...
	Если ОбработкаСтрокиXML.НайтиНедопустимыеСимволыXML(ЗначениеШтрихкода) > 0 Тогда
		ДанныеРазбора = РазборКодаМаркировкиИССлужебный.РазобратьКодМаркировки(ЗначениеШтрихкода);
		Если ДанныеРазбора <> Неопределено
			И ДанныеРазбора.СоставКодаМаркировки <> Неопределено
			И ДанныеРазбора.СоставКодаМаркировки.Свойство("GTIN") Тогда
			ЗначениеШтрихкода = ДанныеРазбора.СоставКодаМаркировки.GTIN;
		Иначе
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПервоначальноеЗаполнениеПоЧастичномуВыбытию()
	...
	Если МожетВыбыватьЧастично И ЧастичноеВыбытие
		И ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(ВидПродукции, Истина)
		...
		Если ДанныеПоШтрихкодам.ЕстьОшибки Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(ДанныеПоШтрихкодам.ТекстОшибки);
...
Процедура ОбновитьПараметрыЧастичногоВыбытия(Инициализация = Ложь)
	...
	Если Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(ВидПродукции)
...
Процедура ЗаполнитьОписаниеНоменклатуры()
	...
	Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(ВидПродукции)
...
Процедура ПриИзмененииРежимаВыбытия()
	...
	Если ЧастичноеВыбытиеВариантУчета = Перечисления.ВариантыУчетаЧастичногоВыбытияИС.НастроеннаяНоменклатура Тогда
		...
		ИнтеграцияИСПереопределяемый.ЗаполнитьСлужебныеРеквизитыВКоллекции(ЭтотОбъект, ТаблицаНоменклатуры, ПараметрыСканирования);
...
Процедура ПриИзмененииКоличестваЧастичногоВыбытия()
	...
	Если ВыбытиеБутылкиДоступно
		И КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(ВидПродукции) Тогда
...
Процедура ПриИзмененииРеквизитаСвязанногоСОстаткомВскрытойУпаковки(Форма)

	Если Форма.ЧастичноеВыбытиеОстаток <> Неопределено
		И (Форма.ЧастичноеВыбытиеОстаток - Форма.ЧастичноеВыбытиеКоличество) < 0 Тогда
		Форма.ЧастичноеВыбытиеНеКорректныйОстаток = Истина;
	Иначе
		Форма.ЧастичноеВыбытиеНеКорректныйОстаток = Форма.ЧастичноеВыбытиеНеКорректныйОстатокГИС;
	КонецЕсли;
...
Процедура НастроитьЭлементыФормыЧастичногоВыбытия(Форма)
	...
	Если ЭтоЧастичноеВыбытие Тогда
		...
		Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ИспользоватьЧислительИЗнаменательПриЧастичномВыбытииНаККТ(Форма.ВидПродукции) Тогда
	...
	Если ЭтоЧастичноеВыбытие
		И Форма.ЧастичноеВыбытиеНеКорректныйОстаток
		И (КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(Форма.ВидПродукции, Истина)
			И Форма.ПараметрыСканирования.ВидОперацииИСМП <> ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ВидОперацииРозничныйВозврат()
			Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(Форма.ВидПродукции)
			И Форма.ПараметрыСканирования.ОперацияКонтроляАкцизныхМарок <> "Возврат") Тогда
...
Процедура РасчетОграниченийПолейЧастичногоВыбытия(Форма)
	...
	Если ЗначениеЗаполнено(Форма.ЧастичноеВыбытиеОстаток)
		И Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ПоддерживаетсяПодключениеКОборудованиюРозлива(Форма.ВидПродукции)
		И Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(Форма.ВидПродукции) Тогда
	...
	ИначеЕсли ЗначениеЗаполнено(Форма.ЧастичноеВыбытиеОстатокГИСМТ)
		И Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ПоддерживаетсяПодключениеКОборудованиюРозлива(Форма.ВидПродукции)
		И Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(Форма.ВидПродукции)
		И Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииИСМПЕГАИС(Форма.ВидПродукции) Тогда
...
Процедура ЗакрытьФорму()
	...
	СтруктурыДанных.ДанныеВыбора.АлкогольнаяПродукция            = АлкогольнаяПродукция;
	СтруктурыДанных.ДанныеВыбора.Справка2                        = Справка2;
	...
	Если ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.ОбъемноСортовойУчет") Тогда
		...
		СтруктурыДанных.ДанныеВыбора.КодМаркировки = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ОСУПоШтрихкодуEAN(
			GTIN, СтруктурыДанных.ДанныеВыбора.КоличествоПотребительскихУпаковок);
	ИначеЕсли ЭтоВыборКодаМаркировкиВскрытойПотребительскойУпаковки(ЭтотОбъект) Тогда
		СтруктурыДанных.ДанныеВыбора.ОбработатьБезМаркировки           = Ложь;
		СтруктурыДанных.ДанныеВыбора.ЭтоШтрихкодНоменклатуры           = Ложь;
	...
	Если РежимПечатиЭтикеток Тогда
		Если ТребуетВзвешивания
			И КонструкторИСМПКлиентСервер.КодМаркировкиМожетВключатьВес(ВидПродукции)
		...
		ИначеЕсли ТребуетВзвешивания
			И КонструкторИСМПКлиентСервер.КодМаркировкиМожетВключатьОбъем(ВидПродукции)
			И ПараметрыСканирования.ЗапрашиватьКоличествоМерногоТовара Тогда
			СтруктурыДанных.ДополнитьПолныйКодМаркировки = "3353";
		ИначеЕсли КонструкторИСМПКлиентСервер.КодМаркировкиМожетВключатьЗаводскойСерийныйНомер(ВидПродукции)
...
Функция ЗакрытьПослеСканированияШтрихкодаНоменклатуры()
	...
	ИначеЕсли УказатьСрокГодности
		И Не ЗначениеЗаполнено(ГоденДо) Тогда
		Возврат Ложь;
	ИначеЕсли ЗапрашиватьАлкогольнуюПродукцию
		И Не ЗначениеЗаполнено(АлкогольнаяПродукция) Тогда
		Возврат Ложь;
	ИначеЕсли ЗапрашиватьСправку2
		И Не ЗначениеЗаполнено(Справка2) Тогда
		Возврат Ложь;
...
Процедура ОбновитьДанныеGTIN()
	...
	Если ЗначениеЗаполнено(ВидПродукции)
		И (КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетНаборы(ВидПродукции)
		Или КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетГрупповыеУпаковки(ВидПродукции)) Тогда
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоВидПродукцииСОбязательнымУказаниемСроковГодности(ВидПродукции, ПараметрыСканирования)
	
	ОперацияНанесения = Неопределено;
	ПараметрыСканирования.Свойство("ОперацияНанесения", ОперацияНанесения);
	Если ЗначениеЗаполнено(ОперацияНанесения) Тогда
		ВидОперации = Новый Массив;
		ВидОперации.Добавить(ОперацияНанесения);
		ВидОперации.Добавить(ПараметрыСканирования.ВидОперацииИСМП);
	Иначе
		ВидОперации = ПараметрыСканирования.ВидОперацииИСМП;
	КонецЕсли;
	
	Возврат КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСОбязательнымУказаниемСроковГодности(
		ВидПродукции, ВидОперации);
...
Функция ЗаписатьДанныеRFIDВИнформационнуюБазу(TID, КодМаркировки, ДекодированныеДанные)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(КодМаркировки)
			И Не ЗначениеЗаполнено(ЗаписьНабора.ХешСуммаЗначенияШтрихкода) Тогда
			ЗаписьНабора.ХешСуммаЗначенияШтрихкода = ОбщегоНазначенияИС.ХэшСуммаСтроки(КодМаркировки);
		КонецЕсли;
