...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НачислениеНДСПриОтгрузке = ?(Параметры.Свойство("НачислениеНДСПриОтгрузке"), Параметры.НачислениеНДСПриОтгрузке, Ложь);
...
Процедура ЗаполнитьТаблицуКОформлению()
	...
	|	ГДЕ
	|		ДанныеРегистра.ТипСчетаФактуры = &ТипСчетаФактуры
	...
	Если Не НачислениеНДСПриОтгрузке Тогда 
		ТекстУсловий = ТекстУсловий + "
	|	И ТИПЗНАЧЕНИЯ(ДанныеРегистра.Основание) <> ТИП(Документ.ОтгрузкаТоваровКлиенту)";
	КонецЕсли;
