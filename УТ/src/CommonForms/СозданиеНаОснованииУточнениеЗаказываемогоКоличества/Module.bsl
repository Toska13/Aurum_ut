...
Процедура ТоварыКоличествоУпаковокПриИзменении(Элемент)
	...
	СтрокаТовары.СчетОтметок = СтрокаТовары.Отметка;
	СтраницыВыбраноСтрокПереключить();
...
Процедура ЗаполнитьТоварыПоЗначениюПереключателяВсеИзДокументаНеОбеспечено()
	...
	ПараметрыСоздания.ИмяМенеджераДокумента = Параметры.ИмяМенеджераДокумента;
	...
	Если ВсеИзДокументаНеОбеспечено = 0 Тогда // заполнение все из документа
		...
		Текст =
			...
			|	МИНИМУМ(ВЫРАЗИТЬ(Товары.Номенклатура КАК Справочник.Номенклатура).ТипНоменклатуры) КАК ТипНоменклатуры,
	...
	Иначе // заполнение не обеспечено
		...
		Текст =
			...
			|	Товары.Подразделение КАК Подразделение,
			|	ВЫБОР
			|		КОГДА ВЫРАЗИТЬ(Товары.Номенклатура КАК Справочник.Номенклатура).ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
		...
		Текст =
			...
			|	ЗапасыИПотребности.РезервироватьПоМереПоступленияИзЗаблокированныхОстаток
			|		+ ЗапасыИПотребности.РезервироватьПоМереПоступленияОстаток
			|		+ ЗапасыИПотребности.КОбеспечениюОстаток
			|		+ ЗапасыИПотребности.ОтложитьРезервированиеОстаток КАК Требуется,
			|	ВЫБОР
			...
			|				Товары.Назначение КАК Назначение
			|			ИЗ
			|				ТоварыСгруппированные КАК Товары)) КАК ЗапасыИПотребности
			|ИНДЕКСИРОВАТЬ ПО
		...
		Текст =
			...
			|	Товары.Номенклатура КАК Номенклатура,
			|	ВЫРАЗИТЬ(Товары.Номенклатура КАК Справочник.Номенклатура).ТипНоменклатуры КАК ТипНоменклатуры,
			|	Товары.Характеристика КАК Характеристика,
			|	Товары.Упаковка КАК Упаковка,
			|	Товары.Склад КАК Склад,
			|	Товары.Подразделение КАК Подразделение,
			...
			|	ВЫБОР
			|		КОГДА ЕСТЬNULL(Потребности.Требуется - Потребности.Обеспечено, 0) > 0
			|			ТОГДА 1
			|		ИНАЧЕ 0
			|	КОНЕЦ КАК СчетОтметок,
	...
	СтраницыВыбраноСтрокПереключитьНаСервере();
...
&НаСервере
Процедура СтраницыВыбраноСтрокПереключитьНаСервере()
	
	СчетОтметок = Товары.Итог("СчетОтметок");
	Если СчетОтметок = 0 Тогда
		Элементы.СтраницыВыбраноСтрок.ТекущаяСтраница = Элементы.СтраницаВыбраноСтрок0;
	Иначе
		Элементы.СтраницыВыбраноСтрок.ТекущаяСтраница = Элементы.СтраницаВыбраноСтрокНе0;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура СтраницыВыбраноСтрокПереключить()
