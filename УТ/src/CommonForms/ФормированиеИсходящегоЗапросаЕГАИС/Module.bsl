...
Процедура ПриОткрытии(Отказ)
	...
	Если Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросДанныхОрганизации")
		И ЗначениеЗаполнено(ОрганизацияЕГАИС)
		И ЗначениеЗаполнено(ЗначениеПараметра)
		И Параметры.Свойство("ЗапросДанныхПриОткрытии")
		Тогда
		ПодключитьОбработчикОжидания("ЗапроситьДанныеОрганизацииЕГАИСПриОткрытии", 0.5, Истина);
	КонецЕсли;
...
Процедура ПриЗавершенииОбмена(Изменения, ДополнительныеПараметры) Экспорт
	...
	Если Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросДанныхОрганизации") Тогда
		...
		Оповестить("Запись_КлассификаторОрганизацийЕГАИС", МассивОрганизацийЕГАИС[0]);
	Иначе
...
Функция ТекстПакетаЗапросов()
	...
	Для Каждого СтрокаТаблицы Из ОбработанныеЗапросы Цикл
		...
		ЗапросовОбработано = ЗапросовОбработано + 1;
	КонецЦикла;
...
&НаКлиенте
Процедура ЗапроситьДанныеОрганизацииЕГАИСПриОткрытии()
