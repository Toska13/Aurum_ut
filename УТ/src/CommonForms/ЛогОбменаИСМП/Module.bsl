...
Процедура ЗаполнитьПоИсточнику(Источник)
	...
	ИначеЕсли ТипЗнч(Источник) = Тип("Структура") Тогда
		...
		ТекстСообщения = Источник.ПротоколОбменаXML;
		ПротоколОбменаИСМП.ЗаменитьИменаУстаревшихПеречисленийВТекстеПротоколаОбмнена(ТекстСообщения);
	...
	Иначе
		...
		Если Выборка.Следующий() Тогда
			ЗаполнитьЗначенияСвойств(ЭтотОбъект, Выборка, "ДатаСоздания, Документ, Операция, ИдентификаторЗапроса");
			ТекстСообщения = ПротоколОбменаИС.ТекстСообщенияXMLИзПротокола(Источник);
			ПротоколОбменаИСМП.ЗаменитьИменаУстаревшихПеречисленийВТекстеПротоколаОбмнена(ТекстСообщения);
			ПротоколОбмена = ОбщегоНазначения.ЗначениеИзСтрокиXML(ТекстСообщения)
		КонецЕсли;
