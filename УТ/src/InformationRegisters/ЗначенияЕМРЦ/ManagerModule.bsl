...
Процедура ЗаполнитьЗначенияРегистраЕМРЦ() Экспорт
	...
	Попытка
		...
		НоваяСтрока = НаборЗаписей.Добавить();
		НоваяСтрока.ОсобенностьУчета = Перечисления.ВидыМаркированнойПродукцииБПО.Табак; 
		НоваяСтрока.Период           = Дата('20250301');
		НоваяСтрока.Цена             = 135;
		...
		НоваяСтрока = НаборЗаписей.Добавить();
		НоваяСтрока.ОсобенностьУчета = Перечисления.ВидыМаркированнойПродукцииБПО.Табак; 
		НоваяСтрока.Период           = Дата('20260301');
		НоваяСтрока.Цена             = 153;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьЗначенияРегистраЕМРЦ", Истина);
