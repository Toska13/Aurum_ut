...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЕстьПравоДобавления = ЕстьПравоИзменения И МашиночитаемыеДоверенности.ЕстьПравоДобавления();
	Элементы.ЗагрузитьИзРеестраФНС.Видимость = ЕстьПравоИзменения;
	Элементы.ЗагрузитьИзФайла.Видимость = ЕстьПравоИзменения;
	Элементы.ГруппаСоздать.Видимость = ЕстьПравоДобавления;
	...
	РежимОткрытияОкна = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "РежимОткрытияОкна", РежимОткрытияОкна);
	УстановитьОтборДинамическогоСпискаМЧД();
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = МашиночитаемыеДоверенностиКлиент.ИмяОповещенияИзмененаПометкаУдаленияМЧД() Тогда
		Элементы.Список.Обновить();	
	КонецЕсли;	
...
Процедура РекомендацииПодписьМЧДНажатие(Элемент)
	...
	Если Не ЗначениеЗаполнено(СтатусыДоверенности.ТекстОшибкиПроверкиМЧД) Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыОшибкиКриптографии = Новый Структура();
	ПараметрыОшибкиКриптографии.Вставить("КраткоеПредставлениеОшибки", СтатусыДоверенности.ТекстОшибкиПроверкиМЧД);
	ПараметрыОшибкиКриптографии.Вставить("ЗаголовокПроблемы", НСтр("ru = 'Проверка подписи машиночитаемой доверенности'"));
	ПараметрыОшибкиКриптографии.Вставить("ДанныеПодписи", СтатусыДоверенности.ДанныеПодписи);
	ПараметрыОшибкиКриптографии.Вставить("ДанныеСертификата", СтатусыДоверенности.ДанныеСертификата);
	ПараметрыОшибкиКриптографии.Вставить("Сертификат", СтатусыДоверенности.Сертификат);
...
Процедура СписокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, КлючЗаписиИСтрокиДинамическогоСписка)
	...
	Для Каждого КлючИЗначение Из КлючЗаписиИСтрокиДинамическогоСписка Цикл
		...
		Если ПустаяСтрока(СтрокаСписка.Данные.СтатусВРеестреФНС) Тогда
			СтрокаСписка.Данные.СтатусВРеестреФНС = СтрокаСписка.Данные.Доверенность.СтатусВРеестреФНС;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(СтрокаСписка.Данные.Номер) Тогда
			КоличествоПередоверенныхМЧД = 
				КоличествоПередоверенныхМЧД(СтрокаСписка.Данные.Номер, КлючЗаписиИСтрокиДинамическогоСписка);
		Иначе
			КоличествоПередоверенныхМЧД = 0;
		КонецЕсли;
		...
		ИначеЕсли КоличествоПередоверенныхМЧД > 0 Тогда
			ТекстПередоверия = НСтр("ru='Передоверена %1 раз(а)'");
			ТекстПередоверия = СтрШаблон(ТекстПередоверия, КоличествоПередоверенныхМЧД);
...
&НаСервереБезКонтекста
Функция КоличествоПередоверенныхМЧД(НомерДоверенности, СтрокиДинамическогоСписка)
...
Процедура ОбновитьИспользованиеПараметровСписка()
	...
	УстановитьДополнительныеПараметрыДинамическогоСписка();
...
Процедура ОбновитьОтборПоОрганизации()
	...
	Если ЗначениеЗаполнено(ВыбраннаяОрганизация) Тогда
		...
		|			И НЕ ОчередьЗапросовРРПереопределяемый.ВидЗапроса ЕСТЬ NULL)
		|			ТОГДА ВЫБОР
		|					КОГДА ОчередьЗапросовРРПереопределяемый.ВидЗапроса = &ТекстРегистрацияВРР
		|						ТОГДА &ТекстВОчередиНаРегистрацию
		|					ИНАЧЕ &ТекстНеЗарегистрирована
		|				КОНЕЦ
		|		ИНАЧЕ ВЫБОР
		|				КОГДА ОчередьЗапросовРРПереопределяемый.ВидЗапроса = &ТекстОтзывВРР
		|					ТОГДА &ТекстВОчередиНаОтзыв
		|				ИНАЧЕ &ТекущийСтатусВРеестреФНС
		|			КОНЕЦ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОчередьЗапросовВРеестрМЧДЭДО КАК ОчередьЗапросовРРПереопределяемый
		|		ПО ЖурналМашиночитаемыхДоверенностейПереопределяемый.Доверенность = ОчередьЗапросовРРПереопределяемый.Доверенность
		|{ГДЕ
...
Процедура УстановитьУсловноеОформление()
	...
	ОформлениеОшибкиРегистрации = УсловноеОформление.Элементы.Добавить();
	ПоляСписка = ОформлениеОшибкиРегистрации.Поля.Элементы.Добавить();
	ПоляСписка.Поле = Новый ПолеКомпоновкиДанных(Элементы.СтатусВРеестреФНС.Имя);
	УсловиеОшибкаРегистрации = ОформлениеОшибкиРегистрации.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	...
	ОформлениеОшибкиРегистрации.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветОтрицательногоЧисла);
...
// Получает структуру для заполнения формы предварительного просмотра доверенности
//
// Параметры:
...
//  * ОписаниеОшибкиРегистрации - Строка, Неопределено -
//  * ДатаОбновленияСтатуса - Дата -
//  * ТекстОшибкиПроверкиМЧД - Строка -
//  * ДатаОтзыва - Дата -
//  * Отозвана - Булево -
//  * ТекстОшибкиПроверкиМЧДКлассифицирован - Булево -
...
Функция ПолучитьСтатусыДоверенности(Ссылка)
	...
	Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.МЧД003") Тогда
		СтатусыДоверенности = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка,
			"Подписана, Верна, СтатусВРеестреФНС, ДатаПрекращения, ОписаниеОшибкиРегистрации, ДатаОбновленияСтатуса, ТекстОшибкиПроверкиМЧД");
	...
	Иначе
		СтатусыДоверенности = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка,
			"Подписана, Верна, СтатусВРеестреФНС, ДатаОтзыва, Отозвана, ДатаОбновленияСтатуса, ТекстОшибкиПроверкиМЧД");
		СтатусыДоверенности.Вставить("ОписаниеОшибкиРегистрации", Неопределено);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СтатусыДоверенности.ТекстОшибкиПроверкиМЧД) Тогда
		ТекстОшибкиПроверкиМЧДКлассифицирован =
			Не ЭлектроннаяПодпись.ОшибкаПоКлассификатору(СтатусыДоверенности.ТекстОшибкиПроверкиМЧД) = Неопределено;
		СтатусыДоверенности.Вставить("ТекстОшибкиПроверкиМЧДКлассифицирован", ТекстОшибкиПроверкиМЧДКлассифицирован);
	Иначе
		СтатусыДоверенности.Вставить("ТекстОшибкиПроверкиМЧДКлассифицирован", Ложь);
	КонецЕсли;
	
	Если СтатусыДоверенности.Подписана И СтатусыДоверенности.ТекстОшибкиПроверкиМЧДКлассифицирован Тогда
		ДанныеФайла = МашиночитаемыеДоверенности.ДанныеФайлаДоверенностиИПодписи(Ссылка);
		СтатусыДоверенности.Вставить("ДанныеПодписи", ДанныеФайла.ДанныеПодписи);
		СвойстваПодписи = ЭлектроннаяПодпись.СвойстваПодписи(ДанныеФайла.ДанныеПодписи, Истина);
		СтатусыДоверенности.Вставить("ДанныеСертификата", СвойстваПодписи.Сертификат);
		СтатусыДоверенности.Вставить("Сертификат", ЭлектроннаяПодпись.СсылкаНаСертификат(СвойстваПодписи.Отпечаток));
	Иначе
		СтатусыДоверенности.Вставить("ДанныеПодписи", Неопределено);
		СтатусыДоверенности.Вставить("ДанныеСертификата", Неопределено);
		СтатусыДоверенности.Вставить("Сертификат", Неопределено);
	КонецЕсли;
...
&НаСервере
Процедура УстановитьДополнительныеПараметрыДинамическогоСписка() 

	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Список, "ТекстРегистрацияВРР"		, НСтр("ru='Регистрация в РР'"));
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Список, "ТекстВОчередиНаРегистрацию"	, НСтр("ru='В очереди на регистрацию'"));
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Список, "ТекстНеЗарегистрирована"	, НСтр("ru='Не зарегистрирована'"));
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Список, "ТекстОтзывВРР"				, НСтр("ru='Отзыв в РР'"));
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Список, "ТекстВОчередиНаОтзыв"		, НСтр("ru='В очереди на отзыв'"));
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Список, "ТекущийСтатусВРеестреФНС"	, "");
	
КонецПроцедуры

&НаСервере
Процедура УстановитьОтборДинамическогоСпискаМЧД()  
	
	ДоступныеПоляОтбора = Список.Отбор.ДоступныеПоляОтбора;
	
	Организация = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры.ДополнительныеПараметры, 
		"Организация");
	
	Сертификат = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры.ДополнительныеПараметры, 
		"СертификатКриптографии");
		
	ЭлементОтбораДоверенность = ДоступныеПоляОтбора.НайтиПоле(Новый ПолеКомпоновкиДанных("Доверенность"));
	Если ЭлементОтбораДоверенность <> Неопределено 
		И ЗначениеЗаполнено(Сертификат) 
		И ЗначениеЗаполнено(Организация) Тогда  
		
		НовыйОтбор = МашиночитаемыеДоверенности.НовыйОтборМЧД();
		НовыйОтбор.Доверитель = Организация;
		НовыйОтбор.Сертификат = Сертификат;
		Доверенности = МашиночитаемыеДоверенности.ПолучитьДоверенностиОрганизации(НовыйОтбор);
		
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, 
			"Доверенность", Доверенности, ВидСравненияКомпоновкиДанных.ВСписке,, Истина); 
