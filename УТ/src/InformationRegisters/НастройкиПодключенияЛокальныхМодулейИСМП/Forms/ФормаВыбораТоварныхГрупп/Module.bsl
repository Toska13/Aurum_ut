...
Процедура ЗаполнитьСписокТоварныхГруппСУчетомВыбранных()
	...
	ТоварныеГруппыПроверки = Новый Массив;
	Для Каждого ВидПродукции Из КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииОбязательнойОнлайнПроверкиПередРозничнойПродажейНаДату() Цикл
		ТоварныеГруппыПроверки.Добавить(КонструкторИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппаПоВидуПродукции(ВидПродукции));
	КонецЦикла;
	
	Для Каждого ТоварнаяГруппа Из ТоварныеГруппыПроверки Цикл
		СписокТоварныхГрупп.Добавить(
			ТоварнаяГруппа,
			Строка(ТоварнаяГруппа),
			ВыбранныеТоварныеГруппы.НайтиПоЗначению(ТоварнаяГруппа) <> Неопределено);
	КонецЦикла;
