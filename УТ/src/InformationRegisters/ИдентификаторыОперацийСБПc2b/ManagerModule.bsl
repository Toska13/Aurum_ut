...
Функция ОтложенныеОперации() Экспорт
	...
	Запрос.Текст =
		...
		|	ИдентификаторыОперацийСБПc2b.ПараметрыАктивации КАК ПараметрыАктивации,
		|	ВЫБОР
		|		КОГДА ИдентификаторыОперацийСБПc2b.ДокументОперации ССЫЛКА Документ.ПлатежнаяСсылкаСБП
		|			ТОГДА ВЫРАЗИТЬ(ИдентификаторыОперацийСБПc2b.ДокументОперации КАК Документ.ПлатежнаяСсылкаСБП).ОснованиеПлатежа
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	Пока ВыборкаИдентификаторУчастника.Следующий() Цикл
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			ДанныеОперации.Вставить("ДокументОснование", ВыборкаДетальныеЗаписи.ДокументОснование);
...
//
// Возвращаемое значение:
//  Строка, Неопределено - идентификатор участника СБП. Неопределено если данные по документу не найдены.
//
...
Функция ДанныеОпераций(ДокументыОпераций) Экспорт
	...
	Запрос.Текст =
		...
		|		ПО ИдентификаторыОперацийСБПc2b.Идентификатор = ДанныеОперацийСБПc2b.Идентификатор
		|ГДЕ
		|	ИдентификаторыОперацийСБПc2b.ДокументОперации В(&ДокументыОпераций)";
...
Функция ОперацииНастройкиПодключенияЗаПериод(
	...
	ПараметрыНастройки = СистемаБыстрыхПлатежейСлужебный.ПараметрыНастройкиПодключения(
		НастройкаПодключения);
	...
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(ВТ_ДокументыОпераций.ДокументОперации) <> ТИП(Документ.ПлатежнаяСсылкаСБП)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("НачалоПериода", НачалоПериода);
	Запрос.УстановитьПараметр("НастройкаПодключения", НастройкаПодключения);
	Запрос.УстановитьПараметр("ИдентификаторУчастника", ПараметрыНастройки.ИдентификаторУчастника);
