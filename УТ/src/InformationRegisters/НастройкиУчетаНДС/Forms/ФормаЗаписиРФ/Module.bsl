...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОсновноеНалогообложение = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС Тогда 
		...
		Если Запись.ВариантУчетаНДСПриИзмененииВидаДеятельности = Перечисления.ВариантыУчетаНДСПриИзмененииВидаДеятельностиНаНеоблагаемую.ВключатьВСтоимость Тогда
			ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(МассивПроверяемыхПолей, "СтатьяРасходовНеНДС");
		КонецЕсли;
	Иначе
	...
	Если НЕ Запись.РаздельныйУчетТоваровПоНалогообложениюНДС Тогда
		ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(МассивПроверяемыхПолей, "СтатьяРасходовСписаниеНДС");
	КонецЕсли;
...
Процедура ПолучитьПараметрыУчетнойПолитикиИСистемыНалогообложения(ПериодЗаписи = Неопределено)
	...
	Если НЕ ИспользуетсяСреднескользящаяВПериодеДействияНастроекНДС Тогда
		ИспользуетсяСреднескользящаяВПериодеДействияНастроекНДС =
			РегистрыСведений.НастройкиУчетаНДС.ИспользуетсяСреднескользящаяВПериодеДействияНастроекНДС(Организация, ПериодЗаписи);
	КонецЕсли;
...
&НаСервере
Процедура ОткрытьИсториюИзмененийЗавершениеСервер(Результат)
...
Процедура УстановитьСвойстваЭлементов(Форма)
	...
	Элементы.ГруппаРаздельныйУчет.Видимость = Запись.РаздельныйУчетТоваровПоНалогообложениюНДС;
	Элементы.ГруппаОсновная.ОтображатьЗаголовок = Запись.РаздельныйУчетТоваровПоНалогообложениюНДС;
	
	Если НЕ ПрименяетсяЛьготноеНалогообложениеНДСнаУСН
		И Запись.РаздельныйУчетТоваровПоНалогообложениюНДС
		И НЕ Запись.ПрименяетсяОсвобождениеОтУплатыНДС Тогда
	...
	Элементы.ФормироватьНДСПредъявленныйПриВключенииВСтоимость.Доступность =
		...
		И НЕ Запись.ПрименяетсяОсвобождениеОтУплатыНДС
		И НЕ Форма.ИспользуетсяМетодОценкиСтоимостиТоваровПоСреднескользящей;
	Элементы.ГруппаФормироватьНДСПредъявленныйПриВключенииВСтоимостьКомментарий.Видимость =
	...
	Элементы.СтатьяРасходовСписаниеНДС.Видимость                           = Элементы.СтатьяРасходовСписаниеНДС.Видимость
		И НЕ ПрименяетсяЛьготноеНалогообложениеНДСнаУСН;
	...
	Элементы.СписыватьНДСПоРасходамНеПринимаемымВНУ.Доступность =
		НЕ Запись.ПрименяетсяОсвобождениеОтУплатыНДС;
		
	Форма.ТолькоПросмотр = Форма.ТолькоПросмотр Или Форма.ОбособленноеПодразделение;
	
	Элементы.СтатьяЗатратНеНДС.АвтоОтметкаНезаполненного = Ложь;
	Элементы.СтатьяЗатратНеНДС.ОтметкаНезаполненного = Запись.ВариантУчетаНДСПриИзмененииВидаДеятельности <>
		ПредопределенноеЗначение("Перечисление.ВариантыУчетаНДСПриИзмененииВидаДеятельностиНаНеоблагаемую.ВключатьВСтоимость");
