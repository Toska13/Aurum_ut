...
Функция КомплектПечатныхФорм(ТипОбъекта, Знач МассивСсылок = Неопределено, ВариантИспользования = 0) Экспорт
	...
	ТаблицаКоманд = ДополнительныеОтчетыИОбработки.ДополнительныеКоманды(Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма, ТипОбъекта, Истина);
...
//  * ЕстьОрганизация       - Булево - признак наличия организации в объекте.
//  * ЕстьПартнер           - Булево - признак наличия партнера в объекте.
//  * ЕстьУсловияДобавления - Булево - признак наличия условий добавления команд печати "Комплект документов на принтер"
//                                     и "Комплект документов с настройкой..". Если Истина, то в модуле менеджера
//                                     объекта должна быть размещена экспортная функция ДобавлятьКомандыПечатиКомплектаДокументов,
//                                     которая возвращает значение Тип("Булево") - результат выполнения условий.
//  * ЕстьУсловияВидимости  - Булево - признак наличия условий видимости команд печати "Комплект документов на принтер"
//                                     и "Комплект документов с настройкой.." . Если Истина, то в модуле менеджера объекта
//                                     должна быть размещена экспортная процедура ПриОпределенииКомандыПечатиКомплектаДокументов,
//                                     в которой реализовано добавление условий видимости команды методом. См. ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды.
//
Функция ТаблицаОбъектовДляПечатиКомплектно() Экспорт
	...
	ТаблицаОбъектов.Колонки.Добавить("ЕстьУсловияДобавления");
	ТаблицаОбъектов.Колонки.Добавить("ЕстьУсловияВидимости");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьЗаказыКлиентов") Тогда
		...
		НоваяСтрока.ЕстьУсловияДобавления = Истина;
		НоваяСтрока.ЕстьУсловияВидимости = Ложь;
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьДоверенностиНаПолучениеТМЦ") Тогда
		...
		НоваяСтрока.ЕстьОрганизация = Истина;
		НоваяСтрока.ЕстьПартнер = Истина;
		НоваяСтрока.ЕстьУсловияДобавления = Ложь;
		НоваяСтрока.ЕстьУсловияВидимости = Ложь;
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5") Тогда
		НоваяСтрока = ТаблицаОбъектов.Добавить();
		НоваяСтрока.ТипОбъекта = Метаданные.Документы.ОтгрузкаТоваровКлиенту.ПолноеИмя();
		НоваяСтрока.Представление = Метаданные.Документы.ОтгрузкаТоваровКлиенту.Представление();
		НоваяСтрока.ЕстьОрганизация = Истина;
		НоваяСтрока.ЕстьПартнер = Истина;
		НоваяСтрока.ЕстьУсловияДобавления = Истина;
		НоваяСтрока.ЕстьУсловияВидимости = Ложь;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПередачуНаОтветственноеХранениеСПравомПродажи") Тогда
		...
		НоваяСтрока.ЕстьПартнер = Истина;
		НоваяСтрока.ЕстьУсловияДобавления = Истина;
		НоваяСтрока.ЕстьУсловияВидимости = Истина;
	КонецЕсли;
	...
	ИспользоватьВыкупТоваровХранителем =
						ПолучитьФункциональнуюОпцию("ИспользоватьПередачуНаОтветственноеХранениеСПравомПродажи");
	
	Если ИспользоватьВыкупТоваровХранителем Тогда
		НоваяСтрока = ТаблицаОбъектов.Добавить();
		НоваяСтрока.ТипОбъекта = Метаданные.Документы.ВыкупТоваровХранителем.ПолноеИмя();
		НоваяСтрока.Представление = Метаданные.Документы.ВыкупТоваровХранителем.Представление();
		НоваяСтрока.ЕстьОрганизация = Истина;
		НоваяСтрока.ЕстьПартнер = Истина;
		НоваяСтрока.ЕстьУсловияДобавления = Истина;
		НоваяСтрока.ЕстьУсловияВидимости = Ложь;
...
Процедура ПриОпределенииВидовПодключаемыхКоманд(ВидыПодключаемыхКоманд) Экспорт
	...
	ВидПечать = ВидыПодключаемыхКоманд.Найти("Печать", "Имя");
	Индекс = ВидыПодключаемыхКоманд.Индекс(ВидПечать);
	Вид = ВидыПодключаемыхКоманд.Вставить(Индекс);
	ЗаполнитьЗначенияСвойств(Вид, ВидПечать);
	Вид.Имя = "ПечатьКомплектаДокументовСНастройкой";
...
Процедура ПриОпределенииСоставаНастроекПодключаемыхОбъектов(НастройкиПрограммногоИнтерфейса) Экспорт
	...
	Настройка.Ключ          = "ДобавитьПечатьКомплектаДокументовСНастройкой";
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	ПараметрыИсточников = Новый Соответствие;
	
	ТаблицаОбъектовДляПечатиКомплектно = ТаблицаОбъектовДляПечатиКомплектно();
	Для Каждого Строка Из ТаблицаОбъектовДляПечатиКомплектно Цикл
		ИмяТипа = СтрЗаменить(Строка.ТипОбъекта, ".", "Ссылка.");
		
		ПараметрыКоманды = Новый Структура;
		ПараметрыКоманды.Вставить("ТипОбъекта", Тип(ИмяТипа));
		ПараметрыКоманды.Вставить("ЕстьУсловияДобавления", Строка.ЕстьУсловияДобавления);
		ПараметрыКоманды.Вставить("ЕстьУсловияВидимости", Строка.ЕстьУсловияВидимости);
		
		ПараметрыИсточников.Вставить(Строка.ТипОбъекта, ПараметрыКоманды);
	КонецЦикла;
	
	Для Каждого Источник Из Источники.Строки Цикл
		
		ПараметрыИсточника = ПараметрыИсточников[Источник.ПолноеИмя];
		Если ПараметрыИсточника = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ТипОбъекта = ПараметрыИсточника.ТипОбъекта;
		
		МенеджерОбъекта = Источник.Менеджер;
		
		Если ПараметрыИсточника.ЕстьУсловияДобавления Тогда
			Если Не МенеджерОбъекта.ДобавлятьКомандыПечатиКомплектаДокументов() Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		
		//КомплектДокументовНаПринтер
		НоваяКоманда = Команды.Добавить();
		НоваяКоманда.Вид = "ПечатьКомплектаДокументовСНастройкой";
		НоваяКоманда.Обработчик = "УправлениеПечатьюУТКлиент.НачатьПечатьКомплектаДокументов";
		НоваяКоманда.Идентификатор = "КомплектДокументов";
		НоваяКоманда.Представление = НСтр("ru = 'Комплект документов на принтер'");
		НоваяКоманда.ТипПараметра = ТипОбъекта;
		НоваяКоманда.РежимЗаписи = "Проводить";
		НоваяКоманда.Порядок = 1;
		
		Если ПараметрыИсточника.ЕстьУсловияВидимости Тогда
			МенеджерОбъекта.ПриОпределенииКомандыПечатиКомплектаДокументов(НоваяКоманда);
		КонецЕсли;
		
		//КомплектДокументовСНастройкой
		НоваяКоманда = Команды.Добавить();
		НоваяКоманда.Вид = "ПечатьКомплектаДокументовСНастройкой";
		НоваяКоманда.Обработчик = "УправлениеПечатьюУТКлиент.ОткрытьФормуПечатиКомплектаДокументовСНастройкой";
		НоваяКоманда.Идентификатор = "КомплектДокументовСНастройкой";
		НоваяКоманда.Представление = НСтр("ru = 'Комплект документов с настройкой...'");
		НоваяКоманда.ТипПараметра = ТипОбъекта;
		НоваяКоманда.РежимЗаписи = "Проводить";
		НоваяКоманда.Порядок = 2;
