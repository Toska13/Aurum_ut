...
Процедура Печать(Команда)
	...
	Если КомплектПечатныхФорм.НайтиСтроки(Новый Структура("Печатать", Истина)).Количество() > 0 Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеФормированияПечатныхФорм", ЭтотОбъект);
		КомандаПечати = УправлениеПечатьюКлиент.СоздатьКомандуПечати();
		КомандыПечатиОбъекта = КомандыПечатиОбъекта(КомандаПечати);
		УправлениеПечатьюКлиент.ВыполнитьКомандыПечати(Объекты.ВыгрузитьЗначения(), КомандыПечатиОбъекта, ОписаниеОповещения, , Истина);
...
&НаКлиенте
Процедура ПослеФормированияПечатныхФорм(Результат, ДополнительныеПараметры) Экспорт
...
&НаСервере
Функция КомандыПечатиОбъекта(КомандаПечати)
	
	Результат = Новый Массив();
	
	КомандыПечатиОбъекта = УправлениеПечатью.КомандыПечатиОбъекта(Объекты[0].Значение.Метаданные());
	
	Для Каждого КомандаПечатиОбъекта Из КомандыПечатиОбъекта Цикл
		Если КомандаПечатиОбъекта.МенеджерПечати = "СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки" Тогда
			ПрефиксВнешнихПечатныхФорм = "ВнешняяПечатнаяФорма.";
		Иначе
			ПрефиксВнешнихПечатныхФорм = "";
		КонецЕсли;
		
		Отбор = Новый Структура("Имя", ПрефиксВнешнихПечатныхФорм + КомандаПечатиОбъекта.Идентификатор);
		НайденныеКоманды = КомплектПечатныхФорм.НайтиСтроки(Отбор);
		
		Если НайденныеКоманды.Количество() И НайденныеКоманды[0].Печатать Тогда
			КомандаВКомплект = ОбщегоназначенияКлиентСервер.СкопироватьРекурсивно(КомандаПечати);
			ЗаполнитьЗначенияСвойств(КомандаВКомплект, КомандаПечатиОбъекта);
			
			Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(КомандаПечатиОбъекта.ДополнительныеПараметры, "МенеджерПечати", "") <> "" Тогда
				КомандаВКомплект.МенеджерПечати = КомандаПечатиОбъекта.ДополнительныеПараметры.МенеджерПечати;
			КонецЕсли;
			
			Если КомандаВКомплект.ДополнительныеПараметры.Свойство("ИспользоватьСинонимДляКомплекта") Тогда
				КомандаВКомплект.ДополнительныеПараметры.ИспользоватьСинонимДляКомплекта = Истина;
			КонецЕсли;
			
			ИдентификакторКакКомплект = Новый Массив;
			Для Сч = 1 По НайденныеКоманды[0].Экземпляров Цикл
				ИдентификакторКакКомплект.Добавить(КомандаВКомплект.Идентификатор);
			КонецЦикла;
			КомандаВКомплект.Идентификатор = СтрСоединить(ИдентификакторКакКомплект, ",");
			
			Результат.Добавить(КомандаВКомплект);
		КонецЕсли;
	КонецЦикла;
