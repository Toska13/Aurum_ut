...
Функция ДанныеКодовМаркировки(КодыМаркировки) Экспорт
	...
	|	&КлючВидовПродукции                             КАК ВидПродукции,
	...
	ВидыПродукцииПоддерживающиеПеременноеКоличество = КонструкторИСМПКлиентСервер.ВидыПродукцииПоддерживающиеПеременноеКоличествоМернойПродукции();
	
	Запрос.УстановитьПараметр("ИсходныеДанные", ИсходныеДанные);
	Запрос.УстановитьПараметр("ВидыПродукцииПоддерживающиеПеременноеКоличество", ВидыПродукцииПоддерживающиеПеременноеКоличество);
	...
	Для Каждого СтрокаТаблицы Из ДанныеПула Цикл
		ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗаменитьПроверитьПолныйКодМаркировки(СтрокаТаблицы);
	КонецЦикла;
...
Процедура ПриЗаполненииОграниченияДоступа(Списки) Экспорт
	...
	УправлениеДоступомИСПереопределяемый.ПриЗаполненииОграниченияДоступа(
		Метаданные.РегистрыСведений.ПулКодовМаркировкиСУЗ, Списки);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = ОбновлениеИнформационнойБазыГосИС.ДобавитьОбработчикОбновленияПереходаНаКонструкторНастроек(
		Обработчики,
		Метаданные.РегистрыСведений.ПулКодовМаркировкиСУЗ,
		Новый УникальныйИдентификатор("f5a473fd-c896-4a2e-8da6-3b6256d7f1ab"));
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ОбъектМетаданных                      = Метаданные.РегистрыСведений.ПулКодовМаркировкиСУЗ;
	ПолноеИмяОбъекта                      = ОбъектМетаданных.ПолноеИмя();
	ПараметрыВыборки                      = Параметры.ПараметрыВыборки;
	ПараметрыВыборки.ПолныеИменаРегистров = ПолноеИмяОбъекта;
	ПараметрыВыборки.СпособВыборки        = ОбновлениеИнформационнойБазы.СпособВыборкиИзмеренияНезависимогоРегистраСведений();
	ПараметрыВыборки.МаксимумВыборки      = 50;
	...
	Запрос.Текст =
		...
		|ИЗ
		|	РегистрСведений.ПулКодовМаркировкиСУЗ КАК Таблица";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Для Каждого СтрокаКлюча Из ОбновляемыеДанные Цикл
		...
		НаборЗаписей.Отбор.ЗаказНаЭмиссию.Установить(СтрокаКлюча.ЗаказНаЭмиссию);
		...
		Попытка
			...
			Для Каждого СтрокаНабора Из НаборЗаписей Цикл
				...
				ЗначениеСтарогоПоля = СтрокаНабора.УдалитьВидПродукции;
				Если ЗначениеЗаполнено(ЗначениеСтарогоПоля)
					И Не ЗначениеЗаполнено(СтрокаНабора.ТоварнаяГруппа) Тогда
					
					СтрокаНабора.ТоварнаяГруппа = ОбновлениеИнформационнойБазыИСМП.ТоварнаяГруппаПоПеречислениюВидПродукции(
						СтрокаНабора.УдалитьВидПродукции,
						КешКонвертаций);
				...
				Если ТипЗнч(СтрокаНабора.Шаблон) = Тип("ПеречислениеСсылка.ШаблоныКодовМаркировкиСУЗ")
					И ЗначениеЗаполнено(СтрокаНабора.Шаблон) Тогда
					
					СтрокаНабора.Шаблон = ОбновлениеИнформационнойБазыИСМП.ШаблонКодаМаркировкиПоЗначениюПеречисления(
						СтрокаНабора.Шаблон,
						СтрокаНабора.ТоварнаяГруппа,
						КешКонвертаций);
					ЕстьИзменения = Истина;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗарезервироватьКодыПодЗаказ", Истина);
...
Процедура ЗарезервироватьКодыПодЗаказ(Параметры, АдресРезультата) Экспорт
	...
	Попытка
		...
		|	СвободныеКоды.ТоварнаяГруппа         КАК ТоварнаяГруппа,
		...
		|	ВозможныеСвободныеКоды.ТоварнаяГруппа         КАК ТоварнаяГруппа,
		...
		|	ДоступноКРезрвированиюПоИзлишкамПредварительно.ТоварнаяГруппа     КАК ТоварнаяГруппа,
		...
		|	ДоступноКРезрвированиюПоИзлишкамПредварительно.ТоварнаяГруппа,
	...
	Исключение
		...
		ТекстОшибки = ТекстОшибки + " " + ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
...
// Заполняет дерево упаковок данными о существующих упаковках по документу
//
// Параметры:
//   ДеревоУпаковок - ДеревоЗначений - дерево упаковок для заполнения
//   Документ - ДокументСсылка - документ, по которому формируется дерево упаковок
//   ТоварнаяГруппа - СправочникСсылка.ТоварныеГруппыГИСМТ - товарная группа для фильтрации
//   ДляПечати - Булево - признак формирования данных для печати
Процедура ЗаполнитьДеревоСуществующихУпаковокПоДокументу(ДеревоУпаковок,
	Документ, ТоварнаяГруппа = Неопределено, ДляПечати = Истина) Экспорт
	...
	ПараметрыВыполнения.Вставить("ТоварнаяГруппа", ТоварнаяГруппа);
	...
	ТекущаяТоварнаяГруппа             = Неопределено;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТекущийКодМаркировки <> Выборка.КодМаркировки Тогда
			...
			ПредставлениеНоменклатуры = ОбщегоНазначенияИС.ПредставлениеНоменклатуры(
				...
				Выборка.ПредставлениеИзКешаОписанияОстатков, Выборка.ТоварнаяГруппа));
			
			Если ТекущаяХешСуммаУпаковки <> Выборка.ХешСуммаУпаковки Тогда
				
				ТекущаяХешСуммаУпаковки = Выборка.ХешСуммаУпаковки;
				
				Если ТекущаяТоварнаяГруппа <> Выборка.ТоварнаяГруппа Тогда
					ТекущаяТоварнаяГруппа = Выборка.ТоварнаяГруппа;
					ШаблоныГрупповойУпаковки = ИнтеграцияИСМПКлиентСервер.ШаблоныКодовПоВидуПродукции(ТекущаяТоварнаяГруппа, 4);
					ТоварнаяГруппаИспользуетГрупповыеУпаковки = ШаблоныГрупповойУпаковки.Количество() > 0;
					ЭтоТабачнаяПродукция = ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМОТП(ТекущаяТоварнаяГруппа);
				КонецЕсли;
				
				Если ТоварнаяГруппаИспользуетГрупповыеУпаковки
...
Процедура УдалениеНеиспользованныхКодовМаркировки() Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТаблицаНастроек.ТоварнаяГруппа,
	...
	|	ПО ПулКодовМаркировкиСУЗ.ТоварнаяГруппа = ТаблицаНастроек.ТоварнаяГруппа
...
Функция НоваяТаблицаДанныхДляПечатиЭтикеток() Экспорт
	...
	ТаблицаИтог.Колонки.Добавить("Шаблон",               Новый ОписаниеТипов("СправочникСсылка.ШаблоныКодовМаркировкиСУЗ"));
	...
	ТаблицаИтог.Колонки.Добавить("ТоварнаяГруппа",       Новый ОписаниеТипов("СправочникСсылка.ТоварныеГруппыГИСМТ"));
	...
	МассивТиповПризнакаМаркировкиОстатков.Добавить(Тип("СправочникСсылка.ВидыПродукцииИС"));
...
// 	РежимПолученияКодов - Строка - "ПоУмолчанию" - коды без упаковок + уникальные упаковки
//                                 "ОтчетОНанесении" - все коды с родительскими упаковками + полные коды родительских упаковок
//                                 "ВсеБезИерархии" - все коды без упаковок (например, списание кодов маркировки)
// Возвращаемое значение:
// 	Массив из Структура - Коды маркировки.
Функция КодыМаркировкиПоОснованию(Основания, РежимПолученияКодов = "ПоУмолчанию") Экспорт
	...
	СписокЗапросов.Добавить(
		...
		|	ЕСТЬNULL(ПулКодовМаркировкиСУЗ.ШтрихкодУпаковки.ХешСуммаЗначенияШтрихкода, """") КАК ХешСуммаЗначенияШтрихкода,
		|	ПулКодовМаркировкиСУЗ.ШтрихкодУпаковки КАК ШтрихкодУпаковки
	...
	Если РежимПолученияКодов = "ПоУмолчанию"
		Или РежимПолученияКодов = "ОтчетОНанесении" Тогда
		СписокЗапросов.Добавить(
			"ВЫБРАТЬ
	...
	Если РежимПолученияКодов = "ПоУмолчанию" Тогда
		СписокЗапросов.Добавить(
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			"УникальныеУпаковкиВерхнегоУровня");
	ИначеЕсли РежимПолученияКодов = "ОтчетОНанесении" Тогда
		...
		СписокЗапросов.Добавить(
			...
			|	КодыМаркировки.GTIN,
			|	КодыМаркировки.СрокГодности,
			|	ЕСТЬNULL(ПулКодовМаркировкиСУЗ.ПолныйКодМаркировки, """") КАК ПолныйКодМаркировкиУпаковки
			|ИЗ
			|	КодыМаркировки КАК КодыМаркировки
			|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПулКодовМаркировкиСУЗ КАК ПулКодовМаркировкиСУЗ
			|	ПО ПулКодовМаркировкиСУЗ.КодМаркировки = КодыМаркировки.КодМаркировкиУпаковки
			|	И ПулКодовМаркировкиСУЗ.ХешСуммаКодаМаркировки = КодыМаркировки.ХешСуммаЗначенияШтрихкода
			|ГДЕ
			|	КодыМаркировки.ШтрихкодУпаковки <> ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка)
			|	И КодыМаркировки.КодМаркировки <> КодыМаркировки.КодМаркировкиУпаковки
			|	И КодыМаркировки.ХешСуммаКодаМаркировки <> КодыМаркировки.ХешСуммаЗначенияШтрихкода
			|УПОРЯДОЧИТЬ ПО
			|	КодыМаркировки.GTIN,
			|	КодыМаркировки.СрокГодности",
			"КодыСРодительскимиУпаковками");
	...
	//@skip-warning
	Выборка = РезультатЗапроса["КодыБезИерархии"].Выбрать();
	
	Пока Выборка.Следующий() Цикл
		
		ТекущаяСтрока = Новый Структура;
		ТекущаяСтрока.Вставить("Штрихкод",                          Выборка.ПолныйКодМаркировки);
		...
		ТекущаяСтрока.Вставить("ФорматBase64",                      Истина);
	...
	Если РежимПолученияКодов = "ПоУмолчанию" Тогда
		...
		//@skip-warning
		Выборка = РезультатЗапроса["УникальныеУпаковкиВерхнегоУровня"].Выбрать();
		Пока Выборка.Следующий() Цикл
			
			КодМаркировки = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ШтрихкодВBase64(Выборка.КодМаркировкиУпаковки);
	...
	ИначеЕсли РежимПолученияКодов = "ОтчетОНанесении" Тогда
		...
		//@skip-warning
		Выборка = РезультатЗапроса["КодыСРодительскимиУпаковками"].Выбрать();
		
		Пока Выборка.Следующий() Цикл
			
			Если ЗначениеЗаполнено(Выборка.ПолныйКодМаркировкиУпаковки) Тогда
				ШтрихкодУпаковки = Выборка.ПолныйКодМаркировкиУпаковки;
			Иначе
				ШтрихкодУпаковки = Выборка.КодМаркировкиУпаковки;
			КонецЕсли;
			...
			ТекущаяСтрока.Вставить("Количество",                        1);
			ТекущаяСтрока.Вставить("ШтрихкодМаркиАлкогольнойПродукции", "");
			ТекущаяСтрока.Вставить("ШтрихкодУпаковки",                  ШтрихкодУпаковки);
			ТекущаяСтрока.Вставить("ФорматBase64",                      Истина);
...
Процедура ЗаписатьДанныеКодаМаркировки(ДанныеШтрихкода, ПараметрыСканирования, ДанныеЗаписи = Неопределено) Экспорт
	...
	ДанныеДляЗаписи.Вставить("ПолныйКодМаркировки",     ДанныеШтрихкода.ПолныйКодМаркировки);
	...
	ДанныеДляЗаписи.Вставить("ТоварнаяГруппа",          КонструкторИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппаПоВидуПродукции(ДанныеШтрихкода.ВидПродукции));
	...
	НаборЗаписей = СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.КодМаркировки.Установить(ДанныеДляЗаписи.КодМаркировки);
	НаборЗаписей.Отбор.ХешСуммаКодаМаркировки.Установить(ДанныеДляЗаписи.ХешСуммаКодаМаркировки);
	...
	Если Не ЗначениеЗаполнено(МенеджерЗаписи.Шаблон) Тогда
		Если ДанныеШтрихкода.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая Тогда
			МенеджерЗаписи.Шаблон = ИнтеграцияИСМПКлиентСервер.ШаблонКодаМаркировкиПоВидуПродукции(МенеджерЗаписи.ТоварнаяГруппа, 4);
		Иначе
			МенеджерЗаписи.Шаблон = ИнтеграцияИСМПКлиентСервер.ШаблонКодаМаркировкиПоВидуПродукции(МенеджерЗаписи.ТоварнаяГруппа);
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		...
		ДанныеШтрихкода.ПолныйКодМаркировки      = ДанныеШтрихкода.ПолныйКодМаркировки;
...
Функция ПредставлениеКИЗ(СтрокаДанных) Экспорт
	...
	Если ЗначениеЗаполнено(СтрокаДанных.СпособВводаВОборот)
		...
		Представление = ИнтеграцияИСМПКлиентСервер.ПредставлениеПустойНоменклатурыПродукцииПоддерживающейИндивидуализациюКИЗ(
			СтрокаДанных,
			Префикс);
...
Процедура ДополнитьЗапросКодамиСМРЦ(Запрос, ЭтоЗаказНаЭмиссию, ДополнительноеУсловие) Экспорт
	...
	|	И ПулКодовМаркировки.Шаблон В
	|	(ВЫБРАТЬ РАЗЛИЧНЫЕ СправочникШаблоныКодовМаркировкиСУЗ.Ссылка
	|	ИЗ Справочник.ШаблоныКодовМаркировкиСУЗ КАК СправочникШаблоныКодовМаркировкиСУЗ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ШаблоныКодовМаркировкиСУЗ.ШаблоныРазбораКодовМаркировки КАК ТЧШаблоныРазбораКодовМаркировки
	|		ПО СправочникШаблоныКодовМаркировкиСУЗ.Ссылка = ТЧШаблоныРазбораКодовМаркировки.Ссылка
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ШаблоныРазбораКодовМаркировкиИСМП КАК СправочникШаблоныРазбораКодовМаркировкиИСМП
	|			ПО ТЧШаблоныРазбораКодовМаркировки.ШаблонРазбораКодовМаркировки = СправочникШаблоныРазбораКодовМаркировкиИСМП.Ссылка
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ШаблоныРазбораКодовМаркировкиИСМП.СоставКодаМаркировки КАК ТЧСоставКодаМаркировки
	|				ПО СправочникШаблоныРазбораКодовМаркировкиИСМП.Ссылка = ТЧСоставКодаМаркировки.Ссылка
	|				И ТЧСоставКодаМаркировки.ЭлементСоставаКодаМаркировки = ЗНАЧЕНИЕ(Перечисление.ЭлементыСоставаКодаМаркировкиИСМП.МРЦ6)
	|	ГДЕ СправочникШаблоныКодовМаркировкиСУЗ.ТипКодаМаркировки = ЗНАЧЕНИЕ(Перечисление.ТипыКодовМаркировкиИСМП.ГрупповаяУпаковка))
	|" + ДополнительноеУсловие;
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Количество() > 0 Тогда
		ВидыПродукцииПоТоварнымГруппам = Новый Соответствие;
		...
		Пока Выборка.Следующий() Цикл
			ВидыПродукции = ВидыПродукцииПоТоварнымГруппам[Выборка.ТоварнаяГруппа];
			Если ВидыПродукции = Неопределено Тогда
				ВидыПродукции = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(Выборка.ТоварнаяГруппа);
				ВидыПродукцииПоТоварнымГруппам.Вставить(Выборка.ТоварнаяГруппа, ВидыПродукции);
			КонецЕсли;
			ДанныеРазбораКодаМаркировки = ГрупповаяОбработкаШтрихкодовИС.ВидУпаковкиИПредставлениеШтрихкода(
				...
				ВидыПродукции, Неопределено, Неопределено,
...
Функция КодМаркировкиДляСохраненияВПул(СтрокаУпаковки, ПолучатьДанныеРазбора = Истина) Экспорт
	...
	Если СтрокаУпаковки.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская
		...
		Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(ВидПродукции)
		...
		Попытка
			...
			Для Каждого СтрокаНабораДанных Из НаборДанных Цикл
				...
				НормализованныйКод = РазборКодаМаркировкиИССлужебныйКлиентСервер.НормализоватьКодМаркировки(
					СтрокаДанных,
					СтрокаНабораДанных.ТоварнаяГруппа,
...
Процедура РаспечататьЭтикеткиИЗафиксироватьДатуПечатиДляОбъектовПечати(ДанныеПечати, СтруктураОбъектовПечати,
	...
	КэшПредставленияНоменклатуры = ИсходнаяТаблица.СкопироватьКолонки(
		"Номенклатура,Характеристика,Серия,ТоварнаяГруппа,ПредставлениеНоменклатуры");
	
	СтруктураПоиска = Новый Структура;
	СтруктураПоиска.Вставить("Номенклатура");
	СтруктураПоиска.Вставить("Характеристика");
	СтруктураПоиска.Вставить("Серия");
	СтруктураПоиска.Вставить("ТоварнаяГруппа");
...
Функция ТаблицаДляПечатиНеРаспечатанныеКодыПоДокументу(ТаблицаНоменклатуры, СтруктураОтбораДокументы, РезервироватьСвободныеКоды, МаркировкаОстатков)
	...
	Запрос.Текст =
		...
		|		КОГДА &МаркировкаОстатков = ЛОЖЬ
		|			ТОГДА ПулКодовМаркировкиСУЗ.МаркировкаОстатков = ЛОЖЬ
		|		ИНАЧЕ ПулКодовМаркировкиСУЗ.МаркировкаОстатков = ИСТИНА
		|			ИЛИ ПулКодовМаркировкиСУЗ.СпособВводаВОборот = ЗНАЧЕНИЕ(Перечисление.СпособыВводаВОборотСУЗ.МаркировкаВнеПроизводстваИмпорта)
		|	КОНЕЦ
...
Функция ЗапросСвободныхКодовПоЗаказу(Заказ, ТаблицаНоменклатуры, МаркировкаОстатков = Неопределено)
	...
	Запрос.УстановитьПараметр("ЭлементКонструктораДляОпределенияДоступныхВидовПродукции",
		КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭлементКонструктораДляОпределенияДоступныхВидовПродукции());
	Запрос.УстановитьПараметр("ТаблицаНоменклатуры",     ТаблицаНоменклатуры);
	...
	Запрос.Текст =
	Справочники.ТоварныеГруппыГИСМТ.ТекстЗапросаТоварныхГруппСОтборомПоЭлементуКонструктора(,
		"ДоступныеТоварныеГруппы") + "
	|;
	|ВЫБРАТЬ
	|	МаркированныеТоварыОснований.Организация        КАК Организация,
	|	МаркированныеТоварыОснований.ТоварнаяГруппа     КАК ТоварнаяГруппа,
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДоступныеТоварныеГруппы КАК ДоступныеТоварныеГруппы
	|	ПО МаркированныеТоварыОснований.ТоварнаяГруппа = ДоступныеТоварныеГруппы.Ссылка
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	ТоварнаяГруппа,
	|	Номенклатура,
	...
	|	Пул.ЗаказНаЭмиссию.ТоварнаяГруппа КАК ТоварнаяГруппа,
	...
	|			И Товары.ТоварнаяГруппа     = Пул.ЗаказНаЭмиссию.ТоварнаяГруппа
	|			И Товары.Номенклатура       = Пул.Номенклатура
	|			И Товары.Характеристика     = Пул.Характеристика
	|			И ВЫБОР
	|				КОГДА Товары.Шаблон = ЗНАЧЕНИЕ(Справочник.ШаблоныКодовМаркировкиСУЗ.ПустаяСсылка)
	|				ТОГДА ИСТИНА
	|				ИНАЧЕ Товары.Шаблон = Пул.Шаблон
	|			  КОНЕЦ
	...
	|	И Пул.Шаблон.ТипКодаМаркировки В
	|		(ЗНАЧЕНИЕ(Перечисление.ТипыКодовМаркировкиИСМП.ЕдиницаТовара),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыКодовМаркировкиИСМП.Комплект))
	...
	|		КОГДА &МаркировкаОстатков = НЕОПРЕДЕЛЕНО ТОГДА ИСТИНА
	|		КОГДА &МаркировкаОстатков = ЛОЖЬ
	|			ТОГДА Пул.МаркировкаОстатков = ЛОЖЬ
	|		ИНАЧЕ Пул.МаркировкаОстатков = ИСТИНА
	|			ИЛИ Пул.СпособВводаВОборот = ЗНАЧЕНИЕ(Перечисление.СпособыВводаВОборотСУЗ.МаркировкаВнеПроизводстваИмпорта)
	|	КОНЕЦ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	ВозможныеСвободныеКоды.ТоварнаяГруппа,
	...
	|	ВозможныйИсточникСвободныхКодов.ТоварнаяГруппа
	...
	|	ЗаказНаЭмиссиюТовары.Ссылка.ТоварнаяГруппа     КАК ТоварнаяГруппа,
	...
	|		 И ДокументыСОшибкамиОформления.ТоварнаяГруппа = ЗаказНаЭмиссиюТовары.Ссылка.ТоварнаяГруппа
	|ГДЕ
	|	ЗаказНаЭмиссиюТовары.Ссылка.Проведен
	|	И ЗаказНаЭмиссиюТовары.Шаблон.ТипКодаМаркировки В
	|		(ЗНАЧЕНИЕ(Перечисление.ТипыКодовМаркировкиИСМП.ЕдиницаТовара),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыКодовМаркировкиИСМП.Комплект))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	Пул.ЗаказНаЭмиссию.ТоварнаяГруппа,
	|	Пул.Номенклатура,
	|	Пул.Характеристика,
	|	Пул.Шаблон,
	...
	|		 И ДокументыСОшибкамиОформления.ТоварнаяГруппа = Пул.ЗаказНаЭмиссию.ТоварнаяГруппа
	|		 И Пул.Шаблон.ТипКодаМаркировки В
	|			(ЗНАЧЕНИЕ(Перечисление.ТипыКодовМаркировкиИСМП.ЕдиницаТовара),
	|			ЗНАЧЕНИЕ(Перечисление.ТипыКодовМаркировкиИСМП.Комплект))
	|
	...
	|	ОжидаетсяПоДокументамСОшибкамиПредварительно.ТоварнаяГруппа,
	...
	|		 И ДокументыСОшибкамиОформления.ТоварнаяГруппа = МаркированныеТоварыОснований.ТоварнаяГруппа
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция ЗапросСостоянияКодовПоЗаказу(
	...
	|ВЫБРАТЬ
	|	ЗаказНаЭмиссиюТовары.Ссылка              КАК ЗаказНаЭмиссию
	|ПОМЕСТИТЬ ЭмиссияЗавершена
	|ИЗ
	|	Документ.ЗаказНаЭмиссиюКодовМаркировкиСУЗ.Товары КАК ЗаказНаЭмиссиюТовары
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыДокументовИСМП КАК СтатусыДокументовИСМП
	|		ПО СтатусыДокументовИСМП.Документ = ЗаказНаЭмиссиюТовары.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ ЗаказыСКодамиВПуле КАК ЗаказыСКодамиВПуле
	|		ПО ЗаказыСКодамиВПуле.ЗаказНаЭмиссию = ЗаказНаЭмиссиюТовары.Ссылка
	|ГДЕ
	|	ЗаказНаЭмиссиюТовары.Ссылка.Проведен
	|	И ЗаказНаЭмиссиюТовары.Ссылка.ДокументОснование = &Заказ
	|	И СтатусыДокументовИСМП.Статус В(
	|		ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаказовНаЭмиссиюКодовМаркировкиИСМП.СУЗЗакрыт),
	|		ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаказовНаЭмиссиюКодовМаркировкиИСМП.СУЗКодыМаркировкиЭмитированы),
	|		ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаказовНаЭмиссиюКодовМаркировкиИСМП.ЦЭМКодыМаркировкиЭмитированы))
	|	И ЗаказыСКодамиВПуле.ЗаказНаЭмиссию ЕСТЬ NULL
	|ИНДЕКСИРОВАТЬ ПО
	|	ЗаказНаЭмиссию
	|;
	|
	...
	|	ЗаказНаЭмиссиюТовары.Характеристика      КАК Характеристика,
	|	ВЫБОР
	|		КОГДА ЗаказНаЭмиссиюТовары.Ссылка В (ВЫБРАТЬ ЗаказНаЭмиссию ИЗ ЭмиссияЗавершена)
	|			ТОГДА ЗаказНаЭмиссиюТовары.Количество
	|		ИНАЧЕ 0
	...
	|	ОжидаетсяПоДокументуОснованиюПредварительно.ТоварнаяГруппа    КАК ТоварнаяГруппа,
	...
	|	СУММА(ОжидаетсяПоДокументуОснованиюПредварительно.КоличествоРаспечатано) КАК КоличествоРаспечатано
	|ПОМЕСТИТЬ Ожидается
	|ИЗ
	...
	|	ОжидаетсяПоДокументуОснованиюПредварительно.ТоварнаяГруппа,
	...
	|	ИЛИ СУММА(ОжидаетсяПоДокументуОснованиюПредварительно.КоличествоРаспечатано) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Пул.ЗаказНаЭмиссию.Организация  КАК Организация,
	|	Пул.ТоварнаяГруппа              КАК ТоварнаяГруппа,
	|	Пул.Номенклатура                КАК Номенклатура,
	|	Пул.Характеристика              КАК Характеристика,
	...
	|	0,
	|	0,
	|	СУММА(ВЫБОР КОГДА Ожидается.КоличествоРаспечатано = 0 ТОГДА Ожидается.Количество ИНАЧЕ 0 КОНЕЦ),
	|	0,
	|	СУММА(Ожидается.КоличествоРаспечатано),
	|	NULL
	|ИЗ
	...
	|	ДоступноКРезрвированию.ТоварнаяГруппа,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОбщийРасчет.ТоварнаяГруппа                 КАК ТоварнаяГруппа,
...
Процедура СформироватьТаблицуМаркированныхТоваровОснований(Запрос, ИмяВременнойТаблицыСКолонкойДокументОснование)
	...
	|	Товары.ТоварнаяГруппа     КАК ТоварнаяГруппа,
	...
	|	КОНЕЦ,
	|	ОКР(Товары.Количество / ВЫБОР
	|		КОГДА ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке ЕСТЬ NULL
	|			ИЛИ ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке = 0
	|		ТОГДА 1
...
Функция НоваяТаблицаНоменклатуры(Параметры = Неопределено)
	...
	Если ПереданаТаблицаНоменклатуры Тогда
		...
		Если ЗначениеЗаполнено(Параметры.Документ)
			И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Параметры.Документ, "ТоварнаяГруппа") Тогда
			ТоварнаяГруппаДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.Документ, "ТоварнаяГруппа");
		Иначе
			ТоварнаяГруппаДокумента = Неопределено;
		КонецЕсли;
		...
		Для Каждого СтрокаТаблицы Из Параметры.ТаблицаНоменклатуры Цикл
			...
			Если Не ЗначениеЗаполнено(НоваяСтрока.ТоварнаяГруппа)
				И ЗначениеЗаполнено(ТоварнаяГруппаДокумента) Тогда
				НоваяСтрока.ТоварнаяГруппа = ТоварнаяГруппаДокумента;
			КонецЕсли;
		КонецЦикла;
...
Процедура ЗаполнитьДанныеДляАгрегацииДереваУпаковок(СтрокиМаркированнойПродукции, ПараметрыВыполнения)
	...
	Для Каждого СтрокаМаркированнойПродукции Из СтрокиМаркированнойПродукции Цикл
		...
		Если ТипЗнч(СтрокаМаркированнойПродукции.ТипУпаковки) = Тип("ПеречислениеСсылка.ПрочиеЗоныПересчетаПродукцииИС") Тогда
...
Функция НастройкиУдаленияНеиспользованныхКодовМаркировки()
	...
	ДатыПоТоварнымГруппам = Новый Соответствие;
	
	ДанныеЭлемента = КонструкторИСОбщегоНазначенияКлиентСервер.ДанныеЭлементаКонструктора(
		Справочники.ЭлементыКонструктораВидовПродукцииИС.СрокИспользованияНеОплаченныхЭмитированныхКодовМаркировки);
	
	ДанныеТоварныхГрупп = КонструкторИСОбщегоНазначенияКлиентСервер.ДанныеЭлементаКонструктора(
	...
	Для Каждого КлючИЗначение Из ДанныеЭлемента.ВидыПродукции Цикл
		ДанныеТоварнойГруппы = ДанныеТоварныхГрупп.ВидыПродукции[КлючИЗначение.Ключ];
		Если ДанныеТоварнойГруппы <> Неопределено Тогда
			Для Каждого ЗначениеТоварнойГруппы Из ДанныеТоварнойГруппы Цикл
				ТоварнаяГруппа = ЗначениеТоварнойГруппы.Ключ;
				Если ЗначениеЗаполнено(ТоварнаяГруппа) Тогда
					Для Каждого Значение Из КлючИЗначение.Значение Цикл
						Если ЗначениеЗаполнено(Значение.Ключ) Тогда
							ТекущееЗначение = ДатыПоТоварнымГруппам[ТоварнаяГруппа];
							Если ТекущееЗначение = Неопределено
								Или ТекущееЗначение < Значение.Ключ Тогда
								ДатыПоТоварнымГруппам.Вставить(ТоварнаяГруппа, Значение.Ключ);
							КонецЕсли;
						КонецЕсли;
					КонецЦикла
				КонецЕсли;
	...
	ТаблицаНастроек.Колонки.Добавить("ТоварнаяГруппа", Новый ОписаниеТипов("СправочникСсылка.ТоварныеГруппыГИСМТ"));
	...
	Для Каждого КлючИЗначение Из ДатыПоТоварнымГруппам Цикл
		НоваяСтрока = ТаблицаНастроек.Добавить();
		НоваяСтрока.ТоварнаяГруппа = КлючИЗначение.Ключ;
		НоваяСтрока.ДатаЭмиссии    = ДатаОтсчета - Сутки * КлючИЗначение.Значение;
	КонецЦикла;
