...
Процедура ВиртуальнаяАгрегация(Команда)
	...
	ПараметрыФормы = Новый Структура();
	ПараметрыФормы.Вставить("Документ", Документ);
	ПараметрыФормы.Вставить("ТоварнаяГруппа", ТекущиеДанные.ТоварнаяГруппа);
...
Функция ТаблицаДляРезервирования()
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.МаркировкаТоваровИСМП") Тогда
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Документ, "Организация, ТоварнаяГруппа, Операция");
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ПеремаркировкаТоваровИСМП") Тогда
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Документ, "Организация, ТоварнаяГруппа");
		СпособВводаВОборот = ПеремаркировкаТоваровКлиентСервер.СпособВводаВОборотПриПеремаркировке(РеквизитыДокумента.ТоварнаяГруппа);
	Иначе
	...
	ПараметрыЗаполнения.Вставить("ТоварнаяГруппа",     РеквизитыДокумента.ТоварнаяГруппа);
	ПараметрыЗаполнения.Вставить("СпособВводаВОборот", СпособВводаВОборот);
	ПараметрыЗаполнения.Вставить("Шаблон",
		ИнтеграцияИСМПКлиентСервер.ШаблонКодаМаркировкиПоВидуПродукции(РеквизитыДокумента.ТоварнаяГруппа));
	...
	ТаблицаДляРезервирования.Колонки.Добавить("ТоварнаяГруппа",     Новый ОписаниеТипов("СправочникСсылка.ТоварныеГруппыГИСМТ"));
	ТаблицаДляРезервирования.Колонки.Добавить("Шаблон",             Новый ОписаниеТипов("СправочникСсылка.ШаблоныКодовМаркировкиСУЗ"));
...
Процедура ЗаказатьКодыМаркировки()
	...
	Для Каждого СтрокаТЧ Из ПулКодовМаркировки Цикл
		Если СтрокаТЧ.КоличествоЗаказать > 0 Тогда
			...
			Товары = ТоварныеГруппы[СтрокаТЧ.ТоварнаяГруппа];
			
			Если Товары = Неопределено Тогда
				Товары = Новый Массив();
				ТоварныеГруппы.Вставить(СтрокаТЧ.ТоварнаяГруппа, Товары);
			КонецЕсли;
	...
	Для Каждого КлючИЗначение Из ТоварныеГруппы Цикл
		...
		ДанныеЗаполнения.Вставить("ТоварнаяГруппа",    КлючИЗначение.Ключ);
