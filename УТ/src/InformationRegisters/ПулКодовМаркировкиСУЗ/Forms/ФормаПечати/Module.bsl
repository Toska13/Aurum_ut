...
Процедура УправлениеЭлементамиФормыПриСоздании()
	...
	ПечатьЭтикетокИСМППереопределяемый.УстановитьПараметрыВыбораШаблонаЭтикетки(
		ЭтотОбъект,
		...
		Если НЕ Параметры.ТолькоАгрегированныеКоды Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				СтрокаТаблицы.Содержимое = ОбщегоНазначенияИС.ПредставлениеНоменклатуры(
					...
					Выборка.ПредставлениеНоменклатуры, Выборка.ТоварнаяГруппа));
			КонецЦикла;
...
Функция ЗапросПроизвольныеКодыДляПечати(ТаблицаКодовМаркировки)
	...
	|ГДЕ
	|	КодыМаркировки.ТоварнаяГруппа ЕСТЬ NULL
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	КодыМаркировки КАК КодыМаркировки
	|ГДЕ
	|	НЕ КодыМаркировки.ТоварнаяГруппа ЕСТЬ NULL
	|СГРУППИРОВАТЬ ПО
	|	КодыМаркировки.ИндексСтрокиДерева
	|ИМЕЮЩИЕ
...
Процедура ЗаполнитьДеревоАгрегированныхКодовПроизвольнымиКодами(РезультатЗапроса, ДеревоУпаковок)
	...
	СоставУпаковки.Колонки.Добавить("ТоварнаяГруппа", Новый ОписаниеТипов("СправочникСсылка.ТоварныеГруппыГИСМТ"));
		...
		Пока ВыборкаНоменклатура.Следующий() Цикл
			...
			Иначе
				ПредставлениеОстатков = ИнтеграцияИСМПКлиентСервер.ПредставлениеGTINОстаткиПоВидуПродукции(
					ВыборкаНоменклатура.ПредставлениеНоменклатуры, ВыборкаНоменклатура.ТоварнаяГруппа);
...
Процедура ЗаполнитьЭлементДереваПоСтрокеДерева(Приемник, Источник, ПараметрыУпаковки, ПоляДерева)
	...
	Иначе
		...
		Если ПараметрыУпаковки.СоставУпаковки.Количество() = 1 Тогда
			...
			ТоварнаяГруппа  = НайденнаяСтрока.ТоварнаяГруппа;
		...
		ИначеЕсли Источник.ВидУпаковки = Перечисления.ВидыУпаковокИС.Набор
			...
			ТоварнаяГруппа = ПараметрыУпаковки.СоставУпаковки[0].ТоварнаяГруппа;
		Иначе
			...
			ИначеЕсли НайденныеСтроки.Количество() > 1 Тогда
				ТоварнаяГруппа  = НайденныеСтроки[0].ТоварнаяГруппа;
			КонецЕсли;
		КонецЕсли;
		...
		Если Источник.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая Тогда
			...
			Если ТоварнаяГруппа <> Неопределено Тогда
				ЭтоТабачнаяПродукция = ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМОТП(ТоварнаяГруппа);
			Иначе
			...
			ИначеЕсли ТоварнаяГруппа <> Неопределено Тогда
				Приемник.Шаблон = ИнтеграцияИСМПКлиентСервер.ШаблонКодаМаркировкиПоВидуПродукции(ТоварнаяГруппа, 4);
			КонецЕсли;
		ИначеЕсли Источник.ВидУпаковки = Перечисления.ВидыУпаковокИС.Набор Тогда
			...
			Если Шаблон <> Неопределено Тогда
				Приемник.Шаблон = Шаблон;
			ИначеЕсли ТоварнаяГруппа <> Неопределено Тогда
				Приемник.Шаблон = ИнтеграцияИСМПКлиентСервер.ШаблонКодаМаркировкиПоВидуПродукции(ТоварнаяГруппа, 5);
			КонецЕсли;
		Иначе
			Если НайденнаяСтрока = Неопределено
				И НайденныеСтроки.Количество() > 0 Тогда
				ШаблоныГрупповойУпаковки = ИнтеграцияИСМПКлиентСервер.ШаблоныКодовПоВидуПродукции(ТоварнаяГруппа, 4);
			...
			Иначе
				...
				Если ТоварнаяГруппа <> Неопределено Тогда
					Приемник.Шаблон = ИнтеграцияИСМПКлиентСервер.ШаблонКодаМаркировкиПоВидуПродукции(ТоварнаяГруппа);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
