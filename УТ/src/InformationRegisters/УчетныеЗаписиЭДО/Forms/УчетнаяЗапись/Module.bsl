...
Перем ПараметрыОбработчикаОжидания, ПаролиСертификатов, УникальныйИдентификаторЗаявки1СЭДО,
	...
	ОтветДляЗаявкиТакском,ВыполняетсяЗакрытие,ДанОтветНаВопросОНедействительныхСертификатахПриОткрытии,
	ТребуетсяПереходВЦентрЭДОПослеЗаписи, ОбработчикДействияПослеЗаписи;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// СервисыМобильногоПодписания
	НастроитьВидимостьКнопокДобавитьИзСервисовМобильногоПодписания();
	// Конец СервисыМобильногоПодписания
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ПроверкаСертификатовНаДействительность();
	...
	Запрос = УчетныеЗаписиЭДО.ЗапросСертификатовУчетнойЗаписиДляВыводаНаФорму("СертификатыУчетныхЗаписей");
	...
	ДополнительныеСвойстваСертификата = Новый Соответствие;
	Если Не РезультатЗапроса.Пустой() Тогда
		ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Сертификаты.Добавить(ВыборкаДетальныеЗаписи.Сертификат);
			ДополнительныеСвойства = Новый Структура("Доверенность, Должность",
				ВыборкаДетальныеЗаписи.Доверенность, ВыборкаДетальныеЗаписи.Должность);
			ДополнительныеСвойстваСертификата.Вставить(ВыборкаДетальныеЗаписи.Сертификат, ДополнительныеСвойства);
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого СвойстваСертификата Из СвойстваСертификатов Цикл
		...
		НоваяСтрока.Доверенность = ДополнительныеСвойстваСертификата[НоваяСтрока.Сертификат].Доверенность;
		...
		НоваяСтрока.Должность = ДополнительныеСвойстваСертификата[НоваяСтрока.Сертификат].Должность;
		НоваяСтрока.ДолжностьПоСертификату = СвойстваСертификата.Значение.Должность;
		НоваяСтрока.ТребуетсяЗаполнитьДолжность = 
			УчетныеЗаписиЭДОСлужебный.ТребуетсяЗаполнитьДолжностьДляСертификатаПоИНН(ДанныеОрганизации.ИНН,
				СвойстваСертификата.Значение.Ссылка);
	КонецЦикла;
	...
	ПараметрыПриСозданииНаСервере = АвтоматическоеПолучениеЭДО.НовыеПараметрыФормыЭлементаПриЧтенииНаСервере();
	ПараметрыПриСозданииНаСервере.ИдентификаторЭДО = Запись.ИдентификаторЭДО;
	ПараметрыПриСозданииНаСервере.ГруппаПредупреждение = Элементы.ГруппаПредупреждениеАвтоматическогоПолученияЭДО;
	АвтоматическоеПолучениеЭДО.ФормаЭлементаПриЧтенииНаСервере(ЭтотОбъект, ПараметрыПриСозданииНаСервере);
	...
	Если ИспользоватьИнтеграциюЦентраЭДО Тогда
		...
		НастройкиИнтеграции = ОбщийМодульИнтеграцияЦентраЭДО.НастройкиИнтеграции(Запись.ИдентификаторЭДО);
		Если ЗначениеЗаполнено(НастройкиИнтеграции) Тогда
			УчетнаяЗаписьЦентраЭДО = НастройкиИнтеграции.УчетнаяЗаписьЦентраЭДО;
			НастроитьОтображениеСостоянияВыгрузкиВЦентрЭДО();
		КонецЕсли;
	КонецЕсли;
	НастроитьОтображениеКнопкиПереходаВЦентрЭДО(ЕстьПравоНастройкиОбмена, ИспользоватьИнтеграциюЦентраЭДО,
		Запись.СпособОбменаЭД, НастройкиИнтеграции);
	НастроитьОтображениеКнопкиПереносДокументовВЦентрЭДО(ЕстьПравоНастройкиОбмена, ИспользоватьИнтеграциюЦентраЭДО,
		Запись.СпособОбменаЭД, НастройкиИнтеграции);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("Записать")
		...
		Если Запись.СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезОператораЭДОТакском") 
			И ЗаВремяРедактированияВФормеБылПереводВХАБ() Тогда
		
			Отказ = Истина;
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Во время редактирования был обновлен способ обмена электронными документами. Сохранение изменений учетной записи ЭДО невозможно.
			|Необходимо открыть учетную запись ЭДО и внести изменения повторно.'"));
	...
	Если ПроверитьЗаполнениеРеквизитов() Тогда
		Отказ = Истина;
		ВыполнитьОбработкуОповещения(ОбработчикДействияПослеЗаписи, Ложь);
		ОбработчикДействияПослеЗаписи = Неопределено;
		Возврат;
	КонецЕсли;
	...
	Если Запись.ИспользоватьЦентрЭДО Тогда
...
Процедура ПриОткрытии(Отказ)
	...
	ДанОтветНаВопросОНедействительныхСертификатахПриОткрытии = Ложь;
	Если ЕстьНедействительныеСертификаты Тогда
		Если ТолькоПросмотр Тогда
			ПредупредитьОНедействительныхСертификатах(Ложь);
		Иначе
			СпроситьОбУдаленииНедействительныхСертификатов(Ложь);
		КонецЕсли;
	КонецЕсли;
	
	Если ОбновлятьСостояниеВыгрузкиВЦентрЭДО Тогда
		ОбновитьСостояниеВыгрузкиВЦентрЭДОЧерезИнтервал();
	КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	ФормаОткрыта = Открыта();
	Если Закрыть И ФормаОткрыта Тогда
		Закрыть();
	ИначеЕсли Не Закрыть И ФормаОткрыта Тогда
		ПроверитьНаличиеНедействительныхСертификатов();
		Если ЕстьНедействительныеСертификаты И Не ВыполняетсяЗакрытие Тогда
			СпроситьОбУдаленииНедействительныхСертификатов(Ложь);
		КонецЕсли;
	КонецЕсли;
	...
	Если ОбработчикДействияПослеЗаписи <> Неопределено Тогда
		ВыполнитьОбработкуОповещения(ОбработчикДействияПослеЗаписи, Истина);
		ОбработчикДействияПослеЗаписи = Неопределено;
	КонецЕсли;
		...
		Если ЕстьНедействительныеСертификаты Тогда
			Если ТолькоПросмотр Тогда
				Если Не ВыполняетсяЗакрытие Тогда
					Отказ = Истина;
					ПредупредитьОНедействительныхСертификатах(Истина);
				КонецЕсли;
				Возврат;
			КонецЕсли;
			Если Не ДанОтветНаВопросОНедействительныхСертификатахПриОткрытии Тогда
				Отказ = Истина;
			ИначеЕсли Не ВыполняетсяЗакрытие Тогда
				Отказ = Истина;
				СпроситьОбУдаленииНедействительныхСертификатов(Истина);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ОбработкаОповещенияВыгрузкиВЦентрЭДО(ИмяСобытия, Параметр);
...
Процедура ДекорацияАдресОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Запись.АдресОрганизацииЗначение = АдресОрганизацииЗначение;
...
&НаКлиенте
Процедура НадписьПереходВЦентрЭДООбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	
	Если НавигационнаяСсылка = ИдентификаторСсылкиОткрытьФормуСостоянияВыгрузкиВЦентрЭДО() Тогда
		СтандартнаяОбработка = Ложь;
		ЭлектронныеДокументыЭДОИнтеграцияЦентраЭДОКлиент.ОткрытьФормуСостоянияВыгрузкиВЦентрЭДО(
			Запись.ИдентификаторЭДО, ЭтотОбъект);
	КонецЕсли;
...
Процедура СписокСертификатовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элементы.СписокСертификатов.ТекущийЭлемент = Элементы.СписокСертификатовДоверенность Тогда
		...
		КонецЕсли;
	ИначеЕсли Элементы.СписокСертификатов.ТекущийЭлемент = Элементы.СписокСертификатовДолжность Тогда
		Поле.ТолькоПросмотр = Не ТекущиеДанные.ТребуетсяЗаполнитьДолжность;
	Иначе 
...
Процедура СписокСертификатовПриАктивизацииЯчейки(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ДолжностьПоСертификату) Тогда
		Элементы.СписокСертификатовДолжность.СписокВыбора.ЗагрузитьЗначения(
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТекущиеДанные.ДолжностьПоСертификату));
	Иначе
		Элементы.СписокСертификатовДолжность.СписокВыбора.Очистить();
	КонецЕсли;
...
Процедура СписокСертификатовПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Отказ = Истина;
	СписокСертификатовПередНачаломДобавленияПродолжение();
	
КонецПроцедуры

&НаКлиенте
Процедура СписокСертификатовПередНачаломДобавленияПродолжение()
...
&НаКлиенте
Процедура СписокСертификатовДолжностьПриИзменении(Элемент)
	Модифицированность = Истина;
	ИзмененСоставСертификатов = Истина;
КонецПроцедуры
...
&НаКлиенте
Процедура ДобавитьИзСервисаМояПодпись(Команда)

	ДобавитьСертификатИзСервисовМобильногоПодписания(
		ПредопределенноеЗначение("Перечисление.ВидыМобильныхПодписей.МояПодпись"));
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьИзСервисаГосКлючУКЭП(Команда)
	
	ДобавитьСертификатИзСервисовМобильногоПодписания(
		ПредопределенноеЗначение("Перечисление.ВидыМобильныхПодписей.ГосключУКЭП"));
...
&НаКлиенте
Процедура ПереходВЦентрЭДО(Команда)
	...
	ОбработчикДействияПослеЗаписи = Новый ОписаниеОповещения("ПерейтиВЦентрЭДОПослеЗаписи", ЭтотОбъект);
...
&НаКлиенте
Процедура ПереносДокументовВЦентрЭДО(Команда)
	
	Если Не Модифицированность Тогда
		ПеренестиДокументыВЦентрЭДО();
		Возврат;
	КонецЕсли;
	
	ОбработчикДействияПослеЗаписи = Новый ОписаниеОповещения("ПеренестиДокументыВЦентрЭДОПослеЗаписи", ЭтотОбъект);
...
Процедура ВыполнитьСохранениеДанныхВСервисахПослеПолученияВсехСертификатов(ДанныеСертификатов, Этапы) Экспорт
	...
	Если Запись.ИспользоватьЦентрЭДО Тогда
		ПараметрыСохранения = НовыеПараметрыСохраненияДанныхЧерезЦентрЭДО();
		ПараметрыСохранения.ДанныеСертификатов = ДанныеСертификатов;
		ПараметрыСохранения.Этапы = Этапы;
		СохранитьДанныеЧерезЦентрЭДО(ПараметрыСохранения);
		Возврат;
	КонецЕсли;
	...
	Если Результат <> Неопределено
		...
		Если ЗначениеЗаполнено(СостояниеЗаявки.ИдентификаторОрганизации) Тогда
			...
			Если СостояниеЗаявки.Результат Тогда
				...
				ИзмененСоставСертификатов = Ложь;
				ИзмененыРегистрационныеДанные = Ложь;
				ВключитьАвтоматическоеПолучениеЭДО(СостояниеЗаявки.ИдентификаторОрганизации);
			Иначе
				Закрыть = Ложь;
...
Процедура ВключитьАвтоматическоеПолучениеЭДО(ИдентификаторЭДО)
	...
	Если Не ИспользоватьАвтоматическоеПолучениеЭДО() Тогда
		Возврат;
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ВключитьАвтоматическоеПолучениеЭДОЗавершение", ЭтотОбъект);
...
Функция ИспользоватьАвтоматическоеПолучениеЭДО()
	Возврат АвтоматическоеПолучениеЭДО.Использовать();
КонецФункции
...
&НаКлиенте
Процедура ВключитьАвтоматическоеПолучениеЭДОЗавершение(Результат, Контекст = Неопределено) Экспорт
...
#Область СохранениеДанныхЧерезЦентрЭДО
...
&НаКлиенте
Функция НовыеПараметрыСохраненияДанныхЧерезЦентрЭДО()
...
// Параметры:
//  ПараметрыСохранения - см. НовыеПараметрыСохраненияДанныхЧерезЦентрЭДО
&НаКлиенте
Процедура СохранитьДанныеЧерезЦентрЭДО(ПараметрыСохранения)
	
	Этапы = ПараметрыСохранения.Этапы;
	
	Если Этапы.ИзмененСоставСертификатов ИЛИ Этапы.ИзмененыРегистрационныеДанные Тогда
		
		ЗарегистрироватьСертификатЧерезЦентрЭДО(ПараметрыСохранения);
		
	ИначеЕсли Этапы.ИзмененыНастройкиУведомлений Тогда
		
		ОбновитьНастройкиУведомленийЧерезЦентрЭДО(ПараметрыСохранения);
		 
	ИначеЕсли Этапы.ИзмененыПараметрыУчетнойЗаписи Тогда
		
		ОбновитьПараметрыУчетнойЗаписиЧерезЦентрЭДО(ПараметрыСохранения);
...
//  ПараметрыСохранения - см. НовыеПараметрыСохраненияДанныхЧерезЦентрЭДО
&НаКлиенте
Процедура ЗарегистрироватьСертификатЧерезЦентрЭДО(ПараметрыСохранения)
	
	ДанныеСертификатов = ПараметрыСохранения.ДанныеСертификатов;
	
	ОбщийМодульУчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("УчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент");
	
	ПараметрыРегистрации = ОбщийМодульУчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент.НовыеПараметрыРегистрацииСертификатовВЭДО();
	...
	ПараметрыРегистрации.ИдентификаторОрганизации = Запись.ИдентификаторЭДО;
	ПараметрыРегистрации.Доверенности = ДанныеСертификатов.Доверенности;
	ПараметрыРегистрации.Должности = ДанныеСертификатов.Должности;
	ПараметрыРегистрации.СертификатыДляПодписания = ДанныеСертификатов.ОтборСертификатов;
	
	Оповещение = Новый ОписаниеОповещения("ЗарегистрироватьСертификатЧерезЦентрЭДОЗавершение", ЭтотОбъект,
		ПараметрыСохранения);
		
	ОбщийМодульУчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент.НачатьОтправкуСертификатовДляРегистрацииВЭДО(
...
//  ПараметрыСохранения - см. НовыеПараметрыСохраненияДанныхЧерезЦентрЭДО
&НаКлиенте
Процедура ЗарегистрироватьСертификатЧерезЦентрЭДОЗавершение(Результат, ПараметрыСохранения) Экспорт
	
	Если Результат.Успех Тогда
		УникальныйИдентификаторЗаявки1СЭДО = Результат.ИдентификаторЗаявки;
		ПаролиСертификатов                 = Результат.ПаролиСертификатов;
		ОжиданиеОперацийБЭДКлиент.ИнициализироватьПараметрыОбработчикаОжидания(ПараметрыОбработчикаОжидания);
		ПодключитьОбработчикОжидания("Подключаемый_ПроверитьРезультатРегистрацииСертификатЧерезЦентрЭДО",
			ПараметрыОбработчикаОжидания.ТекущийИнтервал, Истина);
...
&НаКлиенте
Процедура Подключаемый_ПроверитьРезультатРегистрацииСертификатЧерезЦентрЭДО()
	...
	ОбщийМодульУчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("УчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент");
	ОбщийМодульУчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент.НачатьПолучениеРезультатаРегистрацииСертификатовЭДО(
...
//  ПараметрыСохранения - см. НовыеПараметрыСохраненияДанныхЧерезЦентрЭДО
&НаКлиенте
Процедура ОбновитьНастройкиУведомленийЧерезЦентрЭДО(ПараметрыСохранения)
	...
	ОбщийМодульУчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("УчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент");
	ПараметрыОбновления = ОбщийМодульУчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент.НовыеПараметрыОбновленияНастройкиУведомленийУчетнойЗаписиЭДО();
	ПараметрыОбновления.ОповещениеОЗавершении = Оповещение;
	ПараметрыОбновления.УчетнаяЗаписьЦентраЭДО = УчетнаяЗаписьЦентраЭДО;
	ПараметрыОбновления.ИдентификаторЭДО = Запись.ИдентификаторЭДО;
	...
	ОбщийМодульУчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент.ОбновитьНастройкиУведомленийУчетнойЗаписиЭДО(ПараметрыОбновления);
...
//  ПараметрыСохранения - см. НовыеПараметрыСохраненияДанныхЧерезЦентрЭДО
&НаКлиенте
Процедура ОбновитьПараметрыУчетнойЗаписиЧерезЦентрЭДО(ПараметрыСохранения)
	
	Оповещение = Новый ОписаниеОповещения("ОбновитьПараметрыУчетнойЗаписиЧерезЦентрЭДОЗавершение",
		ЭтотОбъект, ПараметрыСохранения);
	ПараметрыОбновления = Новый Структура;
	ПараметрыОбновления.Вставить("ИдентификаторЭДО", Запись.ИдентификаторЭДО);
	ПараметрыОбновления.Вставить("Наименование", Запись.НаименованиеУчетнойЗаписи);
	ПараметрыОбновления.Вставить("Назначение", Запись.НазначениеУчетнойЗаписи);
	ПараметрыОбновления.Вставить("ПодробноеОписание", Запись.ПодробноеОписаниеУчетнойЗаписи);
	ОбщийМодульУчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("УчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент");
	ОбщийМодульУчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент.ОбновитьИнформациюОбУчетнойЗаписиЭДО(
...
//  ПараметрыСохранения - см. НовыеПараметрыСохраненияДанныхЧерезЦентрЭДО
&НаКлиенте
Процедура ОбновитьПараметрыУчетнойЗаписиЧерезЦентрЭДОЗавершение(Результат, ПараметрыСохранения) Экспорт
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СписокСертификатов.Отозван");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
	...
	ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СписокСертификатов.ТребуетсяЗаполнитьДолжность");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
	
	ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СписокСертификатов.Должность");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	
	ПолеЭлемента = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных("СписокСертификатовДолжность");
...
Процедура ЗаполнитьСрокДействияСертификата(Форма)
	...
	Для Каждого СтрокаТЧ Из Форма.СписокСертификатов Цикл
		...
		Если СтрокаТЧ.Отозван Тогда
			СрокДействия = НСтр("ru = 'Отозван'");
		КонецЕсли;
...
Процедура ПодготовитьФормуНаСервере()
	...
	Если ЕстьНедействительныеСертификаты Тогда
		Элементы.СтраницыФормы.ТекущаяСтраница = Элементы.СтраницаСертификаты;
	ИначеЕсли ЗначениеЗаполнено(Параметры.ТекущаяСтраницаФормы) Тогда
		ТекущаяСтраница = Элементы.Найти(Параметры.ТекущаяСтраницаФормы);
		Элементы.СтраницыФормы.ТекущаяСтраница = ?(ТекущаяСтраница = Неопределено, Элементы.СтраницаОсновное, ТекущаяСтраница);
	КонецЕсли;
...
Процедура ЗарегистрироватьСертификатТакскомОповещение(РезультатВыполнения, Контекст) Экспорт
	...
	Если РезультатВыполнения = Истина Тогда
		...
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Подключение1СТакском") Тогда
			...
			МодульПодключение1СТакскомКлиент.ЗарегистрироватьСертификатАбонента(Контекст.ДобавляемыйСертификат,
				Запись.ИдентификаторЭДО,
				Запись.Организация,
				Новый ОписаниеОповещения("ЗарегистрироватьСертификатТакскомЗавершение", ЭтотОбъект, Контекст), ДополнительныеОбработчики);
		Иначе
			Подключение1СТакскомКлиентБЭД.ЗарегистрироватьСертификатАбонента(Контекст.ДобавляемыйСертификат,
				Запись.ИдентификаторЭДО,
				Запись.Организация,
				Новый ОписаниеОповещения("ЗарегистрироватьСертификатТакскомЗавершение", ЭтотОбъект, Контекст), ДополнительныеОбработчики);
		КонецЕсли;
...
Процедура ЗаписатьСертификатТакском(Знач Контекст)
	...
	ДобавитьСертификатыУчетныхЗаписейЭДО(Запись.ИдентификаторЭДО,
		СписокСертификатов.Выгрузить(, "Сертификат, Доверенность, Должность"));
...
&НаСервере
Функция ЗаВремяРедактированияВФормеБылПереводВХАБ() 
	Если Не Запись.СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ДанныеУчетнойЗаписи = УчетныеЗаписиЭДО.ДанныеУчетнойЗаписи(Запись.ИдентификаторЭДО); 
	Возврат ДанныеУчетнойЗаписи.СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО;
...
Функция ПроверитьЗаполнениеРеквизитов()
	...
	Если Не ИмеютсяДействующиеСертификаты() Тогда
		
		ОшибкаЗаполнения = Истина;
		СпроситьОДобавленииДействующегоСертификата();
		
	КонецЕсли;
	
	ПроверитьЗаполненностьДоверенностей(ОшибкаЗаполнения);
	ПроверитьЗаполненностьДолжностей(ОшибкаЗаполнения);
...
Процедура ЗарегистрироватьДобавитьСертификат(ВыбранноеЗначение, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Запись.СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезОператораЭДОТакском") Тогда
		Если Не РазделениеВключено()
				И ИнтеграцияБИПБЭДКлиент.ИспользуетсяИнтернетПоддержкаПользователей() Тогда
...
Процедура ДобавитьСертификатыУчетныхЗаписейЭДО(Знач ИдентификаторЭДО, Знач Сертификаты)
	...
	Для Каждого СтрокаТаблицы Из Сертификаты Цикл
		...
		Должности.Вставить(СтрокаТаблицы.Сертификат, СтрокаТаблицы.Должность);
	КонецЦикла;
	УчетныеЗаписиЭДОСлужебный.ЗаписатьСертификатыУчетнойЗаписи(ИдентификаторЭДО, 
		Сертификаты.ВыгрузитьКолонку("Сертификат"), , Доверенности, Должности);
...
Процедура ДобавитьСертификатВСписок(ДобавляемыйСертификат, ТребуетсяСинхронизация)
	...
	ЗаполнитьЗначенияСвойств(СтрокаСертификата, СвойстваСертификатов[ДобавляемыйСертификат],, "Должность");
	...
	СтрокаСертификата.ДолжностьПоСертификату = СвойстваСертификатов[ДобавляемыйСертификат].Должность;
	СтрокаСертификата.ТребуетсяЗаполнитьДолжность = 
		УчетныеЗаписиЭДОСлужебный.ТребуетсяЗаполнитьДолжностьДляСертификата(Запись.Организация, ДобавляемыйСертификат);
	Если СтрокаСертификата.ТребуетсяЗаполнитьДолжность Тогда
		СтрокаСертификата.Должность = СвойстваСертификатов[ДобавляемыйСертификат].Должность;
	КонецЕсли;
	ЗаполнитьСрокДействияСертификата(ЭтотОбъект);
...
Функция ДанныеСертификатов(Знач СписокСертификатов, ДоступныеСертификаты)
	...
	Результат = Новый Структура("Список, ОтборСертификатов, Доверенности, Должности");
	Результат.Доверенности = Новый Соответствие;
	Результат.Должности = Новый Соответствие;
	...
	Для каждого СтрокаТаблицы Из ТаблицаВсехСертификатов Цикл
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаТаблицы.Должность) Тогда
			Результат.Должности.Вставить(СтрокаТаблицы.Сертификат, СтрокаТаблицы.Должность);
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Процедура ПроверкаСертификатовНаДействительность()
	Параметры.Свойство("ЕстьНедействительныеСертификаты", ЕстьНедействительныеСертификаты);
	Если Не ЕстьНедействительныеСертификаты Тогда
		ЕстьНедействительныеСертификаты = УчетныеЗаписиЭДО.ЕстьНедействительныеСертификаты(Запись.ИдентификаторЭДО, Истина);	
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура СпроситьОбУдаленииНедействительныхСертификатов(ЗакрытьФормуПриОтказе)
	
	ИдентификаторОператора = УчетныеЗаписиЭДОКлиентСервер.ИдентификаторОператора(Запись.ИдентификаторЭДО);
	ОператорКонтролируетНедействительныеСертификаты =
		СервисНастроекЭДОВызовСервера.ОператорКонтролируетНедействительныеСертификаты(ИдентификаторОператора);
	ОповещениеУдалениеНедействительныхСертификатов
		= Новый ОписаниеОповещения("ВопросОбУдаленииНедействительныхСертификатовЗавершение", ЭтотОбъект, ЗакрытьФормуПриОтказе);
	ПоказатьВопрос(ОповещениеУдалениеНедействительныхСертификатов,
		УчетныеЗаписиЭДОСлужебныйКлиент.ТекстВопросаУдаленияНедействительныхСертификатов(ОператорКонтролируетНедействительныеСертификаты),
		РежимДиалогаВопрос.ДаНет);
	
КонецПроцедуры

&НаКлиенте
Процедура ВопросОбУдаленииНедействительныхСертификатовЗавершение(Результат, ЗакрытьФормуПриОтказе) Экспорт
	
	ДанОтветНаВопросОНедействительныхСертификатахПриОткрытии = Истина;
	...
	ИначеЕсли Результат = КодВозвратаДиалога.Да Тогда
		УдалитьНедействительныеСертификатыНаСервере();
		Если Не ИмеютсяДействующиеСертификаты() И Не ЗакрытьФормуПриОтказе Тогда
			СпроситьОДобавленииДействующегоСертификата();
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ПредупредитьОНедействительныхСертификатах(ЗакрытиеФормы)
	
	ИдентификаторОператора = УчетныеЗаписиЭДОКлиентСервер.ИдентификаторОператора(Запись.ИдентификаторЭДО);
	ОператорКонтролируетНедействительныеСертификаты =
		СервисНастроекЭДОВызовСервера.ОператорКонтролируетНедействительныеСертификаты(ИдентификаторОператора);
	ОповещениеОНедействительныхСертификатах
		= Новый ОписаниеОповещения("ПредупредитьОНедействительныхСертификатахЗавершение", ЭтотОбъект, ЗакрытиеФормы);
	ПоказатьПредупреждение(ОповещениеОНедействительныхСертификатах, 
		УчетныеЗаписиЭДОСлужебныйКлиент.ТекстПредупрежденияОНедействительныхСертификатах(
			ОператорКонтролируетНедействительныеСертификаты));
	
КонецПроцедуры
		
&НаКлиенте
Процедура ПредупредитьОНедействительныхСертификатахЗавершение(ЗакрытиеФормы) Экспорт
...
&НаКлиенте
Асинх Процедура СпроситьОДобавленииДействующегоСертификата()
	
	Ответ = Ждать ВопросАсинх(УчетныеЗаписиЭДОСлужебныйКлиент.ТекстВопросаОДобавленииДействующегоСертификата(),
		РежимДиалогаВопрос.ДаНет);
...
&НаСервере
Процедура УдалитьНедействительныеСертификатыНаСервере()
	
	ИндексСертификатов = СписокСертификатов.Количество() - 1;
	Пока ИндексСертификатов >= 0 Цикл
		Сертификат = СписокСертификатов.Получить(ИндексСертификатов);
		Если (Сертификат.СрокДействияВДнях < 0 Или Сертификат.Отозван) И Не Сертификат.Скрыть Тогда
			СписокСертификатов.Удалить(ИндексСертификатов);
		КонецЕсли;
		ИндексСертификатов = ИндексСертификатов - 1;
	КонецЦикла;
...
&НаКлиенте
Процедура ПроверитьНаличиеНедействительныхСертификатов()
	ЕстьНедействительныеСертификаты = Ложь;
	Для Каждого Сертификат Из СписокСертификатов Цикл
		Если (Сертификат.СрокДействияВДнях < 0 Или Сертификат.Отозван) И Не Сертификат.Скрыть Тогда
			ЕстьНедействительныеСертификаты = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
&НаКлиенте
Функция ИмеютсяДействующиеСертификаты()
...
&НаКлиенте
Процедура ПроверитьЗаполненностьДоверенностей(ОшибкаЗаполнения)
...
&НаКлиенте
Процедура ПроверитьЗаполненностьДолжностей(ОшибкаЗаполнения)
	
	ВыбранныеСертификаты = СписокСертификатов.НайтиСтроки(Новый Структура("Скрыть", Ложь));
	Счетчик = 0;
	Для Каждого СтрокаСертификата Из ВыбранныеСертификаты Цикл
		ТребуетсяЗаполнениеДолжности = СтрокаСертификата.ТребуетсяЗаполнитьДолжность;
		Если ТребуетсяЗаполнениеДолжности И Не ЗначениеЗаполнено(СтрокаСертификата.Должность) Тогда
			ОшибкаЗаполнения = Истина;
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Не заполнена должность для сертификата %1'"),
				СтрокаСертификата.Представление);
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,
				СтрШаблон("СписокСертификатов[%1].Должность", Формат(Счетчик, "ЧГ=;")));
		КонецЕсли;
		Счетчик = Счетчик + 1;
	КонецЦикла;
...
Процедура ПерейтиВЛичныйКабинетОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(СертификатПодписи) Тогда
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Подключение1СТакском") Тогда
			МодульПодключение1СТакскомКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("Подключение1СТакскомКлиент");
			МодульПодключение1СТакскомКлиент.ОткрытьЛичныйКабинетАбонента(СертификатПодписи);
		Иначе
			Подключение1СТакскомКлиентБЭД.ОткрытьЛичныйКабинетАбонента(СертификатПодписи);
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура НастроитьОтображениеКнопкиПереходаВЦентрЭДО(ЕстьПравоНастройкиОбмена, ИспользоватьИнтеграциюЦентраЭДО,
		СпособОбменаЭД, НастройкиИнтеграции)
	
	Если Не ЕстьПравоНастройкиОбмена Или Не ИспользоватьИнтеграциюЦентраЭДО
		Или СпособОбменаЭД <> Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО Тогда 
		Элементы.ПереходВЦентрЭДО.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(НастройкиИнтеграции) Тогда
		Элементы.ПереходВЦентрЭДО.Видимость = Истина;
		Возврат;
	КонецЕсли;
	
	Элементы.ПереходВЦентрЭДО.Видимость = ЗначениеЗаполнено(НастройкиИнтеграции.УчетнаяЗаписьЦентраЭДО)
		И НастройкиИнтеграции.ТребуетсяЗавершитьПереходВЦентрЭДО;
	
КонецПроцедуры

&НаСервере
Процедура НастроитьОтображениеКнопкиПереносДокументовВЦентрЭДО(ЕстьПравоНастройкиОбмена, ИспользоватьИнтеграциюЦентраЭДО,
	СпособОбменаЭД, НастройкиИнтеграции)
	
	Если Не ЕстьПравоНастройкиОбмена Или Не ИспользоватьИнтеграциюЦентраЭДО
		Или СпособОбменаЭД <> Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО
		Или Не ЗначениеЗаполнено(НастройкиИнтеграции) Тогда
		Элементы.ПереносДокументовВЦентрЭДО.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	
	Элементы.ПереносДокументовВЦентрЭДО.Видимость = ЗначениеЗаполнено(НастройкиИнтеграции.УчетнаяЗаписьЦентраЭДО)
		И Не НастройкиИнтеграции.ТребуетсяЗавершитьПереходВЦентрЭДО;
	
КонецПроцедуры

&НаСервере
Процедура НастроитьОтображениеСостоянияВыгрузкиВЦентрЭДО()
	
	Элементы.ГруппаПереходВЦентрЭДО.Видимость = Ложь;
	
	Если Не ЗначениеЗаполнено(УчетнаяЗаписьЦентраЭДО) Тогда
		Возврат;
	КонецЕсли;
	
	Состояние = СостояниеВыгрузкиВЦентрЭДО(Запись.ИдентификаторЭДО);
	Если Не ЗначениеЗаполнено(Состояние.ДатаНачала) Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Состояние.ДатаЗавершения) Тогда
		ОбновлятьСостояниеВыгрузкиВЦентрЭДО = Истина;
	КонецЕсли;
	
	ОбновитьОтображениеСостоянияВыгрузкиВЦентрЭДО(ЭтотОбъект, Состояние);
	
КонецПроцедуры

// Параметры:
//  Форма - см. РегистрСведений.УчетныеЗаписиЭДО.Форма.УчетнаяЗапись
//  Состояние - см. СостояниеВыгрузкиВЦентрЭДО
&НаКлиентеНаСервереБезКонтекста
Процедура ОбновитьОтображениеСостоянияВыгрузкиВЦентрЭДО(Форма, Состояние)
	...
	Элементы.ГруппаПереходВЦентрЭДО.Видимость = Истина;
	
	ЧастиЗаголовка = Новый Массив; // Массив из Строка,ФорматированнаяСтрока
	ЧастиЗаголовка.Добавить(НСтр("ru = 'Состояние выгрузки в центр ЭДО:'"));
	ЧастиЗаголовка.Добавить(" ");
	
	ТекстВыгружено = СтрШаблон(НСтр("ru = 'Выгружено %1%%'"), Состояние.Прогресс);
	ЧастиЗаголовка.Добавить(Новый ФорматированнаяСтрока(ТекстВыгружено, , , ,
		ИдентификаторСсылкиОткрытьФормуСостоянияВыгрузкиВЦентрЭДО()));
	
	Элементы.НадписьПереходВЦентрЭДО.Заголовок = Новый ФорматированнаяСтрока(ЧастиЗаголовка);
...
//  Строка
&НаКлиентеНаСервереБезКонтекста
Функция ИдентификаторСсылкиОткрытьФормуСостоянияВыгрузкиВЦентрЭДО()
	
	Возврат "ОткрытьФормуСостоянияВыгрузкиВЦентрЭДО";
	
КонецФункции

&НаКлиенте
Процедура ОбновитьСостояниеВыгрузкиВЦентрЭДОЧерезИнтервал()
	
	ПодключитьОбработчикОжидания("Подключаемый_ОбновитьСостояниеВыгрузкиВЦентрЭДО", 15, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ОбновитьСостояниеВыгрузкиВЦентрЭДО()
	
	Состояние = СостояниеВыгрузкиВЦентрЭДО(Запись.ИдентификаторЭДО);
	
	ОбновитьСостояниеВыгрузкиВЦентрЭДО(Состояние);
...
//  Состояние - см. СостояниеВыгрузкиВЦентрЭДО
&НаКлиенте
Процедура ОбновитьСостояниеВыгрузкиВЦентрЭДО(Состояние)
...
&НаСервереБезКонтекста
Функция СостояниеВыгрузкиВЦентрЭДО(Знач ИдентификаторЭДО)
...
//  Состояние - см. СостояниеВыгрузкиВЦентрЭДО
&НаКлиенте
Процедура ОбработкаОповещенияВыгрузкиВЦентрЭДО(ИмяСобытия, Состояние)
	
	Если ИмяСобытия = ИнтеграцияЦентраЭДОКлиент.ИмяСобытияОповещенияОбновлениеСостоянияВыгрузкиВЦентрЭДО() Тогда
...
&НаКлиенте
Процедура ПерейтиВЦентрЭДОПослеЗаписи(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ПерейтиВЦентрЭДО()
	...
	Оповещение = Новый ОписаниеОповещения("ПерейтиВЦентрЭДОЗавершение", ЭтотОбъект);
	
	МодульИнтеграцияЦентраЭДОКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ИнтеграцияЦентраЭДОКлиент");
	ПараметрыПерехода = МодульИнтеграцияЦентраЭДОКлиент.НовыеПараметрыПереходаВЦентрЭДО();
	ПараметрыПерехода.ИдентификаторЭДО = Запись.ИдентификаторЭДО;
	Если ЗначениеЗаполнено(УчетнаяЗаписьЦентраЭДО) Тогда
		ПараметрыПерехода.УчетнаяЗаписьЦентраЭДО = УчетнаяЗаписьЦентраЭДО;
	КонецЕсли;
	
	МодульИнтеграцияЦентраЭДОКлиент.ПерейтиВЦентрЭДО(Оповещение, ПараметрыПерехода);
	
КонецПроцедуры

// Параметры:
//  Результат - См. ИнтеграцияЦентраЭДОКлиент.ПерейтиВЦентрЭДОЗавершение.Результат
//  ДополнительныеПараметры - Неопределено
&НаКлиенте
Процедура ПерейтиВЦентрЭДОЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
#Область ПереносДокументовВЦентрЭДО

// Параметры:
//  Результат - Булево
//  ДополнительныеПараметры - Неопределено
&НаКлиенте
Процедура ПеренестиДокументыВЦентрЭДОПослеЗаписи(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ПеренестиДокументыВЦентрЭДО()
	
	МодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДОКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
		"ЭлектронныеДокументыЭДОИнтеграцияЦентраЭДОКлиент");
	МодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДОКлиент.ЗапуститьВыгрузкуВЦентрЭДО(
		Запись.ИдентификаторЭДО, ЭтотОбъект);
...
&НаСервере
Процедура НастроитьВидимостьКнопокДобавитьИзСервисовМобильногоПодписания()

	ДоступноИспользованиеСервисовМобильногоПодписания = 
		ИнтеграцияБСПБЭД.ДоступноИспользованиеСервисовМобильногоПодписания();

	Элементы.ДобавитьИзСервисовМобильногоПодписания.Видимость =
		ДоступноИспользованиеСервисовМобильногоПодписания;

	Если ДоступноИспользованиеСервисовМобильногоПодписания Тогда
		Элементы.СписокСертификатовДобавитьИзСервисаМояПодпись.Видимость =
			ИнтеграцияБСПБЭД.ДоступноИспользованиеСервисаМояПодпись();
		Элементы.СписокСертификатовДобавитьИзСервисаГосКлючУКЭП.Видимость =
			ИнтеграцияБСПБЭД.ДоступноИспользованиеСервисаГосключ();
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ДобавитьСертификатИзСервисовМобильногоПодписания(ВидМобильнойПодписи)
	
	ЗарегистрироватьДобавитьСертификат = Новый ОписаниеОповещения(
		"ЗарегистрироватьДобавитьСертификат",
		ЭтотОбъект,
		);
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ВидМобильнойПодписи", ВидМобильнойПодписи);
	
	ОткрытьФорму("Обработка.ПодписаниеДанныхВМобильномЭДО.Форма.ФормаПодписанияСоглашенияНаЗагрузкуСертификата",
		ПараметрыФормы,
		ЭтотОбъект,,,,
		ЗарегистрироватьДобавитьСертификат,
