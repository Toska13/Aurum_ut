...
Процедура ЗарегистрироватьСчетаФактурыОжидаетОплатыНДС(СчетаФактуры, ДокументРегистратор, Проведен) Экспорт
	...
	СчетаФактуры.Свернуть("СчетФактура,ВидАгентскогоДоговора,ОплатаЧерезЕдиныйЛицевойСчет,ДатаДокументаПеречисленияНалога","СуммаОплаты");
...
Функция ПолучитьСуммуНДСКОплатеВБюджет(СчетФактура) Экспорт
	...
	Если ТипЗнч(СчетФактура) = Тип("ДокументСсылка.ЗаявлениеОВвозеТоваров") Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ОкруглятьСуммыНДСиАкцизовДоЦелыхВЗаявленияхОВвозеТоваров") Тогда
			Возврат Окр(Товары.Итог("СуммаНДС"));
		Иначе
			Возврат Товары.Итог("СуммаНДС");
		КонецЕсли;
	ИначеЕсли ТипЗнч(СчетФактура) = Тип("ДокументСсылка.ТаможеннаяДекларацияИмпорт") Тогда
			Товары = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СчетФактура, "Товары").Выгрузить();
			Возврат Товары.Итог("СуммаНДС");
...
Процедура ОбновитьСостояниеПодтвержденияОплаты(СчетаФактуры, Проведен)
	...
	Если Проведен Тогда
		...
		|	СчетаФактуры.ДатаДокументаПеречисленияНалога,
		|	СчетаФактуры.СуммаОплаты
		|ПОМЕСТИТЬ СчетаФактуры
		|ИЗ
		|	&СчетаФактуры КАК СчетаФактуры
		...
		|	ВЫБОР
		|		КОГДА СчетаФактуры.ОплатаЧерезЕдиныйЛицевойСчет
		|			ТОГДА ЕСТЬNULL(Оплачено.ДатаПодтвержденияОплаты, СчетаФактуры.ДатаДокументаПеречисленияНалога)
		|		ИНАЧЕ
		|			ЕСТЬNULL(Оплачено.ДатаПодтвержденияОплаты, ДАТАВРЕМЯ(1,1,1))
		|	КОНЕЦ КАК ДатаПодтвержденияОплаты
		|ИЗ
		|	СчетаФактуры КАК СчетаФактуры
