...
Процедура СоздатьЗаданияПоДаннымТаблицыЗначений(ТаблицаФормированияЗаданий) Экспорт
	...
	Для Каждого ТекущиеДанные Из ТаблицаФормированияЗаданий Цикл
		...
		НовоеЗадание = Задания.Добавить();
		НовоеЗадание.ПериодЗадания = ТекущиеДанные.ПериодЗадания;
		НовоеЗадание.Организация = ТекущиеДанные.Организация;
		НовоеЗадание.ВидЗадания = ТекущиеДанные.ВидЗадания;
		НовоеЗадание.Документ = ТекущиеДанные.Документ;
		НовоеЗадание.ИдентификаторЗаписи = Новый УникальныйИдентификатор;
...
Процедура ВыполнитьОбработкуПорции(ПараметрыДанных) Экспорт
	...
	ВидыЗаданийНедоступныеКПакетнойОбработке = НеоперативнаяОчередьЗаданийПовтИсп.ВидыЗаданийНедоступныеКПакетнойОбработкеГранулРасчета();
	
	Если Не ОбщегоНазначения.ИнформационнаяБазаФайловая() И Не ПараметрыДанных.ВыполнениеЗаданийВТранзакцииПроведения
		И ВидыЗаданийНедоступныеКПакетнойОбработке.Найти(ПараметрыДанных.ВидЗадания) = Неопределено Тогда
