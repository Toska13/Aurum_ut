...
Функция СоздатьКлючАналитики(ПараметрыАналитики, ЗаписьПриОбновленииИБ = Ложь) Экспорт
	...
	Попытка
		...
		ПредставлениеКлюча = НаименованиеКлючаАналитики(СтрокаНабора);
		
		СправочникОбъект = Справочники.КлючиАналитикиУчетаНоменклатуры.СоздатьЭлемент();
		СправочникОбъект.Наименование = ПредставлениеКлюча;
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru = 'Создание ключа аналитики учета номенклатуры'");
		
		ТекстСообщения = НСтр("ru = 'Не удалось создать ключ аналитики учета номеклатуры ""%1"" по причине: %2'");
...
Функция МестаУчета(Операция, Склад, Подразделение, Партнер, Договор = Неопределено) Экспорт
	...
	ИспользоватьДоговор = 
		...
		Или (Операция = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности)
		...
		Или (Операция = Перечисления.ХозяйственныеОперации.ПоставкаПодПринципала)
		Или (Операция = Перечисления.ХозяйственныеОперации.ВводОстатковОтгруженныхТоваров);
