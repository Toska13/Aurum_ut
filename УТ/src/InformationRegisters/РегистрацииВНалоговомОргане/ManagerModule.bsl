...
Процедура ПоместитьВременнуюТаблицуДанныеПодразделений(МенеджерВременныхТаблиц) Экспорт
	...
	Запрос.Текст =
		"// Подразделения документов указанные явно и через связь склада с подразделением.
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ПодразделенияДокументов.Ссылка КАК Ссылка,
		|	ПодразделенияДокументов.Организация КАК Организация,
		|	ПодразделенияДокументов.Период КАК Период,
		|	ПодразделенияДокументов.Подразделение КАК Подразделение
		|ПОМЕСТИТЬ СписокПодразделений
		|ИЗ 
		|	(ВЫБРАТЬ
		|		ТаблицаДанныхДокументов.Ссылка КАК Ссылка,
		|		ТаблицаДанныхДокументов.Организация КАК Организация,
		|		ТаблицаДанныхДокументов.Период КАК Период,
		|		ТаблицаДанныхДокументов.Подразделение КАК Подразделение
		|	ИЗ
		...
		|		ТаблицаДанныхДокументов.Организация КАК Организация,
		|		ТаблицаДанныхДокументов.Период КАК Период,
		|		ТаблицаДанныхДокументов.Склад.Подразделение КАК Подразделение
		|	ИЗ
		|		ТаблицаДанныхДокументов КАК ТаблицаДанныхДокументов)
		|	КАК ПодразделенияДокументов
		|ГДЕ
		|	ПодразделенияДокументов.Подразделение <> ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
		|;
		|
		|ВЫБРАТЬ
		|	СписокПодразделений.Ссылка                КАК Ссылка,
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СписокПодразделений КАК СписокПодразделений
		|		ПО ДанныеРегистрацийОрганизаций.Подразделение = СписокПодразделений.Подразделение
		|		И ДанныеРегистрацийОрганизаций.Организация = СписокПодразделений.Организация
		|		И ДанныеРегистрацийОрганизаций.Период <= СписокПодразделений.Период
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|// Основная регистрация в налоговом органе.
		|ВЫБРАТЬ
		|	ТаблицаДанныхДокументов.Ссылка                КАК Ссылка,
		|	ДанныеРегистрацийОрганизаций.Организация      КАК Организация,
		|	МАКСИМУМ(ДанныеРегистрацийОрганизаций.Период) КАК Период
