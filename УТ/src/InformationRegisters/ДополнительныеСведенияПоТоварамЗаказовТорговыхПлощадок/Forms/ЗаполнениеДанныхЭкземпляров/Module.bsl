...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "МП_ОбновлениеСведенийПоЗаказамИОтправлениям" 
				И Источник = УчетнаяЗапись Тогда
		ОформитьДокументыОтгрузкиЗавершение(Параметр);
		
	ИначеЕсли ИмяСобытия = "Запись_ЗаказКлиента" И Источник = Заказ Тогда
		ЗаполнитьДанныеФормыПоЗаказу();
	КонецЕсли;
...
Процедура ТекстДокументыНаОснованииОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ОформитьДокументыОтгрузки" Тогда
		ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку("e1cib/command/РегистрСведений.ЗаказыТорговыхПлощадок.Команда.ОформитьДокументыОтгрузки");
	КонецЕсли;
...
Процедура СписокПриАктивизацииЯчейки(Элемент)
	...
	Если ДоступностьОчисткиЗначения Тогда
		...
		Если ТекущиеДанные <> Неопределено Тогда
			...
			Если СтрокиДанных.Количество() > 0 Тогда
				СтатусОтправления = СтрокиДанных[0].ДанныеОтправления.Статус;
				
				ДоступностьОчисткиЗначения =
					(СтатусОтправления = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.ОжидаетСборки"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "СписокУИН"
			...
			И Не ТекущиеДанные.Отменено Тогда
		Если ТекущиеДанные.Статус <> ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.ОжидаетСборки") Тогда
			Возврат;
		КонецЕсли;
...
Процедура ОтменитьСтроки(Команда)
	...
	Если Не ЗаказПроведен Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отмена невозможна, т.к. %1 не проведен. Для отмены строк проведите документ.'"),
			Заказ);
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, Заказ);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПолныйКодМаркировки");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<указан, но не найден в ИБ>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветПоясняющийТекст);
...
Процедура ЗаполнитьДанныеПоНомерамОтправлений(ЗначенияПараметров)
	...
	Для Каждого СоставДанных Из ЗначенияПараметров.ДанныеПоНомерамОтправлений Цикл
		...
		НоваяСтрока.ОтправлениеВДоставке              =
			(ЗначенияПараметров.ОтправленияВДоставке.Найти(НоваяСтрока.НомерОтправления) <> Неопределено);
		НоваяСтрока.ОтправлениеВКонечномСтатусе       =
			(ЗначенияПараметров.ОтправленияВКонечномСтатусе.Найти(НоваяСтрока.НомерОтправления) <> Неопределено);
	КонецЦикла;
...
Процедура ПриАктивизацииСтрокиСписка()
	...
	Если СтрокиДанных.Количество() = 0 Тогда
		...
		ЭтоОтправлениеВДоставке              = Ложь;
		ЭтоКонечныйСтатус                    = Ложь;
		ЭтоНовоеОтправление                  = Ложь;
		ЭтоОтправлениеСборки                 = Ложь;
		ЭтоОтправлениеОтменено               = Ложь;
	Иначе
		...
		ЭтоОтправлениеВДоставке              = ДанныеТекущегоОтправления.ОтправлениеВДоставке;
		ЭтоКонечныйСтатус                    = ДанныеТекущегоОтправления.ОтправлениеВКонечномСтатусе;
		ЭтоНовоеОтправление                  = ЭтоОтправлениеБезДокументаОтгрузки
												И (СтрНайти(ТекущиеДанные.НомерОтправления, Постфикс) <> 0);
		ЭтоОтправлениеСборки                 =
			(ДанныеОтправления.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.ОжидаетСборки"));
		ЭтоОтправлениеОтменено               =
			(ДанныеОтправления.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.Отменен"));
	КонецЕсли;
	
	Элементы.СписокОтменитьСтроки.Доступность                 = ЭтоОтправлениеСборки
	...
	Элементы.СписокПеренестиВОтправление.Доступность          = ЭтоОтправлениеСборки;
	Элементы.КонтекстноеМенюПеренестиВОтправление.Доступность = Элементы.СписокПеренестиВОтправление.Доступность;
	...
	Если ЭтоГруппировка Тогда
		...
		ИначеЕсли ЭтоОтправлениеВДоставке Тогда
			Подсказка = НСтр("ru = 'Отправление в доставке. Отслеживайте изменение статуса отправления.'");
		ИначеЕсли ЭтоОтправлениеОтменено Тогда
			Подсказка = НСтр("ru = 'Отправление отменено.'");
		ИначеЕсли ЭтоКонечныйСтатус Тогда
			Подсказка = НСтр("ru = 'Обработка отправления завершена.'");
		Иначе
	...
	Если ТекущиеДанные.Отменено Тогда
		...
		Иначе
			Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Товар отменен на торговой площадке и в документе заказа по причине: ""%1"".'"),
				?(ЗначениеЗаполнено(ТекущиеДанные.ПричинаОтмены), СокрЛП(ТекущиеДанные.ПричинаОтмены), НСтр("ru = '<не указано>'")));
		КонецЕсли;
		
		Возврат;
	ИначеЕсли ТекущиеДанные.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.Отменен") Тогда
		Подсказка = НСтр("ru = 'Товар отменен на торговой площадке.'");
		Возврат;
	КонецЕсли;
	...
	Если ТекущиеДанные.ТребуетсяМаркировка Тогда
		Если Не ЗначениеЗаполнено(ТекущиеДанные.ШтрихкодУпаковки)
				И Не ЗначениеЗаполнено(ТекущиеДанные.ПолныйКодМаркировки) Тогда
	...
	ИначеЕсли ПустаяСтрока(Подсказка) Тогда
		...
		Иначе
			Подсказка = НСтр("ru = 'Товар отгружен.'");
		КонецЕсли;
	КонецЕсли;
	
	Если Не ПустаяСтрока(ТекущиеДанные.ОписаниеОшибки) Тогда         
		ТекстСообщения = НСтр("ru = 'Требуется код маркировки для товара'");
		Если ТекущиеДанные.Статус <> ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.ОжидаетСборки")
			    И ЗначениеЗаполнено(ТекущиеДанные.ПолныйКодМаркировки)
				И СтрНайти(ТекущиеДанные.ОписаниеОшибки, ТекстСообщения) = 1 Тогда
			ДополнительнаяИнформация.Добавить(СтрЗаменить(ТекущиеДанные.ОписаниеОшибки, 
				ТекстСообщения, НСтр("ru = 'В документе отгрузки требуется указать код маркировки для товара'")));
		Иначе
...
&НаСервере
Процедура ЗаполнитьДанныеФормыПоЗаказу()
	
	РеквизитыЗаказа = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Заказ, "Проведен, Склад");
...
Процедура ПодготовитьНомерОтправленияДляПереноса(НомерОтправления, Заказ, ИсточникНомераОтправления)
	...
	Если ПустаяСтрока(НомерОтправления) Тогда
		...
		Запрос.Текст =
			...
			|	ДЛИНАСТРОКИ(СведенияПоТоварам.НомерОтправления) КАК ДлинаНомераОтправления
			|ИЗ
			...
			|
			|УПОРЯДОЧИТЬ ПО
			|	ДлинаНомераОтправления УБЫВ,
			|	МаксимальныйНомерНовогоОтправления УБЫВ";
...
Процедура ВопросОтменыЗавершение(Ответ, ОтменяемыеСтроки) Экспорт
	...
	ИначеЕсли ВидМаркетплейса = ПредопределенноеЗначение("Перечисление.ВидыМаркетплейсов.МаркетплейсЯндексМаркет") Тогда
		ПричинаПоУмолчанию = ИнтеграцияСЯндексМаркетВызовСервера.ПолучитьПричинуОтменыЗаказовПоУмолчанию();
		ВыборПричиныОтменыЗавершение(ПричинаПоУмолчанию, ОтменяемыеСтроки);
	КонецЕсли;
...
Функция ОтменитьСтрокиЗаказаНаСервере(Знач УчетнаяЗапись, Знач Заказ, Знач ПричинаОтмены,
	...
	ОтменаВыполнена = РегистрыСведений.ЗаказыТорговыхПлощадок.ОтменитьЗаказ(
		Заказ,
		Неопределено,
		ПричинаОтмены,
		КлючиЗаписей,
...
Функция РезультатПроверкиУчетаНомеровГТД(ТекущиеДанные)
	...
	ТекстСообщения = "";
	Если ТекущиеДанные.ТребуетсяНомерГТД Тогда
		Если Не ТекущиеДанные.ВестиУчетПоГТД Тогда
			ТекстСообщения =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		ИначеЕсли ТекущиеДанные.ВестиУчетПоГТД И Не ЗначениеЗаполнено(ТекущиеДанные.ДокументОтгрузки) Тогда
			Если ТекущиеДанные.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.ОжидаетСборки") Тогда
				ТекстСообщения = НСтр("ru = 'Номер ГТД будет заполнен автоматически из данных документа отгрузки после его оформления.'");
			Иначе
				ТекстСообщения = НСтр("ru = 'Номер ГТД заполнен автоматически по данным торговой площадки.'");
			КонецЕсли;
		ИначеЕсли ТекущиеДанные.ВестиУчетПоГТД И ЗначениеЗаполнено(ТекущиеДанные.ДокументОтгрузки) Тогда
			Если ЗначениеЗаполнено(ТекущиеДанные.НомерГТД) Тогда
				Если ТекущиеДанные.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.ОжидаетСборки") Тогда
					ТекстСообщения = НСтр("ru = 'Номер ГТД заполнен автоматически из данных документа отгрузки.'");
				Иначе
					ТекстСообщения = НСтр("ru = 'Номер ГТД заполнен автоматически по данным торговой площадки.'");
				КонецЕсли;
			Иначе
				ТекстСообщения = НСтр("ru = 'В документе отгрузки не указан номер ГТД, который требуется для торговой площадки.'");
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ТекущиеДанные.ТребуетсяРНПТ Тогда
		...
		ИначеЕсли ТекущиеДанные.ПрослеживаемыйТовар И Не ЗначениеЗаполнено(ТекущиеДанные.ДокументОтгрузки) Тогда
			Если ТекущиеДанные.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.ОжидаетСборки") Тогда
				ТекстСообщения = НСтр("ru = 'РНПТ будет заполнен автоматически из данных документа отгрузки после его оформления.'");
			Иначе
				ТекстСообщения = НСтр("ru = 'РНПТ заполнен автоматически по данным торговой площадки.'");
			КонецЕсли;
		ИначеЕсли ТекущиеДанные.ПрослеживаемыйТовар И ЗначениеЗаполнено(ТекущиеДанные.ДокументОтгрузки) Тогда
			Если ЗначениеЗаполнено(ТекущиеДанные.НомерГТД) Тогда
				Если ТекущиеДанные.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.ОжидаетСборки") Тогда
					ТекстСообщения = НСтр("ru = 'РНПТ заполнен автоматически из данных документа отгрузки.'");
				Иначе
					ТекстСообщения = НСтр("ru = 'РНПТ заполнен автоматически по данным торговой площадки.'");
				КонецЕсли;
			Иначе
				ТекстСообщения = НСтр("ru = 'В документе отгрузки не указан РНПТ, который требуется для торговой площадки.'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция РезультатПроверкиКодаМаркировки(ТекущиеДанные)
	...
	Если ТекущиеДанные.ТребуетсяМаркировка Тогда
		Если Не ЗначениеЗаполнено(ТекущиеДанные.ШтрихкодУпаковки) 
				И Не ЗначениеЗаполнено(ТекущиеДанные.ПолныйКодМаркировки)
				И Не ЗначениеЗаполнено(ТекущиеДанные.ДокументОтгрузки) Тогда
			ТекстСообщения =
		...
		ИначеЕсли Не ЗначениеЗаполнено(ТекущиеДанные.ШтрихкодУпаковки) 
					И ЗначениеЗаполнено(ТекущиеДанные.ПолныйКодМаркировки) Тогда
			Штрихкод           = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.Base64ВШтрихкод(ТекущиеДанные.ПолныйКодМаркировки);
			ПримечаниеКРазбору = Неопределено;
			ДанныеРазбора      = РазборКодаМаркировкиИССлужебныйКлиент.РазобратьКодМаркировки(Штрихкод,, ПримечаниеКРазбору);
			
			Если ДанныеРазбора = Неопределено Тогда
				ТекстСообщения = НСтр("ru = 'Код маркировки был указан в личном кабинете торговой площадки, но не найден в информационной базе.'")
					+ " " + ПримечаниеКРазбору.ТекстОшибки;
			Иначе
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Код маркировки %1 был указан в личном кабинете торговой площадки, но не найден в информационной базе.'"),
					ДанныеРазбора.НормализованныйКодМаркировки);
			КонецЕсли;
		ИначеЕсли ЗначениеЗаполнено(ТекущиеДанные.ДокументОтгрузки) Тогда
			Если ЗначениеЗаполнено(ТекущиеДанные.ШтрихкодУпаковки) Тогда
				Если ТекущиеДанные.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.ОжидаетСборки") Тогда
					ТекстСообщения = НСтр("ru = 'Код маркировки заполнен автоматически из данных документа отгрузки.'");
				Иначе
					ТекстСообщения = НСтр("ru = 'Код маркировки заполнен автоматически по данным торговой площадки.'");
				КонецЕсли;
			Иначе
				ТекстСообщения =
...
&НаКлиенте
Процедура ОформитьДокументыОтгрузкиЗавершение(РезультатПроверкиЗаказов)
