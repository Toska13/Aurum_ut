...
Процедура ТекстЗапросаДанныеЗаказовИОтправленийПоДополнительнымСведениям(ТекстыЗапроса, ИндексыПакетов,
	...
	ТекстЗапроса = ТекстЗапросаПереопределениеДокументовОтгрузкиИНомеровОтправлений("СведенияПоТоварамПредварительно");
	Тексты.Добавить(ТекстЗапроса);
	...
	ИндексыПакетов.Добавить(ИмяПакета, ТекущийИндексПакетаЗапроса(ИндексыПакетов) + 5);

КонецПроцедуры

// Формирует часть текста пакетного запроса по получению сведений по товарам заказов торговых площадок.
// При хранении документа отгрузки в регистре дополнительных сведений необходимо правильно определять
// документ отгрузки и номер отправления в случаях, когда товар переносился из одного отправления в другое.
//
// Параметры:
//  ИмяТаблицыДанных - Строка - имя временной таблицы, в которой содержатся данные регистра.
//
// Возвращаемое значение:
//  Строка
//
Функция ТекстЗапросаПереопределениеДокументовОтгрузкиИНомеровОтправлений(ИмяТаблицыДанных) Экспорт
	...
	ТекстЗапроса =
		...
		|ИЗ
		|	ДанныеСведенийПоТоварам КАК СведенияПоТоварам
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеСведенийПоТоварам КАК ПриоритетныеДокументыОтгрузки
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ДанныеСведенийПоТоварам", ИмяТаблицыДанных);
	Тексты.Добавить(ТекстЗапроса);
	
	// Позиции документа отгрузки полностью перенесены в другое отправление
	ТекстЗапроса =
		"ВЫБРАТЬ
		|	СведенияПоТоварам.Заказ КАК Заказ,
		|	СведенияПоТоварам.ДокументОтгрузки КАК ДокументОтгрузки,
		|	МИНИМУМ(СведенияПоТоварам.НомерОтправления) КАК НомерОтправления,
		|	МИНИМУМ(СведенияПоТоварам.НомерРодительскогоОтправления) КАК НомерРодительскогоОтправления
		|ПОМЕСТИТЬ ПолностьюПеренесенныеОтправления
		|ИЗ
		|	ДанныеСведенийПоТоварам КАК СведенияПоТоварам
		|
		|СГРУППИРОВАТЬ ПО
		...
		|
		|ИМЕЮЩИЕ
		|	МИНИМУМ(СведенияПоТоварам.ПозицияПеренесена) = ИСТИНА";
	...
	ТекстЗапроса =
		...
		|ИЗ
		|	ДанныеСведенийПоТоварам КАК СведенияПоТоварам
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеСведенийПоТоварам КАК ПриоритетныеНомераОтправлений
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеСведенийПоТоварам КАК ПриоритетныеДокументыОтгрузки
		|		ПО СведенияПоТоварам.Заказ = ПриоритетныеДокументыОтгрузки.Заказ
		|			И СведенияПоТоварам.НомерОтправления = ПриоритетныеДокументыОтгрузки.НомерОтправления
		...
		|	СведенияПоТоварам.Заказ,
		|	СведенияПоТоварам.ИдентификаторЭкземпляра,
		|	ПриоритетныеНомераОтправлений.НомерОтправления,
		|	ПриоритетныеНомераОтправлений.НомерРодительскогоОтправления
		|ИЗ
		|	ДанныеСведенийПоТоварам КАК СведенияПоТоварам
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПолностьюПеренесенныеОтправления КАК ПриоритетныеНомераОтправлений
		|		ПО СведенияПоТоварам.Заказ = ПриоритетныеНомераОтправлений.Заказ
		|			И СведенияПоТоварам.ДокументОтгрузки = ПриоритетныеНомераОтправлений.ДокументОтгрузки
		|
		|ИНДЕКСИРОВАТЬ ПО
...
Функция СведенияОбОтправлениях(Заказ, СведенияПоТоварам, ДокументыОтгрузкиЗаказов, ТребуетсяЗаписьСведенийОбОтправлениях = Ложь)
	...
	Пока ДокументыОтгрузкиЗаказов.НайтиСледующий(Отбор) Цикл
		...
		Пока ВыборкаДокументовОтгрузки.Следующий() Цикл
			...
			Если Параметры.ИспользоватьНомерОтправления 
					И ДанныеПоНомерамОтправлений.Получить(Параметры.НомерОтправления) = Неопределено Тогда
