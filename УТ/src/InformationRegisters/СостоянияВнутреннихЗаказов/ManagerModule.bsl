...
Функция ВременнаяТаблицаОбеспечениеЗаказа()
	...
	ТекстЗапроса =
		...
		|	МАКСИМУМ(Таблица.КонтролируемаяПартия) КАК КонтролируемаяПартия,
...
Функция ВременнаяТаблицаЧастичноВНаличии()
	...
	ТекстЗапроса =
		...
		|	И Заказы.КонтролируемаяПартия = ЗНАЧЕНИЕ(Справочник.КонтролируемыеПартии.ПустаяСсылка)
		|	И(
		|		ИСТИНА В(
		|			ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|			РегистрСведений.РаспределениеЗапасов КАК Сведения
		|		ГДЕ
		|			Сведения.ЗаказНаОтгрузку = Заказы.КонтролируемаяПартия
		|				И Сведения.СостояниеЗапаса В(
		|					ЗНАЧЕНИЕ(Перечисление.СостоянияЗапаса.НеОбеспечен),
		|					ЗНАЧЕНИЕ(Перечисление.СостоянияЗапаса.НаСогласованииПоставки),
		|					ЗНАЧЕНИЕ(Перечисление.СостоянияЗапаса.ОжидаетсяКПоступлению),
		|					ЗНАЧЕНИЕ(Перечисление.СостоянияЗапаса.ОжидаетсяКПоступлениюСБлокировкой),
		|					ЗНАЧЕНИЕ(Перечисление.СостоянияЗапаса.ОжидаетсяКПоступлениюНеПодлежитРазблокировке))
		|				И Сведения.ХарактеристикаПотребности = ЗНАЧЕНИЕ(Перечисление.ХарактеристикиПотребности.РезервироватьПоМереПоступленияИзЗаблокированных))";
