...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УдалитьНастройкиИнтеркампани", Истина);
	Методы.Вставить("УдалитьНастройкиОтправкиЭДО", Истина);
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		И (НастройкиОтправкиЭлектронныхДокументовПоВидам.УдалитьВидДокумента <> ЗНАЧЕНИЕ(Перечисление.ТипыДокументовЭДО.ПустаяСсылка)
	...
	|	ИЛИ НастройкиОтправкиЭлектронныхДокументовПоВидам.ВерсияФормата = &СчетНаОплату_5_01)";
	...
	Запрос.УстановитьПараметр("СчетНаОплату_5_01", ФорматыЭДО_ФНС.ПоддерживаемыеФорматы().СчетНаОплату_5_01);
	...
	Для Каждого СтрокаДанных Из ВыбранныеДанные Цикл
		...
		Попытка
			...
			Если НаборЗаписей.Количество() Тогда
				...
				ОбработатьДанные_ЗаполнитьТребуетсяОтветнаяПодпись(НаборЗаписей, ТипыДокументовПоВиду, Записать);
...
Процедура ОбработатьДанные_ЗаполнитьТребуетсяОтветнаяПодпись(НаборЗаписей, ТипыДокументовПоВиду, Записать)
	
	ФорматыФНС = ФорматыЭДО_ФНС.ПоддерживаемыеФорматы();
	
	Для Каждого Запись Из НаборЗаписей Цикл
		Если Запись.ТребуетсяОтветнаяПодпись
			И Запись.ВерсияФормата = ФорматыФНС.СчетНаОплату_5_01 Тогда
