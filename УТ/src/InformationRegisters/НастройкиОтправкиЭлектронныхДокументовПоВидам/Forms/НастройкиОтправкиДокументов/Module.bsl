...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоступнаяФункциональность = ИнтеграцияЭДО.ФункциональностьФормыНастройкаОтправкиДокументов();
	
	ИспользуютсяДоговорыКонтрагентов = ИнтеграцияЭДО.ИспользуютсяДоговорыКонтрагентов()
		И ДоступнаяФункциональность.ИспользоватьДоговор;
	...
	Элементы.ИсходящиеДокументыДополнительныеНастройки.Видимость = ДоступнаяФункциональность.НастройкаРегламента;
	Элементы.ФормаРасширенныйРежим.Видимость = ДоступнаяФункциональность.РасширенныйРежим;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если (ИмяСобытия = "ОбновитьТекущиеДелаПоЭДО"
...
Процедура НеПоказыватьОкноРучнойПроверкиМЧД(Команда)
	...
	ОбщегоНазначенияКлиент.ХранилищеОбщихНастроекСохранить(КлючОбъекта, КлючНастроек, НастройкиРучнойПроверки);
...
Функция НачатьСохранениеНастроек(ПропуститьПроверки = Ложь, ОповещениеКоманды = Неопределено)
	...
	Если Результат Тогда
		Модифицированность = Ложь;
		УстановитьКлючНастройкиОтправки(ЭтотОбъект);
		Оповестить("ОбновитьТекущиеДелаПоЭДО", Контрагент, Организация);
...
Функция ЗаписатьНастройки()
	...
	ВсеПустыеЗначенияДоговораСКонтрагентомЭДО = ОбменСКонтрагентамиИнтеграция.ВсеПустыеЗначенияДоговораСКонтрагентомЭДО();
	
	Если ВсеПустыеЗначенияДоговораСКонтрагентомЭДО.Количество() > 1 И Не ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
		ДоговорКонтрагента = Неопределено;
		ИсточникБлокировкиПустыхДоговоров = Новый ТаблицаЗначений();
		ИсточникБлокировкиПустыхДоговоров.Колонки.Добавить("Отправитель");
		ИсточникБлокировкиПустыхДоговоров.Колонки.Добавить("Получатель");
		ИсточникБлокировкиПустыхДоговоров.Колонки.Добавить("Договор");
		ИсточникБлокировкиПустыхДоговоров.ЗагрузитьКолонку(ВсеПустыеЗначенияДоговораСКонтрагентомЭДО, "Договор");
		ИсточникБлокировкиПустыхДоговоров.ЗаполнитьЗначения(Организация, "Отправитель");
		ИсточникБлокировкиПустыхДоговоров.ЗаполнитьЗначения(Контрагент, "Получатель");
	КонецЕсли;
	...
	Запрос = Новый Запрос;
			...
			|	И НастройкиОтправкиЭлектронныхДокументов.Договор В (&Договора)";
	Запрос.УстановитьПараметр("Отправитель", Организация);
	Запрос.УстановитьПараметр("Получатель", Контрагент);
	Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
		Запрос.УстановитьПараметр("Договора", ДоговорКонтрагента);
	Иначе
		Запрос.УстановитьПараметр("Договора", ВсеПустыеЗначенияДоговораСКонтрагентомЭДО);
	КонецЕсли;
	...
	Попытка
		...
		Если ДоговорКонтрагента = Неопределено И ВсеПустыеЗначенияДоговораСКонтрагентомЭДО.Количество() > 1 Тогда			
			ЭлементБлокировкиДанных.ИсточникДанных = ИсточникБлокировкиПустыхДоговоров;
			ЭлементБлокировкиДанных.ИспользоватьИзИсточникаДанных("Отправитель", "Отправитель");
			ЭлементБлокировкиДанных.ИспользоватьИзИсточникаДанных("Получатель", "Получатель");
			ЭлементБлокировкиДанных.ИспользоватьИзИсточникаДанных("Договор", "Договор");
		Иначе
		...
		ЭтоНоваяЗапись = Результат.Пустой();
		Запись = РегистрыСведений.НастройкиОтправкиЭлектронныхДокументов.СоздатьМенеджерЗаписи();

		Если Не ЭтоНоваяЗапись И Выборка.Количество() > 1 Тогда
			Пока Выборка.Следующий() Цикл
				Если Выборка.Договор = ДоговорКонтрагента Тогда
					Продолжить;
				КонецЕсли;
		...
		Если НаборЗаписей.Количество() = 0 И ДоговорКонтрагента = Неопределено Тогда
			Для Каждого ПустойДоговор Из ВсеПустыеЗначенияДоговораСКонтрагентомЭДО  Цикл
				Если ПустойДоговор = ДоговорКонтрагента Тогда
					Продолжить;
				КонецЕсли;
				НаборЗаписей.Отбор.Договор.Установить(ПустойДоговор);
				НаборЗаписей.Прочитать();
...
Функция КлючНастройкиОтправкиИзменен(КлючНастройкиОтправки, Организация, Контрагент, ДоговорКонтрагента)
	...
	Если ДоговорКонтрагента = Неопределено И Не ЗначениеЗаполнено(КлючНастройкиОтправки.Договор) Тогда
...
Процедура УдалитьНастройкуОтправкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ПолучитьИзВременногоХранилища(Результат.АдресРезультата) = Истина Тогда
		...
		Оповестить("ОбновитьТекущиеДелаПоЭДО");
		Закрыть();
