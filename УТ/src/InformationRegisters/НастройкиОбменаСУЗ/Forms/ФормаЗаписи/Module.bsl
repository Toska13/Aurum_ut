...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Записано Тогда
		...
		Запись.ПроизводственныйОбъект = Параметры.ПроизводственныйОбъект;
		Запись.ОбменНаСервере          = Истина;
		ОбменНаСервере                 = 1;
		Запись.ИдентификаторСоединения = "";
		ФорматОбмена = Перечисления.ВерсииФорматаОбменаСУЗ.V3;
	...
	Элементы.Токен.Видимость = Записано И ЗначениеЗаполнено(Запись.Токен);
	
	УпрощеннаяИнтеграцияБЭД = ИнтеграцияИС.УпрощеннаяИнтеграцияБЭД();
	Элементы.ГруппаВыполнятьОбмен.Видимость      = Не УпрощеннаяИнтеграцияБЭД;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗаписьСтанцииУправленияЗаказамиИСМП"
		И Параметр = Запись.СтанцияУправленияЗаказами Тогда
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	КлючЗаписиКеш = ИнициализироватьКлючЗаписиКеш();
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Не ЗначениеЗаполнено(ТекущийОбъект.СтанцияУправленияЗаказами) Тогда
		ТекущийОбъект.СтанцияУправленияЗаказами = СоздатьСУЗ(Отказ);
	КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("ОбновитьГиперссылкуНастроитьПодключениеСУЗ");
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ТребуетсяПроверкаНаДубли
		...
		|	  ИЛИ &БезОтбораПроизводственныйОбъект)";
		...
		Запрос.УстановитьПараметр("БезОтбораПроизводственныйОбъект", Не ИспользуетсяПроизводственныйОбъект);
		...
		Если НЕ Запрос.Выполнить().Пустой() Тогда
			...
			Иначе
				ОбщегоНазначения.СообщитьПользователю(
					СтрШаблон(
						НСтр("ru = 'Настройка обмена для организации %1 уже существует'"),
						Запись.Организация),,
					"Запись.Организация",,
					Отказ);
			КонецЕсли;
...
&НаКлиенте
Процедура ИдентификаторСУЗПриИзменении(Элемент)
	
	Если ЗначениеЗаполнено(ИдентификаторСУЗ) Тогда
		ПриИзмененииИдентификаторСУЗ();
	Иначе
		ПриОчисткеИдентификаторСУЗ();
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ИдентификаторСУЗОчистка(Элемент, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ИдентификаторСУЗНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	ОткрытьФорму(
		"Справочник.СтанцииУправленияЗаказамиИСМП.Форма.ФормаВыбора", ,
		Элемент,,,,,
...
&НаКлиенте
Процедура ИдентификаторСУЗОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	Запись.СтанцияУправленияЗаказами = ВыбранноеЗначение;
	ЗаполнитьСлужебныеРеквизитыФормы();
	СтандартнаяОбработка = Ложь;
...
&НаКлиенте
Процедура ИдентификаторСУЗОткрытие(Элемент, СтандартнаяОбработка)
	
	Если ЗначениеЗаполнено(Запись.СтанцияУправленияЗаказами) Тогда
		
		СтандартнаяОбработка = Ложь;
		
		ПараметрыФормы = Новый Структура("Ключ", Запись.СтанцияУправленияЗаказами);
		ОткрытьФорму(
			"Справочник.СтанцииУправленияЗаказамиИСМП.Форма.ФормаЭлемента",
			ПараметрыФормы,
			ЭтотОбъект,,,,,
			РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПолучитьКодФИАС(Команда)
	...
	Если Не ПроверитьСоздатьСУЗ() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьРасписаниеОбменаНаСервере()
	...
	Если ЗаданияОбмена.Количество() <> 1 Тогда
		ЗаписатьОшибкуПоискаРегламентногоЗадания();
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не найдено регламентное задание обмена с ИС МП'"));
		ОбменПоРасписанию = Ложь;
	Иначе
...
Функция УдалитьНедопустимыеСимволыИзСтроки(Знач ДанныеСтроки)
	...
	Пока Истина Цикл
		...
		ПозицияНедопустимогоСимвола = ОбработкаСтрокиXML.НайтиНедопустимыеСимволыXML(ДанныеСтроки);
...
Процедура ОбработатьНавигационнуюСсылку(Знач НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если (НавигационнаяСсылкаФорматированнойСтроки = "ПроверитьПодключениеКСУЗ"
		  Или НавигационнаяСсылкаФорматированнойСтроки = "Повторить")
		И Не ЗначениеЗаполнено(Запись.СтанцияУправленияЗаказами) Тогда
...
Функция ПроверитьЗаполнениеНаКлиенте()
	...
	Если Не ЗначениеЗаполнено(ИдентификаторСУЗ) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не заполнено поле ""Идентификатор (OMS ID)""'"),, "ИдентификаторСУЗ",, ЕстьОшибки);
	КонецЕсли;
...
Функция ТребуетсяПодключениеИнтернетПоддержки()
	...
	Возврат ТребуетсяПодключениеИнтернетПоддержки;
...
Процедура ПриИзмененииТекстаАдресаПроизводственногоОбъекта(Текст)
	...
	Запись.ПроизводственныйОбъектАдрес = ОбщегоНазначенияИСВызовСервера.ЗначенияПолейКонтактнойИнформации(
...
Процедура НачатьПолучениеКодаФИАС()
	...
	Для Каждого ТоварнаяГруппа Из КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступныеТоварныеГруппы() Цикл
		Если МаркировкаТоваровКлиентСервер.ДоступныКодыФИАС(ТоварнаяГруппа) Тогда
			МассивТоварныхГрупп.Добавить(ТоварнаяГруппа);
		КонецЕсли;
	КонецЦикла;
	
	КоличествоВидовПродукции = МассивТоварныхГрупп.Количество();
	...
	ИначеЕсли КоличествоВидовПродукции = 1 Тогда
		ПолучитьМестаОсуществленияДеятельности(МассивТоварныхГрупп[0]);
	Иначе
		СписокВыбора = Новый СписокЗначений;
		СписокВыбора.ЗагрузитьЗначения(МассивТоварныхГрупп);
...
&НаКлиенте
Процедура ПолучитьМестаОсуществленияДеятельности(ТоварнаяГруппа)
	...
	ДополнительныеПараметры.Вставить("СтанцияУправленияЗаказами", Запись.СтанцияУправленияЗаказами);
	ДополнительныеПараметры.Вставить("ПроизводственныйОбъект",    Запись.ПроизводственныйОбъект);
	ДополнительныеПараметры.Вставить("ТоварнаяГруппа",            ТоварнаяГруппа);
	...
	ПараметрыОповещения.Вставить("ТоварнаяГруппа", ТоварнаяГруппа);
...
Процедура ПослеПолученияМестОсуществленияДеятельности(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли СписокМОД.Количество() = 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'В системе ""Честный знак"" по товарной группе ""%1"" места осуществления деятельности не найдены.'"),
			ДополнительныеПараметры.ТоварнаяГруппа);
...
&НаКлиентеНаСервереБезКонтекста
Функция ИнициализироватьКлючЗаписиКеш()

	Возврат Новый Структура("Организация, ПроизводственныйОбъект");
...
Процедура ЗаполнитьСлужебныеРеквизитыФормы()
	...
	Если ЗначениеЗаполнено(Запись.СтанцияУправленияЗаказами) Тогда
		...
		ДанныеНастройкиСУЗ = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			Запись.СтанцияУправленияЗаказами, "ФорматОбмена, Идентификатор", Истина);
		ФорматОбмена     = ДанныеНастройкиСУЗ.ФорматОбмена;
		ИдентификаторСУЗ = ДанныеНастройкиСУЗ.Идентификатор;
...
&НаСервере
Функция СоздатьСУЗ(Отказ)
	
	Попытка
		
		ОбъектСУЗ = Справочники.СтанцииУправленияЗаказамиИСМП.СоздатьЭлемент();
		ОбъектСУЗ.ФорматОбмена  = ФорматОбмена;
		ОбъектСУЗ.Идентификатор = ИдентификаторСУЗ;
		ОбъектСУЗ.Наименование  = СтрШаблон(
			"%1: %2",
			Строка(Запись.Организация),
			Прав(ИдентификаторСУЗ, 4));
		
		ОбъектСУЗ.Записать();
		Результат = ОбъектСУЗ.Ссылка;
		
	Исключение
		
		ТекстОшибки = НСтр("ru = 'При записи справочника ""Станция управление заказами"" произошла ошибка:'")
			+ Символы.ПС + "%1";
		
		ОбщегоНазначения.СообщитьПользователю(
			СтрШаблон(ТекстОшибки, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке())),,,,Отказ);
...
&НаСервереБезКонтекста
Функция ДанныеСУЗПоИдентификатору(ИдентификаторСУЗ)
	
	Результат = Новый Структура;
	Результат.Вставить("СтанцияУправленияЗаказами", Справочники.СтанцииУправленияЗаказамиИСМП.ПустаяСсылка());
	Результат.Вставить("ФорматОбмена",              Перечисления.ВерсииФорматаОбменаСУЗ.V3);
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	|	СтанцииУправленияЗаказамиИСМП.Ссылка КАК СтанцияУправленияЗаказами,
	|	СтанцииУправленияЗаказамиИСМП.ФорматОбмена КАК ФорматОбмена
	|ИЗ
	|	Справочник.СтанцииУправленияЗаказамиИСМП КАК СтанцииУправленияЗаказамиИСМП
	|ГДЕ
	|	СтанцииУправленияЗаказамиИСМП.Идентификатор = &Идентификатор
	|";
	
	Запрос.УстановитьПараметр("Идентификатор", ИдентификаторСУЗ);
...
&НаКлиенте
Процедура ПриИзмененииИдентификаторСУЗ()
	
	ДанныеСУЗ = ДанныеСУЗПоИдентификатору(ИдентификаторСУЗ);
	
	Запись.СтанцияУправленияЗаказами = ДанныеСУЗ.СтанцияУправленияЗаказами;
	ФорматОбмена                     = ДанныеСУЗ.ФорматОбмена;
	
	Если ЗначениеЗаполнено(Запись.СтанцияУправленияЗаказами) Тогда
		Если ФорматОбмена = ПредопределенноеЗначение("Перечисление.ВерсииФорматаОбменаСУЗ.V3") Тогда
			Запись.ИспользоватьЗащищенноеСоединение = Истина;
		ИначеЕсли ФорматОбмена = ПредопределенноеЗначение("Перечисление.ВерсииФорматаОбменаСУЗ.V2") Тогда
			Запись.ИспользоватьЗащищенноеСоединение = Истина;
		ИначеЕсли ФорматОбмена = ПредопределенноеЗначение("Перечисление.ВерсииФорматаОбменаСУЗ.V1") Тогда
...
&НаКлиенте
Процедура ПриОчисткеИдентификаторСУЗ()
	
	Запись.СтанцияУправленияЗаказами = Неопределено;
	ФорматОбмена = ПредопределенноеЗначение("Перечисление.ВерсииФорматаОбменаСУЗ.V3");
	
КонецПроцедуры

&НаКлиенте
Функция ПроверитьСоздатьСУЗ()
	
	Отказ = Ложь;
	Если Не ЗначениеЗаполнено(Запись.СтанцияУправленияЗаказами) Тогда
		Запись.СтанцияУправленияЗаказами = СоздатьСУЗ(Отказ);
	КонецЕсли;
