...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ЭтоПодключениеСобственнойОрганизации Тогда
		СтраницыФормы.ТекущаяСтраница = СтраницыФормы.ПодчиненныеЭлементы.СтраницаРеквизитыПодключения;
	Иначе
...
Процедура ГруппаСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	ПриСменеСтраницы(Элемент, ТекущаяСтраница);
...
Процедура КлючAPIПриИзмененииНаСервере()
	...
	КлючAPI = ?(ЗначениеЗаполнено(КлючAPI), УникальныйИдентификатор, "");
...
Функция НастройкаПодключения()
	...
	НастройкаПодключения.ЛогинПользователя = ЛогинАдминистратора;
...
Функция НачатьПолучениеСведенийНаСервере(НастройкаПодключения)
	...
	СтруктураПараметров.Вставить("АдресДанныхЛогаЗапроса",            ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор));
	СтруктураПараметров.Вставить("ПараметрыЛогированияЗапросовВЕТИС", ПараметрыСеанса.ПараметрыЛогированияЗапросовВЕТИС);
...
Функция ПроверитьВыполнениеФоновогоЗаданияНаСервере()
	...
	Исключение
		ЗаголовокОшибки = НСтр("ru='Не удалось получить сведения хозяйствующего субъекта'");
		ТекстОшибки = СформироватьТекстОшибки(
			ЗаголовокОшибки,
...
Процедура ОбработатьРезультатВыполненияФоновогоЗадания()
	...
	Если РезультатВыполнения <> Неопределено
		И РезультатВыполнения.ИзвлекатьДанныеЛогаЗапросов Тогда
		ИнтеграцияВЕТИСВызовСервера.ИзвлечьЛогЗапросовИзРезультатаОбмена(РезультатВыполнения);
	КонецЕсли;
...
Процедура ЗакончитьПолучениеСведенийПредприятий()
	...
	Если Предприятия.Количество() > 0 Тогда
		ЗаписатьСинхронизациюКлассификаторов(Отказ, Истина);
	КонецЕсли;
...
Процедура НачатьПолучениеПользователей()
	...
	Попытка
		...
		РезультатПолученияПользователей = ЗаявкиВЕТИСВызовСервера.ПодготовитьЗапросПользователейХозяйствующегоСубъекта(
			...
			УникальныйИдентификатор, НастройкаПодключения());
...
Процедура НачатьПолучениеВетеринарноСопроводительныхДокументов()
	...
	Исключение
		РезультатПолученияВСДокументов = Неопределено;
		ТекстОшибки = СформироватьТекстОшибки(
			ЗаголовокОшибки, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ПоказатьОшибкуПолучения();
	КонецПопытки;
...
&НаСервере
Процедура ЗаписатьСинхронизациюКлассификаторов(Отказ, СинхронизацияПредприятий = Ложь)
...
Функция ПроверитьЗаполнениеТекущейСтраницы()
	...
	Для Каждого ОшибкаЭлемент Из СписокОшибок Цикл
		ОбщегоНазначенияКлиент.СообщитьПользователю(ОшибкаЭлемент.Представление,, ОшибкаЭлемент.Значение);
