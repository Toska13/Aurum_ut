...
Функция ЗагрузитьЗаказыТорговойПлощадки(УчетнаяЗапись, Знач Параметры = Неопределено) Экспорт
	...
	ИначеЕсли НастройкиУчетнойЗаписи.ВидМаркетплейса = Перечисления.ВидыМаркетплейсов.МаркетплейсЯндексМаркет Тогда
		Если Параметры = Неопределено Тогда
			Параметры = ИнтеграцияСЯндексМаркетСервер.НовыеПараметрыЗагрузкиЗаказов();
...
Функция ОбновитьСтатусыЗаказовТорговойПлощадки(УчетнаяЗапись, Знач Параметры = Неопределено,
	...
	Для Каждого ДанныеОтправления Из СтрокиЗаказа Цикл
		...
		Попытка
			...
			ЗаполнитьДополнительныеДанныеЗаписиПоДаннымОтправления(Запись, ДанныеОтправления);
...
Функция ТекстЗапросаДинамическогоСписка(СокращенныйТекст = Ложь, ИспользоватьПометки = Ложь,
	...
	Иначе
		ДополнительныеПоля = "
			...
			|	ВЫБОР
			|		КОГДА ДокументыЗаказа.Приоритет = &ВысшийПриоритет
			|			ТОГДА 0
			|		КОГДА ДокументыЗаказа.Приоритет = &НизшийПриоритет
			|			ТОГДА 2
			|		ИНАЧЕ 1
			...
			|	&ТекстЗапросаДанныеДокументаОтгрузки КАК ДанныеДокументаОтгрузки,
			|	ВЫБОР
			|		КОГДА ЗаказыТорговыхПлощадок.СрокОтгрузки = ДАТАВРЕМЯ(1, 1, 1)
			|			ТОГДА 2
			|		КОГДА ЗаказыТорговыхПлощадок.Статус В (&СтатусыЗаказовВДоставке)
			|				И ЗаказыТорговыхПлощадок.ИдентификаторОтгрузки <> """"
			|			ТОГДА 1
			|		КОГДА ЗаказыТорговыхПлощадок.Действие = ЗНАЧЕНИЕ(Перечисление.ДействияПриОбменеСТорговымиПлощадками.ПодтвердитьОтгрузку)
			|				И ЗаказыТорговыхПлощадок.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовТорговыхПлощадок.ГотовКОтгрузке)
			|			ТОГДА 1
			|		ИНАЧЕ 0
...
Функция ЗаписатьСведенияОбОтправлениях(Заказ, ДанныеПоНомерамОтправлений) Экспорт
	...
	УчетнаяЗапись = УчетнаяЗаписьИзДанныхПоНомерамОтправлений(ДанныеПоНомерамОтправлений);
	ВидМаркетплейса = ИнтеграцияСМаркетплейсамиПовтИсп.ВидМаркетплейсаУчетнойЗаписи(УчетнаяЗапись);
	...
	Если Не ТребуетсяУказаниеДокументаОтгрузки Тогда
		Возврат ТекстОшибки;
	КонецЕсли;
...
Функция ПроверитьЗаказы(Заказы, РезультатПроверкиЗаказов = Неопределено,
	...
	ДанныеДляПроверки = ДанныеДляПроверкиЗаказов(Заказы, ЗаполнитьСведенияОбЭкземплярах);
	
	ВыборкаОтправленийСНезаполненнымиДанными = ДанныеДляПроверки.ОтправленияСНезаполненнымиДанными.Выбрать();
	...
	ВыборкаОтправленийИзмененных = ДанныеДляПроверки.ОтправленияИзмененные.Выбрать();
	Пока ВыборкаОтправленийИзмененных.Следующий() Цикл
		РезультатПроверкиЗаказа = ИнтеграцияСМаркетплейсамиКлиентСервер.РезультатПроверкиЗаказа(
			ВыборкаОтправленийИзмененных.Заказ,
			РезультатПроверкиЗаказов);
		
		НомерОтправления = ВыборкаОтправленийИзмененных.НомерОтправления;
		ДокументОтгрузки = ВыборкаОтправленийИзмененных.ДокументОтгрузки;
		
		Если Не ПустаяСтрока(НомерОтправления) Тогда
			РезультатПроверкиЗаказа.ОтправленияИзмененные.Добавить(НомерОтправления);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ДокументОтгрузки)
				И РезультатПроверкиЗаказа.ИзмененныеДокументыОтгрузки.Получить(ДокументОтгрузки) = Неопределено Тогда
			РезультатПроверкиЗаказа.ИзмененныеДокументыОтгрузки.Вставить(ДокументОтгрузки, НомерОтправления);
		КонецЕсли;
	КонецЦикла;
	...
	ВыборкаЗаказов = ДанныеДляПроверки.ДанныеЗаказов.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	...
	Если ЗаполнитьСведенияОбЭкземплярах Тогда
		ВыборкаЗаказов = ДанныеДляПроверки.ЭкземплярыБезДокументовОтгрузки.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаЗаказов.Следующий() Цикл
			РезультатПроверкиЗаказа = ИнтеграцияСМаркетплейсамиКлиентСервер.РезультатПроверкиЗаказа(
...
// Выполняет поиск данных заказа по строке поиска. Возможные варианты поиска:
//  - по номеру заказа;
//  - номеру отправления;
...
// 
// Возвращаемое значение:
//  - Неопределено - данные по строке поиска не найдены.
//  - См. НовыйРезультатПоискаДанныхЗаказаПоСтрокеПоиска
//
Функция ДанныеЗаказаПоСтрокеПоиска(СтрокаПоиска) Экспорт
	
	Результат = НовыйРезультатПоискаДанныхЗаказаПоСтрокеПоиска();
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
		|	""НомерЗаказа"" КАК ИмяПоляПоиска,
		|	Заказы.Заказ КАК Заказ,
		|	Заказы.ДокументОтгрузки КАК ДокументОтгрузки,
		|	Заказы.ИдентификаторЗаказа КАК ИдентификаторЗаказа,
		|	Заказы.НомерЗаказа КАК НомерЗаказа,
		|	Заказы.НомерОтправления КАК НомерОтправления,
		|	Заказы.ДатаСозданияЗаказа КАК ДатаСозданияЗаказа,
		|	Заказы.ИдентификаторСкладаТорговойПлощадки КАК ИдентификаторСклада
		|ИЗ
		|	РегистрСведений.ЗаказыТорговыхПлощадок КАК Заказы
		|ГДЕ
		|	Заказы.НомерЗаказа = &СтрокаПоиска
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|	Заказы.ИдентификаторЗаказа,
		|	Заказы.НомерЗаказа,
		|	Заказы.НомерОтправления,
		|	Заказы.ДатаСозданияЗаказа,
		|	Заказы.ИдентификаторСкладаТорговойПлощадки
		|ИЗ
		|	РегистрСведений.ЗаказыТорговыхПлощадок КАК Заказы
		|ГДЕ
		|	Заказы.НомерОтправления = &СтрокаПоиска
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|	РегистрСведений.ЗаказыТорговыхПлощадок КАК Заказы
		|ГДЕ
		|	Заказы.ИдентификаторОтгрузки = &СтрокаПоиска";
...
// Конструктор параметров для открытия формы просмотра состава заказа.
// 
// Возвращаемое значение:
//  Структура:
...
//   * ЭтоFBO - Булево
//
Функция НовыеПараметрыОткрытияФормыПросмотраЗаказа() Экспорт
	...
	Результат.Вставить("СтатусОтправления", Перечисления.СтатусыЗаказовТорговыхПлощадок.ПустаяСсылка());
	Результат.Вставить("ДатаОтгрузки", Дата(1, 1, 1));
	Результат.Вставить("ДатаДоставки", Дата(1, 1, 1));
	Результат.Вставить("ДатаПередачиВДоставку", Дата(1, 1, 1));
	...
	Результат.Вставить("ТрекНомерОтправления", "");
	Результат.Вставить("НаименованиеСклада", "");
	Результат.Вставить("АдресСодержимогоЗаказа", "");
	Результат.Вставить("ДокументЗаказа", Документы.ЗаказКлиента.ПустаяСсылка());
	Результат.Вставить("ДокументОтгрузки", Неопределено);
	Результат.Вставить("ДополнительныеСведения", "");
	Результат.Вставить("ЭтоFBO", Ложь);
...
Функция НовыйСписокАтрибутовИнформацииОбОтгрузке(УчетнаяЗапись) Экспорт
	...
	АтрибутыОтвета.Вставить("ИдентификаторыЗаказов",           Новый Массив); 
	АтрибутыОтвета.Вставить("ЗаказыЗагруженные",               Новый Массив); 
	АтрибутыОтвета.Вставить("ЗаказыНеЗагруженные",             Новый Массив); 
	АтрибутыОтвета.Вставить("ЗаказыНеЗагруженныеОтмененные",   Новый Массив); 
	АтрибутыОтвета.Вставить("ВсеНеЗагруженныеЗаказыОтменены",  Ложь);  	
	...
	АтрибутыОтвета.Вставить("КоличествоГрузовыхМестФакт",      0);
	...
	АтрибутыОтвета.Вставить("СоответствиеЗаказовИОтправлений", Новый Соответствие);
	...
	АтрибутыОтвета.Вставить("КоличествоЗаказовПлан",           0);
	АтрибутыОтвета.Вставить("КоличествоЗаказовПодтверждено",   0);
	АтрибутыОтвета.Вставить("КоличествоЗаказовФакт",           0);
	...
	АтрибутыОтвета.Вставить("ВремяПриемаНачало",               Дата(1,1,1));
	АтрибутыОтвета.Вставить("ВремяПриемаОкончание",            Дата(1,1,1));   
	АтрибутыОтвета.Вставить("ПодписанАктПриемаПередачи",       Ложь); 
...
Функция СформироватьОтгрузку(УчетнаяЗапись, МетодДоставки, ДатаОтгрузки, КоличествоГрузовыхМест = 0) Экспорт
	...
	Если ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоOzon(ИнформацияОбОтгрузке.ВидМаркетплейса) Тогда
...
Функция ПолучитьИнформациюОбОтгрузке(УчетнаяЗапись, МетодДоставки, ДатаОтгрузки,
	...
	ИначеЕсли ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоЯндексМаркет(ИнформацияОбОтгрузке.ВидМаркетплейса) Тогда
		ИнформацияОбОтгрузке = ИнтеграцияСЯндексМаркетСервер.ИнформацияОбОтгрузке(
			УчетнаяЗапись,
			ИдентификаторОтгрузки,
			ДатаОтгрузки);  
...
Функция ПодтвердитьОтгрузку(УчетнаяЗапись, МетодДоставки, ИдентификаторОтгрузки,
	...
	ИначеЕсли ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоЯндексМаркет(ИнформацияОбОтгрузке.ВидМаркетплейса) Тогда
		...
		Ошибка = ИнформацияОбОтгрузке.Ошибка;
		Если ПустаяСтрока(Ошибка.ОписаниеОшибки)
				И ИнформацияОбОтгрузке.ДоступныеДействия.Найти("УказатьКоличествоПалет") <> Неопределено Тогда
			
			Ошибка = ИнтеграцияСЯндексМаркетСервер.ПередатьКоличествоУпаковок(
				УчетнаяЗапись,
				ИдентификаторОтгрузки,
				КоличествоГрузовыхМест);
		КонецЕсли;
		
		Если Ошибка= Неопределено Или ТипЗнч(Ошибка) = Тип("Структура") И ПустаяСтрока(Ошибка.ОписаниеОшибки) Тогда
			Ошибка = ИнтеграцияСЯндексМаркетСервер.ПодтвердитьОтгрузку(УчетнаяЗапись, ИдентификаторОтгрузки);
		КонецЕсли;  
		
		Если Ошибка= Неопределено Или ТипЗнч(Ошибка) = Тип("Структура") И ПустаяСтрока(Ошибка.ОписаниеОшибки) Тогда
			ОтгрузкаПодтверждена = Истина;	
		КонецЕсли;
	...
	Если ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоЯндексМаркет(ИнформацияОбОтгрузке.ВидМаркетплейса) Тогда
		Если ОтгрузкаПодтверждена И ИнформацияОбОтгрузке.Статус = "Сформирована" Тогда  
			ИнформацияОбОтгрузке.ПовторитьЗапрос = Истина;		
		Иначе  
			ИнформацияОбОтгрузке.ПовторитьЗапрос = Ложь;	
		КонецЕсли;
	КонецЕсли;
	...
	Если НастройкиСклада <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ДокументОбъект.Контрагент) Тогда
			ДокументОбъект.БанковскийСчетПеревозчика =
				Справочники.БанковскиеСчетаКонтрагентов.ПолучитьБанковскийСчетПоУмолчанию(ДокументОбъект.Контрагент);
		КонецЕсли;
	КонецЕсли;
...
Функция СформироватьТранспортнуюНакладную(ИнформацияОбОтгрузке, НастройкиСклада = Неопределено, ЗаданиеНаПеревозку = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументОбъект.Плательщик) Тогда
		...
		Иначе
			...
			ПараметрыСчета.Организация = ДокументОбъект.Плательщик;
			ДокументОбъект.БанковскийСчетПлательщика =
	...
	Пока ВыборкаДокументовОтгрузки.Следующий() Цикл
		...
		Иначе
			Если ВыборкаДокументовОтгрузки.ЗаказВРаботе = 1 Тогда
...
// Параметры:
//   ИнформацияОбОтгрузке - см. НовыйСписокАтрибутовИнформацииОбОтгрузке
//   НастройкиСклада - см. ИнтеграцияСМаркетплейсамиСервер.НовыйНастройкиСкладаFBS
//                   - Неопределено - не передаются настройки склада.
//   ТолькоЗаказыВСтатусеПередаетсяВДоставку - Булево
//
// Возвращаемое значение:
//   Соответствие из КлючИЗначение - данные перезаполняемых отправлений:
//     * Ключ - Строка - номер отправления.
//     * Значение - Структура:
//         ** Документ - ДокументСсылка.РеализацияТоваровУслуг
//                     - ДокументСсылка.ПередачаТоваровХранителю
//         ** Проведен - Булево
//
Функция ПолучитьОтправленияДляПерезаполненияНастроекДоставки(ИнформацияОбОтгрузке, НастройкиСклада = Неопределено,
			ТолькоЗаказыВСтатусеПередаетсяВДоставку = Истина) Экспорт
	...
	Пока ВыборкаДокументовОтгрузки.Следующий() Цикл
		Если ТолькоЗаказыВСтатусеПередаетсяВДоставку И Не ВыборкаДокументовОтгрузки.ЗаказПередаетсяВДоставку Тогда
			Продолжить;
		КонецЕсли;
...
Функция СтатусыПечатиЭтикеток() Экспорт
	...
	СтатусыПечатиЭтикетки = Новый Массив; // Массив из ПеречислениеСсылка.СтатусыЗаказовТорговыхПлощадок 
...
//
// Возвращаемое значение:
//   См. НовыйРезультатПолученияФайла
//
Функция ПолучитьФайлЭтикетокГрузовыхМестОтгрузки(ИнформацияОбОтгрузке, ТранспортнаяНакладная,
	...
	ИначеЕсли ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоЯндексМаркет(ИнформацияОбОтгрузке.ВидМаркетплейса) Тогда
		РезультатПолучения = ИнтеграцияСЯндексМаркетСервер.ПолучитьЭтикеткиГрузомест(
			ИнформацияОбОтгрузке.УчетнаяЗапись,
			ИнформацияОбОтгрузке.ИдентификаторОтгрузки, ,
			Адрес);
		
		АдресДвоичныхДанных = РезультатПолучения.АдресФайла;
		РасширениеФайла = "pdf";
	КонецЕсли;
	...
	ДанныеФайла = НовыйРезультатПолученияФайла();
	ДанныеФайла.АдресДвоичныхДанных = АдресДвоичныхДанных;
	ДанныеФайла.РасширениеФайла = РасширениеФайла;
...
//   НаименованиеФайла     - Строка - наименование, под которым будет записан файл.
//   Адрес                 - Строка, УникальныйИдентификатор - адрес во временном хранилище.
//   ПолучитьАкт           - Булево - используется, если печатные формы накладной и акта разделены по разным методам.
//
// Возвращаемое значение:
//   См. НовыйРезультатПолученияФайла
//
Функция ПолучитьФайлНакладнойАкта(ИнформацияОбОтгрузке, ТранспортнаяНакладная,
			НаименованиеФайла = "", Адрес = "", ПолучитьАкт = Истина) Экспорт
	...
	ИначеЕсли ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоЯндексМаркет(ИнформацияОбОтгрузке.ВидМаркетплейса) Тогда
		Если ПолучитьАкт Тогда
			РезультатПолучения = ИнтеграцияСЯндексМаркетСервер.АктПриемаПередачи(
				ИнформацияОбОтгрузке.УчетнаяЗапись,
				ИнформацияОбОтгрузке.ИдентификаторОтгрузки,
				Адрес);
			...
			РасширениеФайла = "pdf";
		Иначе
			РезультатПолучения = ИнтеграцияСЯндексМаркетСервер.ТранспортнаяНакладная(
				ИнформацияОбОтгрузке.УчетнаяЗапись,
				ИнформацияОбОтгрузке.ИдентификаторОтгрузки,
				Адрес);
...
// Получает печатную форму фактического акта приема-передачи.
//
// Параметры:
//   ИнформацияОбОтгрузке  - см. НовыйСписокАтрибутовИнформацииОбОтгрузке.
//   ТранспортнаяНакладная - ДокументСсылка.ТранспортнаяНакладная - транспортная накладная,
...
//   См. НовыйРезультатПолученияФайла
//
Функция ПолучитьФайлФактическогоАктаПриемаПередачи(ИнформацияОбОтгрузке, ТранспортнаяНакладная,
			НаименованиеФайла = "", Адрес = "") Экспорт
	...
	Если ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоЯндексМаркет(ИнформацияОбОтгрузке.ВидМаркетплейса) Тогда
		РезультатПолучения = ИнтеграцияСЯндексМаркетСервер.ФактическийАктПриемаПередачи(
			ИнформацияОбОтгрузке.УчетнаяЗапись,
			ИнформацияОбОтгрузке.ИдентификаторОтгрузки,
			Адрес);
...
// Получает печатную форму акта о расхождениях.
//
// Параметры:
//   ИнформацияОбОтгрузке  - см. НовыйСписокАтрибутовИнформацииОбОтгрузке.
//   ТранспортнаяНакладная - ДокументСсылка.ТранспортнаяНакладная - транспортная накладная,
...
//   См. НовыйРезультатПолученияФайла
//
Функция ПолучитьФайлАктаОРасхождениях(ИнформацияОбОтгрузке, ТранспортнаяНакладная,
			НаименованиеФайла = "", Адрес = "") Экспорт
	...
	Если ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоЯндексМаркет(ИнформацияОбОтгрузке.ВидМаркетплейса) Тогда
		РезультатПолучения = ИнтеграцияСЯндексМаркетСервер.АктРасхождений(
			ИнформацияОбОтгрузке.УчетнаяЗапись,
			ИнформацияОбОтгрузке.ИдентификаторОтгрузки,
			Адрес);
...
//  ЗаполнитьСведенияОбЭкземплярах - Булево
//
// Возвращаемое значение:
//  Структура:
//   * ДанныеЗаказов - РезультатЗапроса
//   * ОтправленияИзмененные - РезультатЗапроса
//   * ОтправленияСНезаполненнымиДанными - РезультатЗапроса
//   * ЭкземплярыБезДокументовОтгрузки - РезультатЗапроса
//
Функция ДанныеДляПроверкиЗаказов(Заказы, ЗаполнитьСведенияОбЭкземплярах)
	
	ТекстыЗапроса = Новый СписокЗначений; // СписокЗначений из Строка
	
	// Данные из регистра получим один раз, далее будем переиспользовать уже их
	ТекстЗапроса =
		"ВЫБРАТЬ
		|	Заказы.Заказ КАК Заказ,
		|	Заказы.ДокументОтгрузки КАК ДокументОтгрузки,
		|	Заказы.УчетнаяЗапись КАК УчетнаяЗапись,
		|	Заказы.Статус КАК Статус,
		|	Заказы.НомерОтправления КАК НомерОтправления,
		|	Заказы.НомерРодительскогоОтправления КАК НомерРодительскогоОтправления
		|ПОМЕСТИТЬ ЗаказыТорговыхПлощадок
		|ИЗ
		|	РегистрСведений.ЗаказыТорговыхПлощадок КАК Заказы
		|ГДЕ
		|	Заказы.Заказ В(&Заказы)
		|
		...
		|ПОМЕСТИТЬ ДополнительныеСведенияПоТоварамЗаказовТорговыхПлощадок
		|ИЗ
	...
	ТекстыЗапроса.Добавить(ТекстЗапроса, "ДанныеРегистров");
	
	// Данные по заказам и документам отгрузки получим отдельно в целях исключения выборки всех данных из таблицы
	ТекстЗапроса =
		"ВЫБРАТЬ
		|	ТоварыЗаказов.Ссылка КАК Ссылка,
		|	ТоварыЗаказов.КодСтроки КАК КодСтроки,
		|	ТоварыЗаказов.Отменено КАК Отменено
		|ПОМЕСТИТЬ ТоварыЗаказов
		|ИЗ
		|	Документ.ЗаказКлиента.Товары КАК ТоварыЗаказов
		|ГДЕ
		|	ТоварыЗаказов.Ссылка В(&Заказы)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Ссылка,
		|	КодСтроки
		...
		|ИЗ
		|	ЗаказыТорговыхПлощадок КАК Заказы
		|ГДЕ
		|	Заказы.ДокументОтгрузки <> НЕОПРЕДЕЛЕНО
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|ИЗ
		|	ДополнительныеСведенияПоТоварамЗаказовТорговыхПлощадок КАК СведенияПоТоварам
		|ГДЕ
		...
		|ИЗ
		|	ВсеДокументыОтгрузки КАК ДокументыОтгрузки
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПередачаТоваровХранителю КАК ДанныеДокумента
		|		ПО ДокументыОтгрузки.ДокументОтгрузки = ДанныеДокумента.Ссылка
		|ГДЕ
		|	ДокументыОтгрузки.ДокументОтгрузки ССЫЛКА Документ.ПередачаТоваровХранителю
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|		ПО ДокументыОтгрузки.ДокументОтгрузки = ДанныеДокумента.Ссылка
		|ГДЕ
		|	ДокументыОтгрузки.ДокументОтгрузки ССЫЛКА Документ.РеализацияТоваровУслуг
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ДокументОтгрузки";
	
	ТекстыЗапроса.Добавить(ТекстЗапроса, "ДанныеЗаказовИДокументовОтгрузки");
	...
	ТекстЗапроса =
		...
		|	НЕ ЕСТЬNULL(ТоварыЗаказов.Отменено, ИСТИНА)
		|		И (СведенияПоТоварам.ДокументОтгрузки = НЕОПРЕДЕЛЕНО
		|			ИЛИ ЕСТЬNULL(ДанныеДокументовОтгрузки.Проведен, ЛОЖЬ)) КАК НетОтгрузки,
		|	ВЫБОР
		...
		|		ПО СведенияПоТоварам.ДокументОтгрузки = ДанныеДокументовОтгрузки.ДокументОтгрузки
	...
	ТекстыЗапроса.Добавить(ТекстЗапроса, "ЭкземплярыБезДокументовОтгрузки");
	...
	ТекстЗапроса =
		...
		|	Заказы.ДокументОтгрузки КАК ДокументОтгрузки,
		|	Заказы.УчетнаяЗапись КАК УчетнаяЗапись,
		|	ДанныеУчетныхЗаписей.ВидМаркетплейса КАК ВидМаркетплейса,
		|	ДанныеУчетныхЗаписей.ВидМаркетплейса В (&МаркетплейсыБезДокументаОтгрузки) КАК ДокументОтгрузкиВСведениях
		|ПОМЕСТИТЬ ОтборЗаказовТорговыхПлощадок
		|ИЗ
		|	ЗаказыТорговыхПлощадок КАК Заказы
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УчетныеЗаписиМаркетплейсов КАК ДанныеУчетныхЗаписей
		|		ПО Заказы.УчетнаяЗапись = ДанныеУчетныхЗаписей.Ссылка
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Заказ,
		|	ДокументОтгрузки
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ОтборЗаказов.Заказ КАК Заказ,
		|	ОтборЗаказов.ВидМаркетплейса КАК ВидМаркетплейса,
		|	ОтборЗаказов.УчетнаяЗапись КАК УчетнаяЗапись
		|ПОМЕСТИТЬ УчетныеЗаписиЗаказов
		|ИЗ
		|	ОтборЗаказовТорговыхПлощадок КАК ОтборЗаказов
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Заказ";
	
	ТекстыЗапроса.Добавить(ТекстЗапроса, "УчетныеЗаписиЗаказов");
	...
	ТекстЗапроса =
		...
		|	СведенияПоТоварам.НомерРодительскогоОтправления КАК НомерРодительскогоОтправления,
		|	СведенияПоТоварам.ИдентификаторЭкземпляра КАК ИдентификаторЭкземпляра,
		|	СведенияПоТоварам.ПозицияКОтгрузке КАК ПозицияКОтгрузке,
		|	СведенияПоТоварам.НетПривязкиКОтгрузке КАК НетПривязкиКОтгрузке,
		|	СведенияПоТоварам.ПозицияПеренесена КАК ПозицияПеренесена
		|ПОМЕСТИТЬ СведенияПоТоварамСДокументамиОтгрузки
		|ИЗ
		|	СведенияПоТоварам КАК СведенияПоТоварам
		|ГДЕ
		|	ИСТИНА В
	...
	ТекстЗапроса = РегистрыСведений.ДополнительныеСведенияПоТоварамЗаказовТорговыхПлощадок.ТекстЗапросаПереопределениеДокументовОтгрузкиИНомеровОтправлений(
		"СведенияПоТоварамСДокументамиОтгрузки");
	ТекстыЗапроса.Добавить(ТекстЗапроса);
	...
	ТекстЗапроса =
		...
		|	СведенияПоТоварам.НетПривязкиКОтгрузке КАК НетПривязкиКОтгрузке,
		|	СведенияПоТоварам.ПозицияПеренесена КАК ПозицияПеренесена
		|ПОМЕСТИТЬ СведенияПоТоварамСУчетомПереопределения
		|ИЗ
		|	СведенияПоТоварамСДокументамиОтгрузки КАК СведенияПоТоварам
	...
	ТекстыЗапроса.Добавить(ТекстЗапроса, "СведенияПоТоварамСУчетомПереопределения");
	
	ТекстЗапроса =
		"ВЫБРАТЬ
		|	Заказы.УчетнаяЗапись КАК УчетнаяЗапись,
		|	Заказы.Заказ КАК Заказ,
		|	СведенияПоТоварам.Заказ КАК ЗаказИзСведений,
		|	Заказы.НомерОтправления КАК НомерОтправления,
		|	Заказы.НомерРодительскогоОтправления КАК НомерРодительскогоОтправления,
		|	Заказы.ДокументОтгрузки КАК ДокументОтгрузки,
		|	ЕСТЬNULL(СведенияПоТоварам.ДокументОтгрузки, НЕОПРЕДЕЛЕНО) КАК ДокументОтгрузкиИзСведений,
		|	Заказы.Статус КАК Статус,
		|	ЕСТЬNULL(СведенияПоТоварам.ИдентификаторЭкземпляра, 0) КАК ИдентификаторЭкземпляра,
		|	ЕСТЬNULL(СведенияПоТоварам.ПозицияКОтгрузке, ЛОЖЬ) КАК ПозицияКОтгрузке,
		|	ЕСТЬNULL(СведенияПоТоварам.НетПривязкиКОтгрузке, ЛОЖЬ) КАК НетПривязкиКОтгрузке,
		|	ЕСТЬNULL(СведенияПоТоварам.ПозицияПеренесена, ЛОЖЬ) КАК ПозицияПеренесена
		|ПОМЕСТИТЬ ВсеОтправленияПоЗаказам
		|ИЗ
		|	ЗаказыТорговыхПлощадок КАК Заказы
		|		ЛЕВОЕ СОЕДИНЕНИЕ СведенияПоТоварам КАК СведенияПоТоварам
		|		ПО Заказы.Заказ = СведенияПоТоварам.Заказ
		|			И Заказы.НомерОтправления = СведенияПоТоварам.НомерОтправления
		|ГДЕ
		|	ИСТИНА В
		|			(ВЫБРАТЬ ПЕРВЫЕ 1
		|				ИСТИНА
		...
		|				ОтборЗаказовТорговыхПлощадок КАК Отбор
		|			ГДЕ
		|				Отбор.Заказ = Заказы.Заказ
		|				И Отбор.ДокументОтгрузки = Заказы.ДокументОтгрузки
		|				И НЕ Отбор.ДокументОтгрузкиВСведениях)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	Заказы.УчетнаяЗапись,
		|	Заказы.Заказ,
		|	СведенияПоТоварам.Заказ,
		|	ЕСТЬNULL(СведенияПоТоварам.НомерОтправления, Заказы.НомерОтправления),
		|	ЕСТЬNULL(СведенияПоТоварам.НомерРодительскогоОтправления, Заказы.НомерРодительскогоОтправления),
		|	ЕСТЬNULL(СведенияПоТоварам.ДокументОтгрузки, НЕОПРЕДЕЛЕНО),
		|	ЕСТЬNULL(СведенияПоТоварам.ДокументОтгрузки, НЕОПРЕДЕЛЕНО),
		|	Заказы.Статус,
		|	ЕСТЬNULL(СведенияПоТоварам.ИдентификаторЭкземпляра, 0),
		|	ЕСТЬNULL(СведенияПоТоварам.ПозицияКОтгрузке, ЛОЖЬ),
		|	ЕСТЬNULL(СведенияПоТоварам.НетПривязкиКОтгрузке, ЛОЖЬ),
		|	ЕСТЬNULL(СведенияПоТоварам.ПозицияПеренесена, ЛОЖЬ)
		|ИЗ
		|	ЗаказыТорговыхПлощадок КАК Заказы
		|		ЛЕВОЕ СОЕДИНЕНИЕ СведенияПоТоварамСУчетомПереопределения КАК СведенияПоТоварам
		|		ПО Заказы.Заказ = СведенияПоТоварам.Заказ
		|ГДЕ
		|	ИСТИНА В
		|			(ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|	ЕСТЬNULL(МАКСИМУМ(ЗаказыТорговыхПлощадок.ИдентификаторЭкземпляра), 0) КАК МаксимальныйИдентификаторЭкземпляра,
		|	ЕСТЬNULL(МАКСИМУМ(ЗаказыТорговыхПлощадок.ДокументОтгрузки <> НЕОПРЕДЕЛЕНО
		|				И (ЗаказыТорговыхПлощадок.ЗаказИзСведений ЕСТЬ NULL
		|					ИЛИ ЗаказыТорговыхПлощадок.ПозицияКОтгрузке
		|						И (ЗаказыТорговыхПлощадок.НетПривязкиКОтгрузке
		|							ИЛИ ЗаказыТорговыхПлощадок.ДокументОтгрузки <> ЗаказыТорговыхПлощадок.ДокументОтгрузкиИзСведений))), ИСТИНА) КАК ЕстьРасхождения,
		|	ЕСТЬNULL(СУММА(ВЫБОР
		|				КОГДА ЗаказыТорговыхПлощадок.ЗаказИзСведений ЕСТЬ NULL
		|					ТОГДА 0
		|				КОГДА НЕ ЗаказыТорговыхПлощадок.НетПривязкиКОтгрузке
		|					ТОГДА 0
		|				КОГДА НЕ ЗаказыТорговыхПлощадок.ПозицияПеренесена
		|					ТОГДА 0
		...
		|ИЗ
		|	ВсеОтправленияПоЗаказам КАК ЗаказыТорговыхПлощадок
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ЕСТЬNULL(УчетныеЗаписиЗаказов.ВидМаркетплейса, ЗНАЧЕНИЕ(Перечисление.ВидыМаркетплейсов.ПустаяСсылка)) КАК ВидМаркетплейса,
		...
		|	ЕСТЬNULL(ДанныеДокументовОтгрузки.Номер, """") КАК НомерДокументаОтгрузки,
		|	ЕСТЬNULL(ДанныеДокументовОтгрузки.Дата, """") КАК ДатаДокументаОтгрузки,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеДокументовОтгрузки КАК ДанныеДокументовОтгрузки
		|		ПО ОтправленияПоЗаказам.ДокументОтгрузки = ДанныеДокументовОтгрузки.ДокументОтгрузки
		|		ЛЕВОЕ СОЕДИНЕНИЕ УчетныеЗаписиЗаказов КАК УчетныеЗаписиЗаказов
		|		ПО ОтправленияПоЗаказам.Заказ = УчетныеЗаписиЗаказов.Заказ
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ЕСТЬNULL(МАКСИМУМ(ЗаказыТорговыхПлощадок.ВидМаркетплейса), ЗНАЧЕНИЕ(Перечисление.ВидыМаркетплейсов.ПустаяСсылка)),
		|	СведенияПоТоварам.НомерОтправления,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ УчетныеЗаписиЗаказов КАК ЗаказыТорговыхПлощадок
	...
	ТекстыЗапроса.Добавить(ТекстЗапроса, "ДанныеЗаказов");
	...
	ТекстЗапроса =
		...
		|				И СведенияПоТоварам.ДокументОтгрузки = ОтправленияПоЗаказам.ДокументОтгрузки
		|	ГДЕ
		|		ОтправленияПоЗаказам.ДокументОтгрузки ЕСТЬ NULL
		|		И ИСТИНА В
		|				(ВЫБРАТЬ ПЕРВЫЕ 1
		|					ИСТИНА
		|				ИЗ
		...
		|					ИСТИНА
		|				ИЗ
		|					ДанныеДокументовОтгрузки КАК ДанныеДокументовОтгрузки
		|				ГДЕ
		|					СведенияПоТоварам.ДокументОтгрузки = ДанныеДокументовОтгрузки.ДокументОтгрузки
		|					И НЕ ДанныеДокументовОтгрузки.Проведен)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	ТекстыЗапроса.Добавить(ТекстЗапроса, "ОтправленияИзмененные");
	...
	ТекстыЗапроса.Добавить(ТекстЗапроса, "ОтправленияСНезаполненнымиДанными");
	...
	Запрос.УстановитьПараметр(
		"СтатусыВДоставке", Перечисления.СтатусыЗаказовТорговыхПлощадок.СтатусыЗаказовВДоставке(Ложь));
	Запрос.УстановитьПараметр(
		"МаркетплейсыБезДокументаОтгрузки",
		ИнтеграцияСМаркетплейсамиСервер.МаркетплейсыБезДокументаОтгрузкиВЗаказах());
	
	УстановитьПривилегированныйРежим(Истина);
	Результат = ОбщегоНазначенияУТ.ПолучитьРезультатыЗапроса(Запрос, ТекстыЗапроса);
	УстановитьПривилегированныйРежим(Ложь);
...
Функция ПерезаполнитьДокументыОтгрузки(РезультатПроверкиЗаказов, СведенияПоТоварам, Автор = Неопределено)
	...
	Для Каждого ДанныеПоЗаказу Из РезультатПроверкиЗаказов Цикл
		...
		Если РезультатПроверкиЗаказа.ИзмененныеДокументыОтгрузки.Количество() > 0 Тогда
			...
			Для Каждого ДанныеДокументаОтгрузки Из РезультатПроверкиЗаказа.ИзмененныеДокументыОтгрузки Цикл
				ДокументОтгрузки = ДанныеДокументаОтгрузки.Ключ;
				НомерОтправления = ДанныеДокументаОтгрузки.Значение;
				
				Если ПроблемныеДокументы.Найти(ДокументОтгрузки) <> Неопределено Тогда
					Продолжить;
					
				ИначеЕсли ПустаяСтрока(НомерОтправления) Тогда
					Продолжить;
				КонецЕсли;
...
Функция СоздатьДокументыОтгрузки(РезультатПроверкиЗаказов, СведенияПоТоварам, Автор = Неопределено)
	...
	Для Каждого ДанныеПоЗаказу Из РезультатПроверкиЗаказов Цикл
		...
		ТребуетсяУказаниеДокументаОтгрузки =
			ИнтеграцияСМаркетплейсамиСервер.ТребуетсяУказаниеДокументаОтгрузкиВЗаказахТорговыхПлощадок(
				РезультатПроверкиЗаказа.ВидМаркетплейса);
		...
		Для Каждого НомерОтправления Из РезультатПроверкиЗаказа.ОтправленияБезДокументовОтгрузки Цикл
			...
			Если ТребуетсяУказаниеДокументаОтгрузки И ЗначениеЗаполнено(ДокументОтгрузки) Тогда
				НачатьТранзакцию();
...
//  ДанныеПоНомерамОтправлений - Соответствие из КлючИЗначение:
//   * Ключ - Строка - номер отправления.
//   * Значение - см. ИнтеграцияСМаркетплейсамиКлиентСервер.РезультатПроверкиЗаказа
//
// Возвращаемое значение:
//  СправочникСсылка.УчетныеЗаписиМаркетплейсов
//
Функция УчетнаяЗаписьИзДанныхПоНомерамОтправлений(ДанныеПоНомерамОтправлений)
	
	Результат = Справочники.УчетныеЗаписиМаркетплейсов.ПустаяСсылка();
	
	Для Каждого КлючЗначение Из ДанныеПоНомерамОтправлений Цикл
		
		ДанныеПоНомеруОтправления = КлючЗначение.Значение;
		Результат = ДанныеПоНомеруОтправления.УчетнаяЗапись;
		Прервать;
...
Процедура ЗаполнитьДополнительныеДанныеЗаписиПоДаннымОтправления(ДанныеЗаписи, ДанныеОтправления)
	
	ДанныеЗаполнения = Новый Структура;
	ДанныеЗаполнения.Вставить("ОписаниеОшибки", Неопределено);
	ДанныеЗаполнения.Вставить("Действие", Неопределено);
	
	ЗаполнитьЗначенияСвойств(ДанныеЗаполнения, ДанныеОтправления);
	
	Для Каждого КлючЗначение Из ДанныеЗаполнения Цикл
		Если КлючЗначение.Значение <> Неопределено Тогда
			ДанныеЗаписи[КлючЗначение.Ключ] = КлючЗначение.Значение;
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

// Конструктор результата поиска данных заказа по строке поиска.
// 
// Возвращаемое значение:
//  Структура:
//   * ИмяПоляПоиска - Строка - имя поля, поиск по которому дал результат.
//   * Заказ - ДокументСсылка.ЗаказКлиента
//   * ДокументОтгрузки - Неопределено
//                      - ДокументСсылка.РеализацияТоваровУслуг
//                      - ДокументСсылка.ПередачаТоваровХранителю
...
//
Функция НовыйРезультатПоискаДанныхЗаказаПоСтрокеПоиска()
	
	Результат = Новый Структура;
	
	Результат.Вставить("ИмяПоляПоиска", "");
	Результат.Вставить("Заказ", Документы.ЗаказКлиента.ПустаяСсылка());
	Результат.Вставить("ДокументОтгрузки", Неопределено);
	Результат.Вставить("ИдентификаторЗаказа", "");
	Результат.Вставить("НомерЗаказа", "");
	Результат.Вставить("НомерОтправления", "");
	Результат.Вставить("ДатаСозданияЗаказа", Дата(1, 1, 1));
...
Функция ВыборкаДокументовОтгрузки(УчетнаяЗапись, НомераОтправлений, Итоги = Ложь, СведенияПоДоставке = Ложь)
	...
	ТаблицаОтправлений = НоваяТаблицаОтправлений();
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицуИзМассива(ТаблицаОтправлений, НомераОтправлений, "НомерОтправления");
	...
	ТекстЗапроса =
		...
		|	Заказы.Заказ КАК Заказ,
		|	Заказы.ДокументОтгрузки КАК ДокументОтгрузки
		|ПОМЕСТИТЬ ОтборРегистраЗаказыТорговыхПлощадок
		|ИЗ
		|	РегистрСведений.ЗаказыТорговыхПлощадок КАК Заказы
		|ГДЕ
		|	ИСТИНА В
		...
		|				ИСТИНА
		|			ИЗ
		|				ТаблицаОтправлений КАК Отбор
		|			ГДЕ
		|				Заказы.НомерОтправления = Отбор.НомерОтправления)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Заказ,
		|	ДокументОтгрузки
		...
		|	Заказы.ДокументОтгрузки КАК ДокументОтгрузки,
		|	Заказы.Статус КАК Статус,
		|	Заказы.ИдентификаторОтгрузки КАК ИдентификаторОтгрузки
		|ПОМЕСТИТЬ ДанныеРегистраЗаказыТорговыхПлощадок
		|ИЗ
		|	РегистрСведений.ЗаказыТорговыхПлощадок КАК Заказы
		|ГДЕ
		|	ИСТИНА В
		...
		|				ИСТИНА
		|			ИЗ
		|				ОтборРегистраЗаказыТорговыхПлощадок КАК Отбор
		|			ГДЕ
		|				Заказы.Заказ = Отбор.Заказ
		|				И Заказы.ДокументОтгрузки = Отбор.ДокументОтгрузки)
		|	И Заказы.УчетнаяЗапись = &УчетнаяЗапись
		|
		|ИНДЕКСИРОВАТЬ ПО НАБОРАМ
		|(
		|	(Заказ),
	...
	ИмяОсновнойТаблицы = "ДанныеРегистраЗаказыТорговыхПлощадок";
	
	Если Не ДокументОтгрузкиВЗаказах Тогда
		ТекстЗапроса =
			"ВЫБРАТЬ
			...
			|ПОМЕСТИТЬ ОтборРегистраДополнительныеСведенияПоТоварамЗаказовТорговыхПлощадок
			|ИЗ
			...
			|				ТаблицаОтправлений КАК Отбор
			|			ГДЕ
			|				Сведения.НомерОтправления = Отбор.НомерОтправления)
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	Заказ,
			|	ИдентификаторТовара,
			...
			|	Заказы.Статус КАК Статус
			|ПОМЕСТИТЬ ЗаказыТорговыхПлощадокСУчетомДополнительныхСведений
			|ИЗ
			|	РегистрСведений.ДополнительныеСведенияПоТоварамЗаказовТорговыхПлощадок КАК Сведения
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеРегистраЗаказыТорговыхПлощадок КАК Заказы
			|		ПО Сведения.Заказ = Заказы.Заказ
			|			И (Заказы.ДокументОтгрузки = НЕОПРЕДЕЛЕНО)
			|ГДЕ
			|	ИСТИНА В
			|			(ВЫБРАТЬ ПЕРВЫЕ 1
			|				ИСТИНА
			|			ИЗ
			|				ОтборРегистраДополнительныеСведенияПоТоварамЗаказовТорговыхПлощадок КАК Отбор
			|			ГДЕ
			|				Сведения.Заказ = Отбор.Заказ
			|				И Сведения.ИдентификаторТовара = Отбор.ИдентификаторТовара
			|				И Сведения.ИдентификаторЭкземпляра = Отбор.ИдентификаторЭкземпляра)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			...
			|	Заказы.Статус
			|ИЗ
			|	ДанныеРегистраЗаказыТорговыхПлощадок КАК Заказы
			|		ЛЕВОЕ СОЕДИНЕНИЕ ОтборРегистраДополнительныеСведенияПоТоварамЗаказовТорговыхПлощадок КАК Сведения
			|		ПО Заказы.Заказ = Сведения.Заказ
			|ГДЕ
			|	Сведения.Заказ ЕСТЬ NULL
			|
			|ИНДЕКСИРОВАТЬ ПО НАБОРАМ
			|(
			|	(Заказ),
		...
		ИмяОсновнойТаблицы = "ЗаказыТорговыхПлощадокСУчетомДополнительныхСведений";
	КонецЕсли;
	...
	ТекстЗапроса =
		...
		|ИЗ
		|	ДанныеРегистраЗаказыТорговыхПлощадок КАК ЗаказыТорговыхПлощадок
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ДанныеРегистраЗаказыТорговыхПлощадок", ИмяОсновнойТаблицы);
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&СведенияПоДоставке КАК СведенияПоДоставке,", ТекстСведенийПоДоставке);
...
Функция ПроверитьОтправления(ИнформацияОбОтгрузке)
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицуИзМассива(
		ТаблицаОтправлений, ИнформацияОбОтгрузке.ОтправленияВОтгрузке, "НомерОтправления");
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицуИзМассива(
		ТаблицаОтправлений, ИнформацияОбОтгрузке.ОтправленияВПланеОтгрузки, "НомерОтправления");
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицуИзМассива(
		ТаблицаОтправлений, ИнформацияОбОтгрузке.ОтправленияНеВОтгрузке, "НомерОтправления");
	...
	Если ДокументОтгрузкиВЗаказах Тогда
		Запрос.Текст = Запрос.Текст + ОбщегоНазначения.РазделительПакетаЗапросов() +
			...
			|ИЗ
			|	ТаблицаОтправлений КАК ТаблицаОтправлений
			|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеРегистраЗаказыТорговыхПлощадок КАК ЗаказыТорговыхПлощадок
			|		ПО ТаблицаОтправлений.НомерОтправления = ЗаказыТорговыхПлощадок.НомерОтправления
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	Заказ";
	Иначе
		Запрос.Текст = Запрос.Текст + ОбщегоНазначения.РазделительПакетаЗапросов() +
			...
			|	Сведения.Заказ КАК Заказ,
			|	Сведения.ДокументОтгрузки КАК ДокументОтгрузки
			|ПОМЕСТИТЬ ДанныеРегистраДополнительныеСведенияПоТоварамЗаказовТорговыхПлощадок
			|ИЗ
			|	РегистрСведений.ДополнительныеСведенияПоТоварамЗаказовТорговыхПлощадок КАК Сведения
			|ГДЕ
			|	ИСТИНА В
			...
			|ПОМЕСТИТЬ ЗаказыТорговыхПлощадок
			|ИЗ
			|	ДанныеРегистраДополнительныеСведенияПоТоварамЗаказовТорговыхПлощадок КАК Сведения
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеРегистраЗаказыТорговыхПлощадок КАК ЗаказыТорговыхПлощадок
			|		ПО Сведения.Заказ = ЗаказыТорговыхПлощадок.Заказ
			|			И (ЗаказыТорговыхПлощадок.ДокументОтгрузки = НЕОПРЕДЕЛЕНО)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			|	ТаблицаОтправлений.НомерОтправления,
			|	ЗаказыТорговыхПлощадок.Заказ,
			|	ЗаказыТорговыхПлощадок.ДокументОтгрузки,
			|	ЗаказыТорговыхПлощадок.Статус,
			|	ЗаказыТорговыхПлощадок.ИдентификаторОтгрузки
			...
			|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеРегистраЗаказыТорговыхПлощадок КАК ЗаказыТорговыхПлощадок
			|		ПО ТаблицаОтправлений.НомерОтправления = ЗаказыТорговыхПлощадок.НомерОтправления
			|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеРегистраДополнительныеСведенияПоТоварамЗаказовТорговыхПлощадок КАК Сведения
			|		ПО ТаблицаОтправлений.НомерОтправления = Сведения.НомерОтправления
			|ГДЕ
			|	Сведения.НомерОтправления ЕСТЬ NULL
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	Заказ";
	КонецЕсли;
	...
	Если Не ИнформацияОбОтгрузке.ОтгрузкаОтменена
			...
			ИначеЕсли ЭтоЯндексМаркет Тогда   
				Если ТребуетсяПовторнаяЗагрузкаЗаказов Тогда
					Шаблон = НСтр("ru = 'В отгрузку включены заказы, которые не загружены в учетную систему или требуют повторной загрузки: %1.'");
				Иначе
					Шаблон = НСтр("ru = 'В отгрузку включены заказы, которые не загружены в учетную систему: %1.'");
				КонецЕсли; 
			КонецЕсли;  
			
			Если ЭтоOzon Тогда
				СписокДокументов = СтрСоединить(ИнформацияОбОтгрузке.ОтправленияНезагруженные, ", ");
			ИначеЕсли ЭтоЯндексМаркет И Не ИнформацияОбОтгрузке.ВсеНеЗагруженныеЗаказыОтменены Тогда
				СписокДокументов = СтрСоединить(ИнформацияОбОтгрузке.ЗаказыНеЗагруженные, ", ");
			КонецЕсли;
...
// Параметры:
//  УчетнаяЗапись - СправочникСсылка.УчетныеЗаписиМаркетплейсов
//  ДанныеДляПереноса - Соответствие из КлючИЗначение:
//   * Ключ - Строка - идентификатор отгрузки.
//   * Значение - Массив из Строка - идентификаторы заказов.
// 
// Возвращаемое значение:
//  См. ИнтеграцияСМаркетплейсамиСервер.НоваяОшибка
//
Функция ПеренестиЗаказыВСледующуюОтгрузку(УчетнаяЗапись, ДанныеДляПереноса) Экспорт
	
	Результат = ИнтеграцияСМаркетплейсамиСервер.НоваяОшибка();
	
	ПеренесенныеЗаказы = Новый Массив; // Массив из Строка
	
	ВидМаркетплейса = ИнтеграцияСМаркетплейсамиПовтИсп.ВидМаркетплейсаУчетнойЗаписи(УчетнаяЗапись);
	Если ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоЯндексМаркет(ВидМаркетплейса) Тогда
		
		Для Каждого КлючЗначение Из ДанныеДляПереноса Цикл
			ИдентификаторОтгрузки = КлючЗначение.Ключ;
			ИдентификаторыЗаказов = КлючЗначение.Значение;
			
			Ошибка = ИнтеграцияСЯндексМаркетСервер.ПеренестиЗаказыВСледующуюОтгрузку(
				УчетнаяЗапись,
				ИдентификаторОтгрузки,
				ИдентификаторыЗаказов);
			
			Если Ошибка <> Неопределено Тогда
				ИнтеграцияСМаркетплейсамиСервер.ДополнитьОшибку(Результат, Ошибка);
			Иначе
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ПеренесенныеЗаказы, ИдентификаторыЗаказов);
			КонецЕсли;
		КонецЦикла;
		
	КонецЕсли;
	
	Если ПеренесенныеЗаказы.Количество() > 0 Тогда
		ОчиститьСрокОтгрузкиВЗаказах(ВидМаркетплейса, УчетнаяЗапись, ПеренесенныеЗаказы);
	КонецЕсли;
...
Функция ОчиститьСрокОтгрузкиВЗаказах(ВидМаркетплейса, УчетнаяЗапись, НомераЗаказов)
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	&ТекстЗапросаПолейОбъектаМетаданных КАК Поля
		|ИЗ
		|	РегистрСведений.ЗаказыТорговыхПлощадок КАК Заказы
		|ГДЕ
		|	Заказы.НомерЗаказа В (&НомераЗаказов)";
	
	ТекстЗапросаПолейОбъектаМетаданных = ИнтеграцияСМаркетплейсамиСервер.ТекстЗапросаПолейОбъектаМетаданных(
		"РегистрСведений.ЗаказыТорговыхПлощадок",
		"Заказы");
	
	Запрос.Текст = СтрЗаменить(
		Запрос.Текст,
		"&ТекстЗапросаПолейОбъектаМетаданных КАК Поля",
		ТекстЗапросаПолейОбъектаМетаданных);
	Запрос.Параметры.Вставить("НомераЗаказов",НомераЗаказов);
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			Запись.СрокОтгрузки = Дата(1, 1, 1);
			Запись.ИдентификаторОтгрузки = "";
		КонецЦикла;
	...
	Исключение
		...
		ШаблонСообщенияОНевыведенныхДанных = НСтр("ru = ';%1 номер заказа;;%1 номера заказа;%1 номеров заказов;'");
		НомераДляВывода = ИнтеграцияСМаркетплейсамиКлиентСервер.ЭлементыМассиваДляВыводаСообщений(
			НомераЗаказов,
			ШаблонСообщенияОНевыведенныхДанных);
		
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось очистить срок отгрузки для заказов: %1.'"),
			СтрСоединить(НомераДляВывода, ", "));
		
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ПолныйТекстОшибки = ТекстОшибки + Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
...
Функция НоваяТаблицаОтправлений()
	
	МетаданныеРесурса = Метаданные.РегистрыСведений.ЗаказыТорговыхПлощадок.Ресурсы.НомерОтправления;
	
	Результат = Новый ТаблицаЗначений;
	Результат.Колонки.Добавить(МетаданныеРесурса.Имя, МетаданныеРесурса.Тип);
...
Процедура ОбновитьНастройкиДоставкиВОтправлениях(ИнформацияОбОтгрузке, МетодДоставки)
	...
	Если ПерезаполняемыеОтправления.Количество() > 0 Тогда
		ОбновитьДанныеПоДоставке(ИнформацияОбОтгрузке,)
	КонецЕсли;
...
//
Процедура ОбновитьДанныеПоДоставке(ИнформацияОбОтгрузке, Заказы = Неопределено)  
...
Процедура ДополнитьТекстыЗапросаПоДоставкеВыборкойДанныхПоРаспоряжениям(ТекстыЗапроса)
	...
	|	РаспоряженияНаОтгрузкуОбороты.Номенклатура КАК Номенклатура,
	|	РаспоряженияНаОтгрузкуОбороты.Характеристика КАК Характеристика,
	|	ЗаказКлиентаТовары.Назначение КАК Назначение,
	|	РаспоряженияНаОтгрузкуОбороты.Серия КАК Серия,
	|	РаспоряженияНаОтгрузкуОбороты.КодСтроки КАК КодСтроки,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТоварыЗаказов КАК ЗаказКлиентаТовары
	...
	|ИЗ
	|	ТоварыЗаказов КАК ЗаказКлиентаТовары
	...
	|ГДЕ
	|	ЗаказКлиентаТовары.КодСтроки ЕСТЬ НЕ NULL 
...
Процедура ДополнитьТекстыЗапросаПоДоставкеВыборкойДанныхПоОтправлениям(ТекстыЗапроса, ВидМаркетплейса)
	...
	Иначе
		ТекстЗапроса =
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	ЗаказыТорговыхПлощадок.Заказ КАК Распоряжение,
			|	ЕСТЬNULL(Сведения.НомерОтправления, ЗаказыТорговыхПлощадок.НомерОтправления) КАК НомерОтправления,
			|	НЕ ЗаказыТорговыхПлощадок.Статус В (&СтатусыНеотгружаемыхЗаказовРаботе) КАК ОтправлениеОтгружено
			|ПОМЕСТИТЬ ВсеОтправленияРаспоряжений
			|ИЗ
			|	РегистрСведений.ЗаказыТорговыхПлощадок КАК ЗаказыТорговыхПлощадок
	...
	|				ИСТИНА
	|			ИЗ
	|				ВсеОтправленияРаспоряжений КАК Отбор
	|			ГДЕ
	|				СведенияПоТоварам.Заказ = Отбор.Распоряжение
	|				И СведенияПоТоварам.НомерОтправления = Отбор.НомерОтправления)
	|
	|ИНДЕКСИРОВАТЬ ПО НАБОРАМ
	|(
	|	(ИдентификаторТовара),
	...
	Если ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоOzon(ВидМаркетплейса) Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	СтатусыПубликации.ИдентификаторFBOSKU КАК ИдентификаторТовара,
		|	СтатусыПубликации.Упаковка КАК Упаковка
		|ПОМЕСТИТЬ ВсеУпаковкиТоваровПоИдентификатору
		|ИЗ
		...
		|				ИСТИНА
		|			ИЗ
		|				ДанныеРегистраДополнительныеСведенияПоТоварамЗаказовТорговыхПлощадок КАК Отбор
		|			ГДЕ
		|				СтатусыПубликации.ИдентификаторFBOSKU = Отбор.ИдентификаторТовара)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	СтатусыПубликации.ИдентификаторFBSSKU,
		...
		|				ДанныеРегистраДополнительныеСведенияПоТоварамЗаказовТорговыхПлощадок КАК Отбор
		|			ГДЕ
		|				СтатусыПубликации.ИдентификаторFBSSKU = Отбор.ИдентификаторТовара)";
	...
	ИначеЕсли ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоЯндексМаркет(ВидМаркетплейса) Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	СтатусыПубликации.ИдентификаторПредложения КАК ИдентификаторТовара,
		|	СтатусыПубликации.Упаковка КАК Упаковка
		|ПОМЕСТИТЬ ВсеУпаковкиТоваровПоИдентификатору
		|ИЗ
		|	РегистрСведений.СтатусыПубликацииТоваровЯндексМаркет КАК СтатусыПубликации
		|ГДЕ
		...
		|				ДанныеРегистраДополнительныеСведенияПоТоварамЗаказовТорговыхПлощадок КАК Отбор
		|			ГДЕ
		|				СтатусыПубликации.ИдентификаторПредложения = Отбор.ИдентификаторОбъектаМаркетплейса)";
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ВсеУпаковки.ИдентификаторТовара КАК ИдентификаторТовара,
	|	МАКСИМУМ(ВсеУпаковки.Упаковка) КАК Упаковка
	|ПОМЕСТИТЬ УпаковкиТоваровПоИдентификатору
	|ИЗ
	|	ВсеУпаковкиТоваровПоИдентификатору КАК ВсеУпаковки
	|ГДЕ
	|	ВсеУпаковки.Упаковка <> ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|
	|СГРУППИРОВАТЬ ПО
	|	ВсеУпаковки.ИдентификаторТовара
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Упаковка
	|;
	...
	|ПОМЕСТИТЬ КоэффициентыУпаковокТоваровПоИдентификатору
	|ИЗ
	|	УпаковкиТоваровПоИдентификатору КАК ВсеУпаковки
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК СпрУпаковки
	|		ПО ВсеУпаковки.Упаковка = СпрУпаковки.Ссылка
	...
	ТекстЗапроса =
		...
		|				ТОГДА СведенияПоТоварам.Количество
		|			ИНАЧЕ 0
		|		КОНЕЦ * ЕСТЬNULL(КоэффициентыУпаковок.Коэффициент, 1)) КАК КоличествоКОтгрузке,
		|	СУММА(ВЫБОР
		|			КОГДА ВсеОтправленияРаспоряжений.ОтправлениеОтгружено
		|				ТОГДА СведенияПоТоварам.Количество
		|			ИНАЧЕ 0
		|		КОНЕЦ * ЕСТЬNULL(КоэффициентыУпаковок.Коэффициент, 1)) КАК КоличествоОтгружено
		|ПОМЕСТИТЬ ТоварыПоОтправлениям
		|ИЗ
		|	ДанныеРегистраДополнительныеСведенияПоТоварамЗаказовТорговыхПлощадок КАК СведенияПоТоварам
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВсеОтправленияРаспоряжений КАК ВсеОтправленияРаспоряжений
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ КоэффициентыУпаковокТоваровПоИдентификатору КАК КоэффициентыУпаковок
		|		ПО СведенияПоТоварам.ИдентификаторТовара = КоэффициентыУпаковок.ИдентификаторТовара
		|
		|СГРУППИРОВАТЬ ПО
		|	СведенияПоТоварам.Заказ,
		|	ТоварыПоРаспоряжениям.Склад,
...
// Конструктор результата получения файла.
// 
// Возвращаемое значение:
//  Структура:
...
//
Функция НовыйРезультатПолученияФайла()
	
	Результат = Новый Структура;
	Результат.Вставить("АдресДвоичныхДанных", "");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "РегистрыСведений.ЗаказыТорговыхПлощадок.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "2.5.27.23";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("8709123c-06f9-4118-b969-d64480cac3e8");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = 
		"РегистрыСведений.ЗаказыТорговыхПлощадок.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Очищает данные измерения ""%1"" по заказам учетных записей торговой площадки %2.'"),
		Метаданные.РегистрыСведений.ЗаказыТорговыхПлощадок.Измерения.ДокументОтгрузки.Представление(),
	...
	Изменяемые.Добавить(Метаданные.РегистрыСведений.ЗаказыТорговыхПлощадок.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	МетаданныеРегистра = Метаданные.РегистрыСведений.ЗаказыТорговыхПлощадок;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ДанныеРегистра.Заказ КАК Заказ,
		|	ДанныеРегистра.ДокументОтгрузки КАК ДокументОтгрузки
		|ИЗ
		|	РегистрСведений.ЗаказыТорговыхПлощадок КАК ДанныеРегистра
		|ГДЕ
		|	ДанныеРегистра.УчетнаяЗапись.ВидМаркетплейса = ЗНАЧЕНИЕ(Перечисление.ВидыМаркетплейсов.МаркетплейсЯндексМаркет)
		|	И ДанныеРегистра.ДокументОтгрузки <> НЕОПРЕДЕЛЕНО";
...
Процедура ОбработатьПорциюДанныхДляПереходаНаНовуюВерсию(ДанныеДляОбработки, ОбработанныеЗаписи, МетаданныеРегистра)
	...
	Попытка
		...
		НовыеЗаписи = СоздатьНаборЗаписей();
		
		Пока ДанныеДляОбработки.Следующий() Цикл
			ЗаполнитьЗначенияСвойств(ОбработанныеЗаписи.Добавить(), ДанныеДляОбработки);
			
			СтараяСтрока = СтарыеЗаписи.Добавить();
			ЗаполнитьЗначенияСвойств(СтараяСтрока, ДанныеДляОбработки);
			
			Если ПустаяСтрока(СтараяСтрока.НомерРодительскогоОтправления) Тогда
				НоваяСтрока = НовыеЗаписи.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ДанныеДляОбработки);
				НоваяСтрока.ДокументОтгрузки = Неопределено;
			КонецЕсли;
		КонецЦикла;
		
		СтарыеЗаписи.Записать(РежимЗамещения.Удаление);
		НовыеЗаписи.Записать(РежимЗамещения.Добавление);
