...
&НаКлиенте
Процедура ДекорацияОшибокНетОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ПерейтиПоНавигационнойСсылке(НавигационнаяСсылкаFBO);
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияОшибокНетFBSОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ПерейтиПоНавигационнойСсылке(НавигационнаяСсылкаFBS);
КонецПроцедуры
...
&НаКлиенте
Процедура ДекорацияПредупреждениеFBSОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ПерейтиПоНавигационнойСсылке(НавигационнаяСсылкаFBS);
КонецПроцедуры
...
Процедура ПолучитьЭтикетки(Команда)
	...
	Если Не ПустаяСтрока(Ошибка.КодОшибки) Тогда
		ИнтеграцияСМаркетплейсамиКлиент.ВывестиСостояние(Ошибка,, Истина, БиблиотекаКартинок.ЛоготипOzon2);
	Иначе     
		Если ЭтоFBO Тогда
			ОбновитьНаСервереFBO(); 
		Иначе  
...
Процедура УстановитьВидимостьДоступность()
	...
	Элементы.ОткрытьРасписаниеПолучениеЭтикеток.Видимость = 
		ЗначениеЗаполнено(УчетнаяЗапись)
			И Не ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоЯндексМаркет(ВидМаркетплейса);
...
Процедура ОбновитьНаСервереFBS()
	...
	Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	РаспоряженияРасходногоОрдера.Распоряжение КАК Распоряжение
		|ПОМЕСТИТЬ ВТРаспоряженияРасходногоОрдера
		|ИЗ
		|	Документ.РасходныйОрдерНаТовары.ТоварыПоРаспоряжениям КАК РаспоряженияРасходногоОрдера
		|ГДЕ
		|	РаспоряженияРасходногоОрдера.Ссылка В(&Объекты)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	Заказы.Ссылка В(&Объекты)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	РаспоряженияРасходногоОрдера.Распоряжение
		|ИЗ
		|	ВТРаспоряженияРасходногоОрдера КАК РаспоряженияРасходногоОрдера
		|ГДЕ
		|	РаспоряженияРасходногоОрдера.Распоряжение ССЫЛКА Документ.ЗаказКлиента
		|	И РаспоряженияРасходногоОрдера.Распоряжение <> &ПустойЗаказКлиента
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЗаказКлиента
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДокументыОтгрузки.ЗаказКлиента КАК ЗаказКлиента,
		|	ДокументыОтгрузки.Ссылка КАК ДокументОтгрузки
		|ПОМЕСТИТЬ ВТДокументыОтгрузки
		|ИЗ
		|	Документ.РеализацияТоваровУслуг.Товары КАК ДокументыОтгрузки
		|ГДЕ
		|	ДокументыОтгрузки.Ссылка В(&Объекты)
		|	И ДокументыОтгрузки.ЗаказКлиента ССЫЛКА Документ.ЗаказКлиента
		|	И ДокументыОтгрузки.ЗаказКлиента <> &ПустойЗаказКлиента
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДокументыОтгрузки.ЗаказКлиента,
		...
		|				ВТРаспоряженияРасходногоОрдера КАК РаспоряженияРасходногоОрдера
		|			ГДЕ
		|				ДокументыОтгрузки.Ссылка = РаспоряженияРасходногоОрдера.Распоряжение
		|				И РаспоряженияРасходногоОрдера.Распоряжение ССЫЛКА Документ.РеализацияТоваровУслуг)
		|	И ДокументыОтгрузки.ЗаказКлиента ССЫЛКА Документ.ЗаказКлиента
		|	И ДокументыОтгрузки.ЗаказКлиента <> &ПустойЗаказКлиента
		|
		|ОБЪЕДИНИТЬ ВСЕ
		...
		|	ДокументыОтгрузки.Ссылка
		|ИЗ
		|	Документ.ПередачаТоваровХранителю.Товары КАК ДокументыОтгрузки
		|ГДЕ
		|	ДокументыОтгрузки.Ссылка В(&Объекты)
		|	И ДокументыОтгрузки.ЗаказКлиента ССЫЛКА Документ.ЗаказКлиента
		|	И ДокументыОтгрузки.ЗаказКлиента <> &ПустойЗаказКлиента
		...
		|			ГДЕ
		|				ДокументыОтгрузки.Ссылка = РаспоряженияРасходногоОрдера.Распоряжение
		|				И РаспоряженияРасходногоОрдера.Распоряжение ССЫЛКА Документ.ПередачаТоваровХранителю)
		|	И ДокументыОтгрузки.ЗаказКлиента ССЫЛКА Документ.ЗаказКлиента
		|	И ДокументыОтгрузки.ЗаказКлиента <> &ПустойЗаказКлиента
		|
		|ИНДЕКСИРОВАТЬ ПО
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВЫРАЗИТЬ(ДокументыОтгрузки.ЗаказКлиента КАК Документ.ЗаказКлиента) КАК ЗаказКлиента
		|ДОБАВИТЬ ВТЗаказы
		|ИЗ
		|	ВТДокументыОтгрузки КАК ДокументыОтгрузки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ПОМЕСТИТЬ ВТОбъектыПечатиПредварительно
		|ИЗ
		...
		|				ВТЗаказы КАК Заказы
		|			ГДЕ
		|				ЗаказыТорговыхПлощадок.Заказ = Заказы.ЗаказКлиента)
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ
		...
		|	ЗаказыТорговыхПлощадок.УчетнаяЗапись.ВидМаркетплейса,
		...
		|				ВТДокументыОтгрузки КАК ДокументыОтгрузки
		|			ГДЕ
		|				ЗаказыТорговыхПлощадок.Заказ = ДокументыОтгрузки.ЗаказКлиента
		|				И ЗаказыТорговыхПлощадок.ДокументОтгрузки = ДокументыОтгрузки.ДокументОтгрузки)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Заказ,
		|	НомерОтправления
		...
		|	Заказы.Отправление КАК Отправление,
		...
		|ИЗ
		|	ВТОбъектыПечатиПредварительно КАК Заказы
		|ГДЕ
		|	НЕ Заказы.ВидМаркетплейса В (&МаркетплейсыБезДокументаОтгрузки)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	ЕСТЬNULL(Сведения.ДокументОтгрузки, Заказы.Отправление),
		|	Заказы.НомерЗаказа,
		|	ЕСТЬNULL(Сведения.НомерОтправления, Заказы.НомерОтправления),
		|	Заказы.СтатусЗаказа
		|ИЗ
		|	ВТОбъектыПечатиПредварительно КАК Заказы
		...
		|			И Заказы.НомерОтправления = Сведения.НомерРодительскогоОтправления
		|ГДЕ
		|	Заказы.ВидМаркетплейса В(&МаркетплейсыБезДокументаОтгрузки)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Отправление
		|;
		...
		|	ЕСТЬNULL(ПрисоединенныеФайлы.Ссылка, НЕОПРЕДЕЛЕНО) КАК Ссылка,
		|	ЕСТЬNULL(ПрисоединенныеФайлы.Наименование, """") КАК Наименование,
		|	ЕСТЬNULL(ВЫРАЗИТЬ(ПрисоединенныеФайлы.ПутьКФайлу КАК СТРОКА(1024)), """") КАК ПутьКФайлу,
		|	ЕСТЬNULL(ПрисоединенныеФайлы.Расширение, """") КАК Расширение
		|ИЗ
		|	ВТ_ОбъектыПечати КАК ОбъектыПечати
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЗаказКлиентаПрисоединенныеФайлы КАК ПрисоединенныеФайлы
		|		ПО ОбъектыПечати.Отправление = ПрисоединенныеФайлы.ВладелецФайла
		|			И (НЕ ПрисоединенныеФайлы.ПометкаУдаления)
		|ГДЕ
		|	ОбъектыПечати.Отправление = НЕОПРЕДЕЛЕНО
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ОбъектыПечати.Заказ,
		|	ОбъектыПечати.УчетнаяЗапись,
		|	ОбъектыПечати.Отправление,
		|	ОбъектыПечати.НомерЗаказа,
		|	ОбъектыПечати.НомерОтправления,
		|	ОбъектыПечати.СтатусЗаказа,
		|	ЕСТЬNULL(ПрисоединенныеФайлы.Ссылка, НЕОПРЕДЕЛЕНО),
		|	ЕСТЬNULL(ПрисоединенныеФайлы.Наименование, """"),
		|	ЕСТЬNULL(ВЫРАЗИТЬ(ПрисоединенныеФайлы.ПутьКФайлу КАК СТРОКА(1024)), """"),
		|	ЕСТЬNULL(ПрисоединенныеФайлы.Расширение, """")
		|ИЗ
		|	ВТ_ОбъектыПечати КАК ОбъектыПечати
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПередачаТоваровХранителюПрисоединенныеФайлы КАК ПрисоединенныеФайлы
		|		ПО ОбъектыПечати.Отправление = ПрисоединенныеФайлы.ВладелецФайла
		|			И (НЕ ПрисоединенныеФайлы.ПометкаУдаления)
		|ГДЕ
		|	ОбъектыПечати.Отправление ССЫЛКА Документ.ПередачаТоваровХранителю
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|ИЗ
		|	ВТ_ОбъектыПечати КАК ОбъектыПечати
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.РеализацияТоваровУслугПрисоединенныеФайлы КАК ПрисоединенныеФайлы
		|		ПО ОбъектыПечати.Отправление = ПрисоединенныеФайлы.ВладелецФайла
		|			И (НЕ ПрисоединенныеФайлы.ПометкаУдаления)
		|ГДЕ
		|	ОбъектыПечати.Отправление ССЫЛКА Документ.РеализацияТоваровУслуг";
	
	Запрос.УстановитьПараметр("Объекты", Объекты);
	Запрос.УстановитьПараметр("ПустойЗаказКлиента", Документы.ЗаказКлиента.ПустаяСсылка());
...
&НаКлиенте
Процедура ПоказатьПечатныеФормы(ПечатьСразуНаПринтер = Ложь)
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("ПечатныеДокументы",         ПолучитьПакетДокументов());
	ПараметрыОткрытия.Вставить("СортироватьПоНаименованию", Истина);
	...
	Для Каждого СтрокаТабличнойЧасти Из Этикетки Цикл
		Если СтрокаТабличнойЧасти.Пометка Тогда
			...
			Если ВРег(ДанныеФайла.Расширение) = ВРег("zip") Тогда
				...
				Для Каждого Файл Из ФайлыPDF Цикл
					Попытка
						...
						ИнформацияОФайле.Вставить("АдресДвоичныхДанных", ПоместитьВоВременноеХранилище(СодержимоеФайла, УникальныйИдентификатор));
						ИнформацияОФайле.Вставить("РасширениеФайла",     "pdf");
						
						Результат.Вставить(СтрокаТабличнойЧасти.НомерПоставки + " (" + Формат(Индекс, "ЧН=; ЧГ=0") + ")", ИнформацияОФайле);
			...
			ИначеЕсли ВРег(ДанныеФайла.Расширение) = ВРег("pdf") Тогда
				ИнформацияОФайле = Новый Структура;
				ИнформацияОФайле.Вставить("АдресДвоичныхДанных", ДанныеФайла.СсылкаНаДвоичныеДанныеФайла);
				ИнформацияОФайле.Вставить("РасширениеФайла",     "pdf");
				
				Результат.Вставить(СтрокаТабличнойЧасти.НомерПоставки, ИнформацияОФайле);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьЭтикеткиНаСервере()
	...
	Для Каждого Строка Из ТаблицаУчетныхЗаписей Цикл
		...
		НомераОтправлений = ПолучитьНомераОтправленийДляПечатиЭтикеток(Этикетки, ПараметрыОтбора);
		...
		ИначеЕсли ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоЯндексМаркет(ВидМаркетплейса) Тогда
			Ошибка = ИнтеграцияСЯндексМаркетСервер.ПолучитьЭтикеткиОтправлений(
				Строка.УчетнаяЗапись,
				НомераОтправлений,
				Истина); 		
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПолучитьНомераОтправленийДляПечатиЭтикеток(Этикетки, ПараметрыОтбора)
	
	НомераОтправлений = Новый Массив;
	
	СтатусыПечатиЭтикетки = РегистрыСведений.ЗаказыТорговыхПлощадок.СтатусыПечатиЭтикеток();
	
	Для Каждого СтатусОтправлений Из СтатусыПечатиЭтикетки Цикл
		ПараметрыОтбора.Вставить("СтатусЗаказа", СтатусОтправлений);
		СтрокиТаблицы = Этикетки.НайтиСтроки(ПараметрыОтбора);
...
&НаСервере
Процедура ЗаполнитьНавигационныеСсылки()
	
	Если ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоOzon(ВидМаркетплейса) Тогда
		НавигационнаяСсылкаFBO = ИнтеграцияСМаркетплейсомOzonСервер.АдресЗаказовFBO();
		НавигационнаяСсылкаFBS = ИнтеграцияСМаркетплейсомOzonСервер.АдресЗаказовFBS();
		
	ИначеЕсли ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоЯндексМаркет(ВидМаркетплейса) Тогда
		ИдентификаторКабинета = "";
		НавигационнаяСсылкаFBO = ИнтеграцияСЯндексМаркетСервер.АдресЗаказовFBO(УчетнаяЗапись, ИдентификаторКабинета);
		НавигационнаяСсылкаFBS = ИнтеграцияСЯндексМаркетСервер.АдресЗаказовFBS(УчетнаяЗапись, ИдентификаторКабинета);
	КонецЕсли;
