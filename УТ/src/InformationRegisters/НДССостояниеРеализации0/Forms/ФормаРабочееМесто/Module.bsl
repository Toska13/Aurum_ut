...
Процедура ПодтверждениеСтавкиНДССервер(ПараметрыПодтверждения)
	...
	Попытка
		...
		Если ПерезаписатьДокумент Тогда
			...
			Если ТипЗнч(ДокументРеализации) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда 
				ДокументыРеализацииСКорректировками.Удалить(ДокументыРеализацииСКорректировками.Найти(ДокументРеализации));
				ДокументыРеализацииСКорректировками.Добавить(Регистратор);
				ОбщегоНазначенияУТ.УдалитьПовторяющиесяЭлементыМассива(ДокументыРеализацииСКорректировками);
			КонецЕсли;
...
Процедура ВыписатьСчетФактуруНаНеподтвержденныеСтавки(СчетФактура, ДокументОснование, Организация, Дата)
	...
	КонецЕсли;
	ДанныеЗаполнения.Вставить("НачислениеНДСПриОтгрузке", ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту"));
