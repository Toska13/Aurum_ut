...
//
// Возвращаемое значение:
//  Неопределено, Структура - Неопределено, если данные по заказу не найдены, либо структура с параметрами:
//   * НастройкаПодключения - СправочникСсылка.НастройкиПодключенияКОнлайнЗаказам - настройка, с помощью которой
//       было выполнено создание заказа.
//   * СтатусЗаказа - Строка - сохраненный статус заказа.
//   * ИдентификаторЗаказа - Строка - идентификатор заказа.
//   * URLЗаказа - Строка - ссылка на страницу заказа.
//   * СуммаОплаты - Число - сумма оплаты.
//   * ДокументЗаказа - ОпределяемыйТип.ДокументОнлайнЗаказаБИП - документ отражающий операцию в системе.
//
...
//    * URLЗаказа - Строка - URL онлайн-заказа;
//    * СтатусЗаказа - Строка - статус онлайн-заказа;
//    * СпособОплаты - Строка - способ оплаты;
//    * СуммаОплаты - Число - сумма оплаты;
//
...
//
// Возвращаемое значение:
//  Неопределено, Структура - Неопределено, если данные по заказу не найдены, либо структура с данными:
//   * ИдентификаторЗаказа - Строка - идентификатор заказа.
//   * НастройкаПодключения - СправочникСсылка.НастройкиПодключенияКОнлайнЗаказам - настройка, с помощью которой
//       было выполнено создание заказа.
//   * СтатусЗаказа - Строка - сохраненный статус заказа.
...
Процедура ЗаписатьДанныеЗаказа(
	...
	Попытка
		...
		Если ДанныеЗаказа.ПараметрыФискализации = Неопределено Тогда
			Запись.ДанныеЧека = Неопределено;
		Иначе
			Запись.ДанныеЧека = Новый ХранилищеЗначения(ДанныеЗаказа.ПараметрыФискализации.ДанныеЧека);
		КонецЕсли;
...
//  ДанныеОплаты - Структура - содержит данные оплаты:
//    * СпособОплаты - ПеречислениеСсылка.СпособыОплатыОнлайнЗаказов - способ оплаты заказа;
//    * СуммаОплаты - Число - сумма оплаты по заказу;
//  ДанныеФискализации - Структура - содержит данные Фискализации:
//    * ПараметрыФискализации - Структура - параметры операции фискализации в сервисе;
//  ЭтоУстановкаТерминальногоСтатуса - Булево - признак того что устанавливаемый статус является терминальным.
//
Процедура ЗаписатьСтатусЗаказа(
		...
		ДанныеОплаты,
		ДанныеФискализации,
		ЭтоУстановкаТерминальногоСтатуса) Экспорт
	...
	Попытка
		...
		Если ЭтоУстановкаТерминальногоСтатуса
			Или Запись.КоличествоПопыток > МаксимальноеКоличествоПопытокЗапросаСтатуса() Тогда
			Запись.ОтложенноеПолучениеСтатуса = Ложь;
			Запись.ПараметрыФискализации = Неопределено;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ДанныеОплаты)
			И ЗначениеЗаполнено(ДанныеОплаты.СпособОплаты) Тогда
			Запись.СпособОплаты = ДанныеОплаты.СпособОплаты;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ДанныеОплаты)
			И ЗначениеЗаполнено(ДанныеОплаты.СпособОплаты)
			И ЗначениеЗаполнено(ДанныеОплаты.СуммаОплаты) Тогда
			Запись.СуммаОплаты = ДанныеОплаты.СуммаОплаты;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ДанныеФискализации) Тогда
			Если ЗначениеЗаполнено(ДанныеФискализации.ПараметрыФискализации) Тогда
				Запись.ПараметрыФискализации = Новый ХранилищеЗначения(ДанныеФискализации.ПараметрыФискализации);
			Иначе
				Запись.ПараметрыФискализации = Неопределено;
			КонецЕсли;
		КонецЕсли;
...
// Выполняет установку признака отложенного получения URL Share
//
// Параметры:
//  ДокументЗаказа - ОпределяемыйТип.ДокументОнлайнЗаказаБИП - документ основание заказа;
//  Значение - Булево - признак отложенного получения ссылки в сервисе Share.
//
Процедура УстановитьОтложенноеПолучениеURLShare(
		ДокументЗаказа,
		Значение) Экспорт
	...
	Попытка
		...
		Запись.ОтложенноеПолучениеURLShare = Значение;
...
// Возвращает данные чека отправленные в сервис онлайн-заказов по документу операции
//
// Параметры:
//  ДокументЗаказа - ОпределяемыйТип.ДокументОнлайнЗаказаБИП - документ основание заказа.
//
// Возвращаемое значение:
//   Структура - данные чека переданные в сервис онлайн-заказов.
//
Функция ДанныеЧека(ДокументЗаказа) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ИдентификаторыОнлайнЗаказов.ДанныеЧека КАК ДанныеЧека
	|ИЗ
	|	РегистрСведений.ИдентификаторыОнлайнЗаказов КАК ИдентификаторыОнлайнЗаказов
	|ГДЕ
	|	ИдентификаторыОнлайнЗаказов.ДокументЗаказа = &ДокументЗаказа";
	...
	Если Выборка.Следующий()
		И Выборка.ДанныеЧека <> Неопределено Тогда
		Возврат Выборка.ДанныеЧека.Получить();
	КонецЕсли;
...
// Возвращает признак необходимости актуализации URLShare по документу операции
//
// Параметры:
//  ДокументЗаказа - ОпределяемыйТип.ДокументОнлайнЗаказаБИП - документ основание заказа.
//
// Возвращаемое значение:
//   Булево - признак необходимости актуализации URLShare по документу операции.
//
Функция НеобходимаАктуализацияURLShare(ДокументЗаказа) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ИдентификаторыОнлайнЗаказов.ОтложенноеПолучениеURLShare КАК ОтложенноеПолучениеURLShare
	|ИЗ
	|	РегистрСведений.ИдентификаторыОнлайнЗаказов КАК ИдентификаторыОнлайнЗаказов
	|ГДЕ
	|	ИдентификаторыОнлайнЗаказов.ДокументЗаказа = &ДокументЗаказа";
	...
	Если Выборка.Следующий() Тогда
		Возврат Выборка.ОтложенноеПолучениеURLShare;
	КонецЕсли;
...
Функция ОтложенныеЗаказы() Экспорт
	...
	|	ИдентификаторыОнлайнЗаказов.СтатусЗаказа КАК СтатусЗаказа,
	|	ИдентификаторыОнлайнЗаказов.СпособОплаты КАК СпособОплаты,
	|	ИдентификаторыОнлайнЗаказов.ПараметрыФискализации КАК ПараметрыФискализации
	|ИЗ
	|	РегистрСведений.ИдентификаторыОнлайнЗаказов КАК ИдентификаторыОнлайнЗаказов
	|ГДЕ
	...
	Пока ВыборкаПоНастройкеПодключения.Следующий() Цикл
		...
		Пока ВыборкаПоСпособуОплаты.Следующий() Цикл
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				ДанныеОперации.Вставить("ПараметрыФискализации", ВыборкаДетальныеЗаписи.ПараметрыФискализации.Получить());
...
// Дополнить данные заказов данными оплат.
// 
// Параметры:
//  ДанныеЗаказовПоНастройкам - Соответствие из КлючИЗначение - данные операций:
//    *Ключ - СправочникСсылка.НастройкиПодключенияКОнлайнЗаказам - настройка создания заказа.
//    *Значение - Массив из Структура - данные заказов с отложенным получением статуса.
//  ДокументыПоСпособуОплаты - Соответствие из КлючИЗначение - Документы по способу оплаты:
//    *Ключ - ПеречислениеСсылка.СпособыОплатыОнлайнЗаказов - способ оплаты
//    *Значение - Массив из ОпределяемыйТип.ДокументОнлайнЗаказаБИП - ссылки на документы.
//
	...
	Для Каждого СпособОплаты Из ДокументыПоСпособуОплаты Цикл
		...
		Для Каждого ДанныеЗаказаПоНастройке Из ДанныеЗаказовПоНастройкам Цикл
			Для Каждого ДанныеЗаказа Из ДанныеЗаказаПоНастройке.Значение Цикл
				...
				ИначеЕсли СпособОплаты.Ключ = Перечисления.СпособыОплатыОнлайнЗаказов.Эквайринг Тогда
					ДанныеЗаказа.ПараметрыОплаты.Вставить("ИнтернетЭквайринг", ДанныеОпераций.Получить(ДанныеЗаказа.ДокументЗаказа));
				ИначеЕсли СпособОплаты.Ключ = Перечисления.СпособыОплатыОнлайнЗаказов.ОзонПэй Тогда
					ДанныеЗаказа.ПараметрыОплаты.Вставить("ОзонПэй", ДанныеОпераций.Получить(ДанныеЗаказа.ДокументЗаказа));
				ИначеЕсли СпособОплаты.Ключ = Перечисления.СпособыОплатыОнлайнЗаказов.ЯндексПэй Тогда
					ДанныеЗаказа.ПараметрыОплаты.Вставить("ЯндексПэй", ДанныеОпераций.Получить(ДанныеЗаказа.ДокументЗаказа));
				КонецЕсли;
