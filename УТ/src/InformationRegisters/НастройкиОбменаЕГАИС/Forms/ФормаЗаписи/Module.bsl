...
Процедура ПриОткрытии(Отказ)
	...
	Если Не Записано
		...
		Если УпрощеннаяИнтеграцияБЭД Тогда
			ТекущийЭлемент = Элементы.АдресУТМ;
		Иначе
...
Процедура СмТакжеНастройкиИСервисОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	СтруктураБыстрогоОтбора.Вставить(ИмяПоляОрганизация, ОрганизацияЕГАИС);
	
	ПараметрыФормы.Вставить("СтруктураБыстрогоОтбора", СтруктураБыстрогоОтбора);
	
	ОткрытьФорму("Обработка.ВходящиеСообщенияУТМЕГАИС.Форма.Форма", ПараметрыФормы, ЭтотОбъект);
...
Процедура ЗаполнитьДанные()
	...
	Если ДанныеСопоставления <> Неопределено Тогда
		...
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			ТекстСообщенияXML = ПротоколОбменаИС.ТекстСообщенияXMLИзПротокола(Выборка.Ссылка, Ложь);
...
Процедура ОбновитьТекстСопоставления(Форма)
	...
	Если ЗначениеЗаполнено(Форма.ОрганизацияЕГАИС) Тогда
		...
		Если Не Форма.УпрощеннаяИнтеграцияБЭД Тогда
...
Процедура НачатьФормированиеЗапросаНаЗагрузкуОрганизации(ОповещениеПриЗакрытии = Неопределено)
	...
	ПараметрыФормы.Вставить("ЗапросДанныхПриОткрытии", Истина);
...
Асинх Процедура ПолучениеКодаФСРАР_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.КодФСРАР <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(Запись.ИдентификаторФСРАР) Тогда
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	Иначе
		...
		Если Не Записано И РасписаниеОбмена = Неопределено Тогда
			
			РасписаниеОбмена = Новый РасписаниеРегламентногоЗадания;
			РасписаниеОбмена.ПериодПовтораВТечениеДня = 900;
			РасписаниеОбмена.ДатаКонца = НачалоДня(ТекущаяДатаСеанса()) + 60*60*24*2;
			ОбменПоРасписанию = Истина;
...
Асинх Процедура ПослеПроверкиПодключенияКУТМ(РезультатПроверки, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(КодФСРАРИзУТМ) Тогда
		...
		ПодключениеНастроеноКорректно = Истина;
		Если Не ЗначениеЗаполнено(Запись.ИдентификаторФСРАР) Тогда
			
			Запись.ИдентификаторФСРАР = КодФСРАРИзУТМ;
			Модифицированность = Истина;
			ЗаполнитьДанные();
			УказанПравильныйКодФСРАР = Истина;
			Элементы.СтраницыПроверкаПодключения.ТекущаяСтраница = Элементы.СтраницыПроверкаПодключения.ПодчиненныеЭлементы.ПроверкаПодключенияКорректно;
			
			Если ЗаписатьНастройкуОбмена() Тогда
				...
				Если ТребуетсяЗапроситьДанныеОрганизацииЕГАИС
					И ПравоДобавленияКлассификатораОрганизаций Тогда
					НачатьФормированиеЗапросаНаЗагрузкуОрганизации();
				КонецЕсли;
				Возврат;
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ОткрытьWebИнтерфейсУТМ" Тогда
		...
		Если Запись.ИспользоватьЗащищенноеСоединение Тогда
			ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку(
				СтрШаблон("https://%1:%2/", Запись.АдресУТМ, Формат(Запись.ПортУТМ, "ЧГ=0")),
				Новый ОписаниеОповещения("ПослеОткрытияWebИнтерфейсаУТМ", ЭтотОбъект));
		Иначе
			ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку(
				СтрШаблон("http://%1:%2/", Запись.АдресУТМ, Формат(Запись.ПортУТМ, "ЧГ=0")),
				Новый ОписаниеОповещения("ПослеОткрытияWebИнтерфейсаУТМ", ЭтотОбъект));
		КонецЕсли;
...
Функция СформироватьГиперссылкуСмТакже()
	...
	Если ОбщегоНазначенияЕГАИС.ЭтоРасширеннаяВерсияГосИС()
		...
		ТекстГиперссылки = НСтр("ru = 'Просмотр входящих сообщений в УТМ'");
			МассивСтрок.Добавить(Новый ФорматированнаяСтрока(ТекстГиперссылки,,,,
				"ОткрытьВходящиеСообщенияУТМЕГАИС"));
...
&НаКлиенте
Процедура СброситьПроверкуПодключения()
	ПодключениеНастроеноКорректно = Ложь;
	УказанПравильныйКодФСРАР = Ложь;
	ОбновитьТекстСопоставления(ЭтотОбъект);
	Элементы.СтраницыПроверкаПодключения.ТекущаяСтраница = Элементы.СтраницыПроверкаПодключения.ПодчиненныеЭлементы.ВыполняетсяПроверкаПодключенияКУТМ;
