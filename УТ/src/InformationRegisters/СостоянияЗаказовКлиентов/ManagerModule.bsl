...
//
// Параметры:
// 	МассивСсылок - Массив Из ДокументСсылка - массив заказов, таблицы состояний которых требуется получить
//
// Возвращаемое значение:
// 	Структура - структура состояний содержащая значения:
// 		* ТаблицаСостоянийЗаказов - ТаблицаЗначений.
// 		* ТаблицаПредыдущихСостоянийЗаказов - ТаблицаЗначений.
//
Функция ТаблицыСостоянийЗаказов(МассивСсылок) Экспорт
	...
	Результат.Вставить("ТаблицаСостоянийЗаказов", ТаблицаСостоянийЗаказов);
	Результат.Вставить("ТаблицаПредыдущихСостоянийЗаказов", ТаблицаПредыдущихСостоянийЗаказов);
...
Процедура ПриОпределенииДоступныхВидовОповещенийПользователей(ОписаниеВидовОповещений) Экспорт
	...
	НовыйВидОповещений.Идентификатор = "97303299-a061-4401-af2c-cf94765427e7";
	НовыйВидОповещений.Наименование = НСтр("ru = 'Ожидающие согласования заказы (Продажи)'", ОбщегоНазначения.КодОсновногоЯзыка());
	НовыйВидОповещений.ТипОповещения = Перечисления.ТипыОповещенийПользователей.Неоперативное;
	НовыйВидОповещений.ИмяШаблонаСКД = Метаданные.РегистрыСведений.СостоянияЗаказовКлиентов.Макеты.Оповещения_ОжидаетсяСогласование.ПолноеИмя();
	...
	НовыйВидОповещений = ОповещенияПользователейОСобытиях.ДобавитьОписаниеВидаОповещенийПользователей(ОписаниеВидовОповещений);
	НовыйВидОповещений.ГруппаВидаОповещений = ГруппаВидаОповещенийПродажи;
	НовыйВидОповещений.Идентификатор = "857b24b4-8f05-4829-8cf4-9cc879494dea";
	НовыйВидОповещений.Наименование = НСтр("ru = 'Готовые к закрытию заказы (Продажи)'", ОбщегоНазначения.КодОсновногоЯзыка());
	НовыйВидОповещений.ТипОповещения = Перечисления.ТипыОповещенийПользователей.Неоперативное;
	НовыйВидОповещений.ИмяШаблонаСКД = Метаданные.РегистрыСведений.СостоянияЗаказовКлиентов.Макеты.Оповещения_ГотовКЗакрытию.ПолноеИмя();
...
// Возвращает параметры актуализации состояния заказов.
//
// Возвращаемое значение:
// 	Структура:
// 		* ПравилоОтбораЗаписей - Строка.
// 		* ФункцияСравненияЗаписей - Строка.
// 		* ФункцияСравненияЗаписейВоВременнуюТаблицу - Строка.
//
Функция ПараметрыАктуализацииСостоянийЗаказов() Экспорт
	
	Результат = Новый Структура;
	
	Результат.Вставить("ПравилоОтбораЗаписей",
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Результат.Вставить("ФункцияСравненияЗаписейВоВременнуюТаблицу",
		"ВЫБРАТЬ
	...
	Результат.Вставить("ФункцияСравненияЗаписей",
		СтрЗаменить(Результат.ФункцияСравненияЗаписейВоВременнуюТаблицу,
			"ПОМЕСТИТЬ ИзменениеПереопределяемый", ""));
...
//
//	Параметры:
//		МассивСсылок - Массив Из ДокументСсылка - ссылки на документы, по которым
...
Функция ТекстЗапросаЧастичноВНаличии()
	...
	ТекстЗапроса =
		...
		|						ЗНАЧЕНИЕ(Перечисление.СостоянияЗапаса.ОжидаетсяКПоступлениюСБлокировкой),
		|						ЗНАЧЕНИЕ(Перечисление.СостоянияЗапаса.ВНаличииОжидаетсяРазблокировка))
		|					И Сведения.ХарактеристикаПотребности В(
		|						ЗНАЧЕНИЕ(Перечисление.ХарактеристикиПотребности.РезервироватьПоМереПоступления),
		|						ЗНАЧЕНИЕ(Перечисление.ХарактеристикиПотребности.ОтложитьРезервирование),
		|						ЗНАЧЕНИЕ(Перечисление.ХарактеристикиПотребности.КОбеспечению)))
...
Функция ТекстЗапросаДопустимыхОтклонения(СоотвествиеТипов)
	...
	Если СоотвествиеТипов["Документ.ЗаказКлиента"] <> Неопределено
		Или СоотвествиеТипов["Документ.ЗаявкаНаВозвратТоваровОтКлиента"] <> Неопределено
		Тогда
		...
		|ИЗ
		|	РегистрНакопления.РаспоряженияНаОтгрузкуИВозврат.Обороты(,,, Распоряжение В (&МассивЗаказов)
		|	И ВЫБОР
		|		КОГДА Показатель В
		...
		|
		|ВЫБРАТЬ
		|	Подзапрос.Распоряжение КАК ЗаказКлиента,
		...
		|	СУММА(Подзапрос.КОтгрузкеКоличествоОборот - Подзапрос.ОтгруженоКоличествоОборот) КАК КОтгрузкеКонечныйОстаток,
		|	СУММА(Подзапрос.КОтгрузкеКоличествоОборот) КАК КОтгрузкеПриход,
		|	СУММА(Подзапрос.КОформлениюКоличествоОборот - Подзапрос.ОформленоКоличествоОборот) КАК КОформлениюКонечныйОстаток,
		|	СУММА(Подзапрос.КОформлениюКоличествоОборот) КАК КОформлениюПриход,
		|	СУММА(ВЫБОР
		|			КОГДА Подзапрос.КодСтроки = 0
		|				ТОГДА ВЫБОР
		|					КОГДА &ТекстУсловияДокументПродажи
		|						ТОГДА Подзапрос.КОтгрузкеКоличествоОборот
		|					ИНАЧЕ Подзапрос.КОформлениюКоличествоОборот
		|				КОНЕЦ
		|			ИНАЧЕ Подзапрос.ЗаказаноКоличествоОборот
		|		КОНЕЦ
		|		- ВЫБОР
		|			КОГДА &ТекстУсловияДокументПродажи
		|				ТОГДА (Подзапрос.ОтгруженоКоличествоОборот
		|				- Подзапрос.СписаноДоППСКоличествоОборот
		|				- Подзапрос.ПринятоКоличествоОборот)
		|			ИНАЧЕ Подзапрос.ОформленоКоличествоОборот
		|		КОНЕЦ) КАК ЗаказаноКонечныйОстаток,
		|	СУММА(ВЫБОР
		|			КОГДА Подзапрос.КодСтроки = 0
		|				ТОГДА ВЫБОР
		|					КОГДА &ТекстУсловияДокументПродажи
		|						ТОГДА Подзапрос.КОтгрузкеСуммаОборот
		|					ИНАЧЕ Подзапрос.КОформлениюСуммаОборот
		|				КОНЕЦ
		|			ИНАЧЕ Подзапрос.ЗаказаноСуммаОборот
		|		КОНЕЦ
		|		- ВЫБОР
		|			КОГДА &ТекстУсловияДокументПродажи
		|				ТОГДА (Подзапрос.ОтгруженоСуммаОборот
		|				- Подзапрос.СписаноДоППССуммаОборот
		|				- Подзапрос.ПринятоСуммаОборот)
		|			ИНАЧЕ Подзапрос.ОформленоСуммаОборот
		|		КОНЕЦ) КАК СуммаКонечныйОстаток,
		|	СУММА(ВЫБОР
		|			КОГДА Подзапрос.КодСтроки = 0
		|				ТОГДА Подзапрос.КОформлениюКоличествоОборот
		|			ИНАЧЕ Подзапрос.ЗаказаноСуммаОборот
		|		КОНЕЦ) КАК СуммаПриход,
		|	СУММА(Подзапрос.ОформленоСуммаОборот) КАК СуммаРасход
		...
		|		ВЫБОР
		|			КОГДА ОстаткиИОбороты.Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		|				ТОГДА ЕСТЬNULL(ТаблицаСкладыСерии.Склад, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка))
		|			ИНАЧЕ ОстаткиИОбороты.Склад
		|		КОНЕЦ КАК Склад,
		|		ВЫБОР
		|			КОГДА ОстаткиИОбороты.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|				ТОГДА ЕСТЬNULL(ТаблицаСкладыСерии.Серия, ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка))
		|			ИНАЧЕ ОстаткиИОбороты.Серия
		|		КОНЕЦ КАК Серия,
		|		ОстаткиИОбороты.ЗаказаноКоличествоОборот,
		|		ОстаткиИОбороты.КОформлениюКоличествоОборот,
		|		ОстаткиИОбороты.ОформленоКоличествоОборот,
		|		ОстаткиИОбороты.КОтгрузкеКоличествоОборот,
		|		ОстаткиИОбороты.ОтгруженоКоличествоОборот,
		|		ОстаткиИОбороты.СписаноДоППСКоличествоОборот,
		|		ОстаткиИОбороты.ПринятоКоличествоОборот,
		|		ОстаткиИОбороты.КОтгрузкеСуммаОборот,
		|		ОстаткиИОбороты.ОтгруженоСуммаОборот,
		|		ОстаткиИОбороты.ЗаказаноСуммаОборот,
		|		ОстаткиИОбороты.КОформлениюСуммаОборот,
		|		ОстаткиИОбороты.ОформленоСуммаОборот,
		|		ОстаткиИОбороты.СписаноДоППССуммаОборот,
		|		ОстаткиИОбороты.ПринятоСуммаОборот
		|	ИЗ
		|		#РаспоряженияНаОтгрузкуОборотыРазвернутая КАК ОстаткиИОбороты
		|			ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаСкладыСерии КАК ТаблицаСкладыСерии
		|			ПО ОстаткиИОбороты.Распоряжение = ТаблицаСкладыСерии.Распоряжение
		|			И ОстаткиИОбороты.КодСтроки = ТаблицаСкладыСерии.КодСтроки
		|			И ОстаткиИОбороты.Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)) КАК Подзапрос
		|СГРУППИРОВАТЬ ПО
		...
		|	Подзапрос.Серия
		|ИМЕЮЩИЕ
		|	СУММА(Подзапрос.КОтгрузкеКоличествоОборот - Подзапрос.ОтгруженоКоличествоОборот) <> 0
		|	ИЛИ СУММА(Подзапрос.КОтгрузкеКоличествоОборот) <> 0
		|	ИЛИ СУММА(Подзапрос.КОформлениюКоличествоОборот - Подзапрос.ОформленоКоличествоОборот) <> 0
		|	ИЛИ СУММА(Подзапрос.КОформлениюКоличествоОборот) <> 0
		|	ИЛИ СУММА(Подзапрос.ЗаказаноКоличествоОборот - Подзапрос.ОтгруженоКоличествоОборот) <> 0
		|	ИЛИ СУММА(Подзапрос.ЗаказаноСуммаОборот - Подзапрос.ОтгруженоСуммаОборот) <> 0
		|	ИЛИ СУММА(Подзапрос.ЗаказаноСуммаОборот) <> 0
		|	ИЛИ СУММА(Подзапрос.ОформленоСуммаОборот) <> 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	СУММА(ЗаказыОстатки.КОтгрузкеКонечныйОстаток) КАК КОтгрузкеКонечныйОстаток,
		...
		|				ТОГДА ВЫБОР
		|					КОГДА ЗаказыКлиентовОстатки.КОформлениюКонечныйОстаток <= ЗаказыКлиентовОстатки.КОформлениюПриход *
		|						(&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100)
		|					И ЗаказыКлиентовОстатки.КОформлениюКонечныйОстаток >= -(ЗаказыКлиентовОстатки.КОформлениюПриход *
		|						(&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100))
		|						ТОГДА 0
		...
		|			КОГДА ЗаказыКлиентовОстатки.Номенклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины В (&МерныеТипыВеличин)
		|				ТОГДА ВЫБОР
		|					КОГДА ЗаказыКлиентовОстатки.КОтгрузкеКонечныйОстаток <= ЗаказыКлиентовОстатки.КОтгрузкеПриход *
		|						(&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100)
		|					И ЗаказыКлиентовОстатки.КОтгрузкеКонечныйОстаток >= -(ЗаказыКлиентовОстатки.КОтгрузкеПриход *
		|						(&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100))
		|						ТОГДА 0
		|					ИНАЧЕ ЕСТЬNULL(ЗаказыКлиентовОстатки.КОтгрузкеКонечныйОстаток, 0)
		|				КОНЕЦ
		|			ИНАЧЕ ЕСТЬNULL(ЗаказыКлиентовОстатки.КОтгрузкеКонечныйОстаток, 0)
		...
		|			КОГДА ЗаказыКлиентовОстатки.Номенклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины В (&МерныеТипыВеличин)
		|				ТОГДА ВЫБОР
		|					КОГДА ЗаказыКлиентовОстатки.ЗаказаноКонечныйОстаток <= ЗаказыКлиентовОстатки.КОформлениюПриход *
		|						(&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100)
		|					И ЗаказыКлиентовОстатки.ЗаказаноКонечныйОстаток >= -(ЗаказыКлиентовОстатки.КОформлениюПриход *
		|						(&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100))
		|						ТОГДА 0
		...
		|			КОГДА ЗаказыКлиентовОстатки.Номенклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины В (&МерныеТипыВеличин)
		|				ТОГДА ВЫБОР
		|					КОГДА ЗаказыКлиентовОстатки.СуммаКонечныйОстаток <= ЗаказыКлиентовОстатки.СуммаПриход *
		|						(&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100)
		|					И ЗаказыКлиентовОстатки.СуммаКонечныйОстаток >= -ЗаказыКлиентовОстатки.СуммаПриход *
		|						(&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100)
		...
		|				ТОГДА ВЫБОР
		|					КОГДА ТоварыКОтгрузкеОстатки.КОтгрузкеКонечныйОстаток <= ТоварыКОтгрузкеОстатки.КОтгрузкеПриход *
		|						(&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100)
		|					И ТоварыКОтгрузкеОстатки.КОтгрузкеКонечныйОстаток >= -(ТоварыКОтгрузкеОстатки.КОтгрузкеПриход *
		|						(&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100))
		|						ТОГДА 0
		...
		|	ИЗ
		|		РегистрНакопления.ТоварыКОтгрузке.ОстаткиИОбороты(&НачПериодТоварыКОтгрузке, &КонПериодТоварыКОтгрузке,,,
		...
		ПараметрыФормированияТаблицы.НачалоПериода = "&НачПериодЗаказыКлиентов";
		ПараметрыФормированияТаблицы.КонецПериода = "&КонПериодЗаказыКлиентов";
		...
		ПараметрыФормированияТаблицы.Показатели = "Заказано, КОформлению, КОтгрузке, Оформлено, Отгружено, СписаноДоППС, Принято";
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			"&ТекстУсловияДокументПродажи",
			ПродажиСервер.ТекстУсловияДокументПродажи("Подзапрос.Распоряжение"));
...
Функция ТекстЗапросаСостояниеВзаиморасчетов()
	...
	ТекстЗапроса = "
		...
		|		ГДЕ
		|			ОбъектыРасчетов.Объект В (&МассивЗаказов)
		|			И Расчеты.АналитикаУчетаПоПартнерам.Организация = ОбъектыРасчетов.Организация
		...
		|						ИЗ
		|							ВтСвязьЗаказовНакладных КАК Т)
		|		И Расчеты.АналитикаУчетаПоПартнерам.Организация = Расчеты.ОбъектРасчетов.Организация
		|		И Расчеты.Активность
		|	) КАК Расчеты
		|СГРУППИРОВАТЬ ПО
		|	Расчеты.Заказ
