...
Процедура СмТакжеНастройкиИСервисОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	Если СтрНачинаетсяС(НавигационнаяСсылкаФорматированнойСтроки, "ОткрытьЖурналМашиночитаемыеДоверенности") Тогда
...
Процедура ТокеныПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, ЭтоГруппа, Параметр)
	...
	Если Копирование Тогда
		...
		ПараметрыОткрытия.Вставить("ПроизводственныйОбъект", ТекущиеДанные.ПроизводственныйОбъект);
	Иначе
		ПараметрыОткрытия.Вставить("ТипТокенаАвторизации",   ТипТокенаАвторизации);
...
Процедура НастроитьЭлементыФормыНаСервере()
	...
	Иначе
		ПредставлениеТипаТокена = "";
		Если ТипТокенаАвторизации = Перечисления.ТипыТокеновАвторизации.ИСМП Тогда
			ПредставлениеТипаТокена = НСтр("ru = 'ИС МП'");
		ИначеЕсли ТипТокенаАвторизации = Перечисления.ТипыТокеновАвторизации.СУЗ Тогда
			ПредставлениеТипаТокена = НСтр("ru = 'СУЗ'");
		ИначеЕсли ТипТокенаАвторизации = Перечисления.ТипыТокеновАвторизации.ИСМПРозница Тогда
			ПредставлениеТипаТокена = НСтр("ru = 'Розница'");
		КонецЕсли;
		
		Заголовок = СокрЛП(СтрШаблон(НСтр("ru = 'Токены авторизации %1'"), ПредставлениеТипаТокена));
	...
	ТребуетсяАктуализация = Ложь;
		...
		Для Каждого Напоминание Из Напоминания Цикл
			...
			ИначеЕсли Напоминание.Отсутствует Тогда
				Продолжить;
			ИначеЕсли ЗначениеЗаполнено(ТипТокенаАвторизации) И ТипТокенаАвторизации <> Напоминание.ТипТокенаАвторизации Тогда
				Продолжить;
			КонецЕсли;
...
Функция СформироватьГиперссылкуСмТакже()
	...
	ТекстГиперссылки = НСтр("ru = 'Ответственные за актуализацию токенов авторизации'");
	МассивСтрок.Добавить(
		Новый ФорматированнаяСтрока(ТекстГиперссылки,,,,
		"Справочник.ОтветственныеЗаАктуализациюТокеновАвторизацииИСМП.Форма.ФормаСписка"));
	МассивСтрок.Добавить("; ");
	
	ТекстГиперссылки = НСтр("ru = 'Настройки электронной подписи и шифрования'");
	МассивСтрок.Добавить(
		Новый ФорматированнаяСтрока(ТекстГиперссылки,,,,
		"ОбщаяФорма.НастройкиЭлектроннойПодписиИШифрования"));
	
	Если ОбщегоНазначения.ПодсистемаСуществует(
		"ЭлектронноеВзаимодействие.ОбменСКонтрагентами.МашиночитаемыеДоверенности") Тогда
		
		МассивСтрок.Добавить("; ");
		ТекстГиперссылки = НСтр("ru = 'Машиночитаемые доверенности'");
		МассивСтрок.Добавить(
			Новый ФорматированнаяСтрока(ТекстГиперссылки,,,,
			"ОткрытьЖурналМашиночитаемыеДоверенности"));
