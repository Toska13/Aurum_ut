...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ЭтоРасширеннаяВерсияГосИС() Тогда
		...
		МодульИнтеграцияИСМППереопределяемый.ПриОпределенииИспользованияПроизводственногоОбъекта(ИспользуетсяПроизводственныйОбъект);
	Иначе
		ИспользуетсяПроизводственныйОбъект = Ложь;
	КонецЕсли;
...
Процедура УправлениеЭлементамиФормы(Форма)
	...
	ЭтоТокенUUID = ЭтоТокенСУЗ
		Или Форма.ТипТокенаАвторизации = ПредопределенноеЗначение("Перечисление.ТипыТокеновАвторизации.ИСМП")
			И СтрДлина(Форма.Токен) = 36;
	
	Форма.Элементы.ПроизводственныйОбъект.Видимость = ЭтоТокенСУЗ И Форма.ИспользуетсяПроизводственныйОбъект;
	Форма.Элементы.ДействуетДо.Видимость            = ЭтоТокенUUID;
	Форма.Элементы.РасшифрованныйТокен.Видимость    = Не ЭтоТокенUUID;
...
Процедура РасшифроватьТокен(Форма)
	...
	Если Форма.ТипТокенаАвторизации = ПредопределенноеЗначение("Перечисление.ТипыТокеновАвторизации.СУЗ")
		Или Форма.ТипТокенаАвторизации = ПредопределенноеЗначение("Перечисление.ТипыТокеновАвторизации.ИСМПРозница")
		Или СтрДлина(Форма.Токен) = 36 Тогда
...
Функция РасшифроватьТокенJWT(Токен)
	...
	Попытка
		РезультатРазбора = ОбщегоНазначенияИС.РасшифроватьТокенJWT(Токен);
	Исключение
		Возврат Результат;
	КонецПопытки;
...
Функция ПроверитьЗаполнениеДанных()
	...
	Если ТипТокенаАвторизации = ПредопределенноеЗначение("Перечисление.ТипыТокеновАвторизации.СУЗ")
		И Не ЗначениеЗаполнено(ПроизводственныйОбъект)
		И ИспользуетсяПроизводственныйОбъект Тогда
	...
	Если (ТипТокенаАвторизации = ПредопределенноеЗначение("Перечисление.ТипыТокеновАвторизации.СУЗ")
		Или ТипТокенаАвторизации = ПредопределенноеЗначение("Перечисление.ТипыТокеновАвторизации.ИСМП")
			И СтрДлина(Токен) = 36)
