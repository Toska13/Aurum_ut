...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ТекущаяСтрока", ТекущийКодТНВЭД) Тогда
...
Процедура ОбработкаВыбораЗначенияСписка(ВыбраннаяСтрока, ИмяСписка, СтандартнаяОбработка)
	...
	ОчиститьСообщения();
	Если Не ОбщегоНазначенияИСКлиент.ВыборСтрокиСпискаКорректен(Элементы[ИмяСписка], Истина) Тогда
		Возврат;
	КонецЕсли;
...
Функция ТекстЗапросаСписокТНВЭД()
	...
	|
	|ГДЕ
	|	(КлассификаторНСИЗЕРНО.РазделУчета = &РазделУчета Или &БезОтбораПоРазделуУчета)
	...
	|	КлассификаторНСИЗЕРНО.РазделУчета";
...
Процедура ЗаполнитьСписокТНВЭД()
	...
	Запрос.УстановитьПараметр("БезОтбораПоРазделуУчета", Не ЗначениеЗаполнено(РазделУчета));
