...
//                         Интернет-эквайрингу.
//                       - СправочникСсылка.НастройкиПодключенияКПлатежнымСистемам - настройка подключения к
//                         различным платежным системам
...
Функция ШаблоныНазначенийДокументаОперации(
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ИнтернетЭквайринг") Тогда
		...
		ЭтоИнтернетЭквайринг = МодульИнтернетЭквайрингСлужебный.ЭтоНастройкаПодключения(НастройкаПодключения);
	КонецЕсли;
	...
	ИначеЕсли ЗначениеЗаполнено(ПлатежнаяСистема) Тогда
		ПредопределенныеШаблоны = МодульПлатежныеСистемыСлужебный.НастройкиПодключения(ПлатежнаяСистема).ШаблоныНазначений;
