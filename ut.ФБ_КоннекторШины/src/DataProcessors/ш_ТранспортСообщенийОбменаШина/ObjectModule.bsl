...
Функция ПодключениеУстановлено() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ТипСообщенияОбмена) Тогда
		
		СообщениеОбОшибке = НСтр("ru = 'Ошибка подключения: Не указан тип сообщения обмена.'");
	...
	Если НЕ ЗначениеЗаполнено(Система) Тогда
		...
		СообщениеОбОшибке = НСтр("ru = 'Ошибка подключения: Не указан система корреспондент.'");
		ТранспортСообщенийОбмена.ЗаписатьСообщениеВЖурналРегистрации(ЭтотОбъект);
...
Функция ОтправитьСообщение()
	...
	Пакет.СистемаНазначения = Система;
	Пакет.ТипСообщения = ТипСообщенияОбмена;
	Пакет.ВидПакета = Перечисления.ш_ВидыПакетов.Адресный;
	
	ДвоичныеДанные = Новый ДвоичныеДанные(СообщениеОбмена);
	
	Пакет.Пакет = Base64Строка(ДвоичныеДанные);
...
Функция ПолучитьСообщение(ШаблонИмениСообщения)
	...
	|	ш_ОчередьПакетовИзШины.ТекстОшибки КАК ТекстОшибки
	|ИЗ
	|	РегистрСведений.ш_ОчередьПакетовИзШины КАК ш_ОчередьПакетовИзШины
	|ГДЕ
	|	ш_ОчередьПакетовИзШины.Система = &Система
	|	И ш_ОчередьПакетовИзШины.ТипСообщения = &ТипСообщения
	...
	Запрос.УстановитьПараметр("ТипСообщения", ТипСообщенияОбмена);
	
	ТаблицаФайловСообщенийОбмена = Запрос.Выполнить().Выгрузить();
	...
	Если ТаблицаФайловСообщенийОбмена.Количество() = 0 Тогда
		...
		СообщениеОбОшибке = СтрШаблон(СообщениеОбОшибке, Система, ТипСообщенияОбмена);
	...
	Иначе
		...
		ДвоичныеДанные = Base64Значение(ТаблицаФайловСообщенийОбмена[0].Пакет);
		ДвоичныеДанные.Записать(СообщениеОбмена);
		
		рег = РегистрыСведений.ш_ОчередьПакетовИзШины.СоздатьМенеджерЗаписи();
		ЗаполнитьЗначенияСвойств(рег, ТаблицаФайловСообщенийОбмена[0]);
		рег.ДатаОбработки = ТекущаяДатаСеанса();
		рег.Обработано = Истина;
		рег.Записать(Истина);
