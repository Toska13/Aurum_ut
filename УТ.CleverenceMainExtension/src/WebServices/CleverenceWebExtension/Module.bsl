...
Функция Invoke(ИмяСобытия, Параметры, ИдБазыСмартс, exchtype, connectiontoken)
	...
	Исключение
		...
		Возврат "# Ошибка при выполнении события: "+ ИмяСобытия +", описание ошибки: "+ ОписаниеОшибки;
	КонецПопытки;
	
	СтруктураОтвета = КлеверенсТСД_МодульВебМетодов.СформироватьПараметрыGZIPWeb(Результат,exchtype,connectiontoken, МодульОнлайн);
		
	Возврат СтруктураОтвета.Результат;
