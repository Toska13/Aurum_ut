...
Функция ПроверитьПолучитьВыделенныеВСпискеСсылки(Список, ПроверятьКоличество, ВСпискеСсылки) Экспорт
	...
	Если ПроверятьКоличество И
		...
		ПоказатьПредупреждение(, НСтр("be = ""Каманда не можа быць выканана для абранага аб'екта!""; en = 'The command cannot be executed for the specified object!'; es = '¡No se puede ejecutar el comando para el objeto especificado!'; ka = 'კომანდა ვერ შეიძლება შესრულდეს მითითებული ობიექტისთვის!'; kk = 'Берілген нысанды орындау мүмкін емес команда!'; ro = 'Comanda nu poate fi executată pentru obiectul specificat!'; ru = 'Команда не может быть выполнена для указанного объекта!'; tr = 'Belirtilen nesne için komut gerçekleştirilemiyor!'; uk = ""Команда не може бути виконана для вказаного об'єкта!""; uz = ""Belgilangan obyekt uchun buyruq bajarib bo'lmaydi!"""));
	КонецЕсли;
...
Функция ПроверитьТипПараметраКоманды(Знач Параметр, Знач ОжидаемыйТип) Экспорт
	...
	Если НЕ Результат Тогда
		ПоказатьПредупреждение(,НСтр("be = 'Дзеянне не можа быць выканана для выбранага элемента.'; en = 'The action cannot be performed for the selected item.'; es = 'La acción no puede ser realizada para el elemento seleccionado.'; ka = 'მოქმედება არ შეიძლება შესრულდეს არჩეული ელემენტისთვის.'; kk = 'Таңдалған элементке әрекет орындалмайды.'; ro = 'Acțiunea nu poate fi efectuată pentru elementul selectat.'; ru = 'Действие не может быть выполнено для выбранного элемента.'; tr = 'Seçilen öğe için işlem yapılamıyor.'; uk = 'Дія не може бути виконана для вибраного елемента.'; uz = ""Tanlangan element uchun amal bajarib bo'lmaydi."""));
	КонецЕсли;
