...
Процедура ОткрытьДинамическийСписок(Объект = Неопределено, Владелец = Неопределено, РежимВыбора = Ложь, ЗакрыватьПриВыборе = Ложь) Экспорт
	...
	Уникальность = Истина;
	ОткрытьФорму("Обработка.ИТК_ДинамическийСписок.Форма", Параметры, Владелец, Уникальность);
...
Процедура ОткрытьВыборПользователей(Владелец, ВыбранныеПользователи, ОписаниеОповещенияЗавершение, СтандартнаяОбработка = Неопределено) Экспорт
	...
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("Заголовок", НСтр("ru = 'Выбор пользователей';
														|en = 'User choice'"));
	
	ДоступныеПользователи = ИТК_АнализПравДоступа.ВсеПользователиСписок();
	Для Каждого Пользователь Из ДоступныеПользователи Цикл 
		
		Пользователь.Пометка = НЕ ЗначениеЗаполнено(ВыбранныеПользователи)
									ИЛИ (ВыбранныеПользователи.НайтиПоЗначению(Пользователь.Значение) <> Неопределено);
		
	КонецЦикла;
	
	ИТК_ФормаКлиент.ОткрытьСписокЗначений(Владелец, ИТК_Перечисления.ВариантФормыСписокЗначенийВыборНесколькихЗначений(),
											ДоступныеПользователи, ТипЗначения, ОписаниеОповещенияЗавершение, СтандартнаяОбработка, ДополнительныеПараметры);
...
Процедура ОткрытьНовуюФорму(Форма) Экспорт
