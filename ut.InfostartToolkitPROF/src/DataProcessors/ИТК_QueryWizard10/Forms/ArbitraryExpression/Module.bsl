...
Procedure FieldsBeforeExpand(Val Item, 
	...
	Try
		DataProcessor = GetForm("DataProcessor.ИТК_QueryWizard10.Form.QueryWizard");
	Except
		Message(ErrorDescription());
		Return;
	EndTry;

	CurrentItems = Fields.FindByID(Row);
	...
	If Prop <> Undefined Then
		SourcesBeforeExpandAtServer(QueryWizardAddress, CurrentQuerySchemaSelectQuery, 
                                    CurrentQuerySchemaOperator, NestedQueryPositionAddress, Row);
	EndIf;
...
Procedure BeforeClose(Cancel, Exit, MessageText, StandardProcessing)
	...
	If (Expression <> OldExpression)
		...
		ShowQueryBox(Notification, NStr("ru = 'Сохранить изменения?';
