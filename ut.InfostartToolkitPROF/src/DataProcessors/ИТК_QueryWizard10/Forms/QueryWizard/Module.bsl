...
Function PreFillAtClient()
	...
	For Each PageItem In PagesState Do
		If PageItem.Value Then
			...
			ElsIf  PageItem.Key = "TotalsPage" Then
...
Procedure PostFillAtClient(Val PagesItems)
	...
	If PagesItems.Property("TotalsPage") AND Items.TotalsPage.Visible Then
...
Procedure MoveQueryBatchAtServer(Val Index, Val NewIndex, Val QuerySchema)
	QuerySchema.QueryBatch.Move(Index, NewIndex);
EndProcedure
...
Procedure MoveIndexAtServer(Val Index, Val NewIndex, Query)
	Query.Index.Move(Index, NewIndex);
EndProcedure
...
Procedure MoveOrderAtServer(Val Index, Val NewIndex, Query)
	Query.Order.Move(Index, NewIndex);
EndProcedure
...
Procedure ConditionsBeforeEditEnd(Item, NewRow, CancelEdit, Cancel)
	...
	If NOT(CheckCondition(Item.CurrentData["Condition"])) Then
		ShowErrorMessage();
		Cancel = True;
		Return;
	EndIf;
...
Procedure ConditionAddAll(Command)
	...
	If (ConditionItems.GetItems().Count() = 1)
		...
		AllFieldsForConditionsBeforeExpandAtServer(ConditionItems.GetID())	;
	EndIf;
...
Procedure ShowQueryEditor(Val QueryText, OldQueryText = "", Val StartRow = -1, Val StartCol = -1)
	...
	Form = OpenForm("DataProcessor.ИТК_QueryWizard10.Form.QueryEditor", Params, ThisForm,,,,Notification,
...
Procedure JoinsBeforeEditEnd(Item, NewRow, CancelEdit, Cancel)
	...
	If CancelEdit Then
		ChangeJoinExpression(TmpString);
		CurrentItems["Expression"] = TmpString;
	EndIf;

	If NOT(ChangeJoinExpression()) Then
		ShowErrorMessage();
		Cancel = True;
		Return;
	EndIf;
...
&AtServer
Function ChangeJoinExpression(Val NewExpression = Undefined, Val ShowMessage = True, ErrorMessage = Undefined)
	Var CurrentRow;
	Var CurrentItems;
	Var Expression;
	Var QuerySchema;
...
Procedure TotalsExpressionsBeforeRowChange(Item, Cancel)
	...
	If Item.CurrentItem.Name = "TotalsExpressionsExpression" Then
		Parent = TotalsExpressions.FindByID(Item.CurrentRow);
		Expressions = TotalsExpressionBeforeRowChangeAtServer(Item.CurrentData["Expression"], Item.CurrentData["Index"]);
...
Function TotalsExpressionBeforeRowChangeAtServer(Val Expression,  Val Index)
	...
	Expressions = GetAgregateExpressions(Expression, True);
	Count = Expressions.Count();
	Arr = New Array;
	For Pos = 0 To Count - 1 Do
		Try
...
Procedure TotalsExpressionsBeforeEditEnd(Item, NewRow, CancelEdit, Cancel)
	...
	If StartEdit Then
		...
		If Item.CurrentItem.Name = "TotalsExpressionsExpression" Then
			If NOT(ChangeTotalsExpression(CurrentItems["Expression"])) Then
				ShowErrorMessage();
				Cancel = True;
				Return;
			EndIf;
...
&AtServer
Function ChangeTotalsExpression(Expression, ErrorMessage = Undefined)
	Var QuerySchema;
	Var Query;
	Var CurrentRow;
	Var CurrentItems;
...
Procedure ExpressionChange(Val IsExpression, Val NewExpression = Undefined, Val IsExpressionChanged = Undefined)
    ...
    OpenForm("DataProcessor.ИТК_QueryWizard10.Form.ArbitraryExpression", Params, ThisForm,,,,Notification,
             FormWindowOpeningMode.LockOwnerWindow);
EndProcedure
...
Procedure SourcesParametersOfTheVirtualTable(Command)
	...
	OpenForm("DataProcessor.ИТК_QueryWizard10.Form.AvailableTableParameters", Params, ThisForm,,,,Notification,
             FormWindowOpeningMode.LockOwnerWindow);
EndProcedure
...
Procedure SourcesCreateDescriptionOfTheTemporaryTable(Command)
	...
	Params.Insert("NestedQueryPositionAddress", NestedQueryPositionAddress);
	Notification = New NotifyDescription("TableParametersChanged", ThisForm, Params);
	OpenForm("DataProcessor.ИТК_QueryWizard10.Form.TempTable", Params, ThisForm,,,,Notification,
             FormWindowOpeningMode.LockOwnerWindow);
EndProcedure
...
Procedure AvailableTablesBeforeExpand(Item, Row, Cancel)
	...
	If NOT(IsFakeItem(CurrentItems)) Then
	    Return;
	EndIf;
	AvailableTablesBeforeExpandAtServer(Row);
EndProcedure
...
Procedure SourcesBeforeExpand(Item, Row, Cancel)
	...
	If NOT(IsFakeItem(CurrentItems)) Then
	    Return;
	EndIf;
	SourcesBeforeExpandAtServer(Row);
EndProcedure
...
Procedure AllFieldsForGroupingBeforeExpand(Item, Row, Cancel)
	...
	If CurrentItems["AvailableField"]
		...
		Return;
	EndIf;
	AllFieldsForGroupingBeforeExpandAtServer(Row);
EndProcedure

&AtServer
Procedure AllFieldsForGroupingBeforeExpandAtServer(Row)
	FillSourcesBeforeExpand(Row, AllFieldsForGrouping);
EndProcedure
...
Procedure AllFieldsForConditionsBeforeExpand(Item, Row, Cancel)
	...
	If NOT(IsFakeItem(CurrentItems)) Then
	    Return;
	EndIf;
	AllFieldsForConditionsBeforeExpandAtServer(Row);
EndProcedure

&AtServer
Procedure AllFieldsForConditionsBeforeExpandAtServer(Row)
...
Procedure AllFieldsForIndexBeforeExpand(Item, Row, Cancel)
	...
	If (CurrentItems["Name"] = "")
		...
		Return;
	EndIf;
	AllFieldsForIndexBeforeExpandAtServer(Row);
EndProcedure
...
Procedure AllFieldsForOrderBeforeExpand(Item, Row, Cancel)
	...
	If (CurrentItems["Name"] = "")
		...
		Return;
	EndIf;
	AllFieldsForOrderBeforeExpandAtServer(Row);
EndProcedure
...
Procedure AllFieldsForTotalsBeforeExpand(Item, Row, Cancel)
	...
	If CurrentItems["IsAlias"]
		...
		Return;
	EndIf;
	AllFieldsForTotalsBeforeExpandAtServer(Row);
EndProcedure
...
Procedure EditNestedQuery(Val NotificationName, Val NestedQuerySourceIndex, Val IsNew, Val QueryText = Undefined)
	...
	OpenForm("DataProcessor.ИТК_QueryWizard10.Form.QueryWizard", Params,, True,,,
...
Procedure RenameSource(Command)
	...
	SourcesSetAction("BeforeRowChange", "BeginRenameSource");
...
Procedure SourcesBeforeEditEnd(Item, NewRow, CancelEdit, Cancel)
	...
	If NOT(StartEdit) OR
		TmpString = Item.CurrentData["Presentation"] Then
	    Return;
	EndIf;
...
Function ChangeSourceName()
	...
	SourcesSetAction("BeforeRowChange", "SourcesBeforeRowChange");
...
Procedure SourcesReplaceTheTable(Command)
	...
	OpenForm("DataProcessor.ИТК_QueryWizard10.Form.TableSelecting", Params, ThisForm,,,,Notification,
             FormWindowOpeningMode.LockOwnerWindow);
EndProcedure
...
Procedure SummingFieldsBeforeRowChange(Item, Cancel)
	...
	Parent = SummingFields.FindByID(Item.CurrentRow);
	If Parent.GetItems().Count() > 0 Then
		Cancel = True;
		Return;
	EndIf;

	Expressions = SummingFieldsBeforeRowChangeAtServer(Item.CurrentData["Name"], "", Item.CurrentData["Index"]);
...
Procedure JoinConditionChanged(ChildForm, Params) Export
	...
	If Params["Changed"] Then
		ErrorMessage = "";
		If NOT(ChangeJoinExpression(Params["Expression"], False, ErrorMessage)) Then
			StartRow = -1;
			StartCol = -1;
...
Procedure JoinsExpressionStartChoice(Item, ChoiceData, StandardProcessing)
	StandardProcessing = False;
	JoinsExpressionChangeInDialog(Item.EditText);
EndProcedure
...
Procedure ConditionsConditionStartChoice(Item, ChoiceData, StandardProcessing)
	StandardProcessing = False;
	ConditionChangeInDialog(Item.EditText);
EndProcedure
...
&AtClient
Procedure JoinsOnActivateRow(Item)
	Var CurrentItems;
	Var ReceivingItem;
	Var CurrentRow;
	...
	If MoveJoinTable >= 0 Then
		...
		EndIf;
		MakeJoin(CurrentItems, ReceivingItem);
		Item.CurrentRow = Items.Joins.CurrentRow;
	EndIf;
	MoveJoinTable = -1;
...
Procedure SummingFieldsNameStartChoice(Item, ChoiceData, StandardProcessing)
	StandardProcessing = False;
	SummingFieldChangeInDialog(Item.EditText);
EndProcedure
...
Procedure TotalsExpressionsExpressionStartChoice(Item, ChoiceData, StandardProcessing)
	StandardProcessing = False;
	TotalsExpressionChangeInDialog(Item.EditText);
EndProcedure
...
Procedure TotalsExpressionChanged(ChildForm, Params) Export
	...
	If Params["Changed"] Then
		ErrorMessage = "";
		If NOT(ChangeTotalsExpression(Params["Expression"], ErrorMessage)) Then
			StartRow = -1;
			StartCol = -1;
			GetErrorTextBounds(StartRow, StartCol, ErrorMessage);
			ChildForm.SetTextSelectionBounds(StartRow, StartCol);
...
&AtClient
Procedure BeginRenameSource(Item, Cancel)
	TmpString = Item.CurrentData["Presentation"];
EndProcedure

&AtServer
Procedure SourcesSetAction(Val ActionName, Val Action)
	Items.Sources.SetAction(ActionName, Action);
EndProcedure

&AtClient
Procedure SourcesBeforeRowChange(Item, Cancel)
	Cancel = True;
	SourcesChangeTheCurrentItem(Undefined);
EndProcedure
// }
