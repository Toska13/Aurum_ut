...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	|	Таблица.ПериодОтклонения КАК ПериодОтклонения,
	...
	|	Себестоимость.ПериодОтклонения КАК ПериодОтклонения
	|ИЗ
...
Функция СохранитьНеИзмененныеПервичныеДвижения(ИзмененныеПериоды)
	...
	|		ПО Т.ХозяйственнаяОперация = Правила.ХозяйственнаяОперация
	|
	|СГРУППИРОВАТЬ ПО
	|	&ТекстПоляГруппировкиБезПартий
	|ИМЕЮЩИЕ
	...
	Иначе
		...
		// Нет изменений - все первичные движения возьмем из ИБ
		ШаблонТекстаЗапроса =
		"ВЫБРАТЬ
		|	&Регистратор КАК Регистратор,
		|	&ТекстПоляРегистра,
