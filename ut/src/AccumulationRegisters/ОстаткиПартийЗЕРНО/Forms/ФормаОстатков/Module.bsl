...
Процедура НастроитьОтборСпискаДляВыбора()
	...
	Если ЗначениеЗаполнено(Параметры.РазделУчета) Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список,
			"РазделУчета", Параметры.РазделУчета, ВидСравненияКомпоновкиДанных.Равно,, Истина);
	КонецЕсли;
	
	ЗаполненоОКПД2 = ЗначениеЗаполнено(Параметры.ОКПД2);
	
	Если ЗаполненоОКПД2 Тогда
		Если Параметры.РазделУчета = ПредопределенноеЗначение("Перечисление.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна") Тогда
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список,
...
Процедура ПриИзмененииОтбора(Форма, Имя, ОбновлятьКлючи = Истина, ОбновлятьНадписьКлючиАдресов = Истина)
	...
	Если ОбновлятьКлючи Тогда
		...
		Иначе
			Форма[ИмяЭлементаОтбораКлючи].ЗагрузитьЗначения(
				КлючиДляОтбора(Форма[ИмяЭлементаОтбораСписок].ВыгрузитьЗначения(), Неопределено, ИмяЭлементаОтбора));
		КонецЕсли;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция КлючиДляОтбора(Знач ДанныеДляОтбора, Знач Подразделение, Знач ИмяЭлементаОтбора)
