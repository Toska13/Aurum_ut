...
Процедура ПриПоискеЗаменыСсылок(ПарыЗамен, НеобработанныеЗначенияОригиналов) Экспорт
	...
	Для Каждого НеобработанныйДубль Из НеобработанныеЗначенияОригиналов Цикл
		...
		ПарыЗамен.Вставить(
			НеобработанныйДубль.ЗаменяемоеЗначение,
			РегистрыСведений.АналитикаУчетаНоменклатуры.СоздатьКлючАналитики(ПараметрыКлюча));
	КонецЦикла;
...
Процедура ОчиститьДвиженияСтоимостьТоваровПослеЗаменыСсылокКлючейАналитики(Результат, ТаблицаПоиска) Экспорт
	
	КлючиКУдалению = Результат.ОчередьКНепосредственномуУдалению;
	Если Не КлючиКУдалению.Количество() Тогда
		Возврат;
	КонецЕсли;
	...
	// Выберем ключи аналитики для удаления.
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ДанныеСправочника.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ДублиКлючей
	...
	|	Справочник.КлючиАналитикиУчетаНоменклатуры КАК ДанныеСправочника
	|ГДЕ
	|	ДанныеСправочника.Ссылка В (&ДублиКлючей)
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|";
	Запрос.УстановитьПараметр("ДублиКлючей", КлючиКУдалению);
	
	Запрос.Выполнить();
	
	// Очистим движения регистра СтоимостьТоваров по удаляемым ключам
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СтоимостьТоваров.Регистратор КАК Регистратор
	|ИЗ
	...
	|ГДЕ
	|	Т.Регистратор = &Регистратор
	|	И НЕ Т.АналитикаУчетаНоменклатуры В (&ДублиКлючей)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Т.НомерСтроки";
	РазмерПорцииДанных = РасчетСебестоимостиПовтИсп.ЗначенияТехнологическихПараметров().КоличествоСтрокВТЗ;
	...
	Для Каждого Регистратор Из Регистраторы Цикл
		...
		Исключение
			...
			Выборка.Сбросить();
			Пока Выборка.Следующий() Цикл
				Результат.ЕстьОшибки = Результат.ЕстьОшибки Или Истина;
				ОписаниеОшибки = Результат.Ошибки.Добавить();
				ОписаниеОшибки.Ссылка = Выборка.АналитикаУчетаНоменклатуры;
				ОписаниеОшибки.ПредставлениеОбъектаОшибки = ОбщегоНазначения.ПредметСтрокой(Регистратор);
				ОписаниеОшибки.ОбъектОшибки = Регистратор;
				ОписаниеОшибки.ИнформацияОбОшибке = ИнформацияОбОшибке();
				ОписаниеОшибки.ТекстОшибки = ТекстОшибки;
				ОписаниеОшибки.ТипОшибки = "ОшибкаЗаписи";
			КонецЦикла;
