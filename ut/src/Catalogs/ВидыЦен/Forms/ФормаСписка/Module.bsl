...
Процедура СписокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Если ?(НайденныйПараметр = Неопределено, Ложь, НайденныйПараметр.Значение) Тогда
		РеквизитыДляЗаполнения.Добавить("КоличествоСкладовВсего");
		КоличествоСкладов = "КоличествоСкладов";
	Иначе
		КоличествоСкладов = "";
	КонецЕсли;
	...
	Если ?(НайденныйПараметр = Неопределено, Ложь, НайденныйПараметр.Значение) Тогда
		РеквизитыДляЗаполнения.Добавить("КоличествоФорматовМагазиновВсего");
		КоличествоФорматовМагазинов = "КоличествоФорматовМагазинов";
	Иначе
		КоличествоФорматовМагазинов = "";
	КонецЕсли;

	НайденныйПараметр = ПараметрыСписка.Найти("ИспользоватьПартнеров");
	Если ?(НайденныйПараметр = Неопределено, Ложь, НайденныйПараметр.Значение) Тогда
		РеквизитыДляЗаполнения.Добавить("КоличествоПартнеровВсего");
		КоличествоПартнеров = "КоличествоПартнеров";
	Иначе
		КоличествоПартнеров = "";
	КонецЕсли;
	...
	Если ?(НайденныйПараметр = Неопределено, Ложь, НайденныйПараметр.Значение) Тогда
		РеквизитыДляЗаполнения.Добавить("КоличествоСоглашенийВсего");
		КоличествоСоглашений = "КоличествоСоглашений";
	Иначе
		КоличествоСоглашений = "";
	КонецЕсли;
	
	ПроверенВыводРеквизитовВСписок = Ложь;
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		СтрокаСпискаДанные = Строки[Выборка.ВидЦены].Данные;
		Если Не ПроверенВыводРеквизитовВСписок Тогда
			Если Не ПустаяСтрока(КоличествоСкладов)
				И СтрокаСпискаДанные.Свойство(КоличествоСкладов) Тогда
				РеквизитыДляЗаполнения.Добавить(КоличествоСкладов);
			КонецЕсли;
			Если Не ПустаяСтрока(КоличествоФорматовМагазинов)
				И СтрокаСпискаДанные.Свойство(КоличествоФорматовМагазинов) Тогда
				РеквизитыДляЗаполнения.Добавить(КоличествоФорматовМагазинов);
			КонецЕсли;
			Если Не ПустаяСтрока(КоличествоПартнеров)
				И СтрокаСпискаДанные.Свойство(КоличествоПартнеров) Тогда
				РеквизитыДляЗаполнения.Добавить(КоличествоПартнеров);
			КонецЕсли;
			Если Не ПустаяСтрока(КоличествоСоглашений)
				И СтрокаСпискаДанные.Свойство(КоличествоСоглашений) Тогда
				РеквизитыДляЗаполнения.Добавить(КоличествоСоглашений);
			КонецЕсли;
			СтрокаРеквизитыДляЗаполнения = СтрСоединить(РеквизитыДляЗаполнения, ", ");
			ПроверенВыводРеквизитовВСписок = Истина;
		КонецЕсли;
...
Процедура УстановитьНастройкиПоФО()
	...
	Элементы.НадписьВПартнерах.Видимость						= ИспользоватьПартнеров;
	Элементы.КомандаВПартнерах1.Видимость						= ИспользоватьПартнеров;
	Элементы.КомандаВПартнерах2.Видимость						= ИспользоватьПартнеров;
	Элементы.КомандаВПартнерах4.Видимость						= ИспользоватьПартнеров;
