...
Функция СообщенияЗагрузкиСпискаПартий(ДополнительныеПараметры) Экспорт
	...
	РазделУчета     = ДополнительныеПараметры.ПараметрыОбработкиДокумента.РазделУчета;
	...
	АбстрактноеСообщениеXML.РазделУчета        = РазделУчета;
	...
	ПараметрыФормирования.РазделУчета  = РазделУчета;
...
Функция СообщениеЗагрузкиПартииПоНомеру(НомерПартии, ДополнительныеПараметры, ПараметрыОбмена) Экспорт
	...
	СообщениеXML.РазделУчета              = ДополнительныеПараметры.ПараметрыОбработкиДокумента.РазделУчета;
	СообщениеXML.ДополнительноеОписание    = СтрШаблон(НСтр("ru = '№ партии: %1'"), НомерПартии);
	
	Если ДополнительныеПараметры.ПараметрыОбработкиДокумента.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна Тогда
	...
	ИначеЕсли ДополнительныеПараметры.ПараметрыОбработкиДокумента.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.Зерно Тогда
	...
	ИмяПоляНомерПартии = ИнтеграцияЗЕРНОСлужебный.ИмяПоляНомерПартии(ДополнительныеПараметры.ПараметрыОбработкиДокумента.РазделУчета);
...
Функция СообщенияЗагрузкиПартий(ПараметрыФормирования, ПараметрыОбмена = Неопределено)
	...
	Если ПараметрыФормирования.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна Тогда
		ПространствоИмен   = ИнтеграцияЗЕРНОСлужебный.ПространствоИменПоИмениПакетаXDTO("zerno_api_gpb", ПараметрыОбмена.ПараметрыОптимизации);
		ИмяСвойстваЗапроса = "RequestGetListGpb";
	Иначе
		ПространствоИмен   = ИнтеграцияЗЕРНОСлужебный.ПространствоИменПоИмениПакетаXDTO("zerno_api_lots", ПараметрыОбмена.ПараметрыОптимизации);
	...
	СообщениеXML.РазделУчета     = ПараметрыФормирования.РазделУчета;
	СообщениеXML.ТипСообщения     = Перечисления.ТипыЗапросовИС.Исходящий;
	...
	Если ЗначениеЗаполнено(ПараметрыФормирования.НомерПартии) Тогда
		ИнтеграцияЗЕРНОСлужебный.ЗаполнитьСвойствоXDTO(
			ЗапросПартийXDTO,
			ИнтеграцияЗЕРНОСлужебный.ИмяПоляНомерПартии(ПараметрыФормирования.РазделУчета),
	...
	Если ЗначениеЗаполнено(ПараметрыФормирования.ИдентификаторПартии) Тогда
		ИнтеграцияЗЕРНОСлужебный.ЗаполнитьСвойствоXDTO(
			ЗапросПартийXDTO,
			ИмяПоляИдентификаторПартии(ПараметрыФормирования.РазделУчета),
...
Функция ПартияЗерна(Идентификатор, Организация, Подразделение, РазделУчета, ПараметрыОбмена, ПараметрыПолучения = Неопределено, ДатаКонтроля = Неопределено) Экспорт
...
Функция ПараметрыЗагрузкиПартии() Экспорт
	...
	ВозвращаемоеЗначение.Вставить("РазделУчета",       Неопределено);
...
Функция ЗагрузитьПартиюЗерна(ЭлементДанных, ПараметрыЗагрузкиПартии, СуществующийОбъект = Неопределено, ДанныеСопоставления = Неопределено) Экспорт
	...
	РазделУчета    = ПараметрыЗагрузкиПартии.РазделУчета;
	...
	ИначеЕсли РазделУчета = Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна Тогда
		...
		Если ЭлементДанных.GpbFromInProduct <> Неопределено Тогда
			ДобавитьПредшествующиеПартии(
				...
				Организация,
				Подразделение,
				Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна,
				ПараметрыОбмена,
				ПараметрыЗагрузкиПартии,
				СуществующийОбъект.Дата);
			ДобавитьПредшествующиеПартии(
				...
				Организация,
				Подразделение,
				Перечисления.РазделыУчетаЗЕРНО.Зерно,
				ПараметрыОбмена,
				ПараметрыЗагрузкиПартии,
				СуществующийОбъект.Дата);
		КонецЕсли;
...
Процедура ДобавитьПредшествующиеПартии(ПредшествующиеПартии, Источник, Организация, Подразделение, РазделУчета, ПараметрыОбмена, ПараметрыЗагрузкиПартии, ДатаРодительскойПартии)
...
Функция ИмяПоляИдентификаторПартии(РазделУчета) Экспорт
...
Функция ИмяПоляПартия(РазделУчета) Экспорт
