...
// Структура создания штрихкода упаковки.
// 
// Возвращаемое значение:
//  Структура - содержит реквизиты элемента справочника и вспомогательные служебные данные.

// Структура создания штрихкода упаковки.
// 
// Возвращаемое значение:
//  Структура - содержит реквизиты элемента справочника и вспомогательные служебные данные:
// * ВидПродукции - Неопределено, СправочникСсылка.КлючиВидовПродукцииИС, СправочникСсылка.ВидыПродукцииИС - 
// * ТипУпаковки - Неопределено, ПеречислениеСсылка.ТипыУпаковок - 
// * ТипШтрихкода - Неопределено, ПеречислениеСсылка.ТипыШтрихкодов - 
// * Штрихкод - Строка - 
// * ХешСумма - Строка - 
// * Номенклатура - Неопределено, ОпределяемыйТип.Номенклатура - 
// * Характеристика - Неопределено, ОпределяемыйТип.ХарактеристикаНоменклатуры - 
// * Упаковка - Неопределено, ОпределяемыйТип.Упаковка - 
// * Серия - Неопределено, ОпределяемыйТип.СерияНоменклатуры - 
// * Количество - Число - 
...
//    * ИмяКолонкиКоличествоПотребительскихУпаковок - Строка - Имя колонки "КоличествоПотребительскихУпаковок"
Функция ПараметрыСозданияШтрихкодовУпаковокПоДеревуМаркируемойПродукции(ДеревоМаркированнойПродукции, ПараметрыСканирования, ДополнительныеПараметры = Неопределено) Экспорт
...
//   Параметры - См. ПараметрыСозданияШтрихкодовУпаковокПоДеревуМаркируемойПродукции
// Возвращаемое значение:
//   Структура - результат обработки:
...
Функция РезультатСозданияШтрихкодовУпаковокПоДеревуМаркируемойПродукции(ДеревоМаркированнойПродукции,
	...
	МодульПроверкаИПодборПродукцииИСМПКлиентСервер = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ПроверкаИПодборПродукцииИСМПКлиентСервер");
...
Процедура ПродолжитьПослеПоискаШтрихкодаУпаковкиПоХешСумме(
	...
	Если РезультатПоиска <> Неопределено
		...
		ЭтоСтрокаДанныхЧастичногоВыбытия = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоСтрокаДанныхЧастичногоВыбытия(
			СтрокаДерева, Параметры.ПараметрыСканирования, Истина);
	...
	Иначе
		...
		ВидПродукции = Справочники.КлючиВидовПродукцииИС.ПустаяСсылка();
		...
		КонецЕсли;
		ДанныеНовойУпаковки.ВидПродукции = ВидПродукцииКлюча;
		
		Если СтрокаДерева.ТипУпаковки = Перечисления.ПрочиеЗоныПересчетаПродукцииИС.ГруппировкаОбъемноСортовогоУчетаПоGTIN Тогда
			ЗначениеШтрихкода = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ОСУПоШтрихкодуEAN(
				СтрокаДерева.GTIN, СтрокаДерева[Параметры.ИмяКолонкиКоличествоПотребительскихУпаковок]);
		Иначе
		...
		Если ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоСтрокаДанныхЧастичногоВыбытия(
				СтрокаДерева, Параметры.ПараметрыСканирования, Истина) Тогда
	...
	ИначеЕсли СтрокаДерева.ТипУпаковки = Перечисления.ПрочиеЗоныПересчетаПродукцииИС.ГруппировкаОбъемноСортовогоУчетаПоGTIN
	...
	ИначеЕсли ПараметрыПродолженияПоиска.ДеревоМаркированнойПродукции.ТипУпаковки = Перечисления.ПрочиеЗоныПересчетаПродукцииИС.ПачкиБезБлока
		Или ПараметрыПродолженияПоиска.ДеревоМаркированнойПродукции.ТипУпаковки = Перечисления.ПрочиеЗоныПересчетаПродукцииИС.БлокиБезКоробки
		Или ПараметрыПродолженияПоиска.ДеревоМаркированнойПродукции.ТипУпаковки = Перечисления.ПрочиеЗоныПересчетаПродукцииИС.ПродукцияБезУпаковки Тогда
		ЭтоСтрокаДереваВерхнегоУровня = Истина;
	КонецЕсли;
	...
	Если ЭтоСтрокаДереваВерхнегоУровня Тогда
		...
		Если ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЭтоСтрокаДанныхЧастичногоВыбытия(
				СтрокаДерева, Параметры.ПараметрыСканирования)
...
Процедура СоздатьШтрихкодыУпаковокПоДаннымДерева(ДеревоУпаковок, ПараметрыСканирования, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ПараметрыСозданияШтрихкодов = ПараметрыСозданияШтрихкодовУпаковокПоДеревуМаркируемойПродукции(ДеревоУпаковок, ПараметрыСканирования, ДополнительныеПараметры);
...
Функция ДанныеДляВычисленияХешСуммы(ШтрихкодУпаковкиТовараОбъект) Экспорт
	...
	Если ДанныеДляВычисления.Количество() = 0 Тогда
		...
		ВидыПродукцииОбъемноСортовогоУчета = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииОбъемноСортовогоУчета();
		Если ВидыПродукцииОбъемноСортовогоУчета.Количество() Тогда
			ПримечаниеКРезультатуРазбора = Неопределено;
			ДанныеРазбора = РазборКодаМаркировкиИССлужебный.РазобратьКодМаркировки(
				ШтрихкодУпаковкиТовараОбъект.ЗначениеШтрихкода, ВидыПродукцииОбъемноСортовогоУчета[0], ПримечаниеКРезультатуРазбора);
		Иначе
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(Команды) Экспорт
	...
	Если ПравоДоступа("Просмотр", Метаданные.Обработки.ПроверкаКодовМаркировкиИСМП) Тогда
		...
		ТипыШтрихкодовИСМП.Добавить(Перечисления.ТипыШтрихкодов.SSCC18);
		ТипыШтрихкодовИСМП.Добавить(Перечисления.ТипыШтрихкодов.SSCC20);
...
Функция ДанныеДляПечатиШтрихкодовУпаковокИСМП(
	...
	Пока ВыборкаКоды.Следующий() Цикл
		...
		Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ПечатьЭтикетокИСМПКлиентСервер");
		...
		Если МассивШаблоновКодовМаркировок <> Неопределено Тогда
			Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияИСМПКлиентСервер");
...
Функция РаспечататьНовыйКодУпаковки(ШтрихкодУпаковки, Шаблон) Экспорт
	...
	Если Шаблон = Неопределено Тогда
		Если ТипШтрихкодаУпаковки = Перечисления.ТипыШтрихкодов.SSCC
			Или ТипШтрихкодаУпаковки = Перечисления.ТипыШтрихкодов.SSCC18
			Или ТипШтрихкодаУпаковки = Перечисления.ТипыШтрихкодов.SSCC20 Тогда
...
Функция СформироватьПечатнуюФормуШтрихкодыУпаковок(МассивОбъектов, ОбъектыПечати)
	...
	Пока Выборка.Следующий() Цикл
		...
		РезультатРазбора = РазборКодаМаркировкиИССлужебный.РазобратьКодМаркировки(Выборка.Штрихкод, КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЕГАИСМаркируемые());
...
Функция СформироватьПечатнуюФормуЭтикеткаИСМП(МассивОбъектов, ПараметрыПечати)
	...
	Если ПроверятьДоступность = Неопределено Тогда
		Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияИСМП");
...
Функция ДопустимыеТипыШтрихкодовДляПечати()
	...
	МассивТипов.Добавить(Перечисления.ТипыШтрихкодов.SSCC18);
	МассивТипов.Добавить(Перечисления.ТипыШтрихкодов.SSCC20);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбработатьДанныеДляПереходаНаНовуюВерсиюЗаполнениеХешСуммаЗначенияШтрихкодаGS1");
	Методы.Вставить("ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсиюЗаполнениеХешСуммаЗначенияШтрихкодаGS1");
...
Процедура ЗаполнитьСвойствоХешСуммаЗначенияШтрихкода(ДанныеШтрихкодаУпаковок, ПараметрыШтрихкода = Неопределено) Экспорт
	...
	Если ПараметрыШтрихкода.ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC")
		Или ПараметрыШтрихкода.ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC18")
		Или ПараметрыШтрихкода.ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC20") Тогда
...
//   ПараметрыСозданияШтрихкодов - См. ПараметрыСозданияШтрихкодовУпаковокПоДеревуМаркируемойПродукции
...
Функция ПропускатьГруппировкуОбъемноСортовогоУчетаПоGTIN(СтрокаДерева)
	...
	Возврат СтрокаДерева.ТипУпаковки = Перечисления.ПрочиеЗоныПересчетаПродукцииИС.ГруппировкаОбъемноСортовогоУчетаПоGTIN
