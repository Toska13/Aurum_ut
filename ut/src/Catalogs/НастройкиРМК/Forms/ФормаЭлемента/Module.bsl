...
//++ Локализация
	...
	Если ПереводыСБПc2b.ПереводыСБПДоступны() Тогда
		ДействующиеДоговора = РегистрыСведений.НастройкиИнтеграцииСПлатежнымиСистемамиУТ.ДействующиеДоговораСНастройкамиИнтеграции(Перечисления.ВариантыНастройкиСБП.c2b);
		Для Каждого ДействующийДоговор Из ДействующиеДоговора Цикл
			...
			НоваяСтрокаНастроек.Магазин            = ДействующийДоговор.Магазин;
			НоваяСтрокаНастроек.КассовыеСсылки     = ПараметрыНастройкиПодключения.НастройкиСБПc2b.КассовыеСсылки;
			НоваяСтрокаНастроек.ПодключениеКассовойСсылки
				 = ПараметрыНастройкиПодключения.НастройкиСБПc2b.ПодключениеКассовойСсылки;
		КонецЦикла;
	КонецЕсли;
	
	ОпределитьДоступныеДоговорыПодключения();
	УстановитьРежимОтображенияСсылкиСБП();
	
//-- Локализация
	
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПоверитьКорректностьУказанияОборудованияВНастройкеРМК();
	КонецЕсли;
...
//++ Локализация
	...
	Если ИмяСобытия = "ЗавершитьСозданиеКассовойСсылкиСБП" Тогда
		...
		ВыбранныйМагазин = Параметр.Магазин;
		...
		Для Каждого СтрокаТЧ Из Объект.ПлатежныеСистемы Цикл
			Если СтрокаТЧ.ДоговорПодключенияКПлатежнойСистеме = Параметр.ДоговорПодключения
				И СтрокаТЧ.Магазин = Параметр.Магазин Тогда
				СтрокаПлатежнойСистемы = СтрокаТЧ;
				Прервать;
			ИначеЕсли ОрганизацииДоговоров.Получить(СтрокаТЧ.ДоговорПодключенияКПлатежнойСистеме) = ОрганизацияПоВыбранномуДоговору
				И СтрокаТЧ.Магазин = Параметр.Магазин Тогда
				
				Если ЗначениеЗаполнено(СтрокаТЧ.Магазин) Тогда
					ШаблонСообщения = НСтр("ru = 'По организации ""%1"", магазин ""%2"" - кассовая ссылка СБП указана (договор ""%3"")'");
		...
		СтрокаПлатежнойСистемы.Магазин = Параметр.Магазин;
...
&НаКлиенте
Процедура КассыККМПередУдалением(Элемент, Отказ)
	
	ОчиститьСообщения();
	ПроверитьВозможностьУдаленияПодключаемогоОборудованияПоТаблицеЭквайринговыеТерминалы(Элемент, Отказ);
...
&НаСервере
Процедура ПоверитьКорректностьУказанияОборудованияВНастройкеРМК()
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПодключаемоеОборудование КАК СправочникПодключаемоеОборудование
	|		ПО НастройкиРМКЭквайринговыеТерминалы.ПодключаемоеОборудование = СправочникПодключаемоеОборудование.Ссылка
	|ГДЕ
	|	НастройкиРМКЭквайринговыеТерминалы.Ссылка = &Ссылка
	|	И НЕ СправочникПодключаемоеОборудование.УстройствоИспользуется";
	...
	Пока Выборка.Следующий() Цикл
		...
		ШаблонСообщения = НСтр("ru = 'В настройках РМК некорректно настроен эквайринговый терминал:
		|В строке ""%1"", терминал ""%2"" - указано подключаемое оборудование ""%3"" со снятым флагом ""Устройстройство используется""'");
...
&НаКлиенте
Процедура ПроверитьВозможностьУдаленияПодключаемогоОборудованияПоТаблицеЭквайринговыеТерминалы(Элемент, Отказ)
	
	Если Не (Элемент = Элементы.КассыККМ Или Элемент = Элементы.Кассы)
		Или Элемент.ТекущиеДанные = Неопределено
		Или Не ЗначениеЗаполнено(Элемент.ТекущиеДанные.ПодключаемоеОборудование) Тогда
		Возврат;
	КонецЕсли;
	
	ПодключаемоеОборудование = Элемент.ТекущиеДанные.ПодключаемоеОборудование;
	НайденныеЭквайринговыеТерминалы = Объект.ЭквайринговыеТерминалы.НайтиСтроки(Новый Структура("ПодключаемоеОборудованиеККТ", ПодключаемоеОборудование));
	Если НайденныеЭквайринговыеТерминалы.Количество() > 0 Тогда
		
		Если Элемент = Элементы.КассыККМ Тогда
			ПроверяемаяТаблицаФормы = Объект.КассыККМ;
			СвязаннаяТаблицаФормы = Объект.Кассы;
		Иначе
			ПроверяемаяТаблицаФормы = Объект.Кассы;
			СвязаннаяТаблицаФормы = Объект.КассыККМ;
		КонецЕсли;
		
		СтрокиПроверяемойТаблицы = ПроверяемаяТаблицаФормы.НайтиСтроки(Новый Структура("ПодключаемоеОборудование", ПодключаемоеОборудование));
		СтрокиСвязаннойТаблицы = СвязаннаяТаблицаФормы.НайтиСтроки(Новый Структура("ПодключаемоеОборудование", ПодключаемоеОборудование));

		Если СтрокиСвязаннойТаблицы.Количество() = 0 И 
			СтрокиПроверяемойТаблицы.Количество() = 1 Тогда
			Отказ = Истина;
			
			СтрокиСообщения = Новый Массив;
			ШаблонСообщения = НСтр("ru = 'Есть настройка для эквайрингового терминала ""%1"", связанная с оборудованием ""%2"".'");
			СтрокиСообщения.Добавить(СтрШаблон(ШаблонСообщения, НайденныеЭквайринговыеТерминалы[0].ЭквайринговыйТерминал,
											ПодключаемоеОборудование));
			СтрокиСообщения.Добавить(НСтр("ru = 'Сначала на закладке ""Эквайринговые терминалы"" нужно удалить эту настройку.'"));
			ОбщегоНазначенияКлиент.СообщитьПользователю(СтрСоединить(СтрокиСообщения, Символ(32)));
		КонецЕсли;
...
Процедура УстановитьРежимОтображенияСсылкиСБП()
	...
	Элементы.ПлатежныеСистемыМагазин.Видимость = ЭтоКассоваяСсылка;
...
Процедура ОткрытьФормуКассовойСсылкиСБП(ВыбраннаяСтрока = Неопределено)
	...
	ПараметрыОткрытияФормы.Вставить("Магазин", Неопределено);
	ПараметрыОткрытияФормы.Вставить("ИдентификаторОплаты", Неопределено);
	ПараметрыОткрытияФормы.Вставить(
		"НастройкиПодключенияКПлатежнымСистемамДействующихДоговоров",
		НастройкиПодключенияКПлатежнымСистемамДействующихДоговоров);
	...
	Если ВыбраннаяСтрока <> Неопределено Тогда
		...
		ПараметрыОткрытияФормы.Магазин = ТекущаяСтрока.Магазин;
...
&НаСервере
Процедура ОпределитьДоступныеДоговорыПодключения()
	
	СкладыКассККМ = РозничныеПродажи.СкладыКассККМВИерархии(Объект.КассыККМ.Выгрузить().ВыгрузитьКолонку("КассаККМ"));
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	НастройкиИнтеграцииСПлатежнымиСистемамиУТ.Договор,
	|	НастройкиИнтеграцииСПлатежнымиСистемамиУТ.Договор.Наименование
	|ПОМЕСТИТЬ ПоддерживаемыеДоговорыПодключения
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПоддерживаемыеДоговорыПодключения.Договор КАК ДоговорПодключения,
	|	ПоддерживаемыеДоговорыПодключения.ДоговорНаименование КАК ДоговорНаименование
	|ИЗ
	|	ПоддерживаемыеДоговорыПодключения КАК ПоддерживаемыеДоговорыПодключения
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДоговорНаименование";
	...
	ДоступныеДоговорыПодключения.ЗагрузитьЗначения(Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ДоговорПодключения"));
