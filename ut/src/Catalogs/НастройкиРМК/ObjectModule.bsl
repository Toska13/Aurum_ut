...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого СтрокаТЧ Из ЭквайринговыеТерминалы Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧ.ПодключаемоеОборудованиеККТ) Тогда
			
			ТекстОшибки = НСтр("ru='Не заполнено поле ""ККТ""'");
			АдресОшибки = НСтр("ru='в строке %НомерСтроки% списка ""Эквайринговые терминалы""'");
			АдресОшибки = СтрЗаменить(АдресОшибки, "%НомерСтроки%", СтрокаТЧ.НомерСтроки);
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ТекстОшибки + " " + АдресОшибки,
				ЭтотОбъект,
				ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ЭквайринговыеТерминалы", СтрокаТЧ.НомерСтроки, "ПодключаемоеОборудованиеККТ"),
				,
				Отказ);
				
		Иначе
			НайденныеКассы = Кассы.НайтиСтроки(Новый Структура("ПодключаемоеОборудование", СтрокаТЧ.ПодключаемоеОборудованиеККТ));	
			НайденныеКассыККМ = КассыККМ.НайтиСтроки(Новый Структура("ПодключаемоеОборудование", СтрокаТЧ.ПодключаемоеОборудованиеККТ));	
			Если НайденныеКассыККМ.Количество() = 0
				И НайденныеКассы.Количество() = 0 Тогда
				
				ТекстОшибки = НСтр("ru='Настройте кассу ККМ или кассу предприятия, связанную с ККТ эквайрингового терминала'");
				АдресОшибки = НСтр("ru='в строке %НомерСтроки% списка ""Эквайринговые терминалы""'");
				АдресОшибки = СтрЗаменить(АдресОшибки, "%НомерСтроки%", СтрокаТЧ.НомерСтроки);
