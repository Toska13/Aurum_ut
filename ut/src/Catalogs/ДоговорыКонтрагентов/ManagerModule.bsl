...
Функция ПолучитьБлокируемыеРеквизитыОбъекта() Экспорт
	...
	Результат.Добавить("ВариантОформленияЗакупок; ИспользоватьОформлениеДокументовРаздельнойЗакупкиТоваров");
	Результат.Добавить("ОформлениеПоступления");
	Результат.Добавить("ПриемкаТоваров");
	Результат.Добавить("ПриемкаУслуг");
	Результат.Добавить("ВариантОформленияДокументовПоступленияПоУслугам; ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслуг");
	Результат.Добавить("ЕжемесячноеПоступлениеУслуг");
    Результат.Добавить("ОформлениеПоступленияУслуг");
	Результат.Добавить("ПорядокОтраженияНеотфактурованныхПоставокНУ");
	...
	Результат.Добавить("КачествоТоваровГарантированоПоставщиком");
...
Функция ПараметрыВзаиморасчеты(ДанныеЗаполнения = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ЗначениеСсылочногоТипа(ДанныеЗаполнения) Тогда
		...
		ПорядокРасчетов       = СтруктураДанныеЗаполнения.ПорядокРасчетов;
	...
	ВлияющиеРеквизиты = "ТипДоговора,ДоговорСКомиссионером,КомиссионныеПродажи25,ПорядокРасчетов";
	...
	СтруктураПараметров.МожетЯвлятьсяОбъектомРасчетов    = ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов
...
Процедура ЗадатьОписаниеЗаполняемыхРеквизитовПоСтатистике(ОписаниеРеквизитов) Экспорт
	...
	Параметры = ЗаполнениеОбъектовПоСтатистике.ПараметрыЗаполняемыхРеквизитов();
	Параметры.РазрезыСбораСтатистики.ИспользоватьВсегда = "Организация, ОплатаВВалюте";
	...
	Параметры = ЗаполнениеОбъектовПоСтатистике.ПараметрыЗаполняемыхРеквизитов();
	Параметры.РазрезыСбораСтатистики.ИспользоватьВсегда = "Контрагент, ОплатаВВалюте";
...
// Возвращает признак того, что указанный тип договора используется для расчетов с клиентом.
//
// Параметры:
//  ТипДоговора - ПеречислениеСсылка.ТипыДоговоров - проверяемый тип договора.
//
// Возвращаемое значение:
//  Булево - Истина, если это договор с клиентом.
//
Функция ЭтоДоговорСКлиентом(ТипДоговора) Экспорт
	
	ЭтоДоговорСКлиентом = Ложь;
	
	Если ТипДоговора = Перечисления.ТипыДоговоров.РеализацияЧерезКомиссионера
		Или ВзаиморасчетыКлиентСервер.ТипыДоговоровСКлиентом().НайтиПоЗначению(ТипДоговора) <> Неопределено Тогда
		ЭтоДоговорСКлиентом = Истина;
	КонецЕсли;
	
	Возврат ЭтоДоговорСКлиентом;
	
КонецФункции

// Проверяет флаги партнера "Клиент" и "Поставщик" в зависимости от хозяйственной операции и типа договора.
//
// Параметры:
//  Партнер - СправочникСсылка.Партнеры - Партнер, для которого проверяются флаги
//  ХозяйственнаяОперация - ПеречислениеСсылка.ХозяйственныеОперации - Тип хозяйственной операции
//  ЭтоНовыйДоговор - Булево - Признак нового договора
//  ВыводитьСообщенияПользователю - Булево - Флаг вывода сообщений пользователю
//  ФлагиПартнера - Структура - Флаги партнера "Клиент" и "Поставщик" (по умолчанию Неопределено)
//  ТекстСообщения - Строка - Текст сообщения для пользователя (по умолчанию "")
//
// Возвращаемое значение:
//  Булево - Результат проверки (Истина, если проверка пройдена успешно)
Функция ПроверитьФлагиПартнераКлиентПоставщик(Партнер,
											ХозяйственнаяОперация,
											ЭтоНовыйДоговор,
											ВыводитьСообщенияПользователю,
											ФлагиПартнера = Неопределено,
	...
	Если ЗначениеЗаполнено(Партнер) Тогда
		...
		ОперацииЗакупки = ЗакупкиСервер.ХозяйственныеОперацииПоОсновной(
			Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика);
		ОперацииИмпорта = ЗакупкиСервер.ХозяйственныеОперацииПоОсновной(Перечисления.ХозяйственныеОперации.ЗакупкаПоИмпорту);
		ОперацииВСтранахЕАЭС = ЗакупкиСервер.ХозяйственныеОперацииПоОсновной(
			Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС);
		ОперацииПроизводстваУПереработчика2_5 = ЗакупкиСервер.ХозяйственныеОперацииПоОсновной(
			Перечисления.ХозяйственныеОперации.ПроизводствоУПереработчика2_5);
			
		Если ФлагиПартнера = Неопределено Тогда
			ФлагиПартнера = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Партнер, "Клиент, Поставщик");
		КонецЕсли;
		
		Если Не ФлагиПартнера.Клиент = Истина И Не ФлагиПартнера.Поставщик = Истина Тогда
			Отказ = Истина;
		...
		ИначеЕсли Не ФлагиПартнера.Клиент = Истина 
		...
		ИначеЕсли Не ФлагиПартнера.Поставщик = Истина
			И Не (ОперацииЗакупки.Найти(ХозяйственнаяОперация) = Неопределено
			Или Не ОперацииИмпорта.Найти(ХозяйственнаяОперация) = Неопределено
			Или Не ОперацииВСтранахЕАЭС.Найти(ХозяйственнаяОперация) = Неопределено
	...
	Если Отказ И ВыводитьСообщенияПользователю Тогда
...
// 
// Параметры:
//  ОбъектИлиСсылка - СправочникОбъект.ДоговорыКонтрагентов, СправочникСсылка.ДоговорыКонтрагентов - договор.
...
// Возвращает массив регулярных платежей.
//
// Параметры:
//  ДатаНачала - Дата.
//  ДатаОкончания - Дата.
...
//  Массив из Дата.
//
Функция ДатыРегулярныхПлатежей(Знач ДатаНачала, Знач ДатаОкончания, Знач Периодичность, Знач День) Экспорт
	
	Периоды = Новый ТаблицаЗначений;
	Периоды.Колонки.Добавить("НачалоПериода", Новый ОписаниеТипов("Дата"));
	Периоды.Колонки.Добавить("КонецПериода", Новый ОписаниеТипов("Дата"));
	Периоды.Колонки.Добавить("ДатаПлатежа", Новый ОписаниеТипов("Дата"));
	
	ДатаНачалаПериода = ПланированиеКлиентСерверПовтИсп.РассчитатьДатуНачалаПериода(ДатаНачала, Периодичность);
	ДатаОкончанияПериода = ПланированиеКлиентСерверПовтИсп.РассчитатьДатуОкончанияПериода(ДатаНачалаПериода, 
		Периодичность);
	
	Пока ДатаНачалаПериода < ДатаОкончания Цикл
		
		Стр = Периоды.Добавить();
		Стр.НачалоПериода = ДатаНачалаПериода;
		Стр.КонецПериода = ДатаОкончанияПериода;
		Стр.ДатаПлатежа = Стр.НачалоПериода + (День - 1) * 86400;
		
		Если Стр.ДатаПлатежа > Стр.КонецПериода Тогда
			Стр.ДатаПлатежа = Стр.КонецПериода;
		КонецЕсли;
		
		Если Стр.ДатаПлатежа < ДатаНачала Тогда
			Периоды.Удалить(Стр);
		КонецЕсли;
	...
	Возврат Периоды.ВыгрузитьКолонку("ДатаПлатежа");
...
Процедура ПриОпределенииДоступныхВидовОповещенийПользователей(ОписаниеВидовОповещений) Экспорт
	ГруппаВидаОповещенийДоговоры = ОповещенияПользователейОСобытиях.ГруппаВидаОповещений("ДоговорыКонтрагентов");
	
	НовыйВидОповещений = ОповещенияПользователейОСобытиях.ДобавитьОписаниеВидаОповещенийПользователей(ОписаниеВидовОповещений);
	НовыйВидОповещений.ГруппаВидаОповещений = ГруппаВидаОповещенийДоговоры;
	НовыйВидОповещений.Идентификатор = "fc0af7b5-12da-43d8-a416-8c713dd5d4aa";
	НовыйВидОповещений.Наименование = НСтр("ru = 'Приближение даты окончания действия договора'", ОбщегоНазначения.КодОсновногоЯзыка());
	НовыйВидОповещений.ТипОповещения = Перечисления.ТипыОповещенийПользователей.Неоперативное;
	НовыйВидОповещений.ИмяШаблонаСКД = Метаданные.Справочники.ДоговорыКонтрагентов.Макеты.Оповещения_ОкончаниеСрокаДействияПриближается.ПолноеИмя();
	НовыйВидОповещений.ШаблонТекстаОповещения = "ru = 'Заканчивается срок действия договора [ДатаОкончанияДействия].'"; // @NStr-1
	НовыйВидОповещений.ТекстСгруппированногоОповещения = "ru = 'Заканчивается срок действия договора.'"; // @NStr-1
	
	НовыйВидОповещений = ОповещенияПользователейОСобытиях.ДобавитьОписаниеВидаОповещенийПользователей(ОписаниеВидовОповещений);
	НовыйВидОповещений.ГруппаВидаОповещений = ГруппаВидаОповещенийДоговоры;
	НовыйВидОповещений.Идентификатор = "88ea3524-fcef-48e5-907f-b339b1886347";
	НовыйВидОповещений.Наименование = НСтр("ru = 'Окончание срока действия договора'", ОбщегоНазначения.КодОсновногоЯзыка());
	НовыйВидОповещений.ТипОповещения = Перечисления.ТипыОповещенийПользователей.Неоперативное;
	НовыйВидОповещений.ИмяШаблонаСКД = Метаданные.Справочники.ДоговорыКонтрагентов.Макеты.Оповещения_ОкончаниеСрокаДействияПросрочено.ПолноеИмя();
	НовыйВидОповещений.ШаблонТекстаОповещения = "ru = 'Закончился срок действия договора [ДатаОкончанияДействия].'"; // @NStr-1
	НовыйВидОповещений.ТекстСгруппированногоОповещения = "ru = 'Закончился срок действия договора.'"; // @NStr-1
КонецПроцедуры
...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	ОтборыИспользуются = Параметры.Свойство("Отбор");
	Если ОтборыИспользуются Тогда
		// Копируем значение из параметров для возможности модификации:
		Отбор = ОбщегоНазначения.СкопироватьРекурсивно(Параметры.Отбор);
	Иначе
	...
	Если ОтборыИспользуются
		И Отбор.Свойство("НазначениеОбеспечения") Тогда
		
		// Используем направление деятельности из назначения для отбора договоров с порядком расчетов "ПоДоговорамКонтрагентов":
		НазначениеОбеспечения = Отбор.НазначениеОбеспечения;
		Отбор.Удалить("НазначениеОбеспечения"); // Не используем напрямую в отборе запроса.
		
		Если Не Отбор.Свойство("НаправлениеДеятельности")
			И ЗначениеЗаполнено(НазначениеОбеспечения) Тогда
				
			НаправленияДеятельности = Справочники.Назначения.НаправленияДеятельностиНазначений(
				ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(НазначениеОбеспечения));
			НаправлениеДеятельности = НаправленияДеятельности.Получить(НазначениеОбеспечения);
			
			СтандартнаяОбработка = Ложь;
			Запрос.УстановитьПараметр("НаправлениеДеятельностиНазначения", НаправлениеДеятельности);
			ТекстУсловий = ТекстУсловий + "
			|	И (Договоры.НаправлениеДеятельности В (&НаправлениеДеятельностиНазначения, 
			|											ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка))
			|		ИЛИ Договоры.ПорядокРасчетов <> ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоДоговорамКонтрагентов))"; // query-part
	...
	Если ПроверятьСоглашение И Параметры.Свойство("Соглашение") Тогда
		...
		Если ЗначениеЗаполнено(Параметры.Соглашение)
			И ТипЗнч(Параметры.Соглашение) = Тип("СправочникСсылка.СоглашенияСПоставщиками")
			И (Не ОтборыИспользуются
				Или Не Отбор.Свойство("ХозяйственнаяОперация")) Тогда
				
				// Нет отбора по хозяйственной операции, но указано соглашение.
				// Берем в качестве отбора хозяйственную операцию из соглашения для согласованности данных:
				Отбор.Вставить(
					"ХозяйственнаяОперация",
					Справочники.СоглашенияСПоставщиками.ХозяйственнаяОперацияСоглашения(Параметры.Соглашение));
	...
	Если ОтборыИспользуются Тогда
		...
		Для Каждого ТекущийОтбор Из Отбор Цикл
			...
			ИначеЕсли ТекущийОтбор.Ключ = "ХозяйственнаяОперация" Тогда
				...
				ИначеЕсли ТекущийОтбор.Значение = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки  
						Или ТекущийОтбор.Значение = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС Тогда
						
					Если ТекущийОтбор.Значение = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки Тогда 
						ХозяйственныеОперации = ЗакупкиСервер.ХозяйственныеОперацииПоОсновной(Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика);
					ИначеЕсли ТекущийОтбор.Значение = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС Тогда 
						ХозяйственныеОперации = ЗакупкиСервер.ХозяйственныеОперацииПоОсновной(Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС);
					КонецЕсли;
					
					ТекстУсловий = ТекстУсловий + "
					|	И Договоры.ХозяйственнаяОперация В (&ХозяйственныеОперации)";
					
					Запрос.УстановитьПараметр("ХозяйственныеОперации", ХозяйственныеОперации);
					
					СписокВариантовПоУслугам = Новый СписокЗначений;
                    СписокВариантовПоУслугам.Добавить(Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется);
					СписокВариантовПоУслугам.Добавить(Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется);
					
					ТекстУсловий = ТекстУсловий + "
					|	И (Договоры.ВариантОформленияДокументовПоступленияПоУслугам В (&СписокВариантовПоУслугам)
					|		ИЛИ Договоры.ВариантОформленияЗакупок = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваровИУслуг))"; 
					Запрос.УстановитьПараметр("СписокВариантовПоУслугам", СписокВариантовПоУслугам);
			...
			ИначеЕсли ТекущийОтбор.Ключ = "ПодходитДляПодбораВДоговорЗаказчика" Тогда
				
				ТекстУсловий = ТекстУсловий + "
					|	И ВЫБОР
					|		КОГДА Договоры.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СПокупателем)
					|			И Договоры.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиенту)
					|			ИЛИ Договоры.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СДавальцем2_5)
					|			ТОГДА ИСТИНА
					|		ИНАЧЕ ЛОЖЬ
					|	КОНЕЦ = &ПодходитДляПодбораВДоговорЗаказчика";
				
				Запрос.УстановитьПараметр("ПодходитДляПодбораВДоговорЗаказчика", ТекущийОтбор.Значение);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("eadc8581-e029-11ed-8803-782b4641589c");
	...
	СписокОписаний.Добавить(НСтр("ru = 'Обновление справочника ""Договоры с контрагентами"":'"));
	СписокОписаний.Добавить(НСтр("ru = 'Заполняет реквизит ""Вариант определения графика исполнения в договоре"" для договоров контрагентов.'"));
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		= ЗНАЧЕНИЕ(Перечисление.ВариантыОпределенияГрафикаИсполненияВДоговоре.ПустаяСсылка)
	|
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ТекстыЗапроса.Добавить(
		...
		|	ДоговорыКонтрагентов.ВариантОпределенияГрафикаИсполненияВДоговоре КАК ВариантОпределенияГрафикаИсполненияВДоговоре
		|ИЗ
		|	Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
		|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если Не ЗначениеЗаполнено(Выборка.ВариантОпределенияГрафикаИсполненияВДоговоре) Тогда
				СправочникОбъект.ВариантОпределенияГрафикаИсполненияВДоговоре = ?(Выборка.ЗаданГрафикИсполнения,
					Перечисления.ВариантыОпределенияГрафикаИсполненияВДоговоре.Договор,
					Перечисления.ВариантыОпределенияГрафикаИсполненияВДоговоре.ЗаказыНакладные);
			КонецЕсли;
		...
		Исключение
			...
			Если ПричинаИсключения = "ПлохиеДанные" Тогда
				ПроблемаСДанными = ПроблемаСДанными(Выборка.Ссылка, ИнформацияОбОшибке());
			...
			ИначеЕсли ПричинаИсключения <> "Блокировка" Тогда
				...
				ВызватьИсключение НСтр("ru = 'Ошибка блокировки при записи элемента справочника ""Договоры контрагентов"".'");
			КонецЕсли;
		КонецПопытки;
	КонецЦикла;
...
Функция ПроблемаСДанными(СсылкаНаОбъект, ИнформацияОбОшибке)
	Возврат Новый Структура("СсылкаНаОбъект, ИнформацияОбОшибке", СсылкаНаОбъект, ИнформацияОбОшибке);
КонецФункции
