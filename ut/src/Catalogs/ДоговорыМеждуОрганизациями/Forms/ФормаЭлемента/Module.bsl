...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоговорыМеждуОрганизациямиЛокализация.ПриСозданииНаСервереФормаЭлемента(ЭтотОбъект, Отказ , СтандартнаяОбработка);
...
Процедура ПриОткрытии(Отказ)
	...
	ДоговорыМеждуОрганизациямиЛокализацияКлиент.ПриОткрытии(Отказ, ЭтотОбъект);
	// СтандартныеПодсистемы.РаботаСФайлами
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	// Конец СтандартныеПодсистемы.Свойства
	ДоговорыМеждуОрганизациямиЛокализация.ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты, ЭтотОбъект);
	
КонецПроцедуры

&НаСервере
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	ДоговорыМеждуОрганизациямиЛокализация.ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи, ЭтотОбъект);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ДоговорыМеждуОрганизациямиЛокализация.ПослеЗаписиНаСервереФормаЭлемента(ЭтотОбъект, ТекущийОбъект, ПараметрыЗаписи);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	КонецЕсли;
	ДоговорыМеждуОрганизациямиЛокализацияКлиент.ОбработкаОповещения(ИмяСобытия, Параметр, Источник, ЭтотОбъект);
...
Процедура ПриОкончанииИзмененияРеквизитаЛокализацииНаСервере(ИмяЭлемента, ПараметрыОбработки)
	...
	ДоговорыМеждуОрганизациямиЛокализация.ПриОкончанииИзмененияРеквизита(ИмяЭлемента, ЭтотОбъект, ПараметрыОбработки);
...
Процедура РежимРаздельногоУчетаНДСПриИзменении(Элемент)
	...
	Объект.ЗакупкаПодДеятельностьОпределяетсяВДокументе = РежимРаздельногоУчетаНДС = 1;
...
Процедура ПорядокРасчетовПриИзменении(Элемент)
	...
	ПорядокРасчетовПриИзмененииНаСервере();
...
Процедура Подключаемый_ВыполнитьКомандуЛокализации(Команда)
	...
	ДоговорыМеждуОрганизациямиЛокализацияКлиент.ВыполнитьКомандуЛокализации(Команда, ЭтотОбъект);
...
Процедура ПродолжитьВыполнениеКомандыЛокализацииНаСервере(ИмяКоманды, ПараметрыОбработки)
	...
	ДоговорыМеждуОрганизациямиЛокализация.ВыполнитьКомандуЛокализации(ЭтотОбъект, ИмяКоманды, ПараметрыОбработки);
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ДоговорыМеждуОрганизациямиЛокализация.ПриЧтенииСозданииНаСервере(ЭтотОбъект);
КонецПроцедуры
...
Процедура НаправлениеДеятельностиПриИзмененииСервер()
	...
	Если ЗначенияРеквизитовНазначения.УчетЗатрат
		...
		РежимРаздельногоУчетаНДС = ?(Объект.ЗакупкаПодДеятельностьОпределяетсяВДокументе, 1, 0);
...
&НаСервере
Процедура ПорядокРасчетовПриИзмененииНаСервере()
	
	ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтотОбъект, "ПорядокРасчетов")
...
Процедура ОрганизацияПолучательПриИзмененииСервер()
	...
	ЗаполнениеОбъектовПоСтатистике.ЗаполнитьПодчиненныеРеквизитыОбъекта(Объект, "ОрганизацияПолучатель", , Истина);
...
Процедура НалогообложениеНДСПриИзмененииСервер()
	...
	Если Не УчетНДСУПКлиентСервер.ОтображатьНДСВИтогахДокументаПродажи(Объект.НалогообложениеНДС) 
...
Процедура ОбработатьБанковскиеСчета()
	...
	Если Не БанковскийСчетСоответствуетПорядкуОплаты(Объект.БанковскийСчетПолучателя) Тогда
...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеПриИзменении(Элемент)
	...
	ИначеЕсли Элемент.Имя = Элементы.ДатаНачалаДействия.Имя Тогда
...
Процедура ЗаполнитьБанковскийСчетОрганизацииПоУмолчанию(ВладелецСчета, ОплатаВВалюте, СчетКЗаполнению, ВалютаРегламентированногоУчета)
	...
	ПараметрыСчета = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации(ВладелецСчета);
	ПараметрыСчета.ИсключитьСчетаВВалюте = ОплатаВВалюте;
	ПараметрыСчета.Валюта = ВалютаРегламентированногоУчета;
	
	СчетКЗаполнению = Справочники.БанковскиеСчетаОрганизаций.ПолучитьБанковскийСчетПоУмолчанию(ПараметрыСчета);
...
Процедура ОбновитьЗаголовокФормы()
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Заголовок = ПредставлениеТипа + " (" + НСтр("ru='создание'") + ")";
	Иначе
		Заголовок = Объект.Наименование + " (" + ПредставлениеТипа + ")";
	КонецЕсли;
...
Процедура ЗаполнитьНалогообложениеНДС()
	...
	Если ЗначениеЗаполнено(Объект.НалогообложениеНДС)
		И Объект.НалогообложениеНДСОпределяетсяВДокументе Тогда
...
&НаСервере
Процедура АктуализироватьЗакупкуПодДеятельность(ЗаполнитьВидДеятельности = Истина)
	
	ПараметрыЗаполнения = Справочники.ДоговорыМеждуОрганизациями.ПараметрыЗаполненияВидаДеятельностиНДС(Объект);
	
	Если ЗначениеЗаполнено(Объект.ЗакупкаПодДеятельность)
		И Объект.ЗакупкаПодДеятельностьОпределяетсяВДокументе Тогда
		
		Объект.ЗакупкаПодДеятельность = Неопределено;
		
	ИначеЕсли ЗаполнитьВидДеятельности
	...
	ВидимостьРежимРаздельногоУчетаНДС = Элементы.ЗакупкаПодДеятельность.Видимость;
	
	Элементы.РежимРаздельногоУчетаНДСОпределяетсяЗначением.Видимость = ВидимостьРежимРаздельногоУчетаНДС;
	Элементы.РежимРаздельногоУчетаНДС.Видимость = ВидимостьРежимРаздельногоУчетаНДС;
...
Процедура УстановитьТекстПодсказкиПорядокЗачетаДокументов()
	...
	Если Объект.ПорядокЗачетаОплатНакладных = ПредопределенноеЗначение("Перечисление.ПорядкиЗачетаОплатНакладных.ПоДатеВидуДокумента") Тогда
		...
		|Документы в пределах дня будут зачитываться по виду: сначала переносы авансов и задолженности, накладные, возвраты товаров и корректировки накладных, оплаты, возвраты ДС и пр.'");
