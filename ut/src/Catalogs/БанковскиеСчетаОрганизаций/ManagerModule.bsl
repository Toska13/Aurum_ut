...
// Функция определяет банковский счет организации по заданным параметрам.
//
// Возвращает банковский счет организации, если найден один банковский счет.
// Возвращает пустую ссылку, если банковский счет не найден или счетов больше одного.
//
// Параметры:
//  СтруктураПараметров - см. ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации.
//
// Возвращаемое значение:
//	СправочникСсылка.БанковскиеСчетаОрганизаций - Найденный банковский счет организации.
//
Функция ПолучитьБанковскийСчетПоУмолчанию(СтруктураПараметров) Экспорт
	
	Организация = СтруктураПараметров.Организация;
	Валюта = СтруктураПараметров.Валюта;
	НаправлениеДеятельности = СтруктураПараметров.НаправлениеДеятельности;
	КорреспондирующийСчет = СтруктураПараметров.КорреспондирующийСчет;
	ИсключитьСчетаВПереданнойВалюте = СтруктураПараметров.ИсключитьСчетаВВалюте;
	
	КорреспонденцияСоСчетомЦифровогоРубля = Неопределено;
	
	//++ Локализация
	Если ЗначениеЗаполнено(КорреспондирующийСчет) Тогда
		КорреспонденцияСоСчетомЦифровогоРубля = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КорреспондирующийСчет, "СчетЦифровогоРубля");
	КонецЕсли;
	//-- Локализация
	...
	|	БанковскиеСчетаОрганизаций.Ссылка = &ОсновнойБанковскийСчет КАК Основной
	|ИЗ
	...
	|	И (БанковскиеСчетаОрганизаций.СчетЦифровогоРубля = &КорреспонденцияСоСчетомЦифровогоРубля
	|		ИЛИ &КорреспонденцияСоСчетомЦифровогоРубля = Неопределено)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Основной УБЫВ
	|;
	...
	Если Не ИсключитьСчетаВПереданнойВалюте Тогда
		УсловиеПоВалюте = "(БанковскиеСчетаОрганизаций.ВалютаДенежныхСредств = &Валюта
	...
	Запрос = Новый Запрос;
	Запрос.Текст = СтрЗаменить(ТекстЗапроса, "&УсловиеПоВалюте", УсловиеПоВалюте);
	
	Если ЗначениеЗаполнено(Организация) Тогда
		ОсновнойБанковскийСчет = Справочники.Организации.ПолучитьОсновнойБанковскийСчет(Организация);
	Иначе
	...
	Запрос.УстановитьПараметр("КорреспонденцияСоСчетомЦифровогоРубля", 
		?(ЗначениеЗаполнено(КорреспонденцияСоСчетомЦифровогоРубля), КорреспонденцияСоСчетомЦифровогоРубля, Неопределено));
	Запрос.УстановитьПараметр("ОсновнойБанковскийСчет", ОсновнойБанковскийСчет);
	
	Результат = Запрос.ВыполнитьПакет();
	ВыборкаПоНаправлению  = Результат[0].Выбрать();
	ВыборкаБезНаправления = Результат[1].Выбрать();
	
	Если ВыборкаПоНаправлению.Следующий() И (ВыборкаПоНаправлению.Количество() = 1 Или ВыборкаПоНаправлению.Основной) Тогда
		
		БанковскийСчетОрганизации = ВыборкаПоНаправлению.БанковскийСчетОрганизации;
		
	ИначеЕсли ВыборкаБезНаправления.Следующий() И (ВыборкаБезНаправления.Количество() = 1 Или ВыборкаБезНаправления.Основной) Тогда
...
//		* СчетЦифровогоРубля - Булево - признак счета цифрового рубля.
//		* ИдентификаторСчетаЦифровогоРубля - Строка - идентификатор счета цифрового рубля.
...
Функция ПолучитьРеквизитыБанковскогоСчетаОрганизации(БанковскийСчет) Экспорт
	...
	|	БанковскиеСчета.СчетЦифровогоРубля                КАК СчетЦифровогоРубля,
	|	БанковскиеСчета.ИдентификаторСчетаЦифровогоРубля  КАК ИдентификаторСчетаЦифровогоРубля,
	...
	|		ПО Банки.КодСтраны = СтраныМира.Код
	|ГДЕ
	...
	РеквизитыСчета.Вставить("СчетЦифровогоРубля", Ложь);
	РеквизитыСчета.Вставить("ИдентификаторСчетаЦифровогоРубля", "");
...
//  см. ДенежныеСредстваСервер.ИнициализироватьНастройкиПолейФормы.
//
Функция НастройкиПолейФормыСчетаЦифровогоРубля() Экспорт
...
// Устарела. Следует использовать ПолучитьБанковскийСчетПоУмолчанию.
...
//	КорреспондирующийСчет - СправочникСсылка.БанковскиеСчетаОрганизаций - счет, с которым корреспондирует получаемый счет.
//	ИсключитьСчетаВВалюте - Булево - если Истина, будут исключены счета в валюте, переданной в параметре Валюта.
//
// Возвращаемое значение:
//	СправочникСсылка.БанковскиеСчетаОрганизаций - Найденный банковский счет организации.
//
Функция ПолучитьБанковскийСчетОрганизацииПоУмолчанию(Организация, Валюта = Неопределено,
	НаправлениеДеятельности = Неопределено, КорреспондирующийСчет = Неопределено) Экспорт
	...
	СтруктураПараметров.КорреспондирующийСчет = КорреспондирующийСчет;
	
	Возврат ПолучитьБанковскийСчетПоУмолчанию(СтруктураПараметров);
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Иначе
		...
		Если Не ВидФормы = "ФормаЭлемента"
		...
		СтандартнаяОбработка = Ложь;
		Если Значение.СчетЦифровогоРубля Тогда
			ВыбраннаяФорма = "ФормаЭлементаСчетЦифровогоРубля";
		Иначе
			ВыбраннаяФорма = "ФормаЭлемента";
		КонецЕсли;
...
Процедура ЗаполнитьМеждународныеРеквизиты(ИспользуетсяБанкДляРасчетов, Макет, Выборка, ТабличныйДокумент)
	...
	Иначе
		Если Выборка.СчетЦифровогоРубля Тогда
			Область = Макет.ПолучитьОбласть("МеждународныеРеквизиты_СчетЦифровогоРубля");
		Иначе
...
Процедура ЗаполнитьНациональныеРеквизиты(ИспользуетсяБанкДляРасчетов, Макет, Выборка, ТабличныйДокумент)
	...
	Иначе
		Если Выборка.СчетЦифровогоРубля Тогда
			Область = Макет.ПолучитьОбласть("НациональныеРеквизиты_СчетЦифровогоРубля");
		Иначе
...
Функция ТекстЗапросаПечати()
	...
	|	БанковскиеСчетаОрганизаций.СчетЦифровогоРубля КАК СчетЦифровогоРубля,
	|	БанковскиеСчетаОрганизаций.ИдентификаторСчетаЦифровогоРубля КАК ИдентификаторСчетаЦифровогоРубля,
	|	ЕСТЬNULL(Банки.Наименование, """") КАК БанкНаименование,
	|	ЕСТЬNULL(Банки.МеждународноеНаименование, """") КАК БанкМеждународноеНаименование,
	|	ЕСТЬNULL(Банки.Код, """") КАК БанкКод,
	...
	|	ЕСТЬNULL(Банки.АдресМеждународный, """") КАК БанкАдресМеждународный,
	|	ЕСТЬNULL(Банки.СВИФТБИК, """") КАК БанкСВИФТБИК,
	...
	|	ЕСТЬNULL(БанкиДляРасчетов.МеждународноеНаименование, """") КАК БанкДляРасчетовМеждународноеНаименование,
	|	ЕСТЬNULL(БанкиДляРасчетов.АдресМеждународный, """") КАК БанкДляРасчетовАдресМеждународный,
	|	ЕСТЬNULL(СтраныБанков.Ссылка, ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)) КАК БанкСтрана,
	...
	|	ЕСТЬNULL(БанкиДляРасчетов.Наименование, """") КАК БанкДляРасчетовНаименование,
	|	ЕСТЬNULL(БанкиДляРасчетов.Код, """") КАК БанкДляРасчетовКод,
	|	БанковскиеСчетаОрганизаций.ГородБанкаМеждународный КАК ГородБанкаМеждународный,
	|	ЕСТЬNULL(Банки.ГородМеждународный, """") КАК БанкГородМеждународный,
	|	ЕСТЬNULL(БанкиДляРасчетов.СВИФТБИК, """") КАК БанкДляРасчетовСВИФТБИК,
	...
	|	ЕСТЬNULL(СтраныБанков.МеждународноеНаименование, """") КАК БанкСтранаМеждународноеНаименование,
	|	ЕСТЬNULL(СтраныБанковДляРасчетов.МеждународноеНаименование, """") КАК БанкДляРасчетовСтранаМеждународноеНаименование,
	...
	|	ЕСТЬNULL(БанкиДляРасчетов.ГородМеждународный, """") КАК БанкДляРасчетовГородМеждународный,
	...
	|		ПО Банки.КодСтраны = СтраныБанков.Код
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлассификаторБанков КАК БанкиДляРасчетов
	|		ПО БанковскиеСчетаОрганизаций.БанкДляРасчетов = БанкиДляРасчетов.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтраныМира КАК СтраныБанковДляРасчетов
	|		ПО БанкиДляРасчетов.КодСтраны = СтраныБанковДляРасчетов.Код
	|ГДЕ
