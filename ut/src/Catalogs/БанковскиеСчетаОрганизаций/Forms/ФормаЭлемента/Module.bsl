...
Процедура ПередЗаписью(Отказ)
	...
	Если ИспользоватьТекстКорреспондентаБюджет Тогда
		Объект.ТекстКорреспондентаБюджет = ТекстКорреспондентаБюджет;
	Иначе
		Объект.ТекстКорреспондентаБюджет = "";
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент = УсловноеОформление.Элементы.Добавить();
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТекстКорреспондентаБюджет.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ИспользоватьТекстКорреспондентаБюджет");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Ложь;
...
Функция РеквизитыФормы(Форма)
	...
	РеквизитыФормы.Вставить("ИспользоватьТекстКорреспондентаБюджет");
...
Процедура ВладелецПриИзмененииСерверЛокализация()
	...
	Если Не ИспользоватьТекстКорреспондентаБюджет Тогда
		ТекстКорреспондентаБюджет = НаименованиеВладельцаСчета(Объект.Владелец);
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_ИспользоватьТекстКорреспондентаБюджетПриИзмененииЛокализация(Элемент)
...
Процедура ПриЧтенииСозданииНаСервереЛокализация()
	...
	ИспользоватьТекстКорреспондентаБюджет = ЗначениеЗаполнено(Объект.ТекстКорреспондентаБюджет);
	Если ИспользоватьТекстКорреспондентаБюджет Тогда
		ТекстКорреспондентаБюджет = Объект.ТекстКорреспондентаБюджет;
	Иначе
		ТекстКорреспондентаБюджет = НаименованиеВладельцаСчета(Объект.Владелец);
	КонецЕсли;
...
Процедура ЗаполнитьТекстКорреспондента()
	...
	ТекстКорреспондента = НаименованиеВладельцаСчета(Объект.Владелец);
...
&НаСервереБезКонтекста
Функция НаименованиеВладельцаСчета(ВладелецСчета)
	...
	|";
	Запрос.УстановитьПараметр("Ссылка", ВладелецСчета);
	
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
	
	Если Выборка.Следующий() Тогда
		Результат = СокрЛП(Выборка.Наименование);
	Иначе
		Результат = "";
	КонецЕсли;
...
Если Не ПодключеныОбработчикиЛокализация И ПолучитьФункциональнуюОпцию("ПоддержкаПлатежейРФ") Тогда
	...
	ПодключаемыеОбработчикиСобытийЭлементов.Вставить("ИспользоватьТекстКорреспондентаБюджет", "ПриИзменении");
