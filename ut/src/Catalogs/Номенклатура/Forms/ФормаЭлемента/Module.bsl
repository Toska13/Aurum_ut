...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		ИнтеграцияИС.ИнициализироватьНастройкиФормыДляУстановкиКлючаВидовПродукции(ЭтотОбъект);
	КонецЕсли;
	//-- Локализация
	...
	Иначе	
		...
		Для Каждого ИспользуетсяДопЯзык Из МультиязычностьСервер.СведенияОЯзыках().Используется Цикл
			Если ИспользуетсяДопЯзык.Значение = Истина Тогда
				ПараметрыСозданияСтруктура.Вставить(СтрШаблон("Наименование%1", ИспользуетсяДопЯзык.Ключ), "");
			КонецЕсли;
		КонецЦикла;
		...
		//++ Локализация
		ЗаполнитьИОтобразитьКартинкиРеквизитов();
		//-- Локализация
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	//++ Локализация
	ЗаполнитьКлючВидовПродукции(Объект.Ссылка);
	ИнтеграцияИС.ИнициализироватьНастройкиФормыДляУстановкиКлючаВидовПродукции(ЭтотОбъект);
	ИнтеграцияИС.ЗаполнитьДанныеПоКлючуВидовПродукции(ЭтотОбъект);
	//-- Локализация
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	// Конец СтандартныеПодсистемы.Свойства
	СобытияФорм.ОбработкаПроверкиЗаполненияНаСервере(ЭтотОбъект, Отказ, ПроверяемыеРеквизиты);
...
Процедура ПодготовитьДанныеДляИнтерактиваПередЗаписью(Отказ)
	...
	Если Не Отказ
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПодакцизныеТовары")
			И УчетПодакцизныхТоваров.ЭтоПодакцизныйТовар(ПараметрыСоздания().ИсточникКопирования)
			И УчетПодакцизныхТоваров.ПодакцизныеКатегорииТовара(ПараметрыСоздания().ИсточникКопирования).Количество() > 0 Тогда
				СоответствияСкопированныхОбъектов.Вставить("ПодакцизныеКатегории");
				ЕдиницаИзмеренияИсточника = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыСоздания().ИсточникКопирования, "ЕдиницаИзмерения");
				
				Если ЕдиницаИзмеренияИсточника = Объект.ЕдиницаИзмерения Тогда
					ПараметрыКопированияДополнительныхДанных.Добавить("КопироватьПодакцизныеКатегории",
						НСтр("ru = 'Копировать подакцизные категории'"),
						Истина);
				Иначе
					ПараметрыКопированияДополнительныхДанных.Добавить("КопироватьПодакцизныеКатегорииБезКоэффициента",
						НСтр("ru = 'Копировать подакцизные категории (без коэффициента)'"),
						Истина);
				КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ПараметрКопирования = ПараметрыКопированияДополнительныхДанных.НайтиПоЗначению("КопироватьПодакцизныеКатегории");
	Если ПараметрКопирования <> Неопределено
		И ПараметрКопирования.Пометка Тогда
		
		УчетПодакцизныхТоваров.ПроверитьДанныеКопированияПодакцизныхКатегорий(
			ПараметрыСоздания().ИсточникКопирования, Отказ);
		
	КонецЕсли;
	
	// ++ Локализация
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, "КлючВидовПродукции") Тогда
		ТекущийОбъект.ДополнительныеСвойства.Вставить("КлючВидовПродукции", ЭтотОбъект["КлючВидовПродукции"]);
	КонецЕсли;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрКопирования <> Неопределено
		...
		УчетПодакцизныхТоваров.СкопироватьПодакцизныеКатегории(Объект.Ссылка,
			ПараметрыСоздания().ИсточникКопирования, Истина);
		
	КонецЕсли;
	
	ПараметрКопирования = ПараметрыКопированияДополнительныхДанных.НайтиПоЗначению("КопироватьПодакцизныеКатегорииБезКоэффициента");
	Если ПараметрКопирования <> Неопределено
		И ПараметрКопирования.Пометка Тогда
		
		УчетПодакцизныхТоваров.СкопироватьПодакцизныеКатегории(Объект.Ссылка,
			ПараметрыСоздания().ИсточникКопирования, Ложь);
		
	КонецЕсли;
		
	Элементы.ТекстДанныхПоНезаписаннымПодакцизнымКатегориям.Видимость = Ложь;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_ПодакцизнаяКатегорияНоменклатуры"
		И Параметр.Номенклатура = Объект.Ссылка Тогда
		НастроитьФормуИОбновитьКарточку();
...
&НаКлиенте
Процедура ФлагФГИСПриИзменении(Элемент)

	//++ Локализация
	ИнтеграцияИСКлиент.ФлагФГИСПриИзменении(ЭтотОбъект);
	НастроитьФормуИОбновитьКарточку();
	//-- Локализация
...
&НаКлиенте
Процедура ВидПродукцииФГИСПриИзменении(Элемент)

	//++ Локализация
	ИнтеграцияИСКлиент.ВидПродукцииФГИСПриИзменении(ЭтотОбъект);
	НастроитьФормуИОбновитьКарточку();
	//-- Локализация
...
&НаКлиенте
Процедура ВидыПродукцииФГИСОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)

	//++ Локализация
	СтандартнаяОбработка = Ложь;

	Если ТребуетсяОбновлениеКлассификатораВидовПродукции Тогда

		Если ИнтерактивнаяЗагрузкаКлассификаторовДоступна Тогда

			ОписаниеОповещения = Новый ОписаниеОповещения("Подключаемый_ПослеЗагрузкиКлассификаторов", ЭтотОбъект);

			ПараметрыОбновления = РаботаСКлассификаторамиКлиент.НовыйПараметрыОбновленияКлассификаторов();
			ПараметрыОбновления.Владелец           = ЭтотОбъект;
			ПараметрыОбновления.ОписаниеОповещения = ОписаниеОповещения;
			ПараметрыОбновления.Идентификаторы     = Новый Массив;
			ПараметрыОбновления.Идентификаторы.Добавить(
				ОбщегоНазначенияИСКлиентСервер.ИдентификаторКлассификатораВидовПродукцииИС());

			РаботаСКлассификаторамиКлиент.ОбновитьКлассификаторы(ПараметрыОбновления);

		Иначе

			ПоказатьПредупреждение(, НСтр("ru = 'Интерактивная загрузка классификаторов недоступна. Обратитесь к администратору.'"));
	...
	Иначе
		...
		ПараметрыОткрытияФормы.Вставить("КлючВидовПродукции", КлючВидовПродукции);

		ОткрытьФорму("ОбщаяФорма.НастройкаИнтеграцииИС",
			ПараметрыОткрытияФормы,,
			УникальныйИдентификатор,,,
			Новый ОписаниеОповещения("НастройкаИнтеграцииСФГИСЗавершение", ЭтотОбъект),
			РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура НаборУпаковокОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если Объект.НаборУпаковок = ПредопределенноеЗначение("Справочник.НаборыУпаковок.ИндивидуальныйДляНоменклатуры") Тогда
		...
		ОткрытьФорму("Справочник.УпаковкиЕдиницыИзмерения.ФормаСписка", Новый Структура("Отбор", Отбор), ЭтотОбъект);
...
Процедура НоменклатураМногооборотнаяТараПриИзмененииСервер()
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтруктураСтроки, СтруктураДействий, Неопределено);
...
Функция ПараметрыПереходаПоГиперссылке(ИмяЭлемента, Гиперссылка)
	...
	ИначеЕсли ИмяЭлемента = "ГиперссылкаПерейтиПодакцизныеКатегорииНоменклатуры" Тогда
		
		ПараметрыФормы = Новый Структура("Номенклатура", Объект.Ссылка);
		
		ПараметрыПереходаПоГиперссылке.ИмяФормы = "РегистрСведений.КатегорииПодакцизныхТоваров.Форма.ФормаНоменклатуры";
		ПараметрыПереходаПоГиперссылке.ПараметрыФормы = ПараметрыФормы;
		ПараметрыПереходаПоГиперссылке.РежимОткрытияОкнаФормы = РежимОткрытияОкнаФормы.БлокироватьОкноВладельца;
		
	ИначеЕсли ИмяЭлемента = "ГиперссылкаПерейтиТекущаяПодакцизнаяКатегорияНоменклатуры" Тогда
		
		ПараметрыТекущейПодакцизнойКатегории = ПолучитьДанныеДляПереходаКТекущейПодакцизнойКатегории();
		Если ПараметрыТекущейПодакцизнойКатегории = Неопределено Тогда
			Если Не ЭтоБазоваяВерсия Тогда
				ПараметрыФормы = Новый Структура("Номенклатура, ТекущийПериод", Объект.Ссылка, Истина);
				ПараметрыПереходаПоГиперссылке.ИмяФормы = "РегистрСведений.КатегорииПодакцизныхТоваров.Форма.ФормаНоменклатуры";			
			Иначе
				ПараметрыОтбора = Новый Структура("Номенклатура", Объект.Ссылка);
				ПараметрыФормы = Новый Структура("ЗначенияЗаполнения", ПараметрыОтбора);
				ПараметрыПереходаПоГиперссылке.ИмяФормы = "РегистрСведений.КатегорииПодакцизныхТоваров.Форма.ФормаЗаписи";
			КонецЕсли;
		Иначе
			ПараметрыМассив = Новый Массив;
			ПараметрыМассив.Добавить(ПараметрыТекущейПодакцизнойКатегории);
			КлючЗаписиРегистра = Новый("РегистрСведенийКлючЗаписи.КатегорииПодакцизныхТоваров", ПараметрыМассив); 
...
&НаСервере
Функция ПолучитьДанныеДляПереходаКТекущейПодакцизнойКатегории()
	
	ПередаваемыеПараметры = Неопределено;
	ТаблицаКатегорий = УчетПодакцизныхТоваров.ПолучитьИсториюКатегорийНоменклатуры(Объект.Ссылка,, ТекущаяДатаСеанса()); 
	Если ТаблицаКатегорий.Количество() = 1 Тогда
		ПередаваемыеПараметры = Новый Структура("Период, Номенклатура, Страна", ТаблицаКатегорий[0].Период, Объект.Ссылка, ТаблицаКатегорий[0].Страна);
	КонецЕсли;
...
&НаКлиенте
Процедура ГиперссылкаПерейтиТекущаяПодакцизнаяКатегорияНоменклатуры(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ГиперссылкаПерейтиПодакцизнаяКатегорияНоменклатуры(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура УстановитьАктивностьТекущийЭлемент()
	ТекущийЭлемент = Элементы[ТекущийЭлементВыбор];
КонецПроцедуры

&НаКлиенте

&НаКлиенте
Процедура КодВидаНоменклатурнойКлассификацииОчистка(Элемент, СтандартнаяОбработка)
...
Процедура ПрослеживаемыйТоварПриИзменении(Элемент)
	...
	ПрослеживаемыйТоварПриИзмененииНаКлиенте();
	
КонецПроцедуры

&НаКлиенте
Процедура ПрослеживаемыйТоварПриИзмененииНаКлиенте()
...
&НаКлиенте
Процедура ПереноситьРНПТМатериалаПриИзменении(Элемент)
...
Процедура КодТРУПриИзменении(Элемент)
	...
	//++ Локализация
	Если Не ЗначениеЗаполнено(Объект.КодТРУ) Тогда
		Возврат;
	КонецЕсли;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСовпаденияНоменклатуры

&НаКлиенте
Процедура СовпаденияНоменклатурыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	//++ Локализация
	Если Поле.Имя = "СовпаденияНоменклатурыКартинкаОткрытия" Тогда
...
Процедура ЗаполнитьНаименованиеДляПечатиПоШаблону(Команда)
	...
	КонецЕсли;
	ШаблонНаименованияДляПечатиНоменклатуры = СокрЛП(ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(Объект.ВидНоменклатуры, "ШаблонНаименованияДляПечатиНоменклатуры"));
	Если ШаблонНаименованияДляПечатиНоменклатуры = "[Наименование]" 
		ИЛИ ПустаяСтрока(ШаблонНаименованияДляПечатиНоменклатуры) Тогда 
		РезультатЗаполнения = ЗаполнитьМультиязычноеНаименованиеПолноеПоНаименованию();
		Если РезультатЗаполнения.Свойство("ЗначенияНаРазныхЯзыках") Тогда 
			ДополнительныеПараметры = Новый Структура("Объект, ИмяРеквизита, Форма", Объект, "НаименованиеПолное", ЭтотОбъект);
			МультиязычностьКлиент.ПослеВводаСтрокНаРазныхЯзыках(РезультатЗаполнения, ДополнительныеПараметры);
		КонецЕсли;
	КонецЕсли;
...
// БиблиотекаРаспознаванияДокументов

&НаКлиенте
Процедура ВыбратьПохожуюПозициюНоменклатуры(Команда)
	
	//++ Локализация
	ТекущиеДанные = Элементы.СовпаденияНоменклатуры.ТекущиеДанные;
...
Процедура ВидНоменклатурыПриИзмененииНаСервере(ЗаполнениеПоИсточникуКопирования = Ложь)
	...
	//++ Локализация
	ЗаполнитьКлючВидовПродукции(Объект.ВидНоменклатуры);
	ПриИзмененииКлючаВидовПродукции();
	//-- Локализация

	ОбновитьКешРеквизитовВидаНоменклатуры();
	
	ЗаполняемыйОбъект = Объект;
	НоменклатураЛокализация.ДополнитьОбъектФормы(ЭтотОбъект, ЗаполняемыйОбъект);

	Справочники.Номенклатура.ЗаполнитьРеквизитыПоВидуНоменклатуры(ЗаполняемыйОбъект,
																	Ложь,
																	Ложь,
																	ВидНоменклатурыДоИзменения,
																	ЗаполнениеПоИсточникуКопирования);
	
	НоменклатураЛокализация.ВосстановитьОбъектФормы(ЭтотОбъект, ЗаполняемыйОбъект);
	...
	ИначеЕсли ЗаполнениеПоИсточникуКопирования
		...
		Объект.Наименование = ?(ФормироватьРабочееНаименование, "", ПараметрыСоздания().Наименование);
		
		Для Каждого ИспользуетсяДопЯзык Из МультиязычностьСервер.СведенияОЯзыках().Используется Цикл
			Если ИспользуетсяДопЯзык.Значение = Истина Тогда
				ИмяРеквизита = СтрШаблон("Наименование%1", ИспользуетсяДопЯзык.Ключ);
				Объект[ИмяРеквизита] = ПараметрыСоздания()[ИмяРеквизита];
			КонецЕсли;
		КонецЦикла;
		
		Объект.НаименованиеПолное = ?(ФормироватьНаименованиеДляПечати, "", ПараметрыСоздания().НаименованиеПолное);
...
Процедура ДобавлениеФайлаНеПрисоединенноЗавершение(ПомещенныйФайл, ДополнительныеПараметры) Экспорт
	...
	ЭтотОбъект[ДополнительныеПараметры.ИмяРеквизита] = ОписаниеДанныхФайла();
	
	СтруктураИмениФайла = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ПомещенныйФайл.Имя);
	
	ЭтотОбъект[ДополнительныеПараметры.ИмяРеквизита].ПолноеИмя		= СтруктураИмениФайла.Имя;
	ЭтотОбъект[ДополнительныеПараметры.ИмяРеквизита].ИмяБезРасширения	= СтруктураИмениФайла.ИмяБезРасширения;
	ЭтотОбъект[ДополнительныеПараметры.ИмяРеквизита].Расширение		= ОбщегоНазначенияКлиентСервер.РасширениеБезТочки(
...
&НаКлиенте
Процедура ТоварУчаствуетВДеятельностиПоСтавкеНДС0ПриИзменении(Элемент)
...
Процедура НастроитьФормуИОбновитьКарточку()
	...
	Элементы.СрокКонтроляКачества.Доступность = Объект.КонтролироватьКачествоПоступившегоТовара;
	...
	РеквизитыСПереопределеннойОбязательностьюЗаполнения = Справочники.Номенклатура.РеквизитыСПереопределеннойОбязательностьюЗаполнения(
															Объект.ТипНоменклатуры,
															НоменклатураЛокализация.ОсобенностьУчетаИлиКлючВидовПродукции(ЭтотОбъект),
															ЗаполняемыйОбъект);
	...
	НастройкиВидимостиИЗаголовков = Справочники.Номенклатура.НастройкиВидимостиИЗаголовков(ЗаполняемыйОбъект, РежимВидимостиПоказатьТолькоВажные);
	...
	НоменклатураЛокализация.НастроитьФормуИОбновитьКарточку(ЭтотОбъект, ВидимостьЭлементов);
	
	//++ Локализация
	СвойстваЭлементовФормыБРД();
	//-- Локализация
...
Процедура РазвернутьСворачиваемуюГруппу(ИмяГруппы)
	...
	Если ГруппаРодитель = ЭтотОбъект Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеОбязательныхРеквизитов(Отказ)
	...
	НоменклатураЛокализация.ПроверитьЗаполнениеОбязательныхРеквизитов(ЭтотОбъект, Отказ);
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
	...
	НепроверяемыеРеквизиты	= Справочники.Номенклатура.НепроверяемыеРеквизиты(ЗаполняемыйОбъект);

	НоменклатураЛокализация.ВосстановитьОбъектФормы(ЭтотОбъект, ЗаполняемыйОбъект);

	РеквизитыФормы			= Справочники.Номенклатура.РеквизитыФормыТаблицыНастроек();
	...
	Для Каждого СтрокаТаблицы Из ТаблицаНастроекРеквизитов Цикл
		...
		ЭтоРеквизитФормы	= РеквизитыФормы.Получить(СтрокаТаблицы.ИмяРеквизита) <> Неопределено;
		ЗначениеРеквизита	= ?(ЭтоРеквизитФормы,
								ЭтотОбъект[СтрокаТаблицы.ИмяРеквизита],
		...
		Если Не ЗначениеЗаполнено(ЗначениеРеквизита) Тогда
			...
			Если СтрокаТаблицы.ИмяРеквизита = "Штрихкод" Тогда
				Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , ПутьКРеквизиту, , Отказ);
				КонецЕсли;
			...
			ПолеФормы = ?(ЭтоРеквизитФормы, СтрокаТаблицы.ИмяРеквизита, "Объект." + СтрокаТаблицы.ИмяРеквизита);
...
Процедура ПроверитьУникальностьЭлементаПоШтрихкоду()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНеуникальныеШтрихкодыНоменклатуры") Тогда
		...
		|				ИСТИНА
		|			ИЗ
		|				Справочник.ВидыНоменклатуры.РеквизитыДляКонтроляНоменклатуры КАК РеквизитыКонтроля
		|			ГДЕ
		|				РеквизитыКонтроля.Ссылка = &ВидНоменклатуры
...
Процедура НастроитьДоступностьБлокируемыхРеквизитов()
	...
	НоменклатураЛокализация.НастроитьДоступностьБлокируемыхРеквизитов(ЭтотОбъект);
...
Процедура ЗаполнитьПоОбъектуКопирования()
	...
	Если ПараметрыСоздания().Свойство("Качество")
		...
		Для Каждого ИспользуетсяДопЯзык Из МультиязычностьСервер.СведенияОЯзыках().Используется Цикл
			Если ИспользуетсяДопЯзык.Значение = Истина Тогда
				ИмяРеквизита = СтрШаблон("Наименование%1", ИспользуетсяДопЯзык.Ключ);
				ПараметрыСозданияСтруктура[ИмяРеквизита] = Объект[ИмяРеквизита] + ", " + Строка(Объект.Качество);
			КонецЕсли;
		КонецЦикла;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодакцизныеТовары")
		И УчетПодакцизныхТоваров.ЭтоПодакцизныйТовар(ПараметрыСоздания.ИсточникКопирования) Тогда
		Элементы.ТекстДанныхПоНезаписаннымПодакцизнымКатегориям.Видимость = Истина;
	КонецЕсли;
...
Функция ЗаполнитьНаименованиеПоШаблонуСервер(ВариантыФормирования)
	...
	Если МассивВариантов.Найти("Рабочее") <> Неопределено Тогда
		...
		ШаблонКачество		= "[Качество]";
		ШаблонНаименования	= РеквизитыВидаНоменклатуры.ШаблонРабочегоНаименованияНоменклатуры;
		
		Если ЗначениеЗаполнено(Объект.Качество)
			И Объект.Качество <> Перечисления.ГрадацииКачества.Новый
			И СтрНайти(ШаблонНаименования, ШаблонКачество) = 0 Тогда
			
			ШаблонНаименования = ШаблонНаименования + " + "", "" + " + ШаблонКачество;
			
		КонецЕсли;
		
		НовоеРабочееНаименование = НоменклатураСервер.НаименованиеПоШаблону(ШаблонНаименования, СправочникОбъект);
...
&НаСервере
Функция ЗаполнитьМультиязычноеНаименованиеПолноеПоНаименованию()
	РезультатЗаполнения = Новый Структура;
	РезультатЗаполнения.Вставить("Модифицированность", Истина);
	РезультатЗаполнения.Вставить("ХранениеВТабличнойЧасти", Истина);
	РезультатЗаполнения.Вставить("ОсновнойЯзык", ТекущийЯзык().КодЯзыка );
	
	ИмяРеквизита = "Наименование";
	ЗначенияНаРазныхЯзыках = Новый Массив;

	ЗначениеЯзыка = Новый Структура;

	Для Каждого Язык Из Метаданные.Языки Цикл 
		Если СтрСравнить(Язык.КодЯзыка, ТекущийЯзык().КодЯзыка) = 0 Тогда
			Продолжить;
		КонецЕсли;
		Суффикс = МультиязычностьСервер.СуффиксЯзыка(Язык.КодЯзыка);
		ЗначениеЯзыка = Новый Структура;
		ЗначениеЯзыка.Вставить("КодЯзыка", Язык.КодЯзыка);
		ЗначениеЯзыка.Вставить("ЗначениеРеквизита", Объект[ИмяРеквизита + "Язык1"]);
		ЗначениеЯзыка.Вставить("Суффикс", "Язык1");
		ЗначенияНаРазныхЯзыках.Добавить(ЗначениеЯзыка);
	КонецЦикла; 
	РезультатЗаполнения.Вставить("ЗначенияНаРазныхЯзыках", ЗначенияНаРазныхЯзыках);
...
Функция ИменаСворачиваемыхГрупп()
	...
	ИменаСворачиваемыхГрупп.Добавить(Элементы.СворачиваемаяГруппаКонтрольКачества.Имя);
	...
	ИменаСворачиваемыхГрупп.Добавить(Элементы.СворачиваемаяГруппаПодакцизныхКатегорий.Имя);
...
Функция ИмяЭлементаПереходаКСвязанномуСписку(Элементы, ИмяЭлемента, ИмяГиперссылки)
	...
	Если ИмяГиперссылки Тогда
		...
		ИначеЕсли ИмяЭлемента = Элементы.ГиперссылкаПерейтиСоставНабора.Имя Тогда
			НовоеИмя = Элементы.КомандаПерейтиСоставНабора.Имя;
	...
	Иначе
		...
		ИначеЕсли ИмяЭлемента = Элементы.КомандаПерейтиСоставНабора.Имя Тогда
			НовоеИмя = Элементы.ГиперссылкаПерейтиСоставНабора.Имя;
...
Функция ЭлементПереходаКСвязанномуСписку(Элементы, ИмяЭлемента)
	...
	Если ИмяЭлемента = Элементы.ГиперссылкаПерейтиУпаковкиНоменклатуры.Имя
		...
		Или ИмяЭлемента = Элементы.ГиперссылкаПерейтиСтавкиНДСНоменклатуры.Имя
		Или ИмяЭлемента = Элементы.ГиперссылкаПерейтиПодакцизныеКатегорииНоменклатуры.Имя Тогда
...
&НаСервере
Процедура ЗаполнитьИОтобразитьКартинкиРеквизитов()

	КартинкиРеквизитов = КартинкиРеквизитовСоздаваемогоОбъекта(Параметры);
	Если Не КартинкиРеквизитов.Количество() = 0 Тогда
		ЗаполнитьКартинкиРеквизитовСоздаваемогоОбъекта(КартинкиРеквизитов);
		НастроитьКартинкиРеквизитовФормы();
		ИзменитьОтображениеСпискаСовпадений();
	КонецЕсли;

КонецПроцедуры

&НаСервереБезКонтекста
Функция КартинкиРеквизитовСоздаваемогоОбъекта(ПараметрыОткрытия)
	
	КартинкиРеквизитов = Новый Массив;
	ДополнительныеДанные = ПараметрыОткрытия.ДополнительныеДанные;
	РаспознанныйДокумент = Неопределено;
	
	Если ДополнительныеДанные = Неопределено
		Или Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДополнительныеДанные, "РаспознанныйДокумент") Тогда
		Возврат КартинкиРеквизитов;
	КонецЕсли;

	РаспознанныйДокумент = ДополнительныеДанные.РаспознанныйДокумент;
	ИменаРеквизитовКартинок = ИменаРеквизитовКартинок();
	
	КартинкиРаспознанногоДокумента = КартинкиРаспознанногоДокумента(РаспознанныйДокумент,
		ДополнительныеДанные.ИмяТаблицы);
	...
	СвязанныеКолонки = РаспознаваниеДокументовСлужебный.ПараметрыСозданияНовогоЭлемента(
		ПараметрыОткрытия.ИмяЭлемента,
		ПараметрыОткрытия.ТипДокумента,
		ПараметрыОткрытия.Направление);
		
	Для Каждого РеквизитЭлемента Из СвязанныеКолонки Цикл
		
		Если ИменаРеквизитовКартинок.Найти(РеквизитЭлемента.ИмяРеквизита) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ПараметрыРеквизита = ПараметрыРеквизитаКартинки(РеквизитЭлемента.Реквизит);
		
		ПараметрыСвязаннойКолонки = Неопределено;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(РеквизитЭлемента, "ИмяРеквизита") Тогда
			...
			Если ПараметрыСвязаннойКолонки <> Неопределено Тогда
				...
				ПараметрыРеквизита.Картинка = ДвоичныеДанныеКартинки(КартинкиРаспознанногоДокумента, ПараметрыСвязаннойКолонки);
				ПараметрыРеквизита.ВысотаКартинки = ?(ПараметрыРеквизита.Картинка <> Неопределено,
					ПараметрыСвязаннойКолонки.СтрокВИзображении, 0);
				ПараметрыРеквизита.Координаты = ПараметрыСвязаннойКолонки.Координаты;
			КонецЕсли;
		КонецЕсли;
		
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(РеквизитЭлемента, "ПустаяСсылка") Тогда
			Если ПараметрыСвязаннойКолонки <> Неопределено Тогда
				ПараметрыРеквизита.Значение = ПараметрыСвязаннойКолонки.Значение;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(ПараметрыРеквизита.Значение) Тогда
				ПараметрыРеквизита.Значение = РеквизитЭлемента.ПустаяСсылка;
			КонецЕсли;
		КонецЕсли;
		
		КартинкиРеквизитов.Добавить(ПараметрыРеквизита);
...
&НаСервереБезКонтекста
Функция ИменаРеквизитовКартинок()

	ИменаРеквизитов = Новый Массив;
	ИменаРеквизитов.Добавить("Номенклатура");
	ИменаРеквизитов.Добавить("Артикул");
	ИменаРеквизитов.Добавить("ЕдиницаИзмерения");
	ИменаРеквизитов.Добавить("СтавкаНДС");
	
	Возврат ИменаРеквизитов;

КонецФункции // ИменаРеквизитовКартинок()

&НаСервереБезКонтекста
Функция ПараметрыРеквизитаКартинки(ИмяРеквизита)
...
КонецФункции // ПараметрыРеквизитаКартинки()

&НаСервереБезКонтекста
Функция ДвоичныеДанныеКартинки(КартинкиРеквизитов, ПараметрыСвязаннойКолонки)
	
	Картинка = Неопределено;
	
	Если Не РаспознаваниеДокументовКлиентСервер.ВсеКоординатыНулевые(ПараметрыСвязаннойКолонки.Координаты) Тогда
		
		СтрокаКартинки = КартинкиРеквизитов.Найти(ПараметрыСвязаннойКолонки.НомерСтроки, "НомерСтроки");
		Если Не СтрокаКартинки = Неопределено Тогда
			Картинка = СтрокаКартинки.ОбластьИзображения.Получить();
			Если ТипЗнч(Картинка) = Тип("ДвоичныеДанные") И Картинка.Размер() = 0 Тогда
				Картинка = Неопределено;
			КонецЕсли;
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция КартинкиРаспознанногоДокумента(РаспознанныйДокумент, ИмяТаблицы)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ИмяТаблицы.НомерСтроки КАК НомерСтроки,
	|	ИмяТаблицы.ОбластьИзображения КАК ОбластьИзображения
	|ИЗ
	|	&ИмяТаблицы КАК ИмяТаблицы
	|ГДЕ
	|	ИмяТаблицы.Ссылка = &РаспознанныйДокумент
	|	И ИмяТаблицы.ИмяРеквизита В(&ИменаРеквизитов)";
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяТаблицы", СтрШаблон("Документ.РаспознанныйДокумент.%1", ИмяТаблицы));
	
	Запрос.УстановитьПараметр("РаспознанныйДокумент", РаспознанныйДокумент);
	Запрос.УстановитьПараметр("ИменаРеквизитов", ИменаРеквизитовКартинок());
	РезультатЗапроса = Запрос.Выполнить();
...
&НаСервере
Процедура ЗаполнитьКартинкиРеквизитовСоздаваемогоОбъекта(ЗначенияОткрытия)
	...
	Для Каждого ПереданныйРеквизит Из ЗначенияОткрытия Цикл
		...
		Если ДанныеРеквизита.ВысотаКартинки Тогда
			ХешКартинки = ХешКартинкиРеквизита(ДанныеРеквизита.Картинка);
			
			Отбор = Новый Структура("ХешКартинки", ХешКартинки);
			Если РеквизитыСоздаваемогоОбъекта.НайтиСтроки(Отбор).Количество()
				Или ХешКартинки = Неопределено Тогда
				
				ДанныеРеквизита.ВысотаКартинки = 0;
			КонецЕсли;
			
			ДанныеРеквизита.ХешКартинки = ХешКартинки;
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция ХешКартинкиРеквизита(ДвоичныеДанные)
...
&НаСервере
Процедура НастроитьКартинкиРеквизитовФормы()
	...
	Для Каждого РеквизитСоздаваемогоОбъекта Из РеквизитыСоздаваемогоОбъекта Цикл
		...
		Если РеквизитСоздаваемогоОбъекта.ВысотаКартинки > 0
			...
			ИмяРеквизита = СтрШаблон("%1Картинка", РеквизитСоздаваемогоОбъекта.Имя);
			ЭтотОбъект[ИмяРеквизита] = РаспознаваниеДокументовКомплекты.КартинкаКакТекстHTML(ДвоичныеДанные);
...
&НаСервере
Процедура СвойстваЭлементовФормыБРД()
	...
	Для Каждого РеквизитСоздаваемогоОбъекта Из РеквизитыСоздаваемогоОбъекта Цикл
		...
		Если РеквизитСоздаваемогоОбъекта.Имя = "Наименование" Тогда
			...
			Если РеквизитСоздаваемогоОбъекта.ВысотаКартинки > 1 Тогда
				Элементы.НаименованиеКартинка.Высота = Цел(РеквизитСоздаваемогоОбъекта.ВысотаКартинки / 2) + 1;
			КонецЕсли;
		КонецЕсли;
		...
		Если РеквизитСоздаваемогоОбъекта.ВысотаКартинки > 0
			И ЗначениеЗаполнено(РеквизитСоздаваемогоОбъекта.Картинка) Тогда
			ИмяРеквизита = СтрШаблон("%1Картинка", РеквизитСоздаваемогоОбъекта.Имя);
			Элементы[ИмяРеквизита].Видимость = Элементы[РеквизитСоздаваемогоОбъекта.Имя].Видимость;
		КонецЕсли;
		
	КонецЦикла;
	
	Элементы.ГруппаСовпадения.Видимость = Параметры.Ключ.Пустая() И СовпаденияНоменклатуры.Количество() > 0;

КонецПроцедуры

&НаСервере
Процедура ИзменитьОтображениеСпискаСовпадений()
	...
	ДанныеПоиска = Новый Структура("Наименование", Объект.Наименование);
	
	ОтборМетаданных = Новый Массив;
	
	ДанныеПоиска.Вставить("Артикул", Объект.Артикул);
	...
	Для Каждого Кандидат Из Кандидаты Цикл
		НоваяСтрока = СовпаденияНоменклатуры.Добавить();
...
Процедура НастройкаИнтеграцииСФГИСЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		КлючВидовПродукции = Результат.КлючВидовПродукции;
		ПриИзмененииКлючаВидовПродукции();
		Модифицированность = Истина;
		НастроитьФормуИОбновитьКарточку();
	КонецЕсли;
...
&НаСервере
Процедура ПриИзмененииКлючаВидовПродукции()
...
&НаСервере
Процедура ЗаполнитьКлючВидовПродукции(Ссылка)

	Данные = ОбщегоНазначенияИС.ОписаниеНоменклатуры(Ссылка)[Ссылка];
	...
	ТребуетсяОбновлениеКлассификатораВидовПродукции = Не ЗначениеЗаполнено(КлючВидовПродукции)
		И Данные.ИдентификаторыВидовПродукции <> Неопределено;

КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ПослеЗагрузкиКлассификаторов(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ПриСозданииНаСервереЛокализация(Отказ, СтандартнаяОбработка)
	...
	ИнтерактивнаяЗагрузкаКлассификаторовДоступна = РаботаСКлассификаторами.ИнтерактивнаяЗагрузкаКлассификаторовДоступна();
...
Процедура НачатьВыборКодаВидаНоменклатурнойКлассификации_Завершение(Результат, Параметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура")
		...
		НастроитьПодсказкуПоляКлассификатора(Элементы.КодВидаНоменклатурнойКлассификации.Имя,
											Объект.НаименованиеВидаНоменклатурнойКлассификации);
...
&НаКлиенте
Процедура ПриИзмененииТНВЭДиОКПД2(ОбновлятьКарточку = Истина)

	УказанТНВЭД = ЗначениеЗаполнено(Объект.КодТНВЭД);
	УказанОКПД2 = ЗначениеЗаполнено(Объект.КодОКПД2);

	Если УказанТНВЭД Или УказанОКПД2 Тогда
		ДоступныеВидыПродукции = ВидыПродукцииПоТНВЭДиОКПД2(Объект.КодТНВЭД, Объект.КодОКПД2);
		Если ДоступныеВидыПродукции.Количество() = 1
			И ВидПродукцииГИСМТ <> ДоступныеВидыПродукции[0] Тогда
			Если Не ЗначениеЗаполнено(ВидПродукцииГИСМТ) Тогда
				ВидПродукцииГИСМТ = ДоступныеВидыПродукции[0];
				ИнтеграцияИСКлиент.ВидПродукцииГИСМТПриИзменении(ЭтотОбъект);
				Если ОбновлятьКарточку Тогда
					НастроитьФормуИОбновитьКарточку();
				КонецЕсли;
			Иначе
				Если УказанТНВЭД И УказанОКПД2 Тогда
					НачалоВопроса = СтрШаблон(
						НСтр("ru='Указанным ТН ВЭД %1 и ОКПД2 %2'"),
						Строка(Объект.КодТНВЭД),
						Строка(Объект.КодОКПД2));
				ИначеЕсли УказанТНВЭД Тогда
					НачалоВопроса = СтрШаблон(
						НСтр("ru='Указанному ТН ВЭД %1'"),
						Объект.КодТНВЭД);
				Иначе
					НачалоВопроса = СтрШаблон(
						НСтр("ru='Указанному ОКПД2 %1'"),
						Объект.КодОКПД2);
				КонецЕсли;
				ДополнительныеПараметры = Новый Структура;
				ДополнительныеПараметры.Вставить("ВидПродукции", ДоступныеВидыПродукции[0]);
				ПоказатьВопрос(
					Новый ОписаниеОповещения("ВопросИзменитьВидПродукцииЗавершение",
						ЭтотОбъект,
						ДополнительныеПараметры),
					СтрШаблон(
						НСтр("ru='%1 соответствует вид продукции %2.
							|Изменить текущий вид продукции %3 на %2'"),
						НачалоВопроса,
						ДоступныеВидыПродукции[0],
						ВидПродукцииГИСМТ),
					РежимДиалогаВопрос.ДаНет);
			КонецЕсли;
		КонецЕсли;
...
&НаКлиенте
Процедура ВопросИзменитьВидПродукцииЗавершение(Результат, ДополнительныеПараметры) Экспорт

	Если Результат = КодВозвратаДиалога.Да Тогда
		ВидПродукцииГИСМТ = ДополнительныеПараметры.ВидПродукции;
		ИнтеграцияИСКлиент.ВидПродукцииГИСМТПриИзменении(ЭтотОбъект);
		НастроитьФормуИОбновитьКарточку();
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ВидыПродукцииПоТНВЭДиОКПД2(ТНВЭД, ОКПД2)

	ДанныеДляУточнения = ОбщегоНазначенияИСМПКлиентСервер.ДанныеДляУточненияВидовПродукцииПоТНВЭДиОКПД2();
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЕСТЬNULL(МАКСИМУМ(ЗначенияКлассификаторов.ТНВЭД), """") КАК ТНВЭД,
	|	ЕСТЬNULL(МАКСИМУМ(ЗначенияКлассификаторов.ОКПД2), """") КАК ОКПД2
	|ИЗ
	|(ВЫБРАТЬ
	...
	|ГДЕ
	|	КлассификаторТНВЭД.Ссылка = &ТНВЭД
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	NULL КАК ТНВЭД,
	|	КлассификаторОКПД2.Код КАК ОКПД2
	|ИЗ
	|	Справочник.КлассификаторОКПД2 КАК КлассификаторОКПД2
	|ГДЕ
	|	КлассификаторОКПД2.Ссылка = &ОКПД2) КАК ЗначенияКлассификаторов";

	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		ЗаполнитьЗначенияСвойств(ДанныеДляУточнения, Выборка);
	КонецЕсли;

	Возврат ИнтерфейсИСМПОбщегоНазначенияВызовСервера.ВидыПродукцииПоДаннымУточнения(ДанныеДляУточнения, Ложь);
