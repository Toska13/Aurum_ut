	...
	Если (ЗначениеЗаполнено(Объект.Контрагент) Или ЗначениеЗаполнено(Объект.ТорговыйОбъект))
		...
		Если Выборка.Следующий() Тогда
			...
			Если УпрощеннаяИнтеграцияБЭД Тогда
				ТекстСообщения = НСтр("ru = 'Организация ""%2"" уже сопоставлена с организацией ЕГАИС ""%1""'");
				ТекстСообщения = СтрШаблон(ТекстСообщения, Выборка.Ссылка, Выборка.Контрагент);
			Иначе
...
Процедура ПредставлениеАдресаПриИзменении(Элемент)
	...
	Объект.Адрес = ОбщегоНазначенияИСВызовСервера.ЗначенияПолейКонтактнойИнформации(Текст, ВидКонтактнойИнформацииАдреса);
...
Процедура ЗапроситьОрганизациюПоКодуФСРАР(Команда)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПараметрыФормы.Вставить("ОрганизацияЕГАИС", Объект.Ссылка);
	КонецЕсли;
...
Процедура ЗаполнитьКомментарийАдресаСервер()
	...
	Если ПустаяСтрока(Объект.Адрес) Тогда
		...
		Объект.Адрес = ОбщегоНазначенияИСВызовСервера.ЗначенияПолейКонтактнойИнформации(Объект.ПредставлениеАдреса, ВидКонтактнойИнформацииАдреса);
		Возврат;
	КонецЕсли;
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.ФорматОбмена) Тогда
		...
		КонецЕсли;
	Иначе
		Объект.ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V4;
	КонецЕсли;
