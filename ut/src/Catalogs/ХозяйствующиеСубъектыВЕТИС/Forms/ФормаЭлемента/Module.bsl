...
#Область ОбработчикиСобытийЭлементовТаблицыФормыУполномоченноеГашение
...
Процедура СинхронизироватьСписокПредприятий(Команда)
	...
	Иначе
		...
		ОбновленныеПредприятия = ВыполнитьСинхронизациюПредприятий(Ложь);
		Для каждого Предприятие Из ОбновленныеПредприятия Цикл
			ОтобразитьИзменениеДанных(Предприятие, ВидИзмененияДанных.Изменение);
		КонецЦикла;
...
Процедура ЗагрузитьВсе(Команда)
	...
	Иначе
		...
		ОбновленныеПредприятия = ВыполнитьСинхронизациюПредприятий(Истина);
		Для каждого Предприятие Из ОбновленныеПредприятия Цикл
			ОтобразитьИзменениеДанных(Предприятие, ВидИзмененияДанных.Изменение);
		КонецЦикла;
...
Процедура ВопросПриСинхронизацииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если ЗаписаноУспешно Тогда
		ОбновленныеПредприятия = ВыполнитьСинхронизациюПредприятий(ДополнительныеПараметры.ЗагрузитьНовые);
		Для каждого Предприятие Из ОбновленныеПредприятия Цикл
			ОтобразитьИзменениеДанных(Предприятие, ВидИзмененияДанных.Изменение);
		КонецЦикла;
	КонецЕсли;
...
Функция ТаблицаИдентификаторовПредприятий()
	...
	ТаблицаПредприятий.Колонки.Добавить("ИдентификаторВерсии",	Метаданные.ОпределяемыеТипы.УникальныйИдентификаторИС.Тип);
	...
	Пока Не ПолучениеДанныхЗавершено Цикл
		...
		Для Каждого ДанныеПредприятия Из ДанныеВЕТИС.Список Цикл
			...
			НоваяСтрока.ИдентификаторВерсии = ДанныеПредприятия.enterprise.uuid;
...
&НаСервере
Функция ВыполнитьСинхронизациюПредприятий(ЗагрузитьНовые)

	ОбновленныеПредприятия = Новый Массив;
	...
	Если ТаблицаИдентификаторовПредприятий = Неопределено Тогда
		Возврат ОбновленныеПредприятия;
	КонецЕсли;
	...
	Попытка
		...
		|	ИдентификаторыGLNПредприятий.GLN КАК GLN,
		|	ИдентификаторыGLNПредприятий.Идентификатор КАК Идентификатор,
		|	ИдентификаторыGLNПредприятий.ИдентификаторВерсии КАК ИдентификаторВерсии
		|ПОМЕСТИТЬ ИдентификаторыGLNПредприятий
		|ИЗ
		|	&ИдентификаторыGLNПредприятий КАК ИдентификаторыGLNПредприятий
		|;
		...
		|	ИдентификаторыGLNПредприятий.GLN КАК GLN,
		|	ИдентификаторыGLNПредприятий.Идентификатор КАК Идентификатор,
		|	ИдентификаторыGLNПредприятий.ИдентификаторВерсии <> ПредприятияВЕТИС.ИдентификаторВерсии КАК ОбъектИзменен
		|ИЗ
		|	ИдентификаторыGLNПредприятий КАК ИдентификаторыGLNПредприятий
		...
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			Если Выборка.ОбъектИзменен Тогда
				ДанныеПредприятия = ЦерберВЕТИСВызовСервера.ПредприятиеПоGUID(Выборка.Идентификатор, Истина);
				СправочникОбъект = Выборка.Предприятие.ПолучитьОбъект();
				Предприятие = ИнтеграцияВЕТИС.ЗагрузитьПредприятие(ДанныеПредприятия.Элемент, СправочникОбъект);
				Если ЗначениеЗаполнено(Предприятие) Тогда
					ОбновленныеПредприятия.Добавить(Предприятие);
				КонецЕсли;
			КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
		МодульУправлениеДоступом = ОбщегоНазначения.ОбщийМодуль("УправлениеДоступом");
		МодульУправлениеДоступом.ПослеЗаписиНаСервере(
			ЭтотОбъект,
			РеквизитФормыВЗначение("Объект", Тип("СправочникОбъект.ХозяйствующиеСубъектыВЕТИС")),
			Новый Структура);
	КонецЕсли;
	// Конец СтандартныеПодсистемы.УправлениеДоступом
