...
Процедура Загрузить(Команда)
	...
	ЗагрузитьПартию_ДлительнаяОперация();
...
&НаКлиенте
Процедура ЗагрузитьПартию_ДлительнаяОперация()
	
	ПараметрыОбработкиДокументов = ИнтеграцияСАТУРНСлужебныйКлиентСервер.ПараметрыОбработкиДокументов();
	ПараметрыОбработкиДокументов.Ссылка             = ПредопределенноеЗначение("Справочник.ПартииСАТУРН.ПустаяСсылка");
...
Процедура Подключаемый_ПриЗавершенииОперации(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого ЭлементДанных Из Результат Цикл
		...
		Для Каждого ОбъектЗагрузки Из ЭлементДанных.Объект Цикл
			...
			Если Не ТипЗнч(ОбъектЗагрузки) = Тип("СправочникСсылка.ПартииСАТУРН") Тогда
				Продолжить;
			КонецЕсли;
			
			Партия = ОбъектЗагрузки;
			Прервать;
	...
	ОбщегоНазначенияКлиент.ОповеститьОбИзмененииОбъекта(Партия);
...
Процедура ЗагрузитьРодительскуюПартиюНаСервере()
	...
	Если Не ЗначениеЗаполнено(ИдентификаторРодительскойПартии) Тогда
		Возврат;
	КонецЕсли;
