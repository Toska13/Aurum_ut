...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НастроитьВидимостьЭлементовОплатыЭСФССЛокализация();
	ЗаполнитьПараметрыПодключенияНСПКЛокализация();
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	КассаККМЗаписатьПараметрыПодключенияОрганизацииНСПКЛокализацияПриЗаписиНаСервере(ЭтотОбъект, ТекущийОбъект);
...
Процедура ТипКассыПриИзмененииНаСервере()
	...
	Если Объект.ТипКассы = Перечисления.ТипыКассККМ.ФискальныйРегистратор
			ИЛИ Объект.ТипКассы = Перечисления.ТипыКассККМ.АвтономнаяККМ Тогда
...
Процедура ПротоколОбменаНСПКПриИзменении(Элемент)
	ПротоколОбменаНСПКПриИзмененииЛокализация(Элемент);
КонецПроцедуры
...
&НаКлиенте
Процедура ПротоколОбменаНСПКПриИзмененииЛокализация(Элемент)
	
	//++ Локализация
	ПротоколОбменаНСПКПриИзмененииСерверЛокализация();
	//-- Локализация
...
Функция ПараметрыПодключенияКСервисуНСПКЗаполнены(ПараметрыОперации, ТекстСообщения)
	...
	Если Не ЗначениеЗаполнено(ПараметрыОперации.ПротоколОбмена) Тогда
		ТекстСообщения = ТекстСообщения + ?(ЗначениеЗаполнено(ТекстСообщения), Символы.ПС, "") + НСтр("ru = 'Поле ""Протокол обмена НСПК"" не заполнено.'");
		ПараметрыЗаполнены = Ложь;
	КонецЕсли;
...
&НаСервере
Процедура ПротоколОбменаНСПКПриИзмененииСерверЛокализация()
...
&НаСервере
Процедура ЗаполнитьПараметрыПодключенияНСПКЛокализация();
	
	//++ Локализация
	Если НЕ (ПолучитьФункциональнуюОпцию("ОплатаЭлектроннымиСертификатамиФСС")
		И ЗначениеЗаполнено(Объект.Ссылка)
		И Объект.ТипКассы = Перечисления.ТипыКассККМ.ФискальныйРегистратор) Тогда
		
		Возврат;
	КонецЕсли;
	
	ПараметрыПодключенияНСПК = ЭлектронныеСертификатыНСПКУТ.ПараметрыПодключенияОрганизацииНСПК(Объект.Владелец);
...
&НаСервере
Процедура НастроитьВидимостьЭлементовОплатыЭСФССЛокализация()
	
	Элементы.ЛокализацияПараметрыНСПК.Видимость = Ложь;
	Элементы.КлючОрганизацииНСПК.Видимость = Ложь;
	...
	Если Объект.ТипКассы = Перечисления.ТипыКассККМ.ФискальныйРегистратор
			ИЛИ Объект.ТипКассы = Перечисления.ТипыКассККМ.АвтономнаяККМ Тогда
		Элементы.ЛокализацияПараметрыНСПК.Видимость = ПолучитьФункциональнуюОпцию("ОплатаЭлектроннымиСертификатамиФСС");
		Элементы.КлючОрганизацииНСПК.Видимость = ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций");
		
		Элементы.ТестовоеПодключениеНСПК.Доступность = Истина;
		Элементы.ТестовоеПодключениеНСПК.ОтображениеПодсказки = ОтображениеПодсказки.Нет;
		Если Не ЗначениеЗаполнено(Объект.ПротоколОбменаНСПК) 
			Или Объект.ПротоколОбменаНСПК = Перечисления.ПротоколОбменаФЭСНСПК.ВерсияV1 Тогда
			Элементы.ТестовоеПодключениеНСПК.Доступность = Ложь;
			Элементы.ТестовоеПодключениеНСПК.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСправа;
		КонецЕсли;
...
&НаСервере
Процедура КассаККМЗаписатьПараметрыПодключенияОрганизацииНСПКЛокализацияПриЗаписиНаСервере(Форма, ТекущийОбъект)
