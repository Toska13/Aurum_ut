...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПроверитьДоступностьИнтеграционныхМетодов = 
		Не Параметры.ДоступныИнтеграционныеМетоды И Параметры.ПроверитьДоступностьИнтеграционныхМетодов;
	...
	ДобавитьУдалитьВариантВыбораЗагрузкиИнтеграционнымиМетодами(ДоступныИнтеграционныеМетоды);
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПроверитьДоступностьИнтеграционныхМетодов Тогда
		ПодключитьОбработчикОжидания("ПроверитьДоступностьИнтеграционныхМетодов", 0.1, Истина);
	КонецЕсли;
...
Процедура СписокЗагруженныхОбъектовПередУдалением(Элемент, Отказ)
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("СписокЗагруженныхОбъектовПометкаУдаленияЗавершение", ЭтотОбъект);
...
Процедура СписокОтменаПроведения(Команда)
	...
	Если ТипЗнч(Элементы.СписокЗагруженныхОбъектов.ТекущаяСтрока) = Тип("Число") Тогда
		...
		НераспроведенныеДокументы = Новый Массив;
		ШаблонСообщения = НСтр("ru = 'Для документа %1 не выполнена отмена проведения. %2.'");
		ДанныеОНераспроведенныхДокументах = ОбщегоНазначенияУТВызовСервера.ОтменаПроведенияДокументов(МассивСсылок);
		
		Для Каждого ИнформацияОДокументе Из ДанныеОНераспроведенныхДокументах Цикл
			Если ПустаяСтрока(ИнформацияОДокументе.ОписаниеОшибки) Тогда
				НераспроведенныеДокументы.Добавить(ИнформацияОДокументе.Ссылка);
		...
		РаспроведенныеДокументы = ОбщегоНазначенияКлиентСервер.РазностьМассивов(МассивСсылок, НераспроведенныеДокументы);
		
		Если НераспроведенныеДокументы.Количество() > 0 Тогда
			КнопкиДиалога = Новый СписокЗначений;
			КнопкиДиалога.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru = 'ОК'"));
			
			ТекстДиалога = НСтр("ru = 'Не удалось выполнить отмену проведения одного или нескольких документов.'");
			ДополнительныеПараметры = ДополнительныеПараметрыОповещенияПослеОбработкиДанных(РаспроведенныеДокументы);
			ОписаниеОповещения = Новый ОписаниеОповещения("ПроверитьПроведенностьДокументовЗавершение",
															ОбщегоНазначенияУТКлиент,
															ДополнительныеПараметры);
			
			ПоказатьВопрос(ОписаниеОповещения, ТекстДиалога, КнопкиДиалога);
		Иначе
			ОповещениеПроведенияОтменыПроведенияДокумента(Элементы.СписокЗагруженныхОбъектов, МассивСсылок, Заголовок);
		КонецЕсли;
	Иначе
...
Процедура СписокПровести(Команда)
	...
	Если ТипЗнч(Элементы.СписокЗагруженныхОбъектов.ТекущаяСтрока) = Тип("Число") Тогда
		...
		Если НепроведенныеДокументы.Количество() > 0 Тогда
			...
			ДополнительныеПараметры = ДополнительныеПараметрыОповещенияПослеОбработкиДанных(ПроведенныеДокументы);
...
Процедура СписокУстановитьСнятьПометкуУдаления(Команда)
	...
	Если ВыделенныеСтроки.Количество() > 1 Тогда
		...
		ПараметрыОбновленияСписка = ПараметрыОбновленияСписка();
	...
	Иначе
		ОбщегоНазначенияУТКлиент.УстановитьПометкуУдаления(Элементы.СписокЗагруженныхОбъектов, Заголовок, ОповещениеОЗавершении);
	КонецЕсли;
...
Функция СопоставитьЗагружаемыеДанныеНаСервере()
	...
	Если ПустаяСтрока(АдресЗагруженныхДанных) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Требуется повторить получение данных.'"));
...
//    * ВидМаркетплейса        - ПеречислениеСсылка.ВидыМаркетплейсов - вид учетной записи.
...
Функция ПараметрыПодготовкиКСопоставлениюДанных()
	...
	ВидМаркетплейса = ДанныеТорговойПлощадки.ВидМаркетплейса; // ПеречислениеСсылка.ВидыМаркетплейсов
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("УчетнаяЗапись",         УчетнаяЗапись);
	ПараметрыФормы.Вставить("ВидМаркетплейса",       ВидМаркетплейса);
	ПараметрыФормы.Вставить("ИмяОбъектаМетаданных",  ИмяОбъектаМетаданных);
...
Процедура ПослеСопоставленияДанныхИзФормыСопоставления(Результат, ПолеОбработчикаСобытияПриИзменении) Экспорт
	...
	ОжидаемоеПолеОбработчикаСобытияПриИзменении =
		ИнтеграцияСМаркетплейсамиКлиент.ПолеОбработчикаСобытияПриИзмененииПриСопоставленииДанных(
			ПараметрыПодготовкиКСопоставлениюДанных().ИмяОбъектаМетаданных);
	
	ЭтоИзменениеПоляСопоставления =
		(ПолеОбработчикаСобытияПриИзменении = ОжидаемоеПолеОбработчикаСобытияПриИзменении);
	...
	Для Каждого ДанныеПоляСопоставления Из Результат Цикл
		Если ЭтоИзменениеПоляСопоставления
				И ДанныеПоляСопоставления.Значение.Количество() > 10 Тогда
		...
		Иначе
			...
			Для Каждого ДанныеОбработки Из ДанныеПоляСопоставления.Значение Цикл
				...
				Для Каждого СтрокаОбработки Из СтрокиОбработки Цикл
					...
					Иначе
						ЗаполнитьРезультатСопоставленияСтроки(
							СтрокаОбработки,
							КолонкиОбязательныеПриПроверке,
							КолонкиДополнительныеПриПроверке);
					КонецЕсли;
...
&НаКлиенте
Процедура СопоставитьЗагружаемыеДанные(ШаблонБезОшибокЗаполнения = Истина, ЭтоПовторноеСопоставление = Ложь)
	...
	Иначе
		...
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ЭтоПовторноеСопоставление", ЭтоПовторноеСопоставление);
		
		Обработчик = Новый ОписаниеОповещения("ЗавершениеСопоставленияЗагружаемыхДанныхКлиент", ЭтотОбъект, ДополнительныеПараметры);
		ДлительныеОперацииКлиент.ОжидатьЗавершение(ФоновоеЗадание, Обработчик, НастройкиОжидания);
	КонецЕсли;
...
Процедура ЗавершениеСопоставленияЗагружаемыхДанныхКлиент(ФоновоеЗадание, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ФоновоеЗадание.Статус = "Выполнено" Тогда
		...
		ОжидаемыеПараметры.Вставить("ЭтоПовторноеСопоставление", Ложь);
		
		Если ДополнительныеПараметры <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(ОжидаемыеПараметры, ДополнительныеПараметры);
		КонецЕсли;
		
		ВсеСтрокиСопоставлены = (ФильтрТаблицаСопоставления <> "Несопоставленные") 
									И (ФильтрТаблицаСопоставления <> "ПодобраноАвтоматически");
		ВыполнитьШагСопоставлениеЗагружаемыхДанныхКлиент(ВсеСтрокиСопоставлены, ОжидаемыеПараметры.ЭтоПовторноеСопоставление);
...
&НаКлиенте
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхКлиент(ВсеСтрокиСопоставлены = Неопределено, ЭтоПовторноеСопоставление = Ложь)
	...
	Иначе
		...
		Если Не ЭтоПовторноеСопоставление Тогда
			ПоказатьВопросЗаписиДанныхСОшибкамиЗаполнения();
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура СписокЗагруженныхОбъектовПометкаУдаленияЗавершение(Команда, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Функция ДополнительныеПараметрыОповещенияПослеОбработкиДанных(ОбработанныеДокументы)
	
	ДополнительныеПараметры = Новый Структура;
	
	ДополнительныеПараметры.Вставить("Список",               Элементы.СписокЗагруженныхОбъектов);
	ДополнительныеПараметры.Вставить("ПроведенныеДокументы", ОбработанныеДокументы);
	ДополнительныеПараметры.Вставить("ТипСписка",            Заголовок);
...
&НаКлиенте
Функция ПараметрыОбновленияСписка()
	...
	Возврат ПараметрыОбновленияСписка;
...
&НаКлиенте
Процедура ПроверитьДоступностьИнтеграционныхМетодов()
	
	ВидМаркетплейса = ДанныеТорговойПлощадки.ВидМаркетплейса; // ПеречислениеСсылка.ВидыМаркетплейсов
	Если Не ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоOzon(ВидМаркетплейса) Тогда
		Возврат;
	КонецЕсли;
	
	Элементы.ДекорацияПолучениеСпискаДоступныхРолейУчетнойЗаписи.Видимость = Истина;
	
	ДлительнаяОперация = ЗапуститьПолучениеДоступныхРолейУчетнойЗаписи(УчетнаяЗапись, УникальныйИдентификатор);
	
	ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
	ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
	ОповещениеОЗавершении = Новый ОписаниеОповещения(
		"ЗапуститьПолучениеДоступныхРолейУчетнойЗаписиЗавершение", ЭтотОбъект);
...
&НаСервереБезКонтекста
Функция ЗапуститьПолучениеДоступныхРолейУчетнойЗаписи(УчетнаяЗапись, УникальныйИдентификатор)
...
//
&НаКлиенте
Процедура ЗапуститьПолучениеДоступныхРолейУчетнойЗаписиЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ЗаполнитьДоступныеРолиУчетнойЗаписиНаСервере(АдресРезультата)
	...
	Если ИспользуетсяПозаказныйОтчетОРеализации Тогда
		ДоступныИнтеграционныеМетоды = ДоступныеРоли.ДоступноЧтениеПозаказныхОтчетовОРеализации
	Иначе
		ДоступныИнтеграционныеМетоды = ДоступныеРоли.ДоступноЧтениеОтчетовОРеализации;
	КонецЕсли;
...
&НаСервере
Процедура ДобавитьУдалитьВариантВыбораЗагрузкиИнтеграционнымиМетодами(ДоступныИнтеграционныеМетоды)
	
	ЗначениеИнтеграционныйМетод = "ИнтеграционнымиМетодами";
	ПредставлениеИнтеграционныйМетод = НСтр("ru = 'Обращение к сервису'");
	
	ЭлементВариантаЗагрузки = Элементы.ВариантВыгрузкиЗагрузки.СписокВыбора.НайтиПоЗначению(ЗначениеИнтеграционныйМетод);
	
	Если Не ДоступныИнтеграционныеМетоды И ЭлементВариантаЗагрузки <> Неопределено Тогда
		Элементы.ВариантВыгрузкиЗагрузки.СписокВыбора.Удалить(ЭлементВариантаЗагрузки);
		
	ИначеЕсли ДоступныИнтеграционныеМетоды И ЭлементВариантаЗагрузки = Неопределено Тогда
		Элементы.ВариантВыгрузкиЗагрузки.СписокВыбора.Добавить(
			ЗначениеИнтеграционныйМетод, ПредставлениеИнтеграционныйМетод);
	КонецЕсли;
