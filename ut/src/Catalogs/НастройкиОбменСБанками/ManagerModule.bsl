...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗагрузитьНастройкиОбменаИзФайла", Истина);
	Методы.Вставить("ЗаполнитьВерсиюФормата");
	Методы.Вставить("ЗарегистрироватьДанныеКОбработке");
	Методы.Вставить("ПерезаполнитьНастройкиОбменСБанками");
	Методы.Вставить("ПолучитьНастройкиОбменаССервераБанка", Истина);
...
Процедура ЗагрузитьНастройкиОбменаИзФайла(Знач ДанныеНастроек, АдресХранилища) Экспорт
	...
	ДанныеНастроек.Свойство("ТребуетсяSMSАвторизация", ТребуетсяSMSАвторизация);
	...
	Если ЗначениеЗаполнено(НоваяНастройкаОбмена) Тогда
			
		ОбменСБанкамиСлужебный.СохранитьПараметрыОбмена(НоваяНастройкаОбмена, ТребуетсяSMSАвторизация);
...
Функция ПолучитьНастройкиОбмена(ПараметрыПолученияНастроек)
	...
	Если ДанныеПоТипу.ЕстьОшибка Тогда
		Если Не ПараметрыПолученияНастроек.ПробнаяОперация Тогда
			...
			ТекстОшибки = ОбменСБанкамиСлужебный.ТекстОшибкиЧтенияДвоичныхДанныхИзБанка(
				ПараметрыПолученияНастроек.НастройкаОбмена, Результат.Тело, ДанныеПоТипу.ПредставлениеОшибки);
