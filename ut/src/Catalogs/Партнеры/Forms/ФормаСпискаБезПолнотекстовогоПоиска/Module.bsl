...
Процедура Подключаемый_ОбновитьКомандыЭДО()
	...
	Если ИспользоватьПартнеровКакКонтрагентов Тогда
		...
		Если ТекущаяСтрока <> Неопределено Тогда
			КонтрагентПартнера = ПартнерыИКонтрагентыВызовСервера.КонтрагентПартнера(ТекущаяСтрока);
			Если КонтрагентПартнера <> Неопределено Тогда
				ОбновитьКомандыЭДОДляКонтрагентаПартнера(КонтрагентПартнера);
			Иначе
...
&НаКлиенте
Процедура ОтметитьДублямиВыделенные(Команда)
	ПоискИЗаменаДублейНСИКлиент.ОтметитьДублямиВыделенные(Элементы.Список);
КонецПроцедуры
...
// ЭлектронноеВзаимодействие.ОбменСКонтрагентами
&НаСервере
Функция ВычислитьЗначенияАлгоритмовЭДО(Алгоритмы, ПараметрыАлгоритма)
	ЗначенияАлгоритмов = Новый Соответствие;
	Для Каждого КлючИЗначение Из Алгоритмы Цикл
		СтрокаВычисления = СтрШаблон("%1(Параметры)", КлючИЗначение.Ключ);
		Значение = ОбщегоНазначения.ВычислитьВБезопасномРежиме(СтрокаВычисления, ПараметрыАлгоритма);
...
&НаКлиенте
Процедура ОбновитьКомандыЭДОДляКонтрагентаПартнера(Контрагент)
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, "ПараметрыУправленияВидимостьюЭДО") Тогда 
		ПараметрыУправленияВидимостью = ЭтотОбъект.ПараметрыУправленияВидимостьюЭДО;
	Иначе
		ПараметрыУправленияВидимостью = Неопределено;
	КонецЕсли;
	Если ПараметрыУправленияВидимостью = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	КомандыДляПроверкиАлгоритмами = Новый Соответствие;
	ОбъектыДляПроверки = Новый Массив;
	ОбъектыДляПроверки.Добавить(Контрагент);
	
	Для Каждого Команда Из ПараметрыУправленияВидимостью.КомандыСУсловиямиВидимости Цикл
		КомандаЭлемент = Элементы[Команда.ИмяВФорме];
		Видимость = Истина;
		
		Если ЗначениеЗаполнено(Команда.ИмяРеквизитаУсловия) Тогда
			Видимость = Ложь;
		...
		ИначеЕсли Команда.ЗначениеУсловия = Неопределено Тогда
			Видимость = Ложь;
		Иначе
			Видимость = Истина;
		КонецЕсли;
	...
	Если КомандыДляПроверкиАлгоритмами.Количество() > 0 Тогда
		...
		КонецЦикла;
		ЗначенияАлгоритмов = ВычислитьЗначенияАлгоритмовЭДО(Алгоритмы, ОбъектыДляПроверки);
		Для Каждого КомандаИАлгоритм Из КомандыДляПроверкиАлгоритмами Цикл
