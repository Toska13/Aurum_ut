...
Функция КлючевыеРеквизиты() Экспорт
	...
	Результат.Вставить("ОблагаетсяАкцизом");
	Результат.Вставить("ПодразделениеПроизводства");
...
//      * ОблагаетсяАкцизом - Булево - признак, что по данному товару необходимо рассчитывать сумму акциза.
//      * ПодразделениеПроизводства - СправочникСсылка.СтруктураПредприятия - подразделение, в котором был произведен подакцизный товар.
//
// Возвращаемое значение:
...
Функция ВидЗапасовДокумента(Организация, ХозяйственнаяОперация = Неопределено, РеквизитыДокумента = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(РеквизитыДокумента) Тогда
		...
		Реквизиты.ОблагаетсяАкцизом = Ложь;
		Реквизиты.ПодразделениеПроизводства = Справочники.СтруктураПредприятия.ПустаяСсылка();
	...
	Если ЗначениеЗаполнено(Реквизиты)
		И ЗначениеЗаполнено(Реквизиты.СохраняемыйВидЗапасов) Тогда
...
Функция ВидЗапасовДляВозвратаТоваровОтКлиента(ВидЗапасовОтгрузки, Организация = Неопределено) Экспорт
	...
	Если Выборка.Следующий() Тогда
		ВидЗапасов = ВидЗапасовДокумента(?(Организация = Неопределено, Выборка.Организация, Организация),
...
//		* ОблагаетсяАкцизом - Булево, Массив Из Булево - признак, что по данному товару необходимо рассчитывать сумму акциза.
//		* ПодразделениеПроизводства - СправочникСсылка.СтруктураПредприятия - подразделение, в котором был произведен подакцизный товар.
//
Функция ПараметрыОтбораВидовЗапасов() Экспорт
	...
	Параметры.Вставить("ОблагаетсяАкцизом", Ложь);
	Параметры.Вставить("ПодразделениеПроизводства", Справочники.СтруктураПредприятия.ПустаяСсылка());
...
Функция ВидЗапасовПоОтборам(ПоляОтбора) Экспорт
	...
	Запрос.УстановитьПараметр("ОблагаетсяАкцизом", ПоляОтбора.ОблагаетсяАкцизом);
	Запрос.УстановитьПараметр("ПодразделениеПроизводства", ПоляОтбора.ПодразделениеПроизводства);
...
Процедура ЗаменитьДублиВидовЗапасов(Параметры = Неопределено, АдресХранилищаРезультата = "") Экспорт
	...
	|	ВидыЗапасов.ОблагаетсяАкцизом КАК ОблагаетсяАкцизом,
	|	ВидыЗапасов.ПодразделениеПроизводства КАК ПодразделениеПроизводства,
	...
	|			И ВидыЗапасов.ОблагаетсяАкцизом = Отборы.ОблагаетсяАкцизом
	|			И ВидыЗапасов.ПодразделениеПроизводства = Отборы.ПодразделениеПроизводства
	...
	|			И Дубли.ОблагаетсяАкцизом = Корректные.ОблагаетсяАкцизом
	|			И Дубли.ПодразделениеПроизводства = Корректные.ПодразделениеПроизводства
...
Функция ТекстЗапросаПоискаДублейВидовЗапасов(ЭтоОбработчикОбновления = Ложь)
	...
	|	ВидыЗапасов.ОблагаетсяАкцизом,
	|	ВидыЗапасов.ПодразделениеПроизводства,
	|	ВидыЗапасов.ВидЦены
	|
	|ИМЕЮЩИЕ
	|	СУММА(1) > 1
...
Функция СоздатьВидЗапасов(СтруктураПолей)
	...
	Попытка
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(СтруктураПолей.ОблагаетсяАкцизом) Тогда
			ЭлементБлокировки.УстановитьЗначение("ОблагаетсяАкцизом", СтруктураПолей.ОблагаетсяАкцизом);
		КонецЕсли;
		Если ЗначениеЗаполнено(СтруктураПолей.ПодразделениеПроизводства) Тогда
			ЭлементБлокировки.УстановитьЗначение("ПодразделениеПроизводства", СтруктураПолей.ПодразделениеПроизводства);
		КонецЕсли;
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
...
Функция СтруктураВидаЗапасов(Организация, ХозяйственнаяОперация = Неопределено, РеквизитыДокумента = Неопределено) Экспорт
	...
	Если РеквизитыДокумента <> Неопределено Тогда
		...
		Если СтруктураВидЗапасов.ТипЗапасов <> Перечисления.ТипыЗапасов.Работа
			И ПолучитьФункциональнуюОпцию("ФормироватьВидыЗапасовПоГруппамФинансовогоУчета") Тогда
...
Функция ВидЗапасовПоОтборамКэш(ПараметрыАналитики)
	...
	Результат = ЗначениеНастроекПовтИсп.ВидЗапасовПоОтборам(СтруктураАналитики.Организация,
		...
		СтруктураАналитики.ОблагаетсяАкцизом,
		СтруктураАналитики.ПодразделениеПроизводства);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Справочники.ВидыЗапасов.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "11.5.26.33";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("b07c9520-04e6-423a-b833-89d3a9d664a0");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Справочники.ВидыЗапасов.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Порядок = Перечисления.ПорядокОбработчиковОбновления.Обычный;
	
	СписокОписаний = Новый Массив;
	СписокОписаний.Добавить(НСтр("ru = 'Обновление справочника ""Виды запасов"":'"));
	СписокОписаний.Добавить(НСтр("ru = '- Заполнение реквизита ""Налогообложение НДС"" для комиссионных видов запасов, переданных между организациями.'"));
	...
	Изменяемые.Добавить(Метаданные.Справочники.ВидыЗапасов.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Справочники.ВидыЗапасов.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Справочник.ВидыЗапасов КАК ВидыЗапасов
	|ГДЕ
	|	ВидыЗапасов.Организация <> ВидыЗапасов.ВладелецТовара
	|	И ВидыЗапасов.ВладелецТовара ССЫЛКА Справочник.Организации
	|	И ВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
	|	И НЕ ВидыЗапасов.НалогообложениеНДС В
	...
	ОбновлениеИнформационнойБазы.ОтметитьКОбработке(Параметры, Запрос.Выполнить());
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний = Новый Массив;
	СписокОписаний.Добавить(НСтр("ru = 'Не удалось обработать справочник ""Виды запасов"" по обработчику:'"));
	СписокОписаний.Добавить(НСтр("ru = '- Заполнение реквизита ""Налогообложение НДС"" для комиссионных видов запасов, переданных между организациями.'"));
	
	ДопустимыеТипыНалогообложенияНДС = Новый Массив;
	ДопустимыеТипыНалогообложенияНДС.Добавить(Перечисления.ТипыНалогообложенияНДС.ПустаяСсылка());
	ДопустимыеТипыНалогообложенияНДС.Добавить(Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС);
	ДопустимыеТипыНалогообложенияНДС.Добавить(Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС);
	
	Выборка = Справочники.ВидыЗапасов.ВыбратьПоСсылкам(ОбновляемыеДанные, "Ссылка");
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			СправочникОбъект = ОбновлениеИнформационнойБазыУТ.ПроверитьПолучитьОбъект(
								...
								Параметры.Очередь); // СправочникОбъект.ВидыЗапасов -
			
			ПричинаИсключения	= "ПлохиеДанные";
			Рекомендация		= НСтр("ru = 'Проверьте данные справочника вручную.'");
			
			Если СправочникОбъект <> Неопределено Тогда
				Если ДопустимыеТипыНалогообложенияНДС.Найти(СправочникОбъект.НалогообложениеНДС) = Неопределено Тогда
					СправочникОбъект.НалогообложениеНДС =
						?(Перечисления.ТипыНалогообложенияНДС.ТипыСНДС().Найти(СправочникОбъект.НалогообложениеНДС) <> Неопределено,
