...
//	ИсключитьСчетаВПереданнойВалюте - Булево - если Истина, будут исключены счета в валюте, переданной в параметре Валюта.
//
// Возвращаемое значение:
//	СправочникСсылка.БанковскиеСчетаКонтрагентов - Найденный банковский счет получателя.
//
Функция ПолучитьБанковскийСчетПоУмолчанию(Получатель, Валюта = Неопределено, КорреспондирующийСчет = Неопределено,
	ИсключитьСчетаВПереданнойВалюте = Ложь) Экспорт
	...
	|	И &УсловиеПоВалюте
	|	И (БанковскиеСчетаКонтрагентов.СчетЦифровогоРубля = &КорреспонденцияСоСчетомЦифровогоРубля
	|		ИЛИ &КорреспонденцияСоСчетомЦифровогоРубля = НЕОПРЕДЕЛЕНО)";
	
	Если Не ИсключитьСчетаВПереданнойВалюте Тогда
		УсловиеПоВалюте = "(БанковскиеСчетаКонтрагентов.ВалютаДенежныхСредств = &Валюта
...
Функция ПолучитьРеквизитыБанковскогоСчета(БанковскийСчет) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		СчетЦифровогоРубля               = Выборка.СчетЦифровогоРубля;
		ИдентификаторСчетаЦифровогоРубля = Выборка.ИдентификаторСчетаЦифровогоРубля;
	Иначе
	...
	СтруктураРеквизитов.Вставить("СчетЦифровогоРубля", СчетЦифровогоРубля);
	СтруктураРеквизитов.Вставить("ИдентификаторСчетаЦифровогоРубля", ИдентификаторСчетаЦифровогоРубля);
...
Процедура ПриЗаполненииОграниченияДоступа(Ограничение) Экспорт
	...
	|	И ЧтениеОбъектаРазрешено(Т.Владелец)
	...
	|	И( ИзменениеОбъектаРазрешено(Владелец)
	|	))";
...
Функция ТекстЗапросаПечати()
	...
	|	БанковскиеСчетаКонтрагентов.СчетЦифровогоРубля КАК СчетЦифровогоРубля,
	|	БанковскиеСчетаКонтрагентов.ИдентификаторСчетаЦифровогоРубля КАК ИдентификаторСчетаЦифровогоРубля,
	...
	|		ПО БанковскиеСчетаКонтрагентов.БанкДляРасчетов = БанкиДляРасчетов.Ссылка
