...
Процедура ПриФормированииЗаказаНаОплатуСБП(
	...
	//++ Локализация
	СистемаБыстрыхПлатежейУТ.ПриФормированииЗаказаНаОплатуСБПс2b(
		ДокументОплаты, ЗаказНаОплату, НастройкаПодключения, ДополнительныеПараметры);
	
	РозничныеПродажиЛокализация.ЗаполнитьНазначениеПлатежаПоШаблонуНазначенияСБП(ДокументОплаты, ЗаказНаОплату);
	//-- Локализация
...
Процедура ПриЗагрузкеСтатусаОперации(
	...
	Попытка
		...
		ИначеЕсли РезультатОбработки.СтатусОперации = НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииОтменена() Тогда
		...
		Иначе
			...
			Если СтатусОплаты = Перечисления.ТипыСтатусовОплатыСБП.Выполнена Тогда
				...
				ИдентификаторОплатыСБП = ПереводыСБПc2b.ИдентификаторыОперации(РезультатОбработки.ПараметрыОперации);
				...
				|ГДЕ
				|	ОперацияПоПлатежнойКарте.ИдентификаторОплатыСБП = &ИдентификаторОплатыСБП
				|	И ОперацияПоПлатежнойКарте.ДоговорЭквайринга.СпособПроведенияПлатежа
				|		= ЗНАЧЕНИЕ(Перечисление.СпособыПроведенияПлатежей.СистемаБыстрыхПлатежей)
				|	И ОперацияПоПлатежнойКарте.Проведен";
				Запрос.УстановитьПараметр("ИдентификаторОплатыСБП", ИдентификаторОплатыСБП);
