...
//  ВидПродукции - СправочникСсылка.ВидыПродукцииИС - фильтр по виду продукции.
...
Функция ОбработатьКодМаркировкиДляОтчетаОНанесении(КодМаркировки, ПараметрыРазбора)
	...
	СодержитРазделительGS  = ОбработкаСтрокиXML.НайтиНедопустимыеСимволыXML(КодМаркировки) > 0;
	...
	Если ЭталоннаяДлинаКМ <> ДлинаКодаМаркировки
		И Не (КонструкторИСМПКлиентСервер.КодМаркировкиМожетВключатьЗаводскойСерийныйНомер(ПараметрыРазбора.ВидПродукции)
	...
	Если ДанныеРазбора.СоставКодаМаркировки.Свойство("ВключаетКриптоХвост") Тогда
