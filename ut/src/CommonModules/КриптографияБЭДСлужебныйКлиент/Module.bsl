...
//  ВходящийКонтекст - См. УстановитьCryptoProCSPПослеВводаРегистрационныхДанных.ВходящийКонтекст
Процедура УстановитьКриптопровайдерПослеПолученияДистрибутива(ДлительнаяОперация, ВходящийКонтекст) Экспорт
	...
	РезультатВыполнения = Новый Структура("Выполнено, ОписаниеОшибки", Ложь, ""); 
	...
	Если ДлительнаяОперация = Неопределено Тогда
		...
		РезультатВыполнения.ОписаниеОшибки = СтрСоединить(МассивСтрок, " ");
		ВыполнитьОбработкуОповещения(ВходящийКонтекст.ОповещениеОЗавершении, РезультатВыполнения);
		Возврат;
	КонецЕсли;
	...
	Иначе
		РезультатВыполнения.ОписаниеОшибки = ДлительнаяОперация.КраткоеПредставлениеОшибки;
		ВыполнитьОбработкуОповещения(ВходящийКонтекст.ОповещениеОЗавершении, РезультатВыполнения);
		Возврат;
	КонецЕсли;
...
Асинх Процедура ОбработатьВыборПодписанта(РезультатВыбораСертификата, ПараметрыОповещения) Экспорт
	...
	ВидМобильнойПодписи = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОписаниеДанных, "ВидМобильнойПодписи");
	
	СертификатыЭДО = КриптографияБЭДСлужебныйВызовСервера.ЗарегистрированныеВЭДОСертификаты(
		ОписаниеДанных.ВыбранныйПодписант,
		ВидМобильнойПодписи);
...
// Параметры:
//  СвойстваПодписи - Структура - Свойства подписи, полученные в сервисе мобильной подписи
...
//
Функция НайтиСоздатьСертификатПодписантаИзСвойствПодписи(СвойстваПодписи, Подписант, ВидМобильнойПодписи) Экспорт
