...
Функция РезультатУстановкиШтамповВДокумент(
	...
	Ответ = СсылкаДляЗагрузки(СлужебнаяИнформацияЗапроса);
	
	Если Ответ.Отказ Тогда
		Результат.ЕстьОшибки = Истина;
		Результат.ИнформацияОбОшибках = Ответ.ТекстОшибки;
		Возврат Результат;
	КонецЕсли;
	
	URLДляЗагрузки = Ответ.URLРедиректа;
...
// Возвращает ссылку для загрузки файла в сервис.
//
// Параметры:
...
//
Функция СсылкаДляЗагрузки(Знач СлужебнаяИнформацияЗапроса)
	
	ЗапросКСервису = ЗапросКСервисуСсылкаДляЗагрузки(СлужебнаяИнформацияЗапроса);
...
//  СлужебнаяИнформацияЗапроса - См. СлужебнаяИнформацияЗапроса
//  URLДляЗагрузки             - Строка - URL-адрес файла для загрузки
// 
// Возвращаемое значение:
//  См. НовыйРезультатВыполненияЗапроса
//
Функция РезультатВыполненияЗапросаСоздатьЗаданиеВСервисеНаУстановкуШтамповВДокумент(
	Знач  ПараметрыДобавленияШтампов, Знач СлужебнаяИнформацияЗапроса, URLДляЗагрузки)
	...
	ЗапросКСервису = ЗапросКСервисуДобавитьШтампыВДокумент(URLДляЗагрузки, ДвоичныеДанные, СлужебнаяИнформацияЗапроса);
	Результат = ВыполнитьЗапрос(ЗапросКСервису);
...
Функция ЗапросКСервисуСсылкаДляЗагрузки(СлужебнаяИнформацияЗапроса)
	
	ЗапросСервиса = НовыйЗапросСервиса();
	ЗапросСервиса.СтрокаЗапроса            = "/v2/stamp";
	...
	ЗапросСервиса.Наименование             = НСтр(
		"ru = 'Получение ссылки для загрузки файла'",
		ОбщегоНазначения.КодОсновногоЯзыка());
		
	ЗапросСервиса.ДопустимыеКодыСостояния.Добавить(
		Интеграция1СШтампСлужебныйКлиентСервер.КодСостоянияРедиректаЗапроса());
...
Функция ЗапросКСервисуДобавитьШтампыВДокумент(Знач URLДляЗагрузки, Знач ДвоичныеДанные, Знач СлужебнаяИнформацияЗапроса)
	
	ЗапросСервиса = НовыйЗапросСервиса();
	ЗапросСервиса.СтрокаЗапроса            = URLДляЗагрузки;
	ЗапросСервиса.ТипПередаваемогоКонтента = ТипИсходящегоКонтентаДанныеДокументаИШтампов();
