...
Функция КлючПриглашенияПоНатуральнымКлючам(НатуральныеКлючиПриглашения) Экспорт
	...
	Если НатуральныеКлючиПриглашения.Тип = Перечисления.ТипыПриглашений.НаИдентификатор Тогда
		...
		|	РегистрСведений.ПриглашенияКОбменуЭлектроннымиДокументами КАК ПриглашенияКОбменуЭлектроннымиДокументами
		|ГДЕ
		|	ПриглашенияКОбменуЭлектроннымиДокументами.ИдентификаторКонтрагента = &ИдентификаторКонтрагента
		|	И ПриглашенияКОбменуЭлектроннымиДокументами.ИдентификаторОрганизации = &ИдентификаторОрганизации";
	...
	ИначеЕсли НатуральныеКлючиПриглашения.Тип = Перечисления.ТипыПриглашений.ЗаявкаНаРоуминг Тогда
		...
		|	РегистрСведений.ПриглашенияКОбменуЭлектроннымиДокументами КАК ПриглашенияКОбменуЭлектроннымиДокументами
		|ГДЕ
		|	ПриглашенияКОбменуЭлектроннымиДокументами.ИНН = &ИНН
		|	И ПриглашенияКОбменуЭлектроннымиДокументами.ИдентификаторОрганизации = &ИдентификаторОрганизации
...
Контрагент = Неопределено, ТекстПриглашения = Неопределено, ОписаниеОшибки = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Контрагент) Тогда
		...
		МенеджерЗаписи.ИНН = ПараметрыКонтрагента[ИмяРеквизитаИНН];
		МенеджерЗаписи.КПП = ПараметрыКонтрагента[ИмяРеквизитаКПП];
	КонецЕсли;
...
Функция ДанныеПриглашения(КлючПриглашения, ЗаполнятьПредставлениеКонтрагента = Истина) Экспорт
	...
	ЗаменяемаяЧастьТекстаЗапроса = "ПРЕДСТАВЛЕНИЕ(ПриглашенияКОбменуЭлектроннымиДокументами.Контрагент)";
	Если Не СтрНайти(ИтоговыйЗапрос.Текст, ЗаменяемаяЧастьТекстаЗапроса) Тогда
	...
	КонецЕсли;
	Если Не ЗаполнятьПредставлениеКонтрагента Тогда
		ТекстЗамены = """""";
		ИтоговыйЗапрос.Текст = СтрЗаменить(ИтоговыйЗапрос.Текст, ЗаменяемаяЧастьТекстаЗапроса, ТекстЗамены);
	КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОтправитьПриглашения", Истина);
	Методы.Вставить("ПринятьОтклонитьПриглашения", Истина);
	Методы.Вставить("ШаблонСоглашенияНаРоуминг", Истина);
