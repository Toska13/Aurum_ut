...
Функция ПараметрыОкруженияФормыИнтернетПоддержкаИСервисы(Форма) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПлатежныеСистемы.БазоваяФункциональностьПлатежныхСистем") Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПлатежныеСистемы.ЯндексПэй") Тогда
			ВнедренныеСистемы.Добавить("Яндекс Пэй");
			ВнедренныеСистемыПодсказка.Добавить("Яндекс Пэй");
		КонецЕсли;
...
//   * ВерсияПодсистемыНаНачалоОбновления - Строка - номер версии подсистемы, к которой относится
//       обработчик обновления, на момент начала обновления приложения. Может быть полезно для
//       ветвления логики выполнения обработчика в зависимости от версии, с которой выполняется обновление приложения.
...
//   * ОбновляемыеДанные - Структура - если у обработчика обновления установлено свойство Многопоточный
//       в значение Истина, то в этом свойстве в обработчик передается порция обновляемых данных,
//       которая должна быть обработана в текущем потоке. Получать таблицу значений обновляемых данных
//       следует с помощью вызова функции
//       ОбновлениеИнформационнойБазы.ДанныеДляОбновленияВМногопоточномОбработчике, а не напрямую.
//
...
//                   "ИнтернетЭквайринг" - для подсистемы "Интернет-эквайринг".
//                   "ОзонПэй" - для платежной системы Ozon Pay.
//                   "ЯндексПэй" - для платежной системы Яндекс Пэй.
//                   "ПлатежныеСистемы" - Ozon Pay и Яндекс Пэй.
...
//
// Параметры:
//  ПлатежнаяСистема - Строка - платежная система. Доступные значения определяются методами
//    НастройкиПлатежныхСистемКлиентСервер.ИмяПодсистемыСБП(),
...
Функция МаксимальноеКоличествоПопытокЗапросаСтатуса(ПлатежнаяСистема, Оплата) Экспорт
	...
	Если ПлатежнаяСистема = НастройкиПлатежныхСистемКлиентСервер.ИмяПодсистемыСБП()
		Или ПлатежнаяСистема = НастройкиПлатежныхСистемКлиентСервер.ИмяПодсистемыИнтернетЭквайринг()
		Или ПлатежнаяСистема = НастройкиПлатежныхСистемКлиентСервер.ИмяПодсистемыПлатежныеСистемы() Тогда
		Если Оплата Тогда
			Возврат 400;
		Иначе
			Возврат 10;
...
//
Функция НовыйИтеративныйВызовОперации(Знач ДлительностьОперации = 0) Экспорт
	
	ДлительностьОперации = ?(
		ДлительностьОперации = 0 Или ДлительностьОперации = Неопределено,
...
Функция ВозможенВызовОперации(НастройкиВызова) Экспорт
	...
	Если НастройкиВызова.Длительность > 2
...
// Проверяет значение длительности операции платежных систем.
// Если значение равно 0, то устанавливает значение по умолчанию - 60 секунд.
// 
// Параметры:
//  Параметры - Произвольный - для подсистемы ОбновлениеВерсииИБ.
//
Процедура УстановитьДлительностьОперацийПоУмолчанию(Параметры = Неопределено) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	ТекущееЗначение = ДлительностьОперацииПлатежныхСистем();
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьДлительностьОперацийПоУмолчанию");
...
Функция ЕстьНастройкиПодключения()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ИнтернетЭквайринг") Тогда
		...
		ЕстьИнтернетЭквайринг = МодульНастройкиПодключенияКИнтернетЭквайрингу.ЕстьНастройкиПодключения();
	КонецЕсли;
	...
	Возврат ЕстьСБП Или ЕстьИнтернетЭквайринг Или ЕстьПлатежныеСистемы;
...
//
// Параметры:
//  НастройкаПодключения - Произвольный - ссылка на элемент справочника 
...
//  Неопределено - если не удалось найти параметры оплаты
//
Функция ПараметрыОплатыПоНастройке(НастройкаПодключения) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПлатежныеСистемы.БазоваяФункциональностьПлатежныхСистем") Тогда
		...
		Если МодульПлатежныеСистемыСлужебный.ЭтоНастройкаПодключения(НастройкаПодключения) Тогда
			Возврат МодульПлатежныеСистемыСлужебный.ПараметрыОплатыПоНастройке(НастройкаПодключения);
		КонецЕсли;
	КонецЕсли;
