...
Процедура ОформитьЗаказ(СтруктураЗаказа, Проводить = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументЗаказ.ТочкаДоставки) Тогда
		
		ДокументЗаказ.ЗонаДоставки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
			ДокументЗаказ.ТочкаДоставки,
			"ЗонаДоставки");
		
	КонецЕсли;
	
	ЭтапыОплатыСервер.ПроверитьЗаполнитьЭтапыГрафикаОплаты(ДокументЗаказ);
...
Процедура УправлениеЭлементомФормыСклад(Объект, Данные, Элемент) Экспорт
	...
	НеПолучатьГруппыСкладовТолькоЗаказах = (ТипЗнч(Объект) <> Тип("ФормаКлиентскогоПриложения") 
												И ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента"));
	
	ЗаполнитьСписокВыбораСклада(Данные, Элемент, НеПолучатьГруппыСкладов, НеПолучатьГруппыСкладовТолькоЗаказах);
...
//  Элемент - ПолеФормы - для которого выполняются настройки.
//  НеПолучатьГруппыСкладов - Булево - не получать группы складов, которые запрещено выбирать в реквизите.
//  НеПолучатьГруппыСкладовТолькоЗаказах - Булево - Не получать группы складов, у которых установлен ВыборГруппы тоолько в заказах.
//
Процедура ЗаполнитьСписокВыбораСклада(Данные, Элемент, НеПолучатьГруппыСкладов, НеПолучатьГруппыСкладовТолькоЗаказах = Ложь) Экспорт
	
	Элемент.СписокВыбора.ЗагрузитьЗначения(ВсеДоступныеСкладыПартнера(
								Данные, НеПолучатьГруппыСкладов, НеПолучатьГруппыСкладовТолькоЗаказах));
...
//
Функция ВсеДоступныеСкладыПартнера(Данные, НеПолучатьГруппыСкладов, НеПолучатьГруппыСкладовТолькоЗаказах)
	...
	Иначе
		Если НеПолучатьГруппыСкладовТолькоЗаказах Тогда
			Запрос.Текст = Запрос.Текст + "
		    	        |	И ВЫБОР КОГДА Склады.ЭтоГруппа ТОГДА НЕ Склады.ВыборГруппы = ЗНАЧЕНИЕ(Перечисление.ВыборГруппыСкладов.РазрешитьВЗаказах) ИНАЧЕ ИСТИНА КОНЕЦ";
		КонецЕсли;
