...
// Процедура проверяет реквизит дерева на ограничения по длине
// 
// Параметры:
//  ДеревоДанных - ДеревоЗначений, СтрокаДереваЗначений - Дерево данных
//  Ошибки - Неопределено, Массив из Структура - ошибки, накопленные за время обработки. см. ОбщегоНазначенияБЭДКлиентСервер.НовыеПараметрыОшибки.
//  ПолноеИмяРеквизита - Строка - Полное имя реквизита
//  МинимальнаяДлина - Число - Минимальная длина
//  МаксимальнаяДлина - Число - Максимальная длина
//  НомерСтроки - Неопределено, Число - Номер строки
//
Процедура ПроверитьДлинуСтрокиЗначенияДерева(ДеревоДанных, Ошибки, ПолноеИмяРеквизита,
	МинимальнаяДлина, МаксимальнаяДлина, НомерСтроки = Неопределено) Экспорт
	
	РеквизитСтрокаДерева = Неопределено;
	Реквизит = ЗначениеРеквизитаВДереве(ДеревоДанных, ПолноеИмяРеквизита, Ложь, РеквизитСтрокаДерева);
	
	Если ЗначениеЗаполнено(Реквизит) Тогда
		
		Если СтрДлина(Реквизит) < МинимальнаяДлина Или МаксимальнаяДлина < СтрДлина(Реквизит) Тогда
			...
			ДобавитьОшибкуПроверкиДереваДанных(Ошибки, РеквизитСтрокаДерева, ТекстОшибки,, НомерСтроки);
...
// Процедура проверяет на заполненность реквизит дерева
// 
// Параметры:
//  ДеревоДанных - СтрокаДереваЗначений - Дерево данных
//  Ошибки - Неопределено
//         - Массив Из см. ОбщегоНазначенияБЭДКлиентСервер.НовыеПараметрыОшибки
//  ПолноеИмяРеквизита - Строка - Полное имя реквизита
//  НомерСтроки - Неопределено, Число - Номер строки
//
Процедура ПроверитьЗаполненностьЗначенияДерева(ДеревоДанных, Ошибки, ПолноеИмяРеквизита, НомерСтроки = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(Реквизит) Тогда
		...
		ДобавитьОшибкуПроверкиДереваДанных(Ошибки, РеквизитСтрокаДерева,
			ТекстОшибкиНеЗаполненногоЗначенияДерева(),, НомерСтроки);
...
// Параметры:
//  Дерево - ДеревоЗначений - дерево данных, в котором необходимо сохранить данные
//  Коллекция - Структура, ВыборкаИзРезультатаЗапроса, СтрокаТаблицыЗначений - коллекция для сохранения в дереве
//  НазваниеТаблицы - Строка - название таблицы в дереве.
//
...
Процедура ДобавитьЗначениеВДерево(ДеревоДанных, ИмяРеквизита, ЗначениеРеквизита) Экспорт
	...
	СтрокаДерева["Уровень" + НомерУровня] = НазваниеКолонки(ИмяРеквизита);
	Если ТипЗнч(ЗначениеРеквизита) = Тип("ДеревоЗначений") Тогда
		СтрокаДерева.Значение = ЗначениеРеквизита.Скопировать();
	Иначе
...
Процедура ПроверитьЗаполнениеРеквизитовДереваДанныхРекурсивно(ДеревоДанных, Ошибки, ДатаПоФорматуФНС = Ложь) Экспорт
	...
	ПараметрыПроверки = ДеревоЭлектронногоДокументаБЭДСлужебный.НовыеПараметрыПроверкиДереваДанных();
	ПараметрыПроверки.Вставить("ДатаПоФорматуФНС", ДатаПоФорматуФНС);
	
	ДеревоЭлектронногоДокументаБЭДСлужебный.ПроверитьЗаполнениеРеквизитовДереваДанных(ДеревоДанных, Ошибки,,, ПараметрыПроверки);
