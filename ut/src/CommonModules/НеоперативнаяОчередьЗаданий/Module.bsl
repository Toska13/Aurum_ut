...
Процедура ЗаписатьОшибкуВЖурналРегистрации(МетаданныеОшибка, ВидЗадания, ТекстОшибки, Документ = Неопределено) Экспорт
	...
	ИмяСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		"ru='Неоперативное допроведение документов (%1)'", ОбщегоНазначения.КодОсновногоЯзыка()), ВидЗадания);
...
Процедура УдалитьЗаданияОчереди(ИмяОчереди, ИдентификаторыЗаданий) Экспорт
	...
	ТаблицаПорцийИдентификаторов = Новый ТаблицаЗначений;
	ТаблицаПорцийИдентификаторов.Колонки.Добавить("НомерПорции", ОбщегоНазначения.ОписаниеТипаЧисло(10,0));
	ТаблицаПорцийИдентификаторов.Колонки.Добавить("ИдентификаторЗадания", Новый ОписаниеТипов("УникальныйИдентификатор"));
	ТаблицаПорцийИдентификаторов.Индексы.Добавить("НомерПорции");
	
	ТекущийНомерПорции = 1;
	РазмерТекущейПорции = 0;
	Для Каждого ТекущийИдентификатор Из ИдентификаторыЗаданий Цикл
		
		РазмерТекущейПорции = РазмерТекущейПорции + 1;
		
		НоваяСтрока = ТаблицаПорцийИдентификаторов.Добавить();
		НоваяСтрока.НомерПорции = ТекущийНомерПорции;
		НоваяСтрока.ИдентификаторЗадания = ТекущийИдентификатор;
		
		Если РазмерТекущейПорции = 1000 Тогда
			ТекущийНомерПорции = ТекущийНомерПорции + 1;
			РазмерТекущейПорции = 0;
		КонецЕсли;
	...
	Для НомерПорции = 1 По ТекущийНомерПорции Цикл
		
		Запрос.УстановитьПараметр("ИдентификаторыЗаданий", ТаблицаПорцийИдентификаторов.Скопировать(
			Новый Структура("НомерПорции", НомерПорции)).ВыгрузитьКолонку("ИдентификаторЗадания"));
		...
		Если ЗаписиЗаданий.Количество() > 0 Тогда
			...
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ИдентификаторыОшибок, ЗаписиЗаданий.ВыгрузитьКолонку(
				"ИдентификаторОшибки"), Истина);
...
// Из регистра очереди в одной транзакции пакетно удаляются записи с пометкой "Выполнено с ошибкой" по переданной таблице заданий с ошибками и
// создаются новые задания в регистре очереди без пометки "Выполнено с ошибкой" по переданной таблице новых заданий к регистрации.
// 
// Параметры:
//  МенеджерРегистраОчереди - РегистрСведенийМенеджер - Менеджер регистра очереди.
//  ТаблицаЗаданийСОшибкамиКУдалению - ТаблицаЗначений - Таблица заданий к удалению:
//   * Измерение - Произвольный - Поле измерения регистра очереди. 
//   В таблице должны быть перечислены поля соответствующие всем измерениям регистра очереди.
//   * ИдентификаторОшибки - УникальныйИдентификатор - Идентификатор ошибки у записи задания.
//  ТаблицаЗаданийКРегистрации - ТаблицаЗначений - Таблица заданий к регистрации (см. ТаблицаФормированияЗаданий в модуле менеджера регистра очереди)
//  ИдентификаторыОшибок - Массив Из УникальныйИдентификатор - Идентификаторы ошибок.
Процедура УдалитьЗаданияСОшибкамиИЗарегистрироватьНовыеБезОшибок(МенеджерРегистраОчереди, ТаблицаЗаданийСОшибкамиКУдалению, ТаблицаЗаданийКРегистрации, ИдентификаторыОшибок) Экспорт
	
	НачатьТранзакцию();

	Попытка

		ИдентификаторыОшибокОчереди = ТаблицаЗаданийСОшибкамиКУдалению.ВыгрузитьКолонку("ИдентификаторОшибки");

		УстановитьПривилегированныйРежим(Истина);
		
		НаборЗаписей = МенеджерРегистраОчереди.СоздатьНаборЗаписей();
		НаборЗаписей.Загрузить(ТаблицаЗаданийСОшибкамиКУдалению);
		НаборЗаписей.Записать(РежимЗамещения.Удаление);
		
		МенеджерРегистраОчереди.СоздатьЗаданияПоДаннымТаблицыЗначений(ТаблицаЗаданийКРегистрации);
	...
	ТаблицаЗаданийСОшибкамиКУдалению.Очистить();
	ТаблицаЗаданийКРегистрации.Очистить();
...
Процедура ДобавитьЭтап_РасчетСуммДокументовВВалютахУчетаИПерепроведениеПоФинансовымРегистрам(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Допроведение документов'"; // @НСтр-1
...
Процедура ОписаниеПроверок_РасчетСуммДокументовВВалютахУчетаИПерепроведениеПоФинансовымРегистрам(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Расхождения в регистрах взаиморасчетов'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Не должно быть документов, по которым имеются расхождения в регистрах взаиморасчетов'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки, НСтр(
		"ru='Расхождения в регистрах взаиморасчетов'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПроверкаЭтапа_РасхождениеСуммВОперативныхИФинансовыхРегистрахВзаиморасчетов(ПараметрыПроверки) Экспорт
	...
	ШаблонПредставленияРегистровРасчетов = НСтр("ru = ' ""%1"" и ""%2""'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ДобавитьЭтап_ОтражениеДвиженийДокументовПоУправленческомуБалансу(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Управленческий баланс'"; // @НСтр-1
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
...
Функция ИдентификаторыПоГрануламРасчета(ТаблицаГранулРасчета, ПолноеИмяРегистраЗаданий, ВидЗадания = Неопределено)
	...
	ТаблицаГранулРасчетаРегистр = МенеджерРегистра.ТаблицаГранулРасчета(); // ТаблицаЗначений
	КолонкиТаблицыГранулРасчета = ТаблицаГранулРасчетаРегистр.Колонки;
...
Процедура ЗафиксироватьОшибкиПоГрануламРасчетаПриАварийномЗавершенииОбработкиДанных(ТаблицаГранулРасчета, ПолноеИмяРегистраЗаданий, ВидЗадания = Неопределено)
	...
	Попытка
		...
		ШаблонОшибки = НСтр("ru='Не удалось обработать данные по виду задания ""%1"" из-за аварийного завершения работы фонового процесса.'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если РезультатыПроверки.ПроблемныеДокументыСуммыВВалютахУчета.Количество() > 0 Тогда
		...
		Для Каждого Документ Из ПроблемныеДокументы Цикл
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Попытка перепроведения документов'", ОбщегоНазначения.КодОсновногоЯзыка()),
