...
Процедура ПриЗаписиКонстантыИспользоватьОбменЭД(Значение) Экспорт
	...
	Если ИспользоватьПолучениеПоКлючуДоступа() Тогда
		Возврат;
	КонецЕсли;
	
	УчетныеЗаписиОтсутствуют = РегистрыСведений.УчетныеЗаписиЭДО.УчетныеЗаписиОтсутствуют();
	Если УчетныеЗаписиОтсутствуют Тогда
		ВключитьИспользованиеТокеновДоступа();
	КонецЕсли;
...
Функция ВключитьПоУчетнымЗаписямЭДО(ИдентификаторыЭДО, РасшифрованныеМаркеры) Экспорт
	...
	Если ЗначениеЗаполнено(ИдентификаторыПоИнтеграциямЭДО.ИдентификаторыДляЛокальногоЭДО) Тогда
		...
		ИспользоватьПолучениеПоТокенамДоступа = АвтоматическоеПолучениеЭДОПовтИсп.ИспользоватьПолучениеПоТокенамДоступа();
		
		Если Не ИспользоватьПолучениеПоТокенамДоступа
			И ДоступноПолучениеПоТокенам()
			И ВключатьАвтоматически() Тогда
			ИспользоватьПолучениеПоТокенамДоступа = ВключитьИспользованиеТокеновДоступа();
		КонецЕсли;
		...
		ИначеЕсли ИспользоватьПолучениеПоТокенамДоступа Тогда
...
// Возвращаемое значение:
//  Булево
Функция ДоступноПолучениеПоТокенам() Экспорт
...
#Область АвтоматическоеВключение
...
// Возвращаемое значение:
//  Булево
Функция ВключатьАвтоматическиДляУчетныхЗаписейЭДО(ИдентификаторыЭДО) Экспорт
	
	Доступность = ДоступноПолучениеПоТокенам();
	...
	ЕстьУчетныеЗаписи = УчетныеЗаписиЭДО.ЕстьУчетныеЗаписиЧерезСервис1СЭДО(ИдентификаторыЭДО);
...
// Возвращаемое значение:
//  Булево
Функция ДоступнаПроверкаПодписи(КонтекстДиагностики = Неопределено)
...
// Возвращаемое значение:
//  Структура:
//  * ЭтоВозобновление - Булево
//  * ИдентификаторыЭДО - Массив из Строка
Функция ПараметрыАвтоматическогоВключения(Сертификаты) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ЭтоВозобновление", Ложь);
	Результат.Вставить("ИдентификаторыЭДО", Новый Массив);
	
	Если Не ДоступноПолучениеПоТокенам() Тогда
		Возврат Результат;
	КонецЕсли;
	
	Использовать = АвтоматическоеПолучениеЭДОПовтИсп.ИспользоватьПолучениеПоКлючуДоступа()
		Или АвтоматическоеПолучениеЭДОПовтИсп.ИспользоватьПолучениеПоТокенамДоступа();
	
	ВключатьАвтоматически = ВключатьАвтоматически();
	Если Не Использовать И Не ВключатьАвтоматически Тогда
		Возврат Результат;
	КонецЕсли;
	
	ИдентификаторыЭДО = УчетныеЗаписиЭДО.ИдентификаторыУчетныхЗаписейЧерезСервис1СЭДОСертификатов(Сертификаты);
	
	Если Использовать Тогда
		Результат.ЭтоВозобновление = Истина;
		Результат.ИдентификаторыЭДО =
			РегистрыСведений.СостоянияАвтоматическогоПолученияЭДО.ОстановленоПоИдентификаторамЭДО(ИдентификаторыЭДО);
		Возврат Результат;
	КонецЕсли;
	
	Если ВключатьАвтоматически Тогда
...
// Возвращаемое значение:
//  Булево
Функция ВключатьАвтоматически()
	
	УстановитьПривилегированныйРежим(Истина);
	ВключатьАвтоматически = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		КлючОбъектаНастроек(), КлючНастройкиВключатьАвтоматически(), Истина);
	УстановитьПривилегированныйРежим(Ложь);
	
	Возврат ВключатьАвтоматически;
	
КонецФункции

// Параметры:
//  Значение - Булево
Процедура УстановитьВключатьАвтоматически(Значение) Экспорт
	
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
		КлючОбъектаНастроек(), КлючНастройкиВключатьАвтоматически(), Значение);
...
// Возвращаемое значение:
//  Строка
Функция КлючНастройкиВключатьАвтоматически()
...
// Возвращаемое значение:
//  Булево
Функция ЕстьВозможностьПолучитьДокументыИзСервисаЭДО(КлючСинхронизации, КонтекстДиагностики)
...
// Возвращаемое значение:
//  Строка
Функция ВидОперацииВключениеИспользованияТокеновДоступаКСервисуЭДО()
...
Функция ВключитьИспользованиеТокеновДоступаПоУчетнымЗаписямЭДО(ИдентификаторыЭДО, РасшифрованныеМаркеры)
	...
	Использовать = АвтоматическоеПолучениеЭДОПовтИсп.ИспользоватьПолучениеПоТокенамДоступа();
	...
	Для Каждого ИдентификаторЭДО Из ИдентификаторыЭДО Цикл
		...
		РасшифрованныйМаркер = РасшифрованныеМаркеры[ИдентификаторЭДО];
		Если РасшифрованныйМаркер = Неопределено Тогда
			Состояние = УстановитьПризнакОстановки(ИдентификаторЭДО, Истина);
		...
		Состояние = УстановитьПризнакОстановки(ИдентификаторЭДО, Ложь);
...
//  
// Возвращаемое значение:
//  См. РегистрыСведений.СостоянияАвтоматическогоПолученияЭДО.УстановитьПризнакОстановки
Функция УстановитьПризнакОстановки(ИдентификаторЭДО, Значение = Ложь)
	
	УстановитьПривилегированныйРежим(Истина);
	Состояние = РегистрыСведений.СостоянияАвтоматическогоПолученияЭДО.УстановитьПризнакОстановки(
		ИдентификаторЭДО, Значение);
	Возврат Состояние;
...
Функция ВключитьИспользованиеТокеновДоступа(КонтекстДиагностики = Неопределено) Экспорт
	...
	Если Не НастройкиЭДО.ИспользуетсяОбменЭлектроннымиДокументами(КонтекстДиагностики)
		Или Не ДоступноПолучениеПоТокенам() Тогда
		Возврат Использовать;
	КонецЕсли;
	
	ВключитьИспользованиеСервисаОблачнойПодписи();
	
	ДоступнаПроверкаПодписи = ДоступнаПроверкаПодписи(КонтекстДиагностики);
	Если Не ДоступнаПроверкаПодписи Тогда
		Возврат Использовать;
	КонецЕсли;
	...
	Попытка
		...
		Если ИдентификаторЭДО = Неопределено Тогда
			...
			Если Использовать Тогда
				...
				УстановитьВключатьАвтоматически(Ложь);
				ОбновитьПовторноИспользуемыеЗначения();
			КонецЕсли;
		Иначе
			РегистрыСведений.СостоянияАвтоматическогоПолученияЭДО.Удалить(ИдентификаторЭДО);
...
Функция Получить(ВидОперации, РасшифрованныеМаркеры = Неопределено)
	...
	ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер("АвтоматическоеПолучениеЭДО");
	Попытка
	...
	Исключение
		...
		ДиагностикаЭДО.ОбработатьОшибки(КонтекстДиагностики);
	КонецПопытки;
	ОбщегоНазначенияБЭД.ЗакончитьЗамер("АвтоматическоеПолучениеЭДО", ОписаниеЗамера, Загружено);
	Возврат Результат;
...
//  * ИтогДействийПоЭДО - см. ЭлектронныеДокументыЭДОКлиентСервер.НовыйИтогВыполненияДействийПоЭДО
//  * КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Функция НовыйРезультатВариантаПолучения()
...
Функция ПолучитьПоУчетнымЗаписямЦентровЭДО()
	...
	Для Каждого ИдентификаторыЭДОПоУчетнойЗаписиЦентраЭДО Из ИдентификаторыЭДОПоУчетнымЗаписямЦентровЭДО Цикл
		...
		ТекстОшибок = ?(Не ЕстьКритичнаяОшибка, "",
			ОбработкаНеисправностейБЭД.ТекстОшибокКонтекстаДиагностики(КонтекстДиагностики));
		
		Для Каждого ИдентификаторЭДО Из ИдентификаторыЭДО Цикл
			СостояниеПолучения = РегистрыСведений.СостоянияАвтоматическогоПолученияЭДО.Записать(
				ИдентификаторЭДО, ЕстьКритичнаяОшибка, ДатаНачала, Неопределено, ТекстОшибок);
			СостояниеПоИдентификаторамЭДО.Вставить(ИдентификаторЭДО, СостояниеПолучения);
		КонецЦикла;
...
Процедура ЗаполнитьРезультатПолученияБезПроверкиПодписи(Результат, ИдентификаторЭДО)
	...
	Остановлено = Ложь;
	СостояниеПолучения = РегистрыСведений.СостоянияАвтоматическогоПолученияЭДО.Записать(
		ИдентификаторЭДО, ЕстьОшибки, ДатаНачала, Остановлено, ТекстСообщения);
...
Процедура ЗаполнитьРезультатПолученияПоИдентификаторуЭДО(Результат, ИдентификаторЭДО, ДатаНачала, РезультатДействийЭДО)
	...
	Если РезультатПроверки.ЕстьКритичныеОшибки Тогда
		Остановлено = Истина;
		ПричинаОстановки = СтрСоединить(РезультатПроверки.ПредставленияОшибок,
	...
	СостояниеПолучения = РегистрыСведений.СостоянияАвтоматическогоПолученияЭДО.Записать(
		ИдентификаторЭДО, РезультатПроверки.ЕстьОшибки, ДатаНачала, Остановлено, ПричинаОстановки);
	Результат.СостояниеПоИдентификаторамЭДО.Вставить(ИдентификаторЭДО, СостояниеПолучения);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВключитьПоУчетнымЗаписямЭДО", Истина);
	Методы.Вставить("ПолучитьПоРасписанию", Истина);
	Методы.Вставить("ПолучитьЧерезИнтервал", Истина);
