...
Функция ОписаниеМетаданныхПроверкиВозможностиВнесенияИзлишкаВНакладную() Экспорт
	...
	Структура.Вставить("ПоказательПроверки", "Оформлено");
...
//  ОписаниеМетаданных	 - см. ОписаниеМетаданныхПроверкиВозможностиВнесенияИзлишкаВНакладную
//  Отказ				 - Булево
//
Процедура ПроверкаВозможностиВнесенияИзлишкаВНакладную(Акт, ИмяРегистра, ОписаниеМетаданных, Отказ) Экспорт
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ТЗИзмененныеТоварыНакладных, СтруктураДействий, Неопределено);
	...
	СтрокиСОшибками = ВыполнитьКонтрольДобавленияИзлишкаВНакладную(ИмяРегистра,
		...
		ОписаниеМетаданных.ПоказательПроверки);
...
// 	КоличествоИзмененныхСтрокСоответствие   - Структура -
...
Функция УстановитьВариантДействияВСтроках(
	Форма,
	...
	Для Каждого ИДСтроки Из Форма.Элементы.Товары.ВыделенныеСтроки Цикл
		...
		Если ЗначениеЗаполнено(ДействиеСДокументомНедостачи)
			...
			ВариантДействияСДокументом = ДействиеСДокументомНедостачи;
		Иначе
			ВариантДействияСДокументом = ДействиеСДокументомИзлишки;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ДействиеСТоваромНедостачи)
			И ТекущаяСтрока.КоличествоУпаковокРасхождения < 0 Тогда
			ВариантДействияСТоваром = ДействиеСТоваромНедостачи;
		ИначеЕсли ЗначениеЗаполнено(ДействиеСТоваромИзлишки)
			И ТекущаяСтрока.КоличествоУпаковокРасхождения> 0 Тогда
			ВариантДействияСТоваром = ДействиеСТоваромИзлишки;
		КонецЕсли;
		
		ИзменитьДействиеВСтроке = РасхожденияКлиентСервер.ИзменитьДействиеВСтроке(
			ТекущаяСтрока,
			ДействиеСДокументомНедостачи,
			ДействиеСДокументомИзлишки,
			ДействиеСТоваромНедостачи,
			ДействиеСТоваромИзлишки,
			ПоВинеСтороннейКомпании);
		
		Если ИзменитьДействиеВСтроке Тогда
			КоличествоИзмененныхСтрокСоответствие[ВариантДействияСДокументом] = КоличествоИзмененныхСтрокСоответствие[ВариантДействияСДокументом] + 1;
			Если ЗначениеЗаполнено(ВариантДействияСТоваром) Тогда
				КоличествоИзмененныхСтрокСоответствие[ВариантДействияСТоваром] = КоличествоИзмененныхСтрокСоответствие[ВариантДействияСТоваром] + 1;
			КонецЕсли;
...
Процедура АктОРасхожденияхСписокОбработатьПараметрОснования(Форма, ЭтоАктыОРасхожденияПослеОтгрузки) Экспорт
	...
	ИначеЕсли Форма.ТипОснованияАктаОРасхождении =
		...
		Форма.Заголовок = НСтр("ru = 'Акт о расхождениях после отгрузки товаров клиенту и до перехода права собственности'");
		
		ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы,
			"Партнер", "Заголовок", НСтр("ru = 'Клиент'"));
		
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Форма.Список,
			"СтатусИзменение", НСтр("ru = 'Изменение отгрузки товаров клиенту'"));
...
//
// Параметры:
//  ДокументАктОбъект  - ДокументОбъект.АктОРасхожденияхПослеОтгрузки, ДокументОбъект.АктОРасхожденияхПослеОтгрузки - документ, для которого выполняется проверка типа
//
Процедура ПроверитьТипОснованияАктаПриСоздании(ДокументАктОбъект) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДокументАктОбъект.ТипОснованияАктаОРасхождении) Тогда
		...
		ВызватьИсключение НСтр("ru = 'При создании документа не определен тип основания акта о расхождениях.'");
	...
	Иначе
		...
		Если РасхожденияКлиентСервер.ТипОснованияРеализацияТоваровУслуг(Форма.Объект.ТипОснованияАктаОРасхождении) Тогда
			...
			Если ПравоДоступа("Чтение", Метаданные.Документы.ОтгрузкаТоваровКлиенту) Тогда
				...
				|	И РеализацияТоваровУслугТовары.Ссылка В(&ДокументыОснования)
		...
		ИначеЕсли РасхожденияКлиентСервер.ТипОснованияОтгрузкаТоваровКлиенту(Форма.Объект.ТипОснованияАктаОРасхождении) Тогда
			...
			|		ИНАЧЕ ВЫРАЗИТЬ(ОтгрузкаТоваровКлиентуТовары.Сумма / ОтгрузкаТоваровКлиентуТовары.КоличествоУпаковок КАК ЧИСЛО(31, 2))
			|	КОНЕЦ                                               КАК Цена,
			...
			|	0                                                   КАК СуммаНДС,
			|	ОтгрузкаТоваровКлиентуТовары.Сумма                  КАК СуммаСНДС,
			...
			|	ОтгрузкаТоваровКлиентуТовары.Сумма                  КАК СуммаСНДСПоДокументу,
			...
			|ГДЕ
			|	ОтгрузкаТоваровКлиентуТовары.Ссылка В (&ДокументыОснования)
			|	И ОтгрузкаТоваровКлиентуТовары.Номенклатура.ТипНоменклатуры В (&ТипыНоменклатуры)
			|УПОРЯДОЧИТЬ ПО
			|	Реализация,
			|	НомерСтроки
			|;
			...
			|ГДЕ
			|	ОтгрузкаТоваровКлиентуСерии.Ссылка В (&ДокументыОснования)
			|	И ОтгрузкаТоваровКлиентуСерии.Номенклатура.ТипНоменклатуры В (&ТипыНоменклатуры)";
		...
		ИначеЕсли РасхожденияКлиентСервер.ТипОснованияПоступлениеТоваровОтХранителя(Форма.Объект.ТипОснованияАктаОРасхождении)
			...
			|	0                                 КАК СуммаНДСПоДокументу,
			|	ТаблицаТовары.Сумма               КАК СуммаСНДСПоДокументу,
		...
		ИначеЕсли Форма.Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратТоваровОтКлиента Тогда
			...
			|	ВозвратТоваровОтКлиентаТовары.Склад КАК Склад
			|ИЗ
		...
		РезультатЗапросаТовары = РезультатыЗапроса[РезультатыЗапроса.ВГраница()-1];
		РезультатЗапросаСерии = РезультатыЗапроса[РезультатыЗапроса.ВГраница()];
		Если Форма.ВариантЗаполнения = "Вариант1" Тогда
			Если РезультатЗапросаТовары.Пустой() Тогда
				Форма.Объект.Товары.Очистить();
				Форма.Объект.Серии.Очистить();
			Иначе
				Форма.Объект.Серии.Загрузить(РезультатЗапросаСерии.Выгрузить());
				Форма.Объект.Товары.Загрузить(РезультатЗапросаТовары.Выгрузить());
			КонецЕсли;
		ИначеЕсли Форма.ВариантЗаполнения = "Вариант2" Тогда
			...
			ТаблицаТоварыОснования = РезультатЗапросаТовары.Выгрузить();
			Если ТаблицаТоварыОснования.Колонки.Найти("Реализация") <> Неопределено Тогда
				ЗаполнятьРеализацию = Истина;
				ЗаполнятьПодразделение = РасхожденияКлиентСервер.ТипОснованияОтгрузкаТоваровКлиенту(Форма.Объект.ТипОснованияАктаОРасхождении)
					Или (РасхожденияКлиентСервер.ТипОснованияРеализацияТоваровУслуг(Форма.Объект.ТипОснованияАктаОРасхождении)
						И Форма.Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности);
			Иначе
				ЗаполнятьРеализацию = Ложь;
			КонецЕсли;
			
			КэшРеквизитовУпаковки = Новый Соответствие;
				...
				Если НайденныеСтрокиДокумента.Количество() > 0 Тогда
					Для Каждого СтрТЧ Из НайденныеСтрокиДокумента Цикл
						...
						Если ЗначениеЗаполнено(СтрТЧ.Упаковка) Тогда;
							КлючКэша = Строка(СтрТЧ.Номенклатура.УникальныйИдентификатор()) + "_" + Строка(СтрТЧ.Упаковка.УникальныйИдентификатор());
							Если КэшРеквизитовУпаковки.Получить(КлючКэша) = Неопределено Тогда
								...
								КэшРеквизитовУпаковки.Вставить(КлючКэша, РеквизитыУпаковки);
							Иначе
								РеквизитыУпаковки = КэшРеквизитовУпаковки.Получить(КлючКэша);
							КонецЕсли;
						Иначе
						...
						Если ЗаполнятьРеализацию Тогда
							...
							СтрТЧ.ЗаказКлиента = СтрТабл.ЗаказКлиента;
							СтрТЧ.КодСтроки    = СтрТабл.КодСтроки;
							Если ЗаполнятьПодразделение Тогда
								СтрТЧ.Подразделение = СтрТабл.Подразделение;
							КонецЕсли;
						Иначе
...
Функция ПолучитьЗагруженныеТоварыИзХранилища(ТаблицаТовары, ДокументыОснования ,АдресТоваровВХранилище) Экспорт
...
// 	ЗаполнитьСтавкуНДСПоВидуЦен - Булево - признак заполнения ставки НДС по виду цены.
// 	
Процедура ОбработкаВыбораПодборНаСервере(Форма,
										КэшированныеЗначения,
										ВыбранноеЗначение,
										ЭтоАктОРасхожденияПослеОтгрузки,
										ЗаполнитьСтавкуНДСПоВидуЦен = Ложь) Экспорт
	...
	Если РасхожденияКлиентСервер.ТипОснованияРеализацияТоваровУслуг(Форма.Объект.ТипОснованияАктаОРасхождении)
		Или РасхожденияКлиентСервер.ТипОснованияОтгрузкаТоваровКлиенту(Форма.Объект.ТипОснованияАктаОРасхождении)
	...
	Если Не УчетНДСНеВедется Тогда
		...
		Если ЗаполнитьСтавкуНДСПоВидуЦен Тогда
			СтруктураДействий.Вставить("ЗаполнитьСтавкуНДСПоВидуЦен", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДСПоВидуЦен(Форма.Объект));
		КонецЕсли;
	КонецЕсли;
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		ТекущаяСтрока = ТЧТовары.Добавить();
	...
	Если ТипЗнч(Форма.Объект.Ссылка) = Тип("ДокументСсылка.АктОРасхожденияхПослеПриемки") Тогда
		СтруктураДействий.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", Форма.КонтрольКачестваКэш);
	КонецЕсли;
...
Функция СтрокаАктаСодержитДействияТребующиеОбработки(ТекСтрока, ТипАкта, ТипОперации = "")
	...
	Если ТипАкта = Тип("ДокументСсылка.АктОРасхожденияхПослеОтгрузки") Тогда
		
		Если ТекСтрока.ДействиеСТоваром = ПредопределенноеЗначение("Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.Ожидать") Тогда
			Возврат Ложь;
		КонецЕсли;
		...
		Если ТекСтрока.Действие = ПредопределенноеЗначение("Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ПустаяСсылка")
				...
				Или ТекСтрока.Действие = ПредопределенноеЗначение("Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ОтгрузитьСейчас")
					И ТекСтрока.ДействиеСТоваром = ПредопределенноеЗначение("Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.Списать") Тогда
	...
	ИначеЕсли ТипАкта = Тип("ДокументСсылка.АктОРасхожденияхПослеПеремещения") Тогда
...
Процедура СформироватьНадписьСпособаОтраженияРасхождений(Форма, Товары, НадписьРасхождения, НадписьОформитьДокументы, ТипАкта, ТипОперации = "") Экспорт
	...
	Если КоличествоСтрокРасхождений > 0 Тогда
		...
		НСтр("ru='строке, строках, строках, ж'"));
...
Процедура АктОРасхожденияхУстановитьУсловноеОформлениеФормыДокумента(Форма) Экспорт
	...
	ПараметрыУсловногоОформления.Форма = Форма;
...
Процедура ЗаполнитьДоговорПоУмолчанию(Объект, ХозяйственнаяОперацияДоговора) Экспорт
	...
	ИначеЕсли РасхожденияКлиентСервер.ТипОснованияОтгрузкаТоваровКлиенту(Объект.ТипОснованияАктаОРасхождении) Тогда
		
		Объект.Договор = ПродажиСервер.ПолучитьДоговорПоУмолчанию(Объект,
			ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.РеализацияКлиенту"), Объект.Валюта);
...
Процедура СформироватьТаблицуДокументовОснований(Форма, МассивДокументовОснований = Неопределено) Экспорт
	...
	Если РасхожденияКлиентСервер.ТипОснованияРеализацияТоваровУслуг(Форма.Объект.ТипОснованияАктаОРасхождении) Тогда
		...
		ЕстьПравоЧтенияОтгрузкиТоваровКлиента = ПравоДоступа("Чтение", Метаданные.Документы.ОтгрузкаТоваровКлиенту);
		
		Если ЕстьПравоЧтенияОтгрузкиТоваровКлиента Тогда
			...
			|ГДЕ
			|	Таблица.Ссылка.Проведен
			|	И РеализацияТоваровУслугТовары.Ссылка В(&МассивОснований)
			|	И РеализацияТоваровУслугТовары.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
			|
			|СГРУППИРОВАТЬ ПО
			|	РеализацияТоваровУслугТовары.ЗаказКлиента,
		...
		ТекстыЗапросаТЧТовары = Новый Массив;
		...
		|	И РеализацияТоваровУслуг.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
		|";
		
		ТекстыЗапросаТЧТовары.Добавить(ТекстЗапроса);
		...
		ТекстЗапроса = СтрСоединить(ТекстыЗапросаТЧТовары, ОбщегоНазначения.ТекстОбъединитьВсе());
	...
	ИначеЕсли РасхожденияКлиентСервер.ТипОснованияОтгрузкаТоваровКлиенту(Форма.Объект.ТипОснованияАктаОРасхождении)
		Тогда
		
		Запрос.Текст = "
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		|	ОтгрузкаТоваровКлиенту.Ссылка                                                              КАК Реализация,
		|	ОтгрузкаТоваровКлиенту.Дата                                                                КАК Дата,
		|	ЕСТЬNULL(ОтгрузкаТоваровКлиентуТовары.ЗаказКлиента, НЕОПРЕДЕЛЕНО)                          КАК ЗаказКлиента,
		|	ЕСТЬNULL(ОтгрузкаТоваровКлиентуТовары.Склад, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка))     КАК Склад,
		|	ЕСТЬNULL(НаправленияДеятельности.Назначение, ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)) КАК Назначение
		|ИЗ
		|	Документ.ОтгрузкаТоваровКлиенту КАК ОтгрузкаТоваровКлиенту
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтгрузкаТоваровКлиенту.Товары КАК ОтгрузкаТоваровКлиентуТовары
		|		ПО (ОтгрузкаТоваровКлиентуТовары.Ссылка = ОтгрузкаТоваровКлиенту.Ссылка)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НаправленияДеятельности КАК НаправленияДеятельности
		|		ПО ОтгрузкаТоваровКлиенту.НаправлениеДеятельности = НаправленияДеятельности.Ссылка
		|			И НаправленияДеятельности.УчетЗатрат
		|ГДЕ
		|	ОтгрузкаТоваровКлиенту.Ссылка В(&МассивОснований)
		|ИТОГИ ПО
		|	Реализация";
	...
	ИначеЕсли РасхожденияКлиентСервер.ТипОснованияПоступлениеТоваровОтХранителя(Форма.Объект.ТипОснованияАктаОРасхождении)
		...
		|	ЕСТЬNULL(ПоступлениеТоваровОтХранителяТовары.Распоряжение, НЕОПРЕДЕЛЕНО) КАК ЗаказКлиента,
		...
		|	ПО ПоступлениеТоваровОтХранителя.Ссылка = ПоступлениеТоваровОтХранителяТовары.Ссылка
		|	
		|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НаправленияДеятельности КАК НаправленияДеятельности
...
Процедура УправлениеВидимостьюНДС(Форма) Экспорт
	...
	ВидимостьНалогообложениеНДС =
		УчетНДСУПКлиентСервер.ОтображатьНДСВИтогахДокументаПродажи(Объект.НалогообложениеНДС)
		И Не РасхожденияКлиентСервер.ТипОснованияОтгрузкаТоваровКлиенту(Объект.ТипОснованияАктаОРасхождении);
	
	ВидимостьСуммаСНДС =  НЕ Объект.ЦенаВключаетНДС;
...
Процедура УстановитьЗначенияКешируемыхРеквизитовФормыАкта(Форма) Экспорт
	...
	Форма.ИспользоватьОтгрузкуБезПереходаПраваСобственности =
...
Процедура ФормированиеНадписиДокументыОснование(Форма, ТребуетсяОбновлениеСпискаОснований = Истина) Экспорт
	...
	Иначе
		...
		Иначе
			...
			ИначеЕсли РасхожденияКлиентСервер.ТипОснованияОтгрузкаТоваровКлиенту(
					Форма.Объект.ТипОснованияАктаОРасхождении) Тогда
				
				ТекстПредставление = ТекстПредставление + " " + НСтр("ru = 'отгрузок товаров'");
...
Процедура УправлениеСтраницамиВыборДействия(Форма, Параметры) Экспорт
	...
	Иначе
		...
		Для Каждого РеквизитФормы Из РеквизитыФормы Цикл
			...
			ЭтоРеквизитДействие =
				ИмяРеквизита = "ДействиеИзлишки"
				Или ИмяРеквизита = "ДействиеНедостачи"
				Или ИмяРеквизита = "ДействиеСДокументомНедостачи"
				Или ИмяРеквизита = "ДействиеСДокументомИзлишки"
				Или ИмяРеквизита = "ДействиеСТоваромНедостачи"
				Или ИмяРеквизита = "ДействиеСТоваромИзлишки";
			
			Если ЭтоРеквизитДействие Тогда
		...
		Если Форма.ИмяФормы = "Обработка.РаботаСАктамиРасхождений.Форма.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки" Тогда
			Форма.ДействиеСДокументомИзлишки = Параметры.ДействиеСДокументомИзлишки;
			Форма.ДействиеСДокументомНедостачи = Параметры.ДействиеСДокументомНедостачи;
			Форма.ДействиеСТоваромИзлишки = Параметры.ДействиеСТоваромИзлишки;
			Форма.ДействиеСТоваромНедостачи = Параметры.ДействиеСТоваромНедостачи;
		Иначе
...
Процедура ИзменитьОснованиеАкта(АктОРасхождениях, Основание, ТаблицаИзменяемыхСтрок, ТаблицаИзменяемыхСерий)
	...
	СтруктураДействий = СтруктураДействийПоТЧОснованияАкта(ДокументОснование);
	...
	КэшированныеЗначения = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
	ХозяйственнаяОперацияАкта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(АктОРасхождениях, "ХозяйственнаяОперация");
	...
	Для Каждого СтрокаКИзменениюПоАкту Из ТаблицаИзменяемыхСтрок Цикл
		...
		Если НайденныеСтроки.Количество() = 0
			И ТипЗнч(Основание) = Тип("ДокументСсылка.ПоступлениеТоваровОтХранителя")
			И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Основание, "ВозвратПорчи") Тогда
			
			ПараметрыПоиска.Удалить("Номенклатура");
			ПараметрыПоиска.Удалить("Характеристика");
			
			ПараметрыПоиска.Вставить("НоменклатураОприходование", СтрокаКИзменениюПоАкту.Номенклатура);
			ПараметрыПоиска.Вставить("ХарактеристикаОприходование", СтрокаКИзменениюПоАкту.Характеристика);
		...
		Если НайденныеСтроки <> Неопределено Тогда
			...
			ИначеЕсли НайденныеСтроки.Количество() = 0 Тогда
				...
				Если ХозяйственнаяОперацияАкта = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
					ИзменяемаяСтрока.Склад = Неопределено;
				КонецЕсли;
				ИзменяемаяСтрока.КоличествоУпаковок = СтрокаКИзменениюПоАкту.КоличествоУпаковокПоАкту;
				...
				ОбрабатываемыеСтроки.Добавить(ИзменяемаяСтрока);
				ДобавленныеСтроки.Добавить(ИзменяемаяСтрока);
				...
				Если КоличествоКИзменению < 0 Тогда
					...
					Для Каждого ИзменяемаяСтрока Из НайденныеСтроки Цикл
						...
						Иначе
							...
							КоличествоУпаковокДоИзменения = ИзменяемаяСтрока.КоличествоУпаковок;
							ИзменяемаяСтрока.КоличествоУпаковок = ИзменяемаяСтрока.КоличествоУпаковок
	...
	Если ЕстьКоличествоПоРНПТ Тогда
		...
		Для Каждого ИзменяемаяСтрока Из ДобавленныеСтроки Цикл
...
Функция ПараметрыПоискаПоДокументуОснованию(Основание, СтрокаКИзменениюПоАкту)
	...
	ХозяйственнаяОперация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Основание, "ХозяйственнаяОперация");
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		Или ТипЗнч(Основание) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту")
		...
		ИначеЕсли (ТипЗнч(Основание) = Тип("ДокументСсылка.ПередачаТоваровХранителю")
				Или ТипЗнч(Основание) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту"))
			И СтрокаКИзменениюПоАкту.СтатусУказанияСерий <> 0 Тогда
...
Функция ПараметрыПоискаПоТЧСерииДокументаОснования(Основание, СтрокаКИзменениюПоАкту)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		...
		Или ТипЗнч(Основание) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг")
		Или ТипЗнч(Основание) = Тип("ДокументСсылка.ВозвратТоваровОтКлиента")
...
Функция СтруктураДействийПоТЧОснованияАкта(ДокументОснование) Экспорт
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		ДополнитьСтруктуруДействийПоТЧТоварыРеализации(СтруктураДействий, ДокументОснование);

	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда
		
		ДополнитьСтруктуруДействийПоТЧТоварыОтгрузкиТоваровКлиенту(СтруктураДействий, ДокументОснование);
		
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
		
		ДополнитьСтруктуруДействийПоТЧТоварыПоступления(СтруктураДействий, ДокументОснование);
		
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПоступлениеТоваровНаСклад") Тогда
		
		ДополнитьСтруктуруДействийПоТЧТоварыПоступленияНаСклад(СтруктураДействий, ДокументОснование);
		
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПриемкаТоваровНаХранение") Тогда
		
		ДополнитьСтруктуруДействийПоТЧТоварыПриемкиТоваровНаХранение(СтруктураДействий, ДокументОснование);
		
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПередачаТоваровХранителю") Тогда
		
		ДополнитьСтруктуруДействийПоТЧТоварыПередачаТоваровХранителю(СтруктураДействий, ДокументОснование);
		
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ВозвратТоваровПоставщику") Тогда
		
		ДополнитьСтруктуруДействийПоТЧТоварыВозвратаПоставщику(СтруктураДействий, ДокументОснование);
		
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ВозвратТоваровОтКлиента") Тогда
		
		ДополнитьСтруктуруДействийПоТЧТоварыВозвратОтКлиента(СтруктураДействий, ДокументОснование);
		
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
		
		ДополнитьСтруктуруДействийПоТЧТоварыПеремещение(СтруктураДействий, ДокументОснование);
		
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ОтгрузкаТоваровСХранения") Тогда
		
		ДополнитьСтруктуруДействийПоТЧТоварыОтгрузкаХранителю(СтруктураДействий, ДокументОснование);
		
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПоступлениеТоваровОтХранителя") Тогда
		
		ДополнитьСтруктуруДействийПоТЧТоварыПоступлениеОтХранителя(СтруктураДействий, ДокументОснование);
...
Процедура ДополнитьСтруктуруДействийПоТЧТоварыОтгрузкиТоваровКлиенту(СтруктураДействий, Объект)
...
Процедура ДополнитьСтруктуруДействийПоТЧТоварыПеремещение(СтруктураДействий, Объект)
...
Функция  ШапкаАктаИОснованиеСоответствуют(РезультатЗапросаПоШапке, АктОРасхождениях, Основание) 
	...
	Если ТипЗнч(АктОРасхождениях) = Тип("ДокументСсылка.АктОРасхожденияхПослеОтгрузки")
		...
		ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда
			...
			Если НЕ РасхожденияКлиентСервер.ТипОснованияОтгрузкаТоваровКлиенту(
				ВыборкаАкт.ТипОснованияАктаОРасхождении) Тогда
				Возврат Ложь;
			КонецЕсли;
		...
		Если ВыборкаАкт.Организация              <> ВыборкаОснование.Организация
			...
			Или (ВыборкаАкт.ЦенаВключаетНДС       <> ВыборкаОснование.ЦенаВключаетНДС
				И ВыборкаОснование.ЦенаВключаетНДС <> Неопределено)
...
Процедура ПроверитьТабличныеЧастиАктаИОснования(АктОРасхождениях, Основание, СтруктураВозврата, ИзменениеДокумента)
	...
	Если ТипЗнч(АктОРасхождениях) = Тип("ДокументСсылка.АктОРасхожденияхПослеОтгрузки") Тогда
		...
		ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда
			...
			Запрос.Текст = ТекстЗапросаТабличныеЧастиАктаОРасхожденияхПослеОтгрузкиИОтгрузкиТоваров(
				АктОРасхождениях, Основание, СтруктураВозврата);
...
Функция ТекстЗапросаПоШапкеОснования(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда
		...
		Возврат ТекстЗапросаПоШапкеОтгрузкаТоваровКлиенту(Основание);
...
Функция ТекстЗапросаПоШапкеОтгрузкаТоваровКлиенту(Основание)
	...
	|	ОтгрузкаТоваровКлиенту.Проведен              КАК Проведен
	|ИЗ
	|	Документ.ОтгрузкаТоваровКлиенту КАК ОтгрузкаТоваровКлиенту
	|ГДЕ
...
Функция ТекстЗапросаТабличныеЧастиАктаОРасхожденияхПослеОтгрузкиИВозвратаПоставщику(АктОРасхождениях, Основание, СтруктураВозврата, ИзменениеДокумента)
	...
	|	ВозвратТоваровПоставщикуСерии.Склад,
	...
	|				И АктОРасхожденияхПослеОтгрузкиТовары.Склад = ВозвратТоваровПоставщикуТовары.Склад
	|	
	|	СГРУППИРОВАТЬ ПО
	|		АктОРасхожденияхПослеОтгрузкиТовары.Назначение,
	|		АктОРасхожденияхПослеОтгрузкиТовары.Характеристика,
	...
	|				И ВозвратТоваровПоставщикуТовары.Склад = АктОРасхожденияхПослеОтгрузкиТовары.Склад
	|	ГДЕ
	...
	|				И АктОРасхожденияхСерии.Склад = ВозвратПоставщикуСерии.Склад
	|	
	|	СГРУППИРОВАТЬ ПО
	...
	|				И ВозвратПоставщикуСерии.Склад = АктОРасхожденияхСерии.Склад
	|	ГДЕ
...
Функция ТекстЗапросаТабличныеЧастиАктаОРасхожденияхПослеОтгрузкиИРеализации(АктОРасхождениях, Основание, СтруктураВозврата)
	...
	|	СУММА(ВЫБОР
	|			КОГДА АктОРасхожденияхПослеОтгрузкиТовары.Действие В (
	...
	|						)
	|				И АктОРасхожденияхПослеОтгрузкиТовары.ДействиеСТоваром В (
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.УчестьПриИнвентаризации),
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.Списать),
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.СписатьИзПути),
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.Вернуть),
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.ОприходоватьВПуть)
	|						)
	|				ТОГДА АктОРасхожденияхПослеОтгрузкиТовары.КоличествоУпаковок
	|			ИНАЧЕ АктОРасхожденияхПослеОтгрузкиТовары.КоличествоУпаковокПоДокументу
	|		КОНЕЦ) КАК КоличествоУпаковок,
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	АктОРасхожденияхПослеОтгрузкиТовары.Ссылка.ХозяйственнаяОперация,
	|	АктОРасхожденияхПослеОтгрузкиТовары.Склад,
	|	АктОРасхожденияхПослеОтгрузкиТовары.Упаковка,
	|	АктОРасхожденияхПослеОтгрузкиТовары.Серия,
	|	АктОРасхожденияхПослеОтгрузкиТовары.СтавкаНДС,
	...
	|	АктОРасхожденияхПослеОтгрузкиСерии.Склад КАК Склад,
	|	СУММА(ВЫБОР
	|			КОГДА АктОРасхожденияхПослеОтгрузкиСерии.Действие В (
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ПокупкаПерепоставленного),
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ВозвратПерепоставленного),
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ДопоставкаНеТребуется),
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ТребуетсяДопоставка)
	|						)
	|				И АктОРасхожденияхПослеОтгрузкиСерии.ДействиеСТоваром В (
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.УчестьПриИнвентаризации),
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.Списать),
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.СписатьИзПути),
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.Вернуть),
	...
	|	АктОРасхожденияхПослеОтгрузкиСерии.Ссылка = &АктОРасхождениях
	|	И АктОРасхожденияхПослеОтгрузкиСерии.Реализация = &Основание
	|	И АктОРасхожденияхПослеОтгрузкиСерии.Ссылка.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|
	|СГРУППИРОВАТЬ ПО
	|	АктОРасхожденияхПослеОтгрузкиСерии.Склад,
	|	АктОРасхожденияхПослеОтгрузкиСерии.Серия,
	...
	|					ИЛИ АктОРасхожденияхПослеОтгрузкиТовары.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности))
	...
	|					ИЛИ РеализацияТоваровУслугТовары.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности))
...
Функция ТекстЗапросаТабличныеЧастиАктаОРасхожденияхПослеОтгрузкиИОтгрузкиТоваров(АктОРасхождениях, Основание, СтруктураВозврата)
	...
	|	АктОРасхожденияхПослеОтгрузкиТовары.Склад               КАК Склад,
	|	АктОРасхожденияхПослеОтгрузкиТовары.Серия               КАК Серия,
	|	АктОРасхожденияхПослеОтгрузкиТовары.СтатусУказанияСерий КАК СтатусУказанияСерий,
	|	МАКСИМУМ(АктОРасхожденияхПослеОтгрузкиТовары.Цена)      КАК Цена,
	|	МАКСИМУМ(АктОРасхожденияхПослеОтгрузкиТовары.ВидЦены)   КАК ВидЦены,
	|	СУММА(ВЫБОР
	|			КОГДА АктОРасхожденияхПослеОтгрузкиТовары.Действие В (
	...
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ТребуетсяДопоставка)
	|						)
	|				И АктОРасхожденияхПослеОтгрузкиТовары.ДействиеСТоваром = ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.УчестьПриИнвентаризации)
	|				ТОГДА АктОРасхожденияхПослеОтгрузкиТовары.КоличествоУпаковок
	|			ИНАЧЕ АктОРасхожденияхПослеОтгрузкиТовары.КоличествоУпаковокПоДокументу
	|	КОНЕЦ)                                                  КАК КоличествоУпаковок,
	|	СУММА(АктОРасхожденияхПослеОтгрузкиТовары.КоличествоУпаковокПоДокументу) КАК КоличествоУпаковокПоДокументу
	...
	|	АктОРасхожденияхПослеОтгрузкиТовары.Ссылка = &АктОРасхождениях
	|	И АктОРасхожденияхПослеОтгрузкиТовары.Реализация = &Основание
	|	И АктОРасхожденияхПослеОтгрузкиТовары.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтгрузкаТоваровКлиенту)
	|
	|СГРУППИРОВАТЬ ПО
	|	АктОРасхожденияхПослеОтгрузкиТовары.Номенклатура,
	|	АктОРасхожденияхПослеОтгрузкиТовары.Характеристика,
	...
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ТребуетсяДопоставка)
	|						)
	|				И АктОРасхожденияхПослеОтгрузкиСерии.ДействиеСТоваром = ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоТоварамВАктеПослеОтгрузки.УчестьПриИнвентаризации)
	|				ТОГДА АктОРасхожденияхПослеОтгрузкиСерии.Количество
	|			ИНАЧЕ АктОРасхожденияхПослеОтгрузкиСерии.КоличествоПоДокументу
	|		КОНЕЦ)                                        КАК Количество,
	|	СУММА(АктОРасхожденияхПослеОтгрузкиСерии.КоличествоПоДокументу) КАК КоличествоПоДокументу
	...
	|//////////////////////////////////////////////////////////////////////////////// 2
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ОтгрузкаТоваровКлиентуТовары.Ссылка.ОтгрузкаПоЗаказам     КАК ОтгрузкаПоЗаказам,
	|	ОтгрузкаТоваровКлиентуТовары.Номенклатура                 КАК Номенклатура,
	|	ОтгрузкаТоваровКлиентуТовары.Характеристика               КАК Характеристика,
	|	ОтгрузкаТоваровКлиентуТовары.Назначение                   КАК Назначение,
	|	ОтгрузкаТоваровКлиентуТовары.Упаковка                     КАК Упаковка,
	...
	|	ОтгрузкаТоваровКлиентуТовары.Склад                        КАК Склад,
	|	ВЫБОР
	|		КОГДА ОтгрузкаТоваровКлиентуТовары.СтатусУказанияСерийНаСкладах = 0
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|		ИНАЧЕ
	|			ОтгрузкаТоваровКлиентуТовары.Серия
	|	КОНЕЦ                                                     КАК Серия,
	|	ОтгрузкаТоваровКлиентуТовары.СтатусУказанияСерийНаСкладах КАК СтатусУказанияСерий,
	|	СУММА(ОтгрузкаТоваровКлиентуТовары.КоличествоУпаковок)    КАК КоличествоУпаковок
	|ПОМЕСТИТЬ ОтгрузкаТоваровКлиенту
	|ИЗ
	|	Документ.ОтгрузкаТоваровКлиенту.Товары КАК ОтгрузкаТоваровКлиентуТовары
	|ГДЕ
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ОтгрузкаТоваровКлиентуТовары.Ссылка.ОтгрузкаПоЗаказам,
	|	ОтгрузкаТоваровКлиентуТовары.Номенклатура,
	|	ОтгрузкаТоваровКлиентуТовары.Характеристика,
	|	ОтгрузкаТоваровКлиентуТовары.Назначение,
	|	ОтгрузкаТоваровКлиентуТовары.Упаковка,
	|	ОтгрузкаТоваровКлиентуТовары.ЗаказКлиента,
	|	ОтгрузкаТоваровКлиентуТовары.КодСтроки,
	|	ОтгрузкаТоваровКлиентуТовары.Склад,
	|	ВЫБОР
	|		КОГДА ОтгрузкаТоваровКлиентуТовары.СтатусУказанияСерийНаСкладах = 0
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|		ИНАЧЕ
	|			ОтгрузкаТоваровКлиентуТовары.Серия
	|	КОНЕЦ,
	|	ОтгрузкаТоваровКлиентуТовары.СтатусУказанияСерийНаСкладах
	|;
	|
	|//////////////////////////////////////////////////////////////////////////////// 3
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|	ОтгрузкаТоваровКлиентуСерии.Склад             КАК Склад,
	|	ОтгрузкаТоваровКлиентуСерии.Серия             КАК Серия,
	|	СУММА(ОтгрузкаТоваровКлиентуСерии.Количество) КАК Количество
	|ПОМЕСТИТЬ ОтгрузкаТоваровКлиентуСерии
	|ИЗ
	|	Документ.ОтгрузкаТоваровКлиенту.Серии КАК ОтгрузкаТоваровКлиентуСерии
	|ГДЕ
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ОтгрузкаТоваровКлиентуСерии.Номенклатура,
	|	ОтгрузкаТоваровКлиентуСерии.Характеристика,
	|	ОтгрузкаТоваровКлиентуСерии.Назначение,
	|	ОтгрузкаТоваровКлиентуСерии.Склад,
	|	ОтгрузкаТоваровКлиентуСерии.Серия
	|;
	|
	|//////////////////////////////////////////////////////////////////////////////// 4
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|		СУММА(АктОРасхожденияхПослеОтгрузкиТовары.КоличествоУпаковок)            КАК КоличествоУпаковокПоАкту,
	|		СУММА(АктОРасхожденияхПослеОтгрузкиТовары.КоличествоУпаковокПоДокументу) КАК КоличествоУпаковокПоОснованиюАкт,
	|		СУММА(ЕСТЬNULL(ОтгрузкаТоваровКлиентуТовары.КоличествоУпаковок, 0))      КАК КоличествоУпаковокОснование
	|	ИЗ
	|		АктОРасхожденияПриОтгрузке КАК АктОРасхожденияхПослеОтгрузкиТовары
	|			ЛЕВОЕ СОЕДИНЕНИЕ ОтгрузкаТоваровКлиенту КАК ОтгрузкаТоваровКлиентуТовары
	|			ПО АктОРасхожденияхПослеОтгрузкиТовары.Номенклатура = ОтгрузкаТоваровКлиентуТовары.Номенклатура
	|				И АктОРасхожденияхПослеОтгрузкиТовары.Характеристика = ОтгрузкаТоваровКлиентуТовары.Характеристика
	|				И АктОРасхожденияхПослеОтгрузкиТовары.Назначение = ОтгрузкаТоваровКлиентуТовары.Назначение
	|				И АктОРасхожденияхПослеОтгрузкиТовары.Упаковка = ОтгрузкаТоваровКлиентуТовары.Упаковка
	|				И АктОРасхожденияхПослеОтгрузкиТовары.ЗаказКлиента = ОтгрузкаТоваровКлиентуТовары.ЗаказКлиента
	|				И АктОРасхожденияхПослеОтгрузкиТовары.Склад = ОтгрузкаТоваровКлиентуТовары.Склад
	|				И АктОРасхожденияхПослеОтгрузкиТовары.Серия = ОтгрузкаТоваровКлиентуТовары.Серия
	|				И (ОтгрузкаТоваровКлиентуТовары.КодСтроки = АктОРасхожденияхПослеОтгрузкиТовары.КодСтроки
	|					ИЛИ НЕ ОтгрузкаТоваровКлиентуТовары.ОтгрузкаПоЗаказам
	|						И АктОРасхожденияхПослеОтгрузкиТовары.КодСтроки = 0
	|						И ОтгрузкаТоваровКлиентуТовары.КодСтроки > 0)
	|	
	|	СГРУППИРОВАТЬ ПО
	|		АктОРасхожденияхПослеОтгрузкиТовары.Назначение,
	|		АктОРасхожденияхПослеОтгрузкиТовары.Характеристика,
	...
	|		СУММА(ОтгрузкаТоваровКлиентуТовары.КоличествоУпаковок) КАК КоличествоУпаковокОснование
	|	ИЗ
	|		ОтгрузкаТоваровКлиенту КАК ОтгрузкаТоваровКлиентуТовары
	|			ЛЕВОЕ СОЕДИНЕНИЕ АктОРасхожденияПриОтгрузке КАК АктОРасхожденияхПослеОтгрузкиТовары
	|			ПО ОтгрузкаТоваровКлиентуТовары.Номенклатура = АктОРасхожденияхПослеОтгрузкиТовары.Номенклатура
	|				И ОтгрузкаТоваровКлиентуТовары.Характеристика = АктОРасхожденияхПослеОтгрузкиТовары.Характеристика
	|				И ОтгрузкаТоваровКлиентуТовары.Назначение = АктОРасхожденияхПослеОтгрузкиТовары.Назначение
	|				И ОтгрузкаТоваровКлиентуТовары.Упаковка = АктОРасхожденияхПослеОтгрузкиТовары.Упаковка
	|				И ОтгрузкаТоваровКлиентуТовары.ЗаказКлиента = АктОРасхожденияхПослеОтгрузкиТовары.ЗаказКлиента
	|				И ОтгрузкаТоваровКлиентуТовары.Склад = АктОРасхожденияхПослеОтгрузкиТовары.Склад
	|				И ОтгрузкаТоваровКлиентуТовары.Серия = АктОРасхожденияхПослеОтгрузкиТовары.Серия
	|				И (ОтгрузкаТоваровКлиентуТовары.КодСтроки = АктОРасхожденияхПослеОтгрузкиТовары.КодСтроки
	|					ИЛИ НЕ ОтгрузкаТоваровКлиентуТовары.ОтгрузкаПоЗаказам
	|						И АктОРасхожденияхПослеОтгрузкиТовары.КодСтроки = 0
	|						И ОтгрузкаТоваровКлиентуТовары.КодСтроки > 0)
	...
	|		ОтгрузкаТоваровКлиентуТовары.Номенклатура,
	|		ОтгрузкаТоваровКлиентуТовары.Серия,
	|		ОтгрузкаТоваровКлиентуТовары.СтатусУказанияСерий,
	|		ОтгрузкаТоваровКлиентуТовары.Склад,
	|		ОтгрузкаТоваровКлиентуТовары.ЗаказКлиента,
	|		ОтгрузкаТоваровКлиентуТовары.Назначение,
	|		ОтгрузкаТоваровКлиентуТовары.Характеристика,
	|		ОтгрузкаТоваровКлиентуТовары.КодСтроки) КАК ВложенныйЗапрос
	|;
	|
	|//////////////////////////////////////////////////////////////////////////////// 5
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|		СУММА(АктОРасхожденияхСерии.Количество)                    КАК КоличествоПоАкту,
	|		СУММА(АктОРасхожденияхСерии.КоличествоПоДокументу)         КАК КоличествоПоОснованиюАкт,
	|		СУММА(ЕСТЬNULL(ОтгрузкаТоваровКлиентуСерии.Количество, 0)) КАК КоличествоОснование
	|	ИЗ
	|		АктОРасхожденияхСерии КАК АктОРасхожденияхСерии
	|			ЛЕВОЕ СОЕДИНЕНИЕ ОтгрузкаТоваровКлиентуСерии КАК ОтгрузкаТоваровКлиентуСерии
	|			ПО АктОРасхожденияхСерии.Номенклатура = ОтгрузкаТоваровКлиентуСерии.Номенклатура
	|				И АктОРасхожденияхСерии.Характеристика = ОтгрузкаТоваровКлиентуСерии.Характеристика
	|				И АктОРасхожденияхСерии.Назначение = ОтгрузкаТоваровКлиентуСерии.Назначение
	|				И АктОРасхожденияхСерии.Склад = ОтгрузкаТоваровКлиентуСерии.Склад
	|				И АктОРасхожденияхСерии.Серия = ОтгрузкаТоваровКлиентуСерии.Серия
	|	
	|	СГРУППИРОВАТЬ ПО
	|		АктОРасхожденияхСерии.Назначение,
	|		АктОРасхожденияхСерии.Характеристика,
	...
	|		СУММА(ОтгрузкаТоваровКлиентуСерии.Количество) КАК КоличествоОснование
	|	ИЗ
	|		ОтгрузкаТоваровКлиентуСерии КАК ОтгрузкаТоваровКлиентуСерии
	|			ЛЕВОЕ СОЕДИНЕНИЕ АктОРасхожденияхСерии КАК АктОРасхожденияхСерии
	|			ПО ОтгрузкаТоваровКлиентуСерии.Номенклатура = АктОРасхожденияхСерии.Номенклатура
	|				И ОтгрузкаТоваровКлиентуСерии.Характеристика = АктОРасхожденияхСерии.Характеристика
	|				И ОтгрузкаТоваровКлиентуСерии.Назначение = АктОРасхожденияхСерии.Назначение
	|				И ОтгрузкаТоваровКлиентуСерии.Склад = АктОРасхожденияхСерии.Склад
	|				И ОтгрузкаТоваровКлиентуСерии.Серия = АктОРасхожденияхСерии.Серия
	|	ГДЕ
	|		АктОРасхожденияхСерии.КоличествоПоДокументу ЕСТЬ NULL 
	|	
	|	СГРУППИРОВАТЬ ПО
	...
	|		ОтгрузкаТоваровКлиентуСерии.Склад,
	|		ОтгрузкаТоваровКлиентуСерии.Назначение,
	|		ОтгрузкаТоваровКлиентуСерии.Характеристика) КАК ВложенныйЗапрос
	|;
	|
	|//////////////////////////////////////////////////////////////////////////////// 6
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	ТоварыСостояния.ВидЦены                          КАК ВидЦены,
...
Функция ТекстЗапросаТабличныеЧастиАктаОРасхожденияхПослеПриемкиИВозвратаОтКлиента(АктОРасхождениях, Основание, СтруктураВозврата, ИзменениеДокумента)
	...
	|	ВЫБОР
	|		КОГДА НЕ АктОРасхожденияхПослеПриемкиТовары.СтатусУказанияСерийПолучатель В (13,14)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|		ИНАЧЕ АктОРасхожденияхПослеПриемкиТовары.Серия
	|	КОНЕЦ КАК Серия,
	|	АктОРасхожденияхПослеПриемкиТовары.СтатусУказанияСерийПолучатель КАК СтатусУказанияСерий,
	...
	|		ИНАЧЕ АктОРасхожденияхПослеПриемкиТовары.Серия
	|	КОНЕЦ,
	|	АктОРасхожденияхПослеПриемкиТовары.СтатусУказанияСерийПолучатель,
	...
	|
	|ИМЕЮЩИЕ
	|	МАКСИМУМ(ТаблицаТовары.СтатусУказанияСерий) В  (2, 4, 6, 8, 10)) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.Номенклатура,
	|	ВложенныйЗапрос.Характеристика,
	...
	|				И АктОРасхожденияхСерии.Склад = ВозвратТоваровОтКлиентаСерии.Склад
	|	
	|	СГРУППИРОВАТЬ ПО
	|		АктОРасхожденияхСерии.Характеристика,
	|		АктОРасхожденияхСерии.Назначение,
	...
	|		ВозвратТоваровОтКлиентаСерии.Склад,
	|		СУММА(0),
	|		СУММА(0),
	...
	|				И ВозвратТоваровОтКлиентаСерии.Склад = АктОРасхожденияхСерии.Склад
	|	ГДЕ
	|		АктОРасхожденияхСерии.КоличествоПоДокументу ЕСТЬ NULL 
	|	
	|	СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаТабличныеЧастиАктаОРасхожденияхПослеПриемки(АктОРасхождениях, Основание, СтруктураВозврата, ИзменениеДокумента)
	...
	|			ТОГДА АктОРасхожденияхПослеПриемкиТовары.Серия
	|		КОГДА НЕ АктОРасхожденияхПослеПриемкиТовары.СтатусУказанияСерийПолучатель В (13,14)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|		ИНАЧЕ АктОРасхожденияхПослеПриемкиТовары.Серия
	|	КОНЕЦ КАК Серия,
	...
	|		ПО АктОРасхожденияхПослеПриемкиТовары.Ссылка = ДанныеДокумента.Ссылка
	|ГДЕ
	|	АктОРасхожденияхПослеПриемкиТовары.Ссылка = &АктОРасхождениях
	|	И АктОРасхожденияхПослеПриемкиТовары.ДокументОснование = &Основание
	|
...
Функция ТекстЗапросаТабличныеЧастиАктаОРасхожденияхПослеОтгрузкиИОтгрузкиСХранения(АктОРасхождениях, Основание, СтруктураВозврата, ИзменениеДокумента)
	...
	|	ОтгрузкаТоваровСХраненияТовары.ДокументПоступления,
	...
	|				И АктОРасхожденияхПослеОтгрузкиТовары.ДокументПоступления = ОтгрузкаТоваровСХраненияТовары.ДокументПоступления
	...
	|				И ОтгрузкаТоваровСХраненияТовары.ДокументПоступления = АктОРасхожденияхПослеОтгрузкиТовары.ДокументПоступления
	...
	|	ТоварыСостояния.КодСтроки                        КАК КодСтроки,
	|	ТоварыСостояния.Серия                            КАК Серия,
	|	ТоварыСостояния.ДокументПоступления              КАК ДокументПоступления,
	|	ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки, 1)    КАК КоэффициентУпаковки,
	|	ТоварыСостояния.Цена                             КАК Цена,
	|	ТоварыСостояния.Склад                            КАК Склад,
	|	ТоварыСостояния.КоличествоУпаковокПоАкту         КАК КоличествоУпаковокПоАкту,
...
Функция ТекстЗапросаТабличныеЧастиАктаОРасхожденияхПослеОтгрузкиИПередачиХранителю(АктОРасхождениях,
	...
	|	ВЫБОР
	|		КОГДА ПередачаТоваров.СтатусУказанияСерийНаСкладах = 0
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|		ИНАЧЕ ПередачаТоваров.Серия
	|	КОНЕЦ                          КАК Серия,
	|	ПередачаТоваров.СтатусУказанияСерийНаСкладах КАК СтатусУказанияСерий,
	...
	|		ИНАЧЕ ПередачаТоваров.Серия
	|	КОНЕЦ,
	|	ПередачаТоваров.СтатусУказанияСерийНаСкладах,
	...
	|		ПередачаТоваров.СтатусУказанияСерий КАК СтатусУказанияСерий,
	|		МАКСИМУМ(ЗНАЧЕНИЕ(Справочник.ВидыЦен.ПустаяСсылка)) КАК ВидЦены,
	|		0                              КАК Цена,
	|		ПередачаТоваров.Склад          КАК Склад,
	|		ПередачаТоваров.ЗаказКлиента   КАК ЗаказКлиента,
	...
	|		ПередачаТоваров.СтатусУказанияСерий,
	|		ПередачаТоваров.ЗаказКлиента,
	|		ПередачаТоваров.КодСтроки,
...
Процедура ОбработатьУказаниеСерий(Форма, ПараметрыФормыУказанияСерий, СтруктураДействий) Экспорт
	...
	ЭтоАктПослеОтгрузки = (ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.АктОРасхожденияхПослеОтгрузки");
	...
	Если ЭтоАктПослеОтгрузки Тогда
		ИсходноеДействиеСТоваром = ТекущаяСтрока.ДействиеСТоваром;
	КонецЕсли;
	...
	Если ЭтоАктПослеОтгрузки Тогда
		...
		ПустоеДействиеСТоваром = Перечисления.ВариантыДействийПоТоварамВАктеПослеОтгрузки.ПустаяСсылка();
		
	ИначеЕсли ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.АктОРасхожденияхПослеПеремещения" Тогда
		ПустоеДействие = Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ПустаяСсылка();
	Иначе
	...
	Если НайденныеСтрокиТоваров.Количество() > 1 И ТекущаяСтрока[ИменаРеквизитов.ЗаполненоПоОснованию] = Истина Тогда
		...
		Для Каждого Стр Из НайденныеСтрокиТоваров Цикл
			Если Стр.Количество > Стр.КоличествоПоДокументу Тогда
				...
				ОбрабатываемыеСтроки.Добавить(Стр);
	...
	Если ЭтоАктПослеОтгрузки Тогда
		СтруктураПоиска.ДействиеСТоваром = ИсходноеДействиеСТоваром;
	КонецЕсли;
	...
	Если СтрокиСИзлишками.Количество() > 0 И СтрокиСНедостачей.Количество() > 0 Тогда
		...
		Для Каждого Стр Из СтрокиСИзлишками Цикл
			...
			Если ЭтоАктПослеОтгрузки Тогда
				НоваяСтрокаСерии.ДействиеСТоваром = ПустоеДействиеСТоваром;
			КонецЕсли;
			Стр.Количество = Стр.КоличествоПоДокументу;
		...
		СтруктураИнициализации.Вставить("ДействиеСТоваром", ПустоеДействиеСТоваром);
		...
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
		СброситьДействие = (ТекущаяСтрока.Действие = ПустоеДействие);
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(НоваяСтрокаТовары, СтруктураДействий, КэшированныеЗначения);
	КонецЕсли;
	...
	Если Не ИсходноеДействие = ПустоеДействие
		...
		Для Каждого Стр Из НайденныеСтрокиТоваров Цикл
			...
			Если ЭтоАктПослеОтгрузки Тогда
				Стр.ДействиеСТоваром = ПустоеДействиеСТоваром;
			КонецЕсли;
		КонецЦикла;
...
Функция ВыполнитьКонтрольДобавленияИзлишкаВНакладную(ИмяРегистра,
	ИмяПоляЗаказ,
	ИмяПоляКОформлению,
	ИменаПолейУникальности,
	ТаблицаДвижений,
	МассивНакладных,
	ПоказательПроверки = "Оформлено")
	...
	Если ИмяРегистра = "РаспоряженияНаОтгрузкуИВозврат" Тогда
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ТаблицаДанных.Распоряжение,
		|	&ПолеУникальности,
		|	ТаблицаДанных.ПоказательОформленоКоличество КАК Количество
		|ПОМЕСТИТЬ ВтДвиженияНакладных
		|ИЗ
		|	#РаспоряженияНаОтгрузкуДвиженияРазвернутая КАК ТаблицаДанных
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ТаблицаДанных.Показатель = &Показатель
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	&ПолеУникальности,
		|	(ТаблицаДанных.ПоказательКОформлениюКоличествоОборот -
		|		ТаблицаДанных.ПоказательОформленоКоличествоОборот) КАК КОформлениюОстаток
		|ПОМЕСТИТЬ ВтНовыеОстаткиРегистраЗаказы
		|ИЗ
		|	#РаспоряженияНаОтгрузкуОборотыРазвернутая КАК ТаблицаДанных
		|ГДЕ
		|	(ТаблицаДанных.ПоказательКОформлениюКоличествоОборот -
		|		ТаблицаДанных.ПоказательОформленоКоличествоОборот) <> 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицаДанных.Распоряжение КАК Заказ,
		|	&ПолеУникальности,
		|	СУММА(ТаблицаДанных.КОформлениюОстаток) КАК Количество
		|ИЗ
		...
		|
		|ИМЕЮЩИЕ
		|	СУММА(ТаблицаДанных.КОформлениюОстаток) < 0";
		
		Если ПоказательПроверки = "ОформленоПоВозвратам" Тогда

			ПоказательКОформлению = "КОформлениюПоВозвратам";
			ПоказательОформлено = "ОформленоПоВозвратам";
		
		ИначеЕсли ПоказательПроверки = "Отгружено" Тогда
		
			ПоказательКОформлению = "КОтгрузке";
			ПоказательОформлено = "Отгружено";

		Иначе
		
			ПоказательКОформлению = "КОформлению";
			ПоказательОформлено = "Оформлено";

		КонецЕсли;
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПоказательКОформлению", ПоказательКОформлению);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПоказательОформлено", ПоказательОформлено);
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст,
			"&Показатель",
			СтрШаблон("ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.%1)", ПоказательОформлено));
		...
		ПараметрыФормированияТаблицы.ТекстУсловия =
			...
			|		И Активность
			|		И НЕ Сторно";
		ПараметрыФормированияТаблицы.Показатели = ПоказательОформлено;
		...
		ПараметрыФормированияТаблицы.Показатели = ПоказательКОформлению + ", " + ПоказательОформлено;
		ПараметрыФормированияТаблицы.Ресурсы = "Количество";
	...
	Иначе
		...
		|	&ИмяПоляЗаказ,
		|	&ПолеУникальности,
		|	&ИмяПоляКОформлениюОстаток
		|ПОМЕСТИТЬ ВтНовыеОстаткиРегистраЗаказы
		|ИЗ
		|	#ИмяРегистраОстатки КАК ТаблицаДанных
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	&ПолеУникальности,
		|	СУММА(&ИмяПоляКОформлениюОстаток) КАК Количество
		|ИЗ
		|	ВтНовыеОстаткиРегистраЗаказы КАК ТаблицаДанных
		|
		|СГРУППИРОВАТЬ ПО
		...
		|
		|ИМЕЮЩИЕ
		|	СУММА(&ИмяПоляКОформлениюОстаток) < 0";
		
		ИмяТаблицы = СтрШаблон("РегистрНакопления.%1", ИмяРегистра);
		ИмяТаблицыОстатки = СтрШаблон("%1.Остатки(, &ИмяПоляЗаказ В (&Заказы))", ИмяТаблицы);
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#ИмяРегистраОстатки", ИмяТаблицыОстатки);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#ИмяРегистра", ИмяТаблицы);
	...
	ПоляУникальности = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
		ИменаПолейУникальности,
		",",
		Истина,
		Истина);
...
Процедура ОпределитьДоступныеСпособыОтработкиАктаВЗависимостиОтСостоянияДокументаЭДО(Форма) Экспорт
	...
	ИначеЕсли РасхожденияКлиентСервер.ТипОснованияПередачаТоваровХранителю(Форма.Объект.ТипОснованияАктаОРасхождении)
		...
		ДобавитьВсеВозможныеСпособыОтработкиАктаПоПередаче(Форма);
		
	ИначеЕсли РасхожденияКлиентСервер.ТипОснованияОтгрузкаТоваровСХранения(Форма.Объект.ТипОснованияАктаОРасхождении)
		Тогда
		
		ДобавитьВсеВозможныеСпособыОтработкиАктаПоОтгрузке(Форма);
	...
	ИначеЕсли РасхожденияКлиентСервер.ТипОснованияПриемкаТоваровНаХранение(Форма.Объект.ТипОснованияАктаОРасхождении)
		...
		ДобавитьВсеВозможныеСпособыОтработкиАктаПоПриемкеИПоступлению(Форма);
...
Процедура ДобавитьСпособыОтработкиКорректировкаРеализации(Форма) Экспорт
	...
	Если Форма.ИспользоватьКорректировкиРеализаций
			И Форма.Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту
		Тогда
		...
		Форма.ДоступныеСпособыОтработкиАкта.Добавить(
			Перечисления.СпособыОтраженияРасхожденийАктПриемкиКлиента.ОформлениеКорректировкиПоСогласованию);
		
		Форма.ДоступныеСпособыОтработкиАкта.Добавить(
			Перечисления.СпособыОтраженияРасхожденийАктПриемкиКлиента.ОформлениеКорректировокКакНовыеПервичныеДокументы);
	
	ИначеЕсли Форма.ИспользоватьКорректировкиРеализаций
...
// Параметры:
// 	Форма - ФормаКлиентскогоПриложения - Изменяемая форма, содержит в том числе:
// 		* ДоступныеСпособыОтработкиАктаПоСостояниюЭДО - СписокЗначений Из ПеречислениеСсылка.СпособыОтраженияРасхожденийАктПриемкиКлиента - 
// 		* ДоступныеСпособыОтработкиАкта               - СписокЗначений Из ПеречислениеСсылка.СпособыОтраженияРасхожденийАктПриемкиКлиента - 
Процедура ДобавитьВсеВозможныеСпособыОтработкиАктаПоПередаче(Форма) Экспорт
	...
	Форма.ДоступныеСпособыОтработкиАкта.Добавить(
		Перечисления.СпособыОтраженияРасхожденийАктПриемкиКлиента.ИсправлениеПервичныхДокументов);
	ДобавитьСпособыОтработкиКорректировкаПередачи(Форма);
...
// 		* ДоступныеСпособыОтработкиАктаПоСостояниюЭДО - СписокЗначений Из ПеречислениеСсылка.СпособыОтраженияРасхожденийАктПриемкиКлиента - 
// 		* ДоступныеСпособыОтработкиАкта               - СписокЗначений Из ПеречислениеСсылка.СпособыОтраженияРасхожденийАктПриемкиКлиента - 
Процедура ДобавитьСпособыОтработкиКорректировкаПередачи(Форма) Экспорт
	
	Если Форма.ИспользоватьИсправлениеДокументов Тогда
		Форма.ДоступныеСпособыОтработкиАкта.Добавить(
			Перечисления.СпособыОтраженияРасхожденийАктПриемкиКлиента.ОформлениеКорректировкиКакИсправлениеПервичныхДокументов);
	КонецЕсли;
...
// 		* ДоступныеСпособыОтработкиАктаПоСостояниюЭДО - СписокЗначений Из ПеречислениеСсылка.СпособыОтраженияРасхожденийАктПриемкиКлиента - 
// 		* ДоступныеСпособыОтработкиАкта               - СписокЗначений Из ПеречислениеСсылка.СпособыОтраженияРасхожденийАктПриемкиКлиента - 
Процедура ДобавитьВсеВозможныеСпособыОтработкиАктаПоОтгрузке(Форма) Экспорт
	...
	Форма.ДоступныеСпособыОтработкиАкта.Добавить(
		Перечисления.СпособыОтраженияРасхожденийАктПриемкиКлиента.ИсправлениеПервичныхДокументов);
	ДобавитьСпособыОтработкиКорректировкаОтгрузки(Форма);
...
// 		* ДоступныеСпособыОтработкиАктаПоСостояниюЭДО - СписокЗначений Из ПеречислениеСсылка.СпособыОтраженияРасхожденийАктПриемкиКлиента - 
// 		* ДоступныеСпособыОтработкиАкта               - СписокЗначений Из ПеречислениеСсылка.СпособыОтраженияРасхожденийАктПриемкиКлиента - 
Процедура ДобавитьСпособыОтработкиКорректировкаОтгрузки(Форма) Экспорт
...
// Параметры:
// 	Форма - ФормаКлиентскогоПриложения - Изменяемая форма, содержит в том числе:
// 		* ДоступныеСпособыОтработкиАктаПоСостояниюЭДО - СписокЗначений Из ПеречислениеСсылка.СпособыОтраженияАктовОРасхожденияПослеПоступления - 
// 		* ДоступныеСпособыОтработкиАкта               - СписокЗначений Из ПеречислениеСсылка.СпособыОтраженияАктовОРасхожденияПослеПоступления - 
Процедура ДобавитьВсеВозможныеСпособыОтработкиАктаПоПриемкеИПоступлению(Форма) Экспорт
	
	ДобавитьСпособыОтработкиАктаПоПриемкеИПоступлениюИсправление(Форма);
	ДобавитьСпособыОтработкиАктаПоПриемкеИПоступлениюКорректировка(Форма);
...
// 		* ДоступныеСпособыОтработкиАктаПоСостояниюЭДО - СписокЗначений Из ПеречислениеСсылка.СпособыОтраженияАктовОРасхожденияПослеПоступления - 
// 		* ДоступныеСпособыОтработкиАкта               - СписокЗначений Из ПеречислениеСсылка.СпособыОтраженияАктовОРасхожденияПослеПоступления - 
Процедура ДобавитьСпособыОтработкиАктаПоПриемкеИПоступлениюИсправление(Форма) Экспорт
...
// 		* ДоступныеСпособыОтработкиАктаПоСостояниюЭДО - СписокЗначений Из ПеречислениеСсылка.СпособыОтраженияАктовОРасхожденияПослеПоступления - 
// 		* ДоступныеСпособыОтработкиАкта               - СписокЗначений Из ПеречислениеСсылка.СпособыОтраженияАктовОРасхожденияПослеПоступления - 
Процедура ДобавитьСпособыОтработкиАктаПоПриемкеИПоступлениюКорректировка(Форма) Экспорт
