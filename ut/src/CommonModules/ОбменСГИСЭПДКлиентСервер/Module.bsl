...
Функция СоответствиеТитуловИШагов(Документ = Неопределено) Экспорт
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.ЭлектронныйПутевойЛист")
		...
		СоответствиеТитулов.Вставить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_ОткМ"), НСтр("ru='Отклонено при медосмотре'"));
	КонецЕсли;
...
Функция ПрефиксТитулаПоЭлементуРегламентаЭДО(ЭлементРегламента) Экспорт
	...
	Если ЗначениеЗаполнено(ЭлементРегламента) Тогда
		...
		ИначеЕсли ЭлементРегламента = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул5") Тогда
			...
			Результат.ПолеУИД = Результат.ВПрограмме + "УИДМинтранс";
			Результат.ШаблонИмениФайла = Результат.ДляФайла + "_ИдГО_ _ИдПЕР_ _GGGGMMDD_ГУИДФайла";	
		...
		ИначеЕсли ЭлементРегламента = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул6") Тогда
			...
			Результат.ПолеУИД = Результат.ВПрограмме + "УИДМинтранс";
			Результат.ШаблонИмениФайла = Результат.ДляФайла + "_ИдПЕР_ _ИдГО_ _GGGGMMDD_ГУИДФайла";	
...
Процедура ИзменитьОформлениеФормы(Форма, ИмяЭлемента = Неопределено) Экспорт
	...
	ИначеЕсли СтрЗаканчиваетсяНа(Форма.ИмяФормы, "ТитулПеревозчикаРазмерПлатыИПорядокРасчетов") Тогда
		
		ТитулПеревозчикаНалоговаяСтавка = Форма.ТитулПеревозчикаНалоговаяСтавка;
		
		Если ТитулПеревозчикаНалоговаяСтавка = "без НДС" Тогда
			
			Форма.ТитулПеревозчикаСтоимостьПеревозкиСНалогом = Неопределено;
			
		Иначе
		
			ТитулПеревозчикаСтоимостьПеревозкиБезНалога = Форма.ТитулПеревозчикаСтоимостьПеревозкиБезНалога;
			
			НДС = РасчетНДСПоСтавкеСтрокой(ТитулПеревозчикаСтоимостьПеревозкиБезНалога, ТитулПеревозчикаНалоговаяСтавка); 
			
			Если НДС > 0 Тогда
				Форма.ТитулПеревозчикаСтоимостьПеревозкиСНалогом = ТитулПеревозчикаСтоимостьПеревозкиБезНалога + НДС;
			КонецЕсли;
...
Функция РасчетНДСПоСтавкеСтрокой(СуммаБезНДС, СтавкаСтрокой)
	...
	Если СтавкаСтрокой = "0%" Тогда
		Результат = 0;
	ИначеЕсли СтавкаСтрокой = "5%" Тогда
		Результат = СуммаБезНДС * 5 / 100;
	ИначеЕсли СтавкаСтрокой = "7%" Тогда
		Результат = СуммаБезНДС * 7 / 100;
	ИначеЕсли СтавкаСтрокой = "9,09%" Тогда
		Результат = СуммаБезНДС * 9.09 / 100;
	ИначеЕсли СтавкаСтрокой = "10%" Тогда
		Результат = СуммаБезНДС * 10 / 100;
	ИначеЕсли СтавкаСтрокой = "16,67%" Тогда
		Результат = СуммаБезНДС * 16.67 / 100;
	ИначеЕсли СтавкаСтрокой = "18,03%" Тогда
		Результат = СуммаБезНДС * 18.03 / 100;
	...
	ИначеЕсли СтавкаСтрокой = "5/105" Тогда
		Результат = СуммаБезНДС * 5 / 105;
	ИначеЕсли СтавкаСтрокой = "7/107" Тогда
		Результат = СуммаБезНДС * 7 / 107;
...
//  ИдентификаторСтрокиГрупповойОтправки - Неопределено - Идентификатор строки групповой отправки
// 
// Возвращаемое значение:
...
Процедура ЗаполнитьРеквизитыУчастника(Участник, ПрефиксРеквизитов, КонтейнерРеквизитов, Форма, ПрефиксТитула = Неопределено) Экспорт
	...
	Если ПрефиксРеквизитов = "ТитулФрахтователяФрахтователь"
		...
		Если РеквизитыУчастника.Свойство("ЮЛ_ИНН") И ЗначениеЗаполнено(РеквизитыУчастника.ЮЛ_ИНН) Тогда
			...
			ПрефиксАдреса = "ЮЛ";
		Иначе
			Если ПрефиксРеквизитов = "ТитулФрахтователяФрахтователь" Тогда
				ПрефиксКонтактов = "ФЛ";
	...
	Для Каждого КиЗ Из РеквизитыУчастника Цикл
		Если КиЗ.Ключ <> ПостфиксТелефоны И КиЗ.Ключ <> ПостфиксЭлПочта Тогда
			...
			Если ПрефиксРеквизитов = "ТитулФрахтователяФрахтователь" И СтрНачинаетсяС(КороткоеИмя, "ИП") Тогда
				КороткоеИмя = СтрЗаменить(КороткоеИмя, "ИП", "ФЛ"); 
			КонецЕсли;
...
Процедура ЗаполнитьРеквизитыПоСсылке(Элемент, Форма, ТекущиеДанные = Неопределено, ПрефиксТитула = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ЗначениеРеквизита) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
		ПоказатьОшибкиПоФизЛицу(Форма, Элемент, ЗначениеРеквизита, Форма.Объект.Организация);	
	КонецЕсли;
...
Процедура ОкруглениеДатыПриРегулировании(Дата, Направление, СтандартнаяОбработка, ШагОкругления = Неопределено) Экспорт
	...
	Дата = ОкруглитьВремя(Дата + Направление * Шаг, Шаг);
...
Процедура ПоказатьОшибкиПоФизЛицу(Форма, ЭлементФормы, ФизическоеЛицо, Организация = Неопределено)  
	...
	СтруктураОшибок = ПолучитьОшибкиПоФизЛицу(ФизическоеЛицо, Организация);
...
Процедура ПоказатьОшибкиПоАдресу(Форма, КонтейнерРеквизитов, Префикс, ИмяЭлемента)
	...
	Иначе
		...
		АдресВСвободнойФорме = Неопределено; 
		Если ТипЗнч(КонтейнерРеквизитов) = Тип("Структура") Тогда
			КонтейнерРеквизитов.Свойство(Префикс + "КодСтраны", КодСтраны);
			КонтейнерРеквизитов.Свойство(Префикс + "Представление", АдресВСвободнойФорме);
		Иначе
			КодСтраны = КонтейнерРеквизитов[Префикс + "КодСтраны"];
			АдресВСвободнойФорме = КонтейнерРеквизитов[Префикс + "Представление"];
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(КодСтраны) И Не ЗначениеЗаполнено(АдресВСвободнойФорме) Тогда
...
Функция ПолучитьОшибкиПоФизЛицу(ФизическоеЛицо, Организация = Неопределено) 
	
	СтруктураОшибок = НовоеОписаниеОшибокЗаполнения();
	
	Если Не ЗначениеЗаполнено(ФизическоеЛицо) 
		Или ТипЗнч(ФизическоеЛицо) = Тип("Строка") Тогда
		Возврат СтруктураОшибок;
	КонецЕсли;
    ...
    Попытка   
		РеквизитыФизЛица = МодульОбменСГИСЭПД.ПолучитьРеквизитыФизЛица(ФизическоеЛицо, Организация);  
		ПостфиксТелефоны = "НомераТелефонов";
		Если Не РеквизитыФизЛица.Свойство(ПостфиксТелефоны) ИЛИ РеквизитыФизЛица[ПостфиксТелефоны].Количество() <= 0 Тогда 
			СтруктураОшибок.ЕстьОшибки = Истина;
			СтруктураОшибок.ОписаниеОшибок = "Не заполнен номер телефона физического лица";
		КонецЕсли;
	Исключение
		Возврат СтруктураОшибок;
    КонецПопытки;
