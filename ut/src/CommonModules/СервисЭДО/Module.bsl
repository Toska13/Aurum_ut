...
Функция ВыполнитьОперацию(ПараметрыОперации, КонтекстДиагностики, КлючСинхронизации = Неопределено) Экспорт
	...
	Если ИгнорироватьОшибкуВыполненияОперации(ОшибкаОператора) Тогда
		...
		Если ТребуетсяПовторныйЗапросМаркера(ОшибкаОператора) Тогда
			...
			Если КлючСинхронизации <> Неопределено Тогда
				УдалитьМаркерИзБезопасногоХранилища(КлючСинхронизации.ИдентификаторУчетнойЗаписи,
...
Функция Авторизоваться(КлючСинхронизации, КонтекстДиагностики,
	...
	КэшированиеМаркераПоддерживается = КэшированиеМаркераПоддерживается(КлючСинхронизации);
	...
	Если КэшированиеМаркераПоддерживается Тогда
		МаркерИзБезопасногоХранилища = ЗашифрованныйМаркерИзБезопасногоХранилища(
			КлючСинхронизации.ИдентификаторУчетнойЗаписи, СертификатДляПолученияМаркера, Истина);
	КонецЕсли;
	...
	Если КэшированиеМаркераПоддерживается И ДатаОкончанияДействияМаркера <> Неопределено Тогда
		ДатаОкончанияДействияМаркера =
			ДатаОкончанияДействияМаркераПослеСверкиСТекущимВременемНаСервере1С(ДатаОкончанияДействияМаркера);
	КонецЕсли;
	
	Если ДатаОкончанияДействияМаркера = Неопределено Тогда
		УдалосьОпределитьДатуОкончанияДействияМаркера = Ложь;
	Иначе
		УдалосьОпределитьДатуОкончанияДействияМаркера = Истина;
		КлючСинхронизации.ДатаОкончанияДействияМаркера = ДатаОкончанияДействияМаркера;
	КонецЕсли;
	
	Маркер = Результат.Ответ.ПолучитьТелоКакДвоичныеДанные();
	
	Если КэшированиеМаркераПоддерживается И УдалосьОпределитьДатуОкончанияДействияМаркера Тогда
...
Функция ПриоритетРаспаковкиКонтейнера(КодТранзакции, ТипДокументаСтрокой = "") Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		МодульОбменСГИСЭПД = ОбщегоНазначения.ОбщийМодуль("ОбменСГИСЭПД");
		МодульОбменСГИСЭПД.ПриОпределенииПриоритетаРаспаковкиКонтейнера(КодТранзакции, ТипДокументаСтрокой, Приоритет);
	КонецЕсли;
...
// Определяет, является ли код ошибки кодом ошибки удаленного из БД Хаба маркера.
// 
// Параметры:
//  КодОшибки - Число
// 
// Возвращаемое значение:
//  Булево
Функция ЭтоКодОшибкиМаркераУдаленногоИзХаба(КодОшибки) Экспорт
	
	Возврат КодОшибки = 1102;
...
Процедура ПеревестиУчетнуюЗаписьИзТакскомВХаб(КлючСинхронизации, ДвоичныеДанныеСертификата) Экспорт
	...
	ПараметрыВыполненияОперации = НовыеПараметрыВыполненияОперацииСервиса(КлючСинхронизации);
	ПараметрыВыполненияОперации.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО;
	ПараметрыВыполненияОперации.ПредставлениеОперации = НСтр("ru = 'Аутентификация в сервисе 1С-ЭДО'");
	ПараметрыВыполненияОперации.АдресРесурса = "CertificateLogin";
	ПараметрыВыполненияОперации.ТребуетсяМаркер = Ложь;
	ПараметрыВыполненияОперации.Метод = ИнтернетСоединениеБЭД.HTTPМетоды().POST;
	ДобавитьПараметрЗапроса(ПараметрыВыполненияОперации, "edxClientId", ИдентификаторУчетнойЗаписи);
	ДобавитьПараметрЗапроса(ПараметрыВыполненияОперации, "Gateway", ОповещенияОСобытияхЭДО.АдресДляУведомленийЭДО(Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО)); 
	ДобавитьПараметрЗапроса(ПараметрыВыполненияОперации, "users1c", "true");
	
	ДобавитьВЗаголовкиЗапросаВерсиюБЭД(ПараметрыВыполненияОперации.Заголовки, Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО);
	...
	КодОшибкиОтсутствияСертификатаВХабе = 3103;
	Если Результат.Успех Или Результат.ОшибкаОператора.Код = КодОшибкиОтсутствияСертификатаВХабе Тогда
		УчетныеЗаписиЭДО.ИзменитьСпособОбменаУчетнойЗаписиПереводимойИзТакском(ИдентификаторУчетнойЗаписи);
	Иначе
		УчетныеЗаписиЭДО.ДобавитьОшибкуПереводаИзТакскомВХаб(ИдентификаторУчетнойЗаписи, Результат.ОшибкаОператора);
	КонецЕсли;
	СинхронизацияЭДОКлиентСервер.СброситьТекущуюУчетнуюЗапись(КонтекстДиагностики);
...
Функция ИгнорироватьОшибкуВыполненияОперации(ОшибкаОператора)
	...
	Возврат ЭтоКодОшибкиПовторнойОтправки(ОшибкаОператора.Код)
		...
		Или ЭтоКодОшибкиМаркераУдаленногоИзХаба(ОшибкаОператора.Код);
...
//  - Неопределено
//  - См. НовыйТокенАвторизации
Функция ОбновитьТокенАвторизации(ИдентификаторУчетнойЗаписи, ТокенОбновления, КонтекстДиагностики)
...
// Возвращаемое значение:
//  Булево
Функция КэшированиеМаркераПоддерживается(КлючСинхронизации)
	
	Возврат КлючСинхронизации.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО
		И Не ЗначениеЗаполнено(КлючСинхронизации.КлючДоступаКСервисуЭДО);
...
Функция МаркерПросрочен(КлючСинхронизации)
	...
	Если Не КэшированиеМаркераПоддерживается(КлючСинхронизации) Тогда
		Возврат Ложь;
	КонецЕсли;
...
//  АктуализироватьМаркеры - Булево - если Истина - проверяет срок действия маркеров в безопасном хранилище,
//                           удаляет просроченные.
// 
// Возвращаемое значение:
//  - Неопределено
//  - См. НовыеДанныеЗашифрованногоМаркера
Функция ЗашифрованныйМаркерИзБезопасногоХранилища(ИдентификаторУчетнойЗаписи, Сертификат, АктуализироватьМаркеры = Ложь)
	...
	Если АктуализироватьМаркеры Тогда
...
//  ИдентификаторУчетнойЗаписи - Строка
//  Сертификат - СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования
Процедура УдалитьМаркерИзБезопасногоХранилища(ИдентификаторУчетнойЗаписи, Сертификат)
	...
	ДанныеМаркеровПоСертификатам.Удалить(Сертификат);
	
	ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(ИдентификаторУчетнойЗаписи,
		ДанныеМаркеровПоСертификатам, КлючЗашифрованногоМаркераВБезопасномХранилище());
...
// Параметры:
//  ДатаОкончанияДействияМаркера - Строка - дата в формате RFC1123
// 
// Возвращаемое значение:
//  Неопределено,Дата
Функция ДатаОкончанияДействияМаркераПослеСверкиСТекущимВременемНаСервере1С(ДатаОкончанияДействияМаркера)
	
	НормализованнаяДатаОкончанияДействияМаркера =
		ОбщегоНазначенияКлиентСервер.ДатаRFC1123(ДатаОкончанияДействияМаркера);
	ТекущаяУниверсальнаяДатаСервера = ТекущаяУниверсальнаяДата();
	
	МаркерУжеПросрочен = НормализованнаяДатаОкончанияДействияМаркера <= ТекущаяУниверсальнаяДатаСервера;
	Если МаркерУжеПросрочен Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	РазницаВСекундах = НормализованнаяДатаОкончанияДействияМаркера - ТекущаяУниверсальнаяДатаСервера;
	РазницаВЧасах = РазницаВСекундах / 3600;
	МодульРазницыВЧасах = ?(РазницаВЧасах >= 0, РазницаВЧасах, -РазницаВЧасах);
	
	МаксимальнаяДопустимаяПогрешностьВЧасах = 1;
	
	Если МодульРазницыВЧасах > МаксимальнаяДопустимаяПогрешностьВЧасах Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат НормализованнаяДатаОкончанияДействияМаркера;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьПараметрыУведомлений", Истина);
	Методы.Вставить("ОтправитьРегистрационныйПакет1СЭДО", Истина);
	Методы.Вставить("ПолучитьПараметрыУведомлений", Истина);
	Методы.Вставить("ОтправитьРегистрационныйПакет1СЭДО", Истина);
	Методы.Вставить("ОбновитьПараметрыУведомлений", Истина);
	Методы.Вставить("ПолучитьСостояниеРегистрацииВСервисе1СЭДО", Истина);
