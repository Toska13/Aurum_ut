...
Функция ИнициализацияКонфигурацииЛокальногоМодуляЧЗ(Организация, ПараметрыУстановки, ИдентификаторФискальногоНакопителя = "") Экспорт
	...
	Если ПараметрыУстановки.ФорматОбмена = ПредопределенноеЗначение("Перечисление.ВерсииФорматаОбменаЛокальногоМодуляИСМП.V2") Тогда
		URLЗапроса = "api/v2/init";
	Иначе
...
Функция ИзменитьПарольАдминистратораЛокальногоМодуляЧЗ(Организация, ПараметрыУстановки, НовыйПароль) Экспорт
	...
	Если ПараметрыУстановки.ФорматОбмена = ПредопределенноеЗначение("Перечисление.ВерсииФорматаОбменаЛокальногоМодуляИСМП.V2") Тогда
		URLЗапроса = "api/v2/changePassword";
	Иначе
...
Функция НастройкаТоварныхГруппЛокальногоМодуляЧЗ(Организация, ПараметрыУстановки, ПараметрыНастройкиТоварныхГрупп, ИдентификаторФискальногоНакопителя = "") Экспорт
	...
	Если ПараметрыУстановки.ФорматОбмена = ПредопределенноеЗначение("Перечисление.ВерсииФорматаОбменаЛокальногоМодуляИСМП.V2") Тогда
		ВозвращаемоеЗначение.ТекстОшибки = НСтр("ru = 'Версия формата обмена V2 не поддерживает метод настройки товарных групп.'");
	Иначе
...
Функция ЗапроситьКонфигурациюЛокальногоМодуляЧЗ(Организация, ПараметрыУстановки) Экспорт
	...
	Если ПараметрыУстановки.ФорматОбмена = ПредопределенноеЗначение("Перечисление.ВерсииФорматаОбменаЛокальногоМодуляИСМП.V2") Тогда
		ВозвращаемоеЗначение.ТекстОшибки = НСтр("ru = 'Версия формата обмена V2 не поддерживает метод получения конфигурации Локального модуля.'");
	Иначе
...
//
// Параметры:
//   ВидПродукции - СправочникСсылка.ВидыПродукцииИС - Вид продукции (В тестовом контуре адреса серверов
...
Функция ОрганыВластиУстанавливающиеБлокировкуНаКМПоДаннымГИСМТ(МассивИдентификаторов) Экспорт
	...
	СоответствиеПредставлений.Вставить("VETRF", НСтр("ru = 'ВетИС (аннулированный ВСД)'"));
	СоответствиеПредставлений.Вставить("RD",    НСтр("ru = 'Государственные органы контроля за соблюдением требований технических регламентов'"));
...
// Преобразовывает текстовое представление товарной группы в значение вида продукции и наоборот.
//
// Параметры:
//  ЗначениеПоиска - СправочникСсылка.ТоварныеГруппыГИСМТ, СправочникСсылка.ВидыПродукцииИС, Строка - значение для перекодировки
//  ДанныеДляУточнения - см. ДанныеДляУточненияТоварнойГруппы
// 
// Возвращаемое значение:
//  СправочникСсылка.ВидыПродукцииИС, Строка - Имя товарной группы.
Функция ТоварнаяГруппа(ЗначениеПоиска, ДанныеДляУточнения = Неопределено) Экспорт
	
	Результат = ПредопределенноеЗначение("Справочник.ВидыПродукцииИС.ПустаяСсылка");
	
	Если ЗначениеПоиска <> Неопределено Тогда
		Если ТипЗнч(ЗначениеПоиска) = Тип("Строка") Тогда
			Результат = ВидПродукции(
				КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоРеквизитуТоварнойГруппы("Имя", ЗначениеПоиска),
				ДанныеДляУточнения);
		Иначе
			Результат = КонструкторИСМПОбщегоНазначенияКлиентСервер.ИмяТоварнойГруппы(ЗначениеПоиска);
		КонецЕсли;
	КонецЕсли;
...
// Преобразовывает числовое значение товарной группы в значение вида продукции и наоборот.
//
// Параметры:
//  ЗначениеПоиска - СправочникСсылка.ТоварныеГруппыГИСМТ, СправочникСсылка.ВидыПродукцииИС, Строка, Число - значение для перекодировки
//  ДанныеДляУточнения - см. ДанныеДляУточненияТоварнойГруппы
// 
// Возвращаемое значение:
//  СправочникСсылка.ВидыПродукцииИС, Число - Код товарной группы.
Функция ТоварнаяГруппаЧислом(Знач ЗначениеПоиска, ДанныеДляУточнения = Неопределено) Экспорт
	...
	Если ЗначениеПоиска <> Неопределено Тогда
		...
		Если ТипЗнч(ЗначениеПоиска) = Тип("Число") Тогда
			Результат = ВидПродукции(
				КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоРеквизитуТоварнойГруппы("Код", ЗначениеПоиска),
				ДанныеДляУточнения);
		Иначе
			Результат = КонструкторИСМПОбщегоНазначенияКлиентСервер.КодТоварнойГруппы(ЗначениеПоиска);
		КонецЕсли;
	КонецЕсли;
...
// Инициализирует структуру для уточнения данных по виду продукции, ТНВЭД и ОКПД2.
//
// Возвращаемое значение:
//  Структура из КлючИЗначение:
//   * ВидПродукции - СправочникСсылка.ВидыПродукцииИС - Вид продукции для уточнения неоднозначности,
//   * ТНВЭД        - Строка - код ТНВЭД,
//   * ГруппаТНВЭД  - Строка - код группы ТНВЭД,
//   * ОКПД2        - Строка - код ОКПД2,
//   * ГруппаОКПД2  - Строка - код группы ОКПД2.
Функция ДанныеДляУточненияТоварнойГруппы() Экспорт
	
	Результат = ОбщегоНазначенияИСМПКлиентСервер.ДанныеДляУточненияВидовПродукцииПоТНВЭДиОКПД2();
	Результат.Вставить("ВидПродукции", Неопределено);
...
Функция ГруппаСтатусовРазрешительныхДокументов(Знач ЗначениеПоиска) Экспорт
	...
	МассивЗеленыхСтатусов.Добавить(27);
...
Функция ДоступныеМетодыИнтерфейса() Экспорт
	...
	ВозвращаемоеЗначение.Вставить("ТоварныеГруппыПоОрганизации",                     "ТоварныеГруппыПоОрганизации");
	ВозвращаемоеЗначение.Вставить("РегистрацияМестОсуществленияДеятельностиГИСМТ",   "РегистрацияМестОсуществленияДеятельностиГИСМТ");
	ВозвращаемоеЗначение.Вставить("УдалениеМестаОсуществленияДеятельностиГИСМТ",     "УдалениеМестаОсуществленияДеятельностиГИСМТ");
	ВозвращаемоеЗначение.Вставить("АктуализацияСпискаМестОсуществленияДеятельности", "АктуализацияСпискаМестОсуществленияДеятельности");
...
Функция ОбработатьКодМаркировкиНаЛокальномМодулеЧестногоЗнака(ДанныеПроверки, ДанныеУстановленныхЛокальныхМодулей, СтруктураДополнительныхПараметров, РезультатНастройкиЛМ, ОбменНаСервере)
	...
	МассивСтруктурКодовИдентификацииДляОтправки = Новый Массив;
	...
	Для Каждого ДанныеШтрихкода Из ДанныеПроверки Цикл
		...
		СтруктураКодовИдентификацииДляОтправки = Новый Структура;
		СтруктураКодовИдентификацииДляОтправки.Вставить("cis", ДанныеШтрихкода.КодИдентификацииДляОтправкиВЛокальныйМодуль);
		
		Если ЗначениеЗаполнено(ДанныеШтрихкода.ВидПродукции) Тогда
			СтруктураКодовИдентификацииДляОтправки.Вставить("pg", ТоварнаяГруппаЧислом(ДанныеШтрихкода.ВидПродукции));
		КонецЕсли;
		
		МассивСтруктурКодовИдентификацииДляОтправки.Добавить(СтруктураКодовИдентификацииДляОтправки);
		СоответствиеКИЗСтрокам.Вставить(ДанныеШтрихкода.КодИдентификацииДляОтправкиВЛокальныйМодуль, ДанныеШтрихкода);
		
	КонецЦикла;
	
	URLЗапросаФорматV2 = "api/v2/cis/outCheck";
	URLЗапросаФорматV1 = "api/v1/cis/outCheck";
	
	ПараметрыЗапросаФорматV1 = Новый Структура;
	ПараметрыЗапросаФорматV1.Вставить("cis_list", МассивКодовИдентификацииДляОтправки);
	
	ПараметрыЗапросаФорматV2 = Новый Структура;
	ПараметрыЗапросаФорматV2.Вставить("cis_list", МассивСтруктурКодовИдентификацииДляОтправки);
	
	ТелоЗапросаJSONФорматV1 = ОбменДаннымиИСМПКлиентСервер.ОбъектВТекстJSON(ПараметрыЗапросаФорматV1, Истина);
	ТелоЗапросаJSONФорматV2 = ОбменДаннымиИСМПКлиентСервер.ОбъектВТекстJSON(ПараметрыЗапросаФорматV2, Истина);
	
	Для Каждого ДанныеЛокальногоМодуля Из ДанныеУстановленныхЛокальныхМодулей Цикл
		
		Если ДанныеЛокальногоМодуля.ФорматОбмена = ПредопределенноеЗначение("Перечисление.ВерсииФорматаОбменаЛокальногоМодуляИСМП.V2") Тогда
			
			URLЗапроса      = URLЗапросаФорматV2;
			ТелоЗапросаJSON = ТелоЗапросаJSONФорматV2;
			
		Иначе
			
			URLЗапроса      = URLЗапросаФорматV1;
			ТелоЗапросаJSON = ТелоЗапросаJSONФорматV1;
				...
				Если ДанныеПоКодамИдентификации <> Неопределено Тогда
					...
					Если ТипЗнч(ДанныеПоКодамИдентификации) = Тип("Массив") И ДанныеПоКодамИдентификации.Количество() Тогда
						...
						Для Каждого СтрокаДанныхПоКодамИдентификации Из ДанныеПоКодамИдентификации Цикл
							...
							ЭтоПродукцияМОТП = КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(ДанныеШтрихкода.ВидПродукции);
							...
							Если ПараметрыКодаМаркировки.Продан Тогда
								...
								Если ЭтоПродукцияМОТП Тогда
									ПараметрыКодаМаркировки.Статус = ПредопределенноеЗначение("Перечисление.СтатусыКодовМаркировкиИСМП.ВыведенИзОборота");
								Иначе
...
Функция ЗафиксироватьФактПродажиВозвратаНаЛокальномМодулеЧестныйЗнак(МассивКодовИдентификацииДляОтправки, Операция, НастройкиПодключенияЛокальногоМодуля, РезультатНастройкиЛМ)
	...
	Если НастройкиПодключенияЛокальногоМодуля.ФорматОбмена = ПредопределенноеЗначение("Перечисление.ВерсииФорматаОбменаЛокальногоМодуляИСМП.V2") Тогда
		Если Операция = ОперацииПриРаботеСКодамиМаркировки().Продажа Тогда
			URLЗапроса = "api/v2/cis/sell";
		Иначе
			URLЗапроса = "api/v2/cis/return";
		КонецЕсли;
	Иначе
		Если Операция = ОперацииПриРаботеСКодамиМаркировки().Продажа Тогда
...
//  * Статус - ПеречислениеСсылка.СтатусыКодовМаркировкиИСМП - статус кода по данным ЛМ
...
Функция ИнициализироватьПараметрыКодаМаркировкиПоДаннымЛокальногоМодуля() Экспорт
	...
	ПараметрыКодаМаркировки.Вставить("МинимальнаяРозничнаяЦена",                     0);
...
Функция ПараметрыКодаМаркировкиПоДаннымЛокальногоМодуля(ЭлементДанных) Экспорт
	...
	Значение = ЭлементДанных["mrp"];
	Если Не Значение = Неопределено Тогда
		
		ПараметрыКодаМаркировки.МинимальнаяРозничнаяЦена = Значение;
		ПривестиЗначениеМРЦ(ПараметрыКодаМаркировки, "МинимальнаяРозничнаяЦена");
...
Функция ВидПродукции(ВидыПродукции, ДанныеДляУточнения)
	
	Результат = ПредопределенноеЗначение("Справочник.ВидыПродукцииИС.ПустаяСсылка");
	
	Если ВидыПродукции.Количество() > 0 Тогда
		Если ДанныеДляУточнения <> Неопределено Тогда
			Если ЗначениеЗаполнено(ДанныеДляУточнения.ВидПродукции)
				И ВидыПродукции.Найти(ДанныеДляУточнения.ВидПродукции) <> Неопределено Тогда
				Результат = ДанныеДляУточнения.ВидПродукции;
			ИначеЕсли ЕстьДанныеДляУточненияВидаПродукции(ДанныеДляУточнения) Тогда
				ВидыПродукцииПоДаннымУточнения = ИнтерфейсИСМПОбщегоНазначенияВызовСервера.ВидыПродукцииПоДаннымУточнения(ДанныеДляУточнения);
				Если ВидыПродукцииПоДаннымУточнения.Количество() = 1
					И ВидыПродукции.Найти(ВидыПродукцииПоДаннымУточнения[0]) <> Неопределено Тогда
					Результат = ВидыПродукцииПоДаннымУточнения[0];
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Результат) Тогда
			Результат = ВидыПродукции[0];
		КонецЕсли;
	КонецЕсли;
...
Функция ЕстьДанныеДляУточненияВидаПродукции(ДанныеДляУточнения) Экспорт
	
	Возврат ЗначениеЗаполнено(ДанныеДляУточнения.ТНВЭД)
		Или ЗначениеЗаполнено(ДанныеДляУточнения.ГруппаТНВЭД)
		Или ЗначениеЗаполнено(ДанныеДляУточнения.ОКПД2)
		Или ЗначениеЗаполнено(ДанныеДляУточнения.ГруппаОКПД2);
...
//  * Статус - ПеречислениеСсылка.СтатусыКодовМаркировкиИСМП - статус кода по данным разрешительного режима
//  * ОсобоеСостояние - ПеречислениеСсылка.СтатусыКодовМаркировкиИСМП - особое состояние кода маркировки по данным
...
Функция ПараметрыКодаМаркировкиПоДаннымРазрешительногоРежима(ЭлементДанных, ВидПродукции = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ДанныеДляУточнения = ДанныеДляУточненияТоварнойГруппы();
	ДанныеДляУточнения.ВидПродукции = ВидПродукции;
	
	Если Значение <> Неопределено И Значение.Количество() Тогда
		ПараметрыКодаМаркировки.ВидПродукции = ТоварнаяГруппаЧислом(Значение[0], ДанныеДляУточнения);
	КонецЕсли;
	...
	ПараметрыКодаМаркировки.Статус = СтатусКодаМаркировкиПоДаннымГИСМТ(СтруктураДанныхСтатуса, КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(ВидПродукцииДляРазбораКода));
	...
	Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(ВидПродукцииДляРазбораКода) Тогда
	...
	ИначеЕсли КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноУказаниеМинимальнойРозничнойЦены(ВидПродукцииДляРазбораКода) Тогда
...
Функция СтатусКодаМаркировкиПоДаннымГИСМТ(Знач СтруктураКода, ЭтоПродукцияМОТП = Ложь) Экспорт
	...
	Если ЭтоПродукцияМОТП Тогда
		...
		Если СтруктураКода.Продано Тогда
			Возврат ПредопределенноеЗначение("Перечисление.СтатусыКодовМаркировкиИСМП.Продан");
...
Процедура ПривестиЗначениеМРЦ(СтрокаДанных, ИмяКолонки = "МРЦ") Экспорт
	
	Если ТипЗнч(СтрокаДанных[ИмяКолонки]) = Тип("Число") Тогда
		СтрокаДанных[ИмяКолонки] = СтрокаДанных[ИмяКолонки] / 100;
	КонецЕсли;
