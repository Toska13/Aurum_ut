...
//		* ЕстьОтборПоНомеруГТД - Булево - ИСТИНА, признак того, что при подборе остатков товаров учитываются остатки в разрезе номеров ГТД.
...
//		* ПодбиратьСуммыПриНулевомКоличестве- Булево - используется для получения суммовых показателей остатков
//														независимо от количественных остатков.
...
//		* РассчитатьСуммуАкциза - Булево - определяет необходимость расчета суммы акциза для облагаемых акцизом видов запасов.
//		* ПериодНачисленияАкциза - Неопределено, Дата - используется для определения подакцизности товара и действующей ставки акциза.
//		* ПриоритетПодбораПодакцизныхЗапасов - Булево - определяет использование приоритета подбора запасов по признаку Облагается акцизом.
//		* ЗаполнятьПодакцизныеТоварыВидамиЗапасовПоУмолчанию - Булево - определяет необходимость заполнения видов запасов 
//																		по умолчанию при нехватке остатков по товарам 
//																		организаций для подакцизных товаров в документах,
//																		в которых нет возможности указать виды запасов
//																		вручную.
//		* КорректировкаРеализацииПослеПереходаПраваСобственности - Булево - используется для подбора аналитик по месту
//																			хранения при подборе запасов оприходования
//																			документа корректировки реализации.
//		* ПериодДокументаОснования - Неопределено, Дата - используется для получения данных ИБ на дату документа основания.
//		* ПодбиратьКомиссионныеЗапасыПоНалогообложениюКомитента - Булево - используется для отбора комиссионных типов
//																			запасов по налогообложению комитента и
//																			применяемой ставке НДС при продаже.
//
Функция ПараметрыЗаполненияВидовЗапасов() Экспорт
	...
	Результат.ОтборыВидовЗапасов.ОблагаетсяАкцизом = Новый Массив;
	Результат.ОтборыВидовЗапасов.ОблагаетсяАкцизом.Добавить(Ложь);
	
	Результат.Вставить("ЕстьОтборПоНомеруГТД", Ложь);
	...
	Результат.Вставить("КорректировкаРеализацииПослеПереходаПраваСобственности", Ложь);
	
	Результат.Вставить("РассчитатьСуммуАкциза", Ложь);
	Результат.Вставить("ПериодНачисленияАкциза", Неопределено);
	Результат.Вставить("ПриоритетПодбораПодакцизныхЗапасов", Ложь);
	Результат.Вставить("ЗаполнятьПодакцизныеТоварыВидамиЗапасовПоУмолчанию", Ложь);
	Результат.Вставить("ПериодДокументаОснования", Неопределено);
	Результат.Вставить("ПодбиратьКомиссионныеЗапасыПоНалогообложениюКомитента", Ложь);
	
	Возврат Результат;
	
КонецФункции

// Возвращает коллекцию параметров, которая используется при подборе видов запасов в режиме ручного указания данных
// документа.
//
// Возвращаемое значение:
//	Структура - коллекция, которая содержит следующие свойства:
//		* ИгнорироватьОшибкиКонтроляОстатков - Булево - признак, по которому определяется необходимость продолжения
//														выполнения транзакции по подбору запасов документа.
//		* НеЗаполнятьВидыЗапасовПодакцизныхТоваров - Булево - признак, по которому определяется необходимость заполнения
//																вида запасов по умолчанию для подакцизных товаров.
//
Функция ПараметрыЗаполненияВидовЗапасовВручную() Экспорт
	
	ПараметрыЗаполнения = Новый Структура;
	ПараметрыЗаполнения.Вставить("ИгнорироватьОшибкиКонтроляОстатков",			Ложь);
	ПараметрыЗаполнения.Вставить("НеЗаполнятьВидыЗапасовПодакцизныхТоваров",	Ложь);
...
Процедура ЗаполнитьВидыЗапасовПоТоварамОрганизаций(ДокументОбъект,
	...
	Если Не ПараметрыЗаполненияВидовЗапасов.ТаблицаОстатковУжеСформирована
		...
		И ПараметрыЗаполненияВидовЗапасов.ИмяТаблицыОстатков <> "ПереданныеМеждуОрганизациямиТовары"
		И ПараметрыЗаполненияВидовЗапасов.ИмяТаблицыОстатков <> "ПринятыеТовары" Тогда
...
Процедура СформироватьВТДоступныеВидыЗапасов(ДокументОбъект,
	...
	Период				= ?(ПараметрыЗаполненияВидовЗапасов.ПериодДокументаОснования = Неопределено,
							ДокументОбъект.Дата,
							ПараметрыЗаполненияВидовЗапасов.ПериодДокументаОснования);
	ОтборыВидовЗапасов	= ПараметрыЗаполненияВидовЗапасов.ОтборыВидовЗапасов;
	
	УчетНДСУП.СформироватьВТНастройкиУчетаНДСКонтрагентов(МенеджерВременныхТаблиц, Период);
	...
	|	ВЫБОР
	|		КОГДА НастройкиУчетаНДС.Комитент ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ					КАК ОтборПоСтавкеНДСКомитента,
	|	ВЫБОР
	|		КОГДА НастройкиУчетаНДС.Комитент ЕСТЬ NULL
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ НастройкиУчетаНДС.СтавкаНДС
	...
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНастройкиУчетаНДСКонтрагентов КАК НастройкиУчетаНДС
	|		ПО ВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
	|			И ВидыЗапасов.Контрагент = НастройкиУчетаНДС.Комитент
	|			И &ПодбиратьКомиссионныеЗапасыПоНалогообложениюКомитента
	|
	|ГДЕ
	...
	|				И (НастройкиУчетаНДС.Комитент ЕСТЬ NULL
	|					ИЛИ (ЕСТЬNULL(НастройкиУчетаНДС.ОсновноеНалогообложение, НЕОПРЕДЕЛЕНО) В (&НалогообложенияСНДС)
	|						И ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС) В (&НалогообложениеНДС))
	|					ИЛИ (ЕСТЬNULL(НастройкиУчетаНДС.ОсновноеНалогообложение, НЕОПРЕДЕЛЕНО) В (&НалогообложенияБезНДС)
	...
	|	ВидыЗапасов.Ссылка,
	|	ВЫБОР
	|		КОГДА Настройка.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|				И НЕ НастройкиУчетаНДСОрганизаций.Организация ЕСТЬ NULL
	|			ТОГДА ИСТИНА
	|		КОГДА Настройка.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
	|				И НЕ НастройкиУчетаНДСКонтрагентов.Комитент ЕСТЬ NULL
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА Настройка.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|			ТОГДА ЕСТЬNULL(НастройкиУчетаНДСОрганизаций.СтавкаНДС, НЕОПРЕДЕЛЕНО)
	|		КОГДА Настройка.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
	|			ТОГДА ЕСТЬNULL(НастройкиУчетаНДСКонтрагентов.СтавкаНДС, НЕОПРЕДЕЛЕНО)
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ
	|ИЗ
	|	Справочник.ВидыЗапасов КАК ВидыЗапасов
	...
	|		ПО ВидыЗапасов.Ссылка = ВидыЗапасовДокумента.ВидЗапасов
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНастройкиУчетаНДСОрганизаций КАК НастройкиУчетаНДСОрганизаций
	|		ПО ВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|			И ВидыЗапасов.Организация = НастройкиУчетаНДСОрганизаций.Организация
	|			И &ПодбиратьКомиссионныеЗапасыПоНалогообложениюКомитента
	|			И Настройка.СпособПередачиТоваров = ЗНАЧЕНИЕ(Перечисление.СпособыПередачиТоваров.ПередачаНаКомиссию)
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНастройкиУчетаНДСКонтрагентов КАК НастройкиУчетаНДСКонтрагентов
	|		ПО ВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
	|			И ВидыЗапасов.Контрагент = НастройкиУчетаНДСКонтрагентов.Комитент
	|			И &ПодбиратьКомиссионныеЗапасыПоНалогообложениюКомитента
	|			И Настройка.СпособПередачиТоваров = ЗНАЧЕНИЕ(Перечисление.СпособыПередачиТоваров.ПередачаНаКомиссию)
	|
	|ГДЕ
	...
	|				И (НастройкиУчетаНДСКонтрагентов.Комитент ЕСТЬ NULL
	|					ИЛИ (ЕСТЬNULL(НастройкиУчетаНДСКонтрагентов.ОсновноеНалогообложение, НЕОПРЕДЕЛЕНО) В (&НалогообложенияСНДС)
	|						И ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС) В (&НалогообложениеНДС))
	|					ИЛИ (ЕСТЬNULL(НастройкиУчетаНДСКонтрагентов.ОсновноеНалогообложение, НЕОПРЕДЕЛЕНО) В (&НалогообложенияБезНДС)
	|						И ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС) В(&НалогообложениеНДС))
	|					)
	|		)
	...
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТНастройкиУчетаНДСОрганизаций
	|
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТНастройкиУчетаНДСКонтрагентов";
	...
	Если КонтролироватьНалогообложениеНДС
		...
		ОрганизацииБезНДС = УчетНДСУП.ОрганизацииБезНДС(Период);
		
	Иначе
		ОрганизацииБезНДС = Новый Массив;
	КонецЕсли;
	
	УчетНДСУП.СформироватьВТНастройкиУчетаНДСОрганизаций(МенеджерВременныхТаблиц, Период, ОрганизацииБезНДС);
	...
	Запрос.УстановитьПараметр("НалогообложенияСНДС",	Перечисления.ТипыНалогообложенияНДС.ТипыСНДС());
	Запрос.УстановитьПараметр("НалогообложенияБезНДС",	Перечисления.ТипыНалогообложенияНДС.ТипыБезНДС());
	
	Запрос.УстановитьПараметр("ПодбиратьКомиссионныеЗапасыПоНалогообложениюКомитента",
								ПараметрыЗаполненияВидовЗапасов.ПодбиратьКомиссионныеЗапасыПоНалогообложениюКомитента);
...
//		* ОтображатьСуммуАкциза - Булево - признак необходимости отображения колонки, содержащей информацию 
//													о сумме акциза.
//		* ИспользоватьПодакцизныеТовары - Булево - признак возможности указания облагаемых акцизом видов запасов вручную.
...
//		* ИмяКолонкиНазначение - Строка - имя колонки, содержащей информацию о назначении товара.
//		* МестоХраненияВТабличнойЧасти - Булево - признак того, что источником места хранения номенклатуры является
//													реквизит Склад табличной части Товары. Для игнорирования значений
//													табличной части установить значение параметра ЛОЖЬ и
//													инициализировать значение параметра Склад.
...
//		* АдресДанныхДокументаВХранилище - Строка - адрес временного хранилища, в котором хранится структура данных
//													документа, для которого выполняется редактирование табличной части.
//
Функция ПараметрыРедактированияВидовЗапасов() Экспорт
	...
	Результат.Вставить("ИмяКолонкиНазначение",           "Назначение");
	Результат.Вставить("МестоХраненияВТабличнойЧасти",   Истина);
	Результат.Вставить("РедактироватьВидыЗапасов",       Истина);
	Результат.Вставить("ОтображатьДокументыРеализации",  Ложь);
	Результат.Вставить("ОтображатьСуммуАкциза",          Ложь);
	Результат.Вставить("ИспользоватьПодакцизныеТовары",  Ложь);
	...
	Результат.Вставить("АдресДанныхДокументаВХранилище", "");
...
Функция ПоместитьТоварыИВидыЗапасовВХранилище(Форма, ПараметрыРедактированияВидовЗапасов = Неопределено) Экспорт
	...
	Объект                  = Форма.Объект;
	Товары                  = Объект[ПараметрыРедактированияВидовЗапасов.ИмяТЧТовары];
	ВидыЗапасов             = Объект[ПараметрыРедактированияВидовЗапасов.ИмяТЧВидыЗапасов];
	...
	Иначе
		...
		Если ПараметрыРедактированияВидовЗапасов.ИмяКолонкиНазначение <> "Назначение" Тогда
			Если ТаблицаТоваров.Колонки.Найти("Назначение") <> Неопределено Тогда
				ТаблицаТоваров.Колонки.Удалить("Назначение");
			КонецЕсли;
			
			КолонкаНазначение = ТаблицаТоваров.Колонки[ПараметрыРедактированияВидовЗапасов.ИмяКолонкиНазначение]; // КолонкаТаблицыЗначений
			КолонкаНазначение.Имя = "Назначение";
		КонецЕсли;
		
		Если Не ПараметрыРедактированияВидовЗапасов.МестоХраненияВТабличнойЧасти Тогда
			Если ТаблицаТоваров.Колонки.Найти("Склад") <> Неопределено Тогда
				ТаблицаТоваров.Колонки.Удалить("Склад");
			КонецЕсли;
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ПараметрыРедактированияВидовЗапасов.Организация) Тогда
		ПараметрыРедактированияВидовЗапасов.Организация = Объект.Организация;
	КонецЕсли;
	
	ПараметрыРедактированияВидовЗапасов.ВидыЗапасовУказаныВручную = Объект.ВидыЗапасовУказаныВручную;
	
	Если Объект.Свойство("ЦенаВключаетНДС") Тогда
		ПараметрыРедактированияВидовЗапасов.ЦенаВключаетНДС = Объект.ЦенаВключаетНДС;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ПараметрыРедактированияВидовЗапасов.Склад)
		И Объект.Свойство("Склад") Тогда
		
		ПараметрыРедактированияВидовЗапасов.Склад = Объект.Склад;
	...
	Если Не ЗначениеЗаполнено(ПараметрыРедактированияВидовЗапасов.Подразделение)
		...
		ПараметрыРедактированияВидовЗапасов.Подразделение = Объект.Подразделение;
	...
	ПараметрыРедактированияВидовЗапасов.Регистратор		= Объект.Ссылка;
	ПараметрыРедактированияВидовЗапасов.ДатаДокумента	= Объект.Дата;
	
	ДанныеДокумента = ОбщегоНазначенияУТ.СтруктураСоСвойствамиОбъектаМетаданных(Объект.Ссылка.Метаданные());
	ЗаполнитьЗначенияСвойств(ДанныеДокумента, Объект);
	
	ДанныеДокумента.Вставить(ПараметрыРедактированияВидовЗапасов.ИмяТЧТовары, Товары.Выгрузить());
	
	ПараметрыРедактированияВидовЗапасов.АдресДанныхДокументаВХранилище = ПоместитьВоВременноеХранилище(
																			ДанныеДокумента,
																			УникальныйИдентификатор);
...
Процедура ПроверитьЗаполнениеПодразделенияВТабличнойЧасти(ДокументОбъект,
	...
	Если Не ЗначениеЗаполнено(ДокументОбъект[ИмяТЧ]) Тогда
		Возврат;
	КонецЕсли;
	...
	// Если проверяются только услуга или работа, то флаг "СписатьНаРасходы" не используется для проверки условия. 
	// При этом он может отсутствовать в документе, поэтому необходимо отключить его использование.
	ТоварТара = Новый Массив;
	ТоварТара.Добавить(Перечисления.ТипыНоменклатуры.Товар);
	ТоварТара.Добавить(Перечисления.ТипыНоменклатуры.МногооборотнаяТара);
	
	ПроверятьСписаниеНаРасходы = Ложь;
	Для Каждого ПроверяемыйТипНоменклатуры Из ТоварТара Цикл
		
		Если ТипыНоменклатуры.Найти(ПроверяемыйТипНоменклатуры) <> Неопределено Тогда
			ПроверятьСписаниеНаРасходы = Истина;
			Прервать;
		КонецЕсли;
	...
	ВыгружаемыеКолонки.Добавить("НомерСтроки");
	ВыгружаемыеКолонки.Добавить("Номенклатура");
	ВыгружаемыеКолонки.Добавить("Назначение");
	ВыгружаемыеКолонки.Добавить("Подразделение");
	
	Если ПроверятьСписаниеНаРасходы Тогда
		ВыгружаемыеКолонки.Добавить("СписатьНаРасходы");
	КонецЕсли;
	
	ВыгружаемыеКолонки = СтрСоединить(ВыгружаемыеКолонки, ", ");
	
	Если ТипЗнч(ДокументОбъект[ИмяТЧ]) = Тип("ТаблицаЗначений") Тогда
		ТаблицаТовары = ДокументОбъект[ИмяТЧ].Скопировать(, ВыгружаемыеКолонки); // ТаблицаЗначений
	Иначе
		ТаблицаТовары = ДокументОбъект[ИмяТЧ].Выгрузить(, ВыгружаемыеКолонки); // ТаблицаЗначений
	КонецЕсли;
	
	Если Не ПроверятьСписаниеНаРасходы Тогда
		ТаблицаТовары.Колонки.Добавить("СписатьНаРасходы", Новый ОписаниеТипов("Булево"));
	КонецЕсли;
...
Функция ПараметрыПроверкиЗаполненияНомеровГТД() Экспорт
	...
	ПараметрыПроверки.Вставить("ПредставлениеНомераГТД", "");
...
Процедура ПроверитьЗаполнениеНомеровГТД(ДокументОбъект, Отказ, ПараметрыПроверки = Неопределено) Экспорт
	...
	ПредставлениеНомераГТД =
		?(ПустаяСтрока(ПараметрыПроверки.ПредставлениеНомераГТД),
			НСтр("ru = 'Номер ГТД'"),
			ПараметрыПроверки.ПредставлениеНомераГТД);
						
	ШаблонОшибкаНеЗаполненНомерГТД = НСтр("ru = 'Не заполнена колонка ""%1"" в строке %2 списка ""%3""'");
...
Процедура ПроверитьНаличиеИзделийВТЧСУчетомПоГТД(ДокументОбъект, Отказ, ПараметрыПроверки = Неопределено) Экспорт
	...
	ИспользоватьУчетПрослеживаемыхИмпортныхТоваров =
		Не ПараметрыПроверки.РазрешитьПрослеживаемыйТоварБезНомеровГТД
		И УчетПрослеживаемыхТоваровЛокализация.ИспользоватьУчетПрослеживаемыхИмпортныхТоваров(Дата(1, 1, 1));
	...
	|			И &ИспользоватьУчетПрослеживаемыхИмпортныхТоваров)
	|		ИЛИ (ДанныеНоменклатуры.ВестиУчетПоГТД
	...
	|	
	|ГДЕ
	|	ДанныеНоменклатуры.ПрослеживаемыйТовар
	|	И &ИспользоватьУчетПрослеживаемыхИмпортныхТоваров
	...
	ШаблонИмпортныйТовар		= НСтр("ru = 'В строке %1 списка ""%2"" выбрана номенклатура, для которой ведется учет по ГТД.
										|При включенной опции ""Запретить операции с товарами без номеров ГТД"" выбор такой номенклатуры запрещен.'");
	ШаблонПрослеживаемыйТовар	= НСтр("ru = 'В строке %1 списка ""%2"" выбрана прослеживаемая номенклатура.
										|При включенном учете прослеживаемых импортных товаров выбор такой номенклатуры запрещен.'");
	
	Пока Выборка.Следующий() Цикл
		ТекстСообщения	= ?(Выборка.ПрослеживаемыйТовар, ШаблонПрослеживаемыйТовар, ШаблонИмпортныйТовар);
...
Процедура ПроверитьНаличиеИзделийВШапкеСУчетомПоГТД(ДокументОбъект, Отказ) Экспорт
	...
	ИменаРеквизитов		= "ВестиУчетПоГТД, ПрослеживаемыйТовар, ТипНоменклатуры";
	РеквизитыИзделия	= ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОбъект.Номенклатура, ИменаРеквизитов);
	
	ТекстСообщения = "";
	
	Если РеквизитыИзделия.ПрослеживаемыйТовар = Истина
		И ИспользоватьУчетПрослеживаемыхИмпортныхТоваров Тогда
		
		ТекстСообщения = НСтр("ru = 'В документе выбрана прослеживаемая номенклатура.
							|При включенном учете прослеживаемых импортных товаров выбор такой номенклатуры запрещен.'");
		
	ИначеЕсли РеквизитыИзделия.ВестиУчетПоГТД = Истина
		И ЗапретитьОформлениеОперацийСИмпортнымиТоварамиБезНомеровГТД
		...
		ТекстСообщения = НСтр("ru = 'В документе выбрана номенклатура, для которой ведется учет по ГТД.
							|При включенной опции ""Запретить операции с товарами без номеров ГТД"" выбор такой номенклатуры запрещен.'");
		
	КонецЕсли;
	
	Если Не ПустаяСтрока(ТекстСообщения) Тогда
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ДокументОбъект, "Номенклатура", , Отказ);
	КонецЕсли;
...
Процедура ДобавитьЭтап_ИсправлениеДублейВидовЗапасов(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов,
															ТекущийРодитель,
															Справочники.ОперацииЗакрытияМесяца.ИсправлениеДублейВидовЗапасов,
															Истина,
															Ложь,
															Ложь);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Исправление дублей видов запасов'"; // @НСтр-1
...
Процедура ДобавитьЭтап_ИсправлениеОтрицательныхОстатковТоваровОрганизаций(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ИсправлениеОтрицательныхОстатковТоваровОрганизаций);
		
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Исправление отрицательных остатков товаров организаций'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
...
Процедура ДобавитьЭтап_ОформлениеПередачТоваров(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ОформлениеПередачТоваров);
		
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Оформление передач товаров'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
...
Процедура ДобавитьЭтап_ОформлениеОстатковТоваровКПередаче(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ОформлениеОстатковТоваровКПередаче);
	
	НоваяСтрока.МультиязычноеНаименование =
		"ru = 'Оформление передач и возвратов товаров (по данным, введенным до версии 2.4)'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
	НоваяСтрока.ТекстВыполнить = НСтр("ru='Оформить'");
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
...
Процедура ДобавитьЭтап_ОформлениеВыкупаТоваров(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ОформлениеВыкупаТоваров);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Оформление выкупа товаров'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
	НоваяСтрока.ТекстВыполнить = НСтр("ru='Оформить'");
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
...
Процедура ДобавитьЭтап_ОформлениеОтчетовПоКомиссииМеждуОрганизациями(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ОформлениеОтчетовПоКомиссииМеждуОрганизациями);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Оформление отчетов по комиссии между организациями'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
	НоваяСтрока.ТекстВыполнить     = НСтр("ru='Оформить'");
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
...
Процедура ДобавитьЭтап_ОформлениеОтчетовКомитентамОСписании(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Оформление отчетов комитентам о списании'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
	НоваяСтрока.ТекстВыполнить     = НСтр("ru='Оформить'");
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
...
Функция ПараметрЗаполненияВидовЗапасовПоУмолчаниюИнициализирован(ПараметрыЗаполненияВидовЗапасов)
...
Процедура ЗаполнитьВидыЗапасовПоТаблицеОстатковСформироватьВТНовыеРезервы(ДокументОбъект, МенеджерВременныхТаблиц, Отказ, ПараметрыЗаполненияВидовЗапасов)
	...
	ПараметрЗаполненияВидовЗапасовПоУмолчаниюИнициализирован = ПараметрЗаполненияВидовЗапасовПоУмолчаниюИнициализирован(
																ПараметрыЗаполненияВидовЗапасов);
	...
	Если ЗначениеЗаполнено(ПараметрыЗаполненияВидовЗапасов.ИмяПоляОрганизация) Тогда
		Организация = ДокументОбъект[ПараметрыЗаполненияВидовЗапасов.ИмяПоляОрганизация];
	ИначеЕсли ТипЗнч(ПараметрыЗаполненияВидовЗапасов.ОтборыВидовЗапасов.Организация) = Тип("Массив") Тогда
		Если ЗначениеЗаполнено(ПараметрыЗаполненияВидовЗапасов.ОтборыВидовЗапасов.Организация) Тогда
			Организация = ПараметрыЗаполненияВидовЗапасов.ОтборыВидовЗапасов.Организация[0];
		Иначе
			Организация = Справочники.Организации.ПустаяСсылка();
		КонецЕсли;
	Иначе
		Организация = ПараметрыЗаполненияВидовЗапасов.ОтборыВидовЗапасов.Организация;
	КонецЕсли;
	
	СтранаРегистрации = ЗначениеНастроекКлиентСерверПовтИсп.СтранаРегистрацииОрганизации(Организация);
	
	Если ЗначениеЗаполнено(ПараметрыЗаполненияВидовЗапасов.ПериодНачисленияАкциза) Тогда
		Дата = ПараметрыЗаполненияВидовЗапасов.ПериодНачисленияАкциза;
	Иначе
	...
	Запрос.УстановитьПараметр("ИспользоватьПодакцизныеТовары",
								ПолучитьФункциональнуюОпцию("ИспользоватьПодакцизныеТовары"));
	...
	ИндексНеУТКА = 1;
	ИндексНеУТКА = ИндексНеУТКА - ?(ПараметрыЗаполненияВидовЗапасов.ПодбиратьЗапасыРазныхПериодов, 2, 0);
	
	ВыборкаДанныеДокумента	= МассивРезультатов[3 - ИндексНеУТКА].Выбрать();
	ВыборкаТаблицаТоваров	= МассивРезультатов[4 - ИндексНеУТКА].Выбрать();
	ТаблицаОстатков			= МассивРезультатов[6 - ИндексНеУТКА].Выгрузить(); // ТаблицаЗначений -
	ВыборкаОстаткиПоСтрокам	= МассивРезультатов[7 - ИндексНеУТКА].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	...
	ТаблицаОстатков.ЗагрузитьКолонку(ТаблицаОстатков.ВыгрузитьКолонку("ОрганизацияВидаЗапасов"), "ОрганизацияОстатка");
	...
	ОблагаетсяАкцизом = ПараметрыЗаполненияВидовЗапасов.ОтборыВидовЗапасов.ОблагаетсяАкцизом;
	ТолькоНеНалогооблагаемыеАкцизомТовары = ЗначениеЗаполнено(ОблагаетсяАкцизом);
	
	ПараметрыЗаполненияВидовЗапасовВручную = ?(ДополнительныеСвойства.Свойство("ПараметрыЗаполненияВидовЗапасовВручную"),
												ДополнительныеСвойства.ПараметрыЗаполненияВидовЗапасовВручную,
	...
	Пока ВыборкаОстаткиПоСтрокам.Следующий() Цикл
				...
				ИначеЕсли Выборка.НастройкаПередачи = Перечисления.СпособыПередачиТоваров.ПередачаНаКомиссию
					...
					Иначе	
						...
						НалогообложениеКомиссионногоВидаЗапасов =
							?(Перечисления.ТипыНалогообложенияНДС.ТипыСНДС().Найти(ПараметрыУчетаПоОрганизации.ОсновноеНалогообложениеНДСПродажи) <> Неопределено,
								Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС,
								Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС);
					КонецЕсли;
		...
		Если КоличествоТовара <> 0
			...
			КонтролироватьПодакцизныеТовары = ВыборкаОстаткиПоСтрокам.ПодакцизныйТовар
												И Не ТолькоНеНалогооблагаемыеАкцизомТовары
												И Не ПараметрЗаполненияВидовЗапасовПоУмолчаниюИнициализирован
												И Не ПараметрыЗаполненияВидовЗапасов.ЗаполнятьПодакцизныеТоварыВидамиЗапасовПоУмолчанию;
			
			Если (ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию
						И Не КонтролироватьПодакцизныеТовары)
					Или ((Не ВыборкаОстаткиПоСтрокам.ПодакцизныйТовар
							Или ТолькоНеНалогооблагаемыеАкцизомТовары
							Или ПараметрыЗаполненияВидовЗапасов.ЗаполнятьПодакцизныеТоварыВидамиЗапасовПоУмолчанию)
						И ПроводитьБезКонтроляОстатковТоваровОрганизаций
				...
				Если ВыборкаТаблицаТоваров.НайтиСледующий(ОтборСтрок) Тогда
					...
					Если ПараметрыЗаполненияВидовЗапасов.ВладелецТовараВШапке Тогда
						СтруктураВидаЗапасовПоУмолчанию.ТипЗапасов	= ?(ВыборкаТаблицаТоваров.ТипЗапасов = Перечисления.ТипыЗапасов.Работа,
																		...
																		СтруктураВидаЗапасовПоУмолчанию.ТипЗапасов);
					Иначе
					...
					СтруктураВидаЗапасовПоУмолчанию.ГруппаФинансовогоУчета	= ВыборкаТаблицаТоваров.ГруппаФинансовогоУчета;
					
					КлючВидаЗапасов = КлючКэшаОтбораВидовЗапасов(ПараметрыЗаполненияВидовЗапасов.ВладелецТовараВШапке,
																СтруктураВидаЗапасовПоУмолчанию);
					ВидЗапасовПоУмолчанию = ВидыЗапасовПоУмолчанию[КлючВидаЗапасов];
					...
					Если ВидЗапасовПоУмолчанию = Неопределено Тогда
						...
						ВидыЗапасовПоУмолчанию.Вставить(КлючВидаЗапасов, ВидЗапасовПоУмолчанию);
					КонецЕсли;
			...
			Если (Не ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию
					Или КонтролироватьПодакцизныеТовары)
				И Не ПараметрыЗаполненияВидовЗапасовВручную.ИгнорироватьОшибкиКонтроляОстатков Тогда
				...
				Если ((ВыборкаОстаткиПоСтрокам.ПодакцизныйТовар
							И Не ТолькоНеНалогооблагаемыеАкцизомТовары
							И Не ПараметрыЗаполненияВидовЗапасов.ЗаполнятьПодакцизныеТоварыВидамиЗапасовПоУмолчанию)
						Или Не ПроводитьБезКонтроляОстатковТоваровОрганизаций
...
Функция КонтролироватьНалогообложениеНДС(ПараметрыЗаполненияВидовЗапасов)
	...
	Если Не ЗначениеЗаполнено(НалогообложениеНДС)
		Или Не ПолучитьФункциональнуюОпцию("ИспользоватьУчетНДС") Тогда
...
Процедура СформироватьВТСторноРезервов(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	|		КОНЕЦ КАК КоличествоПоРНПТ,
	|		ЕСТЬNULL(ДатыПоступления.ДатаПоступления, ДАТАВРЕМЯ(2399, 1, 1)) КАК ДатаПоступления
	|	ИЗ
	|		РегистрНакопления.РезервыТоваровОрганизаций КАК РезервыТоваровОрганизаций
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК КлючиАналитики
	|			ПО РезервыТоваровОрганизаций.АналитикаУчетаНоменклатуры = КлючиАналитики.Ссылка
	|			
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДатыПоступленияТоваровОрганизаций КАК ДатыПоступления
	|			ПО РезервыТоваровОрганизаций.КорВидЗапасов = ДатыПоступления.ВидЗапасов
	|				И КлючиАналитики.Номенклатура = ДатыПоступления.Номенклатура
	|				И КлючиАналитики.Характеристика = ДатыПоступления.Характеристика
	|				И КлючиАналитики.Серия = ДатыПоступления.Серия
	|				И КлючиАналитики.Назначение = ДатыПоступления.Назначение
	|				И РезервыТоваровОрганизаций.НомерГТД = ДатыПоступления.НомерГТД
	|	ГДЕ
	...
	|		ЕСТЬNULL(ДатыПоступления.ДатаПоступления, ДАТАВРЕМЯ(2399, 1, 1))
	|	ИЗ
	|		ВТСторноРезервовПредыдущихПериодов КАК СторноРезервов
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК КлючиАналитики
	|			ПО СторноРезервов.АналитикаУчетаНоменклатуры = КлючиАналитики.Ссылка
	|			
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДатыПоступленияТоваровОрганизаций КАК ДатыПоступления
	|			ПО СторноРезервов.КорВидЗапасов = ДатыПоступления.ВидЗапасов
	|				И КлючиАналитики.Номенклатура = ДатыПоступления.Номенклатура
	|				И КлючиАналитики.Характеристика = ДатыПоступления.Характеристика
	|				И КлючиАналитики.Серия = ДатыПоступления.Серия
	|				И КлючиАналитики.Назначение = ДатыПоступления.Назначение
	|				И СторноРезервов.НомерГТД = ДатыПоступления.НомерГТД
	|	ГДЕ
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК КлючиАналитики
	|			ПО СформированныеРезервы.АналитикаУчетаНоменклатуры = КлючиАналитики.Ссылка
	|			
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДатыПоступленияТоваровОрганизаций КАК ДатыПоступления
	|			ПО СформированныеРезервы.КорВидЗапасов = ДатыПоступления.ВидЗапасов
	|				И КлючиАналитики.Номенклатура = ДатыПоступления.Номенклатура
	|				И КлючиАналитики.Характеристика = ДатыПоступления.Характеристика
	|				И КлючиАналитики.Серия = ДатыПоступления.Серия
	|				И КлючиАналитики.Назначение = ДатыПоступления.Назначение
	|				И СформированныеРезервы.НомерГТД = ДатыПоступления.НомерГТД
	|	ГДЕ
...
Процедура ДозаполнитьВидыЗапасовВОтчетахПоКомиссииИнтеркампани(ДокументОбъект,
	...
	|
	|/////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ОтборОстатков";
	
	ВыгружаемыеКолонки	= "АналитикаУчетаНоменклатуры, НомерГТД";
	
	Запрос.УстановитьПараметр("Организация",	ДокументОбъект.Организация);
	Запрос.УстановитьПараметр("ВидыЗапасов",	ДокументОбъект.ВидыЗапасов.Выгрузить(, ВыгружаемыеКолонки));
	
	МассивРезультатов = Запрос.ВыполнитьПакет();
	ТаблицаОстатков = МассивРезультатов[МассивРезультатов.ВГраница() - 2].Выгрузить();
	
	КолонкиОтбораСтрок = "АналитикаУчетаНоменклатуры, НомерГТД, ВидЗапасовКомитента";
	
	ВидыЗапасов = ДокументОбъект.ВидыЗапасов.Выгрузить();
	ВидыЗапасов.Сортировать("АналитикаУчетаНоменклатуры, НомерГТД, Количество ВОЗР");
	ВидыЗапасов.Индексы.Добавить("Количество");
	ВидыЗапасов.Индексы.Добавить(КолонкиОтбораСтрок);
	ВидыЗапасов.Индексы.Добавить("ВидЗапасовКомитента");
	
	ДокументОбъект.ВидыЗапасов.Сортировать("АналитикаУчетаНоменклатуры, НомерГТД, Количество ВОЗР");
	
	ОтборСтрокОстатков	= Новый Структура(ВыгружаемыеКолонки);
	ОтборСтрокДокумента	= Новый Структура(КолонкиОтбораСтрок);
	...
	Для Каждого СтрокаОстатка Из ТаблицаОстатков Цикл
		...
		Для Каждого СтрокаЗапасов Из ВидыЗапасов.НайтиСтроки(ОтборСтрокДокумента) Цикл
	...
	СтрокиСПустымВидомЗапасовКомитента = ВидыЗапасов.НайтиСтроки(ОтборСтрок);
	...
	ДокументОбъект.ВидыЗапасов.Загрузить(ВидыЗапасов);
...
Процедура ЗаполнитьВидыЗапасовПоТаблицеОстатковКомиссионныхТоваровИнтеркампани(ДокументОбъект,
	...
	СтруктураВидаЗапасовПоУмолчанию = Новый Структура(ИменаРеквизитов);
	...
	|			ТОГДА ЕСТЬNULL(ДанныеНоменклатуры.ГруппаФинансовогоУчета, ЗНАЧЕНИЕ(Справочник.ГруппыФинансовогоУчетаНоменклатуры.ПустаяСсылка))
	...
	Остатки			= Результат[Результат.ВГраница() - 1].Выгрузить();
	ВыборкаТовары	= Результат[Результат.ВГраница()].Выбрать();
...
Процедура ЗаполнитьВидыЗапасовПоТаблицеОстатков(ДокументОбъект,
	...
	МассивРезультатов = Запрос.ВыполнитьПакет();
	ВыборкаДанныеДокумента	= МассивРезультатов[2].Выбрать();
	ВыборкаТаблицаТоваров	= МассивРезультатов[3].Выбрать();
	...
	РезультатЗапроса		= МассивРезультатов[МассивРезультатов.ВГраница() - 2];
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СгруппированнаяТаблицаОстатков.АналитикаУчетаНоменклатурыОтгрузки КАК АналитикаУчетаНоменклатурыОтгрузки,
	|	СгруппированнаяТаблицаОстатков.Серия					КАК Серия,
	|	СгруппированнаяТаблицаОстатков.ВидЗапасов				КАК ВидЗапасов,
	|	СгруппированнаяТаблицаОстатков.ВидЗапасовПолучателя		КАК ВидЗапасовПолучателя,
	|	СгруппированнаяТаблицаОстатков.ВидЗапасовОтгрузки		КАК ВидЗапасовОтгрузки,
	|	СгруппированнаяТаблицаОстатков.КорВидЗапасов			КАК КорВидЗапасов,
	|	СгруппированнаяТаблицаОстатков.НомерГТД					КАК НомерГТД,
	|	СгруппированнаяТаблицаОстатков.ДокументРеализации		КАК ДокументРеализации,
	|	СгруппированнаяТаблицаОстатков.ДокументОприходования	КАК ДокументОприходования,
	|	СУММА(СгруппированнаяТаблицаОстатков.КоличествоОстаток)	КАК КоличествоОстаток
	|ИЗ
	|	(ВЫБРАТЬ
	|		ЕСТЬNULL(АналитикаОтгрузки.КлючАналитики, ТаблицаОстатков.АналитикаУчетаНоменклатуры) КАК АналитикаУчетаНоменклатурыОтгрузки,
	|		ВЫБОР
	|			КОГДА АналитикаОтгрузки.КлючАналитики ЕСТЬ NULL
	|				ТОГДА ТаблицаОстатков.Серия
	|			ИНАЧЕ АналитикаОтгрузки.Серия
	|		КОНЕЦ										КАК Серия,
	|		ТаблицаОстатков.ВидЗапасов					КАК ВидЗапасов,
	|		ТаблицаОстатков.ВидЗапасовПолучателя		КАК ВидЗапасовПолучателя,
	|		ТаблицаОстатков.ВидЗапасовОтгрузки			КАК ВидЗапасовОтгрузки,
	|		ТаблицаОстатков.КорВидЗапасов				КАК КорВидЗапасов,
	|		ТаблицаОстатков.НомерГТД					КАК НомерГТД,
	|		ТаблицаОстатков.ДокументРеализации			КАК ДокументРеализации,
	...
	|			ПО ТаблицаОстатков.Номенклатура = АналитикаОтгрузки.Номенклатура
	|				И ТаблицаОстатков.Характеристика = АналитикаОтгрузки.Характеристика
	|				И ТаблицаОстатков.СерияОтгрузки = АналитикаОтгрузки.Серия
	|				И ТаблицаОстатков.СкладОтгрузки = АналитикаОтгрузки.МестоХранения
	|				И ТаблицаОстатков.НазначениеОтгрузки = АналитикаОтгрузки.Назначение
	|				И ТаблицаОстатков.СтатьяКалькуляции = АналитикаОтгрузки.СтатьяКалькуляции
	|	
	|	ГДЕ
	|		ТаблицаОстатков.ОстаткиДокумента
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ЕСТЬNULL(АналитикаОтгрузки.КлючАналитики, ТаблицаОстатков.АналитикаУчетаНоменклатуры),
	|		ВЫБОР
	|			КОГДА АналитикаОтгрузки.КлючАналитики ЕСТЬ NULL
	|				ТОГДА ТаблицаОстатков.Серия
	|			ИНАЧЕ АналитикаОтгрузки.Серия
	|		КОНЕЦ,
	|		ТаблицаОстатков.ВидЗапасов,
	|		ТаблицаОстатков.ВидЗапасовПолучателя,
	|		ТаблицаОстатков.ВидЗапасовОтгрузки,
	|		ТаблицаОстатков.КорВидЗапасов,
	|		ТаблицаОстатков.НомерГТД,
	|		ТаблицаОстатков.ДокументРеализации,
	|		ТаблицаОстатков.ДокументОприходования
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		ТаблицаОстатков.ДокументРеализации			КАК ДокументРеализации,
	|		ТаблицаОстатков.ДокументОприходования		КАК ДокументОприходования,
	|		МАКСИМУМ(ТаблицаОстатков.КоличествоОстаток)	КАК КоличествоОстаток
	|	ИЗ
	|		ВтОстатки КАК ТаблицаОстатков
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК АналитикаОтгрузки
	|			ПО ТаблицаОстатков.Номенклатура = АналитикаОтгрузки.Номенклатура
	...
	|	
	|	ГДЕ
	|		НЕ ТаблицаОстатков.ОстаткиДокумента
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ЕСТЬNULL(АналитикаОтгрузки.КлючАналитики, ТаблицаОстатков.АналитикаУчетаНоменклатуры),
	|		ВЫБОР
	...
	|		ТаблицаОстатков.ДокументОприходования
	|	
	|	) КАК СгруппированнаяТаблицаОстатков
	|
	|СГРУППИРОВАТЬ ПО
	|	СгруппированнаяТаблицаОстатков.АналитикаУчетаНоменклатурыОтгрузки,
	|	СгруппированнаяТаблицаОстатков.Серия,
	|	СгруппированнаяТаблицаОстатков.ВидЗапасов,
	|	СгруппированнаяТаблицаОстатков.ВидЗапасовПолучателя,
	|	СгруппированнаяТаблицаОстатков.ВидЗапасовОтгрузки,
	|	СгруппированнаяТаблицаОстатков.КорВидЗапасов,
	|	СгруппированнаяТаблицаОстатков.НомерГТД,
	|	СгруппированнаяТаблицаОстатков.ДокументРеализации,
	|	СгруппированнаяТаблицаОстатков.ДокументОприходования
	|";
	
	ТаблицаОстатков = Запрос.Выполнить().Выгрузить();
	ТаблицаОстатков.Индексы.Добавить(КлючПоискаОстатков);
	
	ТаблицаОстатковПоСтрокам = Неопределено;
	...
	Пока ВыборкаОстаткиПоСтрокам.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Для Каждого КлючИЗначение Из ОтборОстатков Цикл
				Если Не ЗначениеЗаполнено(КлючИЗначение.Значение) Тогда
					ОтборОстатков[КлючИЗначение.Ключ] =
						ТаблицаОстатков.Колонки[КлючИЗначение.Ключ].ТипЗначения.ПривестиЗначение(КлючИЗначение.Значение);
				КонецЕсли;
			КонецЦикла;
			...
			Если СтрокиОстатков.Количество() > 0 Тогда
				Для Каждого СтрокаОстатков Из СтрокиОстатков Цикл
					Если (КоличествоОстаток > 0
						...
						Если Не ЗначениеЗаполнено(Выборка.Серия)
							И Выборка.Серия <> СтрокаОстатков.Серия Тогда
							
							ТекущаяСтрокаОстатков = СтрокаОстатков;
							
							Если ТаблицаОстатковПоСтрокам = Неопределено Тогда
								ТаблицаОстатковПоСтрокам = ОбщегоНазначенияУТ.ДанныеДерева(
									РезультатЗапроса.Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкам), 1);
								ТаблицаОстатковПоСтрокам.Индексы.Добавить(КлючПоискаОстатков);
							КонецЕсли;
							
							СтрокиТоваров = ТаблицаОстатковПоСтрокам.НайтиСтроки(ОтборОстатков);
							
							Если СтрокиТоваров.Количество() > 1 Тогда
								УдаляемаяСтрока = Неопределено;
								
								Для Каждого СтрокаОстаткаТоваров Из СтрокиТоваров Цикл
									
									Если Выборка.НомерСтроки = СтрокаОстаткаТоваров.НомерСтроки Тогда
										Если Выборка.АналитикаУчетаНоменклатурыОтгрузки = СтрокаОстаткаТоваров.АналитикаУчетаНоменклатурыОтгрузки Тогда
											УдаляемаяСтрока = СтрокаОстаткаТоваров;
										КонецЕсли;
									...
									Если (КоличествоОстаток > 0
											И СтрокаОстаткаТоваров.КоличествоОстаток <= 0)
										Или (КоличествоОстаток < 0
											И СтрокаОстаткаТоваров.КоличествоОстаток >= 0) Тогда
										
										Продолжить;
										
									КонецЕсли;
									
									Если СтрокаОстатков.Серия = СтрокаОстаткаТоваров.Серия Тогда
										ОстатокПоСтроке = СтрокаОстатков.КоличествоОстаток - Выборка.КоличествоОстаток;
										
										Если (КоличествоОстаток > 0
												И ОстатокПоСтроке < СтрокаОстаткаТоваров.КоличествоОстаток)
											Или (КоличествоОстаток < 0
												И ОстатокПоСтроке > СтрокаОстаткаТоваров.КоличествоОстаток) Тогда
								...
								Если УдаляемаяСтрока <> Неопределено Тогда
									ТаблицаОстатковПоСтрокам.Удалить(УдаляемаяСтрока)
								КонецЕсли;
							КонецЕсли;
			...
			ИначеЕсли (КоличествоТовара > 0
				...
				Если ТекущаяСтрокаОстатков <> Неопределено Тогда
					ТекущаяСтрокаОстатков.КоличествоОстаток = ТекущаяСтрокаОстатков.КоличествоОстаток
																- СтруктураДанныхСтроки.Количество;
				КонецЕсли;
...
Функция ЗапросРаспределенияБезУчетаПриоритетовТаблицыТоваровНаТаблицуОстатков(МенеджерВременныхТаблиц,
	...
	|	ТаблицаНоменклатуры.ВестиУчетПоГТД КАК ВестиУчетПоГТД,
	|	ВЫБОР
	|		КОГДА ТаблицаНоменклатуры.ПрослеживаемыйТовар
	|				И &ИспользоватьУчетПрослеживаемыхИмпортныхТоваров
	...
	|	&ТекстПоляВыборкиТаблицаТоваровСтавкаНДС_ КАК СтавкаНДС,
	|	&ТекстПоляВыборкиСтавкиНДСБазоваяСтавкаНДС_ КАК БазоваяСтавкаНДС,
	...
	|		ПО &ТекстПоляТаблицаТоваровСтавкаНДС_ = СтавкиНДС.Ссылка
	...
	|		ПО ТаблицаНоменклатуры.Ссылка = ВтПодакцизныеТовары.Номенклатура
	|
	|ГДЕ
	...
	|	ТаблицаНоменклатуры.ВестиУчетПоГТД,
	|	ВЫБОР
	|		КОГДА ТаблицаНоменклатуры.ПрослеживаемыйТовар
	|				И &ИспользоватьУчетПрослеживаемыхИмпортныхТоваров
	|				И НАЧАЛОПЕРИОДА(ТаблицаДанныхДокумента.Дата, МЕСЯЦ) >= &ДатаНачалаУчетаПрослеживаемыхИмпортныхТоваров
	...
	|	&ТекстПоляГруппировкиТаблицаТоваровСтавкаНДС_,
	|	&ТекстПоляГруппировкиСтавкиНДСБазоваяСтавкаНДС_,
	...
	|	ВтОстаткиБезГруппировки.ОтборПоСтавкеНДСКомитента КАК ОтборПоСтавкеНДСКомитента,
	|	ВтОстаткиБезГруппировки.СтавкаНДСКомитента КАК СтавкаНДСКомитента
	|ПОМЕСТИТЬ ВтОстатки
	|ИЗ
	...
	|		ВЫБОР
	|			КОГДА НЕ ДоступныеВидыЗапасов.ВидЗапасов ЕСТЬ NULL
	|				ТОГДА ДоступныеВидыЗапасов.ОтборПоСтавкеНДСКомитента
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ КАК ОтборПоСтавкеНДСКомитента,
	|		ВЫБОР
	|			КОГДА НЕ ДоступныеВидыЗапасов.ВидЗапасов ЕСТЬ NULL
	|				ТОГДА ДоступныеВидыЗапасов.СтавкаНДСКомитента
	|			ИНАЧЕ НЕОПРЕДЕЛЕНО
	|		КОНЕЦ КАК СтавкаНДСКомитента
	|	ИЗ
	...
	|			ПО Остатки.ВидЗапасов = ДоступныеВидыЗапасов.ВидЗапасов
	|				И &ПодбиратьКомиссионныеЗапасыПоНалогообложениюКомитента
	|	
	|	ГДЕ
	...
	|	ВтОстаткиБезГруппировки.ОтборПоСтавкеНДСКомитента,
	|	ВтОстаткиБезГруппировки.СтавкаНДСКомитента
	|;
	|
	|/////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ПО ИСТИНА
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтавкиНДС КАК ТаблицаСтавкиНДС
	|		ПО ТаблицаДокумента.БазоваяСтавкаНДС = ТаблицаСтавкиНДС.Ссылка
	...
	|			И (НЕ Остатки.ОтборПоСтавкеНДСКомитента
	|				ИЛИ ((Остатки.СтавкаНДСКомитента = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	|						И НЕ ТаблицаСтавкиНДС.ПеречислениеСтавкаНДС В (&ИсключаемыеСтавкиНДС))
	|					ИЛИ ТаблицаДокумента.БазоваяСтавкаНДС = Остатки.СтавкаНДСКомитента))
	|		
	...
	ТекстЗапросаЗаменыПоляСтавкаНДС(МенеджерВременныхТаблиц, Запрос.Текст, ПараметрыЗаполнения);
	...
	Запрос.УстановитьПараметр("ПодбиратьКомиссионныеЗапасыПоНалогообложениюКомитента",
								ПараметрыЗаполнения.ПодбиратьКомиссионныеЗапасыПоНалогообложениюКомитента);
	Запрос.УстановитьПараметр("ИсключаемыеСтавкиНДС", УчетНДСЛокализация.ИсключаемыеПеречисленияСтавкиНДС());
...
//	ПараметрыЗаполнения - см. ПараметрыЗаполненияВидовЗапасов.
//
// Возвращаемое значение:
...
Функция ЗапросРаспределенияСУчетомПриоритетовТаблицыТоваровНаТаблицуОстатков(МенеджерВременныхТаблиц,
	...
	Если ПараметрыЗаполнения.ПодбиратьЗапасыРазныхПериодов Тогда
		ПараметрыТекстаЗапроса = УчетПодакцизныхТоваров.ПараметрыТекстаЗапросаВТПодакцизныеКатегорииТоваров();
		ПараметрыТекстаЗапроса.ОтборКатегорийПоДате = Ложь;
		
		ТекстЗапросаВтПодакцизныеТовары = УчетПодакцизныхТоваров.ТекстЗапросаВТПодакцизныеКатегорииТоваров(
											МенеджерВременныхТаблиц,
	...
	ТекстЗапроса = ТекстЗапросаВтПодакцизныеТовары + ОбщегоНазначения.РазделительПакетаЗапросов() + "
	|ВЫБРАТЬ
	|	МАКСИМУМ(ТаблицаТоваров.НомерСтроки)						КАК НомерСтроки,
	|	ТаблицаДанныхДокумента.Дата									КАК ДатаДокумента,
	...
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.ГруппыФинансовогоУчетаНоменклатуры.ПустаяСсылка)
	|		ИНАЧЕ ТаблицаНоменклатуры.ГруппаФинансовогоУчета
	|	КОНЕЦ														КАК ГруппаФинансовогоУчета,
	|	ТаблицаТоваров.Номенклатура									КАК Номенклатура,
	|	ТаблицаНоменклатуры.ТипНоменклатуры							КАК ТипНоменклатуры,
	...
	|			И СтавкиНДС.РасчетнаяСтавка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПодакцизныеКатегорииТоваров КАК ВтПодакцизныеТовары
	|		ПО ТаблицаТоваров.Номенклатура = ВтПодакцизныеТовары.Номенклатура
	|			И &УсловиеОтбораПодакцизныхТоваров
	|
	|ГДЕ
	|	ТаблицаНоменклатуры.ТипНоменклатуры В(ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),
	|											ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
	...
	|	ОстаткиСгруппированные.ОтборПоСтавкеНДСКомитента			КАК ОтборПоСтавкеНДСКомитента,
	|	ОстаткиСгруппированные.СтавкаНДСКомитента					КАК СтавкаНДСКомитента
	|ПОМЕСТИТЬ ВтОстатки
	|ИЗ
	|	(
	...
	|	ОстаткиСгруппированные.ОтборПоСтавкеНДСКомитента,
	|	ОстаткиСгруппированные.СтавкаНДСКомитента
	|
	|ИМЕЮЩИЕ
...
#Область ЗаменаИспользованияНеобязательныхПолей	
	...
	ТекстЗапросаЗаменыПоляСтавкаНДС(МенеджерВременныхТаблиц, ТекстЗапроса, ПараметрыЗаполнения);
	...
	ТекстУсловиеОтбораПодакцизныхТоваров = ?(ПараметрыЗаполнения.ПодбиратьЗапасыРазныхПериодов,
											"ТаблицаТоваров.Период = ВтПодакцизныеТовары.Период",
											"ИСТИНА");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеОтбораПодакцизныхТоваров", ТекстУсловиеОтбораПодакцизныхТоваров);
...
Процедура СформироватьРезервыПоОрганизациямДляКонтроля(ДокументОбъект,
	...
	Для Каждого Организация Из МассивОрганизаций Цикл
		...
		Если ТаблицаОшибок.Количество() <> 0 Тогда
			...
			Если Не ПараметрыСеанса.ПроводитьБезКонтроляОстатковТоваровОрганизаций Тогда
				
				ОтборРаботы = Новый Структура("ТипНоменклатуры", Перечисления.ТипыНоменклатуры.Работа);
				ОшибкиТолькоПоРаботам = ТаблицаОшибок.НайтиСтроки(ОтборРаботы).Количество() = ТаблицаОшибок.Количество();
				
				Если Константы.КонтролироватьОстаткиТоваровОрганизаций.Получить()
					И Не ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию(ПараметрыЗаполненияВидовЗапасов)
					// Если у пользователя есть соответствующая роль, то потребление работ не контролируется
					И (Не РазрешитьОформлениеВыпускаЗаднимЧислом
						Или Не ОшибкиТолькоПоРаботам) Тогда
					Отказ = Истина;
				КонецЕсли;
...
Функция КлючКэшаОтбораВидовЗапасов(ВладелецТовараВШапке, ОтборВидовЗапасов)
	
	ЧастиКлючаКэша = Новый Массив;
	ПустоеЗначение = "ПустоеЗначение";
	
	КлючКэша = ?(ЗначениеЗаполнено(ОтборВидовЗапасов.ТипЗапасов),
				Строка(ОтборВидовЗапасов.ТипЗапасов),
				ПустоеЗначение);
	ЧастиКлючаКэша.Добавить(КлючКэша);
	
	КлючКэша = ?(ЗначениеЗаполнено(ОтборВидовЗапасов.ВидЦены),
				Строка(ОтборВидовЗапасов.ВидЦены.УникальныйИдентификатор()),
				ПустоеЗначение);
	ЧастиКлючаКэша.Добавить(КлючКэша);
	
	КлючКэша = ?(ЗначениеЗаполнено(ОтборВидовЗапасов.ГруппаФинансовогоУчета),
				Строка(ОтборВидовЗапасов.ГруппаФинансовогоУчета.УникальныйИдентификатор()),
				ПустоеЗначение);
	ЧастиКлючаКэша.Добавить(КлючКэша);
	
	Если Не ВладелецТовараВШапке Тогда
		КлючКэша = ?(ЗначениеЗаполнено(ОтборВидовЗапасов.ВладелецТовара),
					Строка(ОтборВидовЗапасов.ВладелецТовара.УникальныйИдентификатор()),
					ПустоеЗначение);
		ЧастиКлючаКэша.Добавить(КлючКэша);
		
		КлючКэша = ?(ЗначениеЗаполнено(ОтборВидовЗапасов.Контрагент),
					Строка(ОтборВидовЗапасов.Контрагент.УникальныйИдентификатор()),
					ПустоеЗначение);
		ЧастиКлючаКэша.Добавить(КлючКэша);
		
		КлючКэша = ?(ЗначениеЗаполнено(ОтборВидовЗапасов.Договор),
					Строка(ОтборВидовЗапасов.Договор.УникальныйИдентификатор()),
					ПустоеЗначение);
		ЧастиКлючаКэша.Добавить(КлючКэша);
	КонецЕсли;
	
	КлючКэша = СтрСоединить(ЧастиКлючаКэша);
	
	Возврат КлючКэша;
	
КонецФункции

Процедура ТекстЗапросаЗаменыПоляСтавкаНДС(МенеджерВременныхТаблиц, ТекстЗапроса, ПараметрыЗаполнения)
	
	ПодбиратьКомиссионныеЗапасы = ПараметрыЗаполнения.ПодбиратьКомиссионныеЗапасыПоНалогообложениюКомитента;
	
	ОбщегоНазначенияУТ.ЗаменитьОтсутствующиеПоляВтВТекстеЗапроса(
		МенеджерВременныхТаблиц,
		ТекстЗапроса,
		"&ТекстПоляВыборкиТаблицаТоваровСтавкаНДС_",
		"ТаблицаТоваров",
		"СтавкаНДС",
		?(ПодбиратьКомиссионныеЗапасы, "ТаблицаТоваров.СтавкаНДС", "МАКСИМУМ(ТаблицаТоваров.СтавкаНДС)"),
		"ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)");
	
	ОбщегоНазначенияУТ.ЗаменитьОтсутствующиеПоляВтВТекстеЗапроса(
		МенеджерВременныхТаблиц,
		ТекстЗапроса,
		"&ТекстПоляВыборкиСтавкиНДСБазоваяСтавкаНДС_",
		"ТаблицаТоваров",
		"СтавкаНДС",
		?(ПодбиратьКомиссионныеЗапасы,
			"ЕСТЬNULL(СтавкиНДС.СоответствующаяБазоваяСтавка, ТаблицаТоваров.СтавкаНДС)",
			"МАКСИМУМ(ЕСТЬNULL(СтавкиНДС.СоответствующаяБазоваяСтавка, ТаблицаТоваров.СтавкаНДС))"
			),
		"ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)");
	...
	ОбщегоНазначенияУТ.ЗаменитьОтсутствующиеПоляВтВТекстеЗапроса(
		...
		"ТаблицаТоваров",
		"СтавкаНДС",
		?(ПодбиратьКомиссионныеЗапасы, "ТаблицаТоваров.СтавкаНДС", "НЕОПРЕДЕЛЕНО"),
		"НЕОПРЕДЕЛЕНО");
...
Функция ТаблицаОшибокЗаполненияВидовЗапасов()
	...
	ТаблицаОшибок.Колонки.Добавить("ПрослеживаемыйТовар");
...
Процедура СообщитьОбОшибкахКонтроляТоваровОрганизаций(ДокументОбъект, ТаблицаОшибок, ПараметрыЗаполненияВидовЗапасов, Организация = Неопределено)
	...
	Если ПараметрыЗаполненияВидовЗапасов.СообщатьОбОшибкахЗаполнения
		...
		ТаблицаОшибок.Свернуть("Номенклатура, Характеристика, Серия, Склад, Назначение, ЕдиницаИзмерения, НеУказанНомерГТД, ПрослеживаемыйТовар, ТипНоменклатуры",
								"Количество,Сумма");
		...
		Для Каждого СтрокаТаблицы Из ТаблицаОшибок Цикл
			...
			ИначеЕсли Не КонтролироватьНалогообложениеНДС
				И Не СтрокаТаблицы.ПрослеживаемыйТовар Тогда
				ТекстСообщения = НСтр("ru = 'По организации %Организация%%ПробелСклад%%МестоХраненияНаименование%%ПробелСклад%%Склад% не хватает %Количество% %ЕдиницаИзмерения% %ПредставлениеТипаНоменклатуры% %ПредставлениеНоменклатуры% с указанными номерами ГТД'");
			ИначеЕсли Не КонтролироватьНалогообложениеНДС Тогда
				ТекстСообщения = НСтр("ru = 'По организации %Организация%%ПробелСклад%%МестоХраненияНаименование%%ПробелСклад%%Склад% не хватает %Количество% %ЕдиницаИзмерения% %ПредставлениеТипаНоменклатуры% %ПредставлениеНоменклатуры% с указанными РНПТ'");
			ИначеЕсли СтрокаТаблицы.ПрослеживаемыйТовар Тогда
				ТекстСообщения = НСтр("ru = 'По организации %Организация%%ПробелСклад%%МестоХраненияНаименование%%ПробелСклад%%Склад% не хватает %Количество% %ЕдиницаИзмерения% %ПредставлениеТипаНоменклатуры% %ПредставлениеНоменклатуры% с указанными РНПТ по налогообложению %НалогообложениеНДС%'");
			Иначе
				ТекстСообщения = НСтр("ru = 'По организации %Организация%%ПробелСклад%%МестоХраненияНаименование%%ПробелСклад%%Склад% не хватает %Количество% %ЕдиницаИзмерения% %ПредставлениеТипаНоменклатуры% %ПредставлениеНоменклатуры% с указанными номерами ГТД по налогообложению %НалогообложениеНДС%'");
			КонецЕсли;
...
Процедура СообщитьОбОшибкахЗаполненияВидовЗапасовКомитентов(ДокументОбъект, ТаблицаОшибок, ПараметрыЗаполненияВидовЗапасов)
	...
	Если ТаблицаОшибок.Количество() > 0
		...
		ТаблицаОшибок.Свернуть("Номенклатура, Характеристика, Серия, Склад, Назначение, ЕдиницаИзмерения, НеУказанНомерГТД, ПрослеживаемыйТовар",
								"Количество, Сумма");
...
Процедура СообщитьОбОшибкахЗаполненияВидовЗапасовПоТаблицеКОформлению(
	...
	Если ПараметрыЗаполненияВидовЗапасов.СообщатьОбОшибкахЗаполнения
		...
		ТаблицаОшибок.Свернуть(
			"Номенклатура,Характеристика,Серия,Склад,СкладОтгрузки,Назначение,ЕдиницаИзмерения,НеУказанНомерГТД,ПрослеживаемыйТовар,Валюта,ТипНоменклатуры",
			"Количество,Сумма");
		...
		Для Каждого СтрокаТаблицы Из ТаблицаОшибок Цикл
			...
			Если СтрокаТаблицы.НеУказанНомерГТД
				И СтрокаТаблицы.ПрослеживаемыйТовар Тогда
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КонтрольГТД%", НСтр("ru = 'с указанными РНПТ'"));
			ИначеЕсли СтрокаТаблицы.НеУказанНомерГТД Тогда
...
Процедура ТаблицаОстатковКОформлениюОтчетаКомитенту(ДокументОбъект, МенеджерВременныхТаблиц, ДатаОстатков)
	...
	|	ГДЕ
	|		ТоварыКОформлению.Активность
	|		И ТоварыКОформлению.Период <= &Дата
	...
	|		ТоварыКОформлению.ДокументПродажи				КАК ДокументРеализации
	|	ИЗ
	|		РегистрНакопления.ТоварыКОформлениюОтчетовКомитенту.Остатки(&ДатаОстатков,
	|				Валюта = &Валюта
	...
	Запрос.УстановитьПараметр("ДатаОстатков",	Новый Граница(КонецПериода, ВидГраницы.Включая));
...
Процедура ТаблицаОстатковПринятыхТоваров(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	|	СУММА(ТаблицаТоваров.Количество)		КАК Количество
	|ПОМЕСТИТЬ ВтТаблицаТоваровВозврат
	|ИЗ
	|	ТаблицаТоваров КАК ТаблицаТоваров
	|
	|СГРУППИРОВАТЬ ПО
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТаблицаТоваров.ДокументПоступления	КАК ДокументПоступления,
	|	АналитикиУчета.КлючАналитики		КАК АналитикаУчетаНоменклатуры,
	|	АналитикиУчета.Номенклатура			КАК Номенклатура,
	|	АналитикиУчета.Характеристика		КАК Характеристика,
	|	АналитикиУчета.Назначение			КАК Назначение,
	|	АналитикиУчета.Серия				КАК Серия
	|ПОМЕСТИТЬ ТаблицаТоваровСКлючамиПоВсемСкладам
	|ИЗ
	|	ТаблицаТоваров КАК ТаблицаТоваров
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК АналитикиУчета
	|		ПО ТаблицаТоваров.Номенклатура = АналитикиУчета.Номенклатура
	|			И ТаблицаТоваров.Характеристика = АналитикиУчета.Характеристика
	|			И (ТаблицаТоваров.Серия = АналитикиУчета.Серия
	|				ИЛИ АналитикиУчета.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка))
	|			И ТаблицаТоваров.Назначение = АналитикиУчета.Назначение
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументПоступления,
	|	АналитикаУчетаНоменклатуры,
	...
	|		ПО ТаблицаТоваров.АналитикаУчетаНоменклатуры = КлючиАналитики.Ссылка
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаТоваровСКлючамиПоВсемСкладам КАК ТаблицаТоваровВозврата
	|		ПО ТаблицаТоваров.Ссылка = ТаблицаТоваровВозврата.ДокументПоступления
	|			И КлючиАналитики.Номенклатура = ТаблицаТоваровВозврата.Номенклатура
	|			И КлючиАналитики.Характеристика = ТаблицаТоваровВозврата.Характеристика
	|			И КлючиАналитики.Назначение = ТаблицаТоваровВозврата.Назначение
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументов
	|		ПО ТаблицаТоваров.Ссылка = РеестрДокументов.СторнируемыйДокумент
	|			И РеестрДокументов.Проведен
	|			И НЕ РеестрДокументов.ДополнительнаяЗапись
	|
	|ГДЕ
	...
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаТоваровСКлючамиПоВсемСкладам КАК ТаблицаТоваровВозврата
	|		ПО ДанныеДокумента.ДокументОснование = ТаблицаТоваровВозврата.ДокументПоступления
	|			И ТаблицаТоваров.Номенклатура = ТаблицаТоваровВозврата.Номенклатура
	|			И ТаблицаТоваров.Характеристика = ТаблицаТоваровВозврата.Характеристика
	|			И ТаблицаТоваров.Назначение = ТаблицаТоваровВозврата.Назначение
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументов
	|		ПО ТаблицаТоваров.Ссылка = РеестрДокументов.СторнируемыйДокумент
	|			И РеестрДокументов.Проведен
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДанныеДокумента.ПервичныйДокумент			КАК Ссылка,
	|	ТаблицаТоваров.ВидЗапасов					КАК ВидЗапасов,
	|	ТаблицаТоваров.АналитикаУчетаНоменклатуры	КАК АналитикаУчетаНоменклатуры,
	|	ТаблицаТоваров.НомерГТД						КАК НомерГТД,
	|	ТаблицаТоваров.Количество					КАК Количество,
	|	ВЫБОР
	|		КОГДА НЕ &ИспользоватьУчетПрослеживаемыхИмпортныхТоваров
	|				ИЛИ НАЧАЛОПЕРИОДА(ДанныеДокумента.Дата, МЕСЯЦ) < &ДатаНачалаУчетаПрослеживаемыхИмпортныхТоваров
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(ТаблицаТоваров.КоличествоПоРНПТ КАК ЧИСЛО(23, 11))
	|	КОНЕЦ										КАК КоличествоПоРНПТ
	|ПОМЕСТИТЬ ВТТоварыУведомленияОВвозеПрослеживаемыхТоваров
	|ИЗ
	|	Документ.УведомлениеОВвозеПрослеживаемыхТоваров.Товары КАК ТаблицаТоваров
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.УведомлениеОВвозеПрослеживаемыхТоваров КАК ДанныеДокумента
	|		ПО ТаблицаТоваров.Ссылка = ДанныеДокумента.Ссылка
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаТоваровСКлючамиПоВсемСкладам КАК ТаблицаТоваровВозврата
	|		ПО ДанныеДокумента.ПервичныйДокумент = ТаблицаТоваровВозврата.ДокументПоступления
	|			И ТаблицаТоваров.АналитикаУчетаНоменклатуры = ТаблицаТоваровВозврата.АналитикаУчетаНоменклатуры
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПриобретениеТоваровУслуг КАК ДанныеДокументаПоступления
	|		ПО ДанныеДокумента.ПервичныйДокумент = ДанныеДокументаПоступления.Ссылка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрПоступлений
	|		ПО ДанныеДокументаПоступления.Ссылка = РеестрПоступлений.СторнируемыйДокумент
	|			И РеестрПоступлений.Проведен
	|			И НЕ РеестрПоступлений.ДополнительнаяЗапись
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрУведомлений
	|		ПО ТаблицаТоваров.Ссылка = РеестрУведомлений.СторнируемыйДокумент
	|			И РеестрУведомлений.Проведен
	|			И НЕ РеестрУведомлений.ДополнительнаяЗапись
	|
	|ГДЕ
	|	ДанныеДокумента.Проведен
	|	И ДанныеДокумента.РНПТ <> """"
	|	И ТаблицаТоваров.НомерГТД <> ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
	|	И РеестрПоступлений.Ссылка ЕСТЬ NULL
	|	И РеестрУведомлений.Ссылка ЕСТЬ NULL
	|	И ДанныеДокументаПоступления.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС)
	//-- Локализация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	ВЫБОР
	|		КОГДА НЕ &ИспользоватьУчетПрослеживаемыхИмпортныхТоваров
	|				ИЛИ НАЧАЛОПЕРИОДА(ДанныеШапки.Дата, МЕСЯЦ) < &ДатаНачалаУчетаПрослеживаемыхИмпортныхТоваров
	|			ТОГДА 0
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПриобретениеТоваровУслуг КАК ДанныеШапки
	|		ПО ДанныеДокумента.Ссылка = ДанныеШапки.Ссылка
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаТоваровСКлючамиПоВсемСкладам КАК ТаблицаТоваров
	...
	|	И РеестрДокументов.Ссылка ЕСТЬ NULL
	|		И (ДанныеШапки.ХозяйственнаяОперация НЕ В (
	...
	|	ВЫБОР
	//++ Локализация
	|		КОГДА НЕ УведомлениеОВвозеПрослеживаемыхТоваров.НомерГТД ЕСТЬ NULL
	|			ТОГДА УведомлениеОВвозеПрослеживаемыхТоваров.НомерГТД
	//-- Локализация
	|		КОГДА НЕ ДанныеДокумента.НомерГТД ЕСТЬ NULL
	|			ТОГДА ДанныеДокумента.НомерГТД
	...
	|			И ДанныеДокумента.АналитикаУчетаНоменклатуры.Номенклатура = ТаблицаТоваровВозврата.Номенклатура
	|			И ДанныеДокумента.АналитикаУчетаНоменклатуры.Характеристика = ТаблицаТоваровВозврата.Характеристика
	|			И (ДанныеДокумента.АналитикаУчетаНоменклатуры.Серия = ТаблицаТоваровВозврата.Серия
	|				ИЛИ ДанныеДокумента.АналитикаУчетаНоменклатуры.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка))
	|			И ДанныеДокумента.АналитикаУчетаНоменклатуры.Назначение = ТаблицаТоваровВозврата.Назначение
	|		
	...
	|			И ДанныеДокумента.АналитикаУчетаНоменклатуры.Номенклатура = ТаблицаТоваровПриобретения.Номенклатура
	|			И ДанныеДокумента.АналитикаУчетаНоменклатуры.Характеристика = ТаблицаТоваровПриобретения.Характеристика
	|			И (ДанныеДокумента.АналитикаУчетаНоменклатуры.Серия = ТаблицаТоваровПриобретения.Серия
	|				ИЛИ ДанныеДокумента.АналитикаУчетаНоменклатуры.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|				ИЛИ ТаблицаТоваровПриобретения.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка))
	|			И ДанныеДокумента.АналитикаУчетаНоменклатуры.Назначение = ТаблицаТоваровПриобретения.Назначение
	|			И ДанныеДокумента.АналитикаУчетаНоменклатуры.МестоХранения = ТаблицаТоваровПриобретения.МестоХранения
	...
	//++ Локализация
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.УведомлениеОВвозеПрослеживаемыхТоваров.Товары КАК УведомлениеОВвозеПрослеживаемыхТоваров
	|			ПО УведомлениеОВвозеПрослеживаемыхТоваров.Ссылка.ПервичныйДокумент = ДанныеДокумента.Ссылка
	|			И УведомлениеОВвозеПрослеживаемыхТоваров.АналитикаУчетаНоменклатуры = ТаблицаТоваровВозврата.АналитикаУчетаНоменклатуры
	|			И УведомлениеОВвозеПрослеживаемыхТоваров.Ссылка.Проведен
	//-- Локализация
	|
	|ГДЕ
	|	&ЭтоВозвратПоставщику
	|	И ДанныеШапки.Проведен
	|	И ДанныеШапки.ХозяйственнаяОперация В (
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСНеотфактурованнаяПоставка),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаНеотфактурованнаяПоставка),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки),
	...
	|	ЕСТЬNULL(ПоступлениеТоваровНаСклад.ВидЗапасовНаСкладе, ДанныеДокумента.ВидЗапасов) КАК ВидЗапасов,
	|	ЕСТЬNULL(ПоступлениеТоваровНаСклад.АналитикаУчетаНоменклатуры, ДанныеДокумента.АналитикаУчетаНоменклатуры) КАК АналитикаУчетаНоменклатуры,
	|	ДанныеДокумента.НомерГТД			КАК НомерГТД,
	|	ДанныеДокумента.Количество			КАК Количество,
	|	ВЫБОР
	|		КОГДА НЕ &ИспользоватьУчетПрослеживаемыхИмпортныхТоваров
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ТаможеннаяДекларацияИмпорт КАК ДанныеШапки
	|		ПО ДанныеДокумента.Ссылка = ДанныеШапки.Ссылка
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаТоваровСКлючамиПоВсемСкладам КАК ТаблицаТоваров
	...
	|			И ДанныеДокумента.АналитикаУчетаНоменклатуры = ТаблицаТоваров.АналитикаУчетаНоменклатуры
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровНаСклад.ВидыЗапасов КАК ПоступлениеТоваровНаСклад
	|			ПО ПоступлениеТоваровНаСклад.Ссылка.Распоряжение = ДанныеДокумента.ДокументПоступления
	|			И ПоступлениеТоваровНаСклад.АналитикаУчетаНоменклатурыТоварыУПартнеров = ДанныеДокумента.АналитикаУчетаНоменклатуры
	|			И ПоступлениеТоваровНаСклад.Ссылка.Проведен
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрПоступлений
	|		ПО ПоступлениеТоваровНаСклад.Ссылка = РеестрПоступлений.СторнируемыйДокумент
	|			И РеестрПоступлений.Проведен
	|			И НЕ РеестрПоступлений.ДополнительнаяЗапись
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДеклараций
	|		ПО ДанныеДокумента.Ссылка = РеестрДеклараций.СторнируемыйДокумент
	|			И РеестрДеклараций.Проведен
	|			И НЕ РеестрДеклараций.ДополнительнаяЗапись
	|
	|ГДЕ
	|	ДанныеШапки.Проведен
	|	И РеестрДеклараций.Ссылка ЕСТЬ NULL
	|	И РеестрПоступлений.Ссылка ЕСТЬ NULL
	//++ Локализация
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ДанныеДокумента.НомерГТД					КАК НомерГТД,
	|	ДанныеДокумента.Количество					КАК Количество,
	|	ДанныеДокумента.КоличествоПоРНПТ			КАК КоличествоПоРНПТ
	|ИЗ
	|	ВТТоварыУведомленияОВвозеПрослеживаемыхТоваров КАК ДанныеДокумента
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|
	|ВЫБРАТЬ
	|	ДанныеШапки.ДокументОснование				КАК Ссылка,
	|	ДанныеДокумента.ВидЗапасов					КАК ВидЗапасов,
	|	ДанныеДокумента.АналитикаУчетаНоменклатуры	КАК АналитикаУчетаНоменклатуры,
	|	ДанныеДокумента.НомерГТД					КАК НомерГТД,
	|	ДанныеДокумента.Количество					КАК Количество,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КорректировкаПриобретения КАК ДанныеШапки
	|		ПО ДанныеДокумента.Ссылка = ДанныеШапки.Ссылка
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаТоваровСКлючамиПоВсемСкладам КАК ТаблицаТоваров
	|		ПО ДанныеШапки.ДокументОснование = ТаблицаТоваров.ДокументПоступления
	|			И ДанныеДокумента.АналитикаУчетаНоменклатуры = ТаблицаТоваров.АналитикаУчетаНоменклатуры
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументов
	|		ПО ДанныеДокумента.Ссылка = РеестрДокументов.СторнируемыйДокумент
	...
	|	И (ДанныеШапки.ДокументОснование.ХозяйственнаяОперация НЕ В (
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСНеотфактурованнаяПоставка),
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки),
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаНеотфактурованнаяПоставка),
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки),
	...
	|	&ЭтоВозвратПоставщику
	|	И ДанныеШапки.Проведен
	|	И ДанныеШапки.ДокументОснование.ХозяйственнаяОперация В (
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСНеотфактурованнаяПоставка),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаНеотфактурованнаяПоставка),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки),
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровПоставщику КАК ДанныеШапки
	|		ПО ДанныеДокумента.Ссылка = ДанныеШапки.Ссылка
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаТоваровСКлючамиПоВсемСкладам КАК ТаблицаТоваров
	|		ПО ДанныеДокумента.ДокументПоступления = ТаблицаТоваров.ДокументПоступления
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТТоварыУведомленияОВвозеПрослеживаемыхТоваров
	//-- Локализация
	|";
	
	ЭтоВозвратПоставщику = ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ВозвратТоваровПоставщику");
	Запрос.УстановитьПараметр("Ссылка", ДокументОбъект.Ссылка);
	Запрос.УстановитьПараметр("ЭтоВозвратПоставщику", ЭтоВозвратПоставщику);
	...
	Если ЭтоВозвратПоставщику Тогда
		...
		"ВЫБРАТЬ
		|	ДанныеДокумента.Организация КАК Организация,
		|	ПринятыеТовары.Ссылка КАК ДокументПоступления,
		...
		|	СУММА(ПринятыеТовары.Количество) КАК КоличествоОстаток,
		|	СУММА(ПринятыеТовары.КоличествоПоРНПТ) КАК КоличествоПоРНПТОстаток
		|	ПОМЕСТИТЬ ВтПринятыеТовары
		|ИЗ
		...
		|	ПринятыеТовары.АналитикаУчетаНоменклатуры.Номенклатура,
		|	ПринятыеТовары.АналитикаУчетаНоменклатуры.Характеристика,
		|	ПринятыеТовары.АналитикаУчетаНоменклатуры.Серия,
		|	ПринятыеТовары.АналитикаУчетаНоменклатуры.Назначение,
		|	ПринятыеТовары.ВидЗапасов,
		|	ПринятыеТовары.Ссылка
		|
		|ИМЕЮЩИЕ
		|	СУММА(ПринятыеТовары.Количество) <> 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ПринятыеТовары.ВидЗапасов,
		|	ВЫРАЗИТЬ(ПринятыеТовары.АналитикаУчетаНоменклатуры КАК Справочник.КлючиАналитикиУчетаНоменклатуры).Номенклатура,
		|	ВЫРАЗИТЬ(ПринятыеТовары.АналитикаУчетаНоменклатуры КАК Справочник.КлючиАналитикиУчетаНоменклатуры).Характеристика,
		|	ВЫРАЗИТЬ(ПринятыеТовары.АналитикаУчетаНоменклатуры КАК Справочник.КлючиАналитикиУчетаНоменклатуры).Серия,
		|	ВЫРАЗИТЬ(ПринятыеТовары.АналитикаУчетаНоменклатуры КАК Справочник.КлючиАналитикиУчетаНоменклатуры).Назначение,
		|	ПринятыеТовары.НомерГТД,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ПринятыеТовары.Организация КАК Организация,
		|	ПринятыеТовары.ДокументПоступления КАК ДокументПоступления,
		|	ПринятыеТовары.ВидЗапасов КАК ВидЗапасов,
		|	ПринятыеТовары.Номенклатура КАК Номенклатура,
		|	ПринятыеТовары.Характеристика КАК Характеристика,
		|	ПринятыеТовары.Серия КАК Серия,
		|	ПринятыеТовары.Назначение КАК Назначение,
		|	ПринятыеТовары.НомерГТД КАК НомерГТД,
		|	ВЫБОР
		|		КОГДА ПринятыеТовары.КоличествоОстаток > ТаблицаТоваровВозврат.Количество
		|			ТОГДА ТаблицаТоваровВозврат.Количество
		|		ИНАЧЕ ПринятыеТовары.КоличествоОстаток
		|	КОНЕЦ КАК КоличествоОстаток,
		|	ВЫБОР
		|		КОГДА ПринятыеТовары.КоличествоОстаток > ТаблицаТоваровВозврат.Количество
		|			ТОГДА (ПринятыеТовары.КоличествоПоРНПТОстаток / ПринятыеТовары.КоличествоОстаток) * ТаблицаТоваровВозврат.Количество
		|		ИНАЧЕ ПринятыеТовары.КоличествоПоРНПТОстаток
		|	КОНЕЦ КАК КоличествоПоРНПТОстаток
		|		
		|ИЗ
		|	ВТПринятыеТовары КАК ПринятыеТовары
		|
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтТаблицаТоваровВозврат КАК ТаблицаТоваровВозврат
		|		ПО ТаблицаТоваровВозврат.Номенклатура = ПринятыеТовары.Номенклатура
		|			И ТаблицаТоваровВозврат.Характеристика = ПринятыеТовары.Характеристика
		|			И (ТаблицаТоваровВозврат.Серия = ПринятыеТовары.Серия
		|				ИЛИ ТаблицаТоваровВозврат.СтатусУказанияСерий <> 14)
		|			И ТаблицаТоваровВозврат.Назначение = ПринятыеТовары.Назначение
		|			И ТаблицаТоваровВозврат.ДокументПоступления = ПринятыеТовары.ДокументПоступления
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДанныеДокумента.Организация КАК Организация,
		|	ТаблицаТоваров.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры
		|ПОМЕСТИТЬ ВтТаблицаТоваровПоАналитики
		|ИЗ
		|	ТаблицаТоваров КАК ТаблицаТоваров
		|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаДанныхДокумента КАК ДанныеДокумента
		|		ПО (ИСТИНА)
		...
		|ИЗ
		|	ТаблицаОстатков КАК ТаблицаОстатков
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтТаблицаТоваровПоАналитики КАК ТаблицаТоваров
		|		ПО (ТаблицаТоваров.Организация = ТаблицаОстатков.Организация)
		|			И (ТаблицаТоваров.АналитикаУчетаНоменклатуры = ТаблицаОстатков.АналитикаУчетаНоменклатуры)";
		...
		РезультатЗапроса = Запрос.ВыполнитьПакет();
		ДанныеПоПринятымОстаткам = РезультатЗапроса[РезультатЗапроса.ВГраница()-2].Выгрузить();
		ДанныеПоОстаткамОрганизации = РезультатЗапроса[РезультатЗапроса.ВГраница()].Выгрузить();
		
		ТаблицаОстаток = Новый ТаблицаЗначений();
		ТаблицаОстаток.Колонки.Добавить("Организация", Новый ОписаниеТипов("СправочникСсылка.Организации"));
		ТаблицаОстаток.Колонки.Добавить("ДокументПоступления", Новый ОписаниеТипов("ДокументСсылка.ПриобретениеТоваровУслуг"));
		ТаблицаОстаток.Колонки.Добавить("ВидЗапасов", Новый ОписаниеТипов("СправочникСсылка.ВидыЗапасов"));
		ТаблицаОстаток.Колонки.Добавить("АналитикаУчетаНоменклатуры", Новый ОписаниеТипов("СправочникСсылка.КлючиАналитикиУчетаНоменклатуры"));
		ТаблицаОстаток.Колонки.Добавить("НомерГТД", Новый ОписаниеТипов("СправочникСсылка.НомераГТД"));
		ТаблицаОстаток.Колонки.Добавить("КоличествоОстаток", Новый ОписаниеТипов("Число"));
		ТаблицаОстаток.Колонки.Добавить("КоличествоПоРНПТОстаток", Новый ОписаниеТипов("Число"));
		ТаблицаОстаток.Колонки.Добавить("ДатаПоступления", Новый ОписаниеТипов("Дата"));
	
		ШаблонПоиска = Новый Структура;
		ШаблонПоиска.Вставить("Номенклатура");
		ШаблонПоиска.Вставить("Характеристика");
		ШаблонПоиска.Вставить("Серия");
		ШаблонПоиска.Вставить("Назначение");
		ШаблонПоиска.Вставить("Организация");
		ШаблонПоиска.Вставить("НомерГТД");
		
		ШаблонПоискаПоОстаткам = Новый Структура;
		ШаблонПоискаПоОстаткам.Вставить("Организация");
		ШаблонПоискаПоОстаткам.Вставить("ДокументПоступления");
		ШаблонПоискаПоОстаткам.Вставить("ВидЗапасов");
		ШаблонПоискаПоОстаткам.Вставить("АналитикаУчетаНоменклатуры");
		ШаблонПоискаПоОстаткам.Вставить("НомерГТД");
		ШаблонПоискаПоОстаткам.Вставить("ДатаПоступления");
		
		Для Каждого СтрокаОстатка Из ДанныеПоОстаткамОрганизации Цикл
			
			ЗаполнитьЗначенияСвойств(ШаблонПоиска, СтрокаОстатка);
			СтрокиПоПриемке = ДанныеПоПринятымОстаткам.НайтиСтроки(ШаблонПоиска);
			
			Для Каждого СтрокаПоПриемке Из СтрокиПоПриемке Цикл
				Если СтрокаПоПриемке.КоличествоОстаток <> 0 И СтрокаОстатка.КоличествоОстаток <> 0 Тогда
					
					ЗаполнитьЗначенияСвойств(ШаблонПоискаПоОстаткам, СтрокаОстатка);
					ЗаполнитьЗначенияСвойств(ШаблонПоискаПоОстаткам, СтрокаПоПриемке);
					СтрокиТекущихОстатков = ТаблицаОстаток.НайтиСтроки(ШаблонПоискаПоОстаткам);
					
					Если СтрокиТекущихОстатков.Количество() = 0 Тогда
						СтрокаТекущегоОстатка = ТаблицаОстаток.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТекущегоОстатка, ШаблонПоискаПоОстаткам);
					Иначе
						СтрокаТекущегоОстатка = СтрокиТекущихОстатков[0];
					КонецЕсли;
					
					КоличествоКРаспределению = Мин(СтрокаПоПриемке.КоличествоОстаток, СтрокаОстатка.КоличествоОстаток);
					КоличествоПоРНПТКРаспределению = (СтрокаПоПриемке.КоличествоПоРНПТОстаток / СтрокаПоПриемке.КоличествоОстаток) * КоличествоКРаспределению;
					
					СтрокаОстатка.КоличествоОстаток = СтрокаОстатка.КоличествоОстаток - КоличествоКРаспределению;
					СтрокаОстатка.КоличествоПоРНПТОстаток = СтрокаОстатка.КоличествоПоРНПТОстаток - КоличествоПоРНПТКРаспределению;
					СтрокаПоПриемке.КоличествоОстаток = СтрокаПоПриемке.КоличествоОстаток - КоличествоКРаспределению;
					СтрокаПоПриемке.КоличествоПоРНПТОстаток = СтрокаПоПриемке.КоличествоПоРНПТОстаток - КоличествоПоРНПТКРаспределению;
					
					СтрокаТекущегоОстатка.КоличествоОстаток = СтрокаТекущегоОстатка.КоличествоОстаток + КоличествоКРаспределению;
					СтрокаТекущегоОстатка.КоличествоПоРНПТОстаток = СтрокаТекущегоОстатка.КоличествоПоРНПТОстаток + КоличествоПоРНПТКРаспределению;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицаОстаток.Организация КАК Организация,
		|	ТаблицаОстаток.ВидЗапасов КАК ВидЗапасов,
		|	ВЫРАЗИТЬ(ТаблицаОстаток.АналитикаУчетаНоменклатуры КАК Справочник.КлючиАналитикиУчетаНоменклатуры) КАК АналитикаУчетаНоменклатуры,
		|	ТаблицаОстаток.НомерГТД КАК НомерГТД,
		|	ТаблицаОстаток.КоличествоОстаток КАК КоличествоОстаток,
		|	ТаблицаОстаток.КоличествоПоРНПТОстаток КАК КоличествоПоРНПТОстаток,
		|	ТаблицаОстаток.ДатаПоступления КАК ДатаПоступления,
		|	ТаблицаОстаток.ДокументПоступления КАК ДокументПередачи
		|ПОМЕСТИТЬ ВвТаблицаОстатков
		|ИЗ
		...
		|	ТаблицаОстаток.Организация КАК Организация,
		|	ТаблицаОстаток.ВидЗапасов КАК ВидЗапасов,
		|	ТаблицаОстаток.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
		|	ЗНАЧЕНИЕ(Справочник.КлючиАналитикиУчетаНоменклатуры.ПустаяСсылка) КАК АналитикаУчетаНоменклатурыОтгрузки,
		|	ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка) КАК ВидЗапасовОтгрузки,
		|	ТаблицаОстаток.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
		|	ТаблицаОстаток.АналитикаУчетаНоменклатуры.Характеристика КАК Характеристика,
		|	ТаблицаОстаток.АналитикаУчетаНоменклатуры.Серия КАК Серия,
		|	ТаблицаОстаток.АналитикаУчетаНоменклатуры.Назначение КАК Назначение,
		|	ТаблицаОстаток.НомерГТД КАК НомерГТД,
		|	ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка) КАК Склад,
		|	ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка) КАК КорВидЗапасов,
		|	ТаблицаОстаток.ДатаПоступления КАК ДатаПоступления,
		|	СУММА(ТаблицаОстаток.КоличествоОстаток) КАК КоличествоОстаток,
		|	СУММА(ТаблицаОстаток.КоличествоПоРНПТОстаток) КАК КоличествоПоРНПТОстаток,
		|	ТаблицаОстаток.ДокументПередачи КАК ДокументПередачи,
		|	СУММА(0) КАК СуммаОстаток
		|ПОМЕСТИТЬ ТаблицаОстатков
		|ИЗ
		|	ВвТаблицаОстатков КАК ТаблицаОстаток
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаОстаток.Организация,
		|	ТаблицаОстаток.НомерГТД,
		|	ТаблицаОстаток.АналитикаУчетаНоменклатуры,
		|	ТаблицаОстаток.ВидЗапасов,
		|	ТаблицаОстаток.ДатаПоступления,
		|	ТаблицаОстаток.АналитикаУчетаНоменклатуры.Номенклатура,
		|	ТаблицаОстаток.АналитикаУчетаНоменклатуры.Характеристика,
		|	ТаблицаОстаток.АналитикаУчетаНоменклатуры.Серия,
		|	ТаблицаОстаток.АналитикаУчетаНоменклатуры.Назначение,
		|	ТаблицаОстаток.ДокументПередачи
		|
		|ИМЕЮЩИЕ
		|	СУММА(ТаблицаОстаток.КоличествоОстаток) <> 0";
		
		Запрос.УстановитьПараметр("ТаблицаОстаток", ТаблицаОстаток);
		
	Иначе // Это не возврат
		...
		|	СУММА(0) КАК СуммаОстаток,
		|	-СУММА(ПринятыеТовары.Количество) КАК КоличествоОстаток,
		|	-СУММА(ВЫРАЗИТЬ(ПринятыеТовары.КоличествоПоРНПТ КАК ЧИСЛО(23, 11))) КАК КоличествоПоРНПТОстаток
		|ИЗ
		|	ТаблицаВидыЗапасов КАК ПринятыеТовары
		|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаДанныхДокумента КАК ДанныеДокумента
		|		ПО ИСТИНА
		...
		|		ПО ПринятыеТовары.ДокументПоступления = СгруппированнаяТаблицаТоваров.ДокументПоступления
		|			И ПринятыеТовары.АналитикаУчетаНоменклатуры = СгруппированнаяТаблицаТоваров.АналитикаУчетаНоменклатуры
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК КлючиАналитики
		|		ПО ПринятыеТовары.АналитикаУчетаНоменклатуры = КлючиАналитики.Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	ДанныеДокумента.Организация,
		|	ПринятыеТовары.НомерГТД,
...
Процедура ТаблицаОстатковПереданныхМеждуОрганизациямиТоваров(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		ДанныеЗапасов.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатурыОтгрузки,
	|		ДанныеЗапасов.ВидЗапасов КАК ВидЗапасовОтгрузки,
	...
	|		ДанныеЗапасов.ВидЗапасовПолучателя КАК ВидЗапасов,
	|		ДанныеЗапасов.НомерГТД КАК НомерГТД,
	|		ДанныеЗапасов.Количество КАК Количество,
	|		ВЫБОР
	|			КОГДА НЕ &ИспользоватьУчетПрослеживаемыхИмпортныхТоваров
	|					ИЛИ НАЧАЛОПЕРИОДА(ДанныеШапки.Дата, МЕСЯЦ) < &ДатаНачалаУчетаПрослеживаемыхИмпортныхТоваров
	|				ТОГДА 0
	|			ИНАЧЕ ВЫРАЗИТЬ(ДанныеЗапасов.КоличествоПоРНПТ КАК ЧИСЛО(23, 11))
	|		КОНЕЦ КАК КоличествоПоРНПТ,
	|		ДанныеЗапасов.Ссылка КАК ДокументПередачи
	|	ИЗ
	|		Документ.ПередачаТоваровМеждуОрганизациями.ВидыЗапасов КАК ДанныеЗапасов
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПередачаТоваровМеждуОрганизациями КАК ДанныеШапки
	|			ПО ДанныеЗапасов.Ссылка = ДанныеШапки.Ссылка
	|			
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаДанныхДокумента КАК ТаблицаДанныхДокумента
	|			ПО ДанныеШапки.ХозяйственнаяОперация = ТаблицаДанныхДокумента.ХозяйственнаяОперацияПередачи
	|				И ДанныеШапки.ОрганизацияПолучатель = ТаблицаДанныхДокумента.Организация
	|				И ДанныеШапки.Организация = ТаблицаДанныхДокумента.ОрганизацияПолучатель
	|				И ДанныеШапки.Проведен
	|				И ДанныеШапки.Дата <= КОНЕЦПЕРИОДА(ТаблицаДанныхДокумента.Дата, МЕСЯЦ)
	|			
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК КлючиАналитикиУчетаНоменклатуры
	|			ПО ДанныеЗапасов.АналитикаУчетаНоменклатуры = КлючиАналитикиУчетаНоменклатуры.Ссылка
	|			
	...
	|	ГДЕ
	|		ДанныеШапки.Ссылка В
	|				(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|					ТаблицаТоваров.ДокументПередачи КАК Ссылка
	|				ИЗ
	|					ТаблицаТоваров КАК ТаблицаТоваров)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ДанныеЗапасов.АналитикаУчетаНоменклатурыОтгрузки,
	|		ДанныеЗапасов.ВидЗапасовПолучателя,
	|		ДанныеЗапасов.АналитикаУчетаНоменклатуры,
	|		ДанныеШапки.Организация,
	|		ДанныеЗапасов.ВидЗапасов,
	|		ДанныеЗапасов.НомерГТД,
	|		-ДанныеЗапасов.Количество,
	|		ВЫБОР
	|			КОГДА НЕ &ИспользоватьУчетПрослеживаемыхИмпортныхТоваров
	|					ИЛИ НАЧАЛОПЕРИОДА(ДанныеШапки.Дата, МЕСЯЦ) < &ДатаНачалаУчетаПрослеживаемыхИмпортныхТоваров
	|				ТОГДА 0
	|			ИНАЧЕ -ВЫРАЗИТЬ(ДанныеЗапасов.КоличествоПоРНПТ КАК ЧИСЛО(23, 11))
	|		КОНЕЦ,
	|		ДанныеЗапасов.ДокументПередачи
	|	ИЗ
	|		Документ.ВозвратТоваровМеждуОрганизациями.ВидыЗапасов КАК ДанныеЗапасов
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровМеждуОрганизациями КАК ДанныеШапки
	|			ПО ДанныеЗапасов.Ссылка = ДанныеШапки.Ссылка
	|			
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаДанныхДокумента КАК ТаблицаДанныхДокумента
	|			ПО ДанныеШапки.ХозяйственнаяОперация = ТаблицаДанныхДокумента.ХозяйственнаяОперацияВозврата
	|				И ДанныеШапки.Организация = ТаблицаДанныхДокумента.Организация
	|				И ДанныеШапки.ОрганизацияПолучатель = ТаблицаДанныхДокумента.ОрганизацияПолучатель
	|				И ДанныеШапки.Проведен
	|				И ДанныеШапки.Дата <= КОНЕЦПЕРИОДА(ТаблицаДанныхДокумента.Дата, МЕСЯЦ)
	|			
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК КлючиАналитикиУчетаНоменклатуры
	|			ПО ДанныеЗапасов.АналитикаУчетаНоменклатуры = КлючиАналитикиУчетаНоменклатуры.Ссылка
	|	ГДЕ
	|		ДанныеЗапасов.Ссылка <> &Ссылка
	|		И ДанныеЗапасов.ДокументПередачи В
	|				(ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Функция ИзмеренияРедактируемыхВидовЗапасов()
	...
	ИменаПолейИзмерений.Добавить("НомерГТД");
