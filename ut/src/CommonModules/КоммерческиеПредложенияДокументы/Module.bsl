...
// Функция возвращает данные категорий и изобаржений из сервиса по идентификаторам номенклатуры. Если по переданным идентификаторам
//   не удалось получить данные из сервиса или карточки не куплены - в возвращаемом значении запись будет отсутствовать.
//
// Параметры:
//  ИдентификаторыСервиса - Массив, Строка - идентификаторы запрашиваемой номенклатуры.
...
//    * Ключ - Строка - идентификатор номенклатуры
//    * Значение - Структура - содержит:
//		** ДанныеКатегории - Структура - содержит:
//      	*** КатегорияВСервисеИдентификатор - Строка - идентификатор категории.
//      	*** КатегорияВСервисеПредставление - Строка - наименование категории.
//		** ВнешнийАдресИзображения - Строка - внешний адрес на изображение
//
Функция ДанныеКатегорийИзображенийСервиса(Знач ИдентификаторыСервиса) Экспорт
	...
	Если Не ДанныеСервиса = Неопределено Тогда
		Для Каждого СтрокаДанных Из ДанныеСервиса Цикл
			...
			ДанныеКатегории = СтрокаДанных.Категория;
			Если ТипЗнч(ДанныеКатегории) = Тип("Структура") И ДанныеКатегории.Свойство("Идентификатор") Тогда
				ДанныеСервиса.Вставить("ДанныеКатегории",
					Новый Структура("КатегорияВСервисеИдентификатор, КатегорияВСервисеПредставление",
					ДанныеКатегории.Идентификатор, ДанныеКатегории.Наименование));
			КонецЕсли;
			
			ВнешниеАдресаИзображений = СтрокаДанных.Изображения.ВыгрузитьКолонку("ИзображениеURL");
			Если ВнешниеАдресаИзображений.Количество() > 0 Тогда
				ДанныеСервиса.Вставить("ВнешнийАдресИзображения",ВнешниеАдресаИзображений[0]);	
			КонецЕсли;
			
			Если ДанныеСервиса.Количество() > 0 Тогда
				Результат.Вставить(СтрокаДанных.Идентификатор,ДанныеСервиса);
			КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.КоммерческиеПредложения.КоммерческиеПредложенияДокументы.ЗапросКоммерческихПредложенийПоставщиков") Тогда
		// Установка учетной записи и представления контактной информации в докуменах ЗапросКоммерческихПредложенийПоставщиков
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "2.5.27.21";
		Обработчик.Идентификатор = Новый УникальныйИдентификатор("da6dda3c-c25c-4a7b-9e6b-4a7410a82e14");
		Обработчик.Процедура = 
			"Документы.ЗапросКоммерческихПредложенийПоставщиков.ОбработатьДанныеДляПереходаНаНовуюВерсию";
		...
		Обработчик.ПроцедураЗаполненияДанныхОбновления = 
			"Документы.ЗапросКоммерческихПредложенийПоставщиков.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
		Обработчик.ПроцедураПроверки = ИмяПроцедурыПроверки;
		Обработчик.Комментарий = СтрШаблон(ШаблонКомментария, НСтр("ru = 'Заполняет документ'"),
			Метаданные.Документы.ЗапросКоммерческихПредложенийПоставщиков.Синоним);
		Обработчик.Порядок = Перечисления.ПорядокОбработчиковОбновления.Обычный;
		Обработчик.ЧитаемыеОбъекты      = "Документ.ЗапросКоммерческихПредложенийПоставщиков";
		Обработчик.ИзменяемыеОбъекты    = "Документ.ЗапросКоммерческихПредложенийПоставщиков";
		Обработчик.БлокируемыеОбъекты   = "Документ.ЗапросКоммерческихПредложенийПоставщиков";
	КонецЕсли;
	
	// Добавление варианта отчета "Результат опроса"
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВариантыОтчетов") Тогда
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "2.5.27.21";
		Обработчик.Идентификатор = Новый УникальныйИдентификатор("5a3e9dcb-a781-4d9e-81bc-eb05bb95984c");
		Обработчик.Процедура = 
			"КоммерческиеПредложенияДокументы.ЗарегистрироватьВариантыОтчетов";
		...
		Обработчик.ПроцедураПроверки = ИмяПроцедурыПроверки;
		Обработчик.Комментарий = СтрШаблон(ШаблонКомментария, НСтр("ru = 'Создает (обновляет) варианты отчета'"),
			Метаданные.Отчеты.АналитическийОтчетПоАнкетированию.Синоним);
		Обработчик.Порядок = Перечисления.ПорядокОбработчиковОбновления.Обычный;
...
// Регистрирует новые варианты для типовых отчетов
// 
// Параметры:
//  Параметры - см. ОбновлениеИнформационнойБазы.ОсновныеПараметрыОтметкиКОбработке.
//
Процедура ЗарегистрироватьВариантыОтчетов(Параметры) Экспорт
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВариантыОтчетов") Тогда
		НастроитьВариантыОтчетов(Неопределено);
	КонецЕсли;
	
	Параметры.ОбработкаЗавершена =
	ОбновлениеИнформационнойБазы.ОбработкаДанныхЗавершена(Параметры.Очередь, "");
	
КонецПроцедуры

// Настраивает варианты типовых отчетов
// 
// Параметры:
//  Настройки - см. ВариантыОтчетовПереопределяемый.НастроитьВариантыОтчетов
//
Процедура НастроитьВариантыОтчетов(Настройки = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Анкетирование") Тогда
		Если Настройки = Неопределено Тогда
			...
			ПолноеИмяОтчета = Метаданные.Отчеты.АналитическийОтчетПоАнкетированию.ПолноеИмя();
			Отчет = Справочники.ИдентификаторыОбъектовМетаданных.НайтиПоРеквизиту("ПолноеИмя", ПолноеИмяОтчета);
			ОписаниеОтчета.Вставить("Отчет", Отчет);
		Иначе
			ОписаниеОтчета = ВариантыОтчетов.ОписаниеОтчета(Настройки, Метаданные.Отчеты.АналитическийОтчетПоАнкетированию);
		КонецЕсли;
	КонецЕсли;
	
	// ЭлектронноеВзаимодействие.ЗапросКоммерческихПредложенийПоставщиков
	Если ОписаниеОтчета <> Неопределено Тогда
		КлючВарианта = Метаданные.Документы.ЗапросКоммерческихПредложенийПоставщиков.Макеты.РезультатОпросаБидзаар.Имя;
		НаименованиеВарианта = Метаданные.Документы.ЗапросКоммерческихПредложенийПоставщиков.Макеты.РезультатОпросаБидзаар.Представление();
		ЗарегистрироватьВариантОтчетаДляЗКП(ОписаниеОтчета, КлючВарианта, НаименованиеВарианта);
	КонецЕсли;
	// Конец ЭлектронноеВзаимодействие.ЗапросКоммерческихПредложенийПоставщиков
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗарегистрироватьВариантыОтчетов");
	Методы.Вставить("СохранитьПользовательскиеИзмененияВДокументахИсточниках", Истина);
...
//
// Возвращаемое значение:
//  Структура - Данные товара для ЭД:
// * НоменклатураСсылка - ОпределяемыйТип.НоменклатураБЭД
// * ХарактеристикаСсылка - ОпределяемыйТип.ХарактеристикаНоменклатурыБЭД
// * ЕдиницаИзмеренияСсылка - ОпределяемыйТип.УпаковкаНоменклатурыБЭД 
// * НоменклатураНаименование - Строка 
// * ХарактеристикаНаименование - Строка 
// * ЕдиницаИзмеренияКодОКЕИ - Число 
// * ЕдиницаИзмеренияНаименование - Строка
// * Описание - Строка
// * АртикулПоставщика - Строка
// * Производитель - Неопределено
// * Штрихкоды - Массив Из Строка
// * РеквизитыТовара - Неопределено 
// * Сервис1СНоменклатура - Неопределено 
// * ТорговаяМарка - Неопределено 
// * Изображение - Строка
...
// Возвращает единицу измерения по умолчанию, если прикладное решение такую предоставляет. Иначе Неопределено
// 
// Возвращаемое значение:
// 	ОпределяемыйТип.УпаковкаНоменклатурыБЭД, Неопределено - Единица измерения по умолчанию
Функция ЕдиницаИзмеренияПоУмолчанию() Экспорт
	
	Результат = Неопределено;
	
	КоммерческиеПредложенияДокументыПереопределяемый.ПриОпределенииЕдиницыИзмеренияПоУмолчанию(Результат);
	
	Возврат Результат;
	
КонецФункции

#Область ХранилищеНастроекДанныхФормы

// Загружает сохраненные настройки панели изображений формы.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - Форма, в которую необходимо загрузить сохраненные настройки.
Процедура ЗагрузитьСохраненныеНастройкиПанелиИзображенийФормы(Форма) Экспорт
	
	#Если Не ВнешнееСоединение Тогда
		
	КлючОбъекта = СтрШаблон("%1/НастройкиФормы", Форма.ИмяФормы);

	КлючНастроек = "ВидимостьПанелиИзображений";
	ЗагрузитьУстановитьСохраненныеНастройкиЭлементаФормы(
		Форма, КлючОбъекта, КлючНастроек, "ГруппаТоварыИзображение", "Видимость", Истина);
	
	ЗначениеСвойства = Истина;
	КлючНастроек = "ПометкаКнопкиИзображения";
	ЗагрузитьУстановитьСохраненныеНастройкиЭлементаФормы(
		Форма, КлючОбъекта, КлючНастроек, "ПанельИзображение", "Пометка", ЗначениеСвойства);
		
	ОбщегоНазначенияКлиентСервер.УстановитьРеквизитФормыПоПути(Форма, "ПанельОтображается", ЗначениеСвойства);

	#КонецЕсли
	
КонецПроцедуры

// Сохраняет настройки панели изображений формы
// 
// Параметры:
//  ИмяФормы - Строка - Имя формы, настройки панели изображений которой необходимо сохранить
//  ПанельОтображается - Булево - Текущее значение свойства, необходимое для установки
Процедура СохранитьНастройкиПанелиИзображенийФормы(ИмяФормы, ПанельОтображается) Экспорт

	КлючОбъекта = СтрШаблон("%1/НастройкиФормы", ИмяФормы);
	
	КлючНастроек = "ВидимостьПанелиИзображений";
	СохранитьНастройкиЭлементаФормы(
		КлючОбъекта, КлючНастроек, "ГруппаТоварыИзображение", "Видимость", ПанельОтображается);
		
	КлючНастроек = "ПометкаКнопкиИзображения";
	СохранитьНастройкиЭлементаФормы(
		КлючОбъекта, КлючНастроек, "ПанельИзображение", "Пометка", ПанельОтображается);	
...
Процедура ЗарегистрироватьВариантОтчетаДляЗКП(ОписаниеОтчета, КлючВарианта, НаименованиеВарианта)
	
	ВариантСсылка = ВариантОтчетаРезультатОпроса(ОписаниеОтчета.Отчет, КлючВарианта);
	...
	Попытка
		СКД = Документы.ЗапросКоммерческихПредложенийПоставщиков.ПолучитьМакет(КлючВарианта);
		Вариант = СКД.ВариантыНастроек[НаименованиеВарианта];
	Исключение
		Возврат;
	КонецПопытки;
	
	ВариантОбъект.Настройки = Новый ХранилищеЗначения(Вариант.Настройки);
	
	ВариантОбъект.Наименование = НаименованиеВарианта;
	ВариантОбъект.Отчет = ОписаниеОтчета.Отчет;
	...
	ВариантОбъект.Назначение = Перечисления.НазначенияВариантовОтчетов.ДляКомпьютеровИПланшетов;
	ПользователиВарианта = Новый СписокЗначений;
	УведомитьПользователей = Ложь;
	
	ЭтоНовыйВариантОтчета = ВариантОбъект.Ссылка.Пустая();
	...
	Попытка
		...
		Если Не ЭтоНовыйВариантОтчета Тогда
			...
			ЭлементБлокировки.УстановитьЗначение("Ссылка", ВариантОбъект.Ссылка);
			Блокировка.Заблокировать();
		КонецЕсли;
		
		Если ЭтоНовыйВариантОтчета И ВариантыОтчетов.НаименованиеЗанято(ВариантОбъект.Отчет, ВариантОбъект.Ссылка, ВариантОбъект.Наименование) Тогда
			ОтменитьТранзакцию();
			Возврат;
		КонецЕсли;
...
Функция ВариантОтчетаРезультатОпроса(Отчет, КлючВарианта)
...
Процедура ЗагрузитьУстановитьСохраненныеНастройкиЭлементаФормы(
		Форма, КлючОбъекта, КлючНастроек, ИмяЭлемента, ИмяСвойства, ЗначениеСвойства)
	
	НастройкиПоУмолчанию = НовыйЭлементНастроекСвойствЭлементовФормы(
		ИмяЭлемента, ИмяСвойства, ЗначениеСвойства);

	НастройкиЭлементаФормы =
		ОбщегоНазначения.ХранилищеНастроекДанныхФормЗагрузить(КлючОбъекта, КлючНастроек, НастройкиПоУмолчанию);	
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, НастройкиЭлементаФормы.ИмяЭлемента,
		НастройкиЭлементаФормы.ИмяСвойства, НастройкиЭлементаФормы.Значение);	
	
	ЗначениеСвойства = НастройкиЭлементаФормы.Значение;
	
КонецПроцедуры

Процедура СохранитьНастройкиЭлементаФормы(КлючОбъекта, КлючНастроек, ИмяЭлемента, ИмяСвойства, ЗначениеСвойства)

	ЗначениеНастройки = НовыйЭлементНастроекСвойствЭлементовФормы(
		ИмяЭлемента, ИмяСвойства, ЗначениеСвойства);

	ОбщегоНазначенияВызовСервера.ХранилищеНастроекДанныхФормСохранить(
		КлючОбъекта, КлючНастроек, ЗначениеНастройки);
	
КонецПроцедуры

// Конструктор. Новый элемент настроек свойств элементов формы.
// 
// Параметры:
//  ИмяЭлемента - Строка
//  ИмяСвойства - Строка
...
// * ИмяСвойства - Строка 
// * Значение - Строка 
Функция НовыйЭлементНастроекСвойствЭлементовФормы(ИмяЭлемента, ИмяСвойства, Значение)
