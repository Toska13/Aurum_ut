...
Процедура ОбработкаНавигационнойСсылкиФормы_Контрагент(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка, Форма) Экспорт
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ОткрытьНастройкиУчетаНДСПриУСНКомитента" Тогда
		ОрганизацииКлиентЛокализация.ПроверитьЧтоЭтоНеНовыйЭлемент(Форма,
			НСтр("ru = 'Настройки учета НДС при УСН Комитента'"),
			Новый ОписаниеОповещения("ОткрытьНастройкиУчетаНДСПриУСНКомитентовОбъектЗаписан", 
				ЭтотОбъект,
...
#Область ОбработчикиСобытийЭлементовШапкиФормыПомощникНового

// Обработчик при изменении банка в форме ПомощникНового
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - форма помощника.
//  Постфикс - Строка - дополнение к наименованию реквизита на форме.
//
Процедура ПриИзмененииБанкаЛокализация(Форма, Постфикс) Экспорт
	
	//++ Локализация
	Элементы = Форма.Элементы;
	Элементы["ГруппаОшибкаВНомереСчета" + Постфикс].Видимость = Ложь;
	
	Форма["ЭтоIBAN" + Постфикс] = Форма["ЭтоIBAN" + Постфикс]
						И Не Форма["СтранаБанка" + Постфикс] = ПредопределенноеЗначение("Справочник.СтраныМира.Россия");
	
	Если Форма.МеждународныеРеквизитыБанковскихСчетов Тогда
		
		Форма["ИностранныйБанк" + Постфикс] = НЕ (ЗначениеЗаполнено(Форма["СтранаБанка" + Постфикс])
									И Форма["СтранаБанка" + Постфикс] = ПредопределенноеЗначение("Справочник.СтраныМира.Россия"));
		
		ДанныеСчета = ДенежныеСредстваКлиентСервер.НовыеДанныеБанковскогоСчета();
		ДенежныеСредстваКлиентСервер.ЗаполнитьДанныеБанковскогоСчета(ДанныеСчета, Форма, Постфикс);
		ДенежныеСредстваКлиентЛокализация.ПроверитьНомерСчета(Форма, Постфикс, ДанныеСчета);
...
Процедура ДополнитьДанныеИсторииНаименования(СтрокаИсторииНаименований, ИмяРеквизитаСокращенноеНаименование, НаименованиеСокращенное) Экспорт
	//++ Локализация
	Если СтрНайти(ВРег(НаименованиеСокращенное), ВРег("ИП Глава крестьянского (фермерского) хозяйства")) = 1 Тогда // @Non-NLS-1
		СтрокаИсторииНаименований[ИмяРеквизитаСокращенноеНаименование] = НаименованиеСокращенное;
	КонецЕсли;
	//-- Локализация
КонецПроцедуры
	...
	Иначе
		...
		Если РезультатСравнения.ЕстьИзменения Тогда
			...
			Если РезультатСравнения.ЕстьИзмененияВЗаполненныхРеквизитах
				И НЕ Форма.ЗаданВопросОПерезаполнении Тогда 
					
				Форма.ЗаданВопросОПерезаполнении = Истина;
...
Процедура ПроверкаКонтрагента(СсылкиКонтрагенты, ПараметрыВыполнения) Экспорт
	
	Если ЗначениеЗаполнено(СсылкиКонтрагенты) Тогда
		
		Контрагент = СсылкиКонтрагенты[0];
		Если ТипЗнч(СсылкиКонтрагенты[0]) = Тип("СправочникСсылка.Партнеры") Тогда
			
			Контрагент = ПартнерыИКонтрагентыВызовСервера.КонтрагентПартнера(СсылкиКонтрагенты[0]);
			
		КонецЕсли;
		
		ПроверкаКонтрагентаКлиент.Проверить(Контрагент, ПараметрыВыполнения.Форма);
...
Процедура ПоискКонтрагентов(СсылкиКонтрагенты, ПараметрыВыполнения) Экспорт
...
Процедура ЗаполнениеРеквизитовКонтрагентаПоИННЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры.Форма.ЗаданВопросОПерезаполнении = Ложь;
...
Процедура ЗаполнитьРеквизитыКонтрагентаПоПолученнымДанным(РеквизитыКонтрагента, Форма)
	...
	Иначе
		...
		ЕстьИзмененияКонтактнойИнформации = Ложь;
		
		Если РеквизитыКонтрагента.Свойство("Телефон") Тогда
			ЕстьИзмененияКонтактнойИнформации = Истина;
		...
		Если ЕстьИзмененияКонтактнойИнформации Тогда
			Оповестить("ИзменениеКонтактнойИнформации_ПриЗаполненииДанных",, Форма);
		КонецЕсли;
		...
		Если ЗаполнениеРеквизитовПоДаннымИННВозможно Тогда
			...
			Если РезультатЗапуска.ЗаданиеЗапущено Тогда
				...
				ОповещениеОЗавершении = Новый ОписаниеОповещения(
					"ВыполнитьЗаполнениеРеквизитовПоИННОповещение", ЭтотОбъект, Форма);
				
				ДлительныеОперацииКлиент.ОжидатьЗавершение(
					РезультатЗапуска.РезультатФоновогоЗадания, ОповещениеОЗавершении, ПараметрыОжидания);
...
Процедура ВыполнитьЗаполнениеРеквизитовПоИННОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		ПартнерыИКонтрагентыЛокализацияКлиент.ВыполнитьЗаполнениеРеквизитовПоИНН(ДополнительныеПараметры,
...
Процедура ОткрытьНастройкиУчетаНДСПриУСНКомитентовОбъектЗаписан(Результат, ДополнительныеПараметры) Экспорт
	...
	ОткрытьФорму("РегистрСведений.НастройкиУчетаНДСПриУСНКомитентов.ФормаЗаписи",
		Новый Структура("Комитент", Контрагент),
		Форма,,,,,
