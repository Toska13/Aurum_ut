...
Процедура ОбработкаОповещения(Форма, ИмяСобытия, Параметр, Источник, ДополнительныеПараметры) Экспорт
	...
	ОбработкаОповещенияПриЗаписиДокументаВыводИзОборотаИСМП(Источник, Параметр, ИмяСобытия, МестоВызова);
...
Процедура ОбработкаОповещенияПриЗаписиДокументаВыводИзОборотаИСМП(Источник, Параметр, ИмяСобытия, МестоВызова)
	
	Если Не ТипЗнч(Источник) = Тип("ДокументСсылка.ВыводИзОборотаИСМП") Тогда
		Возврат;
	КонецЕсли;
	
	ОписаниеСобытия = ИнтеграцияИСКлиентСервер.ПреобразоватьИмяСобытияОповещенияВоВнутреннийФормат(ИмяСобытия);
	СтруктураДокументСсылка = Новый Структура("Ссылка", Неопределено);
	
	Если Не МестоВызова.Объект = Неопределено Тогда
		ЗаполнитьЗначенияСвойств(СтруктураДокументСсылка, МестоВызова.Объект);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(МестоВызова.Ссылка) И Не ЗначениеЗаполнено(СтруктураДокументСсылка.Ссылка) Тогда
		СтруктураДокументСсылка.Ссылка = МестоВызова.Ссылка;
	КонецЕсли;
	
	Если Не ИнтеграцияИСКлиентСервер.ЭтоСобытиеОповещенияИзмененОбъект(ОписаниеСобытия)
		Или Не ЗначениеЗаполнено(СтруктураДокументСсылка.Ссылка) Тогда
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(Параметр) = Тип("Структура") Тогда
		
		Если Параметр.Свойство("Основание")
			И Параметр.Основание = СтруктураДокументСсылка.Ссылка Тогда
			...
			СоответствиеТребованиямГИСМТКлиент.ОбновитьГиперссылкуСтатусаПослеПринудительногоЗавершенияПроверкиНаСоответствиеТребованиямГИСМТ(
				МестоВызова.Форма,
				СтруктураДокументСсылка.Ссылка);
...
#Область ТоварнаяГруппа

Процедура ТоварнаяГруппаПриИзменении(Форма, Элемент) Экспорт
...
Процедура ПриНачалеВыбораСырья(Элемент, ТекущиеДанные, СтандартнаяОбработка, ВключаяПустойВидПродукции = Истина) Экспорт
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.ВидПродукцииГИСМТ) Тогда
			ВидыПродукцииОтбор = Новый Массив;
			Если ВключаяПустойВидПродукции Тогда
				ВидыПродукцииОтбор.Добавить(ПредопределенноеЗначение("Справочник.ВидыПродукцииИС.ПустаяСсылка"));
			КонецЕсли;
			
			ВидыПродукцииОтбор.Добавить(ТекущиеДанные.ВидПродукцииГИСМТ);
			ПараметрыФормы.Вставить("ВидПродукцииИС", ВидыПродукцииОтбор);
		КонецЕсли;
		
		ОткрытьФорму(
			"Справочник.КлассификаторСырьяГИСМТ.ФормаВыбора", ПараметрыФормы, Элемент,,,,,
			РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
