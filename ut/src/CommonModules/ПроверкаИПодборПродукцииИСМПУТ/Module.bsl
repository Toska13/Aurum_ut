...
Процедура ЕстьМаркируемаяПродукцияВКоллекции(Коллекция, ВидМаркируемойПродукции, ЕстьМаркируемаяПродукция) Экспорт
	...
	Запрос.УстановитьПараметр("ВидыПродукции", ВидМаркируемойПродукции);
...
Процедура ЗаполнитьПродукциюДокумента(Контекст, ТаблицаМаркируемойПродукции, Сценарий = 0) Экспорт
	...
	ИначеЕсли ТипЗнч(Контекст) = Тип("ДанныеФормыСтруктура") Тогда
		Если ЭтоДанныеФормыСтруктураДокументаПоНаименованию(Контекст, "ЧекККМ")
			...
			РезультатЗапроса = ЗапросМаркируемойПродукцииЧекККМ(Контекст);
		Иначе
			РезультатЗапроса = ЗапросМаркируемойПродукцииДанныеФормыСтруктура(Контекст);
		КонецЕсли;
	ИначеЕсли ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(Контекст, "ВозвратТоваровОтКлиента") Тогда
		РезультатЗапроса = ЗапросМаркируемойПродукцииВозвратТоваровОтКлиента(Контекст);
	ИначеЕсли ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(Контекст, "ПриобретениеТоваровУслуг") Тогда
		РезультатЗапроса = ЗапросМаркируемойПродукцииПриобретениеТоваровУслуг(Контекст);
	...
	ИначеЕсли ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(Контекст, "РеализацияТоваровУслуг") Тогда
		РезультатЗапроса = ЗапросМаркируемойПродукцииРеализацияТоваровУслуг(Контекст);
	ИначеЕсли ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(Контекст, "РасходныйОрдерНаТовары") Тогда
		РезультатЗапроса = ЗапросМаркируемойПродукцииРасходныйОрдерНаТовары(Контекст);
...
Функция АдресДанныхПроверкиМаркируемойПродукцииЧекККМ(Объект, УникальныйИдентификатор) Экспорт
	
	МассивУпаковок = Новый Массив();
	
	Для Каждого СтрокаТаблицы Из Объект.АкцизныеМарки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.АкцизнаяМарка) Тогда
			НовыйЭлемент.ШтрихкодУпаковки = СтрокаТаблицы.АкцизнаяМарка;
		КонецЕсли;
		МассивУпаковок.Добавить(НовыйЭлемент);
		
	КонецЦикла;
	
	ТаблицаМаркируемойПродукции = ПроверкаИПодборПродукцииИС.ТаблицаПродукцииДокумента(Объект);
	
	ДанныеХранилища = Новый Структура("МассивУпаковок, ТаблицаМаркируемойПродукции",
		МассивУпаковок,
...
// При закрытии формы проверки и подбора в форме РМК.
// 
// Параметры:
...
Процедура ПриЗакрытииФормыПроверкиИПодбораВФормеРМК(Форма, Результат) Экспорт
	...
	Если Форма.ИмяФормы = "Документ.ЧекККМ.Форма.ФормаДокументаРМК"
		...
		Форма.Объект.АкцизныеМарки.Очистить();
		СтрокиМаркируемыхТоваров = Новый Массив;
		Для Каждого СтрокаТаблицы Из Форма.Объект.Товары Цикл
			
			Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(СтрокаТаблицы.ВидПродукцииИС, Истина)
				Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(СтрокаТаблицы.ВидПродукцииИС) Тогда
				СтрокиМаркируемыхТоваров.Добавить(СтрокаТаблицы);
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого СтрокаПоВидуПродукции Из СтрокиМаркируемыхТоваров Цикл
		...
		ПрисутствуютВидыПродукцииСМРЦ = Ложь;
		Для Каждого СтрокаТаблицы Из ДобавленныеСтроки Цикл
			ПрисутствуютВидыПродукцииСМРЦ = ПрисутствуютВидыПродукцииСМРЦ
				Или КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(СтрокаТаблицы.ВидПродукцииИС);
		КонецЦикла;
		...
		// Привести цены к МРЦ
		Если ПрисутствуютВидыПродукцииСМРЦ Тогда
...
Функция ЗапросМаркируемойПродукцииДанныеФормыСтруктура(ДанныеФормыСтруктура, ИмяКоллекции = "Товары")
	
	Возврат ЗапросМаркируемойПродукцииДанныеФормыКоллекция(ДанныеФормыСтруктура[ИмяКоллекции]);
	
КонецФункции

Функция ЗапросМаркируемойПродукцииДанныеФормыКоллекция(ДанныеФормыКоллекция)
	...
	|		ПО ВТ_Товары.Номенклатура = ОписаниеНоменклатуры.Номенклатура
	|		И (ОписаниеНоменклатуры.КлючВидовПродукции.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
	|			ИЛИ ОписаниеНоменклатуры.КлючВидовПродукции.ВидПродукцииЕГАИС В (&ВидПродукцииЕГАИСМаркируемый))
	|		И НЕ ОписаниеНоменклатуры.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииСырье)
	|СГРУППИРОВАТЬ ПО
	...
	Запрос.УстановитьПараметр("ВидПродукцииЕГАИСМаркируемый", КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЕГАИСМаркируемые());
...
Функция ЗапросМаркируемойПродукцииПриобретениеТоваровУслуг(Документ)
...
Функция ЗапросМаркируемойПродукцииПоступлениеТоваровОтХранителя(Документ)
...
Функция ЗапросМаркируемойПродукцииПоступлениеТоваровОтКлиента(Документ)
...
Функция ЗапросМаркируемойПродукцииПриемкаТоваровНаХранение(Документ)
...
Функция ЗапросМаркируемойПродукцииВозвратТоваровОтКлиента(Документ)
...
Функция ЗапросМаркируемойПродукцииЧекККМ(ФормаОбъект)
...
Функция ЗапросМаркируемойПродукцииРеализацияТоваровУслуг(Документ)
...
Функция ЗапросМаркируемойПродукцииВнутреннееПотребление(Документ)
...
Функция ЗапросМаркируемойПродукцииСборкаТоваров(Документ)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатуры
	|		ПО Шапка.Номенклатура = ОписаниеНоменклатуры.Номенклатура
	|		И (ОписаниеНоменклатуры.КлючВидовПродукции.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
	|			ИЛИ ОписаниеНоменклатуры.КлючВидовПродукции.ВидПродукцииЕГАИС В (&ВидПродукцииЕГАИСМаркируемый))
	|		И НЕ ОписаниеНоменклатуры.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииСырье)
	|ГДЕ
...
Функция ЗапросМаркируемойПродукцииПередачаТоваровМеждуОрганизациями(Документ)
...
Функция ЗапросМаркируемойПродукцииВозвратТоваровМеждуОрганизациями(Документ)
...
Функция ЗапросМаркируемойПродукцииПередачаТоваровХранителю(Документ)
...
Функция ЗапросМаркируемойПродукцииОтгрузкаТоваровКлиенту(Документ)
...
Функция ЗапросМаркируемойПродукцииОтгрузкаТоваровСХранения(Документ)
...
Функция ЗапросМаркируемойПродукцииПеремещениеТоваров(Документ)
...
Функция ЗапросМаркируемойПродукцииРасходныйОрдерНаТовары(Документ)
...
Функция ЗапросМаркируемойПродукцииПриходныйОрдерНаТовары(Документ)
...
Функция ЗапросМаркируемойПродукцииОтборРазмещениеТоваров(Документ)
...
Функция ЗапросМаркируемойПродукцииВозвратТоваровПоставщику(Документ)
...
Функция ЗапросМаркируемойПродукцииКорректировкаРеализации(Документ)
...
Функция ЗапросМаркируемойПродукцииПрочееОприходованиеТоваров(Документ)
	...
	|			ИЛИ ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЕГАИС В (&ВидПродукцииЕГАИСМаркируемый))
...
Функция ИмяТабличнойЧасти(ПараметрыСканирования) Экспорт
	
	Ссылка = ПараметрыСканирования.СсылкаНаОбъект;
	Если Не ЗначениеЗаполнено(Ссылка) Тогда
		Ссылка = Документы.ЧекККМ.ПустаяСсылка();
	КонецЕсли;
	ИмяДокумента = Ссылка.Метаданные().Имя;
	ПараметрыУказанияСерий = Документы[ИмяДокумента].ПараметрыУказанияСерий(Ссылка);
	
	Если ПараметрыУказанияСерий.Свойство("ИмяТЧТовары") Тогда
		Возврат ПараметрыУказанияСерий.ИмяТЧТовары;
	ИначеЕсли ПараметрыУказанияСерий.Свойство("ТЧ") Тогда
		Возврат ПараметрыУказанияСерий.ТЧ.ИмяТЧТовары;
	ИначеЕсли ПараметрыУказанияСерий.Свойство("Товары") Тогда
		Возврат ПараметрыУказанияСерий.Товары.ИмяТЧТовары;
	ИначеЕсли ИмяДокумента = "РасходныйОрдерНаТовары" Тогда
		Возврат "ОтгружаемыеТовары";
	ИначеЕсли ИмяДокумента = "ОтборРазмещениеТоваров" Тогда
		Возврат "ТоварыОтбор";
...
Процедура ПроверитьДополнитьКолонкиСтрокиПодобраннойПродукции(Форма) Экспорт
	
	//Для склада в табличной части
	Если ТипЗнч(Форма.ПодобраннаяМаркируемаяПродукция) = Тип("ТаблицаЗначений") Тогда
		Если Форма.ПодобраннаяМаркируемаяПродукция.Колонки.Найти("Склад") = Неопределено Тогда
			Форма.ПодобраннаяМаркируемаяПродукция.Колонки.Добавить("Склад", Новый ОписаниеТипов("СправочникСсылка.Склады"));
		КонецЕсли;
	КонецЕсли;
	
	Для Каждого СтрокаТЧ Из Форма.ПодобраннаяМаркируемаяПродукция Цикл
		СтрокаТЧ.Склад = Форма.ПараметрыСканирования.Склад;
	КонецЦикла;
