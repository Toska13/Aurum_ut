...
Процедура ДополнительноОбработатьНовостьПослеПолученияПередЗаписью(ОбъектНовости, НовостьХДТО) Экспорт
	...
	Если ОбъектНовости.Важность > 0 И ЗначениеЗаполнено(ОбъектНовости.ЛентаНовостей) Тогда
		ОбязательныйКанал = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектНовости.ЛентаНовостей, "ОбязательныйКанал");
		Если Не ОбязательныйКанал Тогда
...
//	// Оформление
//	Элемент.Оформление.УстановитьЗначениеПараметра("Шрифт", Новый Шрифт(ШрифтыСтиля.ТекстВторогоУровня, , , Истина)); // Жирный
//	// Использование
...
// Обработчик изменения состояния новостей (прочтенности, оповещений, отметки флажком и т.п.).
// При работе в модели сервиса:
// - ВыбранныеПользователи - массив ссылок, но в НЕразделенном режиме справочник Пользователи недоступен,
//   поэтому на все ссылки будет "ОбъектНеНайден".
//
// Параметры:
//  ВыбранныеПользователи - Массив из СправочникСсылка.Пользователи - Массив пользователей (может состоять из единственного значения),
//                            для которых было изменено состояние новостей;
//  ВыбранныеНовости      - Массив из СправочникСсылка.Новости - массив новостей (может состоять из единственного значения)
//                            с измененным состоянием.
//  ПредыдущееСостояние   - Структура - структура предыдущих значений состояния новостей. 
//                                    (см. ОбработкаНовостейКлиентСервер.НовыйОписательЗначенийСостоянияНовости); Возможны ключи:
//    * Прочтена                - Булево - Признак прочтенности. Если Неопределено, то было невозможно получить предыдущее состояние;
//    * Пометка                 - Число  - Признак пометки флажком. Если Неопределено, то было невозможно получить предыдущее состояние;
//    * ОповещениеВключено      - Булево - Признак снятия оповещения. Если Неопределено, то было невозможно получить предыдущее состояние;
//    * ДатаНачалаОповещения    - Дата   - Дата начала оповещения (UTC+0). Если Неопределено, то было невозможно получить предыдущее состояние;
//    * УдаленаИзСпискаНовостей - Булево - Признак помещения в корзину. Если Неопределено, то было невозможно получить предыдущее состояние;
//    * ПрочитатьПозже          - Булево - Признак настройки прочтения позже. Если Неопределено, то было невозможно получить предыдущее состояние;
//  НовоеСостояние        - Структура - структура значений для заполнения реквизитов
//                                    регистра сведений СостоянияНовостей.
//                                    (см. ОбработкаНовостейКлиентСервер.НовыйОписательЗначенийСостоянияНовости); Возможны ключи:
//    * Прочтена                - Булево - Признак прочтенности. Если Неопределено, то не изменялось;
//    * Пометка                 - Число  - Признак пометки флажком. Если Неопределено, то не изменялось;
//    * ОповещениеВключено      - Булево - Признак снятия оповещения. Если Неопределено, то не изменялось;
//    * ДатаНачалаОповещения    - Дата   - Дата начала оповещения (UTC+0). Если Неопределено, то не изменялось;
//    * УдаленаИзСпискаНовостей - Булево - Признак помещения в корзину. Если Неопределено, то не изменялось;
//    * ПрочитатьПозже          - Булево - Признак настройки прочтения позже. Если Неопределено, то не изменялось.
//
Процедура ПослеИзмененияСостоянияНовостей(
			ЗНАЧ ВыбранныеПользователи,
			ЗНАЧ ВыбранныеНовости,
			ЗНАЧ ПредыдущееСостояние,
			ЗНАЧ НовоеСостояние) Экспорт
