...
Функция ПреобразоватьФорматЭД(ПотокИсходногоXML, Параметры = Неопределено, КонтекстДиагностики = Неопределено) Экспорт
	...
	Если Параметры = Неопределено Тогда
		...
		Преобразователь.ДобавитьПараметр("ВерсияБЭД",
			СтрШаблон("'%1'", ОбновлениеИнформационнойБазыБЭД.ВерсияБиблиотеки()));
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВыполнитьЗаполнениеПравилПреобразования");
	Методы.Вставить("ЗаполнитьПереченьСпециальныхОбстоятельствДокументаЭДО");
