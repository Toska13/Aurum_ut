...
Процедура ПодготовитьСтрокиВыписки(ДокументыКЗагрузке, БанковскийСчет, РеквизитыСчета, НастройкиЗагрузки) Экспорт
	...
	ДокументыКЗагрузке.Колонки.Добавить("ИдентификаторОперацииСБП", Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(32)));
	...
	КлиентБанкЛокализация.ПодготовитьСтрокиВыписки(ДокументыКЗагрузке, БанковскийСчет, РеквизитыСчета, НастройкиЗагрузки);
	...
	ТаблицаДокументов = ДокументыКЗагрузке.Скопировать(
		...
		|ЭтоЗагрузкаBankFeeds, ИдентификаторОперацииСБП, СуммаДокумента");
	...
	Отбор = Новый Структура;
	Отбор.Вставить("Загружать", Истина);
	Отбор.Вставить("НайденСчетКонтрагента", Ложь);
	СтрокиКЗагрузке = ДокументыКЗагрузке.НайтиСтроки(Отбор);
	ТаблицаДокументов = ДокументыКЗагрузке.Скопировать(
		СтрокиКЗагрузке,
...
Процедура ОбновитьДокументы(ДокументыКЗагрузке, БанковскийСчет, НастройкиЗагрузки) Экспорт
	...
	Для каждого СтрокаДокумента Из СтрокиКЗагрузке Цикл
		...
		Если РеквизитыХозОперации.Найти("БанковскийСчетКонтрагента") <> Неопределено И Не ЗначениеЗаполнено(ДокументОбъект.БанковскийСчетКонтрагента) Тогда
			...
			// Не заполнен счет, но указан контрагент, у которого нет дублей счета по платежу из выписки - создание счета или подбор созданного в процессе загрузки
			// Если указан контрагент, у которого есть дубли счета по платежу, тогда выбор счета выполняется вручную.
			ИначеЕсли ЗначениеЗаполнено(ДокументОбъект.Контрагент) И СтрокаДокумента.СоздаватьКонтрагента Тогда
		...
		Если СуммыРазличаются
			...
			Если СтрокаДокумента.Исходящий Тогда
				ЗаполнитьРасшифровкуСписания(СтрокаДокумента, ДокументОбъект, РеквизитыХозОперации, НастройкиЗагрузки, ДоступныеСтатьиДДС);
			Иначе
				ЗаполнитьРасшифровкуПоступления(СтрокаДокумента, ДокументОбъект, РеквизитыХозОперации, НастройкиЗагрузки, ДоступныеСтатьиДДС);
			КонецЕсли;
...
Процедура СоздатьДокументы(ДокументыКЗагрузке, БанковскийСчет, НастройкиЗагрузки) Экспорт
	...
	ПроводитьДокументы = НастройкиЗагрузки.ПроводитьДокументы;
	ОстаткиЗаявок = Неопределено;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьЗаявкиНаРасходованиеДенежныхСредств")
		И Не РазрешитьПлатежиБезУказанияЗаявок Тогда
		ОстаткиЗаявок = ОстаткиЗаявокНаОплатуПоСчету(БанковскийСчет, РеквизитыСчета);
	КонецЕсли;
	...
	Для каждого СтрокаДокумента Из СтрокиКЗагрузке Цикл
		...
		Если СтрокаДокумента.Исходящий Тогда
			ЗаполнитьРеквизитыСписания(ДокументыКЗагрузке, СтрокаДокумента, ДокументОбъект, НастройкиЗагрузки, Префикс);
			ПодобратьЗаявкиНаОплату(ОстаткиЗаявок, СтрокаДокумента, ДокументОбъект);
		Иначе
			ЗаполнитьРеквизитыПоступления(ДокументыКЗагрузке, СтрокаДокумента, ДокументОбъект, НастройкиЗагрузки);
		КонецЕсли;
		...
		Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзДругойОрганизации 
			...
			Если СтрокаДокумента.Исходящий Тогда
				ЗеркальныйПлатежСсылка = Документы.СписаниеБезналичныхДенежныхСредств.НайтиЗеркальныйПлатеж(ДокументОбъект);
				...
				КонецЕсли;
			Иначе
				ЗеркальныйПлатежСсылка = Документы.ПоступлениеБезналичныхДенежныхСредств.НайтиЗеркальныйПлатеж(ДокументОбъект);
				Если ЗначениеЗаполнено(ЗеркальныйПлатежСсылка) И ЗеркальныйПлатежСсылка.СуммаДокумента = ДокументОбъект.СуммаДокумента Тогда
...
Функция СоздатьКонтрагента(ДокументыКЗагрузке, СтрокаДокумента) Экспорт
	...
	РеквизитыКонтрагента = НайтиСоздатьКонтрагента(СтрокаДокумента, ДанныеКонтрагента, ДанныеПартнера);
	
	СтрокаДокумента.Контрагент = РеквизитыКонтрагента.Контрагент;
	СтрокаДокумента.Партнер = РеквизитыКонтрагента.Партнер;
	...
	Если Не ПустаяСтрока(ДанныеКонтрагента.ИНН) Тогда
		...
		ЗаполнитьКонтрагентаВДокументахЗагрузки(
			РеквизитыКонтрагента.Контрагент, РеквизитыКонтрагента.Партнер, ДокументыКЗагрузке, СтруктураПоиска);
	...
	Возврат РеквизитыКонтрагента.Контрагент;
	
КонецФункции

// Создает новый элемент справочника "БанковскиеСчетаКонтрагентов" или подбирает из уже созданного при загрузке текущей выписки.
//
// Параметры:
//    ДокументыКЗагрузке - ТаблицаЗначений - Таблица документов выписки
...
Функция СоздатьБанковскийСчетКонтрагента(ДокументыКЗагрузке, СтрокаДокумента, ВладелецСчета) Экспорт
	...
	// Поиск счета, созданного в процессе текущей загрузки выписки, и создание счета при отсутствии такого счета
...
Процедура ЗаполнитьКонтрагентов(ДокументыКЗагрузке, ТаблицаДокументов) Экспорт
	...
	Запрос.Текст = "
	|ВЫБРАТЬ // Контрагент уже заполнен или выбран для загруженного платежа - ищем действующий счет
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|// Контрагент не подобран из справочника или первая загрузка - ищем по реквизитам платежа
	|ВЫБРАТЬ
	|	ТаблицаДокументов.НомерСтроки КАК НомерСтроки,
		...
		Если СтрокаДокумента <> Неопределено Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если НайденоКонтрагентов = 1 Тогда
					...
					СтрокаДокумента.СоздаватьКонтрагента = Не (СтрокаДокумента.НайденКонтрагент И СтрокаДокумента.НайденСчетКонтрагента);
...
// Формирует наименование банка-получателя платежа с учетом РКЦ.
//
// Параметры:
//   Банк - Строка - наименование банка
//   РКЦБанка - Строка - наименование РКЦ банка
//
// Возвращаемое значение:
//   Строка - наименование банка-получателя платежа
//
Функция НаименованиеБанкаПолучателяПлатежа(Банк, РКЦБанка) Экспорт

	Если ЗначениеЗаполнено(РКЦБанка) Тогда
		
		ЧастиНаименования = Новый Массив;
		ЧастиНаименования.Добавить(РКЦБанка);
		ЧастиНаименования.Добавить(Банк);
		Результат = СтрСоединить(ЧастиНаименования, "//");
...
Процедура ЗаполнитьРеквизитыСписания(ДокументыКЗагрузке, СтрокаДокумента, ДокументОбъект, НастройкиЗагрузки, Префикс)
	...
	Если Не (ДокументОбъект.ТипПлатежногоДокумента = Перечисления.ТипыПлатежныхДокументов.ИнкассовоеПоручение
		...
		ДокументОбъект.ОплатаПоЗаявкам = Не ДенежныеСредстваСервер.РазрешеныПлатежиБезУказанияЗаявок(ДокументОбъект);
	Иначе
		КлиентБанкЛокализация.РазрешитьОплатуБезЗаявки(ДокументОбъект);
	КонецЕсли;
	
	КлиентБанкЛокализация.ЗаполнитьРеквизитыПлательщика(ДокументОбъект, СтрокаДокумента);
...
Процедура ЗаполнитьРеквизитыПоступления(ДокументыКЗагрузке, СтрокаДокумента, ДокументОбъект, НастройкиЗагрузки)
...
Процедура ЗаполнитьРасшифровкуСписания(СтрокаДокумента, ДокументОбъект, РеквизитыХозОперации, НастройкиЗагрузки, ДоступныеСтатьиДДС)

	ПроводитьДокументы = НастройкиЗагрузки.ПроводитьДокументы;
	
	Если РеквизитыХозОперации.Найти("РасшифровкаПлатежа") <> Неопределено Тогда
		
		Если СтрокаДокумента.НайденоОснованиеПлатежа
			И РеквизитыХозОперации.Найти("РасшифровкаПлатежа.ОбъектРасчетов") <> Неопределено Тогда // УИП
		...
		Иначе
			...
			КлиентБанкЛокализация.ДобавитьПараметрыЗаполненияРасшифровки(НастройкиЗагрузки, ПараметрыДобавленияСтроки);
...
Процедура ЗаполнитьРасшифровкуПоступления(СтрокаДокумента, ДокументОбъект, РеквизитыХозОперации, НастройкиЗагрузки, ДоступныеСтатьиДДС)
...
Функция НайтиСоздатьКонтрагента(ИсходныеДанные, Отбор, ДанныеПартнера)

	Результат = Новый Структура("Контрагент, Партнер");
	
	// поиск существующего контрагента
	Результат.Контрагент = СсылкаИзТаблицыСОтбором("Справочник.Контрагенты", Отбор);
	
	Если ЗначениеЗаполнено(Результат.Контрагент) Тогда
		
		Результат.Партнер = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Результат.Контрагент, "Партнер");
		Возврат Результат;
		
	КонецЕсли;
	
	// создание нового контрагента, если не найден существующий
	...
	Попытка
		...
		НовыйКонтрагент.Партнер = СоздатьПартнера(ДанныеПартнера);
		НовыйКонтрагент.Записать();
		
		// проверка наличия контрагента, исключая вновь созданного
		Исключения = Новый Структура("Ссылка", НовыйКонтрагент.Ссылка);
		Результат.Контрагент = СсылкаИзТаблицыСОтбором("Справочник.Контрагенты", Отбор, Исключения);
		
		Если ЗначениеЗаполнено(Результат.Контрагент) Тогда
			ВызватьИсключение "Контрагент существует";
		КонецЕсли;
	...
	Исключение
		...
		Если Не ЗначениеЗаполнено(Результат.Контрагент) Тогда
			...
			// ошибка записи партнера или контрагента
			Событие = СтрШаблон("%1 %2", НСтр("ru = 'Загрузка платежей из банка.'", ОбщегоНазначения.КодОсновногоЯзыка()),
							НСтр("ru = 'Создание контрагента'", ОбщегоНазначения.КодОсновногоЯзыка()));
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка записи контрагента %1 по причине: 
								|%2'"), Отбор.НаименованиеПолное, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если Не НовыйКонтрагент.ЭтоНовый() Тогда
		
		// обработка нового контрагента
		Результат.Партнер = НовыйКонтрагент.Партнер;
		Результат.Контрагент = НовыйКонтрагент.Ссылка;
		
		ДатаДействия = ?(ЗначениеЗаполнено(ИсходныеДанные.ДатаПроведения), ИсходныеДанные.ДатаПроведения, ИсходныеДанные.Дата);
		ЗаполнитьАдресКонтрагента(ДатаДействия, Результат.Контрагент, Отбор.ЮрФизЛицо, ИсходныеДанные.АдресКонтрагента);
		
		КлиентБанкЛокализация.ЗапуститьЗаполнениеДанныхКонтрагента(Результат.Контрагент, Отбор, ДатаДействия);
		
	Иначе
		// партнер существующего контрагента 
		Результат.Партнер = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Результат.Контрагент, "Партнер");
	КонецЕсли;
...
Функция СсылкаИзТаблицыСОтбором(ИмяТаблицы, ПараметрыЗапроса, ПараметрыИсключения = Неопределено)
	...
	Если ПараметрыИсключения <> Неопределено Тогда
		
		Для Каждого ТекущийПараметрИсключения Из ПараметрыИсключения Цикл
			
			СписокУсловий.Добавить(СтрШаблон("Таблица.%1 <> &%1", ТекущийПараметрИсключения.Ключ));
			Запрос.УстановитьПараметр(ТекущийПараметрИсключения.Ключ, ТекущийПараметрИсключения.Значение);
...
Функция НайтиСоздатьБанковскийСчетКонтрагента(Отбор)
	...
	// Поиск счета, созданного в процессе текущей загрузки выписки
...
Процедура ЗаполнитьСсылкиНаДокументы(ДокументыКЗагрузке, ТаблицаДокументов, БанковскийСчет)
	...
	|	ТаблицаДокументов.ИдентификаторОперацииСБП, 
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеДокумента.БанковскийСчетПолучатель, ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка)
	|			ТОГДА ДанныеДокумента.Контрагент
	|		ИНАЧЕ ДанныеДокумента.БанковскийСчетПолучатель.Владелец
	|	КОНЕЦ КАК Контрагент,
	|	ДанныеДокумента.ПодотчетноеЛицо КАК ПодотчетноеЛицо,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеДокумента.БанковскийСчетПолучатель, ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка)
	|			ТОГДА ДанныеДокумента.БанковскийСчетКонтрагента
	...
	|	ДанныеДокумента.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеДокумента.БанковскийСчетОтправитель, ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка)
	|			ТОГДА ДанныеДокумента.Контрагент
	|		ИНАЧЕ ДанныеДокумента.БанковскийСчетОтправитель.Владелец
	|	КОНЕЦ КАК Контрагент,
	|	ДанныеДокумента.ПодотчетноеЛицо КАК ПодотчетноеЛицо,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеДокумента.БанковскийСчетОтправитель, ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка)
	|			ТОГДА ДанныеДокумента.БанковскийСчетКонтрагента
	|		ИНАЧЕ ДанныеДокумента.БанковскийСчетОтправитель
	|	КОНЕЦ КАК СчетКонтрагента
	|
	|ИЗ
	|	ТаблицаДокументов
	...
	|		И (ДанныеДокумента.НомерВходящегоДокумента = ТаблицаДокументов.Номер
	|			ИЛИ ДанныеДокумента.НомерВходящегоДокумента = ТаблицаДокументов.НомерСокр
	|			ИЛИ ДанныеДокумента.НомерВходящегоДокумента = """"
	|				И НЕ ДанныеДокумента.ПроведеноБанком
	|				И ДанныеДокумента.ИдентификаторОперацииСБП <> """"
	|				И ТаблицаДокументов.ИдентификаторОперацииСБП <> """"
	|				И ДанныеДокумента.ИдентификаторОперацииСБП = ТаблицаДокументов.ИдентификаторОперацииСБП
	|			ИЛИ ДанныеДокумента.НомерВходящегоДокумента = """"
	|				И НЕ ДанныеДокумента.ПроведеноБанком
	|				И ДанныеДокумента.ИдентификаторОперацииСБП <> """"
	|				И ТаблицаДокументов.ИдентификаторОперацииСБП = """"
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СтрокаДокумента <> Неопределено Тогда
			...
			СтрокаДокумента.Контрагент = Выборка.Контрагент;
			СтрокаДокумента.ПодотчетноеЛицо = Выборка.ПодотчетноеЛицо;
			СтрокаДокумента.СчетКонтрагента = Выборка.СчетКонтрагента;
			СтрокаДокумента.НайденКонтрагент = (ЗначениеЗаполнено(СтрокаДокумента.Контрагент) Или ЗначениеЗаполнено(СтрокаДокумента.ПодотчетноеЛицо));
			СтрокаДокумента.НайденСчетКонтрагента = (ЗначениеЗаполнено(СтрокаДокумента.СчетКонтрагента));
			СтрокаДокумента.СоздаватьКонтрагента = Не (СтрокаДокумента.НайденКонтрагент И СтрокаДокумента.НайденСчетКонтрагента);
...
Процедура ЗаполнитьПлатежиКЗагрузкеПоУИП(Исходящий, БанковскийСчет, ДокументыКЗагрузке, СписокУИП)
	...
	ЭтоСчетГОЗ = КлиентБанкЛокализация.ЭтоСчетГОЗ(БанковскийСчет);
	
	Если ЭтоСчетГОЗ Тогда
		
		КлиентБанкЛокализация.ЗаполнитьПлатежиКЗагрузкеГОЗПоУИП(Исходящий, БанковскийСчет, ДокументыКЗагрузке, СписокУИП);
		Возврат;
	...
	Для Каждого ЭлементДанных Из ДанныеВыбора Цикл
		...
		Для Каждого СтрокаТаблицы Из СтрокиТаблицы Цикл
			...
			Если СтрокаДокумента <> Неопределено Тогда
				...
				ЗаполнитьЗначенияСвойств(СтрокаДокумента, Выборка, "Контрагент, Партнер, ОснованиеПлатежа");
				
				Если ЗначениеЗаполнено(Выборка.ОснованиеПлатежа) И Не ЗначениеЗаполнено(Выборка.ОбъектРасчетов) Тогда
					СтрокаДокумента.ОбъектРасчетов = ОбъектыРасчетовСервер.ОбъектРасчетовПоОснованиюПлатежа(
						Выборка.ОснованиеПлатежа)
				Иначе
...
Процедура ЗаполнитьВидОперацииПоУмолчанию(СтрокиКЗагрузке)
	...
	Для Каждого СтрокаТаблицы Из СтрокиКЗагрузке Цикл
		...
		Если СтрокаТаблицы.Исходящий Тогда
			...
			Если ТипЗнч(СтрокаТаблицы.Контрагент) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
				СтрокаТаблицы.ВидОперации = Перечисления.ХозяйственныеОперации.ВыдачаДенежныхСредствПодотчетнику;
			Иначе
		...
		Иначе
			...
			Если ТипЗнч(СтрокаТаблицы.Контрагент) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
				СтрокаТаблицы.ВидОперации = Перечисления.ХозяйственныеОперации.ВозвратДенежныхСредствОтПодотчетника;
			Иначе
...
Функция ОстаткиЗаявокНаОплатуПоСчету(БанковскийСчет, РеквизитыСчета)

	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Заявки.Ссылка КАК Ссылка,
		|	ЕСТЬNULL(РаспределенияПоСчетам.БанковскийСчетКасса, НЕОПРЕДЕЛЕНО) КАК БанковскийСчет,
		|	Заявки.ОперацияССамозанятым КАК ОперацияССамозанятым,
		|	Заявки.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
		|	Заявки.ХозяйственнаяОперацияПоЗарплате КАК ХозяйственнаяОперацияПоЗарплате,
		...
		|	Заявки.ИдентификаторОперацииСБП КАК ИдентификаторОперацииСБП,
		|	Заявки.Дата КАК Дата
		|ПОМЕСТИТЬ ЗаявкиНаОплату
		|ИЗ
		|	Документ.ЗаявкаНаРасходованиеДенежныхСредств КАК Заявки
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаРасходованиеДенежныхСредств.РаспределениеПоСчетам КАК РаспределенияПоСчетам
		|		ПО Заявки.Ссылка = РаспределенияПоСчетам.Ссылка
		|ГДЕ
		|	Заявки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаРасходованиеДенежныхСредств.КОплате)
		|	И Заявки.ФормаОплатыЗаявки В (ЗНАЧЕНИЕ(Перечисление.ФормыОплаты.ПустаяСсылка),
		|		ЗНАЧЕНИЕ(Перечисление.ФормыОплаты.Безналичная),
		|		ЗНАЧЕНИЕ(Перечисление.ФормыОплаты.СистемаБыстрыхПлатежей))
		|	И (Заявки.Валюта = &Валюта
		|			ИЛИ Заявки.ПланированиеСуммы = ЗНАЧЕНИЕ(Перечисление.СпособыПланированияОплатыЗаявок.ВВалютеВзаиморасчетов))
		|	И Заявки.Организация = &Организация
		|	И (РаспределенияПоСчетам.БанковскийСчетКасса = &БанковскийСчет
		|			ИЛИ РаспределенияПоСчетам.БанковскийСчетКасса ЕСТЬ NULL)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ДенежныеСредстваКВыплатеОстатки.ЗаявкаНаРасходованиеДенежныхСредств КАК ЗаявкаНаРасходованиеДенежныхСредств,
		|	ДенежныеСредстваКВыплатеОстатки.БанковскийСчетКасса КАК БанковскийСчетКасса,
		...
		|				(ВЫБРАТЬ
		|					ЗаявкиНаОплату.Ссылка,
		|					ЗаявкиНаОплату.БанковскийСчет
		|				ИЗ
		|					ЗаявкиНаОплату)) КАК ДенежныеСредстваКВыплатеОстатки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЗаявкиНаОплату.Ссылка КАК Ссылка,
		|	ЗаявкиНаОплату.ОперацияССамозанятым КАК ОперацияССамозанятым,
		|	ЗаявкиНаОплату.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
		|	ЗаявкиНаОплату.ХозяйственнаяОперацияПоЗарплате КАК ХозяйственнаяОперацияПоЗарплате,
		|	ЗаявкиНаОплату.Контрагент КАК Контрагент,
		|	ЗаявкиНаОплату.ПодотчетноеЛицо КАК ПодотчетноеЛицо,
		|	ЗаявкиНаОплату.ОрганизацияПолучатель КАК ОрганизацияПолучатель,
		|	ЗаявкиНаОплату.СписокФизЛиц КАК СписокФизЛиц,
		|	ЗаявкиНаОплату.СписокКонтрагентов КАК СписокКонтрагентов,
		|	ЗаявкиНаОплату.СуммаДокумента КАК СуммаДокумента,
		|	СУММА(ОстаткиЗаявок.СуммаОстаток) КАК СуммаОстаток,
		|	ЗаявкиНаОплату.ИдентификаторОперацииСБП КАК ИдентификаторОперацииСБП,
		|	ЗаявкиНаОплату.Дата КАК Дата
		|ПОМЕСТИТЬ ЗаявкиКВыплате
		|ИЗ
		|	ЗаявкиНаОплату КАК ЗаявкиНаОплату
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОстаткиЗаявок КАК ОстаткиЗаявок
		|		ПО ЗаявкиНаОплату.Ссылка = ОстаткиЗаявок.ЗаявкаНаРасходованиеДенежныхСредств
		|			И ЗаявкиНаОплату.БанковскийСчет = ОстаткиЗаявок.БанковскийСчетКасса
		|
		|СГРУППИРОВАТЬ ПО
		|	ЗаявкиНаОплату.Ссылка,
		|	ЗаявкиНаОплату.ОперацияССамозанятым,
		|	ЗаявкиНаОплату.ХозяйственнаяОперация,
		|	ЗаявкиНаОплату.ХозяйственнаяОперацияПоЗарплате,
		|	ЗаявкиНаОплату.Контрагент,
		|	ЗаявкиНаОплату.ПодотчетноеЛицо,
		|	ЗаявкиНаОплату.ОрганизацияПолучатель,
		|	ЗаявкиНаОплату.СписокФизЛиц,
		|	ЗаявкиНаОплату.СписокКонтрагентов,
		|	ЗаявкиНаОплату.СуммаДокумента,
		|	ЗаявкиНаОплату.ИдентификаторОперацииСБП,
		|	ЗаявкиНаОплату.Дата
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицыРасшифровкаПлатежа.ЗаявкаНаРасходованиеДенежныхСредств КАК ЗаявкаНаРасходованиеДенежныхСредств,
		|	ТаблицыРасшифровкаПлатежа.Контрагент КАК Получатель,
		|	СУММА(ТаблицыРасшифровкаПлатежа.Сумма) КАК Сумма
		|ПОМЕСТИТЬ СписаноПоЗаявкам
		|ИЗ
		|	Документ.СписаниеБезналичныхДенежныхСредств.РасшифровкаПлатежа КАК ТаблицыРасшифровкаПлатежа
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеБезналичныхДенежныхСредств КАК ДокументыСписания
		|		ПО ТаблицыРасшифровкаПлатежа.Ссылка = ДокументыСписания.Ссылка
		|			И (ДокументыСписания.СписокКонтрагентов)
		|			И (ДокументыСписания.Проведен)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ЗаявкиКВыплате КАК ЗаявкиКВыплате
		|		ПО ТаблицыРасшифровкаПлатежа.ЗаявкаНаРасходованиеДенежныхСредств = ЗаявкиКВыплате.Ссылка
		|			И (ЗаявкиКВыплате.СписокКонтрагентов)
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицыРасшифровкаПлатежа.ЗаявкаНаРасходованиеДенежныхСредств,
		|	ТаблицыРасшифровкаПлатежа.Контрагент
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ТаблицыРасшифровкаПлатежа.ЗаявкаНаРасходованиеДенежныхСредств,
		|	ДокументыСписания.Контрагент,
		|	СУММА(ТаблицыРасшифровкаПлатежа.Сумма)
		|ИЗ
		|	Документ.СписаниеБезналичныхДенежныхСредств.РасшифровкаПлатежа КАК ТаблицыРасшифровкаПлатежа
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеБезналичныхДенежныхСредств КАК ДокументыСписания
		|		ПО ТаблицыРасшифровкаПлатежа.Ссылка = ДокументыСписания.Ссылка
		|			И (НЕ ДокументыСписания.СписокКонтрагентов)
		|			И (ДокументыСписания.Проведен)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ЗаявкиКВыплате КАК ЗаявкиКВыплате
		|		ПО ТаблицыРасшифровкаПлатежа.ЗаявкаНаРасходованиеДенежныхСредств = ЗаявкиКВыплате.Ссылка
		|			И (ЗаявкиКВыплате.СписокКонтрагентов)
		...
		|ВЫБРАТЬ
		|	ТаблицыРасшифровкаПлатежа.ЗаявкаНаРасходованиеДенежныхСредств,
		|	ТаблицыЛицевыеСчетаСотрудников.ФизическоеЛицо,
		|	СУММА(ТаблицыЛицевыеСчетаСотрудников.Сумма)
		|ИЗ
		|	Документ.СписаниеБезналичныхДенежныхСредств.РасшифровкаПлатежа КАК ТаблицыРасшифровкаПлатежа
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеБезналичныхДенежныхСредств КАК ДокументыСписания
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеБезналичныхДенежныхСредств.ЛицевыеСчетаСотрудников КАК ТаблицыЛицевыеСчетаСотрудников
		|			ПО (ТаблицыЛицевыеСчетаСотрудников.Ссылка = ДокументыСписания.Ссылка)
		|				И (ДокументыСписания.Проведен)
		|		ПО ТаблицыРасшифровкаПлатежа.Ссылка = ДокументыСписания.Ссылка
		|			И (ДокументыСписания.СписокФизЛиц)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ЗаявкиКВыплате КАК ЗаявкиКВыплате
		|		ПО ТаблицыРасшифровкаПлатежа.ЗаявкаНаРасходованиеДенежныхСредств = ЗаявкиКВыплате.Ссылка
		|			И (ЗаявкиКВыплате.СписокФизЛиц)
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицыРасшифровкаПлатежа.ЗаявкаНаРасходованиеДенежныхСредств,
		|	ТаблицыЛицевыеСчетаСотрудников.ФизическоеЛицо
		...
		|ВЫБРАТЬ
		|	ТаблицыРасшифровкаПлатежа.ЗаявкаНаРасходованиеДенежныхСредств,
		|	ДокументыСписания.ПодотчетноеЛицо,
		|	СУММА(ТаблицыРасшифровкаПлатежа.Сумма)
		|ИЗ
		|	Документ.СписаниеБезналичныхДенежныхСредств.РасшифровкаПлатежа КАК ТаблицыРасшифровкаПлатежа
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеБезналичныхДенежныхСредств КАК ДокументыСписания
		|		ПО ТаблицыРасшифровкаПлатежа.Ссылка = ДокументыСписания.Ссылка
		|			И (НЕ ДокументыСписания.СписокФизЛиц)
		|			И (ДокументыСписания.Проведен)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ЗаявкиКВыплате КАК ЗаявкиКВыплате
		|		ПО ТаблицыРасшифровкаПлатежа.ЗаявкаНаРасходованиеДенежныхСредств = ЗаявкиКВыплате.Ссылка
		|			И (ЗаявкиКВыплате.СписокФизЛиц)
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЗаявкиКВыплате.Ссылка КАК Ссылка,
		|	ЗаявкиКВыплате.ОперацияССамозанятым КАК ОперацияССамозанятым,
		|	ЗаявкиКВыплате.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
		|	ЗаявкиКВыплате.ХозяйственнаяОперацияПоЗарплате КАК ХозяйственнаяОперацияПоЗарплате,
		|	ЗаявкиКВыплате.Контрагент КАК Получатель,
		|	ЗаявкиКВыплате.СуммаОстаток КАК СуммаОстаток,
		|	ЗаявкиКВыплате.СуммаДокумента КАК СуммаПоЗаявке,
		|	ЗаявкиКВыплате.СписокФизЛиц КАК СписокФизЛиц,
		|	ЗаявкиКВыплате.СписокКонтрагентов КАК СписокКонтрагентов,
		|	ЗаявкиКВыплате.ИдентификаторОперацииСБП КАК ИдентификаторОперацииСБП,
		|	ЗаявкиКВыплате.Дата КАК Дата
		|ПОМЕСТИТЬ ДетальноПоПолучателям
		|ИЗ
		|	ЗаявкиКВыплате КАК ЗаявкиКВыплате
		|ГДЕ
		|	НЕ ЗаявкиКВыплате.СписокКонтрагентов
		|	И НЕ ЗаявкиКВыплате.СписокФизЛиц
		|	И НЕ ЗаявкиКВыплате.ХозяйственнаяОперация В (
		|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыдачаДенежныхСредствПодотчетнику),
		...
		|
		|ВЫБРАТЬ
		|	ЗаявкиКВыплате.Ссылка,
		|	ЗаявкиКВыплате.ОперацияССамозанятым,
		|	ЗаявкиКВыплате.ХозяйственнаяОперация,
		|	ЗаявкиКВыплате.ХозяйственнаяОперацияПоЗарплате,
		|	РасшифровкаПлатежа.Контрагент,
		|	ЗаявкиКВыплате.СуммаОстаток,
		|	РасшифровкаПлатежа.Сумма,
		|	ЗаявкиКВыплате.СписокФизЛиц,
		|	ЗаявкиКВыплате.СписокКонтрагентов,
		|	ЗаявкиКВыплате.ИдентификаторОперацииСБП,
		|	ЗаявкиКВыплате.Дата
		|ИЗ
		|	ЗаявкиКВыплате КАК ЗаявкиКВыплате
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаРасходованиеДенежныхСредств.РасшифровкаПлатежа КАК РасшифровкаПлатежа
		|		ПО ЗаявкиКВыплате.Ссылка = РасшифровкаПлатежа.Ссылка
		|			И (ЗаявкиКВыплате.СписокКонтрагентов)
		|			И (НЕ ЗаявкиКВыплате.СписокФизЛиц)
		|			И (НЕ ЗаявкиКВыплате.ХозяйственнаяОперация В (
		|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыдачаДенежныхСредствПодотчетнику),
		...
		|	ЗаявкиКВыплате.ХозяйственнаяОперация,
		|	ЗаявкиКВыплате.ХозяйственнаяОперацияПоЗарплате,
		|	ЗаявкиКВыплате.ПодотчетноеЛицо,
		|	ЗаявкиКВыплате.СуммаОстаток,
		|	ЗаявкиКВыплате.СуммаДокумента,
		|	ЗаявкиКВыплате.СписокФизЛиц,
		|	ЗаявкиКВыплате.СписокКонтрагентов,
		|	ЗаявкиКВыплате.ИдентификаторОперацииСБП,
		|	ЗаявкиКВыплате.Дата
		...
		|	ЗаявкиКВыплате КАК ЗаявкиКВыплате
		|ГДЕ
		|	НЕ ЗаявкиКВыплате.СписокФизЛиц
		|	И ЗаявкиКВыплате.ХозяйственнаяОперация В (
		|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыдачаДенежныхСредствПодотчетнику),
		|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыдачаЗаймаСотруднику))
		|
		|ОБЪЕДИНИТЬ ВСЕ
		...
		|		ПО ЗаявкиКВыплате.Ссылка = ЛицевыеСчетаСотрудников.Ссылка
		|			И (ЗаявкиКВыплате.СписокФизЛиц)
		|ГДЕ
		|	НЕ ЗаявкиКВыплате.СписокФизЛиц
		|	И ЗаявкиКВыплате.ХозяйственнаяОперация В (
		...
		|	ЗаявкиКВыплате.ХозяйственнаяОперация,
		|	ЗаявкиКВыплате.ХозяйственнаяОперацияПоЗарплате,
		|	ЗаявкиКВыплате.ОрганизацияПолучатель,
		|	ЗаявкиКВыплате.СуммаОстаток,
		|	ЗаявкиКВыплате.СуммаДокумента,
		|	ЗаявкиКВыплате.СписокФизЛиц,
		|	ЗаявкиКВыплате.СписокКонтрагентов,
		...
		|	ЗаявкиКВыплате КАК ЗаявкиКВыплате
		|ГДЕ
		|	ЗаявкиКВыплате.ХозяйственнаяОперация В (
		|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОплатаДенежныхСредствВДругуюОрганизацию),
		|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратДенежныхСредствВДругуюОрганизацию),
		|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВнутренняяПередачаДенежныхСредств))
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ДетальноПоПолучателям.Ссылка КАК Ссылка,
		|	ДетальноПоПолучателям.ОперацияССамозанятым КАК ОперацияССамозанятым,
		|	ДетальноПоПолучателям.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
		|	ДетальноПоПолучателям.ХозяйственнаяОперацияПоЗарплате КАК ХозяйственнаяОперацияПоЗарплате,
		|	ДетальноПоПолучателям.Получатель КАК Получатель,
		|	ДетальноПоПолучателям.СуммаПоЗаявке КАК СуммаПоЗаявке,
		|	СписаноПоЗаявкам.Сумма КАК СуммаСписано,
		|	ВЫБОР
		|		КОГДА ДетальноПоПолучателям.СписокФизЛиц
		|			ТОГДА ДетальноПоПолучателям.СуммаПоЗаявке - ЕСТЬNULL(СписаноПоЗаявкам.Сумма, 0)
		|		КОГДА ДетальноПоПолучателям.СписокКонтрагентов
		|			ТОГДА ДетальноПоПолучателям.СуммаПоЗаявке - ЕСТЬNULL(СписаноПоЗаявкам.Сумма, 0)
		|		ИНАЧЕ ДетальноПоПолучателям.СуммаОстаток
		|	КОНЕЦ КАК Остаток,
		|	ДетальноПоПолучателям.ИдентификаторОперацииСБП КАК ИдентификаторОперацииСБП,
		|	ДетальноПоПолучателям.Дата КАК Дата
		|ИЗ
		|	ДетальноПоПолучателям КАК ДетальноПоПолучателям
		|		ЛЕВОЕ СОЕДИНЕНИЕ СписаноПоЗаявкам КАК СписаноПоЗаявкам
		|		ПО ДетальноПоПолучателям.Ссылка = СписаноПоЗаявкам.ЗаявкаНаРасходованиеДенежныхСредств
		|			И ДетальноПоПолучателям.Получатель = СписаноПоЗаявкам.Получатель
		|ГДЕ
		|	ВЫБОР
		|			КОГДА ДетальноПоПолучателям.СписокФизЛиц
		|				ТОГДА ДетальноПоПолучателям.СуммаПоЗаявке - ЕСТЬNULL(СписаноПоЗаявкам.Сумма, 0)
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ЗаявкиНаОплату
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ОстаткиЗаявок
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ЗаявкиКВыплате
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ СписаноПоЗаявкам
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ДетальноПоПолучателям";
	
	Запрос.УстановитьПараметр("БанковскийСчет", БанковскийСчет);
	Запрос.УстановитьПараметр("Валюта", РеквизитыСчета.Валюта);
	Запрос.УстановитьПараметр("Организация", РеквизитыСчета.Организация);
	
	Возврат Запрос.Выполнить().Выгрузить();

КонецФункции

Процедура ПодобратьЗаявкиНаОплату(ОстаткиЗаявок, Платеж, ДокументОбъект)

	Если ОстаткиЗаявок = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если Не Платеж.НайденКонтрагент Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыОтбора = Новый Структура;
	ПараметрыОтбора.Вставить("ОперацияССамозанятым", ДокументОбъект.ОперацияССамозанятым);
	
	Если ЗначениеЗаполнено(ДокументОбъект.Контрагент) Тогда
		ПараметрыОтбора.Вставить("Получатель", ДокументОбъект.Контрагент);
	ИначеЕсли ЗначениеЗаполнено(ДокументОбъект.ПодотчетноеЛицо) Тогда
		ПараметрыОтбора.Вставить("Получатель", ДокументОбъект.ПодотчетноеЛицо);
	Иначе
		
		ОрганизацияПолучатель = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект.БанковскийСчетПолучатель, "Владелец");
		ПараметрыОтбора.Вставить("Получатель", ОрганизацияПолучатель);
		
	КонецЕсли;
	
	ПараметрыОтбора.Вставить("ИдентификаторОперацииСБП", Платеж.ИдентификаторОперацииСБП);
	
	ОтобранныеОстатки = ОстаткиЗаявок.НайтиСтроки(ПараметрыОтбора);
	
	Если ОтобранныеОстатки.Количество() = 0
		И Не ПустаяСтрока(Платеж.ИдентификаторОперацииСБП) Тогда
		
		ПараметрыОтбора.Вставить("ИдентификаторОперацииСБП", "");
		ОтобранныеОстатки = ОстаткиЗаявок.НайтиСтроки(ПараметрыОтбора);
		
		Если ОтобранныеОстатки.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;
		
	ИначеЕсли ОтобранныеОстатки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	НоваяТаблица = ДокументОбъект.РасшифровкаПлатежа.ВыгрузитьКолонки();
	
	Для Каждого СтрокаТабличнойЧасти Из ДокументОбъект.РасшифровкаПлатежа Цикл
		
		СуммаРаспределения = СтрокаТабличнойЧасти.Сумма;
		
		Для Каждого ТекущийОстаток Из ОтобранныеОстатки Цикл
			
			ОперацияСовпадает = 
				(ТекущийОстаток.ХозяйственнаяОперация = ДокументОбъект.ХозяйственнаяОперация
					И ТекущийОстаток.ХозяйственнаяОперацияПоЗарплате <> ДокументОбъект.ХозяйственнаяОперация)
				Или (ТекущийОстаток.ХозяйственнаяОперация <> ДокументОбъект.ХозяйственнаяОперация
					И ТекущийОстаток.ХозяйственнаяОперацияПоЗарплате = ДокументОбъект.ХозяйственнаяОперация);
			
			Если ОперацияСовпадает
				И ТекущийОстаток.Дата <= ДокументОбъект.Дата Тогда
				
				НоваяСтрока = НоваяТаблица.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТабличнойЧасти);
				НоваяСтрока.Сумма = Мин(ТекущийОстаток.Остаток, СуммаРаспределения);
				СуммаРаспределения = СуммаРаспределения - НоваяСтрока.Сумма;
				ТекущийОстаток.Остаток = ТекущийОстаток.Остаток - НоваяСтрока.Сумма;
				НоваяСтрока.ЗаявкаНаРасходованиеДенежныхСредств = ТекущийОстаток.Ссылка;
	...
	ДокументОбъект.РасшифровкаПлатежа.Загрузить(НоваяТаблица);
