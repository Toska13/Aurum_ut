...
// Осуществляет пересчет цен и сумм в тч Товары после редактирования курса валюты
//
// Параметры:
//  Таблица            - ТабличнаяЧасть -  Таблица, в которой необходим пересчет
//  ПараметрыПересчета - см. ПараметрыПересчетаСуммТаблицыПриИзмененииКурса
//
Процедура ПересчитатьСуммыТаблицыПриИзмененииКурса(Таблица, ПараметрыПересчета) Экспорт
	
	Если Таблица.Итог("СуммаСНДС") = 0 Тогда
		Возврат;
	КонецЕсли;
	
	КоэффициентПересчета = ПараметрыПересчета.НоваяСуммаДокументаСНДС / Таблица.Итог("СуммаСНДС");
	
	МассивСумм = Новый Массив;
	Для н = 0 По Таблица.Количество()-1 Цикл
		
		ТекСтрока = Таблица[н];
		ТекСтрока.Цена = Окр(ТекСтрока.Цена * КоэффициентПересчета, 2);
		МассивСумм.Добавить(Окр(ТекСтрока.СуммаСНДС * КоэффициентПересчета, 2));
		
	КонецЦикла;
	
	Если Не ПараметрыПересчета.ПересчитыватьСуммы Тогда
		Возврат
	КонецЕсли;
	
	МассивСумм = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(ПараметрыПересчета.НоваяСуммаДокументаСНДС, МассивСумм);
...
// Параметры пересчета сумм таблицы при изменении курса.
// 
// Параметры:
//  ЦенаВключаетНДС         - Булево - Истина, если цена включает ндс.
//  НоваяСуммаДокументаСНДС - Число  - Рассчитанная сумма в валюте документа.
//  ИмяКоличества           - Строка - Имя поля количество.
//  ПересчитыватьСуммы      - Булево - Флаг, определяющий необходимость пересчета полей Сумма и СуммаНДС.
// 
// Возвращаемое значение:
...
// * ИмяКоличества                     - Строка - Имя поля количество.
// * СтавкаНДС                         - СправочникСсылка.СтавкиНДС - общая для всех строк таблицы ставка НДС.
Функция ПараметрыПересчетаСуммТаблицыПриИзмененииКурса(ЦенаВключаетНДС, НоваяСуммаДокументаСНДС, ИмяКоличества = "КоличествоУпаковок", ПересчитыватьСуммы = Истина) Экспорт
	
	ПараметрыПересчета = Новый Структура;
	ПараметрыПересчета.Вставить("ЦенаВключаетНДС",                   ЦенаВключаетНДС);
	ПараметрыПересчета.Вставить("НоваяСуммаДокументаСНДС",           НоваяСуммаДокументаСНДС);
	ПараметрыПересчета.Вставить("ПересчитыватьАвтоматическиеСкидки", Ложь);
	ПараметрыПересчета.Вставить("ПересчитыватьРучныеСкидки",         Ложь);
	ПараметрыПересчета.Вставить("ИмяКоличества",                     ИмяКоличества);
	ПараметрыПересчета.Вставить("СтавкаНДС",                         Справочники.СтавкиНДС.ПустаяСсылка());
	ПараметрыПересчета.Вставить("ПересчитыватьСуммы",                ПересчитыватьСуммы);
