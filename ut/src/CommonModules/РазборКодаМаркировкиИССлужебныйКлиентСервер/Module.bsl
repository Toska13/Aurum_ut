...
Функция КонтрольноеЧислоSSCC(Штрихкод) Экспорт
	...
	Для НомерПозиции = 1 По Позиций Цикл
		Число = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(Сред(Штрихкод, НомерПозиции, 1));
		Если Число = Неопределено Тогда
			Возврат Неопределено;
		КонецЕсли;
		Цифры.Добавить(Число);
	КонецЦикла;
...
//   ВидыПродукции - Неопределено, СправочникСсылка.ВидыПродукцииИС, Массив Из СправочникСсылка.ВидыПродукцииИС - 
...
Функция РазобратьКодМаркировки(ДанныеДляРазбора, ВидыПродукции, ПримечаниеКРезультатуРазбора, Настройки, МодульКонтекста, ПользовательскиеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ВидыПродукции) И ТипЗнч(ВидыПродукции) = Тип("СправочникСсылка.ВидыПродукцииИС") Тогда
	...
	ИначеЕсли ЗначениеЗаполнено(ВидыПродукции) И ТипЗнч(ВидыПродукции) = Тип("Массив") Тогда
		Для Каждого Значение Из ВидыПродукции Цикл
			Если ЗначениеЗаполнено(Значение) И ТипЗнч(Значение) = Тип("СправочникСсылка.ВидыПродукцииИС") Тогда
	...
	Для Каждого ШаблонКодаМаркировки Из НайденныеШаблоны Цикл
		...
		Если ДанныеРезультата.ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC")
			И ПараметрыРазбораКодаМаркировки.ПользовательскиеПараметры <> Неопределено
			И ПараметрыРазбораКодаМаркировки.ПользовательскиеПараметры.Свойство("ТипШтрихкодаSSCC")
			И (ПараметрыРазбораКодаМаркировки.ПользовательскиеПараметры.ТипШтрихкодаSSCC =
					ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC18")
				Или ПараметрыРазбораКодаМаркировки.ПользовательскиеПараметры.ТипШтрихкодаSSCC =
					ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC20")) Тогда
			ДанныеРезультата.ТипШтрихкода = ПараметрыРазбораКодаМаркировки.ПользовательскиеПараметры.ТипШтрихкодаSSCC;
		КонецЕсли;
...
// * ВидыПродукции - Неопределено, Массив из СправочникСсылка.ВидыПродукцииИС -
...
Функция НормализоватьКодМаркировки(РезультатРазбора, ВидПродукции, ПараметрыНормализации = Неопределено) Экспорт
	...
	Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(ВидПродукции)
		Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ВидПродукции) Тогда
	...
	ИначеЕсли РезультатРазбора.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.ОбъемноСортовойУчет") Тогда
		...
		Возврат ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ОСУПоШтрихкодуEAN(
			...
			РезультатРазбора.СоставКодаМаркировки.КоличествоВложенныхЕдиниц,
			ПараметрыНормализации.НачинаетсяСоСкобки);
	...
	ВключаетИдентификаторыПрименения = РезультатРазбора.СоставКодаМаркировки.ВключаетИдентификаторыПрименения;
	...
	ВключатьМРЦ = ПараметрыНормализации.ВключатьМРЦ
		...
		И КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(ВидПродукции);
	...
	ВключатьСрокГодности = ПараметрыНормализации.ВключатьСрокГодности
		...
		И КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСОбязательнымУказаниемСроковГодности(ВидПродукции); // Молочная продукция
	...
	ВключатьКодТНВЭД = РезультатРазбора.СоставКодаМаркировки.Свойство("КодТНВЭД")
		...
		И Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(ВидПродукции); // Обувь
	
	Если РезультатРазбора.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Потребительская") Тогда
		
		Если Не РезультатРазбора.СоставКодаМаркировки.Свойство("GTIN") Тогда
			ВызватьИсключение НСтр("ru = 'В составе кода маркировки отсутствует GTIN.'")
		КонецЕсли;
		
		Если Не ВключаетИдентификаторыПрименения Тогда
			
			Если ВключатьМРЦ
				И Не ПараметрыНормализации.ДобавитьМРЦПоУмолчанию Тогда
			...
			ИначеЕсли ПараметрыНормализации.ДобавитьМРЦПоУмолчанию Тогда
			...
			ИначеЕсли Не ВключатьМРЦ Тогда
	...
	ИначеЕсли ВключатьСрокГодности Тогда
...
Функция МРЦПоВидуУпаковки(МРЦСтрокой, ВидУпаковки) Экспорт
	...
	Если ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Потребительская") Тогда
		...
		// Если цена <= 5000, то высокая вероятность, что это реальное МРЦ.
		Если МРЦ > 500000 Тогда
			Возврат РезультатПроверки;
		КонецЕсли;
...
Функция ПользовательскиеПараметрыРазбораКодаМаркировки() Экспорт
	...
	ПользовательскиеПараметры.Вставить("ТипШтрихкодаSSCC",                                               Неопределено);
...
Функция ВидУпаковкиСоответствуетРазбору(ВидПродукцииИлиКлючВидовПродукции, ВидУпаковки, ДанныеРазбора, ЗаполнятьВидПродукцииПодВидУпаковки = Ложь) Экспорт
	
	Если ДанныеРазбора = Неопределено Или Не ЗначениеЗаполнено(ВидУпаковки) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ТипЗнч(ВидПродукцииИлиКлючВидовПродукции) = Тип("СправочникСсылка.КлючиВидовПродукцииИС") Тогда
		ВидПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(ВидПродукцииИлиКлючВидовПродукции, "ГИСМТ");
		Если Не ЗначениеЗаполнено(ВидПродукции) Тогда
			ВидПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(ВидПродукцииИлиКлючВидовПродукции, "ЕГАИС");
		КонецЕсли;
	Иначе
		ВидПродукции = ВидПродукцииИлиКлючВидовПродукции;
	КонецЕсли;
	Если ЗначениеЗаполнено(ВидПродукции) И ТипЗнч(ВидПродукции) <> Тип("Массив") Тогда
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ВидПродукции) Тогда
			ВидыПродукцииДляПроверки = ВидПродукции;
		Иначе
			ВидыПродукцииДляПроверки = ДанныеРазбора.ВидыПродукции;
		КонецЕсли;
		
		ВидыПродукцииВидаУпаковки = Новый Массив;
	
		Для Каждого ВидПродукцииИС Из ВидыПродукцииДляПроверки Цикл
			ВидыУпаковокПоВидуПродукции = ДанныеРазбора.ВидыУпаковокПоВидамПродукции[ВидПродукцииИС];
			Если ВидыУпаковокПоВидуПродукции = Неопределено
				Или ВидыУпаковокПоВидуПродукции.Найти(ВидУпаковки) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
		...
		Если ЗаполнятьВидПродукцииПодВидУпаковки Тогда
			Если ВидыПродукцииВидаУпаковки.Количество() = 1 Тогда
				ВидПродукцииИлиКлючВидовПродукции = ВидыПродукцииВидаУпаковки[0];
			КонецЕсли;
		КонецЕсли;
...
Функция КодМаркировкиСоответствуетОписанию(ПараметрыРазбораКодаМаркировки, ПараметрыОписанияКодаМаркировки, СоставКодаМаркировки, ЗначенияЭлементовКодаМаркировки = Неопределено)
	...
	Если Не ПараметрыОписанияКодаМаркировки.ВключаетИдентификаторыПрименения
		И Не ПараметрыРазбораКодаМаркировки.НачинаетсяСоСкобки
		И Не ПараметрыРазбораКодаМаркировки.СодержитРазделительGS
		И ПараметрыОписанияКодаМаркировки.ОписаниеЭлементовКодаМаркировки.Количество() > 1
		И ПараметрыОписанияКодаМаркировки.ОписаниеЭлементовКодаМаркировки[0].Имя = "GTIN"
		И ПараметрыОписанияКодаМаркировки.ОписаниеЭлементовКодаМаркировки[1].Имя = "СерийныйНомер"
		И СтрДлина(ПараметрыРазбораКодаМаркировки.КодМаркировки) > 18
		И Лев(ПараметрыРазбораКодаМаркировки.КодМаркировки, 2) = "01"
		И Сред(ПараметрыРазбораКодаМаркировки.КодМаркировки, 17, 2) = "21" Тогда
		// Частный случай с табачной продукцией.
		// Для шаблона "GTIN (14 chars) + СерийныйНомер (7 chars) + МРЦСтрокой (4 chars)" GTIN не должен начинаться с 01.
		Возврат Ложь;
	КонецЕсли;
	...
	Если ПараметрыОписанияКодаМаркировки.ВключаетИдентификаторыПрименения Тогда
		...
		// РазборКодаМаркировкиИССлужебныйКлиентСервер.РазобратьСтрокуШтрихкодаGS1,
	...
	Если Не ПараметрыОписанияКодаМаркировки.ВключаетИдентификаторыПрименения
		...
		И КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ПараметрыОписанияКодаМаркировки.ВидыПродукции[0]) Тогда
	...
	Если ЗначенияЭлементовКМ[СвойствоПриоритетнойПроверки] <> Неопределено Тогда
		...
		Для Каждого ВидПродукции Из КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииКоторымДоступноПолеМРЦ() Цикл
			Если ПараметрыОписанияКодаМаркировки.ВидыПродукции.Найти(ВидПродукции) <> Неопределено Тогда
				ВидыПродукции.Добавить(ВидПродукции);
			КонецЕсли;
		КонецЦикла;
		
		Если СоставКодаМаркировки.МРЦСтрокой = "000000" Или СоставКодаМаркировки.МРЦСтрокой = "AAAA" Тогда
			Для Каждого ВидПродукции Из КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииСодержащиеВКодеМаркировкиНулевойМРЦ() Цикл
				Если ПараметрыОписанияКодаМаркировки.ВидыПродукции.Найти(ВидПродукции) <> Неопределено Тогда
					ВидыПродукции.Добавить(ВидПродукции);
				КонецЕсли;
			КонецЦикла;
...
Функция ОбработатьРезультатНесколькихШаблонов(Результат, ДанныеРезультата)
	...
	Если Результат.Количество() = 2
		И КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(Результат[0].ВидыПродукции)
		И КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(Результат[1].ВидыПродукции) Тогда
...
Функция НайденНедопустимыйСимволXML(Знач Текст) Экспорт
	...
	Возврат ОбработкаСтрокиXML.НайтиНедопустимыеСимволыXML(Текст) > 0;
...
// * РезультатРазбора - см. РазобратьСтрокуШтрихкодаGS1, Неопределено - Предварительный разбор при флаге СодержитРазделительGS
...
//    ** ВидыПродукции - Неопределено, Массив Из СправочникСсылка.ВидыПродукцииИС - Виды продукции фильтра.
...
Функция БазовыйШаблонНормализацииКодаМаркировки(ДанныеРезультата, ПараметрыРазбораКодаМаркировки, ЗначенияЭлементовКодаМаркировки)
	...
	Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(ДанныеРезультата.ВидыПродукции[0])
		Или КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ДанныеРезультата.ВидыПродукции[0]) Тогда
	...
	ИначеЕсли КонструкторИСМПКлиентСервер.ЭтоСырьеГИСМТ(ДанныеРезультата.ВидыПродукции[0]) Тогда
	...
	ИначеЕсли ДанныеРезультата.ВидыПродукции.Количество() > 1 Тогда
		...
		Если (Булево(ПродукцияМОТП) + Булево(ПрочаяПродукцияИСМП)) > 1 Тогда
	...
	Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(ВидПродукции) Тогда
...
//  ВидыПродукции - Неопределено, СправочникСсылка.ВидыПродукцииИС, Массив Из СправочникСсылка.ВидыПродукцииИС - пользовательский отбор по виду продукции.
...
// * ВидыПродукции - Массив Из СправочникСсылка.ВидыПродукцииИС - Виды продукции текущего разбора
//
...
Функция РазобратьКодМаркировкиУпрощенноПоШаблону(ПараметрыОбработкиКодаМаркировки, ПараметрыПроверяемогоШаблона, КешНастроекРазбораКодаМаркировки)
	...
	Для Каждого ОписаниеЭлементаКМ Из ДанныеШаблона.ОписаниеЭлементовКодаМаркировки Цикл
		...
		Если ОписаниеЭлементаКМ.Имя = "МРЦСтрокой" Или ОписаниеЭлементаКМ.Имя = "ШтрихкодАкцизнойМарки" Тогда
			...
			КонецЕсли;
		ИначеЕсли ОписаниеЭлементаКМ.Имя = "SSCC"
			И Число(Прав(Значение, 1)) <> КонтрольноеЧислоSSCC(Лев(Значение, ОписаниеЭлементаКМ.Длина - 1)) Тогда
			РезультатОбработкиШаблона.ТребуетсяПолныйРазбор = Истина;
			Возврат РезультатОбработкиШаблона;
		Иначе
...
Функция ХешВидовПродукции(Значения) Экспорт
	...
	Возврат РазборКодаМаркировкиИССлужебныйВызовСервера.ХешВидовПродукции(Значения);
...
Функция РазобратьЛогистическуюУпаковкуРасширеннаяВерсия(РезультатРазбора, ПараметрыНормализации, ВидПродукции)
	...
	Если РезультатРазбора.ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC") Тогда
		...
		Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(ВидПродукции)
	...
	ИначеЕсли РезультатРазбора.ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC18") Тогда
	
		Возврат Прав(РезультатРазбора[ПараметрыНормализации.ИмяСвойстваКодМаркировки], 18);
		
	ИначеЕсли РезультатРазбора.ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC20") Тогда
