...
Процедура ОбработкаЗаполненияТТНИсходящейЕГАИС(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда
		...
		ЗаполнитьТТНИсходящуюЕГАИСНаОснованииОтгрузкиТоваровКлиенту(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
...
Процедура ПроверитьЗаполнениеЧека(Объект, Отказ) Экспорт
	...
	Запрос.УстановитьПараметр("ВидыПродукцииЕГАИСМаркируемые", КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЕГАИСМаркируемые());
	...
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЕГАИС В (&ВидыПродукцииЕГАИСМаркируемые)
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ЗаполнитьТоварыАкцизныеМарки(ДокументОбъект, ШтрихкодыУпаковок,
	...
	ШтрихкодированиеИС.ВыделитьШтрихкодыСодержащиеВидыПродукции(
		ШтрихкодыУпаковок, КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЕГАИСМаркируемые());
...
Процедура ЗаполнитьАктСписанияЕГАИСНаОснованииВнутреннегоПотребленияТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Запрос.УстановитьПараметр("ВидыПродукции",     КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЕГАИС());
...
Процедура ЗаполнитьТТНИсходящуюЕГАИСНаОснованииВозвратаТоваровПоставщику(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	ДокументОснование.Грузополучатель       КАК Грузополучатель,
...
Процедура ЗаполнитьТТНИсходящуюЕГАИСНаОснованииОтгрузкиТоваровКлиенту(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	ДокументСсылка.Организация           КАК Поставщик,
	...
	|	ДокументСсылка.СпособДоставки        КАК ТипДоставки,
	|	ДокументСсылка.Валюта                КАК Валюта
	|ИЗ
	...
	МассивСтрок.Добавить(СтрШаблон(ШаблонКурсыВалют(),                 "ВалютаОснования"));
	МассивСтрок.Добавить(СтрШаблон(ШаблонОформленныеДокументы(Истина), "ТТНИсходящаяЕГАИС"));
	МассивСтрок.Добавить(ИнтеграцияИСУТ.ТекстЗапросаОтгрузкаТоваровКлиенту());
	МассивСтрок.Добавить(";
	|ВЫБРАТЬ
	|	Товары.Номенклатура                                КАК Номенклатура,
	|	Товары.Характеристика                              КАК Характеристика,
	...
	|ПОМЕСТИТЬ НераспределенныеСерии
	|ИЗ
	|	ТаблицаТоварыОтгрузкаТоваровКлиенту КАК Т
	|	ЛЕВОЕ СОЕДИНЕНИЕ ТоварыТаблицыТовары КАК Товары
	|		ПО Товары.Номенклатура = Т.Номенклатура
	|		И Товары.Характеристика = Т.Характеристика
	|		И Товары.Серия = Т.Серия
	...
	МассивСтрок.Добавить(ШаблонСопоставление());
	МассивСтрок.Добавить(ШаблонОстатокКоличествоСумма());
	МассивСтрок.Добавить(ШаблонКодыМаркировки("ТТНИсходящаяЕГАИС", "ОтгрузкаТоваровКлиенту"));
...
Процедура ЗаполнитьОтчетПроизводстваЕГАИСНаОснованииСборкаТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЕГАИС В (&ВидыПродукцииЕГАИСНеМаркируемые)
...
Процедура ПодготовитьРезультатПроверки(РезультатПроверки)
	...
	РезультатПроверки.Вставить("ВидПродукцииИС", КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЕГАИСМаркируемые());
...
Процедура СформироватьДанныеОснованияСборкиТоваров(ДокументОснование, ДокументЕГАИС, ТаблицаДанных)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО Шапка.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЕГАИС В (&ВидыПродукцииЕГАИСНеМаркируемые)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыАлкогольнойПродукции КАК СправочникВидыАлкогольнойПродукции
	|		ПО СправочникНоменклатура.ВидАлкогольнойПродукции = СправочникВидыАлкогольнойПродукции.Ссылка
...
Функция СформироватьГиперссылкуСмТакжеВРаботеТТНВходящаяЕГАИС(Параметры)
	...
	Если Запрос.Выполнить().Пустой() Тогда
		Возврат Новый ФорматированнаяСтрока(
			ТекстГиперссылки,, ЦветаСтиля.НезаполненноеПолеТаблицы,, ИмяФормы);
	Иначе
		Возврат Новый ФорматированнаяСтрока(
			ТекстГиперссылки,,,, ИмяФормы);
	КонецЕсли;
