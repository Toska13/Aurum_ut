...
Функция СертификатОтозванИлиИстек(Протокол) Экспорт
	...
	Если Не Протокол.Контекст.РезультатПроверкиСредствамиКриптографии
		Или Не Протокол.Критерии.Общие.АккредитацияУЦДействуетНаДатуВыдачиСертификата
		Или Не Протокол.Критерии.Общие.АккредитацияУЦДействуетНаДатуПроверки Тогда
		Возврат Результат;
	КонецЕсли;
...
// Возвращаемое значение:
//  Число
Функция РассчитатьПроцентОбработанныхОбъектов(КоличествоОбработанныхОбъектов, КоличествоОбъектовКОбработке) Экспорт
	...
	Иначе
		// 5% полосы резервируем под получение данных, проценты по количеству считаем от 95.
		ПроцентВыполнения = Окр(Мин(КоличествоОбработанныхОбъектов * 95 / КоличествоОбъектовКОбработке, 95));
	КонецЕсли;
...
// в состав XML-файла доверенности, если выбрано значение вида документа "Загранпаспорт гражданина Российской Федерации".
