...
//  Подробно - Булево
//  Разделитель - Неопределено - Перенос строки.
//              - Строка
// 
// Возвращаемое значение:
//  Строка
Функция ТекстОшибокКонтекстаДиагностики(КонтекстДиагностики, Подробно = Ложь, Разделитель = Неопределено) Экспорт
	...
	МассивПредставлений = Новый Массив; // См. Массив из Строка
	
	ИмяСвойства = ?(Подробно, "ПодробноеПредставлениеОшибки", "КраткоеПредставлениеОшибки");
	
	Для Каждого Ошибка Из Ошибки Цикл
		МассивПредставлений.Добавить(Ошибка[ИмяСвойства]);
	КонецЦикла;
	...
	Возврат СтрСоединить(МассивПредставлений, Разделитель);
...
Процедура ИнициализацияСообщенийОшибок(СообщенияОшибок)
	...
	СообщенияОшибок.Вставить("3103", НСтр("ru = 'Сертификат не привязан к ID Такском. Необходимо привязать сертификат вручную на Админ.Такском.'")); // Сертификат не связан ни с одним абонентом Такском 403 3103
...
// Возвращаемое значение:
//  Булево
Функция ОшибкаСИдентификаторомЗафиксирована(Ошибки, ИдентификаторОшибки) Экспорт
	
	Результат = Ложь;
	Для Каждого Ошибка Из Ошибки Цикл
		Если Ошибка.ВидОшибки.Идентификатор = ИдентификаторОшибки Тогда
			Результат = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("АрхивСИнформациейДляТехПоддержки", Истина);
