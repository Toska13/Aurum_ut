...
Процедура ПриСозданииПодсказокФормы(Форма, ГруппаЭлементов) Экспорт
	...
	Если Не БизнесСеть.ИспользоватьОбменБизнесСеть()
		Или Не ПоказыватьРекомендации() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ОбновитьПодсказкиТорговыеПредложенияОбработчикЗадания", Истина);
КонецПроцедуры
