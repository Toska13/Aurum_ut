...
//                            - Дата - длительности срока жизни доступа к электронному документу, содержит дату и время
//                                     в часом поясе сервера. Допускается значение до 30 дней.
//   * ИнициаторПубликации - Строка - механизм инициирующий публикацию документа в сервис 1С:Share. При интеграции с другими сервиса
...
Функция РезультатФормированияURLСсылкиНаЭлектронныйДокумент(Знач ЭлектронныйДокумент, Знач ДополнительныеФайлы,
	...
	Если НастройкиПубликации <> Неопределено Тогда
		...
		Если ТипЗнч(ДлительностьСрокаЖизни) = Тип("Дата") Тогда
			СрокЖизниВМинутах = ДатаВМинуты(ДлительностьСрокаЖизни);
...
#Область ИнтеграцияПодсистемБСП
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОпубликованныеДанныеДокумента", Истина);
	Методы.Вставить("ДанныеЭлектронногоДокументаПоИдентификатору", Истина);
	Методы.Вставить("ЗагрузитьЭлектронныйДокументВОбъектУчета", Истина);
	Методы.Вставить("СоздатьВУчетеЭлектронныйДокумент", Истина);
	Методы.Вставить("ДвоичныеДанныеФайлаПоВнешнейСсылке", Истина);
...
// Вычисляется количество минут из разницы исходной даты и текущей даты сервера.
//
// Параметры:
...
//
Функция ДатаВМинуты(ИсходнаяДата)
	
	Если Не ЗначениеЗаполнено(ИсходнаяДата)
		Или ИсходнаяДата <= ТекущаяДатаСеанса() Тогда
		Возврат 0;
	КонецЕсли;
	
	Возврат Окр((НачалоМинуты(ИсходнаяДата) - НачалоМинуты(ТекущаяДатаСеанса()))/60);
