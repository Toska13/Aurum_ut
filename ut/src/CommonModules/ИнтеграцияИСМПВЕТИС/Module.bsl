...
Процедура СформироватьДанныеДокументаОснования(ДанныеОснования, ДокументОснование, ДанныеСформированы) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ВетИС") Тогда
		МодульВЕТИС = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияВЕТИС");
...
Процедура ПроверитьДанныеСтрокиПоСрокуГодностиДокументаОснования(СтрокаДанных, ПравилоПроверки, ПараметрыСканирования) Экспорт
	...
	МодульВЕТИС = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияВЕТИСКлиентСервер");
...
Процедура ДополнитьВложенныеШтрихкодыДаннымиВЕТИС(ВложенныеШтрихкоды, ПараметрыСканирования) Экспорт
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ВетИС") Тогда
		Возврат;
	ИначеЕсли Не ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗаполнятьДанныеВЕТИС(ПараметрыСканирования) Тогда
		Возврат;
...
Процедура ПолучитьСсылкуПоСтроковомуИдентификаторуПроисхождения(ИдентификаторПроисхожденияВЕТИССтрокой, ИдентификаторПроисхожденияВЕТИССсылка, Кеш) Экспорт
	...
	Модуль = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияВЕТИС");
...
Процедура ДоработатьЗапросЗаполненияЗаказаНаЭмиссиюПоМаркировке(Запрос) Экспорт
	...
	ШаблонМаркировки = "ЗНАЧЕНИЕ(Справочник.ШаблоныКодовМаркировкиСУЗ.ПустаяСсылка)";
...
Процедура ОбработкаЗаполненияМаркировкиТоваровИСМП(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если Не ДокументОбъект.ДополнительныеСвойства.Свойство("НеЗаполнятьТабличнуюЧасть") Тогда
		...
		Если Не ЗначениеЗаполнено(ДокументОбъект.ТоварнаяГруппа) Тогда
			...
			Если ТоварныеГруппы.Количество() = 1 Тогда
		...
		|ГДЕ
		|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукции)
		|СГРУППИРОВАТЬ ПО
...
// Определяет признаки "Скоропортящаяся продукция", "идентификатору происхождения ВетИС строкой". Ожидаемое поведение:
//  Скоропорт:
...
//  ЕстьКолонкаВСДСтрокой - Булево - Признак наличия колонки строкового представления идентификатора ВСД.
Процедура ДоработатьЗапросЗаполненияМаркируемойПродукцииДокументаМаркировка(Запрос, ЕстьКолонкаВСДСтрокой = Ложь) Экспорт
	...
	ПолеИдентификаторПроисхожденияВЕТИССтрокой = ТипЗнч(Запрос) = Тип("Строка");
	ПолеСкоропортящаясяПродукция = Не ПолеИдентификаторПроисхожденияВЕТИССтрокой;
	ИспользуетсяПодсистемаВетИС = ИспользуетсяПодсистемаВетИС();
	
	Если ПолеИдентификаторПроисхожденияВЕТИССтрокой Тогда
		Если ИспользуетсяПодсистемаВетИС Тогда
			ТекстЗамены = "ЕСТЬNULL(Товары.ИдентификаторПроисхожденияВЕТИС.Идентификатор, """")";
		ИначеЕсли ЕстьКолонкаВСДСтрокой Тогда
			ТекстЗамены = "Товары.ИдентификаторПроисхожденияВЕТИССтрокой";
		Иначе
			ТекстЗамены = "Товары.Ссылка.ИдентификаторПроисхожденияВЕТИССтрокой";
		КонецЕсли;
		Запрос = СтрЗаменить(Запрос, "&ИдентификаторПроисхожденияВЕТИССтрокой", ТекстЗамены);
	КонецЕсли;
	
	Если ПолеСкоропортящаясяПродукция Тогда
		Если ИспользуетсяПодсистемаВетИС Тогда
			ТекстЗамены = "ЕСТЬNULL(Товары.ИдентификаторПроисхожденияВЕТИС.СкоропортящаясяПродукция, ЛОЖЬ)";
		Иначе
			ТекстЗамены = "Товары.Ссылка.СкоропортящаясяПродукция";
		КонецЕсли;
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&СкоропортящаясяПродукция", ТекстЗамены);
	КонецЕсли;
...
Процедура ПриОпределенииТекстаЗапросаДляРасчетаСтатусаОформления(
	...
	ЧастиЗапроса.Добавить("ВЫБРАТЬ
		...
		|ПОМЕСТИТЬ %1
		|ИЗ
		|	ДанныеТаблицаТовары КАК ТаблицаТовары
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	...
	ЧастиЗапроса.Добавить("УНИЧТОЖИТЬ ДанныеТаблицаТовары");
