...
Функция ПроверитьПараметрыПодключения(
	...
	Для Каждого ОписаниеКлюча Из ПараметрыАутентификации Цикл
		Если ОписаниеКлюча.Значение <> НастройкиПлатежныхСистемКлиентСервер.УдалитьНечитаемыеСимволы(ОписаниеКлюча.Значение) Тогда
			ОписаниеКлюча.Значение = НастройкиПлатежныхСистемКлиентСервер.УдалитьНечитаемыеСимволы(
...
Функция ЗаписатьПараметрыПодключения(
	...
	Попытка
		...
		Для Каждого ПараметрАутентификации Из НастройкиЗаполнения.ПараметрыАутентификации Цикл 
			Если ПараметрАутентификации.Значение <> НастройкиПлатежныхСистемКлиентСервер.УдалитьНечитаемыеСимволы(ПараметрАутентификации.Значение) Тогда
				ПараметрАутентификации.Значение = НастройкиПлатежныхСистемКлиентСервер.УдалитьНечитаемыеСимволы(
...
// Устанавливает значение константы предопределенных шаблонов сообщений подсистемы.
//
// Параметры:
//  ДанныеШаблонов - Структура - данные предопределенных шаблонов сообщений подсистемы,
//   структурированный по сценариям перевода.
//   * c2b - Структура - перечень предопределенных шаблонов сообщений c2b сценария,
//       структурированный по типам назначения.
//     ** Ключ - Строка - тип назначения шаблона, может принимать значения - Письмо, SMS.
...
Процедура УстановитьШаблоныСообщенийПоТипам(ДанныеШаблонов) Экспорт
	...
	СистемаБыстрыхПлатежейСлужебный.УстановитьШаблоныСообщенийПоТипам(ДанныеШаблонов);
...
// Устарела. Следует использовать НастройкиПлатежныхСистем.ИзображениеQRКода.
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПараметрыПодключенияПоУчастникуСБП", Истина);
	Методы.Вставить("ПроверитьПараметрыПодключения", Истина);
	Методы.Вставить("ПолучитьУчастниковСБП", Истина);
