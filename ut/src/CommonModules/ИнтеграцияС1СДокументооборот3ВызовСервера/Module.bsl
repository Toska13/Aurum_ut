...
Функция ПодходящиеПравилаИнтеграции(СписокОбъектовИС, ТолькоПриоритетные = Ложь, СвернутьПоПравилуДО = Ложь) Экспорт
	...
	ЗапросВсеПравила = Новый Запрос(
		...
		|	ВсеПравила.ИспользоватьУсловиеПрименимостиПриВыгрузке КАК ИспользоватьУсловиеПрименимостиПриВыгрузке,
		|	ВсеПравила.УсловиеПрименимостиПриВыгрузке КАК УсловиеПрименимостиПриВыгрузке,
...
// список связей с создаваемым объектом ДО, которые автоматически формируются по данным объекта ИС; список новых
// объектов, которые были созданы в ДО в соответствии с настройками автоматического создания связей; список
// обязательных связей, которые требуется выбрать интерактивно.
//
// Параметры:
...
Функция ПредварительныеДанныеДляВыгрузкиВДО(ОбъектИС, Правило) Экспорт
	...
	Результат.Вставить("НовыеОбъектыДО", НовыеОбъектыДО);
...
// Обновляет дополнительные данные задачи, которые находятся во временном хранилище.
//
// Параметры:
//   АдресДополнительныхДанных - Строка - адрес дополнительных данных задачи во временном хранилище.
//   НоваяВерсияДанных - Строка - новая версия данных задачи.
//   НовыйРезультатДействияТекстом - Строка - новый результат действия задачи.
//
Процедура ОбновитьДанныеЗадачи(АдресДополнительныхДанных, НоваяВерсияДанных, НовыйРезультатДействияТекстом) Экспорт
	
	ДополнительныеДанныеПоЗадаче = Неопределено;
	Если ЭтоАдресВременногоХранилища(АдресДополнительныхДанных) Тогда
		ДополнительныеДанныеПоЗадаче = ПолучитьИзВременногоХранилища(АдресДополнительныхДанных); // см. ИнтеграцияС1СДокументооборот3.ДополнительныеДанныеПоЗадаче
	КонецЕсли;
	
	Если ДополнительныеДанныеПоЗадаче <> Неопределено Тогда
		ДополнительныеДанныеПоЗадаче.ОбластьДействия.РезультатДействияТекстом = НовыйРезультатДействияТекстом;
		ДополнительныеДанныеПоЗадаче.ОбластьДействия.ВерсияДанныхДействияЗадачи = НоваяВерсияДанных;
	КонецЕсли;
	
КонецПроцедуры

// Сохраняет результат действия задачи.
//
// Параметры:
//   РезультатДействияТекстом - Строка - комментарий к задаче.
//   ДействиеЗадачиID - Строка - идентификатор задачи.
//   ВерсияДанныхДействияЗадачи - Строка - текущая версия данных задачи.
//   АдресДополнительныхДанных - Строка - адрес дополнительных данных задачи во временном хранилище.
//
// Возвращаемое значение:
//   Строка - новая версия данных.
//   Неопределено - если функционал не поддерживается в текущей версии ДО.
//
Функция СохранитьРезультатДействия(РезультатДействияТекстом, ДействиеЗадачиID, ВерсияДанныхДействияЗадачи,
		АдресДополнительныхДанных) Экспорт
	
	Если Не ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("3.0.20.21") Тогда
	...
	ОбновитьДанныеЗадачи(АдресДополнительныхДанных, Ответ.newDataVersion, РезультатДействияТекстом);
	
	Возврат Ответ.newDataVersion;
...
Функция ПравилоПодходитОбъекту(ДанныеПравила, ДанныеОбъектаИС, Приоритет)
	...
	Пока ДанныеПравила.Следующий() Цикл
		Если ДанныеПравила.ИспользоватьУсловиеПрименимостиПриВыгрузке Тогда
			
			ПравилоПодходит = ИнтеграцияС1СДокументооборот3.РезультатВыраженияНаВстроенномЯзыке(
				ДанныеПравила.УсловиеПрименимостиПриВыгрузке,
				ДанныеОбъектаИС.Ссылка.ПолучитьОбъект());
			Прервать;
