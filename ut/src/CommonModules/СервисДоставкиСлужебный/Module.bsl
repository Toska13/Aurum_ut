...
Процедура ПроверитьРеквизитыАдреса(Объект, ИмяРеквизита, ЕстьОшибки, ВыводитьПредупреждения) Экспорт
	...
	Если СтрНайти(ИмяРеквизита, "ЮридическийАдрес") Тогда
		...
		Если СтрНайти(ИмяРеквизита, "Отправитель") Тогда
			ЗаголовокПоля = "Отправитель";
		ИначеЕсли СтрНайти(ИмяРеквизита, "Получатель") Тогда
			ЗаголовокПоля = "Получатель";
		ИначеЕсли СтрНайти(ИмяРеквизита, "Плательщик") Тогда
...
Процедура ПроверитьУникальныйИдентификаторАдреса(Объект, ИмяРеквизита, ЕстьОшибки, ВыводитьПредупреждения) Экспорт
	...
	Если СтруктураАдреса.addressType <> "ВСвободнойФорме" Тогда
		ИдентификаторАдреса = СтруктураАдреса.Id;
...
Процедура СообщитьОбОшибкеПодключенияКБизнесСети(Организация, Отказ = Ложь) Экспорт
	
	ТекстСообщения = НСтр(
		"ru='Организация ""%1"" не подключена в сервисе 1С:Бизнес-сеть. Необходимо подключить организацию.'");
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Организация);
	ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , , , Отказ);
	
КонецПроцедуры

// Формирует список описаний элементов классификатора Характеры груза и записывает полученные данные в параметр СохраняемыеДанные
// 
// Параметры:
//  СохраняемыеДанные - Структура - структура для сохранения полученных данных классификатора, обязательные ключи:
//   * СписокХарактеровГруза - СписокЗначений Из Структура:
//    ** Идентификатор - Строка - идентификатор элемента классификатора
//    ** Наименование - Строка - наименование элемента классификатора
//   * ОписаниеХарактеровГруза - Соответствие Из КлючИЗначение - ключами являются идентификаторы элементов классификатора 
//  ПараметрыОтбора - Структура - параметры получения кэшируемых значений:
//   * ОрганизацияБизнесСетиСсылка - ОпределяемыйТип.ОрганизацияСервисДоставки -   
//   * ТипГрузоперевозки - Число - 
//
// Возвращаемое значение:
//  СписокЗначений Из Структура- описание характеров груза:
//   * Идентификатор - Строка - идентификатор элемента классификатора
//   * Наименование - Строка - строковое представление элемента классификатора
Функция СписокКлассификатораХарактерыГруза(СохраняемыеДанные, ПараметрыОтбора) Экспорт
	
	СписокКлассификатора = Новый СписокЗначений;
	Если ЗначениеЗаполнено(СохраняемыеДанные.СписокХарактеровГруза) Тогда
		СписокКлассификатора = СохраняемыеДанные.СписокХарактеровГруза;

	Иначе

		КэшГрузы = СервисДоставки.ПолучитьКэшированныеЗначения(
			ПараметрыОтбора.ОрганизацияБизнесСетиСсылка, ПараметрыОтбора.ТипГрузоперевозки, "Грузы");
		
		Если ЗначениеЗаполнено(КэшГрузы) Тогда
			ДанныеКэша = КэшГрузы[СервисДоставкиКлиентСервер.ИмяМетодаПолучитьХарактерыГруза()]; // см. СервисДоставки.ПолучитьХарактерыГруза
			Для Каждого СтрокаДанных Из ДанныеКэша.Список Цикл
				ЗначениеСписка = Новый Структура("Идентификатор, Наименование", СтрокаДанных.Идентификатор,
					СтрокаДанных.Наименование);
				СписокКлассификатора.Добавить(ЗначениеСписка, СтрокаДанных.Наименование);
			КонецЦикла;

		Иначе
			СписокКлассификатора = ПолучитьХарактерыГрузаИзСервиса(ПараметрыОтбора.ТипГрузоперевозки, 
				ПараметрыОтбора.ОрганизацияБизнесСетиСсылка);
		КонецЕсли;

		Для Каждого ЭлементСписка Из СписокКлассификатора Цикл
			СохраняемыеДанные.ОписаниеХарактеровГруза.Вставить(ЭлементСписка.Значение.Идентификатор, 
				ЭлементСписка.Представление);
		КонецЦикла;
		
		СохраняемыеДанные.Вставить("СписокХарактеровГруза", СписокКлассификатора);
	КонецЕсли;
	
	Возврат СписокКлассификатора;
КонецФункции

// Возвращает структуру, содержащую данные проверки по применимости способа доставки из основания и количество уже оформленных Заказов на доставку
// 
// Параметры:
//  ДокументыОснования - Массив Из ОпределяемыйТип.ОснованиеЗаказаСервисДоставки
//	ПараметрыПроверки - Структура - содержит ключи:
//	 * ПроверитьСпособДоставки - Булево - Истина, если ДокументОснование - Реализация товаров и услуг
//	 * ПолучитьКоличествоЗаказов - Булево - 
// 
// Возвращаемое значение:
//  Структура - Данные проверки основания заказа на доставку:
// * СпособДоставкиПрименим - Булево - Ложь, если параметры доставки не отображаются на форме при выбранном способе доставки 
// * КоличествоЗаказовПоОснованиям - СписокЗначений Из Число -
// * ТекстПояснения - Строка
Функция ДанныеПроверкиОснованияЗаказаНаДоставку(ДокументыОснования, ПараметрыПроверки) Экспорт
	Результат = Новый Структура("СпособДоставкиПрименим, КоличествоЗаказовПоОснованиям, ТекстПояснения",
		Истина, Новый СписокЗначений, "");
	Если ПараметрыПроверки.ПроверитьСпособДоставки Тогда
		СервисДоставкиПереопределяемый.ПроверитьСпособДоставкиОснования(ДокументыОснования, Результат);
	КонецЕсли;
	Если ПараметрыПроверки.ПолучитьКоличествоЗаказов Тогда
		Результат.КоличествоЗаказовПоОснованиям = 
			РегистрыСведений.ДанныеЗаказовСервисДоставки.КоличествоЗаказовПоОснованиям(ДокументыОснования);
	КонецЕсли;
	
	Возврат Результат;
КонецФункции

// Для переданного документа формирует массив, состоящий самого документа, связанного я ним Заказа клиента
// и связанных с Заказом клиента документов Реализация товаров и услуг.
// 
// Параметры:
...
Функция ДокументыОснованияДляПолученияЗаказаНаДоставку(ДокументОснование) Экспорт
	...
	ТекстыЗапросов.Добавить(ТекстЗапросаСвязанныеДокументы());
	ТекстыЗапросов.Добавить("ВЫБРАТЬ
	|	ВТСвязанныеДокументы.Ссылка КАК Ссылка
	|ИЗ
	...
	ОбщегоНазначенияКлиентСервер.
		ДополнитьМассив(ДокументыОснования, Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка"), Истина);
	
	Возврат ДокументыОснования;
КонецФункции

// См. БизнесСеть.ДоступныеПодключенныеОрганизации
...
Процедура ПроверитьРегистрациюОрганизации(Организация, Отказ)
	...
	Если Не ОрганизацияПодключена(Организация) Тогда
		СообщитьОбОшибкеПодключенияКБизнесСети(Организация, Отказ);
	КонецЕсли;
...
Функция ПолучитьХарактерыГрузаИзСервиса(ТипГрузоперевозки, ОрганизацияБизнесСетиСсылка)
	Результат = Новый СписокЗначений;
	
	ПараметрыЗапроса = СервисДоставки.НовыйПараметрыЗапросаПолучитьХарактерыГруза(); 
	ПараметрыЗапроса.ОрганизацияБизнесСетиСсылка = ОрганизацияБизнесСетиСсылка;
	ПараметрыЗапроса.ТипГрузоперевозки = ТипГрузоперевозки;
	
	РезультатЗапроса = СервисДоставки.ПолучитьХарактерыГруза(ПараметрыЗапроса);
	...
	Для Каждого СтрокаДанных Из РезультатЗапроса.Список Цикл
		ЗначениеСписка = Новый Структура("Идентификатор, Наименование");
		ЗаполнитьЗначенияСвойств(ЗначениеСписка, СтрокаДанных);
		Результат.Добавить(ЗначениеСписка, СтрокаДанных.Наименование);
	КонецЦикла;
...
// Текст запроса получения связанных документов с типом ЗаказКлиента и РеализацияТоваровУслуг
// 
// Возвращаемое значение:
//  Строка - Текст запроса заказ и связанные документы
Функция ТекстЗапросаСвязанныеДокументы() Экспорт
	Возврат "ВЫБРАТЬ
	|	СвязанныеДокументы.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ВТСвязанныеДокументы
	|ИЗ
	|	КритерийОтбора.СвязанныеДокументы(&ЗначениеОтбора) КАК СвязанныеДокументы
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(СвязанныеДокументы.Ссылка) В (&ТипыСвязанныхДокументовОснований)
	|	И &ПолучитьСвязанныеДокументы";
КонецФункции
