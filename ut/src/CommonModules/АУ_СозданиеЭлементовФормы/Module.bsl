...
Процедура СоздатьРеквизиты(Форма,структураРеквизитов) Экспорт
	...
	Для Каждого тРеквизит Из структураРеквизитов Цикл
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, тРеквизит.Ключ) Тогда
			Продолжить;
		КонецЕсли;
...
Функция СоздатьПолеФлажок(Форма,
	...
	НовыйЭлемент = СоздатьПоле(Форма,ИмяПоля,РодительПоля,Заголовок,2,ПутьКДанным,СтруктураСвойств,СтруктураСобытий,ЭлементПеред);
...
Функция СоздатьДинамическийСписок(Форма,
	...
	ТаблицаФормы = СоздатьТаблицаФормы(Форма,ИмяСписка,РодительСписка,ИмяСписка,СтруктураКолонок,СтруктураСвойств,СтруктураСобытий,СвояКоманднаяПанель,ЭлементПеред); 
...
Функция СоздатьТаблицаФормы(Форма,
	...
	Если СвояКоманднаяПанель Тогда
		ГруппаДинамическогоСписка = СоздатьГруппаОбычнаяБезОтображения(Форма,"ДополнительнаяГруппаКоманднаяПанель" + ИмяТаблицы,РодительТаблицы,1,1,1,,ЭлементПеред);
		КоманднаяПанельСписка  = СоздатьГруппаКоманднаяПанель(Форма,ИмяТаблицы + "КоманднаяПанель2",ГруппаДинамическогоСписка);
...
Функция СоздатьВыпадающийСписок(Форма,
	...
	НовыйЭлемент = СоздатьПоле(Форма,ИмяСписка,РодительСписка,ЗаголовокЭлемента,1,ИмяСписка,СтруктураСвойств,СтруктураСобытий,ЭлементПеред);
...
Функция СоздатьПереключатель(Форма,
	...
	НовыйЭлемент = СоздатьПоле(Форма,ИмяПереключателя,РодительЭлемента,ЗаголовокЭлемента,4,ИмяПереключателя,СтруктураСвойств,СтруктураСобытий,ЭлементПеред);
