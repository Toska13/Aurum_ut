...
Функция ПолучитьОбщиеМакетыИнформационныхСсылок() Экспорт
	...
	// Локализация
	Если ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени("ОбщийМакет.ИнформационныеСсылки_Общие") <> Неопределено Тогда
...
//  Структура - с полями:
//  	* КодВерный - Булево
//  	* ТекстСообщения - Строка - заполняется, если код некорректен
//
...
Процедура СформироватьГруппыВывода(Форма, ТаблицаСсылок, ГруппаФормы, КоличествоГрупп, КоличествоСсылокВГруппе, 
	...
	// Локализация
	СайтыСАвторизацией = СайтыСДополнительнойАвторизацией();
	// Конец Локализация
	...
	Для Итерация = 1 По КоличествоСсылок Цикл 
		...
		ИмяЭлементы = "ЭлементСсылки";		
		// Локализация
		Для каждого Сайт Из СайтыСАвторизацией Цикл
			Если СтрНайти(Адрес, Сайт) > 0 Тогда
				ИмяЭлементы = "_ЭлементСсылки";	
				Прервать;		
			КонецЕсли;
		КонецЦикла;
		// Конец Локализация
		...
		ЭлементСсылки = 
			Форма.Элементы.Добавить(ИмяЭлементы + Формат(Итерация, "ЧГ=;"), Тип("ДекорацияФормы"), ГруппаСсылки);
...
// Локализация
Функция СайтыСДополнительнойАвторизацией()
	
	Сайты = Новый Массив();
	
	Сайты.Добавить("its.1c.ru");
	Сайты.Добавить("portal.1c.ru");	
	Сайты.Добавить("releases.1c.ru");
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОпределитьСостояниеПодключенияВФоне", Истина);
	Методы.Вставить("ЧтениеНовостейСлужбыПоддержки", Истина);
