...
Процедура ЗагрузитьМЧДИзФайла(АдресХранилища) Экспорт
	...
	ДополнительныеПараметры.Вставить("ВариантПроверкиСертификатовМЧДЭДО", 
		РезультатЗагрузкиМЧД.ВариантПроверкиСертификатовМЧДЭДО);
...
Процедура ПроверитьПодписиДоверенностей(Доверенности, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(Доверенности) Тогда
		...
		ВсегоДоверенностей = Доверенности.Количество() + Параметры.ДоверенностиПроверенные.Количество();
		
		ПроцентПрогресса = 
			МашиночитаемыеДоверенностиКлиентСервер.РассчитатьПроцентОбработанныхОбъектов(
				Параметры.ДоверенностиПроверенные.Количество(), ВсегоДоверенностей);
			
		Параметры.КонтекстОжиданияОперации.ПараметрыОжиданияОперации.ПроцентПрогресса = ПроцентПрогресса;
		
		ОжиданиеОперацийБЭДКлиент.ОбновитьПрогресс(
			Параметры.КонтекстОжиданияОперации.ПараметрыОжиданияОперации, Параметры.КонтекстОжиданияОперации);
...
Процедура ЗагрузитьПерезаполнитьМЧДИзФайлаЗавершение(РезультатПроверки, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(РезультатПроверки) Тогда
		...
		ОбрабатыватьОшибкиЗагрузкиМЧД = Истина;
		
		Если ДополнительныеПараметры.Свойство("ВариантПроверкиСертификатовМЧДЭДО")
			И ДополнительныеПараметры.ВариантПроверкиСертификатовМЧДЭДО = 
				ПредопределенноеЗначение("Перечисление.ВариантыПроверкиСертификатовМЧДЭДО.НеПроверятьПоСрокуДействия") Тогда
				
			ПротоколПроверкиПодписиМЧД = РезультатПроверки.РезультатПроверкиПодписиМЧД.СвойстваПодписи.Протокол;
			Если ПротоколПроверкиПодписиМЧД <> Неопределено
				...
				ОбрабатыватьОшибкиЗагрузкиМЧД = Ложь;
			КонецЕсли;
		КонецЕсли;
		
		Если ОбрабатыватьОшибкиЗагрузкиМЧД Тогда
			КонтекстДиагностики = РезультатПроверки.КонтекстДиагностики;
			ОбработатьРезультатПроверкиМЧД(РезультатПроверки);
		КонецЕсли;
