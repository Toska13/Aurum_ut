...
Функция ПолучитьТекстЗапросаЦены() Экспорт
	...
	Если ЦенообразованиеВызовСервера.ИспользуетсяЦенообразование25() Тогда
		...
		|	втНоменклатура.Номенклатура.ПометкаУдаления КАК НоменклатураПометкаУдаления,
		...
		|	1,
		|	ЕСТЬNULL(ЕдиницыИзмерения.Код, """"),
		|	втНоменклатура.Номенклатура.ПометкаУдаления,
...
Процедура ПолучитьТекстыЗапросовКаталог(ТекстыЗапросов) Экспорт
	...
	|	ЕСТЬNULL(СтавкиНДСНоменклатуры.СтавкаНДС, НоменклатураСправочник.СтавкаНДС) КАК НоменклатураСтавкаНДСЗначение,
	|	ВЫБОР
	...
	|			Номенклатура В (ВЫБРАТЬ ТаблицаНоменклатуры.Номенклатура ИЗ втНоменклатура КАК ТаблицаНоменклатуры) 
	|			И Страна = &СтранаРегистрации ИЛИ Страна = ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)) КАК СтавкиНДСНоменклатуры
	|		ПО (втНоменклатура.Номенклатура = СтавкиНДСНоменклатуры.Номенклатура)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ЕСТЬNULL(СтавкиНДСНоменклатуры.СтавкаНДС, НоменклатураСправочник.СтавкаНДС),
	|	ВЫБОР
	|		КОГДА &КлассифицироватьПоВиду
	|			ТОГДА НоменклатураСправочник.ВидНоменклатуры
	|		ИНАЧЕ НоменклатураСправочник.Родитель
...
Процедура УстановитьПараметрыЗапросаКаталог(ПрикладныеПараметры, ПараметрыКомпоновки) Экспорт
	...
	ОбменССайтамиУТ.УстановитьПараметрКомпоновки(ПараметрыКомпоновки, "Период", ТекущаяДатаСеанса());
	ОбменССайтамиУТ.УстановитьПараметрКомпоновки(ПараметрыКомпоновки, "СтранаРегистрации", ЗначениеНастроекКлиентСерверПовтИсп.СтранаРегистрацииОрганизации(ПрикладныеПараметры.Организация));
	//-- НЕ ГОСИС
...
Процедура ПолучитьТекстЗапросаЗаказы(ТекстЗапроса) Экспорт
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РаспоряженияНаОтгрузкуИВозврат КАК Оформлено
	|			ПО ЗаказыДляВыгрузки.Заказ = Оформлено.Распоряжение
	|			 И Оформлено.Показатель = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Оформлено)
	|	
	|	СГРУППИРОВАТЬ ПО
...
Процедура ЗарегистрироватьИзмененияВУзлах(Объект, МассивУзловТовары, МассивУзловЗаказы, Замещение = Ложь) Экспорт
	...
	Если ТипОбъекта = Тип("РегистрСведенийНаборЗаписей.ЦеныНоменклатуры")
		...
		ИЛИ ТипОбъекта = Тип("РегистрСведенийНаборЗаписей.ДоступныеОстаткиТоваров") Тогда
		...
		Если Замещение Тогда
			...
			Для каждого Запись Из СтарыйНаборЗаписей Цикл
				...
				Если ТипОбъекта = Тип("РегистрСведенийНаборЗаписей.ДоступныеОстаткиТоваров") Тогда
					
					Если Не (Запись.Назначение.Пустая() И Запись.КонтролируемаяПартия.Пустая()) Тогда
						Продолжить;
					КонецЕсли;
				КонецЕсли;
...
Процедура ОтменитьЗаказ(ПрикладныеПараметры, ДокументСсылка) Экспорт
	...
	Если ЕстьИзменения Тогда
		ЭтапыОплатыСервер.ПроверитьЗаполнитьЭтапыГрафикаОплаты(ОбъектЗаказ);
...
Функция ТекстВтДоступныеТовары()
	...
	ТекстЗапроса =
		"ВЫБРАТЬ
		|	ДоступныеОстаткиТоваров.Номенклатура КАК Номенклатура,
		|	ДоступныеОстаткиТоваров.Характеристика КАК Характеристика,
		|	ДоступныеОстаткиТоваров.Склад КАК Склад,
		|	ДоступныеОстаткиТоваров.Доступно КАК Свободно
		|ПОМЕСТИТЬ ВтДоступныеТовары
		|ИЗ
		|	РегистрСведений.ДоступныеОстаткиТоваров КАК ДоступныеОстаткиТоваров
		|ГДЕ
		|	ДоступныеОстаткиТоваров.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|		И ДоступныеОстаткиТоваров.КонтролируемаяПартия = ЗНАЧЕНИЕ(Справочник.КонтролируемыеПартии.ПустаяСсылка)
		|		И ДоступныеОстаткиТоваров.Доступно <> 0
		|ИНДЕКСИРОВАТЬ ПО
