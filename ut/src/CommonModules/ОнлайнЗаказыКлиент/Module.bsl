...
// Возвращает способ оплаты, соответствующий Яндекс Пэй
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.СпособыОплатыОнлайнЗаказов -
//  
Функция СпособОплатыЯндексПэй() Экспорт
	
	Возврат ПредопределенноеЗначение("Перечисление.СпособыОплатыОнлайнЗаказов.ЯндексПэй");
...
//  Владелец - ФормаКлиентскогоПриложения - форма которая будет установлена в качестве владельца.
//
Процедура ОткрытьФормуURLЗаказа(Знач ДокументЗаказа, Знач Владелец)
...
Процедура ОткрытьФормуURLЗаказаПродолжение(
	...
	ИначеЕсли ПараметрыСозданияЗаказа.НастройкаОнлайнЗаказовДоступна
		...
		Иначе
			...
			ПараметрыОткрытия.Вставить("Владелец", Параметры.Владелец);
