...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	РеквизитыПлательщика.Вставить("Объект", Объект.БанковскийСчет);
	РеквизитыПлательщика.Вставить("ЮрФизЛицо", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.БанковскийСчет, "Владелец.ЮрФизЛицо"));
	...
	Если Объект.ПеречислениеВБюджет Тогда
		ПлатежиВБюджет.ОшибкиЗаполненияРеквизитовПлательщика(Объект, РеквизитыПлательщика, Не ФлагОбменСБанками, Отказ);
		ПлатежиВБюджет.ОшибкиЗаполненияРеквизитовПолучателя(Объект, РеквизитыПолучателя, Не ФлагОбменСБанками, Отказ);
		ДенежныеСредстваСерверЛокализация.ПроверитьЗаполнениеНалоговыхРеквизитов(Объект, Отказ, МассивНепроверяемыхРеквизитов, ФлагОбменСБанками);
	Иначе
		
		ДенежныеСредстваСерверЛокализация.ОшибкиЗаполненияРеквизитовПлательщикаПолучателя(Истина, Объект, РеквизитыПлательщика, Не ФлагОбменСБанками, Отказ);
		ДенежныеСредстваСерверЛокализация.ОшибкиЗаполненияРеквизитовПлательщикаПолучателя(Ложь, Объект, РеквизитыПолучателя, Не ФлагОбменСБанками, Отказ);
...
Процедура ОбработкаЗаполнения(Объект, ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ДокументОПредстоящейПоставке") Тогда
			
		ЗаполнитьПоДОПП(Объект, ДанныеЗаполнения, ДанныеЗаполнения);
	...
	ИначеЕсли ТипОснования = Тип("Структура") Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьВыплатыСамозанятым") Тогда
			...
			Если ДанныеЗаполнения.Свойство("Договор")
				...
				И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеЗаполнения.Договор, "ДоговорССамозанятым")
...
Процедура ПередЗаписью(Объект, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	ПроверитьПлатежныеСсылкиЗаявок(Объект, Отказ);
...
// Заполняет реквизиты плательщика объекта на основе данных заполнения.
//
// Параметры:
//   Объект - ДокументОбъект.СписаниеБезналичныхДенежныхСредств - объект документа, в который заполняются реквизиты
//   ДанныеЗаполнения - Структура - структура с данными заполнения, содержащая свойства ИННПлательщика, КПППлательщика, ТекстПлательщика
Процедура ОбработатьЗаполнениеРеквизитовПлательщика(Объект, ДанныеЗаполнения) Экспорт

	//++ Локализация
	НаДату = ?(Объект.ПроведеноБанком, Объект.ДатаПроведенияБанком, ТекущаяДатаСеанса());
	РеквизитыПлательщика = ДенежныеСредстваСерверЛокализация.РеквизитыПлательщика(Объект, НаДату);
...
Функция СформироватьПлатежноеПоручение(МассивОбъектов, ОбъектыПечати)
	...
	|	ЕСТЬNULL(БанковскиеСчетаКонтрагентов.НомерСчетаВладельца, """") КАК НомерСчетаВладельцаПолучателя,
	...
	|	ЕСТЬNULL(БанковскиеСчетаКонтрагентов.ЭтоБанкДляРасчетов, ЛОЖЬ) КАК ЭтоБанкДляРасчетов,
	|
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			Если ПоддержкаПлатежейРФ
				Или Не ПустаяСтрока(Выборка.ТекстПлательщика) Тогда
			...
			Иначе
				...
				Иначе
					...
					ПрямыеРасчеты = Не ЗначениеЗаполнено(Выборка.БанкДляРасчетов);
					ТекстПлательщика = ДенежныеСредстваСервер.НаименованиеПлательщикаПолучателяПлатежа(
						ТекстПлательщика, ПрямыеРасчеты, Выборка.НомерСчета, Строка(Выборка.Банк), Выборка.Город);
				КонецЕсли;
			КонецЕсли;
			...
			ИначеЕсли ТипЗнч(Выборка.Получатель) = Тип("СправочникСсылка.Организации") Тогда
				...
				ТекстПолучателя = ДенежныеСредстваСервер.НаименованиеПлательщикаПолучателяПлатежа(
					ТекстПолучателя, 
					Не Выборка.ЭтоБанкДляРасчетов,
					Выборка.НомерСчетаВладельцаПолучателя,
					Выборка.НаименованиеБанкаПолучателя,
					Выборка.ГородБанкаПолучателя);
			...
			ОбластьМакета.Параметры.НаименованиеБанкаПлательщика = 
				КлиентБанкСервер.НаименованиеБанкаПолучателяПлатежа(
					Выборка.НаименованиеБанкаПлательщика, Выборка.НаименованиеРКЦПлательщика) 
			...
			Если Не Выборка.СчетЦифровогоРубляПолучателя Тогда
				ОбластьМакета.Параметры.НаименованиеБанкаПолучателя =
					КлиентБанкСервер.НаименованиеБанкаПолучателяПлатежа(
						Выборка.НаименованиеБанкаПолучателя, Выборка.НаименованиеРКЦ)
					+ " " + Выборка.ГородБанкаПолучателя;
			КонецЕсли;
			...
			Если ПоддержкаПлатежейРФ Тогда
				КПППлательщика = ?(Выборка.КПППлательщика = "", "0", Выборка.КПППлательщика);
			Иначе
			...
			ОбластьМакета.Параметры.КПППлательщика = "КПП " + КПППлательщика;
...
// Дополнить виды налогов
// 
// Параметры:
//  ВидыНалогов - СписокЗначений из СправочникСсылка.ВидыНалоговВзносов.
Процедура ДополнитьВидыНалогов(ВидыНалогов) Экспорт
...
Процедура ЗаполнитьПоДОПП(Объект, Знач ДокументОснование, ДанныеЗаполнения)
	
	Запрос = Новый Запрос("
	|ВЫБРАТЬ
	|	ДанныеДокумента.Организация    КАК Организация,
	|	&ТекущаяДата                   КАК ЖелательнаяДатаПлатежа,
	|	ДанныеДокумента.СуммаОбеспечительногоПлатежа КАК СуммаОП,
	|	&Ссылка                        КАК ДокументОснование,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПеречислениеВБюджет) КАК ХозяйственнаяОперация
	|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрока = Объект.РасшифровкаПлатежа.Добавить();
		НоваяСтрока.ВалютаВзаиморасчетов = ВалютаРегламентированногоУчета;
		НоваяСтрока.Сумма = Выборка.СуммаОП;
		НоваяСтрока.СуммаВзаиморасчетов = Выборка.СуммаОП;
	КонецЦикла;
...
Процедура ИнициализироватьДокумент(Объект, ДанныеЗаполнения = Неопределено)
	...
	Если ДанныеЗаполнения <> Неопределено
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеЗаполнения, "ПеречислениеВБюджет") Тогда
...
Функция ПодобратьРеквизитыПлатежаВБюджет(Объект, ИсключаяРеквизиты = "") Экспорт
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(БанковскиеСчетаКонтрагентов.Закрыт, ЛОЖЬ)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаКонтрагентов.ПустаяСсылка)
...
Процедура ЗаполнитьИменаРеквизитовПоХозяйственнойОперации(Объект, МассивВсехРеквизитов, МассивРеквизитовОперации)
	...
	МассивВсехРеквизитов.Добавить("ТипПлатежаФЗ275");
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаПоставщику Тогда
		...
		МассивРеквизитовОперации.Добавить("ТипПлатежаФЗ275");
...
Процедура ПроверитьПлатежныеСсылкиЗаявок(Объект, Отказ)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ЗаявкаНаРасходованиеДенежныхСредств.ПлатежнаяСсылкаСБП КАК ПлатежнаяСсылкаСБП
		|ПОМЕСТИТЬ ПлатежныеСсылкиЗаявок
		|ИЗ
		|	Документ.ЗаявкаНаРасходованиеДенежныхСредств КАК ЗаявкаНаРасходованиеДенежныхСредств
		|ГДЕ
		|	ЗаявкаНаРасходованиеДенежныхСредств.ПлатежнаяСсылкаСБП <> """"
		|	И ЗаявкаНаРасходованиеДенежныхСредств.Ссылка В(&Заявки)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	КОЛИЧЕСТВО(ПлатежныеСсылкиЗаявок.ПлатежнаяСсылкаСБП) КАК КоличествоПлатежныхСсылок
		|ИЗ
		|	ПлатежныеСсылкиЗаявок КАК ПлатежныеСсылкиЗаявок
		|
		|ИМЕЮЩИЕ
		|	КОЛИЧЕСТВО(ПлатежныеСсылкиЗаявок.ПлатежнаяСсылкаСБП) > 1";
	
	РасшифровкаПлатежа = Объект.РасшифровкаПлатежа.Выгрузить();
	СписокЗаявок = РасшифровкаПлатежа.ВыгрузитьКолонку("ЗаявкаНаРасходованиеДенежныхСредств");
	
	Запрос.УстановитьПараметр("Заявки", СписокЗаявок);
	
	Если Не Запрос.Выполнить().Пустой() Тогда
		
		ТекстСообщения = НСтр("ru = 'Различные платежные ссылки СБП в заявках'");
...
// Возвращает данные о получателе для указанной хозяйственной операции.
//
// Параметры:
//   Объект - ДокументОбъект.СписаниеБезналичныхДенежныхСредств, ДанныеФормыСтруктура - документ, содержащий данные получателя для хозяйственной операции
//
// Возвращаемое значение:
//   Структура:
//     * Получатель - СправочникСсылка.Контрагенты, СправочникСсылка.ФизическиеЛица, СправочникСсылка.Организации, Неопределено - получатель денежных средств
//     * СчетПолучателя - СправочникСсылка.БанковскиеСчетаКонтрагентов, СправочникСсылка.БанковскиеСчетаОрганизаций, Неопределено - банковский счет получателя
//     * ПравоСубъект - ПеречислениеСсылка.ЮрФизЛицо - тип субъекта (юридическое или физическое лицо)
//
Функция ДанныеПолучателяХозяйственнойОперации(Объект) Экспорт
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаДенежныхСредствВДругуюОрганизацию
		...
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.БанковскийСчетПолучатель, "Владелец, Владелец.ЮрФизЛицо");
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СнятиеНаличныхДенежныхСредств Тогда
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.КассаПолучатель, "Владелец, Владелец.ЮрФизЛицо");
		Результат.Получатель = ЗначенияРеквизитов.Владелец;
		Результат.ПравоСубъект = ЗначенияРеквизитов.ВладелецЮрФизЛицо;
		Результат.СчетПолучателя = Неопределено;
