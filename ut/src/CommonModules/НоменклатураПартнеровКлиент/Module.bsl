...
//  ДоступностьОтборов                - Булево                      - признак, что Отборы доступны на форме.
//
Процедура ОткрытьФормуВыбораНоменклатурыПартнера(ВладелецФормы,
												Партнер,
												ТекущаяСтрока,
												ПредставлениеНоменклатурыПартнера,
												ОповещениеОЗакрытие,
												АктВыполненныхРабот = Ложь,
												ДоступностьОтборов = Истина) Экспорт
	...
	Если ТипЗнч(ТекущаяСтрока) = Тип("ДанныеФормыЭлементДерева")
		И ТекущаяСтрока.ПолучитьРодителя() = Неопределено Тогда
		ПараметрыОтбора.Вставить("ИспользоватьХарактеристики", Ложь);
	КонецЕсли;
	...
	СтруктураПараметров.Вставить("ДоступностьОтборов" , ДоступностьОтборов);
...
//  ТекущиеДанные	- ДанныеФормыЭлементКоллекции	- текущая строка.
//
Процедура УстановитьПараметрыВыбораЭлементаФормы(ПараметрыВыбора, Партнер, ТекущиеДанные) Экспорт
	...
	НовыеПараметрыВыбора = Новый Массив;
	НовыеПараметрыВыбора.Добавить(Новый ПараметрВыбора("Отбор.Владелец", Партнер));
	НовыеПараметрыВыбора.Добавить(Новый ПараметрВыбора("Отбор.Номенклатура", ТекущиеДанные.Номенклатура));
	НовыеПараметрыВыбора.Добавить(Новый ПараметрВыбора("Отбор.Характеристика", ТекущиеДанные.Характеристика));
	
	ЕдиницаИзмерения = ЗначениеЕдиницыИзмеренияПоСтрока(ТекущиеДанные);
	НовыеПараметрыВыбора.Добавить(Новый ПараметрВыбора("Отбор.Упаковка", ЕдиницаИзмерения));
