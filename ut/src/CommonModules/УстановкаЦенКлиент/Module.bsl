...
Функция ПараметрыИзмененияВЭлементеДереваЦен(ВариантВыполнения) Экспорт
	...
	ИначеЕсли ВариантВыполнения = "ИзменитьУпаковкуВСтрокеДереваЦен"
			Или ПараметрыВызова.ВариантВыполнения = "ИзменитьВалютуВСтрокеДереваЦен"
			Или ПараметрыВызова.ВариантВыполнения = "ИзменитьСтавкуНДСВСтрокеДереваЦен" Тогда
...
Процедура ПрименитьИзмененияВДеревеЦен(ЭлементДереваЦен, ПараметрыВызова) Экспорт
	...
	Для Каждого ЭлементДереваЦенТекущий Из ЭлементыДереваЦенТекущие Цикл
		...
		ИначеЕсли ПараметрыВызова.ВариантВыполнения = "ИзменитьСтавкуНДСВСтрокеДереваЦен" Тогда
			
			ИмяТекущейКолонки 	= ПараметрыВызова.ИмяТекущейКолонки;
			ТекущееЗначение 	= ПараметрыВызова.ТекущееЗначение;
			ЭлементДереваЦенТекущий[ИмяТекущейКолонки] = ТекущееЗначение;
			
			ЭлементДереваЦенТекущий["ИзмененаВручную" + ИмяТекущейКолонки] = 
					ЭлементДереваЦенТекущий[ИмяТекущейКолонки] <> ЭлементДереваЦенТекущий["Исходная" + ИмяТекущейКолонки];
			ЭлементДереваЦенТекущий["ИзмененаАвтоматически" + ИмяТекущейКолонки] = Ложь;
...
Процедура ОткрытьФормуЗаполненияСтавокНДС(Форма, Дата) Экспорт
	
	ОчиститьСообщения();
	
	Элементы = Форма["Элементы"];
	
	ИмяПроцедурыЗавершения = "НастройкиЗаполненияСтавокНДСЗавершение";
	...
	Для Каждого СтрокаВидыЦены ИЗ Форма.ВыбранныеЦены Цикл
		...
		Если СтрокаВидыЦены.ВариантУказанияСтавокНДС =
				ПредопределенноеЗначение("Перечисление.ВариантыУказанияСтавокНДСВПрайсЛисте.УказыватьСтавкиНДСПострочно") Тогда
			
			МассивВидовЦен.Добавить(Новый Структура("ВидЦены, Страна",	СтрокаВидыЦены.Ссылка, 
																		СтрокаВидыЦены.Страна));
		КонецЕсли;
			
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(МассивВидовЦен) Тогда
		ТекстСообщения = НСтр("ru = 'В прайс-листе отсутствуют виды цен с построчным заполнением ""Ставки НДС.""'");
	...
	ПараметрыОткрытия.Вставить("ТолькоВыделенные", ТолькоВыделенные);
	ПараметрыОткрытия.Вставить("ДатаДействия", Дата);
	
	ОткрытьФорму(
		"Обработка.ПрайсЛист.Форма.ФормаНастройкиЗаполненияСтавокНДС",
		ПараметрыОткрытия,
		ЭтотОбъект,,,, Новый ОписаниеОповещения(ИмяПроцедурыЗавершения, Форма), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
