...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьКешиОператоровЭДОИФорматов", Истина);
	Методы.Вставить("ОбновитьПоддерживаемыеФорматыАбонентовИзСервисаНастроек", Истина);
	Методы.Вставить("ПолучитьПриглашения", Истина);
	Методы.Вставить("СинхронизироватьКлассификаторПолномочий_МЧД003", Истина);
	Методы.Вставить("СинхронизироватьКлассификаторПолномочийФНС_МЧД002", Истина);
	Методы.Вставить("СинхронизироватьКлассификаторыОграниченийМЧД", Истина);
	Методы.Вставить("СинхронизироватьСопоставленияКлассификатораОграниченийСОбъектамиБЭД", Истина);
...
Функция ОтправитьОбъекты(ЗапросОбъектов, КонтекстДиагностики, ОтпечаткиСертификатов, ВыбранныеСертификаты = Неопределено,
	...
	ПараметрыПереводаИзТакском = ПараметрыПереводаИзТакском();
	ОбрабатыватьТолькоОбъектыAPIТакском = ТипЗнч(ДополнительныеПараметры) = Тип("Структура")
		И ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеПараметры.ПараметрыВыполнения, "ВыполнитьТолькоПоТакском", Ложь);
	...
	Если ОбрабатыватьТолькоОбъектыAPIТакском Тогда
		Отбор.СпособОбмена = "&СпособыОбмена";
	КонецЕсли;
	
	СпособОбменаЧерезОператораЭДОТакском = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском;
	...
	|	КонтейнерыОбъектов.Объект.ТипЭлементаРегламента КАК ТипЭлементаРегламента, 
	|	КонтейнерыОбъектов.Объект.ЭлектронныйДокумент КАК ЭлектронныйДокумент
	|ИЗ
	...
	Если ОбрабатыватьТолькоОбъектыAPIТакском Тогда
		ИтоговыйЗапрос.УстановитьПараметр("СпособыОбмена", СпособОбменаЧерезОператораЭДОТакском);
		ПараметрыПереводаИзТакском.СпособОбменаДляПодмены = СпособОбменаЧерезОператораЭДОТакском;
	КонецЕсли;
	...
	Пока ВыборкаТранспортныйКонтейнер.Следующий() Цикл
		...
		ЭтоКонтейнерУчетнойЗаписиПереведеннойИзТакском = Ложь;
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			ЭтоКонтейнерУчетнойЗаписиПереведеннойИзТакском = Не ОбрабатыватьТолькоОбъектыAPIТакском 
			И ВыборкаДетальныеЗаписи.СпособОбмена = СпособОбменаЧерезОператораЭДОТакском
			И СписокУчетныхЗаписейПереведенныхИзТакском.Найти(ВыборкаДетальныеЗаписи.Отправитель) <> Неопределено;
			
			Если ЭтоКонтейнерУчетнойЗаписиПереведеннойИзТакском 
				И ВыборкаДетальныеЗаписи.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияОтправителя 
				И Не ЭлектронныеДокументыЭДО.ЭтоВходящийЭДО(ВыборкаДетальныеЗаписи.ЭлектронныйДокумент) Тогда

				Если УстановитьСпособОбменаЧерезХаб(ВыборкаДетальныеЗаписи.ЭлектронныйДокумент,
					ВыборкаДетальныеЗаписи.ТранспортныйКонтейнер) Тогда
					ОписаниеКонтейнера.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО;
				КонецЕсли;
			КонецЕсли;
		...
		Если ЭтоКонтейнерУчетнойЗаписиПереведеннойИзТакском Тогда
			Продолжить;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(РасшифрованныеМаркеры) Тогда
		ДанныеДляСинхронизации = СинхронизацияЭДОСлужебный.ДанныеДляСинхронизацииИзРасшифрованныхМаркеров(
			РасшифрованныеМаркеры, КонтекстДиагностики, ОбрабатыватьТолькоОбъектыAPIТакском);
	Иначе
		ДанныеДляСинхронизации = СинхронизацияЭДОСлужебный.ДанныеДляСинхронизации(ОтпечаткиСертификатов,
			КонтекстДиагностики, ИдентификаторыОтправителей, ВыбранныеСертификаты, ПараметрыПереводаИзТакском);
	КонецЕсли;
...
Функция НовыеДанныеОбъектов() Экспорт
	...
	ДанныеОбъектов.Колонки.Добавить("ЗакрытыйПакет", Новый ОписаниеТипов("Булево"));
...
Функция НовоеОписаниеДокумента() Экспорт
	...
	ОписаниеДокумента.Вставить("ДатаДокумента", Дата(1, 1, 1));
...
//
Функция ДанныеДляСинхронизации(ОтпечаткиСертификатов, КонтекстДиагностики, УчетныеЗаписи = Неопределено, ВыбранныеСертификаты = Неопределено, ПараметрыПереводаИзТакском = Неопределено) Экспорт
	Возврат СинхронизацияЭДОСлужебный.ДанныеДляСинхронизации(ОтпечаткиСертификатов, КонтекстДиагностики, УчетныеЗаписи, ВыбранныеСертификаты, ПараметрыПереводаИзТакском);
КонецФункции
...
// Возвращаемое значение:
// 	Структура - Описание:
// * СпособОбменаДляПодмены - ПеречислениеСсылка.СпособыОбменаЭД - в случае указания, способ обмена, 
//							в ключах синхронизации будет подменен на указанное значение.
Функция ПараметрыПереводаИзТакском() Экспорт

	Результат = Новый Структура;
	Результат.Вставить("СпособОбменаДляПодмены", Перечисления.СпособыОбменаЭД.ПустаяСсылка());
	Возврат Результат;
...
//    * Ключ - Строка - идентификатор учетной записи
//    * Значение - см. НовоеОписаниеДатЗапросовДанныхУОператораЭлектронногоДокументооборота
Функция ДатыПолученияУчетныхЗаписейПереведенныхИзТакскомВХаб(ИдентификаторыЭДО) Экспорт
	Результат = Новый Соответствие;
	...
	Запрос.Текст =
		...
		|	УчетныеЗаписиПереведенныеИзТакском.ДатаПолучения,
		|	УчетныеЗаписиПереведенныеИзТакском.ДатаПолученияЭПД,
		|	УчетныеЗаписиПереведенныеИзТакском.ПроверочнаяДатаПолучения
		|ИЗ
		|	РегистрСведений.УчетныеЗаписиПереведенныеИзТакскомЭДО КАК УчетныеЗаписиПереведенныеИзТакском
		|ГДЕ
		|	УчетныеЗаписиПереведенныеИзТакском.ИдентификаторЭДО В (&ИдентификаторыЭДО)";
	...
	Пока Выборка.Следующий() Цикл
		Даты = НовоеОписаниеДатЗапросовДанныхУОператораЭлектронногоДокументооборота();
		ЗаполнитьЗначенияСвойств(Даты, Выборка);
				
		Результат.Вставить(Выборка.ИдентификаторЭДО, Даты);
	КонецЦикла;
...
Функция ВидОшибкиНекорректныйТокенДоступаКСервисуЭДО(ИдентификаторУчетнойЗаписи) Экспорт
	...
	ВидОшибки.ОбработчикиНажатия.Вставить("Авторизоваться",
		"СинхронизацияЭДОСлужебныйКлиент.ОбработатьОшибкуНекорректныйТокенДоступаКСервисуЭДО");
...
// 
// Возвращаемое значение:
//  Булево - Способ обмена успешно установлен
Функция УстановитьСпособОбменаЧерезХаб(ЭлектронныйДокумент, ТранспортныйКонтейнер)
	СпособОбменаУстановлен = Истина;
	НачатьТранзакцию();
	Попытка
		Если ЗначениеЗаполнено(ЭлектронныйДокумент) Тогда
			ЭлектронныеДокументыЭДО.ЗаблокироватьДокументДляИзменения(ЭлектронныйДокумент, Ложь);
			ЭлектронныйДокументОбъект = ЭлектронныйДокумент.ПолучитьОбъект();
			ЭлектронныйДокументОбъект.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО;
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ЭлектронныйДокументОбъект);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ТранспортныйКонтейнер) Тогда
			ТранспортныеКонтейнерыЭДО.ЗаблокироватьКонтейнер(ТранспортныйКонтейнер);
			ТранспортныйКонтейнерОбъект = ТранспортныйКонтейнер.ПолучитьОбъект();
			ТранспортныйКонтейнерОбъект.СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО;
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ТранспортныйКонтейнерОбъект);
		КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Изменение способа обмена в документе и транспортном контейнере'", 
			ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			
			СпособОбменаУстановлен = Ложь;
	КонецПопытки;
	
	Возврат СпособОбменаУстановлен;
КонецФункции
