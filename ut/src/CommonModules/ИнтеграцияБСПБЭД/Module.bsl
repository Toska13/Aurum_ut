...
//  Массив из СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования
//
Функция ЗарегистрированныеВЭДОСертификаты(ПараметрыПодписанта, ВидМобильнойПодписи) Экспорт
	...
	Запрос.Текст =
		...
		|	И &ОтборПодписанта
		|	И СертификатыПодписантовМобильнойПодписи.ВидМобильнойПодписи = &ВидМобильнойПодписи";
	...
	Если ИнтеграцияБСПБЭДСлужебныйКлиентСервер.ВерсияБСППоддерживаетГосключ(
		...
		Запрос.УстановитьПараметр("ВидМобильнойПодписи", ВидМобильнойПодписи);
...
// Определяет доступно ли использование сервисов мобильного подписания
// Возвращаемое значение:
//  Булево - доступно или не доступно
//
Функция ДоступноИспользованиеСервисовМобильногоПодписания() Экспорт
	
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.СервисМобильнойПодписи") Тогда
		Возврат Ложь;
	КонецЕсли;

	Возврат ДоступноИспользованиеСервисаМояПодпись()
		Или ДоступноИспользованиеСервисаГосключ();

КонецФункции

// Определяет доступно ли использование сервиса Моя подпись
// Возвращаемое значение:
//  Булево - доступно или не доступно
//
Функция ДоступноИспользованиеСервисаМояПодпись() Экспорт
	...
	Возврат МодульСервисМобильнойПодписи.ИспользоватьСервисМобильнойПодписи();

КонецФункции

// Определяет доступно ли использование сервиса Госключ
// Возвращаемое значение:
//  Булево - доступно или не доступно
//
Функция ДоступноИспользованиеСервисаГосключ() Экспорт
	
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.СервисМобильнойПодписи") Тогда
		Возврат Ложь;
	КонецЕсли;

	Если Не ИнтеграцияБСПБЭДСлужебныйКлиентСервер.ВерсияБСППоддерживаетГосключ(
			СтандартныеПодсистемыСервер.ВерсияБиблиотеки()) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Возврат МодульСервисМобильнойПодписи.ИспользоватьГосключ();
