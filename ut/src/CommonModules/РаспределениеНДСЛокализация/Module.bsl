...
// 
// Параметры:
// 	ТаблицыРаспределения - Соответствие Из ПеречислениеСсылка.ТипыНалогообложенияНДС, ТаблицаЗначений -
...
// Рассчитывает, какую сумму НДС нужно восстановить и списать на прочие расходы при смене вида деятельности с облагаемой на 
// необлагаемую или распределяемую, если вычет ранее заявлялся.
// 
// Параметры:
// 	ТаблицыРаспределения - Соответствие Из ПеречислениеСсылка.ТипыНалогообложенияНДС, ТаблицаЗначений -
// 	ДанныеДляРаспределения - Структура Из ТаблицаЗначений - 
// 	Реквизиты - Структура - состоит в том числе из:
//		* Ссылка - ДокументСсылка -
Процедура РассчитатьВосстановлениеНДСИСписаниеНаРасходы(ТаблицыРаспределения, ДанныеДляРаспределения, Реквизиты) Экспорт
...
//
// Параметры:
// 	ДанныеВыручки - Структура - Параметр, в котором возвращаются данные выручки. Структура с двумя ключами:
...
Процедура ПолучитьДанныеВыручки(ДанныеВыручки, Дата, НачалоПериода, Организация, ДополнительныеПараметрыПолученияВыручки) Экспорт
	...
	|ПОМЕСТИТЬ ДокументыЭкспортИсходные
	|ИЗ
	...
	|		ПО
	|			ДанныеПервичныхДокументов.Организация = ДанныеОснованийСчетовФактур.Организация
	|			И ДанныеПервичныхДокументов.Документ = ДанныеОснованийСчетовФактур.Регистратор
	|			И НЕ ДанныеОснованийСчетовФактур.ИсправлениеОшибок 
	|			И НЕ ДанныеОснованийСчетовФактур.КорректировкаПоСогласованиюСторон
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ 
	|			РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументовКорректировочные
	|		ПО
	|			ДанныеПервичныхДокументовКорректировочные.Организация = ДанныеОснованийСчетовФактур.Организация
	|			И ДанныеПервичныхДокументовКорректировочные.Документ = ДанныеОснованийСчетовФактур.КорректируемыйДокумент
	|			И (ДанныеОснованийСчетовФактур.ИсправлениеОшибок ИЛИ ДанныеОснованийСчетовФактур.КорректировкаПоСогласованиюСторон)
	|ГДЕ
	|	ДанныеОснованийСчетовФактур.Период МЕЖДУ &ПериодНачало И &ДатаРаспределения
	|	И ЕСТЬNULL(ДанныеПервичныхДокументов.ДатаРегистратора, ЕСТЬNULL(ДанныеПервичныхДокументовКорректировочные.ДатаРегистратора, ДАТАВРЕМЯ(1,1,1))) <= &ПериодОкончание
	...
	|ПОМЕСТИТЬ ДокументыЭкспортСторно
	|ИЗ
	|	РегистрСведений.ИсправленияДокументов КАК ИсправлениеДокументов
	|ГДЕ
	|	ИсправлениеДокументов.Период > &ПериодОкончание
	|	И ИсправлениеДокументов.Период <= &ДатаРаспределения
	|	И ИсправляемыйДокумент В
	|				(ВЫБРАТЬ
	|					Т.Документ
	|				ИЗ
	|					ДокументыЭкспортИсходные КАК Т)
	|	И ИсправлениеДокументов.СторнированИсправляемыйДокумент = ИСТИНА
	|;
	|
	|///////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДокументыЭкспортИсходные.НалогообложениеНДС КАК НалогообложениеНДС,
	|	ДокументыЭкспортИсходные.Документ КАК Документ,
	|	ДокументыЭкспортИсходные.Выручка КАК Выручка,
	|	ДокументыЭкспортИсходные.КорректировкаВыручки КАК КорректировкаВыручки
	|ПОМЕСТИТЬ ДокументыЭкспорт
	|ИЗ
	|	ДокументыЭкспортИсходные КАК ДокументыЭкспортИсходные
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		ДокументыЭкспортСторно КАК ДокументыЭкспортСторно
	|	ПО
	|		ДокументыЭкспортИсходные.Документ = ДокументыЭкспортСторно.ИсправляемыйДокумент
	|ГДЕ
	|	ДокументыЭкспортСторно.АктуальныйДокумент ЕСТЬ NULL
	|;
	|
	|///////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|					И КорректируемыйПериод = ДАТАВРЕМЯ(1,1,1)
	...
	|								ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПродажи.НалогИсчисляетПокупатель))
	|					И НЕ ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.АвансыПолученные)
	|					И КорректируемыйПериод МЕЖДУ &ПериодНачало И &ПериодОкончание
	|					И НЕ СтавкаНДС В (ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС0), ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС))) КАК НДСЗаписиКнигиПродажОбороты
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	|		ПО
	|			ДанныеПервичныхДокументов.Организация = ДанныеОснованийСчетовФактур.Организация
	|			И ДанныеПервичныхДокументов.Документ = ДанныеОснованийСчетовФактур.КорректируемыйДокумент
	|
	|	ГДЕ
	|		ДанныеОснованийСчетовФактур.Период МЕЖДУ &ПериодНачало И &ДатаРаспределения
	|		И ДанныеПервичныхДокументов.ДатаРегистратора МЕЖДУ &ПериодНачало И &ПериодОкончание
	|		И ДанныеОснованийСчетовФактур.Организация В (&Организация)
	...
	|			ГДЕ
	|				Т.ДокументОснование = ДанныеОснованийСчетовФактур.Регистратор
	|				И НЕ Т.Ссылка.ПометкаУдаления
	|				И НЕ Т.Ссылка.Исправление)
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	ДатаРаспределения = Неопределено;
	Если ДополнительныеПараметрыПолученияВыручки <> Неопределено Тогда 
		Если ДополнительныеПараметрыПолученияВыручки.Свойство("ВключатьВсеДокументыЭкспорта") Тогда
			ВключатьВсеДокументыЭкспорта = ДополнительныеПараметрыПолученияВыручки.ВключатьВсеДокументыЭкспорта;
		КонецЕсли;
		Если ДополнительныеПараметрыПолученияВыручки.Свойство("ДатаРаспределения") Тогда
			ДатаРаспределения = ДополнительныеПараметрыПолученияВыручки.ДатаРаспределения;
		КонецЕсли;
	КонецЕсли;
	...
	Если ДатаРаспределения = Неопределено Тогда 
		ДатаРаспределения = ПериодОкончание;
	КонецЕсли;
	...
	ДанныеВыручки.Вставить("ВыручкаПоНалогообложениямНДС", Результат[3].Выгрузить());
	ДанныеВыручки.Вставить("ДокументыЭкспорт",             Результат[4].Выгрузить());
	//-- Локализация
...
//
// Параметры:
// 	 МассивОрганизаций - Массив Из СправочникСсылка.Организации, СправочникСсылка.Организации - организации, для которых надо выполнить оценку
...
Процедура РаспределитьПартииНДСКРаспределениюЭкспортПоДокументамТекущегоПериода(ПартииНДСКРаспределениюЭкспорт, Реквизиты, МенеджерВременныхТаблиц, РаспределятьНаВесьЭкспорт)
	...
	Если РаспределятьНаВесьЭкспорт Тогда
		...
		ДополнительныеПараметрыПолученияВыручки.Вставить("ДатаРаспределения", Реквизиты.ДатаРаспределения);
...
Процедура РаспределитьПартииНДСКРаспределениюЭкспортПоДокументамПрошлыхПериодов(ПартииНДСКРаспределениюЭкспорт, Реквизиты, МенеджерВременныхТаблиц)
	...
	|		И ПартииНДСКРаспределению.ДокументРеализации <> ЗНАЧЕНИЕ(Документ.РеализацияТоваровУслуг.ПустаяСсылка)
	|;
	|
	|ВЫБРАТЬ
	|	*
