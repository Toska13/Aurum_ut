...
Процедура ЗарегистрироватьСчетаФактурыОжидаетОплатыНДС(Объект, Проведен)
	...
	СтрокаСчетаФактуры.ДатаДокументаПеречисленияНалога = ?(Объект.ОплатаЧерезЕдиныйЛицевойСчет, Объект.Дата, Дата(1,1,1));
...
Процедура ЗаполнитьНомераГТД(Объект)
	...
	|	ДанныеНоменклатуры.ЕдиницаИзмеренияТНВЭД	КАК ЕдиницаИзмерения
	|ПОМЕСТИТЬ Товары
	|ИЗ
	...
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА Товары.ТипНомераГТД = ЗНАЧЕНИЕ(Перечисление.ТипыНомеровГТД.НомерРНПТ)
	|				И Товары.ЕдиницаИзмерения <> ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|			ТОГДА Товары.ЕдиницаИзмерения
	|		КОГДА Товары.ТипНомераГТД = ЗНАЧЕНИЕ(Перечисление.ТипыНомеровГТД.НомерРНПТ)
	|			ТОГДА &ЕдиницаИзмеренияТНВЭД
	...
	Запрос.УстановитьПараметр("ЕдиницаИзмеренияТНВЭД", ЕдиницаИзмеренияТНВЭД);
	...
	Пока Выборка.Следующий() Цикл
		...
		ПараметрыДляЗаполнения.Вставить("ЕдиницаИзмерения");
...
Функция ТекстЗапросаТаблицаЖурналУчетаСчетовФактур(Запрос, ТекстыЗапроса, Регистры)
	...
	|	(ВЫБРАТЬ
	|		СУММА(Разделы.ТаможеннаяСтоимость + Разделы.СуммаПошлины + Разделы.СуммаАкциза) КАК СуммаПоСчетуФактуре,
