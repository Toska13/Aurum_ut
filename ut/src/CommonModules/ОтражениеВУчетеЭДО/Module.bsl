...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьВходящиеДокументыНастроек");
	Методы.Вставить("ОтразитьЭлектронныеДокументы", Истина);
...
// Определяет необходимость контроля отражения в учете электронных документов
//
// Возвращаемое значение:
//  Булево
Функция ТребуетсяКонтрольОтраженияВУчете() Экспорт
	
	Возврат ПолучитьФункциональнуюОпцию("ИспользоватьКонтрольОтраженияВУчетеЭДО");
...
//  * Успех - Булево
//  * Ошибка - Строка
Функция ОтразитьДанныеЭлектронногоДокумента(ДанныеЭлектронногоДокумента, СпособОбработки, ОбъектыУчета = Неопределено,
	...
	Попытка
		...
		Иначе
			РезультатОтражения = ОтразитьДанныеФормализованногоЭлектронногоДокумента(ДанныеЭлектронногоДокумента, ДанныеРазбора,
				СпособОбработки, ОбъектыУчета);
			Результат.Успех = РезультатОтражения.Успех;
			Результат.Ошибка = РезультатОтражения.Ошибка;
		КонецЕсли;
...
Процедура ОтразитьДанныеЭлектронногоДокументаСКонтролем(ДанныеДокумента, СпособОбработки = Неопределено) Экспорт
	...
	КлючЗамера = "ОтразитьДанныеЭлектронногоДокументаСКонтролем";
	...
	ОбщегоНазначенияБЭД.ЗакончитьЗамер(КлючЗамера, ОписаниеЗамера, ОбъектыУчета.Количество());
КонецПроцедуры
...
Функция ОтразитьЭлектронныеДокументы(СпособыОбработкиДокументов) Экспорт
	...
	Для Каждого ДанныеДокумента Из ДанныеДляОтраженияВУчете Цикл
		...
		ОбъектыУчета = Новый Массив; // Массив Из ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО
		РезультатОтражения = ОтразитьДанныеЭлектронногоДокумента(ДанныеДокумента, СпособОбработки, ОбъектыУчета);
		Если Не РезультатОтражения.Успех Тогда
			ВидОшибки = ВидОшибкиНеУдалосьВыполнитьОтражениеВУчете();
			ВидОшибки.ОписаниеПроблемы = РезультатОтражения.Ошибка;
			Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(ВидОперацииОтражениеВУчете(),
				ВидОшибки, РезультатОтражения.Ошибка, РезультатОтражения.Ошибка);
...
// Снимает электронный документ с контроля сопоставления номенклатуры, отключает контроль отражения в учете
// в части сопоставления номенклатуры.
//
// Параметры:
//  ЭлектронныйДокумент - ДокументСсылка.ЭлектронныйДокументВходящийЭДО
Процедура ОтключитьКонтрольСопоставленияНоменклатурыПоДокументу(ЭлектронныйДокумент) Экспорт

	Если Не ЭлектронныеДокументыЭДО.ЭтоВходящийЭДО(ЭлектронныйДокумент) Тогда
		ВызватьИсключение НСтр("ru = 'Контроль сопоставления номенклатуры доступен только для входящих документов'");
	КонецЕсли;

	УстановитьПривилегированныйРежим(Истина);
	
	ТребуетсяИзменениеКонтроляОтраженияВУчете = Ложь;

	НачатьТранзакцию();
	Попытка
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.КонтрольСопоставленияНоменклатурыЭДО");
		...
		Если ТребуетсяЗапись Тогда
			...
			ТребуетсяИзменениеКонтроляОтраженияВУчете = Истина;
		КонецЕсли;
			
		ЗафиксироватьТранзакцию();
	Исключение
		ОтменитьТранзакцию();
		ВидОперации = НСтр("ru = 'Отключение контроля сопоставления номенклатуры по документу'");
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ПодробныйТекстОшибки = ОбщегоНазначенияБЭД.ПодробноеПредставлениеИнформацииОбОшибке(ИнформацияОбОшибке);
		КраткийТекстОшибки = ОбщегоНазначенияБЭД.КраткоеПредставлениеИнформацииОбОшибке(ИнформацияОбОшибке);
		ЭлектронноеВзаимодействие.ОбработатьОшибку(ВидОперации, ПодробныйТекстОшибки, КраткийТекстОшибки,
	...
	Если ТребуетсяИзменениеКонтроляОтраженияВУчете Тогда
		НаборИзменений = Новый Соответствие;
		НаборИзменений.Вставить("СопоставитьНоменклатуру", Ложь);
		ИзменитьКонтрольОтраженияВУчете(ЭлектронныйДокумент, НаборИзменений);
	КонецЕсли;
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ИдентификаторОтправителя) Тогда
			...
			Если Запрос.Выполнить().Пустой() Тогда
				...
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Нельзя создать настройку получения по идентификаторам, без общей настройки по организации
...
//  * Успех - Булево
//  * Ошибка - Строка
Функция ОтразитьДанныеФормализованногоЭлектронногоДокумента(ДанныеЭлектронногоДокумента, ДанныеДляОтражения,
	СпособОбработки, ДокументыУчета = Неопределено)
	...
	Попытка
		...
		Результат.Успех = Не ЗначениеЗаполнено(РезультатОтраженияВУчете.ОписаниеОшибки);
		Результат.Ошибка = РезультатОтраженияВУчете.ОписаниеОшибки;
...
// Возвращаемое значение:
//  Строка
Функция ВидОперацииОтражениеВУчете()
	
	Возврат НСтр("ru = 'Отражение в учете'");
	
КонецФункции

// Возвращаемое значение:
//  См. ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки
Функция ВидОшибкиНеУдалосьВыполнитьОтражениеВУчете()
	
	ВидОшибки = ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки();
	ВидОшибки.Идентификатор = "НеУдалосьВыполнитьОтражениеВУчете";
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Не удалось выполнить отражение в учете'");
