...
// Дополняет параметры сканирования для документа Приемка товаров при необходимости передавать специфику ВетИС:
//   * Взводит флаг "ЗаполнятьДанныеВЕТИС".
//   * Определяет тип идентификатора происхождения ВетИС.
// 
// Параметры:
...
//   ПараметрыСканирования - См. ШтрихкодированиеОбщегоНазначенияИСКлиент.ПараметрыСканирования.
//
Процедура ДополнитьПараметрыСканированияПриемкаТоваров(Операция, ПараметрыСканирования) Экспорт
	
	СтандартнаяОбработка = Истина;
	ИнтеграцияИСМПВЕТИСКлиентСерверПереопределяемый.ДополнитьПараметрыСканированияПриемкаТоваров(Операция, ПараметрыСканирования, СтандартнаяОбработка);
	Если Не СтандартнаяОбработка Тогда
		Возврат;
	КонецЕсли;
...
// Проверяет, является ли вид продукции ГИС МТ связанным с ВетИС
//
// Параметры:
...
// Возвращаемое значение:
//  Булево - Истина, если вид продукции ГИС МТ связан с подконтрольной ВетИС продукцией.
Функция ЭтоВидПродукцииИСМПВЕТИС(Знач ВидПродукции) Экспорт
	...
	Если ТипЗнч(ВидПродукции) = Тип("Массив") Тогда
		Для Каждого ВидПродукцииИзМассива Из ВидПродукции Цикл
			Если ВидыПродукцииПодконтрольныеВЕТИС.Найти(ВидПродукцииИзМассива) <> Неопределено Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЦикла;
		Возврат Ложь;
	...
	ИначеЕсли ТипЗнч(ВидПродукции) = Тип("СправочникСсылка.ТоварныеГруппыГИСМТ") Тогда
		Для Каждого ВидПродукцииИзМассива Из КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукции(ВидПродукции) Цикл
			Если ВидыПродукцииПодконтрольныеВЕТИС.Найти(ВидПродукцииИзМассива) <> Неопределено Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЦикла;
	...
	Возврат ВидыПродукцииПодконтрольныеВЕТИС.Найти(ВидПродукции) <> Неопределено;
