...
Процедура ПриОпределенииОбщихПараметровБазовойФункциональности(ОбщиеПараметры) Экспорт
	...
	ОбщиеПараметры.МинимальнаяВерсияПлатформы = "8.3.27.1859";
	ОбщиеПараметры.РекомендуемаяВерсияПлатформы = "8.3.27.1989";
...
Процедура ПриДобавленииОбработчиковУстановкиПараметровСеанса(Обработчики) Экспорт
	...
	Обработчики.Вставить("ПроводитьБезКонтроляОстатковРетроБонусовКлиентов" , "РетроБонусыСервер.УстановитьПараметрыСеанса");
...
Процедура ПриДобавленииИсключенийПоискаСсылок(ИсключенияПоискаСсылок) Экспорт
	...
	ИсключенияПоискаСсылок.Добавить(Метаданные.РегистрыСведений.ЗаданияКРаспределениюВзаиморасчетов.ПолноеИмя());
	ИсключенияПоискаСсылок.Добавить(Метаданные.РегистрыСведений.ЗаданияНеоперативногоДопроведенияДокументов.ПолноеИмя());
	...
	ИсключенияПоискаСсылок.Добавить(Метаданные.РегистрыСведений.ДублиНСИ.ПолноеИмя());
	ИсключенияПоискаСсылок.Добавить(Метаданные.РегистрыСведений.СтоимостьТоваров.ПолноеИмя());
...
Процедура ПриОпределенииПодчиненныхОбъектов(ПодчиненныеОбъекты) Экспорт
	...
	ОбщегоназначенияУТ.ПриОпределенииПодчиненныхОбъектов(ПодчиненныеОбъекты);
...
Процедура ПослеЗаменыСсылок(Результат, ПараметрыВыполнения, ТаблицаПоиска) Экспорт
	...
	Если ТаблицаПоиска.Количество() > 0 Тогда
		...
		ТипыСсылок = ТаблицаПоиска.ВыгрузитьКолонку("ТипСсылки");
		ТипыСсылок = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ТипыСсылок);
		
		РазностьМассивов = ОбщегоНазначенияКлиентСервер.РазностьМассивов(МассивТиповКлючейРеестраДокументов, ТипыСсылок);
		
		Если РазностьМассивов.Количество() < МассивТиповКлючейРеестраДокументов.Количество() Тогда
			КлючиРеестраДокументовСервер.УдалениеДублейКлючейРеестраДокументов();
		КонецЕсли;
		
	КонецЕсли;
	
	Справочники.КлючиАналитикиУчетаНоменклатуры.ОчиститьДвиженияСтоимостьТоваровПослеЗаменыСсылокКлючейАналитики(Результат, ТаблицаПоиска);
...
Процедура ПриДобавленииПереименованийОбъектовМетаданных(Переименования) Экспорт
	//++ НЕ ГОСИС
	ОбновлениеИнформационнойБазыУТ.ПриДобавленииПереименованийОбъектовМетаданных(Переименования);


	ОбщегоНазначенияЛокализация.ПриДобавленииПереименованийОбъектовМетаданных(Переименования);
	//-- НЕ ГОСИС
...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	Если Метаданные.ОбщиеМодули.Найти("ИнтеграцияСМаркетплейсамиСервер") <> Неопределено Тогда
		МодульИнтеграцияСМаркетплейсамиСервер = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияСМаркетплейсамиСервер");
		МодульИнтеграцияСМаркетплейсамиСервер.ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки);
	КонецЕсли;
