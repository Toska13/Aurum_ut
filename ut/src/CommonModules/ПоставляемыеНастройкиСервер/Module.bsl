...
//  ДополнительныеПараметры - Структура, Неопределено - дополнительные параметры загрузки.
//    Данные будут без изменения переданы в метод ПоставляемыеНастройкиПереопределяемый.ПриЗагрузкеФайлаНастройки.
//
// Возвращаемое значение:
...
//         - "НеизвестнаяНастройкаИлиПрограмма" - программа
...
//    * ОтсутствующиеИдентификаторы - Массив - содержит не обнаруженные в сервисе идентификаторы.
//
Функция ОбновитьПоставляемыеНастройки(
		Идентификаторы,
	...
	РезультатОбновления.Вставить("ОтсутствующиеИдентификаторы", Новый Массив);
	...
	РезультатОбновления.ОтсутствующиеИдентификаторы = ОбщегоНазначенияКлиентСервер.РазностьМассивов(
		Идентификаторы,
		РезультатОперации.ДанныеНастроек.ВыгрузитьКолонку("Идентификатор"));
...
Функция ПолучитьФайлыНастроек(
	...
	Результат = ДанныеПоставляемыхНастроекИзКэша(Идентификаторы);
...
#Область УправлениеПредприятием

// Добавляет настройки в список доступных для обновления. Метод следует использовать,
// если настройка включается удаленно через отдельный файл манифеста.
//
// Параметры:
//  Настройки - Массив из Структура - содержит описание настроек см. ОписаниеНастройки.
//
Процедура ДобавитьНастройки(Настройки) Экспорт
	
	// Для модели сервиса необходимо обновлять манифест поставляемых данных в Менеджере сервиса.
	// На текущий момент такой возможности нет.
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("ru = 'Добавление настроек при работе в моделе сервиса не поддерживается.'");
	КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ДобавитьЗаданиеОбновленияПоставляемыхНастроек");
	Методы.Вставить("ОбновитьСлужебныеДанныеНастроек");
	Методы.Вставить("ОбновлениеПоставляемыхНастроек", Истина);
...
Процедура ПриОпределенииОбработчиковПоставляемыхДанных(Обработчики) Экспорт
	...
	Для Каждого ВидДанных Из ВидыДанных Цикл
		Обработчик = Обработчики.Добавить();
		Обработчик.ВидДанных = ВидДанных;
		Обработчик.КодОбработчика = ВидДанных;
		Обработчик.Обработчик = ОбщегоНазначения.ОбщийМодуль("ПоставляемыеНастройкиВМоделиСервиса");
	КонецЦикла;
...
//
// Возвращаемое значение:
//  Структура - параметры обновления настройки.
//
...
// АПК:142-выкл конструктор структуры
...
Процедура ПриЗагрузкеФайлаНастройки(
	...
	Если Обработан Тогда
		...
		// Версии поставляемых настроек, Версии поставляемых настроек областей данных
		// и Кэш данных настроек, данные должны быть обновлены после
...
//
Процедура ОбновитьСлужебныеДанныеНастройки(
		ДанныеНастройки,
		ДанныеФайла,
		ОбновитьКэш = Истина) Экспорт
...
// Определяет список поставляемых настроек доступных конфигурации.
//
// Параметры:
...
Функция СлужебнаяОпределитьДанныеНастроек(
	...
	РезультатОперации = ПоставляемыеНастройкиСервис.АктуальныеВерсииПоставляемыхНастроек(
		Идентификаторы,
		ДанныеАутентификации);
	...
	РезультатЗагрузки = ПоставляемыеНастройкиСервис.ЗагрузитьФайлыПоставляемыхНастроек(
		РезультатОперации.ДанныеНастроек,
		ДанныеАутентификации);
...
Функция СлужебнаяОпределитьДанныеНастроекВМоделиСервиса(Идентификаторы)
	...
	РезультатОперации = ДанныеПоставляемыхНастроекИзКэша(Идентификаторы);
...
//
Функция ОбработатьФайлыНастроек(
		ДанныеНастроек,
		ДополнительныеПараметры)
...
//
Функция ДанныеПоставляемыхНастроекИзКэша(Идентификаторы)
