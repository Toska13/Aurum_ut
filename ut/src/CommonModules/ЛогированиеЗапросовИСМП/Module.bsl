...
Функция ИнформацияОбОкружении() Экспорт
	...
	Для Каждого КлючИЗначение Из ПараметрыКонтроля Цикл
		...
		ПредставлениеПараметра = СинонимыНастроекСканирования.Получить(ИмяПараметраНастройки);
		Если ПредставлениеПараметра = Неопределено Тогда
			ПредставлениеПараметра = ОбщегоНазначенияИСКлиентСервер.ПредставлениеВстроенногоИмени(ИмяПараметраНастройки);
		КонецЕсли;
		
		ОписаниеНастроек = "";
		
		ЭлементКонструктора = НастройкаПараметровСканированияСлужебныйКлиентСервер.КонвертироватьИмяЭлементаЭлементКонструктора(
			СтрШаблон("%1Настройка", ИмяПараметраНастройки));
		Если ЭлементКонструктора <> Неопределено Тогда
			...
			Если НастройкаПользователя <> Неопределено Тогда
				ОписаниеНастроек = НастройкаПользователя;
			КонецЕсли;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ОписаниеНастроек) Тогда
			ДанныеОписания = СтрРазделить(ОписаниеНастроек, Символы.ПС);
			ОписаниеНастроек = СтрСоединить(ДанныеОписания, "; ")
		Иначе
			ОписаниеНастроек = НСтр("ru = 'Стандартно'");
		КонецЕсли;
	...
	ОтключенаТабачнаяПродукция = Истина;
	Для Каждого ВидПродукцииМОТП Из КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииМОТП() Цикл
		Если ОбщегоНазначенияИСМПВызовСервера.ВестиУчетМаркируемойПродукции(ВидПродукцииМОТП) Тогда
			ОтключенаТабачнаяПродукция = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого КлючИЗначение Из НастройкиСканирования Цикл
		...
		Если Не ЗначениеЗаполнено(Заголовок) Тогда
			Заголовок = ОбщегоНазначенияИСКлиентСервер.ПредставлениеВстроенногоИмени(ИмяНастройки);
		КонецЕсли;
		Значение = СинонимыНастроекСканирования.Получить(КлючИЗначение.Значение);
		Если Значение = Неопределено Тогда
			Значение = ОбщегоНазначенияИСКлиентСервер.ПредставлениеВстроенногоИмени(КлючИЗначение.Значение);
		КонецЕсли;
		ДанныеОкружения.Добавить(
			СтрШаблон(
				"%1: %2",
...
Процедура УдалитьФайлЛога(ИдентификаторЗаписи, ПараметрыЛогирования)
	...
	ОбщегоНазначенияИС.ПроверитьРасположениеФайлаВоВременномКаталоге(ИмяФайла);
