...
Функция ЭтоНеФормализованныйКодМаркировки(ПараметрыРазбораКодаМаркировки, Настройки, ДанныеРезультата, РезультатБезФильтра) Экспорт
	...
	Если Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(Настройки.ДоступныеВидыПродукции) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если ФильтрПоВидуПродукции.Использовать
		И Не КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(ФильтрПоВидуПродукции.ВидыПродукции) Тогда
		Возврат Ложь;
	КонецЕсли;
