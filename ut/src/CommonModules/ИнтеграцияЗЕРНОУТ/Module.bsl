...
Процедура ЗаполнитьТабличнуюЧастьДокумента(ТабличнаяЧасть, РезультатЗапроса, ДанныеЗаполнения, СоответствиеКолонокКоличествоЗЕРНО, БезСопоставления = Истина, ДополнительныеПараметры = Неопределено)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДополнительныеПараметры <> Неопределено
			...
			ПотребительскиеСвойстваПоОДКПД2 = ИнтеграцияЗЕРНО.ПотребительскиеСвойстваПродукцииПоДаннымОКПД2(
				НоваяСтрока.ОКПД2, ДополнительныеПараметры.НазначениеПартии, ДополнительныеПараметры.Операция, ДополнительныеПараметры.СтранаНазначения);
...
Процедура ЗаполнитьТабличнуюЧастьФормированиеПартийЗЕРНО(ДокументОбъект, ДанныеЗаполнения, ТекстТоварыДокумента, ДопустимыйЗнак)
	...
	ВидыПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЗЕРНО();
	Если ЗначениеЗаполнено(ДокументОбъект.РазделУчета) Тогда
		ВидыПродукции = ИнтеграцияЗЕРНОКлиентСервер.ВидыПродукции(ДокументОбъект.РазделУчета);
	КонецЕсли;
	
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		Если ДанныеЗаполнения.ИмяФильтра = "ЗаполнениеСВидомПродукции" Тогда
			ФильтрВидПродукции = ДанныеЗаполнения.ЗначениеФильтра;
			ВидыПродукции   = ИнтеграцияЗЕРНОКлиентСервер.ВидыПродукции(ФильтрВидПродукции);
		КонецЕсли;
		Основание = ДанныеЗаполнения.Основание;
	Иначе
		Основание = ДанныеЗаполнения;
	...
	ДополнительныеПараметры.Вставить("Операция",                ДокументОбъект.Операция);
	ДополнительныеПараметры.Вставить("НазначениеПартии",        ДокументОбъект.НазначениеПартии);
	ДополнительныеПараметры.Вставить("СтранаНазначения",        ДокументОбъект.СтранаНазначения);
	...
	Если Не ЗначениеЗаполнено(ДокументОбъект.РазделУчета) Тогда
		ВидПродукции = ИнтеграцияИС.ВидПродукцииПоТабличнойЧасти(ДокументОбъект, "ЗЕРНО");
		Если ЗначениеЗаполнено(ВидПродукции) Тогда
			РазделыУчета = ИнтеграцияЗЕРНОКлиентСервер.РазделыУчета(ВидПродукции);
			Если РазделыУчета.Количество() Тогда
				ДокументОбъект.РазделУчета = РазделыУчета[0];
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаЗаполненияДокументаФормированиеПартийЗЕРНО(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ОприходованиеИзлишковОтгруженныхТоваров") Тогда 
		ЗаполнитьФормированиеПартийЗЕРНОНаОснованииОприходованиеИзлишковОтгруженныхТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
...
Процедура ЗаполнитьФормированиеПартийЗЕРНОНаОснованииПрочегоОприходованияТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО В (&ВидыПродукции)
	|СГРУППИРОВАТЬ ПО
	|	Серии.Номенклатура,
...
Процедура ЗаполнитьФормированиеПартийЗЕРНОНаОснованииОприходованиеИзлишковОтгруженныхТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|ГДЕ
	|	Товары.Ссылка = &ДокументОснование
	|	И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО В (&ВидыПродукцииЗерно)
	|СГРУППИРОВАТЬ ПО
	|	Товары.Номенклатура,
	|	Товары.Характеристика,
	|	Товары.Серия,
...
Функция ТекстЗапросаЗаполненияФормированиеПартийПриПроизводствеЗЕРНОИзПрикладногоДокумента(ТекстТоварыДокумента, ДопустимыйЗнак)
	...
	|		ПО ТоварыКОформлению.Номенклатура = СправочникНоменклатура.Ссылка
	|ГДЕ
	|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО В (&ВидыПродукции)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТоварыКОформлению.Номенклатура,
	|	ТоварыКОформлению.Характеристика,
...
Процедура ЗаполнитьФормированиеПартийПриПроизводствеЗЕРНОНаОснованииСборкиТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО ДокументСсылка.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|ГДЕ
	|	ДокументСсылка.Ссылка = &ДокументОснование
	...
	Если Не ЭтоПерезаполнение
		...
		Если ДокументОбъект.Перевозка Тогда
			...
			ИначеЕсли ЗначениеЗаполнено(ДокументОбъект.ПунктНазначенияСтрокой) И Не ЗначениеЗаполнено(ДокументОбъект.ПунктНазначения) Тогда
				...
				ДокументОбъект.ПунктНазначения = ОбщегоНазначенияИСВызовСервера.ЗначенияПолейКонтактнойИнформации(ДокументОбъект.ПунктНазначенияСтрокой, ВидКонтактнойИнформации);
...
Процедура ЗаполнитьТабличнуюЧастьКорректировочныйОформлениеСДИЗЗЕРНО(ДокументОбъект, ДанныеЗаполнения, ИмяДокумента, ТоварыСерии = Неопределено)
	...
	Документы.ОформлениеСДИЗЗЕРНО.ЗаполнитьКорректировочныйСДИЗНаОснованииТаблицыКорректировки(ДокументОбъект, Запрос, Основание, ТаблицаКорректируемыйДокумент);
...
Процедура ОбработкаЗаполненияДокументаОформлениеСДИЗЗЕРНО(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда
		ЗаполнитьОформлениеСДИЗЗЕРНОНаОснованииОтгрузкиТоваровКлиенту(
			ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
...
Процедура ЗаполнитьОформлениеСДИЗЗЕРНОНаОснованииОтгрузкиТоваровКлиенту(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ОписаниеТабличнойЧасти = ИнтеграцияИСУТ.ТекстЗапросаОтгрузкаТоваровКлиенту();
	ЗаполнитьТабличнуюЧастьОформлениеСДИЗЗЕРНО(ДокументОбъект, ДанныеЗаполнения, "ОтгрузкаТоваровКлиенту", ОписаниеТабличнойЧасти);
...
Процедура ОбработкаЗаполненияДокументаСписаниеПартийЗЕРНО(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.СписаниеОтгруженныхТоваров") Тогда
		ЗаполнитьСписаниеПартийЗЕРНОНаОснованииСписанияОтгруженныхТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
...
Процедура ЗаполнитьСписаниеПартийЗЕРНОНаОснованииСписанияОтгруженныхТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
...
Процедура ЗаполнитьВозвратТоваровПоставщикуНаОснованииОформленияСДИЗЗЕРНО(ДокументОбъект, ДанныеЗаполнения) Экспорт
	...
	ЗаполнитьТабличныеЧастиПрикладногоДокументаНаОснованииОформленияСДИЗЗЕРНО(
		ДокументОбъект,
		СтруктураЗаполненияТабличныхЧастейПоОформлениюСДИЗЗЕРНО(РезультатЗапроса.Товары, Истина, Истина));
