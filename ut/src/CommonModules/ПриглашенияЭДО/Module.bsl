...
//  ЗаполнятьПредставлениеКонтрагента - Булево
//
// Возвращаемое значение:
//  см. ПриглашенияЭДОСлужебный.ДанныеПриглашения
Функция ДанныеПриглашения(КлючПриглашения, ЗаполнятьПредставлениеКонтрагента = Истина) Экспорт
	
	Возврат ПриглашенияЭДОСлужебный.ДанныеПриглашения(КлючПриглашения, ЗаполнятьПредставлениеКонтрагента);
...
Функция ДанныеПриглашенияПоПараметрам(ПараметрыПриглашения, ЗаполнятьПредставлениеКонтрагента = Истина) Экспорт
	...
	Возврат ДанныеПриглашения(КлючПриглашения, ЗаполнятьПредставлениеКонтрагента);
...
Функция УчастникЭДОПоКлючуИПриглашению(Знач ПараметрыПриглашения, Знач КлючУчастникаЭДО,
	КэшЦиклов = Неопределено) Экспорт
	...
	УчастникЭДОИзПриглашения = Неопределено;
	...
	КлючКэша = "";
	КэшУчастниковЭДОПоКлючамИПриглашениям = Неопределено;
	Если КэшЦиклов <> Неопределено Тогда
		Если КэшЦиклов["КэшУчастниковЭДОПоКлючамИПриглашениям"] = Неопределено Тогда
			КэшЦиклов["КэшУчастниковЭДОПоКлючамИПриглашениям"] = Новый Соответствие;
		КонецЕсли;
		КэшУчастниковЭДОПоКлючамИПриглашениям = КэшЦиклов["КэшУчастниковЭДОПоКлючамИПриглашениям"];
	КонецЕсли;
	...
	Иначе
		...
		КлючКэша = СтрШаблон("%1/%2/%3/%4",
			ПараметрыПриглашения.ИдентификаторОрганизации,
			ПараметрыПриглашения.ИдентификаторКонтрагента,
			КлючУчастникаЭДО.ИНН,
			КлючУчастникаЭДО.КПП);
		Если КэшУчастниковЭДОПоКлючамИПриглашениям <> Неопределено
			И КэшУчастниковЭДОПоКлючамИПриглашениям[КлючКэша] <> Неопределено Тогда
			Возврат КэшУчастниковЭДОПоКлючамИПриглашениям[КлючКэша];
		КонецЕсли;
		
		ЗаполнятьПредставлениеКонтрагента = Ложь;
		ДанныеПриглашения = ДанныеПриглашенияПоПараметрам(ПараметрыПриглашения, ЗаполнятьПредставлениеКонтрагента);

	КонецЕсли;

	Если Не ДанныеПриглашения = Неопределено Тогда
	...
	Если УчастникЭДОИзПриглашения = Неопределено Тогда
		Если КэшУчастниковЭДОПоКлючамИПриглашениям <> Неопределено Тогда
			КэшУчастниковЭДОПоКлючамИПриглашениям[КлючКэша] = ОписаниеУчастникаЭДО;
		КонецЕсли;
		Возврат ОписаниеУчастникаЭДО;
	КонецЕсли;
	...
	ДоступноКэшированиеРезультата = Истина;
	Если Не ЗначениеЗаполнено(УчастникЭДО) Тогда
		
		ДоступноКэшированиеРезультата = Ложь; // Кэширование не доступно, т.к. данные зависят от ДатаСведений.
	...
	Если КэшУчастниковЭДОПоКлючамИПриглашениям <> Неопределено И ДоступноКэшированиеРезультата Тогда
		КэшУчастниковЭДОПоКлючамИПриглашениям[КлючКэша] = ОписаниеУчастникаЭДО;
	КонецЕсли;
	Возврат ОписаниеУчастникаЭДО;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПараметрыСвязиИдентификаторов", Истина);
