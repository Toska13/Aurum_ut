...
//  для строк, относящихся к подсистеме СБП.
...
Процедура ПриОпределенииДоступностиКомандСпискаНастроек(
	...
	ИмяПодсистемы = НастройкиПлатежныхСистемКлиентСервер.ИмяПодсистемыСБП();
	
	Если КешПараметров.ДоступностьНастроек.Получить(ИмяПодсистемы) Тогда
		ДоступныеКоманды.Добавить("ОткрытьШаблоныНазначения");
	КонецЕсли;
	Если КешПараметров.ДоступностьКассовыхСсылок.Получить(ИмяПодсистемы) Тогда
		ДоступныеКоманды.Добавить("ОткрытьКассовыеСсылки");
	КонецЕсли;
...
//  ВидимостьКассовыхСсылок - Булево - признак использования функциональности добавления кассовых ссылок в мастере
//    настройки подключения к СБП.
Процедура ПриПодготовкеПараметровФормыСпискаНастроек(
		ВидимостьПодсистемы,
		ДоступностьНастроек,
		ВидимостьКассовыхСсылок) Экспорт
	
	Если ВидимостьПодсистемы Тогда
		ДоступностьНастроек = СистемаБыстрыхПлатежей.НастройкаПодключенияДоступна();
		НастройкиПодключенияПрограммы = СистемаБыстрыхПлатежейСлужебный.НастройкиПодключенияПрограммы();
		ВидимостьКассовыхСсылок = НастройкиПодключенияПрограммы.c2b.ИспользоватьНастройкуКассовыхСсылок;
	КонецЕсли;
...
// Определяет полные имена объектов метаданных, которые могут выступать
// в качестве оснований платежа через СБП.
//
// Параметры:
//  ИменаОбъектов - Массив Из Строка - имена объектов метаданных оснований платежа.
//
Процедура ПриОпределенииИменДокументовСКомандой(ИменаОбъектов) Экспорт
	
	ИменаДокументовСКомандой = СистемаБыстрыхПлатежейСлужебный.ИменаДокументовСКомандой();
	
	Для Каждого ИмяДокументаСКомандой Из ИменаДокументовСКомандой Цикл
		ИменаОбъектов.Добавить(ИмяДокументаСКомандой);
	КонецЦикла;
...
//
// Параметры:
//  Настройки - см. СистемаБыстрыхПлатежейСлужебный.НастройкиПодключенияПрограммы.
//
Процедура ПриОпределенииНастроекПодключенияПрограммы(Настройки) Экспорт
	...
	НастройкиСБПc2b.Вставить("ШаблоныНазначений", НастройкиПлатежныхСистемСлужебный.НовыйШаблоныНазначений());
	...
	НастройкиСБПb2b.Вставить("ШаблоныНазначений", НастройкиПлатежныхСистемСлужебный.НовыйШаблоныНазначений());
...
Процедура ПриОпределенииДоступныхВариантовНастройки(
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.СистемаБыстрыхПлатежей.ПереводыСБПb2b") Тогда
		ДоступныеВарианты.Добавить(Перечисления.ВариантыНастройкиСБП.b2b);
		ВариантНастройки = Перечисления.ВариантыНастройкиСБП.b2b;
	Иначе
		Форма.Элементы.ВариантНастройкиb2b.Видимость = Ложь;
	КонецЕсли;
...
//  ВариантНастройки - Перечисления.ВариантыНастройкиСБП - Вариант настройки СБП.
//
Процедура ПриЗаполненииПараметровВыбораНастройкиОплаты(ПараметрыВыбораОплаты, ВариантНастройки) Экспорт
	
	ПараметрыВыбораОплаты.Добавить(Новый ПараметрВыбора("Отбор.ВариантНастройки", ВариантНастройки));
