...
// Конструктор структуры, описывающей новый объект Документооборота.
//
// Параметры:
//   ID - Строка - идентификатор нового объекта Документооборота.
//   Тип - Строка - тип XDTO нового объекта Документооборота.
//   СсылкаНаОбъект - ЛюбаяСсылка - ссылка на исходный объект интегрированной системы.
//
// Возвращаемое значение:
//   Структура:
//     * ID - Строка - идентификатор нового объекта Документооборота.
...
//     * СсылкаНаОбъект - ЛюбаяСсылка - ссылка на исходный объект интегрированной системы.
//
Функция НовыйОбъектДО(ID, Тип, СсылкаНаОбъект) Экспорт
	
	НовыйОбъектДО = Новый Структура;
	НовыйОбъектДО.Вставить("ID", ID);
	НовыйОбъектДО.Вставить("Тип", Тип);
	НовыйОбъектДО.Вставить("СсылкаНаОбъект", СсылкаНаОбъект);
	
	Возврат НовыйОбъектДО;
...
Функция ПредставлениеТипаОбъектаXDTO(ТипXDTO, МножественноеЧисло = Ложь, ДляФормыВыбора = Ложь) Экспорт
	...
	ДобавитьПредставленияСправочников(МножественноеЧисло, ДляФормыВыбора, ПредставленияТипов);
	ДобавитьПредставленияПеречислений(МножественноеЧисло, ДляФормыВыбора, ПредставленияТипов);
...
Функция ТребуемоеИнтерактивноеДействие() Экспорт
	...
	ТребуемоеИнтерактивноеДействие.Вставить("ВыполнитьПодписание", Ложь);
...
// Конструктор структуры, описывающей объект для создания на основании задачи.
//
// Возвращаемое значение:
//   Структура:
//     * ПравилоИнтеграции - СправочникСсылка.ПравилаИнтеграцииС1СДокументооборотом3 - содержит ссылку на правило
//         интеграции для создания объекта ИС.
//                         - Неопределено - если требуется создать документ ИС на основании документа ЭДО.
//     * ДокументЭДО - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО
//     * ПредставлениеОбъектаИС - Строка - представление создаваемого объекта ИС.
//
Функция ОбъектДляСозданияНаОснованииЗадачи() Экспорт
	
	ПодходящийОбъект = Новый Структура;
	ПодходящийОбъект.Вставить("ПравилоИнтеграции", Неопределено);
	ПодходящийОбъект.Вставить("ДокументЭДО",
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО("", ""));
	ПодходящийОбъект.Вставить("ПредставлениеОбъектаИС", "");
	
	Возврат ПодходящийОбъект;
...
//
Функция СвязьПоID(Элементы, ID, Тип = Неопределено) Экспорт
	...
	Для Каждого Элемент Из Элементы Цикл
		Если Элемент.СвязанныйОбъектID = ID И ((Тип = Неопределено) Или (Элемент.СвязанныйОбъектТип = Тип)) Тогда
			Возврат Элемент;
		Иначе
...
Процедура ДобавитьПредставленияПеречислений(МножественноеЧисло, ДляФормыВыбора, ПредставленияТипов)
...
Процедура ДобавитьПредставленияСправочников(МножественноеЧисло, ДляФормыВыбора, ПредставленияТипов)
	
	ЕдинственноеЧисло = Не МножественноеЧисло И Не ДляФормыВыбора;
	
	Если ЕдинственноеЧисло Тогда
		Представление = НСтр("ru = 'Контрагент, Организация, Сотрудник'");
	ИначеЕсли ДляФормыВыбора Тогда
		Представление = НСтр("ru = 'Выбор стороны'");
	Иначе
		Представление = НСтр("ru = 'Контрагенты, Организации, Сотрудники'");
	КонецЕсли;
	...
	Если ЕдинственноеЧисло Тогда
		Представление = НСтр("ru = 'Вид проекта'");
	ИначеЕсли ДляФормыВыбора Тогда
		Представление = НСтр("ru = 'Выбор вида проекта'");
	Иначе
		Представление = НСтр("ru = 'Виды проектов'");
	КонецЕсли;
	...
	Если ЕдинственноеЧисло Тогда
		Представление = НСтр("ru = 'Банк'");
	ИначеЕсли ДляФормыВыбора Тогда
		Представление = НСтр("ru = 'Выбор банка'");
	Иначе
		Представление = НСтр("ru = 'Банки'");
	КонецЕсли;
	ПредставленияТипов.Вставить("DMBank", Представление);
	
	Если ЕдинственноеЧисло Тогда
		Представление = НСтр("ru = 'Страна'");
	ИначеЕсли ДляФормыВыбора Тогда
		Представление = НСтр("ru = 'Выбор страны'");
	Иначе
		Представление = НСтр("ru = 'Страны'");
	КонецЕсли;
...
Функция ФорматДатыСроков(ИспользоватьДатуИВремяВСроках)
	...
	Если ИспользоватьДатуИВремяВСроках Тогда
		Возврат НСтр("ru = 'ДФ=''dd.MM.yy HH:mm''; ДП=''не определен'''");
	Иначе
		Возврат НСтр("ru = 'ДФ=''dd.MM.yy''; ДП=''не определен'''");
	КонецЕсли;
