...
// Добавляет запрос в пакет запросов для получения реквизитов расчета акциза при пакетной обработке строк табличной части.
//
// Параметры:
//  СтруктураДействий - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ.СтруктураДействий
//  ОписаниеЗапроса - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОписаниеЗапроса
//  КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения
//
Процедура ДополнитьТекстЗапросаРеквизитыРасчетаАкциза(
	СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	
	Перем СтруктураПараметровДействия;
	
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
			"ЗаполнитьРеквизитыРасчетаАкциза",
			СтруктураДействий,
			КэшированныеЗначения,
			СтруктураПараметровДействия) Тогда
		...
		ТекстЗапроса =
			УчетПодакцизныхТоваров.ТекстЗапросаДанныеПодакцизныхТоваров(Истина, "ВтИсточникДанных");
		Если ОписаниеЗапроса.ТекстыЗапросов.НайтиПоЗначению(ТекстЗапроса) = Неопределено Тогда
			МассивТекстовЗапроса = СтрРазделить(ТекстЗапроса, ";");
			Счетчик = 1;
			Если МассивТекстовЗапроса.Количество() > 0 Тогда
				Для Каждого Текст Из МассивТекстовЗапроса Цикл
					ПозицияИз = СтрНайти(Текст, "ИЗ", НаправлениеПоиска.СНачала);
					ПозицияПоместить = СтрНайти(Текст, "ПОМЕСТИТЬ", НаправлениеПоиска.СНачала);
					НаименованиеТаблицы = Сред(Текст, ПозицияПоместить, ПозицияИз - ПозицияПоместить);
					ОписаниеЗапроса.ТекстыЗапросов.Добавить(Текст, СокрЛП(СтрЗаменить(НаименованиеТаблицы, "ПОМЕСТИТЬ", "")));
					Счетчик = Счетчик + 1;
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
		...
		Если ТекстЗапроса = Неопределено Тогда
			...
			|		 * Товары.Количество КАК НалоговаяБазаАкцизы,
			...
			|		 * Товары.Количество КАК СуммаАкциза,
			...
			|	ЕСТЬNULL(ПодакцизныеТовары.КоэффициентПересчетаЕдиницИзмерения, 0) КАК КоэффициентПересчета
			|ИЗ
			|	ВтИсточникДанных КАК Товары
		...
		ОписаниеЗапроса.ПараметрыЗапроса.Вставить("Период", ?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДатаСеанса()));
		ОписаниеЗапроса.ПараметрыЗапроса.Вставить("СтранаРегистрации",
			ЗначениеНастроекКлиентСерверПовтИсп.СтранаРегистрацииОрганизации(Организация));
		ОписаниеЗапроса.ПараметрыЗапроса.Вставить("БазоваяВалюта",  ВалютаРегламентированногоУчета);
		ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ЗаполнитьРеквизитыРасчетаАкциза");
...
// Заполняет  реквизиты Подакцизных товаров в текущей обрабатываемой строке табличной части в механизмах
// пакетной обработки строк табличных частей.
//
// Параметры:
//  ТекущаяСтрока - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ
...
Процедура ЗаполнитьРеквизитыРасчетаАкциза(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если Не СтруктураДействий.Свойство("ЗаполнитьРеквизитыРасчетаАкциза") Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеДляОбработки = ПакетнаяОбработкаТабличнойЧастиСервер.ДанныеДляОбработкиСтроки(
		"ЗаполнитьРеквизитыРасчетаАкциза", КэшированныеЗначения);
	
	Если ДанныеДляОбработки <> Неопределено Тогда
		ТекущаяСтрока.ПодакцизныйТовар            = ДанныеДляОбработки[0].ПодакцизныйТовар;
		ТекущаяСтрока.КатегорияПодакцизныхТоваров = ДанныеДляОбработки[0].КатегорияПодакцизныхТоваров;
		ТекущаяСтрока.ТвердаяСтавкаАкциза         = ДанныеДляОбработки[0].ТвердаяСтавкаАкциза;
		ТекущаяСтрока.ЕдиницаИзмеренияАкциза      = ДанныеДляОбработки[0].ЕдиницаИзмеренияАкциза;
		ТекущаяСтрока.КоэффициентПересчета        = ДанныеДляОбработки[0].КоэффициентПересчета;
		ТекущаяСтрока.НалоговаяБазаАкцизы         = ДанныеДляОбработки[0].НалоговаяБазаАкцизы;
		ТекущаяСтрока.СуммаАкциза                 = ДанныеДляОбработки[0].СуммаАкциза;
		ТекущаяСтрока.МаркируемаяПродукция        = ДанныеДляОбработки[0].МаркируемаяПродукция;
	Иначе
		ТекущаяСтрока.ПодакцизныйТовар = Ложь;
		ТекущаяСтрока.КатегорияПодакцизныхТоваров = Справочники.КатегорииПодакцизныхТоваров.ПустаяСсылка();
		ТекущаяСтрока.ТвердаяСтавкаАкциза = 0;
		ТекущаяСтрока.ЕдиницаИзмеренияАкциза = Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка();
		ТекущаяСтрока.КоэффициентПересчета = 0;
		ТекущаяСтрока.НалоговаяБазаАкцизы  = 0;
...
Функция ТекстЗапросаДанныеПодакцизныхТоваров(ИспользоватьПодакцизныеТовары, ИмяТаблицыДляОтбора) Экспорт
	...
	Если ИспользоватьПодакцизныеТовары Тогда
		...
		|	ПодакцизныеТовары.КатегорияПодакцизногоТовара КАК КатегорияПодакцизногоТовара,
