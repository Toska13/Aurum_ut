...
Процедура КритерийПримененияЗаОбъемПродажПриИзмененииНаСервере(Форма) Экспорт
	...
	УстановитьФорматЗначенияУсловияПредоставления(
		Форма.Объект.КритерийОграниченияПримененияЗаОбъемПродаж,
		Форма.Элементы.ЗначениеУсловияПредоставления);
	
	УстановитьФорматЗначенияУсловияПредоставления(
		Форма.Объект.КритерийОграниченияПримененияЗаОбъемПродаж,
		Форма.Элементы.ЗначениеУсловияПредоставленияЗаНакопленныйОбъемПродаж);
...
// Округляет значение условия ограничения.
//
// Параметры:
// 	КритерийОграничения - ПеречислениеСсылка.КритерииОграниченияПримененияСкидкиНаценкиЗаОбъемПродаж
// 	ЗначениеУсловияОграничения - Число
//
Процедура ОкруглитьЗначениеУсловияОграничения(КритерийОграничения, ЗначениеУсловияОграничения) Экспорт
	
	ЗначениеУсловияОграничения = Окр(ЗначениеУсловияОграничения,
		СоответствиеКритериевОграниченийИКоличестваРазрядов().Получить(КритерийОграничения));
...
// Возвращает соответствие количества разрядов критериям ограничений применения скидок.
//
// Возвращаемое значение:
// 	Соответствие Из ПеречислениеСсылка.КритерииОграниченияПримененияСкидкиНаценкиЗаОбъемПродаж
//
Функция СоответствиеКритериевОграниченийИКоличестваРазрядов()
	
	Результат = Новый Соответствие();
	Результат.Вставить(ПредопределенноеЗначение("Перечисление.КритерииОграниченияПримененияСкидкиНаценкиЗаОбъемПродаж.Сумма"), 2);
	Результат.Вставить(ПредопределенноеЗначение("Перечисление.КритерииОграниченияПримененияСкидкиНаценкиЗаОбъемПродаж.Количество"), 3);
	Результат.Вставить(ПредопределенноеЗначение("Перечисление.КритерииОграниченияПримененияСкидкиНаценкиЗаОбъемПродаж.КоличествоОдинаковых"), 0);
	Результат.Вставить(ПредопределенноеЗначение("Перечисление.КритерииОграниченияПримененияСкидкиНаценкиЗаОбъемПродаж.КоличествоРазличных"), 0);
	Результат.Вставить(ПредопределенноеЗначение("Перечисление.КритерииОграниченияПримененияСкидкиНаценкиЗаОбъемПродаж.ПустаяСсылка"), 0);
	
	Возврат Результат;
	
КонецФункции

// Устанавливает формат редактирования значения условия представления скидки.
//
// Параметры:
// 	КритерийОграничения - ПеречислениеСсылка.КритерииОграниченияПримененияСкидкиНаценкиЗаОбъемПродаж
// 	ЗначениеУсловияОграниченияЭлемент - ПолеФормы
//
Процедура УстановитьФорматЗначенияУсловияПредоставления(КритерийОграничения,
	ЗначениеУсловияОграниченияЭлемент)
	
	КоличествоРазрядов = СоответствиеКритериевОграниченийИКоличестваРазрядов().Получить(КритерийОграничения);
	ЗначениеУсловияОграниченияЭлемент.ФорматРедактирования = СтрШаблон(
		НСтр("ru = 'ЧДЦ=%1;'"),
		Формат(КоличествоРазрядов, НСтр("ru = 'ЧДЦ=0; ЧН=;'")));
