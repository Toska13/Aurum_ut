...
// Проверяет корректность указанного первичного документа. На основании одного первичного документа
// с типом "Отгрузка товаров клиенту" допустимо ввести только один документ ввода остатков отгруженных товаров.
// 
// Параметры:
// 	ДокументОбъект - ДокументОбъект.ВводОстатковТоваров - проверяемый документ ввода остатков.
// 	Отказ - Булево - признак отказа.
//
Процедура ПроверитьЗаполнениеПервичногоДокумента(ДокументОбъект, Отказ) Экспорт
	
	Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковОтгруженныхТоваров
		И Не ДокументОбъект.Исправление
		И ЗначениеЗаполнено(ДокументОбъект.Партия) Тогда
		
		Запрос = Новый Запрос();
		Запрос.УстановитьПараметр("Ссылка", ДокументОбъект.Ссылка);
		Запрос.УстановитьПараметр("ПервичныйДокумент", ДокументОбъект.Партия);
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|	И ТаблицаРегистра.Регистратор ССЫЛКА Документ.ВводОстатковТоваров
		|	И ТаблицаРегистра.Регистратор <> &Ссылка";
		...
		Если Выборка.Следующий() Тогда
			...
			ТекстСообщения = СтрШаблон(НСтр("ru = 'На основании документа %1 уже были введены начальные остатки:
				|%2.'"),
				ДокументОбъект.Партия,
				Выборка.Регистратор);
...
Функция ОписаниеХозяйственныхОперацийВводаОстатковВзаиморасчетовСПартнерами(УчитыватьФО)
	...
	Если УчитыватьФО Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьРетроБонусыПоставщиков") Тогда
			
			ОписаниеХозяйственнойОперации = ОписаниеХозяйственныхОперацийВводаОстатковВзаиморасчетовСПартнерами.Добавить();
			ОписаниеХозяйственнойОперации.ХозяйственнаяОперация               = Перечисления.ХозяйственныеОперации.ВводОстатковПремийПоставщиков;
			ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
			ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru = 'Ввод начальных остатков задолженности по премиям поставщиков %1'");
...
Функция ОписаниеХозяйственныхОперацийВводаОстатковТоваров()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5") Тогда
		...
		ОписаниеХозяйственнойОперации = ОписаниеХозяйственныхОперацийВводаОстатковТоваров.Добавить();
		ОписаниеХозяйственнойОперации.ХозяйственнаяОперация               = Перечисления.ХозяйственныеОперации.ВводОстатковОтгруженныхТоваров;
		ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru = 'Ввод остатков товаров, отгруженных клиентам %1'");
		ОписаниеХозяйственнойОперации.ПорядокВДереве                      = 17;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьМногооборотнуюТару") Тогда
		...
		ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru = 'Ввод остатков возвратной тары, переданной клиентам %1'");
		ОписаниеХозяйственнойОперации.ПорядокВДереве                      = 1;
		...
		ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru = 'Ввод остатков возвратной тары, принятой от поставщиков %1'");
		ОписаниеХозяйственнойОперации.ПорядокВДереве                      = 2;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьАдресноеХранение") Тогда
		...
		ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru = 'Ввод остатков при переходе на использование адресного хранения остатков %1'");
	...
	Если (ПолучитьФункциональнуюОпцию("ИспользоватьОрдернуюСхемуПриОтгрузке")
		...
		ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru = 'Ввод остатков при переходе на использование складских помещений %1'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводство")
		...
		ОписаниеХозяйственнойОперации.ДокументВводаОстатков    = ИмяДокументаВводаНачальныхОстатков;
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента = НСтр("ru = 'Ввод остатков материалов, переданных в производство %1'");
		ОписаниеХозяйственнойОперации.ПорядокВДереве           = 3;
	КонецЕсли;
	
	Если РасчетСебестоимостиПовтИсп.ИспользуетсяСреднескользящаяОценка() И Не ПолучитьФункциональнуюОпцию("БазоваяВерсия") Тогда
		ОписаниеХозяйственнойОперации = ОписаниеХозяйственныхОперацийВводаОстатковТоваров.Добавить();
		ОписаниеХозяйственнойОперации.ХозяйственнаяОперация    = Перечисления.ХозяйственныеОперации.ВводОстатковОтклоненийВСтоимостиТоваров;
		ОписаниеХозяйственнойОперации.ДокументВводаОстатков    = ИмяДокументаВводаНачальныхОстатков;
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента = НСтр("ru = 'Ввод остатков отклонений в стоимости товаров %1'");
		ОписаниеХозяйственнойОперации.ПорядокВДереве           = 17;
	КонецЕсли;
