...
//Реализация
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.АктВыполненныхРабот")
		...
		ВидОперации = Перечисления.СервисСверкиРасчетовВидыОпераций.Реализация;
...
//  ПоОтправленным - Булево - признак получения данных по отправленным документам.
//
// Возвращаемое значение:
//  РезультатЗапроса
//
Функция ДанныеИнформационнойБазыКСверке(Организация = Неопределено, ДатаНачала = '00010101', ДатаОкончания = '00010101', ПоОтправленным = Ложь) Экспорт
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СервисСверкиРасчетовУстаревшиеIDУчастниковСверки КАК УстаревшиеIDУчастниковСверки
	|		ПО РеестрСверки.Контрагент = УстаревшиеIDУчастниковСверки.УчастникСверки
	|ГДЕ
	|	ВЫБОР
	|		КОГДА &ПоОтправленным
	|			ТОГДА РеестрСверки.Статус = ЗНАЧЕНИЕ(Перечисление.СервисСверкиРасчетовСтатусыДокументов.Получено)
	|		ИНАЧЕ РеестрСверки.Статус = ЗНАЧЕНИЕ(Перечисление.СервисСверкиРасчетовСтатусыДокументов.КОтправке)
	|	КОНЕЦ
	|	И (&ПоОтправленным
	|			ИЛИ НЕ ЕСТЬNULL(КонтрагентыБизнесСеть.Идентификатор, УстаревшиеIDУчастниковСверки.Идентификатор) ЕСТЬ NULL)
	...
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ
	|	И РеестрСверки.ДатаДокумента МЕЖДУ &ДатаНачала И &ДатаОкончания
	...
	|	И ВложенныйЗапрос.Период МЕЖДУ &ДатаНачала И &ДатаОкончания
	|	
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.Период,
...
Процедура ЗарегистрироватьОснованияСФКОбмену(ДокументОбъект) Экспорт
	...
	|		ПО Основания.Ссылка = СчетФактураВыданный.Ссылка
	...
	|ВЫБРАТЬ
	|	Основания.ДокументОснование КАК Ссылка,
	|	СчетФактураВыданный.Контрагент КАК Организация,
	|	СчетФактураВыданный.Организация КАК Контрагент
	|ИЗ
	|	Документ.СчетФактураВыданный.ДокументыОснования КАК Основания
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный КАК СчетФактураВыданный
	|		ПО Основания.Ссылка = СчетФактураВыданный.Ссылка
	...
	|	И ТИПЗНАЧЕНИЯ(СчетФактураВыданный.Контрагент) = ТИП(Справочник.Организации)";
...
Функция КППНаДату(Организация, ДатаСведений) Экспорт
	...
	Возврат ОрганизацииЛокализацияПовтИсп.КППНаДату(Организация,ДатаСведений);
...
Процедура ОтчетПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Форма.Параметры.Свойство("ПараметрКоманды") Тогда
		Форма.Параметры.РежимРасшифровки = Истина;
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|ГДЕ
		|	СверкаВзаиморасчетов.Ссылка В(&Ссылка)";
		
		Запрос.УстановитьПараметр("Ссылка", Форма.Параметры.ПараметрКоманды);
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			ЗаполнитьЗначенияСвойств(Форма.Отчет, Выборка);
		КонецЕсли;
	КонецЕсли;
...
Функция ТекстЗапросаДляКнигиПродаж(ТолькоДопЛисты = Ложь, ЭтоДекларация = Ложь) Экспорт
	...
	Если ЭтоДекларация Тогда
		...
		|				КОНЕЦ
		|		ИНАЧЕ ВЫБОР
		|				КОГДА ЕСТЬNULL(Контрагенты.НаименованиеПолное, ЕСТЬNULL(КонтрагентыОрганизации.НаименованиеПолное, """")) = """"
		|					ТОГДА ЕСТЬNULL(Контрагенты.Наименование, ЕСТЬNULL(КонтрагентыОрганизации.Наименование, """"))
		|				ИНАЧЕ ЕСТЬNULL(Контрагенты.НаименованиеПолное, ЕСТЬNULL(КонтрагентыОрганизации.НаименованиеПолное, """"))
		|			КОНЕЦ
		|	КОНЕЦ КАК КонтрагентНаименование,
		|	ДанныеДляСверкиПредварительные.Контрагент КАК Контрагент,
		|	ЕСТЬNULL(Контрагенты.ИНН, ЕСТЬNULL(КонтрагентыОрганизации.ИНН, """")) КАК КонтрагентИНН,
		|	ЕСТЬNULL(Контрагенты.КПП, ЕСТЬNULL(КонтрагентыОрганизации.КПП, """")) КАК КонтрагентКПП,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Организации КАК КонтрагентыОрганизации
		|		ПО ДанныеДляСверкиПредварительные.Контрагент = КонтрагентыОрганизации.Ссылка
		...
		|		И СчетФактураВыданный.НомерСтроки = 1,
		|	Справочник.Организации КАК Организации
		|ГДЕ
		...
		|		ИНАЧЕ ЕСТЬNULL(Контрагенты.КПП, ЕСТЬNULL(КонтрагентыОрганизации.КПП, """"))
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА ДанныеДляСверкиПредварительные.Контрагент = &Организация
		|			ТОГДА Организации.КПП
...
Функция ТекстЗапросаДляКнигиПокупок(ЭтоДекларация = Ложь) Экспорт
	...
	Если ЭтоДекларация Тогда
		...
		|		КОГДА ДанныеДляСверкиПредварительные.Контрагент = &Организация
		|			ТОГДА Организации.ИНН
		|		ИНАЧЕ ЕСТЬNULL(Контрагенты.ИНН, ЕСТЬNULL(КонтрагентыОрганизации.ИНН, """"))
...
Функция ВключитьОповещенияПользователей() Экспорт
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru = 'Не удалось включить все или некоторые оповещения сервиса Сверка 2.0'") + Символы.ПС
...
Функция ОтключитьОповещенияПользователей() Экспорт
	...
	Исключение
		ВключеноУспешно = Ложь;
		ТекстОшибки = НСтр("ru = 'Не удалось отключить все или некоторые оповещения сервиса Сверка 2.0'") + Символы.ПС
			+ ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(СервисСверкиРасчетов.ИмяСервиса(), УровеньЖурналаРегистрации.Ошибка, , , ТекстОшибки);
	КонецПопытки;
