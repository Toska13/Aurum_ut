...
//  	поддерживаются значения Перечисления.НаправленияЭДО.Исходящий или Перечисления.НаправленияЭДО.Входящий
//
// Возвращаемое значение:
//  ТаблицаЗначений:
...
//
Функция АктуальныеЭлектронныеДокументы(Знач ОбъектыУчета, Направление = Неопределено) Экспорт
	...
	|	И ОбъектыУчетаДокументовЭДО.Актуальный
	|	И &УсловиеНаправление
	|";
	
	Если Направление = Перечисления.НаправленияЭДО.Исходящий
		Или Направление = Перечисления.НаправленияЭДО.Входящий Тогда

		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеНаправление", 
			"ОбъектыУчетаДокументовЭДО.ЭлектронныйДокумент Ссылка " + 
			ЭлектронныеДокументыЭДО.ИмяТаблицыДокументаЭДО(Направление = Перечисления.НаправленияЭДО.Входящий));
			
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеНаправление", "Истина");
	КонецЕсли;
...
Процедура РазорватьСвязьЭлектронногоДокументаСОбъектомУчета(ЭлектронныйДокумент, ОбъектУчета) Экспорт
	...
	Если ТипЗнч(ЭлектронныйДокумент) = Тип("ДокументСсылка.ЭлектронныйДокументВходящийЭДО")
		...
		СвойстваДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЭлектронныйДокумент, "ФорматОсновногоТитула,
			|ВидДокумента.ТипДокумента");
		Если ОбщийМодульОтражениеВУчетеЭДО().ДоступноОтражениеВУчете(СвойстваДокумента.ФорматОсновногоТитула,
			СвойстваДокумента.ВидДокументаТипДокумента) Тогда
...
Процедура ПослеЗагрузкиПодтвержденияПоСчетуФактуре(ЭлектронныйДокумент, ДокументыПодтверждения, КонтекстДиагностики) Экспорт
	...
	Если ОбщегоНазначенияБЭД.ЭтоРежимЦентрЭДО() Тогда
		...
		МодульЦентрЭДО.ПослеЗагрузкиПодтвержденияПоСчетуФактуре(ЭлектронныйДокумент, ДокументыПодтверждения);
	КонецЕсли;
...
Процедура ПослеФормированияЭлектронногоДокумента(ЭлектронныйДокумент, КонтекстДиагностики) Экспорт
	...
	Если ОбщегоНазначенияБЭД.ЭтоРежимЦентрЭДО() Тогда
		МодульЦентрЭДО = ОбщегоНазначения.ОбщийМодуль("ЦентрЭДО");
		МодульЦентрЭДО.ПослеФормированияЭлектронногоДокумента(ЭлектронныйДокумент);
	КонецЕсли;
...
Процедура ПослеЗагрузкиНовогоЭлектронногоДокумента(ЗагруженныйДокумент, КонтекстДиагностики) Экспорт
	...
	Если ОбщегоНазначенияБЭД.ЭтоРежимЦентрЭДО() Тогда
		МодульЦентрЭДО = ОбщегоНазначения.ОбщийМодуль("ЦентрЭДО");
		МодульЦентрЭДО.ПослеЗагрузкиНовогоЭлектронногоДокумента(ЗагруженныйДокумент.ЭлектронныйДокумент);
	КонецЕсли;
...
Процедура ПослеУтвержденияЭлектронногоДокумента(ЭлектронныйДокумент, КонтекстДиагностики) Экспорт
	...
	Если ОбщегоНазначенияБЭД.ЭтоРежимЦентрЭДО() Тогда
		МодульЦентрЭДО = ОбщегоНазначения.ОбщийМодуль("ЦентрЭДО");
		МодульЦентрЭДО.ПослеУтвержденияЭлектронногоДокумента(ЭлектронныйДокумент);
	КонецЕсли;
...
Процедура ПослеПодписанияЭлектронногоДокумента(ЭлектронныйДокумент, КонтекстДиагностики) Экспорт
	...
	Если ОбщегоНазначенияБЭД.ЭтоРежимЦентрЭДО() Тогда
		МодульЦентрЭДО = ОбщегоНазначения.ОбщийМодуль("ЦентрЭДО");
		МодульЦентрЭДО.ПослеПодписанияЭлектронногоДокумента(ЭлектронныйДокумент);
	КонецЕсли;
...
Процедура ПослеЗавершенияОбменаЭлектроннымДокументом(ЭлектронныйДокумент, КонтекстДиагностики) Экспорт
	...
	Если ОбщегоНазначенияБЭД.ЭтоРежимЦентрЭДО() Тогда
		МодульЦентрЭДО = ОбщегоНазначения.ОбщийМодуль("ЦентрЭДО");
		МодульЦентрЭДО.ПослеЗавершенияОбменаЭлектроннымДокументом(ЭлектронныйДокумент);
	КонецЕсли;
...
Процедура ПослеАннулированияЭлектронногоДокумента(ЭлектронныйДокумент, КонтекстДиагностики) Экспорт	
	...
	Если ОбщегоНазначенияБЭД.ЭтоРежимЦентрЭДО() Тогда
		МодульЦентрЭДО = ОбщегоНазначения.ОбщийМодуль("ЦентрЭДО");
		МодульЦентрЭДО.ПослеАннулированияЭлектронногоДокумента(ЭлектронныйДокумент);
	КонецЕсли;
...
// См. ЭлектронныеДокументыЭДОСобытия.ПослеОтклоненияЭлектронногоДокумента
//
Процедура ПослеОтклоненияЭлектронногоДокумента(ЭлектронныйДокумент, КонтекстДиагностики) Экспорт
	
	Если ЭлектронныеДокументыЭДО.ЭтоВходящийЭДО(ЭлектронныйДокумент) Тогда
		ОтражениеВУчетеЭДО.ОтключитьКонтрольСопоставленияНоменклатурыПоДокументу(ЭлектронныйДокумент);
	КонецЕсли;
	
КонецПроцедуры

// См. ЭлектронныеДокументыЭДОСобытия.ПослеПринудительногоЗакрытияЭлектронногоДокумента
//
Процедура ПослеПринудительногоЗакрытияЭлектронногоДокумента(ЭлектронныйДокумент, КонтекстДиагностики) Экспорт
...
// См. ЭлектронныеДокументыЭДОСобытия.ПослеВозвращенияВРаботуЭлектронногоДокумента
//
Процедура ПослеВозвращенияВРаботуЭлектронногоДокумента(ЭлектронныйДокумент, КонтекстДиагностики) Экспорт
	...
	СвойстваДокумента = 
		ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЭлектронныйДокумент, "ВидДокумента, ФорматОсновногоТитула");
	ТипДокумента = ТипыДокументовПоВиду[СвойстваДокумента.ВидДокумента];
	
	Если Не ОтражениеВУчетеЭДО.ДоступноОтражениеВУчете(СвойстваДокумента.ФорматОсновногоТитула, ТипДокумента) Тогда
		Возврат;
	КонецЕсли;
	
	Документ = Новый Структура("СпособОбработки");
	Настройки = Новый Структура("ОтражатьТолькоСопоставленные", Истина);
	СопоставлениеНоменклатурыКонтрагентовПереопределяемый.ПриОтраженииВУчетеЭлектронногоДокументаСНоменклатурой(
		Документ, Настройки);
		
	Если Не Настройки.ОтражатьТолькоСопоставленные Тогда
		Возврат;
	КонецЕсли;
	
	ОтражениеВУчетеЭДО.ВыполнитьКонтрольСопоставленияНоменклатурыДокументов(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ЭлектронныйДокумент), КонтекстДиагностики);
...
Процедура ПослеЗагрузкиИнформацииПолучателяПоАктуСверкиВзаиморасчетов(ЭлектронныйДокумент, ОписаниеДанных, КонтекстДиагностики) Экспорт	
	...
	Если ОбщегоНазначенияБЭД.ЭтоРежимЦентрЭДО() Тогда
		МодульЦентрЭДО = ОбщегоНазначения.ОбщийМодуль("ЦентрЭДО");
		МодульЦентрЭДО.ПослеЗагрузкиИнформацииПолучателяПоАктуСверкиВзаиморасчетов(ЭлектронныйДокумент);
	КонецЕсли;
...
Процедура ПослеЗагрузкиДополнительныхСведенийПоАктуОРасхождениях(ЭлектронныйДокумент, ОписаниеДанных, КонтекстДиагностики) Экспорт	
	...
	Если ОбщегоНазначенияБЭД.ЭтоРежимЦентрЭДО() Тогда
		МодульЦентрЭДО = ОбщегоНазначения.ОбщийМодуль("ЦентрЭДО");
		МодульЦентрЭДО.ПослеЗагрузкиДополнительныхСведенийПоАктуОРасхождениях(ЭлектронныйДокумент);
	КонецЕсли;
...
Процедура ПередЗаписьюОбъектаУчетаДокументаЭДО(Источник, Отказ, ЗаписьЭлементаСправочника = Ложь)
	...
	ЭтоЗаписьОбъектаПриСинхронизацииДанных = Не УчетныеЗаписиЭДО.ЕстьУчетныеЗаписиЭДО();
	
	Если Не ДокументооборотНастроен И Не ЭтоЗаписьОбъектаПриСинхронизацииДанных Тогда
...
Процедура ЗакрытьПринудительноЭлектронныйДокумент(Источник)
	...
	АктуальныеЭлектронныеДокументы = АктуальныеЭлектронныеДокументы(
		Источник.Ссылка, Перечисления.НаправленияЭДО.Исходящий).ВыгрузитьКолонку("ЭлектронныйДокумент");
...
Функция СостоянияЭДООбъектовУчета(Знач ОбъектыУчета, Знач ВидДокумента = Неопределено)
	...
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	ЕСТЬNULL(СостоянияДокументовЭДО.Состояние, ЗНАЧЕНИЕ(Перечисление.СостоянияДокументовЭДО.НеСформирован)) КАК
		|		СостояниеЭД
		|ИЗ
		|	РегистрСведений.ОбъектыУчетаДокументовЭДО КАК ОбъектыУчетаДокументовЭДО
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияДокументовЭДО КАК СостоянияДокументовЭДО
		|		ПО СостоянияДокументовЭДО.ЭлектронныйДокумент = ОбъектыУчетаДокументовЭДО.ЭлектронныйДокумент
		|ГДЕ
		|	ОбъектыУчетаДокументовЭДО.ОбъектУчета В (&ОбъектыУчета)
		|	И ОбъектыУчетаДокументовЭДО.Актуальный
		|	И ОбъектыУчетаДокументовЭДО.ЭлектронныйДокумент.ВидДокумента = &ВидДокумента");
	...
	ЗаменяемаяЧастьТекстаЗапроса = "И ОбъектыУчетаДокументовЭДО.ЭлектронныйДокумент.ВидДокумента = &ВидДокумента";
	Если Не СтрНайти(Запрос.Текст, ЗаменяемаяЧастьТекстаЗапроса) Тогда
		// Запрос всегда должен быть в консистентном состоянии с частью текста запроса.
		ВызватьИсключение НСтр("ru = 'Не найдена заменяемая часть текста запроса.'");
	КонецЕсли;
	Если ЗначениеЗаполнено(ВидДокумента) Тогда
		Запрос.Параметры.Вставить("ВидДокумента", ВидДокумента);
	Иначе
		ТекстЗамены = "";
		Запрос.Текст = СтрЗаменить(Запрос.Текст, ЗаменяемаяЧастьТекстаЗапроса, ТекстЗамены);
	КонецЕсли;
...
// * GLN - Строка
//
Функция РеквизитыКонтрагента(Реквизиты)
