			...
			Иначе
				...
				Если Не ПустаяСтрока(СтрокаДанных.ТелефонКонтрагента) Тогда
					СтрокаДанных.НаименованиеКонтрагента =
						СтрокаДанных.НаименованиеКонтрагента
						+ ", " + СтрШаблон(НСтр("ru = 'тел. %1'"), СтрокаДанных.ТелефонКонтрагента);
				КонецЕсли;
			КонецЕсли;
...
Функция ПодтвердитьОтгрузку(УчетнаяЗапись, ИдентификаторОтгрузки) Экспорт    
	...
	Если  ОтветСервиса.Ошибка<>Неопределено И ЗначениеЗаполнено(ОтветСервиса.Ошибка.ОписаниеОшибки) Тогда  
		Если СтрНайти(ОтветСервиса.Ошибка.ОписаниеОшибки,"Reasons: orders are in wrong status")>0 Тогда   
			ОтветСервиса.Ошибка.ОписаниеОшибки = СтрЗаменить(ОтветСервиса.Ошибка.ОписаниеОшибки, НСтр("ru = 'по причине:'"), НСтр("ru = 'по причине: отгрузку невозможно подтвердить, некоторые заказы отгрузки в некорректном статусе.'"));
		КонецЕсли;
	КонецЕсли;	
...
Функция ПеренестиЗаказыВСледующуюОтгрузкуВСервисе(УчетнаяЗапись, ИдентификаторОтгрузки, ИдентификаторыЗаказов)
	...
	Если ОтветСервиса.Ошибка<>Неопределено И ЗначениеЗаполнено(ОтветСервиса.Ошибка.ОписаниеОшибки) Тогда  
		Если СтрНайти(ОтветСервиса.Ошибка.ОписаниеОшибки,"already have processed requests for exclusion from shipment")>0 Тогда   
			ОтветСервиса.Ошибка.ОписаниеОшибки = СтрЗаменить(ОтветСервиса.Ошибка.ОписаниеОшибки, НСтр("ru = 'по причине:'"), НСтр("ru = 'по причине: выбранные заказы уже переносились в следующую отгрузку, перенос возможен однократно.'"));
		КонецЕсли;
	КонецЕсли;
