...
Процедура ДополнитьЗначенияКонстантКонтролируемыхВБазовойВерсии(Результат) Экспорт 
	...
	Результат.Вставить(Метаданные.Константы.ИспользоватьКонтрольКачестваТоваров.Имя, Ложь);
	...
	Результат.Вставить(Метаданные.Константы.НеИспользоватьУправлениеДоставкой20.Имя, Ложь);
	Результат.Вставить(Метаданные.Константы.НеИспользоватьУправлениеДоставкой25.Имя, Ложь);
...
Процедура ДополнитьКонстантыДоступныеВБазовойВерсии(Результат) Экспорт 
	...
	Результат.Вставить(Метаданные.Константы.ИспользоватьНеуникальныеШтрихкодыНоменклатуры.Имя);
...
Процедура СохранитьЗначениеРеквизита_НастройкиНоменклатуры(КонстантаИмя, КонстантаЗначение, Форма) Экспорт
	...
	Если КонстантаИмя <> "" Тогда
		...
		Если КонстантаИмя = "ИспользоватьВыплатыСамозанятым" Тогда
			ОбщегоНазначенияУТКлиентСервер.ОтображениеПредупрежденияПриРедактировании(
				Форма.Элементы.ИспользоватьВыплатыСамозанятым, КонстантаЗначение);
		КонецЕсли;
...
Процедура ПриСозданииНаСервере_Продажи(Форма) Экспорт
	...
	Элементы.ПояснениеИспользованиеЭтаповОплатыВПродажах.Заголовок = НСтр(
		"ru = 'Настройка определяет возможные варианты планирования оплаты в документах ""Заказ клиента"", ""Отчет комиссионера"", ""Заявка на возврат товаров от клиента"".'");
