...
Процедура ОбработкаЗаполненияДокумента(Объект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.КоммерческоеПредложениеКлиенту") Тогда
		...
		СтруктураЗаполненияСтавкиНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект);
...
// Параметры заполнения условий закупки по коммерческим предложениям.
// 
// Возвращаемое значение:
//  Структура - Параметры заполнения условий закупки по коммерческим предложениям:
// * Документ - ДокументСсылка.ЗаявкаНаЗакупку, ДокументСсылка.ЗаявкаНаОбеспечение - Документ-основание запроса коммерческих предложений.
// * ДатаКурса - Дата - Для которой будут использоваться курсы валют при пересчете цен в валюту заполняемого документа.
// * Валюта - СправочникСсылка.Валюты - В которую будут пересчитываться полученные цены для заполнения документа.
// * ЦенаВключаетНДС - Неопределено, Булево - При указании значения флага из документа источника при необходимости пересчитывает цену в соответствии с этим флагом.
// * ЕстьПартнер - Булево - Флаг, определяющий наличие колонки "Партнер" в заполняемой таблице.
// * ЕстьКонтрагент - Булево - Флаг, определяющий наличие колонки "Контрагент" в заполняемой таблице.
// * ЕстьНоменклатураСтрокой - Булево - Флаг, определяющий наличие колонки "НоменклатураСтрокой" в заполняемой таблице.
// * НомераСтрок - Массив Из Число - Номера строк заполняемой таблицы, в которых необходимо заполнить цены.
// * ДействияСИзмененнымиСтроками - Структура - Описание действий пакетной обработки, которые будут выполнены с измененными строками:
// ** ПересчитатьКоличествоУпаковок - Неопределено - Действие для пересчета количества в количество упаковок, т.к. распределение цен выполняется по единицам хранения.
Функция ПараметрыЗаполненияУсловийЗакупкиПоКоммерческимПредложениям() Экспорт 
	
	ВозвращаемоеЗначение = Новый Структура;
	
	ВозвращаемоеЗначение.Вставить("Документ", Документы.ЗаявкаНаЗакупку.ПустаяСсылка());
	...
	ВозвращаемоеЗначение.Вставить("ЕстьПартнер", Ложь);
	ВозвращаемоеЗначение.Вставить("ЕстьКонтрагент", Ложь);
	ВозвращаемоеЗначение.Вставить("ЕстьНоменклатураСтрокой", Ложь);
	ВозвращаемоеЗначение.Вставить("НомераСтрок", Новый Массив);
	
	ВозвращаемоеЗначение.Вставить("ДействияСИзмененнымиСтроками", Новый Структура("ПересчитатьКоличествоУпаковок"));
...
Функция ИспользоватьЗапросыКоммерческихПредложенийПоставщиков() Экспорт
	Возврат КоммерческиеПредложенияДокументыЛокализация.ИспользоватьЗапросыКоммерческихПредложенийПоставщиков();
КонецФункции
...
// * СтрокиСНезаполненнымиУсловиямиЗакупки - Массив Из СтрокаТаблицыЗначений, ДанныеФормыЭлементКоллекции - строки, для которых отсутствуют условия закупки.
...
Функция ЗаполнитьУсловияЗакупкиПоКоммерческимПредложениям(Товары, ПараметрыЗаполнения) Экспорт
	...
	Возврат КоммерческиеПредложенияДокументыЛокализация.ЗаполнитьУсловияЗакупкиПоКоммерческимПредложениям(Товары, ПараметрыЗаполнения);
...
// См. КоммерческиеПредложенияДокументыПереопределяемый.ПриОпределенииЕдиницыИзмеренияПоУмолчанию.
//
Процедура ПриОпределенииЕдиницыИзмеренияПоУмолчанию(ЕдиницаИзмерения) Экспорт

	КодЕдиницыИзмерения = "796";
	ЕдиницаИзмерения = Справочники.УпаковкиЕдиницыИзмерения.НайтиПоКоду(КодЕдиницыИзмерения);
...
// Вызывается при копировании документа.
//
// Параметры:
//  ДокументОбъект - ДокументОбъект - заполняемый документ.
Процедура ПриКопировании(ДокументОбъект) Экспорт
	
	ДокументОбъект.ДокументОснование = Неопределено;
	ДокументОбъект.ИдентификаторЗКПВСервисе = Неопределено;
	
	Для Каждого СтрокаТоваров Из ДокументОбъект.Товары Цикл
		
		СтрокаТоваров.ИдентификаторСтрокиЗапроса = Неопределено;
		СтрокаТоваров.ИдентификаторДляСопоставленияНоменклатуры = Неопределено;
		СтрокаТоваров.НоменклатураПокупателяИдентификатор = Неопределено;
		СтрокаТоваров.НоменклатураПокупателяПредставление = Неопределено;
		СтрокаТоваров.НоменклатураВСервисеИдентификатор = Неопределено;
		СтрокаТоваров.ХарактеристикаВСервисеИдентификатор = Неопределено;
		
	КонецЦикла;
	
	СтруктураПересчетаСуммы = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВТЧ(ДокументОбъект);
	СтруктураЗаполненияСтавкиНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(ДокументОбъект);
	СтруктураЗаполненияСтавкиНДС.ИнициализацияВходящегоДокумента = Истина;
	КэшированныеЗначения = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
...
Процедура ПриВставкеСтрокИзБуфераОбмена(Форма, КоличествоСтрок, СтандартнаяОбработка) Экспорт
	...
	ПараметрыДействия.НалогообложениеНДС = Форма.НалогообложениеНДС;
	...
	Если ДанныеБуфера.Колонки.Найти("СуммаРучнойСкидки") <> Неопределено Тогда
...
#Область ПараметрыВыбора
	...
	МассивТиповНоменклатуры = НоменклатураКлиентСервер.ОтборПоТоваруМногооборотнойТареУслугеРаботе(Ложь);
...
Процедура КоммерческоеПредложениеКлиентуПриИзмененииНалогообложенияНДС(Форма)
	...
	СтруктураЗаполненияСтавкиНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект);
	СтруктураЗаполненияСтавкиНДС.Вставить("НалогообложениеНДС", Форма.НалогообложениеНДС);
...
Процедура КоммерческоеПредложениеКлиентуСоздатьДополнительныеРеквизитыФормы(Форма)
	...
	Если Не Форма.ДополнительныеРеквизитыСозданы Тогда
		...
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ИспользоватьГрафикиОплаты",
		                                                  Новый ОписаниеТипов("Булево")));
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ИспользуетсяЦенообразование25",
		                                                  Новый ОписаниеТипов("Булево")));
...
Процедура КоммерческоеПредложениеКлиентуОбработатьВводШтрихкода(Форма, ДанныеШтрихкода, РезультатОбработки)
	...
	ПараметрыЗаполнитьСтавкуНДС =  ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДСПоВидуЦен(Объект);
	ПараметрыЗаполнитьСтавкуНДС.НалогообложениеНДС = Форма.НалогообложениеНДС;
	СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьСтавкуНДСПоВидуЦен", ПараметрыЗаполнитьСтавкуНДС);
