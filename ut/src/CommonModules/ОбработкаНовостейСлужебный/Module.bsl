...
Процедура ПередЗапускомПрограммы() Экспорт
	...
	СтандартнаяОбработка = Истина; // Для отключения надо явно установить в Ложь.
	ИнтеграцияПодсистемБИП.ПередЗапускомПрограммыПроверитьИзменениеВерсииПлатформы(СтандартнаяОбработка);
	ОбработкаНовостейПереопределяемый.ПередЗапускомПрограммыПроверитьИзменениеВерсииПлатформы(СтандартнаяОбработка);
	Если СтандартнаяОбработка <> Ложь Тогда
		Если ОбработкаНовостей.РазрешенаРаботаСНовостямиТекущемуПользователю() Тогда // На текущий момент еще не установлен СтатусПодсистемы.
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск");
	Методы.Вставить("ОбновлениеИнформационнойБазы_ОбластьДанных_ПерейтиНаВерсию");
	Методы.Вставить("ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск");
	Методы.Вставить("ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию");
	Методы.Вставить("УстановкаПараметровСеанса");
	Методы.Вставить("ПересчитатьОтборыПоНовостям_РедкоМеняющиеся", Истина);
...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	НовоеИсключение = Настройки.ИсключенияПроверки.Добавить();
	НовоеИсключение.ПолноеИмяОбъекта = "ОбщийМодуль.ОбработкаНовостейСлужебный";
	НовоеИсключение.ИмяПроцедуры = "ПроверитьИзменениеВерсииПлатформы";
	НовоеИсключение.ФрагментСодержания = "ФоновоеЗадание = ДлительныеОперации.ВыполнитьВФоне(ИмяМетода, КонтекстВыполненияВложенный, ПараметрыВыполнения)";
	
	МетодыОбработкаНовостей = Новый Соответствие;
	МетодыОбработкаНовостей.Вставить("ПересчитатьОтборыПоНовостям_РедкоМеняющиеся", Истина);
	
	Настройки.ИсключенияКогдаУказанныйМетодНеВызывается.Вставить(
		"ОбщийМодуль.ОбработкаНовостейСлужебный",
		МетодыОбработкаНовостей);
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		Если ДляРедактирования = Истина Тогда
			Результат = ХранилищаНастроек.НастройкиНовостей.Загрузить(
				"НастройкиПоказаНовостейНастроенныеАдминистраторомДляОбластиДанныхДляРедактирования", // АПК:1297 Это идентификатор.
				"");
		Иначе
			Результат = ХранилищаНастроек.НастройкиНовостей.Загрузить(
				"НастройкиПоказаНовостейНастроенныеАдминистраторомДляОбластиДанных", // АПК:1297 Это идентификатор.
				"");
		КонецЕсли;
	Иначе
		Результат = ХранилищаНастроек.НастройкиНовостей.Загрузить(
			"НастройкиПоказаНовостейНастроенныеАдминистратором", // АПК:1297 Это идентификатор.
			"");
	КонецЕсли;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ОбновлениеПериодическихСвойствНовостей"; // АПК:1297 Это идентификатор.
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru='Обновление периодических свойств новостей'")); // АПК:1297 Это идентификатор.
...
#Область Обработка_ОбновлениеРасписания
	...
	НачатьШагВыполнения(КонтекстВыполнения, "ОбновлениеРасписания"); // АПК:1297 Это идентификатор.
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ОбновлениеПривязокКМетаданным"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru='Обновление привязок к метаданным'")); // АПК:1297 Это идентификатор.
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ОптимизироватьОтборыПоНовостям"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_ВыполнениеЗапроса
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ВыполнениеЗапроса/УдалениеОтборовПоНесуществующимКатегориям"); // АПК:1297 Это идентификатор.
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_ДляОбластиДанных
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ОбработкаРезультатаЗапроса/УдалениеОтборовПоНесуществующимКатегориям_ДляОбластиДанных"); // АПК:1297 Это идентификатор.
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_Пользовательские
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ОбработкаРезультатаЗапроса/УдалениеОтборовПоНесуществующимКатегориям_Пользовательские"); // АПК:1297 Это идентификатор.
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_Общие
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ОбработкаРезультатаЗапроса/УдалениеОтборовПоНесуществующимКатегориям_Общие"); // АПК:1297 Это идентификатор.
...
#Область Оптимизация_УдалениеЗначенийОтборов_ДляОбластиДанных
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:УдалениеЗначенийОтборов_ДляОбластиДанных"); // АПК:1297 Это идентификатор.
...
#Область Оптимизация_УдалениеЗначенийОтборов_Пользовательские
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:УдалениеЗначенийОтборов_Пользовательские"); // АПК:1297 Это идентификатор.
...
#Область Оптимизация_ЛишниеКатегорииВНовостях
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:Оптимизация_ЛишниеКатегорииВНовостях"); // АПК:1297 Это идентификатор.
...
//  КонтекстВыполнения - Структура, Неопределено - структура контекста выполнения.
//
Процедура ПересчитатьОтборыПоНовостям_РедкоМеняющиеся(КонтекстВыполнения = Неопределено, АдресРезультата = Неопределено) Экспорт

#Область ИнициализацияМетода

	НаименованиеМетода = "ОбработкаНовостейСлужебный.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Проверки
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:ПраваДоступа"); // АПК:1297 Это идентификатор.
	Если (НЕ ПривилегированныйРежим())
		...
		КонтекстВыполнения.ЖурналРегистрации.Уровень = УровеньЖурналаРегистрации.Ошибка;
		ЗаписатьКонтекстВыполненияВЖурналРегистрации(КонтекстВыполнения);
		Возврат;
	КонецЕсли;

	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:КатегорииЗагружены"); // АПК:1297 Это идентификатор.
...
#Область СохранениеВерсииПлатформы
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:СохранениеВерсииПлатформы"); // АПК:1297 Это идентификатор.
...
#Область ОсновныеДействия
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ВыполнениеЗапроса/Пересчет"); // АПК:1297 Это идентификатор.
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ОбработкаРезультатаЗапроса/Пересчет"); // АПК:1297 Это идентификатор.
	Если НЕ Результат.Пустой() Тогда
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ПересчитатьОтборыПоНовостям_Общие"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ПересчитатьОтборыПоНовостям_ДляОбластиДанных"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ПересчитатьОтборыПоНовостям_Пользовательские"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_ПолучениеСпискаПользователей
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ВыполнениеЗапроса/ПолучениеСпискаПользователей"); // АПК:1297 Это идентификатор.
...
#Область Обработка_ОчисткаПредыдущегоРезультата
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ОчисткаПредыдущегоРезультата"); // АПК:1297 Это идентификатор.
...
Функция Проверка_ДоступныНеразделенныеДанные(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если КонтекстВыполнения <> Неопределено Тогда
		НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru='Проверка:ДоступныНеразделенныеДанные'")); // АПК:1297 Это идентификатор.
	КонецЕсли;
...
Функция Проверка_ДоступныРазделенныеДанные(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если КонтекстВыполнения <> Неопределено Тогда
		НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru='Проверка:ДоступныРазделенныеДанные'")); // АПК:1297 Это идентификатор.
	КонецЕсли;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ЗагрузитьВсеСтандартныеЗначения"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.СоединитьсяССерверомНовостей"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Проверки
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:Протокол"); // АПК:1297 Это идентификатор.
	Если ((ВРег(Протокол) <> ВРег("http")))
		...
		КонтекстВыполнения.ЖурналРегистрации.Уровень = УровеньЖурналаРегистрации.Предупреждение;
		ЗаписатьКонтекстВыполненияВЖурналРегистрации(КонтекстВыполнения);
		Возврат Ложь;
	КонецЕсли;

	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:Порт"); // АПК:1297 Это идентификатор.
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:НачалоПопытокСоединения"); // АПК:1297 Это идентификатор.
	...
	Пока (СоединениеУстановлено <> Истина)
		...
		НачатьШагВыполнения(КонтекстВыполнения, "Основные:ПопыткаСоединения"); // АПК:1297 Это идентификатор.
		Попытка
...
#Область Обработка
	...
	Если (СостояниеПодсистемы = "Активна")
		...
		Запрос.УстановитьПараметр("ИдентификаторИконкиНовости"       , "ИконкаВажнойНовостиДляОповещенияПользователя"); // АПК:1297 Это идентификатор.
				...
				Если Выборка.Важность = 1 Тогда // Очень важная
					ОписаниеНовости = Новый Структура;
						...
						Если Выборка.СпособОповещенияСсылка = Перечисления.СпособыОповещенияПользователяОНовостях.Отключено Тогда
							ОписаниеНовости.Вставить("СпособОповещения", "Отключено"); // АПК:1297 Это идентификатор.
						ИначеЕсли Выборка.СпособОповещенияСсылка = Перечисления.СпособыОповещенияПользователяОНовостях.СистемаВзаимодействия Тогда
							ОписаниеНовости.Вставить("СпособОповещения", "СистемаВзаимодействия"); // АПК:1297 Это идентификатор.
						Иначе
							ОписаниеНовости.Вставить("СпособОповещения", "ПоУмолчанию"); // АПК:1297 Это идентификатор.
						КонецЕсли;
			...
			Для Каждого ТекущаяЛентаНовостей Из МассивЛентНовостей Цикл
				Если ВРег(ТекущийМакет.Имя) = (ВРег("СтандартныеЗначения_" + ТекущаяЛентаНовостей.ЛентаНовостейКод)) Тогда
					...
					ВсеПоставляемыеДанные.Вставить(
						"Справочник.Новости." + ТекущийМакет.Имя, // АПК:1297 Это идентификатор.
						СодержимоеМакета);
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
...
#Область ЗаполнениеШаблонов_Новости
	...
	Если МассивСсылокНовостей.Количество() > 0 Тогда
		...
		Для Каждого ТекущаяНовость Из МассивСсылокНовостей Цикл
			...
			НовостьXDTO = СериализаторXDTO.ЗаписатьXDTO(НовостьОбъект);
			ПредставлениеНовостьXDTO = ОбработкаНовостейКлиентСервер.ВставитьОтступВМногострочнуюСтроку(
				ПредставлениеОбъектаХДТО(НовостьXDTO),
				Символы.Таб,
				Истина);
...
#Область ЗаполнениеШаблонов_Новости_ОбщиеДанные
			...
			РезультатТекущейНовости = СтрЗаменить(РезультатТекущейНовости, "%ЭлементСправочникаНовости%", ПредставлениеНовостьXDTO);
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_Константы
	...
	ЦентрализованноРазрешенаРаботаСНовостямиЧерезИнтернет = РазделениеВключено ИЛИ РазрешенДоступКИнтернетСервисам;
	...
	РезультатДанныеОПрограммномОкружении_Константы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Константа.НастройкиНовостей:
			...
			|Централизованное разрешение доступа к интернет-сервисам: %4
			|	РазделениеВключено: %5
			|	РазрешенДоступКИнтернетСервисам: %6'"),
		...
		ЦентрализованноРазрешенаРаботаСНовостямиЧерезИнтернет,
		РазделениеВключено,
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_ДругиеПараметрыПрограммногоОкружения
	...
	ВременныйТекст = НСтр("ru='Строка подключения: %СтрокаСоединенияИнформационнойБазы%
		...
		|ОбщегоНазначения.РазрешенДоступКИнтернетСервисам(): %ОбщегоНазначения_РазрешенДоступКИнтернетСервисам%
	...
	ВременныйТекст = СтрЗаменить(ВременныйТекст, "%ОбщегоНазначения_РазрешенДоступКИнтернетСервисам%", РазрешенДоступКИнтернетСервисам);
...
#Область Обработка
	...
	Если Вариант = "Для форм, все поля" Тогда // АПК:1297 это идентификатор.
		...
		Запрос.УстановитьПараметр("СпособОповещенияОтключеноСсылка"  , Перечисления.СпособыОповещенияПользователяОНовостях.Отключено);
		Запрос.УстановитьПараметр("СпособОповещенияПоУмолчаниюСсылка", Перечисления.СпособыОповещенияПользователяОНовостях.ПоУмолчанию);
		Запрос.УстановитьПараметр("СпособОповещенияПоУмолчаниюСтрока", "ПоУмолчанию"); // АПК:1297 Это идентификатор.
		...
		Если НЕ Результат.Пустой() Тогда
			...
			Для Каждого ТекущаяНовость Из Таблица Цикл
				...
				Если ТекущаяНовость.СпособОповещенияСсылка = Перечисления.СпособыОповещенияПользователяОНовостях.Отключено Тогда
					ТекущаяНовость.СпособОповещения = "Отключено"; // АПК:1297 Это идентификатор.
				ИначеЕсли ТекущаяНовость.СпособОповещенияСсылка = Перечисления.СпособыОповещенияПользователяОНовостях.СистемаВзаимодействия Тогда
					ТекущаяНовость.СпособОповещения = "СистемаВзаимодействия"; // АПК:1297 Это идентификатор.
				Иначе
					ТекущаяНовость.СпособОповещения = "ПоУмолчанию"; // АПК:1297 Это идентификатор.
				КонецЕсли;
			КонецЦикла;
			Если НастройкиПолученияНовостейЭталон.ПолучатьКатегорииНовостей = Истина Тогда
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ВсеОбновленияНовостей"; // АПК:1297 Это идентификатор.
	КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	...
	ЦентрализованноРазрешенаРаботаСНовостямиЧерезИнтернет = ОбщегоНазначения.РазделениеВключено() ИЛИ ОбщегоНазначения.РазрешенДоступКИнтернетСервисам();
	ЛокальноРазрешенаРаботаСНовостямиЧерезИнтернет        = ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостямиЧерезИнтернет();
	ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет           = ЦентрализованноРазрешенаРаботаСНовостямиЧерезИнтернет И ЛокальноРазрешенаРаботаСНовостямиЧерезИнтернет;
...
#Область Обработка_ЗагрузкаНовостей
	...
	// 1. Получение и обработка новостей из лент новостей (с обязательным заполнением измерения "ЛентаНовостей").
	Операция = "ЗагрузкаНовостей"; // АПК:1297 Это идентификатор.
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "ПолучитьИОбработатьНовостиПоЛентамНовостей"); // АПК:1297 Это идентификатор.
		...
		Запрос.Текст = "
			...
			|	)
			|";
		Запрос.УстановитьПараметр("РазрешенаРаботаСНовостямиЧерезИнтернет", ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет);
...
#Область Обработка_ОбновлениеКлассификаторов
	...
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "ОбновитьСтандартныеСпискиДляНовостей_Ежедневно"); // АПК:1297 Это идентификатор.

		Если ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет = Истина Тогда
...
#Область Обработка_АвтоудалениеНовостей
	...
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "УдалитьНовости_Ежедневно"); // АПК:1297 Это идентификатор.
...
#Область Обработка_ОбновлениеПериодическихСвойств
	...
	Операция = "БИП:Новости/ОбновлениеПериодическихСвойств"; // Идентификатор операции расписания.
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "ОбновлениеПериодическихСвойствНовостей"); // АПК:1297 Это идентификатор.
...
#Область Обработка_ОбновлениеПривязокКМетаданным
	...
	Операция = "БИП:Новости/ОбновлениеПривязокКМетаданным"; // Идентификатор операции расписания.
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "ОбновлениеПривязокКМетаданным"); // АПК:1297 Это идентификатор.
...
#Область Обработка_ПроверкаВерсииПлатформы
	...
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "ПроверитьИзменениеВерсииПлатформы"); // АПК:1297 Это идентификатор.
...
#Область Обработка_ЗаполнитьПрограммныеЗначенияКатегорий_Общие
	...
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "ЗаполнитьПрограммныеЗначенияКатегорий_Общие"); // АПК:1297 Это идентификатор.
...
#Область Обработка_ВсеОбновленияНовостейДляОбластиДанных
	...
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "ВсеОбновленияНовостейДляОбластиДанных"); // АПК:1297 Это идентификатор.
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ВсеОбновленияНовостейДляОбластиДанных"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_ЗаполнитьПрограммныеЗначенияКатегорий_ДляОбластиДанных
	...
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "ЗаполнитьПрограммныеЗначенияКатегорий_ДляОбластиДанных"); // АПК:1297 Это идентификатор.
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.УдалитьНовости_Ежедневно"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_ПолучитьСписокДанныхДляУдаления
	...
	НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru='УдалениеНовостей_ПолучитьСписокДанныхДляУдаления'")); // АПК:1297 Это идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_Удаление
	...
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, НСтр("ru='УдалениеНовостей_Удаление'")); // АПК:1297 Это идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ПолучитьИОбработатьНовостиПоЛентамНовостей"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
	...
	ДобавитьШагВыполнения(КонтекстВыполнения, "Получение:РазрешенаРаботаСНовостямиЧерезИнтернет", "",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Массив лент новостей для обработки: %1
				...
				|Централизованное разрешение доступа к интернет-сервисам: %3
				|	РазделениеВключено: %4
				|	РазрешенДоступКИнтернетСервисам: %5'"),
			ОбработкаНовостейКлиентСервер.ПредставлениеЗначения(МассивЛентНовостей, ";"),
			ЛокальноРазрешенаРаботаСНовостямиЧерезИнтернет,
			ЦентрализованноРазрешенаРаботаСНовостямиЧерезИнтернет,
			РазделениеВключено,
			РазрешенДоступКИнтернетСервисам));
...
#Область Обработка_ВыполнениеЗапроса
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ВыполнениеЗапроса"); // АПК:1297 Это идентификатор.
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, "WARN.QUERY.EMPTY",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='ПолучитьИОбработатьНовостиПоЛентамНовостей, не удалось определить список лент новостей
					...
					|ИЛИ включен запрет получение новостей из интернет,
					|ИЛИ включен централизованный запрет на работу с интернет-сервисами.'"),
				МассивЛентНовостей.Количество()));
	КонецЕсли;
...
#Область Обработка_ОбработкаЗапроса
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ОбработкаРезультатаЗапроса"); // АПК:1297 Это идентификатор.
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ОбновитьСтандартныеСпискиДляНовостей_Ежедневно"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
		КонтекстВыполнения.ЖурналРегистрации.ИмяСобытия = НСтр("ru='БИП:Новости.Все обновления новостей'", КодОсновногоЯзыка);
	КонецЕсли;
...
#КонецОбласти
	...
	Если ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет = Истина Тогда
		...
		КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения,
			...
			"Обработки.УправлениеНовостями.ОбновитьВсеСтандартныеСпискиССервера"); // АПК:1297 Это идентификатор.
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ПересчитатьНеобработанныеКатегорииНовостей"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_ОбновлениеКатегорийИЗначений_ПолучениеДанных
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:ОбновлениеКатегорийИЗначений/ПолучениеДанных"); // АПК:1297 Это идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_ОбновлениеКатегорийИЗначений_ДобавлениеИндексовИКолонок
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:ОбновлениеКатегорийИЗначений/ДобавлениеИндексовИКолонок"); // АПК:1297 Это идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_ОбновлениеКатегорийИЗначений_ЗаполнениеКатегорий_Простые
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:ОбновлениеКатегорийИЗначений/ЗаполнениеПростыхКатегорий"); // АПК:1297 Это идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_ОбновлениеКатегорийИЗначений_ЗаполнениеКатегорий_ИнтервалыВерсий
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:ОбновлениеКатегорийИЗначений/ЗаполнениеКатегорийИнтервалыВерсий"); // АПК:1297 Это идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_ОбновлениеКатегорийИЗначений_ЗаполнениеПростыхЗначений
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:ОбновлениеКатегорийИЗначений/ЗаполнениеПростыхЗначений"); // АПК:1297 Это идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_ОбновлениеКатегорийИЗначений_ЗаполнениеЗначенийИнтерваловВерсий
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:ОбновлениеКатегорийИЗначений/ЗаполнениеЗначенийИнтервалыВерсий"); // АПК:1297 Это идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_УдалениеНеправильныхЗаписей
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:УдалениеНеправильныхЗаписей"); // АПК:1297 Это идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_ПереносПравильныхЗаписей
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:ПереносПравильныхЗаписей"); // АПК:1297 Это идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_ОчисткаМусора
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:ОчисткаМусора"); // АПК:1297 Это идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ПроверитьИзменениеВерсииПлатформы"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_ПолучениеНастроек
	...
	НачатьШагВыполнения(КонтекстВыполнения, "ПолучитьТекущиеНастройки"); // АПК:1297 Это идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_ПересчетОтборов
	...
	НачатьШагВыполнения(КонтекстВыполнения, "ПересчетОтборов"); // АПК:1297 Это идентификатор.
	ЛогШаговВыполнения = Новый Массив;
		...
		Если МенеджерЗаданийАктивен = Истина Тогда
			Если МассивВыполняемыхФоновыхЗаданий.Количество() = 0 Тогда
				ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор);
					...
					//   * ПараметрыВыполнения.ИдентификаторФормы          = Новый УникальныйИдентификатор; // Уже установлен. // Отсутствует для процедуры.

				ФоновоеЗадание = ДлительныеОперации.ВыполнитьВФоне(ИмяМетода, КонтекстВыполненияВложенный, ПараметрыВыполнения);
				...
				ЛогШаговВыполнения.Добавить(НСтр("ru='Редко меняющиеся отборы будут пересчитаны (запущено фоновое задание)'")
					+ ": " + СокрЛП(ФоновоеЗадание.ИдентификаторЗадания) + ", " + ФоновоеЗадание.Статус);
			Иначе
...
Процедура СбросСостоянияНовостиДляВсехПользователей(НовостьСсылка) Экспорт
	...
	// В этом методе не осуществляется вызов ОбработкаНовостейПереопределяемый.ПослеИзмененияСостоянияНовостей;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.НайтиИОбработатьНеактивныеЛентыНовостей"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область ОсновныеДействия
	...
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(
		КонтекстВыполнения,
		"Цикл:ЛентыНовостей",
		НаименованиеМетода + "/Цикл:ЛентыНовостей"); // АПК:1297 Это идентификатор.
	Если НЕ Результат.Пустой() Тогда
...
Функция ПолучитьРегламентноеЗаданиеВсеОбновленияНовостейНеразделенное(ИсточникВызова)
	...
	Если ЕстьОшибка = Истина Тогда
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'", КодОсновногоЯзыка), // ИмяСобытия.
			НСтр("ru='Новости. Сервис и регламент. ПолучитьРегламентноеЗаданиеВсеОбновленияНовостейНеразделенное'", КодОсновногоЯзыка), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
Функция ПолучитьРегламентноеЗаданиеВсеОбновленияНовостейДляОбластиДанных(ИсточникВызова)
	...
	Если ЕстьОшибка = Истина Тогда
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'", КодОсновногоЯзыка), // ИмяСобытия.
			НСтр("ru='Новости. Сервис и регламент. ПолучитьРегламентноеЗаданиеВсеОбновленияНовостейДляОбластиДанных'", КодОсновногоЯзыка), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостейДляОбластиДанных, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.СбросУНовостейПризнакаТребуетсяПересчетОтборов"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.УдалениеНовостей_ПометитьНаУдалениеДанные"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_Инициализация
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Инициализация"); // АПК:1297 Это идентификатор.
...
#Область Обработка_ОчисткаРегистровСведенийШаг1
	...
	НачатьШагВыполнения(КонтекстВыполнения, "ОчиститьРегистрыСведений.Шаг1"); // АПК:1297 Это идентификатор.
...
#Область Обработка_ПометкаНовостейНаУдаление
	...
	НачатьШагВыполнения(КонтекстВыполнения, "ПометкаНаУдалениеНовостей"); // АПК:1297 Это идентификатор.
...
#Область Обработка_Очистка_КатегорииНовостейПростыеНеобработанные
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Очистка_КатегорииНовостейПростыеНеобработанные"); // АПК:1297 Это идентификатор.
...
#Область Обработка_Очистка_КатегорииНовостейИнтервалыВерсийНеобработанные
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Очистка_КатегорииНовостейИнтервалыВерсийНеобработанные"); // АПК:1297 Это идентификатор.
...
#Область Обработка_ОчисткаРегистровСведенийШаг2
	...
	НачатьШагВыполнения(КонтекстВыполнения, "ОчиститьРегистрыСведений.Шаг2"); // АПК:1297 Это идентификатор.
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск"; // АПК:1297 Это идентификатор.
	КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
...
#Область Проверки
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:ПравильностьВнедренияПодсистемы"); // АПК:1297 Это идентификатор.
...
#Область ОсновныеДействия_ВключитьРаботуСНовостями
	НачатьШагВыполнения(КонтекстВыполнения, "Заполнение:ВключитьРаботуСНовостями"); // АПК:1297 Это идентификатор.
...
#Область ОсновныеДействия_ОчисткаДатыЗагрузкиПоследнихНовостей
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Очистка:РС.ДатыЗагрузкиПоследнихНовостей"); // АПК:1297 Это идентификатор.
...
#Область ОсновныеДействия_ОчисткаДатыЗагрузкиПоследнихНовостейМинус3Месяца
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Заполнение:ДатаЗагрузкиНовостейМинус3Месяца"); // АПК:1297 Это идентификатор.
	Запрос = Новый Запрос;
	Запрос.Текст = "
		|ВЫБРАТЬ
		|	Спр.Ссылка КАК ЛентаНовостей
...
#Область ОсновныеДействия_УстановкаРасписания_Неразделенное
	НачатьШагВыполнения(КонтекстВыполнения, "Заполнение:РасписаниеНеразделенное"); // АПК:1297 Это идентификатор.
...
#Область ОсновныеДействия_УстановкаРасписания_ДляОбластиДанных
	НачатьШагВыполнения(КонтекстВыполнения, "Заполнение:РасписаниеДляОбластиДанных"); // АПК:1297 Это идентификатор.
...
#Область ПереопределяемыеДействия
	...
	НачатьШагВыполнения(КонтекстВыполнения, "ПереопределяемыеДействия"); // АПК:1297 Это идентификатор.
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск"; // АПК:1297 Это идентификатор.
	КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонтекстВыполнения.ЖурналРегистрации.ИмяСобытия = НСтр("ru='БИП:Новости.Обновление ИБ'", КодОсновногоЯзыка);
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию"; // АПК:1297 Это идентификатор.
	КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонтекстВыполнения.ЖурналРегистрации.ИмяСобытия = НСтр("ru='БИП:Новости.Обновление ИБ'", КодОсновногоЯзыка);
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ОбновлениеИнформационнойБазы_ОбластьДанных_ПерейтиНаВерсию"; // АПК:1297 Это идентификатор.
	КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонтекстВыполнения.ЖурналРегистрации.ИмяСобытия = НСтр("ru='БИП:Новости.Обновление ИБ'", КодОсновногоЯзыка);
...
Функция ПроверитьПравильностьВнедренияПодсистемы(ВариантПроверки = "Все") Экспорт
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ПроверитьПравильностьВнедренияПодсистемы"; // АПК:1297 Это идентификатор.
...
Процедура ЗаписатьСообщениеВЖурналРегистрации(
			ИмяСобытия,
			ИдентификаторШага,
			УровеньРегистрации = "Ошибка", // АПК:1297 Это идентификатор.
			ОбъектМетаданных = "",
			Данные = Неопределено,
			Комментарий = "",
...
Функция Проверка_РазрешенаРаботаСНовостями(КонтекстВыполнения)
	...
	НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru='Проверка:РазрешенаРаботаСНовостями'")); // АПК:1297 Это идентификатор.
...
Функция Проверка_МонопольныйРежимОтключен(КонтекстВыполнения)
	...
	НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru='Проверка:МонопольныйРежим'")); // АПК:1297 Это идентификатор.
...
Функция Проверка_ЭтоОбычныйПользователь(КонтекстВыполнения)
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:Пользователи.ЭтоСеансВнешнегоПользователя"); // АПК:1297 Это идентификатор.
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ЗаполнитьПрограммныеЗначенияКатегорий_Общие"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_ЗаполнитьПрограммныеЗначенияКатегорий_Общие
	...
	НачатьШагВыполнения(КонтекстВыполнения, "ЗаполнитьПрограммныеЗначенияКатегорий_Общие"); // АПК:1297 Это идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ЗаполнитьПрограммныеЗначенияКатегорий_ДляОбластиДанных"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_ЗаполнитьПрограммныеЗначенияКатегорий_ДляОбластиДанных
	...
	НачатьШагВыполнения(КонтекстВыполнения, "ЗаполнитьПрограммныеЗначенияКатегорий_ДляОбластиДанных"); // АПК:1297 Это идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
//  ТекстHTML     - Строка - уже подготовленный текст, который по-умолчанию будет отображен в форме просмотра новости.
//
// Возвращаемое значение:
//   Строка - окончательный текст новости (то, что внутри <body></body>).
//
Процедура ДополнительноОбработатьТекстНовостиПередПоказом(Знач ОбъектНовости, ТекстHTML) Экспорт

	ИнтеграцияПодсистемБИП.ДополнительноОбработатьТекстНовостиПередПоказом(ОбъектНовости, ТекстHTML);
	ОбработкаНовостейПереопределяемый.ДополнительноОбработатьТекстНовостиПередПоказом(ОбъектНовости, ТекстHTML);
...
//
Процедура ДополнительноОбработатьТекстСпискаНовостейПередПоказом(ТекстHTML) Экспорт

	ИнтеграцияПодсистемБИП.ДополнительноОбработатьТекстСпискаНовостейПередПоказом(ТекстHTML);
	ОбработкаНовостейПереопределяемый.ДополнительноОбработатьТекстСпискаНовостейПередПоказом(ТекстHTML);
...
// Дополнительно передается НовостьXDTO - на случай, если передаются дополнительные теги, их можно отсюда получить.
//
// Параметры:
//  ОбъектНовости - СправочникОбъект.Новости - Новость, у которой необходимо изменить данные;
//  НовостьXDTO   - ОбъектXDTO - полученный объект новости.
//
Процедура ДополнительноОбработатьНовостьПослеПолученияПередЗаписью(ОбъектНовости, Знач НовостьXDTO) Экспорт

	ИнтеграцияПодсистемБИП.ДополнительноОбработатьНовостьПослеПолученияПередЗаписью(ОбъектНовости, НовостьXDTO);
	ОбработкаНовостейПереопределяемый.ДополнительноОбработатьНовостьПослеПолученияПередЗаписью(ОбъектНовости, НовостьXDTO);
...
#Область НастройкиМоделиСервиса
