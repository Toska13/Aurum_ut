...
Процедура ПараметрыСервисаДоставки(ПараметрыФормы) Экспорт
	...
	ДанныеОрганизаций = СервисДоставкиСлужебный.ДоступныеПодключенныеОрганизации();
	...
	Если ПараметрыФормы.ЗаполнятьОрганизациюПоУмолчанию И Не ЗначениеЗаполнено(ОрганизацияБизнесСети) Тогда
		ОрганизацияБизнесСети = ОрганизацияПоУмолчанию(ТипГрузоперевозки, ДанныеОрганизаций);
	...
	Если ЗначениеЗаполнено(ТипГрузоперевозки) И ПараметрыФормы.ДокументыОснования.Количество() > 0 Тогда
		...
		Если СтрНачинаетсяС(ПараметрыФормы.ИмяФормы, "КарточкаЗаказа") И ПараметрыФормы.РежимМастера = 0 Тогда
			...
			ИначеЕсли ПараметрыФормы.ИмяФормы = "КарточкаЗаказаСДЭК" Тогда
				ПараметрыЗаказа.ТипЗаказа = 1; // Интернет-магазин
			КонецЕсли;
...
Процедура ПриСозданииНаСервере(Форма, МестоРазмещенияКоманд = Неопределено, ПараметрыОповещения = Неопределено) Экспорт
	...
	Если ЭлементУправления <> Неопределено Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма.Объект, "Ссылка")
			...
			ДокументыОснования = СервисДоставкиСлужебный.ДокументыОснованияДляПолученияЗаказаНаДоставку(Форма.Объект.Ссылка);
			Результат = РегистрыСведений.ДанныеЗаказовСервисДоставки.КоличествоЗаказовПоОснованиям(ДокументыОснования);
			КоличествоЗаказов = 0;
			Для Каждого ЭлементСписка Из Результат Цикл
				КоличествоЗаказов = КоличествоЗаказов + ЭлементСписка.Значение;
			КонецЦикла;
			
			Если Результат.Количество() > 0 И КоличествоЗаказов > 0 Тогда
...
Процедура ОформитьЗаказНаДоставку(Знач ПараметрыЗапроса, Знач АдресРезультата) Экспорт
	...
	Результат = НовыйПараметрыОтветаОформитьЗаказНаДоставку();
	Если Отказ Или Не ЗначениеЗаполнено(РезультатЗапроса) Тогда
...
// Параметры:
//  ПараметрыЗапроса - Структура - параметры для формирования запроса:
//    * ОрганизацияБизнесСетиСсылка - ОпределяемыйТип.ОрганизацияСервисДоставки - организация, подключенная к бизнес сети.
//  АдресРезультата	 - Строка - адрес хранилища с результатом.
//
...
// Получить характеры груза.
// 
// Параметры:
//  ПараметрыЗапроса -Структура:
//   * ОрганизацияБизнесСетиСсылка - Неопределено, ОпределяемыйТип.ОрганизацияСервисДоставки -
//   * ТипГрузоперевозки - Число - 
//  АдресРезультата - Строка - Адрес результата
// 
// Возвращаемое значение:
// - Неопределено - если не удалось выполнить запрос к сервису
// - Структура - пустая структура, если запрос был выполнен и не вернул необрабатываемый реузльтат, или структура с ключами:
//		* Ошибки - Массив Из Строка - обязательное свойство, массив ошибок может быть пыстым
//		* Список - Массив Из Структура - массив не пустой, если запрос выполнен без ошибок:
//			** Идентификатор - Строка -
//			** Наименование - Строка -
Функция ПолучитьХарактерыГруза(Знач ПараметрыЗапроса, Знач АдресРезультата = "") Экспорт
	Отказ = Ложь;
	Результат = Неопределено;
	
	Если ЗначениеЗаполнено(ПараметрыЗапроса.ОрганизацияБизнесСетиСсылка) Тогда
		СервисДоставкиСлужебный.ПроверитьОрганизациюБизнесСети(ПараметрыЗапроса.ОрганизацияБизнесСетиСсылка, Отказ);
	Иначе
		Отказ = Истина;
	КонецЕсли;
	...
	ЗапросСервиса = ЗапросСервисаПолучитьХарактерыГруза(ПараметрыЗапроса);
	ЗапросСервиса.Вставить("ОрганизацияБизнесСети", ПараметрыЗапроса.ОрганизацияБизнесСетиСсылка);
	РезультатЗапроса = ВыполнитьЗапрос(ЗапросСервиса, Отказ);
	...
	Результат = Новый Структура();
	Если ТипЗнч(РезультатЗапроса) = Тип("Структура") Тогда
		ОбработатьРезультатЗапросаПолучитьХарактерыГруза(РезультатЗапроса, Результат, ПараметрыЗапроса);
	КонецЕсли;
...
Функция НовыйПараметрыЗапросаСоздатьИзменитьЗаказНаДоставку() Экспорт
	...
	Параметры.Вставить("ДокументыОснованияДанные", Новый Массив);
...
//
// Возвращаемое значение:
//  Структура - см. НовыйПараметрыЗапросаСоздатьИзменитьЗаказНаДоставку 
//
...
// Возвращает инициализированную структуру параметров запроса Получить заказ на доставку.
// 
// Возвращаемое значение:
//  Структура:
//    * ТипГрузоперевозки - Число -
//    * РежимМастера - Число - 
//    * Идентификатор - Строка - 
//    * ЭтоСозданиеШаблона - Булево - 
//    * ЭтоЗаполнениеКопированием - Булево - 
Функция НовыйПараметрыЗапросаПолучитьЗаказНаДоставку() Экспорт
	
	Результат = СервисДоставкиКлиентСервер.НовыйОбязательныеПараметрыЗапроса();
...
//    * ТипГрузоперевозки - Число -
//    * Идентификатор - Строка -
//    * Идентификаторы - СписокЗначений Из Строка -
//    * ОтменитьЗаказПлатно - Неопределено -
//
Функция НовыйПараметрыЗапросаОтменитьЗаказНаДоставку() Экспорт
	...
	Результат.Вставить("ОтменитьЗаказПлатно", Неопределено);
...
//    * ТипГрузоперевозки - Число -
//    * СписокЗаказов - Массив Из Структура - свойства структуры: см. НовыйПараметрыЗапросаСоздатьИзменитьОтправитьЗаказНаДоставку
//    * Отправка - Булево
//    * ГрупповаяОбработка - Булево
//    * ПараметрыДанныеФормы - Структура -
...
// 
// Возвращаемое значение:
//  Структура - параметры запроса получить график движения заказа по трек номеру:
//    * ТипГрузоперевозки - Число -
//    * ТрекНомер - Строка 
...
//  Структура - параметры, необходимые для формирования запроса:
//   * ТипГрузоперевозки - Число -
//   * Параметры - Массив Из Структура - см. НовыйЭлементПараметровЗапросаЗагрузитьФайлыПечатныхФорм
//
...
// Новый элемент параметров запроса загрузить файлы печатных форм.
// 
// Возвращаемое значение:
//  Структура - структура элемента параметров запроса загрузить файлы печатных форм:
//   * ИдентификаторЗаказа - Строка - 
//   * ТипГрузоперевозки - Число - 
//   * ОрганизацияБизнесСетиСсылка - ОпределяемыйТип.ОрганизацияСервисДоставки -
//   * Список - Массив Из Структура - свойства структуры: см. НовыйПечатнаяФормаДляЗапроса
...
// Конструктор параметров запроса Получить журнал заказов на доставку из сервиса.
// 
// Возвращаемое значение:
//  Структура - параметры запроса получить журнал заказов на доставку из сервиса:
//   * ТипГрузоперевозки - Число - 
//   * Страница - Число - 
//   * Сортировка - Строка - 
//   * МассивНомеров - Массив Из Неопределено-
//   * ДатаСозданияОт - Неопределено - 
//   * ДатаСозданияДо - Неопределено - 
//   * СписокСтатусов - Массив из Неопределено - 
Функция НовыйПараметрыЗапросаПолучитьЖурналЗаказовНаДоставкуИзСервиса() Экспорт
	...
	Результат.Вставить("ДатаСозданияОт", Неопределено);
	Результат.Вставить("ДатаСозданияДо", Неопределено);
...
// Конструктор параметров запроса получить печатную форму из сервиса.
// 
// Возвращаемое значение:
//  Структура - параметры запроса получить печатную форму из сервиса:
//   * ТипГрузоперевозки - Число - 
...
// 
// Возвращаемое значение:
//  Структура - параметры запроса получить грузоперевозчиков. Содержит свойства:
//    * ОрганизацияБизнесСетиСсылка - Неопределено - 
//    * ТипГрузоперевозки - Число - 
//    * ГрузоперевозчикИНН - Строка - 
//    * ГрузоперевозчикКПП - Строка -
//    * ТребуетсяПроверкаАвторизации - Булево -
// 
...
// 
// Возвращаемое значение:
//  Структура - параметры запроса получить пункты выдачи клиента. Содержит свойства:
//    * ОрганизацияБизнесСетиСсылка - Неопределено - 
//    * ТипГрузоперевозки - Число - 
//    * ТипЗагрузки - Число -
//
...
// 
// Возвращаемое значение:
//  Структура - параметры запроса установить тариф по умолчанию. Содержит свойства:
//    * ГрузоперевозчикИдентификатор - Строка - 
...
// 
// Возвращаемое значение:
//  Структура - параметры запроса получить данные грузоперевозчика:
//   * ТипГрузоперевозки - Число
//   * Идентификатор - Строка
...
Функция НовыйПараметрыЗапросаПолучитьТарифы() Экспорт
	...
	Результат.Вставить("ИдентификаторыТП", Новый Массив);
	Результат.Вставить("ГрузоперевозчикИдентификатор", "");
	Результат.Вставить("ТарифИдентификатор", "");
	...
	Результат.Вставить("ВремяОтгрузкиФиксированное", Ложь);
	...
	Результат.Вставить("ВремяДоставкиФиксированное", Ложь);
...
// Конструктор параметров запроса СохранитьПараметрыТарифа.
// 
// Возвращаемое значение:
//  Структура - параметры запроса сохранить параметры тарифа:
//   * ТипГрузоперевозки - Число -
//   * Идентификатор - Строка -
//   * ФормаОплаты - Число -
//   * Услуги - ТаблицаЗначений:
//     ** Идентификатор - Строка
//     ** Свойства - Массив Из Структура - свойства структуры: см. НовыйПараметрыСвойстваУслуги
//   * Терминалы - ТаблицаЗначений:
//     ** Идентификатор - Строка
//     ** Направление - Число
//     ** АдресПредставление - Строка
...
// Конструктор параметров запроса ПолучитьХарактерыГруза
// 
// Возвращаемое значение:
//  Структура - :
// * ОрганизацияБизнесСетиСсылка - Неопределено, ОпределяемыйТип.ОрганизацияСервисДоставки - 
// * ТипГрузоперевозки - Число - 
Функция НовыйПараметрыЗапросаПолучитьХарактерыГруза() Экспорт
	Результат = СервисДоставкиКлиентСервер.НовыйОбязательныеПараметрыЗапроса();
	Результат.Вставить("ОрганизацияБизнесСетиСсылка", Неопределено);
	Возврат Результат;
КонецФункции

// Конструктор параметров запроса ПолучитьТарифыИУслуги
// 
// Возвращаемое значение:
//  См. СервисДоставкиКлиентСервер.НовыйОбязательныеПараметрыЗапроса
Функция НовыйПараметрыЗапросаПолучитьТарифыИУслуги() Экспорт

	Возврат СервисДоставкиКлиентСервер.НовыйОбязательныеПараметрыЗапроса();
...
//  См. СервисДоставкиКлиентСервер.НовыйОбязательныеПараметрыЗапроса
//
Функция НовыйПараметрыЗапросаПолучитьСписокКонтрагентов() Экспорт
...
//  См. СервисДоставкиКлиентСервер.НовыйОбязательныеПараметрыЗапроса
//
Функция НовыйПараметрыЗапросаСинхронизироватьДанныеРегистровПоЧастичнойДоставке() Экспорт
...
// Новый параметры запроса записать настройки авторизации.
// 
// Возвращаемое значение:
//  Структура - параметры запроса записать настройки авторизации:
//   * ТипГрузоперевозки - Число - 
//   * Логин - Строка -
//   * Пароль - Строка -
//   * ПравоДоступа - Строка -
...
// Конструктор параметров печатной формы для параметров запроса ПолучитьФайлыПечатныхФорм.
// 
// Возвращаемое значение:
...
Функция НовыйПечатнаяФормаДляЗапроса() Экспорт
	...
	Результат.Вставить("ПараметрыГенерации", Новый Массив);
	Результат.Вставить("ПроверитьГенерацию", Ложь);
...
Функция ПараметрыЗапросаПолучитьНаложенныеПлатежи() Экспорт
	...
	Параметры = СервисДоставкиКлиентСервер.НовыйОбязательныеПараметрыЗапроса();
...
// Новый параметры запроса отправить запрос на получение регистрационных данных.
// 
// Возвращаемое значение:
//  Структура - параметры запроса отправить запрос на получение регистрационных данных:
//   * ТипГрузоперевозки - Число - 
//   * ЗарегистрироватьОрганизацию - Булево - 
//   * ЗарегистрироватьПунктВыдачи - Булево - 
//   * ЗарегистрироватьПользователя - Булево - 
...
//   * ИдентификаторОрганизации - Строка - 
//   * ИдентификаторПунктаВыдачи - Строка - 
//   * НаименованиеПунктаВыдачи - Строка - 
//   * АдресПунктаВыдачи - Строка - 
//   * ТелефонПунктаВыдачи - Строка - 
//   * НаименованиеПользователя - Строка - 
//   * НомерТелефонаПользователя - Строка - 
//   * АдресЭлектроннойПочтыПользователя - Строка - 
...
// Новый параметры ответа оформить заказ на доставку.
// 
// Возвращаемое значение:
//  Структура - Новый параметры ответа отменить заказ на доставку:
//   * СостояниеИдентификатор - Число
//   * ДатаСозданияЗаказа - Дата
//   * ОжидатьРасчета - Булево
//   * Ошибки - Массив из Строка
//   * Ошибка - Строка
Функция НовыйПараметрыОтветаОформитьЗаказНаДоставку() Экспорт
	
	Параметры = Новый Структура();
	
	Параметры.Вставить("СостояниеИдентификатор", 0);
	Параметры.Вставить("ДатаСозданияЗаказа", Дата(1, 1, 1));
	Параметры.Вставить("ОжидатьРасчета", Ложь);
	Параметры.Вставить("Ошибки", Новый Массив);
...
//   * ФормаОплаты - Число - 
//   * ТипЗаказа - Число -
//   * ИдентификаторыТП - Массив из Строка - идентификаторы (коды) тарифных планов (подтипов заказа), 
//   * Валюта - Структура - :
//    ** Код - Строка - 
//    ** Наименование - Строка - 
...
//    ** Идентификатор - Строка - Идентификатор свойства
//    ** Свойства - Массив из Структура - Массив свойств
//   * КомпенсацияДоставки - ТаблицаЗначений - Компенсация доставки:
//    ** ПорогСтоимости - Число - Порог стоимости при частичной доставки в целых числах
//    ** Сумма - Число - Сумма
...
Функция НовыйПараметрыЗаказаНаДоставку() Экспорт
	...
	Параметры.Вставить("ИдентификаторыТП", Новый Массив);
	...
	Параметры.Вставить("Пропуск", НовыйПараметрыПропуска());
	Параметры.Вставить("Промокод", "");
	...
	ТаблицаУслуги.Колонки.Добавить("Идентификатор");
	ТаблицаУслуги.Колонки.Добавить("Использовать", СервисДоставкиСлужебный.Типы().Булево);
	ТаблицаУслуги.Колонки.Добавить("Стоимость", СервисДоставкиСлужебный.Типы().Число);
	ТаблицаУслуги.Колонки.Добавить("Свойства", СервисДоставкиСлужебный.Типы().Массив);
	...
	// Плательщики услуг
	ТаблицаПлательщикиУслуг = Новый ТаблицаЗначений;
	ТаблицаПлательщикиУслуг.Колонки.Добавить("Идентификатор", СервисДоставкиСлужебный.Типы().Строка);
	ТаблицаПлательщикиУслуг.Колонки.Добавить("ПлательщикРоль", СервисДоставкиСлужебный.Типы().Число);
	
	Параметры.Вставить("ПлательщикиУслуг", ТаблицаПлательщикиУслуг);
	
	// Компенсация доставки
	КомпенсацияДоставки = Новый ТаблицаЗначений;
	КомпенсацияДоставки.Колонки.Добавить("ПорогСтоимости", СервисДоставкиСлужебный.Типы().Число);
	КомпенсацияДоставки.Колонки.Добавить("Сумма", СервисДоставкиСлужебный.Типы().Число);
	КомпенсацияДоставки.Колонки.Добавить("СуммаНДС", СервисДоставкиСлужебный.Типы().Число);
	КомпенсацияДоставки.Колонки.Добавить("СтавкаНДС", СервисДоставкиСлужебный.Типы().Строка);
	
	Параметры.Вставить("КомпенсацияДоставки", КомпенсацияДоставки);
...
Функция НовыйПараметрыВремениОтгрузкиДоставки() Экспорт
	...
	Параметры.Вставить("Фиксированное", Ложь);
...
// Возвращаемое значение:
//  Структура - параметры, необходимые для формирования запроса:
//    * id - Строка - ИдентификаторДокумента
//    * number - Строка - НомерДокумента
Функция НовыйПараметрыДокументыОснованияДанные() Экспорт
...
//    * ХарактерГрузаСтрокой  - Строка - описание содержимого груза из классификатора перевозчика.
//    * ХарактерГрузаИдентификатор  - Строка - идентификатор содержимого груза из классификатора перевозчика.
//
Функция ПараметрыПоУмолчанию(ТипГрузоперевозки, Параметры = "") Экспорт
	...
	|	МАКСИМУМ(ВЫРАЗИТЬ(ВЫБОР
	|		КОГДА НастройкиДоставки.НаименованиеПараметра = ""ХарактерГрузаСтрокой""
	|			ТОГДА НастройкиДоставки.Значение
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК СТРОКА(150))) КАК ХарактерГрузаСтрокой,
	|	МАКСИМУМ(ВЫРАЗИТЬ(ВЫБОР
	|		КОГДА НастройкиДоставки.НаименованиеПараметра = ""ХарактерГрузаИдентификатор""
	|			ТОГДА НастройкиДоставки.Значение
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК СТРОКА(36))) КАК ХарактерГрузаИдентификатор,
	|	МАКСИМУМ(ВЫРАЗИТЬ(ВЫБОР
	|		КОГДА НастройкиДоставки.НаименованиеПараметра = ""ГрузВидРасшифровки""
	|			ТОГДА НастройкиДоставки.Значение
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК СТРОКА(150))) КАК ГрузВидРасшифровки,
	|	МАКСИМУМ(ВЫРАЗИТЬ(ВЫБОР
	|		КОГДА НастройкиДоставки.НаименованиеПараметра = ""ВыгружатьКодыМаркировки""
	|			ТОГДА НастройкиДоставки.Значение
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК БУЛЕВО)) КАК ВыгружатьКодыМаркировки
	...
	|	ВременнаяТаблица.ХарактерГрузаСтрокой КАК ХарактерГрузаСтрокой,
	|	ВременнаяТаблица.ХарактерГрузаИдентификатор КАК ХарактерГрузаИдентификатор,
	|	ВременнаяТаблица.ГрузВидРасшифровки КАК ГрузВидРасшифровки,
	|	ВременнаяТаблица.ВыгружатьКодыМаркировки КАК ВыгружатьКодыМаркировки,
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить(СервисДоставкиКлиентСервер.ИмяМетодаПолучитьХарактерыГруза(), Истина);
...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	ИсключениеПроверки.ПолноеИмяОбъекта = "ОбщийМодуль.СервисДоставки";
	ИсключениеПроверки.ИмяПроцедуры = "ВыполнитьДлительнуюОперацию";
	...
	Настройки.ИсключенияКогдаУказанныйМетодНеВызывается.Вставить("ОбщийМодуль.СервисДоставки", Методы);
...
//   * ТипГрузоперевозки - Число - Необязательный параметр. Если указан, будет добавлен во все параметры методов,
//                                 входящих в Очередь.
//   * ОрганизацияБизнесСетиСсылка - ОпределяемыйТип.ОрганизацияСервисДоставки - Необязательный параметр.
//                                                                               Если указан, будет добавлен во все
//                                                                               параметры методов, входящих в Очередь.
//  АдресРезультата - Строка - Адрес временного хранилища, в который помещаются результаты выполненных методов типов
//                             СписокЗначений Из Произвольный, Неопределено.
Процедура МенеджерДлительныхОпераций(Знач Параметры, Знач АдресРезультата) Экспорт
	...
	Если Не Отказ Тогда
		...
		Для Каждого Операция Из Очередь Цикл
			...
			РезультатОперации = ВыполнитьДлительнуюОперацию(Операция, Параметры, РезультатОчередь);
...
//  ПараметрыМетода - Структура, Неопределено - Параметры вызываемого метода
Процедура ЗапуститьФоновоеЗадание(Форма, ПараметрыМенеджера, ПараметрыМетода = Неопределено) Экспорт
	...
	Если ПараметрыМетода = Неопределено Тогда
		ПараметрыМетода = ПараметрыМенеджера;
	Иначе
	...
	ФоновоеЗадание = ДлительныеОперации.ВыполнитьПроцедуру(ПараметрыВыполнения,
		 "СервисДоставки.МенеджерДлительныхОпераций", ПараметрыМетода, АдресРезультата);
						...
						Если ЗначениеЗаполнено(РезультатЗапроса) Тогда
							...
							Если РезультатЗапроса.СписокПлатежей.Количество() > 0 Тогда
								...
								Исключение
									...
									ЗаписьЖурналаРегистрации(НСтр("ru='Сервис доставки.Загрузка данных наложенных платежей.'",
...
Функция ПараметрыЗаказаНаДоставку(ТипГрузоперевозки, Основания = Неопределено, ОрганизацияБизнесСети = Неопределено, ВидРасшифровки = "") Экспорт
	
	ПараметрыЗаказа = НовыйПараметрыЗаказаНаДоставку();
	ПараметрыЗаказа.ТипГрузоперевозки = ТипГрузоперевозки;
	
	ДанныеПоУмолчанию = ПараметрыПоУмолчанию(ПараметрыЗаказа.ТипГрузоперевозки);
	ДобавитьВидРасшифровкиПоУмолчанию(ПараметрыЗаказа, ВидРасшифровки, ДанныеПоУмолчанию);
	...
	ДобавитьПараметрыПоУмолчанию(ПараметрыЗаказа, ДанныеПоУмолчанию);
	...
	ПараметрыЗаказа.Удалить("ПлательщикиУслуг");
	ПараметрыЗаказа.Удалить("КомпенсацияДоставки");
	
	Возврат ПараметрыЗаказа;
	
КонецФункции

Функция ОрганизацияПоУмолчанию(ТипГрузоперевозки, ДоступныеПодключенныеОрганизации = Неопределено) Экспорт
	
	ОрганизацияБизнесСетиСсылка = ПараметрыПоУмолчанию(ТипГрузоперевозки, "ОрганизацияБизнесСети").ОрганизацияБизнесСети;
	
	Если ДоступныеПодключенныеОрганизации = Неопределено Тогда
	...
	Если Не ЗначениеЗаполнено(ОрганизацияБизнесСетиСсылка) Тогда
		Если ДоступныеПодключенныеОрганизации.Количество() = 1 Тогда
...
#Область ПропускДляПроезда

// Заполнить список выбора Заказать за.
// 
// Параметры:
//  СписокВыбора - СписокЗначений из Число
Процедура ЗаполнитьСписокВыбораЗаказатьЗа(СписокВыбора) Экспорт
	
	СписокВыбора.Очистить();
	СписокВыбора.Добавить(1, Нстр("ru = '1 ч.'"));
	СписокВыбора.Добавить(24, Нстр("ru = '24 ч.'"));
	
КонецПроцедуры

// Заполнить список выбора Возмещение оплаты.
// 
// Параметры:
//  СписокВыбора - СписокЗначений из Число
Процедура ЗаполнитьСписокВыбораВозмещениеОплаты(СписокВыбора) Экспорт
	
	СписокВыбора.Очистить();
	СписокВыбора.Добавить(1, Нстр("ru = 'По чеку на адресе'"));
	СписокВыбора.Добавить(2, Нстр("ru = 'Включить в счет'"));
	
КонецПроцедуры

// Заполнить представление пропуска.
// 
// Параметры:
//  ДанныеПропуска - Структура - Данные пропуска:
//  * ПропускВозмещениеОплаты - Число - 
//  * ПропускВремяЗаказа - Число - 
//  * ПропускЗаказывать - Булево - 
//  * ПропускСтоимость - Число - 
// 
// Возвращаемое значение:
//  Строка - Представление пропуска
Функция ПредставлениеПропуска(ДанныеПропуска) Экспорт
	
	Результат = "Нет";
	Если Не ЗначениеЗаполнено(ДанныеПропуска) Тогда
		Возврат Результат;
	КонецЕсли;
	
	ЗаказатьЗа = Новый СписокЗначений();
	ВозмещениеОплаты = Новый СписокЗначений();
	
	СервисДоставки.ЗаполнитьСписокВыбораЗаказатьЗа(ЗаказатьЗа);
	СервисДоставки.ЗаполнитьСписокВыбораВозмещениеОплаты(ВозмещениеОплаты);
	
	Если ДанныеПропуска.ПропускЗаказывать Тогда
		
		Результат = СтрШаблон(НСтр("ru = 'Заказать за %1'"), 
			ЗаказатьЗа.НайтиПоЗначению(ДанныеПропуска.ПропускВремяЗаказа).Представление);
		Если ДанныеПропуска.ПропускСтоимость > 0 Тогда
			Результат = СтрШаблон(НСтр("ru = '%1, стоимость %2, %3'"), Результат, ДанныеПропуска.ПропускСтоимость,
			НРег(ВозмещениеОплаты.НайтиПоЗначению(ДанныеПропуска.ПропускВозмещениеОплаты).Представление));
		КонецЕсли;
...
//   * Очередь - СписокЗначений из Произвольный- Порядок выполнения операций
//  Отказ - Булево - Отказ
...
//
// Возвращаемое значение:
//  Произвольный, Неопределено - Результат выполнения операции
//
		...
		Если Не ПустаяСтрока(Операция.Значение.ЗависитОт) Тогда
			Для Каждого ЭлементОчереди Из ОчередьВыполненныхМетодов Цикл
				Если ЭлементОчереди.Представление = Операция.Значение.ЗависитОт Тогда
					...
					Если ЭлементОчереди.Значение <> Неопределено
						И ЭлементОчереди.Значение.Ошибки.Количество() = 0 Тогда
						Если ТипЗнч(Операция.Значение.ЗависимыеПоля) = Тип("Соответствие") Тогда
							Для Каждого КлючЗначение Из Операция.Значение.ЗависимыеПоля Цикл
								Параметры[КлючЗначение.Ключ] = ПолучитьЗначение(ЭлементОчереди.Значение.Данные, КлючЗначение.Значение);
							КонецЦикла;
						Иначе
							ЗаполнитьЗначенияСвойств(Параметры, ЭлементОчереди.Значение.Данные, Операция.Значение.ЗависимыеПоля);
						КонецЕсли;
					Иначе
...
Функция НовыйПараметрыОтветаПолучитьЗаказНаДоставку()
	...
	Результат.Вставить("ТипЗаказа", 0); // 1 - Интернет-магазин, 2 - Доставка
	Результат.Вставить("ПодтипыЗаказа", Новый Массив);
	...
	Результат.Вставить("ТранспортТипДоставки", "");
	Результат.Вставить("ТранспортНомерДоставки", "");
	Результат.Вставить("ВодительНаименованиеДоставки", "");
	Результат.Вставить("ВодительНомерТелефонаДоставки", "");
	...
	Результат.Вставить("ТранспортТипОтгрузки", "");
	Результат.Вставить("ТранспортНомерОтгрузки", "");
	Результат.Вставить("ВодительНаименованиеОтгрузки", "");
	Результат.Вставить("ВодительНомерТелефонаОтгрузки", "");
...
Функция НовыйПараметрыГруза()
	...
	Параметры.Вставить("СодержимоеИдентификатор", "");
	Параметры.Вставить("Стоимость", 0);
	Параметры.Вставить("ВидРасшифровки", "");
...
Функция НовыйПараметрыПлательщикУслуги()

	Результат = Новый Структура;
	
	Результат.Вставить("Идентификатор", "");
	Результат.Вставить("Наименование", "");
	Результат.Вставить("ПлательщикРоль", 0);
...
Функция НовыйПараметрыЭлементаГруза()
	...
	Параметры.Вставить("ИдентификаторГрузовогоМеста", 0); // Число(5). номер грузового места
	Параметры.Вставить("КодМаркировки", ""); // Строка(150). код маркировки честного знака
...
Функция НовыйПараметрыПоУмолчанию()
	...
	Параметры.Вставить("ХарактерГрузаСтрокой", "");
	Параметры.Вставить("ХарактерГрузаИдентификатор", "");
	Параметры.Вставить("ГрузВидРасшифровки", "");
...
Функция НовыйПараметрыЗаказаЖурналаЗаказовНаДоставку()
	...
	Параметры.Вставить("ВодительНаименованиеОтгрузки", "");
	Параметры.Вставить("ВодительНомерТелефонаОтгрузки", "");
	Параметры.Вставить("ТранспортТипОтгрузки", "");
	Параметры.Вставить("ТранспортНомерОтгрузки", "");
	Параметры.Вставить("ВодительНаименованиеДоставки", "");
	Параметры.Вставить("ВодительНомерТелефонаДоставки", "");
	Параметры.Вставить("ТранспортТипДоставки", "");
	Параметры.Вставить("ТранспортНомерДоставки", "");
	Параметры.Вставить("ВнутреннийНомер", "");
...
// Новые параметры тарифа.
// 
// Возвращаемое значение:
//  Структура - Новые параметры тарифа:
// * Идентификатор - Строка - 
// * Наименование - Строка - 
// * Описание - Строка - 
// * СпособОтгрузки - Число - 
// * ПунктПриемаГрузаТипИдентификатор - Число - 
// * СпособДоставки - Число - 
// * ПунктВыдачиГрузаТипИдентификатор - Число - 
// * Услуги - Массив из См. НовыеБазовыеПараметрыСущности - 
...
// Новые параметры сущности наименование идентификатор. Базовый конструктор для обмена данных.
// 
// Возвращаемое значение:
//  Структура - Новые параметры сущности:
// * Идентификатор - Строка - 
// * Наименование - Строка - 
Функция НовыеБазовыеПараметрыСущности(Идентификатор = "", Наименование = "")
...
// Новые параметры для открытия формы ПараметрыПропуска.
// 
// Возвращаемое значение:
...
//   * Стоимость - Число - 
//   * ВозмещениеОплаты - Число - 
Функция НовыйПараметрыПропуска() Экспорт
	Возврат СервисДоставкиКлиентСервер.НовыйПараметрыПропуска();
КонецФункции
...
Процедура УстановитьПараметрыЗапросаСервиса(ЗапросСервиса, ПараметрыЗапроса, ТипГрузоперевозки, ИмяМетода = "")
	...
	ИначеЕсли ИмяМетода = СервисДоставкиКлиентСервер.ИмяПроцедурыПолучитьФайлыПечатныхФорм() Тогда
		...
		Если СервисДоставкиКлиентСервер.ЭтоСДЭК(ТипГрузоперевозки) Тогда
			ЗапросСервиса.Метод = "POST";
		ИначеЕсли СервисДоставкиКлиентСервер.ЭтоДеловыеЛинии(ТипГрузоперевозки) Тогда
			ЗапросСервиса.Метод = "POST";
			Адрес = "delivery/orders/%1/forms";
			ЗапросСервиса.Контент = "application/vnd-1cbn-deliv-nfiles.v2+json";
			ЗапросСервиса.Заголовки.Вставить("Accept", "application/vnd-1cbn-deliv-files.v1+json");
		Иначе
			СтрокаПараметров = "";
...
Функция ВыполнитьЗапрос(ЗапросСервиса, Отказ, Кэш = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СтрокаАутентификации) Тогда
		Если ЭтоСервисИнтеграции1СЭлемент Тогда
			Запрос.Заголовки.Вставить("Idempotency-Key", Новый УникальныйИдентификатор);
			Запрос.Заголовки.Вставить("DeliveryType", ЗапросСервиса.ТипГрузоперевозки);
...
Функция НовыйЗапросСервиса(ПараметрыЗапроса = Неопределено, ИмяМетода = "")
	...
	ЗапросСервиса.Вставить("ТипГрузоперевозки", ТипГрузоперевозки);
...
Функция ЗаполнитьДанныеДляСервисаСозданиеИзменениеЗаказаНаДоставку(ПараметрыЗапроса)
	...
	Если ЭтоСДЭК Тогда
		ДобавитьЗначение(Данные, "tariffPlanIds", ПолучитьЗначение(Параметры, "ИдентификаторыТП", , Новый Массив));
		ДобавитьЗначение(Данные, "numberShipment", Параметры.НомерОтправления, Истина);
	КонецЕсли;
	...
	Если ЭтоДеловыеЛинии Тогда
		ДобавитьЗначение(Данные, "baseDocumentsData", Параметры.ДокументыОснованияДанные, Истина);
	КонецЕсли;
	...
	ДобавитьЗначение(Данные, "shipment.time.exactTime", Параметры.ВремяОтгрузкиФиксированное, Истина);
	...
	ДобавитьЗначение(Данные, "shipment.driverPass.need", Параметры.ПропускЗаказывать, Ложь);
	ДобавитьЗначение(Данные, "shipment.driverPass.orderTime", Параметры.ПропускВремяЗаказа, Ложь);
	ДобавитьЗначение(Данные, "shipment.driverPass.cost", Параметры.ПропускСтоимость, Ложь);
	ДобавитьЗначение(Данные, "shipment.driverPass.type", Параметры.ПропускВозмещениеОплаты, Ложь);
	...
	ДобавитьЗначение(Данные, "delivery.time.exactTime", Параметры.ВремяДоставкиФиксированное, Истина);
	...
	ДобавитьЗначение(Данные, "payment.promoCode", Параметры.Промокод, Истина);
	...
	Если ЭтоДеловыеЛинии Тогда
		ДобавитьЗначение(Данные, "cargo.id", Параметры.ГрузСодержимоеИдентификатор, Истина);
	КонецЕсли;
	...
	ДобавитьЗначение(Данные, "cargo.typeView", Параметры.ГрузВидРасшифровки, Истина);
	...
	Для Каждого ТекущаяУслуга Из ПараметрыЗапроса.Услуги Цикл
		...
		ДобавитьЗначение(НоваяУслуга, "value", ТекущаяУслуга.Стоимость);
	...
	Если ЭтоСДЭК Тогда
		...
		Для Каждого ОписаниеГрузовогоМеста Из Параметры.ГрузовыеМестаВГХ Цикл
		...
		КомпенсацияДоставки = Новый Массив;
		Для Каждого Элемент Из Параметры.КомпенсацияДоставки Цикл
			
			ОписаниеЭлемента = Новый Структура;
			
			ДобавитьЗначение(ОписаниеЭлемента, "value", Элемент.Сумма, Истина);
			ДобавитьЗначение(ОписаниеЭлемента, "vat_sum", Элемент.СуммаНДС, Истина);
			ДобавитьЗначение(ОписаниеЭлемента, "vatCode", Элемент.СтавкаНДС, Истина);
			ДобавитьЗначение(ОписаниеЭлемента, "threshold", Элемент.ПорогСтоимости, Истина);
			
			КомпенсацияДоставки.Добавить(ОписаниеЭлемента);
			
		КонецЦикла;
		ДобавитьЗначение(Данные, "delivery.recipientCost", КомпенсацияДоставки);
	...
	Для Каждого ТекущаяПозиция Из ПараметрыЗапроса.Товары Цикл
		...
		Если ЭтоСДЭК Тогда
			...
			ДобавитьЗначение(НоваяПозиция, "marking", ТекущаяПозиция.КодМаркировки, Истина);
		КонецЕсли;
	...
	Если ЭтоДеловыеЛинии Тогда
		ПлательщикиУслуг = Новый Массив;
		Для Каждого ТекущаяУслуга Из ПараметрыЗапроса.ПлательщикиУслуг Цикл
			НовыйПлательщик = Новый Структура();
			ДобавитьЗначение(НовыйПлательщик, "id", ТекущаяУслуга.Идентификатор, Истина);
			ДобавитьЗначение(НовыйПлательщик, "payer", ТекущаяУслуга.ПлательщикРоль, Истина);
			ПлательщикиУслуг.Добавить(НовыйПлательщик);
		КонецЦикла;
			
		ДобавитьЗначение(Данные, "servicesPayers", ПлательщикиУслуг);
	КонецЕсли;
...
Функция ЗапросСервисаПолучениеСпискаЗаказовНаДоставкуСОтборами(ПараметрыЗапроса, Отказ = Неопределено)
	...
	Если ПараметрыЗапроса.Свойство("Источник") И ЗначениеЗаполнено(ПараметрыЗапроса.Источник) Тогда
		ДобавитьПараметрАдресаHTML(ЗапросСервиса.Адрес, "source", ПараметрыЗапроса.Источник);
	КонецЕсли;
...
Функция ЗапросСервисаПолучениеФайловДоступныхПечатныхФорм(ПараметрыЗапроса, Отказ)
	...
	ТипГрузоперевозки = ПараметрыЗапроса.ТипГрузоперевозки;
	ЭтоДеловыеЛинии = СервисДоставкиКлиентСервер.ЭтоДеловыеЛинии(ТипГрузоперевозки);
	Если СервисДоставкиКлиентСервер.ЭтоСДЭК(ТипГрузоперевозки) Или ЭтоДеловыеЛинии Тогда
		...
		Для Каждого ПечатнаяФорма Из ПараметрыЗапроса.Список Цикл
			...
			МассивПараметровГенерации = Новый Массив;
			Для Каждого ДоступныйПараметр Из ПечатнаяФорма.ПараметрыГенерации Цикл
				ДанныеДоступногоПараметра = Новый Структура;
				ДобавитьЗначение(ДанныеДоступногоПараметра, "name", ДоступныйПараметр.Наименование, Истина);
				ДобавитьЗначение(ДанныеДоступногоПараметра, "amount", ДоступныйПараметр.Количество, Истина);
				МассивПараметровГенерации.Добавить(ДанныеДоступногоПараметра);
			КонецЦикла;
			...
			Если ЭтоДеловыеЛинии Тогда
				ДобавитьЗначение(ДанныеПечатнойФормы, "controlCreate", ПечатнаяФорма.ПроверитьГенерацию, Ложь);
				ДобавитьЗначение(ДанныеПечатнойФормы, "docId", ПечатнаяФорма.ИдентификаторДокумента, Ложь);
			КонецЕсли;
			ДобавитьЗначение(ДанныеПечатнойФормы, "createParameters", МассивПараметровГенерации, Ложь);
...
Функция ЗапросСервисаПолучениеТарифов(ПараметрыЗапроса, Отказ = Неопределено)
	...
	ДобавитьЗначение(Данные, "shipment.time.exactTime", ПараметрыЗапроса.ВремяОтгрузкиФиксированное, Истина);
	...
	ДобавитьЗначение(Данные, "delivery.time.exactTime", ПараметрыЗапроса.ВремяДоставкиФиксированное, Истина);
	...
	Если СервисДоставкиКлиентСервер.ЭтоСДЭК(ТипГрузоперевозки) Тогда
		...
		ДобавитьЗначение(Данные, "type", ПараметрыЗапроса.ТипЗаказа, Истина, 0);
		ДобавитьЗначение(Данные, "tariffPlanIds", ПараметрыЗапроса.ИдентификаторыТП, Истина, Новый Массив);
	...
	Иначе
		...
		ДобавитьЗначение(Данные, "payment.promoCode", ПараметрыЗапроса.Промокод, Истина);
...
Функция ЗапросСервисаПолучениеСпискаДоступныхТерминалов(ПараметрыЗапроса, Отказ = Неопределено)
	...
	Если СервисДоставкиКлиентСервер.ЭтоСДЭК(ТипГрузоперевозки) Тогда
		Если ЗначениеЗаполнено(ПараметрыЗапроса.ИдентификаторыТП) Тогда
			ДобавитьЗначение(Данные, "tariffPlanIds", ПараметрыЗапроса.ИдентификаторыТП, Истина);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПараметрыЗапроса.Первые) Тогда
			ДобавитьЗначение(Данные, "limit", ПараметрыЗапроса.Первые, Истина);
		КонецЕсли;
	КонецЕсли;
...
Функция ЗапросСервисаПолучитьНаселенныеПункты(ПараметрыЗапроса, Отказ = Неопределено)
	...
	ДобавитьЗначение(Данные, "tariffPlanIds", ПараметрыЗапроса.ПодтипыЗаказа, Истина);
...
Функция ЗапросСервисаЗаписьНастроекАвторизации(ПараметрыЗапроса, Отказ = Неопределено)
	...
	ДобавитьЗначение(Данные, "inn", ПараметрыЗапроса.ИНН, Истина);
	ДобавитьЗначение(Данные, "kpp", ПараметрыЗапроса.КПП, Истина);
	ДобавитьЗначение(Данные, "configuration", ПараметрыЗапроса.Конфигурация, Истина);
...
Функция ЗапросСервисаПолучитьХарактерыГруза(ПараметрыЗапроса)

	ЗапросСервиса = НовыйЗапросСервиса(ПараметрыЗапроса);
	
	ЗапросСервиса.Наименование = НСтр("ru = 'Получение классификатора Характеры груза'");
	ЗапросСервиса.Адрес = "deliveryCatalogs/cargos";
	ЗапросСервиса.Метод = "GET";
	Если ПараметрыЗапроса.ТипГрузоперевозки = СервисДоставкиКлиентСервер.ТипГрузоперевозкиСервис1СДоставка() Тогда
		ЗапросСервиса.Контент = "application/vnd-1cbn-delivery-catalog-cargos.v1+json";
	КонецЕсли;
	ЗапросСервиса.Аутентификация = Истина;
	ЗапросСервиса.ОбработкаРезультата = Неопределено;
...
Процедура ОбработатьРезультатЗапросаСоздатьИзменитьЗаказНаДоставку(СтруктураОтвета, Результат,
	...
	Для Каждого Ответ Из МассивОтветовКоллекция Цикл
		...
		Если ГрупповаяОбработка Тогда
			...
			Продолжить;
		КонецЕсли;
		РезультатЗапроса.Вставить("Ошибка", ПолучитьЗначение(Ответ, "error", "", ""));
...
Процедура ОбработатьРезультатЗапросаПолучитьЗаказНаДоставку(СтруктураОтвета, Результат)
	...
	Если ЭтоСДЭК Тогда
		...
		ПараметрыЗаказа.Вставить("НомерОтправления", ПолучитьЗначение(Данные, "numberShipment"));
	...
	ПараметрыЗаказа.Вставить("ВремяОтгрузкиФиксированное", ПолучитьЗначение(Данные, "shipment.time.exactTime", "Булево"));
	...
	ПараметрыЗаказа.Вставить("ВодительНаименованиеОтгрузки", ПолучитьЗначение(Данные, "shipment.driver.name"));
	ПараметрыЗаказа.Вставить("ВодительНомерТелефонаОтгрузки", ПолучитьЗначение(Данные, "shipment.driver.phone"));
	ПараметрыЗаказа.Вставить("ТранспортТипОтгрузки", ПолучитьЗначение(Данные, "shipment.vehicle.type"));
	ПараметрыЗаказа.Вставить("ТранспортНомерОтгрузки", ПолучитьЗначение(Данные, "shipment.vehicle.licencePlate"));
	ПараметрыЗаказа.Вставить("ПропускЗаказывать", ПолучитьЗначение(Данные, "shipment.driverPass.need"));
	ПараметрыЗаказа.Вставить("ПропускВремяЗаказа", ПолучитьЗначение(Данные, "shipment.driverPass.orderTime"));
	ПараметрыЗаказа.Вставить("ПропускСтоимость", ПолучитьЗначение(Данные, "shipment.driverPass.cost"));
	ПараметрыЗаказа.Вставить("ПропускВозмещениеОплаты", ПолучитьЗначение(Данные, "shipment.driverPass.type"));
	...
	ПараметрыЗаказа.Вставить("ВремяДоставкиФиксированное", ПолучитьЗначение(Данные, "delivery.time.exactTime", "Булево"));
	...
	ПараметрыЗаказа.Вставить("ГрузСодержимоеИдентификатор", ПолучитьЗначение(Данные, "cargo.id"));
	...
	ПараметрыЗаказа.Вставить("ГрузВидРасшифровки", ПолучитьЗначение(Данные, "cargo.typeView"));
	...
	Если ЭтоСДЭК Тогда
		...
		ПараметрыЗаказа.Вставить("ПодтипыЗаказа", ПолучитьЗначение(Данные, "tariffPlanIds", , Новый Массив));
		...
		Для Каждого ТекущаяУслуга Из Услуги Цикл
			...
			НоваяУслуга.Стоимость = ПолучитьЗначение(ТекущаяУслуга, "value", , 0);
		...
		// КомпенсацияДоставки
		КомпенсацияДоставки = ПолучитьЗначение(Данные, "delivery.recipientCost", , Новый Массив);
		Для Каждого Элемент Из КомпенсацияДоставки Цикл
			НоваяКомпенсация = ПараметрыЗаказа.КомпенсацияДоставки.Добавить();
			НоваяКомпенсация.Сумма = ПолучитьЗначение(Элемент, "value", , 0);
			НоваяКомпенсация.СуммаНДС = ПолучитьЗначение(Элемент, "vat_sum", , 0);
			НоваяКомпенсация.СтавкаНДС = ПолучитьЗначение(Элемент, "vatCode", , "");
			НоваяКомпенсация.ПорогСтоимости = ПолучитьЗначение(Элемент, "threshold", , 0);
		КонецЦикла;
	...
	Иначе
		...
		ПараметрыЗаказа.Вставить("Промокод", ПолучитьЗначение(Данные, "payment.promoCode",, Ложь));
	
		// Данные об исполнителе
		ПараметрыЗаказа.Вставить("Курьер", ПолучитьЗначение(Данные, "performer.name"));
		ПараметрыЗаказа.Вставить("ВодительНаименованиеДоставки", ПолучитьЗначение(Данные, "performer.name"));
		ПараметрыЗаказа.Вставить("ИсполнительЮридическоеЛицоНаименование", ПолучитьЗначение(Данные, "performer.legalName"));
		ПараметрыЗаказа.Вставить("ВодительНомерТелефонаДоставки", ПолучитьЗначение(Данные, "performer.phone"));
		ПараметрыЗаказа.Вставить("ТранспортТипДоставки", ПолучитьЗначение(Данные, "performer.carModel"));
		ПараметрыЗаказа.Вставить("ТранспортНомерДоставки", ПолучитьЗначение(Данные, "performer.carNumber"));
		...
		// Плательщики услуг
		ПлательщикиУслугПолученные = ПолучитьЗначение(Данные, "servicesPayers", , Новый Массив);
		ПлательщикиУслуг = Новый Массив;
		Для Каждого Плательщик Из ПлательщикиУслугПолученные Цикл
			НовыйПлательщик = НовыйПараметрыПлательщикУслуги();
			НовыйПлательщик.Идентификатор = ПолучитьЗначение(Плательщик, "id");
			НовыйПлательщик.Наименование = ПолучитьЗначение(Плательщик, "name");
			НовыйПлательщик.ПлательщикРоль = ПолучитьЗначение(Плательщик, "payer");
			ПлательщикиУслуг.Добавить(НовыйПлательщик);
		КонецЦикла;
		ПараметрыЗаказа.Вставить("ПлательщикиУслуг", ПлательщикиУслуг);
		
	КонецЕсли;
	
	Если Не ПараметрыЗаказа.Свойство("СостояниеИдентификатор") Или ПараметрыЗаказа.СостояниеИдентификатор = 0 Тогда
...
Процедура ОбработатьРезультатЗапросаПолучитьМультизаказНаДоставку(СтруктураОтвета, Результат)
	...
	ПараметрыМультизаказа.Вставить("ВодительНаименованиеОтгрузки", ПолучитьЗначение(Данные, "shipment.driver.name"));
	ПараметрыМультизаказа.Вставить("ВодительНомерТелефонаОтгрузки", ПолучитьЗначение(Данные, "shipment.driver.phone"));
	ПараметрыМультизаказа.Вставить("ТранспортТипОтгрузки", ПолучитьЗначение(Данные, "shipment.vehicle.type"));
	ПараметрыМультизаказа.Вставить("ТранспортНомерОтгрузки", ПолучитьЗначение(Данные, "shipment.vehicle.licencePlate"));
	...
	// Данные об исполнителе
	ПараметрыМультизаказа.Вставить("ВодительНаименованиеДоставки", ПолучитьЗначение(Данные, "performer.name"));
	ПараметрыМультизаказа.Вставить("ИсполнительЮридическоеЛицоНаименование", ПолучитьЗначение(Данные, "performer.legalName"));
	ПараметрыМультизаказа.Вставить("ВодительНомерТелефонаДоставки", ПолучитьЗначение(Данные, "performer.phone"));
	ПараметрыМультизаказа.Вставить("ТранспортТипДоставки", ПолучитьЗначение(Данные, "performer.carModel"));
	ПараметрыМультизаказа.Вставить("ТранспортНомерДоставки", ПолучитьЗначение(Данные, "performer.carNumber"));
...
Процедура ОбработатьРезультатЗапросаПолучитьЗаказыНаДоставку(СтруктураОтвета, Результат, ПараметрыЗапроса)
	...
	Результат.Вставить("Уведомление", ПолучитьЗначение(СтруктураОтвета, "notification", "", ""));
	...
	Для Каждого ТекущийЗаказ Из СписокЗаказовОтвета Цикл
		...
		СписокПредставленийДокументов = ПолучитьЗначение(ТекущийЗаказ, "carrierDocuments", , Новый Массив);
		ДополнитьПараметрыЗаказаДокументамиПеревозчика(ПараметрыЗаказа, СписокПредставленийДокументов);
...
Процедура ДополнитьПараметрыЗаказаДокументамиПеревозчика(ПараметрыЗаказа, СписокПредставленийДокументов)

	ДокументыПеревозчика = Новый СписокЗначений;
	ДокументыПеревозчика.ЗагрузитьЗначения(СписокПредставленийДокументов);
	КоличествоДокументовПеревозчика = СписокПредставленийДокументов.Количество();
	ПараметрыЗаказа.Вставить("ДокументыПеревозчика", ДокументыПеревозчика);
	ПараметрыЗаказа.Вставить("ЕстьДокументыПеревозчика", КоличествоДокументовПеревозчика > 1);
	
	Если КоличествоДокументовПеревозчика = 1 Тогда
		ПредставлениеДляСписка = ДокументыПеревозчика[0].Значение;
	ИначеЕсли КоличествоДокументовПеревозчика > 1 Тогда
		ПредставлениеДляСписка = СтрШаблон(НСтр("ru = 'Всего документов: %1'"), ДокументыПеревозчика.Количество());
	Иначе
		ПредставлениеДляСписка = "";
	КонецЕсли;
	ПараметрыЗаказа.Вставить("ДокументПеревозчикаПредставление", ПредставлениеДляСписка);
КонецПроцедуры
...
Процедура ОбработатьРезультатЗапросаПолучитьТарифы(СтруктураОтвета, Результат, ПараметрыЗапроса = Неопределено)
	...
	Если СервисДоставкиКлиентСервер.ЭтоСДЭК(ТипГрузоперевозки) Тогда
		ОбработатьРезультатПолучитьТарифы_Элемент(СтруктураОтвета, Результат, ПараметрыЗапроса);
		Возврат;
	КонецЕсли;
	...
	Для Каждого ТекущийЭлемент Из СписокЭлементов Цикл
		...
		Для Каждого ТекущаяДополнительнаяУслуга Из ДополнительныеУслуги Цикл
			...
			Для Каждого СвойствоДополнительнойУслуги Из ТекущаяДополнительнаяУслуга.properties Цикл
				...
				НовоеСвойство.Вставить("ТипЗначения", СервисДоставкиСлужебный.ПолучитьОписаниеТипаПоСтруктуре(СтруктураТипа));
				НовоеСвойство.Вставить("ЕдиницаИзмерения", ПолучитьЗначение(СвойствоДополнительнойУслуги, "unit"));
				НовоеСвойство.Вставить("Использовать", ПолучитьЗначение(СвойствоДополнительнойУслуги, "isSelected"));
				Если НовоеСвойство.Использовать = Истина Тогда
		...
		ДанныеЭлемента.Вставить("ГрафикДоставки", Новый Массив);
		ДополнитьДанныеТарифаГрафикомДоставкиПриНеобходимости(ТекущийЭлемент, ДанныеЭлемента, ТипГрузоперевозки);
...
Процедура ОбработатьРезультатПолучитьТарифы_Элемент(Ответ, Результат, ПараметрыЗапроса = Неопределено)
	
	Список = Новый Массив;
	
	БлокОтветаСписок = ПолучитьЗначение(Ответ, "_embedded", , Новый Структура);
	СписокЭлементов = ПолучитьЗначение(БлокОтветаСписок, "items", , Новый Массив);
	...
	Для Каждого ТекущийЭлемент Из СписокЭлементов Цикл
		...
		Тариф.Вставить("Идентификатор", ПолучитьЗначение(ТекущийЭлемент, "id"));
		Тариф.Вставить("Ошибка", ПолучитьЗначение(ТекущийЭлемент, "error", "", ""));
		Тариф.Вставить("МинимальныйСрок", ПолучитьЗначение(ТекущийЭлемент, "termMin"));
		Тариф.Вставить("МаксимальныйСрок", ПолучитьЗначение(ТекущийЭлемент, "termMax"));
		Тариф.Вставить("Стоимость", ПолучитьЗначение(ТекущийЭлемент, "cost"));
		Тариф.Вставить("СуммаСкидки", ПолучитьЗначение(ТекущийЭлемент, "discount"));
		Тариф.Вставить("ДетализацияСтоимости", Новый Массив);
		
		Для Каждого ТекущаяУслуга Из ПолучитьЗначение(ТекущийЭлемент, "costDetails") Цикл
			Услуга = НовыйДетализацияСтоимостиУслуги();
			Услуга.Идентификатор = ПолучитьЗначение(ТекущаяУслуга, "id");
			Услуга.Стоимость = ПолучитьЗначение(ТекущаяУслуга, "cost");
			Тариф.ДетализацияСтоимости.Добавить(Услуга);
		КонецЦикла;
		
		Список.Добавить(Тариф);
...
Процедура ДополнитьДанныеТарифаГрафикомДоставкиПриНеобходимости(ДанныеОтвета, ЗаполняемыеДанные, ТипГрузоперевозки)

	Если Не СервисДоставкиКлиентСервер.ЭтоДеловыеЛинии(ТипГрузоперевозки) Тогда
		Возврат;
	КонецЕсли;
	
	ГрафикТарифа = ПолучитьЗначение(ДанныеОтвета, "deliveryDates");
	Если ГрафикТарифа = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ЭтоПерваяСтрока = Истина;
	Для Каждого ОписаниеГрафика Из ГрафикТарифа Цикл
		СтрокаГрафика = Новый Структура("КартинкаСтроки, Дата, Описание");
		СтрокаГрафика.КартинкаСтроки =
			?(ЭтоПерваяСтрока, БиблиотекаКартинок.СостояниеГотовностиЗаказаКОформлениюГотовСервисДоставки,
			БиблиотекаКартинок.СостояниеГотовностиЗаказаКОформлениюПроверенСервисДоставки);
		СтрокаГрафика.Дата = ПолучитьЗначение(ОписаниеГрафика, "date", "Дата");
		СтрокаГрафика.Описание = ПолучитьЗначение(ОписаниеГрафика, "name");
		ЗаполняемыеДанные.ГрафикДоставки.Добавить(СтрокаГрафика);
...
Процедура ОбработатьРезультатЗапросаПолучитьДоступныеТерминалы(СтруктураОтвета, Результат, ПараметрыЗапроса)
	...
	Результат.Вставить("Направление", ПараметрыЗапроса.Направление);
...
Функция ОбработатьРезультатЗапросаПолучитьНаложенныеПлатежи(СтруктураОтвета)
	...
	Для Каждого ТекущийПлатеж Из СписокОтвета Цикл
		...
		Если СписокИдентификаторов.Найти(ПараметрыПлатежа.Идентификатор) = Неопределено Тогда
			СписокИдентификаторов.Добавить(ПараметрыПлатежа.Идентификатор);
		Иначе
			// Записи с дублями идентификаторов могут возникнуть только в результате ошибок,
			// связанных с ручной отладкой функциональности. Такие дубли следует отбрасывать.
			Продолжить;
		КонецЕсли;
...
Процедура ОбработатьРезультатЗапросаПолучитьЖурналЗаказовНаДоставкуИзСервиса(СтруктураОтвета, Результат)
	...
	Для Каждого ТекущийЗаказ Из СписокЗаказовОтвета Цикл
		...
		Если СервисДоставкиКлиентСервер.ЭтоДеловыеЛинии(ТипГрузоперевозки) Тогда
			...
			ПараметрыЗаказа.Вставить("ВодительНаименованиеОтгрузки", ПолучитьЗначение(ТекущийЗаказ, "derival.driver.driverName"));
			ПараметрыЗаказа.Вставить("ВодительНомерТелефонаОтгрузки", ПолучитьЗначение(ТекущийЗаказ, "derival.driver.driverPhone"));
			ПараметрыЗаказа.Вставить("ТранспортТипОтгрузки", ПолучитьЗначение(ТекущийЗаказ, "derival.vehicle.vehicleType"));
			ПараметрыЗаказа.Вставить("ТранспортНомерОтгрузки", ПолучитьЗначение(ТекущийЗаказ, "derival.vehicle.vehicleLicencePlate"));
			ПараметрыЗаказа.Вставить("ВодительНаименованиеДоставки", ПолучитьЗначение(ТекущийЗаказ, "arrival.driver.driverName"));
			ПараметрыЗаказа.Вставить("ВодительНомерТелефонаДоставки", ПолучитьЗначение(ТекущийЗаказ, "arrival.driver.driverPhone"));
			ПараметрыЗаказа.Вставить("ТранспортТипДоставки", ПолучитьЗначение(ТекущийЗаказ, "arrival.vehicle.vehicleType"));
			ПараметрыЗаказа.Вставить("ТранспортНомерДоставки", ПолучитьЗначение(ТекущийЗаказ, "arrival.vehicle.vehicleLicencePlate"));
			ПараметрыЗаказа.Вставить("ВнутреннийНомер", ПолучитьЗначение(ТекущийЗаказ, "orderNumber"));
...
Функция ОбработатьРезультатЗапросаПолучитьТарифыИУслуги(РезультатЗапроса, ПараметрыЗапроса)
	...
	ДанныеТарифныеПланы = ПолучитьЗначение(РезультатЗапроса, "tariffPlans", , Новый Массив);
	
	Тарифы = Новый Массив;
	УслугиТарифов = Новый СписокЗначений;
	НесовместимыеУслуги = Новый Массив;
	ТарифныеПланы = Новый Соответствие;
	...
	Для Каждого Тариф Из ДанныеТарифы Цикл
		...
		Для Каждого Услуга Из УслугиТарифа Цикл
			ПараметрыТарифа.Услуги.Добавить(НовыеБазовыеПараметрыСущности(
				ПолучитьЗначение(Услуга, "id", , ""), ПолучитьЗначение(Услуга, "name", , "")));
		КонецЦикла;
	...
	Для Каждого ТарифныйПлан Из ДанныеТарифныеПланы Цикл
		
		ПараметрыТарифногоПлана = Новый Структура;
		ПараметрыТарифногоПлана.Вставить("Идентификатор", ПолучитьЗначение(ТарифныйПлан, "id", , ""));
		ПараметрыТарифногоПлана.Вставить("ТипыЗаказов", ПолучитьЗначение(ТарифныйПлан, "order_type_id", "Массив", Новый Массив));
		ПараметрыТарифногоПлана.Вставить("Наименование", ПолучитьЗначение(ТарифныйПлан, "name", , ""));
		ПараметрыТарифногоПлана.Вставить("Описание", ПолучитьЗначение(ТарифныйПлан, "description", , ""));
		ПараметрыТарифногоПлана.Вставить("Отгрузка", ПолучитьЗначение(ТарифныйПлан, "importantShipment", , Ложь));
		ПараметрыТарифногоПлана.Вставить("Доставка", ПолучитьЗначение(ТарифныйПлан, "importantDelivery", , Ложь));
		
		ТарифныеПланы.Вставить(ПараметрыТарифногоПлана.Идентификатор, ПараметрыТарифногоПлана);
	...
	Результат.Вставить("ТарифныеПланы", ТарифныеПланы);
...
Процедура ОбработатьРезультатЗапросаПолучитьХарактерыГруза(СтруктураОтвета, Результат, ПараметрыЗапроса)
	...
	Для Каждого ТекущийЭлемент Из СписокЭлементов Цикл
		НовыйСписокЭлементов.Добавить(НовыеБазовыеПараметрыСущности(
			ПолучитьЗначение(ТекущийЭлемент, "id"), ПолучитьЗначение(ТекущийЭлемент, "name")));
	КонецЦикла;
	
	Результат.Вставить("Список", НовыйСписокЭлементов);
	
	Кэш = Новый Структура(СервисДоставкиКлиентСервер.ИмяМетодаПолучитьХарактерыГруза(), Результат);
	ОбновитьКэшированныеЗначения(ПараметрыЗапроса.ОрганизацияБизнесСетиСсылка, 
		ПараметрыЗапроса.ТипГрузоперевозки, Кэш, "Грузы");
КонецПроцедуры
...
Процедура ДобавитьПараметрыПоУмолчанию(Параметры, ДанныеПоУмолчанию)
	...
	Если Не ЗначениеЗаполнено(Параметры.Груз.Содержимое) Тогда
		Параметры.Груз.Содержимое = ДанныеПоУмолчанию.ХарактерГрузаСтрокой;
		Параметры.Груз.СодержимоеИдентификатор = ДанныеПоУмолчанию.ХарактерГрузаИдентификатор;
	КонецЕсли;
	
КонецПроцедуры

Процедура ДобавитьВидРасшифровкиПоУмолчанию(Параметры, ВидРасшифровки, ДанныеПоУмолчанию)
	
	Если Не ЗначениеЗаполнено(Параметры.Груз.ВидРасшифровки) Тогда
		
		Результат = "";
		
		Если ЗначениеЗаполнено(ВидРасшифровки) Тогда
			Результат = ВидРасшифровки;
		ИначеЕсли ЗначениеЗаполнено(ДанныеПоУмолчанию.ГрузВидРасшифровки) Тогда
			Результат = ДанныеПоУмолчанию.ГрузВидРасшифровки;
		Иначе
			Результат = "ПоТоварам";
		КонецЕсли;
		
		Параметры.Груз.ВидРасшифровки = Результат;
...
//
// Параметры:
// 	ДатаСтрокой - Строка - Переданная строка с датой
//
// Возвращаемое значение:
//  Дата - Преобразованная  дата
//  
Функция ПрочитатьДатуJSONБезРазделителя(ДатаСтрокой) Экспорт
...
// Проверяет заполение полей по водителю и транспорту
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - форма, из которой вызвана процедура.
//  Направление - Строка - Направление перевозки груза
// 
// Возвращаемое значение:
//  Булево - Результат проверки
Функция ТранспортЗаполнен(Форма, Направление) Экспорт

	Результат = Ложь;

	Если ЗначениеЗаполнено(Форма["ВодительНаименование" + Направление])
		Или ЗначениеЗаполнено(Форма["ВодительНомерТелефона" + Направление])
		Или ЗначениеЗаполнено(Форма["ТранспортТип" + Направление])
		Или ЗначениеЗаполнено(Форма["ТранспортНомер" + Направление]) Тогда
		Результат = Истина;
	КонецЕсли;
