...
//
// Параметры:
//  ДополнительныеПрава - Строка - дополнительные права для проверки, заполненные через ";".
...
//
Функция ИндексыСПАРККонтрагента(Контрагент, ВидКонтрагента) Экспорт
...
// Вызывается из форм, в которые встроен показ индексов 1СПАРК Риски и факторов риска.
//
// Параметры:
...
Процедура ПриСозданииНаСервере(Форма, КонтрагентОбъект, Контрагент, ВидКонтрагента, ПараметрыОтображения) Экспорт
	...
	Если СтандартнаяОбработкаБиблиотекой = Истина Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ИндексыСПАРКРиски") Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ФакторыРискаСПАРК") Тогда
			// Есть несколько способов отображения данных факторов риска на форме:
			//  - библиотека самостоятельно заполняет декорацию,
			//  - ответственные за внедрение пишут код, который (на основании данных индексов) рисует их на форме
			//      в произвольных элементах управления.
			// По умолчанию библиотека самостоятельно заполняет декорацию. Если это поведение необходимо изменить,
			//  то надо прописать код в "СПАРКРискиПереопределяемый.ПриСозданииНаСервере".
			СПАРКФакторыРиска.ОтобразитьФакторыРиска(
				Форма.ФакторыРискаСПАРК,
				Форма,
				КонтрагентОбъект,
				Контрагент,
				ПараметрыОтображения);
...
//    *Ссылка   - СправочникСсылка.ТипыСобытийСПАРКРиски - ссылка на элемент справочника;
...
// Возвращает события мониторинга СПАРК контрагента
//
// Параметры:
//  Контрагент - ОпределяемыйТип.КонтрагентБИП, Строка - Контрагент или ИНН контрагента;
//  ВидКонтрагента - ПеречислениеСсылка.ВидыКонтрагентовСПАРКРиски - вид проверки данных контрагента;
//  ПараметрыЗапроса - Структура - параметры запроса к сервису.
// 
// Возвращаемое значение:
//  см. СПАРКРискиКлиентСервер.НовыйДанныеСобытийМониторингСПАРК
//
Функция СобытияМониторингаСПАРККонтрагента(
		Контрагент,
		ВидКонтрагента,
		ПараметрыЗапроса = Неопределено) Экспорт
	
	Результат = СПАРКРискиКлиентСервер.НовыйДанныеСобытийМониторингСПАРК();
	Результат.РаботаВМоделиСервиса = ОбщегоНазначения.РазделениеВключено();
	Результат.ВидКонтрагента = ВидКонтрагента;
	Результат.ДанныеАутентификацииЗаполнены =
		ИнтернетПоддержкаПользователей.ЗаполненыДанныеАутентификацииПользователяИнтернетПоддержки();
	...
	РезультатСобытияМониторинга = СПАРКРискиСервис.СобытияМониторингаСПАРККонтрагента(
		ОбластьДанныхЛокальная,
		ОписаниеКонтрагента,
		ПараметрыЗапроса);
	
	Если РезультатСобытияМониторинга <> Неопределено Тогда
		
		Результат.Вставить("ЗначенияСобытийМониторинга", РезультатСобытияМониторинга.ЗначенияСобытийМониторинга);
		
		ВидОшибки = РезультатСобытияМониторинга.ВидОшибки;
		Если Результат.ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ПустаяСсылка() Тогда
			Результат.Вставить("ВидОшибки", ВидОшибки);
		Иначе
			// Некоторые виды общих ошибок приоритетнее локальных ошибок по каждому ИНН.
			Если (ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ВнутренняяОшибкаСервиса)
					ИЛИ (ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ИнтернетПоддержкаНеПодключена)
					ИЛИ (ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ИспользованиеЗапрещено)
					ИЛИ (ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ИспользованиеОтключено)
					ИЛИ (ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ОшибкаАутентификации)
					ИЛИ (ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ОшибкаПодключения)
					ИЛИ (ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ПревышеноКоличествоПопытокАутентификации)
					ИЛИ (ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ЗапрещенДоступКИнтернетСервисам)
					ИЛИ (ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ТребуетсяОплатаИлиПревышенЛимит) Тогда
				Результат.Вставить("ВидОшибки", ВидОшибки);
			КонецЕсли;
		КонецЕсли;
		Результат.Вставить("ТекстОшибки",              РезультатСобытияМониторинга.СообщениеОбОшибке);
	Иначе
		Результат.Вставить("ВидОшибки",                Перечисления.ВидыОшибокСПАРКРиски.НеизвестнаяОшибка);
		Результат.Вставить("ТекстОшибки",              НСтр("ru='Ошибка получения событий мониторинга СПАРК'"));
	КонецЕсли;
...
Функция ЗапроситьНовуюСправкуВСервисеСПАРКРиски(Контрагент) Экспорт
	...
	ИдентификаторУслуги = ИдентификаторУслугиСправкиПоКомпаниям();
	Если Не ИнтернетПоддержкаПользователей.УслугаПодключена(ИдентификаторУслуги) Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(РезультатВызоваОперации.ИнформацияОбОшибке);
		ПодготовитьОписаниеОшибки(Результат, РезультатВызоваОперации);
		Возврат Результат;
		
	КонецЕсли;
	
	СвойстваСправочниковКонтрагенты = СвойстваСправочниковКонтрагентов();
	...
	ИначеЕсли Не ИННСоответствуетТребованиям(ДанныеКонтрагента.ИНН, ВидКонтрагента, ОшибкаИНН) Тогда
	...
	Если Не ЗначениеЗаполнено(ДанныеКонтрагента.ВидКонтрагента)
		...
		РезультатВызоваОперации = СПАРКРискиСервис.НачатьПодготовкуСправокЮридическихЛиц(
			СписокИНН);
	Иначе
		РезультатВызоваОперации = СПАРКРискиСервис.НачатьПодготовкуСправокИндивидуальныхПредпринимателей(
			СписокИНН);
	КонецЕсли;
...
Функция АктуальнаяСправка(ПараметрыПоиска) Экспорт
	
	Параметры = Новый Структура("ИНН, Контрагент");
	ЗаполнитьЗначенияСвойств(Параметры, ПараметрыПоиска);
	Контрагент = Параметры.Контрагент;
	ИНН = Параметры.ИНН;
	
	Результат = Новый Структура("АктуальнаяСправка, Описание");
	ШаблонОписания = НСтр("ru = 'Справка от %1'");
	...
	Если Контрагент <> Неопределено Тогда 
		...
		Запрос.Текст = 
			...
			|ГДЕ
			|	СправкиСПАРКРиски.Контрагент = &Контрагент
			|	И СправкиСПАРКРиски.Состояние = &Состояние
			|
			|УПОРЯДОЧИТЬ ПО
			|	Дата УБЫВ";
		
		Запрос.УстановитьПараметр("Контрагент", Контрагент);
		Запрос.УстановитьПараметр("Состояние", Перечисления.СостоянияСправкиСПАРКРиски.Получена);
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Результат.АктуальнаяСправка = ВыборкаДетальныеЗаписи.Ссылка;
			ДатаДокумента = ВыборкаДетальныеЗаписи.Дата;
			Результат.Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонОписания,
	...
	Если Результат.АктуальнаяСправка = Неопределено
		И Не ПустаяСтрока(ИНН) Тогда
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия              = "2.8.4.2";
		Обработчик.Процедура           = "СПАРКФакторыРиска.НастройкаОбновленияФакторовРискаСПАРК";
		Обработчик.НачальноеЗаполнение = Ложь;
		Обработчик.ОбщиеДанные         = Ложь;
		Обработчик.Комментарий         = НСтр("ru='1СПАРК Риски. Настройка обновления факторов риска СПАРК.'");
		Обработчик.РежимВыполнения     = "Отложенно";
		Обработчик.Идентификатор       = Новый УникальныйИдентификатор("325c208e-066a-4447-9c45-bca3ee5d8b5b");
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьДанныеИндивидуальныхПредпринимателей");
	Методы.Вставить("ЗаполнитьНовыеПоляТиповСобытий");
	Методы.Вставить("ЗаполнитьНовыеРесурсыИндексыСПАРКРиски");
	Методы.Вставить("НачальноеЗаполнениеДанныхСПАРКРиски");
	Методы.Вставить("ОбновлениеИнформационнойБазы_АктуализироватьОписаниеТиповСобытий");
	Методы.Вставить("ОбновлениеИнформационнойБазы_ОчиститьРезультатыПостановкиНаМониторинг_2_1_9_2");
	Методы.Вставить("ОбщиеДанныеНачальноеЗаполнениеДанныхСПАРКРиски");
	Методы.Вставить("ОбщиеДанныеПерейтиНаВерсию_2_1_8_15");
	Методы.Вставить("ПерейтиНаВерсию_2_1_7_10");
	Методы.Вставить("ПерейтиНаВерсию_2_1_7_11");
...
Процедура ПодготовитьОписаниеОшибки(Результат, РезультатВызоваОперации)
	...
	ИначеЕсли РезультатВызоваОперации.ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ЗапрещенДоступКИнтернетСервисам Тогда
...
Функция ДанныеДляПросмотраФайлаСправки(Справка, УникальныйИдентификатор) Экспорт
	...
	// Вид контрагента может быть не заполнен, если не завершено
	// обновление информационной базы.
	ВидКонтрагента = ОбработатьВидКонтрагента(Выборка.ВидКонтрагента);
	...
	Если Результат.ВидОшибки.Пустая()
		И Результат.ДанныеФайла = Неопределено Тогда
...
Процедура ПроверитьСостояниеСправки(
	...
	Если ВидКонтрагента = Перечисления.ВидыКонтрагентовСПАРКРиски.ЮридическоеЛицо Тогда
		РезультатВызоваОперации = СПАРКРискиСервис.СостояниеПодготовкиСправокЮридическихЛиц(
			Идентификаторы);
	Иначе
		РезультатВызоваОперации = СПАРКРискиСервис.СостояниеПодготовкиСправокИндивидуальныхПредпринимателей(
			Идентификаторы);
	КонецЕсли;
	...
	Если СостояниеСправкиВСервисе <> СостоянияСправки.Готовится Тогда
		УстановитьСостояниеСправки(ДокументСсылка, СостояниеСправкиВСервисе);
		ОтменитьЗаданиеПроверкиГотовностиСправкиСПАРКРиски(ИдентификаторСправки);
...
Процедура ЗагрузитьФайлСправки(
	...
	Если ВидКонтрагента = Перечисления.ВидыКонтрагентовСПАРКРиски.ЮридическоеЛицо Тогда
		РезультатВызоваОперации = СПАРКРискиСервис.ЗагрузитьФайлСправкиЮридическихЛиц(
			ИдентификаторСправки);
	Иначе
		РезультатВызоваОперации = СПАРКРискиСервис.ЗагрузитьФайлСправкиИндивидуальныхПредпринимателей(
			ИдентификаторСправки);
	КонецЕсли;
...
//     * Контрагент     - ОпределяемыйТип.КонтрагентБИП - контрагент;
//     * ИНН            - Строка.
//
...
//  ВидКонтрагента - ПеречислениеСсылка.ВидыКонтрагентовСПАРКРиски - вид данных контрагента.
...
Процедура ЗапуститьФоновоеЗаданиеИндексыСПАРК(
	...
	РезультатЗапускаФоновогоЗадания = ДлительныеОперации.ВыполнитьВФоне(
		"СПАРКРискиСервис.ЗагрузитьИндексыКонтрагентовВФоне",
...
Процедура ОбновитьИндексыКонтрагентовОтчет(
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если Не Отказ
			И ОписанияКонтрагентов.Количество() >= 1000 Тогда
			
			РезультатВызоваВебСервиса = СПАРКРискиСервис.ЗагрузитьИндексыКонтрагентов(
				КонтрагентОбластьДанных,
				ОписанияКонтрагентов);
			Если ЗначениеЗаполнено(РезультатВызоваВебСервиса.ВидОшибки) Тогда
				Отказ = Истина;
			КонецЕсли;
...
Функция URLВебВерсииСПАРК(Контрагент) Экспорт
	...
	РезультатПолучения = СПАРКРискиСервис.URLВебВерсииСПАРК(
		Реквизиты.ИНН);
...
Процедура ЗаполнитьНовыеПоляТиповСобытий(Параметры) Экспорт
	...
	// производства.
	РезультатВызова = СПАРКРискиСервис.ТипыСобытийСообщенийМониторинга();
...
Процедура ЗаполнитьВидКонтрагента()
	...
	Пока Истина Цикл
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Исключение
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось установить вид контрагента ИНН %1:
						|%2'"),
...
Процедура ЗаполнитьОписаниеОшибкиФормированияОтчета(ДокументРезультат, ВидОшибки) Экспорт
	...
	ИначеЕсли ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ЗапрещенДоступКИнтернетСервисам Тогда
		ИмяОбласти = "ЗапрещенДоступКИнтернетСервисам";
