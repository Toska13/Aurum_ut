...
//
Процедура ОбработатьОтветБанкаНаОтправкуДокументаAsync(Знач ОтветБанка, Знач ПакетОбменСБанками) Экспорт
	
	ДанныеПоТипу = ОбменСБанкамиСлужебный.ПрочитатьXDTOИзДвоичныхДанныхПоТипу(ОтветБанка, "ResultBank");
	
	Если ДанныеПоТипу.ЕстьОшибка Тогда
		ВызватьИсключение ДанныеПоТипу.ПредставлениеОшибки;
	Иначе
		ResultBank = ДанныеПоТипу.Значение;
	КонецЕсли;
...
//   * ДатаПолученияИтоговойВыписки - Дата - последняя дата получения итоговой выписки.
//   * ТекстОшибки - Строка - текст ошибки, которая возникла при последнем получении выписки.
//   * ТекстОшибкиНастройкиАвтоматическойВыписки - Строка - текст ошибки, связанный с настройкой автоматической выписки.
//   * СоздаватьОтсутствующиеЭлементы - Булево - признак создания новых элементов при загрузке автоматической выписки.
//   * ПроводитьДокументы - Булево - признак проведения документов при загрузки автоматической выписки.
//   * ОтносительнаяДатаПолученияВыписки - Строка - строковое представление даты получения выписки
//   * ОтносительнаяДатаПолученияИтоговойВыписки - Строка - строковое представление даты получения итоговой выписки
//   * ВидНастройкиАвтоматическойВыписки - Строка - см. ОбменСБанкамиСлужебныйКлиентСервер.ВидыНастройкиАвтоматическойВыписки()
//   * РасписаниеПромежуточнойВыписки - Структура - данные расписания для промежуточной выписки
//      ** ВремяНачала - Дата - время начала выполнения регламетного задания
//      ** ВремяКонца - Дата - время окончания выполнения регламетного задания
//      ** ПериодПовтораДней - Число - период времени в днях, через который выполняется выписка
//      ** ПериодПовтораВТечениеДня - Число - период времени в секундах, через который выполняется выписка
//   * ВремяНачалаИтоговойВыписки - Дата - время выполнения итоговой выписки
//   * ОшибкаВЖурналеРегистрации - Булево - признак наличия ошибки при последней загрузке автоматической выписки.
//   * ДатаСинхронизации - Дата - вреия выполнения последней синхронизации с банком
//
Функция ПараметрыАвтоматическогоПолученияВыписки(Знач НастройкаОбмена) Экспорт
	
	СтруктураВозврата = Новый Структура;
	ПустаяДата = Дата(1, 1, 1);
	СтруктураВозврата.Вставить("АвтоматическоеПолучениеВыписки", Ложь);
	СтруктураВозврата.Вставить("ДатаПолученияВыписки", ПустаяДата);
	СтруктураВозврата.Вставить("ДатаПолученияИтоговойВыписки", ПустаяДата);
	СтруктураВозврата.Вставить("ТекстОшибки", "");
	ТекстОшибкиНастройкиАвтоматическойВыписки = ОбменСБанкамиСлужебный.ОшибкиРаботыАвтоматическойВыписки(НастройкаОбмена);
	СтруктураВозврата.Вставить("ТекстОшибкиНастройкиАвтоматическойВыписки", ТекстОшибкиНастройкиАвтоматическойВыписки);
	СтруктураВозврата.Вставить("ТекстПредупрежденийНастройкиАвтоматическойВыписки", "");
	Если Не ЗначениеЗаполнено(ТекстОшибкиНастройкиАвтоматическойВыписки) Тогда
		ПредупрежденияРасписанияАвтоматическойВыписки =
			ОбменСБанкамиСлужебный.ПредупрежденияРасписанияАвтоматическойВыписки(НастройкаОбмена);
		Если ЗначениеЗаполнено(ПредупрежденияРасписанияАвтоматическойВыписки) Тогда
			ПредупрежденияРасписания = Новый Массив;
			Для каждого ПредупреждениеРасписания Из ПредупрежденияРасписанияАвтоматическойВыписки Цикл
				ПредупрежденияРасписания.Добавить(ПредупреждениеРасписания);
			КонецЦикла;
			ТекстПредупрежденийНастройкиАвтоматическойВыписки = СтрСоединить(ПредупрежденияРасписания, Символы.ПС);
			СтруктураВозврата.Вставить("ТекстПредупрежденийНастройкиАвтоматическойВыписки",
				ТекстПредупрежденийНастройкиАвтоматическойВыписки);
		КонецЕсли;
	КонецЕсли;

	СтруктураВозврата.Вставить("СоздаватьОтсутствующиеЭлементы", Истина);
	СтруктураВозврата.Вставить("ПроводитьДокументы", Истина);
	СтруктураВозврата.Вставить("ОшибкаВЖурналеРегистрации", Ложь);
	СтруктураВозврата.Вставить("ДатаСинхронизации", ПустаяДата);
	...
	ОтносительнаяДатаПолученияВыписки = ОтносительнаяДатаСинхронизации(СтруктураВозврата.ДатаПолученияВыписки);
	СтруктураВозврата.Вставить("ОтносительнаяДатаПолученияВыписки", ОтносительнаяДатаПолученияВыписки);
	
	ОтносительнаяДатаПолученияИтоговойВыписки = ОтносительнаяДатаСинхронизации(
		СтруктураВозврата.ДатаПолученияИтоговойВыписки);
	СтруктураВозврата.Вставить("ОтносительнаяДатаПолученияИтоговойВыписки", ОтносительнаяДатаПолученияИтоговойВыписки);
	
	Если ЗначениеЗаполнено(Выборка.ТекстОшибки) Тогда
		СтруктураВозврата.ОшибкаВЖурналеРегистрации = Истина;
	КонецЕсли;
	
	ВидыНастройкиАвтоматическойВыписки = ОбменСБанкамиСлужебныйКлиентСервер.ВидыНастройкиАвтоматическойВыписки();
	СтруктураВозврата.Вставить("ВидНастройкиАвтоматическойВыписки", ВидыНастройкиАвтоматическойВыписки.БезНастройки);
	Если ЗначениеЗаполнено(Выборка.РегламентноеЗаданиеСтандартнойНастройкиВыписки) Тогда
		СтруктураВозврата.ВидНастройкиАвтоматическойВыписки = ВидыНастройкиАвтоматическойВыписки.Стандартная;
	Иначе
		Если ЗначениеЗаполнено(Выборка.РегламентноеЗаданиеПромежуточнойВыписки) Тогда
			СтруктураВозврата.ВидНастройкиАвтоматическойВыписки = ВидыНастройкиАвтоматическойВыписки.Расширенная;
			УстановитьПривилегированныйРежим(Истина);
			РегламентноеЗадание = РегламентныеЗаданияСервер.Задание(Выборка.РегламентноеЗаданиеПромежуточнойВыписки);
			Если РегламентноеЗадание <> Неопределено Тогда
				Расписание = РегламентноеЗадание.Расписание;
				РасписаниеПромежуточнойВыписки = Новый Структура;
				РасписаниеПромежуточнойВыписки.Вставить("ВремяНачала", Расписание.ВремяНачала);
				РасписаниеПромежуточнойВыписки.Вставить("ВремяКонца", Расписание.ВремяКонца);
				РасписаниеПромежуточнойВыписки.Вставить("ПериодПовтораДней", Расписание.ПериодПовтораДней);
				РасписаниеПромежуточнойВыписки.Вставить("ПериодПовтораВТечениеДня", Расписание.ПериодПовтораВТечениеДня);
				СтруктураВозврата.Вставить("РасписаниеПромежуточнойВыписки", РасписаниеПромежуточнойВыписки);
			КонецЕсли;
			УстановитьПривилегированныйРежим(Ложь);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Выборка.РегламентноеЗаданиеИтоговойВыписки) Тогда
			СтруктураВозврата.ВидНастройкиАвтоматическойВыписки = ВидыНастройкиАвтоматическойВыписки.Расширенная;
			УстановитьПривилегированныйРежим(Истина);
			РегламентноеЗадание = РегламентныеЗаданияСервер.Задание(Выборка.РегламентноеЗаданиеИтоговойВыписки);
			Если РегламентноеЗадание <> Неопределено Тогда
				Расписание = РегламентноеЗадание.Расписание;
				СтруктураВозврата.Вставить("ВремяНачалаИтоговойВыписки", РегламентноеЗадание.Расписание.ВремяНачала);
			КонецЕсли;
			УстановитьПривилегированныйРежим(Ложь);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОстановитьАвтоматическоеПолучениеВыписки(Знач НастройкаОбмена) Экспорт
	...
	НастройкиЗадания = ОбменСБанкамиСлужебный.НовыеНастройкиЗаданияАвтоматическойВыписки(НастройкаОбмена, "БезНастроек");
...
// Включает регламентное задание для автоматического получения выписки для стандартной настройки.
//
// Параметры:
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - настройка обмена с банком.
//
Процедура ВключитьРегламентноеЗаданиеПоСтандартнойНастройке(НастройкаОбмена) Экспорт
	
	НастройкиЗадания = ОбменСБанкамиСлужебный.НовыеНастройкиЗаданияАвтоматическойВыписки(НастройкаОбмена, "Стандартные");
	ОбменСБанкамиСлужебный.УстановитьНастройкиЗаданияАвтоматическойВыписки(НастройкиЗадания);
	
КонецПроцедуры

// Возвращает данные по дате и периоду для загрузки выписки банка
//
// Параметры:
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - настройка обмена с банком.
//  Дата - Дата - дата, относительно которой рассчитывается период загрузки.
// 
// Возвращаемое значение:
//  Структура - параметры периода загрузки выписки. Содержит поля:
//   * ПериодЗагрузкиВыписки - Число - число дней, за которые банк разрешает загружать выписку
//   * ДатаНачалаЗагрузки - Дата - дата, с которой будет загружаться выписка банка
//
Функция ДанныеПериодаЗапросаВыписки(НастройкаОбмена, Дата) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ПериодЗагрузкиВыписки", 14);
	Результат.Вставить("ДатаНачалаЗагрузки", '00010101');
	
	Банк = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НастройкаОбмена, "Банк");
	НастройкиОбмена = ПоставляемыеНастройкиОбмена(Банк);
	Если ЗначениеЗаполнено(НастройкиОбмена) Тогда
		Результат.ПериодЗагрузкиВыписки = НастройкиОбмена.ПериодЗапросаВыписки;
	КонецЕсли;
	
	СекундВСутках = 86400;
	ДатаЗагрузкиОтБанка = Дата - (Результат.ПериодЗагрузкиВыписки * СекундВСутках);
	Результат.ДатаНачалаЗагрузки = ДатаЗагрузкиОтБанка;
	
	Возврат Результат;
	
КонецФункции

// Возвращает заголовок ошибки с гиперссылкой на журнал регистрации, в зависимости от прав пользователя
//
// Параметры:
//  ДатаОшибки - Дата - дата возникновения ошибки.
// 
// Возвращаемое значение:
//  Строка - заголовок ошибки
//
Функция ЗаголовокСостоянияОшибка(ДатаОшибки) Экспорт
	
	Если Пользователи.РолиДоступны("ПросмотрЖурналаРегистрации") Тогда
		ТекстЗаголовка = НСтр("ru = '%1 при синхронизации с банком произошла ошибка. Подробнее в <a href = ""ЖурналРегистрации"">Журнале регистрации</a>.'");
	Иначе
		ТекстЗаголовка = НСтр("ru = '%1 при синхронизации с банком произошла ошибка. Подробнее в журнале регистрации, обратитесь к администратору.'");
	КонецЕсли;
	
	ЗаголовокОшибки = СтрШаблон(ТекстЗаголовка, Формат(ДатаОшибки, "ДФ='dd.MM.yyyy ""в"" HH:mm'"));
	
	Возврат СтроковыеФункции.ФорматированнаяСтрока(ЗаголовокОшибки);
	
КонецФункции

// Выполняет запись ошибки в регистр сведений "ПараметрыОбменСБанками"
//
// Параметры:
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - настройка обмена с банком.
//  ТекстОшибки - Строка - текст ошибки
//  Дата - Дата - дата для записи в ресурс "ДатаПолученияВыписки" (необязательный)
// 
// Возвращаемое значение:
//  Структура - параметры периода загрузки выписки. Содержит поля:
//   * ПериодЗагрузкиВыписки - Число - число дней, за которые банк разрешает загружать выписку
//   * ДатаПолученияВыписки - Дата - дата, с которой будет загружаться выписка банка
//
Процедура ЗаписатьОшибкуВПараметрыОбмена(НастройкаОбмена, ТекстОшибки, ДатаНачала = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ДатаНачала) Тогда
		МенеджерЗаписи.ДатаПолученияВыписки = ДатаНачала;
	КонецЕсли;
...
Функция РеквизитыБанка(БИК) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		СтруктураВозврата.Ссылка = Выборка.Ссылка;
	КонецЕсли;
...
// Получает значения реквизитов настройки обмена с банком.
//
// Параметры:
...
//      * АвтоматическоеПолучениеВыписки - Булево - Истина, если используется получение выписки по регламентному заданию.
...
//      * Банк - ОпределяемыйТип.БанкОбменСБанками - ссылка на банк в настройке обмена.
//
Процедура ПолучитьЗначенияРеквизитовНастройкиОбмена(Знач НастройкаОбмена, ЗапрашиваемыеРеквизиты) Экспорт
	...
	РеквизитыНастройкиОбмена = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НастройкаОбмена,
		...
		|СертификатПодключения, ДополнительнаяОбработка, ОтправлятьДокументыБезПодписиSMS, Организация, Банк");
	...
	Если ЗапрашиваемыеРеквизиты.Свойство("АвтоматическоеПолучениеВыписки") Тогда
		ПараметрыАвтоВыписки = ПараметрыАвтоматическогоПолученияВыписки(НастройкаОбмена);
		ЗапрашиваемыеРеквизиты.АвтоматическоеПолучениеВыписки = ПараметрыАвтоВыписки.АвтоматическоеПолучениеВыписки;
	КонецЕсли;
...
Функция РеквизитыЭлектронногоДокумента(Знач ДокументСсылка) Экспорт
	...
	|	СостоянияОбменСБанками.СообщениеОбмена.Банк КАК Банк
	|ИЗ
	...
	Если Результат.Следующий() Тогда
		СтруктураВозврата = Новый Структура("Состояние, СообщениеОбмена, ПрограммаБанка, НастройкаОбмена, Банк");
...
Функция ДанныеПрисоединенногоФайла(ПрисоединенныйФайл) Экспорт
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПрисоединенныйФайл, "ПодписанЭП") Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ДвоичныеДанныеСертификата = ЭлектроннаяПодпись.СертификатИзДвоичныхДанныхПодписи(Подпись);
			Если ДвоичныеДанныеСертификата <> Неопределено Тогда
				
				СтруктураПодписей = Новый Структура;
				СтруктураПодписей.Вставить("НомерСтроки", Выборка.НомерСтроки);
				СтруктураПодписей.Вставить("Сертификат",  ДвоичныеДанныеСертификата);
				СтруктураПодписей.Вставить("Подпись",     Подпись);
...
Процедура ИзменитьСообщениеОбмена(СообщениеОбмена, Знач Реквизиты) Экспорт
	...
	Если Реквизиты.Свойство("УдалитьЭП") Тогда
		...
		Если ЗначениеЗаполнено(ПрисоединенныйФайл) Тогда
			УстановитьПривилегированныйРежим(Истина);
			МассивПодписей = ЭлектроннаяПодпись.ПодписиОбъекта(ПрисоединенныйФайл);
...
Процедура НачатьОперациюСПаролемВБезопасномХранилище(Контекст) Экспорт
	...
	Если Контекст.ИдентификаторМетода = "Прочитать" Тогда
		...
		ДанныеАвторизацииНастройкаОбмена = ОбменСБанкамиСлужебный.ДанныеАвторизацииНастройкаОбмена(Контекст.НастройкаОбмена);
		ДанныеАвторизации = ДанныеАвторизацииНастройкаОбмена.ДанныеАвторизации;

		Контекст.Вставить("ДанныеАвторизации", ДанныеАвторизации);
	...
	ПараметрыНаправления.Вставить("Идентификатор",
		ОбменСБанкамиСлужебный.ИдентификаторМетодаОперацииСПаролемВБезопасномХранилище(Контекст.ИдентификаторМетода));
	ПараметрыНаправления.Вставить("Направление", "0");
	
	ПараметрыОперации = Новый Структура();
	ПараметрыОперации.Вставить("ПараметрыМетода", Контекст);
	ПараметрыОперации.Вставить(
		"Идентификатор",
		ОбменСБанкамиСлужебный.ИдентификаторМетодаОперацииСПаролемВБезопасномХранилище("2_353_538_853"));
	ПараметрыОперации.Вставить("Направление", ПараметрыНаправления);
	
	Если ПараметрыОперации.ПараметрыМетода.Свойство("КлючевыеПараметры") Тогда
		ПараметрыОперации.ПараметрыМетода.КлючевыеПараметры.Вставить("ОбластьДанных",
			РаботаВМоделиСервиса.ЗначениеРазделителяСеанса());		
	КонецЕсли;
	
	ОбменСБанкамиСлужебный.ПродолжитьОперациюСПаролемВБезопасномХранилище(ПараметрыОперации);
			
	ЗаписатьДанныеВБезопасноеХранилище = Ложь;
	
	Если Контекст.ИдентификаторМетода = "Сохранить" Тогда
		
		ЗаписатьДанныеВБезопасноеХранилище = Истина;
		
	ИначеЕсли Контекст.ИдентификаторМетода = "Прочитать" И НЕ Контекст.Успех Тогда
		
		Контекст.ДанныеАвторизации = Неопределено;
		ЗаписатьДанныеВБезопасноеХранилище = Истина;
		
	КонецЕсли;
	
	Если ЗаписатьДанныеВБезопасноеХранилище Тогда
		
		ОбменСБанкамиСлужебный.СохранитьДанныеАвторизацииОбменСБанками(Контекст.НастройкаОбмена, Контекст.ДанныеАвторизации);
...
Функция ЕстьДанныеАвторизацииВБезопасномХранилище(НастройкаОбмена) Экспорт
	
	ЕстьДанные = Ложь;
	
	ДанныеАвторизацииНастройкаОбмена = ОбменСБанкамиСлужебный.ДанныеАвторизацииНастройкаОбмена(НастройкаОбмена);
	Если ЗначениеЗаполнено(ДанныеАвторизацииНастройкаОбмена.ДанныеАвторизации) Тогда
...
// Удаляет ДанныеАвторизации или пароль авторизации по автовыписки у текущего пользователя для
//  указанной НастройкаОбмена в Безопасном хранилище
//
// Параметры:
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - ссылка на настройку обмена с банками.
//  Вид - Строка - параметр, указывающий какую именно информацию нужно удалить. Возможные значения:
//   * "ДляПользователя" - для удаления данных авторизации текущего пользователя для указанной НастройкаОбмена
//   * "ДляАвтовыписки" - для удаления пароля от Автовыписки для указанной НастройкаОбмена
//
// См. ОбменСБанкамиСлужебный.УдалитьДанныеАвторизацииОбменСБанками()
//
Процедура УдалитьДанныеАвторизацииОбменСБанками(НастройкаОбмена, Вид) Экспорт
	
	ОбменСБанкамиСлужебный.УдалитьДанныеАвторизацииОбменСБанками(НастройкаОбмена, Вид);
...
// 
// Возвращаемое значение:
// Массив из ДокументСсылка.ПакетОбменСБанками - список пакетов обмена к отправке
//
Функция ПодготовленныеКОтправкеПакеты(Знач НастройкаОбмена) Экспорт
	
	ЗапросПодготовленныхПакетов = Новый Запрос;
	ЗапросПодготовленныхПакетов.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	...
	|	И ПакетЭД.Дата >= &ДатаНачала
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	|
	|ВЫБРАТЬ
	|	ВТ_ПодготовленныеПакеты.Ссылка КАК Пакет,
	|	ТЧСообщенияВПакете.Сообщение КАК СообщениеОбмена,
	|	ТЧСообщенияВПакете.Сообщение.ВидЭД КАК ВидЭД,
	|	ЕСТЬNULL(СвязанныеОбъектыОбменСБанками.СсылкаНаОбъект, НЕОПРЕДЕЛЕНО) КАК СвязанныйОбъект
	|ПОМЕСТИТЬ ВТ_ОбъектыВПакетах
	|ИЗ
	|	ВТ_ПодготовленныеПакеты КАК ВТ_ПодготовленныеПакеты
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПакетОбменСБанками.Сообщения КАК ТЧСообщенияВПакете
	|	ПО ВТ_ПодготовленныеПакеты.Ссылка = ТЧСообщенияВПакете.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СвязанныеОбъектыОбменСБанками КАК СвязанныеОбъектыОбменСБанками
	|	ПО ТЧСообщенияВПакете.Сообщение = СвязанныеОбъектыОбменСБанками.СообщениеОбмена
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СвязанныйОбъект
	|;
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТ_ОбъектыВПакетах.Пакет КАК Ссылка
	|ИЗ
	|	ВТ_ОбъектыВПакетах КАК ВТ_ОбъектыВПакетах
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияОбменСБанками КАК СостоянияОбменСБанками
	|	ПО ВТ_ОбъектыВПакетах.СвязанныйОбъект = СостоянияОбменСБанками.СсылкаНаОбъект
	|
	|ГДЕ
	|	СостоянияОбменСБанками.СсылкаНаОбъект ЕСТЬ NULL
	|	ИЛИ ВТ_ОбъектыВПакетах.СообщениеОбмена = СостоянияОбменСБанками.СообщениеОбмена
	|	ИЛИ ВТ_ОбъектыВПакетах.ВидЭД <> СостоянияОбменСБанками.СообщениеОбмена.ВидЭД
	|	ИЛИ СостоянияОбменСБанками.Состояние В (
	...
	// Платежные поручения действительны в течение 10 дней с момента создания,
	// поэтому более старые неотправленные пакеты отправлять уже не нужно. 
	ЗапросПодготовленныхПакетов.УстановитьПараметр("ДатаНачала", НачалоДня(ТекущаяДатаСеанса()) - 10 * ДлинаСуток);
		...
		Если РеквизитыНастройкиОбмена.ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен Тогда
			...
			Если Не ТребуетсяПодпись Тогда
				...
				Если ЗначениеЗаполнено(МассивБанковскихСчетов) Тогда
					...
					НовыйПакет = Неопределено;
					ОбменСБанкамиСлужебный.СоздатьПакетОбменСБанками(
						НастройкаОбмена, МассивСообщенийЗапросов, НовыйПакет);
...
Функция ДанныеСертификатовСообщенийОбмена(Знач МассивСообщенийОбмена, Знач НастройкаОбмена) Экспорт
	...
	Пока ВыборкаСообщениеОбмена.Следующий() Цикл
		...
		Пока ВыборкаСертификаты.Следующий() И НЕ ВыборкаСертификаты.Подпись = NULL Цикл
			
			Подпись = ВыборкаСертификаты.Подпись.Получить();
			ДвоичныеДанныеСертификата = ЭлектроннаяПодпись.СертификатИзДвоичныхДанныхПодписи(Подпись);
			Если ДвоичныеДанныеСертификата <> Неопределено Тогда
				
				МассивДанныхСертификатов.Добавить(ДвоичныеДанныеСертификата);
...
Функция ДоступныеСертификаты(Знач НастройкаОбмена) Экспорт
	...
	|	И СертификатыПодписейОрганизации.СертификатЭП В
	|			(ВЫБРАТЬ
...
Функция ДанныеСертификатовБанка(НастройкаОбмена) Экспорт
	...
	|	Справочник.НастройкиОбменСБанками.СертификатыПодписейОрганизации КАК НастройкиОбменСБанкамиСертификатыЭП
	|ГДЕ
	|	НастройкиОбменСБанкамиСертификатыЭП.СертификатЭП В
	|			(ВЫБРАТЬ
	|				ВТСертификатыДоступныеПользователю.Ссылка
	|			ИЗ
	|				ВТСертификатыДоступныеПользователю КАК ВТСертификатыДоступныеПользователю)
...
Функция ДанныеСертификатовПоНастройкеОбмена(Знач НастройкаОбмена) Экспорт
	...
	|	И НастройкиОбменСБанкамиСертификатыПодписейОрганизации.СертификатЭП.Ссылка В
	|			(ВЫБРАТЬ
	|				ВТСертификатыДоступныеПользователю.Ссылка
	|			ИЗ
	|				ВТСертификатыДоступныеПользователю КАК ВТСертификатыДоступныеПользователю)";
...
//   * Проекты - Массив - массив строк поддерживаемых проектов, описание строк
//                        в ОбменСБанкамиСлужебныйКлиентСервер.ДоступныеПроекты
//   * ИдентификаторВК - Строка - идентификатор используемой банком внешней компоненты;
//   * ВремяЖизниСессии - Строка - время жизни авторизованной сессии на стороне банка в секундах.
//   * ПериодЗагрузкиВыписки - Число - число дней, за которые банк разрешает загружать выписку
...
Функция ПоставляемыеНастройкиОбмена(Банк, ВключитьФО = Ложь, ЕстьПравоСозданияНастройкиЭДО = Ложь) Экспорт
	...
	Для Индекс = 2 По КоличествоЗаписей Цикл
		...
		СпособАутентификацииПользователя = Макет.Область(Индекс, 4).Текст;
		...
		Настройки.Вставить("СпособАутентификации", СпособАутентификацииПользователя);
		...
		ДоступныеПроекты = СтрРазделить(Макет.Область(Индекс, 8).Текст, ",", Ложь);
		Настройки.Вставить("Проекты", ДоступныеПроекты);
		...
		ВремяЖизниСессииСтрока = Макет.Область(Индекс, 13).Текст;
		ВремяЖизниСессии = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(ВремяЖизниСессииСтрока);
		
		Если Не ЗначениеЗаполнено(ВремяЖизниСессии) Тогда
			ВремяЖизниСессии = 0;
		КонецЕсли;
		
		Настройки.Вставить("ВремяЖизниСессии", ВремяЖизниСессии);
		
		ПериодЗапросаВыпискиИзМакета = Макет.Область(Индекс, 14).Текст;
		Если ОбщегоНазначенияКлиентСервер.ЭтоЧисло(ПериодЗапросаВыпискиИзМакета) Тогда
			ПериодЗапросаВыписки = Число(ПериодЗапросаВыпискиИзМакета);
		Иначе
			ПериодЗапросаВыписки = 14;
		КонецЕсли;
		Настройки.Вставить("ПериодЗапросаВыписки", ПериодЗапросаВыписки);
...
// Получает параметры письма для запроса автоматической выписки
//
// Параметры:
...
// Возвращаемое значение:
//  Структура - 
//   * ОписаниеНастройки - Структура:
//      ** Ссылка - СправочникСсылка.НастройкиОбменСБанками
//      ** ИспользоватьЭП - Булево
//      ** ПрограммаБанка - ПеречислениеСсылка.ПрограммыБанка
//   * ТекстПисьма - Строка - текст письма на запрос о подключении автоматической выписки
//
Функция ПараметрыПисьмаПоАвтоматическойВыписке(Организация, Банк) Экспорт
	Параметры = Новый Структура;
	ОписаниеНастройки = ОбменСБанкамиСлужебный.НастройкаОбменаСОтправкойПисем(Организация, Банк);
	Параметры.Вставить("ОписаниеНастройки", ОписаниеНастройки);
	ТекстПисьма = ТекстПисьмаЗапросОПодключенииАвтоматическойВыписки(Организация, Банк);
	Параметры.Вставить("ТекстПисьма", ТекстПисьма);
	
	Возврат Параметры;
КонецФункции

// Проверяет возможность отправки сообщения через почту
//
// Параметры:
//   Организация - ОпределяемыйТип.Организация - организация, указанная в настройке;
//   Банк - ОпределяемыйТип.БанкОбменСБанками - банк, указанный в настройке;
//
// Возвращаемое значение:
//  Булево - Истина отправка почтой доступна, Ложь отправка почтой не доступна
//
Функция ДоступнаОтправкаСообщенийПочтой(Организация, Банк) Экспорт
	
	Результат = Ложь;
	
	УстановитьПривилегированныйРежим(Истина);
	ЕстьПоддержкаПисем = ПолучитьФункциональнуюОпцию("ПисьмаОбменСБанками");
	УстановитьПривилегированныйРежим(Ложь);
	
	ОписаниеНастройки = ОбменСБанкамиСлужебный.НастройкаОбменаСОтправкойПисем(Организация, Банк);
	Если ЕстьПоддержкаПисем И ЗначениеЗаполнено(ОписаниеНастройки.Ссылка) Тогда
		Результат = Истина;
	Иначе
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
			МодульРаботаСПочтовымиСообщениямиКлиент = ОбщегоНазначения.ОбщийМодуль(
				"РаботаСПочтовымиСообщениямиВызовСервера");
			Если МодульРаботаСПочтовымиСообщениямиКлиент.ЕстьДоступныеУчетныеЗаписиДляОтправки() Тогда
				Результат = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
// Получает текст письма на запрос о подключении автоматической выписки
//
// Параметры:
//   Организация - ОпределяемыйТип.Организация - организация, указанная в настройке;
//   Банк - ОпределяемыйТип.БанкОбменСБанками - банк, указанный в настройке;
//
// Возвращаемое значение:
//  Строка  - текст письма на запрос о подключении автоматической выписки
//
Функция ТекстПисьмаЗапросОПодключенииАвтоматическойВыписки(Организация, Банк) Экспорт
	
	МассивСтрок = Новый Массив;
	ЗапросОПодключенииАвтоматическойВыписки = УправлениеПечатью.МакетПечатнойФормы(
		"Справочник.НастройкиОбменСБанками.ЗапросОПодключенииАвтоматическойВыписки");
	ТекстПисьма = ЗапросОПодключенииАвтоматическойВыписки.ПолучитьТекст();
	МассивСтрок.Добавить(ТекстПисьма);
	
	Подпись = ОбменСБанкамиСлужебный.НоваяПодпись();
	МассивСтрок.Добавить(Подпись);
	
	РеквизитыОрганизации = ОбщегоНазначенияБЭД.ДанныеЮрФизЛица(Организация); 
	МассивСтрок.Добавить(РеквизитыОрганизации.СокращенноеНаименование);
	МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'ИНН: %1'"), РеквизитыОрганизации.ИНН));
	МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'КПП: %1'"), РеквизитыОрганизации.КПП));
	
	Счета = ОбменСБанкамиСлужебный.НомераБанковскихСчетов(Организация, Банк);
	Если Счета.Количество() > 0 Тогда
		РеквизитыБанка = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Банк, "Код, Наименование");
		МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Номер счета: %1'"), Счета[0]));
		МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Банк: %1'"), РеквизитыБанка.Наименование));
		МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'БИК: %1'"), РеквизитыБанка.Код));
	КонецЕсли;
	
	ТекстПисьмаДляБанка = СтрСоединить(МассивСтрок, Символы.ПС);
	
	Возврат ТекстПисьмаДляБанка;
...
Функция ДанныеУстановленныхПодписейИСертификатов(СообщениеОбмена, ИмяМодуля) Экспорт
	...
	Для каждого ЭлементКоллекции Из УстановленныеПодписи Цикл
		...
		Сертификат = ЭлементКоллекции.Сертификат;
		Если ТипЗнч(Сертификат) = Тип("ХранилищеЗначения") Тогда
			Сертификат = Сертификат.Получить();
		КонецЕсли;
		СоответствиеВозврата.Вставить(СтрокаЭП, Сертификат);
	КонецЦикла;
...
Функция ПодписанныеДанныеBase64(СообщениеОбмена) Экспорт
	...
	МассивДокументы.Добавить(СообщениеОбмена);
	РеквизитыДокументов = РеквизитыЭлектронныхДокументов(МассивДокументы, Ложь);
	Если РеквизитыДокументов.Количество() <> 0
		И РеквизитыДокументов[0].ПрограммаБанка = Перечисления.ПрограммыБанка.SberAPI Тогда
		
		Возврат ПолучитьСтрокуИзДвоичныхДанных(ДанныеЭД);
...
//    ПрограммаБанка - ПеречислениеСсылка.ПрограммыБанка - вид программы банка.
//
// Возвращаемое значение:
//    СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования - созданный сертификат.
//
Функция СоздатьСертификатСбербанка(Знач ДвоичныеДанныеСертификата, Знач Организация, ПрограммаБанка) Экспорт
...
// Удаляет параметры указанной сессии установленного сеанса на сервере
//
// Параметры:
//  КлючСессии - СправочникСсылка.НастройкиОбменСБанками - ссылка на настройку обмена с банком
//
Процедура УдалитьСессиюНаСервереСбербанк(КлючСессии) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	СохраненныеСессии = Новый Соответствие(ПараметрыСеанса.СессииОбменСБанками);
	Если СохраненныеСессии.Получить(КлючСессии) <> Неопределено Тогда
		СохраненныеСессии.Удалить(КлючСессии);
		ПараметрыСеанса.СессииОбменСБанками = Новый ФиксированноеСоответствие(СохраненныеСессии);
	КонецЕсли;
...
Функция ПараметрыУстановленнойСессииСбербанк(Ключ) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	Возврат ОбменСБанкамиСлужебный.ПараметрыУстановленнойСессииСбербанк(Ключ);
...
// 	ЗаблокированныеКриптопрофили - Массив Из Строка, Неопределено - содержит список заблокированных профилей.
...
//		* СертификатыКлиента - Массив из СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования - 
//				найденные или созданные сертификаты клиента
//	Или Неопределено, если не удалось прочитать.
//
Функция ПрочитатьСертификатыСбербанка(
	...
	ПолученныеСертификаты = ЗначениеСвойстваXDTO(ЭД, "Certificates.Certificate");
	
	Если ПолученныеСертификаты = Неопределено Тогда
		// Не удалось получить сертификаты.
		Возврат Неопределено;
	
	ИначеЕсли ТипЗнч(ПолученныеСертификаты) = Тип("ОбъектXDTO") Тогда
		// Если сертификат только один, то из XML он будет прочитан как одиночный ОбъектXDTO, 
		// а не СписокXDTO, поэтому поместим его в массив для единообразного обхода в цикле.
		ПолученныеСертификаты = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПолученныеСертификаты);
	КонецЕсли;
	
	Для Каждого ДанныеСертификата Из ПолученныеСертификаты Цикл
		...
		// Могут быть записи с пустыми сертификатами, такие пропускаем.
		Если ДанныеСертификата.Последовательность().Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		ИначеЕсли client = "1" И active = "1" Тогда
			...
			Если СертификатКриптографии.ДатаОкончания < ТекущаяДатаСеанса() Тогда
				Продолжить;
			КонецЕсли;
			НовыйСертификат = СоздатьСертификатСбербанка(
				ДвоичныеДанныеСертификата,
				Организация,
				Перечисления.ПрограммыБанка.СбербанкОнлайн);
...
// Проверяет текущее состояние обмена с банками и формирует данные для уведомлений.
// 
// Возвращаемое значение:
//  Структура: см. ОбменСБанкамиСлужебный.НовыеСведенияОСостоянииИДанныхУведомленийОбменаСБанками.
//
Функция ТекущееСостояниеИДанныеУведомленийОбменаСБанками() Экспорт
	
	Результат = ОбменСБанкамиСлужебный.НовыеСведенияОСостоянииИДанныхУведомленийОбменаСБанками();
	
	СведенияОСостоянии = ОбменСБанкамиСлужебный.ТекущееСостояниеОбменаСБанками();
	
	Результат.ЕстьПравоЧтенияДанных = СведенияОСостоянии.ЕстьПравоЧтенияДанных;
	Результат.ИспользуетсяОбменСБанками = СведенияОСостоянии.ИспользуетсяОбменСБанками;
	
	Если Не Результат.ЕстьПравоЧтенияДанных Тогда
		Возврат Результат;
	КонецЕсли;
	
	Если Не Результат.ИспользуетсяОбменСБанками Тогда
		Возврат Результат;
	КонецЕсли;
	
	ТекущееСостояниеПисем = Результат.ТекущееСостояниеПисем;
	
	Если СведенияОСостоянии.ПисьмаОбменСБанкамиЕстьНастройка Тогда
		
		ТекущееСостояниеПисем.ЕстьНастройка = Истина;
		ТекущееСостояниеПисем.ДанныеОНепрочитанныхПисьмах = ОбменСБанкамиСлужебный.ДанныеОНепрочитанныхПисьмах();
		ТекущееСостояниеПисем.КоличествоНеотправленныхПисем = ОбменСБанкамиСлужебный.КоличествоНеотправленныхПисем();
		
	КонецЕсли;
	
	ТекущееСостояниеПоддержкиСессии = Результат.ТекущееСостояниеПоддержкиСессии;
	
	Если СведенияОСостоянии.ПоддержкаСессииОбменСБанкамиЕстьНастройка Тогда
		
		ТекущееСостояниеПоддержкиСессии.ЕстьНастройка = Истина;
		ТекущийПользователь = Пользователи.ТекущийПользователь();
		ДанныеУведомленийДляПользователя = ОбменСБанкамиСлужебный.ДанныеДляУведомленийОбОтклоненныхДокументах(ТекущийПользователь);
		ТекущееСостояниеПоддержкиСессии.ДанныеДляУведомлений = ДанныеУведомленийДляПользователя;
...
// Проверяет текущее состояние обмена с банками и формирует данные для уведомлений о непрочитанных письмах с банками.
// 
// Возвращаемое значение:
//  Структура: см. ОбменСБанкамиСлужебный.НовыеСведенияОСостоянииИДанныхОНепрочитанныхПисьмах.
//
Функция ТекущееСостояниеОбменаСБанкамиИДанныеОНепрочитанныхПисьмах() Экспорт
	
	Результат = ОбменСБанкамиСлужебный.НовыеСведенияОСостоянииИДанныхОНепрочитанныхПисьмах();
...
// Проверяет, поддерживает ли конфигурация запись событий уровня "Информация" в журнал регистрации.
// 
// Возвращаемое значение:
//  Булево - Истина, если запись событий "Информация " в журнале регистрации разрешена,
//           иначе возвращает Ложь.
Функция ЖурналРегистрацииПоддерживаетСобытияИнформация() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	ИспользованиеЖурналаРегистрации = ПолучитьИспользованиеЖурналаРегистрации();
	Возврат ИспользованиеЖурналаРегистрации.Найти(УровеньЖурналаРегистрации.Информация) <> Неопределено;
...
// Запускает фоновое задание для ожидания завершения обработки документов банком, отменяя предыдущее при необходимости.
//
// Параметры:
//  ДанныеСессии - Структура - данные сессии обмена с банком, 
//                             см. ОбменСБанкамиСлужебныйКлиентСервер.НовыеДанныеСессииОбменаСБанком
//  ИдентификаторЗаданияОжидания - Строка - идентификатор предыдущего фонового задания для отмены
//
// Возвращаемое значение:
//  Структура - параметры запущенной длительной операции, см. ДлительныеОперации.ВыполнитьФункцию, или Неопределено

Функция ЗапускЗаданияОжиданиеЗавершенияОбработкиДокументовБанком(
	Знач ДанныеСессии, 
	ИдентификаторЗаданияОжидания = Неопределено) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	НастройкаОбмена = ДанныеСессии.НастройкаОбмена;
	ВремяЖизниСессии = ДанныеСессии.ВремяЖизниСессии;
	ТолькоОбменДепозитами = ДанныеСессии.ТолькоОбменДепозитами;
	
	Если ЗначениеЗаполнено(ИдентификаторЗаданияОжидания) Тогда
		ДлительныеОперации.ОтменитьВыполнениеЗадания(ИдентификаторЗаданияОжидания);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ДанныеСессии.ИмяЗадания) Тогда
		
		Банк = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НастройкаОбмена, "Банк");
		ДанныеСессии.ИмяЗадания = СтрШаблон(НСтр("ru = '1С:ДиректБанк. Проверка статусов документов в банке: %1'"),
			Банк);
		
	КонецЕсли;
	
	// Если ДанныеСессии.ВремяЖизниСессии = 0, для банка не доступна поддержка сессии обмена для
	// ожидания завершения обработки документов, в этом случае нужно проверить, доступен ли обмен депозитами
	// для настройки обмена, т.к. если используется обмен депозитами, то поддерживаем сессию
	// обмена открытой до завершения обработки отправленной заявки на депозит.
	//
	НужноЗапускатьЗадание = Ложь;
	Если ВремяЖизниСессии > 0 Или ТолькоОбменДепозитами Тогда
		НужноЗапускатьЗадание = ЕстьДокументыВСостоянииОбработки(НастройкаОбмена, ТолькоОбменДепозитами);
	КонецЕсли;
	
	Если Не НужноЗапускатьЗадание Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = ДанныеСессии.ИмяЗадания;
	ПараметрыВыполнения.ЗапуститьВФоне = Истина;
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, 
		"ОбменСБанкамиСлужебный.ОбновитьДанныеОткрытойСессииОбмена", ДанныеСессии);
	
	Возврат ДлительнаяОперация;
	
КонецФункции

// Проверяет возможность запуска ожидания обработки документов банком.
// Возвращает данные для проверки состояния электронных документов отправленных в банк.
//
// Параметры:
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - настройка обмена с банками.
// 
// Возвращаемое значение:
//  Булево - Если Истина, есть документы, по которым не получен статус завершения обработки.
//
Функция ВозможноЗапуститьОжиданиеОбработкиОтправленныхДокументов(НастройкаОбмена) Экспорт
	
	Результат = Ложь;
	
	ПрограммаАсинхронныйОбмен = Перечисления.ПрограммыБанка.АсинхронныйОбмен;
	
	РеквизитыНастройкиОбмена = Новый Структура("ПрограммаБанка, АвтоматическоеПолучениеВыписки, Банк");
	ПолучитьЗначенияРеквизитовНастройкиОбмена(НастройкаОбмена, РеквизитыНастройкиОбмена);
	ВремяЖизниСессии = ОбменСБанкамиСлужебный.ВремяЖизниСессииОбменаСБанком(РеквизитыНастройкиОбмена.Банк);
	
	// Если ВремяЖизниСессии = 0, для банка не доступна поддержка сессии обмена для
	// ожидания завершения обработки документов
	//
	ПоддерживаетсяОжиданиеОбработкиДокументов = ЗначениеЗаполнено(ВремяЖизниСессии);
	
	Если РеквизитыНастройкиОбмена.АвтоматическоеПолучениеВыписки 
		Или Не РеквизитыНастройкиОбмена.ПрограммаБанка = ПрограммаАсинхронныйОбмен Тогда
		Возврат Результат;
	КонецЕсли;
	
	// Если для банка не доступна поддержка сессии обмена для ожидания завершения обработки документов,
	// тогда проверяем доступность обмена по депозитам.
	// В случае когда поддержка сессии обмена доступна, нет необходимости проверять доступность депозитов, т.к.
	// ВидыЭДОбменСБанками.ЗаявкаНаОткрытиеДепозита по умолчанию включена в состав видов документов доступных
	// для ожидания завершения обработки.
	// Если используется обмен депозитами, то поддерживаем сессию обмена открытой до завершения обработки
	// отправленной заявки на депозит.
	//
	ЕстьПоддержкаДепозитов = Ложь;
	Если Не ПоддерживаетсяОжиданиеОбработкиДокументов Тогда
		ЕстьПоддержкаДепозитов = ОбменСБанкамиДепозитыСлужебный.НастройкаПоддерживаетОбменДепозитами(НастройкаОбмена);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ВремяЖизниСессии) 
		Или ЕстьПоддержкаДепозитов Тогда
		Результат = ЕстьДокументыВСостоянииОбработки(НастройкаОбмена, ЕстьПоддержкаДепозитов);
	КонецЕсли;
...
// Устанавливает признак доставки уведомления в регистре сведений СостоянияОбменСБанками. 
//
// Параметры:
//  МассивСсылокНаОбъекты - Массив из ОпределяемыйТип.ВладелецОбменСБанками - ссылки на объект по которым
//    отправленны уведомления.
//  
Процедура ОтметитьОтправленныеУведомленияОбОтклоненныхДокументах(МассивСсылокНаобъекты) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	МенеджерЗаписи = РегистрыСведений.СостоянияОбменСБанками.СоздатьМенеджерЗаписи();
	
	Для Каждого ТекЭлемент Из МассивСсылокНаобъекты Цикл
		
		МенеджерЗаписи.СсылкаНаОбъект = ТекЭлемент;
		МенеджерЗаписи.Прочитать();
		
		Если МенеджерЗаписи.Выбран() Тогда
			
			МенеджерЗаписи.ТребуетсяУведомлениеПользователю = Ложь;
			МенеджерЗаписи.Записать();
...
//
// Параметры:
//  МассивОбъектов - Массив из ОпределяемыйТип.ВладелецОбменСБанками - содержит ссылки на документы информационной базы.
// 
// Возвращаемое значение:
//  Булево - Истина, если хотя бы для одного из документов требуется электронная подпись
//            по настройкам обмена с банками.
//
Функция ТребуетсяКриптографияДляОбработкиДокументов(Знач МассивОбъектов)
	...
	ТекстЗапросаНастроекКриптографии = СтрЗаменить(ТекстЗапросаНастроекКриптографии(), 
			"ТЧ_НастройкиОбмена.Ссылка.Банк.Код", 
			"ТЧ_НастройкиОбмена.Ссылка.Банк." + ИмяПрикладногоРеквизитаБИК);
	
	ТаблицаКэшаПараметровЭД = Новый ТаблицаЗначений;
	ТаблицаКэшаПараметровЭД.Колонки.Добавить("Организация");
	ТаблицаКэшаПараметровЭД.Колонки.Добавить("Банк");
	ТаблицаКэшаПараметровЭД.Колонки.Добавить("ВидЭД");
	...
	Для Каждого СсылкаНаДокумент Из МассивОбъектов Цикл
		...
		Если Не ЗначениеЗаполнено(ПараметрыЭД.Банк) Тогда
			Продолжить;
		КонецЕсли;
		
		ОтборКэша = Новый Структура("Организация, Банк, ВидЭД", 
			ПараметрыЭД.Организация, ПараметрыЭД.Банк, ПараметрыЭД.ВидЭД);
		
		НайденныеСтрокиКэша = ТаблицаКэшаПараметровЭД.НайтиСтроки(ОтборКэша);
		...
		НоваяСтрокаКэша = ТаблицаКэшаПараметровЭД.Добавить();
		НоваяСтрокаКэша.Организация = ПараметрыЭД.Организация;
		НоваяСтрокаКэша.Банк = ПараметрыЭД.Банк;
		НоваяСтрокаКэша.ВидЭД = ПараметрыЭД.ВидЭД;
...
Функция ПараметрыОбменаПоРеквизитам(РеквизитыОбмена)
	...
	РеквизитыНастройкиОбмена = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НастройкаОбмена, "ПрограммаБанка,
		|ИмяВнешнегоМодуля, ИспользуетсяКриптография, АутентификацияПоСертификату, ДополнительнаяОбработка, Банк");
...
// Возвращает значение параметра ТребуетсяSMSАвторизация для настройки обмена
//
// Параметры:
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - настройка обмена с банком.
//
// Возвращаемое значение:
//  ПеречислениеСсылка.SMSАвторизацияОбменСБанками - значение параметра ТребуетсяSMSАвторизация;
//
// См. ОбменСБанкамиСлужебный.ТребуетсяSMSАвторизация()
//
Функция ТребуетсяSMSАвторизация(НастройкаОбмена) Экспорт
	
	Возврат ОбменСБанкамиСлужебный.ТребуетсяSMSАвторизация(НастройкаОбмена);
	
КонецФункции

// Устанавливает значение параметра ТребуетсяSMSАвторизация для настройки обмена
//
// Параметры:
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - настройка обмена с банком.
//  ТребуетсяSMSАвторизация - ПеречислениеСсылка.SMSАвторизацияОбменСБанками - значение параметра ТребуетсяSMSАвторизация;
//
Процедура УстановитьТребуетсяSMSАвторизация(НастройкаОбмена, ТребуетсяSMSАвторизация) Экспорт
	
	ЗначенияПараметровОбмена = Новый Структура;
	ЗначенияПараметровОбмена.Вставить("ТребуетсяSMSАвторизация", ТребуетсяSMSАвторизация);
	РегистрыСведений.ПараметрыОбменСБанками.СохранитьЗначенияПараметровОбмена(
		НастройкаОбмена,
		ЗначенияПараметровОбмена);
...
Функция МаркерБанкаПоСертификату(АдресСервера, ИдентификаторОрганизации, ДвоичныеДанныеСертификата, ВерсияФормата = Неопределено, ПробнаяОперация = Ложь, НастройкаОбмена = Неопределено) Экспорт
	...
	ДанныеПоТипу = ОбменСБанкамиСлужебный.ПрочитатьXDTOИзДвоичныхДанныхПоТипу(Результат.Тело, "ResultBank");
	Если ДанныеПоТипу.ЕстьОшибка Тогда
		ТекстОшибки = ОбменСБанкамиСлужебный.ТекстОшибкиЧтенияДвоичныхДанныхИзБанка(НастройкаОбмена, Результат.Тело,
			ДанныеПоТипу.ПредставлениеОшибки);
...
Функция МаркерБанкаПоSMS(Знач АдресСервера, Знач ИдентификаторОрганизации, Знач ИдентификаторСессии, Знач ОдноразовыйПароль, Знач ВерсияAPI, Знач НастройкаОбмена = Неопределено) Экспорт
	...
	Если ДанныеПоТипу.ЕстьОшибка Тогда
		...
		ОписаниеОперации = НСтр("ru = 'Аутентификация на сервере банка'");
		ОбработатьОшибку(ОписаниеОперации, ДанныеПоТипу.ПредставлениеОшибки, ТекстСообщения);
		Возврат Неопределено;
	Иначе
		ResultBank = ДанныеПоТипу.Значение;
	КонецЕсли;
...
//   * ИдентификаторСессии - Строка - идентификатор установленной сессии.
//   * НеверныеДанныеАутентификации - Булево - Истина, если переданные неверные данные аутентификации, т.е.
//      HTTP код ответа сервера "401" или в ответе банка ResultBank.Error.Code = "1201".
...
Функция БазоваяАутентификация(Знач АдресСервера, Знач ИдентификаторОрганизации, Знач ДанныеАутентификации, Знач ВерсияAPI, Знач НастройкаОбмена = Неопределено) Экспорт
	...
	Если Не Результат.Статус Тогда
		Если ЗначениеЗаполнено(Результат.КодСостояния) Тогда
			...
			СтруктураВозврата.НеверныеДанныеАутентификации = Результат.КодСостояния = 401;
		Иначе
			Шаблон = НСтр("ru = 'Ошибка отправки данных аутентификации на сервер банка.
								|Адрес ресурса: %1
								|Ошибка: %2'");
	...
	Если НЕ ResultBank.Success = Неопределено Тогда
		Если НЕ ResultBank.Success.LogonResponse = Неопределено Тогда
			...
			Иначе
				ТребуетсяSMSАвторизация = Неопределено;
			КонецЕсли;
			
			// Установим признак ТребуетсяSMSАвторизация.
			ЗначенияПараметровОбмена = Новый Структура;
			Если (СтруктураВозврата.ТребуетсяSMSАвторизация
				ИЛИ ТребуетсяSMSАвторизация = Перечисления.SMSАвторизацияОбменСБанками.Требуется)
				И ЗначениеЗаполнено(НастройкаОбмена) Тогда
				
				ЗначенияПараметровОбмена.Вставить(
					"ТребуетсяSMSАвторизация",
					Перечисления.SMSАвторизацияОбменСБанками.Требуется);
				РегистрыСведений.ПараметрыОбменСБанками.СохранитьЗначенияПараметровОбмена(
					НастройкаОбмена,
					ЗначенияПараметровОбмена);
				
			ИначеЕсли ТребуетсяSMSАвторизация = Перечисления.SMSАвторизацияОбменСБанками.НеТребуется
				И ЗначениеЗаполнено(НастройкаОбмена) Тогда
				
				ЗначенияПараметровОбмена.Вставить(
					"ТребуетсяSMSАвторизация",
					Перечисления.SMSАвторизацияОбменСБанками.НеТребуется);
				РегистрыСведений.ПараметрыОбменСБанками.СохранитьЗначенияПараметровОбмена(
					НастройкаОбмена,
					ЗначенияПараметровОбмена);
				
			ИначеЕсли Не ЗначениеЗаполнено(ТребуетсяSMSАвторизация)
				И ЗначениеЗаполнено(НастройкаОбмена) Тогда
				
				ЗначенияПараметровОбмена.Вставить(
					"ТребуетсяSMSАвторизация",
					Перечисления.SMSАвторизацияОбменСБанками.ПустаяСсылка());
				РегистрыСведений.ПараметрыОбменСБанками.СохранитьЗначенияПараметровОбмена(
					НастройкаОбмена,
					ЗначенияПараметровОбмена);
...
Функция ОтносительнаяДатаСинхронизации(Знач ДатаСинхронизации) Экспорт
	...
	Иначе
		Возврат ОбменДаннымиСервер.ОтносительнаяДатаСинхронизации(ДатаСинхронизации);
	КонецЕсли;
...
Функция ЗапросыВыписок(
	...
	Если МассивБанковскихСчетов.Количество() = 0 Тогда
		...
		ШаблонСообщения = НСтр("ru = 'У %1 нет доступных счетов в %2 для получения выписки.  
			|Сначала необходимо добавить счета в справочнике банковских счетов.'");
		ТекстСообщения = СтрШаблон(
			ШаблонСообщения,
			Строка(РеквизитыНастройкиОбмена.Организация),
			Строка(РеквизитыНастройкиОбмена.Банк));
...
Функция ПодключитьДополнительнуюОбработку(Знач ДополнительнаяОбработка, ИмяОбъекта = Неопределено) Экспорт
	...
	ИмяОбъекта = ДополнительныеОтчетыИОбработки.ПодключитьВнешнююОбработку(ДополнительнаяОбработка);
...
Функция ДанныеПакетаXMLСтрокойСбербанк(ЭД, НастройкаОбмена, СообщениеОбмена, ТипДокумента, ФродПараметры, ТекстОшибки, Логин = Неопределено)
	...
	Для Каждого ЭП Из МассивПодписей Цикл
		Подпись = ЭП.Подпись;
		ДвоичныеДанныеСертификата = ЭП.Сертификат;
...
Процедура СохранитьВыпискуСбербанка(НастройкаОбмена, ВыпискаXDTO, МассивВыписок, СоздаватьОперацииВыписки)
	...
	Если ДанныеВыписок.Количество() > 0 Тогда
...
// Получает реквизиты настройки обмена с банком.
//
// Параметры:
...
//
Функция РеквизитыНастройкиОбмена(Знач Ссылка) Экспорт
...
// См. ОбменСБанкамиСлужебный.ДанныеАвторизацииНастройкаОбмена
//
Функция ДанныеАвторизацииНастройкаОбмена(НастройкаОбмена) Экспорт
	
	Возврат ОбменСБанкамиСлужебный.ДанныеАвторизацииНастройкаОбмена(НастройкаОбмена);
	
КонецФункции

// См. ОбменСБанкамиСлужебный.СохранитьДанныеАвторизацииОбменСБанками
//
Процедура СохранитьДанныеАвторизацииОбменСБанками(НастройкаОбмена,
												  ДанныеАвторизации = Неопределено,
												  ПарольАвтовыписка = Неопределено,
												  ДанныеАвторизацииSberAPI = Неопределено) Экспорт
	
	ОбменСБанкамиСлужебный.СохранитьДанныеАвторизацииОбменСБанками(НастройкаОбмена, ДанныеАвторизации, ПарольАвтовыписка, ДанныеАвторизацииSberAPI);
...
//   Структура - см. ДлительныеОперации.ВыполнитьВФоне.
//
Функция ЗапускЗаданияПолученияИзвещенийОСостоянийЭД(
	Знач ПараметрыПроцедуры,
	Знач УникальныйИдентификатор) Экспорт
...
//    из которого нужно выбрать только те, которые нужно будет подписать по SMS;  
//
// Возвращаемое значение:
...
//    из которого нужно выбрать связанные сообщения обмена и только те, которые нужно будет подписать по SMS;
//
// Возвращаемое значение:
//   Массив из ДокументСсылка.СообщениеОбменСБанками - массив сообщений, формируемый процедурой
//
...
// Проверяет наличие документов в состоянии обработки по настройке обмена.
//
// Параметры:
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - настройка обмена с банками.
//  ТолькоОбменДепозитами - Булево - значение по умолчанию Ложь. Если Истина, поддержка сессии доступна только для обмена депозитами.
// 
// Возвращаемое значение:
//  Булево - Если Истина, есть документы по которым не получен статус завершения обработки. 
//
Функция ЕстьДокументыВСостоянииОбработки(НастройкаОбмена, ТолькоОбменДепозитами = Ложь)
	
	Результат = Ложь;
	
	ПериодДействияВСекундах = ОбменСБанкамиСлужебный.ПериодДействияБанковскогоДокументаВСекундах();
	ДатаНачала = НачалоДня(ТекущаяДатаСеанса())- ПериодДействияВСекундах;
	ВидыДокументовДляОжидания = Неопределено;
	ВидЭДЗаявкаНаДепозит = Перечисления.ВидыЭДОбменСБанками.ЗаявкаНаОткрытиеДепозита;
	ЕстьЗаявкиНаДепозитВОбработке = Ложь;
	
	Если ТолькоОбменДепозитами Тогда
		ВидыДокументовДляОжидания = Новый Массив;
		ВидыДокументовДляОжидания.Добавить(ВидЭДЗаявкаНаДепозит);
	Иначе
		ВидыДокументовДляОжидания = ОбменСБанкамиСлужебный.ВидыДокументовДляОжиданияЗавершенияОбработки();
	КонецЕсли;
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	
	// Нужно проверить, что у исходящих документов из ВидыДокументовДляОжидания установлены правильные настройки подтверждения.
	//
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("НастройкаОбмена", НастройкаОбмена);
	Запрос.УстановитьПараметр("ВидыДокументовДляОжиданияЗавершенияОбработки", ВидыДокументовДляОжидания);
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СпособыПодтвержденияДокументовОбменСБанками.ЭлектроннаяПодпись)
	|		КОГДА НастройкиОбменСБанкамиИсходящиеДокументы.СпособПодтвержденияДокумента = ЗНАЧЕНИЕ(Перечисление.СпособыПодтвержденияДокументовОбменСБанками.ПустаяСсылка)
	|				И НастройкиОбменСБанкамиИсходящиеДокументы.ИспользоватьSMS
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СпособыПодтвержденияДокументовОбменСБанками.SMS)
	|		ИНАЧЕ НастройкиОбменСБанкамиИсходящиеДокументы.СпособПодтвержденияДокумента
	|	КОНЕЦ КАК СпособПодтверждения
	|ПОМЕСТИТЬ ВТ_НастройкиПодтверждения
	|ИЗ
	|	Справочник.НастройкиОбменСБанками.ИсходящиеДокументы КАК НастройкиОбменСБанкамиИсходящиеДокументы
	|ГДЕ
	|	НастройкиОбменСБанкамиИсходящиеДокументы.Ссылка = &НастройкаОбмена
	|	И НастройкиОбменСБанкамиИсходящиеДокументы.Формировать
	|	И НЕ НастройкиОбменСБанкамиИсходящиеДокументы.ПодтвердитьВБанке
	|	И НастройкиОбменСБанкамиИсходящиеДокументы.СпособПодтвержденияДокумента <> ЗНАЧЕНИЕ(Перечисление.СпособыПодтвержденияДокументовОбменСБанками.ВЛичномКабинетеБанка)
	|	И НастройкиОбменСБанкамиИсходящиеДокументы.ИсходящийДокумент В(&ВидыДокументовДляОжиданияЗавершенияОбработки)
	|	И (НастройкиОбменСБанкамиИсходящиеДокументы.ИспользоватьSMS
	|			ИЛИ НастройкиОбменСБанкамиИсходящиеДокументы.ИспользоватьЭП
	|			ИЛИ НастройкиОбменСБанкамиИсходящиеДокументы.СпособПодтвержденияДокумента <> ЗНАЧЕНИЕ(Перечисление.СпособыПодтвержденияДокументовОбменСБанками.ПустаяСсылка))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_НастройкиПодтверждения.ВидЭД КАК ВидЭД,
	|	ВТ_НастройкиПодтверждения.СпособПодтверждения КАК СпособПодтверждения
	|ИЗ
	|	ВТ_НастройкиПодтверждения КАК ВТ_НастройкиПодтверждения";
	
	РезультатЗапросаНастройкиПодтверждения = Запрос.Выполнить();
	
	Если РезультатЗапросаНастройкиПодтверждения.Пустой() Тогда
		Возврат Результат;
	КонецЕсли;
	
	ВидыЭДНастройкиПодтверждения = РезультатЗапросаНастройкиПодтверждения.Выгрузить().ВыгрузитьКолонку("ВидЭД");
	
	// Пока по заявке на депозит не получено извещение о состоянии депозита, заявка на депозит не считается исполненной,
	// даже если состояние ЭД заявки на депозит = "Исполнено"
	//
	ИндексЗаявкиНаДепозит = ВидыЭДНастройкиПодтверждения.Найти(ВидЭДЗаявкаНаДепозит);
	Если Не ИндексЗаявкиНаДепозит = Неопределено Тогда
		Банк = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НастройкаОбмена,"Банк");
		
		// Обычно банк обрабатывает заявки на депозит в течение 15 секунд, 
		// поэтому для заявок нужен другой период проверки.
		//
		ДлинаСутокВСекундах = 86400;
		ДатаНачалаОтбора = НачалоДня(ТекущаяДатаСеанса()) - ДлинаСутокВСекундах;
		ЕстьЗаявкиНаДепозитВОбработке = ОбменСБанкамиДепозитыСлужебный.ЕстьЗаявкиНаДепозитОжидающиеИсполнения(
			Банк, ДатаНачалаОтбора);
		
		Если Не ТолькоОбменДепозитами Тогда
			ВидыЭДНастройкиПодтверждения.Удалить(ИндексЗаявкиНаДепозит);
		Иначе
			Возврат ЕстьЗаявкиНаДепозитВОбработке;
		КонецЕсли;
		
	КонецЕсли;
	
	СтатусыДокументоВОбработке = Новый Массив;
	СтатусыДокументоВОбработке.Добавить(Перечисления.СтатусыОбменСБанками.Отправлен);
	СтатусыДокументоВОбработке.Добавить(Перечисления.СтатусыОбменСБанками.Доставлен);
	СтатусыДокументоВОбработке.Добавить(Перечисления.СтатусыОбменСБанками.Принят);
	
	Запрос.УстановитьПараметр("ВидыЭДНастройкиПодтверждения", ВидыЭДНастройкиПодтверждения);
	Запрос.УстановитьПараметр("СтатусыОбработкиБанком", СтатусыДокументоВОбработке);
	Запрос.УстановитьПараметр("Дата", ДатаНачала);
	
	// Считаем, что банк формирует извещения о состоянии ЭД при каждом изменении статуса
	// Проверяем по видам документов наличие статусов "В обработке".
	
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	|	СообщениеОбменСБанками.ВидЭД КАК ВидЭД,
	|	СообщениеОбменСБанками.Статус КАК Статус
	|ПОМЕСТИТЬ ВТ_СообщенияОбмена
	|ИЗ
	|	РегистрСведений.СостоянияОбменСБанками КАК СостоянияОбменСБанками
	...
	|			И (СообщениеОбменСБанками.ВидЭД В (&ВидыЭДНастройкиПодтверждения))
	|			И (СообщениеОбменСБанками.Дата >= &Дата)
	|			И (СообщениеОбменСБанками.Статус В (&СтатусыОбработкиБанком))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ВТ_СообщенияОбмена.ВидЭД КАК ВидЭД,
	|	ВТ_СообщенияОбмена.Статус КАК Статус
	|ИЗ
	|	ВТ_СообщенияОбмена КАК ВТ_СообщенияОбмена
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_НастройкиПодтверждения КАК ВТ_НастройкиПодтверждения
	|		ПО ВТ_СообщенияОбмена.ВидЭД = ВТ_НастройкиПодтверждения.ВидЭД
	|ГДЕ
	|	ВТ_НастройкиПодтверждения.СпособПодтверждения = ЗНАЧЕНИЕ(Перечисление.СпособыПодтвержденияДокументовОбменСБанками.SMS)
	|				И ВТ_СообщенияОбмена.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОбменСБанками.Принят)
	|			ИЛИ ВТ_НастройкиПодтверждения.СпособПодтверждения = ЗНАЧЕНИЕ(Перечисление.СпособыПодтвержденияДокументовОбменСБанками.ЭлектроннаяПодпись)
	|				И ВТ_СообщенияОбмена.Статус В (&СтатусыОбработкиБанком)";
	
	РезультатЗапроса = Запрос.Выполнить();
	
	Если Не РезультатЗапроса.Пустой() Или ЕстьЗаявкиНаДепозитВОбработке Тогда
		Результат = Истина;
	КонецЕсли;
