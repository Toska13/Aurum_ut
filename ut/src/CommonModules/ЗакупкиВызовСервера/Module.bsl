...
Функция ПолучитьКоличествоСоглашенийСПоставщиком(Знач Партнер,
	...
	ДанныеСтроки.Вставить("КоличествоСоглашений", 0);
	
	Действия = Новый Структура;
	Действия.Вставить("ЗаполнитьКоличествоСоглашенийПоставщика", ПараметрыДействия);
	...
	Возврат ДанныеСтроки.КоличествоСоглашений;
...
// Устарела. Следует использовать одноименный метод из модуля СопоставлениеНоменклатурыКонтрагентовКлиент. 
...
//
Функция ПроверитьДанныеПоОтборуИПартнеру(Договор, Знач ДанныеОтбора, ПартнерДокумента, СообщениеОбОшибке = "") Экспорт
	
	ДанныеОтбора = ОбщегоНазначения.СкопироватьРекурсивно(ДанныеОтбора); // Для модификации.
	ПроверкаПройдена = Истина;
	МассивПараметров = Новый Массив;
	
	#Область НестандартныеПараметры
	НаправлениеДеятельностиНазначения = Неопределено;
	Если ДанныеОтбора.Свойство("НазначениеОбеспечения") Тогда
		
		// Используем направление деятельности из назначения для отбора договоров с порядком расчетов "ПоДоговорамКонтрагентов":
		НазначениеОбеспечения = ДанныеОтбора.НазначениеОбеспечения;
		ДанныеОтбора.Удалить("НазначениеОбеспечения");
			
		Если ЗначениеЗаполнено(НазначениеОбеспечения)
			И Не ДанныеОтбора.Свойство("НаправлениеДеятельности") Тогда
			...
			НаправлениеДеятельностиНазначения = НаправленияДеятельности.Получить(НазначениеОбеспечения);
			
			МассивПараметров.Добавить("НаправлениеДеятельности");
			МассивПараметров.Добавить("ПорядокРасчетов");
			
		КонецЕсли;
		
	КонецЕсли;
	
	Если ДанныеОтбора.Свойство("Соглашение") Тогда
		
		Соглашение = ДанныеОтбора.Соглашение;
		ДанныеОтбора.Удалить("Соглашение");
		
		Если ЗначениеЗаполнено(Соглашение)
			И Не ДанныеОтбора.Свойство("ХозяйственнаяОперация") Тогда
			...
			ХозяйственнаяОперация = Справочники.СоглашенияСПоставщиками.ХозяйственнаяОперацияСоглашения(Соглашение);
			ДанныеОтбора.Вставить("ХозяйственнаяОперация", ХозяйственнаяОперация);
		КонецЕсли;
	...
	МассивПараметров = ОбщегоНазначенияКлиентСервер.СвернутьМассив(МассивПараметров);
	...
	Если ПроверкаПройдена
		И ЗначениеЗаполнено(НаправлениеДеятельностиНазначения)
		И ЗначениеЗаполнено(РеквизитыДоговора.НаправлениеДеятельности)
		И НаправлениеДеятельностиНазначения <> РеквизитыДоговора.НаправлениеДеятельности Тогда
		ПроверкаПройдена = Ложь;
	КонецЕсли;
