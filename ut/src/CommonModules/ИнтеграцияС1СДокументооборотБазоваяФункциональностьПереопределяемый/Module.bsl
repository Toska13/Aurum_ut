...
Процедура ПередДобавлениемКомандПечати(ИмяФормы, КомандыПечати, СтандартнаяОбработка) Экспорт
	...
	Если ИмяФормы = Метаданные.Справочники.ВидыРетроБонусовКлиентов.Формы.ФормаЭлемента.ПолноеИмя()
			...
			Или ИмяФормы = Метаданные.Справочники.ВидыРетроБонусовПоставщиков.Формы.ФормаЭлемента.ПолноеИмя()
			Или ИмяФормы = Метаданные.Справочники.ВидыРетроБонусовПоставщиков.Формы.ФормаСписка.ПолноеИмя()
			...
			Или ИмяФормы = Метаданные.Документы.ЗаказНаДоставку.Формы.ФормаДокумента.ПолноеИмя()
			Или ИмяФормы = Метаданные.Документы.ЗаказНаДоставку.Формы.ФормаСписка.ПолноеИмя()
			...
			Или ИмяФормы = Метаданные.Документы.Рейс.Формы.ФормаДокумента.ПолноеИмя()
			Или ИмяФормы = Метаданные.Документы.Рейс.Формы.ФормаСписка.ПолноеИмя()
			...
			Или ИмяФормы = Метаданные.Документы.СписаниеРетроБонусовПоставщика.Формы.ФормаДокумента.ПолноеИмя()
			Или ИмяФормы = Метаданные.Документы.СписаниеРетроБонусовПоставщика.Формы.ФормаСписка.ПолноеИмя()
			...
			Или ИмяФормы = Метаданные.Документы.УсловияРетроБонусовПоставщика.Формы.ФормаДокумента.ПолноеИмя()
			Или ИмяФормы = Метаданные.Документы.УсловияРетроБонусовПоставщика.Формы.ФормаСписка.ПолноеИмя()
...
// Конец СтандартныеПодсистемы_Печать
...
Процедура ДополнитьСоответствиеТипов(Таблица) Экспорт
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("3.0.16.33") Тогда
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДобавитьСтрокуСоответствияТипов(
			Таблица, "DMVATRateUniversal", Тип("СправочникСсылка.СтавкиНДС"));
	Иначе
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДобавитьСтрокуСоответствияТипов(
...
Процедура ПередЗаписьюОбъектаИС(ОбъектИС, Отказ,
	...
	Если (ТипЗнч(ОбъектИС) = Тип("ДокументОбъект.УсловияРетроБонусовКлиентов")
				Или ТипЗнч(ОбъектИС) = Тип("ДокументОбъект.УсловияРетроБонусовПоставщика"))
			И Константы.ИспользоватьСогласованиеЧерез1СДокументооборот.Получить() Тогда
...
Процедура ПриДобавленииПодсистемыВДеревоИнтегрируемыхОбъектов(Подсистема, Отказ) Экспорт
	...
	Если Подсистема.Имя = Метаданные.Подсистемы.Закупки.Подсистемы.РетроБонусыПоставщиков.Имя
			И Не ПолучитьФункциональнуюОпцию("ИспользоватьРетроБонусыПоставщиков") Тогда
		Отказ = Истина;
	КонецЕсли;
...
Процедура ПриЗаполненииСвойстваОбъектаXDTOПоЗначению(Значение, Свойства, ТипОбъекта, ИмяСвойства, ТипСвойства,
	...
	Если ТипСвойства = "DMVATRate" И ИмяСвойства = "СтавкаНДС" Тогда
		...
		Если ТипЗнч(Значение) = Тип("СправочникСсылка.СтавкиНДС") Тогда
...
Процедура ПриИзмененииСостоянияСогласования(ПредметСогласования, Состояние, ВызовИзФормыОбъекта, Установил,
	...
	Если ТипОбъекта <> Тип("ДокументСсылка.КоммерческоеПредложениеКлиенту")
			...
			И ТипОбъекта <> Тип("ДокументСсылка.УсловияРетроБонусовПоставщика")
			И ТипОбъекта <> Тип("ДокументСсылка.СписаниеРетроБонусовКлиента")
			И ТипОбъекта <> Тип("ДокументСсылка.СписаниеРетроБонусовПоставщика")
			И ТипОбъекта <> Тип("СправочникСсылка.ВидыРетроБонусовКлиентов")
			И ТипОбъекта <> Тип("СправочникСсылка.ВидыРетроБонусовПоставщиков")
	...
	Запрос = Новый Запрос(
		...
		|			ИЛИ (ВЫРАЗИТЬ(ПравилаЗаполненияРеквизитовИС.ИмяРеквизитаОбъектаДО КАК СТРОКА(7))) = ""status_"")
...
Процедура ПриОпределенииЗначенияРеквизитаПоОбъектуXDTO(Результат, ЗначениеДляЗаполнения, ТипРеквизитаОбъектаИС,
	...
	Если ТипРеквизитаОбъектаИС = Тип("ПеречислениеСсылка.КомпанияЧастноеЛицо") Тогда
		
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЭтоОбъектноеЗначение(Прокси, ЗначениеДляЗаполнения) Тогда
			
			Если ЗначениеДляЗаполнения.objectID.ID = "ЮрЛицо" Тогда
				Результат = Перечисления.КомпанияЧастноеЛицо.Компания;
				
			ИначеЕсли ЗначениеДляЗаполнения.objectID.ID = "ЮрЛицоНеРезидент" Тогда
				Результат = Перечисления.КомпанияЧастноеЛицо.Компания;
				
			ИначеЕсли ЗначениеДляЗаполнения.objectID.ID = "ФизЛицо" Тогда
				Результат = Перечисления.КомпанияЧастноеЛицо.ЧастноеЛицо;
	...
	Если ТипРеквизитаОбъектаИС = Тип("СправочникСсылка.СтавкиНДС") Тогда
		...
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(Прокси, ЗначениеДляЗаполнения, "DMVATRateUniversal") Тогда
			...
			Результат = РегистрыСведений.ОбъектыИнтегрированныеС1СДокументооборотом.СсылкаНаОбъектИСПоДаннымДокументооборота(
				ЗначениеДляЗаполнения.objectID.ID,
				ЗначениеДляЗаполнения.objectID.type,
				ТипРеквизитаОбъектаИС);
			
			Если Результат = Неопределено Тогда
				
				// Возможно, соответствия созданы синхронизацией данных и унаследовали идентификаторы.
				СсылкаПоИдентификатору = Справочники.СтавкиНДС.ПолучитьСсылку(
					Новый УникальныйИдентификатор(ЗначениеДляЗаполнения.objectID.ID));
				...
				Запрос = Новый Запрос(
					...
					|ГДЕ
					|	СтавкиНДС.Ссылка = &СсылкаПоИдентификатору");
				...
				Иначе
					...
					//++ Локализация
					Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ЗначениеДляЗаполнения, "predefinedName") Тогда
						ЗначениеПеречисления = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗначениеРеквизитаОбъектаИСПоЗначениюДляЗаполнения(
						...
						Если ЗначениеЗаполнено(ЗначениеПеречисления) Тогда
							Результат = УчетНДСЛокализация.СтавкаНДСПоПеречислению(ЗначениеПеречисления);
						КонецЕсли;
					КонецЕсли;
					
					Если Не ЗначениеЗаполнено(Результат)
							И ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ЗначениеДляЗаполнения, "externalObject")
							И Лев(ЗначениеДляЗаполнения.externalObject.type, 13) = "Перечисление." Тогда
						VATRate = НРег(ЗначениеДляЗаполнения.externalObject.ID);
						Если VATRate = "безндс" Тогда
							Результат = Справочники.СтавкиНДС.БезНДС;
						ИначеЕсли VATRate = "ндс0" Тогда
							Результат = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС0);
						ИначеЕсли VATRate = "ндс5" Тогда
							Если ЗначениеДляЗаполнения.calculatedRate = Истина Тогда
								Результат = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС5_105);
							Иначе
								Результат = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС5);
							КонецЕсли;
						ИначеЕсли VATRate = "ндс7" Тогда
							Если ЗначениеДляЗаполнения.calculatedRate = Истина Тогда
								Результат = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС7_107);
							Иначе
								Результат = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС7);
							КонецЕсли;
						ИначеЕсли VATRate = "ндс10" Тогда
							Если ЗначениеДляЗаполнения.calculatedRate = Истина Тогда
								Результат = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС10_110);
							Иначе
								Результат = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС10);
							КонецЕсли;
						ИначеЕсли VATRate = "ндс18" Тогда
							Если ЗначениеДляЗаполнения.calculatedRate = Истина Тогда
								Результат = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС18_118);
							Иначе
								Результат = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС18);
							КонецЕсли;
						ИначеЕсли VATRate = "ндс20" Тогда
							Если ЗначениеДляЗаполнения.calculatedRate = Истина Тогда
								Результат = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС20_120);
							Иначе
								Результат = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС20);
							КонецЕсли;
						ИначеЕсли VATRate = "ндс22" Тогда
							Если ЗначениеДляЗаполнения.calculatedRate = Истина Тогда
								Результат = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС22_122);
							Иначе
								Результат = УчетНДСЛокализация.СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС22);
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
					//-- Локализация
					...
					Если Не ЗначениеЗаполнено(Результат)
							И ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ЗначениеДляЗаполнения, "name")
							И ЗначениеДляЗаполнения.name <> "" Тогда
						
						// Продолжим поиск.
						Ставка = 0;
						Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ЗначениеДляЗаполнения, "rate") Тогда
							Ставка = ЗначениеДляЗаполнения.rate;
						КонецЕсли;
						РасчетнаяСтавка = Ложь;
						Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ЗначениеДляЗаполнения, "calculatedRate") Тогда
							РасчетнаяСтавка = ЗначениеДляЗаполнения.calculatedRate;
						КонецЕсли;
						Страна = Неопределено;
						Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ЗначениеДляЗаполнения, "country") Тогда
							Страна = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗначениеРеквизитаОбъектаИСПоЗначениюДляЗаполнения(
								Тип("СправочникСсылка.СтраныМира"),
								ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗначениеДляЗаполненияРеквизитаОбъектаИС(
									ЗначениеДляЗаполнения.country));
						КонецЕсли;
...
Процедура ПриОпределенииНеобходимостиВыводитьКомандуПрисоединенныхФайловДО(Форма, НужноВыводитьКоманду) Экспорт
	...
	Если Форма.ИмяФормы = "Справочник.ДоговорыКонтрагентов.Форма.ФормаЭлемента"
			Или Форма.ИмяФормы = "Документ.КоммерческоеПредложениеКлиенту.Форма.ФормаДокумента" Тогда
		
		Ссылка = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СсылкаНаИнтегрируемыйОбъектИзФормы(Форма);
		...
		Попытка
			...
			Если ВидФормы = "ФормаЭлемента" Или ВидФормы = "ФормаДокумента" Тогда
				НужноВыводитьКоманду = ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.НастроенаИнтеграцияДляОбъекта(Ссылка);
			Иначе
		...
		Если НужноВыводитьКоманду И Форма.ИмяФормы = "Справочник.ДоговорыКонтрагентов.Форма.ФормаЭлемента" Тогда
			Если Форма.ИспользоватьИнтеграциюДО2 Тогда
				...
				Форма.Элементы.СтраницаФайлы.Видимость =
					ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ЕстьХранимыеФайлыВИС(Ссылка);
...
Процедура ПриОпределенииСокращенногоНаименованияКонфигурации(СокращенноеНаименование) Экспорт
	...
	ИначеЕсли ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ") Тогда
		СокращенноеНаименование = НСтр("ru = '1С:ERP.УХ'");
...
Процедура ПриОпределенииТиповОбъектовПоддерживающихИнтеграцию(Типы) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьРетроБонусыПоставщиков") Тогда
		ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(Типы, Тип("СправочникСсылка.ВидыРетроБонусовПоставщиков"));
		ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(Типы, Тип("ДокументСсылка.НачислениеРетроБонусовПоставщика"));
		ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(Типы, Тип("ДокументСсылка.СписаниеРетроБонусовПоставщика"));
		ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(Типы, Тип("ДокументСсылка.УсловияРетроБонусовПоставщика"));
		ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(Типы, Тип("ДокументСсылка.АктПремииПоставщика"));
	КонецЕсли;
	
	Если НЕ ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25() Тогда
		
		ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(Типы, Тип("ДокументСсылка.ЗаказНаДоставку"));
		ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(Типы, Тип("ДокументСсылка.Рейс"));
...
Процедура ПриПроверкеСоответствияПравилаФункциональнымОпциям(Правило, ТипОбъектаДО, ТипОбъектаИС, Отказ,
	...
	МассивТиповРетроБонусовКлиентов = Новый Массив; // Массив Из Строка
	МассивТиповРетроБонусовКлиентов.Добавить("Справочник.ВидыРетроБонусовКлиентов");
	МассивТиповРетроБонусовКлиентов.Добавить("Документ.УсловияРетроБонусовКлиентов");
	МассивТиповРетроБонусовКлиентов.Добавить("Документ.НачислениеРетроБонусовКлиента");
	МассивТиповРетроБонусовКлиентов.Добавить("Документ.СписаниеРетроБонусовКлиента");
	МассивТиповРетроБонусовКлиентов.Добавить("Документ.АктПремииКлиенту");
	
	Если МассивТиповРетроБонусовКлиентов.Найти(ТипОбъектаИС) <> Неопределено
			И Не ПолучитьФункциональнуюОпцию("ИспользоватьРетроБонусыКлиентов") Тогда
		Отказ = Истина;
	КонецЕсли;
	
	МассивТиповРетроБонусовПоставщиков = Новый Массив; // Массив Из Строка
	МассивТиповРетроБонусовПоставщиков.Добавить("Справочник.ВидыРетроБонусовПоставщиков");
	МассивТиповРетроБонусовПоставщиков.Добавить("Документ.УсловияРетроБонусовПоставщика");
	МассивТиповРетроБонусовПоставщиков.Добавить("Документ.НачислениеРетроБонусовПоставщика");
	МассивТиповРетроБонусовПоставщиков.Добавить("Документ.СписаниеРетроБонусовПоставщика");
	МассивТиповРетроБонусовПоставщиков.Добавить("Документ.АктПремииПоставщика");
	
	Если МассивТиповРетроБонусовПоставщиков.Найти(ТипОбъектаИС) <> Неопределено
			И Не ПолучитьФункциональнуюОпцию("ИспользоватьРетроБонусыПоставщиков") Тогда
		Отказ = Истина;
	КонецЕсли;
	
	ТипыДоставка25 = Новый Массив; // Массив Из Строка
	ТипыДоставка25.Добавить("Документ.ЗаказНаДоставку");
	ТипыДоставка25.Добавить("Документ.Рейс");
	
	Если ТипыДоставка25.Найти(ТипОбъектаИС) <> Неопределено
...
Процедура ПриСозданииПоСсылке(ОбъектИС, ОбъектXDTO, ЗаполняемыйОбъектИС = Неопределено) Экспорт
	...
	Если ТипЗнч(ОбъектИС) = Тип("СправочникОбъект.Организации")
		...
		ОбъектИС.ЮрФизЛицо = ИнтеграцияС1СДокументооборотУП.ЮрФизЛицоИзОбъектаXDTO(ИсточникXDTO);
		
	КонецЕсли;
	
	Если ТипЗнч(ОбъектИС) = Тип("СправочникОбъект.Контрагенты") И Не ЗначениеЗаполнено(ОбъектИС.Партнер) Тогда
		ГУИД = ОбъектИС.ПолучитьСсылкуНового().УникальныйИдентификатор();
		СвязанныйПартнер = Справочники.Партнеры.ПолучитьСсылку(ГУИД);
		ДанныеИзЗаполняемогоДокумента = Новый Структура("Партнер");
		ЗаполнитьЗначенияСвойств(ДанныеИзЗаполняемогоДокумента, ЗаполняемыйОбъектИС);
		Если ОбщегоНазначения.СсылкаСуществует(СвязанныйПартнер) Тогда
			ОбъектИС.Партнер = СвязанныйПартнер;
		ИначеЕсли ЗначениеЗаполнено(ДанныеИзЗаполняемогоДокумента.Партнер) Тогда
			ОбъектИС.Партнер = ДанныеИзЗаполняемогоДокумента.Партнер;
		Иначе
			Запрос = Новый Запрос(
				"ВЫБРАТЬ
			...
			Запрос.УстановитьПараметр("Наименование", ОбъектИС.Наименование);
			Выборка = Запрос.Выполнить().Выбрать();
			Если Выборка.Количество() = 1 Тогда
				Выборка.Следующий();
				ОбъектИС.Партнер = Выборка.Партнер;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Если ТипЗнч(ОбъектИС) = Тип("СправочникОбъект.Партнеры") Тогда
		...
		Если ИсточникXDTO <> Неопределено И ЗначениеЗаполнено(ИсточникXDTO.fullName) Тогда
			ОбъектИС.НаименованиеПолное = ИсточникXDTO.fullName;
		Иначе
			ОбъектИС.НаименованиеПолное = ОбъектXDTO.name;
		КонецЕсли;
		
		ЮрФизЛицоИзОбъектаXDTO = ИнтеграцияС1СДокументооборотУП.ЮрФизЛицоИзОбъектаXDTO(ИсточникXDTO);
		Если ЮрФизЛицоИзОбъектаXDTO = Перечисления.ЮрФизЛицо.ЮрЛицо
				Или ЮрФизЛицоИзОбъектаXDTO = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент Тогда
			ОбъектИС.ЮрФизЛицо = Перечисления.КомпанияЧастноеЛицо.Компания;
		Иначе
			ОбъектИС.ЮрФизЛицо = Перечисления.КомпанияЧастноеЛицо.ЧастноеЛицо;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ОбъектИС.ГруппаДоступа) Тогда
			ОбъектИС.ГруппаДоступа = ИнтеграцияС1СДокументооборотУП.ГруппаДоступаПартнеровПоУмолчанию();
		КонецЕсли;
		
		Если Не ОбъектИС.Клиент И Не ОбъектИС.Поставщик И Не ОбъектИС.ПрочиеОтношения И Не ОбъектИС.Конкурент
				И Не ОбъектИС.ОбслуживаетсяТорговымиПредставителями И Не ОбъектИС.Перевозчик Тогда
			// Вид отношений с партнером неизвестен.
			ОбъектИС.Клиент = Истина;
			ОбъектИС.Поставщик = Истина;
		КонецЕсли;
		
		НачатьТранзакцию();
		Попытка
			СсылкаНовогоПартнера = ОбъектИС.ПолучитьСсылкуНового();
			ГУИД = СсылкаНовогоПартнера.УникальныйИдентификатор();
			СвязанныйКонтрагент = Справочники.Контрагенты.ПолучитьСсылку(ГУИД);
			Если ОбщегоНазначения.СсылкаСуществует(СвязанныйКонтрагент) Тогда
				ПартнерКонтрагента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СвязанныйКонтрагент, "Партнер");
				Если Не ЗначениеЗаполнено(ПартнерКонтрагента) Тогда
					Контрагент = СвязанныйКонтрагент.ПолучитьОбъект();
					Контрагент.Заблокировать();
					Контрагент.Партнер = СсылкаНовогоПартнера;
					Контрагент.Записать();
				КонецЕсли;
			КонецЕсли;
			ДанныеИзЗаполняемогоДокумента = Новый Структура("Контрагент");
			ЗаполнитьЗначенияСвойств(ДанныеИзЗаполняемогоДокумента, ЗаполняемыйОбъектИС);
			Если ЗначениеЗаполнено(ДанныеИзЗаполняемогоДокумента.Контрагент) Тогда
				ПартнерКонтрагента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					ДанныеИзЗаполняемогоДокумента.Контрагент, "Партнер");
				Если Не ЗначениеЗаполнено(ПартнерКонтрагента) Тогда
					Контрагент = ДанныеИзЗаполняемогоДокумента.Контрагент.ПолучитьОбъект();
					Контрагент.Заблокировать();
					Контрагент.Партнер = СсылкаНовогоПартнера;
					Контрагент.Записать();
				КонецЕсли;
	...
	Если ОбъектXDTO.objectID.type = "DMVATRateUniversal" И ТипЗнч(ОбъектИС) = Тип("СправочникОбъект.СтавкиНДС") Тогда
		
		ОбъектИС.НачалоПериода = ОбъектXDTO.beginOfPeriod;
		ОбъектИС.КонецПериода = ОбъектXDTO.endOfPeriod;
		ОбъектИС.Ставка = ОбъектXDTO.rate;
		ОбъектИС.РасчетнаяСтавка = ОбъектXDTO.calculatedRate;
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ОбъектXDTO, "country") Тогда
			ОбъектИС.Страна = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗначениеРеквизитаОбъектаИСПоЗначениюДляЗаполнения(
				Тип("СправочникСсылка.СтраныМира"),
				ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗначениеДляЗаполненияРеквизитаОбъектаИС(
					ОбъектXDTO.country));
		КонецЕсли;
		НоваяСтрока = ОбъектИС.ТипыНалогообложенияНДС.Добавить();
		НоваяСтрока.ТипНалогообложенияНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС;
