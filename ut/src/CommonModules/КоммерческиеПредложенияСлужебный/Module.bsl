...
Функция ОтправитьЗапросКоммерческихПредложенийАсинхронно(Знач ПараметрыОтправки) Экспорт
	...
	Иначе
		...
		Результат.ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
			...
			ПараметрыОтправки, ПараметрыВыполнения);
...
Процедура ПроверитьИзменениеЗапросаКоммерческихПредложенийПоставщиков(Источник) Экспорт
	...
	Если Источник.ЗапрашиватьПредложенияПоставщиков = КоммерческиеПредложенияДокументыКлиентСервер.СпособРазмещенияБидзаар() Тогда
		// Для Бидзаар контроль производится только в форме документа
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьИзменениеВложенияЗапросаКоммерческихПредложенийПоставщиков(Источник, Отказ) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Источник, "ЭтоИзображениеТоварнойПозиции")
		И Источник.ЭтоИзображениеТоварнойПозиции Тогда
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(Источник.ДополнительныеСвойства) = Тип("Структура") И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Источник.ДополнительныеСвойства, "РегистрацияОтветаИзСервиса") Тогда
		Возврат;
	КонецЕсли;
...
Функция ДанныеЗапросаКоммерческихПредложенийПоставщиков(Знач Документ, ПроверятьЗаполнение, Отказ) Экспорт
	...
	Если Не Отказ И ПроверятьЗаполнение Тогда
		...
		ВалидацияВложенийЗапросаКоммерческихПредложенийПоставщиков(Документ, Результат.Ошибки);	
...
// Подбирает изображение из системы для автоподстановки в строку товарной позиции документа ЗКПП 
// согласно обозначенному алгоритму:
// Шаг 1. Получение сохраненного значения из нового регистра сведений ИзображенияЗапросовКоммерческихПредложений.
//        Если сохраненное значение имеется, второй шаг пропускается.
// Шаг 2. Получение изображения из процедуры ТаблицаПодобранныхИзображенийДляПубликацииВЗКП,
//        которая возвращает таблицу значений с изображениями, подобранными согласно алгоритму,
//        определенному на уровне прикладного решения и отфильтрованными на уровне библиотеки.
// 
// Параметры:
...
// 
// Возвращаемое значение:
//  Неопределено, ОпределяемыйТип.ПрисоединенныеФайлыИзображенийЗКППоставщика - Если не удалось подобрать изображение,
...
Функция ПодобранноеИзображениеТовараЗКП(Номенклатура, Характеристика) Экспорт
	...
	Результат = СохраненноеИзображениеТовараЗКПУПоставщика(Номенклатура, Характеристика, ИзображениеОчищено);
	УстановитьПривилегированныйРежим(Ложь);
	
	Если ЗначениеЗаполнено(Результат) Или ИзображениеОчищено Тогда
		Возврат Результат;
	КонецЕсли;
	
	ТаблицаИзображений = ТаблицаПодобранныхИзображенийДляПубликацииВЗКП(Номенклатура, Характеристика, Истина);
	
	Если ТаблицаИзображений.Количество() > 0 Тогда
		Результат = ТаблицаИзображений[0].Изображение;
	КонецЕсли;
...
// Подбирает изображения из системы для автоподстановки в строки товарных позиций документа ЗКПП 
// согласно алгоритму функции см. ПодобранноеИзображениеТовараЗКП.
// 
// Параметры:
...
//    * Значение - ОпределяемыйТип.ПрисоединенныеФайлыИзображенийЗКППоставщика - подобранное изображение, если изображение
//    		не нашлось, то в соответствии будет отсутствовать ключ.
//
Функция ПодобранныеИзображенияТоваровЗКП(Знач КортежиСсылок) Экспорт
	
	Результат = Новый Соответствие;
	
	НоменклатураИХарактеристики = Новый ТаблицаЗначений;
	НоменклатураИХарактеристики.Колонки.Добавить("Ключ", Новый ОписаниеТипов("Строка"));
	НоменклатураИХарактеристики.Колонки.Добавить("Номенклатура", Метаданные.ОпределяемыеТипы.НоменклатураБЭД.Тип);
	НоменклатураИХарактеристики.Колонки.Добавить("Характеристика", Метаданные.ОпределяемыеТипы.ХарактеристикаНоменклатурыБЭД.Тип);
		
	Для каждого ЭлементКоллекции Из КортежиСсылок Цикл
		НоваяСтрока = НоменклатураИХарактеристики.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, ЭлементКоллекции.Значение);
		НоваяСтрока.Ключ = ЭлементКоллекции.Ключ;
	КонецЦикла;
	...
	НоменклатураИХарактеристикиДляПоиска = НоменклатураИХарактеристики.Скопировать();
	НоменклатураИХарактеристикиДляПоиска.Свернуть("Номенклатура, Характеристика");
	УстановитьПривилегированныйРежим(Истина);
	СохраненныеИзображенияТоваров = СохраненныеИзображенияТоваровЗКПУПоставщика(НоменклатураИХарактеристикиДляПоиска);
	УстановитьПривилегированныйРежим(Ложь);
	
	СтруктураПоиска = Новый Структура("Номенклатура, Характеристика");
	Для Каждого СтрокаСохраненногоИзображения Из СохраненныеИзображенияТоваров Цикл
		ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаСохраненногоИзображения);
		НайденныеСтроки = НоменклатураИХарактеристики.НайтиСтроки(СтруктураПоиска);
		
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			//Если изображение было очищено пользователем, то ФайлИзображения будет пустой
			//В таком случае в результат он не добавляется, но из дальнейшего подбора убирается
			Если ЗначениеЗаполнено(СтрокаСохраненногоИзображения.ФайлИзображения) Тогда
				Результат.Вставить(НайденнаяСтрока.Ключ, СтрокаСохраненногоИзображения.ФайлИзображения);
			КонецЕсли;
			НоменклатураИХарактеристики.Удалить(НайденнаяСтрока);		
		КонецЦикла;
	КонецЦикла;
	...
	НоменклатураИХарактеристикиДляПоиска = НоменклатураИХарактеристики.Скопировать();
	НоменклатураИХарактеристикиДляПоиска.Свернуть("Номенклатура, Характеристика");
	ТаблицаИзображений = ТаблицаПодобранныхИзображенийТоварныхПозицийДляПубликацииВЗКП(НоменклатураИХарактеристикиДляПоиска);
	
	СтруктураПоиска = Новый Структура("Номенклатура");
	Для Каждого СтрокаИзображения Из ТаблицаИзображений Цикл
		СтруктураПоиска.Номенклатура = СтрокаИзображения.Владелец;
		НайденныеСтроки = НоменклатураИХарактеристики.НайтиСтроки(СтруктураПоиска);
		
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			Результат.Вставить(НайденнаяСтрока.Ключ, СтрокаИзображения.Изображение);
		КонецЦикла;
	КонецЦикла;
...
// Возвращает таблицу изображений товарной позиции, подобранных на стороне прикладного решения,
// прошедших валидацию и отсортированных, доступных для выбора или автоподстановки в документ ЗКПП
// для дальнейшей публикации.
// 
// Параметры:
//  Номенклатура - ОпределяемыйТип.НоменклатураБЭД
//  Характеристика - ОпределяемыйТип.ХарактеристикаНоменклатурыБЭД
//  РежимПодбораОдиночный - Булево - Истина - подбирается изображение для автоподстановки.
// 
// Возвращаемое значение:
//  см. НовыйРезультатПодбораИзображенийВЗКП
Функция ТаблицаПодобранныхИзображенийДляПубликацииВЗКП(Номенклатура, 
	Характеристика, РежимПодбораОдиночный = Ложь) Экспорт
		
	ПараметрыПодбора = НовыеПараметрыПодбораИзображенийВЗКП();
	ПараметрыПодбора.Номенклатура = Номенклатура;
	ПараметрыПодбора.Характеристика = Характеристика;
	ПараметрыПодбора.РежимПодбораОдиночный = РежимПодбораОдиночный;
	
	РезультатПодбора = НовыйРезультатПодбораИзображенийВЗКП();
	КоммерческиеПредложенияПереопределяемый.ПодобратьИзображенияДляПубликацииВЗКП(ПараметрыПодбора, РезультатПодбора);
	
	ТаблицаСтрокСОшибками = ОшибкиВалидацииИзображенийТаблицы(РезультатПодбора);
	Для Каждого СтрокаСОшибками Из ТаблицаСтрокСОшибками Цикл
		РезультатПодбора.Удалить(СтрокаСОшибками.СтрокаИзображения);
	КонецЦикла;
	
	РезультатПодбора.Сортировать("ПорядокСортировки");
	
	МаксимальноеКоличествоИзображений = 100;
	ИсключитьЛишниеИзображенияИзТаблицы(РезультатПодбора, МаксимальноеКоличествоИзображений);
			
	Возврат РезультатПодбора;
	
КонецФункции

// Возвращает таблицу изображений товарных позиций, подобранных на стороне прикладного решения,
// прошедших валидацию и отсортированных, доступных для выбора или автоподстановки в документ ЗКПП для дальнейшей публикации.
// 
// Параметры:
...
Функция ТаблицаПодобранныхИзображенийТоварныхПозицийДляПубликацииВЗКП(НоменклатураИХарактеристики) Экспорт
		
	ПараметрыПодбора = НовыеПараметрыПодбораИзображенийВЗКП();
	ПараметрыПодбора.НоменклатураИХарактеристики = НоменклатураИХарактеристики;
...
// Длительная операция. Записывает сохраненные в КЭШе изображения, выбранные в ТЧ Товары на форме документа 
// 	ЗКППоставщика
// 	
// 	Примечание:
// 		Входная таблица должна содержать уникальные записи. 
// 
// Параметры:
//  ТаблицаИзображений - ТаблицаЗначений:
//  * Номенклатура - ОпределяемыйТип.НоменклатураБЭД
//  * Характеристика - ОпределяемыйТип.ХарактеристикаНоменклатурыБЭД
//  * Изображение - ОпределяемыйТип.ПрисоединенныеФайлыИзображенийЗКППоставщика, Неопределено - Если пользователь 
//		очистил/не выбрал изображение, сохраняется пустое значение
Процедура СохранитьВыбранныеИзображенияТоварныхПозицийЗКП(Знач ТаблицаИзображений) Экспорт

	// Проверим наличие дублей. Если имеются, вызовем исключение
	СвернутаяТаблицаИзображений = ТаблицаИзображений.Скопировать();
	СвернутаяТаблицаИзображений.Свернуть("Номенклатура, Характеристика");
	Если СвернутаяТаблицаИзображений.Количество() < ТаблицаИзображений.Количество() Тогда
		ВызватьИсключение НСтр("ru = 'В таблице изображений присутствуют дубли строк'", 
	...
	Попытка 
		...
		ЭлементБлокировки = БлокировкаДанных.Добавить("РегистрСведений.ИзображенияЗапросовКоммерческихПредложений");
		ЭлементБлокировки.ИсточникДанных = ТаблицаИзображений;
		ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура");
		ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Характеристика", "Характеристика");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		БлокировкаДанных.Заблокировать();
		
		ИсключитьРанееСохраненныеИдентичныеИзображенияЗКП(ТаблицаИзображений);
	
		Для Каждого СтрокаИзмененныхИзображений Из ТаблицаИзображений Цикл
			
			СохранитьИзображениеТовараЗКПУПоставщика(
				СтрокаИзмененныхИзображений.Номенклатура,
				СтрокаИзмененныхИзображений.Характеристика, 
				СтрокаИзмененныхИзображений.Изображение);	
...
// Выполняет валидацию изображений запроса коммерческих предложений поставщиков.
// 
// Параметры:
//  Документ - ОпределяемыйТип.ЗапросКоммерческихПредложенийПоставщиков - Ссылка на документ, изображения которого 
//  	необходимо проверить.
//  Отказ - Булево - Устанавливается в Истина, если валидация хотя бы по одной из строк не пройдена
Процедура ВалидацияИзображенийЗапросаКоммерческихПредложенийПоставщиков(Документ, Отказ) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ЗапросКоммерческихПредложенийПоставщиковТовары.НомерСтроки,
		|	ЗапросКоммерческихПредложенийПоставщиковТовары.Изображение,
		...
		|	И ЗапросКоммерческихПредложенийПоставщиковТовары.Изображение <> НЕОПРЕДЕЛЕНО
		|	И ЗапросКоммерческихПредложенийПоставщиковТовары.ИсточникДобавленияТовара <> &ИсточникДобавленияТовараЗКП1СНоменклатура";

	Запрос.УстановитьПараметр("Ссылка", Документ);
	Запрос.УстановитьПараметр("ИсточникДобавленияТовараЗКП1СНоменклатура", ИсточникДобавленияТовараЗКП1СНоменклатура());
	...
	ТаблицаСтрокСОшибками = ОшибкиВалидацииИзображенийТаблицы(РезультатЗапроса.Выгрузить());

	Для Каждого СтрокаСОшибками Из ТаблицаСтрокСОшибками Цикл
		
		НомерСтрокиДокумента = СтрокаСОшибками.СтрокаИзображения.НомерСтроки;
		
		ПолеСообщения = СтрШаблон("Объект.Товары[%1].ИзображениеВыбрано", НомерСтрокиДокумента - 1);
		
		ОбщегоНазначения.СообщитьПользователю(СтрокаСОшибками.ОшибкиСтроки, , ПолеСообщения, , Отказ);	
		
	КонецЦикла;			
	
КонецПроцедуры

// Выполняет валидацию изображений таблицы и возвращает ошибки валидации
// 
// Параметры:
//  ТаблицаИзображений - ТаблицаЗначений - Обязательно должна содержать колонку с именем Изображение. 
//  										Тип колонки должен быть ссылкой на справочник присоединенных файлов
//  										или двоичные данные. 
// Возвращаемое значение:
//  ТаблицаЗначений:
//  * СтрокаИзображения - СтрокаТаблицыЗначений - Строка входящей таблицы изображений, не прошедшая валидацию.
//  * ОшибкиСтроки - Строка - Ошибки строки изображений, разделенные переносом каретки (Символы.ПС)
//  
Функция ОшибкиВалидацииИзображенийТаблицы(ТаблицаИзображений) Экспорт
	
	Результат = Новый ТаблицаЗначений;
	Результат.Колонки.Добавить("СтрокаИзображения", Новый ОписаниеТипов("СтрокаТаблицыЗначений"));
	Результат.Колонки.Добавить("ОшибкиСтроки", Новый ОписаниеТипов("Строка"));
	
	МинимальноеКоличествоПикселей = 100; // px
	МаксимальныйРазмерФайлаВМегабайтах = 30; // мб
	РазрядностьЧисла = 1024 * 1024;
	ДопустимыеФорматыИзображений = СтрСоединить(РасширенияИзображений(), ",");

	ТекстОшибкиРазмерМеньше = СтрШаблон(НСтр("ru = 'Размер изображения должен быть больше %1x%1 px'",
		ОбщегоНазначения.КодОсновногоЯзыка()), МинимальноеКоличествоПикселей);
	ТекстОшибкиВесБольше = СтрШаблон(НСтр("ru = 'Размер изображения должен быть меньше %1 мб'", 
		ОбщегоНазначения.КодОсновногоЯзыка()), МаксимальныйРазмерФайлаВМегабайтах);
	ТекстОшибкиДопустимыйФормат = СтрШаблон(НСтр("ru = 'Допустимы форматы изображений: %1'", 
		ОбщегоНазначения.КодОсновногоЯзыка()), ДопустимыеФорматыИзображений);
	ТекстОшибкиПомеченныйИлиБитыйФайл = НСтр("ru = 'Файл изображения помечен на удаление и очищен как ненужный: %1'", 
		ОбщегоНазначения.КодОсновногоЯзыка());
	
	//Проверка возможности выполнить оптимальную предварительную проверку ссылок на изображения.
	//Она будет выполнена, если колонка "Изображение" содержит один тип ссылки (данные с типом "ДвоичныеДанные" игнорируются).
	ТипПервойСсылки = Неопределено;
	ОдинТипСсылокНаИзображения = Истина;
	СсылкиНаИзображения = Новый Массив;
	Для Каждого Строка Из ТаблицаИзображений Цикл
		Если ТипЗнч(Строка.Изображение) <> Тип("ДвоичныеДанные") Тогда
			//Запоминаем тип первой ссылки на изображение
			Если ТипПервойСсылки = Неопределено Тогда
				ТипПервойСсылки = ТипЗнч(Строка.Изображение);
			КонецЕсли;
			//Сравниваем тип первой ссылки на изображение со всеми другими ссылками
			Если ТипПервойСсылки <> ТипЗнч(Строка.Изображение) Тогда
				ОдинТипСсылокНаИзображения = Ложь;
				Прервать;	
			КонецЕсли;
			СсылкиНаИзображения.Добавить(Строка.Изображение);			
		КонецЕсли;
	КонецЦикла;
	
	ПредварительнаяПроверка = ОдинТипСсылокНаИзображения И СсылкиНаИзображения.Количество() > 0;
	
	Если ПредварительнаяПроверка Тогда
		ПроверенныеСсылки = ПроверкаСсылокИзображений(СсылкиНаИзображения);
		ДвоичныеДанныеФайлов = РаботаСФайлами.ДвоичныеДанныеФайлов(ПроверенныеСсылки)
	КонецЕсли;
		
	Для Каждого Строка Из ТаблицаИзображений Цикл
	
		МассивОшибокСтроки = Новый Массив;
		
		Если ТипЗнч(Строка.Изображение) = Тип("ДвоичныеДанные") Тогда
			ДвоичныеДанныеИзображения = Строка.Изображение;	
		Иначе
			Если ПредварительнаяПроверка Тогда
				Если ПроверенныеСсылки.Найти(Строка.Изображение) = Неопределено Тогда
					ОбщийТекстОшибокСтроки = СтрШаблон(ТекстОшибкиПомеченныйИлиБитыйФайл, Строка.Изображение);
					ДобавитьОшибкиВалидацииСтрокиТаблицыИзображений(Результат, Строка, ОбщийТекстОшибокСтроки);	
					Продолжить;		
				Иначе
					ДвоичныеДанныеИзображения = ДвоичныеДанныеФайлов.Получить(Строка.Изображение);		
				КонецЕсли;	
			Иначе
				Если Не ОбщегоНазначения.СсылкаСуществует(Строка.Изображение) 
					ИЛИ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Строка.Изображение, "ПометкаУдаления") Тогда
					ОбщийТекстОшибокСтроки = СтрШаблон(ТекстОшибкиПомеченныйИлиБитыйФайл, Строка.Изображение);
					ДобавитьОшибкиВалидацииСтрокиТаблицыИзображений(Результат, Строка, ОбщийТекстОшибокСтроки);	
					Продолжить;		
				Иначе
					ДвоичныеДанныеИзображения = РаботаСФайлами.ДвоичныеДанныеФайла(Строка.Изображение);		
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если СтрНайти(ДопустимыеФорматыИзображений, НРег(Строка(Картинка.Формат()))) = 0 Тогда
			ДобавитьОшибкиВалидацииСтрокиТаблицыИзображений(Результат, Строка, ТекстОшибкиДопустимыйФормат);
			Продолжить;
		КонецЕсли;

		Если Ширина < МинимальноеКоличествоПикселей Или Высота < МинимальноеКоличествоПикселей Тогда					
			МассивОшибокСтроки.Добавить(ТекстОшибкиРазмерМеньше);				
		КонецЕсли;
		
		Если Картинка.РазмерФайла() / РазрядностьЧисла > МаксимальныйРазмерФайлаВМегабайтах Тогда
			МассивОшибокСтроки.Добавить(ТекстОшибкиВесБольше);				
		КонецЕсли;
				
		Если МассивОшибокСтроки.Количество() > 0 Тогда
			ОбщийТекстОшибокСтроки = СтрСоединить(МассивОшибокСтроки, Символы.ПС);
			ДобавитьОшибкиВалидацииСтрокиТаблицыИзображений(Результат, Строка, ОбщийТекстОшибокСтроки);
		КонецЕсли;
...
// Обрабатывает выбранный из файловой системы файл изображения, указываемый для товарной позиции документа ЗКППоставщика
// Выполняет валидацию выбранного файла изображения и создание объекта в системе при успешной валидации. 
// Для прикладного справочника товарной позиции реализовано переопределение для создания объекта в системе средствами 
// прикладного решения. 
// 
// Параметры:
//  ДокументСсылка - ОпределяемыйТип.ЗапросКоммерческихПредложенийПоставщиков
//  ДанныеДляДобавленияФайла - см. НовыеДанныеДляДобавленияИзображенияИзФайловойСистемы
//  ЭтоТоварРучногоОписания - Булево - Признак использования в качестве товарной позиции вручную описываемого товара
// 
// Возвращаемое значение:
//  см. НовыйРезультатОбработкиИзображения 
Функция ОбработатьДобавлениеИзображения(ДокументСсылка, ДанныеДляДобавленияФайла, ЭтоТоварРучногоОписания) Экспорт

	Результат = НовыйРезультатОбработкиИзображения();	
	
	ТаблицаИзображений = Новый ТаблицаЗначений();
	ТаблицаИзображений.Колонки.Добавить("Изображение", Новый ОписаниеТипов("ДвоичныеДанные"));	
	НоваяСтрока = ТаблицаИзображений.Добавить();
	НоваяСтрока.Изображение = ПолучитьИзВременногоХранилища(ДанныеДляДобавленияФайла.АдресДвоичныхДанныхФайла);
	
	ОшибкиВалидации = ОшибкиВалидацииИзображенийТаблицы(ТаблицаИзображений);
	
	Если ОшибкиВалидации.Количество() > 0 Тогда
		Результат.Отказ = Истина;
		Результат.Ошибки = ОшибкиВалидации[0].ОшибкиСтроки;
		Возврат Результат;	
	КонецЕсли;
		
	Если ЭтоТоварРучногоОписания Тогда
		
		ДополнительныеРеквизиты = Новый Структура("ЭтоИзображениеТоварнойПозиции", Истина);			
		ПараметрыДобавленияФайла = РаботаСФайлами.ПараметрыДобавленияФайла(ДополнительныеРеквизиты);
		...
		Результат.Изображение = РаботаСФайлами.ДобавитьФайл(ПараметрыДобавленияФайла, 
			ДанныеДляДобавленияФайла.АдресДвоичныхДанныхФайла);
						
	Иначе
				
		КоммерческиеПредложенияПереопределяемый.ПриДобавленииИзображенияИзФайловойСистемы(
			ДанныеДляДобавленияФайла, Результат.Изображение, Результат.Отказ, Результат.Ошибки);		
...
// Изображения товарных позиций документа ЗКП поставщика. 
// 	С признаком ТолькоТекущие = Истина получает присоединенные	файлы документа, на которые ссылается реквизит 
// 	Изображение табличной части Товары документа ЗКП поставщика. 
// 	Иначе все присоединенные файлы документа с установленным свойством ЭтоИзображениеТоварнойПозиции 
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка.ЗапросКоммерческихПредложенийПоставщиков - Документ, изображения товарных позиций
//  	которого необходимо получить 
//  ТолькоТекущие - Булево - Признак необходимости получения изображений ручных товаров из табличной части документа.
// 
// Возвращаемое значение:
//  Массив Из СправочникСсылка.ЗапросКоммерческихПредложенийПоставщиковПрисоединенныеФайлы
Функция ИзображенияРучныхТоварныхПозицийЗКППоставщика(ДокументСсылка, ТолькоТекущие = Истина) Экспорт
		
	Запрос = Новый Запрос;	
	Если ТолькоТекущие Тогда
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ЗапросКоммерческихПредложенийПоставщиковТовары.Изображение КАК Изображение
		|ИЗ
		|	Документ.ЗапросКоммерческихПредложенийПоставщиков.Товары КАК ЗапросКоммерческихПредложенийПоставщиковТовары
		...
		|		ПО ЗапросКоммерческихПредложенийПоставщиковТовары.Ссылка = ЗапросКоммерческихПредложенийПоставщиковПрисоединенныеФайлы.ВладелецФайла
		|		И
		|			ЗапросКоммерческихПредложенийПоставщиковТовары.Изображение = ЗапросКоммерческихПредложенийПоставщиковПрисоединенныеФайлы.Ссылка
		|		И ЗапросКоммерческихПредложенийПоставщиковТовары.Ссылка = &ДокументСсылка
		|		И НЕ ЗапросКоммерческихПредложенийПоставщиковПрисоединенныеФайлы.ПометкаУдаления
		|		И ЗапросКоммерческихПредложенийПоставщиковПрисоединенныеФайлы.ЭтоИзображениеТоварнойПозиции";
	Иначе
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ЗапросКоммерческихПредложенийПоставщиковПрисоединенныеФайлы.Ссылка КАК Изображение
		|ИЗ
		|	Справочник.ЗапросКоммерческихПредложенийПоставщиковПрисоединенныеФайлы КАК
		|		ЗапросКоммерческихПредложенийПоставщиковПрисоединенныеФайлы
		|ГДЕ
		|	ЗапросКоммерческихПредложенийПоставщиковПрисоединенныеФайлы.ВладелецФайла = &ДокументСсылка
		|	И НЕ ЗапросКоммерческихПредложенийПоставщиковПрисоединенныеФайлы.ПометкаУдаления
		|	И ЗапросКоммерческихПредложенийПоставщиковПрисоединенныеФайлы.ЭтоИзображениеТоварнойПозиции";		
	КонецЕсли;
	...
	Возврат РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Изображение");
	
КонецФункции

// Помечает на удаление присоединенные файлы документа ЗКППоставщика с признаком ЭтоИзображениеТоварнойПозиции, если 
// 	на него нет ссылок в реквизите Изображение табличной части Товары документа. 
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка.ЗапросКоммерческихПредложенийПоставщиков
Процедура УдалитьНеАктуальныеИзображенияРучныхТоваровЗКППоставщика(ДокументСсылка) Экспорт
	
	ТекущиеИзображенияТоварныхПозицийДокумента = ИзображенияРучныхТоварныхПозицийЗКППоставщика(ДокументСсылка);
	ВсеИзображенияТоварныхПозицийДокумента = ИзображенияРучныхТоварныхПозицийЗКППоставщика(ДокументСсылка, Ложь);
	НеАктуальныеИзображенияТоварныхПозицийДокумента = 
		ОбщегоНазначенияКлиентСервер.РазностьМассивов(
			ВсеИзображенияТоварныхПозицийДокумента, ТекущиеИзображенияТоварныхПозицийДокумента);

	Для Каждого Изображение Из НеАктуальныеИзображенияТоварныхПозицийДокумента Цикл
		УдалитьФайлИзображенияРучногоОписания(Изображение);	
	КонецЦикла;
		
КонецПроцедуры

// Количество и размер присоединенных файлов ЗКППоставщика.
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка.ЗапросКоммерческихПредложенийПоставщиков - Документ, количество и объем 
//  	присоединенных файлов которого необходимо получить
// 
// Возвращаемое значение:
//  см. НовыйКоличествоИРазмерПрисоединенныхФайловЗКППоставщика
Функция КоличествоИРазмерПрисоединенныхФайловЗКППоставщика(ДокументСсылка) Экспорт
	
	Результат = НовыйКоличествоИРазмерПрисоединенныхФайловЗКППоставщика();
	...
	Запрос.Текст = 
		...
		|	И НЕ ЗапросКоммерческихПредложенийПоставщиковПрисоединенныеФайлы.ЭтоИзображениеТоварнойПозиции
		|	И НЕ ЗапросКоммерческихПредложенийПоставщиковПрисоединенныеФайлы.ПометкаУдаления";
...
// Источник добавления в ЗКП товара из внутреннего справочника
// 
// Возвращаемое значение:
//  Число
Функция ИсточникДобавленияТовараЗКПВнутреннийСправочник() Экспорт
	Возврат 0;
КонецФункции

// Источник добавления в ЗКП товара из 1С:Номенклатуры
// 
// Возвращаемое значение:
//  Число
Функция ИсточникДобавленияТовараЗКП1СНоменклатура() Экспорт
	Возврат 1;
КонецФункции

// Источник добавления в ЗКП товара описываемого вручную
// 
// Возвращаемое значение:
//  Число
Функция ИсточникДобавленияТовараЗКПРучнойТовар() Экспорт
	Возврат 2;
КонецФункции
...
// Новые параметры подбора изображений в ЗКП.
// 
// Возвращаемое значение:
//  Структура - Новые параметры подбора изображений ВЗКП:
//		* Номенклатура - ОпределяемыйТип.НоменклатураБЭД - номенклатура для подбора изображения по одной позиции
//					   - Неопределено - при подборе изображений по нескольким позициям
//  	* Характеристика - ОпределяемыйТип.ХарактеристикаНоменклатурыБЭД - характеристика для подбора изображения
//  																	   по одной позиции.
//  					 - Неопределено - при подборе изображений по нескольким позициям												   
//  	* РежимПодбораОдиночный - Булево - определяет вариант подбора изображений по одной позиции,
//  								 	   если Истина, то будет выбрано одно изображение по позиции,
//  								 	   если Ложь, то будут выбраны все изображения по позиции. 
//  	* НоменклатураИХарактеристики - Неопределено - при подборе изображений по одной позиции 
//  								  - ТаблицаЗначений - таблица для подбора изображений по нескольким позициям,
//														  если указана, то параметры для подбора по одной позиции
//														  игнорируются, содержит колонки:
//  		** Номенклатура - ОпределяемыйТип.НоменклатураБЭД - Номенклатура товарной позиции
//  		** Характеристика - ОпределяемыйТип.ХарактеристикаНоменклатурыБЭД - характеристика товарной позиции
//
Функция НовыеПараметрыПодбораИзображенийВЗКП() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("Номенклатура", Неопределено);
	Результат.Вставить("Характеристика", Неопределено);
	Результат.Вставить("РежимПодбораОдиночный", Ложь);
	Результат.Вставить("НоменклатураИХарактеристики", Неопределено);
	
	Возврат Результат;
	
КонецФункции

// Новый результат подбора изображений в ЗКП.
// 
// Возвращаемое значение:
//  ТаблицаЗначений:
// * Изображение - ОпределяемыйТип.ПрисоединенныеФайлыИзображенийЗКППоставщика - Подобранное изображение 
// 		Доступные форматы изображений см. РасширенияИзображений
// * ПорядокСортировки - Число - Более приоритетными считаются записи с меньшей величиной числового значения в порядке
// 		сортировки.
// * ПредставлениеВладельца - Строка - Используется для отображения на форме выбора изображений.
// * Владелец - ОпределяемыйТип.ВладелецПрисоединенныхФайлов, Неопределено - Используется для формирования гиперссылки 
// 																			перехода на владельца. Не обязателен.
Функция НовыйРезультатПодбораИзображенийВЗКП() Экспорт
	
	ОписаниеТиповИзображений = Метаданные.ОпределяемыеТипы.ПрисоединенныеФайлыИзображенийЗКППоставщика.Тип;
	
	Результат = Новый ТаблицаЗначений;
	Результат.Колонки.Добавить("Изображение", ОписаниеТиповИзображений);
	Результат.Колонки.Добавить("ПорядокСортировки", Новый ОписаниеТипов("Число"));
	Результат.Колонки.Добавить("ПредставлениеВладельца", Новый ОписаниеТипов("Строка"));
	Результат.Колонки.Добавить("Владелец", Неопределено);
...
Функция РасширенияИзображений() Экспорт
	...
	РасширенияИзображений.Добавить("rle");
...
//  ОтносительныйАдрес - Булево - признак относительного адреса изображения
// 
// Возвращаемое значение:
//  Строка - Адрес изображения в хранилище
//
Функция АдресИзображенияВХранилище(Знач АдресВСервисе, ОтносительныйАдрес = Истина) Экспорт
	
	Адрес = "";
	ДанныеИзображения = ДанныеФайлаПоВнешнейСсылке(АдресВСервисе, ОтносительныйАдрес);
	Если ДанныеИзображения.Статус И ЭтоФайлКартинки(ДанныеИзображения.Путь) Тогда
...
// 
// Параметры:
//  ПолноеИмяФайла - Строка - Полный адрес файла из которого необходимо вычленить непосредственное имя файла
//  							и его расширение
// 
// Возвращаемое значение:
//  см. НовыйИмяФайлаИРасширение
Функция ИмяФайлаИРасширение(Знач ПолноеИмяФайла) Экспорт
	
	Результат = НовыйИмяФайлаИРасширение();
	
	РазбиениеАдресаИзображения = СтрРазделить(ПолноеИмяФайла, "/\");
	ИмяФайла = РазбиениеАдресаИзображения.Получить(РазбиениеАдресаИзображения.Количество() - 1);		
	ИмяФайла = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(ИмяФайла);									
	Результат.ИмяБезРасширения = СтрРазделить(ИмяФайла, ".")[0];		
	РасширениеФайла = ОбщегоНазначенияКлиентСервер.ПолучитьРасширениеИмениФайла(ИмяФайла);		
	Результат.РасширениеБезТочки = ОбщегоНазначенияКлиентСервер.РасширениеБезТочки(РасширениеФайла);
		
	Возврат Результат;
	
КонецФункции

// Конструктор. Новые данные для добавления изображения из файловой системы в документ ЗКППоставщика
// 
// Параметры:
//  Номенклатура - ОпределяемыйТип.НоменклатураБЭД - Номенклатура, для которой добавляется файл
//  Характеристика - ОпределяемыйТип.ХарактеристикаНоменклатурыБЭД - Характеристика, для которой добавляется файл
//  АдресДвоичныхДанныхФайла - Строка - Адрес временного хранилища с двоичными данными файла
//  ИмяБезРасширения - Строка - Непосредственное имя файла без расширения
...
//  РасширениеБезТочки - Строка - Расширение файла без точки
//  		
Функция НовыеДанныеДляДобавленияИзображенияИзФайловойСистемы(Номенклатура, Характеристика, АдресДвоичныхДанныхФайла, 
		ИмяБезРасширения, РасширениеБезТочки) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("Номенклатура", Номенклатура);
	Результат.Вставить("Характеристика", Характеристика);
	Результат.Вставить("АдресДвоичныхДанныхФайла", АдресДвоичныхДанныхФайла);
...
// Возвращает ссылку на двоичные данные картинки
// 
// Параметры:
//  ФайлКартинки - ОпределяемыйТип.ПрисоединенныйФайл - ссылка на элемент справочника с файлом.
...
// 
// Возвращаемое значение:
//  Строка - адрес во временном хранилище, по которому помещены данные файла.
Функция НавигационнаяСсылкаКартинки(ФайлКартинки, ИдентификаторФормы = Неопределено) Экспорт
	...
	Если ДвоичныеДанныеФайла <> Неопределено Тогда
		Результат = ПоместитьВоВременноеХранилище(ДвоичныеДанныеФайла, ИдентификаторФормы);
	КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
    ...
    Методы.Вставить("ОтправитьЗапросКоммерческихПредложений", Истина);
    Методы.Вставить("ОтменитьЗапросКоммерческихПредложений", Истина);
    Методы.Вставить("СохранитьВыбранныеИзображенияТоварныхПозицийЗКП", Истина);
...
// Конструктор. Новый результат обработки изображения.
// 
// Возвращаемое значение:
//  Структура:
// * Отказ - Булево - Признак не успешности выполнения обработки
// * Ошибки - Строка - Строка описаний ошибок, разделенных переносом каретки (Символы.ПС)
// * Изображение - Неопределено, ОпределяемыйТип.ПрисоединенныеФайлыИзображенийЗКППоставщика - Добавленное изображение
//
Функция НовыйРезультатОбработкиИзображения()
	
	Результат = Новый Структура;
	Результат.Вставить("Отказ", Ложь);
	Результат.Вставить("Ошибки", "");
	Результат.Вставить("Изображение", Неопределено);
	
	Возврат Результат;
	
КонецФункции

Процедура ИсключитьРанееСохраненныеИдентичныеИзображенияЗКП(ТаблицаИзображений)
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТаблицаИзображений", ТаблицаИзображений);
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ТаблицаИзображений.Номенклатура,
		|	ТаблицаИзображений.Характеристика,
		|	ТаблицаИзображений.Изображение
		|ПОМЕСТИТЬ ВТ_ТаблицаИзображений
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ТаблицаИзображений.Номенклатура,
		|	ВТ_ТаблицаИзображений.Характеристика,
		|	ВТ_ТаблицаИзображений.Изображение
		|ИЗ
		|	ВТ_ТаблицаИзображений КАК ВТ_ТаблицаИзображений
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИзображенияЗапросовКоммерческихПредложений КАК
		|			ИзображенияЗапросовКоммерческихПредложений
		|		ПО ВТ_ТаблицаИзображений.Номенклатура = ИзображенияЗапросовКоммерческихПредложений.Номенклатура
		|		И ВТ_ТаблицаИзображений.Характеристика = ИзображенияЗапросовКоммерческихПредложений.Характеристика
		|		И ВТ_ТаблицаИзображений.Изображение = ИзображенияЗапросовКоммерческихПредложений.ФайлИзображения
		|ГДЕ
		|	ИзображенияЗапросовКоммерческихПредложений.Номенклатура ЕСТЬ NULL";
	
	РезультатЗапроса = Запрос.Выполнить();
	
	ТаблицаИзображений = РезультатЗапроса.Выгрузить();
		
КонецПроцедуры

Процедура ВалидацияВложенийЗапросаКоммерческихПредложенийПоставщиков(Документ, Ошибки)

	ДопустимыйОбщийРазмер = 30;
	ДанныеВложений = КоличествоИРазмерПрисоединенныхФайловЗКППоставщика(Документ);
	ОбщийРазмерВМегабайтах = Окр(ДанныеВложений.Размер / 1024 / 1024, 0);
	
	Если ОбщийРазмерВМегабайтах > ДопустимыйОбщийРазмер Тогда
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Превышение допустимого общего размера вложений в %1 Мб'", ОбщегоНазначения.КодОсновногоЯзыка()), 
			ДопустимыйОбщийРазмер);;
		ОбщегоНазначенияБЭД.ДобавитьОшибку(Ошибки, ТекстОшибки);
	КонецЕсли;
	
КонецПроцедуры

// См. РегистрыСведений.ИзображенияЗапросовКоммерческихПредложений.СохранитьИзображениеТовара
Процедура СохранитьИзображениеТовараЗКПУПоставщика(Номенклатура, Характеристика, Изображение)
	
	РегистрыСведений.ИзображенияЗапросовКоммерческихПредложений.СохранитьИзображениеТовара(
		Номенклатура, Характеристика, Изображение);
		
КонецПроцедуры

// См. РегистрыСведений.ИзображенияЗапросовКоммерческихПредложений.СохраненноеИзображениеТовара
Функция СохраненноеИзображениеТовараЗКПУПоставщика(Номенклатура, Характеристика, ИзображениеОчищено)
	
	Возврат РегистрыСведений.ИзображенияЗапросовКоммерческихПредложений.СохраненноеИзображениеТовара(
		Номенклатура, Характеристика, ИзображениеОчищено);
		
КонецФункции

// См. РегистрыСведений.ИзображенияЗапросовКоммерческихПредложений.СохраненныеИзображенияТоваров
Функция СохраненныеИзображенияТоваровЗКПУПоставщика(НоменклатураИХарактеристики)
	
	Возврат РегистрыСведений.ИзображенияЗапросовКоммерческихПредложений.СохраненныеИзображенияТоваров(НоменклатураИХарактеристики);
		...
		Для Каждого Позиция Из Товары.Строки Цикл
			...
			Если ЗначениеЗаполнено(СпособОписанияТовара.Значение) Тогда
				...
				ИначеЕсли СпособОписанияТовара.Значение = "Рубрика" Тогда
					...
					СтрокаДерева = Позиция.Строки.Найти("Товары.НомерСтроки.Товар.Изображение", "ПолныйПуть", Истина);
					CustomerSpecification.Вставить("image", СтрокаДерева.Значение);
					Если Не ЗначениеЗаполнено(СтрокаДерева.Значение) Тогда
						CustomerSpecification.Удалить("image");	
					КонецЕсли;
...
Функция ЗапросКоммерческихПредложенийИзМоделиСервиса(Знач МодельСервиса)
	...
	Для Каждого ПозицияСервиса Из МодельСервиса.proposalRequirements.positions Цикл
		...
		Если ЗначениеЗаполнено(ЗначениеСвойстваМоделиСервиса(ПозицияСервиса, "product.customSpecification")) Тогда
			...
			ТоварнаяПозиция.Товар.Вставить("Штрихкоды",
				ЗначениеСвойстваМоделиСервиса(ПозицияСервиса, "product.customSpecification.barcodes"));
			ТоварнаяПозиция.Товар.Вставить("Изображения",
				ЗначениеСвойстваМоделиСервиса(ПозицияСервиса, "product.customSpecification.imageUrls"));
			...
			РубрикаСервиса = Новый Структура;
						
			// Заполнение идентификатора категории 1СН
			РубрикаСервиса.Вставить("ИдентификаторРубрики", ЗначениеСвойстваМоделиСервиса(ПозицияСервиса,
					"product.ref1CnSpecification.categoryId"));
		
			// Заполнение представления категории 1СН
			РубрикаСервиса.Вставить("НаименованиеРубрики", ЗначениеСвойстваМоделиСервиса(ПозицияСервиса,
					"product.ref1CnSpecification.categoryName"));
			
			Сервис1СНоменклатура = Новый Структура;
			Сервис1СНоменклатура.Вставить("Рубрика", РубрикаСервиса);
		...
		ИначеЕсли ЗначениеЗаполнено(ЗначениеСвойстваМоделиСервиса(ПозицияСервиса, "product.ref1CnSpecification")) Тогда
			...
			ТоварнаяПозиция.Товар.Вставить("Изображения",
				ЗначениеСвойстваМоделиСервиса(ПозицияСервиса, "product.ref1CnSpecification.imageUrls"));
...
//              * image 			 - Строка	- Изображение товарной позиции в формате base64.
//
...
// Конструктор нового имени файла и расширения.
// 
// Возвращаемое значение:
//  Структура:
// * ИмяБезРасширения - Строка - Имя самого файла без расширения
// * РасширениеБезТочки - Строка - Расширение файла без точки
Функция НовыйИмяФайлаИРасширение()
...
//  Булево - Результат проверки
//
Функция ЭтоФайлКартинки(Знач ПроверяемоеЗначение)
	
	ЭтоФайлКартинки = Ложь;
	
	Если ТипЗнч(ПроверяемоеЗначение) = Тип("Строка") И ЭтоАдресВременногоХранилища(ПроверяемоеЗначение) Тогда
		ПроверяемоеЗначение = ПолучитьИзВременногоХранилища(ПроверяемоеЗначение);
	КонецЕсли;
	
	Если ТипЗнч(ПроверяемоеЗначение) <> Тип("ДвоичныеДанные") Тогда
		Возврат ЭтоФайлКартинки;	
	КонецЕсли;
	
	Картинка = Новый Картинка(ПроверяемоеЗначение);		
...
Функция ДанныеФайлаПоВнешнейСсылке(Знач ВнешнийАдрес, ОтносительныеАдресаКартинок)
	
	Если ОтносительныеАдресаКартинок Тогда
		
		БазовыеПараметрыСоединения = БизнесСетьПовтИсп.ПараметрыСоединения("БизнесСеть"); 
	...
	Возврат ПолучениеФайловИзИнтернета.СкачатьФайлВоВременноеХранилище(ВнешнийАдрес, Неопределено, Истина);
	
КонецФункции

// Конструктор. Количество и размер присоединенных файлов ЗКППоставщика.
// 
// Возвращаемое значение:
//  Структура:
// * КоличествоФайлов - Число - Количество присоединенных
// * Размер - Число - Общий размер присоединенных файлов
Функция НовыйКоличествоИРазмерПрисоединенныхФайловЗКППоставщика()
...
Процедура УдалитьФайлИзображенияРучногоОписания(ФайлИзображения) 
	
	Если Не ЗначениеЗаполнено(ФайлИзображения) Или Не ОбщегоНазначения.СсылкаСуществует(ФайлИзображения) Тогда
		Возврат;
	КонецЕсли;
	
	Попытка
		ФайлИзображенияОбъект = ФайлИзображения.ПолучитьОбъект();
		ФайлИзображенияОбъект.УстановитьПометкуУдаления(Истина);
	Исключение
		ВидОперации = НСтр("ru = 'Установка пометки удаления неактуального изображения товарной позиции'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ЭлектронноеВзаимодействие.ОбработатьОшибку(ВидОперации, ПодробныйТекстОшибки,, "КоммерческиеПредложения");
		ОбщегоНазначения.СообщитьПользователю(КраткийТекстОшибки);
	КонецПопытки;

КонецПроцедуры

Процедура ДобавитьОшибкиВалидацииСтрокиТаблицыИзображений(ТаблицаОшибок, Строка, ТекстОшибок)

	НоваяСтрокаОшибок = ТаблицаОшибок.Добавить();
	НоваяСтрокаОшибок.СтрокаИзображения = Строка;
	НоваяСтрокаОшибок.ОшибкиСтроки = ТекстОшибок;
	
КонецПроцедуры

Процедура ИсключитьЛишниеИзображенияИзТаблицы(РезультатПодбора, МаксимальноеКоличествоИзображений)

	КоличествоПодобранныхИзображений = РезультатПодбора.Количество();
	Если КоличествоПодобранныхИзображений > МаксимальноеКоличествоИзображений Тогда		
		МассивСтрок = Новый Массив;
		Для Счетчик = 0 По МаксимальноеКоличествоИзображений - 1 Цикл
			МассивСтрок.Добавить(РезультатПодбора[Счетчик]);				
		КонецЦикла;
		РезультатПодбора = РезультатПодбора.Скопировать(МассивСтрок);
	КонецЕсли;
	
КонецПроцедуры

// Проверка существования ссылок присоединенных файлов и отсутствия пометки удаления
// 
// Параметры:
//  СсылкиНаИзображения - Массив - Массив ссылок на присоединенные файлы, которые необходимо проверить,
//								   все элементы массива должы быть одного типа.
//   
// Возвращаемое значение:
//  Массив - массив ссылок на присоединенные файлы, прошедших проверку
//
Функция ПроверкаСсылокИзображений(СсылкиНаИзображения)
	ТекстЗапроса = 
		"ВЫБРАТЬ
		|	ТаблицаФайлов.Ссылка КАК Ссылка
		|ИЗ
		|	&ИмяТаблицы КАК ТаблицаФайлов
		|ГДЕ
		|	ТаблицаФайлов.Ссылка В (&Ссылки)
		|	И Не ТаблицаФайлов.ПометкаУдаления";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИмяТаблицы", ОбщегоНазначения.ИмяТаблицыПоСсылке(СсылкиНаИзображения[0]));
	
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапроса;
	Запрос.УстановитьПараметр("Ссылки", СсылкиНаИзображения);
