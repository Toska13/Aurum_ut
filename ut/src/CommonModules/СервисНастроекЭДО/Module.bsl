...
Функция ТипДокументаПоЗначениюСервиса(Значение) Экспорт
	...
	ТипыДокументов.Вставить("СчетОферта", Перечисления.ТипыДокументовЭДО.СчетОферта);
...
//    ** ВерсияПоддерживаемогоРегистрационногоПакета - Строка
//    ** ПоддерживаетсяОбменБезПриглашений - Булево
//
Функция ОбработкаРезультатаКаталогОператоров(ПолученныеДанные, КонтекстДиагностики = Неопределено) Экспорт
	...
	Для Каждого ОператорЭДО Из ПолученныеДанные.operators Цикл
		...
		НоваяСтрока.ВерсияПоддерживаемогоРегистрационногоПакета = ОператорЭДО.regPackageVersion;
		НоваяСтрока.ПоддерживаетсяОбменБезПриглашений = ОператорЭДО.isNoInvitationsEnabled;
	КонецЦикла;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОтправитьДанныеАбонентаВСервисНастроек", Истина);
