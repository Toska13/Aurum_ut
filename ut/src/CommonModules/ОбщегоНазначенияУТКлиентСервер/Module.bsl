...
//
// Параметры:
//	ЧислоДляСклонения - Число - число, определяет склонение.
//	ПараметрыПредметаИсчисления - Строка - параметры предмета исчисления как в методе ЧислоПрописью.
//
// Возвращаемое значение:
//  Строка - слово в правильном склонении.
//
Функция СклонениеСлова(ЧислоДляСклонения, ПараметрыПредметаИсчисления) Экспорт
	
	Результат = "";
	СловоПрописью = ЧислоПрописью(ЧислоДляСклонения, , ПараметрыПредметаИсчисления);
	МассивПараметров = СтрРазделить(ПараметрыПредметаИсчисления, ",", Ложь);
	ДлинаСловоПрописью = СтрДлина(СловоПрописью);
	
	Для каждого Параметр Из МассивПараметров Цикл
		Параметр = СокрЛП(Параметр);
		НачалоПараметра = СтрНайти(СловоПрописью, Параметр);
		Если НачалоПараметра > 0 Тогда
			ДлинаПараметр = СтрДлина(Параметр);
			КонецПараметра = ДлинаПараметр + НачалоПараметра - 1;
			Если СтрНайти(СловоПрописью, " " + Параметр + " ") > 0 
				ИЛИ (НачалоПараметра = 1 И Сред(СловоПрописью, КонецПараметра + 1, 1) = " ")
				ИЛИ ((ДлинаСловоПрописью - ДлинаПараметр) = (НачалоПараметра-1) И Сред(СловоПрописью, НачалоПараметра - 1, 1) = " ") Тогда
				Возврат Параметр;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
// Преобразовывает строку, в которой перечислены имена переменных (колонок / элементов и т.д.) через запятую в массив
// 
// Параметры:
//  СтрокаИмен - Строка - Любая строка с перечислением имен через запятую, которую можно преобразовать в структуру с помощью "Новый Структура(СтрокаИмен);"
// 
// Возвращаемое значение:
// 	Массив из строка
// 
Функция СтрокаИменВМассив(СтрокаИмен) Экспорт
	
	ВозвращаемоеЗначение = Новый Массив;
	
	Для Каждого КлючИЗначение Из Новый Структура(СтрокаИмен) Цикл
...
// Проверяет строку с адресом сайта на корректность введенных данных
//
// Параметры:
//  Адрес - Строка - содержит строку с адресом сайта, которую необходимо проверить.
//  
// Возвращаемое значение:
//   Булево - Ложь, если проверка закончилась успешно, Истина в обратном случае.
//
Функция ПроверитьАдресСайта(Адрес) Экспорт
	Отказ = Ложь;

	// Проверка на строковый тип и отсутствие пустого значения
	Если ТипЗнч(Адрес) <> Тип("Строка") Или ПустаяСтрока(СокрЛП(Адрес)) Тогда
		Отказ = Истина;
	КонецЕсли;
	
	// Парсинг URI через БСП-функцию
	Попытка
	...
	Хост = НРег(СтруктураURI.Хост); 
	
	// Проверка для www. 
	Если СтрНайти(Хост, "www.") = 1 Тогда
		МассивЗначений = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Хост,".");
		Если МассивЗначений.Количество() < 3 Тогда
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
	
	// Проверка две точки подряд
	...
	// Проверка хоста на наличие точки
	Если СтрНайти(Хост, ".") = 0 Тогда
		Отказ = Истина;
	КонецЕсли;
...
// Формирует путь к элементу текущей строки таблицы формы для выдачи сообщений в форме.
// Для совместного использования с процедурой СообщитьПользователю
// (для передачи в параметр "Поле"). 
//
// Параметры:
//  ИмяЭлементаТаблицыФормы - Строка - Имя элемента таблицы, к которой принадлежит строка.
//  ИмяКолонки - Строка - имя реквизита.
//
// Возвращаемое значение:
//  Строка - путь к ячейке таблицы.
//
Функция ПутьКЭлементуТекущейСтрокиТаблицыФормы(ИмяЭлементаТаблицыФормы, ИмяКолонки) Экспорт
	Возврат СтрШаблон(
		"Элементы.%1.ТекущиеДанные.%2",
		ИмяЭлементаТаблицыФормы,
		ИмяКолонки)
КонецФункции
...
Процедура ДобавитьПараметрВыбора(Элемент, ИмяПараметра, ЗначениеПараметра) Экспорт
	...
	Если ТипЗнч(Элемент) = Тип("ПолеНабораДанныхСхемыКомпоновкиДанных") Тогда
		...
		ЗначениеПараметра = ПараметрыВыбора.Значение; // СписокЗначений -
...
// Сортирует значения массива
// 
// Параметры:
//  ИсходныйМассив - Массив Из Произвольный - Исходный массив
//  Направление - НаправлениеСортировки - Направление сортировки
// 
// Возвращаемое значение:
//  Массив Из Произвольный - Массив сортировать
Функция СортированныйМассив(ИсходныйМассив, Направление = Неопределено) Экспорт
...
//  ИзмененныеРеквизиты - Массив из Строка - Список измененных реквизитов.
Процедура ОчиститьНеиспользуемыеРеквизиты(Объект, ПараметрыРеквизитовОбъекта, ИменаТабличныхЧастей = "", ИзмененныеРеквизиты = Неопределено) Экспорт
...
Функция ДанныеПоЧастямПути(Объект, ЧастиПутиКДанным, ИдентификаторСтроки, ЗначениеОтсутствия = Неопределено)
	...
	ИначеЕсли ЭтоФорма И ЧастиПутиКДанным[0] = "Форма" Тогда // "Форма.РеквизитФормы", "Форма.ТаблицаФормы", "Форма.ТаблицаФормы.Колонка"
		ЧастиПутиКДанным.Удалить(0);
	ИначеЕсли Не ЭтоФорма И ЧастиПутиКДанным[0] = "Форма" Тогда
		Возврат ЗначениеОтсутствия;
	КонецЕсли;
		...
		ИначеЕсли ЧастиПутиКДанным.Количество() = 2 Тогда 
			...
			Иначе
				Если ЗначениеЗаполнено(Данные) Тогда
					Попытка
						Данные = ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(Данные, ЧастиПутиКДанным[1]);
					Исключение
						Возврат ЗначениеОтсутствия;
					КонецПопытки;
				Иначе
				...
				КонецЕсли;
			КонецЕсли;
		ИначеЕсли ЧастиПутиКДанным[0] = "Элементы"
			И ЧастиПутиКДанным.Количество() > 2
			И ЧастиПутиКДанным[2] = "ТекущиеДанные" Тогда
			
			ИмяЭлементаТаблицы = ЧастиПутиКДанным[1];
			Данные = Объект.Элементы[ИмяЭлементаТаблицы].ТекущиеДанные;
			
			Если ЧастиПутиКДанным.Количество() = 4 Тогда 
				ИмяРеквизита = ЧастиПутиКДанным[3];
				Данные = Данные[ИмяРеквизита];
			КонецЕсли;
