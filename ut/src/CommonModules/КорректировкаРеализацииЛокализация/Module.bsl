...
Процедура ОбработкаЗаполнения(Объект, ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		...
		ШтрихкодыУпаковок = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения.ДокументОснование, "ШтрихкодыУпаковок");
		Если ШтрихкодыУпаковок <> Неопределено Тогда
			Объект.ШтрихкодыУпаковок.Загрузить(ШтрихкодыУпаковок.Выгрузить());
		КонецЕсли;
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
...
Функция ПолучитьДанныеДляПечатнойФормыУКД(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ЕСТЬNULL(Исправления.ИсходныйДокумент, ДанныеИсходныхДокументов.ИсходныйДокумент) КАК ИсходныйДокумент,
	...
	|	МАКСИМУМ(ДанныеИсходныхДокументов.НомерИсправленияСчетаФактуры) КАК НомерИсправленияСчетаФактуры,
	|	МАКСИМУМ(ДанныеИсходныхДокументов.ДатаИсправленияСчетаФактуры) КАК ДатаИсправленияСчетаФактуры
	|ИЗ ДанныеИсходныхДокументов КАК ДанныеИсходныхДокументов
	|	ЛЕВОЕ СОЕДИНЕНИЕ ДанныеИсходныхДокументов КАК Исправления
	|	ПО ДанныеИсходныхДокументов.Ссылка = Исправления.Ссылка
	|		И ДанныеИсходныхДокументов.НомерСчетаФактуры = Исправления.НомерСчетаФактуры
	|		И ДанныеИсходныхДокументов.ДатаСчетаФактуры = Исправления.ДатаСчетаФактуры
	|СГРУППИРОВАТЬ ПО
	|	ДанныеИсходныхДокументов.Ссылка,
	|	ЕСТЬNULL(Исправления.ИсходныйДокумент, ДанныеИсходныхДокументов.ИсходныйДокумент),
	|	ДанныеИсходныхДокументов.НомерСчетаФактуры,
	|	ДанныеИсходныхДокументов.ДатаСчетаФактуры
	|
	|ИТОГИ ПО
	|	Ссылка
	|;
	...
	|				ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)) В (&ВидПродукции))
...
Процедура ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	Если Не ПараметрыЗаполнения.НужныДанныеПоИсходнымДокументам Тогда
		Запрос.Текст = Запрос.Текст + ОбщегоНазначения.РазделительПакетаЗапросов() + "УНИЧТОЖИТЬ ВтСвязанныеДокументы";
	КонецЕсли;
...
Процедура ПолучитьИсходныеДокументы(МенеджерВременныхТаблиц)
	...
	|	ДанныеОснования.НомерСтроки КАК НомерСтроки,
	|	ДанныеОснования.Номенклатура КАК Номенклатура,
	|	ДанныеОснования.Характеристика КАК Характеристика,
	|	ДанныеОснования.НоменклатураНабора КАК НоменклатураНабора,
	|	ДанныеОснования.ХарактеристикаНабора КАК ХарактеристикаНабора,
	|	ДанныеОснования.Серия КАК Серия,
	|	ДанныеОснования.Назначение КАК Назначение,
	|	ДанныеОснования.Содержание КАК Содержание
	|ПОМЕСТИТЬ ВтДанныеКорректировки
	|ИЗ
	|	Документ.КорректировкаРеализации.Расхождения КАК ДанныеОснования
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		ТаблицаДанныхДокументов КАК ДанныеДокументов
	|	ПО
	|		ДанныеОснования.Ссылка = ДанныеДокументов.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ТаблицаДанныхДокументов.Содержание КАК Содержание
	|
	|ПОМЕСТИТЬ ВтДанныеПоПоследнимИзменениям
	|ИЗ
	|	ВтСвязанныеДокументы КАК ВтПредыдущиеДокументы
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтДанныеКорректировки КАК ТаблицаДанныхДокументов
	|		ПО ВтПредыдущиеДокументы.Основной = ТаблицаДанныхДокументов.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг КАК ШапкаОснования
	|		ПО ШапкаОснования.Ссылка = ВтПредыдущиеДокументы.Подчиненный
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг.Товары КАК ДанныеОснования
	|		ПО ШапкаОснования.Ссылка = ДанныеОснования.Ссылка
	|			И ТаблицаДанныхДокументов.Номенклатура = ДанныеОснования.Номенклатура
	|			И ТаблицаДанныхДокументов.Характеристика = ДанныеОснования.Характеристика
	|			И ТаблицаДанныхДокументов.НоменклатураНабора = ДанныеОснования.НоменклатураНабора
	|			И ТаблицаДанныхДокументов.ХарактеристикаНабора = ДанныеОснования.ХарактеристикаНабора
	|			И ТаблицаДанныхДокументов.Серия = ДанныеОснования.Серия
	|			И ТаблицаДанныхДокументов.Назначение = ДанныеОснования.Назначение
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВтПредыдущиеДокументы.Основной,
	|	ШапкаОснования.Ссылка,
	|	ШапкаОснования.МоментВремени,
	|	ТаблицаДанныхДокументов.Номенклатура КАК Номенклатура,
	|	ТаблицаДанныхДокументов.Характеристика КАК Характеристика,
	|	ТаблицаДанныхДокументов.НоменклатураНабора КАК НоменклатураНабора,
	|	ТаблицаДанныхДокументов.ХарактеристикаНабора КАК ХарактеристикаНабора,
	...
	|		ПО ШапкаОснования.Ссылка = ВтПредыдущиеДокументы.Подчиненный
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РеализацияУслугПрочихАктивов.Доходы КАК ДанныеОснования
	|		ПО ШапкаОснования.Ссылка = ДанныеОснования.Ссылка
	|			И ТаблицаДанныхДокументов.Содержание = ДанныеОснования.Содержание
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		ПО ШапкаОснования.Ссылка = ВтПредыдущиеДокументы.Подчиненный
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.АктВыполненныхРабот.Услуги КАК ДанныеОснования
	|		ПО ШапкаОснования.Ссылка = ДанныеОснования.Ссылка
	|			И ТаблицаДанныхДокументов.Номенклатура = ДанныеОснования.Номенклатура
	|			И ТаблицаДанныхДокументов.Характеристика = ДанныеОснования.Характеристика
	|			И ТаблицаДанныхДокументов.НоменклатураНабора = ДанныеОснования.НоменклатураНабора
	...
	|			
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КорректировкаРеализации.Расхождения КАК ДанныеОснования
	|		ПО ШапкаОснования.Ссылка = ДанныеОснования.Ссылка
	|			И ТаблицаДанныхДокументов.Номенклатура = ДанныеОснования.Номенклатура
	|			И ТаблицаДанныхДокументов.Характеристика = ДанныеОснования.Характеристика
	|			И ТаблицаДанныхДокументов.НоменклатураНабора = ДанныеОснования.НоменклатураНабора
	...
	|	ЕСТЬNULL(ДанныеПоИзменениям.ИсходныйДокумент, ДанныеОснования.ДокументОснование) КАК ИсходныйДокумент,
	|	ЕСТЬNULL(ДанныеПоИзменениям.МоментВремени, ДанныеОснования.ДокументОснование.МоментВремени) КАК МоментВремени,
	|	ТаблицаДанныхДокументов.Номенклатура КАК Номенклатура,
	|	ТаблицаДанныхДокументов.Характеристика КАК Характеристика,
	|	ТаблицаДанныхДокументов.НоменклатураНабора КАК НоменклатураНабора,
	|	ТаблицаДанныхДокументов.ХарактеристикаНабора КАК ХарактеристикаНабора,
	...
	|			И ТаблицаДанныхДокументов.Содержание = ДанныеПоИзменениям.Содержание
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КорректировкаРеализации КАК ДанныеОснования
	|		ПО ТаблицаДанныхДокументов.Ссылка = ДанныеОснования.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДанныеОснования.Основной КАК Основной,
	|	ДанныеОснования.ИсходныйДокумент КАК Предыдущий
	|ПОМЕСТИТЬ ВтДанныеПоИсходнымДокументамПромежуточная
	|ИЗ
	|	ВтДанныеПоПоследнимИзменениямСОснованием КАК ДанныеОснования
	|ГДЕ ДанныеОснования.ИсходныйДокумент В (
	|	ВЫБРАТЬ ПЕРВЫЕ 1
	|		ВременнаяТаблица.ИсходныйДокумент
	|	
	|	ИЗ ВтДанныеПоПоследнимИзменениямСОснованием КАК ВременнаяТаблица
	|	ГДЕ ВременнаяТаблица.Основной = ДанныеОснования.Основной
	|		И ВременнаяТаблица.Номенклатура = ДанныеОснования.Номенклатура
	|		И ВременнаяТаблица.Характеристика = ДанныеОснования.Характеристика
	|		И ВременнаяТаблица.НоменклатураНабора = ДанныеОснования.НоменклатураНабора
	|		И ВременнаяТаблица.ХарактеристикаНабора = ДанныеОснования.ХарактеристикаНабора
	|		И ВременнаяТаблица.Серия = ДанныеОснования.Серия
	|		И ВременнаяТаблица.Назначение = ДанныеОснования.Назначение
	|		И ВременнаяТаблица.Содержание = ДанныеОснования.Содержание
	|	УПОРЯДОЧИТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДанныеПоИсходнымДокументам.Основной КАК Основной,
	|	ЕСТЬNULL(КорректировкаИсправительная.Ссылка, ДанныеПоИсходнымДокументам.Предыдущий) КАК Предыдущий
	|ПОМЕСТИТЬ ВтДанныеПоИсходнымДокументам
	|ИЗ
	|	ВтДанныеПоИсходнымДокументамПромежуточная КАК ДанныеПоИсходнымДокументам
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.КорректировкаРеализации КАК КорректировкаИсправительная
	|		ПО КорректировкаИсправительная.ИсправляемыйДокумент = ДанныеПоИсходнымДокументам.Предыдущий
	|		И КорректировкаИсправительная.Проведен
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ПОМЕСТИТЬ ДанныеДляРасчетаСКорректировкамиПромежуточная
	|ИЗ
	|	ВтДанныеПоИсходнымДокументам КАК ВтДанныеПоИсходнымДокументам
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КорректировкаРеализации КАК ДанныеДокумента
	|		ПО ВтДанныеПоИсходнымДокументам.Предыдущий = ДанныеДокумента.Ссылка
	|			И ДанныеДокумента.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ИсправлениеОшибок)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.КорректировкаРеализации КАК ДанныеДокументаКорректировки
	|		ПО ДанныеДокумента.ДокументОснование = ДанныеДокументаКорректировки.ДокументОснование
	|			И ДанныеДокумента.МоментВремени >= ДанныеДокументаКорректировки.МоментВремени
	...
	|	ДанныеОснования.Ссылка КАК ИсходныйДокумент,
	...
	|
	|ИЗ
	|	ВтДанныеПоИсходнымДокументам КАК ТаблицаДанныхДокументов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг КАК ДанныеОснования
	|		ПО ТаблицаДанныхДокументов.Предыдущий = ДанныеОснования.Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ТаблицаДанныхДокументов.Основной,
	|	ДанныеОснования.Ссылка КАК ИсходныйДокумент,
	...
	|	ТаблицаДанныхДокументов.Основной,
	|	ДанныеОснования.Ссылка КАК ИсходныйДокумент,
	|	ЕСТЬNULL(ДанныеСКорректировкамиИИсправлениями.Номер, ДанныеОснования.Номер) КАК Номер,
	|	ЕСТЬNULL(ДанныеСКорректировкамиИИсправлениями.Дата, ДанныеОснования.Дата) КАК Дата,
	|	ЕСТЬNULL(ДанныеСКорректировкамиИИсправлениями.НомерИсправления, НЕОПРЕДЕЛЕНО) КАК НомерИсправления,
	|	ВЫБОР
	|		КОГДА ДанныеОснования.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ИсправлениеОшибок)
	|			ТОГДА ДанныеОснования.Дата
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ДатаИсправления
	|ИЗ
	|	ВтДанныеПоИсходнымДокументам КАК ТаблицаДанныхДокументов
	...
	|		ПО ТаблицаДанныхДокументов.Предыдущий = ДанныеОснования.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеСКорректировкамиИИсправлениями КАК ДанныеСКорректировкамиИИсправлениями
	|		ПО ТаблицаДанныхДокументов.Предыдущий = ДанныеСКорректировкамиИИсправлениями.Ссылка
	|;
	|/////////////////////////////////////////////////////////////////////////////
...
Функция ПредметыРасчетовПоДокументу(ДокументСсылка, ЧекКоррекцииИсправленииОшибки = Ложь) Экспорт
	...
	РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "ДокументОснование, ЦенаВключаетНДС, Организация, Дата");
	...
	|	СпрНоменклатура.ТипИзмеряемойВеличины В (&МерныеТипыЕдиницИзмерений) КАК МернаяЕдиницаИзмерения
	|ПОМЕСТИТЬ ТаблицаТовары
	|ИЗ
	|	Документ.КорректировкаРеализации.Товары КАК ТабличнаяЧасть
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаНоменклатуры КАК СпрНоменклатура
	|		ПО (ТабличнаяЧасть.Номенклатура = СпрНоменклатура.Номенклатура)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтПодакцизныеТовары КАК ВтПодакцизныеТовары
	|		ПО ТабличнаяЧасть.Номенклатура = ВтПодакцизныеТовары.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО ТабличнаяЧасть.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|ГДЕ
	|	ТабличнаяЧасть.Ссылка = &Ссылка
	|	И (НЕ &ВернутьМногооборотнуюТару
	|			ИЛИ НЕ &ТребуетсяЗалогЗаТару
	...
	ТаблицаТоваровСРаспределениемПоВидамЗапасов(
		Товары,
		ДанныеДляВидовЗапасов(ДокументСсылка, РеквизитыДокумента.ДокументОснование, ПараметрыЗалогаЗаТару));
	
	Если ЧекКоррекцииИсправленииОшибки = Ложь Тогда
		ДанныеДляИСМП = РозничныеПродажиЛокализация.ДанныеДляИСМП(ДокументСсылка, МенеджерВременныхТаблиц);
		ФормированиеФискальныхЧековСерверПереопределяемый.ТаблицаТоваровСРаспределениемКодовМаркировкиИСМП(
			Товары, ДанныеДляИСМП);
	КонецЕсли;
...
Функция ДокументСодержитМаркированныйТовар(ДокументСсылка) Экспорт
	...
	|		ПО КорректировкаРеализацииРасхождения.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	...
	|";
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылка);
	Запрос.УстановитьПараметр("ВидыПродукции", КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииИСМП(Истина));
...
// Определяет, доступен ли механизм отражения данных в прошлых периодах.
// 
// Параметры:
...
// 
// Возвращаемое значение:
//  Булево - Не корректировать налоговую базу прошлого периода
Функция НеКорректироватьНалоговуюБазуПрошлогоПериода(Организация, Дата) Экспорт
	
	НеКорректироватьНалоговуюБазуПрошлогоПериода = Ложь;
	//++ Локализация
	
	
	//-- Локализация
	
	Возврат НеКорректироватьНалоговуюБазуПрошлогоПериода;
	
КонецФункции

// Заполняет значения реквизитов отражения корректировки реализации в учете значениями по умолчанию
// на основании данных учетных политик.
// 
// Параметры:
//	Форма - Структура - коллекция содержащая сведения о документе и включающая следующие поля:
//		* Объект - ДокументОбъект.КорректировкаРеализации - объект документа.
//		* КорректировкаПрошлогоПериода - Булево - различается год документа и его основания.
//		* ДатаОснования - Дата - дата документа-основания.
// 
Процедура ЗаполнитьОтражениеВУчетеПоУмолчанию(Форма) Экспорт
	
	Форма.Объект.ОтразитьВПрибылиУбыткахПрошлыхЛет = Форма.КорректировкаПрошлогоПериода;
...
Процедура ЗаполнитьШтрихкодыУпаковокПоАктуОРасхожденияхПослеОтгрузки(Объект, СтруктураПараметров)
	...
	Если СтруктураПараметров.Свойство("АктОРасхожденияхПослеОтгрузкиОснование") Тогда
		...
		Запрос.УстановитьПараметр(
			"ВариантыДействийПоРасхождениямКодовМаркировкиИСМППризнать",
			Перечисления.ВариантыДействийПоРасхождениямКодовМаркировкиИСМП.Признать);
