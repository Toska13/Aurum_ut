...
//
// Параметры:
//  Объект - ДокументОбъект.ЧекККМ - Обрабатываемый объект
...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если ИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции()
		И Объект.Статус = Перечисления.СтатусыЧековККМ.КОплате Тогда
	...
	УчитываемыеВидыПродукции = ОбщегоНазначенияИСМПВызовСервера.УчитываемыеВидыМаркируемойПродукции();
	...
	Если ИспользуетсяВетеринарныйСертификат
		...
		|////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЧекККМРазрешенияНаПродажу.ИдентификаторСтроки,
		|	ЧекККМРазрешенияНаПродажу.ТипДокумента,
		|	ЧекККМРазрешенияНаПродажу.НомерДокумента,
		|	ЧекККМРазрешенияНаПродажу.ДатаДокумента
		|ПОМЕСТИТЬ ЧекККМРазрешенияНаПродажу
		|ИЗ
		|	&РазрешенияНаПродажу КАК ЧекККМРазрешенияНаПродажу
		|;
		|
		|////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ЧекККМРазрешенияНаПродажу.ДатаДокумента
		|ИЗ
		|	ЧекККМТовары КАК ЧекККМТовары
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СпрНоменклатура
		|		ПО ЧекККМТовары.Номенклатура = СпрНоменклатура.Ссылка
		|			И СпрНоменклатура.ПродаетсяПоРазрешению
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ЧекККМРазрешенияНаПродажу КАК ЧекККМРазрешенияНаПродажу
		|		ПО ЧекККМТовары.ИдентификаторСтроки = ЧекККМРазрешенияНаПродажу.ИдентификаторСтроки";
		Запрос.УстановитьПараметр("Товары", Объект.Товары.Выгрузить());
		Запрос.УстановитьПараметр("РазрешенияНаПродажу", Объект.РазрешенияНаПродажу.Выгрузить());
		...
		Пока Выборка.Следующий() Цикл
			Если ЗначениеЗаполнено(Выборка.ТипДокумента)
				И ЗначениеЗаполнено(Выборка.НомерДокумента)
				И ЗначениеЗаполнено(Выборка.ДатаДокумента) Тогда
				
				Продолжить;
			КонецЕсли;
			
			Текст = НСтр("ru = 'Для ""%Номенклатура%"" в строке %НомерСтроки% ""Товары"" не указано разрешение на продажу.'");
			Текст = СтрЗаменить(Текст, "%НомерСтроки%",  Выборка.НомерСтроки);
			Текст = СтрЗаменить(Текст, "%Номенклатура%", Выборка.Номенклатура);
			
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(
				Ошибки,
				СтрШаблон("Объект.%1[%%1].Номенклатура", "Товары"),
				Текст,
				"",
	...
	Если Не(Объект.Статус = Перечисления.СтатусыЧековККМ.КОплате
		Или Объект.Статус = Перечисления.СтатусыЧековККМ.Пробит) Тогда

		// Отключение проверки незаполненной цены и суммы для табачной продукции в случае, когда включен контроль по МРЦ 
		ДатаДокумента = ?(ЗначениеЗаполнено(Объект.Дата), Объект.Дата, ТекущаяДатаСеанса());
		
		ВидыПродукцииСМРЦ = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииКоторымДоступноПолеМРЦ();
		...
		Если РозничныеПродажиЛокализация.КонтролироватьЦенуПродажиТабачнойПродукции(ДатаДокумента) Тогда
			...
			|	ПО ЧекККМТовары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
			|ГДЕ
			|	ЕСТЬNULL(ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ, ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)) НЕ В (&ВидыПродукцииСМРЦ)
			|	И (ЧекККМТовары.Цена = 0
			|			ИЛИ ЧекККМТовары.Сумма = 0)";
			
			Запрос.УстановитьПараметр("Товары", Объект.Товары.Выгрузить());
			
			Запрос.УстановитьПараметр("ВидыПродукцииСМРЦ", ВидыПродукцииСМРЦ);
			...
			Выборка = РезультатЗапроса.Выбрать();
			Пока Выборка.Следующий() Цикл
				Текст = НСтр("ru = 'Для ""%Номенклатура%"" в строке %НомерСтроки% ""Товары"" не указана цена или сумма.'");
				Текст = СтрЗаменить(Текст, "%НомерСтроки%",  Выборка.НомерСтроки);
				Текст = СтрЗаменить(Текст, "%Номенклатура%", Выборка.Номенклатура);
...
Функция ПредметыРасчетовПоДокументу(ДокументСсылка, ЧекКоррекцииИсправленииОшибки = Ложь) Экспорт
	...
	|	Товары.ИспользованСертификатФСС КАК ИспользованСертификатФСС
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РазрешенияНаПродажу.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	РазрешенияНаПродажу.ТипДокумента КАК ТипДокумента,
	|	РазрешенияНаПродажу.НомерДокумента КАК НомерДокумента,
	|	РазрешенияНаПродажу.ДатаДокумента КАК ДатаДокумента
	|ПОМЕСТИТЬ РазрешенияНаПродажу
	|ИЗ
	|	Документ.ЧекККМ.РазрешенияНаПродажу КАК РазрешенияНаПродажу
	|ГДЕ
	|	РазрешенияНаПродажу.Ссылка = &Ссылка
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ИдентификаторСтроки
	|;
	...
	|	КОНЕЦ КАК Штрихкод,
	|	
	|	РазрешенияНаПродажу.ТипДокумента КАК РазрешениеТипДокумента,
	|	РазрешенияНаПродажу.НомерДокумента КАК РазрешениеНомерДокумента,
	|	РазрешенияНаПродажу.ДатаДокумента КАК РазрешениеДатаДокумента,
	|	
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РазрешенияНаПродажу КАК РазрешенияНаПродажу
	|		ПО    РазрешенияНаПродажу.ИдентификаторСтроки   = Товары.ИдентификаторСтроки
	|
	|;
	|УНИЧТОЖИТЬ Штрихкоды;
	|УНИЧТОЖИТЬ ШтрихкодыНоменклатуры;
	|УНИЧТОЖИТЬ ПриоритетыШтрихКода;
	|УНИЧТОЖИТЬ РазрешенияНаПродажу;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Товары.ИдентификаторСтроки,
	...
	|	Товары.Сумма,
	|	Товары.Штрихкод,
	|	Товары.РазрешениеТипДокумента,
	|	Товары.РазрешениеНомерДокумента,
	|	Товары.РазрешениеДатаДокумента,
	|	Товары.КодВидаНоменклатурнойКлассификации,
	|	Товары.ИспользованСертификатФСС
...
Функция ТоварыФССВДокументеКОплате(ТаблицаТоваров, ЦенаВключаетНДС) Экспорт
	...
	Пока ВыборкаЗапроса.Следующий() Цикл
		...
		ТоварнаяПозиция.Вставить("ИдентификаторСтроки", ВыборкаЗапроса.ИдентификаторСтроки);
...
Функция ОбработатьДанныеШтрихкода(Форма, ДанныеШтрихкода, ПараметрыСканирования, ВложенныеШтрихкоды = Неопределено) Экспорт
	...
	Иначе
		...
		Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияЕГАИС(ДанныеШтрихкода.ВидПродукции)
			И Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(ДанныеШтрихкода.ВидПродукции, Истина) Тогда
		...
		Иначе
			...
			Возврат ОбработатьДанныеШтрихкодаНемаркируемойПродукции(Форма, ДанныеШтрихкода, ПараметрыСканирования);
...
Функция ОбработатьДанныеШтрихкодаПотребительскойУпаковки(Форма, ДанныеШтрихкода, ПараметрыСканирования, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	// Если КМ имеет МРЦ, проверить корректность значения МРЦ на минимальную цену
	Если РозничныеПродажиЛокализация.КонтролироватьЦенуПродажиПоМРЦДляВидаПродукцииИС(ДанныеШтрихкода.ВидПродукции) Тогда
		...
		Если РозничныеПродажиКлиентСерверЛокализация.РозничныеПродажиТабачнойПродукцииТолькоПоМРЦ(Форма) Тогда
			...
			МассивШтрихкодовУпаковок.Добавить(ДанныеШтрихкода.ШтрихкодУпаковки);
			
			РезультатПроверки = РозничныеПродажиЛокализация.ПроверитьШтрихкодыУпаковокНаМинимальнуюЦену(
				Форма,
				МассивШтрихкодовУпаковок,
	...
	МаркируемаяПродукция    = ДанныеШтрихкода.МаркируемаяПродукция;
	ОбработатьБезМаркировки = ДанныеШтрихкода.ЭтоШтрихкодНоменклатуры;
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеШтрихкода, "ОбработатьБезМаркировки") Тогда
		ОбработатьБезМаркировки = ОбработатьБезМаркировки Или ДанныеШтрихкода.ОбработатьБезМаркировки;
	КонецЕсли;
	
	РозничныеПродажиТабачнойПродукцииТолькоПоМРЦ = Ложь;
	Если РозничныеПродажиКлиентСерверЛокализация.РозничныеПродажиТабачнойПродукцииТолькоПоМРЦ(Форма) 
		И КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(ДанныеШтрихкода.ВидПродукции) Тогда
		РозничныеПродажиТабачнойПродукцииТолькоПоМРЦ = Истина;
	КонецЕсли;
	...
	Если РозничныеПродажиТабачнойПродукцииТолькоПоМРЦ Тогда
		ПоляПоиска.Вставить("Цена", РозничныеПродажиКлиентСерверЛокализация.МРЦПоДаннымШтрихкода(ДанныеШтрихкода));
		ПоляПоиска.Вставить(РозничныеПродажиКлиентСерверЛокализация.ИмяРеквизитаУстановленаМРЦ(), Истина);
	КонецЕсли;
	...
	Если Товары.Количество() > 0 Тогда
		...
		Если Не ОбработатьБезМаркировки И ВсегоВКеше < ВсегоПоПараметрамПоиска Тогда
	...
	Иначе
		...
		Если РозничныеПродажиТабачнойПродукцииТолькоПоМРЦ Тогда
			СтрокаТовары.Цена = РозничныеПродажиКлиентСерверЛокализация.МРЦПоДаннымШтрихкода(ДанныеШтрихкода);
			СтрокаТовары[РозничныеПродажиКлиентСерверЛокализация.ИмяРеквизитаУстановленаМРЦ()] = Истина;
		КонецЕсли;
		...
		Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(ДанныеШтрихкода.ВидПродукции) Тогда
			СтруктураДействий.Вставить("ЗаполнитьНоменклатуруЕГАИС",
			ПакетнаяОбработкаТабличнойЧастиКлиентСерверЛокализация.ПараметрыЗаполненияНоменклатурыЕГАИСДляЧека(Форма.Объект));
		КонецЕсли;
	...
	Если РозничныеПродажиТабачнойПродукцииТолькоПоМРЦ Тогда
		// Нужно уменьшить количество в строке с не установленной МРЦ, берется первая попавшаяся строка.
		ПоляПоиска.Удалить("Цена");
		ПоляПоиска[РозничныеПродажиКлиентСерверЛокализация.ИмяРеквизитаУстановленаМРЦ()] = Ложь;
		ПозицииГдеНеУстановленаМРЦ = Объект.Товары.НайтиСтроки(ПоляПоиска);
		Если ПозицииГдеНеУстановленаМРЦ.Количество() Тогда
			СтрокаГдеНеУстановленаМРЦ = ПозицииГдеНеУстановленаМРЦ[0];
			Если ОбновляемаяСтрока.Количество >= СтрокаГдеНеУстановленаМРЦ.Количество Тогда
				Объект.Товары.Удалить(СтрокаГдеНеУстановленаМРЦ);
			Иначе
				СтрокаГдеНеУстановленаМРЦ.Количество = СтрокаГдеНеУстановленаМРЦ.Количество - ОбновляемаяСтрока.Количество;
				РезультатОбработки.ИзмененныеСтроки.Добавить(СтрокаГдеНеУстановленаМРЦ);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ОбработатьДанныеШтрихкодаУпаковкиМаркируемойПродукции(Форма, ДанныеШтрихкода, ПараметрыСканирования, ВложенныеШтрихкоды) Экспорт
	...
	ЗаполнитьТоварыИАкцизныеМаркиМаркируемойпродукции(Форма, ВложенныеШтрихкоды.ДеревоУпаковок, ПараметрыЗаполнения, МассивСтрокДляОбработки, Неопределено);
	
	Если МассивСтрокДляОбработки.Количество() Тогда
		КэшированныеЗначения = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
		...
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			...
			СтруктураДействий,
			Форма.Объект[ПараметрыЗаполнения.ИмяТабличнойЧастиТовары],
			КэшированныеЗначения);
			
		Для Каждого СтрокаТЧТовары Из МассивСтрокДляОбработки Цикл
			
			ДобавитьСериюВТЧСерииВСоответствииСоСтатусомУказанияСерий(Форма.Объект, СтрокаТЧТовары, ПараметрыЗаполнения.ПараметрыУказанияСерий);
...
//  МассивСтрокДляОбработки - Массив Из СтрокаТабличнойЧасти
//  УпаковкаВерхнегоУровня - СправочникСсылка.ШтрихкодыУпаковокТоваров - Упаковка верхнего уровня.
//
Процедура ЗаполнитьТоварыИАкцизныеМаркиМаркируемойПродукции(Форма, ДеревоУпаковок, ПараметрыЗаполнения, МассивСтрокДляОбработки, УпаковкаВерхнегоУровня = Неопределено) Экспорт
	...
	Для Каждого СтрокаДерева Из ДеревоУпаковок.Строки Цикл
		...
		Если СтрокаДерева.ВидПродукции <> Неопределено
			...
			Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(СтрокаДерева.ВидПродукции, Истина) Тогда
			...
			ИначеЕсли КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияЕГАИС(СтрокаДерева.ВидПродукции)
				И Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(СтрокаДерева.ВидПродукции, Истина) Тогда
				...
				Если СтрокиТЧДляУменьшенияКоличества = Неопределено
					...
					МассивСтрокДляОбработки.Добавить(СтрокаТЧТовары);
				...
				ДобавитьСериюВТЧСерииВСоответствииСоСтатусомУказанияСерий(ДокументОбъект, СтрокаТЧТовары, ПараметрыЗаполнения.ПараметрыУказанияСерий);
...
// Обрабатывает штрихкод немаркируемой продукции после выбора номенклатуры в форме уточнения данных.
// Сценарий: включена ФО неуникальных штрихкодов, штрихкод сопоставлен с маркируемой и немаркируемой номенклатурой. После сканирования
// штрихкода появляется окно уточнения данных БГосИС, в котором пользователь выбирает немаркируемую продукцию.
//
// Параметры:
//	Форма - ФормаКлиентскогоПриложения
//	ДанныеШтрихкода - см. ШтрихкодированиеОбщегоНазначенияИС.ИнициализироватьРезультатОбработкиШтрихкода.ДанныеШтрихкода
...
//  см. ШтрихкодированиеОбщегоНазначенияИС.ИнициализироватьРезультатОбработкиШтрихкода
//
Функция ОбработатьДанныеШтрихкодаНемаркируемойПродукции(Форма, ДанныеШтрихкода, ПараметрыСканирования) Экспорт
	...
	ПоляПоиска.Вставить("МаркируемаяПродукция", Ложь);
	...
	Если ЗначениеЗаполнено(ОбновляемаяСтрока.Серия)
		...
		НоменклатураКлиентСервер.ОбновитьКешированныеЗначенияДляУчетаСерий(
			СтрокаТовары, КэшированныеЗначения, ПараметрыУказанияСерий);
		НоменклатураСервер.ЗаполнитьСтатусыУказанияСерийПриОкончанииРедактированияСтрокиТЧ(Объект,
			ПараметрыУказанияСерий, СтрокаТовары.ПолучитьИдентификатор(), Форма.КэшированныеЗначения);
	КонецЕсли;
...
//  СтрокаТЧТовары - СтрокаТабличнойЧасти
//  ПараметрыУказанияСерий - Структура
//
Процедура ДобавитьСериюВТЧСерииВСоответствииСоСтатусомУказанияСерий(ДокументОбъект, СтрокаТЧТовары, ПараметрыУказанияСерий)
	
	Если НоменклатураКлиентСервер.ВЭтомСтатусеСерииУказываютсяВТЧСерии(
		СтрокаТЧТовары.СтатусУказанияСерий, ПараметрыУказанияСерий) Тогда
		
		СтруктураПоискаСерий = Новый Структура;
		СтруктураПоискаСерий.Вставить("Номенклатура",   СтрокаТЧТовары.Номенклатура);
		СтруктураПоискаСерий.Вставить("Характеристика", СтрокаТЧТовары.Характеристика);
		СтруктураПоискаСерий.Вставить("Серия",          СтрокаТЧТовары.Серия);
		...
		Если СтрокиСерии.Количество() = 0 Тогда
			...
			СтрокаТЧСерии.Номенклатура   = СтрокаТЧТовары.Номенклатура;
			СтрокаТЧСерии.Характеристика = СтрокаТЧТовары.Характеристика;
			СтрокаТЧСерии.Серия          = СтрокаТЧТовары.Серия;
...
Функция ОбработатьДанныеШтрихкодаЕдиницыАлкогольнойПродукции(Форма, ДанныеШтрихкода, ПараметрыСканирования) Экспорт
	...
	МассивДобавленныхСтрокДляОбработки = Новый Массив;
	...
	Пока РаспределенноеКоличество < КоличествоПродукции Цикл
		...
		Если СтрокиТЧДляУменьшенияКоличества = Неопределено
			...
			МассивДобавленныхСтрокДляОбработки.Добавить(СтрокаТЧ);
		...
		Иначе
			...
			ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
				МассивДобавленныхСтрокДляОбработки,
				СтруктураДействий,
				Форма.Объект.Товары,
				КэшированныеЗначения);
			
			Для Каждого СтрокаТЧ  Из МассивДобавленныхСтрокДляОбработки Цикл
				Если Не НоменклатураКлиентСервер.ВЭтомСтатусеСерииУказываютсяВТЧТовары(
...
// Заполняет товары и акцизные марки при закрытии формы проверки и подбора, вызывается из 
// см. ПроверкаИПодборПродукцииЕГАИСУТВызовСервера.ПриЗакрытииФормыПроверкиИПодбораВФормеРМК.
// 
// Параметры:
//  ДокументОбъект - ДокументОбъект
//  ДеревоУпаковок - ДеревоЗначений - дерево упаковок.
//  ПараметрыЗаполнения - Структура
//
Процедура ЗаполнитьТоварыИАкцизныеМаркиПриЗакрытииФормыПроверкиИПодбора(ДокументОбъект, ДеревоУпаковок, ПараметрыЗаполнения) Экспорт
	
	МассивСтрокДляОбработки = Новый Массив;
	ЗаполнитьТоварыИАкцизныеМарки(ДокументОбъект, ДеревоУпаковок, ПараметрыЗаполнения, МассивСтрокДляОбработки, Неопределено);
	Если МассивСтрокДляОбработки.Количество() Тогда
		КэшированныеЗначения = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
		ПроверитьСериюРассчитатьСтатус = Новый Структура;
		...
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			МассивСтрокДляОбработки,
			СтруктураДействий,
			ДокументОбъект[ПараметрыЗаполнения.ИмяТабличнойЧастиТовары],
			КэшированныеЗначения);
...
//  ДокументОбъект - ДокументОбъект - 
//  ДеревоУпаковок - ДеревоЗначений - дерево упаковок.
//  ПараметрыЗаполнения - Структура - таблица штрихкодов упаковок и их хеш-сумм.
//  МассивСтрокДляОбработки - Массив Из СтрокаТабличнойЧасти
//  УпаковкаВерхнегоУровня - СправочникСсылка.ШтрихкодыУпаковокТоваров - Упаковка верхнего уровня.
//
Процедура ЗаполнитьТоварыИАкцизныеМарки(ДокументОбъект, ДеревоУпаковок, ПараметрыЗаполнения, МассивСтрокДляОбработки, УпаковкаВерхнегоУровня = Неопределено) Экспорт
