...
Функция ПолучитьДанныеДляПечатнойФормыМХ3(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ВЫБОР
	|		КОГДА Товары.Порча
	|			ТОГДА Товары.ХарактеристикаОприходование
	...
	СтруктураДанныхДляПечати.Вставить("РезультатПоТабличнойЧасти", МассивРезультатов[4]);
...
Функция ПолучитьДанныеДляПечатнойФормыМ4(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	КОНЕЦ КАК Цена,
	|	ЕСТЬNULL(СуммыДокументовВВалютахУчета.СуммаБезНДСРегл, ТаблицаТовары.Сумма) КАК Сумма
	|ПОМЕСТИТЬ ВтТовары
	|ИЗ
	...
	|	ТаблицаТовары.Склад.Наименование КАК СкладНаименование,
	...
	Результат.Вставить("РезультатПоШапке", МассивРезультатов[МассивРезультатов.ВГраница() - 2]);
