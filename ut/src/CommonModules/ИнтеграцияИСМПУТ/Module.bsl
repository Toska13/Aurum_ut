...
Процедура ОпределитьИспользованиеАктовОРасхожденииПослеПриемки(Документ, Используются) Экспорт
	...
	Используются = ПолучитьФункциональнуюОпцию("ИспользоватьАктыРасхожденийПослеПриемки")
		И ТипЗнч(Документ) <> Тип("ДокументСсылка.ПоступлениеТоваровОтКлиента");
...
Процедура ОпределитьСкладДокументаОснования(Склад, ДокументОснование) Экспорт
	...
	Если МетаданныеОснования = Метаданные.Документы.ЗаказПоставщику Тогда
		Возврат;
	КонецЕсли;
...
Функция ТекстЗапросаОпределениеСклада(ПроверяемыйДокумент) Экспорт
	...
	Если ТипЗнч(ПроверяемыйДокумент) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
		...
		|ГДЕ
		|	ПриобретениеТоваровУслугТовары.Ссылка = &ПроверяемыйДокумент
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ПриобретениеТоваровУслуг.Ссылка = &ПроверяемыйДокумент";
	...
	ИначеЕсли ТипЗнч(ПроверяемыйДокумент) = Тип("ДокументСсылка.ПриемкаТоваровНаХранение") Тогда
		...
		|ГДЕ
		|	ПриемкаТоваровНаХранениеТовары.Ссылка = &ПроверяемыйДокумент
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|ГДЕ
		|	ПриемкаТоваровНаХранение.Ссылка = &ПроверяемыйДокумент";
	...
	ИначеЕсли ТипЗнч(ПроверяемыйДокумент) = Тип("ДокументСсылка.ПоступлениеТоваровОтХранителя") Тогда
		...
		|ГДЕ
		|	ПоступлениеТоваровОтХранителя.Ссылка = &ПроверяемыйДокумент";
		
	ИначеЕсли ТипЗнч(ПроверяемыйДокумент) = Тип("ДокументСсылка.ПоступлениеТоваровОтКлиента") Тогда
		
		Возврат "ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ПоступлениеТоваровОтКлиентаТовары.Склад КАК Склад
		|ИЗ
		|	Документ.ПоступлениеТоваровОтКлиента.Товары КАК ПоступлениеТоваровОтКлиентаТовары
		|ГДЕ
		|	ПоступлениеТоваровОтКлиентаТовары.Ссылка = &ПроверяемыйДокумент
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ПоступлениеТоваровОтКлиента.Склад КАК Склад
		|ИЗ
		|	Документ.ПоступлениеТоваровОтКлиента КАК ПоступлениеТоваровОтКлиента
		|ГДЕ
		|	ПоступлениеТоваровОтКлиента.Ссылка = &ПроверяемыйДокумент";
...
// 
// Параметры:
//  ПараметрыОкончанияПроверки - Структура - (См. ПроверкаИПодборПродукцииИС.ЗафиксироватьРезультатПроверкиИПодбора)
Процедура ОтразитьРезультатыСканированияВДокументе(ПараметрыОкончанияПроверки) Экспорт
	...
	Если ТипПроверяемогоДокумента = Тип("ДокументСсылка.ПриобретениеТоваровУслуг")
		...
		Или ТипПроверяемогоДокумента = Тип("ДокументСсылка.ПриемкаТоваровНаХранение")
		Или ТипПроверяемогоДокумента = Тип("ДокументСсылка.ПоступлениеТоваровОтХранителя")
		Или ТипПроверяемогоДокумента = Тип("ДокументСсылка.ПоступлениеТоваровОтКлиента")
	Тогда
	...
	ИначеЕсли ТипПроверяемогоДокумента = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		...
		Или ТипПроверяемогоДокумента = Тип("ДокументСсылка.ОтгрузкаТоваровСХранения")
		Или ТипПроверяемогоДокумента = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту")
	Тогда
...
//
Функция ПриОпределенииТаблицыПродукцииПринятойБезРасхождений(ДокументОснование, НоменклатураПоДаннымЭДО) Экспорт
...
Процедура ПриЗаполненииЗначенияШтрихкодовПродукцииПоДокументу(ДокументСсылка, ТаблицаТоваровЗначенийШтрихкодов) Экспорт
	...
	Если ТипПроверямогоДокумента = Тип("ДокументСсылка.АктОРасхожденияхПослеПриемки") Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		               |		ПО АктОРасхожденияхШтрихкодыУпаковок.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
		               |			И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
		               |ГДЕ
...
Функция ВидыПродукцииДокумента(ДокументСсылка) Экспорт
	...
	Запрос.УстановитьПараметр("ВидыПродукцииГИСМТ", КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииИСМП(Истина));
...
//  ТаблицаМаркируемойПродукции - ТаблицаЗначений - таблица маркируемой продукции документа. (См. ЭлектронноеВзаимодействиеИСМП.ТаблицаМаркируемойПродукцииДокумента)
//
Процедура ЗаполнитьМаркируемуюПродукциюДокумента(ДокументСсылка, ТаблицаМаркируемойПродукции) Экспорт
	...
	Если ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(ДокументСсылка, "КорректировкаПриобретения")
		...
		Или ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(ДокументСсылка, "РеализацияТоваровУслуг")
		Или ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(ДокументСсылка, "ПередачаТоваровХранителю")
		Или ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(ДокументСсылка, "ОтгрузкаТоваровСХранения")
		Или ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(ДокументСсылка, "ОтгрузкаТоваровКлиенту")
		Или ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(ДокументСсылка, "ВозвратТоваровПоставщику")
		Или ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(ДокументСсылка, "ПередачаТоваровМеждуОрганизациями")
		Или ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(ДокументСсылка, "ВозвратТоваровМеждуОрганизациями")
		Или ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(ДокументСсылка, "КорректировкаРеализации")
		Или ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(ДокументСсылка, "ПриемкаТоваровНаХранение")
		Или ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(ДокументСсылка, "ПоступлениеТоваровОтХранителя")
	Тогда
...
Процедура СформироватьТаблицуМаркированныхТоваровОснований(Запрос, ИсточникОснований) Экспорт
	...
	|		ПО ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ = ЗначенияТоварныеГруппы.ВидПродукции
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ТоварныеГруппыГИСМТ КАК ТоварныеГруппы
	|		ПО ТоварныеГруппы.Ссылка = ЗначенияТоварныеГруппы.ТоварнаяГруппа
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ШаблоныКодовМаркировкиСУЗ КАК ШаблоныКМ
	|		ПО ШаблоныКМ.ТоварнаяГруппа = ЗначенияТоварныеГруппы.ТоварнаяГруппа
	|		И ШаблоныКМ.ИспользоватьПоУмолчанию
	|		И НЕ ШаблоныКМ.Архивный
	|ГДЕ
	|	НЕ Товары.Отменено
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО Заказ.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		(ВЫБРАТЬ РАЗЛИЧНЫЕ ЗначенияЭлементовКонструктора.ВидПродукции КАК ВидПродукции,
	|			ЗначенияЭлементовКонструктора.Значение КАК ТоварнаяГруппа
	|		ИЗ РегистрСведений.ЗначенияЭлементовКонструктораВидовПродукцииИС КАК ЗначенияЭлементовКонструктора
	...
	|	ТоварыПредварительно.ТоварнаяГруппа     КАК ТоварнаяГруппа,
	...
	|	ТоварыПредварительно.ТоварнаяГруппа,
...
Функция ТаблицаТоварыДляЭДО(ИсходнаяТаблица, ИмяКолонкиКоличество) Экспорт
	...
	Товары.Колонки.Добавить("ЕдиницаИзмеренияКод", ОбщегоНазначения.ОписаниеТипаСтрока(5));
...
//   ВидыМаркируемойПродукции - СправочникСсылка.ВидыПродукцииИС, Массив Из СправочникСсылка.ВидыПродукцииИС - 
...
Процедура СформироватьТаблицуМаркируемойПродукцииДокумента(Документ, ТаблицаПродукции, ВидыМаркируемойПродукции, КакДокументаОснования = Ложь) Экспорт
	...
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.СборкаТоваров") Тогда
		...
		|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ КАК ВидПродукцииГИСМТ,
		|	СУММА(ТаблицаТовары.Количество) КАК Количество
		|ИЗ
		|	ТаблицаТоварыТовары КАК ТаблицаТовары
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		|		ПО ОписаниеНоменклатурыИС.Номенклатура = ТаблицаТовары.Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК Наборы
		|		ПО Наборы.Номенклатура = ТаблицаТовары.Номенклатура.НаборУпаковок
		|ГДЕ
		...
		|	ТаблицаТовары.Серия,
		|	ОписаниеНоменклатурыИС.КлючВидовПродукции,
		|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ
		|ИМЕЮЩИЕ
	...
	Запрос.УстановитьПараметр("ВидыПродукцииСырье",      КонструкторИСМПКлиентСервер.ВидыПродукцииСырьеГИСМТ());
...
Процедура УстановитьУсловиеПоВидуМаркируемойПродукции(ТекстЗапроса,
	...
	Если ТипЗнч(ВидМаркируемойПродукции) = Тип("СправочникСсылка.ВидыПродукцииИС")
		Или ТипЗнч(ВидМаркируемойПродукции) = Тип("Массив")
		Или ТипЗнч(ВидМаркируемойПродукции) = Тип("ФиксированныйМассив") Тогда 
		УсловиеМаркируемаяПродукция = "ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидМаркируемойПродукции)
		|	И НЕ ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииСырье)
		|	Или ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЕГАИС В (&ВидМаркируемойПродукции)";
	Иначе
		УсловиеМаркируемаяПродукция = "ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
			|	И НЕ ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииСырье)";
	КонецЕсли;
...
Процедура УстановитьУсловиеПоНефасованнойПродукции(ТекстЗапроса, ИмяТаблицыТовары = "Товары", КакДокументаОснования = Ложь) Экспорт
	...
	Если КакДокументаОснования Тогда
		ТекстЗамены =
			"ЕСТЬNULL(Наборы.КоличествоВПотребительскойУпаковке,ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке) <> 0";
	Иначе
		ТекстЗамены =
			"ЕСТЬNULL(Наборы.КоличествоВПотребительскойУпаковке,ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке) = 0";
	КонецЕсли;
...
Процедура ПроверкаПриФормированииУПД_5_02_НаГруппуСкладов(Склад, УчастникОбмена, Источник, Отказ, ВыводитьСообщение = Истина) Экспорт
	...
	ЭтоГруппаСкладов = ТипЗнч(Склад) = Тип("СправочникСсылка.Склады")
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Склад, "ЭтоГруппа") = Истина;
...
Процедура ОбработкаЗаполненияДокументаВыводИзОборотаИСМП(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда
		ЗаполнитьВыводИзОборотаИСМПНаОснованииОтгрузкиТоваровКлиенту(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
...
Процедура ОбработкаЗаполненияДокументаМаркировкаТоваровИСМП(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументОбъект.ТоварнаяГруппа)
		И ДокументОбъект.ОперацияНанесенияТолькоДляНаборов
		И Не МаркировкаТоваровКлиентСервер.ДоступенОтчетОНанесении(ДокументОбъект.ТоварнаяГруппа) Тогда
...
Процедура ОбработкаЗаполненияДокументаВозвратВОборотИСМП(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ПоступлениеТоваровОтКлиента") Тогда
		ЗаполнитьВозвратВОборотИСМПНаОснованииПоступленияТоваровОтКлиента(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
...
Процедура ОбработкаЗаполненияДокументаПеремаркировкаТоваровИСМП(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ПоступлениеТоваровОтКлиента") Тогда
		ЗаполнитьПеремаркировкуИСМПНаОснованииПоступленияТоваровОтКлиента(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ОтчетОРозничныхВозвратах") Тогда
...
Процедура ОбработкаЗаполненияДокументаОтгрузкаТоваровИСМП(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда
		ЗаполнитьОтгрузкуТоваровИСМПНаОснованииОтгрузкиТоваровКлиенту(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ВозвратТоваровПоставщику") Тогда
...
Процедура ОбработкаЗаполненияДокументаРазрешениеНаОтгрузкуИСМП(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда
		ЗаполнитьРазрешениеНаОтгрузкуИСМПНаОснованииОтгрузкиТоваровКлиенту(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ВозвратТоваровПоставщику") Тогда
...
Функция ПриОпределенииСопоставленногоКлассификатораТНВЭД(КодТНВЭД, Наименование) Экспорт
	...
	ЭлементСправочника = РаботаСНоменклатуройУТ.НайтиСоздатьЭлементКлассификатораТНВЭД(КодТНВЭД);
...
// Возвращает сокращенное строковое представление меры количества предмета расчета ККТ.
//
// Параметры:
//   МераКоличестваПредметаРасчета - ПеречислениеСсылка.МераКоличестваПредметаРасчетаККТ - мера количества предмета расчета
//
// Возвращаемое значение:
//   Строка - сокращенное представление меры количества
Функция СокращеноеПредставлениеМерыКоличестваПредметаРасчета(МераКоличестваПредметаРасчета) Экспорт
	
	Если МераКоличестваПредметаРасчета = Перечисления.МераКоличестваПредметаРасчетаККТ.Литр Тогда
		Возврат "л.";
	ИначеЕсли МераКоличестваПредметаРасчета = Перечисления.МераКоличестваПредметаРасчетаККТ.Миллилитр Тогда
		Возврат "мл.";
	ИначеЕсли МераКоличестваПредметаРасчета = Перечисления.МераКоличестваПредметаРасчетаККТ.Штука Тогда
		Возврат "шт.";
	ИначеЕсли МераКоличестваПредметаРасчета = Перечисления.МераКоличестваПредметаРасчетаККТ.Килограмм Тогда
		Возврат "кг.";
	ИначеЕсли МераКоличестваПредметаРасчета = Перечисления.МераКоличестваПредметаРасчетаККТ.Грамм Тогда
		Возврат "гр.";
	Иначе
		Возврат НРег(МераКоличестваПредметаРасчета);
	КонецЕсли;
...
Функция ПарамерыУказанияСерийПеремещениеМеждуМОДИСМП(Объект) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.ОтправительОбъект) Тогда
		ПараметрыУказанияСерий.ИмяПоляСклад            = "ОтправительОбъект";
	ИначеЕсли ЗначениеЗаполнено(Объект.Отправитель) Тогда
		ПараметрыУказанияСерий.ИмяПоляСклад            = "Отправитель";
	Иначе
		ПараметрыУказанияСерий.ИмяПоляСклад = "";
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПараметрыУказанияСерий.ИмяПоляСклад) И ЗначениеЗаполнено(Объект[ПараметрыУказанияСерий.ИмяПоляСклад]) Тогда
...
Процедура ДополнитьПараметрыУказанияСерийПроверкаИПодборПродукцииИСМП(ПараметрыУказанияСерий, Форма) Экспорт
	...
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	НовыйРеквизит = Новый РеквизитФормы("ОтправительОбъект", Новый ОписаниеТипов("СправочникСсылка.Склады"));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	НовыйРеквизит = Новый РеквизитФормы("ПолучательОбъект", Новый ОписаниеТипов("СправочникСсылка.Склады"));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	НовыйРеквизит = Новый РеквизитФормы("ДвижениеРанееПоступившейПартии", Новый ОписаниеТипов("Булево"));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	...
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	НовыйРеквизит = Новый РеквизитФормы("КонтролируемаяПартияОтправителя", Новый ОписаниеТипов("СправочникСсылка.КонтролируемыеПартии"), "ПодобраннаяМаркируемаяПродукция");
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	...
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	НовыйРеквизит = Новый РеквизитФормы("СтатусУказанияСерийОтгруженныхТоваров", Новый ОписаниеТипов("Число"), "ПодобраннаяМаркируемаяПродукция");
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
...
Процедура ДополнитьПараметрыУказанияСерийПроизводстваПроверкаИПодборПродукцииИСМП(ПараметрыУказанияСерий, Форма) Экспорт
	
	Если Форма.Сценарий = 0 Тогда
		ПараметрыУказанияСерий = ПараметрыУказанияСерий.ВыходныеИзделия;
	ИначеЕсли ПараметрыУказанияСерий.Свойство("МатериалыИРаботы") Тогда
		ПараметрыУказанияСерий = ПараметрыУказанияСерий.МатериалыИРаботы;
	ИначеЕсли ПараметрыУказанияСерий.Свойство("РасходМатериаловИРабот") Тогда
		ПараметрыУказанияСерий = ПараметрыУказанияСерий.РасходМатериаловИРабот;
	КонецЕсли;
...
Процедура ОтразитьРезультатыПроверкиИПодбораВДокументеПоступления(ПараметрыОкончанияПроверки)
	...
	ИзменитьДокументПоступления = Не ПараметрыОкончанияПроверки.СоздаватьАктОРасхождениях;
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПоступлениеТоваровОтКлиента") Тогда
		ИзменитьДокументПоступления = Не ЭлектронноеВзаимодействиеИСМП.ДокументСвязанСЭлектронным(ПараметрыОкончанияПроверки.ПроверяемыйДокумент);
	КонецЕсли;
	
	АктОРасхождениях = СформированныйАктОРасхождениях(ПараметрыОкончанияПроверки.ПроверяемыйДокумент);

	Если ИзменитьДокументПоступления
		И Не ЗначениеЗаполнено(АктОРасхождениях) Тогда
	...
	Если ПараметрыОкончанияПроверки.СоздаватьАктОРасхождениях Или АктОРасхождениях <> Неопределено Тогда
...
Процедура ЗаполнитьСерииВДокументе(ПараметрыОкончанияСканирования, ДокументОбъект)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаПодобраннойПродукции КАК ТаблицаПодобраннойПродукции
	|		ПО ТабличнаяЧастьСерии.Номенклатура = ТаблицаПодобраннойПродукции.Номенклатура
	|			И ТабличнаяЧастьСерии.Характеристика = ТаблицаПодобраннойПродукции.Характеристика
	|ГДЕ
	|	ТабличнаяЧастьСерии.Ссылка = &ПроверяемыйДокумент
	|	И ТаблицаПодобраннойПродукции.Номенклатура ЕСТЬ NULL";
	...
	ИменяПолейРазбиенияСумм = "Сумма,СуммаНДС,СуммаСНДС,СуммаВзаиморасчетов,СуммаНДСВзаиморасчетов,СуммаРучнойСкидки";
	ПоляСумм = Новый Структура(ИменяПолейРазбиенияСумм);
				...
				Пока ТекущаяСтрока <= МаксимальнаяСтрока И Распределить > 0 Цикл
					...
					Если СерияВСтрокеТовары Тогда
						Если ИспользованоКоличествоПоТекущейСтроке < СтрокаДополнения.Количество
							...
							ЗаполнитьЗначенияСвойств(ПоляСумм, СтрокаДополнения);
							...
							Для Каждого КлючИЗначение Из ПоляСумм Цикл
								Если КлючИЗначение.Значение <> Неопределено Тогда
									Сумма = КлючИЗначение.Значение * Коэффициент;
									СтрокаДополнения[КлючИЗначение.Ключ] = Сумма;
									НоваяСтрокаТовары[КлючИЗначение.Ключ] = НоваяСтрокаТовары[КлючИЗначение.Ключ] - СтрокаДополнения[КлючИЗначение.Ключ];
								КонецЕсли;
							КонецЦикла;
					...
					Если СерияВСтрокеТовары Тогда
						Если МаксимальнаяСтрока >= 0 Тогда
							...
							Для Каждого КлючИЗначение Из ПоляСумм Цикл
								Если КлючИЗначение.Значение <> Неопределено Тогда
									Сумма = КлючИЗначение.Значение * Коэффициент;
									НоваяСтрокаТовары[КлючИЗначение.Ключ] = Сумма;
								КонецЕсли;
							КонецЦикла;
...
Процедура ОтразитьИзменениеКоличестваВТабличныхЧастяхТоварыСерииПрочегоОприходованияТоваров(ДокументОбъект, ПараметрыОкончанияПроверки)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО ОписаниеНоменклатурыИС.Номенклатура = Товары.Номенклатура
	|		И (ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
	|			ИЛИ ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЕГАИС <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка))
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК Наборы
	|			ПО Наборы.Номенклатура = Товары.Номенклатура.НаборУпаковок
	|ГДЕ
	|	ЕСТЬNULL(Наборы.КоличествоВПотребительскойУпаковке, ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке) = 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
...
Процедура ОтразитьРезультатыПроверкиИПодбораВДокументеИнтеркампани(ПараметрыОкончанияПроверки, ТипОбъекта)
	...
	ТаблицаПризнаков = Новый ТаблицаЗначений();
	ТаблицаПризнаков.Колонки.Добавить("ВидПродукции",                                          Новый ОписаниеТипов("СправочникСсылка.КлючиВидовПродукцииИС"));
	ТаблицаПризнаков.Колонки.Добавить("УменьшатьКоличествоТовараПоКоличествуПодобранныхМарок", Новый ОписаниеТипов("Булево"));
	
	Для Каждого СтрокаДанныхКонтроля Из ПараметрыОкончанияПроверки.ДанныеКонтроляВидовПродукцииПоРазделеннымДатам Цикл
		
		НоваяСтрока = ТаблицаПризнаков.Добавить();
		НоваяСтрока.ВидПродукции = СтрокаДанныхКонтроля.Ключ;
		НоваяСтрока.УменьшатьКоличествоТовараПоКоличествуПодобранныхМарок = СтрокаДанныхКонтроля.Значение.УменьшатьКоличествоТовараПоКоличествуПодобранныхМарок;
	...
	Запрос.Текст = "ВЫБРАТЬ
	|	ТаблицаПризнаков.ВидПродукции,
	|	ТаблицаПризнаков.УменьшатьКоличествоТовараПоКоличествуПодобранныхМарок
	|ПОМЕСТИТЬ ТаблицаПризнаков
	|ИЗ
	...
	|	Подобрано.Характеристика,
	|	Подобрано.Серия,
	|	ЕСТЬNULL(ТаблицаПризнаков.УменьшатьКоличествоТовараПоКоличествуПодобранныхМарок, ЛОЖЬ) КАК УменьшатьКоличествоТовараПоКоличествуПодобранныхМарок,
	...
	|		ПО ОписаниеНоменклатурыИС.Номенклатура = Подобрано.Номенклатура
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаПризнаков КАК ТаблицаПризнаков
	|		ПО ТаблицаПризнаков.ВидПродукции = ОписаниеНоменклатурыИС.КлючВидовПродукции
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////4
	|ВЫБРАТЬ
	...
	|	Товары.Характеристика,
	|	Товары.Серия,
	|	Товары.УменьшатьКоличествоТовараПоКоличествуПодобранныхМарок,
	|	-Товары.Количество
	|ИЗ
	|	Подобрано КАК Товары
	|;
	...
	|ИЗ
	|	Итог КАК Итог
	|ГДЕ Итог.УменьшатьКоличествоТовараПоКоличествуПодобранныхМарок
	|	ИЛИ Итог.Недостача < 0
	|УПОРЯДОЧИТЬ ПО
	|	Недостача Убыв";
...
Процедура ОтразитьРезультатыПроверкиИПодбораВИсходящемДокументе(ПараметрыОкончанияПроверки, ТипОбъекта)
	...
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда
		ОтразитьИзменениеКоличестваВТабличныхЧастяхТоварыСерииОтгрузкаТоваровКлиенту(ДокументОбъект, ПараметрыОкончанияПроверки);
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.ОтгрузкаТоваровСХранения") Тогда
		ОтразитьИзменениеКоличестваВТабличныхЧастяхТоварыСерииОтгрузкаТоваровСХранения(ДокументОбъект, ПараметрыОкончанияПроверки);
...
Процедура ОтразитьИзменениеКоличестваВТабличныхЧастяхТоварыСерииРеализацииТоваровУслуг(ДокументОбъект, ПараметрыОкончанияПроверки)
	...
	ТаблицаПризнаков.Колонки.Добавить("ВидПродукции",                                          Новый ОписаниеТипов("СправочникСсылка.КлючиВидовПродукцииИС"));
	ТаблицаПризнаков.Колонки.Добавить("УменьшатьКоличествоТовараПоКоличествуПодобранныхМарок", Новый ОписаниеТипов("Булево"));
	ТаблицаПризнаков.Колонки.Добавить("ДоступноПолеМРЦ",                                       Новый ОписаниеТипов("Булево"));
	...
	Для Каждого СтрокаДанныхКонтроля Из ПараметрыОкончанияПроверки.ДанныеКонтроляВидовПродукцииПоРазделеннымДатам Цикл
		...
		НоваяСтрока.ВидПродукции = СтрокаДанныхКонтроля.Ключ;
		НоваяСтрока.УменьшатьКоличествоТовараПоКоличествуПодобранныхМарок = СтрокаДанныхКонтроля.Значение.УменьшатьКоличествоТовараПоКоличествуПодобранныхМарок;
		НоваяСтрока.ДоступноПолеМРЦ = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(НоваяСтрока.ВидПродукции);
	...
	|	ТаблицаПризнаков.ВидПродукции,
	|	ТаблицаПризнаков.УменьшатьКоличествоТовараПоКоличествуПодобранныхМарок,
	|	ТаблицаПризнаков.ДоступноПолеМРЦ
	|ПОМЕСТИТЬ ТаблицаПризнаков
	|ИЗ
	|	&ТаблицаПризнаков КАК ТаблицаПризнаков
	|;
	...
	|		ПО ОписаниеНоменклатурыИС.Номенклатура = Товары.Номенклатура
	|ГДЕ
	|	Не ЕСТЬNULL(ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ, ЗНАЧЕНИЕ(Справочник.КлючиВидовПродукцииИС.ПустаяСсылка)) В (&ВидыПродукцииСырье)
	|
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////3
	...
	|	Подобрано.Серия,
	|	ЕСТЬNULL(ТаблицаПризнаков.УменьшатьКоличествоТовараПоКоличествуПодобранныхМарок, ЛОЖЬ) КАК УменьшатьКоличествоТовараПоКоличествуПодобранныхМарок,
	|	ЕСТЬNULL(ТаблицаПризнаков.ДоступноПолеМРЦ, ЛОЖЬ) КАК ДоступноПолеМРЦ,
	|	Подобрано.Количество КАК Количество
	|ПОМЕСТИТЬ Подобрано
	|ИЗ
	|	ПодобраноПредварительно КАК Подобрано
	...
	ЕстьТоварыУчитываемыеПоМРЦ = Ложь;
	
	Если ВыборкаРасхождения.Количество() > 0 Тогда
	
		УпаковкиТоваров = Пакет[Количество-2].Выгрузить();
		УпаковкиТоваров.Индексы.Добавить("Номенклатура,Характеристика");
		...
		Если ВыборкаРасхождения.ДоступноПолеМРЦ Тогда
			ЕстьТоварыУчитываемыеПоМРЦ = Истина;
		КонецЕсли;
		
	КонецЕсли;
	
	// Приведение цены товаров к МРЦ
	Если Не ЕстьТоварыУчитываемыеПоМРЦ Тогда
		Возврат;
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДокументОбъект, "Контрагент") Тогда
		ТекЮрФизЛицо = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект.Контрагент, "ЮрФизЛицо");
	...
	Если РозничныеПродажиЛокализация.КонтролироватьЦенуПродажиТабачнойПродукции(ДатаДокумента) Тогда
		РозничныеПродажиЛокализация.ПривестиЦеныВДокументеКМРЦ(
			ДокументОбъект,
			ПараметрыОкончанияПроверки.ТаблицаПодобраннойПровереннойПродукции);
	КонецЕсли;
...
Процедура ОтразитьИзменениеКоличестваВТабличнойЧастиТоварыКорректировкиРеализации(ДокументОбъект, ПараметрыОкончанияПроверки)
	...
	НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(ДокументОбъект, ПараметрыУказанияСерий.Товары);
...
// Переносит маркируемую продукцию из формы проверки и подбора в документ Отгрузка товаров клиенту
//   * Удаляет из табличных частей Товары, Серии отсутствующую в данных проверки маркируемую продукцию.
//   * Не меняет прочие товарные строки.
// 
// Параметры:
//  ДокументОбъект           - ДокументОбъект.ОтгрузкаТоваровКлиенту - документ к изменению
//  ПараметрыОкончанияПроверки - Структура - (См. ПроверкаИПодборПродукцииИС.ЗафиксироватьРезультатПроверкиИПодбора).
//
Процедура ОтразитьИзменениеКоличестваВТабличныхЧастяхТоварыСерииОтгрузкаТоваровКлиенту(ДокументОбъект, ПараметрыОкончанияПроверки)
	...
	СтруктураЗаполнения = СтруктураЗаполнения(
		СтруктураДействийПриДобавленииНовойСтрокиОтгрузкаТоваровКлиенту(ДокументОбъект),
		СтруктураДействийПриИзмененииСтрокиОтгрузкаТоваровКлиенту(ДокументОбъект),
		КлючПоиска,
		КлючСтрокиСерий);
	
	ПараметрыУказанияСерий = НоменклатураСервер.ПараметрыУказанияСерий(ДокументОбъект, Документы.ОтгрузкаТоваровКлиенту);
...
// Переносит маркируемую продукцию из формы проверки и подбора в документ Отгрузка товаров с хранения
//   * Удаляет из табличных частей Товары, Серии отсутствующую в данных проверки маркируемую продукцию.
//   * Не меняет прочие товарные строки.
// 
// Параметры:
//  ДокументОбъект           - ДокументОбъект.ОтгрузкаТоваровСХранения - документ к изменению
//  ПараметрыОкончанияПроверки - Структура - (См. ПроверкаИПодборПродукцииИС.ЗафиксироватьРезультатПроверкиИПодбора).
//
Процедура ОтразитьИзменениеКоличестваВТабличныхЧастяхТоварыСерииОтгрузкаТоваровСХранения(ДокументОбъект, ПараметрыОкончанияПроверки)
	...
	СтруктураЗаполнения = СтруктураЗаполнения(
		СтруктураДействийПриДобавленииНовойСтрокиОтгрузкаТоваровСХранения(ДокументОбъект),
		СтруктураДействийПриИзмененииСтрокиОтгрузкаТоваровСХранения(ДокументОбъект),
		КлючПоиска,
		КлючСтрокиСерий);
	
	ПараметрыУказанияСерий = НоменклатураСервер.ПараметрыУказанияСерий(ДокументОбъект, Документы.ОтгрузкаТоваровСХранения);
...
Процедура ОтразитьИзменениеКоличестваВТабличнойЧастиТоварыАктОРасхожденияхПриОтгрузке(ДокументОбъект, ТаблицаРасхожденийКодовМаркировки)
	...
	Запрос.УстановитьПараметр("ТипРасхожденияНедостача", Перечисления.ТипыРасхожденийИСМП.Недостача);
	Запрос.УстановитьПараметр("ТипРасхожденияИзлишек",   Перечисления.ТипыРасхожденийИСМП.Излишек);
	Запрос.УстановитьПараметр("ВариантыДействийПоРасхождениямКодовМаркировкиИСМППризнать", Перечисления.ВариантыДействийПоРасхождениямКодовМаркировкиИСМП.Признать);
	Запрос.УстановитьПараметр("ВариантыДействийПоРасхождениямКодовМаркировкиИСМПНеПризнать", Перечисления.ВариантыДействийПоРасхождениямКодовМаркировкиИСМП.НеПризнать);
	...
	ТаблицаТовары.Свернуть("ТекстовоеОписание, Номенклатура,Характеристика,Серия, Назначение, Упаковка, ВидЦены, Цена,
	                       |СтавкаНДС, ЗаказКлиента, КодСтроки, Склад, СтатусУказанияСерий, Реализация, НомерПаспорта,
...
Процедура РаспределитьСтрокиИзлишекВАктеОРасхождениях(МассивСтрок, ТаблицаТовары,
	...
	ИзлишекПризнать = Неопределено;
	ИзлишекНеПризнать = Неопределено;
	...
	Для Каждого СтрокаТаблицыТовары Из МассивСтрок Цикл
		...
		Если КоличествоИзлишекПризнать > 0 И КоличествоИзлишекНеПризнать > 0 Тогда
			...
			Если СтрокаТаблицыТовары.КоличествоПоДокументу = 0
				...
				Продолжить;
			Иначе
				ИзлишекНеПризнать = УменьшитьКоличествоИзлишек(СтрокаТаблицыТовары, ТаблицаТовары, СтруктураЗаполнения, КоличествоИзлишекНеПризнать);
			КонецЕсли;
			...
			Иначе
				ИзлишекПризнать = УменьшитьКоличествоИзлишек(СтрокаТаблицыТовары, ТаблицаТовары, СтруктураЗаполнения, КоличествоИзлишекПризнать, Действие);
			КонецЕсли;
	...
	Если КоличествоИзлишекНеПризнать > 0 И ИзлишекНеПризнать <> Неопределено Тогда
		ИзлишекНеПризнать.Количество = ИзлишекНеПризнать.Количество + КоличествоИзлишекНеПризнать;
		ИзлишекНеПризнать.КоличествоУпаковок = ИзлишекНеПризнать.КоличествоУпаковок + КоличествоИзлишекНеПризнать;
		ОбработатьСтрокуТабличнойЧасти(ИзлишекНеПризнать, СтруктураЗаполнения.Изменение);
		КоличествоИзлишекНеПризнать = 0;
	КонецЕсли;
	
	Если КоличествоИзлишекПризнать > 0 И ИзлишекПризнать <> Неопределено Тогда 
		ИзлишекПризнать.Количество = ИзлишекПризнать.Количество + КоличествоИзлишекПризнать;
		ИзлишекПризнать.КоличествоУпаковок = ИзлишекПризнать.КоличествоУпаковок + КоличествоИзлишекПризнать;
		ОбработатьСтрокуТабличнойЧасти(ИзлишекПризнать, СтруктураЗаполнения.Изменение);
		КоличествоИзлишекНеПризнать = 0;
	КонецЕсли;
...
Процедура УменьшитьКоличествоНедостача(СтрокаТаблицыТовары, ТаблицаТовары, СтруктураЗаполнения, КоличествоНедостача, Действие = Неопределено)
	...
	Если СтрокаТаблицыТовары.КоличествоПоДокументу = 0 Тогда
		СтрокаТаблицыТовары.ЗаполненоПоРеализации = Ложь;
	КонецЕсли;
	ОбработатьСтрокуТабличнойЧасти(СтрокаТаблицыТовары, СтруктураЗаполнения.Изменение);
...
Функция УменьшитьКоличествоИзлишек(СтрокаТаблицыТовары, ТаблицаТовары, СтруктураЗаполнения, КоличествоИзлишек, Действие = Неопределено)
...
//   * Удаляет из табличной части "ШтрихкодыУпаковок" документа отсутствующие штрихкоды верхнего уровня.
...
//
Процедура ОтразитьИзмененияТабличнойЧастиШтрихкодыУпаковок(ДокументОбъект, ТаблицаШтрихкодовВерхнегоУровня, ИмяТаблицы = "ШтрихкодыУпаковок") Экспорт
	
	ТаблицаШтрихкодовВерхнегоУровня.Колонки.Штрихкод.Имя = "ЗначениеШтрихкода";
	ДокументОбъект[ИмяТаблицы].Загрузить(ТаблицаШтрихкодовВерхнегоУровня);
...
Процедура ОтразитьИзменениеКоличестваВТабличныхЧастяхТоварыСерии(ПараметрыОтражения, ЗагрузитьВДокумент = Истина)
	...
	СтруктураПоискаБезСерий = Новый Структура(СтруктураЗаполнения.КлючПоиска);
	СтруктураПоискаСерий = Новый Структура(СтруктураЗаполнения.КлючСтрокиСерий);
	
	ТаблицаТовары.Колонки.Добавить("Удалить",       Новый ОписаниеТипов("Булево"));
	ТаблицаТовары.Колонки.Добавить("ДоступноПоСериям", ОбщегоНазначения.ОписаниеТипаЧисло(10, 3));
	ТаблицаТовары.Индексы.Добавить(СтруктураЗаполнения.КлючПоиска);
	
	Если ТаблицаСерии <> Неопределено Тогда
		ТаблицаСерии.Колонки.Добавить("Удалить", Новый ОписаниеТипов("Булево"));
	...
	Пока ВыборкаРасхождения.Следующий() Цикл
		...
		ЗаполнитьЗначенияСвойств(СтруктураПоиска,         ВыборкаРасхождения);
		ЗаполнитьЗначенияСвойств(СтруктураПоискаБезСерий, ВыборкаРасхождения);
		СтруктураПоискаБезСерий.Серия = Справочники.СерииНоменклатуры.ПустаяСсылка();
		ЗаполнитьЗначенияСвойств(СтруктураПоискаСерий,    СтруктураПоиска);
		...
		Если Недостача>0 Тогда
			...
			Если СтрокиТовары.Количество() = 0 Тогда
				СтрокиТовары = ТаблицаТовары.НайтиСтроки(СтруктураПоискаБезСерий);
			КонецЕсли;
			...
			Для Каждого СтрокаТаблицыТовары Из СтрокиТовары Цикл
				...
				НовоеКоличество = СтрокаТаблицыТовары.Количество - Списать;
				Недостача = Недостача - Списать;
				СтрокаТаблицыТовары.Удалить = НовоеКоличество = 0;
				Если НЕ СтрокаТаблицыТовары.Удалить Тогда
					СтрокаТаблицыТовары.Количество = НовоеКоличество;
					ОбработатьСтрокуТабличнойЧасти(СтрокаТаблицыТовары, СтруктураЗаполнения.Изменение);
				Иначе 
					СтрокаТаблицыТовары.ДоступноПоСериям = СтрокаТаблицыТовары.Количество;
					СтрокаТаблицыТовары.Количество = 0;
				КонецЕсли;
			...
			КонецЦикла;
		ИначеЕсли СтрокиТовары.Количество() > 0 Тогда
				
			СтрокаТаблицыТовары = СтрокиТовары[0];
			СтрокаТаблицыТовары.Количество = СтрокаТаблицыТовары.Количество - Недостача;
			ОбработатьСтрокуТабличнойЧасти(СтрокаТаблицыТовары, СтруктураЗаполнения.Изменение);
			
			Если ЗначениеЗаполнено(СтрокаТаблицыТовары.Серия) И ЕстьСтрокиСерий Тогда
				ЗаполнитьЗначенияСвойств(СтруктураПоискаСерий, СтрокаТаблицыТовары, ДополнениеСерий);
				СтрокиСерий = ТаблицаСерии.НайтиСтроки(СтруктураПоискаСерий);
				Если СтрокиСерий.Количество() > 0 Тогда
					СтрокаСерии = СтрокиСерий[0];
		...
		Иначе
			...
			Если СтрокиТовары.Количество() = 0 Тогда
				...
				Если ЗначениеЗаполнено(НоваяСтрокаТовары.Серия) И ТаблицаСерии <> Неопределено Тогда
					НоваяСтрокаСерии = ТаблицаСерии.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокаСерии, НоваяСтрокаТовары);
				КонецЕсли;
			...
			Иначе
				...
				ЗаполнитьПоСтроке = -Недостача;
				//В документе строка без серии, в результате проверки - с серией. Распределим серии.
				Для Каждого СтрокаТаблицыТовары Из СтрокиТовары Цикл
					Если ЗаполнитьПоСтроке = 0 Тогда
						Прервать;
					ИначеЕсли СтрокаТаблицыТовары.ДоступноПоСериям = 0 Тогда
						Продолжить;
					КонецЕсли;
					Вернуть = Мин(СтрокаТаблицыТовары.ДоступноПоСериям, ЗаполнитьПоСтроке);
					
					НоваяСтрокаТовары = ТаблицаТовары.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокаТовары, СтрокаТаблицыТовары);
					НоваяСтрокаТовары.Серия = ВыборкаРасхождения.Серия;
					НоваяСтрокаТовары.Количество = Вернуть;
					НоваяСтрокаТовары.Удалить = Ложь;
					СтрокаТаблицыТовары.ДоступноПоСериям = СтрокаТаблицыТовары.ДоступноПоСериям - Вернуть;
					ЗаполнитьПоСтроке = ЗаполнитьПоСтроке - Вернуть;
					ОбработатьСтрокуТабличнойЧасти(НоваяСтрокаТовары, СтруктураЗаполнения.Изменение);
					
					ЗаполнитьЗначенияСвойств(СтруктураПоискаСерий, НоваяСтрокаТовары, ДополнениеСерий);
					СтрокиСерий = ТаблицаСерии.НайтиСтроки(СтруктураПоискаСерий);
					Если СтрокиСерий.Количество() > 0 Тогда
						СтрокаСерии = СтрокиСерий[0];
					Иначе
						СтрокаСерии = ТаблицаСерии.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаСерии, НоваяСтрокаТовары,,"Количество");
					КонецЕсли;
					СтрокаСерии.Количество = СтрокаСерии.Количество + Вернуть;
				КонецЦикла;
				
				Если ЗаполнитьПоСтроке > 0 Тогда
					СтрокаТаблицыТовары = СтрокиТовары[0];
					СтрокаТаблицыТовары.Количество = ЗаполнитьПоСтроке;
					СтрокаТаблицыТовары.Серия = ВыборкаРасхождения.Серия;
					СтрокаТаблицыТовары.Удалить = Ложь;
					ОбработатьСтрокуТабличнойЧасти(СтрокаТаблицыТовары, СтруктураЗаполнения.Изменение);
					
					ЗаполнитьЗначенияСвойств(СтруктураПоискаСерий, СтрокаТаблицыТовары, ДополнениеСерий);
					СтруктураПоискаСерий.Серия = ВыборкаРасхождения.Серия;
					СтрокиСерий = ТаблицаСерии.НайтиСтроки(СтруктураПоискаСерий);
					Если СтрокиСерий.Количество() > 0 Тогда
						СтрокаСерии = СтрокиСерий[0];
					Иначе
...
//  ПараметрыОкончанияПроверки - Структура - cм. ПроверкаИПодборПродукцииИС.ЗафиксироватьРезультатПроверкиИПодбора
//
...
Функция ТекстЗапросаМаркируемойПродукции(ДокументСсылка,
	...
	Иначе
		...
		|	ОписаниеНоменклатурыИС.КлючВидовПродукции    КАК КлючВидовПродукции,
		|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ КАК ВидПродукцииГИСМТ,
		|	СУММА(ТаблицаТовары.%3)          КАК Количество
		|ИЗ
		|	Документ.%1.%2 КАК ТаблицаТовары
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		|		ПО ОписаниеНоменклатурыИС.Номенклатура = ТаблицаТовары.Номенклатура
...
Процедура ЗаполнитьШапкуДокумента(ДокументОбъект, ДанныеЗаполнения, Запрос, ТекстОшибкиЗаполнения = "")
	...
	Если РеквизитыДокумента.Количество() Тогда
		Реквизиты = РеквизитыДокумента[0];
	Иначе
		
		Если Не ЗначениеЗаполнено(ТекстОшибкиЗаполнения) Тогда
			ТекстОшибкиЗаполнения = СтрШаблон(НСтр("ru = 'По документу %1 отсутствуют данные для заполнения.'"), Основание);
		КонецЕсли;
		
		ВызватьИсключение ТекстОшибкиЗаполнения;
...
Процедура ЗаполнитьСерииПоШтрихкодамУпаковокПриВводеНаОсновании(ДокументОбъект, КоличественныеПоля)
	
	Если ДокументОбъект.ШтрихкодыУпаковок.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ВыполненоЗаполнение = Ложь;
	Товары = ДокументОбъект.Товары.Выгрузить(); //ТаблицаЗначений - 
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПеремещениеМеждуМОДИСМП") Тогда
		Содержимое = ЭлектронноеВзаимодействиеИСМП.Содержимое(ДокументОбъект.ШтрихкодыУпаковок.Выгрузить(),
			КонструкторИСМПКлиентСервер.ВидыПродукцииВнутреннегоПеремещенияМеждуМОД());
	Иначе
		Содержимое = ЭлектронноеВзаимодействиеИСМП.Содержимое(
			ДокументОбъект.ШтрихкодыУпаковок.Выгрузить(),
			КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукции(ДокументОбъект.ТоварнаяГруппа));
	КонецЕсли;
	Ключ = Новый Структура("Номенклатура,Характеристика,Серия");
	Для Каждого СтрокаСодержимого Из Содержимое Цикл
		ЗаполнитьЗначенияСвойств(Ключ, СтрокаСодержимого);
		Если Товары.НайтиСтроки(Ключ).Количество() > 0 Тогда
			Продолжить;
		КонецЕсли;
		Ключ.Серия = Справочники.СерииНоменклатуры.ПустаяСсылка();
		ТоварыБезСерий = Товары.НайтиСтроки(Ключ);
		Если ТоварыБезСерий.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		Пересчитать = СтрокаСодержимого.Количество;
		Для Каждого СтрокаТовар Из ТоварыБезСерий Цикл
			ПересчитатьПоСтроке = Мин(СтрокаТовар.Количество, Пересчитать);
			
			Если ПересчитатьПоСтроке < СтрокаТовар.Количество Тогда
				Коэффициент = ПересчитатьПоСтроке / СтрокаТовар.Количество;
				НоваяСтрока = Товары.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТовар);
				Для Каждого ИмяПоля Из КоличественныеПоля Цикл
					СтрокаТовар[ИмяПоля] = СтрокаТовар[ИмяПоля] * Коэффициент;
					НоваяСтрока[ИмяПоля] = НоваяСтрока[ИмяПоля] - СтрокаТовар[ИмяПоля];
				КонецЦикла;
			КонецЕсли;
			
			СтрокаТовар.Серия = СтрокаСодержимого.Серия;
			СтрокаТовар.СтатусУказанияСерий = 14;
			Пересчитать = Пересчитать - ПересчитатьПоСтроке;
			ВыполненоЗаполнение = Истина;
			Если Пересчитать = 0 Тогда
				Прервать;
			КонецЕсли;
		КонецЦикла;
	...
	Если ВыполненоЗаполнение Тогда
...
Процедура ЗаполнитьТабличнуюЧастьЗаказаНаЭмиссиюКодовМаркировкиСУЗ(ДокументОбъект, ДанныеЗаполнения, ИмяДокумента, ИмяТабличнойЧасти = "Товары")
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		Если ДанныеЗаполнения.ИмяФильтра = "ЗаполнениеСВидомПродукции" Тогда
			ДокументОбъект.ТоварнаяГруппа = ДанныеЗаполнения.ЗначениеФильтра;
		КонецЕсли;
		Основание = ДанныеЗаполнения.Основание;
	Иначе
		Основание = ДанныеЗаполнения;
	...
	Если ЗначениеЗаполнено(ДокументОбъект.ТоварнаяГруппа) Тогда
		ВидыПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииПоЗначениюЭлементаКонструктора(
			Справочники.ЭлементыКонструктораВидовПродукцииИС.ТоварныеГруппыГИСМТ, ДокументОбъект.ТоварнаяГруппа);
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаЗаполненияЗаказаНаЭмиссиюКодовМаркировкиСУЗИзПрикладногоДокумента(ИмяДокумента, ИмяТабличнойЧасти);
	...
	Если Не ЗначениеЗаполнено(ДокументОбъект.ТоварнаяГруппа) Тогда
		ИнтеграцияИСМП.ЗаполнитьТоварнуюГруппуПоТабличнойЧасти(ДокументОбъект);
	КонецЕсли;
	
КонецПроцедуры

Функция ТекстЗапросаЗаполненияЗаказаНаЭмиссиюКодовМаркировкиСУЗИзПрикладногоДокумента(ИмяДокумента, ИмяТабличнойЧасти)
	...
	ТекстЗапроса = СтрШаблон(
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		|		ПО ТаблицаДокумента.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
		|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
	...
	|	И ОформленныеТовары.Шаблон.ТипКодаМаркировки <> ЗНАЧЕНИЕ(Перечисление.ТипыКодовМаркировкиИСМП.ГрупповаяУпаковка)
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция КоличественныеПоляМаркировкиТоваровИСМП()
	...
	Результат.Добавить("СуммаАкциза");
	Возврат Результат;
КонецФункции
...
Процедура ЗаполнитьТабличныеЧастиМаркировкиТоваровИСМП(
	...
	ТоварныеГруппыБезВводаВОборот = Новый Массив;
	Для Каждого ТоварнаяГруппа Из ТоварныеГруппыМаркировки Цикл
		Если Не МаркировкаТоваровКлиентСервер.ТоварнаяГруппаПодлежитВводуВОборот(ТоварнаяГруппа) Тогда
			ТоварныеГруппыБезВводаВОборот.Добавить(ТоварнаяГруппа);
		КонецЕсли;
	КонецЦикла;
	...
	Запрос.УстановитьПараметр("ВидыПродукции",     ВидыПродукции);
	Запрос.УстановитьПараметр("ТоварныеГруппыБезВводаВОборот", ТоварныеГруппыБезВводаВОборот);
	...
	Иначе
		...
		Если ЕстьШтрихкодыУпаковок Тогда
			...
			ЗаполнитьСерииПоШтрихкодамУпаковокПриВводеНаОсновании(ДокументОбъект, КоличественныеПоляМаркировкиТоваровИСМП());
		КонецЕсли;
	КонецЕсли;
	...
	Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМОТП(ДокументОбъект.ТоварнаяГруппа)
...
Функция ТекстЗапросаЗаполненияМаркировкиТоваровИСМПИзПрикладногоДокумента(
	...
	Для Каждого ТоварыСерии Из ТабличныеЧастиТоварыСерии Цикл
		...
		ТекстЗапроса = 
			...
			|		И ТаблицаТовары.Характеристика = ТаблицаСерии.Характеристика
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
			|		ПО ТаблицаСерии.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
			|ГДЕ
	...
	|	И (ОформленныеТовары.Ссылка.ТоварнаяГруппа В(&ТоварныеГруппыБезВводаВОборот)
...
Процедура ЗаполнитьМаркировкаТоваровИСМПНаОснованииПересчетаТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииСырье)
	|ГДЕ
	...
	|	ДокументСсылка.Склад       КАК Склад,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеТабличнойЧастиТовары.ЕстьСырье, ЛОЖЬ)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотСырьяОстатки)
	|		ИНАЧЕ
	|			ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотМаркировкаОстатков)
	|	КОНЕЦ КАК Операция,
	...
	|");
	Запрос.УстановитьПараметр("ВидыПродукцииСырье", КонструкторИСМПКлиентСервер.ВидыПродукцииСырьеГИСМТ());
	Запрос.УстановитьПараметр("ТоварнаяГруппа", ДокументОбъект.ТоварнаяГруппа);
...
Процедура ЗаполнитьМаркировкаТоваровИСМПНаОснованииПриобретенияТоваровУслуг(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО ПриобретениеТоваровУслуг.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииСырье)
	|ГДЕ
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеТабличнойЧастиТовары.ЕстьСырье, ЛОЖЬ)
	|			ТОГДА ЕСТЬNULL(ДокументСсылка.Контрагент.СтранаРегистрации, ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка))
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)
	|	КОНЕЦ КАК Страна,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеТабличнойЧастиТовары.ЕстьСырье, ЛОЖЬ)
	|			ТОГДА ДокументСсылка.Номер
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК НомерПервичногоДокумента,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеТабличнойЧастиТовары.ЕстьСырье, ЛОЖЬ)
	|			ТОГДА ДокументСсылка.Дата
	|		ИНАЧЕ """"
	...
	|			ТОГДА ВЫБОР 
	|				КОГДА ЕСТЬNULL(ДанныеТабличнойЧастиТовары.ЕстьСырье, ЛОЖЬ)
	|					ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотСырьяПолучениеТовараИзЕАЭС)
	|				ИНАЧЕ 
	|					ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотТрансграничнаяТорговля)
	|				КОНЕЦ
	|		ИНАЧЕ 
	|			ВЫБОР 
	|				КОГДА ЕСТЬNULL(ДанныеТабличнойЧастиТовары.ЕстьСырье, ЛОЖЬ)
	|					ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотСырьяПолучениеТовараВнеЕАЭС)
	|				ИНАЧЕ 
	|					ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотПроизводствоВнеЕАЭС)
	|			КОНЕЦ
	|	КОНЕЦ                      КАК Операция
...
Процедура ЗаполнитьМаркировкаТоваровИСМПНаОснованииСборкиТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	КОНЕЦ КАК ОперацияНанесенияТолькоДляНаборов,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеТабличнойЧастиТовары.ЕстьСырье, ЛОЖЬ)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотСырьяПроизводствоРФ)
	|		ИНАЧЕ
	|			ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВводВОборотПроизводствоРФ)
	|	КОНЕЦ КАК Операция,
	|	ДокументСсылка.Ссылка      КАК ДокументОснование,
...
Функция КоличественныеПоляВыводаИзОборотаИСМП()
	ПоляСумм = Новый Массив;
	ПоляСумм.Добавить("Количество");
	ПоляСумм.Добавить("КоличествоУпаковок");
	ПоляСумм.Добавить("Сумма");
	ПоляСумм.Добавить("СуммаНДС");
	ПоляСумм.Добавить("СуммаСНДС");
	ПоляСумм.Добавить("КоличествоПотребительскихУпаковок");
	Возврат ПоляСумм;	
КонецФункции
...
Процедура ЗаполнитьШапкуВыводаИзОборотаИСМП(ДокументОбъект, ДанныеЗаполнения, Запрос)
	...
	ЗаполнитьАдресДокументаВыводИзОборотаИСМП(ДокументОбъект, ДатаСведений);
...
Процедура ЗаполнитьТабличныеЧастиВыводаИзОборотаИСМП(
	...
	Если Штрихкоды <> Неопределено Тогда
		...
		ЗаполнитьСерииПоШтрихкодамУпаковокПриВводеНаОсновании(ДокументОбъект, КоличественныеПоляВыводаИзОборотаИСМП());
	КонецЕсли;
	
	Если ВыводИзОборотаКлиентСервер.ДоступноПолеОСУ(ДокументОбъект.ТоварнаяГруппа)
...
Процедура ЗаполнитьАдресДокументаВыводИзОборотаИСМП(ДокументОбъект, ДатаСведений)
	
	МассивСсылок = Новый Массив;
	
	Если ЗначениеЗаполнено(ДокументОбъект.Контрагент) Тогда
		МассивСсылок.Добавить(ДокументОбъект.Контрагент);
	...
	ИначеЕсли ЗначениеЗаполнено(ДокументОбъект.Организация) Тогда
		МассивСсылок.Добавить(ДокументОбъект.Организация);
	...
	ВладелецИнформации = МассивСсылок[0];
	
	СтруктураОтбора = Новый Структура("Объект", ВладелецИнформации);
	ДанныеПоАдресам = КонтактнаяИнформация.НайтиСтроки(СтруктураОтбора);
	
	Если ДанныеПоАдресам.Количество() Тогда
		ДокументОбъект.АдресПлощадки        = ДанныеПоАдресам[0].Значение;
		ДокументОбъект.АдресПлощадкиСтрокой = ДанныеПоАдресам[0].Представление;
...
Процедура УдалитьКодыОСУИзТабличнойЧасти(ДокументОбъект)
	...
	Для Каждого КлючИЗначение Из ЗначенияШтрихкодов Цикл
		...
		ВидПродукции = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ДокументОбъект.ТоварнаяГруппа);
...
Функция ТекстЗапросаЗаполненияВыводаИзОборотаИСМПИзСписаниеНедостачТоваров(ИмяДокумента, ТабличныеЧастиТоварыСерии, ТабличнаяЧастьШтрихкоды)
	...
	Если ТабличнаяЧастьШтрихкоды <> Неопределено Тогда
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		|		ПО Штрихкоды.%3.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
...
Процедура ЗаполнитьВыводИзОборотаИСМПНаОснованииРеализацииТоваровУслуг(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	Контрагенты.СтранаРегистрации КАК СтранаНазначения,
	...
	ЗаполнитьВыводИзОборотаПриУведомленииНаОтгрузку(ДокументОбъект);

КонецПроцедуры

Процедура ЗаполнитьВыводИзОборотаИСМПНаОснованииОтгрузкиТоваровКлиенту(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	Контрагенты.СтранаРегистрации КАК СтранаНазначения,
	|	""Отгрузка товаров клиенту"" КАК НаименованиеПервичногоДокумента,
	|	ДокументСсылка.Подразделение    КАК ПроизводственныйОбъект
	|ИЗ
	|	Документ.ОтгрузкаТоваровКлиенту КАК ДокументСсылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Контрагенты
	|		ПО Контрагенты.Ссылка = ДокументСсылка.Контрагент
	|ГДЕ
	|	ДокументСсылка.Ссылка = &ДокументОснование
	...
	ЗаполнитьТабличныеЧастиВыводаИзОборотаИСМП(ДокументОбъект, ДанныеЗаполнения, "ОтгрузкаТоваровКлиенту", ОписаниеТабличнойЧасти, ТабличнаяЧастьШтрихкоды);
	ЗаполнитьВыводИзОборотаПриОбъемноСортовомУчете(ДокументОбъект);
...
Процедура ЗаполнитьВыводИзОборотаПриОбъемноСортовомУчете(ДокументОбъект)
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ВыводИзОборотаИСМП")
		И ВыводИзОборотаКлиентСервер.ДоступноПолеОСУ(ДокументОбъект.ТоварнаяГруппа)
		И Не КонструкторИСМПКлиентСервер.ЭтоСырьеГИСМТ(ДокументОбъект.ТоварнаяГруппа) Тогда
	
		ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(ДокументОбъект.Ссылка);
		ДанныеПоШтрихкодам = ШтрихкодированиеИС.ВложенныеШтрихкодыУпаковок(
			ДокументОбъект.ШтрихкодыУпаковок.Выгрузить().ВыгрузитьКолонку("ШтрихкодУпаковки"),
...
Процедура ЗаполнитьВыводИзОборотаПриУведомленииНаОтгрузку(ДокументОбъект)
	
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ВыводИзОборотаИСМП")
		И ВыводИзОборотаКлиентСервер.ДоступноУведомлениеОбОтгрузкеНаЭкспорт(
			КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ДокументОбъект.ТоварнаяГруппа), 
			ДокументОбъект.Причина) Тогда
			
		ДокументОбъект.ТранспортныйДокумент = Ложь;
		ДокументОбъект.НомерПеревозочногоДокумента = Неопределено;
		ДокументОбъект.ДатаПеревозочногоДокумента = Неопределено;
		ДокументОбъект.НомерТранспортногоСредства = Неопределено;
		ДокументОбъект.ТранспортноеСредство = Неопределено;
		ДокументОбъект.ИдентификаторЭлектронногоПеревозочногоДокумента = "";
		
		ТранспортныеНакладные = Документы.ТранспортнаяНакладная.ТранспортныеНакладныеДокументовОснований(ДокументОбъект.ДокументОснование);
		Если ТранспортныеНакладные.Количество() = 0 Тогда
			...
			Если ВыборкаЭПД.Следующий() Тогда
				...
				ДокументОбъект.ИдентификаторЭлектронногоПеревозочногоДокумента = ИдентификаторДокумента;
				ДокументОбъект.ТранспортныйДокумент = Истина;
		...
		ИначеЕсли ТранспортныеНакладные.Количество() = 1 Тогда
			...
			ДанныеПеревозки = ИнтеграцияИСУТ.ДанныеПеревозки(ДокументОбъект.ДокументОснование, ТранспортныеНакладные[0].ТранспортнаяНакладная);
...
Процедура ЗаполнитьТабличнуюЧастьПеремаркировкиТоваровИСМП(
	...
	ВидыПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииИСМП(Ложь);
	Если ЗначениеЗаполнено(ДокументОбъект.ТоварнаяГруппа) Тогда
		ВидыПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииПоЗначениюЭлементаКонструктора(
			Справочники.ЭлементыКонструктораВидовПродукцииИС.ТоварныеГруппыГИСМТ, ДокументОбъект.ТоварнаяГруппа);
	КонецЕсли;
...
Функция ТекстЗапросаЗаполненияПеремаркировкиТоваровИСМПИзПрикладногоДокумента(ИмяДокумента, ТабличнаяЧастьШтрихкоды)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО ОформленныеШтрихкоды.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукции)
	|ГДЕ
...
Процедура ЗаполнитьПеремаркировкуИСМПНаОснованииПоступленияТоваровОтКлиента(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.Перемаркировка) КАК Операция
	|ИЗ
	|	Документ.ПоступлениеТоваровОтКлиента КАК ДокументСсылка
	|ГДЕ
	|	ДокументСсылка.Ссылка = &ДокументОснование
	|");
	
	ЗаполнитьШапкуПеремаркировкиТоваровИСМП(ДокументОбъект, ДанныеЗаполнения, Запрос);
	ЗаполнитьТабличнуюЧастьПеремаркировкиТоваровИСМП(ДокументОбъект, ДанныеЗаполнения, "ПоступлениеТоваровОтКлиента");
...
Процедура ЗаполнитьТабличнуюЧастьВозвратаВОборотИСМП(
	...
	Если ЗначениеЗаполнено(ДокументОбъект.ТоварнаяГруппа) Тогда
		ВидыПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииПоЗначениюЭлементаКонструктора(
			Справочники.ЭлементыКонструктораВидовПродукцииИС.ТоварныеГруппыГИСМТ, ДокументОбъект.ТоварнаяГруппа);
	Иначе
		ВсеВидыПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииИСМП();
		ВидыПродукции = Новый Массив;
		Для Каждого ВидПродукции Из ВсеВидыПродукции Цикл
			Если ВозвратВОборотКлиентСервер.ДоступныеВидыВозврата(ВидПродукции).Количество() > 0 Тогда
	...
	Если ДокументОбъект.Товары.Количество() = 0 И ЗначениеЗаполнено(ДокументОбъект.ТоварнаяГруппа) Тогда
...
Функция ТекстЗапросаЗаполненияВозвратаВОборотИСМПИзПрикладногоДокумента(ИмяДокумента, ТабличнаяЧастьШтрихкоды)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО ШтрихкодыУпаковокТоваров.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
...
Процедура ЗаполнитьВозвратВОборотИСМПНаОснованииВозвратаТоваровОтКлиента(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	ДокументСсылка.Организация     КАК Организация,
	|	НЕ ДокументСсылка.Проведен     КАК ЕстьОшибкиПроведен,
	|	ЕСТЬNULL(ВидыВозвратаВОборотИСМП.Ссылка, ЗНАЧЕНИЕ(Справочник.ВидыВозвратаВОборотИСМП.ПустаяСсылка)) КАК ВидВозврата
	|ИЗ
	|	Документ.ВозвратТоваровОтКлиента КАК ДокументСсылка
	...
	|				ТОГДА ВидыВозвратаВОборотИСМП.Имя = ""REMOTE_SALE_RETURN""
	|			КОГДА ДокументСсылка.ДокументРеализации ССЫЛКА Документ.РеализацияТоваровУслуг
	|				ТОГДА ВидыВозвратаВОборотИСМП.Имя = ""OWN_USE_RETURN""
	|			ИНАЧЕ ВидыВозвратаВОборотИСМП.Имя = ""RETAIL_RETURN""
	|		КОНЕЦ
	|ГДЕ
	|	ДокументСсылка.Ссылка = &ДокументОснование");
...
Процедура ЗаполнитьВозвратВОборотИСМПНаОснованииПоступленияТоваровОтКлиента(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|				ТОГДА ВидыВозвратаВОборотИСМП.Имя = ""REMOTE_SALE_RETURN""
	|			ИНАЧЕ ВидыВозвратаВОборотИСМП.Имя = ""OWN_USE_RETURN""
	|		КОНЕЦ
	|ГДЕ
	|	ДокументСсылка.Ссылка = &ДокументОснование
	|");
	...
	ЗаполнитьШапкуВозвратаВОборотИСМП(ДокументОбъект, ДанныеЗаполнения, Запрос);
	ЗаполнитьТабличнуюЧастьВозвратаВОборотИСМП(ДокументОбъект, ДанныеЗаполнения, "ПоступлениеТоваровОтКлиента");
...
Процедура ЗаполнитьВозвратВОборотИСМПНаОснованииОтчетаОРозничныхВозвратах(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	Документ.ОтчетОРозничныхВозвратах КАК ДокументСсылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыВозвратаВОборотИСМП КАК ВидыВозвратаВОборотИСМП
	|		ПО ВидыВозвратаВОборотИСМП.Имя = ""RETAIL_RETURN""
	|ГДЕ
	|	ДокументСсылка.Ссылка = &ДокументОснование
	|");
...
Функция КоличественныеПоляОтгрузкиТоваровИСМП()
	Результат = Новый Массив;
	Результат.Добавить("Количество");
	Результат.Добавить("КоличествоУпаковок");
	Результат.Добавить("КоличествоПотребительскихУпаковок");
...
Процедура ЗаполнитьШапкуОтгрузкиТоваровИСМП(ДокументОбъект, ДанныеЗаполнения, Запрос)
	...
	ВсеВидыПродукции = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.УчитываемыеВидыМаркируемойПродукции();
	ВидыПродукции    = Новый Массив;
	Для Каждого ВидПродукции Из ВсеВидыПродукции Цикл
		Если ОтгрузкаТоваровКлиентСервер.ОперацияОтгрузкаВЕАЭСПриОСУДоступна(ВидПродукции) Тогда
			ВидыПродукции.Добавить(ВидПродукции);
		КонецЕсли;
	КонецЦикла;
	Запрос.УстановитьПараметр("ВидыПродукцииОтгрузкаВЕАЭСПриОСУ", ВидыПродукции);
...
Процедура ЗаполнитьТабличныеЧастиОтгрузкиТоваровИСМП(
	...
	Иначе
		Если ЭлектронноеВзаимодействиеИСМП.ДокументСвязанСЭлектронным(Основание, Истина) Тогда
			...
			ВидыПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииМЕХ();
			ТоварныеГруппы = КонструкторИСОбщегоНазначенияКлиентСервер.ЗначенияЭлементаКонструктораПоВидуПродукции(
				Справочники.ЭлементыКонструктораВидовПродукцииИС.ТоварныеГруппыГИСМТ, ВидыПродукции);
			Если ТоварныеГруппы.Количество() Тогда
				ДокументОбъект.ТоварнаяГруппа = ТоварныеГруппы[0];
			КонецЕсли;
		...
		ВсеВидыПродукции = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.УчитываемыеВидыМаркируемойПродукции();
		ВидыПродукции    = Новый Массив;
		ПустаяОперация = Не ЗначениеЗаполнено(ДокументОбъект.Операция);
		Для Каждого ВидПродукции Из ВсеВидыПродукции Цикл
			Если (ПустаяОперация Или ДокументОбъект.Операция = Перечисления.ВидыОперацийИСМП.ОтгрузкаВЕАЭСПриОСУ)
				И ОтгрузкаТоваровКлиентСервер.ОперацияОтгрузкаВЕАЭСПриОСУДоступна(ВидПродукции)
				Или (ПустаяОперация Или ДокументОбъект.Операция = Перечисления.ВидыОперацийИСМП.ОтгрузкаЕАЭССПризнаниемКИ)
				И ОтгрузкаТоваровКлиентСервер.ОперацияОтгрузкаЕАЭССПризнаниемКИДоступна(ВидПродукции)
				Или (ПустаяОперация Или ДокументОбъект.Операция = Перечисления.ВидыОперацийИСМП.ОтгрузкаПродажа)
				И ОтгрузкаТоваровКлиентСервер.ОперацияОтгрузкаПродажаДоступна(ВидПродукции)
				Или (ПустаяОперация Или ДокументОбъект.Операция = Перечисления.ВидыОперацийИСМП.ОтгрузкаКомиссия)
				И ОтгрузкаТоваровКлиентСервер.ОперацияОтгрузкаКомиссияДоступна(ВидПродукции)
				Или (ПустаяОперация Или ДокументОбъект.Операция = Перечисления.ВидыОперацийИСМП.ОтгрузкаАгент)
				И ОтгрузкаТоваровКлиентСервер.ОперацияОтгрузкаАгентДоступна(ВидПродукции) Тогда
				ВидыПродукции.Добавить(ВидПродукции);
			КонецЕсли;
		КонецЦикла;
		КонецЕсли;
	...
	Если Штрихкоды <> Неопределено Тогда
		...
		Если ДокументОбъект.Операция <> Перечисления.ВидыОперацийИСМП.ОтгрузкаВЕАЭСПриОСУ Тогда
			ШтрихкодыОснования = Пакет[ИндексТовары+1].Выгрузить().ВыгрузитьКолонку("ШтрихкодУпаковки");
			ШтрихкодированиеИС.ВыделитьШтрихкодыСодержащиеВидыПродукции(
				ШтрихкодыОснования,
				КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ДокументОбъект.ТоварнаяГруппа));
			Для Каждого ЭлементМассива Из ШтрихкодыОснования Цикл
				ДокументОбъект.ШтрихкодыУпаковок.Добавить().ШтрихкодУпаковки = ЭлементМассива;
			КонецЦикла;
			ЗаполнитьСерииПоШтрихкодамУпаковокПриВводеНаОсновании(ДокументОбъект, КоличественныеПоляОтгрузкиТоваровИСМП());
		КонецЕсли;
...
Процедура ЗаполнитьОтгрузкуТоваровИСМПНаОснованииРеализацииТоваровУслуг(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииОтгрузкаВЕАЭСПриОСУ)
	|ГДЕ
...
Процедура ЗаполнитьОтгрузкуТоваровИСМПНаОснованииОтгрузкиТоваровКлиенту(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО ОтгрузкаТоваровКлиентуТовары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииОтгрузкаВЕАЭСПриОСУ)
	|ГДЕ
	|	ОтгрузкаТоваровКлиентуТовары.Ссылка = &ДокументОснование
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	ТабличныеЧасти.Добавить(ОписаниеТабличнойЧасти);
	ТабличнаяЧастьШтрихкоды = Новый Структура("ИмяТаблицыШтрихкодыУпаковок, ИмяКолонкиШтрихкодУпаковки", "ШтрихкодыУпаковок", "ШтрихкодУпаковки");
	ЗаполнитьТабличныеЧастиОтгрузкиТоваровИСМП(ДокументОбъект, ДанныеЗаполнения, "ОтгрузкаТоваровКлиенту", ТабличныеЧасти, ТабличнаяЧастьШтрихкоды);
...
Процедура ЗаполнитьОтгрузкуТоваровИСМПНаОснованииВозвратаТоваровПоставщику(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО ВозвратТоваровПоставщикуТовары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииОтгрузкаВЕАЭСПриОСУ)
	|ГДЕ
...
Процедура ЗаполнитьОтгрузкуТоваровИСМПНаОснованииКорректировкиРеализации(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО КорректировкаРеализацииТовары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииОтгрузкаВЕАЭСПриОСУ)
	|ГДЕ
...
Процедура ЗаполнитьРазрешениеНаОтгрузкуИСМПНаОснованииОтгрузкиТоваровКлиенту(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
...
Процедура ЗаполнитьРазрешениеНаОтгрузкуИСМПНаОснованииВозвратаТоваровПоставщику(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО ВозвратТоваровПоставщику.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииРазрешенияНаОтгрузку)
	|ГДЕ
...
Процедура ЗаполнитьРазрешениеНаОтгрузкуИСМПНаОснованииВозвратаТоваровМеждуОрганизациями(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО ВозвратТоваровМеждуОрганизациями.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииРазрешенияНаОтгрузку)
	|ГДЕ
...
Процедура ЗаполнитьРазрешениеНаОтгрузкуИСМПНаОснованииПередачиТоваровМеждуОрганизациями(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО ПередачаТоваровМеждуОрганизациями.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииРазрешенияНаОтгрузку)
	|ГДЕ
...
Процедура ЗаполнитьШапкуРазрешениеНаОтгрузкуИСМП(ДокументОбъект, ДанныеЗаполнения, Запрос)
	...
	Запрос.УстановитьПараметр(
		...
		КонструкторИСМПКлиентСервер.ВидыПродукцииТребующиеРазрешенияНаОтгрузку());
...
Функция КоличественныеПоляПеремещенияМеждуМОДИСМП()
	Результат = Новый Массив;
	Результат.Добавить("Количество");
	Результат.Добавить("КоличествоУпаковок");
	Возврат Результат;
...
Процедура ЗаполнитьПеремещениеМеждуМОДИСМП(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|		ИНАЧЕ
	|			ДокументСсылка.СкладПолучатель
	|		КОНЕЦ КАК Получатель,
	|	ДокументСсылка.СкладОтправитель КАК ОтправительОбъект,
	|	ДокументСсылка.СкладПолучатель КАК ПолучательОбъект,
	|	ЕСТЬNULL(ДанныеРегистрацииОтправителя.КПП, СправочникОрганизацииОтправитель.КПП) КАК КППОбособленногоПодразделенияОтправителя,
	|	ЕСТЬNULL(ДанныеРегистрацииПолучателя.КПП, СправочникОрганизацииПолучатель.КПП)  КАК КППОбособленногоПодразделенияПолучателя,
	|	ДокументСсылка.Комментарий                       КАК Комментарий,
	|	ДокументСсылка.Дата                              КАК ДатаОтгрузки
	|ИЗ
	|	Документ.ПеремещениеТоваров КАК ДокументСсылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Организации КАК СправочникОрганизацииОтправитель
	|		ПО ДокументСсылка.Организация = СправочникОрганизацииОтправитель.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Организации КАК СправочникОрганизацииПолучатель
	|		ПО ДокументСсылка.ОрганизацияПолучатель = СправочникОрганизацииПолучатель.Ссылка
	...
	|ГДЕ
	|	ДокументСсылка.Ссылка = &ДокументОснование
	|	И (СправочникОрганизацииОтправитель.ЮрФизЛицо = ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ИндивидуальныйПредприниматель)
	|		ИЛИ (ЕСТЬNULL(ДанныеРегистрацииОтправителя.КПП, СправочникОрганизацииОтправитель.КПП) <> ЕСТЬNULL(ДанныеРегистрацииПолучателя.КПП, СправочникОрганизацииПолучатель.КПП)));
	|");
	...
	ТабличнаяЧастьШтрихкоды = Новый Структура("ИмяТаблицыШтрихкодыУпаковок, ИмяКолонкиШтрихкодУпаковки", "ШтрихкодыУпаковок", "ШтрихкодУпаковки");
	ЗаполнитьТабличныеЧастиПеремещенияМеждуМОДИСМП(ДокументОбъект, ДанныеЗаполнения, "ПеремещениеТоваров", ТоварыСерии, ТабличнаяЧастьШтрихкоды);
...
Процедура ЗаполнитьШапкуПеремещенияМеждуМОДИСМП(ДокументОбъект, ДанныеЗаполнения, Запрос)
	...
	ТекстОшибкиОформления = СтрШаблон(
		НСтр("ru = 'По документу %1 не требуется перемещение между МОД.'"),
		Основание);
	
	ЗаполнитьШапкуДокумента(ДокументОбъект, ДанныеЗаполнения, Запрос, ТекстОшибкиОформления);
	...
	Если Не ОбщегоНазначенияИСВызовСервера.ПартнерОрганизацияЭтоИндивидуальныйПредприниматель(ДокументОбъект.Организация) Тогда
		...
		Если Не ЗначениеЗаполнено(ДокументОбъект.Отправитель) И Не ЗначениеЗаполнено(ДокументОбъект.Получатель) Тогда
			...
			ВызватьИсключение ТекстОшибкиОформления;
...
Процедура ЗаполнитьДанныеАдресовПеремещенияМеждуМОДИСМП(ДокументОбъект, ДатаСведений)
	...
	ПараметрыЗаполненияМОД                = ЭлектронноеВзаимодействиеИСМП.ПараметрыУказанияДанныхПоМОДВЭлектронномДокументеУПД_5_02(ДокументОбъект.ТоварнаяГруппа);
	ПараметрыЗаполненияМОДПоВидуПродукции = ПараметрыЗаполненияМОД.ПараметрыУказанияМОДПоВидамПродукции.Получить(ДокументОбъект.ТоварнаяГруппа);
	
	ВидКонтактнойИнформации = Справочники.ВидыКонтактнойИнформации.АдресСклада;
	МассивСсылок            = Новый Массив;
	
	Если ПараметрыЗаполненияМОДПоВидуПродукции <> Неопределено Тогда
		
		ОрганизацияИндивидуальныйПредприниматель = ОбщегоНазначенияИСВызовСервера.ПартнерОрганизацияЭтоИндивидуальныйПредприниматель(ДокументОбъект.Организация);
		
		ТребуетсяУказаниеКодаФИАСОтправителя = ПараметрыЗаполненияМОДПоВидуПродукции.ТребуетсяУказаниеКодаФИАСПоМОДОтправителя
			И ПараметрыЗаполненияМОДПоВидуПродукции.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяИндивидуальныхПредпринимателей
			И ОрганизацияИндивидуальныйПредприниматель
			
			Или ПараметрыЗаполненияМОДПоВидуПродукции.ТребуетсяУказаниеКодаФИАСПоМОДОтправителя
			И ПараметрыЗаполненияМОДПоВидуПродукции.ТребуетсяУказаниеКодаФИАСПоМОДОтправителяЮридическихЛиц
			И Не ОрганизацияИндивидуальныйПредприниматель;
			
		ТребуетсяУказаниеКодаФИАСПолучателя = ПараметрыЗаполненияМОДПоВидуПродукции.ТребуетсяУказаниеКодаФИАСПоМОДПолучателя
			И ПараметрыЗаполненияМОДПоВидуПродукции.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяИндивидуальныхПредпринимателей
			И ОрганизацияИндивидуальныйПредприниматель
			
			Или ПараметрыЗаполненияМОДПоВидуПродукции.ТребуетсяУказаниеКодаФИАСПоМОДПолучателя
			И ПараметрыЗаполненияМОДПоВидуПродукции.ТребуетсяУказаниеКодаФИАСПоМОДПолучателяЮридическихЛиц
			И Не ОрганизацияИндивидуальныйПредприниматель;
			
		Если ТребуетсяУказаниеКодаФИАСОтправителя Тогда
		...
		Если ТребуетсяУказаниеКодаФИАСПолучателя Тогда
...
Процедура ЗаполнитьКодыФИАСПоАдресамОтправителяПолучателя(ДокументОбъект)
	...
	ДокументОбъект.ЗаполнитьКодыФИАСПоАдресамОтправителяПолучателя();
...
Процедура ЗаполнитьТабличныеЧастиПеремещенияМеждуМОДИСМП(
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		Если ДанныеЗаполнения.Свойство("ИмяФильтра")
			И ДанныеЗаполнения.ИмяФильтра = "ЗаполнениеСВидомПродукции" Тогда
			ВидыПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииПоЗначениюЭлементаКонструктора(
				Справочники.ЭлементыКонструктораВидовПродукцииИС.ТоварныеГруппыГИСМТ, ДанныеЗаполнения.ЗначениеФильтра);
		КонецЕсли;
		Основание = ДанныеЗаполнения.Основание;
	Иначе
		Основание = ДанныеЗаполнения;
	...
	Если Штрихкоды <> Неопределено Тогда
		...
		ШтрихкодированиеИС.ВыделитьШтрихкодыСодержащиеВидыПродукции(
			...
			КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ДокументОбъект.ТоварнаяГруппа));
		Для Каждого ЭлементМассива Из ШтрихкодыОснования Цикл
			НоваяСтрокаШтрихкодовУпаковок = ДокументОбъект.ШтрихкодыУпаковок.Добавить();
			НоваяСтрокаШтрихкодовУпаковок.ШтрихкодУпаковки = ЭлементМассива;
		КонецЦикла;
		
		ЗаполнитьСерииПоШтрихкодамУпаковокПриВводеНаОсновании(ДокументОбъект, КоличественныеПоляПеремещенияМеждуМОДИСМП());
...
Функция СтруктураДействийПриДобавленииНовойСтрокиПередачаТоваровХранителю(Объект)
	...
	ИспользоватьСоглашения = Обработчик.СоглашенияСКлиентамиПрименимы() И Обработчик.ИспользоватьСоглашенияСКлиентами();
	ДоговорПоНовойКомиссии = Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию
...
Функция СтруктураДействийПриДобавленииНовойСтрокиОтгрузкаТоваровСХранения(Объект)
...
Функция СтруктураДействийПриДобавленииНовойСтрокиОтгрузкаТоваровКлиенту(Объект)
...
Функция СтруктураДействийПриИзмененииСтрокиПоступления(ДокументПоступления)
	...
	ТипДокумента = ТипЗнч(ДокументПоступления);
	
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ПересчитатьКоличествоУпаковок");
	
	Если ТипДокумента = Тип("ДокументОбъект.ПриобретениеТоваровУслуг")
		Или ТипДокумента = Тип("ДокументОбъект.ВозвратТоваровОтКлиента") Тогда
		СтруктураПересчетаСуммы = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(ДокументПоступления);
		СтруктураДействий.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
		СтруктураДействий.Вставить("ПересчитатьСуммуСНДС", СтруктураПересчетаСуммы);
	КонецЕсли;
	...
	Если ТипДокумента = Тип("ДокументОбъект.ПриобретениеТоваровУслуг") Тогда
...
Функция СтруктураДействийПриИзмененииСтрокиОтгрузкаТоваровСХранения(Объект)
...
Функция СтруктураДействийПриИзмененииСтрокиОтгрузкаТоваровКлиенту(Объект)
...
Функция СтруктураДействийПриДобавленииНовойСтрокиАктОРасхождениях(Объект)
	...
	СтруктураПересчетаСуммыСуффикс = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(Объект);
...
Процедура ОбработатьСтрокуТабличнойЧасти(СтрокаТабличнойЧасти, СтруктураДействий)
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтрокаТабличнойЧастиСтруктурой, СтруктураДействий, Неопределено);
