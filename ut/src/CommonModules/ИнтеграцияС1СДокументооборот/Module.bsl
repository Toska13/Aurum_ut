...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("НачатьСогласование", Истина);
	Методы.Вставить("ПолучитьФайлыСвязанныхДокументовАсинхронно", Истина);
	Методы.Вставить("ПрерватьСогласование", Истина);
	Методы.Вставить("ПрерываниеСогласованияВозможно", Истина);
	Методы.Вставить("СогласованиеВозможно", Истина);
	Методы.Вставить("СостояниеСогласования", Истина);
...
Процедура ПриСозданииНаСервере(Форма, МестоРазмещенияКоманд = Неопределено, ПараметрыОповещения = Неопределено,
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.УбратьДублиКомандыИнтеграции(
		Форма,
		Метаданные.ОбщиеКоманды.ИнтеграцияС1СДокументооборотСоздатьБизнесПроцесс.Имя);
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.УбратьДублиКомандыИнтеграции(
		Форма,
		Метаданные.ОбщиеКоманды.ИнтеграцияС1СДокументооборотСоздатьПисьмо.Имя);
...
Процедура ПерейтиНаВерсию_3_0_4_2() Экспорт
	...
	ЗапросПравила = Новый Запрос(
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПравилаИнтеграцииС1СДокументооборотом.ПравилаЗаполненияРеквизитовИС КАК ПравилаИнтеграцииС1СДокументооборотомПравилаЗаполненияРеквизитовИС
		|		ПО (ПравилаИнтеграцииС1СДокументооборотом.Ссылка = ПравилаИнтеграцииС1СДокументооборотомПравилаЗаполненияРеквизитовИС.Ссылка)
		|ГДЕ
		|	НЕ ПравилаИнтеграцииС1СДокументооборотом.ПометкаУдаления
		|	И ПравилаИнтеграцииС1СДокументооборотом.ТипОбъектаИС ПОДОБНО ""Справочник.%""
		|	И ПравилаИнтеграцииС1СДокументооборотомПравилаЗаполненияРеквизитовИС.РежимИзмененияДанныхПроведенногоДокумента <> &ПустойРежим");
	ЗапросПравила.УстановитьПараметр("ПустойРежим", ПустойРежим);
