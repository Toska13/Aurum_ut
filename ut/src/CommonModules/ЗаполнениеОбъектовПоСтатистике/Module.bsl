...
//      Если параметр не задан, то описание реквизитов будет получено из функции
//      ЗадатьОписаниеЗаполняемыхРеквизитовПоСтатистике() определенной в модуля менеджера объекта.
//  ПроверятьНастройку - Булево - если признак Истина, подчиненные реквизиты заполняются в зависимости от настройки
//      "Заполнять реквизиты документов по статистике".
//
// Возвращаемое значение:
//  Структура - реквизиты объекта, которые были заполнены по статистике,
//      где Ключ - имя реквизита, а Значение - его новое значение.
//
Функция ЗаполнитьПодчиненныеРеквизитыОбъекта(Объект, КлючевойРеквизит, ОписаниеЗаполняемыхРеквизитов = Неопределено, ПроверятьНастройку = Истина) Экспорт
...
Функция ПолучитьЗначенияРеквизитов(Ссылка, ОписаниеЗаполняемыхРеквизитов, ДанныеКлючевыхРеквизитов = Неопределено) Экспорт
	...
	ОписаниеРеквизитовОбъекта =
		ОбщегоНазначения.СкопироватьРекурсивно(
			ЗаполнениеОбъектовПоСтатистикеСлужебный.ОписаниеРеквизитовОбъекта(
...
//     * ПараметрыСбораСтатистики - Структура - см. НовыйПараметрыСбораСтатистики().
//
Функция ПараметрыЗаполняемыхРеквизитов() Экспорт
	...
	ПараметрыСбораСтатистики = НовыйПараметрыСбораСтатистики();
	...
	Параметры.Вставить("ПараметрыСбораСтатистики", ПараметрыСбораСтатистики);
...
Процедура ДобавитьОписаниеЗаполняемыхРеквизитов(ОписаниеЗаполняемыхРеквизитов, ИменаРеквизитов, Параметры = Неопределено) Экспорт
	...
	Для Каждого Реквизит Из СтрРазделить(ИменаРеквизитов, ",", Ложь) Цикл
		...
		Если ЗначениеЗаполнено(Параметры) Тогда
			...
			КлючевыеРеквизиты.Вставить("ПараметрыСбораСтатистики", Параметры.ПараметрыСбораСтатистики);
		КонецЕсли;
...
// Возвращает структуру параметров для сбора статистики.
//
// Возвращаемое значение:
//  Структура - параметры сбора статистики, содержит ключи:
//  	* РазмерВыборки - Число - количество выбираемых объектов, по умолчанию 5.
//  	* ЧастотаИспользованияЗначения - Число - частота использования значения (вес значения в выборке), может принимать
//  		значения от 0 до 1, по умолчанию 0.5
//  		вес значения в выборке выше этой величины позволяет считать значение "частотным".
//  	* ПриОтсутствииДанныхПолучатьЗначениеБезУчетаАвтора - Булево - если Истина, при отсутствии данных по автору
//  		будет получено значение без учета автора.
//
Функция НовыйПараметрыСбораСтатистики() Экспорт
	...
	ПараметрыСбораСтатистики.Вставить("ПриОтсутствииДанныхПолучатьЗначениеБезУчетаАвтора", Ложь);
...
Функция ЗаполнитьРеквизиты(Объект, ОтборПоРеквизитуРодителю, ДанныеЗаполнения, ОписаниеЗаполняемыхРеквизитов)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		ЗначенияИзДанныхЗаполнения =
			ОбщегоНазначения.СкопироватьРекурсивно(
				ЗаполнениеОбъектовПоСтатистикеСлужебный.РеквизитыЗаполняемыеИзДанныхЗаполнения(ИмяОбъектаМетаданных));
...
//  	остальные ключи см. ЗаполнениеОбъектовПоСтатистикеСлужебный.ОписаниеРеквизитовОбъекта
...
Функция ПараметрыСбораСтатистики(Знач ОписаниеРеквизитовОбъекта, ЗаполняемыйРеквизит)
	...
	Если ОписаниеРеквизитовОбъекта.ПараметрыСбораСтатистики.Свойство(ЗаполняемыйРеквизит.ИмяРеквизита) Тогда
		Возврат ОписаниеРеквизитовОбъекта.ПараметрыСбораСтатистики[ЗаполняемыйРеквизит.ИмяРеквизита];
	КонецЕсли;
...
Процедура ЗначениеРеквизитаПоСтатистике(ОписаниеРеквизитовОбъекта, ЗаполняемыйРеквизит)
	...
	ЗаполняемыйРеквизит.НовоеЗначение = ЗаполнениеОбъектовПоСтатистикеСлужебный.ЗначениеРеквизитаПоСтатистике(
	...
	Если Не ДанныеАктуальны(ЗаполняемыйРеквизит.НовоеЗначение) Тогда
		ЗаполняемыйРеквизит.НовоеЗначение = Неопределено;
	КонецЕсли;
	
	Если ЗаполняемыйРеквизит.НовоеЗначение = Неопределено
		И ПараметрыСбораСтатистики.ПриОтсутствииДанныхПолучатьЗначениеБезУчетаАвтора Тогда
		ЗаполняемыйРеквизит.НовоеЗначение = ЗаполнениеОбъектовПоСтатистикеСлужебный.ЗначениеРеквизитаПоСтатистике(
			ОписаниеРеквизитовОбъекта.Объект.Ссылка, ОписаниеРеквизитовОбъекта.Объект.ИмяОбъектаМетаданных,
			ЗаполняемыйРеквизит.ИмяРеквизита, ЗначенияРеквизитовРодителей,
			ПараметрыСбораСтатистики, Истина);
		Если Не ДанныеАктуальны(ЗаполняемыйРеквизит.НовоеЗначение) Тогда
			ЗаполняемыйРеквизит.НовоеЗначение = Неопределено;
		КонецЕсли;
	КонецЕсли;
...
Функция ДанныеАктуальны(Значение)
	...
	ИмяРеквизитаЗакрыт = "Закрыт";
	
	УстановитьПривилегированныйРежим(Истина);
	Если ОбщегоНазначения.ЭтоСсылка(ТипЗначения)
		И ОбщегоНазначения.ЭтоСтандартныйРеквизит(Значение.Метаданные().СтандартныеРеквизиты, "ПометкаУдаления")
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Значение, ИмяРеквизитаПометкаУдаления) Тогда
		ДанныеАктуальны = Ложь;
	ИначеЕсли (ТипЗначения = Тип("СправочникСсылка.СтруктураПредприятия")
		Или ТипЗначения = Тип("СправочникСсылка.Организации"))
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Значение, ИмяРеквизитаСтатус) = Перечисления.СтатусыОрганизацийПодразделений.НеДействует Тогда
		ДанныеАктуальны = Ложь;
	ИначеЕсли (ТипЗначения = Тип("СправочникСсылка.БанковскиеСчетаКонтрагентов")
		Или ТипЗначения = Тип("СправочникСсылка.БанковскиеСчетаОрганизаций"))
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Значение, ИмяРеквизитаЗакрыт) Тогда
		ДанныеАктуальны = Ложь;
	КонецЕсли;
