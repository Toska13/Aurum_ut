...
// Возвращает дополнительные параметры регламентного задания.
//
// Параметры:
//   ПрефиксСервиса - Строка - ключ из см. ПрефиксыСервиса.
//
// Возвращаемое значение:
//   Структура - список дополнительных параметров регламентного задания:
//     * ИмпортироватьДанные     - Булево - признак импорта данных при обновлении товарного каталога.
//     * ОбновлятьСопоставленные - Булево - признак обновления сопоставленных товаров при обновлении товарного каталога.
//     * ЭтоЗапускВручную        - Булево - признак запуска регламентного задания вручную.
//     * Идентификаторы          - см. ИнтеграцияСМаркетплейсамиКлиентСервер.НовыйОтборПоИдентификаторам.
//
Функция ДополнительныеПараметрыРегламентногоЗадания(ПрефиксСервиса) Экспорт

	Параметры = Новый Структура;
	
	Параметры.Вставить("ИмпортироватьДанные",     Ложь);
	Параметры.Вставить("ОбновлятьСопоставленные", Ложь);
	Параметры.Вставить("ЭтоЗапускВручную",        Ложь);
	Параметры.Вставить("Идентификаторы", ИнтеграцияСМаркетплейсамиКлиентСервер.НовыйОтборПоИдентификаторам());

	Если ПрефиксСервиса = "ОбновлениеТоварногоКаталога" Тогда
		Параметры.ОбновлятьСопоставленные = Истина;
		
	ИначеЕсли ПрефиксСервиса = ИнтеграцияСМаркетплейсамиКлиентСервер.ПрефиксИмпортаТоварногоКаталога() Тогда
		
		Параметры.ИмпортироватьДанные     = Истина;
		Параметры.ОбновлятьСопоставленные = Истина;
...
// Возвращает навигационную ссылку на товар на витрине площадки.
//
// Параметры:
//   SKU - Строка - SKU товара.
//
// Возвращаемое значение:
//   Строка - навигационная ссылка на товар на витрине площадки.
//
...
// Возвращает навигационную ссылку страницы редактирования товара на площадке.
//
// Параметры:
//   ИдентификаторПлощадки - Строка - идентификатор товара на площадке.
//
// Возвращаемое значение:
//   Строка - навигационная ссылка страницы редактирования товара на площадке.
//
Функция НавигационнаяСсылкаРедактированияТовара(ИдентификаторПлощадки) Экспорт

	Возврат СтрШаблон("https://seller.ozon.ru/app/products/%1/edit/preview", ИдентификаторПлощадки);

КонецФункции

// Возвращает навигационную ссылку страницы списка товаров с отбором на площадке.
//
// Параметры:
//   ИдентификаторПубликации - Строка - идентификатор товара продавца.
//   Статус                  - ПеречислениеСсылка.СтатусыОбъектовМаркетплейсаOzon - статус товара.
//
// Возвращаемое значение:
//   Строка - навигационная ссылка страницы списка товаров с отбором на площадке.
//
Функция НавигационнаяСсылкаНаТоварВСписке(ИдентификаторПубликации, Статус) Экспорт

	Если Статус = ПредопределенноеЗначение("Перечисление.СтатусыОбъектовМаркетплейсаOzon.ВАрхиве") Тогда
		Фильтр = "archived";
	Иначе
		Фильтр = "all";
	КонецЕсли;
	
	Возврат СтрШаблон("https://seller.ozon.ru/app/products?filter=%1&search=%2", Фильтр, ИдентификаторПубликации);
...
Функция НаименованиеРегламентногоЗадания(Идентификатор) Экспорт
	...
	Наименования.Вставить("OzonProductCatalogImport_",
		НСтр("ru = 'Ozon. Импорт данных товарного каталога'"));
...
//
// Возвращаемое значение:
//  Структура - Префиксы сервиса:
//   * НастройкиСервиса - Строка - 
//   * ВыгрузкаОстатков - Строка - 
//   * ЗагрузкаОстатков - Строка - 
//   * ВыгрузкаЦен - Строка - 
//   * ЗагрузкаЦен - Строка - 
//   * ОбновлениеТоварногоКаталога - Строка - 
//   * ИмпортТоварногоКаталога - Строка - 
//   * ЗагрузкаИнформацииОПродажахТоваров - Строка - 
//   * ОбновлениеИнформацииОПродажахТоваров - Строка - 
//   * ЗагрузкаНеобработанныхЗаказовСТорговойПлощадки - Строка - 
//   * ОбновлениеСтатусовЗаказовТорговойПлощадки - Строка - 
//   * ПолучениеЭтикетокДляЗаказовТорговойПлощадки - Строка - 
//
...
Функция КонстантыСервиса(Знач ВВерхнемРегистре = Истина) Экспорт
	...
	Результат.Вставить("МаксимальноеКоличествоИзображенийДляТовара",      30);
	...
	Результат.Вставить("ИдентификаторАтрибута_СтранаПроисхождения",       "4389");
...
// Формирует ключ атрибута по его идентификатору для установки пути к реквизиту 1С по умолчанию.
//
// Параметры:
//  ИдентификаторАтрибута - Строка
//
// Возвращаемое значение:
//  Строка
//
Функция КлючАтрибутаПоИдентификатору(ИдентификаторАтрибута) Экспорт
	Возврат СтрШаблон("%1%2", ПрефиксКлючаАтрибута(), ИдентификаторАтрибута);
КонецФункции

// Извлекает идентификатор атрибута из его ключа.
//
// Параметры:
//  КлючАтрибута - Строка
//
...
//  Строка
//
Функция ИдентификаторАтрибутаПоКлючу(КлючАтрибута) Экспорт
	Возврат СтрЗаменить(КлючАтрибута, ПрефиксКлючаАтрибута(), "");
КонецФункции

Функция ПрефиксКлючаАтрибута()
	Возврат "Атрибут_";
КонецФункции
