...
Процедура СформироватьЗапросПроверкиУсловийЗакупки(ТекстЗапроса, ИмяТаблицы, ЭтоПроверкаШапки)
	...
	Для Каждого ОписаниеПроверки Из ОписаниеПроверокСоответствияЗначенийСоглашению() Цикл
		...
		|	ПРЕДСТАВЛЕНИЕ(СоглашениеСПоставщиком.ИмяРеквизита) КАК ИмяРеквизитаСоглашение,
	...
	Если ДобавлятьПроверкиСоглашения
		...
		|	ПРЕДСТАВЛЕНИЕ(СоглашениеСПоставщиком.ХозяйственнаяОперация) КАК ХозяйственнаяОперацияСоглашение"); // @query-part
	...
	Если ЭтоПроверкаШапки И ДобавлятьПроверкиСоглашения Тогда
		...
		|	ПРЕДСТАВЛЕНИЕ(СоглашениеСПоставщиком.Склад) КАК СкладСоглашение"); // @query-part
	ИначеЕсли ИмяТаблицы = "Документ.ЗаявкаНаЗакупку" Тогда
	...
	Если ИмяТаблицы = "Документ.ПриобретениеТоваровУслуг" Тогда
		...
		|	ПРЕДСТАВЛЕНИЕ(СоглашениеСПоставщиком.Валюта) КАК ВалютаВзаиморасчетовСоглашение"); // @query-part
		#КонецОбласти
	КонецЕсли;
...
Функция НовоеОписаниеПроверкиСоответствияЗначенияСоглашению(ОписаниеПроверок, ИмяРеквизита, ИмяПоляЕстьОшибки, ПредставлениеРеквизита, ПустоеЗначениеВЗапросе = "")
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого ОписаниеПроверки Из ОписаниеПроверок Цикл
			...
			ТекстОшибки = СтрЗаменить(ТекстОшибки,
				...
				Выборка[ОписаниеПроверки.ИмяРеквизита + "Соглашение"]);
		...
		Если НаличиеПроверок.ЕстьОшибкиЗаполненияСклад И Выборка.ЕстьОшибкиЗаполненияСклад Тогда
			...
			Если ЭтоЗаявкаНаЗакупку Тогда
				...
				Если Выборка.СкладСоглашениеЕстьГруппа Тогда
					ТекстОшибки = НСтр("ru = 'Группа складов соглашения ""%СкладСоглашение%"" не содержит склад-получатель ""%Склад%""%АдресОшибки%'");
				Иначе
					ТекстОшибки = НСтр("ru = 'Склад соглашения ""%СкладСоглашение%"" не должен отличаться от значения в документе ""%Склад%""%АдресОшибки%'");
				КонецЕсли;
