...
Асинх Функция ЗапросКМ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		Если Не ДанныеОперации.Свойство("ПараметрыXML") Тогда
			ВызватьИсключение НСтр("ru = 'Не сформированы параметры запроса XML'");
		КонецЕсли;
...
Асинх Функция НачатьРаботуПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	...
	Если НЕ ИспользоватьТСПИоТ Тогда
		ОписаниеОшибки = СтрШаблон(НСтр("ru='Данная команда доступна только при использовании ""%1""'"), Наименование1СТСПиОТ());
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, "PiotStart", ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		Возврат РезультатВыполнения;
	КонецЕсли;
	...
	Если НЕ ПроверкаАутентификацияПИоТ Тогда 
		Попытка                                
			...
			РезультатВызова = Ждать ОбъектДрайвера.НачатьРаботуПИоТАсинх(ПараметрыПодключения.ИДУстройства, ПараметрыXML, СтатусСостоянияXML); // Структура
			ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, РезультатВызова);
		Исключение
			ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()); 
			ЗавершитьЗаписьЛогаИсключением(ЗаписьЛога, ТекстОшибки);
		...
		Иначе                                                       
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Аутентификация %1 не пройдена.'"), НаименованиеТСПиОТ());
...
Асинх Функция ПроверкаКМПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	...
	Попытка                                
		...
		РезультатВызова = Ждать ОбъектДрайвера.ПроверкаКМПИоТАсинх(ПараметрыПодключения.ИДУстройства, ДанныеОперации.ЗапросJSON, РезультатПроверки, РезультатПараметрыXML, СтатусСостоянияXML); // Структура
		ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, РезультатВызова);
	Исключение
		ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗавершитьЗаписьЛогаИсключением(ЗаписьЛога, ТекстОшибки);
	...
	Если РезультатВызова.Значение Тогда              
		РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);     
	Иначе
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Проверка КМ в %1 не пройдена.'"), НаименованиеТСПиОТ());
		РезультатВыполнения = Ждать ПолучитьОшибкуПИоТ(ОбъектДрайвера, ЗаписьЛога, ТекстОшибки);
	КонецЕсли;                                  
...
Асинх Функция СтатусПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	...
	Попытка                                
		...
		РезультатВызова = Ждать ОбъектДрайвера.СтатусПИоТАсинх(ПараметрыПодключения.ИДУстройства, СтатусСостояния); // Структура
		ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, РезультатВызова);
	Исключение
		ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗавершитьЗаписьЛогаИсключением(ЗаписьЛога, ТекстОшибки);
	...
	Если РезультатВызова.Значение Тогда              
		РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);     
	Иначе
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка определения состояния %1'"), НаименованиеТСПиОТ());
		РезультатВыполнения = Ждать ПолучитьОшибкуПИоТ(ОбъектДрайвера, ЗаписьЛога, ТекстОшибки);
	КонецЕсли;    
...
Асинх Функция СлужебнаяИнформацияПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации) Экспорт
	...
	Попытка
		...
		РезультатВызова = Ждать ОбъектДрайвера.СлужебнаяИнформацияПИоТАсинх(ПараметрыПодключения.ИДУстройства, СлужебнаяИнформация); // Структура
		ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, РезультатВызова);
	Исключение
		ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗавершитьЗаписьЛогаИсключением(ЗаписьЛога, ТекстОшибки);
	...
	Иначе
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Служебная информация %1 не получена.'"), НаименованиеТСПиОТ());
		РезультатВыполнения = Ждать ПолучитьОшибкуПИоТ(ОбъектДрайвера, ЗаписьЛога, ТекстОшибки);
	КонецЕсли;    
...
Асинх Функция ПодключитьТСПИоТАсинх(ПараметрыПодключения) 
	...
	Если НЕ РезультатПодключения.Подключено Тогда
		...
		Если Не РезультатПодключения.Подключено Тогда
			...
			Если Не РезультатУстановки.Установлено Тогда
				...
				Результат.ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Ошибка установки ""%1""'"), Наименование1СТСПиОТ());
				Возврат Результат;
			КонецЕсли;
			...
			Если Не РезультатПодключения.Подключено Тогда
				Результат.Подключено = Ложь;
				Результат.ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Ошибка подключения ""%1""'"), Наименование1СТСПиОТ());
				Возврат Результат;
			КонецЕсли;
		КонецЕсли;                                     
	...
	Исключение             
		ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ОписаниеОшибки = СтрШаблон(НСтр("ru='Ошибка инцицилизации %1'"), НаименованиеТСПиОТ()) + Символы.ПС + ТекстОшибки;
...
// ++ НеМобильноеПриложение
// ++ Локализация
// @skip-check NStr-contains-camelcase
Функция НаименованиеТСПиОТ()
	Возврат НСтр("ru = 'ТС ПиОТ'");
КонецФункции

// @skip-check NStr-contains-camelcase
Функция Наименование1СТСПиОТ()
	Возврат НСтр("ru = '1С:ТС ПиОТ'");
КонецФункции
// -- Локализация
// -- НеМобильноеПриложение 
