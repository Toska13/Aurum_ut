...
Процедура ПровестиТестНастройки(Обработчик, НастройкаОбмена, ВыводитьОкноОжидания=Ложь) Экспорт
	...
	ИначеЕсли ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.SberAPI") Тогда
		ОбменСБанкамиSberAPIСлужебныйКлиент.ПровестиТестНастройки(Обработчик,
			НастройкаОбмена, ПараметрыОбмена, ВыводитьОкноОжидания);
...
Процедура ПослеОтправкиЗапросовСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Исключение
			...
			Если СтрНайти(ТекстСообщения, НСтр("ru = 'Код: 103'")) <> 0 Тогда
				
				ОповещениеПослеОтправкиЗапросов = Неопределено;
				Если ДополнительныеПараметры.Свойство("ОповещениеПослеОтправкиЗапросов", ОповещениеПослеОтправкиЗапросов)
					И ОповещениеПослеОтправкиЗапросов <> Неопределено
					И ОповещениеПослеОтправкиЗапросов.ИмяПроцедуры = "ПослеОтправкиСообщенияОбменаВСбербанк" Тогда
					ЭтоПлатежки = Истина;
				Иначе
					ЭтоПлатежки = Ложь;
				КонецЕсли;
				
				ВывестиСообщениеОбОшибкеУПШ103(ЭтоПлатежки);
				
				ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеОтправкиЗапросов, "ОшибкаУПШ103");
...
Процедура ПослеОтправкиПлатежныхДокументовСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		...
		Если СтрНайти(Результат.КраткоеПредставлениеОшибки, НСтр("ru = 'Код: 103'")) <> 0 Тогда
			ЭтоОшибкаУПШ = Истина;
		Иначе
			ЭтоОшибкаУПШ = Ложь;
		КонецЕсли;
		Если Не ЭтоОшибкаУПШ Тогда
			ОбработатьОшибку(ВидОперации, Результат.ПодробноеПредставлениеОшибки, Результат.КраткоеПредставлениеОшибки,
				ДополнительныеПараметры.НастройкаОбмена);
		КонецЕсли;
			
		Если ЭтоОшибкаУПШ Тогда
			
			ВывестиСообщениеОбОшибкеУПШ103(Истина);
...
Функция ДоступнаРаботаНаМобильномКлиенте(ПрограммаБанка) Экспорт
	...
	Если ЭтоМобильныйКлиент
		И (ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.SberAPI")
...
Процедура ВывестиСообщениеОбОшибкеУПШ103(ЭтоПлатежки = Ложь) Экспорт
	
	МассивСтрок = Новый Массив;
	ТекстСообщения = НСтр("ru = 'Сбербанк перешел на новую технологию обмена SberAPI.'");
	МассивСтрок.Добавить(ТекстСообщения);
	МассивСтрок.Добавить(Символы.ПС);
	МассивСтрок.Добавить(Символы.ПС);
	ТекстСообщения = НСтр("ru = 'Чтобы продолжить работу, настройте интеграцию согласно статье'");
	МассивСтрок.Добавить(ТекстСообщения);
	МассивСтрок.Добавить(" ");
	МассивСтрокГиперссылки = Новый Массив;
	МассивСтрокГиперссылки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Условия подключения к сервису Сбербанка.'")));
	МассивСтрок.Добавить(Новый ФорматированнаяСтрока(МассивСтрокГиперссылки,,,,"https://its.1c.ru/db/metod81#content:6747:hdoc"));
	Если ЭтоПлатежки Тогда
		МассивСтрок.Добавить(Символы.ПС);
		МассивСтрок.Добавить(Символы.ПС);
		ТекстСообщения = НСтр("ru = 'После завершения настройки создайте новые платежные поручения и отправьте их.'");
		МассивСтрок.Добавить(ТекстСообщения);
	КонецЕсли;
	ПоказатьПредупреждение(, Новый ФорматированнаяСтрока(МассивСтрок), , НСтр("ru = 'Предупреждение от банка'"));
...
Процедура ПродолжитьОперациюСПаролемВБезопасномХранилище(Результат, Контекст) Экспорт
...
Процедура ОтправкаСообщенийОбмена(Параметры) Экспорт
	...
	ИначеЕсли СтруктураКОтправке.ЧерезТокенSberAPI.Количество() > 0 Тогда
		ОбменСБанкамиSberAPIСлужебныйКлиент.ОтправитьПлатежныеДокументы(СтруктураКОтправке.ЧерезТокенSberAPI,
			Параметры);
	ИначеЕсли СтруктураКОтправке.ЧерезЛогинПарольSberAPI.Количество() > 0 Тогда
		ОбменСБанкамиSberAPIСлужебныйКлиент.ОтправитьПлатежныеДокументы(СтруктураКОтправке.ЧерезЛогинПарольSberAPI,
			Параметры);
	...
	ИначеЕсли СтруктураКОтправке.АльфаБанкОнлайн.Количество() > 0 Тогда
...
Процедура ОткрытьФормуЭлектронногоДокументаПослеПолученияОтпечатков(Отпечатки, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ПараметрыОткрытия) = Тип("Структура") И ПараметрыОткрытия.Свойство("ПараметрыФормы") Тогда
		ДополнительныеПараметрыФормы = ПараметрыОткрытия.ПараметрыФормы;
	КонецЕсли;
	...
	Если Не ДополнительныеПараметрыФормы = Неопределено Тогда
		
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ПараметрыФормы, ДополнительныеПараметрыФормы, Истина);
...
Процедура ПослеОбработкиДокументов(РезультатЗадания, Параметры) Экспорт
	...
	Если РезультатЗадания.Статус = "Ошибка" Тогда
		...
		Если СтрНайти(РезультатЗадания.КраткоеПредставлениеОшибки, НСтр("ru = 'Код: 103'")) <> 0 Тогда
			ЭтоОшибкаУПШ = Истина;
		Иначе
			ЭтоОшибкаУПШ = Ложь;
		КонецЕсли;
	...
	Иначе // выполнено
		...
		Если Результат <> Неопределено Тогда
			...
			ЕстьПлатежкиУПШдляSberAPI = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "ЕстьПлатежкиУПШдляSberAPI");
			Если ЕстьПлатежкиУПШдляSberAPI = Истина Тогда
				
				Сообщение = НСтр("ru = 'Выполнен переход на новый формат обмена с банком SberAPI.'");
				Сообщение = Сообщение  + Символы.ПС + НСтр("ru = 'Чтобы отправить платежные поручения, пожалуйста, создайте их заново.'");
				Сообщение = Сообщение  + Символы.ПС + НСтр("ru = 'Документы, отправляемые ранее, теперь отправить через SberAPI не получится.'");
				Сообщение = Сообщение + Символы.ПС + Символы.ПС + НСтр("ru = 'Для быстрого создания нового платежного поручения воспользуйтесь функцией «Скопировать».'");
				ПоказатьПредупреждение(, Сообщение, , НСтр("ru = 'Отправка платежных поручений'"));
...
Процедура УстановитьРасширениеПослеОтвета(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		Оповещение = Новый ОписаниеОповещения("ПодключитьРасширениеПослеУстановки", ЭтотОбъект,
			ДополнительныеПараметры);
		НачатьУстановкуРасширенияПолученияИнформацииОКомпьютере(Оповещение);
	Иначе
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеПолученияФрода, Неопределено);
	КонецЕсли;
