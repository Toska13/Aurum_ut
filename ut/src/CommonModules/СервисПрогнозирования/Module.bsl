...
Функция ОтправитьЗаявкуВСервис() Экспорт
	...
	Если Не ЗначениеЗаполнено(НастройкиСервиса.ТокенПриложения) Тогда
		...
		Возврат ЗарегистрироватьПриложениеВСервисеПоИТС(Ответ.Тикет, Ложь);
...
// Проверка вида плана и наличие идентификатора модели и создание модели при отсутствии.
// 
// Параметры:
...
Функция ПроверитьСоздатьМодель(ВидПлана) Экспорт
	...
	РеквизитыВидаПлана = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидПлана, "ИдентификаторМоделиПрогнозирования, Наименование");
	Если РеквизитыВидаПлана.ИдентификаторМоделиПрогнозирования > 0 Тогда
		Возврат СтруктураОтвета();
	КонецЕсли;
	
	Возврат СоздатьМодель(ВидПлана, РеквизитыВидаПлана.Наименование);
	
КонецФункции

// Создание модели на сервисе с заполнением идентификатора модели.
// 
// Параметры:
//  ВидПлана - СправочникСсылка.ВидыПланов - 
//  ИмяМодели - Строка - Наименование вида плана прогнозирования.
// 
// Возвращаемое значение:
//  см. СтруктураОтвета.
Функция СоздатьМодель(ВидПлана, ИмяМодели) Экспорт
	...
	ТелоЗапроса = Новый Структура();
	ТелоЗапроса.Вставить("modelName", ИмяМодели);
	ТелоЗапроса.Вставить("service",   НастройкиСервиса.ИмяСервиса);
	...
	Иначе
		СтруктураОтвета = СтруктураОтвета();
		СтруктураОтвета.ТекстОшибки = ТекстНеРазрешенОбменССервисом();
	КонецЕсли;
...
// Проверка данных связанных с конфигурацией и создание или обновление конфигурации модели.
// 
// Параметры:
//  ВидПлана - СправочникСсылка.ВидыПланов - 
// 
...
Функция ПроверитьСоздатьКонфигурациюМодели(ВидПлана) Экспорт
	...
	Возврат СоздатьКонфигурациюМодели(ВидПлана);
	
КонецФункции

// Создание или обновление конфигурации модели на сервисе.
// 
// Параметры:
//  ВидПлана - СправочникСсылка.ВидыПланов - 
// 
...
// Сбросить настройки сервиса в значения по умолчанию и очистить информацию сервиса.
//
// Параметры:
...
// Доступно подключение тестовых периодов.
//
// Возвращаемое значение:
//  Булево
Функция ДоступноПодключениеТестовыхПериодов() Экспорт
	
	Возврат ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПодключениеСервисовСопровождения");
...
Функция ВыгрузитьПериодическуюКоллекцию(ИмяКоллекции, ДатаПервойЗаписи, ДатаПоследнейЗаписи)
	...
	Пока ЕстьНевыгруженныйПериод(ИмяКоллекции) Цикл
		...
		ТекущееКоличествоОбъектов = 0;
		Если ПорцияВыгрузки.КоличествоДанных >= 1000 
			И Метод = "PUT" Тогда
			
			// Разделение порции на первичную с первыми 999 и вторичную с оставшимися объектами.
			// Во избежание получения некорректного ответа с сервиса "�V�K-�O�JM.)V�2400��".
			// Проблемный ответ возникает при запросе создания массива данных "PUT" от 1000 объектов и более.
			ДиапазонЧастиПорции = Новый Структура;
			ДиапазонЧастиПорции.Вставить("ЛеваяГраница",  0);
			ДиапазонЧастиПорции.Вставить("ПраваяГраница", 998);
			
			ДанныеКоллекции = ТаблицуЗначенийПериодическойКоллекцииВМассивСтруктур(
				ПорцияВыгрузки.Коллекция, ДоступныеРеквизиты, ДиапазонЧастиПорции); // Структура -
			...
			ДиапазонЧастиПорции.Вставить("ЛеваяГраница",  999);
			ДиапазонЧастиПорции.Вставить("ПраваяГраница", ПорцияВыгрузки.КоличествоДанных - 1);
...
Процедура ОчиститьИнформациюСервиса() Экспорт
	...
	РегистрыСведений.СервисПрогнозированияИдентификаторыОбучения.ОчиститьИдентификаторыОбучения();
...
Процедура ЗапуститьАвтообновлениеПрогнозов()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ИнфоОбучения.ВремяИзмененияСостояния + ДнейДоСледующегоОбновления <= ТекущаяДатаСеанса() Тогда
			Ответ = ПроверитьСоздатьМодель(Выборка.Ссылка);
			Если Не ПустаяСтрока(Ответ.ТекстОшибки) Тогда
				Продолжить;
			КонецЕсли;
			Ответ = ПроверитьСоздатьКонфигурациюМодели(Выборка.Ссылка);
			Если Не ПустаяСтрока(Ответ.ТекстОшибки) Тогда
				Продолжить;
			КонецЕсли;
...
Функция ОбработатьОтветСервисаВнутренний(СтруктураОтвета, ИмяДействия, СвойстваЗапроса, ДополнительныйПараметр = Неопределено)
	...
	ИначеЕсли ИмяДействия = "ПолучениеПрогнозаСервиса" Тогда
		...
		Если Не ЭтоИтерируемыйТип
			Или СтруктураОтвета.Количество() = 0 Тогда
			Ответ.ТекстОшибки = ТекстПриПолученииДанныхПроизошлаОшибка();
			Возврат Ответ;
		КонецЕсли;
...
Функция ТекстНеРазрешенОбменССервисом()
	
	Возврат НСтр("ru = 'Запрещен обмен с сервисом прогнозирования.
	|Включите настройку ""Разрешен обмен с сервисом"" на форме редактирования настроек из рабочего места по пути:
	|Еще - Команды сервиса прогнозирования - Настройки сервиса'");
