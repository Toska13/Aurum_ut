...
Функция ПолучитьТекстЗапросаПоСлужебнымРеквизитамТЧ(СтруктураДействий, СтруктураДопДанных, ПакетнаяОбработка = Ложь) Экспорт
	...
	ПакетнаяОбработкаТабличнойЧастиСерверЛокализация.ПриОпределенииШаблонаЗапросаПоСлужебнымРеквизитамТЧ(
		ЧастьШаблонаЗапроса, СтруктураДействий, ПакетнаяОбработка);
		
	ШаблонЗапроса = ШаблонЗапроса + ЧастьШаблонаЗапроса;
...
Процедура ЗаполнитьСтавкуНДСВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Иначе
		...
		Если СтруктураПараметровДействия.НалогообложениеНДСВТабличнойЧасти Тогда
			НалогообложениеНДС = ТекущаяСтрока.НалогообложениеНДС;
		Иначе
		...
		КэшированныеЗначения.Свойство("ИсходящийДокумент", ИсходящийДокумент);
		Если ИсходящийДокумент = Неопределено Тогда
			Если СтруктураПараметровДействия.Свойство("ИнициализацияВходящегоДокумента") Тогда 
				КэшированныеЗначения.Вставить("ИсходящийДокумент", Не СтруктураПараметровДействия.ИнициализацияВходящегоДокумента);
			Иначе
				КэшированныеЗначения.Вставить("ИсходящийДокумент", Ложь);
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьСтавкуНДСПоВидуЦенВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	Перем СтруктураПараметровДействия;
	
	СтруктураДействий.Свойство("ЗаполнитьСтавкуНДСПоВидуЦен", СтруктураПараметровДействия);
	ЗаполнитьСтавкуНДСПоВидуЦенТекущейСтроки("СтавкаНДС", ТекущаяСтрока, СтруктураПараметровДействия, КэшированныеЗначения, Истина);
	
КонецПроцедуры

Процедура ЗаполнитьСтавкуНДССуффиксПоВидуЦенВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	Перем СтруктураПараметровДействия;
	
	СтруктураДействий.Свойство("ЗаполнитьСтавкуНДССуффиксПоВидуЦен", СтруктураПараметровДействия);
		
	Если ЗначениеЗаполнено(СтруктураПараметровДействия.Суффикс) Тогда

		СтавкаССуффиксом = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.СтавкаССуффиксом(СтруктураПараметровДействия.Суффикс);
		ЗаполнитьСтавкуНДСПоВидуЦенТекущейСтроки(СтавкаССуффиксом.СтавкаНДС, ТекущаяСтрока, СтруктураПараметровДействия, КэшированныеЗначения, Ложь);
...
Процедура ПриИзмененииТипаНоменклатуры(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если ПараметрыДействия.Свойство("ЕстьСписатьНаРасходы") И ПараметрыДействия.ЕстьСписатьНаРасходы Тогда
		СписатьНаРасходы = ТекущаяСтрока.СписатьНаРасходы;
	Иначе
	...
	ТекущаяСтрока.СкладОбязателен = ?(ЗначениеЗаполнено(ТекущаяСтрока.ТипНоменклатуры) И Не (ЭтоРабота Или Отменено Или СписатьНаРасходы), 1, 0);
...
//  КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения.
//
Процедура ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества(ТекущаяСтрока, Действия, КэшированныеЗначения) Экспорт
	
	ДанныеДляОбработки = ДанныеДляОбработкиСтроки("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", КэшированныеЗначения);
	КонтрольКачества.ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества(ТекущаяСтрока, ДанныеДляОбработки[0],
		Действия);
	
КонецПроцедуры

// Действие необходимо вызывать при изменении любого из поля в строке документа, приводящего к изменению поля КонтролируемаяПартия.
//
// Параметры:
//  ТекущаяСтрока - Структура - структура со свойствами строки документа.
//  Действия - Структура - структура с действиями, которые нужно произвести.
//  КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения.
//
Процедура ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества(ТекущаяСтрока, Действия, КэшированныеЗначения) Экспорт
	
	ДанныеДляОбработки = ДанныеДляОбработкиСтроки("ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества", КэшированныеЗначения);
	КонтрольКачества.ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества(ТекущаяСтрока, ДанныеДляОбработки[0],
		Действия);
	
КонецПроцедуры

// Действие необходимо вызывать при изменении любого из поля в строке документа, приводящего к изменению поля КонтролируемаяПартияОтправителя.
//
// Параметры:
//  ТекущаяСтрока - Структура - структура со свойствами строки документа.
//  Действия - Структура - структура с действиями, которые нужно произвести.
//  КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения.
//
Процедура ЗаполнитьКонтролируемуюПартиюОтправителя(ТекущаяСтрока, Действия, КэшированныеЗначения) Экспорт
	
	ДанныеДляОбработки = ДанныеДляОбработкиСтроки("ЗаполнитьКонтролируемуюПартиюОтправителя", КэшированныеЗначения);
	КонтрольКачества.ЗаполнитьКонтролируемуюПартиюОтправителя(ТекущаяСтрока, ДанныеДляОбработки[0],
		Действия);
...
Процедура ПроверитьЗаполнитьСклад(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	СписатьНаРасходы = Ложь;
	Если ТипЗнч(ПараметрыДействия) = Тип("Структура")
		И ПараметрыДействия.Свойство("ИмяПоляСписатьНаРасходы") И ПараметрыДействия.ИмяПоляСписатьНаРасходы <> "" Тогда
		СписатьНаРасходы = ТекущаяСтрока[ПараметрыДействия.ИмяПоляСписатьНаРасходы];
	КонецЕсли;
	...
	Если СкладГруппа
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока[ИмяПоляСклад])
			...
			И (ДанныеДляОбработки.ОчиститьСклад
				Или СписатьНаРасходы) Тогда
...
Процедура ПроверитьСериюПоВладельцу(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	ДанныеПоСтроке = ДанныеДляОбработкиСтроки("ПроверитьСериюПоВладельцу", КэшированныеЗначения);
	
	Если ДанныеПоСтроке <> Неопределено Тогда
		Результат = ДанныеПоСтроке[0];
		
		Если Результат.РазличаютсяВладельцыСерииИНоменклатуры Тогда
			ТекущаяСтрока.Серия = Справочники.СерииНоменклатуры.ПустаяСсылка();
		КонецЕсли;
		
		Если ЕстьКолонкаВИсточникеДанных("СерииИспользуются", КэшированныеЗначения) Тогда
			ТекущаяСтрока.СерииИспользуются = Результат.СерииИспользуются;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьПризнакПодакцизныйТовар(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	УчетПодакцизныхТоваров.ЗаполнитьПризнакПодакцизныйТоварПакетно(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	
КонецПроцедуры

// Заполняет реквизиты расчета Акциза для строки документа
//
// Параметры:
//	ТекущаяСтрока - Структура - текущая строка табличной части.
//	СтруктураДействий - Структура - допустимые действия для табличной части.
//	КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения.
Процедура ЗаполнитьРеквизитыРасчетаАкциза(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	УчетПодакцизныхТоваров.ЗаполнитьРеквизитыРасчетаАкциза(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	
КонецПроцедуры

Процедура ПересчитатьСуммуАкцизаВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	УчетПодакцизныхТоваров.ПересчитатьСуммуАкцизаПакетно(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
...
// Заполняет соглашение в табличной части
//
// Параметры:
//	ТекущаяСтрока - Структура - текущая строка табличной части.
//	СтруктураДействий - Структура - допустимые действия для табличной части.
//	КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения.
//
Процедура ЗаполнитьСоглашениеПоставщикаПоУмолчанию(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	ТекущаяСтрока.Соглашение = ДанныеДляОбработкиСтроки("ПараметрыСоглашенийПоставщика", КэшированныеЗначения)[0].Соглашение;
	
КонецПроцедуры 

// Заполняет служебные реквизиты соглашения в табличной части
//
// Параметры:
//	ТекущаяСтрока - Структура - текущая строка табличной части.
//	СтруктураДействий - Структура - допустимые действия для табличной части.
//	КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения.
//
Процедура ЗаполнитьРеквизитыПоСоглашениюПоставщика(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	ЗаполнитьЗначенияСвойств(ТекущаяСтрока,
		ДанныеДляОбработкиСтроки("ЗаполнитьРеквизитыПоСоглашениюПоставщика", КэшированныеЗначения)[0]);
	
КонецПроцедуры 

// Заполняет количество соглашений поставщика в табличной части
//
// Параметры:
//	ТекущаяСтрока - Структура - текущая строка табличной части.
//	СтруктураДействий - Структура - допустимые действия для табличной части.
//	КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения.
//
Процедура ЗаполнитьКоличествоСоглашенийПоставщика(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	ТекущаяСтрока.КоличествоСоглашений = ДанныеДляОбработкиСтроки("ПараметрыСоглашенийПоставщика", КэшированныеЗначения)[0].КоличествоСоглашений;
	
КонецПроцедуры


// Заполняет контрагента в табличной части
//
// Параметры:
//	ТекущаяСтрока - Структура - текущая строка табличной части.
//	СтруктураДействий - Структура - допустимые действия для табличной части.
//	КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения.
//
Процедура ЗаполнитьКонтрагентаПартнераПоУмолчанию(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	ТекущаяСтрока.Контрагент = ДанныеДляОбработкиСтроки("ЗаполнитьКонтрагентаПартнераПоУмолчанию", КэшированныеЗначения)[0].Контрагент;
	
КонецПроцедуры

// Заполняет договор в табличной части
//
// Параметры:
//	ТекущаяСтрока - Структура - текущая строка табличной части.
//	СтруктураДействий - Структура - допустимые действия для табличной части.
//	КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения.
//
Процедура ЗаполнитьДоговорПоУмолчанию(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	ТекущаяСтрока.Договор = ДанныеДляОбработкиСтроки("ЗаполнитьДоговорПоУмолчанию", КэшированныеЗначения)[0].Договор;
...
//	КэшированныеЗначения - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения.
//
Процедура ЗаполнитьОперациюПоСоглашениюДоговору(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	ТекущаяСтрока.ХозяйственнаяОперация = 
		ДанныеДляОбработкиСтроки("ЗаполнитьОперациюПоСоглашениюДоговору", КэшированныеЗначения)[0].ХозяйственнаяОперация;
	
КонецПроцедуры

Процедура ЗаполнитьПредставлениеУсловийЗакупки(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	ПредставленияСсылок = ДанныеДляОбработкиСтроки("ЗаполнитьПредставлениеУсловийЗакупки", КэшированныеЗначения)[0];
	ТекущаяСтрока.ПредставлениеУсловийЗакупки = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПредставлениеУсловийЗакупки(
		ПредставленияСсылок.Партнер,
		ТекущаяСтрока.Сумма,
		ТекущаяСтрока.СуммаСНДС);
	
КонецПроцедуры

Процедура ПроверитьРегистрациюШтрихкода(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	ДанныеПоСтроке = ДанныеДляОбработкиСтроки("ПроверитьРегистрациюШтрихкода", КэшированныеЗначения);
	
	Если ДанныеПоСтроке <> Неопределено Тогда
		Результат = ДанныеПоСтроке[0];
		
		ТекущаяСтрока.Зарегистрирован = Результат.Зарегистрирован;
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковСтрокКоллекции(ОбработчикиСтрок) Экспорт
	...
	ОбработчикиСтрок.Добавить("ЗаполнитьПризнакСписатьНаРасходыИПодразделение",
		"ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ЗаполнитьПризнакСписатьНаРасходыИПодразделение");
	...
	ОбработчикиСтрок.Добавить("ЗаполнитьПризнакПодакцизныйТовар",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьПризнакПодакцизныйТовар");
	
	ОбработчикиСтрок.Добавить("ЗаполнитьРеквизитыРасчетаАкциза",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьРеквизитыРасчетаАкциза");
	
	ОбработчикиСтрок.Добавить("ПересчитатьСуммуАкциза",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ПересчитатьСуммуАкцизаВСтрокеТЧ");
	...
	ОбработчикиСтрок.Добавить("ЗаполнитьСоглашениеПоставщикаПоУмолчанию",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьСоглашениеПоставщикаПоУмолчанию");
	
	ОбработчикиСтрок.Добавить("ЗаполнитьРеквизитыПоСоглашениюПоставщика",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьРеквизитыПоСоглашениюПоставщика");
	
	ОбработчикиСтрок.Добавить("ЗаполнитьКоличествоСоглашенийПоставщика",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьКоличествоСоглашенийПоставщика");
	
	ОбработчикиСтрок.Добавить("ЗаполнитьКонтрагентаПартнераПоУмолчанию",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьКонтрагентаПартнераПоУмолчанию");
	
	ОбработчикиСтрок.Добавить("ЗаполнитьДоговорПоУмолчанию",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьДоговорПоУмолчанию");
	
	ОбработчикиСтрок.Добавить("ЗаполнитьОперациюПоСоглашениюДоговору",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьОперациюПоСоглашениюДоговору");
	
	ОбработчикиСтрок.Добавить("ЗаполнитьНалогообложениеНДСЗакупки",
		"УчетНДСУП.ЗаполнитьПакетноНалогообложениеНДСЗакупки");
	...
	ОбработчикиСтрок.Добавить("ЗаполнитьСтавкуНДСПоВидуЦен",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьСтавкуНДСПоВидуЦенВСтрокеТЧ");
	
	ОбработчикиСтрок.Добавить("ЗаполнитьСтавкуНДССуффиксПоВидуЦен",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьСтавкуНДССуффиксПоВидуЦенВСтрокеТЧ");
	
	УчетНДСЛокализация.ПриДобавленииОбработчиковСтрокКоллекции(ОбработчикиСтрок);
	...
	ОбработчикиСтрок.Добавить("ЗаполнитьПомещениеВСтрокеТЧ",
		"ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ЗаполнитьПомещениеВСтрокеТЧ");
	...
	ОбработчикиСтрок.Добавить("УстановитьСуммуНДСРеглУпр",
		"ПакетнаяОбработкаТабличнойЧастиКлиентСервер.УстановитьСуммуНДСРеглУпрВСтрокеТЧ");
	
	ОбработчикиСтрок.Добавить("ПересчитатьСуммуНДСОтгрузки",
		"ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПересчитатьСуммуНДСОтгрузкиВСтрокеТЧ");
	...
	ОбработчикиСтрок.Добавить("ЗаполнитьКонтролируемуюПартиюОтправителя",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьКонтролируемуюПартиюОтправителя");
	...
	ОбработчикиСтрок.Добавить("ПроверитьСериюПоВладельцу",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ПроверитьСериюПоВладельцу");
	
	ОбработчикиСтрок.Добавить("ПроверитьРегистрациюШтрихкода",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ПроверитьРегистрациюШтрихкода");
	...
	ОбработчикиСтрок.Добавить("ЗаполнитьПредставлениеУсловийЗакупки",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьПредставлениеУсловийЗакупки");
	
	ОбработчикиСтрок.Добавить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества");
		
	ОбработчикиСтрок.Добавить("ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества");
...
Процедура ДополнитьТекстыЗапросовИсточниковДанных(Действия, КэшированныеЗначения)
	...
	ЗакупкиСервер.ДополнитьТекстЗапросаЗаполнитьПараметрыСоглашенийПоставщика(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	ЗакупкиСервер.ДополнитьТекстЗапросаЗаполнитьРеквизитыПоСоглашениюПоставщика(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	ЗакупкиСервер.ДополнитьТекстЗапросаЗаполнитьДоговорПоУмолчанию(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	ЗакупкиСервер.ДополнитьТекстЗапросаЗаполнитьОперациюПоСоглашениюДоговору(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	ЗакупкиСервер.ДополнитьТекстЗапросаЗаполнитьПредставлениеУсловийЗакупки(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	...
	УчетНДСУП.ДополнитьТекстЗапросаСтавкаНДСПоВидуЦен(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	...
	КонтрольКачества.ПакетнаяОбработкаДополнитьИсточникомДанныхКонтрольКачества(Действия, ОписаниеЗапроса,
		КэшированныеЗначения);
	...
	ПартнерыИКонтрагенты.ДополнитьТекстЗапросаЗаполнитьКонтрагентаПартнераПоУмолчанию(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	...
	НоменклатураСервер.ДополнитьТекстЗапросаПроверитьСериюПоВладельцу(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	...
	РегистрыСведений.ШтрихкодыНоменклатуры.ДополнитьТекстЗапросаПроверитьРегистрациюШтрихкода(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	
	УчетПодакцизныхТоваров.ДополнитьТекстЗапросаЗаполнитьПризнакПодакцизныйТовар(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	УчетПодакцизныхТоваров.ДополнитьТекстЗапросаРеквизитыРасчетаАкциза(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	УчетПодакцизныхТоваров.ДополнитьТекстЗапросаПересчитатьСуммуАкциза(Действия, ОписаниеЗапроса, КэшированныеЗначения);
...
Процедура ЗаполнитьДополнительныеКолонки(СтруктураДействий, КэшированныеЗначения)
	...
	УчетНДСУП.ЗаполнитьКолонкуВидЦены(ОбрабатываемаяКоллекция, СтруктураДействий, КэшированныеЗначения);
...
Функция ДанныеФормыКоллекцияВТаблицуЗначений(Коллекция, КэшированныеЗначения, Строки = Неопределено)
	...
	Если ТаблицаИсточник.Колонки.Найти("НомерСтроки") = Неопределено 
		Или ТипЗнч(Коллекция) = Тип("ДанныеФормыКоллекция") Тогда
			Счетчик = 0;
	...
	Если Не КэшированныеЗначения.Свойство("ИнициализацияЗавершена")
		...
		Если ОписаниеЗапроса.ТекстыЗапросов.Количество() > 1 Тогда
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru = 'Не удалось выполнить пакетную обработку строк таблицы.
					|Подробности см. в журнале регистрации.'");
				
				ТекстОшибки = СтрШаблон(НСтр(
						"ru = 'Не удалось выполнить пакетную обработку строк таблицы.
						|Выполняемые действия:
						|%1
						|По причине:
						|%2.'", ОбщегоНазначения.КодОсновногоЯзыка()),
						СтрСоединить(КэшированныеЗначения.МетодыКВыполнению.ВыгрузитьКолонку("Обработка"), Символы.ПС),
						ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				
				ЗаписьЖурналаРегистрации(
					"ПакетнаяОбработкаТабличнойЧастиСервер.ИнициализироватьПакетнуюОбработку", 
					УровеньЖурналаРегистрации.Ошибка,,,
					ТекстОшибки);
...
Процедура ЗаполнитьСтавкуНДСПоВидуЦенТекущейСтроки(ИмяКолонкиСтавкиНДС, ТекущаяСтрока, СтруктураПараметровДействия, КэшированныеЗначения, ВывестиСообщение) Экспорт
	
	Перем ТипНоменклатуры;
	Перем ОрганизацияДляСтавкиНДС;
	Перем ИсходящийДокумент;
	
	Если Не СтруктураПараметровДействия.ИспользуетсяЦенообразование25 Тогда
		Возврат;
	КонецЕсли;
	...
	Если СтруктураПараметровДействия.ВернутьМногооборотнуюТару Тогда
	...
	СтруктураВидаЦен = УчетНДСУП.СтавкаНДСПоВидуЦенПакетно(КэшированныеЗначения);
	...
	Если Не СтруктураВидаЦен.УказыватьСтавкуНДС Тогда
	...
	Если Не КэшированныеЗначения.Свойство("ИспользоватьМногострановойУчет") Тогда
		КэшированныеЗначения.Вставить("ИспользоватьМногострановойУчет", ПолучитьФункциональнуюОпцию("ИспользоватьМногострановойУчет"));
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СтруктураВидаЦен.СтавкаНДС) Тогда
		...
		Если (НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС
			...
			Или СтруктураВидаЦен.ОшибкаПродажиНаЭкспорт Тогда
				
			ТекстОшибки = НСтр("ru = 'Ставка НДС установленная для вида цены, не соответствует указанному в документе налогообложению.'");
		...
		Иначе
			...
			Если КэшированныеЗначения.ИспользоватьМногострановойУчет Тогда
				
				Если ЗначениеЗаполнено(СтруктураПараметровДействия.СтранаРегистрацииОрганизации)
						И Не СтруктураВидаЦен.Страна = СтруктураПараметровДействия.СтранаРегистрацииОрганизации Тогда
					ТекстОшибки = НСтр("ru = 'Страна в виде цен не соответствует стране регистрации организации.'");
				КонецЕсли;
	...
	Иначе
		Если СтруктураВидаЦен.УказыватьСтавкуНДС Тогда
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'На %1 не установлена Ставка НДС для выбранного вида цен.'"),
				Формат(СтруктураПараметровДействия.Дата, НСтр("ru='ДФ=''dd MMMM yyyy'''")));
		КонецЕсли;
	КонецЕсли;
	
	Если ВывестиСообщение И ЗначениеЗаполнено(ТекстОшибки)  Тогда
		...
		ВидЦены = СтруктураВидаЦен.ВидЦены;
		Если ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("ВидЦены", КэшированныеЗначения)
			И ЗначениеЗаполнено(ТекущаяСтрока.ВидЦены) Тогда
			ВидЦены = ТекущаяСтрока.ВидЦены;
		КонецЕсли;
	
		ТекстШаблона = СтрШаблон(
				НСтр("ru = 'Номенклатуру ""%1"" по виду цен ""%2"" продавать нельзя.'") + " ",
				ТекущаяСтрока.Номенклатура,
				ВидЦены);
		...
		Если ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("НомерСтроки", КэшированныеЗначения) Тогда
			...
			ТекстШаблона = СтрШаблон(
				НСтр("ru = 'В строке %1 - номенклатуру ""%2"" по виду цен ""%3"" продавать нельзя.'") + " ",
				ТекущаяСтрока.НомерСтроки,
				ТекущаяСтрока.Номенклатура,
				ВидЦены);
...
Функция ДействиеИсключение(Действие)
	...
	ИсключенияИзДействий.Добавить("ЗаполнитьСтавкуНДСПоВидуЦен");
