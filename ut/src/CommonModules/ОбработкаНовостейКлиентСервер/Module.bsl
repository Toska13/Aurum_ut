...
#Область СостоянияНовостей

// Возвращает структуру со значениями ресурсов для регистра сведений СостоянияНовостей.
// Если передано значение "Неопределено", значит при установке новых значений (используя это значение)
//   надо оставить текущие значения или значения по-умолчанию.
//
// Параметры:
//  Прочтена                - Булево, Неопределено;
//  Пометка                 - Число, Неопределено;
//  ОповещениеВключено      - Булево, Неопределено;
//  ДатаНачалаОповещения    - Дата, Неопределено;
//  УдаленаИзСпискаНовостей - Булево, Неопределено;
//  ПрочитатьПозже          - Булево, Неопределено;
//
// Возвращаемое значение:
//   Структура - см. в коде.
//
Функция НовыйОписательЗначенийСостоянияНовости(
			Прочтена = Неопределено,
			Пометка = Неопределено,
			ОповещениеВключено = Неопределено,
			ДатаНачалаОповещения = Неопределено,
			УдаленаИзСпискаНовостей = Неопределено,
			ПрочитатьПозже = Неопределено) Экспорт
	...
	Результат = Новый Структура;
		Результат.Вставить("Прочтена"               , Неопределено);
		Результат.Вставить("Пометка"                , Неопределено);
		Результат.Вставить("ОповещениеВключено"     , Неопределено);
		Результат.Вставить("ДатаНачалаОповещения"   , Неопределено);
		Результат.Вставить("УдаленаИзСпискаНовостей", Неопределено);
		Результат.Вставить("ПрочитатьПозже"         , Неопределено);

	Если ТипЗнч(Прочтена) = ТипБулево Тогда
		Результат.Прочтена = Прочтена;
	КонецЕсли;
	Если (ТипЗнч(Пометка) = ТипЧисло) Тогда
		Если Пометка < 0 Тогда 
			Результат.Пометка = 0;
		ИначеЕсли Пометка > 9 Тогда 
			Результат.Пометка = 9;
		Иначе
			Результат.Пометка = Пометка;
		КонецЕсли;
	КонецЕсли;
	Если ТипЗнч(ОповещениеВключено) = ТипБулево Тогда
		Результат.ОповещениеВключено = ОповещениеВключено;
	КонецЕсли;
	Если ТипЗнч(ДатаНачалаОповещения) = ТипДата Тогда
		Результат.ДатаНачалаОповещения = ДатаНачалаОповещения;
	КонецЕсли;
	Если ТипЗнч(УдаленаИзСпискаНовостей) = ТипБулево Тогда
		Результат.УдаленаИзСпискаНовостей = УдаленаИзСпискаНовостей;
	КонецЕсли;
	Если ТипЗнч(ПрочитатьПозже) = ТипБулево Тогда
		Результат.ПрочитатьПозже = ПрочитатьПозже;
	КонецЕсли;
...
Функция НовыйКонтекстВыполнения(
	...
	Результат = Новый Структура;
		...
		Результат.Вставить("ЖурналРегистрации", Новый Структура);
			Результат.ЖурналРегистрации.Вставить("ИмяСобытия"        , "БИП:Новости.Сервис и регламент"); // АПК:1297 Это идентификатор.
