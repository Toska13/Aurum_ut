...
Функция ОтправитьСообщения(Организация = Неопределено, ДатаНачала = '00010101', ДатаОкончания = '00010101', ОтправлятьТолькоПолныеПакеты = Ложь) Экспорт
	...
	Пока ВыборкаОрганизации.Следующий() Цикл
		...
		Пока ВыборкаДокументы.Следующий() Цикл
			...
			ВидОперации          = ВидОперации(ВыборкаДокументы);
			НаименованиеОперации = НаименованиеОперации(ВидОперации);
			...
			ДокументСообщения.Операция     = ВидОперации;
			
			ТаблицаПредставленияДокумента  = НовыйТаблицаПредставленияДокумента();
			...
			ДокументСообщения.ПервичныйДокумент = ПредставлениеДокументаПоТаблице(ТаблицаПредставленияДокумента);
	
			ТаблицаПредставленияСФ  = НовыйТаблицаПредставленияДокумента();
			...
			ДокументСообщения.СчетФактура = ПредставлениеДокументаПоТаблице(ТаблицаПредставленияСФ);
	
			ДокументСообщения.Сумма    = ?(ВыборкаДокументы.Сумма >= 0, ВыборкаДокументы.Сумма, - ВыборкаДокументы.Сумма);
			ДокументСообщения.СуммаНДС = ?(ВыборкаДокументы.СуммаНДС >= 0, ВыборкаДокументы.СуммаНДС, - ВыборкаДокументы.СуммаНДС);
...
Процедура ОбработатьУчастниковСверкиПослеОбновленияИдентификаторов() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	ИспользуетсяСервисСверкиРасчетов = ИспользуетсяСервисСверкиРасчетов();
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СервисСверкиРасчетовУчастникиСверкиСОбновленнымиID.УчастникСверки КАК УчастникСверки,
	|	СервисСверкиРасчетовУчастникиСверкиСОбновленнымиID.ИдентификаторДоОбновления <> """"
	|		И СервисСверкиРасчетовУчастникиСверкиСОбновленнымиID.ИдентификаторПослеОбновления = """" КАК УдаленИдентификатор
	|ИЗ
	|	РегистрСведений.СервисСверкиРасчетовУчастникиСверкиСОбновленнымиID КАК СервисСверкиРасчетовУчастникиСверкиСОбновленнымиID
	|ГДЕ
	|	ВЫБОР
	|			КОГДА &ДляАктуализацииКешаКонтрагентов
	|				ТОГДА ТИПЗНАЧЕНИЯ(СервисСверкиРасчетовУчастникиСверкиСОбновленнымиID.УчастникСверки) = ТИП(Справочник.Контрагенты)
	|						И СервисСверкиРасчетовУчастникиСверкиСОбновленнымиID.ИдентификаторДоОбновления = """"
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ";
	
	Запрос.УстановитьПараметр("ДляАктуализацииКешаКонтрагентов", НЕ ИспользуетсяСервисСверкиРасчетов);
	...
	Пока Выборка.Следующий() Цикл
		Если НЕ ИспользуетсяСервисСверкиРасчетов Тогда
			ОбновитьКэшДанныхОРелевантныхКонтрагентах();
			Прервать;
		КонецЕсли;
		ПриАктуализацииIDУчастникаСверки(Выборка.УчастникСверки, Выборка.УдаленИдентификатор);
	КонецЦикла;
	
	// Очистим регистр.
	НаборЗаписейРасхождения = РегистрыСведений.СервисСверкиРасчетовУчастникиСверкиСОбновленнымиID.СоздатьНаборЗаписей();
	НаборЗаписейРасхождения.Записать();
	
	// Отключаем рег задание по обработке участников сверки.
	ОпределитьНеобходимостьОбработкиУчастниковСверки();
...
Процедура ЗарегистрироватьКСверкеРанееВведенныеДокументы(МассивОрганизаций = Неопределено, МассивКонтрагентов = Неопределено, ЭтоВключениеСервиса = Ложь) Экспорт
	
	Если Не ЭтоВключениеСервиса
		И Не ИспользуетсяСервисСверкиРасчетов() Тогда
		Возврат;
	КонецЕсли;
...
Функция ИтогиСверки(Организация = Неопределено, Контрагент = Неопределено, ДатаНачала = '00010101', ДатаОкончания = '00010101', ИдентификаторыРасхождений = Неопределено, ВключатьОбособленныеПодразделения = Ложь) Экспорт
	
	СтруктураИтогов = Новый Структура();
	СтруктураИтогов.Вставить("ВсегоДокументов", 0);
	СтруктураИтогов.Вставить("НеСверено",       0);
	СтруктураИтогов.Вставить("Сверено",         0);
	СтруктураИтогов.Вставить("Критичные",       0);
	СтруктураИтогов.Вставить("Незначительные",  0);
	СтруктураИтогов.Вставить("БезРасхождений",  0);
	...
	|ПОМЕСТИТЬ КонтрагентыОтКоторыхОжидаемДокументы
	|ИЗ
	|	РегистрСведений.СервисСверкиРасчетовОбнаруженныеРасхождения КАК СервисСверкиРасчетовОбнаруженныеРасхождения
	|ГДЕ
	|	НЕ &ОтбиратьПоИдентификаторам
	|	И (НЕ &ОтбиратьПоОрганизации
	|			ИЛИ СервисСверкиРасчетовОбнаруженныеРасхождения.Организация = &Организация
	|			ИЛИ ВЫБОР
	|				КОГДА &ВключатьОбособленныеПодразделения
	|					ТОГДА СервисСверкиРасчетовОбнаруженныеРасхождения.Организация.ГоловнаяОрганизация = &Организация
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ)
	|	И (НЕ &ОтбиратьПоКонтрагенту
	|			ИЛИ СервисСверкиРасчетовОбнаруженныеРасхождения.Контрагент = &Контрагент
	|			ИЛИ СервисСверкиРасчетовОбнаруженныеРасхождения.Контрагент В ИЕРАРХИИ (&Контрагент))
	...
	|	ВЫБОР
	|		КОГДА СервисСверкиРасчетовРеестрДокументов.Статус = ЗНАЧЕНИЕ(Перечисление.СервисСверкиРасчетовСтатусыДокументов.КОтправке)
	|				ИЛИ НЕ КонтрагентыОтКоторыхОжидаемДокументы.Контрагент ЕСТЬ NULL
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК НеСверено,
	|	ВЫБОР
	|		КОГДА СервисСверкиРасчетовРеестрДокументов.Статус = ЗНАЧЕНИЕ(Перечисление.СервисСверкиРасчетовСтатусыДокументов.Получено)
	|				И КонтрагентыОтКоторыхОжидаемДокументы.Контрагент ЕСТЬ NULL
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Сверено,
	|	0 КАК Критичные,
	|	0 КАК Незначительные,
	|	0 КАК ЕстьРасхождение
	|ПОМЕСТИТЬ ИтогиРасхожденийПредварительная
	|ИЗ
	|	РегистрСведений.СервисСверкиРасчетовРеестрДокументов КАК СервисСверкиРасчетовРеестрДокументов
	|		ЛЕВОЕ СОЕДИНЕНИЕ КонтрагентыОтКоторыхОжидаемДокументы КАК КонтрагентыОтКоторыхОжидаемДокументы
	|		ПО СервисСверкиРасчетовРеестрДокументов.Организация = КонтрагентыОтКоторыхОжидаемДокументы.Организация
	|			И СервисСверкиРасчетовРеестрДокументов.Контрагент = КонтрагентыОтКоторыхОжидаемДокументы.Контрагент
	|ГДЕ
	|	НЕ &ОтбиратьПоИдентификаторам
	|	И (НЕ &ОтбиратьПоОрганизации
	|			ИЛИ СервисСверкиРасчетовРеестрДокументов.Организация = &Организация
	|			ИЛИ ВЫБОР
	|				КОГДА &ВключатьОбособленныеПодразделения
	|					ТОГДА СервисСверкиРасчетовРеестрДокументов.Организация.ГоловнаяОрганизация = &Организация
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ)
	|	И (НЕ &ОтбиратьПоКонтрагенту
	|			ИЛИ СервисСверкиРасчетовРеестрДокументов.Контрагент = &Контрагент
	|			ИЛИ СервисСверкиРасчетовРеестрДокументов.Контрагент В ИЕРАРХИИ (&Контрагент))
	|	И СервисСверкиРасчетовРеестрДокументов.ДатаДокумента >= &НачалоПериода
	|	И СервисСверкиРасчетовРеестрДокументов.ДатаДокумента <= &КонецПериода
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА &ОтбиратьПоИдентификаторам
	|			ТОГДА 1
	|		КОГДА СервисСверкиРасчетовОбнаруженныеРасхождения.ПервичныйДокументОрганизации = """"
	|				И СервисСверкиРасчетовОбнаруженныеРасхождения.ПервичныйДокументКонтрагента <> """"
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	0,
	...
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СервисСверкиРасчетовРеестрДокументов.Статус, ЗНАЧЕНИЕ(Перечисление.СервисСверкиРасчетовСтатусыДокументов.Получено)) <> ЗНАЧЕНИЕ(Перечисление.СервисСверкиРасчетовСтатусыДокументов.КОтправке)
	|				И (СервисСверкиРасчетовОбнаруженныеРасхождения.СуммаПоДаннымКонтрагента <> СервисСверкиРасчетовОбнаруженныеРасхождения.СуммаПоДаннымОрганизации
	|					ИЛИ СервисСверкиРасчетовОбнаруженныеРасхождения.СуммаВалПоДаннымКонтрагента <> 0
	|						И СервисСверкиРасчетовОбнаруженныеРасхождения.СуммаВалПоДаннымОрганизации <> 0
	|						И СервисСверкиРасчетовОбнаруженныеРасхождения.СуммаВалПоДаннымКонтрагента <> СервисСверкиРасчетовОбнаруженныеРасхождения.СуммаВалПоДаннымОрганизации
	|					ИЛИ НЕ(СервисСверкиРасчетовОбнаруженныеРасхождения.ОперацияКонтрагента = &ОперацияПоступление
	|								И СервисСверкиРасчетовОбнаруженныеРасхождения.НДСПоДаннымКонтрагента = 0
	|								И СервисСверкиРасчетовОбнаруженныеРасхождения.НДСПоДаннымОрганизации <> СервисСверкиРасчетовОбнаруженныеРасхождения.НДСПоДаннымКонтрагента)
	|						И (СервисСверкиРасчетовОбнаруженныеРасхождения.СчетФактураОрганизации <> СервисСверкиРасчетовОбнаруженныеРасхождения.СчетФактураКонтрагента
	|							ИЛИ СервисСверкиРасчетовОбнаруженныеРасхождения.НДСПоДаннымКонтрагента <> СервисСверкиРасчетовОбнаруженныеРасхождения.НДСПоДаннымОрганизации))
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА СервисСверкиРасчетовОбнаруженныеРасхождения.СуммаПоДаннымКонтрагента <> СервисСверкиРасчетовОбнаруженныеРасхождения.СуммаПоДаннымОрганизации
	|				ИЛИ СервисСверкиРасчетовОбнаруженныеРасхождения.СуммаВалПоДаннымКонтрагента <> 0
	|					И СервисСверкиРасчетовОбнаруженныеРасхождения.СуммаВалПоДаннымОрганизации <> 0
	|					И СервисСверкиРасчетовОбнаруженныеРасхождения.СуммаВалПоДаннымКонтрагента <> СервисСверкиРасчетовОбнаруженныеРасхождения.СуммаВалПоДаннымОрганизации
	|				ИЛИ НЕ(СервисСверкиРасчетовОбнаруженныеРасхождения.ОперацияКонтрагента = &ОперацияПоступление
	...
	|		ПО СервисСверкиРасчетовОбнаруженныеРасхождения.Организация = СервисСверкиРасчетовРеестрДокументов.Организация
	|			И СервисСверкиРасчетовОбнаруженныеРасхождения.Контрагент = СервисСверкиРасчетовРеестрДокументов.Контрагент
	|			И СервисСверкиРасчетовОбнаруженныеРасхождения.Документ = СервисСверкиРасчетовРеестрДокументов.Документ
	|ГДЕ
	|	(НЕ &ОтбиратьПоОрганизации
	|			ИЛИ СервисСверкиРасчетовОбнаруженныеРасхождения.Организация = &Организация
	|			ИЛИ ВЫБОР
	|				КОГДА &ВключатьОбособленныеПодразделения
	|					ТОГДА СервисСверкиРасчетовОбнаруженныеРасхождения.Организация.ГоловнаяОрганизация = &Организация
	...
	|			ИЛИ СервисСверкиРасчетовОбнаруженныеРасхождения.Контрагент = &Контрагент
	|			ИЛИ СервисСверкиРасчетовОбнаруженныеРасхождения.Контрагент В ИЕРАРХИИ (&Контрагент))
	|	И (НЕ &ОтбиратьПоИдентификаторам
	|			ИЛИ СервисСверкиРасчетовОбнаруженныеРасхождения.ИдентификаторРасхождения В (&ИдентификаторыРасхождений))
	|	И СервисСверкиРасчетовОбнаруженныеРасхождения.Дата >= &НачалоПериода
	|	И СервисСверкиРасчетовОбнаруженныеРасхождения.Дата <= &КонецПериода
	|	И СервисСверкиРасчетовОбнаруженныеРасхождения.КонтрагентНеПроизводилСверку = ЛОЖЬ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(СУММА(ИтогиРасхожденийПредварительная.ВсегоДокументов), 0) КАК ВсегоДокументов,
	|	ЕСТЬNULL(СУММА(ИтогиРасхожденийПредварительная.НеСверено), 0) КАК НеСверено,
	|	ЕСТЬNULL(СУММА(ИтогиРасхожденийПредварительная.Сверено), 0) КАК Сверено,
	|	ЕСТЬNULL(СУММА(ИтогиРасхожденийПредварительная.Критичные), 0) КАК Критичные,
	|	ЕСТЬNULL(СУММА(ИтогиРасхожденийПредварительная.Незначительные), 0) КАК Незначительные,
	|	ЕСТЬNULL(СУММА(ИтогиРасхожденийПредварительная.ЕстьРасхождение), 0) КАК СРасхождениями
	|ИЗ
	|	ИтогиРасхожденийПредварительная КАК ИтогиРасхожденийПредварительная";
	
	Запрос.УстановитьПараметр("ОтбиратьПоОрганизации", ЗначениеЗаполнено(Организация));
	Запрос.УстановитьПараметр("ВключатьОбособленныеПодразделения", ВключатьОбособленныеПодразделения);
	Запрос.УстановитьПараметр("Организация", Организация); 
	Запрос.УстановитьПараметр("ОтбиратьПоКонтрагенту", ЗначениеЗаполнено(Контрагент));
	Запрос.УстановитьПараметр("Контрагент", Контрагент);
	Запрос.УстановитьПараметр("ОтбиратьПоИдентификаторам", ЗначениеЗаполнено(ИдентификаторыРасхождений));
	Запрос.УстановитьПараметр("ИдентификаторыРасхождений", ИдентификаторыРасхождений);
	Запрос.УстановитьПараметр("НачалоПериода", ДатаНачала);
	Запрос.УстановитьПараметр("КонецПериода", ДатаОкончания);
	Запрос.УстановитьПараметр("ОперацияПоступление", Перечисления.СервисСверкиРасчетовВидыОпераций.Поступление);
	...
	Если ЗначениеЗаполнено(ИдентификаторыРасхождений) Тогда
		СтруктураИтогов.ВсегоДокументов = 0;
		СтруктураИтогов.НеСверено       = 0;
		СтруктураИтогов.БезРасхождений  = 0;
		СтруктураИтогов.Сверено          = Выборка.Критичные + Выборка.Незначительные;
	Иначе
		СтруктураИтогов.БезРасхождений = Выборка.Сверено - Выборка.Критичные - Выборка.Незначительные;
	КонецЕсли;
...
Процедура ЗарегистрироватьОрганизацииВСервисе(АдресХранилища) Экспорт
	...
	Для Каждого Организация Из ОрганизацииДляПодключения Цикл
		ТекстОшибки = "";
		ПриАктуализацииIDУчастникаСверки(Организация, Истина, ТекстОшибки, Истина);
		НоваяСтрокаОшибок = ТаблицаОшибокРегистрации.Добавить();
		НоваяСтрокаОшибок.Организация = Организация;
		НоваяСтрокаОшибок.ТекстОшибки = ТекстОшибки;
	КонецЦикла;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВключитьИспользованиеОбменаБизнесСети");
	
	Методы.Вставить("СверитьРасчетыРегламентноеЗадание", Истина);
	Методы.Вставить("ОбработатьУчастниковСверкиПослеОбновленияИдентификаторов", Истина);
	Методы.Вставить("ЗарегистрироватьОрганизацииВСервисе", Истина);
	Методы.Вставить("ОбменятьсяССервисом", Истина);
...
Функция ТекстБаннераСервисСверкиРасчетовКнигаПокупок() Экспорт
	...
	Массив = Новый Массив;
	Массив.Добавить(Новый ФорматированнаяСтрока(Нстр("ru = 'Какие счета-фактуры ваши продавцы ввели в программу с ошибками?'"), ШрифЗаголовка));
	Массив.Добавить(Символы.ПС);
	Массив.Добавить(Новый ФорматированнаяСтрока(" ", Новый Шрифт(,5)));
	Массив.Добавить(Символы.ПС);
	Массив.Добавить(Новый ФорматированнаяСтрока(Нстр("ru = 'Включите сервис'") + " ", ШрифтТекста));
	Массив.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = '1С:Сверка 2.0'"), ШрифтТекста, , , "ПодключитьсяКСервисуСверки"));
	Массив.Добавить(Новый ФорматированнаяСтрока(Нстр("ru = ' и узнавайте о расхождениях в книге покупок заранее'"), ШрифтТекста));
...
Функция ТекстБаннераСервисСверкиРасчетовКнигаПродаж() Экспорт
	...
	Массив = Новый Массив;
	Массив.Добавить(Новый ФорматированнаяСтрока(Нстр("ru = 'Какие счета-фактуры ваши покупатели ввели в программу с ошибками?'"), ШрифЗаголовка));
	Массив.Добавить(Символы.ПС);
	Массив.Добавить(Новый ФорматированнаяСтрока(" ", Новый Шрифт(,5)));
	Массив.Добавить(Символы.ПС);
	Массив.Добавить(Новый ФорматированнаяСтрока(Нстр("ru = 'Включите сервис'") + " ", ШрифтТекста));
	Массив.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = '1С:Сверка 2.0'"), ШрифтТекста, , , "ПодключитьсяКСервисуСверки"));
	Массив.Добавить(Новый ФорматированнаяСтрока(Нстр("ru = ' и узнавайте о расхождениях в книге продаж заранее'"), ШрифтТекста));
...
Функция ТекстБаннераСервисСверкиРасчетовДекларацияПоНДС() Экспорт
	...
	Массив = Новый Массив;
	Массив.Добавить(Новый ФорматированнаяСтрока(Нстр("ru = 'Какие счета-фактуры ваши контрагенты ввели в программу с ошибками?'"), ШрифЗаголовка));
	Массив.Добавить(Символы.ПС);
	Массив.Добавить(Новый ФорматированнаяСтрока(" ", Новый Шрифт(,5)));
	Массив.Добавить(Символы.ПС);
	Массив.Добавить(Новый ФорматированнаяСтрока(Нстр("ru = 'Включите сервис'") + " ", ШрифтТекста));
	Массив.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = '1С:Сверка 2.0'"), ШрифтТекста, , , "ПодключитьсяКСервисуСверки"));
	Массив.Добавить(Новый ФорматированнаяСтрока(Нстр("ru = ' и узнавайте о расхождениях в Декларации по НДС заранее'"), ШрифтТекста));
...
Функция ТекстБаннераСервисСверкиРасчетовПомощникРасчетаНДС() Экспорт
	...
	Массив.Добавить(Новый ФорматированнаяСтрока(Нстр("ru = 'Включите сервис'") + " ", ШрифтТекста));
	Массив.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = '1С:Сверка 2.0'"), ШрифтТекста, , , "ПодключитьсяКСервисуСверки"));
	Массив.Добавить(Новый ФорматированнаяСтрока(Нстр("ru = ' и узнавайте о расхождениях в книгах покупок, продаж и в Декларации по НДС заранее'"), ШрифтТекста));
...
// Определяет, нужно ли обработать участников сверки с обновленными иденитификаторами, и исходя из этого 
// отключает или планирует регламентное задание.
// 
Процедура ОпределитьНеобходимостьОбработкиУчастниковСверки() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	ИспользованиеРегЗадания = ЕстьЗадачиКОбработкеРегЗаданием();
	
	Расписание = РасписаниеРегЗаданияОбработкиУчастниковСверки();
	
	ПараметрыЗадания = Новый Структура;
	ПараметрыЗадания.Вставить("Метаданные", Метаданные.РегламентныеЗадания.СервисСверкиРасчетовОбработатьУчастниковСверки);
	...
	Если ИспользованиеРегЗадания Тогда // добавим новое рег.задание
		...
		ПараметрыЗадания.Вставить("Наименование", НСтр("ru = '1С:Сверка 2.0 (обработка после изменения ID)'"));
...
Процедура ПолучитьРасхождения(Организация, ДатаНачала, ДатаОкончания, ЭтоВызовИзОтчета)
	...
	Для Каждого ТекущаяОрганизация Из ПодключенныеОрганизации Цикл
		...
		Если ПолученыРасхожденияПоОрганизации Тогда 
			ИмяОповещения = "ПолучениеНовыхРасхожденийПоОрганизации";
...
Функция ВидОперации(Выборка)
	...
	Возврат СервисСверкиРасчетовПереопределяемый.ВидОперации(
		Выборка.Документ, ДополнительныеСведенияДляОпределенияОперации);
	
КонецФункции

Функция НаименованиеОперации(ВидОперации)
	
	ИндексЗначенияПеречисления = Перечисления.СервисСверкиРасчетовВидыОпераций.Индекс(ВидОперации);
	Возврат Метаданные.Перечисления.СервисСверкиРасчетовВидыОпераций.ЗначенияПеречисления[ИндексЗначенияПеречисления].Имя;
...
Функция ПреобразоватьТекстВТаблицу(ПараметрыМетода, ДанныеСервиса)
	...
	Для Каждого ТекущаяСтрока Из ДанныеСервиса Цикл
		...
		Для Каждого ТекущаяСтрокаДетально Из ТекущаяСтрока.ДокументыПоДаннымОрганизации Цикл
			...
			НовыйДокументОрганизации.Операция   = СтруктураДанныхДокумента.Операция;
			...
			НовыйДокументОрганизации.Количество = 1; 
			...
			// НДС всегда общей суммой.
			НДСПоДаннымОрганизации      = СтруктураДанныхДокумента.НДС;
		...
		Для Каждого ТекущаяСтрокаДетально Из ТекущаяСтрока.ДокументыПоДаннымКонтрагента Цикл
			...
			НовыйДокументКонтрагента.Операция   = СтруктураДанныхДокумента.Операция;
			...
			НовыйДокументКонтрагента.Количество = 1;
			...
			// НДС всегда общей суммой.
			НДСПоДаннымКонтрагента      = СтруктураДанныхДокумента.НДС;
		...
		Если ТаблицаДокументовОрганизации.Количество() > 0 Тогда
			НоваяСтрока.ОперацияОрганизации = ТаблицаДокументовОрганизации[0].Операция;
		КонецЕсли;
		Если ТаблицаДокументовКонтрагента.Количество() > 0 Тогда
			НоваяСтрока.ОперацияКонтрагента = ТаблицаДокументовКонтрагента[0].Операция;
		КонецЕсли;
...
Функция НовыйТаблицаРасхождений()
	...
	ТаблицаПараметров.Колонки.Добавить("ОперацияОрганизации",                    Новый ОписаниеТипов("ПеречислениеСсылка.СервисСверкиРасчетовВидыОпераций"));
	ТаблицаПараметров.Колонки.Добавить("ОперацияКонтрагента",                    Новый ОписаниеТипов("ПеречислениеСсылка.СервисСверкиРасчетовВидыОпераций"));
...
Процедура ОтправитьСообщенияОРасхождениях(Пользователи)
	...
	Для Каждого Пользователь Из УстраненныеРасхожденияПоПользователям Цикл
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Обнаружены устраненные расхождения на %1'"), Формат(ТекущаяДатаСеанса(), "ДЛФ=Д"));
...
Функция НовыйТаблицаПредставленияДокумента() Экспорт
	
	ТаблицаПредставлениеДокумента= Новый ТаблицаЗначений();
	ТаблицаПредставлениеДокумента.Колонки.Добавить("Операция", Новый ОписаниеТипов("ПеречислениеСсылка.СервисСверкиРасчетовВидыОпераций"));
	...
	ТаблицаПредставлениеДокумента.Колонки.Добавить("Количество", ОбщегоНазначения.ОписаниеТипаЧисло(2,0));
	
	Возврат ТаблицаПредставлениеДокумента;
	
КонецФункции

Функция ПредставлениеДокументаПоТаблице(ТаблицаДокументов) Экспорт
	
	ТаблицаДокументов.Свернуть("Номер, Дата", "Количество");
	...
	Для Каждого ТекущийДокумент из ТаблицаДокументов Цикл
		Если НЕ ЗначениеЗаполнено(ТекущийДокумент.Номер)
			...
			Продолжить;
		КонецЕсли;
		НесколькоДокументов = ТекущийДокумент.Количество > 1;
		ТекущийДокументОбОтгрузке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПредставления,
			...
			?(НесколькоДокументов, " (и еще " + Строка(ТекущийДокумент.Количество - 1)+ ")", ""));
		Если ЗначениеЗаполнено(ПредставлениеДокумента) Тогда
			ПредставлениеДокумента = ПредставлениеДокумента + "; " + ТекущийДокументОбОтгрузке; 
		Иначе
...
Функция СообщениеОНовыхРасхожденияхДляПользователя(ВыборкаПоОрганизациям, ПользовательВсегоНДС, ПользовательВсегоРасхождений, ПользовательВсегоСумма)
	...
	ЧастиСообщения = Новый Массив;
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	НСтр("ru='Обнаружены расхождения на %1'"), Формат(ТекущаяДатаСеанса(), "ДЛФ=Д"));
	ЧастиСообщения.Добавить(Новый ФорматированнаяСтрока(ТекстСообщения, Новый Шрифт(Новый Шрифт,, 14)));
	ЧастиСообщения.Добавить(Символы.ПС);
	ЧастиСообщения.Добавить(Символы.ПС);
	...
	ЧастиСообщения.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Новые расхождения:'"), Новый Шрифт(Новый Шрифт,, 12)));
	ЧастиСообщения.Добавить(Символы.ПС);
	...
	Пока ВыборкаПоОрганизациям.Следующий() Цикл
		...
		Если Не СообщениеПоПользователюСформировано Тогда
			ЧастиСообщения.Добавить(Символы.ПС);
			ЧастиСообщения.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Наша организация:'"),,ЦветаСтиля.СервисСверкиРасчетовЦветТекстаПодсказки));
			ЧастиСообщения.Добавить(Символы.ПС);
		...
		Пока ВыборкаПоКонтрагентам.Следующий() Цикл
			...
			Если НЕ СообщениеПоОрганизацииСформировано Тогда
				ЧастиСообщения.Добавить(Символы.ПС);
				ЧастиСообщения.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='  Контрагент:'"),,ЦветаСтиля.ЦветТекстаДополнительныеСведения));
				ЧастиСообщения.Добавить(Символы.ПС);
				...
				Иначе
					КонтрагентНеНайден = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Контрагент не найден ИНН:%1 КПП:%2'"), ВыборкаПоКонтрагентам.ИННКонтрагента, ВыборкаПоКонтрагентам.КППКонтрагента);
				...
				Иначе
					...
					Если Не СообщениеПоОрганизацииСформировано Тогда
						Если ЗначениеЗаполнено(ВыборкаПоДокументам.Документ) Тогда
							...
							НСтр("ru='Документ %1'"),ВыборкаПоДокументам.ПервичныйДокументОрганизации);
							Документ = Новый ФорматированнаяСтрока(ПредставлениеДокумента, , , , ПолучитьНавигационнуюСсылку(ВыборкаПоДокументам.Документ));
						Иначе
							ПредставлениеДокумента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='<Документ %1>'"),ВыборкаПоДокументам.ПервичныйДокументКонтрагента);
						...
						ТекстСуммаДокумента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru=' на сумму %1 руб.'"), Формат(ВыборкаПоДокументам.СуммаДокумента, "ЧЦ=10; ЧДЦ=2"));
						ТекстСуммаДокумента =
...
Процедура ДополнитьСообщениеОставшимисяРасхождениями(ОсталасьСумма, ОсталосьРасхождений, ОсталсяНДС, ЧастиСообщения)
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='  и еще %1 на сумму %2 руб.'"), ПредставлениеКоличестваРасхождений, Формат(ОсталасьСумма, "ЧЦ=10; ЧДЦ=2"));
...
Функция УстраненныеРасхожденияПоПользователям(МенеджерВременныхТаблиц)
	...
	Пока ВыборкаПоПользователям.Следующий() Цикл
		...
		ЧастиСообщения = Новый Массив;
		ЧастиСообщения.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Устранены расхождения по контрагентам:'"), Новый Шрифт(Новый Шрифт,, 12)));
		ЧастиСообщения.Добавить(Символы.ПС);
		ЧастиСообщения.Добавить(Символы.ПС); 
...
Процедура ДополнитьКэшКонтрагентовБизнесСетиДаннымиСервиса(ТаблицаПоДаннымСервиса)
	...
	|		ПО ДанныеСервиса.Контрагент = СервисСверкиРасчетовУстаревшиеIDУчастниковСверки.УчастникСверки
	|ГДЕ
	|	КонтрагентыБизнесСеть.Контрагент ЕСТЬ NULL
	|			И СервисСверкиРасчетовУстаревшиеIDУчастниковСверки.УчастникСверки ЕСТЬ NULL
...
Процедура ПриАктуализацииIDУчастникаСверки(УчастникСверки, ОтсутствуетИдентификатор = Ложь, ТекстОшибки = "", ЭтоВключениеСервиса = Ложь) Экспорт
	...
	Если ТипЗнч(УчастникСверки) = Тип("СправочникСсылка.Контрагенты") Тогда
		
		// Очистим документы к сверке по контрагенту.
		РегистрыСведений.СервисСверкиРасчетовРеестрДокументов.ОчиститьДанныеПоРегистру(Неопределено, УчастникСверки);
		
		МассивКонтрагентов = Новый Массив;
		МассивКонтрагентов.Добавить(УчастникСверки);
		ЗарегистрироватьКСверкеРанееВведенныеДокументы(Неопределено, МассивКонтрагентов);
		
	ИначеЕсли ТипЗнч(УчастникСверки) = Тип("СправочникСсылка.Организации") Тогда
		
		Ошибка = Ложь;
		Если ОтсутствуетИдентификатор И Не БизнесСеть.ОрганизацияПодключена(УчастникСверки) Тогда
			// Переподключим организацию к сервису.
			РезультатПодключенияОрганизациюКСервису = СервисСверкиРасчетовМенеджерОбмена.РезультатПодключенияОрганизациюКСервису(УчастникСверки);
			
			Если ЗначениеЗаполнено(РезультатПодключенияОрганизациюКСервису.ТекстОшибки) Тогда
				Ошибка = Истина;
				ТекстОшибки = РезультатПодключенияОрганизациюКСервису.ТекстОшибки;
				МетаданныеОбъекта = Метаданные.Справочники.Организации;
				ПодробныйТекстОшибки = СтрШаблон(НСтр("ru='Не удалось обновить данные авторизации по организации %1 по причине: %2'"),
					УчастникСверки, РезультатПодключенияОрганизациюКСервису.ТекстОшибки);
				ЗаписьЖурналаРегистрации(СервисСверкиРасчетов.ИмяСервиса(), УровеньЖурналаРегистрации.Ошибка, МетаданныеОбъекта,, ПодробныйТекстОшибки);
			КонецЕсли;
		КонецЕсли;
		
		// Очистим документы к сверке по организации.
		РегистрыСведений.СервисСверкиРасчетовРеестрДокументов.ОчиститьДанныеПоРегистру(УчастникСверки, Неопределено);
		
		Если Не Ошибка Тогда
			МассивОрганизаций = Новый Массив;
			МассивОрганизаций.Добавить(УчастникСверки);
			ЗарегистрироватьКСверкеРанееВведенныеДокументы(МассивОрганизаций, Неопределено, ЭтоВключениеСервиса);
		КонецЕсли;
...
Функция ЕстьЗадачиКОбработкеРегЗаданием()
	...
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	СервисСверкиРасчетовУчастникиСверкиСОбновленнымиID.УчастникСверки
	|ИЗ
	|	РегистрСведений.СервисСверкиРасчетовУчастникиСверкиСОбновленнымиID КАК СервисСверкиРасчетовУчастникиСверкиСОбновленнымиID";
...
Функция РасписаниеРегЗаданияОбработкиУчастниковСверки()
...
// Очищает регистры сверки до даты начала работы сервиса.
//
Процедура ОчиститьРегистрыСверкиДоДатыНачалаСверки() Экспорт
	...
	|	СервисСверкиРасчетовРеестрДокументов.Операция КАК Операция,
	|	СервисСверкиРасчетовРеестрДокументов.ПервичныйДокумент КАК ПервичныйДокумент,
	|	СервисСверкиРасчетовРеестрДокументов.СчетФактура КАК СчетФактура,
	|	СервисСверкиРасчетовРеестрДокументов.Сумма КАК Сумма,
	|	СервисСверкиРасчетовРеестрДокументов.СуммаНДС КАК СуммаНДС
	|ИЗ
	|	РегистрСведений.СервисСверкиРасчетовРеестрДокументов КАК СервисСверкиРасчетовРеестрДокументов
	|ГДЕ
	|	СервисСверкиРасчетовРеестрДокументов.ДатаДокумента < &ДатаНачалаСверкиРасчетов
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СервисСверкиРасчетовОбнаруженныеРасхождения.КППКонтрагента КАК КППКонтрагента,
	|	СервисСверкиРасчетовОбнаруженныеРасхождения.КонтрагентНеПроизводилСверку КАК КонтрагентНеПроизводилСверку,
	|	СервисСверкиРасчетовОбнаруженныеРасхождения.ОперацияОрганизации КАК ОперацияОрганизации,
	|	СервисСверкиРасчетовОбнаруженныеРасхождения.ОперацияКонтрагента КАК ОперацияКонтрагента,
	...
	|	РегистрСведений.СервисСверкиРасчетовОбнаруженныеРасхождения КАК СервисСверкиРасчетовОбнаруженныеРасхождения
	|ГДЕ
	|	СервисСверкиРасчетовОбнаруженныеРасхождения.ПериодСверки < &ДатаНачалаСверкиРасчетов
	|	И СервисСверкиРасчетовОбнаруженныеРасхождения.ПериодСверки <> ДАТАВРЕМЯ(1, 1, 1)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СервисСверкиРасчетовДанныеСервиса.ПериодСверки КАК ПериодСверки,
	...
	|	СервисСверкиРасчетовДанныеСервиса.Сумма КАК Сумма,
	|	СервисСверкиРасчетовДанныеСервиса.СуммаВал КАК СуммаВал,
	...
	|	СервисСверкиРасчетовДанныеСервиса.НомерСчетФактуры КАК НомерСчетФактуры,
	|	СервисСверкиРасчетовДанныеСервиса.ДатаСчетФактуры КАК ДатаСчетФактуры,
	|	СервисСверкиРасчетовДанныеСервиса.Операция КАК Операция,
	|	СервисСверкиРасчетовДанныеСервиса.ПредставлениеДокумента КАК ПредставлениеДокумента
	|ИЗ
	|	РегистрСведений.СервисСверкиРасчетовДанныеСервиса КАК СервисСверкиРасчетовДанныеСервиса
	|ГДЕ
	|	СервисСверкиРасчетовДанныеСервиса.ПериодСверки < &ДатаНачалаСверкиРасчетов
	|	И СервисСверкиРасчетовДанныеСервиса.ПериодСверки <> ДАТАВРЕМЯ(1, 1, 1)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СервисСверкиРасчетовОчередьЗапросов.ЗапросОбрабатывается КАК ЗапросОбрабатывается,
	|	СервисСверкиРасчетовОчередьЗапросов.ВремяБлокировки КАК ВремяБлокировки
	|ИЗ
	|	РегистрСведений.СервисСверкиРасчетовОчередьЗапросов КАК СервисСверкиРасчетовОчередьЗапросов
	|ГДЕ
	|	СервисСверкиРасчетовОчередьЗапросов.ПериодСверки < &ДатаНачалаСверкиРасчетов
	|	И СервисСверкиРасчетовОчередьЗапросов.ПериодСверки <> ДАТАВРЕМЯ(1, 1, 1)";
	
	ДатаНачалаСверкиРасчетов = ДатаНачалаСверкиРасчетов();
	Запрос.УстановитьПараметр("ДатаНачалаСверкиРасчетов", ДатаНачалаСверкиРасчетов);
	
	Результат = Запрос.ВыполнитьПакет();
	
	ЗаписиКУдалению = Результат[0].Выбрать();
	ИмяРегистра     = "СервисСверкиРасчетовРеестрДокументов";
	ОчиститьРегистрДоДатыНачалаСверки(ИмяРегистра, ЗаписиКУдалению, ДатаНачалаСверкиРасчетов);
	
	ЗаписиКУдалению = Результат[1].Выбрать();
	ИмяРегистра     = "СервисСверкиРасчетовОбнаруженныеРасхождения";
	ОчиститьРегистрДоДатыНачалаСверки(ИмяРегистра, ЗаписиКУдалению, ДатаНачалаСверкиРасчетов);
	
	ЗаписиКУдалению = Результат[2].Выбрать();
	ИмяРегистра     = "СервисСверкиРасчетовДанныеСервиса";
	ОчиститьРегистрДоДатыНачалаСверки(ИмяРегистра, ЗаписиКУдалению, ДатаНачалаСверкиРасчетов);
	
	ЗаписиКУдалению = Результат[3].Выбрать();
	ИмяРегистра     = "СервисСверкиРасчетовОчередьЗапросов";
	ОчиститьРегистрДоДатыНачалаСверки(ИмяРегистра, ЗаписиКУдалению, ДатаНачалаСверкиРасчетов);
	
КонецПроцедуры

Процедура ОчиститьРегистрДоДатыНачалаСверки(ИмяРегистра, ЗаписиКУдалению, ДатаНачалаСверкиРасчетов)
	
	Если ЗаписиКУдалению.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	МаксимальныйРазмерПакета = 1000; // максимальное количество записей к удалению в одном пакете
	КоличествоЗаписейВПакете = 0;
	...
	Пока ЗаписиКУдалению.Следующий() Цикл
		...
		КоличествоЗаписейВПакете = КоличествоЗаписейВПакете + 1;
		Если КоличествоЗаписейВПакете >= МаксимальныйРазмерПакета Тогда
			УдалитьПакетЗаписейРегистра(ДатаНачалаСверкиРасчетов, ИмяРегистра, НаборЗаписей);
			КоличествоЗаписейВПакете = 0;
			НаборЗаписей = РегистрыСведений[ИмяРегистра].СоздатьНаборЗаписей();
			НаборЗаписей.РасширенныеРежимыЗамещения = Истина;
		КонецЕсли;
	...
	Если КоличествоЗаписейВПакете > 0 Тогда 
		УдалитьПакетЗаписейРегистра(ДатаНачалаСверкиРасчетов, ИмяРегистра, НаборЗаписей);
	КонецЕсли;

КонецПроцедуры

Процедура УдалитьПакетЗаписейРегистра(ДатаНачалаСверкиРасчетов, ИмяРегистра, НаборЗаписей)
	
	Попытка
		НаборЗаписей.Записать(РежимЗамещения.Удаление);
	Исключение
		ШаблонОшибки = НСтр("ru = 'Не удалось очистить регистр %1 до %2.
		|Подробное описание ошибки:
		|%3'");
		ТекстОшибки = СтрШаблон(ШаблонОшибки,
		ИмяРегистра,
		Формат(ДатаНачалаСверкиРасчетов, "ДФ='MMMM yyyy'"), 
