...
Функция ПредметыРасчетовПоДокументу(ДокументСсылка, ЧекКоррекцииИсправленииОшибки = Ложь) Экспорт
	...
	|	
	|	ЕСТЬNULL(ТаблицаНоменклатуры.ОсобенностьУчета, ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПустаяСсылка)) КАК
	|		ОсобенностьУчета,
	|	ВЫБОР КОГДА ТаблицаНоменклатуры.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
	|			И ЕСТЬNULL(ТабличнаяЧасть.Характеристика.Принципал, ТаблицаНоменклатуры.Принципал) <> НЕОПРЕДЕЛЕНО
	|			И ЕСТЬNULL(ТабличнаяЧасть.Характеристика.Принципал, ТаблицаНоменклатуры.Принципал) <> &Организация
	|	ТОГДА ЕСТЬNULL(ТабличнаяЧасть.Характеристика.Контрагент, ТаблицаНоменклатуры.Контрагент)
	|	ИНАЧЕ ЕСТЬNULL(ТаблицаНоменклатуры.Контрагент, ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка))
	|	КОНЕЦ КАК
	|		Агент,
	|	
	...
	|	ПО ТабличнаяЧасть.Номенклатура = ТаблицаНоменклатуры.Номенклатура
