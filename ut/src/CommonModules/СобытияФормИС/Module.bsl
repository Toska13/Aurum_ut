...
Процедура ВыполнитьПереопределяемуюКоманду(Знач Форма, Знач ПараметрыВызова, Знач Источник, Результат) Экспорт
	...
	СобытияФормИСПереопределяемый.ВыполнитьПереопределяемуюКоманду(Форма, ПараметрыВызова, Источник, Результат);
...
#Область РаботаСКлючомВидовПродукции

Процедура КлючВидовПродукцииИнициализация(Форма, ПутьКДанным = "") Экспорт
	
	Реквизиты = Форма.ПолучитьРеквизиты(ПутьКДанным);
	ИмяРеквизита = "КлючВидовПродукции";
	
	Если НайтиРеквизитПоИмени(Реквизиты, ИмяРеквизита) = Неопределено Тогда
		
		Реквизиты = Новый Массив;
		
		Реквизиты.Добавить(Новый РеквизитФормы(
			ИмяРеквизита,
			Новый ОписаниеТипов("СправочникСсылка.КлючиВидовПродукцииИС"),
			ПутьКДанным));
		
		ИменаРеквизитов = КонструкторИСОбщегоНазначенияКлиентСервер.ИменаПолейКлючаВидовПродукции();
...
Процедура КлючВидовПродукцииЗаполнитьДанные(Форма, ЗаполняемыеСтроки = Неопределено, ИмяРеквизитаНоменклатура = "Номенклатура") Экспорт
	
	Таблица = Новый ТаблицаЗначений;
	Таблица.Колонки.Добавить("ИндексСтроки", ОбщегоНазначения.ОписаниеТипаЧисло(5));
	Таблица.Колонки.Добавить("Номенклатура", Метаданные.ОпределяемыеТипы.Номенклатура.Тип);
	
	Если ТипЗнч(ЗаполняемыеСтроки) = Тип("ДанныеФормыЭлементКоллекции") Тогда
		ОбрабатываемыеСтроки = Новый Массив;
		ОбрабатываемыеСтроки.Добавить(ЗаполняемыеСтроки);
	Иначе
		ОбрабатываемыеСтроки = ЗаполняемыеСтроки;
	КонецЕсли;
	
	ИменаПолей = КонструкторИСОбщегоНазначенияКлиентСервер.ИменаПолейКлючаВидовПродукции();
	ИменаПолей.Вставить(0, "КлючВидовПродукции");
	
	Если ОбрабатываемыеСтроки <> Неопределено Тогда
		ИндексСтроки = 0;
		Для Каждого Строка Из ОбрабатываемыеСтроки Цикл
			Если ЗначениеЗаполнено(Строка[ИмяРеквизитаНоменклатура]) Тогда
				СтрокаТаблицы = Таблица.Добавить();
				СтрокаТаблицы.ИндексСтроки = ИндексСтроки;
				СтрокаТаблицы.Номенклатура = Строка[ИмяРеквизитаНоменклатура];
			Иначе
				Для Каждого Имя Из ИменаПолей Цикл
					Строка[Имя] = Неопределено;
				КонецЦикла;
			КонецЕсли;
			ИндексСтроки = ИндексСтроки + 1;
		КонецЦикла;
	Иначе
		Если ЗначениеЗаполнено(Форма[ИмяРеквизитаНоменклатура]) Тогда
			СтрокаТаблицы = Таблица.Добавить();
			СтрокаТаблицы.ИндексСтроки = 0;
			СтрокаТаблицы.Номенклатура = Форма[ИмяРеквизитаНоменклатура];
		Иначе
			Для Каждого Имя Из ИменаПолей Цикл
	...
	Для Индекс = 1 По ИменаПолей.ВГраница() Цикл
		ИменаПолейДляЗапроса.Добавить(СтрШаблон("ОписаниеНоменклатуры.КлючВидовПродукции.%1 КАК %1", ИменаПолей[Индекс]));
	КонецЦикла;
	...
	|	Таблица.ИндексСтроки,
	|	Таблица.Номенклатура,
	|	ОписаниеНоменклатуры.КлючВидовПродукции КАК КлючВидовПродукции,
	|	%1
	|ИЗ
	|	Таблица КАК Таблица
	...
	СтрСоединить(ИменаПолейДляЗапроса, ","));
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		Если ОбрабатываемыеСтроки <> Неопределено Тогда
			Данные = ОбрабатываемыеСтроки[Выборка.ИндексСтроки];
		Иначе
			Данные = Форма;
		КонецЕсли;
		Для Каждого Имя Из ИменаПолей Цикл
			Данные[Имя] = Выборка[Имя];
		КонецЦикла;
	КонецЦикла;
...
Процедура ВывестиПоказатель(Выборка, Кнопка, ТекстПоказателя, ДействиеДоступно, ВыводитьКоличество = Истина) Экспорт
...
Процедура ЗаполнениеРеквизитовГосИС(Форма)
	...
	Для Каждого ИмяМодуляЗаполнения Из МодулиЗаполнения Цикл
		Модуль = ОбщегоНазначения.ОбщийМодуль(ИмяМодуляЗаполнения);
		Модуль.ЗаполнениеРеквизитовФормы(Форма);
	КонецЦикла;
	
	ПроверкаИПодборПродукцииИС.ЗаполнениеРеквизитовФормы(Форма);
