...
Функция ВыполнитьДействияЭДО(ПараметрыВыполнения, КонтекстДиагностики = Неопределено) Экспорт
	...
	УчетныеЗаписиЭДО.ИзменитьСпособОбменаВсехУчетныхЗаписейПереводимыхИзТакском();
...
Процедура ПродолжитьВыполнениеДействийПоЭДО(ПараметрыВыполнения, РезультатДействий)
	...
	ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер("ВыполнениеДействийЭДО");
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.ЗакрытьПринудительно) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.ЗакрытьПринудительно];
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("ЗакрытьПринудительно", ОписаниеЗамера, Обработано);
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.ВернутьВРаботу) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.ВернутьВРаботу];
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("ВернутьВРаботу", ОписаниеЗамера, Обработано);
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.ОтправитьВАрхив) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.ОтправитьВАрхив];
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("ОтправитьВАрхив", ОписаниеЗамера, Обработано);
	КонецЕсли;
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.ОтклонитьПодписание) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.ОтклонитьПодписание];
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("ОтклонитьПодписание", ОписаниеЗамера, Обработано);
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.Сформировать) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.Сформировать];
		Если ЗначениеЗаполнено(РезультатДействий.ОшибкиФормирования) Тогда
			ОбщегоНазначенияБЭД.ЗакончитьЗамер("Сформировать", ОписаниеЗамера, Обработано,
				НСтр("ru = 'Ошибки при формировании документа'"));
			Возврат;
		КонецЕсли;
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("Сформировать", ОписаниеЗамера, Обработано);
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.СформироватьДополнительнуюИнформацию) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.СформироватьДополнительнуюИнформацию];
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("СформироватьДополнительнуюИнформацию", ОписаниеЗамера, Обработано);
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.СформироватьИсправление) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.СформироватьИсправление];
		Если ЗначениеЗаполнено(РезультатДействий.ОшибкиФормирования) Тогда
			ОбщегоНазначенияБЭД.ЗакончитьЗамер("СформироватьИсправление", ОписаниеЗамера, Обработано,
				НСтр("ru = 'Ошибки при формировании исправления документа'"));
			Возврат;
		КонецЕсли;
		
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("СформироватьИсправление", ОписаниеЗамера, Обработано);
	КонецЕсли;
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.СформироватьИзвещение) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.СформироватьИзвещение];
		Если ЗначениеЗаполнено(РезультатДействий.ОшибкиФормирования) Тогда
			ОбщегоНазначенияБЭД.ЗакончитьЗамер("СформироватьИзвещение", ОписаниеЗамера, Обработано,
				НСтр("ru = 'Ошибки при формировании извещения'"));
			Возврат;
		КонецЕсли;
		
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("СформироватьИзвещение", ОписаниеЗамера, Обработано);
	КонецЕсли;
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.Переформировать) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.Переформировать];
		Если ЗначениеЗаполнено(РезультатДействий.ОшибкиФормирования) Тогда
			ОбщегоНазначенияБЭД.ЗакончитьЗамер("Переформировать", ОписаниеЗамера, Обработано,
				НСтр("ru = 'Ошибки при переформировании документа'"));
			Возврат;
		КонецЕсли;
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("Переформировать", ОписаниеЗамера, Обработано);
	КонецЕсли;
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.Отклонить) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.Отклонить];
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("Отклонить", ОписаниеЗамера, Обработано);
	КонецЕсли;
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.Аннулировать) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.Аннулировать];
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("Аннулировать", ОписаниеЗамера, Обработано);
	КонецЕсли;
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.ПринятьАннулирование) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.ПринятьАннулирование];
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("ПринятьАннулирование", ОписаниеЗамера, Обработано);
	КонецЕсли;
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.ОтклонитьАннулирование) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.ОтклонитьАннулирование];
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("ОтклонитьАннулирование", ОписаниеЗамера, Обработано);
	КонецЕсли;
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.Перенаправить) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.Перенаправить];
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("Перенаправить", ОписаниеЗамера, Обработано);
	КонецЕсли;
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.Утвердить) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.Утвердить];
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("Утвердить", ОписаниеЗамера, Обработано);
	КонецЕсли;
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.СформироватьОтвет) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.СформироватьОтвет];
		Если ЗначениеЗаполнено(РезультатДействий.ОшибкиФормирования) Тогда
			ОбщегоНазначенияБЭД.ЗакончитьЗамер("СформироватьОтвет", ОписаниеЗамера, Обработано,
				НСтр("ru = 'Ошибки при формировании ответа документа'"));
			Возврат;
		КонецЕсли;
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("СформироватьОтвет", ОписаниеЗамера, Обработано);
	КонецЕсли;
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.Подписать) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.Подписать];
		Если ЗначениеЗаполнено(РезультатДействий.ПараметрыПодписания) Тогда
			ОбщегоНазначенияБЭД.ЗакончитьЗамер("ПодписатьНаСервере", ОписаниеЗамера, Обработано,
				НСтр("ru = 'Подписание выполняется на клиенте'"));
			Возврат;
		КонецЕсли;
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("ПодписатьНаСервере", ОписаниеЗамера, Обработано);
	КонецЕсли;
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.ОтменитьОтправку) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.ОтменитьОтправку];
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("ОтменитьОтправку", ОписаниеЗамера, Обработано);
	КонецЕсли;
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.ПодготовитьКОтправке) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.ПодготовитьКОтправке];
		Если ЗначениеЗаполнено(РезультатДействий.ЗашифрованныеМаркеры) Тогда
			ОбщегоНазначенияБЭД.ЗакончитьЗамер("ПодготовитьКОтправке", ОписаниеЗамера, Обработано,
				НСтр("ru = 'Расшифровка маркеров на клиенте'"));
			Возврат;
		КонецЕсли;
		
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("ПодготовитьКОтправке", ОписаниеЗамера, Обработано);
	КонецЕсли;
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.ЗагрузитьПриглашения) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.ЗагрузитьПриглашения];
		Если ЗначениеЗаполнено(РезультатДействий.ЗашифрованныеМаркеры) Тогда
			ОбщегоНазначенияБЭД.ЗакончитьЗамер("ЗагрузитьПриглашения", ОписаниеЗамера, Обработано,
				НСтр("ru = 'Расшифровка маркеров на клиенте'"));
			Возврат;
		КонецЕсли;
		
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("ЗагрузитьПриглашения", ОписаниеЗамера, Обработано);
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.ПринятьПриглашения) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.ПринятьПриглашения];
		Если ЗначениеЗаполнено(РезультатДействий.ЗашифрованныеМаркеры) Тогда
			ОбщегоНазначенияБЭД.ЗакончитьЗамер("ПринятьПриглашения", ОписаниеЗамера, Обработано,
				НСтр("ru = 'Расшифровка маркеров на клиенте'"));
			Возврат;
		КонецЕсли;
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("ПринятьПриглашения", ОписаниеЗамера, Обработано);
	КонецЕсли;
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.Отправить) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.Отправить];
		Если ЗначениеЗаполнено(РезультатДействий.ЗашифрованныеМаркеры) Тогда
			ОбщегоНазначенияБЭД.ЗакончитьЗамер("Отправить", ОписаниеЗамера, Обработано,
				НСтр("ru = 'Расшифровка маркеров на клиенте'"));
			Возврат;
		КонецЕсли;
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("Отправить", ОписаниеЗамера, Обработано);
	КонецЕсли;
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.Загрузить) Тогда
		...
		Обработано = РезультатДействий.Итог.ОбработаноПоДействиям[Перечисления.ДействияПоЭДО.Загрузить];
		Если ЗначениеЗаполнено(РезультатДействий.ЗашифрованныеМаркеры) Тогда
			ОбщегоНазначенияБЭД.ЗакончитьЗамер("Загрузить", ОписаниеЗамера, Обработано,
				НСтр("ru = 'Расшифровка маркеров на клиенте'"));
			Возврат;
		КонецЕсли;
		ОбщегоНазначенияБЭД.ЗафиксироватьЭтап("Загрузить", ОписаниеЗамера, Обработано);
	КонецЕсли;
	ОбщегоНазначенияБЭД.ЗакончитьЗамер("ВыполнениеДействийЭДО", ОписаниеЗамера);
КонецПроцедуры
...
Процедура ЗаблокироватьДокументДляИзменения(ЭлектронныйДокумент, ЭтоВходящийДокумент = Ложь) Экспорт
	
	РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЭлектронныйДокумент,
		"ИдентификаторДокумента, ИдентификаторДокументооборота, ИдентификаторСвязи");
	
	КлючДокумента = ЭлектронныеДокументыЭДО.НовыйКлючДокументаДляБлокировки();
	ЗаполнитьЗначенияСвойств(КлючДокумента, РеквизитыДокумента);
	
	ЭлектронныеДокументыЭДО.ЗаблокироватьДокументДляИзмененияПоКлючу(КлючДокумента, ЭтоВходящийДокумент);
	
КонецПроцедуры

// Блокирует либо входящие, либо исходящие документы для изменения.
// Передача и входящих и исходящих документов в одном массиве недопустима.
// 
// Параметры:
//  ЭлектронныеДокументы - Массив из ДокументСсылка.ЭлектронныйДокументВходящийЭДО,ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
Процедура ЗаблокироватьДокументыДляИзменения(ЭлектронныеДокументы)
	
	ЭтоВходящиеДокументы = Ложь;
	РеквизитыДокументов = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ЭлектронныеДокументы,
		"ИдентификаторДокумента, ИдентификаторДокументооборота, ИдентификаторСвязи");
	...
	Для Каждого ЭлектронныйДокумент Из ЭлектронныеДокументы Цикл
		ЭтоВходящиеДокументы = ЭлектронныеДокументыЭДО.ЭтоВходящийЭДО(ЭлектронныйДокумент);
		РеквизитыДокумента = РеквизитыДокументов[ЭлектронныйДокумент];
		
		КлючДокументаДляБлокировки = ЭлектронныеДокументыЭДО.НовыйКлючДокументаДляБлокировки();
		ЗаполнитьЗначенияСвойств(КлючДокументаДляБлокировки, РеквизитыДокумента);
...
Функция ДокументыПакетаДляИзменения(ПакетДокументов, Действие, КонтекстДиагностики) Экспорт
	...
	ЗаблокироватьДокументыДляИзменения(ДокументыПакета);
...
Процедура ЗаполнитьИсходящийДокументОбъект(ДокументОбъект, НастройкиОтправки, ОписаниеСообщения,
	...
	Если ДокументОбъект.ТипРегламента = Перечисления.ТипыРегламентовЭДО.Формализованный
		И ДокументОбъект.ФорматОсновногоТитула = ФорматыЭДО_ФНС.ПоддерживаемыеФорматы().СчетНаОплату_5_01 Тогда
		ДокументОбъект.ТребуетсяПодтверждение = Ложь;
	КонецЕсли;
...
Функция СоздатьПрисоединенныйФайл(Владелец, ОписаниеДанных, НоваяСсылкаНаФайл = Неопределено, 
	...
	Файл = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ОписаниеДанных.ИмяФайла);
	...
	ПараметрыДобавленияФайла.Вставить("ПолноеИмяФайла", Файл.Имя);
	
	АдресДвоичныхДанных = ПоместитьВоВременноеХранилище(ОписаниеДанных.ДвоичныеДанные);
	КлючЗамера = "РаботаСФайлами.ДобавитьФайлЭДО";
	ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер(КлючЗамера);
	ПрисоединенныйФайл = РаботаСФайлами.ДобавитьФайл(ПараметрыДобавленияФайла, АдресДвоичныхДанных,,,НоваяСсылкаНаФайл);
	ОбщегоНазначенияБЭД.ЗакончитьЗамер(КлючЗамера, ОписаниеЗамера);
...
Функция НастройкиОтправкиДокументаПоПараметрамДокумента(ПараметрыДокумента, СодержаниеДокумента)
...
//
Функция ПроверитьПараметрыСозданияДокумента(ПараметрыСозданияДокумента, СодержаниеДокумента)
...
Функция СформироватьИсправление(ЭлектронныйДокумент, ПараметрыВыполнения, РезультатДействий)
	...
	Попытка
		...
		ЗаблокироватьДокументДляИзменения(ЭлектронныйДокумент, Ложь);
...
Процедура СформироватьИзвещение(ЭлектронныйДокумент, РезультатДействий)
	...
	Попытка
		...
		ЗаблокироватьДокументДляИзменения(ЭлектронныйДокумент, ЭтоВходящийЭДО);
		...
		Пока ВыборкаФайлов.Следующий() Цикл
			...
			Если ОписаниеСообщения.Данные.ЕстьОшибки Тогда
				...
				ОшибкиЗаполнения = ОписаниеСообщения.Данные.Ошибки.ЗаполнениеДанных;
				КлючСообщения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОшибкиЗаполнения[0],
					"КлючСообщения", Неопределено);
						
				Если ЗначениеЗаполнено(КлючСообщения) Тогда
					
					ДанныеДокумента = Новый Структура("Отправитель, Получатель, ВидДокумента");
					ДанныеДокумента.Отправитель = ПараметрыДокумента.Организация;
					ДанныеДокумента.Получатель = ПараметрыДокумента.Контрагент;
					ДанныеДокумента.ВидДокумента = ЭлектронныеДокументыЭДО.ВидДокументаПоТипу(
						Перечисления.ТипыДокументовЭДО.ИзвещениеОПолучении);
					ОписаниеОшибки = ДобавитьОшибкуФормированияИсходящегоДокумента(РезультатДействий, ДанныеДокумента);
					ОписаниеОшибки.ОшибкиДанных = ОписаниеСообщения.Данные.Ошибки;
					
				Иначе
					
					Для каждого ОшибкаЗаполнения Из ОшибкиЗаполнения Цикл
						ДобавитьОшибкуВыполненияДействия(ОшибкаЗаполнения.ТекстОшибки, 
...
Функция ЗакрытьПринудительно(ЭлектронныйДокумент, КонтекстДиагностики, Комментарий = "") Экспорт
	...
	ПослеПринудительногоЗакрытия(Результат, ЭлектронныйДокумент, КонтекстДиагностики);
...
Функция ОстановитьДокумент(ЭлектронныйДокумент, ПричинаОстановки, Действие, КонтекстДиагностики, Комментарий = "", ВызыватьИсключение = Истина)
...
Функция ВозобновитьДокумент(ЭлектронныйДокумент, Действие, КонтекстДиагностики, Комментарий = "")
	...
	ПослеВозвращенияВРаботу(Результат, ЭлектронныйДокумент, КонтекстДиагностики);
...
// Параметры:
//  РезультатЗакрытия - Булево
//  ЭлектронныйДокумент - ДокументСсылка.ЭлектронныйДокументВходящийЭДО,ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Процедура ПослеПринудительногоЗакрытия(РезультатЗакрытия, ЭлектронныйДокумент, КонтекстДиагностики)
	
	Если РезультатЗакрытия = Истина Тогда
		ЭлектронныеДокументыЭДОСобытия.ПослеПринудительногоЗакрытияЭлектронногоДокумента(ЭлектронныйДокумент,
			КонтекстДиагностики);
	КонецЕсли;
...
// Параметры:
//  РезультатВозвращенияВРаботу - Булево
//  ЭлектронныйДокумент - ДокументСсылка.ЭлектронныйДокументВходящийЭДО,ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Процедура ПослеВозвращенияВРаботу(РезультатВозвращенияВРаботу, ЭлектронныйДокумент, КонтекстДиагностики)
	
	Если РезультатВозвращенияВРаботу = Истина Тогда
		ЭлектронныеДокументыЭДОСобытия.ПослеВозвращенияВРаботуЭлектронногоДокумента(ЭлектронныйДокумент,
			КонтекстДиагностики);
	КонецЕсли;
...
Процедура ОтклонитьДокумент(Сообщение, РезультатДействий, Комментарий = "", ЭлементДляОтклонения = Неопределено)
	...
	Если РезультатОтклонения.Отказ Тогда
		Если ЗначениеЗаполнено(РезультатОтклонения.ОшибкиФормирования) Тогда
			РезультатДействий.ОшибкиФормирования = РезультатОтклонения.ОшибкиФормирования;
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		Если ОписаниеСообщения.Данные.ЕстьОшибки Тогда
			...
			Если ЗначениеЗаполнено(КлючСообщения) Тогда
				...
				ОписаниеОшибки = ДобавитьОшибкуФормированияИсходящегоДокумента(Результат, ДанныеДокумента);
				ОписаниеОшибки.ОшибкиДанных = ОписаниеСообщения.Данные.Ошибки;
			...
			Иначе
				...
				ТекстОшибки = ОбщегоНазначенияБЭД.СоединитьОшибки(ОшибкиЗаполнения);
				ДобавитьОшибкуВыполненияДействия(ТекстОшибки, КонтекстВыполненияДействия, КонтекстДиагностики);
...
Процедура ПослеОтклонения(РезультатОтклонения, РезультатДействий)
	...
	ЭлектронныеДокументыЭДОСобытия.ПослеОтклоненияЭлектронногоДокумента(
		РезультатОтклонения.СообщениеОбъект.ЭлектронныйДокумент, РезультатДействий.КонтекстДиагностики);
...
Процедура АннулироватьДокумент(Сообщение, РезультатДействий, Комментарий = "")
	...
	Если Результат.Отказ Тогда
		Если ЗначениеЗаполнено(Результат.ОшибкиФормирования) Тогда
			РезультатДействий.ОшибкиФормирования = Результат.ОшибкиФормирования;
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Функция Утвердить(Сообщение, КонтекстДиагностики, НаборОбъектовУчета = Неопределено)
	...
	Попытка
		...
		СформироватьМаршрут = Ложь;
		Если РегламентыЭДО.ДокументБезОтветногоТитула(ПараметрыДокумента) 
			Или ПараметрыДокумента.ТипРегламента = Перечисления.ТипыРегламентовЭДО.АктОРасхождениях Тогда
				
			СформироватьМаршрут = ПараметрыДокумента.ТребуетсяПодтверждение
		...
		ДополненияСостоянийЭДО = Неопределено;
		Если СформироватьМаршрут Тогда
			ПараметрыМаршрута = СформироватьМаршрутПодписания(СообщениеОбъект);
...
Функция ВыборкаДокументовДляФормированияОтвета(ОбъектыДействий) Экспорт
	...
	Если ЗначениеЗаполнено(ОбъектыДействий.ОбъектыУчета) Тогда
		...
		ТекстЗапроса = ИнтеграцияЭДО.ЗапросАктуальныхЭлектронныхДокументов(
			...
			|
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	ЭлектронныеДокументыОбъектовУчета.ЭлектронныйДокумент
			|ИЗ
			|	ЭлектронныеДокументыОбъектовУчета КАК ЭлектронныеДокументыОбъектовУчета
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭлектронныйДокументВходящийЭДО КАК ЭлектронныйДокументВходящийЭДО
			|		ПО ЭлектронныеДокументыОбъектовУчета.ЭлектронныйДокумент = ЭлектронныйДокументВходящийЭДО.Ссылка
			|			И (ЭлектронныйДокументВходящийЭДО.ТребуетсяПодтверждение = ЛОЖЬ)
			|			И (ЭлектронныйДокументВходящийЭДО.ТипРегламента = ЗНАЧЕНИЕ(Перечисление.ТипыРегламентовЭДО.АктОРасхождениях))
			|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СообщениеЭДО КАК СформированныеОтветы
			|		ПО (СформированныеОтветы.ЭлектронныйДокумент = ЭлектронныйДокументВходящийЭДО.Ссылка)
			|			И (СформированныеОтветы.ТипЭлементаРегламента В (&ОтветныеЭлементыРегламента))
			|			И (СформированныеОтветы.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыСообщенийЭДО.НеСформирован))
			|			И (НЕ СформированныеОтветы.ПометкаУдаления)
			|ГДЕ
			|	ЭлектронныйДокументВходящийЭДО.ТребуетсяПодтверждение = ЛОЖЬ
			|	И ЭлектронныйДокументВходящийЭДО.ТипРегламента = ЗНАЧЕНИЕ(Перечисление.ТипыРегламентовЭДО.АктОРасхождениях)
			|	И СформированныеОтветы.ЭлектронныйДокумент ЕСТЬ NULL
			|	И НЕ ЭлектронныйДокументВходящийЭДО.ИспользоватьЦентрЭДО";
...
Функция СформироватьОтвет(ЭлектронныйДокумент, КонтекстДиагностики, ОписаниеДанныхЭД = Неопределено) Экспорт
	...
	Попытка
		...
		ТребуетсяОтветныйТитул = ПараметрыДокумента.ТребуетсяПодтверждение;
		Если ПараметрыДокумента.ТипРегламента = Перечисления.ТипыРегламентовЭДО.АктОРасхождениях И Не ТребуетсяОтветныйТитул Тогда
			СвойстваАктОРасхождениях = СвойстваОсновногоФайла.Содержание.СвойстваДокументовПоФорматам.АктОРасхождениях;
			
			ВариантыФормирования = ФорматыЭДО.ВариантыФормированияИтоговПриемки();
			Если СвойстваАктОРасхождениях.ФормируютсяИтогиПриемки 
				И (СвойстваАктОРасхождениях.ВариантФормированияИтоговПриемки = ВариантыФормирования.ПродавцомФормируютсяДопСведенияОСогласииНесогласииСРезультатамиПриемки 
				ИЛИ СвойстваАктОРасхождениях.ВариантФормированияИтоговПриемки = ВариантыФормирования.ПродавцомФормируютсяДопСведенияИные) Тогда
				
				ТребуетсяОтветныйТитул = Истина;
			КонецЕсли;
		КонецЕсли;
		
		Если (Не ТребуетсяОтветныйТитул
				И НЕ (МодульОбменСГИСЭПД <> Неопределено 
...
Функция ОбработатьРезультатыЗапросаДанныхСообщенийДляПодписания(ПараметрыВыполнения, РезультатыЗапроса, КонтекстДиагностики)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияОтправителя Тогда
			...
			ИначеЕсли Выборка.Направление = Перечисления.НаправленияЭДО.Входящий
				И Выборка.ТипРегламента = Перечисления.ТипыРегламентовЭДО.Неформализованный
				И (ПараметрыВыполнения.ДополнительныеПараметрыДействий.Получить(
					Перечисления.ДействияПоЭДО.Аннулировать) <> Неопределено
				Или ПараметрыВыполнения.ДополнительныеПараметрыДействий.Получить(
					Перечисления.ДействияПоЭДО.ОтклонитьАннулирование) <> Неопределено) Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		...
		Если Выборка.Направление = Перечисления.НаправленияЭДО.Исходящий
			...
			Если ЗначениеЗаполнено(НайденныеСтроки) Тогда
				...
				Отбор = Новый Структура;
				Отбор.Вставить("ИдентификаторФормата", Выборка.ФорматОсновногоТитула);
				Отбор.Вставить("Отключен", Истина);
				ОтключенныеФорматы = ФорматыЭлектронныхДокументов.НайтиСтроки(Отбор);
				Если ОтключенныеФорматы.Количество() <> 0 Тогда
					НоваяСтрока = Ошибки.СообщенияВУстаревшемФормате.Добавить();
					НоваяСтрока.ПредставлениеФормата = ОтключенныеФорматы[0].ПредставлениеФормата;
					НоваяСтрока.ДатаОкончанияДействияФормата = ОтключенныеФорматы[0].ДатаОкончанияДействия;
					НоваяСтрока.Сообщение = Выборка.Ссылка;
...
// * СообщенияВУстаревшемФормате - см. НовыйНаборСообщенийВУстаревшемФормате
Функция НовоеОписаниеОшибокОбработкиДанныхПриПодписании()
	...
	ОписаниеОшибок.Вставить("СообщенияВУстаревшемФормате", НовыйНаборСообщенийВУстаревшемФормате());
...
//  * ДатаОкончанияДействияФормата - Дата
//  * НомерДокумента - Строка
//  * ДатаДокумента - Дата
//  * ВидДокумента - СправочникСсылка.ВидыДокументовЭДО
//  * ТипДокумента - ПеречислениеСсылка.ТипыДокументовЭДО
//
Функция НовыйНаборСообщенийВУстаревшемФормате()
	...
	КолонкиНабораСообщений.Добавить("ПредставлениеФормата");
	КолонкиНабораСообщений.Добавить("ДатаОкончанияДействияФормата");
...
Процедура ОбработатьОшибкиДанныхДляПодписания(Ошибки, КонтекстДиагностики, ПараметрыВыполнения) Экспорт
	...
	Если ЗначениеЗаполнено(Ошибки.СообщенияВУстаревшемФормате) Тогда
		Если Ошибки.СообщенияВУстаревшемФормате.Количество() = 1 Тогда
			СтрокаТаблицы = Ошибки.СообщенияВУстаревшемФормате[0];
			ДобавитьОшибкуДокументВУстаревшемФормате(КонтекстДиагностики, СтрокаТаблицы, Истина);
		Иначе
			Для Каждого СтрокаТаблицы Из Ошибки.СообщенияВУстаревшемФормате Цикл
				ДобавитьОшибкуДокументВУстаревшемФормате(КонтекстДиагностики, СтрокаТаблицы);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
// Параметры:
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
//  ДанныеСообщения - СтрокаТаблицыЗначений: см. НовыйНаборСообщенийВУстаревшемФормате
//  ИспользоватьОбработчикОшибки - Булево
//
Процедура ДобавитьОшибкуДокументВУстаревшемФормате(КонтекстДиагностики, ДанныеСообщения, 
	ИспользоватьОбработчикОшибки = Ложь)
	...
	ШаблонСообщения = Нстр("ru = 'Документ ""%1"" сформирован в формате ""%2"", срок действия которого истек %3'")
		+ Символы.ПС + НСтр("ru = 'Для подписания и отправки необходимо переформировать электронный документ.'");
	
	ПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, 
		ПредставлениеДокумента, ДанныеСообщения.ПредставлениеФормата, 
			Формат(ДанныеСообщения.ДатаОкончанияДействияФормата, "ДЛФ=ДД"));
	
	Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(ВидОперации, 
		ВидОшибкиДокументыВУстаревшемФормате(ИспользоватьОбработчикОшибки), ПредставлениеОшибки, ПредставлениеОшибки);
	ПодсистемыБЭД = ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД();
	ОбработкаНеисправностейБЭД.ДобавитьОшибку(КонтекстДиагностики, Ошибка, ПодсистемыБЭД.ОбменСКонтрагентами);
...
Функция ВидОшибкиДокументыВФорматеНеПоддержанномОператором(ИспользоватьОбработчик = Ложь) Экспорт
	...
	Если ИспользоватьОбработчик Тогда
		ВидОшибки.ВыполнятьОбработчикАвтоматически = Истина;
		ВидОшибки.АвтоматическиВыполняемыйОбработчик =
			"ЭлектронныеДокументыЭДОКлиент.ВывестиПредупреждениеОДокументеВНеПоддерживаемомФормате";
	КонецЕсли;	
...
// Возвращаемое значение:
//  См. ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки
Функция ВидОшибкиДокументыВУстаревшемФормате(ИспользоватьОбработчик = Ложь)
	
	ВидОшибки = ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки();
	ВидОшибки.Идентификатор = "ДокументыВУстаревшемФормате";
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Документы сформированы в устаревшем формате'");
	ВидОшибки.ОписаниеПроблемы = НСтр("ru = 'Формат документов больше не действует'");
	Если ИспользоватьОбработчик Тогда
		ВидОшибки.ВыполнятьОбработчикАвтоматически = Истина;
		ВидОшибки.АвтоматическиВыполняемыйОбработчик =
			"ЭлектронныеДокументыЭДОКлиент.ВывестиПредупреждениеОДокументеВНеПоддерживаемомФормате";
...
Процедура УстановитьПростуюПодпись(ДанныеСообщения, Пользователь, ДанныеСотрудника, РезультатДействий)
	...
	КонтекстДиагностики = РезультатДействий.КонтекстДиагностики;
	ЭлектронныйДокумент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеСообщения.Ссылка, "ЭлектронныйДокумент");
	...
	НачатьТранзакцию();
	Попытка
		ЗаблокироватьДокументДляИзменения(ЭлектронныйДокумент, ЭтоВходящийДокумент);
...
Функция ПроверитьПодписиПоМЧДИсходящихСообщений(ПодписанныеСообщения, РезультатДействий)
	...
	Для Каждого ПодписанноеСообщение Из ПодписанныеСообщения Цикл
		...
		Если ПодписанноеСообщение.ТребуетсяМЧД И НЕ ЗначениеЗаполнено(ПодписанноеСообщение.ВыбраннаяМЧД)
			...
			Сертификат = ПодписанноеСообщение.ВыбранныйСертификат;
			
			ДоверенностиПодобранныеПоСертификату = ПодписанноеСообщение.Доверенности.Получить(Сертификат); 
			Если ДоверенностиПодобранныеПоСертификату <> Неопределено Тогда
				
				Если ДоверенностиПодобранныеПоСертификату.Количество() > 0 Тогда
					ПодписанноеСообщение.ВыбраннаяМЧД = ДоверенностиПодобранныеПоСертификату[0];
				КонецЕсли;
...
Функция ОбновитьДанныеПриПодписании(ПодписанноеСообщение, РезультатыПроверокПодписей, РезультатДействий)
	...
	РеквизитыСообщения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПодписанноеСообщение.Ссылка,
		"Направление, ЭлектронныйДокумент, ЭлектронныйДокумент.Организация");
	ЭтоВходящийДокумент = ЭлектронныеДокументыЭДО.ЭтоВходящийЭДО(РеквизитыСообщения.ЭлектронныйДокумент);
	
	ОрганизацияДляПодписания = Неопределено;
	Если ЗначениеЗаполнено(ПодписанноеСообщение.ВыбраннаяМЧД) Тогда
		НаправлениеЭДО = РеквизитыСообщения.Направление;
		...
		Иначе
			ОрганизацияДляПодписания = РеквизитыСообщения.ЭлектронныйДокументОрганизация;
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		...
		ЗаблокироватьДокументДляИзменения(РеквизитыСообщения.ЭлектронныйДокумент, ЭтоВходящийДокумент);
...
Процедура ПодготовитьКОтправкеДокументыПакета(ПакетДокументов, РезультатДействий)
	...
	Попытка
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
			// Аналог блокировки из РаботаСФайлами.ВыполнитьДействияПередЗаписьюПрисоединенногоФайла.
		...
		ЗакрытыйПакет = ПакетыДокументовЭДО.ЭтоЗакрытыйПакет(ПакетДокументов);
		ОтветНаЗакрытыйПакет = СинхронизацияЭДО.НовыеДанныеОбъектов();
		...
		Пока СвойстваДокументаСообщения.Следующий() Цикл
			...
			Если ЭтоОсновнойОбъект Тогда
				ДанныеОбъектов = ДанныеОсновныхОбъектов;
			ИначеЕсли ЗакрытыйПакет Тогда
				ДанныеОбъектов = ОтветНаЗакрытыйПакет;
			Иначе
				ДанныеОбъектов = СинхронизацияЭДО.НовыеДанныеОбъектов();
			КонецЕсли;
			...
			ДанныеОбъекта.ЗакрытыйПакет = ЗакрытыйПакет;
			...
			Если ЭтоОсновнойОбъект Или ЗакрытыйПакет Тогда
				Продолжить;
			КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(ПричинаОстановки)
			...
			СвойстваСообщений = СвойстваСообщенийДокументовПакета(ДокументыПакета, Ложь);
			...
			РезультатПодготовки = 
				СинхронизацияЭДО.ПодготовитьОбъектыКОтправке(ДанныеОсновныхОбъектов, КонтекстДиагностики);
		...
		Если Не ЗначениеЗаполнено(ПричинаОстановки) 
			И ЗначениеЗаполнено(ОтветНаЗакрытыйПакет) Тогда
			
			НеподходящиеДокументы = Новый Массив;
			СвойстваСообщений = СвойстваСообщенийДокументовПакета(ДокументыПакета, Истина);
			Если Не ПодготовленыВсеОтветыНаЗакрытыйПакет(ДокументыПакета, СвойстваСообщений, НеподходящиеДокументы) Тогда
				ДобавитьОшибкуПодготовкиОтветаНаЗакрытыйПакет(ПакетДокументов, НеподходящиеДокументы, КонтекстДиагностики);
				ДобавитьОшибкуОтменыОперации(КонтекстВыполненияДействия, КонтекстДиагностики);
				ОтменитьТранзакцию();
				Возврат;
			КонецЕсли;
			...
			РезультатПодготовки = 
				СинхронизацияЭДО.ПодготовитьОбъектыКОтправке(ОтветНаЗакрытыйПакет, КонтекстДиагностики);
...
Процедура ЗаполнитьДанныеОбъектаДляПодготовкиКОтправке(ДанныеОбъекта, СвойстваДокумента, СвойстваСообщения, ТипСообщения, ФайлыСообщения, ИдентификаторыОснований, КонтекстДиагностики)
	...
	Если ДанныеОбъекта.ТипРегламента = Перечисления.ТипыРегламентовЭДО.АктОРасхождениях 
		И ДанныеОбъекта.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияОтправителя 
		И Не ДанныеОбъекта.ТребуетсяПодтверждение Тогда
		
		ПараметрыСообщенияОтправителя = ЭлектронныеДокументыЭДО.РаспознатьСообщение(СвойстваСообщения.Ссылка);
		Если ПараметрыСообщенияОтправителя.Свойство("СвойстваДокументовПоФорматам") 
			И ПараметрыСообщенияОтправителя.СвойстваДокументовПоФорматам.Свойство("АктОРасхождениях") Тогда
			СвойстваАкта = ПараметрыСообщенияОтправителя.СвойстваДокументовПоФорматам.АктОРасхождениях;
			
			ВариантФормированияИтоговПриемки = "";
			ВариантыФормированияИтоговПриемки = ФорматыЭДО.ВариантыФормированияИтоговПриемки();
			Если СвойстваАкта.Свойство("ВариантФормированияИтоговПриемки", ВариантФормированияИтоговПриемки) 
				И Не ПустаяСтрока(ВариантФормированияИтоговПриемки) Тогда
				
				ДанныеОбъекта.ТребуетсяПодтверждение = (ВариантФормированияИтоговПриемки = 
...
//  * Состояние - ПеречислениеСсылка.СостоянияСообщенийЭДО
//  * ЭлектронныйДокумент - ДокументСсылка.ЭлектронныйДокументВходящийЭДО,ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
Функция СвойстваСообщенийДокументовПакета(ДокументыПакета, Знач ЗакрытыйПакет)
	
	ТипыЭлементовРегламента = ТипыЭлементовРегламентаСообщенийОтправляемыхВОтветНаПакет(ЗакрытыйПакет);
	
	Запрос = Новый Запрос(ТекстЗапросаСвойствОсновныхСообщенийДокументовПакета());
	Запрос.УстановитьПараметр("ЭлектронныеДокументы", ДокументыПакета);
	Запрос.УстановитьПараметр("ТипЭлементаРегламента", ТипыЭлементовРегламента);
	Возврат Запрос.Выполнить().Выгрузить();
...
Функция ТекстЗапросаСвойствОсновныхСообщенийДокументовПакета()
	Возврат
		...
		|ГДЕ
		|	СообщениеЭДО.ЭлектронныйДокумент В (&ЭлектронныеДокументы)
		|	И СообщениеЭДО.ТипЭлементаРегламента В (&ТипЭлементаРегламента)";
КонецФункции
...
//
// Возвращаемое значение:
//  Массив из см. ПеречислениеСсылка.ТипыЭлементовРегламентаЭДО - Список типов сообщений отправляемых в ответ на пакет.
Функция ТипыЭлементовРегламентаСообщенийОтправляемыхВОтветНаПакет(ЗакрытыйПакет)
	
	ТипыЭлементовРегламента = Новый Массив();
	
	Если ЗакрытыйПакет Тогда
		...
		ТипыЭлементовРегламента.Добавить(Перечисления.ТипыЭлементовРегламентаЭДО.УС_ОтказВПодписи);
		ТипыЭлементовРегламента.Добавить(Перечисления.ТипыЭлементовРегламентаЭДО.УОУ);
		ТипыЭлементовРегламента.Добавить(Перечисления.ТипыЭлементовРегламентаЭДО.ПОА);
	Иначе
		ТипыЭлементовРегламента.Добавить(Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияОтправителя);
	КонецЕсли;
	
	Возврат ТипыЭлементовРегламента;
...
//  ДокументыПакета - См. ПакетыДокументовЭДО.ДокументыПакета
//  СвойстваСообщений - см. СвойстваОсновныхСообщенийПакетаДляПодготовкиКОтправке
//  НеподходящиеДокументы - Неопределено
//  - Массив из ДокументСсылка.ЭлектронныйДокументВходящийЭДО, ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
// Возвращаемое значение:
//  Булево
Функция ПодготовленыВсеОтветыНаЗакрытыйПакет(ДокументыПакета, СвойстваСообщений, НеподходящиеДокументы = Неопределено)
	
	Если НеподходящиеДокументы = Неопределено Тогда
		НеподходящиеДокументы = Новый Массив;
	КонецЕсли;
	
	ДокументыСПодтверждениями = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ДокументыПакета, "ТребуетсяПодтверждение");
	Для Каждого КлючИЗначение Из ДокументыСПодтверждениями Цикл
		
		Если Не КлючИЗначение.Значение Тогда
			Продолжить;
		КонецЕсли;
		
		Отбор = Новый Структура("ЭлектронныйДокумент", КлючИЗначение.Ключ);
		ПодготовленныеСообщения = СвойстваСообщений.НайтиСтроки(Отбор);
		Если ПодготовленныеСообщения = Неопределено Тогда
			НеподходящиеДокументы.Добавить(КлючИЗначение.Ключ);
			Продолжить;
		КонецЕсли;
		
		ЕстьПодготовленныйОтвет = Ложь;
		Для Каждого Сообщение Из ПодготовленныеСообщения Цикл
			Если Сообщение.Состояние = Перечисления.СостоянияСообщенийЭДО.ПодготовкаКОтправке Тогда
				ЕстьПодготовленныйОтвет = Истина;
				Продолжить;
			КонецЕсли;
		КонецЦикла;
		
		Если Не ЕстьПодготовленныйОтвет Тогда
			НеподходящиеДокументы.Добавить(КлючИЗначение.Ключ);
		КонецЕсли;
...
//  ДокументыПакета - См. ПакетыДокументовЭДО.ДокументыПакета
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Процедура ДобавитьОшибкуПодготовкиОтветаНаЗакрытыйПакет(ИдентификаторПакета, ДокументыПакета, КонтекстДиагностики)
	
	ВидОперации = НСтр("ru = 'Подготовка к отправке.'");
	ВидОшибки = ОбработкаНеисправностейБЭДКлиентСервер.ВидОшибкиНеизвестнаяОшибка();
	
	КраткоеПредставление = 
		НСтр("ru = 'Не удалось подготовить к отправке ответ на пакет электронных документов с признаком ""закрытый"".
		|Все сообщения в ответе должны быть отправлены единовременно.'");
		
	МассивСтрок = Новый Массив;
	МассивСтрок.Добавить(КраткоеПредставление);
	МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Идентификатор пакета: %1'"), ИдентификаторПакета));
	МассивСтрок.Добавить(НСтр("ru = 'В пакете есть документы, по которым еще не подготовлен ответ:'"));
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивСтрок, ДокументыПакета);
	ПодробноеПредставление = СтрСоединить(МассивСтрок, Символы.ПС);
	
	Ошибка = 
		ОбработкаНеисправностейБЭД.НоваяОшибка(ВидОперации, ВидОшибки, ПодробноеПредставление, КраткоеПредставление);
	ПодсистемыБЭД = ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД();
	ОбработкаНеисправностейБЭД.ДобавитьОшибку(КонтекстДиагностики, Ошибка, ПодсистемыБЭД.ОбменСКонтрагентами);
...
Функция ОтменитьОтправку(НаборСообщений, КонтекстДиагностики)
	...
	ЭлектронныеДокументыПоСообщениям = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(НаборСообщений,
		"ЭлектронныйДокумент");
	ЭлектронныеДокументы = ОбщегоНазначения.ВыгрузитьКолонку(ЭлектронныеДокументыПоСообщениям, "Значение", Истина);
	
	НачатьТранзакцию();
	Попытка
		ЗаблокироватьДокументыДляИзменения(ЭлектронныеДокументы);
...
Процедура ВыполнитьДействиеЗагрузить(ПараметрыВыполнения, РезультатДействий)
	...
	Если ПараметрыВыполнения.РасшифрованныеМаркеры = Неопределено Тогда
		...
		ПараметрыПереводаИзТакском = СинхронизацияЭДО.ПараметрыПереводаИзТакском();
		Если ПараметрыВыполнения.ВыполнитьТолькоПоТакском Тогда
			ПараметрыПереводаИзТакском.СпособОбменаДляПодмены = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском;
		КонецЕсли;
		...
		ДанныеДляСинхронизации = СинхронизацияЭДО.ДанныеДляСинхронизации(ОтпечаткиСертификатов,
			...
			ПараметрыВыполнения.ВыбранныеСертификаты, ПараметрыПереводаИзТакском);
	...
	Если ЗначениеЗаполнено(ПараметрыВыполнения.РасшифрованныеМаркеры) Тогда
		...
		УчетныеЗаписиЭДО.СкорректироватьСпособОбменаВКлючахСинхронизации(КлючиСинхронизации, ПараметрыВыполнения);
	...
	Если ЗначениеЗаполнено(КлючиСинхронизации)
		...
		УчетныеЗаписиЭДО.СкорректироватьСпособОбменаВКлючахСинхронизации(КлючиСинхронизации, ПараметрыВыполнения);
		Если Не ПараметрыВыполнения.ВыполнитьТолькоПоТакском Тогда
			УчетныеЗаписиЭДО.ПеревестиУчетныеЗаписиТакскомВХаб(КлючиСинхронизации);
		КонецЕсли;
...
Процедура ДополнитьДанныеОбъектовДляЗагрузки(ДанныеОбъектов)
	...
	ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер("ЗаполнениеТаблицыКонтейнеровДаннымиИзФорматов");
	...
	Для Каждого СтрокаТаблицы Из ДанныеОбъектов Цикл
		...
		ДополнитьДанныеОбъектаСодержаниемИнформацииОтправителя(СтрокаТаблицы, КэшЦиклов);
	...
	// Для поиск в СинхронизацияЭДО.ДанныеОбъектаПоИдентификатору добавим индекс по ИдентификаторСтроки.
	ОбщегоНазначенияБЭД.ДобавитьИндексПриНеобходимости(ДанныеОбъектов, "ИдентификаторСтроки");
	...
	КонецЕсли;
	ОбщегоНазначенияБЭД.ЗакончитьЗамер("ЗаполнениеТаблицыКонтейнеровДаннымиИзФорматов", ОписаниеЗамера,
		ДанныеОбъектов.Количество());
КонецПроцедуры
...
Процедура ДополнитьДанныеОбъектаСодержаниемИнформацииОтправителя(ДанныеОбъекта, КэшЦиклов)
	...
	Если ЗначениеЗаполнено(Содержание) Тогда
		...
		Если ЗначениеЗаполнено(Содержание.Отправитель.ИНН) Тогда
			...
			Если КэшЦиклов <> Неопределено И КэшЦиклов["СоздаватьКонтрагентовАвтоматически"] <> Неопределено Тогда
				СоздаватьКонтрагентовАвтоматическиБЭД = КэшЦиклов["СоздаватьКонтрагентовАвтоматически"];
			Иначе
				СоздаватьКонтрагентовАвтоматическиБЭД = НастройкиЭДО.СоздаватьКонтрагентовАвтоматически();
				Если КэшЦиклов <> Неопределено Тогда
					КэшЦиклов["СоздаватьКонтрагентовАвтоматически"] = СоздаватьКонтрагентовАвтоматическиБЭД;
				КонецЕсли;
			КонецЕсли;
			...
			Если ОбменПоПриглашению Тогда
				...
				СтруктураУчастникаЭДО = ПриглашенияЭДО.УчастникЭДОПоКлючуИПриглашению(
					ПараметрыПриглашения,
					КлючКонтрагента,
					КэшЦиклов);
...
Процедура ДополнитьДанныеОбъектовВидамиДокументов(ДанныеОбъектов, РезультатЗапроса)
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			КонецЕсли;
			ДанныеОбъекта.ВидДокумента = ЭлектронныеДокументыЭДО.НайтиСоздатьВидДокумента(ПараметрыПоиска);
		КонецЕсли;
	КонецЦикла;
...
Функция ПроверитьПодписиПоОбъектамКонтейнеров(ДанныеОбъектов, МенеджерКриптографии, КонтекстДиагностики) Экспорт
	...
	ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер("ПроверитьПодписиПоОбъектамКонтейнеров");
	Результат = Новый Соответствие;
	...
	КонецЦикла;
	ОбщегоНазначенияБЭД.ЗакончитьЗамер("ПроверитьПодписиПоОбъектамКонтейнеров", ОписаниеЗамера, ДанныеОбъектов.Количество());
	Возврат Результат;
...
Функция ЗагрузитьОбработатьДанныеОбъектовКонтейнеров(ДанныеОбъектов, КонтекстДиагностики, ОтпечаткиСертификатов,
	...
	Если КриптографияБЭД.ДоступнаПроверкаПодписиНаСервере()
		Или (АвтоматическоеПолучениеЭДО.Использовать()
...
Процедура ЗагрузитьДанныеОбъекта(ДанныеОбъекта, ПроверенныеПодписи, КонтекстДиагностики, РезультатЗагрузкиДанных)
	ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер("СозданиеДокументаЭДОПоДаннымСтрокиТаблицыКонтейнеров");
	КлючБлокировкиДокумента = ЭлектронныеДокументыЭДО.НовыйКлючДокументаДляБлокировки();
	КлючБлокировкиДокумента.ИдентификаторДокументооборота = ДанныеОбъекта.ИдентификаторДокументооборота;
	
	НачатьТранзакцию();
	Попытка
		ЭлектронныеДокументыЭДО.ЗаблокироватьДокументДляИзмененияПоКлючу(КлючБлокировкиДокумента);
		ЭлектронныеДокументыЭДО.ЗаблокироватьДокументДляИзмененияПоКлючу(КлючБлокировкиДокумента, Истина);
	...
	Исключение
		...
		Возврат;
	КонецПопытки;
	ОбщегоНазначенияБЭД.ЗакончитьЗамер("СозданиеДокументаЭДОПоДаннымСтрокиТаблицыКонтейнеров", ОписаниеЗамера);
	...
	Попытка
		...
		Для Каждого ДанныеОбъекта Из ДанныеОбъектовПакета.Значение Цикл
			...
			Если ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияОтправителя Тогда
				...
				ДополнительныеДанныеКарточки = ДанныеОбъекта.Карточка.ДополнительныеПараметры;
				ClosedMessage = 
					ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеДанныеКарточки, 
						"ClosedMessage", "false");
				Если ТипЗнч(ClosedMessage) = Тип("Массив") Тогда
					ЗакрытыйПакет = ОбщегоНазначения.JSONВЗначение(ClosedMessage.Получить(0));
				Иначе
					ЗакрытыйПакет = ОбщегоНазначения.JSONВЗначение(ClosedMessage);
				КонецЕсли;
			КонецЕсли;
		...
		ПакетыДокументовЭДО.ЗагрузитьДокументыПакета(СвойстваДокументовДляОписанияПакета, ЗакрытыйПакет);
...
Функция ЗагрузитьОтветнуюПодпись(ДанныеОбъекта, ПроверенныеПодписи, КонтекстДиагностики)
	...
	УстановленныеПодписи = ЭлектроннаяПодпись.ПодписиОбъекта(СообщениеОбъект.ОсновнойФайл);
...
Процедура ЗаполнитьРеквизитыДокументаПоНаименованиюФайла(ДанныеОбъекта, Результат)
	...
	РеквизитыДокументаПоНаименованиюФайла = РеквизитыДокументаИзНаименованияФайла(
		...
		Не ЗначениеЗаполнено(ДанныеОбъекта.ОписаниеДокумента.Номер),
		Не ЗначениеЗаполнено(ДанныеОбъекта.ОписаниеДокумента.ДатаДокумента));
...
//                  - Дата
Функция РеквизитыДокументаИзНаименованияФайла(Знач ИмяФайла, ОпределятьВид = Истина, ОпределятьНомер = Истина,
	...
	РезультатПолученияВидаДокумента = Неопределено;
	РезультатПолученияДатыДокумента = Неопределено;
	
	Если ОпределятьВид Тогда
		РезультатПолученияВидаДокумента = ВидДокументаИзНаименованияФайла(ИмяФайла);
		Если РезультатПолученияВидаДокумента.Успех Тогда
			Результат.ВидДокумента = РезультатПолученияВидаДокумента.ВидДокумента;
		КонецЕсли;
	КонецЕсли;
	Если ОпределятьДату Тогда
		РезультатПолученияДатыДокумента = ДатаДокументаИзНаименованияФайла(ИмяФайла);
		Если РезультатПолученияДатыДокумента.Успех Тогда
			Результат.ДатаДокумента = РезультатПолученияДатыДокумента.ДатаДокумента;
		КонецЕсли;
	КонецЕсли;
	Если ОпределятьНомер Тогда
		ИмяФайлаДляПолученияНомера = ИмяФайла;
		Если ЗначениеЗаполнено(РезультатПолученияВидаДокумента)
			И РезультатПолученияВидаДокумента.Успех Тогда
			ИмяФайлаДляПолученияНомера = СтрЗаменить(ИмяФайлаДляПолученияНомера, РезультатПолученияВидаДокумента.НайденнаяСтрока, "");
		КонецЕсли;
		Если ЗначениеЗаполнено(РезультатПолученияДатыДокумента)
			И РезультатПолученияДатыДокумента.Успех Тогда
			ИмяФайлаДляПолученияНомера = СтрЗаменить(ИмяФайлаДляПолученияНомера,
				РезультатПолученияДатыДокумента.НайденнаяСтрока, "");
		КонецЕсли;
		
		РезультатПолученияНомераДокумента = НомерДокументаИзНаименованияФайла(ИмяФайлаДляПолученияНомера);
		Если РезультатПолученияНомераДокумента.Успех Тогда
			Результат.НомерДокумента = РезультатПолученияНомераДокумента.НомерДокумента;
		КонецЕсли;
	КонецЕсли;
...
Функция ВидДокументаИзНаименованияФайла(ИмяФайла)
	...
	Результат = Новый Структура("Успех, ВидДокумента, НайденнаяСтрока", Ложь, Неопределено, "");
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРегулярныхВыражений Цикл
		...
		Результат.Успех = Истина;
		Результат.НайденнаяСтрока = РезультатПоиска.Значение;
		
		ТипДокумента = СтрокаТаблицы.ТипДокумента;
		Результат.ВидДокумента = ЭлектронныеДокументыЭДО.ВидДокументаПоТипу(ТипДокумента);
		Прервать;
	КонецЦикла;
...
Функция ТаблицаРегулярныхВыраженийДляОпределенияТипаДокумента() 
	...
	НоваяСтрока.РегулярноеВыражение = "дополнительное[\s_]+соглашение|доп\.?[\s_]*соглашение|дс(?:[_\s\.]|$)";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ДополнительноеСоглашение;
	НоваяСтрока.РегулярноеВыражение = "dopolnitelnoe[\s_]+soglashenie|dop\.?[\s_]*soglashenie|ds(?:[_\s\.]|$)";
...
Функция НомерДокументаИзНаименованияФайла(ИмяФайла)
	...
	Результат = Новый Структура("Успех, НомерДокумента, НайденнаяСтрока", Ложь, "", "");
	...
	РегулярныеВыраженияДляПоискаНомераДокумента.Добавить("(\d{1,4})(?!\d)"); // 1, 12, 123, 1234
	...
	Для Каждого РегулярноеВыражение Из РегулярныеВыраженияДляПоискаНомераДокумента Цикл
		...
		Результат.Успех = Истина;
		Результат.НайденнаяСтрока = РезультатПоиска.Значение;
		Результат.НомерДокумента = СокрЛП(СтрЗаменитьПоРегулярномуВыражению(РезультатПоиска.Значение,
			"[№#]\s*|[Nn][o°]|_\s*", ""));
		Прервать;
	КонецЦикла;
...
Функция ДатаДокументаИзНаименованияФайла(ИмяФайла)
	...
	Результат = Новый Структура("Успех, ДатаДокумента, НайденнаяСтрока", Ложь, Дата(1, 1, 1), "");
	...
	РегулярныеВыраженияДляПоискаДатыДокумента.Добавить("\d{2}\/\d{2}\/\d{4}"); // DD/MM/YYYY
	РегулярныеВыраженияДляПоискаДатыДокумента.Добавить("(?<!\d)\d{1,2}[\s_]+\w+[\s_]+\d{4}"); // DD MM YYYY 22 апреля 2025, 22_апреля_2025
	...
	Для Каждого РегулярноеВыражение Из РегулярныеВыраженияДляПоискаДатыДокумента Цикл
		...
		Результат.ДатаДокумента = ОбщегоНазначенияБЭД.ДатаПоСтроковомуПредставлению(РезультатПоиска.Значение);
		Если ЗначениеЗаполнено(Результат.ДатаДокумента) Тогда
			Результат.Успех = Истина;
			Результат.НайденнаяСтрока = РезультатПоиска.Значение;
		КонецЕсли;
		Прервать;
...
Процедура ВыполнитьДействиеЗагрузитьПриглашения(ПараметрыВыполнения, РезультатДействий)
	...
	Если ЗначениеЗаполнено(КлючиСинхронизации) Тогда
		УчетныеЗаписиЭДО.УдалитьПереведенныеУчетныеЗаписиТакскомаИзКлючейСинхронизации(КлючиСинхронизации);
...
// Возвращаемое значение:
//  См. СоздатьКонтекстВыполненияДействия
Функция СоздатьКонтекстВыполненияДействияПоПакету(ИдентификаторПакета, Действие)
...
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Процедура ДобавитьОшибкуВыполненияДействия(ТекстОшибки, КонтекстВыполнения, КонтекстДиагностики)
...
//  КонтекстВыполнения - См. СоздатьКонтекстВыполненияДействия
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Процедура ДобавитьОшибкуОтменыОперации(КонтекстВыполнения, КонтекстДиагностики)
...
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
//  ЭтоПереводВХаб - Булево
Процедура ОстановитьДокументыПослеИзмененияПриглашения(ИдентификаторОрганизации, ИдентификаторКонтрагента, ПричинаОстановки, КонтекстДиагностики, ЭтоПереводВХаб = Ложь) Экспорт
	...
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
		Если ЭтоПереводВХаб Тогда
			ОстановитьДокумент(Выборка.ЭлектронныйДокумент, ПричинаОстановки, Действие, КонтекстДиагностики, "", Ложь);
		Иначе
...
Процедура ОбновитьСвязанныеДокументы(ТипДокумента, ИдентификаторыОснований, Действие, КонтекстДиагностики, ЭтоВходящийЭДО = Ложь) Экспорт
	...
	ЭтоИсправляющийТипДокумента = ЭлектронныеДокументыЭДО.ЭтоИсправляющийТипДокумента(ТипДокумента);
	
	ЭтоКорректирующийТипДокумента = ЭлектронныеДокументыЭДО.ЭтоКорректирующийТипДокумента(ТипДокумента);
	...
	Для Каждого СтрокаТаблицы Из ИдентификаторыОснований Цикл
		КлючБлокировкиДокумента = ЭлектронныеДокументыЭДО.НовыйКлючДокументаДляБлокировки();
		Если ЗначениеЗаполнено(СтрокаТаблицы.ИдентификаторДокументооборота) Тогда
			КлючБлокировкиДокумента.ИдентификаторДокументооборота = СтрокаТаблицы.ИдентификаторДокументооборота;
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаТаблицы.ИдентификаторСвязи) Тогда
			КлючБлокировкиДокумента.ИдентификаторСвязи = СтрокаТаблицы.ИдентификаторСвязи;
		КонецЕсли;
		КлючиДокументовДляБлокировки.Добавить(КлючБлокировкиДокумента);
	КонецЦикла;
	
	НачатьТранзакцию();
	Попытка
		ЭлектронныеДокументыЭДО.ЗаблокироватьДокументыДляИзмененияПоКлючам(КлючиДокументовДляБлокировки, ЭтоВходящийЭДО);
		...
		КорректируемыеТипыДокументов = ЭлектронныеДокументыЭДО.КорректируемыеТипыДокументов();
...
Функция ИзменитьДанныеВходящегоДокумента(ЭлектронныйДокумент, ДанныеДляИзменения) Экспорт
	...
	Попытка
		...
		ЗаблокироватьДокументДляИзменения(ЭлектронныйДокумент, Истина);
...
Функция ДоступныеДействияПоЭДО(СостояниеДокумента, СвойстваДокумента) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		МодульОбменСГИСЭПД = ОбщегоНазначения.ОбщийМодуль("ОбменСГИСЭПД");
		МодульОбменСГИСЭПД.ПриОпределенииДоступныхДействийПоЭДО(
			Действия, СостояниеДокумента, СвойстваДокумента);
	КонецЕсли;
...
Функция ОписаниеСообщенияПолучателя(ДанныеОбъектаУчета, ПараметрыДокумента, СвойстваОсновногоФайла, Формат, ДанныеТЭДО)
	...
	Подписи = ЭлектроннаяПодпись.ПодписиОбъекта(СвойстваОсновногоФайла.Ссылка);
...
Функция ОписаниеСлужебногоСообщения(ДанныеУчастниковЭДО, СвойстваФайлаОснования, ТипДокумента, ТипЭлементаРегламента, Комментарий = "")
	...
	Подписи = ЭлектроннаяПодпись.ПодписиОбъекта(СвойстваФайлаОснования.Ссылка);
...
// Параметры:
//  УстановленныеПодписи - См. ЭлектроннаяПодпись.ПодписиОбъекта
...
Функция УстановленныеПодписиФайла(ПрисоединенныйФайл, ВидПодписи = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВидПодписи)
		...
		УстановленныеПодписи = КриптографияБЭД.УстановленныеПодписи(ПрисоединенныйФайл);
		Для Каждого Подпись Из УстановленныеПодписи Цикл
...
Функция ЕстьНевалидныеПодписи(ФайлДокумента, КонтекстДиагностики = Неопределено)
	...
	УстановленныеПодписи = ЭлектроннаяПодпись.ПодписиОбъекта(ФайлДокумента);
...
Функция СообщениеМожетПодписыватьсяТолькоОднойСтороной(ДополнительныйКонтекстПроверки)
	...
	Если ДополнительныйКонтекстПроверки.СвойстваДокумента.ТипРегламента =
			Перечисления.ТипыРегламентовЭДО.Формализованный
		И ДополнительныйКонтекстПроверки.СвойстваСообщения.ТипЭлементаРегламента =
			Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияОтправителя Тогда
		РеквизитыЭД = 
			ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
				ДополнительныйКонтекстПроверки.СвойстваСообщения.ЭлектронныйДокумент,
					"ВидДокумента.ТипДокумента, ТребуетсяПодтверждение", Истина);
		Если РеквизитыЭД.ТребуетсяПодтверждение
			И ЭлектронныеДокументыЭДО.ЭтоТипБезОтветногоТитулаПриФормализованномОбмене(
				РеквизитыЭД.ВидДокументаТипДокумента) Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Возврат РегламентыЭДО.ЕстьИнформацияПолучателя(ДополнительныйКонтекстПроверки.СвойстваДокумента.ТипРегламента)
		...
		И Не ДополнительныйКонтекстПроверки.СвойстваДокумента.ТипРегламента = Перечисления.ТипыРегламентовЭДО.АктОРасхождениях;
...
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Процедура ПроверитьПодписиПоМЧДПослеЗагрузкиДанных(ФайлыСообщенийДляПроверкиПодписейПоМЧД, КонтекстДиагностики)
	
	ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер("ПроверитьПодписиПоМЧДПослеЗагрузкиДанных");
	УстановитьПривилегированныйРежим(Истина);
	...
	ОбщегоНазначенияБЭД.ЗакончитьЗамер("ПроверитьПодписиПоМЧДПослеЗагрузкиДанных", ОписаниеЗамера,
		СведенияОПодписяхДляПроверки.Количество());
КонецПроцедуры
...
Процедура ПроверитьНепроверенныеПодписиПоМЧДВФоне(КонтекстДиагностики = Неопределено) Экспорт
	...
	ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер("ПроверитьНепроверенныеПодписиПоМЧДВФоне");
	КоличествоЗамеров = 0;
	УстановитьПривилегированныйРежим(Истина);
	...
	Пока Истина Цикл
		...
		КоличествоЗамеров = КоличествоЗамеров + Количество;
	...
	КонецЦикла;
	ОбщегоНазначенияБЭД.ЗакончитьЗамер("ПроверитьПодписиПоМЧДПослеЗагрузкиДанных", ОписаниеЗамера, КоличествоЗамеров);
КонецПроцедуры 
...
Функция ПроверитьПодписьПоМЧД(ФайлСообщения, НаправлениеСообщения, ОписаниеФайла, СвойстваПодписи,
	...
	КлючЗамера = "ПроверитьПодписьПоМЧД";
	ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер(КлючЗамера);
	ХешПодписи = КриптографияБЭД.ХешПодписи(СвойстваПодписи.Подпись);
	...
	КомментарийЗавершенияЗамераБезПроверки = НСтр("ru = 'Проверка подписи не требуется'");
	Если Не РегламентыЭДО.ЭтоИнформацияУчастникаЭДО(ТипЭлементаРегламента) 
		...
		РезультатПроверки.ПроверкаВыполнена = Истина;
		ОбщегоНазначенияБЭД.ЗакончитьЗамер(КлючЗамера, ОписаниеЗамера,, КомментарийЗавершенияЗамераБезПроверки);
		Возврат РезультатПроверки;
	КонецЕсли;
...
Функция ПроверитьПодписиПоМЧДПриРегистрацииСертификатовПродолжение(РезультатыПроверокПодписейПоМЧД, 
	...
	Для Каждого ЭлементНабора Из РезультатыПроверокПодписейПоМЧД Цикл
		...
		РезультатПроверкиПодписиМЧД = ПараметрыПроверки.СведенияМЧД.ПротоколПроверкиМЧД.Получить();
		...
		МашиночитаемыеДоверенности.ОтразитьРезультатПроверкиМЧД(МЧД, ПараметрыПроверки.СведенияМЧД.Верна, 
			ПараметрыПроверки.ТекстОшибки, , РезультатПроверкиПодписиМЧД);
...
// Возвращаемое значение:
//  Строка
Функция КлючЗаданияФоновойПроверкиПодписейПоМЧД()
...
// Переносит сообщения пользователю из длительной операции в ошибки из результата работы
// 
// Параметры:
//  РезультатыПоИнтеграциямЭДО - см. ЭлектронныеДокументыЭДО.ВыполнитьДействияПоИнтеграциямЭДО
//  Сообщения - ФиксированныйМассив из СообщениеПользователю
//
Процедура ОбработатьСообщенияДлительнойОперации(РезультатыПоИнтеграциямЭДО, Сообщения) Экспорт
	
	ДанныеПоДокументам = РезультатыПоИнтеграциямЭДО["ЭлектронныеДокументыЭДО"];
	
	Если ДанныеПоДокументам = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ДанныеПоДокументам.ОшибкиФормирования) Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого ОшибкаФормирования Из ДанныеПоДокументам.ОшибкиФормирования Цикл
		
		Если ОшибкаФормирования.ОшибкиДанных = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Каждого Сообщение Из Сообщения Цикл
			...
			Если Сообщение.КлючДанных = ОшибкаФормирования.ОписаниеОбъектаУчета.ОбъектУчета Тогда
				
				ДанныеОшибки = ОбщегоНазначенияБЭДКлиентСервер.НовыеПараметрыОшибки();
				ДанныеОшибки.ПутьКДаннымСообщения = Сообщение.ПутьКДанным;
				
				ОбщегоНазначенияБЭД.ДобавитьОшибку(ОшибкаФормирования.ОшибкиДанных.ЗаполнениеДанных,
					Сообщение.Текст, ДанныеОшибки);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПроверитьНепроверенныеПодписиПоМЧДВФоне", Истина);
