...
#Область ИнформированиеОбУдаленииУстаревшейФункциональности

// Устанавливает видимость и текст заголовка декорации, информирующей об удалении ценообразования версии 2.0.
//
// Параметры:
// 	Элементы - ЭлементыФормы
// 	ДобавитьСсылкуНаПомощник - Булево
//
Процедура УстановитьСвойстваДекорацииИнформированиеОбУдаленииЦенообразованияВерсии20(Элементы, ДобавитьСсылкуНаПомощник = Истина) Экспорт
	
	ВидимостьЭлемента = Не ЦенообразованиеВызовСервера.ИспользуетсяЦенообразование25('39990101');
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ДекорацияИнформированиеОбУдаленииЦенообразованияВерсии20",
		"Видимость",
		ВидимостьЭлемента);
	...
	Если ВидимостьЭлемента Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			...
			"ДекорацияИнформированиеОбУдаленииЦенообразованияВерсии20",
			"Заголовок",
			ТекстДекорацииИнформированиеОбУдаленииЦенообразованияВерсии20(ДобавитьСсылкуНаПомощник));
		
	КонецЕсли;
	
КонецПроцедуры

// Возвращает заголовок информационного сообщения об отключении ценообразование версии 2.0.
//
// Параметры:
// 	ДобавитьСсылкуНаПомощник - Булево
//
...
// 	Строка
//
Функция ТекстДекорацииИнформированиеОбУдаленииЦенообразованияВерсии20(ДобавитьСсылкуНаПомощник) Экспорт
	
	ВерсияПодсистемы20 = "11.0";
	ВерсияПодсистемы25 = "11.5";
	ВерсияПрограммы = "11.6.2";
	
	ЗаголовокСообщения = СтрШаблон(
		НСтр("ru = 'Внимание! Функциональность ""Ценообразование версии %1""'")
			+ " "
			+ НСтр("ru = 'устарела и будет удалена в версии программы %2.'")
			+ " "
			+ НСтр("ru = 'Пожалуйста, перейдите на ценообразование версии %3.'"),
		ВерсияПодсистемы20,
		ВерсияПрограммы,
		ВерсияПодсистемы25);
	
	ФорматированныеСтроки = Новый Массив;
	ФорматированныеСтроки.Добавить(Новый ФорматированнаяСтрока(БиблиотекаКартинок.Предупреждение));
	ФорматированныеСтроки.Добавить("  ");
	ФорматированныеСтроки.Добавить(Новый ФорматированнаяСтрока(ЗаголовокСообщения));
	
	Если ДобавитьСсылкуНаПомощник Тогда
		
		СсылкаНаОткрытиеПомощника = "e1cib/app/Обработка.ПомощникПереходаНаЦенообразованиеВерсии25";
		
		ФорматированныеСтроки.Добавить(" ");
		ФорматированныеСтроки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Для этого следует воспользоваться'")));
		ФорматированныеСтроки.Добавить(" ");
		ФорматированныеСтроки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'помощником'"),,,,
			СсылкаНаОткрытиеПомощника));
		ФорматированныеСтроки.Добавить(".");
