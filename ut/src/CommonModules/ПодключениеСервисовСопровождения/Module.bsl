...
Процедура ПриИзмененииДанныхАутентификации() Экспорт
	...
	Попытка
		...
		ЗапросыПодключения.Заполнить(Неопределено);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьДанныеСервисовСопровождения");
	Методы.Вставить("ПодключитьТестовыйПериод", Истина);
	Методы.Вставить("ПроверитьСостояниеЗапросаНаПодключение", Истина);
...
Процедура СоздатьРегламентноеЗаданиеПроверкиПодключения(
	...
	Попытка
		...
		ИдентификаторыЗапросовНабор.Заполнить(Неопределено);
...
Функция ИнформацияОДоступныхТестовыхПериодах(ИдентификаторыСервисов)
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		РезультатОперации.Ошибка = Истина;
		Если РезультатОтправки.КодОшибки = "LockError" Тогда
			РезультатОперации.ИнформацияОбОшибке = РезультатОтправки.ИнформацияОбОшибке;
		Иначе
