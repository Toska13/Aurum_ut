...
//
// Параметры:
// 	КодФормы - Строка - код формы
//
// Возвращаемое значение:
// 	Булево - Истина, если это любая из форм подбора
//
Функция ЭтоФормаПодбора(КодФормы) Экспорт
	
	Возврат КодФормы = "Обработка_ПодборТоваровВДокументПродажи_Форма"
...
//
// Параметры:
// 	КодФормы - Строка - код формы подбора
//
// Возвращаемое значение:
// 	Булево - Истина, если это форма подбора в документы закупки
//
Функция ЭтоФормаПодбораВДокументыЗакупки(КодФормы) Экспорт
	
	Возврат КодФормы = "Обработка_ПодборТоваровВДокументЗакупки_Форма";
...
//
Функция ЭтоФормаПодбораВДокументыПродажи(КодФормы) Экспорт
...
//
Функция ЭтоФормаВыбораХарактеристик(КодФормы) Экспорт
	
	Возврат КодФормы = "Справочник_ХарактеристикиНоменклатуры_ФормаВыбора";
...
//
Функция ЭтоФормаВыбораНоменклатуры(КодФормы) Экспорт
	
	Возврат КодФормы = "Справочник_Номенклатура_ФормаВыбора";
...
//
Функция ЭтоФормаСпискаНоменклатуры(КодФормы) Экспорт
	
	Возврат КодФормы = "Справочник_Номенклатура_ФормаСписка";
...
//
Функция ЭтоФормаПодобныеТоварыНоменклатуры(КодФормы) Экспорт
	
	Возврат КодФормы = "Справочник_Номенклатура_ПодобныеТовары";
...
//
Функция ЭтоФормаПодбораТоваровПоКатегориям(КодФормы) Экспорт
	
	Возврат КодФормы = "Обработка_ПодборПоТоварнымКатегориям_Форма";
...
//
Функция ЭтоФормаПрайсЛиста(КодФормы) Экспорт
	
	Возврат КодФормы = "Обработка_ПрайсЛист_Форма"
		Или КодФормы = "Обработка_ПрайсЛист_ФормаНастройки";
...
// 	Булево - Истина, если это форма прайс-листа
//
Функция ЭтоФормаПрайсЛистаПоставщиков(КодФормы) Экспорт
	
	Возврат КодФормы = "Обработка_ПрайсЛистПоставщика_Форма"
		Или КодФормы = "Обработка_ПрайсЛистПоставщика_ФормаНастройки";
...
Функция ИмяТекущейСтраницыПоВариантуПоиска(Форма) Экспорт
	...
	Если ЭтоФормаПодбора(Форма.КодФормы) Тогда
...
//	ЗначениеОтбора - Массив Из СправочникСсылка - массив значений отбора
...
Процедура СнятьОтборПоСтрокеПоискаНоменклатуры(Форма) Экспорт
	...
	Если Не ЭтоФормаПодбораТоваровПоКатегориям(Форма.КодФормы) Тогда
...
Процедура СнятьОтборПоСтрокеПоискаХарактеристик(Форма) Экспорт
	...
	Если Не ЭтоФормаПодбора(Форма.КодФормы) Тогда
		Возврат;
	КонецЕсли;
...
Процедура СнятьОтборПоСтрокеПоискаНоменклатурыПартнера(Форма) Экспорт
	...
	Если Не ЭтоФормаПодбораВДокументыЗакупки(Форма.КодФормы) Тогда
		Возврат;
	КонецЕсли;
...
Процедура УстановитьОтборПоДиапазонуЦен(Форма) Экспорт
	...
	ЭтоФормаПодбора = ЭтоФормаПодбора(Форма.КодФормы);
...
Процедура УдалитьОтборПоИерархииНоменклатуры(Форма) Экспорт
	...
	Если Не ЭтоФормаВыбораХарактеристик(Форма.КодФормы) Тогда
...
Процедура УстановитьТекущиеСтраницыПоВариантуПоиска(Форма) Экспорт
	...
	Если ЭтоФормаПодбора(Форма.КодФормы) Тогда
		...
		Если ЭтоФормаПодбораВДокументыЗакупки(Форма.КодФормы) Тогда
...
//
Функция ЕстьТоварыПродаваемыеСовместно(Объект) Экспорт
...
//
Процедура УстановитьВидимостьКомандыПодбораНоменклатурыПродаваемойСовместно(Форма) Экспорт
...
Процедура УстановитьДоступностьЭлементовФильтров(Форма) Экспорт
	...
	ЭтоФормаПодбора = ЭтоФормаПодбора(Форма.КодФормы);
	ЭтоФормаВыбораХарактеристик = ЭтоФормаВыбораХарактеристик(Форма.КодФормы);
	ЭтоФормаСпискаНоменклатуры = ЭтоФормаСпискаНоменклатуры(Форма.КодФормы);
	ЭтоФормаВыбораНоменклатуры = ЭтоФормаВыбораНоменклатуры(Форма.КодФормы);
	
	Если Не ЭтоФормаВыбораХарактеристик Тогда
		
		Форма.Элементы.ИерархияНоменклатуры.Доступность = Форма.ИспользоватьФильтры;
		Если Не ЭтоФормаПодобныеТоварыНоменклатуры(Форма.КодФормы) Тогда
...
// Возвращает параметры построения списка номенклатуры или характеристик.
//
// Возвращаемое значение:
// 	Структура:
// 		* ШаблонЗапроса - Строка
// 		* ЗаменыТекста - Соответствие Из Строка
// 		* ВременныеТаблицы - Массив Из Строка
// 		* ТипФормы - Строка
// 		* ТипСписка - Строка
// 		* ЕстьКурсыВБудущем - Булево
// 		* ЕстьЦеныВБудущем - Булево
// 		* ЕстьЦеныНоменклатурыПоставщиковВБудущем - Булево
// 		* ВариантРасчетаЦен - ПеречислениеСсылка.ВариантыРасчетаЦенПоСоглашениямСКлиентами
// 		* ВариантРасчетаЦенПоставщика - Строка
// 		* ИспользуетсяЦенообразование25 - Булево
// 		* РежимОтбораПоАссортименту - Булево
// 		* РежимПодбораБезСуммовыхПараметров - Булево
// 		* ЭтоПартнер - Булево
// 		* ТолькоВНаличии - Булево
// 		* НеИспользоватьВидыЦенПоУсловиюПродаж - Булево
// 		* НесколькоВидовЦен - Булево
// 		* НесколькоСкладов - Булево
// 		* ОтображатьОстаткиНаСкладах - Булево
// 		* ОтображатьОстаткиПоНазначению - Булево
// 		* ИспользоватьХарактеристикиНоменклатуры - Булево
// 		* ИспользоватьКомплекты - Булево
//
Функция НовыйПараметрыПостроенияЗапросаСписка() Экспорт
	
	ВариантРасчета = ПредопределенноеЗначение("Перечисление.ВариантыРасчетаЦенПоСоглашениямСКлиентами.ПустаяСсылка");
	
	Результат = Новый Структура;
	Результат.Вставить("ШаблонЗапроса", "");
	Результат.Вставить("ЗаменыТекста", Новый Соответствие());
	Результат.Вставить("ВременныеТаблицы", Новый Массив());
	Результат.Вставить("ТипФормы", "");
	Результат.Вставить("ТипСписка", "");
	Результат.Вставить("ЕстьКурсыВБудущем", Ложь);
	Результат.Вставить("ЕстьЦеныВБудущем", Ложь);
	Результат.Вставить("ЕстьЦеныНоменклатурыПоставщиковВБудущем", Ложь);
	Результат.Вставить("ВариантРасчетаЦен", ВариантРасчета);
	Результат.Вставить("ВариантРасчетаЦенПоставщика", "");
	Результат.Вставить("ИспользуетсяЦенообразование25", Ложь);
	Результат.Вставить("РежимОтбораПоАссортименту", Ложь);
	Результат.Вставить("РежимПодбораБезСуммовыхПараметров", Ложь);
	Результат.Вставить("ЭтоПартнер", Ложь);
	Результат.Вставить("ТолькоВНаличии", Ложь);
	Результат.Вставить("НеИспользоватьВидыЦенПоУсловиюПродаж", Ложь);
	Результат.Вставить("НесколькоВидовЦен", Ложь);
	Результат.Вставить("НесколькоСкладов", Ложь);
	Результат.Вставить("ОтображатьОстаткиНаСкладах", Ложь);
	Результат.Вставить("ОтображатьОстаткиПоНазначению", Ложь);
	Результат.Вставить("ОтображатьОстаткиКонтролируемойПартии", Ложь);
	Результат.Вставить("ИспользоватьХарактеристикиНоменклатуры", Ложь);
	Результат.Вставить("ИспользоватьКомплекты", Ложь);
	
	Возврат Результат;
	
КонецФункции

// Возвращает шаблон структуры параметров для получения информации о цене и остатках товара.
//
// Возвращаемое значение:
// 	Структура:
// 		* Номенклатура - СправочникСсылка.Номенклатура - номенклатура
...
// 		* Склады - СправочникСсылка.Склады, СписокЗначений Из СправочникСсылка.Склады - склад
// 		* ВидыЦен - СправочникСсылка.ВидыЦен, СписокЗначений Из СправочникСсылка.ВидыЦен - вид цены
// 		* КонтролируемаяПартия - СправочникСсылка.КонтролируемыеПартии - контролируемая партия
// 		* ПолучитьЦену - Булево - признак получения цены
// 		* ПолучитьОстатки - Булево - признак получения остатков
//
Функция НовыйПараметрыПолученияЦеныИОстатковТовара() Экспорт
	...
	Результат.Вставить("Соглашение", ПредопределенноеЗначение("Справочник.СоглашенияСКлиентами.ПустаяСсылка"));
	Результат.Вставить("Валюта", ПредопределенноеЗначение("Справочник.Валюты.ПустаяСсылка"));
	Результат.Вставить("Склады", ПредопределенноеЗначение("Справочник.Склады.ПустаяСсылка"));
	Результат.Вставить("ВидыЦен", ПредопределенноеЗначение("Справочник.ВидыЦен.ПустаяСсылка"));
	Результат.Вставить("КонтролируемаяПартия", ПредопределенноеЗначение("Справочник.КонтролируемыеПартии.ПустаяСсылка"));
	Результат.Вставить("ПолучитьЦену", Ложь);
	Результат.Вставить("ПолучитьОстатки", Ложь);
