...
// Путь к серверу с информацией по СДИЗ.
// 
// Параметры:
//  ИдентификаторФГИС - Строка - Идентификатор СДИЗ
//  РазделУчета - ПеречислениеСсылка.РазделыУчетаЗЕРНО - Раздел учета
// 
// Возвращаемое значение:
//  Строка - Путь к серверу с информацией по СДИЗ
Функция ПутьКСерверуСИнформациейПоСДИЗ(ИдентификаторФГИС, РазделУчета) Экспорт
	
	АдресСервера = ИнтеграцияЗЕРНОСлужебныйКлиентСервер.АдресСервера();
	Если РазделУчета = ПредопределенноеЗначение("Перечисление.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна") Тогда
...
// Путь к серверу с информацией о партии.
// 
// Параметры:
//  ИдентификаторФГИС - Строка - Идентификатор партии
//  РазделУчета - ПеречислениеСсылка.РазделыУчетаЗЕРНО - Раздел учета
// 
// Возвращаемое значение:
//  Строка - Путь к серверу с информацией о партии
Функция ПутьКСерверуСИнформациейОПартии(ИдентификаторФГИС, РазделУчета) Экспорт
...
//   Возвращает виды продукции ИС по разделу учета ЗЕРНО
// Параметры:
//  РазделыУчета - ПеречислениеСсылка.РазделыУчетаЗЕРНО, Массив Из ПеречислениеСсылка.РазделыУчетаЗЕРНО - исходные виды продукции
// 
// Возвращаемое значение:
//  Массив Из СправочникСсылка.ВидыПродукцииИС - расширенные виды продукции
Функция ВидыПродукции(Знач РазделыУчета) Экспорт
	
	Результат = Новый Массив;
	Если Не ЗначениеЗаполнено(РазделыУчета) Тогда
		Результат = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЗЕРНО();
	ИначеЕсли ТипЗнч(РазделыУчета) = Тип("ПеречислениеСсылка.РазделыУчетаЗЕРНО") Тогда
		Результат = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииПоЗначениюЭлементаКонструктора(
			ПредопределенноеЗначение("Справочник.ЭлементыКонструктораВидовПродукцииИС.РазделыУчетаЗЕРНО"), РазделыУчета);
	ИначеЕсли ТипЗнч(РазделыУчета) = Тип("Массив") Или ТипЗнч(РазделыУчета) = Тип("ФиксированныйМассив") Тогда
		Для Каждого РазделУчета Из РазделыУчета Цикл
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Результат, ВидыПродукции(РазделУчета));
		КонецЦикла;
	КонецЕсли;
...
// Виды продукции ФГИС "Зерно" по видам продукции:
//   Возвращает разделы учета для указанных видов продукции
// Параметры:
//  ВидыПродукции - СправочникСсылка.ВидыПродукцииИС, Массив Из СправочникСсылка.ВидыПродукцииИС - виды продукции
// 
// Возвращаемое значение:
//  Массив Из ПеречислениеСсылка.РазделыУчетаЗЕРНО - разделы учета.
Функция РазделыУчета(Знач ВидыПродукции) Экспорт
	
	Возврат КонструкторИСОбщегоНазначенияКлиентСервер.ЗначенияЭлементаКонструктораПоВидуПродукции(
		ПредопределенноеЗначение(
			"Справочник.ЭлементыКонструктораВидовПродукцииИС.РазделыУчетаЗЕРНО"),
			ВидыПродукции);
...
//  РазделУчета - ПеречислениеСсылка.РазделыУчетаЗЕРНО, Массив Из ПеречислениеСсылка.РазделыУчетаЗЕРНО - Вид продукции.
//  ИмяПоляВвода - Строка - имя поля ввода номенклатуры.
Процедура УстановитьПараметрыВыбораНоменклатуры(Форма, РазделУчета, ИмяПоляВвода = "ТоварыНоменклатура") Экспорт
	
	СобытияФормИСКлиентСерверПереопределяемый.УстановитьПараметрыВыбораНоменклатуры(
		Форма, ВидыПродукции(РазделУчета), ИмяПоляВвода);
...
// Возвращает структуру с именами реквизитов, используемых для определения доступности действий.
//
// Возвращаемое значение:
//   Структура:
//     * Организация - Строка, Неопределено - имя реквизита организации
//     * Подразделение - Строка, Неопределено - имя реквизита подразделения
Функция ИменаРеквизитовОпределенияДоступностиДействий() Экспорт
	
	ИменаРеквизитов = Новый Структура();
	ИменаРеквизитов.Вставить("Организация",   Неопределено);
	ИменаРеквизитов.Вставить("Подразделение", Неопределено);
...
Функция ИнициализироватьПараметрыНастройкиОбмена(ВидНастройкиОбмена) Экспорт
	...
	Если ВидНастройкиОбмена = ПредопределенноеЗначение("Перечисление.ВидыНастроекОбменаЗЕРНО.ЗагрузкаСДИЗ") Тогда
		ПараметрыНастройкиОбмена.Вставить("РазделУчета",
			ПредопределенноеЗначение("Перечисление.РазделыУчетаЗЕРНО.ПустаяСсылка"));
