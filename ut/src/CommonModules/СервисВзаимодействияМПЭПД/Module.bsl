		...
		Для Каждого СтрокаМассива Из ДанныеОтвета.List Цикл
			...
			Если Не ПустаяСтрока(СтрокаМассива.ServiceData) Тогда
				...
				Если СервисВзаимодействияМПЭПДКлиентСервер.ЭтоТипСообщенияОбОтправкеТитулаИзМП(СтруктураДанныхСообщения.Type)
					Или СервисВзаимодействияМПЭПДКлиентСервер.ЭтоТипСообщенияОПросмотреТитулаВМП(СтруктураДанныхСообщения.Type)
...
Процедура ОбслуживаниеПолучениеНовыхУведомлений(Организация = Неопределено) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ВсеУчетныеЗаписи Цикл
		...
		|	НЕ (РеестрЭПД.Ссылка ЕСТЬ NULL) КАК ДокументЕстьВРеестреЭПД,
		|	РеестрЭПД.ОжидаемПросмотрМП КАК ОжидаемПросмотрМП,
		|	РеестрЭПД.ТекущийШаг КАК ТекущийШаг,
		|	РеестрЭПД.ТекущийШагВыполнен КАК ТекущийШагВыполнен
		|ИЗ
		...
		Для Каждого СтрокаМассива Из МассивСобытийИУведомлений Цикл
			...
			МассивЗагружаемыхВзаимодействий = СервисВзаимодействияМПЭПДКлиентСервер.МассивЗагружаемыхТиповВзаимодействий();
			Если МассивЗагружаемыхВзаимодействий.Найти(СтрокаМассива.ВидСообщения) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			...
			Если Не ЗначениеЗаполнено(СтрокаМассива.ИдентификаторМП)
				Или Не ЕстьДокумент
				Или СтрокаМассива.ВидСообщения = Перечисления.ТипыВзаимодействияМПЭПД.ПовторнаяРегистрацияМП
				Или СтрокаМассива.ВидСообщения = Перечисления.ТипыВзаимодействияМПЭПД.ИзменениеСведенийМП
				Или Не ВыборкаДокументов.ДокументЕстьВРеестреЭПД Тогда
				Продолжить;
			КонецЕсли;
			...
			Если ЭтоСообщение Тогда
				Если Не ЗначениеЗаполнено(СтрокаМассива.ДанныеСообщения) Тогда
					Продолжить;
				КонецЕсли;
				Тип = Неопределено;
				...
				ПроверятьТитулыРеестраИСообщенияНаСовпадение = Истина;
				ЭтоТипСообщенияОбОтправкеТитула = 
					СервисВзаимодействияМПЭПДКлиентСервер.ЭтоТипСообщенияОбОтправкеТитулаИзМП(Тип);
				Если ЭтоТипСообщенияОбОтправкеТитула Тогда
					ТипДляОпределенияТитула =
						СервисВзаимодействияМПЭПДКлиентСервер.ПривестиТипСообщенияКВидуПолученияТитулаБЭД(Тип);
						ТекущийШагМП = Тип;
				Иначе
					Если Тип = Неопределено Тогда
						ТекущийШагМП = "";
					Иначе
						ТекущийШагМП = Тип + ?(Принято = Ложь, "_reject", "");
					КонецЕсли;
				...
				ОжидаемПросмотрМП = ОжидаетсяПросмотрТитулаВМП(ВыборкаДокументов,
					СтруктураДанныхСообщения, ВыборкаДокументов);
			Иначе
				Если ТипЗнч(ВыборкаДокументов.ОбъектУчета) = Тип("ДокументСсылка.ЭлектроннаяТранспортнаяНакладная") Тогда
					ТитулСобытияУведомления = Неопределено;
					ПроверятьТитулыРеестраИСообщенияНаСовпадение = Ложь;
				Иначе
					Если СтрокаМассива.ВидСообщения = Перечисления.ТипыВзаимодействияМПЭПД.ПривязкаВодителя 
						Или СтрокаМассива.ВидСообщения = Перечисления.ТипыВзаимодействияМПЭПД.ОтвязкаВодителя Тогда
						ТитулСобытияУведомления = Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул1;
					Иначе
						ТитулСобытияУведомления = Неопределено;
					КонецЕсли;
				КонецЕсли;
			...
			Попытка
				...
				Если ПроверятьТитулыРеестраИСообщенияНаСовпадение Тогда
					ТитулСобытияИРеестраСовпадают = ТитулСобытияУведомления = ТитулИзРеестра;
				Иначе
					ТитулСобытияИРеестраСовпадают = Истина;
				КонецЕсли;
				
				Если ЭтоСообщение И Не ТитулСобытияИРеестраСовпадают Тогда
					МенеджерЗаписиРеестраЭПД.ТекущийШагМП = ТекущийШагМП;
					Если МенеджерЗаписиРеестраЭПД.ОжидаемПросмотрМП Тогда
						МенеджерЗаписиРеестраЭПД.ОжидаемПросмотрМП = ОжидаемПросмотрМП;
					КонецЕсли;
					
				ИначеЕсли ЭтоСообщение И ЕстьДанныеМП И ТитулСобытияИРеестраСовпадают Тогда
					...
					Если ВыборкаДокументов.ТекущийШагВыполнен Тогда
				...
				ИначеЕсли ЭтоПривязкаДокумента И ЕстьДанныеМП И ТитулСобытияИРеестраСовпадают Тогда
					МенеджерЗаписиРеестраЭПД.МобильноеПриложение = ВыборкаДанныхМП.ИдентификаторМП;
					МенеджерЗаписиРеестраЭПД.ИдентификаторЗаписиМП = ВыборкаДанныхМП.ИдентификаторЗаписиМП;
					МенеджерЗаписиРеестраЭПД.НаименованиеМП = ВыборкаДанныхМП.НаименованиеМП;

				ИначеЕсли ТитулСобытияИРеестраСовпадают Тогда
					// Событие отвязки или удаления МП.
					...
					МенеджерЗаписиРеестраЭПД.ТекущийШагМП = "";
					МенеджерЗаписиРеестраЭПД.ОжидаемПросмотрМП = Ложь;
				Иначе
					ОтменитьТранзакцию();
					Продолжить;
				КонецЕсли;
			...
			Исключение
				...
				РегистрацияВЖурнале(УровеньЖурналаРегистрации.Ошибка,
		...
		Если ЗначениеЗаполнено(НоваяТочкаУведомлений) ИЛИ ЗначениеЗаполнено(НоваяТочкаСобытий) Тогда
			ТекущаяЗапись = РегистрыСведений.НастройкиВзаимодействияМПЭПД.ПараметрыЗаписиРегистра(
				СтрокаТаблицы.УчетнаяЗаписьЭДО);
...
Процедура СервисОтправитьНастройкиДополнительныхПолей(МассивНастроек) Экспорт
	...
	МассивМП = СервисВзаимодействияМПЭПДВызовСервера.СписокПодключенныхМП();
...
// Добавляет в массив реквизиты МП.
//  
// Параметры:
...
Процедура ПриЗаполненииКонтрагентаПриРаспаковкеКонтейнера(ПараметрыКонтейнера, РезультатРаспаковки, ПараметрыПриглашения, Контрагент) Экспорт
	...
	Если ЭтоИсходящийКонтейнер Тогда
		...
		ЗаполнятьПредставлениеКонтрагента = Ложь;
		ДанныеПриглашения = ПриглашенияЭДО.ДанныеПриглашенияПоПараметрам(
			ПараметрыПриглашения,
			ЗаполнятьПредставлениеКонтрагента);
...
// Возвращает признак необходимости добавления данных МП.
//
// Параметры:
//  ПервичнаяЗаписьРегистра	 - Структура, РегистрСведенийМенеджер.РеестрЭПД	 - 
//  ИзмененнаяЗаписьРегистра - Структура, РегистрСведенийМенеджер.РеестрЭПД	 - 
// 
// Возвращаемое значение:
//  Булево - Признак необходимости добавления данных МП.
//
Функция НужноДобавитьДанныеМП(ПервичнаяЗаписьРегистра, ИзмененнаяЗаписьРегистра) Экспорт
	
	Если ТипЗнч(ИзмененнаяЗаписьРегистра.Ссылка) <> Тип("ДокументСсылка.ЭлектроннаяТранспортнаяНакладная")
		И ТипЗнч(ИзмененнаяЗаписьРегистра.Ссылка) <> Тип("ДокументСсылка.ЭлектронныйПутевойЛист") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ТекущийШагИзменился = ПервичнаяЗаписьРегистра.ТекущийШаг <> ИзмененнаяЗаписьРегистра.ТекущийШаг
		Или ПервичнаяЗаписьРегистра.ТекущийШагВыполнен <> ИзмененнаяЗаписьРегистра.ТекущийШагВыполнен;
		   
	ШагиТребующиеПросмотрМП = ШагиТребующиеПросмотрМП();
	ЭтотШагТребуетРасчетаПросмотра =
		ШагиТребующиеПросмотрМП.Найти(ИзмененнаяЗаписьРегистра.ТекущийШаг) <> Неопределено
		И ИзмененнаяЗаписьРегистра.ТекущийШагВыполнен;
		
	НужноДобавитьДанныеМП = ТекущийШагИзменился Или ЭтотШагТребуетРасчетаПросмотра;
		
	Возврат НужноДобавитьДанныеМП
...
//  СоответствиеЭлектронныхДокументов - Неопределено, Соответствие из ДокументСсылка - Соответствие электронных документов и документов учета.
//		Обязательно при записи новых документов, т.к. в этом случае записей в регистре ОбъектыУчетаДокументовЭДО еще нет.
//
Процедура ДобавитьДанныеМобильногоПриложенияДляЗаписиВРеестр(КоллекцияЗаписейРегистра, ЗаписьИзФормы,
		СоответствиеЭлектронныхДокументов = Неопределено) Экспорт
	...
	ОписаниеТиповЭлектронныйДокумент = Новый ОписаниеТипов("ДокументСсылка.ЭлектронныйДокументВходящийЭДО, ДокументСсылка.ЭлектронныйДокументИсходящийЭДО");
	ТаблицаДоступныхВзаимодействий.Колонки.Добавить("Документ", ОписаниеТиповДокумент); 
	ТаблицаДоступныхВзаимодействий.Колонки.Добавить("ЭлектронныйДокумент", ОписаниеТиповЭлектронныйДокумент); 
	...
	ЗапросСОтборомПоЭлектронномуДокументу = СоответствиеЭлектронныхДокументов <> Неопределено;
	МожноРассчитатьДанныеОПросмотреМП = Ложь;
	ШагиТребующиеПросмотрМП = ШагиТребующиеПросмотрМП();
	
	// Подготовка таблицы доступных взаимодействий.
	Для Каждого ЗаписьРегистра Из КоллекцияЗаписейРегистра Цикл
		...
		Если Не МожноРассчитатьДанныеОПросмотреМП
			И ШагиТребующиеПросмотрМП.Найти(ЗаписьРегистра.ТекущийШаг) <> Неопределено
			И ЗаписьРегистра.ТекущийШагВыполнен Тогда
			МожноРассчитатьДанныеОПросмотреМП = Истина;
		КонецЕсли;
		...
		Если ТипЗнч(Документ) = Тип("ДокументСсылка.ЭлектроннаяТранспортнаяНакладная") Тогда
			МассивДоступныхИндексов = СервисВзаимодействияМПЭПДКлиентСервер.МассивДоступныхИндексовРаботПоРолиУчастника(
				Документ, РольУчастника);
			ПоТитуламМогутБытьВзаимодействия = ИндексРаботыДоступен
				Или Титул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул7
				Или Титул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_ДопТитул7
				Или Титул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул8
				Или Титул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_ДопТитул8;
			Если МассивДоступныхИндексов.Количество() = 0 Или Не ПоТитуламМогутБытьВзаимодействия Тогда
				Продолжить;
			КонецЕсли;
		Иначе
		...
		МассивДоступныхВидовВзаимодействия = 
			СервисВзаимодействияМПЭПДКлиентСервер.МассивДоступныхВидовВзаимодействияПоТитулу(Титул);
		Для Каждого ВидВзаимодействия Из МассивДоступныхВидовВзаимодействия Цикл
			...
			Если ЗапросСОтборомПоЭлектронномуДокументу Тогда
				НоваяСтрока.ЭлектронныйДокумент = СоответствиеЭлектронныхДокументов[Документ];
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	...
	Если ЗапросСОтборомПоЭлектронномуДокументу Тогда
		СтруктураЗапроса = ТекстИИндексЗапросаСообщенийМП_ОтборПоЭлектронномуДокументу();
		ТекстЗапроса = СтруктураЗапроса.ТекстЗапроса; 
		ИндексВыборкиДокументов = СтруктураЗапроса.ИндексРезультата;
	Иначе
		СтруктураЗапроса = ТекстИИндексЗапросаСообщенийМП_ОтборПоОбъектамУчета();
		ТекстЗапроса = СтруктураЗапроса.ТекстЗапроса; 
		ИндексВыборкиДокументов = СтруктураЗапроса.ИндексРезультата;
	КонецЕсли;
	...
	Если МожноРассчитатьДанныеОПросмотреМП Тогда
		СтруктураЗапроса = ТекстИИндексЗапросаДанныхОПросмотреМП();
		РазделительПакетаЗапросов = ОбщегоНазначения.РазделительПакетаЗапросов();
		Запрос.Текст = Запрос.Текст + РазделительПакетаЗапросов + СтруктураЗапроса.ТекстЗапроса;
		ИндексДанныхОПросмотреМП = ИндексВыборкиДокументов + СтруктураЗапроса.ИндексРезультата + 1;
	КонецЕсли;
	
	// Получение результата запроса.
	Если МожноРассчитатьДанныеОПросмотреМП Тогда
		МассивРезультатов = Запрос.ВыполнитьПакет();
		ВыборкаДокументов = МассивРезультатов[ИндексВыборкиДокументов].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		ВыборкаДанныхОПросмотреДокумента = МассивРезультатов[ИндексДанныхОПросмотреМП].Выбрать();
	Иначе
	...
	// Добавление в записи регистра данных МП, доступных для текущего титула (ТаблицаДоступныхВзаимодействий).
	Для Каждого ЗаписьРегистра Из КоллекцияЗаписейРегистра Цикл
		ВыборкаДокументов.Сбросить();
		ТитулЗаписиРегистра = ТитулТекущегоШагаДокумента(ЗаписьРегистра.Ссылка, ЗаписьРегистра.ТекущийШаг);
		
		МожетБытьДобавленПризнакОжиданияПросмотраМП =
			МожноРассчитатьДанныеОПросмотреМП
			И ШагиТребующиеПросмотрМП.Найти(ЗаписьРегистра.ТекущийШаг) <> Неопределено
			И ЗаписьРегистра.ТекущийШагВыполнен;
		...
		Если Не ВыборкаДокументов.НайтиСледующий(СтруктураПоискаПоДокументу) Тогда
			...
			УбратьДанныеМПИзЗаписиРегистра(ЗаписьРегистра);
			Продолжить;
		КонецЕсли;
		
		ВыборкаДанныхПоДокументу = ВыборкаДокументов.Выбрать();
		
		// Добавление документов в КЭШ взаимодействия с МП.
		Если ЗапросСОтборомПоЭлектронномуДокументу Тогда
			ПриЗаписиРеестраДобавитьДокументВКэшВзаимодействияМПЭПД(Документ, ВыборкаДанныхПоДокументу);
		КонецЕсли;
		
		Пока ВыборкаДанныхПоДокументу.Следующий() Цикл
			ЭтоСообщение = СервисВзаимодействияМПЭПДКлиентСервер.ЭтоТипВзаимодействияСообщение(
				ВыборкаДанныхПоДокументу.ВидСообщения);
			Если Не ЭтоСообщение Тогда
				Прервать;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ВыборкаДанныхПоДокументу.ДанныеСообщения) Тогда
				СтруктураПризнаков = ТипИПризнакПринятоСообщенияМП(ВыборкаДанныхПоДокументу.ДанныеСообщения);
				Тип = СтруктураПризнаков.Тип;
				ТитулПринят = СтруктураПризнаков.Принято;
				ЭтоТипСообщенияОбОтправкеТитула = СтруктураПризнаков.ЭтоТипСообщенияОбОтправкеТитула;
				
				ТитулВСообщении = СоответствиеТитуловТипамМП.Получить(Тип);
		...
		ЭтоПривязкаДокумента = 
			СервисВзаимодействияМПЭПДКлиентСервер.ЭтоПривязкаДокумента(ВыборкаДанныхПоДокументу.ВидСообщения);
			
		Если Не ЭтоПривязкаДокумента И Не ЭтоСообщение Тогда
			// Обработка отвязки документа от МП.
			УбратьДанныеМПИзЗаписиРегистра(ЗаписьРегистра, МожетБытьДобавленПризнакОжиданияПросмотраМП);
		...
		ИначеЕсли ЭтоСообщение Тогда
			...
			ИначеЕсли Тип <> Неопределено Тогда
				ЗаписьРегистра.ТекущийШагМП = Тип + ?(ТитулПринят = Ложь, "_reject", "");
			КонецЕсли;		
		...
		Если МожетБытьДобавленПризнакОжиданияПросмотраМП Тогда
			ПриЗаписиРеестраУстановитьПризнакОжиданияПросмотраМП(ЗаписьРегистра, ВыборкаДанныхПоДокументу,
					ВыборкаДанныхОПросмотреДокумента, СтруктураПоискаПоДокументу);
		КонецЕсли;
		
		// Отметка о том, что данные сообщения МП просмотрены на форме.
...
//  ТолькоАктивные - Булево - Возвращаются только активные МП.
//  ВернутьМассив - Булево - Признак возвращения массива, в ином случае вернется таблица.
// 
// Возвращаемое значение:
...
//@skip-check doc-comment-description-ends-on-dot
Функция СписокПодключенныхМП(Отборы = Неопределено, ТолькоАктивные = Ложь) Экспорт
...
//@skip-check bsl-ql-hub
//@skip-check doc-comment-description-ends-on-dot
Функция СтруктураДанныхМП(ИдентификаторЭДО, Роль, Идентификатор = Неопределено,
		СтруктураКлючевыхПолей = Неопределено) Экспорт
...
Функция ТекстИИндексЗапросаСообщенийМП_ОтборПоОбъектамУчета()
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТаблицаДоступныхВзаимодействий.Документ КАК Документ,
	|	ВТ_ТаблицаДоступныхВзаимодействий.ВидВзаимодействия КАК ВидВзаимодействия,
	|	ОбъектыУчетаДокументовЭДО.ЭлектронныйДокумент КАК ЭлектронныйДокумент
	|ПОМЕСТИТЬ ВТ_ТаблицаВзаимодействийСЭлектроннымДокументом
	|ИЗ
	|	ВТ_ТаблицаДоступныхВзаимодействий КАК ВТ_ТаблицаДоступныхВзаимодействий
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОбъектыУчетаДокументовЭДО КАК ОбъектыУчетаДокументовЭДО
	|		ПО ВТ_ТаблицаДоступныхВзаимодействий.Документ = ОбъектыУчетаДокументовЭДО.ОбъектУчета
	|			И (ОбъектыУчетаДокументовЭДО.Актуальный = ИСТИНА)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТаблицаВзаимодействийСЭлектроннымДокументом.Документ КАК Документ,
	|	ВТ_ТаблицаВзаимодействийСЭлектроннымДокументом.ВидВзаимодействия КАК ВидВзаимодействия,
	|	ЕСТЬNULL(ЭлектронныйДокументВходящийЭДО.ИдентификаторДокументооборота, ЭлектронныйДокументИсходящийЭДО.ИдентификаторДокументооборота) КАК ИдентификаторДокументооборота
	|ПОМЕСТИТЬ ВТ_ТаблицаВзаимодействийС_ИД_ДО
	|ИЗ
	|	ВТ_ТаблицаВзаимодействийСЭлектроннымДокументом КАК ВТ_ТаблицаВзаимодействийСЭлектроннымДокументом
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭлектронныйДокументВходящийЭДО КАК ЭлектронныйДокументВходящийЭДО
	|		ПО ВТ_ТаблицаВзаимодействийСЭлектроннымДокументом.ЭлектронныйДокумент = ЭлектронныйДокументВходящийЭДО.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭлектронныйДокументИсходящийЭДО КАК ЭлектронныйДокументИсходящийЭДО
	|		ПО ВТ_ТаблицаВзаимодействийСЭлектроннымДокументом.ЭлектронныйДокумент = ЭлектронныйДокументИсходящийЭДО.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТаблицаВзаимодействийС_ИД_ДО.Документ КАК Документ,
	...
	|	ПодключенныеМПЭПД.Наименование КАК НаименованиеМП
	|ИЗ
	|	ВТ_ТаблицаВзаимодействийС_ИД_ДО КАК ВТ_ТаблицаВзаимодействийС_ИД_ДО
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.КэшВзаимодействияМПЭПД КАК КэшВзаимодействияМПЭПД
	|		ПО ВТ_ТаблицаВзаимодействийС_ИД_ДО.ИдентификаторДокументооборота = КэшВзаимодействияМПЭПД.ИдентификаторДокумента
	|			И ВТ_ТаблицаВзаимодействийС_ИД_ДО.ВидВзаимодействия = КэшВзаимодействияМПЭПД.ВидСообщения
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	КэшВзаимодействияМПЭПД.Дата УБЫВ
	|ИТОГИ ПО
	|	Документ";
	ИндексРезультата = 3;
	
	Возврат Новый Структура("ТекстЗапроса, ИндексРезультата", ТекстЗапроса, ИндексРезультата);
	
КонецФункции

Функция ТекстИИндексЗапросаСообщенийМП_ОтборПоЭлектронномуДокументу()
	...
	|	ТаблицаДоступныхВзаимодействий.Документ КАК Документ,
	|	ТаблицаДоступныхВзаимодействий.ВидВзаимодействия КАК ВидВзаимодействия,
	|	ТаблицаДоступныхВзаимодействий.ЭлектронныйДокумент КАК ЭлектронныйДокумент
	|ПОМЕСТИТЬ ВТ_ТаблицаВзаимодействийСЭлектроннымДокументом
	|ИЗ
	|	&ТаблицаДоступныхВзаимодействий КАК ТаблицаДоступныхВзаимодействий
	|;
...
Функция ТекстИИндексЗапросаДанныхОПросмотреМП()
	...
	|			(ВЫБРАТЬ
	|				ВТ_ТаблицаВзаимодействийСЭлектроннымДокументом.Документ
	|			ИЗ
	|				ВТ_ТаблицаВзаимодействийСЭлектроннымДокументом)";
	ИндексРезультата = 0;
...
// Возвращает признак ожидание просмотра данных МП.
//
// Параметры:
//  ДанныеРеестра - РегистрСведенийЗапись.РеестрЭПД, ВыборкаИзРезультатаЗапроса - Данные с записью регистра, необходимо передать значения
//																				  ресурсов ТекущийШаг, ОжидаемПросмотрМП.
//  ДанныеСообщения - Структура, Соответствие из Строка - Данные сообщения сервера МП. Неопределено - если сообщения нет.
//  РеквизитыДокумента - ВыборкаИзРезультатаЗапроса - Выборка с реквизитами ТитулПереадресовкаИдентификаторФайла, ДопТитулПереадресовкаИдентификаторФайла
//													  ТитулПеревозчикаЗаменыИдентификаторФайла, ДопТитулПеревозчикаЗаменыИдентификаторФайла.
// 
// Возвращаемое значение:
//  Булево - Признак ожидания просмотра данных МП.
//
Функция ОжидаетсяПросмотрТитулаВМП(ДанныеРеестра, ДанныеСообщения, РеквизитыДокумента)
	
	Если Не ЗначениеЗаполнено(ДанныеСообщения) Или Не ДанныеРеестра.ОжидаемПросмотрМП Тогда
		Возврат ДанныеРеестра.ОжидаемПросмотрМП;
	КонецЕсли;
	
	ТекущийТипСообщения = Неопределено;
	ДанныеСообщения.Свойство("Type", ТекущийТипСообщения);
	
	ИмяФайлИзСообщенияМП = Неопределено;
	ДанныеСообщения.Свойство("fileid", ИмяФайлИзСообщенияМП);
	Если Не ЗначениеЗаполнено(ИмяФайлИзСообщенияМП) Тогда
		Возврат Истина;
	КонецЕсли;
	...
	ТекущийШаг = ДанныеРеестра.ТекущийШаг;
	
	Если СоответствиеТитулов.Получить(ТекущийШаг) = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул7
		Или СоответствиеТитулов.Получить(ТекущийШаг) = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_ДопТитул7 Тогда
		РеквизитИмениФайлаТитула = "ТитулПереадресовкаИдентификаторФайла";
		РеквизитИмениФайлаДопТитула = "ДопТитулПереадресовкаИдентификаторФайла";
		ОжидаемыйТипСообщения = "readressing_viewed";
	ИначеЕсли СоответствиеТитулов.Получить(ТекущийШаг) = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул8
		Или СоответствиеТитулов.Получить(ТекущийШаг) = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_ДопТитул8 Тогда
		РеквизитИмениФайлаТитула = "ТитулПеревозчикаЗаменыИдентификаторФайла";
		РеквизитИмениФайлаДопТитула = "ДопТитулПеревозчикаЗаменыИдентификаторФайла";
		ОжидаемыйТипСообщения = "replacement_viewed";
	Иначе
		Возврат Истина;
	КонецЕсли;
	
	Если ТекущийТипСообщения = ОжидаемыйТипСообщения Тогда
		ИмяФайлаТитула = РеквизитыДокумента[РеквизитИмениФайлаТитула];
		ИмяФайлаДопТитула = РеквизитыДокумента[РеквизитИмениФайлаДопТитула];
		ТитулПросмотренВМП = ИмяФайлаТитула = ИмяФайлИзСообщенияМП
			Или ИмяФайлаДопТитула = ИмяФайлИзСообщенияМП;
	Иначе
		ТитулПросмотренВМП = Ложь;
	КонецЕсли;
	
	Возврат Не ТитулПросмотренВМП;
...
Процедура ПриЗаписиРеестраУстановитьПризнакОжиданияПросмотраМП(ЗаписьРегистра, ВыборкаДанныхПоДокументу,
	ВыборкаДанныхОПросмотреДокумента, СтруктураПоискаПоДокументу)
	
	Если ВыборкаДанныхОПросмотреДокумента.НайтиСледующий(СтруктураПоискаПоДокументу) Тогда
		ВыборкаДанныхПоДокументу.Сбросить();
		ЗаписьРегистра.ОжидаемПросмотрМП = Истина;
		Пока ВыборкаДанныхПоДокументу.Следующий() Цикл
			ЭтоСообщение = СервисВзаимодействияМПЭПДКлиентСервер.ЭтоТипВзаимодействияСообщение(
				ВыборкаДанныхПоДокументу.ВидСообщения);
			Если Не ЭтоСообщение Или Не ЗначениеЗаполнено(ВыборкаДанныхПоДокументу.ДанныеСообщения) Тогда
				Продолжить;
			КонецЕсли;
			
			СтруктураДанныхСообщения = СтруктураИзJson(ВыборкаДанныхПоДокументу.ДанныеСообщения);
			ОжидаемПросмотрМП = ОжидаетсяПросмотрТитулаВМП(ЗаписьРегистра, СтруктураДанныхСообщения,
				ВыборкаДанныхОПросмотреДокумента);
			// Нашли сообщение с данными о просмотре.
			Если Не ОжидаемПросмотрМП Тогда
...
Процедура ПриЗаписиРеестраДобавитьДокументВКэшВзаимодействияМПЭПД(Документ, ВыборкаДанныхПоДокументу)
	
	Пока ВыборкаДанныхПоДокументу.Следующий() Цикл
		Если ЗначениеЗаполнено(ВыборкаДанныхПоДокументу.ДокументЭПД) Тогда
			Продолжить;
		КонецЕсли;
		МенеджерЗаписи = РегистрыСведений.КэшВзаимодействияМПЭПД.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.Идентификатор = ВыборкаДанныхПоДокументу.Идентификатор;
		МенеджерЗаписи.Прочитать();
		МенеджерЗаписи.ДокументЭПД = Документ;
...
Процедура УбратьДанныеМПИзЗаписиРегистра(ЗаписьРегистра, ПризнакОжиданияПросмотраМП = Неопределено)
	...
	ПризнакОжиданияПросмотраМП = Ложь;
	
КонецПроцедуры

Функция ТипИПризнакПринятоСообщенияМП(ДанныеСообщения)
	
	Результат = Новый Структура;
	Результат.Вставить("Тип");
	Результат.Вставить("Принято");
	Результат.Вставить("ЭтоТипСообщенияОбОтправкеТитула", Ложь);
	
	Если ЗначениеЗаполнено(ДанныеСообщения) Тогда
		СтруктураДанныхСообщения = СтруктураИзJson(ДанныеСообщения);
		Тип = Неопределено;
		СтруктураДанныхСообщения.Свойство("Type", Тип);
		...
		ЭтоТипСообщенияОбОтправкеТитула = 
			СервисВзаимодействияМПЭПДКлиентСервер.ЭтоТипСообщенияОбОтправкеТитулаИзМП(Тип);
		Если ЭтоТипСообщенияОбОтправкеТитула Тогда
			Тип = СервисВзаимодействияМПЭПДКлиентСервер.ПривестиТипСообщенияКВидуПолученияТитулаБЭД(Тип);
		КонецЕсли;
		...
		Результат.ЭтоТипСообщенияОбОтправкеТитула = ЭтоТипСообщенияОбОтправкеТитула;
	КонецЕсли;
...
Функция СоответствиеВидаПодписи(ТекущееЗначение)
	...
	ВсеПредставления.Вставить(Перечисления.ВидыПодписиМПЭПД.НеИспользуется, "Simple");
...
Функция ТитулТекущегоШагаДокумента(Документ, ТекущийШаг)
	
	СоответствиеТитуловИШагов = ОбменСГИСЭПДКлиентСервер.СоответствиеТитуловИШагов(Документ);
	СоответствиеТитуловИШагов = СервисВзаимодействияМПЭПДКлиентСервер.ПеревернутьСоответствие(
		СоответствиеТитуловИШагов);
	ТитулЗаписиРегистра = СоответствиеТитуловИШагов.Получить(ТекущийШаг);

	Возврат ТитулЗаписиРегистра;
...
Процедура ОбновитьТокенДоступа(ОтветСервиса, ПараметрыОперации, ОписаниеСоединения, ТекущийШаг)
	...
	ПараметрыОбновления.Токен = ПараметрыОперации.Токен;
	...
	Если РезультатЗапроса.Успех и РезультатЗапроса.Ответ.КодСостояния = 200 Тогда
		ДанныеОтвета = СтруктураИзJson(РезультатЗапроса.Ответ.ПолучитьТелоКакСтроку(), Истина);
...
// Выполнение операции на сервере МП.
// 
// Параметры:
// 	ПараметрыОперации - см. ПараметрыВыполненияОперацииСервиса
// Возвращаемое значение:
