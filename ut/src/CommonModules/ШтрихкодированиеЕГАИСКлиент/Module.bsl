...
Функция ДанныеПроверкиПередОбработкойШтрихкода(Штрихкод) Экспорт
	...
	ДанныеРазбора = РазборКодаМаркировкиИССлужебныйКлиент.РазобратьКодМаркировки(
		Штрихкод, ВидыПродукцииЕГАИС);
	
	ВыводитьСообщениеОбОшибке = Ложь;
	Если ДанныеРазбора = Неопределено Тогда
		ВыводитьСообщениеОбОшибке = Истина;
	Иначе
		ЕстьВидПродукцииЕГАИС = Ложь;
		Для Каждого ВидПродукции Из ВидыПродукцииЕГАИС Цикл
			Если ДанныеРазбора.ВидыУпаковокПоВидамПродукции[ВидПродукции].Количество() > 0 Тогда
				ЕстьВидПродукцииЕГАИС = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если Не ЕстьВидПродукцииЕГАИС Тогда
			ВыводитьСообщениеОбОшибке = Истина;
		КонецЕсли;
	КонецЕсли;
