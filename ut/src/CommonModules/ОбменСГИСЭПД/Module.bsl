...
Процедура ПриОпределенииДоступныхДействийПоЭДО(Действия, СостояниеДокумента, СвойстваДокумента) Экспорт
	...
	ТипДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СвойстваДокумента.ВидДокумента, "ТипДокумента");

	Если ЭтоДокументЭПД(ТипДокумента) Тогда
		Действия.Удалить(Перечисления.ДействияПоЭДО.Аннулировать);
		Действия.Удалить(Перечисления.ДействияПоЭДО.Перенаправить);
	КонецЕсли;
...
//  Приоритет - Число - приоритет, где 0 - самый высокий
Процедура ПриОпределенииПриоритетаРаспаковкиКонтейнера(КодТранзакции, ТипДокументаСтрокой, Приоритет) Экспорт
	
	Если КодТранзакции = "ShipperInformation"
		Или КодТранзакции = "ChartererInformation"
		Или КодТранзакции = "SendOrder"
		Или КодТранзакции = "OwnerVehicleInfo"
		Или КодТранзакции = "TransactionEDF_T1"
		Или КодТранзакции = "CarrierInformation_1" Тогда
...
// Добавить данные в состояния элементов регламента.
// 
// Параметры:
...
Процедура ДобавитьДанныеВСостоянияЭлементовРегламента(ПараметрыДокумента, СостоянияЭлементовРегламента) Экспорт 
	
	Если ТипЗнч(СостоянияЭлементовРегламента) <> Тип("ТаблицаЗначений")
		Или СостоянияЭлементовРегламента.Колонки.Найти("Ссылка") <> Неопределено Тогда   
		Возврат;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ЭлектронныйДокумент", ПараметрыДокумента.Ссылка);
	...
	Пока Выборка.Следующий() Цикл
		...
		Отбор.Вставить("ТипЭлементаРегламента", Выборка.ТипЭлементаРегламента); 
		Отбор.Вставить("Направление", Выборка.Направление); 
		Отбор.Вставить("Дата", Выборка.Дата);
		
		Строки = СостоянияЭлементовРегламента.НайтиСтроки(Отбор);
...
Функция ДвоичныеДанныеДокументаЭПД(Объект, Данные = Неопределено, АдресВоВременномХранилище = Неопределено) Экспорт
	...
	Если ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭТрН Тогда
		...
		МассивВозможныхПустыхЗначений.Добавить("//Файл/Документ/СодФХЖ1/СумНал/СумНДС");
...
// @skip-check function-return-types
// @skip-check doc-comment-ref-link
Функция ПриРаспознаванииДокумента(ТипДокумента, УспешностьРаспознования) Экспорт 
...
// Определяет использование универсальных сообщений
// 
// Параметры:
//  ПараметрыДокумента - См. РегламентыЭДО.НовыеПараметрыОпределенияИспользованияУС
//  ИспользоватьУниверсальныеСообщения - Булево
Процедура ПриОпределенииИспользованияУниверсальныхСообщений(ПараметрыДокумента, ИспользоватьУниверсальныеСообщения) Экспорт
	Если ЭтоТипРегламентаЭПД(ПараметрыДокумента.ТипРегламента) Тогда
		ИспользоватьУниверсальныеСообщения = Ложь;
	КонецЕсли;
КонецПроцедуры
		...
		Если ОтборПодписанта <> Неопределено Тогда
			Для Каждого КиЗ Из ОтборПодписанта Цикл
				...
				Если Результат.Свойство(КиЗ.Ключ, ЗначениеОтбора)
					И СтрСравнить(ЗначениеОтбора, КиЗ.Значение) <> 0 Тогда
...
Функция ПроверитьЗаполнениеДокументаЭПД(ДанныеДляПроверки, ТекущийТитул, ВидДокумента) Экспорт
	...
	ДеревоУзлов.Колонки.Добавить("УсловноОбязательный", Новый ОписаниеТипов("Булево"));
...
// Создает запись в реестре ЭПД.
//
// Параметры:
//  Данные								 - Структура - См. ОбменСГИСЭПДКлиентСервер.НоваяСтруктураДляРеестраЭПД.
//  ЗаписьИзФормы						 - Булево - Истина, если запись документа была вызвана из его формы.
//  СоответствиеЭлектронныхДокументов	 - Соответствие из ДокументСсылка - Соответствие электронных документов и документов учета.
//
Процедура ЗаписатьВРеестрЭПД(Данные, ЗаписьИзФормы = Ложь, СоответствиеЭлектронныхДокументов = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД.СервисВзаимодействияМПЭПД") Тогда
		МодульСервисВзаимодействияМПЭПД = ОбщегоНазначения.ОбщийМодуль("СервисВзаимодействияМПЭПД");
		НужноДобавитьДанныеМП = МодульСервисВзаимодействияМПЭПД.НужноДобавитьДанныеМП(
			МенеджерЗаписи, Данные);
	Иначе
		НужноДобавитьДанныеМП = Ложь;
	КонецЕсли;
	
	ЗаполнитьЗначенияСвойств(МенеджерЗаписи, Данные);
	
	Если НужноДобавитьДанныеМП Тогда
		Массив = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(МенеджерЗаписи);
		МодульСервисВзаимодействияМПЭПД.ДобавитьДанныеМобильногоПриложенияДляЗаписиВРеестр(
			Массив, ЗаписьИзФормы, СоответствиеЭлектронныхДокументов);
	КонецЕсли;
...
Процедура ДобавитьБаннер(Форма) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД.СервисВзаимодействияМПЭПД") Тогда	
		...
		ТекстБаннера = "<h1>Мобильное приложение 1С-ЭПД</h1>
...
//  СсылкаБаннера - Строка
// 
// Возвращаемое значение:
//  Строка - Текст Баннера HTML.
Функция ТекстБаннераHTML(ТекстБаннера, ЛеваяКартинка, ПраваяКартинка, СсылкаБаннера) Экспорт
	...
	|	</div>" + ?(ОбщегоНазначения.ЭтоВебКлиент() = Ложь, "
...
Функция ПредставлениеАдреса(ДанныеЗаполнения, ПрефиксРеквизитов) Экспорт
	...
	СтруктураРеквизитовАдреса.Вставить("Квартира", "");
	
	Для Каждого КиЗ Из СтруктураРеквизитовАдреса Цикл
		Если ДанныеЗаполнения.Свойство(ПрефиксРеквизитов + КиЗ.Ключ) Тогда
			СтруктураРеквизитовАдреса.Вставить(КиЗ.Ключ, ДанныеЗаполнения[ПрефиксРеквизитов + КиЗ.Ключ]);
		КонецЕсли;
	КонецЦикла;
	
	Если СтруктураРеквизитовАдреса.КодРегиона = "77"
		Или  СтруктураРеквизитовАдреса.КодРегиона = "78"
		Или  СтруктураРеквизитовАдреса.КодРегиона = "92" Тогда
		СтруктураРеквизитовАдреса.Город = "";
	КонецЕсли;
	
	Если ДанныеЗаполнения.Свойство(ПрефиксРеквизитов + "Представление") Тогда
		СтруктураРеквизитовАдреса.Вставить("Представление", ДанныеЗаполнения[ПрефиксРеквизитов + "Представление"]);	
		СтруктураРеквизитовАдреса.Вставить("КодСтраны", ДанныеЗаполнения[ПрефиксРеквизитов + "КодСтраны"]); 	
	КонецЕсли;
	
	Если СтруктураРеквизитовАдреса.Город = СтруктураРеквизитовАдреса.НаселенныйПункт Тогда
		 СтруктураРеквизитовАдреса.НаселенныйПункт = "";
	КонецЕсли;
		
	КонтактнаяИнформация = КонтактнаяИнформацияВXML(СтруктураРеквизитовАдреса,,ОбменСГИСЭПДПереопределяемый.ВидФактическийАдресКонтрагента());
	
	Результат =  ПредставлениеКонтактнойИнформации(КонтактнаяИнформация);

	КодСтраны = "";
	Если СтруктураРеквизитовАдреса.Свойство("КодСтраны", КодСтраны) 
		И ЗначениеЗаполнено(КодСтраны) И КодСтраны <> "643" Тогда
		
		Результат = "Код страны: " + КодСтраны + ", " + Результат; 
...
Процедура ЗаполнитьТаблицуЗначенийРеквизитовПоПараметрамФормы(Параметры, СтруктураРеквизитов) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(Параметры.Основание) Тогда
		...
		И ТипЗнч(Параметры.Ключ) = Тип("ДокументСсылка.ЭлектроннаяТранспортнаяНакладная") Тогда
			...
			Для Каждого ТитулКиЗ Из СтруктураРеквизитовОснования Цикл
				...
				Для Каждого РеквизитКиЗ Из ПоследняяВерсия Цикл
					...
					ИначеЕсли РеквизитКиЗ.Ключ = "СсылкаТитулГрузоотправителяГрузоотправитель"
						Или РеквизитКиЗ.Ключ = "СсылкаТитулГрузоотправителяПеревозчик" 
						Или РеквизитКиЗ.Ключ = "СсылкаТитулГрузоотправителяГрузополучатель"
						Или РеквизитКиЗ.Ключ = "СсылкаТитулГрузоотправителяВалюта" Тогда 	
					...
					ИначеЕсли СтрНачинаетсяС(РеквизитКиЗ.Ключ, "ТитулГрузоотправителяГрузоотправитель")
						...
						Или СтрНачинаетсяС(РеквизитКиЗ.Ключ, "ТитулГрузоотправителяГрузополучатель")
						...
						Или РеквизитКиЗ.Ключ = "ТитулГрузоотправителяНижнийПределВлажностиПриПеревозке"
						Или РеквизитКиЗ.Ключ = "ТитулГрузоотправителяСтоимостьГруза" 
						Или РеквизитКиЗ.Ключ = "ТитулГрузоотправителяКодВалюты"
						Или РеквизитКиЗ.Ключ = "ТитулГрузоотправителяНаименованиеВалюты" Тогда
						...
						Если ОбластьНайдено <> Неопределено Тогда
							СтруктураРеквизитовТитулаЗаполнение.Вставить(РеквизитКиЗ.Ключ, РеквизитКиЗ.Значение);
						Иначе
							Если ИмяРеквизита = "ТитулГрузоотправителяГрузы.СсылкаВалютаСтоимости" Тогда
								СтруктураРеквизитовТитулаЗаполнение.Вставить(РеквизитКиЗ.Ключ, РеквизитКиЗ.Значение);								
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
			...
			ВидТары = Неопределено;
			ИтераторЦикла = 1;
			Пока ПоследняяВерсия.Свойство("ТитулГрузоотправителяГрузы__" + ИтераторЦикла + "__ВидТары", ВидТары) Цикл
				СтруктураРеквизитовТитулаЗаполнение.Вставить("ТитулГрузоотправителяГрузы__" + ИтераторЦикла + "__СпособУпаковки", ВидТары);
			...
			СтруктураРеквизитовТитулаЗаполнение.Свойство("ТитулГрузоотправителяГрузоотправительНомераТелефонов__1__Телефон", Телефон);
...
Процедура ЗаполнитьДополнительныеРеквизитыФормы(Форма, ПрефиксТитула, КонтейнерОтображаемойВерсии = Неопределено) Экспорт
	...
	Для Каждого ОписаниеГруппыДопРеквизитов Из Форма.ОписаниеРеквизитовФормы.ОписаниеДопРеквизитов Цикл
		Если СтрНачинаетсяС(ПрефиксТитула, ОписаниеГруппыДопРеквизитов.Ключ) Тогда
...
Функция НайтиСоздатьЭПД(ДеревоДляСовместимостиСБЭД, ДеревоРазбора, ПараметрыОбработки, НайденныйОбъект)
	...
	ДокументОбъект.ДополнительныеСвойства.Вставить("ЭлектронныйДокумент", ПараметрыОбработки.ЭлектронныйДокумент);
...
Функция ИнициализироватьУзелДереваРекурсивно(Знач МассивИменУзлов, Знач ИмяРеквизита, Знач НомерРекурсии, Знач Заполнено, 
		...
		Если СтруктураУзла <> Неопределено Тогда
			...
			Иначе
				СтрокаУзла.Обязательный = ПроверитьУсловие(СтруктураУзла.ПоляУсловий, 
											...
											ДанныеФормирования);
				СтрокаУзла.УсловноОбязательный = Истина;
			КонецЕсли;
	...
	Иначе
		...
		Если ТипЗнч(Родитель) <> Тип("ДеревоЗначений") Тогда
			...
			Если СтрокаУзла.УсловноОбязательный И СтрокаУзла.Обязательный Тогда
				Родитель.Обязательный = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура СоздатьДеревоСоответствийИОформитьКнопки(ИмяФормы,
	...
	Если ПустаяСтрока(Префикс) Тогда
		...
		ИначеЕсли ИмяФормы = "Документ.ЭлектронныйЗаказЗаявка.Форма.Прицеп" Тогда
			Префикс = "ТитулПеревозчика";
		Иначе
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Функция ОбязательныеЭлементыЗаполнены(ОбъектСДаннымиДокумента,
	...
	ЕстьПроверкаОбязательностиПодчиненныхЭлементов = Ложь;
	...
	Для Каждого СтрокаДерева Из ВеткаДерева.Строки Цикл
		...
		Если СтрокаДерева.ЭтоОбязательнаяСтруктура Тогда
			...
			Если ЕстьПроверкаОбязательности(СтрокаДерева.СтруктураПроверкиОбязательности) Тогда
				...
				ЕстьПроверкаОбязательностиПодчиненныхЭлементов = Истина;
	...
	Если Не РезультатТекущейВетки.ЕстьЗаполненныеЭлементы
		И ЕстьПроверкаОбязательностиПодчиненныхЭлементов 
		И ЕстьОбязательныеНеЗаполненныеЭлементы Тогда
			
		РезультатТекущейВетки.ОбязательныеЭлементыЗаполнены = Ложь;
	
	ИначеЕсли Не РезультатТекущейВетки.ЕстьЗаполненныеЭлементы
