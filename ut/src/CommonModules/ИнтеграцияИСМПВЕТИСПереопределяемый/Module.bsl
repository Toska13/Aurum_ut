...
Процедура ОбработкаЗаполненияМаркировкиТоваровИСМП(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ДокументОбъект.ДополнительныеСвойства.Свойство("ОтчетПроизводственнойЛинии") Тогда
		...
		|		КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СгруппированныеТоварыКОформлению.Номенклатура) > 1
		|			ТОГДА Неопределено
		|		ИНАЧЕ МАКСИМУМ(СгруппированныеТоварыКОформлению.ВидПродукции)
		...
		Если Не ЗначениеЗаполнено(ДокументОбъект.ТоварнаяГруппа)
			И ЗначениеЗаполнено(ДанныеНоменклатуры.ВидПродукции) Тогда
			ТоварныеГруппы = КонструкторИСОбщегоНазначенияКлиентСервер.ЗначенияЭлементаКонструктораПоВидуПродукции(
				Справочники.ЭлементыКонструктораВидовПродукцииИС.ТоварныеГруппыГИСМТ,
				ДанныеНоменклатуры.ВидПродукции);
