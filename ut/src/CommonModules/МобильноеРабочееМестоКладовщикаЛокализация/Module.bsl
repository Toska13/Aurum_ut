...
Функция ОбработатьВводШтрихкода(ДокументСсылка, ШтрихкодКоличество, КэшированныеЗначения, ПараметрыСканирования = Неопределено) Экспорт
	...
	ПараметрыСканирования.ДопустимыеВидыПродукции = Новый Массив(ОбщегоНазначенияИСМПКлиентСерверПовтИсп.УчитываемыеВидыМаркируемойПродукции());
	
	ИмяМетаданныхОбъекта = ДокументСсылка.Метаданные().Имя;
	Менеджер = Документы[ИмяМетаданныхОбъекта];
	
	ПараметрыУказанияСерий = НоменклатураСервер.ПараметрыУказанияСерий(ДокументСсылка, Менеджер);
	Если ИмяМетаданныхОбъекта = "ОтборРазмещениеТоваров" Тогда
		ПараметрыУказанияСерий = ПараметрыУказанияСерий.Отбор;
	ИначеЕсли ИмяМетаданныхОбъекта = "РасходныйОрдерНаТовары" Тогда
		ПараметрыУказанияСерий = ПараметрыУказанияСерий.ОтгружаемыеТовары;
	КонецЕсли;
