...
Функция ПодсистемаНеИспользуется() Экспорт
	...
	Для Каждого ВидПродукции Из КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииМЕХ() Цикл
		ДатаНачалаЭксплуатацииИСМП = Модуль.ДатаОбязательнойМаркировкиПродукции(ВидПродукции);
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(ДатаНачалаЭксплуатацииИСМП) Тогда
		Для Каждого ВидПродукции Из КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииМЕХ() Цикл
			ДатаНачала = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДатаНачалаПроизводства(ВидПродукции);
			Если ЗначениеЗаполнено(ДатаНачала) Тогда
				ДатаНачалаЭксплуатацииИСМП = ДатаНачала;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ДатаНачалаЭксплуатацииИСМП) Тогда
		ДатаНачалаЭксплуатацииИСМП = '00010101';
	КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВыполнитьОбменРегламентноеЗадание");
...
Функция ОтправитьДляОбработки(ТелоЗапроса, АдресСервера, Порт, АдресРесурса, SOAPAction) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'ГИСМ'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка отправки POST-запроса к серверу ГИСМ'") + Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Функция ОбработатьОчередьПолученияДанныхРегламентнымЗаданием(GLN, СертификатыДляПодписанияНаСервере)
	...
	РезультатПодписания = Подписать(
		...
		СтрокаСертификата.СертификатКриптографии, МенеджерКриптографии);
