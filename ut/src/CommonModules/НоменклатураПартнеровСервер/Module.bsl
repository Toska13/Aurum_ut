...
Процедура ПриИзмененииНоменклатурыСопоставления_НоменклатураКонтрагентов(Форма, ДанныеСопоставления) Экспорт
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ДанныеСопоставления, СтруктураДействий, Неопределено);
...
Процедура УстановитьПараметрыДинамическогоСписка_ФормаВыбораНоменклатурыКонтрагентов(Параметры, Список) Экспорт
	...
	Если Параметры.Свойство("Отбор") Тогда
		...
		Если Параметры.Отбор.Свойство("ТипНоменклатуры") Тогда
			...
			ИначеЕсли ТипЗнч(Параметры.Отбор.ТипНоменклатуры) = Тип("ФиксированныйМассив")
						ИЛИ ТипЗнч(Параметры.Отбор.ТипНоменклатуры) = Тип("Массив") Тогда
...
//  Владелец - ОпределяемыйТип.ВладелецНоменклатурыБЭД - владелец номенклатуры партнера.
//
// Возвращаемое значение:
//  СправочникСсылка.Номенклатура - ссылка номенклатуры.
//
Функция НайтиСсылкуНаНоменклатуруПоИдентификаторуНоменклатурыПартнера(Идентификатор, Владелец) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
               ...
               |	И СпрНоменклатура.Владелец = &Владелец";
...
//
Функция НайтиСсылкуНаНоменклатуруПартнераПоИдентификатору(Идентификатор, Владелец) Экспорт
...
Функция ВыборкаДляСопоставленияНоменклатуры(СсылкаНаОбъект) Экспорт
	...
	ИначеЕсли ТипЗнч(СсылкаНаОбъект) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТЧ", "Документ." + НаименованиеДокумента + ".ЗаменяющиеТовары");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Упаковка", "Товары.Номенклатура.ЕдиницаИзмерения");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Идентификатор", """""");
...
Процедура ДополнитьТекстЗапросаЗаполнитьНоменклатуруПартнераПоНоменклатуре(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		...
		СписокЗапросов = ТекстыЗапросовЗаполнитьНоменклатуруПартнераПакетнаяОбработка(КэшированныеЗначения);
		...
		// Вместо ссылки "Истина", если партнер в таблице
		ОписаниеЗапроса.ПараметрыЗапроса.Вставить("ВладелецНоменклатурыПартнер", ПараметрыДействия); 
...
Процедура ДополнитьТекстЗапросаПроверитьСопоставленнуюНоменклатуруПартнера(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНоменклатуруПартнеров")
		...
		ОписаниеЗапроса.ПараметрыЗапроса.Вставить("ВладелецНоменклатурыПартнер", ПараметрыДействия.Партнер);
...
Функция СформироватьТаблицуТоваровДляСопоставленияНоменклатурыПартнеров(Таблица)
	...
	|		ИНАЧЕ ВЫРАЗИТЬ(Товары.Упаковка КАК Справочник.УпаковкиЕдиницыИзмерения).Наименование
	...
	|		ПО (ВременнаяТаблица_Товары.ЭтоТоварДругогоКачества)
	|		И (НЕ ВременнаяТаблица_Товары.БазоваяУпаковка)
	|		И ВременнаяТаблица_Товары.НаименованиеУпаковки = УпаковкиЕдиницыИзмерения.Наименование
	|		И (ЕСТЬNULL(&ТекстЗапросаКоэффициентаУпаковки, 1) = ЕСТЬNULL(&ПоискТекстЗапросаКоэффициентаУпаковки, 1))
	|		И ВременнаяТаблица_Товары.Номенклатура = УпаковкиЕдиницыИзмерения.Владелец
	|СГРУППИРОВАТЬ ПО
	|	ВременнаяТаблица_Товары.НомерСтроки,
...
//
Функция ТекстыЗапросовЗаполнитьНоменклатуруПартнераПакетнаяОбработка(КэшированныеЗначения)
	...
	|	ВтИсточникДанных.НоменклатураПартнера                                                        КАК НоменклатураПартнераИсходная,
	|	ВтИсточникДанных.Номенклатура                                                                КАК НоменклатураИсходная,
	|	ВтИсточникДанных.Номенклатура.ЕдиницаИзмерения                                               КАК ЕдиницаИзмеренияИсходная,
	|	ВтИсточникДанных.Характеристика                                                              КАК ХарактеристикаИсходная,
	|   
	|	ТоварыДругогоКачества.Номенклатура ЕСТЬ НЕ NULL                                              КАК ЕстьНоменклатураДругогоКачества,
	|	ЕСТЬNULL(ТоварыДругогоКачества.Номенклатура, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)) КАК НоменклатураДругогоКачества,
	|
	|	УпаковкиЕдиницыИзмерения.Ссылка                                                              КАК УпаковкаИсходная,
	|	УпаковкиЕдиницыИзмерения.Наименование                                                        КАК УпаковкаИсходнаяНаименование,
	|	УпаковкиЕдиницыИзмерения.Ссылка ЕСТЬ НЕ NULL                                                 КАК УпаковкаИсходнаяЗаполнена,
	|	&УпаковкаИсходнаяКоэффициент                                                                 КАК УпаковкаИсходнаяКоэффициент
	|
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК УпаковкиЕдиницыИзмерения
	|		ПО ВтИсточникДанных.Упаковка = УпаковкиЕдиницыИзмерения.Ссылка";
	
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("Партнер", КэшированныеЗначения) Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПолеПартнер", "ВтИсточникДанных.Партнер"); // @query-part-2
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПолеПартнер", "&ВладелецНоменклатурыПартнер"); // @query-part-2
	КонецЕсли;
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&УпаковкаИсходнаяКоэффициент", 
		Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки(
		"УпаковкиЕдиницыИзмерения",
		"ВтИсточникДанных.Номенклатура"));
		
	Результат.Добавить(ТекстЗапроса, "ПараметрыНоменклатуры");
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("Партнер", КэшированныеЗначения) Тогда 
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			"&Партнер",
			"ВтИсточникДанных.Партнер");
				
	КонецЕсли;
		
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ РАЗРЕШЕННЫЕ
	|	ПараметрыНоменклатуры.ИдентификаторСтрокиВТ                                                                  КАК ИдентификаторСтрокиВТ,
	|	ПараметрыНоменклатуры.Партнер                                                                                КАК Партнер,
	|	ПараметрыНоменклатуры.НоменклатураПартнераИсходная                                                           КАК НоменклатураПартнераИсходная,
	|	ПараметрыНоменклатуры.НоменклатураИсходная                                                                   КАК Номенклатура,
	|	ПараметрыНоменклатуры.ХарактеристикаИсходная                                                                 КАК Характеристика,
	|	ВЫБОР
	|		КОГДА ПараметрыНоменклатуры.УпаковкаИсходнаяЗаполнена
	|			ТОГДА ПараметрыНоменклатуры.УпаковкаИсходная
	|		ИНАЧЕ ПараметрыНоменклатуры.ЕдиницаИзмеренияИсходная
	|	КОНЕЦ                                                                                                        КАК Упаковка,
	|	ЛОЖЬ                                                                                                         КАК ЭтоТоварДругогоКачества
	|ПОМЕСТИТЬ ПараметрыПоискаНоменклатурыКонтрагентов
	|ИЗ
	|	ПараметрыНоменклатуры КАК ПараметрыНоменклатуры
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ПараметрыНоменклатуры.Партнер                                                                                КАК Партнер,
	|	ПараметрыНоменклатуры.НоменклатураПартнераИсходная                                                           КАК НоменклатураПартнераИсходная,
	|	ПараметрыНоменклатуры.НоменклатураДругогоКачества                                                            КАК Номенклатура,
	|	ПараметрыНоменклатуры.ХарактеристикаИсходная                                                                 КАК Характеристика,
	|	ВЫБОР
	|		КОГДА ПараметрыНоменклатуры.УпаковкаИсходнаяЗаполнена
	|			ТОГДА ЕСТЬNULL(УпаковкиНоменклатурыДругогоКачества.Ссылка, ПараметрыНоменклатуры.УпаковкаИсходная)
	|		ИНАЧЕ ПараметрыНоменклатуры.НоменклатураДругогоКачества.ЕдиницаИзмерения
	|	КОНЕЦ                                                                                                        КАК Упаковка,
	|	ИСТИНА                                                                                                       КАК ЭтоТоварДругогоКачества
	|ИЗ
	|	ПараметрыНоменклатуры КАК ПараметрыНоменклатуры
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК УпаковкиНоменклатурыДругогоКачества
	|		ПО ПараметрыНоменклатуры.НоменклатураДругогоКачества = УпаковкиНоменклатурыДругогоКачества.Владелец
	|			И ПараметрыНоменклатуры.УпаковкаИсходнаяНаименование = УпаковкиНоменклатурыДругогоКачества.Наименование
	|			И (ПараметрыНоменклатуры.УпаковкаИсходнаяЗаполнена)
	|			И (ПараметрыНоменклатуры.УпаковкаИсходнаяКоэффициент = &УпаковкаДругогоКачестваКоэффициент)
	|ГДЕ
	|	ПараметрыНоменклатуры.ЕстьНоменклатураДругогоКачества";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&УпаковкаДругогоКачестваКоэффициент", 
		Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки(
		"УпаковкиНоменклатурыДругогоКачества",
		"ПараметрыНоменклатуры.НоменклатураДругогоКачества"));
	
	Результат.Добавить(ТекстЗапроса, "ПараметрыПоискаНоменклатурыКонтрагентов");
	
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПараметрыПоискаНоменклатурыПартнеров.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|	// Если подобралась исходная номенклатура, то выбираем ее. Если нет, то получаем номенклатуру контрагента с приоритетом по товару  другого качества:
	|	ЕСТЬNULL(ЕСТЬNULL(МАКСИМУМ(ВЫБОР
	|					КОГДА НоменклатураКонтрагентов.Ссылка = ПараметрыПоискаНоменклатурыПартнеров.НоменклатураПартнераИсходная
	|						ТОГДА НоменклатураКонтрагентов.Ссылка
	|					ИНАЧЕ NULL
	|				КОНЕЦ), МАКСИМУМ(ВЫБОР
	|					КОГДА ПараметрыПоискаНоменклатурыПартнеров.ЭтоТоварДругогоКачества
	|						ТОГДА НоменклатураКонтрагентов.Ссылка
	|					ИНАЧЕ NULL
	|				КОНЕЦ)), ЕСТЬNULL(МАКСИМУМ(ВЫБОР
	|					КОГДА ПараметрыПоискаНоменклатурыПартнеров.ЭтоТоварДругогоКачества
	|						ТОГДА NULL
	|					ИНАЧЕ НоменклатураКонтрагентов.Ссылка
	|				КОНЕЦ), ЗНАЧЕНИЕ(Справочник.НоменклатураКонтрагентов.ПустаяСсылка))) КАК НоменклатураПартнера
	|
	|ИЗ
	|	ПараметрыПоискаНоменклатурыКонтрагентов КАК ПараметрыПоискаНоменклатурыПартнеров
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НоменклатураКонтрагентов КАК НоменклатураКонтрагентов
	|		ПО ПараметрыПоискаНоменклатурыПартнеров.Партнер = НоменклатураКонтрагентов.Владелец 
	|			И ПараметрыПоискаНоменклатурыПартнеров.Номенклатура = НоменклатураКонтрагентов.Номенклатура
	|			И ПараметрыПоискаНоменклатурыПартнеров.Характеристика = НоменклатураКонтрагентов.Характеристика
	|			И ПараметрыПоискаНоменклатурыПартнеров.Упаковка = НоменклатураКонтрагентов.Упаковка
	|			И НЕ НоменклатураКонтрагентов.ПометкаУдаления
	|			И НЕ НоменклатураКонтрагентов.Недействителен
	|
	|СГРУППИРОВАТЬ ПО
	|	ПараметрыПоискаНоменклатурыПартнеров.ИдентификаторСтрокиВТ";
...
Функция ТекстыЗапросовЗаполнитьНоменклатуруПоНоменклатуреПартнера()
	...
	|	КОНЕЦ КАК ХарактеристикиИспользуются,
	|	ВЫБОР
	|		КОГДА НоменклатураПартнеров.Номенклатура ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	...
	|	КОНЕЦ КАК НеобходимоПерезаполнитьНоменклатуру,
	|	ВЫБОР
	|		КОГДА НоменклатураПартнеров.Характеристика ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	...
	|	НоменклатураПартнера.НеобходимоПерезаполнитьХарактеристики КАК НеобходимоПерезаполнитьХарактеристики,
	|	ВЫБОР
	|		КОГДА НоменклатураПартнера.Упаковка ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		КОГДА НоменклатураПартнера.Упаковка.Владелец <> ЗНАЧЕНИЕ(Справочник.НаборыУпаковок.БазовыеЕдиницыИзмерения)
	|			ТОГДА ВЫБОР
	|					КОГДА НоменклатураПартнера.ВДокументеНоменклатураДругогоКачества
...
Функция ТекстыЗапросовПроверитьСопоставленнуюНоменклатуруПартнера()
	...
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ) КАК КоличествоНоменклатурыПартнера
	|ИЗ
	|	ВтИсточникДанных КАК ВтИсточникДанных
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НоменклатураКонтрагентов КАК НоменклатураКонтрагентов
	|		ПО (НоменклатураКонтрагентов.Владелец = &ВладелецНоменклатурыПартнер)
	|		И (НЕ НоменклатураКонтрагентов.ПометкаУдаления)
