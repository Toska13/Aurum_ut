...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВключитьИспользованиеРеестраДоверенностейФНСЭДО");
	Методы.Вставить("ОтключитьРежимСтрогойПроверкиДоверенностей");
	
	Методы.Вставить("ОбновитьСтатусМЧДИзРеестраФНС", Истина);
	Методы.Вставить("ДанныеДляПодписанияДляКлиента", Истина);
	Методы.Вставить("ЗарегистрироватьВРеестреФНС", Истина);
	Методы.Вставить("ПодписатьЗарегистрироватьМЧД", Истина);
	Методы.Вставить("ПодписатьОтменитьМЧД", Истина);
	Методы.Вставить("ПроверитьПодписиМЧД", Истина);
	Методы.Вставить("УстановитьПометкуВернаПоМестуОбработкиЭДО", Истина);
	Методы.Вставить("УстановитьПометкуОтозванаПоМестуОбработкиЭДО", Истина);
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Если ИспользоватьУпрощенныйРежимРаботыСДоверенностями() Тогда
		ТекстЗапроса = ТекстЗапросаДляРегистрацииДанныхВУпрощенномРежимРаботыСДоверенностями();
	Иначе
		ТекстЗапроса = ТекстЗапросаДляРегистрацииДанныхКОбработке();	
	КонецЕсли;
	...
	Пока Не ОтработаныВсеДанныеМЧД003 
		...
		Запрос.УстановитьПараметр("МЧД003Ссылка", МЧД003);
		Запрос.УстановитьПараметр("МЧДОрганизацийСсылка", МЧДОрганизаций);
		Запрос.УстановитьПараметр("МЧДКонтрагентовСсылка", МЧДКонтрагентов);
		
		РезультатыЗапроса = Запрос.ВыполнитьПакет();
		
		ДанныеМЧД003 = РезультатыЗапроса[0].Выгрузить();
		ДанныеМЧДОрганизаций = РезультатыЗапроса[1].Выгрузить();
		ДанныеМЧДКонтрагентов = РезультатыЗапроса[2].Выгрузить();
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Для Каждого СтрокаДанных Из ВыбранныеДанные Цикл
		...
		Попытка
			...
			Если Объект <> Неопределено Тогда
				...
				ВыполнитьПроверкуПодписиДоверенности(Объект, Записать);
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось обработать машиночитаемую доверенность: %1 по причине:
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ШаблонСообщения = НСтр("ru = 'Не удалось обработать некоторые машиночитаемые доверенности (пропущены): %1'");
		ТекстСообщения = СтрШаблон(ШаблонСообщения, ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ШаблонСообщения = НСтр("ru = 'Обработана очередная порция машиночитаемых довереностей: %1'");
...
//  Параметры - см. ОбновлениеИнформационнойБазы.ОсновныеПараметрыОтметкиКОбработке
//
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию_КонвертацияПравилПроверки(Параметры) Экспорт
...
// Возвращает значение настройки использования режима строгой проверки доверенности
//
// Возвращаемое значение:
// Булево
...
// Возвращаемое значение:
//  Строка
Функция ПредставлениеМЧДНеПодходит() Экспорт
...
//  * Отозвана - Булево
//  * ДатаОтзыва - Дата
//  * ПротоколПроверкиМЧД - ХранилищеЗначения - см. КриптографияБЭДКлиентСервер.НовыйРезультатПроверкиПодписи
...
Функция НовыеСведенияМЧД() Экспорт
	...
	Сведения.Вставить("ПротоколПроверкиМЧД", Новый ХранилищеЗначения(Неопределено, Новый СжатиеДанных(9)));
...
Процедура ОтразитьРезультатПроверкиМЧД(МЧД, Результат, ТекстОшибки, ОписаниеОшибкиРегистрации = "", 
	...
	Попытка
		...
		Если Не ПроверенаВручную Или СнятьОтметкуРучнойПроверки Тогда
			...
			ВариантПроверкиСертификатаДоверителя = ВариантПроверкиСертификатаДоверителя();
			Если РезультатПроверкиПодписиМЧД <> Неопределено 
				И ВариантПроверкиСертификатаДоверителя = 
					Перечисления.ВариантыПроверкиСертификатовМЧДЭДО.НеПроверятьПоСрокуДействия Тогда
					
				ПротоколПроверкиПодписиМЧД = РезультатПроверкиПодписиМЧД.СвойстваПодписи.Протокол;
				Если ПротоколПроверкиПодписиМЧД <> Неопределено
					И МашиночитаемыеДоверенностиКлиентСервер.СертификатОтозванИлиИстек(ПротоколПроверкиПодписиМЧД)
					И ПротоколПроверкиПодписиМЧД.Контекст.РезультатПроверкиСредствамиКриптографии = Истина Тогда
					МЧДОбъект.Верна = Истина;
					МЧДОбъект.ТекстОшибкиПроверкиМЧД = "";
				КонецЕсли;
...
Процедура ЗаполнитьПодписанаВерна(ОбъектМЧД, ДанныеДляПроверки, ТребуетсяПроверкаМЧДНаКлиенте = Ложь,
	...
	ИначеЕсли ЭлектроннаяПодпись.ПроверятьЭлектронныеПодписиНаСервере() Тогда
		...
		Если РезультатПроверкиДоверенности.Свойство("РезультатПроверкиПодписиМЧД") Тогда
			РезультатПроверкиНаСервере.РезультатПроверкиПодписиМЧД = РезультатПроверкиДоверенности.РезультатПроверкиПодписиМЧД;
		КонецЕсли;
	Иначе
...
Функция ПроверенаВручную(Доверенность) Экспорт
	...
	СвойстваДоверенностиПроверка = НовыйСвойстваДоверенностиДляОпределенияРучнойПроверкиДоверенности();
	ЗаполнитьЗначенияСвойств(СвойстваДоверенностиПроверка, СвойстваДоверенности);
	
	ПроверенаВручную = ОпределитьРучнуюПроверкуПоСвойствамДоверенности(СвойстваДоверенностиПроверка);
...
Функция ПроверитьПодписиМЧД() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		КоличествоОбработанных = КоличествоОбработанных + 1;
		СообщитьПрогрессВыполненияПоКоличествуОбъектов(КоличествоОбработанных, Выборка.Количество());
...
#Область Справочники_МашиночитаемыеДоверенности_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "МашиночитаемыеДоверенности.ОбработатьДанныеДляПереходаНаНовуюВерсию_КонвертацияПравилПроверки";
	Обработчик.Версия = "1.9.16.3";
	Обработчик.РежимВыполнения = "Отложенно";
...
#Область МашиночитаемыеДоверенности_ОтключитьРежимСтрогойПроверкиДоверенностей
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = 
		"Константы.РежимСтрогойПроверкиДоверенностейОтключенЭДО.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.НачальноеЗаполнение = Истина;
...
#Область МашиночитаемыеДоверенности_УстановитьВариантПроверкиСертификатовНеПроверятьПоСрокуДействия

	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = 
		"Константы.ВариантПроверкиСертификатовМЧДЭДО.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "1.9.16.46";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("e2a8387f-6c91-4236-b70b-8ca48e39791a");
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "ЭлектронноеВзаимодействие.ЗаполнениеДанныхОбновления";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.ЗапускатьИВПодчиненномУзлеРИБСФильтрами = Истина;
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: Применение варианта проверки сертификатов доверителей'");
	Обработчик.ЧитаемыеОбъекты = "Константа.ВариантПроверкиСертификатовМЧДЭДО";
	Обработчик.ИзменяемыеОбъекты = "Константа.ВариантПроверкиСертификатовМЧДЭДО";
	Обработчик.БлокируемыеОбъекты = "Константа.ВариантПроверкиСертификатовМЧДЭДО";
...
Функция ЗагрузитьМЧДИзФайла(АдресХранилища, ТребуетсяПроверкаМЧДНаКлиенте = Ложь, ОбновлятьСуществующий = Ложь,
	...
	Результат.Вставить("ТекстОшибки", "");
	Результат.Вставить("ВариантПроверкиСертификатовМЧДЭДО", ВариантПроверкиСертификатаДоверителя());
...
Функция ИмяПодсистемы() Экспорт
	...
	Возврат ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().МашиночитаемыеДоверенности;
	...
	Если ЗначениеЗаполнено(СведенияДоверенности.ПолныеДанные) Тогда
		...
		Если ЗначениеЗаполнено(СведенияМЧД.НомерРодительскойДоверенности)
			...
			СведенияМЧД.РодительскиеДанныеМЧД = ПолучитьРодительскиеДанныеДоверенности(
				СведенияМЧД.НомерРодительскойДоверенности, СведенияМЧД.ИННДоверителяРодительскойДоверенности, СведенияМЧД.ИННДоверителя, НомераМЧД);
		КонецЕсли;
...
Функция СведенияОСтатусахДоверенностей(СправочникМЧД, ДанныеМЧД)
	...
	ОписаниеЗамера = ОбщегоНазначенияБЭД.НачатьЗамер("ПроверкаМЧДВРРМЧД");
	УстановитьПривилегированныйРежим(Истина);
	Результат = Новый Соответствие;
	...
	КонецЦикла;
	ОбщегоНазначенияБЭД.ЗакончитьЗамер("ПроверкаМЧДВРРМЧД", ОписаниеЗамера, ДанныеМЧД.Количество());
	Возврат Результат;
...
Функция АвторизоватьсяНаСервереМЧД(ПоказыватьСлужебныеСообщения = Истина) Экспорт
	...
	РезультатПодключения = ПроверитьПодключениеКСерверуМЧД();
	
	Если Не РезультатПодключения.СервисАвторизацииДоступен Тогда
		Если ПоказыватьСлужебныеСообщения Тогда
			ТекстОшибкиПоУмолчанию =
				НСтр("ru = 'Сервис авторизации на сервере МЧД распределенного реестра временно недоступен.'"); 
			СтруктураОтвета = Новый Структура("ТекстОшибкиПоУмолчанию", ТекстОшибкиПоУмолчанию);
			ВывестиИЗаписатьОшибкуМЧД(СтруктураОтвета);
		КонецЕсли;
		Возврат Результат;
	КонецЕсли;
...
Функция РезультатПроверкиДоверенности(ДанныеДоверенности, РезультатПроверкиПодписи, КонтекстДиагностики) Экспорт
	...
	РезультатПроверки = НовыйОбщийРезультатПроверкиПодписиДоверенности();
	РезультатПроверки.РезультатПроверкиПодписиМЧД = РезультатПроверкиПодписи;
	РезультатПроверки.КонтекстДиагностики = КонтекстДиагностики;
...
Функция НовыйОбщийРезультатПроверкиПодписиДоверенности(Знач РезультатПоУмолчанию = Ложь) Экспорт
	
	Результат = Новый Структура();
	Результат.Вставить("Результат", РезультатПоУмолчанию);
	Результат.Вставить("РезультатПроверкиПодписиМЧД", Неопределено);
	Результат.Вставить("ТекстОшибки", "");
...
Функция НачатьПроверкуПодписейДокументовПоМЧД(ПараметрыПроверки) Экспорт 
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, 
		"МашиночитаемыеДоверенности.ВыполнитьПроверкуПодписиДокументовПоМЧД", ПараметрыПроверки);
...
Функция ВыполнитьПроверкуПодписиДокументовПоМЧД(Параметры) Экспорт 
	...
	Пока ВыборкаЭлектронныйДокумент.Следующий() Цикл
		...
		СообщитьПрогрессВыполненияПоКоличествуОбъектов(КоличествоОбработанныхДокументов, КоличествоОбъектовКОбработке);
...
// Сообщает прогресс выполнения длительной операции в зависимости от количества объектов к обработке и 
// обработанных объектов.
// 
// Параметры:
//  КоличествоОбработанныхОбъектов - Число
//  КоличествоОбъектовКОбработке - Число
//  ИнтервалОбновленияПрогресса - Число
// 
Процедура СообщитьПрогрессВыполненияПоКоличествуОбъектов(КоличествоОбработанныхОбъектов, КоличествоОбъектовКОбработке, 
	ИнтервалОбновленияПрогресса = 5)
	
	Если ИнтервалОбновленияПрогресса <= 0 Тогда
		ИнтервалОбновленияПрогресса = 5;
	КонецЕсли;
	
	// Сообщать о прогрессе пропуская количество объектов, указанное в ИнтервалОбновленияПрогресса.
	Если КоличествоОбработанныхОбъектов = 0 
		Или КоличествоОбработанныхОбъектов / ИнтервалОбновленияПрогресса <> Цел(КоличествоОбработанныхОбъектов / ИнтервалОбновленияПрогресса) Тогда
		Возврат;
	КонецЕсли;

	ПроцентВыполнения = 
		МашиночитаемыеДоверенностиКлиентСервер.РассчитатьПроцентОбработанныхОбъектов(
			КоличествоОбработанныхОбъектов, КоличествоОбъектовКОбработке);
...
// Выполняет проверку доступности сервера распределенного реестра МЧД
// Параметры:
//  НастройкиПодключения - см. НовыйНастройкиПодключенияКСерверуМЧД 
//
// Возвращаемое значение:
//  Булево
Функция СервисАвторизацииСервераМЧДДоступен(НастройкиПодключения)
	
	ШаблонТекста = НСтр("ru = 'Начало проверки связи с сервером МЧД распределенного реестра по адресу %1.'");
	Операция = СтрШаблон(ШаблонТекста, НастройкиПодключения.АдресСервераБезАутентификации);
	ЗаписьЖурналаРегистрации(ИмяПодсистемы(), УровеньЖурналаРегистрации.Информация,,, Операция);
	
	РесурсНаСервере = НастройкиПодключения.РесурсКорняAPI + "/token";
	...
	ЗапросHTTP = Новый HTTPЗапрос(РесурсНаСервере + ПараметрыРесурсаНаСервере, ЗаголовкиHTTP);
	ОписаниеСоединения =
		ИнтернетСоединениеБЭД.ОписаниеHTTPСоединения(НастройкиПодключения.АдресСервераБезАутентификации, 15);
	Результат = ИнтернетСоединениеБЭД.ВызватьHTTPМетод(ОписаниеСоединения, ЗапросHTTP,
		ИнтернетСоединениеБЭД.HTTPМетоды().GET, Операция);
...
// Определяет текущий индекс строки в массиве с настройками для подключения к серверу распределенного реестра МЧД
// 
// Параметры:
//  СписокНастроек - Массив из см. НовыйНастройкиПодключенияКСерверуМЧД
// 
// Возвращаемое значение:
//  Число
Функция ОпределитьКлючНастройкиДляПодключения(СписокНастроек)
	
	Если СписокНастроек.Количество() = 1 Тогда
		Индекс = 0;
	Иначе
		
		Граница = СписокНастроек.Количество() - 1;
...
// Возвращает результат проверки доступности сервиса авторизации сервера распределенного реестра МЧД
// и настройки подключения к нему.
// Если сервис авторизации доступен, сохраняет в хранилище общих настроек настройки подключения к нему. 
// 
// Возвращаемое значение:
//  Структура:
// * НастройкиПодключения - Неопределено 
// * СервисАвторизацииДоступен - Булево
Функция ПроверитьПодключениеКСерверуМЧД()
	
	Результат = НовыйРезультатПодключенияКСервисуАвторизации();
	
	СписокНастроек = НастройкиПодключенияКСерверуМЧД();
	
	Пока СписокНастроек.Количество() > 0 Цикл
		
		КлючНастройки = ОпределитьКлючНастройкиДляПодключения(СписокНастроек);
		ТекущиеНастройки = СписокНастроек.Получить(КлючНастройки);
			
		СервисАвторизацииДоступен = СервисАвторизацииСервераМЧДДоступен(ТекущиеНастройки);
		Если СервисАвторизацииДоступен Тогда
			
			ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(КлючОбъектаХранилищаНастроек(),
				КлючНастройкиПодключенияКСерверуМЧД(), ТекущиеНастройки);
			
			Результат.СервисАвторизацииДоступен = Истина;
			Результат.НастройкиПодключения = ТекущиеНастройки;
						
			Возврат Результат; 
			
		Иначе
				
			СписокНастроек.Удалить(КлючНастройки);	
...
// Возвращаемое значение:
//  см. НовыйНастройкиПодключенияКСерверуМЧД
Функция СвойстваСервераМЧД() Экспорт
	
	Результат = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(КлючОбъектаХранилищаНастроек(), 
		КлючНастройкиПодключенияКСерверуМЧД());
...
// Возвращаемое значение:
//  Массив из см. НовыйНастройкиПодключенияКСерверуМЧД 
Функция НастройкиПодключенияКСерверуМЧД()
	
	АдресаСерверовМЧД = АдресаСерверовМЧД();
	
	Результат = Новый Массив();
	Для Каждого АдресСервера Из АдресаСерверовМЧД Цикл
		
		CоставныеЧастиАдреса = ОбщегоНазначенияКлиентСервер.СтруктураURI(АдресСервера);
		АдресСервераБезАутентификации = CоставныеЧастиАдреса.Схема + "://" + CоставныеЧастиАдреса.ИмяСервера + "/";
				
		Результат.Вставить(0, НовыйНастройкиПодключенияКСерверуМЧД(АдресСервера, АдресСервераБезАутентификации));
...
// * НастройкиПодключения - Неопределено
// * СервисАвторизацииДоступен - Булево
Функция НовыйРезультатПодключенияКСервисуАвторизации()
	
	 Результат = Новый Структура;
	 Результат.Вставить("НастройкиПодключения", Неопределено);
	 Результат.Вставить("СервисАвторизацииДоступен", Ложь);
...
//
// Возвращаемое значение:
//  Структура - информация о настройках подключения к серверу распределенного реестра МЧД:
// * АдресСервера - Строка
// * АдресСервераБезАутентификации - Строка
// * РесурсКорняAPI - Строка
// * ИспользоватьРасширенияAPI - Булево
...
Функция НовыйНастройкиПодключенияКСерверуМЧД(Знач АдресСервера, Знач АдресСервераБезАутентификации)
	
	Результат = Новый Структура();
	Результат.Вставить("АдресСервера", АдресСервера);
	Результат.Вставить("АдресСервераБезАутентификации", АдресСервераБезАутентификации);
	Результат.Вставить("РесурсКорняAPI", "/applications/MChD/api/clientpoa");
...
// Адреса прокси-серверов для работы с распределенным реестром МЧД.
// 
// Возвращаемое значение:
//  Массив из Строка
Функция АдресаСерверовМЧД()
	
	Результат = Новый Массив();
	Результат.Добавить("https://1cpoagate.1c.ru/");
	Результат.Добавить("https://1cpoagate2.1c.ru/");
	Результат.Добавить("https://1cpoagate3.1c.ru/");
	
	Возврат Результат;
	
КонецФункции

// Возвращает ключ объекта хранилища общих настроек. Используется для сохранения в хранилище общих настроек.
// 
// Возвращаемое значение:
//  Строка
Функция КлючОбъектаХранилищаНастроек()
...
// Возвращаемое значение:
//  Строка
Функция КлючНастройкиПодключенияКСерверуМЧД()
	
	Возврат "НастройкиПодключенияКСерверуМЧД";
...
// Возвращаемое значение:
//  ПеречислениеСсылка.ВариантыПроверкиСертификатовМЧДЭДО
//
Функция ВариантПроверкиСертификатаДоверителя() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	Результат = Константы.ВариантПроверкиСертификатовМЧДЭДО.Получить();
	Если Результат.Пустая() Тогда
		Результат = Перечисления.ВариантыПроверкиСертификатовМЧДЭДО.ПолнаяПроверка;
	КонецЕсли;
	УстановитьПривилегированныйРежим(Ложь);
...
#Область ПроцедурыИФункцииОбработчиковОбновления

// Возвращаемое значение:
//  Строка 
Функция ТекстЗапросаДляРегистрацииДанныхКОбработке()
	...
	ТекстЗапроса = "ВЫБРАТЬ ПЕРВЫЕ 1000
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ ПЕРВЫЕ 1000
	               |	МЧДОрганизаций.Ссылка КАК Ссылка
	               |ИЗ
	               |	Справочник.МашиночитаемыеДоверенностиОрганизаций КАК МЧДОрганизаций
	               |ГДЕ
	               |	МЧДОрганизаций.Ссылка > &МЧДОрганизацийСсылка
	               |	И МЧДОрганизаций.Подписана
	               |	И МЧДОрганизаций.ХешФайла <> """"
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Ссылка
	               |;
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ ПЕРВЫЕ 1000
	               |	МЧДКонтрагентов.Ссылка КАК Ссылка
	               |ИЗ
	               |	Справочник.МашиночитаемыеДоверенностиКонтрагентов КАК МЧДКонтрагентов
	               |ГДЕ
	               |	МЧДКонтрагентов.Ссылка > &МЧДКонтрагентовСсылка
	               |	И МЧДКонтрагентов.Подписана
	               |	И МЧДКонтрагентов.ХешФайла <> """"
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Ссылка";
...
// Возвращаемое значение:
//  Строка
Функция ТекстЗапросаДляРегистрацииДанныхВУпрощенномРежимРаботыСДоверенностями()
		...
		ТекстЗапроса = "ВЫБРАТЬ ПЕРВЫЕ 1000
	               ...
	               |	И (МЧД003.Подписана
	               |	И МЧД003.ХешФайла <> """"
	               |	ИЛИ МЧД003.Подписана
	               |	И МЧД003.ХешФайла <> """"
	               |	И МЧД003.ОбработанаВУпрощенномРежимеРаботыСДоверенностями = ЛОЖЬ)
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Ссылка
	               |;
	               ...
	               |	И (МЧДОрганизаций.Подписана
	               |	И МЧДОрганизаций.ХешФайла <> """"
	               |	ИЛИ МЧДОрганизаций.Подписана
	               |	И МЧДОрганизаций.ХешФайла <> """"
	               |	И МЧДОрганизаций.ОбработанаВУпрощенномРежимеРаботыСДоверенностями = ЛОЖЬ)
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Ссылка
	               |;
	               ...
	               |	И (МЧДКонтрагентов.Подписана
	               |	И МЧДКонтрагентов.ХешФайла <> """"
	               |	ИЛИ МЧДКонтрагентов.Подписана
	               |	И МЧДКонтрагентов.ХешФайла <> """"
	               |	И МЧДКонтрагентов.ОбработанаВУпрощенномРежимеРаботыСДоверенностями = ЛОЖЬ)
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Ссылка";
...
// Выполняет проверку подписи доверенности. Процедура выполняется только в БизКубе, т.к. там доступна проверка через DSS.
// 
// Параметры:
//  ДоверенностьОбъект - СправочникОбъект.МашиночитаемыеДоверенностиКонтрагентов
//  				   - СправочникОбъект.МашиночитаемыеДоверенностиОрганизаций
//                     - СправочникОбъект.МЧД003
//                     
//  Записать - Булево
Процедура ВыполнитьПроверкуПодписиДоверенности(ДоверенностьОбъект, Записать)
	
	Если Не ИспользоватьУпрощенныйРежимРаботыСДоверенностями() Тогда
		Возврат;
	КонецЕсли;
	
	Если ДоверенностьОбъект.ОбработанаВУпрощенномРежимеРаботыСДоверенностями Тогда
		Возврат;
	КонецЕсли;
	...
	СвойстваДоверенностиПроверка = НовыйСвойстваДоверенностиДляОпределенияРучнойПроверкиДоверенности();
	ЗаполнитьЗначенияСвойств(СвойстваДоверенностиПроверка, ДоверенностьОбъект);
	ДоверенностьПроверенаВручную = ОпределитьРучнуюПроверкуПоСвойствамДоверенности(СвойстваДоверенностиПроверка);
	 
	ДанныеДляПроверкиПодписи = ДанныеФайлаДоверенностиИПодписи(ДоверенностьОбъект.Ссылка);
	ДанныеПодписи = ДанныеДляПроверкиПодписи.ДанныеПодписи;
		
	РезультатПроверкиПодписи = КриптографияБЭД.ПроверитьПодпись(
		МенеджерКриптографии, ДанныеДляПроверкиПодписи.ДанныеФайла, ДанныеПодписи, КонтекстДиагностики, Ложь);
		
	Если РезультатПроверкиПодписи.ОшибкаИнициализацииПроверки Тогда
		Возврат;
	КонецЕсли;	
	
	ПроверкаПодписиДетали = РезультатПроверкиПодписи.СвойстваПодписи.Комментарий;
	
	ПодписьВерна = РезультатПроверкиПодписи.СвойстваПодписи.ПодписьВерна;
	ЭтоОшибкаМатематическойПроверкиПодписи = ЭтоОшибкаМатематическойПроверкиПодписи(ПроверкаПодписиДетали);	
	
	Если ПодписьВерна 
		Или Не ЭтоОшибкаМатематическойПроверкиПодписи Тогда
	
		РезультатПроверкиСредствамиКриптографии = Истина;
		
		ДанныеСертификатаПодписи = ПолучитьДанныеСертификатаПодписи(РезультатПроверкиПодписи, ДанныеПодписи);
		Если ДанныеСертификатаПодписи <> Неопределено Тогда
				
			Сертификат = Новый СертификатКриптографии(ДанныеСертификатаПодписи);
				
			КонтекстПроверкиСертификата = 
				КриптографияБЭДКлиентСервер.НовыйКонтекстПроверкиКвалифицированногоСертификата(Сертификат.Выгрузить(), 
					РезультатПроверкиСредствамиКриптографии, ДанныеПодписи);
			
			РезультатПроверкиСертификата = 
				КриптографияБЭДКлиентСервер.ПроверитьКвалифицированныйСертификат(Сертификат, КонтекстПроверкиСертификата);
			
			ПротоколПроверки = РезультатПроверкиСертификата.Протокол;
			ПроверкаСертификатаЗавершиласьУспехом = РезультатПроверкиСертификата.Успех;
			
			Если Не ПроверкаСертификатаЗавершиласьУспехом Тогда 
				Ошибка = КриптографияБЭДКлиентСервер.СформироватьОшибкуПоПротоколуПроверкиКвалификации(Сертификат, 
					ПротоколПроверки);
				РезультатПроверкиПодписи.ОписаниеОшибки = Ошибка.ПодробноеПредставлениеОшибки;
				РезультатПроверкиПодписи.СвойстваПодписи.Комментарий = Ошибка.ПодробноеПредставлениеОшибки;
			КонецЕсли;
			
			РезультатПроверкиПодписи.СвойстваПодписи.ПодписьВерна = ПроверкаСертификатаЗавершиласьУспехом;
			РезультатПроверкиПодписи.СвойстваПодписи.Протокол = ПротоколПроверки;
			
			РезультатПроверки = РезультатПроверкиПодписи.СвойстваПодписи.ПодписьВерна;
		
		Иначе
			
			РезультатПроверки = ПодписьВерна;
			РезультатПроверкиПодписи.ОписаниеОшибки = ПроверкаПодписиДетали;
	...
	Если Не ДоверенностьПроверенаВручную Тогда
		ДоверенностьОбъект.Верна = РезультатПроверки;
		ДоверенностьОбъект.ТекстОшибкиПроверкиМЧД = РезультатПроверкиПодписи.ОписаниеОшибки;
	КонецЕсли;
	
	ДоверенностьОбъект.ПротоколПроверки = Новый ХранилищеЗначения(РезультатПроверкиПодписи, Новый СжатиеДанных(9));
	ДоверенностьОбъект.ОбработанаВУпрощенномРежимеРаботыСДоверенностями = Истина;
	
	ЗаписатьМашиночитаемуюДоверенностьВЖурнал(ДоверенностьОбъект, Ложь);
...
// Возвращаемое значение:
//  Булево
Функция ИспользоватьУпрощенныйРежимРаботыСДоверенностями()
	
	ИспользоватьУпрощенныйРежимРаботыСДоверенностями = Ложь;
	Возврат ИспользоватьУпрощенныйРежимРаботыСДоверенностями;

КонецФункции

// Определить ручную проверку по свойствам доверенности.
// 
// Параметры:
//  СвойстваДоверенности - Структура:
// * Верна - Булево
// * ТекстОшибкиПроверкиМЧД - Строка
// 
// Возвращаемое значение:
//  Булево
Функция ОпределитьРучнуюПроверкуПоСвойствамДоверенности(Знач СвойстваДоверенности)
	
	Возврат СвойстваДоверенности.Верна 
		И СтрНайти(СвойстваДоверенности.ТекстОшибкиПроверкиМЧД, НСтр("ru='Проверена вручную:'")) > 0;
...
// * Верна - Булево
// * ТекстОшибкиПроверкиМЧД - Строка
Функция НовыйСвойстваДоверенностиДляОпределенияРучнойПроверкиДоверенности()
	
	Возврат Новый Структура("Верна, ТекстОшибкиПроверкиМЧД", Ложь, "");
	
КонецФункции

// Получить данные сертификата подписи.
// 
// Параметры:
//  РезультатПроверкиПодписи см. КриптографияБЭДКлиентСервер.НовыйРезультатПроверкиПодписи
//  ДанныеПодписи - ДвоичныеДанные
// 
// Возвращаемое значение:
//  Неопределено, ДвоичныеДанные - Данные сертификата подписи
Функция ПолучитьДанныеСертификатаПодписи(РезультатПроверкиПодписи, ДанныеПодписи)
	
	ДанныеСертификата = РезультатПроверкиПодписи.СвойстваПодписи.Сертификат;
	
	Если ДанныеСертификата <> Неопределено Тогда
		Возврат ДанныеСертификата;
	КонецЕсли;
		
	ДанныеСертификата = КриптографияБЭДКлиентСервер.ДанныеСертификатаИзДвоичныхДанныхПодписи(ДанныеПодписи);	
		
	Возврат ДанныеСертификата;
	
КонецФункции

// Проверяет, что переданная строка содержит ошибку проверки математической подписи.
// 
// Параметры:
//  ТекстОшибки - Строка
// 
// Возвращаемое значение:
//  Булево
Функция ЭтоОшибкаМатематическойПроверкиПодписи(ТекстОшибки)
	
	Результат = Ложь;
	
	Ошибки = ОшибкиМатематическойПроверкиПодписиDSS();
	
	Для Каждого Ошибка Из Ошибки Цикл
		
		Результат = СтрНайти(НРег(ТекстОшибки), Ошибка.Ключ) > 0;
...
// Возвращает коды и описание ошибок проверки математической корректности подписи с использованием сервиса SVS (DSS)
// 
// Возвращаемое значение:
//  Соответствие из КлючИЗначение
Функция ОшибкиМатематическойПроверкиПодписиDSS()
	
	Результат = Новый Соответствие;
	Результат.Вставить("0x80090006"); // Подпись недействительна
	Результат.Вставить("0x80096010"); // Цифровая подпись объекта не проверена
	Результат.Вставить("0x80091007"); // Некорректное значение хэша
	Результат.Вставить("0x8009310b"); // Встречено неверное значение тега АСН.1
