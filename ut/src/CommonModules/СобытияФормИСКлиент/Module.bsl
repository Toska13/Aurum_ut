...
Процедура ВыполнитьПереопределяемуюКоманду(Форма, Команда, ДополнительныеПараметры) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПараметрыИнтеграцииГосИС") Тогда
		...
		ПроверкаИПодборПродукцииИСКлиент.ВыполнитьПереопределяемуюКоманду(Форма, Команда, ДополнительныеПараметры);
...
Процедура ОбработкаОповещенияВФормеДокументаОснования(МестоВызова, Событие, Подсистема) Экспорт
	...
	ИначеЕсли ИнтеграцияИСКлиентСервер.ЭтоСобытиеОповещенияИзмененОбъект(ОписаниеСобытия) Тогда
		...
		Если ЧастиИмениОбъекта.Количество() = 2 
			...
			СсылкаНаОбъект = МестоВызова.Объект.Ссылка;
			Если ЗначениеЗаполнено(МестоВызова.Ссылка) Тогда
				СсылкаНаОбъект = МестоВызова.Ссылка;
			КонецЕсли;
			ОбрабатываютсяДокументы = Подсистема.МодульВызовСервера.ИменаДокументовДляДокументаОснования(СсылкаНаОбъект);
	...
	Если Событие.Обработано Тогда
		...
		Если Событие.Параметр = Неопределено
			Или ОбязательныеПараметрыОповещения.Основание = МестоВызова.Объект.Ссылка
			Или ЗначениеЗаполнено(МестоВызова.Ссылка) И ОбязательныеПараметрыОповещения.Основание = МестоВызова.Ссылка Тогда
	...
	Если Результат <> Неопределено
		...
		Если ПараметрыСканирования <> Неопределено
			...
			Если Форма.СоответствиеШтрихкодовСтрокДерева.Получить(НормализованныйШтрихкод) <> Неопределено Тогда
				Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(ВидПродукции) Тогда
...
// Возвращаемое значение:
//  Структура - Описание:
// * ВидПродукции - СправочникСсылка.ВидыПродукцииИС, Неопределено - Необязательный параметр. Нужно заполнять в случае
