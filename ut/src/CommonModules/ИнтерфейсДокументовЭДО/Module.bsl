...
Процедура НастроитьСтраницуСообщений(Форма, ЭлементТитулаСхемы) Экспорт
	...
	Если Не ВыводитьЧатУниверсальныхСообщений() Тогда
...
Функция СформироватьФайлыВыгрузкиЭДВФорматеPDF(Знач ЭлектронныеДокументы, Знач АдресДанныхВХранилище) Экспорт
	...
	Для каждого ДанныеПечатнойФормы Из ПечатныеФормы Цикл
		...
		РаботаСФайламиБЭД.СократитьИмяФайла(ПолноеИмяФайла);
...
Функция СформироватьФайлыВыгрузкиЭДДокументооборотЦеликом(Знач ЭлектронныеДокументы, Знач АдресДанныхВХранилище) Экспорт
	...
	Для каждого ЭлектронныйДокумент Из ЭлектронныеДокументы Цикл
		...
		Для каждого ФайлДокумента Из ФайлыДокумента Цикл
			...
			ИмяФайла = КаталогСообщения
				+ ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ПолныеИменаФайлов[ПрисоединенныйФайл]).Имя;
...
Процедура ИнициализироватьРазделыТекущихДел(ОписаниеРазделов) Экспорт
	...
	ДобавитьОписаниеРаздела(ОписаниеРазделов, "СообщенияВх", НСтр("ru = 'Сообщения'"));
	...
	ДобавитьОписаниеРаздела(ОписаниеРазделов, "СообщенияИсх", НСтр("ru = 'Сообщения'"));
...
Функция ЗапросКоличестваТекущихЭлектронныхДокументов(Параметры) Экспорт
	...
	ИначеЕсли Раздел = "СообщенияВх" Тогда
		...
		ТекстЗапроса =
			...
			|ИЗ
			|	(ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 1000
			|		ОсновнаяТаблица.ЭлектронныйДокумент
			|	ИЗ
			|		РегистрСведений.ТекстовыеСообщенияДокументовЭДО КАК ОсновнаяТаблица
			|	ГДЕ
			|		ОсновнаяТаблица.Прочитано = ЛОЖЬ
			|		И ОсновнаяТаблица.ЭлектронныйДокумент ССЫЛКА Документ.ЭлектронныйДокументВходящийЭДО) КАК ВложенныйЗапрос";
	...
	ИначеЕсли Раздел = "СообщенияИсх" Тогда
		...
		ТекстЗапроса =
			...
			|	ГДЕ
			|		ОсновнаяТаблица.Прочитано = ЛОЖЬ 
			|		И ОсновнаяТаблица.ЭлектронныйДокумент ССЫЛКА Документ.ЭлектронныйДокументИсходящийЭДО) КАК ВложенныйЗапрос";	
	Иначе
...
Функция НастройкиПодбораУчетногоДокумента(ЭлектронныйДокумент, СпособОбработки = "") Экспорт
		...
		Если СодержаниеСообщения <> Неопределено Тогда
			...
			Иначе
				...
				ИспользоватьИнтеркампани = 
					ОбщегоНазначенияБЭД.ЗначениеФункциональнойОпции("ИспользоватьОбменЭДМеждуОрганизациями");
				Если ИспользоватьИнтеркампани Тогда
					ОрганизацияОтправитель = 
						ИнтеграцияЭДО.СсылкаНаОбъектПоИННКПП("Организации", Отправитель.ИНН, Отправитель.КПП);
				КонецЕсли;
	...
	Если ЗначениеЗаполнено(Контрагент) И ЗначениеЗаполнено(ОрганизацияОтправитель) Тогда
		Отправители = ОбщегоНазначенияБЭДКлиентСервер.МассивЗначений(Контрагент, ОрганизацияОтправитель);
	КонецЕсли;
	...
	Настройки.Вставить("Контрагент", ?(Отправители.Количество(), Отправители, Контрагент));
...
//  * ВключатьАвтоматическоеПолучениеЭДО - Булево
//	* ПоказыватьСписокДокументовТакскомТребующихЗавершения - Булево
Функция ПараметрыОтправкиПолученияДокументовЭДО(УчетныеЗаписиОбмена = Неопределено) Экспорт
	...
	СкрыватьВопросСпискаДокументовТакскомКЗавершению = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"ПереводИзТакскомКлючНастроек",
		"НеПоказыватьВопросСпискаДокументовТакскомКЗавершению",
		Ложь);
	
	ПоказыватьСписокДокументовТакскомТребующихЗавершения = Не СкрыватьВопросСпискаДокументовТакскомКЗавершению
		И УчетныеЗаписиЭДО.ПоказыватьСписокДокументовТакскомТребующихЗавершения(); 
	ПараметрыОтправкиПолучения.Вставить("ПоказыватьСписокДокументовТакскомТребующихЗавершения", 
		ПоказыватьСписокДокументовТакскомТребующихЗавершения);
	
	ВключатьАвтоматическоеПолучениеЭДО = АвтоматическоеПолучениеЭДО.ВключатьАвтоматическиДляУчетныхЗаписейЭДО(
		ПараметрыВыполненияДействий.ОбъектыДействий.ИдентификаторыОрганизаций);
	ПараметрыОтправкиПолучения.Вставить("ВключатьАвтоматическоеПолучениеЭДО", ВключатьАвтоматическоеПолучениеЭДО);
...
// Возвращаемое значение:
//  Булево
Функция ВыводитьЧатУниверсальныхСообщений() Экспорт
	Возврат Ложь; //Отключено до поддержки сервисом нескольких сообщений формата 4002
КонецФункции
...
Функция ТекстЗапросаПоТекущемуТрафику()
	...
	|	Сумма(ВЫБОР
	|		КОГДА
	|			ЭлектронныйДокументИсходящийЭДО.ВидДокумента.ТипДокумента = ЗНАЧЕНИЕ(Перечисление.ТипыДокументовЭДО.СчетФактура)
	|				ИЛИ ЭлектронныйДокументИсходящийЭДО.ВидДокумента.ТипДокумента = ЗНАЧЕНИЕ(Перечисление.ТипыДокументовЭДО.КорректировочныйСчетФактура)
	|				ИЛИ ЭлектронныйДокументИсходящийЭДО.ВидДокумента.ТипДокумента = ЗНАЧЕНИЕ(Перечисление.ТипыДокументовЭДО.УПД)
	|				ИЛИ ЭлектронныйДокументИсходящийЭДО.ВидДокумента.ТипДокумента = ЗНАЧЕНИЕ(Перечисление.ТипыДокументовЭДО.УКД)
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ) КАК СЧФ,
	|	Сумма(ВЫБОР
	...
	|			ТОГДА ОКР((ЕСТЬNULL(ОтправленныеПоВидамЭД.НеСЧФ, 0) + 1) / 2)
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ДанныеЗаполненияНастройкиОтправкиПриглашений", Истина);
	Методы.Вставить("ЗагрузитьФормализованныйДокументПоФайлуXMLВФоне", Истина);
	Методы.Вставить("ОбновитьДанныеДляНастройкиОтправкиПриглашений", Истина);
	Методы.Вставить("СформироватьФайлыВыгрузкиЭДВФорматеPDF", Истина);
	Методы.Вставить("СформироватьФайлыВыгрузкиЭДДляФНС", Истина);
	Методы.Вставить("ФайлСДаннымиОЗадолженности", Истина);
