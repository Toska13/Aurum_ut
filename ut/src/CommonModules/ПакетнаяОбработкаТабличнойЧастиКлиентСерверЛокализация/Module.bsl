...
// Дополняет структуру действий над строкой табличной части при изменении элемента формы.
// 
// Параметры:
...
Процедура ДополнитьСтруктуруДействийПриИзмененииЭлемента(Форма, Элемент, СтруктураДействий) Экспорт
	...
	Если Форма.ИмяФормы = "Документ.ЧекККМ.Форма.ФормаДокументаРМК"
		...
		Если Элемент = "Номенклатура" Или Элемент = "Характеристика" Или Элемент = "Серия" Тогда
			СтруктураДействий.Вставить("ЗаполнитьСобственникаТовараИСтавкуНДСПриУСНКомитентов", ПараметрыЗаполненияСобственникаТовараИСтавкиНДСПриУСНКомитентов(Форма.Объект, Форма.Объект.Подразделение));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПараметрыИнтеграцииГосИС") Тогда
		Возврат;
	КонецЕсли;
	ЕГАИС = Форма.ПараметрыИнтеграцииГосИС.Получить("ЕГАИС") <> Неопределено;
	ИСМП = Форма.ПараметрыИнтеграцииГосИС.Получить("ИСМП") <> Неопределено;
			...
			Если ЕГАИС Или ИСМП Тогда
				...
				Если (Форма.ИмяФормы = "Документ.ЧекККМ.Форма.ФормаДокументаРМК"
						Или (Форма.ИмяФормы = "Документ.ЧекККМВозврат.Форма.ФормаДокументаРМК"
							И Не ЗначениеЗаполнено(Форма.Объект.ЧекККМ)))
					И РозничныеПродажиКлиентСерверЛокализация.РозничныеПродажиТабачнойПродукцииТолькоПоМРЦ(Форма) Тогда
					СтруктураДействий.Вставить(
						"ПроверитьУстановкуМРЦОчиститьЦенуДляТабачнойПродукции",
						ПараметрыПроверкиУстановкиМРЦОчисткиЦеныДляТабачнойПродукции(Форма));
				КонецЕсли;
			КонецЕсли;
	...
	ИначеЕсли Форма.ИмяФормы = "Документ.ЗаказКлиента.Форма.ФормаДокумента"
		Или Форма.ИмяФормы = "Документ.ЗаявкаНаВозвратТоваровОтКлиента.Форма.ФормаДокумента"
		Или Форма.ИмяФормы = "Обработка.ПодборТоваровВДокументПродажи.Форма.Форма" Тогда
		
		Если Элемент = "Номенклатура"
			Или Элемент = "НоменклатураПризнаки" Тогда
	...
	ИначеЕсли (Форма.ИмяФормы = "Документ.ВозвратТоваровОтПокупателя.Форма.ФормаДокумента"
		...
		Если Форма.ПараметрыИнтеграцииГосИС.Получить("ФормаПроверкиИПодбора") <> Неопределено
			И Форма.ПараметрыИнтеграцииГосИС.Получить("ФормаПроверкиИПодбора")[0].ИспользоватьКолонкуСтатусаПроверкиПодбора Тогда
			СтруктураДействий.Вставить("ЗаполнитьВидПродукцииИС", Новый Структура("Номенклатура", "ВидПродукцииИС"));
			СтруктураДействий.Вставить("ЗаполнитьПризнакМаркируемаяПродукцияГИСМТ",
				Новый Структура("Номенклатура", "МаркируемаяПродукция"));
		КонецЕсли;
...
// Дополняет структуру действий, для заполнения "Ставки НДС" по настройке учета комитента.   
// 
// Параметры:
//  Объект - ДанныеФормыСтруктура, ДокументОбъект - данные документа.
//  СтруктураДействий - см. ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ.СтруктураДействий.
//
Процедура ДобавитьДействиеЗаполненияСтавкиНДСПоНастройкеУчета(Объект, СтруктураДействий) Экспорт
	...
	Параметры.Вставить("НалогообложениеНДС");
	Параметры.Вставить("Комитент");
	...
	Если ЗначениеЗаполнено(Параметры.Комитент) Тогда
		
		СтруктураДействий.Вставить("ЗаполнитьСтавкуНДСПоНастройкеУчетаПриУСНКомитента", Параметры);
...
// Дополняет коллекцию кэшируемой информации о товаре по штрихкоду.
//
// Параметры:
//	ИнформацияПоШтрихкоду - см. ПакетнаяОбработкаТабличнойЧастиКлиентСервер.СтруктураКэшируемойИнформацииПоШтрихкоду.
//
Процедура ДополнитьСтруктуруКэшируемойИнформацииПоШтрихкоду(ИнформацияПоШтрихкоду) Экспорт
...
Процедура ПоляСтрокиПоДействиям(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения, ДополнительныеПараметрыЗаполнения) Экспорт
	...
	Если СтруктураДействий.Свойство("ЗаполнитьЕстьВскрытыеПотребительскиеУпаковкиИС", СтруктураПараметровДействия)
		...
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ДополнитьОписаниеПолей("ЗаполнитьЕстьВскрытыеПотребительскиеУпаковкиИС",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ЗаполнитьПризнакМаркируемаяПродукцияЕГАИС", СтруктураПараметровДействия) 
		...
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ДополнитьОписаниеПолей("ЗаполнитьПризнакМаркируемаяПродукцияЕГАИС",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ЗаполнитьПризнакМаркируемаяПродукцияГИСМТ", СтруктураПараметровДействия) 
		...
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ДополнитьОписаниеПолей("ЗаполнитьПризнакМаркируемаяПродукцияГИСМТ",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ЗаполнитьСобственникаТовараИСтавкуНДСПриУСНКомитентов") Тогда
	
		ЧитаемыеПоля = Новый Структура();
		ЧитаемыеПоля.Вставить("Номенклатура");
		ЧитаемыеПоля.Вставить("Характеристика");
		ЧитаемыеПоля.Вставить("Серия");
		ЧитаемыеПоля.Вставить("Комитент");
		...
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ДополнитьОписаниеПолей("ЗаполнитьСобственникаТовараИСтавкуНДСПриУСНКомитентов",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
			
	КонецЕсли;
	
	Если СтруктураДействий.Свойство("ПроверитьУстановкуМРЦОчиститьЦенуДляТабачнойПродукции", СтруктураПараметровДействия) Тогда
		...
		ИзменяемыеПоля.Вставить("СуммаНДС");
		ИзменяемыеПоля.Вставить(РозничныеПродажиКлиентСерверЛокализация.ИмяРеквизитаУстановленаМРЦ());
		Если СтруктураПараметровДействия <> Неопределено
			И СтруктураПараметровДействия.ИмяФормы = "Документ.ЧекККМ.Форма.ФормаДокументаРМК" Тогда
			...
			Если СтруктураПараметровДействия.ИспользоватьБонусныеБаллы = Истина Тогда
				ИзменяемыеПоля.Вставить("СуммаБонусныхБалловКСписанию"); 
				ИзменяемыеПоля.Вставить("СуммаБонусныхБалловКСписаниюВВалюте"); 
				ИзменяемыеПоля.Вставить("СуммаНачисленныхБонусныхБалловВВалюте"); 
			КонецЕсли;	
		КонецЕсли;
		
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ДополнитьОписаниеПолей("ПроверитьУстановкуМРЦОчиститьЦенуДляТабачнойПродукции",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	КонецЕсли;
...
// Возвращает параметры, используемые при заполнении собственника товара и ставки НДС при использовании УСН у комитентов.
// Если передан второй параметр, то в возвращаемой структуре будет использовано именно его значение.
// 
// Параметры:
...
//  Подразделение - СправочникСсылка.СтруктураПредприятия, Неопределено - Если значение задано, то оно будет записано в возвращаемом значении.
//	 
// Возвращаемое значение:
...
//      * ПерезаполнитьСобственникаТовара - Булево
//
Функция ПараметрыЗаполненияСобственникаТовараИСтавкиНДСПриУСНКомитентов(Объект, Подразделение = Неопределено) Экспорт
	
	ПараметрыЗаполненияСтавкиНДСПриУСНКомитентов = Новый Структура;
	ПараметрыЗаполненияСтавкиНДСПриУСНКомитентов.Вставить("Организация",		Объект.Организация);
	ПараметрыЗаполненияСтавкиНДСПриУСНКомитентов.Вставить("Склад",				Объект.Склад);

	ПараметрыЗаполненияСтавкиНДСПриУСНКомитентов.Вставить("Подразделение", Подразделение);
	Если Не ЗначениеЗаполнено(Подразделение)
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "Подразделение") Тогда
		ПараметрыЗаполненияСтавкиНДСПриУСНКомитентов.Вставить("Подразделение", Объект.Подразделение);
	КонецЕсли;
	
	ПараметрыЗаполненияСтавкиНДСПриУСНКомитентов.Вставить("Дата",				Неопределено);
	Если Не Объект.Ссылка.Пустая() Тогда
		ПараметрыЗаполненияСтавкиНДСПриУСНКомитентов.Вставить("Дата",			Объект.Дата);
	КонецЕсли;
	
	ПараметрыЗаполненияСтавкиНДСПриУСНКомитентов.Вставить("ПерезаполнитьСобственникаТовара", Истина);
	
	Возврат ПараметрыЗаполненияСтавкиНДСПриУСНКомитентов;
	
КонецФункции

// Возвращает параметры, используемые при проверке установки МРЦ и очистке цены для табачной продукции в табличной части объекта.
//
// Параметры:
...
//		* ИмяФормы - Строка -
//		* НовыеСтроки - Булево - 
//		* ИспользоватьБонусныеБаллы - Булево -
//
Функция ПараметрыПроверкиУстановкиМРЦОчисткиЦеныДляТабачнойПродукции(Форма) Экспорт
	
	ПараметрыПроверкиУстановкиМРЦОчисткиЦеныДляТабачнойПродукции = Новый Структура;
	ПараметрыПроверкиУстановкиМРЦОчисткиЦеныДляТабачнойПродукции.Вставить("ИмяФормы", Форма.ИмяФормы);
	ПараметрыПроверкиУстановкиМРЦОчисткиЦеныДляТабачнойПродукции.Вставить("НовыеСтроки", Истина);
	ПараметрыПроверкиУстановкиМРЦОчисткиЦеныДляТабачнойПродукции.Вставить("ИспользоватьБонусныеБаллы", Ложь);
	Если Форма.ИмяФормы = "Документ.ЧекККМ.Форма.ФормаДокументаРМК" Тогда
		ПараметрыПроверкиУстановкиМРЦОчисткиЦеныДляТабачнойПродукции.Вставить("ИспользоватьБонусныеБаллы", Форма.ИспользоватьБонусныеПрограммыЛояльности);
	ИначеЕсли Форма.ИмяФормы = "Документ.РеализацияТоваровУслуг.Форма.ФормаДокумента" Тогда
		ПараметрыПроверкиУстановкиМРЦОчисткиЦеныДляТабачнойПродукции.Вставить("ИспользоватьБонусныеБаллы", Форма.ИспользоватьБонусныеПрограммыЛояльности);
	КонецЕсли;	
	Возврат ПараметрыПроверкиУстановкиМРЦОчисткиЦеныДляТабачнойПродукции;
