...
Функция ЗагрузитьФайлНаСервис(ТокенДоступа, ПрисоединенныйФайл, ОбновитьФайлНаСервисе = Истина) Экспорт
	...
	Если Не ОбновитьФайлНаСервисе Тогда
		ИнформацияОРесурсе = ПолучитьИнформациюОРесурсе(ТокенДоступа, ПрисоединенныйФайл, "public_url");
		Если ИнформацияОРесурсе.КодСостояния = 200 Тогда
			РезультатПубликации.ПубличнаяСсылка = ИнформацияОРесурсе.public_url; //Ссылка на опубликованный ресурс. Включается в ответ только если указанный файл или папка опубликован.
			РезультатПубликации.Отказ = Не ЗначениеЗаполнено(ИнформацияОРесурсе.public_url);
	...
	Если Не ЗапросУспешноОбработан(РезультатОтправки.КодСостояния) Тогда
		...
		Если РезультатОтправки.КодСостояния = 507 Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС + НСтр("ru = 'Недостаточно свободного места.'");
		КонецЕсли;
...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Форма.ИмяФормы = "Справочник.Номенклатура.Форма.ФормаЭлемента" Тогда
		...
		Если Не ЗначениеЗаполнено(Форма.Объект["Ссылка"]) Тогда
			...
			ВходящиеПараметры.Вставить("НоменклатураКонтрагента", Неопределено);
			...
			Если ЗначениеЗаполнено(ВходящиеПараметры.УчетнаяЗапись) Тогда
				Если ЗначениеЗаполнено(ВходящиеПараметры.НоменклатураКонтрагента) Тогда
					НоменклатураКонтрагента = ВходящиеПараметры.НоменклатураКонтрагента;
				Иначе
...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	ИнтеграцияСМаркетплейсамиСИ.ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки);
...
// Добавить поиск через сервис.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - форма, на которую необходимо добавить кнопку поиска.
//  МестоРазмещения - Неопределено, ГруппаФормы - родитель добавляемой кнопки.
//  ПередЭлементом - Неопределено, ДекорацияФормы, ГруппаФормы, КнопкаФормы, ТаблицаФормы, ПолеФормы - элемент, перед
//                   которым необходимо добавить кнопку поиска.
//
Процедура ДобавитьПоискЧерезСервис(Форма, МестоРазмещения, ПередЭлементом = Неопределено) Экспорт
	
	// Команда поиска через сервис.
	МетаданныеКоманды = Метаданные.РегистрыСведений.ЗаказыТорговыхПлощадок.Команды.ВыполнитьПоискЧерезСервис;
	ИмяКоманды = ИнтеграцияСМаркетплейсамиКлиентСервер.ИмяКомандыВыполнитьПоискЧерезСервис();
	
	КомандаФормы = Форма.Команды.Добавить(ИмяКоманды);
	КомандаФормы.Действие = "Подключаемый_ВыполнитьПереопределяемуюКоманду";
	КомандаФормы.Заголовок = МетаданныеКоманды.Синоним;
	КомандаФормы.Подсказка = МетаданныеКоманды.Подсказка;
	КомандаФормы.Отображение = МетаданныеКоманды.Отображение;
	КомандаФормы.Картинка = МетаданныеКоманды.Картинка;
	КомандаФормы.СочетаниеКлавиш = Новый СочетаниеКлавиш(Клавиша.F7);
	
	НовыйЭлемент = Форма.Элементы.Вставить(ИмяКоманды, Тип("КнопкаФормы"), МестоРазмещения, ПередЭлементом);
...
Функция ОтразитьДанныеОРасхожденияхЭлектронногоДокумента(Знач ПервичныйДокумент, ДанныеЭлектронногоДокумента, 
				...
				Если ДанныеМаркировки <> Неопределено И ЗначениеЗаполнено(ДанныеМаркировки.Значение) Тогда
					Для Каждого СтрокаМаркировки Из ДанныеМаркировки.Строки Цикл
						...
						Если Не КоличествоПоОСУ = Неопределено 
							...
							ЗначениеШтрихкода = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ОСУПоШтрихкодуEAN(
								ДанныеГТИН, КоличествоПоОСУ, Ложь);
							ДобавитьШтрихкодУпаковкиИзСтрокиДерева(Штрихкоды, ЗначениеШтрихкода, 
	...
	ШтрихкодыУпаковокИзФайла = ШтрихкодированиеИС.ДанныеШтрихкодовУпаковок(
		ШтрихкодыУпаковок.ВыгрузитьКолонку("ЗначениеШтрихкода"),
		ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(ОбъектУчета),,
		Истина);
...
Процедура ПолучитьТабличныеЧастиИзДокумента(Знач ПервичныйДокумент, ТоварыИзДокумента, ШтрихкодыУпаковокИзДокумента) Экспорт
	...
	Если ТаблицаТоварыДляЭДО.Количество() Тогда
		ДанныеШапки = ШтрихкодированиеИС.ПолучитьСтруктуруРеквизитовДокумента(ПервичныйДокумент);
...
#Область ОписаниеДокументаОтгрузки

// Определяет виды торговых площадок, у которых не используется документ отгрузки в регистре заказов торговых площадок.
// 
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.ВидыМаркетплейсов
//
Функция МаркетплейсыБезДокументаОтгрузкиВЗаказах() Экспорт
	
	Результат = Новый Массив; // Массив из ПеречислениеСсылка.ВидыМаркетплейсов
	Результат.Добавить(Перечисления.ВидыМаркетплейсов.МаркетплейсЯндексМаркет);
	Возврат Результат;
	
КонецФункции

// Определяет необходимость указания документа отгрузки в регистре сведений Заказы торговых площадок.
// 
// Параметры:
//  ВидМаркетплейса - ПеречислениеСсылка.ВидыМаркетплейсов - проверяемый вид торговой площадки.
// 
// Возвращаемое значение:
//  Булево
//
Функция ТребуетсяУказаниеДокументаОтгрузкиВЗаказахТорговыхПлощадок(ВидМаркетплейса) Экспорт
	
	МаркетплейсыБезДокументаОтгрузки = МаркетплейсыБезДокументаОтгрузкиВЗаказах();
	
	Результат = (МаркетплейсыБезДокументаОтгрузки.Найти(ВидМаркетплейса) = Неопределено);
	Возврат Результат;
...
// Получает имя файла без расширения из полного пути к файлу.
//
// Параметры:
//  ПутьКФайлу - Строка - полный путь к файлу.
//  РасширениеФайла - Строка - переменная для получения расширения.
//
// Возвращаемое значение:
//  Строка - имя файла без расширения.
//
Функция ИмяФайлаБезРасширения(ПутьКФайлу, РасширениеФайла = "") Экспорт

	ИмяФайла = СтрЗаменить(ПутьКФайлу, "\", "/");
	...
	ИмяФайла = Сред(ИмяФайла, 1, СтрНайти(ИмяФайла, ".") - 1);
...
//   ВключаемыеПоля            - см. ВключаемыеПоляДляЗапросаЗначенийПолейОбъектаМетаданных
//                             - Неопределено
...
Функция ПустаяТаблицаОбъектаМетаданных(ИмяОбъектаМетаданных, ПривилегированныйРежим = Ложь,
			Знач ВключаемыеПоля = Неопределено, ИсключаемыеТипыДляКолонок = Неопределено) Экспорт

	ТекстЗапроса = СтрЗаменить(
		"ВЫБРАТЬ ПЕРВЫЕ 0
		|	&ТекстЗапросаПоляОбъектаМетаданных КАК Поля
		|ИЗ
	...
	ТекстЗапроса = СтрЗаменить(
		ТекстЗапроса,
		"&ТекстЗапросаПоляОбъектаМетаданных КАК Поля",
		ТекстЗапросаПолейОбъектаМетаданных(ИмяОбъектаМетаданных, "ОбъектМетаданных", ВключаемыеПоля));
...
// Формирует набор полей для запроса данных объекта метаданных.
// Отсутствующие поля игнорируются без генерации исключений.
// 
// Параметры:
//  ВключатьВсеПоля - Булево - признак включения для всех полей по умолчанию.
// 
// Возвращаемое значение:
//  Структура:
//   * ВключатьИзмерения - Булево
//   * ВключатьРесурсы - Булево
//   * ВключатьСтандартныеРеквизиты - Булево
//   * ВключатьРеквизиты - Булево
//   * ВключатьТабличныеЧасти - Булево
//
Функция ВключаемыеПоляДляЗапросаЗначенийПолейОбъектаМетаданных(ВключатьВсеПоля = Истина) Экспорт
	
	Результат = Новый Структура;
	
	Результат.Вставить("ВключатьИзмерения", ВключатьВсеПоля);
	Результат.Вставить("ВключатьРесурсы", ВключатьВсеПоля);
	Результат.Вставить("ВключатьСтандартныеРеквизиты", ВключатьВсеПоля);
	Результат.Вставить("ВключатьРеквизиты", ВключатьВсеПоля);
	Результат.Вставить("ВключатьТабличныеЧасти", ВключатьВсеПоля);
...
//
Функция ТекстЗапросаПолейОбъектаМетаданных(ИмяОбъектаМетаданных, ПсевдонимТаблицы, Знач ВключаемыеПоля = Неопределено,
			Знач ИсключаемыеПоля = Неопределено, ПрефиксПолей = "") Экспорт
	
	Если ВключаемыеПоля = Неопределено Тогда
		ВключаемыеПоля = ВключаемыеПоляДляЗапросаЗначенийПолейОбъектаМетаданных();
	КонецЕсли;
	...
	Если ВключаемыеПоля.ВключатьТабличныеЧасти И Кандидаты.ТабличныеЧасти <> Неопределено Тогда
...
Функция ЗаписатьЗаказыКлиентов(Результат, ТаблицаЗаказов, Параметры, ПараметрыПрогресса = Неопределено) Экспорт
	...
	ТаблицаОстатков = РезультатЗапроса[РезультатЗапроса.ВГраница() - 1].Выгрузить();
	...
	ВыборкаНомерДокумента = РезультатЗапроса[РезультатЗапроса.ВГраница()].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			...
			Если Параметры.ОбработатьВариантОбеспечения Тогда
				Если НовыйЗаказ.Статус = Перечисления.СтатусыЗаказовТорговыхПлощадок.Отменен Тогда
					...
					Если Не ЗначениеЗаполнено(СтрокаТабличнойЧасти.ПричинаОтмены) Тогда
						ДанныеПричиныОтмены = НовыеДанныеПричиныОтменыЗаказа();
...
Функция СоздатьКонтрагента(ДанныеЗаполнения, ВидМаркетплейса = Неопределено, ИспользоватьПривилегированныйРежим = Истина) Экспорт
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось записать контрагента ""%1"" по причине: %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ПеренестиНоменклатуруПартнера(Параметры, Скопировать = Истина) Экспорт
	...
	ОтправитьСерверноеУведомлениеНаКлиент(Параметры.ИмяСобытияОповещения, Параметры.УчетнаяЗапись);
...
//  ЗаписыватьПрисоединенныеФайлы - Булево
//
// Возвращаемое значение:
...
Функция ОбновитьДанныеСтатусовПубликацииПоНоменклатуреПартнера(УчетнаяЗаписьМаркетплейса,
			МассивНоменклатурыПартнера = Неопределено, ЗаписыватьПрисоединенныеФайлы = Ложь) Экспорт
	...
	Попытка
		...
		Если НовыеЗаписи.Количество() > 0 Тогда
			ДополнитьНовыеЗаписиСтатусовПубликацииДополнительнымиДанными(
				РеквизитыУчетнойЗаписи.ВидМаркетплейса, НовыеЗаписи, ДополнительныеДанные, ЗаписыватьПрисоединенныеФайлы);
...
Функция НастройкиСкладов(УчетнаяЗапись, Знач ИдентификаторыСкладов) Экспорт
	...
	Пока ВыборкаСкладов.Следующий() Цикл
		...
		Если Не ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка20() Тогда
			НастройкиСпособаДоставки = НовыйНастройкиСкладаFBS(Ложь, Истина);
	...
	Для Каждого ИдентификаторСклада Из ИдентификаторыСкладов Цикл   
		Если ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка20() Тогда
			...
			НастройкиСкладов.Вставить(ИдентификаторСклада, НастройкиСклада);
		Иначе   
			НастройкиСклада = НовыйНастройкиСкладаFBS(Ложь);
		КонецЕсли;
	КонецЦикла;
...
#Область ЗначенияТорговойПлощадки

// Конструктор сопоставленного значения торговой площадки.
// 
// Возвращаемое значение:
//  Структура:
//   * Значение - Строка - значение торговой площадки.
//   * Представление - Строка - представление значения торговой площадки.
//
Функция НовоеЗначениеТорговойПлощадки() Экспорт
...
// Добавляет сопоставленное значение и описание значения торговой площадки в переданную коллекцию.
//
// Параметры:
//  Коллекция - Структура - коллекция значений торговой площадки:
//   * Ключ - Строка - сопоставленное значение.
//   * Значение - см. НовоеЗначениеТорговойПлощадки
//  Значение - Строка - сопоставленное значение.
//  ЗначениеТорговойПлощадки - Строка - значение торговой площадки.
//  Представление - Строка - представление значения торговой площадки.
//
Процедура ДобавитьЗначениеТорговойПлощадкиВКоллекцию(Коллекция, Значение, ЗначениеТорговойПлощадки, Представление) Экспорт
	
	ОписаниеЗначения = НовоеЗначениеТорговойПлощадки();
	
	ОписаниеЗначения.Значение = ЗначениеТорговойПлощадки;
	ОписаниеЗначения.Представление = Представление;
	
	Коллекция.Вставить(Значение, ОписаниеЗначения);
	
КонецПроцедуры

// Получает значение торговой площадки из коллекции по ключу. Если значения в коллекции нет - вернется пустая структура.
// Не вызывает исключения, если искомого значения в коллекции нет.
// 
// Параметры:
//  Коллекция - Структура - коллекция значений торговой площадки:
//   * Ключ - Строка - сопоставленное значение.
//   * Значение - см. НовоеЗначениеТорговойПлощадки
//  Значение - Строка - сопоставленное значение, значение торговой площадки которого нужно получить.
// 
// Возвращаемое значение:
//  См. НовоеЗначениеТорговойПлощадки
//
Функция ЗначениеТорговойПлощадкиИзКоллекции(Коллекция, Значение) Экспорт
	
	Результат = НовоеЗначениеТорговойПлощадки();
	
	ПроверяемыеДанные = Новый Структура;
	ПроверяемыеДанные.Вставить(Значение, Неопределено);
	ЗаполнитьЗначенияСвойств(ПроверяемыеДанные, Коллекция);
	
	ЗначениеТорговойПлощадки = ПроверяемыеДанные[Значение]; // см. НовоеЗначениеТорговойПлощадки
	Если ЗначениеТорговойПлощадки = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
	
	ЗаполнитьЗначенияСвойств(Результат, ЗначениеТорговойПлощадки);
	Возврат Результат;
	
КонецФункции

// Готовит данные по переданной коллекции для поиска сопоставленного значения по значению торговой площадки.
//
// Параметры:
//  Коллекция - Структура - коллекция значений торговой площадки:
//   * Ключ - Строка - сопоставленное значение.
...
//   * Ключ - Строка - значение торговой площадки.
//   * Значение - Строка - сопоставленное значение.
//
Функция ДанныеДляПоискаСопоставленныхЗначенийТорговойПлощадки(Коллекция) Экспорт
	
	Результат = Новый Соответствие;
	
	Для Каждого КлючЗначение Из Коллекция Цикл
		ОписаниеЗначенияТорговойПлощадки = КлючЗначение.Значение; // см. НовоеЗначениеТорговойПлощадки
		Результат.Вставить(ОписаниеЗначенияТорговойПлощадки.Значение, КлючЗначение.Ключ);
	КонецЦикла;
...
// Конструктор результата обработки штрихкода.
// 
// Возвращаемое значение:
//  Структура:
//   * ИмяФормы - Строка - имя открываемой формы.
//   * ПараметрыОткрытия - Структура - произвольные параметры открытия указанной формы
//
Функция НовыйРезультатОбработкиШтрихкода() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ИмяФормы", "");
	Результат.Вставить("ПараметрыОткрытия", Новый Структура);
	Возврат Результат;
...
Функция СоздатьПартнера(ДанныеЗаполнения, ВидМаркетплейса = Неопределено, ИспользоватьПривилегированныйРежим = Истина)
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось записать партнера ""%1"" по причине: %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ЗаполнитьДополнительныеСведенияПоТоварам(Заказ, СтрокаЗаказа, ИсточникДанных, ИдентификаторЭкземпляра = 1)
	...
	ВидМаркетплейса = ИнтеграцияСМаркетплейсамиПовтИсп.ВидМаркетплейсаУчетнойЗаписи(Заказ.УчетнаяЗапись);
	...
	Для Индекс = 1 По СтрокаЗаказа.КоличествоУпаковок Цикл
		...
		Если ИнтеграцияСМаркетплейсамиКлиентСервер.ЭтоЯндексМаркет(ВидМаркетплейса) Тогда
			...
			ДопСведения.Вставить("НомерРодительскогоОтправления",    ИсточникДанных.НомерОтправления);
			ДопСведения.Вставить("ИдентификаторОтправления",         ИсточникДанных.ИдентификаторОтправления);
		Иначе  
			ДопСведения.Вставить("ИдентификаторТовара",              ИсточникДанных.ИдентификаторSKU);
			ДопСведения.Вставить("ИдентификаторОбъектаМаркетплейса", "");
			ДопСведения.Вставить("НомерРодительскогоОтправления",    "");
			ДопСведения.Вставить("ИдентификаторОтправления",         "");
		КонецЕсли;
...
Процедура ДополнитьНовыеЗаписиСтатусовПубликацииДополнительнымиДанными(ВидМаркетплейса, НовыеЗаписи,
			ДополнительныеДанные, ЗаписыватьПрисоединенныеФайлы = Ложь)
	
	Если ВидМаркетплейса = Перечисления.ВидыМаркетплейсов.МаркетплейсOzon Тогда
		
		ИнтеграцияСМаркетплейсомOzonСервер.ДополнитьЗаписиСтатусовПубликацииДаннымиПубликации(
			НовыеЗаписи, ДополнительныеДанные, ЗаписыватьПрисоединенныеФайлы);
...
#Область ОбновлениеИнформационнойБазыСлужебный

// Добавляет элемент блокировки регистра по всем измерениям.
// 
// Параметры:
...
//                 - ТаблицаЗначений
//
Процедура ДобавитьБлокировкуРегистраПоВсемИзмерениям(МетаданныеРегистра, Блокировка, ИсточникДанных) Экспорт
...
// Определяет дополнительные параметры для отметки данных к обработке при переходе на новую версию.
// 
// Параметры:
//  ПолноеИмяРегистра - Строка - полное имя обрабатываемого регистра.
// 
// Возвращаемое значение:
//  См. ОбновлениеИнформационнойБазы.ДополнительныеПараметрыОтметкиОбработки
//
Функция ДополнительныеПараметрыОтметкиОбработкиПриПереходеНаНовуюВерсию(ПолноеИмяРегистра) Экспорт
...
// Регистрирует данные регистра к обработке при переходе на новую версию.
// 
// Параметры:
//  Параметры - см. ОбновлениеИнформационнойБазы.ОсновныеПараметрыОтметкиКОбработке
//  МетаданныеРегистра - ОбъектМетаданныхРегистрСведений - метаданные обрабатываемого регистра.
//  РезультатЗапроса - РезультатЗапроса - результат запроса, содержащий данные, которые необходимо отметить к обработке.
//
Процедура ОтметитьКОбработкеПриПереходеНаНовуюВерсию(Параметры, МетаданныеРегистра, РезультатЗапроса) Экспорт
	...
	ДополнительныеПараметры = ДополнительныеПараметрыОтметкиОбработкиПриПереходеНаНовуюВерсию(ПолноеИмяРегистра);
	
	ВключаемыеПоля = ВключаемыеПоляДляЗапросаЗначенийПолейОбъектаМетаданных(Ложь);
	ВключаемыеПоля.ВключатьИзмерения = Истина;
	
	ТаблицаРегистра = ПустаяТаблицаОбъектаМетаданных(ПолноеИмяРегистра, Истина, ВключаемыеПоля);
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			МетаданныеРегистра.Представление());
