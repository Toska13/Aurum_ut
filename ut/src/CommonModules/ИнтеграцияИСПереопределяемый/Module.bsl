...
Процедура ПередЗаписьюОбъекта(Объект, Отказ, РежимЗаписи = Неопределено, РежимПроведения = Неопределено) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("СправочникОбъект.ХозяйствующиеСубъектыВЕТИС")
		И ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов")
		И ТипЗнч(Объект.Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
		Объект.ЕстьНесопоставленные = Ложь;
	КонецЕсли;
	//-- НЕ ГОСИС
...
Процедура ЗаполнитьСлужебныеРеквизитыВКоллекции(Форма, ТабличнаяЧасть, ПараметрыСканирования = Неопределено) Экспорт
	...
	Если КолонкиРеквизитов.Найти("МаркируемаяПродукция")<> Неопределено Тогда
		ИмяОбработчика = "ЗаполнитьПризнакМаркируемаяПродукция";
		Если ПараметрыСканирования<>Неопределено Тогда
			Если ПараметрыСканирования.ЕГАИС И Не ПараметрыСканирования.ИСМП Тогда
				ИмяОбработчика = "ЗаполнитьПризнакМаркируемаяПродукцияЕГАИС";
			ИначеЕсли ПараметрыСканирования.ИСМП И Не ПараметрыСканирования.ЕГАИС Тогда
				ИмяОбработчика = "ЗаполнитьПризнакМаркируемаяПродукцияГИСМТ";
			КонецЕсли;
		КонецЕсли;
		ПараметрыЗаполненияРеквизитов.Вставить(ИмяОбработчика,
...
Процедура ОбработатьСтрокуТабличнойЧасти(СтрокаТабличнойЧасти, СтруктураДействий, ДокументОбъект) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТабличнойЧастиСтруктурой, "СтавкаНДС") Тогда
		...
		Если СтрокаТабличнойЧастиСтруктурой.СтавкаНДС = Неопределено Тогда
			СтрокаТабличнойЧастиСтруктурой.СтавкаНДС = Справочники.СтавкиНДС.ПустаяСсылка();
		КонецЕсли;
	КонецЕсли;
...
// В данной процедуре требуется переопределить текст запроса, определяющий сопоставление классификатора ОКПД2 и данных регистра сведений КодыОКПД2ИСМП.
//   Требования к тексту запроса:
//     Если классификатора ОКПД2 не используется, переопределение также не заполнять.
//     Результат запроса обязательно должен содержать следующие поля:
//   Колонки:
//     КодОКПД2           - Строка - код.
//     ВидПродукции       - СправочникСсылка.ВидыПродукцииИС - вид продукции.
//     НаименованиеПолное - Строка - наименование.
//     КодОКПД2Ссылка     - Произвольный - ссылка на сопоставленный элемент классификатора.
//     Сопоставлено       - Число - 0, если в базе не найден соотвествующий элемент классификатора, 1 - если найден.
// Параметры:
//  ТекстЗапроса - Строка - Переопределяемый текст запроса.
Процедура ПриОпределенииТекстаЗапросаСопоставлениеКодовОКПД2(ТекстЗапроса) Экспорт
	...
	|	ВЫБОР
	|		КОГДА КлассификаторОКПД2.Ссылка ЕСТЬ NULL
	|		ТОГДА Неопределено
	|		ИНАЧЕ КлассификаторОКПД2.Ссылка
	|	КОНЕЦ КАК КодОКПД2Ссылка,
	|	ВЫБОР
	|		КОГДА КлассификаторОКПД2.Ссылка ЕСТЬ NULL
	|		ТОГДА 0
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК Сопоставлено
	|ИЗ
	|	РегистрСведений.КодыОКПД2ИСМП КАК КодыОКПД2ИСМП
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлассификаторОКПД2 КАК КлассификаторОКПД2
	|	ПО КодыОКПД2ИСМП.КодОКПД2 = КлассификаторОКПД2.Код";
	
КонецПроцедуры

// В данной процедуре требуется переопределить текст запроса, возвращающий номенклатуру, ТН ВЭД и ОКПД2.
//   Требования к тексту запроса:
//     В результате запроса должна присутствовать только номенклатура у которой заполнен ТН ВЭД или ОКПД2.
//     Результат запроса обязательно должен содержать следующие поля:
//   Колонки:
//     Ссылка - ОпределяемыйТип.Номенклатура - ссылка на номенклатуру.
//     ТНВЭД  - Строка - код ТН ВЭД номенклатуры.
//     ОКПД2  - Строка - код ОКПД2 номенклатуры.
// Параметры:
//  ТекстЗапроса - Строка - Переопределяемый текст запроса.
//  ИмяВременнойТаблицы - Строка - Имя временной таблицы в которую необходимо поместить данные запроса.
//  ИспользоватьОтборНоменклатуры - Булево -
//    Если Истина, тогда в результате запроса должна присутствовать только номенклатура, которая содержится
//    во временной таблице "ТаблицаОтбораНоменклатуры", в колонке "Ссылка".
//    Если Ложь, тогда временная таблица для отбора номенклатуры, в запросе отсутствует.
Процедура ПриОпределенииТекстаЗапросаТНВЭДиОКПД2Номенклатуры(ТекстЗапроса, ИмяВременнойТаблицы, ИспользоватьОтборНоменклатуры) Экспорт
	...
	Если ИспользоватьОтборНоменклатуры Тогда
		...
		|ПО Номенклатура.Ссылка = ТаблицаОтбораНоменклатуры.Ссылка";
	Иначе
		ТекстОтбораНоменклатуры = "";
	КонецЕсли;
	ТекстЗапроса = СтрШаблон(
	...
	|	Номенклатура.КодТНВЭД.Код  КАК ТНВЭД,
	|	Номенклатура.КодОКПД2.Код  КАК ОКПД2
	|ПОМЕСТИТЬ %1
	|ИЗ
	|	Справочник.Номенклатура КАК Номенклатура
	|	%2
	...
	|	ИЛИ Номенклатура.КодОКПД2 <> ЗНАЧЕНИЕ(Справочник.КлассификаторОКПД2.ПустаяСсылка)",
	ИмяВременнойТаблицы,
	ТекстОтбораНоменклатуры);
	
КонецПроцедуры

// В данной процедуре требуется переопределить текст запроса, определяющий получение описания номенклатуры.
// Текст запроса должен содержать поля ОбщегоНазначенияИС.ИнициализироватьДанныеОписанияНоменклатуры.
// Параметры:
//  ТекстЗапроса - Строка - Переопределяемый текст запроса.
Процедура ПриОпределенииТекстаЗапросаОписаниеНоменклатуры(ТекстЗапроса) Экспорт
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ОписаниеНоменклатурыИС.Номенклатура КАК Номенклатура,
	|	ЕСТЬNULL(ОписаниеНабораУпаковок.ЕмкостьПотребительскойУпаковки, ОписаниеНоменклатурыИС.ЕмкостьПотребительскойУпаковки) КАК ЕмкостьПотребительскойУпаковки,
	|	ЕСТЬNULL(ОписаниеНабораУпаковок.КоличествоВПотребительскойУпаковке, ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке) КАК КоличествоВПотребительскойУпаковке,
	|	ЕСТЬNULL(ОписаниеНабораУпаковок.ПотребительскаяУпаковка, ОписаниеНоменклатурыИС.ПотребительскаяУпаковка) КАК ПотребительскаяУпаковка,
	|	ЕСТЬNULL(ОписаниеНабораУпаковок.ВариантЧастичногоВыбытия, ОписаниеНоменклатурыИС.ВариантЧастичногоВыбытия) КАК ВариантЧастичногоВыбытия,
	|	ЕСТЬNULL(ОписаниеНабораУпаковок.ВариантИспользованияЕдиницыХранения, ОписаниеНоменклатурыИС.ВариантИспользованияЕдиницыХранения) КАК ВариантИспользованияЕдиницыХранения,
	|	ЕСТЬNULL(ОписаниеНабораУпаковок.УпаковкаЧастичногоВыбытия, ОписаниеНоменклатурыИС.УпаковкаЧастичногоВыбытия) КАК УпаковкаЧастичногоВыбытия,
	|	НЕ НастройкиЧастичногоВыбытияПродукцииИС.НоменклатураЧастичногоВыбытия ЕСТЬ NULL
	|		ИЛИ ЕСТЬNULL(ОписаниеНабораУпаковок.КоличествоВПотребительскойУпаковке, ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке) <> 1
	|		ИЛИ ЕСТЬNULL(ОписаниеНабораУпаковок.ЕмкостьПотребительскойУпаковки, ОписаниеНоменклатурыИС.ЕмкостьПотребительскойУпаковки) > 0 КАК ПроизвольнаяЕдиницаУчета,
	|	ЕСТЬNULL(ОписаниеНабораУпаковок.КоличествоВПотребительскойУпаковке, ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке) = 0 КАК ТребуетВзвешивания,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНабораУпаковок
	|		ПО ОписаниеНоменклатурыИС.Номенклатура.ИспользоватьУпаковки
	|			И ОписаниеНоменклатурыИС.Номенклатура.НаборУпаковок = ОписаниеНабораУпаковок.Номенклатура
...
Процедура ЗаполнитьИменаРеквизитовДляЗаполненияПараметровУказанияСерий(ИменаРеквизитов, ТипОбъекта) Экспорт
	...
	ИначеЕсли ТипОбъекта = Метаданные.Документы.ПеремещениеМеждуМОДИСМП Тогда
		ИменаРеквизитов = "Отправитель,Получатель,ОтправительОбъект,ПолучательОбъект,Дата";
	КонецЕсли;
	//-- НЕ ГОСИС
	Возврат;
...
Процедура ЗаполнитьПараметрыУказанияСерий(Параметры, ТипОбъекта, Объект) Экспорт
	...
	ИначеЕсли ТипОбъекта = Метаданные.Обработки.ПроверкаИПодборПродукцииИС Тогда
	...
	ИначеЕсли ТипОбъекта = Метаданные.ОбщиеФормы.ФормаУточненияДанныхИС
		Или ТипОбъекта = Метаданные.Обработки.ПроверкаИПодборПродукцииИС.Формы.УточнениеПодобраннойПродукции Тогда
...
Процедура ЗаполнитьТекстЗапросаЗаполненияСтатусовУказанияСерий(ТекстЗапроса, ТипОбъекта, ПараметрыУказанияСерий) Экспорт
	...
	ИначеЕсли ТипОбъекта = Метаданные.Обработки.ВиртуальнаяАгрегацияУпаковокИСМП
		ИЛИ ТипОбъекта = Метаданные.Обработки.ПроверкаИПодборПродукцииИС Тогда
...
Процедура ПриГенерацииСерий(Объект, Товары, ВыделенныеСтроки, ПараметрыУказанияСерий, РезультатЗаполненияСерий) Экспорт
	...
	ТаблицаДанныхДляГенерацииСерий.Колонки.Добавить("МРЦ", РаботаСКурсамиВалют.ОписаниеТипаДенежногоПоля());
	ТаблицаДанныхДляГенерацииСерий.Колонки.Добавить("ГоденДо", ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя));
	ТаблицаДанныхДляГенерацииСерий.Колонки.Добавить("Справка2", Новый ОписаниеТипов("СправочникСсылка.Справки2ЕГАИС"));
	ТаблицаДанныхДляГенерацииСерий.Колонки.Добавить("ДатаПроизводства", ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя));
	
	ДанныеДляГенерации = ?(ВыделенныеСтроки = Неопределено, Товары, ВыделенныеСтроки);
	...
	|ВЫРАЗИТЬ(Таблица.Справка2 КАК Справочник.Справки2ЕГАИС) КАК Справка2,
	...
	|			ТОГДА ВтТовары.ГоденДо
	|			ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	|	КОНЕЦ КАК ГоденДо,
	|	ВЫБОР
	|		КОГДА ВтВидыНоменклатуры.ИспользоватьДатуПроизводстваСерии = ИСТИНА
	|			ТОГДА ВтТовары.ДатаПроизводства
	|			ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	|	КОНЕЦ КАК ДатаПроизводства,
	|	ВЫБОР
	|		КОГДА ВтВидыНоменклатуры.ИспользоватьПроизводителяЕГАИССерии = ИСТИНА
	|			ТОГДА КлассификаторОрганизацийЕГАИС.Ссылка
	|			ИНАЧЕ ЗНАЧЕНИЕ(Справочник.КлассификаторОрганизацийЕГАИС.ПустаяСсылка)
	|	КОНЕЦ КАК ПроизводительЕГАИС,
	|	ВЫБОР
	|		КОГДА ВтВидыНоменклатуры.ИспользоватьПроизводителяЕГАИССерии = ИСТИНА
	|			ТОГДА ВтТовары.Справка2
	|			ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Справки2ЕГАИС.ПустаяСсылка)
	|	КОНЕЦ КАК Справка2ЕГАИСПроизводительЕГАИС,
	|	ВЫБОР
	|		КОГДА ВтВидыНоменклатуры.ИспользоватьСправку2ЕГАИССерии = ИСТИНА
	|			ТОГДА ВтТовары.Справка2
	|			ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Справки2ЕГАИС.ПустаяСсылка)
	|	КОНЕЦ КАК Справка2ЕГАИС,
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА ВтТовары.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	...
	|	КОНЕЦ) КАК Серия,
	|	МАКСИМУМ(ВтВидыНоменклатуры.ВидНоменклатуры)                КАК ВидНоменклатуры,
	|	МАКСИМУМ(ВтВидыНоменклатуры.ИспользоватьСерии)              КАК ИспользоватьСерии,
	|	МАКСИМУМ(ВтВидыНоменклатуры.АвтоматическиГенерироватьСерии) КАК АвтоматическиГенерироватьСерии,
	|	МАКСИМУМ(ВтВидыНоменклатуры.ИспользоватьМРЦМОТПСерии) КАК ИспользоватьМРЦМОТПСерии,
	|	МАКСИМУМ(ВтВидыНоменклатуры.ИспользоватьСрокГодностиСерии) КАК ИспользоватьСрокГодностиСерии,
	|	МАКСИМУМ(ВтВидыНоменклатуры.ИспользоватьДатуПроизводстваСерии) КАК ИспользоватьДатуПроизводстваСерии,
	|	МАКСИМУМ(ВтВидыНоменклатуры.ИспользоватьСправку2ЕГАИССерии) КАК ИспользоватьСправку2ЕГАИССерии,
	|	МАКСИМУМ(ВтВидыНоменклатуры.ИспользоватьПроизводителяЕГАИССерии) КАК ИспользоватьПроизводителяЕГАИССерии,
	|	МАКСИМУМ(ВтТовары.ЕстьОшибка) КАК ЕстьОшибка,
	|	МАКСИМУМ(ВтТовары.ТекстОшибки) КАК ТекстОшибки
	|ИЗ
	|	ВтТовары КАК ВтТовары
	...
	|		ПО ВтТовары.Справка2.АлкогольнаяПродукция.Производитель = КлассификаторОрганизацийЕГАИС.Ссылка
	...
	|			ИНАЧЕ СерииНоменклатуры.МаксимальнаяРозничнаяЦенаМОТП = 0
	|		КОНЕЦ)
	|		И (ВЫБОР
	|			КОГДА ВтВидыНоменклатуры.ИспользоватьСрокГодностиСерии = ИСТИНА
	|				ТОГДА СерииНоменклатуры.ГоденДо = ВтТовары.ГоденДо
	|			ИНАЧЕ СерииНоменклатуры.ГоденДо = ДАТАВРЕМЯ(1, 1, 1)
	|		КОНЕЦ)
	|		И (ВЫБОР
	|			КОГДА ВтВидыНоменклатуры.ИспользоватьДатуПроизводстваСерии = ИСТИНА
	|				ТОГДА СерииНоменклатуры.ДатаПроизводства = ВтТовары.ДатаПроизводства
	|			ИНАЧЕ СерииНоменклатуры.ДатаПроизводства = ДАТАВРЕМЯ(1, 1, 1)
	|		КОНЕЦ)
	|		И (ВЫБОР
	|			КОГДА ВтВидыНоменклатуры.ИспользоватьСправку2ЕГАИССерии = ИСТИНА
	...
	|		И (ВЫБОР
	|			КОГДА ВтВидыНоменклатуры.ИспользоватьПроизводителяЕГАИССерии = ИСТИНА
	|				ТОГДА СерииНоменклатуры.ПроизводительЕГАИС = КлассификаторОрганизацийЕГАИС.Ссылка
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ)
	|СГРУППИРОВАТЬ ПО
	...
	ТаблицаРезультатЗаполненияГенерации = Запрос.Выполнить().Выгрузить();
	
	РезультатЗаполненияСерий.ЗаполнениеЗавершено = Истина;
	...
	//Генерация
	Для Каждого СтрокаТЧ Из ТаблицаРезультатЗаполненияГенерации Цикл
		
		Если Не СтрокаТЧ.ИспользоватьСерии Тогда
			
			СтрокаТЧ.ЕстьОшибка = Истина;
			СтрокаТЧ.ТекстОшибки = СтрШаблон(НСтр("ru = 'Для номенклатуры %1 серии не используются.'"), СтрокаТЧ.НоменклатураНаименование);
			
		ИначеЕсли НЕ СтрокаТЧ.АвтоматическиГенерироватьСерии И Не ЗначениеЗаполнено(СтрокаТЧ.Серия) Тогда
			
			СтрокаТЧ.ЕстьОшибка = Истина;
			СтрокаТЧ.ТекстОшибки = СтрШаблон(НСтр("ru = 'Для номенклатуры %1 не предусмотрена автоматическая генерация серий.'"), СтрокаТЧ.НоменклатураНаименование);
			
		ИначеЕсли СтрокаТЧ.АвтоматическиГенерироватьСерии И Не ЗначениеЗаполнено(СтрокаТЧ.Серия) Тогда
			
			Если Не ПравоДоступа("Добавление", Метаданные.Справочники.СерииНоменклатуры) Тогда
				
				СтрокаТЧ.ЕстьОшибка = Истина;
				СтрокаТЧ.ТекстОшибки = НСтр("ru = 'Генерация серий недоступна пользователю.'");
			...
			Иначе
				...
				Попытка
					...
					НоваяСерия.Заполнить(СтрокаТЧ);
					НоваяСерия.Записать();
					
					СтрокаТЧ.Серия = НоваяСерия.Ссылка;
					
				Исключение
					
					РезультатЗаполненияСерий.ЗаполнениеЗавершено = Ложь;
					СтрокаТЧ.ЕстьОшибка = Истина;
					СтрокаТЧ.ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось сгенерировать серию для номенклатуры %1 по причине: %2'"),
		...
		КлючПоиска.Вставить("Номенклатура", СтрокаТЧ.Номенклатура);
		КлючПоиска.Вставить("Серия", Справочники.СерииНоменклатуры.ПустаяСсылка());
		Если СтрокаТЧ.ИспользоватьМРЦМОТПСерии Тогда
			КлючПоиска.Вставить("МРЦ", СтрокаТЧ.МаксимальнаяРозничнаяЦенаМОТП);
		КонецЕсли;
		Если СтрокаТЧ.ИспользоватьДатуПроизводстваСерии Тогда
			КлючПоиска.Вставить("ДатаПроизводства", СтрокаТЧ.ДатаПроизводства);
		КонецЕсли;
		Если СтрокаТЧ.ИспользоватьСрокГодностиСерии Тогда
			КлючПоиска.Вставить("ГоденДо", СтрокаТЧ.ГоденДо);
		КонецЕсли;
		Если СтрокаТЧ.ИспользоватьСправку2ЕГАИССерии Тогда
			КлючПоиска.Вставить("Справка2", СтрокаТЧ.Справка2ЕГАИС);
		ИначеЕсли СтрокаТЧ.ИспользоватьПроизводителяЕГАИССерии Тогда
			КлючПоиска.Вставить("Справка2", СтрокаТЧ.Справка2ЕГАИСПроизводительЕГАИС);
		КонецЕсли;
		ИсходныеСтроки = ТаблицаДанныхДляГенерацииСерий.НайтиСтроки(КлючПоиска);
		Для Каждого СтрокаДляГенерации Из ИсходныеСтроки Цикл
			ЗаполнитьЗначенияСвойств(СтрокаДляГенерации, СтрокаТЧ, "Серия, ЕстьОшибка, ТекстОшибки");
		КонецЦикла;
		
		Если СтрокаТЧ.ЕстьОшибка Тогда
			Если РезультатЗаполненияСерий.СписокОшибок = Неопределено Тогда
				РезультатЗаполненияСерий.СписокОшибок = Новый Массив;
			КонецЕсли;
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(
				Ошибки, "СгенерироватьСерии", СтрокаТЧ.ТекстОшибки);
		КонецЕсли;
		
	КонецЦикла;
	
	//Подготовка результата
	Для Каждого СтрокаТЧ Из ТаблицаДанныхДляГенерацииСерий Цикл
		СтруктураЗаполненияСтроки = Новый Структура;
		СтруктураЗаполненияСтроки.Вставить("Серия", СтрокаТЧ.Серия);
		Если ЗначениеЗаполнено(СтрокаТЧ.Серия) Тогда
			СтруктураЗаполненияСтроки.Вставить("СтатусУказанияСерий", 2);
		КонецЕсли;
		РезультатЗаполненияСерий.Серии.Добавить(СтруктураЗаполненияСтроки);
	КонецЦикла;
	Если Ошибки.СписокОшибок.Количество() > 0 Тогда 
		РезультатЗаполненияСерий.СписокОшибок = Ошибки;
	КонецЕсли;
...
// При определении использования интеграции с БЭД.
// 
// Параметры:
//  Использовать - Булево - Использовать. По-умолчанию Ложь
Процедура ПриОпределенииИспользованияУпрощеннойИнтеграцииБЭД(Использовать) Экспорт
	
	Возврат;
	
КонецПроцедуры

// При определении функции автоматического создания номенклатуры по данным сервиса 1С-Номенклатура.
// 
// Параметры:
//  Создавать - Булево - Использовать. По-умолчанию Ложь
Процедура ПриОпределенииАвтоматическогоСозданияНоменклатурыПоGTIN(Создавать) Экспорт
