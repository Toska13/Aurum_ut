...
Функция ОбъектМетаданныхРазделенХотьОднимРазделителем(Знач ОбъектМетаданных, Кэш) Экспорт
	...
	Если ОбъектМетаданных.РасширениеКонфигурации() <> Неопределено И Не РаботаВМоделиСервиса.РазделениеВключено() Тогда
		// Собственный объект метаданных при установке расширения в сервисе всегда будет считаться разделенным,
		// т.к. расширение устанавливается с областью действия - разделение данных
...
Процедура КонтрольСсылкиНаНеразделенныеДанныеПриВыгрузке(Контейнер, Знач ПроверяемаяСсылка, Знач ИсходныйОбъект, Знач ИсходныйОбъектМетаданных, Знач ИмяПоля)
	...
	Если ОбщегоНазначенияБТС.ЭтоСсылочныеДанныеПоддерживающиеПредопределенныеЭлементы(ОбъектМетаданных) Тогда
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПроверяемаяСсылка, "Предопределенный") Тогда //@skip-check bsl-legacy-check-string-literal
	...
	ШаблонОшибки =
		НСтр("ru = 'Объект метаданных %1 включен в перечень объектов, для которых не требуется сопоставление ссылок при выгрузке / загрузке
              ...
              |%2,
              |но при этом для него не обеспечивается требования отсутствия несопоставимых ссылок при выгрузке.
              |
              |Несопоставимая ссылка обнаружена при выгрузке объекта %3, у которого в качестве значения реквизита %4
              |установлена ссылка на объект %1, которая не сможет быть корректно сопоставлена при загрузке данных.
              |Требуется пересмотреть логику использования объекта %1 и обеспечить для него отсутствие несопоставимых ссылок
              ...
              |%5
              |---------------------------------------------------------------------------------------------------------------------------
              |2. Сериализация объекта несопоставимой ссылки
              |---------------------------------------------------------------------------------------------------------------------------
              |%6
              |---------------------------------------------------------------------------------------------------------------------------'");
