...
Процедура ПриПолученииПредставления(ТипДанных, Данные, Представление, СтандартнаяОбработка) Экспорт
	...
	Если ТипДанных = Тип("ДокументСсылка.ЗаявлениеОВвозеТоваров") Тогда 
		
		Если (Данные.СуммаДокумента <> 0) И (Данные.СуммаДокумента <> NULL) Тогда
			Представление = Данные.Представление
				+ " " + НСтр("ru = 'на сумму'")
				+ " " + Данные.СуммаДокумента
				+ " " + ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Данные.Ссылка, "Организация.ВалютаРегламентированногоУчета");
