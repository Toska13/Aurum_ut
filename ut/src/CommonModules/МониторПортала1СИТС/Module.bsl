...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВключитьПолучениеУведомленийМонитораПортала");
	Методы.Вставить("ПроверитьНаличиеНовойИнформацииВФоне", Истина);
	Методы.Вставить("ПроверитьНаличиеУведомлений", Истина);
...
Функция ОтпечатокДанныхМонитора(Логин, ДанныеМонитора)
	...
	Исключение
		ДанныеСтрока = Логин + ",ОшибкаФормированияОтпечаткаДанных";
	КонецПопытки;;
...
Функция ПроверитьНаличиеУведомлений() Экспорт
	...
	ИначеЕсли РезультатОперации.ИмяОшибки = "НеЗаполненыДанныеАутентификации" Тогда
		Возврат "НетПредупреждений";
	ИначеЕсли РезультатОперации.ИмяОшибки = ИмяОшибкиНетИзменений() Тогда
		ОбновитьНастройкиПолученияУведомленийПортала(
			ДатаОбновления);
		Возврат "НетПредупреждений";
	Иначе
...
Процедура ВызватьОперациюДанныеМонитора(
	...
	Попытка
		...
		Если Результат.Данные.authResult.blockStatus = "429"
			Или Результат.Данные.authResult.blockStatus = 429 Тогда
		...
		ИначеЕсли (Результат.Данные.authResult.blockStatus <> "200"
				И Результат.Данные.authResult.blockStatus <> 200) Тогда
			
			Результат.ИмяОшибки         = ИмяОшибкиВнутренняяОшибкаСервиса();
...
Функция УведомленияПортала1СИТС(
	...
	Если РезультатЗагрузкиСодержимого.КодСостояния = 304 Тогда
		Результат.ИмяОшибки = ИмяОшибкиНетИзменений();
		Возврат Результат;
	КонецЕсли;
...
Процедура ПриОшибкеВызоваОперацииСервиса(РезультатЗагрузкиСодержимого, URLОперации, Результат)
	...
	Если РезультатЗагрузкиСодержимого.КодОшибки = "ConnectError" Тогда
		...
		Результат.ИмяОшибки = ИмяОшибкиОшибкаСоединения();
		Результат.СообщениеОбОшибке =
	...
	ИначеЕсли РезультатЗагрузкиСодержимого.КодСостояния < 0 Тогда
		
		Результат.ИмяОшибки         = ИмяОшибкиЗапрещенДоступКИнтернетСервисам();
...
Процедура ПроверитьДоступностьСервиса(Домен, РезультатОперации)
	...
	ИначеЕсли РезультатПроверки.ИмяОшибки = "ConnectError" Тогда
		...
		РезультатОперации.ИмяОшибки = ИмяОшибкиОшибкаСоединения();
	...
	ИначеЕсли РезультатПроверки.ИмяОшибки = "ServerError" Тогда
		...
		РезультатОперации.ИмяОшибки = ИмяОшибкиСервисВременноНедоступен();
		РезультатОперации.СообщениеОбОшибке = РезультатПроверки.СообщениеОбОшибке;
		
	ИначеЕсли РезультатПроверки.ИмяОшибки = "LockError" Тогда
		
		РезультатОперации.ИмяОшибки = ИмяОшибкиЗапрещенДоступКИнтернетСервисам();
		РезультатОперации.СообщениеОбОшибке = РезультатПроверки.СообщениеОбОшибке;
...
// Возвращает имя ошибки "ВнутренняяОшибкаСервиса".
//
// Возвращаемое значение:
//  Строка - имя ошибки.
//
Функция ИмяОшибкиВнутренняяОшибкаСервиса()
	Возврат "ВнутренняяОшибкаСервиса";
КонецФункции

// Возвращает имя ошибки "СервисВременноНедоступен".
//
// Возвращаемое значение:
//  Строка - имя ошибки.
//
Функция ИмяОшибкиСервисВременноНедоступен()
	Возврат "СервисВременноНедоступен";
КонецФункции

// Возвращает имя ошибки "ОшибкаСоединения".
//
// Возвращаемое значение:
//  Строка - имя ошибки.
//
Функция ИмяОшибкиОшибкаСоединения()
	Возврат "ОшибкаСоединения";
КонецФункции

// Возвращает имя ошибки "НетИзменений".
//
// Возвращаемое значение:
//  Строка - имя ошибки.
//
Функция ИмяОшибкиНетИзменений()
	Возврат "НетИзменений";
КонецФункции

// Возвращает имя ошибки "ЗапрещенДоступКИнтернетСервисам".
//
// Возвращаемое значение:
//  Строка - имя ошибки.
//
Функция ИмяОшибкиЗапрещенДоступКИнтернетСервисам()
	Возврат "ЗапрещенДоступКИнтернетСервисам";
КонецФункции
