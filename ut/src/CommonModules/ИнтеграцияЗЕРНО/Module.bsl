...
//Возвращаемое значение:
//   Массив Из ПеречислениеСсылка.РазделыУчетаЗЕРНО - виды продукции ЗЕРНО по переданной номенклатуре
//
Функция РазделыУчетаПоТаблицеНоменклатуры(Товары) Экспорт
	...
	|/////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО КАК ВидПродукции
	|ПОМЕСТИТЬ ВидыПродукции
	|ИЗ
	|	РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|ГДЕ
	|	ОписаниеНоменклатурыИС.Номенклатура В (ВЫБРАТЬ Товары.Номенклатура ИЗ Товары КАК Товары)
	|;
	|
	|/////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЗначенияЭлементов.Значение КАК РазделУчета
	|ИЗ
	|	РегистрСведений.ЗначенияЭлементовКонструктораВидовПродукцииИС КАК ЗначенияЭлементов
	|ГДЕ
	|	ЗначенияЭлементов.Значение ССЫЛКА Перечисление.РазделыУчетаЗЕРНО
	|	И ЗначенияЭлементов.ВидПродукции В (ВЫБРАТЬ ВидыПродукции.ВидПродукции ИЗ ВидыПродукции КАК ВидыПродукции)";
	
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("РазделУчета");
...
Функция ИнициализиоватьОчередьСообщений()
	...
	ОчередьСообщений.Колонки.Добавить("ВидПродукции"); // Устарело
	ОчередьСообщений.Колонки.Добавить("РазделУчета");
...
Функция СостояниеОбмена(Организации) Экспорт
	...
	|	КлючиПартийЗЕРНО.РазделУчета КАК РазделУчета
	...
	|		И ВладельцыПартий.РазделУчета = Значение(Перечисление.РазделыУчетаЗЕРНО.Зерно)
	|",
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВладельцыПартий КАК ВладельцыПартий
	|		ПО СоответствиеКлючейОрганизацийИОрганизаций.Организация = ВладельцыПартий.Организация
	|		И ВладельцыПартий.РазделУчета = Значение(Перечисление.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна)
	|",
...
Функция ХэшиНастроекРегламентныхЗаданий()
	...
	ПараметрыНастройкиОбмена = Новый Структура("РазделУчета, ТипОрганизации", Перечисления.РазделыУчетаЗЕРНО.Зерно, 0);
	...
	ПараметрыНастройкиОбмена = Новый Структура("РазделУчета, ТипОрганизации", Перечисления.РазделыУчетаЗЕРНО.Зерно, 1);
	...
	ПараметрыНастройкиОбмена = Новый Структура("РазделУчета, Операция", Перечисления.РазделыУчетаЗЕРНО.Зерно, Перечисления.ВидыОперацийЗЕРНО.ЗапросПартий);
	...
	ПараметрыНастройкиОбмена = Новый Структура("РазделУчета, Операция", Перечисления.РазделыУчетаЗЕРНО.Зерно, Перечисления.ВидыОперацийЗЕРНО.ЗапросПартийНаХранении);
	...
	ПараметрыНастройкиОбмена = Новый Структура("РазделУчета", Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна);
...
Функция ДанныеВыбораОКПД2(Текст, РазделУчета) Экспорт
	...
	|	И КлассификаторНСИЗЕРНО.РазделУчета В (&РазделУчета)
	...
	Если Не ЗначениеЗаполнено(РазделУчета) Тогда
		ПараметрРазделУчета = Новый Массив;
		ПараметрРазделУчета.Добавить(Перечисления.РазделыУчетаЗЕРНО.Зерно);
		ПараметрРазделУчета.Добавить(Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна);
	Иначе
		ПараметрРазделУчета = РазделУчета;
	КонецЕсли;
	
	Запрос.УстановитьПараметр("РазделУчета", ПараметрРазделУчета);
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеВыбора.Добавить(Новый Структура("Код, Представление", Выборка.Идентификатор, Представление));
	КонецЦикла;
...
Функция ДанныеВыбораКодТНВЭД(Текст, КодыОКПД2, Знач ДанныеКэша = Неопределено) Экспорт
	...
	Пока ДанныеТНВЭД.Следующий() Цикл
		...
		ДанныеВыбора.Добавить(Новый Структура("КодТНВЭД, Представление", ДанныеТНВЭД.КодТНВЭД, Представление));
...
Функция ПроблемыКлассификаторовВПанелиОбмена(РасширенноеОписаниеТребуетсяЗагрузка = Ложь) Экспорт
	...
	РазделениеВключено             = ОбщегоНазначения.РазделениеВключено();
	ИдентификаторыКлассификаторов  = ИдентификаторыКлассификаторов();
	ИдентификаторыБГосИС           = ОбщегоНазначенияИС.ИдентификаторыКлассификатораВидовПродукцииИС();
	ИдентификаторыДляЗагрузки      = Новый Массив();
	ВидыКлассификаторовКОбновлению = Новый Массив();
	
	Для каждого Идентификатор Из ИдентификаторыБГосИС Цикл
		ИдентификаторыКлассификаторов.Вставить(Идентификатор.Ключ, Идентификатор.Значение);
	КонецЦикла;
	...
	Если РазделениеВключено Тогда
		...
		Если Не ЗначениеЗаполнено(ИдентификаторКлассификатора) Тогда
			ДоступныеКлассификаторыДляЗагрузки.Добавить("Классификаторы ФГИС ""Зерно""");
		КонецЕсли;
		
		ТекстыЗапроса  = Новый СписокЗначений;
		
		Для каждого КлючИЗначение Из ИдентификаторыБГосИС Цикл
	...
	Иначе
		...
		Иначе
			...
			Для Каждого СтрокаТаблицы Из ДоступныеОбновления.ДоступныеВерсии Цикл
				...
				ВидКлассификатора = ИдентификаторыКлассификаторов[СтрокаТаблицы.Идентификатор].ВидКлассификатора;
				Если ВидКлассификатора <> Неопределено Тогда
					ВидыКлассификаторовКОбновлению.Добавить(ВидКлассификатора);
				КонецЕсли;
...
Процедура ПартияЗЕРНОПриЗаписиПриЗаписиОбработчик(Источник, Отказ) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	КлассификаторНСИЗЕРНО.РазделУчета КАК РазделУчета
	|ИЗ
	|	Справочник.КлассификаторНСИЗЕРНО КАК КлассификаторНСИЗЕРНО
	|ГДЕ
	...
	Запись.РазделУчета = Выборка.РазделУчета;
