...
// -- Локализация
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыБПО.УстановитьВерсиюФорматаОбменаККМED";
	Обработчик.Версия = "3.2.9.6";
	Обработчик.РежимВыполнения = "Оперативно";
...
Процедура УстановитьВерсиюФорматаОбменаККМED(Параметры = Неопределено) Экспорт
	
	Если ОбщегоНазначенияБПО.ИспользуетсяОфлайнОборудование() Тогда
		МодульМенеджерОфлайнОборудования = ОбщегоНазначенияБПО.ОбщийМодуль("МенеджерОфлайнОборудования");
		МодульМенеджерОфлайнОборудования.УстановитьВерсиюФорматаОбменаККМED(Параметры);
	КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьПараметрыРегистрацииОборудования", Истина);
	Методы.Вставить("ОтключитьИспользованиеРегламентногоЗадания", Истина);
	Методы.Вставить("ОбновитьМакетыЭтикеток", Истина);
	Методы.Вставить("СоздатьВидыКонтактнойИнформации", Истина);
	Методы.Вставить("ОбновитьШаблоныЭтикетокИЦенников", Истина);
	Методы.Вставить("ОбновитьПодключаемоеОборудование", Истина);
	Методы.Вставить("ОбновитьПоставляемыеДрайвера", Истина);
	Методы.Вставить("УстановитьСрокХраненияОперацийСОборудованием", Истина);
	Методы.Вставить("УстановитьСрокХраненияОпераций", Истина);
