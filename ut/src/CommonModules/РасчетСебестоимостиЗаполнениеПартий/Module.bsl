...
Функция ОписаниеДвиженийФактуровкаПоставок(ПараметрыРасчета)
	...
	ОписаниеДвижений.Вставить("ЕстьРазделУчета", Истина);
...
Функция ТекстТекущиеНеотфактурованныеПоставки() // использует ИсправленныеДокументы
	Возврат 
		...
		|	Т.Период КАК Период,
		|	ВЫБОР
		|		КОГДА НЕ Исправления.ДатаСторнируемогоДокумента ЕСТЬ NULL
		|			ТОГДА Исправления.ДатаСторнируемогоДокумента
		...
		|		КОГДА ТИПЗНАЧЕНИЯ(ПоступлениеУслуг.Распоряжение) = ТИП(Документ.ЗаказПоставщику)
		|			ТОГДА ПоступлениеУслуг.Распоряжение
		|		ИНАЧЕ ЕСТЬNULL(ИсправленныеДокументы.СторнируемыйДокумент, НЕОПРЕДЕЛЕНО) КОНЕЦ) КАК ПриоритетныйИсточник,
...
Функция ТекстТекущиеФактуровкиПоставок()
	ТекстЗапроса = 
		...
		|		 И ЕСТЬNULL(Т.КорАналитикаУчетаНоменклатуры.Назначение.ВидДеятельностиНДС, ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка))
		|				<> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)
		|				И НЕ Т.РазделУчета В (ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыВПути),
		...
		|		ПО Т.Организация = НастройкиУчетаНДСПриУСН.Организация
		...
		|";
	//++ Локализация
	СтрокаЗамены = "ЕСТЬNULL(НастройкиУчетаНДСПриУСН.ОсновноеНалогообложение, УчетныеПолитикиОрганизаций.НалогообложениеНДСПоУмолчанию)";
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "УчетныеПолитикиОрганизаций.НалогообложениеНДСПоУмолчанию", СтрокаЗамены);
	//-- Локализация
	Возврат ТекстЗапроса;
КонецФункции
...
Функция ТекстТекущиеКорректировкиФактуровок()
	Возврат 
		...
		|	ДД.ПериодОтклонения КАК ПериодОтклонения,
		|	ДД.Регистратор,
...
Функция ТекстСторноДвиженияИсправленныхДокументовФактуровкиПоставок() // использует ИсправленныеДокументы
	...
	Возврат " 
		...
		|	ДанныеРегистра.ПериодОтклонения                      КАК ПериодОтклонения,
		...
		|		ПО ИсправленныеДокументы.СторнируемыйДокумент = ДанныеРегистра.Регистратор
		...
		|	ДанныеРегистра.ПериодОтклонения,
		|	ИсправленныеДокументы.Регистратор,
		|	ДанныеРегистра.АналитикаУчетаНоменклатуры,
		|	ДанныеРегистра.РазделУчета,
		|	ДанныеРегистра.ВидЗапасов,
...
Процедура ЗаполнитьРасчетнуюПартиюФактуровкиПоставок(ПараметрыРасчета, РасчетнаяПартия, Расход, Приход)
	...
	ИначеЕсли РасчетнаяПартия.ТипЗаписи = Перечисления.ТипыЗаписейПартий.КорректировкаПриобретения
	 ...
	 ИЛИ РасчетнаяПартия.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость Тогда
		// ДокументИсточник не меняется, нужно сохранить запись как есть
	...
	ИначеЕсли Расход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.КорректировкаПриобретенияПрошлогоПериода
	 ...
	 ИЛИ Расход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость
	 ИЛИ Расход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ДопРасходыСПартией Тогда
		...
		Если Приход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.СторноДопРасходов
		 ИЛИ Расход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ВключениеНДСВСтоимость
	 	 ИЛИ Расход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость Тогда 
	...
	// кроме уменьшения фактуровки поставки прошлого периода, т.к. такая коректировка возвращает партию поступления товаров 
	// товаров на склад, а отклонения рассчитываются только при списании партии поступления товаров.
	...
	Если (Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки
			И (НЕ Приход.Сторно 
				...
				И Расход.ТипЗаписи <> Перечисления.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость)
		...
		ИЛИ Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаНеотфактурованнаяПоставка
			И НЕ Расход.Сторно)
		И НЕ (Приход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.Прошлое И Расход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.СторноПотребления)
		И Расход.ТипЗаписи <> Перечисления.ТипыЗаписейПартий.КорректировкаПриобретения Тогда
		...
		// По неотфактурованным услугам нужно рассчитывать отклонения по НДС, т.к. сумма НДС используется
		// при расчете детализации партий.
		// См. запрос формирования временной таблицы ВТДопРасходыДляРаспределенияПредварительная в функции ТекстРаспределениеДопРасходов
		// общего модуля РасчетСебестоимостиНДС.
	 	Если НЕ ЕстьОтклонения И ЗначениеЗаполнено(Приход.КорРазделУчета)
	 	 ИЛИ НЕ ЕстьОтклонения И ТипЗнч(Приход.Партия) = Тип("ДокументСсылка.ПоступлениеУслугВПодразделение") Тогда
			Для Каждого ИмяРесурса Из СуммовыеРесурсыНДС Цикл
	...
	Если ЕстьОтклонения Тогда
		...
		Если РасчетнаяПартия.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ДопРасходыСПартией И Отклонения.ДопРасходыРегл <> 0
			И Отклонения.НастройкаХозяйственнойОперации = Справочники.НастройкиХозяйственныхОпераций.ВключениеНДСВСтоимость Тогда
			Отклонения.НастройкаХозяйственнойОперации = Справочники.НастройкиХозяйственныхОпераций.РаспределениеРасходовНаСебестоимость;
		ИначеЕсли РасчетнаяПартия.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ДопРасходыСПартией И Не Приход.НастройкаХозяйственнойОперации.Пустая() Тогда
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(РасчетнаяПартия.ВидЗапасов) Тогда
			...
			Если НЕ ЗначениеЗаполнено(Отклонения.СтатьяРасходовСписания) Тогда
		...
		Отклонения.ПериодОтклонения    = Приход.ПериодПоступления;
		...
		Если ЗначениеЗаполнено(Приход.КорРазделУчета) И НЕ ОтноситьОтклоненияНаПрочиеДоходыРасходы
		 И Отклонения.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ОтклонениеВСтоимости Тогда
			...
			ОтклоненияНаСклад.ПериодОтклонения			 = Приход.ПериодПоступления;
...
Процедура ФактуровкаПоставокПриСреднескользящей(ПараметрыРасчета) Экспорт
	...
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	Фактуровки.АналитикаУчетаНоменклатуры,
	|	Фактуровки.ВидЗапасов,
	|	Фактуровки.Организация,
	|	Фактуровки.Партия КАК Партия,
	|	Фактуровки.АналитикаУчетаПартий КАК АналитикаУчетаПартий,
	|	Фактуровки.АналитикаФинансовогоУчета КАК АналитикаФинансовогоУчета,
	|	Фактуровки.ВидДеятельностиНДС КАК ВидДеятельностиНДС,
	|	МАКСИМУМ(Фактуровки.ИдентификаторФинЗаписи) КАК ИдентификаторФинЗаписи
	|
	|ПОМЕСТИТЬ ФактуровкиПоставокИдентификаторыФинЗаписи
	|ИЗ
	|	РегистрНакопления.СебестоимостьТоваров КАК Фактуровки
	|ГДЕ
	|	Фактуровки.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И Фактуровки.Организация В (&ОрганизацииСреднескользящая)
	|	И Фактуровки.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|	И Фактуровки.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки)
	|	И Фактуровки.РасчетПартий
	|	И Фактуровки.ИдентификаторФинЗаписи <> &ИдентификаторНеиспользуемойФинЗаписи
	|СГРУППИРОВАТЬ ПО
	|	Фактуровки.АналитикаУчетаНоменклатуры,
	|	Фактуровки.ВидЗапасов,
	|	Фактуровки.Организация,
	|	Фактуровки.Партия,
	|	Фактуровки.АналитикаУчетаПартий,
	|	Фактуровки.АналитикаФинансовогоУчета,
	|	Фактуровки.ВидДеятельностиНДС
	|;
	|
	|////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	И Остатки.Организация В (&ОрганизацииСреднескользящая)
	...
	|	Остатки.АналитикаФинансовогоУчета КАК КорАналитикаФинансовогоУчета,
	|	Остатки.ВидДеятельностиНДС КАК КорВидДеятельностиНДС,
	|	ЕСТЬNULL(Идентификаторы.ИдентификаторФинЗаписи, """") КАК ИдентификаторФинЗаписиИсх,
	// Ресурсы.
	|	Остатки.Количество
	...
	|			И НЕ РеестрДокументов.ДополнительнаяЗапись
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ФактуровкиПоставокИдентификаторыФинЗаписи КАК Идентификаторы
	|	ПО Идентификаторы.АналитикаУчетаНоменклатуры = Остатки.АналитикаУчетаНоменклатуры
	|		И Идентификаторы.ВидЗапасов = Остатки.ВидЗапасов
	|		И Идентификаторы.Организация = Остатки.Организация
	|		И Идентификаторы.Партия = Остатки.Партия
	|		И Идентификаторы.АналитикаУчетаПартий = Остатки.АналитикаУчетаПартий
	|		И Идентификаторы.АналитикаФинансовогоУчета = Остатки.АналитикаФинансовогоУчета
	|		И Идентификаторы.ВидДеятельностиНДС = Остатки.ВидДеятельностиНДС
	|ГДЕ
	|	Остатки.Количество < 0
	|	И Остатки.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки)
	|	И (
	...
	РасчетСебестоимостиУниверсальныеАлгоритмы.ИнициализироватьПрочиеПоляРаспределенияМетодомУменьшаемогоОстатка(
		...
		"Партия, АналитикаУчетаПартий, АналитикаФинансовогоУчета, ВидДеятельностиНДС, ИдентификаторФинЗаписиИсх");
...
Функция ФактуровкаПоставокПриСреднескользящейФормированиеДвижений(ПараметрыРасчета)
	...
	|	(ВЫБОР
	|		КОГДА ДД.ИдентификаторФинЗаписиИсх <> """" ТОГДА ДД.ИдентификаторФинЗаписиИсх
	|		ИНАЧЕ ДД.ИдентификаторФинЗаписи КОНЕЦ) КАК ИдентификаторФинЗаписи,
	|	ИСТИНА КАК ЭтоТочноРасчетноеДвижение
	|ИЗ
	|	ВТРезультатРаспределения КАК ДД
	|
...
Функция ОписаниеЦепочекПартийТоваров(ПараметрыРасчета) Экспорт
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеПриемника(ОписаниеЦепочек,
		Перечисления.ТипыЗаписейПартий.ВключениеНДСВСтоимость, ПоляПотреблений + ", ДокументИсточник, Регистратор, ИдентификаторСтроки");
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.ВключениеНДСВСтоимость,
		Перечисления.ТипыЗаписейПартий.ДопРасходыСПартией,     ПоляПотреблений + ", ДокументИсточник, Регистратор, ИдентификаторСтроки");
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.ВключениеНДСВСтоимость,
		Перечисления.ТипыЗаписейПартий.ДопРасходыБезПартии,    ПоляПотреблений + ", ДокументИсточник, Регистратор, ИдентификаторСтроки");
	
	// ВключениеАкцизаВСтоимость <- (ДопРасходыСПартией, ДопРасходыБезПартии, СторноДопРасходов, КорректировкаСтоимости, КорректировкаПриобретенияПрошлогоПериода)
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеПриемника(ОписаниеЦепочек,
		Перечисления.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость, ПоляПотреблений + ", ДокументИсточник, ИдентификаторСтроки");
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость,
		Перечисления.ТипыЗаписейПартий.ДопРасходыСПартией, ПоляПотреблений + ", ДокументИсточник, ИдентификаторСтроки");
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость,
		Перечисления.ТипыЗаписейПартий.ДопРасходыБезПартии, ПоляПотреблений + ", ДокументИсточник, ИдентификаторСтроки");
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость,
		Перечисления.ТипыЗаписейПартий.СторноДопРасходов, 	ПоляПотреблений + ", ДокументИсточник, ИдентификаторСтроки");
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость,
		Перечисления.ТипыЗаписейПартий.КорректировкаСтоимости, ПоляПотреблений + ", ДокументИсточник, ИдентификаторСтроки");
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость,
		Перечисления.ТипыЗаписейПартий.КорректировкаПриобретенияПрошлогоПериода, ПоляПотреблений + ", ДокументИсточник, ИдентификаторСтроки");
	...
	// ПеремещениеНаОстаток <- (Потребление)
	// Тип записи ПеремещениеНаОстаток не имеет приемников, он формирует остаток на конец месяца.
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеПриемника(ОписаниеЦепочек,
		Перечисления.ТипыЗаписейПартий.ПеремещениеНаОстаток, "Регистратор, АналитикаУчетаНоменклатуры, ВидЗапасов, Организация");
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.ПеремещениеНаОстаток,
	...
	// ВозвратПрошлыхПериодов <- (Прошлое, Потребление)
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеПриемника(ОписаниеЦепочек,
		Перечисления.ТипыЗаписейПартий.ВозвратПрошлыхПериодов, "ЗаказКлиента, КорАналитикаУчетаНоменклатуры, КорВидЗапасов, Организация");
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.ВозвратПрошлыхПериодов,
		Перечисления.ТипыЗаписейПартий.Прошлое, 				"ЗаказКлиента, АналитикаУчетаНоменклатуры, ВидЗапасов, Организация");
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.ВозвратПрошлыхПериодов,
		Перечисления.ТипыЗаписейПартий.Потребление, 			"ЗаказКлиента, АналитикаУчетаНоменклатуры, ВидЗапасов, Организация");
	
	// СторноВозвратаПрошлыхПериодов <- (ВозвратПрошлыхПериодов)
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеПриемника(ОписаниеЦепочек,
		Перечисления.ТипыЗаписейПартий.СторноВозвратаПрошлыхПериодов, "ДокументИсточник, АналитикаУчетаНоменклатуры, ВидЗапасов, Организация");
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.СторноВозвратаПрошлыхПериодов,
		Перечисления.ТипыЗаписейПартий.ВозвратПрошлыхПериодов, "Регистратор, АналитикаУчетаНоменклатуры, ВидЗапасов, Организация");
...
Функция ТекстПрошлыеРеализации() // вт ПрошлыеРеализации
	Возврат "
		...
		|	(ВЫБОР
		|		КОГДА ДД.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтчетДавальцуКорректировкаПрошлогоПериода)
		...
		|	ЕСТЬNULL(ДД.КорВидЗапасов.ТипЗапасов, ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПустаяСсылка)) КАК КорТипЗапасов,
...
Функция ТекстИсправленныеДокументы() Экспорт // вт ИсправленныеДокументы
	Возврат "
		...
		|	Реестр.СторнируемыйДокумент    КАК СторнируемыйДокумент,
		|	РеестрИсточник.ДатаДокументаИБ КАК ДатаСторнируемогоДокумента,
		|	Реестр.ДатаДокументаИБ         КАК Период,
		|	Реестр.Ссылка                  КАК Регистратор
		...
		|
		|ГДЕ
		|	Реестр.ДатаДокументаИБ МЕЖДУ &НачалоПериода И &КонецПериода
		|	И Реестр.Организация В (&МассивОрганизаций)
		|	И Реестр.СторноИсправление
		|	И Реестр.Проведен
		|	И НЕ Реестр.ДополнительнаяЗапись
		|
...
Функция ТекстТекущиеПриходыПартийТоваров() // использует ЕстьДекларации, ВозвратныеОтходы, ЕстьУведомленияОВвозе, ЕстьФактуровки
	Возврат 
		...
		//-- Локализация
		|		КОГДА Т.СлужебноеВидДвиженияПриход ТОГДА 10
		|		ИНАЧЕ 90 КОНЕЦ), //	Приоритет
		|	Аналитика.Номенклатура,
		|	ВЫБОР КОГДА Т.Организация В (&ОрганизацииСФИФОСкользящая)
		|		ТОГДА Т.АналитикаУчетаПартий.НалогообложениеНДС
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ, // НалогообложениеПартии
		|
		|	Т.Период,
		|	ВЫБОР
		|		КОГДА ЛОЖЬ ТОГДА Т.Период // для удобства расстановки тегов
		...
		//-- Устарело_Производство21
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ), // ЗаказНаПроизводство
		|	ВЫБОР
		|		КОГДА Т.Организация В (&ОрганизацииСФИФОСкользящая)
		|		 ИЛИ Т.РазделУчета В (
		|				ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.СобственныеТоварыВПути))
		|			ТОГДА Т.АналитикаУчетаПартий
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ, // АналитикаУчетаПартий
		|	ВЫБОР
		|		КОГДА ЛОЖЬ ТОГДА НЕОПРЕДЕЛЕНО // для удобства расстановки тегов

		|		ИНАЧЕ Т.АналитикаФинансовогоУчета
		|	КОНЕЦ, // АналитикаФинансовогоУчета
		|	Т.ВидДеятельностиНДС,
		|
        |	Т.ХозяйственнаяОперация,
		|	(ВЫБОР
		...
		|		КОГДА Т.ВидДеятельностиНДСДокумента <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)
		|			ТОГДА Т.ВидДеятельностиНДСДокумента
		|		ИНАЧЕ Т.ВидДеятельностиНДС КОНЕЦ), // ВидДеятельностиНДСДокумента
		|	Т.ВидДеятельностиНДСОтгрузки,
		|	Т.АналитикаФинансовогоУчетаДокумента,
		|	Т.ОперацияУчетаСебестоимости,
		|	Т.СтатьяКалькуляции,
		|	ЕСТЬNULL(Корректировка.ДокументОснование, Т.ДокументИсточник), // ДокументИсточник
		|	(ВЫБОР
		// Для исправительного документа приоритетным источником является сам исправительный документ. 
		|		КОГДА Т.РазделУчета В (
		|			ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.СобственныеТоварыВПути))
		...
		|		 И ПриобретениеВидыЗапасов.ЗаказПоставщику <> ЗНАЧЕНИЕ(Документ.ЗаказПоставщику.ПустаяСсылка)
		|			ТОГДА ПриобретениеВидыЗапасов.ЗаказПоставщику
		|		ИНАЧЕ ЕСТЬNULL(ИсправленныеДокументы.СторнируемыйДокумент, НЕОПРЕДЕЛЕНО) КОНЕЦ), // ПриоритетныйИсточник
		|	Т.КорНаправлениеДеятельности,
		|	Т.КодСтроки,
		|	Т.НастройкаХозяйственнойОперации,
		|	Т.НастройкаСчетовУчета,
...
Функция ТекстОстатокСгруппированный()
	Возврат 
		...
		|	МИНИМУМ(ВЫБОР
		// Возврат прошлых периодов должен быть отработан в самом начале, поэтому период поступления оставляем пустым.
		...
		// Исключаем документы, для которых введено сторно или исправление, т.к. такие документы являются источниками для сторно записей.
...
Функция ТекстТекущиеОборотыПартийТоваров() // использует ВозвратныеОтходы
	ТекстЗапроса =
		...
		|			ТОГДА КОНЕЦПЕРИОДА(Т.Период, ДЕНЬ)
		|		КОГДА Т.РазделУчета В (ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыВПути),
		|				ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаКомиссиюВПути),
		|				ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПереданныеНаКомиссиюВПути))
		|			И Т.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности),
		...
		|				И Т.Организация В (&ОрганизацииСФИФОСкользящая))
		|		ИЛИ Т.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Дополнение)
		|		ИЛИ Т.РазделУчета В (ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыВПути),
		|			ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаКомиссиюВПути),
		|			ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПереданныеНаКомиссиюВПути))
		|			И Т.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности),
		|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионераБезПереходаПраваСобственности))
		...
		// Если в кор аналитике учета номенклатуры не заполнено назначение, например, при передаче перерабочику, но
		// назначение заполнено в аналитике учета номенклатуры, вид деятельности НДС определяется по указанному в аналитике назначению. 
		|		КОГДА НЕ Т.СлужебноеВидДвиженияПриход
		...
		|				И Т.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности))
		|			ТОГДА (ВЫБОР
		|				КОГДА Т.АналитикаУчетаНоменклатуры.Назначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт)
		|					ТОГДА (ВЫБОР КОГДА ЕСТЬNULL(Т.АналитикаУчетаНоменклатуры.Номенклатура.КодТНВЭД.СырьевойТовар, ЛОЖЬ)
		|						ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортСырьевыхТоваровУслуг)
		|					ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортНесырьевыхТоваров) КОНЕЦ)
		|				ИНАЧЕ Т.АналитикаУчетаНоменклатуры.Назначение.ВидДеятельностиНДС КОНЕЦ)
		|
		//++ Локализация
		...
		|	Т.СтатьяКалькуляции,
		|	ВЫБОР
		|		КОГДА Т.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности),
		|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионераБезПереходаПраваСобственности))
		|			И Т.РазделУчета В (ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыВПути),
		|				ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаКомиссиюВПути),
		|				ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПереданныеНаКомиссиюВПути))
		|			ТОГДА Т.Регистратор
		|		КОГДА Т.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.КорректировкаПриобретения)
...
Функция ТекстТекущиеДопРасходыСПартиейЗаявленияОВвозе()
	ТекстЗапроса = " 
		...
		|		ИЛИ ДД.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость)
		|	)
...
Функция ТекстРеализацииПрошлыхМесяцев() // использует ПрошлыеРеализации 
	Возврат 
		...
		|
		|	ДД.Период,
		// Период поступления определяется датой партии. Для пустой партии - концом предыдущего периодаю 
		|	ВЫБОР
		...
		|		КОГДА Прошлые.КорТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияДавальца)
		...
		|	ЗНАЧЕНИЕ(Справочник.КлючиАналитикиУчетаПоПартнерам.ПустаяСсылка) 		КАК АналитикаУчетаПоПартнерам,
		|	(ВЫБОР
		|		КОГДА Прошлые.ЗаказКлиента <> НЕОПРЕДЕЛЕНО
		|		 И ДД.ЗаказКлиента = НЕОПРЕДЕЛЕНО
		|			ТОГДА Прошлые.ЗаказКлиента
		|		ИНАЧЕ ДД.ЗаказКлиента КОНЕЦ)										КАК ЗаказКлиента,
		|	ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) 					КАК Подразделение,
		|	НЕОПРЕДЕЛЕНО 															КАК АналитикаРасходов,
		|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка) 			КАК СтатьяРасходовСписания,
		|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиДоходов.ПустаяСсылка) 			КАК СтатьяДоходов,
		...
		|		КОГДА Прошлые.КорТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияДавальца)
		|			ТОГДА ДД.КорВидЗапасов
		|		ИНАЧЕ ДД.ВидЗапасов КОНЕЦ), // ВидЗапасов
		|	ДД.Организация,
		|	ДД.Партия,
		|	ДД.АналитикаУчетаПартий,
		|	ДД.АналитикаФинансовогоУчета,
		...
		|		 И ДД.ЗаказКлиента = НЕОПРЕДЕЛЕНО
		|			ТОГДА Прошлые.ЗаказКлиента
		|		ИНАЧЕ ДД.ЗаказКлиента КОНЕЦ) // ЗаказКлиента
		|";
КонецФункции
...
Функция ТекстСторноДвиженияИсправленныхДокументовТекущегоПериода() // использует ИсправленныеДокументы
	...
	ТекстЗапроса = " 
		...
		|			ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость))
		|			ТОГДА ДанныеРегистра.Партия
		|		КОГДА ДанныеРегистра.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности),
		|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионераБезПереходаПраваСобственности))
		|			И ДанныеРегистра.РазделУчета В (ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыВПути),
		|			ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаКомиссиюВПути),
		|			ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПереданныеНаКомиссиюВПути))
		|		ТОГДА ДанныеРегистра.Партия
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) КАК Партия,
		...
		//++ Локализация
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТНастройкиУчетаНДСПриУСН КАК НастройкиУчетаНДСПриУСН
		|		ПО ДанныеРегистра.Организация = НастройкиУчетаНДСПриУСН.Организация
		|			И УчетныеПолитикиОрганизаций.СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.Упрощенная)
		//-- Локализация
...
Функция ТекстСторноДвиженияИсправленныхДокументовПрошлогоПериода() // использует ИсправленныеДокументы
	...
	Возврат " 
		...
		|	ДанныеРегистра.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
		|	ВЫБОР
		|		КОГДА ДанныеРегистра.Организация В (&ОрганизацииСФИФОСкользящая)
		|			ИЛИ ДанныеРегистра.РазделУчета В (
		|				ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки),
		|				ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.СобственныеТоварыВПути))
		|		ТОГДА ДанныеРегистра.АналитикаУчетаПартий.НалогообложениеНДС
		...
		|		ИНАЧЕ &КонецПредыдущегоПериода КОНЕЦ)            КАК ПериодПоступления,
		|	ВЫБОР 
		|		КОГДА ДанныеРегистра.ПериодОтклонения <> ДАТАВРЕМЯ(1,1,1)
		|			ТОГДА ДанныеРегистра.ПериодОтклонения
		|		ИНАЧЕ ДанныеРегистра.Период КОНЕЦ КАК ПериодОтклонения,
		|	ИсправленныеДокументы.Регистратор,
		...
		|	ДанныеРегистра.Организация,
		|	ВЫБОР
		|		КОГДА ДанныеРегистра.Партия <> НЕОПРЕДЕЛЕНО
		|		 И (ДанныеРегистра.Организация В (&ОрганизацииСФИФОСкользящая)
		|			ИЛИ ДанныеРегистра.РазделУчета В (
		|				ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки),
		|				ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.СобственныеТоварыВПути)))
		|			ТОГДА ДанныеРегистра.Партия
		// Для плановой стоимости выпуска сохраняем исходную партию, т.к. для этой записи партия не будет подбираться.
		|		КОГДА ДанныеРегистра.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтражениеПлановойСтоимостиВыпуска)
		|			ТОГДА ДанныеРегистра.Партия
		|		КОГДА ДанныеРегистра.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Партия)
		|		 И ДанныеРегистра.Организация В (&ОрганизацииСФИФОСкользящая)
		|			ТОГДА ДанныеРегистра.Регистратор
		...
		|	ВЫБОР
		|		КОГДА ДанныеРегистра.Организация В (&ОрганизацииСФИФОСкользящая)
		|			ИЛИ ДанныеРегистра.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтражениеПлановойСтоимостиВыпуска)
		|			ИЛИ ДанныеРегистра.РазделУчета В (
		|				ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НеотфактурованныеПоставки),
		|				ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.СобственныеТоварыВПути))
		|		ТОГДА ДанныеРегистра.АналитикаУчетаПартий
...
Процедура ЗаполнитьРасчетнуюПартиюСебестоимостиТоваров(ПараметрыРасчета, РасчетнаяПартия, Расход, Приход)
	...
	РазделыУчетаТоварыВПути = Новый Массив;
	РазделыУчетаТоварыВПути.Добавить(Перечисления.РазделыУчетаСебестоимостиТоваров.ТоварыВПути);
	РазделыУчетаТоварыВПути.Добавить(Перечисления.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаКомиссиюВПути);
	РазделыУчетаТоварыВПути.Добавить(Перечисления.РазделыУчетаСебестоимостиТоваров.ТоварыПереданныеНаКомиссиюВПути);
	
	ЭтоРеализацияТоварыВПути = РазделыУчетаТоварыВПути.Найти(Расход.РазделУчета) <> Неопределено
		И (Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности
		ИЛИ Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияЧерезКомиссионераБезПереходаПраваСобственности);
	...
	// Корректировка реализации прошлого периода, при наличии других корректировок реализации того же документа в текущем периоде.
	// Источником не может быть сам документ расхода.
	ИначеЕсли Расход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ВозвратПрошлыхПериодов
		И (Приход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.Потребление)
		И Приход.Регистратор = Расход.Регистратор
		И ТипЗнч(Расход.Регистратор) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
			Возврат;
	
	// Расход из товаров в пути должен подбирать партию из того же документа реализации, которым делался приход.
	ИначеЕсли Расход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.Потребление
		И Приход.РазделУчета = Расход.РазделУчета
		И ЭтоРеализацияТоварыВПути
	...
	ИначеЕсли Приход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ВозвратПрошлыхПериодов
	 ...
	 И Расход.ТипЗаписи <> Перечисления.ТипыЗаписейПартий.СторноВозвратаПрошлыхПериодов
	 И Расход.Регистратор <> Неопределено
	 И Расход.Регистратор <> Приход.Регистратор Тогда
		Возврат;
		
	ИначеЕсли Расход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ВозвратПрошлыхПериодов
	 И Приход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.Прошлое
	 И Расход.ДокументИсточник <> Приход.ДокументИсточник Тогда
	 	Возврат;
		
	// Для приемника с хоз операцией "Сторно списания на расходы" приемником может быть только регистратор,
	// указанный в документе - источнике.
	ИначеЕсли Расход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ВозвратПрошлыхПериодов
	 И Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СторноСписанияНаРасходы
	...
	Если НЕ ЭтоРеализацияТоварыВПути Тогда
		...
		Если ЗначениеЗаполнено(РасчетнаяПартия.КорПартия)
			И РазделыУчетаТоварыВПути.Найти(Расход.КорРазделУчета) <> Неопределено
	...
	ИначеЕсли ПараметрыРасчета.УчетныеПолитики.ФИФОСкользящая.ИспользуютВТекущемПериоде.Найти(РасчетнаяПартия.Организация) <> Неопределено Тогда
		...
		Если РасчетнаяПартия.ВидДвижения = ВидДвиженияНакопления.Приход Тогда
			РасчетнаяПартия.Партия = ?(РасчетнаяПартия.Сторно, РасчетнаяПартия.ДокументИсточник, РасчетнаяПартия.Регистратор);
		КонецЕсли;
	...
	ИначеЕсли РасчетнаяПартия.ВидДвижения = ВидДвиженияНакопления.Приход
	 ...
	 И РасчетнаяПартия.ТипЗаписи <> Перечисления.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость
	...
	ИЛИ РасчетнаяПартия.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ВключениеНДСВСтоимость
	ИЛИ РасчетнаяПартия.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость
	...
	Если РасчетнаяПартия.ТипЗаписи <> Перечисления.ТипыЗаписейПартий.Перемещение
		...
		И РасчетнаяПартия.ТипЗаписи <> Перечисления.ТипыЗаписейПартий.ПеремещениеНаОстаток
	...
	ИначеЕсли (Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5
		ИЛИ Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцуКорректировкаПрошлогоПериода
		ИЛИ Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПроизводствоПродукцииИзДавальческогоСырья2_5
		ИЛИ Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РемонтПродукцииИзДавальческогоСырья2_5
		ИЛИ Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыполнениеРаботСИспользованиемДавальческогоСырья2_5
		ИЛИ Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыполнениеРемонтныхРаботСИспользованиемДавальческогоСырья2_5
	...
	Если РазделыУчетаТоварыВПути.Найти(Расход.КорРазделУчета) <> Неопределено
		И (Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности
		ИЛИ Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияЧерезКомиссионераБезПереходаПраваСобственности) Тогда
	 ...
	 ИЛИ Расход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ДопРасходыСПартией Тогда
	 	...
	 	Если Приход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.СторноДопРасходов
	 	 ...
	 	 ИЛИ Расход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость Тогда
	 		КоличествоСписания = 1;
	 		РасходБазис = 1;
	 		// Если базис расхода больше единицы, значит источник сгруппирован, а приемник - нет. 
			// Например, заявление о ввозе товаров по документу приобретения прошлого периода с несколькими одинаковыми строками.
			// Если будет > 1, то все стоимостные данные будут кратно увеличиваться при каждом закрытие месяца.
			Если РасходБазис > 1 Тогда
				РасходБазис = 1;
			КонецЕсли;
	 	Иначе
	 		КоличествоСписания = Приход.Количество;
	...
	// Для приемников с типом записи ВключениеАкцизаВСтоимость и ВключениеНДСВСтоимость используется один источник,
	// поэтому базу корректировать не нужно.
	И НЕ Расход.ТипЗаписи =  Перечисления.ТипыЗаписейПартий.ВключениеАкцизаВСтоимость 
	И НЕ Расход.ТипЗаписи =  Перечисления.ТипыЗаписейПартий.ВключениеНДСВСтоимость Тогда
...
Функция ОписаниеДвиженийПартийВыручки(ПараметрыРасчета)
	...
	ОписаниеДвижений.Вставить("Показатели", "Количество, Стоимость, СтоимостьБезНДС, ДопРасходы, ДопРасходыБезНДС,
		...
		|ВыручкаПостояннаяРазница, ВыручкаВременнаяРазница,
		|СтоимостьРегл, ПостояннаяРазница, ВременнаяРазница,
		|СтоимостьЗабалансовая, Трудозатраты, ПостатейныеПостоянныеСНДС, ПостатейныеПостоянныеБезНДС,
		|ПостатейныеПеременныеСНДС, ПостатейныеПеременныеБезНДС,
		|СтоимостьУпр, ДопРасходыУпр, ТрудозатратыУпр, ПостатейныеПостоянныеУпр, ПостатейныеПеременныеУпр,
...
Функция ТекстОписаниеДанныхПартийВыручки()
	...
	ТекстЗапроса = 
		...
		|	ЛОЖЬ 												  КАК РасчетЗавершен,
		|	Т.Период											  КАК Период,
		|	Т.ПериодКорректировки								  КАК ПериодКорректировки,
		|	Т.Регистратор										  КАК Регистратор,
		|	ЛОЖЬ 												  КАК ЭтоСторно,
		|
		...
		|   Т.СтоимостьРегл,
		|   Т.СуммаВыручкиРегл,
		|   Т.ВыручкаПостояннаяРазница,
		|   Т.ВыручкаВременнаяРазница,
		|   Т.ПостояннаяРазница,
		|   Т.ВременнаяРазница,
...
Функция ТекстИнициализацияПартийВыручки()
	Возврат "
		...
		|	СУММА(Т.ВыручкаПостояннаяРазница) КАК ВыручкаПостояннаяРазница,
		|	СУММА(Т.ВыручкаВременнаяРазница) КАК ВыручкаВременнаяРазница,
		|	СУММА(Т.ПостояннаяРазница) КАК ПостояннаяРазница,
		|	СУММА(Т.ВременнаяРазница) КАК ВременнаяРазница,
		...
		|	СУММА(Т.ПостатейныеПостоянныеУпр) КАК ПостатейныеПостоянныеУпр,
		|	СУММА(Т.ПостатейныеПеременныеУпр) КАК ПостатейныеПеременныеУпр,
		|	СУММА(Т.СуммаАкцизаУпр) КАК СуммаАкцизаУпр,
		|	СУММА(Т.СуммаАкцизаРегл) КАК СуммаАкцизаРегл,
		|
		...
		|	МИНИМУМ(Т.ВыручкаПостояннаяРазница) КАК ВыручкаПостояннаяРазница,
		|	МИНИМУМ(Т.ВыручкаВременнаяРазница) КАК ВыручкаВременнаяРазница,
		...
		|	МИНИМУМ(Т.СуммаАкцизаУпр) КАК СуммаАкцизаУпр,
		|	МИНИМУМ(Т.СуммаАкцизаРегл) КАК СуммаАкцизаРегл,
...
Функция ТекстПартииДляВыручки()
	Возврат 
		...
		|   0 КАК ВыручкаПостояннаяРазница,
		|   0 КАК ВыручкаВременнаяРазница,
		|   0 КАК ПостояннаяРазница,
		|   0 КАК ВременнаяРазница,
		...
		|   0 КАК ПостатейныеПостоянныеУпр,
		|   0 КАК ПостатейныеПеременныеУпр,
		|	0 КАК СуммаАкцизаУпр,
		|	0 КАК СуммаАкцизаРегл,
		|
...
Процедура ЗаполнитьРасчетнуюПартиюВыручкиИСебестоимостьПродаж(ПараметрыРасчета, РасчетнаяПартия, Расход, Приход)
	...
	|СуммаВВалютеВзаиморасчетов, СуммаБезНДСВВалютеВзаиморасчетов, СуммаВВалютеДокумента, СуммаБезНДСВВалютеДокумента,
	|СуммаАкцизаУпр, СуммаАкцизаРегл");
...
Процедура ПересортицаПартийТоваров(ПараметрыРасчета) Экспорт
	...
	ИмяВременнойТаблицы = "ВТДвиженияСебестоимостиПересортицаПартий";
	ИмяРегистра = Метаданные.РегистрыНакопления.СебестоимостьТоваров.Имя;
	ОписаниеПриемника = ПараметрыРасчета.Движения[ИмяРегистра];
...
Функция ТекстТоварыКПередачеРасход() // использует ВтТоварыКПередаче
	ТекстЗапроса =
		...
		//++ Локализация
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТНастройкиУчетаНДСПриУСН КАК НастройкиУчетаНДСПриУСН
		|		ПО ДД.ОрганизацияПолучатель = НастройкиУчетаНДСПриУСН.Организация
		|			И УчетныеПолитикиОрганизаций.СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.Упрощенная)
		//-- Локализация
		|";
	//++ Локализация
