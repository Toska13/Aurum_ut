...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ОбновлениеИнформационнойБазыГосИС.ДобавитьОбработчикОбновленияПереходаНаКонструкторНастроек(
		...
		Новый УникальныйИдентификатор("ffc94232-ee89-4a26-9a25-f8c123f650d3"));
...
Функция СообщенияПоДокументу(ДокументСсылка, ОперацияРасчетаСтатусов, ДополнительныеПараметры)
	...
	ПараметрыФормированияСообщения.ИмяПоляНомерЗаявки      = ИмяПоляНомерЗаявки(РазделУчета);
...
Процедура ОбработкаЗагрузкиПолученныхДанных(ЭлементОчереди, ПараметрыОбмена, ПолученныеДанные, ИзмененныеОбъекты, ПрочитаноВерсиейAPI) Экспорт
	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийЗЕРНО.ПогашениеСДИЗАннулирование Тогда
		...
		ИмяПоляНомерЗаявки  = ИмяПоляНомерЗаявки(ЭлементОчереди.РазделУчета);
		ПараметрыОбновления = РегистрыСведений.СтатусыОбъектовСинхронизацииЗЕРНО.ВозвращаемоеЗначениеДальнейшиеДействияСтатус(
...
Функция РезультирующийСтатусДокумента(ДокументСсылка) Экспорт
	...
	Для Каждого СтрокаТаблицы Из Статусы Цикл
		...
		Корректировочный = СтрокаТаблицы.Корректировочный;
...
Функция ИмяПоляНомерЗаявки(РазделУчета)
