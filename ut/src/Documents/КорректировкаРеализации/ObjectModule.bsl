...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если КорректировкаРеализации2_5 
		И КорректировкаРеализацииЛокализация.НеКорректироватьНалоговуюБазуПрошлогоПериода(Организация, Дата) Тогда
		КорректироватьНалоговуюБазуПрошлогоПериода = Не ОтразитьВПрибылиУбыткахПрошлыхЛет;
	КонецЕсли;
	...
	НоменклатураСервер.ОчиститьНеиспользуемыеСерии(ЭтотОбъект, ПараметрыУказанияСерий.Расхождения);
	...
	ПараметрыДляВыбора.КорректировкаРеализации2_5 = КорректировкаРеализации2_5;
	ПараметрыДляВыбора.ОтразитьВПрибылиУбыткахПрошлыхЛет = ОтразитьВПрибылиУбыткахПрошлыхЛет;
	ПараметрыДляВыбора.КорректироватьНалоговуюБазуПрошлогоПериода = КорректироватьНалоговуюБазуПрошлогоПериода;
	...
	УстановитьКодСтрокиВСтрокахТабличнойЧасти();
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
		...
		ВариантыОтраженийСОбязательнымСкладом =
			Документы.КорректировкаРеализации.ВариантыОтраженийСОбязательнымСкладом();
		
		Если ВидКорректировки =
				ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.РеализацияПерепоставленногоТовара")
				Или ВидКорректировки =
					ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратНедопоставленногоТовара") Тогда
			
			Для Каждого СтрокаТЧ Из Товары Цикл
				
				Если ВариантыОтраженийСОбязательнымСкладом.НайтиПоЗначению(СтрокаТЧ.ВариантОтражения) <> Неопределено Тогда
					...
					ИначеЕсли Не ЗначениеЗаполнено(СтрокаТЧ.Склад) Тогда
						...
						ТекстОшибки = НСтр("ru='Не заполнена колонка ""Отправитель"" в строке %НомерСтроки% списка ""Товары""'");
						ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", СтрокаТЧ.НомерСтроки);
						
						ОбщегоНазначения.СообщитьПользователю(
							ТекстОшибки,
							ЭтотОбъект,
							ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", СтрокаТЧ.НомерСтроки, "Склад"),
							,
							Отказ);
			...
			Для Каждого СтрокаТЧ Из Расхождения Цикл
				...
				Если ВариантыОтраженийСОбязательнымСкладом.НайтиПоЗначению(СтрокаТЧ.ВариантОтражения) <> Неопределено Тогда
					...
					ИначеЕсли Не ЗначениеЗаполнено(СтрокаТЧ.Склад) Тогда
						...
						ТекстОшибки = НСтр("ru='Не заполнена колонка ""Отправитель"" в строке %НомерСтроки% списка ""Расхождения""'");
						ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", СтрокаТЧ.НомерСтроки);
						
						ОбщегоНазначения.СообщитьПользователю(
							ТекстОшибки,
							ЭтотОбъект,
							ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Расхождения", СтрокаТЧ.НомерСтроки, "Склад"),
							,
							Отказ);
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПервичныйДокумент") Тогда
		...
		МассивНепроверяемыхРеквизитов.Добавить("Расхождения.Серия");
		...
		ПараметрыУказанияСерийТовары = ПараметрыУказанияСерий.Товары;
		ПараметрыУказанияСерийТовары.ИмяТЧТовары	= "ТаблицаКорректировки";
		ПараметрыУказанияСерийТовары.ИмяТЧСерии		= "ТаблицаКорректировки";
		
		ДанныеДокумента = Новый Структура;
		ДанныеДокумента.Вставить("Объект",					ЭтотОбъект);
		ДанныеДокумента.Вставить("ПараметрыУказанияСерий",	ПараметрыУказанияСерийТовары);
	...
	Иначе
		...
		КоллекцияПараметров = Новый Массив;
		КоллекцияПараметров.Добавить(ПараметрыУказанияСерий.Товары);
		КоллекцияПараметров.Добавить(ПараметрыУказанияСерий.Расхождения);
	...
	Если КорректировкаРеализации2_5 И НачалоГода(ДатаОснования) < НачалоГода(Дата) Тогда
	...
	УчетНДСЛокализация.ПроверитьУслугиПоСтавкеНДСКомитента(ЭтотОбъект, Отказ, "Товары", ДатаДокументаОснования());
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	КонецЕсли;
	Согласован = Ложь;
	КорректировкаРеализации2_5 = Истина;
	...
	ПараметрыЗаполнитьСтавкуНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(ЭтотОбъект);
	...
	КэшРасхождения = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
	КэшТовары = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
	СтруктураДействий.Вставить("СкорректироватьСтавкуНДС", ПараметрыЗаполнитьСтавкуНДС);
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Расхождения, СтруктураДействий, КэшРасхождения);
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Товары, СтруктураДействий, КэшТовары);
	
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
	СтруктураДействий.Вставить("ПересчитатьСуммуСНДС", СтруктураПересчетаСуммы);
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		КэшРасхождения.ОбработанныеСтроки,
		СтруктураДействий,
		Расхождения,
		Неопределено);
		
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		КэшТовары.ОбработанныеСтроки,
		СтруктураДействий,
		Товары,
		Неопределено);
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		...
		ДатаОснования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения.ДокументОснование, "Дата");
		ДатаДокумента = ?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДатаСеанса());

		ДатаКурса = ДатаОснования;
		
		Если НачалоГода(ДатаДокумента) > НачалоГода(ДатаОснования) Тогда
			ОтразитьВПрибылиУбыткахПрошлыхЛет = Истина;
		КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ХозяйственнаяОперация)
		...
		Если ТипЗнч(ДанныеЗаполнения.ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
			И ДанныеЗаполнения.Свойство("ХозяйственнаяОперацияОснования")
			И ДанныеЗаполнения.ХозяйственнаяОперацияОснования =
				Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
			
			ХозяйственнаяОперация = ДанныеЗаполнения.ХозяйственнаяОперацияОснования;
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
			И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеЗаполнения, "ДокументОснование") Тогда
		ОснованиеИзДанныхЗаполнения = ДанныеЗаполнения.ДокументОснование;
	ИначеЕсли ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ДанныеЗаполнения)) Тогда
		ОснованиеИзДанныхЗаполнения = ДанныеЗаполнения;
	Иначе
		ОснованиеИзДанныхЗаполнения = Неопределено;
	КонецЕсли;
	ОтветственныеЛицаСервер.УстановитьМенеджера(ЭтотОбъект, ОснованиеИзДанныхЗаполнения);
...
Процедура ЗаполнитьПоРеализацииТоваровУслуг(Знач ДокументОснование, ДанныеЗаполнения, ПараметрыОформления = Неопределено)
	...
	НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(ЭтотОбъект, ПараметрыУказанияСерий.Расхождения);
...
Процедура ЗаполнитьПоАктуОРасхожденияхПослеОтгрузки(Знач СтруктураПараметров, ДанныеЗаполнения)
	...
	ЭтоВозвратВПуть = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураПараметров, "ЭтоВозвратВПуть", Ложь);
	
	ДанныеЗаполнения = Новый Структура;
	ДанныеЗаполнения.Вставить("ДокументОснование", ДокументОснованиеРеализация);
	ДанныеЗаполнения.Вставить("ЭтоВозвратВПуть", ЭтоВозвратВПуть);
	...
	Если Выборка.СпособОтраженияРасхождений = Перечисления.СпособыОтраженияРасхожденийАктПриемкиКлиента.ИсправлениеПервичныхДокументов
		И Выборка.ХозяйственнаяОперацияОснования <> Перечисления.ХозяйственныеОперации.ПереходПраваСобственности
		Тогда
	...
	Если Выборка.ХозяйственнаяОперацияОснования = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
		ДополнительныеСвойства.Вставить("ХозяйственнаяОперацияОснования", Выборка.ХозяйственнаяОперацияОснования);
	КонецЕсли;
	...
	Документы.КорректировкаРеализации.ЗаполнитьТоварыПоАктуПриемки(ЭтотОбъект, ЭтоВозвратВПуть);
...
Процедура УстановитьКодСтрокиВСтрокахТабличнойЧасти()
	
	// Для строк ТЧ Расхождения с Вариантом отражения = Вернуть в путь заполняем код строки. Так как по этим
	// строкам будет создаваться Поступление товаров от клиента
	ПараметрыПоиска = Новый Структура;
	ПараметрыПоиска.Вставить("ВариантОтражения", Перечисления.ВариантыОтраженияКорректировокРеализаций.ВернутьВПуть);
	ПараметрыПоиска.Вставить("КодСтроки", 0);
	
	СтрокиБезКлюча = Расхождения.НайтиСтроки(ПараметрыПоиска);
	ТекущийКод = МаксимальныйКодСтроки;
	
	Для Каждого СтрокаТЧ Из СтрокиБезКлюча Цикл
		ТекущийКод = ТекущийКод + 1;
		СтрокаТЧ.КодСтроки = ТекущийКод;
	КонецЦикла;
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	ПодбиратьПодакцизныеТовары	= Ложь;
	ДатаОснования				= ДатаДокументаОснования();
	...
	Иначе
		...
		ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5 =
			ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
			И ПолучитьФункциональнуюОпцию("ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5")
		...
		Если Не ПодбиратьПодакцизныеТовары Тогда
			МассивВариантовОтражения.Добавить(Перечисления.ВариантыОтраженияКорректировокРеализаций.СписатьНаРасходы);
		КонецЕсли;
		МассивВариантовОтражения.Добавить(Перечисления.ВариантыОтраженияКорректировокРеализаций.ВернутьВПуть);
		...
		Если Не Проведен
			...
			Если ЕстьСтрокиСписания Тогда
				ОтборСтрок = Новый Структура("НаДоходыРасходы", Истина);
				ВидыЗапасовСтрокСписания = ВидыЗапасовОприходование.Выгрузить(ОтборСтрок);
			КонецЕсли;
			
			Если ТипКорректировки = Перечисления.ТипыКорректировки.КорректировкаДоВводаОстатковТоваровИУслуг Тогда
				ВидыЗапасовОприходование.Очистить();
				
				ТаблицаВидыЗапасовОприходование = ТаблицаДляЗаполненияВидыЗапасовОприходование(МенеджерВременныхТаблиц);
				
				Если ТаблицаВидыЗапасовОприходование.Количество() > 0 Тогда
					ЗапасыСервер.ЗаполнитьВидыЗапасовПоУмолчанию(МенеджерВременныхТаблиц, ТаблицаВидыЗапасовОприходование);
					ЗаполнитьДопКолонкиВидовЗапасовОприходования(ТаблицаВидыЗапасовОприходование, МассивВариантовОтражения);
				КонецЕсли;
			Иначе
				Если ЕстьСтрокиСписания Тогда
					ВидыЗапасовОприходование.Очистить();
				...
				ПараметрыЗаполнения.КорректировкаРеализацииПослеПереходаПраваСобственности = ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5;
				...
				ИменаКолонокГруппировки		= "АналитикаУчетаНоменклатуры, ВидЗапасов, КорВидЗапасов, НомерГТД, Склад,
												|СкладОтгрузки, ВидЗапасовОтгрузки, СтавкаНДС, НоменклатураПартнера,
				...
				ИменаКолонокСуммирования	= "Количество, КоличествоПоРНПТ, СуммаСНДС, СуммаНДС,
												...
												|СебестоимостьПР, СебестоимостьВР, СуммаНДСРегл, СуммаНДСУпр";
			...
			Если ЕстьСтрокиСписания Тогда
				ОбщегоНазначенияУТ.ДобавитьСтрокиВТаблицу(ВидыЗапасовОприходование, ВидыЗапасовСтрокСписания);
			КонецЕсли;
		КонецЕсли;
	...
	Иначе
		...
		Если Не Проведен
			...
			ПараметрыЗаполнения.ИмяТаблицыПриходуемыхВидовЗапасов = "ВтВидыЗапасовОприходование";
				...
				ПараметрыЗаполнения.ПериодДокументаОснования = ДатаОснования;
				ПараметрыЗаполнения.ОтборыВидовЗапасов.НалогообложениеНДС = НалогообложениеНДС;
				
				НалогообложениеПродажи			=
					УчетНДСУП.ПараметрыУчетаПоОрганизации(Организация, Дата).ОсновноеНалогообложениеНДСПродажи;
				НалогообложениеПродажиОснования	=
					УчетНДСУП.ПараметрыУчетаПоОрганизации(Организация, ДатаОснования).ОсновноеНалогообложениеНДСПродажи;
				
				НалогообложениеОтбора = ?(НалогообложениеПродажи = НалогообложениеПродажиОснования,
											НалогообложениеНДС,
											НалогообложениеПродажи);
			...
			ИменаКолонокСуммирования = "Количество, КоличествоПоРНПТ, СуммаСНДС, СуммаНДС";
			ВидыЗапасовСписание.Свернуть("АналитикаУчетаНоменклатуры, ВидЗапасов, КорВидЗапасов, НомерГТД, Склад, СтавкаНДС, НоменклатураПартнера",
...
Процедура ЗаполнитьДопКолонкиВидовЗапасов(ВидыЗапасов, МассивВариантовОтражения, ЗаполнятьВидЗапасовОтгрузки = Ложь, ЗаполнятьСебестоимость = Ложь) Экспорт
	...
	ИменаКолонокГруппировки		= "АналитикаУчетаНоменклатуры, АналитикаУчетаНаборов, Склад, Упаковка, ЗаказКлиента,
									|КорАналитикаУчетаНоменклатуры, КодТНВЭД, ОбъектРасчетов,
	...
	Для Каждого СтрокаТоваров Из ТаблицаТовары Цикл
		...
		// Реквизит заполняется для документов, созданных на основании РеализацияТоваровУслуг с операциями
		// РеализацияКлиенту и ПереходПраваСобственности.
		// По КорАналитикаУчетаНоменклатуры осуществляется подбор запасов, в связи с чем необходимо нормализовать
		// значения аналитик после подбора.
		Если ЗаполнятьВидЗапасовОтгрузки
			И ЗначениеЗаполнено(СтрокаТоваров.КорАналитикаУчетаНоменклатуры) Тогда
			
			СтруктураПоиска.АналитикаУчетаНоменклатуры = СтрокаТоваров.КорАналитикаУчетаНоменклатуры;
...
Процедура ЗаполнитьДопКолонкиВидовЗапасовОприходования(ТаблицаВидыЗапасов, ВариантыОтражения)
	...
	ВыгружаемыеКолонки			= "НомерСтроки" + ", " + ИменаКолонокГруппировки + ", " + ИменаКолонокСуммирования;
	
	ТаблицаТовары = Расхождения.ВыгрузитьКолонки(ВыгружаемыеКолонки);
	ТаблицаТовары.Колонки.Добавить("ВидЗапасов");
	ТаблицаТовары.Колонки.Добавить("НаДоходыРасходы");
	...
	Для Каждого СтрокаРасхождений Из Расхождения Цикл
		...
		СтрокаЗапасы = ТаблицаВидыЗапасов.Найти(СтрокаРасхождений.НомерСтроки, "НомерСтроки");
		
		СтрокаТоваров.ВидЗапасов = СтрокаЗапасы.ВидЗапасов;
	...
	ИменаКолонокГруппировки = ИменаКолонокГруппировки + ", " + "ВидЗапасов, НаДоходыРасходы";
	...
	Для Каждого СтрокаТоваров Из ТаблицаТовары Цикл
		...
		НоваяСтрока.ВидЗапасовОтгрузки	= НоваяСтрока.ВидЗапасов;
...
Процедура ЗаполнитьАналитикиУчетаНоменклатурыВТабличныхЧастяхТоваров(ТаблицыДокумента = Неопределено)
	...
	Если ТипЗнч(ДокументОснование) <> Тип("ДокументСсылка.РеализацияУслугПрочихАктивов")
		...
		СинхронизироватьАналитики = Ложь;
		ЗаполнятьКорАналитикуУчетаНоменклатуры = Ложь;
		...
		Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
			...
			Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
				...
				ЗаполнятьКорАналитикуУчетаНоменклатуры =
					ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту
					Или ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности;
			Иначе
			...
			ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
				И ПолучитьФункциональнуюОпцию("ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5") Тогда
				
				// Формирование коллекции поиска строк, для заполнения корреспондирующих аналитик.
				ВариантыРасхождений = Новый Массив;
				
				Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
					ВариантыРасхождений.Добавить(Перечисления.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализациюУчестьПриИнвентаризации);
					ВариантыРасхождений.Добавить(Перечисления.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализациюУвеличитьСкладскиеОстатки);
					ВариантыРасхождений.Добавить(Перечисления.ВариантыОтраженияКорректировокРеализаций.УвеличитьРеализациюУчестьПриИнвентаризации);
					ВариантыРасхождений.Добавить(Перечисления.ВариантыОтраженияКорректировокРеализаций.УвеличитьРеализациюУменьшитьСкладскиеОстатки);
				Иначе
					ВариантыРасхождений.Добавить(Перечисления.ВариантыОтраженияКорректировокРеализаций.ВернутьВПуть);
				КонецЕсли;
				
				Для Каждого ЗначениеРасхождения Из ВариантыРасхождений Цикл
					Если ТаблицаРасхождения.Найти(ЗначениеРасхождения, "ВариантОтражения") <> Неопределено Тогда
						СинхронизироватьАналитики = Истина;
						
						Прервать;
					КонецЕсли;
				КонецЦикла;
				
				Если СинхронизироватьАналитики Тогда
					Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
						ИменаПолейКорАналитики = ИменаПолей;
						ИменаПолейКорАналитики.СтатусУказанияСерий = "СтатусУказанияСерийНаСкладах";
					Иначе
						ИменаПолейКорАналитики = РегистрыСведений.АналитикаУчетаНоменклатуры.ИменаПолейКоллекцииПоУмолчанию();
						ИменаПолейКорАналитики.Работа				= "Подразделение";
						ИменаПолейКорАналитики.СтатусУказанияСерий	= "СтатусУказанияСерийПереданныхТоваров";
					КонецЕсли;
					
					ИменаПолейКорАналитики.АналитикаУчетаНоменклатуры = "КорАналитикаУчетаНоменклатуры";
					...
					РегистрыСведений.АналитикаУчетаНоменклатуры.ЗаполнитьВКоллекции(ТаблицаРасхождения,
																					МестаУчета,
																					ИменаПолейКорАналитики);
				КонецЕсли;
				
				Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
					СкладУчета = Договор;
					
					ИменаПолей.Произвольный			= "";
		...
		Если Не СинхронизироватьАналитики
			И ЗаполнятьКорАналитикуУчетаНоменклатуры Тогда
			
			АналитикиУчета = ТаблицаРасхождения.ВыгрузитьКолонку("АналитикаУчетаНоменклатуры");
			ТаблицаРасхождения.ЗагрузитьКолонку(АналитикиУчета, "КорАналитикаУчетаНоменклатуры");
			
		ИначеЕсли СинхронизироватьАналитики Тогда
			Для Каждого СтрокаТЧ Из ТаблицаРасхождения Цикл
				АналитикаУчетаНоменклатуры = ?(ВариантыРасхождений.Найти(СтрокаТЧ.ВариантОтражения) = Неопределено,
												СтрокаТЧ.АналитикаУчетаНоменклатуры,
												СтрокаТЧ.КорАналитикаУчетаНоменклатуры);
				
				СтрокаТЧ.КорАналитикаУчетаНоменклатуры	= СтрокаТЧ.АналитикаУчетаНоменклатуры;
				СтрокаТЧ.АналитикаУчетаНоменклатуры		= АналитикаУчетаНоменклатуры;
			КонецЦикла;
		Иначе
			Для Каждого СтрокаТЧ Из ТаблицаРасхождения Цикл
				СтрокаТЧ.КорАналитикаУчетаНоменклатуры = Неопределено;
			КонецЦикла;
		КонецЕсли;
...
Функция ВременныеТаблицыДанныхДокументаСписание(МассивВариантовОтражения)
	...
	|	ВЫБОР
	|		КОГДА &ИспользоватьПодакцизныеТовары
	|		И ТаблицаВидыЗапасов.НаДоходыРасходы
	|		И НЕ ЗНАЧЕНИЕ(Перечисление.ВариантыОтраженияКорректировокРеализаций.ОтразитьНаПрочихДоходах) В
	|		(&МассивВариантовОтражения)
	...
	ИспользоватьПодакцизныеТовары = ?(ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту,
		ПолучитьФункциональнуюОпцию("ИспользоватьПодакцизныеТовары"), Ложь);
...
///////////////////////////////////////////////////////////////////////////////
// Оприходование
Функция ВременныеТаблицыДанныхДокументаОприходование(МассивВариантовОтражения, ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5)
	...
	|	ВЫБОР
	|		КОГДА &ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5
	|				И ТаблицаВидыЗапасов.КорАналитикаУчетаНоменклатуры <> ЗНАЧЕНИЕ(Справочник.КлючиАналитикиУчетаНоменклатуры.ПустаяСсылка)
	...
	Запрос.УстановитьПараметр("ПродажаПоЗаказам",					ПродажаПоЗаказам);
	Запрос.УстановитьПараметр("ТипКорректировки",					ТипКорректировки);
	Запрос.УстановитьПараметр("ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5",
								ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5);
...
Функция ТаблицаДляЗаполненияВидыЗапасовОприходование(МенеджерВременныхТаблиц)
	...
	Возврат Результат[Результат.ВГраница()].Выгрузить();
...
Функция ВременныеТаблицыДанныхДокументаКорректировкаВыручки()
	...
	|	&НалогообложениеНДС КАК НалогообложениеНДС,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СтруктураПредприятия.ВариантОбособленногоУчетаТоваров, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ВариантыОбособленногоУчетаТоваров.ПоМенеджерамПодразделения)
	|				И &ФормироватьВидыЗапасовПоПодразделениямМенеджерам
	|			ТОГДА &Менеджер
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	|	КОНЕЦ КАК Менеджер,
...
Процедура ЗаполнитьВидыЗапасовКорректировкаВыручкиКорректировкиДоВводаОстатков(МенеджерВременныхТаблиц)
	...
	|	СУММА(ТаблицаТоваров.СуммаНДСРегл)				КАК СуммаНДСРегл,
	|	СУММА(ТаблицаТоваров.СуммаНДСУпр)				КАК СуммаНДСУпр,
	...
	ТаблицаТоваровОприходования = РезультатЗапроса[РезультатЗапроса.ВГраница()].Выгрузить();
...
Процедура ЗаполнитьВидыЗапасовКорректировкаВыручкиОбычнойКорректировки(МенеджерВременныхТаблиц)
	...
	|	СУММА(ТаблицаТоваров.СуммаНДСРегл),
	...
	ДеревоВидовЗапасов.Колонки.Добавить("СуммаНДСРегл");
	ДеревоВидовЗапасов.Колонки.Добавить("СуммаНДСУпр");
										...
										Для Каждого СтрокаПоСтатьям Из СтрокиПоОбъектамРасчета.Строки Цикл
											Для Каждого СтрокаАналитикаРасходов Из СтрокаПоСтатьям.Строки Цикл
												Для Каждого СтрокаАналитикаАктивовПассивов Из СтрокаАналитикаРасходов.Строки Цикл
													...
													Если СтрокиРасхождений[0].СуммаНДСРегл <> 0 Тогда
														РаспределенныеСуммы = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(СтрокиРасхождений[0].СуммаНДСРегл, БазаРаспределения);
														СтрокаАналитикаАктивовПассивов.Строки.ЗагрузитьКолонку(РаспределенныеСуммы, "СуммаНДСРегл");
													КонецЕсли;
													
													Если СтрокиРасхождений[0].СуммаНДСУпр <> 0 Тогда
														РаспределенныеСуммы = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(СтрокиРасхождений[0].СуммаНДСУпр, БазаРаспределения);
														СтрокаАналитикаАктивовПассивов.Строки.ЗагрузитьКолонку(РаспределенныеСуммы, "СуммаНДСУпр");
													КонецЕсли;
...
// Заполняет колонки Сумма НДС (регл.) и Сумма НДС (упр.) в табличной части Расхождения документа.
//
Процедура ЗаполнитьСуммыНДСРеглУпр() Экспорт
	
	ДатаКурса = ТекущаяДатаСеанса();
	
	ЭтоИсправлениеОшибок = ВидКорректировки = Перечисления.ХозяйственныеОперации.ИсправлениеОшибок
		И ЗначениеЗаполнено(ДокументОснование);
	
	Если ЭтоИсправлениеОшибок Тогда
		...
		"ВЫБРАТЬ
		|	ВЫРАЗИТЬ(ВЫБОР
		|		КОГДА СУММА(ТоварыОснования.СуммаНДС) = 0
		|			ТОГДА 0
		|		ИНАЧЕ СУММА(ТоварыОснования.СуммаНДСРегл) / СУММА(ТоварыОснования.СуммаНДС)
		|	КОНЕЦ КАК ЧИСЛО(10, 6)) КАК КоэффициентРегл,
		|	ВЫРАЗИТЬ(ВЫБОР
		|		КОГДА СУММА(ТоварыОснования.СуммаНДС) = 0
		|			ТОГДА 0
		|		ИНАЧЕ СУММА(ТоварыОснования.СуммаНДСУпр) / СУММА(ТоварыОснования.СуммаНДС)
		|	КОНЕЦ КАК ЧИСЛО(10, 6)) КАК КоэффициентУпр
		|ИЗ
		|	Документ.РеализацияТоваровУслуг.Товары КАК ТоварыОснования
		|ГДЕ
		...
		КоэффициентРегл = Выборка.КоэффициентРегл;
		КоэффициентУпр = Выборка.КоэффициентУпр;
	...
	Для Каждого СтрокаТЧ Из Расхождения Цикл
		...
		СуммаБезНДС = СтрокаТЧ.СуммаСНДС - СтрокаТЧ.СуммаНДС;
		
		Если ВалютаВзаиморасчетов = ВалютаРегламентированногоУчета Тогда
			
			СтрокаТЧ.СуммаНДСРегл = СтрокаТЧ.СуммаНДС;
			
		ИначеЕсли ОплатаВВалюте Тогда
			
			Если ЭтоИсправлениеОшибок Тогда
				
				СуммаБезНДСРегл = СуммаБезНДС * КоэффициентРегл;
				
			Иначе
				
				СуммаБезНДСРегл = РаботаСКурсамиВалютУТ.ПересчитатьВВалюту(СуммаБезНДС,
					БазоваяВалюта,
			...
			СтрокаТЧ.СуммаНДСРегл = ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(
				СуммаБезНДСРегл,
				СтрокаТЧ.СтавкаНДС,
				Ложь);
		...
		Иначе
			...
			Если ЭтоИсправлениеОшибок Тогда
				...
				СуммаСНДСРегл = СтрокаТЧ.СуммаСНДС * КоэффициентРегл;
				
			Иначе
					
				СуммаСНДСРегл = РаботаСКурсамиВалютУТ.ПересчитатьВВалюту(СтрокаТЧ.СуммаСНДС,
					БазоваяВалюта,
					ВалютаВзаиморасчетов,
					ВалютаРегламентированногоУчета,
					ДатаКурса);
			...
			СтрокаТЧ.СуммаНДСРегл = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(
				СуммаСНДСРегл,
				СтрокаТЧ.СтавкаНДС,
				Истина);
		...
		Если ВалютаВзаиморасчетов = ВалютаУправленческогоУчета Тогда
			...
			СтрокаТЧ.СуммаНДСУпр = СтрокаТЧ.СуммаНДС;
			
		ИначеЕсли ОплатаВВалюте Тогда
			
			Если ЭтоИсправлениеОшибок Тогда
				
				СуммаБезНДСУпр = СуммаБезНДС * КоэффициентУпр;
				
			Иначе
				
				СуммаБезНДСУпр = РаботаСКурсамиВалютУТ.ПересчитатьВВалюту(СуммаБезНДС,
					БазоваяВалюта,
					ВалютаВзаиморасчетов,
			...
			СтрокаТЧ.СуммаНДСУпр = ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(
				СуммаБезНДСУпр,
				СтрокаТЧ.СтавкаНДС,
				Ложь);
		...
		Иначе
			...
			Если ЭтоИсправлениеОшибок Тогда
				...
				СуммаСНДСУпр = СтрокаТЧ.СуммаСНДС * КоэффициентУпр;
				
			Иначе
					
				СуммаСНДСУпр = РаботаСКурсамиВалютУТ.ПересчитатьВВалюту(СтрокаТЧ.СуммаСНДС,
					БазоваяВалюта,
					ВалютаВзаиморасчетов,
					ВалютаУправленческогоУчета,
					ДатаКурса);
			...
			СтрокаТЧ.СуммаНДСУпр = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(
				СуммаСНДСУпр,
				СтрокаТЧ.СтавкаНДС,
				Истина);
...
Процедура ЗаполнитьРасхождения() Экспорт
	...
	ЭтоПереходПраваСобственности =
		ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности
		Или ДополнительныеСвойства.Свойство("ХозяйственнаяОперацияОснования")
			И ДополнительныеСвойства.ХозяйственнаяОперацияОснования =
...
Процедура ЗаполнитьРасхожденияПоРазнице()
	...
	ХозяйственнаяОперацияПараметр = Неопределено;
	Если ЗначениеЗаполнено(ХозяйственнаяОперация) Тогда
		ХозяйственнаяОперацияПараметр = ХозяйственнаяОперация;
	ИначеЕсли ДополнительныеСвойства.Свойство("ХозяйственнаяОперацияОснования") Тогда
		ХозяйственнаяОперацияПараметр = ДополнительныеСвойства.ХозяйственнаяОперацияОснования;
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("ХозяйственнаяОперация", ХозяйственнаяОперацияПараметр);
	Запрос.УстановитьПараметр("НовыеДанные", Товары.Выгрузить());
	Запрос.УстановитьПараметр("КорректировкаУслугПрочихАктивов", КорректировкаУслугПрочихАктивов);
	Запрос.УстановитьПараметр("КорректировкаТекущегоПериода", Не КорректировкаПрошлогоПериода);
	Запрос.УстановитьПараметр("КорректировкаРеализации2_5", КорректировкаРеализации2_5);
	Запрос.УстановитьПараметр("ОтразитьВПрибылиУбыткахПрошлыхЛет", ОтразитьВПрибылиУбыткахПрошлыхЛет);
	Запрос.УстановитьПараметр("КорректироватьНалоговуюБазуПрошлогоПериода", КорректироватьНалоговуюБазуПрошлогоПериода);
	...
	|			ТОГДА НовыеДанные.КодСтроки
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК КодСтрокиОснования,
	...
	|	ВЫБОР
	|		КОГДА &КорректировкаУслугПрочихАктивов
	|			И (&КорректировкаТекущегоПериода
	|				ИЛИ &КорректировкаРеализации2_5
	|					И (НЕ &ОтразитьВПрибылиУбыткахПрошлыхЛет
	|						ИЛИ &КорректироватьНалоговуюБазуПрошлогоПериода))
	|			ТОГДА НовыеДанные.СтатьяДоходов
	|		ИНАЧЕ
	...
	|					И (НЕ &ОтразитьВПрибылиУбыткахПрошлыхЛет
	|						ИЛИ &КорректироватьНалоговуюБазуПрошлогоПериода))
	|			ТОГДА НовыеДанные.АналитикаДоходов
	|		ИНАЧЕ
	|			НЕОПРЕДЕЛЕНО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НовыеДанныеИзТаблицы.КодСтрокиОснования,
	...
	|	ВЫРАЗИТЬ(ВЫБОР
	|		КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|			ТОГДА ИсходныеДанные.КодСтроки
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ЧИСЛО(10, 0)) КАК КодСтрокиОснования,
	...
	|					И (НЕ &ОтразитьВПрибылиУбыткахПрошлыхЛет
	|						ИЛИ &КорректироватьНалоговуюБазуПрошлогоПериода))
	|			ТОГДА ВЫРАЗИТЬ(ИсходныеДанные.СтатьяДоходов КАК ПланВидовХарактеристик.СтатьиДоходов)
	|		ИНАЧЕ
	|			ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиДоходов.ПустаяСсылка)
	|	КОНЕЦ КАК СтатьяДоходов,
	|	ВЫБОР
	...
	|					И (НЕ &ОтразитьВПрибылиУбыткахПрошлыхЛет
	|						ИЛИ &КорректироватьНалоговуюБазуПрошлогоПериода))
	|			ТОГДА ИсходныеДанные.АналитикаДоходов
	|		ИНАЧЕ
	|			НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК АналитикаДоходов,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НовыеДанные.КодСтрокиОснования КАК КодСтрокиОснования,
	|	НовыеДанные.НоменклатураНабора КАК НоменклатураНабора,
	|	НовыеДанные.ХарактеристикаНабора КАК ХарактеристикаНабора,
	|	НовыеДанные.Номенклатура КАК Номенклатура,
	|	НовыеДанные.НоменклатураПартнера КАК НоменклатураПартнера,
	...
	|		ПО НовыеДанные.КодСтрокиОснования = ИсходныеДанные.КодСтрокиОснования
	|			И НовыеДанные.Номенклатура = ИсходныеДанные.Номенклатура
	...
	|
	|ВЫБРАТЬ
	|	ИсходныеДанные.КодСтрокиОснования,
	|	ИсходныеДанные.НоменклатураНабора,
	|	ИсходныеДанные.ХарактеристикаНабора,
	|	ИсходныеДанные.Номенклатура,
	|	ИсходныеДанные.НоменклатураПартнера,
	...
	|		ПО ИсходныеДанные.КодСтрокиОснования = НовыеДанные.КодСтрокиОснования
	|			И ИсходныеДанные.Номенклатура = НовыеДанные.Номенклатура
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДанныеРасхождений.КодСтрокиОснования,
	...
	|	ВЫРАЗИТЬ(ДанныеРасхождений.Номенклатура КАК Справочник.Номенклатура).ТипНоменклатуры
	|
	|ИМЕЮЩИЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДанныеРасхождений.КодСтрокиОснования КАК КодСтрокиОснования,
	...
	|		ПО ДанныеРасхождений.КодСтрокиОснования = ДанныеТаблицы.КодСтрокиОснования
	|			И ДанныеРасхождений.Номенклатура = ДанныеТаблицы.Номенклатура
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	УчтенныеРасхождения.КодСтрокиОснования КАК КодСтрокиОснования,
	...
	|
	|ВЫБРАТЬ
	|	УчтенныеРасхождения.КодСтрокиОснования,
...
Функция ДатаДокументаОснования()
