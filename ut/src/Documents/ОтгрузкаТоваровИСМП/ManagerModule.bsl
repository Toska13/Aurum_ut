...
//  Массив Из ПеречислениеСсылка.СтатусыОбработкиОтгрузкиТоваровИСМП - Конечные статусы.
...
//
// Возвращаемое значение:
//  Массив Из ПеречислениеСсылка.СтатусыОбработкиОтгрузкиТоваровИСМП - Статусы ошибок.
//
...
Процедура ОтразитьРезультатыПроверкиИПодбора(ПараметрыОкончанияПроверки) Экспорт
	...
	ДоступенСрокГодности = КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСоСрокамиГодности(
		ДокументОбъект.ТоварнаяГруппа,, ДокументОбъект.Операция);
...
Процедура ЗаполнитьПродукциюДокумента(ТаблицаМаркируемойПродукции, ДокументСсылка, Сценарий) Экспорт
	...
	|	ОтгрузкаТоваровИСМПТовары.Ссылка.ТоварнаяГруппа
	|ИЗ
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ОбновлениеИнформационнойБазыГосИС.ДобавитьОбработчикОбновленияПереходаНаКонструкторНастроек(
		...
		Новый УникальныйИдентификатор("a3752866-7776-4e2e-bc15-36988e67d856"));
...
Функция ОтгрузкаТоваровJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	ДоступныеТоварныеГруппы = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступныеТоварныеГруппы(Шапка);
	
	Если ДоступныеТоварныеГруппы.Найти(Шапка.ТоварнаяГруппа) = Неопределено Тогда
		
		СообщениеJSON.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'Товарная группа ""%1"" не поддерживается для отгрузки товаров.'"),
	...
	ДоступныеОперации = ОтгрузкаТоваровКлиентСервер.ДоступныеОперации(Шапка.ТоварнаяГруппа);
	
	Если ДоступныеОперации.Найти(Шапка.Операция) = Неопределено Тогда
	...
	Если ОтгрузкаТоваровКлиентСервер.ДоступноПолеПолеGLNКонтрагента(Шапка.ТоварнаяГруппа, Шапка.Операция)
		...
		Для Каждого СтрокаУникальности Из ТаблицаУникальности Цикл
			...
			Для Каждого СтрокаТоваров Из НайденныеСтроки Цикл
				...
				Если ЗначениеЗаполнено(СтрокаТоваров.ВидДокументаСертификации) Тогда
					...
					ДанныеТекущегоСертификата.Вставить("certificate_type",   ИнтерфейсИСМПОбщегоНазначения.ВидДокументаСертификации(СтрокаТоваров.ВидДокументаСертификации));
	...
	Иначе
		...
		ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(Шапка, Неопределено, ВидыПродукции);
		...
		ДоступнаДатаПроизводства = МаркировкаТоваровКлиентСервер.ДляТоварнойГруппыИспользуетсяДатаПроизводства(ВидыПродукции, Шапка.Операция);
		ДоступенСрокГодности = КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСоСрокамиГодности(ВидыПродукции,, Шапка.Операция);
		...
		Для Каждого СтрокаТоваров Из ИсточникДанных Цикл
			...
			РезультатРазбора = ШтрихкодированиеОбщегоНазначенияИС.НоваяСтруктураОбработкиШтрихкода(
				СтрокаТоваров.Штрихкод, ВидыПродукции, ПользовательскиеПараметрыРазбораКодаМаркировки);
...
Функция ОбработатьДанныеШтрихкодаПотребительскойУпаковки(Форма, ДанныеШтрихкода, ПараметрыСканирования, ПараметрыЗаполнения = Неопределено)
	...
	ДоступенСрокГодности = КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоВидПродукцииСоСрокамиГодности(
		ИсточникДанных.ТоварнаяГруппа,, ИсточникДанных.Операция);
