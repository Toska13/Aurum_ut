...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ТаблицаТоваровКоличествоУпаковокВПути.Видимость =
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТаблицаТоваровКоличествоУпаковокВПути.Имя);
...
Процедура ПерезаполнитьКоличествоПодобрано(СЗаполнением = Ложь, ТоварыРеализации = Неопределено, ШтрихкодыУпаковокРеализации = Неопределено)
	...
	Если СЗаполнением Тогда
		...
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
			СтруктураПоиска = Новый Структура("Номенклатура, Характеристика, КодСтроки, ЗаказКлиента");
		Иначе
	...
	ОбрабатываемыеСтрокиБезПересчетаРучнойСкидки = Новый Массив;
	Для каждого ТекСтрока Из ТаблицаТоваров Цикл
		
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
			КоличествоУпаковокКОформлению = ТекСтрока.КоличествоУпаковокВПути;
			КоличествоКОформлению = ТекСтрока.КоличествоВПути;
		Иначе
			КоличествоУпаковокКОформлению = ТекСтрока.КоличествоУпаковокВЗаказе;
			КоличествоКОформлению = ТекСтрока.КоличествоВЗаказе;
		КонецЕсли;
		
		КоличествоУпаковокДоИзм = ТекСтрока.КоличествоУпаковок;
		ТекСтрока.КоличествоУпаковок = ?(ПоОрдеру, ТекСтрока.КоличествоУпаковокВОрдере, КоличествоУпаковокКОформлению);
		ТекСтрока.Количество = ?(ПоОрдеру, ТекСтрока.КоличествоВОрдере, КоличествоКОформлению);
		Если ТекСтрока.КоличествоУпаковок <> КоличествоУпаковокДоИзм Тогда
			Если ТекСтрока.ЕстьРучноеИзменение ИЛИ КоличествоКОформлению = 0 Тогда
				ОбрабатываемыеСтроки.Добавить(ТекСтрока);
			Иначе
				КоэффициентПересчета = (ТекСтрока.Количество / КоличествоКОформлению);
				...
				ОбрабатываемыеСтрокиБезПересчетаРучнойСкидки.Добавить(ТекСтрока);
			КонецЕсли; 
		КонецЕсли;
		Если СЗаполнением Тогда
		...
		Если СЗаполнением Тогда
			Если (ТекСтрока.КоличествоУпаковокВНакладной = КоличествоУпаковокКОформлению
	...
	ОбрабатываемыеСтроки = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ОбрабатываемыеСтроки);
	ОбрабатываемыеСтроки = ОбщегоНазначенияКлиентСервер.РазностьМассивов(ОбрабатываемыеСтроки, МассивУдаляемыхСтрок);
	...
	ОбрабатываемыеСтрокиБезПересчетаРучнойСкидки
		= ОбщегоНазначенияКлиентСервер.РазностьМассивов(ОбрабатываемыеСтрокиБезПересчетаРучнойСкидки, МассивУдаляемыхСтрок);
		
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		ОбрабатываемыеСтрокиБезПересчетаРучнойСкидки,
		СтруктураДействийБезПересчетаРучнойСкидки,
		ТаблицаТоваров,
		КэшированныеЗначения);
