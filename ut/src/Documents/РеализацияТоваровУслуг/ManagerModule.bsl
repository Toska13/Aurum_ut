...
Процедура ЗаполнитьИменаРеквизитовПоХозяйственнойОперации(ХозяйственнаяОперация, МассивВсехРеквизитов, МассивРеквизитовОперации) Экспорт
	...
	МассивВсехРеквизитов.Добавить("НомерКомиссионера");
	МассивВсехРеквизитов.Добавить("ДатаКомиссионера");
	МассивВсехРеквизитов.Добавить("НаименованиеДокументаКомиссионера");
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияЧерезКомиссионера Тогда
		...
		МассивРеквизитовОперации.Добавить("Склад");
		МассивРеквизитовОперации.Добавить("Товары.Склад");
		МассивРеквизитовОперации.Добавить("НомерКомиссионера");
		МассивРеквизитовОперации.Добавить("ДатаКомиссионера");
		МассивРеквизитовОперации.Добавить("НаименованиеДокументаКомиссионера");
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	РеализацияПоПереходуПраваСобственности = Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности;
	
	Если РеализацияЧерезКомиссионера
		Или РеализацияПоПереходуПраваСобственности Тогда
	...
	ПараметрыУказанияСерий.ТолькоСерииДляСебестоимости = РеализацияПоРеглУчету
															Или РеализацияПоПереходуПраваСобственности;
...
Функция НаименованияВходящихДокументов() Экспорт
	...
	РеализацияТоваровУслугЛокализация.ДополнитьНаименованияВходящихДокументов(МассивНаименований);
...
Функция ПараметрыЗаполненияНалогообложенияНДСПродажи(Объект) Экспорт
	...
	ИначеЕсли ДанныеОбъекта.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда 
...
Функция ПараметрыВзаиморасчеты(ДанныеЗаполнения = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ЗначениеСсылочногоТипа(ДанныеЗаполнения) Тогда
		...
		ЗаказКлиента = СтруктураДанныеЗаполнения.ЗаказКлиента;
		Товары = СтруктураДанныеЗаполнения.Товары;
	...
	СтруктураПараметров.ИзменяетПланОплаты                   = ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.РеализацияКлиенту")
		Или ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПереходПраваСобственности")
	...
	РазрешитьРучноеРедактированиеГрафика = Ложь;
	
	Если ТипЗнч(РеализацияПоЗаказам) = Тип("Булево")
		И РеализацияПоЗаказам Тогда
		
		ДокументыОснования = Новый Массив;
		
		Если ЗначениеЗаполнено(ЗаказКлиента) Тогда
			ДокументыОснования.Добавить(ЗаказКлиента);
		Иначе
			
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ДокументыОснования,
				Товары.Выгрузить().ВыгрузитьКолонку("ЗаказКлиента"),
				Истина);
			
		КонецЕсли;
		
		ДокументыПоВидам = ПродажиСервер.ДокументыПоПризнакуНаличияОбъектовРасчетов(ДокументыОснования);
		РазрешитьРучноеРедактированиеГрафика = ДокументыПоВидам.ДокументыБезОбъектовРасчетов.Количество() > 0;
		
	КонецЕсли;
	
	Если РазрешитьРучноеРедактированиеГрафика Тогда
	...
	Если РеализацияЧерезКомиссионера Тогда
		...
		СтруктураПараметров.НомерВходящегоДокумента = "Объект.НомерКомиссионера";
		СтруктураПараметров.ДатаВходящегоДокумента = "Объект.ДатаКомиссионера";
		СтруктураПараметров.НаименованиеПервичногоДокумента = "Объект.НаименованиеДокументаКомиссионера";
	КонецЕсли;
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда 
		СтруктураПараметров.ДатаПереходаПраваСобственности   = "Объект.Дата";
	Иначе
	...
	СтруктураПараметров.ЕстьДатаПереходаПраваСобственности =
		...
		Или ПолучитьФункциональнуюОпцию("ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5")
			И ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности;
	...
	СтруктураПараметров.ТСП                                  = "Объект.Склад";
...
Функция ТекстЗапросаЗаполненияСтатусовУказанияСерий(ПараметрыУказанияСерий) Экспорт
	...
	|							ИЛИ &ОперацияДокумента = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
...
Функция ПроверкаПередСменойСтатуса(ВыборкаПроверки, НовыйСтатус, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ЗначениеНовогоСтатуса = Перечисления.СтатусыРеализацийТоваровУслуг.КПредоплате Тогда
		...
		Если ВыборкаПроверки.РеализацияПоЗаказам Тогда
			...
			Если ВыборкаПроверки.ХозяйственнаяОперация =
				Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
				
				ТекстОшибки = НСтр("ru='У документа %Документ% статус ""%Статус%"" не установлен, т.к. реализация оформлена по распоряжению'");
			Иначе
...
Функция СтруктураСостояниеВыполненияДокумента() Экспорт
	...
	СтруктураСостояние.Вставить("ВыводитьТаблицыДоставка", 3);
	...
	СтруктураСостояние.Вставить("ИмяРегистраОтгрузкаУслуг", "РаспоряженияНаОтгрузкуИВозврат");
...
Функция ПараметрыРегистрацииСчетовФактурВыданных(Объект) Экспорт
	...
	ПараметрыРегистрации.ПереходПраваСобственности = (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности);
...
Функция ПараметрыРегистрацииУведомленийОПеремещенииПрослеживаемыхТоваров(Объект) Экспорт
	...
	ПараметрыРегистрации.Вставить("ПереходПраваСобственности", (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности));
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	НЕОПРЕДЕЛЕНО КАК ПериодКорректировки
	|
	|ИЗ
	...
	ИспользоватьПодакцизныеТовары = Ложь;
	...
	Если Реквизиты.РеализацияПоЗаказу
		И Реквизиты.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
		
		ДокументыОснования = Новый Массив;
		
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "ЗаказКлиента, Товары");
		Если ЗначениеЗаполнено(ЗначенияРеквизитов.ЗаказКлиента) Тогда
			ДокументыОснования.Добавить(ЗначенияРеквизитов.ЗаказКлиента);
		Иначе
			
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ДокументыОснования,
				ЗначенияРеквизитов.Товары.Выгрузить().ВыгрузитьКолонку("ЗаказКлиента"),
				Истина);
			
		КонецЕсли;
		
		ДокументыПоВидам = ПродажиСервер.ДокументыПоПризнакуНаличияОбъектовРасчетов(ДокументыОснования);
		ЭтоПереходПраваСобственностиБезЗаказа = ДокументыПоВидам.ДокументыБезОбъектовРасчетов.Количество() > 0;
		ЭтоПереходПраваСобственностиНаОснованииОтгрузки = ДокументыПоВидам.ДокументыБезОбъектовРасчетов.Количество() = 1
			И ТипЗнч(ДокументыПоВидам.ДокументыБезОбъектовРасчетов.Получить(0)) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту");
		
	Иначе
		
		ЭтоПереходПраваСобственностиБезЗаказа = Ложь;
		ЭтоПереходПраваСобственностиНаОснованииОтгрузки = Ложь;
		
	КонецЕсли;
	
	Запрос.УстановитьПараметр("ЭтоПереходПраваСобственностиБезЗаказа", ЭтоПереходПраваСобственностиБезЗаказа);
	Запрос.УстановитьПараметр("ЭтоПереходПраваСобственностиНаОснованииОтгрузки", ЭтоПереходПраваСобственностиНаОснованииОтгрузки);
...
Процедура ТекстЗапросаДанныеДокументаШапка(СписокЗапросов)
	...
	|	ДанныеДокумента.ПорядокРасчетов КАК ПорядокРасчетов,
	|	ДанныеДокумента.НомерКомиссионера КАК НомерКомиссионера,
	|	ДанныеДокумента.ДатаКомиссионера КАК ДатаКомиссионера,
	|	ДанныеДокумента.НаименованиеДокументаКомиссионера КАК НаименованиеДокументаКомиссионера
...
Процедура ТекстЗапросаДанныеДокументаТаблицаВидыЗапасов(СписокЗапросов)
	...
	|	ТаблицаВидыЗапасов.ВидЗапасов.ОблагаетсяАкцизом КАК ОблагаетсяАкцизом,
	|	ТаблицаВидыЗапасов.ВидЗапасов.ПодразделениеПроизводства КАК ПодразделениеПроизводства,
...
Функция ТекстЗапросаТаблицаТоварыКОформлениюОтчетовКомитенту(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если НЕ ((ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту
			...
			ИЛИ ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности
...
Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	|			И НЕ &ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности),
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности),
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионераБезПереходаПраваСобственности))
	...
	|		КОГДА ТаблицаВидыЗапасов.ТипЗапасов <> ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
	|		 И &ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности),
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности))
...
Функция ТекстЗапросаТаблицаУслугиКОформлениюОтчетовПринципалу(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если (ДокументОбъект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.РеализацияКлиенту
			...
			И ДокументОбъект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ПереходПраваСобственности)
			Или ДокументОбъект.Статус <> Перечисления.СтатусыРеализацийТоваровУслуг.Отгружено Тогда
		Возврат "";
	КонецЕсли;
...
Процедура СформироватьСуммыДокументаВВалютахУчета(Запрос, ТекстыЗапроса, Регистры = Неопределено, ДокументОбъект = Неопределено) Экспорт
	...
	НеПересчитыватьСуммыНДС = (Запрос.Параметры.ХозяйственнаяОперация =
...
Функция ТекстЗапросаТаблицаРаспоряженияНаОтгрузку(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект = Неопределено)
	...
	Если ДокументОбъект = Неопределено
		Или (ДокументОбъект.Статус = Перечисления.СтатусыРеализацийТоваровУслуг.ВПути
		...
		|	ТаблицаТовары.КодСтроки КАК КодСтроки,
		|	ВЫБОР
		|		КОГДА ТаблицаТовары.НоменклатураТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),
		|			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
	...
	Если ДокументОбъект = Неопределено 
		...
		|	И ДанныеДокумента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.Отгружено)
		|	И ДанныеДокумента.РеализацияПоЗаказам
		|	И ДанныеДокумента.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
		|	И НЕ ЕСТЬNULL(ТаблицаТовары.ЗаказКлиентаЭтоЗаказКакСчет,
		...
		|	ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Оформлено) КАК Показатель,
		|	ТаблицаТовары.Количество КАК Количество,
		|	ТаблицаТовары.СуммаВзаиморасчетов КАК Сумма,
		|	ТаблицаТовары.СуммаНДС КАК СуммаНДС,
		|	ТаблицаТовары.СуммаНДСРегл КАК СуммаНДСРегл,
	...
	Если ДокументОбъект = Неопределено
		Или ДокументОбъект.РеализацияПоЗаказам Тогда
		
		ТекстЗапроса =
		// 1. Для хозяйственной операции <> Переход права собственности сверх заказа работает, как обычно -
		// добавляются движения заказа
		"ВЫБРАТЬ
		|	ТаблицаТовары.НомерСтроки КАК НомерСтроки,
		|	ДанныеДокумента.Дата КАК Период,
		|	ТаблицаТовары.ЗаказКлиента КАК Распоряжение,
		...
		|	ТаблицаТовары.КодСтроки КАК КодСтроки,
		|	ВЫБОР
		|		КОГДА ТаблицаПоказателей.Ссылка В (ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.КОтгрузке),
		...
		|ОБЪЕДИНИТЬ ВСЕ
		|
		// 2. Для [хозяйственной операции] = [переход права собственности] сверх заказа не работает -
		// добавляются только движения с показателем Оформлено. Так, потому что для случая сверх заказа в
		// переходе права собственности нужно еще добавлять движения с показателем Отгружено, что совсем неверно.
		// Поэтому для двухходовой реализации сверх заказа может быть в документе Отгрузка товаров клиенту
		// (с добавлением движений заказа), а в реализации завершение - добавление движений с показателем
		// Оформлено для [кода строки] = 0.
		|ВЫБРАТЬ
		|	ТаблицаТовары.НомерСтроки КАК НомерСтроки,
		|	ДанныеДокумента.Дата КАК Период,
		|	ТаблицаТовары.ЗаказКлиента КАК Распоряжение,
	...
	Если ДокументОбъект <> Неопределено
		И МассивЗапросов.Количество() = 0 Тогда
		Возврат "";
	КонецЕсли;
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ТекстУсловияЗаказКакСчет",
		"ВЫБОР
		|	КОГДА ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
		|		ТОГДА ИСТИНА
		|	ИНАЧЕ НЕ ЕСТЬNULL(ТаблицаТовары.ЗаказКлиентаЭтоЗаказКакСчет, НЕ &ИспользоватьРасширенныеВозможностиЗаказаКлиента)
		|КОНЕЦ");
...
Функция ТекстЗапросаТаблицаДвиженияСерийТоваров(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СкладскиеОперации.ДвижениеВФинансовомУчете)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СкладскиеОперации.ОтгрузкаКлиенту)
	|	КОНЕЦ,
	|	&Ссылка,
	|	&Период,
	|	ВЫБОР
	...
	|				ИЛИ ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоСкладскоеДвижение
	|ИЗ
...
Функция ТекстЗапросаВтТаблицаВидыЗапасов(Запрос, ТекстыЗапроса, ДокументОбъект)
	...
	|	ВЫБОР
	|		КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиентуРеглУчет) ИЛИ &ВернутьМногооборотнуюТару
	|			И Аналитика.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	|		ТОГДА НЕОПРЕДЕЛЕНО
	|		КОГДА &НалогообложениеНДС В (&ТипыНалогообложенияНДСНеУчитываетсяВСтоимости) И ВидыЗапасов.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|		КОГДА &НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт) И ВидыЗапасов.СтавкаНДС <> &СтавкаНДСНаЭкспорт
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
	|		КОГДА &НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт) И ВидыЗапасов.КодТНВЭДСырьевойТовар
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортСырьевыхТоваровУслуг)
	...
	|	ЕСТЬNULL(АналитикаРасчетов.КлючАналитики, ЕСТЬNULL(АналитикаРасчетовРеализацияЧерезКомиссионера.КлючАналитики, &АналитикаУчетаПоПартнерам)) КАК АналитикаУчетаПоПартнерам,
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК АналитикаРасчетовРеализацияЧерезКомиссионера
	|	ПО АналитикаРасчетов.Партнер = &КлиентПартнер
	|		И АналитикаРасчетов.Организация = &Организация
	|		И АналитикаРасчетов.Контрагент = &КлиентКонтрагент
	|		И АналитикаРасчетов.Договор = &КлиентДоговор
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	|	ВидыЗапасовТЧ.НоменклатураПартнера,
	...
	|			И ВидыЗапасовТЧ.НоменклатураПартнера = СтрокиТоваров.НоменклатураПартнера
	|
	|ГДЕ
	|	ВЫБОР
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	ИмяРегистра = "ВТ_ДанныеШапкиДокументаОбъединение";
	
	ТекстЗапросаДанныеШапкиДокументаОбъединение =
	"ВЫБРАТЬ
	|	ДанныеШапкиДокумента.Дата КАК Дата,
	...
	|ПОМЕСТИТЬ ВТ_ДанныеШапкиДокументаОбъединение
	|ИЗ
	|	(ВЫБРАТЬ
	...
	ТекстыЗапроса.Добавить(ТекстЗапросаДанныеШапкиДокументаОбъединение, ИмяРегистра);
	...
	|		ДанныеДокументаШапка КАК ДанныеДокументаШапка
	|	ГДЕ
	|		ДанныеДокументаШапка.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоДоговорамНакладным)) КАК
	|		ОбъектыРасчетовДокумента
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ДанныеШапкиДокументаОбъединение КАК ДанныеШапкиДокумента
	|		ПО ИСТИНА
	|ГДЕ
	|	ОбъектыРасчетовДокумента.ОбъектРасчетов <> ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)";
	
	ТекстЗапросаКурсыВалютДокумента = ВзаиморасчетыПроведение.ПолучитьТаблицуКурсовВалютНаправленийДеятельности(
		ТекстЗапросаКурсыВалютДокументаНаправленияДеятельности);
	...
	Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту
		...
		|	ВЫБОР
		|		КОГДА &ЭтоПереходПраваСобственностиБезЗаказа
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ДанныеДокументаШапка.РеализацияПоЗаказам
		|	КОНЕЦ КАК НакладнаяПоЗаказам,
		|	ВЫБОР
		|		КОГДА ДанныеДокументаШапка.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.Отгружено)
	...
	Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту
		...
		|	ВЫБОР
		|		КОГДА ДанныеДокументаШапка.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.КПредоплате)
		...
		|		ПО Таблица.Ссылка = ДанныеДокументаШапка.Ссылка
		|ГДЕ
		|	ДанныеДокументаШапка.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.Отгружено)
		|	ИЛИ ДанныеДокументаШапка.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.Отгружено)
		|		И ДанныеДокументаШапка.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиенту),
		|												ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиентуРеглУчет),
		|												ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности),
		|												ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионера),
		...
		|			И НЕ ЕСТЬNULL(ДанныеДокументаШапка.Договор.ЗаданГрафикИсполнения,ЛОЖЬ)
		|				ИЛИ ДанныеДокументаШапка.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоНакладным))";
	...
	Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту
		...
		|	ЕСТЬNULL(ТаблицаОбъектовРасчетов.Ссылка, Таблица.ОбъектРасчетов) КАК ОбъектРасчетов,
		|	ДанныеДокументаШапка.Дата КАК ДатаОтгрузки,
		|	ВЫБОР
		|		КОГДА ДанныеДокументаШапка.ХозяйственнаяОперация В
		|			(ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности),
		|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионераБезПереходаПраваСобственности))
		|		И НЕ ДанныеДокументаШапка.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.КПредоплате),
		...
		|	ВЫБОР
		|		КОГДА &ЭтоПереходПраваСобственностиНаОснованииОтгрузки
		|			ТОГДА Таблица.СуммаВзаиморасчетов
		|		КОГДА ДанныеДокументаШапка.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиенту),
		|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности),
		|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиентуРеглУчет),
		|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионера))
		|		ИЛИ ДанныеДокументаШапка.ХозяйственнаяОперация В
		...
		|	КОНЕЦ КАК УвеличитьКОтгрузке,
		|	ВЫБОР
		|		КОГДА &ЭтоПереходПраваСобственностиНаОснованииОтгрузки
		|			ТОГДА 0
		|		ИНАЧЕ Таблица.СуммаВзаиморасчетов
		|	КОНЕЦ КАК УменьшитьКОтгрузке,
		|	ВЫБОР
		|		КОГДА ДанныеДокументаШапка.ХозяйственнаяОперация В
		|			(ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионера),
		|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаНаКомиссию),
		...
		|	&ЭтоПереходПраваСобственностиНаОснованииОтгрузки КАК ЭтоПереходПраваСобственностиНаОснованииОтгрузки,
		|	ВЫБОР
		|		КОГДА Таблица.НоменклатураТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
		...
		|	ВЫБОР
		|		КОГДА &ЭтоПереходПраваСобственностиБезЗаказа
		|			ТОГДА &ЭтоПереходПраваСобственностиНаОснованииОтгрузки
		|		ИНАЧЕ ДанныеДокументаШапка.РеализацияПоЗаказам
		|	КОНЕЦ КАК НакладнаяПоЗаказам,
		|	ДанныеДокументаШапка.ПорядокРасчетов КАК ПорядокРасчетов,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеДокументаШапка КАК ДанныеДокументаШапка
		|		ПО Таблица.Ссылка = ДанныеДокументаШапка.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ОбъектыРасчетов КАК ТаблицаОбъектовРасчетов
		|		ПО &ЭтоПереходПраваСобственностиНаОснованииОтгрузки
		|		И ДанныеДокументаШапка.ПорядокРасчетов <> ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоДоговорамКонтрагентов)
		|		И Таблица.ЗаказКлиента = ТаблицаОбъектовРасчетов.Объект
		|		И НЕ ТаблицаОбъектовРасчетов.ПометкаУдаления
		|		И ТаблицаОбъектовРасчетов.Служебный
		|ГДЕ
	...
	|			(ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности),
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионераБезПереходаПраваСобственности))
	|		И НАЧАЛОПЕРИОДА(ДанныеДокументаШапка.Дата,
	|			ДЕНЬ) <> НАЧАЛОПЕРИОДА(ДанныеДокументаШапка.ДатаПереходаПраваСобственности, ДЕНЬ)
	...
	|	И НЕ (Таблица.ОбъектРасчетовОбъект ССЫЛКА Документ.ЗаказКлиента
	|	И Таблица.ОбъектРасчетовОрганизация = ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация))";
	
	#КонецОбласти
	
	ВзаиморасчетыПроведениеКлиенты.ПроведениеПродажи(Запрос, ТекстыЗапроса, Регистры, ТекстПродажа, ТекстПланированиеОплат, ТекстЗачетАванса, ТекстТовары);
	...
	ВзаиморасчетыПроведениеКлиенты.ПроведениеОплатыОтКлиента(Запрос, ТекстыЗапроса, Регистры, ТекстОплата);
	...
	ВзаиморасчетыПроведениеКлиенты.ПроведениеРасчетыСКлиентамиПланОплат(Запрос, ТекстыЗапроса, Регистры, ТекстПланированиеОплат);
	
КонецПроцедуры

Процедура ОтразитьРезерв(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	
	Если ДокументОбъект.Статус <> Перечисления.СтатусыРеализацийТоваровУслуг.КПредоплате
			Или ДокументОбъект.ХозяйственнаяОперация =
...
Процедура ОформитьОтгрузкуТоваров(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если ДокументОбъект.Статус = Перечисления.СтатусыРеализацийТоваровУслуг.КПредоплате
		...
		Или ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияЧерезКомиссионера
		Или ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияЧерезКомиссионераБезПереходаПраваСобственности
		Или ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
		Возврат;
	КонецЕсли;
...
Функция ОписаниеОперацийУчетаСебестоимости(ДокументОбъект) Экспорт
	...
	|	ТаблицаВидыЗапасов.Количество 			КАК Количество,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
	|			И ТаблицаДокумента.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	...
	|	КОНЕЦ									КАК ХозяйственнаяОперация,
	|	ТаблицаВидыЗапасов.ИдентификаторСтроки 	КАК ИдентификаторФинЗаписи,
	// Заполнение поля должно соответствовать функции ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж().
	|	&НастройкаХозяйственнойОперации	КАК НастройкаХозяйственнойОперации 
	|
	|ИЗ
	|	ДанныеДокументаШапка КАК ТаблицаДокумента
...
Функция ТекстЗапросаТаблицаДвиженияПоПрочимАктивамПассивам(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности
		И ДокументОбъект.Статус = Перечисления.СтатусыРеализацийТоваровУслуг.Отгружено Тогда
...
Функция ТекстЗапросаДоставка(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект, ДопПараметры)
	...
	Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности
	 ...
	 ИЛИ ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияЧерезКомиссионераБезПереходаПраваСобственности
	 ИЛИ ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиентуРеглУчет
	 ИЛИ ДокументОбъект.Статус = Перечисления.СтатусыРеализацийТоваровУслуг.КПредоплате
	 ИЛИ НЕ ДокументОбъект.ТребуетсяДоставка
	 ИЛИ ДокументОбъект.СпособДоставки25 = Перечисления.СпособыДоставки25.Самовывоз
	 ИЛИ ДокументОбъект.СпособДоставки25 = Перечисления.СпособыДоставки25.НашКурьер Тогда
		Возврат "";
	КонецЕсли;
	...
	|	НАЧАЛОПЕРИОДА(ШапкаДокумента.Дата, ДЕНЬ) КАК ПервичнаяДатаОтгрузки,
	|	ТаблицаТовары.Назначение КАК Назначение
	|ИЗ
	|	Документ.РеализацияТоваровУслуг.Товары КАК ТаблицаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг КАК ШапкаДокумента
	|		ПО ТаблицаТовары.Ссылка = ШапкаДокумента.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг.СкладыСведенияДоставки КАК ТаблицаСкладыСведенияДоставки
	...
	|	И &ТребуетсяДоставка
	|	И &Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.КПредоплате)
	|	И &ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|	И &ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионера)
	|	И &ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионераБезПереходаПраваСобственности)
	...
	|	ТаблицаТовары.Характеристика,
	|	ТаблицаТовары.Серия,
	|	ТаблицаТовары.КодСтрокиДоставки,
	|	ВЫБОР
	|		КОГДА ШапкаДокумента.ВидПланированияДоставки = ЗНАЧЕНИЕ(Перечисление.ВидыПланированияДоставки.ТекущимДокументом)
	|			ТОГДА &Ссылка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Документ.РеализацияТоваровУслуг.Товары", "ДанныеДокументаТаблицаТовары");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Документ.РеализацияТоваровУслуг КАК ШапкаДокумента", "ДанныеДокументаШапка КАК ШапкаДокумента");
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|		И ТабЧасть.Ссылка.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьАктыРасхожденийПослеОтгрузки") Тогда
		...
		Если КомандаСоздания <> Неопределено Тогда
			...
			КомандаСоздания.Представление = НСтр("ru = 'Акт о расхождениях после отгрузки товаров клиенту и перехода права собственности'");
		КонецЕсли;
	...
	#Область Документы_ЗаказНаДоставку
...
// 	СтрокаТаблицыЗначений, Неопределено -
//
Функция ДобавитьКомандуСоздатьПереходПраваСобственностиНаОснованииЗаказа(КомандыСозданияНаОсновании) Экспорт
	
	Если ПравоДоступа("Добавление", Метаданные.Документы.РеализацияТоваровУслуг) Тогда
		КомандаСоздатьНаОсновании = КомандыСозданияНаОсновании.Добавить();
		КомандаСоздатьНаОсновании.Обработчик =
			"СозданиеНаОснованииУТКлиент.ПереходПраваСобственностиНаОснованииЗаказа";
		КомандаСоздатьНаОсновании.Идентификатор = "ПереходПраваСобственностиНаОснованииЗаказа";
		КомандаСоздатьНаОсновании.Представление = НСтр("ru = 'Переход права собственности'");
		КомандаСоздатьНаОсновании.РежимЗаписи = "Проводить";
		КомандаСоздатьНаОсновании.МножественныйВыбор = Истина;
...
// Возвращает текст запроса заполнения документа на основании заказа клиента.
//
// Возвращаемое значение:
//  Строка -
//
Функция ТекстЗапросаЗаполненияНаОснованииЗаказа() Экспорт
	...
	|	ЗаказКлиента.АдресДоставкиЗначение КАК АдресДоставкиЗначение,
	|
	|	ВЫБОР
	|		КОГДА НЕ &ИспользуетсяДоставка20
	...
	|				И НЕ ЗаказКлиента.ЭтоЗаказКакСчет
	|			ТОГДА ЗаказКлиента.СпособДоставки25
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СпособыДоставки25.ПустаяСсылка)
	|	КОНЕЦ КАК СпособДоставки25,
	|
	|	ВЫБОР
	|		КОГДА &ИспользуетсяДоставка25
	|				И НЕ ЗаказКлиента.ЭтоЗаказКакСчет
	|			ТОГДА ЗаказКлиента.ТочкаДоставки
...
Процедура ДобавитьКомандыОтчетов(КомандыОтчетов, Параметры) Экспорт
	...
	КомандаОтчет = Отчеты.СостояниеДоставки.ДобавитьКомандуОтчета(
		КомандыОтчетов,
		Параметры,
		"СостояниеДоставкиКонтекст");
	
	Если КомандаОтчет <> Неопределено Тогда
		
		Операции = Новый Массив; // Массив из ПеречислениеСсылка.ХозяйственныеОперации
		...
		Операции.Добавить(Перечисления.ХозяйственныеОперации.РеализацияКлиентуРеглУчет);
		Операции.Добавить(Перечисления.ХозяйственныеОперации.ПереходПраваСобственности);
		...
		ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(
			КомандаОтчет,
			Метаданные.Документы.РеализацияТоваровУслуг.Реквизиты.Организация.Имя,
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	СинонимМакетаРеализацииТоваровУслуг = НСтр("ru = 'Реализация товаров'");
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыПечати, "ИспользоватьСинонимДляКомплекта", Ложь) Тогда
		СинонимМакетаРеализацииТоваровУслуг = ПараметрыПечати.СинонимМакетаДляКомплекта;
	КонецЕсли;
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Накладная") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			"Накладная",
			СинонимМакетаРеализацииТоваровУслуг,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "НакладнаяБезСкидок") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"НакладнаяБезСкидок",
			СинонимМакетаРеализацииТоваровУслуг,
	...
	ИначеЕсли (ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоЗаказам
		...
		Если МассивЗаказов <> Неопределено Тогда
			...
			Запрос = Новый Запрос(
				...
				|	NULL КАК НомерПоДаннымКлиента,
				|	NULL КАК ДатаПоДаннымКлиента,
				|	ОтгрузкаТоваров.Номер,
				|	ОтгрузкаТоваров.Дата,
				|	&СинонимОтгрузкиТоваров
				|ИЗ
				...
				|ГДЕ
				|	ПервичныйДокумент.Ссылка В(&МассивЗаказов)
				|");
			...
			Запрос.УстановитьПараметр("СинонимОтгрузкиТоваров", НСтр("ru='Отгрузка товаров клиенту'", ОбщегоНазначения.КодОсновногоЯзыка()));
			Выборка = Запрос.Выполнить().Выбрать();
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора,
	...
	Если ДанныеОтбора.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту
		Или ДанныеОтбора.ХозяйственнаяОперация =
			Перечисления.ХозяйственныеОперации.РеализацияКлиентуРеглУчет 
		Или ДанныеОтбора.ХозяйственнаяОперация =
		...
		ХозяйственныеОперацииРаспоряжений.Добавить(
			Перечисления.ХозяйственныеОперации.ОтгрузкаТоваровКлиенту);
	...
	Иначе
		...
		ТоварыРеализации.Колонки.Добавить("Количество", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
		...
		ТоварыРеализации.Колонки.Добавить("КоличествоВЗаказе", ОбщегоНазначенияУТ.ОписаниеТипаКоличественногоПоля());
		...
		ТоварыРеализации.Колонки.Добавить("СуммаНДСРегл", ОбщегоНазначенияУТ.ОписаниеТипаДенежногоПоля());
		ТоварыРеализации.Колонки.Добавить("СуммаНДСУпр", ОбщегоНазначенияУТ.ОписаниеТипаДенежногоПоля());
	...
	Запрос.Текст =
		...
		|	ТоварыРеализации.СуммаНДСРегл КАК СуммаНДСРегл,
		|	ТоварыРеализации.СуммаНДСУпр КАК СуммаНДСУпр,
		...
		|ГДЕ
		|	ВЫБОР
		|		КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ
		...
		|					И НЕ ЗаказКлиента.Соглашение.ИспользуютсяДоговорыКонтрагентов)
		...
		|				ИЛИ &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности))
		...
		|ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА ОтгрузкаТовары.Ссылка.Исправление
		|			ТОГДА ОтгрузкаТовары.Ссылка.ИсправляемыйДокумент
		|		ИНАЧЕ
		|			ОтгрузкаТовары.Ссылка
		|	КОНЕЦ КАК Ссылка,
		|	ВЫБОР
		|		КОГДА ОтгрузкаТовары.ЗаказКлиента ССЫЛКА Документ.ЗаказКлиента
		|			ТОГДА ВЫРАЗИТЬ(ОтгрузкаТовары.ЗаказКлиента КАК Документ.ЗаказКлиента).ЭтоЗаказКакСчет
		|		КОГДА ОтгрузкаТовары.ЗаказКлиента ССЫЛКА Документ.ЗаявкаНаВозвратТоваровОтКлиента
		|			ТОГДА ВЫРАЗИТЬ(ОтгрузкаТовары.ЗаказКлиента КАК Документ.ЗаявкаНаВозвратТоваровОтКлиента).ЭтоЗаказКакСчет
		|		ИНАЧЕ
		|			ЛОЖЬ
		...
		|ГДЕ
		|	ВЫБОР
		|		КОГДА ОтгрузкаТовары.ЗаказКлиента В (
		...
		|	И ВЫБОР
		|		КОГДА &ОтобратьПоЗаказу
		|			ТОГДА ОтгрузкаТовары.Ссылка В (&МассивЗаказов)
		|		ИНАЧЕ ОтгрузкаТовары.Ссылка.Партнер = &Партнер
		|			И ОтгрузкаТовары.Ссылка.Контрагент = &Контрагент
		|			И ОтгрузкаТовары.Ссылка.Договор = &Договор
		|			И ОтгрузкаТовары.Ссылка.Организация = &Организация
		|			И ОтгрузкаТовары.Ссылка.ХозяйственнаяОперация В (&ХозяйственныеОперацииРаспоряжений)
		|			И (НЕ &ИспользоватьСоглашенияСКлиентами
		|				ИЛИ ОтгрузкаТовары.Ссылка.Соглашение = &Соглашение)
		|			И ОтгрузкаТовары.Ссылка.Сделка = &Сделка
		|			И ОтгрузкаТовары.Ссылка.Валюта = &ВалютаВзаиморасчетов
		|			И ВЫБОР
		|				КОГДА ОтгрузкаТовары.Ссылка.НалогообложениеНДС В (
		|				                            ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортСырьевыхТоваровУслуг),
		|				                            ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортНесырьевыхТоваров)
		|				                            )
		|					ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт)
		|				ИНАЧЕ ОтгрузкаТовары.Ссылка.НалогообложениеНДС
		|			КОНЕЦ = &НалогообложениеНДС
		|			И &ЦенаВключаетНДС = ИСТИНА
		|			И (НЕ &ИспользоватьНаправленияДеятельности
		|				ИЛИ ОтгрузкаТовары.Ссылка.НаправлениеДеятельности = &НаправлениеДеятельности)
		|	КОНЕЦ
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВводОстатковТоваров.Партия КАК Ссылка,
		|	ЛОЖЬ КАК ЭтоЗаказКакСчет
		|ИЗ
		|	Документ.ВводОстатковТоваров КАК ВводОстатковТоваров
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументов
		|		ПО ВводОстатковТоваров.Ссылка = РеестрДокументов.СторнируемыйДокумент
		|			И РеестрДокументов.Проведен
		|			И НЕ РеестрДокументов.ДополнительнаяЗапись
		|ГДЕ
		|	ВЫБОР
		|		КОГДА &ОтобратьПоЗаказу
		|			ТОГДА ВводОстатковТоваров.Партия В (&МассивЗаказов)
		|		ИНАЧЕ ВводОстатковТоваров.Партнер = &Партнер
		|			И ВводОстатковТоваров.Контрагент = &Контрагент
		|			И ВводОстатковТоваров.Договор = &Договор
		|			И ВводОстатковТоваров.Организация = &Организация
		|			И ВводОстатковТоваров.ХозяйственнаяОперация В (&ХозяйственныеОперацииРаспоряжений)
		|			И ВводОстатковТоваров.ЦенаВключаетНДС = &ЦенаВключаетНДС
		|			И ВводОстатковТоваров.Валюта = &ВалютаВзаиморасчетов
		|			И ВЫБОР
		|				КОГДА ВводОстатковТоваров.Партия.НалогообложениеНДС В (ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортСырьевыхТоваровУслуг), ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортНесырьевыхТоваров))
		|					ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт)
		|					ИНАЧЕ ВводОстатковТоваров.Партия.НалогообложениеНДС
		|			КОНЕЦ = &НалогообложениеНДС
		|	КОНЕЦ
		|	И ВводОстатковТоваров.Проведен
		|	И РеестрДокументов.Ссылка ЕСТЬ NULL
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Ссылка
		...
		|//////////////////////////////////////////////////////////////////////////////// 4
		|ВЫБРАТЬ
		|	ОтгрузкаТовары.ЗаказКлиента КАК Распоряжение,
		|	ОтгрузкаТовары.Ссылка КАК ОтгрузкаТоваров
		|ПОМЕСТИТЬ ВТОтгрузкиТоваровПоРаспоряжениям
		|ИЗ
		|	Документ.ОтгрузкаТоваровКлиенту.Товары КАК ОтгрузкаТовары
		|ГДЕ
		|	ОтгрузкаТовары.Ссылка.ОтгрузкаПоЗаказам
		|	И НЕ ОтгрузкаТовары.ЗаказКлиента В(
		|				ЗНАЧЕНИЕ(Документ.ЗаказКлиента.ПустаяСсылка),
		|				ЗНАЧЕНИЕ(Документ.ОтгрузкаТоваровКлиенту.ПустаяСсылка),
		|				ЗНАЧЕНИЕ(Документ.ЗаявкаНаВозвратТоваровОтКлиента.ПустаяСсылка),
		|				НЕОПРЕДЕЛЕНО
		...
		|	И ВЫБОР
		|		КОГДА ОтгрузкаТовары.ЗаказКлиента ССЫЛКА Документ.ЗаказКлиента
		|			ТОГДА НЕ ВЫРАЗИТЬ(ОтгрузкаТовары.ЗаказКлиента КАК Документ.ЗаказКлиента).ЭтоЗаказКакСчет
		|		КОГДА ОтгрузкаТовары.ЗаказКлиента ССЫЛКА Документ.ЗаявкаНаВозвратТоваровОтКлиента
		|			ТОГДА НЕ ВЫРАЗИТЬ(ОтгрузкаТовары.ЗаказКлиента КАК Документ.ЗаявкаНаВозвратТоваровОтКлиента).ЭтоЗаказКакСчет
		|		ИНАЧЕ
		|			ИСТИНА
		|	КОНЕЦ
		|	И &ОтобратьПоЗаказу
		|	И ОтгрузкаТовары.Ссылка В (&МассивЗаказов)
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 5
		|	ВЫБРАТЬ
		|		РаспоряженияОформленоОбороты.Распоряжение КАК Распоряжение,
		|		РаспоряженияОформленоОбороты.Номенклатура КАК Номенклатура,
		|		РаспоряженияОформленоОбороты.Характеристика КАК Характеристика,
		|		РаспоряженияОформленоОбороты.КодСтроки КАК КодСтроки,
		|		РаспоряженияОформленоОбороты.ОтгруженоКоличествоОборот
		|			+ РаспоряженияОформленоОбороты.ОприходованоДоППСКоличествоОборот
		|			- РаспоряженияОформленоОбороты.СписаноДоППСКоличествоОборот
		|			- РаспоряженияОформленоОбороты.ОформленоКоличествоОборот
		|			- РаспоряженияОформленоОбороты.ПринятоКоличествоОборот КАК КОформлению,
		|		РаспоряженияОформленоОбороты.ОтгруженоСуммаОборот
		|			+ РаспоряженияОформленоОбороты.ОприходованоДоППССуммаОборот
		|			- РаспоряженияОформленоОбороты.СписаноДоППССуммаОборот
		|			- РаспоряженияОформленоОбороты.ОформленоСуммаОборот
		|			- РаспоряженияОформленоОбороты.ПринятоСуммаОборот КАК Сумма,
		|		РаспоряженияОформленоОбороты.ОформленоСчетовФактурСуммаНДСРеглОборот
		|			- РаспоряженияОформленоОбороты.ОформленоСуммаНДСРеглОборот КАК СуммаНДСРегл,
		|		РаспоряженияОформленоОбороты.ОформленоСчетовФактурСуммаНДСУпрОборот
		|			- РаспоряженияОформленоОбороты.ОформленоСуммаНДСУпрОборот КАК СуммаНДСУпр
		|	ПОМЕСТИТЬ ВТРаспоряженияОформленоОтгрузкиПоРаспоряжениям
		|	ИЗ
		|		#РаспоряженияНаОтгрузкуОборотыРазвернутая1 КАК РаспоряженияОформленоОбороты
		|	ГДЕ
		|		&ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
		|		И (РаспоряженияОформленоОбороты.ОтгруженоКоличествоОборот
		|			+ РаспоряженияОформленоОбороты.ОприходованоДоППСКоличествоОборот
		|			- РаспоряженияОформленоОбороты.СписаноДоППСКоличествоОборот
		|			- РаспоряженияОформленоОбороты.ОформленоКоличествоОборот
		|			- РаспоряженияОформленоОбороты.ПринятоКоличествоОборот <> 0
		|			ИЛИ РаспоряженияОформленоОбороты.ОтгруженоСуммаОборот
		|				+ РаспоряженияОформленоОбороты.ОприходованоДоППССуммаОборот
		|				- РаспоряженияОформленоОбороты.СписаноДоППССуммаОборот
		|				- РаспоряженияОформленоОбороты.ОформленоСуммаОборот
		|				- РаспоряженияОформленоОбороты.ПринятоСуммаОборот <> 0
		|			ИЛИ РаспоряженияОформленоОбороты.ОформленоСчетовФактурСуммаНДСРеглОборот
		|				- РаспоряженияОформленоОбороты.ОформленоСуммаНДСРеглОборот <> 0
		|			ИЛИ РаспоряженияОформленоОбороты.ОформленоСчетовФактурСуммаНДСУпрОборот
		|				- РаспоряженияОформленоОбороты.ОформленоСуммаНДСУпрОборот <> 0)
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 6
		|ВЫБРАТЬ
		...
		|	СУММА(ТаблицаЗаказы.СуммаНДСРегл) КАК СуммаНДСРегл,
		|	СУММА(ТаблицаЗаказы.СуммаНДСУпр) КАК СуммаНДСУпр
		|ПОМЕСТИТЬ ТаблицаОстатки
		|ИЗ
		|	(ВЫБРАТЬ
		|		РаспоряженияНаОтгрузкуИВозвратОбороты.Распоряжение КАК ЗаказКлиента,
		|		РаспоряженияНаОтгрузкуИВозвратОбороты.Номенклатура КАК Номенклатура,
		|		РаспоряженияНаОтгрузкуИВозвратОбороты.Характеристика КАК Характеристика,
		|		РаспоряженияНаОтгрузкуИВозвратОбороты.КодСтроки КАК КодСтроки,
		|		МАКСИМУМ(РаспоряженияНаОтгрузкуИВозвратОбороты.Склад) КАК Склад,
		|		СУММА(РаспоряженияНаОтгрузкуИВозвратОбороты.КОтгрузкеКоличество
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.ОтгруженоКоличество) КАК КОформлению,
		|		СУММА(РаспоряженияНаОтгрузкуИВозвратОбороты.КОтгрузкеСумма
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.ОтгруженоСумма) КАК Сумма,
		|		СУММА(РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСчетовФактурСуммаНДСРегл
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСуммаНДСРегл) КАК СуммаНДСРегл,
		|		СУММА(РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСчетовФактурСуммаНДСУпр
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСуммаНДСУпр) КАК СуммаНДСУпр
		|	ИЗ
		|		(
		|			ВЫБРАТЬ
		...
		|					) КАК ТаблицаОбороты
		|		) КАК РаспоряженияНаОтгрузкуИВозвратОбороты
		|	ГДЕ
		|		&ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
		|
		|	СГРУППИРОВАТЬ ПО
		|		РаспоряженияНаОтгрузкуИВозвратОбороты.Распоряжение,
		|		РаспоряженияНаОтгрузкуИВозвратОбороты.Номенклатура,
		|		РаспоряженияНаОтгрузкуИВозвратОбороты.Характеристика,
		|		РаспоряженияНаОтгрузкуИВозвратОбороты.КодСтроки
		|	ИМЕЮЩИЕ
		|		СУММА(РаспоряженияНаОтгрузкуИВозвратОбороты.КОтгрузкеКоличество
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.ОтгруженоКоличество) <> 0
		|		 ИЛИ СУММА(РаспоряженияНаОтгрузкуИВозвратОбороты.КОтгрузкеСумма
		|				- РаспоряженияНаОтгрузкуИВозвратОбороты.ОтгруженоСумма) <> 0
		|			ИЛИ СУММА(РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСчетовФактурСуммаНДСРегл
		|				- РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСуммаНДСРегл) <> 0
		|			ИЛИ СУММА(РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСчетовФактурСуммаНДСУпр
		|				- РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСуммаНДСУпр) <> 0
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		|	ВЫБРАТЬ
		...
		|		РаспоряженияНаОтгрузкуИВозвратОбороты.КодСтроки КАК КодСтроки,
		|		ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка) КАК Склад,
		|		СУММА(РаспоряженияНаОтгрузкуИВозвратОбороты.ОтгруженоКоличество
		|			+ РаспоряженияНаОтгрузкуИВозвратОбороты.ОприходованоДоППСКоличество
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.СписаноДоППСКоличество
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.ПринятоКоличество
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоКоличество) КАК КОформлению,
		|		СУММА(РаспоряженияНаОтгрузкуИВозвратОбороты.ОтгруженоСумма
		|			+ РаспоряженияНаОтгрузкуИВозвратОбороты.ОприходованоДоППССумма
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.СписаноДоППССумма
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.ПринятоСумма
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСумма) КАК Сумма,
		|		СУММА(РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСчетовФактурСуммаНДСРегл
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСуммаНДСРегл) КАК СуммаНДСРегл,
		|		СУММА(РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСчетовФактурСуммаНДСУпр
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСуммаНДСУпр) КАК СуммаНДСУпр
		|	ИЗ
		|		#РаспоряженияНаОтгрузкуОборотыРазвернутая2 КАК РаспоряженияНаОтгрузкуИВозвратОбороты
		|	ГДЕ
		|		&ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
		|	
		|	СГРУППИРОВАТЬ ПО
		...
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.СписаноДоППСКоличество
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.ПринятоКоличество
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоКоличество) <> 0
		|		ИЛИ СУММА(РаспоряженияНаОтгрузкуИВозвратОбороты.ОтгруженоСумма
		|			+ РаспоряженияНаОтгрузкуИВозвратОбороты.ОприходованоДоППССумма
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.СписаноДоППССумма
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.ПринятоСумма
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСумма) <> 0
		|		ИЛИ СУММА(РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСчетовФактурСуммаНДСРегл
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСуммаНДСРегл) <> 0
		|		ИЛИ СУММА(РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСчетовФактурСуммаНДСУпр
		|			- РаспоряженияНаОтгрузкуИВозвратОбороты.ОформленоСуммаНДСУпр) <> 0
		...
		|	
		|	ВЫБРАТЬ
		|		РаспоряженияОтгружено.Распоряжение КАК ЗаказКлиента,
		|		РаспоряженияОтгружено.Номенклатура КАК Номенклатура,
		|		РаспоряженияОтгружено.Характеристика КАК Характеристика,
		|		РаспоряженияОтгружено.КодСтроки КАК КодСтроки,
		|		ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка) КАК Склад,
		|		ВЫБОР
		|			КОГДА РаспоряженияОтгружено.ОтгруженоКоличество < ЕСТЬNULL(РаспоряженияОформленоОбороты.КОформлению, 0)
		|				ТОГДА РаспоряженияОтгружено.ОтгруженоКоличество
		|			ИНАЧЕ
		|				ЕСТЬNULL(РаспоряженияОформленоОбороты.КОформлению, 0)
		|		КОНЕЦ КАК КОформлению,
		|		ВЫБОР
		|			КОГДА РаспоряженияОтгружено.ОтгруженоСумма < ЕСТЬNULL(РаспоряженияОформленоОбороты.Сумма, 0)
		|				ТОГДА РаспоряженияОтгружено.ОтгруженоСумма
		|			ИНАЧЕ
		|				ЕСТЬNULL(РаспоряженияОформленоОбороты.Сумма, 0)
		|		КОНЕЦ КАК Сумма,
		|		ЕСТЬNULL(РаспоряженияОформленоОбороты.СуммаНДСРегл, 0) КАК СуммаНДСРегл,
		|		ЕСТЬNULL(РаспоряженияОформленоОбороты.СуммаНДСУпр, 0) КАК СуммаНДСУпр
		|	ИЗ
		|		#РаспоряженияНаОтгрузкуДвиженияРазвернутая3 КАК РаспоряженияОтгружено
		|			ЛЕВОЕ СОЕДИНЕНИЕ ВТРаспоряженияОформленоОтгрузкиПоРаспоряжениям КАК РаспоряженияОформленоОбороты
		|				ПО РаспоряженияОтгружено.Распоряжение = РаспоряженияОформленоОбороты.Распоряжение
		|					И РаспоряженияОтгружено.Номенклатура = РаспоряженияОформленоОбороты.Номенклатура
		|					И РаспоряженияОтгружено.Характеристика = РаспоряженияОформленоОбороты.Характеристика
		|					И РаспоряженияОтгружено.КодСтроки = РаспоряженияОформленоОбороты.КодСтроки
		|	ГДЕ
		|		&ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
		|		И (ВЫБОР
		|			КОГДА РаспоряженияОтгружено.ОтгруженоКоличество < ЕСТЬNULL(РаспоряженияОформленоОбороты.КОформлению, 0)
		...
		|					ЕСТЬNULL(РаспоряженияОформленоОбороты.Сумма, 0)
		|			КОНЕЦ <> 0
		|			ИЛИ ЕСТЬNULL(РаспоряженияОформленоОбороты.СуммаНДСРегл, 0) <> 0
		|			ИЛИ ЕСТЬNULL(РаспоряженияОформленоОбороты.СуммаНДСУпр, 0) <> 0)
		|		
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|		РаспоряженияДвижения.СуммаНДСРегл КАК СуммаНДСРегл,
		|		РаспоряженияДвижения.СуммаНДСУпр КАК СуммаНДСУпр
		|	ИЗ
		...
		|		И ВЫБОР
		|			КОГДА &ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
		|				ТОГДА РаспоряженияДвижения.Показатель = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Отгружено)
		|			ИНАЧЕ
		|				РаспоряженияДвижения.Показатель = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Оформлено)
		|		КОНЕЦ
		...
		|		И ВЫБОР
		|				КОГДА &СкладРеализации = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		|					ТОГДА ИСТИНА
		|				КОГДА ВЫРАЗИТЬ(&СкладРеализации КАК Справочник.Склады).ЭтоГруппа
		...
		|//////////////////////////////////////////////////////////////////////////////// 7
		|ВЫБРАТЬ
		|	Распоряжение КАК Ссылка,
		|	МАКСИМУМ(Регистратор) КАК ИсправлениеСсылка
		|ПОМЕСТИТЬ ТаблицаИсправлений
		|ИЗ
		...
		|		ИЗ
		|			ВТЗаказы)
		|	И ТИПЗНАЧЕНИЯ(Регистратор) = ТИП(Документ.ОтгрузкаТоваровКлиенту)
		|	И ТИПЗНАЧЕНИЯ(Распоряжение) = ТИП(Документ.ОтгрузкаТоваровКлиенту)
		|СГРУППИРОВАТЬ ПО
		|	Распоряжение
		|;
		|
		...
		|	ТаблицаОстатки.СуммаНДСРегл КАК СуммаНДСРегл,
		|	ТаблицаОстатки.СуммаНДСУпр КАК СуммаНДСУпр,
		...
		|		ЕСТЬNULL(
		|			ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, &ТекстЗапросаКоэффициентУпаковки2),
		...
		|		И ЗаказТоварыСклад.ДатаНачалаОрдернойСхемыПриОтгрузке <=
		|			&ТекущаяДата КАК ОрдернаяСхемаПриОтгрузке,
		|	ВЫБОР
		...
		|	ТаблицаОстатки.Сумма КАК СуммаВзаиморасчетов,
		|	ТаблицаОстатки.Склад КАК Склад,
		|	ТаблицаОстатки.Количество КАК КоличествоВПути,
		|	ЕСТЬNULL(НакладнаяТовары.Количество, 0) КАК КоличествоВНакладной,
		|	ОтгрузкаТовары.НомерСтроки КАК НомерСтроки,
		|	ОтгрузкаТовары.Ссылка.Сделка КАК Сделка,
		|	ОтгрузкаТовары.Ссылка.Дата КАК ДатаОтгрузки,
		|	ЕСТЬNULL(НакладнаяТовары.Упаковка, ОтгрузкаТовары.Упаковка) КАК Упаковка,
		|	ОтгрузкаТовары.Серия КАК Серия,
		|	ОтгрузкаТовары.ВидЦены КАК ВидЦены,
		|	ОтгрузкаТовары.Количество КАК КоличествоВЗаказе,
		|	ВЫБОР
		|		КОГДА ОтгрузкаТовары.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
		|			ТОГДА ТаблицаОстатки.Сумма / ТаблицаОстатки.Количество
		|		ИНАЧЕ
		|			ТаблицаОстатки.Сумма / ТаблицаОстатки.Количество
		...
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.КлассификаторТНВЭД.ПустаяСсылка)
		|	КОНЕЦ КАК КодТНВЭД,
		|	ТаблицаОстатки.Сумма КАК Сумма,
		|	0 КАК СуммаНДС,
		|	ТаблицаОстатки.СуммаНДСРегл КАК СуммаНДСРегл,
		|	ТаблицаОстатки.СуммаНДСУпр КАК СуммаНДСУпр,
		...
		|	0 КАК СуммаБонусныхБалловКСписанию,
		|	0 КАК СуммаБонусныхБалловКСписаниюВВалюте,
		|	0 КАК СуммаНачисленныхБонусныхБалловВВалюте,
		|	ТаблицаОстатки.Номенклатура.ТипНоменклатуры КАК ТипНоменклатуры,
		|	ОтгрузкаТовары.НоменклатураНабора КАК НоменклатураНабора,
		|	ОтгрузкаТовары.ХарактеристикаНабора КАК ХарактеристикаНабора,
		|	ОтгрузкаТовары.Ссылка.Подразделение КАК Подразделение,
		|	0 КАК КлючСвязи,
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(НакладнаяТовары.Упаковка, ОтгрузкаТовары.Упаковка) = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
		|			ТОГДА 1
		|		ИНАЧЕ ЕСТЬNULL(ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, &ТекстЗапросаКоэффициентУпаковки4), 1)
		|	КОНЕЦ КАК Коэффициент,
		|	ТаблицаОстатки.Количество /
		|		ЕСТЬNULL(
		|			ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, &ТекстЗапросаКоэффициентУпаковки4),
		|			1) КАК КоличествоУпаковок,
		|	ОтгрузкаТовары.СрокПоставки КАК СрокПоставки,
		|	ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ПустаяСсылка) КАК ВариантОбеспечения,
		|	ВЫБОР
		|		КОГДА ОтгрузкаТовары.Обособленно
		|			ТОГДА ОтгрузкаТовары.Назначение
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|	КОНЕЦ КАК Назначение,
		|	ОтгрузкаТовары.Склад.ИспользоватьОрдернуюСхемуПриОтгрузке = ИСТИНА
		|		И ОтгрузкаТовары.Склад.ДатаНачалаОрдернойСхемыПриОтгрузке <=
		|			&ТекущаяДата КАК ОрдернаяСхемаПриОтгрузке,
		|	ОтгрузкаТовары.Склад.ИспользоватьОрдернуюСхемуПриОтгрузке = ИСТИНА
		|		И ОтгрузкаТовары.Склад.ДатаНачалаОрдернойСхемыПриОтгрузке <=
		|			&ТекущаяДата КАК ПроверятьОтгрузку
		|ИЗ
		|	ТаблицаОстатки КАК ТаблицаОстатки
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаИсправлений КАК ТаблицаИсправлений
		|		ПО ТаблицаОстатки.ЗаказКлиента = ТаблицаИсправлений.Ссылка
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтгрузкаТоваровКлиенту.Товары КАК ОтгрузкаТовары
		|		ПО ТаблицаОстатки.Номенклатура = ОтгрузкаТовары.Номенклатура
		|			И ТаблицаОстатки.Характеристика = ОтгрузкаТовары.Характеристика
		|			И ТаблицаОстатки.КодСтроки = ОтгрузкаТовары.КодСтроки
		|			И ТаблицаИсправлений.ИсправлениеСсылка = ОтгрузкаТовары.Ссылка
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВтТоварыРеализацииПоЗаказу КАК НакладнаяТовары
		|		ПО ТаблицаОстатки.Номенклатура = НакладнаяТовары.Номенклатура
		|			И ТаблицаОстатки.Характеристика = НакладнаяТовары.Характеристика
		|			И ТаблицаОстатки.КодСтроки = НакладнаяТовары.КодСтроки
		...
		|		ПО СтавкиНДСНоменклатуры.Номенклатура = ОтгрузкаТовары.Номенклатура
		...
		|		И Заказ.Склад.ДатаНачалаОрдернойСхемыПриОтгрузке <=
		|			&ТекущаяДата КАК ОрдернаяСхемаПриОтгрузке,
		|	ВЫБОР
		|		КОГДА ЗаказТовары.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))
		|			ТОГДА ЛОЖЬ
		...
		|	ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка) КАК Склад,
		|	МАКСИМУМ(ТаблицаОстатки.Количество) КАК КоличествоВПути,
		|	СУММА(ЕСТЬNULL(НакладнаяТовары.Количество, 0)) КАК КоличествоВНакладной,
		|	МАКСИМУМ(ЕСТЬNULL(ОтгрузкаТовары.НомерСтроки, 0)) КАК НомерСтроки,
		|	МАКСИМУМ(ЕСТЬNULL(ОтгрузкаТовары.Ссылка.Сделка, ЗНАЧЕНИЕ(Справочник.СделкиСКлиентами.ПустаяСсылка))) КАК Сделка,
		|	МАКСИМУМ(ЕСТЬNULL(ОтгрузкаТовары.Ссылка.Дата, ДАТАВРЕМЯ(1, 1, 1))) КАК ДатаОтгрузки,
		|	ТаблицаОстатки.Номенклатура.ЕдиницаИзмерения КАК Упаковка,
		|	МАКСИМУМ(ВЫБОР
		|		КОГДА ОтгрузкаТовары.Серия ЕСТЬ НЕ NULL И ОтгрузкаТовары.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|			ТОГДА ОтгрузкаТовары.Серия
		...
		|	МАКСИМУМ(ВЫБОР
		|		КОГДА ТаблицаОстатки.Количество = 0
		|			ТОГДА 0
		|		ИНАЧЕ
		|			ТаблицаОстатки.Сумма / ТаблицаОстатки.Количество
		|	КОНЕЦ) КАК Цена,
		|	МАКСИМУМ(ВЫБОР
		|		КОГДА ОтгрузкаТовары.Ссылка ЕСТЬ NULL
		...
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.КлассификаторТНВЭД.ПустаяСсылка)
		|	КОНЕЦ КАК КодТНВЭД,
		|	МАКСИМУМ(ТаблицаОстатки.Сумма) КАК Сумма,
		|	0 КАК СуммаНДС,
		|	МАКСИМУМ(ТаблицаОстатки.СуммаНДСРегл) КАК СуммаНДСРегл,
		|	МАКСИМУМ(ТаблицаОстатки.СуммаНДСУпр) КАК СуммаНДСУпр,
		...
		|	0 КАК СуммаНачисленныхБонусныхБалловВВалюте,
		|	ТаблицаОстатки.Номенклатура.ТипНоменклатуры КАК ТипНоменклатуры,
		|	МАКСИМУМ(ЕСТЬNULL(ОтгрузкаТовары.НоменклатураНабора, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))) КАК НоменклатураНабора,
		|	МАКСИМУМ(ЕСТЬNULL(ОтгрузкаТовары.ХарактеристикаНабора, ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка))) КАК ХарактеристикаНабора,
		|	МАКСИМУМ(ЕСТЬNULL(ОтгрузкаТовары.Ссылка.Подразделение, ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка))) КАК Подразделение,
		|	0 КАК КлючСвязи,
		|	1 КАК Коэффициент,
		|	МАКСИМУМ(ТаблицаОстатки.Количество) КАК КоличествоУпаковок,
		|	МАКСИМУМ(ЕСТЬNULL(ОтгрузкаТовары.СрокПоставки, 0)) КАК СрокПоставки,
		|	ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ПустаяСсылка) КАК ВариантОбеспечения,
		|	ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка) КАК Назначение,
		|	МАКСИМУМ(ВЫБОР
		|		КОГДА ОтгрузкаТовары.Склад ЕСТЬ NULL
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ
		|			ОтгрузкаТовары.Склад.ИспользоватьОрдернуюСхемуПриОтгрузке = ИСТИНА
		|				И ОтгрузкаТовары.Склад.ДатаНачалаОрдернойСхемыПриОтгрузке <= &ТекущаяДата
		...
		|ИЗ
		|	ТаблицаОстатки КАК ТаблицаОстатки
		|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтгрузкаТоваровКлиенту.Товары КАК ОтгрузкаТовары
		|		ПО ТаблицаОстатки.Номенклатура = ОтгрузкаТовары.Номенклатура
		|			И ТаблицаОстатки.Характеристика = ОтгрузкаТовары.Характеристика
		|			И ТаблицаОстатки.КодСтроки = ОтгрузкаТовары.КодСтроки
		|			И ОтгрузкаТовары.КодСтроки = 0
		|			И ТаблицаОстатки.ЗаказКлиента = ОтгрузкаТовары.ЗаказКлиента
		|			И ОтгрузкаТовары.Ссылка.Проведен
		...
		|	И ЕСТЬNULL(ОтгрузкаТовары.Ссылка, ОприходованиеТовары.Ссылка) ЕСТЬ НЕ NULL
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ТаблицаОстатки.Количество КАК КоличествоВПути,
		|	ЕСТЬNULL(НакладнаяТовары.Количество, 0) КАК КоличествоВНакладной,
		|	ВводОстатковТовары.НомерСтроки КАК НомерСтроки,
		|	ЗНАЧЕНИЕ(Справочник.СделкиСКлиентами.ПустаяСсылка) КАК Сделка,
		|	ВводОстатковТовары.Ссылка.Дата КАК ДатаОтгрузки,
		|	ЕСТЬNULL(НакладнаяТовары.Упаковка, ВводОстатковТовары.Упаковка) КАК Упаковка,
		|	ВводОстатковТовары.Серия КАК Серия,
		|	ЗНАЧЕНИЕ(Справочник.ВидыЦен.ПустаяСсылка) КАК ВидЦены,
		|	ВводОстатковТовары.Количество КАК КоличествоВЗаказе,
		|	ВЫБОР
		...
		|	КОНЕЦ КАК Цена,
		|	&СтавкаНДСВводОстатковТоваров КАК СтавкаНДС,
		|	ВЫБОР
		|		КОГДА &НалогообложениеНДСЭкспорт
		|			ТОГДА ТаблицаОстатки.КодТНВЭД
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.КлассификаторТНВЭД.ПустаяСсылка)
		...
		|	ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка) КАК НоменклатураНабора,
		|	ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка) КАК ХарактеристикаНабора,
		|	ВводОстатковТовары.Ссылка.Подразделение КАК Подразделение,
		|	0 КАК КлючСвязи,
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(НакладнаяТовары.Упаковка, ВводОстатковТовары.Упаковка) = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
		|			ТОГДА 1
		|		ИНАЧЕ ЕСТЬNULL(ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, &ТекстЗапросаКоэффициентУпаковки5), 1)
		|	КОНЕЦ КАК Коэффициент,
		|	ТаблицаОстатки.Количество /
		|		ЕСТЬNULL(
		|			ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, &ТекстЗапросаКоэффициентУпаковки5),
		|			1) КАК КоличествоУпаковок,
		|	ДАТАВРЕМЯ(1, 1, 1) КАК СрокПоставки,
		|	ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ПустаяСсылка) КАК ВариантОбеспечения,
		|	ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка) КАК Назначение,
		|	ЛОЖЬ КАК ОрдернаяСхемаПриОтгрузке,
		|	ЛОЖЬ КАК ПроверятьОтгрузку
		|ИЗ
		|	ТаблицаОстатки КАК ТаблицаОстатки
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВводОстатковТоваров.Товары КАК ВводОстатковТовары
		|		ПО ТаблицаОстатки.Номенклатура = ВводОстатковТовары.Номенклатура
		|			И ТаблицаОстатки.Характеристика = ВводОстатковТовары.Характеристика
		|			И ТаблицаОстатки.КодСтроки = ВводОстатковТовары.КодСтроки
		|			И ТаблицаОстатки.ЗаказКлиента = ВводОстатковТовары.Ссылка.Партия
		...
		|			И ТаблицаОстатки.ЗаказКлиента = НакладнаяТовары.ЗаказКлиента
		|	ЛЕВОЕ СОЕДИНЕНИЕ СтавкиНДСНоменклатуры КАК СтавкиНДСНоменклатуры
		|		ПО СтавкиНДСНоменклатуры.Номенклатура = ВводОстатковТовары.Номенклатура
		|	ЛЕВОЕ СОЕДИНЕНИЕ ОсновныеСтавкиНДС КАК ОсновныеСтавкиНДС
		|		ПО ИСТИНА
		// Если для ВНО есть исправление, то работает с последним исправлением
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументов
		...
		|	ТоварыРеализации.СуммаНДСРегл,
		|	ТоварыРеализации.СуммаНДСУпр,
		...
		|	Документ.ОтгрузкаТоваровКлиенту.Товары КАК ОтгрузкаТовары
		|ГДЕ
		|	ОтгрузкаТовары.Ссылка В(&МассивЗаказов)
		|	И ОтгрузкаТовары.Номенклатура.ВариантОформленияПродажи В(&МассивВариантовОформления)
		|	И ОтгрузкаТовары.Ссылка.Проведен
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА ОтгрузкаШапка.ОтгрузкаПоЗаказам
		|			ТОГДА ОтгрузкаТовары.ЗаказКлиента
		|		ИНАЧЕ ОтгрузкаШапка.Ссылка
		|	КОНЕЦ КАК Ссылка
		|ИЗ
		|	Документ.ОтгрузкаТоваровКлиенту.Товары КАК ОтгрузкаТовары
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтгрузкаТоваровКлиенту КАК ОтгрузкаШапка
		|		ПО ОтгрузкаТовары.Ссылка = ОтгрузкаШапка.Ссылка
		|ГДЕ
		|	ВЫБОР
		|			КОГДА &ОтобратьПоЗаказу
		|				ТОГДА ОтгрузкаШапка.Ссылка В (&МассивЗаказов)
		|						ИЛИ ОтгрузкаШапка.Ссылка = &Регистратор
		|			ИНАЧЕ ОтгрузкаШапка.Партнер = &Партнер
		|					И ОтгрузкаШапка.Контрагент = &Контрагент
		|					И ОтгрузкаШапка.Договор = &Договор
		|					И ОтгрузкаШапка.Организация = &Организация
		|					И ОтгрузкаШапка.ХозяйственнаяОперация В (&ХозяйственныеОперацииРаспоряжений)
		|					И (НЕ &ИспользоватьСоглашенияСКлиентами
		|						ИЛИ ОтгрузкаШапка.Соглашение = &Соглашение)
		|					И ОтгрузкаШапка.Сделка = &Сделка
		|					И ОтгрузкаШапка.Валюта = &ВалютаВзаиморасчетов
		|					И (НЕ &ИспользоватьНаправленияДеятельности
		|						ИЛИ ОтгрузкаШапка.НаправлениеДеятельности = &НаправлениеДеятельности)
		|		КОНЕЦ
		|	И ВЫБОР
		|			КОГДА ВЫРАЗИТЬ(&СкладРеализации КАК Справочник.Склады).ЭтоГруппа
		|				ТОГДА ОтгрузкаШапка.Склад В ИЕРАРХИИ (&СкладРеализации)
		|						ИЛИ ОтгрузкаШапка.Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		|			ИНАЧЕ ОтгрузкаШапка.Склад В (ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка), &СкладРеализации)
		|					ИЛИ ВЫРАЗИТЬ(ОтгрузкаШапка.Склад КАК Справочник.Склады).ЭтоГруппа
		|		КОНЕЦ
		|	И &ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		...
		|						ИЛИ &ИспользоватьСоглашенияСКлиентами
		|							И НЕ ЗаявкаНаВозвратТоваровОтКлиента.Соглашение.ИспользуютсяДоговорыКонтрагентов)
		...
		|ГДЕ
		|	СправочникНоменклатура.ВариантОформленияПродажи В (&МассивВариантовОформления)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Ссылка
		|;
		...
		|	ВТЗаказы.ЭтоЗаказКакСчет КАК ЭтоЗаказКакСчет
		...
		|
		|ВЫБРАТЬ
		|	ВТОтгрузкиТоваровПоРаспоряжениям.Распоряжение КАК Ссылка,
		|	ВЫБОР
		|		КОГДА ВТОтгрузкиТоваровПоРаспоряжениям.Распоряжение ССЫЛКА Документ.ЗаказКлиента
		|			ТОГДА ВЫРАЗИТЬ(ВТОтгрузкиТоваровПоРаспоряжениям.Распоряжение КАК Документ.ЗаказКлиента).ЭтоЗаказКакСчет
		|		КОГДА ВТОтгрузкиТоваровПоРаспоряжениям.Распоряжение ССЫЛКА Документ.ЗаявкаНаВозвратТоваровОтКлиента
		|			ТОГДА ВЫРАЗИТЬ(ВТОтгрузкиТоваровПоРаспоряжениям.Распоряжение КАК Документ.ЗаявкаНаВозвратТоваровОтКлиента).ЭтоЗаказКакСчет
		|		ИНАЧЕ
		|			ЛОЖЬ
		|	КОНЕЦ КАК ЭтоЗаказКакСчет
		|ИЗ
		|	ВТОтгрузкиТоваровПоРаспоряжениям КАК ВТОтгрузкиТоваровПоРаспоряжениям
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 15
		|ВЫБРАТЬ
		|	РаспоряженияОтгруженоОбороты.Регистратор КАК Регистратор,
		|	РаспоряженияОтгруженоОбороты.Распоряжение КАК Распоряжение,
		|	РаспоряженияОтгруженоОбороты.Номенклатура КАК Номенклатура,
		|	РаспоряженияОтгруженоОбороты.Характеристика КАК Характеристика,
		|	РаспоряженияОтгруженоОбороты.КодСтроки КАК КодСтроки,
		|	РаспоряженияОтгруженоОбороты.Склад КАК Склад,
		|	РаспоряженияОтгруженоОбороты.Серия КАК Серия,
		|	РаспоряженияОтгруженоОбороты.ОтгруженоКоличество КАК ОтгруженоКоличество
		|ПОМЕСТИТЬ ВТ_РаспоряженияНаОтгрузкуОтгружено
		|ИЗ
		|	#РаспоряженияНаОтгрузкуДвиженияРазвернутая4 КАК РаспоряженияОтгруженоОбороты
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 16
		|ВЫБРАТЬ
		...
		|		ЗаказыОстатки.Склад,
		|		ВЫБОР
		|			КОГДА ТаблицаСкладов.ИспользоватьОрдернуюСхемуПриОтгрузке = ИСТИНА
		|					И ТаблицаСкладов.ДатаНачалаОрдернойСхемыПриОтгрузке <= &ТекущаяДата
		|				ТОГДА ИСТИНА
		|			ИНАЧЕ ЛОЖЬ
		|		КОНЕЦ,
		|		ЗаказыОстатки.Серия,
		...
		|		КОНЕЦ,
		|		0,
		|		-ЗаказыОстатки.ОтгруженоКоличество
		|	ИЗ
		|		ВТ_РаспоряженияНаОтгрузкуОтгружено КАК ЗаказыОстатки
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК ТаблицаСкладов
		|			ПО ЗаказыОстатки.Склад = ТаблицаСкладов.Ссылка
		...
		|	ГДЕ
		|		ЗаказыОстатки.Регистратор <> &Регистратор
		|	) КАК ТаблицаЗаказы
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗаказы.ЗаказКлиента,
	...
	ПараметрыФормированияТаблицы.ТекстУсловия =
		...
		|			Распоряжение
		|		ИЗ
		|			ВТОтгрузкиТоваровПоРаспоряжениям)
		|	И Номенклатура.ВариантОформленияПродажи В (&МассивВариантовОформления)";
	...
	ПараметрыФормированияТаблицы.Показатели =
		"Отгружено, ОприходованоДоППС, СписаноДоППС, Оформлено, Принято, ОформленоСчетовФактур";
	...
	ПараметрыФормированияТаблицы.Группировка = "Распоряжение, Номенклатура, Характеристика, КодСтроки";
	ПараметрыФормированияТаблицы.Показатели =
		"Отгружено, ОприходованоДоППС, СписаноДоППС, Оформлено, КПриемке, Принято, ОформленоСчетовФактур";
	ПараметрыФормированияТаблицы.Ресурсы = "Количество, Сумма, СуммаНДСРегл, СуммаНДСУпр";
	...
	// РаспоряженияНаОтгрузкуДвиженияРазвернутая 3
	ПараметрыФормированияТаблицы = РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат.НовыйПараметрыФормированияРазвернутойТаблицы();
	ПараметрыФормированияТаблицы.ТекстУсловия =
		"(Регистратор, Распоряжение) В 
		|		(ВЫБРАТЬ
		|			ОтгрузкаТоваров,
		|			Распоряжение
		|		ИЗ
	...
	РаспоряженияНаОтгрузкуДвиженияРазвернутая3 =
		РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат.ТекстЗапросаРазвернутаяТаблица(
			ПараметрыФормированияТаблицы);
	...
	// РаспоряженияНаОтгрузкуДвиженияРазвернутая 4
	ПараметрыФормированияТаблицы = РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат.НовыйПараметрыФормированияРазвернутойТаблицы();
	ПараметрыФормированияТаблицы.ТекстУсловия =
		"(Распоряжение, &ЭтоЗаказКакСчет) В
		|		(ВЫБРАТЬ
	...
	ПараметрыФормированияТаблицы.Группировка =
		"Регистратор, Распоряжение, Номенклатура, Характеристика, КодСтроки, Склад, Серия";
	ПараметрыФормированияТаблицы.Показатели = "Отгружено";
	ПараметрыФормированияТаблицы.Ресурсы = "Количество";
	ПараметрыФормированияТаблицы.ФизическаяТаблица = Истина;
	
	РаспоряженияНаОтгрузкуДвиженияРазвернутая4 =
		РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат.ТекстЗапросаРазвернутаяТаблица(
			ПараметрыФормированияТаблицы);
	...
	Если ЗначениеЗаполнено(ДанныеОтбора.Организация) Тогда
		ОрганизацияДляРасчетаСтавкиНДС = ДанныеОтбора.Организация;
	ИначеЕсли ТипЗнч(МассивЗаказов) = Тип("Массив") И МассивЗаказов.Количество() > 0 Тогда
		ОрганизацияДляРасчетаСтавкиНДС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(МассивЗаказов[0], "Организация");
	КонецЕсли;
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст,
		"&СтавкаНДСОтгрузкаТоваровБезРаспоряжения",
		УчетНДСУП.ТекстЗапросаСтавкаНДС(
			"ОтгрузкаТовары.Ссылка.НалогообложениеНДС",
			"ОтгрузкаТовары.Номенклатура",
			"&ВернутьМногооборотнуюТару"));
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст,
		"&СтавкаНДСВводОстатковТоваров",
		УчетНДСУП.ТекстЗапросаСтавкаНДС(
			"ВводОстатковТовары.Ссылка.Партия.НалогообложениеНДС",
			"ВводОстатковТовары.Номенклатура",
			"&ВернутьМногооборотнуюТару"));
	
	УчетНДСУП.УстановитьПараметрыЗапросаСтавкиНДС(
		Запрос.Параметры,
		ОрганизацияДляРасчетаСтавкиНДС,
		?(ЗначениеЗаполнено(ДанныеОтбора.Ссылка),
			ДанныеОтбора.Ссылка.Дата,
			НачалоДня(ТекущаяДатаСеанса())));
			
	Запрос.УстановитьПараметр("СтранаРегистрации",
		ЗначениеНастроекКлиентСерверПовтИсп.СтранаРегистрацииОрганизации(ОрганизацияДляРасчетаСтавкиНДС));
	
	// Коэффициенты упаковок
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстЗапросаКоэффициентУпаковки4",
		Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки(
			"ОтгрузкаТовары.Упаковка",
			"ОтгрузкаТовары.Номенклатура"));
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстЗапросаКоэффициентУпаковки5",
		Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки(
			"ВводОстатковТовары.Упаковка",
			"ВводОстатковТовары.Номенклатура"));
...
// 
// Параметры:
//  СсылкаОбъект - ДокументСсылка.РеализацияТоваровУслуг - Документ, по которому нужно получить данные.
...
Функция СуммыПоЗаказам(СсылкаОбъект, ЗначенияРеквизитов = Неопределено) Экспорт
	...
	|//////////////////////////////////////////////////////////////////////////////// 1
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТоварыРеализации.Заказ КАК Распоряжение,
	|	ОтгрузкаТоваров.Ссылка КАК Исправление,
	|	ОтгрузкаТоваров.Дата КАК ДатаОтгрузки
	|ПОМЕСТИТЬ ОтгрузкиТоваров
	|ИЗ
	|	Документ.ОтгрузкаТоваровКлиенту КАК ОтгрузкаТоваров
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТТовары КАК ТоварыРеализации
	|		ПО ОтгрузкаТоваров.Ссылка = ТоварыРеализации.Заказ
	|ГДЕ
	|	ОтгрузкаТоваров.Проведен
	|	И &ЭтоПереходПраваСобственности2_5
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	Документ.ОтгрузкаТоваровКлиенту КАК ОтгрузкаТоваров
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТТовары КАК ТоварыРеализации
	|		ПО ОтгрузкаТоваров.ИсправляемыйДокумент = ТоварыРеализации.Заказ
	|ГДЕ
	|	ОтгрузкаТоваров.Проведен
	|	И &ЭтоПереходПраваСобственности2_5
	|;
	...
	|		КОНЕЦ                            КАК СуммаВзаиморасчетовПоТаре,
	|		ВЫБОР
	|			КОГДА НЕ &ЭтоПереходПраваСобственности2_5
	|				ТОГДА ДАТАВРЕМЯ(1, 1, 1)
	|			КОГДА ЗаказТовары.Ссылка ЕСТЬ НЕ NULL
	|				ТОГДА ЗаказТовары.ДатаОтгрузки
	|			КОГДА ЗаявкаНаВозвратТовары.Ссылка ЕСТЬ НЕ NULL
	|				ТОГДА ЗаявкаНаВозвратТовары.ДатаОтгрузки
	|			КОГДА ПервичныйДокумент.Ссылка ЕСТЬ НЕ NULL
	|				ТОГДА ПервичныйДокумент.Дата
	|			КОГДА ОтгрузкиТоваров.Распоряжение ЕСТЬ НЕ NULL
	|				ТОГДА ОтгрузкиТоваров.ДатаОтгрузки
	|			ИНАЧЕ
	|				&Дата
	|		КОНЕЦ                            КАК ДатаОтгрузки
	|	ИЗ ВТТовары КАК Товары
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента.Товары КАК ЗаказТовары
	|			ПО Товары.КодСтроки = ЗаказТовары.КодСтроки
	|				И Товары.Заказ = ЗаказТовары.Ссылка
	|				И Товары.Номенклатура = ЗаказТовары.Номенклатура
	|				И Товары.Характеристика = ЗаказТовары.Характеристика
	...
	|			ПО Товары.КодСтроки = ЗаявкаНаВозвратТовары.КодСтроки
	|				И Товары.Заказ = ЗаявкаНаВозвратТовары.Ссылка
	|				И Товары.Номенклатура = ЗаявкаНаВозвратТовары.Номенклатура
	|				И Товары.Характеристика = ЗаявкаНаВозвратТовары.Характеристика
	|				И ЗаявкаНаВозвратТовары.Ссылка.Проведен
	|				И &ЭтоПереходПраваСобственности2_5
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПервичныйДокумент КАК ПервичныйДокумент
	|			ПО Товары.Заказ = ПервичныйДокумент.Ссылка
	|				И ПервичныйДокумент.Проведен
	|				И &ЭтоПереходПраваСобственности2_5
	|		ЛЕВОЕ СОЕДИНЕНИЕ ОтгрузкиТоваров КАК ОтгрузкиТоваров
	|			ПО Товары.Заказ = ОтгрузкиТоваров.Распоряжение
	|				И &ЭтоПереходПраваСобственности2_5
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументов
	|			ПО ОтгрузкиТоваров.Исправление = РеестрДокументов.СторнируемыйДокумент
	|				И РеестрДокументов.Проведен
	|				И НЕ РеестрДокументов.ДополнительнаяЗапись
	|				И &ЭтоПереходПраваСобственности2_5
	|	ГДЕ
	...
	|	
	|	ВЫБРАТЬ
	|		ВЫБОР КОГДА ЕСТЬNULL(ТаблицаТовары.Заказ.Номер, """") = """"
	|				ТОГДА ТаблицаТовары.ЗаказШапка
	|			ИНАЧЕ ТаблицаТовары.Заказ
	|		КОНЕЦ                        КАК Заказ,
	|		ТаблицаТовары.ПоЗаказам КАК СверхЗаказа,
	|		ВЫБОР
	|			КОГДА ТаблицаТовары.Номенклатура.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	|					ИЛИ НЕ ТаблицаТовары.ВернутьМногооборотнуюТару
	...
	|		ВЫБОР 
	|			КОГДА ТаблицаТовары.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	|				И ТаблицаТовары.ТребуетсяЗалогЗаТару
	|				ТОГДА ТаблицаТовары.Сумма
	|			ИНАЧЕ 0 
	|		КОНЕЦ                            КАК СуммаЗалогаЗаТару,
	|		ВЫБОР 
	...
	|			КОГДА ЗаявкаНаВозврат.Ссылка ЕСТЬ НЕ NULL
	|				ТОГДА ЗаявкаНаВозврат.Дата
	|			ИНАЧЕ
	|				ДАТАВРЕМЯ(1, 1, 1)
	|		КОНЕЦ                            КАК ДатаОтгрузки
	|	ИЗ ВТТовары КАК ТаблицаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента КАК ЗаказКлиента
	|			ПО ТаблицаТовары.Заказ = ЗаказКлиента.Ссылка
	|				И &ЭтоПереходПраваСобственности2_5
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаВозвратТоваровОтКлиента КАК ЗаявкаНаВозврат
	|			ПО ТаблицаТовары.Заказ = ЗаявкаНаВозврат.Ссылка
	|				И &ЭтоПереходПраваСобственности2_5
	|	ГДЕ
	|		ТаблицаТовары.КодСтроки = 0) КАК ВложенныйЗапрос
	|СГРУППИРОВАТЬ ПО
	...
	Если ЗначенияРеквизитов = Неопределено Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ЭтоПереходПраваСобственности2_5", "ЛОЖЬ");
	...
	СуммыПоЗаказам = Запрос.Выполнить().Выгрузить();
	
	Если ЗначенияРеквизитов = Неопределено Или Не ЗначенияРеквизитов.ЭтоПереходПраваСобственности2_5 Тогда
		СуммыПоЗаказам.Колонки.Удалить("ДатаОтгрузки");
	КонецЕсли;
	
	Возврат СуммыПоЗаказам;
...
Функция ТекстЗапросаТоварыПереданные() Экспорт
	...
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионера),
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионераБезПереходаПраваСобственности))
	|			И НЕ ВтПолитикиУчетаСерий.УчетСерийВПереданныхНаКомиссиюТоваров
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыНоменклатуры КАК ВтВидыНоменклатуры
	|		ПО ВтНоменклатура.ВидНоменклатуры = ВтВидыНоменклатуры.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПолитикиУчетаСерий КАК ВтПолитикиУчетаСерий
	|		ПО ВтВидыНоменклатуры.ПолитикаУчетаСерий = ВтПолитикиУчетаСерий.Ссылка
	|	
	|СГРУППИРОВАТЬ ПО
	|	ВтТоварыНакладной.Номенклатура,
	|	ВтТоварыНакладной.Характеристика,
	...
	|		ВЫБОР
	|			КОГДА ЕСТЬNULL(Остатки.КоличествоОстаток * Остатки.Знак, 0) < ТоварыПереданные.КоличествоОстаток * ЕСТЬNULL(Остатки.Знак, 1)
...
Функция ОбработчикДействий(ХозяйственнаяОперация = Неопределено) Экспорт
	...
	Обработчик = Документы.РеализацияТоваровУслуг;
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Доступность =
		...
		И ПравоДоступа("Чтение",     Метаданные.РегистрыСведений.РаспоряженияНаОтгрузкуИВозвратКВыполнению);
	...
	ДоступностьВПути = ПолучитьФункциональнуюОпцию("ИспользоватьОтгрузкуБезПереходаПраваСобственности2_0");
	...
	|	ИЗ
	|		(ВЫБРАТЬ
	|			РаспоряженияНаОтгрузкуИВозвратКВыполнению.Распоряжение КАК Ссылка,
	|			НЕОПРЕДЕЛЕНО КАК Склад,
	|			НЕОПРЕДЕЛЕНО КАК Номенклатура,
	|			НЕОПРЕДЕЛЕНО КАК Характеристика,
	|			НЕОПРЕДЕЛЕНО КАК Серия
	|		ИЗ
	|			РегистрСведений.РаспоряженияНаОтгрузкуИВозвратКВыполнению КАК РаспоряженияНаОтгрузкуИВозвратКВыполнению
	|		ГДЕ
	|			(РаспоряженияНаОтгрузкуИВозвратКВыполнению.Распоряжение ССЫЛКА Документ.ЗаказКлиента
	|				ИЛИ РаспоряженияНаОтгрузкуИВозвратКВыполнению.Распоряжение ССЫЛКА Документ.ЗаявкаНаВозвратТоваровОтКлиента)
	|			И РаспоряженияНаОтгрузкуИВозвратКВыполнению.СостояниеКОформлению <> ЗНАЧЕНИЕ(Перечисление.СостояниеОформленияДокументовПоРаспоряжению.Оформлены)
	|			И РаспоряженияНаОтгрузкуИВозвратКВыполнению.Распоряжение ЕСТЬ НЕ NULL
	|			И НЕ РаспоряженияНаОтгрузкуИВозвратКВыполнению.ХозяйственнаяОперация В(&ХозяйственныеОперации)
	|	
	|		ОБЪЕДИНИТЬ ВСЕ
	|	
	|		ВЫБРАТЬ
...
Функция ТекстЗапросаТаблицаРеестрДокументов(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект = Неопределено)
	...
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ДатаКомиссионера <> ДАТАВРЕМЯ(1,1,1)
	|			ТОГДА ДанныеДокумента.ДатаКомиссионера
	|		ИНАЧЕ
	|			ДанныеДокумента.Дата
	|	КОНЕЦ КАК ДатаПервичногоДокумента,
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.НомерКомиссионера <> """"
	|			ТОГДА ДанныеДокумента.НомерКомиссионера
	|		ИНАЧЕ
	|			&НомерНаПечать
	|	КОНЕЦ КАК НомерПервичногоДокумента,
	|	ДанныеДокумента.НаименованиеДокументаКомиссионера КАК НаименованиеПервичногоДокумента,
...
Функция ТекстЗапросаЗаказыКОформлению(Организация = Неопределено, Склад = Неопределено) Экспорт
	...
	ПараметрыФормированияТаблицы = РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат.НовыйПараметрыФормированияРазвернутойТаблицы();
	ПараметрыФормированияТаблицы.ПараметрыВиртуальнойТаблицы =
		"&Склад В (Склад, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		|		И (Распоряжение ССЫЛКА Документ.ЗаказКлиента
		|			ИЛИ Распоряжение ССЫЛКА Документ.ЗаявкаНаВозвратТоваровОтКлиента)";
	ПараметрыФормированияТаблицы.Группировка = "Распоряжение, Склад";
	ПараметрыФормированияТаблицы.Показатели = "КОтгрузке";
	ПараметрыФормированияТаблицы.Ресурсы = "Количество";
	...
	ТекстЗапроса =
		...
		|	ГДЕ
		|		РаспоряженияНаОтгрузку.КОтгрузкеКоличествоОборот > 0
		...
		|					И Склад.ДатаНачалаОрдернойСхемыПриОтгрузке <= &ТекущаяДата
