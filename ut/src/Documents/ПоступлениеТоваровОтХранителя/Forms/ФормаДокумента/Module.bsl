...
Процедура ПередЗаписьюПредложитьПодобратьМногооборотнуюТаруЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ОбщегоНазначенияУТКлиент.ОбработатьЗаписьОбъектаВФорме(ЭтотОбъект, ДополнительныеПараметры.ПараметрыЗаписи);
...
Процедура НадписьЗаголовокЗаказыНажатие(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму("ОбщаяФорма.ПросмотрСпискаДокументов", ПараметрыОткрытия, ЭтотОбъект, , , , Неопределено,
...
Процедура СкладПриИзменении(Элемент)
	...
	ИспользуютсяЗаявкиКакСчета = ИспользуютсяЗаявкиКакСчета();
	
	Если ИспользуютсяЗаявкиКакСчета Тогда
		СкладПриИзмененииСервер(Ложь, Ложь, Истина);
	ИначеЕсли Объект.ПоРаспоряжениям Тогда
...
Процедура СкладПриИзмененииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	СкладПриИзмененииСервер(ОчиститьСтроки, ОтвязатьСтроки, Истина);
...
Процедура ТоварыНоменклатураОприходованиеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыЗаполненияАртикула = Новый Структура;
	ПараметрыЗаполненияАртикула.Вставить("Номенклатура", "Артикул");
	ПараметрыЗаполненияАртикула.Вставить("НоменклатураИсходногоКачества", "АртикулОприходование");
	СтруктураДействий.Вставить("ЗаполнитьПризнакАртикул", ПараметрыЗаполненияАртикула);
	СтруктураДействий.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", КонтрольКачестваКэш);
...
Процедура РазбитьСтроку(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ДополнительныеПараметры = УчетПрослеживаемыхТоваровКлиентСерверЛокализация.ДополнитьОписаниеПараметровРазбиенияСтрокиТабличнойЧасти(ТекущиеДанные, ДополнительныеПараметры);
...
Процедура ПроверитьКоличествоВДокументеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		ОбщегоНазначенияУТКлиент.ВывестиСообщениеПревышенияПоКоличеству(Результат, НСтр("ru = 'введенного на основании распоряжения.'"));
		РассчитатьИтоговыеПоказатели(ЭтотОбъект);
	КонецЕсли;
...
Процедура ПодобратьПереданныеТовары(Команда)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени(Истина,
		"Документ.ПоступлениеТоваровОтХранителя.ФормаДокумента.Команда.ПодобратьПереданныеТовары");
...
Процедура ДополнитьМногооборотнойТарой(Команда)
	...
	МногооборотнаяТараКлиент.ПодобратьМногооборотнуюТару(ЭтотОбъект, "Товары", "Номенклатура,Характеристика,Количество");
	...
	Если Объект.ВозвратПорчи Тогда 
		...
		Если ЕстьСтрокиСПорчей Тогда
			...
			Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5") Тогда
				...
				ТекстВопроса =
					НСтр("ru = 'При выполнении операции будет очищена информация о качестве поступающих товаров. Продолжить?'");
				
			Иначе
				
				ТекстВопроса =
					НСтр("ru = 'При выполнении операции будет очищена информация о качестве возвращаемых товаров. Продолжить?'");
				
			КонецЕсли;
			
			ПоказатьВопрос(Новый ОписаниеОповещения("ВозвратПорчиЗавершение", ЭтотОбъект),
				ТекстВопроса,
...
Процедура ПоРаспоряжениямПриИзмененииНаКлиенте()
	...
	Если Объект.ПоРаспоряжениям Тогда
		...
		Если Не ИспользоватьПоступлениеПоНесколькимЗаказам Тогда
			ОписаниеОповещения = Новый ОписаниеОповещения("ПоРаспоряжениямПриИзмененииОткрытьПодборРаспоряженияЗавершение", ЭтотОбъект);
			ОткрытьПодборРаспоряжения(ОписаниеОповещения);
			Возврат;
		Иначе
			Если Объект.Товары.Количество() = 0 Тогда
...
Процедура ПоРаспоряжениямПриИзмененииЗадатьВопрос(ТекстВопроса)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПоРаспоряжениямПриИзмененииЗадатьВопросЗавершение", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура ПоРаспоряжениямПриИзмененииНаСервере()
	...
	Если Объект.ПоРаспоряжениям Тогда
		...
		Если ТипЗнч(Объект.Распоряжение) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
			
			ЭтоВозвратПорчи = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Распоряжение, "ВозвратПорчи");
			Если ЭтоВозвратПорчи = Истина Тогда
...
Процедура ХозяйственнаяОперацияПриИзмененииСервер()
	...
	Если КонтрольКачества.ОбновитьПараметрыДействий(
			Документы.ПоступлениеТоваровОтХранителя,
			Объект,
...
&НаСервере
Процедура СкладПриИзмененииСервер(ОчиститьСтроки, ОтвязатьСтроки, НепосредственноеИзменениеПоляСклад = Ложь)
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	Колонки = "Номенклатура,Характеристика,Упаковка,КоличествоУпаковок,Цена,НомерГТД,
		|НоменклатураНабора,ХарактеристикаНабора,Порча,НоменклатураОприходование,ХарактеристикаОприходование";
	
	Если ИспользоватьКодСтроки Тогда
		Колонки = "Распоряжение," + Колонки;
	КонецЕсли;
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	Если ИспользоватьКодСтроки Тогда
		ЗаказыКлиентСервер.ДобавитьДействиеЗаполнитьПризнакРасхождениеЗаказ(СтруктураДействийСДобавленнымиСтроками, Объект.ПоРаспоряжениям);
	КонецЕсли;
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	СписокСвойств =
		"НоменклатураНабора, Номенклатура, Характеристика, ХарактеристикаНабора, Упаковка, Серия, КоличествоУпаковок, Цена";
...
Процедура ОбработкаПодбораТоваровИзЗаказа(АдресВХранилище)
	...
	СтруктураДействий.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", КонтрольКачестваКэш);
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(ДобавленныеСтроки, СтруктураДействий, Объект.Товары);
...
Процедура НастроитьФормуПоПорче()
	...
	ОтображатьЭлементыПорчи = Объект.ВозвратПорчи;
	
	КомандаВозвратПорчиЗаголовок = НСтр("ru = 'Возврат товаров другого качества'");
	ОтгруженныйТоварЗаголовок = НСтр("ru = 'Ранее был отгружен товар'");
	ВозвращаемыйТоварЗаголовок = НСтр("ru = 'Возвращается товар другого качества'");
	
	
	ДоступностьЭлементовПорчи = Не Объект.ПоРаспоряжениям
		И Не ЗначениеЗаполнено(Объект.Распоряжение)
		И Не ЗначениеЗаполнено(Объект.ДокументПередачи);
	
	Элементы.ВозвратПорчи.Доступность = ДоступностьЭлементовПорчи;
	
	Элементы.ВозвратПорчи.Заголовок = КомандаВозвратПорчиЗаголовок;
	Элементы.ТоварыГруппаРанееОтгруженныйТовар.Заголовок = ОтгруженныйТоварЗаголовок;
	Элементы.ТоварыГруппаВозвращаемыйТовар.Заголовок = ВозвращаемыйТоварЗаголовок;
	
	Элементы.ВозвратПорчи.Пометка = ОтображатьЭлементыПорчи;
	Элементы.ТоварыГруппаРанееОтгруженныйТовар.ОтображатьВШапке = ОтображатьЭлементыПорчи;
	Элементы.ТоварыГруппаВозвращаемыйТовар.Видимость = ОтображатьЭлементыПорчи;
...
Процедура ОчиститьОтключитьНастроитьФормуПоПорче()
	...
	Для Каждого СтрокаТовары Из Объект.Товары Цикл
		...
		Если СтрокаТовары.Порча Тогда
			ОбрабатываемыеСтроки.Добавить(СтрокаТовары);
		КонецЕсли;
		СтрокаТовары.Порча = Ложь;
		СтрокаТовары.НоменклатураОприходование = Справочники.Номенклатура.ПустаяСсылка();
		СтрокаТовары.ХарактеристикаОприходование = Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка();
	...
	Если ПараметрыУказанияСерий.ИспользоватьСерииНоменклатуры Тогда
		...
		Для Каждого СтрокаСерии Из Объект.Серии Цикл
			...
			СтрокаСерии.НоменклатураОприходование = Справочники.Номенклатура.ПустаяСсылка();
			СтрокаСерии.ХарактеристикаОприходование = Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка();
...
Процедура УстановитьУсловноеОформление()
	...
	#Область УсловноеОформление_Порча
	
	// оформление ХарактеристикаОприходование
	Элемент = УсловноеОформление.Элементы.Добавить();
	...
	// свойства НоменклатураОприходование, ХарактеристикаОприходование
	Элемент = УсловноеОформление.Элементы.Добавить();
...
Процедура УстановитьВозможностьРедактированияНД()
	...
	ПараметрыДоступностиНД.НакладнаяПоЗаказу = Объект.ПоРаспоряжениям;
...
Процедура УстановитьСвойстваЭлементов()
	...
	Если Объект.ПоРаспоряжениям Тогда
		...
		Если ЗначениеЗаполнено(Объект.Распоряжение) Тогда
			МожноРедактироватьРаспоряжение = УправлениеДоступом.ИзменениеРазрешено(Объект.Распоряжение);
		Иначе
			МожноРедактироватьРаспоряжение = Истина;
			Для Каждого СтрокаТоваров Из Объект.Товары Цикл
				МожноРедактироватьРаспоряжение = МожноРедактироватьРаспоряжение И УправлениеДоступом.ИзменениеРазрешено(СтрокаТоваров.Распоряжение);
				Если Не МожноРедактироватьРаспоряжение Тогда
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		
		ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(Элементы, МассивЭлементов, "ТолькоПросмотр", Не МожноРедактироватьРаспоряжение);
...
Процедура ЗаполнитьМногооборотнуюТаруИзХранилищаСервер(АдресТарыВХранилище)
	...
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействийИзмененныеСтроки, Объект, КомиссионерНеВедетУчетПоРНПТ, Ложь);
...
Процедура ОткрытьПодборПоРаспоряжениямОрдерам()
	...
	ОткрытьФорму("ОбщаяФорма.ПодборТоваровИзЗаказа", ПараметрыФормы, ЭтотОбъект, УникальныйИдентификатор, , , ,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ЗаполнитьПоЗаказуНаСервере()
	...
	Если ИспользоватьПоступлениеПоНесколькимЗаказам Тогда
		МассивЗаказов =
			Документы.ПоступлениеТоваровОтХранителя.РаспоряженияНакладной(Объект.Ссылка, РеквизитыШапки);
	Иначе
...
&НаСервере
Функция ИспользуютсяЗаявкиКакСчета()
	
	РаспоряженияОснования = Объект.Товары.Выгрузить(, "Распоряжение").ВыгрузитьКолонку("Распоряжение");
	РаспоряженияОснования.Добавить(Объект.Распоряжение);
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("РаспоряженияОснования", РаспоряженияОснования);
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ГДЕ
	|	ТаблицаДокументов.Ссылка В (&РаспоряженияОснования)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
