...
Функция УведомлениеОПланируемомИмпортеЕГАИСXML(ДокументСсылка)
	...
	|	Продукция.Представление                                КАК ПродукцияПредставление,
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		Если Не ЗначениеЗаполнено(СтрокаТЧ.ОбъемДАЛ) Тогда
			ОбменДаннымиЕГАИСКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, 
				СтрШаблон(НСтр("ru = 'Для номенклатуры в строке %1 не установлен объем в декалитрах.'"), СтрокаТЧ.НомерСтроки));
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого СтрокаТЧ Из АлкогольнаяПродукция Цикл
		Если СтрокаТЧ.ПроизводительТипОрганизации = Перечисления.ТипыОрганизацийЕГАИС.ЮридическоеЛицоРФ 
			Или СтрокаТЧ.ПроизводительТипОрганизации = Перечисления.ТипыОрганизацийЕГАИС.ИндивидуальныйПредпринимательРФ Тогда
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Алкогольная продукция ""%1"", производимая в РФ не может быть импортирована.'"),
				СтрокаТЧ.ПродукцияПредставление);
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		XDTO = РаботаСXMLЕГАИС.ОбъектXDTO(ПространствоИмен, "InvoicePlannedImportType_v1");
		...
		РаботаСXMLЕГАИС.ЗаполнитьСвойствоXDTO(Содержимое, "UNFSM_FLAG",        Ложь,                     СообщениеXML);
		РаботаСXMLЕГАИС.ЗаполнитьСвойствоXDTO(Содержимое, "BONDEDWAREHOUSE",   Шапка.ТаможенныйСкладКод, СообщениеXML);
...
Процедура ЗаполнитьСтатусИИдентификаторПоКвитанции(ДокументОбъект, ДанныеДокумента, СтатусОбработки, Операция) Экспорт
	...
	Если СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийЕГАИС.Ошибка Тогда
		...
		Описание = ?(ОшибкаЭтапаПроверки,
			...
			ДанныеДокумента.Объект.OperationResult.OperationComment);
