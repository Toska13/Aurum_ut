...
// 
// Параметры:
//  ОбъектИлиСсылка - ДокументОбъект.СборкаТоваров, ДокументСсылка.СборкаТоваров - документ.
...
Функция РеквизитыКонтролируемойПартии(ОбъектИлиСсылка, ИмяПоляКонтролируемаяПартия) Экспорт
	...
	Реквизиты.Партнер = Неопределено;
	...
	Реквизиты.Отправитель = РеквизитыОбъекта.Склад;
...
// 
// Параметры:
//  Объект - ДокументОбъект.СборкаТоваров - объект.
// 
// Возвращаемое значение:
//  Структура - Структура с полями:
//   * Товары - Структура - Структура с полями:
//   ** Ключ - Строка
//   ** ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества - см. КонтрольКачества.НовыйПараметрыДействия
//   * Шапка - Структура - Структура с полями:
//   ** ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества - см. КонтрольКачества.НовыйПараметрыДействия
//   ** Ключ - Строка
Функция КонтрольКачестваПараметрыДействий(Объект) Экспорт
	...
	ПараметрыДействий.Вставить("Ключ", "Товары");
	...
	ПараметрыДействия.ЧитаемыеПоля = "Номенклатура";
	...
	ПараметрыДействия.ПараметрыЗапроса.Вставить("КонтрольКачестваХозяйственнаяОперация", Объект.ХозяйственнаяОперация);
	...
	ПараметрыДействия.ТекстЗапросаДанныеДокумента =
		...
		|	ЛОЖЬ КАК Установить,
		|	&КонтрольКачестваХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СборкаТоваров) КАК Сбросить
	...
	ВсеПараметры.Добавить(Новый Структура("Ключ,Параметры", "Товары", ПараметрыДействий));
	
	// Шапка.
	
	ПараметрыДействий = Новый Структура();
	ПараметрыДействий.Вставить("Ключ", "Шапка");
	...
	ПараметрыДействия.ТекстЗапросаДанныеДокумента =
		...
		|	&КонтрольКачестваДатаПоступления КАК ДатаПоступления,
		|	ЛОЖЬ КАК Установить,
		|	&КонтрольКачестваХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РазборкаТоваров) КАК Сбросить
		|ПОМЕСТИТЬ ДанныеДокументаДляЗаполненияПроконтролироватьКачествоДатаПодтвержденияКачества
		|ИЗ
		|	#ИсточникДанных КАК Таблица
		|ИНДЕКСИРОВАТЬ ПО
	...
	ВсеПараметры.Добавить(Новый Структура("Ключ,Параметры", "Шапка", ПараметрыДействий));
	
	Возврат Новый ФиксированныйМассив(ВсеПараметры);
...
Функция МакетФормыВыбораНазначений() Экспорт
	...
	ОписаниеКолонок.ПутиКДанным.ПроконтролироватьКачество = "Объект.ПроконтролироватьКачество";
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Запрос.Текст =
		...
		|	ДанныеШапки.ИсправляемыйДокумент.Дата КАК ПериодКорректировки,
		|	ДанныеШапки.КонтролируемаяПартия КАК КонтролируемаяПартия,
		|	ДанныеШапки.ПроконтролироватьКачество КАК ПроконтролироватьКачество,
...
Процедура ТекстЗапросаДанныеДокументаШапка(СписокЗапросов)
	...
	ТекстЗапросаШапка = 
		...
		|	ДанныеДокумента.ПроконтролироватьКачество КАК ПроконтролироватьКачество,
		|	ДанныеДокумента.ДатаПодтвержденияКачества КАК ДатаПодтвержденияКачества,
...
Функция РаспределитьСтоимостьКомплектующихНаКомплекты(ТаблицаКомплектов, ТаблицаКомплектующих, Реквизиты)
	...
	Для Каждого СтрокаВхода Из ТаблицаКомплектующих Цикл
		...
		ИдентификаторФинЗаписиВхода = Строка(Новый УникальныйИдентификатор);
		ИдентификаторФинЗаписиВыхода = "";

		Счетчик = 0;
		ЗаписиКомплектующиеБезИдентификатора = Новый Массив();
		ЗаписиКомплектаБезИдентификатора = Новый Массив();
		...
		Для Каждого СтрокаВыхода Из ТаблицаКомплектов Цикл
			...
			ЭтоЗначимыйВыход = РаспределенноеКоличество[Счетчик] <> 0;
			
			Если НЕ ЭтоЗначимыйВыход Тогда
			ИначеЕсли ЗначениеЗаполнено(СтрокаВыхода.ИдентификаторСтроки) Тогда
				ИдентификаторФинЗаписиВыхода = СтрокаВыхода.ИдентификаторСтроки;
			Иначе
				ИдентификаторФинЗаписиВыхода = ИдентификаторФинЗаписиВхода;
			КонецЕсли;
			...
			ЗаписьКомплектующие.ИдентификаторФинЗаписи = ИдентификаторФинЗаписиВыхода;
			//	партионный учет версии 2.2
			...
			ЗаписьКомплект.ИдентификаторФинЗаписи = ИдентификаторФинЗаписиВыхода; 
			...
			Если ЭтоЗначимыйВыход Тогда
				ЗаписьКомплект.ИдентификаторСтроки = "";
			Иначе
				ЗаписьКомплект.ИдентификаторСтроки = СтрокаВыхода.ИдентификаторСтроки;
				Если ПустаяСтрока(ИдентификаторФинЗаписиВыхода) Тогда
					ЗаписиКомплектующиеБезИдентификатора.Добавить(ЗаписьКомплектующие);
					ЗаписиКомплектаБезИдентификатора.Добавить(ЗаписьКомплект);
				КонецЕсли;
			КонецЕсли;
		...
		Для Каждого ЗаписьКомплектующие Из ЗаписиКомплектующиеБезИдентификатора Цикл
			ЗаписьКомплектующие.ИдентификаторФинЗаписи = ИдентификаторФинЗаписиВыхода;
		КонецЦикла;

		Для Каждого ЗаписьКомплект Из ЗаписиКомплектаБезИдентификатора Цикл
			ЗаписьКомплект.ИдентификаторФинЗаписи = ИдентификаторФинЗаписиВыхода;
		КонецЦикла;
...
#Область СписаниеНаДругиеПартии_РасходМатериалов
	...
	|	ВтВидыЗапасов.КорВидДеятельностиНДС						КАК ВидДеятельностиНДСДокумента,
	|
...
#Область ВнутреннееПоступление_ПриходПродукции
	...
	|	ВтВидыЗапасов.ХозяйственнаяОперация	 КАК ХозяйственнаяОперация,
	|	&НастройкаХозяйственнойОперации		 КАК НастройкаХозяйственнойОперации,
	|	ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимостиТоваров.ПоКорДвижению) КАК СпособОпределенияСебестоимости
	|
	|ИЗ
	|	Документ.СборкаТоваров КАК ТаблицаДокумента
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтТаблицаСебестоимостьТоваров КАК ВтВидыЗапасов
