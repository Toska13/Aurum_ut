...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Перечисления.СтатусыВнутреннихЗаказов.ЗаполнитьСписокВыбора(Элементы.Статус.СписокВыбора, Ложь);
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	КонтролируемаяПартияСсылкаНового = Справочники.КонтролируемыеПартии.ПолучитьСсылку();
	ЕстьДоступКСкладуОтправителю = ЕстьДоступКСкладуОтправителю();
	...
	УстановитьВидимостьГруппыКонтрольКачества();
	КонтрольКачестваКэш = КонтрольКачества.КэшДанныхМеханизма(Объект);
	ВариантОбеспеченияИФлагОбособленно = Документы.ЗаказНаПеремещение.ВариантОбеспеченияИФлагОбособленно(Объект);
...
Процедура СкладПолучательПриИзменении(Элемент)
	...
	СкладПолучательПриИзмененииСервер(Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ПеремещениеЗаблокированныхТоваровПриИзменении(Элемент)
	
	ПеремещениеЗаблокированныхТоваровПриИзмененииСервер();
	
КонецПроцедуры

&НаКлиенте
Процедура КонтролируемаяПартияОтправителяПриИзменении(Элемент)
	
	КонтролируемаяПартияОтправителяПриИзмененииСервер();
...
Процедура ИспользоватьДлительностьПеремещенияПриИзменении(Элемент)
	...
	ИспользоватьДлительностьПеремещенияПриИзмененииСервер();
...
&НаКлиенте
Процедура ТоварыВариантОбеспеченияНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	Если Объект.ПеремещениеЗаблокированныхТоваров Тогда
		...
		НедопустимыеДействия.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.КОбеспечению"));
		НедопустимыеДействия.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.НеТребуется"));
...
&НаКлиенте
Процедура ТоварыОкончаниеПоступленияПриИзменении(Элемент)
...
&НаКлиенте
Процедура ТоварыНачалоОтгрузкиПриИзменении(Элемент)
	
	ТекущаяСтрока = Элементы.Товары.ТекущиеДанные;
	Если Не Объект.ИспользоватьДлительностьПеремещения Тогда
		ТекущаяСтрока.ОкончаниеПоступления = ТекущаяСтрока.НачалоОтгрузки;
	КонецЕсли;
...
Процедура ПодобратьТовары(Команда)
	...
	КонтролируемаяПартия = Объект.КонтролируемаяПартияОтправителя;
	Если Объект.ПеремещениеЗаблокированныхТоваров И Не ЗначениеЗаполнено(КонтролируемаяПартия) Тогда
		КонтролируемаяПартия = КонтролируемаяПартияСсылкаНового; // чтобы не отображались остатки без партий.
	КонецЕсли;
	...
	Если Объект.ПеремещениеЗаблокированныхТоваров Тогда
		...
		Действия = ОбеспечениеВДокументахКлиентСервер.ВсеДействия();
		Действия.Удалить(Действия.НайтиПоЗначению(ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.НеТребуется")));
		Действия.Удалить(Действия.НайтиПоЗначению(ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.КОбеспечению")));
		ПараметрыФормы.Вставить("ДопустимыеОбособленныеДействия", Действия);
		ПараметрыФормы.Вставить("ДопустимыеНеобособленныеДействия", Действия);
...
Процедура УстановитьУсловноеОформление()
	...
	КонтрольКачества.ДобавитьОтбор(ФлагНедоступен, "Объект.КонтролируемаяПартияОтправителя", ПустаяПартия, НеРавно);
	КонтрольКачества.ДобавитьОтбор(ДатаНеОбязательна, "Объект.Товары.ОкончаниеПоступления", '00010101');
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ПеремещениеЗаблокированныхТоваров");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
	...
	ОтборЭлемента = ГруппаИли.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.КонтролируемаяПартияОтправителя");
...
&НаСервере
Процедура СкладПолучательПриИзмененииСервер(НепосредственноеИзменениеПоляСклад = Ложь)
...
&НаСервере
Процедура ПеремещениеЗаблокированныхТоваровПриИзмененииСервер()
	
	ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(
		НоменклатураСервер.ПараметрыУказанияСерий(Объект, Документы.ЗаказНаПеремещение));
	...
	Если Объект.ПеремещениеЗаблокированныхТоваров Тогда
		...
		УстанавливаемыйВариант = Перечисления.ВариантыОбеспечения.РезервироватьПоМереПоступления;
		Для каждого СтрокаТаблицы Из Объект.Товары Цикл
			
			Если СтрокаТаблицы.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.КОбеспечению
					Или СтрокаТаблицы.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.НеТребуется Тогда
				СтрокаТаблицы.ВариантОбеспечения = УстанавливаемыйВариант;
			КонецЕсли;
	...
	Иначе
		...
		Объект.КонтролируемаяПартияОтправителя = Справочники.КонтролируемыеПартии.ПустаяСсылка();
...
&НаСервере
Процедура КонтролируемаяПартияОтправителяПриИзмененииСервер()
...
&НаСервере
Процедура ИспользоватьДлительностьПеремещенияПриИзмененииСервер()
	
	Если Объект.ИспользоватьДлительностьПеремещения Тогда
		Объект.ДлительностьПеремещения = ДлительностьПеремещения;
	Иначе
		Для Каждого СтрокаТаблицы Из Объект.Товары Цикл
			СтрокаТаблицы.ОкончаниеПоступления = СтрокаТаблицы.НачалоОтгрузки;
		КонецЦикла;
		ДлительностьПеремещения = Объект.ДлительностьПеремещения;
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	ПоляДляЗаполнения = "НоменклатураНабора,ХарактеристикаНабора,Номенклатура,Характеристика,ХарактеристикиИспользуются,Упаковка,
		|КоличествоУпаковок,Количество,ВариантОбеспечения,Обособленно,Серия";
	ПоляПоиска = "НоменклатураНабора, ХарактеристикаНабора, Номенклатура, Характеристика, Упаковка, 
...
&НаСервере
Процедура УстановитьВидимостьГруппыКонтрольКачества()
	
	Элементы.ГруппаКонтрольКачестваОтступ.Видимость = Объект.ПеремещениеЗаблокированныхТоваров;
...
Процедура ПриОкончанииРедактированияНабора(АдресВоВременномХранилище)
	...
	СтруктураДействийСИзмененнымиСтроками = Новый Структура;
	СтруктураДействийСИзмененнымиСтроками.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", КонтрольКачестваКэш);
	СтруктураДействийСИзмененнымиСтроками.Вставить("ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества", КонтрольКачестваКэш);
...
&НаСервере
Функция ЕстьДоступКСкладуОтправителю()
	
	Если ЗначениеЗаполнено(Объект.СкладОтправитель) Тогда
		
		Если Не ПравоДоступа("Чтение", Объект.СкладОтправитель.Метаданные()) Тогда
			Возврат Ложь;
		КонецЕсли;
		Запрос = Новый Запрос(
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		Запрос.УстановитьПараметр("Ссылка", Объект.СкладОтправитель);
...
Процедура УстановитьВидимостьОбеспечения()
	...
	ЕстьПраво = ОбеспечениеВДокументахСервер.ЕстьПравоПросмотраГиперссылкиСостоянияОбеспечения()
		И ЕстьДоступКСкладуОтправителю;
	Элементы.СтраницыГиперссылкаСостояниеОбеспечения.Видимость = ЕстьПраво И Не Объект.ПеремещениеЗаблокированныхТоваров;
	Элементы.ТоварыКОбеспечению.Видимость = Не Объект.ПеремещениеЗаблокированныхТоваров;
	Элементы.ТоварыНеОбеспечивать.Видимость = Не Объект.ПеремещениеЗаблокированныхТоваров;
	Элементы.ТоварыКонтекстноеМенюКОбеспечению.Видимость = Не Объект.ПеремещениеЗаблокированныхТоваров;
	Элементы.ТоварыКонтекстноеМенюНеОбеспечивать.Видимость = Не Объект.ПеремещениеЗаблокированныхТоваров;
	...
	Если Не ПравоДоступа("Добавление", Метаданные.Документы.КорректировкаНазначенияТоваров)
		...
		Или Объект.ПеремещениеЗаблокированныхТоваров Тогда
...
Процедура ЗаполнитьМногооборотнуюТаруИзХранилищаСервер(АдресТарыВХранилище)
	...
	СтруктураДействийДобавленныеСтроки.Вставить("ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества", КонтрольКачестваКэш);
...
Функция СкорректироватьСтрокиЗаказа(СтруктураДополнительныхПараметров)
	...
	ОбрабатываемыеСтроки = Объект.Товары.НайтиСтроки(Новый Структура("Отменено", Истина));
	СтруктураДействий = Новый Структура();
	СтруктураДействий.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", КонтрольКачестваКэш);
	СтруктураДействий.Вставить("ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества", КонтрольКачестваКэш);
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
