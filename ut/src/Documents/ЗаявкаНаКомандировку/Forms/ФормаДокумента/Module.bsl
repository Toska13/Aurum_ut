...
Процедура ЧислоДнейПриИзменении(Элемент)
	...
	Элементы.ЧислоДней.Заголовок = ОбщегоНазначенияУТКлиентСервер.СклонениеСлова(
		ЧислоДней, НСтр("ru='день, дня, дней, м'"));
...
Функция БронированиеСсылка(ИдентификаторБронирования)
	...
	Возврат Документы.ОперацияСЭлектроннымБилетом.ПолучитьСсылку(Новый УникальныйИдентификатор(ИдентификаторБронирования));
...
&НаКлиенте
Процедура РасходыКомментарийРасходовНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияКомментария(Элемент.ТекстРедактирования, ЭтотОбъект,
		"Элементы.Расходы.ТекущиеДанные.КомментарийРасходов", НСтр("ru = 'Детализация расходов'"));
...
Функция СкачатьВсеНаСервере(Каталог)
	...
	|ИЗ
	|	Справочник.ОперацияСЭлектроннымБилетомПрисоединенныеФайлы КАК ДанныеСправочника
	|ГДЕ
...
Процедура ВывестиИнформациюОБронировании()
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		Справочник.ОперацияСЭлектроннымБилетомПрисоединенныеФайлы КАК Файлы
	|	ПО
	...
	|	ДанныеДокумента.ДатаНачала                              КАК ДатаНач,
	|	ДанныеДокумента.ДатаОкончания                           КАК ДатаКон,
	|	ДанныеДокумента.СуммаДокумента                          КАК Сумма,
	|	ПРЕДСТАВЛЕНИЕ(ДанныеДокумента.Валюта)                   КАК Валюта,
	|	ПРЕДСТАВЛЕНИЕ(ДанныеДокумента.ТипУслуги)                КАК ТипУслуги,
	|	ВЫРАЗИТЬ(ДанныеДокумента.ДеталиБронирования КАК Строка(100)) КАК Детали,
	|	ДанныеДокумента.Ссылка                                  КАК Ссылка,
	|	КОЛИЧЕСТВО(Файлы.Ссылка)                                КАК КоличествоФайлов
	|ИЗ
	|	Документ.БронированиеSmartway КАК ДанныеДокумента
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		Справочник.БронированиеSmartwayПрисоединенныеФайлы КАК Файлы
	|	ПО
	|		Файлы.ВладелецФайла = ДанныеДокумента.Ссылка
	|ГДЕ
	|	ДанныеДокумента.ДокументОснование = &Ссылка
	...
	Запрос.Текст = СтрСоединить(МассивТекстов, ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении());
...
Процедура УправлениеЭлементамиФормы()
	...
	Элементы.РасходыСтатьяБюджетов.Видимость = Документы.ЗаявкаНаКомандировку.ИспользоватьЛимитыБДР();
