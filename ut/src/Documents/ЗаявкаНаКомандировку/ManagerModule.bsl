...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	НоваяКоманда = Документы.ОперацияСЭлектроннымБилетом.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
	Если НоваяКоманда <> Неопределено Тогда
		НоваяКоманда.Представление = НСтр("ru = 'Электронный билет'");
		НоваяКоманда.ВидимостьВФормах = "ФормаДокумента, ФормаСписка";
		НоваяКоманда.МножественныйВыбор = Ложь;
	КонецЕсли;
...
// Возвращает флаг использования лимитов БДР в заявках на командировку.
// 
// Возвращаемое значение:
//  Булево
...
Процедура ЗаполнитьПоОснованию(Объект, Ссылка) Экспорт
	...
	Если ТипОбъекта = Тип("ДокументОбъект.ОперацияСЭлектроннымБилетом") Тогда
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "Документы.ЗаявкаНаКомандировку.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "11.5.26.45";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("55fc8b34-5135-4b0b-acf7-9f6e221c19e2");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ЗаявкаНаКомандировку.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Порядок = Перечисления.ПорядокОбработчиковОбновления.Обычный;
	Обработчик.Комментарий = НСтр("ru = 'Заполняет первую строку табличной части ""Расходы"".'");
	
	Читаемые = Новый Массив;
	Читаемые.Добавить(Метаданные.Документы.ЗаявкаНаКомандировку.ПолноеИмя());
	Обработчик.ЧитаемыеОбъекты = СтрСоединить(Читаемые, ",");
	
	Изменяемые = Новый Массив;
	Изменяемые.Добавить(Метаданные.Документы.ЗаявкаНаКомандировку.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Документы.ЗаявкаНаКомандировку.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.ЗаявкаНаКомандировку";
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаКомандировку.Расходы КАК ЗаявкаНаКомандировкуРасходы
	|		ПО ЗаявкаНаКомандировку.Ссылка = ЗаявкаНаКомандировкуРасходы.Ссылка
	|ГДЕ
	|	ЗаявкаНаКомандировку.ПредполагаемаяСуммаРасходов <> 0
	|	И ЗаявкаНаКомандировкуРасходы.Ссылка ЕСТЬ NULL
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "Документ.ЗаявкаНаКомандировку";
	...
	Для Каждого Выборка Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			НоваяСтрока.ПланируемаяДатаРасходов = ДокументОбъект.ДатаОкончания;
			НоваяСтрока.Сумма = ДокументОбъект.ПредполагаемаяСуммаРасходов;
			НоваяСтрока.КомментарийРасходов = ДокументОбъект.КомментарийРасходов;
