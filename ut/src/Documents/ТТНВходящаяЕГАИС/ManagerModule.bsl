...
Функция СтатусПослеПолученияДанных(ДокументСсылка, Операция, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Операция = Перечисления.ВидыДокументовЕГАИС.ТТН
		Или Операция = Перечисления.ВидыДокументовЕГАИС.УведомлениеОРегистрацииДвиженияТТН
		Или Операция = Перечисления.ВидыДокументовЕГАИС.КвитанцияЗапросаНаОтменуПроведенияТТНПодтверждение Тогда
...
// 
// Возвращаемое значение:
// 	Массив из ПеречислениеСсылка.ДальнейшиеДействияПоВзаимодействиюЕГАИС - дальнейшие действия
//
...
// 
// Параметры:
//   Списки - см. УправлениеДоступомПереопределяемый.ПриЗаполненииОграниченияДоступа.Ограничение.
...
Функция ВходящееСообщение(ДокументСсылка)
...
Функция ЗавершенаПроверкаИПодбор(Документ) Экспорт
	...
	КонецЕсли;
	СтатусПроверкиИПодбора = РегистрыСведений.СтатусыПроверкиИПодбораИС.ТекущийСтатус(Ссылка, 0);
...
Процедура ЗаполнитьАкцизныеМарки(ДокументОбъект, ДеревоУпаковок, УпаковкаВерхнегоУровня = Неопределено) Экспорт
	...
	Для Каждого СтрокаДерева Из ДеревоУпаковок.Строки Цикл
		...
		Если СтрокаДерева.ВидУпаковки = Перечисления.ВидыУпаковокИС.НеЯвляетсяУпаковкой Тогда
			Продолжить;
		ИначеЕсли СтрокаДерева.ТипУпаковки = Перечисления.ПрочиеЗоныПересчетаПродукцииИС.ОбъемноСортовойУчет Тогда
			Продолжить;
		ИначеЕсли ТипЗнч(СтрокаДерева.ТипУпаковки) = Тип("ПеречислениеСсылка.ПрочиеЗоныПересчетаПродукцииИС") Тогда
			ЗаполнитьАкцизныеМарки(ДокументОбъект, СтрокаДерева);
			Продолжить;
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(СтрокаДерева.Справка2) Тогда
			ЗаполнитьАкцизныеМарки(ДокументОбъект, СтрокаДерева, СтрокаДерева.ШтрихкодУпаковки);
		КонецЕсли;
...
//   Особенность: выполняется пересчет из количества ЕГАИС, которое указывается в этом документе
// 
// Параметры:
//  ТаблицаМаркируемойПродукции - См. Обработки.ПроверкаИПодборПродукцииИС.ПодобраннаяМаркируемаяПродукция
//  ДокументСсылка - ДокументСсылка.ТТНВходящаяЕГАИС - Документ, для которого выполняется заполнение
...
Процедура ЗаполнитьПродукциюДокумента(ТаблицаМаркируемойПродукции, ДокументСсылка, Сценарий) Экспорт
	...
	|;
	|" + ИнтеграцияЕГАИС.ТекстЗапросаВТКоэффициентыПересчетаВЕдиницыЕГАИС("Товары", "КоэффициентыПересчета") + "
	|;
	|ВЫБРАТЬ
	|	Товары.АлкогольнаяПродукция КАК АлкогольнаяПродукция,
	|	Товары.Номенклатура КАК Номенклатура,
	|	Товары.Характеристика КАК Характеристика,
	|	Товары.Серия КАК Серия,
	|	СУММА(Товары.Количество / ВЫБОР
	|		КОГДА ЕСТЬNULL(КоэффициентыПересчета.Коэффициент, 0) = 0 ТОГДА 1
	|		ИНАЧЕ КоэффициентыПересчета.Коэффициент КОНЕЦ * ВЫБОР
	|		КОГДА ЕСТЬNULL(ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке, 0) = 0 ТОГДА 1
	|		ИНАЧЕ ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке КОНЕЦ) КАК Количество,
	|	Товары.Справка2 КАК Справка2
	|ИЗ
	|	Товары КАК Товары
	|	ЛЕВОЕ СОЕДИНЕНИЕ КоэффициентыПересчета КАК КоэффициентыПересчета
	|		ПО КоэффициентыПересчета.Номенклатура = Товары.Номенклатура
	|		И КоэффициентыПересчета.Характеристика = Товары.Характеристика
	|		И КоэффициентыПересчета.Серия = Товары.Серия
	|		И КоэффициентыПересчета.АлкогольнаяПродукция = Товары.АлкогольнаяПродукция
...
// Дерево упаковок для входящей ТТН ЕГАИС.
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка.ТТНВходящаяЕГАИС - Документ ссылка
// 
// Возвращаемое значение:
//  ДеревоЗначений - Штрихкоды упаковок для формы проверки.
Функция ШтрихкодыУпаковокСообщения(ДокументСсылка) Экспорт
	...
	|	Товары.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	Товары.АлкогольнаяПродукция КАК АлкогольнаяПродукция,
	|	ПРЕДСТАВЛЕНИЕ(Товары.АлкогольнаяПродукция) КАК ПредставлениеНоменклатуры,
	|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЕГАИС В (&МаркируемаяПродукция) КАК МаркируемаяПродукция,
	|	Товары.Номенклатура КАК Номенклатура,
	|	Товары.Характеристика КАК Характеристика,
	|	Товары.Серия КАК Серия,
	...
	|		КОГДА ЕСТЬNULL(КоэффициентыПересчета.Коэффициент, 0) = 0 ТОГДА 1
	|		ИНАЧЕ КоэффициентыПересчета.Коэффициент КОНЕЦ * ВЫБОР
	|		КОГДА ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке = 0 ТОГДА 1
	|		ИНАЧЕ ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке КОНЕЦ КАК КоличествоПоСтроке,
	|	ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке = 0 КАК ТребуетВзвешивания,
	|	ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке <> 1 КАК ПроизвольнаяЕдиницаУчета,
	|	ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке КАК Количество,
	|	Товары.Справка2 КАК Справка2,
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылка);
	Запрос.УстановитьПараметр("МаркируемаяПродукция", КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЕГАИСМаркируемые());
	ТаблицаТовары = Запрос.Выполнить().Выгрузить();
	
	ДеревоУпаковок = Новый ДеревоЗначений;
	Для Каждого ИмяКолонки Из КолонкиДерева() Цикл
		ДеревоУпаковок.Колонки.Добавить(ИмяКолонки);
	КонецЦикла;
	
	СообщениеОбмена = ДанныеДокументаТТНv3(ДокументСсылка);
	
	Если СообщениеОбмена <> Неопределено
		И (СообщениеОбмена.ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V3
			Или СообщениеОбмена.ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V4) Тогда
		
		Для Каждого ЭлементДанных Из СообщениеОбмена.Данные.Content.Position Цикл
			...
			Если MarkInfo <> Неопределено Тогда
				...
				Для Каждого boxpos Из MarkInfo.boxpos Цикл
					...
					ДанныеШтрихкодаУпаковки = Новый Структура(СтрСоединить(КолонкиДерева(),","));
					ДанныеШтрихкодаУпаковки.Вставить("АкцизныеМарки", Новый Массив);
					
					Если СтрокаТЧ <> Неопределено Тогда
						ЗаполнитьЗначенияСвойств(ДанныеШтрихкодаУпаковки, СтрокаТЧ);
					КонецЕсли;
		...
		ДобавитьКодыОСУНемаркируемойПродукцииВДерево(ДеревоУпаковок, ТаблицаТовары);
...
//   Особенность: выполняется пересчет в количество ЕГАИС, которое указывается в этом документе
//   * Сохраняет результаты проверки в регистр безусловно
//
// Параметры:
...
Процедура ОтразитьРезультатыПроверкиИПодбора(ПараметрыОкончанияПроверки) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	|	ТаблицаПродукции.АлкогольнаяПродукция КАК АлкогольнаяПродукция,
	...
	|	ТаблицаПродукции.КоличествоПодобрано КАК КоличествоПодобрано,
	|	ТаблицаПродукции.Справка2 КАК Справка2
	|ПОМЕСТИТЬ Товары
	|ИЗ
	...
	|	Товары.Характеристика КАК Характеристика,
	|	Товары.Серия КАК Серия,
	|	СУММА(Товары.КоличествоПодобрано * ВЫБОР
	|		КОГДА ЕСТЬNULL(КоэффициентыПересчета.Коэффициент, 0) = 0 ТОГДА 1
	|		ИНАЧЕ КоэффициентыПересчета.Коэффициент КОНЕЦ / ВЫБОР
	|		КОГДА ЕСТЬNULL(ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке, 0) = 0 ТОГДА 1
	|		ИНАЧЕ ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке КОНЕЦ) КАК КоличествоПодобрано,
	|	Товары.Справка2 КАК Справка2
	|ИЗ
	|	Товары КАК Товары
	|	ЛЕВОЕ СОЕДИНЕНИЕ КоэффициентыПересчета КАК КоэффициентыПересчета
	...
	Для Каждого СтрокаТаблицы Из ТаблицаПродукции Цикл
		...
		Для Каждого СтрокаТовары Из НайденныеСтроки Цикл
			...
			Если СтрокаТовары.Номенклатура <> СтрокаТаблицы.Номенклатура
				Или СтрокаТовары.Характеристика <> СтрокаТаблицы.Характеристика
				Или СтрокаТовары.Серия <> СтрокаТаблицы.Серия Тогда
				СтрокаТовары.Номенклатура = СтрокаТаблицы.Номенклатура;
				СтрокаТовары.Характеристика = СтрокаТаблицы.Характеристика;
				СтрокаТовары.Серия = СтрокаТаблицы.Серия;
				ИзмененныеСтроки.Добавить(СтрокаТовары);
			КонецЕсли;
			
			ЗаполнитьПоСтроке = Мин(Заполнить, СтрокаТовары.Количество - СтрокаТовары.КоличествоФакт);
			СтрокаТовары.КоличествоФакт = СтрокаТовары.КоличествоФакт + ЗаполнитьПоСтроке;
			Заполнить = Заполнить - ЗаполнитьПоСтроке;
		КонецЦикла;
		
		Если Заполнить > 0 И НайденныеСтроки.Количество() Тогда
			СтрокаТовары = НайденныеСтроки[0];
			СтрокаТовары.КоличествоФакт = СтрокаТовары.КоличествоФакт + Заполнить;
		КонецЕсли;
		
	КонецЦикла;
	
	ОбработатьСтрокиТабличнойЧасти(ДокументОбъект, ИзмененныеСтроки);
	
	ДокументОбъект.АкцизныеМарки.Очистить();
	ЗаполнитьАкцизныеМарки(ДокументОбъект, ПараметрыОкончанияПроверки.ДеревоМаркированнойПродукции);
	
	ИндексСтрокиТовары = 0;
	ТоварыКоличество   = ДокументОбъект.Товары.Количество();
	
	ДокументОбъект.ЕстьРасхождения = Ложь;
	Пока Не ДокументОбъект.ЕстьРасхождения
		И (ИндексСтрокиТовары < ТоварыКоличество) Цикл
		ДокументОбъект.ЕстьРасхождения = ДокументОбъект.Товары[ИндексСтрокиТовары].Количество <> ДокументОбъект.Товары[ИндексСтрокиТовары].КоличествоФакт;
...
Процедура ОбработатьСтрокиТабличнойЧасти(ДокументОбъект, ИзмененныеСтроки)
	
	ПараметрыУказанияСерий = ПараметрыУказанияСерий(ДокументОбъект);
	Склад = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект.Грузополучатель, "ТорговыйОбъект");
...
// * Штрихкод - Строка - 
Функция ВходящиеАкцизныеМарки(ДокументСсылка, Товары = Неопределено)
...
#Область ПроверкаИПодборСлужебные
...
Функция КолонкиДерева()
	...
	Результат.Добавить("НормализованныйШтрихкод");
	Результат.Добавить("ТипУпаковки");
	Результат.Добавить("ВидУпаковки");
	...
	Результат.Добавить("МаркируемаяПродукция");
	...
	Результат.Добавить("КоличествоПодчиненныхВсего");
	Результат.Добавить("КоличествоПодчиненныхНеПроверялось");
	Результат.Добавить("ПредставлениеНоменклатуры");
	Возврат Результат;
	
КонецФункции

// Добавляет штрихкоды упаковок и акцизных марок в дерево
//
			...
			Для Каждого Штрихкод Из ДанныеУпаковки.boxnum Цикл
				...
				Если ЗначениеЗаполнено(Штрихкод) Тогда
					...
					Если СтрокаДерева = Неопределено Тогда
						...
						СтрокаДерева.НормализованныйШтрихкод = Штрихкод;
				...
				Если ДанныеШтрихкодаУпаковки <> Неопределено Тогда
					...
					Для Каждого КодАкцизнойМарки Из ДанныеШтрихкодаУпаковки.АкцизныеМарки Цикл
						...
						ЗаполнитьЗначенияСвойств(СтрокаАкцизнойМарки, ДанныеШтрихкодаУпаковки);
						СтрокаАкцизнойМарки.Штрихкод             = КодАкцизнойМарки;
						СтрокаАкцизнойМарки.НормализованныйШтрихкод = КодАкцизнойМарки;
						СтрокаАкцизнойМарки.ТипУпаковки          = Перечисления.ТипыУпаковок.МаркированныйТовар;
						СтрокаАкцизнойМарки.ВидУпаковки          = Перечисления.ВидыУпаковокИС.Потребительская;
						СтрокаАкцизнойМарки.КоличествоПотребительскихУпаковок = 1;
					КонецЦикла;
...
Процедура ДобавитьКодыОСУНемаркируемойПродукцииВДерево(ДеревоУпаковок, ТаблицаТовары)
	
	Для Каждого СтрокаТовар Из ТаблицаТовары Цикл
		Если Не СтрокаТовар.МаркируемаяПродукция Тогда
			СтрокаАкцизнойМарки = ДеревоУпаковок.Строки.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаАкцизнойМарки, СтрокаТовар);
			СтрокаАкцизнойМарки.Штрихкод    = НСтр("ru = '<неприменимо>'");
			СтрокаАкцизнойМарки.ТипУпаковки = Перечисления.ТипыУпаковок.МонотоварнаяУпаковка;
			СтрокаАкцизнойМарки.ВидУпаковки = Перечисления.ВидыУпаковокИС.НеЯвляетсяУпаковкой;
			СтрокаАкцизнойМарки.Количество  = СтрокаТовар.КоличествоПоСтроке;
			Если Не СтрокаТовар.ТребуетВзвешивания Тогда
				СтрокаАкцизнойМарки.КоличествоПотребительскихУпаковок  = СтрокаТовар.КоличествоПоСтроке;
				СтрокаАкцизнойМарки.КоличествоПодчиненныхВсего         = СтрокаТовар.КоличествоПоСтроке;
				СтрокаАкцизнойМарки.КоличествоПодчиненныхНеПроверялось = СтрокаТовар.КоличествоПоСтроке;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
