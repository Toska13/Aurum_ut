...
Процедура СформироватьЗаданияКФормированиюДвиженийПоНДС(ТаблицаИзменений) Экспорт
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданныйАванс КАК СчетФактура
...
Процедура ЗаполнитьПолученныеАвансыДляСФ(ОтборАвансов, ПолученныеАвансы, РасчетныеДокументыБезСчетаФактуры = Неопределено) Экспорт
	...
	Пока ВыборкаАванс.Следующий() Цикл
		...
		Для каждого Ставка Из МассивСтавок Цикл
			...
			Если СуммаНДС = 0
				И Не КосмическаяДеятельность
				И Не ОблагаетсяНДСУПокупателя Тогда
					Продолжить;
			КонецЕсли;
		...
		Для каждого СтрокаАвансы Из Авансы Цикл
			...
			Если ВозможныйТипДокументаОснования.СодержитТип(ТипЗнч(СтрокаАвансы.ДокументОснование))
				...
				Иначе
					...
					Если ПолученныеАвансы.Колонки.Найти("НетДанныхПлатежноРасчетногоДокумента") <> Неопределено Тогда
						Если РеквизитыСчетаФактуры.НомерПлатежноРасчетногоДокумента = ""
							И РеквизитыСчетаФактуры.ДатаПлатежноРасчетногоДокумента = '00010101' Тогда
							НоваяСтрока.НетДанныхПлатежноРасчетногоДокумента = Истина;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли; 
				
				Если ПолученныеАвансы.Колонки.Найти("ОснованиеСторно") <> Неопределено
					И ТипЗнч(СтрокаАвансы.ДокументОснование) = Тип("ДокументСсылка.Сторно") Тогда
					НоваяСтрока.ОснованиеСторно = Истина;
				КонецЕсли;
	...
	ПоляГруппировкиИтоговые = Новый Массив;
	ПоляГруппировкиИтоговые.Добавить("Контрагент, ДокументОснование, ОбъектРасчетов, НалогообложениеНДС, НаправлениеДеятельности, ВалютаДокумента,
		|СтавкаНДС, Ответственный, Подразделение, Руководитель, ГлавныйБухгалтер, ДатаВыписки, СФСформирован, Исправление, СуммаСчетаФактуры,
		|СчетФактура, ПравилоОтбораАванса");
	Если Не ПолученныеАвансы.Колонки.Найти("Организация") = Неопределено Тогда
		ПоляГруппировкиИтоговые.Добавить("Организация");
	КонецЕсли;
	Если ПолученныеАвансы.Колонки.Найти("НетДанныхПлатежноРасчетногоДокумента") <> Неопределено Тогда
		ПоляГруппировкиИтоговые.Добавить("НетДанныхПлатежноРасчетногоДокумента");
	КонецЕсли;
	Если ПолученныеАвансы.Колонки.Найти("ОснованиеСторно") <> Неопределено Тогда
		ПоляГруппировкиИтоговые.Добавить("ОснованиеСторно");
	КонецЕсли;
	
	ПолученныеАвансы.Свернуть(СтрСоединить(ПоляГруппировкиИтоговые,","), ПоляСуммирования);
...
Функция ТаблицаПолученныеАвансы() Экспорт
	...
	ПолученныеАвансы.Колонки.Добавить("Исправление",              Новый ОписаниеТипов("Булево"));
...
Функция ЕстьСчетаФактурыКОформлению(Параметры) Экспорт
	...
	ПравилаОтбораАвансов = Новый Соответствие;
	Если ТипЗнч(Параметры.МассивОрганизаций) = Тип("СправочникСсылка.Организации") Тогда
		НастройкиУчетаНДС = НастройкиНалоговУчетныхПолитик.ДействующиеПараметрыНалоговУчетныхПолитикНаДату("НастройкиУчетаНДС",
					Параметры.МассивОрганизаций,
					Параметры.КонецПериода,
					Истина);
		Если НастройкиУчетаНДС <> Неопределено Тогда
			ПравилаОтбораАвансов.Вставить(НастройкиУчетаНДС.ПравилоОтбораАвансовДляРегистрацииСчетовФактур, Параметры.МассивОрганизаций);
		КонецЕсли;
	ИначеЕсли ТипЗнч(Параметры.МассивОрганизаций) = Тип("Массив") Тогда
		Для Каждого Организация Из Параметры.МассивОрганизаций Цикл
			НастройкиУчетаНДС = НастройкиНалоговУчетныхПолитик.ДействующиеПараметрыНалоговУчетныхПолитикНаДату("НастройкиУчетаНДС",
						...
						Истина);
			Если НастройкиУчетаНДС <> Неопределено Тогда     
				Если ПравилаОтбораАвансов.Получить(НастройкиУчетаНДС.ПравилоОтбораАвансовДляРегистрацииСчетовФактур) = Неопределено Тогда
					МассивОрганизаций = Новый Массив;
					МассивОрганизаций.Добавить(Организация);
					ПравилаОтбораАвансов.Вставить(НастройкиУчетаНДС.ПравилоОтбораАвансовДляРегистрацииСчетовФактур, МассивОрганизаций);
				Иначе
					ПравилаОтбораАвансов[НастройкиУчетаНДС.ПравилоОтбораАвансовДляРегистрацииСчетовФактур].Добавить(Организация);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	ПолученныеАвансыОбщие = ТаблицаПолученныеАвансы();
	
	Для Каждого Правило Из ПравилаОтбораАвансов Цикл
		
		ПравилоОтбораАванса = Правило.Ключ;
		МассивОрганизаций = Правило.Значение;
		...
		ОтборАвансов.Вставить("Организация", МассивОрганизаций);
		...
		ОбщегоНазначенияУТ.ДобавитьСтрокиВТаблицу(ПолученныеАвансыОбщие, ПолученныеАвансы);
		
	КонецЦикла;
	
	Если Параметры.Свойство("ВернутьПолнуюТаблицу") Тогда
		Возврат ПолученныеАвансыОбщие;
	Иначе
		Возврат ПолученныеАвансыОбщие.НайтиСтроки(Новый Структура("СФСформирован", Ложь)).Количество();
	КонецЕсли;
...
Функция АвансыПоПлатежномуДокументу(ПлатежныйДокумент, Организация) Экспорт
	...
	Возврат СчетФактураВыданныйАвансЛокализация.АвансыПоПлатежномуДокументу(ПлатежныйДокумент, Организация);
...
Функция ТаблицаЗарегистрированныхСчетовФактур(МенеджерВременныхТаблиц)
	...
	|	СчетФактураВыданныйАванс.Исправление          КАК Исправление,
	...
	|	СчетФактураВыданныйАванс.НомерПлатежноРасчетногоДокумента,
	|	СчетФактураВыданныйАванс.ДатаПлатежноРасчетногоДокумента
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
