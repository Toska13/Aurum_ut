...
Функция РеквизитыПоступлениеБезналичныхДенежныхСредств(Ссылка)
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ДанныеДокумента.ДатаПроведенияБанком	КАК Дата,
	|	ДанныеДокумента.Ссылка					КАК Ссылка,
	|	ДанныеДокумента.Проведен				КАК Проведен,
	|	ДанныеДокумента.ХозяйственнаяОперация	КАК ХозяйственнаяОперация,
	|	ВЫБОР
...
Процедура ЗаполнитьПоПлатежномуДокументу(РеквизитыЗаполнения)
	...
	Если Не ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		...
		КонецЕсли;
	Иначе
		Если МассивАналитикУчетаПоПартнерам.Количество() > 0 
			И РегистрыСведений.ЗаданияКРаспределениюВзаиморасчетов.АктуальныйПериодРассчитанныхДанныхПоВзаиморасчетам(МассивАналитикУчетаПоПартнерам,, КонецМесяца(РеквизитыЗаполнения.Дата)) <> Неопределено Тогда
			ТекстСообщения = ВзаиморасчетыСервер.ТекстПредупрежденияЗаданияРаспределенияВзаиморасчетов();
...
Процедура ЗаполнитьДатуПолученияАванса()
	...
	Если Выборка.Следующий()
		И ПравилоОтбораАванса <> Перечисления.ПорядокРегистрацииСчетовФактурНаАванс.КромеЗачтенныхВТечениеКвартала
		И ЗначениеЗаполнено(Выборка.ДатаПолученияАванса) Тогда
