...
// 
// Параметры:
//  ОбъектИлиСсылка - ДокументОбъект.ВозвратТоваровОтКлиента, ДокументСсылка.ВозвратТоваровОтКлиента - документ.
...
// 
// Параметры:
//  Объект - ДокументОбъект.ВозвратТоваровОтКлиента - объект.
// 
// Возвращаемое значение:
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаявкиНаВозврат(ЗаявкаНаВозвратТоваров, ВозвратТоваровОтКлиента) Экспорт
	...
	|		ЗаказыОстатки.КОформлениюПоВозвратамКоличествоОборот -
	|			ЗаказыОстатки.ОформленоПоВозвратамКоличествоОборот КАК КОформлению
	|	ИЗ
	|		#ЗаявкиНаВозвратОборотыРазвернутая КАК ЗаказыОстатки
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|		-ЗаказыДвижения.Количество
	|	ИЗ
	|		РегистрНакопления.РаспоряженияНаОтгрузкуИВозврат КАК ЗаказыДвижения
	|	ГДЕ
	|		ЗаказыДвижения.Регистратор = &Регистратор
	|		И ЗаказыДвижения.Распоряжение = &ЗаявкаНаВозвратТоваров
	|		И Показатель В (ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.КОформлениюПоВозвратам),
	...
	ПараметрыФормированияТаблицы.ТекстУсловия =
		"Распоряжение = &ЗаявкаНаВозвратТоваров";
	ПараметрыФормированияТаблицы.Группировка = "Номенклатура, Характеристика, КодСтроки";
...
Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|		 И &ПартионныйУчетВерсии22 И &ФИФОСкользящаяОценка
	...
	// При возврате не по документу реализации в аналитике учета номенклатуры указан склад возврата, поэтому
	// проверяется признак цеховой кладовой склада возврата. 
	|		КОГДА ВидыЗапасов.СпособОпределенияСебестоимости В (
	|			ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимостиВозврата.ИзТекущегоДокумента),
	|			ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимостиВозврата.Вручную))
	|		 И ВидыЗапасов.ЦеховаяКладовая
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
	// При возврате по документу реализации проверяется признак цеховой кладовой склада отгрузки.
	|		КОГДА ВидыЗапасов.СпособОпределенияСебестоимости = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимостиВозврата.ИзДокументаПродажи)
	|		 И ВидыЗапасов.ЦеховаяКладоваяОтгрузки 
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
	...
	|			И (ВидыЗапасов.СкладОтгрузки <> ВидыЗапасов.СкладВозврата
	...
	|			ИЛИ ВидыЗапасов.Назначение <> ВидыЗапасов.НазначениеОтгрузки И &УчитыватьСебестоимостьТоваровПоНазначениям)
	|		ТОГДА ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.СторноРеализацииВозвратНаДругойСклад)
	|		ИНАЧЕ ВидыЗапасов.НастройкаХозяйственнойОперации
	...
	|ГДЕ
	|	НЕ (ВидыЗапасов.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара) И &ВозвратПереданнойМногооборотнойТары)
	|	И &ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОтКомиссионера)
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки
...
Функция ТекстЗапросаТаблицаДвиженияНоменклатураНоменклатура(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаВидыЗапасов.СкладВозврата КАК КорСклад,
...
Функция АдаптированныйТекстЗапросаДвиженийПоРегистру(ИмяРегистра) Экспорт
	...
	ПереопределениеРасчетаПараметров.Вставить("ИспользоватьОрдернуюСхемуПриПоступлении",
		"	ВЫБОР
		|		КОГДА ТаблицаТовары.Ссылка.Дата >= ТаблицаТовары.Склад.ДатаНачалаОрдернойСхемыПриПоступлении
...
Процедура ЗапланироватьПоступлениеТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ИсточникДанных.Ссылка.КонтролируемаяПартия              КАК КонтролируемаяПартия,
	...
	|	ВЫБОР
	|		КОГДА ИсточникДанных.Ссылка.ЗаявкаНаВозвратТоваровОтКлиента = ЗНАЧЕНИЕ(Документ.ЗаявкаНаВозвратТоваровОтКлиента.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ НЕ ИсточникДанных.Ссылка.ЗаявкаНаВозвратТоваровОтКлиента.ЭтоЗаказКакСчет
...
Процедура ОформитьПоступлениеТоваровПоОдноходовке(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ИНАЧЕ ИсточникДанных.Номенклатура
	|	КОНЕЦ													КАК Номенклатура,
	|	ИсточникДанных.Номенклатура								КАК НоменклатураДляСоединения,
	|	ВЫБОР
	|		КОГДА ИсточникДанных.Порча
	|			ТОГДА ИсточникДанных.ХарактеристикаОприходование
	|		ИНАЧЕ ИсточникДанных.Характеристика
	|	КОНЕЦ													КАК Характеристика,
	|	ИсточникДанных.Характеристика							КАК ХарактеристикаДляСоединения,
	|	ИсточникДанных.Назначение								КАК Назначение,
	|	ИсточникДанных.ПроконтролироватьКачество                КАК ПроконтролироватьКачество,
	|	ЛОЖЬ                                                    КАК ДвижениеРанееПоступившейПартии,
	|	ИсточникДанных.Ссылка.КонтролируемаяПартия              КАК КонтролируемаяПартия,
	...
	ПереопределяемыеПоляСоединенияССериями.Вставить("Номенклатура", "НоменклатураДляСоединения");
	ПереопределяемыеПоляСоединенияССериями.Вставить("Характеристика", "ХарактеристикаДляСоединения");
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры)
	...
	ВзаиморасчетыПроведениеКлиенты.ПроведениеВозвратаТоваровОтКлиента(Запрос, ТекстыЗапроса, Регистры, ТекстВозврат, ТекстРасшифровкаПлатежа);
...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	// Прочие поля
	|	ВЫБОР
	|		КОГДА &ВозвратЧерезКомиссионера И ТаблицаВидыЗапасов.ПериодПродажи < &НачалоМесяцаПериода
	...
	|	КОНЕЦ КАК ТипЗаписи,
	|	ВЫБОР
	|		КОГДА ТаблицаВидыЗапасов.СпособОпределенияСебестоимости В (ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимостиВозврата.Вручную),
	|																	ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимостиВозврата.ИзТекущегоДокумента))
	|			ТОГДА ДАТАВРЕМЯ(1,1,1)
	|		КОГДА Реализация.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности)
	|		 И Реализация.ДатаПереходаПраваСобственности <> ДАТАВРЕМЯ(1,1,1)
	|			ТОГДА Реализация.ДатаПереходаПраваСобственности
	|		ИНАЧЕ ЕСТЬNULL(РеестрДокументовРеализация.ДатаДокументаИБ, ДАТАВРЕМЯ(1,1,1))
	|	КОНЕЦ КАК ПериодПродажи
	|
	|ИЗ
	|	Документ.ВозвратТоваровОтКлиента КАК ТаблицаДокумента
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтТаблицаВидыЗапасов КАК ТаблицаВидыЗапасов
	...
	|		ПО Реализация.Ссылка = ТаблицаВидыЗапасов.ДокументРеализации
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументовРеализация
	|		ПО ТаблицаВидыЗапасов.ДокументРеализации = РеестрДокументовРеализация.Ссылка
	|			И НЕ РеестрДокументовРеализация.ДополнительнаяЗапись
	|ГДЕ
	|	ТаблицаДокумента.Ссылка В (&Ссылка)
	|	И ТаблицаВидыЗапасов.СпособОпределенияСебестоимости = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимостиВозврата.ИзДокументаПродажи)
	|	И (ТаблицаВидыЗапасов.ПериодПродажи < &НачалоМесяцаПериода
	//Условия возврата на другой склад (ХО СторноРеализацииВозвратНаДругойСклад), для этой ХО будет использоваться движение аналогичное возврату в прошлом периоде
	|		ИЛИ ТаблицаВидыЗапасов.СкладОтгрузки <> ТаблицаВидыЗапасов.СкладВозврата
	...
	|	И ТаблицаВидыЗапасов.СкладОтгрузки = ТаблицаВидыЗапасов.СкладВозврата
	|	И ТаблицаВидыЗапасов.НоменклатураОприходование = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|	И ТаблицаВидыЗапасов.ВидЗапасовОтгрузки = ТаблицаВидыЗапасов.ВидЗапасов
	|	И (ТаблицаВидыЗапасов.Назначение = ТаблицаВидыЗапасов.НазначениеОтгрузки
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Просмотр", Метаданные.Документы.ВозвратТоваровОтКлиента) Тогда
		...
		КомандаПечати.НаименованиеПечатнойФормы = НСтр("ru = 'Возврат от клиента'");
	КонецЕсли;
...
Функция ДанныеДляПечатиДебетКредитНота(ПараметрыПечати, МассивОбъектов, ДебетКредитНота, КодЯзыка) Экспорт
	...
	|	ЕСТЬNULL(ВТ_РеквизитыДокументаПродажи.БанковскийСчетОрганизации.СчетЦифровогоРубля, ЛОЖЬ) КАК ЭтоСчетЦифровогоРубля,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВТ_РеквизитыДокументаПродажи.БанковскийСчетОрганизации.СчетЦифровогоРубля, ЛОЖЬ)
	|			ТОГДА ВТ_РеквизитыДокументаПродажи.БанковскийСчетОрганизации.ИдентификаторСчетаЦифровогоРубля
	|		ИНАЧЕ ВТ_РеквизитыДокументаПродажи.БанковскийСчетОрганизации.НомерСчета
...
Функция ТекстЗапросаТаблицаРаспоряженияНаОтгрузку(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ПО ТаблицаПоказателей.Ссылка В (ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.ОформленоПоВозвратам),
...
Функция ТекстЗапросаВтТаблицаВидыЗапасов(Запрос, ТекстыЗапроса)
	...
	|		И Аналитика.МестоХранения = КлючиОприходования.МестоХранения
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
...
Функция ТекстЗапросаТаблицаПереданнаяВозвратнаяТара(Запрос, ТекстыЗапроса, Регистры)
	...
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ВозвратТарыОтКлиентаПрошлыхПериодов)
	|		КОГДА (ВтТаблицаВидыЗапасов.СкладОтгрузки <> ВтТаблицаВидыЗапасов.СкладВозврата
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|		И ТабЧасть.Ссылка.ЗаявкаНаВозвратТоваровОтКлиента <> ЗНАЧЕНИЕ(Документ.ЗаявкаНаВозвратТоваровОтКлиента.ПустаяСсылка) КАК ПоГрафику,
...
// Возвращает признак возврата после перехода права собственности
// 
// Параметры:
// 	ДокументВозврата - ДокументСсылка.ВозвратТоваровОтКлиента - проверяемый документ
// Возвращаемое значение:
// 	Булево - признак возврата после перехода права собственности
//
Функция ЭтоВозвратПослеПереходаПраваСобственности(ДокументВозврата) Экспорт
	
	ДокументРеализации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументВозврата, "ДокументРеализации");
	
	Возврат ТипЗнч(ДокументРеализации) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
			И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументРеализации, "ХозяйственнаяОперация") = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности;
...
Процедура ЗаполнитьПоЗаказамОрдерам(Товары, Регистратор, ПараметрыЗаполнения, ШтрихкодыУпаковок = Неопределено) Экспорт
	...
	Если ОтборПоСкладу Тогда
		...
		Отбор,
		"ИСТИНА",
		"Склад = &Склад ИЛИ Склад В ИЕРАРХИИ (&Склад)",
		"=");
	КонецЕсли;
...
Процедура ЗаполнитьШапкуДокументаПоЗаказу(Объект, ПараметрыЗаполнения, МассивЗаказов) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.ЗаявкаНаВозвратТоваровОтКлиента) Тогда
		Объект.Склад = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ЗаявкаНаВозвратТоваровОтКлиента, "Склад");
	КонецЕсли;
...
Процедура ОбновитьЗависимыеРеквизитыТабличнойЧасти(Товары, ПараметрыЗаполнения, ПересчитыватьСумму = Истина) Экспорт
	...
	СтруктураПересчетаСуммы = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(ПараметрыЗаполнения.РеквизитыШапки);
