...
// 
// Параметры:
//  Объект - ДокументОбъект.АктИнвентаризацииСАТУРН - Объект в котором происходит заполнение
//  СписокСтрок - Неопределено - Список строк
// 
// Возвращаемое значение:
//   Структура - структура сообщения для пользователя о результатах заполнения
...
//  МассивСтрок - Массив Из СтрокаТабличнойЧасти - Массив строк
...
//			ЭлементДанных.Вставить("productionDate",     СтрокаТовары.ДатаПроизводства);
	...
	Для Каждого КлючИЗначение Из СпискиПродукции Цикл
		...
		ИнтерфейсСАТУРН.ДобавитьАргументШапки(АргументыОперации, "docDate", ЗаписатьДатуJSON(Шапка.ДатаРегистрации, ФорматДатыJSON.ISO, ВариантЗаписиДатыJSON.УниверсальнаяДата));
