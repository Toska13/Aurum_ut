...
//
// Возвращаемое значение:
//   См. РегистрыСведений.СтатусыДокументовСАТУРН.РассчитатьСтатусы
//
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ОбновлениеИнформационнойБазыСАТУРН.ДобавитьОбработчикПриПереходеНаНовуюВерсиюAPI(
		...
		Новый УникальныйИдентификатор("316141d4-61ab-47c3-8318-c83b1206cc77"),
		Обработчики);
...
// 
// Параметры:
//  Объект - ДокументОбъект.ПроизводственнаяОперацияСАТУРН - Заполняемый документ
//  СписокСтрок - Неопределено - Список строк
// 
// Возвращаемое значение:
//  Структура - структура сообщения пользователю о результатах заполнения
...
Процедура ОбработкаЗагрузкиПолученныхДанных(ЭлементОчереди, ПараметрыОбмена, ПолученныеДанные, ИзмененныеОбъекты) Экспорт
	...
	Если ЭлементОчереди.Операция = Перечисления.ВидыОперацийСАТУРН.ПроизводственнаяОперацияСозданиеДокумента Тогда
		...
		ИдентификаторЧислом = ОписаниеТипаЧисло.ПривестиЗначение(Идентификатор);
		...
		ИнтерфейсСАТУРН.ДобавитьАргументШапки(РеквизитыИсходящегоСообщенияОснования.АргументыОперации, "id", ИдентификаторЧислом);
		ИнтерфейсСАТУРН.ДобавитьАргументtheCard(РеквизитыИсходящегоСообщенияОснования.АргументыОперации, "id", ИдентификаторЧислом);
	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийСАТУРН.ПроизводственнаяОперацияЧтениеДокумента Тогда
		...
		Попытка
			...
			Для Каждого СтрокаТаблицы Из ВходящиеДанные[0]._tparts.tbrProducedBatchInfo Цикл
				
				СтруктураХеша = СтруктураХешаПоДаннымТоваров(СтрокаТаблицы);
				ХешДанных = ИнтеграцияИС.ХешированиеДанныхSHA256(ОбщегоНазначения.ЗначениеВJSON(СтруктураХеша));
				
				МассивСтрок = РеквизитыИсходящегоСообщенияОснования.ПараметрыЗапроса.НомерСтрокПоИдентификаторамПартийТовары[ХешДанных];
				...
				НомерСтроки = МассивСтрок[0];
				
				СтрокаТовары        = ДокументОбъект.Товары[НомерСтроки - 1];
				ИдентификаторПартии = Формат(СтрокаТаблицы.batchIdReg, "ЧРД=; ЧРГ=; ЧГ=;");
				...
				// может быть найдено несколько строк с одинаковым хешем, необходимо распределить
				Если МассивСтрок.Количество() > 1 Тогда
					МассивСтрок.Удалить(0);
					РеквизитыИсходящегоСообщенияОснования.ПараметрыЗапроса.НомерСтрокПоИдентификаторамПартийТовары.Вставить(ХешДанных, МассивСтрок);
				КонецЕсли;
...
Функция ПроизводственнаяОперацияJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	|	Сырье.ИдентификаторСтрокиТоваров КАК ИдентификаторСтроки,
	...
	ИнтерфейсСАТУРН.ДобавитьАргументШапки(АргументыОперации, "description",          СтрШаблон(НСтр("ru = '%1 №%2'"), Шапка.НаименованиеПервичногоДокумента, Шапка.Номер));
	ИнтерфейсСАТУРН.ДобавитьАргументШапки(АргументыОперации, "docDate",              ЗаписатьДатуJSON(Шапка.ДатаПроизводства, ФорматДатыJSON.ISO, ВариантЗаписиДатыJSON.УниверсальнаяДата));
	ИнтерфейсСАТУРН.ДобавитьАргументШапки(АргументыОперации, "warehouseDestination", Шапка.МестоХраненияИдентификатор);
	ИнтерфейсСАТУРН.ДобавитьАргументШапки(АргументыОперации, "id",                   0);
	
	ИнтерфейсСАТУРН.ДобавитьАргументtheCard(АргументыОперации, "id", 0);
	
	СписокСырья     = Новый Соответствие();
	СписокПродукции = Новый Массив();
	
	ВсегоСтрокСырья     = 0;
	ВсегоСтрокПродукции = 0;
	...
	Для Каждого ЭлементКоллекции Из Сырье Цикл
		...
		ЭлементДанных.Вставить("patBatchId",        ЭлементКоллекции.ПартияСАТУРНИдентификатор);
		ЭлементДанных.Вставить("patCountPuCounted", ЭлементКоллекции.КоличествоУпаковок);
		
		ЭлементДанных.Вставить("sourceBase",            0);
		ЭлементДанных.Вставить("sourceBatch",           0);
		ЭлементДанных.Вставить("sourceCountSourceBase", 0);
		ЭлементДанных.Вставить("sourcePuUnit",          "");
		
		МассивСырья = СписокСырья.Получить(ЭлементКоллекции.ИдентификаторСтроки);
		
		Если МассивСырья = Неопределено Тогда
			МассивСырья = Новый Массив;
		КонецЕсли;
		
		МассивСырья.Добавить(ЭлементДанных);
		СписокСырья.Вставить(ЭлементКоллекции.ИдентификаторСтроки, МассивСырья);
		
		ВсегоСтрокСырья = ВсегоСтрокСырья + 1;
		
	КонецЦикла;
	
	НомерСтрокПоИдентификаторамПартийТовары = Новый Соответствие();
	...
	Для Каждого ЭлементКоллекции Из Товары Цикл
		...
		ЭлементДанных.Вставить("createdPatId",    ЭлементКоллекции.ПАТИдентификатор);
		ЭлементДанных.Вставить("clBatchCode",     ЭлементКоллекции.НомерПартии);
		...
		ЭлементДанных.Вставить("puKgWeight",      ЭлементКоллекции.КоличествоВУпаковкеСАТУРН);
		ЭлементДанных.Вставить("countPuUnit",     ЭлементКоллекции.КоличествоУпаковок);
		...
		ЭлементДанных.Вставить("destinationBatchId",    0);
		
		МассивСырья = СписокСырья.Получить(ЭлементКоллекции.ИдентификаторСтроки);
		Если МассивСырья <> Неопределено Тогда
			ЭлементДанных.Вставить("sourceList", МассивСырья);
		КонецЕсли;
		
		СтруктураХеша = СтруктураХешаПоДаннымТоваров(ЭлементДанных);
		
		ХешПоиска = ИнтеграцияИС.ХешированиеДанныхSHA256(ОбщегоНазначения.ЗначениеВJSON(СтруктураХеша));
		
		// по хешу может быть несколько различных строк товаров
		МассивЗначенийПоХешу = НомерСтрокПоИдентификаторамПартийТовары.Получить(ХешПоиска);
		
		Если МассивЗначенийПоХешу = Неопределено Тогда
			МассивЗначенийПоХешу = Новый Массив;
		КонецЕсли;
		
		МассивЗначенийПоХешу.Добавить(ЭлементКоллекции.НомерСтроки);
		НомерСтрокПоИдентификаторамПартийТовары.Вставить(ХешПоиска, МассивЗначенийПоХешу);
		
		СписокПродукции.Добавить(ЭлементДанных);
		
		ВсегоСтрокПродукции = ВсегоСтрокПродукции + 1;
		
	КонецЦикла;
	
	АбстрактноеСообщениеJSON.ПараметрыЗапроса.Вставить("НомерСтрокПоИдентификаторамПартийТовары", НомерСтрокПоИдентификаторамПартийТовары);
	АбстрактноеСообщениеJSON.ПараметрыЗапроса.Вставить("КоличествоСтрок",                         ВсегоСтрокПродукции);
	
	ИнтерфейсСАТУРН.ДобавитьАргументtheCard(АргументыОперации, "createdPatList", СписокПродукции);
...
Функция СтруктураХешаПоДаннымТоваров(ЭлементДанных)
	
	СтруктураХеша = Новый Структура();
	
	СтрокаРеквизитов = "createdPatId, clBatchCode, baseUnitType, puUnit, countPuUnit, itemMarkName,
		|producerArticleNum, expirationDate, patprductId, batchCodes, puKgWeight";
	ДанныеРеквизитов = Новый Структура(СтрокаРеквизитов);
	
	ЗаполнитьЗначенияСвойств(ДанныеРеквизитов, ЭлементДанных);
	
	СтруктураХеша.Вставить("ПАТ",                   ДанныеРеквизитов.createdPatId);
	СтруктураХеша.Вставить("НомерПартии",           ДанныеРеквизитов.clBatchCode);
	СтруктураХеша.Вставить("ТипИзмеряемойВеличины", ДанныеРеквизитов.baseUnitType);
	СтруктураХеша.Вставить("НаименованиеУпаковки",  ДанныеРеквизитов.puUnit);
	СтруктураХеша.Вставить("ТорговоеНаименование",  ДанныеРеквизитов.itemMarkName);
	СтруктураХеша.Вставить("АртикулПроизводителя",  ДанныеРеквизитов.producerArticleNum);
	СтруктураХеша.Вставить("СрокГодности",          ДанныеРеквизитов.expirationDate);
	
	Если Не ЗначениеЗаполнено(ДанныеРеквизитов.createdPatId) Тогда
		СтруктураХеша.ПАТ = Формат(ДанныеРеквизитов.patprductId, "ЧРГ=; ЧГ=;");
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ДанныеРеквизитов.clBatchCode) Тогда
		СтруктураХеша.НомерПартии = ДанныеРеквизитов.batchCodes;
	КонецЕсли;
	
	Если Не ТипЗнч(СтруктураХеша.СрокГодности) = Тип("Дата") Тогда
		СтруктураХеша.СрокГодности = ОбщегоНазначенияИСКлиентСервер.ДатаИзСтрокиUNIX(СтруктураХеша.СрокГодности, 1);
	КонецЕсли;
	
	Возврат СтруктураХеша;
