...
Функция ПараметрыВзаиморасчеты(ДанныеЗаполнения = Неопределено) Экспорт
	...
	ВлияющиеРеквизиты = "УдержатьВознаграждение,ХозяйственнаяОперация,СпособРасчетаВознаграждения";
...
Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|ГДЕ
	|	НЕ ТаблицаВидыЗапасов.ЭтоПрослеживаемаяРабота
	|	И (ТаблицаВидыЗапасов.СуммаВознаграждения + ТаблицаВидыЗапасов.СуммаНДСВознаграждения) <> 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И ТаблицаВидыЗапасов.СуммаНДСВознаграждения <> 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ВтРаботыУслуги КАК ТаблицаУслуги
	|ГДЕ
	|	(ТаблицаУслуги.СуммаВознаграждения + ТаблицаУслуги.СуммаНДСВознаграждения) <> 0
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки
	|";
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры)
	...
	ВзаиморасчетыПроведениеПоставщики.ПроведениеОплатыПоставщику(Запрос, ТекстыЗапроса, Регистры, ТекстОплата,"2");
...
Функция ТекстЗапросаТоварыПереданные() Экспорт
	...
	|		ТоварыПереданныеОстатки.ВидЗапасов КАК ВидЗапасов,
	...
	|		ТоварыПереданныеРегистратор.ВидЗапасов,
	|		ВЫБОР
	|			КОГДА ТоварыПереданныеРегистратор.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|				ТОГДА -ТоварыПереданныеРегистратор.Количество
	|			ИНАЧЕ ТоварыПереданныеРегистратор.Количество
	|		КОНЕЦ
	|	ИЗ
	...
	|		ИНАЧЕ ТоварыПереданныеОстатки.КоличествоОстаток - ЕСТЬNULL(Товары.Количество,0)
	|	КОНЕЦ КАК КоличествоОсталосьПодобрать,
	|	ЕСТЬNULL(Товары.Количество,0) КАК КоличествоПодобрано,
	|	ЕСТЬNULL(Товары.Количество, 0) КАК КоличествоВНакладной
	|
	|ИЗ
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	ЕСТЬNULL(Товары.НомерСтроки, 99999),
	|	Номенклатура,
	|	Характеристика,
	|	Серия,
	|	НомерГТД
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.НаименованиеПечатнойФормы = НСтр("ru = 'Отчет комиссионера о продажах'");
