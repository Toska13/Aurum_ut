...
// Создает, заполняет графики документа регулярными платежами и проводит его. Источником расчета являются реквизиты договора.
//
// Параметры:
//  ГрафикСсылка - ДокументСсылка.ГрафикИсполненияДоговора.
//  ДоговорСсылка - СправочникСсылка.ДоговорыКонтрагентов.
//
Процедура СоздатьЗаполнитьРегулярнымиПлатежами(ГрафикСсылка, ДоговорСсылка = Неопределено) Экспорт
	
	ДокументГрафик = ГрафикСсылка.ПолучитьОбъект();
	
	Если ДокументГрафик = Неопределено Тогда
		ДокументГрафик = Документы.ГрафикИсполненияДоговора.СоздатьДокумент();
		ДокументГрафик.УстановитьСсылкуНового(ГрафикСсылка);
		ДокументГрафик.Заполнить(Неопределено);
		ДокументГрафик.Дата = ТекущаяДатаСеанса();
		ДокументГрафик.Договор = ДоговорСсылка;
	КонецЕсли;
	
	РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументГрафик.Договор,
		"Дата, ДатаНачалаДействия, ДатаОкончанияДействия, ПериодичностьРегулярногоПлатежа, ДеньРегулярногоПлатежа,
		|СуммаРегулярногоПлатежа");
	
	ТекущаяДата = НачалоДня(ТекущаяДатаСеанса());
	ДатаНачалаПлатежей = Макс(ТекущаяДата, РеквизитыДоговора.Дата, РеквизитыДоговора.ДатаНачалаДействия);
	ДатаОкончанияПлатежей = ?(ЗначениеЗаполнено(РеквизитыДоговора.ДатаОкончанияДействия),
		РеквизитыДоговора.ДатаОкончанияДействия, ДобавитьМесяц(ДатаНачалаПлатежей, 12));
	
	ДатыРегулярныхПлатежей = Справочники.ДоговорыКонтрагентов.ДатыРегулярныхПлатежей(ДатаНачалаПлатежей,
		ДатаОкончанияПлатежей, РеквизитыДоговора.ПериодичностьРегулярногоПлатежа, РеквизитыДоговора.ДеньРегулярногоПлатежа);
	
	СтрокиКУдалению = Новый Массив;
		
	Для Каждого ДанныеСтроки Из ДокументГрафик.ЭтапыГрафикаОплаты Цикл
		Если ДанныеСтроки.ДатаПлатежа >= ТекущаяДата
			Или ДанныеСтроки.ДатаПлатежа <= РеквизитыДоговора.ДатаНачалаДействия Тогда
			СтрокиКУдалению.Добавить(ДанныеСтроки);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого УдаляемаяСтрока Из СтрокиКУдалению Цикл
		ДокументГрафик.ЭтапыГрафикаОплаты.Удалить(УдаляемаяСтрока);
	КонецЦикла;
	
	СтрокиКУдалению.Очистить();
	
	Для Каждого ДанныеСтроки Из ДокументГрафик.ЭтапыГрафикаИсполненияДоговора Цикл
		Если ДанныеСтроки.ДатаПоГрафику >= ТекущаяДата
			Или ДанныеСтроки.ДатаПоГрафику <= РеквизитыДоговора.ДатаНачалаДействия Тогда
			СтрокиКУдалению.Добавить(ДанныеСтроки);
		КонецЕсли;
	КонецЦикла;
		
	Для Каждого УдаляемаяСтрока Из СтрокиКУдалению Цикл
		ДокументГрафик.ЭтапыГрафикаИсполненияДоговора.Удалить(УдаляемаяСтрока);
	КонецЦикла;
		
	Для Каждого ДатаПлатежа Из ДатыРегулярныхПлатежей Цикл
		
		НоваяСтрока = ДокументГрафик.ЭтапыГрафикаОплаты.Добавить();
		НоваяСтрока.ДатаПлатежа = ДатаПлатежа;
		НоваяСтрока.СуммаПлатежа = РеквизитыДоговора.СуммаРегулярногоПлатежа;
		НоваяСтрока.ВариантОтсчета = Перечисления.ВариантыОтсчетаДатыПлатежа.ОтДатыОтгрузки;
		НоваяСтрока.ВариантОплаты = Перечисления.ВариантыКонтроляОплатыПоставщику.КредитПослеПоступления;
		
		НоваяСтрока = ДокументГрафик.ЭтапыГрафикаИсполненияДоговора.Добавить();
		НоваяСтрока.ДатаПоГрафику = ДатаПлатежа;
		НоваяСтрока.ДатаОтгрузки = ДатаПлатежа;
		НоваяСтрока.СуммаИсполнения = РеквизитыДоговора.СуммаРегулярногоПлатежа;
	КонецЦикла;
	
	ДокументГрафик.ЭтапыГрафикаОплаты.Сортировать("ДатаПлатежа");
	ДокументГрафик.ЭтапыГрафикаИсполненияДоговора.Сортировать("ДатаПоГрафику");
		
	ДокументГрафик.ДополнительныеСвойства.Вставить("ПропуститьОбновлениеГрафика", Истина);
	
	Если ДокументГрафик.ПроверитьЗаполнение() Тогда
		ДокументГрафик.Записать(РежимЗаписиДокумента.Проведение);
	КонецЕсли;
...
Процедура ПолучитьДанныеДокумента(Запрос, ДокументСсылка)
	...
	|	ГрафикИсполненияДоговораЭтапыГрафикаОплаты.ДатаОтгрузки                  КАК ДатаОтгрузки,
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры)
	...
	ВзаиморасчетыПроведениеКлиенты.ПроведениеРасчетыСКлиентамиПланОтгрузок(Запрос, ТекстыЗапроса, Регистры, ТекстПланыОтгрузок, Истина);
	...
	ВзаиморасчетыПроведениеПоставщики.ПроведениеРасчетыСПоставщикамиПланПоставок(Запрос, ТекстыЗапроса, Регистры, ТекстПланыПоставок, Истина);
