...
// Статус и дальнейшие действия оформление СДИЗ.
// 
// Параметры:
//  ДанныеДокумента - Структура - Данные документа:
//    * ТребуетсяВыделитьПартии - Булево - 
// 
// Возвращаемое значение:
//  Структура - Статус и дальнейшие действия оформление СДИЗ:
// * Статус - ПеречислениеСсылка.СтатусыОбработкиОформлениеСДИЗЗЕРНО - 
// * ДальнейшиеДействия - Массив Из ПеречислениеСсылка.ДальнейшиеДействияПоВзаимодействиюЗЕРНО- 
...
// 
// Возвращаемое значение:
// 	Массив из ПеречислениеСсылка.ДальнейшиеДействияПоВзаимодействиюЗЕРНО - дальнейшие действия.
//
...
// Получить данные для печатной формы расширенной этикетки СДИЗ.
// 
// Параметры:
//  ПараметрыПечати - См. Печать.ОбъектыПечати
//  МассивОбъектов - Массив Из ДокументСсылка.ОформлениеСДИЗЗЕРНО - Массив объектов
// 
// Возвращаемое значение:
//  РезультатЗапроса, Неопределено - Получить данные для печатной формы расширенной этикетки СДИЗ
Функция ПолучитьДанныеДляПечатнойФормыРасширеннойЭтикеткиСДИЗ(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ОформлениеСДИЗЗЕРНО.РазделУчета                                   КАК РазделУчета,
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ОбновлениеИнформационнойБазыГосИС.ДобавитьОбработчикОбновленияПереходаНаКонструкторНастроек(
		...
		Новый УникальныйИдентификатор("e9f21e9e-a205-408d-a76d-dbbc4e1495f9"));
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ОбновлениеИнформационнойБазыЗЕРНО.ЗарегистрироватьДанныеСсылочногоТипаПриПереходеНаКонстукторНастроекПоВидуПродукции(
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ОбъектМетаданных = ПустаяСсылка().Метаданные();
	ОбновлениеИнформационнойБазыЗЕРНО.ОбработатьДанныеДляПереходаНаНовуюВерсиюКонструктораНастроек(
		ОбъектМетаданных, Параметры);
...
Функция ЗаполнитьПартии(Объект, СписокСтрок = Неопределено, ЗаполнитьОстаткиПартии = Ложь, ПартииКорректировки = Ложь) Экспорт
	...
	|		ПО ТаблицаСоответствияПоКодам.РазделУчета = &РазделУчета
	...
	|		ПО КлючиПартийЗЕРНО.РазделУчета = &РазделУчета
	...
	Запрос.УстановитьПараметр("РазделУчета",         Объект.РазделУчета);
	...
	ПараметрыЗаполнения.Вставить("ЭтоППЗ",                 Объект.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна);
...
// * ОбъектРасчета - Неопределено, ДокументСсылка.ОформлениеСДИЗЗЕРНО, ДокументОбъект.ОформлениеСДИЗЗЕРНО, СправочникСсылка.СДИЗЗЕРНО, Массив из ДокументСсылка.ОформлениеСДИЗЗЕРНО, Массив из СправочникСсылка.СДИЗЗЕРНО - 
...
//  ТабличнаяЧастьТовары - Неопределено, ТаблицаЗначений, ДанныеФормыКоллекция - таблица для расчета. По умолчанию берется из табличной части ДокументОбъект.Товары
...
//
Процедура ЗаполнитьКорректировочныйСДИЗНаОснованииТаблицыКорректировки(ДокументОбъект, Запрос, Основание, ТаблицаКорректируемыйДокумент) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументОбъект.РазделУчета) Тогда
		Запрос.УстановитьПараметр("РазделУчета", ДокументОбъект.РазделУчета);
	Иначе
...
Процедура ОбработкаЗагрузкиПолученныхДанных(ЭлементОчереди, ПараметрыОбмена, ПолученныеДанные, ИзмененныеОбъекты, ПрочитаноВерсиейAPI) Экспорт
	...
	РазделУчета                 = ЭлементОчереди.РазделУчета;
	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийЗЕРНО.ФормированиеПартииИзДругихПартий Тогда
		...
		Попытка
			...
			ПараметрыЗагрузкиПартии.РазделУчета     = ЭлементОчереди.РазделУчета;
		...
		Попытка
			...
			Для Каждого СтрокаДанных Из ДанныеДляОбработки Цикл
				...
				Если ЭлементОчереди.Операция = Перечисления.ВидыОперацийЗЕРНО.ОформлениеСДИЗЭлеватор
					...
					ПараметрыОбработкиСообщений.РазделУчета   = ЭлементОчереди.РазделУчета;
	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийЗЕРНО.ЗапросПогашенийСДИЗ Тогда
		...
		ИмяПоляНомерПартии = ИнтеграцияЗЕРНОСлужебный.ИмяПоляНомерПартииСоздать(
			ЭлементОчереди.РазделУчета);
	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийЗЕРНО.АннулированиеСДИЗ
		...
		Если ВРег(ПолученныеДанные.resultOperation) = "DONE" Тогда
			...
			ИмяПоляНомерСДИЗ             = ИнтеграцияЗЕРНОСлужебный.ИмяПоляНомерСДИЗ(ЭлементОчереди.РазделУчета);
	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийЗЕРНО.ОформлениеСДИЗОтказПогашенияСДИЗ
		...
		Попытка
			...
			ИмяПоляНомерПартии = ИнтеграцияЗЕРНОСлужебный.ИмяПоляНомерПартииСоздать(ЭлементОчереди.РазделУчета);
			ИмяПоляПричина     = ИнтеграцияЗЕРНОСлужебный.ИмяПоляПричинаОтказаОтГашения(ЭлементОчереди.РазделУчета);
...
Функция ТекстыЗапросовЗаполненияКорректировочногоОформленияСДИЗПриВводеНаОснованииПрикладныхДокументов()
	...
	|	И (&РазделУчета = Неопределено Или ТаблицаДокументы.РазделУчета = &РазделУчета)
	...
	|	И (&РазделУчета = Неопределено Или ТаблицаКорректируемыеДокументы.РазделУчета = &РазделУчета)");
...
Функция СформироватьПартииЗЕРНО(ДокументСсылка, ДополнительныеПараметры)
	...
	|	Шапка.РазделУчета       КАК РазделУчета,
	|
	...
	ЭтоЗерно = РазделУчета = Перечисления.РазделыУчетаЗЕРНО.Зерно;
	Если ЭтоЗерно Тогда
	...
	АбстрактноеСообщениеXML.ПараметрыЗапроса.Вставить("РазделУчета", РазделУчета);
	...
	Для Каждого СтрокаПартииСгруппированная Из ПартииСгруппированные Цикл
		...
		Для Каждого СтрокаПартии Из СтрокиДокумента Цикл
			...
			СообщениеXML.ПараметрыЗапроса.Вставить("РазделУчета", РазделУчета);
...
Функция АннулированиеСДИЗЗЕРНО(ДокументСсылка, ДополнительныеПараметры)
	...
	ИмяПоляНомерСДИЗ   = ИнтеграцияЗЕРНОСлужебный.ИмяПоляНомерСДИЗ(РазделУчета);
	ИмяПоляНомерПартии = ИнтеграцияЗЕРНОСлужебный.ИмяПоляНомерПартии(РазделУчета);
...
//	|	ДанныеКорректировки.ИдентификаторСтрокиТовары КАК ИдентификаторСтрокиТовары,
	...
	Если Шапка.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.Зерно Тогда
		ПространствоИмен   = ИнтеграцияЗЕРНОСлужебный.ПространствоИменПоИмениПакетаXDTO("zerno_api_sdiz", ПараметрыОбмена.ПараметрыОптимизации);
	...
	ИмяПоляНомерСДИЗ = ИнтеграцияЗЕРНОСлужебный.ИмяПоляНомерСДИЗ(РазделУчета);
	ИмяПоляПричина   = ИнтеграцияЗЕРНОСлужебный.ИмяПоляПричинаОтказаОтГашения(РазделУчета);
...
Функция КорректировкаСДИЗЗЕРНО(ДокументСсылка, ДополнительныеПараметры)
	...
	ИмяПоляИдентификаторЗаявки = ИнтеграцияЗЕРНОСлужебный.ИмяПоляИдентификаторОтказаПогашения(РазделУчета);
	ИмяПоляНомерСДИЗ           = ИнтеграцияЗЕРНОСлужебный.ИмяПоляНомерСДИЗ(РазделУчета);
	ИмяПоляПричина             = ИнтеграцияЗЕРНОСлужебный.ИмяПоляПричинаОтказаОтГашения(РазделУчета);
	ИмяПоляНомерПартии         = ИнтеграцияЗЕРНОСлужебный.ИмяПоляНомерПартии(РазделУчета);
	ИмяПоляКорректируемыйСДИЗ  = ИнтеграцияЗЕРНОСлужебный.ИмяПоляКорректируемыйСДИЗ(РазделУчета);
...
Функция СообщениеАннулированияПартии(ЭлементОчереди, ПараметрыЗапроса, ПараметрыОбмена)
	...
	СообщениеXML.РазделУчета      = ЭлементОчереди.РазделУчета;
	...
	Если РазделУчета = Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна Тогда
		ПространствоИмен   = ИнтеграцияЗЕРНОСлужебный.ПространствоИменПоИмениПакетаXDTO("zerno_api_gpb", ПараметрыОбмена.ПараметрыОптимизации);
...
Функция ОформлениеСДИЗРФ(Шапка, РезультатЗапроса, ДополнительныеПараметры)
	...
	ЭтоЗерно = Шапка.РазделУчета = Перечисления.РазделыУчетаЗЕРНО.Зерно;
	Если ЭтоЗерно Тогда
		ПространствоИмен   = ИнтеграцияЗЕРНОСлужебный.ПространствоИменПоИмениПакетаXDTO("zerno_api_sdiz", ПараметрыОбмена.ПараметрыОптимизации);
	...
	ИмяПоляКорректируемыйСДИЗ = ИнтеграцияЗЕРНОСлужебный.ИмяПоляКорректируемыйСДИЗ(Шапка.РазделУчета);
	ИмяПоляНомерПартии        = ИнтеграцияЗЕРНОСлужебный.ИмяПоляНомерПартии(Шапка.РазделУчета);
...
Функция ОформлениеСДИЗИмпортЭкспортXDTO(Шапка, ПараметрыЗаполнения, АбстрактноеСообщениеXML, ПараметрыОбмена)
	...
	Если СвязанныеДокументыПрочие.Количество() Тогда
