...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьСписок(Элементы.ОтборСостояние.СписокВыбора,
		Перечисления.СостоянияЗаказовКлиентов.СписокДоступныхСостоянийРаспоряжений());
...
Процедура УстановитьОтборПоСостояниюСервер()
	...
	ИначеЕсли Состояние = "ВсеОжидающиеИсполнения" Тогда
		...
		МассивСостояний.Добавить(Перечисления.СостоянияЗаказовКлиентов.ОжидаетсяПереходПраваСобственности);
