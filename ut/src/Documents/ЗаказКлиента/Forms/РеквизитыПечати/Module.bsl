...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, Параметры,,"ЗакрыватьПриВыборе,ЗакрыватьПриЗакрытииВладельца,КлючНазначенияИспользования");
	
	ТребуетсяДоставка = Параметры.ТребуетсяДоставка;
	СпособУказанияДоставки = Параметры.СпособУказанияДоставки;
	Если ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25() Тогда
		
		Если ТребуетсяДоставка
			И СпособУказанияДоставки = Перечисления.СпособыУказанияДоставки.НаВесьДокумент Тогда
			
			ТекстПодсказки = НСтр("ru = 'Адрес указан по точке доставки'");
			
			Элементы.АдресДоставки.ТолькоПросмотр = Истина;
			Элементы.АдресДоставки.Подсказка = ТекстПодсказки;
			Элементы.АдресДоставки.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСнизу;
			
		Иначе
			
			ДоставкаТоваров.ЗаполнитьСпискиВыбораАдресовПолучателяОтправителя(Элементы, ДанныеРаспоряженияНаДоставку, Истина);
...
Процедура АдресДоставкиПриИзменении(Элемент)
	...
	Если ПерезаполнятьАдресДоставки
		Или ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25() Тогда
...
Процедура ГрузополучательПриИзмененииСервер()
	...
	Если ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25() Тогда
		...
		Если Не (СпособУказанияДоставки = Перечисления.СпособыУказанияДоставки.НаВесьДокумент
					И ТребуетсяДоставка) Тогда
			
			ДоставкаТоваров.ЗаполнитьСпискиВыбораАдресовПолучателяОтправителя(Элементы, ЭтотОбъект, Истина);
			Если Элементы.АдресДоставки.СписокВыбора.Количество() > 0 Тогда
				
				ПерваяСтруктураВСписке = Элементы.АдресДоставки.СписокВыбора[0].Значение;
				АдресДоставки = ПерваяСтруктураВСписке.АдресДоставки;
				АдресДоставкиЗначение = ПерваяСтруктураВСписке.АдресДоставкиЗначение;
				АдресДоставкиЗначенияПолей = ПерваяСтруктураВСписке.АдресДоставкиЗначенияПолей;
