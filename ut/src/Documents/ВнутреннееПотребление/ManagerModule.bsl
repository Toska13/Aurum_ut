...
Функция ПараметрыЗаполненияДокумента() Экспорт
	...
	ПараметрыЗаполнения.Вставить("НаправлениеДеятельностиРасходовУказаноВСписке", Ложь);
...
Процедура ИнициализироватьПараметрыЗаполнения(ПараметрыЗаполнения, РеквизитыШапки, МассивЗаказов) Экспорт
	...
	ПараметрыЗаполнения.НаправлениеДеятельностиРасходовУказаноВСписке = ИнцициализироватьНаправленияДеятельностиРасходов(МассивЗаказов);
	
КонецПроцедуры

// Возвращает максимальное значение реквизита направление деятельности расходов по заказам.
//
// Параметры:
//  МассивЗаказов - массив из ДокументСсылка.ЗаказНаВнутреннееПотребление, ДокументСсылка.ЗаказНаРемонт - Ссылки на заказы.
// Возвращаемое значение:
//   Булево - значение направления деятельности расходов.
//
Функция ИнцициализироватьНаправленияДеятельностиРасходов(МассивЗаказов) Экспорт
	
	НаправлениеДеятельностиРасходовУказаноВСписке = Ложь;
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказНаВнутреннееПотребление) Тогда
		...
		Текст =
			...
			|	ЗаказНаВнутреннееПотребление.НаправлениеДеятельностиРасходовУказаноВСписке КАК НаправлениеДеятельностиРасходовУказаноВСписке
	...
	ТекстЗапроса =  ТекстЗапроса + 
		"ВЫБРАТЬ
		|	ЕСТЬNULL(МАКСИМУМ(ВТЗаказыОтбор.НаправлениеДеятельностиРасходовУказаноВСписке), ЛОЖЬ) КАК НДРасходовНакладная
		|ИЗ
		|	ВТЗаказыОтбор КАК ВТЗаказыОтбор";
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Если Выборка.Следующий() Тогда
			НаправлениеДеятельностиРасходовУказаноВСписке = Выборка.НДРасходовНакладная;
		КонецЕсли;
	КонецЕсли;
	
	Возврат НаправлениеДеятельностиРасходовУказаноВСписке;
...
Функция РезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, КонтекстВызова) Экспорт
	...
	ТекстыЗапросов.Вставить("ВтНДРасходовУказаноВСписке",    ТекстЗапросаВременнойТаблицыНДРасходов());
...
Функция ДанныеЗаполненияНакладной(МассивСсылок, СвойстваЗаказов = Неопределено) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказНаВнутреннееПотребление) Тогда
		...
		Текст =
			...
			|	Заказ.НаправлениеДеятельностиРасходовУказаноВСписке КАК НаправлениеДеятельностиРасходовУказаноВСписке
			|ИЗ
			...
			|	Заказ.НаправлениеДеятельностиРасходовУказаноВСписке
			|";
		ТекстыЗапроса.Добавить(Текст);
	...
	РеквизитыШапки = Новый Структура("ХозяйственнаяОперация, Организация, Склад, Сделка, Подразделение,
		|ПотреблениеДляДеятельности, НаправлениеДеятельности, НаправлениеДеятельностиРасходовУказаноВСписке");
...
Процедура ЗаполнитьШапкуДокументаПоЗаказу(Объект, ПараметрыЗаполнения, МассивЗаказов) Экспорт
	...
	Объект.НаправлениеДеятельностиРасходовУказаноВСписке = ПараметрыЗаполнения.НаправлениеДеятельностиРасходовУказаноВСписке;
...
Процедура ЗаполнитьПоЗаказамОрдерам(Товары, Регистратор, ПараметрыЗаполнения, ШтрихкодыУпаковок) Экспорт
	...
	Запрос.УстановитьПараметр("НаправлениеДеятельностиРасходовУказаноВСписке", ПараметрыЗаполнения.НаправлениеДеятельностиРасходовУказаноВСписке);
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Запрос.Текст =
		...
		|	Таблица.НаправлениеДеятельности        КАК НаправлениеДеятельности,
		|	Таблица.НаправлениеДеятельностиРасходовУказаноВСписке КАК НаправлениеДеятельностиРасходовУказаноВСписке,
		...
		|	Таблица.ИсправляемыйДокумент.Дата      КАК ПериодКорректировки
		|ИЗ
	...
	Запрос.УстановитьПараметр("НаправлениеДеятельностиРасходовУказаноВСписке", Реквизиты.НаправлениеДеятельностиРасходовУказаноВСписке);
...
Функция ТекстЗапросаВтВидыЗапасов(Запрос, ТекстыЗапроса) Экспорт
	...
	ТекстЗапроса = "
		...
		|	ВЫБОР
		|		КОГДА &НаправлениеДеятельностиРасходовУказаноВСписке
		...
		|	ВЫБОР
		|		КОГДА &НаправлениеДеятельностиРасходовУказаноВСписке
		|			ТОГДА ТаблицаТовары.НаправлениеДеятельности
		|		ИНАЧЕ &НаправлениеДеятельности
		|	КОНЕЦ                                           КАК НаправлениеДеятельности,
		|	КлючиПроизводства.КлючАналитики                 КАК АналитикаПроизводства,
		|	Аналитика.СкладскаяТерритория.ЦеховаяКладовая   КАК ЦеховаяКладовая,
...
Функция ТекстЗапросаТаблицаВтИсходныеПрочиеРасходы(Запрос, ТекстыЗапроса)
	...
	|	И НЕ &ИспользоватьПодакцизныеТовары
	|";
...
Процедура ПараметрыЗапросаДляСозданияНаОсновании(ПараметрыЗапросов, ИменаЗапросов, ИменаПодзапросов, ДанныеОтбора)
	...
	ПараметрыЗапросов.Вставить("НаправлениеДеятельностиРасходовУказаноВСписке", ДанныеОтбора.НаправлениеДеятельностиРасходовУказаноВСписке);
	
	Если ДанныеОтбора.ЗаполнятьПоОрдеру Тогда
		
		ИменаЗапросов.Добавить("ВтЗаказыОтбор");
		ИменаЗапросов.Добавить("ВтНДРасходовУказаноВСписке");
...
Функция ТекстЗапросаВременнойТаблицыНакладной()
	...
	ТекстЗапроса =
		...
		|	Таблица.ГруппаПродукции       КАК ГруппаПродукции,
		|	Таблица.НаправлениеДеятельности КАК НаправлениеДеятельности,
		|	&НаправлениеДеятельностиРасходовУказаноВСписке КАК НаправлениеДеятельностиРасходовУказаноВСписке, 
		|
		|	Таблица.КодСтроки             КАК КодСтроки,
		|	Таблица.Упаковка              КАК Упаковка,
		|
...
Функция ТекстЗапросаВременнойТаблицыНДРасходов()
	
	ТекстЗапроса = 
		"ВЫБРАТЬ
		|	ЕСТЬNULL(МАКСИМУМ(ВТЗаказыОтбор.НаправлениеДеятельностиРасходовУказаноВСписке), ЛОЖЬ) КАК НДРасходовНакладная
		|ПОМЕСТИТЬ ВтНДРасходовУказаноВСписке
		|ИЗ
		|	ВТЗаказыОтбор КАК ВТЗаказыОтбор";
...
Функция ТекстЗапросаОбъединения(Имена, Тексты)
	...
	ТекстЗапроса =
		...
		|	МАКСИМУМ(Набор.НаправлениеДеятельности) КАК НаправлениеДеятельности,
		|	МАКСИМУМ(Набор.НаправлениеДеятельностиРасходовУказаноВСписке) КАК НаправлениеДеятельностиРасходовУказаноВСписке,
...
Функция ТекстЗапросаОстаткиРегистраЗаказы()
	...
	ТекстЗапроса =
		...
		|		ВЫБОР 
		|			КОГДА НДРасходов.НДРасходовНакладная ТОГДА
		|				ВЫБОР 
		|					КОГДА Таблица.Ссылка.НаправлениеДеятельностиРасходовУказаноВСписке ТОГДА
		|						Таблица.НаправлениеДеятельности
		|					ИНАЧЕ
		|						Таблица.Ссылка.НаправлениеДеятельности
		|				КОНЕЦ
		|			ИНАЧЕ
		...
		|				ИСТИНА
		|			ИНАЧЕ
		|				Таблица.Ссылка.НаправлениеДеятельностиРасходовУказаноВСписке
		|		КОНЕЦ КАК НаправлениеДеятельностиРасходовУказаноВСписке,
		|		Таблица.СтатьяРасходов            КАК СтатьяРасходов,
		|		Таблица.АналитикаРасходов         КАК АналитикаРасходов,
		|		Таблица.ГруппаПродукции           КАК ГруппаПродукции,
		...
		|			ЛЕВОЕ СОЕДИНЕНИЕ ВтНДРасходовУказаноВСписке КАК НДРасходов
		|			ПО ИСТИНА";
