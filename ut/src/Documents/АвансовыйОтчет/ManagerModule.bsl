...
Функция ТекстЗапросаДанныеАвансовыхОтчетов(Запрос, ИмяВт = "") Экспорт
	...
	РегистраторыДенежныеСредстваВыдано.Добавить(Тип("ДокументСсылка.ОперацияСЭлектроннымБилетом"));
	...
	Иначе
		...
		ТекстЗапроса = "
			...
			|		КОГДА ДанныеРегистра.Регистратор ССЫЛКА Документ.ОперацияСЭлектроннымБилетом
			|			ТОГДА ВЫРАЗИТЬ(ДанныеРегистра.Регистратор КАК Документ.ОперацияСЭлектроннымБилетом).АвансовыйОтчет
			//++ Локализация
			...
			|		КОГДА ДанныеРегистра.Регистратор ССЫЛКА Документ.ОперацияСЭлектроннымБилетом
			|			ТОГДА ВЫРАЗИТЬ(ДанныеРегистра.Регистратор КАК Документ.ОперацияСЭлектроннымБилетом).АвансовыйОтчет.Дата
			//++ Локализация
			...
			|			ИЛИ (ВЫРАЗИТЬ(ДанныеРегистра.Регистратор КАК Документ.ОперацияСЭлектроннымБилетом).АвансовыйОтчет В (&МассивДокументов)
			|				И ВЫРАЗИТЬ(ДанныеРегистра.Регистратор КАК Документ.ОперацияСЭлектроннымБилетом).ПодотчетноеЛицо = ДанныеРегистра.ПодотчетноеЛицо)
			//++ Локализация
	...
	|	ИЗ
	|		РегистрНакопления.ДенежныеСредстваУПодотчетныхЛиц.Обороты(,, Регистратор) КАК ДенежныеСредства
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОперацияСЭлектроннымБилетом КАК ДанныеДокумента
	|				ПО ДанныеДокумента.Ссылка = ДенежныеСредства.Регистратор
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|	ИЗ
	|		Документ.ОперацияСЭлектроннымБилетом КАК ДанныеДокумента
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОперацияСЭлектроннымБилетом КАК ОперацииБилета
	|				ПО ДанныеДокумента.Ссылка = ОперацииБилета.ЭлектронныйБилет
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ДенежныеСредстваУПодотчетныхЛиц.Обороты(,, Регистратор) КАК ДенежныеСредства
	|				ПО ОперацииБилета.Ссылка = ДенежныеСредства.Регистратор
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|				АвансовыйОтчетНаПечатьВТ3 КАК АвансовыйОтчетНаПечатьВТ3
	|			ПО
	|				ДенежныеСредства.Организация = АвансовыйОтчетНаПечатьВТ3.Организация
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ДанныеДокумента.ДокументОснование.Дата	КАК ПериодКорректировки,
...
Функция ТекстЗапросаТаблицаВтИсходныеПартииПрочихРасходов(Запрос, ТекстыЗапроса)
	...
	|	ВЫРАЗИТЬ(ПрочиеРасходы.СуммаНДС * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр КАК ЧИСЛО(31,2)) КАК НДСУпр,
	|	ВЫРАЗИТЬ(ПрочиеРасходы.СуммаСНДС * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл КАК ЧИСЛО(31,2))
	|		- ВЫРАЗИТЬ(ПрочиеРасходы.СуммаНДС * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл КАК ЧИСЛО(31,2)) КАК СтоимостьРегл,
	|	0 КАК ПостояннаяРазница,
	|	0 КАК ВременнаяРазница,
	|	ВЫРАЗИТЬ(ПрочиеРасходы.СуммаНДС * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл КАК ЧИСЛО(31,2)) КАК НДСРегл,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПрочиеРасходы) КАК ХозяйственнаяОперация,
	|	
	|	ПрочиеРасходы.ИдентификаторСтроки КАК ИдентификаторФинЗаписи,
	|	ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ПрочиеРасходыПодотчетногоЛица) КАК НастройкаХозяйственнойОперации
...
Функция ТекстЗапросаВременнаяТаблицаИзрасходованоВСвязанныхДокументах(Запрос, ТекстыЗапроса)
	...
	|	ВЫБРАТЬ
	|		ДанныеДокумента.Ссылка,
	|		СУММА(ВЫБОР КОГДА ОперацияСЭлектроннымБилетом.ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыОперацийСБилетами.Покупка)
	|					ИЛИ ОперацияСЭлектроннымБилетом.ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыОперацийСБилетами.Доплата)
	|					ТОГДА ОперацияСЭлектроннымБилетом.Сумма
	|				ИНАЧЕ -ОперацияСЭлектроннымБилетом.Сумма
	|			КОНЕЦ) КАК Сумма
	|	ИЗ
	|		Документ.АвансовыйОтчет КАК ДанныеДокумента
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОперацияСЭлектроннымБилетом.Операции КАК ОперацияСЭлектроннымБилетом
	|				ПО ДанныеДокумента.Ссылка = ОперацияСЭлектроннымБилетом.Ссылка.АвансовыйОтчет
	|					И ОперацияСЭлектроннымБилетом.Ссылка.Проведен
	|					И ОперацияСЭлектроннымБилетом.Ссылка.ЭлектронныйБилет = Значение(Документ.ОперацияСЭлектроннымБилетом.ПустаяСсылка)
	|	ГДЕ
	|		ДанныеДокумента.Ссылка = &Ссылка
	|		И НЕ ДанныеДокумента.Мультивалютный
	|	СГРУППИРОВАТЬ ПО
	...
	|	ВЫБРАТЬ
	|		ДанныеДокумента.Ссылка,
	|		СУММА(ВЫБОР КОГДА ОперацииБилета.ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыОперацийСБилетами.Возврат)
	|					ТОГДА -ОперацииБилета.СуммаДокумента
	|				ИНАЧЕ ОперацииБилета.СуммаДокумента
	|			КОНЕЦ) КАК Сумма
	|	ИЗ
	|		Документ.АвансовыйОтчет КАК ДанныеДокумента
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОперацияСЭлектроннымБилетом КАК ДанныеЭлектронногоБилета
	|				ПО ДанныеДокумента.Ссылка = ДанныеЭлектронногоБилета.АвансовыйОтчет
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОперацияСЭлектроннымБилетом КАК ОперацииБилета
	|				ПО ДанныеЭлектронногоБилета.Ссылка = ОперацииБилета.ЭлектронныйБилет
	|				И ОперацииБилета.Проведен
	|	ГДЕ
	|		ДанныеДокумента.Ссылка = &Ссылка
	|		И НЕ ДанныеДокумента.Мультивалютный
	|	СГРУППИРОВАТЬ ПО
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР КОГДА Расходы.БланкСтрогойОтчетности И Расходы.ЭлектронныйБилет <> ЗНАЧЕНИЕ(Документ.ОперацияСЭлектроннымБилетом.ПустаяСсылка) ТОГДА
	...
	|		ИЛИ &СтатьяРасходовСписаниеНДС = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка) ТОГДА
	...
	|	ВЫБОР КОГДА &СтатьяРасходовСписаниеНДС = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка) ТОГДА
	|		Расходы.СтатьяРасходов
	|	ИНАЧЕ
	|		&СтатьяРасходовСписаниеНДС
	|	КОНЕЦ КАК СтатьяРасходовСписаниеНДС,
	|	ВЫБОР КОГДА &СтатьяРасходовСписаниеНДС = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка) ТОГДА
	|		Расходы.АналитикаРасходов
	|	ИНАЧЕ
	|		&АналитикаРасходовСписаниеНДС
	|	КОНЕЦ КАК АналитикаРасходовСписаниеНДС,
	|	Расходы.ИдентификаторСтроки                  КАК ИдентификаторФинЗаписи,
	|	ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.СписаниеНДСПоПриобретеннымЦенностям) КАК НастройкаХозяйственнойОперации
	|ИЗ
	|	Документ.АвансовыйОтчет.ПрочиеРасходы КАК Расходы
