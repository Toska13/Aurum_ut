...
Функция ТекстЗапросаТаблицаЖурналУчетаСчетовФактур(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР
	|		КОГДА &СоставленКомиссионеромОтИмениПродавца
	|			ТОГДА &Контрагент
	|		ИНАЧЕ NULL
	...
	|	ВЫБОР
	|		КОГДА &СоставленКомиссионеромОтИмениПродавца
	|			ТОГДА &ИННКонтрагента 
	|		ИНАЧЕ NULL
	|	КОНЕЦ КАК ИННСубкомиссионера,
	|	ВЫБОР
	|		КОГДА &СоставленКомиссионеромОтИмениПродавца
	|			ТОГДА &КППКонтрагента 
	|		ИНАЧЕ NULL
	...
	|	&Контрагент КАК Субкомиссионер,
	|	NULL КАК НомерСчетаФактурыПродавца,
	|	NULL КАК ДатаСчетаФактурыПродавца,
	|	&ИННКонтрагента КАК ИННСубкомиссионера,
	|	&КППКонтрагента КАК КППСубкомиссионера,
	|	ЛОЖЬ КАК Сторно,
	|	NULL КАК ИсправленныйСчетФактура,
	|	NULL КАК СчетФактураПолученныйОтПродавца,
	|	NULL КАК ИсправлениеСобственнойОшибки
...
Функция ПолучитьСуммыДокументовОснований(ПараметрыЗаполнения) Экспорт
	...
	Если ТаблицаДокументовОснований.Количество() > 0 
		...
		|			И (СФ.НомерСтроки = 1)
		|			И НЕ СФ.Ссылка.Исправление
		|			И НЕ СФ.Ссылка.Корректировочный
		|
		|СГРУППИРОВАТЬ ПО
	...
	Иначе
		...
		|	ЕСТЬNULL(ДокументыОснованияБезУчетаСтавкиНДС.Ссылка, ЗНАЧЕНИЕ(Документ.СчетФактураПолученный.ПустаяСсылка)) КАК ИсходныйДокументБезУчетаСтавкиНДС
		|ПОМЕСТИТЬ ИсходныеСчетаФактуры
		|ИЗ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураПолученный.ДокументыОснования КАК ДокументыОснованияБезУчетаСтавкиНДС
		|		ПО ТаблицаСумм.ДокументПриобретения = ДокументыОснованияБезУчетаСтавкиНДС.ДокументОснование
		|			И (ДокументыОснованияБезУчетаСтавкиНДС.Ссылка.Проведен)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|		ЕСТЬNULL(ИсходныеСчетаФактуры.ИсходныйДокумент, ЗНАЧЕНИЕ(Документ.СчетФактураПолученный.ПустаяСсылка)) КАК ИсходныйДокумент,
		|		ТаблицаСумм.СтавкаНДС КАК СтавкаНДС,
		|		ВЫБОР
		...
		|			ЛЕВОЕ СОЕДИНЕНИЕ ИсходныеСчетаФактуры КАК ИсходныеСчетаФактуры
		|			ПО ТаблицаСумм.ДокументПриобретения = ИсходныеСчетаФактуры.ДокументПриобретения
		|				И ТаблицаСумм.СтавкаНДС = ИсходныеСчетаФактуры.СтавкаНДС
		|	ГДЕ
		|		&Корректировочный
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		...
		|	ВЫБРАТЬ
		|		ТаблицаСумм.ДокументОснование КАК ДокументОснование,
		|		ЕСТЬNULL(ИсходныеСчетаФактуры.ИсходныйДокументБезУчетаСтавкиНДС, ЗНАЧЕНИЕ(Документ.СчетФактураПолученный.ПустаяСсылка)) КАК ИсходныйДокумент,
		|		ТаблицаСумм.СтавкаНДС КАК СтавкаНДС,
		|		ВЫБОР
		|			КОГДА &ВалютаСчетаФактуры = ТаблицаСумм.Валюта
...
Процедура ПоместитьТаблицуСуммТорговыхДокументов(МассивТорговыхДокументов, МенеджерВременныхТаблиц, ИмяВременнойТаблицы)
	...
	Для Каждого Основание Из МассивТорговыхДокументов Цикл
		...
		И ТипЗнч(Основание) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
			ХозяйственнаяОперацияОснования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Основание,
				"ХозяйственнаяОперация");
			Если ХозяйственнаяОперацияОснования = Перечисления.ХозяйственныеОперации.ЗакупкаПоИмпорту
				ИЛИ ХозяйственнаяОперацияОснования = Перечисления.ХозяйственныеОперации.ПриемНаКомиссию Тогда
				УстановитьПривилегированныйРежим(Истина);
				ТаблицыДляДвижений = ПроведениеДокументов.ДанныеДокументаДляПроведения(Основание, );
				УстановитьПривилегированныйРежим(Ложь);
			Иначе
				УстановитьПривилегированныйРежим(Истина);
				ТаблицыДляДвижений = ПроведениеДокументов.ДанныеДокументаДляПроведения(Основание,
					"ЗаполнениеСуммСФПолученный");
				УстановитьПривилегированныйРежим(Ложь);
				ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицыДляДвижений.ТаблицаЗаполнениеСуммСФПолученный,
					ТаблицаСумм);
			КонецЕсли;
