...
Функция СтатусПослеПодготовкиКПередачеДанных(ДокументСсылка, Операция, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Операция = Перечисления.ВидыОперацийИСМП.ЗаказНаЭмиссиюКодовМаркировки
	 Или Операция = Перечисления.ВидыОперацийИСМП.ЗаказНаЭмиссиюКодовМаркировкиЗапросGTINНаОстатки Тогда
		...
		ТоварнаяГруппа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ТоварнаяГруппа");
	...
	Если Операция = Перечисления.ВидыОперацийИСМП.ПолучениеРезультатаОбработкиЗаказаНаЭмиссиюКодовМаркировки
		...
		Если ЗначениеЗаполнено(ДополнительныеПараметры.Статус) Тогда
			...
			Если ДополнительныеПараметры.ОперацияКвитанции = Перечисления.ВидыОперацийИСМП.ЗаказНаЭмиссиюКодовМаркировкиЗапросGTINНаОстатки Тогда
				...
				ДанныеОбъекта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
					ДокументСсылка, "СпособВводаВОборот,Товары");
				ДанныеОбъекта.Вставить("Товары", ДанныеОбъекта.Товары.Выгрузить());
				Если ТребуетсяЗапросGTIN(ДанныеОбъекта) Тогда
			...
			Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ВидыПродукцииПоТоварнойГруппе) Тогда
				Статусы.ПринятДействия.Добавить(
		...
		Иначе
			...
			Иначе
				...
				Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ВидыПродукцииПоТоварнойГруппе) Тогда
					ДанныеСтатусаПоПулу = ДанныеРасчетаСтатусаЗаказаПоПулу(ДокументСсылка, ВидыПродукцииПоТоварнойГруппе);
...
Функция ВсеТребующиеДействия() Экспорт
	...
	ВестиУчетМаркируемойПродукцииМЕХ = Ложь;
	Для Каждого ВидПродукции Из КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииМЕХ() Цикл
		Если ИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции(ВидПродукции) Тогда
			ВестиУчетМаркируемойПродукцииМЕХ = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Если ВестиУчетМаркируемойПродукцииМЕХ Тогда
...
Функция ОбработатьДанныеШтрихкода(Форма, ДанныеШтрихкода, ПараметрыСканирования, ВложенныеШтрихкоды = Неопределено) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Если Результат.ДобавленныеСтроки.Количество() > 0 Тогда
...
// Требуется запрос GTIN.
// 
// Параметры:
//  Объект - ДокументОбъект.ЗаказНаЭмиссиюКодовМаркировкиСУЗ, ДокументСсылка, ДанныеФормыСтруктураСКоллекцией - Объект
// 
// Возвращаемое значение:
//  Булево - Требуется запрос GTIN
...
Процедура ПриЗаполненииОграниченияДоступа(Списки) Экспорт
	...
	УправлениеДоступомИСПереопределяемый.ПриЗаполненииОграниченияДоступа(
		Метаданные.Документы.ЗаказНаЭмиссиюКодовМаркировкиСУЗ, Списки);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ОбновлениеИнформационнойБазыГосИС.ДобавитьОбработчикОбновленияПереходаНаКонструкторНастроек(
		...
		Новый УникальныйИдентификатор("e701817e-8354-446c-a1e7-475a18e54bfc"));
...
Функция ЗаявкаНаЭмиссиюКодовМаркировкиJSON(ДокументСсылка, ДополнительныеПараметры) Экспорт
	...
	|	ШаблоныКодовМаркировкиСУЗ.Код      КАК ШаблонКод,
	...
	|		ПО ТЧТовары.Шаблон = ШаблоныКодовМаркировкиСУЗ.Ссылка
	|ГДЕ
	|	Товары.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ТЧТовары.Шаблон,
	|	ШаблоныКодовМаркировкиСУЗ.Код,
	|	ШаблоныКодовМаркировкиСУЗ.ТипКодаМаркировки,
	...
	ТоварнаяГруппа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ТоварнаяГруппа");
	ВидыПродукцииПоТоварнойГруппе = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ТоварнаяГруппа);
	ЭтоТабачнаяПродукция = КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(ВидыПродукцииПоТоварнойГруппе);
	ЭтоМех = КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ВидыПродукцииПоТоварнойГруппе);
	
	Запрос.УстановитьПараметр("ЭтоТабачнаяПродукция", ЭтоТабачнаяПродукция);
	Запрос.УстановитьПараметр("ЭтоМех", ЭтоМех);
	...
	Если Не ЗаказНаЭмиссиюСУЗКлиентСервер.ПоддерживаетсяФорматОбменаСУЗ(ФорматОбмена, ВидыПродукцииПоТоварнойГруппе) Тогда
	...
	ИспользуетсяВладелецGTIN = ЗаказНаЭмиссиюСУЗКлиентСервер.ДоступноПолеВладелецGTIN(
		Шапка.ТоварнаяГруппа, Шапка.СпособВводаВОборот);
	...
	Если ЭтоТабачнаяПродукция Тогда
		...
		ДоступноУказаниеМРЦ = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДоступноПолеМРЦ(ВидыПродукцииПоТоварнойГруппе);
		ШаблоныГрупповыхУпаковок = ИнтеграцияИСМПКлиентСервер.ШаблоныКодовПоВидуПродукции(Шапка.ТоварнаяГруппа, 4);
		...
		АбстрактноеСообщениеJSON.ТоварнаяГруппа            = Шапка.ТоварнаяГруппа;
		...
		Для Каждого СтрокаТЧ Из ТабличнаяЧастьТовары Цикл
			...
			СтрокаЗаказаКодовМаркировки.Вставить("templateId", СтрокаТЧ.ШаблонКод);
			...
			Если ДоступноУказаниеМРЦ Тогда
				Если ЗначениеЗаполнено(СтрокаТЧ.МРЦ) Тогда
					СтрокаМРЦ = ИнтерфейсМОТП.ЗначениеМРЦСтрокой(СтрокаТЧ.МРЦ);
				ИначеЕсли ШаблоныГрупповыхУпаковок.НайтиПоЗначению(СтрокаТЧ.Шаблон) <> Неопределено Тогда
					СтрокаМРЦ = "000000";
				Иначе
					СтрокаМРЦ = "0000";
				КонецЕсли;
			...
			Если ЗаказНаЭмиссиюСУЗКлиентСервер.ПередаватьСпособИзготовленияКодовМаркировкиВСУЗ(ВидыПродукцииПоТоварнойГруппе) Тогда
			...
			СтрокаЗаказаКодовМаркировки.Вставить("cisType", ИнтерфейсСУЗСлужебный.ТипКодаМаркировкиСтрокой(СтрокаТЧ.ТипКодаМаркировки));
			...
			ИнтерфейсСУЗ.ТелоЗапросаПоФорматуОбмена(ТелоЗапроса, ФорматОбмена, Операция, Шапка.ТоварнаяГруппа);
			...
			СообщениеJSON.ТребуетсяПодписание =
				ЗаказНаЭмиссиюСУЗКлиентСервер.ТребуетсяПодписание(ВидыПродукцииПоТоварнойГруппе, ФорматОбмена);
	...
	Иначе
		...
		ИспользуютсяНаборыИлиГрупповыеУпаковки = ИспользуютсяНаборыИлиГрупповыеУпаковки(ВидыПродукцииПоТоварнойГруппе);
		
		МассивЗаказов = МассивЗаказовПоТаблицеТоваров(ТабличнаяЧастьТовары, ФорматОбмена, ВидыПродукцииПоТоварнойГруппе);
		...
		Для Каждого СодержаниеЗаказа Из МассивЗаказов Цикл
			...
			Если Шапка.СпособВводаВОборот = Перечисления.СпособыВводаВОборотСУЗ.МаркировкаОстатков
				И Шапка.ПриобретенПроизведенПослеДатыОбязательнойМаркировки Тогда
			...
			Для Каждого СтрокаТЧ Из СодержаниеЗаказа Цикл
				...
				Если ЭтоМех Тогда
					...
					КонецЕсли;
					СтрокаЗаказаКодовМаркировки.Вставить("tnvedCode",          ИнтеграцияИСМПКлиентСервер.КодТНВЭДДляПередачиВИСМП(СтрокаТЧ.КодТНВЭД, Шапка.ТоварнаяГруппа));
				...
				Если ФорматОбменаV3 И Не ЭтоМех
					Или ИспользуютсяНаборыИлиГрупповыеУпаковки Тогда
					СтрокаЗаказаКодовМаркировки.Вставить("cisType", ИнтерфейсСУЗСлужебный.ТипКодаМаркировкиСтрокой(СтрокаТЧ.ТипКодаМаркировки));
				КонецЕсли;
...
Функция МассивЗаказовПоТаблицеТоваров(ВходнаяТаблицаТовары, ФорматОбмена, ВидыПродукцииПоТоварнойГруппе)
	...
	Для Каждого СтрокаТаблицы Из ВходнаяТаблицаТовары Цикл
		...
		ИначеЕсли ЭтоМех Тогда
			СтрокаТаблицы.КодТНВЭД = ИнтеграцияИСМПКлиентСервер.КодТНВЭДДляПередачиВИСМП(СтрокаТаблицы.КодТНВЭД, ВидыПродукцииПоТоварнойГруппе)
		КонецЕсли;
		
	КонецЦикла;
	
	Если ЭтоМех Тогда
		ПоляСвертки  = "GTIN, КодТНВЭД, СпособВводаВОборот, ВидКИЗ, РазмерКИЗ, Шаблон, ШаблонКод, ТипКодаМаркировки";
		ПоляПодсчета = ПоляСвертки;
	Иначе
		ПоляСвертки  = "GTIN, ВладелецGTIN, СпособФормированияСерийногоНомера, Шаблон, ШаблонКод, ТипКодаМаркировки, СрокГодности";
		ПоляПодсчета = "GTIN";
	КонецЕсли;
...
Функция СтрокаТаблицыГруппируетсяВЗаказеНаЭмиссию(СтрокаТаблицы)
	...
	Возврат СтрокаТаблицы.СпособФормированияСерийногоНомера <> Перечисления.СпособыФормированияСерийногоНомераСУЗ.Самостоятельно;
	
КонецФункции

Функция ИспользуютсяНаборыИлиГрупповыеУпаковки(ВидыПродукции)
	...
	Для Каждого ВидПродукции Из ВидыПродукции Цикл
		Если КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетНаборы(ВидПродукции) Тогда
			Возврат Истина;
		КонецЕсли;
		Если КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетГрупповыеУпаковки(ВидПродукции) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Функция ОписаниеОстатковJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ПараметрыОписанияОстатков = ИнтеграцияИСМПКлиентСервер.ИспользованиеПараметровОписанияОстатковПоВидуПродукции(ТоварнаяГруппа);
	...
	СообщениеJSON.ТоварнаяГруппа     = ТоварнаяГруппа;
	СообщениеJSON.Операция           = Операция;
...
Функция ПолучениеКодовМаркировки(ДокументСсылка, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Для Каждого СтрокаПрисоединенныйФайл Из ТаблицаПрисоединенныеФайлы Цикл
		...
		Для Каждого СтрокаПротокола Из ПротоколОбмена Цикл
			...
			Если ДанныеОбработки <> Неопределено Тогда
				Для Каждого СтрокаЗаказа Из ДанныеОбработки["products"] Цикл
					...
					НоваяСтрока.Шаблон             = Справочники.ШаблоныКодовМаркировкиСУЗ.ПустаяСсылка();
...
Функция ДанныеРасчетаСтатусаЗаказаПоПулу(Документ, ВидПродукцииТоварнаяГруппа) Экспорт
	...
	Иначе
		...
		Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ВидПродукцииТоварнаяГруппа)
...
Функция ДанныеДоступныхGTIN(Объект, ПроверятьИНН = Ложь, ПроверятьШаблон = Ложь, ОграничениеПоВидуУпаковки = Неопределено) Экспорт
	...
	Если ПроверяемыеGTIN.Количество() > 0 Тогда
		...
		РезультатПроверки = ИнтерфейсИСМПОбщегоНазначения.ДанныеПродукцииПоШтрихкодуEAN(ПроверяемыеGTIN, Организация);
		...
		ДанныеПоШтрихкодамEANРезультат = ШтрихкодированиеОбщегоНазначенияИС.ДанныеПоШтрихкодамEAN(ПроверяемыеGTIN);
		ДанныеПоШтрихкодам = ДанныеПоШтрихкодамEANРезультат.ДанныеПоШтрихкодамEAN;
		...
		Для Каждого GTIN Из ПроверяемыеGTIN Цикл
			...
			Если ДанныеРезультата = Неопределено
				...
				Или ВидыПродукцииПоТоварнойГруппе.Найти(ДанныеРезультата.ВидПродукции) = Неопределено
...
// 
// Параметры:
//  ДокументОбъект - ДокументОбъект.ЗаказНаЭмиссиюКодовМаркировкиСУЗ
...
Процедура ДополнительнаяОбработкаДанныхПриПереходеНаКонструкторНастроек(ДокументОбъект, ПроблемыСДанными, КешКонвертаций) Экспорт
	...
	Для Каждого СтрокаТовары Из ДокументОбъект.Товары Цикл
		...
		Если ТипЗнч(СтрокаТовары.Шаблон) <> Тип("ПеречислениеСсылка.ШаблоныКодовМаркировкиСУЗ")
			Или Не ЗначениеЗаполнено(СтрокаТовары.Шаблон) Тогда
			Продолжить;
		КонецЕсли;
		
		НовыйШаблон = ОбновлениеИнформационнойБазыИСМП.ШаблонКодаМаркировкиПоЗначениюПеречисления(
			СтрокаТовары.Шаблон,
			ДокументОбъект.ТоварнаяГруппа,
			КешКонвертаций);
		
		Если ЗначениеЗаполнено(НовыйШаблон) Тогда
			СтрокаТовары.Шаблон = НовыйШаблон;
		Иначе
			ПроблемыСДанными[ДокументОбъект.Ссылка] = СтрШаблон(
				НСтр("ru = 'Не удалось обработать шаблон ""%1"" при переходе на конструктор настроек видов продукции для объекта ""%2""'"),
				СтрокаТовары.Шаблон,
