...
// 
// Параметры:
//  Объект - ДокументОбъект.ФормированиеПартийПриПроизводствеЗЕРНО - Объект
...
Функция ФормированиеПартииЗЕРНО(ДокументСсылка, ДополнительныеПараметры) Экспорт
	...
	|	ЗНАЧЕНИЕ(Перечисление.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна) КАК РазделУчета,
	|
	...
	|		И КлассификаторНСИЗЕРНО.РазделУчета = Значение(Перечисление.РазделыУчетаЗЕРНО.Зерно)
	|ГДЕ
	|	Сырье.Ссылка = &Ссылка",
	"ПартииЗерно");
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлассификаторНСИЗЕРНО КАК КлассификаторНСИЗЕРНО
	|		ПО РеестрПартийЗЕРНО.ОКПД2 = КлассификаторНСИЗЕРНО.Идентификатор
	|		И КлассификаторНСИЗЕРНО.РазделУчета = Значение(Перечисление.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна)
	|ГДЕ
	|	Сырье.Ссылка = &Ссылка",
	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийЗЕРНО.АннулированиеФормированияПартии Тогда
		...
		Попытка
			...
			Партия = Справочники.РеестрПартийЗЕРНО.ПартияЗерна(
				НомерПартии, ЭлементОчереди.Организация, ЭлементОчереди.Подразделение, РазделУчета, ПараметрыОбмена);
