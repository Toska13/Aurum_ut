...
Функция КонтрольКачестваПараметрыДействий(Объект) Экспорт
	...
	ИспользоватьОрдернуюСхему = СкладыСервер.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач(Объект.Склад, Объект.Дата);
	...
	ПараметрыДействия.ПараметрыЗапроса.КонтрольКачестваСбросить = ИспользоватьОрдернуюСхему;
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ДанныеДокумента.СтатьяДоходов.ПринятиеКНалоговомуУчету КАК СтатьяДоходовПринимаетсяКНалоговомуУчету,
	...
	|	(ВЫБОР
	|		КОГДА ДанныеДокумента.ИсправляемыйДокумент.Дата < НАЧАЛОПЕРИОДА(ДанныеДокумента.Дата, МЕСЯЦ)
	|			ТОГДА ДанныеДокумента.ИсправляемыйДокумент.Дата
	|		ИНАЧЕ ДАТАВРЕМЯ(1,1,1) КОНЕЦ)  КАК ПериодКорректировки,
...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	// Если при пересортице товара на хранении приходуется собственный товар, то отражается перемещение 
	// в раздел учета "Товары на хранении с правом продажи (к оформлению списания)" #Область Перемещение_ТоварНаХранении.
	|	И НЕ (ТаблицаВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ТоварНаХраненииСПравомПродажи)
	|		И ТаблицаВидыЗапасов.ТипЗапасовОприходование = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар))
	|";
	...
	|	НЕОПРЕДЕЛЕНО 									   	КАК КорПартия,
	|	(ВЫБОР
	|		КОГДА ТаблицаВидыЗапасов.ТипЗапасовОприходование = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|			ТОГДА ТаблицаВидыЗапасов.АналитикаСписания
	...
	|	И (ТаблицаДокумента.ПриходоватьТоварыПоСебестоимостиСписания
	|		ИЛИ ТаблицаВидыЗапасов.ТипЗапасовОприходование = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар))
	|";
