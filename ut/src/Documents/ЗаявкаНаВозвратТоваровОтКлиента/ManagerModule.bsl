...
Процедура ЗадатьОписаниеЗаполняемыхРеквизитовПоСтатистике(ОписаниеРеквизитов) Экспорт
	...
	Параметры = ЗаполнениеОбъектовПоСтатистике.ПараметрыЗаполняемыхРеквизитов();
	Параметры.РазрезыСбораСтатистики.ИспользоватьТолькоЗаполненные = "Автор, ХозяйственнаяОперация";
	ЗаполнениеОбъектовПоСтатистике.ДобавитьОписаниеЗаполняемыхРеквизитов(ОписаниеРеквизитов,
		"Соглашение", Параметры);
	
	Параметры = ЗаполнениеОбъектовПоСтатистике.ПараметрыЗаполняемыхРеквизитов();
	Параметры.РазрезыСбораСтатистики.ИспользоватьВсегда = "Автор";
	Параметры.РазрезыСбораСтатистики.ИспользоватьТолькоЗаполненные = "Партнер, Соглашение, ХозяйственнаяОперация";
	...
	Параметры.ЗаполнятьПриУсловии.ПоляОбъектаЗаполнены = "Автор";
	ЗаполнениеОбъектовПоСтатистике.ДобавитьОписаниеЗаполняемыхРеквизитов(ОписаниеРеквизитов,
		"Организация, Контрагент", Параметры);
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Документы.ОтгрузкаТоваровКлиенту.ДобавитьКомандуСоздатьНаОснованииЗаказа(КомандыСозданияНаОсновании);
...
Функция СформироватьЗапросПроверкиПриСменеСтатуса(МассивДокументов, НовыйСтатус, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры <> Неопределено И ДополнительныеПараметры.Свойство("КонтрольВыполненияЗаявки") Тогда
		...
		|	втОстаткиПоЗаказам
		|ИЗ
		|	РегистрНакопления.РаспоряженияНаОтгрузкуИВозврат.Обороты(,,, Распоряжение В (&МассивДокументов)) КАК РаспоряженияНаОтгрузкуОбороты
		|;
		|ВЫБРАТЬ
...
Процедура РассчитатьСостояние(Знач ЗаявкаНаВозвратТоваровОтКлиента,
	...
	Если НЕ ЗначениеЗаполнено(ЗаявкаНаВозвратТоваровОтКлиента) ИЛИ НЕ ПолучитьФункциональнуюОпцию("ИспользоватьРасширенныеВозможностиЗаказаКлиента") Тогда
		Возврат;
	КонецЕсли;
	...
	|ПОМЕСТИТЬ ВТ_ДокументЗаявка
	|ИЗ
	|	Документ.ЗаявкаНаВозвратТоваровОтКлиента КАК ВТ
	|ГДЕ
	|	ВТ.Ссылка = &ДокументЗаявка
	...
	|ИЗ
	|	ВТ_ДокументЗаявка КАК ДокументЗаявка
	|ЛЕВОЕ СОЕДИНЕНИЕ
	...
	Если ПроверитьЗаданияКОтражениюСостоянияЗаказов Тогда
		...
		ТекстЗапросаЕстьЗадания = СостоянияДокументов.ТекстЗапросаЕстьЗаданияКОтражениюСостоянияЗаказов("&ДокументЗаявка");
	...
	ТекстЗапросаРасчеты = "ВЫБРАТЬ 0 КАК СуммаКОплате";
	Если ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.РасчетыСКлиентами) Тогда
		...
		|			ОбъектРасчетов В (ВЫБРАТЬ ВТ.ОбъектРасчетов ИЗ ВТ_ДокументЗаявка КАК ВТ)
		|	) КАК РасчетыСКлиентамиОстатки
		|ПО
		|	ДокументЗаявка.ОбъектРасчетов = РасчетыСКлиентамиОстатки.ОбъектРасчетов
		|";
	КонецЕсли;
...
Функция ТекстЗапросаДляРасчетаСостоянийЗаказов() Экспорт
	...
	ПараметрыФормированияТаблицы.Индексы = "Распоряжение";
	ПараметрыФормированияТаблицы.ИмяВременнойТаблицы = "ВТ_ЗаявкиНаВозвратОборотыРазвернутая";
	
	ВТ_ЗаявкиНаВозвратОборотыРазвернутая =
		РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат.ТекстЗапросаРазвернутаяТаблица(
			ПараметрыФормированияТаблицы);
	
	ТекстЗапроса = ВТ_ЗаявкиНаВозвратОборотыРазвернутая +
		...
		|		И ЕСТЬNULL(ЗаявкиНаВозвратОстатки.КОформлениюПоВозвратамКоличествоОборот, 0) -
		|			ЕСТЬNULL(ЗаявкиНаВозвратОстатки.ОформленоПоВозвратамКоличествоОборот, 0) <> 0)
		...
		|
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	ВТ_ЗаявкиНаВозвратОборотыРазвернутая КАК ЗаявкиНаВозвратОстатки
		|ПО
		|	ДокументЗаявка.Ссылка = ЗаявкиНаВозвратОстатки.Распоряжение
		|
		|ЛЕВОЕ СОЕДИНЕНИЕ
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	Возвращаемые.ПоляСвязи.Добавить("Склад");
	...
	Заменяющие.ПоляСвязи.Добавить("Склад");
...
Функция ДоступныеОстаткиПараметрыВстраивания(ДокументОбъект = Неопределено) Экспорт
	...
	Связи.Склад                 = "Элементы.ЗаменяющиеТовары.ТекущиеДанные.Склад";
	...
	ЗаменяющиеТовары.Вставить("Склад",              Новый ОписаниеТипов("СправочникСсылка.Склады"));
...
// 
// Параметры:
//  ОбъектИлиСсылка - ДокументОбъект.ЗаявкаНаВозвратТоваровОтКлиента, ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента - документ.
...
// 
// Параметры:
//  Объект - ДокументОбъект.ЗаявкаНаВозвратТоваровОтКлиента - объект.
// 
// Возвращаемое значение:
...
Функция КонтрольКачестваПараметрыДействий(Объект) Экспорт
	...
	ПараметрыДействия.ЧитаемыеПоля = "Номенклатура,НоменклатураОприходование,Порча,Склад,ДатаПоступления,Отменено";
	...
	ПараметрыДействия.ПараметрыЗапроса.КонтрольКачестваДатаПоступления = Объект.ДатаПоступления;
	ПараметрыДействия.ПараметрыЗапроса.Вставить("КонтрольКачестваПоступлениеОднойДатой", Объект.ПоступлениеОднойДатой);
	...
	ПараметрыДействия.ТекстЗапросаДанныеДокумента =
		...
		|	ВЫБОР
		|		КОГДА &КонтрольКачестваПоступлениеОднойДатой
		|			ТОГДА &КонтрольКачестваДатаПоступления
		|		ИНАЧЕ Таблица.ДатаПоступления
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Иначе
		Если ВидФормы = "ФормаСписка" Тогда
			Если Не ПравоДоступа("Чтение", Метаданные.РегистрыСведений.СостоянияЗаказовКлиентов) Тогда
				СтандартнаяОбработка = Ложь;
...
Процедура ПослеЗаполненияОбеспечения(Объект, Изменения, Режим, ПараметрыЗаполнения) Экспорт
	...
	ОбеспечениеВДокументахСервер.ПересчитатьКоличествоЕдиниц(Изменения, Объект.ЗаменяющиеТовары);
...
Функция ТекстЗапросаТаблицаРаспоряженияНаОтгрузку(Запрос, ТекстыЗапроса, Регистры)
	...
	|					)
	|			ИЛИ ТаблицаТовары.СтатусУказанияСерий В(10, 14)
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Отменено)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Заказано)
	|	КОНЕЦ КАК Показатель,
	|	ТаблицаТовары.Количество КАК Количество,
	|	ТаблицаТовары.СуммаСНДС КАК Сумма,
	|	ВЫБОР
	|		КОГДА ТаблицаТовары.Отменено
	|			ТОГДА ТаблицаТовары.ПричинаОтмены
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПричиныОтменыЗаказовКлиентов.ПустаяСсылка)
	...
	|	ТаблицаТовары.КодСтроки КАК КодСтроки,
	|	ВЫБОР
	|		КОГДА ТаблицаПоказателей.Ссылка = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.КОтгрузке)
	|			ТОГДА ВЫБОР
	|				КОГДА ТаблицаТовары.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),
	|					ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
	|					ТОГДА ТаблицаТовары.Склад
	...
	|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОтХранителя)
	|					)
	|			ИЛИ ТаблицаПоказателей.Ссылка = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.КОтгрузке)
	...
	|	&Период КАК Период,
	|	ТаблицаВозвращаемыеТовары.Ссылка КАК Распоряжение,
	|	ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА ТаблицаВозвращаемыеТовары.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),
	|			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
	|			ТОГДА ТаблицаВозвращаемыеТовары.Склад
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	|	КОНЕЦ КАК Склад,
	|	ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка) КАК Серия,
	|	ТаблицаПоказателей.Ссылка КАК Показатель,
	|	ТаблицаВозвращаемыеТовары.Количество КАК Количество,
	|	ТаблицаВозвращаемыеТовары.СуммаСНДС КАК Сумма,
	|	ЗНАЧЕНИЕ(Справочник.ПричиныОтменыЗаказовКлиентов.ПустаяСсылка) КАК ПричинаОтмены
	|ИЗ
	...
	|		ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.Выполнена),
	|		ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.Отклонена))
	|	И &ХозяйственнаяОперация НЕ В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОтХранителя),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОтКомиссионера))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	КОНЕЦ КАК Склад,
	|	ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка) КАК Серия,
	|	ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.Отменено) КАК Показатель,
	|	ТаблицаВозвращаемыеТовары.Количество КАК Количество,
	|	ТаблицаВозвращаемыеТовары.СуммаСНДС КАК Сумма,
	|	ЗНАЧЕНИЕ(Справочник.ПричиныОтменыЗаказовКлиентов.ПустаяСсылка) КАК ПричинаОтмены
	|ИЗ
	...
	|ГДЕ
	|	ТаблицаВозвращаемыеТовары.Ссылка = &Ссылка
	|	И ТаблицаВозвращаемыеТовары.Отменено
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры)
	...
	ВзаиморасчетыПроведениеКлиенты.ПроведениеВозвратаТоваровОтКлиента(Запрос, ТекстыЗапроса, Регистры, ТекстВозврат); 
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|		ИНАЧЕ ТабЧасть.Номенклатура
		|	КОНЕЦ КАК Номенклатура,
		|	ВЫБОР
		|		КОГДА ТабЧасть.Порча
		|			ТОГДА ТабЧасть.ХарактеристикаОприходование
		|		ИНАЧЕ ТабЧасть.Характеристика
		|	КОНЕЦ КАК Характеристика,
		|	ТабЧасть.Склад КАК Склад,
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ВыбиратьВариантВыводаСкидокПриПечатиДокументовПродажи") Тогда
		...
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "ЗаявкаНаВозвратБезСкидок";
	...
	СтатусыЗаявки = Новый Массив;
	СтатусыЗаявки.Добавить(Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КОбеспечению);
	СтатусыЗаявки.Добавить(Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КОтгрузке);
	СтатусыЗаявки.Добавить(Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.Выполнена);
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	СинонимМакетаЗаявкиНаВозврат = НСтр("ru = 'Заявка на возврат'");
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыПечати, "ИспользоватьСинонимДляКомплекта", Ложь) Тогда
		СинонимМакетаЗаявкиНаВозврат = ПараметрыПечати.СинонимМакетаДляКомплекта;
	КонецЕсли;
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ЗаявкаНаВозвратБезСкидок") Тогда
...
Функция СформироватьПечатнуюФормуЗаявкиНаВозврат(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, КодЯзыка = Неопределено)
	...
	|						ПРЕДСТАВЛЕНИЕ(Таблица.Номенклатура.ЕдиницаИзмерения)
	|					ИНАЧЕ Таблица.Номенклатура.ЕдиницаИзмерения.МеждународноеСокращение
	|				КОНЕЦ
	|		ИНАЧЕ 
	|				ВЫБОР 
	|					КОГДА &ЭтоОсновнойЯзыкСистемы ТОГДА
	|						ПРЕДСТАВЛЕНИЕ(Таблица.Упаковка)
	|					ИНАЧЕ Таблица.Упаковка.МеждународноеСокращение
	|				КОНЕЦ
...
//	МассивОбъектов  - Массив Из ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента - ссылки на объекты которые нужно распечатать.
//	
// Возвращаемое значение:
...
Функция ПолучитьДанныеДляПечатнойФормыОтборРазмещениеТоваров(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|ИЗ
	|	Документ.ЗаявкаНаВозвратТоваровОтКлиента КАК ДанныеЗадания
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаВозвратТоваровОтКлиента.ЗаменяющиеТовары КАК ТоварыДокумента
	...
	|	И ДанныеЗадания.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КОбеспечению),
	|								ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КОтгрузке),
	|								ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.Выполнена))
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|	ТоварыКОтгрузке.КОтгрузкеОстаток	КАК Количество
	...
	|				ГДЕ
	|					НЕ ДанныеЗадания.ИспользуетсяОрдернаяСхема)) КАК ТоварыКОтгрузке
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка,
	|	Номенклатура
	...
	|ПОМЕСТИТЬ ТоварыЗаявкиНаВозврат
	|ИЗ
	|	Документ.ЗаявкаНаВозвратТоваровОтКлиента.ЗаменяющиеТовары КАК ТаблицаТовары
	...
	|ВЫБРАТЬ
	|	ТаблицаТовары.Ссылка				КАК Ссылка,
	|	ТоварыЗаявкиНаВозврат.НомерСтроки	КАК НомерСтроки,
	|	ТаблицаТовары.Номенклатура			КАК Номенклатура,
	|	ТаблицаТовары.Характеристика		КАК Характеристика,
	|	ТаблицаТовары.Серия					КАК Серия,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТоварыЗаявкиНаВозврат КАК ТоварыЗаявкиНаВозврат
	|		ПО ТаблицаТовары.Ссылка = ТоварыЗаявкиНаВозврат.Ссылка
	|			И ТаблицаТовары.Номенклатура = ТоварыЗаявкиНаВозврат.Номенклатура
	|			И ТаблицаТовары.Характеристика = ТоварыЗаявкиНаВозврат.Характеристика
	|			И ТаблицаТовары.Серия = ТоварыЗаявкиНаВозврат.Серия
	|			И ТаблицаТовары.Склад = ТоварыЗаявкиНаВозврат.Склад
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура,
	|	Склад
	...
	|	ПРЕДСТАВЛЕНИЕ(ТаблицаТоваров.Серия)					КАК ПредставлениеСерии,
	...
	МаксимальныйИндекс = Результаты.ВГраница();
	
	ДанныеЗадания = Новый Структура;
	ДанныеЗадания.Вставить("РезультатПоОшибкам",		Результаты[МаксимальныйИндекс - 3]);
	ДанныеЗадания.Вставить("РезультатПоРаспоряжениям",	Неопределено);
	ДанныеЗадания.Вставить("РезультатПоШапке",			Результаты[МаксимальныйИндекс - 2]);
	ДанныеЗадания.Вставить("РезультатПоТабличнойЧасти",	Результаты[МаксимальныйИндекс - 1]);
	ДанныеЗадания.Вставить("РезультатПоУпаковкам",		Результаты[МаксимальныйИндекс]);
...
// Возвращает текст запроса товаров документа при заполнении на основании.
//
// Параметры:
// 	Отбор - Строка
...
Функция ТекстЗапросаТоварыДокумента(Отбор, ТекстСоединения = "", ДополнятьПоТаблицеСерий = Ложь) Экспорт
	...
	ТекстЗапросаСоединениеТаблиц =
	"Таблица.Ссылка = ДанныеУчета.ЗаказПоставщику
	...
	|	Таблица.Характеристика КАК Характеристика,
	|	Таблица.Назначение КАК Назначение,
	|	&ТекстПоляСерия КАК Серия,
	|	Таблица.КодСтроки КАК КодСтроки,
	|	Таблица.ДатаПоступления КАК ДатаПоступления,
	|	&ТекстПоляКоличество КАК Количество,
	|	Таблица.Упаковка КАК Упаковка,
	|	Таблица.Цена КАК Цена,
	|	Таблица.СтавкаНДС КАК СтавкаНДС,
	|	Таблица.Склад КАК Склад,
	...
	|	Таблица.НоменклатураНабора КАК НоменклатураНабора,
	|	Таблица.ХарактеристикаНабора КАК ХарактеристикаНабора,
	|	Таблица.ИндексНабора КАК ИндексНабора,
	|	Таблица.Порча КАК Порча,
	|	Таблица.Сумма КАК Сумма,
	|	Таблица.СуммаНДС КАК СуммаНДС,
	|	Таблица.СуммаСНДС КАК СуммаСНДС
	...
	|		ПО &МетодСоединения
	|ГДЕ
	|	Таблица.Ссылка.ЭтоЗаказКакСчет = ЛОЖЬ
	|	И НЕ ДанныеУчета.Номенклатура ЕСТЬ NULL
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	Таблица.ДатаПоступления КАК ДатаПоступления,
	|	Таблица.Количество КАК Количество,
	|	Таблица.Количество КАК КОформлению,
	|	Таблица.Упаковка КАК Упаковка,
	|	Таблица.Цена КАК Цена,
	|	Таблица.СтавкаНДС КАК СтавкаНДС,
	|	Таблица.Склад КАК Склад,
	...
	|	ТаблицаТовары КАК Таблица
	|ГДЕ
	|	Таблица.Ссылка.ЭтоЗаказКакСчет = ИСТИНА
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	Если ДополнятьПоТаблицеСерий Тогда
		...
		ТекстУсловияСоединенияСерий =
		"Таблица.Ссылка = ТаблицаСерии.Ссылка
		...
		|		И Таблица.Склад = ТаблицаСерии.Склад";
		
	Иначе
		
		ТекстПоляСерия = "Таблица.Серия";
		ТекстПоляКоличество = "Таблица.Количество";
		ТекстУсловияСоединенияСерий = "ЛОЖЬ";
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&Отбор",
		ОбщегоНазначенияУТ.ТекстОтбораПоКоллекцииОтборов(Отбор));
	
	Возврат ТекстЗапроса;
	
КонецФункции

// Текст запроса товары переданные.
// 
// Возвращаемое значение:
//  См. Документы.ПоступлениеТоваровОтХранителя.ТекстЗапросаТоварыПереданные
//
Функция ТекстЗапросаТоварыПереданные() Экспорт
	
	Возврат Документы.ПоступлениеТоваровОтХранителя.ТекстЗапросаТоварыПереданные();
...
// 
// Параметры:
//  СсылкаОбъект - ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента - Документ, по которому нужно получить данные.
...
Функция СуммыПоЗаказам(СсылкаОбъект, ЗначенияРеквизитов = Неопределено) Экспорт
	...
	Если ЗначенияРеквизитов = Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТаблицаЗначений", "Документ.ЗаявкаНаВозвратТоваровОтКлиента.ЗаменяющиеТовары");
