...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Запрос.Текст =  
		...
		|			И ДвиженияДокумента.Организация = ПланыЗакупок.Организация
...
Процедура СоглашениеПриИзмененииНаСервере()
	...
	РеквизитыСоглашения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Соглашение,
		"Организация, ВидЦеныПоставщика, ЦенаВключаетНДС");
	ВидЦеныПоставщика = РеквизитыСоглашения.ВидЦеныПоставщика;
	Если ЗаполнятьОрганизацию Тогда
		Если ЗначениеЗаполнено(РеквизитыСоглашения.Организация) Тогда
			Объект.Организация = РеквизитыСоглашения.Организация;
			ОрганизацияПриИзмененииСервер();
		КонецЕсли;
		Если Объект.ЦенаВключаетНДС <> РеквизитыСоглашения.ЦенаВключаетНДС Тогда
			Объект.ЦенаВключаетНДС = РеквизитыСоглашения.ЦенаВключаетНДС;
...
Процедура ТоварыДатаПоступленияПриИзменении(Элемент)
	...
	Если ТекущиеДанные.СрокИсполненияЗаказа <> 0 Тогда
...
Функция ТоварыСрокИсполненияЗаказаПриИзмененииНаСервере(ДатаПоступления, СрокИсполненияЗаказа)
	...
	Текст =
		...
		|///////////////////////////////////
		|ВЫБРАТЬ
		|	РасчетДатыЗаказаПереопределяемый.НайденнаяДата КАК Дата
		|ИЗ
...
&НаКлиенте
Процедура ТоварыПоПериодамСтавкаНДСПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПланОплатыСтавкаНДСПриИзменении(Элемент)
	
	ЭтапОплаты = Элементы.ПланОплаты.ТекущиеДанные;
	
	ТекПроцентНДС = УчетНДСУПКлиентСервер.ЗначениеСтавкиНДС(ЭтапОплаты.СтавкаНДС);
	ЭтапОплаты.СуммаНДС = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(ЭтапОплаты.СуммаПлатежа, ТекПроцентНДС, Истина,
...
Процедура ОбовитьСуммыПланаОплат()
	...
	Если Объект.КроссТаблица Тогда
		...
		ПланОплаты.Свернуть("ВариантОплаты, ДатаПлатежа, ДатаПоступления, Замещен, СтавкаНДС","СуммаПлатежа, СуммаНДС, СуммаЗадолженности, СуммаОтгрузок");
		...
		Для каждого ЭтапОплаты Из ПланОплаты Цикл
			...
			Для каждого Период Из ТаблицаПериоды Цикл
				...
				ЭтапОплаты.СуммаОтгрузок = ЭтапОплаты.СуммаОтгрузок + ЭтаФорма["ТоварыПоПериодам" + "СуммаСНДС_"+Период.ИмяКолонки];
			КонецЦикла;
	...
	Иначе
		...
		СуммыПоДатам = Объект.Товары.Выгрузить(Новый Структура("Отменено", Ложь),"ДатаПоступления, Замещен, СтавкаНДС, СуммаНДС, СуммаСНДС");
		СуммыПоДатам.Свернуть("ДатаПоступления, Замещен, СтавкаНДС", "СуммаНДС, СуммаСНДС");
		...
		Для каждого ЭтапОплаты Из ПланОплаты Цикл
			...
			Для каждого СуммаНаДату Из СуммыПоДатам Цикл
				...
				Если ЭтапОплаты.ДатаПлатежа < СуммаНаДату.ДатаПоступления Тогда
					Продолжить;
				КонецЕсли;
				ЭтапОплаты.СуммаОтгрузок = ЭтапОплаты.СуммаОтгрузок + СуммаНаДату.СуммаСНДС;
			КонецЦикла; 
...
Процедура УстановитьУсловноеОформление()
	...
	УчетНДСУП.УстановитьУсловноеОформлениеСуммНДСПоНалогообложениюЗакупки(ЭтотОбъект,
		"ТоварыПоПериодамСтавкаНДС", "ТоварыПоПериодамСуммаНДС", "ТоварыПоПериодамСуммаСНДС");
	УчетНДСУП.УстановитьУсловноеОформлениеСуммНДСПоНалогообложениюЗакупки(ЭтотОбъект,
		"ПланОплатыСтавкаНДС", "ПланОплатыСуммаНДС");
	
	//
	
	ЦеныПредприятияЗаполнениеСервер.УстановитьУсловноеОформлениеЦенаВключаетНДС(ЭтотОбъект);
	ЦеныПредприятияЗаполнениеСервер.УстановитьУсловноеОформлениеЦенаВключаетНДС(ЭтотОбъект, "ТоварыПоПериодамСуммаСНДС");
	...
	ПараметрыУсловногоОформления = СкладыСервер.ПараметрыУсловногоОформленияСкладаВТЧПоТипуНоменклатуры();
	ПараметрыУсловногоОформления.Форма = ЭтотОбъект;
	ПараметрыУсловногоОформления.ИмяПоляВводаСклада = Элементы.ТоварыПоПериодамСклад.Имя;
	ПараметрыУсловногоОформления.ИмяПоляТипНоменклатуры = "ТоварыПоПериодам.ТипНоменклатуры";
	ПараметрыУсловногоОформления.ПроверятьЗаполненностьСклада = Истина;
	ПараметрыУсловногоОформления.ИмяПоляСкладВТЧ = "ТоварыПоПериодам.Склад";
...
Процедура ПриИзмененииКоличестваСуммыСтроки(Объект, Форма, СтрокаТоваров)
	...
	Если ЗначениеЗаполнено(СтрокаТоваров.СтавкаНДС) Тогда
		ПроцентНДС = УчетНДСУПКлиентСервер.ЗначениеСтавкиНДС(СтрокаТоваров.СтавкаНДС);
	Иначе
		ПроцентНДС = 0;
	КонецЕсли;
	СуммаНДС = 0;
	...
	Для каждого СтрокаПериода Из Форма.Периоды Цикл
		...
		Если Объект.ПланироватьПоСумме Тогда
			...
			Если ПроцентНДС <> 0 Тогда
				СтрокаТоваров["СуммаНДС_" + СтрокаПериода.ИмяКолонки] = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(
					СтрокаТоваров["Сумма_" + СтрокаПериода.ИмяКолонки], ПроцентНДС, Объект.ЦенаВключаетНДС, Объект.НалогообложениеНДС);
			Иначе
				СтрокаТоваров["СуммаНДС_" + СтрокаПериода.ИмяКолонки] = 0;
			КонецЕсли;
		Иначе
			СтрокаТоваров["Сумма_" + СтрокаПериода.ИмяКолонки] = 0;
			СтрокаТоваров["СуммаНДС_" + СтрокаПериода.ИмяКолонки] = 0;
		КонецЕсли; 
		СтрокаТоваров["СуммаСНДС_" + СтрокаПериода.ИмяКолонки] = СтрокаТоваров["Сумма_" + СтрокаПериода.ИмяКолонки]
			+ ?(Объект.ЦенаВключаетНДС, 0, СтрокаТоваров["СуммаНДС_" + СтрокаПериода.ИмяКолонки]);
		
		Сумма = Сумма + СтрокаТоваров["Сумма_" + СтрокаПериода.ИмяКолонки];
		СуммаНДС = СуммаНДС + СтрокаТоваров["СуммаНДС_" + СтрокаПериода.ИмяКолонки];
		СуммаСНДС = СуммаСНДС + СтрокаТоваров["СуммаСНДС_" + СтрокаПериода.ИмяКолонки];
...
&НаКлиентеНаСервереБезКонтекста
Процедура РассчитатьИтоговыеПоказатели(Форма, РассчитыватьСтроки = Истина, ЗаполнятьПустыеЦены = Неопределено)

	Объект = Форма.Объект;
	
	Если ЗаполнятьПустыеЦены = Неопределено Тогда
		ЗаполнятьПустыеЦены = РассчитыватьСтроки;
	КонецЕсли;
	
	Если НЕ Объект.КроссТаблица Тогда
		
		Объект.СуммаДокумента = 0;
		Форма.ИтогоСуммаНДС = 0;
		Если Объект.ПланироватьПоСумме Тогда
		
			НайденныеСтроки = Объект.Товары.НайтиСтроки(Новый Структура("Отменено", Ложь));
			Для каждого СтрокаТЧ Из НайденныеСтроки Цикл
				
				Объект.СуммаДокумента = Объект.СуммаДокумента + СтрокаТЧ.Сумма + ?(Объект.ЦенаВключаетНДС, 0, СтрокаТЧ.СуммаНДС);
				Форма.ИтогоСуммаНДС = Форма.ИтогоСуммаНДС + СтрокаТЧ.СуммаНДС;
	...
	ПрефиксКолонкиСуммаНДС   = "ТоварыПоПериодам" + "СуммаНДС_";
	ПрефиксКолонкиСуммаСНДС  = "ТоварыПоПериодам" + "СуммаСНДС_";
	...
	Для каждого СтрокаПериода Из Форма.Периоды Цикл
		...
		Форма[ПрефиксКолонкиСуммаНДС + СтрокаПериода.ИмяКолонки] = 0;
		Форма[ПрефиксКолонкиСуммаСНДС + СтрокаПериода.ИмяКолонки] = 0;
		...
		Для каждого СтрокаПериода Из Форма.Периоды Цикл
			...
			Если РассчитыватьСтроки Тогда
				...
				ИначеЕсли Цена <> СтрокаТЧ["Цена_" + СтрокаПериода.ИмяКолонки] И СтрокаТЧ["Цена_" + СтрокаПериода.ИмяКолонки] <> 0 Тогда
					Цена = 0;
				ИначеЕсли ЗаполнятьПустыеЦены
					И Цена <> СтрокаТЧ["Цена_" + СтрокаПериода.ИмяКолонки] 
				...
				Если ПроцентНДС <> 0 Тогда
					СтрокаТЧ["СуммаНДС_" + СтрокаПериода.ИмяКолонки] = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(
						СтрокаТЧ["Сумма_" + СтрокаПериода.ИмяКолонки], ПроцентНДС, Объект.ЦенаВключаетНДС, Объект.НалогообложениеНДС);
				Иначе
					СтрокаТЧ["СуммаНДС_" + СтрокаПериода.ИмяКолонки] = 0;
				КонецЕсли;
				СтрокаТЧ["СуммаСНДС_" + СтрокаПериода.ИмяКолонки] = СтрокаТЧ["Сумма_" + СтрокаПериода.ИмяКолонки]
					+ ?(Объект.ЦенаВключаетНДС, 0, СтрокаТЧ["СуммаНДС_" + СтрокаПериода.ИмяКолонки]);
				СтрокаТЧ.СуммаНДС = СтрокаТЧ.СуммаНДС + СтрокаТЧ["СуммаНДС_" + СтрокаПериода.ИмяКолонки];
				СтрокаТЧ.СуммаСНДС = СтрокаТЧ.СуммаСНДС + СтрокаТЧ["СуммаСНДС_" + СтрокаПериода.ИмяКолонки];
			КонецЕсли;
			...
			Форма[ПрефиксКолонкиСуммаНДС + СтрокаПериода.ИмяКолонки] = Форма[ПрефиксКолонкиСуммаНДС + СтрокаПериода.ИмяКолонки] + СтрокаТЧ["СуммаНДС_" + СтрокаПериода.ИмяКолонки];
			Форма[ПрефиксКолонкиСуммаСНДС + СтрокаПериода.ИмяКолонки] = Форма[ПрефиксКолонкиСуммаСНДС + СтрокаПериода.ИмяКолонки] + СтрокаТЧ["СуммаСНДС_" + СтрокаПериода.ИмяКолонки];
...
Процедура ОбновитьСтруктуруКроссТаблицы(ТаблицаПериоды, ПересчитыватьИтоговыеПоказатели = Истина)
	...
	Элементы.ТоварыСтавкаНДС.Видимость = Объект.ПланироватьПоСумме;
	Элементы.ТоварыСуммаНДС.Видимость = Объект.ПланироватьПоСумме;
	Элементы.ТоварыСуммаСНДС.Видимость = Объект.ПланироватьПоСумме;
	...
	Элементы.ТоварыПоПериодамСтавкаНДС.Видимость = Объект.ПланироватьПоСумме И НЕ ЭтоОдинПериод;
	Элементы.ТоварыПоПериодамСуммаНДС.Видимость = Объект.ПланироватьПоСумме И НЕ ЭтоОдинПериод;
	Элементы.ТоварыПоПериодамСуммаСНДС.Видимость = Объект.ПланироватьПоСумме И НЕ ЭтоОдинПериод;
	...
	ПараметрыПоляСумма = Новый Структура("Префикс, Заголовок", "Сумма_", НСтр("ru = 'Сумма'"));
	Планирование.ДобавитьПолеКроссТаблицыСумма(ПараметрыВывода, Объект.ПланироватьПоСумме И ЭтоОдинПериод,
		ПараметрыПоляСумма, СтруктураДействий);
	
	ПараметрыПоляСумма = Новый Структура("Префикс, Заголовок", "СуммаНДС_", НСтр("ru = 'Сумма НДС'"));
	Планирование.ДобавитьПолеКроссТаблицыСумма(ПараметрыВывода, Ложь, ПараметрыПоляСумма);
	ПараметрыПоляСумма = Новый Структура("Префикс, Заголовок", "СуммаСНДС_", НСтр("ru = 'Сумма с НДС'"));
	Планирование.ДобавитьПолеКроссТаблицыСумма(ПараметрыВывода, Ложь, ПараметрыПоляСумма);
...
Процедура ОрганизацияПриИзмененииСервер()
	...
	ПараметрыЗаполнения.Период = Объект.НачалоПериода;
	Если ЗначениеЗаполнено(Объект.Партнер) Тогда
		ПараметрыЗаполнения.Контрагент = ПартнерыИКонтрагенты.ПолучитьКонтрагентаПартнераПоУмолчанию(Объект.Партнер);
	Иначе
		ПараметрыЗаполнения.Контрагент = Справочники.Контрагенты.ПустаяСсылка();
	КонецЕсли;
...
Процедура ЦенаВключаетНДСПриИзмененииСервер()
	...
	Если Объект.КроссТаблица Тогда
		...
		Для Каждого СтрокаТЧ Из ТоварыПоПериодам Цикл
			ПриИзмененииКоличестваСуммыСтроки(Объект, ЭтотОбъект, СтрокаТЧ);
		КонецЦикла;
...
Процедура УстановитьВидимость()
	...
	Элементы.Организация.Видимость = ЗаполнятьОрганизацию ИЛИ ЗначениеЗаполнено(Объект.Организация);
	Элементы.ЦенаВключаетНДС.Видимость = ЗначениеЗаполнено(Объект.Организация) И Объект.ПланироватьПоСумме;
	Элементы.ИтогоСуммаНДС.Видимость = ЗначениеЗаполнено(Объект.Организация) И Объект.ПланироватьПоСумме;
...
Функция ПолучитьПараметрыВидаПлана(Знач ВидПлана, Знач ОбновлятьПравило, АдресПравилаЗаполнения, АдресПользовательскихНастроек)
	...
	Реквизиты = "КоличествоПериодов, ЗаполнятьПодразделение, ЗаполнятьОрганизацию, ЗаполнятьПартнера, 
...
Процедура ДобавитьТовары(ТаблицаТоваров, КэшированныеЗначения = Неопределено, ЗагрузкаИзФайла = Ложь)
	...
	НовыеСтрокиТЧТовары = Новый Массив;
	...
	Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		Иначе
			...
			НовыеСтрокиТЧТовары.Добавить(ТекущаяСтрока.НомерСтроки);
	...
	Иначе
		ТоварыКоличествоСтрок = Объект.Товары.Количество();
		ЗаполнитьДатыЗаказаНаСервере(, НовыеСтрокиТЧТовары);
	КонецЕсли;
...
Процедура СкопироватьИзКроссТаблицы(КэшированныеЗначения, ЭтотОбъект)
	...
	Если ЭтотОбъект.КроссТаблица
		...
		Если ЗначениеЗаполнено(Объект.Организация) Тогда
			СтруктураПересчетаСуммы = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(Объект);
			СтруктураДействий.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
			Если ТипЗнч(ЭтотОбъект) <> Тип("ДокументОбъект.ПланЗакупок") Тогда
				СтруктураДействий.Вставить("ПересчитатьСуммуСНДС", СтруктураПересчетаСуммы);
			КонецЕсли;
		КонецЕсли;
...
Функция ПараметрыПреобразованияКроссТаблицы()
	...
	СтруктураРеквизитов.Вставить("СуммаНДС_", "СуммаНДС");
	...
	Если ЗаполнятьПартнера Тогда
		ПоляГруппировки = ПоляГруппировки + ", ВидЦеныПоставщика";
	КонецЕсли;
	...
	СтруктураИтоговыхРеквизитов.Вставить("СуммаНДС", "СуммаНДС");
...
Процедура ЗаполнитьПланОплаты(ГрафикСоглашенияЗаполнен, ГрафикЗаполнен)
	...
	Иначе
		...
		Если Объект.КроссТаблица Тогда
			...
			СуммыОплатыПоПериодам = СоответствиеСуммОплатыТаблицыПериодов();
			...
			Для каждого Период Из ТаблицаПериоды Цикл
				...
				СуммыОплатыПоПериоду = СуммыОплатыПоПериодам.Получить(Период.ИмяКолонки);
				
				Для Каждого СтрокаСуммыОплаты Из СуммыОплатыПоПериоду Цикл
					
					СуммаОплатыПоПериоду = СтрокаСуммыОплаты.Значение.СуммаСНДС;
					...
					ЭтапОплаты.СтавкаНДС = СтрокаСуммыОплаты.Ключ;
					ЭтапОплаты.СуммаНДС = СтрокаСуммыОплаты.Значение.СуммаНДС;
...
Процедура ЗаполнитьЭтапыОплатыПоШаблонуСервер(ГрафикСоглашенияЗаполнен, ГрафикЗаполнен)
	...
	Если Объект.КроссТаблица Тогда
		...
		Для каждого Период Из ТаблицаПериоды Цикл
			...
			СуммыОплатыКРаспределению = ЭтаФорма["ТоварыПоПериодам"].Выгрузить(, "СтавкаНДС, СуммаСНДС_"+Период.ИмяКолонки);
			СуммыОплатыКРаспределению.Свернуть("СтавкаНДС", "СуммаСНДС_"+Период.ИмяКолонки);
			
			Для Каждого СтрокаСуммыОплаты Из СуммыОплатыКРаспределению Цикл
				СуммаОплатыКРаспределению = СтрокаСуммыОплаты["СуммаСНДС_"+Период.ИмяКолонки];
				...
				Для каждого ЭтапОплаты Из ПланОплатыКопия Цикл
					...
					НовСтр.СтавкаНДС = СтрокаСуммыОплаты.СтавкаНДС;
					НовСтр.СуммаНДС = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(НовСтр.СуммаПлатежа, НовСтр.СтавкаНДС,
						Истина, Объект.НалогообложениеНДС);
	...
	Иначе
		...
		СуммыПоДатам = Объект.Товары.Выгрузить(Новый Структура("Отменено", Ложь),"ДатаПоступления, ДатаЗаказа, Замещен, СтавкаНДС, СуммаСНДС");
		СуммыПоДатам.Свернуть("ДатаПоступления, ДатаЗаказа, Замещен, СтавкаНДС", "СуммаСНДС");
		СуммыПоДатам.Сортировать("ДатаПоступления");
		...
		Для каждого СуммаНаДату Из СуммыПоДатам Цикл
			...
			СуммаОплатыКраспределению = СуммаНаДату.СуммаСНДС;
			...
			Для каждого ЭтапОплаты Из ПланОплатыКопия Цикл
				...
				НовСтр.СтавкаНДС = СуммаНаДату.СтавкаНДС;
				НовСтр.СуммаНДС = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(НовСтр.СуммаПлатежа, НовСтр.СтавкаНДС,
					Истина, Объект.НалогообложениеНДС);
...
Процедура ДобавитьЭтапОплатыПоУмолчаниюСервер()
	...
	СуммыПоДатам = Объект.Товары.Выгрузить(Новый Структура("Отменено", Ложь),"ДатаПоступления, СтавкаНДС, СуммаНДС, СуммаСНДС, Замещен");
	СуммыПоДатам.Свернуть("ДатаПоступления, Замещен, СтавкаНДС", "СуммаНДС, СуммаСНДС");
	СуммыПоДатам.Сортировать("ДатаПоступления");
	...
	Для каждого СуммаНаДату Из СуммыПоДатам Цикл
		...
		Если СуммаНаДату.СуммаСНДС = 0 Тогда
			Продолжить;
		КонецЕсли; 
		
		СуммаОтгрузок = СуммаОтгрузок + СуммаНаДату.СуммаСНДС;
		...
		ЭтапОплаты.СуммаПлатежа = СуммаНаДату.СуммаСНДС;
		
		ЭтапОплаты.СтавкаНДС = СуммаНаДату.СтавкаНДС;
		ЭтапОплаты.СуммаНДС = СуммаНаДату.СуммаНДС;
...
&НаСервере
Функция СоответствиеСуммОплатыТаблицыПериодов()
	
	СоответствиеСуммОплатыПоПериодам = Новый Соответствие;
	...
	Для Каждого Период Из ЭтотОбъект.Периоды Цикл
		...
		СуммыСНДСПоПериоду = ЭтотОбъект["ТоварыПоПериодам"].Выгрузить(, "СтавкаНДС, СуммаНДС_"+Период.ИмяКолонки+", СуммаСНДС_"+Период.ИмяКолонки);
		СуммыСНДСПоПериоду.Свернуть("СтавкаНДС", "СуммаНДС_"+Период.ИмяКолонки+", СуммаСНДС_"+Период.ИмяКолонки);
		
		СоответствиеПоПериоду = Новый Соответствие;
		Для Каждого СтрокаПериода Из СуммыСНДСПоПериоду Цикл
			СоответствиеПоПериоду.Вставить(СтрокаПериода.СтавкаНДС, Новый Структура("СуммаНДС, СуммаСНДС",
			СтрокаПериода["СуммаНДС_"+Период.ИмяКолонки], СтрокаПериода["СуммаСНДС_"+Период.ИмяКолонки]));
		КонецЦикла;
		
		СоответствиеСуммОплатыПоПериодам.Вставить(Период.ИмяКолонки, СоответствиеПоПериоду);
		
	КонецЦикла;
	
	Возврат СоответствиеСуммОплатыПоПериодам;
...
&НаСервере
Процедура ЗаполнитьДатыЗаказаНаСервере(ЭтотОбъект = Неопределено, ОтборСтрокТовары = Неопределено)
	...
	|ВЫБРАТЬ
	|	Товары.НомерСтроки КАК НомерСтроки,
	|	ЕСТЬNULL(Соглашения.СрокОтгрузки + Соглашения.СрокДоставки, СпрСпособОбеспечения.СрокИсполненияЗаказа) КАК СрокИсполненияЗаказа,
	|
	|	ВЫБОР
	...
	|		ИНАЧЕ ДОБАВИТЬКДАТЕ(Товары.ДатаПоступления, ДЕНЬ, - ЕСТЬNULL(Соглашения.СрокОтгрузки + Соглашения.СрокДоставки, СпрСпособОбеспечения.СрокИсполненияЗаказа))
	...
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КалендарныеГрафики КАК ГрафикПоДатеПоставки
	|		ПО ГрафикПоДатеПоставки.Календарь = &Календарь
	|		 И ГрафикПоДатеПоставки.ДатаГрафика = Товары.ДатаПоступления
	|		 И ГрафикПоДатеПоставки.Год = ГОД(Товары.ДатаПоступления)
	|
	...
	|		 И ДатаЗаказа.КоличествоДнейВГрафикеСНачалаГода = ГрафикПоДатеПоставки.КоличествоДнейВГрафикеСНачалаГода -
	|			ЕСТЬNULL(Соглашения.СрокОтгрузки + Соглашения.СрокДоставки, СпрСпособОбеспечения.СрокИсполненияЗаказа)
	...
	|			ЕСТЬNULL(Соглашения.СрокОтгрузки + Соглашения.СрокДоставки, СпрСпособОбеспечения.СрокИсполненияЗаказа) + ГрафикПоДатеПоставки.КоличествоДнейВГрафикеСНачалаГода
	...
	Если ОтборСтрокТовары = Неопределено Тогда
		Запрос.УстановитьПараметр("ОтборНомерСтроки", Истина);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборНомерСтроки", "Товары.НомерСтроки В (&НомераСтрок)");
		Запрос.УстановитьПараметр("НомераСтрок", ОтборСтрокТовары);
	КонецЕсли;
...
Функция ПараметрыВидаПлана(Форма)
	...
	ПараметрыВидаПлана = Новый Структура("ЗаполнятьПодразделение, ЗаполнятьОрганизацию, ЗаполнятьПартнера, ЗаполнятьСоглашение, ЗаполнятьСклад, 
...
Функция ПараметрыОтбора(Форма)
	...
	ПараметрыОтбора = Новый Структура("Партнер, Соглашение, Склад, Подразделение, Организация, Назначение");
...
Функция ПроверитьЗаполнениеШапки()
	...
	Если ЗаполнятьОрганизацию И НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
