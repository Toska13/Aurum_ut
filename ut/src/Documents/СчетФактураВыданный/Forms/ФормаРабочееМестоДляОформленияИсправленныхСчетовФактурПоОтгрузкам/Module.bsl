...
Функция ЗаполнитьТоварыИсправленногоСчетаФактуры(ПараметрыЗаполненияТоваровСФ, ЕстьОшибкиЗаполнения)
	...
	Для Каждого СтрокаНоменклатуры Из СтрокиНоменклатуры Цикл
		...
		Если СтрокиКИзменению.Количество() = 0 Тогда 
			...
			НоваяСтрока.СтавкаНДС = УчетНДСУП.СтавкаНДСПоНоменклатуреИНалогообложению(НоваяСтрока.Номенклатура, 
																			ПараметрыЗаполненияТоваровСФ.НалогообложениеНДС, 
																			ПараметрыЗаполненияТоваровСФ.Организация, 
																			ПараметрыЗаполненияТоваровСФ.ДатаСФ,
																			, Истина);
