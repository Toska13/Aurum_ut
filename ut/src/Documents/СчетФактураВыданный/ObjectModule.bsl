...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения, "ХозяйственнаяОперация") = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда 
		ТекстСообщения = НСтр("ru = 'Для документа реализации с операцией ""Переход права собственности"" не требуется оформление счета-фактуры'");
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.АктВыполненныхРабот")
		...
		ПараметрыОткрытия.Вставить("НачислениеНДСПриОтгрузке",
														ПараметрыРегистрации.НачислениеНДСПриОтгрузке);
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		ИначеЕсли ДанныеЗаполнения.Свойство("Корректировочный") И ДанныеЗаполнения.Корректировочный
			...
			И Не ДанныеЗаполнения.Свойство("ОформлениеСчетаФактурыПоОтгрузкеИзРабочегоМеста") Тогда
			
			ЗаполнитьКорректировкуПоДокументуОснованию(ДанныеЗаполнения);
			
		ИначеЕсли ДанныеЗаполнения.Свойство("ДокументОснование") Тогда
			
			КорректировочныйСохранение = ?(ДанныеЗаполнения.Свойство("Корректировочный"), ДанныеЗаполнения.Корректировочный, Ложь);
			
			ЗаполнитьПоДокументуОснованию(ДанныеЗаполнения);
			
			Если ДанныеЗаполнения.Свойство("ОформлениеСчетаФактурыПоОтгрузкеИзРабочегоМеста")
				И ДанныеЗаполнения.Свойство("Корректировочный") Тогда 
				ДанныеЗаполнения.Вставить("Корректировочный", КорректировочныйСохранение);
			КонецЕсли;
		...
		Если НЕ ДанныеЗаполнения.Свойство("Валюта") И ДанныеЗаполнения.Свойство("Организация") Тогда
			...
			Если ДанныеЗаполнения.Свойство("НачислениеНДСПриОтгрузке") И ДанныеЗаполнения.НачислениеНДСПриОтгрузке Тогда
				Если ДанныеЗаполнения.Свойство("Договор") И ЗначениеЗаполнено(ДанныеЗаполнения.Договор) Тогда
					РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения.Договор, "ВалютаВзаиморасчетов, ОплатаВВалюте", Истина);
					Если РеквизитыДоговора.ВалютаВзаиморасчетов <> ДанныеЗаполнения.Валюта
						И РеквизитыДоговора.ОплатаВВалюте = Истина Тогда 
						ДанныеЗаполнения.Вставить("Валюта", РеквизитыДоговора.ВалютаВзаиморасчетов);
					КонецЕсли;
				ИначеЕсли ДанныеЗаполнения.Свойство("ДокументОснование")
					И ЗначениеЗаполнено(ДанныеЗаполнения.ДокументОснование) Тогда 
					ДанныеЗаполнения.Вставить("Валюта", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения.ДокументОснование, "Валюта"));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		Если ДанныеЗаполнения.Свойство("ХозяйственнаяОперация") Тогда
			ДанныеЗаполнения.Вставить("НачислениеНДСПриОтгрузке", ДанныеЗаполнения.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтгрузкаТоваровКлиенту);
		КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не Отказ
		...
		И (Корректировочный Или Исправление Или НачислениеНДСПриОтгрузке) Тогда
	...
	ОбщегоНазначенияУТ.ЗаполнитьИдентификаторыДокумента(ЭтотОбъект, Метаданные().ТабличныеЧасти.Товары.Имя);
	...
	Если ЭтоНовый() И Не ЗначениеЗаполнено(Номер) 
		Или ТребуетсяУстановитьНовыйНомерПриИсправленииОтгрузки() Тогда
		УстановитьНовыйНомер();
	КонецЕсли;
	
	Если ЭтоНовый() И Не ЗначениеЗаполнено(ПредставлениеНомера) 
		Или Исправление 
		Или ТребуетсяУстановитьНовыйНомерПриИсправленииОтгрузки() Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Перевыставленный И НЕ Исправление Или ВводОстатков Или НДССМежценовойРазницы Или НачислениеНДСПриОтгрузке Тогда
		МассивНепроверяемыхРеквизитов.Добавить("Товары.ИсходныйСчетФактура");
	КонецЕсли;
	
	Если Не НачислениеНДСПриОтгрузке Тогда
	...
	Если НачислениеНДСПриОтгрузке Тогда 
		ПроверитьСуммуЗачтенныхАвансов(Отказ);
	КонецЕсли;
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура") И ДанныеЗаполнения.Свойство("Валюта") Тогда
		Валюта = ДанныеЗаполнения.Валюта;
...
Процедура ЗаполнитьПараметрыСчетаФактурыПоОснованию(ВыбранныйСчетФактура = Неопределено) Экспорт
	...
	Если Не ПараметрыСчетаФактуры.НачислениеНДСПриОтгрузке = Неопределено И Не ПараметрыСчетаФактуры.НачислениеНДСПриОтгрузке = НачислениеНДСПриОтгрузке Тогда
		НачислениеНДСПриОтгрузке = ПараметрыСчетаФактуры.НачислениеНДСПриОтгрузке;
		Если НачислениеНДСПриОтгрузке 
			И ЗначениеЗаполнено(Договор) Тогда
			РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Договор, "ВалютаВзаиморасчетов, ОплатаВВалюте");
...
Процедура ЗаполнитьПоДокументуОснованию(ДанныеЗаполнения)
	...
	НачислениеНДСПриОтгрузке = ?(ДанныеЗаполнения.Свойство("НачислениеНДСПриОтгрузке"), ДанныеЗаполнения.НачислениеНДСПриОтгрузке, Ложь);
	Если НачислениеНДСПриОтгрузке 
		И ДанныеЗаполнения.Свойство("Договор", Договор) Тогда
		РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Договор, "ВалютаВзаиморасчетов, ОплатаВВалюте");
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, РеквизитыДоговора);
		ДанныеЗаполнения.Вставить("СуммаБезНДСВал", ПараметрыСчетаФактуры.СуммаБезНДСВал);
	КонецЕсли;
	...
	Если Не НачислениеНДСПриОтгрузке
		...
		Документы.СчетФактураВыданный.ЗаполнитьПлатежноРасчетныеДокументы(
			...
			ПараметрыСчетаФактуры.Организация,
			Дата,
			Ложь);
		СтрокаПлатежноРасчетныеДокументы = Документы.СчетФактураВыданный.СтрокаРасчетноПлатежныхДокументов(ПлатежноРасчетныеДокументы);
		СтрокаАвансовыеСчетаФактуры = Документы.СчетФактураВыданный.СтрокаАвансовыеСчетаФактуры(ПлатежноРасчетныеДокументы);
	КонецЕсли;
...
Процедура ЗаполнитьИсправлениеПоОснованию(ДанныеЗаполнения)
	...
	НачислениеНДСПриОтгрузке  = РеквизитыСчетаФактуры.НачислениеНДСПриОтгрузке;
	...
	Если ЗначениеЗаполнено(РеквизитыСчетаФактуры.Ссылка) Тогда
		...
		Для Каждого РеквизитСФ Из РеквизитыСчетаФактуры Цикл
			...
			ИначеЕсли ИмяРеквизита = "ПлатежноРасчетныеДокументы"
				И РеквизитыСчетаФактуры.НачислениеНДСПриОтгрузке Тогда 
				ПлатежноРасчетныеДокументы.Загрузить(РеквизитСФ.Значение);
		...
		Если НЕ НачислениеНДСПриОтгрузке Тогда
			Документы.СчетФактураВыданный.ЗаполнитьПлатежноРасчетныеДокументы(
				...
				РеквизитыСчетаФактуры.Организация,
				ТекущаяДатаСеанса(),
				Ложь);
			СтрокаПлатежноРасчетныеДокументы = Документы.СчетФактураВыданный.СтрокаРасчетноПлатежныхДокументов(ПлатежноРасчетныеДокументы);
			СтрокаАвансовыеСчетаФактуры = Документы.СчетФактураВыданный.СтрокаАвансовыеСчетаФактуры(ПлатежноРасчетныеДокументы);
		...
		Если НачислениеНДСПриОтгрузке 
			И ЗначениеЗаполнено(РеквизитыСчетаФактуры.Договор) Тогда
			Договор = РеквизитыСчетаФактуры.Договор;
			РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РеквизитыСчетаФактуры.Договор, "ВалютаВзаиморасчетов, ОплатаВВалюте");
			ЗаполнитьЗначенияСвойств(ЭтотОбъект, РеквизитыДоговора);
			Если ПлатежноРасчетныеДокументы.Количество() > 0 Тогда
				Документы.СчетФактураВыданный.ЗаполнитьДанныеСФНаАвансПоОтгрузке(
					ПлатежноРасчетныеДокументы,
					РеквизитыСчетаФактуры.Организация,
					ТекущаяДатаСеанса());
			КонецЕсли;
...
Процедура ЗаполнитьИсправлениеПоСчетуФактуре(ДанныеЗаполнения = Неопределено) Экспорт
	...
	Для Каждого РеквизитСФ Из РеквизитыСчетаФактуры Цикл
		...
		ИначеЕсли ИмяРеквизита = "ПлатежноРасчетныеДокументы"
			И НачислениеНДСПриОтгрузке Тогда 
			ПлатежноРасчетныеДокументы.Загрузить(РеквизитСФ.Значение);
		ИначеЕсли ЗначениеЗаполнено(ЗначениеРеквизита) И Не ИмяРеквизита = "Период" Тогда
			Если ДанныеЗаполнения <> Неопределено Тогда
				ДанныеЗаполнения.Вставить(ИмяРеквизита, ЗначениеРеквизита);
			Иначе
	...
	Если Не НачислениеНДСПриОтгрузке Тогда
		Документы.СчетФактураВыданный.ЗаполнитьПлатежноРасчетныеДокументы(
			ПлатежноРасчетныеДокументы,
			РеквизитыСчетаФактуры.ДокументыОснования.ВыгрузитьКолонку("ДокументОснование"),
			РеквизитыСчетаФактуры.Организация,
			РеквизитыСчетаФактуры.Период,
			Ложь);
		СтрокаПлатежноРасчетныеДокументы = Документы.СчетФактураВыданный.СтрокаРасчетноПлатежныхДокументов(ПлатежноРасчетныеДокументы);
		СтрокаАвансовыеСчетаФактуры = Документы.СчетФактураВыданный.СтрокаАвансовыеСчетаФактуры(ПлатежноРасчетныеДокументы);
	КонецЕсли;
...
Функция ИсправляемыйСчетФактураПоОснованию(Ссылка, Основание, Организация = Неопределено)
	...
	|
	|ВЫБРАТЬ
	|	ИсправлениеОтгрузкиТоваров.ИсправляемыйДокумент КАК Ссылка
	|ИЗ
	|	Документ.ОтгрузкаТоваровКлиенту КАК ИсправлениеОтгрузкиТоваров
	|ГДЕ
	|	ИсправлениеОтгрузкиТоваров.Ссылка = &ДокументОснование
	|	И ИсправлениеОтгрузкиТоваров.Исправление
	|;
	|
	|////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СчетФактураВыданный.НачислениеНДСПриОтгрузке      КАК НачислениеНДСПриОтгрузке,
	...
	|	СчетФактураВыданный.ПлатежноРасчетныеДокументы.(
	...
	|		Сумма,
	|		СуммаРегл
	|	)                                                 КАК ПлатежноРасчетныеДокументы,
	|	
	...
	Если Выборка.Следующий() Тогда
		...
		ТаблицаПлатежноРасчетныхДокументов = Выборка.ПлатежноРасчетныеДокументы.Выгрузить();
		...
		РеквизитыСчетаФактуры.Вставить("ПлатежноРасчетныеДокументы", ТаблицаПлатежноРасчетныхДокументов);
...
Функция ДанныеСчетаФактуры(СчетФактура)
	...
	|	СчетФактураВыданный.Валюта                        КАК Валюта,
	|	СчетФактураВыданный.Организация                   КАК Организация,
	|	СчетФактураВыданный.Дата                          КАК Период,
	|	СчетФактураВыданный.Контрагент                    КАК Контрагент,
	...
	Если Выборка.Следующий() Тогда
		...
		СтруктураДанных.Вставить("ПлатежноРасчетныеДокументы", Выборка.ПлатежноРасчетныеДокументы.Выгрузить());
	КонецЕсли;
...
Функция КорректировочныеДокументыПоОснованию(Основание)
	...
	|
	|ВЫБРАТЬ
	|	ИсправлениеОтгрузки.Ссылка КАК Ссылка,
	|	ИсправлениеОтгрузки.Дата КАК Дата
	|ИЗ
	|	ВТ_СчетаФактурыИсходные КАК СчетаФактурыИсходные
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.ДокументыОснования КАК ОснованияСчетаФактуры
	|			ПО СчетаФактурыИсходные.Ссылка = ОснованияСчетаФактуры.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтгрузкаТоваровКлиенту КАК ИсправлениеОтгрузки
	|			ПО ОснованияСчетаФактуры.ДокументОснование = ИсправлениеОтгрузки.ИсправляемыйДокумент
	|			И ИсправляемыйДокумент.Исправление
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ТребуетсяОформлениеСчетаФактуры КАК ТребуетсяСчетФактура
	|			ПО (ИсправлениеОтгрузки.Ссылка = ТребуетсяСчетФактура.Основание)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата УБЫВ");
...
Функция ТаблицаТоваров(ДанныеЗаполнения = Неопределено) Экспорт
	...
	ЭтоНачислениеНДСПриОтгрузке = ?(ТипЗнч(ДанныеЗаполнения) = Тип("Структура") И ДанныеЗаполнения.Свойство("НачислениеНДСПриОтгрузке"),
		ДанныеЗаполнения.НачислениеНДСПриОтгрузке, НачислениеНДСПриОтгрузке);
	...
	ПараметрыОткрытия.Вставить("НачислениеНДСПриОтгрузке",    ЭтоНачислениеНДСПриОтгрузке);
	...
	ПараметрыЗаполнения.Вставить("ВалютаВзаиморасчетов", ВалютаВзаиморасчетов);
	...
	ПараметрыЗаполнения.Вставить("ПлатежноРасчетныеДокументы", ?(ТипЗнч(ДанныеЗаполнения) = Тип("Структура") И ДанныеЗаполнения.Свойство("ПлатежноРасчетныеДокументы"),
		ДанныеЗаполнения.ПлатежноРасчетныеДокументы.Скопировать(), ПлатежноРасчетныеДокументы.Выгрузить()));
...
Функция ПолучитьПараметрыСчетаФактурыПоОснованиям()
	...
	Результат = Новый Структура("Организация, Контрагент, КППКонтрагента, ИННКонтрагента,
		...
		|СчетФактураОснование, РеквизитыОснований, ИдентификаторГосКонтракта, Ответственный, РеализацияЧерезКомиссионера, СуммаБезНДСВал, НачислениеНДСПриОтгрузке");
	...
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДанныеОснований.Регистратор) = ТИП(Документ.ОтгрузкаТоваровКлиенту)
	|			ТОГДА ДанныеОснований.СуммаБезНДСВал
	|		ИНАЧЕ 0
	...
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК НачислениеНДСПриОтгрузке,
	...
	|	ЛОЖЬ КАК НачислениеНДСПриОтгрузке,
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТекущийДокумент.Ссылка                КАК Ссылка,
	|	ТекущийДокумент.ИсправляемыйДокумент  КАК ДокументОснование,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ИсправлениеОшибок)
	|ИЗ
	|	Документ.ОтгрузкаТоваровКлиенту КАК ТекущийДокумент
	|ГДЕ
	|	ТекущийДокумент.Ссылка В (&ДокументыОснования)
	|	И ТекущийДокумент.Исправление
	|;
	|
	|/////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура ПроверитьДублиСчетФактуры(Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ Исправление
			И НЕ ТипЗнч(Выборка.ДокументОснование) = Тип("ДокументСсылка.ОтчетКомитентуОЗакупках") 
			И НЕ ((Корректировочный Или Выборка.Ссылка.Корректировочный) И НачислениеНДСПриОтгрузке) Тогда
		...
		ИначеЕсли Исправление И СчетФактураОснование <> Выборка.Ссылка И Не НачислениеНДСПриОтгрузке Тогда
...
Процедура ПроверитьСчетФактурыПоОтчетуКомитентуОЗакупках(Отказ)
	...
	Если Не Отказ Тогда
		...
		Запрос = Новый Запрос();
		СчетФактураПолученныйОтПродавца = Товары.ВыгрузитьКолонку("СчетФактураПолученныйОтПродавца");
		СчетФактураПолученныйОтПродавцаНеОснование = ОбщегоНазначенияУТКлиентСервер.РазличияМассивов(СчетФактураПолученныйОтПродавца, ДокументыОснования.ВыгрузитьКолонку("СчетФактураПолученныйОтПродавца"));
		Запрос.УстановитьПараметр("СчетФактураПолученныйОтПродавца", СчетФактураПолученныйОтПродавца);
		Запрос.УстановитьПараметр("СписокДокументовОснований", ДокументыОснования.ВыгрузитьКолонку("ДокументОснование"));
		Запрос.УстановитьПараметр("СчетФактураПолученныйОтПродавцаНеОснование", СчетФактураПолученныйОтПродавцаНеОснование);
		...
		Запрос.Текст = "
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	СчетФактураПолученныйОснования.ИсходныйДокумент КАК ИсходныйДокумент,
			|	СчетФактураПолученныйОснования.Ссылка КАК СчетФактураПолученныйОтПродавца
			|ПОМЕСТИТЬ ИсходныеСФПолученные
			|ИЗ
			|	Документ.СчетФактураПолученный.ДокументыОснования КАК СчетФактураПолученныйОснования
			|ГДЕ
			|	СчетФактураПолученныйОснования.Ссылка В (&СчетФактураПолученныйОтПродавцаНеОснование)
			|	И СчетФактураПолученныйОснования.Ссылка.Проведен
			|;
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	СчетФактураВыданныйТовары.Ссылка КАК СчетФактураПеревыставленный,
			|	СчетФактураПолученныйОснования.СчетФактураПолученныйОтПродавца КАК СчетФактураПолученныйОтПродавца
			|ИЗ
			|	ИсходныеСФПолученные КАК СчетФактураПолученныйОснования
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.Товары КАК СчетФактураВыданныйТовары
			|		ПО СчетФактураВыданныйТовары.СчетФактураПолученныйОтПродавца = СчетФактураПолученныйОснования.ИсходныйДокумент
			|		И СчетФактураВыданныйТовары.Ссылка.Проведен";
		...
		Если Не Отказ Тогда
			ПеревыставленныеСФ = Результат[2].Выгрузить();
			ОтборПоСчетФактуреОтПродавца = Новый Структура;
			Для Каждого СчетФактураПолученный Из СчетФактураПолученныйОтПродавцаНеОснование Цикл
				ОтборПоСчетФактуреОтПродавца.Вставить("СчетФактураПолученныйОтПродавца", СчетФактураПолученный);
				ДанныеПоПеревыставленнымСФ = ПеревыставленныеСФ.НайтиСтроки(ОтборПоСчетФактуреОтПродавца);
				Если ДанныеПоПеревыставленнымСФ.Количество() = 0 Тогда
					ТекстОшибки = СтрШаблон(
						НСтр("ru = 'Нет перевыставленного счета-фактуры по ""%1""'"),
...
Функция ТребуетсяУстановитьНовыйНомерПриИсправленииОтгрузки()
	
	РеквизитыДоИзменения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "Исправление, Корректировочный");
	
	Возврат НачислениеНДСПриОтгрузке
		И Не ЭтоНовый()
		И (РеквизитыДоИзменения.Исправление <> Исправление
		Или РеквизитыДоИзменения.Корректировочный <> Корректировочный);
	
КонецФункции

Процедура ПроверитьСуммуЗачтенныхАвансов(Отказ)
	
	СуммаЗачтенныхАвансовВалютная = ПлатежноРасчетныеДокументы.Итог("Сумма");
	Если Корректировочный Тогда 
		СуммаПоСчетуФактуреВалютная = Товары.Итог("СуммаБезНДСВалУвеличение") - Товары.Итог("СуммаБезНДСВалУменьшение");
	Иначе
		СуммаПоСчетуФактуреВалютная = Товары.Итог("СуммаБезНДСВал");
	КонецЕсли;
	
	Если СуммаЗачтенныхАвансовВалютная > 0
		И СуммаЗачтенныхАвансовВалютная > СуммаПоСчетуФактуреВалютная Тогда 
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Сумма зачтенных авансов %1 %2 превышает сумму по счету-фактуре %3 %2.'", ОбщегоНазначения.КодОсновногоЯзыка()), СуммаЗачтенныхАвансовВалютная, ВалютаВзаиморасчетов, СуммаПоСчетуФактуреВалютная);
