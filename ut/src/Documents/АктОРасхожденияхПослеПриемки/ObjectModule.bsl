...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	КомиссионерВедетУчетПоРНПТ		= Справочники.СоглашенияСКлиентами.КомиссионерВедетУчетПоРНПТ(Соглашение);
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаПоИмпорту
		Или (Не КомиссионерВедетУчетПоРНПТ
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКонтрольКачестваТоваров")
		...
		Индексы = "ДокументОснование,ЗаказПоставщику,Склад";
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.АктКонтроляКачестваТоваров") Тогда
		...
		СоздаватьАктПослеПриемки = ТипЗнч(ДокументПартии) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг")
			Или ТипЗнч(ДокументПартии) = Тип("ДокументСсылка.ВозвратТоваровОтКлиента")
			Или ТипЗнч(ДокументПартии) = Тип("ДокументСсылка.ПоступлениеТоваровОтХранителя")
				И (ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратОтХранителя")
					Или ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратОтКомиссионера"))
			Или ТипЗнч(ДокументПартии) = Тип("ДокументСсылка.ПриемкаТоваровНаХранение")
				И ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПриемНаХранениеСПравомПродажи");
		СоздаватьАктПослеПриемки = СоздаватьАктПослеПриемки
			И ПолучитьФункциональнуюОпцию("ИспользоватьАктыРасхожденийПослеПриемки");
		
		СоздаватьАктПослеПриемкиОтПереработчика = Ложь;
		
		
		СоздаватьАктПослеПриемкиОтДавальца = Ложь;
		
		
		Если СоздаватьАктПослеПриемки
				Или СоздаватьАктПослеПриемкиОтПереработчика
				Или СоздаватьАктПослеПриемкиОтДавальца Тогда
				
				ЗаполненНаОснованииДокумента = Истина;
				ЗаполнитьНаОсновании(ДокументПартии, Ложь);
				// Поле вариант приемки используется в Документы.АктОРасхожденияхПослеПриемки.ЗаполнитьФакт.
				Распоряжение = ДокументОснованиеПриЗаполнении(ДокументПартии);
		...
		ИначеЕсли ТипЗнч(ДокументПартии) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		...
		ИначеЕсли ТипЗнч(ДокументПартии) = Тип("СправочникСсылка.СоглашенияСПоставщиками") Тогда
		...
		ИначеЕсли ТипЗнч(ДокументПартии) = Тип("ДокументСсылка.ЗаказПоставщику")
					Или ТипЗнч(ДокументПартии) = Тип("ДокументСсылка.ЗаказКлиента")
					
					
					
					Или ТипЗнч(ДокументПартии) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
		...
		ИначеЕсли ТипЗнч(ДокументПартии) = Тип("ДокументСсылка.ПоступлениеТоваровНаСклад") Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ПараметрыУказанияСерий = НоменклатураСервер.ПараметрыУказанияСерий(ЭтотОбъект, Документы.АктОРасхожденияхПослеПриемки);
	...
	ИначеЕсли ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратТоваровОтКлиента Тогда
		...
		ОписаниеМетаданных.ПоказательПроверки = "ОформленоПоВозвратам";
		
		РасхожденияСервер.ПроверкаВозможностиВнесенияИзлишкаВНакладную(ЭтотОбъект, "РаспоряженияНаОтгрузкуИВозврат", ОписаниеМетаданных, Отказ);
	КонецЕсли;
...
Процедура ЗаполнитьФактПоАктуКонтроляКачества(АктОснование)
	...
	КлючЗаполненияПроконтролироватьКачество = "Номенклатура,Склад";
	...
	Запрос.УстановитьПараметр("СкладыДокумента", Товары.Выгрузить(, "Склад").ВыгрузитьКолонку("Склад"));
	Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Склады = ПакетРезультатов[0].Выгрузить();
	ТоварыАктаКонтроляКачества = ПакетРезультатов[1].Выгрузить();
	
	РезультирующаяТаблица = Товары.Выгрузить(Новый Массив).СкопироватьКолонки();
	РезультирующаяТаблицаСерии = РезультирующаяТаблица.СкопироватьКолонки();
	
	Для Каждого СтрокаТаблицы Из Склады Цикл
	...
	ТекущаяТаблица = РезультирующаяТаблица;
	...
	ТекущаяТаблица = РезультирующаяТаблицаСерии;
	Для Индекс = 0 По ТекущаяТаблица.Количество() - 1 Цикл
		СтрокаТаблицы = ТекущаяТаблица[Индекс];
		ЗаполнитьЗначенияСвойств(КлючПоиска, СтрокаТаблицы);
		Если ТоварыДокумента.НайтиСтроки(КлючПоиска).Количество() = 0 Тогда
	...
	ПараметрыУказанияСерий = Документы.АктОРасхожденияхПослеПриемки.ПараметрыУказанияСерий(ЭтотОбъект);
	НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(ЭтотОбъект, ПараметрыУказанияСерий);
	НоменклатураСервер.ОчиститьНеиспользуемыеСерии(ЭтотОбъект, ПараметрыУказанияСерий);
	Документы.АктОРасхожденияхПослеПриемки.ОбновитьЗависимыеРеквизитыПослеЗаполненияФактаПоПриемке(ЭтотОбъект);
...
Функция ТекстЗапросаПоОснованиюПоступленияОтХранителя()
	...
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ВЫБОР
	|		КОГДА ТоварыПоступления.Порча
	|			ТОГДА ТоварыПоступления.НоменклатураОприходование
	|		ИНАЧЕ ТоварыПоступления.Номенклатура
	|	КОНЕЦ КАК Номенклатура,
	|	ВЫБОР
	|		КОГДА ТоварыПоступления.Порча
	|			ТОГДА ТоварыПоступления.ХарактеристикаОприходование
	|		ИНАЧЕ ТоварыПоступления.Характеристика
	...
	|	ТоварыПоступления.Распоряжение        КАК ЗаказПоставщику,
	...
	|
	|ИМЕЮЩИЕ
	|	МАКСИМУМ(ТоварыПоступления.СтатусУказанияСерий) В(2, 4, 6, 8, 10)";
