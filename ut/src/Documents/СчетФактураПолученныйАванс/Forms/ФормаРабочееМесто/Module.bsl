...
Процедура ЗаполнитьВыданныеАвансы()
	...
	ДополнительныеПараметры = Документы.СчетФактураПолученныйАванс.ДополнительныеПараметрыАвансовВыданных();
	ДополнительныеПараметры.ИсключитьИУДА = Истина;
	ДополнительныеПараметры.ВРазрезеНП = Ложь;
	
	Результат = Документы.СчетФактураПолученныйАванс.ПолучитьВыданныеАвансы(Организация, НачалоПериода, КонецПериода, ДополнительныеПараметры);
	...
	Для Каждого Строка Из ВыданныеАвансы Цикл
		Если Строка.СчетФактура <> Документы.СчетФактураПолученныйАванс.ПустаяСсылка()
				И Строка.СуммаАвансаРегл <> Строка.СуммаСчетаФактуры
				И НЕ Строка.Исправление Тогда
...
Процедура УстановитьАктуальностьГраницыРасчетов()
	...
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		...
		НачалоРасчета = РегистрыСведений.ЗаданияКРаспределениюВзаиморасчетов.АктуальныйПериодРассчитанныхДанныхПоВзаиморасчетам(
			АналитикиРасчета.АналитикиУчетаПоПартнерам, , ПериодРасчета);
		УстановитьПривилегированныйРежим(Ложь);
		
		Если НачалоРасчета = Неопределено ИЛИ НачалоРасчета > ПериодРасчета Тогда
			АктуальностьГраницыРасчетов = Истина;
		Иначе
...
Процедура АктуализироватьРасчетыСервер()
	...
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		...
		ПоляОтбора.Вставить("Партнер", Неопределено);
		ПоляОтбора.Вставить("Контрагент", Неопределено);
		ПоляОтбора.Вставить("Договор", Неопределено);
		АналитикиУчетаПоПартнерам = РаспределениеВзаиморасчетовВызовСервера.МассивКлючейПартнеровПоОтбору(ПоляОтбора);

		ДокументыКАктуализацииВзаиморасчетов = РегистрыСведений.ЗаданияКРаспределениюВзаиморасчетов.ДокументыКРаспределениюВзаиморасчетовПоСрокам(
			АналитикиУчетаПоПартнерам, , КонецМесяца(КонецПериода));
		
		НеоперативнаяОчередьЗаданий.ВыполнитьЗаданияНеоперативнойОчередиПоСпискуДокументов(
			ДокументыКАктуализацииВзаиморасчетов, Справочники.ВидыНеоперативныхЗаданий.РаспределениеВзаиморасчетовПоСрокам);
