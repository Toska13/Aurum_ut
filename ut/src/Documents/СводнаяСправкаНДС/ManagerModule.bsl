...
Процедура ДополнитьТаблицуПродажПоИнтеркампани(ТаблицаПродаж, ПараметрыРасчета)
	
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ТаблицаПродаж.Сумма КАК Сумма,
	|	ТаблицаПродаж.СуммаНДС КАК СуммаНДС,
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		ОтчетПоКомиссииМеждуОрганизациями.Ссылка.Организация КАК Организация,
	|		НАЧАЛОПЕРИОДА(ОтчетПоКомиссииМеждуОрганизациями.Ссылка.Дата, МЕСЯЦ) КАК Период,
	|		ОтчетПоКомиссииМеждуОрганизациями.АналитикаУчетаНоменклатуры.Номенклатура.ТипНоменклатуры КАК ТипНоменклатуры,
	|		ОтчетПоКомиссииМеждуОрганизациями.СтавкаНДС КАК СтавкаНДС,
	|		ЕСТЬNULL(СуммыДокументовВВалютеРегл.БазаНДСРегл, ОтчетПоКомиссииМеждуОрганизациями.СуммаСНДС - ОтчетПоКомиссииМеждуОрганизациями.СуммаНДС) КАК Сумма,
	|		ЕСТЬNULL(СуммыДокументовВВалютеРегл.СуммаНДСРегл, ОтчетПоКомиссииМеждуОрганизациями.СуммаНДС) КАК СуммаНДС
	|	ИЗ
	|		Документ.ОтчетПоКомиссииМеждуОрганизациями.ВидыЗапасов КАК ОтчетПоКомиссииМеждуОрганизациями
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютахУчета КАК СуммыДокументовВВалютеРегл
	|			ПО (СуммыДокументовВВалютеРегл.Регистратор = ОтчетПоКомиссииМеждуОрганизациями.Ссылка)
	|				И (СуммыДокументовВВалютеРегл.ИдентификаторСтроки = ОтчетПоКомиссииМеждуОрганизациями.ИдентификаторСтроки)
	|	ГДЕ
	|		ОтчетПоКомиссииМеждуОрганизациями.Ссылка.Организация В(&МассивОрганизаций)
	|		И ОтчетПоКомиссииМеждуОрганизациями.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|		И ОтчетПоКомиссииМеждуОрганизациями.Ссылка.Проведен
	|		И (ОтчетПоКомиссииМеждуОрганизациями.Покупатель.ЮрФизЛицо В (ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ФизЛицо))
	|				ИЛИ ОтчетПоКомиссииМеждуОрганизациями.Покупатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка))
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ОтчетПоКомиссииМеждуОрганизациями.Ссылка.Организация,
	|		НАЧАЛОПЕРИОДА(ОтчетПоКомиссииМеждуОрганизациями.Ссылка.Дата, МЕСЯЦ),
	|		ОтчетПоКомиссииМеждуОрганизациями.АналитикаУчетаНоменклатуры.Номенклатура.ТипНоменклатуры,
	|		ОтчетПоКомиссииМеждуОрганизациями.СтавкаНДС,
	|		ЕСТЬNULL(СуммыДокументовВВалютеРегл.БазаНДСРегл, ОтчетПоКомиссииМеждуОрганизациями.СуммаСНДС - ОтчетПоКомиссииМеждуОрганизациями.СуммаНДС),
	|		ЕСТЬNULL(СуммыДокументовВВалютеРегл.СуммаНДСРегл, ОтчетПоКомиссииМеждуОрганизациями.СуммаНДС)
	|	ИЗ
	|		Документ.ОтчетПоКомиссииМеждуОрганизациями.Товары КАК ОтчетПоКомиссииМеждуОрганизациями
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютахУчета КАК СуммыДокументовВВалютеРегл
	|			ПО (СуммыДокументовВВалютеРегл.Регистратор = ОтчетПоКомиссииМеждуОрганизациями.Ссылка)
	|				И (СуммыДокументовВВалютеРегл.ИдентификаторСтроки = ОтчетПоКомиссииМеждуОрганизациями.ИдентификаторСтроки)
	|	ГДЕ
	...
	|		И ОтчетПоКомиссииМеждуОрганизациями.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|		И ОтчетПоКомиссииМеждуОрганизациями.Ссылка.Проведен
	|		И ОтчетПоКомиссииМеждуОрганизациями.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга))
	|		И (ОтчетПоКомиссииМеждуОрганизациями.Покупатель.ЮрФизЛицо В (ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ФизЛицо))
	|				ИЛИ ОтчетПоКомиссииМеждуОрганизациями.Покупатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка))
	|		И (ЕСТЬNULL(ОтчетПоКомиссииМеждуОрганизациями.Характеристика.Принципал, ОтчетПоКомиссииМеждуОрганизациями.Номенклатура.Принципал) = НЕОПРЕДЕЛЕНО
	|				ИЛИ ЕСТЬNULL(ОтчетПоКомиссииМеждуОрганизациями.Характеристика.Принципал, ОтчетПоКомиссииМеждуОрганизациями.Номенклатура.Принципал) = ОтчетПоКомиссииМеждуОрганизациями.Ссылка.Организация)) КАК РозничныеПродажи
	|
	|СГРУППИРОВАТЬ ПО
	...
	|
	|ВЫБРАТЬ
	|	ТаблицаПродажПредварительная.Организация,
	|	ТаблицаПродажПредварительная.Период,
	|	ТаблицаПродажПредварительная.ТипНоменклатуры,
	|	ТаблицаПродажПредварительная.СтавкаНДС,
	|	ТаблицаПродажПредварительная.СуммаНДС,
	|	ТаблицаПродажПредварительная.Сумма
	|ИЗ
	|	ТаблицаПродажПредварительная КАК ТаблицаПродажПредварительная";
...
Процедура СформироватьТаблицуПродаж(Запрос, ПараметрыРасчета, ВидСправок)
	...
	Если ВидСправок = "Обычные" Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПередачиТоваровМеждуОрганизациями")
			...
			ДополнитьТаблицуПродажПоИнтеркампани(ТаблицаПродаж, ПараметрыРасчета);
		КонецЕсли;
