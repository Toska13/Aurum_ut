...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	СтруктураСкладДляФО = Новый Структура;
	СтруктураСкладДляФО.Вставить("Склад", Склад);
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьСтатусыРасходныхОрдеров", СтруктураСкладДляФО) Тогда
	...
	ИспользуетсяОтгрузкаПослеЗадания = ПолучитьФункциональнуюОпцию(
		"НачинатьОтгрузкуПослеФормированияЗаданияНаПеревозку",
		СтруктураСкладДляФО);
	Если Не ИспользуетсяОтгрузкаПослеЗадания
		И ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25() Тогда
		
		ОтгрузкаПоЗаданиюНаПеревозку = Ложь;
		ЗаданиеНаПеревозку = Неопределено;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ЭтоОтгрузкаПоДоставке25 = 
		(ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25()
			И ИспользуетсяОтгрузкаПослеЗадания
			И ОтгрузкаПоЗаданиюНаПеревозку);
	ЭтоОтгрузкаПоДоставке20 =
		(Не ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25()
			И ОтгрузкаПоЗаданиюНаПеревозку);
	
	Если ЭтоОтгрузкаПоДоставке25
		Или ЭтоОтгрузкаПоДоставке20 Тогда
		
		Если Не ДоставкаТоваров.ЕстьДоставкаПоСкладскойОперации(СкладскаяОперация) Тогда
			
			ШаблонСообщения = НСтр("ru = 'Для складской операции %1 доставка не предусмотрена'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонСообщения,
				Строка(СкладскаяОперация));
		...
		ИначеЕсли Не ЗначениеЗаполнено(ЗаданиеНаПеревозку)
			...
			Если ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25() Тогда
				ИмяПоля = НСтр("ru = 'Рейс'");
			Иначе
				ИмяПоля = НСтр("ru = 'Задание на перевозку'");
			КонецЕсли;
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, ИмяПоля);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ТекстСообщения, ЭтотОбъект, "ЗаданиеНаПеревозку", "Объект", Отказ);
			
		ИначеЕсли Статус <> Перечисления.СтатусыРасходныхОрдеров.Отгружен
			И ЗначениеЗаполнено(ЗаданиеНаПеревозку) Тогда
			
			Если ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25() Тогда
				
				СтатусРейса = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаданиеНаПеревозку, "Статус");
				
				Если СтатусРейса = Перечисления.СтатусыРейсов.Отправлен
					Или СтатусРейса = Перечисления.СтатусыРейсов.Завершен
					Или СтатусРейса = Перечисления.СтатусыРейсов.Отменен Тогда
					
					ШаблонСообщения = НСтр("ru = 'Выбранный рейс находится в статусе ""%1"", поэтому его нельзя указать в ордере, который находится в статусе ""%2"". Измените статус ордера на ""%3"".'");
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						ШаблонСообщения,
						Строка(СтатусРейса),
						Строка(Статус),
						Строка(Перечисления.СтатусыРасходныхОрдеров.Отгружен));
					
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						ТекстСообщения, ЭтотОбъект, "Статус", "Объект", Отказ);
			...
			Иначе
				...
				Если СтатусЗаданияНаПеревозку = Перечисления.СтатусыЗаданийНаПеревозку.Отправлено
					...
					ШаблонСообщения = НСтр("ru = 'Выбранное задание на перевозку находится в статусе ""%1"", поэтому его нельзя указать в ордере, который находится в статусе ""%2"". Измените статус ордера на ""%3"".'");
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						ШаблонСообщения,
						Строка(СтатусЗаданияНаПеревозку),
						Строка(Статус),
						Строка(Перечисления.СтатусыРасходныхОрдеров.Отгружен));
		...
		Если ДоставкаТоваровКлиентСервер.ИспользуетсяДоставка25() Тогда
			...
			ШаблонТипНеПоддерживается = НСтр("ru = 'Тип распоряжения ""%1"" не поддерживается для оформления доставки.'");
			ИмяКолонки = Метаданные.Документы.РасходныйОрдерНаТовары.ТабличныеЧасти.ТоварыПоРаспоряжениям.Реквизиты.Распоряжение.Синоним;
			ИмяСписка = Метаданные.Документы.РасходныйОрдерНаТовары.ТабличныеЧасти.ТоварыПоРаспоряжениям.Синоним;
			
			ТипыРаспоряжениеНаДоставку = Метаданные.ОпределяемыеТипы.РаспоряжениеНаДоставку25.Тип.Типы();
			Для каждого СтрокаСРаспоряжением Из ТоварыПоРаспоряжениям Цикл
				
				Если СтрокаСРаспоряжением.Распоряжение.Пустая() Тогда
					Продолжить;
				КонецЕсли;
				
				ТипРаспоряжения = ТипЗнч(СтрокаСРаспоряжением.Распоряжение);
				Если ТипыРаспоряжениеНаДоставку.Найти(ТипРаспоряжения) = Неопределено Тогда
					
					ТекстТипНеПоддерживается = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						ШаблонТипНеПоддерживается,
						Строка(ТипРаспоряжения));
						
					ТекстОшибкиТипРаспоряжения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
						"Колонка",
						"Корректность",
						ИмяКолонки,
						СтрокаСРаспоряжением.НомерСтроки,
						ИмяСписка,
						ТекстТипНеПоддерживается);
						
					Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
						"ТоварыПоРаспоряжениям", СтрокаСРаспоряжением.НомерСтроки, "Распоряжение");
					
					ОбщегоНазначения.СообщитьПользователю(ТекстОшибкиТипРаспоряжения, ЭтотОбъект, Поле,, Отказ);
	...
	Иначе
		...
		Иначе
			...
			ПараметрыПроверки.ИноеПредставлениеУпаковка = НСтр("ru = 'Ед. изм.'");
