...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, Параметры,
		"Номенклатура,ДатаПринятияНаУчет,Организация,КБК,КатегорияПодакцизныхТоваров,МаркируемаяПродукция,
		|НалоговаяБазаАкцизы,ТвердаяСтавкаАкциза,АдвалорнаяСтавкаАкциза,ЕдиницаИзмеренияАкциза,
		|СуммаАкциза, КоэффициентПересчета, ЕстьУчетПодакцизныхТоваров");
	
	
	Если ЕстьУчетПодакцизныхТоваров Тогда
		Если ЕдиницаИзмеренияАкциза <> "" Тогда
			НаименованиеЕдиницыИзмеренияАкциза = Справочники.УпаковкиЕдиницыИзмерения.НайтиПоКоду(ЕдиницаИзмеренияАкциза);
			Элементы.ЕдиницаИзмеренияАкциза.СписокВыбора.Очистить();
			Элементы.ЕдиницаИзмеренияАкциза.СписокВыбора.Добавить(ЕдиницаИзмеренияАкциза, НаименованиеЕдиницыИзмеренияАкциза);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если ПеренестиВДокумент Тогда
		СтруктураРезультат = Новый Структура(
			"НалоговаяБазаАкцизы,ТвердаяСтавкаАкциза,АдвалорнаяСтавкаАкциза,
			|ЕдиницаИзмеренияАкциза,СуммаАкциза,КБК,КодВидаПодакцизногоТовара,МаркируемаяПродукция");
...
&НаКлиенте
Процедура КатегорияПодакцизныхТоваровПриИзменении(Элемент)
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ГруппаДОПП.Видимость = Форма.ВидАкциза <> 0;
	...
	ИначеЕсли Форма.ВидАкциза = 1 Тогда
		...
		ИначеЕсли Форма.ЕстьУчетПодакцизныхТоваров Тогда 
			ТекстСообщения = НСтр("ru='за %1'");
			Форма.ЕдиницаСтавки  = СтрШаблон(ТекстСообщения,
				Форма.НаименованиеЕдиницыИзмеренияАкциза);
		КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "КБК");
	...
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "КатегорияПодакцизныхТоваров");
...
Функция ПроверитьЗаполнениеНаКлиенте()
	...
	Если Не ЗначениеЗаполнено(КБК) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
			"Поле", "Заполнение", НСтр("ru = 'КБК'"));
		Поле = "КБК";
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , Поле, "", Отказ);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(КатегорияПодакцизныхТоваров) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
			"Поле", "Заполнение", НСтр("ru = 'Категория подакцизного товара'"));
		Поле = "КатегорияПодакцизныхТоваров";
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , Поле, "", Отказ);
	КонецЕсли;
...
Процедура ОпределитьНалоговуюБазуПоУмолчанию(Параметры)
	...
	Если Параметры.ВалютаДокумента <> Параметры.ВалютаРеглУчета Тогда
		ДатаДляРасчетаКурса = Параметры.ДатаПринятияНаУчет - 86400;
		СтруктураКурса = РаботаСКурсамиВалютУТ.ПолучитьКурсВалюты(Параметры.ВалютаДокумента, ДатаДляРасчетаКурса);
		Если СтруктураКурса.КурсЗнаменатель = 0 Тогда
			СтруктураКурса.КурсЗнаменатель = 1;
		КонецЕсли;
		ФактурнаяСтоимостьРуб = 
			Окр((Параметры.ФактурнаяСтоимость * (СтруктураКурса.КурсЧислитель/СтруктураКурса.КурсЗнаменатель)), 2);
	Иначе
