...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, ЭтоГруппа, Параметр)
	...
	Если ТекущиеДанные <> Неопределено
		...
		ОткрытьФорму("Документ.ОтчетИСМП.ФормаОбъекта", СтруктураПараметров);
...
&НаКлиенте
Процедура СоздатьОтчетИСМПСведенияОбОтклонениях(Команда)
	СоздатьДокумент(Команда.Имя);
КонецПроцедуры

&НаКлиенте
Процедура СоздатьОтчетИСМПСписокКИНаБалансе(Команда)
	СоздатьДокумент(Команда.Имя);
КонецПроцедуры

&НаКлиенте
Процедура СоздатьОтчетИСМПОстаткиВиртуальногоСклада(Команда)
	СоздатьДокумент(Команда.Имя);
КонецПроцедуры
...
Процедура ОбработатьИПроверитьПереданныеПараметры()
	...
	Если РежимОткрытияПоВидуОперации = ИнтеграцияИСМПКлиентСервер.РежимОткрытияСведенияОбОтклонениях() Тогда
		УстановитьОтборПоВидуОперации(Истина);
		Заголовок = Нстр("ru = 'Сведения об отклонениях'");
	Иначе
		УстановитьОтборПоВидуОперации(Ложь);
		Заголовок = Нстр("ru = 'Остатки кодов маркировки'");
		Элементы.СписокПодтвердитеОбработкуОтклонений.Видимость = Ложь;
	КонецЕсли;
...
&НаСервере
Процедура УстановитьОтборПоВидуОперации(ЭтоСведенияОбОтклонениях)
	
	Если ЭтоСведенияОбОтклонениях Тогда
		ВидСравненияОтобора = ВидСравненияКомпоновкиДанных.Равно;
	Иначе
		ВидСравненияОтобора = ВидСравненияКомпоновкиДанных.НеРавно;
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		Список, "Операция", Перечисления.ВидыОперацийИСМП.ОтчетИСМПСведенияОбОтклонениях, ВидСравненияОтобора,, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура СоздатьДокумент(ИмяКоманды, ПараметрыСоздания = Неопределено)
...
Процедура СоздатьЭлементыФормы()
	...
	Если РежимОткрытияПоВидуОперации = ИнтеграцияИСМПКлиентСервер.РежимОткрытияСведенияОбОтклонениях() Тогда
		...
		СобытияФормИСМП.ДобавитьЭлементФормыКнопка(ЭтотОбъект, "ОтчетИСМПСведенияОбОтклонениях", Элементы.ГруппаСоздать, Нстр("ru = 'Создать'"), Истина);
		
	Иначе
		
		УчитываемыеВидыМаркируемойПродукции = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.УчитываемыеВидыМаркируемойПродукции();
		ДоступенЗапросОстатковВиртуальногоСклада = Ложь;
		Для Каждого ВидПродукции Из УчитываемыеВидыМаркируемойПродукции Цикл
			Если ОбщегоНазначенияИСКлиентСерверПовтИсп.ВидПродукцииИспользуетОбъемноСортовойУчет(ВидПродукции) Тогда
				ДоступенЗапросОстатковВиртуальногоСклада = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		Если ДоступенЗапросОстатковВиртуальногоСклада Тогда
			
			ДобавляемыеКоманды = Новый Массив;
			СобытияФормИСМП.ДобавитьЭлементДляСоздания(ДобавляемыеКоманды, "ОтчетИСМПСписокКИНаБалансе");
			СобытияФормИСМП.ДобавитьЭлементДляСоздания(ДобавляемыеКоманды, "ОтчетИСМПОстаткиВиртуальногоСклада");
			
			ЭлементРодитель = СобытияФормИСМП.ДобавитьЭлементФормыПодменю(
				ЭтотОбъект, "Создать", Элементы.ГруппаСоздать, Нстр("ru = 'Создать'"), Истина);
		...
		Иначе
			...
			СобытияФормИСМП.ДобавитьЭлементФормыКнопка(ЭтотОбъект, "ОтчетИСМПСписокКИНаБалансе", Элементы.ГруппаСоздать, Нстр("ru = 'Создать'"), Истина);
			РежимОткрытияПоВидуОперации = ИнтеграцияИСМПКлиентСервер.РежимОткрытияСписокКИНаБалансе();
