...
Процедура ЗаполнитьТоварыПоАктуОРасхождениях(Объект) Экспорт
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(ОбрабатываемыеСтроки, СтруктураДействий, Объект.Товары, КэшированныеЗначения);
...
// 
// Параметры:
//  ОбъектИлиСсылка - ДокументОбъект.ПриходныйОрдерНаТовары, ДокументСсылка.ПриходныйОрдерНаТовары - документ.
...
Функция РеквизитыКонтролируемойПартии(ОбъектИлиСсылка, ИмяПоляКонтролируемаяПартия) Экспорт
	...
	МенеджерДокумента = ОбщегоНазначения.МенеджерОбъектаПоСсылке(РеквизитыОбъекта.ДокументОснование);
		Реквизиты = МенеджерДокумента.РеквизитыКонтролируемойПартии(
			РеквизитыОбъекта.ДокументОснование,
			ИмяПоляКонтролируемаяПартия);
...
// 
// Параметры:
//  Объект - ДокументОбъект.КорректировкаПриобретения - объект.
// 
// Возвращаемое значение:
...
Функция КонтрольКачестваПараметрыДействий(Объект) Экспорт
	...
	КонтрольКачестваСбросить = Объект.ТипКорректировки <> Перечисления.ТипыКорректировки.ОбычнаяКорректировка
			Или Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.КорректировкаОтчетаПереработчика;
	...
	ПараметрыДействия.ЧитаемыеПоля = "Номенклатура,Склад,СписатьНаРасходы,ВариантОтражения";
	...
	ПараметрыДействия.ПараметрыЗапроса.Вставить("КонтрольКачестваСбросить", КонтрольКачестваСбросить);
	...
	ПараметрыДействия.ТекстЗапросаДанныеДокумента =
		...
		|	ЛОЖЬ КАК Установить,
		|	Таблица.СписатьНаРасходы
		|	ИЛИ Таблица.ВариантОтражения В(
		...
		|		ЗНАЧЕНИЕ(Перечисление.ВариантыОтраженияКорректировокПоступлений.УвеличитьСтоимостьТовара)) КАК Сбросить
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ЕСТЬNULL(ДанныеДокумента.ДокументОснование.Дата, ДАТАВРЕМЯ(1,1,1))                  КАК ПериодКорректировки,
	...
	Запрос.УстановитьПараметр("ДоходыОтразитьВТекущемПериоде", Запрос.Параметры.СтатьяДоходов = ПланыВидовХарактеристик.СтатьиДоходов.ПрибыльУбытокПрошлыхЛет);
	Запрос.УстановитьПараметр("РасходыОтразитьВТекущемПериоде", Запрос.Параметры.СтатьяРасходов = ПланыВидовХарактеристик.СтатьиРасходов.ПрибыльУбытокПрошлыхЛет);
...
Процедура ОформитьИзлишкиНедостачиНаСкладе(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаРасхождения.ПроконтролироватьКачество
	|			ТОГДА ТаблицаРасхождения.КонтролируемаяПартия
	|		ИНАЧЕ
	...
	|	)
	|	И (ТаблицаРасхождения.Ссылка.ДокументОснование ССЫЛКА Документ.ПервичныйДокумент
	|		ИЛИ НЕ ТаблицаРасхождения.Ссылка.ДокументОснование.ХозяйственнаяОперация В (
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаТоварыВПути),
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки),
...
Процедура ИнициализироватьКлючиАналитикиУчетаНоменклатуры(Запрос) Экспорт
	...
	|		ВЫБОР
	|			КОГДА ПрослеживаемыеТовары.СписатьНаРасходы
	|				ТОГДА ПрослеживаемыеТовары.Подразделение
	|			ИНАЧЕ ПрослеживаемыеТовары.Склад
	|		КОНЕЦ КАК Склад,
	|		ВЫБОР
	|			КОГДА ИСТИНА
...
Функция ТекстЗапросаВтТаблицаРасхожденияТоваров(Запрос, ТекстыЗапроса) Экспорт
	...
	|	ЕСТЬNULL(ТаблицаРасхождения.Номенклатура.ПрослеживаемыйТовар, ЛОЖЬ) КАК ПрослеживаемыйТовар,
	|
	|
	...
	|	ВЫБОР
	|		КОГДА ТаблицаРасхождения.Количество = 0 И &СреднеСкользящая
	|		 И ТаблицаРасхождения.ВидЗапасов.ТипЗапасов <> ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.СобственныйТоварПоНеотфактурованнойПоставке)
	|			ТОГДА 0
	...
	|	ВЫБОР
	// При среднескользящей суммовые корректировки любого периода отражаются в регистре "Отклонения в стоимости товаров" через поля доп расходв,
	// кроме корректировки стоимости фактуровки поставки.
	|		КОГДА &СреднеСкользящая И ТаблицаРасхождения.Количество = 0
	|		 И ТаблицаРасхождения.ВидЗапасов.ТипЗапасов <> ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.СобственныйТоварПоНеотфактурованнойПоставке)
	|			ТОГДА ВЫБОР
	|				КОГДА ЕСТЬNULL(Суммы.СуммаПриведеннаяРегл, 0) <> 0
	|					ТОГДА Суммы.СуммаПриведеннаяРегл
	...
	|	ЕСТЬNULL(ТаблицаРасхождения.АналитикаУчетаНоменклатуры.Номенклатура.ПрослеживаемыйТовар, ЛОЖЬ) КАК ПрослеживаемыйТовар,
	|
	|
	|	ТаблицаРасхождения.СписатьНаРасходы КАК СписатьНаРасходы,
	|	ВЫБОР 
	...
	|				И ТаблицаРасхождения.АналитикаУчетаНоменклатуры.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))
	|			ТОГДА 0
	|		  ИНАЧЕ ВЫБОР
	|				КОГДА ЕСТЬNULL(Суммы.СуммаПриведеннаяРегл, 0) <> 0
	|					ТОГДА Суммы.СуммаПриведеннаяРегл
...
Функция ТекстЗапросаТаблицаТоварыОрганизаций(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|	СУММА(ВтТоварыОрганизаций.Количество) КАК Количество,
	...
	|              
	|СГРУППИРОВАТЬ ПО
	|	ВтТоварыОрганизаций.НомерСтроки,
	|	ВтТоварыОрганизаций.ВидДвижения,
	|	ВтТоварыОрганизаций.Период,
	|	ВтТоварыОрганизаций.Организация,
	|	ВтТоварыОрганизаций.ОрганизацияОтгрузки,
	...
	|	ВтТоварыОрганизаций.ХозяйственнаяОперация,
	|	ВтТоварыОрганизаций.НалогообложениеНДС,
	|	ВтТоварыОрганизаций.Первичное
	|
	|ИМЕЮЩИЕ
	|	СУММА(ВтТоварыОрганизаций.Количество) <> 0
	|	ИЛИ СУММА(ВтТоварыОрганизаций.КоличествоПоРНПТ) <> 0
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
...
Функция ТекстЗапросаТаблицаПрочиеДоходы(Запрос, ТекстыЗапроса, Регистры)
	...
	|				ИНАЧЕ &СтатьяДоходов
	|			КОНЕЦ) = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиДоходов.ПрибыльУбытокПрошлыхЛет)
	|		ТОГДА ДАТАВРЕМЯ(1,1,1)
	|		ИНАЧЕ &ПериодКорректировки
	...
	|	&Организация КАК Организация,
	|	ВЫБОР
	|		КОГДА &ДоходыОтразитьВТекущемПериоде
	|		ТОГДА ДАТАВРЕМЯ(1,1,1)
	|		ИНАЧЕ &ПериодКорректировки
	|	КОНЕЦ КАК ПериодКорректировки,
...
Функция ТекстЗапросаТаблицаВтИсходныеПрочиеРасходы(Запрос, ТекстыЗапроса)
	...
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)			КАК ВидДвижения,
	|	ВЫБОР
	|		КОГДА СтатьиРасходов.Ссылка = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПрибыльУбытокПрошлыхЛет)
	|		ТОГДА ДАТАВРЕМЯ(1,1,1)
	|		ИНАЧЕ &ПериодКорректировки
	...
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	|	ВЫБОР
	|		КОГДА &РасходыОтразитьВТекущемПериоде
	|		ТОГДА ДАТАВРЕМЯ(1,1,1)
	|		ИНАЧЕ &ПериодКорректировки
	|	КОНЕЦ КАК ПериодОтклонения,
	|	&Организация КАК Организация,
...
Процедура ОтразитьВвозТоваровИзТаможенногоСоюза(Запрос, ТекстыЗапроса, Регистры)
	...
	Если ПроведениеДокументов.ТребуетсяТаблицаДляДвижений("ТоварыКОформлениюДокументовИмпорта", Регистры) Тогда 
		...
		Если Запрос.Параметры.ХозОперацииЗакупкаВСтранахЕАЭС.Найти(Запрос.Параметры.ХозОперацияОснования) <> Неопределено Тогда 
			...
			КорректировкаПриобретенияЛокализация.ДополнитьТекстЗапросаТаблицаТоварыКОформлениюДокументовИмпорта(ТекстТовары);
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры)
	...
	ВзаиморасчетыПроведениеПоставщики.ПроведениеОплатыПоставщику(Запрос, ТекстыЗапроса, Регистры, ТекстОплата,"3");
...
Функция ТекстЗапросаТаблицаТоварыУслугиКОформлениюОтчетовКомитентуОЗакупках(Запрос, ТекстыЗапроса, Регистры)
	...
	|			ТОГДА ТаблицаРасхождения.СтавкаНДС
	...
	|ПО
	|	ТаблицаРасхождения.Назначение = Назначения.Ссылка
	|ГДЕ
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|ГДЕ
		|	ТабЧасть.ВариантОтражения = ЗНАЧЕНИЕ(Перечисление.ВариантыОтраженияКорректировокПоступлений.УменьшитьЗакупкуУменьшитьСкладскиеОстатки)
		|		ИЛИ ТабЧасть.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
		|			И ТабЧасть.ВариантОтражения = ЗНАЧЕНИЕ(Перечисление.ВариантыОтраженияКорректировокПоступлений.УменьшитьСтоимостьТовара)
	...
	ТекстЗапросаТабЧасть =
		...
		|	ЕСТЬNULL(ТабЧасть.КонтролируемаяПартия.ПланироватьРазблокировкуТовара, ИСТИНА) КАК ПодлежитРазблокировке,
		...
		|	Документ.КорректировкаПриобретения.Расхождения КАК ТабЧасть
		|ГДЕ
		|	ТабЧасть.ВариантОтражения = ЗНАЧЕНИЕ(Перечисление.ВариантыОтраженияКорректировокПоступлений.УвеличитьЗакупкуУвеличитьСкладскиеОстатки)
		|		ИЛИ ТабЧасть.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
		|			И ТабЧасть.ВариантОтражения = ЗНАЧЕНИЕ(Перечисление.ВариантыОтраженияКорректировокПоступлений.УвеличитьСтоимостьТовара)
		|			И НЕ (ТабЧасть.Ссылка.ХозяйственнаяОперация В(
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки),
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки))
		|				И (ЕСТЬNULL(ТабЧасть.Ссылка.Договор.ВариантОформленияЗакупок = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваровИУслуг),
		|					ЛОЖЬ)
		|					ИЛИ ЕСТЬNULL(ТабЧасть.Ссылка.Договор.ВариантОформленияДокументовПоступленияПоУслугам = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется),
		|						ЛОЖЬ)))
		|			И НЕ ТабЧасть.СписатьНаРасходы
		|";
...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	|	(ВЫБОР
	// При использовании среднескользящей корректировка до вводе остатков отражается периодом документа, т.к.
	// в периоде документа - основания не будет движений по регистру "Себестоимость товаров" и отклонения
	// в стоимости не будут распределены.
	|		КОГДА &СреднеСкользящая
	|		 И ТаблицаДокумента.ТипКорректировки = ЗНАЧЕНИЕ(ПеречислениЕ.ТипыКорректировки.КорректировкаДоВводаОстатковТоваровИУслуг)
	|			ТОГДА ТаблицаДокумента.Дата
	|		ИНАЧЕ &ПериодПоступления
	...
	|	ТаблицаВидыЗапасов.АналитикаУчетаНоменклатуры		КАК АналитикаУчетаНоменклатуры,
	|	ТаблицаВидыЗапасов.ВидЗапасов						КАК ВидЗапасов,
	|	&ДокументПоступленияНДС								КАК Партия,
	...
	|		КОГДА ТаблицаВидыЗапасов.Количество = 0
	|		 И НЕ (&СреднеСкользящая
	|				И ТаблицаВидыЗапасов.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.СобственныйТоварПоНеотфактурованнойПоставке))
	|			ТОГДА 0
	|		ИНАЧЕ ТаблицаВидыЗапасов.СуммаСНДСУпр
	|	КОНЕЦ											КАК Стоимость,
	|	-ВЫБОР
	...
	|	ТаблицаВидыЗапасов.ИдентификаторФинЗаписи							  КАК ИдентификаторФинЗаписи,
	// Настройка хозяйственной операции в движении с уменьшенем количества должна отличаться от настройки хозйственной операции
	// в движении с увеличением количества для правильной работы МФУ.
	...
	// кроме корректировки стоимости фактуровки поставки.
	|	 И (НЕ &СреднеСкользящая
	|		ИЛИ ТаблицаВидыЗапасов.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.СобственныйТоварПоНеотфактурованнойПоставке))
	|	 ИЛИ НЕ &ПартионныйУчетВерсии22
	|		ТОГДА ТаблицаВидыЗапасов.СуммаСНДСУпр
	|		ИНАЧЕ 0
	|	КОНЕЦ 											КАК Стоимость,
	...
	|		КОГДА ТаблицаВидыЗапасов.Количество >= 0
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ЗакупкаУПоставщика)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ВозвратТоваровПоставщику)
	|	КОНЕЦ 												КАК НастройкаХозяйственнойОперации
	|
	|ИЗ
	|	Документ.КорректировкаПриобретения КАК ТаблицаДокумента
...
Процедура ОформитьКорректировкуТоваровПоДвухходовке(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ЕСТЬNULL(ДанныеОснование.ВариантПриемкиУслуг, НЕОПРЕДЕЛЕНО) КАК ВариантПриемкиУслуг,
	...
	|	ИсточникДанных.КонтролируемаяПартия          КАК КонтролируемаяПартия,
	...
	|				ИЛИ (НЕ ИсточникДанных.Ссылка.Договор.ВариантОформленияДокументовПоступленияПоУслугам ЕСТЬ NULL
	|				И ИсточникДанных.Ссылка.Договор.ВариантОформленияДокументовПоступленияПоУслугам <> ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПустаяСсылка))
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ)
	// Двухходовка.
	...
	|			ТОГДА ВЫБОР
	|				КОГДА ЕСТЬNULL(ДанныеОснование.ХозяйственнаяОперация, НЕОПРЕДЕЛЕНО) В
	|						(ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаТоварыВПути),
	|						ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки),
	|						ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСТоварыВПути),
	|						ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки),
...
Процедура СформироватьВременнуюТаблицуИсходныхДанных(МенеджерВременныхТаблиц, ДокументОснование, ДокументСсылка = Неопределено) Экспорт
	...
	Иначе
		...
		|	ВЫБОР
		|		КОГДА ТаблицаТовары.Упаковка = ТаблицаТовары.Номенклатура.ЕдиницаИзмерения
...
Функция ТекстЗапросаПоИсходнымДаннымПриобретениеТоваровУслуг()
	...
	|		КОГДА ВЫРАЗИТЬ(ТаблицаТовары.КоличествоУпаковок КАК ЧИСЛО(31,3)) > 0
...
// 
// Параметры:
//  СсылкаОбъект - ДокументСсылка.КорректировкаПриобретения - Документ, по которому нужно получить данные.
...
Функция СуммыПоЗаказам(СсылкаОбъект, ЗначенияРеквизитов = Неопределено) Экспорт
	...
	Если ЗначенияРеквизитов = Неопределено Тогда
		...
		Иначе
			...
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТаблицаЗначений", "Документ.КорректировкаПриобретения.Расхождения");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("7144821c-a835-4238-a7d5-52a2ec41bcee");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.КорректировкаПриобретения КАК РеквизитыДокумента
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.КорректировкаПриобретения КАК РеквизитыДокумента
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			Если ПричинаИсключения = 2 Тогда
				ВызватьИсключение НСтр("ru = 'Не удалось обработать обновление документов ""Корректировка приобретения"".'");
			КонецЕсли;
