...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СобытияФормИСКлиентСерверПереопределяемый.УстановитьПараметрыВыбораНоменклатуры(ЭтотОбъект, ВидыПродукцииЕГАИС);
...
Процедура ВидДокументаПриИзменении(Элемент)
	...
	Если ЭтоПереключениеРегистра3НаРегистр1() И Объект.АкцизныеМарки.Количество() Тогда
		ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения("ВопросПриИзмененииВидаДокументаЗавершение", ЭтотОбъект);
		
		ТекстВопроса = НСтр("ru = 'При изменении операции табличная часть Акцизные марки будет очищена. Продолжить?'");
...
Процедура ТоварыНоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СобытияФормИСКлиентПереопределяемый.ПриНачалеВыбораНоменклатуры(
		ЭтотОбъект, ВидыПродукцииЕГАИС, СтандартнаяОбработка,,
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	ПредыдущийВидДокумента = Объект.ВидДокумента;
...
Процедура Подключаемый_ОбработкаВыбораНоменклатуры(Результат, ДополнительныеПараметры) Экспорт
	...
	ЗаполнитьЗначенияСвойств(ДанныеКешаСтроки, ТекущаяСтрока);
...
Процедура ОбновитьСтатусЕГАИС()
	...
	МенеджерОбъекта = Документы.АктПостановкиНаБалансЕГАИС;
...
Процедура НастроитьЭлементыФормы(Форма)
	...
	Если ЗначениеЗаполнено(Форма.Объект.Ссылка)
		И Форма.Объект.ВидДокумента = АктПостановкиНаБалансВРегистр2 Тогда
...
Процедура УправлениеДоступностьюЭлементовФормы(Форма)
	...
	Если Форма.Объект.ВидДокумента = ПредопределенноеЗначение(
		"Перечисление.ВидыДокументовЕГАИС.АктПостановкиНаБалансВРегистр3") Тогда
...
Процедура ОбработатьПолученныеДанныеТСДНаСервере(ШтрихкодыТСД, ПараметрыСканирования, КэшированныеЗначения)
	...
	ЗагрузкаДанныхТСД = ГрупповаяОбработкаШтрихкодовИС.ОбработатьПолученныеДанныеТСДВДокументе(
		...
		Объект.ВидДокумента = Перечисления.ВидыДокументовЕГАИС.АктПостановкиНаБалансВРегистр3);
...
&НаКлиенте
Функция ЭтоПереключениеРегистра3НаРегистр1()
	
	Возврат ПредыдущийВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.АктПостановкиНаБалансВРегистр3") И 
		Объект.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.АктПостановкиНаБалансВРегистр1");
		
КонецФункции

&НаКлиенте
Процедура ВопросПриИзмененииВидаДокументаЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	Если РезультатВопроса = Неопределено Или Не РезультатВопроса = КодВозвратаДиалога.Да Тогда
		Объект.ВидДокумента = ПредыдущийВидДокумента;
		Возврат;
	КонецЕсли;
...
Процедура ПриИзмененииВидаДокумента()
	...
	Если ЭтоПереключениеРегистра3НаРегистр1() Тогда
		
		ОбновитьДанныеПриИзмененииРегистра3НаРегистр1();
...
&НаСервере
Процедура ОбновитьДанныеПриИзмененииРегистра3НаРегистр1()
	
	Если Объект.АкцизныеМарки.Количество() Тогда
