...
Функция ТекстЗапросаТаблицаРасчетыСКлиентами(Запрос, ТекстыЗапроса, Регистры)
	...
	|	НЕОПРЕДЕЛЕНО КАК НастройкаХозяйственнойОперации,
	|	
	|	ВЫБОР КОГДА ТаблицаРасчетыСКлиентами.ВалютаВзаиморасчетов = &ВалютаРегламентированногоУчета
	|			ТОГДА 0
	|		ИНАЧЕ КурсыВалютВзаиморасчетов.КурсЧислитель
	|	КОНЕЦ КАК КурсЧислительВалютыВзаиморасчетов,
	|	ВЫБОР КОГДА ТаблицаРасчетыСКлиентами.ВалютаВзаиморасчетов = &ВалютаРегламентированногоУчета
	|			ТОГДА 0
	|		ИНАЧЕ КурсыВалютВзаиморасчетов.КурсЗнаменатель
	|	КОНЕЦ КАК КурсЗнаменательВалютыВзаиморасчетов,
	|	ВЫБОР КОГДА ТаблицаРасчетыСКлиентами.ВалютаВзаиморасчетов = &ВалютаУправленческогоУчета
	|			ТОГДА 0
	|		ИНАЧЕ КурсыВалютУпр.КурсЧислитель
	|	КОНЕЦ КАК КурсЧислительВалютыУпр,
	|	ВЫБОР КОГДА ТаблицаРасчетыСКлиентами.ВалютаВзаиморасчетов = &ВалютаУправленческогоУчета
	|			ТОГДА 0
	|		ИНАЧЕ КурсыВалютУпр.КурсЗнаменатель
	...
	|		ИНАЧЕ Неопределено
	|	КОНЕЦ                                         КАК РасчетныйДокумент,
	|	ЗНАЧЕНИЕ(Перечисление.ОперацииВзаиморасчетов.Оплата) КАК ОперацияВзаиморасчетов,
	...
	|	АналитикаПартнера.Договор КАК Договор,
	|	ИСТИНА                                        КАК ИсключатьПриКонтроле,
	|	ТаблицаРасчетыСКлиентами.Сумма                КАК Сумма,
	|	0                                             КАК КОплате,
	|	ТаблицаРасчетыСКлиентами.СуммаРегл            КАК СуммаРегл,
...
Функция ТекстЗапросаТаблицаРасчетыСПоставщиками(Запрос, ТекстыЗапроса, Регистры)
	...
	|	НЕОПРЕДЕЛЕНО                                     КАК НастройкаХозяйственнойОперации,
	|	
	|	ВЫБОР КОГДА ТаблицаРасчетыСПоставщиками.ВалютаВзаиморасчетов = &ВалютаРегламентированногоУчета
	|			ТОГДА 0
	|		ИНАЧЕ КурсыВалютВзаиморасчетов.КурсЧислитель
	|	КОНЕЦ КАК КурсЧислительВалютыВзаиморасчетов,
	|	ВЫБОР КОГДА ТаблицаРасчетыСПоставщиками.ВалютаВзаиморасчетов = &ВалютаРегламентированногоУчета
	...
	|		ИНАЧЕ КурсыВалютВзаиморасчетов.КурсЗнаменатель
	|	КОНЕЦ КАК КурсЗнаменательВалютыВзаиморасчетов,
	|	ВЫБОР КОГДА ТаблицаРасчетыСПоставщиками.ВалютаВзаиморасчетов = &ВалютаУправленческогоУчета
	|			ТОГДА 0
	|		ИНАЧЕ КурсыВалютУпр.КурсЧислитель
	|	КОНЕЦ КАК КурсЧислительВалютыУпр,
	|	ВЫБОР КОГДА ТаблицаРасчетыСПоставщиками.ВалютаВзаиморасчетов = &ВалютаУправленческогоУчета
