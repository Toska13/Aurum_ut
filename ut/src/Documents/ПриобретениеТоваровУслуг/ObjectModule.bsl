...
Функция ПолучитьИтоговыеОстатки()
	...
	|ПОМЕСТИТЬ ВтТоварыИсходнаяСклад
	|ИЗ
	|	&Товары КАК Товары
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|	Товары.Назначение КАК Назначение,
	|	Товары.Серия КАК Серия,
	|	Товары.ТребуетсяОтборПоСерии КАК ТребуетсяОтборПоСерии,
	|	Товары.ЗаказПоставщику КАК ЗаказПоставщику	
	|ПОМЕСТИТЬ ВтТоварыБезСкладов
	|ИЗ
	|	ВтТоварыИсходнаяСклад КАК Товары
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура,
	|	Характеристика,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтТоварыБезСкладов КАК Товары
	...
	|	ВидыЗапасов.ВидЗапасов КАК ВидЗапасов,
	|	ВидыЗапасов.НомерГТД КАК НомерГТД,
	|	ВидыЗапасов.ТребуетсяОтборПоСерии КАК ТребуетсяОтборПоСерии,
	|	ВЫБОР
	|		КОГДА ВидыЗапасов.НомерГТД = ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ЕстьГТД
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТоварыИсходнаяСклад.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
	|	Аналитика.КлючАналитики КАК КлючАналитики,
	|	ТоварыИсходнаяСклад.Склад КАК Склад,
	|	ТоварыИсходнаяСклад.НомерГТД КАК НомерГТД,
	|	ВЫБОР
	...
	|		// только для одноходовки, в двухходовке заказ не делает приход
	...
	|		// только для одноходовки, в двухходовке заказ не делает приход. Особенность, так как движение делает заказ, документом поступления не может быть накладная.
	|			ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.РазделенаТолькоПоНакладным),
	|		// в некоторых документах поле ВариантПриемкиТоваров не заполнено
	|			ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПустаяСсылка),
	|		// в некоторых документах поля ВариантПриемкиТоваров нет
	|			НЕОПРЕДЕЛЕНО)
	|		ТОГДА ТоварыИсходнаяСклад.ЗаказПоставщику
	|	КОНЕЦ КАК ДокументПоступления,
	|	ВЫБОР
	|		КОГДА ТоварыИсходнаяСклад.НомерГТД = ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ЕстьГТД
	|ПОМЕСТИТЬ ВТОтборАналитикСклад
	|ИЗ
	|	ВтТоварыИсходнаяСклад КАК ТоварыИсходнаяСклад
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК Аналитика
	|		ПО ТоварыИсходнаяСклад.Номенклатура = Аналитика.Номенклатура
	|			И ТоварыИсходнаяСклад.Характеристика = Аналитика.Характеристика
	|			И ТоварыИсходнаяСклад.Склад = Аналитика.МестоХранения
	|			И ТоварыИсходнаяСклад.Назначение = Аналитика.Назначение
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТОтборАналитик.КлючАналитики КАК КлючАналитики,
	|	ВТОтборАналитикСклад.КлючАналитики КАК КлючАналитикиСклад,
	|	ВТОтборАналитик.ВидЗапасов КАК ВидЗапасов,
	|	ВТОтборАналитикСклад.Склад КАК Склад,
	|	ВТОтборАналитик.ТребуетсяОтборПоСерии КАК ТребуетсяОтборПоСерии,
	|	ВТОтборАналитикСклад.ДокументПоступления КАК ДокументПоступления
	|ПОМЕСТИТЬ ВТОтборАналитикОбщий
	|ИЗ
	|	ВТОтборАналитик КАК ВТОтборАналитик
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТОтборАналитикСклад КАК ВТОтборАналитикСклад
	|		ПО ВТОтборАналитик.АналитикаУчетаНоменклатуры = ВТОтборАналитикСклад.АналитикаУчетаНоменклатуры
	|			И ВТОтборАналитик.КлючАналитики.Номенклатура = ВТОтборАналитикСклад.КлючАналитики.Номенклатура
	|			И ВТОтборАналитик.КлючАналитики.Назначение = ВТОтборАналитикСклад.КлючАналитики.Назначение
	|			И ВТОтборАналитик.КлючАналитики.Характеристика = ВТОтборАналитикСклад.КлючАналитики.Характеристика
	|			И ВТОтборАналитик.КлючАналитики.Серия = ВТОтборАналитикСклад.КлючАналитики.Серия
	|			И ВТОтборАналитик.КлючАналитики.СтатьяКалькуляции = ВТОтборАналитикСклад.КлючАналитики.СтатьяКалькуляции
	|			И ВТОтборАналитик.НомерГТД = ВТОтборАналитикСклад.НомерГТД
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|					ИЗ
	|						ВТОтборАналитик КАК ТаблицаОтбора)) КАК Остатки
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	Остатки.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
	|	МАКСИМУМ(Остатки.КоличествоОстаток) КАК Количество
	|ПОМЕСТИТЬ ВТДанныеПоОстаткамСклад
	|ИЗ
	|	РегистрНакопления.ТоварыОрганизаций.Остатки(
	|			&ДатаДокумента,
	|			Организация = &Организация
	...
	|						ТаблицаОтбора.КлючАналитики
	|					ИЗ
	|						ВТОтборАналитикСклад КАК ТаблицаОтбора)) КАК Остатки
	|
	|СГРУППИРОВАТЬ ПО
	|	Остатки.АналитикаУчетаНоменклатуры
	|
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОтборАналитикОбщий.КлючАналитики КАК КлючАналитики,
	|	КлючиАналитики.Номенклатура КАК Номенклатура,
	|	КлючиАналитики.Характеристика КАК Характеристика,
	|	КлючиАналитики.Назначение КАК Назначение,
	|	КлючиАналитики.Серия КАК Серия,
	|	ОтборАналитикОбщий.КлючАналитикиСклад.МестоХранения КАК Склад,
	|	ВЫБОР
	|		КОГДА МИНИМУМ(ДанныеПоОстаткам.Количество) > МИНИМУМ(ДанныеПоОстаткамСклад.Количество)
	|			ТОГДА МИНИМУМ(ДанныеПоОстаткамСклад.Количество)
	|		ИНАЧЕ МИНИМУМ(ДанныеПоОстаткам.Количество)
	...
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ) КАК ТребуетсяОтборПоСерии,
	|		ОтборАналитикОбщий.ДокументПоступления КАК ДокументПоступления
	|ПОМЕСТИТЬ ВТОстаткиТоварыОрганизацийОбщий
	|ИЗ
	|	ВТОтборАналитикОбщий КАК ОтборАналитикОбщий
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДанныеПоОстаткам КАК ДанныеПоОстаткам
	|		ПО ОтборАналитикОбщий.КлючАналитики = ДанныеПоОстаткам.АналитикаУчетаНоменклатуры
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДанныеПоОстаткамСклад КАК ДанныеПоОстаткамСклад
	|		ПО ОтборАналитикОбщий.КлючАналитикиСклад = ДанныеПоОстаткамСклад.АналитикаУчетаНоменклатуры 
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК КлючиАналитики
	|		ПО ОтборАналитикОбщий.КлючАналитики = КлючиАналитики.Ссылка
	|
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ОтборАналитикОбщий.КлючАналитики,
	|	КлючиАналитики.Номенклатура,
	|	КлючиАналитики.Характеристика,
	|	КлючиАналитики.Назначение,
	|	КлючиАналитики.Серия,
	|	ОтборАналитикОбщий.КлючАналитикиСклад.МестоХранения,
	|	ОтборАналитикОбщий.ДокументПоступления
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	МАКСИМУМ(-ТоварыКПоступлениюОстатки.КОформлениюПоступленийПоНакладнымОстаток) КАК Количество
	|ПОМЕСТИТЬ ВТОстаткиПоступления
	|ИЗ
	|	РегистрНакопления.ТоварыКПоступлению.Остатки(
	|			,
	|			(ДокументПоступления, Номенклатура, Характеристика, Назначение, Склад) В
	|				(ВЫБРАТЬ
	|					ОстаткиТоварыОрганизацийОбщий.ДокументПоступления,
	|					ОстаткиТоварыОрганизацийОбщий.Номенклатура,
	|					ОстаткиТоварыОрганизацийОбщий.Характеристика,
	|					ОстаткиТоварыОрганизацийОбщий.Назначение,
	|					ОстаткиТоварыОрганизацийОбщий.Склад
	|				ИЗ
	|					ВТОстаткиТоварыОрганизацийОбщий КАК ОстаткиТоварыОрганизацийОбщий)) КАК ТоварыКПоступлениюОстатки
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОтборАналитикОбщий.КлючАналитики КАК АналитикаУчетаНоменклатуры,
	|	КлючиАналитики.Номенклатура КАК Номенклатура,
	|	КлючиАналитики.Характеристика КАК Характеристика,
	|	КлючиАналитики.Назначение КАК Назначение,
	|	КлючиАналитики.Серия КАК Серия,
	|	КлючиАналитики.МестоХранения КАК МестоХранения,
	|	ОтборАналитикОбщий.Склад КАК Склад,
	|	КлючиАналитики.СтатьяКалькуляции КАК СтатьяКалькуляции,
	|	Движения.НомерГТД,
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТОтборАналитикОбщий КАК ОтборАналитикОбщий
	|		ПО Движения.АналитикаУчетаНоменклатуры = ОтборАналитикОбщий.КлючАналитикиСклад
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК КлючиАналитики
	|		ПО ОтборАналитикОбщий.КлючАналитики = КлючиАналитики.Ссылка
	|ГДЕ
	|	Движения.Регистратор = &Ссылка
	|	И Движения.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	И (Движения.АналитикаУчетаНоменклатуры, НомерГТД) В
	|			(ВЫБРАТЬ
	|				ТаблицаОтбора.КлючАналитики, НомерГТД
	|			ИЗ
	|				ВТОтборАналитикСклад КАК ТаблицаОтбора
	|			ГДЕ ТаблицаОтбора.ЕстьГТД)
	|
	|СГРУППИРОВАТЬ ПО
	|	ОтборАналитикОбщий.КлючАналитики,
	|	КлючиАналитики.Номенклатура,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОстаткиТоварыОрганизацийОбщий.КлючАналитики КАК КлючАналитики,
	|	ОстаткиПоступления.Номенклатура КАК Номенклатура,
	|	ОстаткиПоступления.Характеристика КАК Характеристика,
	|	ОстаткиПоступления.Назначение КАК Назначение,
	|	ОстаткиПоступления.Склад КАК Склад,
	|	ОстаткиПоступления.Серия КАК Серия,
	|	ОстаткиТоварыОрганизацийОбщий.Приоритет КАК Приоритет,
	|	ОстаткиТоварыОрганизацийОбщий.ТребуетсяОтборПоСерии КАК ТребуетсяОтборПоСерии,
	|	ВЫБОР
	|		КОГДА ОстаткиПоступления.Количество > ОстаткиТоварыОрганизацийОбщий.Количество
	|			ТОГДА ОстаткиТоварыОрганизацийОбщий.Количество
	|		ИНАЧЕ ОстаткиПоступления.Количество
	|	КОНЕЦ КАК Количество
	|ПОМЕСТИТЬ ВтИтогоОстатки
	|ИЗ
	|	ВТОстаткиПоступления КАК ОстаткиПоступления
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТОстаткиТоварыОрганизацийОбщий КАК ОстаткиТоварыОрганизацийОбщий
	|		ПО (ОстаткиТоварыОрганизацийОбщий.ДокументПоступления = ОстаткиПоступления.ДокументПоступления)
	|			И (ОстаткиТоварыОрганизацийОбщий.Номенклатура = ОстаткиПоступления.Номенклатура)
	|			И (ОстаткиТоварыОрганизацийОбщий.Характеристика = ОстаткиПоступления.Характеристика)
	|			И (ОстаткиТоварыОрганизацийОбщий.Назначение = ОстаткиПоступления.Назначение)
	|			И (ОстаткиТоварыОрганизацийОбщий.Склад = ОстаткиПоступления.Склад)
	|			И (ВЫБОР
	|				КОГДА ОстаткиТоварыОрганизацийОбщий.ТребуетсяОтборПоСерии
	|					ТОГДА ОстаткиТоварыОрганизацийОбщий.Серия = ОстаткиПоступления.Серия
	|			ИНАЧЕ ИСТИНА
	|			КОНЕЦ)
	|;
	|
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		Остатки.КлючАналитики КАК АналитикаУчетаНоменклатуры,
	...
	|		Остатки.ТребуетсяОтборПоСерии КАК ТребуетсяОтборПоСерии,
	|		Остатки.Склад КАК Склад
	|	ИЗ
	|		ВтИтогоОстатки КАК Остатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
...
Процедура ЗаполнитьАналитикамиСУчетомСерий()
	...
	Запрос = Новый Запрос;
	Запрос.Текст = ПолучитьИтоговыеОстатки();
	ВыгружаемыеКолонкиТоваров = "Номенклатура, Характеристика, Назначение, Серия, СтатусУказанияСерий, Склад, АналитикаУчетаНоменклатуры, НомерГТД, ЗаказПоставщику";
	...
	ТоварыКРаспределению = Товары.Выгрузить(, "Номенклатура, Характеристика, Назначение, Склад, Серия, НомерГТД, Количество");
	ТоварыКРаспределению.Свернуть("Номенклатура, Характеристика, Назначение, Склад, Серия, НомерГТД", "Количество");
	Для Каждого СтрокаТовар Из ТоварыКРаспределению Цикл
		
		ПоляОтбора = "Номенклатура, Характеристика, Назначение, Склад";
		ПоляЗаполненияМассив = Новый Массив;
		
		Если ЗначениеЗаполнено(СтрокаТовар.Серия) Тогда
			ПоляОтбора = ПоляОтбора + ", Серия";
			ПоляЗаполненияМассив.Добавить("Серия");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СтрокаТовар.НомерГТД) Тогда
			ПоляЗаполненияМассив.Добавить("НомерГТД");
		КонецЕсли;
		
		ПоляЗаполнения = СтрСоединить(ПоляЗаполненияМассив, ", ");
		
		Если ПоляЗаполнения = "" Тогда
			Продолжить;
		КонецЕсли;
		
		СтруктураПоска = Новый Структура(ПоляОтбора);
		ЗаполнитьЗначенияСвойств(СтруктураПоска, СтрокаТовар);
		НайденныеСтроки = ТаблицаОстатков.НайтиСтроки(СтруктураПоска);
		...
		Для Каждого СтрокаОстатка Из НайденныеСтроки Цикл
			...
			Если ОбработанныеСтроки.Найти(СтрокаОстатка) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			КоличествоКРаспределению = Мин(СтрокаОстатка.Количество, СтрокаТовар.Количество);
			Если КоличествоКРаспределению <> СтрокаОстатка.Количество Тогда 
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТовар, ПоляЗаполнения);
			...
			Иначе
				ЗаполнитьЗначенияСвойств(СтрокаОстатка, СтрокаТовар, ПоляЗаполнения);
				ОбработанныеСтроки.Добавить(СтрокаОстатка);
			КонецЕсли;
			
			СтрокаТовар.Количество = СтрокаТовар.Количество - КоличествоКРаспределению;
	...
	МассивУдаляемыхСтрок = ТаблицаОстатков.НайтиСтроки(ОтборСтрок);
	
	Для Каждого СтрокаТаблицы Из МассивУдаляемыхСтрок Цикл
		ТаблицаОстатков.Удалить(СтрокаТаблицы);
	КонецЦикла;

	КлючПоиска			= КлючПоиска + ", НомерГТД";
	КлючПоискаБезСерии	= КлючПоискаБезСерии + ", НомерГТД";
	...
	Для Каждого СтрокаОстатка Из ТаблицаОстатков Цикл
		...
		Для Каждого СтрокаЗапасов Из ЗаполняемыеСтроки Цикл
			...
			Иначе
				...
				НоваяСтрока.КоличествоПоРНПТ						= ?(Количество <> 0,
					Количество * НоваяСтрока.КоличествоПоРНПТ / СтрокаЗапасов.Количество, 0);
	...
	ТаблицаВидыЗапасовДоп = ТаблицаВидыЗапасов.Скопировать();
	ТаблицаВидыЗапасовДоп.Очистить();
	...
	Для Каждого СтрокаЗапасов Из ТаблицаВидыЗапасов Цикл
		...
		КоличествоПоРНПТ	= СтрокаЗапасов.КоличествоПоРНПТ;
		...
		СтрокаЗапасов.КоличествоПоРНПТ				= ?(Количество <> 0,
			СтрокаЗапасов.Количество * СтрокаЗапасов.КоличествоПоРНПТ / Количество, 0);
		СтрокаЗапасов.КоличествоУпаковок			= СтрокаЗапасов.КоличествоПоРегистру;
		
		Если Количество > СтрокаЗапасов.Количество Тогда
			СтрокаДоп = ТаблицаВидыЗапасовДоп.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаДоп, СтрокаЗапасов);
			СтрокаДоп.АналитикаУчетаНоменклатуры = ТекущаяАналитика;
			СтрокаДоп.Количество = Количество - СтрокаЗапасов.Количество;
			СтрокаДоп.КоличествоУпаковок = Количество - СтрокаЗапасов.Количество;
			СтрокаДоп.КоличествоПоРНПТ = КоличествоПоРНПТ - СтрокаЗапасов.КоличествоПоРНПТ;
		КонецЕсли;
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаВидыЗапасовДоп, ТаблицаВидыЗапасов);
...
Процедура ЗаполнитьДопКолонкиВидовЗапасов(ДораспределятьБезАналитик = Ложь)
	...
	СтруктураПоиска		= Новый Структура("АналитикаУчетаНоменклатуры");
	СтруктураПоискаПоГТД = Новый Структура("АналитикаУчетаНоменклатуры, НомерГТД");
	РеквизитыПоискаБезАналитик = "Номенклатура, Характеристика, Назначение";
	СтруктураПоискаБезАналитикБезГТД = Новый Структура(РеквизитыПоискаБезАналитик);
	РеквизитыПоискаБезАналитик = РеквизитыПоискаБезАналитик + ", НомерГТД";
	СтруктураПоискаБезАналитикСГТД = Новый Структура(РеквизитыПоискаБезАналитик);
	...
	Для Каждого СтрокаТоваров Из ТаблицаТовары Цикл
		...
		ЕстьНомерГТД = ЗначениеЗаполнено(СтрокаТоваров.НомерГТД);
		НайденныеСтроки = Новый Массив;
		
		Если ЕстьНомерГТД Тогда
			ЗаполнитьЗначенияСвойств(СтруктураПоискаПоГТД, СтрокаТоваров);
			НайденныеСтроки = ТаблицаВидыЗапасов.НайтиСтроки(СтруктураПоискаПоГТД);
		КонецЕсли;
		...
		НеРаспределеноБезАналитик = Истина;
		
		Если НайденныеСтроки.Количество() = 0
			И ДораспределятьБезАналитик Тогда
			
			Если ЕстьНомерГТД Тогда
				ЗаполнитьЗначенияСвойств(СтруктураПоискаБезАналитикСГТД, СтрокаТоваров);
				НайденныеСтроки = ТаблицаВидыЗапасов.НайтиСтроки(СтруктураПоискаБезАналитикСГТД);
			КонецЕсли;
			
			Если Не ЕстьНомерГТД Тогда
				ЗаполнитьЗначенияСвойств(СтруктураПоискаБезАналитикБезГТД, СтрокаТоваров);
				НайденныеСтроки = ТаблицаВидыЗапасов.НайтиСтроки(СтруктураПоискаБезАналитикБезГТД);
			КонецЕсли;
			
			НеРаспределеноБезАналитик = Ложь;
		КонецЕсли;
	...
	Если СтруктураРесурсов.Количество > 0
		И ДораспределятьБезАналитик 
		И НеРаспределеноБезАналитик Тогда
		...
		Для Каждого СтрокаЗапасов Из НайденныеСтроки Цикл
			...
			Если (СтрокаЗапасов.Количество = 0
				И СтрокаЗапасов.СуммаСНДС = 0)
				Или ОбработанныеСтроки.Найти(СтрокаЗапасов) <> Неопределено Тогда
