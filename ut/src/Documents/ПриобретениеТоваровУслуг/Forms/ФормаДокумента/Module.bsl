...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ТоварыЗаполнитьСерииПоДаннымПоступленияТоваров.Доступность = ПоступлениеПоРаздельнойЗакупке;
	...
	//++ Локализация
	Если Не ПустаяСтрока(Параметры.ТоварыКассовогоЧекаАдрес) Тогда
		ЗагрузитьКассовыеЧекиНаСервере(Параметры.ТоварыКассовогоЧекаАдрес, Параметры.Ключ.Пустая());
	КонецЕсли;
	//-- Локализация
	...
	ВариантОформленияДокументовПоступленияПоУслугам = ЗакупкиСервер.ПолучитьВариантОформленияДокументовПоступленияПоУслугам(Объект.Договор);
	...
	ЕстьПравоНаДобавлениеКорректировки = ЕстьПравоДобавленияКорректировок();
	
	УстановитьВидимостьФактическойДатыОперации();
КонецПроцедуры
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(РаспознанныеДокументы) Тогда
		ТекущийОбъект.ДополнительныеСвойства.Вставить("РаспознанныеДокументы",
			ОбщегоНазначенияКлиентСервер.СвернутьМассив(РаспознанныеДокументы.ВыгрузитьЗначения()));
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(ТоварыКассовогоЧека) Тогда
		Подключаемый_Команда_УстановитьДоступностьЭлементовПоСтатусуСервер(Команды.ЗагрузитьКассовыеЧеки);
	КонецЕсли;
...
Процедура РежимИсправленияСПрошлойДатыПриИзменении(Элемент)
	...
	Если РежимИсправленияСПрошлойДаты И Не ЗначениеЗаполнено(Объект.ФактическаяДатаОперации) Тогда
		Объект.ФактическаяДатаОперации = Объект.Дата;
	ИначеЕсли Не РежимИсправленияСПрошлойДаты Тогда
		Объект.ФактическаяДатаОперации = Дата(1,1,1);
	КонецЕсли;
	
	Элементы.ФактическаяДатаОперации.Доступность = РежимИсправленияСПрошлойДаты;
...
Процедура ТекстДокументыНаОснованииОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	ПараметрыУстановкиДОПП = Новый Структура();
	ПараметрыУстановкиДОПП.Вставить("Организация", Объект.Организация);
	ПараметрыУстановкиДОПП.Вставить("Основание",   Объект.Ссылка);
	ПараметрыУстановкиДОПП.Вставить("Дата",        Объект.Дата);
	
	УчетНДСУПКлиент.ОбработкаНавигационнойСсылкиДОПП(ЭтотОбъект, ПараметрыУстановкиДОПП, НавигационнаяСсылка, СтандартнаяОбработка);
...
Процедура ТоварыНоменклатураПартнераПриИзменении(Элемент)
	...
	СтруктураДействий.Вставить("ПроверитьЗаполнитьСклад", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияСкладаВСтрокеТЧ(Объект, СкладГруппа,,,"СписатьНаРасходы"));
	СтруктураДействий.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", КонтрольКачестваКэш);
	...
	СтруктураДействий.Вставить("ПриИзмененииТипаНоменклатуры",
		Новый Структура("ЕстьРаботы, ЕстьОтменено, ЕстьСписатьНаРасходы", Истина, Ложь, Истина));
...
Процедура ТоварыНоменклатураПартнераПриИзмененииВопросПользователюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) И Не ПроверитьПараметрыНоменклатурыСервер() Тогда
	...
	СтруктураДействий.Вставить("ЗаполнитьПризнакСписатьНаРасходыИПодразделение",
		ДанныеДляЗаполненияПризнакаСписатьНаРасходы(Объект.ХозяйственнаяОперация, ВариантОформленияДокументовПоступленияПоУслугам));
...
Процедура ТоварыНоменклатураПриИзмененииВопросПользователюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	СтруктураДействий = СтруктураДействийПриИзмененииНоменклатуры(ЭтотОбъект, ТекущаяСтрока);
...
Процедура ПодобратьТовары(Команда)
	...
	Если ЗначениеЗаполнено(Объект.Договор) Тогда
		...
		ЭтоЗакупкаРаботПоДоговоруПереработки = (ТипДоговора = ПредопределенноеЗначение("Перечисление.ТипыДоговоров.СПереработчиком2_5")
												Или ТипДоговора = ПредопределенноеЗначение("Перечисление.ТипыДоговоров.СПереработчиком2_5_ЕАЭС"));
	КонецЕсли;
	...
	ИначеЕсли ЭтоЗакупкаРаботПоДоговоруПереработки Тогда 
		ОтборПоТипуНоменклатуры = Новый ФиксированныйМассив(НоменклатураКлиентСервер.ОтборПоРаботе(Ложь));
		ПараметрыФормы.Вставить("ОтборПоТипуНоменклатуры", ОтборПоТипуНоменклатуры);
	Иначе
...
&НаКлиенте
Процедура ЗагрузитьКассовыеЧеки(Команда)
	//++ Локализация
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеЗагрузкиКассовыхЧеков", ЭтотОбъект);
	
	Если Не ЗначениеЗаполнено(ТоварыКассовогоЧека) Тогда
		ПараметрыФормы = Новый Структура("ПодотчетноеЛицо, КлючПользовательскихНастроек",
			Объект.ПодотчетноеЛицо, Новый УникальныйИдентификатор);
		ОткрытьФорму("Обработка.КассовыйЧекРаспознаваниеДокументов.Форма.ПодборКассовыхЧековДляЗагрузки", ПараметрыФормы,
			ЭтотОбъект, , , , ОписаниеОповещения);
	Иначе
		ЗагрузитьКассовыеЧекиНаСервере();
	КонецЕсли;
	//-- Локализация
КонецПроцедуры
...
Функция ПараметрыРегистрацииУведомленийОВвозеПрослеживаемыхТоваровСервер(Знач Объект)
	...
	Возврат ПриобретениеТоваровУслугЛокализация.ПараметрыРегистрацииУведомленийОВвозеПрослеживаемыхТоваров(Объект);
...
Процедура УстановитьУсловноеОформлениеДляСтатейИАналитик()
	...
	ГруппаОтбораИЛИ.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
		...
		ОтборЭлемента = ГруппаОтбораИЛИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВариантОформленияДокументовПоступленияПоУслугам");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется;
...
&НаСервере
Функция ПроверитьПараметрыНоменклатурыСервер()
	
	Отказ = Ложь;
	
	Документы.ПриобретениеТоваровУслуг.ПроверитьТипыНоменклатуры(Объект.ХозяйственнаяОперация, Объект.Организация, 
		Объект.Договор, Объект.Товары.Выгрузить().ВыгрузитьКолонку("Номенклатура"), Отказ);
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента = ГруппаОтбора2.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ПорядокРасчетов");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов;
	...
	ГруппаОтбора0 = ГруппаОтбора2.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	...
	КонтрольКачества.ДобавитьОтбор(ФлагНедоступен, "Объект.ХозяйственнаяОперация",
...
Процедура ПартнерПриИзмененииСервер()
	...
	ДокументОснование = ЗакупкиСервер.ДокументОснование(ДокументЗакупки, "ПоступлениеПоЗаказам, ЗаказПоставщику, Товары.ЗаказПоставщику");
	ОтветственныеЛицаСервер.УстановитьМенеджера(ДокументЗакупки, ДокументОснование);
...
Процедура ПартнерОчисткаСервер()
	...
	ДокументОснование = ЗакупкиСервер.ДокументОснование(Объект, "ПоступлениеПоЗаказам, ЗаказПоставщику, Товары.ЗаказПоставщику");
...
Процедура СоглашениеПриИзмененииСервер(ПересчитыватьЦены=Истина)
	...
	Объект.ВариантПриемкиУслуг = ЗакупкиСервер.ПолучитьВариантПриемкиУслуг(Объект.Договор);
	
	Если СтарыйСклад <> Объект.Склад
		Или КонтрольКачества.ОбновитьПараметрыДействий(
			Документы.ПриобретениеТоваровУслуг,
			Объект,
			КонтрольКачестваКэш) Тогда
...
Процедура СуммаДокументаПриИзмененииСервер(СуммаДокумента)
	...
	ПараметрыПересчета.ПересчитыватьАвтоматическиеСкидки = Ложь;
...
Процедура ХозяйственнаяОперацияПриИзмененииСервер(ВызовПоКоманде = Истина)
	...
	Если ПоступлениеПоРаздельнойЗакупке
		...
		ИспользуютсяДоговорыКонтрагентов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Соглашение,
			"ИспользуютсяДоговорыКонтрагентов");
		Если ИспользуютсяДоговорыКонтрагентов <> Неопределено И Не ИспользуютсяДоговорыКонтрагентов Тогда
			Объект.Соглашение = Справочники.СоглашенияСПоставщиками.ПустаяСсылка();
		КонецЕсли;
...
&НаСервере
Процедура СкладПриИзмененииСервер(НепосредственноеРедактированиеПоляСклад = Ложь)
	...
	Если КонтрольКачества.ОбновитьПараметрыДействий(
			...
			Объект,
			КонтрольКачестваКэш,
			НепосредственноеРедактированиеПоляСклад) Тогда
...
Процедура ЦенаВключаетНДСПриИзмененииСервер(КэшированныеЗначения)
	...
	//++ Локализация
	ТоварыЗагрузитьКассовыеЧекиУстановитьВидимость();
	//-- Локализация
...
Процедура ПодборТоваровИзЗаказа()
	...
	АдресТоваров = "";
	ХозяйственнаяОперацияПодбор = Объект.ХозяйственнаяОперация;
	
	ПередПодборомТоваровИзЗаказаСервер(МассивКодовСтрок, АдресТоваров, ХозяйственнаяОперацияПодбор);
	
	ПараметрыПодбора = Новый Структура();
	ПараметрыПодбора.Вставить("ХозяйственнаяОперация", ХозяйственнаяОперацияПодбор);
	...
	ПараметрыПодбора.Вставить("ТолькоРаботыУслуги", Не ХозяйственнаяОперацияПодбор = Объект.ХозяйственнаяОперация);
...
&НаСервере
Процедура ПередПодборомТоваровИзЗаказаСервер(МассивКодовСтрок, АдресТоваров, ХозяйственнаяОперацияПодбор)
	...
	ЗаказПоставщикуХО = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ЗаказПоставщику, "ХозяйственнаяОперация");
	
	Если ЗаказПоставщикуХО = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки 
		И Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика Тогда
		ВариантОформленияДокументовПоступленияПоУслугам = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Договор, "ВариантОформленияДокументовПоступленияПоУслугам");
		...
		Если ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется Тогда
			ХозяйственнаяОперацияПодбор = ЗаказПоставщикуХО;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьСтрануПроисхожденияНоменклатуры",
													ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтраныПроисхождения());
	СтруктураДействийСДобавленнымиСтроками.Вставить("ПроверитьЗаполнитьСклад", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияСкладаВСтрокеТЧ(Объект, СкладГруппа,,,"СписатьНаРасходы"));
...
Процедура ОбработатьШтрихкодыСервер(СтруктураПараметровДействия, КэшированныеЗначения)
	...
	ТипыНоменклатуры = НоменклатураСервер.ДопустимыеТипыНоменклатурыДляВыбора(Объект.ХозяйственнаяОперация);
	...
	СтруктураПараметровДействия.ТолькоТовары = Не (ЕстьРаботы
												Или ЕстьУслуги);
	СтруктураПараметровДействия.ТолькоТоварыИРабота = ЕстьРаботы
														И Не ЕстьУслуги;
	СтруктураПараметровДействия.ТолькоТоварыИУслуги = ЕстьУслуги
														И Не ЕстьРаботы;
...
Функция ЗаполнитьСерииПоДаннымПоступленияТоваровНаСервере()
	...
	Если РезультатЗаполненияСерий.ЕстьИзменения Тогда
		...
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ТаблицаТовары, СтруктураДействий, Неопределено);
...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеПриИзменении(Элемент)
	...
	ИначеЕсли Элемент.Имя = "СтруктурнаяЕдиница" Тогда
		СтруктурнаяЕдиницаПриИзменении(Элемент);
	...
	ИначеЕсли Элемент.Имя = "ТоварыПроконтролироватьКачество" Тогда
		ТоварыПроконтролироватьКачествоПриИзменении(Элемент);
	ИначеЕсли Элемент.Имя = "ТоварыДатаПодтвержденияКачества" Тогда
...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеКоманды(Команда)
	...
	ИначеЕсли Команда.Имя = Команды.ЗагрузитьКассовыеЧеки.Имя Тогда
		ЗагрузитьКассовыеЧеки(Команда);
	Иначе
...
Функция ВыполнитьПроверкиПередИзменениемСтатуса()
	...
	Если НЕ ЕстьПравоНаДобавлениеКорректировки И (ЕстьКорректировки ИЛИ ЕстьСчетаФактуры) И ИспользоватьКорректировкиПриобретений Тогда
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	Колонки = "Номенклатура,Характеристика,Упаковка,КоличествоУпаковок,Цена,ПроцентРучнойСкидки,СуммаРучнойСкидки,
		|НомерГТД,Склад,Подразделение,Сертификат,НомерПаспорта";
	...
	СтруктураДействий.Вставить("ПроверитьЗаполнитьСклад",
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияСкладаВСтрокеТЧ(Объект, СкладГруппа,,,"СписатьНаРасходы"));
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	УстановитьРежимИсправленияСПрошлойДаты();
...
Процедура ЗаполнитьПоЗаказуСервер()
	...
	КонтрольКачества.ОбновитьПараметрыДействий(Документы.ПриобретениеТоваровУслуг, Объект, КонтрольКачестваКэш);
	СтруктураДействий = Новый Структура();
	СтруктураДействий.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", КонтрольКачестваКэш);
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Объект.Товары, СтруктураДействий);
...
Процедура УстановитьДоступностьЭлементовПоСтатусуСервер()
	...
	МассивЭлементов.Добавить("СтруктурнаяЕдиница");
	...
	//++ Локализация
	МассивЭлементов.Добавить("ТоварыЗагрузитьКассовыеЧеки");
	//-- Локализация
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	Если Не ПроверитьПараметрыНоменклатурыСервер() Тогда
		Возврат;
	КонецЕсли;
	...
	СтруктураДействий.Вставить("ЗаполнитьДубликатыЗависимыхРеквизитов", ЗависимыеРеквизиты());
	СтруктураДействий.Вставить("ОчиститьСуммуВзаиморасчетов");
	СтруктураДействий.Вставить("ПриИзмененииТипаНоменклатуры", Новый Структура("ЕстьРаботы, ЕстьОтменено, ЕстьСписатьНаРасходы", Истина, Ложь, Истина));
	СтруктураДействий.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", КонтрольКачестваКэш);
	УчетПрослеживаемыхТоваровКлиентСерверЛокализация.ДополнитьОписаниеНастроекЗаполненияСлужебныхРеквизитовТабличнойЧасти(СтруктураДействий);
	...
	КэшированныеЗначения = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
	ОбрабатываемыеСтроки = Новый Массив;
	КопируемыеПоля = "НоменклатураПартнера,
					|Номенклатура,
					|Характеристика,
					|Упаковка,
					|Склад,
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		ТекущаяСтрока = Объект.Товары.Добавить();
		ЗаполнитьЗначенияСвойств(ТекущаяСтрока, СтрокаТовара, КопируемыеПоля);
...
Процедура ПолучитьТоварыИзХранилища(АдресТоваровВХранилище)
	...
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействийСПересчетомСумм, ЭтотОбъект);
	...
	СтруктураДействийСПересчетомСумм.Вставить("ПриИзмененииТипаНоменклатуры",
		Новый Структура("ЕстьРаботы, ЕстьОтменено, ЕстьСписатьНаРасходы", Истина, Ложь, Истина));
	СтруктураДействийСПересчетомСумм.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества",
		КонтрольКачестваКэш);
	
	СтруктураДействийБезПересчетаСумм = Новый Структура;
	СтруктураДействийБезПересчетаСумм.Вставить("ПриИзмененииТипаНоменклатуры",
		Новый Структура("ЕстьРаботы, ЕстьОтменено, ЕстьСписатьНаРасходы", Истина, Ложь, Истина));
	СтруктураДействийБезПересчетаСумм.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества",
		КонтрольКачестваКэш);
	...
	Для Каждого СтрокаТоваров Из ТоварыИзХранилища Цикл
		...
		Если ТоварЗаказа Тогда
			ТоварыЗаказа.Добавить(СтрокаТЧТовары);
		Иначе
			ТоварыНакладной.Добавить(СтрокаТЧТовары);
		КонецЕсли;
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		ТоварыЗаказа,
		СтруктураДействийБезПересчетаСумм,
		Объект.Товары);
		
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		ТоварыНакладной,
		СтруктураДействийСПересчетомСумм,
		Объект.Товары);
	
	ОбрабатываемыеСтроки = Новый Массив;
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОбрабатываемыеСтроки, ТоварыЗаказа);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОбрабатываемыеСтроки, ТоварыНакладной);
	
	Для Каждого СтрокаТЧТовары Из ОбрабатываемыеСтроки Цикл
	...
	НеотфактурованныеПоставкиУслугРабот = ОперацииФактуровка.Найти(Объект.ХозяйственнаяОперация) <> Неопределено
			И ВариантОформленияДокументовПоступленияПоУслугам = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется");
	...
	ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтотОбъект, "ПоступлениеПоЗаказам");
...
Функция ЗаполнитьСкладВВыделенныхСтрокахНаСервере(Знач МассивВыделенныхСтрок, Склад)
	...
	ЗаполненоСтрок = СкладыСервер.ЗаполнитьСкладыВВыделенныхСтроках(Объект.Товары, МассивВыделенныхСтрок, Склад, Истина, "СписатьНаРасходы");
...
Функция ПривязатьСтрокиПоступленияКЗаказамПоставщикам(ОтобратьПоЗаказу)
	...
	Запрос = Новый Запрос("
		...
		|	ТаблицаЗаказы.Подразделение КАК Подразделение,
		...
		|		ЕСТЬNULL(ТоварыЗаказа.Подразделение, ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)) КАК Подразделение,
		|		ЕСТЬNULL(ТоварыЗаказа.СтатьяРасходов, ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)) КАК СтатьяРасходов,
		...
		|					И ЗаказПоставщику.ХозяйственнаяОперация В (&ХозяйственнаяОперация)
		|					И ЗаказПоставщику.Соглашение = &Соглашение
		|					И (ЗаказПоставщику.Валюта = &ВалютаВзаиморасчетов ИЛИ
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПриемНаКомиссию) в (&ХозяйственнаяОперация))
		...
		|		ЕСТЬNULL(ТоварыЗаказа.Назначение, ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)) КАК Назначение,
		|		ЕСТЬNULL(ТоварыЗаказа.СписатьНаРасходы, ЛОЖЬ) КАК СписатьНаРасходы, 
		|		ЕСТЬNULL(ТоварыЗаказа.Подразделение, ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)),
		|		ЕСТЬNULL(ТоварыЗаказа.СтатьяРасходов, ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)),
		...
		|			И ЗаказыДвижения.ЗаказПоставщику.ХозяйственнаяОперация в (&ХозяйственнаяОперация)
		...
		|		ТаблицаЗаказы.СтатьяРасходов,
		|		ТаблицаЗаказы.Подразделение,
	    ...
	    |			ТОГДА ЗаказыПоставщикам.СписатьНаРасходы
	    |		ИНАЧЕ ЛОЖЬ
	    |	КОНЕЦ КАК СписатьНаРасходы,
		|	ЗаказыПоставщикам.СтатьяРасходов КАК СтатьяРасходов,
		|	ЗаказыПоставщикам.Подразделение КАК Подразделение,
	...
	ФактуровкаВвозЕАЭСИлиЗакупкаУПоставщика = Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки
												ИЛИ Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки;
	
	ЗаполнятьАналитикуРасходов = НЕ ФактуровкаВвозЕАЭСИлиЗакупкаУПоставщика;
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика 
		И Не Объект.Договор = Справочники.ДоговорыКонтрагентов.ПустаяСсылка() 
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС 
		И Не Объект.Договор = Справочники.ДоговорыКонтрагентов.ПустаяСсылка() 
		И ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется Тогда
		ХозяйственныеОперации = Новый Массив;
	...
	ВвозЕАЭСИлиЗакупкаУПоставщика = Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС
		Или Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗаполнятьАналитикуРасходов Тогда 
		...
		Если Выборка.КоличествоДублей < 2 Тогда
			...
			Для Каждого СтрокаТовары Из СтрокиТовары Цикл
				...
				Если ВвозЕАЭСИлиЗакупкаУПоставщика Тогда 
					ЗаполнитьЗначенияСвойств(СтрокаТовары, Выборка, "КодСтроки,ЗаказПоставщику,Сделка,Назначение, Подразделение, СтатьяРасходов");
				Иначе
				...
				Если КоличествоВСтроке > НераспределенноеКоличество Тогда
					...
					Если ОбрабатываемыеСтроки.Найти(СтрокаТовары) = Неопределено Тогда
...
Процедура ИзменитьТабличнуюЧастьПоРезультатамПроверки(ВозвращаемыеПараметры, КэшированныеЗначения)
	...
	СтруктураДействийНайденнаяСтрока = Новый Структура;
				
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействийНайденнаяСтрока, ЭтотОбъект);
	
	СтруктураДействийНоваяСтрока = Новый Структура;
			
	Если ИспользоватьСоглашенияСПоставщиками И ЗначениеЗаполнено(Объект.Соглашение) Тогда
		СтруктураДействийНоваяСтрока.Вставить("ЗаполнитьУсловияЗакупок",
			ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияЦеныЗакупкиВСтрокеТЧ(Объект));
	Иначе
		СтруктураДействийНоваяСтрока.Вставить("ЗаполнитьЦенуЗакупки",
			ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияЦеныЗакупкиВСтрокеТЧ(Объект));
	КонецЕсли;
	
	ПараметрыЗаполненияСтавкиНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект);
	ПараметрыЗаполненияСтавкиНДС.УказаннаяСтавкаНДС = УказаннаяСтавкаНДС;
	СтруктураДействийНоваяСтрока.Вставить("ЗаполнитьСтавкуНДС", ПараметрыЗаполненияСтавкиНДС);
	СтруктураДействийНоваяСтрока.Вставить("ЗаполнитьСтавкуНДСВозвратнойТары", Объект.ВернутьМногооборотнуюТару);
	СтруктураДействийНоваяСтрока.Вставить("ПроверитьЗаполнитьСклад",
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияСкладаВСтрокеТЧ(Объект, СкладГруппа,,,"СписатьНаРасходы"));
		
	СтруктураДействийНоваяСтрока.Вставить("ЗаполнитьПризнакХарактеристикиИспользуются",
		Новый Структура("Номенклатура", "ХарактеристикиИспользуются"));
		
	СтруктураДействийНоваяСтрока.Вставить("ЗаполнитьПризнакТипНоменклатуры", Новый Структура("Номенклатура", "ТипНоменклатуры"));
	СтруктураДействийНоваяСтрока.Вставить("ЗаполнитьПризнакАртикул", Новый Структура("Номенклатура", "Артикул"));
	СтруктураДействийНоваяСтрока.Вставить("ЗаполнитьПризнакВедетсяУчетПоГТД", Новый Структура("Номенклатура", "ВедетсяУчетПоГТД"));
	СтруктураДействийНоваяСтрока.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества",
		КонтрольКачестваКэш);
	УчетПрослеживаемыхТоваровКлиентСерверЛокализация.ДополнитьОписаниеНастроекЗаполненияСлужебныхРеквизитовТабличнойЧасти(
		СтруктураДействийНоваяСтрока);
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействийНоваяСтрока, ЭтотОбъект);
	СтруктураДействийНоваяСтрока.Вставить("ЗаполнитьПризнакБезВозвратнойТары", Объект.ВернутьМногооборотнуюТару);
	СтруктураДействийНоваяСтрока.Вставить("ЗаполнитьСлужебныеРеквизитыПоНазначению");
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		ОбрабатываемыеСтроки,
		СтруктураДействийНайденнаяСтрока,
		Объект.Товары,
		КэшированныеЗначения);
...
Процедура ЗаполнитьМногооборотнуюТаруИзХранилищаСервер(АдресТарыВХранилище)
	...
	СтруктураДействийДобавленныеСтроки.Вставить("ПроверитьЗаполнитьСклад", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияСкладаВСтрокеТЧ(Объект, СкладГруппа,,,"СписатьНаРасходы"));
	...
	СтруктураДействийДобавленныеСтроки.Вставить("ПриИзмененииТипаНоменклатуры", Новый Структура("ЕстьРаботы, ЕстьОтменено, ЕстьСписатьНаРасходы", Истина, Ложь, Истина));
	...
	ПакетнаяОбработкаТабличнойЧастиКлиентСерверЛокализация.ДополнитьСтруктуруДействийПриИзмененииЭлемента(ЭтотОбъект, "Номенклатура", СтруктураДействийДобавленныеСтроки);
...
Процедура ТоварыСписатьНаРасходыВопросПользователюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ПроверитьЗаполнитьСклад", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияСкладаВСтрокеТЧ(Объект, СкладГруппа,,, "СписатьНаРасходы"));
	СтруктураДействий.Вставить("ЗаполнитьПодразделениеВСтрокеТЧ", Новый Структура("Подразделение, ЕстьСписатьНаРасходы", Объект.Подразделение, Истина));
...
Процедура ЗаполнитьСтавкуНДСНаСервере(СтавкаНДС, КэшированныеЗначения)
	...
	СтрокиТЧ = РаботаСТабличнымиЧастямиКлиентСервер.ЭлементыКоллекции(Объект.Товары, Элементы.Товары.ВыделенныеСтроки);
	...
	Для каждого СтрокаНаУдаление Из СтрокиНаУдаление Цикл
		СтрокиТЧ.Удалить(СтрокиТЧ.Найти(СтрокаНаУдаление));
	КонецЦикла;
...
Процедура РаспределитьТоварыДокументаПоЗаказам(ТоварыДокументаБезРаспределения, СтрокаБезРаспределения, ОстаткиПоЗаказам, РаспоряжениеЗаказ, СтруктураДействий, СтруктураПоиска, ДобавлятьНераспределенные)
	...
	СтрокаТоваровПоЗаказам = НайденныеСтроки[0];
		...
		Если СтрокаТоваровПоЗаказам.Количество >= НераспределенноеКоличество Тогда
			...
			ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтрокаБезРаспределения, СтруктураДействий, КэшированныеЗначения);
...
Процедура ОбновитьТекстДокументыНаОсновании()
	...
	ПараметрыРегистрацииУведомленийОВвозе = ПриобретениеТоваровУслугЛокализация.ПараметрыРегистрацииУведомленийОВвозеПрослеживаемыхТоваров(Объект);
	...
	Если Не Объект.ПоступлениеПоЗаказам Тогда
		ПредставлениеДОПП = УчетНДСЛокализация.СобытиеДокументыОПредварительныхПоставках(Объект, СуммаВсего);
		ТекстыПоляДокументыНаОсновании.Добавить(ПредставлениеДОПП);
	КонецЕсли;
...
&НаСервере
Процедура УстановитьРежимИсправленияСПрошлойДаты()
	
	РежимИсправленияСПрошлойДаты = ЗначениеЗаполнено(Объект.ФактическаяДатаОперации);
	Элементы.ФактическаяДатаОперации.Доступность = РежимИсправленияСПрошлойДаты;
...
Процедура ОбновитьОбязательностьСкладаВТЧ()
	...
	ПараметрыЗаполнения = Новый Структура("ЕстьРаботы, ЕстьОтменено, ЕстьСписатьНаРасходы", Истина, Ложь, Истина);
	СтруктураДействий = Новый Структура;
...
Процедура ЗаполнитьСписокВыбораОпераций()
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"ТолькоПросмотр",
			...
			Или (Объект.ПоступлениеПоЗаказам И Не ПраваПользователяПовтИсп.ОтклонениеОтУсловийЗакупок()
				И (ЗначениеЗаполнено(Объект.ЗаказПоставщику) Или СписокЗаказов.Количество() > 0)));
...
&НаСервере
Процедура УстановитьВидимостьФактическойДатыОперации()
	
	ОперацииФактуровка = Новый Массив;
	ОперацииФактуровка.Добавить(ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки"));
	ОперацииФактуровка.Добавить(ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки"));
	
	Элементы.ГруппаПрошлаяДата.Видимость = ОперацииФактуровка.Найти(Объект.ХозяйственнаяОперация) = Неопределено;
...
Процедура НастроитьКолонкиСписанияПоРасходамТЧТовары()
	...
	СтрокиДляОчисткиСтатьиАналитикиРасходов = Новый Массив;
	
	Для Каждого СтрокаТовары Из Объект.Товары Цикл
		Если Не СтрокаТовары.СписатьНаРасходы Тогда
			СтрокиДляОчисткиСтатьиАналитикиРасходов.Добавить(СтрокаТовары);
		КонецЕсли;
	...
	Если СтрокиДляОчисткиСтатьиАналитикиРасходов.Количество() > 0 Тогда
		
		СтруктураДействий = Новый Структура;
		ДоходыИРасходыКлиентСервер.ВставитьДействиеОчиститьСтатьюИАналитикуРасходов(СтруктураДействий, ЭтотОбъект);

		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			СтрокиДляОчисткиСтатьиАналитикиРасходов,
			СтруктураДействий,
			Объект.Товары);
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ДанныеДляЗаполненияПризнакаСписатьНаРасходы(ХозяйственнаяОперация, ВариантОформленияДокументовПоступленияПоУслугам)
	...
	ЭтоФактуровкаПоТоварам = ОперацииФактуровка.Найти(ХозяйственнаяОперация) <> Неопределено;
	
	ЭтоФактуровкаПоУслугамИРаботам = ЭтоФактуровкаПоТоварам 
		И ВариантОформленияДокументовПоступленияПоУслугам = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется");
	
	ДанныеЗаполнения.Вставить("ЭтоФактуровкаПоТоварам", ЭтоФактуровкаПоТоварам);
	ДанныеЗаполнения.Вставить("ЭтоФактуровкаПоУслугамИРаботам", ЭтоФактуровкаПоУслугамИРаботам);
...
//++ Локализация
&НаКлиенте
Процедура ПослеЗагрузкиКассовыхЧеков(Адрес, ДопПараметры) Экспорт

	Если Адрес = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ЗагрузитьКассовыеЧекиНаСервере(Адрес);

КонецПроцедуры

&НаСервере
Процедура ЗагрузитьКассовыеЧекиНаСервере(Знач Адрес = Неопределено, Знач Загружать = Истина)
	
	Если Не Адрес = Неопределено Тогда
		ТоварыКассовогоЧека = ПолучитьИзВременногоХранилища(Адрес);
	КонецЕсли;
	
	Если Загружать Тогда
		
		Для Каждого СтрокаЧека Из ТоварыКассовогоЧека Цикл
			
			НоваяСтрока = Объект.Товары.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаЧека.Значение);
			
			СтруктураДействий = СтруктураДействийПриИзмененииНоменклатуры(ЭтотОбъект, НоваяСтрока);
			КэшированныеЗначения = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
			...
			УчетПрослеживаемыхТоваровКлиентСерверЛокализация
				.ДополнитьОписаниеНастроекЗаполненияСлужебныхРеквизитовТабличнойЧасти(СтруктураДействий);
			УчетПрослеживаемыхТоваровКлиентСерверЛокализация.ДополнитьОписаниеНастроекПересчетаРеквизитовТабличнойЧасти(
				Объект, СтруктураДействий, "Склад", Истина);
				
			ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(НоваяСтрока, СтруктураДействий, КэшированныеЗначения);
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаЧека.Значение, ,"Номенклатура, Характеристика");
			...
			ПакетнаяОбработкаТабличнойЧастиВызовСервера.ОбработатьСтрокуТЧ(ТекущаяСтрокаСтруктура, СтруктураДействий,
				КэшированныеЗначения);
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущаяСтрокаСтруктура, "КоличествоПоРНПТ, НомерГТД");
			
			ЕстьМаркируемаяПродукция = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
				НоваяСтрока, "МаркируемаяПродукция");
			ЕстьРазрешенОборот = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
				НоваяСтрока, "РазрешенОборотНемаркированныхОстатков");
			ЕстьСтатусПроверкиГосИС = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
				НоваяСтрока, "СтатусПроверкиГосИС");
				
			Если ЕстьМаркируемаяПродукция И НоваяСтрока.МаркируемаяПродукция И ЕстьСтатусПроверкиГосИС Тогда
				РазрешенОборотБезМарок = ?(ЕстьРазрешенОборот, НоваяСтрока.РазрешенОборотНемаркированныхОстатков, Ложь);
				Если РазрешенОборотБезМарок Тогда
					// Иконка с вопросом: разрешен оборот, марки не отсканированы
					НоваяСтрока.СтатусПроверкиГосИС = 6;
				Иначе
					// Красная иконка: марки обязательны, марки не отсканированы
					НоваяСтрока.СтатусПроверкиГосИС = 2;
				КонецЕсли;
			КонецЕсли;
			
			РаспознанныеДокументы.Добавить(СтрокаЧека.Значение.РаспознанныйДокумент);
		...
		Модифицированность = Истина;
		ТоварыКассовогоЧека = Неопределено;
...
&НаСервере
Процедура ТоварыЗагрузитьКассовыеЧекиУстановитьВидимость()

	Элементы.ТоварыЗагрузитьКассовыеЧеки.Видимость = Объект.ЦенаВключаетНДС
		И Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаЧерезПодотчетноеЛицо
		И ПолучитьФункциональнуюОпцию("ИспользоватьРаспознаваниеДокументов")
		И ПравоДоступа("Чтение", Метаданные.Документы.РаспознанныйДокумент);

КонецПроцедуры
//-- Локализация

&НаКлиентеНаСервереБезКонтекста
Функция СтруктураДействийПриИзмененииНоменклатуры(Форма, ТекущаяСтрока)
	...
	СтруктураДействий.Вставить("ПроверитьХарактеристикуПоВладельцу", ТекущаяСтрока.Характеристика);
	СтруктураДействий.Вставить("ПроверитьЗаполнитьСклад",
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияСкладаВСтрокеТЧ(Объект,
		Форма.СкладГруппа, , ,"СписатьНаРасходы"));
	СтруктураДействий.Вставить("ЗаполнитьНоменклатуруПартнераПоНоменклатуре", Объект.Партнер);
	СтруктураДействий.Вставить("ПроверитьЗаполнитьУпаковкуПоВладельцу"      , ТекущаяСтрока.Упаковка);
	СтруктураДействий.Вставить("ПересчитатьКоличествоЕдиниц");
	
	Если Форма.ИспользоватьСоглашенияСПоставщиками И ЗначениеЗаполнено(Объект.Соглашение) Тогда
	...
	ПараметрыЗаполненияСтавкиНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект);
	ПараметрыЗаполненияСтавкиНДС.УказаннаяСтавкаНДС = Форма.УказаннаяСтавкаНДС;
	СтруктураДействий.Вставить("ЗаполнитьСтавкуНДС", ПараметрыЗаполненияСтавкиНДС);
	СтруктураДействий.Вставить("ЗаполнитьСтавкуНДСВозвратнойТары", Объект.ВернутьМногооборотнуюТару);
	СтруктураДействий.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
	СтруктураДействий.Вставить("ПересчитатьСуммуСНДС", СтруктураПересчетаСуммы);
	...
	СтруктураДействий.Вставить("ЗаполнитьПризнакАртикул", Новый Структура("Номенклатура", "Артикул"));
	СтруктураДействий.Вставить("ПроверитьСериюРассчитатьСтатус", Новый Структура("Склад, ПараметрыУказанияСерий",
		ТекущаяСтрока.Склад, Форма.ПараметрыУказанияСерий));
	СтруктураДействий.Вставить("ЗаполнитьПризнакБезВозвратнойТары", Объект.ВернутьМногооборотнуюТару);
	СтруктураДействий.Вставить("ЗаполнитьДубликатыЗависимыхРеквизитов", ЗависимыеРеквизиты());
	СтруктураДействий.Вставить("ПриИзмененииТипаНоменклатуры",
		Новый Структура("ЕстьРаботы, ЕстьОтменено, ЕстьСписатьНаРасходы", Истина, Ложь, Истина));
	...
	СтруктураДействий.Вставить("ЗаполнитьСлужебныеРеквизитыПоНазначению");
	СтруктураДействий.Вставить("ЗаполнитьПризнакСписатьНаРасходыИПодразделение",
		ДанныеДляЗаполненияПризнакаСписатьНаРасходы(Объект.ХозяйственнаяОперация,
		Форма.ВариантОформленияДокументовПоступленияПоУслугам));
