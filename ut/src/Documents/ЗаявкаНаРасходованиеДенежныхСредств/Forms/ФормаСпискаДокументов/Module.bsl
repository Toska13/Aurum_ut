...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПравоДобавления = ПравоДоступа("Добавление", Метаданные.Документы.ЗаявкаНаРасходованиеДенежныхСредств);
	...
	Элементы.ГруппаСоздать.Доступность = ПравоДобавления;
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ЕстьФайлы" Тогда
		...
		Если СтрокаСписка <> Неопределено Тогда
			РаботаСФайламиКлиент.ОткрытьФормуСпискаФайлов(СтрокаСписка.Ссылка,, ЭтотОбъект);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработатьШтрихкоды(Данные)
	...
	Иначе
		...
		Оповещение = Новый ОписаниеОповещения("ОбработатьШтрихкодыЗавершение", ЭтотОбъект, Данные.Штрихкод);
		
		Если ПодключеныОбработчикиЛокализация Тогда
			ДенежныеСредстваКлиентЛокализация.ПередОбработкойШтрихкодаПлатежнойСсылки(ЭтотОбъект, Данные.Штрихкод, Оповещение);
		Иначе
			ВыполнитьОбработкуОповещения(Оповещение, Данные.Штрихкод);
		КонецЕсли;
...
&НаКлиенте
Процедура ОбработатьШтрихкодыЗавершение(Штрихкод, ДополнительныеПараметры) Экспорт
...
Процедура Подключаемый_ПриСозданииНаСервереЛокализация(Отказ, СтандартнаяОбработка)
	...
	Если ПравоДобавления Тогда
		ПараметрыКоманды = СистемаБыстрыхПлатежейУТ.НовыйПараметрыКомандыСканированияШтрихкодаСБП();
		ПараметрыКоманды.Родитель = Элементы.ЛокализацияГруппаКоманд;
		СистемаБыстрыхПлатежейУТ.ДобавитьКомандуСканированияШтрихкодаСБП(ЭтотОбъект, ПараметрыКоманды);
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_СканироватьQRКодСБПЛокализация(Команда)

	#Если МобильныйКлиент Тогда
		
		Если СредстваМультимедиа.ПоддерживаетсяСканированиеШтрихКодов() Тогда
			
			ОбработчикСканирования = Новый ОписаниеОповещения("ОбработатьQRКодПослеСканирования", ЭтотОбъект);
			СредстваМультимедиа.ПоказатьСканированиеШтрихКодов(, ОбработчикСканирования, , ТипШтрихКода.QRCode);
			
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не поддерживается сканирование штрихкодов'"));
		КонецЕсли;
...
&НаКлиенте
Процедура ОбработатьШтрихкодПлатежнойСсылки(НастройкаПодключения, ДополнительныеПараметры) Экспорт

	Если НастройкаПодключения = Неопределено Тогда
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеЗавершения, ДополнительныеПараметры.Штрихкод);
	Иначе
		
		ПлатежнаяСсылка = ДополнительныеПараметры.Штрихкод;
		ОповещениеОПолученииДанныхЗаполнения = Новый ОписаниеОповещения("СоздатьЗаявкуПоДаннымПлатежнойСсылки", ЭтотОбъект);
...
&НаКлиенте
Процедура СоздатьЗаявкуПоДаннымПлатежнойСсылки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеЗаполнения.КодОшибки) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(ДанныеЗаполнения.СообщениеОбОшибке);
		Возврат;
	КонецЕсли;
	
	Если ДанныеЗаполнения <> Неопределено Тогда
		ПараметрыОткрытия = Новый Структура;
		ПараметрыОткрытия.Вставить("Основание", ДанныеЗаполнения);
		ОткрытьФорму("Документ.ЗаявкаНаРасходованиеДенежныхСредств.ФормаОбъекта", ПараметрыОткрытия);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ОбработатьQRКодПослеСканирования(ШтрихКод, Результат, Сообщение, ДополнительныеПараметры) Экспорт
	...
	#Если МобильныйКлиент Тогда
		...
		Если Не Результат Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Ошибка сканирования QR-кода СБП'"));
		Иначе
			ДенежныеСредстваКлиентЛокализация.ПередОбработкойШтрихкодаПлатежнойСсылки(ЭтотОбъект, ШтрихКод, Неопределено);
		КонецЕсли;
