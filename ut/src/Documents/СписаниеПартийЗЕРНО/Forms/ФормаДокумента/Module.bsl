...
Процедура РазделУчетаПриИзменении(Элемент)
	...
	НастроитьЗависимыеЭлементыФормы(ЭтотОбъект, "РазделУчета");
...
Процедура РазделУчетаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Объект.Товары.Количество() > 0 И ВыбранноеЗначение <> Объект.РазделУчета Тогда
		...
		СтандартнаяОбработка = Ложь;
		ТекстВопроса = НСтр("ru = 'При изменении раздела учета табличная часть Товары и связанные с ней
								  |партии будут очищены. Продолжить?'");
...
Процедура ОбработатьВыборПартии(ВыбранноеЗначение, ТекущиеДанные) Экспорт
	...
	ИзменениеНоменклатуры = ВыбранноеЗначение.Свойство("Номенклатура")
