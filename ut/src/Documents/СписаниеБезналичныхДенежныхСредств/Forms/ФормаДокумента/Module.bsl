...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыВыбораСтатейИАналитик = Документы.СписаниеБезналичныхДенежныхСредств.ПараметрыВыбораСтатейИАналитик(
		Объект.ХозяйственнаяОперация, Объект.ОтражатьКомиссию И Не Объект.ВестиУчетКомиссииВоВзаиморасчетах);
...
Процедура ПередЗаписьюПредложитьПересчитатьСуммуДокументаЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыдачаДенежныхСредствПодотчетнику")
		...
		Если Объект.РасшифровкаПлатежа.Итог("Сумма") <> Объект.СуммаДокумента Тогда
			
			ТекстСообщения = НСтр("ru = 'Сумма по заявкам на оплату не равна сумме документа.'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , Элементы.РасшифровкаПлатежа.Имя, "Объект", Истина);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ДенежныеСредстваСервер.ЗаполнитьДанныеОбъектаРасчетовРасшифровкиПлатежа(Объект, , Истина);
...
Функция ХозяйственнаяОперацияПриИзмененииКлиент()
	...
	УстановитьПризнакОплатыПоЗаявкам();
...
Процедура ХозяйственнаяОперацияПриИзмененииСервер()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВнутренняяПередачаДенежныхСредств
		...
		Если ТекущийБанковскийСчет <> Объект.БанковскийСчет Тогда
			...
			ПриИзмененииБанковскийСчет(СтруктураРеквизитов);
	...
	Если Не ЗначениеЗаполнено(Объект.Контрагент) Тогда
		...
		КонтрагентЮрФизЛицо = Перечисления.ЮрФизЛицо.ПустаяСсылка();
	КонецЕсли;
	...
	Если Не Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаАрендодателю Тогда
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СнятиеНаличныхДенежныхСредств
		 И Не ЗначениеЗаполнено(Объект.КассаПолучатель) Тогда
		Объект.КассаПолучатель = Справочники.Кассы.ПолучитьКассуПоУмолчанию(Объект.Организация, Объект.Валюта);
	КонецЕсли;
	...
	ДенежныеСредстваСервер.ЗаполнитьНалогообложениеНДС(
		Объект,
		Объект.НалогообложениеНДС,
		Элементы.РасшифровкаБезРазбиенияНалогообложениеНДС,
		Элементы.РасшифровкаБезРазбиенияНалогообложениеНДСВозврата,
		Истина);
	ОбновитьНадписьЗеркальныйПлатеж();
...
Функция БанковскийСчетПриИзмененииКлиент(ИмяОбработчика)
	...
	РазрешеныПлатежиБезУказанияЗаявок =
		(РазрешитьПлатежиБезУказанияЗаявокБанковскийСчет И ИспользоватьЗаявкиНаРасходованиеДенежныхСредств)
...
Процедура СуммаКомиссииПриИзменении(Элемент)
	...
	Если Объект.СуммаДокумента <> 0 Тогда
		СтавкаКомиссии = ДенежныеСредстваКлиентСервер.РассчитатьСтавкуКомиссии(Объект.СуммаДокумента, Объект.СуммаКомиссии, "Возврат");
	Иначе
		СтавкаКомиссии = 0;
	КонецЕсли;
...
Процедура КонтрагентПриИзмененииСервер()
	...
	ТекущийБанковскийСчетКонтрагента = Объект.БанковскийСчетКонтрагента;
	...
	Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
		...
		Если ТекущийБанковскийСчетКонтрагента <> Объект.БанковскийСчетКонтрагента
...
Процедура ДоговорЭквайрингаПриИзмененииНаСервере()
	...
	Если Объект.ОтражатьКомиссию Тогда
		...
		Если ЗначенияРеквизитов.ФиксированнаяСтавкаКомиссии Тогда
			...
			Объект.СуммаКомиссии = ДенежныеСредстваКлиентСервер.РассчитатьСуммуКомиссии(Объект.СуммаДокумента, СтавкаКомиссии, "Возврат");
		Иначе
			СтавкаКомиссии = 0;
		КонецЕсли;
...
Процедура ЗаявкаНаРасходованиеДенежныхСредствПриИзмененииСервер()
	...
	Если (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыПоЗарплатномуПроекту
		...
		И НЕ Объект.ПроведеноБанком Тогда
...
Процедура БанковскийСчетПолучательПриИзмененииСервер()
	...
	ПриИзмененииБанковскийСчетПолучатель();
	...
	Иначе
		ОткрытьФорму(
			...
			Новый Структура("СписокДокументов, Заголовок",
				СписокЗаявок,
				НСтр("ru = 'Заявки на оплату (%КоличествоДокументов%)'")
			),
...
Процедура ДоговорПриИзмененииНаСервере()
	...
	ТекущийОбъектРасчетов = ТекущаяСтрока.ОбъектРасчетов;
	ДенежныеСредстваСервер.ПроверитьЗаполнитьОбъектРасчетов(Объект, Элементы.РасшифровкаПлатежа.ТекущаяСтрока);
	ДенежныеСредстваСервер.ЗаполнитьДанныеОбъектаРасчетовРасшифровкиПлатежа(Объект, , Истина);
	Если ТекущийОбъектРасчетов <> ТекущаяСтрока.ОбъектРасчетов Тогда
		ЗаполнитьУИП(Элементы.РасшифровкаПлатежа.ТекущаяСтрока);
	КонецЕсли;
	ОбработатьИзменениеУИП();
...
Процедура НадписьРеквизитыПлательщикаНажатие(Элемент, СтандартнаяОбработка)
	...
	СтруктураПараметров.Вставить("Дата",           ?(Объект.ПроведеноБанком, Объект.ДатаПроведенияБанком, Объект.Дата));
...
Процедура ИдентификаторПлатежаПриИзменении(Элемент)
	...
	ИдентификаторПлатежаПриИзмененииНаСервере();
...
Процедура РасшифровкаПлатежаПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ПриИзмененииРасшифровкаПлатежаСумма();
...
Процедура РасшифровкаПлатежаЗаявкаПриИзмененииСервер(ЗаявкаСсылка)
	...
	Если ТипЗнч(ЗаявкаСсылка) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств") Тогда
		...
		РеквизитыЗаявки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЗаявкаСсылка, "Контрагент, ИдентификаторПлатежа");
		КонтрагентЗаявки = РеквизитыЗаявки.Контрагент;
		...
		Если ПустаяСтрока(Объект.ИдентификаторПлатежа) Тогда
			Объект.ИдентификаторПлатежа = РеквизитыЗаявки.ИдентификаторПлатежа;
		КонецЕсли;
		
	КонецЕсли;
	
	СтрокаРасшифровки = Объект.РасшифровкаПлатежа.НайтиПоИдентификатору(Элементы.РасшифровкаПлатежа.ТекущаяСтрока);
	
	Если СтрокаРасшифровки <> Неопределено
		И Объект.СписокФизЛиц Тогда
		СтрокаРасшифровки.Сумма = СуммаОстаткаЗаявки(ЗаявкаСсылка);
	КонецЕсли;
...
Процедура РасшифровкаБезРазбиенияОбъектРасчетовПриИзмененииСервер(ИдСтроки)
	...
	ДенежныеСредстваСервер.ЗаполнитьДанныеОбъектаРасчетовРасшифровкиПлатежа(
		Объект, Элементы.РасшифровкаПлатежа.ТекущаяСтрока, Истина);
	ЗаполнитьУИП(Элементы.РасшифровкаПлатежа.ТекущаяСтрока);
	ОбработатьИзменениеУИП();
	ОбновитьСтавкуНДСИПересчитатьСуммуНДС(ИдСтроки);
	УстановитьВидимостьДоступностьЭлементовРасшифровкиБезРазбиения();
	УстановитьОграничениеТипаДоговораРасшифровкиПлатежа();
	ВзаиморасчетыРаботаСФормами.ЗаблокироватьОбъектыРасчетов(ЭтотОбъект);
...
Процедура РасшифровкаПлатежаОбъектРасчетовОбработкаВыбораНаСервере(ВыбранноеЗначение)
	...
	Модифицированность = Истина;
	ВзаиморасчетыРаботаСФормами.ЗаблокироватьОбъектыРасчетов(ЭтаФорма);
	
	//++ Локализация


	//-- Локализация
	ЗаполнитьУИП(ИдентификаторСтроки);
...
Процедура ГиперссылкаРеквизитыОбъектаРасчетовНажатиеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Модифицированность = Модифицированность ИЛИ Результат.Модифицированность;
...
Процедура ГиперссылкаРеквизитыОбъектаРасчетовНажатиеЗавершениеНаСервере()
	...
	Для Каждого Индекс Из ИзмененныеСтроки Цикл
		...
		ЗаполнитьУИП(СтрокаРасшифровки.ПолучитьИдентификатор());
		РасшифровкаПлатежаОбъектРасчетовПриИзмененииСервер(СтрокаРасшифровки.ПолучитьИдентификатор());
...
Процедура РасшифровкаОбъектРасчетовПриИзмененииСервер(ИдСтроки)
	...
	ДенежныеСредстваСервер.ЗаполнитьДанныеОбъектаРасчетовРасшифровкиПлатежа(Объект, ИдСтроки, Истина);
	
	ДенежныеСредстваСервер.ПроверитьЗаполнитьДоговор(Объект, ИдСтроки);
	
	УстановитьОграничениеТипаДоговораРасшифровкиПлатежа();
	СформироватьНадписьГиперссылкиРеквизитыОбъектаРасчетов();
	ОбработатьИзменениеУИП(СтрокаТаблицы);
...
Процедура РасшифровкаПлатежаВедомостьНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Не ИспользоватьЗарплатаИКадрыWE Тогда
		//++ Локализация
...
Процедура ПодборПоОстаткамЗавершениеСервер()
	...
	ЗаполнитьПараметрыОбъектаРасчетовВРасшифровке(Не Объект.ОплатаПоЗаявкам);
...
Процедура ПодобратьИзЗаявокЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	ФормыОплаты = ФормыОплатыЗаявокНаБезналичнуюОплату();
	
	ПараметрыПодбора.Вставить("ФормаОплаты",                         ФормыОплаты);
...
&НаСервере
Процедура ПриИзмененииБанковскийСчет(СтруктураРеквизитов)
...
Процедура ВалютаПриИзмененииНаСервере()
	...
	ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтотОбъект, "Валюта");
...
&НаСервере
Процедура ПриИзмененииБанковскийСчетПолучатель()
	...
	ЗаполнитьОрганизациюПолучателя(СтруктураРеквизитов);
...
&НаСервере
Процедура ПриИзмененииРасшифровкаПлатежаСумма()
...
&НаСервере
Процедура ИдентификаторПлатежаПриИзмененииНаСервере()
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ИспользоватьЗаявкиНаКомандировку                  = ПолучитьФункциональнуюОпцию("ИспользоватьЗаявкиНаКомандировку");
...
Функция РеквизитыФормы(Форма)
	...
	РеквизитыФормы.Вставить("ИспользоватьЗаявкиНаКомандировку");
	...
	РеквизитыФормы.Вставить("УИПОтличается");
...
&НаСервере
Процедура ЗаполнитьПараметрыОбъектаРасчетовВРасшифровке(ЗаполнитьУИП = Ложь)
	...
	Для Каждого СтрокаТч Из Объект.РасшифровкаПлатежа Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТч.ОбъектРасчетов) Тогда
			...
			Если ЗаполнитьУИП И ПустаяСтрока(Объект.ИдентификаторПлатежа) И ЗначениеЗаполнено(СтрокаТч.ИдентификаторПлатежа) Тогда
				Объект.ИдентификаторПлатежа = СтрокаТч.ИдентификаторПлатежа;
			КонецЕсли;
		КонецЕсли;
		ЗаполнитьИндексУИПОтличается(СтрокаТч);
	КонецЦикла;
	ОпределитьУИПОтличается();
КонецПроцедуры
...
&НаСервере
Процедура ЗаполнитьОрганизациюПолучателя(СтруктураРеквизитов = Неопределено)
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаДенежныхСредствВДругуюОрганизацию
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитов.Организация) Тогда
			Если ЗначениеЗаполнено(ОрганизацияПолучатель) 
				И ОрганизацияПолучатель <> СтруктураРеквизитов.Организация Тогда
				ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтотОбъект, "БанковскийСчетПолучатель.Владелец");
			КонецЕсли;
...
Процедура ЗаполнитьРасшифровкуПлатежа(ТекущаяСтрокаТаблицы, ОбъектРасчетовДоговор)
	...
	Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
		...
		ЗаполнитьУИП(ТекущаяСтрокаТаблицы.ПолучитьИдентификатор());
	...
	ДенежныеСредстваСервер.СформироватьКурсЗнаменательТекстом(ТекущаяСтрокаТаблицы, Объект.Валюта, ВалютаРеглУчета);
	ДенежныеСредстваСервер.УстановитьОформлениеТочностьКурса(ЭтотОбъект);
...
Процедура ПеречислениеСотрудникуЧерезБанкПриИзмененииСервер()
	...
	Если ВыплатаОдномуФизЛицу() Тогда
...
Процедура ДобавитьСтрокуРасшифровкиПоЗаявке(СтрокаПлатежа, ДополнительныеПараметры)
	...
	Если ПустаяСтрока(Объект.ИдентификаторПлатежа) И ЗначениеЗаполнено(СтрокаПлатежа.ИдентификаторПлатежа) Тогда
		Объект.ИдентификаторПлатежа = СтрокаПлатежа.ИдентификаторПлатежа;
	КонецЕсли;
	
	РасшифровкаПлатежаОбъектРасчетовПриИзмененииСервер(НоваяСтрока.ПолучитьИдентификатор());
	ЗаполнитьКурсИКратностьВСтрокеРасшифровки(НоваяСтрока);
	ДенежныеСредстваКлиентСервер.РассчитатьСуммуВзаиморасчетовВСтрокеРасшифровки(
		НоваяСтрока, Объект.Валюта, ВалютаРеглУчета);
	ДенежныеСредстваСервер.СформироватьКурсЗнаменательТекстом(НоваяСтрока, Объект.Валюта, ВалютаРеглУчета);
	ЗаполнитьРазрешенаРаботаСДочернимиПартнерами(НоваяСтрока);
...
Процедура УстановитьПризнакПлатежаСКонвертацией()
	...
	Если Объект.ПлатежСКонвертацией Тогда
		...
		Если Объект.Валюта <> ВалютыСчетов.Получить(Объект.БанковскийСчетПолучатель) Тогда
			...
			ПараметрыСчета.Валюта = Объект.Валюта;
			Объект.БанковскийСчетПолучатель = Справочники.БанковскиеСчетаОрганизаций.ПолучитьБанковскийСчетПоУмолчанию(
				ПараметрыСчета);
		КонецЕсли;
		
	КонецЕсли;

	НастроитьЗависимыеЭлементыФормыНаСервере("ПлатежСКонвертацией");

КонецПроцедуры

&НаСервере
Функция ВыплатаОдномуФизЛицу()

	Возврат Не Объект.ПеречислениеСотрудникуЧерезБанк
...
&НаКлиенте
Процедура УстановитьПризнакОплатыПоЗаявкам()

	Если Не РазрешеныПлатежиБезУказанияЗаявок
...
Процедура ЗаполнитьПоДоговоруСтрокуРасшифровкиПлатежа(СтрокаРасшифровки, РеквизитыДоговора)
	...
	Иначе
		ИсключаяСвойства = СтрокаИсключенныхСвойствПоДоговору(СтрокаРасшифровки, РеквизитыДоговора);
...
&НаСервере
Функция СуммаОстаткаЗаявки(ЗаявкаНаОплату)

	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ОстаткиКВыплате.СуммаОстаток КАК СуммаОстаток
		|ИЗ
		|	РегистрНакопления.ДенежныеСредстваКВыплате.Остатки(, ЗаявкаНаРасходованиеДенежныхСредств = &Заявка) КАК ОстаткиКВыплате";
...
&НаСервере
Процедура ЗаполнитьУИП(ИдентификаторСтрокиРасшифровки)
	
	Если Объект.ОплатаПоЗаявкам Или ЗначениеЗаполнено(Объект.ИдентификаторПлатежа) И ПереключательРасшифровки = 1 Тогда
		Возврат;
	КонецЕсли;
	
	СтрокаРасшифровки = Объект.РасшифровкаПлатежа.НайтиПоИдентификатору(ИдентификаторСтрокиРасшифровки);
	Если ЗначениеЗаполнено(СтрокаРасшифровки.ИдентификаторПлатежа) Тогда
		Объект.ИдентификаторПлатежа = СтрокаРасшифровки.ИдентификаторПлатежа;
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ОбработатьИзменениеУИП(СтрокаРасшифровки = Неопределено)
	
	Если СтрокаРасшифровки <> Неопределено Тогда
		ЗаполнитьИндексУИПОтличается(СтрокаРасшифровки);
	Иначе
		Для Каждого СтрокаТЧ Из Объект.РасшифровкаПлатежа Цикл
			ЗаполнитьИндексУИПОтличается(СтрокаТЧ);
		КонецЦикла;
...
&НаСервере
Процедура ОпределитьУИПОтличается()
	
	УИПОтличается = Не Объект.ОплатаПоЗаявкам И Объект.РасшифровкаПлатежа.Итог("ИндексУИПОтличается") <> 0;
	НастроитьЗависимыеЭлементыФормыНаСервере("УИПОтличается");
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьИндексУИПОтличается(СтрокаРасшифровки)
	
	Если ЗначениеЗаполнено(Объект.ИдентификаторПлатежа)
		И ЗначениеЗаполнено(СтрокаРасшифровки.ИдентификаторПлатежа)
		И СтрокаРасшифровки.ИдентификаторПлатежа <> Объект.ИдентификаторПлатежа Тогда
		СтрокаРасшифровки.ИндексУИПОтличается = 1;
	Иначе
		СтрокаРасшифровки.ИндексУИПОтличается = 0;
	КонецЕсли;
...
&НаКлиенте
Функция ФормыОплатыЗаявокНаБезналичнуюОплату()

	Если ПодключеныОбработчикиЛокализация Тогда
		Возврат ДенежныеСредстваКлиентСерверЛокализация.ФормыОплатыЗаявокНаБезналичнуюОплату();
	КонецЕсли;
	
	ФормыОплаты = Новый Массив;
	
	ФормыОплаты.Добавить(ПредопределенноеЗначение("Перечисление.ФормыОплаты.Безналичная"));
	ФормыОплаты.Добавить(ПредопределенноеЗначение("Перечисление.ФормыОплаты.ПустаяСсылка"));
...
Процедура Подключаемый_ТипПлатежногоДокументаПриИзмененииЛокализация(Элемент)
	...
	УстановитьМаксимальнуюДлинуНазначенияПлатежа(Элементы.НазначениеПлатежа, Объект.ТипПлатежногоДокумента);
...
&НаКлиенте
Процедура Подключаемый_ДатаПроведенияБанкомПриИзмененииЛокализация()

	ДатаПроведенияБанкомПриИзмененииЛокализацияНаСервере();

КонецПроцедуры

&НаСервере
Процедура ДатаПроведенияБанкомПриИзмененииЛокализацияНаСервере()
...
Процедура Подключаемый_ПеречислениеСотрудникуЧерезБанкПриИзмененииЛокализация(Элемент)
	...
	ПеречислениеСотрудникуЧерезБанкПриИзмененииНаСервереЛокализация();

КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_КассаПолучательПриИзмененииЛокализация(Элемент)

	КассаПолучательПриИзмененииЛокализацияНаСервере();
...
&НаСервере
Процедура ПеречислениеСотрудникуЧерезБанкПриИзмененииНаСервереЛокализация()
...
&НаСервере
Процедура КассаПолучательПриИзмененииЛокализацияНаСервере()
...
Процедура ПриЧтенииСозданииНаСервереЛокализация()
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.ДокументОснование)
			И ДенежныеСредстваСерверЛокализация.МассивОперацийПеречисленияВБюджет().Найти(Объект.ХозяйственнаяОперация) <> Неопределено
			И ЗначениеЗаполнено(Объект.Контрагент) Тогда
			ЗаполнитьРеквизитыПлатежаВБюджет();
		КонецЕсли;
...
Процедура ОпределитьВалютныйПлатежЛокализация()
	...
	ВалютныйПлатеж = ЗначениеЗаполнено(Объект.Валюта) 
...
Процедура ЗаполнитьРеквизитыПлательщика()
	...
	НаДату = ?(Объект.ПроведеноБанком, Объект.ДатаПроведенияБанком, Объект.Дата);
	ДенежныеСредстваСерверЛокализация.ЗаполнитьРеквизитыПлательщика(Объект, НаДату);
...
Процедура ОбновитьРеквизитыПлательщика()
	...
	НаДату = ?(Объект.ПроведеноБанком, Объект.ДатаПроведенияБанком, Объект.Дата);
	ДенежныеСредстваСерверЛокализация.ОбновитьРеквизитыПлательщика(Объект, НаДату);
	УстановитьНадписьРеквизитыПлательщика();
...
Функция ТекущиеРеквизитыПлательщика()
	...
	Результат.Вставить("Объект", Объект.БанковскийСчет);
...
Процедура ЗаполнитьРеквизитыПолучателя()
	...
	НаДату = ?(Объект.ПроведеноБанком, Объект.ДатаПроведенияБанком, Объект.Дата);
	ДанныеПолучателя = СписаниеБезналичныхДенежныхСредствЛокализация.ДанныеПолучателяХозяйственнойОперации(Объект);
	ДенежныеСредстваСерверЛокализация.ЗаполнитьРеквизитыПолучателя(ЭтотОбъект, НаДату, ДанныеПолучателя.Получатель, ДанныеПолучателя.СчетПолучателя);
	УстановитьНадписьРеквизитыПолучателя(ДанныеПолучателя);
	
КонецПроцедуры

&НаСервере
Процедура УстановитьНадписьРеквизитыПолучателя(ДанныеПолучателя)
...
Процедура ЗаполнитьПоПодобномуБюджетномуПлатежу()
	...
	Если Не Объект.ПеречислениеВБюджет
		...
		Или Не ЗначениеЗаполнено(Объект.ВидПеречисленияВБюджет) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоРанееСовершенномуПлатежу(РезультатВыбора)
	...
	СписокСтатусов = ПлатежиВБюджетКлиентСервер.ДоступныеСтатусыПлательщикаПоВидуПлатежа(
		Объект.ВидПеречисленияВБюджет, Объект.КодБК, Объект.Дата);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьМаксимальнуюДлинуНазначенияПлатежа(ЭлементНазначениеПлатежа, ТипПлатежногоДокумента)

	ДоступнаяДлина = ДенежныеСредстваКлиентСерверЛокализация.МаксимальнаяДлинаНазначенияПлатежа(ТипПлатежногоДокумента);
	ЭлементНазначениеПлатежа.ОграничениеТипа = Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(ДоступнаяДлина));
...
Если Не ПодключеныОбработчикиЛокализация И ПолучитьФункциональнуюОпцию("ПоддержкаПлатежейРФ") Тогда
	...
	ПодключаемыеОбработчикиСобытийЭлементов.Вставить("КассаПолучатель",             "ПриИзменении");
