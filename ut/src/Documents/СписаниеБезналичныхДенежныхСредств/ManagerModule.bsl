...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ЗапросМаксимальнаяГраница = Новый Запрос;
	ЗапросМаксимальнаяГраница.Текст = 
		"ВЫБРАТЬ
		|	МАКСИМУМ(СписаниеБезналичныхДенежныхСредств.Дата) КАК Дата
		|ИЗ
		|	Документ.СписаниеБезналичныхДенежныхСредств КАК СписаниеБезналичныхДенежныхСредств";
	
	ВыборкаДаты = ЗапросМаксимальнаяГраница.Выполнить().Выбрать();
	
	Если ВыборкаДаты.Следующий() Тогда
		ДатаПоследнейПорции = ВыборкаДаты.Дата;
	Иначе
		Возврат;
	КонецЕсли;
	...
	|	СписаниеБезналичныхДенежныхСредств.ИННПлательщика КАК ИННПлательщика,
	|	СписаниеБезналичныхДенежныхСредств.КПППлательщика КАК КПППлательщика,
	|	СписаниеБезналичныхДенежныхСредств.ТекстПлательщика КАК ТекстПлательщика
	|ИЗ
	|	Документ.СписаниеБезналичныхДенежныхСредств КАК СписаниеБезналичныхДенежныхСредств
	|ГДЕ
	...
	|	И СписаниеБезналичныхДенежныхСредств.Дата <= &ДатаПоследнейПорции
	|	И НЕ СписаниеБезналичныхДенежныхСредств.Ссылка В (&ИсключаемыеДокументы)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата УБЫВ";
	...
	Пока Истина Цикл
		...
		Запрос.УстановитьПараметр("ДатаПоследнейПорции", ДатаПоследнейПорции);
		Запрос.УстановитьПараметр("ИсключаемыеДокументы", ИсключаемыеДокументы);
		ПакетДокументов = Запрос.Выполнить().Выгрузить();
		...
		ЗапросДанных.УстановитьПараметр("ПроверяемыеДокументы", ПакетДокументов);
		ВыборкаДанныхПоДокументам = ЗапросДанных.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		
		РеквизитыПлательщикаПоДокументу = РеквизитыПлательщикаПоДокументам(ВыборкаДанныхПоДокументам);
		
		СписокДокументов = Новый Массив;
		
		Для Каждого ТекущийДокумент Из ПакетДокументов Цикл
			
			РеквизитыПлательщика = РеквизитыПлательщикаПоДокументу.Получить(ТекущийДокумент.Ссылка);
		...
		Если ПакетДокументов.Количество() < РазмерПакета Тогда
			Прервать;
		КонецЕсли;
		
		ПоследняяСтрокаПорции = ПакетДокументов.Получить(ПакетДокументов.Количество() - 1);
		
		Если ДатаПоследнейПорции <> ПоследняяСтрокаПорции.Дата Тогда
			ИсключаемыеДокументы = Новый Массив;
		КонецЕсли;
		
		ДатаПоследнейПорции = ПоследняяСтрокаПорции.Дата;
		
		Для Каждого СтрокаПакета Из ПакетДокументов Цикл
			
			Если СтрокаПакета.Дата = ДатаПоследнейПорции Тогда
				ИсключаемыеДокументы.Добавить(СтрокаПакета.Ссылка);
			КонецЕсли;
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	РеквизитыПлательщикаПоДокументу = РеквизитыПлательщикаПоДокументам(ВыборкаПоДокументам);
	...
	Пока Документ.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				РеквизитыПлательщика = РеквизитыПлательщикаПоДокументу.Получить(Документ.Ссылка);
...
Функция ТекстЗапросаРеквизитовПлательщикаСГруппировкойПоДокументам()
	...
	Возврат 
		...
		|	ИсходныеДанные.ПеречислениеВБюджет КАК ПеречислениеВБюджет,
		|	0 КАК Признак
		...
		|ВЫБРАТЬ
		|	ИсторияЗначенийРеквизитов.Документ КАК Документ,
		|	ИсторияЗначенийРеквизитов.ПеречислениеВБюджет КАК ПеречислениеВБюджет,
...
Функция РеквизитыПлательщикаПоДокументам(ВыборкаПоДокументам)
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Пока ДанныеПлательщика.Следующий() Цикл
			...
			ЭтоПеречислениеВБюджет = ДанныеПлательщика.ПеречислениеВБюджет;
		...
		Результат.Вставить(ВыборкаПоДокументам.Документ, РеквизитыПлательщика);
