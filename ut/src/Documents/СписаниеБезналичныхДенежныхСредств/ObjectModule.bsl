...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("БанковскийСчет")
			И Не ЗначениеЗаполнено(ДанныеЗаполнения.БанковскийСчет) Тогда
			ДанныеЗаполнения.БанковскийСчет = ДенежныеСредстваСервер.ТекущийБанковскийСчетОрганизации();
		КонецЕсли;
		
		Если ДанныеЗаполнения.Свойство("ИнструкцииБанку")
	...
	Если ЗначениеЗаполнено(ДоговорЭквайринга) Тогда
		РеквизитыДоговораЭквайринга = Справочники.ДоговорыЭквайринга.РеквизитыДоговора(ДоговорЭквайринга);
		СтатьяДвиженияДенежныхСредств = РеквизитыДоговораЭквайринга.СтатьяДвиженияДенежныхСредствВозврат;
		ЗначениеПриЗачислении = Перечисления.СпособыОтраженияЭквайринговойКомиссии.ПриЗачислении;
		ОтражатьКомиссию = (РеквизитыДоговораЭквайринга.СпособОтраженияКомиссии = ЗначениеПриЗачислении
			И РеквизитыДоговораЭквайринга.ВзимаетсяКомиссияПриВозврате);
		
		Если ОтражатьКомиссию Тогда
			Если РеквизитыДоговораЭквайринга.ФиксированнаяСтавкаКомиссии Тогда
				СтавкаКомиссии = РеквизитыДоговораЭквайринга.СтавкаКомиссии;
				СуммаКомиссии = ДенежныеСредстваКлиентСервер.РассчитатьСуммуКомиссии(СуммаДокумента, СтавкаКомиссии);
			КонецЕсли;
			ЗаполнитьЗначенияСвойств(ЭтотОбъект, РеквизитыДоговораЭквайринга,
				"СтатьяРасходов, АналитикаРасходов, НаправлениеДеятельности, ВестиУчетКомиссииВоВзаиморасчетах, ОбъектРасчетовКомиссии");
			Подразделение = РеквизитыДоговораЭквайринга.ПодразделениеРасходов;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(БанковскийСчет) Тогда
		УстановитьФлагОплатыПоЗаявкам(БанковскийСчет);
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		И ДанныеЗаполнения.Свойство("БанковскийСчет")
		И ЗначениеЗаполнено(ДанныеЗаполнения.БанковскийСчет) Тогда
		УстановитьФлагОплатыПоЗаявкам(ДанныеЗаполнения.БанковскийСчет);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	НеобязательноУказаниеЗаявки = (ДенежныеСредстваСервер.РазрешеныПлатежиБезУказанияЗаявок(ЭтотОбъект)
		Или НеКонтролироватьЗаполнениеЗаявки) И Не ОплатаПоЗаявкам;
...
Процедура ЗаполнитьПоОтчетуПоКомиссии(Знач ДокументОснование, ДанныеЗаполнения, КОплате = 0, Вознаграждение = Ложь)
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Если Не ЗначениеЗаполнено(ДанныеЗаполнения.БанковскийСчетПолучатель) Тогда
			ПараметрыСчета = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизации();
			ПараметрыСчета.Организация = ДанныеЗаполнения.ОрганизацияПолучатель;
			ПараметрыСчета.Валюта = ДанныеЗаполнения.Валюта;
			ПараметрыСчета.КорреспондирующийСчет = ДанныеЗаполнения.БанковскийСчет;
			ДанныеЗаполнения.БанковскийСчетПолучатель =
				Справочники.БанковскиеСчетаОрганизаций.ПолучитьБанковскийСчетПоУмолчанию(ПараметрыСчета);
		КонецЕсли;
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	СписаниеБезналичныхДенежныхСредствЛокализация.ОбработатьЗаполнениеРеквизитовПлательщика(ЭтотОбъект, ДанныеЗаполнения);
	...
	Если ТипЗнч(ДанныеЗаполнения) <> Тип("Структура")
		Или Не ДанныеЗаполнения.Свойство("КассаПолучатель")
		Или Не ЗначениеЗаполнено(ДанныеЗаполнения.КассаПолучатель) Тогда
		КассаПолучатель = Справочники.Кассы.ПолучитьКассуПоУмолчанию(Организация, Валюта);
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыПоРезультатуЗапроса(ДокументОснование, РезультатЗапроса, СуммаКОплате, ДанныеЗаполнения, ОрганизацияЗаполнения = Неопределено) Экспорт
	...
	Иначе
		...
		ДанныеЗаполнения.Вставить("ОплатаПоЗаявкам", Ложь);
		...
		Если ЗначениеЗаполнено(Выборка.Договор) И ТипЗнч(ЗаказыПоставщику) <> Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
			...
			Если Не ДанныеЗаполнения.ОплатаПоЗаявкам
				И Не ЗначениеЗаполнено(ДанныеЗаполнения.ИдентификаторПлатежа) Тогда
...
Процедура ЗаполнитьПоЗаявкеНаОбеспечение(Знач ДокументОснование, ДанныеЗаполнения)
	...
	СуммаКОплате = 0; // Не используется, т.к. расшифровка заполняется отдельно
	ЗаполнитьРеквизитыПоРезультатуЗапроса(ДокументОснование, ДанныеЗаявки.РезультатЗапросаШапки, СуммаКОплате, ДанныеЗаполнения);
...
Процедура УстановитьФлагОплатыПоЗаявкам(Счет)

	ДанныеСчетаОрганизации =
		ДенежныеСредстваПовтИсп.ПолучитьРеквизитыБанковскогоСчетаОрганизации(Счет);
