...
Процедура ЗаполнитьСписокРаспоряжений()
	...
	|	ДанныеДокумента.Ссылка.Партнер               КАК Партнер,
	|	ДанныеДокумента.Ссылка.Контрагент            КАК Контрагент,
	|	ДанныеДокумента.Ссылка.Соглашение            КАК Соглашение,
	...
	|	ДанныеДокумента.Ссылка.Менеджер              КАК Менеджер,
	|	ВЫРАЗИТЬ(ДанныеДокумента.Ссылка.Комментарий КАК СТРОКА(100)) КАК Комментарий
	|ИЗ
	|	Документ.КорректировкаРеализации.Расхождения КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.Ссылка В(&Распоряжения)
	|	И ДанныеДокумента.ВариантОтражения = ЗНАЧЕНИЕ(Перечисление.ВариантыОтраженияКорректировокРеализаций.ВернутьВПуть)
	|";
