...
Процедура ЗаполнитьНастройкиПолейФормы(Настройки) Экспорт
	...
	Финансы.НовыйОтбор(ГруппаИ, "Дополнительно.ВалютаРеглУчета", Новый ПолеКомпоновкиДанных("Дополнительно.ВалютаВзаиморасчетов"), , ВидСравненияКомпоновкиДанных.НеРавно);
	Финансы.НовыйОтбор(ГруппаИ, "Дополнительно.ВалютаРеглУчета", Новый ПолеКомпоновкиДанных("Валюта"), , ВидСравненияКомпоновкиДанных.НеРавно);
	...
	Финансы.НовыйОтбор(ГруппаИли, "Дополнительно.ВалютаРеглУчета", Новый ПолеКомпоновкиДанных("Дополнительно.ВалютаВзаиморасчетов"), , ВидСравненияКомпоновкиДанных.Равно);
	Финансы.НовыйОтбор(ГруппаИли, "Дополнительно.ВалютаРеглУчета", Новый ПолеКомпоновкиДанных("Валюта"), , ВидСравненияКомпоновкиДанных.Равно);
	
	Элемент.Свойства.Вставить("ФорматРедактирования", "ЧДЦ=4");
...
Функция ПараметрыВзаиморасчеты(ДанныеЗаполнения = Неопределено) Экспорт
	...
	ВлияющиеРеквизиты = "ХозяйственнаяОперация,ЗаявкаНаРасходованиеДенежныхСредств";
	...
	Если ОбщегоНазначения.ЗначениеСсылочногоТипа(ДанныеЗаполнения) Тогда
		СтруктураДанныеЗаполнения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			ДанныеЗаполнения, 
			"ХозяйственнаяОперация, ЗаявкаНаРасходованиеДенежныхСредств");
		ХозяйственнаяОперация = СтруктураДанныеЗаполнения.ХозяйственнаяОперация;
		ЗаявкаНаРасходованиеДенежныхСредств = СтруктураДанныеЗаполнения.ЗаявкаНаРасходованиеДенежныхСредств;
	ИначеЕсли ДанныеЗаполнения = Неопределено Тогда
		ХозяйственнаяОперация = Неопределено;
		ЗаявкаНаРасходованиеДенежныхСредств = Неопределено;
	Иначе
		ХозяйственнаяОперация = ДанныеЗаполнения.ХозяйственнаяОперация;
		ЗаявкаНаРасходованиеДенежныхСредств = ДанныеЗаполнения.ЗаявкаНаРасходованиеДенежныхСредств;
	КонецЕсли;
	...
	СтруктураПараметров.ОплатаПоЗаявкам                                  = ЗначениеЗаполнено(ЗаявкаНаРасходованиеДенежныхСредств);
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаКурсыВалютДокумента = 
		...
		|ИЗ
		|	Документ.ОперацияПоПлатежнойКарте КАК ДанныеДокументаШапка
		|ГДЕ
	...
	ТекстУменьшитьОплачивается = "
		...
		|	Таблица.Ссылка В (&Ссылка)
		|	И Таблица.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОплатыКлиенту)
		|	И Таблица.Ссылка.ЗаявкаНаРасходованиеДенежныхСредств <> ЗНАЧЕНИЕ(Документ.ЗаявкаНаРасходованиеДенежныхСредств.ПустаяСсылка)
		|	И ЕСТЬNULL(Таблица.Ссылка.ЗаявкаНаРасходованиеДенежныхСредств.КонтролироватьОплатуПоОбъектамРасчетов,ЛОЖЬ)
		|	И &ИспользоватьЗаявкиНаРасходованиеДенежныхСредств";
