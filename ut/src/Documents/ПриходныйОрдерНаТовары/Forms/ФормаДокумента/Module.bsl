...
Процедура СкладПриИзменении(Элемент)
	...
	СкладПомещениеПриИзмененииСервер(Элемент.Имя, Истина);
...
Процедура УстановитьУсловноеОформление()
	...
	КонтрольКачества.ДобавитьОтбор(ФлагНедоступен, "Объект.Товары.ЭтоУпаковочныйЛист", Истина);
	КонтрольКачества.ДобавитьОтбор(ФлагНедоступен, "ПоддерживаетсяКонтрольКачества", Ложь);
	КонтрольКачества.ДобавитьОтбор(ФлагНедоступен, "Объект.ДвижениеРанееПоступившейПартии", Истина);
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ПодлежитРазблокировке");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Ложь;
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ПоддерживаетсяКонтрольКачества = КонтрольКачества.ПоддерживаетсяКонтрольКачества(Объект.Распоряжение);
...
&НаСервере
Процедура СкладПомещениеПриИзмененииСервер(ИмяЭлемента, НепосредственноеИзменениеПоляСклад = Ложь)
...
//			Модифицированность = Истина;
			...
			ПараметрыОткрытия.Вставить("КонтролируемаяПартия",                 Объект.КонтролируемаяПартия);
			ПараметрыОткрытия.Вставить("ДвижениеРанееПоступившейПартии",       Объект.ДвижениеРанееПоступившейПартии);
...
Процедура ПроверитьКоличествоВДокументеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ВозвращаемыеПараметры <> Неопределено Тогда
		ИзменитьТабличнуюЧастьПоРезультатамПроверкиДоп(ВозвращаемыеПараметры, ?(КэшированныеЗначения = Неопределено, ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения(), КэшированныеЗначения));
		Модифицированность = Истина;
	КонецЕсли;
...
Процедура ИзменитьТабличнуюЧастьПоРезультатамПроверкиДоп(ВозвращаемыеПараметры, КэшированныеЗначения)
	...
	ОбрабатываемыеСтрокиПризнак = Новый Массив;
	...
	Для Каждого СтрокаИсточник Из ТаблицаТовары Цикл
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			ОбрабатываемыеСтрокиПризнак.Добавить(СтрокаТЧ);
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества",
		КонтрольКачестваКэш);
	СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества",
		КонтрольКачестваКэш);
	...
	СтруктураДействий.ЗаполнятьНазначения                    = Не (Объект.СкладскаяОперация = ПредопределенноеЗначение("Перечисление.СкладскиеОперации.ВозвратНеПринятыхТоваров"));
...
Процедура НоменклатураПриИзмененииСервер(КэшированныеЗначения)
	...
	СтруктураДействийСТекущейСтрокой.Вставить("ЗаполнитьЕдиницуИзмеренияОбъема",Новый Структура("Номенклатура, Упаковка", "ЕдиницаИзмеренияОбъема"));
	СтруктураДействийСТекущейСтрокой.Вставить("ПересчитатьВесОбъем");
	СтруктураДействийСТекущейСтрокой.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества",
		КонтрольКачестваКэш);
	СтруктураДействийСТекущейСтрокой.Вставить("ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества",
		КонтрольКачестваКэш);
	...
	УпаковочныеЛистыСервер.НоменклатураУпаковочныйЛистПриИзменении(ЭтотОбъект, Объект.Товары,
...
Процедура ВывестиДополнительнуюИнформациюПоНоменклатуре(Номенклатура = Неопределено , Характеристика = Неопределено, Упаковка = Неопределено)
	...
	Если ЗначениеЗаполнено(РеквизитыНоменклатуры.ФайлКартинки) Тогда
		Попытка
			АдресКартинки = РаботаСФайлами.ДанныеФайла(РеквизитыНоменклатуры.ФайлКартинки, УникальныйИдентификатор).СсылкаНаДвоичныеДанныеФайла;
		Исключение
			АдресКартинки = "";
...
Процедура УпаковочныйЛистПриИзмененииСервер(КэшированныеЗначения)
	...
	СтруктураДействийСДобавляемымиСтроками.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества",
		КонтрольКачестваКэш);
	СтруктураДействийСДобавляемымиСтроками.Вставить("ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества",
		КонтрольКачестваКэш);
...
Процедура СоздатьШтрихКод(Команда)
	...
	ЗначенияЗаполнения.Вставить("Серия",			Элементы.Товары.ТекущиеДанные.Серия);
...
Процедура ЗаполнитьСлужебныеРеквизитыТЧТовары(Перенумеровать = Истина, КэшированныеЗначения = Неопределено)
	...
	УпаковочныеЛистыСервер.ЗаполнитьСлужебныеРеквизиты(ЭтотОбъект, Объект.Товары, СтруктураДействий, ,Перенумеровать, КэшированныеЗначения);
	...
	РегистрыСведений.ШтрихкодыНоменклатуры.ПолучитьДанныеПоШтрихкодам(КэшированныеЗначения, МассивШтрихкодов);
	
	Если Объект.Товары.НайтиСтроки(Новый Структура("ЭтоУпаковочныйЛист", Истина)).Количество() > 0 Тогда
		Документы.УпаковочныйЛист.ДополнитьДанныеПоШтрихкодам(КэшированныеЗначения, МассивШтрихкодов);
	КонецЕсли;
	...
	Для Каждого СтрокаТЧ Из Объект.Товары Цикл
		Если Не ЗначениеЗаполнено(СтрокаТЧ.Штрихкод)
			Или ЗначениеЗаполнено(КэшированныеЗначения.Штрихкоды[СтрокаТЧ.Штрихкод])
				И Не КэшированныеЗначения.Штрихкоды[СтрокаТЧ.Штрихкод].НеизвестныйШтрихкод Тогда
...
Процедура ДобавитьСтрокиПоДокументамОтгрузки(МассивСсылок,КэшированныеЗначения)
	...
	СтруктураДействий.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", КонтрольКачестваКэш);
	СтруктураДействий.Вставить("ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества", КонтрольКачестваКэш);
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(СтрокиКОбработке, СтруктураДействий, Объект.Товары);
...
Процедура ЗаполнитьНаСервере(ВидЗаполнения, ДатаПоступления, КэшированныеЗначения)
	...
	УпаковочныеЛистыСервер.ПриЧтенииСозданииФормыСУпаковочнымиЛистами(ЭтотОбъект, Объект.Товары,
		Объект.Ссылка.Метаданные().ПредставлениеОбъекта);
		
	ТекущийОбъект = РеквизитФормыВЗначение("Объект");	
	СобытияФорм.ПриЧтенииНаСервере(ЭтотОбъект, ТекущийОбъект);
	
	ПриходныйОрдерНаТоварыЛокализация.УстановитьПризнакИспользованияМаркируемойПродукции(ЭтотОбъект);
	УстановитьВидимостьЭлементовУпаковкиПриМаркировке();
...
Процедура ЗаполнитьМногооборотнуюТаруИзХранилищаСервер(АдресТарыВХранилище,КэшированныеЗначения)
	...
	СтруктураДействийДобавленныеСтроки.Вставить("ЗаполнитьВесУпаковки",Новый Структура("Номенклатура, Упаковка", "ВесУпаковки"));
	СтруктураДействийДобавленныеСтроки.Вставить("ЗаполнитьОбъемУпаковки",Новый Структура("Номенклатура, Упаковка", "ОбъемУпаковки"));
	СтруктураДействийДобавленныеСтроки.Вставить("ЗаполнитьЕдиницуИзмеренияВеса",Новый Структура("Номенклатура, Упаковка", "ЕдиницаИзмеренияВеса"));
	СтруктураДействийДобавленныеСтроки.Вставить("ЗаполнитьЕдиницуИзмеренияОбъема",Новый Структура("Номенклатура, Упаковка", "ЕдиницаИзмеренияОбъема"));
	СтруктураДействийДобавленныеСтроки.Вставить("ПересчитатьВесОбъем");
	СтруктураДействийДобавленныеСтроки.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества",
		КонтрольКачестваКэш);
	СтруктураДействийДобавленныеСтроки.Вставить("ЗаполнитьПодлежитРазблокировкеДатаПодтвержденияКачества",
		КонтрольКачестваКэш);
	
	СтруктураДействийДобавленныеСтроки.Вставить(
		"ПересчитатьКоличествоУпаковок",
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.СтруктураПересчетаКоличестваУпаковок());
...
Процедура ПолучитьЗагруженныеТоварыИзХранилища(АдресТоваровВХранилище, КэшированныеЗначения)
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(СтрокиДляДополнения, СтруктураДействий, Объект.Товары);
...
Функция ПроверитьТоварыИПоместитьВоВременноеХранилище(АдресХранилища, ВыгруженаТолькоНеМаркируемаяПродукция = Ложь)
	...
	Если ПроверкаПройдена Тогда
		...
		Если ЕстьМаркируемаяПродукция 
			И ТабличнаяЧастьТовары.Колонки.Найти("ВидПродукцииИС") <> Неопределено Тогда
			
			НомерТекущейСтроки = ТабличнаяЧастьТовары.Количество()-1;
			Пока НомерТекущейСтроки >= 0 Цикл
				
				ВедетсяПоштучныйУчетПродукции = Ложь;
				ПриходныйОрдерНаТоварыЛокализация.ОпределениеПризнакаПоэкземплярногоУчетаПродукцииНаДату(ТабличнаяЧастьТовары[НомерТекущейСтроки].ВидПродукцииИС, Объект.Дата, ВедетсяПоштучныйУчетПродукции);
			
				Если ВедетсяПоштучныйУчетПродукции Тогда
