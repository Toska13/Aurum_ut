...
Процедура ЗаполнитьНаОснованииПретензии(ДанныеЗаполнения)
	...
	|	Претензия.ЭтоПретензияПоставщику	КАК КорреспонденцияСДоходами,
	|	ВЫБОР
	|		КОГДА Претензия.ЭтоПретензияПоставщику
	...
	|	ВЫБОР
	|		КОГДА Претензия.ЭтоПретензияПоставщику
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.НачислениеДебиторскойЗадолженности)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.НачислениеКредиторскойЗадолженности)
