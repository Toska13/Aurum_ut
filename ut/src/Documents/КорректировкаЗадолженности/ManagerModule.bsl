...
Функция ТекстЗапросаВременнаяТаблицаЗадолженность(Запрос, ТекстыЗапроса) Экспорт
	...
	|	ОбъектыРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности КАК РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности,
	...
	|				И ДанныеОбъектаРасчетов.УчетДоходов И НЕ ДанныеОбъектаРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности
	|			ИЛИ ДанныеОбъектаРасчетов.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСПоставщиком)
	|				И ДанныеОбъектаРасчетов.УчетРасчетовСПоставщиками И НЕ ДанныеОбъектаРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности
	|		ТОГДА
...
Функция ТекстЗапросаТаблицаРасчетыСКлиентами(Запрос, ТекстыЗапроса, Регистры)
	...
	|	СУММА(Таблица.Сумма) КАК КОплате,
	|	0 КАК КОтгрузке,
	|	
	...
	|	
	|	ВЫБОР
	|		КОГДА СУММА(Таблица.Сумма) = 0
	|			ИЛИ Таблица.Валюта = &ВалютаРегламентированногоУчета
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(Таблица.СуммаРегл) / СУММА(Таблица.Сумма) КАК ЧИСЛО(10, 4))
	...
	|	ВЫБОР
	|		КОГДА СУММА(Таблица.СуммаУпр) = 0
	|			ИЛИ Таблица.Валюта = &ВалютаУпр
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(Таблица.СуммаРегл) / СУММА(Таблица.СуммаУпр) КАК ЧИСЛО(10, 4))
	...
	|		Задолженность.НаправлениеДеятельностиРасчетов КАК НаправлениеДеятельности,
	|
	|		Задолженность.ОбъектРасчетов КАК ОбъектРасчетов,
	...
	|			И Задолженность.НаправлениеДеятельностиРасчетов = РегистрАналитикаУчетаПоПартнерам.НаправлениеДеятельности
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ
