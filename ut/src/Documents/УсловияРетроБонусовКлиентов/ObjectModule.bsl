...
Процедура ЗаполнитьПоУсловиюРетроБонусовКлиентов(ДанныеЗаполнения)
	...
	СтрокиКУдалению = Новый Массив; // Массив из ДокументТабличнаяЧастьСтрока.УсловияРетроБонусовКлиентов.СвойстваНоменклатуры
	...
	Если МаксимальнаяДата <> Дата(1, 1, 1) Тогда
		...
		Иначе
			...
			Если ИмяПредставлениеТаблицы.ИмяТаблицы <> "" Тогда
				...
				ПараметрыПроверки.ПроверятьСуммуПлана = Истина;
				ПараметрыПроверки.СуммаПлана = СуммаПлан;
