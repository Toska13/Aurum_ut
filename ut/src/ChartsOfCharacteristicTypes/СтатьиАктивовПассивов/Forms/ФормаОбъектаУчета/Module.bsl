...
Процедура ИнициализироватьЗначенияРеквизитовФормы()
	...
	Если ЗначениеЗаполнено(Объект.ИсточникДанных) Тогда
		...
		ПланыВидовХарактеристик.СтатьиАктивовПассивов.ИнициализироватьКомпоновщикНастроекИсточникомДанных(
			КомпоновщикНастроекИсточникаДанных, Объект.ИсточникДанных,, УникальныйИдентификатор);
		...
		ЗаполнитьТаблицуРесурсов("РесурсыУпр", МетаданныеИсточника);
		ЗаполнитьТаблицуРесурсов("РесурсыРегл", МетаданныеИсточника);
		ЗаполнитьТаблицуРесурсов("РесурсыВал", МетаданныеИсточника);
		ЗаполнитьТаблицуРесурсов("РесурсыКоличество", МетаданныеИсточника);
...
Процедура УправлениеВидимостьюЭлементовФормы()
	...
	Элементы.СтраницаПоказателиБюджетирования.Видимость = Ложь;
	...
	Если Элементы.СтраницаПоказателиБюджетирования.Видимость Тогда
		Элементы.ГруппаСоставПоказателей.Видимость = Истина;
		Если Показатели.Количество() = 1 Тогда
			Элементы.ОдинПоказатель.Видимость = Истина;
			Элементы.Показатели.Видимость = Ложь;
		Иначе
			Элементы.ОдинПоказатель.Видимость = Ложь;
			Элементы.Показатели.Видимость = Истина;
		КонецЕсли;
	Иначе
		Элементы.ГруппаСоставПоказателей.Видимость = Ложь;
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьТаблицуРесурсов(ИмяТаблицыРесурсов, МетаданныеИсточника)
	
	РесурсыИсточника = МетаданныеИсточника.Ресурсы;
	РеквизитыИсточника = МетаданныеИсточника.Реквизиты;
	ТаблицаРесурсов = ЭтотОбъект[ИмяТаблицыРесурсов]; // ДанныеФормыКоллекция
	
	Для Каждого СтрокаРесурса Из Объект[ИмяТаблицыРесурсов] Цикл
...
&НаСервере
Функция ОпределитьИспользованиеУчета(ИмяТаблицыРесурсов)
...
&НаСервере
Функция ПредставлениеРесурсов(ИмяТаблицыРесурсов)
