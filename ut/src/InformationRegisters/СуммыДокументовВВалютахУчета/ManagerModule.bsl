...
// 	НеПересчитыватьСуммыНДС - Булево - Если Истина, то поля СуммаНДСРегл и СуммаНДСУпр берутся из источника данных.
//
...
//
Процедура СформироватьПоДаннымДокумента(Запрос,
	ТекстыЗапроса,
	Регистры,
	ТекстЗапросаДанныхДокументов,
	НеПересчитыватьСуммыНДС = Ложь) Экспорт
...
Функция ПараметрыДляПроведенияДокумента(Документ, Свойства) Экспорт
	...
	Параметры.ВыполнениеЗаданийНеоперативнойОчереди = Истина;
...
Процедура ОбновитьДвиженияДокументов(РегистраторыСумм) Экспорт
	...
	Для Каждого Ссылка Из Регистраторы Цикл
		...
		Если ТипСсылки <> Тип("ДокументСсылка.РеализацияТоваровУслуг")
			И ТипСсылки <> Тип("ДокументСсылка.РеализацияУслугПрочихАктивов") Тогда
			// Регистры затрат добавляются к обновлению для всех документов, за исключением документов продаж, которые могут оформляться "в путь".
			// В документах реализации движения по себестоимости не зависят от взаиморасчетов, но в случае перепроведения есть риск изменения данных на дату отгрузки "в путь".
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьИсправлениеДокументов") Тогда
			ДокументыСторно = ИсправлениеДокументов.СторноДокументы(Ссылка);
...
Функция РассчитатьСуммыДокументовВВалютахУчета(МассивДокументов) Экспорт
	...
	ТаблицаДокументовДляПересчета = ТаблицаДокументовДляПересчетаСуммВВалютахУчета(МассивДокументов);
...
// Таблица документов для пересчета сумм в валютах учета.
// 
// Параметры:
//  МассивДокументов - Массив из ДокументСсылка - Массив документов
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Таблица документов для пересчета сумм в валютах учета
Функция ТаблицаДокументовДляПересчетаСуммВВалютахУчета(МассивДокументов) Экспорт
	...
	Возврат ТаблицаДокументовДляПересчета;
...
Процедура РассчитатьПоДаннымВзаиморасчетов(ТипРасчетов, МассивДокументов) Экспорт
	...
	ПараметрыРасчета = ПараметрыРасчетаДокументовПоДаннымВзаиморасчетов(ТипРасчетов, МассивДокументов);
	
	РезультатПоДокументам = ПараметрыРасчета.РезультатЗапросаПоДокументам;
	...
	Пока ВыборкаПоДокументу.Следующий() Цикл
		...
		РаспределитьСуммыПоДокументуПоДаннымВзаиморасчетов(ВыборкаПоДокументу, ПараметрыРасчета);

	КонецЦикла;
	
	ОбновитьДвиженияДокументов(ПараметрыРасчета.ДокументыКПересчету);
	
КонецПроцедуры

// Распределить суммы по документу по данным взаиморасчетов.
// 
// Параметры:
//  ВыборкаПоДокументу - ВыборкаИзРезультатаЗапроса - Выборка по документу
//  ПараметрыРасчетаДокументов - см. ПараметрыРасчетаДокументовПоДаннымВзаиморасчетов
//
Процедура РаспределитьСуммыПоДокументуПоДаннымВзаиморасчетов(ВыборкаПоДокументу, ПараметрыРасчетаДокументов) Экспорт
	
	ВалютаУправленческогоУчета = ПараметрыРасчетаДокументов.ВалютаУправленческогоУчета;
	...
	Пока ВыборкаПоОбъектуРасчетов.Следующий() Цикл
		...
		Если КурсПоДоговору И ВыборкаПоДокументу.Валюта = ВыборкаПоДокументу.ВалютаВзаиморасчетов Тогда
			КурсВалютыДокумента = 
				РаботаСКурсамиВалютУТ.ПолучитьКурсВалютыДоговораИзКэша(
					...
					ПараметрыРасчетаДокументов.КэшКурсовВалютДоговора);
		Иначе
			КурсВалютыДокумента = 
				РаботаСКурсамиВалютУТ.ПолучитьКурсВалютыИзКэша(
					ВыборкаПоДокументу.Валюта, 
					ВыборкаПоОбъектуРасчетов.ДатаКурса, 
					ПараметрыРасчетаДокументов.КэшКурсовВалют,
		...
		Иначе
			КурсВалютыДокументаДляБазыНДС = 
				РаботаСКурсамиВалютУТ.ПолучитьКурсВалютыИзКэша(
					ВыборкаПоДокументу.Валюта,
					?(ЗначениеЗаполнено(ВыборкаПоОбъектуРасчетов.ПериодБазыНДС),
						ВыборкаПоОбъектуРасчетов.ПериодБазыНДС,
						ВыборкаПоОбъектуРасчетов.Период),
					ПараметрыРасчетаДокументов.КэшКурсовВалют,
					ВалютаРегламентированногоУчета);
		КонецЕсли;
		...
		РезультатПоискаСуммДляРаспределения = ПараметрыРасчетаДокументов.ТаблицаСуммДляРаспределения.НайтиСтроки(СтруктураПоиска);
		...
		Пока ВыборкаПоТипуЗаписи.Следующий() Цикл
			...
			Если ВыборкаПоТипуЗаписи.ЭтоКорректировкаПриобретения 
				...
				СтрокаТаблицыКурсовНДСКорректировок = ПараметрыРасчетаДокументов.КурсыНДСКорректировок.Найти(ВыборкаПоТипуЗаписи.Ссылка, "Ссылка");
			...
			ИначеЕсли ВыборкаПоТипуЗаписи.ЭтоРеализацияСОтложеннымПереходомПраваСобственности Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Запись = ПараметрыРасчетаДокументов.ТаблицаСуммыДокумента.Добавить();
				...
				Если ПересчитыватьРегл Тогда
					Если ЗначениеЗаполнено(Запись.СтавкаНДС) Тогда
						...
						Если НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ЭлектронныеУслуги Тогда
							...
							Если Не Выборка.НеПересчитыватьСуммыНДС Тогда
				...
				Если Не Выборка.НеПересчитыватьСуммыНДС Тогда
					...
					Если ПересчитыватьУпр
						Или ПересчитыватьРегл Тогда
						...
						Иначе
							...
							Запись.СуммаНДСУпр = ?(КурсВалютыУпрСУчетомЗачтенныхАвансов = 0,
								0,
	...
	ПривестиСуммыДокумента(ПараметрыРасчетаДокументов.ДанныеГрафиковОплат,
		ПараметрыРасчетаДокументов.ТаблицаСуммыДокумента, ПараметрыРасчетаДокументов.КэшКурсовВалют,
		ПараметрыРасчетаДокументов.КэшКурсовВалютДоговора, ВыборкаПоДокументу.Ссылка);
	ЗаписатьДвиженияПроцентныхРасходовДисконтирования(ПараметрыРасчетаДокументов.ДанныеГрафиковОплат,
		ВыборкаПоДокументу.Ссылка);
	...
	НаборЗаписей.Загрузить(ПараметрыРасчетаДокументов.ТаблицаСуммыДокумента);
	НаборЗаписей.ДополнительныеСвойства.Вставить("АктуализацияСуммДокументовВВалютахУчета");
	НаборЗаписей.Записать();
		
	ПараметрыРасчетаДокументов.ТаблицаСуммыДокумента.Очистить();
...
// Параметры расчета документов по данным взаиморасчетов.
// 
// Параметры:
//  ТипРасчетов - Строка - Тип расчетов
//  МассивДокументов - Массив Из ДокументСсылка - Массив документов
// 
// Возвращаемое значение:
//  Структура - Параметры расчета документов по данным взаиморасчетов:
// * РезультатЗапросаПоДокументам - РезультатЗапроса - Результат запроса по документам
// * ДокументыКПересчету - Массив Из ДокументСсылка - Документы к пересчету
// * ТаблицаСуммДляРаспределения - см. СуммыДокументовДляПострочногоРаспределения 
// * КурсыНДСКорректировок - ТаблицаЗначений - Таблица курсов НДС корректировок:
// ** Ссылка - ДокументСсылка - Ссылка на документ
// ** КурсНДСКорректировки - Число - Значение курса
// * ТаблицаСуммыДокумента - ТаблицаЗначений - Структура регистра сведений "СуммыДокументовВВалютахУчета":
// ** СуммаСНДСРегл  - Число
// ** СуммаСНДСУпр - Число
// ** СуммаДляБазыНДС - Число
// * ДанныеГрафиковОплат - см. ГрафикиОплатыДляДисконтирования
// * КэшКурсовВалют - см. РаботаСКурсамиВалютУТ.ИнициализироватьКэшКурсовВалют
// * КэшКурсовВалютДоговора - см. РаботаСКурсамиВалютУТ.ИнициализироватьКэшКурсовВалютДоговоров
// * ВалютаУправленческогоУчета - СправочникСсылка.Валюты, Произвольный - 
//
Функция ПараметрыРасчетаДокументовПоДаннымВзаиморасчетов(ТипРасчетов, МассивДокументов) Экспорт
	...
	|	ВЫБОР 
	|		КОГДА ТИПЗНАЧЕНИЯ(ДанныеРегистра.Регистратор) = ТИП(Документ.РеализацияТоваровУслуг)
	|			 И ВЫРАЗИТЬ(ДанныеРегистра.Регистратор КАК Документ.РеализацияТоваровУслуг).ХозяйственнаяОперация В (
	...
	|			ТОГДА ИСТИНА 
	|		КОГДА ТИПЗНАЧЕНИЯ(ДанныеРегистра.Регистратор) = ТИП(Документ.РеализацияУслугПрочихАктивов)
	|			 И ВЫРАЗИТЬ(ДанныеРегистра.Регистратор КАК Документ.РеализацияУслугПрочихАктивов).ХозяйственнаяОперация В (
	|							ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияОСсОтложеннымПереходомПрав))			
	|			ТОГДА ИСТИНА 
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоРеализацияСОтложеннымПереходомПраваСобственности,
	|	ВЫБОР 
	...
	|	ДанныеРегистра.НеПересчитыватьСуммыНДС    КАК НеПересчитыватьСуммыНДС,
	|	ВЫБОР
	...
	|	ТаблицаДокументов.ЭтоРеализацияСОтложеннымПереходомПраваСобственности КАК ЭтоРеализацияСОтложеннымПереходомПраваСобственности,  
	|
	...
	|	МАКСИМУМ(ЭтоРеализацияСОтложеннымПереходомПраваСобственности), 	
	...
	ДанныеКРасчету = Новый Структура;
	ДанныеКРасчету.Вставить("РезультатЗапросаПоДокументам", РезультатПоДокументам);
	ДанныеКРасчету.Вставить("ДокументыКПересчету", ДокументыКПересчету);
	ДанныеКРасчету.Вставить("ТаблицаСуммДляРаспределения", ТаблицаСуммДляРаспределения);
	ДанныеКРасчету.Вставить("КурсыНДСКорректировок", КурсыНДСКорректировок);
	ДанныеКРасчету.Вставить("ТаблицаСуммыДокумента", ТаблицаСуммыДокумента);
	ДанныеКРасчету.Вставить("ДанныеГрафиковОплат", ДанныеГрафиковОплат);
	ДанныеКРасчету.Вставить("КэшКурсовВалют", КэшКурсовВалют);
	ДанныеКРасчету.Вставить("КэшКурсовВалютДоговора", КэшКурсовВалютДоговора);
	ДанныеКРасчету.Вставить("ВалютаУправленческогоУчета", ВалютаУправленческогоУчета);
...
Функция ДанныеКФормированиюЗаданийНеоперативногоДопроведенияДокументов(НаборЗаписей, ВидыЗаданий,
	...
	Запрос.Текст = "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	// Для документов "Реализация товаров и услуг" и "Реализация услуг и прочих активов", по которым выполнен переход прав собственности,
	// период формирования заданий должен соответствовать дате перехода права собственности
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДанныеКОбработке.Регистратор) = ТИП(Документ.РеализацияТоваровУслуг)
	|				И ВЫРАЗИТЬ(ДанныеКОбработке.Регистратор КАК Документ.РеализацияТоваровУслуг).Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.Отгружено)
	|				И ВЫРАЗИТЬ(ДанныеКОбработке.Регистратор КАК Документ.РеализацияТоваровУслуг).ДатаПереходаПраваСобственности <> ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ВЫРАЗИТЬ(ДанныеКОбработке.Регистратор КАК Документ.РеализацияТоваровУслуг).ДатаПереходаПраваСобственности
	|
	|		КОГДА ТИПЗНАЧЕНИЯ(ДанныеКОбработке.Регистратор) = ТИП(Документ.РеализацияУслугПрочихАктивов)
	|				И ВЫРАЗИТЬ(ДанныеКОбработке.Регистратор КАК Документ.РеализацияУслугПрочихАктивов).Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.Отгружено)
	|				И ВЫРАЗИТЬ(ДанныеКОбработке.Регистратор КАК Документ.РеализацияУслугПрочихАктивов).ДатаПереходаПраваСобственности <> ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ВЫРАЗИТЬ(ДанныеКОбработке.Регистратор КАК Документ.РеализацияУслугПрочихАктивов).ДатаПереходаПраваСобственности
	|
	|		ИНАЧЕ ДанныеКОбработке.Период
	|
	|	КОНЕЦ КАК ПериодЗадания,
	|
	|	ДанныеКОбработке.Регистратор КАК Документ,
	...
	|		ПО РеестрДокументов.Ссылка = ДанныеКОбработке.Регистратор
	|ГДЕ
	|	РеестрДокументов.Организация <> ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)";
	
	ДанныеКФормированиюЗаданий = Запрос.Выполнить().Выгрузить();
	...
	Для Каждого ТекущийВидЗадания Из ВидыЗаданий Цикл
		...
		Если НаборЗаписей.ДополнительныеСвойства.Свойство("АктуализацияСуммДокументовВВалютахУчета")
			И ТекущийВидЗадания = Справочники.ВидыНеоперативныхЗаданий.АктуализацияСуммДокументовВВалютахУчета Тогда
			Продолжить;
		КонецЕсли;
...
// Выполняет проверку условий и запускает выполнение заданий неоперативной очереди в транзакции проведения документа.
// 
// Параметры:
//  Документ - ДокументОбъект - Проводимый документ
Процедура ПроверитьВыполнитьЗаданияНеоперативнойОчереди(Документ) Экспорт
	
	Если ВыполнятьРасчетСуммВТранзакцииПроведенияДокумента(Документ) Тогда
		НеоперативнаяОчередьЗаданий.ВыполнитьЗаданияНеоперативнойОчередиВТранзакцииПроведенияДокумента(Документ.Ссылка,
...
Функция ТекстЗапросаСравненияДокументовДляПересчета()
	...
	|
	|ИМЕЮЩИЕ
	|	(ВЫРАЗИТЬ(СУММА(ВложенныйЗапрос.СуммаУвеличенияРеглРасчеты) КАК ЧИСЛО(30,2)) <> СУММА(ВложенныйЗапрос.СуммаУвеличенияРегл)
	|	ИЛИ ВЫРАЗИТЬ(СУММА(ВложенныйЗапрос.СуммаУвеличенияУпрРасчеты) КАК ЧИСЛО(30,2)) <> СУММА(ВложенныйЗапрос.СуммаУвеличенияУпр)
	|	ИЛИ ВЫРАЗИТЬ(СУММА(ВложенныйЗапрос.СуммаУменьшенияРеглРасчеты) КАК ЧИСЛО(30,2)) <> СУММА(ВложенныйЗапрос.СуммаУменьшенияРегл)
	|	ИЛИ ВЫРАЗИТЬ(СУММА(ВложенныйЗапрос.СуммаУменьшенияУпрРасчеты) КАК ЧИСЛО(30,2)) <> СУММА(ВложенныйЗапрос.СуммаУменьшенияУпр))
	|";
	...
	Попытка
		...
		Для Каждого ОписаниеОбновления Из СоставОбновления Цикл
			Если Таблицы.Свойство("Таблица" + ОписаниеОбновления.Ключ, Таблица) И ЗначениеЗаполнено(Таблица) Тогда
				...
				Набор = ОписаниеОбновления.Значение; // РегистрНакопленияНаборЗаписей - 
...
Процедура ПересчитатьТаблицуВВалютыУчетаПострочно(ВыборкаЗнак, СуммыДокументовВВалютахУчета, КэшКурсовВалют, КэшКурсовВалютДоговора)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.НеПересчитыватьСуммыНДС Тогда
			
			РассчитатьСуммуНДСРегл = Ложь;
			СуммаНДСРегл =  Выборка.СуммаНДСРегл;
		...
		ИначеЕсли Выборка.НеПересчитыватьСуммыНДС Тогда
...
// Рассчитать дисконтирование.
// 
// Параметры:
//  МассивДокументов - Массив из ДокументСсылка - Список документов к расчету
// 
// Возвращаемое значение:
//  Массив из ДокументСсылка - Документы к пересчету
//
Функция РассчитатьДисконтирование(МассивДокументов) Экспорт
	
	ПараметрыДляРасчетаДисконтирования = ПараметрыДляРасчетаДисконтированияПоДокументам(МассивДокументов);
	ВыборкаПоДокументам = ПараметрыДляРасчетаДисконтирования.ВыборкаПоДокументам;
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		РассчитатьДисконтированиеПоДокументу(ВыборкаПоДокументам, ПараметрыДляРасчетаДисконтирования, ТаблицаСуммыДокумента, ДокументыКПересчету);
...
Функция ПривестиСуммыДокумента(ДанныеГрафиковОплат, ТаблицаСуммыДокумента, КэшКурсовВалют, КэшКурсовВалютДоговора, Документ)
	...
	Если ДанныеГрафиковОплат <> Неопределено 
		...
		Если СтруктураДанныхДокумента.Свойство("ОчиститьНачислениеПроцентов")
			И СтруктураДанныхДокумента.ОчиститьНачислениеПроцентов Тогда
			// Необходимо записать набор, если дисконтирование уже было записано, но больше не применяется 
			ЕстьИзменения = Истина;
		ИначеЕсли Не СтруктураДанныхДокумента.ЭтоОтрицательнаяКорректировка Тогда
...
// Параметры для расчета дисконтирования по документам.
// 
// Параметры:
//  МассивДокументов - Массив из ДокументСсылка - Массив документов
// 
// Возвращаемое значение:
//  Структура - Параметры для расчета дисконтирования по документам:
// * ВыборкаПоДокументам - ВыборкаИзРезультатаЗапроса - Данные по документу
// * ДанныеГрафиковОплат - см. ГрафикиОплатыДляДисконтирования
// * КэшКурсовВалют - см. РаботаСКурсамиВалютУТ.ИнициализироватьКэшКурсовВалют
// * КэшКурсовВалютДоговора - см. РаботаСКурсамиВалютУТ.ИнициализироватьКэшКурсовВалютДоговоров
//
Функция ПараметрыДляРасчетаДисконтированияПоДокументам(МассивДокументов) Экспорт
	...
	ПараметрыДляРасчета.Вставить("ВыборкаПоДокументам", ВыборкаПоДокументу);
	ПараметрыДляРасчета.Вставить("ДанныеГрафиковОплат", ДанныеГрафиковОплат);
	ПараметрыДляРасчета.Вставить("КэшКурсовВалют", КэшКурсовВалют);
	ПараметрыДляРасчета.Вставить("КэшКурсовВалютДоговора", КэшКурсовВалютДоговора);
	
	Возврат ПараметрыДляРасчета;
	
КонецФункции

// Рассчитать дисконтирование по документу.
// 
// Параметры:
//  ДанныеПоДокументу - ВыборкаИзРезультатаЗапроса - Данные по документу
//  ПараметрыРасчетаДисконтирования - см. ПараметрыДляРасчетаДисконтированияПоДокументам
//  ТаблицаСуммыДокумента - Неопределено, ТаблицаЗначений - Таблица суммы документа
//  ДокументыКПересчету - Неопределено, Массив Из ДокументСсылка - Документы к пересчету
//
Процедура РассчитатьДисконтированиеПоДокументу(ДанныеПоДокументу, ПараметрыРасчетаДисконтирования, ТаблицаСуммыДокумента = Неопределено, ДокументыКПересчету = Неопределено) Экспорт
	
	Если ТаблицаСуммыДокумента = Неопределено Тогда
		ТаблицаСуммыДокумента = СоздатьНаборЗаписей().ВыгрузитьКолонки();
	КонецЕсли;
	
	Если ДокументыКПересчету = Неопределено Тогда
		ДокументыКПересчету = Новый Массив;
	КонецЕсли;
	
	Выборка = ДанныеПоДокументу.Выбрать();
	...
	ЕстьИзменения = ПривестиСуммыДокумента(ПараметрыРасчетаДисконтирования.ДанныеГрафиковОплат, ТаблицаСуммыДокумента, ПараметрыРасчетаДисконтирования.КэшКурсовВалют,
		ПараметрыРасчетаДисконтирования.КэшКурсовВалютДоговора, ДанныеПоДокументу.Ссылка);
	ЗаписатьДвиженияПроцентныхРасходовДисконтирования(ПараметрыРасчетаДисконтирования.ДанныеГрафиковОплат, ДанныеПоДокументу.Ссылка, ЕстьИзменения);
	Если ЕстьИзменения Тогда
		НаборЗаписей = СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Регистратор.Установить(ДанныеПоДокументу.Ссылка);
		НаборЗаписей.Загрузить(ТаблицаСуммыДокумента);
		НаборЗаписей.ДополнительныеСвойства.Вставить("АктуализацияСуммДокументовВВалютахУчета");
		НаборЗаписей.Записать();
		ДокументыКПересчету.Добавить(ДанныеПоДокументу.Ссылка);
	КонецЕсли;
...
// При оперативном распределении взаиморасчетов и использовании метода оценки стоимости товаров "Среднескользящая" -
// суммы документа в валютах учета необходимо рассчитывать в транзакции проведения документа.
// 
// Параметры:
//  Документ - ДокументОбъект - Проводимый документ
// 
// Возвращаемое значение:
//  Булево - Выполнять расчет сумм в валютах учета в транзакции проведения документа
Функция ВыполнятьРасчетСуммВТранзакцииПроведенияДокумента(Документ)
	
	РассчитыватьСуммыДокументаВТранзакции = Ложь;
	
	НоваяАрхитектураВзаиморасчетов = ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов");

	Если НоваяАрхитектураВзаиморасчетов И Не Документ.ОбменДанными.Загрузка Тогда
		
		СвойстваДвиженийМеханизмов = Документ.ДополнительныеСвойства.ПроведениеДокументов.СвойстваДвиженийМеханизмов;

		Если СвойстваДвиженийМеханизмов.Свойство("Взаиморасчеты") 
			И СвойстваДвиженийМеханизмов.Взаиморасчеты.Свойство("ВыполнятьРаспределениеВзаиморасчетовВТранзакции") Тогда
			ВыполнятьРаспределениеВзаиморасчетовВТранзакции = СвойстваДвиженийМеханизмов.Взаиморасчеты.ВыполнятьРаспределениеВзаиморасчетовВТранзакции;
		Иначе
		...
		Если ВыполнятьРаспределениеВзаиморасчетовВТранзакции Тогда
			
			// Получим таблицу с организациями и периодами
			Запрос = Новый Запрос;
			Запрос.Текст = "
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	НАЧАЛОПЕРИОДА(Задания.ПериодЗадания, МЕСЯЦ) КАК Период,
			|	Задания.Организация КАК Организация
			|ИЗ
			|	РегистрСведений.ЗаданияНеоперативногоДопроведенияДокументов КАК Задания
			|ГДЕ
			|	Задания.ВидЗадания = ЗНАЧЕНИЕ(Справочник.ВидыНеоперативныхЗаданий.АктуализацияСуммДокументовВВалютахУчета)
			...
			Пока Выборка.Следующий() Цикл
				...
				ПараметрыУчетнойПолитики = НастройкиНалоговУчетныхПолитик.ДействующиеПараметрыНалоговУчетныхПолитикНаДату(
					"УчетнаяПолитикаФинансовогоУчета", Выборка.Организация, Выборка.Период);

				Если ПараметрыУчетнойПолитики <> Неопределено 
					И ПараметрыУчетнойПолитики.МетодОценкиСтоимостиТоваров = Перечисления.МетодыОценкиСтоимостиТоваров.Среднескользящая Тогда
					
					РассчитыватьСуммыДокументаВТранзакции = Истина;
					Прервать;
	...
	Возврат РассчитыватьСуммыДокументаВТранзакции;
	
КонецФункции

// Выполняет поиск проблемных документов, по которым суммы в валютах учета не совпадают с движениями по регистрам взаиморасчетов, 
// помещает найденные регистраторы во временную таблицу "ПроблемныеДокументыСуммыВВалютахУчета".
// 
// Поиск проблемных документов выполняется по списку организаций в закрываемом периоде, который задается в параметрах закрытия месяца.
...
// 
// Параметры:
//  ВременныеТаблицыПроверокРасхождений - МенеджерВременныхТаблиц - Содержит временные таблицы с проверками расхождений. В менеджере 
//  временных таблиц должна присутствовать таблица "ВТ_ПериодыЗакрытия" с данными о незакрытых периодах в рамках операции закрытия месяца.
//  ОкончаниеПериодаЗакрытия - Дата - Окончание периода закрытия, указанного в параметрах закрытия месяца.
Процедура НайтиРегистраторыСРасхождениямиВДвиженияхСуммВВалютахУчетаИРегистрахВзаиморасчетов(ВременныеТаблицыПроверокРасхождений, ОкончаниеПериодаЗакрытия) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = ВременныеТаблицыПроверокРасхождений;
	Запрос.УстановитьПараметр("КонецПериода", ОкончаниеПериодаЗакрытия);
	Запрос.Текст = "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	РасчетыСКлиентами.Регистратор КАК Регистратор,
	|	РасчетыСКлиентами.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
	|	РасчетыСКлиентами.Период КАК Период
	|ПОМЕСТИТЬ РегистраторыРегистровВзаиморасчетов
	|ИЗ
	...
	|	И РасчетыСКлиентами.АналитикаУчетаПоПартнерам.Организация В
	|		(ВЫБРАТЬ
	|			ВТ.Организация
	|		ИЗ
	|			ВТ_ПериодыЗакрытия КАК ВТ)
	...
	|		И РасчетыСПоставщиками.Период <= &КонецПериода
	...
	|	И РасчетыСПоставщиками.АналитикаУчетаПоПартнерам.Организация В
	|		(ВЫБРАТЬ
	|			ВТ.Организация
	|		ИЗ
	|			ВТ_ПериодыЗакрытия КАК ВТ)
	...
	|ИЗ
	|	РегистраторыРегистровВзаиморасчетов КАК Расчеты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ РегистраторыРегистровВзаиморасчетов";
	
	СписокРегистраторов = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ДокументРегистратор");

	ЗапросПроблемныеДокументы = Новый Запрос;
	ЗапросПроблемныеДокументы.Текст = ТекстЗапросаДокументовДляПересчета();
	ЗапросПроблемныеДокументы.УстановитьПараметр("МассивДокументов", СписокРегистраторов);
	ЗапросПроблемныеДокументы.УстановитьПараметр("ЭтоПроверка", Истина);
	ЗапросПроблемныеДокументы.УстановитьПараметр("ВалютаУпрУчета", Константы.ВалютаУправленческогоУчета.Получить());
	ЗапросПроблемныеДокументы.УстановитьПараметр("НоваяАрхитектураВзаиморасчетов", Истина);
	ЗапросПроблемныеДокументы.УстановитьПараметр("ПоВсемДокументам", Ложь);

	ПроблемныеДокументы = ЗапросПроблемныеДокументы.Выполнить().Выгрузить().ВыгрузитьКолонку("РасчетныйДокумент");
	...
	|	РеестрДокументов.Организация КАК Организация
	|ПОМЕСТИТЬ ПроблемныеДокументыСуммыВВалютахУчета
	|ИЗ
	|	РегистрСведений.РеестрДокументов КАК РеестрДокументов
	|ГДЕ
	|	РеестрДокументов.Организация В
	|		(ВЫБРАТЬ
	|			ВТ.Организация
	|		ИЗ
	|			ВТ_ПериодыЗакрытия КАК ВТ)
