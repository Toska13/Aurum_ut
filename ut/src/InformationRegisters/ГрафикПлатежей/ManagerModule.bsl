...
//        * ОбъектРасчетов - ОпределяемыйТип.ОбъектРасчетов - объект, в разрезе которого ведутся расчеты
//    Очередь - Число - номер очереди при обновлении данных
//    ПропуститьОбновлениеГрафика - Булево.
//
Процедура РассчитатьГрафикПлатежейПоРасчетамСПоставщиками(ТаблицаОбъектовОплаты, Очередь = Неопределено,
	ПропуститьОбновлениеГрафика = Ложь) Экспорт
	
	НоваяАрхитектураВзаиморасчетов = ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов");
	
	НеОбрабатываемые = НеобрабатываемыеДокументы();
	СтрокиКУдалению = Новый Массив;
	ОбъектыОплатыДоговоры = Новый Массив;
	Для каждого СтрокаТаблицы Из ТаблицаОбъектовОплаты Цикл
		Если НеОбрабатываемые.Найти(ТипЗнч(СтрокаТаблицы.ОбъектОплаты)) <> Неопределено Тогда
			СтрокиКУдалению.Добавить(СтрокаТаблицы);
		КонецЕсли;
		
		Если ТипЗнч(СтрокаТаблицы.ОбъектОплаты) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
			И НЕ ПропуститьОбновлениеГрафика Тогда
			ОбъектыОплатыДоговоры.Добавить(СтрокаТаблицы.ОбъектОплаты);
		КонецЕсли;
	КонецЦикла;
	Для каждого СтрокаТаблицы Из СтрокиКУдалению Цикл
		ТаблицаОбъектовОплаты.Удалить(СтрокаТаблицы);
	...
	Если ОбъектыОплатыДоговоры.Количество() Тогда
		ПролонгацияРегулярныхПлатежей(ОбъектыОплатыДоговоры);
	КонецЕсли;
...
Процедура РассчитатьГрафикПлатежейПоДенежнымСредствамВПути(ТаблицаОбъектовОплаты, Очередь = Неопределено) Экспорт
	...
	|	ДенежныеСредства.Валюта КАК ВалютаОтправителя
	|ПОМЕСТИТЬ ОстаткиВПути
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДоговорыЭквайринга.Ссылка КАК ОбъектОплаты,
	|	ДоговорыЭквайринга.Номер КАК Номер,
	|	ДоговорыЭквайринга.Дата КАК Дата
	|ИЗ
	|	Справочник.ДоговорыЭквайринга КАК ДоговорыЭквайринга
	|ГДЕ
	|	ДоговорыЭквайринга.Ссылка В(&ОбъектОплаты)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОстаткиВПути.ОбъектОплаты КАК ОбъектОплаты,
	|	ОстаткиВПути.БанковскийСчетКасса КАК БанковскийСчетКасса,
	|	ОстаткиВПути.ДатаПлатежа КАК ДатаПлатежа,
	|	ОстаткиВПути.ПлательщикПолучатель КАК ПлательщикПолучатель,
	|	ОстаткиВПути.ПоступлениеСписание КАК ПоступлениеСписание,
	|	ОстаткиВПути.ТипСуммы КАК ТипСуммы,
	|	ОстаткиВПути.Валюта КАК Валюта,
	|	ОстаткиВПути.Сумма КАК Сумма,
	|	ОстаткиВПути.Организация КАК Организация,
	|	ОстаткиВПути.Подразделение КАК Подразделение,
	|	ОстаткиВПути.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ОстаткиВПути.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	ОстаткиВПути.СтатьяДвиженияДенежныхСредств КАК СтатьяДвиженияДенежныхСредств,
	|	ОстаткиВПути.ФормаОплаты КАК ФормаОплаты,
	|	ОстаткиВПути.ОбластьПланирования КАК ОбластьПланирования,
	|	ОстаткиВПути.СуммаДокумента КАК СуммаДокумента,
	|	ОстаткиВПути.КорВалюта КАК КорВалюта,
	|	ОстаткиВПути.ВалютаОтправителя КАК ВалютаОтправителя
	|ИЗ
	|	ОстаткиВПути КАК ОстаткиВПути
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|				(ВЫБРАТЬ
	|					ОстаткиВПути.ВалютаОтправителя,
	|					&БазоваяВалюта
	|				ИЗ
	|					ОстаткиВПути КАК ОстаткиВПути
	|				ОБЪЕДИНИТЬ
	...
	|					&БазоваяВалюта
	|				ИЗ
	|					ОстаткиВПути КАК ОстаткиВПути)) КАК ОтносительныеКурсыВалютСрезПоследних
	|";
	...
	ВыборкаСведенийОДоговорах = Результаты[Результаты.ВГраница() - 2].Выбрать();
	Выборка                   = Результаты[Результаты.ВГраница() - 1].Выбрать();
	КурсыВалют                = Результаты[Результаты.ВГраница()].Выгрузить();
	...
	Для каждого СтрокаОбъектОплаты Из ТаблицаОбъектовОплаты Цикл
		...
		ЭтоДоговорЭквайринга = ВыборкаСведенийОДоговорах.НайтиСледующий(Новый Структура("ОбъектОплаты", СтрокаОбъектОплаты.ОбъектОплаты));
		...
		Пока Выборка.НайтиСледующий(СтруктураПоиска) Цикл
			...
			Если ЭтоДоговорЭквайринга Тогда
				ЗаполнитьЗначенияСвойств(Запись, ВыборкаСведенийОДоговорах);
			КонецЕсли;
			
			Если Выборка.ТипСуммы = Перечисления.ВидыПереводовДенежныхСредств.ПриобретениеВалюты
				Или Выборка.ТипСуммы = Перечисления.ВидыПереводовДенежныхСредств.РеализацияВалюты Тогда
				
				ПараметрыКурсаОтправителя = КурсВалюты(КурсыВалют, Выборка.ВалютаОтправителя);
				ПараметрыКурсаПолучателя = КурсВалюты(КурсыВалют, Выборка.Валюта);
				
				Запись.Сумма = РаботаСКурсамиВалютУТ.ПересчитатьПоКурсу(Запись.Сумма, ПараметрыКурсаОтправителя, ПараметрыКурсаПолучателя);
				
			КонецЕсли;
			
			Запись.Идентификатор = Новый УникальныйИдентификатор();
			ЗаполнитьИсходныеРеквизиты(Запись);
		КонецЦикла;
		Выборка.Сбросить();
		ВыборкаСведенийОДоговорах.Сбросить();
...
Процедура РассчитатьГрафикПлатежейПоОжидаемымПоступлениямДенежныхСредств(ДатыРасчета, НедействительныеДокументыОплаты = Неопределено, Очередь = Неопределено, ОбъектыОплаты = Неопределено) Экспорт
	...
	|	ДанныеДокумента.Контрагент КАК ПлательщикПолучатель
	...
	|	КлючиОжидаемыхПоступлений.ПлательщикПолучатель КАК ПлательщикПолучатель
	...
	|	ОжидаемыеПоступления.ПлательщикПолучатель КАК ПлательщикПолучатель,
	...
	|			И ОжидаемыеПоступления.ПлательщикПолучатель = РегистрАналитикаУчетаПоПартнерам.Контрагент
	|ГДЕ
	...
	|	ОжидаемыеПоступления.ПлательщикПолучатель
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	Пока Выборка.Следующий() Цикл
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(Выборка.ПлательщикПолучатель) Тогда
			СтруктураПоиска.Вставить("ПлательщикПолучатель", Выборка.ПлательщикПолучатель);
		КонецЕсли;
	...
	Для каждого СтрокаТаблицы Из ТаблицаОбъектовОплаты Цикл
		...
		Пока Выборка.НайтиСледующий(СтруктураПоиска) Цикл
			...
			Если ЗначениеЗаполнено(ПоляГрафика.ФормаОплаты) Тогда
				Если ПоляГрафика.ФормаОплаты = Перечисления.ФормыОплаты.Безналичная
					Или ПоляГрафика.ФормаОплаты = Перечисления.ФормыОплаты.СистемаБыстрыхПлатежей Тогда
					...
					Если ПоляГрафика.ОрганизацияПолучатель = Выборка.Организация
						ИЛИ ПоляГрафика.Комиссионер = Выборка.Организация Тогда
						Запись.БанковскийСчетКасса = ?(ЗначениеЗаполнено(ПоляГрафика.БанковскийСчетОрганизацииПолучателя),
						                                                 ПоляГрафика.БанковскийСчетОрганизацииПолучателя,
						                                                 Неопределено);
					КонецЕсли;
					
				ИначеЕсли ПоляГрафика.ФормаОплаты = Перечисления.ФормыОплаты.Наличная
					И (ЗначениеЗаполнено(ПоляГрафика.Касса)
					ИЛИ ЗначениеЗаполнено(ПоляГрафика.КассаПолучатель)) Тогда
					
					Запись.БанковскийСчетКасса = ПоляГрафика.Касса;
					
					Если ПоляГрафика.ОрганизацияПолучатель = Выборка.Организация
						ИЛИ ПоляГрафика.Комиссионер = Выборка.Организация Тогда
						Запись.БанковскийСчетКасса = ?(ЗначениеЗаполнено(ПоляГрафика.КассаПолучатель),
						                                                 ПоляГрафика.КассаПолучатель,
						                                                 Неопределено);
					КонецЕсли;
			...
			Иначе
				...
				Если ПоляГрафика.ОрганизацияПолучатель = Выборка.Организация
					...
					Если Не ЗначениеЗаполнено(Запись.БанковскийСчетКасса) Тогда
						Запись.БанковскийСчетКасса = ?(ЗначениеЗаполнено(ПоляГрафика.БанковскийСчетОрганизацииПолучателя),
						                                                 ПоляГрафика.БанковскийСчетОрганизацииПолучателя,
						                                                 Неопределено);
					КонецЕсли;
	...
	Для каждого ОбъектОплаты Из ОбъектыОплаты Цикл
		...
		Пока Выборка.НайтиСледующий(СтруктураПоиска) Цикл
			...
			Если ЗначениеЗаполнено(ВыборкаРеквизитов.ФормаОплаты) Тогда
				Если ВыборкаРеквизитов.ФормаОплаты = Перечисления.ФормыОплаты.Безналичная
					Или ВыборкаРеквизитов.ФормаОплаты = Перечисления.ФормыОплаты.СистемаБыстрыхПлатежей Тогда
...
Функция ТекстЗапросаГрафикПлатежейПоРасчетамСКлиентамиНоваяАрхитектура()
	...
	|	ПланОплат.ОбъектРасчетов.Дата КАК ДатаВозникновения,
...
Функция ТекстЗапросаГрафикПлатежейПоЗаявкам()
	...
	|	ВЫБОР
	|		КОГДА Заявка.ФормаОплатыЗаявки <> ЗНАЧЕНИЕ(Перечисление.ФормыОплаты.ПустаяСсылка) ТОГДА
...
Функция ИнициализироватьПоляГрафика()
	...
	ПоляГрафика.Вставить("ОрганизацияПолучатель");
	ПоляГрафика.Вставить("БанковскийСчетОрганизацииПолучателя");
	ПоляГрафика.Вставить("Комиссионер");
	ПоляГрафика.Вставить("КассаПолучатель");
	ПоляГрафика.Вставить("Статус");
...
Процедура ЗаполнитьИсходныеРеквизиты(Запись)
...
Функция КурсВалюты(КурсыВалют, Валюта)

	ПараметрыКурса = Новый Структура();
	КурсВалюты = КурсыВалют.Найти(Валюта, "Валюта");
	ПараметрыКурса.Вставить("Валюта", Валюта);
	ПараметрыКурса.Вставить("КурсЧислитель", КурсВалюты.КурсЧислитель);
	ПараметрыКурса.Вставить("КурсЗнаменатель", КурсВалюты.КурсЗнаменатель);
	
	Возврат ПараметрыКурса;

КонецФункции

Процедура ПролонгацияРегулярныхПлатежей(Договоры)
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	ДоговорыРегулярныхПлатежей.ГрафикИсполненияДоговора КАК ГрафикИсполненияДоговора
	|ИЗ
	|	Справочник.ДоговорыКонтрагентов КАК ДоговорыРегулярныхПлатежей
	|ГДЕ
	|	ДоговорыРегулярныхПлатежей.ВариантОпределенияГрафикаИсполненияВДоговоре
	|		= ЗНАЧЕНИЕ(Перечисление.ВариантыОпределенияГрафикаИсполненияВДоговоре.РегулярныеПлатежи)
	|	И ДоговорыРегулярныхПлатежей.ДатаОкончанияДействия = ДАТАВРЕМЯ(1, 1, 1)
	|	И ДоговорыРегулярныхПлатежей.Ссылка В (&Договоры)
	|	И ДоговорыРегулярныхПлатежей.ГрафикИсполненияДоговора <> ЗНАЧЕНИЕ(Документ.ГрафикИсполненияДоговора.ПустаяСсылка)";
	...
	Пока Выборка.Следующий() Цикл
		...
		Документы.ГрафикИсполненияДоговора.СоздатьЗаполнитьРегулярнымиПлатежами(Выборка.ГрафикИсполненияДоговора);
	КонецЦикла;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ПолноеИмяРегистра = Метаданные.РегистрыСведений.ГрафикПлатежей.ПолноеИмя();

	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "РегистрыСведений.ГрафикПлатежей.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "11.5.27.27";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("aa598031-f852-41a7-a198-0adab2cdc360");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления =
		"РегистрыСведений.ГрафикПлатежей.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Порядок = Перечисления.ПорядокОбработчиковОбновления.Обычный;
	
	СписокОписаний = Новый Массив;
	СписокОписаний.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Обновление ""%1"":'"), ПолноеИмяРегистра));
	СписокОписаний.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '- заполнение реквизитов ""%1"", ""%2"", ""%3"", ""%4"", ""%5"".'"),
		Метаданные.РегистрыСведений.ГрафикПлатежей.Реквизиты.Идентификатор.Имя,
		Метаданные.РегистрыСведений.ГрафикПлатежей.Реквизиты.Статус.Имя,
		Метаданные.РегистрыСведений.ГрафикПлатежей.Реквизиты.ИсходныйБанковскийСчетКасса.Имя,
		Метаданные.РегистрыСведений.ГрафикПлатежей.Реквизиты.ИсходнаяДатаПлатежа.Имя,
		Метаданные.РегистрыСведений.ГрафикПлатежей.Реквизиты.ИсходнаяСумма.Имя));
	
	Обработчик.Комментарий = СтрСоединить(СписокОписаний, Символы.ПС);
	
	Читаемые = Новый Массив;
	Читаемые.Добавить(ПолноеИмяРегистра);
	Обработчик.ЧитаемыеОбъекты = СтрСоединить(Читаемые, ",");
	
	Изменяемые = Новый Массив;
	Изменяемые.Добавить(ПолноеИмяРегистра);
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ГДЕ
	|	ГрафикПлатежей.Идентификатор = """"
	|";
	
	РегистрируемыеОбъектыОплаты = Запрос.Выполнить();
	ОбновлениеИнформационнойБазы.ОтметитьКОбработке(Параметры, РегистрируемыеОбъектыОплаты, ДополнительныеПараметры);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПорцияОбъектовОплаты = ОбновлениеИнформационнойБазы.ДанныеДляОбновленияВМногопоточномОбработчике(Параметры);
	
	Если ПорцияОбъектовОплаты.Количество() = 0 Тогда
	...
	СписокОписаний = Новый Массив;
	СписокОписаний.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось обработать задания по обработчику регистра сведений  ""%1"".'"), ПолноеИмяОбъекта));
		
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ПорцияОбъектовОплаты", ПорцияОбъектовОплаты);
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	ПорцияОбъектовОплаты.ОбъектОплаты
	|
	|ПОМЕСТИТЬ ТаблицаОбъектовОплаты
	|
	|ИЗ
	|	&ПорцияОбъектовОплаты КАК ПорцияОбъектовОплаты
	|;
	|
	|ВЫБРАТЬ
	|	ТаблицаОбъектовОплаты.ОбъектОплаты,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаОбъектовОплаты.ОбъектОплаты) = ТИП(Справочник.Претензии)
	|		ТОГДА ВЫРАЗИТЬ(ТаблицаОбъектовОплаты.ОбъектОплаты КАК Справочник.Претензии).Статус
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаОбъектовОплаты.ОбъектОплаты) = ТИП(Документ.ЗаявкаНаВозвратТоваровОтКлиента)
	|		ТОГДА ВЫРАЗИТЬ(ТаблицаОбъектовОплаты.ОбъектОплаты КАК Документ.ЗаявкаНаВозвратТоваровОтКлиента).Статус
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаОбъектовОплаты.ОбъектОплаты) = ТИП(Документ.ЗаявкаНаРасходованиеДенежныхСредств)
	|		ТОГДА ВЫРАЗИТЬ(ТаблицаОбъектовОплаты.ОбъектОплаты КАК Документ.ЗаявкаНаРасходованиеДенежныхСредств).Статус
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаОбъектовОплаты.ОбъектОплаты) = ТИП(Документ.ТаможеннаяДекларацияИмпорт)
	|		ТОГДА ВЫРАЗИТЬ(ТаблицаОбъектовОплаты.ОбъектОплаты КАК Документ.ТаможеннаяДекларацияИмпорт).Статус
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаОбъектовОплаты.ОбъектОплаты) = ТИП(Документ.ЗаказКлиента)
	|		ТОГДА ВЫРАЗИТЬ(ТаблицаОбъектовОплаты.ОбъектОплаты КАК Документ.ЗаказКлиента).Статус
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаОбъектовОплаты.ОбъектОплаты) = ТИП(Справочник.ДоговорыКонтрагентов)
	|		ТОГДА ВЫРАЗИТЬ(ТаблицаОбъектовОплаты.ОбъектОплаты КАК Справочник.ДоговорыКонтрагентов).Статус
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаОбъектовОплаты.ОбъектОплаты) = ТИП(Справочник.ДоговорыКредитовИДепозитов)
	|		ТОГДА ВЫРАЗИТЬ(ТаблицаОбъектовОплаты.ОбъектОплаты КАК Справочник.ДоговорыКредитовИДепозитов).Статус
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаОбъектовОплаты.ОбъектОплаты) = ТИП(Справочник.ДоговорыМеждуОрганизациями)
	|		ТОГДА ВЫРАЗИТЬ(ТаблицаОбъектовОплаты.ОбъектОплаты КАК Справочник.ДоговорыМеждуОрганизациями).Статус
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаОбъектовОплаты.ОбъектОплаты) = ТИП(Справочник.ДоговорыЭквайринга)
	|		ТОГДА ВЫРАЗИТЬ(ТаблицаОбъектовОплаты.ОбъектОплаты КАК Справочник.ДоговорыЭквайринга).Статус
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаОбъектовОплаты.ОбъектОплаты) = ТИП(Документ.ЗаказПоставщику)
	|		ТОГДА ВЫРАЗИТЬ(ТаблицаОбъектовОплаты.ОбъектОплаты КАК Документ.ЗаказПоставщику).Статус
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаОбъектовОплаты.ОбъектОплаты) = ТИП(Документ.ЗаказПоставщику)
	|		ТОГДА ВЫРАЗИТЬ(ТаблицаОбъектовОплаты.ОбъектОплаты КАК Документ.ЗаказПоставщику).Статус
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаОбъектовОплаты.ОбъектОплаты) = ТИП(Документ.ЗаявкаНаКомандировку)
	|		ТОГДА ВЫРАЗИТЬ(ТаблицаОбъектовОплаты.ОбъектОплаты КАК Документ.ЗаявкаНаКомандировку).Статус
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаОбъектовОплаты.ОбъектОплаты) = ТИП(Документ.РеализацияТоваровУслуг)
	|		ТОГДА ВЫРАЗИТЬ(ТаблицаОбъектовОплаты.ОбъектОплаты КАК Документ.РеализацияТоваровУслуг).Статус
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаОбъектовОплаты.ОбъектОплаты) = ТИП(Документ.РеализацияУслугПрочихАктивов)
	|		ТОГДА ВЫРАЗИТЬ(ТаблицаОбъектовОплаты.ОбъектОплаты КАК Документ.РеализацияУслугПрочихАктивов).Статус
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаОбъектовОплаты.ОбъектОплаты) = ТИП(Документ.АктПремииКлиенту)
	|		ТОГДА ВЫРАЗИТЬ(ТаблицаОбъектовОплаты.ОбъектОплаты КАК Документ.АктПремииКлиенту).Статус
	|	КОНЕЦ КАК Статус
	|ИЗ
	|	ТаблицаОбъектовОплаты
	|";
	...
	СписокОписаний = Новый Массив;
	СписокОписаний.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось обработать задания по обработчику ""%1"".'"), ПолноеИмяОбъекта));
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировки.УстановитьЗначение("ОбъектОплаты", Выборка.ОбъектОплаты);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
			Блокировка.Заблокировать();
			...
			Для Каждого Запись Из НаборЗаписей Цикл
				...
				Если НЕ ЗначениеЗаполнено(Запись.ИсходнаяСумма) Тогда
					Запись.ИсходнаяСумма = Запись.Сумма;
				КонецЕсли;
			КонецЦикла;
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось обработать набор записей %Набор%
				|%Регистр% по причине: %Причина%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Регистр%", ПолноеИмяОбъекта);
