...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	ПоддерживаетсяПодключениеКОборудованиюРозлива = КонструкторИСМПОбщегоНазначенияКлиентСервер.ПоддерживаетсяПодключениеКОборудованиюРозлива(ВидПродукции);
	
	МассивНепроверяемыхРеквизитов = Новый Массив;
	
	Если Не ЭтоИндивидуальныйПредприниматель
		Или Не ПоддерживаетсяПодключениеКОборудованиюРозлива Тогда
	...
	Если ЗначениеЗаполнено(СрокРеализации) И ЗначениеЗаполнено(ДатаПодключения)
		И СрокРеализации < ДатаПодключения И ПоддерживаетсяПодключениеКОборудованиюРозлива Тогда
	...
	Если Не ПоддерживаетсяПодключениеКОборудованиюРозлива Тогда
	...
	Если Не ЗначениеЗаполнено(Комментарий)
		...
		И ПоддерживаетсяПодключениеКОборудованиюРозлива Тогда
	...
	Если ЭтоРасширеннаяВерсияГосИС Тогда
		МодульИнтеграцияИСМППереопределяемый = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияИСМППереопределяемый");
...
Процедура АдресПодключенияСтрокойПриИзменении(Элемент)
	...
	АдресПодключения = ОбщегоНазначенияИСВызовСервера.ЗначенияПолейКонтактнойИнформации(
...
Процедура ПодключитьКОборудованию(Команда)
	...
	Если ЕстьДоступныеСертификаты И ПоддерживаетсяПодключениеКОборудованиюРозлива И ИспользуетсяИнтерактивнаяПередачаДанных Тогда
...
Процедура ПолучитьКодФИАС(Команда)
	...
	ПараметрыЗапросаДанных.Параметры.Вставить(
		"ТоварнаяГруппа",
		КонструкторИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппаПоВидуПродукции(ВидПродукции));
...
Функция СоздатьПодключениеКегаКОборудованиюНаСервере()
	...
	ЭтоАлкоголь        = КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(ВидПродукции);
...
Процедура НастроитьЭлементыФормы()
	...
	Элементы.КодФИАС.Видимость                 = ЭтоИндивидуальныйПредприниматель И ПоддерживаетсяПодключениеКОборудованиюРозлива;
	Элементы.ПолучитьКодФИАС.Видимость         = ЭтоИндивидуальныйПредприниматель И ПоддерживаетсяПодключениеКОборудованиюРозлива;
	Элементы.Архивировать.Видимость            = ЗначениеЗаполнено(Документ);
	Элементы.Комментарий.Видимость             = Не ПодключенВоВнешнейСистеме И ПоддерживаетсяПодключениеКОборудованиюРозлива;
	Элементы.ОбъемСлива.Видимость              = Не ПодключенВоВнешнейСистеме И ПоддерживаетсяПодключениеКОборудованиюРозлива И ЭтоРасширеннаяВерсияГосИС;
	Элементы.Организация.Видимость             = ЭтоИСМП;
	Элементы.СрокРеализации.Видимость          = ПоддерживаетсяПодключениеКОборудованиюРозлива;
	Элементы.АдресПодключенияСтрокой.Видимость = ПоддерживаетсяПодключениеКОборудованиюРозлива;
	
	Элементы.АдресПодключенияСтрокой.РедактированиеТекста = Не ЭтоИндивидуальныйПредприниматель;
	
	Если ПоддерживаетсяПодключениеКОборудованиюРозлива Тогда
	...
	Если Не ЭтоИндивидуальныйПредприниматель И ПоддерживаетсяПодключениеКОборудованиюРозлива Тогда
...
Процедура ПриИзмененииВидаПродукции()
	...
	Если ИмяПодсистемы = "ИСМП"
		И КонструкторИСМПОбщегоНазначенияКлиентСервер.ПоддерживаетсяПодключениеКОборудованиюРозлива(ВидПродукции) Тогда
...
Процедура ЗаполнитьДанныеПоШтрихкодуУпаковки(ДанныеШтрихкода = Неопределено)
	...
	Если ЗначениеЗаполнено(КодМаркировки) Тогда
		...
		Если ДанныеВскрытия = Неопределено Тогда
			...
			Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ПоддерживаетсяПодключениеКОборудованиюРозлива(ВидПродукции) Тогда
	...
	Иначе
		...
		Если ВидыПродукции.Количество() = 1 Тогда
			...
			Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(ВидыПродукции[0]) Тогда
				ПоляКлюча.ВидПродукцииЕГАИС = ВидыПродукции[0];
			Иначе
				ПоляКлюча.ВидПродукцииГИСМТ = ВидыПродукции[0];
			КонецЕсли;
			
			ВидПродукции = Справочники.КлючиВидовПродукцииИС.ПолучитьКлюч(ПоляКлюча);
...
Процедура ЗаполнитьКегОтключенияОтОборудования()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Комментарий) Тогда
			...
			Прервать;
		ИначеЕсли (КонструкторИСМПОбщегоНазначенияКлиентСервер.ПоддерживаетсяПодключениеКОборудованиюРозлива(ВидПродукции)
...
Процедура КодМаркировкиПриИзмененииНаСервере()
	...
	Пересечения = ОбщегоНазначенияИСКлиентСервер.ПересечениеМассивов(ВидыПродукции, ВидыПродукцииКлюча);
	
	Если Пересечения.Количество() = 0 Тогда
...
Процедура СкладПриИзмененииНаСервере()
	...
	Если ЭтоРасширеннаяВерсияГосИС
		И КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(ВидПродукции, Истина) Тогда
		МодульСобытияФормИСМП = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("СобытияФормИСМП");
...
Процедура РучнойВводШтрихкодаЗавершение(ДанныеШтрихкода, ДополнительныеПараметры) Экспорт
	...
	ДанныеРазбора = РазборКодаМаркировкиИССлужебныйКлиент.РазобратьКодМаркировки(
		ДанныеШтрихкода.Штрихкод, ВидыПродукцииКлюча);
	
	Если ДанныеРазбора <> Неопределено Тогда
		
		Если ЗначениеЗаполнено(ВидПродукции) Тогда
			ЕстьПотребительскаяУпаковка = Ложь;
			Для Каждого КлючИЗначение Из ДанныеРазбора.ВидыУпаковокПоВидамПродукции Цикл
				Если КлючИЗначение.Значение.Найти(
					...
					И ВидыПродукцииКлюча.Найти(КлючИЗначение.Ключ) <> Неопределено Тогда
					ЕстьПотребительскаяУпаковка = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Если Не ЕстьПотребительскаяУпаковка Тогда
...
Процедура ОбработатьШтрихкодКодаМаркировкиНаСервере(ДанныеШтрихкода)
	...
	Если ЭтоРасширеннаяВерсияГосИС Тогда
		МодульШтрихкодированиеИСВызовСервера = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ШтрихкодированиеИСВызовСервера");
