...
Функция ТокенАвторизации(УчетнаяЗапись, Пользователь) Экспорт
	...
	Если Пользователь <> Пользователи.АвторизованныйПользователь()
		...
		ВызватьИсключение НСтр("ru = 'Недостаточно прав для получения токена доступа к центру ЭДО.'");
	КонецЕсли;
	...
	Если ТипЗнч(Пароль) = Тип("Строка") Тогда
		Возврат ИнтеграцияЦентраЭДО.ТокенАвторизацииБазовый(Логин, Пароль);
	КонецЕсли;
	Возврат Неопределено;
КонецФункции
...
Функция Логин(УчетнаяЗапись, Пользователь)
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ПользователиЦентраЭДО.Логин
		|ИЗ
		|	РегистрСведений.ПользователиЦентраЭДО КАК ПользователиЦентраЭДО
		|ГДЕ
		|	ПользователиЦентраЭДО.УчетнаяЗаписьЦентраЭДО = &УчетнаяЗапись
		|	И ПользователиЦентраЭДО.Пользователь = &Пользователь";
...
Функция Авторизоваться(ПараметрыАвторизации, НовоеПодключение) Экспорт
	...
	Если Справочники.УчетныеЗаписиЦентровЭДО.ЭтоСервис1СФреш(ПараметрыАвторизации.АдресСервиса)
	...
	РезультатПроверкиПрав = ПроверитьПраваПользователяВЦентреЭДО(ПараметрыАвторизации, НовоеПодключение);
...
//  Структура:
//  * ТекстОшибки - Строка
Функция ПроверитьПраваПользователяВЦентреЭДО(ПараметрыАвторизации, НовоеПодключение) Экспорт
	...
	Если НовоеПодключение Тогда
		...
		УчетнаяЗаписьЦентраЭДО = Справочники.УчетныеЗаписиЦентровЭДО.НайтиПоАдресуСервиса(
	...
	РезультатПакета = ИнтеграцияЦентраЭДО.ВыполнитьПакетМетодовСервиса(ПараметрыВыполнения, ПакетМетодов,
		КонтекстДиагностики);
	...
	ПравоНастройкиПодключения = РезультатПакета[0]; // См. ПрограммныйИнтерфейсЦентраЭДОВерсия3.ПроверитьПравоНастройкиПодключения
	ПравоВыполненияДействийПоЭДО = РезультатПакета[1]; // См. ПрограммныйИнтерфейсЦентраЭДОВерсия3.ПроверитьПравоВыполненияДействийПоЭДО
...
Функция Добавить(УчетнаяЗапись, ПараметрыПользователя) Экспорт
	...
	ЗначенияКлюча = Новый Структура("УчетнаяЗаписьЦентраЭДО, Пользователь", УчетнаяЗапись, Пользователь);
	
	НаборЗаписей = СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.УчетнаяЗаписьЦентраЭДО.Установить(ЗначенияКлюча.УчетнаяЗаписьЦентраЭДО);
...
Функция ВыполнитьЗапросВСервис1СФреш(ПараметрыАвторизации, ТелоЗапроса)
	...
	СоединениеССервисом = ИнтеграцияЦентраЭДО.СоединениеССервисомПоАдресу(ПараметрыАвторизации.АдресСервиса);
	...
	Исключение
		...
		ЗаписатьОшибкуПодключенияКЦентруЭДО(РезультатЗапроса.ТекстОшибки,
...
Процедура ЗаписатьОшибкуПодключенияКЦентруЭДО(Знач КраткоеПредставление, Знач ПодробноеПредставление) Экспорт
	ВидОперации = НСтр("ru = 'Подключение к центру ЭДО'");
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("Авторизоваться", Истина);
