...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	|				ТОГДА 1
	|			КОГДА НДССостояниеРеализации0.Состояние = ЗНАЧЕНИЕ(Перечисление.НДССостоянияРеализация0.ЧастичноПодтвержденаРеализация0)
	|				ТОГДА 2
	...
	|	Таблица.Организация                             КАК Организация,
	|	НДСПредъявленный.СчетФактура                    КАК СчетФактура
	|ПОМЕСТИТЬ НДССостояниеРеализации0_ЗаданияКФормированиюДвиженийПоНДС
	|ИЗ
	|	НДССостояниеРеализации0Изменения КАК Таблица
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.НДСПредъявленный КАК НДСПредъявленный
	...
	|ПОМЕСТИТЬ ИмяВТ
	|ИЗ
	|	НДССостояниеРеализации0_ЗаданияКФормированиюДвиженийПоНДС КАК Таблица
	|";
	Запрос.УстановитьПараметр("Регистратор", Отбор.Регистратор.Значение);
