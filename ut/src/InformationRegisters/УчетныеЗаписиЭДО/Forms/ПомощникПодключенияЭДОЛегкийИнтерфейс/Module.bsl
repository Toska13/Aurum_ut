...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ГруппаУчетнаяЗаписьЦентраЭДО.Видимость = ИспользоватьЦентрЭДО;
...
Процедура ПослеПоискаСертификата(СертификатКриптографии, Контекст) Экспорт
	...
	Если СертификатСозданИлиНайден Тогда
		...
		СведенияПоРуководителюИСертификату = СведенияПоРуководителюИСертификату(Сертификат, Организация);
		Руководитель = СведенияПоРуководителюИСертификату.Руководитель;
		ОГРНВСертификате = СведенияПоРуководителюИСертификату.ОГРНВСертификате;
...
Процедура ЗарегистрироватьСертификатЧерезЦентрЭДОЗавершение(Результат, Контекст) Экспорт
	...
	Если Результат.Успех Тогда
		...
		ПаролиСертификатов = Результат.ПаролиСертификатов;
		ОжиданиеОперацийБЭДКлиент.ИнициализироватьПараметрыОбработчикаОжидания(ПараметрыОбработчикаОжидания);
		ПодключитьОбработчикОжидания("ПроверитьРезультатРегистрацииСертификатаЧерезЦентрЭДО",
			ПараметрыОбработчикаОжидания.ТекущийИнтервал, Истина);
	...
	ИначеЕсли Результат.СертификатыУжеЗарегистрированы Тогда
		...
		УникальныйИдентификаторЗаявки1СЭДО = Результат.ИдентификаторЗаявки;
		ПаролиСертификатов = Результат.ПаролиСертификатов;
		РезультатРегистрации = УчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент.НовыйРезультатРегистрацииСертификатовВЭДО();
...
&НаКлиенте
Процедура ПроверитьРезультатРегистрацииСертификатаЧерезЦентрЭДО()
...
Процедура ВыполнитьОтправкуДанныхВСервисНастроек()
	...
	Если ИспользоватьЦентрЭДО Тогда
		Оповещение = Новый ОписаниеОповещения("ОтправкаДанныхВСервисНастроекЧереЦентрЭДОЗавершение", ЭтотОбъект, ИдентификаторЭДО);
		ОбщийМодульУчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
			"УчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент");
...
&НаКлиенте
Процедура ОтправкаДанныхВСервисНастроекЧереЦентрЭДОЗавершение(Результат, ИдентификаторЭДО) Экспорт
...
Процедура ВключитьАвтоматическоеПолучениеЭДО(ОповещениеОЗавершении)
	...
	Если Не ИспользоватьЦентрЭДО
		И СпособОбменаЭД <> ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезСервис1СЭДО") Тогда
...
Процедура УстановитьВидимостьОГРНКПП(Форма)
	...
	Форма.Элементы.ГруппаОГРН.Видимость = Не Форма.ЭтоФилиалИностраннойОрганизации;
...
Процедура ОрганизацияПриИзмененииНаСервере()
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		ЭтоФилиалИностраннойОрганизации = ОбщегоНазначенияБЭДКлиентСервер.ЭтоКППФилиалаИностраннойОрганизации(
			СтруктураДанныхОрганизации.КПП);
		ИзвлечьКодНалоговогоОрганаИзИНН(КодНалоговогоОргана, ИНН);
...
Функция ПроверитьДанныеОрганизации(Форма, ВыводитьСообщения = Ложь)
	...
	Если Не ЗначениеЗаполнено(Форма.ОГРН) И Не Форма.ЭтоФилиалИностраннойОрганизации Тогда
	...
	ЭтоПодписаниеСертификатомРуководителя = ЗначениеЗаполнено(Форма.ОГРНВСертификате);
	ЭтоПодписаниеПоМЧД = ЗначениеЗаполнено(Форма.Доверенность);
	
	Если ЭтоПодписаниеСертификатомРуководителя И Не ЭтоПодписаниеПоМЧД
		И Форма.ОГРНВСертификате <> Форма.ОГРН Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Сертификат",
			СтрШаблон(ТекстОшибки, Форма.ОГРНВСертификате, Форма.ОГРН));
	КонецЕсли;
...
Функция КлючевыеПоляДляРегистрацииЗаполнены()
	...
	Если ИспользоватьЦентрЭДО И Не ЗначениеЗаполнено(УчетнаяЗаписьЦентраЭДО) Тогда
...
&НаСервереБезКонтекста
Функция СведенияПоРуководителюИСертификату(Знач Сертификат, Знач Организация)
	
	Результат = Новый Структура;
	Результат.Вставить("Руководитель", "");
	Результат.Вставить("ОГРНВСертификате", Неопределено);
	...
	Результат.ОГРНВСертификате = ?(ЗначениеЗаполнено(СвойстваСубъекта.ОГРН), СвойстваСубъекта.ОГРН,
		СвойстваСубъекта.ОГРНИП);
	...
	Если ЭтоСертификатФизическогоЛица Тогда
		...
		Результат.Руководитель = СтрШаблон("%1 %2",
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ЗначенияРеквизитов.Должность) Тогда
			Результат.Руководитель = СтрШаблон("%1 %2, %3",
