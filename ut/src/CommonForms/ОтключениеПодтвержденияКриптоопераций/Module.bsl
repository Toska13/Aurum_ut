...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДлительнаяОперация 			= ДлительныеОперации.ВыполнитьВФоне(
		"СервисКриптографииСлужебный.ПолучитьНастройкиПолученияВременныхПаролей", ПараметрыПроцедуры, ПараметрыВыполнения);
...
Процедура ОбратныйОтсчетКодТелефона()
	...
	ТаймерОбщий	= ТаймерСрока - ТекущаяДата();
	...
	ИначеЕсли ОтключитьОбратныйОтсчет <> 99 Тогда
		...
		ИначеЕсли ТаймерОбщий > 0 И Таймер > 0 Тогда
			СтатусКода = "";
		ИначеЕсли ТаймерОбщий > 0 Тогда
			СтатусКода = СтрШаблон(НСтр("ru = 'Истекает через %1:%2'"), Цел(ТаймерОбщий / 60), Формат(ТаймерОбщий % 60, "ЧЦ=2; ЧВН="));
		Иначе
			СтатусКода = НСтр("ru = 'Истек срок действия пароля'");
			Элементы.СтатусКода.ЦветТекста = WebЦвета.Красный;
		КонецЕсли;
		
		Если ТаймерОбщий > 0 ИЛИ Таймер > 0 Тогда
...
Функция ПолучитьПарольНаСервере(Повторно = Ложь, ИдентификаторСессии = Неопределено)
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне(
		"СервисКриптографииСлужебный.ПолучитьВременныйПароль", ПараметрыПроцедуры, ПараметрыВыполнения);
...
Функция ПодтвердитьНаСервере(ИдентификаторСессии = Неопределено)
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне(
		"СервисКриптографииСлужебный.ПолучитьСеансовыйКлюч", ПараметрыПроцедуры, ПараметрыВыполнения);
