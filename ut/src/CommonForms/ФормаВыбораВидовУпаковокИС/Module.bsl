...
Процедура ЗаполнитьСписокВидыУпаковок()
	...
	Если КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетГрупповыеУпаковки(Параметры.ВидПродукции) Тогда
		НоваяСтрока = ТаблицаВидыУпаковок.Добавить();
		НоваяСтрока.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая;
		НоваяСтрока.Представление = Строка(НоваяСтрока.ВидУпаковки);
	КонецЕсли;
	Если КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииИспользуетНаборы(Параметры.ВидПродукции) Тогда
		НоваяСтрока = ТаблицаВидыУпаковок.Добавить();
