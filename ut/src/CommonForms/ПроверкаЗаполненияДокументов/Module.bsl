...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РассчитыватьНаборы                   = Параметры.РассчитыватьНаборы;
	...
	Если ЗначениеЗаполнено(ТипыНоменклатуры) Тогда
		ПараметрВыбора  = Новый ПараметрВыбора("Отбор.ТипНоменклатуры", ТипыНоменклатуры);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Запрос.УстановитьПараметр("УсловиеПоМаркируемойПродукции", Ложь);
	Если ВыгруженаТолькоНеМаркируемаяПродукция Тогда
		НоменклатураЛокализация.ОбновитьПараметрыЗапросаОбработкиПроверкиЗаполненияДокументов(Запрос);
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ОшибкаПоТипуНоменклатуры Тогда
			...
			ПутьКСтрокеТабличнойЧасти = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары",
				НомерСтрокиДляПользователя, "Номенклатура");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,ПутьКСтрокеТабличнойЧасти,,Отказ);
		КонецЕсли;
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	СтруктураДействий.РассчитыватьНаборы                     = РассчитыватьНаборы;
	...
	Если Не ВсеТипыНоменклатуры Тогда
		ШтрихкодированиеНоменклатурыКлиент.ОпределитьУсловияТиповНоменклатурОбработкиШтрихкодов(СтруктураДействий,
			ТипыНоменклатуры);
	КонецЕсли;
...
Процедура ОбработатьШтрихкодыСервер(СтруктураПараметровДействия, КэшированныеЗначения)
	...
	ШтрихкодированиеНоменклатурыСервер.ОбработатьШтрихкоды(ЭтотОбъект, ЭтотОбъект, СтруктураПараметровДействия, КэшированныеЗначения);
...
Процедура Подключаемый_ОбработатьСтрокиТЧ(ДобавленныеСтроки, ИзмененныеСтроки, КэшированныеЗначения = Неопределено) Экспорт
	...
	СтруктураДействийСДобавленнымиСтроками.Вставить("ПроверитьХарактеристикуПоВладельцу");
	СтруктураДействийСДобавленнымиСтроками.Вставить("ПроверитьЗаполнитьУпаковкуПоВладельцу");
	ПакетнаяОбработкаТабличнойЧастиКлиентСерверЛокализация.ДополнитьСтруктуруДействийПриИзмененииЭлемента(
	...
	Если ПараметрыУказанияСерий <> Неопределено Тогда
		СтруктураДействийСДобавленнымиСтроками.Вставить("ПроверитьСериюРассчитатьСтатус",
			Новый Структура("ПараметрыУказанияСерий", ПараметрыУказанияСерий));
	КонецЕсли;
...
Процедура ПослеОбработкиШтрихкодов(ДополнительныеПараметры)
	...
	Если РезультатОбработкиШтрихкода.ТребуетсяОбработкаШтрихкода Тогда
		...
		Если Не ВсеТипыНоменклатуры Тогда
			ШтрихкодированиеНоменклатурыСервер.ОпределитьУсловияТиповНоменклатурОбработкиШтрихкодов(ДанныеДляОбработки,
				ТипыНоменклатуры);
		КонецЕсли;
...
Функция ЗакончитьПроверкуНаСервере()
	...
	ИначеЕсли ЗначениеЗаполнено(МассивСтрокИзлишки) Тогда
		ПревышениеЗапрещено = Истина;
	КонецЕсли;
	...
	ПараметрыЗакрытия.Вставить("ПревышениеЗапрещено",	ПревышениеЗапрещено);
...
Процедура ЗаполнитьСлужебныеРеквизитыПоНоменклатуре()
	...
	ПакетнаяОбработкаТабличнойЧастиСерверЛокализация.ЗаполнитьСлужебныеРеквизиты(ЭтотОбъект);
