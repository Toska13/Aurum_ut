...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидЗапроса = 2 Тогда
		...
		Если Не (ТипЗапроса = "Партии" И РазделУчета = Перечисления.РазделыУчетаЗЕРНО.Зерно) Тогда
...
Функция ВыполнениеЗаявкиЗЕРНОНачалоНаСервере()
	...
	Если ВидЗапроса = 1 Тогда
		...
		Если ТипЗапроса = "Партии" Тогда
			...
			ПараметрыОбработкиСообщений.РазделУчета      = РазделУчета;
...
Процедура ПослеПолученияРезультатОбработкиЗаявки(Изменения, ДополнительныеПараметры) Экспорт
	...
	Если КоличествоОшибок > 0 Тогда
		ТекстРезультатаВыполненияОбмена = Новый ФорматированнаяСтрока(
			СтрШаблон(
				НСтр( "ru = 'Результат обмена данными с ошибками (%1)'"),
				КоличествоОшибок),, ЦветПроблема,, "ОткрытьРезультат");
	Иначе
		ТекстРезультатаВыполненияОбмена = Новый ФорматированнаяСтрока(
			НСтр( "ru = 'Результат обмена данными'"),,,, "ОткрытьРезультат");
	КонецЕсли;
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	ЭтоЗерно                         = Форма.РазделУчета = ПредопределенноеЗначение("Перечисление.РазделыУчетаЗЕРНО.Зерно");
	...
	Элементы.ТипОрганизации.Видимость = ЭтоЗерно И ЭтоЗапросСДИЗ;
	...
	Элементы.РазделУчета.Видимость                    = (ЭтоЗапросСДИЗ Или ЭтоЗапросПартий);
...
Процедура ЗаполнитьРеквизитыШапки(СсылкаНаОбъект)
	...
	Запрос.Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	СДИЗЗЕРНО.Ссылка,
		|	СДИЗЗЕРНО.РазделУчета КАК РазделУчета,
		...
		|	ЗНАЧЕНИЕ(Перечисление.РазделыУчетаЗЕРНО.Зерно) КАК РазделУчета,
		|	NULL,
		|	NULL,
		|	NULL,
		|	NULL,
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Если Не ЗначениеЗаполнено(РазделУчета) Тогда
			РазделУчета = ВыборкаДетальныеЗаписи.РазделУчета;
		КонецЕсли;
...
Процедура ОбработатьПереданныеПараметрыПриСозданииНаСервере()
	...
	Если ЗначениеЗаполнено(Параметры.РазделУчета) Тогда
