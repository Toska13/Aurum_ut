...
Процедура НадписьЗаголовокЗаказыНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ЭтоОтгрузкаБезПереходаПраваСобственности2_5 Или ЭтоОтгрузкаТоваровКлиенту Тогда
		ЗаголовокСпискаЗаказов = НСтр("ru='Распоряжения (%КоличествоДокументов%)'");
	Иначе
...
Процедура ЗаполнитьРеквизитыПриСоздании(ТоварыНакладной)
	...
	ЭтоОтгрузкаТоваровКлиенту = ТипЗнч(Накладная) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту");
	ЭтоОтгрузкаБезПереходаПраваСобственности2_5 =
		ТипЗнч(Накладная) = Тип("ДокументСсылка.СписаниеОтгруженныхТоваров")
			Или ТипЗнч(Накладная) = Тип("ДокументСсылка.ПоступлениеТоваровОтКлиента")
			Или ТипЗнч(Накладная) = Тип("ДокументСсылка.ОприходованиеИзлишковОтгруженныхТоваров");
...
Процедура ЗаполнитьТаблицуТоваров(ТоварыНакладной, ШтрихкодыУпаковокНакладной)
	...
	Если ИспользоватьНакладныеПоНесколькимЗаказам Тогда
		...
		МассивЗаказов = МенеджерНакладной.РаспоряженияНакладной(Накладная, Параметры.РеквизитыШапки);
	Иначе
...
Процедура УдалитьСтрокиБезОтклонений()
	...
	Для Счетчик = 1 По КоличествоСтрок Цикл
		...
		НетОтклоненийЗаказ = РаспоряжениеНакладная
			Или СтрокаТаблицы.КоличествоВЗаказе = 0
			Или СтрокаТаблицы.КоличествоВНакладной = СтрокаТаблицы.КоличествоВЗаказе;
...
Процедура ОбновитьИнформациюПоЗаказамВФорме(ТоварыНакладной)
	...
	Если СписокРаспоряжений.Количество() > 1 Тогда
		Если ЭтоОтгрузкаБезПереходаПраваСобственности2_5 Или ЭтоОтгрузкаТоваровКлиенту Тогда
			НадписьВсегоЗаказов = НСтр("ru = 'Всего распоряжений'");
		Иначе
...
Процедура УстановитьЗаголовкиФормы()
	...
	Если ЭтоОтгрузкаБезПереходаПраваСобственности2_5 Тогда
		Заголовок = НСтр("ru = 'Подбор товаров по распоряжениям'");
	ИначеЕсли ЭтоОтгрузкаТоваровКлиенту И ИспользоватьЗаказы И ИспользоватьОрдернуюСхему Тогда
		Заголовок = НСтр("ru = 'Подбор товаров по распоряжениям/ордерам'");
	ИначеЕсли ЭтоОтгрузкаТоваровКлиенту И ИспользоватьЗаказы Тогда
		Заголовок = НСтр("ru = 'Подбор товаров по распоряжениям'");
	ИначеЕсли ИспользоватьЗаказы И ИспользоватьОрдернуюСхему Тогда
	...
	Если ЭтоОтгрузкаБезПереходаПраваСобственности2_5 Или ЭтоОтгрузкаТоваровКлиенту Тогда
		Элементы.ПоЗаказу.Заголовок = НСтр("ru = 'Только распоряжение, уже выбранное в накладной'");
		Элементы.ПоЗаказам.Заголовок = НСтр("ru = 'Только распоряжения, уже выбранные в накладной'");
		Элементы.Заказ.Заголовок = НСтр("ru = 'Распоряжение'");
	Иначе
		Элементы.ПоЗаказу.Заголовок = НСтр("ru = 'Только заказ, уже выбранный в накладной'");
		Элементы.ПоЗаказам.Заголовок = НСтр("ru = 'Только заказы, уже выбранные в накладной'");
		Элементы.Заказ.Заголовок = НСтр("ru = 'Заказ'");
	КонецЕсли;
	
	Если ЭтоОтгрузкаБезПереходаПраваСобственности2_5 Тогда
		Элементы.КоличествоУпаковокВЗаказе.Заголовок = НСтр("ru = 'В пути'");
	ИначеЕсли ЭтоОтгрузкаТоваровКлиенту Тогда
		Элементы.КоличествоУпаковокВЗаказе.Заголовок = НСтр("ru = 'В распоряжении'");
	Иначе
		Элементы.КоличествоУпаковокВЗаказе.Заголовок = НСтр("ru = 'В заказе'");
	КонецЕсли;
...
Функция ПоместитьТоварыВХранилище()
	...
	Для каждого Строка Из ТаблицаТовары Цикл
		Если Строка.СтрокаВыбрана Тогда
			...
			СтруктураПоискаМарки.Вставить("ВыгружатьВДокумент", Ложь);
			СтруктураПоискаМарки.Вставить("ИзОрдера",           Истина);
...
Функция ПроверитьВыборНесколькихЗаказов()
	...
	Если ИспользоватьНакладныеПоНесколькимЗаказам Тогда
		...
		Если ЭтоОтгрузкаБезПереходаПраваСобственности2_5 Или ЭтоОтгрузкаТоваровКлиенту Тогда
			ШаблонБолееОдногоЗаказа = НСтр("ru = 'Нельзя выбрать товары больше, чем по одному распоряжению.'");
			ШаблонБезЗаказаИПоЗаказу = НСтр("ru = 'Нельзя выбрать товары по распоряжению и без указания распоряжения одновременно.'");
		Иначе
...
Процедура ОтметитьСтроки(Значение)
	...
	Для Каждого СтрокаТоваров Из ТаблицаТовары Цикл
		СтрокаТоваров.СтрокаВыбрана = (СтрокаТоваров.ЗаказИзНакладной И ПоЗаказам Или Не ПоЗаказам) И Значение;
	КонецЦикла;
