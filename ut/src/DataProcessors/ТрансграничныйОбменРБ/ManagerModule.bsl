...
Функция ПолучитьGLN(Идентификатор) Экспорт
	...
	ДопустимыеДлиныИдентификаторов = Новый Соответствие();
	ДопустимыеДлиныИдентификаторов.Вставить(16, Истина);
	ДопустимыеДлиныИдентификаторов.Вставить(17, Истина);
	
	Если ДопустимыеДлиныИдентификаторов[СтрДлина(Идентификатор)] = Неопределено Тогда
		Возврат "";
	КонецЕсли;
	
	Для Каждого Оператор Из Операторы() Цикл
		Если СтрНачинаетсяС(Идентификатор, Оператор.Префикс) Тогда
			ДлинаGLN = 13;
...
Функция СодержаниеТоварнойНакладной(ДвоичныеДанные)
	...
	Результат.ТипДокумента = ТрансграничныйЭДО.СтандартныеТипыДокументов().ТоварнаяНакладнаяРБ;
...
Функция СформироватьОбъектBLRDNR(Данные)
	...
	FunctionCode = ?(АктЗаполненКорректно, "21", "11");
...
Функция Операторы()
	...
	Оператор.Префикс = "DBY";
	Оператор.КодАльфа3Страны = КодАльфа3Страны();
	Оператор.Наименование = НСтр("ru = 'ООО ""Бидмартс""'");
	Результат.Добавить(Оператор);

	Оператор = ТрансграничныйЭДО.ОписаниеОператора();
	Оператор.Префикс = "BBD";
	Оператор.КодАльфа3Страны = КодАльфа3Страны();
	Оператор.Наименование = НСтр("ru = 'ООО ""Электронные документы и накладные""'");
	Результат.Добавить(Оператор);
	
	Оператор = ТрансграничныйЭДО.ОписаниеОператора();
	Оператор.Префикс = "GBY";
	Оператор.КодАльфа3Страны = КодАльфа3Страны();
	Оператор.Наименование = НСтр("ru = 'ООО ""МетаЭра""'");
	Результат.Добавить(Оператор);
...
// Возвращает наименование страны.
// 
// Возвращаемое значение:
//  Строка
//
Функция НаименованиеСтраны()
	Возврат НСтр("ru = 'Республика Беларусь'");
КонецФункции

// Заполняет состояние по входящему отклонению.
// 
// Параметры:
//  ДанныеДляЗаполнения - см. ТрансграничныйЭДО.ДанныеДляЗаполненияСостоянияЭД
...
//
Процедура ЗаполнитьСостояниеПоВходящемуОтклонению(ДанныеДляЗаполнения, СостоянияЭлементовРегламента, Состояние) Экспорт
	
	СтандартныеТипы = ТрансграничныйЭДО.СтандартныеТипыДокументов();
	
	Если ДанныеДляЗаполнения.ТипДокумента = СтандартныеТипы.ТоварнаяНакладнаяРБ
		И ДанныеДляЗаполнения.ИдентификаторДокументооборота = ДанныеДляЗаполнения.ИдентификаторСвязи Тогда
		Состояние = Перечисления.СостоянияДокументовЭДО.ЗакрытСОтклонением;
	КонецЕсли;
	
КонецПроцедуры

// см. ТрансграничныйЭДО.ДополнитьКорректируемыеТипыЭД
Процедура ДополнитьКорректируемыеТипыЭД(ТипыЭД) Экспорт
	
	СтандартныеТипыДокументов = ТрансграничныйЭДО.СтандартныеТипыДокументов();
	ТипыЭД.Вставить(СтандартныеТипыДокументов.ТоварнаяНакладнаяРБ, СтандартныеТипыДокументов.ТоварнаяНакладнаяРБ);
	
КонецПроцедуры

// см. ТрансграничныйЭДО.ДополнитьИсправляющиеТипыЭД
Процедура ДополнитьИсправляющиеТипыЭД(ТипыЭД) Экспорт
	
	СтандартныеТипыДокументов = ТрансграничныйЭДО.СтандартныеТипыДокументов();
	ТипыЭД.Вставить(СтандартныеТипыДокументов.ТоварнаяНакладнаяРБ, Истина);
...
Функция ЭтоСлужебноеИмяДополнительногоПоля(ИмяПоля, Формат) Экспорт
	Возврат ФорматыДляДополнительныхДанных()[Формат] <> Неопределено И ИмяПоля = ИмяПоляИдентификатора();
КонецФункции

// Набор форматов для дополнительных данных.
// 
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//  * Ключ - Строка
//  * Значение - Булево
//
Функция ФорматыДляДополнительныхДанных()
	
	Результат = Новый Соответствие();
	Форматы = ФорматыЭДО.ПоддерживаемыеФорматы();
	Результат.Вставить(Форматы.ФНС.УПД_5_03.ИнформацияПродавца, Форматы.ФНС.УПД_5_03.ИнформацияПродавца);
	Результат.Вставить(Форматы.ФНС.УКД2020.ИнформацияПродавца, Форматы.ФНС.УКД2020.ИнформацияПродавца);
	Возврат Результат;
	
КонецФункции

Функция ИмяПоляИдентификатора()
	Возврат "ВнешнийИдентификаторРБ";
КонецФункции

// Формирует данные дополнительных полей шапки трансграничного документа
// 
// Параметры:
...
//    * Значение - Строка
//
Функция ДанныеДополнительныхПолей(ИдентификаторКонтрагента, ПараметрыЗаполнения) Экспорт
	
	Форматы = ФорматыДляДополнительныхДанных();
	Формат = ПараметрыЗаполнения.НастройкиОбмена.Формат; // Строка
	
	Если НЕ ЭтоОператорРБ(ИдентификаторКонтрагента) ИЛИ Форматы[Формат] = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Идентификатор = "";
	ДеревоДанных = ПараметрыЗаполнения.ОписаниеДанныхЭД; // ДеревоЗначений
	ТребуетсяНайтиИдентификаторОснования = Ложь;
	
	Если Формат = Форматы[ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПродавца] Тогда
		
		ТребуетсяНайтиИдентификаторОснования = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(
			ДеревоДанных, "СведенияОСчетеФактуре.Исправление") = Истина;
		СсылкаНаОбъект = ?(ТребуетсяНайтиИдентификаторОснования,
			ПараметрыЗаполнения.СсылкаНаОбъект, ПараметрыЗаполнения.ОбъектУчета);
		
	ИначеЕсли Формат = Форматы[ПоддерживаемыеФорматы.ФНС.УКД2020.ИнформацияПродавца] Тогда
		
		СсылкаНаОбъект = ПараметрыЗаполнения.СсылкаНаОбъект;
		ТребуетсяНайтиИдентификаторОснования = Истина;
		
	КонецЕсли;
	
	Если ТребуетсяНайтиИдентификаторОснования Тогда
		
		Основания = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(
			ДеревоДанных, "ДокументыОснованияСчетаФактуры");
		...
		Если ЗначениеЗаполнено(Основания) Тогда
			...
			Для Каждого Основание Из Основания Цикл
				Если ТипЗнч(Основание) = ТипЗнч(ПараметрыЗаполнения.СсылкаНаОбъект) Тогда
			...
			ЭлектронныйДокумент = ИнтеграцияЭДО.ОсновнойЭлектронныйДокументОбъектаУчета(СсылкаНаОбъект);
			Если ЗначениеЗаполнено(ЭлектронныйДокумент) Тогда
				Идентификатор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭлектронныйДокумент, "ИдентификаторСвязи");
			КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Идентификатор) И ЗначениеЗаполнено(СсылкаНаОбъект) Тогда
		Идентификатор = Строка(СсылкаНаОбъект.УникальныйИдентификатор());
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Идентификатор) Тогда
		Результат.Вставить(ИмяПоляИдентификатора(), Идентификатор);
		Возврат Результат;
	КонецЕсли;
...
// см. ТрансграничныйЭДО.ЗаполнитьПараметрыСвязиИдентификаторов
Процедура ЗаполнитьПараметрыСвязиИдентификаторов(ПараметрыСвязи) Экспорт
	
	Операторы = Операторы();
	
	Для Каждого ОписаниеСвязи Из ПараметрыСвязи Цикл
		
		ДлинаПрефикса = 3;
		ПрефиксОператора = ВРег(Лев(ОписаниеСвязи.ИдентификаторКонтрагента, ДлинаПрефикса));
		
		Для Каждого ОписаниеОператора Из Операторы Цикл
			
			Если ПрефиксОператора = ВРег(Лев(ОписаниеОператора.Префикс, ДлинаПрефикса)) Тогда
				
				ОписаниеСвязи.ОператорКонтрагента = ПрефиксОператора;
				ОписаниеСвязи.ОператорКонтрагентаПредставление =
					СтрШаблон("%1 (%2)", ОписаниеОператора.Наименование, НаименованиеСтраны());
				ОписаниеСвязи.ОператорыСовпадают =
					ОписаниеСвязи.ОператорКонтрагента = ОписаниеСвязи.ОператорОрганизации;
...
// Проверяет идентификатор оператора ТЭДО.
// 
// Параметры:
//  ИдентификаторУчастника - Строка
// 
...
//  Булево
//
Функция ЭтоОператорТЭДО(ИдентификаторУчастника) Экспорт
	Возврат ЭтоОператорРБ(ИдентификаторУчастника);
КонецФункции
