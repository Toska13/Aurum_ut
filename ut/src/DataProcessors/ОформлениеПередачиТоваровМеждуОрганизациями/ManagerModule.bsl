...
Функция СоздатьДокументыПередачи(Объект) Экспорт
	...
	Для Каждого СтрокаТЧ Из НаправленияПередачи Цикл
		...
		КодыМеха = КодыПередачи.ВыгрузитьКолонку("ШтрихкодУпаковки");
		ШтрихкодированиеИС.ВыделитьШтрихкодыСодержащиеВидыПродукции(
			КодыМеха, КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииМЕХ());
...
Функция ДокументыВПроцессеПередачиМеждуОрганизациями(ОбработкаОбъект) Экспорт
	...
	Для Каждого ВидПродукции Из ВидыПродукцииОСУ Цикл
		Если ВидыПродукцииИСМП.Найти(ВидПродукции) <> Неопределено Тогда
			ОСУ.Добавить(ВидПродукции);
		КонецЕсли;
	КонецЦикла;
	
	ТаблицаДатКонтроля = Новый ТаблицаЗначений;
	ТаблицаДатКонтроля.Колонки.Добавить("ВидПродукции", Новый ОписаниеТипов("СправочникСсылка.ВидыПродукцииИС"));
	ТаблицаДатКонтроля.Колонки.Добавить("ДатаКонтроля", Новый ОписаниеТипов("Дата"));
	Для Каждого ВидПродукции Из УчитываемыеВидыПродукции Цикл
		СтрокаТаблицы = ТаблицаДатКонтроля.Добавить();
		СтрокаТаблицы.ВидПродукции = ВидПродукции;
		ДатаУПДОСУ = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДатаНачалаОформленияУПДОСУ(ВидПродукции);
		ДатаУПДПоштучно = КонструкторИСМПОбщегоНазначенияКлиентСервер.ДатаНачалаОформленияУПДПоштучно(ВидПродукции);
		Если Не ЗначениеЗаполнено(ДатаУПДОСУ) Тогда
			СтрокаТаблицы.ДатаКонтроля = ДатаУПДПоштучно;
		ИначеЕсли Не ЗначениеЗаполнено(ДатаУПДПоштучно) Тогда
			СтрокаТаблицы.ДатаКонтроля = ДатаУПДОСУ;
		Иначе
			СтрокаТаблицы.ДатаКонтроля = Мин(ДатаУПДОСУ, ДатаУПДПоштучно);
		КонецЕсли;
	КонецЦикла;
	...
	Запрос.УстановитьПараметр("ТаблицаДатКонтроля", ТаблицаДатКонтроля);
	
	ВидыПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииМЕХ();
	ТоварныеГруппыМЕХ = КонструкторИСОбщегоНазначенияКлиентСервер.ЗначенияЭлементаКонструктораПоВидуПродукции(
		Справочники.ЭлементыКонструктораВидовПродукцииИС.ТоварныеГруппыГИСМТ, ВидыПродукции);
	Запрос.УстановитьПараметр("ТоварныеГруппыМЕХ", ТоварныеГруппыМЕХ);
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Даты.ВидПродукции КАК ВидПродукции,
	|	Даты.ДатаКонтроля КАК ДатаКонтроля
	|ПОМЕСТИТЬ ТаблицаДатКонтроля
	|ИЗ
	|	&ТаблицаДатКонтроля КАК Даты
	|ИНДЕКСИРОВАТЬ ПО
	|	ВидПродукции
	|;
	|
	...
	|ПОМЕСТИТЬ ДокументыПередачиПоОсобеннностямУчета
	|ИЗ
	...
	|		ПО ПередачаТоваровМеждуОрганизациями.Ссылка = ПередачаТоваровМеждуОрганизациямиТовары.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|	ПО ОписаниеНоменклатурыИС.Номенклатура = ПередачаТоваровМеждуОрганизациямиТовары.Номенклатура
	|ГДЕ
	|	ПередачаТоваровМеждуОрганизациями.Склад = &Склад
	|	И ПередачаТоваровМеждуОрганизациями.Организация <> ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация)
	|	И ПередачаТоваровМеждуОрганизациями.ОрганизацияПолучатель <> ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация)
	|	И Не ПередачаТоваровМеждуОрганизациями.ПометкаУдаления
	|	И (ПередачаТоваровМеждуОрганизациямиТовары.Номенклатура,ПередачаТоваровМеждуОрганизациямиТовары.Характеристика)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДокументыПередачиПоОсобеннностямУчета.Ссылка
	|ПОМЕСТИТЬ ДокументыПередачиПоТоварамОснования
	|ИЗ
	|	ДокументыПередачиПоОсобеннностямУчета КАК ДокументыПередачиПоОсобеннностямУчета
	|	ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаДатКонтроля КАК ТаблицаДатКонтроля
	|	ПО ДокументыПередачиПоОсобеннностямУчета.ВидПродукции = ТаблицаДатКонтроля.ВидПродукции
	|ГДЕ
	|	ТаблицаДатКонтроля.ВидПродукции ЕСТЬ NULL
	|	ИЛИ ТаблицаДатКонтроля.ДатаКонтроля <= ДокументыПередачиПоОсобеннностямУчета.Дата
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	|
	...
	|ГДЕ
	|	ПередачаТоваровМеждуОрганизациями.Ссылка В (ВЫБРАТЬ Ссылка ИЗ ДокументыПередачиПоТоварамОснования)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ОСУ)
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаДатКонтроля КАК ТаблицаДатКонтроля
	|	ПО ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ = ТаблицаДатКонтроля.ВидПродукции
	|	И ДокументыПередачиПоСкладу.Дата >= ТаблицаДатКонтроля.ДатаКонтроля
	|ГДЕ
	...
	Содержимое = ЭлектронноеВзаимодействиеИСМП.ЧастичноеСодержимое(ДокументыПередачиСКодамиМаркировки, Ложь);
	...
	Для Каждого СтрокаТаблицы Из Содержимое Цикл
		Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(СтрокаТаблицы.ВидПродукции, Истина)
	...
	|		И ОтгрузкаТоваровИСМП.ТоварнаяГруппа В (&ТоварныеГруппыМЕХ)
	...
	|		И ПриемкаТоваровИСМП.ТоварнаяГруппа В (&ТоварныеГруппыМЕХ)
...
Функция ТоварыКРасчетуПередачиМеждуОрганизациями(Документы, Склад) Экспорт
	...
	ИспользуемыеВидыПродукции = Новый Массив; //Массив Из СправочникСсылка.ВидыПродукцииИС
	ПартионныеВидыПродукции = Новый Массив; //Массив Из СправочникСсылка.ВидыПродукцииИС
	Запрос = Новый Запрос;
	...
	Массив = Новый Массив; //Массив Из СправочникСсылка.ВидыПродукцииИС
	Если ЕГАИС Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			ИспользуемыеВидыПродукции,
			КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЕГАИСМаркируемые());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			Массив,
			КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЕГАИС());
	КонецЕсли;
	...
	Если ИСМП Тогда
		Для Каждого ВидПродукции Из ОбщегоНазначенияИСМПВызовСервера.УчитываемыеВидыМаркируемойПродукции() Цикл
			Массив.Добавить(ВидПродукции);
			ИспользуемыеВидыПродукции.Добавить(ВидПродукции);
		КонецЦикла;
	КонецЕсли;
	...
	Если ВЕТИС Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			ИспользуемыеВидыПродукции,
			КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииВетИС());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			Массив,
			КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииВетИС());
	КонецЕсли;
	Запрос.УстановитьПараметр("ВЕТИС", Массив);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ПартионныеВидыПродукции, Массив);
	
	Массив = Новый Массив;
	Если ЗЕРНО Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			ИспользуемыеВидыПродукции,
			КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЗЕРНО());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			Массив,
			КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЗЕРНО());
	КонецЕсли;
	...
	Если САТУРН Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			ИспользуемыеВидыПродукции,
			КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииСАТУРН());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			Массив,
			КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииСАТУРН());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			ПартионныеВидыПродукции,
			КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииСАТУРН());
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ВидыПродукции", ИспользуемыеВидыПродукции);
	Запрос.УстановитьПараметр("ВидыПродукцииПартионнаяПродукция", ПартионныеВидыПродукции);
	...
	МассивТекстовЗапросов.Добавить(ИнтеграцияИСУТ.ТекстЗапросаОтгрузкаТоваровКлиенту());
	МассивТекстовЗапросов.Добавить(ИнтеграцияИС.РазделительЗапросовВПакете());
	...
	МассивТекстовЗапросов.Добавить(ТекстЗапросаИтог("ТаблицаТоварыОтгрузкаТоваровКлиенту"));
	МассивТекстовЗапросов.Добавить(ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении());
...
Функция ШтрихкодыУпаковокКПередачеМеждуОрганизациями(Документы, НастройкиЕГАИС) Экспорт
	...
	|	Штрихкоды.Ссылка.Организация КАК ОрганизацияПолучатель
	|ИЗ
	|	Документ.ОтгрузкаТоваровКлиенту.ШтрихкодыУпаковок КАК Штрихкоды
	|ГДЕ
	|	Штрихкоды.Ссылка В (&Документы)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	ВсеШтрихкоды = ШтрихкодыУпаковок.ВыгрузитьКолонку("ШтрихкодУпаковки");
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ВсеШтрихкоды, МаркированныеТовары.ВыгрузитьКолонку("ШтрихкодУпаковки"));
	
	Запрос.УстановитьПараметр("ШтрихкодУпаковки", ВсеШтрихкоды);
	...
	Пока КодыИСМП.Следующий() Цикл
		КодДокумента = ШтрихкодыУпаковок.Найти(КодыИСМП.ШтрихкодУпаковки, "ШтрихкодУпаковки");
		...
		Иначе
			КодДокумента = КодДокумента.ШтрихкодУпаковки;
		КонецЕсли;
...
Процедура ОтфильтроватьОбъектыПоСкладу(Документы, Склад)
	...
	|
	|ВЫБРАТЬ
	|	ОтгрузкаТоваровКлиенту.Ссылка
	|ИЗ
	|	Документ.ОтгрузкаТоваровКлиенту КАК ОтгрузкаТоваровКлиенту
	|ГДЕ
	|	ОтгрузкаТоваровКлиенту.Склад = &Склад
	|	И ОтгрузкаТоваровКлиенту.Ссылка В (&Ссылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция ТекстЗапросаИтог(ИмяВТ, Поместить = Ложь)
	...
	|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЕГАИС В (&ЕГАИС) КАК ЕГАИС,
	|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииВЕТИС В (&ВЕТИС) КАК ВЕТИС,
	|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ИСМП) КАК ИСМП,
	|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО В (&ЗЕРНО) КАК ЗЕРНО,
	|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииСАТУРН В (&САТУРН) КАК САТУРН
	...
	|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЕГАИС В (&ЕГАИС),
	|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииВЕТИС В (&ВЕТИС),
	|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ИСМП),
	|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО В (&ЗЕРНО),
	|	ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииСАТУРН В (&САТУРН)
	|ИМЕЮЩИЕ
...
Процедура РаспределитьШтрихКодыУпаковок(Объект)
	...
	ШтрихкодыУпаковок = Объект.ШтрихкодыУпаковок.ВыгрузитьКолонку("ШтрихкодУпаковки");
	Товары = ШтрихкодированиеИС.ВложенныеШтрихкодыИзвестныхУпаковок(ШтрихкодыУпаковок).МаркированныеТовары;
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		УпаковкаДокумента = СтрокаТЧ.ШтрихкодУпаковки;
		Если ЗначениеЗаполнено(СтрокаТЧ.УпаковкаВерхнегоУровня) Тогда
			УпаковкаДокумента = СтрокаТЧ.УпаковкаВерхнегоУровня.ШтрихкодУпаковки;
		КонецЕсли;
	...
	Если Объект.НастройкиЕГАИС.Количество() > 0 Тогда
		...
		Пока Справки2.Следующий() Цикл
			...
			УпаковкаДокумента = Товар.УпаковкаВерхнегоУровня.ШтрихкодУпаковки;
			...
			Если ПоискОтправителя.Количество() = 0 Тогда
				Продолжить;
			КонецЕсли;
			Количество = ?(Товар.Количество = 0, 1, Товар.Количество);
			Поиск = СтруктураПоискаТовары();
...
Процедура ЗаполнитьОтгрузкуТоваровИСМП(Документ, ДокументПередачи, Товары)
	...
	ТоварныеГруппы = КонструкторИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппаПоВидуПродукции(
		КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииМЕХ());
	Если Не ЗначениеЗаполнено(ТоварныеГруппы) Тогда
		Возврат;
	КонецЕсли;
	
	Документ.ТоварнаяГруппа = ТоварныеГруппы[0];
...
Функция ВыделитьТоварыППЗ(ТоварыЗЕРНО)
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО ДанныеШтрихкодовУпаковок.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО В (&ВидыПродукцииППЗ)
	|";
	...
	Для Каждого СтрокаТаблицы Из ТоварыЗЕРНО Цикл
		СтрокаТаблицы = ВидыПродукцииПоНоменклатуре.Найти(СтрокаТаблицы.Номенклатура, "Номенклатура");
		Если СтрокаТаблицы <> Неопределено Тогда
			СтрокаТаблицы.ППЗ = Истина;
		КонецЕсли;
	КонецЦикла;
