...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВариантНавигации = Перечисления.ВариантыНавигацииВФормахНоменклатуры.ПоНоменклатуре Тогда
		
		ПоискПустойНоменклатуры = СписокНоменклатуры.НайтиПоЗначению(Справочники.Номенклатура.ПустаяСсылка());
		Если ПоискПустойНоменклатуры <> Неопределено Тогда
...
Функция ПреобразоватьРасширеннуюНастройкуВУпрощенную(Форма)
	...
	Для Каждого ЭлементОтбора Из Форма.КомпоновщикНастроекОтбор.Настройки.Отбор.Элементы Цикл
		...
		Если Строка(ЭлементОтбора.ЛевоеЗначение) = Строка(ПолеТолькоВНаличии)
			И ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно Тогда
			...
			Если ЭлементОтбора.Использование Тогда
				...
				Иначе
					УпрощеннаяНастройка.ВариантНаличия = 2;
				КонецЕсли;
				
			Иначе
				УпрощеннаяНастройка.ВариантНаличия = 3;
			КонецЕсли;
