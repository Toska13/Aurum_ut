...
Процедура ЗаполнитьРеквизитыШапкиТОРГ12(ДанныеПечати, Макет, ТабличныйДокумент)
	...
	Если ПродажиСервер.АдресГрузополучателяБратьИзАдресаДоставки(ДанныеПечати.Ссылка)
...
Процедура ЗаполнитьРеквизитыШапкиСчетФактура(ДанныеПечати, ДанныеОснований, СведенияОПоставщике, ДанныеКонтрагентов, ТабличныйДокумент, ОпцииПечатиСчетаФактуры, ДанныеПоставщиков)
	...
	Если ОпцииПечатиСчетаФактуры.ВерсияПостановления1137 >= 8 Тогда 
	...
	ИначеЕсли ОпцииПечатиСчетаФактуры.ВерсияПостановления1137 = 9 Тогда 
		НомераДаты.НомерПостановления = УчетНДСПереопределяемый.НомерПостановления26();
		НомераДаты.ДатаПостановления = УчетНДСПереопределяемый.ДатаПостановления26();
	КонецЕсли;
	...
	Иначе
		...
		Для Каждого СтрокаТаблицы Из ТаблицаПоставщиков Цикл
			...
			ПредставлениеПоставщика = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1; %2'"),
				ПредставлениеПоставщика,
				ФормированиеПечатныхФорм.ОписаниеОрганизации(СтрокаТаблицы.СведенияОПоставщике, "ПолноеНаименование", Ложь));
			
			ПредставлениеАдресПоставщика = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1; %2'"),
				ПредставлениеАдресПоставщика,
				ФормированиеПечатныхФорм.ОписаниеОрганизации(СтрокаТаблицы.СведенияОПоставщике, "ЮридическийАдрес", Ложь));
...
Процедура ЗаполнитьТабличныйДокументСчетФактура(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ОпцииПечатиСчетаФактуры) Экспорт
	...
	Макет2026_2_5        = УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.ПФ_MXL_СчетФактура2026");
	Макет2026Прослеж_2_5 = УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.ПФ_MXL_СчетФактура2026Прослеж");
	...
	МакетКорр_2026_2_5        = УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.ПФ_MXL_КорректировочныйСчетФактура2026");
	МакетКорр_2026Прослеж_2_5 = УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.ПФ_MXL_КорректировочныйСчетФактура2026Прослеж");
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Иначе
			...
			ДействующиеПостановления.Вставить(
				"Постановление2026",
				ДействуетПостановление2026(ДанныеПечати.Дата, ДанныеПечати.ДатаИсправления));
			ДействующиеПостановления.Вставить(
				"Постановление26",
				ДействуетПостановление26(ДанныеПечати.Дата, ДанныеПечати.ДатаИсправления));
			...
			ОпцииПечатиСчетаФактуры.Вставить("ВерсияПостановления1137", ВерсияПостановления1137);
				...
				Если ВерсияПостановления1137 >= 8 Тогда
					...
					Если ИспользоватьПечатныеФормыСФ2_5 Тогда
						...
						Если ДанныеПечати.ЕстьПрослеживаемыеТовары Тогда
							Макет = МакетКорр_2026Прослеж_2_5;
						Иначе
							Макет = МакетКорр_2026_2_5;
						КонецЕсли;
					Иначе
			...
			Если (ЕстьПостановление914 И ЕстьПостановление1137)
				...
				Или ЕстьПостановление1096 И (ЕстьПостановление914
											...
											Или ЕстьПостановление1137_625
											Или ЕстьПостановление981
											Или ЕстьПостановление534)
				Или ЕстьПостановление2026 И (ЕстьПостановление1096
											Или ЕстьПостановление914
											Или ЕстьПостановление1137
											Или ЕстьПостановление1137_625
											Или ЕстьПостановление981
											Или ЕстьПостановление534)
				Или ЕстьПостановление26 И (ЕстьПостановление1096
											Или ЕстьПостановление914
											Или ЕстьПостановление1137
											Или ЕстьПостановление1137_625
											Или ЕстьПостановление981
											Или ЕстьПостановление534
											Или ЕстьПостановление2026) Тогда
			...
			СтрокаБезРасхождений = ДанныеПечати.КорректировочныйСчетФактура
									...
									И СтрокаТовары.Количество = СтрокаТовары.КоличествоДо
			...
			Если Не СтрокаБезРасхождений Тогда
				...
				ПараметрыПечати.Вставить("ПечатьСчетаФактуры",           Истина);
	...
	Для Каждого Элемент Из СтруктураСуммПоСтроке Цикл
			...
			Если ПрослеживаемыеТовары <> Неопределено И ПрослеживаемыеТовары.Количество() > 0 Тогда 
				Для Каждого СтрокаПрослеживаемость Из ПрослеживаемыеТовары Цикл
					СтоимостьПоРНПТСтрокиКомплекта = СтоимостьПоРНПТСтрокиКомплекта(СтрокаТовары.РазницаБезНДСУвеличение, СтрокаПрослеживаемость.ОбщаяСуммаПоКомплекту,
						СтрокаПрослеживаемость.СуммаПоРНПТ, ПараметрыПечати.ВерсияПостановления1137);

					ИтоговыеСуммы["СтоимПрослежУвеличениеВсего"] = ИтоговыеСуммы["СтоимПрослежУвеличениеВсего"] + СтоимостьПоРНПТСтрокиКомплекта;
				КонецЦикла;
			Иначе
				ИтоговыеСуммы["СтоимПрослежУвеличениеВсего"] = ИтоговыеСуммы["СтоимПрослежУвеличениеВсего"] + ?(ПараметрыПечати.ВерсияПостановления1137 >= 8, Окр(СтрокаТовары.РазницаБезНДСУвеличение, 2), Окр(СтрокаТовары.РазницаБезНДСУвеличение));
			КонецЕсли;
		ИначеЕсли РассчитатьИтогиСтоимостиПрослеж И Элемент.Ключ = "СтоимПрослежУменьшение" Тогда		
			Если ПрослеживаемыеТовары <> Неопределено И ПрослеживаемыеТовары.Количество() > 0 Тогда 
				...
				КонецЦикла;
			Иначе
				ИтоговыеСуммы["СтоимПрослежУменьшениеВсего"] = ИтоговыеСуммы["СтоимПрослежУменьшениеВсего"] + ?(ПараметрыПечати.ВерсияПостановления1137 >= 8, Окр(СтрокаТовары.РазницаБезНДСУменьшение, 2), Окр(СтрокаТовары.РазницаБезНДСУменьшение));
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьРеквизитыСтрокиТовара2_5(СтрокаТовары, ПрослеживаемыеТовары, ОбластьМакета, НомерСтроки, ПараметрыПечати, ВыводитьКодНоменклатуры = Истина, ВыводитьКодТНВД = Неопределено, ВыводитьСерии = Ложь)
	...
	СтруктураПараметров.Вставить("ОблагаетсяАкцизом", Ложь);
	СтруктураПараметров.Вставить("Акциз", 0);
	СтруктураПараметров.Вставить("АкцизДоИзменения", 0);
	СтруктураПараметров.Вставить("АкцизПослеИзменения", 0);
	...
	Если ПараметрыПечати.Корректировочный Тогда
		...
		Если ПараметрыПечати.СчетФактураНаАванс Тогда
			СтруктураПараметров.Вставить("ТоварКодТНВЭДДоИзменения",      "");
			СтруктураПараметров.Вставить("КоличествоДоИзменения",         "");
			СтруктураПараметров.Вставить("НаименованиеЕдиницыИзмерения",  "");
			СтруктураПараметров.Вставить("ЦенаДоИзменения",               "");
			СтруктураПараметров.Вставить("СтоимостьБезНДСДоИзменения",    "");
		Иначе
		...
		Если ПрослеживаемыеТовары <> Неопределено И ПрослеживаемыеТовары.Количество() > 0 Тогда
			...
			КоличествоПоРНПТДо = "";
			КоличествоПоРНПТ = "";
			Если ПараметрыПечати.ВерсияПостановления1137 >= 7 Тогда
			...
			Для Каждого СтрокаПрослеживаемость Из ПрослеживаемыеТовары Цикл
				...
				Если ПараметрыПечати.ВерсияПостановления1137 >= 7 Тогда
					ИзменениеСтоимости = СтоимостьПоРНПТСтрокиКомплекта(СтрокаТовары.СуммаБезНДС, СтрокаПрослеживаемость.ОбщаяСуммаПоКомплекту,
						СтрокаПрослеживаемость.СуммаПоРНПТ, ПараметрыПечати.ВерсияПостановления1137) - СтоимостьПоРНПТСтрокиКомплекта(СтрокаТовары.СуммаБезНДСДо, СтрокаПрослеживаемость.ОбщаяСуммаПоКомплекту,
						СтрокаПрослеживаемость.СуммаПоРНПТ, ПараметрыПечати.ВерсияПостановления1137);
		...
		ИначеЕсли ПараметрыПечати.ЕстьПрослеживаемыеТовары Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТовары.КоличествоПоРНПТДо) Тогда
				СтруктураПараметров.Вставить("СтоимПрослежДо",                  ?(ПараметрыПечати.ВерсияПостановления1137 >= 8, Окр(СтрокаТовары.СуммаБезНДСДо, 2), Окр(СтрокаТовары.СуммаБезНДСДо)));
			Иначе
				СтруктураПараметров.Вставить("СтоимПрослежДо",                  "");
			КонецЕсли;

			Если ЗначениеЗаполнено(СтрокаТовары.КоличествоПоРНПТ) Тогда
				СтруктураПараметров.Вставить("СтоимПрослежПосле",               ?(ПараметрыПечати.ВерсияПостановления1137 >= 8, Окр(СтрокаТовары.СуммаБезНДС, 2), Окр(СтрокаТовары.СуммаБезНДС)));
			Иначе	
			...
			Если ЗначениеЗаполнено(СтрокаТовары.КоличествоПоРНПТДо) ИЛИ ЗначениеЗаполнено(СтрокаТовары.КоличествоПоРНПТ) Тогда
				СтруктураПараметров.Вставить("СтоимПрослежУвеличение",          ?(ПараметрыПечати.ВерсияПостановления1137 >= 8, Окр(СтрокаТовары.РазницаБезНДСУвеличение, 2), Окр(СтрокаТовары.РазницаБезНДСУвеличение)));
				СтруктураПараметров.Вставить("СтоимПрослежУменьшение",          ?(ПараметрыПечати.ВерсияПостановления1137 >= 8, Окр(СтрокаТовары.РазницаБезНДСУменьшение, 2), Окр(СтрокаТовары.РазницаБезНДСУменьшение)));
				Если ПараметрыПечати.ВерсияПостановления1137 >= 7 Тогда
	...
	Иначе
		...
		Если ПрослеживаемыеТовары <> Неопределено И ПрослеживаемыеТовары.Количество() > 0 Тогда
			...
			Для Каждого СтрокаПрослеживаемость Из ПрослеживаемыеТовары Цикл
				...
				Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаПрослеживаемость, "НомерСтрокиСФ") Тогда
					...
					СтоимПрослеж = СтоимПрослеж + ?(ПараметрыПечати.ВерсияПостановления1137 >= 8, Окр(СтрокаПрослеживаемость.СуммаПоРНПТ, 2), Окр(СтрокаПрослеживаемость.СуммаПоРНПТ)) + Символы.ПС;
		...
		ИначеЕсли СтрокаТовары.КоличествоПоРНПТ <> 0 Тогда
			...
			СтруктураПараметров.Вставить("СтоимПрослеж",                    ?(ПараметрыПечати.ВерсияПостановления1137 >= 8, Окр(СтрокаТовары.СуммаБезНДС, 2), Окр(СтрокаТовары.СуммаБезНДС)));
		Иначе
	...
	Если ПараметрыПечати.ПечатьСчетаФактуры Тогда
		ТекстБезАкциза = НСтр("ru='без акциза'", Метаданные.Языки.Русский.КодЯзыка);
	Иначе
		ТекстБезАкциза = НСтр("ru='без
							|акциза'", Метаданные.Языки.Русский.КодЯзыка);
	КонецЕсли;
	
	Если Не СтруктураПараметров.ОблагаетсяАкцизом Или Не ПараметрыПечати.ВерсияПостановления1137 >= 7 Тогда
		СтруктураПараметров.Вставить("Акциз", ТекстБезАкциза);
		СтруктураПараметров.Вставить("АкцизДоИзменения", ТекстБезАкциза);
		СтруктураПараметров.Вставить("АкцизПослеИзменения", ТекстБезАкциза);
	КонецЕсли;
...
Функция СтоимостьПоРНПТСтрокиКомплекта(Знач СуммаБезНДС, Знач ОбщаяСуммаПоКомплекту, Знач СуммаПоРНПТ, Знач ВерсияПостановления1137 = 1)
	...
	Стоимость = ?(ВерсияПостановления1137 >= 8, Окр(Стоимость, 2), Окр(Стоимость));
...
Функция СформироватьПечатнуюФормуУПД(СтруктураТипов, ОбъектыПечати, ПараметрыПечати = Неопределено) Экспорт
	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		ИначеЕсли ИмяОбъекта = "ОтчетКомиссионера" Тогда
			...
			Если ПараметрыПечати <> Неопределено И ПараметрыПечати.Свойство("Выданный") И ПараметрыПечати.Выданный Тогда
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОснованияНаРеализацию, СтруктураОбъектов.Значение);
			Иначе
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОснованияКомиссионеру, СтруктураОбъектов.Значение);
			КонецЕсли;
...
Процедура ЗаполнитьТабличныйДокументУКД(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати) Экспорт
	...
	МакетУКД_2026_2_5         = УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.ПФ_MXL_УниверсальныйКорректировочныйДокумент2026");
	МакетУКД_2026Прослеж_2_5  = УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.ПФ_MXL_УниверсальныйКорректировочныйДокумент2026Прослеж");
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		ИначеЕсли НЕ ТипЗнч(ДанныеПечати.Ссылка) = Тип("ДокументСсылка.СчетФактураВыданный") 
			...
			И (ДанныеПечати.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС
			ИЛИ ДанныеПечати.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ЛьготноеНалогообложениеНДСНаУСН)
			И (Не ДанныеПечати.СтатусУПД = 2 ИЛИ ДанныеПечати.Номер = Неопределено) Тогда
		...
		Иначе
			...
			Если ВерсияПостановления1137 >= 8 Тогда
				...
				Если ИспользоватьПечатныеФормыСФ2_5 Тогда
					ОпцииПечати.ИспользоватьПечатныеФормыСФ2_5 = ИспользоватьПечатныеФормыСФ2_5;
					Если ДанныеПечати.ЕстьПрослеживаемыеТовары Или ЕстьПрослеживаемыеТоварыВТабличнойЧасти Тогда
						Макет = МакетУКД_2026Прослеж_2_5;
					Иначе
						Макет = МакетУКД_2026_2_5;
					КонецЕсли;
				Иначе
					Макет = МакетУКД_1096;
				КонецЕсли;
		...
		ИначеЕсли (ЕстьПостановление1137_625 Или ЕстьПостановление1137 Или ЕстьПостановление981 Или ЕстьПостановление534 Или ЕстьПостановление1096 Или ЕстьПостановление2026)
			...
			Если ДанныеПечати.РеализацияЧерезКомиссионера И ДанныеДляПечати.Свойство("РезультатПоКонтрагентам") Тогда
...
Процедура ЗаполнитьРеквизитыШапкиУКД(ДанныеПечати, ДанныеОснований, СведенияОПоставщике, СведенияОПокупателе, ДанныеПоставщиков, Макет, ТабличныйДокумент, ОпцииПечати)
	...
	ИначеЕсли ОпцииПечати.ВерсияПостановления1137 = 9 Тогда 
		НомераДаты.Вставить("НомерПостановления", УчетНДСПереопределяемый.НомерПостановления26());
		НомераДаты.Вставить("ДатаПостановления", УчетНДСПереопределяемый.ДатаПостановления26());
	КонецЕсли;
	...
	Если СтрокиОснований.Количество() > 1 И Не ДанныеПечати.СтатусУПД = 2 Тогда
	...
	ИначеЕсли СтрокиОснований.Следующий() И ЗначениеЗаполнено(СтрокиОснований.НомерСчетаФактуры)
		И Не ДанныеПечати.СтатусУПД = 2 Тогда
...
Процедура ЗаполнитьРеквизитыПодвалаУКД2_5(ОбластьПодвала, ОбластьПодвалаНакладной, ДанныеПечати, ДанныеОснований, СведенияОбОрганизации, СведенияОКонтрагенте, ОпцииПечати)
	...
	ТипДокумента = ТипЗнч(ДанныеПечати.Ссылка);
...
Функция ДействуетПостановление1096(ДатаДокумента, ДатаИсправления)
	...
	Возврат ДатаДокумента >= '20241001';
	
КонецФункции

Функция ДействуетПостановление2026(ДатаДокумента, ДатаИсправления)
	
	Возврат ДатаДокумента >= '20260101';
...
Функция ТекстСообщенияПостановление2026()
	
	Возврат НСтр("ru = 'Для заполнения строки 5б включите опцию ""Использовать печатные формы СФ, УПД, УКД 2.5"" (раздел ""НСИ и администрирование"" - ""Учет НДС, импорт и экспорт"").'");
