...
#Область ОбработчикиСобытийЭлементовТаблицыФормыОстаткиПродукции
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСоставУпаковки
...
Процедура СоставУпаковкиПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если НоваяСтрока Тогда
		ПроверкаИПодборПродукцииИСКлиентСервер.ПересчитатьНомераСтрок(СоставУпаковки);
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыИтоговоеДеревоУпаковок
...
Процедура СгенерироватьСерии(Команда)
	...
	Для Каждого СтрокаСостава Из СоставУпаковки Цикл
		...
		Если СтрокаСостава.ЭтоНабор Тогда
			Для Каждого ВложеннаяЕдиница Из СтрокаСостава.ВложенныеЕдиницы Цикл
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ВложеннаяЕдиница);
				ПолныйСоставУпаковки.Добавить(НоваяСтрока);
				НеобходимоУказатьСерию = НеобходимоУказатьСерию
					Или ИнтеграцияИСКлиентСервер.НеобходимоУказатьСерию(НоваяСтрока.СтатусУказанияСерий);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	
	Если ПолныйСоставУпаковки.Количество() = 0 Или (
		Не НеобходимоУказатьСерию
	...
	РезультатГенерацииСерий = СгенерироватьСерииНаСервере(ПолныйСоставУпаковки);
	
	Если РезультатГенерацииСерий.ЗаполнениеЗавершено Тогда
		ЗаполнитьСерииВСоставеУпаковок(ПолныйСоставУпаковки);
	КонецЕсли;
...
Процедура УправлениеЭлементамиФормыПриСоздании()
	...
	Заголовок = СтрШаблон(НСтр("ru = 'Виртуальная агрегация по товарной группе: %1'"), ТоварнаяГруппа);
	...
	ВидимостьКолонкиШаблон = ИнтеграцияИСМПКлиентСервер.ШаблоныКодовПоВидуПродукции(ТоварнаяГруппа).Количество() > 1;
...
Процедура СформироватьЗаголовокКоличествоУпаковок()
	...
	МассивПодстрок.Добавить(ПросклонятьУпаковку(КоличествоУпаковок, ТоварнаяГруппа));
...
Процедура ДобавитьМонотоварнуюУпаковку(СтрокаОстаткиПродукции, НовыеСтроки, МаксимальныйИндекс)
	...
	Если НоваяСтрока Тогда
		...
		Если СтрокаСостава.ХешСуммаУпаковки = "" Тогда
			ЗаполнитьПредставлениеПустогоЗначенияНабораИГрупповойУпаковки(СтрокаСостава, ТоварнаяГруппа);
		КонецЕсли;
	Иначе
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьПредставлениеПустогоЗначенияНабораИГрупповойУпаковки(СтрокаСостава, ТоварнаяГруппа)
	
	Если СтрокаСостава.ЭтоНабор Тогда
		ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Набор");
		СтрокаСостава.ПредставлениеУпаковки = КонструкторИСМПОбщегоНазначенияКлиентСервер.ПредставлениеВидаУпаковкиПоВидуПродукции(
			ТоварнаяГруппа, ВидУпаковки);
	ИначеЕсли СтрокаСостава.ЭтоГрупповаяУпаковка Тогда
		ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Групповая");
		СтрокаСостава.ПредставлениеУпаковки = КонструкторИСМПОбщегоНазначенияКлиентСервер.ПредставлениеВидаУпаковкиПоВидуПродукции(
			ТоварнаяГруппа, ВидУпаковки);
	КонецЕсли;
...
Процедура ПриВыбореСтрокиСоставаУпаковки(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("ТоварнаяГруппа",                   ТоварнаяГруппа);
...
Процедура ОбработатьИПроверитьПереданныеПараметры(Отказ)
	...
	ТоварнаяГруппа    = Параметры.ТоварнаяГруппа;
	Документ          = Параметры.Документ;
	ЭтоЗаказНаЭмиссию = ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаказНаЭмиссиюКодовМаркировкиСУЗ");
	
	Если ЭтоЗаказНаЭмиссию Тогда
		РеквизитыЗаказа = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Документ, "ТоварнаяГруппа,СервисПровайдер,ДокументОснование");
		Если Не ЗначениеЗаполнено(ТоварнаяГруппа) Тогда
			ТоварнаяГруппа = РеквизитыЗаказа.ТоварнаяГруппа;
		КонецЕсли;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ТоварнаяГруппа) Тогда
		ТекстОшибки = НСтр("ru = 'В форму агрегации не передана товарная группа.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки,,,,Отказ);
		Возврат;
	КонецЕсли;
	...
	Если Результат.Пустой() Тогда
		Если ЭтоЗаказНаЭмиссию Тогда
			Если ЗначениеЗаполнено(РеквизитыЗаказа.СервисПровайдер) Тогда
		...
		ТекстОшибки = СтрШаблон(ТекстОшибки, Документ, ТоварнаяГруппа);
	...
	Иначе
		...
		Если ИспользоватьСерииНоменклатуры Тогда
			Если ЭтоЗаказНаЭмиссию Тогда
				ОснованиеЗаказаНаЭмиссию = РеквизитыЗаказа.ДокументОснование;
			Иначе
...
Функция НачатьПолучениеДанныхЗаполнения()
	...
	ПараметрыПроцедуры.Вставить("ТоварнаяГруппа", ТоварнаяГруппа);
...
Процедура ЗаполнитьЭлементДереваСуществующихУпаковок(Приемник, Источник, ЕдиницыИзмерения, Параметры)
	...
	Для Каждого СтрокаИсточника Из Источник.Строки Цикл
		...
		Если Параметры.ЭтоПроизвольнаяУпаковка Тогда
			Если ЭтоПервыйУровень Тогда
				СтрокаПриемника.ПредставлениеУпаковки = ПросклонятьЕдиницу(СтрокаПриемника.Количество, ТоварнаяГруппа, Истина);
				СтрокаПриемника.ПредставлениеУпаковкиИтог = ПросклонятьУпаковку(СтрокаПриемника.КоличествоИтог, ТоварнаяГруппа);
			ИначеЕсли СтрокаПриемника.ЭтоНабор Тогда
				СтрокаПриемника.ПредставлениеУпаковки = ПросклонятьНабор(СтрокаПриемника.Количество, ТоварнаяГруппа);
				СтрокаПриемника.ПредставлениеУпаковкиИтог = ПросклонятьНабор(СтрокаПриемника.КоличествоИтог, ТоварнаяГруппа);
			ИначеЕсли СтрокаПриемника.ЭтоГрупповаяУпаковка Тогда
				СтрокаПриемника.ПредставлениеУпаковки = ПросклонятьГрупповуюУпаковку(СтрокаПриемника.Количество, ТоварнаяГруппа);
				СтрокаПриемника.ПредставлениеУпаковкиИтог = ПросклонятьГрупповуюУпаковку(СтрокаПриемника.КоличествоИтог, ТоварнаяГруппа);
			Иначе
				СтрокаПриемника.ПредставлениеУпаковки = ЕдиницаИзмерения;
				СтрокаПриемника.ПредставлениеУпаковкиИтог = ПросклонятьЕдиницу(СтрокаПриемника.КоличествоИтог, ТоварнаяГруппа);
			КонецЕсли;
		ИначеЕсли ЭтоПоследнийУровень Тогда
			Если СтрокаПриемника.ЭтоНабор Тогда
				СтрокаПриемника.ПредставлениеУпаковки = ПросклонятьЕдиницу(СтрокаПриемника.Количество, ТоварнаяГруппа);
				СтрокаПриемника.ПредставлениеУпаковкиИтог = ПросклонятьНабор(СтрокаПриемника.КоличествоИтог, ТоварнаяГруппа);
			...
			ИначеЕсли СтрокаПриемника.ЭтоГрупповаяУпаковка Тогда
				...
				Если Не ЭтоПервыйУровень Тогда
					Родитель = СтрокаПриемника.ПолучитьРодителя();
					Родитель.ПредставлениеУпаковки = ПросклонятьГрупповуюУпаковку(Родитель.Количество, ТоварнаяГруппа);
				КонецЕсли;
			Иначе
				СтрокаПриемника.ПредставлениеУпаковки = ЕдиницаИзмерения;
				СтрокаПриемника.ПредставлениеУпаковкиИтог = ПросклонятьУпаковку(СтрокаПриемника.КоличествоИтог, ТоварнаяГруппа);
...
Функция ДобавитьНовыеМонотоварныеУпаковкиВИтоговоеДеревоУпаковок()
	...
	НаименованиеСодержимогоГрупповойУпаковки = КонструкторИСМПОбщегоНазначенияКлиентСервер.ПредставлениеВидаУпаковкиПоВидуПродукции(
		ТоварнаяГруппа,
		ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Групповая"));
	НаименованиеСодержимогоГрупповойУпаковки = НРег(НаименованиеСодержимогоГрупповойУпаковки);
	...
	Для ИндексСтрокиСоставаУпаковки = 0 По ВГраницаСоставаУпаковки Цикл
		...
		Если СтрокаСоставаУпаковки.ЭтоНабор Тогда
			Если СтрокаСоставаУпаковки.ВложенныеЕдиницы.Количество() = 1
				И (СтрокаСоставаУпаковки.ВложенныеЕдиницы[0].Номенклатура <> СтрокаСоставаУпаковки.Номенклатура
				Или СтрокаСоставаУпаковки.ВложенныеЕдиницы[0].Характеристика <> СтрокаСоставаУпаковки.Характеристика) Тогда
				КоличествоСоставляющихНабора = 2;
			Иначе
		...
		ПредставлениеНоменклатуры = ПредставлениеНоменклатуры(
			...
			СтрокаСоставаУпаковки.ПредставлениеНоменклатуры, ТоварнаяГруппа));
		...
		Если ЗначениеЗаполнено(СтрокаСоставаУпаковки.ХешСуммаУпаковки) Тогда
			...
			Для Индекс = 0 По ВГраница Цикл
				...
				КонецЕсли;
				СтрокаАгрегации.ПредставлениеУпаковкиИтог = ПросклонятьУпаковку(СтрокаАгрегации.КоличествоИтог, ТоварнаяГруппа);
			...
			Если СтрокаСоставаУпаковки.ЭтоНабор Тогда
				...
				КонецЕсли;
				СтрокаАгрегации.ПредставлениеУпаковки = ПросклонятьЕдиницу(СтрокаАгрегации.Количество, ТоварнаяГруппа);
				СтрокаАгрегации.КоличествоИтог = СтрокаСоставаУпаковки.Количество;
				СтрокаАгрегации.ПредставлениеУпаковкиИтог = ПросклонятьНабор(СтрокаАгрегации.КоличествоИтог, ТоварнаяГруппа);
			ИначеЕсли СтрокаСоставаУпаковки.ЭтоГрупповаяУпаковка
				...
				СтрокаАгрегации.ПредставлениеУпаковкиИтог = ПросклонятьГрупповуюУпаковку(СтрокаАгрегации.КоличествоИтог, ТоварнаяГруппа);
			КонецЕсли;
...
Функция ДобавитьНовуюПроизвольнуюУпаковкуВИтоговоеДеревоУпаковок()
	...
	СтрокаАгрегации.ПредставлениеУпаковки     = ПросклонятьЕдиницу(СтрокаАгрегации.Количество, ТоварнаяГруппа, Истина);
	СтрокаАгрегации.ПредставлениеУпаковкиИтог = ПросклонятьУпаковку(КоличествоУпаковок, ТоварнаяГруппа);
	...
	Для Каждого СтрокаСоставаУпаковки Из СоставУпаковки Цикл
		...
		Если СтрокаПродукции = Неопределено Тогда
			...
			ПредставлениеНоменклатуры = ИнтеграцияИСМПКлиентСервер.ПредставлениеGTINОстаткиПоВидуПродукции(
				СтрокаПродукции.ПредставлениеНоменклатуры, ТоварнаяГруппа);
		...
		Если СтрокаПродукции.ЭтоНабор Тогда
			...
			СтрокаПродукции.ПредставлениеУпаковки     = ПросклонятьНабор(СтрокаПродукции.Количество, ТоварнаяГруппа);
			СтрокаПродукции.ПредставлениеУпаковкиИтог = ПросклонятьНабор(СтрокаПродукции.КоличествоИтог, ТоварнаяГруппа);
		ИначеЕсли СтрокаПродукции.ЭтоГрупповаяУпаковка Тогда
			СтрокаПродукции.Коэффициент  = СтрокаСоставаУпаковки.ВложенныеЕдиницы[0].Количество;
			СтрокаПродукции.ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.GS1_DataMatrix");
			СтрокаПродукции.ПредставлениеУпаковки     = ПросклонятьГрупповуюУпаковку(СтрокаПродукции.Количество, ТоварнаяГруппа);
			СтрокаПродукции.ПредставлениеУпаковкиИтог = ПросклонятьГрупповуюУпаковку(СтрокаПродукции.КоличествоИтог, ТоварнаяГруппа);
		Иначе
			СтрокаПродукции.Коэффициент               = 1;
			СтрокаПродукции.ПредставлениеУпаковки     = СтрокаСоставаУпаковки.ЕдиницаИзмерения;
			СтрокаПродукции.ПредставлениеУпаковкиИтог = ПросклонятьЕдиницу(СтрокаПродукции.КоличествоИтог, ТоварнаяГруппа);
		КонецЕсли;
...
Процедура ПриГенерацииШтрихкодаУпаковки(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.ТипШтрихкода <> ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC")
		И Результат.ТипШтрихкода <> ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC18")
		И Результат.ТипШтрихкода <> ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC20")
...
Процедура СформироватьПредставлениеПараметровГенерациШтрихкодов(СтрокаАгрегации)
	...
	Если СтрокаАгрегации.ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC")
		Или СтрокаАгрегации.ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC18")
		Или СтрокаАгрегации.ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC20") Тогда
...
Процедура СформироватьЗаголовокСохраненныхПараметровГенерацииШтрихкодов()
	...
	Иначе
		...
		Если ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC")
			Или ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC18")
			Или ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC20") Тогда
...
Функция НачатьЗаписьУпаковок()
	...
	ПроверитьТипШтрихкодаSSCC(ДеревоУпаковок.Строки);
...
&НаСервере
Процедура ПроверитьТипШтрихкодаSSCC(Строки)
	
	Для Каждого Строка Из Строки Цикл
		
		Если Строка.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC
			Или Строка.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC18 Тогда
			Строка.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC20;
		КонецЕсли;
		
		Если Строка.Строки.Количество() > 0 Тогда
			ПроверитьТипШтрихкодаSSCC(Строка.Строки);
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция СгенерироватьСерииНаСервере(ДанныеДляГенерацииСерий)
	
	Возврат ИнтеграцияИС.СгенерироватьСерии(Неопределено, ДанныеДляГенерацииСерий, Неопределено, Неопределено);
...
&НаКлиентеНаСервереБезКонтекста
Функция ПросклонятьУпаковку(Количество, ТоварнаяГруппа)
	
	СклоненияСлова = НовыйСклоненияСлова();
	ВидУпаковки    = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Групповая");
	
	СклоненияСлова.ИменительныйЕдЧисло = КонструкторИСМПОбщегоНазначенияКлиентСервер.ПредставлениеВидаУпаковкиПоВидуПродукции(
		ТоварнаяГруппа, ВидУпаковки,,, Истина);
	СклоненияСлова.ИменительныйМнЧисло = КонструкторИСМПОбщегоНазначенияКлиентСервер.ПредставлениеВидаУпаковкиПоВидуПродукции(
		ТоварнаяГруппа, ВидУпаковки,, "Множественное", Истина);
	СклоненияСлова.РодительныйМнЧисло = КонструкторИСМПОбщегоНазначенияКлиентСервер.ПредставлениеВидаУпаковкиПоВидуПродукции(
		ТоварнаяГруппа, ВидУпаковки, "Родительный", "Множественное", Истина);
...
&НаКлиентеНаСервереБезКонтекста
Функция ПросклонятьЕдиницу(Количество, ТоварнаяГруппа, ПроизвольнаяУпаковка = Ложь)
...
&НаКлиентеНаСервереБезКонтекста
Функция ПросклонятьНабор(Количество, ТоварнаяГруппа)
...
&НаКлиентеНаСервереБезКонтекста
Функция ПросклонятьГрупповуюУпаковку(Количество, ТоварнаяГруппа)
