...
Процедура УправлениеФормой()
	...
	Для Каждого ТипДокумента Из ТипыДокументов Цикл
		...
		Иначе
			...
			НомерДокумента = ?(ЗначениеЗаполнено(ДанныеРаспознанного.НомерДокумента),
				СтрШаблон(НСтр("ru = '№ %1'"), СокрЛП(ДанныеРаспознанного.НомерДокумента)),
				"");
			ДатаДокумента = ?(ЗначениеЗаполнено(ДанныеРаспознанного.ДатаДокумента),
				СтрШаблон(НСтр("ru = 'от %1'"), Формат(ДанныеРаспознанного.ДатаДокумента, "ДЛФ=D")), "");
			ЗаголовокДокумента = Элементы["СоздатьДокумент_" + Ид].Заголовок + " """ + ТипДокумента
				+ """" + СтрШаблон(НСтр("ru = ' %1 %2'"), НомерДокумента, ДатаДокумента);
			Элементы["СоздатьДокумент_" + Ид].Заголовок = СтрЗаменить(ЗаголовокДокумента, "  ", " ");
		КонецЕсли;
	...
	Если НаправлениеДокумента = Перечисления.НаправленияРаспознанногоДокумента.Входящий Тогда
		...
		ВОтКонтрагента = ?(ЗначениеЗаполнено(ДанныеЗаголовка.Контрагент),
			СтрШаблон(НСтр("ru = 'от %1'"), ДанныеЗаголовка.Контрагент),
			"");
	Иначе
		СтрокаТипДокумента = НСтр("ru = 'Реализация'");
		ВОтКонтрагента = ?(ЗначениеЗаполнено(ДанныеЗаголовка.Контрагент),
			СтрШаблон(НСтр("ru = 'в %1'"), ДанныеЗаголовка.Контрагент),
			"");
	КонецЕсли;
	
	ПредставлениеДатыДокумента = ?(ЗначениеЗаполнено(ДанныеЗаголовка.ДатаДокумента),
		СтрШаблон(НСтр("ru = 'от %1'"), Формат(ДанныеЗаголовка.ДатаДокумента, "ДЛФ=D")), "");
		
	Заголовок = СтрШаблон(НСтр("ru = '%1 %2 %3'"),
		СтрокаТипДокумента, ПредставлениеДатыДокумента, ВОтКонтрагента);
		
	УправленияСвойствамиКомандФормы(ЭтотОбъект);
...
Процедура УправленияСвойствамиКомандФормы(Форма)
	
	ДоступностьКнопкиПоУмолчанию = Ложь;
	Для Каждого Строка Из Форма.РеквизитыДокументовНаФорме Цикл
		Если Строка.СоздатьДокумент Или ЗначениеЗаполнено(Строка.Ссылка) Тогда
			ДоступностьКнопкиПоУмолчанию = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Форма.Элементы.ОбработатьДокументы.Доступность = ДоступностьКнопкиПоУмолчанию;
