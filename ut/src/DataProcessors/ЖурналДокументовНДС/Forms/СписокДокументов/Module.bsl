...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_СчетФактураВыданный"
		...
		ИЛИ ИмяСобытия = "Запись_БилетовИБронирований"
...
Процедура Подключаемый_ВыполнитьКомандуИнтеграции(Команда)
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыполнитьПодключаемуюКомандуИнтеграции(Команда, ЭтотОбъект, Элементы.СписокДокументыНДС);
...
Процедура ОбработатьШтрихкоды(Данные)
	...
	ДанныеПоШтрихКоду = ДанныеПоШтрихКодуПечатнойФормы(Данные.Штрихкод, "СписокДокументыНДС");
...
Функция ОписаниеОперацийИТиповДокументов()
	...
	СтрокаБронирование							= ТЗХозОперацииИТипыДокументов.Добавить();
	Строка										= СтрокаБронирование;
	Строка.ХозяйственнаяОперация				= Перечисления.ХозяйственныеОперации.ВходящийНДСПоПриобретению;
	Строка.КлючНазначенияИспользования			= "ДокументыВходящие";
	Строка.ЗаголовокРабочегоМеста				= НСтр("ru = 'Электронные билеты, подтверждающие оплату сумм НДС'");
	Строка.ТипДокумента 						= Тип("ДокументСсылка.ОперацияСЭлектроннымБилетом");
	Строка.ПолноеИмяДокумента					= Метаданные.Документы.ОперацияСЭлектроннымБилетом.ПолноеИмя();
	Строка.ИспользуютсяСтатусы					= Ложь;
	Строка.ДобавитьКнопкуСоздать				= Ложь;
