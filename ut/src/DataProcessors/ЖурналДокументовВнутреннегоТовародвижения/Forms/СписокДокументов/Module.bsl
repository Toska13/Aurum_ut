...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ПараметрыОповещенияЭДО.ИмяДинамическогоСписка = "СписокДокументыВнутреннегоТовародвижения";
...
Процедура СписокУстановитьПометкуУдаления(Команда)
	...
	ОбщегоНазначенияУТКлиент.УстановитьПометкуУдаления(Элементы.СписокДокументыВнутреннегоТовародвижения, Заголовок,
		Оповещение, РаботаСЖурналамиДокументовКлиент.ПараметрыОбновленияСпискаДокументов(ЭтотОбъект,
...
Процедура Подключаемый_ВыполнитьКомандуИнтеграции(Команда)
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыполнитьПодключаемуюКомандуИнтеграции(Команда, ЭтотОбъект, Элементы.СписокДокументыВнутреннегоТовародвижения);
...
Процедура Подключаемый_ВыполнитьКомандуЭДО(Команда)
	...
	Если Не ЭтоКомандаТекущиеДелаЭДО И Не ЭтоКомандаПроизвольныйДокумент Тогда
		Если Элементы.СписокДокументыВнутреннегоТовародвижения.ТекущиеДанные <> Неопределено Тогда
			ТекущийДокумент = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Элементы.СписокДокументыВнутреннегоТовародвижения.ТекущиеДанные, "Ссылка");
	...
	ЭлектронноеВзаимодействиеКлиент.ВыполнитьПодключаемуюКомандуЭДО(Команда, ЭтотОбъект, Элементы.СписокДокументыВнутреннегоТовародвижения);
...
Процедура Подключаемый_ОбновитьКомандыЭДО()
	...
	ОбменСКонтрагентамиКлиент.ОбновитьКоманды(ЭтотОбъект, Элементы.СписокДокументыВнутреннегоТовародвижения);
...
Процедура ОбработатьШтрихкоды(Данные)
	...
	ДанныеПоШтрихКоду = ДанныеПоШтрихКодуПечатнойФормы(Данные.Штрихкод, "СписокДокументыВнутреннегоТовародвижения");
...
Процедура ОбновитьГиперссылкуКОформлению()
	...
	Если НеобходимаГиперссылкаКОформлению Тогда
		...
		Если РезультатРасчета.Статус = "Выполнено" Тогда
			КОформлению = ОбновитьГиперссылкуКОформлениюЗавершение(РезультатРасчета.АдресРезультата);
...
Процедура ОбновитьГиперссылкуКОформлениюОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		КОформлению = ОбновитьГиперссылкуКОформлениюЗавершение(Результат.АдресРезультата);
	КонецЕсли;
