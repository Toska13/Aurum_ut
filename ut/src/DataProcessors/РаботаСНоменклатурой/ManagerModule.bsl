...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ДобавитьКВыгрузкеНоменклатуруПоОтбору", Истина);
	Методы.Вставить("ДобавитьКВыгрузкеНоменклатуруПоСписку", Истина);
	Методы.Вставить("ЗаписатьДанныеВРегистрНоменклатураКВыгрузке", Истина);
	Методы.Вставить("НоваяНоменклатураПоОрганизациям", Истина);
	Методы.Вставить("ОбновитьСостоянияНовойНоменклатуры", Истина);
	Методы.Вставить("ОчиститьСписокВыгрузки", Истина);
	Методы.Вставить("СброситьСостоянияВыгрузкиНоменклатуры", Истина);
...
// Создает команды и элементы формы для отборов по состояниям выгрузки номенклатуры.
//
// Параметры:
//   Форма     - ФормаКлиентскогоПриложения - форма, в которой создаются команды и элементы отбора.
//   ИмяГруппы - Строка - имя группы на форме для размещения элементов.
//   ВидКнопки - Неопределено, ВидКнопкиФормы - вид создаваемой кнопки (например, гиперссылка).
//
Процедура СоздатьКомандыСостояния(Форма, ИмяГруппы, ВидКнопки = Неопределено) Экспорт
	...
	КартинкиСостояний.Вставить("ОжидаетПодписания",      БиблиотекаКартинок.ПодписанЭП);
	...
	Для Каждого ПараметрыСостояния Из КартинкиСостояний Цикл
		...
		Если ЗаполнитьСписокСостояний Тогда
			СписокСостояний = Форма.СписокСостояний;	// СписокЗначений из Произвольный
			СписокСостояний.Добавить(СостояниеВыгрузки, ИмяКоманды);
		КонецЕсли;
	КонецЦикла;
