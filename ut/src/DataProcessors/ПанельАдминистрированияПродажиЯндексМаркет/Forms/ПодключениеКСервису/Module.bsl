...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(КлючДоступа) Тогда	
		ПолучитьПраваДоступаКлючаНаКлиенте();
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементов()
	...
	Элементы.ГруппаПолучитьКод.Видимость = ЗначениеЗаполнено(Элементы.ДекорацияОписаниеПравДоступа.Заголовок) И ЗначениеЗаполнено(КлючДоступа);
	
КонецПроцедуры

&НаКлиенте
Процедура ПолучитьПраваДоступаКлючаНаКлиенте()
	
	ДлительнаяОперация = ПолучитьПраваДоступаКлюча();
	
	Элементы.ДекорацияОписаниеПравДоступа.Заголовок = "";
	УстановитьВидимостьЭлементов();
	
	ОжидатьЗавершениеДлительнойОперации(ДлительнаяОперация, "ПолучениеПраваДоступаКлючаЗавершение");  
...
&НаСервере
Функция ПолучитьПраваДоступаКлюча()

	ИмяМетода = "ИнтеграцияСЯндексМаркетСервер.ПолучитьДанныеКлючаДоступа";
...
&НаКлиенте
Процедура ПолучениеПраваДоступаКлючаЗавершение(РезультатЗадания, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(РезультатЗадания) = Тип("Структура") Тогда
		...
		ИначеЕсли РезультатЗадания.Статус = "Выполнено" 
		 		  И РезультатЗадания.Свойство("АдресРезультата") Тогда
			Результат = ИнтеграцияСЯндексМаркетВызовСервера.ПолучитьРезультатВыполненияФоновогоЗадания(РезультатЗадания.АдресРезультата);

			Если Не ПустаяСтрока(Результат.КодОшибки) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(Результат.ОписаниеОшибки);
			Иначе
				Элементы.ДекорацияОписаниеПравДоступа.Заголовок = Результат.Описание; 
				УстановитьВидимостьЭлементов();
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура КлючДоступаНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	ТекущийЭлемент = Элементы.Подключить;
