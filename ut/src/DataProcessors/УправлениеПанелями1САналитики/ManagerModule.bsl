...
Функция ЗагрузитьПанелиИсточникаИзJSON(Параметры) Экспорт
	...
	Для Каждого РабочийСтол Из Параметры.Содержимое Цикл

		ДанныеРабочегоСтола = JSONВЗначение(РабочийСтол);
		...
		Для Каждого Элемент Из ДанныеРабочегоСтола Цикл
			...
			Если ПараметрыЧтениеИзФайла.ИдентификаторыНастроекПанелей <> Неопределено Тогда
				Индекс = Параметры.Содержимое.Найти(РабочийСтол);
				СтрокаТаблицыИдентификаторов = ПараметрыЧтениеИзФайла.ИдентификаторыНастроекПанелей[Индекс];
				СтрокаТаблицыИдентификаторов.ИдентификаторыЭлементов.Добавить(Элемент["id"]);
			КонецЕсли;
		КонецЦикла;
		
		ЗагрузитьВладельцев(ПараметрыЧтениеИзФайла.ВладельцыАналитикиИзФайла, МассивВладельцев, ОчиститьТаблицу);
		ЗагрузитьПанели(ПараметрыЧтениеИзФайла.ПанелиИзФайла, МассивРабочихСтолов, ОчиститьТаблицу);
		...
		ЗагрузитьОбъектыАналитики(ПараметрыЧтениеИзФайла.ОбъектыАналитикиИзФайла, МассивОбъектовАналитики, ОчиститьТаблицу);
		ЗагрузитьПапки(ПараметрыЧтениеИзФайла.ПапкиИзФайла, Неопределено, МассивПапок, ОчиститьТаблицу);
		ЗагрузитьДиаграммы(ПараметрыЧтениеИзФайла.ДиаграммыИзФайла, Неопределено, МассивДиаграмм, ОчиститьТаблицу);
		ЗагрузитьДашборды(ПараметрыЧтениеИзФайла.ДашбордыИзФайла, Неопределено, МассивДашбордов, ОчиститьТаблицу);
...
// * ИдентификаторыНастроекПанелей - Неопределено, ТаблицаЗначений
Функция ПараметрыЧтениеИзФайла() Экспорт
	...
	Структура.Вставить("ИдентификаторыНастроекПанелей", Неопределено);
...
Функция СкопироватьПанель(Параметры) Экспорт
	...
	ДополнительныеИсточники = Параметры.ДополнительныеИсточники;
	ИдентификаторыНастроекПанелей = Параметры.ИдентификаторыНастроекПанелей;
	...
	Если ВыбранныйОбъект.Тип = "folder" И ИзФайла Тогда
		...
		ПеренестиОбъектыИзФайла(ПереносыПоВладельцам, ВладельцыАналитики, ОбъектыАналитикиИсточник, Папки,
			ДополнительныеИсточники, ИдентификаторыНастроекПанелей, АдресСервера, ИзФайла);
	Иначе
...
// * ИдентификаторыДляПерезаписи - Неопределено, СписокЗначений
Функция ПараметрыКопированияПанелей() Экспорт
	...
	Структура.Вставить("ИдентификаторыДляПерезаписи", Неопределено);
	Структура.Вставить("ДополнительныеИсточники", Неопределено);
	Структура.Вставить("ИдентификаторыНастроекПанелей", Неопределено);
...
Процедура ПриЗагрузкеФайлаНастройки(ДанныеНастройки, ДанныеФайла, Обработан, ДополнительныеПараметры) Экспорт
	
	Обработан = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеПараметры, "ЗагружатьБезОбработки", Ложь);
	
КонецПроцедуры

// См.ПоставляемыеНастройкиПереопределяемый.ПриДобавленииНастроек
Процедура ПриДобавленииНастроек(Настройки) Экспорт
	
	Настройка = ПоставляемыеНастройкиСервер.ОписаниеНастройки();
	Настройка.Идентификатор              = ИдентификаторМанифеста();
	Настройка.ОбновлятьАвтоматически     = Ложь;
...
Процедура ЗагрузитьОбъектыАналитики(ТаблицаОбъектов, ОбъектыАналитикиИзСхемы = Неопределено, ОчищатьТаблицу = Истина)
...
Процедура ЗагрузитьВладельцев(ТаблицаВладельцев, Владельцы = Неопределено, ОчищатьТаблицу = Истина)
...
Процедура ЗагрузитьПанели(ТаблицаПанелей, ПанелиИзСхемы = Неопределено, ОчищатьТаблицу = Истина)
...
Процедура ЗагрузитьПапки(ТаблицаПапок, ИдентификаторыПредставленныхОбъектов = Неопределено, ПапкиИзСхемы = Неопределено, ОчищатьТаблицу = Истина)
...
Процедура ЗагрузитьДиаграммы(ТаблицаДиаграмм, ИдентификаторыПредставленныхОбъектов = Неопределено, ДиаграммыИзСхемы = Неопределено, ОчищатьТаблицу = Истина)
...
Процедура ЗагрузитьДашборды(ТаблицаДашбордов, ИдентификаторыПредставленныхОбъектов = Неопределено, ДашбордыИзСхемы = Неопределено, ОчищатьТаблицу = Истина)
			...
			Если Перенос.ВозможенСЗамещением Тогда
				...
				Если Панель.OwnerName = ВладелецИмяНовыхПанелей Тогда
					...
					Если Строка.Тип = "folder" Тогда
						...
						ДополнительныеПараметры.НовоеИмяПапки = Строка.Название;
...
Процедура ПеренестиОбъектыИзФайла(
	ПереносыПоВладельцам,
	ВладельцыАналитики,
	ОбъектыАналитикиИсточник,
	Папки,
	ДополнительныеИсточники,
	ИдентификаторыНастроекПанелей,
	АдресСервера,
	ИзФайла)
	...
	Для Каждого ПереносПоВладельцу Из ПереносыПоВладельцам Цикл
		...
		Для Каждого ОбъектАналитики Из ОбъектыКПереносу.ОбъектыАналитики Цикл
			ЗаписатьСвязанныеДопИсточники(ОбъектАналитики["id"], ДополнительныеИсточники, ИдентификаторыНастроекПанелей,
				АдресСервера, ИзФайла);
			СхемаJson = ЗначениеВJSON(ОбъектАналитики);
			ЗаписатьСхему(СхемаJson)
		КонецЦикла;
		Для Каждого Диаграмма Из ОбъектыКПереносу.Диаграммы Цикл
			ЗаписатьСвязанныеДопИсточники(Диаграмма["id"], ДополнительныеИсточники, ИдентификаторыНастроекПанелей,
				АдресСервера, ИзФайла);
			СхемаJson = ЗначениеВJSON(Диаграмма);
			ЗаписатьСхему(СхемаJson);
		КонецЦикла;
		Для Каждого Дашборд Из ОбъектыКПереносу.Дашборды Цикл
			ЗаписатьСвязанныеДопИсточники(Дашборд["id"], ДополнительныеИсточники, ИдентификаторыНастроекПанелей,
				АдресСервера, ИзФайла);
...
Функция ДополнительныеПараметрыПолейПапки()
	...
	Структура.Вставить("НовоеИмяПапки", Неопределено);
...
Функция ПоляПапки(Папка, ДополнительныеПараметры = Неопределено)
	...
	НовоеИмяПапки = ДополнительныеПараметры.НовоеИмяПапки;
	...
	Папка_Name = ?(НовоеИмяПапки = Неопределено, Папка.Name, НовоеИмяПапки);
	...
	Если Дополнительные_FileIds <> Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Папка_FileIds, Дополнительные_FileIds, Истина);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Папка_Links, Дополнительные_FileIds, Истина);
	КонецЕсли;
	
	Папка_FolderIds = Папка.Data_FolderIds.ВыгрузитьЗначения();
	Если Дополнительные_FolderIds <> Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Папка_FolderIds, Дополнительные_FolderIds, Истина);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Папка_Links, Дополнительные_FolderIds, Истина);
	КонецЕсли;
	...
	ПоляПапки.Вставить("name", Папка_Name);
...
Процедура СоздатьИПодключитьКПанели(Панель, ВыбранныйОбъект, Параметры, Идентификатор = Неопределено)

	ЭтоРекурсивныйВызов = Ложь;
	Если Идентификатор = Неопределено Тогда
		ИдентификаторОбъекта = Параметры.ИдентификаторОбъекта;
	Иначе
		ИдентификаторОбъекта = Идентификатор;
		ЭтоРекурсивныйВызов = Истина;
	КонецЕсли;
	ИзФайла = Параметры.ИзФайла;
	ВладельцыАналитики = Параметры.ВладельцыАналитики;
	ДиаграммыИсточник = Параметры.ДиаграммыИсточник;
	...
	Дашборды = Параметры.Дашборды;
	ПанелиИсточник = Параметры.ПанелиИсточник;
	ИдентификаторыДляПерезаписи = Параметры.ИдентификаторыДляПерезаписи;
	...
	ИдентификаторПоиск = ИдентификаторыДляПерезаписи.НайтиПоЗначению(ИдентификаторОбъекта);
	Если ИдентификаторПоиск <> Неопределено Тогда
		ИдентификаторНовогоОбъекта = ИдентификаторПоиск.Значение;
	ИначеЕсли ИзФайла Тогда
	...
	ИначеЕсли ОбъектИсточник.Type = "dashboard" Тогда
		...
		Если ИзФайла Тогда
			Для Каждого ПолеСсылка Из ПоляНовогоОбъекта["links"] Цикл
				ДиаграммаВСоставеДашборда = НайтиПоИдентификатору(ДиаграммыИсточник, ПолеСсылка);
				
				Если ДиаграммаВСоставеДашборда = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				
				ВложенныйОбъект = ПанелиИсточник.Строки.Найти(ПолеСсылка, "Идентификатор", Истина);
				СоздатьИПодключитьКПанели(Панель, ВложенныйОбъект, Параметры, ДиаграммаВСоставеДашборда.Id)
			КонецЦикла;
		КонецЕсли;
	...
	Если ПоляПользователяАналитики["links"].Найти(ИдентификаторНовогоОбъекта) = Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			ВладелецАналитики.links,
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИдентификаторНовогоОбъекта),
			Истина);
	КонецЕсли;
	...
	Для Каждого Элемент Из ПоляНовогоОбъекта["links"] Цикл
		...
		ИдентификаторПоиск = ИдентификаторыДляПерезаписи.НайтиПоЗначению(Элемент);
		Если ИдентификаторПоиск <> Неопределено Тогда
			ИдНовогоОбъектаАналитики = ИдентификаторПоиск.Значение;
		Иначе
		...
		Если ПоляПользователяАналитики["links"].Найти(ИдНовогоОбъектаАналитики) = Неопределено Тогда
			...
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
				ВладелецАналитики.links,
				ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИдНовогоОбъектаАналитики),
				Истина);
		КонецЕсли;
		...
		КонецЕсли;
		ПоляПользователяАналитики["data"] = ЗначениеВJSON(Пользователь_DataMap);
		ВладелецАналитики.Data = ПоляПользователяАналитики["data"];
		...
		Если СсылкиСвязанногоОбъекта <> Неопределено Тогда
			Для Каждого Элемент2 Из СсылкиСвязанногоОбъекта Цикл
				СвязанныйОбъект = НайтиПоИдентификатору(ОбъектыАналитикиИсточник, Элемент2);
	...
	Если Не ЭтоРекурсивныйВызов Тогда
		КорневаяПапка = НайтиПоИдентификатору(Папки, Панель.Data_RootFolderId);
	КонецЕсли;
	...
	Если ИзФайла И ЭтоРекурсивныйВызов Тогда
		
		Для каждого ПапкаИсточник Из Параметры.ПапкиИсточник Цикл
			Если ПапкаИсточник.Data_FileIds.Количество() = 0 Тогда
				Продолжить;
			КонецЕсли;
			
			Data_FileIds = ПапкаИсточник.Data_FileIds.ВыгрузитьЗначения();
			ОбъектПринадлежитПапкеИсточнику = Data_FileIds.Найти(ИдентификаторНовогоОбъекта) <> Неопределено;
			Если ОбъектПринадлежитПапкеИсточнику Тогда
				СкопироватьПапкуИсточник(ПапкаИсточник.Id, Параметры, Панель, ИдентификаторНовогоОбъекта);
			КонецЕсли;
		КонецЦикла;
...
Процедура СкопироватьПапкуИсточник(ИдентификаторПапкиИсточника, Параметры, Панель, ИдентификаторФайла = Неопределено, ИдентификаторПапки = Неопределено)
	
	ПапкиИсточник = Параметры.ПапкиИсточник;
	Папки = Параметры.Папки;
	
	НоваяПапка = НайтиПоИдентификатору(Папки, ИдентификаторПапкиИсточника);
	Если НоваяПапка = Неопределено Тогда
		НоваяПапка = Папки.Добавить();
		
		ПапкаИсточник = НайтиПоИдентификатору(ПапкиИсточник, ИдентификаторПапкиИсточника);
		ИсключенияСвойств = "Data, Data_FileIds, Data_FolderIds, Links";
		ЗаполнитьЗначенияСвойств(НоваяПапка, ПапкаИсточник, , ИсключенияСвойств);
	КонецЕсли;
	...
	Если ИдентификаторФайла <> Неопределено Тогда
		ИдентификаторФайлаВМассиве = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИдентификаторФайла);
		ДополнительныеПараметры.Дополнительные_FileIds = ИдентификаторФайлаВМассиве;
		ДополнитьСписокУникальнымЭлементом(НоваяПапка.Data_FileIds, ИдентификаторФайла);
	КонецЕсли;
	Если ИдентификаторПапки <> Неопределено Тогда
		ИдентификаторПапкиВМассиве = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИдентификаторПапки);
		ДополнительныеПараметры.Дополнительные_FolderIds = ИдентификаторПапкиВМассиве;
		ДополнитьСписокУникальнымЭлементом(НоваяПапка.Data_FolderIds, ИдентификаторПапки);
	КонецЕсли;
	
	ПоляНовойПапки = ПоляПапки(НоваяПапка, ДополнительныеПараметры);
	ПоляНовойПапкиJson = ЗначениеВJSON(ПоляНовойПапки);
	ЗаписатьСхему(ПоляНовойПапкиJson);
	
	НоваяПапка.Data = ПоляНовойПапки["data"];
	НоваяПапка.Links.ЗагрузитьЗначения(ПоляНовойПапки["links"]);
	НоваяПапка.Owner = ПоляНовойПапки["owner"];
	НоваяПапка.OwnerName = ПоляНовойПапки["ownerName"];
	
	Для каждого ПапкаИсточник Из ПапкиИсточник Цикл
		Если ПапкаИсточник.Data_FolderIds.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		Data_FolderIds = ПапкаИсточник.Data_FolderIds.ВыгрузитьЗначения();
		Если Data_FolderIds.Найти(ИдентификаторПапкиИсточника) <> Неопределено Тогда
			
			ЭтоКорневаяПапкаИсточника = (ПапкаИсточник.Name = "ROOT_FOLDER_NAME");
			Если ЭтоКорневаяПапкаИсточника Тогда
				КорневаяПапка = Папки.Найти("ROOT_FOLDER_NAME", "Name");
				
				ДополнитьСписокУникальнымЭлементом(КорневаяПапка.Data_FolderIds, ИдентификаторПапкиИсточника);
				ДополнитьСписокУникальнымЭлементом(КорневаяПапка.Links, ИдентификаторПапкиИсточника);
				
				КорневаяПапка_DataTemp = JSONВЗначение(КорневаяПапка.Data);
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
					КорневаяПапка_DataTemp["folderIds"],
					ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИдентификаторПапкиИсточника),
					Истина);
				КорневаяПапка.Data = ЗначениеВJSON(КорневаяПапка_DataTemp);
			Иначе
				СкопироватьПапкуИсточник(ПапкаИсточник.Id, Параметры, Панель, , НоваяПапка.Id);
			КонецЕсли;
...
Процедура ДополнитьСписокУникальнымЭлементом(Список, НовыйЭлемент)
	МассивВременный = Список.ВыгрузитьЗначения();
	НовыйЭлементВМассиве = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(НовыйЭлемент);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивВременный, НовыйЭлементВМассиве, Истина);
	Список.ЗагрузитьЗначения(МассивВременный);
КонецПроцедуры

Функция ВерсияПанелиПрименима(ВерсияПанели) Экспорт
	
	БольшеРавноМинимальной = Ложь;
	МеньшеРавноМаксимальной = Ложь;
	
	ВерсияКонфигурации = СтрРазделить(Метаданные.Версия, ".", Ложь);
	Редакция    = Число(ВерсияКонфигурации[0]);
	Подредакция = Число(ВерсияКонфигурации[1]);
	Версия      = Число(ВерсияКонфигурации[2]);
	Сборка      = Число(ВерсияКонфигурации[3]);
	
	Если ПустаяСтрока(ВерсияПанели.min_config_version) Тогда
		БольшеРавноМинимальной = Истина;
	Иначе
		МинимальнаяВерсияКонфигурации = СтрРазделить(ВерсияПанели.min_config_version, ".", Ложь);
		МинРедакция    = Число(МинимальнаяВерсияКонфигурации[0]);
		МинПодредакция = Число(МинимальнаяВерсияКонфигурации[1]);
		МинВерсия      = Число(МинимальнаяВерсияКонфигурации[2]);
		МинСборка      = Число(МинимальнаяВерсияКонфигурации[3]);
		
		БольшеРавноМинимальной = Редакция    >= МинРедакция
		                  И Подредакция >= МинПодредакция
		                  И Версия      >= МинВерсия
		                  И Сборка      >= МинСборка
	КонецЕсли;
	
	Если ПустаяСтрока(ВерсияПанели.max_config_version) Тогда
		МеньшеРавноМаксимальной = Истина;
	Иначе
		МаксимальнаяВерсияКонфигурации = СтрРазделить(ВерсияПанели.max_config_version, ".", Ложь);
		МаксРедакция    = Число(МаксимальнаяВерсияКонфигурации[0]);
		МаксПодредакция = Число(МаксимальнаяВерсияКонфигурации[1]);
		МаксВерсия      = Число(МаксимальнаяВерсияКонфигурации[2]);
		МаксСборка      = Число(МаксимальнаяВерсияКонфигурации[3]);
		
		МеньшеРавноМаксимальной = Редакция    <= МаксРедакция
		                   И Подредакция <= МаксПодредакция
		                   И Версия      <= МаксВерсия
		                   И Сборка      <= МаксСборка
	КонецЕсли;
	
	Результат = БольшеРавноМинимальной И МеньшеРавноМаксимальной;
...
Функция ИдентификаторМанифеста() Экспорт
	
	Идентификатор = "Manifest_AnalyticsPanels_Trade_11_5";
...
Процедура ЗаписатьСвязанныеДопИсточники(ИдентификаторОбъекта, ДополнительныеИсточники, ИдентификаторыНастроекПанелей, АдресСервера, ИсточникИзФайла);
	
	// ИнтернетПоддержкаПользователей
	Для каждого Строка Из ИдентификаторыНастроекПанелей Цикл
		Если ИсточникИзФайла <> 2 Или Строка.ДопИсточникСкопирован Тогда
			Продолжить;
		КонецЕсли;
		
		ИдентификаторыВерсииПанели = Строка.ИдентификаторыЭлементов.ВыгрузитьЗначения();
		Если ИдентификаторыВерсииПанели.Найти(ИдентификаторОбъекта) <> Неопределено Тогда
			ИдентификаторыДопИсточников = Строка.ИдентификаторыДопИсточников.ВыгрузитьЗначения();
			
			Если ИдентификаторыДопИсточников.Количество() = 0 Тогда
				Продолжить;
			КонецЕсли;
			
			ПутиКФайлам = Новый Массив;
			Для Каждого АдресДвоичныхДанных Из ПрочитатьФайлыНастроек(ИдентификаторыДопИсточников) Цикл
				ПутиКФайлам.Добавить(АдресДвоичныхДанных);
			КонецЦикла;
			
			СодержимоеВременногоФайла = СодержимоеВременногоФайла(ПутиКФайлам);
			Для каждого Содержимое Из СодержимоеВременногоФайла Цикл
				ИсточникиСтруктура = ДополнительныеИсточникиДляЗагрузки(Содержимое, ДополнительныеИсточники, АдресСервера);
				
				Для каждого Источник Из ИсточникиСтруктура.ДополнительныеИсточникиМассив Цикл
					МодульОбработки = Обработки.УправлениеПанелями1САналитики;
					ИсточникJSON = МодульОбработки.ЗначениеВJSON(Источник);
					МодульОбработки.ЗаписатьСхему(ИсточникJSON);
				КонецЦикла;
			КонецЦикла;
			
			Строка.ДопИсточникСкопирован = Истина;
		КонецЕсли;
	КонецЦикла;
	// Конец ИнтернетПоддержкаПользователей
...
Функция ДополнительныеИсточникиДляЗагрузки(Содержимое, ДополнительныеИсточники, АдресСервера)
	
	ФорматСервера = ВерсииФорматовСервера(АдресСервера).Получить("additional-datasource");
...
// Читает файлы настроек на сервере по указанным идентификаторам.
//
// Параметры:
//   Идентификаторы - Массив из Строка - идентификаторы настроек для чтения
//
// Возвращаемое значение:
//   Массив из Строка - адреса файлов настроек
//
Функция ПрочитатьФайлыНастроек(Идентификаторы) Экспорт
	
	Результат = Новый Массив;
	
	ЗарегистрироватьНастройки(Идентификаторы);
	ПараметрыОбновления = Новый Структура("ЗагружатьБезОбработки", Истина);
	ПоставляемыеНастройкиСервер.ОбновитьПоставляемыеНастройки(Идентификаторы, ПараметрыОбновления);
	ФайлыНастроек = ПоставляемыеНастройкиСервер.ПолучитьФайлыНастроек(Идентификаторы, Истина);
	
	Для Каждого Идентификатор Из Идентификаторы Цикл
		Строка = ФайлыНастроек.Найти(Идентификатор, "Идентификатор");
		Если Строка <> Неопределено Тогда
			Результат.Добавить(Строка.АдресФайла);
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗарегистрироватьНастройки(Идентификаторы)
	...
	ПоставляемыеНастройкиСервер.ДобавитьНастройки(Настройки);
	
КонецПроцедуры

// Конец ИнтернетПоддержкаПользователей

// Возвращает содержимое временных файлов, полученных из временного хранилища по указанным адресам.
//
// Параметры:
//   Адрес - Массив из Строка, Строка - Адрес или массив адресов во временном хранилище, содержащих двоичные данные временных файлов.
//
// Возвращаемое значение:
//   Массив из Строка - Массив строк с содержимым временных файлов.
//
Функция СодержимоеВременногоФайла(Адрес) Экспорт
	
	Если ТипЗнч(Адрес) = Тип("Массив") Тогда
		Адреса = Адрес;
	Иначе
	...
	Для Каждого АдресВременногоХранилища Из Адреса Цикл
		ИмяВременногоФайла = ПолучитьИмяВременногоФайла();
		Данные = ПолучитьИзВременногоХранилища(АдресВременногоХранилища); //ДвоичныеДанные
		...
		Содержимое.Добавить(НовыйТекстовыйДокумент.ПолучитьТекст());
