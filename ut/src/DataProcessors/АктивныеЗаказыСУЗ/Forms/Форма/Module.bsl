...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента = ГруппаИЛИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных(Элементы.СписокЗаказовТоварнаяГруппа.ПутьКДанным);
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = ПредопределенноеЗначение("Справочник.ТоварныеГруппыГИСМТ.ПустаяСсылка");
...
Процедура ИзменитьПометку(Пометка)
	...
	Для Каждого Строка Из СтрокиДляОбработки Цикл
		...
		Если Не Пометка
			Или ЗначениеЗаполнено(Строка.ТоварнаяГруппа)
...
Функция ЗагрузитьЗаказыИзСУЗ(ВызовПослеАвторизацииСУЗ = Ложь, ВызовПослеАвторизацииИСМП = Ложь)
	...
	Если ДоступныеТоварныеГруппы.Количество() = 0 Тогда
		ВозвращаемоеЗначение.ТекстОшибки = НСтр(
			"ru = 'Не найдено ни одной товарной группы, для которой включен учет маркируемой продукции'");
		Возврат ВозвращаемоеЗначение;
	КонецЕсли;
	...
	Если НастройкаОбменаСУЗ.ФорматОбмена = Перечисления.ВерсииФорматаОбменаСУЗ.V3 Тогда
		МассивТоварныхГрупп.Добавить(Неопределено);
	Иначе
		МассивТоварныхГрупп = ДоступныеТоварныеГруппы;
	КонецЕсли;
	
	Для Каждого ТоварнаяГруппа Из МассивТоварныхГрупп Цикл
		
		Результат = ИнтерфейсСУЗ.СтатусыБизнесЗаказов_V2(ТоварнаяГруппа, НастройкаОбменаСУЗ);
		...
		Если Результат.СтатусОбработки <> ПредопределенноеЗначение(
			...
			Иначе
				ОбщегоНазначения.СообщитьПользователю(СтрШаблон(
					НСтр("ru = 'Товарная группа: %1. Ошибка получения данных из СУЗ: %2'"),
					ТоварнаяГруппа,
					Результат.ТекстОшибки));
			КонецЕсли;
			Продолжить;
		...
		Для Каждого Строка Из ЗаказыСУЗ Цикл
			...
			СтрокаСпискаЗаказов.ТоварнаяГруппа =
				ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ТоварнаяГруппаПоРеквизиту("Имя", Строка.productGroup);
	...
	Для Каждого СтрокаСпискаЗаказов Из СписокЗаказов Цикл
		...
		Для Каждого СтрокаЗаказа Из СтрокаСпискаЗаказов.СтрокиЗаказа Цикл
			...
			Если Значение <> Неопределено Тогда
				...
				Если Не ЗначениеЗаполнено(СтрокаСпискаЗаказов.ТоварнаяГруппа) Тогда
					СтрокаСпискаЗаказов.ТоварнаяГруппа = Значение.ТоварнаяГруппа;
				КонецЕсли;
...
Процедура ПолучитьДанныеИзБазыПоGTIN(МассивGTIN, Данные)
	...
	СуффиксПодсистемыМаркировкиДляКлючаВидовПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.СуффиксПодсистемыМаркировкиДляКлючаВидовПродукции();
	...
	Для Каждого Элемент Из СоответствиеGTIN Цикл
		...
		Если НайденныеСтроки.Количество() = 1 Тогда
			...
			Данные[Элемент.Ключ].Вставить("ТоварнаяГруппа",
				КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(
					НайденныеСтроки[0].ВидПродукции, СуффиксПодсистемыМаркировкиДляКлючаВидовПродукции));
...
Функция ПолучитьДанныеИзИСМППоGTIN(МассивGTIN, Данные)
	...
	Если ДанныеПродукции <> Неопределено Тогда
		...
		Для Каждого Элемент Из ДанныеПродукции Цикл
			...
			Данные.Вставить(Элемент.Ключ, Новый Структура);
			Данные[Элемент.Ключ].Вставить("ТоварнаяГруппа",
				КонструкторИСМПОбщегоНазначенияКлиентСервер.ТоварнаяГруппаПоВидуПродукции(Элемент.Значение.ВидПродукции));
...
Процедура ЗаполнитьДокументы(МассивИдентификаторовЗаказов)
	...
	|	Заказ.ТоварнаяГруппа                     КАК ТоварнаяГруппа
	|ИЗ
	...
	Для Каждого СтрокаСпискаЗаказов Из СписокЗаказов Цикл
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			Если Не ЗначениеЗаполнено(СтрокаСпискаЗаказов.ТоварнаяГруппа) Тогда
				СтрокаСпискаЗаказов.ТоварнаяГруппа = НайденнаяСтрока.ТоварнаяГруппа;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура НачатьОткрытиеФормыЗаказаСУЗ(СтрокаСпискаЗаказов)
	...
	Если ЗначениеЗаполнено(СтрокаСпискаЗаказов.ТоварнаяГруппа)
...
&НаСервереБезКонтекста
Функция ЗаполнитьСтрокиЗаказаИзСУЗ(Организация, Идентификатор, ТоварнаяГруппа, Данные, ВызовПослеАвторизации = Ложь)
	...
	Для Каждого СтрокаДанных Из Данные Цикл
		...
		Результат = ИнтерфейсСУЗ.СтатусПулаКодовМаркировкиИзБизнесЗаказа_V2(
			ПараметрыЗапроса, ТоварнаяГруппа, НастройкаОбменаСУЗ);
...
Функция ЗакрытьЗаказыВСУЗ()
	...
	Для Каждого СтрокаСпискаЗаказов Из ВыбранныеСтроки Цикл
		...
		Если Не Отказ Тогда
			...
			ПараметрыВыполнения.Вставить("ТоварнаяГруппа", СтрокаСпискаЗаказов.ТоварнаяГруппа);
...
Функция СообщениеJSON(НастройкаОбменаСУЗ, ВходящиеПараметры)
	...
	СообщениеJSON.ТоварнаяГруппа            = ВходящиеПараметры.ТоварнаяГруппа;
