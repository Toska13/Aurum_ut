...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если УпрощеннаяИнтеграцияБЭД Или Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		Элементы.НастройкаВЕТИС.Заголовок = НСтр("ru='Справочники'");
	Иначе
		Элементы.НастройкаВЕТИС.Заголовок = НСтр("ru='Настройки и справочники'");
	КонецЕсли;
	Элементы.Ответственный.Видимость = Не УпрощеннаяИнтеграцияБЭД;
	Элементы.СтраницыОтборОрганизацияВЕТИС.Видимость = Не УпрощеннаяИнтеграцияБЭД;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ТребуетсяНастроитьПодключениеВетИС Тогда
		ПодключитьОбработчикОжидания("ОткрытьФормуСозданияПодключенияВЕТИС", 0.1, Истина);
	КонецЕсли;
...
Процедура НадписьСообщенияОжидающиеОтправкиНажатие(Элемент)
	...
	ИначеЕсли ТребуетсяНастроитьПодключениеВЕТИС Тогда
		
		ОткрытьФормуСозданияПодключенияВЕТИС();
...
Процедура ОбновитьСпискиДокументов()
	...
	ДобавитьДокумент(
		...
		НСтр("ru='Входящие транспортные операции'"),
		Истина,  // Оформите
		Истина,  // Отработайте
		Истина); // Ожидайте
	...
	ДобавитьДокумент(
		...
		НСтр("ru='Исходящие транспортные операции'"),
		Истина,  // Оформите
		Истина,  // Отработайте
		Истина); // Ожидайте
	...
	ДобавитьДокумент(
		...
		НСтр("ru='Инвентаризации продукции'"),
		Истина,  // Оформите
		Истина,  // Отработайте
		Истина); // Ожидайте
	...
	ДобавитьДокумент(
		...
		НСтр("ru='Запросы складского журнала'"),
		Ложь,    // Оформите
		Истина,  // Отработайте
		Истина); // Ожидайте
	...
	ДобавитьДокумент(
		...
		НСтр("ru='Объединения записей складского журнала'"),
		Ложь,    // Оформите
		Истина,  // Отработайте
		Истина); // Ожидайте
...
Процедура ОтобразитьСостояниеОбмена()
	...
	ТребуетсяНастроитьПодключениеВетИС   = Ложь;
	НастройкиПодключения = РезультатыЗапроса["НастройкиПодключенияВЕТИС"].Выбрать();
	Если НастройкиПодключения.Количество() = 0 Тогда
		ПерваяПроблема = НСтр("ru='Настройте подключение к ВетИС.API'");
		...
		ТребуетсяНастроитьПодключениеВетИС = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьПредставлениеЛогирования()
	...
	Если ПараметрыЛогирования.Включено
		...
		Элементы.ПредставлениеЛогированияЗапросов.РасширеннаяПодсказка.Высота = 4;
	
	Иначе
		
		Элементы.ПредставлениеЛогированияЗапросов.РасширеннаяПодсказка.Высота = 3;
...
&НаКлиенте
Процедура ОткрытьФормуСозданияПодключенияВЕТИС()
	
	ПараметрыОткрытияФормы = Новый Структура;
	ПараметрыОткрытияФормы.Вставить("ЭтоПодключениеСобственнойОрганизации", Истина);
	ОткрытьФорму(
		"РегистрСведений.НастройкиПодключенияВЕТИС.Форма.ФормаНовогоПодключения",
		ПараметрыОткрытияФормы, ЭтотОбъект,,,,,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
