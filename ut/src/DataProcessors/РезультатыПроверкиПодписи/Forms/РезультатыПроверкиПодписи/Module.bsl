...
&НаКлиенте
Процедура НастройкаПроверкиНажатие(Элемент)
	
	ОткрытьФорму("Обработка.РезультатыПроверкиПодписи.Форма.ИнформацияОНастройкахПроверкиСертификатов", , ЭтотОбъект, 
...
Процедура ОтобразитьПротоколНаФорме(СвойстваПодписи, СвойстваДоверенности, РезультатПроверки)
	...
	Если ЗначениеЗаполнено(Доверенность) И Не ПолномочияОграничены Тогда
		Элементы.ЗаголовокПроверкаПолномочийВыполненоТекст.Заголовок =
			ТекстЗаголовкаПолномочияДоверенностиПроверены();
	КонецЕсли;
...
Процедура ПоказатьПроверкуМЧД(ПроверкаДоверенности, СвойстваПодписи, СвойстваДоверенности)
	...
	Для Каждого ВложеннаяПроверка Из ПроверкаДоверенности Цикл 
		...
		ИначеЕсли ПроверкаДоверенности[НаименованиеПроверки] = ПроверкаДоверенности.ПроверкаПолномочий Тогда
			...
			Если ПроверкаДоверенности.ПроверкаПолномочий.Выполнено
				...
				Элементы.ЗаголовокПроверкаПолномочийНеВыполненоТекст.Заголовок =
					НСтр("ru = 'Полномочия не прошли автоматическую проверку'");
			КонецЕсли;
		...
		ИначеЕсли ПроверкаДоверенности[НаименованиеПроверки] = ПроверкаДоверенности.ПроверкаПодписиМЧД Тогда
			...
			ПротоколПроверкиПодписиМЧД = МашиночитаемыеДоверенности.ПротоколПроверкиПодписиМЧД(СвойстваДоверенности);
			
			Если ПротоколПроверкиПодписиМЧД <> Неопределено Тогда
					
				СертификатОтозванИлиИстек = 
					МашиночитаемыеДоверенностиКлиентСервер.СертификатОтозванИлиИстек(ПротоколПроверкиПодписиМЧД);
					
				ПроверкаМЧДВыполненаУспешно = 
					МашиночитаемыеДоверенности.ПроверкаМЧДВыполненаУспешно(ПроверкаДоверенности);
				
				Если СертификатОтозванИлиИстек И ПроверкаМЧДВыполненаУспешно Тогда
					
					Элементы.ЗаголовокПроверкаПодписиМЧДНеВыполненоТекст.Заголовок = 
						НСтр("ru = 'Доверенность не изменена, но подпись, возможно, неверна'");
						
					Элементы.НастройкаПроверки.Видимость = ТекстОшибкиКлассифицирован;
	...
	Элементы.ЗаголовокПредставлениеДоверенности.Заголовок = ПредставлениеРезультатаАвтоматическойПроверкиМЧД;
	...
	Если ДоверенностьОтозвана И ДоверенностьОтозванаНаМоментПодписи
		Или ПредставлениеРезультатаАвтоматическойПроверкиМЧД = МашиночитаемыеДоверенности.ПредставлениеМЧДНеПодходит() Тогда 
		Элементы.ЗаголовокПредставлениеДоверенности.ЦветТекста = ЦветаСтиля.ЦветТекстаНеУдачнаяПроверкаМЧД; 
		Элементы.ГруппаПроверкиШапка.ЦветФона = ЦветаСтиля.ЦветФонаПредупреждения;
	КонецЕсли;
...
Функция ТекстЗаголовкаПолномочияДоверенностиПроверены()
	Возврат НСтр("ru = 'Полномочия проверены'");
КонецФункции
...
Процедура ЗаполнитьРеквизитыПолномочий()
	...
	Если ЗначениеЗаполнено(ПроверкаПолномочий) Тогда
		...
		Если ЭтоПередоверие Тогда
			...
			Если ЗначениеЗаполнено(ПроверкаМЧД.РодительскиеДанныеПолучены) Тогда
				НомераРодительскихДоверенностей = ПроверкаМЧД.РодительскиеДанныеПолучены.НомерДоверенности;
			...
			ПолномочияОграничены = 
				Не ПроверкаПолномочий.Успех Или ЗначениеЗаполнено(ТекстОшибкиПроверкиПолномочий)
				Или (ПравилаПроверкиПолномочийМЧДНастроены(СвойстваДоверенности.Ссылка) И Не ПроверкаПолномочий.Успех);
				
			ПолномочияРодительскойДоверенностиОграничены = 
				Не ПроверкаМЧД.РодительскиеПолномочияСоответствуютПолномочиямПередоверия.Успех
					...
					Или (ПравилаПроверкиПолномочийМЧДНастроены(РодительскаяДоверенность) 
						И Не ПроверкаМЧД.РодительскиеПолномочияСоответствуютПолномочиямПередоверия.Успех);
