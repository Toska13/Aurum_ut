...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПоказыватьПомеченныеНаУдаление = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
	...
	Если НЕ ЭлектронныеДокументыЭДО.ЕстьПравоОбработкиДокументов() Тогда
		...
		НедоступныеКоманды.Добавить("ОтказатьсяОтПодписания");
...
&НаКлиенте
Процедура НадписьПредупреждениеОЗавершенииДокументовТакскомОбработкаНавигационнойСсылки(Элемент,
	НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура ВходящиеЭДПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Для каждого ТекущаяСтрока Из Строки Цикл
		...
		Если Данные.Свойство("КоличествоНепрочитанныхСообщений") Тогда
	...
	Если ВыводитьКоличествоНовыхСообщений Тогда
		УстановитьКоличествоНепрочитанныхСообщений(Строки, ЭлектронныеДокументы);
	КонецЕсли;
...
Процедура ОткрытьНоменклатуруКонтрагентов(Команда)
	...
	СопоставлениеНоменклатурыКонтрагентовКлиент.ОткрытьФормуСпискаНоменклатурыКонтрагентов(Новый Структура, ЭтотОбъект);
...
Процедура ПоказатьКомментарийСостояния(Команда)
	...
	ИнтерфейсДокументовЭДОКлиент.ПоказатьКомментарийКСостояниюДокумента(ТекущийКомментарийСостояния);
...
Функция СписокРаздела(Знач Раздел)
	...
	СпискиРазделов.Вставить("СообщенияВх", "ВходящиеЭД");
	...
	СпискиРазделов.Вставить("СообщенияИсх", "ИсходящиеЭД");
...
Функция СтраницаРаздела(Знач Раздел)
	...
	СтраницыРазделов.Вставить("СообщенияВх", "ВходящиеЭД");
	...
	СтраницыРазделов.Вставить("СообщенияИсх", "ИсходящиеЭД");
...
Функция ТаблицаРаздела(Знач Раздел)
	...
	ТаблицыРазделов.Вставить("СообщенияВх", "ВходящиеЭД");
	...
	ТаблицыРазделов.Вставить("СообщенияИсх", "ИсходящиеЭД");
...
Функция ПоляРаздела(Знач Раздел, Знач ПакетноеОтображение)
	...
	ПоляТаблицыВходящие = "ВидДокумента, ДатаПакета, Дата, Номер, СуммаДокумента, Контрагент, Организация, СостояниеЭДО,
		...
		|ЭлектронныйДокумент, КоличествоНепрочитанныхСообщений";
	ПоляТаблицыИсходящие = "ВидДокумента, ДатаПакета, Дата, Номер, СуммаДокумента, Контрагент, Организация, Подписи,
		...
		|ОтражениеВУчете, ЭлектронныйДокумент, КоличествоНепрочитанныхСообщений";
	...
	ИначеЕсли Раздел = "СообщенияВх" Тогда
		...
		Иначе
			ПоляРаздела = "ВидДокумента, Дата, Номер, СуммаДокумента, Контрагент, Организация, СостояниеЭДО,
				|НаправлениеЭД, Ответственный, ДополнительнаяИнформация, ОтражениеВУчете, Описание, КоличествоНепрочитанныхСообщений";
		КонецЕсли;
	
	ИначеЕсли Раздел = "СообщенияИсх" Тогда
...
Процедура СоздатьОтборПоРазделу(Знач Раздел, Знач НастройкиОтображения,
	...
	ИначеЕсли Раздел = "СообщенияВх" Или Раздел = "СообщенияИсх" Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ГруппаОтбора, "ЕстьНепрочитанныеСообщения", Истина, ВидСравненияКомпоновкиДанных.Равно, , Истина);
...
Процедура ПоказатьКомандыРаздела(Знач Раздел)
	...
	ИмяКомандыОтклонения = ?(ВыполненПереходНаУниверсальныеСообщения, "ОтказатьсяОтПодписания", "ОтклонитьДокумент");
	КомандыРазделов.Вставить("Утвердить", "Обновить, ЗакрытьПринудительно, Перенаправить, УтвердитьЭД, 
		|ВходящиеЭДКонтекстноеМенюУтвердитьЭД, КомандыВходящиеЭД, ПоказыватьПомеченныеНаУдаление," + ИмяКомандыОтклонения);
	КомандыРазделов.Вставить("Подписать", "Обновить, ЗакрытьПринудительно, Перенаправить, ПодписатьВх,
		|КомандыВходящиеЭД, ПоказыватьПомеченныеНаУдаление," + ИмяКомандыОтклонения);
	...
	КомандыРазделов.Вставить("СообщенияВх", "Обновить, ЗакрытьПринудительно, Перенаправить, КомандыВходящиеЭД, 
		|ПоказыватьПомеченныеНаУдаление");
	КомандыРазделов.Вставить("СообщенияИсх", "Обновить, ЗакрытьПринудительно, Перенаправить, КомандыИсходящиеЭД, 
		|ПоказыватьПомеченныеНаУдаление");
	...
	ЭлементыКоманд.Вставить("ОтказатьсяОтПодписания", Элементы.КомандаОтказатьсяОтПодписания);
...
Процедура ОбновитьПросмотрПомеченныхНаУдаление()
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
		ИнтерфейсДокументовЭДОКлиентСервер.КлючОбъектаНастроекИнтерфейсаЭДО(),
		"ОтображатьУдаленные", ПоказыватьПомеченныеНаУдаление);
...
Процедура ИнициализироватьДерево()
	...
	ВыводитьРазделыСообщений = ИнтерфейсДокументовЭДО.ВыводитьЧатУниверсальныхСообщений();
	
	Если ВыводитьРазделыСообщений Тогда
		НовЗапись = СтрокаВходящие.Строки.Добавить();
		НовЗапись.Значение = "СообщенияВх";
	КонецЕсли;
	...
	Если ВыводитьРазделыСообщений Тогда
		НовЗапись = СтрокаИсходящие.Строки.Добавить();
		НовЗапись.Значение = "СообщенияИсх";
	КонецЕсли;
...
Функция РаспознатьДокумент(ЭлектронныйДокумент)
	...
	Если Не ДокументРаспознан 
		...
		РезультатРаспознавания = МодульОбменСГИСЭПД.ПриРаспознаванииДокумента(ТипДокумента, ДокументРаспознан); 
	КонецЕсли;
