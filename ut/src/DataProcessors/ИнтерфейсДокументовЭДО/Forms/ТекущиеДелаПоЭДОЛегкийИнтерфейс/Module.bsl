...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТекущийВариантИнтерфейса <> ВариантыИнтерфейса.Легкий Тогда
		ВызватьИсключение ИнтерфейсДокументовЭДО.ТекстСообщенияОНедоступностиОткрытияФормыТекущихДелЭДО( 
			Метаданные.Обработки.ИнтерфейсДокументовЭДО.Формы.ТекущиеДелаПоЭДОЛегкийИнтерфейс.Синоним,
	...
	ВыполненПереходНаУниверсальныеСообщения = РегламентыЭДО.ВыполненПереходНаУниверсальныеСообщения();
	...
	ОтображатьОбластьПредпросмотра = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(ИмяФормы, 
	...
	Элементы.Состояние.СписокВыбора.Вставить(0, 
		"ТребуетВнимания", НСтр("ru = 'Требует внимания'"),, БиблиотекаКартинок.ТребуетВниманияЖелтыйШарБЭД);
	Элементы.СтатусПриглашения.СписокВыбора.Вставить(0, 
		"ТребуетВнимания", НСтр("ru = 'Требует внимания'"),, БиблиотекаКартинок.ТребуетВниманияЖелтыйШарБЭД);
	
	Если ИнтерфейсДокументовЭДО.ВыводитьЧатУниверсальныхСообщений() Тогда
		Элементы.Состояние.СписокВыбора.Вставить(Элементы.Состояние.СписокВыбора.Количество()-1, 
		"НовыеСообщения", НСтр("ru = 'Новые сообщения'"));
	КонецЕсли;
	...
	ИзменитьВидимостьКолонкиВУчете();
	ИзменитьВидимостьКолонкиКоличестваСообщений();
	
	ОбновитьЗаголовкиЭлементов();
	
	Если ЭтоМобильныйКлиент Тогда
		МобильныйЭДО.НастроитьЭлементыФормыТекущиеДелаЭДОЛегкийИнтерфейс(ЭтотОбъект);
	КонецЕсли;
	...
	УчетныеЗаписиЭДО.НастроитьГруппуПредупреждениеОЗавершенииДокументовТакском(ЭтотОбъект);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли СобытиеОбновитьСписки(ИмяСобытия) Тогда
		...
		Если ТребуетсяПереключениеРаздела Тогда
			УстановитьТекущимРазделомВходящиеЭлектронныеДокументы();
		КонецЕсли;
	...
	ИначеЕсли ИмяСобытия = ИнтерфейсДокументовЭДОКлиент.ИмяСобытияИзмененияНастроекТекущихДелЭДО() Тогда
...
Процедура ПриОткрытии(Отказ)
	...
	КонецЕсли;
	Если ПараметрыПриложения[ИмяПараметра].ТребуетсяПроверкаНаличияУчетнойЗаписиЭДО Тогда
		ПараметрыПриложения[ИмяПараметра].ТребуетсяПроверкаНаличияУчетнойЗаписиЭДО = Ложь;
		ИнтерфейсДокументовЭДОКлиент.ПрерватьВыполнениеОперацииДляПодключенияКЭДО(ЭтотОбъект);
	КонецЕсли;
...
Процедура БоковоеМенюПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Иначе
		...
		УстановитьТекущимЭлементомСписокТекущегоРаздела();
	
	КонецЕсли; 
	
	Если ТекущийРаздел = ПредыдущийРаздел Тогда
		Возврат;
	КонецЕсли;
	
	КлючЗамера = СтрШаблон("ПереключениеРазделаЛегкийИнтерфейс.%1", ТекущийРаздел);
	ОписаниеЗамера = ОбщегоНазначенияБЭДКлиент.НачатьЗамер(КлючЗамера);
...
&НаКлиенте
Процедура НадписьПредупреждениеОЗавершенииДокументовТакскомОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	УчетныеЗаписиЭДОКлиент.ОбработкаНавигационнойСсылкиЗавершенияДокументовТакском(Элемент, 
...
&НаКлиенте
Процедура СтраницыВходящиеИсходящиеПриСменеСтраницы(Элемент, ТекущаяСтраница)

	Если ЭтоМобильныйКлиент Тогда
		СведенияОбЭлементахФормы = ЭтотОбъект["МК_СведенияОбЭлементахФормы"];
		Если СведенияОбЭлементахФормы = Неопределено Тогда
			Возврат;
		КонецЕсли;

		МК_АктивироватьТекущуюСтраницу(СведенияОбЭлементахФормы.СведенияОРазделах[ТекущаяСтраница.Имя]);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура РеквизитыДокументаИОтражениеВУчетеHTMLПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если СтрНайти(ВыбраннаяСсылка, ИдентификаторКомандыСменитьОтветственного()) Тогда
		СменитьОтветственного();
	ИначеЕсли СтрНайти(ВыбраннаяСсылка, ИдентификаторКомандыСоздатьДокументУчета()) Тогда
		СоздатьДокументУчета();
	ИначеЕсли СтрНайти(ВыбраннаяСсылка, ИдентификаторКомандыПодобратьДокументУчета1()) Тогда
		ПодобратьДокументУчета1();
	ИначеЕсли СтрНайти(ВыбраннаяСсылка, ИдентификаторКомандыПодобратьДокументУчета2()) Тогда
		ПодобратьДокументУчета2();
	ИначеЕсли СтрНайти(ВыбраннаяСсылка, ИдентификаторКомандыОткрытьДокументУчета1()) Тогда
		ОткрытьДокументУчета1();
	ИначеЕсли СтрНайти(ВыбраннаяСсылка, ИдентификаторКомандыОткрытьДокументУчета2()) Тогда
		ОткрытьДокументУчета2();
	ИначеЕсли СтрНайти(ВыбраннаяСсылка, ИдентификаторКомандыПоказатьВсеДокументыУчета()) Тогда
		ПоказатьВсеДокументыУчета();
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьФайлСВизуализациейHTMLПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	ОткрытьФайлHTMLПриНажатии(ДанныеСобытия, СтандартнаяОбработка);
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьФайлБезВизуализацииHTMLПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	ОткрытьФайлHTMLПриНажатии(ДанныеСобытия, СтандартнаяОбработка);
КонецПроцедуры
...
&НаКлиенте
Процедура ВходящиеЭлектронныеДокументыОбработкаЗапросаОбновления(Элемент)

	МК_ОбновитьСпискиВходящихИИсходящихДокументов();
...
&НаКлиенте
Процедура ИсходящиеЭлектронныеДокументыОбработкаЗапросаОбновления(Элемент)
...
Процедура ПриглашенияПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Для Каждого ТекущаяСтрока Из Строки Цикл
		...
		Если Не ЗначениеЗаполнено(ДанныеСтроки.Контрагент) Тогда
			ДанныеСтроки.Контрагент = СтрШаблон(НСтр("ru='%1 (ИНН: %2)'", "ru"), 
				ДанныеСтроки.Наименование, ДанныеСтроки.ИНН);
		КонецЕсли;
...
&НаКлиенте
Процедура ОшибкиПриИзменении(Элемент)
...
Асинх Процедура Аннулировать(Команда)
	...
	Если ЭтоПакетнаяОбработка Тогда
		РезультатПроверкиВозможностиАннулирования = АннулированиеДоступноДляПакетов(ОбъектыДействий.ПакетыДокументов);
		Если Не РезультатПроверкиВозможностиАннулирования.Результат Тогда
			ПоказатьПредупреждение(, РезультатПроверкиВозможностиАннулирования.ТекстОшибки);
			Возврат;
		КонецЕсли;
	Иначе
		РезультатПроверкиВозможностиАннулирования = АннулированиеДоступноДляДокументов(
...
&НаКлиенте
Асинх Процедура ЗапроситьУточнение(Команда)
	...
	ЭтоПакетнаяОбработка = ЗначениеЗаполнено(ОбъектыДействий.ПакетыДокументов);
	Если ЭтоПакетнаяОбработка И Не ВыполнениеДействийПоЭДОДоступноДляПакетов(ОбъектыДействий.ПакетыДокументов) Тогда
		ВидДействия = ЛегкийИнтерфейсДокументовЭДОКлиент.ВидыВыполняемыхДействийСДокументом().ЗапроситьУточнение;
		ТекстВопроса = ЛегкийИнтерфейсДокументовЭДОКлиент.ТекстВопросаОВыполненииДействияДляДокументаПакета(ВидДействия);
		Ответ = Ждать ВопросАсинх(ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Если Ответ = КодВозвратаДиалога.Да Тогда
			ОставитьДляОбработкиТолькоТекущийДокумент(ОбъектыДействий);
	...
	ДополнительныеПараметры.Вставить("НаборДействий", НаборДействий);
	ДополнительныеПараметры.Вставить("ОсновноеДействие", ОсновноеДействие);
				
	Обработчик = Новый ОписаниеОповещения("ВыполнитьДействияПоЭДОПослеВводаСтроки", ИнтерфейсДокументовЭДОКлиент, 
		ДополнительныеПараметры);
	
	ДополнительныеПараметры = ОбщегоНазначенияБЭДКлиент.ПараметрыВводаСтроки();
	ДополнительныеПараметры.ЗаголовокФормы = НСтр("ru = 'Укажите причины запроса уточнения'");
	...
	ДополнительныеПараметры.КомментарийОбязательностиВвода =
		НСтр("ru = 'Для уточнения необходимо указать причину.'");
	ОбщегоНазначенияБЭДКлиент.ПоказатьВводСтрокиБЭД(Обработчик, ДополнительныеПараметры);
	
КонецПроцедуры

&НаКлиенте
Асинх Процедура ОтклонитьДокумент(Команда)
	...
	Иначе
		...
		ОтклонитьПодписаниеИсходящегоДокумента(ОбъектыДействий, ОписаниеОповещения);
...
&НаКлиенте
Процедура ОтклонитьПодписаниеИсходящегоДокумента(ОбъектыДействий, ОписаниеОповещения)
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ЭлектронныеДокументы", ОбъектыДействий.ЭлектронныеДокументы);
	ДополнительныеПараметры.Вставить("ПакетыДокументов", ОбъектыДействий.ПакетыДокументов);
	ДополнительныеПараметры.Вставить("ОповещениеУспешногоЗавершения", 
...
Асинх Процедура ОтказатьсяОтПодписания(Команда)
	...
	Если Элементы.СтраницыВходящиеИсходящие.ТекущаяСтраница = Элементы.СтраницаВходящиеЭлектронныеДокументы Тогда
		...
		ЭлектронныеДокументыЭДОКлиент.ОтказатьсяОтПодписания(ОбъектыДействий, ОписаниеОповещения);
...
Процедура ЗагрузитьНовыйДокументСДиска(Команда)
	...
	Шаблон = НСтр("ru = 'Для создания документа на основании файла необходимо подключиться к ЭДО и повторить действие.'");
...
Процедура Создать(Команда)
	...
	Шаблон = НСтр("ru = 'Для продолжения отправки документа необходимо подключиться к ЭДО и повторить действие.'");
...
Процедура НовоеПриглашение(Команда)
	...
	Шаблон = НСтр("ru = 'Для продолжения отправки приглашений необходимо подключиться к ЭДО и повторить действие.'");
...
Процедура ЗагрузитьФайлы(Команда)
	...
	Шаблон = НСтр("ru = 'Для выполнения массовой отправки файлов необходимо подключиться к ЭДО и повторить действие.'");
...
Процедура УстановитьПараметрыСписков()
	...
	Для каждого ИмяСписка Из ИменаСписковФормы() Цикл
		...
		ИначеЕсли ИмяСписка = "Ошибки" Тогда
...
Процедура УстановитьВариантОтображенияДокументовСписка(ИмяСписка, НаправлениеСортировки = Неопределено)
	...
	СлужебныеПоляОтбора.Добавить("ЕстьНепрочитанныеСообщения");
...
Процедура УстановитьОтборыНаСервере(Раздел = Неопределено, ПолеОтбора = Неопределено, ПроверятьПересечениеОтборов = Истина)
	...
	ИначеЕсли ПолеОтбора = "Состояние" 
		...
		ПолеСообщений = "ЕстьНепрочитанныеСообщения";
		ЭлементыУжеДобавлены = 
			ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(ГруппаБыстрыйОтбор, ПолеСообщений).Количество() <> 0;
		Если ЭлементыУжеДобавлены Тогда
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ГруппаБыстрыйОтбор, 
				ПолеСообщений, Истина, ВидСравненияКомпоновкиДанных.Равно, НСтр("ru = 'Новые сообщения'"), Ложь, Режим);
		КонецЕсли;
		
		Если Значение = "НовыеСообщения" Тогда
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ГруппаБыстрыйОтбор, 
				ПолеСообщений, Истина, ВидСравненияКомпоновкиДанных.Равно, НСтр("ru = 'Новые сообщения'"), Истина, Режим);
		Иначе
...
Процедура УстановитьОтборПоСостоянию(Знач Значение, ГруппаБыстрыйОтбор, Знач Режим)
	...
	ИначеЕсли Значение = "ОтказаноВПодписиКонтрагенту" Тогда
		
		СписокСостояний.Добавить(Перечисления.СостоянияДокументовЭДО.ОтказаноВПодписиКонтрагенту);
		
	ИначеЕсли Значение = "КонтрагентОтказалВПодписи" Тогда
		
		СписокСостояний.Добавить(Перечисления.СостоянияДокументовЭДО.КонтрагентОтказалВПодписи);
...
Процедура ПредставлениеСостояния(ДанныеСтроки)
	...
	Если ДанныеСтроки.СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.ТребуетсяУтверждение Тогда
	...
	ИначеЕсли ДанныеСтроки.СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.ОтказаноВПодписиКонтрагенту Тогда
		
		ДанныеСтроки.Состояние = НСтр("ru = 'Отказано контрагенту'");
	
	ИначеЕсли ДанныеСтроки.СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.КонтрагентОтказалВПодписи Тогда
		
		ДанныеСтроки.Состояние = НСтр("ru = 'Контрагент отказал'");
		
	ИначеЕсли ДанныеСтроки.СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.ЗакрытСОтклонениемПриглашения Тогда
		
		ДанныеСтроки.Состояние = НСтр("ru = 'Контрагент отклонил приглашение'");
...
Процедура ПриПолученииДанныхСписков(ИмяЭлемента, Настройки, Строки)
	...
	ИспользоватьКолонкуОтражениеВУчете = ПолучитьФункциональнуюОпцию("ИспользоватьКонтрольОтраженияВУчетеЭДО");
	
	Если ИспользоватьКолонкуОтражениеВУчете Тогда
		МассивДокументов = Строки.ПолучитьКлючи();
		ВсеОснования = ИнтеграцияЭДО.ОбъектыУчетаЭлектронныхДокументов(МассивДокументов);
	КонецЕсли;
	...
	ВыводитьКоличествоНовыхСообщений = Ложь;
	Для Каждого ТекущаяСтрока Из Строки Цикл
		
		ДанныеСтроки = ТекущаяСтрока.Значение.Данные;
		
		Если ДанныеСтроки.Свойство("КоличествоНепрочитанныхСообщений") Тогда
			ВыводитьКоличествоНовыхСообщений = Истина;
		КонецЕсли;
		
		Если ИспользоватьКолонкуОтражениеВУчете Тогда
			
			ОтборОснований = Новый Структура("ЭлектронныйДокумент", ДанныеСтроки.ЭлектронныйДокумент);
			Основания = ВсеОснования.НайтиСтроки(ОтборОснований);
	...
	Если ВыводитьКоличествоНовыхСообщений Тогда
		УстановитьКоличествоНепрочитанныхСообщений(Строки, Строки.ПолучитьКлючи());
	КонецЕсли;
...
Процедура ОбновитьОтображениеФильтров()
	...
	Если ЭтоМобильныйКлиент Тогда
		МК_ОформитьКнопкуПоказатьСкрытьФильтры();
	КонецЕсли;
...
Процедура ОбновитьФильтры()
	...
	ДоступныеПоляФильтров = ПоляФильтров();
	Для Каждого Фильтр Из ФильтрыСписка Цикл
		Если ДоступныеПоляФильтров.Найти(Фильтр.Ключ) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Процедура УстановитьФильтрыСписков()
	...
	ДоступныеПоляФильтров = ПоляФильтров();
	ВКэшеЕстьНедоступныеФильтры = Ложь;
	ИсправленныйКэшФильтров = Новый Соответствие;
	
	Для Каждого Раздел Из ИменаСписковФормы() Цикл
		
		НедоступныеФильтры = Новый Массив;
		...
		Для Каждого Фильтр Из Фильтры Цикл
			Если ДоступныеПоляФильтров.Найти(Фильтр.Ключ) = Неопределено Тогда
				ВКэшеЕстьНедоступныеФильтры = Истина;
				НедоступныеФильтры.Добавить(Фильтр.Ключ);
				Продолжить;
			КонецЕсли;
		...
		Для Каждого НедоступныйФильтр Из НедоступныеФильтры Цикл
			Фильтры.Удалить(НедоступныйФильтр);
		КонецЦикла;
		
		ИсправленныйКэшФильтров.Вставить(Раздел, Фильтры);
		
	КонецЦикла;
	
	Если ВКэшеЕстьНедоступныеФильтры Тогда
		КэшФильтровВСписках = Новый ФиксированноеСоответствие(ИсправленныйКэшФильтров);
	КонецЕсли;
...
Процедура ЗаполнитьКэшФильтровВСписках()
	...
	КэшФильтровВВХранилищеНастроек = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(ИмяФормы, "Отборы",
		КэшФильтровВСписках);
	Если ТипЗнч(КэшФильтровВВХранилищеНастроек) = Тип("ФиксированноеСоответствие") Тогда
		КэшФильтровВСписках = КэшФильтровВВХранилищеНастроек;
	Иначе
		КэшФильтровВСписках = Новый ФиксированноеСоответствие(КэшФильтровВВХранилищеНастроек);
	КонецЕсли;
...
&НаСервере
Процедура ИзменитьВидимостьКолонкиВУчете()
	ОтображатьКУчету = ПолучитьФункциональнуюОпцию("ИспользоватьКонтрольОтраженияВУчетеЭДО");
	Элементы.ВходящиеЭлектронныеДокументыОтражениеВУчете.Видимость = ОтображатьКУчету;
	Элементы.ИсходящиеЭлектронныеДокументыОтражениеВУчете.Видимость = ОтображатьКУчету;
КонецПроцедуры

&НаСервере
Процедура ИзменитьВидимостьКолонкиКоличестваСообщений()
	ВыводитьКолонкиСообщений = ИнтерфейсДокументовЭДО.ВыводитьЧатУниверсальныхСообщений();
	Элементы.ВходящиеЭлектронныеДокументыКоличествоНепрочитанныхСообщений.Видимость = ВыводитьКолонкиСообщений;
	Элементы.ИсходящиеЭлектронныеДокументыКоличествоНепрочитанныхСообщений.Видимость = ВыводитьКолонкиСообщений;
КонецПроцедуры
...
Процедура ОбновитьЗаголовкиЭлементов()
	...
	Если ОтложеннаяОтправкаЭлектронныхДокументов Тогда
		Элементы.КомандаПодписатьОтправить.Заголовок = НСтр("ru = 'Создать и подписать'");
	Иначе
		Элементы.КомандаПодписатьОтправить.Заголовок = НСтр("ru = 'Создать, подписать и отправить'");
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьТекущимРазделомВходящиеЭлектронныеДокументы()
	...
	УстановитьСтраницуОжиданияПредпросмотра("Ошибки");
	УстановитьПараметрыПредпросмотра();
	ОбновитьФильтры();
	ВидимостьЭлементовИКоманд();
	ПодключитьОбработчикОжидания("ОбновитьОбластьПросмотра", 0.1, Истина);
...
&НаКлиенте
Процедура УстановитьТекущимЭлементомСписокТекущегоРаздела()

	ТекущийЭлемент = Элементы[ТекущийРаздел];
...
Процедура ПриИзмененииНастроек()
	...
	Для Каждого ИмяСписка Из ИменаСписковФормы() Цикл
		Если ИмяСписка = "Сформировать" Или ИмяСписка = "Приглашения" Или ИмяСписка = "Ошибки" Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОбновитьОбластьПросмотра()
	...
	Если ТекущиеДанные = Неопределено Или Не ОтображатьОбластьПредпросмотра
		...
		Если ЗначениеЗаполнено(ОписаниеЗамера) Тогда
			ОбщегоНазначенияБЭДКлиент.ЗакончитьЗамер(ОписаниеЗамера);
...
Процедура ЗаполнитьРеквизитыФормыНаОснованииТекущихДанных(ТекущиеДанные)
	...
	ТекущийДокументЦентраЭДО = ТекущиеДанные.ИспользоватьЦентрЭДО;
	...
	ИдентификаторОрганизацииТекущегоДокумента = ТекущиеДанные.ИдентификаторОрганизации;
	ИдентификаторТекущегоДокумента = ТекущиеДанные.ИдентификаторДокумента;
	КонтрагентТекущегоДокумента = ТекущиеДанные.Контрагент;
...
Функция СвойстваТекущегоДокументаДляПолученияДанныхКомпонентовПросмотра()
	...
	СвойстваДокумента.ИспользоватьЦентрЭДО = ТекущийДокументЦентраЭДО;
...
Процедура НастроитьПанельВизуализации(ДанныеКомпонента)
	...
	Если ДанныеКомпонента.ТипДанных = 
		...
		Если ДанныеКомпонента.ФайлМожноПроштамповать Тогда
			...
			ОткрытьФайлHTML = ТекстДляПоляОткрытьФайлHTML(ТекущийФайлПредставление);
		Иначе
			Элементы.СтраницыПредпросмотраФайла.ТекущаяСтраница = Элементы.СтраницаФайлБезВизуализации;
			ОткрытьФайлHTML = ТекстДляПоляОткрытьФайлHTML(ТекущийФайлПредставление, Истина);
		КонецЕсли;
...
#Область ПредставлениеДанныхВHTML

&НаКлиентеНаСервереБезКонтекста
Функция ШаблонТекстаДляПоляРеквизитыДокументаИОтражениеВУчетеHTML()
	...
	Возврат 
		...
		|		}
		|	a:link {
		|		color: #1c55ae; text-decoration: none;}
		|	a:visited {
		|		color: #1c55ae; text-decoration: none;}
		|	a:hover {
		|		color: #1c55ae; text-decoration: underline;}
		|	.container {
		|		display:        inline-grid;   /* Устанавливаем контейнер как grid-контейнер */
		|		grid-auto-flow: column;        /* Элементы располагаются в одну строку */
		|		}
		|	.block {
		...
		|<body>
		|<div class=""container"">
		|" + ШаблонТекстаДляУстановкиСодержимогоКонтейнера() + "
		|</div>
		|</body>
		|</html>";
...
&НаКлиентеНаСервереБезКонтекста
Функция ШаблонТекстаДляУстановкиСодержимогоКонтейнера()
	Возврат "%СодержимоеКонтейнера";
КонецФункции

#Область ОбщиеМетоды

&НаКлиентеНаСервереБезКонтекста
Процедура ДобавитьПанельВПолеHTML(ТекстHTML, ИдентификаторПанели, ШаблонТекстаПанели,
	ДобавлятьВКонец = Ложь)
	
	Если ЗначениеЗаполнено(ТекстHTML) И СтрНайти(ТекстHTML, ИдентификаторПанели) Тогда
		ЗаменитьБлочныйЭлементВHTMLПоИдентификаторуЭлемента(ТекстHTML, ИдентификаторПанели, ШаблонТекстаПанели);
	ИначеЕсли ЗначениеЗаполнено(ТекстHTML) Тогда
		ДобавитьБлочныйЭлементВКонтейнерHTML(ТекстHTML, ШаблонТекстаПанели, ДобавлятьВКонец);
	Иначе
		ТекстHTML = ШаблонТекстаДляПоляРеквизитыДокументаИОтражениеВУчетеHTML();
		ТекстHTML = СтрЗаменить(ТекстHTML, ШаблонТекстаДляУстановкиСодержимогоКонтейнера(), ШаблонТекстаПанели);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ЗаменитьБлочныйЭлементВHTMLПоИдентификаторуЭлемента(ТекстHTML, ИдентификаторЭлемента, СтрокаЗамены)
	
	ПреобразованныйИдентификатор = СтрШаблон("<div id=%1", ИдентификаторЭлемента);
	НаборСтрок = СтрРазделить(ТекстHTML, Символы.ПС);
	
	Для Индекс = 0 По НаборСтрок.ВГраница() Цикл
		Если СтрНайти(НаборСтрок[Индекс], ПреобразованныйИдентификатор) = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		НаборСтрок[Индекс] = СтрокаЗамены;
	КонецЦикла;
	
	ТекстHTML = СтрСоединить(НаборСтрок, Символы.ПС);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ДобавитьБлочныйЭлементВКонтейнерHTML(ТекстHTML, ТекстЭлемента, ДобавлятьВКонец = Ложь)
	
	Если ДобавлятьВКонец Тогда
		КонецКонтейнера = "</div>
		|</body>";
		ТекстHTML = СтрЗаменить(ТекстHTML, КонецКонтейнера, ТекстЭлемента + Символы.ПС + КонецКонтейнера);
	Иначе
		НачалоКонтейнера = "<div class=""container"">";
		ТекстHTML = СтрЗаменить(ТекстHTML, НачалоКонтейнера, НачалоКонтейнера + Символы.ПС + ТекстЭлемента);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура УдалитьПанельИзПоляHTML(ТекстHTML, ИдентификаторПанели)
	
	НаборСтрок = СтрРазделить(ТекстHTML, Символы.ПС);
	
	ИндексПоследнейСтроки = НаборСтрок.ВГраница();
	Для Индекс = - ИндексПоследнейСтроки По 0 Цикл
		ТекущееЗначение = НаборСтрок[ - Индекс];
		Если СтрНайти(ТекущееЗначение, ИдентификаторПанели) = 0 Тогда
			Продолжить;
		КонецЕсли;
		НаборСтрок.Удалить(- Индекс);
	КонецЦикла;
	
	ТекстHTML = СтрСоединить(НаборСтрок, Символы.ПС);
	
	УдалитьОтступПанелиИзПоляHTML(ТекстHTML, ИдентификаторПанели);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура УдалитьОтступПанелиИзПоляHTML(ТекстHTML, ИдентификаторПанели)
	
	ИдентификаторОтступа = ИдентификаторОтступаМеждуПанелями(ИдентификаторПанели);
	...
	Для Индекс = - ИндексПоследнейСтроки По 0 Цикл
		ТекущееЗначение = НаборСтрок[ - Индекс];
		Если СтрНайти(ТекущееЗначение, ИдентификаторОтступа) = 0 Тогда
			Продолжить;
		КонецЕсли;
		НаборСтрок.Удалить(- Индекс);
	КонецЦикла;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ДобавитьОтступПослеПанелиВПолеHTML(ТекстHTML, ИдентификаторПанели)
	
	Если СтрНайти(ТекстHTML, ИдентификаторОтступаМеждуПанелями(ИдентификаторПанели)) <> 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для Индекс = 0 По НаборСтрок.ВГраница() Цикл
		Если СтрНайти(НаборСтрок[Индекс], ИдентификаторПанели) <> 0 Тогда
			НаборСтрок.Вставить(Индекс + 1, ШаблонТекстаОтступМеждуПанелями(ИдентификаторПанели));
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиентеНаСервереБезКонтекста
Функция ШаблонТекстаОтступМеждуПанелями(ИдентификаторПанели)
	Возврат "<div id=" + ИдентификаторОтступаМеждуПанелями(ИдентификаторПанели)
		+ "><span style=""color: #808080;""> | </span></div>";
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ИдентификаторОтступаМеждуПанелями(ИдентификаторПанели)
	Возврат "ОтступМеждуПанелями" + ИдентификаторПанели;
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ТекстСсылкиДляПоляHTML(Ссылка, ПредставлениеСсылки)
	Возврат СтрШаблон("<A href=%1>%2</A>", Ссылка, ПредставлениеСсылки);
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ТекстВБлочномЭлементеСОтступомДляПоляHTML(Текст)
	Возврат "<div style=""margin-right: 5px;"">" + Текст + "</div>";
КонецФункции
...
Процедура НастроитьПанельИтогов(ДанныеКомпонента)
	...
	ИдентификаторПанелиИтогов = ИдентификаторПанелиИтогов();
	
	ДобавитьПанельВПолеHTML(РеквизитыДокументаИОтражениеВУчетеHTML,
		ИдентификаторПанелиИтогов, ШаблонТекстаПанелиИтогов());
	
	Если ДанныеКомпонента.АсинхронноеПолучение Тогда
		УдалитьПанельИзПоляHTML(РеквизитыДокументаИОтражениеВУчетеHTML, ИдентификаторПанелиИтогов);
		Возврат;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ПредставлениеДанныхКомпонента.СуммаДокументаСтрокой)
		И Не ЗначениеЗаполнено(ПредставлениеДанныхКомпонента.НДСДокументаСтрокой) Тогда
		УдалитьПанельИзПоляHTML(РеквизитыДокументаИОтражениеВУчетеHTML, ИдентификаторПанелиИтогов);
		Возврат;
	КонецЕсли;

	РеквизитыДокументаИОтражениеВУчетеHTML = СтрЗаменить(РеквизитыДокументаИОтражениеВУчетеHTML,
		"%СуммаДокумента", ПредставлениеДанныхКомпонента.СуммаДокументаСтрокой);
	РеквизитыДокументаИОтражениеВУчетеHTML = СтрЗаменить(РеквизитыДокументаИОтражениеВУчетеHTML,
		"%СуммаНДС", ПредставлениеДанныхКомпонента.НДСДокументаСтрокой);
		
	ДобавитьОтступПослеПанелиВПолеHTML(РеквизитыДокументаИОтражениеВУчетеHTML, ИдентификаторПанелиИтогов);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция ШаблонТекстаПанелиИтогов()
	
	Возврат "<div id=" + ИдентификаторПанелиИтогов()
		+ ">%СуммаДокумента</div><div><span style=""color: #808080;"">%СуммаНДС</span></div>";
		
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ИдентификаторПанелиИтогов()
	Возврат "ПанельИтогов";
КонецФункции
...
Процедура НастроитьПанельОтветственного(ДанныеКомпонента)
	...
	ИдентификаторПанелиОтветственного = ИдентификаторПанелиОтветственного();
	
	Если Не ДанныеКомпонента.ИспользуетсяРазделениеДокументовПоОтветственным Тогда
		УдалитьПанельИзПоляHTML(РеквизитыДокументаИОтражениеВУчетеHTML, ИдентификаторПанелиОтветственного);
		Возврат;
	КонецЕсли;
	
	ДобавитьПанельВПолеHTML(РеквизитыДокументаИОтражениеВУчетеHTML,
		ИдентификаторПанелиОтветственного, ШаблонТекстаПанелиОтветственного(), Истина);
	
	ЗаголовокОтветственный = ?(ЗначениеЗаполнено(ДанныеКомпонента.ОтветственныйПоДокументу),
		Строка(ДанныеКомпонента.ОтветственныйПоДокументу), НСтр("ru='Не указан'"));
	РеквизитыДокументаИОтражениеВУчетеHTML = СтрЗаменить(РеквизитыДокументаИОтражениеВУчетеHTML,
		"%Содержимое", ТекстСсылкиДляПоляHTML(ИдентификаторКомандыСменитьОтветственного(), ЗаголовокОтветственный));
		
	ДобавитьОтступПослеПанелиВПолеHTML(РеквизитыДокументаИОтражениеВУчетеHTML, ИдентификаторПанелиОтветственного);

КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция ШаблонТекстаПанелиОтветственного()
	
	Возврат "<div id=" + ИдентификаторПанелиОтветственного() + ">Ответственный: %Содержимое</div>";
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ИдентификаторПанелиОтветственного()
	Возврат "ПанельОтветственного";
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ИдентификаторКомандыСменитьОтветственного()
	Возврат "СменитьОтветственного";
КонецФункции
...
Процедура НастроитьПанельОтраженияВУчете(ДанныеКомпонента)
	...
	ИдентификаторПанелиОтраженияВУчете = ИдентификаторПанелиОтраженияВУчете();
	...
	ДобавитьПанельВПолеHTML(РеквизитыДокументаИОтражениеВУчетеHTML,
		ИдентификаторПанелиОтраженияВУчете, ШаблонТекстаПанелиОтраженияВУчете(), Истина);
	...
	Если (НетОбъектовУчета И Не ЭтоВходящийЭДО)
		Или (Не ЕстьПравоОбработкиДокументов И НетОбъектовУчета) Тогда
		УдалитьПанельИзПоляHTML(РеквизитыДокументаИОтражениеВУчетеHTML, ИдентификаторПанелиОтраженияВУчете);
		УдалитьОтступПанелиИзПоляHTML(РеквизитыДокументаИОтражениеВУчетеHTML, ИдентификаторПанелиОтветственного());
		Возврат;
	КонецЕсли;
	
	ЗаголовокПанели = ?(МногоОбъектовУчета, НСтр("ru = 'Документы учета:'"), НСтр("ru = 'Документ учета:'"));
		
	ТекстПанелиОтраженияВУчете = "";
	ЗаголовокПодобрать = НСтр("ru = 'Подобрать'");
	
	Если ЕстьПравоОбработкиДокументов И НетОбъектовУчета И ЕстьВозможностьОтраженияВУчетеТекущегоДокумента Тогда
		ТекстПанелиОтраженияВУчете = ТекстПанелиОтраженияВУчете
			+ ТекстСсылкиВПанелиОтраженияВУчете(ИдентификаторКомандыСоздатьДокументУчета(), НСтр("ru = 'Создать'"))
			+ ТекстВБлочномЭлементеСОтступомДляПоляHTML(НСтр("ru = 'или'"))
			+ ТекстСсылкиВПанелиОтраженияВУчете(ИдентификаторКомандыПодобратьДокументУчета1(), ЗаголовокПодобрать);
	КонецЕсли;

	Если ОдинОбъектУчета Или МногоОбъектовУчета Тогда
		ТекстПанелиОтраженияВУчете = ТекстПанелиОтраженияВУчете + ТекстСсылкиВПанелиОтраженияВУчете(
			"ОткрытьДокументУчета1", Строка(ОбъектыУчетаТекущегоДокумента[0].ОбъектУчета));
	КонецЕсли;

	Если ОдинОбъектУчета И ИспользуетсяДополнительныйДокумент И ЕстьПравоОбработкиДокументов Тогда
		...
		ТекстВторогоДокумента = ЗаголовокПодобрать;
		...
		ТекстПанелиОтраженияВУчете = ТекстПанелиОтраженияВУчете + ТекстСсылкиВПанелиОтраженияВУчете(
			ИдентификаторКомандыПодобратьДокументУчета2(), ТекстВторогоДокумента);

	КонецЕсли;

	Если Не ЕстьВозможностьОтраженияВУчетеТекущегоДокумента И Не МногоОбъектовУчета
		Или (ОдинОбъектУчета И Не ИспользуетсяДополнительныйДокумент) Тогда
		ЗаголовокКомандыПоказатьВсеДокументыУчета = ЗаголовокПодобрать;
	ИначеЕсли МногоОбъектовУчета Тогда
		ЗаголовокКомандыПоказатьВсеДокументыУчета =
			СтрШаблон(НСтр("ru = 'и еще (%1)'"), КоличествоОбъектовУчета - 1);
	Иначе
		ЗаголовокКомандыПоказатьВсеДокументыУчета = НСтр("ru = 'Все документы'");
	КонецЕсли;
	
	Если МногоОбъектовУчета
		Или Не ЕстьВозможностьОтраженияВУчетеТекущегоДокумента И НетОбъектовУчета
		Или ДанныеКомпонента.ЭтоПрикладнойВидДокумента
		Или ДанныеКомпонента.ЭтоВнутреннийВидДокумента И МногоОбъектовУчета Тогда
		ТекстПанелиОтраженияВУчете = ТекстПанелиОтраженияВУчете
			+ ТекстСсылкиВПанелиОтраженияВУчете(ИдентификаторКомандыПоказатьВсеДокументыУчета(),
				ЗаголовокКомандыПоказатьВсеДокументыУчета);
	КонецЕсли;
	...
	РеквизитыДокументаИОтражениеВУчетеHTML = СтрЗаменить(РеквизитыДокументаИОтражениеВУчетеHTML,	
		"%Содержимое", ТекстПанелиОтраженияВУчете);

КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция ШаблонТекстаПанелиОтраженияВУчете()
	
	Возврат "<div id=" + ИдентификаторПанелиОтраженияВУчете() + ">%Заголовок</div>%Содержимое"; 
		
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ИдентификаторПанелиОтраженияВУчете()
	Возврат "ПанельОтраженияВУчете";
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ТекстСсылкиВПанелиОтраженияВУчете(Ссылка, ПредставлениеСсылки)
	
	Возврат "<div>" + ТекстСсылкиДляПоляHTML(Ссылка, ПредставлениеСсылки) + "</div>";
		
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ИдентификаторКомандыСоздатьДокументУчета()
	Возврат "СоздатьДокументУчета";
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ИдентификаторКомандыПодобратьДокументУчета1()
	Возврат "ПодобратьДокументУчета1";
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ИдентификаторКомандыПодобратьДокументУчета2()
	Возврат "ПодобратьДокументУчета2";
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ИдентификаторКомандыОткрытьДокументУчета1()
	Возврат "ОткрытьДокументУчета1";
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ИдентификаторКомандыОткрытьДокументУчета2()
	Возврат "ОткрытьДокументУчета2";
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ИдентификаторКомандыПоказатьВсеДокументыУчета()
	Возврат "ПоказатьВсеДокументыУчета";
КонецФункции

#КонецОбласти // ПанельОтраженияВУчете

#Область ПанельВизуализаци

&НаКлиентеНаСервереБезКонтекста
Функция ТекстДляПоляОткрытьФайлHTML(ИмяФайла, РазмещатьИмяФайлаПоЦентру = Ложь)
	...
	Возврат 
		...
		|" + ?(РазмещатьИмяФайлаПоЦентру, "text-align: center", "") + "
		|		}
		|	a:link {
		|		color: #1c55ae; text-decoration: none;}
		|	a:visited {
		...
		|</style>
		|<body>
		|" + ТекстСсылкиДляПоляHTML("ОткрытьФайл", ИмяФайла) + "
		|</body>
		|</html>";
	
КонецФункции

#КонецОбласти // ПанельВизуализаци

#КонецОбласти // ПредставлениеДанныхВHTML
...
Процедура КоличествоДокументов()
	...
	Если ЭтоМобильныйКлиент Тогда
		...
		МК_ОбновитьЗаголовкиПереключателейСтраниц();
	...
	Если ТекущийРаздел = "ВходящиеЭлектронныеДокументы"
...
Процедура НастроитьВидимостьКомандТекущейСтроки(ТекущиеДанные)
	...
	Элементы.ГруппаКнопокУтвердить.Видимость = Ложь;
	...
	Элементы.ГруппаКнопокПредложитьАннулироватьВходящие.Видимость = Ложь;
	...
	Элементы.ГруппаКнопокПредложитьАннулироватьВходящиеКонтекстноеМеню.Видимость = Ложь;
	...
	Элементы.ГруппаКнопокПредложитьАннулироватьИсходящие.Видимость = Ложь;
	...
	Элементы.ГруппаКнопокПредложитьАннулироватьИсходящиеКонтекстноеМеню.Видимость = Ложь;
	...
	Элементы.ЗапроситьУточнениеВходящие.Видимость = Ложь;
	Элементы.ЗапроситьУточнение.Видимость = Ложь;
	...
	Элементы.КомандаПодписатьОтправитьСформировать.Видимость = Ложь;
	Элементы.ЗакрытьПринудительноСформировать.Видимость = Ложь;
	Элементы.СформироватьКонтекстноеМенюПодписатьОтправить.Видимость = Ложь;
	Элементы.СформироватьКонтекстноеМенюЗакрытьПринудительно.Видимость = Ложь;

	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Элементы.КомандаПодписатьОтправитьСформировать.Видимость = Истина;
	Элементы.ЗакрытьПринудительноСформировать.Видимость = Истина;
	Элементы.СформироватьКонтекстноеМенюПодписатьОтправить.Видимость = Истина;
	Элементы.СформироватьКонтекстноеМенюЗакрытьПринудительно.Видимость = Истина;
	...
	Если СостояниеЭДО = ПредопределенноеЗначение("Перечисление.СостоянияДокументовЭДО.ТребуетсяПодписание") 
		...
		Элементы.ГруппаКнопокПредложитьАннулироватьВходящие.Видимость = Истина;
		Элементы.ГруппаКнопокПредложитьАннулироватьВходящиеКонтекстноеМеню.Видимость = Истина;
		
		Элементы.ЗапроситьУточнениеВходящие.Видимость = ВыполненПереходНаУниверсальныеСообщения И Не ЭтоПакетДокументов И Не ЭтоВнутреннийДокумент;
		Элементы.ЗапроситьУточнение.Видимость = ВыполненПереходНаУниверсальныеСообщения И Не ЭтоПакетДокументов И Не ЭтоВнутреннийДокумент;
		Элементы.ЗапроситьУточнениеПакет.Видимость = ВыполненПереходНаУниверсальныеСообщения И ЭтоПакетДокументов И Не ЭтоВнутреннийДокумент;
		Элементы.ЗапроситьУточнениеВходящиеПакет.Видимость = ВыполненПереходНаУниверсальныеСообщения И ЭтоПакетДокументов И Не ЭтоВнутреннийДокумент;
		
	ИначеЕсли АннулированиеДоступно(ТипТекущегоДокумента, СостояниеЭДО) Тогда
		...
		Элементы.ГруппаКнопокПредложитьАннулироватьИсходящие.Видимость = Истина;
		Элементы.ГруппаКнопокПредложитьАннулироватьИсходящиеКонтекстноеМеню.Видимость = Истина;
	...
	Если СостояниеЭДО = 
		...
		Элементы.ГруппаКнопокУтвердить.Видимость = Истина;
	...
	Элементы.КомандаУтвердить.Видимость = Не ЭтоПакетДокументов;
	Элементы.КомандаУтвердитьВходящие.Видимость = Не ЭтоПакетДокументов;
	Элементы.КомандаУтвердитьИсходящие.Видимость = Не ЭтоПакетДокументов;
	Элементы.КомандаУтвердитьВходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов;
	Элементы.КомандаУтвердитьИсходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов;
		
	Элементы.КомандаПодписать.Видимость = Не ЭтоПакетДокументов;
	Элементы.КомандаПодписатьНедоступная.Видимость = Не ЭтоПакетДокументов;
	Элементы.КомандаПодписатьВходящие.Видимость = Не ЭтоПакетДокументов;
	Элементы.КомандаПодписатьИсходящие.Видимость = Не ЭтоПакетДокументов;
	Элементы.КомандаПодписатьВходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов;
	Элементы.КомандаПодписатьИсходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов;
		
	Элементы.КомандаОтказатьсяОтПодписанияУтвердить.Видимость = Не ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияПодписать.Видимость = Не ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияНедоступная.Видимость = Не ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияУтвердитьВходящие.Видимость = Не ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияПодписатьВходящие.Видимость = Не ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияУтвердитьИсходящие.Видимость = Не ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияПодписатьИсходящие.Видимость = Не ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияУтвердитьВходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияПодписатьВходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияУтвердитьИсходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияПодписатьИсходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	
	Элементы.КомандаОтклонитьДокументУтвердить.Видимость = Не ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументПодписать.Видимость = Не ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументНедоступная.Видимость = Не ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументУтвердитьВходящие.Видимость = Не ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументПодписатьВходящие.Видимость = Не ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументУтвердитьИсходящие.Видимость = Не ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументПодписатьИсходящие.Видимость = Не ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументУтвердитьВходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументПодписатьВходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументУтвердитьИсходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументПодписатьИсходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
		
	Элементы.ПринятьАннулирование.Видимость = Не ЭтоПакетДокументов;
	Элементы.КомандаАннулироватьВходящие.Видимость = Не ЭтоПакетДокументов;
	Элементы.КомандаАннулироватьИсходящие.Видимость = Не ЭтоПакетДокументов;
	Элементы.КомандаАннулироватьВходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов;
	Элементы.КомандаАннулироватьИсходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов;
		
	Элементы.ОтклонитьАннулирование.Видимость = Не ЭтоПакетДокументов;
	Элементы.ОтклонитьАннулированиеВходящие.Видимость = Не ЭтоПакетДокументов;
	Элементы.ОтклонитьАннулированиеИсходящие.Видимость = Не ЭтоПакетДокументов;
	Элементы.ОтклонитьАннулированиеВходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов;
	Элементы.ОтклонитьАннулированиеИсходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов;
	
	Элементы.Отправить.Видимость = Не ЭтоПакетДокументов;
	Элементы.ОтправитьВходящие.Видимость = Не ЭтоПакетДокументов;
	Элементы.ОтправитьИсходящие.Видимость = Не ЭтоПакетДокументов;
	Элементы.ОтправитьВходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов;
	Элементы.ОтправитьИсходящиеКонтекстноеМеню.Видимость = Не ЭтоПакетДокументов;
	
	Элементы.АннулироватьВходящие.Видимость = Не ЭтоПакетДокументов И Не ЭтоВнутреннийДокумент;
	Элементы.АннулироватьИсходящие.Видимость = Не ЭтоПакетДокументов И Не ЭтоВнутреннийДокумент;
	Элементы.Аннулировать.Видимость = Не ЭтоПакетДокументов И Не ЭтоВнутреннийДокумент;
	Элементы.ИсходящиеЭлектронныеДокументыКонтекстноеМенюАннулировать.Видимость = Не ЭтоПакетДокументов
		И Не ЭтоВнутреннийДокумент;
	
	Элементы.ЗакрытьПринудительноВходящие.Видимость = Не ЭтоПакетДокументов;
	Элементы.ЗакрытьПринудительноИсходящие.Видимость = Не ЭтоПакетДокументов;
	Элементы.ВходящиеЭлектронныеДокументыЗакрытьПринудительно.Видимость = Не ЭтоПакетДокументов;
	Элементы.ИсходящиеЭлектронныеДокументыКонтекстноеМенюЗакрытьПринудительно.Видимость = Не ЭтоПакетДокументов;
	
	Элементы.КомандаУтвердитьПакет.Видимость = ЭтоПакетДокументов;
	Элементы.КомандаУтвердитьВходящиеПакет.Видимость = ЭтоПакетДокументов;
	Элементы.КомандаУтвердитьИсходящиеПакет.Видимость = ЭтоПакетДокументов;
	Элементы.КомандаУтвердитьВходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов;
	Элементы.КомандаУтвердитьИсходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов;
		
	Элементы.КомандаПодписатьПакет.Видимость = ЭтоПакетДокументов;
	Элементы.КомандаПодписатьНедоступнаяПакет.Видимость = ЭтоПакетДокументов;
	Элементы.КомандаПодписатьВходящиеПакет.Видимость = ЭтоПакетДокументов;
	Элементы.КомандаПодписатьИсходящиеПакет.Видимость = ЭтоПакетДокументов;
	Элементы.КомандаПодписатьВходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов;
	Элементы.КомандаПодписатьИсходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов;
		
	Элементы.КомандаОтказатьсяОтПодписанияУтвердитьПакет.Видимость = ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияПодписатьПакет.Видимость = ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияНедоступнаяПакет.Видимость = ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияУтвердитьВходящиеПакет.Видимость = ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияПодписатьВходящиеПакет.Видимость = ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияУтвердитьИсходящиеПакет.Видимость = ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияПодписатьИсходящиеПакет.Видимость = ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияУтвердитьВходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияПодписатьВходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияУтвердитьИсходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтказатьсяОтПодписанияПодписатьИсходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов И ВыполненПереходНаУниверсальныеСообщения;
	
	Элементы.КомандаОтклонитьДокументУтвердитьПакет.Видимость = ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументПодписатьПакет.Видимость = ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументНедоступнаяПакет.Видимость = ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументУтвердитьВходящиеПакет.Видимость = ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументПодписатьВходящиеПакет.Видимость = ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументУтвердитьИсходящиеПакет.Видимость = ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументПодписатьИсходящиеПакет.Видимость = ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументУтвердитьВходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументПодписатьВходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументУтвердитьИсходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
	Элементы.КомандаОтклонитьДокументПодписатьИсходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов И Не ВыполненПереходНаУниверсальныеСообщения;
		
	Элементы.ПринятьАннулированиеПакет.Видимость = ЭтоПакетДокументов;
	Элементы.КомандаАннулироватьВходящиеПакет.Видимость = ЭтоПакетДокументов;
	Элементы.КомандаАннулироватьИсходящиеПакет.Видимость = ЭтоПакетДокументов;
	Элементы.КомандаАннулироватьВходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов;
	Элементы.КомандаАннулироватьИсходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов;
		
	Элементы.ОтклонитьАннулированиеПакет.Видимость = ЭтоПакетДокументов;
	Элементы.ОтклонитьАннулированиеВходящиеПакет.Видимость = ЭтоПакетДокументов;
	Элементы.ОтклонитьАннулированиеИсходящиеПакет.Видимость = ЭтоПакетДокументов;
	Элементы.ОтклонитьАннулированиеВходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов;
	Элементы.ОтклонитьАннулированиеИсходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов;
	
	Элементы.ОтправитьПакет.Видимость = ЭтоПакетДокументов;
	Элементы.ОтправитьВходящиеПакет.Видимость = ЭтоПакетДокументов;
	Элементы.ОтправитьИсходящиеПакет.Видимость = ЭтоПакетДокументов;
	Элементы.ОтправитьВходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов;
	Элементы.ОтправитьИсходящиеКонтекстноеМенюПакет.Видимость = ЭтоПакетДокументов;
	
	Элементы.АннулироватьВходящиеПакет.Видимость = ЭтоПакетДокументов И Не ЭтоВнутреннийДокумент;
	Элементы.АннулироватьИсходящиеПакет.Видимость = ЭтоПакетДокументов И Не ЭтоВнутреннийДокумент;
	Элементы.АннулироватьПакет.Видимость = ЭтоПакетДокументов И Не ЭтоВнутреннийДокумент;
	Элементы.ИсходящиеЭлектронныеДокументыКонтекстноеМенюАннулироватьПакет.Видимость = ЭтоПакетДокументов
		И Не ЭтоВнутреннийДокумент;
		
	Элементы.ЗакрытьПринудительноВходящиеПакет.Видимость = ЭтоПакетДокументов;
	Элементы.ЗакрытьПринудительноИсходящиеПакет.Видимость = ЭтоПакетДокументов;
	Элементы.ВходящиеЭлектронныеДокументыЗакрытьПринудительноПакет.Видимость = ЭтоПакетДокументов;
	Элементы.ИсходящиеЭлектронныеДокументыКонтекстноеМенюЗакрытьПринудительноПакет.Видимость = ЭтоПакетДокументов;
	
	Элементы.АннулироватьВходящиеВнутреннийДокумент.Видимость = ЭтоВнутреннийДокумент;
	Элементы.АннулироватьИсходящиеВнутреннийДокумент.Видимость = ЭтоВнутреннийДокумент;
	Элементы.АннулироватьВнутреннийДокумент.Видимость = ЭтоВнутреннийДокумент;
	Элементы.ИсходящиеЭлектронныеДокументыКонтекстноеМенюАннулироватьВнутреннийДокумент.Видимость =
		ЭтоВнутреннийДокумент;
...
Процедура ВидимостьЭлементовИКоманд()
	...
	ИначеЕсли ТекущийРаздел = "Сформировать" Тогда
		...
		ТекущиеДанные = Элементы.Сформировать.ТекущиеДанные;
...
&НаКлиентеНаСервереБезКонтекста
Функция АннулированиеДоступно(ТипДокумента, СостоянияДокумента)
	
	Если ТипДокумента <> ПредопределенноеЗначение("Перечисление.ТипыДокументовЭДО.МашиночитаемаяДоверенность")
		И СостоянияДокумента = ПредопределенноеЗначение("Перечисление.СостоянияДокументовЭДО.ТребуетсяУтверждение")
		Или СостоянияДокумента = 
...
Процедура ПоказатьСопоставлениеНоменклатуры()
	...
	НастройкиСопоставления = СопоставлениеНоменклатурыКонтрагентовКлиент.НовыеНастройкиСопоставленияНоменклатуры();
	...
	КонецЦикла;
	НоменклатураИКонтекст = ИнтерфейсДокументовЭДОВызовСервера.НоменклатураИКонтекстСопоставленияПоЭД(
		ЭлектронныеДокументы);
	НоменклатураДляСопоставления = НоменклатураИКонтекст.НоменклатураДляСопоставления;
	НастройкиСопоставления.Организация = НоменклатураИКонтекст.Организация;
	НастройкиСопоставления.ДоговорКонтрагента = НоменклатураИКонтекст.ДоговорКонтрагента;
...
Асинх Процедура ВыполнитьКомандаПодписать()	
	...
	Если Не ОтложеннаяОтправкаЭлектронныхДокументов Или ТекущийРаздел = "ВходящиеЭлектронныеДокументы" Тогда
		ДействияПоЭДООтправить = ПредопределенноеЗначение("Перечисление.ДействияПоЭДО.Отправить");
		ИнтерфейсДокументовЭДОКлиентСервер.ДобавитьДействие(НаборДействий, 
...
Функция СтатусыНеРаспакованныхТранспортныхСообщенийБЭД()
	...
	СтатусыТранспортныхКонтейнеров = Новый Массив;
	СтатусыТранспортныхКонтейнеров.Добавить(
		ПредопределенноеЗначение("Перечисление.СтатусыТранспортныхСообщенийБЭД.Неизвестный"));
	СтатусыТранспортныхКонтейнеров.Добавить(
		ПредопределенноеЗначение("Перечисление.СтатусыТранспортныхСообщенийБЭД.КРаспаковке"));
	Возврат СтатусыТранспортныхКонтейнеров;
...
&НаСервереБезКонтекста
Функция АннулированиеДоступноДляПакетов(ПакетыДокументов)
	...
	Для Каждого ИдентификаторПакета Из ПакетыДокументов Цикл
		СостоянияДокументовПакета = ПакетыДокументовЭДО.СостоянияДокументовПакета(ИдентификаторПакета);
		СостояниеПакета = ПакетыДокументовЭДО.СостояниеПакета(СостоянияДокументовПакета);
		Если Не АннулированиеДоступно(Неопределено, СостояниеПакета) Тогда
			Результат.Результат = Ложь;
			ТекстыОшибок.Добавить(СтрШаблон(
				НСтр("ru = 'Аннулирование пакета ""%1"" недоступно при состоянии ЭДО ""%2""'") + Символы.ПС,
				Строка(ИдентификаторПакета), Строка(СостояниеПакета)));
		КонецЕсли;
	КонецЦикла;
...
&НаСервереБезКонтекста
Функция АннулированиеДоступноДляДокументов(Документы)

	Результат = Новый Структура("Результат, ТекстОшибки", Истина, "");
	
	ТекстыОшибок = Новый Массив;
	
	СостоянияДокументовБЭД = ЭлектронныеДокументыЭДО.СостоянияДокументов(Документы);
	ВидыДокументов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(Документы, "ВидДокумента");
	ТипыДокументов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(
		ОбщегоНазначения.ВыгрузитьКолонку(ВидыДокументов, "Значение"), "ТипДокумента");
	
	Для Каждого СостояниеПоДокументу Из СостоянияДокументовБЭД Цикл
		Документ = СостояниеПоДокументу.Ключ;
		СостояниеДокумента = СостояниеПоДокументу.Значение;
		ВидДокумента = ВидыДокументов[Документ];
		ТипДокумента = ТипыДокументов[ВидДокумента];
		Если Не АннулированиеДоступно(ТипДокумента, СостояниеДокумента) Тогда
			Результат.Результат = Ложь;
			ТекстыОшибок.Добавить(СтрШаблон(
				НСтр("ru = 'Аннулирование документа ""%1"" недоступно при состоянии ЭДО ""%2""'") + Символы.ПС,
				Строка(Документ), Строка(СостояниеДокумента)));
		КонецЕсли;
	КонецЦикла;
...
&НаСервереБезКонтекста
Процедура УстановитьКоличествоНепрочитанныхСообщений(Строки, ЭлектронныеДокументы)
	
	Если Не ЗначениеЗаполнено(ЭлектронныеДокументы) Тогда
		Возврат;
	КонецЕсли;
	
	КоличествоНепрочитанныхСообщенийПоДокументам = РегистрыСведений.ТекстовыеСообщенияДокументовЭДО.КоличествоНепрочитанныхСообщенийПоДокументам(
		ЭлектронныеДокументы);

	Для Каждого ТекущаяСтрока Из Строки Цикл

		Данные = ТекущаяСтрока.Значение.Данные;
		Данные.КоличествоНепрочитанныхСообщений = КоличествоНепрочитанныхСообщенийПоДокументам[Данные.ЭлектронныйДокумент];
...
&НаКлиенте
Процедура СменитьОтветственного()
...
&НаКлиенте
Процедура ОткрытьФайлHTMLПриНажатии(ДанныеСобытия, СтандартнаяОбработка)
...
&НаКлиенте
Процедура Подключаемый_МК_АктивироватьСтраницуВходящие(Команда)
	...
	МК_АктивироватьТекущуюСтраницу(
		СведенияОбЭлементахФормы.СведенияОРазделах.СтраницаВходящиеЭлектронныеДокументы);
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_МК_АктивироватьСтраницуИсходящие(Команда)
	...
	МК_АктивироватьТекущуюСтраницу(
		СведенияОбЭлементахФормы.СведенияОРазделах.СтраницаИсходящиеЭлектронныеДокументы);

КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_МК_ПоказатьСкрытьФильтры(Команда)
	...
	Элементы[СведенияОбЭлементахФормы.ИмяГруппыФильтры].Видимость =
		Не Элементы[СведенияОбЭлементахФормы.ИмяГруппыФильтры].Видимость;
	
	КнопкаПоказатьСкрытьФильтры =
		Элементы[СведенияОбЭлементахФормы.ИмяКомандыПоказатьСкрытьФильтры];

	СведенияОбЭлементахФормы = ЭтотОбъект["МК_СведенияОбЭлементахФормы"];
	
	МК_ОформитьКнопкуПоказатьСкрытьФильтры(КнопкаПоказатьСкрытьФильтры);
	
КонецПроцедуры

&НаКлиенте
Процедура МК_ОформитьКнопкуПоказатьСкрытьФильтры(КнопкаПоказатьСкрытьФильтры = Неопределено)
	...
	Если КнопкаПоказатьСкрытьФильтры = Неопределено Тогда
		КнопкаПоказатьСкрытьФильтры =
			Элементы[СведенияОбЭлементахФормы.ИмяКомандыПоказатьСкрытьФильтры];
	КонецЕсли;
	
	ИнтерфейсМобильногоЭДОКлиентСервер.НастроитьКнопкуФильтры(ЭтотОбъект,
		КнопкаПоказатьСкрытьФильтры,
		СведенияОбЭлементахФормы.ИмяГруппыФильтры);
	
КонецПроцедуры

&НаКлиенте
Процедура МК_ОбновитьСпискиВходящихИИсходящихДокументов()
	
	Если Не ЭтоМобильныйКлиент Тогда
		Возврат;
	КонецЕсли;
	
	Элементы.ВходящиеЭлектронныеДокументы.Обновить();
	Элементы.ИсходящиеЭлектронныеДокументы.Обновить();

КонецПроцедуры

&НаКлиенте
Процедура МК_АктивироватьТекущуюСтраницу(СведенияОСтранице)

	ИнтерфейсМобильногоЭДОКлиентСервер.АктивироватьСтраницуФормыТекущиеДелаПоЭДОЛегкийИнтерфейс(
		ЭтотОбъект, СведенияОСтранице);
...
&НаСервере
Процедура МК_ОбновитьЗаголовкиПереключателейСтраниц()
	...
	МобильныйЭДО.ОбновитьЗаголовкиПереключателейСтраницФормыТекущиеДелаЭДОЛегкийИнтерфейс(
		ЭтотОбъект, СведенияОбЭлементахФормы);
