...
Процедура ОткрытьФормуИсправленияОшибок(Документ, ОписаниеОшибок)
	...
	ПараметрыПроблемПриОбработкеДокументов.ФормаВладелец = ЭтотОбъект;
...
Процедура ПомощникУстраненияОшибокПослеФормированияЭДНаСервере(Знач АдресРезультата, Знач ОбъектыКОтправке,
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СписокДокументовКОтправке, ОбъектыКОтправке);
	Для Каждого ДанныеОбъектаУчета Из ОбъектыУчета Цикл
		Если СформированныеДокументы.Найти(ДанныеОбъектаУчета.ЭлектронныйДокумент) = Неопределено
			И ДанныеОбъектаУчета.Актуальный
			И СписокДокументовКОтправке.Найти(ДанныеОбъектаУчета.ОбъектУчета) = Неопределено Тогда
			СписокДокументовКОтправке.Добавить(ДанныеОбъектаУчета.ОбъектУчета);
		КонецЕсли;
	КонецЦикла;
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("АдресСведенийОбОшибках", АдресРезультата);
	ПараметрыОткрытия.Вставить("СписокДокументовКОтправке", СписокДокументовКОтправке);
