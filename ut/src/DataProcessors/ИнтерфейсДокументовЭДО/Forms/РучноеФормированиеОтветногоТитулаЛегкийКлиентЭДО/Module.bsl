...
Процедура Подписать(Команда)
	...
	ИнтерфейсДокументовЭДОКлиент.ПодписатьДокументИзФормыРучногоФормированияОтветногоТитула(ЭтотОбъект, СтруктураОтвета);
...
Процедура ЗаполнитьДанныеФормы()
	...
	Заголовок = СтрШаблон(Нстр("ru = 'Подписание документа: %1'"), ЭлектронныйДокумент);
...
Процедура НастроитьФорму()
	...
	Если ЭтоУПД Или ЭтоУПД_5_02 Тогда
		...
		ПоказатьОтклонитьПодписание = Ложь;
		Элементы.СведенияОВыбытииМаркированныхТоваров.Видимость = 
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭлектронныйДокумент, "СодержитМаркируемыеТовары");
	...
	ИначеЕсли ЭтоАктСверкиВзаиморасчетов Тогда
		...
		Элементы.ГруппаСтраницаУКД.Видимость = Ложь;
		ПоказатьОтклонитьПодписание = Истина;
	...
	ОтветНаЗакрытыйПакет = Ложь;
	
	Если ЗначениеЗаполнено(ИдентификаторПакета) Тогда
		ОтветНаЗакрытыйПакет = 
			ПакетыДокументовЭДОПовтИспНаВызов.ЭтоЗакрытыйПакет(Строка(ИдентификаторПакета));
	КонецЕсли;
	
	Элементы.ПодписатьИОтправить.Видимость = Не ОтветНаЗакрытыйПакет;
	Элементы.Подписать.Видимость = ОтветНаЗакрытыйПакет;
	Элементы.Подписать.КнопкаПоУмолчанию = ОтветНаЗакрытыйПакет;
	Элементы.ОтклонитьПодписание.Видимость = ?(ОтветНаЗакрытыйПакет, Ложь, ПоказатьОтклонитьПодписание);
