...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СценарийОдинФайл = СценарийОдинФайл И Файлы.Количество() = 1;
...
Процедура ПриОткрытии(Отказ)
	...
	ПодключитьОбработчикОжидания("ПредупредитьОНедопустимыхФайлах", 0.1, Истина);
...
Процедура ФайлыПриАктивизацииСтроки(Элемент)
	...
	Если СценарийОдинФайл
		И СформированоПредставлениеФайла
...
Процедура СохранитьДанныеФайла()
	...
	Если Не СценарийОдинФайл ИЛИ Файлы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Функция КлючевыеПоляНастройкиЗаполнены()
	...
	Для Счетчик = 0 По Файлы.Количество() - 1 Цикл
		...
		Если Файлы[Счетчик].ТипДокумента <> Перечисления.ТипыДокументовЭДО.Прочее Тогда
...
Функция ЗагрузитьФайлыСервер()
	...
	Для Счетчик = 0 По КоличествоСтрок Цикл
		...
		Попытка
			РезультатФормирования = ЭлектронныеДокументыЭДОСлужебный.СоздатьДокументПоФайлу(ПараметрыСоздания,
				ОписаниеФайла);
		Исключение
...
Процедура ДанныеФайлаПоФормализованномуДокументу(ИдентификаторСтроки)
	...
	Если Не ЭлектронныеДокументыЭДО.ТипДокументаПоддерживаетЗагрузкуИзФайлаВФормализованномВиде(
		Содержание.ТипДокумента, Содержание.ТипРегламента) Тогда
		Возврат;
	КонецЕсли;
	ОписаниеОшибки = "";
...
Процедура ДобавитьФайлы(ОписаниеПомещенныхФайлов)
	...
	ДанныеПредупреждения = РаботаСФайламиБЭДКлиентСервер.ИсключитьНедопустимыеФайлы(ОписаниеПомещенныхФайлов);
...
Процедура ДобавитьФайлЗавершить(ОписаниеПомещенныхФайлов, ДополнительныеПараметры) Экспорт
	...
	ПредупредитьОНедопустимыхФайлах();
	ПроверитьКорректностьИмениФайла();
	ЗаполнитьСписокВыборкаПолучетелей();
	УстановитьВидимостьИДоступность(ЭтотОбъект);
...
&НаКлиенте
Процедура ПредупредитьОНедопустимыхФайлах()
	
	РаботаСФайламиБЭДКлиент.ПоказатьПредупреждениеОНедопустимыхФайлах(ДанныеПредупреждения, Истина);
	ДанныеПредупреждения = Неопределено;
