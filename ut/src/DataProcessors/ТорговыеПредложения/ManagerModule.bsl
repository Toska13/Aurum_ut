...
//       ** ПоляГруппировкиТоваровДляЗаказа - Строка - имена колонок таблицы "товары", участвующих в группировке при разделении по заказам
...
Процедура СформироватьЗаказы(Знач ПараметрыПроцедуры, Знач АдресРезультата) Экспорт
	...
	Если ПараметрыПроцедуры.ДополнительныеПараметры.Свойство("ПоляГруппировкиТоваровДляЗаказа") Тогда
		...
		КонецЦикла;
	Иначе
		ПоляГруппировкиТоваровДляЗаказа = "ИдентификаторКонтрагента";
	КонецЕсли;
	...
	Для каждого СтрокаКонтрагента Из Контрагенты Цикл
		...
		Отбор = Новый Структура(ПоляГруппировкиТоваровДляЗаказа);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("СформироватьКарточкуПубликуемогоТорговогоПредложения", Истина);
	Методы.Вставить("ПолучитьДанныеДиагностики", Истина);
	Методы.Вставить("ОбновитьСтатистикуСостояний", Истина);
	Методы.Вставить("СформироватьЗаказы", Истина);
	Методы.Вставить("ОтправитьЗаказы", Истина);
КонецПроцедуры
...
Процедура ПроверитьСопоставлениеНоменклатурыКонтрагентов(Контрагент, ТоварыКонтрагента)
	...
	Для каждого СтрокаЗаказа Из ТоварыКонтрагента Цикл
		...
		Если СоответствиеНайдено = Ложь
			...
			ДополнительныеПараметры.ИспользоватьХарактеристики = СтрокаЗаказа.ИспользованиеХарактеристик;
			Отказ = Ложь;
			СопоставлениеНоменклатурыКонтрагентов.СоздатьОбновитьНоменклатуруКонтрагента(НоменклатураКонтрагента,
