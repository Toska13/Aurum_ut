...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Состояние <> Перечисления.СостоянияМиграцииПриложения.Выполняется Тогда
		...
		Наименование = СтрШаблон(НСтр("ru = '%1 (Миграция приложения)'"), ВидыПриложенийСервер.ПредставлениеТекущегоВидаПриложения());
...
Функция ЗапуститьМонопольноеЗавершение()
	...
	Если ЗапущенныеЗадания.Количество() = 0 Тогда
		...
		ФоновоеЗадание = ФоновыеЗадания.Выполнить("МиграцияПриложений.ЗаданиеВыгрузка",
			ПараметрыЗадания, "МиграцияПриложенийВыгрузка");
