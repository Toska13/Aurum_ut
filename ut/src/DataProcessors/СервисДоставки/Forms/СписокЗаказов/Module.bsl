...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ОбновитьСписокЗаказовНаДоставку" Тогда
		...
		Если ЗначениеЗаполнено(Параметр) И Параметр = ТипГрузоперевозки Тогда
			...
			ОчиститьСообщения = Истина;
			Если Источник = Ложь Тогда
				ОчиститьСообщения = Ложь;
			КонецЕсли;
			ПолучитьЗаказыНаДоставку( , "ОбновитьСписокЗаказовНаДоставку", ОчиститьСообщения);
		КонецЕсли;
...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	КонецЕсли;
	Элементы.ОбщаяКомандаСкопироватьСервисДоставки.Доступность = ТекущиеДанныеОпределены И Не ТекущиеДанные.ЭтоМультизаказ;
...
Процедура СписокЗаказовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "СписокДокументПеревозчикаПредставление" И ТекущиеДанные.ЕстьДокументыПеревозчика Тогда

		ОткрытьФорму("Обработка.СервисДоставки.Форма.СписокПроизвольныхОбъектов",
			Новый Структура("МассивОбъектов, ЗаголовокФормы", 
			ТекущиеДанные.ДокументыПеревозчика.ВыгрузитьЗначения(),
			НСтр("ru = 'Документы перевозчика по заказу'")), , , , , , 
			РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура Печать(Команда)
	...
	Для Каждого ИдентификаторСтроки Из Элементы.Список.ВыделенныеСтроки Цикл
		...
		КоличествоГрузовыхМест = ТекущаяСтрока.КоличествоГрузовыхМест;
	КонецЦикла;
	...
	ПараметрыОткрытияФормы.Вставить("ОрганизацияБизнесСетиСсылка", ОрганизацияБизнесСетиСсылка);
	ПараметрыОткрытияФормы.Вставить("ТипГрузоперевозки", ТипГрузоперевозки);
	Если СервисДоставкиКлиентСервер.ЭтоДеловыеЛинии(ТипГрузоперевозки) И ИдентификаторыЗаказов.Количество() = 1 Тогда
		ПараметрыОткрытияФормы.Вставить("ПараметрыЗаказа", 
			Новый Структура("ИдентификаторЗаказа, КоличествоГрузовыхМест", 
			ИдентификаторыЗаказов[0].Значение, КоличествоГрузовыхМест));
	КонецЕсли;
...
Процедура СоздатьПоШаблону(Команда)
	...
	ПараметрыФормы.Вставить("ТипГрузоперевозки", ТипГрузоперевозки);
	ПараметрыФормы.Вставить("ВариантОтображения", "Шаблоны");
	ПараметрыФормы.Вставить("ДействиеСШаблоном", "СоздатьЗаказ");
	ПараметрыФормы.Вставить("ОрганизацияБизнесСети", ОрганизацияБизнесСетиСсылка);
...
&НаКлиенте
Процедура ПолучитьЗаказыНаДоставку(ВыводитьОкноОжидания = Ложь, Ключ = "", ОчиститьСообщения = Истина)
...
Функция ПараметрыЗапроса(ПараметрыОперации, Отказ)
	...
	Для Каждого Операция Из ПараметрыОперации.Очередь Цикл
		...
		ИначеЕсли ИмяПроцедуры = СервисДоставкиКлиентСервер.ИмяМетодаСинхронизироватьДанныеРегистровПоЧастичнойДоставке() Тогда
			 Операция.Значение = СервисДоставки.НовыйПараметрыЗапросаСинхронизироватьДанныеРегистровПоЧастичнойДоставке();
		КонецЕсли;
...
Функция ЗагрузитьРезультатПолученияЗаказов(Результат, ОперацияВыполнена = Истина)
	...
	Если ЗначениеЗаполнено(Результат) И ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если Результат.Свойство("Уведомление") И Результат.Уведомление <> "" Тогда
			Элементы.ДекорацияУведомление.Заголовок = Результат.Уведомление;
...
Процедура НастроитьФормуПоТипуГрузоперевозки()
	...
	Элементы.Шаблоны.Видимость = ЭтоСДЭК Или ЭтоДеловыеЛинии;
	Элементы.СоздатьПоШаблону.Видимость = ЭтоСДЭК Или ЭтоДеловыеЛинии;
	Элементы.СписокДокументПеревозчикаПредставление.Видимость = ЭтоДеловыеЛинии;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент = УсловноеОформление.Элементы.Добавить();
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.СписокДокументПеревозчикаПредставление.Имя);
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Список.ЕстьДокументыПеревозчика");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветГиперссылкиБЭД);
	Элемент.Оформление.УстановитьЗначениеПараметра("Шрифт", ШрифтыСтиля.ПодчеркнутыйТекстСервисДоставки);
...
Процедура ОткрытьФормуЗаказа(ДанныеПоЗаказу)
	...
	ПараметрыОткрытияФормы.Вставить("ЭтоЗаполнениеКопированием", Ложь);
