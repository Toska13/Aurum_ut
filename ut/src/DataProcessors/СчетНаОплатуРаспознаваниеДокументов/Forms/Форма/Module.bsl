...
Процедура УправлениеФормой()
	...
	Элементы.КомандаСоздатьДокумент.Видимость = ПравоДоступа("Добавление", Метаданные.Документы[ТипДокумента]);
...
Процедура ЗагрузитьСвойстваЯчеекТаблицы() Экспорт
	...
	КлючеваяОперация = "Обработка.СчетНаОплатуРаспознаваниеДокументов.Форма.ЗагрузитьСвойстваЯчеекТаблицы";
	Замер = ОценкаПроизводительностиКлиент.ЗамерВремени(КлючеваяОперация);
...
Процедура НаправлениеПриИзмененииНаСервере()
	...
	Если Элементы.ГруппаПартнер.Видимость Тогда
		...
		Если СправочникиБезПравНаСоздание.НайтиПоЗначению("Партнер") <> Неопределено Тогда
...
Процедура НастроитьВидимостьПоФункциональнымОпциям()
	...
	ДоступноСозданиеПлатежныхПоручений = Истина;
	РаспознаваниеДокументовПереопределяемый.ДоступноСозданиеПлатежныхПоручений(ДоступноСозданиеПлатежныхПоручений);
...
Процедура ИзменитьИсточникиДляВыбораДоговора(ИмяПоля = "")
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСоглашенияСПоставщиками")  
		...
		Если ЗначениеЗаполнено(Соглашение) Тогда
			ИспользуютсяДоговорыКонтрагентов = ИспользуютсяДоговорыКонтрагентов И Соглашение.ИспользуютсяДоговорыКонтрагентов;
...
Процедура ОбновитьСписокВыбораБанковскихСчетов()
	...
	Для Каждого ИмяЭлемента Из ИменаЭлементов Цикл
		...
		Для Каждого ДанныеВыбора Из СписокДляЗагрузки Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Если СправочникиБезПравНаСоздание.НайтиПоЗначению(ИмяЭлемента) <> Неопределено Тогда
...
Процедура ИзменитьУсловияЗакупокПродаж(ИмяПоля = "");  
	...
	Если ЗначениеЗаполнено(Соглашение) Тогда 
		...
		Если Условия <> Неопределено Тогда     
			ИзмененСклад = ЗначениеЗаполнено(Условия.Склад) И Не Объект.РеквизитыДокумента[31].ЗаполненоВручную;
...
Процедура ПерепроверитьНаСервере()
	...
	СкладГруппа = Справочники.Склады.ЭтоГруппаИСкладыИспользуютсяВТЧДокументовПродажи(
		Объект.РеквизитыДокумента[31].Значение);
...
Процедура ОбновитьСтрокиНераспознаннойНоменклатуры()
	...
	КлючеваяОперация 
		= "РаспознаваниеДокументов.Форма.СчетНаОплатуРаспознаваниеДокументов.ОбновитьСтрокиНераспознаннойНоменклатуры";
