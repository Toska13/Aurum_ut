...
Процедура ПоляФормированияПредставленияПередНачаломИзменения(Элемент, Отказ)
	...
	Если Элемент.ТекущиеДанные.Значение = "Наименование" Тогда
		...
		Иначе
			Отказ = Отказ Или Элемент.ТекущиеДанные.Пометка;
...
Процедура ДействияПоОтбору(Команда)
	...
	Отказ = Ложь;
	Если Не ЗначениеЗаполнено(УчетнаяЗапись) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Поле ""Учетная запись"" не заполнено'"), ,
	...
	ПараметрыФормы.Вставить("РежимОткрытияОкна", Неопределено);
	...
	ОткрытьФорму(ФормаИмя, ПараметрыФормы, ЭтотОбъект, УникальныйИдентификатор, , , ОписаниеОповещения, ПараметрыФормы.РежимОткрытияОкна);
...
Процедура ВыгрузитьВExcel(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ВыгрузитьВExcelЗавершениеПодключенияРасширения", ЭтотОбъект);
...
Функция ПолучитьНовуюСтруктуруЭлементаСпискаЗначений()
	...
	СтруктураОтвета.Вставить("Значение", Неопределено);
	СтруктураОтвета.Вставить("Представление", "");
	СтруктураОтвета.Вставить("Пометка", Истина);
	СтруктураОтвета.Вставить("Картинка", Новый Картинка);
			...
			Если Параметры.Свойство("ИмяКоманды", ИмяКоманды) Тогда
				...
				ИначеЕсли ИмяКоманды = "УдалитьПоОтбору" Тогда
					...
					Исключение
						...
						ТекстИсключения = НСтр("ru = '%1'", КодОсновногоЯзыка);
						ТекстИсключения = СтрШаблон(ТекстИсключения, 
							НСтр("ru = 'УправлениеВыгрузкамиВБидзаар.УдалитьПоОтбору'", КодОсновногоЯзыка));
...
Процедура ПослеОтветаВыгрузкаДанных(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли РезультатВопроса = 2 И Не ЗначениеЗаполнено(Элементы.ДанныеВыгрузки.ВыделенныеСтроки) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Нет выделенных строк для выгрузки'"));
		Возврат;
	КонецЕсли;
...
Процедура ПослеПодбораТоваровУслуг(РезультатПодбора, ДополнительныеПараметры) Экспорт
	...
	ИнтеграцияСЭлектроннымиТорговымиПлощадкамиКлиентПереопределяемый.ПередОбработкойРезультатаПодбораТоваровУслуг(
		РезультатПодбора);
...
Процедура УдалитьВсеСтрокиНаСервере()
	...
	Исключение
		...
		ТекстИсключения = НСтр("ru = '%1'", КодОсновногоЯзыка);
		ТекстИсключения = СтрШаблон(ТекстИсключения, 
			НСтр("ru = 'УправлениеВыгрузкамиВБидзаар.ДанныеВыгрузкиПередУдалением'", КодОсновногоЯзыка));
...
// 
// Параметры:
//  РасширениеРаботыСФайламиПодключено - Булево - Результат выполнения подключения расширения
//  ДополнительныеПараметры - Структура - Дополнительные параметры
//
&НаКлиенте
Процедура ВыгрузитьВExcelЗавершениеПодключенияРасширения(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ВыгрузитьВExcelЗавершение()
...
// 
// Параметры:
//  Результат - Булево - Результат выполнения операции получения файлов
//  ДополнительныеПараметры - Структура - Дополнительные параметры
//
&НаКлиенте
...
Функция ПоместитьДанныеВExcelНаСервере(ВариантВыгрузки, Отказ)
	...
	Исключение
		...
		ТекстИсключения = НСтр("ru = '%1'", КодОсновногоЯзыка);
		ТекстИсключения = СтрШаблон(ТекстИсключения, 
			НСтр("ru = УправлениеВыгрузкамиВБидзаар.ВыгрузкаВExcel'", КодОсновногоЯзыка));
