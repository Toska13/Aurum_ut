...
Функция ТаблицаДокументовКВыгрузке(
	...
	Запрос.УстановитьПараметр("ПоддержкаПлатежейРФ", ПолучитьФункциональнуюОпцию("ПоддержкаПлатежейРФ"));
...
// Выводит сообщение об ошибке загрузки файла пользователю и записывает в журнал регистрации.
// 
// Параметры:
//  Счет - СправочникСсылка.БанковскиеСчетаОрганизаций - счет, по которому возникла ошибка.
//  ИмяФайла - Строка - наименование загружаемого файла.
//  ИнформацияОбОшибке - ИнформацияОбОшибке, Неопределено - сведения об ошибке.
//
Процедура ВывестиОшибкуЗагрузкиФайла(Счет, ИмяФайла, ИнформацияОбОшибке) Экспорт

	ШаблонТекста = НСтр("ru = 'Ошибка загрузки файла ""%1"" по счету %2:
								|%3'");
	ТекстОшибкиПодробно = СтрШаблон(
		ШаблонТекста, ИмяФайла, Счет, ?(ИнформацияОбОшибке <> Неопределено, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке), ""));
	Событие = СтрШаблон("%1 %2", НСтр("ru = 'Загрузка платежей из банка.'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	ТекстОшибкиКратко = СтрШаблон(
		ШаблонТекста, ИмяФайла, Счет, ?(ИнформацияОбОшибке <> Неопределено, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке), ""));
...
Функция ТекстЗапросаНастройкиСчетов()
	...
	|	ПО
	|		БанковскиеСчета.Владелец = Организации.Ссылка
	|	
	|ГДЕ
...
Функция ТекстЗапросаПлатежныеПорученияТретьиЛица() Экспорт
	...
	|	ВЫБОР
	|		КОГДА &ПоддержкаПлатежейРФ ТОГДА
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлассификаторБанков КАК БанкиОрганизаций
	|		ПО БанковскийСчетОрганизации.Банк = БанкиОрганизаций.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтраныМира КАК СтраныБанковОрганизаций
	|		ПО БанкиОрганизаций.КодСтраны = СтраныБанковОрганизаций.Код
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлассификаторБанков КАК БанкиДляРасчетовОрганизаций
	|		ПО БанковскийСчетОрганизации.БанкДляРасчетов = БанкиДляРасчетовОрганизаций.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтраныМира КАК СтраныБанковДляРасчетовОрганизаций
	|		ПО БанкиДляРасчетовОрганизаций.КодСтраны = СтраныБанковДляРасчетовОрганизаций.Код
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		Справочник.БанковскиеСчетаКонтрагентов КАК БанковскиеСчетаКонтрагентов
	|	ПО
	|		БанковскиеСчетаКонтрагентов.Ссылка = ДанныеДокумента.БанковскийСчетКонтрагента
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлассификаторБанков КАК БанкиКонтрагентов
	|		ПО БанковскиеСчетаКонтрагентов.Банк = БанкиКонтрагентов.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтраныМира КАК СтраныБанковКонтрагентов
	|		ПО БанкиКонтрагентов.КодСтраны = СтраныБанковКонтрагентов.Код
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлассификаторБанков КАК БанкиДляРасчетовКонтрагентов
	|		ПО БанковскиеСчетаКонтрагентов.БанкДляРасчетов = БанкиДляРасчетовКонтрагентов.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтраныМира КАК СтраныБанковДляРасчетовКонтрагентов
	|		ПО БанкиДляРасчетовКонтрагентов.КодСтраны = СтраныБанковДляРасчетовКонтрагентов.Код
	|	
	|ГДЕ
...
Функция ТекстЗапросаПлатежныеПорученияВнутренние() Экспорт
	...
	|	"""" КАК НомерВедомостиНаВыплатуЗарплаты,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаВедомостиНаВыплатуЗарплаты
	|	
	|ИЗ
	|	Документ.СписаниеБезналичныхДенежныхСредств КАК ДанныеДокумента
	|	
...
Функция ТекстДанныеБанковскогоСчетаОрганизации()
	...
	|		ИНАЧЕ СтраныБанковОрганизаций.Наименование КОНЕЦ КАК ОрганизацияСтранаБанка,
	|	ВЫБОР КОГДА БанковскийСчетОрганизации.РучноеИзменениеРеквизитовБанка ТОГДА БанковскийСчетОрганизации.СтранаБанка.Код
	|		ИНАЧЕ БанкиОрганизаций.КодСтраны КОНЕЦ КАК ОрганизацияСтранаБанкаКод,
	...
	|		ИНАЧЕ СтраныБанковОрганизаций.КодАльфа2 КОНЕЦ КАК ОрганизацияСтранаБанкаИСОКод,
	...
	|		ИНАЧЕ СтраныБанковДляРасчетовОрганизаций.Наименование КОНЕЦ КАК ОрганизацияСтранаБанкаДляРасчетов,
	...
	|		ИНАЧЕ БанкиДляРасчетовОрганизаций.КодСтраны КОНЕЦ КАК ОрганизацияСтранаБанкаДляРасчетовКод,
	...
	|		ИНАЧЕ СтраныБанковДляРасчетовОрганизаций.КодАльфа2 КОНЕЦ КАК ОрганизацияСтранаБанкаДляРасчетовИСОКод,
...
Функция ТекстДанныеБанковскогоСчетаКонтрагента()
	...
	|		ИНАЧЕ СтраныБанковКонтрагентов.Наименование КОНЕЦ КАК КонтрагентСтранаБанка,
	|	ВЫБОР КОГДА БанковскиеСчетаКонтрагентов.РучноеИзменениеРеквизитовБанка ТОГДА БанковскиеСчетаКонтрагентов.СтранаБанка.Код
	|		ИНАЧЕ БанкиКонтрагентов.КодСтраны КОНЕЦ КАК КонтрагентСтранаБанкаКод,
	...
	|		ИНАЧЕ СтраныБанковКонтрагентов.КодАльфа2 КОНЕЦ КАК КонтрагентСтранаБанкаИСОКод,
	...
	|		ИНАЧЕ СтраныБанковДляРасчетовКонтрагентов.Наименование КОНЕЦ КАК КонтрагентСтранаБанкаДляРасчетов,
	...
	|		ИНАЧЕ БанкиДляРасчетовКонтрагентов.КодСтраны КОНЕЦ КАК КонтрагентСтранаБанкаДляРасчетовКод,
	...
	|		ИНАЧЕ СтраныБанковДляРасчетовКонтрагентов.КодАльфа2 КОНЕЦ КАК КонтрагентСтранаБанкаДляРасчетовИСОКод,
...
Процедура ЗаполнитьЗначениеЭкспорта(СтрокаЭкспорта, ВыборкаДляЭкспорта)
	...
	РеквизитыСторон = РеквизитыСторонПлатежа();
	ЗаполнитьРеквизитыСторонПлатежа(РеквизитыСторон, ВыборкаДляЭкспорта);
	...
	СтрокаЭкспорта.ПлательщикСчет = РеквизитыСторон.ПлательщикРасчСчет;
	СтрокаЭкспорта.ПолучательСчет = РеквизитыСторон.ПолучательРасчСчет;
	СтрокаЭкспорта.ВидПлатежа     = ВыборкаДляЭкспорта.ВидПлатежа;
	
	НепрямыеРасчетыУПлательщика   = ЗначениеЗаполнено(РеквизитыСторон.ПлательщикБанкДляРасчетов);
	НепрямыеРасчетыУПолучателя    = ЗначениеЗаполнено(РеквизитыСторон.ПолучательБанкДляРасчетов);
	...
	// Плательщик1
	СтрокаЭкспорта.Плательщик1 = РеквизитыСторон.ПлательщикНаим;
	
	// Плательщик, ПлательщикИНН
	СтрокаЭкспорта.ПлательщикИНН = РеквизитыСторон.ПлательщикИНН;
	СтрокаЭкспорта.Плательщик = "ИНН " + РеквизитыСторон.ПлательщикИНН + " " + СтрокаЭкспорта.Плательщик1;
	...
	Если НепрямыеРасчетыУПлательщика Тогда
		СтрокаЭкспорта.Плательщик2        = РеквизитыСторон.ПлательщикРасчСчет;
		СтрокаЭкспорта.Плательщик3        = РеквизитыСторон.ПлательщикБанк;
		СтрокаЭкспорта.Плательщик4        = РеквизитыСторон.ПлательщикГородБанка;
		СтрокаЭкспорта.Плательщик         = ДенежныеСредстваСервер.НаименованиеПлательщикаПолучателяПлатежа(
			СтрокаЭкспорта.Плательщик, Не НепрямыеРасчетыУПлательщика, СтрокаЭкспорта.Плательщик2, СтрокаЭкспорта.Плательщик3, СтрокаЭкспорта.Плательщик4);
		Если ПустаяСтрока(РеквизитыСторон.ПлательщикКоррСчет) Тогда
			СтрокаЭкспорта.ПлательщикРасчСчет = РеквизитыСторон.ПлательщикСчетВБанкеДляРасчетов;
		Иначе
			СтрокаЭкспорта.ПлательщикРасчСчет = РеквизитыСторон.ПлательщикКоррСчет;
		КонецЕсли;
		СтрокаЭкспорта.ПлательщикБанк1    = РеквизитыСторон.ПлательщикБанкДляРасчетов;
		СтрокаЭкспорта.ПлательщикБанк2    = РеквизитыСторон.ПлательщикГородРЦБанка;
		СтрокаЭкспорта.ПлательщикБИК      = РеквизитыСторон.ПлательщикБИКРЦБанка;
		СтрокаЭкспорта.ПлательщикКорсчет  = РеквизитыСторон.ПлательщикКоррСчетРЦБанка;
	Иначе
		СтрокаЭкспорта.ПлательщикРасчСчет = РеквизитыСторон.ПлательщикРасчСчет;
		СтрокаЭкспорта.ПлательщикБанк1    = РеквизитыСторон.ПлательщикБанк;
		СтрокаЭкспорта.ПлательщикБанк2    = РеквизитыСторон.ПлательщикГородБанка;
		СтрокаЭкспорта.ПлательщикБИК      = РеквизитыСторон.ПлательщикБИКБанка;
		СтрокаЭкспорта.ПлательщикКорсчет  = РеквизитыСторон.ПлательщикКоррСчет;
	КонецЕсли;
	
	// Получатель1
	ПолучательПозицияСимволВК = СтрНайти(РеквизитыСторон.ПолучательНаим, Символы.ВК);
	Если ПолучательПозицияСимволВК = 0 Тогда
		СтрокаЭкспорта.Получатель1 = РеквизитыСторон.ПолучательНаим;
	КонецЕсли;
	
	// Получатель, ПолучательИНН
	СтрокаЭкспорта.ПолучательИНН = РеквизитыСторон.ПолучательИНН;
	СтрокаЭкспорта.Получатель = "ИНН " + РеквизитыСторон.ПолучательИНН + " " + СтрокаЭкспорта.Получатель1;
	...
	Если НепрямыеРасчетыУПолучателя Тогда
		СтрокаЭкспорта.Получатель2        = РеквизитыСторон.ПолучательРасчСчет;
		СтрокаЭкспорта.Получатель3        = РеквизитыСторон.ПолучательБанк;
		СтрокаЭкспорта.Получатель4        = РеквизитыСторон.ПолучательГородБанка;
		СтрокаЭкспорта.Получатель         = ДенежныеСредстваСервер.НаименованиеПлательщикаПолучателяПлатежа(
			СтрокаЭкспорта.Получатель, Не НепрямыеРасчетыУПолучателя, СтрокаЭкспорта.Получатель2, СтрокаЭкспорта.Получатель3, СтрокаЭкспорта.Получатель4);
		Если ПустаяСтрока(РеквизитыСторон.ПолучательКоррСчет) Тогда
			СтрокаЭкспорта.ПолучательРасчСчет = РеквизитыСторон.ПолучательСчетВБанкеДляРасчетов;
		Иначе
			СтрокаЭкспорта.ПолучательРасчСчет = РеквизитыСторон.ПолучательКоррСчет;
		КонецЕсли;
		СтрокаЭкспорта.ПолучательБанк1 = КлиентБанкСервер.НаименованиеБанкаПолучателяПлатежа(
			РеквизитыСторон.ПолучательБанкДляРасчетов, РеквизитыСторон.ПолучательБанкДляРасчетовНаименованиеРКЦ);
		СтрокаЭкспорта.ПолучательБанк2    = РеквизитыСторон.ПолучательГородРЦБанка;
		СтрокаЭкспорта.ПолучательБИК      = РеквизитыСторон.ПолучательБИКРЦБанка;
		СтрокаЭкспорта.ПолучательКорсчет  = РеквизитыСторон.ПолучательКоррСчетРЦБанка;
	Иначе
		СтрокаЭкспорта.ПолучательРасчСчет = РеквизитыСторон.ПолучательРасчСчет;
		СтрокаЭкспорта.ПолучательБанк1 = КлиентБанкСервер.НаименованиеБанкаПолучателяПлатежа(
			РеквизитыСторон.ПолучательБанк, РеквизитыСторон.ПолучательБанкНаименованиеРКЦ);
		СтрокаЭкспорта.ПолучательБанк2    = РеквизитыСторон.ПолучательГородБанка;
		СтрокаЭкспорта.ПолучательБИК      = РеквизитыСторон.ПолучательБИКБанка;
		СтрокаЭкспорта.ПолучательКорсчет  = РеквизитыСторон.ПолучательКоррСчет;
	КонецЕсли;
...
Процедура ЗаписатьВЖурналРегистрации(Комментарий, Уровень = Неопределено, ПараметрыЗаписи = Неопределено)
	...
	Если ПараметрыЗаписи = Неопределено Тогда
		...
		ПараметрыЗаписи.ОбъектМетаданных = Метаданные.Обработки.КлиентБанк;
	КонецЕсли;
...
Функция РеквизитыСторонПлатежа()

	Результат = Новый Структура;
	Результат.Вставить("ПлательщикНаим");
	Результат.Вставить("ПлательщикИНН");
	Результат.Вставить("ПлательщикРасчСчет");
	Результат.Вставить("ПлательщикБанк");
	Результат.Вставить("ПлательщикБИКБанка");
	Результат.Вставить("ПлательщикКоррСчет");
	Результат.Вставить("ПлательщикГородБанка");
	Результат.Вставить("ПлательщикБанкДляРасчетов");
	Результат.Вставить("ПлательщикГородРЦБанка");
	Результат.Вставить("ПлательщикБИКРЦБанка");
	Результат.Вставить("ПлательщикКоррСчетРЦБанка");
	Результат.Вставить("ПлательщикСчетВБанкеДляРасчетов");
	
	Результат.Вставить("ПолучательНаим");
	Результат.Вставить("ПолучательИНН");
	Результат.Вставить("ПолучательРасчСчет");
	Результат.Вставить("ПолучательБанк");
	Результат.Вставить("ПолучательБанкНаименованиеРКЦ");
	Результат.Вставить("ПолучательБИКБанка");
	Результат.Вставить("ПолучательКоррСчет");
	Результат.Вставить("ПолучательГородБанка");
	Результат.Вставить("ПолучательБанкДляРасчетов");
	Результат.Вставить("ПолучательБанкДляРасчетовНаименованиеРКЦ");
	Результат.Вставить("ПолучательГородРЦБанка");
	Результат.Вставить("ПолучательБИКРЦБанка");
	Результат.Вставить("ПолучательКоррСчетРЦБанка");
	Результат.Вставить("ПолучательСчетВБанкеДляРасчетов");
	
	Возврат Результат;

КонецФункции

Процедура ЗаполнитьРеквизитыСторонПлатежа(РеквизитыСторон, ДанныеЗаполнения)

	Если ДанныеЗаполнения.ВидДокумента = Перечисления.ТипыПлатежныхДокументов.ПлатежноеТребование Тогда
		
		РеквизитыСторон.ПлательщикНаим = ДанныеЗаполнения.КонтрагентНаим;
		РеквизитыСторон.ПлательщикИНН = ДанныеЗаполнения.КонтрагентИНН;
		РеквизитыСторон.ПлательщикРасчСчет = ДанныеЗаполнения.КонтрагентРасчСчет;
		РеквизитыСторон.ПлательщикБанк = ДанныеЗаполнения.КонтрагентБанк;
		РеквизитыСторон.ПлательщикБИКБанка = ДанныеЗаполнения.КонтрагентБИКБанка;
		РеквизитыСторон.ПлательщикКоррСчет = ДанныеЗаполнения.КонтрагентКоррСчет;
		РеквизитыСторон.ПлательщикГородБанка = ДанныеЗаполнения.КонтрагентГородБанка;
		РеквизитыСторон.ПлательщикБанкДляРасчетов = ДанныеЗаполнения.КонтрагентБанкДляРасчетов;
		РеквизитыСторон.ПлательщикГородРЦБанка = ДанныеЗаполнения.КонтрагентГородРЦБанка;
		РеквизитыСторон.ПлательщикБИКРЦБанка = ДанныеЗаполнения.КонтрагентБИКРЦБанка;
		РеквизитыСторон.ПлательщикКоррСчетРЦБанка = ДанныеЗаполнения.КонтрагентКоррСчетРЦБанка;
		РеквизитыСторон.ПлательщикСчетВБанкеДляРасчетов = ДанныеЗаполнения.КонтрагентСчетВБанкеДляРасчетов;
		
		РеквизитыСторон.ПолучательНаим = ДанныеЗаполнения.ОрганизацияНаим;
		РеквизитыСторон.ПолучательИНН = ДанныеЗаполнения.ОрганизацияИНН;
		РеквизитыСторон.ПолучательРасчСчет = ДанныеЗаполнения.ОрганизацияРасчСчет;
		РеквизитыСторон.ПолучательБанк = ДанныеЗаполнения.ОрганизацияБанк;
		РеквизитыСторон.ПолучательБанкНаименованиеРКЦ = ДанныеЗаполнения.ОрганизацияБанкНаименованиеРКЦ;
		РеквизитыСторон.ПолучательБИКБанка = ДанныеЗаполнения.ОрганизацияБИКБанка;
		РеквизитыСторон.ПолучательКоррСчет = ДанныеЗаполнения.ОрганизацияКоррСчет;
		РеквизитыСторон.ПолучательГородБанка = ДанныеЗаполнения.ОрганизацияГородБанка;
		РеквизитыСторон.ПолучательБанкДляРасчетов = ДанныеЗаполнения.ОрганизацияБанкДляРасчетов;
		РеквизитыСторон.ПолучательБанкДляРасчетовНаименованиеРКЦ = ДанныеЗаполнения.ОрганизацияБанкДляРасчетовНаименованиеРКЦ;
		РеквизитыСторон.ПолучательГородРЦБанка = ДанныеЗаполнения.ОрганизацияГородРЦБанка;
		РеквизитыСторон.ПолучательБИКРЦБанка = ДанныеЗаполнения.ОрганизацияБИКРЦБанка;
		РеквизитыСторон.ПолучательКоррСчетРЦБанка = ДанныеЗаполнения.ОрганизацияКоррСчетРЦБанка;
		РеквизитыСторон.ПолучательСчетВБанкеДляРасчетов = ДанныеЗаполнения.ОрганизацияСчетВБанкеДляРасчетов;
		
	Иначе
		
		РеквизитыСторон.ПлательщикНаим = ДанныеЗаполнения.ОрганизацияНаим;
		РеквизитыСторон.ПлательщикИНН = ДанныеЗаполнения.ОрганизацияИНН;
		РеквизитыСторон.ПлательщикРасчСчет = ДанныеЗаполнения.ОрганизацияРасчСчет;
		РеквизитыСторон.ПлательщикБанк = ДанныеЗаполнения.ОрганизацияБанк;
		РеквизитыСторон.ПлательщикБИКБанка = ДанныеЗаполнения.ОрганизацияБИКБанка;
		РеквизитыСторон.ПлательщикКоррСчет = ДанныеЗаполнения.ОрганизацияКоррСчет;
		РеквизитыСторон.ПлательщикГородБанка = ДанныеЗаполнения.ОрганизацияГородБанка;
		РеквизитыСторон.ПлательщикБанкДляРасчетов = ДанныеЗаполнения.ОрганизацияБанкДляРасчетов;
		РеквизитыСторон.ПлательщикГородРЦБанка = ДанныеЗаполнения.ОрганизацияГородРЦБанка;
		РеквизитыСторон.ПлательщикБИКРЦБанка = ДанныеЗаполнения.ОрганизацияБИКРЦБанка;
		РеквизитыСторон.ПлательщикКоррСчетРЦБанка = ДанныеЗаполнения.ОрганизацияКоррСчетРЦБанка;
		РеквизитыСторон.ПлательщикСчетВБанкеДляРасчетов = ДанныеЗаполнения.ОрганизацияСчетВБанкеДляРасчетов;
		
		РеквизитыСторон.ПолучательНаим = ДанныеЗаполнения.КонтрагентНаим;
		РеквизитыСторон.ПолучательИНН = ДанныеЗаполнения.КонтрагентИНН;
		РеквизитыСторон.ПолучательРасчСчет = ДанныеЗаполнения.КонтрагентРасчСчет;
		РеквизитыСторон.ПолучательБанк = ДанныеЗаполнения.КонтрагентБанк;
		РеквизитыСторон.ПолучательБанкНаименованиеРКЦ = ДанныеЗаполнения.КонтрагентБанкНаименованиеРКЦ;
		РеквизитыСторон.ПолучательБИКБанка = ДанныеЗаполнения.КонтрагентБИКБанка;
		РеквизитыСторон.ПолучательКоррСчет = ДанныеЗаполнения.КонтрагентКоррСчет;
		РеквизитыСторон.ПолучательГородБанка = ДанныеЗаполнения.КонтрагентГородБанка;
		РеквизитыСторон.ПолучательБанкДляРасчетов = ДанныеЗаполнения.КонтрагентБанкДляРасчетов;
		РеквизитыСторон.ПолучательБанкДляРасчетовНаименованиеРКЦ = ДанныеЗаполнения.КонтрагентБанкДляРасчетовНаименованиеРКЦ;
		РеквизитыСторон.ПолучательГородРЦБанка = ДанныеЗаполнения.КонтрагентГородРЦБанка;
		РеквизитыСторон.ПолучательБИКРЦБанка = ДанныеЗаполнения.КонтрагентБИКРЦБанка;
		РеквизитыСторон.ПолучательКоррСчетРЦБанка = ДанныеЗаполнения.КонтрагентКоррСчетРЦБанка;
		РеквизитыСторон.ПолучательСчетВБанкеДляРасчетов = ДанныеЗаполнения.КонтрагентСчетВБанкеДляРасчетов;
...
Процедура ЗаписатьФайлыПлатежныхПорученийТОФК(Организация, ТаблицаДокументов, КаталогФайлов)
	...
	ТаблицаДокументов.Колонки.Добавить("ОрганизацияКодУБП", Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(8)));
	
	
	КодУБП = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация, "КодУБП");
	ТаблицаДокументов.ЗаполнитьЗначения(КодУБП, "ОрганизацияКодУБП");
	...
	Для Каждого СтрокаДанных Из ТаблицаДокументов Цикл
		...
		ДанныеДокумента = ДанныеДокументаДляВыгрузкиТОФК(ТаблицаДокументов, ДанныеРасшифровки, СтрокаДанных, СтруктураФайла, Исходящий);
		...
		Для Каждого КорневоеСвойство Из ПакетXDTO.КорневыеСвойства Цикл
			...
			ПарметрыОбъектаXDTO = ПараметрыСозданияОбъектаXDTO(Фабрика, КорневоеСвойство.Тип);
			ОбъектXDTO = ЗаполнитьОбъектXDTO(ПарметрыОбъектаXDTO, ДанныеДокумента, СтруктураФайла, "", СтрокаДанных.ОшибкиВыгрузки);
...
Функция ДанныеДокументаДляВыгрузкиТОФК(ТаблицаДокументов, ДанныеРасшифровки, СтрокаДанных, СтруктураФайла, Исходящий)
	
	ДанныеДокумента = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(СтрокаДанных);
		
	ДанныеДокумента.Номер = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеДокумента.Номер, Истина, Истина);
	ДобавитьКлючиКДаннымДокумента(ДанныеДокумента, СтруктураФайла);
	УдалитьНеобязательныеЭлементы(Исходящий, ДанныеДокумента);
	...
	Если Исходящий Тогда
		...
		Если ДанныеДокумента.ОрганизацияЮрФизЛицо <> Перечисления.ЮрФизЛицо.ЮрЛицо
			И Не ЗначениеЗаполнено(ДанныеДокумента.PayerAndRecipient_Payer_KPP) Тогда
			ДанныеДокумента.PayerAndRecipient_Payer_KPP = "0";
		КонецЕсли;
		
		Если ДанныеДокумента.ЮрФизЛицо <> Перечисления.ЮрФизЛицо.ЮрЛицо
			И Не ЗначениеЗаполнено(ДанныеДокумента.PayerAndRecipient_Recip_KPP) Тогда
			ДанныеДокумента.PayerAndRecipient_Recip_KPP = "0";
		КонецЕсли;
		
		ОтборПоДокументу = Новый Структура("Ссылка", ДанныеДокумента.Ссылка);
		
		Если ДанныеДокумента.Операция = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыПоЗарплатномуПроекту
			Или ДанныеДокумента.Операция = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыНаЛицевыеСчета Тогда
			
			ПериодВыплаты = ДанныеДокумента.ДатаВедомостиНаВыплатуЗарплаты;
			
			
			Если ПериодВыплаты = Дата('00010101') Тогда
				ВызватьИсключение СтрШаблон(НСтр("ru = 'В документе %1 не указан период выплаты зарплаты или дата ведомости на выплату зарплаты'"), ДанныеДокумента.Ссылка);
			КонецЕсли;
			
			ДанныеДокумента.Вставить("TPP_PAYPERIOD", Формат(ПериодВыплаты, "ДФ=MM.yyyy"));
			
		КонецЕсли;
		
		Если ДанныеДокумента.Операция = Перечисления.ХозяйственныеОперации.ВозвратДенежныхСредствВДругуюОрганизацию
			Или ДанныеДокумента.Операция = Перечисления.ХозяйственныеОперации.ВозвратОплатыКлиенту
			Или ДанныеДокумента.Операция = Перечисления.ХозяйственныеОперации.ВозвратОплатыНаПлатежнуюКарту Тогда
			ДанныеДокумента.TranscriptPP_ReturnSign = "1";
		Иначе
			ДанныеДокумента.TranscriptPP_ReturnSign = "0";
		КонецЕсли;
		
		ТаблицаРасшифровки = ДанныеРасшифровки.ТаблицаРасшифровки;
		РасшифровкаПлатежа = ТаблицаРасшифровки.Скопировать(ОтборПоДокументу);
		
		ДанныеДокумента.TranscriptPP_TransuseSign = "0";
...
Функция ОписаниеПлатежногоПорученияТОФК(Исходящее)
	...
	ОсновныеРеквизиты.Вставить("TPP_PAYPERIOD", "ПериодВыплаты");
	...
	Если Исходящее Тогда
		...
		ОсновныеРеквизиты.Вставить("PayerAndRecipient_Payer_Code", "ОрганизацияКодУБП");
		...
		// Расшифровка к платежному поручению
		ОсновныеРеквизиты.Вставить("TranscriptPP_TransuseSign", "ПризнакРасшифровки");
		ОсновныеРеквизиты.Вставить("TranscriptPP_ReturnSign", "ПризнакВозврата");
		ОсновныеРеквизиты.Вставить("TranscriptPP_DocNum", "Номер");
		ОсновныеРеквизиты.Вставить("TranscriptPP_DocDate", "Дата");
		
		// Реестр документов-оснований к платежному поручению
		ОсновныеРеквизиты.Вставить("RegistryDocsBaseAttechmentSix_RDOuseSign", "ПризнакРеестраДокументов");
		
		
		ОсновныеРеквизиты.Вставить("SumKBKTotal", "Сумма");
		ОсновныеРеквизиты.Вставить("SumNDSTotal", "СуммаНДС");
...
Функция ЗаполнитьОбъектXDTO(ПараметрыСоздания, ДанныеДокумента, СтруктураОбъекта, ИмяОбъекта, ОшибкиВыгрузки)
	
	Фабрика = ПараметрыСоздания.Фабрика;
	ОбъектДанныхXDTO = Фабрика.Создать(ПараметрыСоздания.Тип);
	...
	Для Каждого ТекущееСвойство Из ОбъектДанныхXDTO.Свойства() Цикл
		...
		Если ТипЗнч(ОбъектДанныхXDTO[ИмяСвойства]) = Тип("СписокXDTO") Тогда
			...
			ПараметрыНовогоОбъекта = ПараметрыСозданияОбъектаXDTO(Фабрика, ТипСвойства);
			
			Для Каждого СтрокаДокумента Из ДанныеДокумента[Ключ] Цикл
				
				ОбъектXDTO = ЗаполнитьОбъектXDTO(ПараметрыНовогоОбъекта, СтрокаДокумента, СтруктураОбъекта[ИмяСвойства], "", ОшибкиВыгрузки);
		...
		Если ТипЗнч(ТипСвойства) = Тип("ТипОбъектаXDTO") Тогда
			...
			ПараметрыНовогоОбъекта = ПараметрыСозданияОбъектаXDTO(Фабрика, ТипСвойства);
			ЗначениеПоля = ЗаполнитьОбъектXDTO(ПараметрыНовогоОбъекта, ДанныеДокумента, СтруктураОбъекта[ИмяСвойства], ИмяСвойства, ОшибкиВыгрузки);
		...
		Если ТекстОшибки <> "" Тогда
			...
			ДобавитьЗамечание(ОшибкиВыгрузки, ТекстОшибки, Символы.ПС + Символы.ПС);
		...
		Иначе
			...
			Исключение
				...
				Шаблон = НСтр("ru = 'Ошибка установки значения свойства ""%1"" по причине:
							|%2'");
				ТекстОшибкиКратко = СтрШаблон(Шаблон, ИмяСвойства, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
				ТекстОшибкиПодробно = СтрШаблон(Шаблон, ИмяСвойства, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				ДобавитьЗамечание(ОшибкиВыгрузки, ТекстОшибкиКратко, Символы.ПС + Символы.ПС);
				ЗаписатьВЖурналРегистрации(ТекстОшибкиПодробно);
...
Функция ИзменитьФорматОтображенияДесятичныхСвойствXML(ТекстXML, ДесятичныеСвойства)
	...
	ЗаписьXML.ЗаписатьАтрибут("Version", "9.0");
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если НачальныйТег Тогда
			НачальныйТег = Ложь;
		Иначе
			ОбменСБанкамиУТ.ЗаписатьУзелXML(ЧтениеXML, ЗаписьXML, ДесятичныеСвойства, ИмяЭлемента);
		КонецЕсли;
...
Процедура УдалитьНеобязательныеЭлементы(Исходящий, ДанныеДокумента)
	
	КлючиКУдалению = Новый Массив;
	
	Если Не ЗначениеЗаполнено(ДанныеДокумента.AdditionalInfo_PayDate) Тогда
		КлючиКУдалению.Добавить("AdditionalInfo_PayDate");
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ДанныеДокумента.PayerAndRecipient_Payer_CorrAcc) Тогда
		КлючиКУдалению.Добавить("PayerAndRecipient_Payer_CorrAcc");
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ДанныеДокумента.PayerAndRecipient_Recip_KPP) Тогда
		КлючиКУдалению.Добавить("PayerAndRecipient_Recip_KPP");
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ДанныеДокумента.PayerAndRecipient_Recip_CheckAcc) Тогда
		КлючиКУдалению.Добавить("PayerAndRecipient_Recip_CheckAcc");
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ДанныеДокумента.PayerAndRecipient_Recip_CorrAcc) Тогда
		КлючиКУдалению.Добавить("PayerAndRecipient_Recip_CorrAcc");
	КонецЕсли;
	
	Если Исходящий Тогда
	
		Если Не ДанныеДокумента.ПеречислениеВБюджет Тогда
			
			Если Не ЗначениеЗаполнено(ДанныеДокумента.TAX_UIN) Тогда
				КлючиКУдалению.Добавить("TAX_UIN");
			КонецЕсли;
			
			КлючиКУдалению.Добавить("TAX_DrawStat");
			КлючиКУдалению.Добавить("TAX_KBK");
			КлючиКУдалению.Добавить("TAX_OKTMO");
			КлючиКУдалению.Добавить("TAX_PayReason");
			КлючиКУдалению.Добавить("TAX_Period");
			КлючиКУдалению.Добавить("TAX_DocNo");
			КлючиКУдалению.Добавить("TAX_DocDate");
			
		КонецЕсли;
		
		Если ДанныеДокумента.Операция <> Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыПоЗарплатномуПроекту
			И ДанныеДокумента.Операция <> Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыНаЛицевыеСчета Тогда
			КлючиКУдалению.Добавить("TPP_PAYPERIOD");
		КонецЕсли;
		
	Иначе
		
		Если Не ЗначениеЗаполнено(ДанныеДокумента.PayerAndRecipient_Payer_BankName) Тогда
			КлючиКУдалению.Добавить("PayerAndRecipient_Payer_BankName");
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ДанныеДокумента.BasicRequisites_PayView) Тогда
			КлючиКУдалению.Добавить("BasicRequisites_PayView");
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ДанныеДокумента.AdditionalInfo_OperType) Тогда
			КлючиКУдалению.Добавить("AdditionalInfo_OperType");
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ДанныеДокумента.PayerAndRecipient_Payer_KPP) Тогда
			КлючиКУдалению.Добавить("PayerAndRecipient_Payer_KPP");
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ДанныеДокумента.PayerAndRecipient_Recip_INN) Тогда
			КлючиКУдалению.Добавить("PayerAndRecipient_Recip_INN");
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ДанныеДокумента.PayerAndRecipient_Recip_BankName) Тогда
			КлючиКУдалению.Добавить("PayerAndRecipient_Recip_BankName");
		КонецЕсли;
		
		КлючиКУдалению.Добавить("PayerAndRecipient_Recip_PersonalAcc");
	...
	Для Каждого ТекущийКлюч Из КлючиКУдалению Цикл
		
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеДокумента, ТекущийКлюч) Тогда
			ДанныеДокумента.Удалить(ТекущийКлюч);
		КонецЕсли;
	...
	Если ЧтениеZip <> Неопределено Тогда
		...
		Иначе
			...
			Для Каждого РасчетныйСчет Из Результат.РасчетныеСчета Цикл
				...
				Для Каждого ТекущийПлатеж Из Результат.ДокументыВыписки Цикл
					
					ОдинаковыйРаздел = 
						КлючСтруктурыИРеквизитСчетаСовпадает(РасчетныйСчет.РазделЛицевогоСчета, ТекущийПлатеж, "АналитическийКодРаздела");
					
					Если ТекущийПлатеж.НаправлениеПлатежа = "1" Тогда
						
						ОдинаковыйСчет = РасчетныйСчет.РасчСчет = ТекущийПлатеж.ПлательщикРасчСчет;
						ОдинаковыйЛицевойСчет = 
							КлючСтруктурыИРеквизитСчетаСовпадает(РасчетныйСчет.НомерЛицевогоСчета, ТекущийПлатеж, "ПлательщикЛицевойСчет");
						
					Иначе
						
						ОдинаковыйСчет = РасчетныйСчет.РасчСчет = ТекущийПлатеж.ПолучательРасчСчет;
						ОдинаковыйЛицевойСчет = 
							КлючСтруктурыИРеквизитСчетаСовпадает(РасчетныйСчет.НомерЛицевогоСчета, ТекущийПлатеж, "ПолучательЛицевойСчет");
						
					КонецЕсли;
					
					Если ОдинаковыйСчет
						И ОдинаковыйРаздел
						И ОдинаковыйЛицевойСчет Тогда
						
						РасчетныйСчет.ДатаНачала = Результат.Заголовок.ДатаНачала;
						РасчетныйСчет.ДатаКонца = Результат.Заголовок.ДатаКонца;
					
						РасчетныйСчет.ВсегоПоступило = ?(РасчетныйСчет.ВсегоПоступило = Неопределено, 0, РасчетныйСчет.ВсегоПоступило)
							+ ?(ТекущийПлатеж.НаправлениеПлатежа = "1", 0, ТекущийПлатеж.Сумма);
						РасчетныйСчет.ВсегоСписано = ?(РасчетныйСчет.ВсегоСписано = Неопределено, 0, РасчетныйСчет.ВсегоСписано) 
							+ ?(ТекущийПлатеж.НаправлениеПлатежа = "1", ТекущийПлатеж.Сумма, 0);
...
Процедура ПрочитатьФайлРаспоряженияОСовершенииПлатежаТОФК(Файл, Результат)
	...
	ЭтоФайлРаспоряжения = Ложь;
	ИмяНабора = Метаданные.ПакетыXDTO.MSC_TransfOrderAcc.ПространствоИмен;
	
	Пока ЧтениеXML.Прочитать() Цикл
		
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			
			Если ЧтениеXML.URIПространстваИмен = ИмяНабора Тогда
				ЭтоФайлРаспоряжения = Истина;
				Прервать;
			КонецЕсли;
	...
	Если Не ЭтоФайлРаспоряжения Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьДанныеРаспоряженияТОФК(СтруктураДанныхФайла, Результат)
	...
	Пока Итерация < 2 Цикл
		...
		Если Итерация = 0 Тогда
			...
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтруктураДанныхФайла, "ExecDate") Тогда
...
Процедура ЗаписатьОстаткиНаСчетахПоДаннымВыписки(БанковскийСчет, РеквизитыСчета, ДанныеВыписки, ДатаНачалаЗагрузки, ДатаКонцаЗагрузки)
	...
	Если ДанныеВыписки <> Неопределено Тогда
		...
		Для каждого РасчетныйСчет Из РасчетныеСчета Цикл
			...
			ОдинаковыйНомерЛицевогоСчета =
				КлючСтруктурыИРеквизитСчетаСовпадает(НомерЛицевогоСчета, РасчетныйСчет, "НомерЛицевогоСчета");
			ОдинаковыйРазделЛицевогоСчета =
				КлючСтруктурыИРеквизитСчетаСовпадает(РазделЛицевогоСчета, РасчетныйСчет, "РазделЛицевогоСчета");
			
			Если НомерСчета = РасчетныйСчет.РасчСчет
				И ОдинаковыйНомерЛицевогоСчета
				И ОдинаковыйРазделЛицевогоСчета Тогда
				...
				ИначеЕсли ЗначениеЗаполнено(ДатаНачала) И ЗначениеЗаполнено(ДатаКонца) Тогда
					...
					Пока ДатаНачала <= ДатаКонца Цикл
						...
						Для Каждого СтрокаДокумента Из ДанныеВыписки.ДокументыВыписки Цикл
							...
							Если Не КлючСтруктурыИРеквизитСчетаСовпадает(РазделЛицевогоСчета, СтрокаДокумента, "АналитическийКодРаздела") Тогда
								Продолжить;
							КонецЕсли;
							
							СуммаДокумента = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(СтрокаДокумента.Сумма);
							
							Если СтрокаДокумента.ПлательщикСчет = НомерСчета
								И КлючСтруктурыИРеквизитСчетаСовпадает(НомерЛицевогоСчета, СтрокаДокумента, "ПлательщикЛицевойСчет") Тогда
								
								ДатаСписания = ДенежныеСредстваКлиентСервер.ПолучитьДатуИзСтроки(СтрокаДокумента.ДатаСписано);
								
								Если ДатаНачала = ДатаСписания Тогда
									Списание = Списание + ?(СуммаДокумента = Неопределено, 0, СуммаДокумента);
								КонецЕсли;
								
							КонецЕсли;
							
							Если СтрокаДокумента.ПолучательСчет = НомерСчета
								И КлючСтруктурыИРеквизитСчетаСовпадает(НомерЛицевогоСчета, СтрокаДокумента, "ПолучательЛицевойСчет") Тогда
								
								ДатаПоступления = ДенежныеСредстваКлиентСервер.ПолучитьДатуИзСтроки(СтрокаДокумента.ДатаПоступило);
								
								Если ДатаНачала = ДатаПоступления Тогда
						...
						ПараметрыЗаписи.Вставить("Поступление", Поступление);
						ПараметрыЗаписи.Вставить("Списание", Списание);
						ПараметрыЗаписи.Вставить("КонечныйОстаток", 
							?(НачальныйОстаток = Неопределено, Неопределено, НачальныйОстаток + Поступление - Списание));
...
Процедура ПроверитьДубликатыПлатежейВыписки(ДокументыКЗагрузке)

	ДубликатыПлатежей = ДокументыКЗагрузке.Скопировать(, 
		"Исходящий, Загружать, БанковскийСчет, Номер, Дата, ПолучательСчет, ПлательщикСчет, ПолучательИНН, ПлательщикИНН, ДатаСписано, ДатаПоступило, ТипПлатежногоДокумента");
	ДубликатыПлатежей.Колонки.Добавить("ВсегоПлатежей", Новый ОписаниеТипов("Число"));
	ДубликатыПлатежей.ЗаполнитьЗначения(1, "ВсегоПлатежей");
	ДубликатыПлатежей.Свернуть("Исходящий, Загружать, БанковскийСчет, Номер, Дата, ПолучательСчет, ПлательщикСчет, ПолучательИНН, ПлательщикИНН, ДатаСписано, ДатаПоступило, ТипПлатежногоДокумента", "ВсегоПлатежей");
	
	ОшибкиДублей = Новый Массив;
	
	Для Каждого СтрокаТаблицы Из ДубликатыПлатежей Цикл
		
		Если СтрокаТаблицы.ВсегоПлатежей > 1
			И СтрокаТаблицы.Загружать Тогда
			
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Существует несколько одинаковых платежей с номером %1 от %2 по счету контрагента %3'"),
				СтрокаТаблицы.Номер, СтрокаТаблицы.Дата, ?(СтрокаТаблицы.Исходящий, СтрокаТаблицы.ПолучательСчет, СтрокаТаблицы.ПлательщикСчет));
			ОшибкиДублей.Добавить(ТекстОшибки);
			
		КонецЕсли;
		
	КонецЦикла;
	
	Если ОшибкиДублей.Количество() > 0 Тогда
		
		ТекстСообщения = СтрСоединить(ОшибкиДублей, Символы.ПС);
...
Функция КлючСтруктурыИРеквизитСчетаСовпадает(РеквизитСчета, ТекущаяСтруктура, ИмяКлюча)

	ЕстьПолеСтруктуры = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущаяСтруктура, ИмяКлюча);
	Возврат (ЕстьПолеСтруктуры И РеквизитСчета = ТекущаяСтруктура[ИмяКлюча])
		Или (ЕстьПолеСтруктуры И ТекущаяСтруктура[ИмяКлюча] = Неопределено)
		Или Не ЕстьПолеСтруктуры;
...
Функция ПараметрыСозданияОбъектаXDTO(Фабрика, ТипМодели)
	
	Параметры = Новый Структура;
	Параметры.Вставить("Фабрика", Фабрика);
	Параметры.Вставить("Тип", ТипМодели);
