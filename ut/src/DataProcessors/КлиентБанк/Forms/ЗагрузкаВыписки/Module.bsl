...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	ОтключитьОбработчикОжидания("ПроверитьПараллельнуюЗагрузкуВыписки");
...
Процедура ОбновитьСписокСчетов(Команда)
	...
	ОбновитьСписокБанковскихСчетов(Истина);
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьСписокСчетовНаСервере(ОбновитьСписок)
	...
	ЗаполнитьДеревоСчетовПоРежимуОтображения(ОбновитьСписок);
...
Процедура ЗагрузитьДокументы(Команда)
	...
	Если ВыполняетсяОбработкаФайлов Тогда
		Возврат;
	КонецЕсли;
	
	ВыполняетсяОбработкаФайлов = Истина;
	ЕстьОшибки = Ложь;
	...
	Если ЕстьОшибки Тогда
		...
		ВыполняетсяОбработкаФайлов = Ложь;
		Возврат;
...
Функция ВывестиОтчетНаСервере(ИндексыСтрок)
	...
	Для каждого ИндексСтроки Из ИндексыСтрок Цикл
		...
		Для каждого Файл Из СписокФайлов Цикл
			...
			Если Не ДенежныеСредстваКлиентСервер.ЕстьСчетВФайлеВыписки(РеквизитыСчета.НомерСчета, Файл.Выписка.ДанныеВыписки)
				И Не Счет.ПрямойОбмен Тогда
				Продолжить;
			КонецЕсли;
			
			Для каждого ДокументВыписки Из Файл.Выписка.ДанныеВыписки.ДокументыВыписки Цикл
				
				Если ДокументВыписки.ПлательщикСчет <> СокрЛП(РеквизитыСчета.НомерСчета)
					И ДокументВыписки.ПолучательСчет <> СокрЛП(РеквизитыСчета.НомерСчета) Тогда
					Продолжить;
				КонецЕсли;
				
				Если ДокументВыписки.ПлательщикЛицевойСчет <> Неопределено
					И ДокументВыписки.ПлательщикЛицевойСчет <> РеквизитыСчета.НомерЛицевогоСчета
					И ДокументВыписки.ПолучательЛицевойСчет <> Неопределено
					И ДокументВыписки.ПолучательЛицевойСчет <> РеквизитыСчета.НомерЛицевогоСчета Тогда
					Продолжить;
				КонецЕсли;
				
				Если ДокументВыписки.АналитическийКодРаздела <> Неопределено
					И ДокументВыписки.АналитическийКодРаздела <> РеквизитыСчета.РазделЛицевогоСчета Тогда
					Продолжить;
				КонецЕсли;
				
				РазличныеСчета = ДокументВыписки.ПлательщикСчет <> ДокументВыписки.ПолучательСчет;
				
				ПлатежОрганизацииПолучателя = 
					ДокументВыписки.ПлательщикСчет = РеквизитыСчета.НомерСчета
					И РазличныеСчета
					И ЗначениеЗаполнено(ДокументВыписки.ПолучательСчет)
					И ДенежныеСредстваКлиентСервер.ЕстьСчетВФайлеВыписки(ДокументВыписки.ПолучательСчет, Файл.Выписка.ДанныеВыписки)
					И Не ЗначениеЗаполнено(ДокументВыписки.ДатаСписано)
					И ЗначениеЗаполнено(ДокументВыписки.ДатаПоступило);
			
				ПлатежОрганизацииПлательщика =
					ДокументВыписки.ПолучательСчет = РеквизитыСчета.НомерСчета
					И РазличныеСчета
					И ЗначениеЗаполнено(ДокументВыписки.ПлательщикСчет)
					И ДенежныеСредстваКлиентСервер.ЕстьСчетВФайлеВыписки(ДокументВыписки.ПлательщикСчет, Файл.Выписка.ДанныеВыписки)
					И ЗначениеЗаполнено(ДокументВыписки.ДатаСписано)
					И Не ЗначениеЗаполнено(ДокументВыписки.ДатаПоступило);
...
Процедура ЗавершитьЗагрузку()
	...
	ЗаполнитьРезультатыЗагрузки(БанковскиеСчета.ПолучитьЭлементы(), ЗагруженныеСчета, ЗагруженныеПлатежи);
...
&НаСервереБезКонтекста
Функция ЗаданиеВыполнено(ИдентификаторЗадания, Счет, ИмяФайла)
	
	Результат = ДлительныеОперации.ЗаданиеВыполнено(ИдентификаторЗадания, Истина);
	
	Если Результат.Статус = "Ошибка" Тогда
		Обработки.КлиентБанк.ВывестиОшибкуЗагрузкиФайла(Счет, ИмяФайла, Результат.ИнформацияОбОшибке);
	КонецЕсли;
	
	Возврат Результат.Статус <> "Выполняется";
...
Функция РазобратьДанныеВыпискиДокумента(ДанныеВыписки, БанковскийСчет)
	...
	КлиентБанкЛокализация.ЗаполнитьДанныеКонтрагентаПоПлатежу(ДанныеДокумента.Исходящий, ДанныеДокумента, ИмяКонтрагента);
...
&НаКлиенте
Процедура ОбновитьСписокБанковскихСчетов(ОбновитьСписок = Ложь)

	ПроизводитсяЗагрузка = ВыполняетсяЗагрузка(БанковскиеСчета.ПолучитьЭлементы())
		Или ВыполняетсяОбработкаФайлов;
	
	Если Не ПроизводитсяЗагрузка Тогда
		ОбновитьСписокСчетовНаСервере(ОбновитьСписок);
		ПрочитатьДанныеВыписок();
	КонецЕсли;
		...
		Если СтрокаДерева.Уровень = 0 Тогда
			...
			Если СтрокаДерева.Ссылка = ОтобранныйБанковскийСчет Тогда
				...
				Если ЗначениеЗаполнено(СтрокаДерева.НастройкаОбмена) Тогда
					...
					Если СтрокаДерева.ПрямойОбмен Тогда
						ТекстСообщения = НСтр("ru = '<Выбран прямой обмен. Данные будут запрошены при загрузке платежей.>'");
					Иначе
			...
			Если Счет.Загружается Тогда
				...
				Для каждого Файл Из ФайлыПоСчету Цикл
					...
					Если ЗначениеЗаполнено(Файл.ИдентификаторЗагрузки) Тогда
						...
						Если ЗаданиеВыполнено(Файл.ИдентификаторЗагрузки, Файл.БанковскийСчет, Файл.ИмяФайла) Тогда
...
&НаКлиенте
Процедура ЗаполнитьРезультатыЗагрузки(ЭлементыДерева, ЗагруженныеСчета, ЗагруженныеПлатежи)
	...
	Для каждого Счет Из ЭлементыДерева Цикл
		...
		Иначе
			ЗаполнитьРезультатыЗагрузки(Счет.ПолучитьЭлементы(), ЗагруженныеСчета, ЗагруженныеПлатежи);
		КонецЕсли;
...
Функция ОшибкаЗаполненияБанком(СтрокиВыписки, ТекстСообщения)
	...
	СекцияРасчетныйСчет = Ложь;
	НайденТегНачальныйОстаток = Ложь;
	НайденТегКонечныйОстаток = Ложь;
	
	Для Каждого СтрокаВыписки Из СтрокиВыписки Цикл
		
		Если СекцияРасчетныйСчет Тогда
			
			Если СтрНайти(ВРег(СтрокаВыписки), "НАЧАЛЬНЫЙОСТАТОК=") <> 0 Тогда
				НайденТегНачальныйОстаток = Истина;
			КонецЕсли;
			
			Если СтрНайти(ВРег(СтрокаВыписки), "КОНЕЧНЫЙОСТАТОК=") <> 0 Тогда
				НайденТегКонечныйОстаток = Истина;
			КонецЕсли;
			
			Если СтрНайти(ВРег(СтрокаВыписки), "КОНЕЦРАСЧСЧЕТ") <> 0 Тогда
				
				СекцияРасчетныйСчет = Ложь;
				
				Если Не НайденТегНачальныйОстаток
					И НайденТегКонечныйОстаток Тогда
					ТекстСообщения = ?(ТекстСообщения <> "", ТекстСообщения + Символы.ПС, "") 
						+ ТекстОбОтсутствииОбязательногоТега("НачальныйОстаток");
				КонецЕсли;
		...
		Если СтрНайти(ВРег(СтрокаВыписки), "СЕКЦИЯРАСЧСЧЕТ") <> 0 Тогда
			СекцияРасчетныйСчет = Истина;
		КонецЕсли;
