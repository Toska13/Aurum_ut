...
Процедура ПроверитьДокументыКВыгрузке()
	...
	ПроверкаФК = Новый Соответствие;
	ПроверкаФК.Вставить("ОрганизацияИНН",               НСтр("ru='ИНН организации'"));
	ПроверкаФК.Вставить("ОрганизацияНаим",              НСтр("ru='Наименование организации'"));
	ПроверкаФК.Вставить("ОрганизацияРасчСчет",          НСтр("ru='Банковский счет организации'"));
	ПроверкаФК.Вставить("КонтрагентРасчСчет",           НСтр("ru='Банковский счет контрагента'"));
	ПроверкаФК.Вставить("КонтрагентНаим",               НСтр("ru='Контрагент'"));
	ПроверкаФК.Вставить("КонтрагентИНН",                НСтр("ru='ИНН контрагента'"));
	ПроверкаФК.Вставить("ЛицевойСчет",                  НСтр("ru='Номер лицевого счета'"));
	ПроверкаФК.Вставить("НазначениеПлатежа",            НСтр("ru='Назначение платежа'"));
	...
	Для каждого ДокументКВыгрузке Из ДокументыКВыгрузке Цикл
		...
		Если Счет.ФорматОбмена = ПредопределенноеЗначение("Перечисление.ФорматОбменаСБанком.ClientBankExchange1c") Тогда
			...
			СтрокаОшибки = СтрокаОшибки + ПустыеОбязательныеПоляПоСписку(ДокументКВыгрузке, Проверки);
			...
			СтрокаОшибки = СтрокаОшибки + ПустыеОбязательныеПоляПоСписку(ДокументКВыгрузке, ПроверкаСчетаОрганизации);
		...
		ИначеЕсли Счет.ФорматОбмена = ПредопределенноеЗначение("Перечисление.ФорматОбменаСБанком.ФедеральноеКазначействоРФ") Тогда
			...
			СтрокаОшибки = СтрокаОшибки + ПустыеОбязательныеПоляПоСписку(ДокументКВыгрузке, ПроверкаФК);
			
			Если ДокументКВыгрузке.ОрганизацияЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо
				И (Не ЗначениеЗаполнено(ДокументКВыгрузке.КПППлательщика)
				Или ДокументКВыгрузке.КПППлательщика = "0") Тогда
			
				Текст = НСтр("ru='КПП организации'");
				СтрокаОшибки = СтрокаОшибки + " " + Текст + ",";
				
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(Счет.РазделЛицевогоСчета) Тогда
				
				Текст = НСтр("ru='Раздел лицевого счета организации'");
				СтрокаОшибки = СтрокаОшибки + " " + Текст + ",";
				
			КонецЕсли;
			
			Если ДокументКВыгрузке.ОрганизацияЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо
				И (Не ЗначениеЗаполнено(ДокументКВыгрузке.КПППолучателя)
				Или ДокументКВыгрузке.КПППолучателя = "0") Тогда
			
				Текст = НСтр("ru='КПП контрагента'");
				СтрокаОшибки = СтрокаОшибки + " " + Текст + ",";
		...
		Если ДокументКВыгрузке.ПеречислениеВБюджет Тогда
			
			ОшибкиЗаполнения = Новый Массив;
			
			Если СтрДлина(ДокументКВыгрузке.ОрганизацияНаим) > 160 Тогда
				ОшибкиЗаполнения.Добавить(НСтр("ru = 'Наименование плательщика превышает 160 символов'"));
			КонецЕсли;
			
			Если СтрДлина(ДокументКВыгрузке.КонтрагентНаим) > 160 Тогда
				ОшибкиЗаполнения.Добавить(НСтр("ru = 'Наименование получателя превышает 160 символов'"));
			КонецЕсли;
			
			ПлатежиВБюджет.ДополнитьОшибкиИННКПППлательщикаПоОбъекту(
				ОшибкиЗаполнения, ДокументКВыгрузке, ДокументКВыгрузке.Дата, ДокументКВыгрузке.ОрганизацияИНН, ДокументКВыгрузке.КПППлательщика);
			
			Если ДокументКВыгрузке.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо 
				И СтрДлина(ДокументКВыгрузке.КПППолучателя) <> 9 Тогда
				ОшибкиЗаполнения.Добавить(НСтр("ru = 'КПП получателя должно содержать 9 символов'"));
			КонецЕсли;
			
			Если СтрДлина(ДокументКВыгрузке.НазначениеПлатежа) > 210 Тогда
				ОшибкиЗаполнения.Добавить(НСтр("ru = 'Текст назначения платежа превышает 210 символов'"));
			КонецЕсли;
			
			СтрокаОшибки = "";
			
			Для Каждого ТекстОшибки Из ОшибкиЗаполнения Цикл
				СтрокаОшибки = СтрокаОшибки + " " + ТекстОшибки + ",";
			КонецЦикла;
			...
			Если ЗначениеЗаполнено(СтрокаОшибки) Тогда
				...
				ДокументКВыгрузке.ЕстьОшибка = Истина;
				ДокументКВыгрузке.Выгружать = Ложь;
				ДокументКВыгрузке.ОписаниеОшибок = 
					?(ДокументКВыгрузке.ОписаниеОшибок <> "", ДокументКВыгрузке.ОписаниеОшибок + ".", "")
					+ Символы.ПС + НСтр("ru='Ошибки заполнения:'") + СтрокаОшибки;
...
Процедура ЗаполнитьИнформациюОПолучателяхПлательщиках(ТаблицаПлатежей)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаПлатежей Цикл
		...
		Если Не ПолучитьФункциональнуюОпцию("ПоддержкаПлатежейРФ")
			Или СтрокаТаблицы.ВидДокумента = Перечисления.ТипыПлатежныхДокументов.ПлатежноеТребование Тогда
		...
		СтрокаТаблицы.ОрганизацияИНН = СокрЛП(СтрокаТаблицы.ОрганизацияИНН);
		СтрокаТаблицы.КПППлательщика = СокрЛП(СтрокаТаблицы.КПППлательщика);
		СтрокаТаблицы.ОрганизацияНаим = СокрЛП(СтрокаТаблицы.ОрганизацияНаим);
		
	КонецЦикла;

КонецПроцедуры

Функция ПустыеОбязательныеПоляПоСписку(ДанныеДокумента, Реквизиты)

	СтрокаОшибки = "";
	
	Для каждого ТекущийРеквизит Из Реквизиты Цикл
		
		Если Не ЗначениеЗаполнено(ДанныеДокумента[ТекущийРеквизит.Ключ]) Тогда
			СтрокаОшибки = СтрокаОшибки + " " + ТекущийРеквизит.Значение + ",";
		КонецЕсли;
