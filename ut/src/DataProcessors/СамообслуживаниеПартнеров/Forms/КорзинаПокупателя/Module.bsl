...
Процедура КорзинаПокупателяКОформлениюПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.НоменклатураНабора) Тогда
		...
		ПараметрыНабора.Вставить("ХарактеристикаНабора", ТекущиеДанные.ХарактеристикаНабора);
		
		НайденныеСтроки = КорзинаПокупателя.НайтиСтроки(ПараметрыНабора);
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			 НайденнаяСтрока.КОформлению = ТекущиеДанные.КОформлению;
		КонецЦикла;
...
Функция ТекстЗапросаДанныеКорзиныПоРегистру()
	...
	|	ЕСТЬNULL(КатегорииПодакцизныхТоваров.ПодакцизныйТовар, ЛОЖЬ) КАК ПодакцизныйТовар
	...
	|		ПО (ИСТИНА)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КатегорииПодакцизныхТоваров.СрезПоследних(&Дата,
	|			&ИспользоватьПодакцизныеТовары И Страна = &СтранаРегистрации) КАК КатегорииПодакцизныхТоваров
	|		ПО КорзинаПокупателя.Номенклатура = КатегорииПодакцизныхТоваров.Номенклатура
	|ГДЕ
...
Функция ТекстЗапросаДанныеПоДоступностиТоваровНаСкладе()
	...
	|		ПО ДатыПлановойПоставки.Номенклатура = ДанныеПоДоступностиИМУЗ.Номенклатура
	|			И ДатыПлановойПоставки.Характеристика = ДанныеПоДоступностиИМУЗ.Характеристика
	|			И ДатыПлановойПоставки.Склад = ДанныеПоДоступностиИМУЗ.Склад
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|";
...
Функция ТекстЗапросаДанныеКорзиныПоТабличнойЧасти(ЕстьДанныеТаблицыЗагрузки)
	...
	Если НЕ ЕстьДанныеТаблицыЗагрузки Тогда
		...
		|////////////////////////////////////////////////////////////////////////////////
		|"
		+ УчетПодакцизныхТоваров.ТекстЗапросаВтПодакцизныеТовары("КорзинаПокупателя", , "СтранаРегистрации")
		...
		|		ПО КорзинаПокупателя.Номенклатура = ВтПодакцизныеТовары.Номенклатура
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|";
	...
	Иначе
		...
		|////////////////////////////////////////////////////////////////////////////////
		|"
		+ УчетПодакцизныхТоваров.ТекстЗапросаВтПодакцизныеТовары("ТаблицаЗагруженныхТоваров", , "СтранаРегистрации")
		+ ОбщегоНазначения.РазделительПакетаЗапросов() + "
		|ВЫБРАТЬ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтПодакцизныеТовары КАК ВтПодакцизныеТовары
		|		ПО ТаблицаЗагруженныхТоваров.Номенклатура = ВтПодакцизныеТовары.Номенклатура
		|ГДЕ
...
Функция ТекстЗапросаРезультирующий()
	...
	Если ДоступенПросмотрОстатков Тогда
		...
		|	СУММА(ОстатокНаСкладе.Количество) КАК Свободно
		...
		|	И ОстатокНаСкладе.СостояниеЗапаса = ЗНАЧЕНИЕ(Перечисление.СостоянияЗапаса.ВНаличии)
		|	И ОстатокНаСкладе.ХарактеристикаПотребности В(
...
Процедура УстановитьЗначенияПараметровВыполнитьЗапрос(ПоРегистру = Истина, ТаблицаТоваров = Неопределено)
	...
	Текст = СтрЗаменить(Текст, "ВтТовары", "ВсеТоварыИСклады");
	Текст = СтрЗаменить(Текст, "РАЗРЕШЕННЫЕ", "");
	...
	Текст =
		...
		|	РасчетДатыПоставкиПереопределяемый.ПлановаяДатаПоставки КАК Дата
	...
	Подстановки = ОбеспечениеВДокументахСервер.ПодстановкиПлановойДатыПоставки(
		"Таблица.СпособОбеспеченияПотребностей",
		"&НачалоТекущегоДня",
	...
	СтруктураПересчетаСуммы = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(ЭтаФорма);
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(КорзинаПокупателя, СтруктураДействий);
...
Процедура ОбработатьДобавлениеТоваров(ТаблицаТоваров)
	...
	Запрос.Текст = Обработки.СамообслуживаниеПартнеров.ВременныеТаблицыДляРасчетаДоступныхОстатков("ВтВсеТоварыИСклады")
		...
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтДатыПлановойПоставки КАК ДатыПлановойПоставки
		|		ПО ДатыПлановойПоставки.Номенклатура = Товары.Номенклатура
		|		 И ДатыПлановойПоставки.Характеристика = Товары.Характеристика
		|		 И ДатыПлановойПоставки.Склад = Товары.Склад
		|
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
	...
	Если ДоступенПросмотрОстатков Тогда
		...
		+ ОбщегоНазначения.РазделительПакетаЗапросов()
		+ УчетПодакцизныхТоваров.ТекстЗапросаВтПодакцизныеТовары("ДанныеСНДС", "ТекущаяДата")
		+ ОбщегоНазначения.РазделительПакетаЗапросов()
		+"ВЫБРАТЬ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтПодакцизныеТовары КАК ВтПодакцизныеТовары
		|		ПО ДанныеСНДС.Номенклатура = ВтПодакцизныеТовары.Номенклатура";
