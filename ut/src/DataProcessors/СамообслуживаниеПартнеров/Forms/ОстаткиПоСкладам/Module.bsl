...
Процедура Сформировать()
	...
	Текст =
		...
		|ГДЕ
		|	Таблица.СпособОбеспеченияПотребностей <> ЗНАЧЕНИЕ(Справочник.СпособыОбеспеченияПотребностей.ПустаяСсылка)
		|ИНДЕКСИРОВАТЬ ПО
	...
	Запрос.Текст = Обработки.СамообслуживаниеПартнеров.ВременныеТаблицыДляРасчетаДоступныхОстатков("ВсеТоварыИСклады")
