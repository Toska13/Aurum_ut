...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не НесколькоНазначений И Не ВидОперации = ВидОперацииВстречнаяКорректировка Тогда
...
Процедура ДеревоПомещенияЯчейкиКоличествоВетвьПриИзменении(Элемент)
	...
	ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураПересчетаЕдиниц, Неопределено);
...
Процедура НастроитьЭлементыПоШагуМногихНазначений()
	...
	Если НесколькоНазначений Тогда
		...
		Если Элементы.ТаблицаТовары.ОтборСтрок.ИндексСтраницы = 0 Тогда
			...
			Элементы.ТаблицаТоварыНаКонтроле.Видимость = Истина;
			Элементы.ТаблицаТоварыКартинкаИзлишекИсточника.Видимость = Истина;
			Элементы.ТаблицаТоварыПотребность.Видимость = Истина;
			Элементы.ГруппаИнформацияПоИсточникамОбеспечения.Видимость = ИспользоватьНесколькоСкладов;
		...
		Иначе
			...
			Элементы.ТаблицаТоварыНаКонтроле.Видимость = Ложь;
			Элементы.ТаблицаТоварыКартинкаИзлишекИсточника.Видимость = Ложь;
			Элементы.ТаблицаТоварыПотребность.Видимость = Ложь;
			Элементы.ГруппаИнформацияПоИсточникамОбеспечения.Видимость = Ложь;
...
Функция ДалееКСозданиюДокументаСервер(Ошибка, ТаблицаКПереносу)
	...
	Если ВидОперации = Перечисления.ВидыОперацийКорректировкиНазначения.ВстречнаяКорректировка Тогда
		...
		ОтборСтрок = Новый Структура("Отметка, ИндексСтраницы", Истина, 1);
		Если ТипЗнч(ТаблицаКПереносу) = Тип("ДанныеФормыКоллекция") Тогда
			ТаблицаРаспределения = ТаблицаКПереносу.Выгрузить(ОтборСтрок);
		Иначе
			ТаблицаРаспределения = ТаблицаКПереносу.Скопировать(ОтборСтрок);
		КонецЕсли;
	...
	Документы.КорректировкаНазначенияТоваров.ДополнитьТаблицуКонтролируемымиПартиями(ТоварыКПереносу, КорректировкаНазначения);
...
&НаСервере
Функция ДалееКПереносуВДокументСервер(Ошибка, Знач ТаблицаКПереносу)
	...
	Если ТаблицаКПереносу = Неопределено Тогда
		ТаблицаКПереносу = ТаблицаТовары.Выгрузить();
	КонецЕсли;
	
	Документы.КорректировкаНазначенияТоваров.ДополнитьТаблицуКонтролируемымиПартиями(ТаблицаКПереносу, КорректировкаНазначения);
...
Процедура ЗаполнитьДерево()
	...
	Для Каждого Строка Из ТаблицаПомещенияЯчейки Цикл
		...
		Если СтруктураЗаполнения.Назначение <> Строка.Назначение
			...
			НовыйКорень.ОбеспеченоНаКонтроле = Строка.СвободныйОстатокНаКонтроле;
		...
		Если Строка.ЕстьЯчейки Тогда
			...
			Если Строка.ЕстьСерии Тогда
				Если ТекущаяСерия <> Строка.Серия Тогда
					...
					НоваяВетвьСерии.ОбеспеченоНаКонтроле = - Строка.СерииКОтгрузкеНаКонтроле;
			...
			ЗаполнитьЗначенияСвойств(
				...
				|Обеспечено,
				|ОбеспеченоЕдиниц,
				|ОбеспеченоНаКонтроле,
				|ОбеспеченоЕдиницНаКонтроле");
				
			НоваяВетвь.ОбеспеченоЕдиниц = НоваяВетвь.ОбеспеченоЕдиниц - НоваяВетвь.ОбеспеченоЕдиницНаКонтроле;
		...
		Иначе
			...
			НоваяВетвьСерии.ОбеспеченоНаКонтроле = Строка.ОбеспеченоЕдиницНаКонтроле - Строка.СерииКОтгрузкеНаКонтроле;
			НоваяВетвьСерии.ОбеспеченоЕдиниц = НоваяВетвьСерии.Обеспечено;
			НоваяВетвьСерии.ОбеспеченоЕдиницНаКонтроле = НоваяВетвьСерии.ОбеспеченоНаКонтроле;
			НоваяВетвьСерии.ЭтоЯчейки = Истина;
			
			НоваяВетвьСерии.ОбеспеченоЕдиниц = НоваяВетвьСерии.ОбеспеченоЕдиниц - НоваяВетвьСерии.ОбеспеченоЕдиницНаКонтроле;
	...
	Для Каждого НоваяВетвьСерии Из ВетвиСерийДляРасчетаОбеспечено Цикл
		...
		Для Каждого СтрокаЯчейки Из Ячейки Цикл
			СтрокаЯчейки.ОбеспеченоЕдиниц = СтрокаЯчейки.ОбеспеченоЕдиниц - СтрокаЯчейки.ОбеспеченоЕдиницНаКонтроле;
		КонецЦикла;
	...
	Для СчетчикТоваров = 0 По КоличествоТоваров - 1 Цикл
		...
		СПровереннымКачеством = СтрокаТовара.Обеспечено - Строка.СвободныйОстатокНаКонтроле;
		
		Серии = СтрокаТовара.ПолучитьЭлементы();
		КоличествоСерий = Серии.Количество();
		
		РаспределитьВТекущемПроходе = 0;
		
		Для НомерПрохода = 1 По 2 Цикл
			
			// Если вручную на шаге "Товары" помощника указали количества,
			// большее чем свободный остаток проверенного товара, то обязательно нужно задействовать непроверенный остаток.
			РаспределитьВТекущемПроходе = РаспределитьПоТовару - ?(НомерПрохода = 1, Макс(0, РаспределитьПоТовару - СПровереннымКачеством), 0);
			РаспределитьПоТовару = РаспределитьПоТовару - РаспределитьВТекущемПроходе;
			
			ПостфиксИмени = ?(НомерПрохода = 1, "", "НаКонтроле");
			ИмяПоля = "ОбеспеченоЕдиниц" + ПостфиксИмени;
			
			// Если серии не используются, то ветвь Серии по факту является ветвью ячеек.
			РаспределеноНаТоварныеМеста = РаспределитьНаТоварныеМеста(
				Макс(0, РаспределитьВТекущемПроходе), Серии, УпаковкиНоменклатуры, ПостфиксИмени);
			РаспределитьВТекущемПроходе = РаспределитьВТекущемПроходе - РаспределеноНаТоварныеМеста;
			
			Для СчетчикСерий = 0 По КоличествоСерий - 1 Цикл
				
				СтрокаСерии = Серии[СчетчикСерий];
				РаспределитьПоСерии = СтрокаСерии[ИмяПоля];
				
				Ячейки = СтрокаСерии.ПолучитьЭлементы();
				КоличествоЯчеек = Ячейки.Количество();
				РаспределеноНаТоварныеМеста = РаспределитьНаТоварныеМеста(
					Макс(0, Мин(РаспределитьВТекущемПроходе, РаспределитьПоСерии)), Ячейки, УпаковкиНоменклатуры, ПостфиксИмени);
				РаспределитьВТекущемПроходе = РаспределитьВТекущемПроходе - РаспределеноНаТоварныеМеста;
				...
				Для СчетчикЯчеек = 0 По КоличествоЯчеек - 1 Цикл
					...
					Если РаспределитьПоСерии > 0
							И (УпаковкиНоменклатуры = Неопределено
								Или УпаковкиНоменклатуры.Получить(СтрокаЯчейки.Упаковка) = Неопределено) Тогда
							
							РаспределитьПоЯчейке = СтрокаЯчейки[ИмяПоля];
							
							Распределить = Мин(РаспределитьВТекущемПроходе, РаспределитьПоСерии, РаспределитьПоЯчейке);
							СтрокаЯчейки.Количество = СтрокаЯчейки.Количество + Распределить;
							РаспределитьВТекущемПроходе = РаспределитьВТекущемПроходе - Распределить;
				...
				Если РаспределитьПоСерии > 0 И КоличествоЯчеек = 0 Тогда
					...
					Распределить = Мин(РаспределитьВТекущемПроходе, РаспределитьПоСерии);
					СтрокаСерии.Количество = СтрокаСерии.Количество + Распределить;
					РаспределитьВТекущемПроходе = РаспределитьВТекущемПроходе - Распределить;
					
				КонецЕсли;
				
				Если РаспределитьВТекущемПроходе = 0 Тогда
					Прервать;
				КонецЕсли;
	...
	Для Каждого СтрокаТовара Из ЭлементыДерева Цикл
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ТаблицаДляРасчетаКоличестваУпаковок, СтруктураДействий);
...
Функция Комплекты(Ячейки, УпаковкиНоменклатуры)
	...
	ТаблицаУпаковок.Колонки.Добавить("ОбеспеченоЕдиницНаКонтроле");
	...
	ТаблицаУпаковок.Свернуть("Упаковка", "ОбеспеченоЕдиниц,ОбеспеченоЕдиницНаКонтроле");
	ТаблицаУпаковок.Индексы.Добавить("Упаковка");
	КоличествоЦелыхКомплектов = 0;
	КоличествоЦелыхКомплектовНаКонтроле = 0;
	Если ТаблицаУпаковок.Количество() > 0 Тогда
		КоличествоЦелыхКомплектов = ТаблицаУпаковок[0].ОбеспеченоЕдиниц;
		КоличествоЦелыхКомплектовНаКонтроле = ТаблицаУпаковок[0].ОбеспеченоЕдиницНаКонтроле;
		Для Каждого Строка Из ТаблицаУпаковок Цикл
			...
			КоличествоЦелыхКомплектовНаКонтроле = Мин(КоличествоЦелыхКомплектовНаКонтроле, Строка.ОбеспеченоЕдиницНаКонтроле);
		КонецЦикла;
	КонецЕсли;
	Возврат Новый Структура("ТаблицаУпаковок,КоличествоЦелыхКомплектов,КоличествоЦелыхКомплектовНаКонтроле",
		ТаблицаУпаковок, КоличествоЦелыхКомплектов, КоличествоЦелыхКомплектовНаКонтроле);
	
КонецФункции

&НаСервере
Функция РаспределитьНаТоварныеМеста(РаспределитьВсего, Ячейки, УпаковкиНоменклатуры, ПостфиксИмени)
	...
	КоличествоЦелыхКомплектов = Комплекты["КоличествоЦелыхКомплектов" + ПостфиксИмени];
	ИмяПоля = "ОбеспеченоЕдиниц" + ПостфиксИмени;
	
	КоличествоЦелыхКомплектов = Мин(КоличествоЦелыхКомплектов, РаспределитьВсего);
	Для Каждого Строка Из ТаблицаУпаковок Цикл
		Строка[ИмяПоля] = КоличествоЦелыхКомплектов;
	КонецЦикла;
	...
	Для СчетчикЯчеек = 0 По КоличествоЯчеек - 1 Цикл
		...
		Если СтрокаУпаковки <> Неопределено Тогда
			Распределить = Макс(0, Мин(СтрокаУпаковки[ИмяПоля], СтрокаЯчейки[ИмяПоля]));
			СтрокаУпаковки[ИмяПоля] = СтрокаУпаковки[ИмяПоля] - Распределить;
...
Процедура РассчитатьОбеспеченоПоЯчейкамСерии(НоваяВетвьСерии, УпаковкиНоменклатуры)
	...
	Ячейки = НоваяВетвьСерии.ПолучитьЭлементы();
	ОбеспеченоЕдиниц = 0;
	ОбеспеченоЕдиницНаКонтроле = 0;
	Если УпаковкиНоменклатуры <> Неопределено Тогда
		Комплекты = Комплекты(Ячейки, УпаковкиНоменклатуры);
		ТаблицаУпаковок = Комплекты.ТаблицаУпаковок;
		ОбеспеченоЕдиниц = Комплекты.КоличествоЦелыхКомплектов;
		ОбеспеченоЕдиницНаКонтроле = Комплекты.КоличествоЦелыхКомплектовНаКонтроле;
	КонецЕсли;
	КоличествоЯчеек = Ячейки.Количество();
	Для СчетчикЯчеек = 0 По КоличествоЯчеек - 1 Цикл
		СтрокаЯчейки = Ячейки[СчетчикЯчеек];
		Если УпаковкиНоменклатуры = Неопределено Или ТаблицаУпаковок.Найти(СтрокаЯчейки.Упаковка, "Упаковка") = Неопределено Тогда
			...
			ОбеспеченоЕдиницНаКонтроле = ОбеспеченоЕдиницНаКонтроле + СтрокаЯчейки.ОбеспеченоЕдиницНаКонтроле;
		КонецЕсли;
	КонецЦикла;
	
	НоваяВетвьСерии.ОбеспеченоЕдиниц = ОбеспеченоЕдиниц;
	НоваяВетвьСерии.ОбеспеченоЕдиницНаКонтроле = ОбеспеченоЕдиницНаКонтроле;
	НоваяВетвьСерии.Обеспечено = ОбеспеченоЕдиниц;
	НоваяВетвьСерии.ОбеспеченоНаКонтроле = ОбеспеченоЕдиницНаКонтроле;
...
Процедура РассчитатьИтогиПоЗаказу()
	...
	Если ИтогиОбеспечено < 0 Или ИтогиОбеспечено > ИтогиПотребность Тогда
		Элементы.СтраницыИтогиОбеспечено.ТекущаяСтраница = Элементы.СтраницаИтогиОбеспеченоПревышение;
	Иначе
		Элементы.СтраницыИтогиОбеспечено.ТекущаяСтраница = Элементы.СтраницаИтогиОбеспечено;
	КонецЕсли;
	Если ИтогиНаСкладе < 0 Или ИтогиНаСкладе > ИтогиПотребность Тогда
		Элементы.СтраницыИтогиНаСкладе.ТекущаяСтраница = Элементы.СтраницаИтогиНаСкладеПревышение;
	Иначе
		Элементы.СтраницыИтогиНаСкладе.ТекущаяСтраница = Элементы.СтраницаИтогиНаСкладе;
	КонецЕсли;
	
	Если ИтогиОбеспеченоСводно < 0 Или ИтогиОбеспеченоСводно > ИтогиПотребностьСводно Тогда
		Элементы.СтраницыИтогиОбеспеченоСводно.ТекущаяСтраница = Элементы.СтраницаИтогиОбеспеченоСводноПревышение;
	Иначе
		Элементы.СтраницыИтогиОбеспеченоСводно.ТекущаяСтраница = Элементы.СтраницаИтогиОбеспеченоСводно;
	КонецЕсли;
	Если ИтогиНаСкладеСводно < 0 Или ИтогиНаСкладеСводно > ИтогиПотребностьСводно Тогда
		Элементы.СтраницыИтогиНаСкладеСводно.ТекущаяСтраница = Элементы.СтраницаИтогиНаСкладеСводноПревышение;
	Иначе
		Элементы.СтраницыИтогиНаСкладеСводно.ТекущаяСтраница = Элементы.СтраницаИтогиНаСкладеСводно;
	КонецЕсли;
	...
	Если Строка.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийКорректировкиНазначения.Резервировать") Тогда
		...
		КонецЕсли;
		ИтогиНазначениеИсточник = СтрШаблон(
			Текст,
			СтрокаИтоговИсточника.ИсходноеНазначение,
			Формат(СтрокаИтоговИсточника.Потребность, "ЧДЦ=3; ЧН=0,000"),
			Формат(СтрокаИтоговИсточника.Запас, "ЧДЦ=3; ЧН=0,000"),
			ИтогиУпаковка);
