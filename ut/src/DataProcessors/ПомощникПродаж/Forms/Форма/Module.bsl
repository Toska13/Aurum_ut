...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОперацияРеализацияКлиенту = Перечисления.ХозяйственныеОперации.РеализацияКлиенту;
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Иначе
		...
		Если Объект.УпрощенноеОбеспечение И ИспользоватьСоглашенияСКлиентами Тогда
			...
			ВариантИФлаг = Новый Структура("ВариантОбеспечения,Обособленно", Объект.ВариантОбеспечения, Неопределено);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	ЭтоОперацияПередачи = ЭтоОперацияПередачи(Объект.ХозяйственнаяОперация)
							ИЛИ (ЭтоОперацияОтгрузкиТоваров(Объект.ХозяйственнаяОперация)
								И (Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.ПередачаТоваровХранителю
...
#Область ПроверкиТабличнойЧастиТовары
	...
	Для ТекИндекс = 0 По Объект.Товары.Количество()-1 Цикл
		...
		Если ТекущаяСтрока.СкладОбязателен
			...
			ТекстСообщения = НСтр("ru='Не заполнена колонка ""%1""%2'");
			
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(ТекстСообщения, Элементы.ТоварыГруппаОтправитель.Заголовок, АдресОшибки),
				,
...
#Область ПроверкиРеквизитовОплатыИОтгрузкиИТабличнойЧастиЭтапыоплаты
	...
	Если ВариантСозданияНеКоммерческоеПредложение
		...
		Если Не ЗаданГрафикИсполнения
			Или Объект.ПорядокРасчетов <> Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов Тогда
	...
	Если ИспользоватьСоглашенияСКлиентами
		И ЭтоОперацияПередачи
		И ЗначениеЗаполнено(Объект.Соглашение)
		И ЗначениеЗаполнено(Объект.ХозяйственнаяОперация)
		И ЭтоОперацияОтгрузкиТоваров(Объект.ХозяйственнаяОперация) Тогда
		
		РеквизитыСоглашения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Соглашение,
			"ВозможнаРеализацияБезПереходаПраваСобственности, Представление");
	
		Если Не РеквизитыСоглашения.ВозможнаРеализацияБезПереходаПраваСобственности Тогда
			
			ТекстОшибки = НСтр("ru = 'По соглашению ""%1"" невозможна отгрузка товаров без перехода права собственности.'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, РеквизитыСоглашения.Представление);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени("Обработка.ПомощникПродаж.Форма.Форма.Событие.ОбработкаВыбора");
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Обработка.ПомощникПродаж.Форма.РеквизитыПечатиАкта" Тогда
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "ОбщаяФорма.РеквизитыПечатиРеализации" Тогда
		...
		Если ВыбранноеЗначение <> Неопределено Тогда
			...
			Если ЗаполнитьУстановитьРеквизитыДоставки Тогда
				ЗаполнитьРеквизитыДоставки("Грузополучатель");
			КонецЕсли;
...
Процедура СоглашениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТолькоИндивидуальные
		...
		Если ЭтоПередачаТоваров(Объект.ВариантОформленияДокументов) Тогда
			...
			Если ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5 Тогда
				ПараметрыВыбораСоглашения.ОперацииПередачиОтгрузки = Истина;
			КонецЕсли;
		КонецЕсли;
	...
	Иначе
		...
		Если ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5 И ЭтоОперацияОтгрузкиТоваров(Объект.ХозяйственнаяОперация) Тогда
			ПараметрыОткрытия.Вставить("ОтгрузкаБезПереходаПраваСобственности25", Истина);
...
Процедура СоглашениеПриИзменении(Элемент)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.ЗамерВремени("Обработка.ПомощникПродаж.Форма.Форма.СоглашениеСКлиентом.ПриИзменении");
...
Процедура ПартнерПриИзменении(Элемент)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.ЗамерВремени("Обработка.ПомощникПродаж.Форма.Элемент.Партнер.ПриИзменении");
...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.ЗамерВремени("Обработка.ПомощникПродаж.Форма.Форма.Элементы.Товары.Выбор");
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	СтруктураЗаполненияСтавкиНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДСПоВидуЦен(Объект);
	СтруктураЗаполненияСтавкиНДС.НалогообложениеНДС = ПараметрыЗаполнитьСтавкуНДС.НалогообложениеНДС;
	СтруктураДействий.Вставить("ЗаполнитьСтавкуНДСПоВидуЦен", СтруктураЗаполненияСтавкиНДС);
...
Процедура ОформленныеДокументыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Элемент.ТекущийЭлемент = Элементы.ОформленныеДокументыПечать Тогда
		...
		Если Элементы.ОформленныеДокументы.ТекущиеДанные <> Неопределено И Элементы.ОформленныеДокументы.ТекущиеДанные.Печать Тогда
			...
			ОткрытьФорму("РегистрСведений.НастройкиПечатиОбъектов.Форма.НастройкаПечатиКомплекта", ПараметрыФормы, ЭтотОбъект);
...
Процедура НоваяПродажа(Команда)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.ЗамерВремени("Обработка.ПомощникПродаж.Форма.Команда.НоваяПродажа");
	
	Если Модифицированность Тогда
		
		ОписаниеОповещения = Новый ОписаниеОповещения("НоваяПродажаЗавершение", ЭтотОбъект);
		ТекстВопроса = НСтр("ru = 'Все несохраненные данные будут потеряны. Начать оформление продажи заново?'");
		
		СписокКнопок = Новый СписокЗначений();
		СписокКнопок.Добавить("Начать", НСтр("ru = 'Начать новую продажу'"));
		СписокКнопок.Добавить("НеНачинать", НСтр("ru = 'Отмена'"));
		
		ПоказатьВопрос(ОписаниеОповещения,
			ТекстВопроса,
			СписокКнопок,,
			"НеНачинать",
			НСтр("ru = 'Оформление новой продажи'"), );
...
Процедура НапечататьДокументы(Команда)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.ЗамерВремени("Обработка.ПомощникПродаж.Форма.Команда.НапечататьДокументы");
...
Процедура ОткрытьПодбор(Команда)
	...
	Если ЭтоОперацияПередачи(Объект.ХозяйственнаяОперация) Тогда
		...
		Если Объект.ВариантОформленияДокументов = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.КоммерческоеПредложение")
			...
			Или Объект.ВариантОформленияДокументов = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.ПередачаТоваровХранителю") Тогда
				
			ОтборПоТипуНоменклатуры = НоменклатураКлиентСервер.ОтборПоТоваруМногооборотнойТаре(Истина, Ложь);
			ПараметрыФормы.Вставить("ОтборПоТипуНоменклатуры", Новый ФиксированныйМассив(ОтборПоТипуНоменклатуры));
			
		КонецЕсли;
		ПараметрыФормы.Вставить("СкрыватьРучныеСкидки",Истина);
		
	ИначеЕсли ЭтоОперацияОтгрузкиТоваров(Объект.ХозяйственнаяОперация) Тогда
		ОтборПоТипуНоменклатуры = НоменклатураКлиентСервер.ОтборПоТоваруМногооборотнойТаре(Истина, Ложь);
		ПараметрыФормы.Вставить("ОтборПоТипуНоменклатуры", Новый ФиксированныйМассив(ОтборПоТипуНоменклатуры));
		ПараметрыФормы.Вставить("СкрыватьРучныеСкидки", Истина);
	КонецЕсли;

	ОперативноеЗаполнение = (Объект.ДатаОтгрузки = НачалоДня(ОбщегоНазначенияКлиент.ДатаСеанса())	// Если оперативное заполнение
		Или Не ЗначениеЗаполнено(Объект.ДатаОтгрузки));

	// Варианты оформления документов, для которых принудительно ставится вариант обеспечения "Отгрузить"
	ОграниченноеОбеспечение = Объект.ВариантОформленияДокументов <> ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.ЗаказКлиента")
		И Объект.ВариантОформленияДокументов <> ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.ЗаказКлиентаПередачаТоваровХранителю")
		И Объект.ВариантОформленияДокументов <> ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.ЗаказКлиентаРеализацияТоваровУслуг");
	
	// Аналогично интерактивной работе:
	УстановитьОтгрузить = Объект.НеОтгружатьЧастями
		И (Объект.ВариантОформленияДокументов = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.ЗаказКлиентаПередачаТоваровХранителю")
	...
	Если (ОграниченноеОбеспечение Или Объект.НеОтгружатьЧастями И ОперативноеЗаполнение И ИспользоватьСоглашенияСКлиентами)
		Или УстановитьОтгрузить Тогда
		
		ВариантИФлагДляПодбора = ПолучитьВариантОбеспеченияИФлагОбособленно(
			ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.Отгрузить"),
			Объект.Соглашение,
			Объект.СтатусЗаказаКлиента);
		ОграничиватьВариантыОбеспечения = Истина;
	ИначеЕсли Объект.УпрощенноеОбеспечение Тогда
		ВариантИФлагДляПодбора = ПолучитьВариантОбеспеченияИФлагОбособленно(
		...
		ОграничиватьВариантыОбеспечения = Истина;
	Иначе
		ОграничиватьВариантыОбеспечения = Ложь;
		ВариантИФлагДляПодбора = ВариантОбеспеченияИФлагОбособленно;
	КонецЕсли;
	...
	ПараметрыФормы.Вставить("ПодборВариантовОбеспечения", Не ОграничиватьВариантыОбеспечения);
	ПараметрыФормы.Вставить("ВариантОбеспеченияИФлагОбособленно", ВариантИФлагДляПодбора);
...
Процедура ДополнительныеРеквизитыПередачаТоваровХранителю(Команда)
	...
	АдресВоВременномХранилище = ПоместитьДополнительныеРеквизитыВоВременноеХранилище("ДополнительныеРеквизитыПередачиТоваров");
...
&НаКлиенте
Процедура ДополнительныеРеквизитыОтгрузкаТоваровКлиенту(Команда)
	
	ВариантОткрытия           = Не Модифицированность
								И ДокументыСформированы;
	АдресВоВременномХранилище = ПоместитьДополнительныеРеквизитыВоВременноеХранилище("ДополнительныеРеквизитыОтгрузкиТоваров");
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ТолькоПросмотр",            ВариантОткрытия);
	ПараметрыФормы.Вставить("ТипДокумента",              "ДокументОбъект.ОтгрузкаТоваровКлиенту");
	ПараметрыФормы.Вставить("АдресВоВременномХранилище", АдресВоВременномХранилище);
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<необходимо заполнить номенклатуру>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение, СписокСвойств = Неопределено)
	...
	ПоляПоиска = "НоменклатураНабора, ХарактеристикаНабора, Номенклатура, Характеристика, Упаковка, ВидЦены, 
		|СрокПоставки, ВариантОбеспечения, Серия, Обособленно";
	Если Не ЗаполнитьСклады Тогда
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТовара, "НоменклатураНабора")
			И ЗначениеЗаполнено(СтрокаТовара.НоменклатураНабора) Тогда
...
Процедура УстановитьПараметрыВыбораСоглашения()
	...
	Если ЭтоРеализация(Объект.ВариантОформленияДокументов) Тогда
		...
		Если Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.ЗаказКлиентаРеализацияТоваровУслуг
			...
			ПараметрВыбора = Новый ПараметрВыбора("Отбор.КомиссионныеПродажи25", Ложь);
			НовыеПараметрыВыбора.Добавить(ПараметрВыбора);
		КонецЕсли
	...
	Если ЭтоПередачаТоваров(Объект.ВариантОформленияДокументов) Тогда
		...
		Если ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5 Тогда
			ПараметрВыбора = Новый ПараметрВыбора("ОтгрузкаБезПереходаПраваСобственности25", Истина);
			НовыеПараметрыВыбора.Добавить(ПараметрВыбора);
		КонецЕсли;
...
Процедура УстановитьСвязиПараметровВыбораДоговора()
	...
	Для Каждого СвязьПараметровВыбора Из ЭлементФормы.СвязиПараметровВыбора Цикл
		...
		Иначе
			ПутьКДаннымХО = СвязьПараметровВыбора.ПутьКДанным;
		КонецЕсли;
	КонецЦикла;
	
	ПутьКДанным = "Объект.ХозяйственнаяОперация";
	Если ЭтоОперацияОтгрузкиТоваров(Объект.ХозяйственнаяОперация) Тогда
		ПутьКДанным = "ОперацияРеализацияКлиенту";
	КонецЕсли;
	
	Если ПутьКДаннымХО = ПутьКДанным Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСтатусыУказанияСерий()
	...
	Если ЭтоРеализация(Объект.ВариантОформленияДокументов)
		...
		Если ЭтоЗаказСРеализациейПередачей(Объект.ВариантОформленияДокументов) Тогда
			ЗаполнитьСтатусыУказанияСерийЗаказСРеализациейПередачей(ПараметрыУказанияСерий);
		Иначе
...
&НаСервере
Процедура ЗаполнитьСтатусыУказанияСерийЗаказСРеализациейПередачей(ПараметрыУказанияСерий)
	
	// Обнуление статусов указания серий.
	НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(Объект, ПараметрыУказанияСерий.Заказ);
	
	// Перезаполнение статусов для строк заказа с реализацией/передачей(отгрузкой).
	СтрокиТоваровРеализацияПередача = Новый Массив;
	Для Каждого ТекСтрока Из Объект.Товары Цикл
		Если ТекСтрока.ВариантОформления = Перечисления.ВариантыОформленияДокументовПродажи.ЗаказКлиента Тогда
			Продолжить;
		КонецЕсли;
		СтрокиТоваровРеализацияПередача.Добавить(ТекСтрока);
	КонецЦикла;
	
	Если СтрокиТоваровРеализацияПередача.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(Объект, ПараметрыУказанияСерий.Реализация,
		СтрокиТоваровРеализацияПередача);
...
&НаСервереБезКонтекста
Функция ЭтоЗаказСРеализациейПередачей(ВариантОформления)
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	СтруктураЗаполненияСтавкиНДС = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДСПоВидуЦен(Объект);
	СтруктураЗаполненияСтавкиНДС.НалогообложениеНДС = ПараметрыЗаполнитьСтавкуНДС.НалогообложениеНДС;
	СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьСтавкуНДСПоВидуЦен", СтруктураЗаполненияСтавкиНДС);
	...
	Если ЭтоРеализация(Объект.ВариантОформленияДокументов) Или ЭтоПередачаТоваров(Объект.ВариантОформленияДокументов) Тогда
		
		СтруктураДействийСДобавленнымиСтроками.Вставить("ПроверитьСериюРассчитатьСтатус",
			Новый Структура("Склад, ПараметрыУказанияСерий", Объект.Склад, ПараметрыУказанияСерий.Реализация));
		
	ИначеЕсли Объект.ВариантОформленияДокументов = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.ЗаказКлиента") Тогда
		
		СтруктураДействийСДобавленнымиСтроками.Вставить("ПроверитьСериюРассчитатьСтатус",
			Новый Структура("Склад, ПараметрыУказанияСерий", Объект.Склад, ПараметрыУказанияСерий.Заказ));
...
Функция ЗаписатьЗаказ(МассивНайденныхДокументов, Отказ, БезПроведения)
	...
	Если ЭтоОперацияОтгрузкиТоваров(ДокументОбъект.ХозяйственнаяОперация) Тогда
		ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту;
	КонецЕсли;
	
	ОтветственныеЛицаСервер.ОчиститьОтветственногоВДокументе(ДокументОбъект);
...
//
&НаСервере
Функция ЗаписатьЗаявкуНаВозврат(МассивНайденныхДокументов, Товары, Склад, Отказ, БезПроведения = Ложь)
	...
	ДокументОбъект.ВариантПриемкиТоваров = Константы.ВариантПриемкиТоваров.Получить();
	...
	Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтКомиссионера Тогда
		...
		ДокументОбъект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПустаяСсылка();
	...
	Если (ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтХранителя 
		ИЛИ Объект.КомиссионныеПродажи25) 
		И ЗначениеЗаполнено(Склад)
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Склад, "ЭтоГруппа") Тогда
	...
	Для Каждого ТекСтрока Из Товары Цикл
		Если ТекСтрока.КоличествоУпаковокКВозврату > 0 Тогда
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ДокументОбъект.ВозвращаемыеТовары, СтруктураДействий, Неопределено);
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ДокументОбъект.Склад) 
			И Не БезПроведения Тогда
			ДокументОбъект.Записать(РежимЗаписиДокумента.Проведение);
		Иначе
			ДокументОбъект.Записать(РежимЗаписиДокумента.Запись);
		КонецЕсли;	
...
// Создает или обновляет документ ПередачаТоваровХранителю.
//
// Параметры:
...
//
&НаСервере
Функция ЗаписатьПередачуТоваровХранителю(МассивНайденныхДокументов, Товары, Отказ)
...
Функция ЗаписатьРеализациюПоЗаказу(МассивНайденныхДокументов, ОснованиеОбъект, Товары, Склад, Отказ, БезПроведения = Ложь, БезСменыСтатусаЗаказа = Ложь)
	...
	ЗаполнитьРеквизитыДоставкиПоОснованию(ДокументОбъект, ОснованиеОбъект.Ссылка);
...
Процедура СоздатьСчетФактуру(МассивНайденныхДокументов, ОснованиеОбъект, Отказ)
	...
	Если ЗначениеЗаполнено(ИмяДокумента) Тогда
		...
		ПараметрыРегистрации.ПереходПраваСобственности = Ложь;
		ПараметрыРегистрации.НачислениеНДСПриОтгрузке = Ложь;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ОснованиеОбъект, "ХозяйственнаяОперация") = Истина Тогда
			ПараметрыРегистрации.ПереходПраваСобственности = (ОснованиеОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности);
			ПараметрыРегистрации.НачислениеНДСПриОтгрузке = (ОснованиеОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтгрузкаТоваровКлиенту);
		КонецЕсли;
...
// Создает или обновляет документ ОтгрузкаТоваровКлиенту.
//
// Параметры:
// 	МассивНайденныхДокументов - Массив Из ДокументСсылка
// 	Товары - ТаблицаЗначений
...
//
&НаСервере
Функция ЗаписатьОтгрузкуТоваровКлиенту(МассивНайденныхДокументов, Товары, Отказ)
	...
	ДокументОбъект = Продажи.НайтиСоздатьДокумент(ЭтотОбъект,
		...
		Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту"),
		МассивНайденныхДокументов,
		СозданНовыйДокумент,
		СтрокаТаблицы); // ДокументОбъект.ОтгрузкаТоваровКлиенту
	...
	ДокументОбъект.Серии.Загрузить(Объект.Серии.Выгрузить());
	ДокументОбъект.ДополнительныеРеквизиты.Загрузить(Объект.ДополнительныеРеквизитыОтгрузкиТоваров.Выгрузить());
	
	ПараметрыУказанияСерий = НоменклатураСервер.ПараметрыУказанияСерий(ДокументОбъект,
		Документы.ОтгрузкаТоваровКлиенту);
...
//
&НаСервере
Функция ЗаписатьОтгрузкуТоваровКлиентуПоЗаказу(МассивНайденныхДокументов, ОснованиеОбъект, Товары, Склад, Отказ)
	...
	ДокументОбъект.ОтгрузкаПоЗаказам         = Истина;
	ДокументОбъект.ЗаказКлиента              = ОснованиеОбъект.Ссылка;
	ДокументОбъект.Склад                     = Склад;
	ДокументОбъект.СпособДоставки            = Перечисления.СпособыДоставки.Самовывоз;
	ДокументОбъект.БанковскийСчетОрганизации = Объект.БанковскийСчет;
	...
	Для Каждого СтрокаТовары Из Товары Цикл
		...
		НоваяСтрока.ЗаказКлиента = ДокументОбъект.ЗаказКлиента;
		НоваяСтрока.Сумма = СтрокаТовары.СуммаСНДС;
...
// Инициализация
	...
	// СозданиеПередачаТоваровХранителю
				...
				ИначеЕсли Не Отказ
					...
					|	И ВЫБОР
					|		КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтгрузкаТоваровКлиенту)
					|			ТОГДА Товары.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар)
					|		ИНАЧЕ Товары.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),
					...
					Если Не РезультатЗапроса.Пустой() Тогда
						...
						Если ЭтоОперацияОтгрузкиТоваров(Объект.ХозяйственнаяОперация) Тогда
							...
							ОснованиеОбъект = ЗаписатьОтгрузкуТоваровКлиенту(МассивНайденныхДокументов, Товары, Отказ);
							
							Если Не Отказ
								И Объект.ПечататьСчетФактуру Тогда
								СоздатьСчетФактуру(МассивНайденныхДокументов, ОснованиеОбъект, Отказ);
							КонецЕсли;
							
						Иначе
							ОснованиеОбъект = ЗаписатьПередачуТоваровХранителю(МассивНайденныхДокументов, Товары, Отказ);
						КонецЕсли;
	...
	// СозданиеПередачаТоваровХранителюПоЗаказу
				...
				ИначеЕсли Не Отказ
					...
					|	ИЛИ НЕ &ИспользоватьРасширенныеВозможностиЗаказаКлиента)
					|	И
					|		Товары.ВариантОформления = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПродажи.ЗаказКлиентаПередачаТоваровХранителю)
					|	И ВЫБОР
					|		КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтгрузкаТоваровКлиенту)
					|			ТОГДА Товары.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар)
					|		ИНАЧЕ Товары.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),
					...
					Если Не РезультатЗапроса.Пустой() Тогда
						...
						Если ЭтоОперацияОтгрузкиТоваров(Объект.ХозяйственнаяОперация) Тогда
							...
							СоздатьОтгрузкуТоваровКлиентуПоЗаказу(Товары,
								МассивДокументовПродажи,
								МассивНайденныхДокументов,
								ОснованиеОбъект,
								Отказ);
						...
						Иначе
							...
							//@skip-check query-in-loop
							СоздатьПередачуТоваровПоЗаказу(Товары,
								МассивДокументовПродажи,
								МассивНайденныхДокументов,
								ОснованиеОбъект,
								Отказ);
	...
	// СозданиеЗаявкиНаВозвратТоваровОтКлиента
			...
			Если Не Отказ
				...
				И Объект.Товары.Итог("КоличествоУпаковокКВозврату") > 0
				И Не ЭтоОперацияОтгрузкиТоваров(Объект.ХозяйственнаяОперация) Тогда
				
				 ТаблицаТоваровВКорзине = Объект.Товары.Выгрузить();
				 Товары = ТаблицаТоваровВКорзине.СкопироватьКолонки();
				 Для Каждого ТекущаяСтрокаТаблицыТоваров Из ТаблицаТоваровВКорзине Цикл
					 Если ТекущаяСтрокаТаблицыТоваров.КоличествоУпаковокКВозврату = 0 Тогда
						 Продолжить;
					 КонецЕсли; 	 
					 
					 НоваяСтрока = Товары.Добавить();
					 ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущаяСтрокаТаблицыТоваров);
					 
				 КонецЦикла;
				
				СоздатьЗаявкуНаВозврат(Товары,
						МассивДокументовПродажи,
						МассивНайденныхДокументов,
						БезПроведения,
						Отказ);
...
Процедура СоздатьДокументыКлиент(Знач Оповещение)
	...
	Если Объект.ХозяйственнаяОперация =
		...
		СоздатьДокументыКлиентПредложитьПодобратьМногооборотнуюТаруЗавершение(Неопределено, ДополнительныеПараметры);
...
&НаКлиенте
Процедура СоздатьДокументыКлиентПредложитьПодобратьМногооборотнуюТаруЗавершение(Значение, ДополнительныеПараметры) Экспорт

	Если ИспользоватьАвтоматическиеСкидкиВПродажах
		И Не ЭтоОперацияПередачи(Объект.ХозяйственнаяОперация) Тогда
		
		ЭтоОперацияОтгрузкиБезЗаказа =
			Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ОтгрузкаТоваровКлиенту")
				И Объект.ВариантОформленияДокументов =
					ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.ПередачаТоваровХранителю");
		
		Если СкидкиИзменились()
			И Не Объект.СкидкиРассчитаны
			И Не ЭтоОперацияОтгрузкиБезЗаказа Тогда
...
Функция УстановитьПометкуУдаленияСервер(ВыделенныеДокументы)
	...
	Счетчик = ВыделенныеДокументы.Количество();
	...
	Попытка
		...
		Пока Счетчик > 0 Цикл
			...
			СтрокаТаблицы = Объект.Документы.НайтиПоИдентификатору(ВыделенныеДокументы[Счетчик]);
...
Процедура ОбновитьТекстИнформационнойНадписиФормируемыеДокументы()
	...
	ИначеЕсли ЭтоПередачаТоваров(Объект.ВариантОформленияДокументов) Тогда
		...
		Если Объект.Товары.Количество() = 0 Тогда
			...
			Иначе
				ИмяПараметраДокумент = НСтр("ru='Передача'");
				Если ЭтоОперацияОтгрузкиТоваров(Объект.ХозяйственнаяОперация) Тогда
					ИмяПараметраДокумент = НСтр("ru='Отгрузка'");
				КонецЕсли;
				ДобавитьТекстНадписиВМассив(МассивНадписей, ИмяПараметраДокумент, "%СуммаПередачи%", СуммыДокументов.СуммаЗаказа, Ложь);
			КонецЕсли;
		КонецЕсли;
		...
		Для Каждого ТекСтрока Из Объект.Товары Цикл
			...
			Иначе
				...
				СуммыДокументов.СуммаПередачи = СуммыДокументов.СуммаПередачи
					+ ТекСтрока.СуммаСНДСБезВозвратнойТары;
		...
		Если ТребуетсяОформлениеПередачи Тогда
			...
			Если ЭтоОперацияОтгрузкиТоваров(Объект.ХозяйственнаяОперация) Тогда
				ИмяПараметраДокумент = НСтр("ru='Отгрузка'");
			КонецЕсли;
			ДобавитьТекстНадписиВМассив(МассивНадписей, ИмяПараметраДокумент, "%СуммаПередачи%", СуммыДокументов.СуммаПередачи, Ложь);
		КонецЕсли;
	...
	ЭтапыОплатыСервер.ПроверитьЗаполнитьЭтапыГрафикаОплаты(ЭтотОбъект);
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоОперацияОтгрузкиТоваров(ХозяйственнаяОперация)
	
	Возврат ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ОтгрузкаТоваровКлиенту");
...
Функция ДоговорПоУмолчанию()
	...
	ОбъектОперация = Объект.ХозяйственнаяОперация;
	Если ЭтоОперацияОтгрузкиТоваров(ОбъектОперация) Тогда
		ОбъектОперация = ОперацияРеализацияКлиенту;
	КонецЕсли;
...
Процедура ЗаполнитьУсловияПродажПоУмолчанию()
	...
	ИначеЕсли Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.ПередачаТоваровХранителю
		...
		Если ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5 Тогда
			ПараметрОтбораХозяйственныеОперации.Добавить(ОперацияРеализацияКлиенту);
	...
	Если УсловияПродажПоУмолчанию <> Неопределено Тогда
		...
		Если	ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5
				И ЗначениеЗаполнено(УсловияПродажПоУмолчанию.Соглашение)
				И УсловияПродажПоУмолчанию.ВозможнаРеализацияБезПереходаПраваСобственности
				И Не ЭтоРеализация(Объект.ВариантОформленияДокументов) Тогда
			Объект.ХозяйственнаяОперация =  Перечисления.ХозяйственныеОперации.ОтгрузкаТоваровКлиенту;
		КонецЕсли;
...
Процедура ЗаполнитьУсловияПродажПоСоглашению()
	...
	Если ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5
		И УсловияПродаж.ВозможнаРеализацияБезПереходаПраваСобственности
		И Не ЭтоРеализация(Объект.ВариантОформленияДокументов)
		И Не Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.ЗаказКлиента Тогда
	...
	Объект.БанковскийСчет = ДенежныеСредстваСервер.ПолучитьБанковскийСчетОрганизацииПоУмолчанию(ПараметрыСчета,
...
Процедура УстановитьВидимостьРеквизитовОплаты()
	...
	Если Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.КоммерческоеПредложение
		Или ЭтоОперацияПередачи(Объект.ХозяйственнаяОперация)
		Или ЭтоОперацияОтгрузкиТоваров(Объект.ХозяйственнаяОперация) Тогда
...
Процедура ЗаполнитьРеквизитыДоставки(ИмяЭлементаФормы)
	...
	Если ИмяЭлементаФормы = "Грузополучатель" Тогда
...
&НаСервере
Процедура ЗаполнитьРеквизитыДоставкиПоОснованию(ДокументОбъект, ДокументОснование)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьРеализациюПоНесколькимЗаказам") Тогда
		...
		ПродажиВызовСервера.СформироватьДанныеЗаполненияРеализации(МассивСсылок, ДокументОбъект.Метаданные().Имя, РеквизитыШапки);
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(РеквизитыШапки, РеквизитыДоставки, Ложь);
	...
	Иначе
		...
		Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПередачаТоваровХранителю") Тогда
			Обработчик = Документы.ПередачаТоваровХранителю.ОбработчикДействийПоТипуЗаказа(ТипЗнч(ДокументОснование));
		...
		ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РеализацияТоваровУслуг") Тогда
			...
			Запрос.УстановитьПараметр("ВводитьНаОснованииНепроведенного", Ложь);
		Иначе
			Возврат;
		КонецЕсли;
		
		Запрос.Текст = Обработчик.ТекстЗапросаЗаполненияНаОснованииЗаказа();
	...
	ДоставкаТоваров.ДополнитьРеквизитамиДоставкиДанныеЗаполнения(РеквизитыДоставки, РеквизитыШапки);
	
	ЗаполнитьЗначенияСвойств(ДокументОбъект, РеквизитыДоставки);
...
Процедура ИнициализироватьПомощникПродаж()
	...
	Объект.ДополнительныеРеквизитыОтгрузкиТоваров.Очистить();	
	ВидыЦен.Очистить();
...
Процедура ПартнерПриИзмененииСервер()
	...
	МассивРеквизитов.Добавить("Партнер");
	ИзмененныеРеквизиты = ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтотОбъект, МассивРеквизитов);
...
Процедура УстановитьВидимостьЭлементовПоОперацииСервер()
	...
	ЭтоОперацияОтгрузкиТоваров = ЭтоОперацияОтгрузкиТоваров(Объект.ХозяйственнаяОперация);
	
	ЭтоОперацияОтгрузкиПоЗаказу = ЭтоОперацияОтгрузкиТоваров
		И Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.ЗаказКлиентаПередачаТоваровХранителю;
	
	ЭтоОперацияОтгрузкиБезЗаказа = ЭтоОперацияОтгрузкиТоваров
		И Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.ПередачаТоваровХранителю;
	
	Если ЭтоОперацияОтгрузкиПоЗаказу Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивРеквизитовОперации, МассивВсехРеквизитов);
	КонецЕсли;
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(Элементы,
		...
		Не (ЭтоОперацияПередачи
			Или ЭтоОперацияОтгрузкиБезЗаказа));
	
	МассивЭлементов = Новый Массив;
	МассивЭлементов.Добавить("ДополнитьМногооборотнойТарой");
	МассивЭлементов.Добавить("ГруппаОплатаТара");
	
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(Элементы,
		МассивЭлементов,
		"Видимость",
		Не ЭтоОперацияОтгрузкиТоваров);
	
	Если ЭтоПередачаТоваров(Объект.ВариантОформленияДокументов) Тогда
		
		МассивЭлементов = Новый Массив;
		МассивЭлементов.Добавить("ДополнительныеРеквизитыОтгрузкаТоваровКлиенту");
		МассивЭлементов.Добавить("ДополнительныеРеквизитыОтгрузкаТоваровКлиенту1");
		ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(Элементы,
			МассивЭлементов,
			"Видимость",
			ЭтоОперацияОтгрузкиТоваров);
		
		МассивЭлементов = Новый Массив;
		МассивЭлементов.Добавить("ДополнительныеРеквизитыПередачаТоваровХранителю");
		МассивЭлементов.Добавить("ДополнительныеРеквизитыПередачаТоваровХранителю1");
		ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(Элементы,
			МассивЭлементов,
			"Видимость",
			ЭтоОперацияОтгрузкиТоваров);
	...
	УстановитьДоступностьДоговора();
	УстановитьВидимостьКомандыЗаполненияВидаЦенПоДоговору();
	УстановитьВидимостьТоварыКоличествоУпаковокКВозврату();
	
КонецПроцедуры

&НаСервере
Процедура УстановитьВидимостьТоварыКоличествоУпаковокКВозврату()
	
	Если Объект.СоздаватьЗаявкуНаВозвратТоваровОтКлиентов Тогда
		
		ЭтоОперацияОтгрузкиТоваров = ЭтоОперацияОтгрузкиТоваров(Объект.ХозяйственнаяОперация);
		Элементы.ТоварыКоличествоУпаковокКВозврату.Видимость = НЕ ЭтоОперацияОтгрузкиТоваров;
		
		Если ЭтоОперацияОтгрузкиТоваров И Объект.Товары.Итог("КоличествоУпаковокКВозврату") > 0 Тогда
			Для Каждого ТекСтрока Из Объект.Товары Цикл
				ТекСтрока.КоличествоУпаковокКВозврату = 0;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПересчитатьКоличествоУпаковокОбъемПредыдущегоЗаказа(ТекущаяСтрока, КэшированныеЗначения)
	...
	Иначе
		Коэффициент = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьКоэффициентУпаковки(ТекущаяСтрока.Упаковка,
																						КэшированныеЗначения,
...
Процедура ПересчитатьКоличествоУпаковокКВозврату(ИдентификаторТекущейСтроки, КэшированныеЗначения)
	...
	Если ТекущаяСтрока["КоличествоКВозврату"] = 0 Тогда
		ТекущаяСтрока["КоличествоУпаковокКВозврату"] = 0;
	Иначе
		Коэффициент = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьКоэффициентУпаковки(ТекущаяСтрока.Упаковка, КэшированныеЗначения, ТекущаяСтрока.Номенклатура).Коэффициент;
		Если Коэффициент <> 0 Тогда
...
Процедура ДоговорПриИзмененииСервер()
	...
	ИзмененныеРеквизиты = ВзаиморасчетыРаботаСФормами.ФормаПриИзмененииРеквизитов(ЭтотОбъект, "Договор");
	Если ИзмененныеРеквизиты.Свойство("ПорядокРасчетов") Тогда
		УстановитьСвойстваЭлементовПоПорядкуРасчетов();
	КонецЕсли;
...
Процедура ОткрытьРеквизитыПечатиРеализации()
	...
	СтруктураПараметров.Вставить("Ссылка",                         СсылкаПоВариантуОформления(Объект.ВариантОформленияДокументов));
...
Процедура УправлениеПометкойКомандыТранспортнойНакладной()
	...
	|ИЗ
	|	#Документ КАК ТаблицаДокумента
	|ГДЕ
	|	ТаблицаДокумента.Ссылка В(&МассивДокументов)
	|	И ТаблицаДокумента.Проведен
	|";
	
	ИмяДокумента = "Документ.РеализацияТоваровУслуг";
	Если ЭтоПередачаТоваров Тогда
		ИмяДокумента = "Документ.ПередачаТоваровХранителю";
		
		ЭтоОперацияОтгрузкиТоваров = ЭтоОперацияОтгрузкиТоваров(Объект.ХозяйственнаяОперация);
		Если ЭтоОперацияОтгрузкиТоваров Тогда
			ИмяДокумента = "Документ.ОтгрузкаТоваровКлиенту";
		КонецЕсли;
	КонецЕсли;
...
Процедура УправлениеСозданиемТранспортныхНакладных(ЭтоИнициализация = Истина)
	...
	Для Каждого Документ Из Объект.Документы Цикл 
		...
		Если ТипЗнч(Документ.Документ) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
			Или ТипЗнч(Документ.Документ) = Тип("ДокументСсылка.ПередачаТоваровХранителю")
			Или ТипЗнч(Документ.Документ) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда
...
Процедура УправлениеТранспортнымиНакладными(ДокументыКОбработке = Неопределено)
	...
	Если СоздаватьТранспортнуюНакладную = Неопределено Тогда
		СоздаватьТранспортнуюНакладную = Элементы.ОформленныеДокументыТранспортнаяНакладная.Пометка;
	КонецЕсли;
	...
	Для Каждого СтрокаДокумент Из ДокументыКОбработке Цикл
		...
		ДанныеДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОснование, "ПометкаУдаления, Проведен");
		ДокументПомеченНаУдаление = ДанныеДокумента.ПометкаУдаления;
		ДокументПроведен          = ДанныеДокумента.Проведен;
		...
		Если ТипДокумента = Тип("ДокументСсылка.РеализацияТоваровУслуг")
			...
			Или ТипДокумента = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту") Тогда
...
Процедура УправлениеСозданиемДоверенностей()
	...
	Если Не (Объект.ФормаОплаты = Перечисления.ФормыОплаты.Безналичная
			...
			Или Объект.ФормаОплаты = Перечисления.ФормыОплаты.СистемаБыстрыхПлатежей)
...
Процедура УправлениеДоверенностями()
	...
	Иначе
		...
		Для Каждого СтрокаДокумент Из Объект.Документы Цикл
			...
			Если Не ОбщегоНазначения.СсылкаСуществует(СтрокаДокумент.Документ) Тогда
				Продолжить;
			КонецЕсли; 
			...
			КонецЕсли;
		КонецЦикла;
		ОбщегоНазначенияУТ.УдалитьСтрокиТаблицы(Объект.Документы, МассиНайденныхДоверенностей);
...
&НаСервере
Процедура СоздатьЗаявкуНаВозврат(Товары,
						МассивДокументовПродажи,
						МассивНайденныхДокументов,
						БезПроведения,
						Отказ)
	...
	Для Каждого ТекСклад Из МассивСкладов Цикл
		...
		Если ЗначениеЗаполнено(ТекСклад) Или МассивСкладов.Количество() = 1 Тогда
			...
			Если ВозвратСГруппыСкладов Тогда
			...
			КонецЕсли;
				...
				//@skip-check query-in-loop
				ПродажаОбъект = ЗаписатьЗаявкуНаВозврат(МассивНайденныхДокументов,
					ТоварыПоСкладу,
					ТекСклад,
					Отказ,
					БезПроведения); // ДокументОбъект.ЗаявкаНаВозвратТоваровОтКлиента
...
Процедура СоздатьРеализацию(Товары, МассивРеализаций, МассивНайденныхДокументов, БезПроведения, Отказ)
	...
	ПродажаОбъект  = ЗаписатьРеализацию(МассивНайденныхДокументов, Товары, Отказ, БезПроведения);
	Если Отказ Тогда
		Возврат;
	КонецЕсли; 
	МассивРеализаций.Добавить(ПродажаОбъект);
	...
	Если НЕ Отказ
		И СоздаватьСчетПКО
		И Объект.СоздаватьСчетНаОплату
		И Товары.Итог("СуммаБезВозвратнойТары") > 0 Тогда
		
		ЗаписатьСчетНаОплату(
			МассивНайденныхДокументов, ПродажаОбъект, ПродажаОбъект.ЭтапыГрафикаОплаты.Выгрузить(), Отказ);
	...
	// Создание счета-фактуры по реализации / акту
	Если НЕ Отказ
		И Товары.Итог("СуммаБезВозвратнойТары") > 0
		И Объект.ХозяйственнаяОперация <> ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПередачаНаКомиссию") Тогда
...
&НаСервере
Процедура СоздатьОтгрузкуТоваровКлиентуПоЗаказу(Товары, МассивОтгрузок, МассивНайденныхДокументов, ОснованиеОбъект, Отказ)

	МассивСкладов = Новый Массив;
	МассивОтгрузок = Новый Массив;
	ОтгрузкаСГруппыСкладов = Ложь;
	
	Если СкладГруппа Тогда
		ВыборГруппы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Склад, "ВыборГруппы");
		
		Если ВыборГруппы = Перечисления.ВыборГруппыСкладов.РазрешитьВЗаказахИНакладных Тогда
			ОтгрузкаСГруппыСкладов = Истина;
	...
	Для Каждого ТекСклад Из МассивСкладов Цикл
		...
		Если ЗначениеЗаполнено(ТекСклад)
			...
			Если ОтгрузкаСГруппыСкладов Тогда
				ТоварыПоСкладу = Товары;
			Иначе
				ОтборПоСкладу  = Новый Структура("Склад", ТекСклад);
				ТоварыПоСкладу = Товары.Скопировать(ОтборПоСкладу);
			...
			ОтгрузкаТоваровОбъект = ЗаписатьОтгрузкуТоваровКлиентуПоЗаказу(МассивНайденныхДокументов,
				ОснованиеОбъект,
				ТоварыПоСкладу,
				ТекСклад,
				Отказ);
			...
			МассивОтгрузок.Добавить(ОтгрузкаТоваровОбъект);
			
			// Создание счета-фактуры по Отгрузке товаров
			Если Не Отказ И Объект.ПечататьСчетФактуру Тогда
				СоздатьСчетФактуру(МассивНайденныхДокументов, ОтгрузкаТоваровОбъект, Отказ);
			КонецЕсли;
			
			// Создание транспортной накладной по Отгрузке товаров
			Если СоздаватьТранспортнуюНакладную И Не Отказ Тогда
				ОснованияТранспортнойНакладной.Добавить(ОтгрузкаТоваровОбъект.Ссылка);
			КонецЕсли;
...
Процедура СоздатьРасходныеОрдера(Товары, МассивНайденныхДокументов, МассивРеализаций, ОснованиеОбъект, БезПроведения, Отказ)
	...
	Для Каждого ТекСклад Из МассивСкладов Цикл
		...
		ПараметрыПереоформленияРасходныхОрдеров.Склад = ТекСклад;
		ПараметрыПереоформленияРасходныхОрдеров.Получатель = Объект.Партнер;
		
		Результат = СкладыСервер.ПереоформитьРасходныеОрдера(ПараметрыПереоформленияРасходныхОрдеров);
	...
	Если ЗначениеЗаполнено(Объект.Соглашение) Тогда
		...
		Иначе
			...
			Если Объект.ОтображатьРекомендацииКПокупке
				...
				|	СУММА(РаспоряженияНаОтгрузку.Количество) КАК ОбъемПродаж,
				|	ВЫБОР КОГДА РАЗНОСТЬДАТ(МИНИМУМ(РаспоряженияНаОтгрузку.Период), &ТекущаяДата, ДЕНЬ) = 0 ТОГДА
				|		СУММА(РаспоряженияНаОтгрузку.Количество)
				|	ИНАЧЕ
				|		СУММА(РаспоряженияНаОтгрузку.Количество) / РАЗНОСТЬДАТ(МИНИМУМ(РаспоряженияНаОтгрузку.Период), &ТекущаяДата, ДЕНЬ)
				...
				|	И РаспоряженияНаОтгрузку.Показатель = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.КОформлению)
				|	И РаспоряженияНаОтгрузку.Активность
				|	И РаспоряженияНаОтгрузку.Количество > 0
				...
				|	СУММА(РаспоряженияНаОтгрузку.Количество) КАК ОбъемПредыдущегоЗаказа
...
&НаКлиентеНаСервереБезКонтекста
Функция СсылкаПоВариантуОформления(ВариантОформленияДокументов)
	
	Результат = ПредопределенноеЗначение("Документ.ЗаказКлиента.ПустаяСсылка");
	
	Если ВариантОформленияДокументов = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.РеализацияТоваровУслуг")
		Или ВариантОформленияДокументов = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.ЗаказКлиентаРеализацияТоваровУслуг") Тогда
		
		Результат = ПредопределенноеЗначение("Документ.РеализацияТоваровУслуг.ПустаяСсылка");
		
	ИначеЕсли ВариантОформленияДокументов = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.ПередачаТоваровХранителю")
		Или ВариантОформленияДокументов = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.ЗаказКлиентаПередачаТоваровХранителю") Тогда
		
		Результат = ПредопределенноеЗначение("Документ.ПередачаТоваровХранителю.ПустаяСсылка");
...
Процедура ЗагрузитьДополнительныеРеквизитыИзХранилищаСервер(ВыбранноеЗначение);
	...
	ИначеЕсли ВыбранноеЗначение.ТипДокумента = "ДокументОбъект.ОтгрузкаТоваровКлиенту" Тогда
		ИмяТаблицы = "ДополнительныеРеквизитыОтгрузкиТоваров";
	Иначе
...
Процедура ПоместитьЭтапыОплатыВоВременноеХранилище(ИмяЭлемента)
	ЭтапыОплатыСервер.ПоместитьЭтапыОплатыВоВременноеХранилище(ЭтотОбъект, ИмяЭлемента);
КонецПроцедуры

&НаСервере
Процедура ЗагрузитьЭтапыОплатыИзВременногоХранилища()
	ЭтапыОплатыСервер.ЗагрузитьЭтапыОплатыИзВременногоХранилища(ЭтотОбъект);
КонецПроцедуры
...
// Параметры:
// 	Действие - Строка
// 	ДополнительныйПараметр - Структура.
// 
&НаКлиенте
...
Процедура АдресДоставкиПриИзменении(Элемент)
	...
	ИмяРеквизитаАдреса = ПолучитьИмяРеквизитаАдресаДоставки(Элемент);
	ОбработатьИзменениеАдреса(Элемент, ИмяРеквизитаАдреса);
...
Процедура ЗаполнитьМногооборотнуюТаруИзХранилищаСервер(АдресТарыВХранилище)
	...
	СтруктураДействийДобавленныеСтроки = ОбщегоНазначения.СкопироватьРекурсивно(СтруктураДействийИзмененныеСтроки); // - Структура -
...
Процедура ЗаполнитьСписокВыбораОпераций(ИзмененВариантОформления = Ложь)
	...
	Если ИзмененВариантОформления Тогда
		...
		Иначе
			...
			ОперацииПродажи.Добавить(Перечисления.ХозяйственныеОперации.ПередачаНаХранениеСПравомПродажи);
			Если ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5 Тогда
				ОперацииПродажи.Добавить(Перечисления.ХозяйственныеОперации.ОтгрузкаТоваровКлиенту);
			КонецЕсли;
		КонецЕсли;
		...
		ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию
			И (Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.ЗаказКлиентаРеализацияТоваровУслуг
				Или Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.РеализацияТоваровУслуг)
...
Процедура ГиперссылкаСостояниеОбеспеченияНажатиеОбщий()
	...
	Если Объект.УпрощенноеОбеспечение Или Не КомандыОбеспеченияДоступны(ЭтотОбъект) Тогда
...
Процедура ПослеЗаполненияОбеспечения(Изменения, Режим = Неопределено, ПараметрыЗаполнения = Неопределено)
	...
	ДобавитьДействияОбеспеченияНаСервере(СтруктураДействий, "СкладОбязателен,ДатаОтгрузкиОбязательна");
	КэшированныеЗначения = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
	ОбрабатываемыеСтроки = Изменения.ВыгрузитьКолонку("Строка");
...
&НаСервереБезКонтекста
Функция ПолучитьВариантОбеспеченияИФлагОбособленно(ВариантОбеспечения, Соглашение, СтатусЗаказаКлиента)

	ВариантИФлаг = Новый Структура("ВариантОбеспечения, Обособленно", ВариантОбеспечения, Ложь);
	Возврат ПродажиСервер.ВариантОбеспеченияИФлагОбособленноПоУмолчанию(
		Соглашение,
		СтатусЗаказаКлиента,
		ВариантИФлаг);
