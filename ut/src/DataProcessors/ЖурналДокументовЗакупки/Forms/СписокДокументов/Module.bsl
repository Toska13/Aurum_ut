...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоВыборДокументовДляНачисленияРетроБонусовПоставщиков() Тогда
		
		ВыборДокументаПриобретения = Истина;
		Элементы.ИнформационнаяНадписьОтбор.Видимость = Ложь;
		Элементы.СписокДокументыЗакупки.РежимВыбора = Истина;
		
		Если ПолучитьФункциональнуюОпцию("ИспользоватьСоглашенияСПоставщиками") Тогда
			Элементы.СписокДокументыЗакупкиСоглашение.Видимость = Истина;
		КонецЕсли;
		
	КонецЕсли;
	
	Если Параметры.Свойство("ОтборТипыДокументов") Тогда
		
		ОтборТипыДокументов.ЗагрузитьЗначения(Параметры.ОтборТипыДокументов);
		
	КонецЕсли;
	
	Если Параметры.Свойство("ОтборХозяйственныеОперации") Тогда
		
		ОтборХозяйственныеОперации.ЗагрузитьЗначения(Параметры.ОтборХозяйственныеОперации);
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ВыборДокументаПриобретения Тогда
...
&НаКлиенте
Процедура СписокДокументыЗакупкиВыборЗначения(Элемент, Значение, СтандартнаяОбработка)
...
Процедура Подключаемый_ВыполнитьКомандуИнтеграции(Команда)
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыполнитьПодключаемуюКомандуИнтеграции(Команда, ЭтотОбъект, Элементы.СписокДокументыЗакупки);
...
Процедура ОбработатьШтрихкоды(Данные)
	...
	ДанныеПоШтрихКоду = ДанныеПоШтрихКодуПечатнойФормы(Данные.Штрихкод, "СписокДокументыЗакупки");
...
&НаСервере
Функция ЭтоВыборДокументовДляНачисленияРетроБонусовПоставщиков()
	
	ФормаВРежимеВыбора = Ложь;
	Если КлючНазначенияИспользования = "ДокументыЗакупкиВыбор" Тогда
		ФормаВРежимеВыбора = Истина
	КонецЕсли;
	
	Возврат ФормаВРежимеВыбора;
...
Процедура НастроитьФормуПоВыбраннымОперациямИДокументам(ТЗХозОперацииИТипыДокументов)
	...
	Если ВыборДокументаПриобретения
	   И ПолучитьФункциональнуюОпцию("ИспользоватьСоглашенияСПоставщиками") Тогда
		
		ДополнитьСписокДаннымиСоглашений(СвойстваСписка.ТекстЗапроса);
...
&НаСервере
Процедура ДополнитьСписокДаннымиСоглашений(ТекстЗапроса)
	
	ФрагментТекстаСоглашение =
	"ВЫБОР
	|	КОГДА РеестрДокументов.Ссылка ССЫЛКА Документ.ПриобретениеТоваровУслуг
	|		ТОГДА ВЫРАЗИТЬ(РеестрДокументов.Ссылка КАК Документ.ПриобретениеТоваровУслуг).Соглашение
	|	КОГДА РеестрДокументов.Ссылка ССЫЛКА Документ.КорректировкаПриобретения
	|		ТОГДА ВЫРАЗИТЬ(РеестрДокументов.Ссылка КАК Документ.КорректировкаПриобретения).Соглашение
	|	КОГДА РеестрДокументов.Ссылка ССЫЛКА Документ.ВозвратТоваровПоставщику
	|		ТОГДА ВЫРАЗИТЬ(РеестрДокументов.Ссылка КАК Документ.ВозвратТоваровПоставщику).Соглашение
	|	КОГДА РеестрДокументов.Ссылка ССЫЛКА Документ.ВыкупПринятыхНаХранениеТоваров
	|		ТОГДА ВЫРАЗИТЬ(РеестрДокументов.Ссылка КАК Документ.ВыкупПринятыхНаХранениеТоваров).Соглашение
	|	ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СоглашенияСПоставщиками.ПустаяСсылка)
	|КОНЕЦ";
	
	ЗаменяемыйТекст = "&Соглашение";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ЗаменяемыйТекст, ФрагментТекстаСоглашение);
