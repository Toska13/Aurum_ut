...
// 
// Параметры:
//  ДанныеФормированияПротокола - см. НовыеДанныеФормированияПротокола
// 
// Возвращаемое значение:
//  ТабличныйДокумент - Представление протокола
Функция ПредставлениеПротокола(ДанныеФормированияПротокола) Экспорт
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	Если Не ЗначениеЗаполнено(ДанныеФормированияПротокола) Тогда
		Возврат ТабличныйДокумент;
	КонецЕсли;
	
	Макет = ПолучитьМакет("ПротоколОбменаПоДокументу_ru");

	ВывестиШапкуВПротокол(ДанныеФормированияПротокола, ТабличныйДокумент, Макет);
	
	ОсновныеСообщения = ДанныеФормированияПротокола.ОсновныеСообщения;
	...
	ВывестиХронологиюОбмена(ДанныеФормированияПротокола.ХронологияОбмена, ТабличныйДокумент, Макет);
	
	Возврат ТабличныйДокумент;
КонецФункции

// Данные Формирования протокола.
// 
// Параметры:
//  ПараметрыФормированияПротокола - см. Обработки.ПротоколОбменаПоДокументуЭДО.НовыеПараметрыФормированияПротокола
// 
// Возвращаемое значение:
//  см. НовыеДанныеФормированияПротокола
Функция ДанныеФормированияПротокола(ПараметрыФормированияПротокола) Экспорт
	...
	ДанныеФормированияПротокола = НовыеДанныеФормированияПротокола();
	ДанныеФормированияПротокола.ДатаФормирования = ТекущаяДатаСеанса();
	ДанныеФормированияПротокола.Документ = ЭлектронныйДокумент;
	ДанныеФормированияПротокола.ИдентификаторДокументооборота = ПараметрыФормированияПротокола.ИдентификаторДокументооборота;
	ДанныеФормированияПротокола.Состояние = ПараметрыФормированияПротокола.СостояниеДокумента;
	ДанныеФормированияПротокола.ПредставлениеДокумента = ЭлектронныеДокументыЭДО.ПредставлениеДокументаПоСвойствам(
		ПараметрыФормированияПротокола.СвойстваДокументаДляПредставления);
	
	УстановитьДанныеКорреспондентов(ДанныеФормированияПротокола, ЭлектронныйДокумент, СведенияОрганизацииИКонтрагента);
	...
	Если СинхронизацияЭДО.ЭтоВнутреннийОбмен(ПараметрыФормированияПротокола.СпособОбмена) Тогда
		ЗаполнитьДатыОтправкиПолученияОсновныхСообщенийПротоколаВнутреннегоОбмена(ОсновныеСообщения, ХронологияОбмена, ЭлектронныйДокумент);
	...
	ДанныеФормированияПротокола.ОсновныеСообщения = ОсновныеСообщения;
	ДанныеФормированияПротокола.ХронологияОбмена = ХронологияОбмена;
	
	Возврат ДанныеФормированияПротокола;
	
КонецФункции

// Конструктор для формирования протокола обмена
// 
// Возвращаемое значение:
//  Структура - Новые данные Формирования протокола:
...
//
Функция НовыеДанныеФормированияПротокола() Экспорт
...
Процедура ВывестиШапкуВПротокол(ДанныеФормированияПротокола, ТабличныйДокумент, Макет)
	
	ОбластьШапка = Макет.ПолучитьОбласть("Шапка");
	ОбластьШапка.Параметры.Заполнить(ДанныеФормированияПротокола);
	ОбластьШапка.Параметры.СостояниеДокументооборота = ПредставлениеЗавершенностиДокументооборота(
		ДанныеФормированияПротокола.Состояние);
	ОбластьШапка.Параметры.ДатаФормированияФорматированная = ОбщегоНазначенияБЭД.ПредставлениеЛокальнойДатыСоСмещением(
		ДанныеФормированияПротокола.ДатаФормирования, "ДЛФ=DT;");
	...
	ВывестиКорреспондента(ПостфиксыКорреспондентов.Отправитель, ДанныеФормированияПротокола, ТабличныйДокумент, Макет);
	
	Если ЗначениеЗаполнено(ДанныеФормированияПротокола.Получатель) Тогда
		ВывестиКорреспондента(ПостфиксыКорреспондентов.Получатель, ДанныеФормированияПротокола, ТабличныйДокумент, Макет);
	КонецЕсли;
...
//  ТабличныйДокумент - ТабличныйДокумент
//  Макет - ТабличныйДокумент
Процедура ВывестиКорреспондента(ВидКорреспондента, ДанныеФормированияПротокола, ТабличныйДокумент, Макет)
	...
	ОбластьШапкаОтправитель.Параметры.Заполнить(ДанныеФормированияПротокола);
	...
	Если ЗначениеЗаполнено(ДанныеФормированияПротокола[ИдентификаторКорреспондента]) Тогда
		...
		ШапкаИдентификатор.Параметры.Заполнить(ДанныеФормированияПротокола);
		...
		Если ЗначениеЗаполнено(ДанныеФормированияПротокола[ПредставлениеОператораОтправителя]) Тогда
			...
			ШапкаОператор.Параметры.Заполнить(ДанныеФормированияПротокола);
...
Функция ПостфиксыКорреспондентов()
	Результат = Новый Структура;
	Результат.Вставить("Отправитель", "Отправителя");
	Результат.Вставить("Получатель", "Получателя");
...
Процедура ВывестиХронологиюОбмена(ХронологияОбмена, ТабличныйДокумент, Макет)
	...
	Для Каждого СтрокаХронологии Из ХронологияОбмена Цикл
		...
		ОбластьХронологияОбмена.Параметры.НомерПозиции = Формат(НомерСтроки, "ЧГ=0;");
...
Процедура ВывестиПодписиВПротокол(ДанныеСообщения, ТабличныйДокумент, Макет)
	...
	Для Каждого Подпись Из ДанныеСообщения.Подписи Цикл
		...
		Если ЗначениеЗаполнено(Подпись.СерийныйНомерСертификата) Тогда
			...
			ОбластьДанныеСертификата.Параметры.ПериодДействияСертификата = ПредставлениеПериодаДействия(Подпись.ДатаНачалаДействияСертификата, 
...
Процедура ВывестиДоверенностьВПротокол(ДанныеПодписи, ТабличныйДокумент, Макет)
	...
	ОбластьДанныеДоверенностиКонец.Параметры.ПериодДействияДоверенности = ПредставлениеПериодаДействия(
...
// Возвращаемое значение:
//  Строка
Функция ПредставлениеПериодаДействия(ДатаНачала, ДатаОкончания)
...
Процедура УстановитьДанныеКорреспондентов(ДанныеФормированияПротокола, ЭлектронныйДокумент, СведенияОрганизацииИКонтрагента)
	
	Если ТипЗнч(ЭлектронныйДокумент) = Тип("ДокументСсылка.ЭлектронныйДокументИсходящийЭДО") Тогда
		ДанныеФормированияПротокола.Отправитель = СведенияОрганизацииИКонтрагента.Организация;
		ДанныеФормированияПротокола.ИдентификаторОтправителя = СведенияОрганизацииИКонтрагента.ИдентификаторОрганизации;
		ДанныеФормированияПротокола.ПредставлениеОтправителя = СведенияОрганизацииИКонтрагента.ПредставлениеОрганизации;
		ДанныеФормированияПротокола.ПредставлениеОператораОтправителя = СведенияОрганизацииИКонтрагента.ОператорОрганизации;
		
		ДанныеФормированияПротокола.Получатель = СведенияОрганизацииИКонтрагента.Контрагент;
		ДанныеФормированияПротокола.ИдентификаторПолучателя = СведенияОрганизацииИКонтрагента.ИдентификаторКонтрагента;
		ДанныеФормированияПротокола.ПредставлениеПолучателя = СведенияОрганизацииИКонтрагента.ПредставлениеКонтрагента;
		ДанныеФормированияПротокола.ПредставлениеОператораПолучателя = СведенияОрганизацииИКонтрагента.ОператорКонтрагента;

	Иначе 
		ДанныеФормированияПротокола.Отправитель = СведенияОрганизацииИКонтрагента.Контрагент;
		ДанныеФормированияПротокола.ИдентификаторОтправителя = СведенияОрганизацииИКонтрагента.ИдентификаторКонтрагента;
		ДанныеФормированияПротокола.ПредставлениеОтправителя = СведенияОрганизацииИКонтрагента.ПредставлениеКонтрагента;
		ДанныеФормированияПротокола.ПредставлениеОператораОтправителя = СведенияОрганизацииИКонтрагента.ОператорКонтрагента;
		
		ДанныеФормированияПротокола.Получатель = СведенияОрганизацииИКонтрагента.Организация;
		ДанныеФормированияПротокола.ИдентификаторПолучателя = СведенияОрганизацииИКонтрагента.ИдентификаторОрганизации;
		ДанныеФормированияПротокола.ПредставлениеПолучателя = СведенияОрганизацииИКонтрагента.ПредставлениеОрганизации;
		ДанныеФормированияПротокола.ПредставлениеОператораПолучателя = СведенияОрганизацииИКонтрагента.ОператорОрганизации;
	КонецЕсли;
...
Процедура ЗаполнитьДатыУточненийИПредложенийОбАннулировании(ЭлементХронологии, ОсновныеСообщения)
	...
	ИначеЕсли ЭлементХронологии.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.УС_ОтказВПодписи 
		Или ЭлементХронологии.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.УС_УОУ Тогда
			
		Если ЭлементХронологии.Направление = Перечисления.НаправленияЭДО.Входящий Тогда
			УстановитьДатуВОсновноеСообщение(ОсновныеСообщения.УведомлениеОбУточнении, 
				"ДатаПолучения",
				ЭлементХронологии.ДатаПодтверждения);
		Иначе
...
//						 ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
Процедура ЗаполнитьДатыОтправкиПолученияОсновныхСообщенийПротоколаВнутреннегоОбмена(ОсновныеСообщения, ХронологияОбмена, ЭлектронныйДокумент)
	ЭтоВходящийДокумент = ТипЗнч(ЭлектронныйДокумент) = Тип("ДокументСсылка.ЭлектронныйДокументВходящийЭДО");
	...
	Если ЭлементХронологии <> Неопределено Тогда
		УстановитьДатуВОсновноеСообщение(ОсновныеСообщения.ТитулОтправителя, "ДатаОтправки",
			ЭлементХронологии.ДатаИзмененияСтатуса);
		УстановитьДатуВОсновноеСообщение(ОсновныеСообщения.ТитулОтправителя, "ДатаПолучения",
			ЭлементХронологии.ДатаИзмененияСтатуса);

	ИначеЕсли ЭлементХронологии.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.УС_ИОП Тогда

		Если ЭтоВходящийДокумент Тогда
			Если ЭлементХронологии.Направление = Перечисления.НаправленияЭДО.Входящий Тогда
				УстановитьДатуВОсновноеСообщение(ОсновныеСообщения.ТитулПолучателя, "ДатаПолучения",
					ЭлементХронологии.ДатаПодтверждения);
			Иначе
				УстановитьДатуВОсновноеСообщение(ОсновныеСообщения.ТитулОтправителя, "ДатаПолучения",
					ЭлементХронологии.ДатаПодтверждения);
...
Функция ОсновныеСообщенияПротоколаОбмена(ЭлектронныйДокумент, СообщенияДокумента, СведенияОрганизацииИКонтрагента)
	...
	Для каждого СообщениеДокумента Из СообщенияДокумента Цикл
		...
		ИначеЕсли СообщениеДокумента.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.УОУ 
			Или СообщениеДокумента.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ПОА_УОУ 
			Или СообщениеДокумента.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.УС_УОУ
			Или СообщениеДокумента.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.УС_ОтказВПодписи Тогда
...
Функция КомментарийСообщенияПротокола(ДанныеСообщения)
	...
	Если ДанныеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ПОА
		...
		Или ДанныеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ПОА_УОУ
		Или ДанныеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.УС_ОтказВПодписи
		Или ДанныеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.УС_УОУ Тогда
		...
		Иначе
			Если ДоверенностьОпределенна(Подпись.СвойстваДоверенности) Тогда
				...
				Если ИмеетсяПротоколПроверки И ПротоколПроверки.ВерсияПротокола = "2.0" Тогда
					...
					ДоверенностьДействует = МашиночитаемыеДоверенностиКлиентСервер.ДоверенностьПроверенаУспешно(
						ПроверкаДоверенности);
						
					Результат = МашиночитаемыеДоверенности.ПредставлениеРезультатаАвтоматическойПроверкиМЧД(
						ДоверенностьДействует, ПроверкаДоверенности, Подпись.СвойстваДоверенности);
				Иначе
					ДоверенностьДействует = МашиночитаемыеДоверенности.ДоверенностьДействительнаПоСвойствам(
						Подпись.СвойстваДоверенности, Подпись.СвойстваПодписи.ДатаПодписи);
...
Функция ПредставленияСубъектовДоверенности(ДанныеСубъекта)
	...
	Для Каждого Субъект Из ДанныеСубъекта Цикл
		ПредставленияСубъектовДоверенности.Добавить(МашиночитаемыеДоверенности.ПредставлениеСубъектаДоверенности(Субъект));
	КонецЦикла;
...
Функция ХронологияОбмена(СообщенияДокумента, СведенияОрганизацииИКонтрагента)
	...
	Для каждого ДанныеСообщенияДокумента Из СообщенияДокумента Цикл
		...
		СтрокаХронологии.ДатаПодтверждения = ДатаПодтверждения(ДанныеСообщенияДокумента.Ссылка, 
			...
			ДанныеСообщенияДокумента.ТипЭлементаРегламента);
...
// Возвращаемое значение:
//  Дата
Функция ДатаПодтверждения(Сообщение, ТипСлужебногоДокумента, ТипЭлементаРегламента)
	...
	Если ТипСлужебногоДокумента = Перечисления.ТипыДокументовЭДО.ИзвещениеОПолучении 
		...
		Если РегламентыЭДО.ЭтоУниверсальноеСообщение(ТипЭлементаРегламента) Тогда
			Результат = ДатаИВремяИзОбработкиФормата(ДанныеЭлектронногоДокумента.НовыйЭД.ЗначениеРеквизита);
		Иначе
...
Функция ДатаИВремяИзОбработкиФормата(ОбработкаФормата)
	
	ДатаИВремяОтправки = ОбработкаФормата.ДатаИВремяФормирования();
...
Функция ДатаИВремяИзДереваРазбора(ДанныеЭлектронногоДокумента, ИмяРеквизитаДаты, ИмяРеквизитаВремени)
	...
	ДатаИВремяОтправки = ЭлектронныеДокументыЭДОСлужебный.ДатаИВремяИзДереваРазбора(ДанныеЭлектронногоДокумента, 
		ИмяРеквизитаДаты, 
		ИмяРеквизитаВремени);
	ЧасовойПоясМСК = "Europe/Moscow";
	Если ЧасовойПоясМСК <> ЧасовойПоясСеанса() Тогда
