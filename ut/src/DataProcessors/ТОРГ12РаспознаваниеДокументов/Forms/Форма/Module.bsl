...
Процедура ПолеПросмотраДокументСформирован(Элемент)
	...
	Если Не Элемент.Документ.readyState = "complete" Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПриИзмененииПоляНаСервере(ИмяПоля)
	...
	ИначеЕсли ИмяПоля = "Подразделение" Тогда  
		ЗаполнитьУстановитьВидимостьСерийДоступностьПерезаполнитьПоОтгрузке();
	ИначеЕсли ИмяПоля = "Договор" Тогда
		ТекущийДоговор = РаспознаваниеДокументовСлужебныйКлиентСервер.ЗначениеРеквизитаДокумента(Объект, "Договор",
			Справочники.ДоговорыКонтрагентов.ПустаяСсылка());
		СтатусДоговора = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущийДоговор, "Статус");
...
Процедура Подключаемый_ОбработкаВыбораПоля(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ДополнительныеДанные = РаспознаваниеДокументовСлужебныйКлиент.ДополнительныеПараметрыСозданияЭлемента(
		ЭтотОбъект, ИмяТаблицы, МассивПараметровВыбораНоменклатуры(Объект.ТипДокумента));
	...
	Если Не ТекущийЭлемент = Элементы.ТаблицаДокумента Тогда
		...
		ДополнительныеДанные.ИННКонтрагента = ИННКонтрагента;
		ДополнительныеДанные.КППКонтрагента = КППКонтрагента;
		ДополнительныеДанные.ХозяйственнаяОперация = ХозяйственнаяОперация;
	КонецЕсли;
...
Процедура ГруппаСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница.Имя = "СтраницаГрупповоеСоздание" Тогда
		Если СвойстваЯчеекТаблицы = Неопределено Тогда
	...
	Для Каждого СтрокаДобавления Из СписокНераспознаннойНоменклатуры Цикл
		ИдентификаторСтроки = СтрокаДобавления.ПолучитьИдентификатор();
		Если Элементы.СписокНераспознаннойНоменклатуры.ВыделенныеСтроки.Найти(ИдентификаторСтроки) = Неопределено Тогда
			Элементы.СписокНераспознаннойНоменклатуры.ВыделенныеСтроки.Добавить(ИдентификаторСтроки);
		КонецЕсли;
	КонецЦикла;
...
Процедура ВариантОбработкиПриИзменении(Элемент)
	...
	Обработчик = Новый ОписаниеОповещения("ПослеПроверкиИзменения", ЭтотОбъект);
	РаспознаваниеДокументовСлужебныйКлиент.ИзменитьВариантОбработки(ЭтотОбъект, Обработчик, Ложь);
...
Процедура ТаблицаДокументаПриИзменении(Элемент)
	...
	Если Элемент.ТекущийЭлемент.Имя = "СтатьяРасходов" Тогда 
...
Процедура СоздатьНоменклатуру(Команда) 
	...
	РаспознаваниеДокументовСлужебныйКлиент.ИнтерактивноеГрупповоеСозданиеНоменклатуры(ЭтотОбъект, 
		МассивПараметровВыбораНоменклатуры(Объект.ТипДокумента));
...
Процедура ЗагрузитьСвойстваЯчеекТаблицы() Экспорт
	...
	КлючеваяОперация = "Обработка.ТОРГ12РаспознаваниеДокументов.Форма.ЗагрузитьСвойстваЯчеекТаблицы";
	Замер = ОценкаПроизводительностиКлиент.ЗамерВремени(КлючеваяОперация);	
...
Процедура ПоказатьСозданиеКомплекта() Экспорт
	...
	ДанныеКомплекта.ВидДокумента = ВидДокумента;
	ОбновитьИЗаполнитьСвязанныеИСозданныеДокументы(ДанныеКомплекта);
...
Процедура ОбновитьИЗаполнитьСвязанныеИСозданныеДокументы(ДанныеКомплекта)

	РаспознаваниеДокументовКомплекты.ОбновитьИЗаполнитьСвязанныеИСозданныеДокументы(ДанныеКомплекта);

КонецПроцедуры // ОбновитьИЗаполнитьСвязанныеИСозданныеДокументы()
...
Процедура НаправлениеПриИзмененииНаСервере()
	...
	РаспознаваниеДокументовПереопределяемый.ПолучитьФункциональнуюОпциюУчетДоговоров(Объект, ВестиУчетПоДоговорам);
	...
	Элементы.ВидДокумента.Заголовок = ЗаголовокВидаДокумента(Объект.Направление);
	...
	Если Объект.Направление = Перечисления.НаправленияРаспознанногоДокумента.Исходящий Тогда;
		...
		Элементы.ВидДокумента.СписокВыбора.Добавить("РТИУ", НСтр("ru = 'Товаров и услуг'"));
		Элементы.ВидДокумента.СписокВыбора.Добавить("РУИПА", НСтр("ru = 'Услуг и прочих активов'"));
		Элементы.ГруппаОтправитель.Заголовок = НСтр("ru = 'Отправитель'");  
		Если Не ПравоДоступа("Добавление",  Метаданные.Документы.РеализацияТоваровУслуг) 
			Или Не ПравоДоступа("Добавление",  Метаданные.Документы.РеализацияУслугПрочихАктивов) Тогда  
	...
	Иначе
		...
		Элементы.ВидДокумента.СписокВыбора.Очистить();
		Элементы.ВидДокумента.СписокВыбора.Добавить("ПТИУ", НСтр("ru = 'Товаров и услуг'"));
		Элементы.ВидДокумента.СписокВыбора.Добавить("ПУИПА", НСтр("ru = 'Услуг и прочих активов'")); 
		Элементы.ГруппаОтправитель.Заголовок = НСтр("ru = 'Получатель'"); 
		Если Не ПравоДоступа("Добавление", Метаданные.Документы.ПриобретениеТоваровУслуг) 
			Или Не ПравоДоступа("Добавление", Метаданные.Документы.ПриобретениеУслугПрочихАктивов) Тогда  
			Элементы.ВидДокумента.Видимость = Ложь;
		КонецЕсли;
...
Процедура НастроитьВидимостьПоФункциональнымОпциям()
	...
	РаспознаваниеДокументовПереопределяемый.ПолучитьФункциональнуюОпциюУчетПоНесколькимСкладам(ИспользоватьУчетПоНесколькимСкладам);
	
	Элементы.ГруппаСклад.Видимость = ИспользоватьУчетПоНесколькимСкладам;
	Элементы.ТСклад.Видимость = ИспользоватьУчетПоНесколькимСкладам;  
	
	ПоступлениеНаНесколькоСкладов = ПолучитьФункциональнуюОпцию("ИспользоватьСкладыВТабличнойЧастиДокументовЗакупки");
	РеализацияСНесколькихСкладов = ПолучитьФункциональнуюОпцию("ИспользоватьСкладыВТабличнойЧастиДокументовПродажи");
	
	Если Объект.Направление = Перечисления.НаправленияРаспознанногоДокумента.Исходящий Тогда
		Если Не РеализацияСНесколькихСкладов Тогда
			Элементы.Склад.ВыборГруппИЭлементов = ГруппыИЭлементы.Элементы;
	...
	Если Объект.Направление = Перечисления.НаправленияРаспознанногоДокумента.Входящий Тогда
		Если Не ПоступлениеНаНесколькоСкладов Тогда
			Элементы.Склад.ВыборГруппИЭлементов = ГруппыИЭлементы.Элементы; 
			Склад = РаспознаваниеДокументовСлужебныйКлиентСервер.ЗначениеРеквизитаДокумента(Объект, "Склад",
				Справочники.Склады.ПустаяСсылка());
			Если Справочники.Склады.ЭтоГруппа(Склад) Тогда
	...
	Элементы.Характеристика.Видимость = ИспользоватьХарактеристики 
		И (ВидДокумента <> "РУИПА" И ВидДокумента <> "ПУИПА");
...
Процедура ЗадатьВопросОСозданииДокумента(ТипДокументаСтрокой, ВидОперации, ИдетСозданиеСФ)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПолученРезультатОтветаНаВопрос", ЭтотОбъект, ДополнительныеПараметры);
	...
	ОткрытьФорму("Обработка.РаспознаваниеДокументов.Форма.ВопросПриНаличииИмеющихсяДокументовБРД", ПараметрыФормы, ЭтотОбъект,,,,ОписаниеОповещения);
...
Процедура ЗаполнитьТаблицуДокумента()
	...
	Для Каждого Запись Из Объект.РеквизитыТабличныхЧастей Цикл
		...
		Если Запись.ИмяРеквизита = "Номенклатура" Тогда
			...
			ПриИзмененииНоменклатуры(ИзменяемаяСтрока.ПолучитьИдентификатор(), Ложь);
...
Процедура НастроитьЭлементыПоРеквизитам()
	...
	Для Каждого РеквизитШапки Из Объект.РеквизитыДокумента Цикл
		...
		Если Объект.ТипДокумента = Перечисления.ТипыДокументовРаспознаваниеДокументов.АктОбОказанииУслуг Тогда
			Если РеквизитШапки.ИмяРеквизита = "Грузоотправитель"
...
Процедура ИзменитьОтображениеПоВидуДокумента()
	...
	ДоступноСозданиеНоменклатуры = Истина;
	Если Не ПравоДоступа("Добавление", Метаданные.Справочники.Номенклатура) Тогда
		ДоступноСозданиеНоменклатуры = Ложь;
	Иначе
		ДоступноСозданиеНоменклатуры = ВидДокумента = "РТИУ" Или ВидДокумента = "ПТИУ";
	КонецЕсли;
	Элементы.СтраницаГрупповоеСоздание.Видимость = ДоступноСозданиеНоменклатуры;
	...
	Если ВидДокумента = "РТИУ" Тогда  
		...
		Элементы.ГруппаТабличнаяЧасть.Заголовок = НСтр("ru = 'Табличная часть'");
	...
	ИначеЕсли ВидДокумента = "РУИПА" Тогда
		...
		Элементы.ГруппаТабличнаяЧасть.Заголовок = НСтр("ru = 'Выручка (доходы)'");
	...
	ИначеЕсли ВидДокумента = "ПУИПА" Тогда
		...
		Элементы.ГруппаТабличнаяЧасть.Заголовок = НСтр("ru = 'Расходы и прочие активы'");
...
Процедура ИзменитьОтображениеПоТипуДокумента()
	...
	Если Объект.ТипДокумента = Перечисления.ТипыДокументовРаспознаваниеДокументов.УПД
		...
		Или Объект.ТипДокумента = Перечисления.ТипыДокументовРаспознаваниеДокументов.ТОРГ12
	...
	ИначеЕсли Объект.ТипДокумента = Перечисления.ТипыДокументовРаспознаваниеДокументов.НеопределенныйДокумент Тогда
...
Процедура ИзменитьИсточникиДляВыбораДоговора(ИмяПоля = "")
	...
	Если Не ИспользуютсяДоговорыКонтрагентов Тогда
		Объект.РеквизитыДокумента[6].Значение = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
	ИначеЕсли ПерезаполнитьДоговор Тогда 			
...
Процедура ИзменитьИсточникиДляВыбораСоглашения(ИмяПоля = "")
	...
	Если ПерезаполнитьСоглашение Тогда
		Если УправлениеДоступом.ЧтениеРазрешено(Справочники.СоглашенияСКлиентами.ПустаяСсылка()) Тогда
			РаспознаваниеДокументовУП.ЗаполнитьСоглашениеКонтрагента(Объект);
		КонецЕсли;
...
Процедура ЗаполнитьПроверитьПоляСклада() 
	...
	Иначе
		ВыборГруппы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Склад, "ВыборГруппы");
		Если ВыборГруппы = Перечисления.ВыборГруппыСкладов.Запретить Тогда
			Объект.РеквизитыДокумента[7].Значение = Справочники.Склады.ПустаяСсылка();
		КонецЕсли;
		ТаблицаЗначений = РеквизитФормыВЗначение("ТаблицаДокумента", Тип("ТаблицаЗначений"));
		ТаблицаЗначений.ЗаполнитьЗначения(Справочники.Склады.ПустаяСсылка(), "ТСклад");
		ТаблицаДокумента.Загрузить(ТаблицаЗначений);
	КонецЕсли;
...
Процедура ОбработатьСозданиеСчетаФактуры()
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ОснованиеСчетаФактуры) Тогда
			...
			ЗначенияЗаполнения.Вставить("Номер", Объект.РеквизитыДокумента[0].Значение);
			ЗначенияЗаполнения.Вставить("ДатаСоставления", Объект.РеквизитыДокумента[1].Значение);
			ЗначенияЗаполнения.Вставить("Организация", ОрганизацияПокупатель);
...
Процедура ЗаполнитьТаблицуСозданнойНоменклатуройНаСервере(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Данные Из ИнформацияОНоменклатуре Цикл
		...
		СтруктураОбъект = Новый Структура("СтатусУказанияСерий, Склад", 0, Объект.РеквизитыДокумента[7].Значение);
		СтруктураПоиска = Новый Структура("Номенклатура, Характеристика, ТСклад",
			СтрокаЗаполнения.Номенклатура, СтрокаЗаполнения.Характеристика, СтрокаЗаполнения.ТСклад);
		ЗаполнитьСтатусыУказанияСерий(СтруктураОбъект, СтруктураПоиска);
...
Процедура НайтиИЗаполнитьСозданныеДокументы()
	...
	ПараметрВидДокумента = ?(ЗначениеЗаполнено(ВидДокумента), ВидДокумента,
		РаспознаваниеДокументовСлужебныйКлиентСервер.ЗначениеРеквизитаДокумента(Объект, "ВидДокумента", ""));
	Если Не ЗначениеЗаполнено(ПараметрВидДокумента) Тогда
    	ПараметрВидДокумента = ?(Объект.Направление = Перечисления.НаправленияРаспознанногоДокумента.Исходящий,
			"РТИУ", "ПТИУ");
	КонецЕсли;
	ДанныеДокумента.Вставить("ВидДокумента", ПараметрВидДокумента);
	...
	Для Каждого СтрокаТаблицыДокумента Из ТаблицаДокумента Цикл
		...
		Для Каждого Колонка Из КолонкиТаблицы Цикл
			Если Колонка.Имя = "ЕдиницаИзмерения" Тогда
				...
				КонецЕсли;
				Если ЕдиницаИзмерения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТаблицыДокумента.Номенклатура, "ЕдиницаИзмерения") Тогда
			...
			ИначеЕсли Колонка.Имя = "ТСклад" Тогда
				ЗначениеСклада = СтрокаТаблицыДокумента[Колонка.Имя];
				Если Не СкладГруппа И Не ЗначениеЗаполнено(СтрокаТаблицыДокумента[Колонка.Имя]) Тогда
					ЗначениеСклада = Объект.РеквизитыДокумента[7].Значение;
				КонецЕсли;
				ДанныеСтрокиТаблицы.Вставить("Склад", ЗначениеСклада);
...
Процедура ОпределениеДействияДляСоздаваемойСчетФактуры(ОснованиеСчетаФактуры, ТипДокументаСтрокой, ПараметрыЗаполнения)
	...
	Если СчетФактураСсылка <> Неопределено Тогда
		ПоказатьОповещениеПользователя(НСтр("ru = 'Создан документ'"),
			ПолучитьНавигационнуюСсылку(СчетФактураСсылка), Строка(СчетФактураСсылка));
		СледующийДокумент();
	КонецЕсли;
...
Процедура ОбновитьСтрокиНераспознаннойНоменклатуры()
	...
	КлючеваяОперация 
		= "РаспознаваниеДокументов.Форма.Торг12РаспознаваниеДокументов.ОбновитьСтрокиНераспознаннойНоменклатуры";
...
Процедура УстановитьУсловноеОформление()
	...
	ДопПараметрыУсловногоОформления = НоменклатураСервер.НовыеПараметрыУсловногоОформленияЕдиницИзмерения();
	ДопПараметрыУсловногоОформления.ИмяПоляЕдиницаИзмерения = Элементы.Упаковка.Имя;
	ДопПараметрыУсловногоОформления.ПутьКПолюУпаковка = "ТаблицаДокумента.ЕдиницаИзмерения";
	НоменклатураСервер.УстановитьУсловноеОформлениеЕдиницИзмерения(ЭтотОбъект, ДопПараметрыУсловногоОформления);
...
&НаСервере
Процедура ПриИзмененииНоменклатуры(ИдентификаторСтроки, ОчищатьСтатьюРасходов = Истина)
	...
	Если ОчищатьСтатьюРасходов Тогда
		ТекущиеДанные.СтатьяРасходов = Неопределено;
	КонецЕсли;
	
	Если Не Объект.ТипДокумента = Перечисления.ТипыДокументовРаспознаваниеДокументов.ТОРГ12 
		И Объект.Направление = Перечисления.НаправленияРаспознанногоДокумента.Исходящий Тогда 
		Элементы.СоздатьАктВыполненныхРабот.Видимость = 
			((ЕстьУслуги Или ЕстьРаботы) И ПравоДоступа("Добавление", Метаданные.Документы.АктВыполненныхРабот));
	КонецЕсли;
...
Процедура УстановитьПараметрыВыбораНоменклатуры()
	...
	НовыйПараметр = Новый ПараметрВыбора("Отбор.ТипНоменклатуры", МассивПараметровВыбораНоменклатуры(
		Объект.ТипДокумента));
...
Функция МассивПараметровВыбораНоменклатуры(Знач ТипДокумента)
		
	НовыйМассив = Новый Массив();
		
	Если ТипДокумента
		= ПредопределенноеЗначение("Перечисление.ТипыДокументовРаспознаваниеДокументов.АктОбОказанииУслуг") Тогда
...
Процедура УстановитьДоступностьКомандСозданияПоПравам()
	...
	Иначе
		...
		ДобавлениеРеализацияУслуг = ПравоДоступа("Добавление", Метаданные.Документы.РеализацияУслугПрочихАктивов);
		...
		ИначеЕсли Объект.Направление = Перечисления.НаправленияРаспознанногоДокумента.Исходящий Тогда
			Элементы.СоздатьАктВыполненныхРабот.Видимость = 
				((ЕстьУслуги Или ЕстьРаботы) И ПравоДоступа("Добавление", Метаданные.Документы.АктВыполненныхРабот));
		КонецЕсли;
...
КонецФункции // ЗаголовокВидаДокумента()
...
Процедура ПослеЗакрытияФормыСозданияЭлементаНаСервере(Знач НоменклатураСсылка, Знач ДополнительныеПараметры)
	...
	СтруктураОбъект = Новый Структура("СтатусУказанияСерий, Склад", 0, Объект.РеквизитыДокумента[7].Значение);
	СтруктураПоиска = Новый Структура("Номенклатура, Характеристика, ТСклад",
		СтрокаТаблицыДокумента.Номенклатура, СтрокаТаблицыДокумента.Характеристика, СтрокаТаблицыДокумента.ТСклад);
	ЗаполнитьСтатусыУказанияСерий(СтруктураОбъект, СтруктураПоиска);
