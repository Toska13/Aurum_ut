...
Функция СоответствиеКонтрольныхРегистров(ВыбранныеРазделы = Неопределено) Экспорт
	...
	Если ВыбранныеРазделы.Найти("Себестоимость") <> Неопределено Тогда // Себестоимость
		...
		РегистрыКонтроля.Вставить(Метаданные.РегистрыНакопления.ОтклоненияВСтоимостиТоваров.ПолноеИмя(), "Себестоимость");
...
Функция ТестироватьЗаПериод(ПараметрыОбработки)
	...
	Для каждого ЭтапТестирования Из ПараметрыОбработки.ЭтапыТестирования Цикл
		...
		КонецЕсли;
		ИсключаемыеПоля = Новый Структура("ИдентификаторФинЗаписи, Содержание");
		ИсключаемыеПоля.Вставить("СпособОпределенияСебестоимости");
		ИсключаемыеПоля.Вставить("ДатаОпределенияСебестоимости");
		ИсключаемыеПоля.Вставить("ОперацияУчетаСебестоимости");
		ИсключаемыеПоля.Вставить("НаборКорректировок");
		ПараметрыСравненияДанных.Вставить("ИсключаемыеПоля", ИсключаемыеПоля);
		...
		Для Каждого РазделРасчета Из ВыбранныеРазделы Цикл
			...
			Если РазделРасчета = "ВзаиморасчетыСПартнерами" Тогда
				...
				Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
					РегистрыСведений.ЗаданияКЗакрытиюМесяца.СоздатьЗаписьРегистра(Период,, ОрганизацииПоПериоду, Справочники.ОперацииЗакрытияМесяца.ФормированиеДвиженийПоРасчетамСПартнерамиИПереоценкаРасчетов);
				Иначе
			...
			Если РазделРасчета = "НДС" Тогда
				...
				Пока ВыборкаДокументовРаспределенияНДС.Следующий() Цикл
					РегистрыСведений.ЗаданияКЗакрытиюМесяца.СоздатьЗаписьРегистра(Период,, ВыборкаДокументовРаспределенияНДС.Организация, Справочники.ОперацииЗакрытияМесяца.РаспределениеНДС);
				КонецЦикла;
