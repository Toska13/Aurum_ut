...
Процедура Условие2ПроверкаУсловия(ТочкаМаршрутаБизнесПроцесса, Результат)
	...
	Результат = АУ_НаПроверке;
	
КонецПроцедуры

Функция СрокИсполненияЗадачиДляДополнительнойПроверки()

	Возврат АУ_СрокПроверки;

КонецФункции

Процедура Действие1ПриСозданииЗадач(ТочкаМаршрутаБизнесПроцесса, ФормируемыеЗадачи, Отказ)
	
	Если АУ_Проверяющий.Пустая() Тогда
		Отказ = Истина;
		ОбщегоНазначения.СообщитьПользователю("Не заполнен проверяющий !");
		Возврат;
	КонецЕсли;
	...
	Для Каждого Задача Из ФормируемыеЗадачи Цикл
		...
		Задача.Автор = Автор;
		Если ТипЗнч(АУ_Проверяющий) = Тип("СправочникСсылка.РолиИсполнителей") Тогда
			Задача.РольИсполнителя = АУ_Проверяющий;
			Задача.ОсновнойОбъектАдресации = АУ_ОсновнойОбъектАдресацииПроверяющий;
			Задача.ДополнительныйОбъектАдресации = АУ_ДополнительныйОбъектАдресацииПроверяющий;
		Иначе
			Задача.Исполнитель = АУ_Проверяющий;
		КонецЕсли;

		Задача.Наименование = НаименованиеЗадачиДляПроверки();
		Задача.СрокИсполнения = СрокИсполненияЗадачиДляДополнительнойПроверки();
		Задача.Важность = Важность;
		Задача.Предмет = Предмет;
