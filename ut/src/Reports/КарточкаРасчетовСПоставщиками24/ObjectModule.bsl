...
Функция ТекстЗапроса()
	...
	|	//  ПлановаяОплата суммируется в валюте расчетов, так как содержит данные плановых и фактических расечтов
	...
	|		АналитикаУчета.Контрагент                                      КАК Контрагент,
	|		АналитикаУчета.Договор                                         КАК Договор,
	|		ВЫБОР КОГДА НЕ РасчетыПоСрокамОстатки.ОбъектРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности
	|			ТОГДА АналитикаУчета.НаправлениеДеятельностиАналитики
	|			ИНАЧЕ РасчетыПоСрокамОстатки.ОбъектРасчетов.НаправлениеДеятельности
	...
	|		АналитикаУчета.Контрагент                       КАК Контрагент,
	|		АналитикаУчета.Договор                          КАК Договор,
	|		ВЫБОР КОГДА НЕ РасчетыПланОплатОстатки.ОбъектРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности
	|			ТОГДА АналитикаУчета.НаправлениеДеятельностиАналитики
	|			ИНАЧЕ РасчетыПланОплатОстатки.ОбъектРасчетов.НаправлениеДеятельности
	|		КОНЕЦ                                           КАК НаправлениеДеятельности,
	...
	|		АналитикаУчета.Контрагент                         КАК Контрагент,
	|		АналитикаУчета.Договор                            КАК Договор,
	|		ВЫБОР КОГДА НЕ РасчетыПланПоставокОстатки.ОбъектРасчетов.РегистрыВзаиморасчетовНеДелятсяПоНаправлениямДеятельности
	|			ТОГДА АналитикаУчета.НаправлениеДеятельностиАналитики
	|			ИНАЧЕ РасчетыПланПоставокОстатки.ОбъектРасчетов.НаправлениеДеятельности
	|		КОНЕЦ                                             КАК НаправлениеДеятельности,
	...
	|				И СУММА(Расчеты.Зачтено) = 0
	|		ТОГДА
	|			//Плановые операции
	|			ВЫБОР КОГДА СУММА(Расчеты.ПлановаяОплата) = 0 
	|					ТОГДА СУММА(Расчеты.ПлановаяПоставка)
	|				ИНАЧЕ СУММА(Расчеты.ПлановаяОплата)
	|			КОНЕЦ
	|		ИНАЧЕ 
	|			//Фактические операции
	|			ВЫБОР КОГДА СУММА(Расчеты.Оплачено) > СУММА(Расчеты.Поставлено) И СУММА(Расчеты.Оплачено) > 0
	...
	|				КОГДА СУММА(Расчеты.Поставлено) <= СУММА(Расчеты.Оплачено) И СУММА(Расчеты.Поставлено) < 0
	|					ТОГДА СУММА(Расчеты.Поставлено)
	|				// АЗДН, движение накладной по заказу
	|				КОГДА СУММА(Расчеты.Зачтено) <> 0
	|					ТОГДА СУММА(Расчеты.Зачтено)
	...
	|	
	|	Расчеты.Заказано                    КАК Заказано,
	|	Расчеты.Оплачено                    КАК Оплачено,
	|	Расчеты.Поставлено                  КАК Поставлено,
	|	Расчеты.Зачтено                     КАК Зачтено,
	|	Расчеты.СписаниеПереоценка          КАК СписаниеПереоценка,
	|	
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&ПоставленоПоставщиком", ВзаиморасчетыОтчеты.ШаблонПоляОтгружено());
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&ЗачтеноПоставщиком", ВзаиморасчетыОтчеты.ШаблонПоляЗачтено());
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&ОплаченоПоставщику", ВзаиморасчетыОтчеты.ШаблонПоляОплачено());
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&СписаниеПереоценка", ВзаиморасчетыОтчеты.ШаблонПоляСписаниеПереоценка());
