...
Процедура СформироватьОтчет(Знач ПараметрыОтчета, АдресХранилища) Экспорт
	...
	РеквизитыДокументаРаспределения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПараметрыОтчета.ДокументРаспределения, "Организация, Дата, Исправление, ИсправляемыйДокумент");
	ПараметрыОтчета.Вставить("ДатаРаспределения", РеквизитыДокументаРаспределения.Дата);
	Если РеквизитыДокументаРаспределения.Исправление = Истина Тогда 
		РеквизитыДокументаРаспределения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РеквизитыДокументаРаспределения.ИсправляемыйДокумент, "Организация, Дата");
	КонецЕсли;
	...
	Если ПараметрыОтчета.ДатаРаспределения = Неопределено Тогда 
		ПараметрыОтчета.Вставить("ДатаРаспределения", Дата(3999,12,31));
	КонецЕсли;
...
Функция ПериодыБазы(ПараметрыОтчета)
	...
	Если НЕ ПараметрыОтчета.ВыручкаПоДокументам Тогда
		...
		|	И НЕ(ПартииНДСКРаспределению.ВидДеятельностиНДС = ПартииНДСКРаспределению.КорВидДеятельностиНДС
...
Процедура ВывестиВОтчетДанныеПоБазеРаспределения(ПериодБазы, ПараметрыОтчета, ДокументРезультат, Макет)
	...
	|	ПО
	|		РаспределениеНДС.Ссылка = ДокументыРаспределения.Документ
	|
	|СГРУППИРОВАТЬ ПО
	...
	Запрос.УстановитьПараметр("ДатаРаспределения", 	КонецДня(ПараметрыОтчета.ДатаРаспределения));
	...
	НачалоПериода = ПериодБазы;
	РаспределениеНДСЛокализация.УстановитьНачалоПериода(НачалоПериода, ПериодБазы, ПараметрыОтчета.Организация);
	ПериодБазыПредставление = БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(НачалоПериода, ПериодБазы, Истина);
...
Процедура ВывестиВОтчетДанныеПоРаспределениюКосвенныхРасходов(ПериодБазы, ПараметрыОтчета, ДокументРезультат, Макет)
	...
	|		Документ.РаспределениеНДС КАК РаспределениеНДС
	|	ПО
	|		ПартииНДСКРаспределению.Регистратор = РаспределениеНДС.Ссылка
	|ГДЕ
	...
	|	РегистрНакопления.ПартииНДСКРаспределению КАК ПартииНДСКРаспределению
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		ДокументыРаспределения КАК ДокументыРаспределенияНДС
	|	ПО
	|		ПартииНДСКРаспределению.Регистратор = ДокументыРаспределенияНДС.Документ
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		Документ.СчетФактураПолученныйНалоговыйАгент КАК СчетФактураПолученныйНА
	|	ПО 
	...
	|ГДЕ
	|	НЕ ПартииНДСКРаспределению.Сторно
	|ИНДЕКСИРОВАТЬ ПО
	...
	|	ПО 
	|		ПартииНДСКРаспределению.ДокументПоступленияРасходов = ТаблицаСчетовФактур.ДокументПоступления
	|ГДЕ
	|	НЕ ПартииНДСКРаспределению.Сторно
	|	И ВЫБОР 
	|		КОГДА ПартииНДСКРаспределению.ПериодБазы <> ДАТАВРЕМЯ(1,1,1)
