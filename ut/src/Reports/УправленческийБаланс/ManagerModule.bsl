...
Процедура ВывестиАктуальностьОтчета(КомпоновщикНастроек, ДокументРезультат) Экспорт
	...
	ОтборОрганизаций = ФинансоваяОтчетностьСервер.НайтиЭлементОтбора(КомпоновщикНастроек.Отбор,"Организация");
	Если ТипЗнч(ОтборОрганизаций) = Тип("ЭлементОтбораКомпоновкиДанных") И ОтборОрганизаций.Использование Тогда
		Если ТипЗнч(ОтборОрганизаций.ПравоеЗначение) = Тип("СписокЗначений") Тогда
			МассивОрганизаций = ОтборОрганизаций.ПравоеЗначение.ВыгрузитьЗначения();
		Иначе
			МассивОрганизаций = ОбщегоНазначенияУТКлиентСервер.Массив(ОтборОрганизаций.ПравоеЗначение);
		КонецЕсли;
	КонецЕсли;
	...
	СостояниеРасчета = РасчетСебестоимостиПрикладныеАлгоритмы.ТекущееСостояниеРасчета(
							ПериодОтчета.ДатаОкончания, МассивОрганизаций, Справочники.ОперацииЗакрытияМесяца.РасчетПартийИСебестоимости);
	...
	ИначеЕсли ЕстьЗаданияНеоперативнойОчередиОтражениеДвиженийПоУправленческомуБалансу(МассивОрганизаций, ПериодОтчета) Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Данные отчета могут быть не актуальны, так как имеются невыполненные задания неоперативной очереди с видом ""%1"" (регистр ""%2"")'", ОбщегоНазначения.КодОсновногоЯзыка()),
					Справочники.ВидыНеоперативныхЗаданий.ОтражениеДвиженийПоУправленческомуБалансу,
					Метаданные.РегистрыСведений.ЗаданияНеоперативногоДопроведенияДокументов.Синоним);
...
// Проверяет наличие заданий неоперативной очереди для отражения движений по управленческому балансу за период.
//
// Параметры:
//  Организации - Неопределено, Массив из СправочникСсылка.Организации - список организаций для проверки
//  ПериодОтчета - СтандартныйПериод - период, для которого выполняется проверка.
//
// Возвращаемое значение:
//   Булево - Истина, если есть хотя бы одно задание.
Функция ЕстьЗаданияНеоперативнойОчередиОтражениеДвиженийПоУправленческомуБалансу(Организации, ПериодОтчета)
	
	Запрос = Новый Запрос;
	Запрос.Текст = "
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ИСТИНА КАК ЕстьЗадания
	|ИЗ
	|	РегистрСведений.ЗаданияНеоперативногоДопроведенияДокументов КАК РегистрОчереди
	|ГДЕ
	|	РегистрОчереди.ВидЗадания = ЗНАЧЕНИЕ(Справочник.ВидыНеоперативныхЗаданий.ОтражениеДвиженийПоУправленческомуБалансу)
	|	И РегистрОчереди.ПериодЗадания <= &КонецПериода
	|	И (РегистрОчереди.Организация В (&МассивОрганизаций)
	|	ИЛИ &ПоВсемОрганизациям)";
	
	Запрос.УстановитьПараметр("КонецПериода", ?(ЗначениеЗаполнено(ПериодОтчета.ДатаОкончания),
		ПериодОтчета.ДатаОкончания, Дата(3999, 12, 31)));
	Запрос.УстановитьПараметр("МассивОрганизаций", Организации);
	Запрос.УстановитьПараметр("ПоВсемОрганизациям", Не ЗначениеЗаполнено(Организации));
