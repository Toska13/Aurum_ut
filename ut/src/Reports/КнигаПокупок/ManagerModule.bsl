...
Процедура СформироватьОсновнойРаздел(СтруктураПараметров, Результат);
	...
	Если СтруктураПараметров.ВерсияПостановленияНДС1137 >= 9 Тогда
		Макет = ПолучитьОбщийМакет("КнигаПокупок26");
	...
	ИначеЕсли СтруктураПараметров.ВерсияПостановленияНДС1137 = 8 Тогда
		Макет = ПолучитьОбщийМакет("КнигаПокупок2026");
		СтруктураСекций.Вставить("СекцияСтрокаРНПТ", Макет.ПолучитьОбласть("СтрокаРНПТ"));
...
Процедура СформироватьДополнительныеЛисты(СтруктураПараметров, Результат)
	...
	Пока ИтогПоПериодам.Следующий() Цикл;
		...
		Пока ИтогПоПериодамКорректировки.Следующий() Цикл
			...
			ИначеЕсли СтруктураПараметров.ВерсияПостановленияНДС1137 >= 9 Тогда
				Макет = ПолучитьОбщийМакет("КнигаПокупок26");
				СтруктураСекций.Вставить("СекцияСтрокаРНПТ", Макет.ПолучитьОбласть("СтрокаРНПТ"));
			ИначеЕсли СтруктураПараметров.ВерсияПостановленияНДС1137 = 8 Тогда
				Макет = ПолучитьОбщийМакет("КнигаПокупок2026");
...
Процедура ИнициализироватьТаблицыДанныхНДС(СтруктураПараметров)
	...
	Если СтруктураПараметров.ЗаполнениеДокумента
		...
		Если СтруктураПараметров.ВерсияПостановленияНДС1137 >= 8 Тогда
			Записи.Колонки.Добавить("СвСчФПродРанОп", Новый ОписаниеТипов("Массив"));
		КонецЕсли;
	...
	Иначе 
		...
		Если СтруктураПараметров.ЗаполнениеДекларации Тогда
			...
			Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
				Раздел8.Колонки.Добавить("СвЧастСчФ", Новый ОписаниеТипов("Массив"));
			КонецЕсли;
		...
		Пока ИтогПоКонтрагенту.Следующий() Цикл
			...
			Пока ИтогПоДокументу.Следующий() Цикл
				...
				Пока ИтогПоДоговору.Следующий() Цикл
					ИтогПоСторноДопЛиста = ИтогПоДоговору.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
					Пока ИтогПоСторноДопЛиста.Следующий() Цикл
					ИтогПоКоду = ИтогПоСторноДопЛиста.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
					Пока ИтогПоКоду.Следующий() Цикл
						ЗаписьКниги = ИтогПоКоду.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
						Пока ЗаписьКниги.Следующий() Цикл
						...
						КонецЦикла; // ЗаписьКниги.Следующий()
					КонецЦикла; // ИтогПоКоду.Следующий()
					КонецЦикла; // ИтогПоСторноДопЛиста.Следующий()
		...
		Пока ИтогПоДокументу.Следующий() Цикл
			...
			Пока ИтогПоДоговору.Следующий() Цикл
				...
				Пока ИтогПоКоду.Следующий() Цикл
					...
					КонецЦикла; // ЗаписьКниги.Следующий()
				КонецЦикла; // ИтогПоКоду.Следующий()
				КонецЦикла; // ИтогПоСторноДопЛиста
			КонецЦикла; // ИтогПоДоговору.Следующий()
		КонецЦикла; // ИтогПоДокументу.Следующий()
	КонецЕсли;
...
Процедура ЗаполнитьСтрокуКнигиПокупок(
	...
	Для Каждого СчетФактура Из СчетаФактуры Цикл
		...
		Если СтрокаДополнительнойИнформации <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(СтрокаДополнительнойИнформации.СвРегНом) И Не ЗаписьКниги.НаАванс Тогда
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивСвРегНом, СтрокаДополнительнойИнформации.СвРегНом);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СтрокаДополнительнойИнформации.СвСФНаОтгрузку) Тогда
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивСвСФНаОтгрузку, СтрокаДополнительнойИнформации.СвСФНаОтгрузку);
			КонецЕсли;
	...
	СчетаФактурыТекст = "";
	Для каждого ЭлементКоллекции Из МассивСвСФНаОтгрузку Цикл
		Если ЗначениеЗаполнено(ЭлементКоллекции.НомерСчетаФактуры) Тогда 
			СчетаФактурыНомерДата = СтрШаблон(НСтр("ru='%1 от %2'"),
				ЭлементКоллекции.НомерСчетаФактуры, Формат(ЭлементКоллекции.ДатаСчетаФактуры, "ДФ=dd.MM.yyyy"));
			СчетаФактурыТекст = СчетаФактурыТекст 
				+ ?(Не ЗначениеЗаполнено(СчетаФактурыТекст), "", ";" + Символы.ПС) + СчетаФактурыНомерДата;
			// Структура для выгрузки книги покупок
			СтруктураСвСчФПродРанОп = Новый Структура("НомСчФПродРанО,ДатаСчФПродРанО");
			СтруктураСвСчФПродРанОп.НомСчФПродРанО = ЭлементКоллекции.НомерСчетаФактуры;
			СтруктураСвСчФПродРанОп.ДатаСчФПродРанО = Формат(ЭлементКоллекции.ДатаСчетаФактуры, "ДФ=dd.MM.yyyy");
			СвСчФПродРанОп.Добавить(СтруктураСвСчФПродРанОп);
			// Структура для выгрузки декларации по НДС
			СтруктураСвЧастСчФ = Новый Структура("НомЧастСчФ,ДатаЧастСчФ");
			СтруктураСвЧастСчФ.НомЧастСчФ  = ЭлементКоллекции.НомерСчетаФактуры;
			СтруктураСвЧастСчФ.ДатаЧастСчФ = Формат(ЭлементКоллекции.ДатаСчетаФактуры, "ДФ=dd.MM.yyyy");
			СвЧастСчФ.Добавить(СтруктураСвЧастСчФ);
		КонецЕсли;
	КонецЦикла;
	
	СоставСтроки.Вставить("НомерОтДатаПриОтгрузке", СчетаФактурыТекст);
	...
	Если СтруктураПараметров.ЗаполнениеДекларации Тогда
		...
		Если (СтруктураПараметров.ПравилаЗаполнения = "2020Кв4"
			...
			ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
			ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1"
			ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2026Кв1")
	...
	ИначеЕсли СтруктураПараметров.ЗаполнениеДокумента
		...
		// Графа 7а с 2026 года
		СоставСтроки.Вставить("СвСчФПродРанОп", СвСчФПродРанОп);
...
Функция ТекстЗапросаПодготовленныеЗаписиКнигиПокупок(СтруктураПараметров)
	...
	|					КОГДА ЗаписиКнигиПокупок.СторнирующаяЗаписьДопЛиста
	|						ТОГДА -ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.ВсегоПокупокВВалюте, ЗаписиКнигиПокупок.ВсегоПокупок)
	...
	|	ЗаписиКнигиПокупок.СторнирующаяЗаписьДопЛиста КАК СторнирующаяЗаписьДопЛиста,
...
Функция ТекстЗапросаДополнительныйЛистКнигиПокупок(СтруктураПараметров)
	...
	|	ПодготовленныеЗаписиКнигиПокупок.СторнирующаяЗаписьДопЛиста КАК СторнирующаяЗаписьДопЛиста,
	|	ПодготовленныеЗаписиКнигиПокупок.ИсправленныйСчетФактура КАК ИсправленныйСчетФактура,
	|	ПодготовленныеЗаписиКнигиПокупок.ДатаИсправления КАК ДатаИсправления,
	|	ПодготовленныеЗаписиКнигиПокупок.НомерИсправления КАК НомерИсправления,
	|	ПодготовленныеЗаписиКнигиПокупок.ДатаКорректировки КАК ДатаКорректировки,
...
Функция ПолучитьДополнительнуюИнформациюПоСчетуФактуре(СтруктураПараметров)
	...
	УчетНДСПереопределяемый.ДополнитьИнформациейОСчетахФактурахНаОтгрузку(ДополнительнаяИнформация, СтруктураПараметров, Запрос);
...
Функция НовыйДополнительнаяИнформацияПоСчетуФактуре()
	...
	ДополнительнаяИнформация.Колонки.Добавить("СвСФНаОтгрузку", Новый ОписаниеТипов("Массив"));
