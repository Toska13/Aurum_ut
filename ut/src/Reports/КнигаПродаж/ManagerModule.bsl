...
Процедура ПодготовитьОтчет(СтруктураПараметров) Экспорт
	...
	Если НЕ (СтруктураПараметров.ВыводитьТолькоДопЛисты И СтруктураПараметров.ФормироватьДополнительныеЛисты) Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СервисСверкиРасчетов") Тогда
			МодульСверкаДеклараций = ОбщегоНазначения.ОбщийМодуль("СервисСверкиРасчетовСверкаДекларацийНДС");
			МодульСверкаДеклараций.ЗапомнитьДанныеОтчета(СтруктураПараметров, Результат);
		КонецЕсли;
	...
	Если СтруктураПараметров.ФормироватьДополнительныеЛисты Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СервисСверкиРасчетов") Тогда
			МодульСверкаДеклараций = ОбщегоНазначения.ОбщийМодуль("СервисСверкиРасчетовСверкаДекларацийНДС");
			МодульСверкаДеклараций.ЗапомнитьДанныеДопЛистов(СтруктураПараметров, Результат);
		КонецЕсли;
...
Функция ПолучитьИтогиЗаПериодКнигаПродаж(СтруктураПараметров) Экспорт
	...
	|	ЕСТЬNULL(СУММА(ЗаписиКнигиПродаж.СуммаБезНДС22), 0) КАК СуммаБезНДС22,
	|	ЕСТЬNULL(СУММА(ЗаписиКнигиПродаж.НДС22), 0) КАК НДС22, 
	|	ЕСТЬNULL(СУММА(ЗаписиКнигиПродаж.СуммаБезНДС20) + СУММА(ЗаписиКнигиПродаж.СуммаБезНДС18), 0) КАК СуммаБезНДС20Или18,
	|	ЕСТЬNULL(СУММА(ЗаписиКнигиПродаж.НДС20) + СУММА(ЗаписиКнигиПродаж.НДС18), 0) КАК НДС20Или18,
	...
	|		ВЫБОР
	|			КОГДА НДСЗаписиКнигиПродажОбороты.СтавкаНДС В (&СтавкиНДС22)
	...
	СтавкиНДС22 = Новый Массив();
	СтавкиНДС22.Добавить(Перечисления.СтавкиНДС.НДС22);
	СтавкиНДС22.Добавить(Перечисления.СтавкиНДС.НДС22_122);
	...
	Запрос.УстановитьПараметр("СтавкиНДС22",  СтавкиНДС22);
	...
	Иначе
		...
		ИтогЗаПериод.Вставить("СуммаБезНДС22",      0);
		ИтогЗаПериод.Вставить("НДС22",              0);
		ИтогЗаПериод.Вставить("СуммаБезНДС20",      0);
		ИтогЗаПериод.Вставить("НДС20",              0);
		ИтогЗаПериод.Вставить("СуммаБезНДС20Или18", 0);
		ИтогЗаПериод.Вставить("НДС20Или18",         0);
...
Процедура СформироватьОсновнойРаздел(СтруктураПараметров, Результат);
	...
	Если СтруктураПараметров.ВерсияПостановленияНДС1137 >= 9 Тогда
		Макет = ПолучитьОбщийМакет("КнигаПродаж26");
		СтруктураСекций.Вставить("СекцияСтрокаРНПТ", Макет.ПолучитьОбласть("СтрокаРНПТ"));
	ИначеЕсли СтруктураПараметров.ВерсияПостановленияНДС1137 = 8 Тогда
		Макет = ПолучитьОбщийМакет("КнигаПродаж2026");
		СтруктураСекций.Вставить("СекцияСтрокаРНПТ", Макет.ПолучитьОбласть("СтрокаРНПТ"));
	ИначеЕсли ПрименяетсяПостановление1448 Тогда
	...
	Если СтруктураПараметров.ЗаполнениеДекларации Тогда
		...
		ИтогиРаздел9 = Новый Структура(
			"СтПродБезНДС22,СтПродБезНДС20Или18,СтПродБезНДС20,СтПродБезНДС18,СтПродБезНДС10,СтПродБезНДС0, СтПродБезНДС7, СтПродБезНДС5,
			|СумНДСВсКПр22,СумНДСВсКПр20Или18,СумНДСВсКПр20,СумНДСВсКПр18,СумНДСВсКПр10,СтПродОсвВсКПр, СумНДСВсКПр7, СумНДСВсКПр5");
		
		ИтогиРаздел9.СтПродБезНДС22      = СтрокаИтогПоОрганизации.СуммаБезНДС22;
		ИтогиРаздел9.СтПродБезНДС20Или18 = СтрокаИтогПоОрганизации.СуммаБезНДС20Или18;
		...
		ИтогиРаздел9.СумНДСВсКПр22      = СтрокаИтогПоОрганизации.НДС22;
		ИтогиРаздел9.СумНДСВсКПр20Или18 = СтрокаИтогПоОрганизации.НДС20Или18;
		...
		Если СтруктураПараметров.ВерсияПостановленияНДС1137 >= 8 Тогда
			ИтогиРаздел9.СтПродБезНДС20 = СтрокаИтогПоОрганизации.СуммаБезНДС20Или18;
			ИтогиРаздел9.СумНДСВсКПр20  = СтрокаИтогПоОрганизации.НДС20Или18;
		КонецЕсли;
	...
	ИначеЕсли СтруктураПараметров.ЗаполнениеДокумента
		...
		ОбщиеСведения = Новый Структура(
			"СтТовРубКоп22,СтТовРубКоп20,СтТовРубКоп18_20,СтТовРубКоп18,СтТовРубКоп10,СтТовРубКоп0,
			|СумНДСРубКоп22,СумНДСРубКоп20,СумНДСРубКоп18_20,СумНДСРубКоп18,СумНДСРубКоп10,СтПродОсвНДСРубКоп");
		ОбщиеСведения.СтТовРубКоп22      = СтрокаИтогПоОрганизации.СуммаБезНДС22;
		ОбщиеСведения.СтТовРубКоп20      = СтрокаИтогПоОрганизации.СуммаБезНДС20;
		ОбщиеСведения.СтТовРубКоп18_20   = СтрокаИтогПоОрганизации.СуммаБезНДС20Или18;
		...
		ОбщиеСведения.СумНДСРубКоп22     = СтрокаИтогПоОрганизации.НДС22;
		ОбщиеСведения.СумНДСРубКоп20     = СтрокаИтогПоОрганизации.НДС20;
		ОбщиеСведения.СумНДСРубКоп18_20  = СтрокаИтогПоОрганизации.НДС20Или18;
...
Процедура СформироватьДополнительныеЛисты(СтруктураПараметров, Результат)
	...
	Если СтруктураПараметров.ЗаполнениеДекларации Тогда
		...
		ИтогиРаздел91 = Новый Структура(
			"ИтСтПродКПр22,ИтСтПродКПр20Или18,ИтСтПродКПр20,ИтСтПродКПр18,ИтСтПродКПр10,ИтСтПродКПр7,ИтСтПродКПр5,ИтСтПродКПр0,
			|СумНДСИтКПр22,СумНДСИтКПр20Или18,СумНДСИтКПр20,СумНДСИтКПр18,СумНДСИтКПр10,СумНДСИтКПр7,СумНДСИтКПр5,ИтСтПродОсвКПр,
			|СтПродВсП1Р9_22,СтПродВсП1Р9_20Или18,СтПродВсП1Р9_20,СтПродВсП1Р9_18,СтПродВсП1Р9_10,СтПродВсП1Р9_7,СтПродВсП1Р9_5,СтПродВсП1Р9_0,
			|СумНДСВсП1Р9_22,СумНДСВсП1Р9_20Или18,СумНДСВсП1Р9_20,СумНДСВсП1Р9_18,СумНДСВсП1Р9_10,СумНДСВсП1Р9_7,СумНДСВсП1Р9_5,СтПродОсвП1Р9Вс");
		...
		ИтогиРаздел91.ИтСтПродКПр22      = ИтогПоКнигеПродаж.СуммаБезНДС22;
		ИтогиРаздел91.ИтСтПродКПр20Или18 = ИтогПоКнигеПродаж.СуммаБезНДС20Или18;
		...
		ИтогиРаздел91.СумНДСИтКПр22      = ИтогПоКнигеПродаж.НДС22;
		ИтогиРаздел91.СумНДСИтКПр20Или18 = ИтогПоКнигеПродаж.НДС20Или18;
		...
		Если СтруктураПараметров.ВерсияПостановленияНДС1137 >= 8 Тогда
			ИтогиРаздел91.ИтСтПродКПр20 = ИтогПоКнигеПродаж.СуммаБезНДС20Или18;
			ИтогиРаздел91.СумНДСИтКПр20 = ИтогПоКнигеПродаж.НДС20Или18;
		КонецЕсли;
	...
	Пока ИтогПоПериодам.Следующий() Цикл
		...
		Пока ИтогПоПериодамКорректировки.Следующий() Цикл;
			...
			СтруктураПараметров.Вставить("ВерсияПостановленияНДС1137", ВерсияПостановленияНДС1137);
			ПрименяетсяПостановление1448 = УчетНДСПереопределяемый.ПрименяетсяПостановление1448(ИтогПоПериодамКорректировки.КонецНалоговогоПериода);
			...
			ИначеЕсли СтруктураПараметров.ВерсияПостановленияНДС1137 = 8 Тогда
				...
				СтруктураСекций.Вставить("СекцияСтрокаРНПТ", Макет.ПолучитьОбласть("СтрокаРНПТ"));
			ИначеЕсли ПрименяетсяПостановление1448 Тогда
				Макет = ПолучитьОбщийМакет("КнигаПродаж1448");
				СтруктураСекций.Вставить("СекцияСтрокаРНПТ", Макет.ПолучитьОбласть("СтрокаРНПТ"));
			ИначеЕсли СтруктураПараметров.ВерсияПостановленияНДС1137 >= 5 Тогда
				Макет = ПолучитьОбщийМакет("КнигаПродаж534"); // Общий макет для книги и доп. листов
				СтруктураСекций.Вставить("СекцияСтрокаРНПТ", Макет.ПолучитьОбласть("СтрокаРНПТ"));
			...
			Если СтруктураПараметров.ЗаполнениеДокумента
				...
				Итого = Новый Структура(
					"СтТовРубКоп22,СтТовРубКоп20,СтТовРубКоп18_20,СтТовРубКоп18,СтТовРубКоп10,СтТовРубКоп0,
					|СумНДСРубКоп22,СумНДСРубКоп20,СумНДСРубКоп18_20,СумНДСРубКоп18,СумНДСРубКоп10,СтПродОсвНДСРубКоп");
				Итого.СтТовРубКоп22      = ИтогЗаПериод.СуммаБезНДС22;
				Итого.СтТовРубКоп20      = ИтогЗаПериод.СуммаБезНДС20;
				Итого.СтТовРубКоп18_20   = ИтогЗаПериод.СуммаБезНДС20Или18;
				...
				Итого.СумНДСРубКоп22     = ИтогЗаПериод.НДС22;
				Итого.СумНДСРубКоп20     = ИтогЗаПериод.НДС20; 
				Итого.СумНДСРубКоп18_20  = ИтогЗаПериод.НДС20Или18;
				...
				Если СтруктураПараметров.ВерсияПостановленияНДС1137 >= 8 Тогда
					Итого.СтТовРубКоп20  = ИтогЗаПериод.СуммаБезНДС20Или18;
					Итого.СумНДСРубКоп20 = ИтогЗаПериод.НДС20Или18;
				КонецЕсли;
			...
			Если СтруктураПараметров.ЗаполнениеДокумента
				...
				Всего = Новый Структура(
					"СтТовРубКоп22,СтТовРубКоп20,СтТовРубКоп18_20,СтТовРубКоп18,СтТовРубКоп10,СтТовРубКоп0,
					|СумНДСРубКоп22,СумНДСРубКоп20,СумНДСРубКоп18_20,СумНДСРубКоп18,СумНДСРубКоп10,СтПродОсвНДСРубКоп");
				Всего.СтТовРубКоп22      = ИтогЗаПериод.СуммаБезНДС22;
				Всего.СтТовРубКоп20      = ИтогЗаПериод.СуммаБезНДС20;
				Всего.СтТовРубКоп18_20   = ИтогЗаПериод.СуммаБезНДС20Или18;
				...
				Всего.СумНДСРубКоп22     = ИтогЗаПериод.НДС22;
				Всего.СумНДСРубКоп20     = ИтогЗаПериод.НДС20;
				Всего.СумНДСРубКоп18_20  = ИтогЗаПериод.НДС20Или18;
	...
	Если СтруктураПараметров.ЗаполнениеДекларации Тогда
		...
		Если СтруктураПараметров.СчетчикСтрокТекущегоСегмента <> 0 Тогда
			...
			Если ПерваяСтрокаСегмента = 1 Тогда 
				...
				КонецЕсли;
				Если ТабличныйДокумент.Области.Найти("СуммаБезНДС18Приложение") <> Неопределено Тогда
				...
				КонецЕсли;
				Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" 
				 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
				...
				КонецЕсли;
				Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
					ТабличныйДокумент.Область("СуммаБезНДС22Приложение").Значение      = ИтогЗаПериод.СуммаБезНДС22;
					ТабличныйДокумент.Область("НДС22Приложение").Значение              = ИтогЗаПериод.НДС22;
					ТабличныйДокумент.Область("СуммаБезНДС20Или18Приложение").Значение = ИтогЗаПериод.СуммаБезНДС20Или18;
					ТабличныйДокумент.Область("НДС20Или18Приложение").Значение         = ИтогЗаПериод.НДС20Или18;
				КонецЕсли;
		...
		ИтогиРаздел91.СтПродВсП1Р9_22      = ИтогЗаПериод.СуммаБезНДС22;
		ИтогиРаздел91.СтПродВсП1Р9_20Или18 = ИтогЗаПериод.СуммаБезНДС20Или18;
		...
		ИтогиРаздел91.СумНДСВсП1Р9_22      = ИтогЗаПериод.НДС22;
		ИтогиРаздел91.СумНДСВсП1Р9_20Или18 = ИтогЗаПериод.НДС20Или18;
		...
		Если СтруктураПараметров.ВерсияПостановленияНДС1137 >= 8 Тогда
			ИтогиРаздел91.СтПродВсП1Р9_20 = ИтогЗаПериод.СуммаБезНДС20Или18;
			ИтогиРаздел91.СумНДСВсП1Р9_20 = ИтогЗаПериод.НДС20Или18;
		КонецЕсли;
...
Процедура ВывестиПодвал(СтруктураПараметров, ТабличныйДокумент, Макет)
	...
	Иначе
		...
		СведенияОЮрФизЛице = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(
			СтруктураПараметров.Организация, СтруктураПараметров.КонецПериода);
		Свидетельство = БухгалтерскийУчетПереопределяемый.СвидетельствоОРегистрации(СведенияОЮрФизЛице, СтруктураПараметров.КонецПериода);
	КонецЕсли;
...
Процедура ИнициализироватьТаблицыДанныхНДС(СтруктураПараметров)
	...
	Если СтруктураПараметров.ЗаполнениеДокумента
		...
		Если СтруктураПараметров.ВерсияПостановленияНДС1137 >= 8 Тогда
			Записи.Колонки.Добавить("СтТовРубКоп22",     ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
			Записи.Колонки.Добавить("СумНДСРубКоп22",    ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
			Записи.Колонки.Добавить("СтТовРубКоп18_20",  ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
			Записи.Колонки.Добавить("СумНДСРубКоп18_20", ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
			Записи.Колонки.Добавить("СвСчФПродРанВыст", Новый ОписаниеТипов("Массив"));
		КонецЕсли;
		...
		Иначе
			ОбщиеСведения = Новый Структура(
				"СтТовРубКоп22,СтТовРубКоп20,СтТовРубКоп18,СтТовРубКоп10,СтТовРубКоп0,
				|СумНДСРубКоп22,СумНДСРубКоп20,СумНДСРубКоп18,СумНДСРубКоп10,СтПродОсвНДСРубКоп");
			Если ПрименяетсяПостановление1448 Тогда
			...
			КонецЕсли;
			Если СтруктураПараметров.ВерсияПостановленияНДС1137 >= 8 Тогда
				ОбщиеСведения.Вставить("СтТовРубКоп22",     ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
				ОбщиеСведения.Вставить("СумНДСРубКоп22",    ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
				ОбщиеСведения.Вставить("СтТовРубКоп18_20",  ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
				ОбщиеСведения.Вставить("СумНДСРубКоп18_20", ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
			КонецЕсли
		КонецЕсли;
	...
	Иначе
		...
		Если СтруктураПараметров.ЗаполнениеДекларации
			И СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			Раздел9.Колонки.Добавить("СтоимПродСФ20Или18", ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
			Раздел9.Колонки.Добавить("СтоимПродСФ22",      ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
			Раздел9.Колонки.Добавить("СумНДССФ20Или18",    ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
			Раздел9.Колонки.Добавить("СумНДССФ22",         ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
			Раздел9.Колонки.Добавить("СвЧастСчФ",          Новый ОписаниеТипов("Массив"));
		КонецЕсли;
		...
		Если СтруктураПараметров.ЗаполнениеДекларации
		   И (СтруктураПараметров.ПравилаЗаполнения = "2025Кв1"
		   Или СтруктураПараметров.ПравилаЗаполнения = "2026Кв1") Тогда
		...
		Если СтруктураПараметров.ЗаполнениеДекларации
			...
			ИтогиРаздел9.Вставить("СтПродБезНДС20Или18");
			ИтогиРаздел9.Вставить("СтПродБезНДС22");
			ИтогиРаздел9.Вставить("СумНДСВсКПр20Или18");
			ИтогиРаздел9.Вставить("СумНДСВсКПр22");
			
			ИтогиРаздел91.Вставить("ИтСтПродКПр22");
			ИтогиРаздел91.Вставить("ИтСтПродКПр20Или18");
			ИтогиРаздел91.Вставить("СумНДСИтКПр22");
			ИтогиРаздел91.Вставить("СумНДСИтКПр20Или18");
			ИтогиРаздел91.Вставить("СтПродВсП1Р9_22");
			ИтогиРаздел91.Вставить("СтПродВсП1Р9_20Или18");
			ИтогиРаздел91.Вставить("СумНДСВсП1Р9_22");
			ИтогиРаздел91.Вставить("СумНДСВсП1Р9_20Или18");
			...
			Пока ЗаписьПоСФ.Следующий() Цикл
				...
				Пока РазделениеПоДоговоруДляАвансов.Следующий() Цикл
					...
					Пока ЗаписьКниги.Следующий() Цикл
						...
						ИтогЗаПериод.СуммаБезНДС20Или18 = ИтогЗаПериод.СуммаБезНДС20Или18 + ЗаписьКниги.СуммаБезНДС20Или18;
						ИтогЗаПериод.НДС20Или18         = ИтогЗаПериод.НДС20Или18 + ЗаписьКниги.НДС20Или18;
						ИтогЗаПериод.СуммаБезНДС22      = ИтогЗаПериод.СуммаБезНДС22 + ЗаписьКниги.СуммаБезНДС22;
						ИтогЗаПериод.НДС22              = ИтогЗаПериод.НДС22 + ЗаписьКниги.НДС22;
				...
				Пока ЗаписьКниги.Следующий() Цикл
					...
					Если ЗаписьКниги.НалоговыйАгентПокупательТоваров Тогда
						...
						Пока ДетальнаяЗапись.Следующий() Цикл
							...
							ИтогЗаПериод.СуммаБезНДС20Или18 = ИтогЗаПериод.СуммаБезНДС20Или18 + ДетальнаяЗапись.СуммаБезНДС20Или18;
							ИтогЗаПериод.НДС20Или18         = ИтогЗаПериод.НДС20Или18 + ДетальнаяЗапись.НДС20Или18;
							ИтогЗаПериод.СуммаБезНДС22      = ИтогЗаПериод.СуммаБезНДС22 + ДетальнаяЗапись.СуммаБезНДС22;
							ИтогЗаПериод.НДС22              = ИтогЗаПериод.НДС22 + ДетальнаяЗапись.НДС22;
							ИтогЗаПериод.СуммаСовсемБезНДС  = ИтогЗаПериод.СуммаСовсемБезНДС + ДетальнаяЗапись.СуммаСовсемБезНДС;
							
							Если СтруктураПараметров.ВерсияПостановленияНДС1137 >= 8 Тогда
								ИтогЗаПериод.СуммаБезНДС20 = ИтогЗаПериод.СуммаБезНДС20Или18;
								ИтогЗаПериод.НДС20         = ИтогЗаПериод.НДС20Или18;
							КонецЕсли;
...
Процедура ЗаполнитьСтрокуКнигиПродаж(СтруктураПараметров, Секция, ЗаписьКниги, НомерСтроки, ДополнительнаяИнформация, ДетальнаяЗапись = Неопределено)
	...
	Иначе
		...
		ПараметрНДС22       = ЗаписьКниги.НДС22;
		ПараметрНДС20Или18  = ЗаписьКниги.НДС20Или18;
		ПараметрНДС20       = ЗаписьКниги.НДС20;
		ПараметрНДС18       = ЗаписьКниги.НДС18;
	КонецЕсли;
	СоставСтроки.Вставить("ВсегоПродаж",        ПараметрВсегоПродаж);
	СоставСтроки.Вставить("СуммаБезНДС22",      ЗаписьКниги.СуммаБезНДС22);
	СоставСтроки.Вставить("НДС22",              ПараметрНДС22); 
	СоставСтроки.Вставить("СуммаБезНДС20Или18", ЗаписьКниги.СуммаБезНДС20Или18);
	СоставСтроки.Вставить("НДС20Или18",         ПараметрНДС20Или18);
	...
	АвансовыеСчетаФактурыМассив = Новый Массив();
	СвЧастСчФ = Новый Массив();
	СвСчФПродРанВыст = Новый Массив();
	...
	Если ДетальнаяЗапись = Неопределено Тогда
		...
		// Графа 11а
		ЗаполнитьСведенияОСчетеФактуреНаАванс(
			АвансовыеСчетаФактурыМассив,
			АвансовыеСчетаФактурыТекст,
			ЗаписьКниги,
			СвСчФПродРанВыст,
			СвЧастСчФ);
	...
	СоставСтроки.Вставить("АвансовыеСчетаФактуры", АвансовыеСчетаФактурыТекст);
	...
	Если СтруктураПараметров.ЗаполнениеДекларации Тогда
		...
		СоставСтроки.Вставить("СтоимПродСФ22",      ЗаписьКниги.СуммаБезНДС22);
		СоставСтроки.Вставить("СтоимПродСФ20Или18", ЗаписьКниги.СуммаБезНДС20Или18);
		...
		СоставСтроки.Вставить("СумНДССФ22",         ЗаписьКниги.НДС22);
		СоставСтроки.Вставить("СумНДССФ20Или18",    ЗаписьКниги.НДС20Или18);
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			СоставСтроки.Вставить("СтоимПродСФ20", ЗаписьКниги.СуммаБезНДС20Или18);
			СоставСтроки.Вставить("СумНДССФ20",    ЗаписьКниги.НДС20Или18);
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			СоставСтроки.Вставить("СвЧастСчФ", СвЧастСчФ);
		КонецЕсли;
	...
	ИначеЕсли СтруктураПараметров.ЗаполнениеДокумента
		...
		Если СтруктураПараметров.ВерсияПостановленияНДС1137 >= 8 Тогда
			СоставСтроки.Вставить("СвСчФПродРанВыст",  СвСчФПродРанВыст);
			СоставСтроки.Вставить("СтТовРубКоп22",     ЗаписьКниги.СуммаБезНДС22);
			СоставСтроки.Вставить("СумНДСРубКоп22",    ЗаписьКниги.НДС22);
			СоставСтроки.Вставить("СумНДСРубКоп18_20", ЗаписьКниги.НДС20Или18);
			СоставСтроки.Вставить("СтТовРубКоп18_20",  ЗаписьКниги.СуммаБезНДС20Или18);
		КонецЕсли;
...
Процедура ЗаполнитьСтрокуКнигиПродажСводныйКомиссионный(ЗаписиКниги, СтруктураПараметров, Секция, НомерСтроки)
	...
	Если СтруктураПараметров.ЗаполнениеДекларации Тогда
		...
		СоставСтроки.Вставить("СтоимПродСФ22", 0);
		...
		СоставСтроки.Вставить("СумНДССФ22",    0);
...
Функция ТекстЗапросаЗаписиРегистраНДСПродажи(СтруктураПараметров)
	...
	|	СУММА(ВЫБОР
	|			КОГДА НДСЗаписиКнигиПродаж.СтавкаНДС В (&СтавкиНДС22)
...
Функция ТекстЗапросаПодготовленныеЗаписиКнигиПродаж(СтруктураПараметров)
	...
	|			КОНЕЦ, ДЕНЬ), ДАТАВРЕМЯ(1,1,1)) КАК ДатаСчетаФактурыСортировка,
	|	ЕСТЬNULL(ВЫБОР
	|			КОГДА СчетаФактурыСОсобымПорядкомОтображения.ИсправленныйСчетФактура ЕСТЬ НЕ NULL 
	|				ТОГДА СчетаФактурыСОсобымПорядкомОтображения.ДатаИсправленногоСчетаФактуры
	...
	|	ВЫБОР
	|		КОГДА ЗаписиКнигиПродаж.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПродажи.НалогИсчисляетПокупатель)
	|			ТОГДА ЗаписиКнигиПродаж.СуммаБезНДС22
	|		КОГДА ЗаписиКнигиПродаж.НаАванс
	|				И НЕ ЗаписиКнигиПродаж.НалоговыйАгентПокупательТоваров
	|			ТОГДА 0
	|		ИНАЧЕ ЗаписиКнигиПродаж.СуммаБезНДС22
	|	КОНЕЦ КАК СуммаБезНДС22,
	|	ЗаписиКнигиПродаж.НДС22 КАК НДС22,
	|	ВЫБОР
	|		КОГДА ЗаписиКнигиПродаж.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПродажи.НалогИсчисляетПокупатель)
	...
	|	ЗаписиКнигиПродаж.НомерСчетаФактурыНаАванс КАК НомерСчетаФактурыНаАванс,
	|	ЗаписиКнигиПродаж.ДатаСчетаФактурыНаАванс КАК ДатаСчетаФактурыНаАванс,
	|	ЗаписиКнигиПродаж.НомерИсправленияСчетаФактурыНаАванс КАК НомерИсправленияСчетаФактурыНаАванс,
	|	ЗаписиКнигиПродаж.ДатаИсправленияСчетаФактурыНаАванс КАК ДатаИсправленияСчетаФактурыНаАванс,
...
Функция ТекстЗапросаКнигаПродаж(СтруктураПараметров)
	...
	|	ПодготовленныеЗаписиКнигиПродаж.СуммаБезНДС22 КАК СуммаБезНДС22,
	|	ПодготовленныеЗаписиКнигиПродаж.НДС22 КАК НДС22,
	|	ПодготовленныеЗаписиКнигиПродаж.СуммаБезНДС20 + ПодготовленныеЗаписиКнигиПродаж.СуммаБезНДС18 КАК СуммаБезНДС20Или18,
	|	ПодготовленныеЗаписиКнигиПродаж.НДС20 + ПодготовленныеЗаписиКнигиПродаж.НДС18 КАК НДС20Или18,
	...
	|	ПодготовленныеЗаписиКнигиПродаж.НомерСчетаФактурыНаАванс КАК НомерСчетаФактурыНаАванс,
	|	ПодготовленныеЗаписиКнигиПродаж.ДатаСчетаФактурыНаАванс КАК ДатаСчетаФактурыНаАванс,
	|	ПодготовленныеЗаписиКнигиПродаж.НомерИсправленияСчетаФактурыНаАванс КАК НомерИсправленияСчетаФактурыНаАванс,
	|	ПодготовленныеЗаписиКнигиПродаж.ДатаИсправленияСчетаФактурыНаАванс КАК ДатаИсправленияСчетаФактурыНаАванс,
	...
	|		ПО ПодготовленныеЗаписиКнигиПродаж.СчетФактураДокумент = СчетФактураПолученный.Ссылка
	|			И СчетФактураПолученный.Исправление
	|ГДЕ
	|		НЕ (ЕСТЬNULL(СчетФактураПолученный.Исправление, Ложь)
	|			И НАЧАЛОПЕРИОДА(ПодготовленныеЗаписиКнигиПродаж.ДатаСчетаФактурыСортировка, КВАРТАЛ) <> НАЧАЛОПЕРИОДА(ПодготовленныеЗаписиКнигиПродаж.СчетФактураДата, КВАРТАЛ))
	|
	|УПОРЯДОЧИТЬ ПО
	...
	|	МАКСИМУМ(НомерСчетаФактурыНаАванс),
	|	МАКСИМУМ(ДатаСчетаФактурыНаАванс),
	|	МАКСИМУМ(НомерИсправленияСчетаФактурыНаАванс),
	|	МАКСИМУМ(ДатаИсправленияСчетаФактурыНаАванс),
...
Функция ТекстЗапросаДополнительныйЛистКнигиПродаж(СтруктураПараметров)
	...
	|		ПО ПодготовленныеЗаписиКнигиПродаж.СчетФактураДокумент = СчетФактураПолученный.Ссылка
	|			И СчетФактураПолученный.Исправление
	|			И ПодготовленныеЗаписиКнигиПродаж.Исправление
	|			И ПодготовленныеЗаписиКнигиПродаж.СторнирующаяЗаписьДопЛиста
	|ГДЕ
...
Процедура ЗаполнитьСведенияОСчетеФактуреНаАванс(АвансовыеСчетаФактурыМассив, АвансовыеСчетаФактурыТекст, ЗаписьКниги, СвСчФПродРанВыст, СвЧастСчФ)
	
	Если ЗначениеЗаполнено(ЗаписьКниги.НомерСчетаФактурыНаАванс) Тогда 
		АвансовыеСчетаФактурыНомерДата = СтрШаблон(НСтр("ru='%1 от %2'"),
			ЗаписьКниги.НомерСчетаФактурыНаАванс, Формат(ЗаписьКниги.ДатаСчетаФактурыНаАванс, "ДФ=dd.MM.yyyy"));
		
		Если АвансовыеСчетаФактурыМассив.Найти(АвансовыеСчетаФактурыНомерДата) = Неопределено Тогда
			АвансовыеСчетаФактурыТекст = АвансовыеСчетаФактурыТекст 
				+ ?(Не ЗначениеЗаполнено(АвансовыеСчетаФактурыТекст), "", ";" + Символы.ПС) + АвансовыеСчетаФактурыНомерДата;
			АвансовыеСчетаФактурыМассив.Добавить(АвансовыеСчетаФактурыНомерДата);
			// Структура для выгрузки книги продаж
			СтруктураСвСчФПродРанВыст = Новый Структура("НомСчФПродРанВ,ДатаСчФПродРанВ");
			СтруктураСвСчФПродРанВыст.НомСчФПродРанВ  = ЗаписьКниги.НомерСчетаФактурыНаАванс;
			СтруктураСвСчФПродРанВыст.ДатаСчФПродРанВ = Формат(ЗаписьКниги.ДатаСчетаФактурыНаАванс, "ДФ=dd.MM.yyyy");
			СвСчФПродРанВыст.Добавить(СтруктураСвСчФПродРанВыст);
			// Структура для выгрузки декларации по НДС
			СтруктураСвЧастСчФ = Новый Структура("НомЧастСчФ,ДатаЧастСчФ");
			СтруктураСвЧастСчФ.НомЧастСчФ  = ЗаписьКниги.НомерСчетаФактурыНаАванс;
			СтруктураСвЧастСчФ.ДатаЧастСчФ = Формат(ЗаписьКниги.ДатаСчетаФактурыНаАванс, "ДФ=dd.MM.yyyy");
			СвЧастСчФ.Добавить(СтруктураСвЧастСчФ);
		КонецЕсли;
	КонецЕсли;
...
Функция НоваяСтрокаИтоговКнигиПродаж()
	...
	Результат.Вставить("СуммаБезНДС22",      0);
	Результат.Вставить("СуммаБезНДС20Или18", 0);
	...
	Результат.Вставить("НДС22",              0);
	Результат.Вставить("НДС20Или18",         0);
