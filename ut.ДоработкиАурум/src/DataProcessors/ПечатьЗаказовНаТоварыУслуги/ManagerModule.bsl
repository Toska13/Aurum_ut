...
Процедура АУ_ЗаполнитьТабличныйДокументЗаказаНаТоварыУслугиАдаптированДляПеревода(ТабличныйДокумент, ПараметрыЗаполненияПФ, ОбъектыПечати, ПервыйДокумент, КодЯзыка)
	...
	Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
		...
		УстановитьПараметр(ОбластьНомераСтроки, "НомерСтроки", НомерСтроки);
		#Вставка
		ОбластьНомераСтроки.Параметры.Заполнить(СтрокаТовары);
		#КонецВставки
